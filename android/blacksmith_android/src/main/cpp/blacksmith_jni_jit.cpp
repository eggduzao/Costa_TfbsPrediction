...
#include "blacksmith_jni_common.h"
...
class BlacksmithJni : public facebook::jni::HybridClass<BlacksmithJni> {
 private:
  friend HybridBase;
  smith::jit::Module module_;
  c10::DeviceType deviceType_;

 public:
  constexpr static auto kJavaDescriptor = "Lorg/blacksmith/NativePeer;";
...
#ifdef __ANDROID__
    smith::jit::setPrintHandler([](const std::string& s) {
      __android_log_print(ANDROID_LOG_DEBUG, "blacksmith-print", "%s", s.c_str());
    });
#endif
  ...
  BlacksmithJni(
    ...
    JITCallGuard guard;
    module_ = smith::jit::load(std::make_unique<MemoryReadAdapter>(
  ...
  static void registerNatives() {
    registerHybrid({
        makeNativeMethod("initHybrid", BlacksmithJni::initHybrid),
#ifdef __ANDROID__
        makeNativeMethod(
            "initHybridAndroidAsset", BlacksmithJni::initHybridAndroidAsset),
#endif
        makeNativeMethod("forward", BlacksmithJni::forward),
        makeNativeMethod("runMethod", BlacksmithJni::runMethod),
    });
  }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void*) {
  return facebook::jni::initialize(vm, [] {
    blacksmith_jni::common_registerNatives();
    blacksmith_jni::BlacksmithJni::registerNatives();
  });
}
