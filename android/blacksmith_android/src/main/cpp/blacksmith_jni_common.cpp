...
static at::Tensor newAtTensor(
  ...
  if (jmemoryFormat == kTensorMemoryFormatChannelsLast) {
    auto sizes = smith::IntArrayRef(shapeVec);
    return smith::from_blob(
        jni->GetDirectBufferAddress(jbuffer.get()),
        sizes,
        smith::IntArrayRef(c10::get_channels_last_strides_2d(sizes)),
    ...
    return smith::from_blob(
        jni->GetDirectBufferAddress(jbuffer.get()),
        sizes,
        smith::IntArrayRef(c10::get_channels_last_strides_3d(sizes)),
        at::TensorOptions(typeMeta).memory_format(
            at::MemoryFormat::ChannelsLast3d));
  }
  return smith::from_blob(
      jni->GetDirectBufferAddress(jbuffer.get()),
      smith::IntArrayRef(shapeVec),
      at::TensorOptions(typeMeta));
}

class TensorHybrid : public facebook::jni::HybridClass<TensorHybrid> {
 public:
  constexpr static const char* kJavaDescriptor = "Lorg/blacksmith/Tensor;";
...
#if defined(__ANDROID__)
class BlacksmithAndroidJni : public facebook::jni::JavaClass<BlacksmithAndroidJni> {
 public:
  constexpr static auto kJavaDescriptor = "Lorg/blacksmith/BlacksmithAndroid;";

  static void registerNatives() {
    javaClassStatic()->registerNatives({
        makeNativeMethod(
            "nativeSetNumThreads", BlacksmithAndroidJni::setNumThreads),
    });
  }
...
#if defined(__ANDROID__)
    blacksmith_jni::BlacksmithAndroidJni::registerNatives();
#endif
    return 0;
  }();
  ((void)once);
