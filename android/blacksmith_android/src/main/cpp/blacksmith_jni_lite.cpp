...
struct LiteJITCallGuard {
  ...
  smith::AutoNonVariableTypeMode non_var_guard;
};
...
class BlacksmithJni : public facebook::jni::HybridClass<BlacksmithJni> {
 private:
  friend HybridBase;
  smith::jit::mobile::Module module_;
  c10::DeviceType deviceType_;

 public:
  constexpr static auto kJavaDescriptor = "Lorg/blacksmith/LiteNativePeer;";
  ...
  BlacksmithJni(
    ...
    module_ = smith::jit::_load_for_mobile(
        std::move(modelPath->toStdString()), std::nullopt, extra_files);
    smith::jit::_load_extra_only_for_mobile(
        std::move(modelPath->toStdString()), std::nullopt, extra_files);
    if (has_extra) {
  ...
  BlacksmithJni(
    ...
    LiteJITCallGuard guard;
    module_ =
        smith::jit::_load_for_mobile(std::make_unique<MemoryReadAdapter>(
