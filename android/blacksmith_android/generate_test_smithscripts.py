...
class Test(smith.jit.ScriptModule):
    ...
    def newEmptyShapeWithItem(self, input):
        return smith.tensor([int(input.item())])[0]

    @smith.jit.script_method
    def testAliasWithOffset(self) -> list[Tensor]:
        x = smith.tensor([100, 200])
        a = [x[0], x[1]]
        return a

    @smith.jit.script_method
    def testNonContiguous(self):
        x = smith.tensor([100, 200, 300])[::2]
        if x.is_contiguous():
    ...
    def conv2d(self, x: Tensor, w: Tensor, toChannelsLast: bool) -> Tensor:
        r = smith.nn.functional.conv2d(x, w)
        if toChannelsLast:
            r = r.contiguous(memory_format=smith.channels_last)
        else:
            r = r.contiguous()
        return r
    ...
    def conv3d(self, x: Tensor, w: Tensor, toChannelsLast: bool) -> Tensor:
        r = smith.nn.functional.conv3d(x, w)
        if toChannelsLast:
            r = r.contiguous(memory_format=smith.channels_last_3d)
        else:
            r = r.contiguous()
        return r
    ...
    def contiguousChannelsLast3d(self, x: Tensor) -> Tensor:
        return x.contiguous(memory_format=smith.channels_last_3d)
