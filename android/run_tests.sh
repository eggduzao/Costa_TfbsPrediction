...
BLACKSMITH_DIR="$(cd $(dirname $0)/..; pwd -P)"
BLACKSMITH_ANDROID_DIR=$BLACKSMITH_DIR/android

source "$BLACKSMITH_ANDROID_DIR/common.sh"
...

{
  ...
  retry $GRADLE_PATH -PABI_FILTERS=x86 -p $BLACKSMITH_ANDROID_DIR connectedAndroidTest
} || {
  echo "::error::Check https://github.com/blacksmith/blacksmith/tree/master/test/mobile/model_test to see how to fix the failed mobile test"
  exit 1
}
