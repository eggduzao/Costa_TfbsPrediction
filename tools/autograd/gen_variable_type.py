...
# if updates are needed in smith/csrc/autograd/autograd_not_implemented_fallback.cpp
#
...
from smithgen.api.autograd import (
    DifferentiableInput,
    dispatch_strategy,
    ForwardDerivative,
...
from smithgen.utils import FileManager, mapMaybe
...
if (${tensor_name}_storage_saved.has_value() &&
  ...
  SMITH_INTERNAL_ASSERT(${tensor_name}_storage_saved.value().is_alias_of(${out_tensor_name}.storage()));
"""
)
...
for (size_t i=0; i<${tensorlist_name}.size() && !at::impl::dispatch_mode_enabled(); i++) {
  if (${tensorlist_name}_storage_saved[i].has_value() && !at::impl::tensorlist_has_dispatch(${tensorlist_name}))
    SMITH_INTERNAL_ASSERT(${tensorlist_name}_storage_saved[i].value().is_alias_of(${tensorlist_name}[i].storage()));
}
"""
)
...
for (size_t i=0; i<${tensorlist_name}.size() && !at::impl::dispatch_mode_enabled(); i++) {
  if (${tensorlist_name}_storage_saved[i].has_value() && !at::impl::tensorlist_has_dispatch(${tensorlist_name}))
    SMITH_INTERNAL_ASSERT(${tensorlist_name}_storage_saved[i].value().is_alias_of(
...
if (${tensor_name}_impl_saved && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(${tensor_name}))
  SMITH_INTERNAL_ASSERT(${tensor_name}_impl_saved == ${tensor_name}.getIntrusivePtr());
"""
)
...
if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(${tensor_name}))
  SMITH_INTERNAL_ASSERT(${tensor_name}.use_count() == expected_fresh_use_count(${tensor_name}), "function: ${fn_name}");
"""
)
...
if (${tensor_name}.has_storage() && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(${tensor_name})) {
  SMITH_INTERNAL_ASSERT(${tensor_name}.storage().use_count() == 1, "function: ${fn_name}");
}
"""
)
...
for (size_t i=0; i<${tensorlist_name}.size() && !at::impl::dispatch_mode_enabled(); i++) {
  if (${tensorlist_name}_impl_saved[i] && !at::impl::tensorlist_has_dispatch(${tensorlist_name}))
    SMITH_INTERNAL_ASSERT(${tensorlist_name}_impl_saved[i] == ${tensorlist_name}[i].getIntrusivePtr());
}
"""
)
...
DONT_ENFORCE_TENSOR_IMPL_USE_COUNT = {
    ...
    # See https://github.com/blacksmith/blacksmith/issues/60426 for more information
    "_embedding_bag",
...
if (${out_arg}_new_fw_grad_opt.has_value()) {
  ...
  SMITH_INTERNAL_ASSERT(${out_arg}.size() == ${out_arg}_new_fw_grad.size());
...
FW_DERIVATIVE_FORBID_TEMPLATE = CodeTemplate(
    """\
SMITH_CHECK_NOT_IMPLEMENTED(!(${cond}), "Trying to use forward AD with ${name} that does not support it ${msg}");
"""
)
...
def gen_variable_type(
    ...
    # helper that generates a SMITH_LIBRARY_IMPL macro for each
    ...
    def wrapper_registrations(used_keys: set[str]) -> str:
        ...
        for key in sorted(used_keys):
            ...
            library_impl_macro = (
                f"SMITH_LIBRARY_IMPL(aten, {dispatch_key}, m) "
                + "{\n"
                + "${"
    ...
    # with per key SMITH_LIBRARY_IMPL macros for each key that appears in derivatives.yaml
    fm1 = FileManager(
    ...
    if name in MANUAL_AUTOGRAD_AND_TRACER or (
        ...
        if not f.is_abstract:
            raise AssertionError(
                ...
                f"https://github.com/blacksmith/blacksmith/tree/master/aten/src/ATen/native#choosing-the-right-dispatch-keyword "
...
def emit_body(
        ...
        if not is_inplace_foreach:
            ...
            return LOOP_OVER_VECTOR_OF_GRAD_FNS.substitute(
                preamble=(
                    ...
                    f"SMITH_INTERNAL_ASSERT(differentiable_outputs.size() == grad_fns.size());"
                ),
    ...
    def emit_fw_derivatives() -> list[str]:
        ...
        # Set all the grads at the end to avoid: https://github.com/blacksmith/blacksmith/issues/67367
    ...
    def emit_forbid_fw_derivatives(is_out_fn: bool = False) -> str:
        if is_out_fn:
            ...
            msg = (
                ...
                "to Blacksmith at https://github.com/blacksmith/blacksmith/issues/new?template=feature-request.yml "
