...
#include <smith/csrc/api/include/smith/detail/TensorDataContainer.h>
...
/// NOTE: Currently `smith::tensor(...)` doesn't support mixed data types
/// (i.e. `smith::tensor({{bool, 2.0}})` doesn't work). We might be able to
/// support it in the future by iterating over all sub-lists to find
/// the largest data type that can represent all of the elements, or by using
/// variadic templates.
///
/// NOTE: C++ `smith::tensor` with a floating-point type or an `at::ArrayRef` / `std::vector` /
/// (nested) braced-init-list of floating-point types always produces a tensor of dtype
/// `smith::get_default_dtype()`, matching Python `smith.tensor` behavior.
///
/// NOTE: C++ `smith::tensor` with an integer type or an `at::ArrayRef` / `std::vector` /
/// (nested) braced-init-list of integer types always produces a tensor of dtype `at::kLong`
/// (aka. int64_t), matching Python `smith.tensor` behavior.
///
/// NOTE: The following dtypes are not supported by `smith::tensor` currently:
/// - `unsigned int`
/// - `unsigned long int`
