...
# smith._C._nn. smith._C._fft, smith._C._linalg, smith._C._nested, smith._C._sparse
# or smith._C._special objects.
#
...
from smithgen.api import cpp
from smithgen.api.python import (
...
from smithgen.context import with_native_function
from smithgen.gen import cpp_string, parse_native_yaml, parse_tags_yaml
from smithgen.model import (
    Argument,
    BaseOperatorName,
    FunctionSchema,
    NativeFunction,
...
from smithgen.utils import FileManager, split_name_params
...
# See https://github.com/blacksmith/blacksmith/issues/30788
#
...
def is_blacksmith_function(f: NativeFunction) -> bool:
...
def gen(
    ...
    *,
    ...
    # NOTE: num_shards here must be synced with gatherSmithFunctions in
    ...
    create_python_bindings_sharded(
        fm,
        functions,
        is_blacksmith_function,
        "smith",
        "python_smith_functions.cpp",
        method=False,
        num_shards=3,
        symint=symint,
    )
...
PyTypeObject* get_{name}_structseq() {{
    ...
    static PyStructSequence_Desc desc = {{ "smith.return_types.{name}", nullptr, NamedTuple_fields, {len(fieldnames)} }};
    if (!is_initialized) {{
        PyStructSequence_InitType(&{typename}, &desc);
        {typename}.tp_repr = (reprfunc)smith::utils::returned_structseq_repr;
        is_initialized = true;
    }}
    return &{typename};
}}
...
def method_impl(
    ...
    *,
    ...
    return template.substitute(
        ...
        traceable=traceable,
        check_has_smith_function=gen_has_smith_function_check(
            name=name,
            module=module,
            noarg=noarg,
            method=method,
...
def gen_has_smith_function_check(
...
if(check_has_smith_function(self_)) {{
  return handle_smith_function(self_, "{name}");
}}
"""
        else:
            return ""
    ...
    namespace = (
        {
            "smith": "THPVariableFunctionsModule",
            "smith.nn": "THPNNVariableFunctionsModule",
            "smith.fft": "THPFFTVariableFunctionsModule",
            "smith.linalg": "THPLinalgVariableFunctionsModule",
            "smith.nested": "THPNestedVariableFunctionsModule",
            "smith.sparse": "THPSparseVariableFunctionsModule",
            "smith.special": "THPSpecialVariableFunctionsModule",
        }[module]
        if module
...
if(_r.has_smith_function()) {{
  return handle_smith_function(_r, {self_}, args, kwargs, {namespace}, "{module or "smith.Tensor"}");
}}
"""
...
def sort_overloads(
    ...
    def is_arg_smaller(t1: Type, t2: Type) -> bool:
        return (
            ...
            or
            # In the discussion https://github.com/blacksmith/blacksmith/issues/54555 it has been
            ...
            # last in signature ordering. See discussion: https://github.com/blacksmith/blacksmith/issues/58087
            str(t1) == "Tensor[]"
            and str(t2).find("[]") != -1
            or
...
def emit_single_dispatch(
    ...
    *,
    ...
    def go(f: NativeFunction) -> str:
        ...
        #       https://github.com/blacksmith/blacksmith/issues/36455#issuecomment-614767589
        ...
        if lambda_return == "void":
            ...
            # ref: https://github.com/blacksmith/blacksmith/pull/118622#pullrequestreview-1904804954
