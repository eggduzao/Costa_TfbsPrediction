...
# a list of `Derivative`. See `smithgen.api.autograd` for the data models.
...
from smithgen.utils import concatMap, IDENT_REGEX, split_name_params
...
def load_derivatives(
    ...
    if key not in _GLOBAL_LOAD_DERIVATIVE_CACHE:
        ...
        # VariableType.cpp where we need a SMITH_LIBRARY_IMPL for every autograd dispatch key used
...
def create_differentiability_info(
    ...
    if len(functions) == 0:
        ...
        raise RuntimeError(
            ...
            f"corresponding to schema {specification}.  Please report a bug to Blacksmith. "
            f"Available signatures:\n{avail}"
        )
