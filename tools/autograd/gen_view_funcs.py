...
from smithgen.model import Argument, NativeFunction, OptionalType
...
if TYPE_CHECKING:
    from smithgen.api.autograd import NativeFunctionWithDifferentiabilityInfo
...
void ${op}::set_symints(std::vector<c10::SymInt> ${symints_vec}) {
  SMITH_INTERNAL_ASSERT(${symints_vec}.size() == num_symints());
  ${set_symints}
}
...
void ${op}::set_tensors(std::vector<at::Tensor> ${tensors_vec}) {
  SMITH_INTERNAL_ASSERT(${tensors_vec}.size() == num_tensors());
  ${set_tensors}
}
...
def view_func_name(
    ...
    namespace = "smith::autograd::generated::" if include_namespace else ""
...
def process_function(fn: NativeFunction, template: CodeTemplate) -> str:
    ...
    return template.substitute(
        ...
        superclass="smith::autograd::ViewFunc",
