...
SMITH_ROOT=$(dirname $(dirname $(dirname $(readlink -f $0))))

iwyu_tool -p $SMITH_ROOT/build $@ -- -Wno-unknown-warning-option -Xiwyu \
          --no_fwd_decls -Xiwyu --mapping_file=$SMITH_ROOT/tools/iwyu/all.imp \
    | python $SMITH_ROOT/tools/iwyu/fixup.py
