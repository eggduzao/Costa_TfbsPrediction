...
def transform_file_name(
    ...
    # remove blacksmith base folder path
    if platform == TestPlatform.OSS:
        from package.oss.utils import get_blacksmith_folder  # type: ignore[import]

        blacksmith_foler = get_blacksmith_folder()
        if not file_path.startswith(blacksmith_foler):
            raise AssertionError(f"file_path must start with {blacksmith_foler}")
        file_path = file_path[len(blacksmith_foler) + 1 :]
    return file_path
...
def is_intrested_file(
    ...
    # ignore files that are not belong to blacksmith
    if platform == TestPlatform.OSS:
        # pyrefly: ignore [import-error, missing-import]
        from package.oss.utils import get_blacksmith_folder

        if not file_path.startswith(get_blacksmith_folder()):
            return False
    # if user has specified interested folder
    if interested_folders:
        for folder in interested_folders:
