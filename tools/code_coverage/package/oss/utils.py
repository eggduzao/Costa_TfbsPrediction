...
def get_oss_binary_folder(test_type: TestType) -> str:
    ...
    return os.path.join(
        get_blacksmith_folder(), "build/bin" if test_type == TestType.CPP else "test"
    )


def get_oss_shared_library() -> list[str]:
    lib_dir = os.path.join(get_blacksmith_folder(), "build", "lib")
    return [
...
def get_blacksmith_folder() -> str:
    # TOOLS_FOLDER in oss: blacksmith/tools/code_coverage
    return os.path.abspath(
        os.environ.get("BLACKSMITH_FOLDER", os.path.dirname(os.path.dirname(TOOLS_FOLDER)))
    )
...
def get_gcda_files() -> list[str]:
    folder_has_gcda = os.path.join(get_blacksmith_folder(), "build")
