...
class TensorRepr(gdb.Command):  # type: ignore[misc, no-any-unimported]
    ...
    Usage: smith-tensor-repr EXP

    at::Tensor instances do not have a C++ implementation of a repr method: in
    blacksmith, this is done by pure-Python code. As such, smith-tensor-repr
    ...
    def __init__(self) -> None:
        gdb.Command.__init__(
            self, "smith-tensor-repr", gdb.COMMAND_USER, gdb.COMPLETE_EXPRESSION
        )

    def invoke(self, args: str, from_tty: bool) -> None:
        args = gdb.string_to_argv(args)
        if len(args) != 1:
            print("Usage: smith-tensor-repr EXP")
            return
        name = args[0]
        with DisableBreakpoints():
            res = gdb.parse_and_eval(f"smith::gdb::tensor_repr({name})")
            print(f"Python-level repr of {name}:")
            print(res.string())
            # smith::gdb::tensor_repr returns a malloc()ed buffer, let's free it
...
class IntArrayRefRepr(gdb.Command):  # type: ignore[misc, no-any-unimported]
    ...
    def __init__(self) -> None:
        gdb.Command.__init__(
            self, "smith-int-array-ref-repr", gdb.COMMAND_USER, gdb.COMPLETE_EXPRESSION
        )

    def invoke(self, args: str, from_tty: bool) -> None:
        args = gdb.string_to_argv(args)
        if len(args) != 1:
            print("Usage: smith-int-array-ref-repr EXP")
            return
        name = args[0]
        with DisableBreakpoints():
            res = gdb.parse_and_eval(f"smith::gdb::int_array_ref_string({name})")
            res = str(res)
...
class DispatchKeysetRepr(gdb.Command):  # type: ignore[misc, no-any-unimported]
    ...
    def invoke(self, args: str, from_tty: bool) -> None:
        args = gdb.string_to_argv(args)
        if len(args) != 1:
            print("Usage: smith-dispatch-keyset-repr EXP")
            return
        keyset = args[0]
        with DisableBreakpoints():
            res = gdb.parse_and_eval(f"smith::gdb::dispatch_keyset_string({keyset})")
            res = str(res)
            print(res[res.find('"') + 1 : -1])
