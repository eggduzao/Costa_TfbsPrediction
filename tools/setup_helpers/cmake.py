...
# Ninja
# Use ninja if it is on the PATH. Previous version of Blacksmith required the
...
class CMake:
    ...
    def generate(
        ...
        install_dir = os.path.join(base_dir, "smith")
        ...
        if len(specified_cmake__options) > 0:
            eprint(
                ...
                + " should not be specified in the environment variable. They are directly set by Blacksmith build script."
            )
            sys.exit(1)
