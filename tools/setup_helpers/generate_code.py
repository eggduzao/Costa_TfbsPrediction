...
def generate_code(
    ...
    if install_dir is None:
        install_dir = os.fspath(gen_dir / "smith/csrc")
        python_install_dir = os.fspath(gen_dir / "smith/testing/_internal/generated")
    else:
    ...
    if subset == "libsmith" or not subset:
        gen_autograd(
            native_functions_path or NATIVE_FUNCTIONS_PATH,
            tags_path or TAGS_PATH,
            autograd_gen_dir,
...
def main() -> None:
    ...
    parser.add_argument(
        "--subset",
        help='Subset of source files to generate. Can be "libsmith" or "pybindings". Generates both when omitted.',
    )
    parser.add_argument(
        "--disable-autograd",
        default=False,
    ...
    parser.add_argument(
        ...
        "--gen_lazy_ts_backend",
        action="store_true",
        help="Enable generation of the smith::lazy SmithScript backend",
    )
    parser.add_argument(
    ...
    install_dir = options.install_dir or os.fspath(options.gen_dir / "smith/csrc")
    ...
    if options.gen_lazy_ts_backend:
        ...
        ts_native_functions = "smith/csrc/lazy/ts_backend/ts_native_functions.cpp"
        ts_node_base = "smith/csrc/lazy/ts_backend/ts_node.h"
        ...
        from smithgen.dest.lazy_ir import GenTSLazyIR
        from smithgen.gen_lazy_tensor import run_gen_lazy_tensor

        run_gen_lazy_tensor(
            aten_path=aten_path,
            source_yaml=ts_backend_yaml,
            backend_name="SmithScript",
