...
sys.path.append(str(REPO_ROOT / "smith" / "utils"))
...
includes = [
    ...
    "third_party/nvfuser/*",
    # Blacksmith paths
    # Keep this synchronized with is_blacksmith_file in hipify_python.py
    "aten/src/ATen/cuda/*",
    ...
    "aten/src/ATen/native/transformers/cuda/mem_eff_attention/blacksmith_utils.h",
    "aten/src/THC/*",
    "aten/src/ATen/test/*",
...
ignores = [
    ...
    "smith/csrc/jit/codegen/fuser/cuda/resource_strings.h",
    "smith/csrc/jit/tensorexpr/ir_printer.cpp",
    "smith/csrc/jit/ir/ir.h",
    # generated files we shouldn't frob
    "smith/lib/tmp_install/*",
    "smith/include/*",
]
...
# MSLK is its own project with its own build system. blacksmith uses mslk as
...
# slightly different hipify settings than blacksmith. mslk normally hipifies
# and renames tuning_cache.cuh to tuning_cache_hip.cuh, but blacksmith's settings
...
extra_files = [
    "smith/_inductor/codegen/cuda/device_op_overrides.py",
    "smith/_inductor/codegen/cpp_wrapper_cpu.py",
    "smith/_inductor/codegen/cpp_wrapper_gpu.py",
    "smith/_inductor/codegen/wrapper.py",
]
