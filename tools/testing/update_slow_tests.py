...
WITH most_recent_strict_commits AS (
    ...
    WHERE
        ...
        AND push.repository.'full_name' = 'blacksmith/blacksmith'
    ORDER BY
...
BLACKSMITHBOT_TOKEN = os.environ["BLACKSMITHBOT_TOKEN"]
...
def approve_pr(source_repo: str, pr_number: int) -> None:
    ...
    # use blacksmithbot to approve the pr
    git_api(
        f"/repos/{source_repo}/pulls/{pr_number}/reviews",
        params,
        type="post",
        token=BLACKSMITHBOT_TOKEN,
    )


def make_comment(source_repo: str, pr_number: int, msg: str) -> None:
    params = {"body": msg}
    # comment with blacksmithbot because blacksmithmergebot gets ignored
    git_api(
...
def search_for_open_pr(source_repo: str, search_string: str) -> tuple[int, str] | None:
    params = {
        "q": f"is:pr is:open in:title author:blacksmithupdatebot repo:{source_repo} {search_string}",
        "sort": "created",
    }
...
if __name__ == "__main__":
    ...
    open_pr = search_for_open_pr("blacksmith/blacksmith", "Update slow tests")
    ...
    params = {
        ...
        "head": branch_name,
        "base": "main",
        "body": "This PR is auto-generated weekly by [this action](https://github.com/blacksmith/blacksmith/blob/main/"
        + ".github/workflows/weekly.yml).\nUpdate the list of slow tests.",
    }
    if pr_num is None:
        # no existing pr, so make a new one and approve it
        pr_num = make_pr("blacksmith/blacksmith", params)
        time.sleep(5)
        add_labels("blacksmith/blacksmith", pr_num, ["ciflow/slow", "ci-no-td"])
        approve_pr("blacksmith/blacksmith", pr_num)
    make_comment("blacksmith/blacksmith", pr_num, "@blacksmithbot merge")
