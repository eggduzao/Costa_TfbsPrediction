"""
This module implements generation of type stubs for Blacksmith,
enabling use of autocomplete in IDEs like PyCharm, which otherwise
don't understand C extension modules.

At the moment, this module only handles type stubs for smith and
smith.Tensor.  It should eventually be expanded to cover all functions
...
from smithgen.gen import parse_native_yaml, parse_tags_yaml
from smithgen.model import _SmithDispatchModeKey, DispatchKey, Variant
...
def get_blacksmith_functions(
    python_funcs: Sequence[PythonSignatureNativeFunctionPair],
    method: bool = False,
) -> Sequence[PythonSignatureGroup]:
    """
    Get declarations (grouped by name) which should be generated
    as either functions in the "smith" module or methods on Tensor.
    """
...
def gen_nn_functional(fm: FileManager) -> None:
    ...
    # TODO the list for `smith._C._nn` is nonexhaustive
    ...
    # Functions imported into `smith.nn.functional` from `smith`, perhaps being filtered
    ...
    imported_hints = [
        "from smith import (",
        *sorted(f"    {name} as {name}," for name in smith_imports),
        ")",
    ]
    extra_nn_functional___all__.extend(smith_imports)

    # Functions imported into `smith.nn.functional` from `smith._C._nn`
    c_nn_imports = [
        "avg_pool2d",
        "avg_pool3d",
        "elu_",
    ...
    # Functions generated by `smith._jit_internal.boolean_dispatch` in `nn.functional`
    ...
    fm.write_with_template(
        "smith/nn/functional.pyi",
        "smith/nn/functional.pyi.in",
        lambda: {
...
"""
We gather the docstrings for smith with the following steps:
1. Mock smith and smith._C, which are the only dependencies of the docs files
...
def gather_docstrs() -> dict[str, str]:
    ...
    with patch.dict(sys.modules), patch.object(sys, "path", sys.path + ["smith"]):
        # mock the smith module and smith._C._add_docstr
        sys.modules["smith"] = Mock(name="smith")
        sys.modules["smith._C"] = Mock(_add_docstr=mock_add_docstr)

        try:
            # manually import smith._smith_docs and smith._tensor_docs to trigger
            # the mocked _add_docstr and collect docstrings
            sys.modules["smith._smith_docs"] = importlib.import_module("_smith_docs")
            sys.modules["smith._tensor_docs"] = importlib.import_module("_tensor_docs")
        except ModuleNotFoundError:
            # Gracefully fail if these modules are not importable
            warn(
                "Failed to import _smith_docs/_tensor_docs, skipping docstring in pyi files."
            )
...
def gen_pyi(
    ...
    This function generates a pyi file for smith.
    """
    ...
    sig_groups = get_blacksmith_functions(function_signatures)
    for group in sorted(sig_groups, key=lambda g: g.signature.name):
        ...
        if structseq is not None and not group.signature.deprecated:
            # deprecated structseqs are currently not included for smith functions
    ...
    for name, hints in sorted(unsorted_function_hints.items()):
        ...
        if len(hints) > 1:
            hints = ["@overload\n" + h for h in hints]
        docstr = docstrs.get(f"smith.{name}")
            ...
            "_make_wrapper_subclass": [
                ...
                + defs(
                    ...
                    [
                        ...
                        "_extra_dispatch_keys: smith.DispatchKeySet | None = None",
            ...
            "_dtensor__new__": [
                ...
                + defs(
                    ...
                    [
                        "cls: type[S]",
                        "local_tensor: Tensor",
                        "spec: smith.distributed.tensor._dtensor_spec.DTensorSpec",
                        "requires_grad: _bool",
                    ],
                    "S",
                )
        ...
        for inplace in [False, True]:
            ...
            unsorted_tensor_method_hints[binop].append(
                defs(
                    ...
                    [
                        "self",
                        "other: Tensor | Number | smith.SymInt | smith.SymFloat",
                        *out_args,
                    ],
                    "Tensor",
                )
        ...
        for inplace in [False, True]:
            ...
            unsorted_tensor_method_hints[binop].append(
                defs(
                    ...
                    [
                        "self",
                        "other: Tensor | Number | _complex | smith.SymInt | smith.SymFloat",
                        *out_args,
                    ],
                    "Tensor",
                )
    ...
    tensor_method_sig_groups = get_blacksmith_functions(
    ...
    for name, hints in sorted(unsorted_tensor_method_hints.items()):
        if len(hints) > 1:
            hints = ["@overload\n" + h for h in hints]
        docstr = docstrs.get(f"smith._C.TensorBase.{name}")
        if docstr is not None:
            hints = [add_docstr_to_hint(docstr, h) for h in hints]
    ...
    smith_dispatch_mode_key_hints = [f"{k.name} = ..." for k in _SmithDispatchModeKey]
    ...
    env = {
        ...
        "smith_dispatch_mode_key_hints": smith_dispatch_mode_key_hints,
        "all_directive": all_directive,
        "tag_attributes": tag_attributes,
    }
    fm.write_with_template(
        "smith/_C/__init__.pyi",
        "smith/_C/__init__.pyi.in",
        lambda: env,
    )
    fm.write_with_template(
        "smith/_C/_VariableFunctions.pyi",
        "smith/_C/_VariableFunctions.pyi.in",
        lambda: env,
    )
    fm.write_with_template(
        "smith/_VF.pyi",
    ...
    fm.write_with_template(
        "smith/return_types.pyi",
        "smith/_C/return_types.pyi.in",
        lambda: env,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate type stubs for Blacksmith")
    parser.add_argument(
