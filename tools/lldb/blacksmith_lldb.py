...
def IntArrayRef_summary(valobj: Any, internal_dict: Any, options: Any) -> str:
    ...
    with DisableBreakpoints():
        ...
        result = target.EvaluateExpression(
            f"smith::gdb::int_array_ref_string({tensor})"
        )
...
def DispatchKeyset_summary(valobj: Any, internal_dict: Any, options: Any) -> str:
    ...
    with DisableBreakpoints():
        ...
        result = target.EvaluateExpression(
            f"smith::gdb::dispatch_keyset_string({keyset})"
        )
        str_result = str(result)
        str_result = str_result[str_result.find('"') + 1 : -1]
        return str_result
...
def Tensor_summary(valobj: Any, internal_dict: Any, options: Any) -> str:
    ...
    blacksmith, this is done by pure-Python code. As such, print <tensor>
    ...
    with DisableBreakpoints():
        target = get_target()
        tensor = valobj.GetName()
        result = target.EvaluateExpression(f"smith::gdb::tensor_repr({tensor})")
        str_result = str(result)
...
def __lldb_init_module(debugger: Any, internal_dict: Any) -> Any:
    debugger.HandleCommand(
        "type summary add c10::IntArrayRef -F blacksmith_lldb.IntArrayRef_summary -w smith"
    )
    debugger.HandleCommand(
        "type summary add c10::DispatchKeySet -F blacksmith_lldb.DispatchKeyset_summary -w smith"
    )
    debugger.HandleCommand(
        "type summary add at::Tensor -F blacksmith_lldb.Tensor_summary -w smith"
    )
    print(
        "Pretty Printing lldb summary for Blacksmith AT types has been installed and is ready for use. "
        "This category is enabled by default. To disable run: `type category disable smith`"
    )
    print(
