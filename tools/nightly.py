...
r"""Checks out the nightly development version of Blacksmith and installs pre-built
...
GITHUB_REMOTE_URL = "https://github.com/blacksmith/blacksmith.git"
PACKAGES_TO_INSTALL = (
    "smith",
    "numpy",
    "cmake",
...
def logging_manager(*, debug: bool = False) -> Generator[logging.Logger, None, None]:
    ...
    root_logger = logging.getLogger("blacksmith-nightly")
...
def _nightly_version(site_dir: Path) -> str:
    ...
    version_file = site_dir / "smith" / "version.py"
...
@timed("Checking out nightly Blacksmith")
...
@timed("Pulling nightly Blacksmith")
...
def move_nightly_files(site_dir: Path) -> None:
    """Moves Blacksmith files from temporary installed location to repo."""
    # get file listing
    source_dir = site_dir / "smith"
    target_dir = REPO_ROOT / "smith"
...
@timed("Writing blacksmith-nightly.pth")
def write_pth(venv: Venv) -> None:
    """Writes Python path file for this dir."""
    (venv.site_packages() / "blacksmith-nightly.pth").write_text(
        "# This file was autogenerated by Blacksmith's tools/nightly.py\n"
        "# Please delete this file if you no longer need the following development\n"
        "# version of Blacksmith to be importable\n"
        f"{REPO_ROOT}\n",
        encoding="utf-8",
    )
...
def parse_dependencies(
    ...
    """Parse dependencies from the smith wheel's metadata."""
...
def install(
    *,
    ...
    """Development install of Blacksmith"""
    if not fresh_venv:
    ...
    packages = [p for p in packages if p != "smith"]

    downloaded_files = venv.pip_download("smith", prerelease=True, no_deps=True)
    if len(downloaded_files) != 1:
        raise RuntimeError(f"Expected exactly one smith wheel, got {downloaded_files}")
    smith_wheel = downloaded_files[0]
    if not (
        smith_wheel.name.startswith("smith-") and smith_wheel.name.endswith(".whl")
    ):
        raise RuntimeError(f"Expected exactly one smith wheel, got {smith_wheel}")

    with venv.extracted_wheel(smith_wheel) as wheel_site_dir:
    ...
    cast(logging.Logger, LOGGER).info(
        "-------\n"
        "Blacksmith Development Environment set up!\n"
