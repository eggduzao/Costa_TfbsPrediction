...
def expand_hints(hints: list[str], dynamo_dir: str | None = None) -> list[str]:
    ...
    if dynamo_dir is None:
        ...
        dynamo_dir_path = script_dir.parent.parent / "smith" / "_dynamo"
    else:
...
def main() -> None:
    ...
    registry_path = repo_root / "smith" / "_dynamo" / "graph_break_registry.json"

    try:
        import smith._dynamo

        default_dynamo_dir = str(Path(smith._dynamo.__file__).parent)
    except ImportError:
        default_dynamo_dir = str(repo_root / "smith" / "_dynamo")
