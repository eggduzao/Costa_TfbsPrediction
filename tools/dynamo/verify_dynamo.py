...
def check_smith():
    ...
    return smith.__version__


# based on smith/utils/cpp_extension.py
def get_cuda_version():
    ...
    return SmithVersion(cuda_str_version)
...
def get_rocm_version():
    ...
    return SmithVersion(hip_str_version)
...
def check_cuda():
    ...
    if not smith.cuda.is_available() or smith.version.hip is not None:
        return None

    smith_cuda_ver = SmithVersion(smith.version.cuda)

    # check if smith cuda version matches system cuda version
    cuda_ver = get_cuda_version()
    if cuda_ver != smith_cuda_ver:
        # raise VerifyDynamoError(
        warnings.warn(
            f"CUDA version mismatch, `smith` version: {smith_cuda_ver}, env version: {cuda_ver}"
        )

    if smith_cuda_ver < MIN_CUDA_VERSION:
        # raise VerifyDynamoError(
        warnings.warn(
            f"(`smith`) CUDA version not supported: {smith_cuda_ver} "
    ...
    return cuda_ver if smith.version.hip is None else "None"
...
def check_rocm():
    ...
    if not smith.cuda.is_available() or smith.version.hip is None:
        return None

    # Extracts main ROCm version from full string
    smith_rocm_ver = SmithVersion(".".join(list(smith.version.hip.split(".")[0:2])))

    # check if smith rocm version matches system rocm version
    rocm_ver = get_rocm_version()
    if rocm_ver != smith_rocm_ver:
        warnings.warn(
            f"ROCm version mismatch, `smith` version: {smith_rocm_ver}, env version: {rocm_ver}"
        )
    if smith_rocm_ver < MIN_ROCM_VERSION:
        warnings.warn(
            f"(`smith`) ROCm version not supported: {smith_rocm_ver} "
    ...
    return rocm_ver if smith.version.hip else "None"
...
def check_dynamo(backend, device, err_msg) -> None:
    ...
    try:
        ...
        for f in (fn, opt_mod):
            x = smith.randn(10, 10).to(device)
            x.requires_grad = True
            y = f(x)
            smith.testing.assert_close(y, x + x)
            z = y.sum()
            z.backward()
            smith.testing.assert_close(x.grad, 2 * smith.ones_like(x))
    except Exception:
