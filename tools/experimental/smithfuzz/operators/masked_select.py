...
class MaskedSelectOperator(Operator):
    ...
    def codegen(
        ...
        return (
            f"_x_ms = smith.arange(max({k}, 1), device={input_names[0]}.device).to({input_names[0]}.dtype)\n"
            f"_mask_ms = smith.zeros_like(_x_ms, dtype=smith.bool)\n"
            f"_mask_ms[:{k}] = True\n"
            f"{output_name} = smith.masked_select(_x_ms, _mask_ms)"
        )
