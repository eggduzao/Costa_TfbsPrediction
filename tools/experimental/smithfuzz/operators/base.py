...
from smithfuzz.tensor_fuzzer import Spec


class Operator(ABC):
    """Base class for all operators in smithfuzz."""
    ...
    def smith_op_name(self) -> str | None:
        """
        Return the smith operation name this operator represents.

        Returns:
            Optional[str]: The smith operation name (e.g., "smith.ops.aten.add", "smith.nonzero").
                          Returns None for non-smith operations like "arg" and "constant".
        """
        raise NotImplementedError("Subclasses must implement smith_op_name")
