...
class IndexSelectOperator(Operator):
    ...
    def fuzz_inputs_specs(self, output_spec: Spec, num_inputs: int = 3) -> list[Spec]:
        ...
        smith.index_select(input, dim, index) returns a tensor with:
    ...
    def codegen(
        ...
        return (
            ...
            f"_index_{output_name} = smith.randint(0, _input_size_{output_name}, ({num_indices},), device={input_names[0]}.device)\n"
            f"{output_name} = smith.index_select({input_names[0]}, {dim}, _index_{output_name})"
        )
