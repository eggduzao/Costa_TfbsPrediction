...
from smithfuzz.operators.masked_select import MaskedSelectOperator
...
from smithfuzz.operators.nonzero import NonzeroOperator
...
from smithfuzz.operators.unique import UniqueOperator
...
def set_operator_weight(op_name: str, weight: float) -> None:
    ...
    Args:
        op_name: The registered operator name (e.g., "add", "arg") OR fully-qualified smith op
                 (e.g., "smith.nn.functional.relu", "smith.matmul")
    ...
    # Fallback: try to locate by fully-qualified smith op name
    for candidate in _global_registry.list_operators().values():
        if getattr(candidate, "smith_op_name", None) == op_name:
            candidate.weight = float(weight)
            return

    raise KeyError(f"Operator '{op_name}' not found by registry name or smith op name")
...
def set_operator_weight_by_smith_op(smith_op_name: str, weight: float) -> None:
    """Set operator weight by fully-qualified smith op name."""
    if weight <= 0:
        raise ValueError("Operator weight must be > 0")
    for candidate in _global_registry.list_operators().values():
        if getattr(candidate, "smith_op_name", None) == smith_op_name:
            candidate.weight = float(weight)
            return
    raise KeyError(f"Smith op '{smith_op_name}' not found in registry")


def set_operator_weights_by_smith_op(weights: dict[str, float]) -> None:
    """Bulk-update weights by fully-qualified smith op names."""
    for name, w in weights.items():
        set_operator_weight_by_smith_op(name, w)
