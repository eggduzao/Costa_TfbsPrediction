...
class EmbeddingOperator(Operator):
    """Operator for smith.nn.functional.embedding."""

    def __init__(self):
        super().__init__("smith.nn.functional.embedding")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.embedding"
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        input_spec = TensorSpec(
            ...
            dtype=smith.int64,  # Indices are typically int64
        )
    ...
    def codegen(
        ...
        return f"{output_name} = smith.nn.functional.embedding(smith.clamp({input_name}.to(smith.int64), 0, {weight_name}.size(0)-1), {weight_name})"


class LinearOperator(Operator):
    """Operator for smith.nn.functional.linear."""

    def __init__(self):
        super().__init__("smith.nn.functional.linear")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.linear"
    ...
    def codegen(
        ...
        if len(input_names) == 2:
            ...
            return f"{output_name} = smith.nn.functional.linear({input_name}.to({target_dtype}), {weight_name}.to({target_dtype}))"
        elif len(input_names) == 3:
            input_name, weight_name, bias_name = input_names
            return f"{output_name} = smith.nn.functional.linear({input_name}.to({target_dtype}), {weight_name}.to({target_dtype}), {bias_name}.to({target_dtype}))"
        else:
            raise ValueError(
...
class ReLUOperator(Operator):
    """Operator for smith.nn.functional.relu."""

    def __init__(self):
        super().__init__("smith.nn.functional.relu")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.relu"
    ...
    def codegen(
        ...
        return f"{output_name} = smith.nn.functional.relu({input_name})"


class SoftmaxOperator(Operator):
    """Operator for smith.nn.functional.softmax."""

    def __init__(self):
        super().__init__("smith.nn.functional.softmax")
    ...
    def codegen(
        ...
        return f"{output_name} = smith.nn.functional.softmax({input_name}, dim=-1)"


class DropoutOperator(Operator):
    """Operator for smith.nn.functional.dropout."""

    def __init__(self):
        super().__init__("smith.nn.functional.dropout")
    ...
    def codegen(
        ...
        return f"{output_name} = smith.nn.functional.dropout({input_name}, p=0.1, training=False)"


class LayerNormOperator(Operator):
    """Operator for smith.nn.functional.layer_norm."""

    def __init__(self):
        super().__init__("smith.nn.functional.layer_norm")
    ...
    def codegen(
        ...
        if len(input_names) == 1:
            return f"{output_name} = smith.nn.functional.layer_norm({input_name}.to({target_dtype}), {normalized_shape})"
        elif len(input_names) == 2:
            weight_name = input_names[1]
            return f"{output_name} = smith.nn.functional.layer_norm({input_name}.to({target_dtype}), {normalized_shape}, weight={weight_name}.to({target_dtype}))"
        else:  # len(input_names) == 3
            weight_name, bias_name = input_names[1], input_names[2]
            return f"{output_name} = smith.nn.functional.layer_norm({input_name}.to({target_dtype}), {normalized_shape}, weight={weight_name}.to({target_dtype}), bias={bias_name}.to({target_dtype}))"


class RMSNormOperator(Operator):
    """Operator for smith.nn.functional.rms_norm (Root Mean Square Normalization).

    RMSNorm is commonly used in modern LLMs like LLaMA. It normalizes by the RMS of the input.
    """

    def __init__(self):
        super().__init__("smith.nn.functional.rms_norm")
        self.weight = 5.0
    ...
    def codegen(
        ...
        if len(input_names) == 1:
            return f"{output_name} = smith.nn.functional.rms_norm({input_name}.to({target_dtype}), {normalized_shape})"
        else:  # len(input_names) == 2
            weight_name = input_names[1]
            return f"{output_name} = smith.nn.functional.rms_norm({input_name}.to({target_dtype}), {normalized_shape}, weight={weight_name}.to({target_dtype}))"


class GELUOperator(Operator):
    """Operator for smith.nn.functional.gelu (Gaussian Error Linear Unit)."""

    def __init__(self):
        super().__init__("smith.nn.functional.gelu")
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.nn.functional.gelu({input_name})"


class SigmoidOperator(Operator):
    """Operator for smith.sigmoid."""

    def __init__(self):
        super().__init__("smith.sigmoid")
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.sigmoid({input_name})"


class TanhOperator(Operator):
    """Operator for smith.tanh."""

    def __init__(self):
        super().__init__("smith.tanh")
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.tanh({input_name})"


class BatchNormOperator(Operator):
    """Operator for smith.nn.functional.batch_norm."""

    def __init__(self):
        super().__init__("smith.nn.functional.batch_norm")
    ...
    def codegen(
        ...
        if len(input_names) == 3:
            return f"{output_name} = smith.nn.functional.batch_norm({input_name}.to({target_dtype}), {running_mean_name}.to({target_dtype}), {running_var_name}.to({target_dtype}), training=False)"
        elif len(input_names) == 4:
            weight_name = input_names[3]
            return f"{output_name} = smith.nn.functional.batch_norm({input_name}.to({target_dtype}), {running_mean_name}.to({target_dtype}), {running_var_name}.to({target_dtype}), weight={weight_name}.to({target_dtype}), training=False)"
        else:  # len(input_names) == 5
            weight_name = input_names[3]
            bias_name = input_names[4]
            return f"{output_name} = smith.nn.functional.batch_norm({input_name}.to({target_dtype}), {running_mean_name}.to({target_dtype}), {running_var_name}.to({target_dtype}), weight={weight_name}.to({target_dtype}), bias={bias_name}.to({target_dtype}), training=False)"


class GroupNormOperator(Operator):
    """Operator for smith.nn.functional.group_norm."""

    def __init__(self):
        super().__init__("smith.nn.functional.group_norm")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.group_norm"
    ...
    def codegen(
        ...
        if len(input_names) == 1:
            return f"{output_name} = smith.nn.functional.group_norm({input_name}.to({target_dtype}), {num_groups})"
        elif len(input_names) == 2:
            weight_name = input_names[1]
            return f"{output_name} = smith.nn.functional.group_norm({input_name}.to({target_dtype}), {num_groups}, weight={weight_name}.to({target_dtype}))"
        else:  # len(input_names) == 3
            weight_name = input_names[1]
            bias_name = input_names[2]
            return f"{output_name} = smith.nn.functional.group_norm({input_name}.to({target_dtype}), {num_groups}, weight={weight_name}.to({target_dtype}), bias={bias_name}.to({target_dtype}))"


class LeakyReLUOperator(Operator):
    """Operator for smith.nn.functional.leaky_relu."""

    def __init__(self):
        super().__init__("smith.nn.functional.leaky_relu")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.leaky_relu"
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.nn.functional.leaky_relu({input_name}, negative_slope=0.01)"


class ELUOperator(Operator):
    """Operator for smith.nn.functional.elu (Exponential Linear Unit)."""

    def __init__(self):
        super().__init__("smith.nn.functional.elu")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.elu"
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.nn.functional.elu({input_name})"


class SiLUOperator(Operator):
    """Operator for smith.nn.functional.silu (Sigmoid Linear Unit, also known as Swish)."""

    def __init__(self):
        super().__init__("smith.nn.functional.silu")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.silu"
    ...
    def codegen(
        ...
        input_name = input_names[0]
        return f"{output_name} = smith.nn.functional.silu({input_name})"


class ScaledDotProductAttentionOperator(Operator):
    """Operator for smith.nn.functional.scaled_dot_product_attention."""

    def __init__(self):
        super().__init__("smith.nn.functional.scaled_dot_product_attention")
    ...
    def codegen(
        ...
        return f"{output_name} = smith.nn.functional.scaled_dot_product_attention({query_name}.to({target_dtype}), {key_name}.to({target_dtype}), {value_name}.to({target_dtype}))"


class MultiHeadAttentionForwardOperator(Operator):
    """Operator for smith.nn.functional.multi_head_attention_forward."""

    def __init__(self):
        super().__init__("smith.nn.functional.multi_head_attention_forward")

    @property
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.nn.functional.multi_head_attention_forward"
    ...
    def codegen(
        ...
        code = f"""{output_name}, _ = smith.nn.functional.multi_head_attention_forward(
