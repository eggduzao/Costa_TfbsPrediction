...
class GatherOperator(Operator):
    ...
    def fuzz_inputs_specs(self, output_spec: Spec, num_inputs: int = 2) -> list[Spec]:
        ...
        smith.gather(input, dim, index) returns a tensor with:
        ...
        if len(output_size) == 1:
            ...
            # Contiguous stride
            input_stride = tuple(
                int(smith.tensor(input_size[i + 1 :]).prod().item())
        ...
        index_spec = TensorSpec(
            size=output_size,
            stride=tuple(
                int(smith.tensor(output_size[i + 1 :]).prod().item())
    ...
    def codegen(
        ...
        return (
            ...
            f"_index_{output_name} = smith.randint(0, _input_size_{output_name}, {output_spec.size}, device={input_names[0]}.device)\n"
            f"{output_name} = smith.gather({input_names[0]}, {dim}, _index_{output_name})"
        )
