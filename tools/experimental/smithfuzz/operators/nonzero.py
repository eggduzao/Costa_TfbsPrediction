...
class NonzeroOperator(Operator):
    ...
    def can_produce(self, output_spec: Spec) -> bool:
        ...
        return (
            ...
            and output_spec.dtype in [smith.int64, smith.long]
    ...
    def codegen(
        ...
        then calls smith.nonzero on it.
        """
        ...
        return (
            f"_x_nz = smith.zeros({shape_literal}, dtype=smith.bool, device={input_names[0]}.device)\n"
            f"_x_nz_flat = _x_nz.reshape(-1)\n"
            f"_x_nz_flat[:{k}] = True\n"
            f"{output_name} = smith.nonzero(_x_nz)"
        )
