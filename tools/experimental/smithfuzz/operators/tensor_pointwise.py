...
from smithfuzz.type_promotion import (
    get_dtype_map,
    get_dtype_name,
...
class PointwiseOperator(Operator):
    ...
    def can_produce(self, output_spec: Spec) -> bool:
        ...
        if isinstance(output_spec, TensorSpec) and output_spec.dtype == smith.bool:
            return False
    ...
    def fuzz_inputs_specs(self, output_spec: Spec, num_inputs: int = 2) -> list[Spec]:
        ...
        # Convert dtype strings back to smith dtypes
    ...
    def codegen(
        ...
        if len(input_names) == 2:
            return f"{output_name} = {self.smith_op_name}({input_names[0]}, {input_names[1]})"
        else:
...
class ClampOperator(Operator):
    """Operator for smith.clamp (element-wise clamping)."""
    ...
    def codegen(
        ...
        return f"{output_name} = smith.clamp({', '.join(args)})"


class CumsumOperator(Operator):
    """Operator for smith.cumsum (cumulative sum along a dimension)."""
    ...
    def codegen(
        ...
        return f"{output_name} = smith.cumsum({input_name}, dim={dim})"
