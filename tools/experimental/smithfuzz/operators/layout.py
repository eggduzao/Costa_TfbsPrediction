...
from smithfuzz.tensor_fuzzer import fuzz_tensor_size, Spec, TensorSpec
...
class ReshapeOperator(LayoutOperatorBase):
    """Operator for smith.reshape() operation."""
    ...
    def codegen(
        ...
        shape_str = str(list(output_spec.size))
        return f"{output_name} = smith.reshape({input_names[0]}, {shape_str})"


class FlattenOperator(LayoutOperatorBase):
    """Operator for smith.flatten() operation."""
    ...
    def smith_op_name(self) -> str | None:
        """Return the smith operation name."""
        return "smith.flatten"

    def can_produce(self, output_spec: Spec) -> bool:
        """Flatten can only produce 1D tensors when using smith.flatten() without start_dim."""
        if not isinstance(output_spec, TensorSpec):
            return False
        # Since we always use smith.flatten() without start_dim, we can only produce 1D tensors
    ...
    def codegen(
        ...
        return f"{output_name} = smith.flatten({input_names[0]})"


class SqueezeOperator(LayoutOperatorBase):
    """Operator for smith.squeeze() operation."""
    ...
    def codegen(
        ...
        return f"{output_name} = smith.squeeze({input_names[0]})"


class UnsqueezeOperator(LayoutOperatorBase):
    """Operator for smith.unsqueeze() operation."""
    ...
    def codegen(
        ...
        return f"{output_name} = smith.unsqueeze({input_names[0]}, dim={dim})"


class SplitOperator(LayoutOperatorBase):
    """Operator for smith.split() operation."""
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # smith.split() splits a tensor along a dimension
    ...
    def codegen(
        ...
        return f"{output_name} = smith.split({input_names[0]}, {split_size}, dim={split_dim})[0]"


class ExpandOperator(LayoutOperatorBase):
    """Operator for smith.expand() operation."""
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # smith.expand() broadcasts a tensor to a new shape
...
class CatOperator(LayoutOperatorBase):
    """Operator for smith.cat() operation."""
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # smith.cat() concatenates tensors along a dimension
    ...
    def codegen(
        ...
        return f"{output_name} = smith.cat([{tensors_str}], dim={cat_dim})"


class StackOperator(LayoutOperatorBase):
    """Operator for smith.stack() operation."""
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # smith.stack() stacks tensors along a new dimension
    ...
    def codegen(
        ...
        tensors_str = ", ".join(input_names)
        return f"{output_name} = smith.stack([{tensors_str}], dim={stack_dim})"


class ChunkOperator(LayoutOperatorBase):
    """Operator for smith.chunk() operation."""
    ...
    def fuzz_inputs_specs(self, output_spec: Spec) -> list[Spec]:
        ...
        # smith.chunk() splits a tensor into chunks along a dimension
        # The output will be one of the chunks from the split
        if len(output_spec.size) == 0:
        ...
        # smith.chunk() tries to split evenly, but the last chunk may be smaller
        input_size = list(output_spec.size)
    ...
    def codegen(
        ...
        return f"{output_name} = smith.chunk({input_names[0]}, {num_chunks}, dim={chunk_dim})[0]"
