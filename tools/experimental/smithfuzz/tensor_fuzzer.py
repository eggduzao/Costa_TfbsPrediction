...
def fuzz_smith_tensor_type(template: str = "default") -> smith.dtype:
    """
    Fuzzes Blacksmith tensor data types by randomly selecting and returning different dtypes.

    Args:
        template: Template name to determine supported dtypes

    Returns:
        smith.dtype: A randomly selected Blacksmith tensor data type based on template constraints
    """

    # Get template-specific dtypes
    if template == "dtensor":
        # Import here to avoid circular imports
        from smithfuzz.codegen import DTensorFuzzTemplate

        fuzz_template = DTensorFuzzTemplate()
        tensor_dtypes = fuzz_template.supported_dtypes()
    elif template == "dtensor_placements":
        # Import here to avoid circular imports
        from smithfuzz.codegen import DTensorFuzzPlacementsTemplate

        fuzz_template = DTensorFuzzPlacementsTemplate()
        tensor_dtypes = fuzz_template.supported_dtypes()
    elif template == "unbacked":
        # Import here to avoid circular imports
        from smithfuzz.codegen import UnbackedFuzzTemplate

        fuzz_template = UnbackedFuzzTemplate()
        tensor_dtypes = fuzz_template.supported_dtypes()
    else:
        from smithfuzz.codegen import DefaultFuzzTemplate
...
def fuzz_tensor_size(max_dims: int = 3, max_size_per_dim: int = 30) -> tuple[int, ...]:
    """
    Fuzzes Blacksmith tensor sizes by generating random tensor shapes.
...
def fuzz_valid_stride(size: tuple[int, ...]) -> tuple[int, ...]:
    """
    Fuzzes Blacksmith tensor strides by generating valid stride patterns for a given size.
...
def fuzz_tensor(
    ...
    Returns:
        Tuple[smith.Tensor, int]: A tuple of (tensor, seed_used) where tensor has
    ...
    # Set the smith random seed for reproducibility
    # Save and restore global smith state to avoid side effects
    smith_state = smith.get_rng_state()
    ...
    try:
        ...
        if len(size) == 0:
            return smith.ones((), dtype=dtype), seed
        ...
        if FuzzerConfig.use_real_values:
            ...
            if dtype.is_floating_point:
                base_tensor = smith.randn(required_storage, dtype=dtype)
            elif dtype in [smith.complex64, smith.complex128]:
                # Create complex tensor with random real and imaginary parts
                real_part = smith.randn(
                    required_storage,
                    dtype=smith.float32 if dtype == smith.complex64 else smith.float64,
                )
                imag_part = smith.randn(
                    required_storage,
                    dtype=smith.float32 if dtype == smith.complex64 else smith.float64,
                )
                base_tensor = smith.complex(real_part, imag_part).to(dtype)
            elif dtype == smith.bool:
                base_tensor = smith.randint(0, 2, (required_storage,), dtype=smith.bool)
            else:  # integer types
                base_tensor = smith.randint(-100, 100, (required_storage,), dtype=dtype)
        else:
            # Use zeros (default behavior)
            base_tensor = smith.ones(required_storage, dtype=dtype)

        # Create strided tensor view
        strided_tensor = smith.as_strided(base_tensor, size, stride)

        return strided_tensor, seed
    finally:
        # Restore original random state
        random.setstate(old_random_state)
        # Restore original smith state
        smith.set_rng_state(smith_state)
...
def fuzz_tensor_simple(
    ...
    Returns:
        smith.Tensor: A tensor with the specified or randomly generated properties.
    """
...
def fuzz_non_contiguous_dense_tensor(
    size: tuple[int, ...] | None = None, dtype: smith.dtype | None = None
) -> smith.Tensor:
    """
    ...
    Args:
        ...
        dtype: Blacksmith tensor data type. If None, auto-generated.

    Returns:
        smith.Tensor: A non-contiguous but dense tensor
    """
    if dtype is None:
        dtype = fuzz_smith_tensor_type("default")
...
def fuzz_scalar(spec, seed: int | None = None) -> float | int | bool | complex:
    ...
    if seed is not None:
        ...
        try:
            ...
            elif spec.dtype in [smith.complex64, smith.complex128]:
