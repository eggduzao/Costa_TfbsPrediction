...
# Add parent directory to path so we can import smithfuzz as a module
...
import smith
from smithfuzz.codegen import convert_graph_to_python_code, create_program_file
from smithfuzz.ops_fuzzer import fuzz_operation_graph, fuzz_spec
from smithfuzz.runner import ProgramRunner
from smithfuzz.visualize_graph import visualize_operation_graph


def _parse_supported_ops_with_weights(spec: str) -> tuple[list[str], dict[str, float]]:
    """Parse --supported-ops string.

    Format: comma-separated fully-qualified smith ops, each optionally with =weight.
    Example: "smith.matmul=5,smith.nn.functional.rms_norm=5,smith.add"
...
def fuzz_and_execute(
    ...
    try:
        ...
        if op_weights:
            from smithfuzz.operators import set_operator_weights
        ...
        for node in operation_graph.nodes.values():
            # Use the fully qualified smith operation name if available
            from smithfuzz.operators import get_operator

            # Try to get the fully qualified smith operation name
            smith_op_name = None
            ...
            try:
                ...
                if (
                    operator
                    and hasattr(operator, "smith_op_name")
                    and operator.smith_op_name
                ):
                    smith_op_name = operator.smith_op_name
            ...
            display_name = smith_op_name if smith_op_name else node.op_name
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Blacksmith Fuzzer - Generate and test random Blacksmith operations"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Comma-separated fully-qualified smith ops to allow, each optionally with =weight. "
            "Examples: 'smith.matmul,smith.nn.functional.rms_norm' or "
            "'smith.matmul=5,smith.nn.functional.rms_norm=5'. Overrides template supported ops."
        ),
    )
