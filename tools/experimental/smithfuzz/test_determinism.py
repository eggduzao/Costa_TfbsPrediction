...
def run_fuzzer_with_seed(seed):
    ...
    smithfuzz_dir = Path("/tmp/smithfuzz")
    if smithfuzz_dir.exists():
        for f in smithfuzz_dir.glob("*.py"):
            f.unlink()
    ...
    # Fallback: parse the exact path the fuzzer ran from stdout: "Running: /tmp/smithfuzz/fuzz_XXXX.py"
    path = None
    for line in lines:
    ...
    if path is None:
        # Fallback: pick the most recently modified fuzz_*.py in /tmp/smithfuzz
        py_files = sorted(
            smithfuzz_dir.glob("fuzz_*.py"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )
        if not py_files:
            print("No Python files generated in /tmp/smithfuzz/")
            return None
        path = str(py_files[0])
