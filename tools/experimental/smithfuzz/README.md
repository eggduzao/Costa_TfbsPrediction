# SmithFuzz - Blacksmith Compile End-to-End Fuzz Testing Framework

SmithFuzz is a comprehensive fuzzing framework for testing Blacksmith operations. It generates random operation graphs, converts them to executable Python code, and validates correctness across eager execution and various `smith.compile()` configurations.

## Overview

SmithFuzz employs a sophisticated four-stage fuzzing pipeline:

1. **Random Specification Generation** - Creates target tensor/scalar specifications with varied shapes, strides, and dtypes
2. **Operation Graph Construction** - Builds Directed Acyclic Graphs (DAGs) of Blacksmith operations through recursive decomposition with dependency tracking
...
```python
TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32)
```

**Step 2: Operation Graph Construction**
```
OperationGraph (root: node_0, target: TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32))
  node_2: arg_0 -> TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32) (depth 0)
  node_3: arg_1 -> TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32) (depth 0)
  node_0: smith.add -> TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32) (depth 2) <- ['node_2', 'node_3']
```
...
def fuzzed_program(arg_0, arg_1, sentinel):
    var_node_2 = arg_0  # TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32)
    var_node_3 = arg_1  # TensorSpec(size=(2, 3), stride=(3, 1), dtype=smith.float32)
    var_node_0 = smith.add(var_node_2, var_node_3)  # TensorSpec(...)
    result = var_node_0 * sentinel
    return result

sentinel = smith.tensor(1.0, requires_grad=True)
arg_0 = smith.randn((2, 3), dtype=smith.float32)
arg_1 = smith.randn((2, 3), dtype=smith.float32)
args = (arg_0, arg_1)
...
```bash
cd tools/experimental/smithfuzz
...
You can restrict the fuzzer to a specific set of fully-qualified smith ops and optionally weight them to bias sampling.

- Restrict to only smith.add and smith.matmul (equal likelihood):

```bash
python fuzzer.py --seed 42 \
  --supported-ops "smith.add,smith.matmul"
```

- Restrict to only smith.add and smith.matmul, and make matmul 5x more likely than add:

```bash
python fuzzer.py --seed 42 \
  --supported-ops "smith.add,smith.matmul=5"
```

Notes:
- Use fully-qualified smith op names (e.g., smith.matmul, smith.nn.functional.rms_norm).
...
SmithFuzz uses a plugin-based operator system where each operation is a class implementing the `Operator` interface:
...
- **Tensor-Tensor**: `smith.add`, `smith.sub`, `smith.mul`, `smith.div`
- **Scalar-Tensor**: Scalar versions of above operations

#### Shape Operations
- `smith.Tensor.view`, `smith.reshape`, `smith.flatten`
- `smith.squeeze`, `smith.unsqueeze`

#### Matrix Operations
- `smith.mm` - Matrix multiplication
- `smith.addmm` - Additive matrix multiplication
- `smith.bmm` - Batch matrix multiplication
- `smith.matmul` - General matrix multiplication

#### Neural Network Operations
- **Layers**: `F.embedding`, `F.linear`
- **Activations**: `F.relu`, `F.leaky_relu`, `F.elu`, `F.gelu`, `F.silu`, `smith.sigmoid`, `smith.tanh`, `F.softmax`
- **Normalization**: `F.layer_norm`, `F.rms_norm`, `F.batch_norm`, `F.group_norm`
- **Regularization**: `F.dropout`

#### Data-Dependent Operations
- `smith.ops.aten.nonzero` - Find non-zero elements
- `smith.ops.aten.masked_select` - Select elements by mask
- `smith.ops.aten.unique` - Find unique elements
- `smith.ops.aten.item` - Extract scalar from tensor
...
**Use Case**: General Blacksmith compilation testing
...
**Use Case**: Testing smith.compile with distributed tensors
...
Total operations executed: 15847
  smith.add                      3421 times ( 21.6%)
  smith.mul                      2890 times ( 18.2%)
  smith.nn.functional.relu       1567 times (  9.9%)
  ...
```
...
```python
from smithfuzz.checks import Check
...
```python
from smithfuzz.fuzzer import fuzz_and_execute
from smithfuzz.ops_fuzzer import fuzz_operation_graph, fuzz_spec
from smithfuzz.codegen import convert_graph_to_python_code
...
SmithFuzz uses a modular operator system. To add a new operation:
...
from smithfuzz.tensor_fuzzer import TensorSpec
...
class MyOperator(Operator):
    ...
    def codegen(self, output_name, input_names, output_spec):
        ...
        return f"{output_name} = smith.my_op({', '.join(input_names)})"
```
...
```python
from smithfuzz.operators.my_op import MyOperator
...
If you want the operator in specific templates, add its smith_op_name to the template's `supported_ops` list in `codegen.py`:
...
ls /tmp/smithfuzz/fuzz_*.py

# Run generated program directly
python /tmp/smithfuzz/fuzz_<hash>.py
```
