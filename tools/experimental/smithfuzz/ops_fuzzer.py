...
from smithfuzz.operators import get_operator, list_operators
from smithfuzz.tensor_fuzzer import (
    fuzz_tensor_size,
    fuzz_smith_tensor_type,
    fuzz_valid_stride,
    ScalarSpec,
    Spec,
    specs_compatible,
...
def _get_template_filtered_operators(
    ...
    for op_name, operator in all_operators.items():
        # Always include operations that don't have a specific smith operation
        # (utility operations like arg, constant, item, scalar ops)
        smith_op = operator.smith_op_name
        if smith_op is None:
        ...
        for supported_op in allowed_ops:
            # Direct smith operation matching
            if smith_op == supported_op:
                should_include = True
                break
...
class OperationNode:
    ...
    Attributes:
        ...
        op_name: Name of the operation (e.g., 'smith.ops.aten.add', 'scalar_add', 'arg')
...
def fuzz_spec(template: str = "default") -> Spec:
    ...
    try:
        ...
        dtype = fuzz_smith_tensor_type(template)
