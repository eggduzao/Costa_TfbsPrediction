...
from smithfuzz.operators import get_operator
from smithfuzz.ops_fuzzer import OperationGraph
from smithfuzz.tensor_descriptor import format_tensor_descriptor
from smithfuzz.tensor_fuzzer import ScalarSpec, Spec, TensorSpec
...
class FuzzTemplate:
    ...
    def _generate_scalar_spec(self, dtype):
        ...
        from smithfuzz.tensor_fuzzer import ScalarSpec
        ...
        if arg_operations:
            for i, (node_id, spec) in enumerate(arg_operations):
                ...
                if isinstance(spec, ScalarSpec):
                    dtype_str = f"smith.{spec.dtype}".replace("smith.smith.", "smith.")
                    if spec.dtype in [
                        smith.int8,
                        smith.int16,
                        smith.int32,
                        ...
                        code_lines.append(
                            f"{arg_name} = int(smith.randint(5, 30, ()).item())"
                        )
                    elif spec.dtype == smith.bool:
                        # For boolean scalars, use randint and cast to bool
                        code_lines.append(
                            f"{arg_name} = bool(smith.randint(0, 2, ()).item())"
                        )
                    else:
                        # For float scalars, use randn
                        code_lines.append(
                            f"{arg_name} = float(smith.randn((), dtype={dtype_str}).item())"
                        )
                    ...
                    if spec.dtype in [
                        ...
                        code_lines.append(
                            f"{arg_name} = smith.as_strided(smith.randint({min_val}, {max_val}, ({storage_size},)).to({dtype_str}), {size_str}, {stride_str})"
                        )
                    elif spec.dtype == smith.bool:
                        ...
                        code_lines.append(
                            f"{arg_name} = smith.as_strided(smith.randint(0, 2, ({storage_size},), dtype=smith.int8).bool(), {size_str}, {stride_str})"
                        )
                    else:
                        code_lines.append(
                            f"{arg_name} = smith.as_strided(smith.randn({storage_size}).to({dtype_str}), {size_str}, {stride_str})"
                        )
...
class DefaultFuzzTemplate(FuzzTemplate):
    def __init__(self):
        from smithfuzz.checks import EagerVsFullGraphDynamicCompileCheck
...
class DTensorFuzzTemplate(FuzzTemplate):
    ...
    def args_codegen(self, arg_operations):
        ...
        code_lines.extend(
            [
                ...
                "",
                "# Sentinel tensor to ensure gradient computation",
                "sentinel_local = smith.tensor(1.0, device='cuda', requires_grad=True)",
        ...
        if arg_operations:
            for i, (node_id, spec) in enumerate(arg_operations):
                ...
                if isinstance(spec, ScalarSpec):
                    ...
                    code_lines.extend(
                        [
                            f"{arg_name}_local = smith.randn((), dtype={dtype_str}, device='cuda', requires_grad=True)",
                    ...
                    if spec.dtype in [
                        ...
                        code_lines.extend(
                            [
                                f"{arg_name}_local = smith.randint(1, 10, {size_str}, dtype={dtype_str}, device='cuda')",
                                f"{arg_name} = DTensor.from_local({arg_name}_local, mesh, placements)",
                            ]
                        )
                    elif spec.dtype == smith.bool:
                        ...
                        code_lines.extend(
                            [
                                f"{arg_name}_local = smith.randint(0, 2, {size_str}, device='cuda').bool()",
                                f"{arg_name} = DTensor.from_local({arg_name}_local, mesh, placements)",
                            ]
                        )
                    else:
                        ...
                        code_lines.extend(
                            [
                                f"{arg_name}_local = smith.randn({size_str}, dtype={dtype_str}, device='cuda', requires_grad=True)",
                                f"{arg_name} = DTensor.from_local({arg_name}_local, mesh, placements)",
                            ]
                        )
    ...
    def epilogue_codegen(self):
        return ["smith.distributed.destroy_process_group()"]
...
class UnbackedFuzzTemplate(FuzzTemplate):
    def __init__(self):
        ...
        super().__init__(
            supported_ops=[
                "smith.ops.aten.item",
                "smith.ops.aten.nonzero",
                "smith.ops.aten.masked_select",
                "smith.ops.aten.unique",
                # Basic arithmetic operations
                "smith.add",
                "smith.sub",
                "smith.mul",
...
class DTensorFuzzPlacementsTemplate(DTensorFuzzTemplate):
    ...
    def fuzz_spec_custom(self):
        ...
        from smithfuzz.tensor_fuzzer import fuzz_valid_stride
    ...
    def imports_codegen(self):
        ...
        for i, imp in enumerate(base_imports):
            if "placement_types import" in imp:
                base_imports[i] = (
                    "from smith.distributed.tensor.placement_types import Replicate, Shard, Partial"
                )
                break
        base_imports.append("import smith.distributed.tensor as dist_tensor")
        return base_imports
    ...
    def args_codegen(self, arg_operations, constant_operations=None):
        ...
        code_lines.extend(
            [
                f"sentinel = dist_tensor.ones((1,), device_mesh=mesh, placements={sentinel_placements}, dtype=smith.float32, requires_grad=True)",
                "",
            ]
        )
            ...
            for i, (node_id, spec) in enumerate(arg_operations):
                if isinstance(spec, TensorSpec):
                    ...
                    if spec.dtype in [
                        ...
                        code_lines.append(
                            f"arg_{i} = dist_tensor.ones({size_str}, device_mesh=mesh, placements={placements}, dtype=smith.int8).bool()"
                        )
                    else:
                        code_lines.append(
...
def convert_graph_to_python_code(
    ...
    if seed is not None:
        code_lines.append(f"smith.manual_seed({seed})")
        code_lines.append("")
