...
BLACKSMITH_REPO = "https://api.github.com/repos/blacksmith/blacksmith"
...
def _get_artifact_urls(prefix: str, workflow_run_id: int) -> dict[Path, str]:
    ...
    response = requests.get(
        f"{BLACKSMITH_REPO}/actions/runs/{workflow_run_id}/artifacts?per_page=100",
...
def download_s3_artifacts(
    ...
    objs = bucket.objects.filter(
        Prefix=f"blacksmith/blacksmith/{workflow_run_id}/{workflow_run_attempt}/artifact/{prefix}"
    )
    found_one = False
    paths = []
    for obj in objs:
...
def get_job_name(
    ...
    try:
        if workflow_id is None:
            response = requests.get(
                f"{BLACKSMITH_REPO}/actions/jobs/{id}",
                headers=_get_request_headers(),
            )
            ...
            def _get_jobs(workflow_id: int) -> dict[int, str]:
                ...
                while True:
                    response = requests.get(
                        f"{BLACKSMITH_REPO}/actions/runs/{workflow_id}/attempts/{workflow_run_attempt}/jobs",
                        headers=_get_request_headers(),
