#!/usr/bin/env python3
"""
Test ownership was introduced in https://github.com/blacksmith/blacksmith/issues/66232.
...
has valid ownership information in a comment header. Valid means:
  ...
  - Each owner label actually exists in Blacksmith
...
def get_blacksmith_labels() -> Any:
    url = "https://ossci-metrics.s3.amazonaws.com/blacksmith_labels.json"
    try:
...
BLACKSMITH_LABELS = get_blacksmith_labels()
...
def check_labels(
    ...
    lint_messages = []
    for label in labels:
        if label not in BLACKSMITH_LABELS:
            lint_messages.append(
                LintMessage(
                    ...
                    replacement=None,
                    description=(
                        f"{label} is not a Blacksmith label "
                        "(please choose from https://github.com/blacksmith/blacksmith/labels)"
                    ),
                )
            )
