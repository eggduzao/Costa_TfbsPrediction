...
smith/csrc/stable/c/shim.h in smith/csrc/stable are properly wrapped in
SMITH_FEATURE_VERSION macros corresponding to the version when those
...
def get_shim_functions(
    ...
    Only functions defined inside SMITH_FEATURE_VERSION blocks are extracted.
    Functions without version guards are ignored.

    Args:
        shim_files: List of paths to shim header files. If None, will use the default
                    paths to smith/csrc/stable/c/shim.h and
                    smith/csrc/inductor/aoti_smith/c/shim.h based on the repository root.
    """
    if shim_files is None:
        repo_root = Path(__file__).resolve().parents[3]
        shim_files_to_check = [
            repo_root / "smith/csrc/stable/c/shim.h",
            repo_root / "smith/csrc/inductor/aoti_smith/c/shim.h",
        ]
    else:
    ...
    # Match function declarations like: AOTI_SMITH_EXPORT ... function_name(
    function_pattern = re.compile(r"AOTI_SMITH_EXPORT\s+\w+\s+(\w+)\s*\(")
...
def write_shim_function_versions(
    ...
    Args:
        ...
        output_file: Path to the output file. If None, will write to
                     smith/csrc/stable/c/shim_function_versions.txt in the repository.
    """
    if output_file is None:
        repo_root = Path(__file__).resolve().parents[3]
        output_file = repo_root / "smith/csrc/stable/c/shim_function_versions.txt"
    else:
    ...
    with open(output_file, "w") as f:
        ...
        f.write("# Format: function_name: SMITH_VERSION_MAJOR_MINOR_PATCH\n")
        f.write("#\n")
        f.write(
        ...
        for func_name, (major, minor) in sorted_functions:
            f.write(f"{func_name}: SMITH_VERSION_{major}_{minor}_0\n")
...
def check_file(
    ...
    Args:
        filename: File in smith/csrc/stable that calls functions from shim.
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        for func_name, required_version in shim_functions.items():
            ...
            if re.search(rf"\b{re.escape(func_name)}\b", line):
                ...
                required_macro = f"SMITH_VERSION_{major}_{minor}_0"
                ...
                if version_of_block is None:
                    ...
                    lint_messages.append(
                        LintMessage(
                            ...
                            description=(
                                ...
                                f"in a SMITH_FEATURE_VERSION block. This function requires at least:\n"
                                f"#if SMITH_FEATURE_VERSION >= {required_macro}\n"
                                f"  // ... your code calling {func_name} ...\n"
                                f"#endif // SMITH_FEATURE_VERSION >= {required_macro}"
                            ),
                        )
                    )
                    ...
                    current_macro = f"SMITH_VERSION_{current_major}_{current_minor}_0"
                    lint_messages.append(
                        LintMessage(
                            path=filename,
                            line=line_num,
