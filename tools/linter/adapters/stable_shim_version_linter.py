...
are properly wrapped in SMITH_FEATURE_VERSION macros corresponding to the
current SMITH_ABI_VERSION.
"""
...
class PreprocessorTracker:
    ...
    def __init__(self):
        ...
        # is_version_block: True if this is a SMITH_FEATURE_VERSION >= SMITH_VERSION_X_Y_0 block
        ...
        self.version_pattern = re.compile(
            r"#(?:if|elif)\s+SMITH_FEATURE_VERSION\s*>=\s*SMITH_VERSION_(\d+)_(\d+)_\d+"
        )
...
def get_current_version() -> tuple[int, int]:
    """
    Get the current Blacksmith version from version.txt.
    This uses the same logic as tools/setup_helpers/gen_version_header.py
    which is used to generate smith/headeronly/version.h from version.h.in.
...
def check_file(filename: str) -> list[LintMessage]:
    ...
    1. All function declarations are within SMITH_FEATURE_VERSION blocks
    2. New functions added in this commit use the current version macro

    For the AOTI shim (smith/csrc/inductor/aoti_smith/c/shim.h), we only
    ...
    is_aoti_shim = "smith/csrc/inductor/aoti_smith/c/shim.h" in filename

    # Get current version
    current_version = get_current_version()
    major, minor = current_version
    expected_version_macro = f"SMITH_VERSION_{major}_{minor}_0"
    expected_version_check = f"#if SMITH_FEATURE_VERSION >= {expected_version_macro}"
    ...
    # Function declaration patterns - looking for AOTI_SMITH_EXPORT or typedef
    function_decl_patterns = [
        re.compile(r"^\s*AOTI_SMITH_EXPORT\s+\w+"),  # AOTI_SMITH_EXPORT functions
        ...
        if inside_extern_c:
            ...
            if is_function_decl:
                ...
                version_of_block_macro = (
                    f"SMITH_VERSION_{tracker_version[0]}_{tracker_version[1]}_0"
                    if tracker_version
                    else None
                )
                ...
                if not inside_version_block:
                    ...
                    if not is_new_line:
                        ...
                        if is_aoti_shim:
                            continue
                        expected_version_macro_str = "SMITH_VERSION_X_Y_Z"
                        expected_version_check_str = (
                            f"#if SMITH_FEATURE_VERSION >= {expected_version_macro}"
                        )
                        additional_text = "\nX, Y, and Z correspond to the SMITH_ABI_VERSION when the function was added."
                    else:
                    ...
                    lint_messages.append(
                        LintMessage(
                            ...
                            replacement=None,
                            description=(
                                f"Function declaration found outside of SMITH_FEATURE_VERSION block. "
                                f"All function declarations must be wrapped in:\n"
                                f"{expected_version_check_str}\n"
                                f"// ... your declarations ...\n"
                                f"#endif // SMITH_FEATURE_VERSION >= {expected_version_macro_str}"
                                f"{additional_text}"
                            ),
                        )
                    )
                    ...
                    lint_messages.append(
                        LintMessage(
                            ...
                            description=(
                                ...
                                f"// ... your declarations ...\n"
                                f"#endif // SMITH_FEATURE_VERSION >= {expected_version_macro}"
                            ),
                        )
                    )
