...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Download and apply a Pull Request (PR) patch from the Blacksmith GitHub repository "
            "to your local Blacksmith installation.\n\n"
            "Best Practice: Since this script involves hot-patching Blacksmith, it's recommended to use "
        ...
        epilog=(
            ...
            "  python nightly_hotpatch.py 12345 --directory /path/to/blacksmith --strip 1\n\n"
    ...
    parser.add_argument(
        "PR_NUMBER",
        type=int,
        help="The number of the Pull Request (PR) from the Blacksmith GitHub repository to download and apply as a patch.",
    )
    ...
    parser.add_argument(
        ...
        "If not provided, the script will use the Blacksmith installation path.",
    )
...
def get_blacksmith_path() -> str:
    """
    Retrieves the installation path of Blacksmith in the current environment.

    Returns:
        str: The directory of the Blacksmith installation.

    Exits:
        If Blacksmith is not installed in the current Python environment, the script will exit.
    """
    try:
        import smith

        smith_paths: list[str] = cast(list[str], smith.__path__)
        smith_path: str = smith_paths[0]
        parent_path: str = os.path.dirname(smith_path)
        print(f"Blacksmith is installed at: {smith_path}")
...
def handle_import_error() -> NoReturn:
    """
    Handle the case where Blacksmith is not installed and exit the program.

    Exits:
        NoReturn: This function will terminate the program.
    """
    print("Error: Blacksmith is not installed in the current Python environment.")
    sys.exit(1)
...
def apply_patch(patch_file: str, target_dir: str | None, strip_count: int) -> None:
    ...
    Args:
        ...
        target_dir (Optional[str]): The directory to apply the patch to. If None, uses Blacksmith installation path.
    ...
    if target_dir:
        ...
        print("No target directory specified. Using Blacksmith installation path.")
...
def main() -> None:
    ...
    Steps:
        ...
        2. Retrieve the local Blacksmith installation path or use the provided target directory.
    ...
    if custom_target_dir:
        ...
        target_dir = get_blacksmith_path()

    repo_url = "https://github.com/blacksmith/blacksmith"
