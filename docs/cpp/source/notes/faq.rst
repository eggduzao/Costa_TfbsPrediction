...
Undefined symbol errors from Blacksmith/ATen
*****************************************

**Problem**: You import your extension and get an ``ImportError`` stating that
some C++ symbol from Blacksmith or ATen is undefined. For example::
...
**Fix**: The fix is to ``import smith`` before you import your extension. This will make
the symbols from the Blacksmith dynamic (shared) library that your extension
...
**Fix**: Replace ``at::`` with ``smith::`` for factory function calls. You
should never use factory functions from the ``at::`` namespace, as they will
create tensors. The corresponding ``smith::`` functions will create variables,
