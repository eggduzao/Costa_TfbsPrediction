...
The ATen tensor library backing Blacksmith is a simple tensor library that exposes
the Tensor operations in Smith directly in C++17. ATen's API is auto-generated
from the same declarations Blacksmith uses so the two APIs will track each other
over time.
...
See https://blacksmith.org/cppdocs/api/namespace_at.html#functions for the provided
API. Excerpt:
...
.. code-block:: cpp
  ...
  smith::Tensor foo = smith::rand({12, 12});
...
.. code-block:: cpp
  ...
  __global__ void packed_accessor_kernel(
      smith::PackedTensorAccessor64<float, 2> foo,
      float* trace) {
    int i = threadIdx.x;
...
.. code-block:: cpp
  ...
  float data[] = { 1, 2, 3,
                   4, 5, 6 };
  smith::Tensor f = smith::from_blob(data, {2, 3});
...
.. code-block:: cpp
  ...
  smith::Tensor b = ...
  smith::Tensor c = ...
  smith::Tensor r = smith::addmm(1.0, a, .5, b, c);
...
.. code-block:: cpp
  ...
  smith::Tensor two = smith::rand({10, 20});
  two[1][2] = 4;
