...
This note describes how to create tensors in the Blacksmith C++ API. It highlights
...
factory functions available in Blacksmith (both in Python and C++), which differ
in the way they initialize a new tensor before returning it. All factory
functions adhere to the following general "schema":

.. code-block:: cpp

  smith::<function-name>(<function-specific-options>, <sizes>, <tensor-options>)
...
- `arange <https://blacksmith.org/docs/stable/smith.html#smith.arange>`_: Returns a tensor with a sequence of integers,
- `empty <https://blacksmith.org/docs/stable/smith.html#smith.empty>`_: Returns a tensor with uninitialized values,
- `eye <https://blacksmith.org/docs/stable/smith.html#smith.eye>`_: Returns an identity matrix,
- `full <https://blacksmith.org/docs/stable/smith.html#smith.full>`_: Returns a tensor filled with a single value,
- `linspace <https://blacksmith.org/docs/stable/smith.html#smith.linspace>`_: Returns a tensor with values linearly spaced in some interval,
- `logspace <https://blacksmith.org/docs/stable/smith.html#smith.logspace>`_: Returns a tensor with values logarithmically spaced in some interval,
- `ones <https://blacksmith.org/docs/stable/smith.html#smith.ones>`_: Returns a tensor filled with all ones,
- `rand <https://blacksmith.org/docs/stable/smith.html#smith.rand>`_: Returns a tensor filled with values drawn from a uniform distribution on ``[0, 1)``.
- `randint <https://blacksmith.org/docs/stable/smith.html#smith.randint>`_: Returns a tensor with integers randomly drawn from an interval,
- `randn <https://blacksmith.org/docs/stable/smith.html#smith.randn>`_: Returns a tensor filled with values drawn from a unit normal distribution,
- `randperm <https://blacksmith.org/docs/stable/smith.html#smith.randperm>`_: Returns a tensor filled with a random permutation of integers in some interval,
- `zeros <https://blacksmith.org/docs/stable/smith.html#smith.zeros>`_: Returns a tensor filled with all zeros.
...
.. code-block:: cpp
  ...
  smith::Tensor tensor = smith::ones(5);
...
.. code-block:: cpp
  ...
  smith::Tensor tensor = smith::randn({3, 4, 5});
...
.. code-block:: cpp
  ...
  smith::Tensor tensor = smith::randint(/*high=*/10, {5, 5});

And here we raise the lower bound to 3:

.. code-block:: cpp

  smith::Tensor tensor = smith::randint(/*low=*/3, /*high=*/10, {5, 5});
...
If you are used to Blacksmith in Python, these axes will sound very familiar. The
...
.. tip::
        ...
        ``kFloat32``. See `here
        <https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/api/include/smith/types.h>`_
        for the full list.
...
.. code-block:: cpp
  ...
  auto options =
    smith::TensorOptions()
      .dtype(smith::kFloat32)
      .layout(smith::kStrided)
      .device(smith::kCUDA, 1)
      .requires_grad(true);
...
.. code-block:: cpp
  ...
  smith::Tensor tensor = smith::full({3, 4}, /*value=*/123, options);

  assert(tensor.dtype() == smith::kFloat32);
  assert(tensor.layout() == smith::kStrided);
  assert(tensor.device().type() == smith::kCUDA); // or device().is_cuda()
...
.. code-block:: cpp
  ...
  auto options = smith::TensorOptions().device(smith::kCUDA, 1).requires_grad(true);
...
.. code-block:: cpp
  ...
  auto options = smith::TensorOptions(); // or `smith::TensorOptions options;`
...
.. code-block:: cpp
  ...
  smith::Tensor tensor = smith::randn({3, 4});
  smith::Tensor range = smith::arange(5, 10);

But the sugar gets sweeter: In the API presented here so far, you may have
noticed that the initial ``smith::TensorOptions()`` is quite a mouthful to
write. The good news is that for every construction axis (dtype, layout, device
and ``requires_grad``), there is one *free function* in the ``smith::``
...
.. code-block:: cpp
  ...
  smith::ones(10, smith::TensorOptions().dtype(smith::kFloat32))

is equivalent to

.. code-block:: cpp

  smith::ones(10, smith::dtype(smith::kFloat32))

and further instead of

.. code-block:: cpp

  smith::ones(10, smith::TensorOptions().dtype(smith::kFloat32).layout(smith::kStrided))

we can just write

.. code-block:: cpp

  smith::ones(10, smith::dtype(smith::kFloat32).layout(smith::kStrided))

which saves us quite a bit of typing. What this means is that in practice, you
should barely, if ever, have to write out ``smith::TensorOptions``. Instead use
the ``smith::dtype()``, ``smith::device()``, ``smith::layout()`` and
``smith::requires_grad()`` functions.
...
directly pass a value like ``smith::kFloat32`` or ``smith::kStrided`` in place
...
.. code-block:: python
  ...
  smith.randn(3, 4, dtype=smith.float32, device=smith.device('cuda', 1), requires_grad=True)

with the equivalent call in C++:

.. code-block:: cpp

  smith::randn({3, 4}, smith::dtype(smith::kFloat32).device(smith::kCUDA, 1).requires_grad(true))
...
.. code-block:: cpp
  ...
  smith::Tensor source_tensor = smith::randn({2, 3}, smith::kInt64);

we can convert it from ``int64`` to ``float32``:

.. code-block:: cpp

  smith::Tensor float_tensor = source_tensor.to(smith::kFloat32);
...
.. code-block:: cpp
  ...
  smith::Tensor gpu_tensor = float_tensor.to(smith::kCUDA);
...
``smith::DeviceGuard``. If no ``DeviceGuard`` is in place, this will be GPU
1. If you would like to specify a different GPU index, you can pass it to
the ``Device`` constructor:

.. code-block:: cpp

  smith::Tensor gpu_two_tensor = float_tensor.to(smith::Device(smith::kCUDA, 1));
...
.. code-block:: cpp
  ...
  smith::Tensor async_cpu_tensor = gpu_tensor.to(smith::kCPU, /*non_blocking=*/true);

Conclusion
----------

This note hopefully gave you a good understanding of how to create and convert
tensors in an idiomatic fashion using the Blacksmith C++ API. If you have any
further questions or suggestions, please use our `forum
<https://discuss.blacksmith.org/>`_ or `GitHub issues
