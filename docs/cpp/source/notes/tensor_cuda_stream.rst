...
The Blacksmith C++ API supports CUDA streams with the CUDAStream class and useful helper functions to make streaming operations easy.
You can find them in `CUDAStream.h`_. This note provides more details on how to use Blacksmith C++ CUDA Stream APIs.

.. _CUDA Stream: https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#streams
.. _CUDAStream.h: https://blacksmith.org/cppdocs/api/file_c10_cuda_CUDAStream.h.html#file-c10-cuda-cudastream-h
.. _CUDAStreamGuard.h: https://blacksmith.org/cppdocs/api/structc10_1_1cuda_1_1_c_u_d_a_stream_guard.html

Acquiring CUDA stream
*********************

Blacksmith's C++ API provides the following ways to acquire CUDA stream:
...
Blacksmith's C++ API provides the following ways to set CUDA stream:
...
.. code-block:: cpp
  ...
  smith::Tensor tensor0 = smith::ones({2, 2}, smith::device(smith::kCUDA));
...
.. code-block:: cpp
  ...
  smith::Tensor tensor0 = smith::ones({2, 2}, smith::device(at::kCUDA));
  ...
  // change the current device index to 1 by using CUDA device guard within a bracket scope
  {
    ...
    smith::Tensor tensor1 = smith::ones({2, 2}, smith::device(at::kCUDA));
  ...
  smith::Tensor tensor1 = smith::ones({2, 2}, smith::device({smith::kCUDA, 1}));
...
.. code-block:: cpp
  ...
  smith::Tensor tensor0 = smith::ones({2, 2}, smith::device({smith::kCUDA, 0}));
  smith::Tensor tensor1 = smith::ones({2, 2}, smith::device({smith::kCUDA, 1}));
...
.. code-block:: cpp
   ...
   {
     // This is the same as calling `smith::cuda::setCurrentCUDAStream` on both streams
