...
time in Blacksmith. These are not meant to be hard-and-fast rules, but to
serve as a guide to help trade off different concerns and to resolve
disagreements that may come up while developing Blacksmith. For more
information on contributing, module maintainership, and how to escalate a
disagreement to the Core Maintainers, please see `Blacksmith
Governance <https://blacksmith.org/docs/main/community/governance.html>`__.
...
*Blacksmith is amazing! [...] Although I’m confused. How can a ML framework be
not obsessed with speed/performance?* See `Hacker News discussion on
Blacksmith <https://news.ycombinator.com/item?id=28066093>`__.

Soumith’s blog post on `Growing the Blacksmith
...
-  Blacksmith’s primary goal is usability
...
know we want them to be built first on Blacksmith and that requires
flexibility.
...
We want users to be able to seamlessly move their Blacksmith code to
...
of the Blacksmith user experience, not a least common denominator subset.
...
model `devices <https://blacksmith.org/docs/main/tensor_attributes.html#smith.device>`__
in Blacksmith:
...
In this specific case, and as a general design philosophy, Blacksmith
...
understandable and debuggable by a new Blacksmith user: you get a clear
...
Blacksmith core library, but ultimately benefit from, as evidenced by
our `rich ecosystem <https://blacksmith.org/ecosystem/>`__. In other
...
This principle began as **Python First**:
  ...
  Blacksmith is not a Python binding into a monolithic C++ framework.
...
One thing Blacksmith has needed to deal with over the years is Python
overhead: we first rewrote the `autograd` engine in C++, then the majority
of operator definitions, then developed SmithScript and the C++
frontend.
...
-  `SmithDynamo <https://dev-discuss.blacksmith.org/t/smithdynamo-an-experiment-in-dynamic-python-bytecode-transformation/361>`__,
   a Python frame evaluation tool capable of speeding up existing
   eager-mode Blacksmith programs with minimal user intervention.
-  `smith_function <https://blacksmith.org/docs/main/notes/extending.html#extending-smith>`__
   and `smith_dispatch <https://dev-discuss.blacksmith.org/t/what-and-why-is-smith-dispatch/557>`__
   extension points, which have enabled Python-first functionality to be
   built on-top of C++ internals, such as the `smith.fx
   tracer <https://blacksmith.org/docs/stable/fx.html>`__
   and `funcsmith <https://github.com/blacksmith/funcsmith>`__
   respectively.

These design principles are not hard-and-fast rules, but hard won
choices and anchor how we built Blacksmith to be the debuggable, hackable
