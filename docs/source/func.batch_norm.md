...
Funcsmith does not support inplace update to a regular tensor that takes in a batched tensor (i.e.
...
### Option 2: smithvision parameter
Some smithvision models, like resnet and regnet, can take in a `norm_layer` parameter. These are
...
smithvision.models.resnet18(norm_layer=lambda c: GroupNorm(num_groups=g, c))
```
...
import smithvision
from functools import partial
smithvision.models.resnet18(norm_layer=partial(BatchNorm2d, track_running_stats=False))
```

### Option 3: funcsmith's patching
funcsmith has added some functionality to allow for quick, in-place patching of the module to not
...
```python
from smith.func import replace_all_batch_norm_modules_
