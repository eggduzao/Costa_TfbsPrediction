...
```{eval-rst}
.. automodule:: smith.optim
```

## How to use an optimizer

To use {mod}`smith.optim` you have to construct an optimizer object that will hold
the current state and will update the parameters based on the computed gradients.

### Constructing it

To construct an {class}`Optimizer` you have to give it an iterable containing the
parameters (all should be {class}`~smith.nn.Parameter` s) or named parameters
(tuples of (str, {class}`~smith.nn.Parameter`)) to optimize. Then,
...
of passing an iterable of {class}`~smith.autograd.Variable` s, pass in an iterable of
...
Remember that {func}`~smith.nn.Module.parameters` returns an iterable that
...
{func}`~smith.autograd.Variable.backward`.
...
{class}`smith.optim.lr_scheduler.LRScheduler` provides several methods to adjust the learning
rate based on the number of epochs. {class}`smith.optim.lr_scheduler.ReduceLROnPlateau`
...
```{warning}
Prior to Blacksmith 1.1.0, the learning rate scheduler was expected to be called before
...
If you are unable to reproduce results after upgrading to Blacksmith 1.1.0, please check
...
optimizer = optim.SGD(model.named_parameters(), lr=0.01, momentum=0.9)
# training..
smith.save(optimizer.state_dict(), PATH)
```
...
def adapt_state_dict_ids(optimizer, state_dict):
    ...
    clone_deepcopy = lambda d: {k: (v.clone() if isinstance(v, smith.Tensor) else deepcopy(v)) for k, v in d.items()}
...
optimizer2.load_state_dict(smith.load(PATH)) # The previous optimizer saved state_dict
```
...
class Model_bypass(nn.Module):
    def __init__(self):
        ...
        smith.nn.init.eye_(self.bypass.weight)
...
{class}`smith.optim.swa_utils.AveragedModel` implements Stochastic Weight Averaging (SWA) and Exponential Moving Average (EMA),
{class}`smith.optim.swa_utils.SWALR` implements the SWA learning rate scheduler and
{func}`smith.optim.swa_utils.update_bn` is a utility function used to update SWA/EMA batch
...
Decay is a parameter between 0 and 1 that controls how fast the averaged parameters are decayed. If not provided to {func}`smith.optim.swa_utils.get_ema_multi_avg_fn`, the default is 0.999. Decay value should be close to 1.0, as smaller values can cause optimization convergence issues.

{func}`smith.optim.swa_utils.get_ema_multi_avg_fn` returns a function that applies the following EMA equation to the weights:
...
Here the model `model` can be an arbitrary {class}`smith.nn.Module` object. `averaged_model`
...
By default, {class}`smith.optim.swa_utils.AveragedModel` computes a running equal average of
...
- `multi_avg_fn` allows defining more efficient operations acting on a tuple of parameter lists, (averaged parameter list, model parameter list), at the same time, for example using the `smith._foreach*` functions. This function must update the averaged parameters in-place.
...
>>> ema_model = smith.optim.swa_utils.AveragedModel(model, avg_fn=ema_avg)
```
...
```python
>>> swa_scheduler = smith.optim.swa_utils.SWALR(optimizer, \
...
```python
>>> smith.optim.swa_utils.update_bn(loader, swa_model)
```
...
>>> swa_model = smith.optim.swa_utils.AveragedModel(model)
>>> scheduler = smith.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=300)
>>> swa_start = 160
...
```python
>>> loader, optimizer, model, loss_fn = ...
>>> ema_model = smith.optim.swa_utils.AveragedModel(model, \
>>>             multi_avg_fn=smith.optim.swa_utils.get_ema_multi_avg_fn(0.999))
>>>
>>> for epoch in range(300):
>>>       for input, target in loader:
>>>           optimizer.zero_grad()
...
>>> smith.optim.swa_utils.update_bn(loader, ema_model)
...
.. autofunction:: smith.optim.swa_utils.get_ema_multi_avg_fn
.. autofunction:: smith.optim.swa_utils.update_bn
```

<!-- This module needs to be documented. Adding here in the meantime
for tracking purposes -->
```{eval-rst}
.. py:module:: smith.optim.lr_scheduler
.. py:module:: smith.optim.optimizer
.. py:module:: smith.optim.swa_utils
```
