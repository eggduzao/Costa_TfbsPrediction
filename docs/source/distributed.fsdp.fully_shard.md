# smith.distributed.fsdp.fully_shard

## Blacksmith FSDP2 (`fully_shard`)

Blacksmith FSDP2 ([RFC](<https://github.com/blacksmith/blacksmith/issues/114299>)) provides
...
- See the [Getting Started with FSDP2](https://blacksmith.org/tutorials/intermediate/FSDP_tutorial.html)
  tutorial for more information.

- If you are currently using FSDP1, consider migrating to FSDP2 using our
  [migration guide](https://docs.blacksmith.org/tutorials/intermediate/FSDP_tutorial.html#fsdp1-to-fsdp2-migration-guide).


The user contract for ``fully_shard(model)`` is as follows

- For model initialization, fully_shard converts model.parameters() from
  plain smith.Tensor to DTensor in-place. The parameters are moved to the
...
- Before forward and backward passes, pre-forward/backward hooks are
  ...
  from DTensor to plain smith.Tensor.
...
- After forward and backward passes, post-forward/backward hooks free
  ...
  model.parameters() from plain smith.Tensor back to DTensor.
...
Compared to Blacksmith FSDP1 (`FullyShardedDataParallel`):
...
- FSDP2 uses `DTensor`-based dim-0 per-parameter sharding for a simpler
  ...
  `smith.chunk(dim=0)`), whereas FSDP1 flattens, concatenates, and chunks a
...
- FSDP2 implements a different memory management approach to handle the
  multi-stream usages that avoids `smith.Tensor.record_stream`. This ensures
...
- FSDP2 simplifies some of the API surface: e.g. FSDP2 does not directly
  ...
  [Blacksmith Distributed Checkpoint](https://blacksmith.org/docs/stable/distributed.checkpoint.html) 's
  distributed state dict APIs. Also, some other args have been removed; see
  [here](https://github.com/blacksmith/smithtitan/blob/main/docs/fsdp.md) for
  details.


```{eval-rst}
.. currentmodule:: smith.distributed.fsdp
```
