...
```{note}
This library is currently in [beta](https://blacksmith.org/blog/blacksmith-feature-classification-changes/#beta).
What this means is that the features generally work (unless otherwise documented)
and we (the Blacksmith team) are committed to bringing this library forward. However, the APIs
may change under user feedback and we don't have full coverage over Blacksmith operations.
...
- {mod}`smith.func` has auto-differentiation transforms (`grad(f)` returns a function that
