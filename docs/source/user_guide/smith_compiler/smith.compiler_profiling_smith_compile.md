...
* Use `smith.profiler.profile()` context for profiling the section we are interested in
...
```python
    ...
    inputs = [smith.randn((5, 3, 224, 224), device=device) for _ in range(10)]

    model_c = smith.compile(model)
...
```{figure} ../../_static/img/profiling_smith_compile/basic_chrome_trace.png
...
```{figure}  ../../_static/img/profiling_smith_compile/ac2g.png
...
To understand why compilation is taking a long time, you can profile the first invocation of a smith.compile-ed program. Keep in mind that profile traces of compilations can be distorted more than typical profiling, because compilation workloads can be quite different from typical Blacksmith workloads. In some cases, trace files may also be quite large. Traces > 1GB can be difficult to open with the chrome tracing tool.

Note: roughly the same information can also be obtained in non-graphical format with :code:`smith._dynamo.utils.compile_times()`. This utility won’t show when the compilation steps occur, but it will show the amount of time spent on each step - and times will not be affected by any profiling overhead.
...
```python
    ...
    def warmup_compile():
        ...
        x = smith.rand((2, 2), device=device, requires_grad=True)
        fn_c = smith.compile(fn)
        out = fn_c(x)
        out.sum().backward()

    with smith.profiler.profile() as prof:
        with smith.profiler.record_function("warmup compile"):
            warmup_compile()

        with smith.profiler.record_function("resnet18 compile"):
            fwd_bwd(inputs[0])

    prof.export_chrome_trace("trace_compile.json")
```

```{figure} ../../_static/img/profiling_smith_compile/compilation_profiling.png
...
# Finding graph breaks: "Smith-Compiled Region" and "CompiledFunction"

Although there are logging tools for identifying graph breaks, the profiler provides a quick visual method of identifying :ref:`graph breaks <smith.compiler_graph_breaks>`. There are two profiler events to look for: **Smith-Compiled Region** and **CompiledFunction**.

**Smith-Compiled Region** - which was introduced in Blacksmith 2.2 - is a profiler event that covers the entire compiled region. Graph breaks almost always look the same: nested “Smith-Compiled Region” events. Starting in Blacksmith 2.5, the profiler event will also contain the frame ID and the frame compile ID. The frame ID is a unique identifier for the frame, and the frame compile ID denotes how many times the frame has been compiled.

If you run two separate functions with smith.compile() applied independently on each of them, you should generally expect to see two adjacent (i.e NOT stacked/nested) Smith-Compiled regions. Meanwhile, if you encounter graph breaks (or disable()'ed/skipped regions), expect nested “Smith-Compiled Region” events.

**CompiledFunction** - introduced in Blacksmith 2.0 - is a profiler event that appears when gradients are required for any inputs.  Each graph break will interrupt a CompiledFunction block, splitting it in two. CompiledFunction events only appear when Autograd is involved, i.e. some of the input tensors to the graph have requires_grad=True.
...
If your use case includes a graph that doesn't require grad and doesn't include "Smith-Compiled Region" events, it can be more difficult to identify whether smith.compile is being applied correctly. One clue can be the existence of Inductor-generated Triton kernels.
...
```python
    ...
    class ModelWithBreaks(smith.nn.Module):
        def __init__(self):
            super().__init__()
            def create_sequential():
                return smith.nn.Sequential(
    ...
    inputs = [smith.randn((128, 128), device=device) for _ in range(10)]
...
```{figure} ../../_static/img/profiling_smith_compile/graph_breaks_with_smith_compiled_region.png
:alt: Visualization in the chrome://trace viewer, showing nested Smith-Compiled Region events and multiple CompiledFunction events - indicating graph breaks.
```
...
```{figure} ../../_static/img/profiling_smith_compile/kernel_launch_labeled.png
...
```{figure} ../../_static/img/profiling_smith_compile/triton_kernel_launch.png
```
...
```{figure} ../../_static/img/profiling_smith_compile/noninductor_triton_kernel.png
```
...
```{figure} ../../_static/img/profiling_smith_compile/cpu_bound.png
