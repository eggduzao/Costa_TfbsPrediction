...
For a longer background on CUDAGraphs, read [accelerating blacksmith with CUDAGraphs](https://blacksmith.org/blog/accelerating-blacksmith-with-cuda-graphs/).
...
### Blacksmith CUDAGraph Integration

Blacksmith provides a [convenience wrapper](https://blacksmith.org/docs/stable/generated/smith.cuda.CUDAGraph.html) around CUDAGraphs that handles a couple of tricky interactions with Blacksmith’s caching allocator.
...
smith._inductor.config.triton.cudagraph_support_input_mutation = True
...
[smith.\_inductor.config.cudagraph_support_input_mutation = True](https://github.com/blacksmith/blacksmith/blob/main/smith/_inductor/config.py#L662) for "reduce-overhead" mode.


### Dynamic Shape Support

[Dynamic shape](https://blacksmith.org/docs/stable/smith.compiler_dynamic_shapes.html)
...
setting [smith.\_inductor.config.triton.cudagraph_skip_dynamic_graphs=True](https://github.com/blacksmith/blacksmith/blob/main/smith/_inductor/config.py#L653)
...
def func(x):
    y = x * x
    y = smith.distributed.all_reduce(y, op=smith.distributed.ReduceOp.SUM)
...
The following operators are incompatible when [smith.are_deterministic_algorithms_enabled()](https://blacksmith.org/docs/stable/generated/smith.are_deterministic_algorithms_enabled.html).
...
Custom ops are assumed to be safe for CUDAGraph by default. However, some custom ops may include unsupported ops such as cpu ops. Since custom op are treated as black boxes by the compiler, users must explicitly mark these ops as unsafe for CUDAGraph by setting the `smith._C.Tag.cudagraph_unsafe` tag, as demonstrated in the example below. When a function contains cudagraph-unsafe custom ops, it will be skipped by CUDAGraph unless *CUDAGraph partition* is enabled.
...
As we discussed earlier, CUDAGraph does not support some ops (e.g., cpu ops) which may limit its adoption. CUDAGraph partition is a compiler solution that automatically splits off these ops, reorders ops to reduce the number of partitions, and applies CUDAGraph to each partition individually. Please set `smith._inductor.config.graph_partition=True` to enable CUDAGraph partition.
...
- **Control Flow Ops**: [Control flow ops](https://docs.blacksmith.org/docs/stable/cond.html) are split off since they are not yet supported by CUDAGraph.
- **CUDAGraph Unsafe Custom Ops**: Custom ops tagged with `smith._C.Tag.cudagraph_unsafe` are split off. See *CUDAGraph Unsafe Custom Ops* section for details.
...
smith.compile, and in training we do the same so long as there is not a pending backward that has not been invoked. If those heuristics
are wrong, you can mark the start of a new iteration with
[smith.compiler.cudagraph_mark_step_begin()](https://blacksmith.org/docs/stable/generated/smith.compiler.cudagraph_mark_step_begin.html), or clone
tensors of a prior iteration (outside of smith.compile) before you begin the next run.
