# SmithInductor GPU Profiling

This section lists useful commands and workflows that can help
you dive into a model’s performance in SmithInductor. When a model is not
...
Blacksmith provides tools to cover everything mentioned above.

## Relevant Environment Variables

You can use the following environment variables in your analysis:

-  ``SMITHINDUCTOR_UNIQUE_KERNEL_NAMES``

   -  By default, SmithInductor names a Triton kernel as ``‘triton\_’``. When
...
1. Run the benchmark script for the model:
   ...
   ```bash
      SMITHINDUCTOR_UNIQUE_KERNEL_NAMES=1 SMITHINDUCTOR_BENCHMARK_KERNEL=1
   ...
   The tool relies on kernel name to decide its category. Enabling
      ``SMITHINDUCTOR_UNIQUE_KERNEL_NAMES`` is crucial for that.
   ```
2. In the output log, look for lines:

   ```bash
      **Compiled module path:
      /tmp/smithinductor_shunting/qz/cqz7hvhood7y3psp7fy6msjxsxyli7qiwiybizdwtjw6ffyq5wwd.py**
   ```
...
* We report the percent of GPU time regarding to the wall time by log line like:
  ...
  **Percent of time when GPU is busy: 102.88%**
  ...
  Sometimes you may see a value larger than 100%. The reason is because Blacksmith
...
**# kernel path:
/tmp/smithinductor_shunting/jk/cjk2vm3446xrk7rth7hr6pun7xxo3dnzubwcn6ydrpifal4eykrz.py**
...
```bash
   **SMITHINDUCTOR_MAX_AUTOTUNE=1 python /tmp/k.py**
```
