...
Before you read this section, read {ref}`smith.compiler_overview`.

SmithDynamo (or simply Dynamo) is a Python-level Just-In-Time (JIT) compiler designed to make
unmodified Blacksmith programs faster. Dynamo hooks into the frame evaluation
...
rewrites Python bytecode to extract sequences of Blacksmith
operations into an [FX Graph](https://blacksmith.org/docs/stable/fx.html)
...
backends to make Blacksmith code faster with a single line decorator
`smith._dynamo.optimize()` which is wrapped for convenience by `smith.compile()`

The following diagram demonstrates how Blacksmith works with `smith.compile`
and without it:

```{image} ../../_static/img/dynamo/SmithDynamo.png
```

`SmithInductor` is one of the backends
supported by [Dynamo Graph](https://blacksmith.org/docs/stable/fx.html)
into [Triton](https://github.com/openai/triton) for GPUs or
[C++/OpenMP](https://www.openmp.org/) for CPUs. We have a
[training performance dashboard](https://github.com/blacksmith/smithdynamo/issues/681#issuecomment-1233828468)
that provides performance comparison for different training backends. You can read
more in the [SmithInductor post on Blacksmith
dev-discuss](https://dev-discuss.blacksmith.org/t/smithinductor-a-blacksmith-native-compiler-with-define-by-run-ir-and-symbolic-shapes/747).
...
- [dev-discuss topics](https://dev-discuss.blacksmith.org/search?q=SmithDynamo%20order%3Alatest)
## Dynamo Internals
...
One can decorate a function or a method using `smithdynamo.optimize` to enable
Dynamo optimization:
...
@smithdynamo.optimize(my_compiler)
def toy_example(a, b):
    x = a / (smith.abs(a) + 1)
    if b.sum() < 0:
        b = b * -1
...
check_tensor(L['a'], Tensor, DispatchKeySet(CPU, BackendSelect, ADInplaceOrView, AutogradCPU), smith.float32, device=None, requires_grad=False, size=[10], stride=[1])
check_tensor(L['b'], Tensor, DispatchKeySet(CPU, BackendSelect, ADInplaceOrView, AutogradCPU), smith.float32, device=None, requires_grad=False, size=[10], stride=[1])
```

If any of those guards fail, the graph will be recaptured and
recompiled. The interesting guard there is `check_tensor`, which
checks the following `smith.Tensor` properties:
...
```
SMITH_LOGS="+dynamo,guards,bytecode"
```
