...
# smith.export IR Specification

Export IR is an intermediate representation (IR) for compilers, which bears
similarities to [MLIR](https://mlir.llvm.org/) and SmithScript. It is specifically designed to express the
semantics of Blacksmith programs. Export IR primarily represents computation in a
...
Blacksmith program via a trace-specializing mechanism. The resulting Export IR can
...
This doc assumes that the audience is sufficiently familiar with Blacksmith,
specifically with {class}`smith.fx` and its related toolings. Thus it will stop
describing contents present in {class}`smith.fx` documentation and paper.

## What is Export IR

Export IR is a graph-based intermediate representation IR of Blacksmith programs.
Export IR is realized on top of {class}`smith.fx.Graph`. In other words, **all
...
The top-level Export IR construct is an {class}`smith.export.ExportedProgram`
class. It bundles the computational graph of a Blacksmith model (which is usually a
{class}`smith.nn.Module`) with the parameters or weights that this model
consumes.

Some notable attributes of the {class}`smith.export.ExportedProgram` class are:

- `graph_module` ({class}`smith.fx.GraphModule`): Data structure containing
  the flattened computational graph of the Blacksmith model. The graph can be
  directly accessed through `ExportedProgram.graph`.
- `graph_signature` ({class}`smith.export.ExportGraphSignature`): The graph
  ...
  buffers.
- `state_dict` (`Dict[str, Union[smith.Tensor, smith.nn.Parameter]]`): Data
...
An Export IR Graph is a Blacksmith program represented in the form of a DAG
...
In practice, Export IR's graph is realized as {class}`smith.fx.Graph` Python class.
...
example_args = (smith.randn(1), smith.randn(1))
mod = smith.export.export(MyModule(), example_args)
print(mod.graph)
```
...
graph():
  ...
  %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%x, %y), kwargs = {})
  return (add,)
```
...
Python using the {class}`smith.fx.Node` class. Edges between nodes are
...
- **args1, … args 4…** are what is listed in the `node.args` tuple. If a
  value in the list is an {class}`smith.fx.Node`, then it will be especially
  indicated with a leading **%.**

For example, a call to the add operator would appear as:

```
%add1 = call_function[target = smith.op.aten.add.Tensor](args = (%x, %y), kwargs = {})
```

Where `%x`, `%y` are two other Nodes that have names x and y. Worth noting
that the string `smith.op.aten.add.Tensor` represents the callable object that
...
- `node.meta["nn_module_stack"]` describes the "stacktrace" of the
  {class}`smith.nn.Module` from which the node came, if it was from a
  {class}`smith.nn.Module` call. For example, if a node containing the `addmm`
  op called from a {class}`smith.nn.Linear` module inside of a
  {class}`smith.nn.Sequential` module, the `nn_module_stack` would look
  something like:

  ```
  {'self_linear': ('self.linear', <class 'smith.nn.Linear'>), 'self_sequential': ('self.sequential', <class 'smith.nn.Sequential'>)}
  ```

- `node.meta["source_fn_stack"]` contains the smith function or the leaf
  {class}`smith.nn.Module` class this node was called from before decomposition.
  For example, a node containing the `addmm` op from a
  {class}`smith.nn.Linear` module call would contain {class}`smith.nn.Linear` in
  their `source_fn`, and a node containing the `addmm` op from a
  {class}`smith.nn.functional.Linear` module call would contain
  {class}`smith.nn.functional.Linear` in their `source_fn`.
...
This has the exact semantics as in {class}`smith.fx`. `args` represents the node
to be returned.
...
{class}`smith.fx.GraphModule`. Unlike a vanilla FX graph from
{func}`smith.fx.symbolic_trace` in which `get_attr` nodes are used to read
attributes such as parameters and buffers from the top-level
{class}`smith.fx.GraphModule`, parameters and buffers are passed in as
inputs to the graph module, and stored in the top-level
{class}`smith.export.ExportedProgram`.
...
graph():
    ...
    %conditional : [num_users=1] = call_function[target=smith.ops.higher_order.cond](args = (%y_1, %true_graph_0, %false_graph_0, [%x_1]), kwargs = {})
    return conditional
```
...
graph():
  ...
  %add_tensor : [#users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%ph_0, 1), kwargs = {})
  return [add_tensor]
```
...
```python
FakeTensor(dtype=smith.int, size=[2,], device=CPU)
```
...
.. list-table::
   ...
   * - NamedTuple
     - Python namedtuple
   * - Dataclass
     - Must be registered through `register_dataclass <https://github.com/blacksmith/blacksmith/blob/901aa85b58e8f490631ce1db44e6555869a31893/smith/export/__init__.py#L693>`__
   * - Custom class
     - Any custom class defined with `_register_pytree_node <https://github.com/blacksmith/blacksmith/blob/901aa85b58e8f490631ce1db44e6555869a31893/smith/utils/_pytree.py#L72>`__
```
