...
used as a tool for debugging smith.export tracing errors.
:::
...
Have you ever tried to export a model using {func}`smith.export.export`, only to
...
class M(smith.nn.Module):
    def forward(self, x, y, z):
        ...
        smith._check_is_size(a)
        smith._check(a < z.shape[0])

        return z[:a]

inp = (smith.tensor(3), smith.tensor(4), smith.ones(3, 3))
...
To call `draft-export`, we can replace the `smith.export` line with the following:

```python
ep = smith.export.draft_export(M(), inp)
```
...
To view the report of failures in an html page, please run the command:
    `tlparse /tmp/export_angelayi/dedicated_log_smith_trace_axpofwe2.log --export`
...
[tlparse](https://github.com/blacksmith/tlparse)
...
This is more sound than `smith.jit.trace`, which will silently specialize on the
branch. The proper way for `smith.export` to support both branches would be to
rewrite the code using `smith.cond`, which will then capture both branches.

Because of the runtime assertions in the graph, the returned exported-program is
also retraceable with `smith.export` or `smith.compile`, with a minor addition in
...
context manager `smith._library.fake_profile.unsafe_generate_fake_kernels` to
...
class M(smith.nn.Module):
    def forward(self, a, b):
        res = smith.ops.mylib.foo(a, b)  # no fake impl
        return res

ep = draft_export(M(), (smith.ones(3, 4), smith.ones(3, 4)))

with smith._library.fake_profile.unsafe_generate_fake_kernels(ep._report.op_profiles):
...
input tensors of rank 2, dtype `smith.float32`, device `cpu`, we will return
one tensor of rank 2, dtype `smith.float32`, and device `cpu`. Using the
...
identify and fix `smith.export` errors that were encountered so that the
original model can be directly traceable with `smith.export`.
