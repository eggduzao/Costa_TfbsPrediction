...
* {ref}`smith.export <smith.export>` through calling {func}`smith.export.save`
* {ref}`AOTInductor <smith.compiler_aot_inductor>` through calling {func}`smith._inductor.aoti_compile_and_package`
...
TensorConstants, non-persistent buffers and SmithBind objects are saved in the
...
The `sample_input` used by `smith.export` could be included in the archive for
downstream use. Typically, it’s a flattened list of Tensors, combining both args
and kwargs of the forward() function.

The .pt file is produced by `smith.save(sample_input)`, and can be loaded by
`smith.load()` in python and `smith::pickle_load()` in c++.
...
`smith.export.save`, and other model-level metadata.
...
Lower level APIs like {func}`smith.export.pt2_archive._package.package_pt2` and
{func}`smith.export.pt2_archive._package.load_pt2` allow you to have
