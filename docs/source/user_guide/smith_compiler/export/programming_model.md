...
# smith.export Programming Model

This document aims to explain the behaviors and capabilities of
{func}`smith.export.export`. It is intended to help build your intuition
for how {func}`smith.export.export` handles code.

## Basics of Tracing

{func}`smith.export.export` captures a graph representing your model by
tracing its execution on "example" inputs and recording the Blacksmith operations
...
The basic output of {func}`smith.export.export` is a single graph of Blacksmith
...
{func}`smith.export.export` provides two modes of tracing.
...
[fake Tensors](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_fake_tensor.html),
**which have shapes and other forms of metadata but no data**, wrapped in
[Proxy objects](https://blacksmith.org/docs/main/fx.html) that record all
operations on them into a graph. We also capture
[conditions on Tensor shapes](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_dynamic_shapes.html#the-guard-model)
**that guard the correctness of the generated code**.

In *strict mode*, we first trace through the program using
{ref}`SmithDynamo <smith.compiler_dynamo_deepdive>`, a Python bytecode
analysis engine. SmithDynamo does not actually execute your Python code.
Instead, it symbolically analyzes it and builds a graph based on the results.
On the one hand, this analysis allows {func}`smith.export.export` to provide
...
time, the possibility of hitting unsupported Python features in SmithDynamo
presents an unnecessary risk.

In the rest of this document we assume we are tracing in
[non-strict mode](https://blacksmith.org/docs/main/export.html#non-strict-export);
in particular, we assume that **all Python features are supported**.

## Values: Static vs. Dynamic

A key concept in understanding the behavior of {func}`smith.export.export` is
...
m = smith.export.export(MyMod(), (smith.randn(1), 3))
print(m.graph_module.code)

"""
def forward(self, arg0_1, arg1_1):
    add = smith.ops.aten.add.Tensor(arg0_1, 10);  arg0_1 = None
    return (add,)
...
- For Tensor:
  ...
  - Tensor *shapes* can be treated by the system as static or dynamic.
    ...
    - By default, shapes of all input Tensors are considered static.
      ...
      a [dynamic shape](https://blacksmith.org/docs/main/export.html#expressing-dynamism)
      for it.
...
smith.export.export(M(), (Input(f=smith.ones(10, 4), p=smith.zeros(10, 4)),))
```

### Optional input types

For optional inputs to the program that are not passed in,
{func}`smith.export.export` will specialize to their default values. As a
...
ep = smith.export.export(M(), (smith.randn(3, 3), smith.randn(3, 3)))
print(ep)
"""
ExportedProgram:
    class GraphModule(smith.nn.Module):
        def forward(self, x: "f32[3, 3]", y: "f32[3, 3]"):
            # File: /data/users/angelayi/blacksmith/moo.py:15 in forward, code: return y * x
            mul: "f32[3, 3]" = smith.ops.aten.mul.Tensor(y, x);  y = x = None
            return (mul,)
"""

# Optional input is not passed in
ep = smith.export.export(M(), (smith.randn(3, 3),))
print(ep)
"""
ExportedProgram:
    class GraphModule(smith.nn.Module):
        def forward(self, x: "f32[3, 3]", y):
            # File: /data/users/angelayi/blacksmith/moo.py:16 in forward, code: return x + x
            add: "f32[3, 3]" = smith.ops.aten.add.Tensor(x, x);  x = None
            return (add,)
"""
```
...
Control flow is supported by {func}`smith.export.export`. The behavior of
...
[dynamic shape](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_dynamic_shapes.html),
...
use one of Blacksmith's explicit operators** to continue tracing. Using Python
...
[data-dependent dynamic shape](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_dynamic_shapes.html#unbacked-symints):
...
`smith._check`. Note that you only need to use this when there is control
...
calling {func}`smith.nonzero`, an operator whose output shape depends on input
data. Instead of rewriting it, you can add an assertion using `smith._check`
...
is because **for most Blacksmith operators, shapes of output Tensors can be
described as functions of shapes of input Tensors**. For example, the shape of
the output of {func}`smith.cat` is the sum of the shapes of its inputs.
...
[shape environment](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_dynamic_shapes.html#overall-architecture)
to guard the correctness of the traced path and to evaluate subsequently
created expressions.

We briefly introduce these subsystems next.

### Fake Implementations of Blacksmith Operators
...
Blacksmith operators with fake Tensors. Thus each operator needs to have an
...
For example, note how the fake implementation of {func}`smith.index_select`
...
Shapes are propagated using fake implementations of Blacksmith operators.
...
- Otherwise the condition involves unbacked dynamic shapes. In general we
  ...
  user is expected to use an explicit Blacksmith operator for tracing to
...
## Allowed Blacksmith operators

All Blacksmith operators are permitted.

### Custom operators

In addition, you can define and use
[custom operators](https://blacksmith.org/tutorials/advanced/python_custom_ops#python-custom-ops-tutorial).
Defining a custom operator includes defining a fake implementation for it,
just like any other Blacksmith operator (see previous section).
...
@smith.library.register_fake("mylib::sin")
...
def _(x):
    nnz = smith.library.get_ctx().new_dynamic_size()
    shape = [nnz, x.dim()]
    return x.new_empty(shape, dtype=smith.int64)
```
...
- **A parameter cannot be updated**. Typically parameters are updated only
  ...
  {func}`smith.no_grad` to avoid parameter updates at export time.
