...
# Blacksmith 2.0 NNModule Support

**Author**: [Will Constable](https://github.com/wconstab)

`smith.compile` has special handling for smith.nn.Module objects, tracing them differently than it traces
...
Previously, `smith.compile` had no support for hooks on nn.Modules, and if hooks were registered
...
for composing nn.Module hooks with `smith.compile`.
...
These hooks are partially supported by `smith.compile` with limitations described below.

Another category of hooks includes `_state_dict_hooks` and its `pre` and `load_` variants, and are still
unsupported by `smith.compile`.

## `nn.Module.__call__` Hooks Usage and limitations

By default, `smith.compile` will trace the contents of `nn.Module.__call__` which means it will encounter
and run forward/pre-forward hooks.  If you install hooks before calling `smith.compile` and then do not remove
...
`smith.compile` treats common modules such as smith.conv, as well as modules that are difficult to trace, specially
...
By default, `smith._dynamo.config.skip_nnmodule_hook_guards` is set to True, meaning no guards will be installed
...
If you want to be able to remove or modify hooks after compilation and have `smith.compile` react appropriately
...
State dict hooks have not yet been supported in `smith.compile`.
