...
`smith.compile` specifically doesn’t handle distributed wrapper modules like DDP or FSDP very well,
so consider applying `smith.compile` to the inner module passed to the wrapper.
...
Due to nuances to how `smith.compile` interacts with `nn.Module` instances,
