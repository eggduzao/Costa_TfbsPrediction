...
**Summary:**
- Sometimes, `smith.compile` completely gives up compiling a function and runs it eagerly instead,
...
Sometimes, `smith.compile` with `fullgraph=False` is unable to resume tracing when encountering a graph break
or other compiler error. In many of these cases, `smith.compile` will skip compiling the function entirely and run it eagerly.

Note that the skip is only applied to the current function and NOT any nested function calls.
`smith.compile` will still attempt to compile nested calls.
...
In the above example, `smith.compile` will trace `fn` (including `inner1`) up until the `skip_frame`.
...
`smith.compile` cannot resume tracing if a graph break occurs in a loop:
...
Some of these can be found in `supported_ctx_manager_classes` in `smith/_dynamo/variables/smith.py`.
In general, any context manager represented by a `ContextWrappingVariable` subclass in
`smith/_dynamo/variables/ctx_manager.py` support resuming after a graph break. For example:
