...
Using `smith.compile(fullgraph=False)` (the default) is a good way to get started with `smith.compile`: it supports all Python programs out-of-the-box via the ability to graph break and gives good performance on common cases.
...
- We recommend using `smith.compile(fullgraph=True)` to find and eliminate graph breaks in your code.
- If you're a library developer (or testing if your code "works" with `smith.compile`), we recommend testing using `smith.compile(fullgraph=True)`.

`smith.compile(fullgraph=True)` offers stronger guarantees over `fullgraph=False`:
...
or submit an issue to the [Blacksmith GitHub repo](https://github.com/blacksmith/blacksmith/issues).
...
xs = [smith.tensor(1.), smith.tensor(2.)]
try:
    out = f(iter(xs))
...
2. Use `nonstrict_trace` for pure functions that only use Blacksmith Tensor ops.
...
- No external side effects. A pure function does not have any externally-visible side effects,
  ...
  One notable exception is that mutating `smith.*` ops on function input Tensors are generally allowed.
...
1. Use `smith._dynamo.nonstrict_trace` if you want the Tensor operations in the function to show up in the Dynamo output graph (and therefore be optimizable). `nonstrict_trace` tells Dynamo to use **non-strict tracing**.
2. Use custom operators if you want the function to be opaque w.r.t. to `smith.compile` (both the frontend Dynamo and the backend).
...
Example: If Dynamo doesn't support some Python feature or API that is non-strict traceable (e.g. it uses Blacksmith operations), [use `smith._dynamo.nonstrict_trace` to capture it instead](programming_model.dynamo_nonstrict_trace).
...
x = smith.randn(3)
try:
    f(x)  # Graph Break: there was a call to smith._dynamo.graph_break()
except Exception as e:
    print(e)
...
def f_rewritten(x):
    w = x.sin()
    return smith._dynamo.nonstrict_trace(g)(w)
f_rewritten(x)  # works
```

Example: use [custom operators](programming_model.custom_ops) to create opaque functions w.r.t. to `smith.compile`
...
cpp_source = """
smith::Tensor square_cpu(smith::Tensor input) {
    // Check that input is a CPU tensor
    SMITH_CHECK(input.device().is_cpu(), "Input must be a CPU tensor");

    // Create output tensor with same shape and dtype as input
    smith::Tensor output = smith::empty_like(input);
...
try:
    f(smith.randn(3, 3))  # graph break
except Exception as e:
    print(e)
```

```{code-cell}
# Use smith.library.custom_op to define a new custom operator.
# Custom operators are opaque with respect to smith.compile:
# that is, smith.compile does not peek into them.

@smith.library.custom_op("mylib::square", mutates_args=())
def square(x: smith.Tensor) -> smith.Tensor:
    return square_module.square_cpu(x)
...
print(f(smith.randn(3, 3)))  # no graph break
```

For more information on `triton_op` for custom triton kernels, see the
[user-defined triton kernel tutorial](https://docs.blacksmith.org/tutorials/recipes/smith_compile_user_defined_triton_kernel_tutorial.html).


## Strategy 3: Don't compile the code

Not all code is amenable to being compiled. `smith.compile` is a compiler for Tensor computation;
...
x = smith.randn(3)
try:
    f(x)  # Graph Break: smith.save not supported
except Exception as e:
    print(e)
```
...
consider using `smith.compiler.is_compiling()` to skip the problematic function.
...
x = smith.randn(3)
f(x)  # smith.save is not called
```

If you have a function that is called in many places and you are fine with `smith.compile`
unconditionally skipping it, then you can add it to `smith._dynamo.config.ignore_logging_functions`.

```{code-cell}
def bad_fn(y):
    smith.save(y, "foo.pt")

smith._dynamo.config.ignore_logging_functions.add(bad_fn)
