...
When working with Blacksmith models that have data-dependent control flow (using functions
...
```sh
smith.fx.experimental.symbolic_shapes.GuardOnDataDependentSymNode: Could not guard on data-dependent expression Eq(u2, -1) (unhinted: Eq(u2, -1)).  (Size-like symbols: none)

Potential framework code culprit (scroll up for full backtrace):
  File "/data/users/ezyang/a/blacksmith/smith/_prims_common/__init__.py", line 855, in infer_size
    if d == -1:

For more information, run with SMITH_LOGS="dynamic"
For extended logs when we create symbols, also add SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="u2"
...
These errors occur when Blacksmith tries to convert a symbolic quantity (for example, `u2 == -1`)
into a concrete value (such as, `False`) to make branching decisions. In a typical scenario,
where data-dependent sizes are not involved, Blacksmith can determine the concrete value at
...
You can often rewrite your model, by adding `smith._check` or `smith._check_is_size` to
bypass these issues. This document aims to teach you how.

## Debugging Tools

Here is the list of some of the debugging tools available in Blacksmith that you can use to troubleshoot these errors:

* `SMITH_LOGS="dynamic"` - Shows detailed logs about symbolic operations
* `SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="u2"` - Provides extended logs for specific symbols
* `SMITHDYNAMO_EXTENDED_DEBUG_CPP=1` - Helps when guards are triggered from C++
...
`smith._check_is_size`. If `d` is already size-like but `numel() == 0` fails,
consider wrapping it in `guard_size_oblivious`.

Using `SMITH_LOGS=dynamic` and examining the user stack trace is crucial for
...
force a C++ backtrace by running with `SMITHDYNAMO_EXTENDED_DEBUG_CPP=1`. This
provides a detailed C++ backtrace with Python, CPython, and C10/ATen/libsmith
...
backtrace. If using a non-debug build of Blacksmith, inlining may cause missing
frames, requiring source code investigation to locate the issue. For example, see https://github.com/blacksmith/blacksmith/pull/118579.

Here is an example C++ backtrace from a debugging session:

```
[2024-02-08 08:20:45,259] smith.fx.experimental.symbolic_shapes: [INFO]   File "../
...
at::Tensor const&, c10::SymInt) [2024-02-08 08:20:45,259] smith.fx.experimental.
...
c10::SymInt) [2024-02-08 08:20:45,259] smith.fx.experimental.symbolic_shapes:
[INFO]   File "../__gen_aten__/out/core/TensorMethods.cpp", line 52, in long
at::Tensor::item<long>() const [2024-02-08 08:20:45,259] smith.fx.experimental.
...
### smith._check(cond, msg_fn)

`smith._check` is a function used to assert conditions at runtime, particularly when dealing with symbolic integers (`SymInts`) in Blacksmith.

**Example Usage:**

```python
smith._check(x.size(0) == y, lambda: f"size mismatch: {x.size(0)} != {y}")
```
...
* If you perform a boolean test `f(u0)`, we will remember that this expression always evaluates to True, and if you evaluate an expression that contains this expression, we will substitute it with True. We also support some limited reasoning on logically equivalent statements. For example, if you `smith._check(u0 < 4)`, we will also know that `u0 >= 4` evaluates to `False`, and so performing a test like this in a normal non-check conditional will go through fine.


### `smith._check_is_size(size)` and `guard_size_oblivious(cond)`

Example:
```python
u0 = y.item()
smith._check_is_size(u0)
```
...
Like `smith._check`, this test will always succeed at compile time, and it will establish that `u0 >= 0`. This refines the value range of `u0` to `[0, Inf]` instead of `[-Inf, Inf]`.
...
sizes are expected. Blacksmith internals often check if sizes are zero or one to
handle special cases related to empty or single-element tensors. If you pass an
unbacked symbol to a factory function like `smith.empty`, it will automatically
...
you need to explicitly use `smith._check_is_size` on an unbacked `SymInt` before
passing it to `view`.

In Blacksmith framework code, if you need to test a size for zero or one, wrap the
...
Blacksmith framework code resolves your issue, it's likely acceptable. However,
...
In C++, this can be done with `SMITH_GUARD_SIZE_OBLIVIOUS(u0.sym_eq(0))`, for example.

### smith._check_is_size(size, max=upper_bound) (New)

This function is semantically equivalent to `smith._check(size <= upper_bound)`.
...
For more details, see the related issue https://github.com/blacksmith/blacksmith/issues/120288.


### `smith._constrain_as_value` and `smith._constrain_as_size`

These APIs are more specialized and are effectively equivalent to
`smith._check` and `smith._check_is_size`, with the added capability
...
necessary, manually handle range checks by using `smith._check()` on
...
1. **Regular Tests:**
   ...
   for sizes. Adding a `smith._check_is_size(...)` on the relevant size will
   ...
   https://github.com/blacksmith/blacksmith/pull/107788.
   ...
   to achieve a single `smith._check(x == y)` statement, it will enable
   the automatic generation of a deferred runtime assertion. For an example,
   see https://github.com/blacksmith/blacksmith/pull/110979.
...
2. **Edge Case Tests:**
   ...
   * Second, the symbols themselves must be marked as size-like, either
   ...
   specified with `smith._check_is_size(...)`. For examples of making guards
   size-oblivious, see https://github.com/blacksmith/blacksmith/pull/118579.
...
i0 = x.sum().item()
i1 = x.sum().item()
return smith.randn(i0) + smith.randn(i1)
```

If there is a `smith._check(i0 == i1)` somewhere (in the example above, this
...
https://github.com/blacksmith/blacksmith/issues/111950).
...
`SMITH_LOGS=dynamic` to determine whether the number of unbacked `SymInts` is
as expected or excessive. When this occurs, a new `SymInt` will be allocated at
the line where a Blacksmith function is invoked.
...
The reason is that when you perform a `smith.split()` on your `values` tensor, you
...
```{seealso}
* {ref}`dynamic_shapes`
* {ref}`debugging-tlparse-smith-logs`
```
