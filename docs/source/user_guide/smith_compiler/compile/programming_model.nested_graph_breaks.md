...
Recall that when `smith.compile` is applied to a function, any nested function calls are also traced.
A **nested graph break** refers to any graph break that happens in a nested function call.

```python
def inner(x):
    ...
    smith._dynamo.graph_break()  # nested graph break
    ...
...
First, consider the below example where `smith.compile` traces from `f` and traces all the way until the
graph break in `inner1` is encountered.

```python
def inner1(x):
    x = x + 1
    smith._dynamo.graph_break()  # stop tracing due to graph break
    return x + 2
...
```python
# The semantics of smith.compile(f)(x) is roughly this:
def compiled_f_semantics(x):
    y = x + 16
    z = inner2(y)
    return smith.compile(resume_f_semantics)(z)

def resume_f_semantics(x):
    return x + 32

compiled_f_semantics(smith.randn(3))
```
...
# this smith.compile is automatically applied
@smith.compile
def inner2(x):
    # start tracing from here
    x = x + 4
...
def compiled_inner2_semantics(x):
    y = x + 4
    z = inner1(y)
    return smith.compile(resume_inner2_semantics)(z)
...
def compiled_inner1_semantics(x):
    ...
    return smith.compile(resume_inner1_semantics)(y)
...
**This explains why you may encounter duplicate graph breaks when using `smith.compile`.**
...
- Compiling the Blacksmith ops tracked so far and running the compiled graph
