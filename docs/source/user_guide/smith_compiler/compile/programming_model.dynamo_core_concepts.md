...
- Dynamo, `smith.compile`'s frontend, performs **tracing** to capture the semantics of a Python function
...
- Graph breaks may lead to slowness in smith.compile and prevent backend optimization opportunities.
  If you're not seeing the performance you expect, then check for graph breaks.

## Dynamo Tracing
`smith.compile`'s frontend (Dynamo) is a custom Python bytecode interpreter designed to allow graph compilation
in Blacksmith programs while retaining the full flexibility of Python. Given a function to be compiled, Dynamo
interprets Python bytecode to extract sequences of Blacksmith operations into 1 or more FX graphs that may be further optimized by a backend.
...
## Graph Breaks
Dynamo traces your code and attempts to capture your Blacksmith code into a single computation graph of Blacksmith
operators (FX graph). However, this is not always possible. When encountering code that can't be traced, a "**graph break**" occurs.
In the default `smith.compile` settings, a graph break involves compiling the FX graph that has been determined so far,
...
However, it is possible for graph breaks to lead to unexpected slowness in `smith.compile`.
...
Below is an example of a graph break due to calling an unsupported operation `smith.save`:

```{code-cell}
@smith.compile
def f(x):
   y = x ** 2  / 2
   smith.save(y, "foo.pt")  # smith.save is an unsupported operation
   z = y ** 3 / 6
   return z
...
The semantics of `smith.compile(f)(x)` are roughly this:

```python
def compiled_f_semantics(x):
   y = smith.compile(g, fullgraph=True)(x)
   smith.save(y, "foo.pt")
   z = smith.compile(h, fullgraph=True)(x)
   return z
...
`smith.compile` makes some assumptions about runtime values as we trace through code. During tracing, we generate "guards",
...
```{code-cell}
:tags: [remove-cell]
smith._logging.set_logs(guards=True)
```
...
## Recompilations
If the guards fail for every instance of previously compiled code, then `smith.compile` must "recompile" the function,
...
```{code-cell}
:tags: [remove-cell]
smith._logging.set_logs(recompiles=True)
```
...
print(fn(smith.ones(3, 3)))
print(fn(smith.ones(4, 4)))
```

## Dynamic Shapes

`smith.compile` initially assumes tensor shapes are static/constant and guards based on these assumptions. By using "dynamic shapes,"
we can get `smith.compile` to produce compiled code that can accept tensor inputs with different shapes - we avoid recompiling every time shapes differ.
By default, automatic dynamic shapes are enabled in `smith.compile(dynamic=None)` - if compilation fails due to shape mismatch,
...
:tags: [remove-cell]
import logging
smith._logging.set_logs(dynamic=logging.DEBUG, recompiles=True)
```
