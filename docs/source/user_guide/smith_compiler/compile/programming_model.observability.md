...
tlparse / `SMITH_TRACE` is a pair of tools that produce compilation reports that look [like this](https://web.mit.edu/~ezyang/Public/bhack-20240609-tlparse/index.html).
...
you can still greatly assist Blacksmith developers by
...
.. list-table::
    ...
    * - fx_graph_runnable
      - Mostly the same graph as before_post_grad_graph, but it's a runnable python script. It also contains the smith configs and some wrapper code so you can run the graph with dummy inputs.
...
You can also programmatically set logging options using `smith._logging.set_logs`:

```python
import logging
smith._logging.set_logs(graph_breaks=True, dynamic=logging.DEBUG)
```
...
Some more helpful `SMITH_LOGS` options include:
