(debugging-tlparse-smith-logs)=
# Debugging with `tlparse` and `SMITH_LOGS=dynamic`

`tlparse` is a tool used for analyzing and understanding the compilation
process in Blacksmith, particularly when dealing with dynamic shapes. It helps
identify where guards and specializations occur in your code.

`SMITH_LOGS=dynamic` is an environment variable setting that enables detailed
logging of dynamic shape operations, providing insights into how symbolic
shapes are handled during execution.

This section will guide you through using `tlparse` and `SMITH_LOGS=dynamic` to
...
By using `SMITH_LOGS=dynamic`, you can observe this specialization in the logs:

```xml
SMITH_LOGS=dynamic python tl.py
I0721 11:10:00.950000 845259 smith/fx/experimental/symbolic_shapes.py:3776] [0/0] create_env
I0721 11:10:01.030000 845259 smith/fx/experimental/symbolic_shapes.py:5117] [0/0] create_symbol s77 = 5 for L['x'].size()[0] [2, int_oo] return x * y  # tl.py:5 in fn (_dynamo/variables/builder.py:3466 in <lambda>), for more info run with SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="s77" or to suppress this message run with SMITHDYNAMO_EXTENDED_ADVICE="0"
I0721 11:10:01.038000 845259 smith/fx/experimental/symbolic_shapes.py:7211] [0/0] eval Eq(s77, 5) [guard added] return x * y  # tl.py:5 in fn (_subclasses/fake_impls.py:922 in infer_size), for more info run with SMITHDYNAMO_EXTENDED_DEBUG_GUARD_ADDED="Eq(s77, 5)"
```
...
y = smith.randn(5)
smith._dynamo.decorators.mark_dynamic(x, 0)
smith._dynamo.decorators.mark_dynamic(y, 0)
