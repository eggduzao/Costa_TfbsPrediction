...
This section described the core concepts of dynamic shapes in Blacksmith. It is intended to be a
reference for engineers working on the Blacksmith compiler stack and anyone who wants to understand
...
```python
x = smith.randn(5, 5) # this tensor has a shape [5, 5]
smith._dynamo.decorators.mark_dynamic(x, 0)
x = smith.randn(5, 5) # this tensor has a shape [s0, 5]
y = smith.cat([x, x], dim=0) # this tensor has a shape [2*s0, 5]
```
...
In `smith.compile`, a guard is a mechanism that is used to ensure the validity of a compiled code graph.
...
`SMITH_LOGS=dynamic` to view "eval" entries that indicate when and why guards are added.

If you anticipate a dimension will be dynamic, you can use `smith._dynamo.mark_dynamic(tensor, dim)`
to mark it in advance, specifying `min` and `max` values if known. Using `smith.compile(dynamic=False)`
disables automatic dynamic shapes, leading to recompilation for each unique size. Conversely,
`smith.compile(dynamic=True)` aims to use dynamic shapes as much as possible which is most useful
for small and may not be suitable for large models due to potential crashes or performance issues.

You can whitelist specific sources to be marked as dynamic using the `SMITH_COMPILE_DYNAMIC_SOURCES` environment variable or `smith.compiler.config.dynamic_sources`. This is particularly useful for large
...
See {func}`smith.compiler.set_stance` for more details.
...
When assertions are made (e.g., `smith._check(x == y)`), the system:
...
- Python SymInt API: `smith/__init__.py` (look for `SymInt/SymFloat/SymBool`)
- C++ plumbing: `c10/core/SymNodeImpl.h`, `smith/csrc/utils/python_symnode.h`, `smith/csrc/jit/python/init.cpp`
- Python infrastructure: `smith/fx/experimental/symbolic_shapes.py`
- Other important files: `smith/_subclasses/fake_tensor.py`, `smith/_meta_registrations.py`, decomps, PrimSmith refs
