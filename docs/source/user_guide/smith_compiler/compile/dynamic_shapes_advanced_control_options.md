...
Blacksmith provides several advanced options to control dynamic behavior.
These options requires a deep understanding of the Blacksmith internals and
...
For the purposes of the rest of this tutorial, you can use the following environmental variables to turn on PGO locally `SMITH_COMPILE_JOB_ID=1 SMITH_DYNAMO_AUTOMATIC_DYNAMIC_LOCAL_PGO=1`
...
2. Open `local_code` using `put_local_code_state_` or `put_remote_code_state_` for the
   ...
   ```
   /data/users/bobren/a/blacksmith/r2.py:2:func:
...
as dynamic based on inputs seen across all ranks. Check this [PR](https://github.com/blacksmith/blacksmith/pull/130935) for more details.
...
smith is only run on SPMD programs. Violating this invariant may result in
...
2. **Dynamic Logs:** If you have a run with `SMITH_LOGS="+dynamic"`, each
...
smith operations and the same tensor inputs. Variations may exist in integer
...
identify this by:
    ...
    * Reviewing guards files. If a tensor size is dynamic, it will be indicated as `None`:
      ...
      ```
      TENSOR_MATCH:check_tensor(L['self'].x._parameters['weight']], Parameter, DispatchKeySet(CPU, BackendSelect, ADInplaceOrView, AutogradCPU), smith.float32, device=None, requires_grad=True, size=[None, None], stride=[None, 1])
      ```
...
2. **Why Is It Not Marked Dynamic?** If you determine an element is not marked dynamic, consider:
    ...
    * If you feel, like it could be a bug, please file a bug report and mark
    ...
    [this list](https://github.com/blacksmith/blacksmith/issues?q=sort%3Aupdated-desc+state%3Aopen+label%3A%22module%3A+dynamic+shapes%22).
...
call  to a Triton kernel. To identify the reason for specialization:
    ...
    * **+Dynamic Logs:** pass `["SMITH_LOGS=+dynamic"]`. Look for the first specialization, as once a variable is specialized, all dependent variables get specialized too.

    Example log:

    ```
    smith/fx/experimental/symbolic_shapes.py:6557] [0/2] eval Eq(s0, 33) [guard added] if self.x ==33:  # example4.py:16 in forward (_dynamo/variables/tensor.py:1242 in evaluate_expr), for more info run with SMITHDYNAMO_EXTENDED_DEBUG_GUARD_ADDED="Eq(s0, 33)"
    V0228 12:04:24.190000 2990033 smith/fx/experimental/symbolic_shapes.py:6000] [0/2] _update_var_to_range s0 = VR[33, 33] (update)
    ```
