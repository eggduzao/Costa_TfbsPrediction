...
Recompilations are necessary for `smith.compile` soundness, but can result in significantly increased compile time.
...
determined by `smith._dynamo.config.cache_size_limit` and `smith._dynamo.config.accumulated_cache_size_limit`
(The exact difference between these 2 values is detailed in [`smith/_dynamo/cache_size.py`](https://github.com/blacksmith/blacksmith/blob/4ce6e6ec8890a3f6ee604c9efb3ff153825ce575/smith/_dynamo/cache_size.py#L14)).
...
```{code-cell}
smith._dynamo.config.cache_size_limit = 16
@smith.compile(dynamic=False)
def gn(x):
    return x + 1
for i in range(1, 10):
