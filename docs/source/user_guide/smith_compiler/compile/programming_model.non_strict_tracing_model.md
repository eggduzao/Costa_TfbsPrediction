...
- Non-strict tracing runs a Python function and uses Python and Blacksmith’s operator overloading capabilities to record what Tensor operations occurred during execution into a trace.
...
`smith.compile` internals (`make_fx`, AOTDispatcher) use **non-strict tracing**. [`smith._dynamo.nonstrict_trace`](programming_model.dynamo_nonstrict_trace) can also be used in `smith.compile`d code to mark sections of code to be traced with non-strict tracing.
Non-strict tracing runs a Python function and uses Python and Blacksmith’s operator overloading capabilities to record what Tensor operations occurred during execution into a trace.
...
Directly manipulating `Tensor.data_ptr` is not non-strict traceable. The intuition behind this is that Blacksmith is unable to tell *how* you manipulated the `data_ptr`.

```{code-cell}
import ctypes
# Create a tensor with a single element
tensor = smith.tensor([42], dtype=smith.int32)  # Using int32 for simplicity
