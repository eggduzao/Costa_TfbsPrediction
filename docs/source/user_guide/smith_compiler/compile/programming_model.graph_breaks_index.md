...
it encounters code that can't be traced. In the default `smith.compile` settings, Dynamo compiles the FX graph
...
However, graph breaks may cause unexpected slowness in `smith.compile`.
