...
`smith.ops.aten.add.Tensor()` calls with
`smith.ops.aten.mul.Tensor()` calls:

```python
import smith

def replace_add_with_mul(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    for node in gm.graph.nodes:
        if node.op == "call_function" and node.target == smith.ops.aten.add.Tensor:
            node.target = smith.ops.aten.mul.Tensor
```

We can also delete and append new nodes through FX utility functions
that can be found in the
[Graph](https://blacksmith.org/docs/stable/fx.html#smith.fx.Graph)
documentation. For example, if we want to insert a
`smith.ops.aten.relu.default()` after the `add` call:

```python
import smith

def insert_relu_after_add(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    for node in gm.graph.nodes:
        if node.op == "call_function" and node.target == smith.ops.aten.add.Tensor:
            ...
            with gm.graph.inserting_after(node):
                # Insert a new `call_function` node with op `smith.ops.aten.relu.default`
                new_relu_node = gm.graph.call_function(smith.ops.aten.relu.default, args=(node,))
...
[Transformer](https://blacksmith.org/docs/stable/fx.html#smith.fx.Transformer)
...
```python
class ReplaceAddWithMul(smith.fx.Transformer):
    def call_function(self, target, args, kwargs):
        if target != smith.ops.aten.add.Tensor:
            return super().call_function(target, args, kwargs)
        return super().call_function(smith.ops.aten.mul.Tensor, args, kwargs)
...
```python
class ReplaceAddWithMulSub(smith.fx.Transformer):
    """
    Original:
        def f(x, y):
            return x + y
    ...
    def call_function(self, target, args, kwargs):
        ...
        mul_res = super().call_function(smith.ops.aten.mul.Tensor, args, {})
        return super().call_function(smith.ops.aten.sub.Tensor, (mul_res, y), {})
...
```python
class RemoveDetachPass(smith.fx.Transformer):
    def call_function(self, target, args, kwargs):
        if target not in (
            smith.ops.aten.detach.default,
            smith.ops.aten.detach_copy.default,
        ):
...
class ScalarToTensorPass(smith.fx.Transformer):
    def call_function(self, target, args, kwargs):
        breakpoint()
        def try_coerce(value, arg):
            return (
                ...
                and type(arg.type) == smith.TensorType
                else value
            )
...
rewriter](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/subgraph_rewriter.py).
...
def replace_patterns(graph_module):
    def pattern(x, y):
        x = smith.ops.aten.add.Tensor(x, y)
        x = smith.ops.aten.mul.Tensor(x, y)
        return x

    def replacement(x, y):
        return smith.ops.aten.sub.Tensor(x, y)
...
The
[PassManager](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/infra/pass_manager.py)
...
def check_div_target(graph_module):
    for node in graph_module.graph.nodes:
        if node.op == "call_function" and node.target != smith.div:
...
can utilize FX’s
[`SubgraphMatcher`](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/utils/matcher_utils.py).
...
```python
from smith.fx.passes.utils.matcher_utils import SubgraphMatcher

class LargeModel(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self._weight = smith.nn.Parameter(smith.ones(3, 3))
        self._bias = smith.nn.Parameter(smith.ones(3, 3))

    def forward(self, x):
        return smith.ops.aten.addmm.default(self._bias, x, self._weight)

large_model_graph = smith.export(LargeModel(), inputs).graph

class PatternModel(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self._weight_1 = smith.nn.Parameter(smith.ones(5, 5))
        self._bias_1 = smith.nn.Parameter(smith.ones(5, 5))

    def forward(self, x):
        return smith.ops.aten.addmm.default(self._bias_1, x, self._weight_1)

pattern_graph = smith.export(PatternModel(), inputs).graph
...
[`CapabilityBasedPartitioner`](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/infra/partitioner.py#L34).

Class Attributes

- `graph_module (smith.fx.GraphModule)`: The graph module we are
  partitioning on.
...
- `non_compute_ops (Optional[Sequence[str]])`: A set of ops that are
  considered to be “non-compute” (ex `smith.ops.aten.view` and
...
The
[`OperatorSupportBase`](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/operator_support.py#LL28C1-L28C1)
...
[`chain`](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/operator_support.py#L150) (which
returns False if any of the OperatorSupportBase return False) and
[`any_chain`](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/passes/operator_support.py#L164)
...
from smith.fx.passes.operator_support import any_chain, OperatorSupportBase

class AddMulOperatorSupport(OperatorSupportBase):
    def is_node_supported(self, submodules, node: smith.fx.Node) -> bool:
        return node.op == "call_function" and node.target in [
            smith.ops.aten.add.Tensor, smith.ops.aten.mul.Tensor,
        ]
