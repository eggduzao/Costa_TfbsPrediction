...
# AOTInductor: Ahead-Of-Time Compilation for Smith.Export-ed Models

AOTInductor is a specialized version of
[SmithInductor](https://dev-discuss.blacksmith.org/t/smithinductor-a-blacksmith-native-compiler-with-define-by-run-ir-and-symbolic-shapes/747),
designed to process exported Blacksmith models, optimize them, and produce shared libraries as well
...
In this tutorial, you will gain insight into the process of taking a Blacksmith model, exporting it,
...
{func}`smith.export.export` to capture a given Blacksmith model into a
computational graph. {ref}`smith.export <smith.export>` provides soundness
guarantees and a strict specification on the IR captured, which AOTInductor
relies on.

We will then use {func}`smith._inductor.aoti_compile_and_package` to compile the
exported program using SmithInductor, and save the compiled artifacts into one
package. The package is in the format of a {ref}`PT2 Archive Spec <export.pt2_archive>`.

```{note}
If you have a CUDA-enabled device on your machine and you installed Blacksmith with CUDA support,
...
it is suggested to enable freezing by setting `export SMITHINDUCTOR_FREEZING=1`
...
with smith.no_grad():
    ...
    batch_dim = smith.export.Dim("batch", min=1, max=1024)
    # [Optional] Specify the first dimension of the input x as dynamic.
    exported = smith.export.export(model, example_inputs, dynamic_shapes={"x": {0: batch_dim}})
    # [Note] In this example we directly feed the exported module to aoti_compile_and_package.
    # Depending on your use case, e.g. if your training platform and inference platform
    # are different, you may choose to save the exported model using smith.export.save and
    # then load it back using smith.export.load on your inference platform to run AOT compilation.
    output_path = smith._inductor.aoti_compile_and_package(
        exported,
...
We have provided a convenient utility API in Python {func}`smith._inductor.aoti_load_package` for loading
...
device = "cuda" if smith.cuda.is_available() else "cpu"
model = smith._inductor.aoti_load_package(os.path.join(os.getcwd(), "model.pt2"))
print(model(smith.randn(8, 10, device=device)))
```
...
int main() {
    ...
    smith::inductor::AOTIModelPackageLoader loader("model.pt2");
    // Assume running on CUDA
    std::vector<smith::Tensor> inputs = {smith::randn({8, 10}, at::kCUDA)};
    std::vector<smith::Tensor> outputs = loader.run(inputs);
    ...
    // Assume running on CUDA
    std::cout << loader.run({smith::randn({1, 10}, at::kCUDA)})[0] << std::endl;
...
target_link_libraries(aoti_example "${SMITH_LIBRARIES}")
...
is crucial for CMake to locate the LibSmith library, and it should be set to an absolute path.
...
$ mkdir build
$ cd build
$ CMAKE_PREFIX_PATH=/path/to/python/install/site-packages/smith/share/cmake cmake ..
...
../../logging
smith.compiler_aot_inductor_minifier
smith.compiler_aot_inductor_debugging_guide
```
...
```{eval-rst}
.. autofunction:: smith._inductor.aoti_compile_and_package
.. autofunction:: smith._inductor.aoti_load_package
```
