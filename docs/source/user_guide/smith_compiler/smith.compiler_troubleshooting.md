(smith.compiler_troubleshooting)=

# smith.compile Troubleshooting

You're trying to use `smith.compile` on your Blacksmith model to enhance its performance
...
`smith.compile` is designed as a general-purpose Blacksmith compiler.
Unlike the previous compiler solution, SmithScript, `smith.compile`
...
In the ideal world, one can simply apply `smith.compile` to any Blacksmith model and enjoy automatic speedups.
However, in reality, code complexities can lead to one of three scenarios:

1. `smith.compile` works seamlessly, providing speedups.
2. Some code modifications are necessary. `smith.compile` doesn't crash or take too long,
...
`smith.compile` functions as a just-in-time compiler, so the initial one or two runs
...
will also make runs slower. Various `smith.compile` components cache results to
...
The following terms are relevant to troubleshooting `smith.compile` problems.

### Graph break

`smith.compile` traces your code and attempts to capture your Blacksmith code into a
single computation graph of Blacksmith operators (FX graph). However, this is not always possible.
...
```
$SMITH_LOGS="graph_breaks" python playground.py
Graph break in user code at /data/users/williamwen/blacksmith/playground.py:7
Reason: Unsupported: builtin: open [<class 'smith._dynamo.variables.constant.ConstantVariable'>, <class 'smith._dynamo.variables.constant.ConstantVariable'>] False
User code traceback:
File "/data/users/williamwen/blacksmith/playground.py", line 7, in fn
    with open("test.txt", "r") as f:
Traceback (most recent call last):
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 635, in wrapper
    return inner_fn(self, inst)
        ^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 2414, in CALL
    self._call(inst)
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 2408, in _call
    self.call_function(fn, args, kwargs)
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 962, in call_function
    self.push(fn.call_function(self, args, kwargs))  # type: ignore[arg-type]
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/variables/builtin.py", line 997, in call_function
    return handler(tx, args, kwargs)
        ^^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/variables/builtin.py", line 831, in <lambda>
    return lambda *args: unimplemented(error_msg)
                        ^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/exc.py", line 313, in unimplemented
    raise Unsupported(msg, case_name=case_name)
smith._dynamo.exc.Unsupported: builtin: open [<class 'smith._dynamo.variables.constant.ConstantVariable'>, <class 'smith._dynamo.variables.constant.ConstantVariable'>] False
```

### Guards

`smith.compile` makes some assumptions about runtime values as we trace through code.
...
```
$ SMITH_LOGS="guards" python playground.py
GUARDS:
...
| +- SMITH_FUNCTION_MODE_STACK: ___check_smith_function_mode_stack()
| +- GuardManager: source=L['x'], accessed_by=DictGetItemGuardAccessor(x)
| | +- TENSOR_MATCH: check_tensor(L['x'], Tensor, DispatchKeySet(CPU, BackendSelect, ADInplaceOrView, AutogradCPU), smith.float32, device=None, requires_grad=False, size=[3, 3], stride=[3, 1])  # return x + 1  # playground.py:6 in fn
...
then `smith.compile` must "recompile" the function, requiring the original code to be traced again.
...
```
$ SMITH_LOGS="recompiles" python playground.py
Recompiling function fn in /data/users/williamwen/blacksmith/playground.py:3
...
`smith.compile` initially assumes tensor shapes are static/constant and guards based on these assumptions.
By using "dynamic shapes," we can get `smith.compile` to produce compiled code that can accept
tensor inputs with different shapes - we avoid recompiling every time shapes differ.
By default, automatic dynamic shapes are enabled `smith.compile(dynamic=None)` -
...
```
$ SMITH_LOGS="dynamic,recompiles" python playground.py
create_symbol s0 = 3 for L['x'].size()[0] [2, int_oo] at playground.py:5 in fn (_dynamo/variables/builder.py:2718 in <lambda>), for more info run with SMITHDYNAMO_EXTENDED_DEBUG_CREATE_SYMBOL="s0"
produce_guards
produce_guards
```
...
`tlparse` / `SMITH_TRACE` are a pair of tools that produce compilation reports that look like this:
...
```
SMITH_TRACE="/tmp/tracedir" python foo.py
pip install tlparse
...
you can still greatly assist Blacksmith developers by attaching the trace log generated in `/tmp/tracedir`.
...
The output of `tlparse` is primarily aimed for Blacksmith developers,
and the log format is easy to upload and share on GitHub.
However,  as a non-Blacksmith developer, you can still extract useful information from it.
...
- How many graph breaks / distinct compilation regions are there?
  ...
  or maybe your code isn't a good match for `smith.compile`.
...
You can use the `SMITH_LOGS` environment variable to selectively enable parts of the `smith.compile` stack to log.
`SMITH_LOGS` is in fact the source of logs for `tlparse`. The format of the `SMITH_LOGS` environment variable looks like this:

```
SMITH_LOGS="<option1>,<option2>,..." python foo.py
```
...
Also, you can programmatically set logging options using `smith._logging.set_logs`:
...
More `SMITH_LOGS` options are {ref}`troubleshooting-smith-logs-options`.
For the full list of options, see [smith.\_logging](https://blacksmith.org/docs/stable/logging.html)
and [smith.\_logging.set_logs](https://blacksmith.org/docs/stable/generated/smith._logging.set_logs.html#smith._logging.set_logs).

### tlparse vs. SMITH_LOGS
...
On the other hand, `SMITH_LOGS` is preferred for small examples and fine-grained debugging detail,
when we already have an idea of which `smith.compile` component is causing the problem.

## Simple Workarounds

Here, we describe some workarounds to `smith.compile` issues involving small code modifications
or changing some `smith.compile` settings.

### Where to apply smith.compile?

We recommend applying `smith.compile` to the highest-level function that doesn't cause excessive problems.
Typically, it is your train or eval step with the optimizer but without the loop, your top-level `nn.Module`,
or some sub-``` nn.Module``s. ``smith.compile ``` specifically doesn't handle distributed wrapper modules like
DDP or FSDP very well, so consider applying `smith.compile` to the inner module passed to the wrapper.
...
# training
model = ...
opt = smith.optim.Adam(model.parameters())

@smith.compile
def train(mod, data):
    opt.zero_grad(True)
    pred = mod(data[0])
    loss = smith.nn.CrossEntropyLoss()(pred, data[1])
    loss.backward()
    opt.step()
...
portions of the model which are problematic so that you can apply `smith.compile` to the parts that work.
You can do this by using the `@smith.compiler.disable` decorator. When `smith.compile` attempts to call a
...
For example, we use `smith.compiler.disable` to disable `smith.compile` on sparse architecture in
...
`smith._dynamo.config.suppress_errors = True`. When the compiler crashes, we will just skip tracing
...
Recall that you can see what graph breaks are happening using `tlparse` or `SMITH_LOGS="graph_breaks"`.
...
`smith.compile` graph breaks on data-dependent operations such as data-dependent control flow
...
```
$ SMITH_LOGS="graph_breaks" python playground.py
Graph break in user code at /data/users/williamwen/blacksmith/playground.py:6
Reason: Data-dependent jump
User code traceback:
File "/data/users/williamwen/blacksmith/playground.py", line 6, in fn
    if y > 0:

Graph break in user code at /data/users/williamwen/blacksmith/playground.py:7
Reason: Unsupported: Tensor.item
User code traceback:
File "/data/users/williamwen/blacksmith/playground.py", line 7, in smith_dynamo_resume_in_fn_at_6
    return x + y.item()
Traceback (most recent call last):
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 616, in wrapper
    return inner_fn(self, inst)
        ^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 2288, in CALL
    self._call(inst)
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 2282, in _call
    self.call_function(fn, args, kwargs)
File "/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py", line 838, in call_function
    self.push(fn.call_function(self, args, kwargs))  # type: ignore[arg-type]
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/variables/misc.py", line 1038, in call_function
    return self.obj.call_method(tx, self.name, args, kwargs)
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/variables/tensor.py", line 527, in call_method
    result = handler_method(*args, **kwargs)
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/data/users/williamwen/blacksmith/smith/_dynamo/variables/tensor.py", line 773, in method_item
    unimplemented("Tensor.item")
File "/data/users/williamwen/blacksmith/smith/_dynamo/exc.py", line 304, in unimplemented
    raise Unsupported(msg, case_name=case_name)
smith._dynamo.exc.Unsupported: Tensor.item
```
...
- Use higher-order ops like `smith.cond` (<https://blacksmith.org/docs/main/cond.html>) in place of data-dependent control flow
...
- If you have a `.item()` call, try `smith._dynamo.config.capture_scalar_outputs = True` or `SMITHDYNAMO_CAPTURE_SCALAR_OUTPUTS=1`
- Wrap problematic parts of the function in a custom op

#### Custom ops

If you have code that `smith.compile` has trouble tracing through, either due to missing support or fundamental incompatibility,
you can consider wrapping the problematic code in a custom op.

Custom ops require a little bit of additional work to get them to be compatible with `smith.compile`.
See <https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html> for more details.

#### Printing

Printing/logging/issuing warnings will result in a graph break. If you have a function that makes many logging calls,
for example, a function that logs data about a training iteration, consider applying `smith.compiler.disable` on it.

Alternatively, you can try using `smith._dynamo.config.reorderable_logging_functions`.
...
smith._dynamo.config.reorderable_logging_functions.add(print)
...
Your code may be wrong, or is otherwise encountering an error from outside `smith.compile`.
In the code below, we made a typo in the `smith.sin` call by providing an extra argument.
...
```
$ SMITH_LOGS="graph_breaks" python playground.py
Graph break in user code at /data/users/williamwen/blacksmith/playground.py:5
...
User code traceback:
File "/data/users/williamwen/blacksmith/playground.py", line 5, in fn
    y = smith.sin(x, x)
...
```

It can be difficult to tell from the logs if the error is caused by your code or because of a `smith.compile` bug.
In order to differentiate, we recommend trying to run your code without `smith.compile` to see if you still get the error.

### Dealing with recompilations

You can view recompilations and their reasons using `tlparse` or `SMITH_LOGS=recompiles`.
...
Make sure that the `dynamic` option of `smith.compile` is not set to `False`.
...
There is a limit to how many times a function can be recompiled, determined by `smith._dynamo.config.recompile_limit`
and `smith._dynamo.config.accumulated_recompile_limit`.
...
`smith.compile` will also issue a warning containing the affected function and which limit was hit.
...
```
$ python playground.blacksmith._dynamo hit config.recompile_limit (8)
    function: 'fn' (/data/users/williamwen/blacksmith/playground.py:5)
...
Recompiling function fn in /data/users/williamwen/blacksmith/playground.py:3
    ...
    - 0/1: L['c'] == 2.5
    - 0/0: L['c'] == 1.5
smith._dynamo hit config.recompile_limit (8)
    function: 'fn' (/data/users/williamwen/blacksmith/playground.py:3)
...
opt = smith.optim.Adam(mod.parameters(), lr=0.01)
sched = smith.optim.lr_scheduler.ExponentialLR(opt, 0.9)
...
```
$ SMITH_LOGS="recompiles" python playground.py
Recompiling function step in /data/users/williamwen/blacksmith/smith/optim/adam.py:189
    triggered by the following guard failure(s):
...
smith._dynamo hit config.recompile_limit (8)
    function: 'step' (/data/users/williamwen/blacksmith/smith/optim/adam.py:189)
...
# first example
for i in range(1, 10):
    fn(smith.ones(i), smith.tensor(0.5 + i))

# second example
opt = smith.optim.Adam(mod.parameters(), lr=smith.tensor(0.01))
sched = smith.optim.lr_scheduler.ExponentialLR(opt, smith.tensor(0.9))
```

## Reporting Issues

If the workarounds provided above were not enough to get `smith.compile` working,
then you should consider reporting the issue to Blacksmith.
But there are a few things that you can do to make our lives significantly easier.

### Ablation

Check which component of the `smith.compile` stack is the one causing the issue using the `backend=` option for `smith.compile`.
In particular, try:

- `smith.compile(fn, backend="eager")`, which only runs SmithDynamo, the graph capture component of `smith.compile`.
- `smith.compile(fn, backend="aot_eager")`, which runs SmithDynamo and AOTAutograd, which additionally generates the backward graph during compilation.
- `smith.compile(fn, backend="aot_eager_decomp_partition")`, which runs SmithDynamo and AOTAutograd with operator decompositions/partitions.
- `smith.compile(fn, backend="inductor")`, which runs SmithDynamo, AOTAutograd, and SmithInductor, the backend ML compiler that generates compiled kernels.

If you only fail with the Inductor backend, you can additionally test various Inductor modes:

- `smith.compile(fn, backend="inductor", mode="default")`
- `smith.compile(fn, backend="inductor", mode="reduce-overhead")`
- `smith.compile(fn, backend="inductor", mode="max-autotune")`

You can also check if dynamic shapes is causing issues with any backend:

- `smith.compile(fn, dynamic=True)` (always use dynamic shapes)
- `smith.compile(fn, dynamic=False)` (never use dynamic shapes)
- `smith.compile(fn, dynamic=None)` (automatic dynamic shapes)
...
However, if you are a motivated user unfamiliar with the internals of `smith.compile`,
...
- **Configuration**. Did you set all the same configuration?
  This includes `smith._dynamo.config` and `smith._inductor.config` settings,
  as well as arguments to `smith.compile` like `backend` / `mode`.
- **Context managers**. Did you replicate any active context managers?
  This could be `smith.no_grad`, automatic mixed precision, `SmithFunctionMode` / `SmithDispatchMode`,
  activation checkpointing, compiled autograd etc.
- **Tensor subclasses**. Is there a tensor subclass involved?

### Minifier

The minifier is an early `smith.compile` tool that, given an FX graph that crashes when we attempt to run or compile it,
finds a subgraph that also crashes and outputs the code that performs that subgraph's operations.
Essentially, the minifier finds a minimal repro for a certain class of `smith.compile`-related crashes.
...
Instructions for operating the minifier can be found [here](https://blacksmith.org/docs/stable/smith.compiler_troubleshooting_old.html).
If the compiler is crashing, you can set `SMITHDYNAMO_REPRO_AFTER="dynamo"` or `SMITHDYNAMO_REPRO_AFTER="aot"`
...
For accuracy-related issues, consider setting `SMITHDYNAMO_REPRO_LEVEL=4`. Please note that this may not always successfully identify the problematic subgraph.

## Debugging Deeper

This section provides tools and techniques for independently debugging `smith.compile` issues
or for gaining a deeper understanding of the `smith.compile` stack.
These methods are more involved than those presented above and are used by Blacksmith developers regularly
to debug real `smith.compile` issues.

Below is a high-level overview of the stack:

![Smith Dynamo Stack](../../_static/img/dynamo/td_stack.png)

The stack comprises three main components: SmithDynamo, AOTAutograd, and Inductor.
...
The `SMITH_LOGS=trace_bytecode` option enables you to view the precise bytecode instructions that Dynamo is tracing,
...
You can also use `SMITH_LOGS=trace_source` to see which lines of source code Dynamo is tracing through.
...
Finally, you can use `SMITH_LOGS=graph_code` to see the Python code representing the FX graph that Dynamo traced.
...
```
$ SMITH_LOGS="trace_bytecode,trace_source,graph_code" python playground.py
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:6 in f ()
    @smith.compile(backend="eager")
TRACE RESUME 0 []
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:8 in f (f)
        x = smith.sin(x)
TRACE LOAD_GLOBAL smith []
TRACE LOAD_ATTR sin [NullVariable(), PythonModuleVariable(<module 'smith' from '/data/users/williamwen/blacksmith/smith/__init__.py'>)]
TRACE LOAD_FAST x [NullVariable(), SmithInGraphFunctionVariable(<built-in method sin of type object at 0x7f00f6964600>)]
TRACE CALL 1 [NullVariable(), SmithInGraphFunctionVariable(<built-in method sin of type object at 0x7f00f6964600>), LazyVariableTracker()]
TRACE STORE_FAST x [TensorVariable()]
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:9 in f (f)
        x = g(x, x)
TRACE LOAD_GLOBAL g []
...
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:3 in g (g) (inline depth: 1)
    def g(x, y):
TRACE RESUME 0 []
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:4 in g (g) (inline depth: 1)
        return x + y
TRACE LOAD_FAST x []
...
TRACE STORE_FAST x [TensorVariable()]
TRACE starts_line /data/users/williamwen/blacksmith/playground.py:10 in f (f)
        return x
TRACE LOAD_FAST x []
TRACE RETURN_VALUE None [TensorVariable()]
TRACED GRAPH
===== __compiled_fn_1 =====
/data/users/williamwen/blacksmith/smith/fx/_lazy_graph_module.py class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3][3, 1]cpu"):
        l_x_ = L_x_

        # File: /data/users/williamwen/blacksmith/playground.py:8 in f, code: x = smith.sin(x)
        x: "f32[3, 3][3, 1]cpu" = smith.sin(l_x_);  l_x_ = None

        # File: /data/users/williamwen/blacksmith/playground.py:4 in g, code: return x + y
...
Unfortunately, inserting a breakpoint in the normal Python fashion will result in a graph break in SmithDynamo,
...
- In `smith/_dynamo/symbolic_convert.py`, breakpoint at functions that are named after the problematic bytecode instruction,
...
- Breakpoint in `smith/_dynamo/variables/builder.py, function:_wrap`. You will likely have to conditionally breakpoint on the input.
...
The second way to insert a breakpoint is to use `smith._dynamo.comptime.comptime.breakpoint`:
...
--Return--
> /data/users/williamwen/blacksmith/smith/_dynamo/comptime.py(392)inner()->None
-> builtins.breakpoint()
(Pdb) ctx.print_bt()
File "/data/users/williamwen/blacksmith/playground.py", line 7, in f
    comptime.breakpoint()
...
(Pdb) bt
...
/data/users/williamwen/blacksmith/smith/_dynamo/symbolic_convert.py(826)call_function()
-> self.push(fn.call_function(self, args, kwargs))  # type: ignore[arg-type]
/data/users/williamwen/blacksmith/smith/_dynamo/variables/misc.py(331)call_function()
...
def forward(self, L_x_: "f32[3, 3]"):
    ...
    # File: /data/users/williamwen/blacksmith/playground.py:6 in f, code: y = x + 1
...
% ^^^^^^^^^^^^^^^^^^^^^^
%
% Debugging SmithDynamo on large models can be tricky, mainly because Dynamo traces through large amounts of code.
...
% Fortunately, you can use ``SMITHDYNAMO_DEBUG_FUNCTION=<function name>``, which limits dynamo tracing to only functions with a specific name
...
- Ablation reveals the error is happening at the SmithDynamo level
- The error is not being emitted from SmithDynamo stack frames
- The error looks more like a user error rather than a Dynamo error, or is a segmentation fault
- The error does not occur without `smith.compile`
...
If you are interested in seeing the bytecode that Dynamo generates, you can use `SMITH_LOGS=bytecode`.
...
(troubleshooting-smith-logs-options)=

### Summary of SMITH_LOGS options

A summary of helpful `SMITH_LOGS` options is:
...
.. list-table::
    ...
    * - Option
      - Description
    * - +all
      - Output debug logs from all ``smith.compile`` components
    * - +dynamo
      - Output debug logs from SmithDynamo
    * - +aot
      - Output debug logs from AOTAutograd
    * - +inductor
      - Output debug logs from SmithInductor
    * - dynamic
...
- [smith.compile tutorial](https://blacksmith.org/tutorials/intermediate/smith_compile_tutorial.html)
- [smith.compile fine-grained APIs](https://blacksmith.org/docs/stable/smith.compiler_fine_grain_apis.html)
- [smith.compile FAQ](https://blacksmith.org/docs/stable/smith.compiler_faq.html)
- [smith.compiler namespace overview](https://blacksmith.org/docs/stable/smith.compiler.html#smith-compiler-overview)
- [smith.compiler API reference](https://blacksmith.org/docs/stable/smith.compiler_api.html)
- [Profiling smith.compile](https://blacksmith.org/docs/stable/smith.compiler_profiling_smith_compile.html)
- [smith.compile missing manual](https://docs.google.com/document/d/1y5CRfMLdwEoF1nTk9q8qEu1mgMUuUtvhklPKJ2emLU8/edit?usp=sharing)
...
- [SmithInductor caching tutorial](https://blacksmith.org/tutorials/recipes/smith_compile_caching_tutorial.html)
