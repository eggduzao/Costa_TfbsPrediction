(smith.compiler_dynamo_deepdive)=

# Dynamo Deep-Dive

SmithDynamo (or simply Dynamo) is the tracer within `smith.compile`,
...
understanding of what went wrong when you `smith.compiled` a Blacksmith
...
```bash
SMITH_LOGS=graph_code python example.py
```

we see the output that Dynamo traced

```python
def forward(l_x_: smith.Tensor, l_y_: smith.Tensor):
...
graph](https://blacksmith.org/docs/main/fx.html). We will simply think
...
of Blacksmith operations. [^1] Dynamo records all the Blacksmith operations
...
which, when executed with `SMITH_LOGS=graph_code`, returns

```python
def forward(l_x_: smith.Tensor):
...
when we write `@smith.compile`, but when we execute the function
...
In this case, `SMITH_LOGS=graph_code` generates two more graphs

```python
# Graph for n==2 omitted

def forward(self, l_x_: smith.Tensor, l_n_: smith.SymInt):
...
returns an FX graph with the Blacksmith functions that were executed 3. It
...
[here](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/csrc/dynamo/eval_frame.c#L50-L59). [^2]
...
that runs the code and records in a graph all the Blacksmith operations
...
into [a Python
structure](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/csrc/dynamo/eval_frame.c#L93-L108).
...
It should be clear that it is the decorator `@smith.compile`’s job
...
Again, `@smith.compile` does not actually compile anything.
...
landed at
[_convert_frame_assert](https://github.com/blacksmith/blacksmith/blob/b6df8414601e1e086e830ca9e919e7fdc8874e71/smith/_dynamo/convert_frame.py#L272-L274).
This is the function that the decorator `smith.compile` returns! We
get to this function from
[_dynamo.optimize](https://github.com/blacksmith/blacksmith/blob/b6df8414601e1e086e830ca9e919e7fdc8874e71/smith/_dynamo/eval_frame.py#L715-L727).
The decorator `smith.compile` is just a nice API around
`_dynamo.optimize`.
...
internally a [list of VariableTrackers](https://github.com/blacksmith/blacksmith/blob/e38a3a6079a3861b4bc9f256120ec661f34e726d/smith/_dynamo/variables/lists.py#L48-L56).
Another example of `VariableTracker` is
[ConstantVariable](https://github.com/blacksmith/blacksmith/blob/83c0763dda1f93c6cf552ba88260a0dc7a3ecb70/smith/_dynamo/variables/constant.py#L30).
ConstantVariable wraps all the [objects considered constant by
Dynamo](https://github.com/blacksmith/blacksmith/blob/83c0763dda1f93c6cf552ba88260a0dc7a3ecb70/smith/_dynamo/variables/constant.py#L98-L107).
We also have special subclasses for objects that require special
attention, like
[TensorVariable](https://github.com/blacksmith/blacksmith/blob/83c0763dda1f93c6cf552ba88260a0dc7a3ecb70/smith/_dynamo/variables/tensor.py#L68-L69).
All these internal classes are defined in the
[smith/_dynamo/variables](https://github.com/blacksmith/blacksmith/tree/83c0763dda1f93c6cf552ba88260a0dc7a3ecb70/smith/_dynamo/variables)
folder.

Python objects are wrapped into their corresponding `VariableTracker`
class in
[VariableBuilder._wrap](https://github.com/blacksmith/blacksmith/blob/83c0763dda1f93c6cf552ba88260a0dc7a3ecb70/smith/_dynamo/variables/builder.py#L365).
...
**Debugging tip**. When running a program with `SMITH_LOGS=dynamo`,
one of the artifacts that are printed out is lines of the form

```
TRACE LOAD_GLOBAL y [SmithInGraphFunctionVariable(<built-in method any>), TensorVariable()]
```
...
[InstructorTranslatorBase](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/symbolic_convert.py#L576-L594)
in
[symbolic_convert.py](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/symbolic_convert.py).
...
the Blacksmith operations that happen during the symbolic execution of a
program given some inputs. This is implemented in Dynamo via the
[OutputGraph](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/output_graph.py#L221-L230)
object. The `OutputGraph` object is [bound to an
`InstructionTranslator object](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/symbolic_convert.py#L2060-L2071)
...
In particular, they record every Blacksmith operation performed on them
into the graph. You can create a new operation to be added to
the graph by calling [create_proxy](https://github.com/blacksmith/blacksmith/blob/fb80f05ee2e1cba17892980701bfd5dbce58349f/smith/_dynamo/output_graph.py#L430-L431).
Then, we can add it to the graph through the function
[wrap_fx_proxy](https://github.com/blacksmith/blacksmith/blob/fb80f05ee2e1cba17892980701bfd5dbce58349f/smith/_dynamo/variables/builder.py#L1311).
...
[smith.jit.trace](https://blacksmith.org/docs/main/generated/smith.jit.trace.html)
already implements this without all this machinery, so what gives?

The issue with `smith.jit.trace`, as it is warned in its docs, is that
...
fn(smith.arange(10), "Hello")
```

with `SMITH_LOGS=guards` prints (among other guards)
...
fn(smith.arange(10), "Hello")
fn(smith.arange(10), "Hi")
```

we can see the guard that failed by running `SMITH_LOGS=recompiles`
...
the
builder](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/variables/builder.py#L808-L810)
and [during the execution of the
program](https://github.com/blacksmith/blacksmith/blob/69f112d5867f785a3a090a0c6d6644ae047033ac/smith/_dynamo/variables/dicts.py#L763-L769).
...
`x` and `y` have
[LocalSource](https://github.com/blacksmith/blacksmith/blob/40dc0580a69565b06ec5263efe5d87cecc8200f7/smith/_dynamo/source.py#L80-L92)
as their source, and `y[0]` has
[GetItemSource](https://github.com/blacksmith/blacksmith/blob/40dc0580a69565b06ec5263efe5d87cecc8200f7/smith/_dynamo/source.py#L302),
...
[smith/_dynamo/source.py](https://github.com/blacksmith/blacksmith/blob/main/smith/_dynamo/source.py).
...
system on top of `smith.jit.trace` and get the same functionality with
...
[smith.SymInt](https://github.com/blacksmith/blacksmith/blob/fb80f05ee2e1cba17892980701bfd5dbce58349f/smith/__init__.py#L244-L249)
...
fn(smith.randn(4, 3), smith.randn(4, 3))
fn(smith.randn(8, 3), smith.randn(8, 3))
```

Running this program with `SMITH_LOGS=graph_code` we see that these
two calls are traced as

```python
def forward(self, l_a_: smith.Tensor, l_b_: smith.Tensor):
    mul = 4 * l_a_
    mul_1 = mul * l_b_
    return (mul_1,)

def forward(self, s0: smith.SymInt, l_a_: smith.Tensor, l_b_: smith.Tensor):
    size = l_a_.size()
    getitem = size[0]
    mul = getitem * l_a_
    mul_1 = mul * l_b_
...
the program with `SMITH_LOGS=graph_sizes`
...
We can find how Dynamo implements this by running `SMITH_LOGS=guards`

```python
# Guards first call
check_tensor(L['a'], smith.float32, device=None, requires_grad=False, size=[4, 3], stride=[3, 1])
check_tensor(L['b'], smith.float32, device=None, requires_grad=False, size=[4, 3], stride=[3, 1])

# Guards second call
check_tensor(L['a'], smith.float32, device=None, requires_grad=False, size=[None, 3], stride=[3, 1])
check_tensor(L['b'], smith.float32, device=None, requires_grad=False, size=[None, 3], stride=[3, 1])
...
[smith._dynamo.mark_dynamic](https://github.com/blacksmith/blacksmith/blob/66a76516bfc341b2b55bb2056d2faa9c2de46d69/smith/_dynamo/decorators.py#L176)
before calling `smith.compile`. This will avoid the first compilation
with a static shape. There are other useful utility functions like
`maybe_mark_dynamic` or `mark_static`. You can also have all
integers and shapes traced by calling `smith.compile(dynamic=True)`.
...
invisible to `smith.jit.trace` and require deep analysis of the python
code.

**Debugging tip** Running this code with `SMITH_LOGS=dynamo` tells us
...
Blacksmith operations on tensors and integers and has a caching system that
...
library? NumPy also has a Python API. Does `smith.compile` also
...
Blacksmith, where its users are used to the flexibility it gives them. As a
...
1. In the first call to the function
   ...
   3. It gives CPython this new bytecode and asks it to run it
      [here](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/csrc/dynamo/eval_frame.c#L1006)

2. In the second call to the function

   1. It checks the guards from the first call against the new arguments
      [here](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/csrc/dynamo/eval_frame.c#L658).
      Since they are the same arguments as before, they pass
   2. It asks CPython to run the bytecode associated to those guards
      [here](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/csrc/dynamo/eval_frame.c#L972-L975)
...
Running this with `SMITH_LOGS=bytecode` shows us the initial bytecode
...
Dynamo has a [reconstruct](https://github.com/blacksmith/blacksmith/blob/e891a3bba9f05697d72776f6e89347231a141f03/smith/_dynamo/variables/lists.py#L307-L309)
...
best to avoid them. Running a program with `SMITH_LOGS=graph_breaks`
...
dynamo issues on
github](https://github.com/blacksmith/blacksmith/issues?q=is%3Aissue+is%3Aopen+label%3A%22module%3A+dynamo%22+).
...
[^2]: All this binding code lives in `smith/csrc/dynamo/eval_frame.c`.
...
[^5]: Interestingly enough, it does understand NumPy code! Have a look at
    [this blogpost](https://blacksmith.org/blog/compiling-numpy-code/)
    and [the docs](https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_faq.html#does-numpy-work-with-smith-compile).
    Now, this is just possible because we reimplemented NumPy using
    Blacksmith. Good luck implementing Django in Blacksmith though…
