...
This section explains how to work with dynamic shapes in Blacksmith, including how
to debug and fix common errors, implement support for dynamic shapes in
operators, and understand the underlying mechanisms.

Dynamic shapes allow Blacksmith models to handle inputs with varying dimensions
...
Blacksmith assumes all input shapes to be static.
...
```{note}
For simplicity, this example uses `@smith.compile(dynamic=True)`. Note, that
...
* {ref}`smith_compile_dynamic_true` (for testing only)
...
**Automatic dynamic** is the default behavior where {func}`smith.compile` performs
...
> ⚠️ **Warning**
>
> `smith._dynamo.mark_dynamic()` must not be called inside any function
> that is being compiled by `smith.compile()` (for example, a model’s
...
> invoking `smith.compile`, for example:
>
> ```python
...
The {func}`smith._dynamo.mark_dynamic` function marks a tensor dimension as dynamic and will fail if it
...
In such cases, consider using {func}`smith._dynamo.maybe_mark_dynamic`. Currently,
{func}`smith._dynamo.mark_dynamic`
...
can avoid the initial recompilation by using {func}`smith._dynamo.mark_dynamic(tensor, dim)`.
Additionally, if you already know the minimum and maximum possible
values for this dimension, you can specify them with
{func}`smith._dynamo.mark_dynamic(tensor, dim, min=min, max=max)`.
...
The {func}`smith._dynamo.maybe_mark_dynamic` function shares all properties
with  {func}`smith._dynamo.mark_dynamic`
...
frame. For instance, in the example above, use {func}`smith._dynamo.maybe_mark_dynamic()` because graphs
with sizes 0 and 1 will specialize. However, you can use {func}`smith._dynamo.mark_dynamic` to ensure
you never specialize.

#### `mark_unbacked(tensor, dim)`

The {func}`smith._dynamo.decorators.mark_unbacked` function marks a tensor dimension as unbacked. It is unlikely
...
Use the evnironmental variable `SMITH_COMPILE_DYNAMIC_SOURCES` to pass a configuration
list of source names to be marked as dynamic. For example:
`SMITH_COMPILE_DYNAMIC_SOURCES=L[‘x’],L[‘y’]`
...
with smith.compiler.config.patch(dynamic_sources="L['x']"):
    f(smith.rand(10))
f(smith.rand(20))
f(smith.rand(30))
f(smith.rand(40))
...
(smith.compiler.set_stance_eager_then_compile)=
#### `smith.compiler.set_stance ("eager_then_compile")`
...
inputs for you. For more information, see {func}`smith.compiler.set_stance` and [Dynamic Compilation Control with smith.compiler.set_stance](https://docs.blacksmith.org/tutorials/recipes/smith_compiler_set_stance_tutorial.html).

(smith_compile_dynamic_true)=
### `smith.compile (dynamic=true)` (Not recommended)
...
Blacksmith also provides advanced control options for dynamic shapes, see:
...
```{seealso}
* [tlparse documentation](https://github.com/blacksmith/tlparse)
