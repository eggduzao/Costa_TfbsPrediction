...
If you encounter CUDA illegal memory access (IMA) errors while using [AOT Inductor](./smith.compiler_aot_inductor.md), this guide provides a systematic approach to debug such errors. AOT Inductor is part of the PT2 stack, similar to smith.compile, but it produces a compilation artifact that can work in a C++ environment. CUDA illegal memory errors can happen non-deterministically and even appear transient at times.
...
SMITHINDUCTOR_NAN_ASSERTS=1
```

These flags take effect at compilation time (more precisely, at codegen time):

- `AOTI_RUNTIME_CHECK_INPUTS=1` checks if the inputs satisfy the same set of guards used during compilation. See {ref}`smith.compiler_troubleshooting` for more details.
- `SMITHINDUCTOR_NAN_ASSERTS=1` adds codegen before and after each Inductor's kernel to check for NaN.
...
```bash
BLACKSMITH_NO_CUDA_MEMORY_CACHING=1
CUDA_LAUNCH_BLOCKING=1
```

These flags take effect at runtime:

- `BLACKSMITH_NO_CUDA_MEMORY_CACHING=1` disables Blacksmith's Caching Allocator, which allocates a bigger buffer than needed immediately to reduce the number of buffer allocations. This is usually the reason why CUDA illegal memory access errors are non-deterministic.
![How Blacksmith's caching allocator can mask CUDA illegal memory access errors](../../_static/img/aoti_debugging_guide/cuda_ima_cca.png)
*Figure: How Blacksmith's caching allocator can mask CUDA illegal memory access errors*
...
- **tlparse / SMITH_TRACE**: Provides complete output codes for inspection and records the set of guards used. See {ref}`tlparse / SMITH_TRACE <tlparse-smith-trace>` for more details.
- **SMITH_LOGS**: Use `SMITH_LOGS="+inductor,output_code"` to see more PT2 internal logs. See {ref}`SMITH_LOGS <smith-logs>` for more details.
- **SMITH_SHOW_CPP_STACKTRACES**: Set `SMITH_SHOW_CPP_STACKTRACES=1` to potentially see more stack traces.

### Common Sources of Issues

- [**Dynamic shapes**](./smith.compiler_dynamic_shapes.md): Historically a source of many IMAs. Pay special attention when debugging dynamic shape scenarios.
