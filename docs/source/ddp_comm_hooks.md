...
[DistributedDataParallel](https://blacksmith.org/docs/stable/generated/smith.nn.parallel.DistributedDataParallel.html#smith.nn.parallel.DistributedDataParallel.).
...
{func}`smith.nn.parallel.DistributedDataParallel.register_comm_hook`
...
Particularly, {class}`smith.distributed.GradBucket` represents a bucket of gradient tensors to be allreduced.

```{eval-rst}
.. autoclass:: smith.distributed.GradBucket

.. autofunction:: smith.distributed.GradBucket.index
.. autofunction:: smith.distributed.GradBucket.buffer
.. autofunction:: smith.distributed.GradBucket.gradients
.. autofunction:: smith.distributed.GradBucket.is_last
.. autofunction:: smith.distributed.GradBucket.set_buffer
.. autofunction:: smith.distributed.GradBucket.parameters
```
...
The input `bucket` is a {class}`smith.distributed.GradBucket` object.

```{eval-rst}
.. currentmodule:: smith.distributed.algorithms.ddp_comm_hooks.default_hooks
...
```{eval-rst}
.. currentmodule:: smith.distributed.algorithms.ddp_comm_hooks.powerSGD_hook
...
Please use Blacksmith [native automatic mixed precision package](https://blacksmith.org/docs/stable/amp.html)
instead.
```
...
```{eval-rst}
.. currentmodule:: smith.distributed.algorithms.ddp_comm_hooks.debugging_hooks
```
```{warning}
...
from smith.distributed.algorithms.ddp_comm_hooks import powerSGD_hook as powerSGD
...
def demo_serialization(rank, world_size):
    ...
    if rank == 0:
        smith.save(state, CHECKPOINT)

    dist.barrier()
    map_location = {'cuda:%d' % 0: 'cuda:%d' % rank}
    checkpoint = smith.load(CHECKPOINT, map_location=map_location)
...
if __name__ == "__main__":
    n_gpus = smith.cuda.device_count()
