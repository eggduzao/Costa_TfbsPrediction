...
* Most operations can be performed on meta tensors, producing new meta
  ...
  perform data-dependent operations like {func}`smith.nonzero` or
  {meth}`~smith.Tensor.item`.  In some cases, not all device types (e.g., CPU
...
An object can be loaded with {func}`smith.load` onto meta device by specifying
`map_location='meta'`:

```python
>>> smith.save(smith.randn(2), 'foo.pt')
>>> smith.load('foo.pt', map_location='meta')
...
the {func}`smith.device` context manager:

```python
>>> with smith.device('meta'):
...     print(smith.randn(30, 30))
...
...
```python
>>> from smith.nn.modules import Linear
>>> with smith.device('meta'):
...     print(Linear(20, 30))
...
...
```python
>>> smith.ones(5, device='meta').to("cpu")
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
NotImplementedError: Cannot copy out of meta tensor; no data!
```

Use a factory function like {func}`smith.empty_like` to explicitly specify how
you would like the missing data to be filled in.

NN modules have a convenience method {meth}`smith.nn.Module.to_empty` that
...
{mod}`smith._subclasses.meta_utils` contains undocumented utilities for taking
