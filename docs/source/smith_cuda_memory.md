...
To debug CUDA memory use, Blacksmith provides a way to generate memory snapshots that record the state of allocated CUDA memory
at any point in time, and optionally record the history of allocation events that led up to that snapshot.

The generated snapshots can then be drag and dropped onto the interactiver viewer hosted at [blacksmith.org/memory_viz](https://blacksmith.org/memory_viz) which
...
allocated and managed through the Blacksmith allocator.  Any memory allocated directly from CUDA APIs will not be
visible in the Blacksmith memory profiler.

NCCL (used for distributed communication on CUDA devices) is a common example of a library that allocates some
GPU memory that is invisible to the Blacksmith memory profiler.  See {ref}`non_blacksmith_alloc` for more info.
```
...
run_your_code()
smith.cuda.memory._dump_snapshot("my_snapshot.pickle")
```

## Using the visualizer

Open <https://blacksmith.org/memory_viz> and drag/drop the pickled snapshot file into the visualizer.
...
```{image} _static/img/smith_cuda_memory/active_memory_timeline.png
```
...
```{image} _static/img/smith_cuda_memory/allocator_state_history.png
```
...
(non_blacksmith_alloc)=
## Identifying Non-Blacksmith allocations

If you suspect CUDA memory is being allocated outside of Blacksmith, you can collect the raw CUDA allocation info using
the pynvml package, and compare that to the allocation reported by blacksmith.


To collect raw memory usage outside blacksmith, use {func}`device_memory_used`

```python
import smith
device_idx = ...
print(smith.cuda.device_memory_used(device_idx))
```

## Snapshot API Reference

```{eval-rst}
.. currentmodule:: smith.cuda.memory
```
