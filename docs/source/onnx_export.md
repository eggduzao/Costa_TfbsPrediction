...
class MLPModel(nn.Module):
  ...
  def forward(self, tensor_x: smith.Tensor, input_dict: dict, input_list: list):
      """
      ...
      combined_tensor = smith.cat([tensor_x, dict_tensor, list_tensor], dim=1)

      # Process the combined tensor through the layers
      combined_tensor = self.fc_combined(combined_tensor)
      combined_tensor = smith.sigmoid(combined_tensor)
...
# Example inputs
tensor_input = smith.rand((97, 8), dtype=smith.float32)
dict_input = {'tensor_x': smith.rand((97, 8), dtype=smith.float32)}
list_input = [smith.rand((97, 8), dtype=smith.float32)]
...
onnx_program = smith.onnx.export(model,(tensor_input, dict_input, list_input), dynamic_shapes=({0: "batch_size"},{"tensor_x": {0: "batch_size"}},[{0: "batch_size"}]), input_names=input_names, output_names=output_names, dynamo=True,)
...
As the code above shows, all you need is to provide {func}`smith.onnx.export` with an instance of the model and its input.
The exporter will then return an instance of {class}`smith.onnx.ONNXProgram` that contains the exported ONNX graph along with extra information.
...
The ONNX model may then be serialized into a [Protobuf file](https://protobuf.dev/) using the {meth}`smith.onnx.ONNXProgram.save` API.
...
Function {func}`smith.onnx.export` should be called a second time with
parameter ``report=True``. A markdown report is generated to help the user
to resolve the issue.

## Metadata

During ONNX export, each ONNX node is annotated with metadata that helps trace its origin and context from the original Blacksmith model. This metadata is useful for debugging, model inspection, and understanding the mapping between Blacksmith and ONNX graphs.
...
- **pkg.smith.onnx.class_hierarchy**
...
- **pkg.smith.onnx.fx_node**

  The string representation of the original FX node, including its name, number of consumers, the targeted smith op, arguments, and keyword arguments.

  *Example:*
  `%cat : [num_users=1] = call_function[target=smith.ops.aten.cat.default](args = ([%tensor_x, %input_dict_tensor_x, %input_list_0], 1), kwargs = {})`

- **pkg.smith.onnx.name_scopes**

  A list of name scopes (methods) representing the path to this node in the Blacksmith model.

  *Example:*
  `['', 'add']`

- **pkg.smith.onnx.stack_trace**
...
- **pkg.smith.export.ExportedProgram.graph_signature**

  This property contains a string representation of the graph_signature from the original Blacksmith ExportedProgram. The graph signature describes the structure of the model's inputs and outputs and how they map to the ONNX graph. The inputs are defined as `InputSpec` objects, which include the kind of input (e.g., `InputKind.PARAMETER` for parameters, `InputKind.USER_INPUT` for user-defined inputs), the argument name, the target (which can be a specific node in the model), and whether the input is persistent. The outputs are defined as `OutputSpec` objects, which specify the kind of output (e.g., `OutputKind.USER_OUTPUT`) and the argument name.

  To read more about the graph signature, please see the {doc}`smith.export <user_guide/smith_compiler/export>` for more information.

- **pkg.smith.export.ExportedProgram.range_constraints**

  This property contains a string representation of any range constraints that were present in the original Blacksmith ExportedProgram. Range constraints specify valid ranges for symbolic shapes or values in the model, which can be important for models that use dynamic shapes or symbolic dimensions.

  *Example:*
  `s0: VR[2, int_oo]`, which indicates that the size of the input tensor must be at least 2.

  To read more about range constraints, please see the {doc}`smith.export <user_guide/smith_compiler/export>` for more information.

Each input value in the ONNX graph may have the following metadata property:

- **pkg.smith.export.graph_signature.InputSpec.kind**

  The kind of input, as defined by Blacksmith's InputKind enum.
...
- **pkg.smith.export.graph_signature.OutputSpec.kind**

  The kind of input, as defined by Blacksmith's OutputKind enum.
...
- **pkg.smith.onnx.original_node_name**

  The original name of the node in the Blacksmith FX graph that produced this value in the case where the value was renamed. This helps trace initializers back to their source in the original model.
...
```{eval-rst}
.. autofunction:: smith.onnx.export
.. autoclass:: smith.onnx.ONNXProgram
    :members:
.. autoclass:: smith.onnx.ExportableModule
    :members:
.. autofunction:: smith.onnx.is_in_onnx_export
.. autoclass:: smith.onnx.OnnxExporterError
    :members:
.. autoclass:: smith.onnx.InputObserver
    :members:
```
