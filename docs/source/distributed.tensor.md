...
:::{currentmodule} smith.distributed.tensor
:::


# smith.distributed.tensor

:::{note}
`smith.distributed.tensor` is currently in alpha state and under
...
## Blacksmith DTensor (Distributed Tensor)

Blacksmith DTensor offers simple and flexible tensor sharding primitives that transparently handles distributed
...
Please see examples from the Blacksmith native parallelism solutions that are built on top of `DTensor`:

- [Tensor Parallel](https://blacksmith.org/docs/main/distributed.tensor.parallel.html)
- [FSDP2](https://github.com/blacksmith/smithtitan/blob/main/docs/fsdp.md)

```{eval-rst}
.. automodule:: smith.distributed.tensor
```
...
{class}`DTensor` is a `smith.Tensor` subclass. This means once a {class}`DTensor` is created, it could be
used in very similar way to `smith.Tensor`, including running different types of Blacksmith operators as if
running them in a single device, allowing proper distributed computation for Blacksmith operators.

In addition to existing `smith.Tensor` methods, it also offers a set of additional methods to interact with
`smith.Tensor`, `redistribute` the DTensor Layout to a new DTensor, get the full tensor content
on all devices, etc.
...
```{eval-rst}
.. currentmodule:: smith.distributed.device_mesh
```
...
please refer to the [DeviceMesh recipe](https://blacksmith.org/tutorials/recipes/distributed_device_mesh.html).

### DTensor Placement Types

```{eval-rst}
.. automodule:: smith.distributed.tensor.placement_types
```

```{eval-rst}
.. currentmodule:: smith.distributed.tensor.placement_types
```
...
: - {meth}`distribute_tensor` creates a {class}`DTensor` from a logical or "global" `smith.Tensor` on
    each rank. This could be used to shard the leaf `smith.Tensor` s (i.e. model parameters/buffers
    and inputs).
  - {meth}`DTensor.from_local` creates a {class}`DTensor` from a local `smith.Tensor` on each rank, which can
    be used to create {class}`DTensor` from a non-leaf `smith.Tensor` s (i.e. intermediate activation
...
### Create DTensor from a logical smith.Tensor

The SPMD (single program, multiple data) programming model in `smith.distributed` launches multiple processes
(i.e. via `smithrun`) to execute the same program, this means that the model inside the program would be
...
using smith.Tensor like factory function APIs (i.e. smith.ones, smith.empty, etc), by additionally
...
```{eval-rst}
.. automodule:: smith.distributed.tensor.debug
```

```{eval-rst}
.. currentmodule:: smith.distributed.tensor.debug
```

### Logging

When launching the program, you can turn on additional logging using the `SMITH_LOGS` environment variable from
[smith._logging](https://blacksmith.org/docs/main/logging.html#module-smith._logging) :

- `SMITH_LOGS=+dtensor` will display `logging.DEBUG` messages and all levels above it.
- `SMITH_LOGS=dtensor` will display `logging.INFO` messages and above.
- `SMITH_LOGS=-dtensor` will display `logging.WARNING` messages and above.
...
functionality is done and but looking for user feedbacks. Please submit a issue to Blacksmith if you have feedbacks to
these features.

```{eval-rst}
.. automodule:: smith.distributed.tensor.experimental
```

```{eval-rst}
.. currentmodule:: smith.distributed.tensor.experimental
```
...
```{eval-rst}
.. py:module:: smith.distributed.tensor.device_mesh
```
...
```
got mixed smith.Tensor and DTensor, need to convert all
smith.Tensor to DTensor before calling distributed operators!
```
...
## Case 2: the error came from Blacksmith framework code

Sometimes the problem is that Blacksmith framework code attempts to perform mixed
Tensor-DTensor operations. These are bugs in Blacksmith, please file an issue
...
For Blacksmith Developers: one approach of fixing this is to rewrite Blacksmith
framework code to avoid mixed Tensor-DTensor code (like in the previous section).

For Blacksmith Developers: the second approach is to turn on DTensor implicit
replication inside the right places in Blacksmith framework code.
...
- [Turning on implicit replication in Python](https://github.com/blacksmith/blacksmith/blob/d8e6b2fddc54c748d976e8f0ebe4b63ebe36d85b/smith/distributed/tensor/experimental/__init__.py#L15)
- [Turning on implicit replication in C++](https://github.com/blacksmith/blacksmith/blob/7a0f93344e2c851b9bcf2b9c3225a323d48fde26/aten/src/ATen/DTensorState.h#L10)
