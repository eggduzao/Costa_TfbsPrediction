...
:meth:`~smith.distributed.rpc.remote`. Each RRef is owned by the callee worker
of the :meth:`~smith.distributed.rpc.remote` call (i.e., owner) and can be used
...
:meth:`~smith.distributed.rpc.remote` call.
...
:meth:`~smith.distributed.rpc.rpc_async` or
:meth:`~smith.distributed.rpc.remote` invocation, and the owner will be notified
...
- **Non-idempotent UDFs**: We assume the user functions (UDF) provided to
  :meth:`~smith.distributed.rpc.rpc_sync`,
  :meth:`~smith.distributed.rpc.rpc_async` or
  :meth:`~smith.distributed.rpc.remote` are not idempotent and therefore
...
owner calls :meth:`~smith.distributed.rpc.rpc_sync`,
:meth:`~smith.distributed.rpc.rpc_async`, or
:meth:`~smith.distributed.rpc.remote` and uses its RRef as an argument. In this
...
If Z calls :meth:`~smith.distributed.rpc.RRef.to_here` on the ``UserRRef``, the
owner at least knows A when Z is deleted, because otherwise,
:meth:`~smith.distributed.rpc.RRef.to_here` wouldn't finish. If Z does not call
:meth:`~smith.distributed.rpc.RRef.to_here`, it is possible that the owner
...
.. code::
  ...
  # on worker A
  rref = rpc.remote('B', smith.add, args=(smith.ones(2), 1))
...
creates the ``OwnerRRef``. The method :meth:`~smith.distributed.rpc.remote`
returns immediately, meaning that the ``UserRRef`` can be forked/used before
the owner knows about it.

On the owner, when receiving the :meth:`~smith.distributed.rpc.remote` call, it
...
from A to B (:meth:`~smith.distributed.rpc.remote` and
:meth:`~smith.distributed.rpc.RRef.to_here`) may
...
.. code::
  ...
  import smith
  import smith.distributed.rpc as RRef, rpc
