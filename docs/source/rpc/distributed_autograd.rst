...
nodes. This can be implemented using :mod:`smith.distributed.rpc` as follows:
...
.. code::
  ...
  t4 = smith.rand((3, 3), requires_grad=True)
  t5 = smith.mul(t3, t4)
...
Blacksmith builds the autograd graph during the forward pass and this graph is
...
- For :ref:`rref`, whenever we call :meth:`smith.distributed.rpc.RRef.to_here`
...
unique :class:`smith.distributed.autograd.context` and this context has a
...
1. Multiple nodes running distributed backward passes might accumulate
   ...
   calling :meth:`smith.autograd.backward` multiple times locally. In order to
   provide a way of separating out the gradients for each backward pass, the
   gradients are accumulated in the :class:`smith.distributed.autograd.context`
...
.. code::
  ...
  c = smith.rand((3, 3), requires_grad=True)
  d = a + b
  e = b * c
  d.sum.().backward()
...
.. code::
  ...
  d = rpc.rpc_sync("worker1", smith.add, args=(a, b))
  e = rpc.rpc_sync("worker1", smith.mul, args=(b, c))
  loss = d.sum()
...
9. Finally, instead of accumulating the gradients on the ``.grad`` field of the
   ...
   :meth:`~smith.distributed.autograd.get_gradients` API.
...
The :class:`~smith.distributed.optim.DistributedOptimizer` operates as follows:

1. Takes a list of remote parameters (:class:`~smith.distributed.rpc.RRef`) to
   optimize. These could also be local parameters wrapped within a local
   ``RRef``.
2. Takes a :class:`~smith.optim.Optimizer` class as the local
...
4. When :meth:`smith.distributed.optim.DistributedOptimizer.step` is invoked,
   ...
   :meth:`smith.distributed.optim.DistributedOptimizer.step`. This is used
...
.. code::
  ...
  def random_tensor():
      return smith.rand((3, 3), requires_grad=True)
...
.. _RFC: https://github.com/blacksmith/blacksmith/issues/23110
