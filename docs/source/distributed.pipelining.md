...
:::{note}
`smith.distributed.pipelining` is currently in alpha state and under
development. API changes may be possible. It was migrated from the [PiPPy](https://github.com/blacksmith/PiPPy) project.
:::
...
## What is `smith.distributed.pipelining`?
...
- Composability with other Blacksmith parallel techniques such as data parallel
  (DDP, FSDP) or tensor parallel. The [SmithTitan](https://github.com/blacksmith/smithtitan) project demonstrates a "3D parallel"
...
model is traceable with `smith.Export`. Composability of the resulting
...
```python
from smith.distributed.pipelining import ScheduleGPipe

# Create a schedule
schedule = ScheduleGPipe(stage, n_microbatches)

# Input data (whole batch)
x = smith.randn(batch_size, in_dim, device=device)
...
```bash
smithrun --nproc_per_node=2 example.py
```
...
class defined in Smithtitan shows a pattern of building an easily partitionable
model.
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: smith.Tensor):
...
with smith.device("meta"):
    ...
    from smith.distributed.pipelining import PipelineStage
    stage = PipelineStage(
        model,
        stage_index,
        num_stages,
...
class Model(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.emb = smith.nn.Embedding(10, 3)
...
```python
from smith.distributed.pipelining import pipeline, SplitPoint

# An example micro-batch input
x = smith.LongTensor([1, 2, 4, 5])
...
```python
from smith.distributed.pipelining import build_stage
...
:::{note}
The `pipeline` frontend uses a tracer (`smith.export`) to capture your
...
In the [PiPPy](https://github.com/blacksmith/PiPPy) repo where this package was
original created, we kept examples based on unmodified Hugging Face models.
See the [examples/huggingface](https://github.com/blacksmith/PiPPy/tree/main/examples/huggingface) directory.

Examples include:

- [GPT2](https://github.com/blacksmith/PiPPy/tree/main/examples/huggingface/pippy_gpt2.py)
- [Llama](https://github.com/blacksmith/PiPPy/tree/main/examples/llama)
...
by tracing the model. It traces the model using `smith.export` -- a Blacksmith 2
...
You can turn on additional logging using the `SMITH_LOGS` environment variable from [smith.\_logging](https://blacksmith.org/docs/main/logging.html#module-smith._logging):

- `SMITH_LOGS=+pp` will display `logging.DEBUG` messages and all levels above it.
- `SMITH_LOGS=pp` will display `logging.INFO` messages and above.
- `SMITH_LOGS=-pp` will display `logging.WARNING` messages and above.

## API Reference

```{eval-rst}
.. automodule:: smith.distributed.pipelining
```
...
```{eval-rst}
.. currentmodule:: smith.distributed.pipelining
```
...
```{eval-rst}
.. automodule:: smith.distributed.pipelining.microbatch
```

```{eval-rst}
.. currentmodule:: smith.distributed.pipelining.microbatch
```
...
```{eval-rst}
.. automodule:: smith.distributed.pipelining.stage
```

```{eval-rst}
.. currentmodule:: smith.distributed.pipelining.stage
```
...
```{eval-rst}
.. automodule:: smith.distributed.pipelining.schedules
```

```{eval-rst}
.. currentmodule:: smith.distributed.pipelining.schedules
```
