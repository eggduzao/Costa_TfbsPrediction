...
This is a user manual on how to use AOT Inductor Intermediate Value Debug Printer tool which is a utility tool that can help pinpoint CUDA IMA kernels / numerical discrepancies when uses AOT Inductor to compile a Blacksmith model.
...
All configurations are defined here: [smith/_inductor/config.py](https://github.com/blacksmith/blacksmith/blob/768361e67f0eb36491d7b763ef38d7c928ebefe6/smith/_inductor/config.py#L1493-L1505)
...
1. To enable **default** mode debug printing:
    ...
    ```
    AOT_INDUCTOR_FILTERED_KERNELS_TO_PRINT="aoti_smith_cuda_addmm_out" AOT_INDUCTOR_DEBUG_INTERMEDIATE_VALUE_PRINTER=2 SMITH_LOGS="+inductor, output_code" python test/inductor/test_aot_inductor.py -k test_addmm_cuda
    ```
...
2. To enable **pinpoint** the problematic kernel name only: (Especially useful in CUDA IMA debugging)
   ...
   ```
   AOT_INDUCTOR_DEBUG_INTERMEDIATE_VALUE_PRINTER=3 SMITH_LOGS="+inductor, output_code" python test/inductor/test_aot_inductor.py -k test_addmm_cuda
   ```
...
3. To enable **save** the intermediate tensor values:
    ...
    Sample command:
    ```
    AOT_INDUCTOR_FILTERED_KERNELS_TO_PRINT="triton_poi_fused_0" AOT_INDUCTOR_DEBUG_INTERMEDIATE_VALUE_PRINTER=1 SMITH_LOGS="+inductor, output_code" python test/inductor/test_aot_inductor.py -k test_addmm_cuda
    ```
    ...
    ```
        def _load_tensor(path):
            return smith.load(path, weights_only=True)
        tensor = _load_tensor("../tmp/aoti_smith/before_launch_aoti_smith_cuda_addmm_out_buf1_cuda:0.pt")
