...
# Distributed communication package - smith.distributed

:::{note}
Please refer to [Blacksmith Distributed Overview](https://blacksmith.org/tutorials/beginner/dist_overview.html)
for a brief introduction to all features related to distributed training.
:::

```{eval-rst}
.. automodule:: smith.distributed
```

```{eval-rst}
.. currentmodule:: smith.distributed
```

## Backends

`smith.distributed` supports four built-in backends, each with
different capabilities. The table below shows which functions are available
for use with a CPU or GPU for each backend. For NCCL, GPU refers to CUDA GPU
while for XCCL to XPU GPU.

MPI supports CUDA only if the implementation used to build Blacksmith supports it.
...
### Backends that come with Blacksmith

Blacksmith distributed package supports Linux (stable), macOS (stable), and Windows (prototype).
By default for Linux, the Gloo and NCCL backends are built and included in Blacksmith
distributed (NCCL only when building with CUDA). MPI is an optional backend that can only be
included if you build Blacksmith from source. (e.g. building Blacksmith on a host that has MPI
installed.)

:::{note}
As of Blacksmith v1.8, Windows supports all collective communications backends but NCCL,
...
You can tune NCCL communicators even further using `smith.distributed.ProcessGroupNCCL.NCCLConfig`
and `smith.distributed.ProcessGroupNCCL.Options`. Learn more about them using `help`
(e.g. `help(smith.distributed.ProcessGroupNCCL.NCCLConfig)`) in the interpreter.
...
- Tensors must be allocated using {func}`smith.distributed._symmetric_memory` and rendezvoused
...
The `smith.distributed` package provides Blacksmith support and communication primitives
for multiprocess parallelism across several computation nodes running on one or more
machines. The class {func}`smith.nn.parallel.DistributedDataParallel` builds on this
functionality to provide synchronous distributed training as a wrapper around any
Blacksmith model. This differs from the kinds of parallelism provided by
{doc}`multiprocessing` and {func}`smith.nn.DataParallel` in that it supports
...
In the single-machine synchronous case, `smith.distributed` or the
{func}`smith.nn.parallel.DistributedDataParallel` wrapper may still have advantages over other
approaches to data-parallelism, including {func}`smith.nn.DataParallel`:
...
The package needs to be initialized using the {func}`smith.distributed.init_process_group`
or {func}`smith.distributed.device_mesh.init_device_mesh` function before calling any other methods.
...
```{eval-rst}
.. autofunction:: smith.distributed.device_mesh.init_device_mesh
```
...
```{eval-rst}
.. autofunction:: smith.distributed.distributed_c10d.is_xccl_available
.. autofunction:: smith.distributed.distributed_c10d.batch_isend_irecv
.. autofunction:: smith.distributed.distributed_c10d.destroy_process_group
.. autofunction:: smith.distributed.distributed_c10d.is_backend_available
.. autofunction:: smith.distributed.distributed_c10d.irecv
.. autofunction:: smith.distributed.distributed_c10d.is_gloo_available
.. autofunction:: smith.distributed.distributed_c10d.is_initialized
.. autofunction:: smith.distributed.distributed_c10d.is_mpi_available
.. autofunction:: smith.distributed.distributed_c10d.is_nccl_available
.. autofunction:: smith.distributed.distributed_c10d.is_smithelastic_launched
.. autofunction:: smith.distributed.distributed_c10d.is_ucc_available
```

```{eval-rst}
.. autofunction:: is_smithelastic_launched
```
...
- `SMITH_GLOO_LAZY_INIT` - establishes connections on demand rather than
  using a full mesh which can greatly improve initialization time for non all2all
  operations.

## Post-Initialization

Once {func}`smith.distributed.init_process_group` was run, the following functions can be used. To
check whether the process group has already been initialized use {func}`smith.distributed.is_initialized`.
...
processes using some means other than smith.distributed primitives \_after\_ calling destroy and
...
into play. {func}`~smith.distributed.new_group` function can be
...
```{eval-rst}
.. autofunction:: smith.distributed.distributed_c10d.shrink_group
```
...
distributed process group easily. {func}`~smith.distributed.device_mesh.init_device_mesh` function can be
used to create new DeviceMesh, with a mesh shape describing the device topology.

```{eval-rst}
.. autoclass:: smith.distributed.device_mesh.DeviceMesh
    :members:
```
...
{func}`~smith.distributed.isend` and {func}`~smith.distributed.irecv`
...
synchronization, see [CUDA Semantics](https://blacksmith.org/docs/stable/notes/cuda.html).
...
- `get_future()` - returns `smith._C.Future` object. Supported for NCCL, also supported for most operations on GLOO
...
output = smith.tensor([rank]).cuda(rank)
s = smith.cuda.Stream()
...
with smith.cuda.stream(s):
    s.wait_stream(smith.cuda.default_stream())
    output.add_(100)
if rank == 0:
...
.. class:: reduce_op
    ...
    :class:`~smith.distributed.ReduceOp` is recommended to use instead.
...
{func}`smith.distributed.init_process_group` (by explicitly creating the store
as an alternative to specifying `init_method`.) There are 3 choices for
Key-Value Stores: {class}`~smith.distributed.TCPStore`,
{class}`~smith.distributed.FileStore`, and {class}`~smith.distributed.HashStore`.
...
Note that you can use `smith.profiler` (recommended, only available after 1.8.1) or `smith.autograd.profiler` to profile collective communication and point-to-point communication APIs mentioned here. All out-of-the-box backends (`gloo`,
`nccl`, `mpi`) are supported and collective communication usage will be rendered as expected in profiling output/traces. Profiling your code is the same as any regular smith operator:
...
Please refer to the [profiler documentation](https://blacksmith.org/docs/main/profiler.html) for a full overview of profiler features.

## Multi-GPU collective functions

:::{warning}
The multi-GPU functions (which stand for multiple GPUs per CPU thread) are
deprecated. As of today, Blacksmith Distributed's preferred programming model
...
please contact Blacksmith Distributed's maintainers.
:::
...
Besides the builtin GLOO/MPI/NCCL backends, Blacksmith distributed supports
...
please refer to [Tutorials - Custom C++ and CUDA Extensions](https://blacksmith.org/tutorials/advanced/cpp_extension.html) and
...
name and the instantiating interface through {func}`smith.distributed.Backend.register_backend`
when imported.

When manually importing this backend and invoking {func}`smith.distributed.init_process_group`
with the corresponding backend name, the `smith.distributed` package runs on
the new backend.
...
The `smith.distributed` package also provides a launch utility in
`smith.distributed.launch`. This helper utility can be used to launch
multiple processes per node for distributed training.

```{eval-rst}
.. automodule:: smith.distributed.launch
...
function in {func}`smith.multiprocessing.spawn`. This helper function
...
For references on how to use it, please refer to [Blacksmith example - ImageNet
implementation](https://github.com/blacksmith/examples/tree/master/imagenet)

Note that this function requires Python 3.4 or higher.

## Debugging `smith.distributed` applications

Debugging distributed applications can be challenging due to hard to understand hangs, crashes, or inconsistent behavior across ranks. `smith.distributed` provides
...
Blacksmith offers a customized wrapper around pdb that streamlines the process.

`smith.distributed.breakpoint` makes this process easy. Internally, it customizes `pdb`'s breakpoint behavior in two ways but otherwise behaves as normal `pdb`.
1. Attaches the debugger only on one rank (specified by the user).
2. Ensures all other ranks stop, by using a `smith.distributed.barrier()` that will release once the debugged rank issues a `continue`
3. Reroutes stdin from the child process such that it connects to your terminal.

To use it, simply issue `smith.distributed.breakpoint(rank)` on all ranks, using the same value for `rank` in each case.

### Monitored Barrier

As of v1.10, {func}`smith.distributed.monitored_barrier` exists as an alternative to {func}`smith.distributed.barrier` which fails with helpful information about which rank may be faulty
when crashing, i.e. not all ranks calling into {func}`smith.distributed.monitored_barrier` within the provided timeout. {func}`smith.distributed.monitored_barrier` implements a host-side
...
the barrier in time. As an example, consider the following function where rank 1 fails to call into {func}`smith.distributed.monitored_barrier` (in practice this could be due
...
### `SMITH_DISTRIBUTED_DEBUG`

With `SMITH_CPP_LOG_LEVEL=INFO`, the environment variable `SMITH_DISTRIBUTED_DEBUG` can be used to trigger additional useful logging and collective synchronization checks to ensure all ranks
are synchronized appropriately. `SMITH_DISTRIBUTED_DEBUG` can be set to either `OFF` (default), `INFO`, or `DETAIL` depending on the debugging level
...
Setting `SMITH_DISTRIBUTED_DEBUG=INFO` will result in additional debug logging when models trained with {func}`smith.nn.parallel.DistributedDataParallel` are initialized, and
`SMITH_DISTRIBUTED_DEBUG=DETAIL` will additionally log runtime performance statistics a select number of iterations. These runtime statistics
...
class TwoLinLayerNet(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self.a = smith.nn.Linear(10, 10, bias=False)
        self.b = smith.nn.Linear(10, 1, bias=False)
...
def worker(rank):
    ...
    print("init ddp")
    ddp_model = smith.nn.parallel.DistributedDataParallel(model, device_ids=[rank])

    inp = smith.randn(10, 10).cuda()
    print("train")
...
if __name__ == "__main__":
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "29501"
    os.environ["SMITH_CPP_LOG_LEVEL"]="INFO"
    os.environ[
        "SMITH_DISTRIBUTED_DEBUG"
...
smith_distributed_debug: INFO
```

The following logs are rendered during runtime (when `SMITH_DISTRIBUTED_DEBUG=DETAIL` is set):
...
In addition, `SMITH_DISTRIBUTED_DEBUG=INFO` enhances crash logging in {func}`smith.nn.parallel.DistributedDataParallel` due to unused parameters in the model. Currently, `find_unused_parameters=True`
must be passed into {func}`smith.nn.parallel.DistributedDataParallel` initialization if there are parameters that may be unused in the forward pass, and as of v1.10, all model outputs are required
to be used in loss computation as {func}`smith.nn.parallel.DistributedDataParallel` does not support unused parameters in the backwards pass. These constraints are challenging especially for larger
models, thus when crashing with an error, {func}`smith.nn.parallel.DistributedDataParallel` will log the fully qualified name of all parameters that went unused. For example, in the above application,
...
RuntimeError: Expected to have finished reduction in the prior iteration before starting a new one. This error indicates that your module has parameters that were not used in producing loss. You can enable unused parameter detection by passing
 the keyword argument `find_unused_parameters=True` to `smith.nn.parallel.DistributedDataParallel`, and by
...
Setting `SMITH_DISTRIBUTED_DEBUG=DETAIL` will trigger additional consistency and synchronization checks on every collective call issued by the user
...
{func}`smith.distributed.init_process_group` and {func}`smith.distributed.new_group` APIs. As a result, these APIs will return a wrapper process group that can be used exactly like a regular process
group, but performs consistency checks before dispatching the collective to an underlying process group. Currently, these checks include a {func}`smith.distributed.monitored_barrier`,
...
{func}`smith.distributed.all_reduce`:
...
def worker(rank):
    dist.init_process_group("nccl", rank=rank, world_size=2)
    smith.cuda.set_device(rank)
    tensor = smith.randn(10 if rank == 0 else 20).cuda()
    dist.all_reduce(tensor)
...
if __name__ == "__main__":
    ...
    os.environ["MASTER_PORT"] = "29501"
    os.environ["SMITH_CPP_LOG_LEVEL"]="INFO"
    os.environ["SMITH_DISTRIBUTED_DEBUG"] = "DETAIL"
    mp.spawn(worker, nprocs=2, args=())
```
...
`SMITH_DISTRIBUTED_DEBUG=DETAIL` and reruns the application, the following error message reveals the root cause:
...
:::{note}
For fine-grained control of the debug level during runtime the functions {func}`smith.distributed.set_debug_level`, {func}`smith.distributed.set_debug_level_from_env`, and
{func}`smith.distributed.get_debug_level` can also be used.
:::

In addition, `SMITH_DISTRIBUTED_DEBUG=DETAIL` can be used in conjunction with `SMITH_SHOW_CPP_STACKTRACES=1` to log the entire callstack when a collective desynchronization is detected. These
...
{func}`smith.distributed.init_process_group` and {func}`smith.distributed.new_group` APIs.


### smith.distributed.debug HTTP Server

The `smith.distributed.debug` module provides a HTTP server that can be used to debug distributed applications. The server can
be started by calling {func}`smith.distributed.debug.start_debug_server`. This
allows users to collect data across all workers at runtime.

```{eval-rst}
.. automodule:: smith.distributed.debug
    :members:
    :undoc-members:
    :show-inheritance:
    :special-members: __init__
...
In addition to explicit debugging support via {func}`smith.distributed.monitored_barrier` and `SMITH_DISTRIBUTED_DEBUG`, the underlying C++ library of `smith.distributed` also outputs log
...
following matrix shows how the log level can be adjusted via the combination of `SMITH_CPP_LOG_LEVEL` and `SMITH_DISTRIBUTED_DEBUG` environment variables.

| `SMITH_CPP_LOG_LEVEL` | `SMITH_DISTRIBUTED_DEBUG` | Effective Log Level |
| --------------------- | ------------------------- | ------------------- |
| `ERROR`               | ignored                   | Error               |
...
- `smith.distributed.DistError`: This is the base type of all distributed exceptions.
- `smith.distributed.DistBackendError`: This exception is thrown when a backend-specific error occurs. For example, if
  the `NCCL` backend is used and the user attempts to use a GPU that is not available to the `NCCL` library.
- `smith.distributed.DistNetworkError`: This exception is thrown when networking
  libraries encounter errors (ex: Connection reset by peer)
- `smith.distributed.DistStoreError`: This exception is thrown when the Store encounters
  an error (ex: TCPStore timeout)

```{eval-rst}
.. autoclass:: smith.distributed.DistError
```

```{eval-rst}
.. autoclass:: smith.distributed.DistBackendError
```

```{eval-rst}
.. autoclass:: smith.distributed.DistNetworkError
```

```{eval-rst}
.. autoclass:: smith.distributed.DistStoreError
```
...
```{eval-rst}
.. autofunction:: smith.distributed.breakpoint
```
...
```{eval-rst}
.. py:module:: smith.distributed.algorithms
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.model_averaging
```

```{eval-rst}
.. py:module:: smith.distributed.elastic
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.data
```

```{eval-rst}
.. py:module:: smith.distributed.launcher
```

```{eval-rst}
.. py:module:: smith.distributed.nn
```

```{eval-rst}
.. py:module:: smith.distributed.nn.api
```

```{eval-rst}
.. py:module:: smith.distributed.nn.jit
```

```{eval-rst}
.. py:module:: smith.distributed.nn.jit.templates
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.ddp_zero_hook
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.debugging_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.default_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.mixed_precision_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.optimizer_overlap_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.post_localSGD_hook
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.powerSGD_hook
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.ddp_comm_hooks.quantization_hooks
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.join
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.model_averaging.averagers
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.model_averaging.hierarchical_model_averager
```

```{eval-rst}
.. py:module:: smith.distributed.algorithms.model_averaging.utils
```

```{eval-rst}
.. py:module:: smith.distributed.argparse_util
```

```{eval-rst}
.. py:module:: smith.distributed.c10d_logger
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.api
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.default_planner
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.filesystem
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.hf_storage
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.quantized_hf_storage
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.metadata
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.optimizer
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.planner
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.planner_helpers
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.resharding
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.state_dict_loader
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.state_dict_saver
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.stateful
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.storage
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.utils
```

```{eval-rst}
.. py:module:: smith.distributed.collective_utils
```

```{eval-rst}
.. py:module:: smith.distributed.constants
```

```{eval-rst}
.. py:module:: smith.distributed.device_mesh
```

```{eval-rst}
.. py:module:: smith.distributed.distributed_c10d
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.agent.server.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.agent.server.local_elastic_agent
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.events.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.events.handlers
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.metrics.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.multiprocessing.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.multiprocessing.errors.error_handler
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.multiprocessing.errors.handlers
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.multiprocessing.redirects
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.multiprocessing.tail_log
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.c10d_rendezvous_backend
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.dynamic_rendezvous
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.etcd_rendezvous
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.etcd_rendezvous_backend
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.etcd_server
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.etcd_store
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.static_tcp_rendezvous
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.rendezvous.utils
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.timer.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.timer.file_based_local_timer
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.timer.local_timer
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.api
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.data.cycling_iterator
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.data.elastic_distributed_sampler
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.distributed
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.log_level
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.logging
```

```{eval-rst}
.. py:module:: smith.distributed.elastic.utils.store
```

```{eval-rst}
.. py:module:: smith.distributed.fsdp.api
```

```{eval-rst}
.. py:module:: smith.distributed.fsdp.fully_sharded_data_parallel
```

```{eval-rst}
.. py:module:: smith.distributed.fsdp.sharded_grad_scaler
```

```{eval-rst}
.. py:module:: smith.distributed.fsdp.wrap
```

```{eval-rst}
.. py:module:: smith.distributed.launcher.api
```

```{eval-rst}
.. py:module:: smith.distributed.logging_handlers
```

```{eval-rst}
.. py:module:: smith.distributed.nn.api.remote_module
```

```{eval-rst}
.. py:module:: smith.distributed.nn.functional
```

```{eval-rst}
.. py:module:: smith.distributed.nn.jit.instantiator
```

```{eval-rst}
.. py:module:: smith.distributed.nn.jit.templates.remote_module_template
```

```{eval-rst}
.. py:module:: smith.distributed.optim.apply_optimizer_in_backward
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_adadelta
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_adagrad
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_adam
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_adamax
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_adamw
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_rmsprop
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_rprop
```

```{eval-rst}
.. py:module:: smith.distributed.optim.functional_sgd
```

```{eval-rst}
.. py:module:: smith.distributed.optim.named_optimizer
```

```{eval-rst}
.. py:module:: smith.distributed.optim.optimizer
```

```{eval-rst}
.. py:module:: smith.distributed.optim.post_localSGD_optimizer
```

```{eval-rst}
.. py:module:: smith.distributed.optim.utils
```

```{eval-rst}
.. py:module:: smith.distributed.optim.zero_redundancy_optimizer
```

```{eval-rst}
.. py:module:: smith.distributed.remote_device
```

```{eval-rst}
.. py:module:: smith.distributed.rendezvous
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.api
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.backend_registry
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.constants
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.functions
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.internal
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.options
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.rref_proxy
```

```{eval-rst}
.. py:module:: smith.distributed.rpc.server_process_global_profiler
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.api
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.ddp
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.fsdp
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.input_reshard
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.loss
```

```{eval-rst}
.. py:module:: smith.distributed.tensor.parallel.style
```

```{eval-rst}
.. py:module:: smith.distributed.utils
```

```{eval-rst}
.. py:module:: smith.distributed.checkpoint.state_dict
```
