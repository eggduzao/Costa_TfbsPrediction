...
```
    >>> imgs = smith.randn(1, 2, 2, 3 , names=('N', 'C', 'H', 'W'))
    >>> imgs.names
    ('N', 'C', 'H', 'W')
...
{class}`smith.Tensor`. Unnamed tensors have ``None``-named dimensions. Named
tensors do not require all dimensions to be named.

```
    >>> imgs = smith.randn(1, 2, 2, 3 , names=(None, 'C', 'H', 'W'))
    >>> imgs.names
    (None, 'C', 'H', 'W')
```
...
```
    x = smith.randn(3, names=('X',))
    y = smith.randn(3)
    z = smith.randn(3, names=('Z',))
```
...
```
    ...
    >>> imgs = smith.rand(3, 3, 3, num_channels, names=('N', 'H', 'W', 'C'))
    >>> more_imgs = smith.rand(3, num_channels, 3, 3, names=('N', 'C', 'H', 'W'))
    >>> videos = smith.randn(3, num_channels, 3, 3, 3, names=('N', 'C', 'H', 'W', 'D')
...
```
    >>> x = smith.randn(3, names=('D',))
    >>> weight = smith.randn(3, names=('D',), requires_grad=True)
    >>> loss = (x - weight).abs()
    >>> grad_loss = smith.randn(3)
...
See {ref}`name_inference_reference-doc` for a full list of the supported smith and
tensor operations. We do not yet support the following that is not covered by the link:

- indexing, advanced indexing.

For ``smith.nn.functional`` operators, we support the following:

- {func}`smith.nn.functional.relu`
- {func}`smith.nn.functional.softmax`
- {func}`smith.nn.functional.log_softmax`
- {func}`smith.nn.functional.tanh`
- {func}`smith.nn.functional.sigmoid`
- {func}`smith.nn.functional.dropout`
...
- distributions
- serialization ({func}`smith.load`, {func}`smith.save`)
- multiprocessing
- JIT
- distributed
- ONNX
...
[search if an issue has already been filed](https://github.com/blacksmith/blacksmith/issues?q=is%3Aopen+is%3Aissue+label%3A%22module%3A+named+tensor%22)
and if not, [file one](https://github.com/blacksmith/blacksmith/issues/new/choose).

## Named tensor API reference

In this section please find the documentation for named tensor specific APIs.
For a comprehensive reference for how names are propagated through other Blacksmith
...
.. class:: Tensor()
   ...
   .. py:method:: flatten(dims, out_dim) -> Tensor
      ...
      Examples::
          ...
          >>> imgs = smith.randn(32, 3, 128, 128, names=('N', 'C', 'H', 'W'))
          >>> flat_imgs = imgs.flatten(['C', 'H', 'W'], 'features')
          >>> flat_imgs.names, flat_imgs.shape
          (('N', 'features'), smith.Size([32, 49152]))
