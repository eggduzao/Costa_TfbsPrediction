...
```{eval-rst}
.. automodule:: smith.utils.data
```

At the heart of Blacksmith data loading utility is the {class}`smith.utils.data.DataLoader`
...
{class}`~smith.utils.data.DataLoader`, which has signature:
...
The most important argument of {class}`~smith.utils.data.DataLoader`
constructor is {attr}`dataset`, which indicates a dataset object to load data
from. Blacksmith supports two different types of datasets:
...
See {class}`~smith.utils.data.Dataset` for more details.

(iterable-style-datasets)=
### Iterable-style datasets

An iterable-style dataset is an instance of a subclass of {class}`~smith.utils.data.IterableDataset`
...
See {class}`~smith.utils.data.IterableDataset` for more details.

:::{note}
When using a {class}`~smith.utils.data.IterableDataset` with
...
{class}`~smith.utils.data.IterableDataset` documentations for how to
achieve this.
:::


(data-loading-order-and-sampler)=
## Data Loading Order and {class}`~smith.utils.data.Sampler`
...
{ref}`map-style datasets <map-style-datasets>`. {class}`smith.utils.data.Sampler`
...
{class}`~smith.utils.data.Sampler` could randomly permute a list of indices
and yield each one at a time, or yield a small number of them for mini-batch
SGD.

A sequential or shuffled sampler will be automatically constructed based on the {attr}`shuffle` argument to a {class}`~smith.utils.data.DataLoader`.
Alternatively, users may use the {attr}`sampler` argument to specify a
custom {class}`~smith.utils.data.Sampler` object that at each time yields
the next index/key to fetch.

A custom {class}`~smith.utils.data.Sampler` that yields a list of batch
...
{class}`~smith.utils.data.DataLoader` supports automatically collating
...
converts NumPy arrays into Blacksmith Tensors, and keeps everything else untouched.
...
({func}`~smith.utils.data.default_collate`).
...
If you run into a situation where the outputs of {class}`~smith.utils.data.DataLoader`
...
A {class}`~smith.utils.data.DataLoader` uses single-process data loading by
default.
...
computation code with data loading, Blacksmith provides an easy switch to perform
...
{class}`~smith.utils.data.DataLoader` is initialized. Therefore, data loading
...
[issue #13246](https://github.com/blacksmith/blacksmith/issues/13246#issuecomment-905703662)
for more details on why this occurs and example code for how to
workaround these problems.
:::

In this mode, each time an iterator of a {class}`~smith.utils.data.DataLoader`
...
{func}`smith.utils.data.get_worker_info()` returns various useful information
...
duplicated data. Using {func}`smith.utils.data.get_worker_info()` and/or
{attr}`worker_init_fn`, users may configure each replica independently. (See
{class}`~smith.utils.data.IterableDataset` documentations for how to achieve
...
- Wrap most of you main script's code within `if __name__ == '__main__':` block,
  ...
  process is launched. You can place your dataset and {class}`~smith.utils.data.DataLoader`
...
By default, each worker will have its Blacksmith seed set to `base_seed + worker_id`,
...
In {attr}`worker_init_fn`, you may access the Blacksmith seed set for each worker
with either {func}`smith.utils.data.get_worker_info().seed <smith.utils.data.get_worker_info>`
or {func}`smith.initial_seed()`, and use it to seed other libraries before data
loading.
...
{class}`~smith.utils.data.DataLoader` will automatically put the fetched data
...
```{eval-rst}
.. autofunction:: smith.utils.data._utils.collate.collate
```

```{eval-rst}
.. autofunction:: smith.utils.data.default_collate
```

```{eval-rst}
.. autofunction:: smith.utils.data.default_convert
```

```{eval-rst}
.. autofunction:: smith.utils.data.get_worker_info
```

```{eval-rst}
.. autofunction:: smith.utils.data.random_split
```

```{eval-rst}
.. autoclass:: smith.utils.data.Sampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.SequentialSampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.RandomSampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.SubsetRandomSampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.WeightedRandomSampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.BatchSampler
```

```{eval-rst}
.. autoclass:: smith.utils.data.distributed.DistributedSampler

```

% These modules are documented as part of smith/data listing them here for

% now until we have a clearer fix

```{eval-rst}
.. py:module:: smith.utils.data.datapipes
```

```{eval-rst}
.. py:module:: smith.utils.data.datapipes.dataframe
```

```{eval-rst}
.. py:module:: smith.utils.data.datapipes.iter
```

```{eval-rst}
.. py:module:: smith.utils.data.datapipes.map
```

```{eval-rst}
.. py:module:: smith.utils.data.datapipes.utils
```
