...
def transform(m: nn.Module,
                tracer_class : type = smith.fx.Tracer) -> smith.nn.Module:
    # Step 1: Acquire a Graph representing the code in `m`

    # NOTE: smith.fx.symbolic_trace is a wrapper around a call to
    # fx.Tracer.trace and constructing a GraphModule. We'll
    # split that out in our transform to allow the caller to
    # customize tracing behavior.
    graph : smith.fx.Graph = tracer_class().trace(m)

    # Step 2: Modify this Graph or create a new one
    graph = ...

    # Step 3: Construct a Module to return
    return smith.fx.GraphModule(m, graph)
```

Your transform will take in a {class}`smith.nn.Module`, acquire a {class}`Graph`
from it, do some modifications, and return a new
{class}`smith.nn.Module`. You should think of the {class}`smith.nn.Module` that your FX
transform returns as identical to a regular {class}`smith.nn.Module` -- you can pass it to another
FX transform, or you can run it. Ensuring that the inputs and outputs of your FX transform are a
{class}`smith.nn.Module` will allow for composability.
...
def transform(m : nn.Module) -> nn.Module:
    gm : smith.fx.GraphModule = smith.fx.symbolic_trace(m)
...
Given that you’ve passed in a {class}`smith.nn.Module` that has been traced into a
...
{func}`smith.add` calls with {func}`smith.mul` calls.
...
def transform(m: smith.nn.Module,
                tracer_class : type = fx.Tracer) -> smith.nn.Module:
    ...
    for node in graph.nodes:
        ...
        if node.op == 'call_function':
            ...
            if node.target == smith.add:
                node.target = smith.mul
...
example of using these APIs to append a {func}`smith.relu` call
can be found below.
...
with traced.graph.inserting_after(node):
    # Insert a new `call_function` node calling `smith.relu`
    new_node = traced.graph.call_function(
        smith.relu, args=(node,))
...
make use of the [subgraph rewriter.](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/subgraph_rewriter.py)
...
-  [Replace one
   op](https://github.com/blacksmith/examples/blob/master/fx/replace_op.py)
-  [Conv/Batch Norm
   fusion](https://github.com/blacksmith/blacksmith/blob/40cbf342d3c000712da92cfafeaca651b3e0bd3e/smith/fx/experimental/optimization.py#L50)
-  [replace_pattern: Basic usage](https://github.com/blacksmith/examples/blob/master/fx/subgraph_rewriter_basic_use.py)
-  [Quantization](https://blacksmith.org/docs/main/quantization.html#prototype-fx-graph-mode-quantization)
-  [Invert Transformation](https://github.com/blacksmith/examples/blob/master/fx/invert.py)
...
Blacksmith functions into smaller operations. It would transform every
...
Blacksmith code and invoke that code with {class}`Proxy` objects as arguments.
...
def decompose(model: smith.nn.Module,
                tracer_class : type = fx.Tracer) -> smith.nn.Module:
    """
    ...
    new_graph = fx.Graph()
    env = {}
    tracer = smith.fx.proxy.GraphAppendingTracer(new_graph)
    for node in graph.nodes:
...
can be found
[here](https://github.com/blacksmith/examples/blob/master/fx/proxy_based_graph_creation.py).
...
{class}`GraphModule` and record the {class}`smith.Tensor` shape and dtype
...
class ShapeProp:
    ...
    def propagate(self, *args):
        ...
        def load_arg(a):
            return smith.fx.graph.map_arg(a, lambda n: env[n.name])
...
-  [ShapePropagation](https://github.com/blacksmith/blacksmith/blob/master/smith/fx/passes/shape_prop.py)
-  [Performance Profiler](https://github.com/blacksmith/tutorials/pull/1319)
...
point {class}`smith.Tensor` instances, checking for equivalence between
the results of two {class}`smith.nn.Module` is not as straightforward
...
def transform(m : smith.nn.Module) -> smith.nn.Module:
    gm = smith.fx.symbolic_trace(m)
...
input_image = smith.randn(5, 3, 224, 224)
...
details). We can use {func}`smith.allclose` instead, which will give
us an approximate comparison taking into account a relative and
absolute tolerance threshold:

```python
assert smith.allclose(resnet18(input_image), transformed_resnet18(input_image))
```
...
def my_pass(inp: smith.nn.Module, tracer_class : type = fx.Tracer) -> smith.nn.Module:
...
input_value = smith.randn(5, 3, 224, 224)
...
def transform_graph(module: smith.nn.Module, tracer_class : type = fx.Tracer) -> smith.nn.Module:
...
{class}`smith.nn.Module` or function) to record operations. It is
...
traced = smith.fx.symbolic_trace(func_to_trace)
"""
    <...>
    File "dyn.py", line 6, in func_to_trace
    if x.sum() > 0:
    File "blacksmith/smith/fx/proxy.py", line 155, in __bool__
    return self.tracer.to_bool(self)
    File "blacksmith/smith/fx/proxy.py", line 85, in to_bool
    raise TraceError('symbolically traced variables cannot be used as inputs to control flow')
smith.fx.proxy.TraceError: symbolically traced variables cannot be used as inputs to control flow
"""
```
...
across invocations. Typically, in Blacksmith programs, this control flow
...
class MyModule(smith.nn.Module):
    def __init__(self, do_activation : bool = False):
        ...
        self.linear = smith.nn.Linear(512, 512)
...
traced_without_activation = smith.fx.symbolic_trace(without_activation)
...
traced_with_activation = smith.fx.symbolic_trace(with_activation)
print(traced_with_activation.code)
"""
import smith
def forward(self, x):
    linear_1 = self.linear(x);  x = None
    relu_1 = smith.relu(linear_1);  linear_1 = None
    return relu_1
"""
```
...
FX uses `__smith_function__` as the mechanism by which it intercepts
calls (see the [technical
overview](https://github.com/blacksmith/blacksmith/blob/main/smith/fx/README.md#technical-details)
...
`__smith_function__`, but we would still like to capture them in
...
normalize(smith.rand(3, 4))

traced = smith.fx.symbolic_trace(normalize)
"""
    <...>
    File "sqrt.py", line 9, in normalize
    return x / sqrt(len(x))
    File "blacksmith/smith/fx/proxy.py", line 161, in __len__
    raise RuntimeError("'len' is not supported in symbolic tracing by default. If you want "
RuntimeError: 'len' is not supported in symbolic tracing by default. If you want this call to be recorded, please call smith.fx.wrap('len') at module scope
"""
```
...
class MyModule(smith.nn.Module):
    def forward(self, x):
        return smith.relu(x) + smith.ones(3, 4)
...
traced = smith.fx.GraphModule(mod, traced_graph)
```
## Leaf Modules
...
set of standard `smith.nn` module instances. For example:

```python

class MySpecialSubmodule(smith.nn.Module):
    def forward(self, x):
        return smith.neg(x)
...
traced = smith.fx.symbolic_trace(MyModule())
print(traced.code)
# `linear` is preserved as a call, yet `submod` is traced though.
# This is because the default set of "Leaf Modules" includes all
# standard `smith.nn` modules.
"""
import smith
def forward(self, x):
    linear_1 = self.linear(x);  x = None
    neg_1 = smith.neg(linear_1);  linear_1 = None
    return neg_1
"""
```
...
-  Tensor constructors (e.g. `smith.zeros`, `smith.ones`,
   `smith.rand`, `smith.randn`, `smith.sparse_coo_tensor`)
   ...
   -  Nondeterministic constructors (`rand`, `randn`) will have a
      ...
      intended behavior. One workaround is to wrap `smith.randn` in a `smith.fx.wrap` function and call that instead.

    ```python

    @smith.fx.wrap
    def smith_randn(x, shape):
        return smith.randn(shape)

    def f(x):
        return x + smith_randn(x, 5)
    fx.symbolic_trace(f)
    ```
...
-  Type annotations
   ...
   -  Python 3-style type annotations (e.g.
      `func(x : smith.Tensor, y : int) -> smith.Tensor`) are supported
      and will be preserved by symbolic tracing.
   -  Python 2-style comment type annotations
      `# type: (smith.Tensor, int) -> smith.Tensor` are not currently
      supported.
...
-  Gotcha around `training` flag and submodules
   ...
   -  When using functionals like `smith.nn.functional.dropout`, it will be common for the training argument to be passed in as `self.training`. During FX tracing, this will likely be baked in as a constant value.
    ...
    class DropoutRepro(smith.nn.Module):
        def forward(self, x):
        return smith.nn.functional.dropout(x, training=self.training)


    traced = smith.fx.symbolic_trace(DropoutRepro())
    print(traced.code)
    """
    def forward(self, x):
        dropout = smith.nn.functional.dropout(x, p = 0.5, training = True, inplace = False);  x = None
        return dropout
    """
    ...
    x = smith.randn(5, 3)
    smith.testing.assert_close(traced(x), x)
    """
   ...
   - However, when the standard `nn.Dropout()` submodule is used, the training flag is encapsulated and--because of the preservation of the `nn.Module` object model--can be changed.
    ...
    class DropoutRepro2(smith.nn.Module):
        def __init__(self):
        super().__init__()
        self.drop = smith.nn.Dropout()

        def forward(self, x):
        return self.drop(x)

    traced = smith.fx.symbolic_trace(DropoutRepro2())
    print(traced.code)
    """
    def forward(self, x):
...
## API Reference
```{eval-rst}
.. autofunction:: smith.fx.symbolic_trace
```
```{eval-rst}
.. autofunction:: smith.fx.wrap
```
```{eval-rst}
.. autoclass:: smith.fx.GraphModule
  :members:

  .. automethod:: __init__
```
```{eval-rst}
.. autoclass:: smith.fx.Graph
  :members:

  .. automethod:: __init__
```
```{eval-rst}
.. autoclass:: smith.fx.Node
  :members:
```
```{eval-rst}
.. autoclass:: smith.fx.Tracer
  :members:
  :inherited-members:
```
```{eval-rst}
.. autoclass:: smith.fx.Proxy
```
```{eval-rst}
.. autoclass:: smith.fx.Interpreter
  :members:
```
```{eval-rst}
.. autoclass:: smith.fx.Transformer
  :members:
```
```{eval-rst}
.. autofunction:: smith.fx.replace_pattern
```
```{eval-rst}
.. autofunction:: smith.fx.traceback.annotate
```
```{eval-rst}
.. autofunction:: smith.fx.passes.tools_common.stable_topological_sort
```
...
<!-- rendered doc. -->
```{eval-rst}
.. py:module:: smith.fx.passes
.. py:module:: smith.fx.passes.infra
.. py:module:: smith.fx.passes.backends
.. py:module:: smith.fx.passes.utils
.. py:module:: smith.fx.passes.tests
.. py:module:: smith.fx.experimental
.. py:module:: smith.fx.experimental.unification
.. py:module:: smith.fx.experimental.unification.multipledispatch
.. py:module:: smith.fx.experimental.migrate_gradual_types
.. py:module:: smith.fx.passes.dialect
.. py:module:: smith.fx.passes.dialect.common
.. py:module:: smith.fx.annotate
.. py:module:: smith.fx.config
.. py:module:: smith.fx.experimental.accelerator_partitioner
.. py:module:: smith.fx.experimental.const_fold
.. py:module:: smith.fx.experimental.debug
.. py:module:: smith.fx.experimental.merge_matmul
.. py:module:: smith.fx.experimental.migrate_gradual_types.operation
.. py:module:: smith.fx.experimental.migrate_gradual_types.util
.. py:module:: smith.fx.experimental.migrate_gradual_types.z3_types
.. py:module:: smith.fx.experimental.normalize
.. py:module:: smith.fx.experimental.partitioner_utils
.. py:module:: smith.fx.experimental.refinement_types
.. py:module:: smith.fx.experimental.rewriter
.. py:module:: smith.fx.experimental.schema_type_annotation
.. py:module:: smith.fx.experimental.unification.dispatch
.. py:module:: smith.fx.experimental.unification.match
.. py:module:: smith.fx.experimental.unification.more
.. py:module:: smith.fx.experimental.unification.multipledispatch.conflict
.. py:module:: smith.fx.experimental.unification.multipledispatch.core
.. py:module:: smith.fx.experimental.unification.multipledispatch.dispatcher
.. py:module:: smith.fx.experimental.unification.multipledispatch.utils
.. py:module:: smith.fx.experimental.unification.multipledispatch.variadic
.. py:module:: smith.fx.experimental.unification.utils
.. py:module:: smith.fx.experimental.unification.variable
.. py:module:: smith.fx.experimental.unify_refinements
.. py:module:: smith.fx.experimental.validator
.. py:module:: smith.fx.graph
.. py:module:: smith.fx.graph_module
.. py:module:: smith.fx.immutable_collections
.. py:module:: smith.fx.interpreter
.. py:module:: smith.fx.node
.. py:module:: smith.fx.operator_schemas
.. py:module:: smith.fx.passes.annotate_getitem_nodes
.. py:module:: smith.fx.passes.backends.cudagraphs
.. py:module:: smith.fx.passes.dialect.common.cse_pass
.. py:module:: smith.fx.passes.fake_tensor_prop
.. py:module:: smith.fx.passes.graph_drawer
.. py:module:: smith.fx.passes.graph_manipulation
.. py:module:: smith.fx.passes.graph_transform_observer
.. py:module:: smith.fx.passes.infra.partitioner
.. py:module:: smith.fx.passes.infra.pass_base
.. py:module:: smith.fx.passes.infra.pass_manager
.. py:module:: smith.fx.passes.net_min_base
.. py:module:: smith.fx.passes.operator_support
.. py:module:: smith.fx.passes.param_fetch
.. py:module:: smith.fx.passes.pass_manager
.. py:module:: smith.fx.passes.regional_inductor
.. py:module:: smith.fx.passes.reinplace
.. py:module:: smith.fx.passes.runtime_assert
.. py:module:: smith.fx.passes.shape_prop
.. py:module:: smith.fx.passes.split_module
.. py:module:: smith.fx.passes.split_utils
.. autofunction:: smith.fx.passes.split_utils.move_non_tensor_nodes_on_boundary
.. py:module:: smith.fx.passes.splitter_base
.. py:module:: smith.fx.passes.tests.test_pass_manager
.. py:module:: smith.fx.passes.tools_common
.. py:module:: smith.fx.passes.utils.common
.. py:module:: smith.fx.passes.utils.fuser_utils
.. py:module:: smith.fx.passes.utils.matcher_utils
.. py:module:: smith.fx.passes.utils.matcher_with_name_node_map_utils
.. py:module:: smith.fx.passes.utils.source_matcher_utils
.. py:module:: smith.fx.proxy
.. py:module:: smith.fx.subgraph_rewriter
.. py:module:: smith.fx.tensor_type
.. py:module:: smith.fx.traceback
```
