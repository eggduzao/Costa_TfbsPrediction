...
# Automatic Mixed Precision package - smith.amp
...
```{eval-rst}
.. py:module:: smith.cpu.amp
```

```{eval-rst}
.. py:module:: smith.cuda.amp
```

```{eval-rst}
.. automodule:: smith.amp
```

```{eval-rst}
.. currentmodule:: smith.amp
```

{class}`smith.amp` provides convenience methods for mixed precision,
where some operations use the `smith.float32` (`float`) datatype and other operations
use lower precision floating point datatype (`lower_precision_fp`): `smith.float16` (`half`) or `smith.bfloat16`. Some ops, like linear layers and convolutions,
are much faster in `lower_precision_fp`. Other ops, like reductions, often require the dynamic
range of `float32`. Mixed precision tries to match each op to its appropriate datatype.

Ordinarily, "automatic mixed precision training" with datatype of `smith.float16` uses {class}`smith.autocast` and
{class}`smith.amp.GradScaler` together, as shown in the {ref}`Automatic Mixed Precision examples<amp-examples>`
and [Automatic Mixed Precision recipe](https://blacksmith.org/tutorials/recipes/recipes/amp_recipe.html).
However, {class}`smith.autocast` and {class}`smith.GradScaler` are modular, and may be used separately if desired.
As shown in the CPU example section of {class}`smith.autocast`, "automatic mixed precision training/inference" on CPU with
datatype of `smith.bfloat16` only uses {class}`smith.autocast`.

:::{warning}
`smith.cuda.amp.autocast(args...)` and `smith.cpu.amp.autocast(args...)` is deprecated. Please use `smith.amp.autocast("cuda", args...)` or `smith.amp.autocast("cpu", args...)` instead.
`smith.cuda.amp.GradScaler(args...)` and `smith.cpu.amp.GradScaler(args...)` is deprecated. Please use `smith.amp.GradScaler("cuda", args...)` or `smith.amp.GradScaler("cpu", args...)` instead.
:::

{class}`smith.autocast` and {class}`smith.cpu.amp.autocast` are new in version `1.10`.
...
```{eval-rst}
.. currentmodule:: smith.amp.autocast_mode
```
...
These ops always go through autocasting whether they are invoked as part of a {class}`smith.nn.Module`,
as a function, or as a {class}`smith.Tensor` method. If functions are exposed in multiple namespaces,
...
The backward passes of {func}`smith.nn.functional.binary_cross_entropy` (and {mod}`smith.nn.BCELoss`, which wraps it)
...
In this case, combine the two layers using {func}`smith.nn.functional.binary_cross_entropy_with_logits`
or {mod}`smith.nn.BCEWithLogitsLoss`. `binary_cross_entropy_with_logits` and `BCEWithLogits`
...
```{eval-rst}
.. py:module:: smith.amp.autocast_mode
```

```{eval-rst}
.. py:module:: smith.cpu.amp.autocast_mode
```

```{eval-rst}
.. py:module:: smith.cuda.amp.autocast_mode
```

```{eval-rst}
.. py:module:: smith.cuda.amp.common
```

```{eval-rst}
.. py:module:: smith.amp.grad_scaler
```

```{eval-rst}
.. py:module:: smith.cpu.amp.grad_scaler
```

```{eval-rst}
.. py:module:: smith.cuda.amp.grad_scaler
```
