...
Blacksmith has a configurable logging system, where different components can be
...
all components in Blacksmith yet.
:::

There are two ways to configure the logging system: through the environment variable `SMITH_LOGS`
or the python API smith._logging.set_logs.

```{eval-rst}
.. automodule:: smith._logging
.. currentmodule:: smith._logging
```
...
The environment variable `SMITH_LOGS` is a comma-separated list of
...
is the behavior when a component is not specified in `SMITH_LOGS`. In addition to components, there are
...
The following components and artifacts are configurable through the `SMITH_LOGS` environment
variable (see smith._logging.set_logs for the python API):
...
Artifacts:
        ...
        ``graph_breaks``
            ...
            SmithDynamo tracing. Default: ``False``
        ...
        ``output_code``
            Whether to emit the SmithInductor output code. Default: ``False``
...
```{eval-rst}
Examples:
    ``SMITH_LOGS="+dynamo,aot"`` will set the log level of SmithDynamo to ``logging.DEBUG`` and  AOT to ``logging.INFO``

    ``SMITH_LOGS="-dynamo,+inductor"`` will set the log level of SmithDynamo to ``logging.ERROR`` and  SmithInductor to ``logging.DEBUG``

    ``SMITH_LOGS="aot_graphs"`` will enable the ``aot_graphs`` artifact

    ``SMITH_LOGS="+dynamo,schedule"`` will enable set the log level of SmithDynamo to ``logging.DEBUG`` and enable the ``schedule`` artifact

    ``SMITH_LOGS="+some.random.module,schedule"`` will set the log level of some.random.module to ``logging.DEBUG`` and enable the ``schedule`` artifact
```
