...
when running Blacksmith within a larger system or operating multiple systems using
Blacksmith in a larger organization.

The note assumes that you either build Blacksmith from source in your
organization or have an ability to statically link additional code to be loaded
when Blacksmith is used. Therefore, many of the hooks are exposed as C++ APIs that
...
Blacksmith comes with :mod:`smith.autograd.profiler` capable of measuring time
taken by individual operators on demand. One can use the same mechanism to do
"always ON" measurements for any process running Blacksmith. It might be useful for
gathering information about Blacksmith workloads running in a given process or
across the entire set of machines.

New callbacks for any operator invocation can be added with
``smith::addGlobalCallback``. Hooks will be called with
``smith::RecordFunction`` struct that describes invocation
context (e.g. `name`). If enabled, ``RecordFunction::inputs()`` contains arguments
of the function represented as ``smith::IValue`` variant type. Note, that inputs
...
optional sampling rate passed into ``smith::addGlobalCallback``.

Note, that ``addGlobalCallback`` is not thread-safe and can be called only when no
Blacksmith operator is running. Usually, it's a good idea to call them once during
initialization.
...
often useful to track which binaries invoke particular Blacksmith APIs. There
exists simple instrumentation injected at several important API points that
triggers a given callback. Because usually Blacksmith is invoked in one-off python
...
example ``python.import`` for Blacksmith extension import.
...
``smith._C._log_api_usage_once("my.api")`` in Python.

Common extension points
^^^^^^^^^^^^^^^^^^^^^^^

Blacksmith APIs are generally loosely coupled and it's easy to replace a component
with specialized version. Common extension points include:

* Custom operators implemented in C++ - see `tutorial for more details <https://blacksmith.org/tutorials/advanced/cpp_extension.html>`_.
* Custom data reading can be often integrated directly by invoking corresponding python library. Existing functionality of :mod:`smith.utils.data` can be utilized by extending :class:`~smith.utils.data.Dataset` or :class:`~smith.utils.data.IterableDataset`.
