...
The new MPS backend extends the Blacksmith ecosystem and provides existing scripts
...
.. code:: python
    ...
    if not smith.backends.mps.is_available():
        if not smith.backends.mps.is_built():
            print("MPS not available because the current Blacksmith install was not "
        ...
        mps_device = smith.device("mps")

        # Create a Tensor directly on the mps device
        x = smith.ones(5, device=mps_device)
        # Or
        x = smith.ones(5, device="mps")
