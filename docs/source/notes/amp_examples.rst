...
:class:`smith.autocast` and :class:`smith.amp.GradScaler` together.

Instances of :class:`smith.autocast` enable autocasting for chosen regions.
Autocasting automatically chooses the precision for operations to improve performance
while maintaining accuracy.

Instances of :class:`smith.amp.GradScaler` help perform the steps of
...
:class:`smith.autocast` and :class:`smith.amp.GradScaler` are modular.
In the samples below, each is used as its individual documentation suggests.

(Samples here are illustrative.  See the
`Automatic Mixed Precision recipe <https://blacksmith.org/tutorials/recipes/recipes/amp_recipe.html>`_
...
::
    ...
    for epoch in epochs:
        for input, target in data:
            ...
            with autocast(device_type='cuda', dtype=smith.float16):
                output = model(input)
...
(see :func:`smith.nn.utils.clip_grad_norm_`) or maximum magnitude (see :func:`smith.nn.utils.clip_grad_value_`)
...
.. currentmodule:: smith.amp.GradScaler
...
batch are accumulated.  If grads are unscaled (or the scale facsmithanges) before accumulation is complete,
...
:func:`smith.autograd.grad`, combines them to create the penalty value,
...
Here's an ordinary example of an L2 penalty without gradient scaling or autocasting::
    ...
    for epoch in epochs:
        for input, target in data:
            ...
            # Creates gradients
            grad_params = smith.autograd.grad(outputs=loss,
...
passed to :func:`smith.autograd.grad` should be scaled.  The resulting gradients
...
Here's how that looks for the same L2 penalty::
    ...
    for epoch in epochs:
        for input, target in data:
            ...
            scaled_grad_params = smith.autograd.grad(outputs=scaler.scale(loss),
                                                     inputs=model.parameters(),
                                                     create_graph=True)
...
Even if :class:`smith.nn.DataParallel` spawns threads to run the forward pass on each device.
...
:class:`smith.nn.parallel.DistributedDataParallel`'s documentation recommends one GPU per process for best
...
Here :class:`smith.nn.parallel.DistributedDataParallel` may spawn a side thread to run the forward pass on each
device, like :class:`smith.nn.DataParallel`.  :ref:`The fix is the same<amp-dataparallel>`:
...
(subclasses of :class:`smith.autograd.Function`), changes are required for
...
* requires a particular ``dtype`` (for example, if it wraps
  `CUDA extensions <https://blacksmith.org/tutorials/advanced/cpp_extension.html>`_
...
is to disable autocast and force execution in ``float32`` ( or ``dtype``) at any points of use where errors occur::
    ...
    with autocast(device_type='cuda', dtype=smith.float16):
        ...
        with autocast(device_type='cuda', dtype=smith.float16, enabled=False):
            output = imported_function(input1.float(), input2.float())

If you're the function's author (or can alter its definition) a better solution is to use the
:func:`smith.amp.custom_fwd` and :func:`smith.amp.custom_bwd` decorators as shown in
...
Consider a custom function that requires ``smith.float32`` inputs.
Apply :func:`custom_fwd(device_type='cuda', cast_inputs=smith.float32)<custom_fwd>` to ``forward``
...
`CUDA` in this example, and locally disable autocast during ``forward`` and ``backward``::
    ...
    class MyFloat32Func(smith.autograd.Function):
        @staticmethod
        @custom_fwd(device_type='cuda', cast_inputs=smith.float32)
