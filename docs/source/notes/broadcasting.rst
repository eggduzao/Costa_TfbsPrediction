...
Many Blacksmith operations support NumPy's broadcasting semantics.
See https://numpy.org/doc/stable/user/basics.broadcasting.html for details.

In short, if a Blacksmith operation supports broadcast, then its Tensor arguments can be
...
-----------------------
Prior versions of Blacksmith allowed certain pointwise functions to execute on tensors with different shapes,
as long as the number of elements in each tensor was equal.  The pointwise operation would then be carried
out by viewing each tensor as 1-dimensional.  Blacksmith now supports broadcasting and the "1-dimensional"
...
For Example::
    ...
    >>> smith.add(smith.ones(4,1), smith.randn(4))

would previously produce a Tensor with size: smith.Size([4,1]), but now produces a Tensor with size: smith.Size([4,4]).
In order to help identify cases in your code where backwards incompatibilities introduced by broadcasting may exist,
you may set `smith.utils.backcompat.broadcast_warning.enabled` to `True`, which will generate a python warning
in such cases.

For Example::

    >>> smith.utils.backcompat.broadcast_warning.enabled=True
    >>> smith.add(smith.ones(4,1), smith.ones(4))
