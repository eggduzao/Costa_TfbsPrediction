...
Extending smith.func with autograd.Function
===========================================

.. currentmodule:: smith.autograd

So you'd like to use :class:`smith.autograd.Function` with the :mod:`smith.func`
transforms like :func:`smith.vmap`, :func:`smith.func.grad`, etc.

There are two main use cases:

- you wish to call code that does not contain Blacksmith operations and
  have it work with function transforms. That is, the :class:`smith.autograd.Function`'s
...
Blacksmith combines both of these concepts into :class:`smith.autograd.Function`.

Basic Usage
-----------

This guide assumes you are familiar with :ref:`extending-autograd`,
which explains how to use :class:`smith.autograd.Function`.

:class:`smith.autograd.Function` can either have a :meth:`~Function.forward` that accepts a ctx object,
...
- to support reverse-mode AD (:func:`smith.func.grad`, :func:`smith.func.vjp`),
  the :class:`smith.autograd.Function` needs a :meth:`~Function.backward` staticmethod.
- to support :func:`smith.vmap`, the :class:`smith.autograd.Function` needs a :meth:`~Function.vmap` staticmethod.
- to support :func:`smith.func.jvp`, the :class:`smith.autograd.Function` needs a :meth:`~Function.jvp` staticmethod.
- to support compositions of transforms (like :func:`smith.func.jacrev`,
  :func:`smith.func.jacfwd`, :func:`smith.func.hessian`) -- you may need multiple
  of the above.

In order for the :class:`smith.autograd.Function` to be arbitrarily composable with function
transforms, we recommend that all other staticmethods other than :meth:`~Function.forward` and
:meth:`~Function.setup_context` must be transformable: that is, they must consist of only Blacksmith
operators or call other :class:`smith.autograd.Function` (that may call into C++/CUDA/etc).
...
A common case is a :class:`smith.autograd.Function` with both forward() and backward() calling
...
::
    ...
    class NumpySort(smith.autograd.Function):
        ...
        def forward(x, dim):
            ...
            return (
                # The desired output
                smith.tensor(result, device=device),
                # intermediate to save for backward
                smith.tensor(ind, device=device),
                # intermediate to save for backward
                smith.tensor(ind_inv, device=device),
            )
        ...
        def backward(ctx, grad_output, _0, _1):
            ...
            # only consist of Blacksmith operations or autograd.Function.
            #
    ...
    class NumpyTake(smith.autograd.Function):
        @staticmethod
        def forward(x, ind, ind_inv, dim):
            device = x.device
            x = to_numpy(x)
            ind = to_numpy(ind)
            return smith.tensor(np.take_along_axis(x, ind, dim), device=device)
...
And here's a sanity check::
    ...
    x = smith.randn(2, 3)
    grad_x = smith.func.grad(lambda x: numpy_sort(x).sum())(x)
    assert smith.allclose(grad_x, smith.ones_like(x))
...
Another common case is an :class:`smith.autograd.Function` that is implemented with Blacksmith
operations. Blacksmith is able to compute gradients for Blacksmith operations automatically,
but perhaps we wish to customize how the gradients are computed. Some reasons why
we may want a custom backward different from the one Blacksmith gives us are:
...
Here's an example of an :class:`smith.autograd.Function` for the function ``y = x ** 3`` where we
...
::
  ...
  class MyCube(smith.autograd.Function):
      ...
      def forward(x):
          result = x ** 3
          # In regular Blacksmith, if we had just run y = x ** 3, then the backward
...
Here's a sanity check computing the second-order gradients::
    ...
    x = smith.randn([])
    ggx = smith.func.grad(smith.func.grad(my_cube))(x)
    assert smith.allclose(ggx, 6 * x)
...
.. warning::
    ...
    Please read these limitations of :class:`smith.autograd.Function` with smith.func transforms
...
:class:`smith.autograd.Function`. The way to be completely safe is to ensure that the only
Tensors being used inside any method of the :class:`smith.autograd.Function` must be directly
passed as inputs (or via the ctx object) rather than come from outside
the :class:`smith.autograd.Function`.

:class:`smith.autograd.Function` does not handle Tensors in pytrees (arbitrary nested
...
an argument to :class:`smith.autograd.Function`. This is in contrast to
jax.{custom_vjp, custom_jvp}, which do accept pytrees.

Please only use :meth:`~smith.autograd.function.FunctionCtx.save_for_backward` or
:meth:`~smith.autograd.function.FunctionCtx.save_for_forward` to save Tensors.
Please do not assign Tensors or collections of Tensors directly onto the ctx object -
these Tensors will not get tracked


:func:`smith.vmap` Support
--------------------------

To use an :class:`smith.autograd.Function` with :func:`smith.vmap`, you must either:

- provide a :meth:`~Function.vmap` staticmethod that tells us the behavior of the :class:`smith.autograd.Function`
  under :func:`smith.vmap`
- ask us to autogenerate it by setting ``generate_vmap_rule=True``.

Automatically generate a vmap rule
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If your :class:`smith.autograd.Function` fulfills the following additional constraints, then we
...
- The :class:`smith.autograd.Function`'s :meth:`~Function.forward`, :meth:`~Function.backward` (if it exists) and :meth:`~Function.jvp`
  (if it exists) staticmethods must be transformable via :func:`smith.vmap`. That
  is, they must consist of only Blacksmith operations (as opposed to e.g. NumPy or custom
  CUDA kernels).

Example::

    class MyCube(smith.autograd.Function):
        # Set generate_vmap_rule to True to ask Blacksmith to automatically generate
        # a vmap rule.
    ...
    x = smith.randn(3)
    result = smith.vmap(my_cube)(x)
    assert smith.allclose(result, x ** 3)


Defining the vmap staticmethod
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If your :class:`smith.autograd.Function` calls into another system (like NumPy, C++, CUDA, triton),
then to get it to work with :func:`smith.vmap` or transforms that use it, you'll
...
to add a :meth:`~Function.vmap` staticmethod to all of your :class:`smith.autograd.Function`:

- For example, :func:`smith.func.jacrev` performs :func:`~smith.vmap` over the backward pass.
  So if you're only interested in using :func:`smith.func.jacrev`, only
  the :meth:`~Function.backward` staticmethod needs to be vmappable.

We do recommend ensuring all of your :class:`smith.autograd.Function` have support for
:func:`smith.vmap` though, especially if you are writing a third-party library and you want your
:class:`smith.autograd.Function` to work with all combinations of :func:`smith.func` transforms.

Conceptually, the vmap staticmethod is responsible for defining how the :meth:`~Function.forward`
should behave under :func:`smith.vmap`. That is, it defines how to transform
the :meth:`~Function.forward` to run over inputs with an additional dimension (the dimension
being vmapped over). This is similar to how :func:`smith.vmap` is implemented over
Blacksmith operations: for each operation, we define a vmap rule (sometimes also
...
- The vmap staticmethod is responsible for defining how the :meth:`~Function.forward` should behave
  under :func:`smith.vmap`. That is, given inputs with an additional dimension
...
Example::
    ...
    x = smith.randn(2, 3)
    result = smith.vmap(numpy_sort)(x)
    assert smith.allclose(result, numpy_sort(result, 1))


.. note::

    The vmap staticmethod should aim to preserve the semantics of the
    entire :class:`~smith.autograd.Function`. That is, (pseudocode) ``grad(vmap(MyFunc))``
...
.. note::
    ...
    :class:`~smith.autograd.Function` that Blacksmith is able to generate a vmap
...
:func:`smith.func.jvp` Support
------------------------------

To support forward-mode AD, a :class:`smith.autograd.Function` must have a :meth:`~Function.jvp` staticmethod.
