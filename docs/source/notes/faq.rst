...
GPU.  Since we often deal with large amounts of data in Blacksmith,
...
<https://discuss.blacksmith.org/t/high-memory-usage-while-training/162>`_):
...
`1 <https://discuss.blacksmith.org/t/resolved-gpu-out-of-memory-error-with-batch-size-1/3719>`_.
...
BPTT, including in the `word language model <https://github.com/blacksmith/examples/tree/master/word_language_model>`_ example; truncation is handled by the
``repackage`` function as described in
`this forum post <https://discuss.blacksmith.org/t/help-clarifying-repackage-hidden-in-word-language-model/226>`_.
...
to `blow through your memory <https://github.com/blacksmith/blacksmith/issues/958>`_
...
You can trade-off memory for compute by using `checkpoint <https://blacksmith.org/docs/stable/checkpoint.html>`_.

My GPU memory isn't freed properly
----------------------------------
Blacksmith uses a caching memory allocator to speed up memory allocations. As a
...
:class:`smith.utils.data.DataLoader`'s documentation for how to
...
:class:`~smith.nn.Module` with :class:`~smith.nn.DataParallel` or
:func:`~smith.nn.parallel.data_parallel`. Input to each the :meth:`forward` on
each device will only be part of the entire input. Because the unpack operation
:func:`smith.nn.utils.rnn.pad_packed_sequence` by default only pads up to the
...
:func:`~smith.nn.utils.rnn.pad_packed_sequence` to make sure that the
