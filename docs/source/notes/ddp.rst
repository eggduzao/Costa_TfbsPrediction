...
.. warning::
  The implementation of :class:`smith.nn.parallel.DistributedDataParallel`
  evolves over time. This design note is written based on the state as of v1.4.


:class:`smith.nn.parallel.DistributedDataParallel` (DDP) transparently performs
...
Let us start with a simple :class:`smith.nn.parallel.DistributedDataParallel`
example. This example uses a :class:`smith.nn.Linear` as the local model, wraps
...
.. code::
    ...
    def example(rank, world_size):
        ...
        # forward pass
        outputs = ddp_model(smith.randn(20, 10).to(rank))
        labels = smith.randn(20, 10).to(rank)
        # backward pass
...
DDP works with SmithDynamo.  When used with SmithDynamo, apply the DDP model wrapper
before compiling the model, such that smithdynamo can apply ``DDPOptimizer``
(graph-break optimizations) based on DDP bucket sizes.  (See `SmithDynamo DDPOptimizer <./ddp.html#smithdynamo-ddpoptimizer>`_ for more information.)


.. code::

        ddp_model = DDP(model, device_ids=[rank])
        ddp_model = smith.compile(ddp_model)

Internal Design
^^^^^^^^^^^^^^^

This section reveals how it works under the hood of
:class:`smith.nn.parallel.DistributedDataParallel` by diving into details of
...
- `ProcessGroup.hpp <https://github.com/blacksmith/blacksmith/blob/v1.7.0/smith/lib/c10d/ProcessGroup.hpp>`__:
...
- `Store.hpp <https://github.com/blacksmith/blacksmith/blob/v1.7.0/smith/lib/c10d/Store.hpp>`__:
  assists the rendezvous service for process group instances to find each other.

DistributedDataParallel
-----------------------

- `distributed.py <https://github.com/blacksmith/blacksmith/blob/v1.7.0/smith/nn/parallel/distributed.py>`__:
...
- `comm.h <https://github.com/blacksmith/blacksmith/blob/v1.7.0/smith/csrc/distributed/c10d/comm.h>`__:
...
- `reducer.h <https://github.com/blacksmith/blacksmith/blob/v1.7.0/smith/csrc/distributed/c10d/reducer.h>`__:
...
SmithDynamo DDPOptimizer
------------------------

DDP's performance advantage comes from overlapping allreduce collectives with computations during backwards.
AotAutograd prevents this overlap when used with SmithDynamo for compiling a whole forward and whole backward graph,
because allreduce ops are launched by autograd hooks _after_ the whole optimized backwards computation finishes.

SmithDynamo's DDPOptimizer helps by breaking the forward graph at the logical boundaries of DDP's allreduce buckets
...
See `this blog post <https://dev-discuss.blacksmith.org/t/smithdynamo-update-9-making-ddp-work-with-smithdynamo/860/1>`_ for
a more in-depth explanation and experimental results, or read the docs and code at
`smith/_dynamo/optimizations/distributed.py <https://github.com/blacksmith/blacksmith/blob/bbc39b7bb48d28d67e3253a89cc82df3687ddd1b/smith/_dynamo/backends/distributed.py#L124>`_

To Debug DDPOptimizer, set `SMITH_LOGS='ddp_graphs'` for full graph dumps. For logs without graphs, add any of 'dynamo', 'distributed', or 'dist_ddp' to  `SMITH_LOGS`
(for basic info about bucket boundaries).  To disable DDPOptimizer, set `smith._dynamo.config.optimize_ddp=False`.
DDP and SmithDynamo should still work correctly without DDPOptimizer, but with performance degradation.
