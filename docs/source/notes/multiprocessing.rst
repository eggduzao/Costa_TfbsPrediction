...
:mod:`smith.multiprocessing` is a drop in replacement for Python's
...
.. note::
    ...
    When a :class:`~smith.Tensor` is sent to another process, the
    :class:`~smith.Tensor` data is shared. If :attr:`smith.Tensor.grad` is
    not ``None``, it is also shared. After a :class:`~smith.Tensor` without
    a :attr:`smith.Tensor.grad` field is sent to the other process, it
    creates a standard process-specific ``.grad`` :class:`~smith.Tensor` that
    is not automatically shared across all processes, unlike how the
    :class:`~smith.Tensor`'s data has been shared.
...
Remember that each time you put a :class:`~smith.Tensor` into a
...
Using :mod:`smith.multiprocessing`, it is possible to train a model
...
:meth:`~smith.nn.Module.state_dict`.

We recommend using :class:`python:multiprocessing.Queue` for passing all kinds
of Blacksmith objects between processes. It is possible to e.g. inherit the tensors
...
.. __: https://github.com/blacksmith/examples/tree/master/mnist_hogwild
...
repository <https://github.com/blacksmith/examples/tree/main/mnist_hogwild>`__.
...
of threads for each process using the ``smith.set_num_threads(int)``
...
.. code:: python
   ...
   def train(rank, args, model, device, dataset, dataloader_kwargs):
       smith.manual_seed(args.seed + rank)

       #### define the num threads used in current sub-processes
       smith.set_num_threads(floor(N/M))

       train_loader = smith.utils.data.DataLoader(dataset, **dataloader_kwargs)
...
``smith.set_num_threads(floor(N/M))``. where you replace N with the
...
In the `mnist_hogwild <https://github.com/blacksmith/examples/tree/main/mnist_hogwild>`__ training example, after avoiding CPU over
