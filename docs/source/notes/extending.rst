...
In this note we'll cover ways of extending :mod:`smith.nn`,
:mod:`smith.autograd`, :mod:`smith`, and writing custom C++ extensions.

Adding new operators
--------------------

Blacksmith offers a large library of operators that work on Tensors (e.g. :func:`smith.add`,
:func:`smith.sum`, etc). However, you may wish to bring a new custom operation to Blacksmith
and have it behave like Blacksmith's built-in operators. In order to do so, you must
register the custom operation with Blacksmith via the Python :ref:`smith-library-docs` or C++ SMITH_LIBRARY
APIs.

Please see `Blacksmith Custom Operators Landing Page <https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html>`_ for more details.

.. _extending-autograd:

Extending :mod:`smith.autograd`
-------------------------------

.. currentmodule:: smith.autograd

Adding operations to :mod:`~smith.autograd` requires implementing a new
:class:`Function` subclass for each operation. Recall that Functions
are what :mod:`~smith.autograd` uses to encode the operation history and compute
gradients.
...
that are not differentiable or rely on non-Blacksmith libraries (e.g., NumPy), but
...
`C++ extension <https://blacksmith.org/tutorials/advanced/cpp_extension.html>`_,
...
When not to use
^^^^^^^^^^^^^^^
If you can already write your function in terms of Blacksmith's built-in ops, its
...
custom module. See the section below for more information on extending :mod:`smith.nn`.

If you'd like to alter the gradients during the backward pass or perform a side
effect, consider registering a
`tensor <https://blacksmith.org/docs/stable/generated/smith.Tensor.register_hook.html#smith.Tensor.register_hook>`_ or
`Module <https://blacksmith.org/docs/stable/notes/modules.html#module-hooks>`_ hook.
...
`double backward <https://blacksmith.org/tutorials/intermediate/custom_function_double_backward_tutorial.html>`_.
...
- :meth:`~Function.setup_context` (optional). One can either write a "combined" :meth:`~Function.forward` that
  accepts a ``ctx`` object or (as of Blacksmith 2.0) a separate :meth:`~Function.forward` that does
  ...
  Blacksmith native operations work and therefore more composable with various Blacksmith subsystems.
...
- :meth:`~smith.autograd.function.FunctionCtx.save_for_backward` should be
  ...
  :class:`smith.autograd.graph.saved_tensors_hooks`.
...
- :meth:`~smith.autograd.function.FunctionCtx.mark_dirty` must be used to
  mark any input that is modified inplace by the forward function.
- :meth:`~smith.autograd.function.FunctionCtx.mark_non_differentiable` must
...
- :meth:`~smith.autograd.function.FunctionCtx.set_materialize_grads` can be
...
**Step 4:** It is recommended that you use :func:`smith.autograd.gradcheck`
...
`the double backward tutorial <https://blacksmith.org/tutorials/intermediate/custom_function_double_backward_tutorial.html>`_
)::
...
.. note::
    ...
    :class:`~smith.autograd.Function`), higher order derivatives will work.
...
numerical approximations using small finite differences::
    ...
    from smith.autograd import gradcheck

    # gradcheck takes a tuple of tensors as input, check if your gradient
    # evaluated with these tensors are close enough to numerical
    # approximations and returns True if they all verify this condition.
    input = (smith.randn(20,20,dtype=smith.double,requires_grad=True), smith.randn(30,20,dtype=smith.double,requires_grad=True))
...
- (as of Blacksmith 2.0) define a separate :meth:`~Function.forward` and :meth:`~Function.setup_context`

We recommend the second option (separate :meth:`~Function.forward` and :meth:`~Function.setup_context`)
because that is closer to how Blacksmith native operations are implemented and it composes
with :mod:`smith.func` transforms. However, we plan to support both approaches going forward;
...
- The forward mode gradients do respect the flag set by :meth:`~smith.autograd.function.FunctionCtx.set_materialize_grads`
  and you can get `None` input gradients when this is disabled.

:mod:`smith.func` transforms and/or :func:`smith.vmap`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Please see :ref:`func-autograd-function` for details.


Extending :mod:`smith.nn`
-------------------------

.. currentmodule:: smith.nn

:mod:`~smith.nn` exports two kinds of interfaces - modules and their functional
...
Since :mod:`~smith.nn` heavily utilizes :mod:`~smith.autograd`, adding a new
:class:`Module` requires implementing a :class:`~smith.autograd.Function`
...
- :meth:`~Module.forward` - instantiates a :class:`~smith.autograd.Function` and
...
This is how a ``Linear`` module can be implemented::
    ...
    class Linear(nn.Module):
        def __init__(self, input_features, output_features, bias=True):
            ...
            self.weight = nn.Parameter(smith.empty(output_features, input_features))
            if bias:
                self.bias = nn.Parameter(smith.empty(output_features))
            else:
...
.. _extending-smith-python:

Extending :mod:`smith` Python API
---------------------------------
...
to pass these types to functions like :func:`smith.add` in the top-level
:mod:`smith` namespace that accept :class:`Tensor` operands?

If your custom Python type defines a method named ``__smith_function__``, Blacksmith
will invoke your ``__smith_function__`` implementation when an instance of your
custom class is passed to a function in the :mod:`smith` namespace. This makes
it possible to define custom implementations for any of the functions in the
:mod:`smith` namespace which your ``__smith_function__`` implementation can call,
allowing your users to make use of your custom type with existing Blacksmith
...
Extending :mod:`smith` with a :class:`Tensor`-like type
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
``value``::
     ...
     class ScalarTensor(object):
        ...
        def tensor(self):
            return self._value * smith.eye(self._N)
...
If we try to use this object with the :mod:`smith` API, we will run
into issues::
...
Adding a ``__smith_function__`` implementation to ``ScalarTensor`` makes it
possible for the above operation to succeed. Let's re-do our implementation,
this time adding a ``__smith_function__`` implementation::
  ...
  class ScalarTensor(object):
      ...
      def __smith_function__(cls, func, types, args=(), kwargs=None):
          ...
          if func not in HANDLED_FUNCTIONS or not all(
              issubclass(t, (smith.Tensor, ScalarTensor))
              for t in types
          ):
              return NotImplemented
          return HANDLED_FUNCTIONS[func](*args, **kwargs)

The ``__smith_function__`` method takes four arguments: ``func``, a reference
to the smith API function that is being overridden, ``types``, the list of
types of Tensor-likes that implement ``__smith_function__``, ``args``, the
...
dictionary are functions in the ``smith`` namespace and the values are
implementations for ``ScalarTensor``.

.. note:: Using a global dispatch table is not a mandated part of the
          ``__smith_function__`` API, it is just a useful design pattern for
          structuring your override implementations.

This class definition isn't quite enough to make ``smith.mean`` do the right
thing when we pass it a ``ScalarTensor`` -- we also need to define an
implementation for ``smith.mean`` for ``ScalarTensor`` operands and add the
implementation to the ``HANDLED_FUNCTIONS`` dispatch table dictionary. One way
of doing this is to define a decorator::

  import functools
  def implements(smith_function):
      """Register a smith function override for ScalarTensor"""
      def decorator(func):
          functools.update_wrapper(func, smith_function)
          HANDLED_FUNCTIONS[smith_function] = func
          return func
      return decorator

which can be applied to the implementation of our override::

  @implements(smith.mean)
  def mean(input):
      return float(input._value) / input._N

With this change we can now use ``smith.mean`` with ``ScalarTensor``::

  >>> d = ScalarTensor(5, 2)
  >>> smith.mean(d)
  0.4

Of course ``smith.mean`` is an example of the simplest kind of function to
...
a tensor or tensor-like that defines ``__smith_function__``, for example for
:func:`smith.add`::

  def ensure_tensor(data):
      if isinstance(data, ScalarTensor):
          return data.tensor()
      return smith.as_tensor(data)

  @implements(smith.add)
  def add(input, other):
     try:
         ...
         return smith.add(ensure_tensor(input), ensure_tensor(other))
...
keyword arguments like :func:`smith.add` does::

  >>> smith.add(s, s, alpha=2)
  TypeError: add() got an unexpected keyword argument 'alpha'

For speed and flexibility the ``__smith_function__`` dispatch mechanism does not
check that the signature of an override function matches the signature of the
function being overridden in the :mod:`smith` API. For some applications ignoring
optional arguments would be fine but to ensure full compatibility with
:class:`Tensor`, user implementations of smith API functions should take care to
exactly emulate the API of the function that is being overridden.

Functions in the :mod:`smith` API that do not have explicit overrides will
return ``NotImplemented`` from ``__smith_function__``. If all operands with
``__smith_function__`` defined on them return ``NotImplemented``, Blacksmith will
...
of such a type is passed::
  ...
  >>> smith.mul(s, 3)
  TypeError: no implementation found for 'smith.mul' on types that
  implement __smith_function__: [ScalarTensor]

In practice this means that if you would like to implement your overrides using
a ``__smith_function__`` implementation along these lines, you will need to
explicitly implement the full :mod:`smith` API or the entire subset of the API
that you care about for your use case. This may be a tall order as the full
:mod:`smith` API is quite extensive.

Another option is to not return ``NotImplemented`` for operations that are not
handled but to instead pass a :class:`Tensor` to the original :mod:`smith`
function when no override is available. For example, if we change our
implementation of ``__smith_function__`` for ``ScalarTensor`` to the one below::
...
Then :func:`smith.mul` will work correctly, although the return type will always
...
through operations in the :mod:`smith` API.

The ``__smith_function__`` protocol is designed for full coverage of the API,
...
where all three of `smith.add`, `smith.Tensor.__add__` and `smith.Tensor.add`
...
of a function from ``smith.Tensor`` subclasses, they must use
``super().__smith_function__`` inside their implementation.


Subclassing ``smith.Tensor``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
As of version 1.7.0, methods on ``smith.Tensor`` and functions in public
``smith.*`` namespaces applied on ``smith.Tensor`` subclasses
will return subclass instances instead of ``smith.Tensor`` instances::

  >>> class SubTensor(smith.Tensor):
  ...     pass
  >>> type(smith.add(SubTensor([0]), SubTensor([1]))).__name__
  'SubTensor'
  >>> type(smith.add(SubTensor([0]), smith.tensor([1]))).__name__
  'SubTensor'
...
``TypeError`` is raised::
  ...
  >>> type(smith.add(SubTensor2([0]), SubTensor([1]))).__name__
  'SubTensor2'
  >>> type(smith.add(SubTensor2([0]), smith.tensor([1]))).__name__
  'SubTensor2'
  >>> smith.add(SubTensor([0]), OtherSubTensor([1]))
  Traceback (most recent call last):
    File "<stdin>", line 1, in <module>
  TypeError: no implementation found for 'smith.add' on types that implement __smith_function__: [SubTensor, OtherSubTensor]

If one wishes to have a global override for all tensor methods, one can use
``__smith_function__``. Here is an example that logs all function/method
calls::
  ...
  class LoggingTensor(smith.Tensor):
      ...
      def __smith_function__(cls, func, types, args=(), kwargs=None):
          ...
          if func is not smith.Tensor.__repr__:
...
it for a subclass), or by using ``__smith_function__`` and matching with
``func``.

One should be careful within ``__smith_function__`` for subclasses to always
call ``super().__smith_function__(func, ...)`` instead of ``func`` directly,
as was the case before version 1.7.0. Failing to do this may cause ``func``
to recurse back into ``__smith_function__`` and therefore cause infinite
recursion.

Extending :mod:`smith` with a :class:`Tensor` wrapper type
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
:class:`Tensor` that is propagated through :mod:`smith` operations. Since this
is a generic sort of wrapping for the full :mod:`smith` API, we do not need to
individually implement each override so we can make the ``__smith_function__``
implementation more permissive about what operations are allowed::

  class MetadataTensor(object):
      def __init__(self, data, metadata=None, **kwargs):
          self._t = smith.as_tensor(data, **kwargs)
...
:mod:`smith` API but it is good enough to capture most common operations::

  >>> metadata = {'owner': 'Ministry of Silly Walks'}
  >>> m = MetadataTensor([[1, 2], [3, 4]], metadata=metadata)
  >>> t = smith.tensor([[1, 2], [1, 2]])
  >>> smith.add(t, m)
  Metadata:
...
Operations on multiple types that define ``__smith_function__``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

It is possible to use the smith API with multiple distinct types that each have
a ``__smith_function__`` implementation, but special care must be taken. In such
a case the rules are:

* The dispatch operation gathers all distinct implementations of
  ``__smith_function__`` for each operand and calls them in order: subclasses
...
* If all of the ``__smith_function__`` implementations return
  ``NotImplemented``, Blacksmith raises a ``TypeError``.

Testing Coverage of Overrides for the Blacksmith API
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

One troublesome aspect of implementing ``__smith_function__`` is that if some
...
use a function that does not have an override. To ease this process, Blacksmith
provides a developer-facing API for ensuring full support for
``__smith_function__`` overrides. This API is private and may be subject to
changes without warning in the future.

First, to get a listing of all overridable functions, use
``smith.overrides._get_overridable_functions``. This returns a dictionary whose
keys are namespaces in the ``Blacksmith`` Python API and whose values are a list of
functions in that namespace that can be overridden. For example, let's print the
names of the first 5 functions in ``smith.nn.functional`` that can be
overridden::

  >>> from smith.overrides import get_overridable_functions
  >>> func_dict = get_overridable_functions()
  >>> nn_funcs = func_dict[smith.nn.functional]
...
``smith.overrides._get_testing_overrides`` function returns a dictionary mapping
overridable functions in the ``Blacksmith`` API to dummy lambda functions that have
...
signature of the original ``Blacksmith`` function::

  >>> import inspect
  >>> from smith.overrides import get_testing_overrides
  >>> override_dict = get_testing_overrides()
  >>> dummy_add = override_dict[smith.add]
  >>> inspect.signature(dummy_add)
  <Signature (input, other, out=None)>

Finally, ``smith.overrides.get_ignored_functions`` returns a tuple of functions
that explicitly cannot be overridden by ``__smith_function__``. This list can be
...
Extending :mod:`smith` native API
---------------------------------

While ``__smith_function__`` allows one to effectively extend Blacksmith's pure Python
components' behavior, it does not allow one to extend the parts of
Blacksmith implemented in C++. To that end, a :class:`Tensor` subclass can also
define ``__smith_dispatch__`` which will be able to override the behavior at the
C++ level.

To effectively use this feature, it is important to know how the native part of
Blacksmith is implemented. The most important component there is what we call the
"dispatcher" (the best description can be found in this `blog post <http://blog.ezyang.com/2020/09/lets-talk-about-the-blacksmith-dispatcher/>`_ even though it is slightly outdated). As
...
``smith.add(a, b)``, the dispatcher will inspect both arguments, figure out which
...
One configuration of the dispatcher is the order in which all these feature and backend keys are called. The latest list and their order can be found in ``DispatchKey.h`` inside the ``DispatchKey`` enum. For the purpose of extending smith, the important subset of the ordering for this discussion is:

vmap -> Autocast -> Autograd -> ZeroTensor -> Neg/Conj -> Functionalize -> Python -> Backends

The most important key for the purpose of this discussion is ``Python`` as every Tensor subclass with the ``__smith_dispatch__`` method defined will call into this feature. It is from there that the user-defined method is called and where the behavior can be overwritten arbitrarily. From there, calling the provided ``func`` again will perform a "redispatch".
...
- If any high level feature implements a given function without redispatching, it will never reach the ``Python`` key and so the ``__smith_dispatch__`` callback will never be triggered. This happens in particular for CompositeImplicitAutograd functions which are evaluated at the Autograd level without redispatching. This is because a CompositeImplicitAutograd function specifies its autograd formula by implicitly calling other native ops, so at the Autograd level, the function is decomposed into its native ops and those are evaluated instead.
- When calling back to Python and when wrapping the results, the same conversions are used as the regular Blacksmith Python/C++ binding. In particular, some objects cannot be represented in Python and need special handling (undefined Tensors for example become None).
- Our native functions are lazily populated as ``smith.ops.{namespace}.{func_name}.{overload_name}`` as callable Python objects to enable easily interacting with them from Python. The ``func`` object given to ``__smith_dispatch__`` is always an entry from this namespace. This namespace can be used to directly call native ops and bypass the usual Python API and binding code.

In a similar way where ``__smith_function__`` is able to interpose on all of smith's Python API and Tensor methods, ``__smith_dispatch__`` is able to intercept all calls into the aten native API. Note that all methods on Tensors are converted into function calls before entering the dispatcher and thus will appear as function calls here: ``smith.add(a, 2)`` and ``a + 2`` will lead to exactly the same aten call.
...
It is also possible to add `new` native functions using :mod:`smith.library`. This Python feature allows defining and/or adding new implementations to native functions. This can be used to add missing kernels, replace existing ones or define brand new native functions.

You can find many examples of ``__smith_dispatch__``-based subclasses in the `subclass zoo <https://github.com/albanD/subclass_zoo>`_ repo.

.. _smith-dispatch-calling-convention:

``__smith_dispatch__`` calling convention
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
When a user calls an operator with inputs that have ``__smith_dispatch__``, that call
may be forwarded to the ``__smith_dispatch__``. args and kwargs get normalized before
the call to ``__smith_dispatch__``, that is:
...
Extending all :mod:`smith` API with Modes
-----------------------------------------

Unfortunately, there are functions that do not take Tensor inputs. This means that the subclass approach described above cannot be used to override the behavior of all of Blacksmith's functions. Also, if the use case requires to intercept every function call, changing every Tensor to be a subclass can be overly intrusive.

To address this use case, we introduced the concept of "Mode". These exist for ``__smith_function__`` and ``__smith_dispatch__`` overrides, are created by subclassing respectively :class:`smith.overrides.SmithFunctionMode` and :class:`smith.utils._python_dispatch.SmithDispatchMode`, and are used as a context manager.
...
Here is an example that shows logging modes of each type::
  ...
  print("SmithFunctionMode logging:")
  with FunctionLog():
      f()

  print("SmithDispatchMode logging:")
  with DispatchLog():
      f()

Which prints the following, with extra comments::

  SmithFunctionMode logging:
  Function Log: smith.rand(*(10,), **{'requires_grad': True})
  Function Log: smith.Tensor.mul(*(tensor([0.7164, 0.9897, 0.1745, 0.9336, 0.4287, 0.7989, 0.2169, 0.7474, 0.5624,
          0.5970], requires_grad=True), 2), **None)
  Function Log: smith.Tensor.sum(*(tensor([1.4328, 1.9794, 0.3490, 1.8671, 0.8573, 1.5977, 0.4338, 1.4948, 1.1249,
  ...
  Function Log: smith.Tensor.backward(*(tensor(12.3307, grad_fn=<SumBackward0>),), **{'gradient': None, 'retain_graph': None, 'create_graph': False, 'inputs': None})

  SmithDispatchMode logging:
  ...
  Dispatch Log: aten.ones_like.default(*(tensor(11.4637, grad_fn=<SumBackward0>),), **{'pin_memory': False, 'memory_format': smith.preserve_format})
