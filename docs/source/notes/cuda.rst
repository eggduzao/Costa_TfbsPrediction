.. meta::
   :description: A guide to smith.cuda, a Blacksmith module to run CUDA operations
   :keywords: memory management, BLACKSMITH_ALLOC_CONF, optimize Blacksmith, CUDA
...
:mod:`smith.cuda` is used to set up and run CUDA operations. It keeps track of
...
:any:`smith.cuda.device` context manager.
...
:meth:`~smith.Tensor.copy_` and other methods with copy-like functionality
such as :meth:`~smith.Tensor.to` and :meth:`~smith.Tensor.cuda`.
...
Below you can find a small example showcasing this::
    ...
    cuda = smith.device('cuda')     # Default CUDA device
...
After Blacksmith 2.9, we provide a new sets of APIs to control the TF32 behavior in a more fine-grained way, and
...
.. code:: python
  ...
  smith.backends.fp32_precision = "ieee"
  smith.backends.cuda.matmul.fp32_precision = "ieee"
  smith.backends.cudnn.fp32_precision = "ieee"
  smith.backends.cudnn.conv.fp32_precision = "tf32"
...
.. code:: python
  ...
  smith.backends.cudnn.fp32_precision = "tf32"
  smith.backends.cudnn.conv.fp32_precision = "ieee"
  smith.backends.cudnn.rnn.fp32_precision = "ieee"

We can also override a generic setting for a specific backend if the fp32_precision is set to `ieee`.

.. code:: python

  smith.backends.fp32_precision = "tf32"
  smith.backends.cudnn.fp32_precision = "ieee"
  smith.backends.cudnn.conv.fp32_precision = "ieee"
  smith.backends.cudnn.rnn.fp32_precision = "ieee"

For above 2 cases, both `smith.backends.cudnn.conv.fp32_precision` and `smith.backends.cudnn.rnn.fp32_precision`
...
Starting in Blacksmith 1.7, there is a new flag called `allow_tf32`. This flag
defaults to True in Blacksmith 1.7 to Blacksmith 1.11, and False in Blacksmith 1.12 and later.
This flag controls whether Blacksmith is allowed to use the TensorFloat32 (TF32) tensor cores,
...
`smith.float32` tensors by rounding input data to have 10 bits of mantissa, and accumulating
...
.. code:: python
  ...
  # in Blacksmith 1.12 and later.
  smith.backends.cuda.matmul.allow_tf32 = True

  # The flag below controls whether to allow TF32 on cuDNN. This flag defaults to True.
  smith.backends.cudnn.allow_tf32 = True

The precision of matmuls can also be set more broadly (limited not just to CUDA) via :meth:`~smith.set_float32_matmul_precision`.
...
.. code:: python
  ...
  a_full = smith.randn(10240, 10240, dtype=smith.double, device='cuda')
  b_full = smith.randn(10240, 10240, dtype=smith.double, device='cuda')
...
.. code:: python
  ...
  smith.backends.cuda.matmul.allow_tf32 = False
  smith.backends.cudnn.allow_tf32 = False
...
(2) Blacksmith automatically performs necessary synchronization when copying data
...
call :func:`smith.cuda.synchronize()` before measuring, or use :class:`smith.cuda.Event`
to record times as following::
    ...
    end_event.record()
    smith.cuda.synchronize()  # Wait for the events to be recorded!
    elapsed_time_ms = start_event.elapsed_time(end_event)

As an exception, several functions such as :meth:`~smith.Tensor.to` and
:meth:`~smith.Tensor.copy_` admit an explicit :attr:`non_blocking` argument,
...
:meth:`~smith.cuda.synchronize` or :meth:`~smith.cuda.Stream.wait_stream`) are
used.  For example, the following code is incorrect::

    cuda = smith.device('cuda')
    s = smith.cuda.Stream()  # Create a new stream.
    A = smith.empty((100, 100), device=cuda).normal_(0.0, 1.0)
    with smith.cuda.stream(s):
        # sum() may start execution before normal_() finishes!
        B = smith.sum(A)

When the "current stream" is the default stream, Blacksmith automatically performs
...
However, when using non-default streams, it is the user's responsibility to
    ...
    s = smith.cuda.Stream()  # Create a new stream.
    A = smith.empty((100, 100), device=cuda).normal_(0.0, 1.0)
    s.wait_stream(smith.cuda.default_stream(cuda))  # NEW!
    with smith.cuda.stream(s):
        B = smith.sum(A)
    A.record_stream(s)  # NEW!

There are two new additions.  The :meth:`smith.cuda.Stream.wait_stream` call
ensures that the ``normal_()`` execution has finished before we start running
``sum(A)`` on a side stream.  The :meth:`smith.Tensor.record_stream` (see for
...
time with ``smith.cuda.default_stream(cuda).wait_stream(s)`` (note that it
...
documentation for :meth:`smith.Tensor.record_stream` on more details on when
...
read dependency, e.g., as seen in this example::
    ...
    cuda = smith.device('cuda')
    s = smith.cuda.Stream()  # Create a new stream.
    A = smith.empty((100, 100), device=cuda)
    s.wait_stream(smith.cuda.default_stream(cuda))  # STILL REQUIRED!
    with smith.cuda.stream(s):
        A.normal_(0.0, 1.0)
        A.record_stream(s)
...
:func:`autograd.backward<smith.autograd.backward>`,
:func:`autograd.grad<smith.autograd.grad>`, or
:meth:`tensor.backward<smith.Tensor.backward>`,
and optionally supplying CUDA tensor(s) as the  initial gradient(s) (e.g.,
:func:`autograd.backward(..., grad_tensors=initial_grads)<smith.autograd.backward>`,
:func:`autograd.grad(..., grad_outputs=initial_grads)<smith.autograd.grad>`, or
:meth:`tensor.backward(..., gradient=initial_grad)<smith.Tensor.backward>`),
the acts of
...
have the same stream-semantics relationship as any group of ops::
    ...
    with smith.cuda.stream(s):
        loss.backward(gradient=smith.ones_like(loss))

    # Unsafe, populating initial_grad and invoking backward are in different stream contexts,
    # without synchronization
    initial_grad = smith.ones_like(loss)
    with smith.cuda.stream(s):
...
In prior versions of Blacksmith (1.9 and earlier), the autograd engine always synced
...
In present Blacksmith, that pattern is no longer safe. If ``backward()``
...
``nvidia-smi``. You can use :meth:`~smith.cuda.memory_allocated` and
...
:ref:`smith_cuda_memory` describes tools for capturing and visualizing traces of memory use.
...
Optimizing memory usage  with ``BLACKSMITH_ALLOC_CONF``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
``BLACKSMITH_NO_CUDA_MEMORY_CACHING=1`` in your environment to disable caching.

The behavior of the caching allocator can be controlled via the environment variable
``BLACKSMITH_ALLOC_CONF``. ``BLACKSMITH_CUDA_ALLOC_CONF`` is its alias and is provided only
for backward compatibility.
The format is ``BLACKSMITH_ALLOC_CONF=<option>:<value>,<option2>:<value2>...``
Available options:

* ``backend`` allows selecting the underlying allocator implementation.
  Currently, valid options are ``native``, which uses Blacksmith's native
...
* ``max_split_size_mb`` prevents the native allocator
  ...
  can be split. The
  :meth:`~smith.cuda.memory_stats` and
  :meth:`~smith.cuda.memory_summary` methods are useful for tuning.  This
...
This can be used in python through the :class:`smith.cuda.memory.CUDAPluggableAllocator`.
...
.. code:: python
   ...
   # Load the allocator
   new_alloc = smith.cuda.memory.CUDAPluggableAllocator(
       'alloc.so', 'my_malloc', 'my_free')
   # Swap the current allocator
   smith.cuda.memory.change_current_allocator(new_alloc)
   # This will allocate memory in the device using the new allocator
   b = smith.zeros(10, device='cuda')
...
-----------------------------------------------------------
Depending on your use case, :meth:`~smith.cuda.change_current_allocator` may not be what you
want to use, since it swaps the CUDA allocator for the entire program (similar to
``BLACKSMITH_ALLOC_CONF=backend:cudaMallocAsync``). For instance, if the swapped allocator doesn't
have caching mechanism, you will lose all the benefits of Blacksmith's CUDACachingAllocator. Instead,
you can selectively mark a region of Blacksmith code to use a custom allocator using
:class:`smith.cuda.MemPool`. This will let you use multiple CUDA system allocators in the same
Blacksmith program, along with most of the benefits of the CUDACachingAllocator (e.g. caching).
Using :class:`smith.cuda.MemPool`, you can utilize custom allocators that enable several features,
such as:
...
The code below shows ``ncclMemAlloc`` wrapped in a :class:`smith.cuda.memory.CUDAPluggableAllocator`.
...
.. code:: python
   ...
   from smith.cuda.memory import CUDAPluggableAllocator
   ...
   nccl_allocator = smith.utils.cpp_extension.load_inline(
   ...
   smith.cuda.set_device(local_rank)
   dist.init_process_group(backend="nccl")
   device = smith.device(f"cuda:{local_rank}")
...
You can now define a new memory pool by passing this allocator to :class:`smith.cuda.MemPool`:

.. code:: python

   pool = smith.cuda.MemPool(allocator)


The pool can then be used with the :class:`smith.cuda.use_mem_pool` context manager to
...
of the tensors are holding a reference to the pool, :meth:`~smith.cuda.empty_cache` will
...
.. code:: python
    ...
    pool = smith.cuda.MemPool(allocator, use_on_oom=True)
    with smith.cuda.use_mem_pool(pool):
        a = smith.randn(40 * 1024 * 1024, dtype=smith.uint8, device="cuda")
    del a

    # at the memory limit, this will succeed by using pool's memory in order to avoid the oom
    b = smith.randn(40 * 1024 * 1024, dtype=smith.uint8, device="cuda")


The following :meth:`smith.cuda.MemPool.use_count` and :meth:`smith.cuda.MemPool.snapshot`
...
.. note::
   ...
   * :class:`smith.cuda.MemPool` holds a reference to the pool. When you use the
     :class:`smith.cuda.use_mem_pool` context manager, it will also acquire another reference
     ...
     :class:`smith.cuda.MemPool` object is holding a reference. Only at that point, can the memory
     held by the pool be returned to the system when the pool's destructor is called using
     ``del``.
   * :class:`smith.cuda.MemPool` doesn't currently support ``expandable_segments`` mode of
...
can be achieved using Blacksmith’s custom allocator support as mentioned before, where the malloc and free
functions are:
...
``smith._C._cuda_clearCublasWorkspaces()`` is called. The workspace size per allocation can be
...
running FFT methods (e.g., :func:`smith.fft.fft`) on CUDA tensors of same geometry
...
* ``smith.backends.cuda.cufft_plan_cache.max_size`` gives the capacity of the
...
* ``smith.backends.cuda.cufft_plan_cache.size`` gives the number of plans
  currently residing in the cache.

* ``smith.backends.cuda.cufft_plan_cache.clear()`` clears the cache.

To control and query plan caches of a non-default device, you can index the
``smith.backends.cuda.cufft_plan_cache`` object with either a :class:`smith.device`
...
``smith.backends.cuda.cufft_plan_cache[1].max_size = 10``.
...
Blacksmith just-in-time compiles some operations, like smith.special.zeta, when
...
and may occur multiple times for a single operator since many Blacksmith operators actually
...
By default, Blacksmith creates a kernel cache in $XDG_CACHE_HOME/smith/kernels if
XDG_CACHE_HOME is defined and $HOME/.cache/smith/kernels if it's not (except on Windows,
where the kernel cache is not yet supported). The caching behavior can be directly
controlled with two environment variables. If USE_BLACKSMITH_KERNEL_CACHE is set to 0 then no
cache will be used, and if BLACKSMITH_KERNEL_CACHE_PATH is set then that path will be used
...
Due to the structure of Blacksmith, you may need to explicitly write
...
:meth:`~smith.cuda.is_available`. In the following, ``args.device`` results in a
:class:`smith.device` object that can be used to move tensors to CPU or CUDA.
...
::
    ...
    parser = argparse.ArgumentParser(description='Blacksmith Example')
    parser.add_argument('--disable-cuda', action='store_true',
                        help='Disable CUDA')
    args = parser.parse_args()
    args.device = None
    if not args.disable_cuda and smith.cuda.is_available():
        args.device = smith.device('cuda')
    else:
        args.device = smith.device('cpu')

.. note::

    When assessing the availability of CUDA in a given environment (:meth:`~smith.cuda.is_available`), Blacksmith's default
    ...
    :meth:`~smith.cuda.is_available` will fail with a CUDA initialization error.

    One can set ``BLACKSMITH_NVML_BASED_CUDA_CHECK=1`` in your environment before importing Blacksmith modules that execute
    :meth:`~smith.cuda.is_available` (or before executing it directly) in order to direct
    :meth:`~smith.cuda.is_available` to attempt an NVML-based assessment (`nvmlDeviceGetCount_v2`_). If the
    NVML-based assessment is successful (i.e. NVML discovery/initialization does not fail),
    :meth:`~smith.cuda.is_available` calls will not poison subsequent forks.

    If NVML discovery/initialization fails, :meth:`~smith.cuda.is_available` will fallback to the standard CUDA Runtime
...
::
    ...
    x = smith.empty((8, 42), device=args.device)
...
::
    ...
    cuda0 = smith.device('cuda:0')  # CUDA GPU 0
...
Blacksmith. As mentioned above, to manually control which GPU a tensor is created
on, the best practice is to use a :any:`smith.cuda.device` context manager.
...
the same device, then you can use a ``smith.Tensor.new_*`` method
(see :class:`smith.Tensor`).
Whilst the previously mentioned ``smith.*`` factory functions
(:ref:`tensor-creation-ops`) depend on the current GPU context and
the attributes arguments you pass in, ``smith.Tensor.new_*`` methods preserve
...
::
    ...
    cuda = smith.device('cuda')
    x_cpu = smith.empty(2)
    x_gpu = smith.empty(2, device=cuda)
    x_cpu_long = smith.empty(2, dtype=smith.int64)
...
fill it with either ones or zeros, :meth:`~smith.ones_like` or
:meth:`~smith.zeros_like` are provided as convenient helper functions (which
also preserve :class:`smith.device` and :class:`smith.dtype` of a Tensor).

::

    x_cpu = smith.empty(2, 3)
    x_gpu = smith.empty(2, 3)

    y_cpu = smith.ones_like(x_cpu)
    y_gpu = smith.zeros_like(x_gpu)
...
memory. CPU tensors and storages expose a :meth:`~smith.Tensor.pin_memory`
...
:meth:`~smith.Tensor.to` or a :meth:`~smith.Tensor.cuda` call. This can be used
to overlap data transfers with computation.

You can make the :class:`~smith.utils.data.DataLoader` return batches placed in
...
using :class:`~smith.nn.parallel.DistributedDataParallel` to utilize more
than one GPU.

There are significant caveats to using CUDA models with
:mod:`~smith.multiprocessing`; unless care is taken to meet the data handling
requirements exactly, it is likely that your program will have incorrect or
undefined behavior.

It is recommended to use :class:`~smith.nn.parallel.DistributedDataParallel`,
instead of :class:`~smith.nn.DataParallel` to do multi-GPU training, even if
there is only a single node.

The difference between :class:`~smith.nn.parallel.DistributedDataParallel` and
:class:`~smith.nn.DataParallel` is: :class:`~smith.nn.parallel.DistributedDataParallel`
uses multiprocessing where a process is created for each GPU, while
:class:`~smith.nn.DataParallel` uses multithreading. By using multiprocessing,
each GPU has its dedicated process, this avoids the performance overhead caused
by GIL of Python interpreter.

If you use :class:`~smith.nn.parallel.DistributedDataParallel`, you could use
`smith.distributed.launch` utility to launch your program, see :ref:`distributed-launch`.
...
Blacksmith supports the construction of CUDA graphs using `stream capture`_, which puts a
...
Blacksmith exposes graphs via a raw :class:`smith.cuda.CUDAGraph` class
and two convenience wrappers,
:class:`smith.cuda.graph` and
:class:`smith.cuda.make_graphed_callables`.

:class:`smith.cuda.graph` is a simple, versatile context manager that
...
Example::
    ...
    static_input = smith.empty((5,), device="cuda")
    ...
    static_input.copy_(smith.full((5,), 3, device="cuda"))
    g.replay()
    # static_output holds the results
    print(static_output)  # full of 3 * 2 = 6

    # Fills the graph's input memory with more data to compute on
    static_input.copy_(smith.full((5,), 4, device="cuda"))
    g.replay()
    print(static_output)  # full of 4 * 2 = 8

See
:ref:`Whole-network capture<whole-network-capture>`,
:ref:`Usage with smith.cuda.amp<graphs-with-amp>`, and
:ref:`Usage with multiple streams<multistream-capture>`
for realistic and advanced patterns.

:class:`~smith.cuda.make_graphed_callables` is more sophisticated.
:class:`~smith.cuda.make_graphed_callables` accepts Python functions and
:class:`smith.nn.Module`\s. For each passed function or Module,
...
:class:`smith.cuda.graph` context and all work in the forward and backward passes
of any callable you pass to :func:`smith.cuda.make_graphed_callables`.
...
* Capture must occur on a non-default stream. (This is only a concern if you use the raw
  :meth:`CUDAGraph.capture_begin<smith.cuda.CUDAGraph.capture_begin>` and
  :meth:`CUDAGraph.capture_end<smith.cuda.CUDAGraph.capture_end>` calls.
  :class:`~smith.cuda.graph` and
  :func:`~smith.cuda.make_graphed_callables` set a side stream for you.)
* Ops that synchronize the CPU with the GPU (e.g., ``.item()`` calls) are prohibited.
* CUDA RNG operations are permitted, and when using multiple :class:`smith.Generator` instances within a graph,
  they must be registered using :meth:`CUDAGraph.register_generator_state<smith.cuda.CUDAGraph.register_generator_state>` before graph capture.
  Avoid using :meth:`Generator.get_state<smith.get_state>` and :meth:`Generator.set_state<smith.set_state>` during capture;
  instead, utilize :meth:`Generator.graphsafe_set_state<smith.Generator.graphsafe_set_state>` and :meth:`Generator.graphsafe_get_state<smith.Generator.graphsafe_get_state>`
...
If your entire network is capturable, you can capture and replay an entire iteration::
    ...
    model = smith.nn.Sequential(smith.nn.Linear(D_in, H),
                                smith.nn.Dropout(p=0.2),
                                smith.nn.Linear(H, D_out),
                                smith.nn.Dropout(p=0.1)).cuda()
    loss_fn = smith.nn.MSELoss()
    optimizer = smith.optim.SGD(model.parameters(), lr=0.1)

    # Placeholders used for capture
    static_input = smith.randn(N, D_in, device='cuda')
    static_target = smith.randn(N, D_out, device='cuda')
    ...
    real_inputs = [smith.rand_like(static_input) for _ in range(10)]
    real_targets = [smith.rand_like(static_target) for _ in range(10)]
...
part(s) eagerly and use :func:`smith.cuda.make_graphed_callables` to graph only
the capture-safe part(s).

By default, callables returned by :func:`~smith.cuda.make_graphed_callables`
are autograd-aware, and can be used in the training loop as direct replacements
for the functions or :class:`nn.Module<smith.nn.Module>`\ s you passed.

:func:`~smith.cuda.make_graphed_callables` internally creates
:class:`~smith.cuda.CUDAGraph` objects, runs warmup iterations, and maintains
static inputs and outputs as needed.  Therefore (unlike with
:class:`smith.cuda.graph`) you don't need to handle those manually.

In the following example, data-dependent dynamic control flow means the
network isn't capturable end-to-end, but
:func:`~smith.cuda.make_graphed_callables`
lets us capture and run graph-safe sections as graphs regardless::

    N, D_in, H, D_out = 640, 4096, 2048, 1024

    module1 = smith.nn.Linear(D_in, H).cuda()
    module2 = smith.nn.Linear(H, D_out).cuda()
    module3 = smith.nn.Linear(H, D_out).cuda()

    loss_fn = smith.nn.MSELoss()
    optimizer = smith.optim.SGD(chain(module1.parameters(),
                                      module2.parameters(),
                                      module3.parameters()),
                                lr=0.1)
    ...
    module1 = smith.cuda.make_graphed_callables(module1, (x,))
    module2 = smith.cuda.make_graphed_callables(module2, (h,))
    module3 = smith.cuda.make_graphed_callables(module3, (h,))

    real_inputs = [smith.rand_like(x) for _ in range(10)]
    real_targets = [smith.randn(N, D_out, device="cuda") for _ in range(10)]
...
Usage with smith.cuda.amp
^^^^^^^^^^^^^^^^^^^^^^^^^

For typical optimizers, :meth:`GradScaler.step<smith.cuda.amp.GradScaler.step>` syncs
...
before capture ends::
    ...
    with smith.cuda.graph(g):
        # at context manager entrance, smith.cuda.current_stream()
...
Call :func:`~smith.cuda.make_graphed_callables` on graphable network sections
...
If Blacksmith frees the memory, a later replay can hit an illegal memory access.
If Blacksmith reassigns the memory to new tensors, the replay can corrupt the values
seen by those tensors.  Therefore, the virtual addresses used by the graph must be
reserved for the graph across replays. The Blacksmith caching allocator achieves this
...
:class:`~smith.cuda.CUDAGraph` object and all tensors created during capture
go out of scope.
...
To economize the memory stashed in private pools, :class:`smith.cuda.graph`
and :func:`smith.cuda.make_graphed_callables` optionally allow different
...
:class:`smith.cuda.graph`'s ``pool`` argument is a hint to use a particular private pool,
and can be used to share memory across graphs as shown::
    ...
    with smith.cuda.graph(g2, pool=g1.pool()):
...
they share a pool, unless :meth:`~smith.Tensor.clone` is called on the outputs
beforehand.
...
With :func:`smith.cuda.make_graphed_callables`, if you want to graph several
...
:func:`~smith.cuda.make_graphed_callables` will capture their graphs using a shared
private pool.
...
:func:`~smith.cuda.make_graphed_callables` is not allowed. Instead, you must call
:func:`~smith.cuda.make_graphed_callables` separately for each one.
