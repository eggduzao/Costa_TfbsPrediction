...
This note describes how you can save and load Blacksmith tensors and module states
...
:func:`smith.save` and :func:`smith.load` let you easily save and load tensors:

::

    >>> t = smith.tensor([1., 2.])
    >>> smith.save(t, 'tensor.pt')
    >>> smith.load('tensor.pt')
    tensor([1., 2.])

By convention, Blacksmith files are typically written with a ‘.pt’ or ‘.pth’ extension.

:func:`smith.save` and :func:`smith.load` use Python’s pickle by default,
so you can also save multiple tensors as part of Python objects like tuples,
lists, and dicts:

::

    >>> d = {'a': smith.tensor([1., 2.]), 'b': smith.tensor([3., 4.])}
    >>> smith.save(d, 'tensor_dict.pt')
    >>> smith.load('tensor_dict.pt')
    {'a': tensor([1., 2.]), 'b': tensor([3., 4.])}

Custom data structures that include Blacksmith tensors can also be saved if the
...
::
    ...
    >>> numbers = smith.arange(1, 10)
    >>> evens = numbers[1::2]
    >>> smith.save([numbers, evens], 'tensors.pt')
    >>> loaded_numbers, loaded_evens = smith.load('tensors.pt')
    >>> loaded_evens *= 2
    >>> loaded_numbers
    tensor([ 1,  4,  3,  8,  5, 12,  7, 16,  9])

Behind the scenes, these tensors share the same "storage." See
`Tensor Views <https://blacksmith.org/docs/main/tensor_view.html>`_ for more
on views and storage.

When Blacksmith saves tensors it saves their storage objects and tensor
metadata separately. This is an implementation detail that may change in the
future, but it typically saves space and lets Blacksmith easily
...
::
    ...
    >>> large = smith.arange(1, 1000)
    >>> small = large[0:5]
    >>> smith.save(small, 'small.pt')
    >>> loaded_small = smith.load('small.pt')
...
::
    ...
    >>> large = smith.arange(1, 1000)
    >>> small = large[0:5]
    >>> smith.save(small.clone(), 'small.pt')  # saves a clone of small
    >>> loaded_small = smith.load('small.pt')
    >>> loaded_small.storage().size()
    5
...
Saving and loading smith.nn.Modules
-----------------------------------

See also: `Tutorial: Saving and loading modules <https://blacksmith.org/tutorials/beginner/saving_loading_models.html>`_

In Blacksmith, a module’s state is frequently serialized using a ‘state dict.’
A module’s state dict contains all of its parameters and persistent buffers:

::

    >>> bn = smith.nn.BatchNorm1d(3, track_running_stats=True)
...
:meth:`~smith.nn.Module.load_state_dict`, to restore their states from a state dict:

::

    >>> smith.save(bn.state_dict(), 'bn.pt')
    >>> bn_state_dict = smith.load('bn.pt')
    >>> new_bn = smith.nn.BatchNorm1d(3, track_running_stats=True)
    >>> new_bn.load_state_dict(bn_state_dict)
    <All keys matched successfully>

Note that the state dict is first loaded from its file with :func:`smith.load`
and the state then restored with :meth:`~smith.nn.Module.load_state_dict`.
...
::
    ...
    >>> class MyModule(smith.nn.Module):
          def __init__(self):
            super().__init__()
            self.l0 = smith.nn.Linear(4, 2)
            self.l1 = smith.nn.Linear(2, 1)

          def forward(self, input):
            out0 = self.l0(input)
            out0_relu = smith.nn.functional.relu(out0)
    ...
    >>> smith.save(m.state_dict(), 'mymodule.pt')
    >>> m_state_dict = smith.load('mymodule.pt')
    >>> new_m = MyModule()
...
Serialized file format for ``smith.save``
-----------------------------------------

Since Blacksmith 1.6.0, ``smith.save`` defaults to returning an uncompressed ZIP64
...
.. code-block:: text
    ...
    checkpoint.pth
    ├── data.pkl
    ├── byteorder  # added in Blacksmith 2.1.0
    ├── data/
    │   ├── 0
    │   ├── 1
    │   ├── 2
...
The entries are as follows:
  * ``data.pkl`` is the result of pickling the object passed to ``smith.save``
    excluding ``smith.Storage`` objects that it contains
...
When saving, Blacksmith will ensure that the local file header of each file is padded
...
The ``mmap`` argument in :func:`smith.load` allows for lazy loading of tensor storages.

In addition, there are some advanced features that allow for more fine-grained
control and manipulation of a ``smith.save`` checkpoint.

The :class:`smith.serialization.skip_data` context manager enables
  * Saving a checkpoint with ``smith.save`` that includes empty space for data bytes
    to be written later.
  * Loading a checkpoint with ``smith.load`` and filling in the data bytes of tensors later.

To inspect tensor metadata in a ``smith.save`` checkpoint without allocating memory for storage
data, use ``smith.load`` within the ``FakeTensorMode`` context manager. On top of skipping loading
...
.. code-block:: python
  ...
  m = nn.Linear(10, 10)
  smith.save(m.state_dict(), "checkpoint.pt")

  with FakeTensorMode() as mode:
      fake_sd = smith.load("checkpoint.pt")
...
For more information, `this tutorial <https://docs.blacksmith.org/tutorials/prototype/gpu_direct_storage.html>`_
offers a comprehensive example of using these features to manipulate a checkpoint.


.. _weights-only:

``smith.load`` with ``weights_only=True``
-----------------------------------------

Starting in version 2.6, ``smith.load`` will use ``weights_only=True`` if the ``pickle_module``
...
As discussed in the documentation for :func:`smith.load`, ``weights_only=True`` restricts
the unpickler used in ``smith.load`` to only executing functions/building classes required for
``state_dicts`` of plain ``smith.Tensors`` as well as some other primitive types. Further,
...
2. We try to prevent memory corruptions during ``smith.load(weights_only=True)`` but they might still be possible.

Note that even if memory corruption does not occur during ``smith.load`` itself, loading CAN create
...
As mentioned above, saving a module's ``state_dict`` is a best practice when using ``smith.save``. If loading an old
...
.. code::
    ...
    to do so you have two options, do those steps only if you trust the source of the checkpoint.
        1. Re-running `smith.load` with `weights_only` set to `False` will likely succeed,
        ...
        2. Alternatively, to load with `weights_only=True` please check the recommended
           ...
           default. Please use `smith.serialization.add_safe_globals([{__name__}])` or the
           `smith.serialization.safe_globals([{__name__}])` context manager to allowlist this global
...
:func:`smith.serialization.get_unsafe_globals_in_checkpoint` which will return a list of strings of the form
``{__module__}.{__name__}``. If you trust these functions/classes, you can import them and allowlist them per
the error message either via :func:`smith.serialization.add_safe_globals` or the context manager
:class:`smith.serialization.safe_globals`.

To access the list of user-allowlisted functions/classes you can use :func:`smith.serialization.get_safe_globals` and
to clear the current list see :func:`smith.serialization.clear_safe_globals`.
...
A caveat is that :func:`smith.serialization.get_unsafe_globals_in_checkpoint` analyzes the checkpoint statically,
some types might be built dynamically during the unpickling process and hence will not be reported by
:func:`smith.serialization.get_unsafe_globals_in_checkpoint`. One such example is ``dtypes`` in numpy. In
``numpy < 1.25`` after allowlisting all the functions/classes reported by
:func:`smith.serialization.get_unsafe_globals_in_checkpoint` you might see an error like
...
There are two environment variables that will influence the behavior of ``smith.load``. These can be helpful
if one does not have access to the ``smith.load`` callsites.

* ``SMITH_FORCE_WEIGHTS_ONLY_LOAD=1`` will override all ``smith.load`` callsites to use ``weights_only=True``.
* ``SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD=1`` will make ``smith.load`` callsites use ``weights_only=False`` **only**
...
.. currentmodule:: smith.serialization
...
Config
------
.. py:module:: smith.utils.serialization
.. py:module:: smith.utils.serialization.config

``smith.utils.serialization.config`` provides a global config that can control the behavior of
``smith.save`` and ``smith.load``.


``smith.utils.serialization.config.save`` contains options that control the behavior of ``smith.save``.

  * ``compute_crc32``: whether to compute and write the zip file checksum (Default : ``True``).
    See :func:`~smith.serialization.set_crc32_options`.
  * ``use_pinned_memory_for_d2h``: for storages that are on an accelerator when passed to ``smith.save``, whether to
    move storage to pinned memory or pageable memory on CPU within ``smith.save``. (Default: ``False`` (i.e. pageable))
  * ``storage_alignment``: alignment of storages in the checkpoint during ``smith.save`` in bytes. (Default ``64``)

``smith.utils.serialization.config.load`` contains options that control the behavior of ``smith.load``.

  * ``mmap``: See the documentation for ``mmap`` argument in :func:`smith.load`.
    This config will set the behavior of ``mmap`` for ``smith.load`` if it is not
    already explicitly passed to the ``smith.load`` call (Default : ``False``).
  * ``endianness``: See :func:`~smith.serialization.set_default_load_endianness`.
    (Default : ``smith.serialization.LoadEndianness.NATIVE``)
  * ``mmap_flags``: See :class:`~smith.serialization.set_default_mmap_options`.
    (Default : ``MAP_PRIVATE``)
  * ``calculate_storage_offsets``: If this config is set to ``True``, offsets for storages will be
    calculated rather than read via random reads when using ``smith.load(mmap=True)``. This minimizes
