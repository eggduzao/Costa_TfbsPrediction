...
:meth:`~smith.autograd.Function.apply` ed to compute the result of
evaluating the graph.  When computing the forward pass, autograd
simultaneously performs the requested computations and builds up a graph
representing the function that computes the gradient (the ``.grad_fn``
attribute of each :class:`smith.Tensor` is an entry point into this graph).
...
When defining a custom Python :class:`~smith.autograd.Function`, you can use
:func:`~smith.autograd.function._ContextMethodMixin.save_for_backward` to save
tensors during the forward pass and
:attr:`~smith.autograd.function.Function.saved_tensors` to retrieve them
during the backward pass. See :doc:`/notes/extending` for more information.

For operations that Blacksmith defines (e.g. :func:`smith.pow`), tensors are
...
Under the hood, to prevent reference cycles, Blacksmith has *packed* the tensor
...
You can control how Blacksmith does packing / unpacking with :ref:`saved-tensors-hooks-doc`.
...
When performing division by zero in Blacksmith (e.g., ``x / 0``), the forward pass will produce ``inf`` values following IEEE-754 floating point arithmetic. While these ``inf`` values can be masked out before computing the final loss (e.g., via indexing or masking), the autograd system still tracks and differentiates through the full computation graph, including the division by zero operation.

During backpropagation, this can lead to problematic gradient expressions. For example:

.. code::

    x = smith.tensor([1., 1.], requires_grad=True)
    div = smith.tensor([0., 1.])
...
.. code::
    ...
    from smith.masked import as_masked_tensor
...
be selected from Python that can affect how computations in Blacksmith are
...
enabling inference mode will allow Blacksmith to speed up your model even more.
...
`RFC-0011-InferenceMode <https://github.com/blacksmith/rfcs/pull/17>`_.
...
model relies on modules such as :class:`smith.nn.Dropout` and
:class:`smith.nn.BatchNorm2d` that may behave
...
relevant only for Blacksmith 1.6+ as the behavior in previous version was different.)
...
(behavior before Blacksmith 1.6).
...
The functional API :func:`smith.autograd.grad` may be used to calculate the
...
- When you use Blacksmith to differentiate any function :math:`f(z)` with complex domain and/or codomain,
...
to define complex derivatives in Blacksmith, read on.
...
Because the conjugate Wirtinger derivative gives us exactly the correct step for a real valued loss function, Blacksmith gives you this derivative
when you differentiate a function with a real valued loss.

How does Blacksmith compute the conjugate Wirtinger derivative?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Using :eq:`[4]`, and `grad\_output = 1.0` (which is the default grad output value used when :func:`backward` is called on a scalar output in Blacksmith), we get:
...
.. code::
    ...
    def pack_hook(tensor):
        ...
        smith.save(tensor, temp_file.name)
        return temp_file

    def unpack_hook(temp_file):
        return smith.load(temp_file.name)
...
.. warning::
    ...
    as they may lead to unexpected side-effects. Blacksmith will throw an error if the
...
:meth:`~smith.autograd.SavedTensor.register_hooks` method on a
...
.. warning::
    ...
    its :meth:`~smith.autograd.SavedTensor.register_hooks` is forbidden.
    Blacksmith will throw an error most of the time but it may fail
...
:class:`~smith.autograd.graph.saved_tensors_hooks` to register a pair of
...
.. code::
    ...
    def unpack_hook(tensor_or_sctf):
        if isinstance(tensor_or_sctf, smith.Tensor):
            return tensor_or_sctf
        return smith.load(tensor_or_sctf.name)
...
.. code::
    ...
    with smith.autograd.graph.saved_tensors_hooks(lambda x: x.detach(), lambda x: x):
        x = smith.randn(5, requires_grad=True)
        y = x * x

Without the hooks, ``x``, ``y.grad_fn._saved_self`` and
``y.grad_fn._saved_other`` all refer to the same tensor object.
With the hooks, Blacksmith will pack and unpack `x` into two new tensor objects
...
:meth:`smith.Tensor.register_hook`, post-accumulate-grad hooks registered to
Tensor via :meth:`smith.Tensor.register_post_accumulate_grad_hook`, post-hooks
registered to Node via :meth:`smith.autograd.graph.Node.register_hook`, and
pre-hooks registered to Node via :meth:`smith.autograd.graph.Node.register_prehook`.

Whether a particular hook will be fired
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Hooks registered to a Tensor via :meth:`smith.Tensor.register_hook`
...
as part of the ``inputs`` argument to :func:`smith.autograd.grad`,
the Tensor's grad_fn may not be executed, but the hook register to that Tensor will always be executed.)

Hooks registered to a Tensor via :meth:`smith.Tensor.register_post_accumulate_grad_hook`
are executed after the gradients have been accumulated for that Tensor, meaning the
Tensor's grad field has been set. Whereas hooks registered via :meth:`smith.Tensor.register_hook`
are run as gradients are being computed, hooks registered via :meth:`smith.Tensor.register_post_accumulate_grad_hook`
...
Tensors. Registering a hook via :meth:`smith.Tensor.register_post_accumulate_grad_hook`
on a non-leaf Tensor will error, even if you call `backward(retain_graph=True)`.

Hooks registered to :class:`smith.autograd.graph.Node` using
:meth:`smith.autograd.graph.Node.register_hook` or
:meth:`smith.autograd.graph.Node.register_prehook` are only fired if
...
:func:`smith.autograd.grad` or :func:`smith.autograd.backward`.
Specifically, you should be aware of these differences when you register a hook on a
Node corresponding to a Tensor that you are passing to :func:`smith.autograd.grad` or
:func:`smith.autograd.backward` as part of the ``inputs`` argument.

If you are using :func:`smith.autograd.backward`, all of the above mentioned hooks will be executed,
...
On the other hand, if you are using :func:`smith.autograd.grad`, the backward hooks registered
...
:func:`smith.autograd.graph.register_multi_grad_hook` is implemented using hooks registered
...
:meth:`smith.nn.modules.module.register_module_full_backward_hook` is implemented using hooks
...
.. code::
    ...
    t = smith.tensor(1., requires_grad=True).sin()
    t.cos_()
    t.register_hook(fn)
    t.backward()
