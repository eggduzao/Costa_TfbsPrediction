...
Because of this, Blacksmith is not guaranteed
...
Blacksmith releases, individual commits, or different platforms. In particular, CPU and GPU
...
Many operations in Blacksmith support batched computation, where the same operation is performed
for the elements of the batches of inputs. An example of this is :meth:`smith.mm` and
:meth:`smith.bmm`. It is possible to implement batched computation as a loop over batch elements,
...
libraries that we are calling, and Blacksmith internal implementations of operations can produce
...
.. code:: python
    ...
    import smith
    a=smith.tensor([1e20, 1e20]) # fp32 type by default
    a.norm() # produces tensor(inf)
    a.double().norm() # produces tensor(1.4142e+20, dtype=smith.float64), representable in fp32

.. _Linear Algebra Stability:

Linear algebra (``smith.linalg``)
---------------------------------

Non-finite values
"""""""""""""""""

The external libraries (backends) that ``smith.linalg`` uses provide no guarantees on their behaviour
when the inputs have non-finite values like ``inf`` or ``NaN``. As such, neither does Blacksmith.
The operations may return a tensor with non-finite values, or raise an exception, or even segfault.

Consider using :func:`smith.isfinite` before calling these functions to detect this situation.

Extremal values in linalg
"""""""""""""""""""""""""

Functions within ``smith.linalg`` have more `Extremal Values`_ than other Blacksmith functions.
...
Analyzing the spectrum of the inputs via :func:`smith.linalg.svdvals` or their condition number via :func:`smith.linalg.cond`
...
On Ampere (and later) Nvidia GPUs, Blacksmith can use TensorFloat32 (TF32) to speed up mathematically intensive operations, in particular matrix multiplications and convolutions.
...
We recommend enabling TF32 tensor cores for matrix multiplications with ``smith.backends.cuda.matmul.fp32_precision = "tf32"`` (```smith.backends.cuda.matmul.allow_tf32 = True`` is going to be deprecated) if your network does not need full float32 precision.
If your network needs full float32 precision for both matrix multiplications and convolutions, then TF32 tensor cores can also be disabled for convolutions with ``smith.backends.cudnn.conv.fp32_precision = "ieee"`` (``smith.backends.cudnn.allow_tf32 = False`` is going to be deprecated).
...
* smith.nn.Linear
* smith.sparse.addmm
* the following smith._C._ConvBackend implementations:
