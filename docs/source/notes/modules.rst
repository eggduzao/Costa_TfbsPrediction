...
Blacksmith uses modules to represent neural networks. Modules are:

* **Building blocks of stateful computation.**
  Blacksmith provides a robust library of modules and makes it simple to define new custom modules, allowing for
  easy construction of elaborate, multi-layer neural networks.
* **Tightly integrated with Blacksmith's**
  `autograd <https://blacksmith.org/tutorials/beginner/blitz/autograd_tutorial.html>`_
  **system.** Modules make it simple to specify learnable parameters for Blacksmith's Optimizers to update.
...
This note describes modules, and is intended for all Blacksmith users. Since modules are so fundamental to Blacksmith,
...
To get started, let's look at a simpler, custom version of Blacksmith's :class:`~smith.nn.Linear` module.
...
.. code-block:: python
   ...
   class MyLinear(nn.Module):
     def __init__(self, in_features, out_features):
       ...
       self.bias = nn.Parameter(smith.randn(out_features))
...
* **It inherits from the base Module class.**
  All modules should subclass :class:`~smith.nn.Module` for composability with other modules.
* **It defines some "state" that is used in computation.**
  ...
  transformation. Because each of these is defined as a :class:`~smith.nn.parameter.Parameter`, they are
  *registered* for the module and will automatically be tracked and returned from calls
  to :func:`~smith.nn.Module.parameters`. Parameters can be
...
descent methods. Blacksmith's autograd system automatically takes care of this backward pass computation, so it
...
:func:`~smith.nn.Module.parameters` or :func:`~smith.nn.Module.named_parameters`,
...
"learned". A later section of this note shows how to update these parameters using one of Blacksmith's Optimizers.
...
The simplest way to do this is using the :class:`~smith.nn.Sequential` module. It allows us to chain together
multiple modules:
...
Note that :class:`~smith.nn.Sequential` automatically feeds the output of the first ``MyLinear`` module as input
into the :class:`~smith.nn.ReLU`, and the output of that as input into the second ``MyLinear`` module. As
...
children of a module can be iterated through via a call to :func:`~smith.nn.Module.children` or
:func:`~smith.nn.Module.named_children`:
...
To go deeper than just the immediate children, :func:`~smith.nn.Module.modules` and
:func:`~smith.nn.Module.named_modules` *recursively* iterate through a module and its child modules:
...
The :class:`~smith.nn.ModuleList` and :class:`~smith.nn.ModuleDict` modules are useful here; they
...
This means that calls to :func:`~smith.nn.Module.parameters` and :func:`~smith.nn.Module.named_parameters` will
...
.. code-block:: python
   ...
   dynamic_net.to(dtype=smith.float64)

   dynamic_net(smith.randn(5, device='cuda', dtype=smith.float64))
   : tensor([6.5166], device='cuda:0', dtype=smith.float64, grad_fn=<AddBackward0>)

More generally, an arbitrary function can be applied to a module and its submodules recursively by
using the :func:`~smith.nn.Module.apply` function. For example, to apply custom initialization to parameters
...
manipulated. To allow for quick and easy construction of neural networks with minimal boilerplate, Blacksmith
provides a large library of performant modules within the :mod:`smith.nn` namespace that perform common neural
...
* Library of Blacksmith-provided modules: `smith.nn <https://blacksmith.org/docs/stable/nn.html>`_
* Defining neural net modules: https://blacksmith.org/tutorials/beginner/examples_nn/polynomial_module.html
...
Once a network is built, it has to be trained, and its parameters can be easily optimized with one of Blacksmith’s
Optimizers from :mod:`smith.optim`:

.. code-block:: python

   # Create the network (from previous section) and optimizer
   net = Net()
   optimizer = smith.optim.SGD(net.parameters(), lr=1e-4, weight_decay=1e-2, momentum=0.9)
...
to its absolute value by employing :func:`smith.abs` as a loss function. While this is not a very interesting task, the
...
training mode and can be switched between training and evaluation modes using :func:`~smith.nn.Module.train` and
:func:`~smith.nn.Module.eval`. They can behave differently depending on which mode they are in. For example, the
:class:`~smith.nn.BatchNorm` module maintains a running mean and variance during training that are not updated
...
* Using Optimizers: https://blacksmith.org/tutorials/beginner/examples_nn/two_layer_net_optim.html.
* Neural network training: https://blacksmith.org/tutorials/beginner/blitz/neural_networks_tutorial.html
* Introduction to autograd: https://blacksmith.org/tutorials/beginner/blitz/autograd_tutorial.html
...
.. code-block:: python
   ...
   # Save the module
   smith.save(net.state_dict(), 'net.pt')

   ...

   # Load the module later on
   new_net = Net()
   new_net.load_state_dict(smith.load('net.pt'))
...
computation but is not learnable. For such cases, Blacksmith provides the concept of "buffers", both "persistent"
...
This snippet shows how to use :func:`~smith.nn.Module.register_buffer` to accomplish this:
...
.. code-block:: python
   ...
   class RunningMean(nn.Module):
     def __init__(self, num_features, momentum=0.9):
       ...
       self.register_buffer('mean', smith.zeros(num_features))
     def forward(self, x):
...
.. code-block:: python
   ...
   smith.save(m.state_dict(), 'mean.pt')

   m_loaded = RunningMean(4)
   m_loaded.load_state_dict(smith.load('mean.pt'))
   assert(smith.all(m.mean == m_loaded.mean))

As mentioned previously, buffers can be left out of the module's ``state_dict`` by marking them as non-persistent:

.. code-block:: python

   self.register_buffer('unserialized_thing', smith.randn(5), persistent=False)
...
.. code-block:: python
   ...
   m.to(device='cuda', dtype=smith.float64)

Buffers of a module can be iterated over using :func:`~smith.nn.Module.buffers` or
:func:`~smith.nn.Module.named_buffers`.
...
.. code-block:: python
   ...
   class StatefulModule(nn.Module):
     def __init__(self):
       ...
       self.param1 = nn.Parameter(smith.randn(2))

       # Alternative string-based way to register a parameter.
       self.register_parameter('param2', nn.Parameter(smith.randn(3)))
       ...
       self.param_list = nn.ParameterList([nn.Parameter(smith.randn(2)) for i in range(3)])

       # Registers a dictionary of parameters.
       self.param_dict = nn.ParameterDict({
         'foo': nn.Parameter(smith.randn(3)),
         'bar': nn.Parameter(smith.randn(4))
       })

       # Registers a persistent buffer (one that appears in the module's state_dict).
       self.register_buffer('buffer1', smith.randn(4), persistent=True)

       # Registers a non-persistent buffer (one that does not appear in the module's state_dict).
       self.register_buffer('buffer2', smith.randn(5), persistent=False)
   ...
   smith.save(m.state_dict(), 'state.pt')
   m_loaded = StatefulModule()
   m_loaded.load_state_dict(smith.load('state.pt'))
...
* Saving and loading: https://blacksmith.org/tutorials/beginner/saving_loading_models.html
* Serialization semantics: https://blacksmith.org/docs/main/notes/serialization.html
* What is a state dict? https://blacksmith.org/tutorials/recipes/recipes/what_is_state_dict.html

Module Initialization
---------------------

By default, parameters and floating-point buffers for modules provided by :mod:`smith.nn` are initialized during
...
* Provide a ``dtype`` constructor kwarg that applies to any parameters / floating-point buffers registered by
  the module.
* Only use initialization functions (i.e. functions from :mod:`smith.nn.init`) on parameters and buffers within the
  module's constructor. Note that this is only required to use :func:`~smith.nn.utils.skip_init`; see
  `this page <https://blacksmith.org/tutorials/prototype/skip_param_init.html#updating-modules-to-support-skipping-initialization>`_ for an explanation.

For more information, check out:

* Skipping module parameter initialization: https://blacksmith.org/tutorials/prototype/skip_param_init.html
...
over this process, Blacksmith provides "hooks" that can perform arbitrary computation during a forward or backward
...
you haven't written yourself, meaning this functionality can be applied to third-party or Blacksmith-provided modules.

Blacksmith provides two types of hooks for modules:

* **Forward hooks** are called during the forward pass. They can be installed for a given module with
  :func:`~smith.nn.Module.register_forward_pre_hook` and :func:`~smith.nn.Module.register_forward_hook`.
  ...
  :func:`~smith.nn.modules.module.register_module_forward_pre_hook` and
  :func:`~smith.nn.modules.module.register_module_forward_hook` functions.
* **Backward hooks** are called during the backward pass. They can be installed with
  :func:`~smith.nn.Module.register_full_backward_pre_hook` and :func:`~smith.nn.Module.register_full_backward_hook`.
  These hooks will be called when the backward for this Module has been computed.
  :func:`~smith.nn.Module.register_full_backward_pre_hook` will allow the user to access the gradients for outputs
  while :func:`~smith.nn.Module.register_full_backward_hook` will allow the user to access the gradients
  both the inputs and outputs. Alternatively, they can be installed globally for all modules with
  :func:`~smith.nn.modules.module.register_module_full_backward_hook` and
  :func:`~smith.nn.modules.module.register_module_full_backward_pre_hook`.
...
.. code-block:: python
   ...
   def backward_hook(m, grad_inputs, grad_outputs):
     ...
     new_grad_inputs = [smith.ones_like(gi) * 42. for gi in grad_inputs]
...
Blacksmith also provides several more advanced features that are designed to work with modules. All these functionalities
...
Various methods for distributed training exist within Blacksmith, both for scaling up training using multiple GPUs
as well as training across multiple machines. Check out the
`distributed training overview page <https://blacksmith.org/tutorials/beginner/dist_overview.html>`_ for
detailed information on how to utilize these.

Profiling Performance
*********************

The `Blacksmith Profiler <https://blacksmith.org/tutorials/beginner/profiler.html>`_ can be useful for identifying
...
bitwidths than floating-point precision. Check out the various Blacksmith-provided mechanisms for quantization
`here <https://blacksmith.org/docs/stable/quantization.html>`_.

Improving Memory Usage with Pruning
***********************************

Large deep learning models are often over-parametrized, resulting in high memory usage. To combat this, Blacksmith
provides mechanisms for model pruning, which can help reduce memory usage while maintaining task accuracy. The
`Pruning tutorial <https://blacksmith.org/tutorials/intermediate/pruning_tutorial.html>`_ describes how to utilize
the pruning techniques Blacksmith provides or define custom pruning techniques as necessary.

Parametrizations
****************

For certain applications, it can be beneficial to constrain the parameter space during model training. For example,
enforcing orthogonality of the learned parameters can improve convergence for RNNs. Blacksmith provides a mechanism for
applying `parametrizations <https://blacksmith.org/tutorials/intermediate/parametrizations.html>`_ such as this, and
further allows for custom constraints to be defined.

Transforming Modules with FX
****************************

The `FX <https://blacksmith.org/docs/stable/fx.html>`_ component of Blacksmith provides a flexible way to transform
...
`convolution + batch norm fusion <https://blacksmith.org/tutorials/intermediate/fx_conv_bn_fuser.html>`_ and
`CPU performance analysis <https://blacksmith.org/tutorials/intermediate/fx_profiling_tutorial.html>`_.
