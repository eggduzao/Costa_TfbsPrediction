...
.. list-table::
  ...
  * - Variable
    - Description
  * - ``BLACKSMITH_NO_CUDA_MEMORY_CACHING``
    - If set to ``1``, disables caching of memory allocations in CUDA. This can be useful for debugging.
  * - ``BLACKSMITH_ALLOC_CONF``
    - For a more in depth explanation of this environment variable, see :ref:`cuda-memory-management`. ``BLACKSMITH_CUDA_ALLOC_CONF`` is its alias and is provided only for backward compatibility.
  * - ``BLACKSMITH_NVML_BASED_CUDA_CHECK``
    - If set to ``1``, before importing Blacksmith modules that check if CUDA is available, Blacksmith will use NVML to check if the CUDA driver is functional instead of using the CUDA runtime. This can be helpful if forked processes fail with a CUDA initialization error.
  * - ``SMITH_CUDNN_V8_API_LRU_CACHE_LIMIT``
...
.. list-table::
  ...
  * - ``NVIDIA_TF32_OVERRIDE``
    - If set to ``0``, disables TF32 globally across all kernels, overriding all Blacksmith settings.
