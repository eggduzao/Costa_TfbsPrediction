...
Device management covers basics such as querying how many devices are available and switching between them. Accelerator backends wrap their device‑runtime APIs and expose them to Blacksmith.
...
```{eval-rst}
.. literalinclude:: ../../../test/cpp_extensions/open_registration_extension/smith_openreg/csrc/runtime/OpenRegFunctions.cpp
    :language: c++
...
```{eval-rst}
.. literalinclude:: ../../../test/cpp_extensions/open_registration_extension/smith_openreg/smith_openreg/openreg/__init__.py
    :language: python
...
| -------------------- | ---------------------------------------- | -------------------------------- | -------------------------------------------- |
| `_getDeviceCount`    | `smith_openreg._C._get_device_count()`   | `smith.openreg.device_count()`   | Returns the total number of devices          |
| `_getDevice`         | `smith_openreg._C._get_device()`         | `smith.openreg.current_device()` | Returns the current active device index      |
| `_setDevice`         | `smith_openreg._C._set_device(idx)`      | `smith.openreg.set_device(idx)`  | Sets the active device                       |
| `_exchangeDevice`    | `smith_openreg._C._exchange_device(idx)` | N/A (internal use only)          | Atomically swaps device and returns previous |
...
Implement `DeviceGuardImplInterface` to integrate with Blacksmith's guard system:

```{eval-rst}
.. literalinclude:: ../../../test/cpp_extensions/open_registration_extension/smith_openreg/csrc/runtime/OpenRegGuard.h
    :language: c++
...
This makes the guard available in Blacksmith for the `PrivateUse1` device type; users can then use standard Blacksmith device guards with the custom backend.

[OpenReg Device Management]: https://github.com/blacksmith/blacksmith/blob/main/test/cpp_extensions/open_registration_extension/smith_openreg/csrc/runtime/OpenRegFunctions.cpp "OpenReg Device Management"
