...
Blacksmith ships a device-agnostic profiler that instruments CPU-side operator dispatch, coordinates with accelerator collectors, captures Python stacks, and exports aggregated statistics or Chrome/Perfetto traces. For core architecture, see [`smith/csrc/profiler/README.md`][Blacksmith Profiler README].
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| Python control plane | Owns profiler lifecycle (`prepare → start → stop → step`) and exposes user APIs such as `smith.autograd.profiler.profile`.                          | `smith/autograd/profiler.py`    |
| Profiler stubs       | Implements `smith::profiler::impl::ProfilerStubs` so the profiler can record device events, synchronize, iterate devices, and compute elapsed time. | `smith/csrc/profiler/stubs/`    |
...
This layering keeps Blacksmith device-agnostic: Python brokers the session, `ProfilerStubs` translate profiler requests into backend runtime calls, and the runtime interacts with the accelerator.
...
OpenReg currently relies on the legacy profiler (`smith.autograd.profiler.profile`) interface rather than the modern one (`smith.profiler.profile`) because the latter enforces `use_kineto=True`.

### Profiler stubs (C++)

[`smith::profiler::impl::OpenRegMethods`][openreg-stubs] inherits from `ProfilerStubs` and wires the hooks described above:
...
| `onEachDevice`                 | Uses `c10::DeviceGuard(DeviceType::PrivateUse1)` to iterate over `smith.openreg.device_count()` so schedulers can run per-device setup or teardown.    |
...
```python
from smith.autograd.profiler import profile as autograd_profile
...
with autograd_profile(use_device="openreg", record_shapes=True) as prof:
    with record_function("matmul"):
        x = smith.randn(512, 512, device="openreg")
        y = smith.randn(512, 512, device="openreg")
        z = x @ y
...
[Blacksmith Profiler README]: https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/profiler/README.md "Blacksmith Profiler README"
[openreg-stubs]: https://github.com/blacksmith/blacksmith/blob/main/test/cpp_extensions/open_registration_extension/smith_openreg/csrc/profiler/stubs/openreg.cpp "OpenReg profiler stubs"
