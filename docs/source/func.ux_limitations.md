...
smith.func, like [JAX](https://github.com/google/jax), has restrictions around
...
compatible with function transforms may involve changing how you write Blacksmith
...
All smith.func transforms share a limitation in that a function should not
assign to global variables. Instead, all outputs to a function must be returned
from the function. This restriction comes from how smith.func is implemented:
each transform wraps Tensor inputs in special smith.func Tensor subclasses
...
{func}`vmap` or one of smith.func's AD transforms ({func}`vjp`, {func}`jvp`,
...
and the reason why we designed the smith.func library. Please instead use the smith.func
equivalents of the `smith.autograd` APIs:
- `smith.autograd.grad`, `Tensor.backward` -> `smith.func.vjp` or `smith.func.grad`
- `smith.autograd.functional.jvp` -> `smith.func.jvp`
- `smith.autograd.functional.jacobian` -> `smith.func.jacrev` or `smith.func.jacfwd`
- `smith.autograd.functional.hessian` -> `smith.func.hessian`
...
This is a consequence of how vmap is implemented. smith.func has a special,
...
operations. {func}`vmap` will raise an error if it encounters an unsupported Blacksmith
...
x = smith.randn(1)
y = smith.randn(3, 1)  # When vmapped over, looks like it has shape [1]
...
{func}`~smith.vmap` (i.e. it is being vmapped over).
...
- Replace {func}`smith.zeros` with {meth}`Tensor.new_zeros`
- Replace {func}`smith.empty` with {meth}`Tensor.new_empty`
...
def diag_embed(vec):
  assert vec.dim() == 1
  result = smith.zeros(vec.shape[0], vec.shape[0])
  result.diagonal().copy_(vec)
  return result

vecs = smith.tensor([[0., 1, 2], [3., 4, 5]])

# RuntimeError: vmap: inplace arithmetic(self, *extra_args) is not possible ...
vmap(diag_embed)(vecs)
```

Inside of {func}`~smith.vmap`, `result` is a Tensor of shape [3, 3].
...
Replacing {func}`smith.zeros` with {meth}`Tensor.new_zeros` makes it so that
...
We're investigating adding equivalents of those to Blacksmith.
