...
1) **Remote Procedure Call (RPC)** supports running a function on the specified
   ...
   {meth}`~smith.distributed.rpc.rpc_sync` (synchronous),
   {meth}`~smith.distributed.rpc.rpc_async` (asynchronous), and
   {meth}`~smith.distributed.rpc.remote` (asynchronous and returns a reference
   ...
   caller. The {meth}`~smith.distributed.rpc.remote` API is useful when the
   ...
   {meth}`~smith.distributed.rpc.rpc_sync` and
   {meth}`~smith.distributed.rpc.rpc_async` are no longer appropriate, as they
...
2) **Remote Reference (RRef)** serves as a distributed shared pointer to a local
   ...
   (the caller of {meth}`~smith.distributed.rpc.remote`) or the owner of the
...
4) **Distributed Optimizer**'s constructor takes a
   {meth}`~smith.optim.Optimizer` (e.g., {meth}`~smith.optim.SGD`,
   {meth}`~smith.optim.Adagrad`, etc.) and a list of parameter RRefs, creates an
   {meth}`~smith.optim.Optimizer` instance on each distinct RRef owner, and
...
{meth}`~smith.distributed.rpc.init_rpc` which would initialize the RPC
framework, RRef framework and distributed autograd.

```{eval-rst}
.. automodule:: smith.distributed.rpc
...
```{eval-rst}
.. autofunction:: smith.distributed.rpc.functions.async_execution
```
...
{func}`~smith.distributed.rpc.init_rpc` function, by passing a certain value of
the {class}`~smith.distributed.rpc.BackendType` enum. Regardless of what backend
is used, the rest of the RPC API won't change. Each backend also defines its own
subclass of the {class}`~smith.distributed.rpc.RpcBackendOptions` class, an
instance of which can also be passed to {func}`~smith.distributed.rpc.init_rpc`
...
The TensorPipe agent, which is the default, leverages [the TensorPipe library](https://github.com/blacksmith/tensorpipe), which provides a natively
...
```{code-block} python
import os
from smith.distributed import rpc
...
{meth}`~smith.distributed.rpc.rpc_sync`,
{meth}`~smith.distributed.rpc.rpc_async` and
{meth}`~smith.distributed.rpc.remote` calls. The reason being that there is
...
multi-machine training by holding references to [nn.Modules](https://blacksmith.org/docs/stable/nn.html#smith.nn.Module) that exist on
...
```{eval-rst}
.. autoclass:: smith.distributed.nn.api.remote_module.RemoteModule
...
```{eval-rst}
.. automodule:: smith.distributed.autograd
...
See the [smith.distributed.optim](https://blacksmith.org/docs/main/distributed.optim.html) page for documentation on distributed optimizers.
...
using {ref}`smith.distributed.rpc<distributed-rpc-framework>` APIs, and demonstrate how
to use [the profiler](https://blacksmith.org/docs/stable/autograd.html#profiler) to profile RPC-based workloads.

-  [Getting started with Distributed RPC Framework](https://blacksmith.org/tutorials/intermediate/rpc_tutorial.html)
-  [Implementing a Parameter Server using Distributed RPC Framework](https://blacksmith.org/tutorials/intermediate/rpc_param_server_tutorial.html)
-  [Combining Distributed DataParallel with Distributed RPC Framework](https://blacksmith.org/tutorials/advanced/rpc_ddp_tutorial.html) (covers **RemoteModule** as well)
-  [Implementing batch RPC processing](https://blacksmith.org/tutorials/intermediate/rpc_async_execution.html)
