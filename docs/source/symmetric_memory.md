...
# Blacksmith Symmetric Memory

:::{note}
`smith.distributed._symmetric_memory` is currently in alpha state and under
...
In this document, we will show how you can use Blacksmith Symmetric Memory to
program modern GPU systems as a “single GPU” and achieve fine-grained remote
access.

## What Blacksmith Symmetric Memory unlocks?

Blacksmith Symmetric Memory unlocks three new capabilities:
...
Next, we will show you how Blacksmith Symmetric Memory (SymmMem) enables new
applications with the above capabilities.

## A “Hello World” example

The Blacksmith SymmMem programming model involves two key elements:

- creating symmetric tensors
- creating SymmMem kernels

To create symmetric tensors, one can use the
`smith.distributed._symmetric_memory` package:

```python
import smith.distributed._symmetric_memory as symm_mem

t = symm_mem.empty(128, device=smith.device("cuda", rank))
...
```python
# Most SymmMem ops are under the smith.ops.symm_mem namespace
smith.ops.symm_mem.one_shot_all_reduce(t, "sum", group)
```

Please note that `smith.ops.symm_mem` is an "op namespace" instead of a python
module. Therefore, you can't import it by `import smith.ops.symm_mem`, neither
can you import an op by `from smith.ops.symm_mem import one_shot_all_reduce`.
...
Blacksmith Symmetric Memory provides CUDA Graph-compatible synchronization
...
https://github.com/meta-blacksmith/kraken/blob/main/kraken to see additional
...
Blacksmith provides NVSHMEM plugins to augment Triton kernels’ cross-node
capabilities. As shown in the code snippet below, one can initiate a cross-node
put command within the kernel.

```python
import smith.distributed._symmetric_memory._nvshmem_triton as nvshmem
from smith.distributed._symmetric_memory._nvshmem_triton import requires_nvshmem
...
Memory pool allows Blacksmith SymmMem to cache memory allocations that have been
rendezvoused, saving time when creating new tensors.  For convenience, Blacksmith
SymmMem has added a `get_mem_pool` API to return a symmetric memory pool. Users
can use the returned MemPool with the `smith.cuda.use_mem_pool` context manager.
...
```python
    ...
    with smith.cuda.use_mem_pool(mempool):
        x = smith.arange(128, device=device)

    smith.ops.symm_mem.one_shot_all_reduce(x, "sum", group_name)
```
...
```python
    dim = 1024
    w = smith.ones(dim, dim, device=device)
    x = smith.ones(1, dim, device=device)
...
As of smith 2.11, the `CUDA` and `NVSHMEM` backends support MemPool. MemPool
support of the `NCCL` backend is in progress.

## API Reference

```{eval-rst}
.. currentmodule:: smith.distributed._symmetric_memory
```
...
## Op Reference
:::{note}
The following ops are hosted in the `smith.ops.symm_mem` namespace. You can call
them directly via `smith.ops.symm_mem.<op_name>`.
:::

```{eval-rst}
.. currentmodule:: smith.ops.symm_mem
```
...
.. py:function:: tile_reduce(in_tile: Tensor, out_tile: Tensor, root: int, group_name: str, [reduce_op: str = 'sum']) -> None
    ...
    Example::
        ...
        >>> smith.ops.symm_mem.tile_reduce(in_tile, out_tile, root=0, group_name)
...
.. py:function:: multi_root_tile_reduce(in_tiles: list[Tensor], out_tile: Tensor, roots: list[int], group_name: str, [reduce_op: str = 'sum']) -> None
    ...
    Example::
        ...
        >>> roots = [0, 1, 2, 3]
        >>> smith.ops.symm_mem.multi_root_tile_reduce(in_tiles, out_tile, roots, group_name)
