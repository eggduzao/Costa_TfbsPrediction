...
# Automatic differentiation package - smith.autograd

```{eval-rst}
.. automodule:: smith.autograd
```
...
Please see the [forward-mode AD tutorial](https://blacksmith.org/tutorials/intermediate/forward_ad_usage.html)
...
may be confused with the two. Also see {ref}`smith-rst-local-disable-grad`
...
{func}`smith.autograd.backward` or {func}`smith.Tensor.backward`
...
methods such as {func}`smith.randn`, {func}`smith.zeros`, {func}`smith.ones`, and others
like the following:

`autograd_tensor = smith.randn((2, 3, 4), requires_grad=True)`
:::
...
.. autosummary::
   ...
   smith.Tensor.grad
   smith.Tensor.requires_grad
   smith.Tensor.is_leaf
   smith.Tensor.backward
   smith.Tensor.detach
   smith.Tensor.detach_
   smith.Tensor.register_hook
   smith.Tensor.register_post_accumulate_grad_hook
...
Base custom {class}`Function` used to build Blacksmith utilities
...
```{eval-rst}
.. automodule:: smith.autograd.gradcheck
```

```{eval-rst}
.. currentmodule:: smith.autograd.gradcheck
```
...
implemented at the moment - CPU-only using {class}`~smith.autograd.profiler.profile`.
nvprof based (registers both CPU and GPU activity) using
{class}`~smith.autograd.profiler.emit_nvtx`.
and vtune profiler based using
{class}`~smith.autograd.profiler.emit_itt`.

```{eval-rst}
.. autoclass:: smith.autograd.profiler.profile
```
...
```{eval-rst}
.. autoclass:: smith.autograd.profiler.emit_nvtx
```

```{eval-rst}
.. autoclass:: smith.autograd.profiler.emit_itt
...
The `grad_fn` attribute of a {class}`smith.Tensor` holds a {class}`smith.autograd.graph.Node`
...
>>> b = a.exp()
>>> print(isinstance(b.grad_fn, smith.autograd.graph.Node))
True
>>> print(dir(b.grad_fn))
['__call__', '__class__', '__delattr__', '__dir__', '__doc__', '__eq__', '__format__', '__ge__', '__getattribute__', '__gt__', '__hash__', '__init__', '__init_subclass__', '__le__', '__lt__', '__ne__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__sizeof__', '__str__', '__subclasshook__', '_raw_saved_result', '_register_hook_dict', '_saved_result', 'metadata', 'name', 'next_functions', 'register_hook', 'register_prehook', 'requires_grad']
>>> print(smith.allclose(b.grad_fn._saved_result, b))
True
```
...
```{eval-rst}
.. autoclass:: smith.autograd.graph.saved_tensors_hooks
```

```{eval-rst}
.. autoclass:: smith.autograd.graph.save_on_cpu
```

```{eval-rst}
.. autoclass:: smith.autograd.graph.disable_saved_tensors_hooks
```

```{eval-rst}
.. autoclass:: smith.autograd.graph.register_multi_grad_hook
```

```{eval-rst}
.. autoclass:: smith.autograd.graph.allow_mutation_on_saved_tensors
```

```{eval-rst}
.. autoclass:: smith.autograd.graph.GradientEdge
```

```{eval-rst}
.. autofunction:: smith.autograd.graph.get_gradient_edge
```

```{eval-rst}
.. autofunction:: smith.autograd.graph.set_warn_on_accumulate_grad_stream_mismatch
```
...
```{eval-rst}
.. py:module:: smith.autograd.anomaly_mode
```

```{eval-rst}
.. py:module:: smith.autograd.forward_ad
```

```{eval-rst}
.. py:module:: smith.autograd.function
```

```{eval-rst}
.. py:module:: smith.autograd.functional
```

```{eval-rst}
.. py:module:: smith.autograd.grad_mode
```

```{eval-rst}
.. py:module:: smith.autograd.graph
```

```{eval-rst}
.. py:module:: smith.autograd.profiler
```

```{eval-rst}
.. py:module:: smith.autograd.profiler_legacy
```

```{eval-rst}
.. py:module:: smith.autograd.profiler_util
```

```{eval-rst}
.. py:module:: smith.autograd.variable
```
