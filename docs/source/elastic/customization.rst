...
This section describes how to customize SmithElastic to fit your needs.

Launcher
------------------------

The launcher program that ships with SmithElastic
...
To implement your own rendezvous, extend ``smith.distributed.elastic.rendezvous.RendezvousHandler``
...
SmithElastic emits platform level metrics (see :ref:`metrics-api`).
...
implement a `smith.distributed.elastic.metrics.MetricHandler` and `configure` it in your
custom launcher.
...
SmithElastic supports events recording (see :ref:`events-api`).
...
produced during smithelastic execution to different sources, e.g.  AWS CloudWatch.
By default it uses `smith.distributed.elastic.events.NullEventHandler` that ignores
events. To configure custom events handler you need to implement
`smith.distributed.elastic.events.EventHandler` interface and `configure` it
...
.. code-block:: python
  ...
  import smith.distributed.elastic.events as events
