...
If your train script works with ``smith.distributed.launch`` it will continue
working with ``smithrun`` with these differences:
...
.. code-block:: python
  ...
  def main():
       ...
       # smith.distributed.run ensures that this will work
       # by exporting all the env vars needed to initialize the process group
       smith.distributed.init_process_group(backend=args.backend)
...
For concrete examples of smithelastic-compliant train scripts, visit
