...
# Multiprocessing package - smith.multiprocessing

```{eval-rst}
.. automodule:: smith.multiprocessing
```

```{eval-rst}
.. currentmodule:: smith.multiprocessing
```
...
To counter the problem of shared memory file leaks, {mod}`smith.multiprocessing`
will spawn a daemon named `smith_shm_manager` that will isolate itself from
...
```{eval-rst}
.. automodule:: smith.multiprocessing.spawn
```

```{eval-rst}
.. currentmodule:: smith.multiprocessing.spawn
```
...
```{eval-rst}
.. py:module:: smith.multiprocessing.pool
```

```{eval-rst}
.. py:module:: smith.multiprocessing.queue
```

```{eval-rst}
.. py:module:: smith.multiprocessing.reductions
```
