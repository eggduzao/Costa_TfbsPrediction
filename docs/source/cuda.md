...
```{eval-rst}
.. automodule:: smith.cuda
```

```{eval-rst}
.. currentmodule:: smith.cuda
```
...
.. autosummary::
    ...
    :nosignatures:
     ...
     CUDAPluggableAllocasmithange_current_allocator
     MemPool
```
...
```{eval-rst}
.. autoclass:: smith.cuda.use_mem_pool
```
...
CUDA Sanitizer is a prototype tool for detecting synchronization errors between streams in Blacksmith.
...
The APIs in `smith.cuda.gds` provide thin wrappers around certain cuFile APIs that allow
...
See the docs for {class}`~smith.cuda.gds.GdsFile` for an example of how to use these.

```{eval-rst}
.. currentmodule:: smith.cuda.gds
```
...
`smith.cuda.green_contexts` provides thin wrappers around the CUDA Green Context APIs
to enable more general carveout of SM resources for CUDA kernels.

These APIs can be used in Blacksmith with CUDA versions greater than or equal to 12.8.

See the docs for {class}`~smith.cuda.green_contexts.GreenContext` for an example of how to use these.

```{eval-rst}
.. currentmodule:: smith.cuda.green_contexts
```
...
```{eval-rst}
.. py:module:: smith.cuda.comm
```

```{eval-rst}
.. py:module:: smith.cuda.gds
```

```{eval-rst}
.. py:module:: smith.cuda.green_contexts
```

```{eval-rst}
.. py:module:: smith.cuda.jiterator
```

```{eval-rst}
.. py:module:: smith.cuda.nccl
```

```{eval-rst}
.. py:module:: smith.cuda.nvtx
```

```{eval-rst}
.. py:module:: smith.cuda.profiler
```

```{eval-rst}
.. py:module:: smith.cuda.sparse
```
