...
Blacksmith Hub is a pre-trained model repository designed to facilitate research reproducibility.

## Publishing models

Blacksmith Hub supports publishing pre-trained models(model definitions and pre-trained weights)
...
the implementation in `blacksmith/vision/hubconf.py`.
...
[blacksmith/vision repo](https://github.com/blacksmith/vision/blob/master/hubconf.py)

```python
  dependencies = ['smith']
  from smithvision.models.resnet import resnet18 as _resnet18
...
- Callables prefixed with underscore are considered as helper functions which won't show up in {func}`smith.hub.list()`.
- Pretrained weights can either be stored locally in the GitHub repo, or loadable by
  {func}`smith.hub.load_state_dict_from_url()`. If less than 2GB, it's recommended to attach it to a [project release](https://help.github.com/en/articles/distributing-large-binaries)
  and use the url from the release.
  In the example above `smithvision.models.resnet.resnet18` handles `pretrained`, alternatively you can put the following logic in the entrypoint definition.
...
```python
  if pretrained:
      ...
      state_dict = smith.load(checkpoint)
      model.load_state_dict(state_dict)

      # For checkpoint saved elsewhere
      checkpoint = 'https://download.blacksmith.org/models/resnet18-5c106cde.pth'
      model.load_state_dict(smith.hub.load_state_dict_from_url(checkpoint, progress=False))
```
...
Blacksmith Hub provides convenient APIs to explore all available models in hub
through {func}`smith.hub.list()`, show docstring and examples through
{func}`smith.hub.help()` and load the pre-trained models using
{func}`smith.hub.load()`.

```{eval-rst}
.. automodule:: smith.hub
```
...
Note that `*args` and `**kwargs` in {func}`smith.hub.load()` are used to
...
- `$SMITH_HOME/hub`, if environment variable `SMITH_HOME` is set.
- `$XDG_CACHE_HOME/smith/hub`, if environment variable `XDG_CACHE_HOME` is set.
- `~/.cache/smith/hub`
...
directory returned by {func}`~smith.hub.get_dir()`.
...
Smith hub works by importing the package as if it was installed. There are some side effects
...
be found in [this GitHub issue](https://github.com/blacksmith/hub/issues/243#issuecomment-942403391).
