```{eval-rst}
.. automodule:: smith.package
.. py:module:: smith.package.analyze

.. currentmodule:: smith.package
```

# smith.package
`smith.package` adds support for creating packages containing both artifacts and arbitrary
Blacksmith code. These packages can be saved, shared, used to load and execute models
...
will help you learn more about `smith.package` and how to use it.
...
The container format for a `smith.package` is ZIP, so any tools that work with standard ZIP files should
work for exploring the contents. Some common ways to interact with ZIP files:

* `unzip my_package.pt` will unzip the `smith.package` archive to disk, where you can freely inspect its contents.
...
└── smithvision
    └── models
        ├── resnet.py
        └── utils.py
~ cd my_package && cat smithvision/models/resnet.py
...
```
...
with ZipFile("my_package.pt") as myzip:
    file_bytes = myzip.read("smithvision/models/resnet.py")
    # edit file_bytes in some way
    myzip.writestr("smithvision/models/resnet.py", new_file_bytes)
```
...
current contents of a `smith.package`.
...
```python
with smith.PackageExporter("package.pt") as exporter:
    # Pickles the object and saves to `my_resources/tensor.pkl` in the archive.
    exporter.save_pickle("my_resources", "tensor.pkl", smith.randn(4))
...
```python
importer = smith.PackageImporter("package.pt")
...
`smith.package` allows for the customization of how classes are packaged. This behavior is accessed through defining the method
...
class Foo:
    ...
    def __reduce_package__(self, exporter: PackageExporter):
        ...
        object inside of the ``smith.package`` archive.

        Returns function w/ arguments to load the object from a
        ``smith.package.PackageImporter``'s Pickler's ``persistent_load`` function.
        """
...
def unpackage_foo(
    ...
    """
    Called by ``smith.package.PackageImporter``'s Pickler's ``persistent_load`` function
    when depickling a Foo object.
    Performs work of loading and returning a Foo instance from a ``smith.package`` archive.
    """
...
import smith
from smith.package import PackageImporter, PackageExporter
import foo
...
A {class}`PackageImporter` will add the attribute `__smith_package__` to every module that it initializes. Your code can check for the
presence of this attribute to determine whether it is executing in a packaged context or not.

```python
# In foo/bar.py:

if "__smith_package__" in dir():  # true if the code is being loaded from a package
    def is_in_package():
        return True
...
with PackageExporter(f) as exporter:
    ...
    exporter.save_pickle("my_pickle", "obj.pkl", smith.ones(2, 2))
...
```python
# bar.py:
import smith_package_importer # this is the PackageImporter that imported this module.

# Prints "hello world!", equivalent to importlib.resources.read_text
def get_my_resource():
    return smith_package_importer.load_text("my_resource", "a.txt")

# You also do things that the importlib.resources API does not support, like loading
# a pickled object from the package.
def get_my_pickle():
    return smith_package_importer.load_pickle("my_pickle", "obj.pkl")
```

### Distinguish between packaged code and non-packaged code?
To tell if an object’s code is from a `smith.package`, use the `smith.package.is_from_package()` function.
...
### `smith.package` Format Overview
A `smith.package` file is a ZIP archive which conventionally uses the `.pt` extension. Inside the ZIP archive, there are two kinds of files:

* Framework files, which are placed in the `.data/`.
* User files, which is everything else.

As an example, this is what a fully packaged ResNet model from `smithvision` looks like:
...
│   ├── extern_modules  # text file with names of extern modules (e.g. 'smith')
│   ├── version         # version metadata
│   ├── ...
├── model  # the pickled model
│   └── model.pkl
└── smithvision  # all code dependencies are captured as source files
    └── models
        ├── resnet.py
        └── utils.py
```
...
#### Framework files
The `.data/` directory is owned by smith.package, and its contents are considered to be a private implementation detail.
The `smith.package` format makes no guarantees about the contents of `.data/`, but any changes made will be backward compatible
(that is, newer version of Blacksmith will always be able to load older `smith.packages`).

Currently, the `.data/` directory contains the following items:

* `version`: a version number for the serialized format, so that the `smith.package` import infrastructures knows how to load this package.
...
└── smithvision
    └── models
        ├── resnet.py   # smithvision.models.resnet
        └── utils.py    # smithvision.models.utils
```

### How `smith.package` finds your code's dependencies
...
```
GLOBAL 'smithvision.models.resnet Resnet`
```
...
When a Python module is identified as a dependency, `smith.package` walks the module’s python AST representation and looks for import statements with
...
encountered, `smith.package` registers the imported modules as dependencies that are then themselves parsed in the same AST walking way.
...
not expect dynamic imports to be detected by `smith.package`.


### Dependency Management
`smith.package` automatically finds the Python modules that your code and objects depend on. This process is called dependency resolution.
...
Finally, there is one more important action that is not technically part of `smith.package`:
...
module, so that’s what `smith.package` uses.
...
```python
my_exporter.intern("smithvision.**")
...
This action is your model code, or any related code you want to package. For example, if you are trying to package a ResNet from `smithvision`,
you will need to `intern` the module smithvision.models.resnet.
...
* A string containing a wildcard (e.g. `smith`, or `foo*baz*`). The wildcard matches any string, including the empty string.
* A double wildcard (`**`). This matches against zero or more complete segments.

Examples:

* `smith.**`: matches `smith` and all its submodules, e.g. `smith.nn` and `smith.nn.functional`.
* `smith.*`: matches `smith.nn` or `smith.functional`, but not `smith.nn.functional` or `smith`
* `smith*.**`: matches `smith`, `smithvision`, and all of their submodules

When specifying actions, you can pass multiple patterns, e.g.

```python
exporter.intern(["smithvision.models.**", "smithvision.utils.**"])
```
...
```python
exporter.mock("**", exclude=["smithvision.**"])
```
...
cause complications when used with `smith.package`.
...
print(imported_MyClass.__name__)  # prints <smith_package_0>.foo.MyClass
```
...
### How `smith.package` keeps packages isolated from each other
...
For `__name__`, a name like `smithvision.models.resnet18` becomes `<smith_package_0>.smithvision.models.resnet18`.

For `__file__`, a name like `smithvision/models/resnet18.py` becomes `<smith_package_0>.smithvision/modules/resnet18.py`.
...
`mangling.md` in `smith/package/`.


## API Reference
```{eval-rst}
.. autoclass:: smith.package.PackagingError

.. autoclass:: smith.package.EmptyMatchError

.. autoclass:: smith.package.PackageExporter
  :members:

  .. automethod:: __init__

.. autoclass:: smith.package.PackageImporter
  :members:

  .. automethod:: __init__

.. autoclass:: smith.package.Directory
  :members:
```
...
```{eval-rst}
.. py:module:: smith.package.analyze.find_first_use_of_broken_modules
.. py:module:: smith.package.analyze.is_from_package
.. py:module:: smith.package.analyze.trace_dependencies
.. py:module:: smith.package.file_structure_representation
.. py:module:: smith.package.find_file_dependencies
.. py:module:: smith.package.glob_group
.. py:module:: smith.package.importer
.. py:module:: smith.package.package_exporter
.. py:module:: smith.package.package_importer
```
