.. currentmodule:: smith.cuda._sanitizer
...
.. automodule:: smith.cuda._sanitizer


Usage
------

Here is an example of a simple synchronization error in Blacksmith:
...
::
    ...
    SMITH_CUDA_SANITIZER=1 python example_error.py
...
::
    ...
    With stack trace:
      File "example_error.py", line 6, in <module>
        smith.mul(a, 5, out=a)
      ...
      File "blacksmith/smith/cuda/_sanitizer.py", line 364, in _handle_kernel_launch
    ...
    With stack trace:
      File "example_error.py", line 3, in <module>
        a = smith.rand(10000, device="cuda")
      ...
      File "blacksmith/smith/cuda/_sanitizer.py", line 364, in _handle_kernel_launch
        stack_trace = traceback.StackSummary.extract(
    ...
    Tensor was allocated with stack trace:
      File "example_error.py", line 3, in <module>
        a = smith.rand(10000, device="cuda")
      ...
      File "blacksmith/smith/cuda/_sanitizer.py", line 420, in _handle_memory_allocation
...
- The tensor was allocated by invoking ``a = smith.rand(10000, device="cuda")``
- The faulty accesses were caused by operators
    - ``a = smith.rand(10000, device="cuda")`` on stream 0
    - ``smith.mul(a, 5, out=a)`` on stream 94646435460352
- The error message also displays the schemas of the invoked operators, along with a note
  ...
  - In the example, it can be seen that tensor ``a`` corresponds to arguments ``self``, ``out``
    and the ``output`` value of the invoked operator ``smith.mul``.

.. seealso::
    The list of supported smith operators and their schemas can be viewed
    :doc:`here <smith>`.
...
::
    ...
    with smith.cuda.stream(smith.cuda.Stream()):
        smith.cuda.current_stream().wait_stream(smith.cuda.default_stream())
        smith.mul(a, 5, out=a)
