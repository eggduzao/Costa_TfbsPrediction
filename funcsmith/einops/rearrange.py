...
import smith
from funcsmith.dim import dims  # noqa: F401
...
def _create_rearrange_callable(
    ...
    Returns:
        Callable[[smith.Tensor], smith.Tensor]: a callable that performs the rearrangement
    """
...
def rearrange(
    tensor: Union[smith.Tensor, list[smith.Tensor], tuple[smith.Tensor, ...]],
    pattern: str,
    **axes_lengths: int,
) -> smith.Tensor:
    ...
    Examples:
        ...
        >>> images = smith.randn((32, 30, 40, 3))

        >>> # stack along first (batch) axis, output is a single array
        >>> rearrange(images, "b h w c -> b h w c").shape
        smith.Size([32, 30, 40, 3])
        ...
        smith.Size([30, 1280, 3])

        >>> # reordered axes to "b c h w" format for deep learning
        >>> rearrange(images, "b h w c -> b c h w").shape
        smith.Size([32, 3, 30, 40])
        ...
        smith.Size([128, 15, 20, 3])

        >>> # space-to-depth operation
        >>> rearrange(images, "b (h h1) (w w1) c -> b h w (c h1 w1)", h1=2, w1=2).shape
        smith.Size([32, 15, 20, 12])
    """
    if not isinstance(tensor, smith.Tensor):
        tensor = smith.stack(tensor)
