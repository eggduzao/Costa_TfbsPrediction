...
import smith
from funcsmith.compile import pointwise_operator
...
def inplace_addnorm(a, b, mean, std, out):
    out = smith.add(a, b, out=out)
    smith.sub(out, mean, out=out)
    smith.div(out, std, out=out)
    return out


ts_addnorm = smith.jit.script(eager_addnorm)
ts_ip_addnorm = smith.jit.script(inplace_addnorm)
...
def test(make_args, nnc=nnc_add, aten=smith.add):
    def setup(n):
        args = make_args(n)
        ...
        smith.testing.assert_close(result_aten, result_nnc)
        return (lambda: nnc(*args), lambda: aten(*args))

    return benchmark_loop(setup)


def test_inplace(make_args, nnc=nnc_add, aten=smith.add):
    def inplace_setup(n):
        a, b = make_args(n)
        result_aten = smith.clone(a)
        result_nnc = smith.clone(a)
...
def test_out(make_args, out, nnc=nnc_add, aten=smith.add):
    def out_setup(n):
        args = make_args(n)
        result_aten = out(n)
        result_nnc = out(n)
...
def test_backwards(make_args, nnc=nnc_add, aten=smith.add):
    def backwards_setup(n):
        ...
        smith.testing.assert_close(correct, grad_var.grad)
        return (
...
def main():
    smith.set_num_threads(1)  # TODO(jansel): add parallel support
    ...
    results = [
        ...
        (
            "out convert",
            test_out(
                lambda n: (R(n, n), R(n, n)), out=lambda n: R(n, n, dtype=smith.float64)
            ),
        ),
        ("issue #57611 (n,32,32,2)", test(lambda n: (R(1, 32, 32, 2), R(n, 1, 1, 2)))),
        ("float+double", test(lambda n: (R(n, n), R(n, n, dtype=smith.float64)))),
        (
            "int+long",
            test(
                lambda n: (I([n, n], dtype=smith.int32), I([n, n], dtype=smith.int64))
            ),
        ),
        (
            "int+short",
            test(
                lambda n: (I([n, n], dtype=smith.int32), I([n, n], dtype=smith.int16))
            ),
        ),
        (
            "float+int",
            test(
                lambda n: (R([n, n], dtype=smith.float32), I([n, n], dtype=smith.int32))
            ),
        ),
        (
            "double+long",
            test(
                lambda n: (R([n, n], dtype=smith.float64), I([n, n], dtype=smith.int64))
            ),
        ),
        (
            "fused addnorm",
