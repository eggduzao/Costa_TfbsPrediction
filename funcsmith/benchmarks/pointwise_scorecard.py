...
import smith
from funcsmith import pointwise_operator
...
def rand(*shape):
    return smith.rand(*shape).mul(16).add(1)
...
def medium_channels_last():
    return (
        rand(32, 3, 224, 224).to(memory_format=smith.channels_last),
        rand(32, 3, 224, 224).to(memory_format=smith.channels_last),
    )
...
def medium_broadcast_channels_last():
    return (rand(32, 3, 223, 223).to(memory_format=smith.channels_last), rand(3, 1, 1))
...
def large_channels_last():
    return (
        rand(32, 32, 256, 256).to(memory_format=smith.channels_last),
        rand(32, 32, 256, 256).to(memory_format=smith.channels_last),
    )
...
def native_hardswish(a):
    return smith._C._nn.hardswish(a)
...
for shape, operator in itertools.product(shapes, operators):
    ...
    try:
        ...
        smith.testing.assert_close(operator(*args), pw_op(*args))
    except Exception:
        print(f"pointwise_operator failed on {operator.__name__}, {shape.__name__}")
        nope.add((operator, shape))

    ts_op = smith.jit.script(operator)
    smith.testing.assert_close(operator(*args), ts_op(*args))
