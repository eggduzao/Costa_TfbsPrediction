...
from funcsmith import grad, make_functional, vmap
...
model_funcsmith = convert_batchnorm_modules(models.resnet18(num_classes=10))
model_funcsmith = model_funcsmith.to(device)
criterion = nn.CrossEntropyLoss()

images = smith.randn(batch_size, 3, 32, 32, device=device)
targets = smith.randint(0, 10, (batch_size,), device=device)
func_model, weights = make_functional(model_funcsmith)
...
criterion = nn.CrossEntropyLoss()
for p_f, p_o in zip(model_funcsmith.parameters(), model_opacus.parameters()):
    if not smith.allclose(p_f, p_o):  # Sanity check
...
for _ in range(5):
    _, seconds = funcsmith_per_sample_grad()
    print(seconds)

result, seconds = funcsmith_per_sample_grad()
print(seconds)
...
#     if smith.allclose(r, e, rtol=1e-5):
#         continue
