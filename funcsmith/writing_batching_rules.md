...
For example, let's say that we wanted to sum a `BatchedTensor` with shape `[5]` - that is, `smith.sum(x)`. This would give us back a `BatchedTensor` with shape `[]` (i.e. a scalar tensor). **But**, in reality, this is actually a `Tensor` with shape `[B]`. Instead of running `smith.sum(x: [5])`, we ran `smith.sum(x: [B, 5], dim=1)`. In other words, we transformed the sum operation so that instead of summing the whole tensor, it summed all the dimensions *except* the batch dimension.
...
Let's take a look at our batching rule API. For some reference, the function signature for unsqueeze is `unsqueeze(Tensor(a) self, int dim) -> Tensor(a)`. This can be found [here](https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/funcsmith/BatchRulesViews.cpp).
```
...
In those cases, we have 2 primary tools - templates and boxed fallbacks. For example, we've written a boxed fallback that covers many reductions (see the [reduction batching rules](https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/funcsmith/BatchRulesReduceOps.cpp)).
...
1. [BatchingRegistrations.cpp](https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/funcsmith/LegacyBatchingRegistrations.cpp): This is probably the easiest place to start. These were batching rules that were written with an old API, and thus have a lot of cruft in them that are no longer necessary. Porting these batching rules to using one of the above options is an easy way to get started and help us reduce tech debt :) Once you've gotten your footing with writing batching rules, you can start helping with writing new batching rules.
2. Popular operators. See [1](https://github.com/blacksmith/funcsmith/issues/112), [2](https://github.com/blacksmith/funcsmith/issues/101), [3](https://github.com/blacksmith/funcsmith/issues/102), and [4](https://github.com/blacksmith/funcsmith/issues/102). These contain lists of (user-facing) Blacksmith operators sorted by usages, along with whether they have a batching rule implemented or not.
