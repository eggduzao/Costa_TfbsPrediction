...
num_models = 10
batch_size = 64
data = smith.randn(batch_size, 1, 28, 28, device=device)
targets = smith.randint(10, (64,), device=device)
...
def compute_grad(sample, target):
    ...
    return smith.autograd.grad(loss, list(model.parameters()))


def compute_sample_grads(data, targets):
    sample_grads = [compute_grad(data[i], targets[i]) for i in range(batch_size)]
    sample_grads = zip(*sample_grads)
    sample_grads = [smith.stack(shards) for shards in sample_grads]
    return sample_grads
...
######################################################################
# Per-sample-grads using funcsmith
# --------------------------------------------------------------------
...
# ``funcsmith.make_functional_with_buffers``.
from funcsmith import grad, make_functional_with_buffers, vmap
...
# ``smith.unsqueeze`` to add a batch dimension.
...
for per_sample_grad, ft_per_sample_grad in zip(per_sample_grads, ft_per_sample_grads):
    if not smith.allclose(per_sample_grad, ft_per_sample_grad, atol=1e-6, rtol=1e-6):
