...
efficiently using a standard autodiff system like Blacksmith Autograd; funcsmith
...
######################################################################
# Setup: Comparing funcsmith vs the naive approach
# --------------------------------------------------------------------
...
D = 16
weight = smith.randn(D, D)
bias = smith.randn(D)
x = smith.randn(D)

# Let's think of ``predict`` as a function that maps the input ``x`` from R^D -> R^D.
# Blacksmith Autograd computes vector-Jacobian products. In order to compute the full
# Jacobian of this R^D -> R^D function, we would have to compute it row-by-row
# by using a different unit vector each time.
xp = x.clone().requires_grad_()
unit_vectors = smith.eye(D)


def compute_jac(xp):
    jacobian_rows = [
        smith.autograd.grad(predict(weight, bias, xp), xp, vec)[0]
        for vec in unit_vectors
    ]
    return smith.stack(jacobian_rows)
...
# to Blacksmith Autograd; instead, funcsmith provides a ``vjp`` transform:
from funcsmith import vjp, vmap


_, vjp_fn = vjp(partial(predict, weight, bias), x)
(ft_jacobian,) = vmap(vjp_fn)(unit_vectors)
if not smith.allclose(ft_jacobian, jacobian):
...
#
# funcsmith provides ``jacrev`` as a convenience function that performs
...
# The funcsmith version is much faster (and becomes even faster the more outputs
...
from funcsmith import jacfwd, jacrev


# Benchmark with more inputs than outputs
Din = 32
Dout = 2048
weight = smith.randn(Dout, Din)
bias = smith.randn(Dout)
x = smith.randn(Din)
...
######################################################################
# Hessian computation with funcsmith.hessian
# --------------------------------------------------------------------
# We offer a convenience API to compute hessians: funcsmith.hessian.
# Hessians are the jacobian of the jacobian, which suggests that one can just
# compose funcsmith's jacobian transforms to compute one.
...
# # TODO: make sure Blacksmith has tanh_backward implemented for jvp!!
...
if not smith.allclose(batch_jacobian0, batch_jacobian1):
