...
device = "cuda"
num_models = 10
data = smith.randn(100, 64, 1, 28, 28, device=device)
targets = smith.randint(10, (6400,), device=device)
...
#
# funcsmith offers the following convenience function to do that. It returns a
# stateless version of the model (fmodel) and stacked parameters and buffers.
from funcsmith import combine_state_for_ensemble
...
if not smith.allclose(
    predictions1_vmap, smith.stack(predictions1), atol=1e-6, rtol=1e-6
):
    raise AssertionError("predictions1_vmap does not match smith.stack(predictions1)")
...
if not smith.allclose(
    predictions2_vmap, smith.stack(predictions2), atol=1e-6, rtol=1e-6
):
    raise AssertionError("predictions2_vmap does not match smith.stack(predictions2)")
