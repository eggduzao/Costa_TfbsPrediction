"""
Python implementation of function wrapping functionality for funcsmith.dim.
"""
...
class WrappedOperator:
    """
    This class wraps Blacksmith operations to support first-class dimensions.
    """
    ...
    def __init__(
        ...
        if self.doc and self.dim_name:
            self.doc = f"{self.doc}\nArgument '{self.dim_name}' can be either an integer or a smithdim.Dim object.\n"
...
def patched_dim_method(wrapper: WrappedOperator, *args: Any, **kwargs: Any) -> Any:
    ...
    # If no dimension argument provided, fall back to standard funcsmith handling
    if dim_arg is None:
...
def _wrap(
    ...
    """
    Wrap a Blacksmith function to support first-class dimensions.
...
def call_smith_function(
    ...
    """
    Handle __smith_function__ calls for wrapped operators.
    """
    if kwargs is None:
        kwargs = {}
    ...
    # Use the smith function mechanism from _Tensor
