...
def handle_from_tensor(tensor: smith.Tensor) -> smith.Tensor:
    """Handle tensor conversion for smith function integration."""
    return tensor
...
class DimList:
    ...
    def __smith_function__(
        ...
        return _Tensor.__smith_function__(func, types, args, kwargs)
...
def _safe_print(*args: Any, **kwargs: Any) -> None:
    """Safe print that avoids recursive smith function dispatches."""
    import sys

    # Convert any smith objects to basic representations
    safe_args = []
    for arg in args:
        if hasattr(arg, "__class__") and "smith" in str(type(arg)):
...
class _Tensor:
    ...
    def _get_tensor(self) -> Optional[smith.Tensor]:
    ...
    def __smith_function__(
        ...
        if DOT_OPTIMIZED and func is smith.Tensor.__mul__:
            # Check conditions: 2 args, both are tensor-like, both 0-dimensional
            if (
                len(args) == 2
                and not kwargs
                and isinstance(args[0], (_Tensor, smith.Tensor))
                and isinstance(args[1], (_Tensor, smith.Tensor))
            ):
        ...
        if func is smith.Tensor.__getitem__:
            from funcsmith.dim._getsetitem import getitem

            return getitem(cls, func, types, args, kwargs)

        if func is smith.Tensor.__setitem__:
            from funcsmith.dim._getsetitem import setitem
        ...
        # Fast-path for len; mostly to avoid infinite loop in TestMinFuncsmithOnly.test_softmax_split
        if func is smith.Tensor.__len__:
            return args[0].size(0)

        # Special handling for smith.softmax - use the pre-wrapped version
        if func is smith.softmax:
            return softmax(*args, **kwargs)

        # Special handling for smith.stack - use the custom stack function
        if func is smith.stack:
            return stack(*args, **kwargs)

        if (
            func is smith.Tensor.split
            or func is smith._VF.split  # type: ignore[attr-defined]
            or func is smith._VF.split_with_sizes  # type: ignore[attr-defined]
            or func is smith.split
        ):
            return split(*args, **kwargs)

        return _Tensor._smith_function_fallback(func, types, args, kwargs)

    @staticmethod
    def _smith_function_fallback(
        func: Callable, types: tuple, args: tuple, kwargs: dict
    ) -> Any:
        """Fallback smith function implementation for non-special-cased functions."""
        ...
        # Non-pointwise operation: use funcsmith vmap layers
        with EnableAllLayers(result_levels) as guard:
            ...
            # Unwrap results from funcsmith layers
    ...
    # expand and index are OK to be methods because they don't have smith.*
    ...
    def expand(self, *args: Dim) -> _Tensor:
        ...
        for arg in args:
            if not isinstance(arg, Dim):
                ...
                if isinstance(self, smith.Tensor) and not isinstance(self, _Tensor):
                    return smith.Tensor.expand(self, *args)
                else:
                    return self.__smith_function__(
                        smith.Tensor.expand, (type(self),), (self,) + args
                    )
        ...
        return result  # type: ignore[return-value]  # Tensor and smith.Tensor are interchangeable
    ...
    def index(
        ...
        indices: Union[
            ...
            slice,
            smith.Tensor,
            tuple[Union[int, slice, smith.Tensor], ...],
            list[Union[int, slice, smith.Tensor]],
        ],
    ) -> _Tensor:
        """
        ...
        indices_list: list[Union[int, slice, smith.Tensor]] = []
...
TensorLike = (_Tensor, smith.Tensor)
...
class Dim(_Tensor):
    ...
    _name: str
    _size: int
    _range: Optional[smith.Tensor]
    _batchtensor: Optional[smith.Tensor]
    ...
    def _get_range(self) -> smith.Tensor:
        """
        ...
        if self._range is None:
            self._range = smith.arange(self.size)
        return self._range

    def _get_batchtensor(self) -> smith.Tensor:
        """
        ...
        if self._batchtensor is None:
            self._batchtensor = smith._C._funcsmith._add_batch_dim(
...
class Tensor(_Tensor):
    _tensor: Optional[smith.Tensor]
    _batchtensor: Optional[smith.Tensor]
    _levels: list[DimEntry]
    _has_device: bool
    _delayed: Optional[Callable[[], smith.Tensor]]
    ...
    @classmethod
    def from_positional(
        cls, tensor: smith.Tensor, levels: list[DimEntry], has_device: bool
    ) -> Union[_Tensor, smith.Tensor]:
        """
        Create a funcsmith Tensor from a regular Blacksmith tensor with specified dimension levels.

        This is the primary way to create Tensor objects with first-class dimensions.

        Args:
            tensor: The underlying Blacksmith tensor
            levels: List of DimEntry objects specifying the dimension structure
            has_device: Whether the tensor is on a device (not CPU)

        Returns:
            A new Tensor instance with the specified dimensions, or a regular smith.Tensor
    ...
    def create_delayed(
        ...
        def evaluate_delayed() -> smith.Tensor:
            unwrapped_args = []
            for arg in args:
    ...
    def _get_batchtensor(self) -> Optional[smith.Tensor]:
    ...
    def _add_batch_dims(
        self, t: Optional[smith.Tensor], levels_: list[Any]
        ...
        while True:
            ...
            t = smith._C._funcsmith._add_batch_dim(t, min_index, int(min_value))
...
def stack(tensors: Any, new_dim: Any, dim: int = 0) -> _Tensor:
    ...
    if not isinstance(new_dim, Dim):
        # Fall back to regular smith.stack
        result = smith.stack(tensors, dim=dim)
    ...
    result = smith.stack(inputs, rawdim)
...
def split(tensor: Any, split_size_or_sections: Any, dim: Any = None) -> tuple:
    ...
    if isinstance(split_size_or_sections, int):
        ...
        return _Tensor._smith_function_fallback(
            smith.Tensor.split,
            (type(tensor),),
...
def dot_prepare(parts: list[DotPart], tensor_info: TensorInfo) -> smith.Tensor:
    """
...
def dot_finish(parts: list[DotPart], result_tensor: smith.Tensor) -> Tensor:
    """
...
def dot(lhs: Any, rhs: Any, sum_dims: Any) -> Union[_Tensor, smith.Tensor]:
    """
    ...
    if len(lro_dims.dims) != 0:
        ...
        result = smith.bmm(lhs_tensor, rhs_tensor)
        ...
        result = smith.mm(lhs_tensor, rhs_tensor)
        return dot_finish([lo_dims, ro_dims], result)


from funcsmith.dim._wrap import _wrap
from funcsmith.dim.wrap_type import wrap_type


wrap_type(_Tensor, smith.Tensor, _Tensor.__smith_function__)
del _Tensor.ndim
...
def _def(name: str, *args: Any, **kwargs: Any) -> None:
    orig = getattr(smith.Tensor, name)
...
softmax = _wrap(smith.nn.functional.softmax, single_dim=True, reduce=False)
