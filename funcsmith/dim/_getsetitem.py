...
class IndexingInfo:
    ...
    self_tensor: Optional[smith.Tensor] = None
...
def has_dims(obj: Any) -> bool:
    ...
    This function checks if the object is either a Dim or a funcsmith Tensor
...
def extractIndices(index: Any, indices: list) -> bool:
    if isinstance(index, tuple):  # mpy::tuple_view::check
        ...
        return True
    elif isinstance(index, smith.Tensor):  # THPVariable_Check
    ...
    if isinstance(index, list):
        ...
        for item in index:
            if (
                isinstance(item, (smith.Tensor, slice))
...
def getitem(cls: Any, func: Any, types: Any, args: Any, kwargs: Any) -> Any:
    ...
    if iinfo.can_call_original:
        # Call original tensor __getitem__ directly, bypassing __smith_function__
        return smith.Tensor.__getitem__(self, index)
...
def setitem(self: Any, index: Any, rhs: Any) -> None:
    ...
    if iinfo.can_call_original:
        # Call original tensor __setitem__ directly, bypassing __smith_function__
        smith._C.TensorBase.__setitem__(self, index, rhs)
        return
    ...
    if iinfo.advanced_indexing:
        ...
        smith._C.TensorBase.__setitem__(iinfo.self_tensor, tup, matched_rhs)
    else:
...
def getsetitem_flat(
    ...
    def add_dim(dim: Any) -> None:
        # Use safe indexing to avoid triggering __smith_function__ on Dim objects
    ...
    for i, level in enumerate(self_info.levels):
        # Use safe indexing to avoid triggering __smith_function__ on DimEntry comparisons
    ...
    for i, inp in enumerate(flat_inputs):
        if tensor_inputs[i] is not None:
            ...
            d = inp
            # Use safe indexing to avoid triggering __smith_function__
