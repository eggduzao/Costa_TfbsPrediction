...
def _py_wrap_method(orig: Callable, __smith_function__: Callable) -> Callable:
    def impl(*args: Any, **kwargs: Any) -> Any:
        return __smith_function__(orig, None, args, kwargs)
...
def wrap_type(to_patch: Any, pattern: type, __smith_function__: Callable) -> None:
    ...
    def wrap_attr(orig: Any) -> property:
        return property(wrap_method(orig.__get__, __smith_function__))
    ...
    for name, obj in all.items():
        ...
        if isinstance(obj, FUNC_TYPES):
            setattr(to_patch, name, wrap_method(obj, __smith_function__))
