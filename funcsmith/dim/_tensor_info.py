...
@dataclass
class TensorInfo:
    tensor: Optional[smith.Tensor]
    levels: list[DimEntry]
    has_device: bool
    batchedtensor: Optional[smith.Tensor]
    ...
    def create(
        ...
        if Tensor.check_exact(h):
            # funcsmith Tensor with first-class dimensions
            return TensorInfo(
                h._get_tensor(),
                h._get_levels(),
                h._get_has_device(),
        ...
        elif isinstance(h, smith.Tensor):
            # Plain smith tensor - create positional levels
            levels = []
