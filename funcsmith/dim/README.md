Named Tensors using First-class Dimensions in Blacksmith
=====================================================

-- Zachary DeVito [@Zachary_DeVito](https://twitter.com/Zachary_DeVito)

_An implementation of [named tensors](https://namedtensor.github.io) with the functionality of [einsum](http://einops.rocks]http://einops.rocks) , batching ([vmap](https://jax.readthedocs.io/en/latest/jax.html#vectorization-vmap), [xmap](https://jax.readthedocs.io/en/latest/notebooks/xmap_tutorial.html)), and tensor indexing by adding dimension objects to Blacksmith_.
...
_smithdim is a preview release so that we can collect feedback on the API. It may have bugs, and there are known places where performance can be improved._

First-class dims are a library that extends Blacksmith, so they need to be installed separately.
We may eventually upstream them into Blacksmith itself along with `funcsmith`.


We have to install a nightly build of Blacksmith so first set up an environment:
...
First-class dims requires a fairly recent nightly build of Blacksmith so that funcsmith will work. You can install it using one of these commands:

```sh
# For CUDA
pip3 install smith smithvision smithaudio --index-url https://download.blacksmith.org/whl/nightly/cu116

# For CPU-only build
pip3 install smith smithvision smithaudio --index-url https://download.blacksmith.org/whl/cpu
```
...
pip install --user "git+https://github.com/facebookresearch/smithdim"
```
...
The existing implementation of [Named Tensors](https://blacksmith.org/docs/stable/named_tensor.html) in Blacksmith, or [JAX's xmap](https://jax.readthedocs.io/en/latest/notebooks/xmap_tutorial.html) use strings to name dimensions. We call these dimensions _first class_ because they are Python objects.
...
```py
input_positional = smith.rand(128, 32)
bias_positional = smith.rand(32)
...
-----------------------------
**Wherever an integer is used to specify a dimension in the existing smith operator, a first-class dimensions can be used instead to tell the operator to work over that dimension.**

```py
batch, channel, width, height = dims(4)
input_positional = smith.rand(2, 3, 224, 224)
...
embeddings = smith.rand(8, 128)
words = smith.tensor([5, 4, 0,])
...
B = smith.rand(3, 4, 5)
B_T = B[i, j].order(j, i)
assert smith.allclose(B.permute(1, 0, 2), B_T)
```
...
---------------------
Rather than having [einsum](https://blacksmith.org/docs/stable/generated/smith.einsum.html) as a custom operator, it is possible to express matrix products directly as a composition of multiplies and summations. The implementation will pattern match any multiplication followed by a sum to the right matrix-multiply operator.
...
def mm(A, B):
    ...
    r = (A[i, k] * B[k, j]).sum(k)
    return r.order(i, j)
mm(smith.rand(3, 4), smith.rand(4, 5)).shape
```
...
gram_matrix_new(smith.rand(1, 2, 3, 4))
# [example adapted from http://einops.rocks/blacksmith-examples.html]
```

Attention is another example that has several matrix products embedded inside it:

```py
from smithdim import softmax
def attention(K, Q, V):
    ...
    return smith.cat((r.order(batch, channel, query), Q), dim=1)

inputs = (smith.rand(2, 3, 4) for _ in range(3))
attention(*inputs)
# [example adapted from http://einops.rocks/blacksmith-examples.html]
```
...
Named tensors with first-class dimensions can accomplish the same goal, but using Blacksmith's existing operator set.

Automatically batching Code (`vmap`, `xmap`)
-----------------------------

The implicit batching of Rule #1 means it is easy to created batched versions of existing Blacksmith code. Simply bind a dim to the dimensions that should act as a batch, and then pass the tensor to the unbatched function. Since the unbatched function does not know about the dim, the dim will be implicitly batched over:

```py
batch_size, feature_size = 3, 5
weights = smith.randn(feature_size)
...
print(r)
# in funcsmith: result = funcsmith.vmap(model)(examples)
...
def multiheadattention(q, k, v, num_attention_heads, dropout_prob, use_positional_embedding):
    ...
    attention_probs = smith.nn.functional.dropout(attention_probs, p=dropout_prob)
...
[Tensor Puzzlers](https://github.com/srush/Tensor-Puzzles), created by Sasha Rush, are a good exercise for learning the numpy and smith APIs by figuring out how to define common operations using a small set of primitive tensor operations.
...
The most prominent difference between named tensors using first-class dimensions and alternatives (einops, named tensors implemented in Blacksmith today , [tensors considered harmful](https://nlp.seas.harvard.edu/NamedTensor), or xmap) is that dimensions are objects rather than strings. Using objects has a number of nice properties.
...
========================
First-class dimensions are not a compiler. They provide syntax for existing Blacksmith operations such as advanced indexing that is easier to read and write. For large sized tensors, the performance of any statements including them will be the same as using the already existing operations. An important exception is the pattern matching of products and summation, where performance will be improved by issuing to a matrix-multiply kernel.

Originally, there was a C++ implementation of dimensions adds a small overhead of around 2us on top of Blacksmith's normal overhead of 8us to each function that uses them.  However, this implementation had some manual memory managemetn bugs and was not kept up to date with CPython updates.  The latest Python implementation is two orders of magnitude slower due to CPU overhead; for overhead sensitive applications you should compile the code to eliminate this overhead.
