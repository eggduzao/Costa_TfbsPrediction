...
class EnableAllLayers:
    """
    RAII-style context manager for enabling funcsmith vmap layers.
    It manages the creation and cleanup of funcsmith dynamic layers.
    ...
    enable all those layers so funcsmith can actually apply the batching
    ...
    def __enter__(self) -> EnableAllLayers:  # noqa: PYI034
        # Create funcsmith dynamic layers
        for i, dim in enumerate(self.levels_to_dim):
            batch_size = dim.size
            level = smith._C._funcsmith._vmap_increment_nesting(batch_size, "different")
            if i == 0:
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        for i in range(len(self.levels_to_dim)):
            popped = smith._C._funcsmith._vmap_decrement_nesting()
            if popped != to_remove - i:
                raise AssertionError(f"Expected layer {to_remove - i}, got {popped}")

    def from_batched(self, batchedtensor: smith.Tensor, has_device: bool) -> Tensor:
        """
        Create a Tensor from a batched tensor by unwrapping funcsmith layers.

        Args:
            batchedtensor: Batched tensor from funcsmith operation
        ...
        while smith._C._funcsmith.is_batchedtensor(tensor):
            ...
            tensor = smith._C._funcsmith.get_unwrapped(tensor)
    ...
    def inplace_update_layers(
        self, batchtensor: smith.Tensor, levels: list[DimEntry]
    ) -> None:
        """
        Update the levels of a batched tensor in place.

        This requires the _maybe_unsafe_set_level binding that we'll add to funcsmith.
        ...
        if not smith._C._funcsmith.is_batchedtensor(batchtensor):
            return
        ...
        for i in reversed(range(len(self.levels_to_dim))):
            ...
            if any(l == DimEntry(self.levels_to_dim[i]) for l in levels):
                ...
                smith._C._funcsmith._maybe_unsafe_set_level(impl, self.levels_start + i)
                impl = smith._C._funcsmith.get_unwrapped(impl)
