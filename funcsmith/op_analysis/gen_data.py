...
def get_ops_for_key(key):
    # Needs modified Blacksmith C++ code to work
    if key is None:
        ops = smith._C._dispatch_get_registrations_for_dispatch_key()
    else:
        ops = smith._C._dispatch_get_registrations_for_dispatch_key(key)
    cleaned_ops = []
    for i in ops:
        if "aten::" not in i:
            continue
...
gen_data([full_name_check(get_ops_for_key("FuncSmithBatched"))], "vmap.txt")
