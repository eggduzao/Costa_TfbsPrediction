...
# We translated his implementation from JAX to Blacksmith.
...
params = [
    smith.Tensor(40, 1).uniform_(-1.0, 1.0).requires_grad_(),
    smith.Tensor(40).zero_().requires_grad_(),
    smith.Tensor(40, 40)
    ...
    .uniform_(-1.0 / math.sqrt(40), 1.0 / math.sqrt(40))
    .requires_grad_(),
    smith.Tensor(1).zero_().requires_grad_(),
]
...
opt = smith.optim.Adam(params, lr=1e-3)
alpha = 0.1
...
def sample_tasks(outer_batch_size, inner_batch_size):
    ...
    def get_batch():
        ...
        for A, phase in zip(As, phases):
            ...
            xs.append(x)
            ys.append(y)
        return smith.tensor(xs, dtype=smith.float), smith.tensor(ys, dtype=smith.float)
...
t_A = smith.tensor(0.0).uniform_(0.1, 0.5)
t_b = smith.tensor(0.0).uniform_(0.0, math.pi)

t_x = smith.empty(4, 1).uniform_(-5, 5)
t_y = t_A * smith.sin(t_x + t_b)
...
for k in range(5):
    ...
    grads = smith.autograd.grad(t_loss, t_params, create_graph=True)
    t_params = [(t_params[i] - alpha * grads[i]) for i in range(len(params))]


test_x = smith.arange(-2 * math.pi, 2 * math.pi, step=0.01).unsqueeze(1)
test_y = t_A * smith.sin(test_x + t_b)
