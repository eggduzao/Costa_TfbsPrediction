...
from smith.func import functional_call, grad_and_value, stack_module_state, vmap
...
parser = argparse.ArgumentParser(description="Funcsmith Ensembled Models")
parser.add_argument(
    "--device",
    type=str,
    default="cpu",
...
def make_spirals(n_samples, noise_std=0.0, rotations=1.0):
    ts = smith.linspace(0, 1, n_samples, device=DEVICE)
    rs = ts**0.5
    thetas = rs * rotations * 2 * math.pi
    signs = smith.randint(0, 2, (n_samples,), device=DEVICE) * 2 - 1
    labels = (signs > 0).to(smith.long).to(DEVICE)

    xs = (
        rs * signs * smith.cos(thetas)
        + smith.randn(n_samples, device=DEVICE) * noise_std
    )
    ys = (
        rs * signs * smith.sin(thetas)
        + smith.randn(n_samples, device=DEVICE) * noise_std
    )
...
def train_step_fn(weights, batch, targets, lr=0.2):
    ...
    # NB: Blacksmith is missing a "functional optimizer API" (possibly coming soon)
...
# does, we used the following additional items that Blacksmith does not have:
...
# 4. Composability between the functional grad API and smith.vmap.
