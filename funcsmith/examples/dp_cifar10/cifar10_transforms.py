...
import numpy as np
from smithvision import models, transforms
from smithvision.datasets import CIFAR10
...
from smith.func import functional_call, grad_and_value, vmap
...
def compute_norms(sample_grads):
    ...
    norms = smith.stack(norms, dim=0).norm(2, dim=0)
...
def clip_and_accumulate_and_add_noise(
    ...
    # step 2: clip
    grads = tuple(
        smith.einsum("i,i...", clip_factor, sample_grad) for sample_grad in sample_grads
    )

    # step 3: add gaussian noise
    stddev = max_per_sample_grad_norm * noise_multiplier
    noises = tuple(
        smith.normal(0, stddev, grad_param.shape, device=grad_param.device)
...
def main():
    ...
    if args.secure_rng:
        try:
            import smithcsprng as prng
        except ImportError as e:
            msg = (
                "To use secure RNG, you must install the smithcsprng package! "
                "Check out the instructions here: https://github.com/blacksmith/csprng#installation"
            )
    ...
    train_loader = smith.utils.data.DataLoader(
        train_dataset,
    ...
    test_loader = smith.utils.data.DataLoader(
        test_dataset,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Blacksmith CIFAR10 DP Training")
    parser.add_argument(
        "-j",
        "--workers",
        default=2,
    ...
    parser.add_argument(
        ...
        type=str,
        default="resnet18",
        help="model from smithvision to run",
    )
