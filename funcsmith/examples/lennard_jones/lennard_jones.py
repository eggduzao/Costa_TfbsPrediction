...
from smith.nn.functional import mse_loss
...
training_size = 1000
r = smith.linspace(0.5, 2 * sigma, steps=training_size, requires_grad=True)

# Create a bunch of vectors that point along positive-x
drs = smith.outer(r, smith.tensor([1.0, 0, 0]))
...
optimiser = smith.optim.Adam(model.parameters(), lr=1e-3)
