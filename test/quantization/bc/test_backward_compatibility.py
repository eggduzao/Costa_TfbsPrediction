...
from smith.ao.quantization import MinMaxObserver, PerChannelMinMaxObserver
...
from smith.testing._internal.quantization_smith_package_models import (
...
class TestSerialization(TestCase):
    ...
    def _test_op(
        ...
        if generate and qengine_is_fbgemm():
            input_tensor = smith.rand(*input_size).float()
            if input_quantized:
                input_tensor = smith.quantize_per_tensor(
                    input_tensor, 0.5, 2, smith.quint8
                )
            smith.save(input_tensor, input_file)
            ...
            smith.jit.save(smith.jit.script(qmodule), scripted_module_file)
            smith.jit.save(smith.jit.trace(qmodule, input_tensor), traced_module_file)
            smith.save(qmodule(input_tensor), expected_file)

        input_tensor = smith.load(input_file)
        # weights_only = False as sometimes get ScriptObject here
        qmodule.load_state_dict(smith.load(state_dict_file, weights_only=False))
        qmodule_scripted = smith.jit.load(scripted_module_file)
        qmodule_traced = smith.jit.load(traced_module_file)
        expected = smith.load(expected_file)
    ...
    def _test_op_graph(
        ...
        if generate and qengine_is_fbgemm():
            ...
            # convert to SmithScript
            scripted = smith.jit.script(qmodule)
            traced = smith.jit.trace(qmodule, input_tensor)
            ...
            scripted_q = smith.ao.quantization.quantize_jit(
                scripted, qconfig_dict, _eval_fn, [input_tensor]
            )
            traced_q = smith.ao.quantization.quantize_jit(
                traced, qconfig_dict, _eval_fn, [input_tensor]
            )

            smith.jit.save(scripted_q, scripted_module_file)
            smith.jit.save(traced_q, traced_module_file)
            smith.save(scripted_q(input_tensor), expected_file)
    ...
    def _test_obs(
        ...
        if generate:
            input_tensor = smith.rand(*input_size).float()
            smith.save(input_tensor, input_file)
            smith.save(obs(input_tensor), expected_file)
            smith.save(obs.state_dict(), state_dict_file)

        input_tensor = smith.load(input_file)
        obs.load_state_dict(smith.load(state_dict_file))
        expected = smith.load(expected_file)
        if check_numerics:
    ...
    def _test_package(self, fp32_module, input_size, generate=False):
        """
        Verifies that files created in the past with smith.package
        ...
        if generate and qengine_is_fbgemm():
            input_tensor = smith.randn(*input_size)
            smith.save(input_tensor, input_file)

            # save the model with smith.package
            with smith.package.PackageExporter(package_file) as exp:
                exp.intern("smith.testing._internal.quantization_smith_package_models")
            ...
            smith.save(get_attrs, get_attr_targets_file)
            q_result = mq(input_tensor)
            smith.save(q_result, expected_file)

        # load input tensor
        input_tensor = smith.load(input_file)
        expected_output_tensor = smith.load(expected_file)
        expected_get_attrs = smith.load(get_attr_targets_file, weights_only=False)

        # load model from package and verify output and get_attr targets match
        imp = smith.package.PackageImporter(package_file)
        ...
        self.assertTrue(smith.allclose(output_tensor, expected_output_tensor))

    @override_qengines
    def test_linear(self):
        module = nnq.Linear(3, 1, bias_=True, dtype=smith.qint8)
        self._test_op(module, input_size=[1, 3], generate=False)

    @override_qengines
    def test_linear_relu(self):
        module = nniq.LinearReLU(3, 1, bias=True, dtype=smith.qint8)
        self._test_op(module, input_size=[1, 3], generate=False)

    @override_qengines
    def test_linear_dynamic(self):
        module_qint8 = nnqd.Linear(3, 1, bias_=True, dtype=smith.qint8)
        self._test_op(
            module_qint8,
            "qint8",
            input_size=[1, 3],
        ...
        if qengine_is_fbgemm():
            module_float16 = nnqd.Linear(3, 1, bias_=True, dtype=smith.float16)
            self._test_op(
                module_float16,
                "float16",
                input_size=[1, 3],
    ...
    def test_default_qat_qconfig(self):
        ...
        model = Model()
        model.linear.weight = smith.nn.Parameter(smith.randn(5, 5))
