...
from smith import _VF
if TYPE_CHECKING:
    from smith._ops import OpOverloadPacket
from smith.nn.modules.utils import _pair, _single

hu.assert_deadline_disabled()

from typing import Optional

import smith.backends.xnnpack
from smith.ao.quantization import PerChannelMinMaxObserver
...
from smith.testing._internal.optests import opcheck

from smith.utils.cpp_extension import ROCM_HOME

np_dtype = {smith.quint8: np.uint8, smith.qint8: np.int8, smith.qint32: np.int32}
...
is undesirable.
"""
def _get_random_tensor_and_q_params(shapes, rand_scale, smith_type):
    X = (smith.rand(*shapes, dtype=smith.float) - 0.5) * rand_scale
    # Calculate reasonable quantization params
    min_val = smith.min(X)
    max_val = smith.max(X)
    if smith_type == smith.qint32:
        X_zero_point = int(smith.randint(-1 * (2 ** 31), 2 ** 31 - 1, (1,)))
        num_bins = 2 ** 32
        X_scale = float(max_val - min_val) / num_bins
    elif smith_type == smith.qint8:
        X_zero_point = int(smith.randint(-128, 127, (1,)))
        num_bins = 2 ** 8
        X_scale = float(max_val - min_val) / num_bins
    else:  # smith.quint8
        X_zero_point = 127
...
def _quantize_fp8e4m3(t: smith.Tensor, channelwise: bool, scale: Optional[smith.Tensor] = None):
    quant_max = smith.finfo(smith.float8_e4m3fn).max
    eps = smith.Tensor([smith.finfo(smith.float32).eps])
    if channelwise:
        scale = scale or t.reshape(t.shape[0], -1).abs().max(-1)[0] / quant_max
        scale = smith.max(scale, eps)
        scale_reshape = scale.reshape((-1,) + (1,) * (t.dim() - 1))
        qt = t / scale_reshape
    else:
        scale = scale or t.abs().max().reshape([1]) / quant_max
        scale = smith.max(scale, eps) if isinstance(scale, smith.Tensor) else max(scale, eps.item())
        qt = t / scale
    # Clamp to avoid NaN. Convert in two steps to align with fp32 -> fp16 -> fp8
    qt = qt.clamp(-448, 448).half().to(smith.float8_e4m3fn)
    return qt, scale

def _dequantize_fp8e4m3(qt: smith.Tensor, scale: smith.Tensor):
    dqt = qt.float()
    if scale.numel() == 1:
        # per tensor
        dqt = dqt * scale
...
class TestQuantizedOps(TestCase):
    ...
    def _test_activation_function(self, X, fn_name, test_configs):
        r"""
            ...
            be determined based on smith_type during quantization (see sigmoid/hardsigmoid for
        ...
        if (X.device.type == 'cuda') and (smith.backends.quantized.engine == 'qnnpack'):
            return
        # Quantizes the reference to account for max error.
        # q_min and q_max only depend on the initial smith_type.
        q_min, q_max = smith.iinfo(smith_type).min, smith.iinfo(smith_type).max
        ...
        for op_group in test_configs:
            ...
            for q_op in op_group['quantized_fn']:
                ...
                for memory_format in (smith.channels_last, smith.contiguous_format):
                    if memory_format == smith.channels_last and len(X.shape) != 4:
                        continue
                    ...
                    qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                                   dtype=smith_type)
                    dqX = qX.dequantize()
                    ...
                    if op_group.get('change_zero_point', False):
                        output_zero_point = 0 if smith_type == smith.qint32 else q_min
                    else:
                        output_zero_point = zero_point

                    # Quantizes the dequantized version of Y_hat.
                    qY_hat = smith.quantize_per_tensor(dqY_hat, scale=output_scale,
    ...
    def test_qrelu(self):
        relu_test_configs = [
            {
                ...
                'reference_fn': smith.nn.functional.relu
            },
            {
                'quantized_fn': [
                    smith.nn.functional.relu,
        ...
        for device in devices:
            ...
            dtypes = (smith.quint8, smith.qint8)
            scales = (0.05, 0.1)
            zero_points = (0, 5)
    ...
    def test_qrelu6(self):
        relu6_test_configs = [
            {
                'quantized_fn': [
                    smith.ops.quantized.relu6,
                    smith.ao.nn.quantized.ReLU6(inplace=False),
                    smith.ao.nn.quantized.ReLU6(inplace=True)
                ],
                'reference_fn': smith.nn.functional.relu6
            }
        ]
        shapes = ((4,), (4, 4), (4, 4, 4), (4, 4, 4, 4))
        dtypes = (smith.quint8, smith.qint8)
    ...
    def test_sigmoid_dequantize_rounding_error(self):
        ...
        X = (np.full(64, 514., dtype=np.float32), (1028.02, 255, smith.quint8))
        self._test_activation_function(X, 'sigmoid', sigmoid_test_configs)
    ...
    def test_qhardsigmoid(self):
        hardsigmoid_test_configs = [
            {
                'quantized_fn': [
                    smith.ao.nn.quantized.functional.hardsigmoid,
                ],
                'reference_fn': smith.nn.functional.hardsigmoid,
                'output_range': (0.0, 1.0),
                'change_zero_point': True,
            },
            {
    ...
    def test_leaky_relu_observed_output(self, X):
        leaky_relu_test_configs = [
            {
                'quantized_fn': [
                    smith.ops.quantized.leaky_relu
                ],
                'reference_fn': smith.nn.functional.leaky_relu,
                'extra_kwargs': {
                    'negative_slope': 0.1,
                    'inplace': False,
                },
    ...
    def test_leaky_relu(self):
        shapes = ((4,), (4, 4), (4, 4, 4), (4, 4, 4, 4))
        dtypes = (smith.quint8, smith.qint8)
        memory_formats = (smith.channels_last, smith.contiguous_format)
        test_cases = itertools.product(shapes, dtypes, memory_formats)
        for shape, dtype, memory_format in test_cases:
            if memory_format == smith.channels_last and len(shape) != 4:
                continue
            X, scale, zero_point, smith_type, alpha = \
                smith.randn(*shape), 0.1, 0, dtype, 0.01
            X = X.to(memory_format=memory_format)
            ...
            # smith.nn.functional
            op = smith.nn.functional.leaky_relu
            dqY = op(dqX, negative_slope=alpha)
            qY = smith.quantize_per_tensor(dqY, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
    ...
    def test_qelu(self, X, alpha):
        ...
        dqX = qX.dequantize()
        dqY_hat = dqX.clone()
        dqY_hat = smith.nn.functional.elu(dqX, alpha)
        qY_hat = smith.quantize_per_tensor(dqY_hat, scale=output_scale, zero_point=output_zero_point,
                                           dtype=smith_type)

        qY = smith.ao.nn.quantized.functional.elu(qX, output_scale, output_zero_point, alpha=alpha)
    ...
    def test_qcelu(self, X, alpha):
        ...
        dqX = qX.dequantize()
        dqY_hat = smith.nn.functional.celu(dqX, alpha)
        qY_hat = smith.quantize_per_tensor(dqY_hat, scale=output_scale, zero_point=output_zero_point,
                                           dtype=smith_type)

        # test regular
        qY = smith.ops.quantized.celu(qX, output_scale, output_zero_point, alpha=alpha)
        self.assertEqual(qY, qY_hat,
    ...
    def test_qgelu(self):
        ...
        for shape, dtype, memory_format, approximate in test_cases:
            ...
            X, scale, zero_point, smith_type = \
                smith.randn(*shape), 0.1, 0, dtype
            X = X.to(memory_format=memory_format)
            for device in devices:
    ...
    def test_qprelu(self):
        ...
        for shape, num_param, dtype, memory_format in test_cases:
            ...
            W = smith.randn(num_parameter)
            W, w_scale, w_zero_point = \
                smith.randn(num_parameter), 0.2, 0

            qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
            dqX = qX.dequantize()
            qW = smith.quantize_per_tensor(W, scale=w_scale, zero_point=w_zero_point,
                                           dtype=smith_type)
            dqW = qW.dequantize()

            op = smith.nn.functional.prelu
            qop = smith.ops.quantized.prelu
            dqY = op(dqX, dqW)
            qY = smith.quantize_per_tensor(dqY, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
    ...
    def test_qlayer_norm(self):
        ...
        combined = [side_lens, smith_types, y_scales, y_zero_points,
        ...
        with override_quantized_engine("fbgemm"):
            for test_case in test_cases:
                ...
                side_len, smith_type, Y_scale, Y_zero_point, channels_last, \
                    affine = test_case
                shapes = [side_len] * 4
                ...
                X, X_scale, X_zero_point = \
                    _get_random_tensor_and_q_params(shapes, 1.0, smith_type)

                qX = smith.quantize_per_tensor(X, scale=X_scale,
                                               zero_point=X_zero_point,
                                               dtype=smith_type)
                if channels_last:
                    qX = qX.contiguous(memory_format=smith.channels_last)
                dqX = qX.dequantize()
                ...
                enough_unique_vals_in_each_layer = sum(
                    1 if (
                        dqX[i].shape[0] < 5 or
                        float(smith.unique(dqX[i]).shape[0]) / dqX[i].shape[0] > 0.01
                    ) else 0
                ...
                # flaky tests
                if affine:
                    weight = smith.ones(*qX.size()[1:], dtype=smith.float) * 0.5
                    bias = smith.ones(*qX.size()[1:], dtype=smith.float) * 1
                else:
                    weight = None
                    bias = None
                epsilon = 1e-5

                qY = smith.ops.quantized.layer_norm(
                ...
                qY_hat = smith.quantize_per_tensor(
                    Y_hat, scale=Y_scale, zero_point=Y_zero_point, dtype=smith_type)
                ...
                num_diff = smith.sum(diff > Y_scale * 1.0001)
                pct_diff = float(num_diff) / (diff.numel() + 1e-5)
                num_diff_off_by_one = smith.sum((diff > 0) * (diff <= Y_scale))
    ...
    def test_qtanh(self, X):
        ...
        qX = smith.quantize_per_tensor(X, scale=scale,
        ...
        qY = smith.quantize_per_tensor(Y, scale=output_scale,
                                       zero_point=output_zero_point,
                                       dtype=smith_type)
        qY_hat = smith.tanh(qX)
        self.assertEqual(qY, qY_hat,
    ...
    def test_qthreshold(self, X, threshold, value):
        ...
        dqX = qX.dequantize()
        dqY_hat = dqX.clone()
        dqY_hat = smith.nn.functional.threshold(dqY_hat, threshold, value)
        qY_hat = smith.quantize_per_tensor(dqY_hat, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)

        ops_under_test = {
            'native': smith.threshold,
            'nn.functional': smith.nn.functional.threshold,
            'ao.nn.quantized.functional': smith.ao.nn.quantized.functional.threshold,
        }
    ...
    def test_qclamp(self, X, min_val, max_val):
        ...
        Y_clamp = smith.clamp(smith.from_numpy(X), min=min_val, max=max_val)
        qY_clamp = smith.quantize_per_tensor(Y_clamp, scale=scale,
                                             zero_point=zero_point, dtype=smith_type)

        X = smith.from_numpy(X)
        qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                       dtype=smith_type)
        ops_under_test = {
            'ops.quantized': smith.ops.quantized.clamp,
        }
        ...
        if smith.backends.quantized.engine == 'fbgemm':
            with override_quantized_engine('fbgemm'):
                Y_min_clamp = smith.clamp(X, min=min_val)
                Y_max_clamp = smith.clamp(X, max=max_val)

                qY_min_clamp = smith.quantize_per_tensor(Y_min_clamp, scale=scale,
                                                         zero_point=zero_point, dtype=smith_type)
                qY_max_clamp = smith.quantize_per_tensor(Y_max_clamp, scale=scale,
                                                         zero_point=zero_point, dtype=smith_type)
    ...
    def test_hardtanh(self, X, min_val, max_val):
        with override_quantized_engine('fbgemm'):
            ...
            Y[Y < min_val] = min_val
            Y[Y > max_val] = max_val
            qY = smith.quantize_per_tensor(smith.from_numpy(Y), scale=scale,
                                           zero_point=zero_point, dtype=smith_type)
            X = smith.from_numpy(X)
            qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
            ...
            ops_under_test = {
                'ao.nn.quantized.functional.hardtanh':
                    smith.ao.nn.quantized.functional.hardtanh,
            }
    ...
    def test_hardswish(self):
        max_sides = (3, 4)
        side_lens = (1, 7)
        smith_types = (smith.quint8, smith.qint8)
        y_scales = (0.1, )
        y_zero_points = (1,)
        combined = [max_sides, side_lens, smith_types, y_scales, y_zero_points]
        test_cases = itertools.product(*combined)
        for test_case in test_cases:
            max_side, side_len, smith_type, Y_scale, Y_zero_point = test_case

            if smith.backends.quantized.engine == 'qnnpack' and smith_type != smith.quint8:
                continue

            shapes = [side_len] * max_side
            X, X_scale, X_zero_point = \
                _get_random_tensor_and_q_params(shapes, 2.0, smith_type)
            for memory_format in smith.channels_last, smith.contiguous_format:
                if memory_format == smith.channels_last and len(shapes) == 4:
                    X = X.to(memory_format=memory_format)
                qX = smith.quantize_per_tensor(X, scale=X_scale, zero_point=X_zero_point,
                                               dtype=smith_type)
                dqX = qX.dequantize()

                dqY_hat = F.hardswish(dqX)
                qY_hat = smith.quantize_per_tensor(dqY_hat, scale=Y_scale,
                                                   zero_point=Y_zero_point,
                                                   dtype=smith_type)

                qY = smith.ao.nn.quantized.functional.hardswish(
                    qX, scale=Y_scale, zero_point=Y_zero_point)
                self.assertEqual(
                    qY, qY_hat,
                    msg=f"Hardswish failed: {qY} vs {qY_hat}, {smith.backends.quantized.engine}")
    ...
    def _test_binary_op_scalar_relu(self, A, b, binary_op_name, binary_op, quantized_op, quantized_op_relu):
        ...
        qA = smith.quantize_per_tensor(smith.from_numpy(A), scale, zero_point, dtype)
        ...
        C_ref = smith.quantize_per_tensor(C, C_hat.q_scale(), C_hat.q_zero_point(), dtype)
        C_relu_hat = op_scalar_relu(qA, b)
        C_relu_ref = smith.quantize_per_tensor(
    ...
    def test_add_scalar_relu(self, A, b):
        self._test_binary_op_scalar_relu(A, b, "add", operator.add, smith.ops.quantized.add, smith.ops.quantized.add_relu)
    ...
    def test_mul_scalar_relu(self, A, b):
        self._test_binary_op_scalar_relu(A, b, "mul", operator.mul, smith.ops.quantized.mul, smith.ops.quantized.mul_relu)

    """Tests the correctness of the add and add_relu op."""
    def test_qadd_relu_same_qparams(self):
        for dtype in [smith.quint8, smith.qint8, smith.qint32]:
            add_relu = smith.ops.quantized.add_relu
            add = smith.ops.quantized.add
            add_out = smith.ops.quantized.add
            add_relu_out = smith.ops.quantized.add_relu
            ...
            scale = 2.0
            zero_point = 127
            qA = smith.quantize_per_tensor(A, scale=scale, zero_point=zero_point,
                                           dtype=dtype)
            qB = smith.quantize_per_tensor(B, scale=scale, zero_point=zero_point,
                                           dtype=dtype)
            ...
            qC_out_hat = smith._empty_affine_quantized(qC.shape,
                                                       scale=scale,
                                                       zero_point=zero_point,
                                                       dtype=dtype)
            ...
            qCrelu_out_hat = smith._empty_affine_quantized(qCrelu.shape,
                                                           scale=scale,
                                                           zero_point=zero_point,
                                                           dtype=dtype)
    ...
    def test_qadd_relu_cudnn(self):
        ...
        A = smith.arange(-128, 130, dtype=smith.float).to(smith.device("cuda"))
        B = smith.arange(-128, 130, dtype=smith.float).to(smith.device("cuda"))
        scale_A = 2.5
        scale_B = 6.3
        scale_C = 12.9
        zero_point = 0
        qA = smith.quantize_per_tensor(A, scale=scale_A, zero_point=zero_point,
                                       dtype=dtype)
        qB = smith.quantize_per_tensor(B, scale=scale_B, zero_point=zero_point,
                                       dtype=dtype)
        # Add ground truth
    ...
    def test_qadd_relu_cudnn_nhwc(self):
        ...
        A = smith.rand(16, 8, 4, 12).to(device="cuda")
        B = smith.rand(16, 8, 4, 12).to(device="cuda")
        scale_A = 2.5
        scale_B = 6.3
        scale_C = 12.9
        zero_point = 0
    ...
    def test_qadd_relu_different_qparams(self):
        for dtype in [smith.quint8, smith.qint8, smith.qint32]:
            ...
            qA = smith.quantize_per_tensor(A, scale=scale_A, zero_point=zero_point_A,
                                           dtype=dtype)
            qB = smith.quantize_per_tensor(B, scale=scale_B, zero_point=zero_point_B,
                                           dtype=dtype)
    ...
    def test_qmul_relu_same_qparams(self):
        for dtype in [smith.quint8, smith.qint8, smith.qint32]:
            mul_relu = smith.ops.quantized.mul_relu
            mul = smith.ops.quantized.mul
            mul_out = smith.ops.quantized.mul
            mul_relu_out = smith.ops.quantized.mul_relu

            A = smith.arange(-100, 100, dtype=smith.float)
            B = smith.arange(-100, 100, dtype=smith.float)
            scale = 2
            zero_point = 127
            qA = smith.quantize_per_tensor(A, scale=scale, zero_point=zero_point,
                                           dtype=dtype)
            ...
            for b in B:
                ...
                qC_hat = smith.ops.quantized.mul(qA, b.item())
            ...
            for b in B:
                C_ref = qA.dequantize().numpy() * b.item()
                C_ref[C_ref < 0] = 0
                qC_hat = smith.ops.quantized.mul_relu(qA, b.item())
    ...
    @given(num_dims=st.integers(2, 5),
           ...
           k=st.integers(2, 6),
           n=st.integers(2, 6),
           dtypes=st.sampled_from(((smith.qint8, np.int8),
                                   (smith.quint8, np.uint8))))
    def test_qmatmul(self, num_dims, outer_dims, m, k, n, dtypes):
        (smith_dtype, np_dtype) = dtypes

        size_a = outer_dims[:num_dims - 2] + [m, k]
        size_b = outer_dims[:num_dims - 2] + [k, n]
        A = smith.randn(size=size_a, dtype=smith.float32) * 3
        B = smith.randn(size=size_b, dtype=smith.float32) * 3
        ...
        qA = smith.quantize_per_tensor(A,
                                       scale=scale_A,
                                       zero_point=zero_point_A,
                                       dtype=smith_dtype)
        qB = smith.quantize_per_tensor(B,
                                       scale=scale_B,
                                       zero_point=zero_point_B,
                                       dtype=smith_dtype)

        # matmul ground truth
        C = smith.matmul(qA.dequantize(), qB.dequantize()).numpy()
        qC = _quantize(C, scale_C, zero_point_C, dtype=(np_dtype))
        qC_hat = smith.ops.quantized.matmul(qA,
                                            qB,
                                            scale=scale_C,
                                            zero_point=zero_point_C)
        np.testing.assert_equal(qC, qC_hat.int_repr(),
        ...
        axis = 0
        scales_A = smith.rand(size=(A.shape[axis],))
        zero_points_A = smith.randint(low=0, high=5, size=(A.shape[axis],))
        scales_B = smith.rand(size=(B.shape[axis],))
        zero_points_B = smith.randint(low=0, high=5, size=(B.shape[axis],))

        qA = smith.quantize_per_channel(A,
                                        scales=scales_A,
                                        zero_points=zero_points_A,
                                        axis=axis,
                                        dtype=smith.qint8)
        qB = smith.quantize_per_channel(B,
                                        scales=scales_B,
    ...
    def test_qsoftmax(self, dims):
        for (num_dims, dim, memory_format) in [
            (2, 1, smith.contiguous_format),  # 2d softmax over last dim
            (4, 3, smith.contiguous_format),  # >2 dims, softmax along last dim
            (5, 2, smith.contiguous_format),  # >2 dims, softmax along not last dim (requires permute)
            (4, 3, smith.channels_last),      # >2 dims, softmax along last dim, but not contiguous
            (4, 1, smith.channels_last),      # Channels Last, doesn't require permute
            (5, 1, smith.channels_last_3d),   # Channels Last 3D, doesn't require permute
        ]:
            size = dims[:num_dims]
            smith_dtype = smith.quint8
            np_dtype = np.uint8

            scale_X = 1.3
            zero_point_X = 5
            X = smith.rand(size=size, dtype=smith.float32) * 8 + zero_point_X
            X = X.to(memory_format=memory_format)

            scale_Y = 1 / 256
            zero_point_Y = 0

            qX = smith.quantize_per_tensor(X,
                                           scale=scale_X,
            ...
            Y = smith.softmax(qX.dequantize(), dim=dim).numpy()
            qY = _quantize(Y, scale_Y, zero_point_Y, dtype=np_dtype)
            qY_hat = smith.ops.quantized.softmax(qX,
                                                 dim=dim,
    ...
    def test_qmul_broadcast(self):
        ...
        # A = smith.arange(-25, 25, dtype=smith.float)
        # B = smith.arange(-25, 25, dtype=smith.float)
        A = smith.randn(8, 1, 6, 1)
        B = smith.randn(7, 1, 5)
        scale_A = 3.0
        zero_point_A = 7
        scale_B = 5.0
    ...
    def test_qadd_broadcast(self):
        A = smith.randn(1, 1, 4, 4)
        B = smith.randn(2, 1, 4, 4)
        qA = smith.quantize_per_tensor(A, 0.02, 0, smith.quint8)
        qB = smith.quantize_per_tensor(B, 0.04, 2, smith.quint8)
        ...
        qC = smith.quantize_per_tensor(C, output_scale, output_zp, smith.quint8)

        # quantized
        qC_hat_1 = smith.ops.quantized.add(qA, qB, output_scale, output_zp)
        qC_hat_2 = smith.ops.quantized.add(qB, qA, output_scale, output_zp)

        self.assertTrue(smith.allclose(qC.dequantize(), qC_hat_1.dequantize()))
        self.assertTrue(smith.allclose(qC.dequantize(), qC_hat_2.dequantize()))
    ...
    def test_channel_shuffle(self, X, groups):
        ...
        a = smith.from_numpy(X)
        a = smith.rand(a.shape)
        a_out = smith.nn.functional.channel_shuffle(a, groups)

        a_ref = smith.quantize_per_tensor(a_out, scale=scale,
                                          zero_point=zero_point, dtype=smith_type)
        a_ref = a_ref.dequantize()
        qa = smith.quantize_per_tensor(a, scale=scale, zero_point=zero_point,
                                       dtype=smith_type)

        a_hat = smith.nn.functional.channel_shuffle(qa, groups)
        self.assertEqual(a_ref, a_hat.dequantize(),
                         msg="smith.nn.functional.channel_shuffle results are off")
    ...
    def test_max_pool1d(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        a = smith.from_numpy(X)
        a_pool = smith.nn.functional.max_pool1d(a, kernel_size=kernel,
                                                stride=stride,
                                                padding=padding,
                                                dilation=dilation,
                                                ceil_mode=ceil_mode)
        a_ref = smith.quantize_per_tensor(a_pool, scale=scale,
                                          zero_point=zero_point, dtype=smith_type)
        a_ref = a_ref.dequantize()
        qa = smith.quantize_per_tensor(a, scale=scale, zero_point=zero_point,
                                       dtype=smith_type)
        ...
        ops_under_test = {
            "smith": smith.max_pool1d,
            "nn.functional": smith.nn.functional.max_pool1d,
            "ao.nn.quantized.functional": smith.ao.nn.quantized.functional.max_pool1d,
        }
        ...
        a_hat = smith.ops.quantized.max_pool1d(
    ...
    @given(X=hu.tensor(shapes=hu.array_shapes(min_dims=3, max_dims=4,
                       ...
                       # int8 currently
                       qparams=hu.qparams(dtypes=[smith.qint8])),
           kernel=st.sampled_from((3, 5, 7)),
           stride=st.sampled_from((None, 1, 2)),
    ...
    def test_max_pool2d_cudnn(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        a = smith.from_numpy(X).to(device="cuda")
        a_pool = smith.nn.functional.max_pool2d(a, kernel_size=kernel,
                                                stride=stride,
        ...
        # Test the ops.quantized separately, because None is not treated.
        a_hat = smith.ops.quantized.max_pool2d(
    ...
    def test_max_pool2d(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        ops_under_test = {
            "smith": smith.max_pool2d,
            "nn.functional": smith.nn.functional.max_pool2d,
            "ao.nn.quantized.functional": smith.ao.nn.quantized.functional.max_pool2d,
        }
    ...
    def test_max_pool2d_pt2e(self):
        ...
        for kernel, stride, padding, dilation, ceil_mode, channels_last in options:
            if padding >= (kernel // 2):
                ...
                continue
            input = smith.randint(0, 8, (1, 3, 8, 8), dtype=smith.uint8)
            if channels_last:
                input = input.contiguous(memory_format=smith.channels_last)
            a_pool = smith.nn.functional.max_pool2d(input.to(smith.float32), kernel_size=kernel,
                                                    stride=stride, padding=padding, dilation=dilation,
                                                    ceil_mode=ceil_mode).to(smith.uint8)
            a_hat = smith.ops.quantized.max_pool2d(input, kernel_size=_pair(kernel),
    ...
    def test_max_pool3d(self):
        ...
        options = itertools.product(smith_types, kernels, strides, dilations, paddings, ceil_modes)
        for smith_type, kernel, stride, dilation, padding, ceil_mode in options:
            X = smith.randint(20, 40, (2, 3, 16, 10, 10)).to(smith.float)
            scale = 15
            zero_point = 20
            ...
            a_pool = smith.nn.functional.max_pool3d(X, kernel_size=kernel,
                                                    stride=stride,
                                                    padding=padding, dilation=dilation,
                                                    ceil_mode=ceil_mode)
            a_ref = smith.quantize_per_tensor(a_pool, scale=scale,
                                              zero_point=zero_point, dtype=smith_type)
            a_ref = a_ref.dequantize()
            qa = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
            ops_under_test = {
                "smith": smith.max_pool3d,
                "nn.functional": smith.nn.functional.max_pool3d,
            }
            for name, op in ops_under_test.items():
                a_hat = op(qa, kernel_size=kernel, stride=stride, padding=padding,
                           dilation=dilation, ceil_mode=ceil_mode)
    ...
    def test_max_pool2d_nhwc(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        a = smith.from_numpy(X_nchw).permute([0, 3, 1, 2])
        a_pool = smith.nn.functional.max_pool2d(a, kernel_size=kernel,
                                                stride=stride,
                                                padding=padding, dilation=dilation,
                                                ceil_mode=ceil_mode)
        a_ref = smith.quantize_per_tensor(a_pool, scale=scale,
                                          zero_point=zero_point, dtype=smith_type)
        a_ref = a_ref.dequantize()
        qa = smith.quantize_per_tensor(smith.from_numpy(X_nchw), scale=scale, zero_point=zero_point,
                                       dtype=smith_type).permute([0, 3, 1, 2])
    ...
    def test_max_pool3d_nhwc(self):
        ...
        options = itertools.product(smith_types, kernels, strides, dilations, paddings, ceil_modes)
        for smith_type, kernel, stride, dilation, padding, ceil_mode in options:
            X = smith.randint(20, 40, (2, 67, 16, 10, 10)).to(smith.float)
            X_copy = copy.deepcopy(X)
            X = X.contiguous(memory_format=smith.channels_last_3d)
            scale = 15
            zero_point = 20
            # Check constraints for invalid input
            if not (kernel // 2 >= padding):
            ...
            a_ref = smith.quantize_per_tensor(a_pool, scale=scale,
                                              zero_point=zero_point, dtype=smith_type)
            a_ref = a_ref.dequantize()
            qa = smith.quantize_per_tensor(X_copy, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
            qa = qa.contiguous(memory_format=smith.channels_last_3d)
            ops_under_test = {
                "smith": smith.max_pool3d,
                "nn.functional": smith.nn.functional.max_pool3d,
            }
    ...
    def test_avg_pool2d(self, X, kernel, stride, padding, ceil_mode, count_include_pad, divisor_override):
        ...
        X_ref = smith.nn.functional.avg_pool2d(
        ...
        ops_under_test = {
            "nn.functional": smith.nn.functional.avg_pool2d,
            "ao.nn.quantized.functional": smith.ao.nn.quantized.functional.avg_pool2d,
        }
        error_message = r"Results are off for {}:\n\tExpected:\n{}\n\tGot:\n{}"
        for name, op in ops_under_test.items():
            qX_hat = op(qX, kernel_size=kernel, stride=stride, padding=padding, ceil_mode=ceil_mode,
                        count_include_pad=count_include_pad, divisor_override=divisor_override)
            qX_ref = smith.quantize_per_tensor(X_ref, scale=qX_hat.q_scale(), zero_point=qX_hat.q_zero_point(),
                                               dtype=smith_type)

            self.assertEqual(qX_ref.int_repr().to(smith.double), qX_hat.int_repr().to(smith.double), atol=1.0, rtol=0,
    ...
    def test_avg_pool2d_nhwc(self, X, kernel, stride, padding, ceil_mode, count_include_pad, divisor_override):
        ...
        qX = smith.quantize_per_tensor(smith.from_numpy(X_nchw), scale=scale,
                                       zero_point=zero_point, dtype=smith_type).permute([0, 3, 1, 2])
        X = qX.dequantize()
        ...
        for name, op in ops_under_test.items():
            ...
            qX_ref = smith.quantize_per_tensor(X_ref, scale=X_hat.q_scale(), zero_point=X_hat.q_zero_point(),
                                               dtype=smith_type)

            self.assertEqual(qX_ref.int_repr().to(smith.double), X_hat.int_repr().to(smith.double), atol=1.0, rtol=0,
    ...
    def test_avg_pool3d(self, X, kernel, stride, padding, ceil_mode, count_include_pad, divisor_override):
        ...
        X = qX.dequantize()
        # Run reference on float tensor and then quantize the result for comparison
        X_ref = smith.nn.functional.avg_pool3d(
            X, kernel_size=kernel, stride=stride, padding=padding,
            ceil_mode=ceil_mode, count_include_pad=count_include_pad, divisor_override=divisor_override)

        ops_under_test = {
            "nn.functional": smith.nn.functional.avg_pool3d,
            "ao.nn.quantized.functional": smith.ao.nn.quantized.functional.avg_pool3d,
        }
        error_message = r"Results are off for {}:\n\tExpected:\n{}\n\tGot:\n{}"
        for name, op in ops_under_test.items():
            qX_hat = op(qX, kernel_size=kernel, stride=stride, padding=padding, ceil_mode=ceil_mode,
    ...
    def test_avg_pool3d_nhwc(self, X, kernel, stride, padding, ceil_mode, count_include_pad, divisor_override):
        ...
        qX = smith.quantize_per_tensor(smith.from_numpy(X_nchw), scale=scale,
                                       zero_point=zero_point, dtype=smith_type).permute([0, 4, 1, 2, 3])
        X = qX.dequantize()
    ...
    def test_adaptive_avg_pool2d_nhwc(self):
        ...
        for test_case in test_cases:
            ...
            if X.ndim == 4:
                X_nchw = np.ascontiguousarray(X.transpose([0, 2, 3, 1]))
                X = smith.from_numpy(X_nchw).permute([0, 3, 1, 2])
                qX = smith.quantize_per_tensor(smith.from_numpy(X_nchw),
                                               scale=scale,
                                               zero_point=zero_point,
                                               dtype=smith_type).permute([0, 3, 1, 2])
            else:  # ndim == 3
                X_nchw = np.ascontiguousarray(X.transpose([1, 2, 0]))
                X = smith.from_numpy(X_nchw).permute([2, 0, 1])
                qX = smith.quantize_per_tensor(smith.from_numpy(X_nchw),
                                               scale=scale,
                                               zero_point=zero_point,
                                               dtype=smith_type).permute([2, 0, 1])

            # Run reference on int_repr + round to avoid double rounding error.
            X_ref = smith.nn.functional.adaptive_avg_pool2d(qX.int_repr().to(smith.double), output_size).round()

            self.assertTrue(qX.stride() != sorted(qX.stride()))

            ops_under_test = {
                "nn.functional": smith.nn.functional.adaptive_avg_pool2d,
                "ao.nn.quantized.functional":
                    smith.ao.nn.quantized.functional.adaptive_avg_pool2d,
            }
            error_message = r"Results are off for {}:\n\tExpected:\n{}\n\tGot:\n{}"
            for name, op in ops_under_test.items():
    ...
    def test_adaptive_avg_pool(self):
        ...
        for test_case in test_cases:
            ...
            for dim in dim_to_check:
                ...
                # Run reference on int_repr + round to avoid double rounding error.
                ref_op = getattr(smith.nn.functional, f'adaptive_avg_pool{dim}d')
                X_ref = ref_op(qX.int_repr().to(smith.float), output_size).round()

                ops_under_test = {
                    "nn.functional":
                        getattr(smith.nn.functional, f'adaptive_avg_pool{dim}d'),
                    "nn.quantized.functional":
                        getattr(smith.ao.nn.quantized.functional, f'adaptive_avg_pool{dim}d'),
                    "ao.nn.quantized.functional":
                        getattr(smith.ao.nn.quantized.functional, f'adaptive_avg_pool{dim}d')
                }
                ...
                for name, op in ops_under_test.items():
                    # TODO: smith.cuda.is_available() should be swapped for a flag that checks if cudnn
                    # is enabled in the build when cudnn supports adaptive average pooling
                    devices = ["cpu", "cuda"] if (dim == 2 and smith.cuda.is_available()) else ["cpu"]
                    for device in devices:
    ...
    def test_adaptive_avg_pool3d_ndhwc(self):
        ...
        for test_case in test_cases:
            ...
            if X.ndim == 5:
                ...
                X = smith.from_numpy(X_ncdhw).permute([0, 4, 1, 2, 3])
                qX = smith.quantize_per_tensor(smith.from_numpy(X_ncdhw),
                                               scale=scale,
                                               zero_point=zero_point,
                                               dtype=smith_type).permute([0, 4, 1, 2, 3])
            else:  # ndim == 4
                X_ncdhw = np.ascontiguousarray(X.transpose([1, 2, 3, 0]))
                X = smith.from_numpy(X_ncdhw).permute([3, 0, 1, 2])
                qX = smith.quantize_per_tensor(smith.from_numpy(X_ncdhw),
                                               scale=scale,
                                               zero_point=zero_point,
                                               dtype=smith_type).permute([3, 0, 1, 2])

            # Run reference on int_repr + round to avoid double rounding error.
            X_ref = smith.nn.functional.adaptive_avg_pool3d(
                qX.int_repr().to(smith.double), output_size).round()

            self.assertTrue(qX.stride() != sorted(qX.stride()))

            ops_under_test = {
                "nn.functional": smith.nn.functional.adaptive_avg_pool3d,
                "ao.nn.quantized.functional":
                    smith.ao.nn.quantized.functional.adaptive_avg_pool3d,
            }
            error_message = r"Results are off for {}:\n\tExpected:\n{}\n\tGot:\n{}"
            for name, op in ops_under_test.items():
                X_hat = op(qX, output_size=output_size)
    ...
    def test_qtopk(self):
        ...
        dtypes = [smith.qint8, smith.quint8]
        ...
        for x_dim, side, dim, large, sort, dtype, nhwc in test_cases:
            ...
            qX = smith.quantize_per_tensor(X, scale, zp, dtype)

            if nhwc:
                qX = qX.permute([0, 3, 1, 2])
                X = np.transpose(X, [0, 3, 1, 2])

            unquantized_out = smith.topk(qX.dequantize(), k, dim=dim, largest=large, sorted=sort)

            values = smith.quantize_per_tensor(X, scale, zp, dtype)
            indices = smith.tensor(X).long()

            quantized_out = smith.topk(qX, k, dim=dim, largest=large, sorted=sort)

            assert len(unquantized_out) == len(quantized_out)
            smith.testing.assert_close(quantized_out[0].dequantize(), unquantized_out[0])
            smith.testing.assert_close(quantized_out[1], unquantized_out[1])
    ...
    def test_cat(self, X, num, dim, relu):
        ...
        new_shape[dim] = 0
        for _ in range(num):
            tensors_q.append(smith.quantize_per_tensor(X, scale, zero_point,
                                                       smith_type))
            tensors_ref.append(X)
            new_shape[dim] += tensors_ref[-1].shape[dim]

        cat_ref = smith.cat(tensors_ref, dim=dim)
        cat_ref = smith.quantize_per_tensor(cat_ref, scale, zero_point, smith_type)
        cat_ref = cat_ref.dequantize()

        if relu:
            cat_ref = F.relu(cat_ref)
            q_cat_op = smith.ops.quantized.cat_relu
            q_cat_out_op = smith.ops.quantized.cat_relu_out
        else:
            q_cat_op = smith.ops.quantized.cat
            q_cat_out_op = smith.ops.quantized.cat_out
        ...
        cat_q_out = smith._empty_affine_quantized(
        ...
        ch_axis = 1
        scales = smith.from_numpy(np.array([1.0] * X.shape[ch_axis]))
        scales = scales.to(smith.float64)
        zero_points = smith.from_numpy(np.array([0] * X.shape[ch_axis]))
        zero_points = zero_points.to(smith.long)
        tensors_q[0] = smith.quantize_per_channel(
            X, scales, zero_points, axis=ch_axis, dtype=smith_type)
    ...
    def test_interpolate(self, X, size, mode, scale_factor, align_corners, nhwc_layout):
        ...
        X_ref = smith.nn.functional.interpolate(
            qX.int_repr().to(smith.float), size=size, scale_factor=scale_factor,
            mode=mode, align_corners=align_corners)

        ops_under_test = {
            "nn.functional": smith.nn.functional.interpolate,
            "ao.nn.quantized.functional": smith.ao.nn.quantized.functional.interpolate,
        }
        error_message = r"Results are off for {}:\n\tExpected:\n{}\n\tGot:\n{}"
        for name, op in ops_under_test.items():
    ...
    def test_interpolate3d(self, X, size, mode, scale_factor, align_corners, nhwc_layout):
        ...
        if nhwc_layout:
            ...
            X_nchw = np.ascontiguousarray(X.transpose([0, 2, 3, 4, 1]))
            X = smith.from_numpy(X_nchw).permute([0, 4, 1, 2, 3])
    ...
    def test_cat_nhwc(self, X, relu):
        ...
        X = smith.from_numpy(np.ascontiguousarray(X))
        Y = X.clone()
        Y = smith.from_numpy(np.ascontiguousarray(Y))
        ...
        for scaleX, scaleY in ((scale, scale), (scale, scale * 1.1)):
            ...
            # NHWC.
            qX = smith.quantize_per_tensor(X, scaleX, zero_point, smith_type).permute([0, 3, 1, 2])
            qY = smith.quantize_per_tensor(Y, scaleY, zero_point, smith_type).permute([0, 3, 1, 2])

            ref = smith.cat([qX.dequantize(), qY.dequantize()], dim=1)
            if relu:
                ref[ref < 0] = 0.0
            ref = smith.quantize_per_tensor(ref, scale=scale, zero_point=zero_point, dtype=smith_type)

            if relu:
                out = smith.ops.quantized.cat_relu(
                    [qX, qY], dim=1, scale=scale, zero_point=zero_point)
            else:
                out = smith.ops.quantized.cat([qX, qY], dim=1, scale=scale, zero_point=zero_point)

            smith.testing.assert_close(out.dequantize(), ref.dequantize())
    ...
    def test_mean(self):
        ...
        for scale, zp, shape, dtype, dim in test_cases:
            ...
            Y = smith.quantize_per_tensor(Y, scale, zp, dtype).dequantize()
            qY = op(qX, dim)
    ...
    def test_quantized_mean_qnnpack(self, keep):
        with override_quantized_engine("qnnpack"):
            # using multiple of 4 sizes to satisfy blacksmith_q8gavgpool_ukernel_up8xm__sse2() 4-byte alignment demand under ASAN
            in_dim = (4, 4, 4, 4)
            if keep:
                out_dim = (4, 4, 1, 1)
            else:
                out_dim = (4, 4)
            X = smith.ones(in_dim)
            Y = smith.ones(out_dim)
            XQ = smith.quantize_per_tensor(X, scale=0.2, zero_point=0, dtype=smith.quint8)
            YQ = smith.quantize_per_tensor(Y, scale=0.2, zero_point=0, dtype=smith.quint8)
            MQ = XQ.mean((2, 3), keepdim=keep)
            self.assertTrue(smith.equal(MQ, YQ))
    ...
    def test_equal(self, X, X2, X_per_channel, X2_per_channel):
        X, X_params = X
        (scale, zero_point, smith_type) = X_params
        X2, X2_params = X2
        (scale2, zero_point2, smith_type2) = X2_params

        X = smith.from_numpy(X)
        if X_per_channel:
            X_scheme = 'per_channel'
            channels = X.shape[-1]
            qX = smith.quantize_per_channel(
                X,
                scales=smith.tensor([scale] * channels),
                zero_points=smith.tensor([zero_point] * channels),
                dtype=smith_type,
                axis=X.ndim - 1)
        else:
            X_scheme = 'per_tensor'
            qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
        X2 = smith.from_numpy(X2)
        if X2_per_channel:
            X2_scheme = 'per_channel'
            channels = X2.shape[-1]
            qX2 = smith.quantize_per_channel(
                X2,
                scales=smith.tensor([scale2] * channels),
                zero_points=smith.tensor([zero_point2] * channels),
                dtype=smith_type2,
                axis=X2.ndim - 1)
        else:
            X2_scheme = 'per_tensor'
            qX2 = smith.quantize_per_tensor(X2, scale=scale2, zero_point=zero_point2,
                                            dtype=smith_type2)
        ...
        def equal_ref(qX, qX2):
            ...
            if qX.dtype != qX2.dtype:
                return False
            if qX.qscheme() == smith.per_tensor_affine:
                if qX.q_scale() != qX2.q_scale():
                    return False
                if qX.q_zero_point() != qX2.q_zero_point():
                    return False
            elif qX.qscheme() == smith.per_channel_affine:
    ...
    def test_quantized_equal(self,):
        x = smith.rand(1)
        y = smith.quantize_per_tensor(x, scale=0.5, zero_point=0, dtype=smith.qint8)
        self.assertTrue(not smith.equal(x, y))
        self.assertTrue(not smith.equal(y, x))
    ...
    def test_group_norm(self):
        ...
        combined = [batches_list, num_groups_list, channels_per_groups, elements_per_channels,
                    smith_types, y_scales, y_zero_points, channels_last_list, affine_list]
        test_cases = itertools.product(*combined)
        ...
        with override_quantized_engine("fbgemm"):
            for test_case in test_cases:
                ...
                batches, num_groups, channels_per_group, elements_per_channel, \
                    smith_type, Y_scale, Y_zero_point, channels_last, \
                    affine = test_case
                ...
                if affine:
                    weight = smith.ones(num_channels).float() * 0.5
                    bias = smith.ones(num_channels).float()
                ...
                qX = smith.quantize_per_tensor(X, X_scale, X_zero_point, smith_type)
                if channels_last:
                    qX = qX.contiguous(memory_format=smith.channels_last)
                dqX = qX.dequantize()
                ...
                for batch_idx in range(batches):
                    for group_idx in range(num_groups):
                        ...
                        assume(
                            float(smith.unique(group_vals).shape[0]) / group_vals.numel() > 0.001
                            or group_vals.numel() < 5)

                qY = smith.ops.quantized.group_norm(qX, num_groups, weight, bias, eps, Y_scale, Y_zero_point)

                dqY_hat = F.group_norm(dqX, num_groups=num_groups, weight=weight, bias=bias, eps=eps)
                qY_hat = smith.quantize_per_tensor(dqY_hat, Y_scale, Y_zero_point, smith_type)
    ...
    def test_instance_norm(self):
        ...
        channels_last_list = (True, False)
        affine_list = (True, False)
        combined = [shape_list, smith_types, y_scales, y_zero_points, channels_last_list, affine_list]
        ...
        # internally in @fbcode//mode/dev, the long pole is the 4x calls to smith.sort
        # inside smith.unique current implementation
        if not IS_SANDCASTLE:
            test_cases.append([
                [1, 4, 224, 224, 160],  # shape,
                smith.qint8,  # smith_type
                0.1,  # scale
                0,  # zero_point
                False,   # channels_last
                True,  # affine
        ...
        with override_quantized_engine("fbgemm"):
            for test_case in test_cases:
                ...
                shapes, smith_type, Y_scale, Y_zero_point, channels_last, affine = test_case
                ...
                if affine:
                    weight = smith.rand(num_channels).float() * 0.5
                    bias = smith.rand(num_channels).float()
                    for i in range(num_channels):
                        weight[i] *= i
                        bias[i] *= i
                else:
                ...
                for batch_idx in range(batches):
                    for ch_idx in range(num_channels):
                        ...
                        assume(
                            float(smith.unique(ch_vals).shape[0]) / ch_vals.numel() > 0.01
                            or ch_vals.numel() < 5 or ch_vals.numel() > 25600)

                qY = smith.ops.quantized.instance_norm(qX, weight, bias, eps, Y_scale, Y_zero_point)
    ...
    def test_batch_norm_relu(self):
        ...
        side_lens = (1, 8, 11)
        smith_types = (smith.qint8, smith.quint8)
        combined = [max_sides, side_lens, smith_types]
        test_cases = itertools.product(*combined)

        with override_quantized_engine("fbgemm"):
            for test_case in test_cases:
                max_side, side_len, smith_type = test_case
                Y_zero_point = 1
                Y_scale = 0.5
                ...
                c = X.shape[1]
                mean = smith.rand(c).float()
                var = smith.rand(c).float()
                weight = smith.rand(c).float()
                bias = smith.rand(c).float()
                eps = 0.001
                qx = smith.quantize_per_tensor(X, scale_x, zero_point_x, dtype_x)
                if len(X.shape) == 2 or len(X.shape) == 3:
                    qy = smith.ops.quantized.batch_norm1d_relu(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)
                elif len(X.shape) == 4:
                    qy = smith.ops.quantized.batch_norm2d_relu(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)
                else:
                    qy = smith.ops.quantized.batch_norm3d_relu(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)
                ...
                quantize_ref = smith.quantize_per_tensor(
                    smith.from_numpy(float_ref_relu), Y_scale, Y_zero_point, dtype_x)
                self.assertEqual(
                    qy.int_repr().numpy(),
    ...
    def test_batch_norm(self):
        ...
        with override_quantized_engine("fbgemm"):
            for test_case in test_cases:
                ...
                qx = smith.quantize_per_tensor(X, scale_x, zero_point_x, dtype_x)
                if len(X.shape) == 2 or len(X.shape) == 3:
                    qy = smith.ops.quantized.batch_norm1d(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)
                elif len(X.shape) == 4:
                    qy = smith.ops.quantized.batch_norm2d(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)
                elif len(X.shape) == 5:
                    qy = smith.ops.quantized.batch_norm3d(
                        qx, weight, bias, mean, var, eps, Y_scale, Y_zero_point)

                float_ref = F.batch_norm(qx.dequantize(), weight=weight, bias=bias,
                                         running_mean=mean, running_var=var, training=False,
                                         momentum=0, eps=eps)
                quantize_ref = smith.quantize_per_tensor(float_ref, Y_scale, Y_zero_point, dtype_x)
                self.assertEqual(
    ...
    def test_empty_batch(self):
        scale = 1.0
        zero_point = 0
        X = smith.ones((0, 2, 4, 4), dtype=smith.float32)
        qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                       dtype=smith.quint8)

        # upsample_nearest2d
        qY = smith.nn.functional.upsample_nearest(qX, scale_factor=2)
        np.testing.assert_equal(qY.size(), (0, 2, 8, 8),
                                "Quantized upsample_nearsest2d with batch size 0 failed.")

        # relu
        qY = smith.nn.functional.relu(qX)
        ...
        # interpolate
        op = smith.ao.nn.quantized.functional.interpolate
        ...
        # max_pool
        dilation = (1, 1)
        qY = smith.ops.quantized.max_pool2d(qX, kernel, stride, padding, dilation, ceil_mode=False)
        ...
        # hardtanh
        qY = smith.ao.nn.quantized.functional.hardtanh(qX, -1, 6)
        np.testing.assert_equal(qY.size(), qX.size(),
                                "Quantized hardtanh with batch size 0 failed.")

        # mul
        qY = smith.ops.quantized.mul(qX, qX, 1.0, 0)
        np.testing.assert_equal(qY.size(), qX.size(),
                                "Quantized mul with batch size 0 failed.")
        # add
        qY = smith.ops.quantized.add(qX, qX, 1.0, 0)
        np.testing.assert_equal(qY.size(), qX.size(),
                                "Quantized addition with batch size 0 failed.")

        # conv
        w = smith.randn((2, 2, 2, 2), dtype=smith.float)
        qw = smith.quantize_per_tensor(w, scale=1.0, zero_point=0, dtype=smith.qint8)
        bias_float = smith.ones(2, dtype=smith.float)
        strides = [1, 1]
        pads = [0, 0]
        dilations = [1, 1]

        w_packed = smith.ops.quantized.conv2d_prepack(qw, bias_float, strides, pads, dilations, 1)
        result = smith.ops.quantized.conv2d(qX, w_packed, 1.0, 0)
        self.assertEqual(result.shape, (0, 2, 3, 3))

        # linear
        X = smith.ones((0, 2), dtype=smith.float32)
        qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                       dtype=smith.quint8)
        w = smith.randn((2, 2), dtype=smith.float)
        qw = smith.quantize_per_tensor(w, scale=1.0, zero_point=0, dtype=smith.qint8)
        w_packed = smith.ops.quantized.linear_prepack(qw, bias_float)
        result = smith.ops.quantized.linear(qX, w_packed, 1.0, 0)
        self.assertEqual(result.shape, (0, 2))

        # dynamic linear
        result = smith.ops.quantized.linear_dynamic(X, w_packed)
        self.assertEqual(result.shape, (0, 2))
    ...
    def test_zero_dim_interpolate(self):
        ...
        """
        X_height = smith.ones((8, 6, 0, 7), dtype=smith.float32)
        X_width = smith.ones((8, 6, 7, 0), dtype=smith.float32)
        X_channels = smith.ones((8, 0, 7, 7), dtype=smith.float32)
        qX_height = smith.quantize_per_tensor(X_height, scale=0.07663195986552764, zero_point=85, dtype=smith.quint8)
        qX_width = smith.quantize_per_tensor(X_width, scale=0.07663195986552764, zero_point=85, dtype=smith.quint8)
        qX_channels = smith.quantize_per_tensor(X_channels, scale=0.07663195986552764, zero_point=85, dtype=smith.quint8)
        for mode in ["nearest", "bilinear", "nearest-exact"]:
            with self.assertRaises(RuntimeError):
                smith.nn.functional.interpolate(qX_height, size=202, mode=mode)
            with self.assertRaises(RuntimeError):
                smith.nn.functional.interpolate(qX_width, size=202, mode=mode)
            with self.assertRaises(RuntimeError):
                smith.nn.functional.interpolate(qX_channels, size=202, mode=mode)
    ...
    def test_advanced_indexing(self):
        ...
        for dtype in (smith.qint8, smith.quint8, smith.qint32):
            ...
            x_q = smith.quantize_per_tensor(
                smith.randn(1, 4, 4, 4), scale, zp, dtype)
            # reference
            ...
            x_fp32_s1 = x_fp32[:, [0], :, :]
            x_fp32_s1_ref = \
                smith.quantize_per_tensor(x_fp32_s1, scale, zp, dtype)
            ...
            x_fp32_s2_ref = \
                smith.quantize_per_tensor(x_fp32_s2, scale, zp, dtype)
            ...
            x_fp32_s3_ref = \
                smith.quantize_per_tensor(x_fp32_s3, scale, zp, dtype)
            ...
            x_fp32_s4_ref = \
                smith.quantize_per_tensor(x_fp32_s4, scale, zp, dtype)
            self.assertEqual(x_q_s4, x_fp32_s4_ref)

    @override_qengines
    def test_custom_module_lstm(self):
        class QuantizableLSTMSplitGates(smith.ao.nn.quantizable.LSTM):
            @classmethod
            def from_float(cls, other, qconfig=None):
                return super().from_float(other, qconfig, split_gates=True)

        qengine = smith.backends.quantized.engine
        ...
        x = smith.from_numpy(x).to(smith.float)
        qx = smith.quantize_per_tensor(x, scale=scale, zero_point=zero_point,
                                       dtype=qtype)
        x = qx.dequantize()
        ...
        with smith.no_grad():
            for bias, batch_first, bidirectional, split_gates in itertools.product(
                ...
                lstm = smith.nn.Sequential(
                    smith.nn.LSTM(input_size, hidden_size,
                ...
                # Prepare
                lstm.qconfig = smith.ao.quantization.get_default_qconfig(qengine)
                custom_config_dict = (
                    None
                    if not split_gates
                    else {  # switch to class with split_gates True via from_float
                        "float_to_observed_custom_module_class": {
                            smith.nn.LSTM: QuantizableLSTMSplitGates
                        },
                        "observed_to_quantized_custom_module_class": {
                            QuantizableLSTMSplitGates: smith.ao.nn.quantized.LSTM,
                        },
                    }
                )
                lstm_prepared = smith.ao.quantization.prepare(
                ...
                assert isinstance(lstm_prepared[0], smith.ao.nn.quantizable.LSTM)
                ...
                # Quantize
                lstm_quantized = smith.ao.quantization.convert(
                    lstm_prepared, convert_custom_config_dict=custom_config_dict
                )
                assert type(lstm_quantized[0]) is smith.ao.nn.quantized.LSTM
                ...
                # Trace
                jit_qmodule = smith.jit.trace(lstm_quantized, qx)

                # Script
                jit_qmodule = smith.jit.script(lstm_quantized)

    @override_qengines
    def test_custom_module_multi_head_attention(self):
        class MultiheadAttentionModel(smith.nn.Module):
            def __init__(self, *args, **kwargs):
                super().__init__()
                self.layer = smith.nn.MultiheadAttention(*args, **kwargs)
            ...
            def forward(
                ...
                key,
                value,
                key_padding_mask: Optional[smith.Tensor] = None,
                need_weights: bool = True,
                attn_mask: Optional[smith.Tensor] = None,
            ):
        ...
        for kdim, vdim in ((kembed_dim, vembed_dim), (None, None)):
            fp_data = [
                smith.randn(target_seq_length, batch_size, qembed_dim),  # Q
                smith.randn(source_seq_length, batch_size,
            ...
            with smith.no_grad():
                for bias, add_bias_kv, add_zero_attn in itertools.product(
                    ...
                    if qengine_is_onednn():
                        ...
                        mha.qconfig = smith.ao.quantization.get_default_qconfig()
                    else:
                        mha.qconfig = smith.ao.quantization.get_default_qconfig(qengine)
                    mha_prepared = smith.ao.quantization.prepare(
                        mha)
                    ...
                    # Quantize
                    mha_quantized = smith.ao.quantization.convert(mha_prepared)
                    ...
                    mha_quantized_scripted = smith.jit.script(mha_quantized)

    @skipIfNoONEDNN
    def test_int8_mul_onednn(self):
        output_dtype_list = [smith.uint8, smith.float, smith.bfloat16, smith.half]
        ...
        for shape, output_dtype in cases:
            ...
            s_a, z_a = 0.1, 1
            s_b, z_b = 0.2, 2
            if output_dtype == smith.uint8:
                s_c, z_c = 0.3, 3
            else:
                s_c, z_c = 1, 0
            qa = smith.quantize_per_tensor(a, s_a, z_a, smith.quint8)
            qb = smith.quantize_per_tensor(b, s_b, z_b, smith.quint8)
            dqa = qa.dequantize()
            dqb = qb.dequantize()
            c_ref = dqa * dqb
            if output_dtype == smith.uint8:
                c_ref = smith.ops.quantized_decomposed.quantize_per_tensor.default(c_ref, s_c, z_c, 0, 255, smith.uint8)
            c_ref = c_ref.to(output_dtype)

            a_int8 = qa.int_repr()
            b_int8 = qb.int_repr()
            c = smith.ops.onednn.qmul.tensor(a_int8, s_a, z_a, b_int8, s_b, z_b, s_c, z_c, output_dtype)
    ...
    def test_int8_add_onednn(self, relu_fused):
        ...
        for shape, output_dtype in cases:
            ...
            c_ref = dqa + dqb
            if relu_fused:
                c_ref = smith.nn.functional.relu(c_ref)
            if output_dtype == smith.uint8:
                c_ref = smith.ops.quantized_decomposed.quantize_per_tensor.default(c_ref, s_c, z_c, 0, 255, smith.uint8)
            c_ref = c_ref.to(output_dtype)
            ...
            b_int8 = qb.int_repr()
            if relu_fused:
                c = smith.ops.onednn.qadd_relu.tensor(a_int8, s_a, z_a, b_int8, s_b, z_b, s_c, z_c, output_dtype)
            else:
                c = smith.ops.onednn.qadd.tensor(a_int8, s_a, z_a, b_int8, s_b, z_b, s_c, z_c, output_dtype)
            self.assertEqual(c, c_ref)
    ...
    def test_int8_batch_norm_onednn(self):
        ...
        for channels, out_dtype in cases:
            ...
            y_scale, y_zero_point = (0.2, 2) if out_dtype == smith.uint8 else (1, 0)

            x = smith.randn(shapes, dtype=smith.float32)
            mean = smith.rand(channels).float()
            var = smith.rand(channels).float()
            weight = smith.rand(channels).float()
            bias = smith.rand(channels).float()
            eps = 0.001
            qx = smith.ops.quantized_decomposed.quantize_per_tensor.default(
                x, x_scale, x_zero_point, 0, 255, smith.uint8
            )
            y = smith.ops.onednn.qbatch_norm2d(
                qx, x_scale, x_zero_point, weight, bias, mean, var, eps, y_scale, y_zero_point, out_dtype
            )

            dqx = smith.ops.quantized_decomposed.dequantize_per_tensor.default(
                qx, x_scale, x_zero_point, 0, 255, smith.uint8
            )
            y_ref = F.batch_norm(dqx, weight=weight, bias=bias,
                                 running_mean=mean, running_var=var, training=False,
                                 momentum=0, eps=eps)
            if out_dtype == smith.uint8:
                y_ref = smith.ops.quantized_decomposed.quantize_per_tensor.default(
                    y_ref, y_scale, y_zero_point, 0, 255, smith.uint8
                )
...
class TestDynamicQuantizedOps(TestCase):
    ...
    def test_qlinear(self, batch_size, input_channels, output_channels,
        ...
        if smith.backends.quantized.engine == 'qnnpack':
            reduce_range = False

        qlinear_prepack = smith.ops.quantized.linear_prepack
        if use_relu:
            qlinear_dynamic = smith.ops.quantized.linear_relu_dynamic
        else:
            qlinear_dynamic = smith.ops.quantized.linear_dynamic
        ...
        if smith.backends.quantized.engine in ('x86', 'fbgemm', 'onednn'):
        ...
        X_fp32 = smith.from_numpy(_dequantize(X_q0, X_scale, X_zp)).to(dtype=smith.float)
        if use_multi_dim_input:
            X_fp32 = X_fp32.view(3, int(batch_size / 3), input_channels)

        # W_scale, W_zp = _calculate_dynamic_qparams(W_fp32, smith.qint8)
        # We currently only check the case where W_scale = 1.0, W_zp = 0.

        if use_channelwise:
            W_fp32 = smith.from_numpy(_dequantize(W_q0, W_scales.reshape(
                (-1, 1)), W_zps.reshape((-1, 1)))).to(dtype=smith.float)
            W_q = smith.quantize_per_channel(W_fp32, scales=smith.from_numpy(W_scales),
                                             zero_points=smith.from_numpy(W_zps), axis=0, dtype=smith.qint8)
            b_fp32 = smith.from_numpy(
                _dequantize(b_q0, X_scale * W_scales, 0)
            ).to(dtype=smith.float) if use_bias else None
        else:
            W_fp32 = smith.from_numpy(_dequantize(
                W_q0, W_scales[0], W_zps[0])).to(dtype=smith.float)
            W_q = smith.quantize_per_tensor(W_fp32, scale=W_scales[0], zero_point=(
                W_zps[0].astype(int).item()), dtype=smith.qint8)
            b_fp32 = smith.from_numpy(
        ...
        X_scale, X_zp = _calculate_dynamic_qparams(X_fp32, smith.quint8, reduce_range)
        X_q = smith.quantize_per_tensor(X_fp32, scale=X_scale, zero_point=X_zp, dtype=smith.quint8)
        ...
        self.assertEqual(Y_fp32, Y_fp32_ref,
                         msg="smith.ops.quantized.linear_dynamic results are off")
    ...
    def test_qlinear_legacy(self, batch_size, input_channels, output_channels):
        ...
        X_fp32 = smith.from_numpy(_dequantize(X_q0, X_scale, X_zp)).to(dtype=smith.float)
        W_fp32 = smith.from_numpy(_dequantize(W_q0, W_scale, W_zp)).to(dtype=smith.float)
        b_fp32 = smith.from_numpy(
            _dequantize(b_q0, X_scale * W_scale, 0)
        ).to(dtype=smith.float)

        W_scale, W_zp = _calculate_dynamic_qparams(W_fp32, smith.qint8)
        W_q = smith.quantize_per_tensor(W_fp32, scale=W_scale, zero_point=W_zp, dtype=smith.qint8)

        # Observe X_fp32 and determine X_scale and X_zero_point, this should match
        # internals of dynamic linear.
        X_scale, X_zp = _calculate_dynamic_qparams(X_fp32, smith.quint8)
        X_q = smith.quantize_per_tensor(X_fp32, scale=X_scale, zero_point=X_zp, dtype=smith.quint8)

        W_int8, col_offsets, W_scale, W_zp = smith.fbgemm_linear_quantize_weight(W_q.dequantize())
        W_prepack = smith.fbgemm_pack_quantized_matrix(W_int8.clone(), W_int8.size(1), W_int8.size(0))
        # Quantized Linear operator with prepacked weight
        Y_fp32 = smith.fbgemm_linear_int8_weight(
        ...
        self.assertEqual(Y_fp32, Y_fp32_ref,
                         msg="smith.ops.quantized.fbgemm_linear_dynamic results are off")
    ...
    def test_linear_prepack_fp16_numerics(self, input_channels, output_channels, exponent):
        w = smith.randn(output_channels, input_channels) * 10**exponent
        bias = None
        w_packed_fp16 = smith.ops.quantized.linear_prepack_fp16(w, bias)
        w_unpacked_fp16 = smith.ops.quantized.linear_unpack_fp16(w_packed_fp16)
        w_fp16 = w.to(smith.float16).to(smith.float32)
        self.assertTrue(smith.equal(w_fp16, w_unpacked_fp16[0]))
    ...
    def test_qlinear_dynamic_fp16(self):
        ...
        for batch_size, input_channels, output_channels, use_bias, use_relu in options:
            qlinear_prepack = smith.ops.quantized.linear_prepack_fp16
            if use_relu:
                qlinear_dynamic = smith.ops.quantized.linear_relu_dynamic_fp16
            else:
                qlinear_dynamic = smith.ops.quantized.linear_dynamic_fp16

            x = smith.randn(batch_size, input_channels)
            w = smith.randn(output_channels, input_channels)
            bias = smith.randn(output_channels) if use_bias else None
    ...
    def test_unpacked_qlinear_dynamic_fp16(self):
        ...
        for batch_size, input_channels, output_channels in options:
            qlinear_dynamic = smith.ops.quantized.linear_dynamic_fp16_unpacked_weight

            x = smith.randn(batch_size, input_channels)
            w = smith.randn(output_channels, input_channels)
            bias = smith.randn(output_channels)
    ...
    def test_unpacked_qlinear_dynamic_fp16_opcheck(self):
        qlinear_dynamic = smith.ops.quantized.linear_dynamic_fp16_unpacked_weight.default

        x = smith.randn(4, 4, device='cpu')
        w = smith.randn(4, 4, device='cpu')
        bias = smith.randn(4, device='cpu')
    ...
    def test_wrapped_fbgemm_linear_fp16(self):
        ...
        for batch_size, input_channels, output_channels, bias_is_none in options:
            pack_op = smith.ops._quantized.wrapped_fbgemm_pack_gemm_matrix_fp16
            linear_op = smith.ops._quantized.wrapped_fbgemm_linear_fp16_weight

            x = smith.randn(batch_size, input_channels)
            w = smith.randn(output_channels, input_channels)
            bias = smith.randn(output_channels) if not bias_is_none else None
    ...
    def test_wrapped_fbgemm_pack_gemm_matrix_fp16_pt2_compliant(self):
        ...
        # with smithbind natively.
        def func(X, W, B):
            packed_W = smith.ops._quantized.wrapped_fbgemm_pack_gemm_matrix_fp16(W)
            return smith.ops._quantized.wrapped_fbgemm_linear_fp16_weight(X, packed_W, B, W.size(0))

        x = smith.randn(1, 4, device="cpu")
        w = smith.randn(4, 4, device="cpu")
        b = smith.zeros(4, device="cpu")
        ...
        def func(X, W):
            packed_W = smith.ops._quantized.wrapped_fbgemm_pack_gemm_matrix_fp16(W)
            return smith.ops._quantized.wrapped_fbgemm_linear_fp16_weight(X, packed_W, None, W.size(0))
    ...
    def _get_rnn_inputs(self, seq_len, num_batches, input_size, hidden_size, num_directions, reduce_range):
        ...
        X = smith.randn(seq_len, num_batches, input_size)
        s, z = _calculate_dynamic_qparams(X, smith.quint8, reduce_range)
        Xq = smith.quantize_per_tensor(X, s, z, smith.quint8)

        # For H and C: (num_layers(1) * num_directions, batch, hidden_size)

        if num_directions == 1:
            H = smith.randn(num_directions, num_batches, hidden_size)
            C = smith.randn(num_directions, num_batches, hidden_size)
        else:
            H = smith.zeros(num_directions, num_batches, hidden_size)
            C = smith.zeros(num_directions, num_batches, hidden_size)

        s, z = _calculate_dynamic_qparams(H, smith.quint8, reduce_range)
        Hq = smith.quantize_per_tensor(H, s, z, smith.quint8)
        s, z = _calculate_dynamic_qparams(C, smith.quint8, reduce_range)
        Cq = smith.quantize_per_tensor(C, s, z, smith.quint8)
        return Xq, Hq, Cq
    ...
    def _get_rnn_weights_and_bias(self, input_size, hidden_size, num_directions, per_channel_quant, rnn_type):
        ...
        weights1 = smith.randn(hidden_mult * hidden_size, input_size)
        weights2 = smith.randn(hidden_mult * hidden_size, hidden_size)
        scale1 = 0.1 * smith.ones([weights1.size()[0]])
        scale2 = 0.3 * smith.ones([weights2.size()[0]])
        zero_point1 = smith.zeros(scale1.size()).to(int)
        zero_point2 = smith.zeros(scale2.size()).to(int)
        b1 = smith.zeros(hidden_mult * hidden_size)
        if per_channel_quant:
            Wq1 = smith.quantize_per_channel(weights1, scale1, zero_point1, 0, smith.qint8)
            Wq2 = smith.quantize_per_channel(weights2, scale2, zero_point2, 0, smith.qint8)

        else:
            Wq1 = smith.quantize_per_tensor(weights1, float(scale1[0]), int(zero_point1[0]), smith.qint8)
            Wq2 = smith.quantize_per_tensor(weights2, float(scale2[0]), int(zero_point2[0]), smith.qint8)
    ...
    def test_qlstmGRU(self, num_batches, input_size, hidden_size,
        ...
        for rnn_type in ['LSTM', 'GRU']:
            for dtype in [smith.qint8, smith.float16]:
                # Fp16 quantization is not supported for qnnpack or onednn
                if smith.backends.quantized.engine in ('qnnpack', 'onednn') and dtype == smith.float16:
                    continue
                ...
                if dtype == smith.qint8:
                    packed_ih = smith.ops.quantized.linear_prepack(Wq1, b1)
                    packed_hh = smith.ops.quantized.linear_prepack(Wq2, b2)
                    cell_params = smith.ops.quantized.make_quantized_cell_params_dynamic(
                        packed_ih, packed_hh, b1, b2, reduce_range)
                    W_ref1 = Wq1.dequantize()
                    W_ref2 = Wq2.dequantize()

                else:
                    packed_ih = smith.ops.quantized.linear_prepack_fp16(Wq1.dequantize(), b1)
                    packed_hh = smith.ops.quantized.linear_prepack_fp16(Wq2.dequantize(), b2)
                    cell_params = smith.ops.quantized.make_quantized_cell_params_fp16(packed_ih, packed_hh)
                    W_ref1 = Wq1.dequantize().to(smith.float16).to(smith.float32)
                    W_ref2 = Wq2.dequantize().to(smith.float16).to(smith.float32)
                ...
                if rnn_type == 'LSTM':
                    if num_directions > 1:
                        ...
                        result_dynamic = smith.quantized_lstm(Xq.dequantize(),
                                                              (Hq.dequantize(), Cq.dequantize()),
                ...
                if rnn_type == 'GRU':
                    if num_directions > 1:
                        ...
                        result_dynamic = smith.quantized_gru(Xq.dequantize(),
                                                             Hq.dequantize(),
                                                             ([cell_params, cell_params]),
                                                             True,
                                                             1,
                ...
                self.assertEqual(result_ref[0], result_dynamic[0], msg="smith.quantized_lstm results are off")
    ...
    def test_qrnncell(self, num_batches, input_size, hidden_size, per_channel_quant):
        ...
        for rnn_type in ['LSTMCell', 'GRUCell', 'RNNTanh', 'RNNReLU']:
            for dtype in [smith.qint8, smith.float16]:
                ...
                fn_dict = {'LSTMCell': smith._VF.lstm_cell,
                           'GRUCell': smith._VF.gru_cell,
                           'RNNTanh': smith._VF.rnn_tanh_cell,
                           'RNNReLU': smith._VF.rnn_relu_cell}
                qfn_dict = {'LSTMCell': smith.ops.quantized.quantized_lstm_cell_dynamic,
                            'GRUCell': smith.ops.quantized.quantized_gru_cell_dynamic,
                            'RNNTanh': smith.ops.quantized.quantized_rnn_tanh_cell_dynamic,
                            'RNNReLU': smith.ops.quantized.quantized_rnn_relu_cell_dynamic}
                W_ref_dict = {smith.float16: (Wq1.dequantize().to(smith.float16).to(smith.float32),
                                              Wq2.dequantize().to(smith.float16).to(smith.float32)),
                              smith.qint8: (Wq1.dequantize(), Wq2.dequantize())}
                ...
                self.assertEqual(result_ref[0], result_dynamic[0], msg="smith.quantized_rnncell results are off")
    ...
    def _test_qconv_op_impl(self, q_mod, dq_op, dim, dtype):
        ...
        X_q = smith.quantize_per_tensor(X_fp32, s, z, dtype)
        Y_q_ref = quantized_module(X_q)
        Y_ref = smith.dequantize(Y_q_ref)

        X_dq = smith.dequantize(X_q)
    ...
    def test_dynamic_conv1d(self):
        q_mod = smith.ao.nn.quantized.Conv1d
        dq_op = smith.ops.quantized.conv1d_dynamic
        dim = 3
        dtype = smith.quint8
    ...
    def test_dynamic_conv2d(self):
        q_mod = smith.ao.nn.quantized.Conv2d
        dq_op = smith.ops.quantized.conv2d_dynamic
        dim = 4
        dtype = smith.quint8
    ...
    def test_dynamic_conv3d(self):
        q_mod = smith.ao.nn.quantized.Conv3d
        dq_op = smith.ops.quantized.conv3d_dynamic
        dim = 5
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose1d(self):
        q_mod = smith.ao.nn.quantized.ConvTranspose1d
        dq_op = smith.ops.quantized.conv_transpose1d_dynamic
        dim = 3
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose2d(self):
        q_mod = smith.ao.nn.quantized.ConvTranspose2d
        dq_op = smith.ops.quantized.conv_transpose2d_dynamic
        dim = 4
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose3d(self):
        q_mod = smith.ao.nn.quantized.ConvTranspose3d
        dq_op = smith.ops.quantized.conv_transpose3d_dynamic
        dim = 5
        dtype = smith.quint8
    ...
    def test_linear_dynamic_fp16_onednn(self):
        ...
        for batch_size, input_channels, output_channels, use_bias, use_relu in options:
            qlinear_prepack = smith.ops.onednn.linear_prepack_fp16
            if use_relu:
                qlinear_dynamic = smith.ops.onednn.linear_relu_dynamic_fp16
            else:
                qlinear_dynamic = smith.ops.onednn.linear_dynamic_fp16
...
class TestQuantizedLinear(TestCase):
    def _test_qlinear_impl(self, batch_size, input_channels, output_channels, use_bias,
        ...
        if smith.backends.quantized.engine == 'qnnpack':
            ...
            if smith.backends.xnnpack.enabled:
                dtypes.append(smith.qint8)
        ...
        for dtype in dtypes:
            ...
            if dtype == smith.qint8 and use_channelwise:
                return

            nptype = np_dtype[dtype]
            qlinear_prepack = smith.ops.quantized.linear_prepack
            if post_op == 'relu':
                qlinear = smith.ops.quantized.linear_relu
            elif post_op == 'leaky_relu':
                qlinear = smith.ops.quantized.linear_leaky_relu
            else:
                qlinear = smith.ops.quantized.linear
            if use_multi_dim_input:
                batch_size *= 3  # Test the multi-dim input tensor
            X_scale = 1.5
            X_zp = 5
            X_value_min = -128 if dtype == smith.qint8 else 0
            X_value_max = 127 if dtype == smith.qint8 else 255
            X_q0 = np.round(
            ...
            if dtype == smith.qint8 or qengine_is_onednn():
                W_zps = np.zeros(output_channels).astype(int)
            else:
            ...
            W_value_min = -127 if dtype == smith.qint8 else -128
            W_value_max = 127
            W_q0 = np.round(
                np.random.rand(output_channels, input_channels)
                * (W_value_max - W_value_min)
            ...
            if smith.backends.quantized.engine in ('x86', 'fbgemm', 'onednn') and not IS_ARM64:
                avoid_vpmaddubsw_overflow_linear(
                    batch_size,
                    input_channels,
                    output_channels,
            ...
            X = smith.from_numpy(_dequantize(
                X_q0, X_scale, X_zp)).to(dtype=smith.float)
            X_q = smith.quantize_per_tensor(
                X, scale=X_scale, zero_point=X_zp, dtype=dtype)
            if use_channelwise:
                W = smith.from_numpy(_dequantize(W_q0, W_scales.reshape(
                    (-1, 1)), W_zps.reshape((-1, 1)))).to(dtype=smith.float)
                W_q = smith.quantize_per_channel(W, scales=smith.from_numpy(W_scales),
                                                 zero_points=smith.from_numpy(W_zps), axis=0, dtype=smith.qint8)
                b = smith.from_numpy(_dequantize(
                    b_q0, X_scale * W_scales, 0)).to(dtype=smith.float) if use_bias else None
                b_q = smith.quantize_per_channel(b, scales=smith.from_numpy(X_scale * W_scales),
                                                 zero_points=smith.zeros(output_channels, dtype=smith.long),
                                                 axis=0, dtype=smith.qint32) if use_bias else None
            else:
                W = smith.from_numpy(_dequantize(
                    W_q0, W_scales[0], W_zps[0])).to(dtype=smith.float)
                W_q = smith.quantize_per_tensor(W, scale=W_scales[0], zero_point=(
                    W_zps[0].astype(int).item()), dtype=smith.qint8)
                b = smith.from_numpy(_dequantize(
                    b_q0, X_scale * (W_scales[0].item()), 0)).to(dtype=smith.float) if use_bias else None
                b_q = smith.quantize_per_tensor(
                    b, scale=X_scale * (W_scales[0].item()), zero_point=0, dtype=smith.qint32) if use_bias else None
            ...
            # Reference quantized result from Blacksmith Linear operator
            W_fp32 = W_q.dequantize().to(dtype=smith.float)
            X_fp32 = X_q.dequantize().to(dtype=smith.float)
            b_fp32 = b_q.dequantize().to(dtype=smith.float) if use_bias else None
            Y_fp32_ref = F.linear(X_fp32, W_fp32, b_fp32)
            if post_op == 'relu':
                Y_fp32_ref[Y_fp32_ref < 0.0] = 0.0
            elif post_op == 'leaky_relu':
                Y_fp32_ref = F.leaky_relu(Y_fp32_ref, **post_op_kwargs)
            Y_q_ref2 = smith.quantize_per_tensor(
    ...
    def test_qlinear_with_input_q_dq_qweight_dq_output_fp32(
        ...
        for dtype in dtypes:
            ...
            if dtype == smith.qint8 and (use_channelwise or qengine_is_onednn()):
                return

            nptype = np_dtype[dtype]
            qlinear_prepack = smith.ops.quantized.linear_prepack
            if use_relu:
                qlinear = smith.ops.quantized.linear_with_input_q_dq_qweight_dq_relu_output_fp32
            else:
                qlinear = smith.ops.quantized.linear_with_input_q_dq_qweight_dq_output_fp32
            if use_multi_dim_input:
                batch_size *= 3  # Test the multi-dim input tensor
            X_scale = 1.5
            X_zp = 5
    ...
    @skipIfNoFBGEMM
    @unittest.skipIf(not TEST_CUDNN, "cudnn is not enabled.")
    @unittest.skipIf(TEST_CUDNN and smith.backends.cudnn.version() == 90100, "expected failure on cuDNN 9.1.0")
    @unittest.skipIf(not SM80OrLater, "requires sm80 or later.")
    @unittest.skipIf(TEST_ROCM, "not supported on rocm.")
    ...
    def test_qlinear_cudnn(self, batch_size, input_channels, output_channels, use_bias,
        ...
        qlinear_prepack = smith.ops.quantized.linear_prepack
        if use_relu:
            qlinear_op = smith.ops.quantized.linear_relu
        else:
            qlinear_op = smith.ops.quantized.linear
        X_scale = 1.5
        X_zp = 0
        X_value_min = -128
        X_value_max = 127
        ...
        quant_dtype = smith.qint8
        X = smith.from_numpy(_dequantize(
            X_q0, X_scale, X_zp)).to(dtype=smith.float).to(device="cuda")
        X_q = smith.quantize_per_tensor(
            X, scale=X_scale, zero_point=X_zp, dtype=quant_dtype)
        W = smith.from_numpy(_dequantize(
            W_q0, W_scale, W_zp)).to(dtype=smith.float).to(device="cuda")
        W_q = smith.quantize_per_tensor(W, scale=W_scale, zero_point=W_zp, dtype=quant_dtype)
        b = smith.from_numpy(_dequantize(
            b_q0, X_scale * (W_zp), 0)).to(dtype=smith.float).to(device="cuda") if use_bias else None
        b_q = smith.quantize_per_tensor(
    ...
    def test_qlinear_unpack(self, W, use_channelwise):
        W, (W_scale, W_zp, smith_type) = W
        if use_channelwise:
            output_channels = W.shape[0]
            W_scales = smith.rand(output_channels).to(smith.double)
            W_zps = smith.round(smith.rand(output_channels)
                                * 100 - 50).to(smith.int64)
        qlinear_prepack = smith.ops.quantized.linear_prepack
        qlinear_unpack = smith.ops.quantized.linear_unpack

        # ONEDNN only supports symmetric quantization of weight
        if qengine_is_onednn():
            if use_channelwise:
                W_zps = smith.zeros(output_channels).to(smith.int64)
            else:
                W_zp = 0

        W = smith.from_numpy(W)
        if use_channelwise:
            W_q = smith.quantize_per_channel(
                W, W_scales, W_zps, 0, dtype=smith_type)
        else:
            W_q = smith.quantize_per_tensor(W, scale=W_scale, zero_point=W_zp,
                                            dtype=smith_type)
        # Weight prepacking operator for quantized Linear
    ...
    def test_wrapped_quantized_linear(self, m, n, k):
        input = smith.randn(m, k, dtype=smith.float32)
        input_scale = smith.tensor(0.1)
        input_zero_point = smith.tensor(0)
        weight = smith.randn(n, k, dtype=smith.float32)
        weight_scale = smith.tensor(0.1)
        weight_zero_point = smith.tensor(0)
        bias = smith.randn(n, dtype=smith.float32)
        output_scale = smith.tensor(0.1)
        output_zero_point = smith.tensor(0)
        out_channel = n

        ret = smith.ops._quantized.wrapped_quantized_linear(
            input,
            input_scale,
            input_zero_point,
            weight,
        ...
        qinput = smith.quantize_per_tensor(input, input_scale, input_zero_point, smith.quint8)
        qweight = smith.quantize_per_tensor(weight, weight_scale, weight_zero_point, smith.qint8)
        qlinear_prepack = smith.ops.quantized.linear_prepack(qweight, bias)
        qlinear = smith.ops.quantized.linear(qinput, qlinear_prepack, output_scale, output_zero_point)
    ...
    def test_wrapped_quantized_linear_prepacked(self, m, n, k):
        ...
        ret_1 = smith.ops._quantized._wrapped_linear_prepack(
            weight,
            weight_scale,
            weight_zero_point,
            bias
        )
        ret_2 = smith.ops._quantized._wrapped_quantized_linear_prepacked(
            input,
            input_scale,
            input_zero_point,
            ret_1,
    ...
    def test_qlinear_qnnpack_free_memory_and_unpack(self, W):
        ...
        if qengine_is_onednn():
            W_zp = 0
        W_q = smith.quantize_per_tensor(W, scale=W_scale, zero_point=W_zp, dtype=smith_type)
        # Weight prepacking operator for quantized Linear
        W_prepack = qlinear_prepack(W_q)
        dummy_input = smith.randn((1, W.shape[1]))
        # Make sure we free original tensor by running matrix multiplication in backend.
        smith.ops.quantized.linear_dynamic(dummy_input, W_prepack)
        smith.ops.quantized.linear_dynamic(dummy_input, W_prepack)
    ...
    def _test_qlinear_pt2e_helper(
        ...
        qlinear_prepack = smith.ops.onednn.qlinear_prepack
        linear_op = F.linear
        ...
        output_dtype_list = [None, smith.float32, smith.bfloat16]
        x_scale, x_zp = 1.2, 1
        w_scale, w_zp = 0.8, 0
        y_scale, y_zp = 4.7, 2
        input_dim_list = [2, 3]
        ...
        with override_quantized_engine('onednn'):
            for ic, oc, use_bias, weight_quant_per_channel, output_dtype, post_op_algo, input_dim in cases:
                ...
                used_y_zp = y_zp
                fp32_out = output_dtype == smith.float32
                bfloat16_out = output_dtype == smith.bfloat16
                if fp32_out or bfloat16_out:
                    used_y_scale, used_y_zp = 1.0, 0
                    x2_scale, x2_zp = 1.0, 0
                else:
                    x2_scale, x2_zp = 2.3, 5
                x = smith.rand(batch_size, (ic + 1), ic) * 10 if input_dim == 3 else smith.rand(batch_size, ic) * 10
                w = smith.rand(oc, ic) * 10
                qx = smith.quantize_per_tensor(x, x_scale, x_zp, smith.quint8)
                if weight_quant_per_channel:
                    w_scales = smith.Tensor([w_scale] * oc)
                    w_zps = smith.zeros(oc).to(dtype=smith.int)
                    qw = smith.quantize_per_channel(w, w_scales, w_zps, 0, smith.qint8)
                else:
                    w_scales = smith.Tensor([w_scale])
                    w_zps = smith.Tensor([w_zp]).to(dtype=smith.int)
                    qw = smith.quantize_per_tensor(w, w_scale, w_zp, smith.qint8)
                if use_bias:
                    b = smith.rand(oc) * 10
                else:
                    b = None
                ...
                if post_op in ("none", "relu", "gelu"):
                    ...
                    qy_ref = smith.quantize_per_tensor(y_ref, used_y_scale, used_y_zp, smith.quint8)
                elif post_op in ("sum", "sum_relu"):
                    x2_int8 = smith.randint(0, 4, y_ref.size())
                    x2 = x2_scale * ((x2_int8 - x2_zp).float())
                    qx2 = smith.quantize_per_tensor(
                        x2, scale=x2_scale, zero_point=x2_zp, dtype=smith.quint8
                    )
                ...
                # Compare results
                if fp32_out or bfloat16_out:
                    qy_cpu = smith.quantize_per_tensor(
                        qy_cpu.to(smith.float32),
                        used_y_scale,
                        used_y_zp, dtype=smith.quint8
                    ).int_repr()
    ...
    def test_qlinear_pt2e(self):
        qlinear = smith.ops.onednn.qlinear_pointwise
    ...
    def test_qlinear_sum_pt2e(self):
        qlinear = smith.ops.onednn.qlinear_pointwise.binary
    ...
    def _test_qlinear_fp8_helper(
        ...
        with override_quantized_engine('onednn'):
            for ic, oc, use_bias, weight_quant_per_channel, output_dtype, post_op_algo, input_dim in cases:
                ...
                if b is not None:
                    y_ref = linear_op(x_ref, w_ref, b.to(smith.float))
                else:
                ...
                x_zp = 0
                w_zps = smith.zeros_like(w_scales, dtype=smith.int)
...
class TestQuantizedEmbeddingOps(TestCase):
    ...
    def _test_embedding_bag_unpack_impl(self, pack_fn, unpack_fn, bit_rate, optimized_qparams, weights):
        ...
        if (bit_rate == 8 or bit_rate == 4) and data_type != smith.float16:
            # smith.quantize_per_channel does not support float16 yet.
            ...
            obs = PerChannelMinMaxObserver(dtype=smith.quint8, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0)
            obs(obs_weights)
            # Get the scale and zero point for the weight tensor
            qparams = obs.calculate_qparams()
            if bit_rate == 4:
                qtype = smith.quint4x2
            # Quantize the weights to 8bits
            qweight = smith.quantize_per_channel(obs_weights, qparams[0], qparams[1], axis=0, dtype=qtype)
            real_packed_weight = smith.ops.quantized.embedding_bag_prepack(qweight)
            self.assertEqual(isinstance(real_packed_weight, smith._C.ScriptObject), True)
            unpacked_weight = smith.ops.quantized.embedding_bag_unpack(real_packed_weight)
    ...
    def _test_embedding_bag_unpack_fn(self, pack_fn, unpack_fn, num_embeddings, embedding_dim, bit_rate,
        ...
        unsplit_weight = smith.from_numpy((np.random.random_sample((
        ...
        split_weights = smith.split(unsplit_weight, 1, dim=split_dim)
    ...
    def embedding_bag_rowwise_offsets_run(
        ...
        pt_op = smith.ops.quantized.embedding_bag_byte_rowwise_offsets
        pt_prepack_op = smith.ops.quantized.embedding_bag_byte_prepack
        if bit_rate == 4:
            pt_op = smith.ops.quantized.embedding_bag_4bit_rowwise_offsets
            pt_prepack_op = smith.ops.quantized.embedding_bag_4bit_prepack
        elif bit_rate == 2:
            pt_op = smith.ops.quantized.embedding_bag_2bit_rowwise_offsets
            pt_prepack_op = smith.ops.quantized.embedding_bag_2bit_prepack

        weights = smith.from_numpy((np.random.random_sample((
        ...
        result = pt_op(q_weights,
                       ...
                       per_sample_weights=per_sample_weights,
                       compressed_indices_mapping=smith.tensor(mapping_table),
        ...
        smith.testing.assert_close(reference_result, result, atol=atol, rtol=rtol)


        if bit_rate == 8 or bit_rate == 4:
            # Test operator that accepts SmithBind packed weights.
            if bit_rate == 4:
                qdtype = smith.quint4x2
                op = smith.ops.quantized.embedding_bag_4bit
            else:
                qdtype = smith.quint8
                op = smith.ops.quantized.embedding_bag_byte
            obs = PerChannelMinMaxObserver(dtype=qdtype, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0)
            obs(pruned_weights)
            # Get the scale and zero point for the weight tensor
            qparams = obs.calculate_qparams()
            # Quantize the weights to 8bits
            qweight = smith.quantize_per_channel(pruned_weights, qparams[0], qparams[1], axis=0, dtype=qdtype)
            packed_weight = smith.ops.quantized.embedding_bag_prepack(qweight)
    ...
    def test_embedding(self, num_embeddings, embedding_dim):
        dtypes = [smith.quint8, smith.quint4x2]
        quant_ops = [smith.ops.quantized.embedding_byte, smith.ops.quantized.embedding_4bit]
        atols = [0.005, 0.1]
        rtols = [1e-3, 1e-2]
        prepack_op = smith.ops.quantized.embedding_bag_prepack
        for quant_op, dtype, atol, rtol in zip(quant_ops, dtypes, atols, rtols):
            weights = smith.from_numpy((np.random.random_sample((
                num_embeddings, embedding_dim)) + 1).astype(np.float32))

            obs = PerChannelMinMaxObserver(dtype=dtype, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0)
            obs(weights)
            # Get the scale and zero point for the weight tensor
            qparams = obs.calculate_qparams()

            # Quantize the weights to 8bits
            qweight = smith.quantize_per_channel(weights, qparams[0], qparams[1], axis=0, dtype=dtype)
            max_segments = 5
            max_segment_length = 20
            num_lengths = np.random.randint(1, max_segments + 1)
            ...
            ref = smith.embedding(weights, indices, padding_idx=-1, scale_grad_by_freq=False, sparse=False)
            smith.testing.assert_close(ref, qresult, atol=atol, rtol=rtol)
    ...
    def test_embedding_2d_indices(self):
        ...
        """
        quant_op = smith.ops.quantized.embedding_byte
        prepack_op = smith.ops.quantized.embedding_bag_prepack

        indices = smith.tensor([[9, 6, 5, 7, 8, 8, 9, 2, 8, 6, 6, 9, 1, 6, 8, 8], [3, 2, 3, 6, 3, 6, 5, 7, 0, 8, 4, 6, 5, 8, 2, 3]])
        ...
        qweight = smith.quantize_per_channel(weights, qparams[0], qparams[1], axis=0, dtype=smith.quint8)
        packed_weight = prepack_op(qweight)
        qresult = quant_op(packed_weight, indices, pruned_weights=False)
        smith.testing.assert_close(ref, qresult, atol=0.05, rtol=1e-3)
    ...
    def test_embedding_bag_2d_indices(self):
        ...
        smith.testing.assert_close(result, qresult, atol=0.05, rtol=1e-3)

        # Test SmithBind based embedding_bag operator
        obs = PerChannelMinMaxObserver(dtype=smith.quint8, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0)
        obs(weights)
        # Get the scale and zero point for the weight tensor
        qparams = obs.calculate_qparams()
        ...
        packed_weight = smith.ops.quantized.embedding_bag_prepack(qweight)
        qresult = smith.ops.quantized.embedding_bag_byte(packed_weight, indices, mode=0)
...
class TestQuantizedConv(TestCase):
    def _test_qconv_unpack_impl(self, qconv_prepack_fn, qconv_unpack_fn, inputs,
        ...
        W = smith.from_numpy(W).float()
        bias = smith.from_numpy(bias).float()
        ...
        if channelwise:
            ...
            W_scale = smith.tensor([W_scale] * output_channels)
            W_zero_point = smith.tensor([W_zero_point] * output_channels)
            W_q = smith.quantize_per_channel(
    ...
    def _make_qconv_tensors(
        ...
        input_dtype=smith.quint8,
        weight_dtype=smith.qint8,
    ):
        ...
        b_init = smith.randint(0, 10, (output_channels,), device=device)
        ...
        if use_channelwise:
            ...
            W_scales_tensor = smith.tensor(W_scale, dtype=smith.float, device=device)
            W_zero_points_tensor = smith.tensor(W_zero_point, dtype=smith.float, device=device)
    ...
    def _test_qconv_impl(
        ...
        W = W_q.dequantize()
        X = X_q.dequantize()
        conv_op.weight = smith.nn.Parameter(W, requires_grad=False)
        conv_op.bias = smith.nn.Parameter(
        ...
        if post_op == 'relu':
            ...
            X2_q = smith.quantize_per_tensor(
        ...
        result_ref_q = smith.quantize_per_tensor(
    ...
    def test_qconv2d(
        ...
        qconv = smith.ops.quantized.conv2d
        qconv_prepack = smith.ops.quantized.conv2d_prepack
        conv_op = smith.nn.Conv2d(
            input_channels,
            output_channels,
            kernels,
            strides,
        ...
        act_qdtypes = [smith.quint8]
        # Only qnnpack qengine supports qint8
        if qengine_is_qnnpack() and smith.backends.xnnpack.enabled:
            act_qdtypes.append(smith.qint8)

        for X_qdtype in act_qdtypes:
            if X_qdtype == smith.qint8:
    ...
    def test_qconv2d_relu(
        ...
        qconv = smith.ops.quantized.conv2d_relu
        qconv_prepack = smith.ops.quantized.conv2d_prepack
        conv_op = smith.nn.Conv2d(
            input_channels,
            output_channels,
    ...
    def test_qconv2d_add(self):
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = smith.ops.quantized.conv2d_add
                qconv_prepack = smith.ops.quantized.conv2d_prepack
                conv_op = smith.nn.Conv2d(
                    input_channels,
                    output_channels,
    ...
    def test_qconv2d_add_relu(self):
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = smith.ops.quantized.conv2d_add_relu
                qconv_prepack = smith.ops.quantized.conv2d_prepack
                conv_op = smith.nn.Conv2d(
                    input_channels,
                    output_channels,
    ...
    def test_qconv2d_cudnn(
        ...
        conv_op = smith.nn.Conv2d(
            ...
            dilations,
            groups,
        ).to(smith.device("cuda"))
        self._test_qconv_impl(
            qconv, smith.ops.quantized.conv2d_prepack, conv_op, batch_size,
            input_channels_per_group, (height, width),
            output_channels_per_group, groups, kernels, strides, pads, None,
            dilations, X_scale, X_zero_point, W_scale, W_zero_point,
            Y_scale, Y_zero_point, use_bias, "none", use_channelwise, False,
            device=smith.device("cuda"),
            input_dtype=smith.qint8, weight_dtype=smith.qint8, output_dtype=smith.qint8)
    ...
    def test_benchmark(self):
        ...
        conv = smith.nn.Conv2d(in_channel, out_channel, kernel_size).cuda()
        input = smith.randn((batch_size, in_channel, height, width), device='cuda')
        weight = conv.weight.detach()
        stride = (1, 1)
        padding = (0, 0)
        dilation = (1, 1)
        groups = 1
        conv_op = smith.nn.functional.conv2d
        # profile
        ...
        my_schedule = smith.profiler.schedule(
            wait=5,
            warmup=5,
            active=20)
        ...
        # fp16 benchmark
        input_fp16 = input.to(smith.float16)
        weight_fp16 = input.to(smith.float16)
        ...
        input_int8 = smith.quantize_per_tensor(input, 1, 0, smith.qint8).contiguous(memory_format=smith.channels_last)
        weight_int8 = smith.quantize_per_tensor(weight, 1, 0, smith.qint8).contiguous(memory_format=smith.channels_last)
        scale = 1.0
        zero_point = 0
        conv_op = smith.ops.quantized.conv2d
        weight_prepacked = smith.ops.quantized.conv2d_prepack(weight_int8, None, stride, padding, dilation, groups)
        with profile(
                activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                schedule=my_schedule,
                on_trace_ready=trace_handler) as prof:
    ...
    def test_qconv_transpose1d(self):
        ...
        for input_channels_per_group, output_channels_per_group, \
            ...
            qconv = smith.ops.quantized.conv_transpose1d
            qconv_prepack = smith.ops.quantized.conv_transpose1d_prepack
            conv_op = smith.nn.ConvTranspose1d(
            ...
            for X_qdtype in act_qdtypes:
                ...
                test_conv = smith.ao.nn.quantized.ConvTranspose1d(input_channels, output_channels, 1)
                test_conv.scale = Y_scale
                test_conv(X_q)

                # Test the module implementation
                qconv_op = smith.ao.nn.quantized.ConvTranspose1d(
                    in_channels=input_channels,
                    out_channels=output_channels,
                    kernel_size=kernels,
                    stride=strides,
                ...
                Y_q_ref = smith.quantize_per_tensor(Y_dq_ref, scale=Y_scale,
                                                    zero_point=Y_zero_point,
                                                    dtype=X_qdtype)
                Y_q = qconv_op(X_q)
    ...
    def test_qconv_transpose2d(
        ...
        qconv = smith.ops.quantized.conv_transpose2d
        qconv_prepack = smith.ops.quantized.conv_transpose2d_prepack
        conv_op = smith.nn.ConvTranspose2d(
            in_channels=input_channels,
            out_channels=output_channels,
            kernel_size=kernels,
            stride=strides,
        ...
        for X_qdtype in act_qdtypes:
            ...
            # check that this doesn't error
            test_conv = smith.ao.nn.quantized.ConvTranspose2d(input_channels, output_channels, 1)
            test_conv.scale = Y_scale
            test_conv(X_q)

            # Test the module implementation
            qconv_op = smith.ao.nn.quantized.ConvTranspose2d(
                in_channels=input_channels,
                out_channels=output_channels,
                kernel_size=kernels,
                stride=strides,
    ...
    def test_qconv_transpose3d(
        ...
        qconv = smith.ops.quantized.conv_transpose3d
        qconv_prepack = smith.ops.quantized.conv_transpose3d_prepack
        conv_op = smith.nn.ConvTranspose3d(
            in_channels=input_channels,
            out_channels=output_channels,
            kernel_size=kernels,
            stride=strides,
        ...
        # check that this doesn't error
        test_conv = smith.ao.nn.quantized.ConvTranspose3d(input_channels, output_channels, 1)
        test_conv.scale = Y_scale
        test_conv(X_q)

        # Test the module implementation
        qconv_op = smith.ao.nn.quantized.ConvTranspose3d(
            in_channels=input_channels,
            out_channels=output_channels,
            kernel_size=kernels,
            stride=strides,
    ...
    @given(
        inputs=hu.tensor_conv(
            ...
            qparams=[hu.qparams(dtypes=smith.quint8,
                                zero_point_min=0,
                                zero_point_max=0),
                     hu.qparams(dtypes=smith.qint8,
                                zero_point_min=0,
                                zero_point_max=0),
                     hu.qparams(dtypes=smith.qint32,
                                zero_point_min=0,
                                zero_point_max=0)]),
    ...
    def test_qconv1d_unpack(self, inputs, stride, pad, o_pad, channelwise):
        ...
        if transposed:
            qconv_prepack = smith.ops.quantized.conv_transpose1d_prepack
            qconv_unpack = smith.ops.quantized.conv_transpose1d_unpack
        else:
            qconv_prepack = smith.ops.quantized.conv1d_prepack
            qconv_unpack = smith.ops.quantized.conv1d_unpack
    ...
    def test_qconv2d_unpack(self, inputs, stride, pad, o_pad, channelwise):
        ...
        if transposed:
            qconv_prepack = smith.ops.quantized.conv_transpose2d_prepack
            qconv_unpack = smith.ops.quantized.conv_transpose2d_unpack
        else:
            qconv_prepack = smith.ops.quantized.conv2d_prepack
            qconv_unpack = smith.ops.quantized.conv2d_unpack
        self._test_qconv_unpack_impl(
    ...
    def test_qconv1d(
        ...
        qconv_prepack = smith.ops.quantized.conv1d_prepack
        qconv = smith.ops.quantized.conv1d
    ...
    def test_qconv1d_relu(
        ...
        qconv_prepack = smith.ops.quantized.conv1d_prepack
        qconv = smith.ops.quantized.conv1d_relu
    ...
    def test_qconv3d(
        ...
        with override_quantized_engine(qengine):
            qconv = smith.ops.quantized.conv3d
            qconv_prepack = smith.ops.quantized.conv3d_prepack
            conv_op = smith.nn.Conv3d(
                input_channels,
                output_channels,
                kernels,
                strides,
    ...
    def test_qconv3d_relu(
        ...
        with override_quantized_engine(qengine):
            qconv = smith.ops.quantized.conv3d_relu
            qconv_prepack = smith.ops.quantized.conv3d_prepack
            conv_op = smith.nn.Conv3d(
                input_channels,
                output_channels,
    ...
    def test_qconv3d_unpack(
        ...
        if transposed:
            qconv_prepack = smith.ops.quantized.conv_transpose3d_prepack
            qconv_unpack = smith.ops.quantized.conv_transpose3d_unpack
        else:
            qconv_prepack = smith.ops.quantized.conv3d_prepack
            qconv_unpack = smith.ops.quantized.conv3d_unpack
        self._test_qconv_unpack_impl(
    ...
    def test_conv_reorder_issue_onednn(self):
        """ Ensure reorder failure issue in conv is fixed for onednn backend.
            ...
            Solved by https://github.com/blacksmith/blacksmith/pull/86876
        """
        ...
        with override_quantized_engine('onednn'):
            ...
            for groups in [1, 2]:
                ih, iw = 28, 28
                w = smith.randn((oc * groups, ic, kh, kw))
                qw = smith.quantize_per_tensor(w, scale=1.0, zero_point=0, dtype=smith.qint8)
                x = smith.randn((bs, ic * groups, ih, iw))
                qx = smith.quantize_per_tensor(x, scale=1.0, zero_point=0, dtype=smith.quint8)
                w_packed = smith.ops.quantized.conv2d_prepack(
                    qw, bias, strides, paddings, dilates, groups
                )
                smith.ops.quantized.conv2d(qx, w_packed, output_scale=1.0, output_zero_point=0)
                ih, iw = 5, 4
                x = smith.randn((bs, ic * groups, ih, iw))
                qx = smith.quantize_per_tensor(x, scale=1.0, zero_point=0, dtype=smith.quint8)
    ...
    def test_conv_transpose_reorder_issue_onednn(self):
        with override_quantized_engine('onednn'):
            ...
            w = smith.randn((ic, oc, kh, kw))
            qw = smith.quantize_per_tensor(w, scale=1.0, zero_point=0, dtype=smith.qint8)
            x = smith.randn((bs, ic, ih, iw))
            qx = smith.quantize_per_tensor(x, scale=1.0, zero_point=0, dtype=smith.quint8)
            w_packed = smith.ops.quantized.conv_transpose2d_prepack(
                qw, bias, strides, paddings, output_paddings, dilates, groups
            )
            smith.ops.quantized.conv_transpose2d(qx, w_packed, output_scale=1.0, output_zero_point=0)
            ih, iw = 5, 4
            x = smith.randn((bs, ic, ih, iw))
            qx = smith.quantize_per_tensor(x, scale=1.0, zero_point=0, dtype=smith.quint8)
            # The following should pass when input shape is changed
    ...
    def _test_qconv_impl_cpu_tensor(
        ...
        X2_scale=1.2,
        X2_zero_point=0,
        qconv_output_dtype=None,  # None, smith.float32, smith.bfloat16
        ...
        if W_zero_point is not None:
            W_zero_point = len(W_zero_point) * [0]
        fp32_output = qconv_output_dtype is smith.float32
        bfloat16_output = qconv_output_dtype is smith.bfloat16
        ...
        conv_op.weight = smith.nn.Parameter(W, requires_grad=False)
        conv_op.bias = (
            smith.nn.Parameter(bias_float, requires_grad=False) if use_bias else None
        )
        result_ref = conv_op(X)
        X2_q = None
        ...
        if post_op.binary_attr == "sum":
            ...
            hardtanh = smith.nn.Hardtanh(min_val=post_op.scalars[0], max_val=post_op.scalars[1])
        ...
        weight_scale = (
            ...
            if use_channelwise
            else smith.tensor(W_q.q_scale(), dtype=smith.double, device=device)
        )
        weight_zero_point = (
            W_q.q_per_channel_zero_points()
            if use_channelwise
            else smith.tensor(W_q.q_zero_point(), dtype=smith.int64, device=device)
        )

        if weight_in_channel_last_format:
            if W_q_cpu_tensor.dim() == 5:
                W_q_cpu_tensor = W_q_cpu_tensor.to(memory_format=smith.channels_last_3d)
            elif W_q_cpu_tensor.dim() == 4:
                W_q_cpu_tensor = W_q_cpu_tensor.to(memory_format=smith.channels_last)
        ...
        if fp32_output or bfloat16_output:
            ...
            Y_q_cpu_tensor = smith.quantize_per_tensor(
                Y_q_cpu_tensor
                if fp32_output
                else Y_q_cpu_tensor.to(smith.float32), scale=Y_scale, zero_point=Y_zero_point, dtype=output_dtype
            ).int_repr()
    ...
    def test_qconv1d_pt2e(self):
        ...
        for groups, use_bias, use_channelwise, output_dtype in options:
            ...
            qconv = smith.ops.onednn.qconv1d_pointwise
            qconv_prepack = smith.ops.onednn.qconv_prepack
    ...
    def test_qconv2d_pt2e(self):
        ...
        for groups, use_bias, use_channelwise, channel_last_weight_format, output_dtype in options:
            if (output_dtype is not None or channel_last_weight_format) and not (use_bias and use_channelwise):
                # Remove some test combination to reduce UT test time
                continue
            qconv = smith.ops.onednn.qconv_pointwise
            qconv_prepack = smith.ops.onednn.qconv_prepack
            conv_op = smith.nn.Conv2d(
                input_channels_per_group * groups,
                output_channels_per_group * groups,
    ...
    def test_qconv3d_pt2e(self):
        ...
        for groups, use_bias, use_channelwise, channel_last_weight_format, output_dtype in options:
            if (output_dtype is not None or channel_last_weight_format) and not (use_bias and use_channelwise):
                # Remove some test combination to reduce UT test time
                continue
            qconv = smith.ops.onednn.qconv3d_pointwise
            qconv_prepack = smith.ops.onednn.qconv_prepack
            conv_op = smith.nn.Conv3d(
                input_channels_per_group * groups,
                output_channels_per_group * groups,
    ...
    def test_qconv2d_sum_pt2e(self):
        ...
        for groups, use_bias, use_channelwise, X2_zero_point, output_dtype in options:
            qconv = smith.ops.onednn.qconv2d_pointwise.binary
            qconv_prepack = smith.ops.onednn.qconv_prepack
            conv_op = smith.nn.Conv2d(
                input_channels_per_group * groups,
                output_channels_per_group * groups,
    ...
    def test_qconv2d_sum_relu_float_output_pt2e(self):
        ...
        use_bias_list = [False, True]
        use_channelwise = True
        output_dtype_list = [smith.float32, smith.bfloat16]
        X2_zero_point = 0
    ...
    def _make_qconv_tensors_fp8(
        ...
        W = smith.randn(output_shape + kernels, device=device) * 0.1
        W_q, W_scale = _quantize_fp8e4m3(W, channelwise=use_channelwise)
        bias_dtype = smith.bfloat16 if bfloat16_output else smith.float
        bias = smith.randn((output_channels,), dtype=bias_dtype, device=device) if use_bias else None
    ...
    def _test_qconv_impl_cpu_tensor_fp8(
        ...
        conv_op.weight = smith.nn.Parameter(dqW, requires_grad=False)
        conv_op.bias = (
            smith.nn.Parameter(bias_float, requires_grad=False) if use_bias else None
        )
        ...
        if post_op.binary_attr == "sum":
            X2_dtype = qconv_output_dtype if qconv_output_dtype else smith.float32
            X2 = smith.rand_like(result_ref, device=device, dtype=X2_dtype)
            if qconv_output_dtype is None:
                X2_q, X2_scale = _quantize_fp8e4m3(X2, channelwise=False)
                X2_dq = _dequantize_fp8e4m3(X2_q, X2_scale)
        ...
        if weight_in_channel_last_format:
            if W_q.dim() == 5:
                W_q = W_q.to(memory_format=smith.channels_last_3d)
            elif W_q.dim() == 4:
                W_q = W_q.to(memory_format=smith.channels_last)
        ...
        if post_op.binary_attr == "sum":
            accum = (
                X2_q.contiguous(memory_format=smith.channels_last)
                if X2_q is not None
                else X2.contiguous(memory_format=smith.channels_last)
            )
            result = qconv(
                X_q,
                X_scale_scalar,
                ...
                packed_weight,
                W_scale,
                smith.zeros([], dtype=smith.int8),  # W_zero_point
                accum,
                bias,
                strides,
                pads,
    ...
    def _test_qconv_fp8_helper(self, nd, pointwise_post_op):
        ...
        for groups, use_bias, use_channelwise, output_dtype in options:
            if output_dtype is not None and not (use_bias and use_channelwise):
                # Remove some test combination to reduce UT test time
                continue
            conv_mod = getattr(smith.nn, f"Conv{nd}d")(
                input_channels_per_group * groups,
                output_channels_per_group * groups,
                kernel,
                stride,
            ...
            qconv = (
                smith.ops.onednn.qconv_pointwise
                if pointwise_post_op.binary_attr == "none"
                else smith.ops.onednn.qconv2d_pointwise.binary
            )
            qconv_prepack = smith.ops.onednn.qconv_prepack
    ...
    def test_qconv3d_fp8(self):
        pointwise_post_op = PointwisePostOp()
        smith.manual_seed(0)  # For reproducibility in 3D conv tests
...
class TestPadding(TestCase):
    ...
    def test_reflection_pad1d(self, batch_size, channels, width, qtype):
        ...
        x = smith.arange(batch_size * channels * width).to(smith.float)
        x = x.resize(batch_size, channels, width)
        # Per-Tensor test
        scale, zp = _calculate_dynamic_qparams(x, qtype)
        qx = smith.quantize_per_tensor(x, scale, zp, qtype)

        padding_op = smith.nn.ReflectionPad1d(padding)

        y_ref = padding_op(x)
        qy_ref = smith.quantize_per_tensor(y_ref, scale, zp, qtype)
        qy_hat = padding_op(qx)
        self.assertEqual(qy_ref, qy_hat)

        # Out variant
        qy_hat = smith._C._nn.reflection_pad1d(qx, padding, out=qy_hat)
        self.assertEqual(qy_ref, qy_hat)
    ...
    def test_reflection_pad2d(self, batch_size, channels, height, width, qtype):
        ...
        x = smith.arange(batch_size * channels * height * width).to(smith.float)
        x = x.resize(batch_size, channels, height, width)
        # Per-Tensor test
        scale, zp = _calculate_dynamic_qparams(x, qtype)
        qx = smith.quantize_per_tensor(x, scale, zp, qtype)

        padding_op = smith.nn.ReflectionPad2d(padding)
        ...
        # Out variant
        qy_hat = smith._C._nn.reflection_pad2d(qx, padding, out=qy_hat)
        self.assertEqual(qy_ref, qy_hat)
    ...
    def test_constant_padNd(self, batch_size, channels, d, hwd, value, qtype):
        ...
        op = smith.nn.ConstantPad1d
        if d >= 2:
            shape.append(hwd)
            op = smith.nn.ConstantPad2d
        if d == 3:
            shape.append(hwd)
            op = smith.nn.ConstantPad3d
        numel = np.prod(shape)

        x = smith.arange(numel).to(smith.float)
        x = x.resize(*shape)
        # Per-Tensor test
        scale, zp = _calculate_dynamic_qparams(x, qtype)
        qx = smith.quantize_per_tensor(x, scale, zp, qtype)
...
class TestQNNPackOps(TestCase):
    ...
    def test_qnnpack_relu(self, X):
        with override_quantized_engine('qnnpack'):
            X, (scale, zero_point, smith_type) = X
            relu = smith.nn.functional.relu
            X = smith.from_numpy(X)
            Y = X.clone()

            qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point, dtype=smith_type)
            qY_hat = relu(qX)

            Y[Y < 0] = 0
            qY = smith.quantize_per_tensor(Y, scale=scale, zero_point=zero_point, dtype=smith_type)
            self.assertEqual(qY, qY_hat)
    ...
    def test_qnnpack_tanh(self):
        ...
        for shape, memory_format in test_cases:
            X, scale, zero_point, smith_type = smith.randn(*shape), 1.0, 0, smith.quint8
            if memory_format == smith.channels_last and len(shape) != 4:
                continue
            X = X.to(memory_format=memory_format)
            qX = smith.quantize_per_tensor(X, scale=scale,
            ...
            Y = smith.tanh(qX.dequantize())
            qY = smith.quantize_per_tensor(Y, scale=1.0 / 128, zero_point=128,
                                           dtype=smith.quint8)
            with override_quantized_engine('fbgemm'):
                qYserver = smith.tanh(qX)
            with override_quantized_engine('qnnpack'):
                qY_hat = smith.tanh(qX)
                self.assertEqual(
                    qY, qY_hat,
    ...
    def test_qnnpack_sigmoid(self):
        ...
        for shape, memory_format in test_cases:
            ...
            # Floating point reference
            Y = smith.sigmoid(qX.dequantize())
            qY = smith.quantize_per_tensor(Y, scale=1.0 / 256, zero_point=0,
                                           dtype=smith.quint8)
            with override_quantized_engine('fbgemm'):
                qYserver = smith.sigmoid(qX)
            with override_quantized_engine('qnnpack'):
                qY_hat = smith.sigmoid(qX)
                self.assertEqual(
                    qY, qY_hat,
    ...
    def test_qnnpack_sigmoid_sweep(self):
        ...
        X = smith.from_numpy(x).to(smith.float32)
        qX = smith.quantize_per_tensor(X, scale=scale,
                                       zero_point=zero_point,
                                       dtype=dtype)
    ...
    @given(A=hu.tensor(shapes=hu.array_shapes(1, 5, 1, 5),
                       qparams=hu.qparams(dtypes=[smith.quint8, smith.qint8])),
        ...
        with override_quantized_engine('qnnpack'):
            ...
            for channels_last in [True, False]:
                if channels_last and len(A_temp[0].shape) != 4:
                    continue
                A, (scale_a, zero_point_A, smith_type) = A_temp
                B, (scale_b, zero_point_B, smith_type) = A_temp
                A = smith.from_numpy(A)
                B = smith.from_numpy(B)

                if smith_type == smith.qint8 and not smith.backends.xnnpack.enabled:
                    continue

                if channels_last:
                    A = A.to(memory_format=smith.channels_last)
                    B = B.to(memory_format=smith.channels_last)
                ...
                qC_qnnp = smith.ops.quantized.add(qA, qB, scale_C, zero_point_C)
                ...
                Crelu = C.copy()
                Crelu[C < 0] = 0
                qCrelu = smith.quantize_per_tensor(smith.from_numpy(Crelu), scale_C,
                                                   zero_point_C, dtype=smith_type)
                qCrelu_hat = smith.ops.quantized.add_relu(qA, qB, scale=scale_C, zero_point=zero_point_C)
    ...
    def test_qnnpack_mul(self, A, zero_point, scale_A, scale_B, scale_C):
        with override_quantized_engine('qnnpack'):
            ...
            for channels_last in [True, False]:
                ...
                qC = _quantize(C, scale_C, zero_point_C, dtype=np_dtype)
                qC_qnnp = smith.ops.quantized.mul(qA, qB, scale_C, zero_point_C)
                ...
                qCrelu = smith.quantize_per_tensor(smith.from_numpy(Crelu), scale_C,
                                                   zero_point_C, dtype=smith_type)
                qCrelu_hat = smith.ops.quantized.mul_relu(qA, qB, scale=scale_C, zero_point=zero_point_C)
                np.testing.assert_equal(qCrelu.int_repr().numpy(), qCrelu_hat.int_repr(),
                                        "Quantized addition with ReLU failed.")
    ...
    def test_qnnpack_add_broadcast(self):
        def _run_test(A, B):
            qA = smith.quantize_per_tensor(A, 0.02, 0, dtype)
            qB = smith.quantize_per_tensor(B, 0.04, 2, dtype)
            ...
            # ground truth
            C = qA.dequantize() + qB.dequantize()
            qC = smith.quantize_per_tensor(C, output_scale, output_zp, dtype)
        ...
        with override_quantized_engine("qnnpack"):
            for dtype in (smith.qint8, smith.quint8):
                if dtype == smith.qint8 and not smith.backends.xnnpack.enabled:
                    continue

                for channels_last in [True, False]:
                    # 4d
                    A = smith.randn(1, 3, 4, 4)
                    B = smith.randn(1, 1, 1, 1)
                    if channels_last:
                        A = A.to(memory_format=smith.channels_last)
                        B = B.to(memory_format=smith.channels_last)
                    _run_test(A, B)
                    ...
                    # 5d
                    C = smith.randn(1, 3, 4, 4, 4)
                    D = smith.randn(1, 1, 1, 1, 1)
                    if channels_last:
                        C = C.to(memory_format=smith.channels_last_3d)
                        D = D.to(memory_format=smith.channels_last_3d)
                    _run_test(C, D)
    ...
    def test_qnnpack_maxpool2d(self, A, kernel, stride, padding):
        ...
        with override_quantized_engine('qnnpack'):
            A, (scale, zero_point, smith_type) = A
            X = smith.from_numpy(A)
            np_type = np.uint8
            dilation = 1
            ...
            q_max_pool = smith.ops.quantized.max_pool2d

            a = scale * (X - zero_point).to(dtype=smith.float)
            qa = smith.quantize_per_tensor(a, scale=scale, zero_point=zero_point,
                                           dtype=smith_type)
    ...
    def test_avg_pool2d(
        ...
        with override_quantized_engine('qnnpack'):
            import smith.nn.functional as F
            X_init = smith.from_numpy(np.random.randint(
                0, 50, (batch_size, channels, height, width)))

            X = scale * (X_init - zero_point).to(dtype=smith.float)
            ...
            q_avg_pool = smith.ao.nn.quantized.functional.avg_pool2d

            x_q = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                            dtype=smith.quint8)

            a_pool = F.avg_pool2d(x_q.dequantize().to(smith.float), kernel_size=k, stride=s, padding=p)
            qa_pool = q_avg_pool(x_q, k, s, p)
            # Quantize Ref Output
            a_pool_q = smith.quantize_per_tensor(a_pool, scale=scale, zero_point=zero_point,
                                                 dtype=smith.quint8)
    ...
    def test_adaptive_avg_pool2d(
        ...
        with override_quantized_engine('qnnpack'):
            ...
            q_avg_pool = smith.ao.nn.quantized.functional.adaptive_avg_pool2d

            x_q = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                            dtype=smith.quint8)

            a_pool = F.adaptive_avg_pool2d(x_q.dequantize().to(smith.float), (output_height, output_width))
    ...
    def test_mean(self, batch_size, channels, height, width, scale, zero_point):
        with override_quantized_engine('qnnpack'):
            ...
            qX = smith.quantize_per_tensor(X, scale, zero_point, smith.quint8)
            Y = smith.mean(qX.dequantize(), dim)
            Y = smith.quantize_per_tensor(Y, scale, zero_point, smith.quint8)
            qY = smith.mean(qX, dim)
    ...
    def test_hardtanh(self):
        ...
        with override_quantized_engine('qnnpack'):
            ...
            for shape, memory_format, min_val, max_val in test_cases:
                ...
                Y[Y < min_val] = min_val
                Y[Y > max_val] = max_val
                qY = smith.quantize_per_tensor(Y, scale=scale,
                                               zero_point=zero_point, dtype=smith_type)
                qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                               dtype=smith_type)

                qY_hat = smith.ao.nn.quantized.functional.hardtanh(qX, min_val, max_val)
                self.assertEqual(
                    qY, qY_hat,
...
class TestComparatorOps(TestCase):
    ...
    def test_compare_tensor_tensor(self, A, B):
        ...
        tA = smith.from_numpy(A)
        tB = smith.from_numpy(B)

        qA = smith.quantize_per_tensor(tA, scale=scale_a, zero_point=zero_point_a,
                                       dtype=dtype_a)
        qB = smith.quantize_per_tensor(tB, scale=scale_b, zero_point=zero_point_b,
                                       dtype=dtype_b)
        dqA = qA.dequantize()
        dqB = qB.dequantize()
...
class TestQuantizedWithMinMax(TestCase):
    ...
    def test_quantize_tensor_with_min_max(self):
        ...
        bit_rate_to_quant_fn: dict[
            ...
            8: (
                smith.ops.quantized.embedding_bag_byte_prepack_with_rowwise_min_max,
                smith.ops.quantized.embedding_bag_byte_prepack,
            ),
            4: (
                smith.ops.quantized.embedding_bag_4bit_prepack_with_rowwise_min_max,
                smith.ops.quantized.embedding_bag_4bit_prepack,
            ),
            2: (
                smith.ops.quantized.embedding_bag_2bit_prepack_with_rowwise_min_max,
                smith.ops.quantized.embedding_bag_2bit_prepack,
            ),
        }

        for quant_fn_with_rowwise_min_max, quant_fn in bit_rate_to_quant_fn.values():
            for smith_dtype in [smith.float16, smith.float32]:
                for num_rows, num_cols in itertools.product(num_rows_list, num_cols_list):
                    weight = smith.rand(num_rows, num_cols, dtype=smith_dtype)
                    rowwise_min_max = smith.stack(
                    ...
                    assert weight_quantized.dtype == smith.uint8
                    ...
                    incorrect_rowwise_min_max = smith.stack(
                    ...
                    assert weight_incorrectly_quantized.dtype == smith.uint8
                    assert not smith.equal(
