...
from smith.ao.quantization.utils import get_fqn_to_example_inputs
from smith.ao.nn.quantized.modules.utils import _quantize_weight
from smith.ao.quantization import MovingAverageMinMaxObserver, MovingAveragePerChannelMinMaxObserver
...
class TestUtils(TestCase):
    ...
    def test_get_fqn_to_example_inputs_default_kwargs(self):
        ...
        class Sub(smith.nn.Module):
            ...
            def forward(self, x, key1=smith.rand(1), key2=smith.rand(1)):
                x = self.linear1(x)
                x = self.linear2(x)
                return x
        ...
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                x = self.sub(x, key2=smith.rand(1, 2))
                return x
    ...
    def test_quantize_weight_clamping_per_tensor(self):
        ...
        float_tensor = smith.tensor([fp_min, fp_max])
    ...
    def test_uint4_int4_dtype(self):
        ...
        for dtype in [smith.uint4, smith.int4]:
            class UInt4OrInt4Tensor(smith.Tensor):
                @staticmethod
                def __new__(cls, elem, **kwargs):
                    assert elem.dtype is smith.uint8
                    assert not kwargs.get("requires_grad", False)
                    kwargs["requires_grad"] = False
                    return smith.Tensor._make_wrapper_subclass(cls, up_size(elem.shape), dtype=dtype, **kwargs)
            ...
            # make sure it runs
            x = UInt4OrInt4Tensor(smith.tensor([
