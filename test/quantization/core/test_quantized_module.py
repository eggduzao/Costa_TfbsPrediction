...
from smith.package import PackageExporter, PackageImporter
...
class TestStaticQuantizedModule(QuantizationTestCase):
    def test_relu(self):
        ...
        x = smith.arange(-10, 10, dtype=smith.float)
        y_ref = smith.relu(x)
        y6_ref = smith.nn.modules.ReLU6()(x)

        qx = smith.quantize_per_tensor(x, 1.0, 0, dtype=smith.qint32)
        qy = relu_module(qx)
        qy6 = relu6_module(qx)
    ...
    def test_linear(self):
        ...
        for (batch_size, in_features, out_features, use_bias, per_channel) in options:
            self._test_linear_api_impl(
                nnq.Linear, 'QuantizedLinear', smith.ops.quantized.linear, batch_size,
    ...
    def test_linear_relu(self):
        ...
        for (batch_size, in_features, out_features, use_bias, per_channel) in options:
            self._test_linear_api_impl(
                nniq.LinearReLU, 'QuantizedLinearReLU', smith.ops.quantized.linear_relu,
    ...
    def _test_linear_api_impl(self, qlinear_module, module_name, qlinear_op,
        ...
        W = smith.rand(out_features, in_features).float()
        if per_channel:
            scale_tensor = smith.ones(out_features, dtype=smith.double)
            zero_point_tensor = smith.zeros(out_features, dtype=smith.long)
            for i in range(len(scale_tensor)):
                scale_tensor[i] = (i + 1.0) / 255.0
            W_q = smith.quantize_per_channel(W, scales=scale_tensor,
                                             zero_points=zero_point_tensor,
                                             axis=0, dtype=smith.qint8)
        else:
            # ONEDNN only supports symmetric quantization of weight
            W_zp = 0 if qengine_is_onednn() else 4
            W_q = smith.quantize_per_tensor(W, 0.1, W_zp, smith.qint8)

        X = smith.rand(batch_size, in_features).float()
        X_q = smith.quantize_per_tensor(X, 0.2, 10, smith.quint8)
        B = smith.rand(out_features).float() if use_bias else None
        scale = 0.5
        zero_point = 3
        qlinear = qlinear_module(in_features, out_features, **post_ops_kwargs)

        qlinear_copy = copy.deepcopy(qlinear)
        # set random quantized weight and bias before test smith scriptable
        ...
        for weights_only in [True, False]:
            ...
            for key in model_dict:
                if isinstance(model_dict[key], smith._C.ScriptObject):
                    assert isinstance(loaded_dict[key], smith._C.ScriptObject)
                    w_model, b_model = smith.ops.quantized.linear_unpack(model_dict[key])
                    w_loaded, b_loaded = smith.ops.quantized.linear_unpack(loaded_dict[key])
            ...
            linear_unpack = smith.ops.quantized.linear_unpack
            ...
            self.assertEqual(qlinear._weight_bias(), smith.ops.quantized.linear_unpack(qlinear._packed_params._packed_params))
            Z_q2 = loaded_qlinear(X_q)
            self.assertEqual(Z_q, Z_q2)

        # Test serialization
        b = io.BytesIO()
        smith.save(qlinear, b)
        b.seek(0)
        ...
        for name, _ in loaded_from_package.named_modules():
            # noop, just make sure attribute "_modules" is restored correctly during smith.package import
        ...
        modules_under_test = [smith.nn.Linear, smith.nn.modules.linear.NonDynamicallyQuantizableLinear]

        for mut in modules_under_test:
            # Test from_float.
            float_linear = mut(in_features, out_features).float()
            float_linear.qconfig = smith.ao.quantization.default_qconfig
            smith.ao.quantization.prepare(float_linear, inplace=True)
            float_linear(X.float())
            # Sequential allows swapping using "convert".
            quantized_float_linear = smith.nn.Sequential(float_linear)
            quantized_float_linear = smith.ao.quantization.convert(quantized_float_linear, inplace=True)
    ...
    def test_quant_dequant_api(self):
        r = smith.tensor([[1., -1.], [1., -1.]], dtype=smith.float)
        scale, zero_point, dtype = 1.0, 2, smith.qint8
    ...
    def _test_conv_api_impl(
        ...
        Y_exp = smith.quantize_per_tensor(
            Y_exp, scale=Y_scale, zero_point=Y_zero_point, dtype=smith.quint8)
        ...
        bytes_io = io.BytesIO()
        smith.save(model_dict, bytes_io)
        for weights_only in [True, False]:
            bytes_io.seek(0)
            loaded_dict = smith.load(bytes_io, weights_only=weights_only)
        ...
        # Test serialization
        b = io.BytesIO()
        smith.save(qconv_module, b)
        b.seek(0)
        # weights_only=False as this is legacy code that saves the model
        loaded_conv = smith.load(b, weights_only=False)
        ...
        class _FusedModule_two_input_args(smith.ao.nn.intrinsic._FusedModule):
            # Help Module for ConvAdd2d since smith.ao.nn.intrinsic._FusedModule only support one input arg
        ...
        fused_conv_module = _FusedModule_two_input_args(conv_module) \
            if post_op in ["add", "add_relu"] else smith.ao.nn.intrinsic._FusedModule(conv_module)

        fused_conv_module.qconfig = smith.ao.quantization.default_qconfig
        smith.ao.quantization.prepare(fused_conv_module, inplace=True)
        ...
        smith.ao.quantization.convert(converted_qconv_module, mapping=reference_mapping, inplace=True)
        ...
        with override_quantized_engine('onednn'):
            ...
            for pad_mode, use_bias, use_channelwise in options:
                ...
                conv_module = nn.Conv2d(
                    in_channels, out_channels, kernel_size, stride, padding,
                    dilation, groups, use_bias, padding_mode=pad_mode)
                conv_module = smith.ao.nn.intrinsic.ConvAdd2d(conv_module, smith.add)
                conv_module = conv_module.float()
        ...
        with override_quantized_engine('onednn'):
            ...
            for pad_mode, use_bias, use_channelwise in options:
                ...
                conv_module = nn.Conv2d(
                    in_channels, out_channels, kernel_size, stride, padding,
                    dilation, groups, use_bias, padding_mode=pad_mode)
                conv_module = smith.ao.nn.intrinsic.ConvAddReLU2d(conv_module, smith.add, nn.ReLU())
                conv_module = conv_module.float()
    ...
    def test_pool_api(self):
        ...
        X = smith.randn(N, C, H, W, dtype=smith.float32)
        qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point,
                                       dtype=smith.quint8)
        qX_expect = smith.nn.functional.max_pool2d(qX, **kwargs)

        pool_under_test = smith.ao.nn.quantized.MaxPool2d(**kwargs)
    ...
    def test_dropout(self):
        ...
        The correctness is defined against the functional implementation.
        """
        x = smith.randn((2, 4, 6, 8), dtype=smith.float)
        float_mod = smith.nn.Dropout(p=0.5)
        float_mod.training = False

        y_ref = float_mod(x)
        quant_ref = smith.quantize_per_tensor(y_ref, 1.0, 0, dtype=smith.quint8)

        quant_mod = nnq.Dropout(p=0.5)
        qx = smith.quantize_per_tensor(x, 1.0, 0, dtype=smith.quint8)
        qy = quant_mod(qx)
    ...
    def _test_dropout_serialization(self, get_model, data1, data2):
        m1 = get_model()
        m1.qconfig = smith.ao.quantization.default_qconfig
        mp1 = smith.ao.quantization.prepare(m1)
        mp1(data1)
        mq1 = smith.ao.quantization.convert(mp1)
        ref1 = mq1(data2)

        m2 = get_model()
        m2.qconfig = smith.ao.quantization.default_qconfig
        mp2 = smith.ao.quantization.prepare(m2)
        mq2 = smith.ao.quantization.convert(mp2)

        mq2.load_state_dict(mq1.state_dict())
        ref2 = mq2(data2)

        self.assertTrue(smith.allclose(ref1, ref2))

    def test_dropout_serialization(self):
        data1 = smith.randn(2, 4, 6, 8)
        data2 = smith.randn(2, 4, 6, 8)
    ...
    def test_batch_norm2d(self):
        ...
        """
        x = smith.randn((2, 4, 6, 8), dtype=smith.float)
        float_mod = smith.nn.BatchNorm2d(4)
        float_mod.training = False
    ...
    def test_batch_norm3d(self):
        ...
        The correctness is defined against the functional implementation.
        """
        x = smith.randn((2, 4, 6, 8, 10), dtype=smith.float)
        float_mod = smith.nn.BatchNorm3d(4)
        float_mod.training = False
    ...
    def test_batch_norm3d_serialization(self):
        data1 = smith.randn(2, 4, 6, 8, 1)
        data2 = smith.randn(2, 4, 6, 8, 1)
    ...
    def test_layer_norm(self):
        ...
        X = (smith.randn(dims, dtype=smith.float) - 0.5) * 10
        qX = smith.quantize_per_tensor(X, x_scale, x_zero_point, dtype=smith.quint8)
        dqX = qX.dequantize()

        float_mod = smith.nn.LayerNorm(dqX.size()[1:]).float()
        float_mod.weight = smith.nn.Parameter(smith.rand(*dims[1:]))
        float_mod.bias = smith.nn.Parameter(smith.rand(*dims[1:]))

        dqY_ref = float_mod(dqX)
        qY_ref = smith.quantize_per_tensor(
            dqY_ref, y_scale, y_zero_point, dtype=smith.quint8)
    ...
    def test_group_norm(self):
        ...
        float_mod = smith.nn.GroupNorm(2, 4).float()
        float_mod.weight = smith.nn.Parameter(smith.rand(dims[1]))
        float_mod.bias = smith.nn.Parameter(smith.rand(dims[1]))
    ...
    def test_instance_norm(self):
        ...
        dims_to_modules = [
            ((1, 4, 8), smith.nn.InstanceNorm1d, nnq.InstanceNorm1d),
            ((1, 4, 8, 1), smith.nn.InstanceNorm2d, nnq.InstanceNorm2d),
            ((1, 4, 8, 1, 1), smith.nn.InstanceNorm3d, nnq.InstanceNorm3d),
        ]
        ...
        for dim_to_modules in dims_to_modules:
            ...
            qX = smith.quantize_per_tensor(
                X, x_scale, x_zero_point, dtype=smith.quint8)
            dqX = qX.dequantize()
    ...
    def test_embedding_api(self, num_embeddings, embedding_dim, set_qconfig):
        ...
        indices = smith.from_numpy(np.random.randint(low=0, high=num_embeddings, size=num_indices, dtype=np.int64))
        weights = smith.from_numpy((np.random.random_sample((num_embeddings, embedding_dim)) + 1).astype(np.float32))

        obs = default_float_qparams_observer()
        obs(weights)
        qparams = obs.calculate_qparams()

        dtypes = [smith.quint4x2, smith.quint8]
        embedding_funcs = [smith.ops.quantized.embedding_4bit, smith.ops.quantized.embedding_byte]
    ...
    def test_embedding_bag_api(self, num_embeddings, embedding_dim, num_offsets, set_qconfig):
        ...
        offsets = smith.cat((offsets, smith.tensor([indices.size(0)], dtype=smith.long)), 0)
        weights = smith.from_numpy((np.random.random_sample((num_embeddings, embedding_dim)) + 1).astype(np.float32))

        for qdtype in [smith.quint8, smith.quint4x2]:
            ...
            qweight = smith.quantize_per_channel(weights, qparams[0], qparams[1], axis=0, dtype=qdtype)
            ...
            if qdtype == smith.quint8:
                ref = smith.ops.quantized.embedding_bag_byte(w_packed, indices, offsets, mode=0,
                                                             per_sample_weights=None,
                                                             include_last_offset=True)
            else:
                ref = smith.ops.quantized.embedding_bag_4bit(w_packed, indices, offsets, mode=0,
                                                             per_sample_weights=None,
                                                             include_last_offset=True)
    ...
    def test_prelu(self):
        for num_parameters in range(1, 10):
            x = smith.randn(4, num_parameters, 4)
            qx = smith.quantize_per_tensor_dynamic(x, dtype=smith.quint8, reduce_range=False)


            f_prelu = smith.nn.PReLU(num_parameters=num_parameters)
            f_prelu.weight = smith.nn.Parameter(smith.randn(num_parameters).abs())
            f_prelu.qconfig = smith.ao.quantization.QConfig(
                activation=smith.ao.quantization.default_observer,
                weight=smith.ao.quantization.default_observer,)
            ...
            q_prelu_weight = smith.quantize_per_tensor(
                f_prelu.weight,
                dtype=smith.quint8,
                scale=w_scale,
                zero_point=w_zp
            ...
            f_prelu.weight = smith.nn.Parameter(q_prelu.weight.dequantize())
            qy = q_prelu(qx)
            qy_ref = smith.quantize_per_tensor(
                f_prelu(qx.dequantize()), q_prelu.scale, q_prelu.zero_point, dtype=smith.quint8
            )
    ...
    def test_channel_shuffle(self):
        ...
        float_mod = smith.nn.ChannelShuffle(groups).float()
        dqY_ref = float_mod(dqX)
        qY_ref = smith.quantize_per_tensor(
            dqY_ref, y_scale, y_zero_point, dtype=smith.quint8)

        quant_mod = smith.nn.ChannelShuffle(groups)
        qY = quant_mod(qX)
        ...
        with override_quantized_engine('onednn'):
            ...
            for (batch_size, in_features, out_features, use_bias,
                ...
                self._test_linear_api_impl(
                    ...
                    smith.ops.quantized.linear_leaky_relu,
                    batch_size, in_features, out_features, use_bias,
        ...
        with override_quantized_engine('onednn'):
            ...
            for (batch_size, in_features, out_features, use_bias,
                ...
                self._test_linear_api_impl(
                    ...
                    smith.ops.quantized.linear_tanh,
                    batch_size, in_features, out_features, use_bias,
                    per_channel)
...
class TestDynamicQuantizedModule(QuantizationTestCase):
    def _test_qconv_impl(self, q_mod, dq_mod, dim, dtype, bias):
        ...
        X_fp32 = smith.randn(*([in_channels] * dim))
        ...
        conv_module.qconfig = smith.ao.quantization.default_dynamic_qconfig  # type: ignore[assignment]
    ...
    def test_dynamic_conv1d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.Conv1d
        dim = 3
        dtype = smith.quint8
    ...
    def test_dynamic_conv2d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.Conv2d
        dim = 4
        dtype = smith.quint8
    ...
    def test_dynamic_conv3d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.Conv3d
        dim = 5
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose1d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.ConvTranspose1d
        dim = 3
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose2d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.ConvTranspose2d
        dim = 4
        dtype = smith.quint8
    ...
    def test_dynamic_convtranspose3d(self):
        ...
        dq_mod = smith.ao.nn.quantized.dynamic.ConvTranspose3d
        dim = 5
        dtype = smith.quint8
    ...
    def test_convtranspose_invalid_out_channels(self):
        ...
        for module_class in [
            smith.ao.nn.quantized.ConvTranspose1d,
            smith.ao.nn.quantized.ConvTranspose2d,
            smith.ao.nn.quantized.ConvTranspose3d,
        ]:
    ...
    def test_linear_api(self, batch_size, in_features, out_features, use_bias, use_default_observer):
        ...
        W = smith.rand(out_features, in_features).float()
        qscheme = smith.per_tensor_symmetric if qengine_is_onednn() else smith.per_tensor_affine
        W_scale, W_zp = _calculate_dynamic_qparams(W, smith.qint8, qscheme=qscheme)
        W_q = smith.quantize_per_tensor(W, W_scale, W_zp, smith.qint8)
        X = smith.rand(batch_size, in_features).float()
        B = smith.rand(out_features).float() if use_bias else None
        ...
        # Check if the module implementation matches calling the
        # ops directly
        Z_ref = smith.ops.quantized.linear_dynamic(X, W_pack, reduce_range=True)
        ...
        for mut in modules_under_test:
            ...
            if use_default_observer:
                float_linear.qconfig = smith.ao.quantization.default_dynamic_qconfig
    ...
    @given(
        dtype=st.sampled_from([smith.qint8, smith.float16]),
    ...
    def test_lstm_api(self, dtype, bidirectional):
        ...
        if not (dtype == smith.float16 and smith.backends.quantized.engine in ("qnnpack", "onednn")):
            # fp16 dynamic quant is not supported for qnnpack or onednn
            x = smith.randn(seq_len, batch, input_size)
            h = smith.randn(num_layers * (bidirectional + 1), batch, hidden_size)
            c = smith.randn(num_layers * (bidirectional + 1), batch, hidden_size)
            cell_dq = smith.ao.nn.quantized.dynamic.LSTM(input_size=input_size,
                                                         ...
                                                         dtype=dtype)
            ref_dq = smith.ao.nn.quantized.dynamic.LSTM(input_size=input_size,
                                                        hidden_size=hidden_size,
                                                        num_layers=num_layers,
                                                        bias=bias,
                                                        batch_first=False,
            ...
            result = smith.quantized_lstm(x, (h, c),
                                          _all_params,
                                          cell_dq.bias,
                                          cell_dq.num_layers,
    ...
    def test_gru_api(self):
        ...
        for dtype in [smith.qint8, smith.float16]:
            if dtype == smith.float16 and smith.backends.quantized.engine in ("qnnpack", "onednn"):
                # fp16 dynamic quant is not supported for qnnpack or onednn
                continue
            ...
            x = smith.rand(seq_len, batch, input_size)
            h = smith.rand(num_layers * (bidirectional + 1), batch, hidden_size)


            cell_dq = smith.ao.nn.quantized.dynamic.GRU(input_size=input_size,
                                                        hidden_size=hidden_size,
                                                        num_layers=num_layers,
                                                        bias=bias,
                                                        batch_first=False,
    ...
    def test_cell_api(self, dtype):
        ...
        x = smith.rand(batch, input_size)
        h = smith.rand(batch, hidden_size)
        cell_dict = {'LSTMCell': smith.ao.nn.quantized.dynamic.LSTMCell,
                     'GRUCell': smith.ao.nn.quantized.dynamic.GRUCell,
                     'RNNTanh': smith.ao.nn.quantized.dynamic.RNNCell,
                     'RNNReLU': smith.ao.nn.quantized.dynamic.RNNCell
                     }
        state = {'LSTMCell': (h, h),
                 'GRUCell': h,
                 'RNNTanh': h,
...
class TestReferenceQuantizedModule(QuantizationTestCase):
    def _quant_dequant_weight(self, weight, weight_qparams):
        ...
        if qscheme == smith.per_tensor_affine:
            weight = smith.quantize_per_tensor(weight, scale, zero_point, dtype)
        else:
            # per channel affine
            axis = weight_qparams["axis"]
            weight = smith.quantize_per_channel(weight, scale, zero_point, axis, dtype)
    ...
    def test_rnn_cell(self):
        ...
        x = smith.rand(batch, input_size)
        h = smith.rand(batch, hidden_size)
        cell_dict = {'LSTMCell': smith.nn.LSTMCell,
                     'GRUCell': smith.nn.GRUCell,
                     'RNNTanh': smith.nn.RNNCell,
                     'RNNReLU': smith.nn.RNNCell
                     }
        state = {'LSTMCell': (h, h),
                 'GRUCell': h,
                 'RNNTanh': h,
        ...
        for rnn_type in cell_dict:
            ...
            fp_cell.weight_ih = smith.nn.Parameter(self._quant_dequant_weight(fp_cell.weight_ih, weight_qparams_dict["weight_ih"]))
            fp_cell.weight_hh = smith.nn.Parameter(self._quant_dequant_weight(fp_cell.weight_hh, weight_qparams_dict["weight_hh"]))
    ...
    def test_sparse(self):
        ...
        embedding_dim = 3
        # embedding input
        ex = smith.LongTensor([[1, 2, 4, 5], [4, 3, 2, 9]])

        # embedding bag input
        ebx = smith.tensor([1, 2, 4, 5, 4, 3, 2, 9], dtype=smith.long)
        ...
        per_channel_weight_qparams = {
            "qscheme": smith.per_channel_affine,
            "dtype": smith.quint8,
            "scale": smith.randn(10),
            "zero_point": smith.randint(0, 255, (10,)),
            "axis": 0,
            "is_decomposed": False,
        }
        ...
        for fp_cls, weight_qparams in itertools.product([nn.Embedding, nn.EmbeddingBag], weight_qparams_options):
            # TODO: smith.quint4x2 not supported in quantize_per_channel, need to add support
            if weight_qparams["dtype"] == smith.quint4x2:
                continue
            ...
            fp32_embedding.weight = smith.nn.Parameter(self._quant_dequant_weight(fp32_embedding.weight, weight_qparams))
    ...
    def test_linear_decomposed_weight_custom_qmin_qmax(self):
        ...
        """
        linear_fp32 = smith.nn.Linear(2, 2)
        qconfig = smith.ao.quantization.default_symmetric_qnnpack_qconfig
        w_obs = qconfig.weight()
        self.assertTrue(w_obs.quant_min == -127)
        self.assertTrue(w_obs.quant_max == 127)
        w_obs(linear_fp32.weight)
        weight_qparams = smith.ao.quantization.utils.get_qparam_dict(w_obs)
        weight_qparams["is_decomposed"] = True
        linear_ref = nnqr.Linear.from_float(linear_fp32, weight_qparams)
        linear_ref_traced = smith.fx.symbolic_trace(linear_ref)
        ...
        for n in linear_ref_traced.graph.nodes:
            ...
            if n.target in (
                ...
                smith.ops.quantized_decomposed.dequantize_per_tensor,
            ):
