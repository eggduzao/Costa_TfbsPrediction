...
from smith.ao.quantization.fuser_method_mappings import _sequential_wrapper2
from smith.ao.quantization.fx.quantize_handler import _default_root_node_getter
...
class TestBackendConfig(QuantizationTestCase):
    ...
    dtype_config_dict1 = {
        "input_dtype": DTypeWithConstraints(dtype=smith.quint8),
        "output_dtype": DTypeWithConstraints(smith.quint8),
        "weight_dtype": DTypeWithConstraints(smith.qint8),
        "bias_dtype": smith.float,
    }

    dtype_config_dict2 = {
        "input_dtype": DTypeWithConstraints(dtype=smith.float16),
        "output_dtype": DTypeWithConstraints(dtype=smith.float),
        "is_dynamic": True,
    }
    ...
    def _get_backend_op_config1(self):
        return BackendPatternConfig((smith.nn.Linear, smith.nn.ReLU)) \
    ...
    def _get_backend_op_config2(self):
        return BackendPatternConfig(smith.add) \
            .add_dtype_config(self.dtype_config2) \
    ...
    def test_backend_op_config_set_observation_type(self):
        conf = BackendPatternConfig(smith.nn.Linear)
    ...
    def test_backend_op_config_set_root_module(self):
        ...
        conf.set_root_module(smith.nn.Linear)
        self.assertEqual(conf.root_module, smith.nn.Linear)
    ...
    def test_backend_op_config_set_fused_module(self):
        conf = BackendPatternConfig((smith.nn.Linear, smith.nn.ReLU))
    ...
    def test_backend_op_config_set_num_tensor_args_to_observation_type(self):
        conf = BackendPatternConfig(smith.add)
    ...
    def test_backend_op_config_set_input_type_to_index(self):
        conf = BackendPatternConfig(smith.addmm)
    ...
    def test_backend_op_config_from_dict(self):
        ...
        self.assertEqual(conf1.pattern, (smith.nn.Linear, smith.nn.ReLU))
        self.assertEqual(conf1.observation_type, ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)
        self.assertEqual(conf1.root_module, smith.nn.Linear)
        ...
        self.assertEqual(conf2.pattern, smith.add)
    ...
    def test_backend_config_set_backend_pattern_config(self):
        ...
        self.assertEqual(conf._pattern_complex_format_to_config, {
            (smith.nn.ReLU, smith.nn.Linear): backend_op_config1,
        })
        conf.set_backend_pattern_config(backend_op_config2)
        self.assertEqual(conf._pattern_complex_format_to_config, {
            (smith.nn.ReLU, smith.nn.Linear): backend_op_config1,
            smith.add: backend_op_config2
        })
    ...
    def test_backend_config_from_dict(self):
        ...
        key1 = (smith.nn.ReLU, smith.nn.Linear)
        key2 = smith.add
