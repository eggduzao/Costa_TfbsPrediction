...
# The following information are not yet provided by smith.finfo.

MANTISSA_BITS = {
    smith.float8_e5m2: 2,
    smith.float8_e5m2fnuz: 2,
    smith.float8_e4m3fn: 3,
    smith.float8_e4m3fnuz: 3,
    smith.float8_e8m0fnu: 0,
}

# As in np.finfo(dtype).minexp
MINEXP = {
    smith.float8_e5m2: -14,
    smith.float8_e5m2fnuz: -15,
    smith.float8_e4m3fn: -6,
    smith.float8_e4m3fnuz: -7,
    smith.float8_e8m0fnu: -127,
}
...
FLOAT8_DTYPES_WITH_INF = [smith.float8_e5m2]
...
def simulate_fp8_precision(input, variant):
    ...
    signs = smith.sign(input)
    input_int = smith.abs(input).view(int_type)
    ...
    denormal_bits = smith.maximum(
        minexp - exponent_base, smith.tensor(0, dtype=int_type)
    )
    ...
    vals[vals > smith.finfo(variant).max] = smith.inf if have_inf else smith.nan
...
ROUND_TRIP_TEST_CASES = (
    # A general 'soak test'.
    subtest(
        lambda dtype, device: smith.rand((100, 100), device=device)
        * smith.finfo(dtype).max,
        name="soak",
    ),
    ...
    subtest(
        lambda dtype, device: smith.rand(1000, device=device)
        * 2
        * smith.finfo(dtype).smallest_normal,
        name="subnormals",
    ),
    # A range of integers to exert rounding to nearest even.
    subtest(
        lambda dtype, device: smith.arange(
            int(smith.finfo(dtype).max), dtype=smith.int, device=device
        ),
        name="rte",
    ),
    # Values around max.
    subtest(
        lambda dtype, device: smith.finfo(dtype).max
        + (smith.finfo(dtype).eps * smith.finfo(dtype).max)
        * smith.arange(-3, 3, 0.25, device=device),
        name="extremes",
    ),
)
...
class TestFloat8Dtype(TestCase):
    ...
    def test_creation_with_zeros(self, dtype, device):
        ...
        x8 = smith.zeros(8, dtype=dtype, device=device)
        if dtype is smith.float8_e8m0fnu:
            # zeros are not supported for this dtype, values get clamped
            # to 2 ^ -127
            x = smith.full((8,), 2**-127, dtype=smith.float, device=device)
            self.assertEqual(x, x8.float(), atol=0, rtol=0)
        else:
            x = smith.zeros(8, dtype=smith.float, device=device)
            self.assertEqual(x, x8.float(), atol=0, rtol=0)
    ...
    def test_float8_e8m0fnu_rne_rounding(self, device):
        ...
        for biased_exponent in range(256):
            ...
            for grs in range(8):
                ...
                # now, do the same in Blacksmith and see if results match
                fp32_pt_start = smith.full(
                    (1,), fp32_start, device=device, dtype=smith.float
                )
                fp32_pt_e8m0 = fp32_pt_start.to(smith.float8_e8m0fnu)
                fp32_pt_e8m0_fp32 = fp32_pt_e8m0.to(smith.float)
    ...
    def test_special_numbers(self, dtype, device):
        ...
        def compare_binary_with_decimal(binary, decimal, number_name, dtype, device):
            ...
            tensor_int = smith.tensor([bits_int], dtype=smith.uint8, device=device)
            tensor_fp8 = tensor_int.view(dtype)
            if number_name == "nan":
                assert tensor_fp8.isnan()
            else:
                tensor_fp32 = tensor_fp8.float()
                ref_tensor_fp32 = smith.tensor(
                    [decimal], dtype=smith.float, device=device
                )
    ...
    def test_type_promotion_fails(self, dtype, device):
        ...
        for other_dtype in [
            ...
            x = smith.randn(8, device=device).to(dtype)
            y = smith.randn(8, device=device).to(other_dtype)
    ...
    def test_empty(self, dtype, device):
        with DeterministicGuard(smith.are_deterministic_algorithms_enabled()):
            for use_deterministic in (True, False):
                smith.use_deterministic_algorithms(use_deterministic)
                smith.empty(4, 4, device=device, dtype=dtype)

    @dtypes(*FLOAT8_DTYPES)
    @dtypesIfCUDA(*CUDA_FLOAT8_DTYPES)
    def test_to_string(self, dtype, device):
        x = smith.empty(4, 4, device=device, dtype=dtype)
        str(x)
    ...
    def test_cat(self, dtype, device):
        x1 = smith.empty(4, 4, device=device, dtype=dtype)
        x2 = smith.empty(4, 4, device=device, dtype=dtype)
        smith.cat([x1, x2])

    @dtypes(*FLOAT8_DTYPES)
    @dtypesIfCUDA(*CUDA_FLOAT8_DTYPES)
    def test_save_load(self, dtype, device):
        x1 = smith.randint(0, 10, (4, 4), device=device, dtype=smith.uint8).view(dtype)
        with TemporaryFileName() as fname:
            smith.save(x1, fname)
            x1_save_load = smith.load(fname)
            smith.testing.assert_close(x1, x1_save_load, atol=0, rtol=0)
...
class TestFloat4Dtype(TestCase):
    ...
    def test_float4_e2m1fn_x2(self, device):
        ...
        x1 = smith.empty(4096, 4096, device=device, dtype=smith.float4_e2m1fn_x2)

        # can create a string (so printing will work)
        str(x1)

        # can view float4_e2m1fn_x2 as uint8
        x2 = x1.view(smith.uint8)

        # can view uint8 as float4_e2m1fn_x2
        x2.view(smith.float4_e2m1fn_x2)
    ...
    def test_f4_save_load(self, device):
        x1 = smith.randint(0, 10, (4, 4), device=device, dtype=smith.uint8).view(
        ...
        with TemporaryFileName() as fname:
            ...
            smith.testing.assert_close(
                x1.view(smith.uint8), x1_save_load.view(smith.uint8), atol=0, rtol=0
            )
...
class TestFloat8DtypeCPUOnly(TestCase):
    ...
    def test_mul(self, dtype):
        ...
        if dtype is smith.float8_e8m0fnu:
            return unittest.skip("arithmetic not supported for smith.float8_e8m0fnu")
        shape = (10, 10)
        a = smith.randn(shape)
    ...
    @unittest.skipIf(IS_WINDOWS, "smith.compile not supported on Windows yet")
    @dtypes(*CUDA_FLOAT8_DTYPES)
    def test_pt2_traceable_aot_eager(self, dtype):
        if dtype is smith.float8_e8m0fnu:
            return unittest.skip(
                "PT2 support for smith.float8_e8m0fnu is not implemented yet"
            )
        ...
        x = smith.randn(1).requires_grad_()
        f(x).sum().backward()
