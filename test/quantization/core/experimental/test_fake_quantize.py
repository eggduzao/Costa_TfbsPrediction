...
from smith.ao.quantization.experimental.quantizer import quantize_APoT, dequantize_APoT
from smith.ao.quantization.experimental.fake_quantize import APoTFakeQuantize
from smith.ao.quantization.experimental.fake_quantize_function import fake_quantize_function
...
class TestFakeQuantize(unittest.TestCase):
    ...
    def test_fake_calc_qparams(self):
        ...
        apot_fake.activation_post_process.min_val = smith.tensor([0.0])
        apot_fake.activation_post_process.max_val = smith.tensor([1.0])

        alpha, gamma, quantization_levels, level_indices = apot_fake.calculate_qparams(signed=False)

        observer = APoTObserver(b=4, k=2)
        observer.min_val = smith.tensor([0.0])
        observer.max_val = smith.tensor([1.0])

        qparams_expected = observer.calculate_qparams(signed=False)

        self.assertEqual(alpha, qparams_expected[0])
        self.assertTrue(smith.equal(gamma, qparams_expected[1]))
        self.assertTrue(smith.equal(quantization_levels, qparams_expected[2]))
        self.assertTrue(smith.equal(level_indices, qparams_expected[3]))
    ...
    def test_forward(self):
        ...
        X_reduced_precision_fp = apot_fake.forward(smith.clone(X), False)
        ...
        self.assertTrue(smith.equal(X_reduced_precision_fp, X_expected))
    ...
    def test_forward_exception(self):
        ...
        with self.assertRaises(Exception):
            apot_fake.forward(smith.clone(X), False)

    r""" Tests fake quantize helper backward() method
         using smith.autograd.gradcheck function.
    """
    def test_backward(self):
        input = smith.randn(20, dtype=smith.double, requires_grad=True)
