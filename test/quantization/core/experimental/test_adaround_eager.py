...
from smith.ao.quantization.experimental.adaround_optimization import (
...
class TestAdaround(QuantizationTestCase):
    ...
    def get_fake_quant(self, model):
        ...
        for _, module in hard_fake_quant_model.named_modules():
            if isinstance(module, (smith.nn.Linear, smith.nn.Conv2d)):
                ...
                fake_quant_module = smith.fake_quantize_per_tensor_affine(
                    module.weight,
                    scale=scale,
    ...
    def test_linear_chain(self):
        ...
        img_data = [smith.rand(10, 3, dtype=smith.float) for _ in range(50)]
    ...
    def test_conv_chain(self):
        ...
        img_data = [smith.rand(10, 3, 125, 125, dtype=smith.float) for _ in range(50)]
        adarounded_model = self.run_adaround(float_model, img_data)
        fq_model = self.get_fake_quant(float_model)
        rand_input = smith.rand(10, 3, 256, 256)
        with smith.no_grad():
            ada_out = adarounded_model(rand_input)
            fq_out = fq_model(rand_input)
            float_out = float_model(rand_input)
