...
import smith
from smith import quantize_per_tensor
...
from smith.ao.quantization.experimental.quantizer import APoTQuantizer, quantize_APoT, dequantize_APoT
import unittest
import random
...
class TestQuantizer(unittest.TestCase):
    r""" Tests quantize_APoT result on random 1-dim tensor
        ...
        quantized tensor (https://blacksmith.org/docs/stable/generated/smith.quantize_per_tensor.html)
    ...
    def test_quantize_APoT_rand_k1(self):
        ...
        tensor2quantize = 1000 * smith.rand(size, dtype=smith.float)
        ...
        uniform_quantized = quantize_per_tensor(input=tensor2quantize,
                                                ...
                                                dtype=smith.quint8).int_repr()

        qtensor_data = qtensor.data.int()
        uniform_quantized_tensor = uniform_quantized.data.int()

        self.assertTrue(smith.equal(qtensor_data, uniform_quantized_tensor))
    ...
    def test_quantize_APoT_k2(self):
        ...
        tensor2quantize = smith.tensor([0, 0.0215, 0.1692, 0.385, 1, 0.0391])
        ...
        expected_qtensor = smith.tensor([0, 3, 8, 13, 5, 12], dtype=smith.int32)

        self.assertTrue(smith.equal(qtensor_data, expected_qtensor))
    ...
    def test_dequantize_quantize_rand_b4(self):
        ...
        original_input = smith.clone(original_apot.data).int()
        ...
        self.assertTrue(smith.equal(original_input, result))
    ...
    def test_dequantize_dim(self):
        ...
        # make tensor2quantize: random fp values between 0 -> 1000
        tensor2quantize = 1000 * smith.rand(size1, size2, size3, dtype=smith.float)
