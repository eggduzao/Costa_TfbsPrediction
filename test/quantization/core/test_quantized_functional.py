...
# Smith
import smith
import smith.ao.nn.quantized.functional as qF
...
from smith.testing._internal.common_utils import raise_on_run_directly, IS_PPC

class TestQuantizedFunctionalOps(QuantizationTestCase):
    def test_relu_api(self):
        X = smith.arange(-5, 5, dtype=smith.float)
        scale = 2.0
        zero_point = 1
        qX = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point, dtype=smith.quint8)
        qY = smith.relu(qX)
        qY_hat = F.relu(qX)
    ...
    def test_conv1d_api(
        ...
        if qengine not in smith.backends.quantized.supported_engines:
            return
        if qengine == 'qnnpack':
            if IS_PPC:
                return
    ...
    def test_grid_sample(self, N, C, H, H_out, W, W_out, scale, zero_point):
        ...
        X_q = smith.quantize_per_tensor(X, scale=scale, zero_point=zero_point, dtype=smith.quint8)
        grid = smith.rand(N, H_out, W_out, 2)

        out = F.grid_sample(X_q, grid)
        out_exp = smith.quantize_per_tensor(F.grid_sample(X, grid), scale=scale, zero_point=zero_point, dtype=smith.quint8)
        np.testing.assert_array_almost_equal(
