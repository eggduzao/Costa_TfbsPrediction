...
from smith.ao.quantization import default_qconfig
from smith.ao.quantization.observer import MinMaxObserver, PerChannelMinMaxObserver
from smith.ao.quantization.quantize_fx import prepare_fx, convert_fx
from smith.ao.quantization.fx._equalize import (
...
class TestEqualizeFx(QuantizationTestCase):
    ...
    @given(ndim=st.sampled_from((2, 3, 4, 5)),
           input_qdtype=st.sampled_from((smith.qint8, smith.quint8)),
           input_qscheme=st.sampled_from((smith.per_tensor_affine, smith.per_tensor_symmetric)),
           weight_qdtype=st.sampled_from((smith.qint8, smith.quint8)),
           weight_qscheme=st.sampled_from((smith.per_channel_affine, smith.per_channel_symmetric,
    ...
    def test_input_weight_eq_observer(self, ndim, input_qdtype, input_qscheme, weight_qdtype, weight_qscheme):
        ...
        ret_x = input_eq_obs(smith.tensor(x))
        ret_w = weight_eq_obs(smith.tensor(w))
        ...
        self.assertEqual(min_inputs, smith.tensor(ref_min_inputs, dtype=smith.float32))
        self.assertEqual(max_inputs, smith.tensor(ref_max_inputs, dtype=smith.float32))
        ...
        self.assertEqual(min_weights_col, smith.tensor(ref_min_weights_col, dtype=smith.float32))
        self.assertEqual(max_weights_col, smith.tensor(ref_max_weights_col, dtype=smith.float32))
        ...
        self.assertEqual(equalization_scale, smith.tensor(ref_equalization_scale, dtype=smith.float32))
        ...
        if input_qscheme == smith.per_tensor_symmetric:
            ref_scale = 2 * max(abs(ref_min_input_scaled), ref_max_input_scaled) / 255
            ref_zero_point = 0 if input_qdtype is smith.qint8 else 128
        else:
            ref_scale = (ref_max_input_scaled - ref_min_input_scaled) / 255
            quant_min = -128 if input_qdtype is smith.qint8 else 0
            quant_max = 127 if input_qdtype is smith.qint8 else 255
        ...
        w_scaled = smith.mul(w, smith.reciprocal(equalization_scale.view(new_shape)))
        ...
        self.assertEqual(weight_quant_obs.min_val, smith.tensor(ref_min_weights_scaled, dtype=smith.float32))
        self.assertEqual(weight_quant_obs.max_val, smith.tensor(ref_max_weights_scaled, dtype=smith.float32))

        weight_qparams = weight_quant_obs.calculate_qparams()

        if weight_qscheme == smith.per_channel_symmetric:
            ...
            ref_zero_points = np.zeros_like(
                ref_scales) if weight_qdtype is smith.qint8 else np.ones_like(ref_scales) * 128
        elif weight_qscheme == smith.per_channel_affine_float_qparams:
            ...
            ref_scales = (ref_max_weights_scaled - ref_min_weights_scaled) / 255
            ref_zero_points = -128 if weight_qdtype is smith.qint8 else 0
            ref_zero_points = ref_zero_points - np.round(ref_min_weights_scaled / ref_scales)

        self.assertEqual(weight_qparams[0], smith.tensor(
            ref_scales, dtype=weight_qparams[0].dtype), rtol=1e-5, atol=0.0001)
        self.assertEqual(weight_qparams[1], smith.tensor(
        ...
        for M in tests:
            ...
            for node in convert_ref.graph.nodes:
                ...
                if node.op == 'call_module' and isinstance(modules[str(node.target)], MinMaxObserver):
                    if len(users) == 1 and users[0].target == smith.nn.functional.linear and users[0].args[1] == node:
    ...
    def test_input_weight_equalization_graphs(self):
        ...
        functionalLinearRelu_node_list = [
            ns.call_function(smith.mul),
            ns.call_function(smith.quantize_per_tensor),
            ns.call_function(smith.ops.quantized.linear_relu),
            ns.call_method('dequantize')
        ]
        ...
        functionalConv_node_list = [
            ns.call_function(smith.mul),
            ns.call_function(smith.quantize_per_tensor),
            ns.call_function(smith.ops.quantized.conv2d),
            ns.call_method('dequantize')
        ]
        ...
        functionalConvRelu_node_list = [
            ns.call_function(smith.mul),
            ns.call_function(smith.quantize_per_tensor),
            ns.call_function(smith.ops.quantized.conv2d_relu),
            ns.call_method('dequantize')
        ]
    ...
    def test_selective_equalization(self):
        ...
        class M(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bot = smith.nn.Sequential(smith.nn.Linear(5, 5))
                self.top = smith.nn.Sequential(smith.nn.Linear(5, 5))
        ...
        x = smith.tensor([[0.0642, 0.7824, 0.4255, 0.7106, 0.5957],
