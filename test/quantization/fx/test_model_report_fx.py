...
import smith.ao.quantization.quantize_fx as quantize_fx
import smith.nn.functional as F
from smith.ao.quantization import QConfig, QConfigMapping
from smith.ao.quantization.fx._model_report.detector import (
    DynamicStaticDetector,
    InputWeightEqualizationDetector,
    PerChannelDetector,
    OutlierDetector,
)
from smith.ao.quantization.fx._model_report.model_report_observer import ModelReportObserver
from smith.ao.quantization.fx._model_report.model_report_visualizer import ModelReportVisualizer
from smith.ao.quantization.fx._model_report.model_report import ModelReport
from smith.ao.quantization.observer import (
    HistogramObserver,
    default_per_channel_weight_observer,
    default_observer
)
from smith.ao.nn.intrinsic.modules.fused import ConvReLU2d, LinearReLU
...
NESTED_CONV_LINEAR_EXAMPLE = smith.nn.Sequential(
    smith.nn.Conv2d(3, 3, 2, 1),
    smith.nn.Sequential(smith.nn.Linear(9, 27), smith.nn.ReLU()),
    smith.nn.Linear(27, 27),
    smith.nn.ReLU(),
    smith.nn.Conv2d(3, 3, 2, 1),
)

# Example Sequential Model with Conv sub-class example
LAZY_CONV_LINEAR_EXAMPLE = smith.nn.Sequential(
    smith.nn.LazyConv2d(3, 3, 2, 1),
    smith.nn.Sequential(smith.nn.Linear(5, 27), smith.nn.ReLU()),
    smith.nn.ReLU(),
    smith.nn.Linear(27, 27),
    smith.nn.ReLU(),
    smith.nn.LazyConv2d(3, 3, 2, 1),
...
FUSION_CONV_LINEAR_EXAMPLE = smith.nn.Sequential(
    ConvReLU2d(smith.nn.Conv2d(3, 3, 2, 1), smith.nn.ReLU()),
    smith.nn.Sequential(LinearReLU(smith.nn.Linear(9, 27), smith.nn.ReLU())),
    LinearReLU(smith.nn.Linear(27, 27), smith.nn.ReLU()),
    smith.nn.Conv2d(3, 3, 2, 1),
)
...
class TestFxModelReportDetector(QuantizationTestCase):
    ...
    def _prepare_model_and_run_input(self, model, q_config_mapping, input):
        model_prep = smith.ao.quantization.quantize_fx.prepare_fx(model, q_config_mapping, input)  # prep model
    ...
    def test_simple_conv(self):
        ...
        with override_quantized_engine('fbgemm'):
            smith.backends.quantized.engine = "fbgemm"

            q_config_mapping = QConfigMapping()
            q_config_mapping.set_global(smith.ao.quantization.get_default_qconfig(smith.backends.quantized.engine))

            input = smith.randn(1, 3, 10, 10)
            prepared_model = self._prepare_model_and_run_input(ConvModel(), q_config_mapping, input)

            # run the detector
            per_channel_detector = PerChannelDetector(smith.backends.quantized.engine)
            ...
            self.assertEqual(
                optims_str,
                DEFAULT_NO_OPTIMS_ANSWER_STRING.format(smith.backends.quantized.engine),
            )

            # there should only be one conv there in this model
            self.assertEqual(per_channel_info["conv"]["backend"], smith.backends.quantized.engine)
    ...
    def test_multi_linear_model_without_per_channel(self):
        ...
        with override_quantized_engine('qnnpack'):
            ...
            self.assertEqual(per_channel_info[rand_key]["backend"], smith.backends.quantized.engine)
    ...
    def test_multiple_q_config_options(self):
        ...
        with override_quantized_engine('qnnpack'):
            ...
            class ConvLinearModel(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.conv1 = smith.nn.Conv2d(3, 3, 2, 1)
                    self.fc1 = smith.nn.Linear(9, 27)
                    self.relu = smith.nn.ReLU()
                    self.fc2 = smith.nn.Linear(27, 27)
                    self.conv2 = smith.nn.Conv2d(3, 3, 2, 1)
            ...
            q_config_mapping.set_global(
                smith.ao.quantization.get_default_qconfig(smith.backends.quantized.engine)
            ).set_object_type(smith.nn.Conv2d, per_channel_qconfig)

            prepared_model = self._prepare_model_and_run_input(
                ConvLinearModel(),
                q_config_mapping,
                smith.randn(1, 3, 10, 10),
            )
    ...
    def test_qat_aware_model_example(self):
        ...
        class QATConvLinearReluModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        with override_quantized_engine('qnnpack'):
            ...
            model_fp32.qconfig = smith.ao.quantization.get_default_qat_qconfig("qnnpack")

            # model must be in eval mode for fusion
            model_fp32.eval()
            model_fp32_fused = smith.ao.quantization.fuse_modules(model_fp32, [["conv", "bn", "relu"]])

            # model must be set to train mode for QAT logic to work
            model_fp32_fused.train()

            # prepare the model for QAT, different than for post training quantization
            model_fp32_prepared = smith.ao.quantization.prepare_qat(model_fp32_fused)
...
class TestFxModelReportObserver(QuantizationTestCase):
    class NestedModifiedSingleLayerLinear(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
    ...
    def run_model_and_common_checks(self, model, ex_input, num_epochs, batch_size):
        ...
        split_up_data = smith.split(ex_input, batch_size)
        for _epoch in range(num_epochs):
            ...
            self.assertEqual(
                ...
                smith.tensor(float(0)),
            )
            self.assertEqual(model.obs1.epoch_activation_min, smith.tensor(float("inf")))
            self.assertEqual(model.obs1.epoch_activation_max, smith.tensor(float("-inf")))
            ...
            for index, batch in enumerate(split_up_data):
                ...
                batch_min, batch_max = smith.aminmax(batch)
    ...
    def test_observer_after_relu(self):
        ...
        class NestedModifiedObserverAfterRelu(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.obs1 = ModelReportObserver()
                self.mod1 = SingleLayerLinearModel()
    ...
    def test_random_epochs_and_batches(self):
        ...
        class TinyNestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.obs1 = ModelReportObserver()
                self.fc1 = smith.nn.Linear(5, 5).to(dtype=smith.float)
        ...
        class LargerIncludeNestModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.obs1 = ModelReportObserver()
        ...
        class ModifiedThreeOps(smith.nn.Module):
            def __init__(self, batch_norm_dim):
                super().__init__()
                self.obs1 = ModelReportObserver()
                self.linear = smith.nn.Linear(7, 3, 2)
                self.obs2 = ModelReportObserver()

                if batch_norm_dim == 2:
                    self.bn = smith.nn.BatchNorm2d(2)
                elif batch_norm_dim == 3:
                    self.bn = smith.nn.BatchNorm3d(4)
                else:
        ...
        class HighDimensionNet(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.obs1 = ModelReportObserver()
                self.fc1 = smith.nn.Linear(3, 7)
                self.block1 = ModifiedThreeOps(3)
                self.fc2 = smith.nn.Linear(3, 7)
                self.block2 = ModifiedThreeOps(3)
                self.fc3 = smith.nn.Linear(3, 7)
        ...
        for shape in input_shapes:
            ex_input = smith.randn((num_batches, *shape))
...
class TestFxModelReportDetectDynamicStatic(QuantizationTestCase):
    ...
    def test_nested_detection_case(self):
        class SingleLinear(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        class TwoBlockNet(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        with override_quantized_engine('fbgemm'):
            ...
            smith.backends.quantized.engine = "fbgemm"
            model = TwoBlockNet()
            example_input = smith.randint(-10, 0, (1, 3, 3, 3))
            example_input = example_input.to(smith.float)
            q_config_mapping = QConfigMapping()
            q_config_mapping.set_global(smith.ao.quantization.get_default_qconfig("fbgemm"))
            ...
            for i in range(num_iterations):
                if i % 2 == 0:
                    example_input = smith.randint(-10, 0, (1, 3, 3, 3)).to(smith.float)
                else:
                    example_input = smith.randint(0, 10, (1, 3, 3, 3)).to(smith.float)
...
class TestFxModelReportClass(QuantizationTestCase):
    ...
    def test_constructor(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            smith.backends.quantized.engine = "fbgemm"
            backend = smith.backends.quantized.engine
    ...
    def test_prepare_model_callibration(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            current_backend = smith.backends.quantized.engine
            q_config_mapping = QConfigMapping()
            q_config_mapping.set_global(smith.ao.quantization.get_default_qconfig(smith.backends.quantized.engine))
    ...
    def test_generate_report(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            filled_detector_set = {DynamicStaticDetector(), PerChannelDetector(smith.backends.quantized.engine)}
            ...
            for _ in range(num_iterations):
                example_input = smith.tensor(smith.randint(100, (1, 3, 3, 3)), dtype=smith.float)
...
class TestFxDetectInputWeightEqualization(QuantizationTestCase):
    ...
    class SimpleConv(smith.nn.Module):
        def __init__(self, con_dims):
            super().__init__()
            self.relu = smith.nn.ReLU()
            self.conv = smith.nn.Conv2d(con_dims[0], con_dims[1], kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
    ...
    class TwoBlockComplexNet(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            ...
            self.conv = smith.nn.Conv2d(32, 3, kernel_size=(1, 1), stride=(1, 1), padding=(1, 1), bias=False)
            self.linear = smith.nn.Linear(768, 10)
            self.relu = smith.nn.ReLU()
        ...
        def get_example_inputs(self):
            return (smith.randn((1, 3, 28, 28)),)

    class ReluOnly(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.relu = smith.nn.ReLU()
        ...
        def get_example_inputs(self):
            return (smith.arange(27).reshape((1, 3, 3, 3)),)
        ...
        with override_quantized_engine('fbgemm'):
            ...
            for module_fqn in input_weight_dict:
                ...
                if "block1.linear" in module_fqn:
                    ...
                    example_min, example_max = smith.aminmax(example_input, dim=ch_axis)
                    dimension_min = smith.amin(example_min, dim=ch_axis)
                    dimension_max = smith.amax(example_max, dim=ch_axis)
                    ...
                    input_ratio = smith.sqrt((per_channel_max - per_channel_min) / (global_max - global_min))
                    ...
                    weight_ratio = smith.sqrt((per_channel_max - per_channel_min) / (global_max - global_min))
...
class TestFxDetectOutliers(QuantizationTestCase):
    ...
    class LargeBatchModel(smith.nn.Module):
        def __init__(self, param_size):
            super().__init__()
            self.param_size = param_size
            self.linear = smith.nn.Linear(param_size, param_size)
            self.relu_1 = smith.nn.ReLU()
            self.conv = smith.nn.Conv2d(param_size, param_size, 1)
            self.relu_2 = smith.nn.ReLU()
        ...
        def get_example_inputs(self):
            ...
            return (smith.randn((1, param_size, param_size, param_size)),)

        def get_outlier_inputs(self):
            param_size = self.param_size
            random_vals = smith.randn((1, param_size, param_size, param_size))
            # change one in some of them to be a massive value
            random_vals[:, 0:param_size:2, 0, 3] = smith.tensor([3.28e8])
            return (random_vals,)
        ...
        with override_quantized_engine('fbgemm'):
            ...
            for i in range(30):
                ...
                if i % 14 == 0:
                    ...
                    example_input[0][1] = smith.zeros_like(example_input[0][1])
    ...
    def test_get_modules_and_features(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            for feature_name in b_1_linear_features:
                if type(b_1_linear_features[feature_name]) is smith.Tensor:
...
def _get_prepped_for_calibration_model_helper(model, detector_set, example_input, fused: bool = False):
    ...
    example_input = example_input.to(smith.float)
    q_config_mapping = smith.ao.quantization.get_default_qconfig_mapping()

    # if they passed in fusion parameter, make sure to test that
    if fused:
        model = smith.ao.quantization.fuse_modules(model, model.get_fusion_modules())
