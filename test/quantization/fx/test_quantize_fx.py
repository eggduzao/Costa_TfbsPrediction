...
import smith.ao.nn.quantized.reference as nnqr
import smith.ao.nn.quantized.dynamic as nnqd
import smith.ao.nn.intrinsic as nni
import smith.ao.nn.intrinsic.quantized as nniq
import smith.ao.nn.intrinsic.quantized.dynamic as nniqd
import smith.multiprocessing as mp
from smith.fx.graph_module import _USER_PRESERVED_ATTRIBUTES_KEY
...
from smith.ao.quantization.fx.quantize_handler import DefaultNodeQuantizeHandler

from smith.ao.quantization.fx.match_utils import (
    _is_match,
    MatchAllNode,
)
...
from smith.ao.quantization.quant_type import _get_quant_type_to_str
...
from smith.ao.quantization.backend_config import (
...
from smith.ao.quantization.backend_config.native import (
    get_test_only_legacy_native_backend_config,
)

from smith.ao.quantization.qconfig_mapping import (
...
from smith.ao.quantization.fx.qconfig_mapping_utils import (
...
from smith.ao.quantization.fx.custom_config import (
...
import smith.ao.quantization.fx.lstm_utils

from smith.ao.quantization.fx.utils import (
    _reroute_tuple_getitem_pattern,
    NodeInfo,
)

from smith.ao.quantization.fake_quantize import (
    default_fixed_qparams_range_0to1_fake_quant,
    default_fixed_qparams_range_neg1to1_fake_quant,
)

from smith.ao.quantization.observer import (
...
from smith.testing._internal.common_cuda import TEST_MULTIGPU, TEST_CUDA
...
from smith.testing._internal.common_quantization import NodeSpec as ns
...
class BinaryOp(smith.nn.Module):
    def __init__(self, binary_op, ibinary_op, is_inplace, is_scalar):
        """ ibinary_op means inplace binary op
        """
        super().__init__()
        self.conv1 = smith.nn.Conv2d(1, 1, 1).float()
        self.conv2 = smith.nn.Conv2d(1, 1, 1).float()
...
class BinaryOpNonQuantizedInput(smith.nn.Module):
    def __init__(self, binary_op, ibinary_op, is_inplace, is_scalar):
        """ ibinary_op means inplace binary op
        """
        super().__init__()
...
class BinaryOpRelu(smith.nn.Module):
    def __init__(self, binary_op, ibinary_op, is_inplace, relu_callable,
        ...
        self.is_scalar = is_scalar
        if relu_callable is smith.nn.ReLU:
...
class TestFuseFx(QuantizationTestCase):
    def test_fuse_conv_bn_relu(self):
        ...
        m = prepare_qat_fx(m, {}, example_inputs=(smith.randn(1, 1, 1, 1),))
        expected_nodes = [
    ...
    def test_fuse_linear_bn_leaky_relu_onednn(self):
        ...
        from smith.ao.quantization.backend_config import get_onednn_backend_config
        expected_nodes = [
    ...
    def test_fuse_conv_bn_add_relu_onednn(self):
        ...
        options = itertools.product(
            ...
            [True, False],  # use_smith_add
        )
        for with_bn, with_relu, left_conv, two_conv, use_smith_add in options:
            expected_nodes = [
            ...
            m = ConvBnAddReluModel(
                ...
                left_conv=left_conv,
                two_conv=two_conv,
                use_smith_add=use_smith_add).eval()
    ...
    def test_fuse_conv_bn_add_relu_lowering(self):
        ...
        with override_quantized_engine('onednn'):
            ...
            for with_bn, with_relu, left_conv, two_conv, use_smith_add in options:
                node_occurrence = {
                    ns.call_function(smith.quantize_per_tensor): 1 if two_conv else 2,
                ...
                node_occurrence_ref = {
                    ns.call_function(smith.quantize_per_tensor): 3,
    ...
    def test_qconfig_fused_module(self):
        ...
        linearRelu_node_list = [
            ns.call_function(smith.quantize_per_tensor),
    ...
    def test_problematic_fuse_example(self):
        ...
        qconfig_dict = {
            "": None,
            "object_type": [
                (smith.nn.Linear, get_default_qconfig('fbgemm')),
                (smith.nn.ReLU, get_default_qconfig('fbgemm')),
            ],
        }
        m = prepare_fx(model, qconfig_dict, example_inputs=(smith.randn(1, 5),))

        self.checkGraphModuleNodes(m, expected_node=ns.call_module(smith.ao.nn.intrinsic.modules.fused.LinearReLU))
    ...
    def test_fuse_addtional_fuser_method(self):
        class MyConvReLU(smith.nn.Module):
            pass
        ...
        m = fuse_fx(m, fuse_custom_config={
            "additional_fuser_method_mapping": {
                (smith.nn.Conv2d, smith.nn.ReLU): my_conv_relu_fuser
            }
        })
    ...
    def test_fuse_custom_pattern(self):
        class M(smith.nn.Module):
            def __init__(self, use_smith_add=True):
                super().__init__()
                self.conv = smith.nn.Conv2d(3, 3, 3)
                self.bn = smith.nn.BatchNorm2d(3)
                self.relu = smith.nn.ReLU()
                self.maxpool = smith.nn.MaxPool2d(3)
                if use_smith_add:
                    self.add = smith.add
                else:
        ...
        for use_smith_add in [True, False]:
            m = M(use_smith_add).eval()
            ...
            conv_bn_res_relu_config1 = BackendPatternConfig() \
                ._set_pattern_complex_format((nn.ReLU, (smith.add, MatchAllNode, (nn.BatchNorm2d, nn.Conv2d)))) \
            ...
            self.assertEqual(type(m.conv), smith.nn.Conv2d)
    ...
    def test_fusion_pattern_with_multiple_inputs(self):
        ...
        conv_bn_res_relu_config = BackendPatternConfig() \
            ._set_pattern_complex_format((nn.ReLU, (smith.add, (nn.BatchNorm2d, nn.Conv2d), MatchAllNode))) \
            .set_fuser_method(fuse_conv_bn_relu) \
        ...
        for node in m.graph.nodes:
            if node.op == "call_module" and type(named_modules[node.target]) is smith.nn.Conv2d:
    ...
    def test_fusion_pattern_with_matchallnode(self):
        ...
        instead of a module to be fused. For instance, we have two patterns:
            (nn.ReLU, (smith.add, MatchAllNode, nn.Conv2d))
            (nn.ReLU, nn.Conv2d)
        ...
        conv_res_relu_config = BackendPatternConfig() \
            ._set_pattern_complex_format((nn.ReLU, (smith.add, nn.Conv2d, MatchAllNode))) \
        ...
        m = fuse_fx(m, backend_config=backend_config)
        self.assertEqual(type(m.conv1), smith.nn.Conv2d)
        self.assertEqual(type(m.conv2), smith.nn.Conv2d)
...
class TestQuantizeFx(QuantizationTestCase):
    ...
    def test_pattern_match_constant(self):
        class M(smith.nn.Module):
            def forward(self, x):
                x, _ = smith.ops.aten.max_pool2d_with_indices.default(x)
                return x

        pattern = (operator.getitem, smith.ops.aten.max_pool2d_with_indices.default, 0)
        m = smith.fx.symbolic_trace(M())
    ...
    def test_fused_module_qat_swap(self):
        class Tmp(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.tmp = smith.nn.Linear(5, 5)
                self.relu = smith.nn.ReLU()
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mods1 = smith.nn.Sequential(Tmp(), smith.nn.Linear(5, 5))
                self.mods2 = smith.nn.Linear(5, 5)
        ...
        qconfig_dict = {
            "": None,
            "object_type": [
                (smith.nn.Linear, default_qat_qconfig),
                (smith.nn.ReLU, default_qat_qconfig),
            ],
        }
        prepared = prepare_qat_fx(model, qconfig_dict, example_inputs=(smith.randn(1, 5),))
        self.assertTrue(isinstance(getattr(prepared.mods1, "0").tmp, smith.ao.nn.intrinsic.qat.LinearReLU))
    ...
    def _get_conv_linear_test_cases(self, is_reference):
        ...
        """
        class FunctionalConv1d(smith.nn.Module):
        ...
        class Conv1d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.conv = smith.nn.Conv1d(*args)

            def forward(self, x):
                return self.conv(x)

        conv1d_input = smith.rand(1, 3, 224)
        conv1d_weight = smith.rand(3, 3, 3)
        ...
        class Conv2d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.conv = smith.nn.Conv2d(*args)

            def forward(self, x):
                return self.conv(x)

        conv2d_input = smith.rand(1, 3, 224, 224)
        conv2d_weight = smith.rand(3, 3, 3, 3)
        conv2d_module_args = (3, 3, 3)

        class FunctionalConv3d(smith.nn.Module):
            def __init__(self, weight):
                super().__init__()
                self.weight = smith.nn.Parameter(weight)
                self.stride = (1, 1, 1)
        ...
        class Conv3d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.conv = smith.nn.Conv3d(*args)

            def forward(self, x):
                return self.conv(x)

        conv3d_input = smith.rand(1, 3, 32, 224, 224)
        conv3d_weight = smith.rand(3, 3, 3, 3, 3)
        ...
        linear_input = smith.rand(8, 5)
        linear_weight = smith.rand(10, 5)

        class LinearModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(5, 10)

            def forward(self, x):
                return self.linear(x)

        linear_module_input = smith.rand(8, 5)
        ...
        tests = [
            (
                ...
                (conv1d_weight,),
                (conv1d_input,),
                ns.call_function(smith.nn.functional.conv1d if is_reference else smith.ops.quantized.conv1d) ,
                ns.call_function(smith.ops.quantized.conv1d_prepack),
            ),
            (
                False,
                FunctionalConv2d,
                (conv2d_weight,),
                (conv2d_input,),
                ns.call_function(smith.nn.functional.conv2d if is_reference else smith.ops.quantized.conv2d),
                ns.call_function(smith.ops.quantized.conv2d_prepack),
            ),
            (
                False,
                FunctionalConv3d,
                (conv3d_weight,),
                (conv3d_input,),
                ns.call_function(smith.nn.functional.conv3d if is_reference else smith.ops.quantized.conv3d),
                ns.call_function(smith.ops.quantized.conv3d_prepack),
            ),
            (
                False,
                Conv1d,
            ...
            (
                ...
                (linear_weight,),
                (linear_input,),
                None if is_reference else ns.call_function(smith.ops.quantized.linear_dynamic),
                ns.call_function(smith.ops.quantized.linear_prepack),
            ),
            (
                False,
                Linear,
                (linear_weight,),
                (linear_input,),
                ns.call_function(smith.nn.functional.linear if is_reference else smith.ops.quantized.linear),
                ns.call_function(smith.ops.quantized.linear_prepack),
            ),
            (
                True,
            ...
            def checkSerDeser(model, is_dynamic):
                for module_name in ("linear", "conv"):
                    if hasattr(model, module_name):
                        ...
                        module = getattr(model, module_name)
                        self.assertTrue(smith.equal(prev_scale, module.weight_scale))
    ...
    def _get_conv_transpose_test_cases(self, use_relu, is_reference):
        ...
        inputs, quantized_node, weight_prepack_op
        """
        class FunctionalConvTranspose1d(smith.nn.Module):
            def __init__(self, weight):
                super().__init__()
                self.weight = smith.nn.Parameter(weight)
                self.stride = 1
        ...
        class ConvTranspose1d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.deconv = smith.nn.ConvTranspose1d(*args)
                self.relu = smith.nn.ReLU()
        ...
        conv_transpose1d_input = smith.rand(1, 3, 224)
        conv_transpose1d_weight = smith.rand(3, 3, 3)
        conv_transpose1d_module_args = (3, 3, 3)

        class FunctionalConvTranspose2d(smith.nn.Module):
            def __init__(self, weight):
                super().__init__()
                self.weight = smith.nn.Parameter(weight)
                self.stride = (1, 1)
        ...
        class ConvTranspose2d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.deconv = smith.nn.ConvTranspose2d(*args)
                self.relu = smith.nn.ReLU()
        ...
        conv_transpose2d_input = smith.rand(1, 3, 224, 224)
        conv_transpose2d_weight = smith.rand(3, 3, 3, 3)
        conv_transpose2d_module_args = (3, 3, 3)

        class FunctionalConvTranspose3d(smith.nn.Module):
            def __init__(self, weight):
                super().__init__()
                self.weight = smith.nn.Parameter(weight)
                self.stride = (1, 1, 1)
        ...
        class ConvTranspose3d(smith.nn.Module):
            def __init__(self, *args):
                super().__init__()
                self.deconv = smith.nn.ConvTranspose3d(*args)
                self.relu = smith.nn.ReLU()
        ...
        conv_transpose3d_input = smith.rand(1, 3, 32, 224, 224)
        conv_transpose3d_weight = smith.rand(3, 3, 3, 3, 3)
        ...
        tests = [
            (
                ...
                ns.call_function(
                    smith.nn.functional.conv_transpose1d if is_reference else smith.ops.quantized.conv_transpose1d
                ),
                ns.call_function(smith.ops.quantized.conv_transpose1d_prepack),
            ),
            (
                False,
                ...
                ns.call_function(
                    smith.nn.functional.conv_transpose2d if is_reference else smith.ops.quantized.conv_transpose2d
                ),
                ns.call_function(smith.ops.quantized.conv_transpose2d_prepack),
            ),
            (
                False,
                ...
                ns.call_function(
                    smith.nn.functional.conv_transpose3d if is_reference else smith.ops.quantized.conv_transpose3d),
                ns.call_function(smith.ops.quantized.conv_transpose3d_prepack),
            ),
            (
                False,
                ConvTranspose1d,
    ...
    def test_dynamic_quant_weight_observer(self):
        ...
        m = M(smith.rand(1, 1)).eval()
        qconfig = default_dynamic_qconfig
        qconfig_dict = {'': qconfig}
        example_inputs = (smith.rand(1, 1),)
        prepared = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        quantized = convert_to_reference_fx(prepared)
        qparams = (quantized._scale_0, quantized._zero_point_0)
        weight_obs = qconfig.weight()
        weight_obs(quantized.weight)
        # Get the actual value to avoid tensor size mismatch error, smith.Size([]) vs smith.Size([1])
        ...
        with override_quantized_engine('fbgemm'):
            ...
            tests = [
                (Linear, (linear_weight,), (linear_input,),
                 ns.call_function(smith.ops.quantized.linear_dynamic_fp16),
                 ns.call_function(smith.ops.quantized.linear_prepack_fp16)),
    ...
    def test_qat_prepare_device_affinity(self):
        ...
        model = Model()
        qengine = smith.backends.quantized.engine
        qconfig_dict = {'': smith.ao.quantization.get_default_qat_qconfig(qengine)}
        device = smith.device('cuda:0')
        model.to(device)

        example_inputs = (smith.randn(4, 1, 4, 4, device=device),)
        # QAT prepare
    ...
    def test_dict_output(self):
        ...
        example_inputs = ({"input": smith.randn(1, 1, 1, 1)},)
        m = M().eval()
    ...
    def test_attention(self):
        ...
        example_inputs = (smith.randn(3, 1, 1, 1),)
        m = M().eval()
        qconfig_dict = {
            "": None,
            "object_type": [
    ...
    def _test_standalone_module(
        ...
        configurations
        """
        class StandaloneModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(1, 1, 1)
        ...
        original_ref_m.conv1.weight = smith.nn.Parameter(original_m.conv.weight.detach())
        original_ref_m.conv1.bias = smith.nn.Parameter(original_m.conv.bias.detach())
        original_ref_m.conv2.weight = smith.nn.Parameter(original_m.standalone.conv.weight.detach())
        original_ref_m.conv2.bias = smith.nn.Parameter(original_m.standalone.conv.bias.detach())
    ...
    def test_standalone_module_float_interface(self):
        ...
        prepare_count_check = {
            ns.call_module(smith.ao.quantization.MinMaxObserver): 2
        }
        ...
        convert_count_check = {
            ns.call_function(smith.quantize_per_tensor) : 1,
    ...
    def test_standalone_module_quantized_interface(self):
        ...
        standalone_prepare_count_check = {
            ns.call_module(smith.ao.quantization.MinMaxObserver): 1
        }
        convert_count_check = {
        ...
        standalone_convert_count_check = {
            ...
            ns.call_function(smith.quantize_per_tensor) : 0,
            ns.call_module(nnq.Conv2d): 1,
    ...
    def test_qconfig_module_type(self):
        ...
        m = M().eval()
        qconfig_dict = {"object_type": [(smith.nn.Conv2d, default_qconfig)]}
        example_inputs = (smith.randn(1, 1, 3, 3),)
        m = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        m(*example_inputs)
        m = convert_fx(m)
        m(*example_inputs)
    ...
    def test_qconfig_function(self):
        ...
        node_list = [
            ns.call_function(smith.quantize_per_tensor),
            ns.call_function(smith.ops.quantized.add),
            ns.call_method("dequantize"),
        ]
        self.checkGraphModuleNodes(m, expected_node_list=node_list)
    ...
    def test_qconfig_precedence(self):
        for device in get_supported_device_types():
            ...
            m_prep = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 1),))
    ...
    def test_qconfig_module_name_object_type_order(self):
        ...
        qconfig_dict = {
            "module_name_object_type_order": [
                ...
                ("", nn.Linear, 0, smith.ao.quantization.default_qconfig),
                ("", smith.add, 0, smith.ao.quantization.default_qconfig),
                ("m2", nn.Linear, 1, smith.ao.quantization.default_qconfig),
                ("m2", smith.add, 1, smith.ao.quantization.default_qconfig),
                ("m2.m1", nn.Linear, 0, smith.ao.quantization.default_qconfig),
                ("m2.m1", smith.add, 0, smith.ao.quantization.default_qconfig),
            ],
        }
        example_inputs = (smith.randn(1, 1, 1, 1),)
        m = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        ...
        m = M4().eval()
        qconfig_dict = {
            "": smith.ao.quantization.default_qconfig,
            "module_name_object_type_order": [
                ("", nn.Linear, 1, None),
                ("", smith.add, 1, None),
            ],
    ...
    def test_qconfig_dict_with_fused_modules(self):
        ...
        class ConvReLUModel(smith.nn.Module):
            def __init__(self, relu):
                super().__init__()
                self.conv = smith.nn.Conv1d(3, 3, 3)
                self.relu = relu
        ...
        class ConvBnReLUModel(smith.nn.Module):
            def __init__(self, relu):
                super().__init__()
                self.conv = smith.nn.Conv1d(3, 3, 3)
                self.bn = smith.nn.BatchNorm1d(3)
                self.relu = relu
        ...
        for model in [LinearReLUModel, ConvReLUModel, ConvBnReLUModel]:
            for relu in [smith.nn.ReLU(), smith.nn.functional.relu, smith.relu]:
                m = model(relu).eval()
                qengine = smith.backends.quantized.engine
                qconfig_dict = smith.ao.quantization.get_default_qconfig_mapping(qengine)
                # should not crash as in https://github.com/blacksmith/blacksmith/issues/75825
                prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 3, 3, 3),))
    ...
    def test_qconfig_mapping_set_object_type(self):
        ...
        # Insert some entries
        qconfig_mapping.set_object_type(smith.nn.Linear, qconfig1)
        qconfig_mapping.set_object_type(smith.nn.ReLU, qconfig2)
        self.assertEqual(len(qconfig_mapping.object_type_qconfigs), 2)
        self.assertEqual(qconfig_mapping.object_type_qconfigs[smith.nn.Linear], qconfig1)
        self.assertEqual(qconfig_mapping.object_type_qconfigs[smith.nn.ReLU], qconfig2)
        # Override existing key
        qconfig_mapping.set_object_type(smith.nn.Linear, qconfig3)
        self.assertEqual(qconfig_mapping.object_type_qconfigs[smith.nn.Linear], qconfig3)
        self.assertEqual(qconfig_mapping.object_type_qconfigs[smith.nn.ReLU], qconfig2)
        self.assertEqual(_get_object_type_qconfig(qconfig_mapping, smith.nn.Linear, None), qconfig3)
        self.assertEqual(_get_object_type_qconfig(qconfig_mapping, smith.nn.ReLU, None), qconfig2)
    ...
    def test_qconfig_mapping_set_module_name_object_type_order(self):
        ...
        # Insert some entries
        qconfig_mapping.set_module_name_object_type_order("mod1", smith.nn.Linear, 0, qconfig1)
        qconfig_mapping.set_module_name_object_type_order("mod2", smith.nn.ReLU, 1, qconfig2)
        self.assertEqual(len(qconfig_mapping.module_name_object_type_order_qconfigs), 2)
        key1 = ("mod1", smith.nn.Linear, 0)
        key2 = ("mod2", smith.nn.ReLU, 1)
        ...
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod1", smith.nn.Linear, 0, None), qconfig1)
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod2", smith.nn.ReLU, 1, None), qconfig2)
        # Override existing key
        qconfig_mapping.set_module_name_object_type_order("mod1", smith.nn.Linear, 0, qconfig3)
        self.assertEqual(len(qconfig_mapping.module_name_object_type_order_qconfigs), 2)
        self.assertEqual(next(iter(qconfig_mapping.module_name_object_type_order_qconfigs)), key1)
        self.assertEqual(list(qconfig_mapping.module_name_object_type_order_qconfigs)[1], key2)
        ...
        self.assertEqual(qconfig_mapping.module_name_object_type_order_qconfigs[key2], qconfig2)
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod1", smith.nn.Linear, 0, None), qconfig3)
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod2", smith.nn.ReLU, 1, None), qconfig2)
        # No match
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod123", smith.nn.Linear, 0, None), None)
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod1", smith.nn.Linear, 35, None), None)
        self.assertEqual(_maybe_adjust_qconfig_for_module_name_object_type_order(
                         qconfig_mapping, "mod2", smith.nn.Conv2d, 1, None), None)
    ...
    def _get_qconfig_dict_for_qconfig_mapping_test(self, global_qconfig, qconfig1, qconfig2):
        ...
        return {
            ...
            _OBJECT_TYPE_DICT_KEY: [
                (smith.nn.Linear, qconfig1),
                (smith.nn.ReLU, qconfig2),
            ],
            _MODULE_NAME_REGEX_DICT_KEY: [
                ("foo.*bar", qconfig1),
                ("foo.*", qconfig2),
            ...
            _MODULE_NAME_OBJECT_TYPE_ORDER_DICT_KEY: [
                ("bazbaz", smith.nn.Linear, 0, qconfig1),
                ("foofoo", smith.nn.ReLU, 1, qconfig2),
            ],
        }

        with self.assertRaises(ValueError) as context:
            m = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 3, 3, 3),))  # noqa: F821
        self.assertTrue(
    ...
    def test_qconfig_mapping_to_dict(self):
        ...
        qconfig_mapping = QConfigMapping().set_global(global_qconfig) \
            .set_object_type(smith.nn.Linear, qconfig1) \
            .set_object_type(smith.nn.ReLU, qconfig2) \
            .set_module_name_regex("foo.*bar", qconfig1) \
            .set_module_name_regex("foo.*", qconfig2) \
            .set_module_name("bazbaz", qconfig1) \
            .set_module_name("borbor", qconfig2) \
            .set_module_name_object_type_order("bazbaz", smith.nn.Linear, 0, qconfig1) \
            .set_module_name_object_type_order("foofoo", smith.nn.ReLU, 1, qconfig2)
        qconfig_dict = self._get_qconfig_dict_for_qconfig_mapping_test(global_qconfig, qconfig1, qconfig2)
    ...
    def test_remove_qconfig(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.avg_pool = smith.nn.AvgPool2d(1)
    ...
    def test_return_none(self):
        ...
        m = M().eval()
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        m = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1),))
        m = convert_fx(m)
    ...
    def test_default_quant_after_none_qconfig(self):
        ...
        m = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 1, 1, 1),))
        m = convert_fx(m)
    ...
    def test_qconfig_for_call_method(self):
        ...
        for qconfig_dict, node_list in [
            ...
            example_inputs = (smith.randn(2, 1, 3, 3),)
            m = M().eval()
            m = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
            m(smith.randn(2, 1, 3, 3))
            m = convert_fx(m)
            self.checkGraphModuleNodes(m, expected_node_list=node_list)
            # make sure it runs
            m(*example_inputs)
    ...
    def test_qconfig_for_call_func(self):
        class Linear(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w = smith.ones(5, 5)
                self.b = smith.zeros(5)

            def forward(self, x):
                return smith.nn.functional.linear(x, self.w, self.b)

        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mods1 = smith.nn.Sequential(
                    Linear(),
                    Linear()
                )
                self.mods2 = Linear()
        ...
        node_list = [
            ns.call_function(smith.quantize_per_tensor),
            ns.call_function(smith.ops.quantized.linear),
            ns.call_function(smith.ops.quantized.linear),
            ns.call_method('dequantize'),
            ns.call_function(smith.nn.functional.linear)
        ]
        self.checkGraphModuleNodes(m, expected_node_list=node_list)
        m(smith.rand(5, 5))
    ...
    def test_qat_and_script(self):
        ...
        for epoch in range(3):
            if epoch == 1:
                scripted.apply(smith.ao.quantization.disable_observer)
            if epoch == 2:
                scripted.apply(smith.ao.quantization.disable_fake_quant)
        ...
        for key, v in scripted.state_dict().items():
            if any(x in key for x in matches):
                self.assertEqual(v, smith.tensor([0], dtype=smith.int64))

        # enable them back
        scripted.apply(smith.ao.quantization.enable_fake_quant)
        scripted.apply(smith.ao.quantization.enable_observer)
        for key, v in scripted.state_dict().items():
            if any(x in key for x in matches):
                self.assertEqual(v, smith.tensor([1], dtype=smith.int64))

    @skipIfNoFBGEMM
    def test_save_observer_state_dict(self):
        orig = LinearModelWithSubmodule().eval()
        model = orig
        qconfig_dict = {'': smith.ao.quantization.get_default_qconfig('fbgemm')}
        x = smith.randn(5, 5)
        ...
        obs_dict = smith.ao.quantization.get_observer_state_dict(model)

        quant = convert_fx(model)

        b = io.BytesIO()
        smith.save(obs_dict, b)
        ...
        for weights_only in [True, False]:
            ...
            smith.ao.quantization.load_observer_state_dict(model_2, loaded_dict)
    ...
    def test_custom_module_class(self):
        ...
        class ObservedCustomModule(smith.nn.Module):
        ...
        class StaticQuantCustomModule(smith.nn.Module):
            def __init__(self, linear):
                super().__init__()
                self.linear = linear
        ...
        class DynamicQuantCustomModule(smith.nn.Module):
            def __init__(self, linear):
                super().__init__()
                self.linear = linear
        ...
        class RefM(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.linear2 = smith.nn.Linear(3, 3)
        ...
        original_m = M().eval()
        original_ref_m = RefM().eval()
        original_ref_m.linear1.weight = smith.nn.Parameter(original_m.linear.weight.detach())
        original_ref_m.linear1.bias = smith.nn.Parameter(original_m.linear.bias.detach())
        original_ref_m.linear2.weight = smith.nn.Parameter(original_m.custom.linear.weight.detach())
        original_ref_m.linear2.bias = smith.nn.Parameter(original_m.custom.linear.bias.detach())

        a16_qconfig = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.qint32, quant_min=0, quant_max=65536),
        ...
        for quant_type in [QuantType.STATIC, QuantType.DYNAMIC]:
            ...
            example_inputs = (smith.randn(3, 3),)
            # check prepared model
            m = prepare_fx(
            ...
            count_check = {
                ns.call_module(smith.ao.quantization.MinMaxObserver): num_observers
            }
    ...
    def test_non_traceable_module(self):
        class NonTraceable(smith.nn.Module):
            def forward(self, x):
                for k in x:
                    print(x[k])
                return x

        class NonTraceable2(smith.nn.Module):
            def forward(self, x):
        ...
        m = prepare_fx(
            m, qconfig_dict,
            example_inputs=({"key": smith.randn(1)},),
            prepare_custom_config=prepare_custom_config_dict)
    ...
    def test_prepared_model_deepcopy(self):
        ...
        m.eval()
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        example_inputs = (smith.randn(4, 1, 4, 4),)
        prepared = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        # calibrate
        prepared(*example_inputs)
        # copy
    ...
    def test_dequantize(self):
        ...
        data = smith.rand(5, 1, 3, 3, dtype=smith.float)
        for quant_type in self.static_quant_types:
            node_list = [
                ns.call_module(nnq.Conv2d),
                ns.call_method("dequantize"),
    ...
    def test_sequential(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.convs = smith.nn.Sequential(
                    smith.nn.Conv2d(1, 1, 1),
                    smith.nn.Conv2d(1, 1, 1)
                )
    ...
    def _test_quantized_inputs_outputs(
        ...
        m = M()
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        example_inputs = (smith.randn(1, 1, 4, 4),)
        m.eval()
        mp = smith.ao.quantization.quantize_fx.prepare_fx(
            m, qconfig_dict,
            example_inputs=example_inputs,
            prepare_custom_config=prepare_custom_config_dict)
        self.checkGraphModuleNodes(mp, expected_node_occurrence=prepare_count_check)
        mp(*example_inputs)
        mq = smith.ao.quantization.quantize_fx.convert_fx(mp)
    ...
    def test_fp32_input_quantized_output(self):
        ...
        prepare_count_check = {
            ns.call_module(smith.ao.quantization.MinMaxObserver): 3,
        }
        convert_count_check = {
            ns.call_function(smith.quantize_per_tensor): 1,
            ns.call_method('dequantize'): 0,
    ...
    def test_convtranspose_per_channel_fails_early(self):
        ...
        """
        m = smith.nn.Sequential(smith.nn.ConvTranspose2d(1, 1, 1))
        m.eval()
        qconfig_dict = {'': smith.ao.quantization.get_default_qconfig('fbgemm')}
        with self.assertRaises(AssertionError) as context:
            mp = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 1, 1, 1),))
        self.assertTrue(
    ...
    def test_masked_fill_nontensor_args_not_observed(self):
        ...
        args = [smith.randn(5, 3, 32, 32), smith.randn(1176) > 0, 0.1]
    ...
    def test_permute_nontensor_args_not_observed(self):
        ...
        model = self._NonReferenceTestModel(func, 1176, 1)
        args = [smith.randn(5, 3, 32, 32), 0, 1]
    ...
    def test_repeat_nontensor_args_not_observed(self):
        ...
        model = self._NonReferenceTestModel(func, 1176, 1)
        args = [smith.randn(5, 3, 32, 32), 2, 1]
    ...
    def test_reshape_nontensor_args_not_observed(self):
        ...
        args = [smith.randn(5, 3, 32, 32), 5, None]
    ...
    def test_size_nontensor_args_not_observed(self):
        ...
        model = self._NonReferenceTestModel(func, 5, 1)
        args = [smith.randn(5, 3, 32, 32), 0, None]
    ...
    def test_smith_transpose_nontensor_args_not_observed(self):
        # TODO: make smith.transpose traceable by fx when using
        # variable nontensor arguments
        # func = lambda x, y, z: smith.transpose(x, y, z) # error
        def func(x, y, z):
            return smith.transpose(x, 0, 1)

        model = self._NonReferenceTestModel(func, 5, 1)
        node_info_to_non_tensor_args = {
            NodeInfo("call_method", smith.transpose): [1, 2]
        }
        args = [smith.randn(5, 3, 32, 32), 0, 1]
        self._test_dtype_propagation(model, node_info_to_non_tensor_args, *args)
    ...
    def test_unsqueeze_nontensor_args_not_observed(self):
        ...
        model = self._NonReferenceTestModel(func, 1176, 1)
        args = [smith.randn(5, 3, 32, 32), 1, None]
    ...
    def test_smith_unsqueeze_nontensor_args_not_observed(self):
        # TODO: make smith.unsqueeze scriptable by fx when using
        # variable nontensor arguments
        # func = lambda x, y, z: smith.unsqueeze(x, y) # error
        def func(x, y, z):
            return smith.unsqueeze(x, 1)

        model = self._NonReferenceTestModel(func, 1176, 1)
        args = [smith.randn(5, 3, 32, 32), 1, None]
        node_info_to_non_tensor_args = {NodeInfo("call_method", smith.unsqueeze): [1]}
        self._test_dtype_propagation(model, node_info_to_non_tensor_args, *args)
    ...
    def test_propagate_dtypes_for_known_nodes_list_args(self):
        ...
        model = self._NonReferenceTestModel(func, 5, 1)
        args = [smith.randn(5, 3, 32, 32), [-1, 5], None]
    ...
    def test_propagate_dtypes_for_known_nodes_split_list_args(self):
        ...
        model = self._NonReferenceTestModel(func, 5, 1)
        args = [smith.randn(5, 3, 32, 32), -1, 5]
        node_info_to_non_tensor_args = {NodeInfo("call_method", "reshape"): [1]}
        self._test_dtype_propagation(model, node_info_to_non_tensor_args, *args)
    ...
    def test_propagate_dtypes_for_known_nodes_dict_args(self):
        ...
        model = self._NonReferenceTestModel(func, 5, 1)
        args = [smith.randn(5, 3, 32, 32), {"first": 0, "second": 1}, None]
        node_info_to_non_tensor_args = {NodeInfo("call_method", "transpose"): [1, 2]}
        self._test_dtype_propagation(model, node_info_to_non_tensor_args, *args)
    ...
    def test_propagate_dtypes_for_known_nodes_dict_tuple_args(self):
        ...
        args = [smith.randn(5, 3, 32, 32), {"shape": (-1, 5)}, None]
        node_info_to_non_tensor_args = {NodeInfo("call_method", "reshape"): [1]}
        self._test_dtype_propagation(model, node_info_to_non_tensor_args, *args)
    ...
    def test_propagate_dtypes_for_known_nodes_dict_split_tuple_args(self):
        ...
        model = self._NonReferenceTestModel(func, 5, 1)
        args = [smith.randn(5, 3, 32, 32), {"first": -1, "second": 5}, None]
    ...
    def test_assert_on_size_after_quant_layer(self):
        ...
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                x = self.conv1(x)
                smith._assert(x.size(1) == 1, 'foobar')
                return x

        m = M().eval()
        example_inputs = (smith.rand(4, 1, 4, 4),)
        m(*example_inputs)
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        mp = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        mp(*example_inputs)
    ...
    def test_fusion_pattern_unquantized(self):
        ...
        example_inputs = (smith.rand(1, 1, 1, 1),)
        mp = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        mp(*example_inputs)
        mc = convert_fx(mp)
    ...
    def test_state_dict(self):
        ...
        m = M1().eval()
        qconfig_dict = {"": default_qconfig}
        m = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 30),))
        m = convert_fx(m)
        state_dict = m.state_dict()
        ...
        m = M2().eval()
        qconfig_dict = {"": default_qconfig}
        m = prepare_fx(m, qconfig_dict, example_inputs=(smith.randn(1, 3, 3, 3),))
        m = convert_fx(m)
        state_dict = m.state_dict()
        self.assertTrue("_packed_weight_0" in state_dict)

        # test load
        ref_weight, ref_bias = smith.ops.quantized.conv2d_unpack(state_dict["_packed_weight_0"])
        data = smith.rand(1, 3, 5, 5)
        ref_res = m(data)
        m = M2().eval()
        ...
        with TemporaryFileName() as fname:
            smith.save(m.state_dict(), fname)
            # weights_only=False as this is loading a ScriptModule
            m.load_state_dict(smith.load(fname, weights_only=False))
    ...
    def test_preserve_qconfig(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            qconfig_dict = {
                "object_type": [
                    (smith.nn.functional.linear, float16_dynamic_qconfig),
                ],
            }
            example_inputs = (smith.rand(5, 5),)
            m = prepare_fx(model, qconfig_dict, example_inputs=example_inputs)
    ...
    def test_not_used(self):
        ...
        m = prepare_fx(m, qconfig_mapping, example_inputs=(smith.randn(1),))
        m = convert_fx(m)
    ...
    def test_qparams_fqn(self):
        ...
        qconfig_dict = {
            "": None,
            "object_type": [
                (smith.nn.functional.linear, default_qconfig),
                (smith.nn.functional.relu, default_qconfig),
            ],
        }
        example_inputs = (smith.rand(5, 5),)
        m = prepare_fx(model, qconfig_dict, example_inputs=example_inputs)
    ...
    def test_no_obs_between_unmatched_node_and_copy_node(self):
        ...
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        example_inputs = (smith.randn(4, 4, 4, 4),)
        mp = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
    ...
    def test_fold_quant_dequant(self):
        ...
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                x = smith.cat((x,), 1)
                tmp = x.size()
                x = smith.nn.functional.linear(x, self.w, self.b)
                y = x * tmp[0]
                return y
        ...
        for n in m.graph.nodes:
            if n.op == "call_method" and n.target == "dequantize":
                dequant = dequant + 1
            if n.op == "call_function" and n.target == smith.quantize_per_tensor:
                quant = quant + 1
    ...
    def test_quant_output_always_observed(self):
        ...
        quantizeable.
        """
        qconfig_dict = {'': smith.ao.quantization.get_default_qat_qconfig('fbgemm')}
        ...
        class M1(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.identity = smith.nn.Identity()
        ...
        self.checkGraphModeFxOp(
            ...
            prepare_expected_node_occurrence={
                ns.call_module(smith.ao.quantization.FusedMovingAvgObsFakeQuantize): 2,
            },
    ...
    def test_shape_followed_by_quantized_op(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv1 = smith.nn.Conv2d(2, 2, 2)
                self.conv2 = smith.nn.Conv2d(2, 2, 2)

            def forward(self, x):
                x = self.conv1(x)
                s = x.shape
                smith._assert(s == x.shape, "")
                x = self.conv2(x)
                return x

        # make sure quantization runs
        m = M().eval()
        example_inputs = (smith.randn(2, 2, 4, 4),)
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
        m = convert_fx(m)
        m(*example_inputs)
        node_occurrence = {
    ...
    def test_trace_quantize_per_tensor(self):
        ...
        m = M().eval()
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=(smith.randn(1, 1, 3, 3),))
        m = convert_fx(m)
        # Make sure this runs without error
        m = smith.fx.Transformer(m).transform()
    ...
    def test_copy_node_has_shared_actpp_instance(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.avgpool2d = smith.nn.AvgPool2d(kernel_size=3)
        ...
        for quant_type in self.static_quant_types:
            ...
            occurrence_map = {
                ...
                QuantType.QAT: {
                    ns.call_module(smith.ao.quantization.FakeQuantize): 2
                }
            }
            if quant_type == QuantType.QAT:
                ...
                actpp_module_class = smith.ao.quantization.FakeQuantize
            else:
                m.eval()
                prepare = prepare_fx
                qconfig = default_qconfig
                actpp_module_class = smith.ao.quantization.MinMaxObserver
            ...
            node_list = [
                ns.call_function(smith.quantize_per_tensor),
                ns.call_module(smith.nn.AvgPool2d),
                ns.call_method("dequantize"),
            ]
            ...
            node_occurrence = {
                ns.call_function(smith.quantize_per_tensor): 2,
    ...
    def test_linear_qint8_activation(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(1, 2, 2, 2)
                self.linear = smith.nn.Linear(8, 5)
        ...
        m = M().eval()
        example_inputs = (smith.rand(2, 1, 5, 5),)
        m = prepare_fx(
            m,
            {"": smith.ao.quantization.QConfig(
                activation=smith.ao.quantization.HistogramObserver.with_args(
                    qscheme=smith.per_tensor_symmetric, dtype=smith.qint8
                ), weight=smith.ao.quantization.default_per_channel_weight_observer)},
            example_inputs=example_inputs)
    ...
    def test_preserve_tuple(self):
        ...
        class LSTM(nn.Module):
            ...
            def forward(self, inputs: smith.Tensor, state: list[smith.Tensor]):
                h = state[0]
                c = state[1]
                return self.lstm(inputs, (h, c))

        m = LSTM().eval()
        example_inputs = (smith.randn(5, 3, 50), smith.randn(2, 3, 50), smith.randn(2, 3, 50))
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
    ...
    def _test_static_lstm_helper(self, model, prepare_node_occurrence, convert_node_occurrence):
        ...
        qconfig_mapping = get_default_qconfig_mapping()
        prepare_custom_config = PrepareCustomConfig() \
            .set_float_to_observed_mapping(smith.nn.LSTM, smith.ao.nn.quantizable.LSTM)
        convert_custom_config = ConvertCustomConfig() \
            .set_observed_to_quantized_mapping(smith.ao.nn.quantizable.LSTM, smith.ao.nn.quantized.LSTM)
        example_inputs = (smith.rand(5, 3, 50), smith.rand(1, 3, 50), smith.randn(1, 3, 50))
    ...
    def test_static_lstm(self):
        ...
        class MyModel(nn.Module):
            ...
            def forward(self, inputs: smith.Tensor, h0: smith.Tensor, c0: smith.Tensor):
        ...
        prepare_node_occurrence = {
            ns.call_module(smith.ao.nn.quantizable.LSTM): 1,
        }
        convert_node_occurrence = {
            ns.call_module(smith.ao.nn.quantized.LSTM): 1,
            ns.call_function(smith.quantize_per_tensor): 3,
            # lstm[0].dequantize()
            # lstm[1][0].dequantize()
            # lstm[1][1].dequantize()
    ...
    def test_static_lstm_with_custom_fixed_qparams(self):
        ...
        inner submodules. This flow requires users to extend `smith.ao.nn.quantizable.LSTM`
        and use the child class in the custom module mapping.
        """
        class MyModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.my_lstm = smith.nn.LSTM(50, 50, 1)
        ...
        qint32_dtype_config = DTypeConfig(input_dtype=smith.qint32, output_dtype=smith.qint32)
        my_backend_config = get_qnnpack_backend_config()
        for config in my_backend_config.configs:
            if config.pattern in [smith.nn.Sigmoid, smith.nn.Tanh, smith.add, smith.mul]:
                config.add_dtype_config(qint32_dtype_config)

        class UserObservedLSTM(smith.ao.nn.quantizable.LSTM):
            """
            ...
            def from_float(cls, float_lstm):
                ...
                # uint16, [-16, 16)
                linear_output_obs_ctr = FixedQParamsObserver.with_args(scale=2 ** -11, zero_point=2 ** 15, dtype=smith.qint32)
                # uint16, [0, 1)
                sigmoid_obs_ctr = FixedQParamsObserver.with_args(scale=2 ** -16, zero_point=0, dtype=smith.qint32)
                # uint16, [-1, 1)
                tanh_obs_ctr = FixedQParamsObserver.with_args(scale=2 ** -15, zero_point=2 ** 15, dtype=smith.qint32)
                # int16, [-16, 16)
                cell_state_obs_ctr = FixedQParamsObserver.with_args(scale=2 ** -11, zero_point=0, dtype=smith.qint32)
                # uint8, [-1, 1)
                hidden_state_obs_ctr = FixedQParamsObserver.with_args(scale=2 ** -7, zero_point=2 ** 7, dtype=smith.quint8)
                example_inputs = (smith.rand(5, 3, 50), (smith.rand(1, 3, 50), smith.randn(1, 3, 50)))
                return smith.ao.quantization.fx.lstm_utils._get_lstm_with_individually_observed_parts(
        ...
        class UserQuantizedLSTM(smith.ao.nn.quantized.LSTM):
            """
            ...
            def from_observed(cls, observed_lstm):
                ...
                return smith.ao.quantization.fx.lstm_utils._get_reference_quantized_lstm_module(
        ...
        example_inputs = (smith.rand(5, 3, 50), smith.rand(1, 3, 50), smith.randn(1, 3, 50))
        prepare_custom_config = PrepareCustomConfig() \
            .set_float_to_observed_mapping(smith.nn.LSTM, UserObservedLSTM)
        convert_custom_config = ConvertCustomConfig() \
            .set_observed_to_quantized_mapping(smith.ao.nn.quantizable.LSTM, UserQuantizedLSTM)
        prepared = prepare_fx(
            m,
            qconfig_mapping,
            example_inputs,
        ...
        node_name_to_expected_quantize_args = {
            "igates": (None, None, smith.quint8),
            "hgates": (None, None, smith.quint8),
            "add": (2 ** -11, 2 ** 15, smith.qint32),  # gates.add
            "input_gate": (2 ** -16, 0, smith.qint32),
            "forget_gate": (2 ** -16, 0, smith.qint32),
            "cell_gate": (2 ** -15, 2 ** 15, smith.qint32),
            "output_gate": (2 ** -16, 0, smith.qint32),
            "mul": (2 ** -11, 0, smith.qint32),  # fgate_cx.mul
            "mul_1": (2 ** -11, 0, smith.qint32),  # igate_cgate.mul
            "add_1": (2 ** -11, 0, smith.qint32),  # fgate_cx_igate_cgate.add
            "mul_2": (2 ** -7, 2 ** 7, smith.quint8),  # ogate_cy.mul
        }
        ...
        for node in cell.graph.nodes:
            ...
            for user in node.users:
                self.assertEqual(user.target, smith.quantize_per_tensor)
    ...
    def test_relu_lowering(self):
        ...
        m = M().eval()
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=(smith.randn(1),))
        m_copy = copy.deepcopy(m)
        m = convert_fx(m)
        m_ref = convert_to_reference_fx(m_copy)
        node_occurrence = {
    ...
    def test_dynamic_with_fusion(self):
        ...
        """
        with override_quantized_engine('fbgemm'):
            class LinearRelu(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.linear = smith.nn.Linear(5, 5)
                    self.relu = smith.nn.ReLU()
            ...
            class M(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.mods1 = smith.nn.Sequential(LinearRelu(), LinearRelu())
                    self.mods2 = Linear()
                    self.relu = F.relu
            ...
            dynamic_quantized_ops = {
                float16_dynamic_qconfig: smith.ops.quantized.linear_relu_dynamic_fp16,
                default_dynamic_qconfig: smith.ops.quantized.linear_relu_dynamic
            }
    ...
    def test_dynamic_linear_input_multiple_use(self):
        ...
        with override_quantized_engine('fbgemm'):
            ...
            for qconfig in [float16_dynamic_qconfig, default_dynamic_qconfig]:
                ...
                example_inputs = (smith.rand(5, 5, 5),)
                m = prepare_fx(model, qconfig_dict, example_inputs=example_inputs)
                m = convert_fx(m)
                m(*example_inputs)
                node_list = [
    ...
    def test_ref_linear_module(self):
        ...
        for M in [M1, M2]:
            m = M().eval()
            example_inputs = (smith.randn(5, 10),)
            m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
            m_copy = copy.deepcopy(m)
            m = convert_fx(m)
            m_ref = convert_to_reference_fx(m_copy)
            ...
            self.assertTrue(smith.equal(result, result_ref))
    ...
    def test_sub_scalar(self):
        ...
        m = M().eval()
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=(smith.rand(3),))
        m = convert_fx(m)
        occurrence = {
            ns.call_function(smith.quantize_per_tensor): 2,
            ns.call_method("dequantize"): 2
    ...
    def test_observer_fqn(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.mods2 = Linear()
                self.mods3 = smith.nn.Linear(5, 5)
        ...
        prepared = prepare_fx(model, {"": default_qconfig}, example_inputs=(smith.randn(1, 5)))
        name_list = []
        for name, mod in prepared.named_modules():
            if isinstance(mod, smith.ao.quantization.observer.MinMaxObserver):
    ...
    def test_conv_lowering(self):
        ...
        for dim in range(1, len(convs) + 1):
            ...
            self.assertTrue(smith.equal(out_ref, out))
        ...
        for check in ["module_name", "object_type"]:
            qconfig_dict = {"": None,
                            "object_type": [
                                ...
                                (smith.add, get_default_qat_qconfig("fbgemm")),
            ...
            if check == "module_name":
                ...
                node_occurrence = {
                    ns.call_function(smith.quantize_per_tensor): 2,
                    ns.call_function(smith.nn.functional.linear): 1,
                    ns.call_function(smith.ops.quantized.linear): 1,
                    ns.call_function(smith.ops.quantized.add): 1,
                    ns.call_method("dequantize"): 2
                }
                order_check = [
                    ns.call_function(smith.nn.functional.linear),
                ...
                node_occurrence = {
                    ns.call_function(smith.quantize_per_tensor): 1,
                    ns.call_function(smith.ops.quantized.linear): 2,
                    ns.call_function(smith.ops.quantized.add): 1,
                    ns.call_function(smith.mul): 1,
                    ns.call_method("dequantize"): 1
                }
                order_check = [
                    ns.call_function(smith.quantize_per_tensor),
    ...
    def test_stack_trace_preserved_linear(self):
        ...
        m = M().eval()
        mp = prepare_fx(m, get_default_qconfig_mapping(), example_inputs=(smith.randn(1, 1),))
    ...
    def test_qat_skip_untraced(self):
        ...
        qconfig_dict = {"": smith.ao.quantization.get_default_qat_qconfig()}
        prepare_custom_config_dict = {
            "non_traceable_module_class": [UnTraceableModuleClass],
            "non_traceable_module_name": ["untraceable_module_name"],
        }
        example_inputs = (smith.randn(2, 2),)
        mod_prep = smith.ao.quantization.quantize_fx.prepare_qat_fx(
        ...
        self.assertTrue(
            isinstance(mod_prep.untraceable_module_class.linear, smith.nn.Linear)
        )
        self.assertTrue(
            isinstance(mod_prep.untraceable_module_name.linear, smith.nn.Linear)
        )
        self.assertTrue(
            type(mod_prep.untraceable_module_class.linear)
            is not smith.ao.nn.qat.modules.linear.Linear,
    ...
    def test_qconfig_dict_setup(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.Conv1d = smith.nn.Conv1d(1, 1, 1)
                self.Conv2d = smith.nn.Conv2d(1, 1, 1)
                self.Conv3d = smith.nn.Conv3d(1, 1, 1)
                self.ConvTranspose1d = smith.nn.ConvTranspose1d(1, 1, 1)
                self.ConvTranspose2d = smith.nn.ConvTranspose2d(1, 1, 1)
                self.ConvTranspose3d = smith.nn.ConvTranspose3d(1, 1, 1)
                self.Linear = smith.nn.Linear(1, 1, 1)
            ...
            def forward(self, x):
                ...
                x = self.Linear(x)
                x = smith.nn.functional.conv1d(x, smith.rand(2, 2))
                x = smith.nn.functional.conv2d(x, smith.rand(2, 2))
                x = smith.nn.functional.conv3d(x, smith.rand(2, 2))
                x = smith.nn.functional.linear(x, smith.rand(2, 2))
                return x
        ...
        for func in [get_default_qconfig_mapping, get_default_qat_qconfig_mapping]:
            for backend in backends:
                ...
                for mod in m.modules():
                    if _is_activation_post_process(mod) and mod.dtype == smith.quint8:
                        if backend == "fbgemm":
                            lower_bnd = 0
                            upper_bnd = 127
                        else:
    ...
    def test_prepare_mode(self):
        ...
        def _test(prepare_fn, qconfig_dict):
            ...
            m1 = copy.deepcopy(m)
            m1.train()
            example_inputs = (smith.randn(1, 5),)
    ...
    def _validate_qconfig_against_backend_config_constraints(
        ...
        """
        qconfig_mapping = QConfigMapping().set_object_type(smith.nn.Linear, qconfig)
        example_inputs = (smith.rand((1, 30), dtype=smith.float),)
        ...
        if satisfies_constraints:
            expected_node_occurrence = {
                ns.call_module(smith.ao.nn.quantized.Linear) : 1,
                ns.call_module(smith.nn.Linear) : 0,
            }
        else:
            expected_node_occurrence = {
                ns.call_module(smith.ao.nn.quantized.Linear) : 0,
                ns.call_module(smith.nn.Linear) : 1,
            }
        try:
    ...
    def test_backend_config_quantization_range(self):
        ...
        dtype_config = DTypeConfig(
            ...
            bias_dtype=smith.float,
        )
        backend_config = BackendConfig() \
            .set_backend_pattern_config(BackendPatternConfig(smith.nn.Linear)
                .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)  # noqa: E128
                .add_dtype_config(dtype_config)
                .set_root_module(smith.nn.Linear)
        ...
        qconfig1 = QConfig(
            activation=MinMaxObserver.with_args(quant_min=0, quant_max=15, dtype=smith.quint8),
            weight=MinMaxObserver.with_args(quant_min=-32, quant_max=31, dtype=smith.qint8, qscheme=smith.per_tensor_symmetric))
        ...
        qconfig2 = QConfig(
            activation=MinMaxObserver.with_args(quant_min=0, quant_max=63, dtype=smith.quint8),
            weight=MinMaxObserver.with_args(dtype=smith.qint8, qscheme=smith.per_tensor_symmetric))
        validate_qconfig(qconfig2, satisfies_constraints=False)

        # Case 3: QConfig weight range falls outside backend range, should fail
        qconfig3 = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.quint8),
            weight=MinMaxObserver.with_args(quant_min=-128, quant_max=127, dtype=smith.qint8, qscheme=smith.per_tensor_symmetric))
    ...
    def test_backend_config_scale_min(self):
        ...
        dtype_config = DTypeConfig(
            input_dtype=DTypeWithConstraints(dtype=smith.quint8, scale_min_lower_bound=2 ** -12),
            output_dtype=DTypeWithConstraints(dtype=smith.quint8, scale_min_lower_bound=2 ** -12),
            weight_dtype=DTypeWithConstraints(dtype=smith.qint8, scale_min_lower_bound=2 ** -12),
            bias_dtype=smith.float,
        )
        ...
        qconfig1 = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.quint8, eps=2 ** -12),
            weight=MinMaxObserver.with_args(dtype=smith.qint8, qscheme=smith.per_tensor_symmetric, eps=2 ** -12))
        validate_qconfig(qconfig1, satisfies_constraints=True)

        # Case 2: QConfig min scale value > backend min scale value, OK
        qconfig2 = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.quint8, eps=2 ** -10),
            weight=MinMaxObserver.with_args(dtype=smith.qint8, qscheme=smith.per_tensor_symmetric, eps=2 ** -10))
        ...
        qconfig3 = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.quint8, eps=2 ** -14),
            weight=MinMaxObserver.with_args(dtype=smith.qint8, qscheme=smith.per_tensor_symmetric))
        validate_qconfig(qconfig3, satisfies_constraints=False)
        ...
        qconfig4 = QConfig(
            activation=MinMaxObserver.with_args(dtype=smith.quint8),
            weight=MinMaxObserver.with_args(dtype=smith.qint8, qscheme=smith.per_tensor_symmetric, eps=2 ** -14))
        validate_qconfig(qconfig4, satisfies_constraints=False)
    ...
    def test_qnnpack_backend_config(self):
        ...
        all_qconfigs: list[tuple[QConfig, str]] = [
            ...
            # TODO: Test these QConfigs once they are fixed, see https://github.com/blacksmith/blacksmith/issues/85862
    ...
    def test_symmetric_qnnpack_qconfig_mapping(self):
        """
        Test whether `smith.ao.quantization.qconfig_mapping._get_symmetric_qnnpack_qconfig_mapping`
    ...
    def test_symmetric_qnnpack_qat_qconfig_mapping(self):
        """
        Test whether `smith.ao.quantization.qconfig_mapping._get_symmetric_qnnpack_qat_qconfig_mapping`
        works with the QNNPACK BackendConfig.
        """
        if "qnnpack" not in supported_qengines:
            return
    ...
    def test_get_execusmith_backend_config(self):
        from smith.ao.quantization.backend_config import get_execusmith_backend_config
        # make sure this runs
        execusmith_backend_config = get_execusmith_backend_config()
    ...
    def test_backend_config_check_for_weight_and_bias(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.tensor((5, 5))
                self.bias = smith.tensor((5,))

            def forward(self, x):
                return smith.addmm(self.bias, x, self.weight)
    ...
    def test__convert_to_reference_decomposed_fx(self):
        ...
        expected_occurrence = {
            ns.call_function(smith.ops.quantized_decomposed.quantize_per_tensor.default): 2,
            ns.call_function(smith.ops.quantized_decomposed.dequantize_per_tensor.default): 2,
        }
        self.checkGraphModuleNodes(
            m,
            expected_node_occurrence=expected_occurrence)
    ...
    def test__convert_to_reference_decomposed_fx_dynamic_quant(self):
        ...
        with override_quantized_engine("qnnpack"):
            m = M().eval()
            qconfig_mapping = get_default_qconfig_mapping("fbgemm") \
                .set_object_type(smith.nn.Linear, default_dynamic_qconfig)
            example_inputs = (smith.randn(1, 5),)
            m = prepare_fx(m, qconfig_mapping, example_inputs)
            m(*example_inputs)
            m_ref = copy.deepcopy(m)
            ...
            m = _convert_to_reference_decomposed_fx(m)
            expected_occurrence = {
                ns.call_function(smith.ops.quantized_decomposed.choose_qparams.tensor): 1,
                ns.call_function(smith.ops.quantized_decomposed.quantize_per_tensor.tensor): 1,
                ns.call_function(smith.ops.quantized_decomposed.dequantize_per_tensor.tensor): 1,
            }
            self.checkGraphModuleNodes(
                m,
                expected_node_occurrence=expected_occurrence)
    ...
    def test__convert_to_reference_decomposed_fx_per_channel_quant(self):
        ...
        example_inputs = (smith.randn(1, 5), smith.randn(10, 5), smith.randn(10,))
        m = prepare_fx(m, qconfig_mapping, example_inputs)
        m(*example_inputs)
        m_ref = copy.deepcopy(m)
        m_ref = convert_to_reference_fx(m_ref)
        ...
        expected_occurrence = {
            ...
            ns.call_function(smith.ops.quantized_decomposed.dequantize_per_tensor.default): 2,
            # for weight
            ns.call_function(smith.ops.quantized_decomposed.quantize_per_channel.default): 1,
            ns.call_function(smith.ops.quantized_decomposed.dequantize_per_channel.default): 1,
        }
        self.checkGraphModuleNodes(
            m,
            expected_node_occurrence=expected_occurrence)
    ...
    def test_change_backend_config_for_fixed_qparam_ops(self):
        ...
        model = prepare_fx(
            ...
            example_inputs=(smith.randn(1, 2, 3, 4),),
    ...
    def test_channel_shuffle_lowering(self):
        ...
        class M1(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.op = smith.nn.ChannelShuffle(2)
        ...
        class M2(smith.nn.Module):
            def forward(self, x):
                return smith.channel_shuffle(x + x, 2) + x

        class M3(smith.nn.Module):
            def forward(self, x):
                return smith.nn.functional.channel_shuffle(x + x, 2) + x

        x = smith.randn(4, 4, 4, 4)
        # smith.channel_shuffle is equivalent to smith.nn.functional.channel_shuffle
        model_node_pairs = [
            (M1().eval(), ns.call_module(smith.nn.ChannelShuffle)),
            (M2().eval(), ns.call_function(smith.channel_shuffle)),
            (M3().eval(), ns.call_function(smith.channel_shuffle))
        ]
        for m, node in model_node_pairs:
            ...
            node_occurrence_ref = {
                node: 1,
                ns.call_function(smith.quantize_per_tensor): 4,
    ...
    def test_match_pattern_with_multiple_args(self):
        ...
        def _get_pattern_configs():
            ...
            backend_pattern_configs.append(
                BackendPatternConfig()
                ._set_pattern_complex_format((smith.reshape, smith.transpose, MatchAllNode))  # noqa: E131
                .set_observation_type(observation_type)
                .set_dtype_configs(dtype_configs)
    ...
    def test_linear_tanh_lowering(self):
        ...
        qconfig_mapping.set_object_type(smith.nn.Linear, qconfig)
        qconfig_mapping.set_object_type(smith.nn.Tanh, qconfig)
        with override_quantized_engine('onednn'):
            m = LinearTanhModel()
            self._test_linear_activation_fusion_lowering_helper(
                m,
    ...
    def test_linear_size_view(self):
        class M(smith.nn.Module):
            def __init__(self, use_relu=False):
                super().__init__()
                self.linear = smith.nn.Linear(16, 32)
                self.relu = smith.nn.ReLU()
    ...
    def test_mixed_dtypes(self):
        ...
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear1 = smith.nn.Linear(5, 5)
                self.linear2 = smith.nn.Linear(5, 5)
                self.sigmoid = smith.nn.Sigmoid()
                self.tanh = smith.nn.Tanh()
                self.float_functional = smith.ao.nn.quantized.FloatFunctional()
        ...
        def make_qconfig(scale, zp, dtype):
            return QConfig(
                ...
                weight=smith.ao.quantization.default_weight_observer)
        ...
        qconfig_mapping = QConfigMapping() \
            ...
            .set_module_name("linear1", make_qconfig(1234, 11, smith.qint32)) \
            .set_module_name("linear2", make_qconfig(2345, 22, smith.quint8)) \
            .set_object_type(smith.nn.Sigmoid, make_qconfig(3456, 33, smith.qint32)) \
            .set_object_type(smith.nn.Tanh, make_qconfig(4567, 44, smith.quint8))
        ...
        weighted_op_qint32_dtype_config = DTypeConfig(
            input_dtype=smith.qint32,
            output_dtype=smith.qint32,
            weight_dtype=smith.qint8,
            bias_dtype=smith.float,
        )
        ...
        for config in backend_config.configs:
            if config.pattern == smith.nn.Linear:
                config.add_dtype_config(weighted_op_qint32_dtype_config)
            elif config.pattern in [smith.nn.Sigmoid, smith.nn.Tanh]:
        ...
        target_to_expected_dtypes = {
            "linear1": smith.qint32,
            "linear2": smith.quint8,
            "sigmoid": smith.qint32,
            "tanh": smith.quint8,
            smith.add: smith.quint8,
        }
        ...
        for node in converted.graph.nodes:
            ...
            # Match [dq - smith.add(linear2_dq, tanh_dq) - q]
            if node.target == "linear2":
                linear2_node = node
            elif node.target == "tanh":
                tanh_node = node
            elif node.target == smith.add:
    ...
    def test_lowering_functional_conv_with_kwargs(self):
        ...
        dim_to_qop = {
            1: smith.ops.quantized.conv1d,
            2: smith.ops.quantized.conv2d,
            3: smith.ops.quantized.conv3d,
        }
        ...
        class Mod(nn.Module):
            def __init__(self, in_channels, out_channels, kernel_size, dimension):
                ...
                self.weight = nn.Parameter(smith.randn(out_channels, in_channels, *kernel_sizes))
        ...
        for dimension in [1, 2, 3]:
            ...
            example_inputs = smith.randn(input_shape)
    ...
    def test_lowering_functional_conv_transpose_with_kwargs(self):
        ...
        dim_to_qop = {
            1: smith.ops.quantized.conv_transpose1d,
            2: smith.ops.quantized.conv_transpose2d,
            3: smith.ops.quantized.conv_transpose3d,
        }
        ...
        class Mod(nn.Module):
            def __init__(self, in_channels, out_channels, kernel_size, dimension):
                ...
                self.op = dim_to_op[dimension]
                kernel_sizes = [kernel_size] * self.dim
                self.weight = nn.Parameter(smith.randn(in_channels, out_channels, *kernel_sizes))
    ...
    def test_lowering_functional_linear_with_kwargs(self):
        class Mod(nn.Module):
            def __init__(self, in_channels, out_channels):
                super().__init__()
                self.weight = nn.Parameter(smith.randn(out_channels, in_channels))
        ...
        model.eval()
        qconfig_mapping = get_default_qconfig_mapping()
        example_inputs = smith.randn(1, 8)
        prepared_model = prepare_fx(model, qconfig_mapping, example_inputs)
        prepared_model(example_inputs)
        quantized_model = convert_fx(prepared_model)
        # This should pass
    ...
    def test_keep_original_weights(self):
        class SubModule(nn.Module):
            ...
            def __init__(self, input_dim, output_dim):
                super().__init__()
                self.w = nn.Parameter(smith.randn(input_dim, output_dim))
                self.b = nn.Parameter(smith.randn(input_dim))
        ...
        class MainModule(nn.Module):
            ...
            def __init__(self, input_dim, hidden_dim, output_dim):
                ...
                self._w = nn.Parameter(smith.randn(output_dim, hidden_dim))
        ...
        model.eval()
        example_inputs = smith.randn(1, input_dim)
...
class TestQuantizeFxOps(QuantizationTestCase):
    def setUp(self):
        super().setUp()
        self.custom_qconfig = smith.ao.quantization.QConfig(
            activation=smith.ao.quantization.observer.HistogramObserver.with_args(
                qscheme=smith.per_tensor_symmetric, dtype=smith.qint8
            ),
            weight=smith.ao.quantization.default_per_channel_weight_observer
        )
        self.common_quant_patterns = {
            smith.nn.ConvTranspose1d: DefaultNodeQuantizeHandler,
            smith.nn.ConvTranspose2d: DefaultNodeQuantizeHandler,
            smith.nn.ELU: DefaultNodeQuantizeHandler,
            smith.nn.LeakyReLU: DefaultNodeQuantizeHandler,
            smith.nn.Hardswish: DefaultNodeQuantizeHandler,
            smith.nn.InstanceNorm1d: DefaultNodeQuantizeHandler,
            smith.nn.InstanceNorm2d: DefaultNodeQuantizeHandler,
            smith.nn.InstanceNorm3d: DefaultNodeQuantizeHandler,
            smith.nn.LayerNorm: DefaultNodeQuantizeHandler,
            smith.nn.SiLU: DefaultNodeQuantizeHandler,
            smith.nn.Mish: DefaultNodeQuantizeHandler,
            smith.nn.GELU: DefaultNodeQuantizeHandler,
            smith.nn.Softmax: DefaultNodeQuantizeHandler,
            smith.nn.functional.elu: DefaultNodeQuantizeHandler,
            smith.nn.functional.hardswish: DefaultNodeQuantizeHandler,
            smith.nn.functional.instance_norm: DefaultNodeQuantizeHandler,
            smith.nn.functional.layer_norm: DefaultNodeQuantizeHandler,
            smith.nn.functional.leaky_relu: DefaultNodeQuantizeHandler,
            smith.nn.functional.silu: DefaultNodeQuantizeHandler,
            smith.nn.functional.mish: DefaultNodeQuantizeHandler,
            smith.nn.functional.gelu: DefaultNodeQuantizeHandler,
            smith.nn.functional.softmax: DefaultNodeQuantizeHandler,
            smith.sum: DefaultNodeQuantizeHandler
        }
    ...
    def test_linear_module(self):
        with override_quantized_engine('fbgemm'):
            ...
            class LinearBnModel(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.linear = smith.nn.Linear(4, 4).float()
                    self.bn = smith.nn.BatchNorm1d(4)
            ...
            # Test linear-bn
            data = (smith.rand((4, 4), dtype=smith.float),)
            for quant_type in self.static_quant_types:
                model = LinearBnModel()
    ...
    def test_functional_linear(self):
        with override_quantized_engine('fbgemm'):
                ...
                def __init__(self, use_bias, has_relu, f_relu):
                    ...
                    if has_relu:
                        if f_relu:
                            ...
                            self.relu_or_id = smith.nn.ReLU()
                    else:
                        self.relu_or_id = smith.nn.Identity()
            ...
            quant_type_to_qlinear_fun = {
                QuantType.DYNAMIC: ns.call_function(smith.ops.quantized.linear_dynamic),
                QuantType.STATIC: ns.call_function(smith.ops.quantized.linear),
                QuantType.QAT: ns.call_function(smith.ops.quantized.linear),
            }
            quant_type_to_qlinear_relu_fun = {
                # we don't have linear_relu_dynamic
                QuantType.DYNAMIC: ns.call_function(smith.ops.quantized.linear_relu_dynamic),
                QuantType.STATIC: ns.call_function(smith.ops.quantized.linear_relu),
                QuantType.QAT: ns.call_function(smith.ops.quantized.linear_relu),
            }
            ...
            for quant_type, use_bias, has_relu, f_relu in options:
                ...
                quant_type_to_prepare_expected_node_occurrence = {
                    QuantType.DYNAMIC: {
                        ns.call_module(smith.ao.quantization.PlaceholderObserver): 1,
                        ns.call_module(smith.ao.quantization.MinMaxObserver): 1,
                    },
                    # There should be 3 observers: after input, weight and activation.
                    # one more observer for smith.nn.Identity when there is no relu
                    QuantType.STATIC: {
                        ns.call_module(smith.ao.quantization.HistogramObserver): 2 if has_relu else 3,
                        ns.call_module(smith.ao.quantization.PerChannelMinMaxObserver): 1,
                    },
                    # There should be 3 observers: after input, weight and activation.
                    QuantType.QAT: {
                        ns.call_module(smith.ao.quantization.FusedMovingAvgObsFakeQuantize): 3 if has_relu else 4,
                    },
                }
                ...
                convert_node_occurrence = {
                    ns.call_function(smith.quantize_per_tensor): 1 if quant_type != QuantType.DYNAMIC else 0,
                    qlinear_fun: 1,
    ...
    def test_linear_dynamic_fp16(self):
        with override_quantized_engine('fbgemm'):
            ...
            for use_bias, has_relu, f_relu, is_reference in options:
                model = FuncLinear(use_bias, has_relu, f_relu)
                if is_reference:
                    qlinear_fun = ns.call_function(smith.nn.functional.linear)
                else:
                    if has_relu:
                        qlinear_fun = ns.call_function(smith.ops.quantized.linear_relu_dynamic_fp16)
                    else:
                        qlinear_fun = ns.call_function(smith.ops.quantized.linear_dynamic_fp16)
                prepare_node_occurrence = {
                    # activation and weight
                    ns.call_module(smith.ao.quantization.PlaceholderObserver): 2
                }
                convert_node_occurrence = {
                    qlinear_fun: 1,
                    # weight
    ...
    def test_linear_static_fp16(self):
        ...
        for use_bias, has_relu, f_relu, is_reference in options:
            model = FuncLinear(use_bias, has_relu, f_relu)
            linear_fun = ns.call_function(smith.nn.functional.linear)
            # when has_relu is False, we are using an nn.Identity and
            # we will insert observer/fake_quant for the output of nn.Identity since
            # it is a copy node, that's why we have extra observer/fake_quant
            # when has_relu is False
            prepare_node_occurrence = {
                ...
                ns.call_module(smith.ao.quantization.PlaceholderObserver): 3 + int(use_bias) + int(not has_relu),
            }
    ...
    def test_conv_module(self):
        ...
        class ConvWrapper(smith.nn.Module):
            def __init__(self, dim):
                super().__init__()
    ...
    def test_functional_conv(self):
        with override_quantized_engine('fbgemm'):
            ...
            """
            convs = {
                1: smith.nn.functional.conv1d,
                2: smith.nn.functional.conv2d,
                3: smith.nn.functional.conv3d,
            }

            class FuncConv(smith.nn.Module):
                def __init__(self, dim, use_bias, has_relu, f_relu):
                    super().__init__()
                    self.dim = dim
                    self.w = smith.randn(tuple([3] * (dim + 2)))
                    self.b = smith.randn(3) if use_bias else None
            ...
            quant_type_to_qconv_fun = {
                QuantType.STATIC: {
                    1: ns.call_function(smith.ops.quantized.conv1d),
                    2: ns.call_function(smith.ops.quantized.conv2d),
                    3: ns.call_function(smith.ops.quantized.conv3d)
                },
                QuantType.QAT: {
                    1: ns.call_function(smith.ops.quantized.conv1d),
                    2: ns.call_function(smith.ops.quantized.conv2d),
            ...
            quant_type_to_qconv_relu_fun = {
                QuantType.STATIC: {
                    1: ns.call_function(smith.ops.quantized.conv1d_relu),
                    2: ns.call_function(smith.ops.quantized.conv2d_relu),
                    3: ns.call_function(smith.ops.quantized.conv3d_relu)
                },
                QuantType.QAT: {
                    1: ns.call_function(smith.ops.quantized.conv1d_relu),
                    2: ns.call_function(smith.ops.quantized.conv2d_relu),
            ...
            for dim, quant_type, use_bias, has_relu, f_relu in options:
                ...
                data_dims = [2, 3] + [4] * dim
                data = (smith.randn(tuple(data_dims), dtype=smith.float),)
    ...
    def _test_binary_op_int8_impl(self, binary_op, ibinary_op, quantized_op):
        data = (smith.randn(1, 1, 1, 1, dtype=smith.float),
                smith.randn(1, 1, 1, 1, dtype=smith.float))
    ...
    def _test_binary_op_relu_int8_impl(self, binary_op, ibinary_op, quantized_op):
        data = (smith.rand((1, 1, 1, 1), dtype=smith.float),
                smith.rand((1, 1, 1, 1), dtype=smith.float))
        quant_type = QuantType.STATIC
        quantized_node = ns.call_function(quantized_op)
        options = itertools.product(
            [True, False], [nn.ReLU, F.relu, smith.relu], [True, False])
    ...
    def _test_binary_op_relu_float16_impl(self, binary_op, ibinary_op):
        ...
        custom_qconfig_dict = {
            "": float16_static_qconfig,
            "object_type": [(smith.nn.Conv2d, None)]
        }
        backend_config = get_test_only_legacy_native_backend_config()
    ...
    def test_add(self):
        self._test_binary_op_int8_impl(
            operator.add, operator.iadd, smith.ops.quantized.add)
    ...
    def test_sub(self):
        ...
        self._test_binary_op_float16_impl(smith.sub, None)

    @unittest.skip("This is no longer needed right now, can enable later with new api")
    def test_div(self):
        self._test_binary_op_float16_impl(operator.truediv, operator.itruediv)
        self._test_binary_op_float16_impl(smith.div, None)

    @skipIfNoFBGEMM
    def test_mul(self):
        self._test_binary_op_int8_impl(
            operator.mul, operator.imul, smith.ops.quantized.mul)
        self._test_binary_op_float16_impl(operator.mul, operator.imul)

    @unittest.skip("This is no longer needed right now, can enable later with new api")
    def test_sum(self):
        class Sum(smith.nn.Module):
            def forward(self, x):
                x = smith.sum(x, [1], keepdim=True)
                x = smith.sum(x, [1])
                return x

        data = smith.randn(1, 2, 3, 4, dtype=smith.float)
        quant_type = QuantType.STATIC
        # testing for fp16 static quant
        # we are producing fp16 patterns
        custom_qconfig_dict = {
            "object_type": [(smith.sum, float16_static_qconfig)]
        }
        node_occurrence = {
    ...
    @unittest.skip("This is no longer needed right now, can enable later with new api")
    def test_bmm(self):
        class BMMMethod(smith.nn.Module):
            def forward(self, x, y):
                return x.bmm(y)

        data = (smith.randn(1, 1, 1, dtype=smith.float),
                smith.randn(1, 1, 1, dtype=smith.float))
        quant_type = QuantType.STATIC
        # testing for fp16 static quant
        # we are producing fp16 patterns
        custom_qconfig_dict = {
            "object_type": [(smith.bmm, float16_static_qconfig),
        ...
        self.checkGraphModeFxOp(
            BinaryOpNonQuantizedInput(smith.bmm, None, False, False), data, quant_type,
            expected_node_occurrence=node_occurrence,
            custom_qconfig_dict=custom_qconfig_dict)

        # TODO: support call_method("bmm")
        # we can transform call_method("bmm") to call_function(smith.bmm)
        # self.checkGraphModeFxOp(
    ...
    def test_add_relu(self):
        self._test_binary_op_relu_int8_impl(
            operator.add, operator.iadd, smith.ops.quantized.add_relu)
    ...
    def test_add_relu_multiple_uses_of_relu(self):
        ...
        m = M().eval()
        example_inputs = (smith.randn(3), smith.randn(3))
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
        m = convert_fx(m)
        node_occurrence = {
            ns.call_function(smith.quantize_per_tensor): 2,
            ns.call_function(smith.ops.quantized.add_relu): 2,
            ns.call_method("dequantize"): 1,
        }
        self.checkGraphModuleNodes(m, expected_node_occurrence=node_occurrence)
    ...
    def test_mul_relu(self):
        self._test_binary_op_relu_int8_impl(
            operator.mul, operator.imul, smith.ops.quantized.mul_relu)
        self._test_binary_op_relu_float16_impl(
    ...
    def test_quantized_add_qat(self):
        ...
        example_inputs = (smith.randn(1, 1, 1, 1),)
        expected_node_occurrence = {
            ns.call_module(smith.ao.quantization.FusedMovingAvgObsFakeQuantize): 5,
        }
    ...
    def test_int8_input_no_unnecessary_fq(self):
        ...
        class M(nn.Module):
            def __init__(self, scalar):
                super().__init__()
                self.scalar = scalar
                self.add_func = smith.ao.nn.quantized.FloatFunctional()

            def forward(self, x):
                return self.add_func.add_scalar(x, self.scalar)

        m = M(0.5)
        mp = smith.ao.quantization.quantize_fx.prepare_qat_fx(
            m, {'': smith.ao.quantization.get_default_qat_qconfig('fbgemm')},
            example_inputs=(smith.randn(1),),
            prepare_custom_config={"input_quantized_idxs": [0]})
        expected_node_occurrence = {
            ns.call_module(smith.ao.quantization.FusedMovingAvgObsFakeQuantize): 1,
        }
        self.checkGraphModuleNodes(
            mp, expected_node_occurrence=expected_node_occurrence)
    ...
    def test_cat(self):
        ...
        m = M().eval()
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
        # two inputs and one output of smith.cat are using same observer, so we have
    ...
    def test_hardswish(self):
        self._test_activation_impl(nn.Hardswish, F.hardswish, nnq.Hardswish, smith.ops.quantized.hardswish)

    def test_elu(self):
        self._test_activation_impl(nn.ELU, F.elu, nnq.ELU, smith.ops.quantized.elu)

    def test_leaky_relu(self):
        self._test_activation_impl(nn.LeakyReLU, F.leaky_relu, nnq.LeakyReLU, smith.ops.quantized.leaky_relu)

    def test_prelu(self):
        class M(smith.nn.Module):
            def __init__(self, num_param: int):
                super().__init__()
                self.op = smith.nn.PReLU(num_parameters=num_param)

            def forward(self, input):
                return self.op(input)

        X = [[smith.randn(4, 4, 4, 4, dtype=smith.float)]]
        options = itertools.product([1, 4], self.static_quant_types, [True, False])
        quantized_nodes = {
            # is_reference
            True: ns.call_module(smith.nn.PReLU),
            False: ns.call_module(smith.ao.nn.quantized.PReLU),
        }
    ...
    def _test_norm_impl(
        ...
        ''' Test for normalization op, float_op can be smith op or functional op,
    ...
    def test_layer_norm(self):
        ...
        self._test_norm_impl(
            nn.LayerNorm, F.layer_norm, [[2, 5, 5]], data, nnq.LayerNorm, smith.ops.quantized.layer_norm)

    def test_instance_norm(self):
        data_1d = (smith.rand((1, 4, 5), dtype=smith.float),)
        data_2d = (smith.rand((1, 4, 5, 1), dtype=smith.float),)
        data_3d = (smith.rand((1, 4, 5, 1, 1), dtype=smith.float),)
        ...
        for dim in [1, 2, 3]:
            ...
            self._test_norm_impl(
                ...
                quantized_module, smith.ops.quantized.instance_norm,
    ...
    def test_norm_weight_bias(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mods1 = Linear()
                self.scale = smith.randn(5, 5)
                self.bias = smith.randn(5, 5)
        ...
        expected_occurrence = {
            ns.call_function(smith.quantize_per_tensor): 1,
            ns.call_function(smith.ops.quantized.linear): 1,
            ns.call_function(smith.ops.quantized.layer_norm): 1,
            ns.call_method("dequantize"): 1,
        }
    ...
    def test_gelu_normal(self):
        module = smith.nn.GELU
        functional = smith.nn.functional.gelu
        qconfig = smith.ao.quantization.get_default_qconfig("fbgemm")
        is_reference = False
        node_list = [
    ...
    def test_softmax_normal(self):
        module = smith.nn.Softmax
        functional = smith.nn.functional.softmax
        qconfig = smith.ao.quantization.get_default_qconfig("fbgemm")
        is_reference = False
        node_list = [
            ns.call_module(smith.ao.nn.quantized.Softmax),
            ns.call_function(functional),
        ]
        self._test_default_node_quant_handler_ops(
            module, functional, qconfig, is_reference, node_list)
    ...
    def test_gelu_reference(self):
        ...
        additional_patterns = {smith.nn.GELU: DefaultNodeQuantizeHandler,
                               smith.nn.functional.gelu: DefaultNodeQuantizeHandler}
        self._test_default_node_quant_handler_ops(
    ...
    def test_softmax_reference(self):
        ...
        additional_patterns = {smith.nn.Softmax: DefaultNodeQuantizeHandler,
                               smith.nn.functional.softmax: DefaultNodeQuantizeHandler}
        self._test_default_node_quant_handler_ops(
            module, functional, qconfig, is_reference, node_list, additional_patterns)
    ...
    def test_silu_reference(self):
        module = smith.nn.SiLU
        functional = smith.nn.functional.silu
    ...
    def test_mish_reference(self):
        module = smith.nn.Mish
        functional = smith.nn.functional.mish
        qconfig = float16_static_qconfig
        is_reference = True
        node_list = [
            ns.call_method("to"),
    ...
    def test_bmm_int_reference(self):
        ...
        data_x = smith.randn((2, 2, 2,))
        data_y = smith.randn((2, 2, 2,))
        example_inputs = (data_x, data_y)
        qconfig_dict = {"": smith.ao.quantization.get_default_qconfig("fbgemm")}
        is_reference = True
        node_list = [
            ns.call_function(smith.bmm),
        ]
    ...
    def test_fixed_qparams_ops_fp16(self):
        ...
        data = (smith.randn((2, 2, 2, 2), dtype=smith.float),)
        quant_type = QuantType.STATIC
    ...
    def test_fixed_qparams_ops_qint8(self):
        ...
        data = (smith.randn((2, 2, 2, 2), dtype=smith.float),)
        quant_type = QuantType.STATIC
        qconfig = smith.ao.quantization.QConfig(
            activation=HistogramObserver.with_args(qscheme=smith.per_tensor_symmetric, dtype=smith.quint8),
            weight=default_weight_observer)
        qconfig_mapping = get_default_qconfig_mapping().set_global(qconfig)
        node_occurrence = {
            ns.call_function(smith.quantize_per_tensor): 7,
    ...
    def test_fixed_qparams_ops_wrong_qconfig(self):
        ...
        self.checkGraphModeFxOp(
            ...
            expected_node_occurrence=node_occurrence, is_reference=True)
        self.assertTrue(isinstance(m.sigmoid, smith.nn.Sigmoid))
        self.assertTrue(isinstance(m.tanh, smith.nn.Tanh))
    ...
    def test_general_shape_ops(self):
        ...
        example_inputs = (smith.rand(1, 3, 10, 10),)
    ...
    def test_copy_node_fp32_input(self):
        ...
        m = M().eval()
        m = prepare_fx(m, {"": default_reuse_input_qconfig}, example_inputs=(smith.randn(1),))
        m = convert_fx(m)
        # make sure it runs
        m(smith.rand(1))
    ...
    def test_getitem(self):
        ...
        self.checkGraphModuleNodes(m2, expected_node_occurrence={
            ns.call_module(smith.ao.quantization.FixedQParamsObserver): 2
        })
        m2 = convert_fx(m2)
        ...
        m3 = M3().eval()
        example_inputs = (smith.rand(1, 2, 3, 4),)
        qconfig_mapping = get_default_qconfig_mapping()
    ...
    def test_fixed_qparams_ops(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.tanh = smith.nn.Tanh()
                self.softmax = smith.nn.Softmax(dim=0)
        ...
        for eval_mode in [True, False]:
            ...
            # nothing to fuse so skipping the fuse step
            m_copy = copy.deepcopy(m)
            example_inputs = (smith.rand(3, 3, 3, 3),)
            ...
            reference_count_check = {
                ns.call_function(smith.quantize_per_tensor) : 12,
    ...
    def test_float_functional(self):
        class SmithAdd(nn.Module):
            """Wrapper around smith.add so that all ops can be found at build"""
            def __init__(self) -> None:
                super().__init__()
        ...
        for quant_type in self.static_quant_types:
            m = M()
            ref_m = smith.ao.quantization.QuantWrapper(M())
            is_qat = quant_type == QuantType.QAT
            if is_qat:
                m.train()
                ref_m.train()
                qconfig = default_qat_qconfig
                expected_act_post_process = smith.ao.quantization.FakeQuantize
            else:
                m.eval()
                ref_m.eval()
                qconfig = default_qconfig
                expected_act_post_process = smith.ao.quantization.MinMaxObserver
            ...
            node_occurrence = {
                ...
                ns.call_module(smith.ao.nn.quantized.FloatFunctional): 0
            }
            self.checkGraphModuleNodes(m, expected_node_occurrence=node_occurrence)
            m(*example_inputs)
            node_list = [
                ...
                ns.call_function(smith.ops.quantized.add),
                ns.call_function(smith.ops.quantized.add),
                ns.call_function(smith.ops.quantized.mul),
                ns.call_function(smith.ops.quantized.mul),
                ns.call_function(smith.ops.quantized.add_relu),
                ns.call_function(smith.cat),
                ns.call_method('dequantize')
            ]
            m = convert_fx(m)
    ...
    def test_embedding_bag(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.emb = smith.nn.EmbeddingBag(num_embeddings=10, embedding_dim=12, include_last_offset=True)
        ...
        for dtype in [smith.quint8, smith.quint4x2]:
            model = M().eval()
            float_qparams_observer = PerChannelMinMaxObserver.with_args(dtype=dtype,
                                                                        qscheme=smith.per_channel_affine_float_qparams,
                                                                        ch_axis=0)
    ...
    def _test_rnn_impl(self, qconfigs, M, module_type_strs, module_types, sample_input):
        ...
        for qconfig, module_type_str in options:
            ...
            if smith.backends.quantized.engine == 'qnnpack' and \
    ...
    def test_rnn_cell(self):
        if smith.backends.quantized.engine not in ('fbgemm', 'qnnpack'):
            return
        ...
        module_types = [smith.nn.LSTMCell, smith.nn.GRUCell, smith.nn.RNNCell]
        sample_input = smith.tensor([[100, -155],
                                     [-155, 100],
                                     [100, -155]], dtype=smith.float)
    ...
    def test_rnn(self):
        ...
        module_types = [smith.nn.LSTM, smith.nn.GRU]
        niter = 10
        sample_input = smith.tensor([[100, -155],
                                     [-155, 100],
                                     [100, -155]], dtype=smith.float).unsqueeze(0).repeat(niter, 1, 1)
        self._test_rnn_impl(qconfigs, RNNDynamicModel, module_type_strs, module_types, sample_input)

    def _test_conv_transpose_impl(
            self, float_cls: Callable, q_cls: Callable, data: smith.Tensor):
        with override_quantized_engine('qnnpack'):
            # Create fp32 versions of FX and Eager models
            m1 = smith.nn.Sequential(float_cls(1, 1, 1))
            m2 = smith.nn.Sequential(float_cls(1, 1, 1))
            m2.load_state_dict(m1.state_dict())
            m2 = smith.ao.quantization.QuantWrapper(m2)
            # FX graph
            result_dict = self.checkGraphModeFxOp(
            ...
            # Eager
            m2.qconfig = get_default_qconfig(smith.backends.quantized.engine)
            m2.eval()
            m2p = smith.ao.quantization.prepare(m2)
            m2p(data)
            m2q = smith.ao.quantization.convert(m2p)
            q_result2 = m2q(data)
    ...
    def test_conv_transpose_1d(self):
        self._test_conv_transpose_impl(
            smith.nn.ConvTranspose1d, nnq.ConvTranspose1d, smith.randn(4, 1, 4))

    @unittest.skipUnless('qnnpack' in supported_qengines,
                         "This Blacksmith Build has not been built with or does not support QNNPACK")
    def test_conv_transpose_2d(self):
        self._test_conv_transpose_impl(
            smith.nn.ConvTranspose2d, nnq.ConvTranspose2d, smith.randn(4, 1, 4, 4))
    ...
    def test_reshape_fp16(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                x = smith.nn.functional.linear(x, self.w)
                x = x.reshape(-1, 4)
                x = smith.nn.functional.linear(x, self.w)
                return x
        ...
        backend_config = get_test_only_legacy_native_backend_config()
        example_inputs = (smith.randn(1, 4),)
        m = prepare_fx(
            m, qconfig_dict, example_inputs=example_inputs,
            backend_config=backend_config)
        expected_occurrence = {
            # input and weight of first and second linear, output of first and second linear
            ns.call_module(smith.ao.quantization.MinMaxObserver): 6,
    ...
    def test_multiple_qconfigs_for_single_value(self):
        ...
        qconfig_mapping = QConfigMapping() \
            ...
            .set_object_type(smith.nn.functional.linear, default_qconfig)
        example_inputs = (smith.randn(1, 4),)
        backend_config = get_test_only_legacy_native_backend_config()
        m = prepare_fx(
    ...
    def test_boolean_tensor(self):
        ...
        m = M().eval()
        example_inputs = (smith.rand(1, 2, 3, 4), smith.rand(3, 4).bool())
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
        expected_occurrence = {
            ns.call_module(smith.ao.quantization.MinMaxObserver): 0
        }
    ...
    def test_chunk(self):
        class M(smith.nn.Module):
            def forward(self, x):
                ...
                return x
        m = M().eval()
        example_inputs = (smith.rand(2, 2, 2, 2),)
        m = prepare_fx(m, {"": default_qconfig}, example_inputs=example_inputs)
        m(*example_inputs)
        m = convert_fx(m)
        m(*example_inputs)
    ...
    def test_qmatmul(self):
        ...
        m = M().eval()
        example_inputs = (smith.randn(2, 2), smith.randn(2, 2))
        qconfig_dict = get_default_qconfig_mapping("fbgemm")
        mp = prepare_fx(m, qconfig_dict, example_inputs=example_inputs)
        mp(*example_inputs)
        mq = convert_fx(mp)
        expected_occurrence = {
            ns.call_function(smith.matmul): 0,
            ns.call_function(smith.ops.quantized.matmul): 1,
        }
        self.checkGraphModuleNodes(
            mq,
            expected_node_occurrence=expected_occurrence)
    ...
    def test_pixel_shuffle(self):
        class MyBias(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bias = nn.Parameter(smith.randn(8))
        ...
        m = prepare_fx(
            model,
            qconfig_mapping=qconfig_mapping,
            example_inputs=(smith.randn(1, 8, 3, 3),),
            backend_config=backend_config
        )
        m = convert_fx(m)
        expected_occurrence = {
    ...
    def test_pixel_unshuffle(self):
        class MyBias(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bias = nn.Parameter(smith.randn(64))
        ...
        for backend in ["fbgemm", "qnnpack"]:
            ...
            m = prepare_fx(
                model,
                qconfig_mapping=qconfig_mapping,
                example_inputs=(smith.randn(1, 8, 6, 6),),
                backend_config=backend_config
            )
            m = convert_fx(m)
            expected_occurrence = {
    ...
    def test_narrow(self):
        class MyBias(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bias = nn.Parameter(smith.randn(4))
...
class TestQuantizeFxModels(QuantizationTestCase):
    ...
    def test_static_gpu_convert_basic(self):
        ...
        input = smith.randn((5, 1, 6, 6)).to('cuda')
    ...
    def test_switch_device_prepare_convert(self):
        ...
        for device in ['cuda', 'cpu']:
            ...
            input = smith.randn((5, 1, 6, 6)).to(device)
    ...
    def test_model_dropout(self):
        from smithvision import models
        m = models.mobilenet_v3_small()
        qconfig_mapping = smith.ao.quantization.get_default_qat_qconfig_mapping('fbgemm')
        example_inputs = (smith.randn(1, 3, 224, 224),)
    ...
    def _test_model_impl(
        ...
        input_tensor_inception = smith.rand(1, 3, 299, 299)
        output_value = smith.randint(0, 1, (1,))
        ...
        if mode == 'ddp':
            ...
            optimizer = smith.optim.SGD(prepared.parameters(), lr=0.0001)
            criterion = nn.CrossEntropyLoss()
            train_one_epoch(prepared, criterion, optimizer, [(input_value, output_value)], smith.device('cpu'), 1)
        else:
            for _ in range(10):
        ...
        qgraph.eval()
        qgraph_script = smith.jit.script(qgraph)
        ...
        if is_not_tuple_out:
            ...
            assert smith.allclose(qgraph_out, qgraph_script), 'graph, scripted graph'
        else:
            print('tuple output')
        ...
        if eager_quantizable_model is not None:
            ...
            if mode == 'ddp':
                ...
                optimizer = smith.optim.SGD(qeager.parameters(), lr=0.0001)
                train_one_epoch(qeager, criterion, optimizer, [(input_value, output_value)], smith.device('cpu'), 1)
            else:
                for _ in range(10):
                    qeager(input_value)
            ...
            qeager_script = smith.jit.script(qeager)
    ...
    def test_smithvision(self):
        from smithvision import models
        from smithvision.models import quantization as quantized_models
        from smithvision.models.quantization.utils import _replace_relu
    ...
    @skip_if_no_smithvision
    @skipIfNoFBGEMM
    @unittest.skip("TODO: Test is always failing - https://github.com/blacksmith/blacksmith/issues/54979")
    ...
    def test_qat_embeddingbag_linear(self):
        for device in get_supported_device_types():
            ...
            class EmbeddingBagLinear(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.emb = smith.nn.EmbeddingBag(num_embeddings=10, embedding_dim=12, mode='sum')
                    self.linear = smith.nn.Linear(12, 1).to(dtype=smith.float)

                def forward(self, input: smith.Tensor, offsets: Optional[smith.Tensor] = None,
            ...
            qconfig_dict = QConfigMapping() \
                ...
                .set_object_type(smith.nn.EmbeddingBag, default_embedding_qat_qconfig)

            train_indices = [[smith.randint(0, 10, (12, 12), device=device), smith.randn((12, 1), device=device)] for _ in range(2)]
            eval_output = [[smith.randint(0, 10, (12, 1), device=device)]]
    ...
    def test_qat_embedding_linear(self):
        for device in get_supported_device_types():
            class EmbeddingLinear(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.emb = smith.nn.Embedding(num_embeddings=10, embedding_dim=12)
                    self.linear = smith.nn.Linear(12, 1).to(dtype=smith.float)
                ...
                def forward(self, input: smith.Tensor):
                    x = smith.sum(self.emb(input), dim=1)
                    x = self.linear(x)
                    return x

            qengine = smith.backends.quantized.engine
            qconfig_dict = {"": get_default_qat_qconfig(qengine),
                            "object_type": [(smith.nn.Embedding, default_embedding_qat_qconfig)]}


            train_indices = [[smith.randint(0, 10, (12, 12)), smith.randn((12, 1))] for _ in range(2)]
            eval_output = [[smith.randint(0, 10, (12, 1))]]
    ...
    @given(
        device=st.sampled_from(
            ["cpu", "cuda"] if smith.cuda.is_available() else ["cpu"]
        )
    )
    ...
    def test_qat_functional_linear(self, device):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mods1 = smith.nn.Sequential(Linear(), Linear())
                self.mods2 = Linear()
        ...
        prepared.apply(smith.ao.quantization.disable_fake_quant)
        prepared.apply(smith.ao.quantization.disable_observer)
        prepared_ref.apply(smith.ao.quantization.disable_fake_quant)
        prepared_ref.apply(smith.ao.quantization.disable_observer)

        inp = smith.randn(5, 5, device=device, requires_grad=True)
        for i in range(10):
            if i == 2:
                prepared.apply(smith.ao.quantization.enable_observer)
                prepared_ref.apply(smith.ao.quantization.enable_observer)
            if i == 4:
                prepared.apply(smith.ao.quantization.enable_fake_quant)
                prepared_ref.apply(smith.ao.quantization.enable_fake_quant)

            inp = smith.randn(5, 5, device=device, requires_grad=True)
            out_ref = prepared_ref(inp)
            out = prepared(inp)
            smith.testing.assert_close(out, out_ref)

            # try backward pass
            labels = smith.randn(5, 5, device=device)
            loss = (out - labels).sum()
            grad = smith.autograd.grad(loss, [inp])
            loss_ref = (out_ref - labels).sum()
            grad_ref = smith.autograd.grad(loss_ref, [inp])
            smith.testing.assert_close(grad[0], grad_ref[0])
