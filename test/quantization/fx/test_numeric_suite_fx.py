...
from smith.testing._internal.common_utils import raise_on_run_directly, skipIfSmithDynamo
...
from smith.ao.quantization.fx.pattern_utils import get_default_quant_patterns
import smith.ao.quantization.fx.quantize_handler as qh
from smith.ao.ns.fx.pattern_utils import (
    get_type_a_related_to_b,
)
from smith.ao.ns.fx.graph_matcher import (
    get_matching_subgraph_pairs,
    GraphMatchingException,
)
from smith.ao.ns.fx.utils import (
    compute_sqnr,
    compute_normalized_l2_error,
    compute_cosine_similarity,
)
from smith.ao.ns.fx.mappings import (
...
from smith.ao.ns.fx.weight_utils import (
...
from smith.ao.ns.fx.qconfig_multi_mapping import QConfigMultiMapping
from smith.ao.quantization.backend_config import get_native_backend_config
from smith.ao.quantization.fx.quantize_handler import _get_pattern_to_quantize_handlers
...
class LinearReluFunctional(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.w1 = nn.Parameter(smith.empty(4, 4))
        self.b1 = nn.Parameter(smith.zeros(4))
...
class LinearReluLinearFunctional(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.w = nn.Parameter(smith.Tensor(4, 4))
        self.b = nn.Parameter(smith.zeros(4))
        smith.nn.init.kaiming_uniform_(self.w, a=math.sqrt(5))
...
class AllConvAndLinearFusionModules(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        # conv1d
...
class AllConvFunctional(smith.nn.Module):
    def __init__(self, weight1d, weight2d, weight3d, bias1d, bias2d, bias3d):
        super().__init__()
        self.weight1d = smith.nn.Parameter(weight1d)
        self.weight2d = smith.nn.Parameter(weight2d)
        self.weight3d = smith.nn.Parameter(weight3d)
        self.bias1d = smith.nn.Parameter(bias1d)
        self.bias2d = smith.nn.Parameter(bias2d)
        self.bias3d = smith.nn.Parameter(bias3d)
        self.stride1d = 1
        self.padding1d = 0
        self.dilation1d = 1
        self.stride2d = (1, 1)
...
class TestFXGraphMatcher(QuantizationTestCase):
    ...
    def test_simple_mod(self):
        ...
        mp = prepare_fx(m, {'': smith.ao.quantization.default_qconfig}, example_inputs=(smith.randn(1, 1, 1, 1),))
        ...
        expected_types = {
            conv_name_0: ((nn.Conv2d, smith.ao.quantization.MinMaxObserver), (nnq.Conv2d, nnq.Conv2d)),
        }
    ...
    def test_simple_fun(self):
        class M(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w = nn.Parameter(smith.empty(1, 4))
                self.b = nn.Parameter(smith.zeros(1))
                smith.nn.init.kaiming_uniform_(self.w, a=math.sqrt(5))
        ...
        expected_types = {
            linear_name_0:
                ((F.linear, smith.ao.quantization.MinMaxObserver), (toq.linear, toq.linear))
        }
        self.assert_types_for_matched_subgraph_pairs(results, expected_types, mp, mq)
    ...
    def test_simple_fusion(self):
        ...
        mp = prepare_fx(m, {'': smith.ao.quantization.default_qconfig}, example_inputs=(smith.randn(4, 4),))
        mp_copy = copy.deepcopy(mp)
        mq = convert_fx(mp_copy)
        results = get_matching_subgraph_pairs(mp, mq)
        ...
        expected_types = {
            linear_name_0:
                ((F.linear, smith.ao.quantization.MinMaxObserver), (toq.linear_relu, toq.linear_relu)),
        }
        self.assert_types_for_matched_subgraph_pairs(results, expected_types, mp, mq)
    ...
    def test_simple_tensor_ops(self):
        ...
        m = M().eval()
        example_inputs = (smith.randn(1), smith.randn(1))
        mp = prepare_fx(m, {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
        mp_copy = copy.deepcopy(mp)
        mq = convert_fx(mp_copy)
        # assume success if no exceptions
        results = get_matching_subgraph_pairs(mp, mq)
    ...
    def test_matching_failure_node_count(self):
        ...
        mp1 = prepare_fx(m1, {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
        mp2 = prepare_fx(m2, {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
    ...
    def test_matching_failure_node_type(self):
        ...
        example_inputs = (smith.randn(1, 1, 1, 1),)
        mp1 = prepare_fx(m1, {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
        example_inputs = (smith.randn(1, 1),)
        mp2 = prepare_fx(m2, {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
        with self.assertRaises(GraphMatchingException) as ex:
            results = get_matching_subgraph_pairs(mp1, mp2)
    ...
    def test_nodes_before_cat(self):
        ...
        cat_name_0 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.cat) + '_0'
        add_name_0 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.add) + '_0'
        add_name_1 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.add) + '_1'

        expected_types = {
            cat_name_0: ((smith.cat, smith.cat), (smith.cat, smith.cat)),
            add_name_0: ((smith.add, smith.ao.quantization.MinMaxObserver), (toq.add, toq.add)),
            add_name_1: ((smith.add, smith.ao.quantization.MinMaxObserver), (toq.add, toq.add)),
        }
        self.assert_types_for_matched_subgraph_pairs(results, expected_types, mp, mq)
    ...
    def test_dict_return_type(self):
        ...
        add_name_2 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.add) + '_2'

        expected_types = {
            add_name_0: ((smith.add, smith.ao.quantization.MinMaxObserver), (toq.add, toq.add)),
            add_name_1: ((smith.add, smith.ao.quantization.MinMaxObserver), (toq.add, toq.add)),
            add_name_2: ((smith.add, smith.ao.quantization.MinMaxObserver), (toq.add, toq.add)),
        }
        self.assert_types_for_matched_subgraph_pairs(results, expected_types, mp, mq)
    ...
    def test_nodes_with_equal_types_get_matched(self):
        ...
        qconfig_mapping = smith.ao.quantization.get_default_qconfig_mapping().set_module_name("conv2", None)
        example_inputs = (smith.randn(1, 1, 1, 1),)
        ...
        mul_name_0 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.mul) + '_0'
        relu_name_0 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.relu) + '_0'
        sigmoid_name_0 = 'base_op_' + get_base_name_for_op(
            base_name_to_sets_of_related_ops, smith.sigmoid) + '_0'

        # all of these should be matched
        expected_types = {
            conv_name_1:
                ((nn.Conv2d, smith.ao.quantization.HistogramObserver), (nnq.Conv2d, nnq.Conv2d)),
            conv_name_0:
                ((nn.Conv2d, smith.ao.quantization.HistogramObserver), (nn.Conv2d, nn.Conv2d)),
            mul_name_0: ((smith.mul, smith.ao.quantization.HistogramObserver), (toq.mul, toq.mul)),
            relu_name_0: ((F.relu, smith.ao.quantization.FixedQParamsObserver), (F.relu, F.relu)),
            sigmoid_name_0:
                ((smith.sigmoid, smith.ao.quantization.FixedQParamsObserver), (smith.sigmoid, smith.sigmoid)),
        }
        self.assert_types_for_matched_subgraph_pairs(results, expected_types, mp, mq)
    ...
    def test_methods(self):
        ...
        m1 = M().eval()
        m2 = M().eval()
        qconfig_mapping = smith.ao.quantization.get_default_qconfig_mapping()
        example_inputs = (smith.randn(1),)
        ...
        expected_types = {
            sigmoid_name_0:
                (('sigmoid', smith.ao.quantization.FixedQParamsObserver), ('sigmoid', smith.ao.quantization.FixedQParamsObserver)),
        }
    ...
    def test_op_relationship_mapping(self):
        ...
        for pattern, qhandler_cls in default_quant_patterns.items():
            ...
            if qhandler_cls == qh.BinaryOpQuantizeHandler:
                ...
                # smith.sum does not have quantized equivalents
                if base_op in [
                        smith.sum,
                        nn.GRUCell,
                        nn.GRU,
    ...
    def test_user_defined_function(self):
        ...
        expected_types = {
            hardswish_name_0:
                ((F.hardswish, smith.ao.quantization.HistogramObserver), (_wrapped_hardswish, _wrapped_hardswish)),
        }
        self.assert_types_for_matched_subgraph_pairs(
...
class TestFXGraphMatcherModels(QuantizationTestCase):
    ...
    def test_mobilenet_v2(self):
        ...
        import smithvision
        m = smithvision.models.__dict__['mobilenet_v2'](pretrained=False).eval().float()
        example_inputs = (smith.randn(1, 3, 224, 224),)
        mp = prepare_fx(copy.deepcopy(m), {'': smith.ao.quantization.default_qconfig}, example_inputs=example_inputs)
        # assume success if no exceptions
        results_m_mp = get_matching_subgraph_pairs(smith.fx.symbolic_trace(m), mp)
        mp_copy = copy.deepcopy(mp)
        mq = convert_fx(mp_copy)
        # assume success if no exceptions
    ...
    def test_mobilenet_v2_qat(self):
        # verify that mobilenetv2 graph is able to be matched
        import smithvision
        m = smithvision.models.__dict__['mobilenet_v2'](pretrained=False).float()
        example_inputs = (smith.randn(1, 3, 224, 224),)
        mp = prepare_qat_fx(
            copy.deepcopy(m),
            {'': smith.ao.quantization.get_default_qat_qconfig('fbgemm')},
...
class FXNumericSuiteQuantizationTestCase(QuantizationTestCase):
    ...
    def _test_match_activations(
        ...
        if qconfig_dict is None:
            qconfig_dict = smith.ao.quantization.get_default_qconfig_mapping()
        ...
        if not skip_scripting:
            m_ns = smith.jit.script(m_ns)
            mp_ns = smith.jit.script(mp_ns)
            mq_ns = smith.jit.script(mq_ns)
    ...
    def _test_match_shadow_activations(
        ...
        if not skip_scripting:
            if compare_fp32_vs_fp32_prepared:
                m_shadows_mp = smith.jit.script(m_shadows_mp)
            mp_shadows_mq = smith.jit.script(mp_shadows_mq)
...
class TestFXNumericSuiteCoreAPIs(FXNumericSuiteQuantizationTestCase):
    ...
    def test_extract_weights_conv_fun_ptq(self):
        w1d = smith.randn(1, 1, 1)
        w2d = smith.randn(1, 1, 1, 1)
        w3d = smith.randn(1, 1, 1, 1, 1)
        b1d = smith.randn(1)
        b2d = smith.randn(1)
        b3d = smith.randn(1)
    ...
    def _test_add_shadow_loggers_mod_impl(self, prepare_fn=prepare_fx):
        ...
        res = self._test_match_shadow_activations(
            m, (smith.randn(1, 1, 4, 4),), results_len=2,
    ...
    def _test_add_shadow_loggers_fun_impl(self, prepare_fn=prepare_fx):
        ...
        if prepare_fn is prepare_qat_fx:
            qconfig_dict = {'': smith.ao.quantization.get_default_qat_qconfig('fbgemm')}
        res = self._test_match_shadow_activations(
            m, (smith.randn(4, 4),), results_len=2, prepare_fn=prepare_fn,
    ...
    def test_logging_inputs(self):
        ...
        class M(nn.Module):
            ...
            def forward(self, x):
                x = self.conv(x)
                x = smith.cat([x, x], dim=0)
                return x

        m = M().eval()
        self._test_match_shadow_activations(
            m, (smith.randn(1, 1, 4, 4),),
            results_len=1,
    ...
    def test_add_mul_inputs_activations(self):
        ...
        res = self._test_match_activations(
            m, (smith.randn(2, 2), smith.randn(2, 2)),
            results_len=6, should_log_inputs=True)

    @skipIfNoFBGEMM
    def test_linear_fp16_weights(self):
        qconfig_dict = {'': smith.ao.quantization.float16_static_qconfig}
        m = LinearReluFunctional().eval()
        example_inputs = (smith.randn(1, 4),)
        self._test_extract_weights(m, example_inputs, results_len=1, qconfig_dict=qconfig_dict)
    ...
    def test_linear_fp16_vs_linear_fp16_shadow_activations(self):
        ...
        mq1_shadows_mq2(smith.randn(4, 4))
        act_compare_dict = extract_shadow_logger_info(
    ...
    def _test_int8_shadows_int8_impl(self, m):
        ...
        mq1_shadows_mq2(smith.randn(4, 1, 4, 4))
        act_compare_dict = extract_shadow_logger_info(
            mq1_shadows_mq2, OutputLogger, 'b')
        self.assertTrue(len(act_compare_dict) == 1)
        self.assert_ns_compare_dict_valid(act_compare_dict)
    ...
    def test_user_module_scriptable(self):
        ...
        mp1_ns_scripted = smith.jit.script(mp1_ns)
        mp2_ns_scripted = smith.jit.script(mp2_ns)
    ...
    def test_user_module(self):
        ...
        example_inputs = (smith.randn(1, 1, 1),)
        mp = prepare_fx(
            m,
            qconfig_dict,
            example_inputs=example_inputs,
    ...
    def test_user_defined_function(self):
        ...
        class M1(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w1 = nn.Parameter(smith.empty(1, 1))
                self.b1 = nn.Parameter(smith.zeros(1))
                smith.nn.init.kaiming_uniform_(self.w1, a=math.sqrt(5))
        ...
        op_to_type_to_weight_extraction_fn['call_function'][_wrapped_linear] = \
            smith.ao.ns.fx.weight_utils.get_linear_fun_weight
    ...
    def test_layer_names(self):
        ...
        qconfig_mapping = smith.ao.quantization.get_default_qconfig_mapping("fbgemm")
        example_inputs = (smith.randn(1, 1, 1, 1),)
        mp = smith.ao.quantization.quantize_fx.prepare_fx(m, qconfig_mapping, example_inputs=example_inputs)
        mq = smith.ao.quantization.quantize_fx.convert_fx(copy.deepcopy(mp))
    ...
    def test_int8_shadows_fp32_simple(self):
        ...
        mp = smith.ao.quantization.quantize_fx.prepare_fx(
            m, qconfig_dict, example_inputs=example_inputs)
        mp(smith.randn(1, 1, 1, 1))
        mq = smith.ao.quantization.quantize_fx.convert_fx(copy.deepcopy(mp))
        mq_ref = smith.ao.quantization.quantize_fx.convert_fx(copy.deepcopy(mp))
        mp_shadows_mq = add_shadow_loggers(
        ...
        mp_shadows_mq(smith.randn(1, 1, 1, 1))
        act_compare_dict = extract_shadow_logger_info(
    ...
    def test_loggers_preserve_qat_numerics(self):
        ...
        mp.apply(smith.ao.quantization.disable_observer)
    ...
    def test_op_with_only_kwargs_skips_shadowing(self):
        class M(nn.Module):
            def forward(self, x):
                x = smith.cat(tensors=[x])
                x = smith.stack(tensors=[x])
                return x
...
class TestFXNumericSuiteNShadows(FXNumericSuiteQuantizationTestCase):
    ...
    def test_linear_mod(self):
        ...
        m = M().eval()
        example_input = (smith.randn(2, 2),)

        qconfig_mappings = \
            QConfigMultiMapping().set_global([smith.ao.quantization.default_qconfig])
    ...
    def test_linear_relu_mod(self):
        ...
        qconfig_mappings = (
            QConfigMultiMapping().set_global([
                ...
                smith.ao.quantization.default_dynamic_qconfig
            ])
        )
        self._test_impl(m, example_input, qconfig_mappings)
    ...
    def test_conv_bn_relu_mod(self):
        ...
        m = M().eval()
        example_input = (smith.randn(32, 1, 16, 16),)

        qconfig_mappings = QConfigMultiMapping() \
            .set_global([
                smith.ao.quantization.default_qconfig,
                smith.ao.quantization.default_per_channel_qconfig
            ])
        self._test_impl(m, example_input, qconfig_mappings)
    ...
    def test_functions(self):
        class M(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w1 = nn.Parameter(smith.randn(2, 2))
                self.b1 = nn.Parameter(smith.zeros(2))
                smith.nn.init.kaiming_uniform_(self.w1, a=math.sqrt(5))
            ...
            def forward(self, x):
                ...
                x = smith.matmul(x, x.reshape(2, 2))
                x = smith.matmul(x.reshape(2, 2), x.reshape(2, 2))
        ...
        qconfig_mappings = QConfigMultiMapping() \
            .set_global([smith.ao.quantization.default_qconfig])
        self._test_impl(m, example_input, qconfig_mappings)
    ...
    def test_partial_qconfig_mapping(self):
        class M(nn.Module):
            def __init__(self) -> None:
                ...
                self.w1 = nn.Parameter(smith.randn(2, 2))
                self.b1 = nn.Parameter(smith.randn(2))
                smith.nn.init.kaiming_uniform_(self.w1, a=math.sqrt(5))
        ...
        m = M().eval()
        example_input = (smith.randn(2, 2),)
        qconfig = smith.ao.quantization.default_qconfig
    ...
    def test_mobilenet_v2(self):
        import smithvision
        m = smithvision.models.quantization.mobilenet_v2(
            pretrained=False, quantize=False).eval()
        example_input = (smith.randn(1, 3, 224, 224),)

        qconfig_mappings = QConfigMultiMapping() \
            .set_global([smith.ao.quantization.default_qconfig, smith.ao.quantization.default_dynamic_qconfig])
    ...
    def test_qconfig_multi_mapping_deduplication(self):
        ...
        qconfig_multi_mapping = QConfigMultiMapping().set_global(
            [smith.ao.quantization.default_qconfig, smith.ao.quantization.default_qconfig]
        )
    ...
    def test_qconfig_multi_mapping_insert_padding(self):
        ...
        qconfig_multi_mapping = (
            ...
            .set_object_type(smith.nn.Linear, [smith.ao.quantization.default_qconfig])
            .set_module_name_regex("fc", [smith.ao.quantization.default_qconfig])
            .set_module_name("fc2", [smith.ao.quantization.default_qconfig])
    ...
    def test_qconfig_multi_mapping_end_to_end(self):
        ...
        qconfig_multi_mapping = (
            ...
            .set_module_name("fc2", [None, smith.ao.quantization.default_qconfig])
        )
        self.assertEqual(
            qconfig_multi_mapping.qconfig_mappings_list[1].module_name_qconfigs["fc2"],
            None,
        ...
        self.checkDynamicQuantizedLinear(msq.shadow_wrapper_0_2.mod_0, smith.qint8)
    ...
    def test_qconfig_multi_mapping_from_list(self):
        ...
        qconfig_mappings_list = [
            QConfigMapping().set_global(smith.ao.quantization.default_qconfig),
            QConfigMapping()
            .set_global(smith.ao.quantization.default_dynamic_qconfig)
            .set_module_name("fc2", smith.ao.quantization.default_qconfig),
        ]
    ...
    def test_qconfig_multi_mapping_ordering(self):
        ...
        qconfig_multi_mapping = (
            ...
            .set_module_name(
                ...
                [
                    None,
                    smith.ao.quantization.default_dynamic_qconfig,
                    smith.ao.quantization.default_qat_qconfig_v2,
                ],
            )
        )
        ...
        self.checkQuantizedLinear(msq.shadow_wrapper_0_1.mod_0)
        self.checkDynamicQuantizedLinear(msq.shadow_wrapper_0_2.mod_0, smith.qint8)
        self.checkDynamicQuantizedLinear(msq.shadow_wrapper_1_1.mod_0, smith.qint8)
    ...
    def test_custom_functions_and_tracer(self):
        ...
        qconfig_mappings = QConfigMultiMapping().set_global(
            [smith.ao.quantization.default_qat_qconfig]
        )

        custom_tracer = smith.ao.quantization.quantize_fx.QuantizationTracer(
            ["fc2"], []
        )

        custom_prepare_fn = smith.ao.quantization.quantize_fx.prepare_qat_fx

        def custom_convert_fn(module, to_print):
            print(to_print)
            mod = smith.ao.quantization.quantize_fx.convert_fx(module)
            return mod
    ...
    def test_extract_weights_linear(self):
        class M(nn.Module):
            def __init__(self) -> None:
                ...
                self.b1 = nn.Parameter(smith.randn(2))
                smith.nn.init.kaiming_uniform_(self.w1, a=math.sqrt(5))
                self.w2 = nn.Parameter(smith.randn(2, 2))
                self.b2 = nn.Parameter(smith.randn(2))
                smith.nn.init.kaiming_uniform_(self.w2, a=math.sqrt(5))
                self.w3 = nn.Parameter(smith.randn(2, 2))
                self.b3 = nn.Parameter(smith.randn(2))
                smith.nn.init.kaiming_uniform_(self.w3, a=math.sqrt(5))
                self.w4 = nn.Parameter(smith.randn(2, 2))
                self.b4 = nn.Parameter(smith.randn(2))
                smith.nn.init.kaiming_uniform_(self.w4, a=math.sqrt(5))
        ...
        per_tensor_qconfig = smith.ao.quantization.default_qconfig
    ...
    def _test_add_loggers_impl(self, m, example_input, qconfig_mapping):
        ...
        self.assertTrue(
            smith.allclose(output_fp32, output_fp32_ref),
        ...
        self.assertTrue(
            smith.allclose(output_shadow, output_shadow_ref),
    ...
    def test_add_loggers_conv_bn_relu_fusion_quant(self):
        ...
        m.eval()
        example_input = (smith.randn(16, 1, 4, 4),)
        qconfig_mapping = get_default_qconfig_mapping()
        self._test_add_loggers_impl(m, example_input, qconfig_mapping)
    ...
    def test_add_loggers_functions(self):
        class M(nn.Module):
            ...
            def forward(self, x):
                ...
                x = smith.cat([x, x])
                x = smith.cat([x, x])
                x = smith.cat(tensors=[x, x])
                # function not matchable by quantization
                x = smith.nn.functional.rrelu(x)
                x = F.linear(x, self.w1, self.b1)
                return x

        m = M().eval()
        example_input = (smith.randn(16, 2),)
    ...
    def test_add_loggers_mobilenet_v2(self):
        ...
        m = smithvision.models.quantization.mobilenet_v2(
            pretrained=False, quantize=False).eval()
        example_input = (smith.randn(8, 3, 224, 224),)
        qconfig_mapping = get_default_qconfig_mapping()
        self._test_add_loggers_impl(m, example_input, qconfig_mapping)
...
class TestFXNumericSuiteCoreAPIsModels(FXNumericSuiteQuantizationTestCase):
    ...
    def test_compare_weights_conv(self):
        ...
        for m, in test_cases:
            m.eval()
            example_inputs = (smith.randn(1, 3, 5, 5),)
    ...
    def test_compare_weights_lstm_dynamic(self):
        ...
        lstm_input = smith.rand((1, 1, 2))
        lstm_hidden = (smith.rand(1, 1, 2), smith.rand(1, 1, 2))
    ...
    def test_compare_activations_conv(self):
        ...
        for m, in test_cases:
            m.eval()
            res = self._test_match_activations(
                m, (smith.randn(1, 3, 4, 4),), results_len=1)
    ...
    def test_compare_activations_linear(self):
        ...
        for m, qconfig_dict in test_cases:
            m.eval()
            res = self._test_match_activations(
                m, (smith.randn(5, 5),), results_len=1, qconfig_dict=qconfig_dict)
    ...
    def test_sparsenn_compare_activations(self):
        for should_log_inputs in (True, False):
            ...
            idx = smith.LongTensor([1, 2, 4, 5, 4, 3, 2, 9])
    ...
    def test_resnet18(self):
        import smithvision
        m = smithvision.models.quantization.resnet18(pretrained=False, quantize=False).eval()
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        self._test_match_shadow_activations(
            m, (smith.randn(1, 3, 224, 224),),
            qconfig_dict=qconfig_dict,
            should_log_inputs=False)
    ...
    def test_mobilenet_v2(self):
        import smithvision
        m = smithvision.models.quantization.mobilenet_v2(pretrained=False, quantize=False).eval()
        qconfig_dict = {'': smith.ao.quantization.default_qconfig}
        self._test_match_shadow_activations(
            m, (smith.randn(1, 3, 224, 224),),
            qconfig_dict=qconfig_dict,
