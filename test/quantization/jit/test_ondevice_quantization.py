...
import smith
import smith._C
from smith.ao.quantization import default_dynamic_qconfig, per_channel_dynamic_qconfig
...
from smith.jit.mobile import _load_for_lite_interpreter, LiteScriptModule
...
from smith.utils import bundled_inputs
...
class MyConvLinearModule(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.conv = smith.nn.Conv2d(3, 5, 3)
        weight = smith.nn.Parameter(smith.ones(5, 5))
        self.weight1 = smith.nn.Parameter(smith.ones(5, 5))
        self.mymod = myMod(weight)

    def forward(self, x):
        conv_output = self.conv(x)
        y = self.mymod(conv_output)
        z = smith.nn.functional.linear(y, self.weight1)
        return z

    def get_example_inputs(self):
        return (smith.rand(1, 3, 12, 7),)
...
class TestOnDeviceDynamicPTQInsertObservers(TestCase):
    ...
    def _check_observer_method(self, model, num_observers):
        ...
        smith._C._jit_pass_inline(orig_scripted_model.graph)
...
class TestOnDeviceDynamicPTQFinalize(TestCase):
    ...
    def _check_against_ref_dynamic_ptq(self, model):
        model.eval()
        inputs = model.get_example_inputs()
        ref_m = smith.jit.script(model)
        smith._C._jit_pass_inline(ref_m.graph)
        qconfig_dict = {"": default_dynamic_qconfig}
        ...
        self.assertTrue(smith.allclose(ref_output, output))
        thrown = False
        try:
            m(*inputs)
        except Exception:
    ...
    def _check_serdes_and_device_side_api_helper(
        ...
        ref_m = convert_dynamic_jit(ref_m)
        buffer = io.BytesIO()
        smith.jit.save(ref_m, buffer)
        buffer.seek(0)
        ref_m = smith.jit.load(buffer)
        ref_output = ref_m(*inputs)
        ...
        if not check_device_side_api:
            ...
            smith._C._quantize_ondevice_ptq_dynamic(m._c, "forward")
            ...
            dict_: dict[str, str] = {}
            bytes = smith._C._save_mobile_module_to_bytes(m._c, dict_)
            m = LiteScriptModule(smith._C._load_mobile_module_from_bytes(bytes))
            fb_output = m(*inputs)
            self.assertTrue(smith.allclose(ref_output, fb_output))
