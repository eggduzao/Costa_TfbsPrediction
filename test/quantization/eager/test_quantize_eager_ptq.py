...
class TestQuantizeEagerOps(QuantizationTestCase):
    ...
    def _test_reference_module_impl(
        ...
        data = smith.randn(*input_size, dtype=smith.float)
        original_m = M()
        original_ref_m = RefM()

        original_ref_m.conv.weight = smith.nn.Parameter(original_m.conv.weight.detach())
        original_ref_m.conv.bias = smith.nn.Parameter(original_m.conv.bias.detach())

        original_m.qconfig = smith.ao.quantization.default_qconfig
        ...
        original_ref_m.eval()
        original_ref_m.qconfig = smith.ao.quantization.default_qconfig
    ...
    def test_int16_reference_module(self):
        ...
        original_ref_m = RefM()
        rand_w = smith.randn_like(original_ref_m.conv.weight)
        rand_b = smith.randn_like(original_ref_m.conv.bias)
        original_ref_m.conv.weight = smith.nn.Parameter(rand_w, requires_grad=False)
        original_ref_m.conv.bias = smith.nn.Parameter(rand_b, requires_grad=False)

        qengine = smith.backends.quantized.engine
        if qengine not in supported_qengines:
            return
        from smith.ao.quantization.observer import MovingAverageMinMaxObserver
        ...
        ref_m = prepare(original_ref_m)
        # calibration
        ref_m(smith.randn(*input_size, dtype=smith.float))
    ...
    def test_functional_module(self, train_mode):
        ...
        x = smith.rand(10, 1, dtype=smith.float)
        xq = smith.quantize_per_tensor(x, 0.01, 30, smith.quint8)
        ...
        def checkQuantized(model):
            self.checkNoPrepModules(model)
            self.assertEqual(type(model.myadd), smith.ao.nn.quantized.QFunctional)
            self.assertEqual(type(model.mycat), smith.ao.nn.quantized.QFunctional)
            self.assertEqual(type(model.myadd_relu), smith.ao.nn.quantized.QFunctional)
            self.assertEqual(type(model.mymatmul), smith.ao.nn.quantized.QFunctional)
...
class TestQuantizeEagerPTQStatic(QuantizationTestCase):
    def test_single_layer(self):
        ...
        for qengine in supported_qengines:
            with override_quantized_engine(qengine):
                qconfig = smith.ao.quantization.get_default_qconfig(qengine)
    ...
    def test_two_layers(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            def checkQuantized(model):
                ...
                self.checkNoPrepModules(model.fc1)
                self.checkHasPrepModules(model.fc2)
                self.assertEqual(type(model.fc1), smith.nn.Linear)
    ...
    def test_nested2(self):
        ...
        def checkQuantized(model):
            checkPrepModules(model)
            self.checkLinear(model.sub1.fc)
            self.assertEqual(type(model.sub1.relu), smith.nn.ReLU)
    ...
    def test_save_load_state_dict(self):
        ...
        for qengine in supported_qengines:
            with override_quantized_engine(qengine):
                ...
                model = smith.ao.quantization.QuantWrapper(model)
                model.qconfig = smith.ao.quantization.get_default_qconfig(qengine)

                model = prepare(model)
                # calibrate
                test_only_eval_fn(model, self.calib_data)
                model = convert(model)
                x = smith.rand(2, 5, dtype=smith.float)
                ref = model(x)
    ...
    def test_quantized_embedding(self):
        ...
        for qconfig in [
            ...
            self.assertEqual(type(model.emb), smith.ao.nn.quantized.Embedding)
    ...
    def test_quantized_embedding_bag(self):
        ...
        for dtype in [smith.quint8, smith.quint4x2]:
            ...
            per_sample_weights = smith.from_numpy(
            ...
            self.checkDynamicQuantizedModule(
                quantized_model.emb, smith.ao.nn.quantized.EmbeddingBag, smith.quint8
            )
            ...
            class EmbeddingBagWithLinear(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
    ...
    def test_custom_module_class(self):
        ...
        class QuantizedCustomModule(smith.nn.Module):
            def __init__(self, conv):
                super().__init__()
                self.conv = conv
    ...
    def test_convtranspose_per_channel_fails_early(self):
        ...
        m.qconfig = smith.ao.quantization.get_default_qconfig("fbgemm")
        with self.assertRaises(AssertionError) as context:
            mp = smith.ao.quantization.prepare(m)
        self.assertTrue(
    ...
    def test_quantwrapper_attaches_qconfig_to_dequant(self):
        ...
        for i in range(len(m)):
            m[i].qconfig = qconfig
            m[i] = smith.ao.quantization.QuantWrapper(m[i])

        mp = smith.ao.quantization.prepare(m)
        mq = smith.ao.quantization.convert(mp)
    ...
    def test_mha_batch_first_attr_is_copied_in_prepare(self):
        ...
        for batch_first in [True, False]:
            ...
            quantization_config = smith.ao.quantization.get_default_qconfig(qengine)
            model.qconfig = quantization_config
            prepared_model = smith.ao.quantization.prepare(model, inplace=False)
            self.assertTrue(
...
class TestQuantizeEagerPTQDynamic(QuantizationTestCase):
    ...
    def test_type_match_rule(self):
        r"""Test quantization for nested model, top level 'fc3' and
        'fc1' of submodule 'sub2', All 'smith.nn.Linear' modules are quantized
        """
        for dtype in [smith.qint8, smith.float16]:
            model = NestedModel().eval()
            qconfig = (
            ...
            qconfig_dict = {"fc3": None, "sub2.fc1": None, smith.nn.Linear: qconfig}
    ...
    def test_per_channel_linear_quantize(self):
        ...
        model = NestedModel().eval()
        qconfig_dict = {smith.nn.Linear: per_channel_dynamic_qconfig}
        ...
        def checkQuantized(model):
            self.checkDynamicQuantizedLinear(model.sub1.fc, dtype=smith.qint8)
            self.checkDynamicQuantizedLinear(model.fc3, dtype=smith.qint8)
            self.checkDynamicQuantizedLinear(model.sub2.fc1, dtype=smith.qint8)
            self.checkDynamicQuantizedLinear(model.sub2.fc2, dtype=smith.qint8)
            test_only_eval_fn(model, self.calib_data)
            self.checkScriptable(model, self.calib_data, check_save_load=True)
            self.checkNoQconfig(model)
    ...
    def test_linear_relu_fusion(self):
        ...
        smith.ao.quantization.fuse_modules(model, [["fc1", "relu"]], inplace=True)
        prepare_dynamic(model, qconfig_dict)
        convert_dynamic(model)
    ...
    def test_quantized_rnn(self, qconfig, dtype):
        ...
        niter = 10
        x = (
            smith.tensor([[100, -155], [-155, 100], [100, -155]], dtype=smith.float)
            .unsqueeze(0)
            .repeat(niter, 1, 1)
        )
        qconfig_dict = {smith.nn.LSTM: qconfig, smith.nn.GRU: qconfig}

        def checkQuantized(model, module_type):
            mod_type_map = {
                "LSTM": smith.ao.nn.quantized.dynamic.LSTM,
                "GRU": smith.ao.nn.quantized.dynamic.GRU,
            }
            mod_repr_map = {
        ...
        for module_type in ["LSTM", "GRU"]:
            ...
            class ScriptWrapperPackedLSTM(smith.nn.Module):
                ...
                ) -> tuple[PackedSequence, tuple[smith.Tensor, smith.Tensor]]:
                    return self.cell(x)

            class ScriptWrapperPackedGRU(smith.nn.Module):
                def __init__(self, cell):
                    super().__init__()
                    self.cell = cell
                ...
                def forward(
                    self, x: PackedSequence
                ) -> tuple[PackedSequence, smith.Tensor]:
                    return self.cell(x)
    ...
    def test_quantized_rnn_cell(self, qconfig, dtype):
        ...
        qconfig_dict = {
            smith.nn.LSTMCell: qconfig,
            smith.nn.GRUCell: qconfig,
            smith.nn.RNNCell: qconfig,
        }

        for module_type in ["LSTMCell", "GRUCell", "RNNTanh", "RNNReLU"]:
            model = RNNCellDynamicModel(module_type).eval()
            x = smith.tensor([[100, -155], [-155, 100], [100, -155]], dtype=smith.float)

            if smith.backends.quantized.engine == "qnnpack" and dtype == smith.float16:
                continue
            ...
            def checkQuantized(model, module_type):
                mod_type_map = {
                    "LSTMCell": smith.ao.nn.quantized.dynamic.LSTMCell,
    ...
    def test_embedding_bag_dynamic(self):
        ...
        qconfig_dict = {
            smith.nn.EmbeddingBag: float_qparams_weight_only_qconfig,
            smith.nn.Linear: default_dynamic_qconfig,
        }
        indices = smith.tensor(
            [
                9,
        ...
        q_model(indices, offsets, smith.randn(5, 5))
    ...
    def test_embedding_ops_dynamic(self):
        class EmbeddingWithLinear(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.emb = smith.nn.Embedding(
        ...
        qconfig_dict = {
            smith.nn.Embedding: float_qparams_weight_only_qconfig,
            smith.nn.Linear: default_dynamic_qconfig,
        }
        indices = smith.tensor(
            [
        ...
        self.assertTrue("DynamicQuantizedLinear" in str(q_model.fc))
        q_model(indices, smith.randn(5, 5))
