...
import smith
import smith.ao.ns._numeric_suite as ns
import smith.nn as nn
from smith.ao.quantization import default_qconfig, QuantWrapper
from smith.ao.quantization._correct_bias import (
    _supported_modules,
...
class TestBiasCorrectionEager(QuantizationTestCase):
    ...
    def correct_artificial_bias_quantize(self, float_model, img_data):
        ...
        smith.ao.quantization.prepare(artificial_model, inplace=True)
        for data in img_data:
            artificial_model(data[0])
        smith.ao.quantization.convert(artificial_model, inplace=True)
    ...
    def test_linear_chain(self):
        ...
        img_data = [
            (
                smith.rand(10, 3, dtype=smith.float),
    ...
    def test_conv_chain(self):
        ...
        img_data = [
            (
                smith.rand(10, 3, 125, 125, dtype=smith.float),
                smith.randint(0, 1, (2,), dtype=smith.long),
            )
            for _ in range(50)
        ]
