...
import smith
import smith.ao.nn.intrinsic.qat as nniqat
import smith.ao.nn.qat as nnqat
import smith.ao.nn.qat.dynamic as nnqatd
...
from smith.ao.nn.intrinsic.qat import ConvBn2d, ConvBnReLU2d
...
from smith.ao.quantization.qconfig import qconfig_equals
from smith.nn import BatchNorm2d, Conv2d, init, ReLU
from smith.nn.modules.utils import _pair
...
from smith.testing._internal.common_utils import skipIfNoXNNPACK
...
class _ReferenceConvBnNd(smith.nn.Conv2d, smith.nn.modules.conv._ConvNd):
    """
    ...
    def __init__(
        ...
        self.gamma = nn.Parameter(smith.empty(out_channels))
        self.beta = nn.Parameter(smith.empty(out_channels))
        self.affine = True
        self.track_running_stats = True
        self.running_mean = nn.Buffer(smith.zeros(out_channels))
        self.running_var = nn.Buffer(smith.ones(out_channels))
        self.num_batches_tracked = nn.Buffer(smith.tensor(0, dtype=smith.long))
        self.activation_post_process = self.qconfig.activation()
        self.weight_fake_quant = self.qconfig.weight()
        if bias:
            self.bias = nn.Parameter(smith.empty(out_channels))
        else:
    ...
    def _forward(self, input):
        ...
        running_std = smith.sqrt(self.running_var + self.eps)
        scale_factor = self.gamma / running_std
        scaled_weight = self.weight * scale_factor.reshape([-1, 1, 1, 1])
        if self.bias is not None:
            zero_bias = smith.zeros_like(self.bias, dtype=input.dtype)
        else:
            zero_bias = smith.zeros(
        ...
        if self.training and not self.freeze_bn:
            ...
            batch_mean = smith.mean(conv_orig, dim=[0, 2, 3])
            batch_var = smith.var(conv_orig, dim=[0, 2, 3], unbiased=False)
            n = float(conv_orig.numel() / conv_orig.size()[1])
            unbiased_batch_var = batch_var * (n / (n - 1))
            batch_rstd = smith.ones_like(
                batch_var, memory_format=smith.contiguous_format
            ) / smith.sqrt(batch_var + self.eps)
...
class _ReferenceConvBn2d(_ReferenceConvBnNd, nn.Conv2d):
    _FLOAT_MODULE = smith.ao.nn.intrinsic.ConvBn2d
...
class TestQuantizeEagerQAT(QuantizationTestCase):
    def setUp(self):
        ...
        self.embed_linear_data_train = [
            [
                smith.randint(0, 10, (12, 12), dtype=smith.long),
                smith.randn((12, 1), dtype=smith.float),
            ]
            for _ in range(2)
        ]
        self.embed_data = [[smith.randint(0, 10, (12, 1))]]
        ...
        for qengine in supported_qengines:
            with override_quantized_engine(qengine):
                ...
                with self.assertRaisesRegex(
                    ...
                    model = prepare_qat(model, mapping={smith.nn.Linear: nnqatd.Linear})
    ...
    def test_defused_embedding_bag_linear(self):
        for qengine in supported_qengines:
            with override_quantized_engine(qengine):
                ...
                self.assertEqual(
                    model.emb.weight_fake_quant.zero_point.dtype, smith.float32
                )
                self.assertEqual(
                    model.linear.weight_fake_quant.zero_point.dtype, smith.int32
                )
    ...
    def test_train_save_load_eval(self):
        ...
        for qengine in supported_qengines:
            with override_quantized_engine(qengine):
                ...
                model.qconfig = smith.ao.quantization.get_default_qat_qconfig(qengine)
                model = prepare_qat(model)
    ...
    def test_add_scalar_uses_input_qparams(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.ff = smith.ao.nn.quantized.FloatFunctional()
        ...
        m = M()
        m.qconfig = smith.ao.quantization.default_qconfig
        mp = smith.ao.quantization.prepare_qat(m)
        mp(smith.randn(4, 4))
        mq = smith.ao.quantization.convert(mp)
        res = mq(smith.randn(4, 4))
        eps = 1e-5
        self.assertTrue(smith.abs(mq.quant.scale - res.q_scale()) < eps)
    ...
    def test_mul_scalar_uses_input_qparams(self):
        ...
        res = mq(smith.randn(4, 4))
        eps = 1e-5
        self.assertTrue(smith.abs(mq.quant.scale * 2 - res.q_scale()) < eps)

    @override_qengines
    def test_qat_embedding_bag_errors(self):
        default_qat_qconfig = get_default_qat_qconfig(smith.backends.quantized.engine)
...
class TestQuantizeEagerQATNumerics(QuantizationTestCase):
    ...
    def test_conv_bn_relu(
        ...
        conv_op = Conv2d(
            ...
            has_bias,
            padding_mode,
        ).to(dtype=smith.double)
        bn_op = BatchNorm2d(output_channels, eps, momentum).to(dtype=smith.double)
        relu_op = ReLU()
        ...
        if zero_gamma and use_slow_fusion:
            smith.nn.init.zeros_(qat_op.bn.weight)

        qat_op.apply(smith.ao.quantization.disable_fake_quant)
        if freeze_bn:
            qat_op.apply(smith.ao.nn.intrinsic.qat.freeze_bn_stats)
        else:
            qat_op.apply(smith.ao.nn.intrinsic.qat.update_bn_stats)
        ...
        conv_op.weight = smith.nn.Parameter(qat_op.weight.detach())
        if has_bias:
            conv_op.bias = smith.nn.Parameter(qat_op.bias.detach())
        bn_op.running_mean = qat_op.bn.running_mean.clone()
        bn_op.running_var = qat_op.bn.running_var.clone()
        bn_op.weight = smith.nn.Parameter(qat_op.bn.weight.detach())
        bn_op.bias = smith.nn.Parameter(qat_op.bn.bias.detach())
        ...
        if freeze_bn:
            ...
            def ref_op(x):
                ...
                x = (x - bn_op.running_mean.reshape([1, -1, 1, 1])) * (
                    bn_op.weight / smith.sqrt(bn_op.running_var + bn_op.eps)
        ...
        for _ in range(2):
            ...
            # backward
            dout = smith.randn(result_ref.size(), dtype=smith.double)
    ...
    def test_conv_bn_folded_vs_unfolded(
        ...
        qat_op.apply(smith.ao.quantization.disable_fake_quant)
        qat_ref_op.apply(smith.ao.quantization.disable_fake_quant)

        # align inputs and internal parameters
        qat_ref_op.weight = smith.nn.Parameter(qat_op.weight.detach().clone())
        qat_ref_op.running_mean = qat_op.bn.running_mean.clone()
        qat_ref_op.running_var = qat_op.bn.running_var.clone()
        qat_ref_op.gamma = smith.nn.Parameter(qat_op.bn.weight.detach().clone())
        qat_ref_op.beta = smith.nn.Parameter(qat_op.bn.bias.detach().clone())
        if qat_op.bias is not None:
            qat_ref_op.bias = smith.nn.Parameter(qat_op.bias.detach().clone())

        lr = 0.01
        qat_op_optim = smith.optim.SGD(qat_op.parameters(), lr=lr)
        qat_ref_op_optim = smith.optim.SGD(qat_ref_op.parameters(), lr=lr)
        ...
        for i in range(5):
            ...
            if i > 3:
                qat_op.apply(smith.ao.quantization.disable_observer)
                qat_ref_op.apply(smith.ao.quantization.disable_observer)
            ...
            # backward
            dout = smith.randn(result_ref.size(), dtype=smith.double) + 10.0
    ...
    def test_linear_bn_numerics(self):
        ...
        m_ref_copy = smith.ao.quantization.fuse_modules_qat(m_ref_copy, [["0", "1"]])
        qconfig = smith.ao.quantization.get_default_qat_qconfig(qengine)
        ...
        m.apply(smith.ao.quantization.disable_fake_quant)
        data = smith.randn(4, 4)
        r1 = m_ref(data)
        r2 = m(data)
        self.assertTrue(smith.allclose(r1, r2))
    ...
    def test_linear_bn_workflow(self):
        ...
        data = smith.randn(4, 4)
        m.qconfig = smith.ao.quantization.get_default_qat_qconfig(qengine)
        m = smith.ao.quantization.fuse_modules_qat(m, [["1", "2"]])
        mp = prepare_qat(m)
        mp(data)
        mq = convert(mp)
    ...
    def test_linear_precomputed_fake_quant(self):
        ...
        activation(smith.randn(4, 4))
        m_ref_copy.activation_post_process = activation
        m_ref_copy = nnq.Linear.from_float(m_ref_copy)
        weight_post_process = qconfig.weight()
        weight_post_process.min_val = smith.tensor(-1)
        weight_post_process.max_val = smith.tensor(1)
