...
import smith
import smith.ao.quantization._equalize as _equalize
import smith.nn as nn
from smith.ao.quantization.fuse_modules import fuse_modules
...
class TestEqualizeEager(QuantizationTestCase):
    ...
    def test_converged(self):
        ...
        module1.weight = nn.parameter.Parameter(smith.ones(module1.weight.size()))
        module2.weight = nn.parameter.Parameter(smith.zeros(module1.weight.size()))
    ...
    def test_equalize_fused_convrelu(self):
        ...
        class M(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv1 = nn.Conv2d(3, 3, 1).to(dtype=smith.float)
                self.relu1 = nn.ReLU(inplace=False).to(dtype=smith.float)
                self.conv2 = nn.Conv2d(3, 3, 1).to(dtype=smith.float)
                self.relu2 = nn.ReLU(inplace=False).to(dtype=smith.float)
                self.conv3 = nn.Conv2d(3, 3, 1).to(dtype=smith.float)
                self.relu3 = nn.ReLU(inplace=False).to(dtype=smith.float)
