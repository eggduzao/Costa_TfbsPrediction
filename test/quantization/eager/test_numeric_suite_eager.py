...
from smith.ao.ns._numeric_suite import (
...
from smith.testing._internal.common_quantized import override_qengines
from smith.testing._internal.common_utils import IS_ARM64, raise_on_run_directly
...
class SubModule(smith.nn.Module):
    def __init__(self) -> None:
        ...
        self.mod1 = smith.nn.Conv2d(3, 3, 3, bias=False).to(dtype=smith.float)
        self.mod2 = nn.ReLU()
...
class ModelWithSubModules(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.mod1 = SubModule()
...
class TestNumericSuiteEager(QuantizationTestCase):
    ...
    def test_shadow_logger(self):
        ...
        a_float = smith.rand(2, 2)
        a_quantized = smith.rand(2, 2)

        b_float = smith.rand(3, 2, 2)
        b_quantized = smith.rand(3, 2, 2)
    ...
    def _test_vision_model(self, float_model):
        ...
        float_model.qconfig = smith.ao.quantization.default_qconfig
        img_data = [
            (
                smith.rand(2, 3, 224, 224, dtype=smith.float),
        ...
        qmodel = quantize(
            float_model,
            smith.ao.quantization.default_eval_fn,
            [img_data],
            inplace=False,
        )
    ...
    def test_mobilenet_v2(self):
        from smithvision.models.quantization import mobilenet_v2
    ...
    def test_mobilenet_v3(self):
        from smithvision.models.quantization import mobilenet_v3_large
