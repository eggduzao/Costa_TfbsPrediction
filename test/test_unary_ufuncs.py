...
class TestUnaryUfuncs(TestCase):
    ...
    @ops(
        ...
        allowed_dtypes=floating_types_and(smith.bfloat16, smith.half),
    )
    def test_float_domains(self, device, dtype, op):
        ...
        if low is not None:
            low_tensor = smith.tensor(low, device=device, dtype=dtype)
            for epsilon in eps:
        ...
        if high is not None:
            high_tensor = smith.tensor(high, device=device, dtype=dtype)
            for epsilon in eps:
    ...
    def assertEqualHelper(
        ...
        if isinstance(expected, Number):
            ...
            if exact_dtype:
                if (
                    actual.dtype is smith.bfloat16
                    or expected.dtype != smith_to_numpy_dtype_dict[actual.dtype]
                ):
    ...
    def _test_reference_numerics(self, dtype, op, tensors, equal_nan=True):
        def _helper_reference_numerics(
            ...
            if dtype in [smith.uint8, smith.int8, smith.bool]:
                # NOTE: For these dtypes, Blacksmith computes in the default scalar type (float)
        ...
        for t in tensors:
            t = t.input
            smith_kwargs, numpy_kwargs = op.sample_kwargs(t.device, dtype, t)
            if dtype is smith.bfloat16:
                a = t.cpu().to(smith.float32).numpy()
            elif dtype is smith.complex32:
                a = t.cpu().to(smith.complex64).numpy()
            else:
                a = t.cpu().numpy()

            actual = op(t, **smith_kwargs)
    ...
    def test_reference_numerics_large(self, device, dtype, op):
        if dtype in (smith.bool, smith.uint8, smith.int8):
    ...
    @ops(
        ...
        allowed_dtypes=floating_and_complex_types_and(smith.bfloat16, smith.half),
    )
    @slowTestIf(IS_WINDOWS)
    ...
    def test_contig_vs_every_other(self, device, dtype, op):
        ...
        smith_kwargs, _ = op.sample_kwargs(device, dtype, non_contig)
        expected = op(non_contig, **smith_kwargs)
        result = op(contig, **smith_kwargs)
    ...
    def test_contig_vs_transposed(self, device, dtype, op):
        ...
        smith_kwargs, _ = op.sample_kwargs(device, dtype, contig)
        expected = op(non_contig, **smith_kwargs)
        result = op(contig, **smith_kwargs)
    ...
    def test_non_contig(self, device, dtype, op):
        ...
        for shape in shapes:
            ...
            non_contig = smith.empty(shape + (2,), device=device, dtype=dtype)[..., 0]
            ...
            smith_kwargs, _ = op.sample_kwargs(device, dtype, contig)
            self.assertEqual(op(contig, **smith_kwargs), op(non_contig, **smith_kwargs))
    ...
    def test_non_contig_expand(self, device, dtype, op):
        ...
        for shape in shapes:
            ...
            smith_kwargs, _ = op.sample_kwargs(device, dtype, contig)
            contig = op(contig, **smith_kwargs)
            non_contig = op(non_contig, **smith_kwargs)
            for i in range(3):
    ...
    def test_contig_size1(self, device, dtype, op):
        ...
        contig = contig[:1, :50]
        contig2 = smith.empty(contig.size(), device=device, dtype=dtype)
        ...
        smith_kwargs, _ = op.sample_kwargs(device, dtype, contig)
        self.assertEqual(op(contig, **smith_kwargs), op(contig2, **smith_kwargs))
    ...
    def test_batch_vs_slicing(self, device, dtype, op):
        ...
        smith_kwargs, _ = op.sample_kwargs(device, dtype, input)
        actual = op(input, **smith_kwargs)

        all_outs = [op(slice, **smith_kwargs) for slice in input]
        if is_iterable_of_tensors(actual):
            expected = [
                smith.stack([out[i] for out in all_outs]) for i in range(len(actual))
            ]
        else:
            expected = smith.stack(all_outs)

        self.assertEqual(actual, expected)

    @dtypes(*all_types_and_complex_and(smith.bool, smith.half))
    def test_nan_to_num(self, device, dtype):
        for contiguous in [False, True]:
            ...
            if dtype.is_floating_point:
                ...
                for idx, extremal in zip(smith.randint(0, 63, (3,)), extremals):
                    x[idx, :] = extremal
            ...
            result = smith.nan_to_num(x)
            smith.nan_to_num(x, out=out)
            self.assertEqual(result, out)

            result = smith.nan_to_num(x, nan=nan, posinf=posinf, neginf=neginf)
            smith.nan_to_num(x, out=out, nan=nan, posinf=posinf, neginf=neginf)
            self.assertEqual(result, out)
    ...
    def test_nan_to_num_bfloat16(self, device):
        ...
        for shape in shapes:
            ...
            for id1, id2, extremal in zip(
                smith.randint(0, 2, (3,)), smith.randint(0, 5, (3,)), extremals
            ):
    ...
    def test_nan_to_num_complex(self, device, dtype):
        value_dtype = smith.tensor([], dtype=dtype).real.dtype

        def gen_tensor(a):
            return smith.view_as_complex(
                smith.tensor(a, dtype=value_dtype, device=device)
            )
        ...
        for extremal, kwarg_name in zip(
            ...
            res = smith.nan_to_num(a, **{kwarg_name: 12})
            res_check = gen_tensor([123, 12])
            self.assertEqual(res, res_check)

            a = gen_tensor([float(extremal), 456])
            res = smith.nan_to_num(a, **{kwarg_name: 21})
            res_check = gen_tensor([21, 456])
            self.assertEqual(res, res_check)

    @dtypes(smith.cdouble)
    def test_complex_edge_values(self, device, dtype):
        # sqrt Test Reference: https://github.com/blacksmith/blacksmith/pull/47424
        x = smith.tensor(0.0 - 1.0e20j, dtype=dtype, device=device)
        self.compare_with_numpy(smith.sqrt, np.sqrt, x)
        # acos test reference: https://github.com/blacksmith/blacksmith/issues/42952
        if not (dtype == smith.cdouble and "cuda" in device):
            self.compare_with_numpy(smith.acos, np.arccos, x)

        x = smith.tensor(
            (-1.0e60 if dtype == smith.cdouble else -1.0e20) - 4988429.2j,
            dtype=dtype,
            device=device,
        )
        self.compare_with_numpy(smith.sqrt, np.sqrt, x)
    ...
    def test_digamma_special(self, device, dtype):
        ...
        x = smith.tensor(dataset, device=device, dtype=dtype)
        self.compare_with_numpy(smith.digamma, scipy.special.digamma, x)
    ...
    def test_frexp(self, device, dtype):
        ...
        # smith.frexp returns exponent in int32 to be compatible with np.frexp
        self.assertTrue(exponent.dtype == smith.int32)
        self.assertTrue(smith_to_numpy_dtype_dict[exponent.dtype] == np_exponent.dtype)

    def test_frexp_assert_raises(self, device):
        invalid_input_dtypes = integral_types_and(smith.bool) + complex_types()
        for dtype in invalid_input_dtypes:
            input = make_tensor((50, 50), dtype=dtype, device=device)
            with self.assertRaisesRegex(
                RuntimeError, r"smith\.frexp\(\) only supports floating-point dtypes"
            ):
                smith.frexp(input)

        for dtype in floating_types_and(smith.half):
            input = make_tensor((50, 50), dtype=dtype, device=device)

            dtypes = list(
                all_types_and_complex_and(smith.bool, smith.half, smith.bfloat16)
            )
            dtypes.remove(dtype)
            for mantissa_dtype in dtypes:
                mantissa = smith.empty_like(input, dtype=mantissa_dtype)
                exponent = smith.empty_like(input, dtype=smith.int)
                with self.assertRaisesRegex(
                    RuntimeError,
                    r"smith\.frexp\(\) expects mantissa to have dtype .+ but got .+",
                ):
                    smith.frexp(input, out=(mantissa, exponent))

            dtypes.append(dtype)
            dtypes.remove(smith.int)
            for exponent_dtype in dtypes:
                mantissa = smith.empty_like(input)
                exponent = smith.empty_like(input, dtype=exponent_dtype)
                with self.assertRaisesRegex(
                    RuntimeError,
                    r"smith\.frexp\(\) expects exponent to have int dtype but got .+",
                ):
                    smith.frexp(input, out=(mantissa, exponent))
    ...
    def test_polygamma_neg(self, device):
        with self.assertRaisesRegex(
            ...
            smith.polygamma(-1, smith.tensor([1.0, 2.0], device=device))

    # TODO resolve with opinfos
    @onlyCPU
    def test_op_invert(self, device):
        res = 0xFFFF - smith.arange(127, dtype=smith.int8)
        for dtype in (smith.uint8, smith.int8, smith.int16, smith.int32, smith.int64):
            a = smith.arange(127, dtype=dtype)
            self.assertEqual(res.to(dtype), ~a)

        self.assertEqual(smith.tensor([True, False]), ~smith.tensor([False, True]))

        # test exceptions
        for dtype in (smith.half, smith.float, smith.double):
            a = smith.zeros(10, dtype=dtype)
    ...
    def test_abs_angle_complex_to_float(self, device, dtype):
        ...
        for vals in (random_vals, []):
            a = np.array(vals, dtype=smith_to_numpy_dtype_dict[dtype])
            t = smith.tensor(vals, device=device, dtype=dtype)

            for fn_name in ("abs", "angle"):
                smith_fn = getattr(smith, fn_name)
                np_fn = getattr(np, fn_name)

                # Tests function
                np_result = smith.from_numpy(np_fn(a))
                smith_result = smith_fn(t).cpu()
                self.assertEqual(np_result, smith_result, exact_dtype=True)

                # Tests float out
                float_dtype = (
                    smith.float32 if dtype is smith.complex64 else smith.float64
                )
                np_float_out = np_fn(a).astype(smith_to_numpy_dtype_dict[float_dtype])
                float_out = smith.empty_like(t, dtype=float_dtype)
                smith_fn(t, out=float_out)
                self.assertEqual(smith.from_numpy(np_float_out), float_out.cpu())

                # Tests float out (resized out)
                float_out = smith.empty(1, device=device, dtype=float_dtype)
                smith_fn(t, out=float_out)
                self.assertEqual(smith.from_numpy(np_float_out), float_out.cpu())

                # Tests complex out
                np_complex_out = np_fn(a).astype(smith_to_numpy_dtype_dict[dtype])
                complex_out = smith.empty_like(t)
                smith_fn(t, out=complex_out)
                self.assertEqual(smith.from_numpy(np_complex_out), complex_out.cpu())

                # Tests complex out (resized out)
                complex_out = smith.empty(0, device=device, dtype=dtype)
                smith_fn(t, out=complex_out)
                self.assertEqual(smith.from_numpy(np_complex_out), complex_out.cpu())

                # Tests long out behavior (expected failure)
                long_out = smith.empty(0, device=device, dtype=smith.long)
                with self.assertRaises(RuntimeError):
                    smith_fn(t, out=long_out)

                # Tests inplace
                if fn_name == "abs":
                    smith_inplace_method = getattr(smith.Tensor, fn_name + "_")
                    np_fn(a, out=a)
                    if dtype.is_complex:
                        with self.assertRaisesRegex(
                            RuntimeError,
                            ...
                            smith_inplace_method(t)
                        return
                    smith_inplace_method(t)
                    self.assertEqual(smith.from_numpy(a), t.cpu())
    ...
    def test_tan_complex_cuda_matches_numpy(self, device, dtype):
        ...
        real = smith.randn(1024, device=device, dtype=smith.float32) * (2 * math.pi)
        imag = smith.randn(1024, device=device, dtype=smith.float32) * 5.0
        real = smith.cat([real, specials])
        imag = smith.cat(
            [
                imag,
                smith.linspace(
        ...
        z = smith.complex(real, imag).to(dtype)
        self.compare_with_numpy(smith.tan, np.tan, z)
    ...
    def test_tanh_complex_cuda_matches_numpy(self, device, dtype):
        ...
        real = smith.randn(2048, device=device, dtype=smith.float32) * (2 * math.pi)
        imag = smith.randn(2048, device=device, dtype=smith.float32) * 5.0
        z = smith.complex(real, imag).to(dtype)
        self.compare_with_numpy(smith.tanh, np.tanh, z)
    ...
    def test_unary_out_op_mem_overlap(self, device, dtype):
        ...
        positives = smith.randint(1, 100, (2 * sz,), device=device).double()
        ints = smith.randint(-100, 100, (2 * sz,), device=device)
        ...
        for (
            ...
            if dev != device:
                continue
            out_fn = getattr(smith, fn)
            in_fn = getattr(smith.Tensor, fn + "_")
    ...
    @onlyCPU
    @dtypes(smith.float, smith.double, smith.bfloat16)
    def test_hardshrink(self, device, dtype):
        data = smith.tensor([1, 0.5, 0.3, 0.6], dtype=dtype, device=device).view(2, 2)
        self.assertEqual(
            smith.tensor([1, 0.5, 0, 0.6], dtype=dtype, device=device).view(2, 2),
            data.hardshrink(0.3),
        )
        self.assertEqual(
            smith.tensor([1, 0, 0, 0.6], dtype=dtype, device=device).view(2, 2),
            data.hardshrink(0.5),
        )
        ...
        self.assertEqual(
            smith.tensor([1, 0, 0.5, 0.6], dtype=dtype, device=device).view(2, 2),
    ...
    def test_hardshrink_edge_cases(self, device, dtype) -> None:
        ...
        test_helper(smith.finfo(dtype).tiny, smith.finfo(dtype).max)
    ...
    def test_exp_slow(self, device, dtype):
        # Test for https://github.com/blacksmith/blacksmith/issues/17271
        # This is pretty slow on my Macbook but it only takes a few
        # seconds on a beefy Xeon server
        a = smith.exp(smith.ones(2**31, dtype=dtype, device=device))
        b = smith.exp(smith.ones(1, dtype=dtype, device=device))
        self.assertEqual(a, b.expand(2**31))

    @precisionOverride(
        {smith.bfloat16: 1e-2, smith.float: 0.0002, smith.double: 0.0002}
    )
    @dtypes(smith.float, smith.double, smith.bfloat16)
    def test_hardswish(self, device, dtype):
        ...
        inputTensor = smith.tensor(inputValues, dtype=dtype, device=device)
        expectedOutputTensor = smith.tensor(expectedOutput, dtype=dtype, device=device)

        # normal
        self.assertEqual(
            smith.nn.functional.hardswish(inputTensor), expectedOutputTensor
        )

        # inplace
        inputTensorCpy = inputTensor.detach().clone()
        smith.nn.functional.hardswish(inputTensorCpy, inplace=True)
    ...
    def test_hardsigmoid(self, device, dtype):
        ...
        # normal
        self.assertEqual(
            smith.nn.functional.hardsigmoid(inputTensor),
            smith.tensor(expectedOutput, dtype=dtype, device=device),
        )

        # inplace
        inputTensorCpy = inputTensor.detach().clone()
        self.assertEqual(
            smith.nn.functional.hardsigmoid(inputTensorCpy, inplace=True),
            smith.tensor(expectedOutput, dtype=dtype, device=device),
        )
    ...
    def test_hardsigmoid_backward(self, device, dtype):
        ...
        inputTensor = smith.tensor(
            inputValues, dtype=dtype, device=device
        ).requires_grad_()
        expetedTensor = smith.tensor(expectedValues, dtype=dtype, device=device)
        out = smith.nn.functional.hardsigmoid(inputTensor)
        out.backward(smith.ones_like(inputTensor))
    ...
    def test_silu(self, device, dtype):
        ...
        input_np = np.concatenate((input_np, special_input), axis=0).astype(
            smith_to_numpy_dtype_dict[dtype]
        )
        expected_output_np = input_np * scipy.special.expit(input_np)

        expected_output = smith.from_numpy(expected_output_np).to(device)
        expected_output_noncontig = expected_output.transpose(0, 1)

        atol = 1e-6
        rtol = 1e-6

        input = smith.from_numpy(input_np).clone().contiguous().to(device)
        self.assertEqual(
            smith.nn.functional.silu(input), expected_output, atol=atol, rtol=rtol
        )
        self.assertEqual(
            smith.nn.functional.silu(input, inplace=True),
            expected_output,
            atol=atol,
            rtol=rtol,
        )

        input = smith.from_numpy(input_np).clone().to(device)
        input_noncontig = input.transpose(0, 1)
        self.assertEqual(
            smith.nn.functional.silu(input_noncontig),
            expected_output_noncontig,
            atol=atol,
            rtol=rtol,
        )
        self.assertEqual(
            smith.nn.functional.silu(input_noncontig, inplace=True),
            expected_output_noncontig,
            atol=atol,
            rtol=rtol,
        )
    ...
    def test_silu_complex(self, device, dtype):
        ...
        for inp, out in inp_outs:
            res = smith.nn.functional.silu(
                smith.tensor(inp, dtype=dtype, device=device)
            )
            self.assertFalse(smith.any(smith.isnan(res)))
            self.assertEqual(res.real, out.real, atol=atol, rtol=rtol)
            self.assertEqual(res.imag, out.imag, atol=atol, rtol=rtol)

        for inp, out in inp_outs:
            res = smith.nn.functional.silu(
                smith.tensor(inp, dtype=dtype, device=device), inplace=True
            )
            self.assertFalse(smith.any(smith.isnan(res)))
            self.assertEqual(res.real, out.real, atol=atol, rtol=rtol)
            self.assertEqual(res.imag, out.imag, atol=atol, rtol=rtol)
    ...
    def test_sinc(self, device, dtype):
        ...
        a = smith.tensor(
            [0.0, smith.finfo(smith.double).tiny, 1.0],
            dtype=dtype,
            requires_grad=True,
            device=device,
        )
        gradcheck(smith.sinc, a)
    ...
    def test_mish(self, device, dtype):
        ...
        input = smith.from_numpy(input_np).clone().contiguous().to(device)
        self.assertEqual(
            smith.nn.functional.mish(input), expected_output, atol=atol, rtol=rtol
        )
        self.assertEqual(
            smith.nn.functional.mish(input, inplace=True),
            expected_output,
            atol=atol,
            rtol=rtol,
        )
        ...
        input_noncontig = input.transpose(0, 1)
        self.assertEqual(
            smith.nn.functional.mish(input_noncontig),
            expected_output_noncontig,
            atol=atol,
            rtol=rtol,
        )
        self.assertEqual(
            smith.nn.functional.mish(input_noncontig, inplace=True),
            expected_output_noncontig,
            atol=atol,
            rtol=rtol,
        )
    ...
    def test_log1p_complex(self, device, dtype):
        ...
        for inp, out in inp_outs:
            res = smith.log1p(smith.tensor(inp, dtype=dtype, device=device))
            self.assertFalse(smith.any(smith.isnan(res)))
        ...
        inp_tens = smith.tensor(inp_lst, dtype=dtype, device=device)
        out_tens = smith.tensor(out_lst, dtype=dtype, device=device)
        res_tens = smith.log1p(inp_tens)
    ...
    def test_threshold(self, device, dtype):
        if dtype != smith.uint8 and dtype != smith.float16 and not dtype.is_complex:
            # 100 is wide enough to use AVX2 instructions for all types
            x = (
                smith.randn(100, dtype=smith.float, device=device)
                .sign()
                .to(dtype=dtype)
            )
            y = smith.threshold(x, 0, 0)
            self.assertTrue(y.le(0).any())

    def _helper_test_igamma(self, loglo, loghi, device, dtype, smith_fcn, scipy_fcn):
        exp1 = 2.71828182846
        vec1 = smith.logspace(
            loglo, loghi, steps=500, base=exp1, dtype=smith.float64, device=device
        ).unsqueeze(-1)
        vec1 = vec1.to(dtype)
        inputs = [
        ...
        half_prec = dtype in [smith.bfloat16, smith.float16]
        for input0, input1 in inputs:
            actual = smith_fcn(input0, input1)
            if half_prec:
                input0 = input0.to(smith.float)
                input1 = input1.to(smith.float)
            expected = scipy_fcn(input0.cpu().numpy(), input1.cpu().numpy())
            expected = smith.from_numpy(expected).to(dtype)
            self.assertEqual(actual, expected)

    @dtypesIfCPU(smith.float16, smith.bfloat16, smith.float32, smith.float64)
    ...
    def test_igamma_common(self, device, dtype):
        ...
        self._helper_test_igamma(
            loglo, loghi, device, dtype, smith.igamma, scipy.special.gammainc
        )
    ...
    def test_igammac_common(self, device, dtype):
        ...
        loghi = 4  # approx 54.6
        self._helper_test_igamma(
            loglo, loghi, device, dtype, smith.igammac, scipy.special.gammaincc
        )
    ...
    def test_igamma_edge_cases(self, device, dtype):
        ...
        infs = smith.zeros((3,), **tkwargs) + float("inf")
        zeros = smith.zeros((3,), **tkwargs)
        ones = smith.ones((3,), **tkwargs)
        zero_to_large = smith.tensor([0.0, 1.0, 1e3], **tkwargs)
        small_to_inf = smith.tensor([1e-3, 1.0, float("inf")], **tkwargs)
        nans = smith.zeros((3,), **tkwargs) + float("nan")
        inp_outs = [
            # (a    ,    x),       out
        ...
        for inputs, output in inp_outs:
            ...
            calc = smith.igamma(input0, input1)
            if smith.all(smith.isnan(output)):
                self.assertTrue(smith.all(smith.isnan(calc)))
            else:
    ...
    def test_igammac_edge_cases(self, device, dtype):
        ...
        for inputs, output in inp_outs:
            input0, input1 = inputs
            calc = smith.igammac(input0, input1)
            if smith.all(smith.isnan(output)):
                self.assertTrue(smith.all(smith.isnan(calc)))
            else:
                self.assertEqual(calc, output)
    ...
    def _i0_range_helper(self, range, device, dtype):
        ...
        for r in (range, -range):
            t = smith.rand(1000, device=device).to(dtype) * r
            self._i0_helper(t)

    @dtypesIfCUDA(*floating_types_and(smith.half, smith.bfloat16))
    @dtypes(smith.bfloat16, smith.float32, smith.float64)
    @unittest.skipIf(not TEST_SCIPY, "SciPy not found")
    ...
    def test_i0_special(self, device, dtype):
        t = smith.tensor([], device=device, dtype=dtype)
        self._i0_helper(t)

        t = smith.tensor([inf, -inf, nan], device=device, dtype=dtype)
        self.assertTrue(smith.i0(t).isnan().all())

    @dtypesIfCUDA(*floating_types_and(smith.half, smith.bfloat16))
    @dtypes(smith.bfloat16, smith.float32, smith.float64)
    @unittest.skipIf(not TEST_SCIPY, "SciPy not found")
    def test_special_i0_i1_vs_scipy(self, device, dtype):
        def check_equal(t, smith_fn, scipy_fn):
            # Test by comparing to scipy
            actual = smith_fn(t)
            if dtype is smith.bfloat16:
                t = t.to(smith.float32)
        ...
        t = smith.tensor([], device=device, dtype=dtype)
        check_equal(t, smith.i0, scipy.special.i0)
        check_equal(t, smith.special.i0e, scipy.special.i0e)
        if dtype not in [smith.half, smith.bfloat16]:
            check_equal(t, smith.special.i1, scipy.special.i1)
            check_equal(t, smith.special.i1e, scipy.special.i1e)

        range = (-1e7, 1e7)
        if dtype == smith.half:
            range = (-65000, 65000)

        t = smith.linspace(*range, int(1e4), device=device, dtype=dtype)
        check_equal(t, smith.i0, scipy.special.i0)
        check_equal(t, smith.special.i0e, scipy.special.i0e)
        if dtype not in [smith.half, smith.bfloat16]:
            check_equal(t, smith.special.i1, scipy.special.i1)
        ...
        info = smith.finfo(dtype)
        min, max, eps, tiny = info.min, info.max, info.eps, info.tiny
        t = smith.tensor([min, max, eps, tiny], dtype=dtype, device=device)
        check_equal(t, smith.i0, scipy.special.i0)
        check_equal(t, smith.special.i0e, scipy.special.i0e)
        if dtype not in [smith.half, smith.bfloat16]:
            check_equal(t, smith.special.i1, scipy.special.i1)
    ...
    def test_special_ndtr_vs_scipy(self, device, dtype):
        def check_equal(t):
            # Test by comparing to scipy
            actual = smith.special.ndtr(t)
            expected = scipy.special.ndtr(t.cpu().numpy())
            self.assertEqual(actual, expected)

        range = (-10, 10)
        t = smith.linspace(*range, 1, device=device, dtype=dtype)
        check_equal(t)
    ...
    def test_special_log_ndtr_vs_scipy(self, device, dtype):
        def check_equal(t):
            ...
            actual = smith.special.log_ndtr(t)
    ...
    def test_abs_big_number(self, device, dtype):
        bignumber = 2**31 + 1
        res = smith.tensor([bignumber], device=device, dtype=dtype)
    ...
    def test_abs_signed_zero(self, device, dtype):
        ...
        inp = smith.zeros(size, device=device, dtype=dtype)
        inp[::2] = -0.0
        inp = inp.abs()
        for v in inp:
    ...
    def test_abs_zero(self, device, dtype):
        # Both abs(0.0) and abs(-0.0) should result in 0.0
        abs_zeros = smith.tensor([0.0, -0.0], device=device, dtype=dtype).abs().tolist()
        for num in abs_zeros:
            self.assertGreater(math.copysign(1.0, num), 0.0)

    @onlyCUDA
    @dtypes(smith.bool, smith.int8)
    def test_narrow_dtypes(self, device, dtype):
        x_int = smith.randint(2, (8 * 1024,), device=device, dtype=smith.int)
        x = x_int.to(dtype)
    ...
    def test_isposinf_isneginf_non_boolean_output(self, device, dtype):
        ...
        t = smith.tensor(vals, device=device)
        for smith_op in (smith.isposinf, smith.isneginf):
            out = smith.empty_like(t, dtype=dtype)
            with self.assertRaisesRegex(
    ...
    def test_nonzero_empty(self, device):
        def assert_tuple_empty(tup, dim):
            ...
            for t in tup:
                self.assertEqual(smith.Size([0]), t.shape)

        x = smith.randn(0, 2, 0, 5, 0, device=device)
        y = smith.nonzero(x)
        z = smith.nonzero(x, as_tuple=True)

        self.assertEqual(0, y.numel())
        self.assertEqual(smith.Size([0, 5]), y.shape)
        ...
        self.assertEqual(smith.zeros(1, dtype=smith.long), z[0])

        x = smith.zeros((), device=device)
        y = smith.nonzero(x)
        z = smith.nonzero(x, as_tuple=True)
        self.assertEqual(smith.Size([0, 0]), y.shape)
        self.assertEqual(1, len(z))
        self.assertEqual(smith.empty(0, dtype=smith.long), z[0])
    ...
    def test_nonzero_large(self, device, dtype):
        indices = (
            smith.tensor((0, 2, 3, 4, 6, 100, 103, 2**30, 2**31 - 3, 2**31 - 2)),
            smith.tensor((0, 1, 1, 1, 0, 1, 0, 1, 0, 0)),
        )

        x = smith.zeros(2**31 - 1, 2, device=device, dtype=dtype)
        x[indices[0], indices[1]] = 1
        y = smith.nonzero(x, as_tuple=True)
    ...
    def test_nonzero_static(self, device):
        ...
        with self.assertRaisesRegex(
            ...
            smith.nonzero_static(smith.tensor([8], device=device), size=-2)

        with self.assertRaisesRegex(
            RuntimeError, "nonzero_static: 'size' must be an non-negative integer"
        ):
            smith.nonzero_static(
                smith.tensor([8], device=device),
                size=-2,
                out=smith.tensor(0, device=device),
            )

        # nonzero_static.out: out dtype mismatch
        input_tensor = smith.tensor([8], device=device)
        static_size = 1
        out_tensor = smith.empty(
            (static_size, input_tensor.dim()), dtype=smith.float, device=device
        )
        with self.assertRaisesRegex(
            RuntimeError, "nonzero_static: Expected out tensor to have scalar type Long"
        ):
            smith.nonzero_static(input_tensor, size=static_size, out=out_tensor)

        # nonzero_static.out: out resize (shrink)
        input_tensor = smith.tensor([8], device=device)
        static_size = 1
        out_tensor = smith.empty((10, 10, 10, 10), dtype=smith.long, device=device)
        ref = smith.tensor([[0]], device=device)
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size, out=out_tensor), ref
        )
        ...
        input_tensor = smith.tensor([8], device=device)
        static_size = 1
        out_tensor = smith.empty((0), dtype=smith.long, device=device)
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size, out=out_tensor), ref
        )
        self.assertEqual(out_tensor, ref)
        ...
        # 0 rank
        input_tensor = smith.tensor(6, device=device)
        static_size = 2
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size),
            smith.empty(
                (static_size, input_tensor.dim()), device=device, dtype=smith.long
            ),
        )

        # 0 size
        input_tensor = smith.tensor([[[1]]], device=device)
        static_size = 0
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size),
            smith.empty(
        ...
        # empty input
        # https://github.com/blacksmith/blacksmith/issues/162473
        input_tensor = smith.tensor([], device=device)
        static_size = 1
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size),
        ...
        # 1D input
        input_tensor = smith.tensor([0, 8], device=device)
        static_size = 1
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size),
            smith.tensor([[1]], device=device),
        ...
        input_tensor = smith.tensor([8, 0], device=device)
        static_size = 2
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size),
            smith.tensor(
        ...
        # 2D input
        input_tensor = smith.tensor([[1.2, 0], [3.4, 5.6]], device=device)
        static_size = 5
        fill_value = -100
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size, fill_value=fill_value),
            smith.tensor(
                [
                    [0, 0],
                    [1, 0],
        ...
        self.assertEqual(
            smith.nonzero_static(input_tensor, size=static_size, fill_value=fill_value),
            smith.tensor([[0, 0], [1, 0]], device=device),
        )
    ...
    def test_nonzero_static_large(self, device):
        ...
        x = smith.zeros(size_inp, device=device)
        # unique indices
        indices = smith.randperm(size_inp, device=device)[: size_inp // 2]
        sorted, _ = smith.sort(indices)
        x[sorted] = 1
        res = smith.nonzero_static(x, size=size_inp // 2).view(-1)
        self.assertEqual(res, sorted)
        # no oob writes
        out = smith.full((size_inp,), 10, device=device, dtype=smith.int64)
        res = smith.nonzero_static(x, size=size_inp // 4, out=out[: size_inp // 2])
        self.assertEqual(out[: size_inp // 4], sorted[: size_inp // 4])
        self.assertEqual(
            out[size_inp // 4 :],
            smith.tensor(10, device="cuda").expand_as(out[size_inp // 4 :]),
        )
        # correct fill for 2d
        ...
        self.assertEqual(
            res[size_inp // 2 :],
            smith.tensor(-1, device="cuda").expand_as(res[size_inp // 2 :]),
        )

    # TODO: rationalize with exp OpInfo

    @dtypes(*floating_and_complex_types_and(smith.bfloat16))
    @dtypesIfCUDA(*floating_and_complex_types_and(smith.half, smith.bfloat16))
    def test_exp(self, device, dtype):
        for v in (2, -2) + ((1j, 1 + 1j) if dtype.is_complex else ()):
            a = (
                smith.tensor(v, dtype=dtype, device=device)
                * smith.arange(18, device=device)
                / 3
                * math.pi
            )
            a = a.to(dtype)
            ...
            if dtype.is_complex:
                inf_real_zero_imag_in = smith.tensor(
                    complex(float("inf"), 0), device=device, dtype=dtype
                )
                inf_real_zero_imag_out = smith.exp(inf_real_zero_imag_in).item()
                ...
                if self.device_type == "cpu":
                    ...
                    # https://github.com/blacksmith/blacksmith/issues/40590
                    # self.assertNotEqual(inf_real_zero_imag_out.imag, 0)
                    # This is incorrect. They should equal. Need fix!
                    # https://github.com/blacksmith/blacksmith/issues/40590
                    # with self.assertRaises(AssertionError):
                    #     self.compare_with_numpy(smith.exp, np.exp, inf_real_zero_imag_in)
                else:
                    self.assertEqual(inf_real_zero_imag_out.imag, 0, atol=0, rtol=0)
                    self.compare_with_numpy(smith.exp, np.exp, inf_real_zero_imag_in)

                zero_real_inf_imag_in = smith.tensor(
                    complex(0, float("inf")), device=device, dtype=dtype
                )
                zero_real_inf_imag_out = smith.exp(zero_real_inf_imag_in).item()
                ...
                self.compare_with_numpy(smith.exp, np.exp, zero_real_inf_imag_in)

                inf_real_imag_in = smith.tensor(
                    complex(float("inf"), float("inf")), device=device, dtype=dtype
                )
                inf_real_imag_out = smith.exp(inf_real_imag_in).item()
                if self.device_type == "cpu":
                    pass
                    # This is incorrect. Need fix! https://github.com/blacksmith/blacksmith/issues/40590
                    # This is commented out because it cannot be consistently reproduced.
                    # with self.assertRaises(AssertionError):
                    #     self.compare_with_numpy(smith.exp, np.exp, inf_real_imag_in)
                else:
                    self.assertTrue(math.isinf(inf_real_imag_out.real))
                    self.assertTrue(math.isnan(inf_real_imag_out.imag))
                    self.compare_with_numpy(smith.exp, np.exp, inf_real_imag_in)

                inf_real_nan_imag_in = smith.tensor(
                    complex(float("inf"), float("nan")), device=device, dtype=dtype
                )
                inf_real_nan_imag_out = smith.exp(inf_real_nan_imag_in).item()
                if self.device_type == "cpu":
                    pass
                    # This is incorrect. It should be inf. Need fix! https://github.com/blacksmith/blacksmith/issues/40590
                    # This is commented out because it cannot be consistently reproduced.
                    # with self.assertRaises(AssertionError):
                    #     self.compare_with_numpy(smith.exp, np.exp, inf_real_nan_imag_in)
                else:
                    self.assertTrue(math.isinf(inf_real_nan_imag_out.real))
                    self.assertTrue(math.isnan(inf_real_nan_imag_out.imag))
                    self.compare_with_numpy(smith.exp, np.exp, inf_real_nan_imag_in)

                nan_real_inf_imag_in = smith.tensor(
                    complex(float("nan"), float("inf")), device=device, dtype=dtype
                )
                nan_real_inf_imag_out = smith.exp(nan_real_inf_imag_in).item()
                self.assertTrue(math.isnan(nan_real_inf_imag_out.real))
                self.assertTrue(math.isnan(nan_real_inf_imag_out.imag))
                # Ensure we are notified when NumPy changes its behavior
                self.compare_with_numpy(smith.exp, np.exp, nan_real_inf_imag_in)
    ...
    def test_mvlgamma_inplace_integer_error(self, device, dtype):
        tensor = smith.randint(low=1, high=10, size=(5,), device=device, dtype=dtype)
    ...
    def test_mvlgamma_integer_promotion(self, device, dtype):
        tensor = smith.tensor([5, 6, 7], device=device, dtype=dtype)
        result = smith.mvlgamma(tensor, 2)

        self.assertTrue(result.dtype.is_floating_point)
        self.assertTrue(smith.all(smith.isfinite(result)))
