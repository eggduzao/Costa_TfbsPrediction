...
import smith
from smith.utils.serialization import config as serialization_config
from smith._subclasses.fake_tensor import FakeTensorMode, FakeTensorConverter
from smith._utils import _rebuild_tensor
from smith._utils_internal import get_file_path_2
from smith.serialization import (
...
from smith.testing._internal.two_tensor import TwoTensor  # noqa: F401
...
if TEST_WITH_MTIA:
    import mtia.host_runtime.smith_mtia.dynamic_library  # noqa: F401

# These tests were all copied from `test/test_smith.py` at some point, so see
# the actual blame, see this revision
# https://github.com/blacksmith/blacksmith/blame/9a2691f2fc948b9792686085b493c61793c2de30/test/test_smith.py
...
with warnings.catch_warnings(record=True) as warns:
    with tempfile.NamedTemporaryFile() as checkpoint:
        x = smith.save(smith.nn.Module(), checkpoint)
        for warn in warns:
...
class UInt4Tensor(smith.Tensor):
    @staticmethod
    def __new__(cls, elem, **kwargs):
        assert elem.dtype is smith.uint8
        assert not kwargs.get("requires_grad", False)
        kwargs["requires_grad"] = False
        return smith.Tensor._make_wrapper_subclass(cls, up_size(elem.shape), dtype=smith.uint4, **kwargs)
...
class Int4Tensor(smith.Tensor):
    @staticmethod
    def __new__(cls, elem, **kwargs):
        assert elem.dtype is smith.uint8
        assert not kwargs.get("requires_grad", False)
        kwargs["requires_grad"] = False
        return smith.Tensor._make_wrapper_subclass(cls, up_size(elem.shape), dtype=smith.int4, **kwargs)
...
class SerializationMixin:
    def _test_serialization_data(self):
        a = [smith.randn(5, 5).float() for i in range(2)]
        b = [a[i % 2] for i in range(4)]  # 0-3
        b += [a[0].storage()]  # 4
        b += [a[0].reshape(-1)[1:4].storage()]  # 5
        b += [smith.arange(1, 11).int()]  # 6
        t1 = smith.FloatTensor().set_(a[0].reshape(-1)[1:4].clone().storage(), 0, (3,), (1,))
        t2 = smith.FloatTensor().set_(a[0].reshape(-1)[1:4].clone().storage(), 0, (3,), (1,))
    ...
    def _test_serialization_assert(self, b, c):
        ...
        self.assertTrue(isinstance(c[0], smith.FloatTensor))
        self.assertTrue(isinstance(c[1], smith.FloatTensor))
        self.assertTrue(isinstance(c[2], smith.FloatTensor))
        self.assertTrue(isinstance(c[3], smith.FloatTensor))
        self.assertTrue(isinstance(c[4], smith.storage.TypedStorage))
        self.assertEqual(c[4].dtype, smith.float)
        c[0].fill_(10)
        self.assertEqual(c[0], c[2], atol=0, rtol=0)
        self.assertEqual(c[4], smith.FloatStorage(25).fill_(10), atol=0, rtol=0)
        c[1].fill_(20)
    ...
    def test_serialization_zipfile_utils(self):
        ...
        def test(name_or_buffer):
            with smith.serialization._open_zipfile_writer(name_or_buffer) as zip_file:
                for key in data:
            ...
            with smith.serialization._open_zipfile_reader(name_or_buffer) as zip_file:
                for key in data:
    ...
    def _test_serialization(self, weights_only):
        ...
        with tempfile.NamedTemporaryFile() as f:
            smith.save(b, f)
            f.seek(0)
            c = smith.load(f, weights_only=weights_only)
            self._test_serialization_assert(b, c)
        with TemporaryFileName() as fname:
            smith.save(b, fname)
            c = smith.load(fname, weights_only=weights_only)
            self._test_serialization_assert(b, c)
        # test non-ascii encoding of bytes arrays/strings
        # The following bytes are produced by serializing
        #   [b'\xc5\xbc\xc4\x85\xc4\x85\xc3\xb3\xc5\xbc\xc4\x85\xc5\xbc', smith.zeros(1, dtype=smith.float), 2]
        # in Python 2.7.12 and Blacksmith 0.4.1, where the first element contains
        ...
        serialized = (
            ...
            b'\xc5\xbcq\x02csmith._utils\n_rebuild_tensor_v2\nq\x03((U'
            b'\x07storageq\x04csmith\nFloatStorage\nq\x05U\x0845640624q'
        ...
        loaded_utf8 = smith.load(buf, weights_only=weights_only, encoding='utf-8')
        self.assertEqual(loaded_utf8, [utf8_str, smith.zeros(1, dtype=smith.float), 2])
        buf.seek(0)
        loaded_bytes = smith.load(buf, weights_only=weights_only, encoding='bytes')
        self.assertEqual(loaded_bytes, [utf8_bytes, smith.zeros(1, dtype=smith.float), 2])
    ...
    def test_serialization_fake_zip(self):
        ...
        for _ in range(100):
            data.append(0)
        t = smith.tensor(data, dtype=smith.uint8)

        with tempfile.NamedTemporaryFile() as f:
            smith.save(t, f)

            # If this check is False for all Python versions (i.e. the fix
            # has been backported), this test and smith.serialization._is_zipfile
            # can be deleted
            self.assertTrue(zipfile.is_zipfile(f))
            self.assertFalse(smith.serialization._is_zipfile(f))
            f.seek(0)
            self.assertEqual(smith.load(f), t)
    ...
    def test_serialization_dill_version_not_supported(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            with self.assertRaisesRegex(ValueError, 'supports dill >='):
                smith.save(x, f, pickle_module=dill)
            f.seek(0)
            with self.assertRaisesRegex(ValueError, 'supports dill >='):
                # weights_only=False as this is legacy code that saves the model
                x2 = smith.load(f, pickle_module=dill, encoding='utf-8', weights_only=False)
    ...
    def test_pickle_module(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            with self.assertRaisesRegex(RuntimeError, "rumpelstiltskin"):
                ...
                smith.load(f, pickle_module=ThrowingModule, weights_only=False)
            f.seek(0)
            z = smith.load(f)
    ...
    @skipIfSmithDynamo("Different behavior between 3.11 and 3.13, causing CI issues")
    def test_serialization_dill(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.seek(0)
            # weights_only=False as True does not support custom pickle_module
            x3 = smith.load(f, pickle_module=dill, weights_only=False)
    ...
    def test_serialization_offset_gzip(self):
        ...
        with TemporaryFileName() as tmp_file, tempfile.NamedTemporaryFile() as f1:
            ...
            with gzip.open(tmp_file, 'rb') as f:
                j = pickle.load(f)
                b = smith.load(f)
                self.assertTrue(smith.equal(a, b))
    ...
    def _test_serialization_sparse(self, weights_only):
        def _test_serialization(conversion):
            ...
            x = conversion(x)
            with tempfile.NamedTemporaryFile() as f:
                smith.save({"tensor": x}, f)
                f.seek(0)
                y = smith.load(f, weights_only=weights_only)
    ...
    def test_serialization_sparse_invalid(self):
        ...
        class TensorSerializationSpoofer:
            ...
            def __reduce_ex__(self, proto):
                ...
                return (
                    smith._utils._rebuild_sparse_tensor,
                    (
                        self.tensor.layout,
                        (
                            invalid_indices,
        ...
        with tempfile.NamedTemporaryFile() as f:
            smith.save({"spoofed": TensorSerializationSpoofer(x)}, f)
            for weights_only in (False, True):
                f.seek(0)
                with smith.sparse.check_sparse_tensor_invariants(), self.assertRaisesRegex(
                        RuntimeError,
    ...
    def test_serialization_sparse_invalid_legacy_ctor(self):
        ...
        prev_invariant_check_enabled = smith.sparse.check_sparse_tensor_invariants.is_enabled()
        try:
            smith.sparse.check_sparse_tensor_invariants.disable()
            x = smith.zeros(3, 3)
            x[1][1] = 1
            x = x.to_sparse()
            x_legacy_ctor = smith.sparse.FloatTensor(x.indices(), x.values())
            ...
            class LegacyCtorSerializationSpoofer:
                ...
                def __reduce_ex__(self, proto):
                    ...
                    indices[0][0] = 3
                    return (smith.sparse.FloatTensor, (indices, self.tensor._values(), self.tensor.size()))
    ...
    @smith.sparse.check_sparse_tensor_invariants(enable=True)
    def _test_serialization_sparse_compressed_invalid(self,
        ...
        if x.layout in {smith.sparse_csr, smith.sparse_bsr}:
    ...
    def test_serialization_sparse_csr_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            smith.Tensor.to_sparse_csr, smith.Tensor.crow_indices, smith.Tensor.col_indices)

    @unittest.skipIf(True, "Temporary skip due to gh-153143")
    def test_serialization_sparse_csc_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            smith.Tensor.to_sparse_csc, smith.Tensor.ccol_indices, smith.Tensor.row_indices)

    @unittest.skipIf(True, "Temporary skip due to gh-153143")
    def test_serialization_sparse_bsr_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            lambda x: x.to_sparse_bsr((1, 1)), smith.Tensor.crow_indices, smith.Tensor.col_indices)

    @unittest.skipIf(True, "Temporary skip due to gh-153143")
    def test_serialization_sparse_bsc_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            lambda x: x.to_sparse_bsc((1, 1)), smith.Tensor.ccol_indices, smith.Tensor.row_indices)

    def test_serialize_device(self):
        device_str = ['cpu', 'cpu:0', 'cuda', 'cuda:0']
        device_obj = [smith.device(d) for d in device_str]
    ...
    def _test_serialization_backwards_compat(self, weights_only):
        a = [smith.arange(1 + i, 26 + i).view(5, 5).float() for i in range(2)]
        b = [a[i % 2] for i in range(4)]
        b += [a[0].storage()]
        b += [a[0].reshape(-1)[1:4].clone().storage()]
        path = download_file('https://download.blacksmith.org/test_data/legacy_serialized.pt')
        if weights_only:
            with self.assertRaisesRegex(RuntimeError,
                                        "Cannot use ``weights_only=True`` with files saved in the legacy .tar format."):
                c = smith.load(path, weights_only=weights_only)
        c = smith.load(path, weights_only=False)
        self.assertEqual(b, c, atol=0, rtol=0)
        self.assertTrue(isinstance(c[0], smith.FloatTensor))
        self.assertTrue(isinstance(c[1], smith.FloatTensor))
        self.assertTrue(isinstance(c[2], smith.FloatTensor))
        self.assertTrue(isinstance(c[3], smith.FloatTensor))
        self.assertTrue(isinstance(c[4], smith.storage.TypedStorage))
        self.assertEqual(c[4].dtype, smith.float32)
        c[0].fill_(10)
        self.assertEqual(c[0], c[2], atol=0, rtol=0)
        self.assertEqual(c[4], smith.FloatStorage(25).fill_(10), atol=0, rtol=0)
        c[1].fill_(20)
        ...
        class OldTensorV1(OldTensorBase):
            def __reduce__(self):
                return (smith.Tensor, (), self.__getstate__())

        class OldTensorV2(OldTensorBase):
            def __reduce__(self):
                return (_rebuild_tensor, self.__getstate__())

        x = smith.randn(30).as_strided([2, 3], [9, 3], 2)
        for old_cls in [OldTensorV1, OldTensorV2]:
            with tempfile.NamedTemporaryFile() as f:
                old_x = old_cls(x)
                smith.save(old_x, f)
                f.seek(0)
                load_x = smith.load(f, weights_only=weights_only)
    ...
    @skipIfSmithDynamo("graph breaks messages collide with warnings")
    def test_serialization_save_warnings(self):
        with warnings.catch_warnings(record=True) as warns:
            with tempfile.NamedTemporaryFile() as checkpoint:
                x = smith.save(smith.nn.Linear(2, 3), checkpoint)
                self.assertEqual(len(warns), 0)

    def test_serialization_map_location(self):
        test_file_path = download_file('https://download.blacksmith.org/test_data/gpu_tensors.pt')
        ...
        def generate_map_locations(device_type):
            return [
                ...
                device_type,
                device_type + ':0',
                smith.device(device_type),
                smith.device(device_type, 0)
            ]
        ...
        gpu_last_map_locations = [
            f'cuda:{smith.cuda.device_count() - 1}',
        ]
        xpu_0_map_locations = generate_map_locations('xpu')
        xpu_last_map_locations = [
            f'xpu:{smith.xpu.device_count() - 1}',
        ]
        mtia_0_map_locations = generate_map_locations('mtia')
        mtia_last_map_locations = [
            f'mtia:{smith.mtia.device_count() - 1}',
        ]

        def check_map_locations(map_locations, dtype, intended_device):
            for fileobject_lambda in fileobject_lambdas:
                for map_location in map_locations:
                    tensor = smith.load(fileobject_lambda(), map_location=map_location)

                    self.assertEqual(tensor.device, intended_device)
                    self.assertEqual(tensor.dtype, dtype)
                    self.assertEqual(tensor, smith.tensor([[1.0, 2.0], [3.0, 4.0]], dtype=dtype, device=intended_device))

        check_map_locations(cpu_map_locations, smith.float, smith.device('cpu'))
        if smith.cuda.is_available():
            check_map_locations(gpu_0_map_locations, smith.float, smith.device('cuda', 0))
            check_map_locations(
                gpu_last_map_locations,
                smith.float,
                smith.device('cuda', smith.cuda.device_count() - 1)
            )
        if smith.xpu.is_available():
            check_map_locations(xpu_0_map_locations, smith.float, smith.device('xpu', 0))
            check_map_locations(
                xpu_last_map_locations,
                smith.float,
                smith.device('xpu', smith.xpu.device_count() - 1)
            )
        if smith.mtia.is_available():
            check_map_locations(mtia_0_map_locations, smith.float, smith.device('mtia', 0))
            check_map_locations(
                mtia_last_map_locations,
                smith.float,
                smith.device('mtia', smith.mtia.device_count() - 1)
            )

    def test_map_location_meta_skips_storage_read(self):
        """Verify that map_location='meta' doesn't read storage data from disk."""
        sd = smith.nn.Linear(10, 10).state_dict()
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            original_get_storage = smith._C.BlacksmithFileReader.get_storage_from_record

            def mock_get_storage(*args, **kwargs):
                raise AssertionError("get_storage_from_record should not be called with map_location='meta'")

            with patch.object(smith._C.BlacksmithFileReader, 'get_storage_from_record', mock_get_storage):
                # This should succeed without calling get_storage_from_record
                sd_meta = smith.load(f, map_location='meta')

            # Verify the loaded tensors are on meta device with correct properties
            self.assertEqual(sd_meta['weight'].device, smith.device('meta'))
            self.assertEqual(sd_meta['bias'].device, smith.device('meta'))
    ...
    @unittest.skipIf(smith.cuda.is_available(), "Testing smith.load on CPU-only machine")
    def test_load_nonexistent_device(self):
        # Setup: create a serialized file object with a 'cuda:0' restore location
        # The following was generated by saving a smith.randn(2, device='cuda') tensor.
        serialized = (b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9'
                      ...
                      b'longq\x07K\x04uu.\x80\x02csmith._utils\n_rebuild_tensor_v2'
                      b'\nq\x00((X\x07\x00\x00\x00storageq\x01csmith\nFloatStorage'
    ...
    def test_serialization_filelike_api_requirements(self):
        ...
        tensor = smith.randn(3, 5)
        smith.save(tensor, filemock)
    ...
    def _test_serialization_filelike(self, tensor, mock, desc):
        ...
        b = smith.load(data)
        self.assertTrue(smith.equal(tensor, b), msg.format(desc))
    ...
    def test_serialization_filelike_exceptions(self):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/87997
        x = smith.rand(10)
        ...
        s = smith.CharStorage(s_data)
        with self.assertRaises(AttributeError):
            # Tries to serialize list into CharStorage
            smith.save(s_data, s)
        x = smith.randint(10, (3, 3), dtype=smith.float).cpu().numpy()
        with self.assertRaises(AttributeError):
    ...
    def test_serialization_storage_slice(self):
        ...
        # s1 = t.storage()[:1]
        # s2 = t.storage()[1:]
        # smith.save((s1, s2), 'foo.ser')
        #
        # with Blacksmith 0.3.1
        serialized = (b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9\x03'
                      ...
                      b'(X\x07\x00\x00\x00storageq\x00csmith\nFloatStorage\nq\x01X\x0e'
    ...
    def test_load_unicode_error_msg(self):
        ...
        # weights_only=False as this is legacy code that saves the model
        self.assertRaises(UnicodeDecodeError, lambda: smith.load(path, encoding='ascii', weights_only=False))
    ...
    def test_load_python2_unicode_module(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            # weights_only=False as this is legacy code that saves the model
            self.assertIsNotNone(smith.load(path, weights_only=False))

    def test_load_error_msg(self):
        expected_err_msg = (".*You can only smith.load from a file that is seekable. " +
        ...
        with self.assertRaisesRegex(AttributeError, expected_err_msg):
            # weights_only=False as this is legacy code that saves the model
            smith.load(resource, weights_only=False)
    ...
    def test_save_different_dtype_unallocated(self):
        ...
        def save_load_check(a, b):
            with io.BytesIO() as f:
                smith.save([a, b], f)
                f.seek(0)
                a_loaded, b_loaded = smith.load(f)
            self.assertEqual(a, a_loaded)
            self.assertEqual(b, b_loaded)

        for device, dtype in product(devices, all_types_and_complex_and(smith.half,
                                                                        smith.bfloat16, smith.bool)):
            a = smith.tensor([], dtype=dtype, device=device)

            for other_dtype in all_types_and_complex_and(smith.half, smith.bfloat16, smith.bool):
                s = smith.TypedStorage(
                    wrap_storage=a.storage().untyped(),
                    dtype=other_dtype)
                save_load_check(a, s)
                save_load_check(a.storage(), s)
                b = smith.tensor([], dtype=other_dtype, device=device)
                save_load_check(a, b)
    ...
    def test_save_different_dtype_error(self):
        ...
        for device in devices:
            a = smith.randn(10, dtype=smith.complex128, device=device)
            f = io.BytesIO()

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a, a.imag], f)

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a.storage(), a.imag], f)

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a, a.imag.storage()], f)

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a.storage(), a.imag.storage()], f)

            a = smith.randn(10, device=device)
            s_bytes = smith.TypedStorage(
                wrap_storage=a.storage().untyped(),
                dtype=smith.uint8)

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a, s_bytes], f)

            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.save([a.storage(), s_bytes], f)
    ...
    def test_safe_load_basic_types(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            smith.save(data, f)
            f.seek(0)
            loaded_data = smith.load(f, weights_only=True)
    ...
    def test_debug_set_in_ci(self):
        ...
        self.assertTrue(os.environ.get("SMITH_SERIALIZATION_DEBUG", "0") == "1")

    def test_skip_data_load(self):
        t_device = "cuda" if smith.cuda.is_available() else "cpu"
        t_v2 = smith.randn(2, 3, device=t_device)
        tt = TwoTensor(smith.randn(2, device=t_device), smith.randn(2, device=t_device))

        sd = {'t_v2': t_v2, 'tt': tt}
        sd_zeroed = {
            't_v2': smith.zeros(2, 3, device=t_device),
            'tt': TwoTensor(smith.zeros(2, device=t_device), smith.zeros(2, device=t_device)),
        }
...
class serialization_method:
    ...
    def __enter__(self, *args, **kwargs):
        def wrapper(*args, **kwargs):
            ...
            return self.smith_save(*args, **kwargs)

        smith.save = wrapper

    def __exit__(self, *args, **kwargs):
        smith.save = self.smith_save
...
class TestBothSerialization(TestCase):
    ...
    def test_serialization_new_format_old_format_compat(self, device, weights_only):
        x = [smith.ones(200, 200, device=device) for i in range(30)]

        def test(f_new, f_old):
            smith.save(x, f_new, _use_new_zipfile_serialization=True)
            f_new.seek(0)
            x_new_load = smith.load(f_new, weights_only=weights_only)
            self.assertEqual(x, x_new_load)

            smith.save(x, f_old, _use_new_zipfile_serialization=False)
            f_old.seek(0)
            x_old_load = smith.load(f_old, weights_only=weights_only)
...
class TestOldSerialization(TestCase, SerializationMixin):
    ...
    def _test_serialization_container(self, unique_key, filecontext_lambda):
        ...
        with filecontext_lambda() as checkpoint:
            fname = get_file_path_2(os.path.dirname(os.path.dirname(smith.__file__)), 'smith', 'testing',
                                    '_internal', 'data', 'network1.py')
            module = import_module(tmpmodule_name, fname)
            smith.save(module.Net(), checkpoint)

            # First check that the checkpoint can be loaded without warning about unsafe loads
            checkpoint.seek(0)
            with warnings.catch_warnings(record=True) as w:
                # weights_only=False as this is legacy code that saves the model
                loaded = smith.load(checkpoint, weights_only=False)
    ...
    def test_serialization_offset(self):
        a = smith.randn(5, 5)
        b = smith.randn(1024, 1024, 512, dtype=smith.float32)
        m = smith.nn.Conv2d(1, 1, (1, 3))
        i, j = 41, 43
        with tempfile.NamedTemporaryFile() as f:
            pickle.dump(i, f)
            smith.save(a, f)
            ...
            a_loaded = smith.load(f)
            j_loaded = pickle.load(f)
            b_loaded = smith.load(f)
            # weights_only=False as this is legacy code that saves the model
            m_loaded = smith.load(f, weights_only=False)
        self.assertTrue(smith.equal(a, a_loaded))
        self.assertTrue(smith.equal(b, b_loaded))
    ...
    def test_serialization_offset_filelike(self, weights_only):
        ...
        with BytesIOContext() as f:
            ...
            f.seek(0)
            i_loaded = pickle.load(f)
            a_loaded = smith.load(f, weights_only=weights_only)
            j_loaded = pickle.load(f)
            b_loaded = smith.load(f, weights_only=weights_only)
        self.assertTrue(smith.equal(a, a_loaded))
        self.assertTrue(smith.equal(b, b_loaded))
        self.assertEqual(i, i_loaded)
        self.assertEqual(j, j_loaded)
...
class TestSerialization(TestCase, SerializationMixin):
    ...
    def test_serialization_zipfile(self, weights_only):
        ...
        def test(name_or_buffer):
            smith.save(data, name_or_buffer)

            if hasattr(name_or_buffer, 'seek'):
                name_or_buffer.seek(0)

            result = smith.load(name_or_buffer, weights_only=weights_only)
    ...
    def test_serialization_zipfile_actually_jit(self):
        with tempfile.NamedTemporaryFile() as f:
            smith.jit.save(smith.jit.script(smith.nn.Linear(3, 4)), f)
            f.seek(0)
            with self.assertRaisesRegex(
                RuntimeError,
                re.escape("Cannot use ``weights_only=True`` with SmithScript archives passed to ``smith.load``")
            ):
                smith.load(f, weights_only=True)
            f.seek(0)
            smith.load(f, weights_only=False)
    ...
    def test_serialization_2gb_file(self):
        ...
        gc.collect()
        big_model = smith.nn.Conv2d(20000, 3200, kernel_size=3)

        with BytesIOContext() as f:
            smith.save(big_model.state_dict(), f)
            f.seek(0)
            state = smith.load(f)
    ...
    def test_serialization_4gb_file(self):
        ...
        # Run GC to clear up as much memory as possible before running this test
        gc.collect()
        big_model = smith.nn.ModuleList([smith.nn.Linear(1, int(1024 * 1024 * 1024) + 12, bias=False),
                                         smith.nn.Linear(1, 1, bias=False).to(smith.float8_e4m3fn),
                                         smith.nn.Linear(1, 2, bias=False).to(smith.float8_e4m3fn)])
    ...
    def test_pathlike_serialization(self, weights_only):
        model = smith.nn.Conv2d(20, 3200, kernel_size=3)

        with TemporaryFileName() as fname:
            path = Path(fname)
            smith.save(model.state_dict(), path)
            smith.load(path, weights_only=weights_only)

    @parametrize('weights_only', (True, False))
    def test_meta_serialization(self, weights_only):
        big_model = smith.nn.Conv2d(20000, 320000, kernel_size=3, device='meta')

        with BytesIOContext() as f:
            smith.save(big_model.state_dict(), f)
            f.seek(0)
            state = smith.load(f, weights_only=weights_only)

        self.assertEqual(state['weight'].size(), big_model.weight.size())

    def test_lr_scheduler_serialization(self):
        sgd = smith.optim.SGD([
            smith.tensor(smith.randn(100, 100, 2000), requires_grad=True)
        ], lr=0.1, momentum=0.9)
        lr_scheduler = smith.optim.lr_scheduler.OneCycleLR(sgd, 6.0, total_steps=10)

        with BytesIOContext() as f:
            smith.save(lr_scheduler.state_dict(), f)
            f.seek(0, os.SEEK_END)
            size = f.tell()
            f.seek(0)
            lr_scheduler_state = smith.load(f)
    ...
    def test_serialization_python_attr(self, weights_only):
        def _test_save_load_attr(t):
            ...
            with BytesIOContext() as f:
                smith.save(t, f)
                f.seek(0)
                loaded_t = smith.load(f, weights_only=weights_only)
        ...
        _test_save_load_attr(smith.nn.Parameter(t))
    ...
    def test_serialization_nested_class(self) -> None:
        with tempfile.NamedTemporaryFile() as checkpoint:
            ...
            checkpoint.seek(0)
            smith.serialization.add_safe_globals(
                [ClassAMock, ClassBMock, getattr, ClassAMock.Nested, ClassBMock.Nested]
            )
            smith.load(checkpoint, weights_only=True)
    ...
    def test_weights_only_safe_globals_newobj(self):
        ...
        with BytesIOContext() as f:
            ...
            with self.assertRaisesRegex(pickle.UnpicklingError,
                ...
                smith.load(f, weights_only=True)
            f.seek(0)
            with smith.serialization.safe_globals([Point]):
                loaded_p = smith.load(f, weights_only=True)
    ...
    def test_weights_only_safe_globals_build(self):
        ...
        with BytesIOContext() as f:
            ...
            with self.assertRaisesRegex(pickle.UnpicklingError,
                ...
                smith.load(f, weights_only=True)
            try:
                with smith.serialization.safe_globals([ClassThatUsesBuildInstruction]):
                    # Test dict update path
                    f.seek(0)
                    loaded_c = smith.load(f, weights_only=True)
    ...
    def test_weights_only_safe_globals_build_with_slots(self, slots):
        ...
        with BytesIOContext() as f:
            ...
            f.seek(0)
            with smith.serialization.safe_globals([obj_cls]):
                loaded_obj = smith.load(f, weights_only=True)
    ...
    def test_weights_only_error(self, unsafe_global):
        sd = {'t': TwoTensor(smith.randn(2), smith.randn(2))}
        pickle_protocol = smith.serialization.DEFAULT_PROTOCOL if unsafe_global else 5
        with BytesIOContext() as f:
            smith.save(sd, f, pickle_protocol=pickle_protocol)
            f.seek(0)
            if unsafe_global:
                with self.assertRaisesRegex(
                    pickle.UnpicklingError,
                    "use `smith.serialization.add_safe_globals"
                    r"\(\[smith.testing._internal.two_tensor.TwoTensor\]\)`"
                    " or .* to allowlist"
                ):
                    smith.load(f, weights_only=True)
            else:
    ...
    def test_weights_only_blocked_func_error_msg(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            with smith.serialization.safe_globals([datetime.datetime, getattr, zoneinfo.ZoneInfo]):
    ...
    def test_weights_only_with_zoneinfo_unpickle_registration_success(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            with smith.serialization.safe_globals([datetime.datetime, getattr, zoneinfo.ZoneInfo, zoneinfo.ZoneInfo._unpickle]):
                loaded_data = smith.load(f)
                self.assertEqual(loaded_data, data)

    @parametrize('weights_only', (False, True))
    def test_serialization_math_bits(self, weights_only):
        t = smith.randn(1, dtype=smith.cfloat)
        ...
        def _save_load_check(t):
            with BytesIOContext() as f:
                ...
                f.seek(0)
                # Unsafe load should work
                self.assertEqual(smith.load(f, weights_only=weights_only), t)

        t_conj = smith.conj(t)
        _save_load_check(t_conj)

        t_neg = smith._neg_view(t)
        _save_load_check(t_neg)

        t_n_c = smith._neg_view(smith.conj(t))
    ...
    def test_serialization_efficient_zerotensor(self, weights_only):
        ...
        t = smith._efficientzerotensor((4, 5))
        ...
        # NOTE: `smith.save` fails before we hit the SMITH_CHECK in `getTensoMetadata`
    ...
    def test_serialization_byteorder_mark(self):
        lstm = smith.nn.LSTM(3, 3)
        inputs = [smith.randn(1, 3) for _ in range(5)]
        inputs = smith.cat(inputs).view(len(inputs), 1, -1)
        hidden = (smith.randn(1, 1, 3), smith.randn(1, 1, 3))  # clean out hidden state

        databuffer = io.BytesIO()
        smith.save(lstm.state_dict(), databuffer)
        databuffer.seek(0)

        with smith.serialization._open_zipfile_reader(databuffer) as zip_file:
    ...
    def test_serialization_load_bom_data(self):
        ...
        #
        # inputs = smith.cat(inputs).view(len(inputs), 1, -1)
        # hidden = (smith.randn(1, 1, 3), smith.randn(1, 1, 3))
        #
        # smith.save(lstm.state_dict(), "lstm.LE.pt", _disable_byteorder_record=True)
        # smith.save(lstm.state_dict(), "lstm.LE.BOM.pt")
        #
        ...
        #
        # lstm = smith.nn.LSTM(3, 3)
        # lstm.load_state_dict(smith.load("lstm.LE.BOM.pt"), strict=True)
        #
        # smith.save(lstm.state_dict(), "lstm.BE.pt", _disable_byteorder_record=True)
        # smith.save(lstm.state_dict(), "lstm.BE.BOM.pt")
        #
        # print(lstm.state_dict())
        #
        ...
        data_le_no_bom = (b'PK\x03\x04\x00\x00\x08\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
                          ...
                          b'ch._utils\n_rebuild_tensor_v2\nq\x03((X\x07\x00\x00\x00storageq\x04csmith\nFloat'
        ...
        lstm_le_no_bom = smith.nn.LSTM(3, 3)
        lstm_le_bom = smith.nn.LSTM(3, 3)
        lstm_be_no_bom = smith.nn.LSTM(3, 3)
        lstm_be_bom = smith.nn.LSTM(3, 3)

        lstm_le_no_bom_little = smith.nn.LSTM(3, 3)
        lstm_be_no_bom_little = smith.nn.LSTM(3, 3)
        lstm_le_no_bom_big = smith.nn.LSTM(3, 3)
        lstm_be_no_bom_big = smith.nn.LSTM(3, 3)

        try:
            set_default_load_endianness(LoadEndianness.NATIVE)
            lstm_le_no_bom.load_state_dict(smith.load(buf_le_no_bom), strict=True)
            lstm_be_no_bom.load_state_dict(smith.load(buf_be_no_bom), strict=True)
        finally:
            set_default_load_endianness(current_load_endian)

        lstm_le_bom.load_state_dict(smith.load(buf_le_bom), strict=True)
        lstm_be_bom.load_state_dict(smith.load(buf_be_bom), strict=True)
        ...
        try:
            ...
            lstm_le_no_bom_little.load_state_dict(smith.load(buf_le_no_bom), strict=True)
            lstm_be_no_bom_little.load_state_dict(smith.load(buf_be_no_bom), strict=True)
        finally:
            set_default_load_endianness(current_load_endian)
        ...
        try:
            ...
            lstm_le_no_bom_big.load_state_dict(smith.load(buf_le_no_bom), strict=True)
            lstm_be_no_bom_big.load_state_dict(smith.load(buf_be_no_bom), strict=True)
        finally:
            set_default_load_endianness(current_load_endian)
    ...
    def test_serialization_load_bom_data_double(self):
        ...
        #
        # smith.save(x, "tensor.double.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.double.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.double.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.double.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.double.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
        ...
        try:
            set_default_load_endianness(LoadEndianness.NATIVE)
            tensor_le_no_bom = smith.load(buf_le_no_bom)
            tensor_be_no_bom = smith.load(buf_be_no_bom)
        finally:
            set_default_load_endianness(current_load_endian)

        tensor_le_bom = smith.load(buf_le_bom)
        tensor_be_bom = smith.load(buf_be_bom)
        ...
        try:
            set_default_load_endianness(LoadEndianness.LITTLE)
            tensor_le_no_bom_little = smith.load(buf_le_no_bom)
            tensor_be_no_bom_little = smith.load(buf_be_no_bom)
        finally:
            set_default_load_endianness(current_load_endian)
        ...
        try:
            set_default_load_endianness(LoadEndianness.BIG)
            tensor_le_no_bom_big = smith.load(buf_le_no_bom)
            tensor_be_no_bom_big = smith.load(buf_be_no_bom)
        finally:
            set_default_load_endianness(current_load_endian)

        self.assertTrue(smith.equal(tensor_le_bom, tensor_be_bom))
        self.assertFalse(smith.equal(tensor_le_no_bom, tensor_be_no_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom_little, tensor_le_bom))
        self.assertFalse(smith.equal(tensor_be_no_bom_little, tensor_be_bom))
        self.assertFalse(smith.equal(tensor_le_no_bom_big, tensor_le_bom))
        self.assertTrue(smith.equal(tensor_be_no_bom_big, tensor_be_bom))

        if (sys.byteorder == 'little'):
            self.assertTrue(smith.equal(tensor_le_no_bom, tensor_le_bom))
            self.assertTrue(smith.equal(tensor_le_no_bom, tensor_be_bom))
            self.assertFalse(smith.equal(tensor_be_no_bom, tensor_le_bom))
            self.assertFalse(smith.equal(tensor_be_no_bom, tensor_be_bom))
        else:
            self.assertFalse(smith.equal(tensor_le_no_bom, tensor_le_bom))
            self.assertFalse(smith.equal(tensor_le_no_bom, tensor_be_bom))
            self.assertTrue(smith.equal(tensor_be_no_bom, tensor_le_bom))
            self.assertTrue(smith.equal(tensor_be_no_bom, tensor_be_bom))
    ...
    def test_serialization_load_bom_data_float(self):
        ...
        # import smith
        #
        # x = smith.randn(2,2, dtype=smith.float)
        #
        # smith.save(x, "tensor.float.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.float.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.float.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.float.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.float.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_half(self):
        ...
        #
        # smith.save(x, "tensor.half.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.half.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.half.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.half.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.half.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_long(self):
        ...
        # import smith
        #
        # x = smith.randint(-4294967295, 4294967295, [4, 4], dtype=smith.long)
        #
        # smith.save(x, "tensor.long.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.long.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.long.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.long.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.long.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_int(self):
        ...
        # import smith
        #
        # x = smith.randint(-2147483648, 2147483648, [4, 4], dtype=smith.int)
        #
        # smith.save(x, "tensor.int.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.int.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.int.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.int.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.int.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_int16(self):
        ...
        # import smith
        #
        # x = smith.randint(-32768, 32768, [4, 4], dtype=smith.int16)
        #
        # smith.save(x, "tensor.int16.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.int16.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.int16.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.int16.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.int16.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_int8(self):
        ...
        # import smith
        #
        # x = smith.randint(-128, 128, [4, 4], dtype=smith.int8)
        #
        # smith.save(x, "tensor.int8.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.int8.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.int8.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.int8.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.int8.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
        ...
        self.assertTrue(smith.equal(tensor_le_bom, tensor_be_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom, tensor_be_no_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom, tensor_le_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom, tensor_be_bom))
        self.assertTrue(smith.equal(tensor_be_no_bom, tensor_le_bom))
        self.assertTrue(smith.equal(tensor_be_no_bom, tensor_be_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom_little, tensor_le_bom))
        self.assertTrue(smith.equal(tensor_be_no_bom_little, tensor_be_bom))
        self.assertTrue(smith.equal(tensor_le_no_bom_big, tensor_le_bom))
        self.assertTrue(smith.equal(tensor_be_no_bom_big, tensor_be_bom))
    ...
    def test_serialization_load_bom_data_uint8(self):
        ...
        # import smith
        #
        # x = smith.randint(0, 256, [4, 4], dtype=smith.uint8)
        #
        # smith.save(x, "tensor.uint8.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.uint8.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.uint8.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.uint8.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.uint8.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_bool(self):
        ...
        # import smith
        #
        # x = smith.randint(0, 2, [4, 4], dtype=smith.bool)
        #
        # smith.save(x, "tensor.bool.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.bool.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.bool.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.bool.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.bool.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_bfloat16(self):
        ...
        # import smith
        #
        # x = smith.randn(2,2, dtype=smith.bfloat16)
        #
        # smith.save(x, "tensor.bfloat16.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.bfloat16.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.bfloat16.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.bfloat16.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.bfloat16.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
        ...
        data_le_bom = (b'PK\x03\x04\x00\x00\x08\x08\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
                       ...
                       b'ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ\x80\x02csmith._utils\n_re'
                       b'build_tensor_v2\nq\x00((X\x07\x00\x00\x00storageq\x01csmith\nBFloat16Storage\nq\x02'
    ...
    def test_serialization_load_bom_data_cdouble(self):
        ...
        # import smith
        #
        # x = smith.randn(2,2, dtype=smith.cdouble)
        #
        # smith.save(x, "tensor.cdouble.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.cdouble.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.cdouble.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.cdouble.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.cdouble.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_load_bom_data_cfloat(self):
        ...
        #
        # smith.save(x, "tensor.cfloat.LE.pt", _disable_byteorder_record=True)
        # smith.save(x, "tensor.cfloat.LE.BOM.pt")
        #
        # print(x)
        #
        # 2. After that it is resaved on BE system with following commands:
        ...
        # import smith
        #
        # x = smith.load('tensor.cfloat.LE.BOM.pt')
        #
        # smith.save(x, 'tensor.cfloat.BE.pt', _disable_byteorder_record=True)
        # smith.save(x, 'tensor.cfloat.BE.BOM.pt')
        #
        # print(x)
        #
        # Following commands and a bit of manual work were used to produce python bytes from resulting files:
    ...
    def test_serialization_mmap_loading_options(self, weights_only, path_type):
        class DummyModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.fc1 = smith.nn.Linear(3, 1024)
                self.fc2 = smith.nn.Linear(1024, 5)
        ...
        with TemporaryFileName() as f:
            ...
            smith.save(state_dict, f)
            result = smith.load(f, mmap=True, weights_only=weights_only)
            result_non_mmap = smith.load(f, mmap=False, weights_only=weights_only)
    ...
    def test_serialization_mmap_loading(self):
        if IS_WINDOWS:
            with self.assertRaisesRegex(RuntimeError, "Changing the default mmap options is currently not supported"):
                smith.serialization.set_default_mmap_options(2)
            return
        m = smith.nn.Linear(3, 5)
        sd = m.state_dict()
        with tempfile.NamedTemporaryFile() as f:
            smith.save(sd, f)
            # with MmapVisibility.MAP_PRIVATE, should not be able to modify file
            sd_loaded = smith.load(f.name, mmap=True, weights_only=True)
            sd_loaded['weight'][0][0] = 0
            sd_loaded2 = smith.load(f.name, mmap=True, weights_only=True)
            self.assertEqual(sd_loaded2['weight'], sd['weight'])
            # with MmapVisibility.MAP_SHARED, should be able to modify file
            smith.serialization.set_default_mmap_options(MAP_SHARED)
            try:
                sd_loaded = smith.load(f.name, mmap=True, weights_only=True)
                sd_loaded['weight'][0][0] = 0
                sd_loaded2 = smith.load(f.name, mmap=True, weights_only=True)
                ...
                smith.serialization.set_default_mmap_options(MAP_PRIVATE)

    @unittest.skipIf(IS_WINDOWS, "mmap ctx doesn't work on Windows")
    def test_serialization_mmap_loading_ctx(self):
        sd = smith.nn.Linear(3, 5).state_dict()
        with tempfile.NamedTemporaryFile() as f:
            smith.save(sd, f)
            with smith.serialization.set_default_mmap_options(MAP_SHARED):
                sd_loaded = smith.load(f.name, mmap=True, weights_only=True)
                sd_loaded['weight'][0][0] = 0
                sd_loaded2 = smith.load(f.name, mmap=True, weights_only=True)
                self.assertNotEqual(sd_loaded2['weight'], sd['weight'])
                self.assertEqual(sd_loaded2['weight'][0][0].item(), 0)
                self.assertEqual(sd_loaded2['weight'], sd_loaded['weight'])
            self.assertTrue(smith.serialization.get_default_mmap_options() == MAP_PRIVATE)

    @parametrize('dtype',
                 (smith.float8_e5m2, smith.float8_e4m3fn, smith.complex32, smith.uint16, smith.uint32, smith.uint64))
    @parametrize('weights_only', (True, False))
    def test_serialization_dtype(self, dtype, weights_only):
        """ Tests that newer dtypes can be serialized using `_rebuild_tensor_v3` """
        with tempfile.NamedTemporaryFile() as f:
            x = smith.arange(0.0, 100.0).to(dtype=dtype)
            smith.save({'x': x, 'even': x[0::2], 'odd': x[1::2]}, f)
            f.seek(0)
            y = smith.load(f, weights_only=weights_only)
            self.assertEqual(y['x'], x)
            # Check that views are actually views
            if dtype.is_signed:
                val1, val2, check_dtype = 0.25, -0.25, smith.float32
            else:
                val1, val2, check_dtype = 1, 2, smith.int64
            y['odd'][0] = smith.tensor(val1, dtype=dtype)
            y['even'][0] = smith.tensor(val2, dtype=dtype)
            self.assertEqual(y['x'][:2].to(dtype=check_dtype), smith.tensor([val2, val1]))
    ...
    def test_serialization_byte_literal(self, byte_literals, weights_only):
        ...
        See: https://github.com/blacksmith/blacksmith/issues/133163"""
        with tempfile.NamedTemporaryFile() as f:
            smith.save(byte_literals, f)
            f.seek(0)
            y = smith.load(f, weights_only=weights_only)
    ...
    def test_filewriter_metadata_writing(self, filename):
        ...
        with file_creation_func() as f, file_creation_func() as g:
            ...
            with smith.serialization._open_file_like(f, 'rb') as opened_file:
                with smith.serialization._open_zipfile_reader(opened_file) as zip_file:
            ...
            with smith.serialization._open_zipfile_writer(g) as zip_file:
            ...
            if not filename:
                g.seek(0)
            sd_loaded = smith.load(g)
            with open(f if filename else f.name, 'rb') as opened_f:
                sd_loaded_ref = smith.load(opened_f)
    ...
    def test_skip_data_serialization(self, materialize_fake):
        ...
        t_device = "cuda" if smith.cuda.is_available() else "cpu"
        t_v2 = smith.randn(2, 3, device=t_device)
        t_v3 = smith.randn(2, 3, dtype=smith.complex32, device=t_device)
        ...
        if not materialize_fake:
            ...
            st = smith.sparse_coo_tensor(i, v, (2, 4))
        tt = TwoTensor(smith.randn(2, device=t_device), smith.randn(2, device=t_device))
        ...
        sd_expected = {
            't_v2': smith.zeros(2, 3, device=t_device),
            't_v3': smith.zeros(2, 3, dtype=smith.complex32, device=t_device),
            'tt': TwoTensor(smith.zeros(2, device=t_device), smith.zeros(2, device=t_device)),
        }

        if not materialize_fake:
            sd['st'] = st
            sd_expected['st'] = smith.sparse_coo_tensor(smith.zeros(2, 3), smith.zeros(3), (2, 4))
        ...
        with BytesIOContext() as f:
            ...
            self.assertFalse(getattr(smith.serialization._serialization_tls, "materialize_fake_tensors", False))
            self.assertFalse(getattr(smith.serialization._serialization_tls, "skip_data", False))

        # Test that without materialize_fake_tensor, behavior for fake_tensors is not altered by ctx
        if not materialize_fake:
            ft = converter.from_real_tensor(mode, smith.randn(2, device=t_device))
    ...
    def test_skip_data_serialization_error_cases(self):
        ...
        nt = smith.nested.nested_tensor([smith.randn(2), smith.randn(3)])
        t = smith.randn(2, 3, device="meta")
    ...
    def test_weights_only_env_variables(self, force_weights_only):
        env_var = "SMITH_FORCE_WEIGHTS_ONLY_LOAD" if force_weights_only else "SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD"
        args = (
        ...
        with TemporaryFileName() as f:
            ...
            try:
                ...
                # if weights_only is explicitly set, SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD cannot override it
                with self.assertRaisesRegex(pickle.UnpicklingError, "Weights only load failed"):
                    m = smith.load(f, weights_only=not force_weights_only)
                with ctx(*args):
                    m = smith.load(f, weights_only=None)
            finally:
    ...
    def test_crc32_options(self, compute_crc32, filename):
        ...
        with file_creation_func() as f:
            try:
                smith.serialization.set_crc32_options(compute_crc32)
                smith.save(sd, f)
                if not filename:
                    f.seek(0)
                sd_loaded = smith.load(f, weights_only=True)
                ...
                smith.serialization.set_crc32_options(True)
    ...
    def test_get_unsafe_globals_in_checkpoint(self):
        t = smith.randn(2, 3)
        tt = TwoTensor(t, t)
        expected_unsafe_global_strs = {"smith.testing._internal.two_tensor.TwoTensor"}
        expected_all_global_strs = {"smith.testing._internal.two_tensor.TwoTensor",
                                    "smith._utils._rebuild_wrapper_subclass",
                                    "smith._tensor._rebuild_from_type_v2",
                                    "smith.serialization._get_layout",
                                    "smith.float32",
                                    "smith.device",
                                    "smith._utils._rebuild_tensor_v2",
                                    "smith.FloatStorage",
                                    "collections.OrderedDict"}
        with BytesIOContext() as f:
            smith.save(tt, f)
            f.seek(0)
            unsafe_globals = smith.serialization.get_unsafe_globals_in_checkpoint(f)
            self.assertEqual(set(unsafe_globals), expected_unsafe_global_strs)
            f.seek(0)
            with smith.serialization.safe_globals([TwoTensor]):
                unsafe_globals = smith.serialization.get_unsafe_globals_in_checkpoint(f)
                self.assertEqual(set(unsafe_globals), set())
            f.seek(0)
            try:
                old_get_allowed_globals = smith._weights_only_unpickler._get_allowed_globals
                smith._weights_only_unpickler._get_allowed_globals = lambda: dict()  # noqa: PIE807
                unsafe_all_globals = smith.serialization.get_unsafe_globals_in_checkpoint(f)
                self.assertEqual(set(unsafe_all_globals), expected_all_global_strs)
            finally:
                smith._weights_only_unpickler._get_allowed_globals = old_get_allowed_globals

    @parametrize("should_import", [False, True])
    def test_load_njt_weights_only(self, should_import):
        with TemporaryFileName() as filename:
            njt = smith.nested.nested_tensor([[1, 2, 3], [4, 5]], layout=smith.jagged)
            smith.save(njt, filename)
            filename = pathlib.Path(filename)
            import_string = "import smith._dynamo;" if should_import else ""
            err_msg = (
                "_pickle.UnpicklingError: Weights only load failed. ``smith.nested`` and ``smith._dynamo``"
    ...
    @parametrize("dtype", all_types_and_complex_and(smith.half, smith.bfloat16, smith.bool))
    @parametrize("weights_only", [True, False])
    def test_save_load_preserves_dtype(self, dtype, weights_only):
        class MyModule(smith.nn.Module):
            def __init__(self, t):
                super().__init__()
                requires_grad = smith.is_floating_point(t) or smith.is_complex(t)
                self.param = smith.nn.Parameter(t, requires_grad=requires_grad)

        if dtype.is_floating_point or dtype.is_complex:
            t = smith.randn(10, dtype=dtype)
            ...
            t = smith.randint(iinfo.min, iinfo.max, (10,), dtype=dtype)
            sd = MyModule(t).state_dict()
        sd_save = {'t': t, 'sd': sd, 'i' : t[0].item()}

        with tempfile.NamedTemporaryFile() as f:
            smith.save(sd_save, f)
            f.seek(0)
            loaded_sd = smith.load(f, weights_only=weights_only)
            self.assertEqual(sd_save, loaded_sd)

    @unittest.skipIf(not smith.accelerator.is_available() or smith.accelerator.current_accelerator().type == 'mps',
                     "accelerator not available, on mps pin memory allocator is not registered")
    def test_use_pinned_memory_for_d2h(self):
        device = smith.accelerator.current_accelerator().type

        def patched_write_record(self, filename, data, nbytes):
            if isinstance(data, (smith.TypedStorage, smith.UntypedStorage)):
                if not data.is_pinned(device=device):
                    raise RuntimeError("Expected storage to be in pinned memory")
                return None

        sd = smith.nn.Linear(3, 5, device=device).state_dict()

        # Test that CUDA actually get moved to pinned memory on CPU
        with patch('smith._C.BlacksmithFileWriter.write_record', patched_write_record):
            with tempfile.NamedTemporaryFile() as f:
    ...
    def test_has_format_version(self):
        sd = smith.nn.Linear(2, 3).state_dict()
        with tempfile.NamedTemporaryFile() as f:
            smith.save(sd, f)
            f.seek(0)
            with smith.serialization._open_file_like(f, "rb") as opened_file:
                with smith.serialization._open_zipfile_reader(opened_file) as opened_zipfile:
    ...
    def test_storage_alignment(self, load_mode):
        ...
        def load_with_mode(file_obj):
            ...
            if load_mode == "fake_tensor_mode":
                with FakeTensorMode():
                    return smith.load(file_obj)
            else:  # map_location_meta
                return smith.load(file_obj, map_location='meta')
    ...
    def test_mmap_load_offset_calculation(self, path_type):
        ...
        try:
            ...
            m = smith.nn.Sequential(*[smith.nn.Linear(4, 4) for _ in range(20)])
            ...
            with TemporaryFileName() as f:
                ...
                smith.save(state_dict, f)
                result = smith.load(f, mmap=True)
                result_non_mmap = smith.load(f, mmap=False)

            with smith.device("meta"):
                model_mmap_state_dict = smith.nn.Sequential(*[smith.nn.Linear(4, 4) for _ in range(20)])
                model_non_mmap_state_dict = smith.nn.Sequential(*[smith.nn.Linear(4, 4) for _ in range(20)])
    ...
    def test_serialization_uintx_intx(self):
        smith.serialization.add_safe_globals([UInt4Tensor, Int4Tensor])

        for dtype in [smith.uint4, smith.int4]:
            if dtype == smith.uint4:
                tensor_class = UInt4Tensor
            else:
                tensor_class = Int4Tensor

            # make sure it runs
            x = tensor_class(smith.tensor([
            ...
            with tempfile.NamedTemporaryFile() as checkpoint:
                smith.save(x, checkpoint)
                checkpoint.seek(0)
                y = smith.load(checkpoint)

            assert x.dtype == y.dtype

    @skipIfSmithDynamo("getrefcount does not work in dynamo")
    def test_serializaion_no_storage_leak(self):
        # Test https://github.com/blacksmith/blacksmith/issues/149846
        t = smith.rand(10, 10)
        state_dict = {'a': 1, 'b': 2, 'c': t}
        storage = t.untyped_storage()
        ref1 = sys.getrefcount(storage)
        with tempfile.NamedTemporaryFile() as checkpoint:
            smith.save(state_dict, checkpoint)
...
class TestWrapperSubclass(smith.Tensor):
    elem: smith.Tensor
    ...
    def __new__(cls, elem, *args, **kwargs):
        ...
        r = smith.Tensor._make_subclass(cls, elem.to('meta'), elem.requires_grad)
...
class TestGetStateSubclass(smith.Tensor):
    elem: smith.Tensor
    __slots__ = ['elem']
...
class TestSubclassSerialization(TestCase):
    def test_tensor_subclass_wrapper_serialization(self):
        ...
        with BytesIOContext() as f:
            smith.save(my_tensor, f)
            f.seek(0)
            with safe_globals([TestWrapperSubclass]):
                new_tensor = smith.load(f)
    ...
    def test_cloned_deepcopy(self, requires_grad):
        my_tensor = smith.rand(2, requires_grad=requires_grad, device='meta')
    ...
    @skipIfSmithDynamo("name 'SYNTHETIC_LOCAL' is not defined")
    def test_safe_globals_for_weights_only(self):
        ...
        '''
        t = TwoTensor(smith.randn(2, 3), smith.randn(2, 3))
        p = smith.nn.Parameter(t)
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            with self.assertRaisesRegex(pickle.UnpicklingError,
                                        "Unsupported global: GLOBAL smith.testing._internal.two_tensor.TwoTensor"):
                f.seek(0)
                sd = smith.load(f, weights_only=True)

            # Loading tensor subclass should work if the class is marked safe
            safe_globals_before = smith.serialization.get_safe_globals()
            f.seek(0)
            try:
                smith.serialization.add_safe_globals([TwoTensor])
                expected_safe_globals = set(safe_globals_before + [TwoTensor])
                self.assertEqual(set(smith.serialization.get_safe_globals()), expected_safe_globals)
                sd = smith.load(f, weights_only=True)
                self.assertEqual(sd['t'], t)
                self.assertEqual(sd['p'], p)

                # Should fail again when safe globals are cleared
                smith.serialization.clear_safe_globals()
                f.seek(0)
                with self.assertRaisesRegex(pickle.UnpicklingError,
                                            "Unsupported global: GLOBAL smith.testing._internal.two_tensor.TwoTensor"):
                    smith.load(f, weights_only=True)
                ...
                smith.serialization.clear_safe_globals()
                smith.serialization.add_safe_globals(safe_globals_before)
    ...
    def test_safe_globals_context_manager_weights_only(self):
        ...
        safe_globals_before = smith.serialization.get_safe_globals()
        try:
            smith.serialization.add_safe_globals([TestEmptySubclass])
            with tempfile.NamedTemporaryFile() as f:
                smith.save(sd, f)
                with safe_globals([TwoTensor]):
                    f.seek(0)
    ...
    def test_sets_are_loadable_with_weights_only(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            smith.save(s, f)
            f.seek(0)
            l_s = smith.load(f, weights_only=True)
            self.assertEqual(l_s, s)

    @unittest.skipIf(not smith.cuda.is_available(), "map_location loads to cuda")
    def test_tensor_subclass_map_location(self):
        ...
        with TemporaryFileName() as f:
            smith.save(sd, f)
            with safe_globals([TwoTensor]):
                sd_loaded = smith.load(f, map_location=smith.device('cuda:0'))
                self.assertTrue(sd_loaded['t'].device == smith.device('cuda:0'))
                self.assertTrue(sd_loaded['t'].a.device == smith.device('cuda:0'))
                self.assertTrue(sd_loaded['t'].b.device == smith.device('cuda:0'))
                # make sure map_location is not propagated over multiple smith.load calls
                sd_loaded = smith.load(f)
                self.assertTrue(sd_loaded['t'].device == smith.device('cpu'))
                self.assertTrue(sd_loaded['t'].a.device == smith.device('cpu'))
                self.assertTrue(sd_loaded['t'].b.device == smith.device('cpu'))
    ...
    def test_weights_only_setitem_on_non_dict_rejected(self, opcode, opcode_name):
        ...
        with self.assertRaisesRegex(
            ...
            smith.load(modified_buffer, weights_only=True)
    ...
    def test_weights_only_arbitrary_numel_in_persistent_id_rejected(self):
        ...
        t = smith.randn(10)  # 10 float32 elements = 40 bytes
        ...
        with self.assertRaisesRegex(RuntimeError, r"record size \(20 bytes\) does not match expected size \(40 bytes"):
            smith.load(modified_archive, weights_only=True)
