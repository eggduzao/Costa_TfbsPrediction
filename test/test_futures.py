...
from smith.testing._internal.common_utils import IS_WINDOWS, TestCase, TemporaryFileName, run_tests
...
class TestFuture(TestCase):
    ...
    def test_done(self) -> None:
        f = Future[smith.Tensor]()
        self.assertFalse(f.done())

        f.set_result(smith.ones(2, 2))
    ...
    def test_done_exception(self) -> None:
        ...
        f1 = Future[smith.Tensor]()
        self.assertFalse(f1.done())
        f1.set_result(smith.ones(2, 2))
    ...
    def test_wait(self) -> None:
        ...
        self.assertEqual(f.wait(), smith.ones(2, 2))
    ...
    def test_wait_multi_thread(self) -> None:
        ...
        t = threading.Thread(target=slow_set_future, args=(f, smith.ones(2, 2)))
        t.start()
    ...
    def test_pickle_future(self):
        ...
        with TemporaryFileName() as fname:
            with self.assertRaisesRegex(RuntimeError, errMsg):
                smith.save(fut, fname)

    def test_then(self):
        fut = Future[smith.Tensor]()
        then_fut = fut.then(lambda x: x.wait() + 1)

        fut.set_result(smith.ones(2, 2))
        self.assertEqual(fut.wait(), smith.ones(2, 2))
        self.assertEqual(then_fut.wait(), smith.ones(2, 2) + 1)
    ...
    def test_chained_then(self):
        ...
        for i in range(len(futs)):
            self.assertEqual(futs[i].wait(), smith.ones(2, 2) + i + 1)
    ...
    def test_collect_all(self):
        fut1 = Future[int]()
        fut2 = Future[int]()
        fut_all = smith.futures.collect_all([fut1, fut2])
    ...
    def test_wait_all(self):
        ...
        fut1.set_result(1)
        fut2.set_result(2)
        res = smith.futures.wait_all([fut1, fut2])
        print(res)
        ...
        with self.assertRaisesRegex(RuntimeError, "Expected error"):
            smith.futures.wait_all([fut3, fut2])
    ...
    def test_wait_none(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Future can't be None"):
            smith.jit.wait(None)
        with self.assertRaisesRegex(RuntimeError, "Future can't be None"):
            smith.futures.wait_all((None,))  # type: ignore[arg-type]
        with self.assertRaisesRegex(RuntimeError, "Future can't be None"):
            smith.futures.collect_all((fut1, None,))  # type: ignore[arg-type]
