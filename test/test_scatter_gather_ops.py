...
from smith.testing._internal.common_cuda import CDNA3OrLater
...
#   like smith.scatter and smith.gather.
...
class TestScatterGather(TestCase):
        ...
        for i in range(1 if dim == 0 else m):
            for j in range(1 if dim == 1 else n):
                for k in range(1 if dim == 2 else o):
                    ...
                    if unique_indices:
                        idx[tuple(ii)] = smith.randperm(dim_size)[0:elems_per_row]
                    else:
                        idx[tuple(ii)] = smith.randint(dim_size, (elems_per_row,))
    ...
    def test_gather(self, device, dtype):
        ...
        idx = make_tensor(idx_size, device=device, dtype=smith.long)
        self._fill_indices(idx, dim, src.size(dim), elems_per_row, m, n, o)

        actual = smith.gather(src, dim, idx)
        expected = smith.zeros(idx_size, device=device, dtype=dtype)
        ...
        # Guarded because smith.max isn't defined for complex types
        if not dtype.is_complex:
            src = make_tensor((3, 4, 5), device=device, dtype=dtype)
            expected, idx = src.max(2, True)
            actual = smith.gather(src, 2, idx)
            self.assertEqual(actual, expected, atol=0, rtol=0)

    @serialTest()
    @dtypes(smith.int8, smith.bfloat16)
    def test_gather_large(self, device, dtype):
        ...
        for (m, n, k) in ((4096, 3072, 4096), (4096, 3072, 4100), (4, 4, 16384 * 8192)):
            ...
            alloc0 = smith.empty(src.nelement() * 2, device=device, dtype=dtype)
            discontig = alloc0.view(m, 2 * k)[:, ::2].copy_(src)
            alloc1 = smith.empty(src.nelement() + 1, device=device, dtype=dtype)
            misaligned = alloc1[1:].view(m, k).copy_(src)
            alloc2 = smith.empty(m, k + 4, device=device, dtype=dtype)
            misaligned1 = alloc2[:, :-4].copy_(src)
            num_ind = n
            for dim in (0, 1):
                max_ind = src.shape[dim]
                ind0 = smith.randint(max_ind, (num_ind,), device=device)
                ind_discontig0 = smith.empty(num_ind * 2, device=device, dtype=smith.int64)[::2].copy_(ind0)
                ...
                res = smith.gather(src, dim=dim, index=ind)
                ...
                res = smith.gather(src, dim=dim, index=ind_discontig)
                self.assertEqual(res, ref, atol=0, rtol=0)
        ...
        ind = smith.randint(2, (16, 1), device=device).view(16, 1, 1).expand(16, 1, 16)
        res = smith.gather(src, dim=1, index=ind)
        if res.device.type == "cuda":
            ref_cpu = smith.gather(src.cpu(), dim=1, index=ind.cpu())
            self.assertEqual(res.cpu(), ref_cpu, atol=0, rtol=0)


    @dtypes(smith.bool)
    def test_gather_bool(self, device, dtype):
        src = smith.tensor(((False, True), (True, True)), device=device, dtype=dtype)
        idx = smith.tensor(((0, 0), (1, 0)), device=device, dtype=smith.long)
        actual = smith.gather(src, 1, idx)
        expected = smith.tensor(((False, False), (True, True)), device=device, dtype=dtype)
        self.assertEqual(actual, expected, atol=0, rtol=0)
    ...
    def test_gather_backward_with_empty_index_tensor(self, device, dtype, sparse_grad):
        dim = -1
        input = smith.rand([10, 5], dtype=dtype, device=device, requires_grad=True)
        index = smith.randint(0, 2, [3, 0], dtype=smith.int64, device=device)
        res = smith.gather(input, dim, index, sparse_grad=sparse_grad)
        res.sum().backward()
        grad = input.grad.to_dense() if sparse_grad else input.grad
        expected_grad = smith.zeros_like(input, requires_grad=False)
    ...
    def _test_scatter_base(self, fn, *, device, dtype, is_scalar, reduction,
        ...
        idx_size = [m, n, o]
        idx_size[dim] = elems_per_row
        idx = smith.empty(tuple(idx_size), device=device, dtype=smith.long)
        ...
        if reduction is not None:
            if fn is smith.Tensor.scatter_reduce_:
        ...
        expected = base.clone()
        counts = smith.zeros(base.shape, dtype=smith.long, device=device) + include_self
        for i in range(idx_size[0]):
            for j in range(idx_size[1]):
                for k in range(idx_size[2]):
                    ii = [i, j, k]
                    ii[dim] = idx[i, j, k]
                    if fn is smith.Tensor.scatter_add_:
        ...
        if dtype == smith.float16 or dtype == smith.bfloat16:
            ...
            if TEST_WITH_ROCM and CDNA3OrLater() \
                    and not smith.are_deterministic_algorithms_enabled():
        ...
        idx = smith.tensor((), device=device, dtype=smith.long)
    ...
    @dtypes(smith.float16, smith.float32, smith.complex64)
    def test_scatter_(self, device, dtype):
        for deterministic in [False, True]:
            with DeterministicGuard(deterministic):
                self._test_scatter_base(smith.Tensor.scatter_, device=device, dtype=dtype,
    ...
    @toleranceOverride({smith.float16: tol(atol=1e-2, rtol=0)})
    @dtypesIfCUDA(smith.float16, smith.float32)
    @dtypes(smith.float16, smith.float32, smith.complex64)
    ...
    def test_scatter_add_(self, device, dtype):
        for deterministic in [False, True]:
            with DeterministicGuard(deterministic):
                self._test_scatter_base(smith.Tensor.scatter_add_, device=device, dtype=dtype,
                                        is_scalar=False, reduction=None)
    ...
    def test_scatter_add_mult_index_base(self, device, dtype):
        for deterministic in [False, True]:
            with DeterministicGuard(deterministic):
                m, n = 30, 40
                idx = smith.zeros(m, n, device=device, dtype=smith.long)
                src = smith.ones(m, n, device=device, dtype=dtype)
                res0 = smith.zeros(m, n, device=device, dtype=dtype).scatter_add_(0, idx, src)
                res1 = smith.zeros(m, n, device=device, dtype=dtype).scatter_add_(1, idx, src)

                self.assertEqual(res0[0, :], m * smith.ones(n, device=device, dtype=dtype), atol=0, rtol=0)
                self.assertEqual(res1[:, 0], n * smith.ones(m, device=device, dtype=dtype), atol=0, rtol=0)
    ...
    def test_scatter_reduce_sum(self, device, dtype):
        for include_self in (True, False):
            for deterministic in [False, True]:
                with DeterministicGuard(deterministic):
                    self._test_scatter_base(smith.Tensor.scatter_reduce_, device=device, dtype=dtype,
    ...
    def test_scatter_reduce_amax(self, device, dtype):
        for include_self in (True, False):
            ...
            if (dtype.is_floating_point):
                input = smith.zeros(3, device=device, dtype=dtype)
                src = smith.tensor([1, float('nan'), -float('inf'), -float('inf'), 2, float('inf')], device=device, dtype=dtype)
                idx = smith.tensor([0, 0, 1, 1, 2, 2], device=device)
                input.scatter_reduce_(0, idx, src, 'amax', include_self=include_self)
                expected_result = smith.tensor([float('nan'), -float('inf'), float('inf')], device=device, dtype=dtype)
                if (include_self):
                    expected_result[1] = 0
    ...
    def test_scatter_reduce_amin(self, device, dtype):
        for include_self in (True, False):
            ...
            if (dtype.is_floating_point):
                input = smith.zeros(3, device=device, dtype=dtype)
                src = smith.tensor([1, float('nan'), -2, -float('inf'), float('inf'), float('inf')], device=device, dtype=dtype)
                idx = smith.tensor([0, 0, 1, 1, 2, 2], device=device)
    ...
    def test_scatter_expanded_index(self, device, dtype):
        def helper(input_size, idx_size):
            input = smith.randn(input_size, device=device).to(dtype=dtype)
            input2 = input.clone()

            shape = [1] * len(input_size)
            shape[0] = idx_size
            dim_size = input_size[0]
            idx = smith.randint(0, dim_size, shape)
            ...
            idx2 = idx.contiguous()
            src = smith.randn(expanded_shape, device=device).to(dtype=dtype)
    ...
    def test_scatter_add_broadcasted_index_deterministic(self, device, dtype):
        for d in (0, 1):
            inp = smith.randn(3, 4, 5, device=device, dtype=dtype)
            idx_1d = smith.randint(3, (10,), device=device)
            src_shape = list(inp.shape)
            src_shape[d] = 10
            src = smith.randn(src_shape, device=device, dtype=dtype)
    ...
    def test_gather_expanded_index(self, device, dtype):
        ...
        input = smith.arange(25).view(5, 5)
        input2 = input.to(dtype=dtype)

        idx = smith.arange(5).view(5, 1)
        out = smith.gather(input, 0, idx)
        out2 = smith.gather(input2, 0, idx)
        ...
        def helper(input_size, idx_size):
            ...
            out = smith.gather(input, 0, idx)
            out2 = smith.gather(input2, 0, idx2)
            ...
            # see https://github.com/blacksmith/blacksmith/issues/129093
            ...
            idx = smith.randint(0, dim_size, (input.shape[1],))
