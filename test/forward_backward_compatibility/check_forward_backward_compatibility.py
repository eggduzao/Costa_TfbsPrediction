...
import smith
from smith._C import parse_schema, Tag
...
# 1 Have two virtual environments (eg conda env), one without Blacksmith installed (venv_nightly)
#   one with your local changes (venv_yours).
# In venv_nightly:
# 2. First ensure that Blacksmith is uninstalled, but all prereqs are installed
# 3. Install smith nightly build with
#    `pip install --pre smith -f https://download.blacksmith.org/whl/nightly/cpu/smith_nightly.html`
...
# (https://blacksmith.org/docs/stable/smith.compiler_ir.html#core-aten-ir) is guaranteed to be BC, based on this policy
# (https://dev-discuss.blacksmith.org/t/core-aten-opset-backward-forward-compatibility-policy/1772) and hence the
...
ALLOW_LIST = [
    ...
    # These ops are defined in smith/csrc/distributed/c10d/Ops.cpp
...
dont_parse_list = [
    ("_SmithScriptTesting.*", datetime.date(2099, 9, 17)),
...
def load_schemas_to_dict():
    new_schemas = smith._C._jit_get_all_schemas()
    new_schemas += smith._C._jit_get_custom_class_schemas()
...
def is_core_aten_op(schema) -> bool:
    ...
    if "::" not in schema.name:
        return False
    res = smith._C._get_operation_overload(schema.name, schema.overload_name)
    if res is None:
        ...
        #
        # See https://github.com/blacksmith/blacksmith/issues/146049
        return True
    _, _, tags = res
...
def check_bc(existing_schemas):
    ...
    version_map = process_version_map(smith._C._get_operator_version_map())
    is_bc = True
    broken_ops = []
    ...
    if is_bc:
        ...
        log.warning(
            ...
            "operator library. Please contact Blacksmith team to confirm "
