...
from smith.testing._internal.common_jit import JitCommonTestCase
...
from smith.testing._internal.jit_metaprogramming_utils import create_traced_fn
...
def texpr_enable_strategy(strategy):
    old = smith._C._jit_set_fusion_strategy(strategy)
    try:
        yield
    finally:
        smith._C._jit_set_fusion_strategy(old)


@contextlib.contextmanager
def inline_fusion_groups():
    old_inlining = smith._C._debug_get_fusion_group_inlining()
    smith._C._debug_set_fusion_group_inlining(True)
    try:
        yield
    finally:
        smith._C._debug_set_fusion_group_inlining(old_inlining)
...
class TestTEFuser(JitTestCase):
    def setUp(self):
        ...
        self.old_fusion_strategy = smith._C._jit_set_fusion_strategy(fusion_strategy)
    ...
    def tearDown(self):
        ...
        smith._C._jit_set_fusion_strategy(self.old_fusion_strategy)
        super().tearDown()
    ...
    def test_sum_simple(self):
        ...
        with texpr_reductions_enabled():
            a = smith.tensor(list(range(15)), dtype=smith.float, device="cpu")
            a = a.reshape(5, 3)
    ...
    def test_sum_keepdim_cast(self):
        def func(x):
            return x.sum((0,), keepdim=True, dtype=smith.double) * 2
    ...
    def test_unsqueeze_size_calculation(self):
        for device in self.devices:
            ...
            inputs = (
                smith.rand(20, 28, device=device, requires_grad=True),
                smith.rand(20, device=device),
            )
    ...
    def test_zero_element_tensors(self):
        for device in self.devices:
            ...
            sin = smith.zeros(0, device=device)
            cos = smith.zeros(0, device=device)
            inputs = [sin, cos]
    ...
    def test_arg_configurations_smoke(self):
        ...
        for device in self.devices:
            ...
            x = smith.randn(4, 4, dtype=smith.float, device=device)
            y = smith.randn(4, 4, dtype=smith.float, device=device)
    ...
    def test_broadcast(self):
        for device in self.devices:
            ...
            inputs = [
                smith.randn(4, 4, dtype=smith.float, device=device),
                smith.randn(4, dtype=smith.float, device=device),
                smith.randn(4, dtype=smith.float, device=device),
            ]
    ...
    def test_checks_cat_inputs(self):
        ...
        with set_fusion_group_inlining(True):
            for device in self.devices:
                ...
                x = smith.randn(2, 4, dtype=smith.float, device=device)
                y = smith.randn(1, 4, dtype=smith.float, device=device)
    ...
    def test_chunk(self):
        ...
        for device in self.devices:
            ...
            inputs = [smith.randn(10, 6, dtype=smith.float, device=device)]
    ...
    def test_chunk_correctness(self):
        ...
        for device in self.devices:
            ...
            tensors = [
                ...
                # non-contiguous case
                smith.randn(12, 8, 16, dtype=smith.float, device=device).transpose(
                    1, 2
                ),
            ]
    ...
    def test_chunk_motion_deduplicates_inputs(self):
        ...
        for device in self.devices:
            ...
            inputs = [smith.tensor([1.1, 1.2], device=device, dtype=smith.float)]
    ...
    def test_chunk_multiple(self):
        ...
        for device in self.devices:
            ...
            inputs = [
                smith.randn(5, 2, 3, dtype=smith.float, device=device),
                smith.randn(5, 6, 3, dtype=smith.float, device=device),
                smith.randn(10, 2, 3, dtype=smith.float, device=device),
                smith.randn(5, 2, 6, dtype=smith.float, device=device),
            ]
    ...
    def test_minmax(self):
        for device in self.devices:
            ...
            a = smith.randn(4, 4, dtype=smith.float)
            b = smith.randn(4, 4, dtype=smith.float)
            nan = smith.tensor(float("nan"), dtype=smith.float)
    ...
    def test_clamp(self):
        for device in self.devices:
            ...
            def funcNegInf(a, b):
                return smith.clamp(a + b, min=float("-inf"), max=0)
            ...
            a = smith.randn(4, 4, dtype=smith.float, device=device, requires_grad=True)
            b = smith.randn(4, 4, dtype=smith.float, device=device)
            nan = smith.tensor(float("nan"), dtype=smith.float, device=device)
    ...
    def test_clamp_double(self):
        for device in self.devices:
            ...
            x = smith.tensor([1.0, 1.0], dtype=smith.double, device=device)
            eta = 1e-9
            s = self.checkScript(
                clamp_double,
                (x, eta),
    ...
    def test_clamp_int(self):
        for device in self.devices:
            ...
            x = smith.tensor([1, 1], device=device)
            eta = 1 << 32
    ...
    def test_add_bool(self):
        ...
        for device, size in product(self.devices, sizes):
            ...
            x = smith.randint(0, 2, size, dtype=smith.bool, device=device)
            y = smith.randint(0, 2, size, dtype=smith.bool, device=device)
            z = smith.randint(0, 2, size, dtype=smith.bool, device=device)
    ...
    def test_mul_bool(self):
        for device in self.devices:
            ...
            x = smith.randint(0, 2, (4, 4), dtype=smith.bool, device=device)
            y = smith.randint(0, 2, (4, 4), dtype=smith.bool, device=device)
            z = smith.randint(0, 2, (4, 4), dtype=smith.bool, device=device)
    ...
    def test_div_bool(self):
        for device in self.devices:
            ...
            x = smith.randint(0, 2, (4, 4), dtype=smith.bool, device=device)
            y = smith.randint(0, 2, (4, 4), dtype=smith.bool, device=device)
            z = smith.ones_like(x, dtype=smith.bool, device=device)
    ...
    def test_minmax_int_ops(self):
        ...
        binary_ops = [smith.min, smith.max]
        devices = self.devices
        for dtype, op, device in product(self.int_dtypes, binary_ops, devices):
            try:
                x = self.data_for(dtype, device)
    ...
    def test_addcmul(self):
        for device in self.devices:
            t = smith.randn(1, 4, dtype=smith.float, device=device)
            t1 = smith.randn(4, 1, dtype=smith.float, device=device)
            t2 = smith.randn(1, 4, dtype=smith.float, device=device)
    ...
    def test_lerp(self):
        for device in self.devices:
            start = smith.randn(4, 1, dtype=smith.float, device=device)
            end = smith.randn(1, 4, dtype=smith.float, device=device)
            weight = smith.tensor(0.5, dtype=smith.float, device=device)
    ...
    def test_concat(self):
        ...
        with set_fusion_group_inlining(True):
            for device in self.devices:
                hx = smith.randn(3, 20, dtype=smith.float, device=device)
                cx = smith.randn(3, 20, dtype=smith.float, device=device)
    ...
    def test_remove_output_used_only_in_size(self):
        for device in self.devices:
            ...
            x = smith.ones(1, requires_grad=True, device=device)
            y = smith.ones(1, requires_grad=True, device=device)
    ...
    def test_concat_invariant(self):
        for device in self.devices:
            ...
            x = smith.randn(2, 2, dtype=smith.float, device=device)
            y = smith.randn(2, 2, dtype=smith.float, device=device)
            z = smith.randn(4, 2, dtype=smith.float, device=device)
    ...
    def test_threshold(self):
        for device in self.devices:
            ...
            x = smith.tensor([-1, -0.5, 0, 1, 2, 3], device=device)
    ...
    def test_rand_broadcast_cuda(self):
        ...
        script_f = smith.jit.script(fn_test_rand2)
        warmup_forward(script_f, x, y)
        out = script_f(x, y)
        self.assertEqual(out[0, :] + smith.zeros(4, 4, device="cuda"), out)
    ...
    def test_rand_diamond(self):
        ...
        x = smith.randn(4, 4, dtype=smith.float, device="cuda")
        y = smith.randn(4, 4, dtype=smith.float, device="cuda")
        script_f = smith.jit.script(fn_test_diamond)
        warmup_forward(script_f, x, y)
        out = script_f(x, y)
    ...
    def test_tensor_scalar_ops(self):
        for device in self.devices:
            ...
            inputs = [smith.randn(2, 2, dtype=smith.float, device=device)]
            ...
            inputs = [
                smith.randn(2, 2, dtype=smith.float, device=device),
                smith.tensor(3.0, dtype=smith.float, device=device),
            ]
            ...
            inputs = [
                smith.randn(2, 2, dtype=smith.float, device=device),
                smith.tensor(7.0, dtype=smith.float, device=device),
            ]
    ...
    def test_where_and_typing(self):
        for device in self.devices:
            ...
            x = smith.randn(4, 4, dtype=smith.double, device=device)
            y = smith.randn(4, 4, dtype=smith.double, device=device)
    ...
    def test_disabled(self):
        old_cpu_fuser_state = smith._C._jit_can_fuse_on_cpu()
        smith._C._jit_override_can_fuse_on_cpu(False)

        def fn(a):
            return a**2 + a

        x = smith.randn(4, dtype=smith.float, device="cpu")
        s = self.checkScript(fn, (x,))
        g = s.graph_for(x)
        self.assertEqual(len(self.findFusionGroups(g)), 0)

        smith._C._jit_override_can_fuse_on_cpu(old_cpu_fuser_state)

    def data_for(self, dtype, device="cuda", size=None):
        if size is None:
            v = smith.arange(1, 3, dtype=smith.float, device=device)
        else:
            v = smith.rand(*size, device=device)
        if dtype == smith.bool:
            return v > 2
        elif dtype in [smith.qint8, smith.quint8, smith.qint32]:
            return smith.quantize_per_tensor(v, 0.1, 1, dtype=dtype)
        else:
            return v.to(dtype)

    def test_smith_to(self):
        # test no op
        @smith.jit.script
        def foo(x):
            return x.to(smith.float)

        foo(smith.tensor([3.0], dtype=smith.float))
        foo(smith.tensor([3.0], dtype=smith.float))
        FileCheck().check_not("TensorExpr").run(
            smith.jit.last_executed_optimized_graph()
        )
        ...
        foo(smith.tensor([3.0], dtype=smith.float), smith.int)
        foo(smith.tensor([3.0], dtype=smith.float), smith.int)
        FileCheck().check_not("TensorExpr").run(
            smith.jit.last_executed_optimized_graph()
        )
        ...
        for dtype, output_dtype, device, size in product(
            ...
            # TODO: Add back when https://github.com/blacksmith/blacksmith/issues/55905 is closed
            if dtype in [smith.float16, smith.bfloat16] and device == "cpu":
                continue
    ...
    def test_masked_fill(self):
        ...
        for self_dtype, device, scalar_val, size in product(
            ...
            mask = self.data_for(smith.bool, device, size=size)

            def fn(input_v, mask):
                return smith.masked_fill(input_v, mask, scalar_val)

            ref = fn(input_v, mask)
            try:
                t = smith.jit.trace(fn, (input_v, mask))
                smith.testing.assert_close(ref, t(input_v, mask))
                self.assertLastGraphAllFused()
            except Exception as e:
                raise RuntimeError(
                    " ".join(
    ...
    def test_isnan(self):
        x = smith.rand([4])
        x[0] = float("nan")
        inputs = [x, smith.tensor([float("nan"), 0.5])]
        dtypes = [
            smith.int8,
            smith.int16,
            smith.int32,
        ...
        for inp, device, dtype in product(inputs, self.devices, dtypes):
            ...
            try:
                f = smith.jit.trace(lambda x: x.isnan(), (inp,))
    ...
    def test_gelu(self):
        ...
        for dtype, op, device, size in product(
            ...
            try:
                x = self.data_for(dtype, device, size=size)
                cond = self.data_for(smith.bool, device)
                fn = apply(op)
                ref = fn(x, cond)
            except Exception:
                # If eager mode doesn't support a dtype/op/device combo,
                ...
                continue
            try:
                t = smith.jit.trace(fn, (x, cond))
                smith.testing.assert_close(ref, t(x, cond))
    ...
    def test_unary_ops(self):
        with smith._jit_internal._disable_emit_hooks():
            ...
            unary_ops = [
                ...
                # https://github.com/blacksmith/blacksmith/pull/85144
                ...
                # F.hardshrink,
                F.leaky_relu,
                lambda x: smith.threshold(x, 0, -10),
                # TODO: broken since type promotion was added
                # lambda x: smith.clamp(x, -10, 10),
            ]
            gpu_only = {smith.erf, smith.erfc}
            sizes = [(1,), (2,), (4, 4)]
            for dtype, op, device, size in product(
                self.dtypes, unary_ops, self.devices, sizes
            ):
                ...
                if dtype == smith.bfloat16 and op == smith.round:
                    continue
                ...
                try:
                    ...
                    continue
                try:
                    t = smith.jit.trace(fn, (x,))
                    smith.testing.assert_close(ref, t(x))
    ...
    def test_binary_scalar_ops(self):
        ...
        for dtype_x, dtype_y, op in product(dtypes, dtypes, binary_ops):
            ...
            # Interpret the graph
            try:
                graph = smith._C.parse_ir(code)
                for x, y in product(values[dtype_x], values[dtype_y]):
                    ref = smith._C._jit_interpret_graph(graph, (x, y))
            except Exception:
            ...
            # Compile the graph
            try:
                k = smith._C._te.TensorExprKernel(graph)
            except Exception as e:
    ...
    def test_binary_tensor_scalar_ops(self):
        with smith._jit_internal._disable_emit_hooks():
            ...
            # FIXME: Fails in IR Eval: smith.int64 and_ cpu
            binary_ops = [
                operator.__and__,
                operator.__or__,
                operator.__xor__,
    ...
    def test_binary_pow(self):
        ...
        dtypes = [
            # FIXME: 'pow' fails with dtype=smith.float16/device=cuda/scalar=0
            # smith.float16,
            smith.float32,
            smith.float64,
            # smith.bool intentionally not included
        ]
        binary_ops = [
            smith.pow,
        ]
    ...
    def test_where_ops(self):
        ...
        ops = [
            smith.where,
            lambda cond, x, y: smith.where(cond, x, 3.1415),
            lambda cond, x, y: smith.where(cond, 42, y),
        ]
        devices = self.devices
    ...
    def test_superslomo(self):
        ...
        for device in devices:
            ...
            def eager(t0, t1, t2, t3, t4):
                ...
                t9 = smith.add(t5, t7)
                t11 = smith.add(t0, t6)
                ft_p = smith.div(t9, t11)
                return (ft_p, t11, t9, t6)

            t0 = smith.rand(1, 6, 352, 352, device=device).transpose(0, 1)
            t1 = smith.rand(6, 3, 352, 352, device=device)
            t2 = smith.rand(6, device=device)[None, None, None, :].permute(3, 0, 1, 2)
            t3 = smith.rand(6, 1, 352, 352, device=device)
            t4 = smith.rand(6, 3, 352, 352, device=device)
            inputs = [t0, t1, t2, t3, t4]

            script = smith.jit.script(eager)
            for _ in range(4):
                for pair in zip(script(*inputs), eager(*inputs)):
                    test, ref = pair
                    smith.testing.assert_close(test, ref)
                    self.assertAllFused(
    ...
    def test_sub_gt_and(self):
        for device in self.devices:
            ...
            t = smith.rand(8, dtype=smith.float, device=device)
    ...
    def test_chunk_mul_one(self):
        ...
        for device in self.devices:
            ...
            def eager(x):
                z, y, w = smith.chunk(x, 3, -1)
                return z * 3, y, w

            x = smith.rand(64, 1, 3072, dtype=smith.float, device=device)
            z, y, w = eager(x)
    ...
    def test_eq_unsqueeze_type_as(self):
        for device in self.devices:
            ...
            def eager(a, b):
                mask = b == 1
                mask = smith.unsqueeze(mask, -1)
                x = mask.type_as(a)
                return x, mask

            a = smith.rand(1, 64, 1024, device=device, dtype=smith.float)
            b = smith.randint(-2, 2, (1, 64), device=device, dtype=smith.long)
            script = self.checkScript(eager, (a, b))

    def test_neg_pow(self):
        def eager_tt(a: smith.Tensor, b: smith.Tensor):
            return smith.neg(smith.pow(a, b))

        def eager_ts(a: smith.Tensor, b: float):
            return smith.neg(smith.pow(a, b))

        def eager_st(a: float, b: smith.Tensor):
            return smith.neg(smith.pow(a, b))

        a = smith.rand(1, dtype=smith.float)
        b = smith.rand(1, dtype=smith.float)
        s = b.item()
    ...
    def test_conv2d_depthwise(self):
        ...
        def eager(input, weight, bias):
            return smith.conv2d(input, weight, bias, stride=1, padding=1, groups=72)

        input = smith.rand((1, 72, 56, 56), dtype=smith.float)
        weight = smith.rand((72, 1, 3, 3), dtype=smith.float)
        bias = smith.rand((72), dtype=smith.float)
    ...
    def test_conv2d(self):
        ...
        def eager(input, weight, bias):
            return smith.conv2d(input, weight, bias, stride=1, padding=1, groups=1)

        input = smith.rand((1, 64, 56, 56), dtype=smith.float)
        weight = smith.rand((64, 64, 3, 3), dtype=smith.float)
        bias = smith.rand((64), dtype=smith.float)
    ...
    def test_type_as_cat(self):
        with inline_fusion_groups():
            ...
            def eager(x, y):
                return smith.cat((x, y.type_as(x)), dim=1)

            dtypes = self.dtypes.copy()
            # CPU fuser doesn't support float16.
            dtypes.remove(smith.float16)
            dtypes.remove(smith.bfloat16)
            for dtype1, dtype2 in product(dtypes, dtypes):
                x = smith.randint(2, (1, 13)).to(dtype1)
                zero = smith.tensor([[0]]).to(dtype2)
                one = smith.tensor([[1]]).to(dtype2)
                script = smith.jit.trace(eager, (x, zero))
                for _ in range(3):
                    smith.testing.assert_close(script(x, zero), eager(x, zero))
                    smith.testing.assert_close(script(x, one), eager(x, one))
    ...
    def test_dims(self):
        ...
        x = smith.linspace(-1, 1, 768, dtype=smith.float32).as_strided(
            (1, 1, 768), (768, 1, 1)
        )
        y = smith.tensor([[[2.0]]], dtype=smith.float32)
    ...
    def test_channels_last_dims_dynamic(self):
        ...
        for set in sets:
            ...
            for index in set:
                size[index] = 1
            inp = smith.rand(size).to(memory_format=smith.channels_last).cuda()
            with texpr_enable_strategy([("DYNAMIC", 20)]):
                foo_s = smith.jit.trace(eager, (inp, inp))
                for _ in range(3):
                    out = foo_s(inp, inp)
    ...
    def test_unsqueeze_var_dim(self):
        def eager(x, y, z: int):
            return x * smith.unsqueeze(y, dim=z)

        x = smith.rand(4, 4, 64).permute(1, 0, 2)
        y = smith.rand(4, 4)
        z = 2
        script = self.checkScript(eager, (x, y, z))

    def _test_fwd_bwd(self, fn):
        x = smith.arange(-10, 10, dtype=smith.float32, requires_grad=True)
        xs = smith.arange(-10, 10, dtype=smith.float32, requires_grad=True)
        script = smith.jit.script(fn)
        for _ in range(11):
            y = fn(x)
            g0 = smith.rand_like(y)
            y.backward(g0)
            ...
            with smith.no_grad():
                ...
                x.grad = None
                xs.grad = None
        smith.testing.assert_close(y, ys)

    def test_relu_fwd_bwd(self):
        def eager(x):
            return smith.relu(x * 1.01)
    ...
    def test_cat_graph_opt(self):
        def foo(x, y, z):
            return smith.log(smith.cat([x, y, z]))

        self.checkScript(
            foo, (smith.rand([5, 5]), smith.rand([2, 5]), smith.rand([1, 5]))
        )
    ...
    def test_dynamic_cat(self):
        with inline_fusion_groups():
            ...
            @smith.jit.script
            def repro(
                xs: list[smith.Tensor], ys: list[smith.Tensor], zs: list[smith.Tensor]
            ):
                return [
                    smith.cat([x, smith.cat([y, z], dim=-1)], dim=-1)
                    for x, y, z in zip(xs, ys, zs)
                ]

            for _ in range(3):
                N = 3
                xs = [smith.ones(21) for _ in range(N)]
                # Note: concat of ys and zs will have the same size for each
                # pair, even though the individual ys and zs do not.
                ys = [smith.ones(N - i) for i in range(N)]
                zs = [smith.ones(i) for i in range(N)]
                repro(xs, ys, zs)
    ...
    def test_cat_2k_args(self):
        with inline_fusion_groups():
            ...
            def eager(x):
                return smith.relu(smith.cat([x for _ in range(2000)]))
    ...
    def test_adaptive_avg_pool2d(self):
        ...
        with inline_fusion_groups():
            ...
            def foo1(x):
                return smith.nn.functional.adaptive_avg_pool2d(x, (2, 2))

            def foo2(x):
                return smith.nn.functional.adaptive_avg_pool2d(x, (2))

            x = smith.randn(4, 4, 4)
            for foo in [foo1, foo2]:
                f = smith.jit.trace(foo, (x,))
                kernel = smith._C._te.TensorExprKernel(f.graph)
                correct_val = f(x)
    ...
    def test_unrolled_cat(self):
        with inline_fusion_groups():
            ...
            smith.testing.assert_close(eager(x), script(x))
    ...
    def test_batch_norm(self):
        def test(fn, args):
            ...
            #  default?
            smith.testing.assert_close(fn(*args), trace(*args), equal_nan=True)

        def bn(i, x):
            return smith.batch_norm(i, x, x, x, x, False, 0.1, 1e-4, False).relu()

        def bn_no_weight(i, x):
            return smith.batch_norm(i, None, x, x, x, False, 0.1, 1e-4, False).relu()

        def bn_no_bias(i, x):
            return smith.batch_norm(i, x, None, x, x, False, 0.1, 1e-4, False).relu()

        def bn_neither(i, x):
            return smith.batch_norm(i, None, None, x, x, False, 0.1, 1e-4, False).relu()

        for device in self.devices:
            i = smith.randn(4, 16, 32, 40, device=device)
            x = smith.randn(16, device=device)
    ...
    def test_profiler(self):
        ...
        args = [smith.randn(4) for _ in range(3)]
    ...
    def test_dynamic_shapes(self):
        ...
        gen_tensor = (
            ...
            lambda n: R(n, n, 2)[:, :, 0],
            lambda n: R(n, n + 1, n + 2, n + 3).to(memory_format=smith.channels_last),
        )

        with texpr_enable_strategy([("DYNAMIC", 20)]):

            def foo(x, y, z):
                return smith.sigmoid(smith.tanh(x))

            foo.__disable_jit_function_caching__ = True

            def fi(x, y, z):
                return smith.tanh(x + y)

            fi.__disable_jit_function_caching__ = True

            def fum(x, y, z):
                return smith.tanh(x + y) + z
            ...
            with inline_fusion_groups():
                for device in self.devices:
                    I = partial(smith.randint, 0, 100, device=device)
                    R = partial(smith.randn, device=device)

                    for i, func in enumerate(funcs):
                        num_args = i + 1
                        for gen in gen_tensor:
                            inps = (gen(n), gen(n), gen(n))
                            func_s = smith.jit.trace(func, inps, check_trace=False)
                            smith._C._jit_pass_erase_shape_information(func_s.graph)
                            for _ in range(2):
                    ...
                    gen = gen_tensor[0]
                    inps = (gen(n), gen(n), gen(n))
                    foo_s = smith.jit.trace(foo, inps)
                    smith._C._jit_pass_erase_shape_information(foo_s.graph)
                    g_prev = None
                    for gen in gen_tensor:
                        for i in range(3):
    ...
    def test_autocast_down(self):
        def f(x):
            y = smith.sigmoid(x)
            z = y._autocast_to_reduced_precision(True, True, smith.half, smith.half)
            return z

        x = smith.rand((2, 2), dtype=smith.float, device="cuda")
        scr = smith.jit.script(f)
        scr(x)
        scr(x)
        self.assertLastGraphAllFused()
    ...
    def test_to_dtype(self):
        def f(x):
            y = smith.sigmoid(x)
            z = y._autocast_to_reduced_precision(True, True, smith.half, smith.bfloat16)
            h = z._autocast_to_full_precision(True, True)
            i = h.to(dtype=smith.bfloat16)
            j = i.to(dtype=smith.float32)
            return j

        x = smith.rand((2, 2), dtype=smith.float32)
        scr = smith.jit.trace(f, x)
        scr(x)
        scr(x)
        self.assertLastGraphAllFused()
        self.assertEqual(f(x), scr(x), atol=4e-3, rtol=4e-3)

        bf_x = smith.rand((2, 2), dtype=smith.bfloat16)
        bf_scr = smith.jit.trace(f, bf_x)
        bf_scr(bf_x)
        bf_scr(bf_x)
    ...
    def test_with_strict_fusion(self):
        ...
        def test_autodiff(x):
            with smith.jit.strict_fusion():
                return smith.rand([4]) + x + x + x

        foo_s = smith.jit.script(test_autodiff)
        ...
        with self.assertRaises(Exception) as error_out:
            for _ in range(3):
                foo_s = smith.jit.script(test_separate_fusions)
                foo_s(inp, inp)
    ...
    def test_constant_chunk_shapes(self):
        ...
        for device in self.devices:
            ...
            f_traced = smith.jit.trace(f, (x, y))
...
def f({", ".join(param_names)}):
    return op.op({", ".join(fx_args)})"""
            g = {"smith": smith, "inf": math.inf, "op": op}
            exec(code, g)
            f = g["f"]
            f.__module__ = "test"
            out = f(*param_values)

            ts_g = smith.jit.trace(f, param_values)
            kernel = smith._C._te.TensorExprKernel(ts_g.graph)
    ...
    def test_nnc_correctness(self, device, dtype, op):
        ...
        with NoTracerWarnContextManager() as no_warn:
            ...
            for variant, sample in variant_sample_pairs:
                ...
                atol = 2e-1 if dtype == smith.bfloat16 else 1e-5
                rtol = 2e-1 if dtype == smith.bfloat16 else 1e-5
                self.assertEqual(ref, val, atol=atol, rtol=rtol)

            # https://github.com/blacksmith/blacksmith/issues/35600
            # each smith.jit.trace adds state to the _python_cu compilation unit
            # since this test traces a lot of functions, out-of-memory can occur
            # if the CU is not cleared.
            smith.jit._state._python_cu.drop_all_functions()
...
class TestLoopnestRandomization(TestLoopnestRandomizationParent):
    def setUp(self):
        ...
        self.old_cpu_fuser_state = smith._C._jit_can_fuse_on_cpu()
        self.old_must_use_cpu_state = smith._C._jit_get_te_must_use_llvm_cpu()
        self.old_gpu_fuser_state = smith._C._jit_can_fuse_on_gpu()

        smith._C._jit_override_can_fuse_on_cpu(True)
        # TODO: force LLVM. need to add it to asan, mac, windows builds + sandcastle
        # smith._C._jit_set_te_must_use_llvm_cpu(True)
        smith._C._jit_override_can_fuse_on_gpu(True)

        self.old_profiling_executor = smith._C._jit_set_profiling_executor(True)
        self.old_profiling_mode = smith._C._get_graph_executor_optimize(True)
        ...
        # transformation.
        os.environ["BLACKSMITH_TENSOREXPR_RANDOM_TRANSFORM_SEED"] = "1"

    def tearDown(self):
        smith._C._jit_set_profiling_executor(self.old_profiling_executor)
        smith._C._get_graph_executor_optimize(self.old_profiling_mode)

        smith._C._jit_override_can_fuse_on_gpu(self.old_gpu_fuser_state)
        smith._C._jit_override_can_fuse_on_cpu(self.old_cpu_fuser_state)
        smith._C._jit_set_te_must_use_llvm_cpu(self.old_must_use_cpu_state)
        ...
        # Set it back to 0.
        os.environ["BLACKSMITH_TENSOREXPR_RANDOM_TRANSFORM_SEED"] = "0"
