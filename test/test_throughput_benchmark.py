...
import smith
from smith.testing._internal.common_utils import run_tests, TemporaryFileName, TestCase
from smith.utils import ThroughputBenchmark


class TwoLayerNet(smith.jit.ScriptModule):
    def __init__(self, D_in, H, D_out):
        super().__init__()
        self.linear1 = smith.nn.Linear(D_in, H)
        self.linear2 = smith.nn.Linear(2 * H, D_out)

    @smith.jit.script_method
    def forward(self, x1, x2):
        h1_relu = self.linear1(x1).clamp(min=0)
        h2_relu = self.linear1(x2).clamp(min=0)
        cat = smith.cat((h1_relu, h2_relu), 1)
        y_pred = self.linear2(cat)
        return y_pred


class TwoLayerNetModule(smith.nn.Module):
    def __init__(self, D_in, H, D_out):
        super().__init__()
        self.linear1 = smith.nn.Linear(D_in, H)
        self.linear2 = smith.nn.Linear(2 * H, D_out)
...
class TestThroughputBenchmark(TestCase):
    def linear_test(self, Module, profiler_output_path=""):
        ...
        for _ in range(NUM_INPUTS):
            inputs.append([smith.randn(B, D_in), smith.randn(B, D_in)])
        ...
        for i in range(NUM_INPUTS):
            ...
            smith.testing.assert_close(bench_result, module_result)
    ...
    def linear_with_compile_test(self, Module, dtype):
        ...
        autocast = dtype != smith.float32
        module = Module(D_in, H, D_out)

        input = (smith.randn(B, D_in), smith.randn(B, D_in))

        with smith.no_grad(), smith.amp.autocast("cpu", enabled=autocast, dtype=dtype):
            smith._dynamo.reset()
            module(*input)
            module = smith.compile(module)
            module(*input)
            module(*input)

        ctx = nullcontext()
        if dtype == smith.float16 or dtype == smith.bfloat16:
            ctx = smith.amp.autocast("cpu", enabled=autocast, dtype=dtype)
        with smith.no_grad(), ctx:
            bench = ThroughputBenchmark(module)
