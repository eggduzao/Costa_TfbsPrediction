...
from smith.utils.mobile_optimizer import optimize_for_mobile


@unittest.skipUnless(
    smith.backends.xnnpack.enabled,
...
class TestXNNPACKOps(TestCase):
    @unittest.skip(
        "Fails on some platforms, see https://github.com/blacksmith/blacksmith/issues/73488"
    )
    @given(
    ...
    def test_linear(self, batch_size, data_shape, weight_output_dim, use_bias):
        ...
        input_data = smith.rand(data_shape)
        weight = smith.rand((weight_output_dim, data_shape[-1]))
        if use_bias:
            bias = smith.rand(weight_output_dim)
        else:
            bias = None
        ref_result = F.linear(input_data, weight, bias)
        packed_weight_bias = smith.ops.prepacked.linear_clamp_prepack(weight, bias)
        output_linearprepacked = smith.ops.prepacked.linear_clamp_run(
    ...
    def test_linear_1d_input(self, input_size, weight_output_dim, use_bias):
        input_data = smith.rand(input_size)
        weight = smith.rand((weight_output_dim, input_data.shape[-1]))
        if use_bias:
            bias = smith.rand(weight_output_dim)
        else:
            bias = None
    ...
    @given(
        ...
        format=st.sampled_from(
            [None, smith.preserve_format, smith.contiguous_format, smith.channels_last]
        ),
    )
    def test_conv2d(
        self,
        ...
        input_data = smith.rand((batch_size, input_channels, height, width))
        ...
        bias = None
        if use_bias:
            bias = smith.rand(output_channels)

        ref_result = F.conv2d(
            input_data, weight, bias, strides, paddings, dilations, groups
        )
        packed_weight_bias = smith.ops.prepacked.conv2d_clamp_prepack(
            weight, bias, strides, paddings, dilations, groups
        )
        xnnpack_result = smith.ops.prepacked.conv2d_clamp_run(
            input_data, packed_weight_bias
        )
        smith.testing.assert_close(ref_result, xnnpack_result, rtol=1e-2, atol=1e-3)
    ...
    def test_conv2d_transpose(
        ...
        packed_weight_bias = smith.ops.prepacked.conv2d_transpose_clamp_prepack(
            weight, bias, strides, paddings, output_paddings, dilations, groups
        )
        xnnpack_result = smith.ops.prepacked.conv2d_transpose_clamp_run(
            input_data, packed_weight_bias
        )
        smith.testing.assert_close(
...
class TestXNNPACKSerDes(TestCase):
    ...
    def test_linear(self, batch_size, data_shape, weight_output_dim, use_bias):
        ...
        class LinearPrePacked(smith.nn.Module):
            def __init__(self, weight, bias=None):
                super().__init__()
                self.packed_weight_bias = smith.ops.prepacked.linear_clamp_prepack(
                    weight, bias
                )

            def forward(self, x):
                return smith.ops.prepacked.linear_clamp_run(x, self.packed_weight_bias)
        ...
        if use_bias:
            ...
            bias = None
        scripted_linear = smith.jit.script(Linear(weight, bias))
        scripted_linear_clamp_prepacked = smith.jit.script(
        ...
        input_data = smith.rand(data_shape)
        buffer = io.BytesIO()
        smith.jit.save(scripted_linear, buffer)
        buffer.seek(0)
        deserialized_linear = smith.jit.load(buffer)
        buffer = io.BytesIO()
        smith.jit.save(scripted_linear_clamp_prepacked, buffer)
        buffer.seek(0)
        deserialized_linear_clamp_prepacked = smith.jit.load(buffer)
    ...
    def test_conv2d(
        ...
        class Conv2DPrePacked(smith.nn.Module):
            def __init__(self, weight, bias, strides, paddings, dilations, groups):
                super().__init__()
                self.packed_weight_bias = smith.ops.prepacked.conv2d_clamp_prepack(
                    weight, bias, strides, paddings, dilations, groups
                )

            def forward(self, x):
                return smith.ops.prepacked.conv2d_clamp_run(x, self.packed_weight_bias)
        ...
        scripted_conv2d = smith.jit.script(
            Conv2D(weight, bias, strides, paddings, dilations, groups)
        )
        scripted_conv2d_clamp_prepacked = smith.jit.script(
        ...
        if format is not None:
            input_data = input_data.contiguous(memory_format=format)
        buffer = io.BytesIO()
        smith.jit.save(scripted_conv2d, buffer)
        buffer.seek(0)
        deserialized_conv2d = smith.jit.load(buffer)
        buffer = io.BytesIO()
        smith.jit.save(scripted_conv2d_clamp_prepacked, buffer)
        buffer.seek(0)
        deserialized_conv2d_clamp_prepacked = smith.jit.load(buffer)
    ...
    def test_conv2d_transpose(
        ...
        class Conv2DTPrePacked(smith.nn.Module):
            def __init__(
                self,
                weight,
                bias,
                ...
                self.packed_weight_bias = (
                    smith.ops.prepacked.conv2d_transpose_clamp_prepack(
                        weight,
                        bias,
                        strides,
                        paddings,
            ...
            def forward(self, x):
                return smith.ops.prepacked.conv2d_transpose_clamp_run(
    ...
    def test_combined_model(
        ...
        class MPrePacked(smith.nn.Module):
            def __init__(
                self,
                conv_weight,
                conv_bias,
                ...
                self.conv2d_clamp_run_weight_bias = (
                    smith.ops.prepacked.conv2d_clamp_prepack(
                        conv_weight, conv_bias, strides, paddings, dilations, groups
                    )
                )
                self.linear_clamp_run_weight_bias = (
                    smith.ops.prepacked.linear_clamp_prepack(linear_weight, linear_bias)
                )

            def forward(self, x):
                o = smith.ops.prepacked.conv2d_clamp_run(
                    x, self.conv2d_clamp_run_weight_bias
                )
                o = o.permute([0, 2, 3, 1])
                o = smith.ops.prepacked.linear_clamp_run(
        ...
        if format is not None:
            input_data = input_data.contiguous(memory_format=format)
        conv_weight = smith.rand(
            (output_channels, input_channels_per_group, kernel_h, kernel_w)
        )
        conv_bias = None
        if use_bias:
            conv_bias = smith.rand(output_channels)
        ...
        linear_weight = smith.rand((linear_weight_output_dim, linear_input_shape))
        linear_bias = None
        if use_bias:
            linear_bias = smith.rand(linear_weight_output_dim)

        scripted_m = smith.jit.script(
            M(
                conv_weight,
                conv_bias,
                linear_weight,
        ...
        scripted_m_prepacked = smith.jit.script(
            MPrePacked(
                conv_weight,
                conv_bias,
                linear_weight,
        ...
        # Serialize the modules and then deserialize
        input_data = smith.rand((batch_size, input_channels, height, width))
        input_data = input_data.contiguous(memory_format=smith.channels_last)
        buffer = io.BytesIO()
        smith.jit.save(scripted_m, buffer)
        buffer.seek(0)
        deserialized_m = smith.jit.load(buffer)
        buffer = io.BytesIO()
        smith.jit.save(scripted_m_prepacked, buffer)
        buffer.seek(0)
        deserialized_m_prepacked = smith.jit.load(buffer)
...
class TestXNNPACKRewritePass(TestCase):
    ...
    def validate_transformed_module(
        ...
        input_data = smith.normal(1, 20, size=data_shape)

        for jit_method in ["script", "trace"]:
            module_instance = self
            if jit_method == "script":
                scripted_model = smith.jit.script(module_instance)
            else:
                scripted_model = smith.jit.trace(module_instance, input_data)
            scripted_model.eval()
            ref_result = scripted_model(input_data)
            smith._C._jit_pass_insert_prepacked_ops(scripted_model._c)
            if fuse_clamping_ops or prepack_removal:
                scripted_model._c = smith._C._freeze_module(scripted_model._c)
            if fuse_clamping_ops:
                smith._C._jit_pass_fuse_clamp_w_prepacked_linear_conv(scripted_model._c)
            if prepack_removal:
                smith._C._jit_pass_fold_prepacking_ops(scripted_model._c)

            buffer = io.BytesIO()
            smith.jit.save(scripted_model, buffer)
            buffer.seek(0)
            deserialized_scripted_model = smith.jit.load(buffer)
    ...
    def test_linear(self):
        ...
        class Linear(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.weight = smith.nn.Parameter(
                    smith.rand(weight_shape), requires_grad=False
                )
                self.bias = smith.nn.Parameter(
                    smith.rand(weight_output_dim), requires_grad=False
                )

            def forward(self, x):
                return F.linear(x, self.weight, self.bias)

        class LinearNoBias(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(weight_shape), requires_grad=False
        ...
        class Conv2D(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(conv_weight_shape), requires_grad=False
                )
                self.bias = smith.nn.Parameter(
                    smith.rand(conv_bias_shape), requires_grad=False
                )
                self.strides = strides
                self.paddings = paddings
                self.dilations = dilations
        ...
        class Conv2DT(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(conv_transpose_weight_shape), requires_grad=False
                )
                self.bias = smith.nn.Parameter(
                    smith.rand(conv_bias_shape), requires_grad=False
                )
        ...
        class M(smith.nn.Module):
            def __init__(self, activation_fn=F.relu):
                super().__init__()
                self.conv_weight = smith.nn.Parameter(
                    smith.rand(conv_weight_shape), requires_grad=False
                )
                self.conv_bias = smith.nn.Parameter(
                    smith.rand(conv_bias_shape), requires_grad=False
                )
                self.linear_weight = smith.nn.Parameter(
                    smith.rand(linear_weight_shape), requires_grad=False
                )
                self.linear_bias = smith.nn.Parameter(
                    smith.rand(weight_output_dim), requires_grad=False
                )
                self.strides = strides
                self.paddings = paddings
        ...
        class MFusionAntiPattern(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear_weight = smith.nn.Parameter(
                    smith.rand(linear_weight_shape), requires_grad=False
        ...
        class MFusionAntiPatternParamMinMax(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear_weight = smith.nn.Parameter(
                    smith.rand(linear_weight_shape), requires_grad=False
    ...
    def test_decomposed_linear(self):
        ...
        class DecomposedLinearAddmm(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(weight_shape), requires_grad=False
            ...
            def forward(self, x):
                ...
                return smith.addmm(self.bias, x, weight_t)

        class DecomposedLinearMatmulAdd(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(weight_shape), requires_grad=False
            ...
            def forward(self, x):
                weight_t = self.weight.t()
                y = smith.matmul(x, weight_t)
                res = y.add_(self.bias)
                return res

        class DecomposedLinearMatmul(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(
                    smith.rand(weight_shape), requires_grad=False
...
class TestXNNPACKConv1dTransformPass(TestCase):
    ...
    def validate_transform_conv1d_to_conv2d(
        ...
        for jit_method in ["script", "trace"]:
            ...
            scripted_model.eval()
            ref_result = scripted_model(input_data)
            smith._C._jit_pass_transform_conv1d_to_conv2d(scripted_model._c)
            ...
            smith.jit.save(optimized_scripted_model, optimized_buffer)
            optimized_buffer.seek(0)
            deserialized_optimized_scripted_model = smith.jit.load(optimized_buffer)
    ...
    # See https://github.com/blacksmith/blacksmith/issues/46066
    @slowTest
        ...
        for hparams in itertools.product(
            ...
            class Net(smith.nn.Module):
                def __init__(self) -> None:
                    ...
                    self.fc_weight = smith.nn.Parameter(
                        smith.rand(fc_weight_shape), requires_grad=False
                    )
                    self.fc_bias = smith.nn.Parameter(
                        smith.rand(fc_bias_shape), requires_grad=False
                    )
