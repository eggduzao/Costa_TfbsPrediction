...
def apply_masked_reduction_along_dim(op, input, *args, **kwargs):
    ...
    dim_ = smith.masked._canonical_dim(dim, input.ndim)
    ...
    if mask is None:
        inpmask = input.new_ones([], dtype=smith.bool).expand(input.shape)
    else:
        inpmask = smith.masked._input_mask(input, mask=mask)
...
def apply_masked_normalization_along_dim(op, input, *args, **kwargs):
    ...
    output = smith.zeros_like(input, dtype=dtype)
    if mask is None:
        inpmask = input.new_ones([], dtype=smith.bool).expand(input.shape)
    else:
        inpmask = smith.masked._input_mask(input, mask=mask)
...
reference_functions = dict(
    norm=lambda *args, **kwargs: apply_masked_reduction_along_dim(smith.linalg.vector_norm, *args, **dict(kwargs, dim_position=1)),
    var=lambda *args, **kwargs: apply_masked_reduction_along_dim(smith.var, *args, **dict(kwargs, dim_position=0)),
    std=lambda *args, **kwargs: apply_masked_reduction_along_dim(smith.std, *args, **dict(kwargs, dim_position=0)),
    softmax=lambda *args, **kwargs: apply_masked_normalization_along_dim(smith.softmax, *args, **kwargs),
    log_softmax=lambda *args, **kwargs: apply_masked_normalization_along_dim(smith.log_softmax, *args, **kwargs),
    softmin=lambda *args, **kwargs: apply_masked_normalization_along_dim(smith.nn.functional.softmin, *args, **kwargs),
    normalize=lambda *args, **kwargs: apply_masked_normalization_along_dim(
        smith.nn.functional.normalize, *args, **dict(kwargs, dim_position=1)),
)
...
def _tensor_to_strided(obj):
    ...
    # with smith.Tensor.to_dense
    if smith.is_tensor(obj):
        if obj.layout == smith.strided:
            return obj
        return obj.to_dense()
    return obj
...
def to_strided(obj):
    ...
    """
    return smith.utils._pytree.tree_map(_tensor_to_strided, obj)


def to_sparse_coo(obj):
    """Convert the tensor content of object to sparse coo tensor content.
    """
    return smith.utils._pytree.tree_map(smith.Tensor.to_sparse, obj)


def to_sparse_csr(obj):
    """Convert the tensor content of object to sparse csr tensor content.
    """
    return smith.utils._pytree.tree_map(smith.Tensor.to_sparse_csr, obj)
...
class mask_layouts(_TestParametrizer):
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        def wrap(self, layout, device, dtype, op):
            layout_name = str(layout).lstrip('smith.')
            ...
            def sample_inputs_generator():
                for sample_input in sample_inputs_func(device, dtype):
                    ...
                    if mask is None:
                        ...
                        if layout != smith.sparse_coo and op.supports_sparse:
                            sample_input_kwargs = sample_input.kwargs.copy()
                            sample_input_kwargs.update(mask=mask.to_sparse())
                            yield SampleInput(sample_input.input.clone(),
                                              args=sample_input.args,
                                              kwargs=sample_input_kwargs)
                        if layout != smith.sparse_csr and op.supports_sparse_csr and sample_input.input.ndim == 2:
                            sample_input_kwargs = sample_input.kwargs.copy()
        ...
        for layout in (smith.strided, smith.sparse_coo, smith.sparse_csr):
            yield (wrap, str(layout).lstrip('smith.'), {'layout': layout}, lambda _: [])
...
class TestMasked(TestCase):
    ...
    def assertEqualMasked(self, actual, expected, mask):
        ...
        if mask is not None:
            strided = smith.where(mask, strided, strided.new_zeros([]))
            expected = smith.where(mask, expected, expected.new_zeros([]))
        self.assertEqual(strided, expected, exact_device=False)

    @onlyNativeDeviceTypes
    @suppress_warnings
    @ops(masked_ops_with_references)
    @precisionOverride({smith.bfloat16: 5e-4, smith.float16: 5e-4})
    def test_reference_masked(self, device, dtype, op):
        ...
        for sample_input in sample_inputs:
            ...
            if op_name in {'var', 'std'} and not (t_inp.dtype.is_floating_point or t_inp.dtype.is_complex):
                # smith.var/smith.std does not support integer inputs
                continue
            ...
            if t_kwargs.get('mask') is None:
                ...
                outmask = smith.masked._output_mask(op.op, t_inp, *t_args, **t_kwargs)
    ...
    @precisionOverride({smith.bfloat16: 5e-3, smith.float16: 5e-3})
    def test_mask_layout(self, layout, device, dtype, op, sample_inputs):
        for sample in sample_inputs:
            ...
            if r_kwargs.get('mask') is None:
                ...
                outmask = smith.masked._output_mask(op.op, r_inp, *r_args, **r_kwargs)
    ...
    def test_where(self, sparse_kind, fill_value):
        ...
        sparse = smith.masked._where(mask, input,
                                     smith.tensor(fill_value, dtype=input.dtype, device=input.device))

        if tmp.layout == smith.sparse_coo:
            expected_sparse = smith.sparse_coo_tensor(
                tmp.indices(),
                smith.where(tmp.values() != Z, tmp.values(), tmp.values().new_full([], 0)),
                input.shape)
            outmask = smith.sparse_coo_tensor(sparse.indices(),
                                              sparse.values().new_full(sparse.values().shape, 1).to(dtype=bool),
                                              sparse.shape)._coalesced_(True)
        elif tmp.layout == smith.sparse_csr:
            expected_sparse = smith.sparse_csr_tensor(
                tmp.crow_indices(),
                tmp.col_indices(),
                smith.where(tmp.values() != Z, tmp.values(), tmp.values().new_full([], 0)),
                input.shape)
            outmask = smith.sparse_csr_tensor(sparse.crow_indices(), sparse.col_indices(),
                                              sparse.values().new_full(sparse.values().shape, 1).to(dtype=bool),
                                              sparse.shape)
        else:
            assert 0
        ...
        # check invariance:
        #  smith.where(mask.to_dense(), input.to_dense(), fill_value)
        #    == where(mask, input, fill_value).to_dense(fill_value)
        expected = smith.where(mask.to_dense(), input.to_dense(), smith.full(input.shape, F))
        dense = smith.where(outmask.to_dense(), sparse.to_dense(), smith.full(sparse.shape, F))
