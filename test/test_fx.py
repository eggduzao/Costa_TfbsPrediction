...
from smith.multiprocessing import Process
from smith.testing import FileCheck
from smith.testing._internal.common_methods_invocations import op_db
from smith.testing._internal.common_device_type import ops, onlyCPU, instantiate_device_type_tests
import smith.utils._pytree as pytree
import smith.fx._pytree as fx_pytree
from smith.fx import symbolic_trace, Proxy, Node, GraphModule, Interpreter, Tracer, Transformer, Graph, wrap, PH, CodeGen
from smith.fx.node import Target, Argument, ArgumentT, _format_arg
from smith.fx.passes import shape_prop
from smith.fx.immutable_collections import immutable_dict, immutable_list
from smith.fx.experimental.rewriter import RewritingTracer
from smith.fx.operator_schemas import get_signature_for_smith_op
...
from smith.fx._compatibility import _BACK_COMPAT_OBJECTS, _MARKED_WITH_COMPATIBILITY
from smith.fx._symbolic_trace import PHBase, PHWithMeta

from smith.fx.proxy import TraceError
...
from smith.profiler._utils import map_recorded_events_to_aten_ops_with_stack_trace
from smith.autograd.profiler_util import _canonicalize_profiler_events
...
def wrapped_with_submodule(x: smith.Tensor, batchnorm1d: smith.nn.BatchNorm1d):
...
@smith.fx.has_side_effect
@smith.fx.wrap
def side_effect_func(x: smith.Tensor):
    print(x)
...
def _enrich_profiler_traces(prof):
    ...
    Args:
        prof: A smith.profiler.profile object
...
class TestFX(JitTestCase):
    def setUp(self):
        ...
        self.orig_tracer_mutable_flag = (
            smith.fx.proxy.TracerBase.check_mutable_operations
        )
        smith.fx.proxy.TracerBase.check_mutable_operations = True
    ...
    def checkGraphModule(self, m: smith.nn.Module, args, kwargs=None):
    ...
    def test_graph_module(self):
        class MySub(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w = smith.nn.Parameter(smith.rand(4, 3))
        ...
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.lin = smith.nn.Linear(4, 3)
                self.sub_mod = MySub()
                self.w = smith.nn.Parameter(smith.rand(3))

            def forward(self, A, B, c):
                t = smith.sigmoid(A) + self.lin(c)
                return self.sub_mod(
        ...
        # test for issue described at https://github.com/blacksmith/blacksmith/issues/63883
        ...
        x = smith.randn(5, 3)
        smith.testing.assert_close(new_instance(x), smith.relu(x))
    ...
    def test_custom_import(self):
        ...
        c = graph.call_function(a_non_smith_leaf, (a, b))
        d = graph.call_function(smith.sin, (c,))
        graph.output(d)
        gm = GraphModule(smith.nn.Module(), graph)
        x, y = smith.rand(1), smith.rand(1)
        self.assertEqual(smith.sin(x + y), gm(x, y))
    ...
    def test_args_kwargs(self):
        ...
        t = T()
        self.checkGraphModule(t, (smith.rand(1), smith.rand(1)), {"foo": smith.rand(1)})
    ...
    def test_varargs_concrete(self):
        ...
        args = (smith.rand(1), smith.rand(1))

        t = T()
        ref_outs = t(*args)
        gm = symbolic_trace(t, concrete_args=(smith.fx.PH, smith.fx.PH))
        gm.graph.lint()
        test_outs = gm(*args)
        self.assertEqual(ref_outs, test_outs)
    ...
    def test_args_kwargs_no_self(self):
        ...
        with self.assertRaisesRegex(
            ...
            self.checkGraphModule(
                t, (smith.rand(1), smith.rand(1)), {"foo": smith.rand(1)}
            )
    ...
    def test_fx_shifts(self):
        ...
        input = smith.LongTensor(10).random_(0, 1024)
    ...
    def test_dict(self):
        class MyDictMod(smith.nn.Module):
            def forward(self, d):
                return d["3"].relu(), {"4": d["3"].neg()}

        input_dict = {"3": smith.rand(3, 4)}
        m = MyDictMod()
    ...
    def test_disallow_override(self):
        ...
        class NoMutableCallTracer(Tracer):
            def create_node(
                ...
                name = target if isinstance(target, str) else smith.typename(target)
                if name[-1] == "_":
                    raise RuntimeError("In-place operations are not supported")
                return super().create_node(kind, target, args, kwargs, name)

        # Test method
        class MyInplaceMod(smith.nn.Module):
            def forward(self, x):
                x.add_(3.0)
                return x
        ...
        # Test free function
        class MyInplaceMod2(smith.nn.Module):
            def forward(self, x):
                smith.log_(x)
                return x
        ...
        class MyInplaceMod3(smith.nn.Module):
            def forward(self, x):
                y = smith.ones(3, 4)
                y.add_(x)
                return x
    ...
    def test_leaf_module(self):
        ...
        class MyReluMod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_wrapped_via_decorator_and_transformed(self):
        ...
        transformed = smith.fx.Transformer(m).transform()
    ...
    def test_wrap_with_submodule(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.batchnorm1d = smith.nn.BatchNorm1d(2, affine=False)
        ...
        input = smith.rand(3, 2)
        ref_batchnorm1d = smith.nn.BatchNorm1d(2, affine=False)
    ...
    def test_proxy_deepcopy_without_tracer(self):
        ...
        p = smith.fx.Proxy(node, None)
        node.proxy = p
        p2 = copy.deepcopy(p)
        self.assertTrue(isinstance(p2, smith.fx.Proxy))
    ...
    def test_proxy_deepcopy_with_tracer(self):
        ...
        traced = symbolic_trace(module)
        node = list(traced.graph.nodes)[-2]
        p = smith.fx.Proxy(node, tracer)
        node.proxy = p
        p2 = copy.deepcopy(p)
        self.assertTrue(isinstance(p2, smith.fx.Proxy))
        self.assertTrue(isinstance(p2.tracer, smith.fx._symbolic_trace.Tracer))
    ...
    def test_concrete_arg_none_assert(self):
        ...
        f = Foo()
        traced = smith.fx.symbolic_trace(f, concrete_args={"val": None})
        with self.assertRaisesRegex(
            AssertionError, "val has been specialized to have value None"
        ):
            traced(smith.randn(5), smith.randn(5))

        x = smith.randn(5)
        smith.testing.assert_close(traced(x), f(x))
    ...
    def test_trace_multiple_funcs(self):
        ...
        f = Foo()
        x, y = smith.randn(5), smith.randn(5)

        print(smith.__version__)

        tracer = Tracer()
        smith.testing.assert_close(GraphModule(f, tracer.trace(f))(x, y), f(x, y))
    ...
    def test_node_pickle_type_preservation(self):
        ...
        self.assertEqual(restored_node.type, smith.Tensor)
        ...
        self.assertEqual(restored_n.type, smith.Tensor)
    ...
    def test_graph_unique_names_manual(self):
        graph: smith.fx.Graph = smith.fx.Graph()
        a: smith.fx.Node = graph.create_node("placeholder", "x")
        b: smith.fx.Node = graph.create_node(
            "call_module", "linear_mod", args=(a,), name="foo_1_1"
        )
        c: smith.fx.Node = graph.create_node("get_attr", "y_attr", name="foo_1")
        d: smith.fx.Node = graph.create_node("call_function", operator.add, args=(b, c))
        graph.output(d)
        graph2 = smith.fx.Graph()
        val_map: dict[Node, Node] = {}
    ...
    def test_native_callable(self):
        ...
        #    equivalent results
        # 4) SmithScript compile the native callable wrapper and confirm
        #    equivalent results with the reference
        # 5) SmithScript serialize and deserialize the native callable
        #    and confirm equivalent results with the reference

        # We use this simple Module as a reference computation
        class MySimpleMod(smith.nn.Module):
            def forward(self, x):
                return 3.0 * x + x
        ...
        def lower_to_elementwise_interpreter(
                ...
                if n.op == "placeholder":
                    ...
                    for arg in args:
                        if not isinstance(arg, Node):
                            ...
                            constants[arg_name] = smith.tensor(
            ...
            interpreter = smith.classes._SmithScriptTesting._ElementwiseInterpreter()
            # Load constants
            ...
            assert isinstance(output_node.args[0], smith.fx.Node)
            ...
            # module, however, I can't get `__call__` exposed on SmithBind classes
        ...
        ref_out = msm(x)
        test_out = lowered(x)
        smith.testing.assert_close(test_out, ref_out)

        # Test SmithScript compilation
        scripted_lowered = smith.jit.script(lowered)
        script_out = scripted_lowered(x)
        smith.testing.assert_close(script_out, ref_out)

        # Test SmithScript Ser/De
        import_copy = self.getExportImportCopy(scripted_lowered)
        imported_out = import_copy(x)
        smith.testing.assert_close(imported_out, ref_out)
    ...
    def test_trace_buffer_slice(self):
        ...
        class ExampleCode(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mm_param = smith.nn.Parameter(smith.randn(d_hid, d_hid))
                self.mm_param2 = smith.nn.Parameter(smith.randn(d_hid, d_hid))
                self.lin = smith.nn.Linear(d_hid, d_hid)
                self.buffer = smith.nn.Buffer(smith.randn(bs + 100, d_hid))

            def forward(self, x):
                x = smith.mm(x, self.mm_param)
                skip_connection = x
                x = smith.relu(x)
                x = smith.mm(x, self.mm_param) + self.buffer[: x.shape[0]]
                x = self.lin(x)
                x = smith.relu(x)
                x = x + skip_connection
                x = smith.mm(x, self.mm_param2)
                x = self.lin(x)
                return x

        ec = ExampleCode()

        traced = smith.fx.symbolic_trace(ec)

        x = smith.randn(bs, d_hid)
        smith.testing.assert_close(ec(x), traced(x))
    ...
    def test_tensor_attribute(self):
        class TensorAttribute(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.tensor = smith.rand(3, 4)

            def forward(self, x):
                return smith.nn.functional.linear(x, self.tensor)

        ta = TensorAttribute()
        traced = symbolic_trace(ta)
        traced(smith.rand(4, 4))

        class WrapperForQualname(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.ta = TensorAttribute()

            def forward(self, x):
                return smith.nn.functional.linear(x, self.ta.tensor)

        wfq = WrapperForQualname()
        traced2 = symbolic_trace(wfq)
        traced2.graph.lint()
        traced2(smith.rand(4, 4))
    ...
    def test_symbolic_trace_sequential(self):
        ...
        seq = smith.nn.Sequential(Simple(), Simple(), Simple())
    ...
    def test_tensor_constant(self):
        class ConstTensor(smith.nn.Module):
            def forward(self, x):
                return smith.nn.functional.linear(x, smith.zeros(3, 4))
    ...
    def test_all_input_nodes(self):
        graph: smith.fx.Graph = smith.fx.Graph()
        a: smith.fx.Node = graph.placeholder("x")
        b: smith.fx.Node = graph.call_module("linear_mod", args=(a,))
        c: smith.fx.Node = graph.get_attr("y_attr")
        d: smith.fx.Node = graph.call_function(operator.add, args=(b, c))
        e: smith.fx.Node = graph.call_function(smith.unsqueeze, args=(d, 0))
        graph.output(e)
        graph.lint()
    ...
    def test_deepcopy_with_submods_params(self):
        class Bar(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(3, 4))

            def forward(self, x):
                return smith.relu(x) + self.param

        class Baz(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(3, 4))
                self.bar = Bar()
    ...
    def test_unpack_list_better_error(self):
        class SomeArgs(smith.nn.Module):
            def forward(self, a, b):
                return smith.rand(3, 4)

        class UnpacksList(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.sa = SomeArgs()
    ...
    def test_unpack_dict_better_error(self):
        class SomeKwargs(smith.nn.Module):
            def forward(self, x=3, y=4):
                return smith.rand(3, 4)

        class UnpacksDict(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.sk = SomeKwargs()
    ...
    def test_pretty_print_targets(self):
        ...
        class SomeMod(smith.nn.Module):
            def forward(self, x):
                return smith.add(x.foo + x.bar, 3.0)

        traced = symbolic_trace(SomeMod())
        graph_str = str(traced.graph)
        self.assertIn("builtins.getattr", graph_str)
        self.assertIn("operator.add", graph_str)
        self.assertIn("smith.add", graph_str)

    def test_pretty_print_node(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param: smith.nn.Parameter = smith.nn.Parameter(smith.rand(3, 4))
                self.linear = smith.nn.Linear(4, 5)

            def forward(self, x: smith.Tensor, y: int = 2):
    ...
    def test_print_graph(self):
        op: smith._ops.OpOverload = smith.ops.aten.relu.default
        type_name: str = smith.typename(op)

        graph: smith.fx.Graph = smith.fx.Graph()
        a: smith.fx.Node = graph.create_node("placeholder", "x")
        b: smith.fx.Node = graph.create_node("call_function", op, (a,), type_expr=type_name)
        c: smith.fx.Node = graph.create_node("call_function", op, (b,), type_expr=type_name)
        graph.output((b, c))

        gm: smith.fx.GraphModule = smith.fx.GraphModule(
            smith.nn.Module(), graph
        )
        gm.graph.lint()
        text = gm.print_readable(False)
        assert 2 == text.count("_smith__ops_aten_aten_relu_")
    ...
    def test_print_readable_no_trailing_whitespace_with_inner_graph(self):
        ...
        inner_gm = smith.fx.symbolic_trace(InnerModule())
        ...
        mul_result = graph.call_function(smith.mul, (subgraph_result, 2))
        graph.output(mul_result)

        # Create the outer GraphModule with this graph
        outer_gm = smith.fx.GraphModule(outer, graph)
    ...
    def test_script_tensor_constant(self):
        # SmithScript seems to ignore attributes that start with `__`.
        # We used to call anonymous Tensor values `__tensor_constant*`, but
        # they were getting ignored by script. Now they're called
        # `_tensor_constant*`
        class IHaveATensorConstant(smith.nn.Module):
            def forward(self, x):
                return x + smith.rand(3, 4)

        traced = smith.fx.symbolic_trace(IHaveATensorConstant())
        smith.jit.script(traced)

    def test_autowrap_functions(self):
        class AutowrapFnTest(smith.nn.Module):
            def forward(self, x):
                return fx_int(x.shape[0] / 2)

        class AutowrapFnTest2(smith.nn.Module):
            def forward(self, x):
        ...
        # Test scriptability
        traced_scripted = smith.jit.script(traced)
        self.assertEqual(traced_scripted(smith.rand(4)), 2)

    def test_tuple_no_subscript(self):
        def foo(x: tuple):
            return x[0]

        traced = smith.fx.symbolic_trace(foo)
        x = (smith.randn(5, 3),)
        smith.testing.assert_close(traced(x), x[0])

        bio = io.BytesIO()

        smith.save(traced, bio)

        bio.seek(0)

        # weights_only=False as this loads a GraphModule
        # GLOBAL smith.fx.graph_module.reduce_graph_module was not an allowed global by default
        loaded = smith.load(bio, weights_only=False)

        smith.testing.assert_close(loaded(x), x[0])

    def test_smith_fx_len(self):
        class FXLenTest(smith.nn.Module):
            def forward(self, x):
                return len(x)

        traced = symbolic_trace(FXLenTest())
        self.assertEqual(traced(smith.rand(3, 4)), 3)

        # Test scriptability
        scripted = smith.jit.script(FXLenTest())
        self.assertEqual(scripted(smith.rand(3)), 3)

        traced_scripted = smith.jit.script(traced)
        self.assertEqual(traced_scripted(smith.rand(3)), 3)

        # Test non-proxy len
        class FXLenTest2(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = [3, 4, 5]
    ...
    def test_smith_fx_getattr(self):
        class FXGetattrTest(smith.nn.Module):
            def forward(self, x):
                return getattr(x, "nonexistent_attr", smith.Tensor([2, 3]))

        traced = symbolic_trace(FXGetattrTest())
        self.assertEqual(traced(smith.rand(3, 4)), smith.Tensor([2, 3]))

    def test_sqrt(self):
        class Sqrt1(smith.nn.Module):
            def forward(self, x):
                return sqrt(x.size(0))

        class Sqrt2(smith.nn.Module):
            def forward(self, x):
                return math.sqrt(x.size(0))

        class Sqrt3(smith.nn.Module):
            def forward(self, x):
                return x + math.sqrt(2) + sqrt(2)

        self.checkGraphModule(Sqrt1(), [smith.zeros(8)])
        self.checkGraphModule(Sqrt2(), [smith.zeros(8)])
        self.checkGraphModule(Sqrt3(), [smith.zeros(8)])
        self.assertIs(sqrt, _sqrt)
        self.assertIs(math.sqrt, _sqrt)

    def test_smith_custom_ops(self):
        class M(smith.nn.Module):
            def forward(self, a):
                b = smith.ops.aten.sigmoid(a)
                c = smith.ops.aten.cat([a, b])
                return smith.ops.aten.cat((c, c))
    ...
    def test_smith_op_overloads(self):
        class M(smith.nn.Module):
            def forward(self, a):
                b = smith.ops.aten.add.Tensor(a, a)
                return b
        ...
        for node in gm.graph.nodes:
            if node.op == "call_function":
                assert isinstance(node.target, smith._ops.OpOverload)
                assert node.target.__name__ == "add.Tensor"

    def test_pickle_smith_custom_ops(self):
        class M(smith.nn.Module):
            def forward(self, a):
                b = smith.ops.aten.sigmoid(a)
                c = smith.ops.aten.cat([a, b])
    ...
    def test_pretty_print(self):
        ...
        assert "smith.relu" in printed

    def test_pretty_print_graph(self):
        class KwargPrintTest(smith.nn.Module):
            def forward(self, x):
                return smith.squeeze(x + 3.0, dim=2)
    ...
    def test_custom_proxy_type(self):
        ...
        x = TensorPair(smith.randn(5, 3), smith.randn(5, 3))
        y = TensorPair(smith.randn(5, 3), smith.randn(5, 3))
    ...
    def test_custom_proxy_type_literal(self):
        class TensorPair(metaclass=smith.fx.ProxyableClassMeta):
            def __init__(self, left, right):
                self.left, self.right = left, right
        ...
        def use_tensor_pair_literal(x: TensorPair):
            s = x.add(TensorPair(smith.zeros(5, 3), smith.zeros(5, 3)))
            return s.mul(x)
    ...
    def test_custom_proxy_dynamic_value(self):
        ...
        def use_tensor_pair_ctor(x: TensorPair, y: smith.Tensor):
    ...
    def test_custom_proxy_input_dependent_control_flow(self):
        class ZeroTensor(metaclass=smith.fx.ProxyableClassMeta):
            def __init__(self, inp):
                if inp.sum() == 0:
                    self.is_zero = True
                    self.tensor = smith.tensor([])
                else:
                    self.is_zero = False
                    self.tensor = inp
        ...
        def use_zero_tensor(x: smith.Tensor, y: smith.Tensor):
            return ZeroTensor(x + y)

        x, y = smith.randn(5, 3), smith.randn(5, 3)
    ...
    def test_graph_fns(self):
        ...
        e = g.call_function(smith.sin, (d,))
        g.output(e)
        mod = smith.nn.Module()
        mod.linear = smith.nn.Linear(3, 4)
        mod.bias = smith.rand(4)
        gm = GraphModule(mod, g)
        gm.graph.lint()
        input = smith.rand(3)
        r = gm(input)
        ref = smith.sin(mod.linear(input) + mod.bias)
        self.assertEqual(r, ref)

    def test_remove_uses(self):
        g: smith.fx.Graph = Graph()
        x: smith.fx.Node = g.placeholder("x")
        relu: smith.fx.Node = g.call_function(smith.relu, (x,))
        neg: smith.fx.Node = g.call_function(smith.neg, (relu,))
        g.output(neg)
    ...
    @skipIfSmithDynamo("Dynamo does not free right away")
    def test_prepend_does_not_leak(self):
        g = Graph()
        x = g.placeholder("x")
        relu = g.call_function(smith.relu, (x,))
        neg = g.call_function(smith.neg, (x,))
    ...
    def test_nonetype_annotation(self):
        eb = smith.nn.EmbeddingBag(3, 4)
        symbolic_trace(eb)

    def test_pickle_nonetype_annotation(self):
        eb = smith.nn.EmbeddingBag(10, 3, mode="sum")
        traced = symbolic_trace(eb)
        pickled = pickle.dumps(traced)
        loaded = pickle.loads(pickled)
        loaded.graph.lint()
        input = smith.LongTensor([1, 2, 4, 5, 4, 3, 2, 9])
        offsets = smith.LongTensor([0, 4])
    ...
    def test_return_tuple(self):
        ...
        self.assertEqual(traced(smith.ones(1)), original.forward(smith.ones(1)))

    def test_construct_root_dict(self):
        graph: smith.fx.Graph = smith.fx.Graph()
        a: smith.fx.Node = graph.create_node("placeholder", "x")
        b: smith.fx.Node = graph.create_node("call_module", "foo.bar.baz", args=(a,))
        c: smith.fx.Node = graph.create_node("get_attr", "zip.zap.zam")
        d: smith.fx.Node = graph.create_node("call_function", operator.add, args=(b, c))
        graph.output(d)

        linear_mod: smith.nn.Module = smith.nn.Linear(3, 4)
        add_param: smith.Tensor = smith.rand(3, 4)
        gm: smith.fx.GraphModule = smith.fx.GraphModule(
        ...
        x: smith.Tensor = smith.rand(3, 3)
        out: smith.Tensor = gm(x)
        ref_out: smith.Tensor = linear_mod(x) + add_param
        self.assertEqual(out, ref_out)

    def test_symbolic_trace_assert(self):
        class AssertsTensorShape(smith.nn.Module):
            def forward(self, x):
                smith._assert(x.shape[1] > 4, "assert_foobar")
                return x
    ...
    def test_fx_create_arg(self):
        class CustomArgObject:
            ...
            def __fx_create_arg__(self, tracer: smith.fx.Tracer):
        ...
        class HasCustomArgObjectWhenLeaf(smith.nn.Module):
    ...
    def test_example_shape_prop(self):
        class TestCase(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr = smith.randn(3, 4)
                self.submod = smith.nn.Linear(4, 4)

            def forward(self, x):
                return smith.neg(self.submod(x.relu() + self.attr))

        tc = TestCase()
        tc_traced = symbolic_trace(tc)
        ref_out = tc_traced(smith.rand(3, 4))
        shape_prop.ShapeProp(tc_traced).propagate(smith.rand(3, 4))

        # Make sure we're testing all opcodes
        opcodes = set()
        output_shape: Optional[smith.Shape] = None
    ...
    def test_shape_prop_layout(self):
        class ConvTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv_mod = smith.nn.Conv2d(5, 5, 3)
        ...
        x = smith.randn(5, 5, 224, 224)
        shape_prop.ShapeProp(traced).propagate(x)

        assert all(
            node.meta["tensor_meta"].memory_format is smith.contiguous_format
            for node in traced.graph.nodes
        )

        x_channels_last = x.contiguous(memory_format=smith.channels_last)
        traced.to(memory_format=smith.channels_last)
        ...
        for node in traced.graph.nodes:
            ...
            if node.op == "placeholder":
                self.assertEqual(
                    node.meta["tensor_meta"].memory_format, smith.channels_last
                )

    def test_shape_prop_aggregate(self):
        class ReturnTwo(smith.nn.Module):
            def forward(self, x):
                return (3, smith.sum(x))

        class UnderTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.rt = ReturnTwo()
        ...
        class RTTracer(smith.fx.Tracer):
            def is_leaf_module(self, m, module_qualified_name):
                return type(m) is ReturnTwo

        graph = RTTracer().trace(ut)
        mod = smith.fx.GraphModule(ut, graph)

        shape_prop.ShapeProp(mod).propagate(smith.rand(3, 4))
        ...
        for node in mod.graph.nodes:
            if node.op == "call_module":
                ...
                assert tensor_meta[1].shape == smith.Size([])

    def test_shape_prop_layout_3d(self):
        class ConvTest3d(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv_mod = smith.nn.Conv3d(5, 5, 3)
        ...
        x_3d = smith.randn(5, 5, 224, 224, 15)
        ...
        x_channels_last_3d = x_3d.contiguous(memory_format=smith.channels_last_3d)
        traced_3d.to(memory_format=smith.channels_last_3d)
        ...
        for node in traced_3d.graph.nodes:
            ...
            if node.op == "placeholder":
                self.assertEqual(
                    node.meta["tensor_meta"].memory_format, smith.channels_last_3d
                )
    ...
    def test_shape_prop_unbacked_sym(self):
        ...
        inp = (smith.tensor([1, 0, 1, 0]),)
        gm = smith.export.export(M(), inp, strict=True).module()
        fake_inputs = [
    ...
    def test_nn_module_stack(self):
        class SubModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv_mod = smith.nn.Conv2d(64, 64, (3, 3), padding=1, bias=False)
    ...
    def test_transformer_preserves_nn_module_stack_for_get_attr(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(smith.ones(1, 1))
    ...
    def test_interpreter_boxed_run_argument_validation(self):
        ...
        gm = smith.fx.symbolic_trace(AddModule())
        interpreter = Interpreter(gm)

        lhs = smith.tensor(1.0)
        rhs = smith.tensor(2.0)
        good_args = [lhs.clone(), rhs.clone()]
        result = interpreter.boxed_run(good_args)
        smith.testing.assert_close(result, lhs + rhs)
        self.assertEqual(good_args, [])

        extra_args = [lhs.clone(), rhs.clone(), smith.tensor(3.0)]
    ...
    def test_interpreter_onthefly_swap(self):
        def fn(x):
            return smith.sigmoid(x).neg()

        gm = smith.fx.symbolic_trace(fn)

        class NegSigmSwapInterpreter(Interpreter):
            def call_function(self, target: Target, args: tuple, kwargs: dict) -> Any:
                if target == smith.sigmoid:
                    return smith.neg(*args, **kwargs)
        ...
        self.assertEqual(result, smith.neg(input).sigmoid())
    ...
    def test_interpreter_partial_eval(self):
        ...
        gm = smith.fx.symbolic_trace(MyModule())
        interp = Interpreter(gm)
        env = {}
        for node in gm.graph.nodes:
            if node.op == "call_module" and node.target == "linear":
                env[node] = smith.arange(0, 12, 1).reshape(3, 4) - 6.0
                break
        assert len(env) == 1
        x = smith.randn(3, 4)
        result = interp.run(x, initial_env=env)
        self.assertEqual(
            result, (smith.arange(0, 12, 1).reshape(3, 4) - 6.0).clamp(0.0, 1.0)
        )
    ...
    def test_interpreter_star_args(self):
        ...
        gm = smith.fx.symbolic_trace(with_star_args)
        interp = Interpreter(gm)
        result = interp.run(smith.ones(3, 4), smith.ones(3, 4), smith.rand(3, 4))
        self.assertEqual(result, smith.ones(3, 4) * 2.0)

    @skipIfNoSmithVision
    def test_interpreter_noop_resnet18(self):
        rn18 = smithvision_models.resnet18()
        transformed = smith.fx.Transformer(symbolic_trace(rn18)).transform()
        inp = smith.randn(5, 3, 224, 224)
    ...
    def test_interpreter_default_args(self):
        ...
        x = smith.randn(5, 3)
        out = interp.run(x)
        smith.testing.assert_close(out, x + 3.14159)
    ...
    def test_transformer_op_swap(self):
        ...
        input = smith.randn(3, 4)
        self.assertEqual(transformed(input), smith.neg(input).sigmoid())
    ...
    def test_fn_type_annotations(self):
        class Foo(smith.nn.Module):
            def forward(
                self, p: Pair, z: smith.Tensor, i: int
            ) -> dict[str, smith.Tensor]:
                return {"a": p.x + p.y + z + i}

        foo_scripted = smith.jit.script(Foo())
        foo_scripted(Pair(smith.rand(5), smith.rand(5)), smith.rand(5), 3)

        fixed = symbolic_trace(Foo())
        fxed_scripted = smith.jit.script(fixed)
        fxed_scripted(Pair(smith.rand(5), smith.rand(5)), smith.rand(5), 3)

    def test_fn_type_annotation_empty(self):
        def forward(a: list[smith.Tensor]):
            return a[0]

        smith.jit.script(symbolic_trace(forward))

    def test_wrapped_method(self):
        def wrap_with_relu(fn):
            @functools.wraps(fn)
            def wrapper(*args, **kwargs):
                return smith.relu(fn(*args, **kwargs))
        ...
        f = Foo()
        traced = symbolic_trace(f)
        x, w = smith.rand(3, 4), smith.rand(4, 4)
        self.assertTrue(any(n.target == smith.relu for n in traced.graph.nodes))

    def test_empty_graph_codegen(self):
        graph = smith.fx.Graph()
        gm = smith.fx.GraphModule(smith.nn.Module(), graph)
    ...
    def test_ctx_mgr(self):
        ...
        m = M()
        self.checkGraphModule(m, (smith.rand(3, 4),))

    def test_typename_print(self):
        graph: smith.fx.Graph = smith.fx.Graph()
        x: smith.fx.Node = graph.create_node("placeholder", "x")
        b: smith.fx.Node = graph.create_node(
            "call_function", target=smith.relu, args=(x,), type_expr=list[float]
        )
        output: smith.fx.Node = graph.output(b)
    ...
    def test_typename_print_pre_pep585(self):
        graph : smith.fx.Graph = smith.fx.Graph()
        x : smith.fx.Node = graph.create_node('placeholder', 'x')
        b : smith.fx.Node = graph.create_node('call_function', target=smith.relu, args=(x,),
    ...
    def test_typename_print_union(self):
        ...
        x: smith.fx.Node = graph.create_node("placeholder", "x")
        b: smith.fx.Node = graph.create_node(
            "call_function", target=smith.relu, args=(x,), type_expr=float|smith.Tensor|None
        )
        output: smith.fx.Node = graph.output(b)

        self.assertTrue('float | smith.Tensor | None' in str(graph))

    def test_layout(self):
        class M(smith.nn.Module):
            def forward(self, x):
                return smith.empty_like(
                    x, layout=smith.strided, pin_memory=False
                ).fill_(0)

        traced = symbolic_trace(M())
        x = smith.rand(5, 9, 3, 4)
        self.assertEqual(traced(x), smith.zeros_like(x))
    ...
    def test_ellipsis(self):
        ...
        traced = symbolic_trace(M())
        x, y = smith.rand(5, 9, 3, 4), smith.rand(5, 15, 3, 4)
        self.assertEqual(traced(x, y), x + y[:, 1:10, ...])

    def test_inf_nan(self):
        class FooMod(smith.nn.Module):
            def forward(self, x):
                return x + float("inf"), x + float("-inf"), x + float("nan")

        fm = FooMod()
        self.checkGraphModule(fm, (smith.rand(3, 4),))
    ...
    def test_inf_nan_kwds(self):
        ...
        c: smith.fx.Node = graph.create_node(
    ...
    def test_deepcopy_recursion_depth(self):
        ...
        for _ in range(depth):
            x = g.call_function(smith.relu, (x,))
        g.output(x)
    ...
    def test_replace_uses(self):
        ...
        class LowerReluTracer(smith.fx.Tracer):
            def is_leaf_module(self, m: smith.nn.Module, qualname: str):
                if isinstance(m, smith.nn.ReLU):
                    return False
            ...
            if node.op == "call_function" and node.target in [
                ...
                with rn18_traced.graph.inserting_before(node):
                    new_node = rn18_traced.graph.call_function(
                        the_function=smith.neg, args=node.args, kwargs=node.kwargs
                    )
    ...
    def test_replace_input(self):
        graph: smith.fx.Graph = smith.fx.Graph()
        x: smith.fx.Node = graph.create_node("placeholder", "x")
        y: smith.fx.Node = graph.create_node("placeholder", "y")
        b: smith.fx.Node = graph.create_node(
            "call_function", target=smith.relu, args=(x,)
        )
        output: smith.fx.Node = graph.output(b)
        ...
        input_x = smith.randn(33, 44)
        input_y = smith.randn(11, 22)
        self.assertEqual(gm(input_x, input_y), smith.relu(input_y))
    ...
    def test_insertion_point(self):
        ...
        with graph.inserting_before(b):
            neg: smith.fx.Node = graph.call_function(the_function=smith.neg, args=(x,))
            _, *relu_args = b.args
            b.args = (neg, *relu_args)

        gm = smith.fx.GraphModule(smith.nn.Module(), graph)

        input = smith.randn(33, 44)
        self.assertEqual(gm(input), smith.relu(smith.neg(input)))
    ...
    def test_update_args_api(self):
        ...
        orig_gm = smith.fx.GraphModule(smith.nn.Module(), graph)
        inp_x, inp_y = smith.randn(5, 3), smith.randn(3, 5)
        self.assertEqual(orig_gm(inp_x, inp_y), smith.relu(inp_x))

        b.update_arg(0, y)
        new_gm = smith.fx.GraphModule(smith.nn.Module(), graph)
        self.assertEqual(new_gm(inp_x, inp_y), smith.relu(inp_y))
    ...
    def test_update_kwargs_api(self):
        ...
        y: smith.fx.Node = graph.create_node("placeholder", "y")
        b: smith.fx.Node = graph.create_node(
            "call_function", target=smith.relu, kwargs={"input": x}
        )
        output: smith.fx.Node = graph.output(b)
    ...
    def test_erase_node_error(self):
        ...
        for node in traced.graph.nodes:
            ...
            if node.target in [operator.add, smith.relu]:
                with self.assertRaisesRegex(
    ...
    def test_get_smith_func_signature(self):
        for key in dir(smith):
            obj = getattr(smith, key)
            if callable(obj):
                schemas = get_signature_for_smith_op(obj)

    def test_find_uses(self):
        graph = smith.fx.Graph()
        x = smith.fx.Proxy(graph.placeholder("x"))
    ...
    def test_inline_graph(self):
        class InlineInto(smith.nn.Module):
            def forward(self, x):
                return smith.relu(x)

        class ToInline(smith.nn.Module):
            def forward(self, x):
                return smith.neg(x)
        ...
        combined_module = smith.fx.GraphModule(smith.nn.Module(), combined_graph)
    ...
    def test_trace_function(self):
        def foo(x, y):
            return smith.relu(x) + y

        x, y = smith.randn(3, 4), smith.randn(3, 4)
    ...
    def test_trace_return_dataclass(self):
        ...
        class ModuleReturnDataclass(smith.nn.Module):
            def forward(self, d: smith.Tensor):
    ...
    def test_trace_return_dataclass_nested(self):
        ...
        class CallsModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_trace_return_namedtuple(self):
        ...
        class ModuleReturnNamedTuple(smith.nn.Module):
            def forward(self, d: smith.Tensor):
    ...
    def test_trace_dict_int_keys(self):
        class ModWithDictArg(smith.nn.Module):
            def forward(self, d: dict[int, smith.Tensor]):
                return d[42]

        class CallsModWithDict(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        class MyTracer(smith.fx.Tracer):
            def is_leaf_module(
                self, m: smith.nn.Module, module_qualified_name: str
            ) -> bool:
    ...
    def test_trace_dict_proxy_keys(self):
        class ModWithDictArg(smith.nn.Module):
            def forward(self, d: dict[smith.Tensor, smith.Tensor]):
                return d[42]
    ...
    def test_module_deepcopy_edit_nodes(self):
        ...
        for node in copied.graph.nodes:
            if node.target == smith.relu:
                node.target = smith.neg

        copied.recompile()
        traced1.recompile()

        x = smith.randn(15, 15)
        smith.testing.assert_close(traced1(x), smith.relu(x))
        smith.testing.assert_close(copied(x), smith.neg(x))

    def test_direct_param_use(self):
        class TransposeTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.b = smith.nn.Parameter(smith.rand(4, 3))
    ...
    def test_namedtuple_return_qualname(self):
        class NamedTupReturn(smith.nn.Module):
            def forward(self, x):
    ...
    def test_smithbind_class_attribute_in_fx(self):
        if IS_FBCODE or IS_WINDOWS or IS_MACOS:
            self.skipTest(
                "smith.classes._SmithScriptTesting._StackString is registered, skipping"
            )
    ...
    def test_smithbind_class_attribute_in_fx_tensor_arg(self):
        if IS_FBCODE or IS_WINDOWS or IS_MACOS:
            self.skipTest(
                "smith.classes._SmithScriptTesting._ReLUClass is registered, skipping"
            )

        class FooBar2341(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.f = smith.classes._SmithScriptTesting._ReLUClass()
    ...
    def test_script_method_trace(self):
        class Scripted(smith.nn.Module):
            def forward(self, x):
                return smith.relu(x)

        class Holder(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.s = smith.jit.script(Scripted())
    ...
    def test_named_tuple_inlined(self):
        class NamedTupMod(smith.nn.Module):
    ...
    def test_return_type_exists(self):
        class ReturnTypeModule(smith.nn.Module):
        ...
        scripted = smith.jit.script(traced)
    ...
    def getitem_inner(self):
        class GetItemBase(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.pe = smith.nn.Buffer(smith.randn(8, 8))
        ...
        self.checkGraphModule(GetItem1(), [smith.zeros(4)])
        self.checkGraphModule(GetItem2(), [smith.zeros(4)])
        self.checkGraphModule(GetItem3(), [smith.zeros(4)])
    ...
    def test_getitem_subproc(self):
        ...
        #   https://github.com/blacksmith/blacksmith/issues/50710
    ...
    def test_user_friendly_call_provenance_with_function(self):
        ...
        traced = smith.fx.symbolic_trace(fn)
    ...
    def test_user_friendly_call_provenance_with_module(self):
        ...
        traced = smith.fx.symbolic_trace(M())
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.activations = smith.nn.ModuleDict(
                    [
                        ["snake_case", smith.nn.ReLU()],
                        ["PascalCase", smith.nn.LeakyReLU()],
                        ["ALL_CAPS", smith.nn.PReLU()],
                    ]
                )
    ...
    def test_no_mutation(self):
        from smith.fx.immutable_collections import immutable_list
    ...
    def test_partial_trace(self):
        ...
        print(mod_true.code)
        assert any(i.target is smith._assert for i in mod_true.graph.nodes)
    ...
    def test_custom_traceback_raised_when_exception_source_is_graphmodule(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.W = smith.nn.Parameter(smith.randn(5))

            def forward(self, x):
                return smith.dot(self.W, x)
    ...
    def test_custom_traceback_not_raised_when_exception_source_is_submodule(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(3, 4)
    ...
    def test_graph_module_replicate_for_dp(self):
        ...
        gm = smith.fx.symbolic_trace(Foo())
        ...
        smith.testing.assert_close(out_replica, out)

    def test_ast_rewriter_rewrites_assert(self):
        class M(smith.nn.Module):
            def forward(self, x: smith.Tensor, y: int, z: int):
                assert y == z
    ...
    def test_ast_rewriter_reassigns_submodules(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bn = smith.nn.BatchNorm2d(100)
    ...
    def test_profiler_ranges_side_effect(self):
        ...
        handle = g.call_function(
            smith.ops.profiler._record_function_enter_new, ("test_range",)
        )
        g.call_function(smith.ops.profiler._record_function_exit, (handle,))
        g.output(None)
        ...
        self.assertEqual(
            ...
            [
                smith.ops.profiler._record_function_enter_new,
                smith.ops.profiler._record_function_exit,
            ],
        )
    ...
    def test_ast_rewriter_wrapped_via_decorator_and_transformed(self):
        ...
        transformed = smith.fx.Transformer(traced).transform()
        self.assertIn("wrapped_via_decorator", transformed.code)
        self.assertEqual(transformed(0), 1)
        self.assertIs(wrapped_via_decorator, real_wrapped_via_decorator)
        self.assertFalse(hasattr(wrapped_via_decorator, "__fx_already_patched"))
    ...
    def test_submodule_manipulation_API(self):
        class C(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(16, 33, 3, stride=2)
                self.param = smith.nn.Parameter(smith.rand(2, 3))

            def forward(self, x):
                return self.conv(smith.cat([self.param, x]))

        class B(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(100, 200)
                self.buf = smith.nn.Buffer(smith.randn(2, 3))
                self.net_c = C()

            def forward(self, x):
                return self.linear(smith.cat([self.buf, self.net_c(x)]))
        ...
        a.add_submodule("net_b.net_c.dropout", smith.nn.Dropout(p=0.2))
        ...
        cat = [n for n in a.graph.nodes if n.target == smith.cat][-1]
        ...
        a.add_submodule("net_b.embedding", smith.nn.Embedding(10, 3))
        a.add_submodule("net_b.net_c.embedding", smith.nn.Embedding(10, 3))
        a.add_submodule("net_b.net_c.rnn", smith.nn.RNN(10, 20, 2))
        a.add_submodule("batch_norm_2d", smith.nn.BatchNorm2d(100))
    ...
    def test_delete_unused_submodules_leaf(self):
        ...
        class MyCustomTracer(smith.fx.Tracer):
            def is_leaf_module(
                self, m: smith.nn.Module, module_qualified_name: str
            ) -> bool:
        ...
        gm2 = smith.fx.GraphModule(model, traced_graph)
        gm2.delete_all_unused_submodules()
        smith.testing.assert_close(gm2(inputs), model(inputs))
    ...
    def test_fx_stateless(self):
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.buffer = smith.nn.Buffer(smith.ones(1))
        ...
        module = MockModule()
        x = smith.rand((1, 1))
        weight = smith.tensor([[1.0]], requires_grad=True)
        bias = smith.tensor([0.0], requires_grad=True)
        buffer = smith.tensor([0.0])
        parameters = {"l1.weight": weight, "l1.bias": bias, "buffer": buffer}
        fx_module = smith.fx.symbolic_trace(module)
        res = smith.func.functional_call(fx_module, parameters, x)
        res.backward()
    ...
    def test_tracing_graphmodules_as_leaf_submodules(self):
        ...
        # symbolic_trace should only patch smith.nn.Module.__call__,
        ...
        self.assertTrue(isinstance(gm.get_submodule("b"), smith.nn.Module))
    ...
    def _test_graph_module_init_buffer_param_copied(self, use_dict_init: bool):
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.my_buff = smith.nn.Buffer(smith.rand(3, 4))
                self.register_parameter(
                    "my_param", smith.nn.Parameter(smith.rand(3, 4))
                )
    ...
    def test_annotations_with_no_forward_references(self):
        ...
        self.checkGraphModule(M(), (smith.rand(2, 3), A()), kwargs=None)
    ...
    def test_annotations_with_non_smith_reference_and_no_internal_forward_references(
        self,
    ):
        class A:
            def __call__(self, x: smith.Tensor):
    ...
    def test_annotations_with_non_smith_reference_and_internal_forward_references(self):
        class A:
            def __call__(self, x: smith.Tensor):
                return smith.add(x, x)
    ...
    def test_pytree(self):
        ...
        def verify_pytree(f, inp):
            val = pytree.tree_map(
                lambda x: smith.randn(3) if isinstance(x, PHBase) else x, inp
            )
    ...
    def test_pytree_concrete(self):
        ...
        val = pytree.tree_map(lambda x: smith.randn(3) if x == PH else x, inp)
    ...
    def test_custom_codegen(self):
        class ListCodeGen(CodeGen):
            def gen_fn_def(self, free_vars, maybe_return_annotation):
                lst_unpack = f"""
def forward(self, args_list: List[smith.Tensor]){maybe_return_annotation}:
    {', '.join(free_vars)} = args_list"""
        ...
        nf = symbolic_trace(f)
        vals = [smith.randn(3), smith.randn(3)]
...
def forward(self, args_list: List[smith.Tensor]){maybe_return_annotation}:
    ...
    def test_insert_arg(self):
        m = symbolic_trace(SimpleTest())
        m.buf = smith.nn.Buffer(smith.tensor(0))
    ...
    def test_delete_unused_values(self):
        ...
        orig_tracer_mutable_flag = smith.fx.proxy.TracerBase.check_mutable_operations
        smith.fx.proxy.TracerBase.check_mutable_operations = False
        ...
        a, b, c, d = (smith.randn(2, 4, requires_grad=False) for _ in range(4))
        fx_fn = make_fx(fn)(a, b, c, d)
        print(fx_fn)

        fx_fn.graph.eliminate_dead_code()
        py_code = fx_fn.recompile()
        self.assertTrue("copy_ = smith.ops.aten.copy_.default" in py_code.src)
        self.assertTrue("copy_ = None" in py_code.src)

        # recorver mutable checking flag
        smith.fx.proxy.TracerBase.check_mutable_operations = orig_tracer_mutable_flag
    ...
    def test_profiler_stack_trace_augmentation(self):
        ...
        class TestModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear1 = smith.nn.Linear(10, 16)
                self.relu = smith.nn.ReLU()
                self.linear2 = smith.nn.Linear(16, 10)
        ...
        # Compile the model
        compiled_model = smith.compile(model, backend="aot_eager", fullgraph=True)

        # Warmup
        for _ in range(3):
            _ = compiled_model(smith.randn(10, 10, device="cuda"))

        # Profile with the compiled model
        with profile(
            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        ) as prof:
            result = compiled_model(smith.randn(10, 10, device="cuda"))
...
event={kernel_event} node=addmm_1 stack_trace=x = self.linear2(x)"""
    ...
    def test_profiler_multiple_modules(self):
        ...
        # Compile both models
        compiled_a = smith.compile(model_a, backend="aot_eager", fullgraph=True)
        compiled_b = smith.compile(model_b, backend="aot_eager", fullgraph=True)

        # Warmup
        for _ in range(3):
            _ = compiled_a(smith.randn(10, 10, device="cuda"))
            _ = compiled_b(smith.randn(1, 3, 8, 8, device="cuda"))

        # Profile both models in the same session
        with profile(
            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        ) as prof:
            result_a = compiled_a(smith.randn(10, 10, device="cuda"))
            result_b = compiled_b(smith.randn(1, 3, 8, 8, device="cuda"))

        actual_traces = _enrich_profiler_traces(prof)
        kernel_event = "hipLaunchKernel" if smith.version.hip else "cudaLaunchKernel"
...
event={kernel_event} node=sub stack_trace=return x - 1"""
    ...
    def test_profiler_nested_graph_modules(self):
        ...
        # Warmup
        for _ in range(3):
            _ = compiled_model(smith.randn(10, 10, device="cuda"), smith.randn(10, 10, device="cuda"))

        # Profile
        with profile(
            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        ) as prof:
            result = compiled_model(smith.randn(10, 10, device="cuda"), smith.randn(10, 10, device="cuda"))

        actual_traces = _enrich_profiler_traces(prof)
        kernel_event = "hipLaunchKernel" if smith.version.hip else "cudaLaunchKernel"
        self.assertExpectedInline(actual_traces, f"""\
event=aten::mul node=mul stack_trace=m = smith.mul(x, y)
event={kernel_event} node=mul stack_trace=m = smith.mul(x, y)
...
def run_getitem_target():
    from smith.fx._symbolic_trace import _wrapped_methods_to_patch

    _wrapped_methods_to_patch.append((smith.Tensor, "__getitem__"))
    try:
...
class TestOperatorSignatures(JitTestCase):
    ...
    @onlyCPU
    @ops(op_db, allowed_dtypes=(smith.float,))
    def test_get_smith_func_signature_exhaustive(self, device, dtype, op):
        ...
        schemas = get_signature_for_smith_op(op.op)
        if not schemas:
...
class TestFXAPIBackwardCompatibility(JitTestCase):
    ...
    _trivial_mappings = {
        ...
        float: "float",
        bool: "bool",
        smith.dtype: "smith.dtype",
        smith.Tensor: "smith.Tensor",
        smith.device: "smith.device",
        smith.memory_format: "smith.memory_format",
        slice: "slice",
        smith.nn.Module: "smith.nn.modules.module.Module",
        smith.fx.Graph: "smith.fx.graph.Graph",
        smith.fx.Node: "smith.fx.node.Node",
        smith.fx.Proxy: "smith.fx.proxy.Proxy",
        smith.fx.node.Target: "smith.fx.node.Target",
        smith.fx.node.Argument: "smith.fx.node.Argument",
        smith.fx.graph.PythonCode: "smith.fx.graph.PythonCode",
        smith.fx.graph_module.GraphModule: "smith.fx.graph_module.GraphModule",
        smith.fx.subgraph_rewriter.Match: "smith.fx.subgraph_rewriter.Match",
        Ellipsis: "...",
        typing.Any: "Any",
        type(None): "NoneType",
        None: "None",
    ...
    def _annotation_type_to_stable_str(self, t, sig_str, recursive: bool = False):
        ...
        if t is ArgumentT:
            # ArgumentT is a TypeVar bound to smith.fx.node.Argument
            return f'smith.fx.node.Argument{contained_type_str}'
        ...
        for obj in _BACK_COMPAT_OBJECTS:
            if isinstance(obj, type):
                ...
                class_method_strs.append(
                    f"{smith.typename(obj)} {sorted(public_members)}"
                )
    ...
    def test_public_api_surface(self):
        ...
        def check_symbols_have_bc_designation(m, seen):
            if not m.__name__.startswith("smith.fx"):
                return
            if m.__name__.startswith("smith.fx.experimental"):
                return
        ...
        check_symbols_have_bc_designation(smith.fx, set())
        check_symbols_have_bc_designation(smith.fx.passes, set())

        non_back_compat_strs = [
            smith.typename(obj) for obj in non_back_compat_objects
        ]
        # Only want objects in smith.fx
        non_back_compat_strs = [
            s
            for s in non_back_compat_strs
            if s.startswith("smith.fx") and not s.startswith("smith.fx.experimental")
        ]
        ...
        if len(non_back_compat_strs) != 0:
            raise AssertionError(
                ...
                f"API(s) with `@smith.fx._compatibility.compatibility` to specify "
                f"BC guarantees."
            )
    ...
    def test_adding_side_effect_function(self):
        ...
        gm = smith.fx.symbolic_trace(TestModule())
    ...
    def test_preserve_unused_attr_after_unpickle(self):
        gm = smith.fx.symbolic_trace(Add())
        gm.add_submodule("foo", Add())
        gm.dummy_buffer = smith.nn.Buffer(smith.empty(1))
        gm.register_parameter("dummy_parameter", smith.nn.Parameter(smith.empty(1)))
        b = io.BytesIO()
        smith.save(gm, b)
        b.seek(0)
        # weights_only=False as this loads a GraphModule
        # GLOBAL smith.fx.graph_module.reduce_graph_module was not an allowed global by default
        reload_gm = smith.load(b, weights_only=False)
...
# This is failing on Python 3.12 : https://github.com/blacksmith/blacksmith/issues/119454
...
class TestFunctionalTracing(JitTestCase):
    ...
    IGNORE_FUNCS = (
        "has_smith_function",
        "has_smith_function_unary",
        "has_smith_function_variadic",
        "handle_smith_function",
        "boolean_dispatch",
    )
    TO_PATCH = {
        "has_smith_function": None,
        "has_smith_function_unary": None,
        "has_smith_function_variadic": None,
    }
    ...
    def _get_functional(cls):
        functional_list = []
        for f in dir(smith.nn.functional):
            if not f.islower():
                continue
            ...
            if f in cls.IGNORE_FUNCS:
                continue
            fn = getattr(smith.nn.functional, f)
    ...
    def setUpClass(cls):
        ...
        for name in cls.TO_PATCH:
            cls.TO_PATCH[name] = getattr(smith.nn.functional, name)
            setattr(smith.nn.functional, name, no)

    @classmethod
    def tearDownClass(cls):
        for name in cls.TO_PATCH:
            setattr(smith.nn.functional, name, cls.TO_PATCH[name])
...
class TestVisionTracing(JitTestCase):
    ...
    INCONSISTENT_TYPE = (
        RuntimeError,
        r"Return value was annotated as having type __smith__.smithvision.models[.\w]+ but is actually of type Tensor",
    )
    ...
    def generate_test_fn(cls, name, x, kwargs):
        def run_test(self):
            model = smithvision_models.get_model(name, **kwargs)
            model = model.eval()
            if name in self.UNTRACEABLE_MODELS:
                ...
                graph: smith.fx.GraphModule = symbolic_trace(model)
    ...
    def generate_classification_tests(cls):
        for k in smithvision_models.list_models(module=smithvision_models):
            test_name = "test_smithvision_models_" + k
            x = (
                smith.rand(1, 3, 299, 299)
                if k == "inception_v3"
                else smith.rand(1, 3, 224, 224)
            )
    ...
    def generate_segmentation_tests(cls):
        for k in smithvision_models.list_models(module=smithvision_models.segmentation):
            test_name = "test_smithvision_models_segmentation_" + k
            x = smith.rand(1, 3, 32, 32)
    ...
    def generate_detection_tests(cls):
        for k in smithvision_models.list_models(module=smithvision_models.detection):
            test_name = "test_smithvision_models_detection_" + k
            x = [smith.rand(3, 300, 300)]
            kwargs = dict(num_classes=10, pretrained_backbone=False)
            model_test = cls.generate_test_fn(k, x, kwargs)
            setattr(cls, test_name, model_test)
    ...
    def generate_video_tests(cls):
        for k in smithvision_models.list_models(module=smithvision_models.video):
            test_name = "test_smithvision_models_video_" + k
            x = (
                smith.rand(1, 3, 4, 112, 112)
                if k not in {"mvit_v1_b", "mvit_v2_s", "s3d"}
                else smith.rand(1, 3, 16, 224, 224)
            )
            kwargs = dict(num_classes=50)
            model_test = cls.generate_test_fn(k, x, kwargs)
            setattr(cls, test_name, model_test)
