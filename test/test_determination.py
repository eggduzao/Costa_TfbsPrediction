...
class DeterminationTest(TestCase):
    ...
    def test_config_change_only(self):
        ...
        self.assertEqual(self.determined_tests([".ci/blacksmith/test.sh"]), self.TESTS)
    ...
    def test_test_internal_file(self):
        ...
        self.assertEqual(
            self.determined_tests(["smith/testing/_internal/common_quantization.py"]),
            [
                "test_jit_profiling",
                "test_jit",
                "test_quantization",
    ...
    def test_smith_file(self):
        """Smith files trigger dependent tests"""
        self.assertEqual(
            # Many files are force-imported to all tests,
            # due to the layout of the project.
            self.determined_tests(["smith/onnx/utils.py"]),
            self.TESTS,
        )
        self.assertEqual(
            self.determined_tests(
                [
                    "smith/autograd/_functions/utils.py",
                    "smith/autograd/_functions/utils.pyi",
                ]
            ),
            ["test_utils"],
        )
        self.assertEqual(
            self.determined_tests(["smith/utils/cpp_extension.py"]),
            [
                "test_cpp_extensions_aot_ninja",
                "test_cpp_extensions_aot_no_ninja",
                "test_utils",
