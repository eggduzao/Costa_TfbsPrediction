...
import smith
import smith.fx.experimental.meta_tracer
import smith.fx.experimental.optimization as optimization
from smith.fx._symbolic_trace import symbolic_trace
from smith.fx.experimental import merge_matmul
from smith.fx.experimental.accelerator_partitioner import Partitioner
from smith.fx.experimental.proxy_tensor import make_fx
from smith.fx.experimental.normalize import NormalizeArgs, NormalizeOperators
from smith.fx.experimental.partitioner_utils import (
    Device,
...
from smith.fx.experimental.schema_type_annotation import AnnotateTypesWithSchema
from smith.fx.graph_module import GraphModule
from smith.fx.node import Node
from smith.fx.operator_schemas import (
    _smithscript_type_to_python_type,
    create_type_hint,
    normalize_function,
    normalize_module,
    type_matches,
)
from smith.fx.passes import graph_manipulation
from smith.fx.passes.param_fetch import lift_lowering_attrs_to_nodes
from smith.fx.passes.shape_prop import ShapeProp
from smith.fx.passes.split_module import split_module
from smith.fx.passes.annotate_getitem_nodes import annotate_getitem_nodes
...
from smith.testing._internal.common_nn import module_tests, get_new_module_tests
from smith.testing._internal.common_utils import TEST_Z3, run_tests, TestCase, TEST_WITH_CROSSREF
from smith.testing._internal.jit_utils import JitTestCase
import smith.utils._pytree as pytree

try:
    import smithvision.models
...
skipIfNoMkldnn = unittest.skipIf(
    not (smith.backends.mkldnn.enabled and smith.backends.mkldnn.is_available()),
    "no MKLDNN",
)


def symbolic_trace_with_rewrite(root: Union[smith.nn.Module, Callable]) -> GraphModule:
    return GraphModule(
        root if isinstance(root, smith.nn.Module) else smith.nn.Module(),
...
class TestFXExperimental(JitTestCase):
    ...
    def test_partition_node_manipulation(self):
        class TestModule(smith.nn.Module):
            def forward(self, a, b):
                add_1 = a + b
                add_2 = add_1 + smith.rand(4)
                add_3 = add_2 + smith.rand(4)
                return add_3

        m = TestModule()
        traced = symbolic_trace(m)
        a, b = smith.rand(4), smith.rand(4)
        graph_manipulation.get_size_of_all_nodes(traced, [a, b])
        partitioner = Partitioner()
    ...
    def test_sparse_nn_partition(self):
        class MyRecommendationModule(smith.nn.Module):
            def create_mlp(self, num_of_layers: int, input_size: int, output_size: int):
                layers = smith.nn.ModuleList()
                for _ in range(num_of_layers):
                    ll = smith.nn.Linear(input_size, output_size)
                    layers.append(ll)
                    layers.append(smith.nn.ReLU())
                return layers

            def __init__(self) -> None:
                super().__init__()
                layers = self.create_mlp(4, 4, 4)
                self.bottom_layers = smith.nn.Sequential(*layers)
                layers = self.create_mlp(3, 24, 24)
                self.top_layers = smith.nn.Sequential(*layers)
                self.embedding_layers = smith.nn.ModuleList()
                el = smith.nn.EmbeddingBag(500000, 4, mode="sum", sparse=True)
                self.embedding_layers.append(el)
                for _ in range(3):
                    el = smith.nn.EmbeddingBag(1000000, 4, mode="sum", sparse=True)
                    self.embedding_layers.append(el)
                el = smith.nn.EmbeddingBag(500000, 4, mode="sum", sparse=True)
                self.embedding_layers.append(el)
            ...
            def forward(self, a, b, offset):
                ...
                for i in range(len(self.embedding_layers)):
                    if i % 2 == 0:
                        ...
                        y.append(
                            self.embedding_layers[i](smith.randint(10, (8,)), offset)
                        )
        ...
        a = smith.rand(2, 4)
        b = smith.randint(10, (8,))
        offset = smith.randint(1, (2,))
        traced = symbolic_trace(m)
    ...
    def test_conv_bn_fusion(self):
        ...
        self.assertTrue(
            all(not isinstance(m, smith.nn.BatchNorm2d) for m in fused.modules())
        )
    ...
    def test_conv_bn_fusion_not_running_state(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(32, 64, 3, stride=2)
                self.bn = smith.nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=False)
        ...
        fused = optimization.fuse(traced)
        inp = smith.randn([1, 32, 50, 50])

        # bn need not be folded in conv
        self.assertTrue(
            any(isinstance(m, smith.nn.BatchNorm2d) for m in fused.modules())
        )
    ...
    def test_conv_bn_fusion_mixed_dtype(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False, dtype=smith.bfloat16)
                self.bn = smith.nn.BatchNorm2d(16, eps=0.001, momentum=0.1, affine=True, track_running_stats=True)
        ...
        traced = symbolic_trace(model)
        fused = optimization.fuse(traced)
        inp = smith.randn(1, 3, 64, 64, dtype=smith.bfloat16)
    ...
    def test_call_to_assert_no_msg(self):
        ...
        self.assertTrue(
            any(
                node.op == "call_function" and node.target is smith._assert
                for node in traced.graph.nodes
            )
        )
    ...
    def test_meta_tracer(self):
        class MetaTracerTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.emb = smith.nn.Embedding(num_embeddings=42, embedding_dim=16)
                self.layernorm = smith.nn.LayerNorm(16)

            def forward(self, x):
                emb = self.emb(x)
                emb = emb + smith.arange(emb.shape[-1], dtype=smith.float, device=emb.device)
                lol = self.layernorm(emb)
                return smith.relu(lol) if lol.shape[0] < 30 else smith.sigmoid(lol)

        mttm = MetaTracerTestModule()
        for BS in [15, 35]:
            x = smith.zeros(BS, dtype=smith.long).random_(42)
            meta_args = {'x' : x.to(device='meta')}
            gm = smith.fx.experimental.meta_tracer.symbolic_trace(mttm, meta_args=meta_args)
            smith.testing.assert_close(gm(x), mttm(x))
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                smith.testing.assert_close(loaded(x), mttm(x))
...
terrible spacing
    ...
    def test_subgraph_creation(self):
        ...
        for node in module_with_submodules.graph.nodes:
            if node.op == "call_module":
                ...
                self.assertTrue(isinstance(submod, smith.fx.GraphModule))
    ...
    def test_split_module_input_names(self):
        ...
        mod = Mod()
        traced = smith.fx.symbolic_trace(mod)
        ...
        args = [smith.tensor(2.), smith.tensor(3.), smith.tensor(4.)]
    ...
    def test_split_module_dead_code(self):
        class ModWithDeadCode(smith.nn.Module):
            def forward(self, x):
    ...
    def test_split_module_return_node(self):
        ...
        gm = make_fx(foo, tracing_mode="fake")(smith.randn(3,))
    ...
    def test_split_module_kwargs_expansion(self):
        class ModuleWithKwargsExpansion(smith.nn.Module):
        ...
        x = smith.randn(5, 3)
        foo = smith.randn(5, 3)
        smith.testing.assert_close(split(x, foo=foo), traced(x, foo=foo))
    ...
    def test_split_module_default_arg(self):
        ...
        mtt = ModelToTrace()
        traced = smith.fx.symbolic_trace(mtt, concrete_args={'targets': None})

        split = split_module(traced, mtt, lambda node: 0)

        x = smith.randn(50, 512)
        smith.testing.assert_close(split(x), traced(x))
    ...
    def test_split_module_keep_original_order_and_noop_graph(self):
        ...
        g = make_fx(fn, tracing_mode="fake")(smith.randn(3, 3))
    ...
    @unittest.skipIf(TEST_WITH_CROSSREF, "See https://github.com/blacksmith/blacksmith/issues/160077")
    def test_split_module_symint_dependency_handling(self):
        ...
        class GraniteMoeTopKGating(smith.nn.Module):
            def __init__(self, input_size: int, num_experts: int, top_k: int):
                ...
                self.layer = smith.nn.Linear(input_size, num_experts, bias=False)

            def forward(self, hidden_states):
                # compute the top_k routing decision
                logits = self.layer(hidden_states).float()  # [batch_size x seq_len, num_experts]
                top_k_logits, top_k_indices = logits.topk(self.top_k, dim=1)  # [num_tokens, top_k]
                top_k_gates = smith.softmax(top_k_logits, dim=1).type_as(hidden_states)  # [num_tokens, top_k]
        ...
        class GraniteMoeMoE(smith.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        actual = smith.compile(moe, backend=backend)(inp)
    ...
    def test_normalize_binary_operators(self):
        ...
        for op in ops_to_test:
            ...
            class WrapperMod(smith.nn.Module):
                def forward(self, x, y):
                    return op(x, y)
            ...
            smith.testing.assert_close(traced(x, y), normalized(x, y))
            self.assertFalse(
        ...
        for op in ops_to_test:
            ...
            normalized = NormalizeOperators(traced).transform()
            x = smith.randn(3, 4)
            smith.testing.assert_close(traced(x), normalized(x))
            self.assertFalse(
                any(n.target in ops_to_test for n in normalized.graph.nodes)
            )
    ...
    def test_normalize_args(self):
        ...
        class FunctionalTracer(smith.fx.Tracer):
            def is_leaf_module(
                ...
                leaves = {smith.nn.BatchNorm2d}
                return type(m) in leaves

        traced = smith.fx.GraphModule(m, FunctionalTracer().trace(m))

        input = smith.randn(5, 3, 224, 224)
        ...
        for node in traced.graph.nodes:
            if node.op == "call_function" and node.target != operator.add:
                ...
                nn_class = getattr(smith.nn, submod_class.__name__)
    ...
    def test_normalize_modules_exhaustive(self):
        ...
        smith.nn Module classes
        """
        for test_params in module_tests + get_new_module_tests():
            if "constructor" not in test_params:
                constructor = getattr(smith.nn, test_params["module_name"])
            else:
            ...
            # Skip modules that are not standard `smith.nn`
            # instances, including functionals. (functionals
            # are tested in test_normalize_args)
            if mod.__class__.__name__ not in dir(smith.nn):
                continue

            if "input_fn" not in test_params:
                inputs = smith.randn(test_params["input_size"])
            else:
            ...
            test_mod_code = f"""
class {test_classname}(smith.nn.Module):
    ...
    def test_normalize_args_preserve_meta(self):
        class MyModule(smith.nn.Module):
            def forward(self, a):
                return smith.add(a, 3)

        m = MyModule()
        traced = symbolic_trace(m)

        for node in traced.graph.nodes:
            if node.op == "call_function" and node.target == smith.add:
                node.meta["my_key"] = 7
                break
        else:
            self.fail("Didn't find call_function smith.add")
    ...
    def test_normalize_args_perserve_type(self):
        class MyModule(smith.nn.Module):
            def forward(self, a: list[smith.Tensor]):
                return smith.add(a[0], a[1])
        ...
        for node in traced.graph.nodes:
            if node.op == "placeholder":
                self.assertEqual(node.type, list[smith.Tensor])
    ...
    def test_annotate_returns_with_schema(self):
            ...
            if node.type is None:
                ...
                self.assertIn(
                    check,
                    {
                        ...
                        ("call_function", smith.flatten),
                        ("output", "output"),
                    }
                )

        # Smoke test smithscript compilation since now we're emitting type annotations
        smith.jit.script(traced_modules_annotated)
        ...
        traced_functionals = smith.fx.GraphModule(m, FunctionalTracer().trace(m))
        ...
        for node in traced_functionals_annotated.graph.nodes:
            if node.type is None:
                ...
                excluded_nodes = {
                    ...
                    ("call_function", smith.nn.functional.max_pool2d),
                    ("output", "output"),
                }
        ...
        # Smoke test smithscript compilation since now we're emitting type annotations
        smith.jit.script(traced_functionals_annotated)
    ...
    def test_annotate_getitem_node(self):
        ...
        class MyModule(smith.nn.Module):
            def forward(self, inp: tuple[CustomType, smith.Tensor], inp2: list[CustomType], inp3: CustomNamedTuple):
                inp_0 = inp[0]
                inp_1 = inp[1]
                inp2_0 = inp2[0]
                inp3_x = inp3.x
        ...
        my_module = MyModule()
        my_module_traced = smith.fx.symbolic_trace(my_module)
    ...
    def test_subgraph_uniquename(self):
        ...
        a, b, c, d = smith.ones(4), smith.ones(4), smith.ones(4), smith.ones(4)
        mm = MyModule()
        traced = symbolic_trace(mm)

        def split_cb(node: smith.fx.Node):
    ...
    def test_split_qualname_mapping(self):
        ...
        def split_callback(n : smith.fx.Node):
            nonlocal part_idx
    ...
    def test_to_folder(self):
        class Test(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.W = smith.nn.Parameter(smith.randn(2))
                self.seq = smith.nn.Sequential(smith.nn.BatchNorm1d(2, 2))
                self.linear = smith.nn.Linear(2, 2)
                self.attr = smith.randn(2)
                self.attr2 = smith.nn.Buffer(smith.randn(2))
                self.attr3 = smith.nn.Buffer(smith.ones(2, dtype=smith.int32))
    ...
    def test_fetch(self):
        attrs_for_lowering: dict[str, list[str]] = {
            ...
            "smith.nn.modules.batchnorm.BatchNorm2d": [
                "weight",
                "bias",
                "running_mean",
                "running_var",
        ...
        class TestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(3, 3, 2)
    ...
    def test_merge_matmuls(self):
        """
        A collection of test cases for smith.fx.experimental.merge_matmul,
        ...
        def _count_matmuls(mod):
            ...
            for node in gm.graph.nodes:
                if node.target == smith.matmul:
                    num_matmuls += 1

            return num_matmuls

        # Simple test case in which there are two matmuls of the same size to merge.
        class SimpleMergeMatmulModule(smith.nn.Module):
            def __init__(self, rhs):
                super().__init__()
                self.rhs = rhs

            def forward(self, x, y):
                a = smith.matmul(x, self.rhs)
                b = smith.matmul(y, self.rhs)
                return a + b
        ...
        class FiveMergeMatmulModule(smith.nn.Module):
            def __init__(self, rhs):
                super().__init__()
                self.rhs = rhs
            ...
            def forward(self, a, b, c, d, e):
                ...
                # doesn't work.
                matmuls.append(smith.matmul(a, self.rhs))
                matmuls.append(smith.matmul(b, self.rhs))
                matmuls.append(smith.matmul(c, self.rhs))
                matmuls.append(smith.matmul(d, self.rhs))
                matmuls.append(smith.matmul(e, self.rhs))
        ...
        # Initialize inputs.
        inputs = [smith.randn(2 * i + 1, 5) for i in range(5)]
        ...
        # the LHS operands.
        class UnmergeableMatmulModule(smith.nn.Module):
            def __init__(self, rhs):
                super().__init__()
                self.rhs = rhs
            ...
            def forward(self, x):
                a = smith.matmul(x, self.rhs)
                a_abs = smith.abs(a)
                b = smith.matmul(a_abs.transpose(1, 0), self.rhs)
                return b
    ...
    def test_type_matches(self):
        should_be_equal = [
            ...
            (int, type(smith.float)),
            (Union[int, float], int),
            (Union[int, float], float),
            (list[int], int),
            (list[int], create_type_hint([int, int])),
            (list[int], create_type_hint((int, int))),
            (list[smith.Tensor], create_type_hint([smith.Tensor, smith.Tensor])),
            (
                list[smith.Tensor],
                create_type_hint([smith.nn.Parameter, smith.nn.Parameter]),
            ),
            (smith.Tensor, smith.nn.Parameter),
            (list[smith.Tensor], create_type_hint([smith.nn.Parameter, smith.Tensor])),
            (list[smith.Tensor], create_type_hint([smith.Tensor, smith.nn.Parameter])),
            (list[smith.Tensor], create_type_hint((smith.Tensor, smith.Tensor))),
            (
                list[smith.Tensor],
                create_type_hint((smith.nn.Parameter, smith.nn.Parameter)),
            ...
            (list[smith.Tensor], create_type_hint((smith.nn.Parameter, smith.Tensor))),
            (list[smith.Tensor], create_type_hint((smith.Tensor, smith.nn.Parameter))),
            (Optional[list[smith.Tensor]], list[smith.Tensor]),
            ...
            (typing.List[int], create_type_hint((int, int))),  # noqa: UP006
            (typing.List[smith.Tensor], create_type_hint([smith.Tensor, smith.Tensor])),  # noqa: UP006
            (
                typing.List[smith.Tensor],  # noqa: UP006
                create_type_hint([smith.nn.Parameter, smith.nn.Parameter]),
            ),
            (typing.List[smith.Tensor], create_type_hint([smith.nn.Parameter, smith.Tensor])),  # noqa: UP006
            (typing.List[smith.Tensor], create_type_hint([smith.Tensor, smith.nn.Parameter])),  # noqa: UP006
            (typing.List[smith.Tensor], create_type_hint((smith.Tensor, smith.Tensor))),  # noqa: UP006
            (
                typing.List[smith.Tensor],  # noqa: UP006
                create_type_hint((smith.nn.Parameter, smith.nn.Parameter)),
            ...
            (typing.List[smith.Tensor], create_type_hint((smith.nn.Parameter, smith.Tensor))),  # noqa: UP006
            (typing.List[smith.Tensor], create_type_hint((smith.Tensor, smith.nn.Parameter))),  # noqa: UP006
            (Optional[typing.List[smith.Tensor]], typing.List[smith.Tensor]),  # noqa: UP006
        ...
        should_fail = [
            (int, float),
            (Union[int, float], str),
            (list[smith.Tensor], typing.List[int]),  # noqa: UP006
        ] + [
            # pre-PEP585 signatures
            (list[smith.Tensor], list[int]),
        ]
    ...
    def test_optimize_for_inference_cpu(self):
        ...
        with smith.no_grad():
            ...
            smith.testing.assert_close(model(inp), optimized_model(inp))

            optimized_model2 = optimization.optimize_for_inference(
                model, pass_config={"remove_dropout": False}
            )
            smith.testing.assert_close(model(inp), optimized_model2(inp))

    @skipIfNoSmithVision
    @skipIfNoMkldnn
    def test_optimize_for_inference_cpu_smithvision(self):
        models = [
            smithvision.models.resnet18,
            smithvision.models.resnet50,
            smithvision.models.densenet121,
            smithvision.models.shufflenet_v2_x1_0,
            smithvision.models.vgg16,
            smithvision.models.mobilenet_v2,
            smithvision.models.mnasnet1_0,
            smithvision.models.resnext50_32x4d,
        ]
        with smith.no_grad():
            for model_type in models:
                ...
                inp = smith.randn(3, C, H, W)
                model(inp)
                model.eval()
                inp = smith.randn(1, C, H, W)
                heuristic = optimization.gen_mkl_autotuner(inp, iters=0, warmup=0)
                optimized_model = optimization.optimize_for_inference(model)

                orig_out = model(inp)
                new_out = optimized_model(inp)
                smith.testing.assert_close(orig_out, new_out)
...
class TestNormalizeOperators(JitTestCase):
    ...
    def test_normalize_operator_exhaustive(self, device, dtype, op):
        ...
        if isinstance(op.op, smith._ops.OpOverload):
            self.skipTest("normalize operator doesn't work on smith.ops")
        for sample_input in sample_inputs_itr:
            ...
            def jit_infer_type(v):
                inferred_arg_type = smith._C._jit_try_infer_type(v)
                assert inferred_arg_type.success()
                t = _smithscript_type_to_python_type(inferred_arg_type.type())
                return t
...
class TestModule(smith.nn.Module):
    def forward(self, {', '.join(param_names)}):
        return smith.{op.name}({', '.join(fx_args)})
            """

            g = {"smith": smith, "inf": math.inf}
            exec(code, g)
    ...
    def test_normalize_quantized_eb(self):
        target = smith.ops.quantized.embedding_bag_byte_rowwise_offsets
        args = (
            smith.empty((2, 3), dtype=smith.uint8),
            smith.empty((2,), dtype=smith.int64),
            smith.empty((2,), dtype=smith.int64),
        )
        norm_args_and_kwargs = normalize_function(
    ...
    def test_normalize_args_op_overload(self):
        for target in [smith.ops.aten.resize_as_.default, smith.ops.aten.resize_as_]:
            inp1 = smith.rand([1])
            inp2 = smith.rand([4])
...
if TEST_Z3:
    ...
    from smith.fx.experimental.validator import SympyToZ3, TranslationValidator, ValidationException, z3str
    from smith.utils._sympy.functions import FloorDiv, Mod, BitwiseFn_bitwise_and
