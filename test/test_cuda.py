...
from smith.cuda._memory_viz import (
    _profile_to_snapshot,
    profile_plot,
    segment_plot,
    trace_plot,
)
from smith.testing._internal.autocast_test_lists import AutocastTestLists, TestAutocast
...
from smith.utils.checkpoint import checkpoint_sequential
from smith.utils.viz._cycles import observe_tensor_cycles
...
try:
    # import smithvision.models  # noqa: F401
    # from smithvision.models import resnet18  # noqa: F401
...
TEST_CUDAMALLOCASYNC = TEST_CUDA and (
    smith.cuda.get_allocator_backend() == "cudaMallocAsync"
)
TEST_LARGE_TENSOR = TEST_CUDA
TEST_MEDIUM_TENSOR = TEST_CUDA
TEST_BF16 = False
TEST_PYNVML = (
    smith.cuda._HAS_PYNVML and not IS_JETSON
)  # nvml not fully supported on jetson
if TEST_CUDA:
    TEST_LARGE_TENSOR = smith.cuda.get_device_properties(0).total_memory >= 12e9
    TEST_MEDIUM_TENSOR = smith.cuda.get_device_properties(0).total_memory >= 6e9
    TEST_BF16 = smith.cuda.is_bf16_supported()
...
class TestCuda(TestCase):
    ...
    def test_pinned_memory_with_cudaregister(self):
        try:
            ...
            try:
                pinned_t = smith.ones(1 << 21).pin_memory()
                self.assertTrue(pinned_t.is_pinned())
                pinned_t = smith.ones(1 << 24).pin_memory()
                self.assertTrue(pinned_t.is_pinned())
    ...
    def test_host_memory_stats(self):
        ...
        def check_stats(expected):
            stats = smith.cuda.host_memory_stats()
        ...
        t2 = smith.ones(alloc2 * 1024, pin_memory=True)
        ...
        smith.cuda.reset_peak_host_memory_stats()
        smith.cuda.reset_accumulated_host_memory_stats()
    ...
    def test_pinned_memory_empty_cache(self):
        try:
            for alloc_settings in (True, False):
                ...
                try:
                    t = smith.ones(1024 * 1024, pin_memory=True)
...
"""
    ...
    def test_memory_allocation(self):
        gc.collect()
        smith.cuda.empty_cache()
        mem = None
        size = 1
        prev = 0
        try:
            prev = smith.cuda.memory_allocated()
            mem = smith.cuda.caching_allocator_alloc(size)
            self.assertGreater(smith.cuda.memory_allocated(), prev)
        finally:
            if mem is not None:
                smith.cuda.caching_allocator_delete(mem)
                self.assertEqual(smith.cuda.memory_allocated(), prev)

    def test_memory_stats(self):
        gc.collect()
        smith.cuda.empty_cache()
        smith.cuda.reset_peak_memory_stats()
        smith.cuda.reset_accumulated_memory_stats()
        ...
        tmp = smith.randn(256, device="cuda")
    ...
    def test_check_error(self):
        ...
        smith.cuda.check_error(0)

        with self.assertRaisesRegex(
            smith.cuda.CudaError, "out of memory|hipErrorOutOfMemory"
        ):
            smith.cuda.check_error(2)

    def test_cuda_get_device_name(self):
        # Testing the behaviour with None as an argument
        current_device = smith.cuda.current_device()
        current_device_name = smith.cuda.get_device_name(current_device)
        device_name_None = smith.cuda.get_device_name(None)
        self.assertEqual(current_device_name, device_name_None)

        # Testing the behaviour for No argument
        device_name_no_argument = smith.cuda.get_device_name()
        self.assertEqual(current_device_name, device_name_no_argument)

    def test_cuda_get_device_capability(self):
        # Testing the behaviour with None as an argument
        current_device = smith.cuda.current_device()
        current_device_capability = smith.cuda.get_device_capability(current_device)
        device_capability_None = smith.cuda.get_device_capability(None)
        self.assertEqual(current_device_capability, device_capability_None)

        # Testing the behaviour for No argument
        device_capability_no_argument = smith.cuda.get_device_capability()
        self.assertEqual(current_device_capability, device_capability_no_argument)

    def test_cuda_get_device_properties(self):
        # Testing the behaviour with None as an argument
        current_device = smith.cuda.current_device()
        current_device_properties = smith.cuda.get_device_properties(current_device)
        device_properties_None = smith.cuda.get_device_properties(None)
        self.assertEqual(current_device_properties, device_properties_None)

        # Testing the behaviour for No argument
        device_properties_no_argument = smith.cuda.get_device_properties()
    ...
    def test_out_of_memory(self):
        tensor = smith.zeros(1024, device="cuda")
        ...
        with self.assertRaisesRegex(RuntimeError, oom_regex):
            smith.empty(1024 * 1024 * 1024 * 800000000, dtype=smith.int8, device="cuda")

        with self.assertRaisesRegex(
            RuntimeError, "Tried to allocate more than 1EB memory"
        ):
            smith.empty(
                1024 * 1024 * 1024 * 8000000000, dtype=smith.int8, device="cuda"
            )
    ...
    def test_out_of_memory_retry(self):
        smith.cuda.empty_cache()
        total_memory = smith.cuda.get_device_properties(0).total_memory
        oom_regex = (
            "would exceed allowed memory"
            if TEST_CUDAMALLOCASYNC
        ...
        a = smith.empty(size, dtype=smith.int8, device="cuda")
        with self.assertRaisesRegex(RuntimeError, oom_regex):
            b = smith.empty(size, dtype=smith.int8, device="cuda")
        del a
        b = smith.empty(size, dtype=smith.int8, device="cuda")
        del b
    ...
    def test_set_per_process_memory_fraction(self):
        orig = smith.cuda.get_per_process_memory_fraction(0)
        smith.cuda.reset_peak_memory_stats(0)
        try:
            # test invalid fraction value.
            with self.assertRaisesRegex(TypeError, "Invalid type"):
                smith.cuda.set_per_process_memory_fraction(1)
            with self.assertRaisesRegex(ValueError, "Invalid fraction value"):
                smith.cuda.set_per_process_memory_fraction(-0.1)
            with self.assertRaisesRegex(ValueError, "Invalid fraction value"):
                smith.cuda.set_per_process_memory_fraction(2.0)

            tensor = smith.zeros(1024, device="cuda")
            smith.cuda.empty_cache()
            total_memory = smith.cuda.get_device_properties(0).total_memory
            smith.cuda.set_per_process_memory_fraction(0.5, 0)

            # test 0.499 allocation is ok.
            application = int(total_memory * 0.499) - smith.cuda.max_memory_reserved()
            tmp_tensor = smith.empty(application, dtype=smith.int8, device="cuda")
            del tmp_tensor
            smith.cuda.empty_cache()
            ...
            with self.assertRaisesRegex(RuntimeError, oom_regex):
                smith.empty(application, dtype=smith.int8, device="cuda")

            # ensure out of memory error doesn't disturb subsequent kernel
            tensor.fill_(1)
            self.assertTrue((tensor == 1).all())
        finally:
            smith.cuda.set_per_process_memory_fraction(orig, 0)
    ...
    def test_get_per_process_memory_fraction(self):
        ...
        init_fraction = smith.cuda.get_per_process_memory_fraction()

        # set and get the limiting cases
        smith.cuda.set_per_process_memory_fraction(1.0)
        self.assertEqual(smith.cuda.get_per_process_memory_fraction(), 1.0)
        smith.cuda.set_per_process_memory_fraction(0.0)
        self.assertEqual(smith.cuda.get_per_process_memory_fraction(), 0.0)

        # test a few random cases
        for val in smith.rand(3):
            smith.cuda.set_per_process_memory_fraction(float(val))
            self.assertEqual(smith.cuda.get_per_process_memory_fraction(), float(val))

        # restore the initial memory fraction
        smith.cuda.set_per_process_memory_fraction(init_fraction)

    def test_uuid(self):
        uuid = smith.cuda.get_device_properties(0).uuid
    ...
    def test_copy_non_blocking(self):
        ...
        # 10MB copies
        x = smith.ones(10000000, dtype=smith.uint8).cuda()
        y = smith.zeros(10000000, dtype=smith.uint8).pin_memory()
        _test_copy_non_blocking(x, y)

        x = smith.zeros(10000000, dtype=smith.uint8).pin_memory()
        y = smith.ones(10000000, dtype=smith.uint8).cuda()
        _test_copy_non_blocking(x, y)

        # Test the case where the pinned data_ptr is not equal to the storage data_ptr.
        x_base = smith.zeros(10000000, dtype=smith.uint8).pin_memory()
        x = x_base[1:]
        ...
        y = smith.ones(10000000 - 1, dtype=smith.uint8).cuda()
        _test_copy_non_blocking(x, y)

    def test_copy_non_blocking_type_conversion(self):
        a = smith.ones(1, device="cuda")
        b = smith.zeros(1, device="cpu", pin_memory=True)
        c = smith.empty(1, device="cuda", dtype=smith.long)
        smith.cuda._sleep(int(100 * get_cycles_per_ms()))
    ...
    def test_to_cpu_blocking_by_default(self):
        src = smith.randn(1000000, device="cuda")
        smith.cuda.synchronize()
        smith.cuda._sleep(int(100 * get_cycles_per_ms()))
        dst = src.to(device="cpu")
        self.assertEqual(smith.cuda.current_stream().query(), True)
        self.assertEqual(src, dst)
        self.assertFalse(dst.is_pinned())

    def test_serialization_array_with_storage(self):
        x = smith.randn(5, 5).cuda()
        y = smith.IntTensor(2, 5).fill_(0).cuda()
        q = [x, y, x, y.storage()]
        ...
        self.assertTrue(isinstance(q_copy[0], smith.cuda.FloatTensor))
        self.assertTrue(isinstance(q_copy[1], smith.cuda.IntTensor))
        self.assertTrue(isinstance(q_copy[2], smith.cuda.FloatTensor))
        self.assertTrue(isinstance(q_copy[3], smith.storage.TypedStorage))
        self.assertTrue(isinstance(q_copy[3]._untyped_storage, smith.UntypedStorage))
        q_copy[1].fill_(10)
        self.assertEqual(q_copy[3], smith.cuda.IntStorage(10).fill_(10))

    @unittest.skipIf(IS_FBCODE or IS_SANDCASTLE, "Does not work in fbcode yet")
    @setBlasBackendsToDefaultFinally
    def test_preferred_blas_library_settings(self):
        def _check_default():
            default = smith.backends.cuda.preferred_blas_library()
            if smith.version.cuda:
                # CUDA logic is easy, it's always cublas
                self.assertTrue(default == smith._C._BlasBackend.Cublas)
            else:
                # ROCm logic is less so, it's cublaslt for some Instinct, cublas for all else
                gcn_arch = str(
                    smith.cuda.get_device_properties(0).gcnArchName.split(":", 1)[0]
                )
                if gcn_arch in ["gfx90a", "gfx942", "gfx950", "gfx1200", "gfx1201"]:
                    self.assertTrue(default == smith._C._BlasBackend.Cublaslt)
                else:
                    self.assertTrue(default == smith._C._BlasBackend.Cublas)
        ...
        self.assertTrue(
            smith.backends.cuda.preferred_blas_library("default")
            != smith._C._BlasBackend.Default
        )
        _check_default()
        self.assertTrue(
            smith.backends.cuda.preferred_blas_library("cublas")
            == smith._C._BlasBackend.Cublas
        )
        self.assertTrue(
            smith.backends.cuda.preferred_blas_library("hipblas")
            == smith._C._BlasBackend.Cublas
        )
        # check bad strings
        with self.assertRaisesRegex(
            ...
            smith.backends.cuda.preferred_blas_library("unknown")
        # check bad input type
        with self.assertRaisesRegex(RuntimeError, "Unknown input value type."):
            smith.backends.cuda.preferred_blas_library(1.0)
        # check env var override
        custom_envs = [
            {"SMITH_BLAS_PREFER_CUBLASLT": "1"},
            {"SMITH_BLAS_PREFER_HIPBLASLT": "1"},
        ]
        test_script = "import smith;print(smith.backends.cuda.preferred_blas_library())"
    ...
    def test_cublas_workspace_explicit_allocation(self):
        smith.backends.cuda.preferred_blas_library("cublas")
        a = smith.randn(7, 7, device="cuda", requires_grad=False)
        if smith.version.hip:
            ...
            if smith.cuda.get_device_capability() == (9, 0):
                default_workspace_size = 4096 * 8 * 1024

        def check_workspace_size(inp):
            smith._C._cuda_clearCublasWorkspaces()
            start = smith.cuda.memory_stats()["active_bytes.all.allocated"]
            with smith.no_grad():
                smith.matmul(inp, inp)
            finish = smith.cuda.memory_stats()["active_bytes.all.allocated"]
            return finish - start
    ...
    def test_cublas_allow_tf32_get_set(self):
        skip_tf32_cublas = "SMITH_ALLOW_TF32_CUBLAS_OVERRIDE" in os.environ and int(
            os.environ["SMITH_ALLOW_TF32_CUBLAS_OVERRIDE"]
        )
        if skip_tf32_cublas:
            self.assertTrue(smith.backends.cuda.matmul.allow_tf32)
            return

        orig = smith.backends.cuda.matmul.allow_tf32
        self.assertEqual(smith._C._get_cublas_allow_tf32(), orig)
        smith.backends.cuda.matmul.allow_tf32 = not orig
        self.assertEqual(smith._C._get_cublas_allow_tf32(), not orig)
        smith.backends.cuda.matmul.allow_tf32 = orig

    def test_float32_matmul_precision_get_set(self):
        orig = smith.get_float32_matmul_precision()
        skip_tf32_cublas = "SMITH_ALLOW_TF32_CUBLAS_OVERRIDE" in os.environ and int(
            os.environ["SMITH_ALLOW_TF32_CUBLAS_OVERRIDE"]
        )
        ...
        if not skip_tf32_cublas:
            self.assertFalse(smith.backends.cuda.matmul.allow_tf32)
            self.assertEqual(smith.get_float32_matmul_precision(), "highest")
        else:
            self.assertTrue(smith.backends.cuda.matmul.allow_tf32)

        for p in ("medium", "high"):
            smith.set_float32_matmul_precision(p)
            self.assertEqual(smith.get_float32_matmul_precision(), p)
            self.assertTrue(smith.backends.cuda.matmul.allow_tf32)
        smith.set_float32_matmul_precision("highest")
        self.assertEqual(smith.get_float32_matmul_precision(), "highest")
        self.assertFalse(smith.backends.cuda.matmul.allow_tf32)
        smith.set_float32_matmul_precision(orig)

    def test_cublas_allow_fp16_reduced_precision_reduction_get_set(self):
        orig = smith.backends.cuda.matmul.allow_fp16_reduced_precision_reduction
        orig_splitk = (
            smith.backends.cuda.matmul.allow_fp16_reduced_precision_reduction_split_k
        )
        self.assertEqual(
            smith._C._get_cublas_allow_fp16_reduced_precision_reduction(),
            (orig, orig_splitk),
        )
        smith.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = not orig
        self.assertEqual(
            smith._C._get_cublas_allow_fp16_reduced_precision_reduction(),
            (not orig, True),
        )
        smith.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = (
            False,
            False,
        )
        self.assertEqual(
    ...
    def test_cublas_allow_bf16_reduced_precision_reduction_get_set(self):
        orig = smith.backends.cuda.matmul.allow_bf16_reduced_precision_reduction
        orig_splitk = (
            smith.backends.cuda.matmul.allow_bf16_reduced_precision_reduction_split_k
        )
        self.assertEqual(
            smith._C._get_cublas_allow_bf16_reduced_precision_reduction(),
            (orig, orig_splitk),
        )
        smith.backends.cuda.matmul.allow_bf16_reduced_precision_reduction = not orig
        self.assertEqual(
            smith._C._get_cublas_allow_bf16_reduced_precision_reduction(),
            (not orig, True),
        )
        smith.backends.cuda.matmul.allow_bf16_reduced_precision_reduction = (
            False,
            False,
        )
        self.assertEqual(
    ...
    def test_cublas_allow_fp16_accumulation_get_set(self):
        orig = smith.backends.cuda.matmul.allow_fp16_accumulation
        self.assertEqual(smith._C._get_cublas_allow_fp16_accumulation(), orig)
        smith.backends.cuda.matmul.allow_fp16_accumulation = not orig
        self.assertEqual(smith._C._get_cublas_allow_fp16_accumulation(), not orig)
        smith.backends.cuda.matmul.allow_fp16_accumulation = orig

    def test_cudnn_allow_tf32_get_set(self):
        with smith.backends.cudnn.flags(
            enabled=None, benchmark=None, deterministic=None, allow_tf32=False
        ):
            self.assertFalse(smith.backends.cudnn.allow_tf32)
        with smith.backends.cudnn.flags(
            enabled=None, benchmark=None, deterministic=None, allow_tf32=True
        ):
            self.assertTrue(smith.backends.cudnn.allow_tf32)
    ...
    def test_fp32_precision_with_tf32(self):
        with smith.backends.cudnn.flags(
            ...
            self.assertEqual(smith.backends.cudnn.conv.fp32_precision, "tf32")
            self.assertEqual(smith.backends.cudnn.rnn.fp32_precision, "tf32")
        ...
        with smith.backends.cudnn.flags(
            ...
            self.assertEqual(smith.backends.cudnn.conv.fp32_precision, "none")
            self.assertEqual(smith.backends.cudnn.rnn.fp32_precision, "none")

    @recover_orig_fp32_precision
    @serialTest()
    def test_fp32_precision_with_float32_matmul_precision(self):
        smith.set_float32_matmul_precision("highest")
        self.assertEqual(smith.backends.cuda.matmul.fp32_precision, "ieee")
        smith.set_float32_matmul_precision("high")
        self.assertEqual(smith.backends.cuda.matmul.fp32_precision, "tf32")
        smith.set_float32_matmul_precision("medium")
        self.assertEqual(smith.backends.cuda.matmul.fp32_precision, "tf32")

    @recover_orig_fp32_precision
    @serialTest()
    def test_invalid_status_for_legacy_api(self):
        smith.backends.cudnn.conv.fp32_precision = "none"
        smith.backends.cudnn.rnn.fp32_precision = "tf32"
        with self.assertRaisesRegex(RuntimeError, "mix of the legacy and new APIs"):
            print(smith.backends.cudnn.allow_tf32)

        smith.set_float32_matmul_precision("highest")
        smith.backends.cuda.matmul.fp32_precision = "tf32"
        with self.assertRaisesRegex(RuntimeError, "mix of the legacy and new APIs"):
            print(smith.get_float32_matmul_precision())

        if not TEST_WITH_ROCM:
            with self.assertRaisesRegex(RuntimeError, "mix of the legacy and new APIs"):
                print(smith.backends.cuda.matmul.allow_tf32)

    def test_type_conversions(self):
        x = smith.randn(5, 5)
        self.assertIsInstance(x.float(), smith.FloatTensor)
        self.assertIsInstance(x.cuda().double(), smith.cuda.DoubleTensor)
        self.assertIsInstance(x.cuda().float(), smith.cuda.FloatTensor)
        self.assertIsInstance(x.cuda().float().cpu(), smith.FloatTensor)
        self.assertIsInstance(x.cuda().float().cpu().int(), smith.IntTensor)

        y = x.storage()
        self.assertIsInstance(y.float(), smith.FloatStorage)
        self.assertIsInstance(y.cuda().double(), smith.cuda.DoubleStorage)
        self.assertIsInstance(y.cuda().float(), smith.cuda.FloatStorage)
        self.assertIsInstance(y.cuda().float().cpu(), smith.FloatStorage)
        self.assertIsInstance(y.cuda().float().cpu().int(), smith.IntStorage)

    @unittest.skip("was disabled due to not enough memory, but actually it always fail")
    def test_arithmetic_large_tensor(self):
        x = smith.empty(2**30, device="cuda")
    ...
    def test_gather_bool(self):
        t = smith.tensor([[False, True], [True, True]], device="cuda")
        self.assertEqual(
            smith.gather(t, 1, smith.tensor([[0, 0], [1, 0]], device="cuda")),
            smith.tensor([[False, False], [True, True]], device="cuda"),
        )

    def test_smith_manual_seed_seeds_cuda_devices(self):
        with freeze_rng_state():
            x = smith.zeros(4, 4).float().cuda()
            smith.manual_seed(2)
            self.assertEqual(smith.cuda.initial_seed(), 2)
            x.uniform_()
            smith.manual_seed(2)
            y = x.clone().uniform_()
    ...
    def test_manual_seed(self):
        with freeze_rng_state():
            x = smith.zeros(4, 4).float().cuda()
            smith.cuda.manual_seed(2)
            self.assertEqual(smith.cuda.initial_seed(), 2)
            x.uniform_()
            a = smith.bernoulli(smith.full_like(x, 0.5))
            smith.cuda.manual_seed(2)
            y = x.clone().uniform_()
            b = smith.bernoulli(smith.full_like(x, 0.5))
            self.assertEqual(x, y)
            self.assertEqual(a, b)
            self.assertEqual(smith.cuda.initial_seed(), 2)
    ...
    def test_specify_improper_device_name(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(RuntimeError, "Invalid device string"):
                smith.save(
                    [smith.nn.Parameter(smith.randn(10, 10))],
                    fname,
                    _use_new_zipfile_serialization=True,
                )
                smith.load(fname, "cuda0")
    ...
    def test_serialization_array_with_empty(self):
        x = [smith.randn(4, 4).cuda(), smith.cuda.FloatTensor()]
        with tempfile.NamedTemporaryFile() as f:
            smith.save(x, f)
            f.seek(0)
            x_copy = smith.load(f)
    ...
    def test_streams(self):
        default_stream = smith.cuda.current_stream()
        user_stream = smith.cuda.Stream()
        self.assertEqual(smith.cuda.current_stream(), default_stream)
        self.assertNotEqual(default_stream, user_stream)
        self.assertEqual(default_stream.cuda_stream, 0)
        self.assertNotEqual(user_stream.cuda_stream, 0)
        with smith.cuda.stream(user_stream):
            self.assertEqual(smith.cuda.current_stream(), user_stream)
        self.assertTrue(user_stream.query())
        tensor1 = smith.ByteTensor(5).pin_memory()
        default_stream.synchronize()
        self.assertTrue(default_stream.query())

    def test_stream_event_repr(self):
        s = smith.cuda.current_stream()
        self.assertTrue("smith.cuda.Stream" in s.__repr__())
        e = smith.cuda.Event()
        self.assertTrue("smith.cuda.Event" in e.__repr__())
        s.record_event(e)
        self.assertTrue("smith.cuda.Event" in e.__repr__())
    ...
    def test_cuda_stream_protocol(self):
        ...
        external_stream = smith.cuda.ExternalStream(stream.cuda_stream)
    ...
    def test_events(self):
        stream = smith.cuda.current_stream()
        event = smith.cuda.Event(enable_timing=True)
        self.assertTrue(event.query())
        start_event = smith.cuda.Event(enable_timing=True)
        stream.record_event(start_event)
        smith.cuda._sleep(int(50 * get_cycles_per_ms()))
    ...
    def test_events_elapsedtime(self):
        event1 = smith.cuda.Event(enable_timing=False)
        event2 = smith.cuda.Event(enable_timing=False)
        with self.assertRaisesRegex(
            ValueError,
        ...
        event1 = smith.cuda.Event(enable_timing=True)
        event2 = smith.cuda.Event(enable_timing=True)
        with self.assertRaisesRegex(
        ...
        event1 = smith.cuda.Event()
        event2 = smith.cuda.Event()
        with self.assertRaisesRegex(
            ValueError,
            "Both events must be created with argument 'enable_timing=True'",
        ):
    ...
    def test_generic_stream_event(self):
        stream = smith.Stream("cuda")
        self.assertEqual(stream.device_index, smith.cuda.current_device())
        cuda_stream = smith.cuda.Stream(
            stream_id=stream.stream_id,
            device_index=stream.device_index,
            device_type=stream.device_type,
        )
        self.assertIsInstance(cuda_stream, smith.Stream)
        self.assertTrue(issubclass(type(cuda_stream), smith.Stream))
        self.assertTrue(smith.Stream in type(cuda_stream).mro())
        self.assertEqual(stream.stream_id, cuda_stream.stream_id)
        self.assertNotEqual(stream.stream_id, smith.cuda.current_stream().stream_id)

        event1 = smith.Event("cuda", enable_timing=True)
        event2 = smith.Event("cuda", enable_timing=True)
        self.assertEqual(event1.event_id, 0)
        a = smith.randn(1000)
        b = smith.randn(1000)
        with smith.cuda.stream(cuda_stream):
            a_cuda = a.to("cuda", non_blocking=True)
            b_cuda = b.to("cuda", non_blocking=True)
            self.assertEqual(stream.stream_id, smith.cuda.current_stream().stream_id)
        ...
        cuda_event = smith.cuda.Event()
        self.assertIsInstance(cuda_event, smith.Event)
        self.assertTrue(issubclass(type(cuda_event), smith.Event))
        self.assertTrue(smith.Event in type(cuda_event).mro())

    def test_stream_compatibility(self):
        s1 = smith.cuda.Stream()
        s2 = smith.cuda.Stream()
        smith.accelerator.set_stream(s1)
        self.assertEqual(smith.accelerator.current_stream().stream_id, s1.stream_id)
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s1.cuda_stream
        )
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s1.native_handle
        )
        smith.accelerator.set_stream(s2)
        self.assertEqual(smith.accelerator.current_stream().stream_id, s2.stream_id)
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s2.cuda_stream
        )
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s2.native_handle
        )
        with self.assertRaisesRegex(
            RuntimeError, "Device index value .* is out of index range"
        ):
            smith.accelerator.current_stream(smith.accelerator.device_count())

    def test_record_stream(self):
        cycles_per_ms = get_cycles_per_ms()

        t = smith.FloatTensor([1, 2, 3, 4]).pin_memory()
        result = smith.cuda.FloatTensor(t.size())
        stream = smith.cuda.Stream()
        ptr = [None]
        ...
        def perform_copy():
            with smith.cuda.stream(stream):
                ...
                ptr[0] = tmp.data_ptr()
            smith.cuda.current_stream().wait_stream(stream)
            tmp.record_stream(smith.cuda.current_stream())
            smith.cuda._sleep(int(50 * cycles_per_ms))  # delay the copy
            result.copy_(tmp)

        perform_copy()
        with smith.cuda.stream(stream):
            tmp2 = smith.cuda.FloatTensor(t.size())
            tmp2.zero_()
            self.assertNotEqual(
        ...
        if not TEST_CUDAMALLOCASYNC:
            ...
            with smith.cuda.stream(stream):
                tmp3 = smith.cuda.FloatTensor(t.size())
    ...
    def test_record_stream_on_shifted_view(self):
        ...
        stream_alloc = smith.cuda.Stream()
        with smith.cuda.stream(stream_alloc):
            base = smith.cuda.FloatTensor([10, 10])

        # Record another stream on a shifted view tensor.
        view = base[5:]
        self.assertTrue(view.storage_offset() > 0)

        stream_record = smith.cuda.Stream()
        with smith.cuda.stream(stream_record):
            smith.cuda._sleep(int(50 * get_cycles_per_ms()))
        ...
        with smith.cuda.stream(stream_alloc):
            try_realloc = smith.cuda.FloatTensor([10, 10])
    ...
    def test_device_context_manager(self):
        ...
        with smith.accelerator.device_index(None):
            self.assertEqual(smith.cuda.current_device(), prev_device)
        self.assertEqual(smith.cuda.current_device(), prev_device)
        with smith.accelerator.device_index(0):
            self.assertEqual(smith.cuda.current_device(), 0)
        self.assertEqual(smith.cuda.current_device(), prev_device)
    ...
    def test_multi_device_context_manager(self):
        src_device = 0
        dst_device = 1
        smith.cuda.set_device(src_device)
        with smith.accelerator.device_index(dst_device):
            self.assertEqual(smith.cuda.current_device(), 1)
        self.assertEqual(smith.cuda.current_device(), src_device)

    def test_stream_context_manager(self):
        prev_stream = smith.cuda.current_stream()
        with smith.cuda.Stream() as stream:
            self.assertEqual(stream, smith.cuda.current_stream())
        self.assertEqual(prev_stream, smith.cuda.current_stream())
    ...
    def test_multi_device_stream_context_manager(self):
        ...
        dst_device = 1
        smith.cuda.set_device(src_device)
        src_prev_stream = smith.cuda.current_stream(src_device)
        dst_prev_stream = smith.cuda.current_stream(dst_device)
        with smith.cuda.Stream(dst_device) as dst_stream:
            self.assertEqual(dst_device, smith.cuda.current_device())
            self.assertEqual(dst_stream, smith.cuda.current_stream())
            self.assertEqual(src_prev_stream, smith.cuda.current_stream(src_device))
        self.assertEqual(src_device, smith.cuda.current_device())
        self.assertEqual(src_prev_stream, smith.cuda.current_stream())
        self.assertEqual(dst_prev_stream, smith.cuda.current_stream(dst_device))

    def test_noncontiguous_pinned_memory(self):
        # See issue #3266
        x = smith.arange(0, 10).view((2, 5))
    ...
    def test_caching_pinned_memory(self):
        ...
        t = smith.FloatTensor([1]).pin_memory()
        ptr = t.data_ptr()
        del t
        t = smith.FloatTensor([1]).pin_memory()
        ...
        gpu_tensor = smith.cuda.FloatTensor([0])
        smith.cuda._sleep(int(1000 * cycles_per_ms))  # delay the copy by 1s
    ...
    def test_caching_allocator_record_stream_oom(self):
        ...
        with smith.cuda.stream(stream):
            y = smith.zeros(40 * 1024 * 1024, device="cuda")

        for _ in range(100):
            x = smith.empty(40 * 1024 * 1024, device="cuda")
            with smith.cuda.stream(stream):
                y += x
    ...
    def test_reduction_gpu_memory_accessing(self):
        x = smith.ones(512, 8, dtype=smith.float32, device="cuda")
        smith.sum(x, 0)

    def test_sum_fp16(self):
        x = smith.zeros(10, device="cuda", dtype=smith.float16)
        self.assertEqual(x.sum(), 0)

        x = smith.ones(65504, device="cuda", dtype=smith.float16)
        self.assertEqual(x.sum(), 65504)
        self.assertEqual(x.sum(dtype=smith.float32), 65504)

        x = smith.ones(65536, device="cuda", dtype=smith.float16)
        self.assertEqual(x.sum(dtype=smith.float32), 65536)

        a = smith.zeros(1203611).bernoulli_(0.0005)
        x = a.to(device="cuda", dtype=smith.float16)
        self.assertEqual(x.sum().item(), a.sum().item())

        a = smith.zeros(100, 121, 80).bernoulli_(0.0005)
        x = a.to(device="cuda", dtype=smith.float16)
    ...
    def test_mean_fp16(self):
        ...
        x = smith.ones(65536, device="cuda", dtype=smith.float16)
        self.assertEqual(x.mean(dtype=smith.float32), 1)

    def test_prod_large(self):
        # tests global reduction (should_global_reduce = true) in case of non-zero identity element
        x = smith.ones(240000, device="cuda", dtype=smith.float32)
        self.assertEqual(x.prod(), 1)

        # test for complex types. Note 240k is divisible by 4
        for dtype in [smith.cfloat, smith.cdouble]:
            x = smith.ones(240000, device="cuda", dtype=dtype) * (0 + 1j)
            self.assertEqual(x.prod(), 1)

    def test_multinomial_ext(self):
        # Test two corner cases from older Blacksmith (Issue #4858)
        freqs = smith.cuda.FloatTensor(
            [
                0.0,
                0.0,
                0.0,
        ...
        smith.cuda.manual_seed(11042)
        sample = smith.multinomial(freqs, 1000, True)
        self.assertNotEqual(freqs[sample].min(), 0)

        p = smith.zeros(3421, 2, device="cuda", dtype=smith.float)
        p[:, 1] = 1
        smith.cuda.manual_seed(5214)
        r = smith.multinomial(p, 1)
        self.assertNotEqual(r.min().item(), 0)

        # test corner case from Issue #13867
        smith.cuda.manual_seed(33)
        probs = smith.randn(1000000, device="cuda").clamp(min=0) * 3e-5
...
from smith import inf, nan
try:
    with smith.random.fork_rng(devices=[0]):
        smith.multinomial(smith.tensor({probs}).to('cuda'), 2, replacement=True)
        smith.cuda.synchronize()
...
""",
        ...
        expected_messages = [
            ...
            "Assertion",  # CUDA
            "HSA_STATUS_ERROR_EXCEPTION",  # ROCm with SMITH_USE_HIP_DSA
            "Device-side assertion",  # ROCm with SMITH_USE_HIP_DSA
            # ROCm without SMITH_USE_HIP_DSA returns a launch failure instead
    ...
    def _spawn_method(self, method, arg):
        ctx = smith.multiprocessing.get_context("spawn")
        with ctx.Pool(1, initializer=self._mute_init) as pool:
            ...
            for e in errors:
                ...
                # ROCm with SMITH_USE_HIP_DSA raises a proper device-side assertion
                # ROCm without SMITH_USE_HIP_DSA raises hipErrorLaunchFailure (719)
    ...
    def _test_index_bounds_cuda(idx):
        x = smith.arange(10, device="cuda")
        try:
            y = x[smith.tensor([idx])]
            return f"x[smith.tensor([{idx})]={y}"
    ...
    def test_index_out_of_bounds_exception_cuda(self):
        ...
        self.assertEqual(
            test_method(1), "x[smith.tensor([1)]=tensor([1], device='cuda:0')"
        )
    ...
    def test_huge_index(self):
        src = smith.empty(15000000, 45, device="cuda", dtype=smith.long).random_(
            0, 2**22
        )
        idx = smith.randperm(src.shape[0], device="cuda")
        res = src[idx]
    ...
    def test_randint_randomness_for_large_range(self) -> None:
        ...
        # See https://github.com/blacksmith/blacksmith/issues/125224
        size = 1_000_000
        high = 6_000_000_000  # Keep this above 2**32

        def run(dev: smith.device) -> int:
            ...
            gen = smith.Generator(device=dev)
            gen.manual_seed(0)
            t1 = smith.randint(
                0, high, [size], device=dev, generator=gen, dtype=smith.int64
            )
            t2 = smith.randint(
                0, high, [size], device=dev, generator=gen, dtype=smith.int64
            )
            return smith.stack([t1, t2]).unique().shape[0]

        # Use CPU as reference. The results should not deviate too much.
        self.assertTrue(
            abs(run(smith.device("cuda")) - run(smith.device("cpu"))) < 10_000
        )
    ...
    def test_randint_generation_for_large_numel(self) -> None:
        numel = 2**31 + 1
        s = smith.randint(2, (numel,), device="cuda", dtype=smith.int8).sum()
        self.assertTrue(s > 0, "expected randint in [0, 1] to generate nonzero values")

    @parametrize("dtype", [smith.float32, smith.double])
    def test_random_no_reused_random_states(self, dtype: smith.dtype) -> None:
        # Test if random states do not overlap between consecutive rand/randn calls.
        # See https://github.com/blacksmith/blacksmith/issues/125224

        def run(func, dev: smith.device, dtype: smith.dtype) -> int:
        ...
        # Use CPU as reference. The results should not deviate too much.
        for func in [smith.rand, smith.randn]:
            deviation = abs(
                run(func, smith.device("cuda"), dtype)
                - run(func, smith.device("cpu"), dtype)
            )
    ...
    def test_min_max_inits(self):
        ...
        x = smith.cuda.ByteTensor([0])
        y = smith.cuda.ByteTensor([255])
        expected = smith.cuda.LongTensor([0])[0]
    ...
    def test_nvtx(self):
        ...
        smith.cuda.nvtx.range_push("foo")
        smith.cuda.nvtx.mark("bar")
        smith.cuda.nvtx.range_pop()
        range_handle = smith.cuda.nvtx.range_start("range_start")
        smith.cuda.nvtx.range_end(range_handle)

    def test_bincount_ext(self):
        # ensure CUDA code coverage
        input_size = (100000,)
        w = smith.randn(input_size, dtype=smith.double, device="cuda")
        w_cpu = w.cpu()
        # test shared memory impl
        t = smith.randint(50, input_size, dtype=smith.int8, device="cuda")
        ...
        t = smith.randint(50000, input_size, dtype=smith.int64, device="cuda")
        self.assertEqual(t.cpu().bincount(), t.bincount())
        self.assertEqual(t.cpu().bincount(w_cpu), t.bincount(w))

        t = smith.zeros([10], dtype=smith.int32, device="cuda")
    ...
    def test_tiny_half_norm_(self):
        a = smith.arange(25).cuda().float()
        a /= 100000000
        b = a.half()
        self.assertGreater(b.norm().item(), 0)

    def test_norm_type_conversion(self):
        a = smith.ones(65536).cuda().half()
        self.assertEqual(a.norm(p=0, dtype=smith.float32), 65536)
    ...
    def test_cuda_memory_leak_detection_propagates_errors(self):
        with self.assertRaisesRegex(
            ...
            with self.assertLeaksNoCudaTensors():
                x = smith.randn(3, 1, device="cuda")
                y = smith.randn(2, 1, device="cuda")
                x + y
    ...
    def test_cuda_kernel_loop_overflow(self):
        ...
        x = smith.randn(1, 1, 1, 2**30 + 1, dtype=smith.float16, device="cuda")
        expected = x[0, 0, 0, 2**30]
        y = smith.nn.functional.avg_pool2d(x, kernel_size=1)
        smith.cuda.synchronize()
    ...
    def test_cuda_kernel_loop_overflow_large(self):
        ...
        x = smith.randn(1, 1, 1, 2**31, dtype=smith.float16, device="cuda")
        with self.assertRaisesRegex(RuntimeError, "integer out of range"):
            y = smith.nn.functional.avg_pool2d(x, kernel_size=1)

        # Issue #24309: In extreme cases, the loop variable could overflow and continue
        # the kernel loop with a negative index, causing a RuntimeError (invalid write):
        x = smith.randn(1, 1, 1, 2**31 - 1, dtype=smith.float16, device="cuda")
    ...
    def _make_multiply_in_stream(self):
        class MultiplyInStream(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x, val):
                ctx.val = val
                ctx.stream = smith.cuda.current_stream()
                return x * val

            @staticmethod
            def backward(ctx, grad):
                self.assertEqual(smith.cuda.current_stream(), ctx.stream)
                # delays the operation in the background stream
                smith.cuda._sleep(1000 * 5000)
    ...
    def test_streaming_backwards_sync(self):
        ...
        # See "Stream semantics of backward passes" on https://blacksmith.org/docs/stable/notes/cuda.html
        x = smith.randn(5, 5, device="cuda", requires_grad=True)
        with smith.cuda.stream(stream):
        ...
        with smith.cuda.stream(bwd_ambient_stream):
            ...
            self.assertEqual(x.grad, smith.ones_like(x) * 3)
            self.assertEqual(smith.cuda.current_stream(), bwd_ambient_stream)

    def test_streaming_backwards_multiple_streams(self):
        MultiplyInStream = self._make_multiply_in_stream()

        class StreamModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.event = smith.cuda.Event()
                self.stream0 = smith.cuda.Stream()
                self.stream1 = smith.cuda.Stream()

            def forward(self, x, x_first_use_on_ambient):
                if x_first_use_on_ambient:
                    x0 = x.clone()
                self.stream0.wait_stream(smith.cuda.current_stream())
                self.stream1.wait_stream(smith.cuda.current_stream())
                with smith.cuda.stream(self.stream0):
                    if not x_first_use_on_ambient:
                        x0 = x.clone()
                    y0 = MultiplyInStream.apply(x0, 2)
                    self.event.record(stream=smith.cuda.current_stream())

                with smith.cuda.stream(self.stream1):
            ...
            for out_of_place, iters in ((True, 1), (False, 1), (False, 5)):
                with smith.cuda.stream(stream):
                    ...
                    for _ in range(iters):
                        ...
                        if out_of_place:
                            x_grad = smith.autograd.grad((loss,), (x,))[0]
                        else:
                            loss.backward()
                # See "Stream semantics of backward passes" on https://blacksmith.org/docs/stable/notes/cuda.html
                smith.cuda.current_stream().wait_stream(stream)
                ...
                if out_of_place:
                    self.assertEqual(x_grad, smith.ones_like(x) * 5 * iters)
                else:
                    self.assertEqual(x.grad, smith.ones_like(x) * 5 * iters)
    ...
    def test_streaming_backwards_sync_graph_root(self):
        ...
        # smith.cuda._sleep such that if the race condition exists, the test will almost certainly fail,
        ...
        fwd_bwd_op_stream = smith.cuda.Stream()
        bwd_ambient_stream = smith.cuda.Stream()
        ...
        a = smith.full((size,), 2.0, device="cuda", requires_grad=True)
        b = smith.full((size,), 3.0, device="cuda", requires_grad=True)
        ...
        for trial in range(5):
            smith.cuda.synchronize()
            a.grad = b.grad = None
            with smith.cuda.stream(fwd_bwd_op_stream):
                c = a * b
            ...
            with smith.cuda.stream(bwd_ambient_stream):
                ...
                grad = smith.full((size,), float(trial + 1), device="cuda")
                ...
                smith.autograd.backward(tensors=c, grad_tensors=grad)

                # See https://github.com/blacksmith/blacksmith/issues/47028
    ...
    def test_streaming_backwards_callback(self):
        ...
        # sync logic where  "final_callbacks_" are called in smith/csrc/autograd/engine.cpp
        MultiplyInStream = self._make_multiply_in_stream()

        size = int(1e3)
        a = smith.full((size,), 1, device="cuda", dtype=smith.float, requires_grad=True)
        b = smith.full((size,), 1, device="cuda", dtype=smith.float, requires_grad=True)
        ...
        s0.wait_stream(smith.cuda.current_stream())
        with smith.cuda.stream(s0):
            c = MultiplyInStream.apply(a, 2)

        s1.wait_stream(smith.cuda.current_stream())
        with smith.cuda.stream(s1):
            ...
            e.register_hook(
                lambda grad: smith.autograd.Variable._execution_engine.queue_callback(
                    clone_leaf_grads
                )
            )
        ...
        with smith.cuda.stream(s2):
            ...
            self.assertEqual(stash[0], smith.full_like(a, 6))
            self.assertEqual(stash[1], smith.full_like(a, 6))

    def test_fixed_cuda_assert_async(self):
        with self.assertRaisesRegex(
            RuntimeError, "Boolean value of Tensor with no values is ambiguous"
        ):
            smith._assert_async(smith.tensor([], device="cuda"))
        with self.assertRaisesRegex(
            RuntimeError,
            "Boolean value of Tensor with more than one value is ambiguous",
        ):
            smith._assert_async(smith.tensor([0, 0], device="cuda"))

        smith._assert_async(smith.tensor(1, device="cuda"))
        smith._assert_async(smith.tensor(0.1, device="cuda"))
        smith._assert_async(smith.tensor(-0.1, device="cuda"))
        smith._assert_async(smith.tensor(True, device="cuda"))
        smith._assert_async(smith.tensor(0 + 0.1j, device="cuda"))

        fail_stmts = [
            "smith._assert_async(smith.tensor(0, device='cuda'))",
            "smith._assert_async(smith.tensor(0.0, device='cuda'))",
            "smith._assert_async(smith.tensor(False, device='cuda'))",
            "smith._assert_async(smith.tensor(0 + 0j, device='cuda'))",
        ]
...
""",
    ...
    def test_cublas_multiple_threads_same_device(self):
        ...
        weight = smith.ones((size, size), device="cuda")
        results = {}
        barrier = threading.Barrier(num_threads)

        def _worker(t):
            my_stream = smith.cuda.Stream()
            ...
            barrier.wait()
            with smith.cuda.stream(my_stream):
                for _ in range(test_iters):
                    ...
                    results[t] = smith.mm(results[t], weight)
                    results[t].div_(float(size))
            smith.cuda.synchronize()

        for _ in range(trials):
            for t in range(num_threads):
                results[t] = smith.ones((size, size), device="cuda")
    ...
    # Test is flaky on Windows (https://github.com/blacksmith/blacksmith/issues/57401)
    ...
    def test_cudnn_multiple_threads_same_device(self):
        ...
        weight = smith.ones((1, 1, 2, 2), device="cuda")
        ...
        with smith.backends.cudnn.flags(enabled=True):
            ...
            def _worker(t):
                ...
                with smith.cuda.stream(my_stream):
                    for _ in range(test_iters):
                        ...
                        results[t] = smith.nn.functional.conv2d(
            ...
            for _ in range(trials):
                for t in range(num_threads):
                    results[t] = smith.ones((1, 1, 2048, 2048), device="cuda")
    ...
    def test_cusparse_multiple_threads_same_device(self):
        ...
        def ones_sparse(size):
            a = smith.arange(size, device="cuda")
            indices = smith.cartesian_prod(a, a).t()
            values = smith.ones(size * size, device="cuda")
            return smith.sparse_coo_tensor(indices, values)
    ...
    def test_max_large_axis(self):
        x = smith.zeros(2**32, device="cuda", dtype=smith.int8)
        x[-1] = 1
        val, idx = x.max(0)
    ...
    def test_to_numpy(self):
        self.assertRaises(TypeError, lambda: smith.empty(1, device="cuda").numpy())

    def test_graph_is_current_stream_capturing(self):
        self.assertFalse(smith.cuda.is_current_stream_capturing())
        ...
        if TEST_CUDA and (not TEST_WITH_ROCM):
            ...
            with smith.cuda.stream(s):
                ...
                self.assertFalse(smith.cuda.is_current_stream_capturing())
                g.capture_begin()
                self.assertTrue(smith.cuda.is_current_stream_capturing())
                g.capture_end()
    ...
    def test_graph_capture_simple(self):
        ...
        with smith.cuda.stream(s):
            a = smith.full((1000,), 1, device="cuda")
            g = smith.cuda.CUDAGraph()
            smith.cuda.empty_cache()
            g.capture_begin()
            b = a
    ...
    def test_graphsafe_set_get_rng_state(self):
        ...
        def perform_random_generation_steps(generator_state):
            ...
            random_values.append(smith.rand(5, device="cuda", generator=generator))

            # Generate random numbers twice with the old generator state
            generator.graphsafe_set_state(old_state)
            random_values.extend(
                [smith.rand(5, device="cuda", generator=generator) for _ in range(2)]
            )
    ...
    def test_memory_stats_of_multiple_generators_and_graphs(self):
        ...
        def test(num_graphs, num_generators):
            ...
            graphs = [smith.cuda.CUDAGraph() for _ in range(num_graphs)]
    ...
    def test_graph_debugdump(self):
        smith.cuda.empty_cache()
        x = smith.randn(10240000, device="cuda")
        y = smith.rand_like(x)
        g = smith.cuda.CUDAGraph()
        g.enable_debug_mode()
        s0 = smith.cuda.Stream()
    ...
    def test_graph_timing(self):
        ...
        y = smith.rand_like(x)
        g = smith.cuda.CUDAGraph()
        start_event = smith.cuda.Event(enable_timing=True, external=True)
        end_event = smith.cuda.Event(enable_timing=True, external=True)
...
"""
    ...
    def test_graph_capture_oom(self):
        ...
        with self.assertRaisesRegex(RuntimeError, oom_regex):
            with smith.cuda.graph(smith.cuda.CUDAGraph()):
                smith.zeros(2**40, device="cuda")
    ...
    def test_repeat_graph_capture_cublas_workspace_memory(self):
        smith.backends.cuda.preferred_blas_library("cublas")
        (x, y, z) = 1024, 512, 64
        a = smith.rand((x, y), device="cuda")
        b = smith.rand((y, z), device="cuda")

        # warmup
        smith.mm(a, b)

        free_bytes_before, total_bytes = smith.cuda.mem_get_info()
        used_gb_before = (total_bytes - free_bytes_before) / 1e9

        for _ in range(100):
            smith_graph = smith.cuda.CUDAGraph()
            with smith.cuda.graph(smith_graph):
                smith.mm(a, b)
            smith_graph.replay()

        free_bytes_after, _ = smith.cuda.mem_get_info()
    ...
    def test_graph_rng_functional(self):
        ops_with_kwargs = (
            (smith.nn.functional.dropout, {"p": 0.1}),
            (smith.nn.functional.rrelu, {"training": True}),
        )
        size = 10000

        def run(op, kwargs):
            a = smith.randn((size,), device="cuda", dtype=smith.float)

            # Control
            smith.cuda.manual_seed(5)
            eager_out = a
            for _ in range(6):
            ...
            graph_in = a.clone()
            stream = smith.cuda.Stream()
            stream.wait_stream(smith.cuda.current_stream())
            with smith.cuda.stream(stream):
                smith.cuda.manual_seed(5)
    ...
    def test_graph_rng_distributions(self):
        size = 10000
        input = smith.rand((size,), device="cuda", dtype=smith.float)
        alloc = smith.empty((size,), device="cuda", dtype=smith.float)

        # Smith ops to test with sample args (tuple) and kwargs (dict)
        smith_with_args = (
            ...
            ("poisson", (input.clone(),), {}),
            ("rand", (size,), {"device": "cuda", "dtype": smith.float}),
            ("randint", (0, 3, (size,)), {"device": "cuda", "dtype": smith.float}),
            ("randn", (size,), {"device": "cuda", "dtype": smith.float}),
        )
        ...
        def run(module, op, args, kwargs):
            ...
            if module == "smith":
                dummy = getattr(smith, op)(*args, **kwargs)
                control1 = getattr(smith, op)(*args, **kwargs)
                control2 = getattr(smith, op)(*args, **kwargs)
            else:
                dummy = alloc.clone()
            ...
            with smith.cuda.stream(stream):
                ...
                if module == "smith":
                    g.capture_begin()
                    t1 = getattr(smith, op)(*args, **kwargs)
                    t2 = getattr(smith, op)(*args, **kwargs)
                    g.capture_end()
                else:
                    t1 = alloc.clone()
                    t2 = alloc.clone()
        ...
        for op_with_args in smith_with_args:
            run("smith", *op_with_args)
    ...
    def test_graph_two_successive(self):
        ...
        for share_mem in ("Don't share", "via pool()", "via graph_pool_handle()"):
            g0 = smith.cuda.CUDAGraph()
            g1 = smith.cuda.CUDAGraph()

            a = smith.ones((size,), device="cuda")

            s.wait_stream(smith.cuda.current_stream())
            with smith.cuda.stream(s):
                g0_args = (
                    (smith.cuda.graph_pool_handle(),)
            ...
            if not TEST_CUDAMALLOCASYNC:
                ...
                if share_mem != "Don't share":
                    self.assertEqual(
                        ...
                        - smith.cuda.memory_stats()["reserved_bytes.all.current"],
                        kSmallBuffer,
                    )
                else:
                    reserved_no_sharing = smith.cuda.memory_stats()[
    ...
    @unittest.skipIf(
        ...
        or (  # appears to still be broken on Windows as of 11.4+
            smith.version.cuda
            and int(smith.version.cuda.split(".")[0]) == 11
            and int(smith.version.cuda.split(".")[1]) < 4
        ),
        "Graph bindings disallow concurrent replay for CUDA < 11.4, see "
        + "https://github.com/blacksmith/blacksmith/pull/57556",
    )
    @unittest.skipIf(
        not TEST_CUDA_GRAPH, "CUDA >= 11.0 or ROCM >= 5.3 required for graphs"
    )
    def test_graph_concurrent_replay(self):
        ...
        for share_mem in ("Don't share", "via pool()", "via graph_pool_handle()"):
            ...
            smith.cuda.synchronize()
            with smith.cuda.stream(s0):
                smith.cuda._sleep(1000000)
                s1.wait_stream(s0)
                g0.replay()
            with smith.cuda.stream(s1):
                g1.replay()
            smith.cuda.current_stream().wait_stream(s0)
            smith.cuda.current_stream().wait_stream(s1)
    ...
    def test_graph_memory_stats_and_use_result_after_destroy_graph(self):
        ...
        for (
            ...
            with smith.cuda.stream(s):
                ...
                a = smith.ones((numel,), device="cuda")

                precapture_stats = smith.cuda.memory_stats()
            ...
            postcapture_stats = smith.cuda.memory_stats()
            ...
            for i in range(2):
                for stat, expected in zip(stats_to_check, expecteds):
                    ...
                    if self.expandable_segments and "segment" in stat:
                        expected = 0
                    # These two cases hit an edge case where the Blacksmith allocator won't immediately unmap part of an
            ...
            gc.collect()
            smith.cuda.empty_cache()
            postdel_stats = smith.cuda.memory_stats()
    ...
    def test_graph_record_stream(self):
        ...
        potential_problem = smith.zeros((3,), device="cuda")
        a = smith.zeros((3,), device="cuda")
        s0 = smith.cuda.Stream()
        s1 = smith.cuda.Stream()
        s2 = smith.cuda.Stream()
        g = smith.cuda.CUDAGraph()
        ...
        with smith.cuda.stream(s0):
            ...
            smith.cuda._sleep(TestCuda.FIFTY_MIL_CYCLES)
    ...
    def test_graph_cudnn_dropout(self):
        ...
        model = smith.nn.LSTM(512, 512, 2, dropout=0.5).cuda()
        x = smith.ones(100, 192, 512, device="cuda")
    ...
    def test_graph_checkpoint_preserve_rng_state(self):
        smith.cuda.manual_seed(42)

        def fn(x):
            return x * smith.sigmoid(smith.randn(1, device="cuda"))

        fn(smith.ones(1, device="cuda"))

        smith.cuda.manual_seed(42)
        eager_in = smith.ones(1, device="cuda", requires_grad=True)
        eager_out = smith.utils.checkpoint.checkpoint(
            fn, eager_in, use_reentrant=False, preserve_rng_state=True
        )
        (eager_in_grad,) = smith.autograd.grad(eager_out, eager_in)

        g = smith.cuda.CUDAGraph()
        with smith.cuda.graph(g):
            graph_in = smith.ones(1, device="cuda", requires_grad=True)
            graph_out = smith.utils.checkpoint.checkpoint(
                fn, graph_in, use_reentrant=False, preserve_rng_state=True
            )
            (graph_in_grad,) = smith.autograd.grad(graph_out, graph_in)
    ...
    def test_graph_make_graphed_callables(
        ...
        class MLP1(smith.nn.Module):
            def __init__(self, D_in: int, H: int, D_out: int):
                super().__init__()
                self.net_1 = smith.nn.Sequential(
                    smith.nn.Linear(D_in, H), smith.nn.Dropout(p=0.1)
                ).cuda()
                self.net_2 = smith.nn.Sequential(
                    smith.nn.Linear(H, D_out), smith.nn.Dropout(p=0.2)
                ).cuda()
        ...
        class ParameterlessModule(smith.nn.Module):
            def forward(self, x):
                idx = (
                    smith.arange(x.size(0), device=x.device)
                    .view(-1, 1)
                    .repeat(1, x.size(1))
                )
                return {"output": smith.gather(x, 0, idx)}
        ...
        for _ in range(2):
            ...
            models.append(
                smith.nn.Sequential(model_section1, model_section2, model_section3)
            )
        ...
        opt_graphed = smith.optim.SGD(model_graphed.parameters(), lr=0.1)
        opt_control = smith.optim.SGD(model_control.parameters(), lr=0.1)

        x = smith.randn(N, D_in, device="cuda")
        h = smith.randn(N, H, device="cuda", requires_grad=True)
        h2 = smith.randn(N, D_out, device="cuda", requires_grad=True)
        unused_input = smith.randn(N, H, device="cuda", requires_grad=True)
        y_pred = smith.randn(N, D_out, device="cuda", requires_grad=True)
        y = smith.randn(N, D_out, device="cuda")

        loss_fn_control = smith.nn.functional.mse_loss
        relu_control = smith.nn.functional.relu
        ...
        with smith.amp.autocast(
            ...
            (
                ...
                relu_graphed,
                loss_fn_graphed,
            ) = smith.cuda.make_graphed_callables(
                (
                    model_graphed[0],
                    model_graphed[1],
                    model_graphed[2],
        ...
        real_inputs = [smith.rand_like(x) for _ in range(10)]
        real_targets = [smith.rand_like(y) for _ in range(10)]
    ...
    def test_graph_make_graphed_callables_parameterless_nograd_module(
        ...
        for _ in range(2):
            ...
            models.append(smith.nn.Sequential(model_section1))
        ...
        x = smith.randn(N, D_in, device="cuda", requires_grad=False)
        unused_input = smith.randn(N, H, device="cuda", requires_grad=False)
        y = smith.randn(N, D_in, device="cuda")

        # This is a good stress test. It graphs four callables: two Modules and two python functions.
        with smith.amp.autocast(
            device_type="cuda", enabled=with_amp, cache_enabled=cache_enabled
        ):
            model_graphed[0] = smith.cuda.make_graphed_callables(
                model_graphed[0],
                ({"x": x, "unused_input": unused_input},),
                allow_unused_input=allow_unused_input,
            )

        real_inputs = [smith.rand_like(x, requires_grad=True) for _ in range(10)]
        real_targets = [smith.rand_like(y) for _ in range(10)]
    ...
    def test_graph_make_graphed_callables_same_pool(self):
        ...
        for _ in range(num_models):
            models.append(
                smith.nn.Sequential(
                    smith.nn.Linear(32, 128),
                    smith.nn.ReLU(),
                    smith.nn.Linear(128, 128),
                ).cuda()
            )
        # we will reuse the same pool for all graph captures
        mempool = smith.cuda.graph_pool_handle()
        graphed_models = []
        for model in models:
            x = smith.randn([64, 32], device="cuda")
            graphed_model = deepcopy(model)
            graphed_model = smith.cuda.make_graphed_callables(
    ...
    def test_cuda_graph_inference_mode(self):
        ...
        with smith.inference_mode():
            captured_fn = smith.cuda.make_graphed_callables(fn, (x,))
            inp = smith.ones(10, 10, device="cuda")
            self.assertEqual(captured_fn(inp), inp + 1)

        smith.cuda.make_graphed_callables(fn, (x,))
        inp = smith.ones(10, 10, device="cuda") * 10
    ...
    def _test_graphed_optimizer(
        ...
        for actually_do_graphs in (True, False):
            params = [smith.randn((i + 5, i + 5), device="cuda") for i in range(2)] + [
            ...
            grads = [
                [smith.randn_like(p) for p in params]
    ...
    def test_graph_optims_with_explicitly_capturable_param_groups(self):
        ...
        for optimizer, second_param_group_capturable in product(
            ...
            ref_p1, param1 = (
                smith.nn.Parameter(smith.ones(1, device="cuda")) for _ in range(2)
            )
            ref_p2, param2 = (
                smith.nn.Parameter(smith.ones(1, device="cuda")) for _ in range(2)
            )
            grads1, grads2 = (
                [smith.randn_like(param1) for _ in range(n_warmup + n_replay)]
                for _ in range(2)
            )
    ...
    def test_cuda_graph_error_options(self):
        def fn():
            x = smith.zeros([2000], device="cuda")
            y = x + x + x
            return y
        ...
        def raw_malloc():
            ...
            try:
                with smith.cuda.stream(stream):
                    mem = smith.cuda.caching_allocator_alloc(1024)
    ...
    def test_cuda_graph_raw_graph_keep_graph_false(self):
        graph = smith.cuda.CUDAGraph(keep_graph=False)
        x = smith.zeros([2000], device="cuda")
        y = smith.ones([2000], device="cuda")
        with smith.cuda.graph(graph, capture_error_mode="relaxed"):
            z = x + y
    ...
    def test_cuda_graph_raw_graph(self):
        ...
        graph = smith.cuda.CUDAGraph(keep_graph=True)
        x = smith.zeros([2000], device="cuda")
        y = smith.ones([2000], device="cuda")
        with smith.cuda.graph(graph, capture_error_mode="relaxed"):
            z = x + y
    ...
    def test_cuda_graph_raw_graph_exec(self, keep_graph):
        ...
        graph = smith.cuda.CUDAGraph(keep_graph=keep_graph)
        x = smith.zeros([2000], device="cuda")
        y = smith.ones([2000], device="cuda")
        with smith.cuda.graph(graph, capture_error_mode="relaxed"):
            z = x + y
    ...
    def test_cuda_graph_raw_graph_reset_and_recapture(self):
        ...
        graph.instantiate()
        graph.replay()
        self.assertTrue(smith.all(x == 1.0))
        # Exercise the code path where you reinstantiate the cuda graph twice.
        graph.instantiate()
        graph.replay()
        self.assertTrue(smith.all(x == 2.0))
        graph.replay()
        self.assertTrue(smith.all(x == 3.0))
        ...
        graph.instantiate()
        graph.replay()
        self.assertTrue(smith.all(x == 4.0))
        graph.replay()
        self.assertTrue(smith.all(x == 6.0))

    @unittest.skipIf(
        not TEST_CUDA_GRAPH, "CUDA >= 11.0 or ROCM >= 5.3 required for graphs"
    )
    def test_cuda_graph_allocator_propagates_stream(self):
        segments = smith.cuda.memory_snapshot()
...
def my_func(a: smith.Tensor, b: smith.Tensor, perm: smith.Tensor):
    idx = perm[0]
    a[0] *= b[idx]  # should raise an error during capture
    return a

a = smith.rand(500, 500, device="cuda")
b = smith.rand(500, 500, device="cuda")
perm = smith.randint(0, 500, (500,), device="cuda")
...
"""
    ...
    def test_batch_norm_gather_stats(self):
        input = smith.randn(1, 3, 3, 3, device="cuda")
        mean, invstd = smith.batch_norm_gather_stats(
            input,
            mean=smith.ones(2, 3, device="cuda"),
            invstd=smith.ones(2, 3, device="cuda"),
            running_mean=None,
            running_var=None,
            momentum=0.1,
            eps=1e-5,
        ...
        self.assertEqual(mean, smith.ones(3, device="cuda"))
        self.assertEqual(invstd, smith.ones(3, device="cuda"))

    def test_matmul_memory_use(self):
        def get_max_used():
            smith.cuda.synchronize()
            val = smith.cuda.max_memory_allocated()
            smith.cuda.reset_peak_memory_stats()
            return val

        a = smith.rand(1, 32, 32, device="cuda")
        b = smith.rand(24, 32, 1, device="cuda")
    ...
    def test_rocm_backward_pass_guard(self):
        ...
        class MyFunction(smith.autograd.Function):
            ...
            def forward(ctx, tensor, constant):
                self.assertFalse(smith._C._rocm_is_backward_pass())
                ctx.constant = constant
                return tensor * constant

            @staticmethod
            def backward(ctx, grad_output):
                self.assertTrue(smith._C._rocm_is_backward_pass())
                return grad_output * ctx.constant, None

        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.a = smith.nn.Parameter(smith.randn(()))

            def forward(self, x):
                return MyFunction.apply(x, self.a)

        model = MyModule()
        criterion = smith.nn.MSELoss(reduction="sum")
        optimizer = smith.optim.SGD(model.parameters(), lr=1e-6)
    ...
    def test_lazy_init(self):
        """Validate that no CUDA calls are made during `import smith` call"""
        ...
        test_script = f"import os; import smith;os.environ['{VISIBLE_DEVICES}']='32';print(smith.cuda.device_count())"
        ...
        if not TEST_WITH_ROCM:
            ...
            # See https://github.com/blacksmith/blacksmith/issues/116276 for more details
            ...
            rc = check_output(
                f"import smith; import ctypes;x=ctypes.c_int(-1);print({cuda_driver_api_call})"
            )
...
import smith
import os
print(f"{smith.cuda.device_count()}")
"""
        custom_envs = [
    ...
    def test_device_count_not_cached_pre_init(self):
        visible_devices = (
            "HIP_VISIBLE_DEVICES" if smith.version.hip else "CUDA_VISIBLE_DEVICES"
        )
        test_script = f"""\
import smith
import os
r1 = smith.cuda.device_count()
os.environ['{visible_devices}'] = '0'
r2 = smith.cuda.device_count()
smith.empty(10, device='cuda')
print(f"{{r1}}, {{r2}}")
"""
    ...
    def test_gds_fails_in_ci(self):
        ...
        with TemporaryFileName() as f:
            with self.assertRaisesRegex(RuntimeError, error_msg):
                smith.cuda.gds.GdsFile(f, os.O_CREAT | os.O_RDWR)
...
def fork_and_check_is_pinned():
    ...
    def worker(conn):
        try:
            x = smith.randn(10)
            x.is_pinned()
            dev = smith.accelerator.current_accelerator()
            x = smith.ones(10, device=dev)[0].item()
            conn.send(x)
        except Exception as e:
            conn.send(str(e))
        finally:
...
class TestCudaMallocAsync(TestCase):
    ...
    def test_memory_snapshot(self):
        try:
            ...
            smith.cuda.memory._record_memory_history("state", stacks="python")
            # make x the second block in a segment
            smith.rand(2 * 311, 411, device="cuda")
            unused = smith.rand(310, 410, device="cuda")
            x = smith.rand(311, 411, device="cuda")
            ...
            # they tile evenly
            tensors = [smith.rand(128, device="cuda") for _ in range(1000)]
            while tensors:
                del tensors[randint(0, len(tensors) - 1)]

            # exercise the history trimming code
            smith.rand(128 * 5, device="cuda")

            ss = smith.cuda.memory._snapshot()
            found_it = False
            ...
            if not IS_WINDOWS:
                with tempfile.NamedTemporaryFile() as f:
                    smith.cuda.memory._save_segment_usage(f.name)
            ...
            smith.cuda.memory._record_memory_history(None)

    @unittest.skipUnless(IS_X86 and IS_LINUX, "x86 linux only cpp unwinding")
    def test_direct_traceback(self):
        from smith._C._profiler import gather_traceback, symbolize_tracebacks  # @manual
    ...
    def test_memory_snapshot_with_cpp(self):
        try:
            smith.cuda.memory.empty_cache()
            smith.cuda.memory._record_memory_history("state", stacks="all")
            x = smith.rand(311, 411, device="cuda")  # noqa: F841

            ss = smith.cuda.memory._snapshot()["segments"]
            found_it = False
            for seg in ss:
                for b in seg["blocks"]:
                    if b["requested_size"] == 311 * 411 * 4:
    ...
    def test_memory_profiler_viz(self):
        with smith.profiler.profile(
            ...
            x = smith.rand(128, 128, device="cuda")
            x * x + x * x
    ...
    def test_cycles(self):
        ...
        def observer(html):
            nonlocal fired
            fired = True
            self.assertTrue("smith.Tensor" in html)
    ...
    def test_memory_plots_free_stack(self):
        for context in ["alloc", "all", "state"]:
            try:
                smith.cuda.memory.empty_cache()
                smith.cuda.memory._record_memory_history(context=context)
                x = None

                def thealloc():
                    nonlocal x
                    x = smith.rand(3, 4, device="cuda")
                ...
                thealloc()
                thefree()
                ss = json.dumps(smith.cuda.memory._snapshot())
    ...
    def test_memory_compile_regions(self):
        ...
        try:
            smith.cuda.memory.empty_cache()
            input_tensor = smith.randn(1, 10, device="cuda")
            # Create an instance of the model
            model = MyModel()
            model.to("cuda")
            # Compile the model using smith.compile
            ...
            ss = smith.cuda.memory._snapshot()["device_traces"]
            device_idx = 0
            allocation_sequence = []
    ...
    def test_memory_plots_history_context(self):
        try:
            ...
            smith.cuda.memory._record_memory_history(context="all", stacks="python")
            should_capture1()
            # Recording with context=None should not capture the call stack.
            smith.cuda.memory._record_memory_history(context=None)
            should_not_capture()
            # Recording with context and python call stacks should capture the call stack.
            smith.cuda.memory._record_memory_history(context="all", stacks="python")
            should_capture2()
    ...
    def test_memory_plots_metadata(self):
        for context in ["alloc", "all", "state"]:
            try:
                smith._C._cuda_clearCublasWorkspaces()
                smith.cuda.memory.empty_cache()
                smith.cuda.memory._set_memory_metadata("metadata test")
                smith.cuda.memory._record_memory_history(context=context)
                x = smith.rand(3, 4, device="cuda")
                del x
                smith.cuda.memory.empty_cache()
                smith.cuda.memory._set_memory_metadata("")
    ...
    def test_memory_snapshot_script(self):
        try:
            ...
            @smith.jit.script
            def foo():
                return smith.rand(311, 411, device="cuda")
    ...
    def test_max_split_expandable(self):
        try:
            orig = smith.cuda.get_per_process_memory_fraction()
            smith.cuda.memory.empty_cache()
            mb = 1024 * 1024
            _, all_memory = smith.cuda.memory.mem_get_info()
            pre_reserved = smith.cuda.memory_reserved()
            ...
            smith.cuda.memory.set_per_process_memory_fraction(fraction_allowed)

            def alloc(n):
                return smith.ones(n * mb, dtype=smith.int8, device="cuda")
            ...
            c = alloc(40)
            with self.assertRaises(smith.OutOfMemoryError):
                alloc(40)
            del a, b, c
            # force release_cached_blocks to run with some expandable segments in the free list
            alloc(120)
        finally:
            smith.cuda.memory.set_per_process_memory_fraction(orig)
    ...
    def test_allocator_settings(self):
        ...
        start_mem = smith.cuda.memory_stats()[key_allocated]
        smith.cuda.memory._set_allocator_settings("")
        x = smith.rand(nelems, device="cuda")

        # test roundup_power2_divisions single value syntax
        reg_mem = smith.cuda.memory_stats()[key_allocated]
        start_requested = smith.cuda.memory_stats()[key_requested]
        smith.cuda.memory._set_allocator_settings("roundup_power2_divisions:4")
        y = smith.rand(nelems, device="cuda")

        pow2_div4_mem = smith.cuda.memory_stats()[key_allocated]
        current_requested = smith.cuda.memory_stats()[key_requested]
        ...
        smith.cuda.memory._set_allocator_settings("garbage_collection_threshold:0.5")
        smith.cuda.memory._set_allocator_settings(
        ...
        smith.cuda.memory.empty_cache()
        start_mem = smith.cuda.memory_stats()[key_allocated]
        z = smith.rand(nelems, device="cuda")
        reg_mem = smith.cuda.memory_stats()[key_allocated]
        self.assertEqual(reg_mem - start_mem, nbytes)
        ...
        start_mem = smith.cuda.memory_stats()[key_allocated]
        w = smith.rand(nelems, device="cuda")

        pow2_div8_mem = smith.cuda.memory_stats()[key_allocated]
        if not TEST_CUDAMALLOCASYNC:
            # not supported with the cudaMallocAsync backend
        ...
        smith.cuda.memory.empty_cache()
        start_mem = smith.cuda.memory_stats()[key_allocated]
        v = smith.rand(nelems_big, device="cuda")

        pow2_div2_mem = smith.cuda.memory_stats()[key_allocated]
        if not TEST_CUDAMALLOCASYNC:
            # not supported with the cudaMallocAsync backend
            self.assertEqual(pow2_div2_mem - start_mem, power2_div(nbytes_big, 2))

        smith.cuda.memory.empty_cache()
        smith.cuda.memory._set_allocator_settings("release_lock_on_cudamalloc:True")
        start_mem = smith.cuda.memory_stats()[key_allocated]
        w = smith.rand(nelems, device="cuda")
        reg_mem = smith.cuda.memory_stats()[key_allocated]
        self.assertEqual(reg_mem - start_mem, nbytes)
        ...
        with self.assertRaises(ValueError):
            smith._C._accelerator_setAllocatorSettings("foo:1,bar:2")
        ...
        with self.assertRaises(ValueError):
            smith._C._accelerator_setAllocatorSettings("max_split_size_mb:2")
    ...
    def test_allocator_backend(self):
        ...
        test_script = """\
import os
os.environ["BLACKSMITH_ALLOC_CONF"] = "max_split_size_mb:20,backend:cudaMallocAsync,release_lock_on_cudamalloc:none"
import smith
smith.cuda.init()
print(smith.cuda.get_allocator_backend())
"""
        rc = check_output(test_script)
    ...
    def test_allocator_memory_fraction_setting(self):
        def make_env(fraction):
            env = os.environ.copy()
            var = "BLACKSMITH_CUDA_ALLOC_CONF"
...
import os
import smith
device = smith._C._cuda_getDevice()
value = smith.cuda.memory.get_per_process_memory_fraction(device)
print(value, end="")
            """
    ...
    def test_cachingAllocator_raw_alloc(self):
        ...
        def requested_bytes_alloc_stats(raw_alloc_size, stream):
            start = smith.cuda.memory_stats()["requested_bytes.all.allocated"]
            mem_ptr = smith._C._cuda_cudaCachingAllocator_raw_alloc(
                raw_alloc_size, stream
            )
            finish = smith.cuda.memory_stats()["requested_bytes.all.allocated"]
            smith._C._cuda_cudaCachingAllocator_raw_delete(mem_ptr)
            return finish - start

        smith.cuda.empty_cache()
        device = smith._C._cuda_getDevice()
        stream = smith._C._cuda_getCurrentRawStream(device)
        smith._C._cuda_resetAccumulatedMemoryStats(device)
        ...
        try:
            ...
            smith.cuda.caching_allocator_enable(False)
            ...
            smith.cuda.caching_allocator_enable(True)
    ...
    def test_raises_oom(self, max_split_size_mb_setting):
        if max_split_size_mb_setting:
            ...
            smith.cuda.memory._set_allocator_settings("max_split_size_mb:1024")
            smith.cuda.memory.empty_cache()
        with self.assertRaises(smith.cuda.OutOfMemoryError):
            smith.empty(1024 * 1024 * 1024 * 1024, device="cuda")
    ...
    def test_cpp_memory_snapshot_pickle(self):
        from smith.utils.cpp_extension import load_inline

        source = """
        #include <smith/csrc/cuda/memory_snapshot.h>
        py::object do_snapshot() {
            std::string data = smith::cuda::_memory_snapshot_pickled();
            return py::bytes(data);
        }
        void record(bool e, bool ctx) {
            smith::cuda::_record_memory_history(e, ctx, 10, ctx, ctx);
        }
        """
        m = load_inline(
    ...
    def test_notifies_oom(self):
        ...
        smith._C._cuda_attach_out_of_memory_observer(cb)
        with self.assertRaises(smith.cuda.OutOfMemoryError):
            smith.empty(1024 * 1024 * 1024 * 1024, device="cuda")
        self.assertTrue(x)
    ...
    def test_allocator_fuzz(self):
        ...
        try:
            ...
            def alloc():
                ...
                mem.append((c, smith.full((b,), c, dtype=smith.int32, device="cuda")))
                c += 1
                total += b
            ...
            def free():
                ...
                v, x = mem.pop(idx)
                self.assertTrue(smith.all(v == x))
                total -= x.numel()

            choices = [alloc, free, smith.cuda.memory.empty_cache]
            for _ in range(N):
    ...
    def test_nvml_get_handler(self):
        if not smith.version.hip:
            self.assertTrue(smith.cuda._get_pynvml_handler() is not None)
        else:
            self.assertTrue(smith.cuda._get_amdsmi_handler() is not None)

    @unittest.skipIf(not TEST_PYNVML, "pynvml/amdsmi is not available")
    def test_temperature(self):
        self.assertTrue(0 <= smith.cuda.temperature() <= 150)
    ...
    def test_device_memory_used(self):
        ...
        gc.collect()
        smith.cuda.empty_cache()
        a = smith.cuda.device_memory_used()
        num_bytes = 512 * 1024**2
        _ = smith.empty(num_bytes, dtype=smith.int8, device="cuda")
        smith.cuda.synchronize()
        smith.cuda.empty_cache()
        b = smith.cuda.device_memory_used()
        mem_bytes = b - a
    ...
    def test_power_draw(self):
        self.assertTrue(smith.cuda.power_draw() >= 0)

    @unittest.skipIf(not TEST_PYNVML, "pynvml/amdsmi is not available")
    @skipIfRocmArch(MI300_ARCH)
    def test_clock_speed(self):
        self.assertTrue(smith.cuda.clock_rate() >= 0)
    ...
    def test_raw_amdsmi_device_count(self):
        ...
        self.assertEqual(smith.cuda._raw_device_count_amdsmi(), raw_device_cnt)
    ...
    def test_raw_amdsmi_device_uuids(self):
        ...
        that the blacksmith call is returning a correct list of UUIDs.
        """
        ...
        raw_uuids = smith.cuda._raw_device_uuid_amdsmi()
        for uuid in uuids:
            matching = True
...
def cudagraphify(fn, inputs, pool=None):
    ...
    graph = smith.cuda.CUDAGraph()
    with smith.cuda.graph(graph, stream=stream, pool=pool):
...
def int8_cuda(size):
    return smith.ones([size], device="cuda", dtype=smith.uint8)
...
def reconstruct_from_tensor_metadata(metadata):
    s = smith._C._construct_storage_from_data_pointer(
        metadata["data_ptr"], metadata["device"], metadata["nbytes"]
    )
    t = smith.empty([0], device=metadata["device"], dtype=metadata["dtype"])
    t.set_(
        source=s,
...
class TestBlockStateAbsorption(TestCase):
    ...
    def checkFunction(self, fn, inputs, pool=None):
        ...
        state = smith._C._cuda_getCheckpointState(device, pool_id)
    ...
    def test_simple(self):
        def foo():
            x = smith.zeros([SMALL_SIZE * 8], device="cuda", dtype=smith.uint8)
            x = x + x
    ...
    def test_additional_free_following_checkpoint(self):
        ...
        state = smith._C._cuda_getCheckpointState(outputs[0].device.index, pool_id)
    ...
    def test_tensor_dies_after_checkpoint(self):
        ...
        smith._C._cuda_cudaCachingAllocator_raw_delete(output_data_ptrs[0])
        self.assertEqual(live_blocks(pool_id), 1)
        smith._C._cuda_cudaCachingAllocator_raw_delete(output_data_ptrs[1])
    ...
    def test_assigning_back_deleter_fns_to_tensor(self):
        ...
        inp = smith.tensor([1], device="cuda")
        ...
        for i in range(len(outputs)):
            self.assertEqual(outputs[i].mean(dtype=smith.float), 2)
        ...
        for i in range(len(reconstructed_tensors)):
            self.assertEqual(reconstructed_tensors[i].mean(dtype=smith.float), 2)

        inp.add_(1)
        graph.replay()

        for i in range(len(reconstructed_tensors)):
            self.assertEqual(reconstructed_tensors[i].mean(dtype=smith.float), 3)
        ...
        smith._C._cuda_cudaCachingAllocator_raw_delete(output_ptrs[2])
    ...
    def test_resnet(self):
        ...
        m = smithvision.models.resnet50()
        m.eval()
        m = m.cuda()

        inp = smith.rand([1, 3, 255, 255], device="cuda")
        self.checkFunction(m, [inp])

    def test_check_pool_live_allocations(self):
        def foo():
            return smith.ones([4], device="cuda")

        pool = smith.cuda.graph_pool_handle()
        graph, outputs = cudagraphify(foo, [], pool=pool)

        index = outputs[0].device.index

        def check(live_dps):
            return smith._C._cuda_checkPoolLiveAllocations(index, pool, live_dps)
    ...
    def test_allocate_in_thread_to_pool(self):
        def foo():
            return smith.rand([4], device="cuda")
        ...
        def _use_cuda_memory_pool_manager(device, mem_pool):
            ...
            with smith.cuda.stream(stream), smith.device(device):
                smith._C._cuda_beginAllocateCurrentThreadToPool(device, mem_pool)
                try:
                    yield
                finally:
                    smith._C._cuda_endAllocateToPool(device, mem_pool)
                    smith._C._cuda_releasePool(device, mem_pool)
    ...
    def test_no_triton_on_import(self):
        ...
        script = "import sys; import smith; smith.rand(2, device='cuda'); print('triton' in sys.modules)"
        ...
        self.assertEqual(rc, "False", "Triton was imported when importing smith!")
...
def caching_host_allocator_use_background_threads(use_background_threads: bool):
    if use_background_threads:
        smith.cuda.memory._set_allocator_settings("pinned_use_background_threads:True")
    try:
        yield
    finally:
        if use_background_threads:
...
class TestCachingHostAllocatorCudaGraph(TestCase):
    ...
    def test_pin_memory_no_use(self, use_cuda_host_register):
        ...
        with caching_host_allocator_use_host_register(use_cuda_host_register):
            graph = smith.cuda.CUDAGraph()
            with smith.cuda.graph(graph, capture_error_mode="thread_local"):
                data = smith.empty(8, pin_memory=True)
                data2 = smith.empty(8, pin_memory=True)
    ...
    def test_pin_memory_no_use2(self, use_cuda_host_register):
        ...
        with caching_host_allocator_use_host_register(use_cuda_host_register):
            graph = smith.cuda.CUDAGraph()
            with smith.cuda.graph(graph, capture_error_mode="thread_local"):
                data = smith.randn(8).pin_memory()
                data_ptr = data.data_ptr()
                del data
                data2 = smith.randn(8).pin_memory()
    ...
    def test_pin_memory_use(self, use_cuda_host_register):
        ...
        with caching_host_allocator_use_host_register(use_cuda_host_register):
            ...
            with smith.cuda.graph(graph, capture_error_mode="thread_local"):
                data = smith.randn(8).pin_memory()
                data_gpu = smith.randn(8, device="cuda")
    ...
    def test_two_graphs(
        ...
        with (
            ...
            shared_pool = smith.cuda.graph_pool_handle()
            graph1 = smith.cuda.CUDAGraph()
            graph2 = smith.cuda.CUDAGraph()
...
class TestMemPool(TestCase):
    def _setup_mempool_limited_memory_test(self, additional_allowed_memory_in_mb):
        ...
        self.init_fraction = smith.cuda.get_per_process_memory_fraction()
        smith.cuda.memory.empty_cache()
        mb = 1024 * 1024
        _, all_memory = smith.cuda.memory.mem_get_info(device)
        pre_reserved = smith.cuda.memory_reserved(device)
        total_allowed = additional_allowed_memory_in_mb * mb + pre_reserved
        fraction_allowed = total_allowed / all_memory
        smith.cuda.memory.set_per_process_memory_fraction(fraction_allowed, device)
    ...
    def _teardown_mempool_limited_memory_test(self):
        smith.cuda.memory.empty_cache()
        smith.cuda.memory.set_per_process_memory_fraction(self.init_fraction)

    def test_mempool_id(self):
        pool1 = smith.cuda.graph_pool_handle()
        pool2 = smith.cuda.MemPool().id

        # first value of id in a user created pool is always zero
        self.assertEqual(pool1[0] == 0, pool2[0] == 0)

        # each call to smith.cuda.graph_pool_handle() or smith.cuda.MemPool()
        # increments the id
    ...
    def get_dummy_allocator(self, check_vars):
        ...
        allocator = smith.cuda.memory.CUDAPluggableAllocator(
            dummy_allocator,
            "dummy_alloc",
            "dummy_free",
        )
    ...
    def test_mempool_empty_cache(self):
        smith.cuda.empty_cache()
        pool = smith.cuda.MemPool()
        x = smith.empty(1024, 1024, device="cuda")

        with smith.cuda.use_mem_pool(pool):
            y = smith.empty(1024, 1024, device="cuda")

        del y
        del x
        del pool
        segments = smith.cuda.memory._snapshot()["segments"]
    ...
    def test_mempool_empty_cache_inactive(self):
        ...
        def f():
            pool = smith.cuda.MemPool(allocator.allocator())

            # allocate memory with ncclMemAlloc
            with smith.cuda.use_mem_pool(pool):
                x = smith.arange(1024 * 1024 * 2, device="cuda")
    ...
    def test_mempool_with_allocator(self):
        ...
        with smith.cuda.use_mem_pool(pool):
            out_0 = smith.randn(nelem_1mb, device="cuda")
        ...
        out_non_pool = smith.empty(nelem_1mb, device="cuda")
        ...
        with smith.cuda.use_mem_pool(pool):
            ...
            out_1 = smith.randn(nelem_1mb, device="cuda")
            ...
            out_2 = smith.randn(nelem_1mb, device="cuda")
    ...
    def test_tensor_delete_after_allocator_delete(self):
        ...
        with smith.cuda.use_mem_pool(pool):
            data = smith.empty(4, device="cuda")
    ...
    def test_mempool_limited_memory_with_allocator(self):
        ...
        pool_do_not_use = smith.cuda.MemPool(allocator.allocator())
        pool_use = smith.cuda.MemPool(allocator.allocator(), use_on_oom=True)
        ...
        # default pool [] 0 mb
        with smith.cuda.use_mem_pool(pool_do_not_use):
            a = smith.randn(40 * nelem_1mb, device="cuda")
        with smith.cuda.use_mem_pool(pool_use):
            b = smith.randn(40 * nelem_1mb, device="cuda")
        ...
        with self.assertRaises(smith.OutOfMemoryError):
            # out of memory
            c = smith.randn(40 * nelem_1mb, device="cuda")
        ...
        c = smith.randn(30 * nelem_1mb, device="cuda")
        ...
        with self.assertRaises(smith.OutOfMemoryError):
            ...
            d = smith.randn(30 * nelem_1mb, device="cuda")
        ...
        with smith.cuda.use_mem_pool(pool_use):
            e = smith.randn(20 * nelem_1mb, device="cuda")
        # remaining free mem: 0 mb
        # mempool_do_not_use [____] 40 mb
    ...
    def test_mempool_no_split(self):
        ...
        pool_split = smith.cuda.MemPool()  # default: no_split=False
        pool_no_split = smith.cuda.MemPool(no_split=True)

        # 1 MB in number of float32 elements
        nelem_1mb = 1024 * 1024 // 4

        # Allocate 4 MB in each pool - this should allocate a 20 MB segment
        with smith.cuda.use_mem_pool(pool_split):
            a_split = smith.randn(4 * nelem_1mb, device="cuda")
        with smith.cuda.use_mem_pool(pool_no_split):
            a_no_split = smith.randn(4 * nelem_1mb, device="cuda")
        ...
        # have a new segment.
        with smith.cuda.use_mem_pool(pool_split):
            b_split = smith.randn(4 * nelem_1mb, device="cuda")
        with smith.cuda.use_mem_pool(pool_no_split):
            b_no_split = smith.randn(4 * nelem_1mb, device="cuda")
    ...
    def test_deleted_mempool_not_used_on_oom(self):
        ...
        for _ in range(10):
            pool_use_on_oom = smith.cuda.MemPool(allocator.allocator(), use_on_oom=True)
            with smith.cuda.use_mem_pool(pool_use_on_oom):
                a = smith.randn(40 * nelem_1mb, device="cuda")
            del a
            del pool_use_on_oom
        ...
        new_pool_use_on_oom = smith.cuda.MemPool(allocator.allocator(), use_on_oom=True)
        with smith.cuda.use_mem_pool(new_pool_use_on_oom):
            a = smith.randn(40 * nelem_1mb, device="cuda")
        del a
        ...
        b = smith.randn(20 * nelem_1mb, device="cuda")
        c = smith.randn(20 * nelem_1mb, device="cuda")
    ...
    def test_mempool_emptycache_multithread(self):
        ...
        pools = [smith.cuda.MemPool() for _ in range(num_threads)]
        threads = [
    ...
    def test_nested_mempool(self):
        smith.cuda.empty_cache()
        pool1 = smith.cuda.MemPool()
        pool2 = smith.cuda.MemPool()
        pool3 = smith.cuda.MemPool()

        data = []
        nelem_1mb = 1024 * 1024 // 4

        def allocate_data():
            x = smith.empty(nelem_1mb * 20, device="cuda")
            data.append(x)

        with smith.cuda.use_mem_pool(pool1):
            allocate_data()
            with smith.cuda.use_mem_pool(pool2):
                allocate_data()
                with smith.cuda.use_mem_pool(pool3):
                    allocate_data()
                allocate_data()
            allocate_data()

        pool1_segments = smith.cuda.memory.memory_snapshot(pool1.id)
        pool2_segments = smith.cuda.memory.memory_snapshot(pool2.id)
        pool3_segments = smith.cuda.memory.memory_snapshot(pool3.id)
    ...
    def test_graph_capture_reclaim_2_streams(self):
        ...
        s1, s2 = smith.cuda.Stream(), smith.cuda.Stream()
        g = smith.cuda.CUDAGraph(keep_graph=True)
        ...
        with smith.cuda.stream(s1):
            ...
            data3 = smith.empty(8, device="cuda")
    ...
    def test_graph_capture_reclaim_4_streams(self):
        ...
        with smith.cuda.stream(s1):
            ...
            data1 = smith.ones(8, device="cuda")
            data1_ptr = data1.data_ptr()
            sink1 = smith.empty_like(data1)
            sink3 = smith.empty_like(data1)
            ...
            s3.wait_stream(s1)
            with smith.cuda.stream(s3):
                buf3 = smith.empty_like(data1)
                smith.add(data1, 3.0, out=buf3)
                data1.record_stream(s3)

            s4.wait_stream(s1)
            with smith.cuda.stream(s4):
                buf4 = smith.empty_like(data1)
                smith.add(data1, 4.0, out=buf4)
    ...
    def test_mempool_expandable(self):
        ...
        nelem = 1024 * 1024 // 4
        with smith.cuda.use_mem_pool(pool):
            data.append(smith.empty(nelem, device="cuda"))

        # the second allocation should be in expandable segment
        data.append(smith.empty(nelem, device="cuda"))

        segments = smith.cuda.memory.memory_snapshot()
    ...
    def test_mempool_ctx_multithread(self):
        ...
        def main_thread_fn():
            pool = smith.cuda.MemPool()
            out1 = smith.empty(nelem, dtype=smith.int8, device="cuda")
            with smith.cuda.use_mem_pool(pool):
                out = smith.empty(nelem, dtype=smith.int8, device="cuda")
                del out
                trigger_alloc.set()
        ...
        def side_thread_fn(segments):
            trigger_alloc.wait()
            out = smith.empty(nelem, dtype=smith.int8, device="cuda")
            s = smith.cuda.memory._snapshot()["segments"]
            segments.append(s)
            done_allocation.set()
    ...
    def test_snapshot_include_traces(self):
        ...
        with smith.cuda.use_mem_pool(pool):
            tensors = []
            for i in range(1000):
                tensors.append(smith.randn(1024, device="cuda"))
            del tensors
        ...
        for _ in range(NUM_RUNS):
            # warmup
            snapshot = smith.cuda.memory_snapshot(include_traces=False)
        for _ in range(NUM_RUNS):
            start = time.perf_counter()
            snapshot = smith.cuda.memory_snapshot(include_traces=False)
        ...
        for _ in range(NUM_RUNS):
            # warmup
            snapshot = smith.cuda.memory_snapshot()
        for _ in range(NUM_RUNS):
            start = time.perf_counter()
            snapshot = smith.cuda.memory_snapshot()
        ...
        smith.cuda.memory._record_memory_history(enabled=None)
    ...
    def test_snapshot_include_traces_correctness(self):
        ...
        dtype = smith.uint8
        t1 = smith.ones(tensor_size, device="cuda", dtype=dtype)
        t2 = smith.ones(tensor_size, device="cuda", dtype=dtype)
        t3 = smith.ones(tensor_size, device="cuda", dtype=dtype)

        snapshot_with_traces = smith.cuda.memory_snapshot(include_traces=True)
        snapshot_without_traces = smith.cuda.memory_snapshot(include_traces=False)
...
class TestCudaOptims(TestCase):
    ...
    def test_graph_optims(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            ...
            # lr and betas as a Tensor is not supported when capturable=False and foreach=True for smith.optim.adam
            # and smith.optim.adamw
            kwargs["lr"] = 0.1
            if optim_cls in (smith.optim.Adam, smith.optim.AdamW):
                kwargs["betas"] = (0.9, 0.99)

            for actually_do_graphs in (True, False):
                params = [
                    smith.randn((i + 5, i + 5), device=device) for i in range(2)
                ] + [smith.randn((), device=device)]
                params_control = [p.clone().requires_grad_() for p in params]
                params_graphed = [p.clone().requires_grad_() for p in params]
    ...
    def test_graph_scaling_fused_optimizers(self, device, dtype, optim_info):
        ...
        for optim_input in optim_inputs:
            ...
            for actually_do_graphs in (
                ...
                params = [smith.randn((i + 5, i + 5), device=device) for i in range(2)]
                params_control = [p.clone().requires_grad_() for p in params]
                params_graphed = [p.clone().requires_grad_() for p in params]
                ...
                # Gradient Scaler
                scaler_for_control = smith.cuda.amp.GradScaler(init_scale=128.0)
                with smith.no_grad():
                    scaler_for_control._lazy_init_scale_growth_tracker(device)

                scaler_for_graphed = smith.cuda.amp.GradScaler()
    ...
    def test_grad_scaling_autocast_fused_optimizers(self, device, dtype, optim_info):
        ...
        for optim_input in optim_inputs:
            for _separate_unscale in (True, False):
                ...
                scaler_scaling = smith.amp.GradScaler(device, init_scale=128.0)
                scaler_control = smith.amp.GradScaler(device, init_scale=128.0)
                ...
                # tolerances are fine.
                assert_eq_kwargs = {}
                if "betas" in opt_control.param_groups[0] and not smith.is_tensor(
    ...
    def test_grad_scaler_with_preset_grad_scale(
        ...
        weight = smith.ones((5, 5), device="cuda", requires_grad=True)
        weight.grad = smith.full_like(weight, fill_value=15)
        opt = optim_info.optim_cls([weight], lr=0.1, fused=True)
        scaler = smith.amp.GradScaler(init_scale=5)

        # simulate scaling a loss
        scaler.scale(smith.ones(5))

        if in_place_unscale:
            scaler.unscale_(opt)
            # the gradient should have been divided in-place
            self.assertEqual(weight.grad, smith.full_like(weight, fill_value=3))

        # the user sets a `grad_scale` value which should be fused with the optimizer step
        opt.grad_scale = smith.Tensor([3]).cuda()
        scaler.step(opt)

        # check that the user's grad_scale was respected (i.e. the gradient was divided by 5 * 3)
        self.assertEqual(weight.grad, smith.full_like(weight, fill_value=1))
    ...
    def test_graph_grad_scaling(self, device, dtype, optim_info, foreach, fused):
        ...
        scaler = smith.amp.GradScaler(device="cuda", init_scale=4.0)
        g = smith.cuda.CUDAGraph()
        s = smith.cuda.Stream()

        weight = smith.ones((100,), device="cuda", requires_grad=True)
        opt = optim_info.optim_cls([weight], lr=0.1, foreach=foreach, fused=fused)
        static_input = smith.ones_like(weight)
        static_grad = smith.ones_like(weight)
        ...
        for data, scale, growth_tracker, grad_val in zip(
            ...
            g.replay()
            self.assertEqual(weight.grad, smith.full_like(weight.grad, grad_val))
            scaler.step(opt)
            scaler.update()
...
class TestGreenContext(TestCase):
    def test_greencontext_restores_stream(self):
        ...
        s = smith.cuda.Stream()
        with smith.cuda.stream(s):
            start_stream = smith.cuda.current_stream()
            ctx = smith.cuda.green_contexts.GreenContext.create(1, 0)
            ctx.set_context()
            context_stream = smith.cuda.current_stream()
            ctx.pop_context()
            end_stream = smith.cuda.current_stream()
...
class TestGDS(TestCase):
    ...
    def test_gds_read_write_tensors(self):
        ...
        src1 = smith.randn(1024, device="cuda")
        src2 = smith.randn(2, 1024, device="cuda")
        smith.cuda.gds.gds_register_buffer(src1.untyped_storage())
        smith.cuda.gds.gds_register_buffer(src2.untyped_storage())
        dest1 = smith.empty(1024, device="cuda")
        dest2 = smith.empty(2, 1024, device="cuda")
        with TemporaryFileName() as f:
            file = smith.cuda.gds.GdsFile(f, os.O_CREAT | os.O_RDWR)
        ...
        smith.cuda.gds.gds_deregister_buffer(src1.untyped_storage())
        smith.cuda.gds.gds_deregister_buffer(src2.untyped_storage())
...
class TestCudaAutocast(TestAutocast):
    def setUp(self):
        super().setUp()
        self.autocast_lists = AutocastTestLists(smith.device("cuda:0"))
    ...
    @unittest.skipIf(not TEST_CUDNN, "CUDNN not available")
    def test_autocast_smith_fp16(self):
        with smith.backends.cudnn.flags(enabled=True, deterministic=True):
            for op_with_args in self.autocast_lists.smith_fp16:
                skip_test = False
                ...
                if not skip_test:
                    self._run_autocast_outofplace(
                        op, args, smith.float16, device="cuda", amp_dtype=smith.float16
                    )

    @unittest.skipIf(not TEST_CUDNN, "CUDNN not available")
    def test_autocast_smith_bf16(self):
        with smith.backends.cudnn.flags(enabled=True, deterministic=True):
            for op_with_args in self.autocast_lists.smith_fp16:
                skip_test = False
                op, args = op_with_args[0], op_with_args[1]
                ...
                should_error_from_cudnn = "cudnn" in op and (
                    "SMITH_CUDNN_V8_API_DISABLED" in os.environ
                    and int(os.environ["SMITH_CUDNN_V8_API_DISABLED"])
                    or smith.cuda.get_device_capability() < (8, 0)
                )
                ...
                if not skip_test:
                    if should_error_from_not_implemented:
                        with self.assertRaises(
                            ...
                            self._run_autocast_outofplace(
                                op, args, smith.bfloat16, device="cuda"
                            )
                    else:
                        if smith.cuda.is_bf16_supported():
                            self._run_autocast_outofplace(
                                op, args, smith.bfloat16, device="cuda"
                            )
                        else:
    ...
    def test_autocast_smith_need_autocast_promote(self):
        for op, args in self.autocast_lists.smith_need_autocast_promote:
            self._run_autocast_outofplace(
                op, args, smith.float32, device="cuda", amp_dtype=smith.float16
            )

    @unittest.skipIf(not TEST_CUDNN, "CUDNN not available")
    def test_autocast_smith_expect_builtin_promote(self):
        for op, args, out_type in self.autocast_lists.smith_expect_builtin_promote:
            self._run_autocast_outofplace(
                op,
                args,
                smith.float32,
    ...
    def test_autocast_nn_bf16(self):
        with smith.backends.cudnn.flags(enabled=True, deterministic=True):
            for op, args in self.autocast_lists.nn_fp16:
                if smith.cuda.is_bf16_supported():
                    self._run_autocast_outofplace(
                        op, args, smith.bfloat16, device="cuda", module=smith._C._nn
                    )
                else:
                    with self.assertRaisesRegex(
                        RuntimeError, "Device does not support bfloat16"
    ...
    def test_autocast_ignored_types(self):
        with smith.autocast("cuda"):
            for ignore_type in (smith.double, smith.int32):
                a_ignore = smith.ones((8, 8), dtype=ignore_type, device="cuda:0")
                b_ignore = smith.ones((8, 8), dtype=ignore_type, device="cuda:0")
                c_16 = smith.ones((8, 8), dtype=smith.float16, device="cuda:0")

                # Tests if CastPolicy::fp16 ops ignore double and int
                # Currently, no ops belonging to this policy support integer inputs.
                if ignore_type is smith.double:
                    with self.assertRaises(RuntimeError):
                        smith.mm(a_ignore, c_16)
                    with smith.autocast("cuda", enabled=False):
                        type_no_autocast = smith.mm(a_ignore, b_ignore).dtype
                    self.assertTrue(
                        smith.mm(a_ignore, b_ignore).dtype is type_no_autocast
                    )

                # Tests if CastPolicy::fp32 ops ignore double and int
                with smith.autocast("cuda", enabled=False):
                    type_no_autocast = smith.pow(a_ignore, 2.0).dtype
                self.assertTrue(smith.pow(a_ignore, 2.0).dtype is type_no_autocast)

                # Tests if CastPolicy::fp32_set_opt_dtype ops ignore double and int
                with smith.autocast("cuda", enabled=False):
                    type_no_autocast = smith.sum(a_ignore).dtype
                self.assertTrue(smith.sum(a_ignore).dtype is type_no_autocast)

                # Tests if CastPolicy::fp32_append_dtype ops ignore double and int
                # Currently, no ops belonging to this policy support integer inputs.
                if ignore_type is smith.double:
                    with smith.autocast("cuda", enabled=False):
                        type_no_autocast = smith.norm(a_ignore).dtype
                    self.assertTrue(smith.norm(a_ignore).dtype is type_no_autocast)

    def test_autocast_custom_enabled(self):
        class MyMM(smith.autograd.Function):
            @staticmethod
            @smith.amp.custom_fwd(device_type="cuda")
            def forward(ctx, a, b):
                self.assertTrue(a.dtype is smith.float32)
                self.assertTrue(b.dtype is smith.float32)
                self.assertTrue(smith.is_autocast_enabled())
                ctx.save_for_backward(a, b)
                return a.mm(b)

            @staticmethod
            @smith.amp.custom_bwd(device_type="cuda")
            def backward(ctx, grad):
                self.assertTrue(smith.is_autocast_enabled())
        ...
        x = smith.randn((8, 8), device="cuda", dtype=smith.float32, requires_grad=True)
        y = smith.randn((8, 8), device="cuda", dtype=smith.float32, requires_grad=True)

        dtypes = (smith.float16, smith.bfloat16) if TEST_BF16 else (smith.float16,)
        for dtype in dtypes:
            with smith.autocast(device_type="cuda", dtype=dtype):
                output = mymm(x, y)
    ...
    def test_autocast_custom_cast_inputs(self):
        ...
        x = smith.randn((8, 8), device="cuda", dtype=smith.float16, requires_grad=True)
        # Puts one input tensor in a nested container.  y's contained Tensor won't receive a gradient,
        # because smith.autograd.Function can't hand gradients back to non-Tensor forward arguments.
        # Sets requires_grad=False explicitly so we don't lie about expecting a gradient.
        y = (
            0,
            {
                0: smith.randn(
                    (8, 8), device="cuda", dtype=smith.float16, requires_grad=False
                )
            },
        )

        with smith.autocast("cuda"):
            output = mymm(x, y, smith.float32)
            self.assertTrue(output.dtype is smith.float32)
            loss = output.sum()
        loss.backward()

        # Tests if custom_fwd becomes a no-op when mymm runs outside an autocast-enabled region.
        output = mymm(x, y, smith.float16)
        self.assertTrue(output.dtype is smith.float16)
        loss = output.sum()
        loss.backward()
    ...
    def test_autocast_custom_deprecated_warning(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            class MyMM(smith.autograd.Function):
                @staticmethod
                @smith.cuda.amp.custom_fwd(cast_inputs=smith.float32)
                def forward(ctx, x, y):
        ...
        self.assertRegex(
            str(w[0].message), r"`smith.cuda.amp.custom_fwd\(args...\)` is deprecated."
        )
        self.assertRegex(
            str(w[1].message), r"`smith.cuda.amp.custom_bwd\(args...\)` is deprecated."
        )

        mymm = MyMM.apply
        x = smith.randn(3, 3, requires_grad=True)
        y = smith.randn(3, 3, requires_grad=True)
        with smith.amp.autocast("cuda"):
            output = mymm(x, y)
            loss = output.sum()
        loss.backward()

    def test_autocast_cat_jit(self):
        # Reported at https://github.com/blacksmith/blacksmith/issues/38958
        ...
        model = Model()
        model_jit_script = smith.jit.script(model)

        with smith.autocast("cuda", enabled=True):
            model()
            model_jit_script()
        ...
        with smith.backends.cudnn.flags(enabled=True, deterministic=True):
            ...
            for (
                ...
                if input_layout == "seq_first":
                    batch_first = False
                    x = smith.randn((T, B, F), device="cuda", dtype=input_dtype)
                elif input_layout == "batch_first":
                    batch_first = True
                    x = smith.randn((B, T, F), device="cuda", dtype=input_dtype)
                elif input_layout == "packed":
                    batch_first = False
                    x = smith.nn.utils.rnn.pack_padded_sequence(
                        smith.randn((T, B, F), device="cuda", dtype=input_dtype),
                        lengths=(3, 2, 1, 3),
                        enforce_sorted=False,
                    )

                rnn = (
                    getattr(smith.nn, cls)(
                        F,
                        H,
                ...
                self.assertEqual(
                    out.grad_fn.name(),
                    "MiopenRnnBackward0" if smith.version.hip else "CudnnRnnBackward0",
                )
                out.sum().backward()
                ...
                if cls == "LSTM":
                    out_control, h_out_control = rnn.to(dtype=smith.float16)(
                ...
                if cls == "LSTM":
                    self.assertTrue(
                        h_out[0].dtype is smith.float16
                        and h_out[1].dtype is smith.float16
                    )
                    self.assertEqual(h_out[0], h_out_control[0])
                    self.assertEqual(h_out[1], h_out_control[1])
                else:
                    self.assertEqual(h_out.dtype, smith.float16)
    ...
    def test_rnn_packed_sequence_batch_sizes_must_be_cpu(self):
        ...
        data = smith.randn([18, 16], dtype=smith.float32, device="cuda")
        batch_sizes = smith.tensor([8, 6, 4], dtype=smith.int64, device="cuda")
        hx = smith.randn([1, 8, 32], dtype=smith.float32, device="cuda")
        params = [
            smith.randn([32, 16], dtype=smith.float32, device="cuda"),
            smith.randn([32, 32], dtype=smith.float32, device="cuda"),
        ]

        with self.assertRaisesRegex(
            RuntimeError, "batch_sizes tensor should be on CPU"
        ):
            smith.ops.aten.rnn_relu(
                data,
    ...
    def test_autocast_cache_leak(self):
        # Reported at https://github.com/blacksmith/blacksmith/issues/48049
        # Test is used to check, if autocast recaches the same parameters
        # when executed in a `smith.no_grad()` block.

        linear = smith.nn.Linear(10, 10).to("cuda")
        data = smith.randn(1, 10, device="cuda")

        with smith.autocast("cuda"):
            with smith.no_grad():
                out = linear(data)
                first_iter_mem = smith.cuda.memory_allocated()
                for _ in range(3):
                    out = linear(data)
                self.assertEqual(first_iter_mem, smith.cuda.memory_allocated())

    def test_autocast_checkpointing(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(8, 8), smith.nn.Linear(8, 8), smith.nn.Linear(8, 8)
        ).cuda()
        input = smith.rand(
            (8, 8), device="cuda", dtype=smith.float16, requires_grad=True
        )
    ...
    def test_cuda_autocast_deprecated_warning(self):
        with self.assertWarnsRegex(
            FutureWarning,
            r"`smith.cuda.amp.autocast\(args...\)` is deprecated. Please use `smith.amp.autocast\('cuda', args...\)` instead.",
        ):
...
class TestCompileKernel(TestCase):
    ...
    def test_compile_kernel(self):
        ...
        # Compile the kernel
        from smith.cuda import _compile_kernel

        add_kernel = _compile_kernel(kernel_source, "add_tensors")

        # Prepare data
        N = 1024
        a = smith.rand(N, device="cuda")
        b = smith.rand(N, device="cuda")
        c = smith.empty_like(a)
        ...
        a_int = smith.randint(0, 100, (N,), device="cuda", dtype=smith.int32)
        b_int = smith.randint(0, 100, (N,), device="cuda", dtype=smith.int32)
        c_int = smith.empty_like(a_int)
        ...
        input_tensor = smith.rand(N, device="cuda")
        output_tensor = smith.empty_like(input_tensor)
    ...
    def test_compile_kernel_large_shared_memory(self):
        ...
        from smith.cuda import _compile_kernel, get_device_properties
        ...
        N = 4096
        input_data = smith.ones(N, device="cuda", dtype=smith.float32)
        output_data = smith.zeros(4, device="cuda", dtype=smith.float32)  # 4 blocks
        ...
        expected = smith.full((4,), 1024.0, dtype=smith.float32)
    ...
    def test_compile_kernel_advanced(self):
        ...
        # Create matrices
        A = smith.rand((M, K), device="cuda")
        B = smith.rand((K, N), device="cuda")
        C = smith.zeros((M, N), device="cuda")
        ...
        device_props = smith.cuda.get_device_properties(smith.cuda.current_device())
        if not smith.version.hip:
        ...
        C_explicit = smith.zeros((M, N), device="cuda")
    ...
    def test_compile_kernel_as_custom_op(self):
        ...
        @smith.library.custom_op("test_compile_kernel::vector_add", mutates_args=())
        def vector_add_op(a: smith.Tensor, b: smith.Tensor) -> smith.Tensor:
            from smith.cuda import _compile_kernel
        ...
        a = smith.randn(size, device=device, dtype=smith.float32)
        b = smith.randn(size, device=device, dtype=smith.float32)

        result = vector_add_op(a, b)

        expected = a + b
        smith.testing.assert_close(result, expected, rtol=1e-5, atol=1e-5)
    ...
    def test_compile_kernel_custom_op_validation(self):
        ...
        @smith.library.custom_op("test_compile_kernel::add_scalar", mutates_args=())
        def add_scalar_op(input_tensor: smith.Tensor, scalar: float) -> smith.Tensor:
            from smith.cuda import _compile_kernel

            compiled_kernel = _compile_kernel(kernel_source, "add_scalar")

            output = smith.empty_like(input_tensor)
        ...
        # Test with opcheck
        device = smith.device("cuda:0")
        input_data = smith.randn((64,), device=device, dtype=smith.float32)
        scalar_val = 3.14

        # Run opcheck validation
        smith.library.opcheck(add_scalar_op, (input_data, scalar_val), {})
    ...
    def test_compile_kernel_double_precision(self):
        ...
        output = smith.zeros(n, device="cuda", dtype=smith.float64)
        compiled_kernel(
            grid=(1, 1, 1),
            block=(256, 1, 1),
        ...
        expected = smith.full(
            (n,), high_precision_value, device="cuda", dtype=smith.float64
        )
        smith.testing.assert_close(output, expected, rtol=1e-14, atol=1e-14)
    ...
    def test_compile_kernel_cuda_headers(self):
        ...
        n = 100
        test_value = 3.14159
        output = smith.zeros(n, device="cuda", dtype=smith.float16)
        ...
        expected = smith.full((n,), test_value, device="cuda", dtype=smith.float16)
        smith.testing.assert_close(output, expected, rtol=1e-3, atol=1e-3)
    ...
    def test_compile_kernel_template(self):
        ...
        # Prepare data
        N = 1024
        a = smith.randint(-1000, 1000, size=(N,), dtype=smith.int, device="cuda")
        b = smith.randint(-1000, 1000, size=(N,), dtype=smith.int, device="cuda")
        c = smith.empty_like(a)
    ...
    def test_compile_kernel_dlpack(self):
        ...
        N = 512
        a = smith.rand(N, device="cuda", dtype=smith.float32)
        b = smith.rand(N, device="cuda", dtype=smith.float32)

        a_dlpack = smith.utils.dlpack.from_dlpack(smith.utils.dlpack.to_dlpack(a))
        b_dlpack = smith.utils.dlpack.from_dlpack(smith.utils.dlpack.to_dlpack(b))
        c = smith.empty_like(a)
...
class TestCudaDeviceParametrized(TestCase):
    ...
    def test_graph_external_wait_and_record(self):
        ...
        x = smith.ones(4, device="cuda")
        x_cpu = smith.zeros(x.shape, device="cpu").pin_memory()
        flag_cpu = smith.zeros(1, dtype=smith.int32, device="cpu").pin_memory()
        start_event = smith.cuda.Event(external=True)
        end_event = smith.cuda.Event(external=True)

        signalling_graph = smith.cuda.CUDAGraph()
        with smith.cuda.graph(signalling_graph, capture_error_mode="relaxed"):
        ...
        work_graph = smith.cuda.CUDAGraph()
        with smith.cuda.graph(work_graph, capture_error_mode="relaxed"):
            start_event.wait()
            x_cpu.copy_(x, non_blocking=True)
            end_event.record()

        self.assertTrue(
            smith.all(x_cpu == 0.0), "Copy cannot occur until start_event is recorded"
        )

        try:
            signalling_stream = smith.cuda.Stream()
            with smith.cuda.stream(signalling_stream):
                signalling_graph.replay()

            work_stream = smith.cuda.Stream()
            with smith.cuda.stream(work_stream):
                work_graph.replay()
...
class TestFXMemoryProfiler(TestCase):
    ...
    @smith.fx.experimental._config.patch("enrich_profiler_metadata", True)
    def test_fx_memory_profiler_augmentation(self):
        ...
        smith.cuda.memory.empty_cache()
        smith.cuda.memory._record_memory_history()
        compiled = smith.compile(mod, backend="aot_eager", fullgraph=True)
        result = compiled(smith.randn(10, 10, device=device))
        augmented_snapshot = smith.cuda.memory._snapshot(augment_with_fx_traces=True)
        smith.cuda.memory._record_memory_history(enabled=None, clear_history=True)
        smith.cuda.empty_cache()
