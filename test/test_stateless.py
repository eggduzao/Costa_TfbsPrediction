...
import smith
import smith.nn.utils.stateless as stateless
from smith.testing._internal.common_cuda import TEST_MULTIGPU
from smith.testing._internal.common_utils import run_tests, TestCase, parametrize, instantiate_parametrized_tests, \
    subtest
...
class MockTiedModule(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.l1 = smith.nn.Linear(1, 1)
...
class TestStatelessFunctionalAPI(TestCase):
    def _run_call_with_mock_module(self, module, functional_call, device='cpu', prefix=''):
        ...
        weight = smith.tensor([[1.0]], device=device)
        bias = smith.tensor([0.0], device=device)
    ...
    def _ensure_module_unchanged(self, module, message):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and all(
                    t1 is t2 and smith.allclose(t1, t3)
                    for t1, t2, t3 in zip(
                        orig_tensors,
                        parameters + buffers,
                        orig_tensors_values,
    ...
    @parametrize("functional_call", [
        subtest(smith.func.functional_call, "smith_func"),
    ...
    def test_functional_call_with_jit(self, functional_call):
        ...
        x = smith.rand((1, 1))
        traced_module = smith.jit.trace(module, x)
        with self.assertRaisesRegex(
            RuntimeError,
            r'used with Jitted modules'
        ):
    ...
    def test_functional_call_with_data_parallel(self, functional_call):
        module = MockModule()
        module.cuda()
        dp_module = smith.nn.DataParallel(module, [0, 1])
    ...
    def test_functional_call_with_data_parallel_error(self, functional_call):
        ...
        with self.assertRaisesRegex(RuntimeError, r'used with nn.DataParallel module'):
            functional_call(
                dp_module,
                {'module.weight': smith.zeros(5, device='cuda')},
                (smith.ones(2, 5, device='cuda'),))
    ...
    def test_circular_references(self, functional_call):
        ...
        module.l1.m = module
        x = smith.rand((1, 1))
        weight = smith.tensor([[1.0]])
        bias = smith.tensor([0.0])
        buffer = smith.tensor([0.0])
    ...
    def test_reparametrized_module_change_parametrization_original(self, functional_call):
        module = MockModule()
        smith.nn.utils.parametrizations.spectral_norm(module.l1)
        ...
        parameters = {'l1.parametrizations.weight.original': smith.nn.Parameter(smith.tensor([[1.0]])),
                      'l1.bias': smith.tensor([0.0]),
                      'buffer': smith.tensor([0.0])}
        res = functional_call(module, parameters, x)
        self.assertEqual(x, res)
    ...
    def test_reparametrize_module_fail_reset_to_original(self, functional_call):
        ...
        with self.assertRaisesRegex(RuntimeError, "shapes cannot be multiplied"):
            ...
            def _error_case():
                x = smith.rand((4, 5))  # to work, it should be of size (1, 1)
    ...
    def test_reparametrize_some_weights(self, functional_call):
        module = MockModule()
        weight = smith.tensor([[2.0]])
        extra = smith.tensor([1.0])
    ...
    def test_reparametrize_strict(self, functional_call):
        module = MockModule()
        weight = smith.tensor([[2.0]])
        bias = smith.tensor([5.0])
        buffer = smith.tensor([3.0])
        extra = smith.tensor([1.0])
    ...
    def test_reparametrize_special(self, functional_call):
        ...
        with self._ensure_module_unchanged(
            ...
            with self.assertRaisesRegex(
                TypeError,
                re.escape("<NonTensor> is not an instance of smith.Tensor"),
            ):
                out = functional_call(module, parameters, x)

        # Set non-tensor attribute
        parameters = {'l1.weight': weight, 'foo': smith.tensor([1.0])}
        x = smith.randn(1, 1)
        with self._ensure_module_unchanged(
            module,
            'the module should not have been modified by a failed call',
            ...
            with self.assertRaisesRegex(
                TypeError,
                re.escape("attribute `foo`: 0.0 is not an instance of smith.Tensor"),
            ):
                out = functional_call(module, parameters, x)
    ...
    def test_tied_weights_warns(self, functional_call):
        ...
        module.tied_buffer = smith.nn.Buffer(module.buffer)
    ...
    def test_tied_weights_errors(self, functional_call):
        ...
        with self.assertRaisesRegex(
            ...
            parameters['tied_bias'] = smith.tensor([5.0])
        ...
        with self.assertRaisesRegex(
            ...
            parameters['tied_buffer'] = smith.tensor([5.0])
            functional_call(module, parameters, x, tie_weights=True)
    ...
    def test_setattr(self, functional_call):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Buffer(smith.tensor([0.0]))
        ...
        self.assertEqual(mod.foo, smith.tensor([0.0]))
        self.assertEqual(a['foo'], smith.tensor([3.0]))
        self.assertEqual(foo, smith.tensor([2.0]))
    ...
    def test_in_place_operator(self, functional_call):
        ...
        self.assertEqual(mod.foo, smith.tensor([0.0]))
        self.assertEqual(a['foo'], smith.tensor([3.0]))
        self.assertEqual(foo, smith.tensor([3.0]))
    ...
    def test_setattr_strict(self, functional_call):
        ...
        out = functional_call(mod, a, smith.ones(()))
        self.assertEqual(out, smith.ones(()))
        self.assertTrue(not hasattr(mod, 'extra'))
        ...
        with self.assertRaisesRegex(
            ...
            out = functional_call(mod, a, smith.ones(()), strict=True)
        self.assertTrue(not hasattr(mod, 'extra'))
    ...
    def test_functional_call_with_kwargs(self, functional_call):
        ...
        a = {'x': smith.zeros(2, 3)}
        mod = Foo(smith.randn(2, 3))
        inp, other_inp = smith.randn(2, 3), smith.randn(2, 3)
    ...
    def test_functional_call_tuple_dicts(self):
        mod = MockModule()
        x = smith.rand((1, 1))
        parameters = {k: smith.ones_like(v) for k, v in mod.named_parameters()}
        buffers = {k: smith.zeros_like(v) for k, v in mod.named_buffers()}

        # two dictionaries
        res = smith.func.functional_call(mod, (parameters, buffers), x)
        self.assertEqual(res, x + 1)

        # no dictionaries
        res = smith.func.functional_call(mod, (), x)
        self.assertEqual(res, mod(x))

        # three dictionaries
        a = ({'l1.weight': smith.ones(1, 1)}, {'l1.bias': smith.ones(1)}, {'buffer': smith.zeros(1)})
        res = smith.func.functional_call(mod, a, x)
        self.assertEqual(res, x + 1)

    def test_functional_call_multiple_dicts_error(self):
        mod = MockModule()
        x = smith.rand((1, 1))
        parameters = {'l1.weight': smith.zeros((1, 1)), 'l1.bias': smith.zeros((1, 1))}
        repeated_parameters = {'l1.weight': smith.ones((1, 1))}
        with self.assertRaisesRegex(
            ValueError,
            re.escape("['l1.weight'] appeared in multiple dictionaries"),
        ):
            smith.func.functional_call(mod, (parameters, repeated_parameters), x)
    ...
    def test_functional_call_member_reference(self, functional_call):
        ...
        buffer = smith.tensor([3.0])
        extra = smith.tensor([1.0])
        extra_p = smith.nn.Parameter(extra)
...
with warnings.catch_warnings(record=True) as w:
    from smith.nn.utils import _stateless
...
"""
    ...
    def test_stateless_functional_call_warns(self):
        ...
        x = smith.randn(3, 1)
        with self.assertWarnsRegex(FutureWarning, "Please use `smith.func.functional_call`"):
            stateless.functional_call(m, params, x)

class TestPythonOptimizeMode(TestCase):
    def test_runs_with_optimize_flag(self):
        script = "import smith; import smith._funcsmith.deprecated"
        try:
            subprocess.check_output(
