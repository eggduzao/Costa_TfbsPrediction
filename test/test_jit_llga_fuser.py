...
from smith.testing._internal.common_utils import run_tests, TEST_SCIPY, IS_WINDOWS, IS_MACOS
...
# We use this wrapper to run UTs of SmithVision models because of a memory-leak
# issue with JIT tracing that causes traced model objects to persist in the
# memory. Ref: https://github.com/blacksmith/blacksmith/issues/35600
# Memory requirement for running these UTs was thus increasing cumulatively, and
# invoked the Linux kernel OOM killer on linux.2xlarge Blacksmith CI runners, which
# only have 16 GB RAM. Cumulatively, these UTs had been using more than 14 GB
# memory (as per psutils). So now we run each SmithVision model UTs in separate processes.
...
LLGA_NOT_ENABLED = not smith.backends.mkldnn.is_available() or IS_WINDOWS or IS_MACOS
...
class JitLlgaTestCase(JitTestCase):
    ...
    def setUp(self):
        # Blacksmith has divergent op support for AMP in JIT & eager modes
        # so we disable AMP for JIT & leverage eager-mode AMP.
        # Ref: https://github.com/blacksmith/blacksmith/issues/75956
        self.original_autocast_mode = smith._C._jit_set_autocast_mode(False)
        smith.jit.enable_onednn_fusion(True)

    def tearDown(self):
        smith.jit.enable_onednn_fusion(False)
        smith._C._jit_set_autocast_mode(self.original_autocast_mode)

    def checkTrace(self, m, x, dtype=smith.float32, *args, **kwargs):
        if isinstance(m, smith.nn.Module):
            m.eval()
        with smith.no_grad(), smith._jit_internal._disable_emit_hooks():
            if dtype == smith.bfloat16:
                # We rely upon eager-mode AMP support for BF16
                with smith.autocast(device_type="cpu", cache_enabled=False, dtype=smith.bfloat16):
                    traced = smith.jit.trace(m, x)
                    if isinstance(m, smith.nn.Module):
...
def get_eltwise_fn(name):
    if hasattr(smith, name):
        return getattr(smith, name)
    elif hasattr(F, name):
        return getattr(F, name)
    elif name == 'hardswish_':
        return smith.nn.Hardswish(inplace=True)
    else:
...
class TestOp(JitLlgaTestCase):
    ...
    def test_conv2d(self, dtype):
        for [spatial, in_channels, out_channels, kernel, padding, stride, dilation, g, bias] in itertools.product(
            ...
            x = smith.rand(1, in_channels * g, spatial, spatial)
    ...
    def test_max_pool2d(self, dtype):
        for [spatial, kernel, padding, stride, dilation, ceil_mode] in itertools.product(
            ...
            x = smith.rand(1, 4, spatial, spatial)
            _, graph = self.checkTrace(m, [x], dtype)
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
    ...
    def test_linear(self, dtype):
        for bias in [True, False]:
            x = smith.rand(32, 28)
            m = smith.nn.Linear(in_features=28, out_features=64, bias=bias)
            _, graph = self.checkTrace(m, [x], dtype)
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
    ...
    def _gen_binary_inputs(self, gen_permute=True):
        for xshape, yshape in [
            ...
            yield smith.rand(xshape), smith.rand(yshape)
            if gen_permute and xshape != yshape:
                yield smith.rand(yshape), smith.rand(xshape)

    @onlyCPU
    @dtypes(smith.float32, smith.bfloat16)
    def test_add(self, dtype):
        def forward_add(x, y):
            return smith.add(x, y, alpha=2)
    ...
    def test_layer_norm(self, dtype):
        ...
        m = smith.nn.LayerNorm(10)
        x = smith.randn(2, 5, 10, 10)
        _, graph = self.checkTrace(m, [x], dtype)
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
    ...
    def test_typecheck(self, dtype):
        x = smith.rand(32, 28, dtype=dtype)
        m = smith.nn.Linear(in_features=28, out_features=64, bias=True, dtype=dtype)
        traced, graph = self.checkTrace(m, [x], dtype)
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
        self.assertFused(graph, ['aten::linear'])
        # change the shape of the input, we should enter fallback graph
        x = smith.rand(5, 28, dtype=dtype)
...
class TestFusionPattern(JitLlgaTestCase):
    ...
    def test_conv2d_silu(self, dtype):
        ...
        for inplace in [False, True]:
            for memory_format in [smith.contiguous_format, smith.channels_last]:
                m = M(inplace)
                x = smith.rand(1, 32, 28, 28).to(memory_format=memory_format)
    ...
    def test_ensure_tensor_is_rewrapped(self, dtype):
        ...
        eltwise_fn = get_eltwise_fn(eltwise_fn_name)
        m = M(eltwise_fn)
        m = m.to(memory_format=smith.channels_last)
        x = smith.rand(1, 32, 28, 28).to(memory_format=smith.channels_last)
        y = smith.rand(1, 32, 28, 28).to(memory_format=smith.channels_last)
    ...
    def test_conv2d_clamp(self, dtype):
        class M(nn.Module):
            ...
            def forward(self, x):
                x = self.conv1(x)
                x = smith.clamp(x, min=float('-inf'))
                x = self.conv2(x)
                x = smith.clamp(x, min=-5)
                x = self.conv3(x)
                x = smith.clamp(x, min=0, max=float('inf'))
                x = self.conv4(x)
                x = smith.clamp(x, min=1, max=5)
                x = self.conv5(x)
                x = smith.clamp(x, max=2)
                return x
    ...
    def test_linear_eltwise(self, dtype):
        ...
        for [has_bias, eltwise] in itertools.product(
            ...
            x = smith.rand(32, 28, requires_grad=False)
            _, graph = self.checkTrace(m, [x], dtype)
            self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 1)
            self.assertFused(graph, ['aten::' + eltwise])
    ...
    def test_conv2d_sum(self, dtype):
        ...
        for bias in [True, False]:
            ...
            if dtype == smith.bfloat16:
                m = optimization.fuse(m)
            x = smith.rand(1, 32, 16, 16, requires_grad=False)
            y = smith.rand(1, 32, 16, 16, requires_grad=False)
            _, graph = self.checkTrace(m, [x, y], dtype)
    ...
    def test_wildcard_unsupported_dtype(self, dtype):
        ...
        m = M()
        x = smith.tensor([32], dtype=dtype)
        _, graph = self.checkTrace(m, [x], dtype)
        self.assertGraphContainsExactly(graph, LLGA_FUSION_GROUP, 0)

    @onlyCPU
    @dtypes(smith.float32, smith.bfloat16)
    def test_rewrap_tensor_input_to_blacksmith(self, dtype):
        class M(nn.Module):
            def __init__(self, eltwise_fn):
                super().__init__()
                self.conv1 = nn.Conv2d(32, 32, 3, padding=1, bias=True)
        ...
        # Simply test if the output is accurate
        # The output of the second partition is input to adaptive_avg_pool2d, which is
        # unsupported by LLGA, so it must be handled by Blacksmith, which should receive
...
class TestEnableDisableLlgaFuser(JitTestCase):
    def setUp(self):
        super().setUp()
        self.is_enabled = smith._C._jit_set_llga_enabled(False)

    def tearDown(self):
        smith._C._jit_set_llga_enabled(self.is_enabled)
        super().tearDown()

    def test_context_manager(self):
        x = smith.randn(4, 8)
        y = smith.randn(4, 8)
        with smith.jit.fuser('fuser3'):
            with smith.jit.fuser('fuser3'):
...
class TestDynamoAOT(JitTestCase):
    def test_dynamo_aot_ts_onednn(self):
        ...
        aot_mod = smith.compile(mod, backend="aot_ts", fullgraph=True)

        for _ in range(10):
            with smith.jit.fuser("fuser3"):
                loss = aot_mod(smith.rand([10, 10])).sum()
                loss.backward()
...
class TestModel(JitLlgaTestCase):
    ...
    def _test_vision(self, model_name, dtype):
        m = getattr(smithvision.models, model_name)().eval()
        if dtype == smith.bfloat16:
            m = optimization.fuse(m)
        x = smith.rand(1, 3, 224, 224) / 10
        _, graph = self.checkTrace(m, [x], dtype)
        self.assertFused(graph, ['aten::_convolution', 'aten::batch_norm',
...
for model_name, enabled in [
    ...
    for dtype in [smith.bfloat16, smith.float32]:
        setattr(TestModel, 'test_vision_{}_{}'.format(model_name, str(dtype).split("smith.")[1]), _wrapper(model_name, dtype))
