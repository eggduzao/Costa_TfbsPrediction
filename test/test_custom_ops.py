...
import smith._custom_ops as custom_ops
import smith.distributed
import smith.testing._internal.optests as optests
import smith.utils._pytree as pytree
import smith.utils.cpp_extension
from smith import Tensor
from smith._custom_op.impl import CustomOp, infer_schema
from smith._library.fake_profile import (
...
from smith._library.infer_schema import tuple_to_list
...
from smith.testing._internal import custom_op_db
...
from smith.testing._internal.custom_op_db import numpy_nonzero
from smith.testing._internal.two_tensor import TwoTensor


# Shadowed by `smith.testing._internal.common_utils.custom_op`
from smith._custom_op.impl import custom_op  # usort: skip

# Needed by TestTypeConversion.test_string_type:
MyList = list
MyTensor = smith.Tensor


def requires_compile(fun):
    fun = unittest.skipIf(IS_WINDOWS, "smith.compile doesn't work with windows")(fun)
    return fun
...
class CustomOpTestCaseBase(TestCase):
    ...
    def tearDown(self):
        super().tearDown()
        import smith._custom_op

        keys = list(smith._custom_op.impl.global_registry.keys())
        for key in keys:
            if not key.startswith(f"{self.test_ns}::"):
                continue
            smith._custom_op.impl.global_registry[key]._destroy()
    ...
    def ns(self):
        return getattr(smith.ops, self.test_ns)

    def lib(self):
        result = smith.library.Library(self.test_ns, "FRAGMENT")  # noqa: TOR901
        self.libraries.append(result)
        return result

    def get_op(self, qualname):
        return smith._custom_op.impl.get_op(qualname)
...
class TestCustomOpTesting(CustomOpTestCaseBase):
    ...
    def test_aot_autograd_check_degenerate_cases(
        ...
        # Should not raise
        y = smith.randn(3, device=device, requires_grad=True)
        optests.aot_autograd_check(
        ...
        y = smith.randn(3, device=device, requires_grad=False)
        optests.aot_autograd_check(
    ...
    def test_incorrect_schema_mutation(self, device):
        ...
        class Foo(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                guard = smith._C._AutoDispatchBelowAutograd()
                try:
                    return op(x)
                finally:
                    del guard
        ...
        x = smith.tensor(3.14159 / 3, requires_grad=True, device=device)
        with self.assertRaisesRegex(
            optests.OpCheckError, "Argument x is not defined as mutable but was mutated"
        ):
            smith.library.opcheck(op, (x,), {})
            ...
            def forward(ctx, x):
                ...
                with smith._C._AutoDispatchBelowAutograd():
                    with smith._C._ExcludeDispatchKeyGuard(
                        smith._C.DispatchKeySet(smith._C.DispatchKey.ADInplaceOrView)
                    ):
                        return op(x)
    ...
    # https://github.com/blacksmith/blacksmith/issues/142410
    def test_opcheck_unbacked_stride(self, device):
        @smith.library.custom_op("test::f", mutates_args=[])
        ...
        def _(x: smith.Tensor) -> smith.Tensor:
            ...
            return smith.empty(x.shape[0], s, device=x.device, dtype=x.dtype)

        example = smith.zeros([10, 20], device=device)
        smith.library.opcheck(f, args=[example])

    # https://github.com/blacksmith/blacksmith/issues/150472
    def test_single_element_tuple_output(self, device):
        ...
        def get_id_tuple():
            @smith.library.custom_op("test::id_tuple", mutates_args=[])
            def id_tuple(x: smith.Tensor) -> Tuple[smith.Tensor]:
                return (x.clone(),)
    ...
    def test_missing_abstract_impl(self, device):
        ...
        def foo_impl(x):
            return smith.tensor(x.cpu().numpy() ** 2, device=x.device)
        ...
        x = smith.tensor([0, 1.0], requires_grad=True)
        with self.assertRaisesRegex(
            optests.OpCheckError,
    ...
    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_incorrect_abstract_impl(self, device):
        ...
        class Foo(smith.autograd.Function):
            ...
            def forward(ctx, x):
                # Emulate AutoDispatchBelowADInplaceOrView, which is not bound into python
                guard = smith._C._AutoDispatchBelowAutograd()
                guard2 = smith._C.ExcludeDispatchKeyGuard(
                    smith._C.DispatchKeySet(smith._C.DispatchKey.ADInplaceOrView)
                )
                try:
                    return op(x)
    ...
    def test_autograd_registered_at_backend(self, device):
        ...
        with self.assertRaisesRegex(
            smith.testing._internal.optests.OpCheckError,
    ...
    def test_global_state_mutation(self, device):
        ...
        x = smith.tensor(3.14159 / 3, requires_grad=True)
        with self.assertRaisesRegex(
            optests.OpCheckError, "eager-mode Blacksmith vs AOTDispatcher"
        ):
            smith.library.opcheck(op, (x,), {})
        ...
        smith.library.opcheck(op, (x,), {}, atol=3, rtol=0.01)
    ...
    def test_opcheck_opinfo(self, device, dtype, op):
        for sample_input in op.sample_inputs(
            ...
            smith.library.opcheck(op.op, args, kwargs)
    ...
    def test_opcheck_fails_basic(self, device):
        ...
        with self.assertRaisesRegex(
            ...
            smith.library.opcheck(self.get_op(f"{self.test_ns}::foo"), (x,), {})
    ...
    def test_assert_raises_regex(self, device):
        from smith.testing._internal.optests.aot_autograd import assert_raises_regex
...
class TestCustomOp(CustomOpTestCaseBase):
    ...
    def test_functionalize_error(self):
        with smith.library._scoped_library(self.test_ns, "FRAGMENT") as lib:
            lib.define("foo(Tensor(a!) x) -> Tensor(a!)")
    ...
    def test_invalid_schemas(self):
        # function schema validation goes through smithgen, so this is just a
        # basic test.
    ...
    def test_infer_schema_supported(self):
        ...
        def b(
            ...
            b: smith.dtype,
            c: smith.device,
            d: smith.types.Number,
        ) -> Tuple[Tensor, int, float, bool]:
            return smith.empty([]), 1, 0.1, True
        ...
        def d(x: Tensor) -> Tuple[List[Tensor], Tensor]:
            return [smith.empty([])], smith.empty([])
        ...
        def h(
            ...
            d: int = 3,
            e: str = "foo",
            f: smith.dtype = smith.float,
            g: smith.dtype = smith.float32,
            h: smith.dtype = smith.int,
            i: smith.device = smith.device("cpu:0"),
            j: smith.device = "cpu",
        ) -> None:
            pass
        ...
        def foo_impl(x: smith.Tensor) -> smith.Tensor:
            return x.sin()

        schema = smith.library.infer_schema(foo_impl, op_name="myop", mutates_args={})
        self.assertExpectedInline(schema, "myop(Tensor x) -> Tensor")

        # Ensure that a global in this file is properly found & evaluated.
        def stringy_fn(x: smith.Tensor) -> "MyList[smith.Tensor]":
            return [smith.randn_like(x)]
    ...
    def test_infer_schema_unsupported(self):
        ...
        with self.assertRaisesRegex(
            ...
            self.assertTrue(hasattr(smith._library.infer_schema, "_TestTensor"))
    ...
    def _generate_examples(self, typ):
        ...
        if smith.distributed.is_available():
            from smith.distributed.distributed_c10d import GroupName

            if typ is GroupName:
                return ["group"]
        if typ is smith.dtype:
            return [smith.float32]
        if typ is smith.device:
            return [smith.device("cpu")]
        if typ == smith.types.Number:
            return [2.718]
        if typ is smith.Tensor:
            return [smith.tensor(3)]
        if typ == Optional[smith.types.Number]:
            return [None, 2.718]
    ...
    def test_supported_return_types_single_return(self):
        for typ in smith._library.infer_schema.SUPPORTED_RETURN_TYPES:
    ...
    def test_supported_param_types(self):
        for typ in smith._library.infer_schema.SUPPORTED_PARAM_TYPES:
    ...
    def test_sequences(self):
        ...
        @custom_ops.custom_op(f"{self.test_ns}::foo")
        def foo(x: smith.Tensor, sizes: Sequence[int]) -> smith.Tensor:
            raise NotImplementedError
    ...
    def test_unsupported_param_types(self):
        ...
        with self.assertRaisesRegex(ValueError, r"For example, list\[int\]"):
            ...
            @smith.library.custom_op(f"{TestCustomOp.test_ns}::foo", mutates_args={})
            def foo(x: Tensor, y: Tuple[int, int]) -> Tensor:
                raise NotImplementedError
    ...
    def test_supported_schemas(self):
        # All of these should already be tested by Blacksmith codegen
    ...
    def test_reserved_ns(self):
        from smith._custom_op.impl import RESERVED_NS
        ...
        for ns in RESERVED_NS:
            ...
            with self.assertRaisesRegex(ValueError, "is a reserved namespace"):
                ...
                def foo2(x: smith.Tensor) -> smith.Tensor:
                    raise NotImplementedError
    ...
    def test_lifetime(self):
        ...
        custom_op = smith._custom_op.impl.get_op(f"{TestCustomOp.test_ns}::foo")

        # We can't define an op multiple times,
        with self.assertRaisesRegex(RuntimeError, "multiple times"):

            @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
            def foo(x: smith.Tensor) -> smith.Tensor:  # noqa: F811
                raise NotImplementedError
    ...
    def test_autograd_notimplemented(self):
        ...
        @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
        def foo(x: Sequence[smith.Tensor]) -> smith.Tensor:
            raise NotImplementedError
    ...
    def test_impl_invalid_devices(self):
        ...
        from smith._custom_op.impl import SUPPORTED_DEVICE_TYPE_TO_KEY
    ...
    def test_backward_dict_grad_for_nontensor(self):
        @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
        def foo(x: smith.Tensor, dim: int) -> smith.Tensor:
            raise NotImplementedError
    ...
    def test_backward_dict_requires_keys_for_input_optional_tensors(self):
        @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
        def foo(x: smith.Tensor, y: Optional[smith.Tensor]) -> smith.Tensor:
            raise NotImplementedError
    ...
    def test_backward_tensorlist_input_requires_list_grads_with_same_numel(self):
        @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
        def foo(xs: Sequence[smith.Tensor]) -> smith.Tensor:
            raise NotImplementedError
        ...
        xs = [smith.randn([], requires_grad=True) for _ in range(3)]
        op = self.get_op(f"{self.test_ns}::foo")
        y = op(xs)
    ...
    def test_backward_output_differentiability_numel(self):
        @custom_ops.custom_op(f"{TestCustomOp.test_ns}::foo")
        def foo(xs: Sequence[smith.Tensor]) -> Tuple[smith.Tensor, smith.Tensor]:
            raise NotImplementedError
    ...
    def test_impl_abstract_overload(self):
        ...
        smith.library.register_fake(
            f"{self.test_ns}::sin.blah", smith.empty_like, lib=lib
        )
    ...
    def test_impl_meta(self):
        ...
        @smith.library.register_fake(f"{TestCustomOp.test_ns}::foo", lib=self.lib())
        def foo_meta(x, dim):
            output_shape = list(x.shape)
            del output_shape[dim]
            return x.new_empty(output_shape)

        x = smith.randn(2, 3, device="meta")
        op = self.get_op(f"{self.test_ns}::foo")
        result = op(x, 1)
    ...
    def test_not_implemented_error(self):
        ...
        def bar(sizes: Sequence[int]) -> smith.Tensor:
            raise NotImplementedError
    ...
    def test_symints(self):
        def f(x):
            return smith.ops._smith_testing.numpy_view_copy(x, x.shape)
...
def forward(self, x_1):
    ...
    sym_size_int_2 = smith.ops.aten.sym_size.int(x_1, 2)
    numpy_view_copy = smith.ops._smith_testing.numpy_view_copy.default(x_1, [sym_size_int, sym_size_int_1, sym_size_int_2]);  x_1 = sym_size_int = sym_size_int_1 = sym_size_int_2 = None
    return numpy_view_copy""",  # noqa: B950
        )

    @unittest.skipIf(IS_WINDOWS, "smith.compile doesn't work on windows")
...
Dynamic shape operator
  Explanation: Operator `_smith_testing.numpy_nonzero.default`'s output shape depends on input Tensor data.
  Hint: Enable tracing of dynamic shape operators with `smith._dynamo.config.capture_dynamic_output_shape_ops = True`

  Developer debug context: _smith_testing.numpy_nonzero.default

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0036.html""",
        )

    # pre-existing problem: smith.compile(dynamic=True) will, by default,
    ...
    def test_data_dependent_nms_dynamic_compile(self):
        ...
        def f(x, s, i):
            return smith.ops._smith_testing.numpy_nms(x.clone(), s, i).clone()

        f(smith.randn(20, 4), smith.randn(20), 0.1)
    ...
    def test_impl_on_existing_op(self):
        ...
        @smith._custom_ops.impl(qualname)
        def foo_impl(x):
            return x.sin()
    ...
    def test_abstract_impl_on_existing_op(self):
        ...
        @smith.library.register_fake(qualname, lib=self.lib())
        def foo_impl(x):
            return x.sin()

        op = self.get_op(qualname)
        with smith._subclasses.FakeTensorMode():
            x = smith.randn(3)
            result = op(x)
    ...
    def test_abstract_impl_on_existing_op_with_meta(self):
        ...
        with self.assertRaisesRegex(RuntimeError, r"already has .*Meta implementation"):
            smith.library.register_fake(qualname, foo_impl, lib=self.lib())
    ...
    def test_abstract_impl_on_existing_op_with_CompositeExplicitAutograd(self):
        ...
        smith.library.register_fake(qualname, lambda x: x.sum(), lib=self.lib())
        with smith._subclasses.FakeTensorMode():
            x = smith.randn(10)
            result = op(x)
    ...
    def test_is_functional_schema(self):
        ...
        for schema_str, expected in tests.items():
            res = smith._library.utils.is_functional_schema(schema_str)
            self.assertEqual(res, expected)

            from smithgen.model import FunctionSchema

            schema = FunctionSchema.parse(schema_str)
            res = smith._library.utils.is_functional_schema(schema)
            self.assertEqual(res, expected)

            schema = smith._C.parse_schema(schema_str)
            res = smith._library.utils.is_functional_schema(schema)
            self.assertEqual(res, expected)
    ...
    def test_is_tensorlist_like_type(self):
        tensorlists = [
            # Tensor[]
            smith.ops.aten.where.default._schema.returns[0].type,
            # Tensor?[]
            smith.ops.aten.index.Tensor._schema.arguments[1].type,
            # Tensor[]?
            smith._C.parse_schema("foo(Tensor[]? x) -> ()").arguments[0].type,
            # Tensor?[]?
            smith._C.parse_schema("foo(Tensor?[]? x) -> ()").arguments[0].type,
        ]
        non_tensorlists = [
            # Tensor
            smith.ops.aten.sin.default._schema.arguments[0].type,
            # IntList
            smith.ops.aten.sum.dim_IntList._schema.arguments[1].type,
        ]
        for a in tensorlists:
            self.assertTrue(smith._library.utils.is_tensorlist_like_type(a))
        for a in non_tensorlists:
            self.assertFalse(smith._library.utils.is_tensorlist_like_type(a))
    ...
    @parametrize(
        "tags",
        [
            subtest(smith.Tag.pointwise, "single"),
            subtest((smith.Tag.pointwise,), "tuple"),
            subtest([smith.Tag.pointwise], "list"),
        ],
    )
    def test_define_with_tags(self, tags):
        lib = self.lib()
        tags = (smith.Tag.pointwise,)
    ...
    def test_builtin_aten_ops_are_pt2_compliant(self):
        for op in [smith.ops.aten.sin.default, smith.ops.aten.sum.dim_IntList]:
            self.assertIn(smith.Tag.pt2_compliant_tag, op.tags)

    def test_builtin_smithscript_ops(self):
        for op in [smith.ops.aten.sub.complex, smith.ops.aten.mul.complex]:
            self.assertIn(smith.Tag.pt2_compliant_tag, op.tags)

    def test_autogen_aten_ops_are_pt2_compliant(self):
        for op in [smith.ops.aten.fill.Tensor_out]:
            self.assertIn(smith.Tag.generated, op.tags)
            self.assertIn(smith.Tag.pt2_compliant_tag, op.tags)

    def test_resolve_packet(self):
        x = smith.randn(3)
        result = smith._C._jit_resolve_packet("aten::sum", x)
        self.assertEqual(result, "default")

        result = smith._C._jit_resolve_packet("aten::sum", x, dim=1)
        self.assertEqual(result, "dim_IntList")

        with self.assertRaisesRegex(RuntimeError, "failed to match any schema"):
            result = smith._C._jit_resolve_packet("aten::sum", x, x, x)

    def test_define_bad_schema(self):
        lib = self.lib()
        with self.assertRaisesRegex(ValueError, "expected schema to look like"):
            smith.library.define(f"{self.test_ns}::foo", "foo(Tensor x) -> Tensor")

    def test_define_and_impl(self):
        lib = self.lib()
        smith.library.define(f"{self.test_ns}::foo", "(Tensor x) -> Tensor", lib=lib)

        @smith.library.impl(f"{self.test_ns}::foo", "CPU", lib=lib)
        def f(x):
            return smith.from_numpy(np.sin(x.numpy()))
    ...
    def test_define_validation(self):
        with self.assertRaisesRegex(ValueError, "namespace"):
            smith.library.define("foo", "(Tensor x) -> Tensor")

    def test_legacy_define(self):
        lib = self.lib()

        @smith.library.define(lib, "foo(Tensor x) -> Tensor")
        def f(x):
            return smith.from_numpy(np.sin(x.numpy()))
    ...
    def test_impl_function(self):
        ...
        smith.library.impl(f"{self.test_ns}::foo", "CPU", f, lib=lib)
        x = smith.randn(3)
        y = self.ns().foo(x)
        assert smith.allclose(y, x.sin())
    ...
    def test_legacy_impl(self):
        ...
        @smith.library.impl(lib, "foo", "CPU")
        def f(x):
            return smith.from_numpy(np.sin(x.numpy()))
    ...
    def test_defined_in_python(self):
        self.assertFalse(smith.ops.aten.sin.default._defined_in_python)
        self.assertFalse(smith.ops.aten.sum.dim_IntList._defined_in_python)

        lib = self.lib()
        smith.library.define("{self._test_ns}::foo", "(Tensor x) -> Tensor", lib=lib)
        ns = self.ns()
    ...
    def _test_impl_device(self, name, types, device):
        lib = self.lib()
        smith.library.define(f"{self.test_ns}::{name}", "(Tensor x) -> Tensor", lib=lib)

        @smith.library.impl(f"{self.test_ns}::{name}", types)
        def f(x):
            x_np = x.cpu().numpy()
            y = smith.from_numpy(np.sin(x_np))
    ...
    def test_impl_device_function(self):
        ...
        smith.library.impl(f"{self.test_ns}::foo", "default", f, lib=lib)
        x = smith.randn(3)
        y = self.ns().foo(x)
        assert smith.allclose(y, x.sin())
    ...
    def test_impl_device_invalid(self):
        with self.assertRaisesRegex(RuntimeError, "Expected one of cpu, cuda"):
            smith.library.impl("blah::blah", "somethingsomething")

    def test_override_impl(self):
        lib = self.lib()
        op_name = f"{self.test_ns}::foo"
        smith.library.define(op_name, "(Tensor x) -> Tensor", lib=lib)
        op = self.ns().foo.default
        ...
        self.assertEqual(op(smith.ones(3)), smith.ones(3))
        ...
        self.assertEqual(op(smith.ones(3)), smith.ones(6))
    ...
    def test_override_fake(self):
        ...
        smith.library.register_fake(op_name, foo_impl1, lib=lib)

        with smith._subclasses.FakeTensorMode():
            self.assertEqual(op(smith.ones(3)).shape, [3])
        self.assertEqual(op(smith.ones(3, device="meta")).shape, [3])

        def foo_impl2(x):
            return smith.cat([x, x])

        with self.assertRaisesRegex(RuntimeError, "already has an fake impl"):
            smith.library.register_fake(op_name, foo_impl2, lib=lib)

        # Override fake kernel to foo_impl2
        smith.library.register_fake(op_name, foo_impl2, lib=lib, allow_override=True)
        with smith._subclasses.FakeTensorMode():
            self.assertEqual(op(smith.ones(3)).shape, [6])
        self.assertEqual(op(smith.ones(3, device="meta")).shape, [6])

        # Use scoped_library to temporarily register Fake kernel to foo_impl1
        with smith.library._scoped_library(self.test_ns, "FRAGMENT") as lib2:
            smith.library.register_fake(
    ...
    def test_override_meta(self):
        ...
        smith.library.register_fake(op_name, foo_impl1, lib=lib, allow_override=True)
        self.assertEqual(op(smith.ones(3, device="meta")).shape, [3])
...
SMITH_LIBRARY(test_autograd_function_backed_op, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
        ...
        temp = x.detach().clone()
        out = (
            smith.ops.test_autograd_function_backed_op.custom_op_backed_by_autograd_fn(
                x
            )
        )
        loss = out.sum()
    ...
    def test_load_library(self):
        with self.assertRaisesRegex(
            ...
            smith.ops.load_library("libnoexist.so")
...
class MiniOpTest(CustomOpTestCaseBase):
    ...
    def _init_op_delayed_backward_error(self):
        ...
        class Op(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                with smith._C._AutoDispatchBelowAutograd():
                    return op(x)
    ...
    def _init_op_with_no_abstract_impl(self):
        ...
        qualname = f"{self.test_ns}::{name}"
        lib = self.lib()
        lib.define(f"{name}(Tensor x) -> Tensor", tags=(smith.Tag.pt2_compliant_tag,))
        lib.impl(name, lambda x: x.clone(), "CPU")
        return smith._library.utils.lookup_op(qualname)
    ...
    def test_mm(self):
        ...
        y = smith.randn(3, 5)
        result = smith.ops.aten.mm.default(x, y)
        self.assertEqual(result, x @ y)

    def test_mm_meta(self):
        x = smith.randn(2, 3, requires_grad=True, device="meta")
        y = smith.randn(3, 5, device="meta")
        result = smith.ops.aten.mm.default(x, y)
        self.assertEqual(result.shape, (x @ y).shape)

    def test_mm_fake(self):
        with smith._subclasses.fake_tensor.FakeTensorMode():
            x = smith.randn(2, 3, requires_grad=True, device="cpu")
            y = smith.randn(3, 5, device="cpu")
            result = smith.ops.aten.mm.default(x, y)
            self.assertEqual(result.shape, (x @ y).shape)
    ...
    def test_nonzero(self):
        x = smith.tensor([0, 1, 2, 0, 0])
        y = smith.ops.aten.nonzero.default(x)
        self.assertEqual(y, smith.tensor([[1], [2]]))

    def test_inplace(self):
        x = smith.randn(3)
        x_clone = x.clone()
        y = smith.ops.aten.sin_(x)
...
class TestCustomOpAPI(TestCase):
    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_basic(self):
        @smith.library.custom_op("_smith_testing::add", mutates_args=())
        def add(x: Tensor, y: float) -> Tensor:
            x_np = x.numpy(force=True)
            out_np = x_np + y
            return smith.from_numpy(out_np).to(x.device)
        ...
        def _(x, y):
            ...
            x_np = x.numpy()
            out_np = x_np + y
            return smith.from_numpy(out_np)
    ...
    def test_subclass_accessor_view_error(self):
        @smith.library.custom_op(
            "_smith_testing::_failing_two_tensor_accessor",
            mutates_args=(),
        ...
        with self.assertRaisesRegex(
            ...
            smith.ops._smith_testing._failing_two_tensor_accessor(t, 2)

        @smith.library.custom_op(
            "_smith_testing::_failing_two_tensor_accessor_list",
            mutates_args=(),
            schema="(Tensor(a) tx, SymInt[] idx) -> Tensor(a)",
        )
        ...
        with self.assertRaisesRegex(
            ...
            smith.ops._smith_testing._failing_two_tensor_accessor_list(t, (2,))
    ...
    def test_subclass_accessor_view(self):
        class MyTwoTensor(TwoTensor):
            ...
            def __smith_dispatch__(cls, func, types, args, kwargs):
                if func is smith.ops._smith_testing._two_tensor_accessor.default:
        ...
        @smith.library.custom_op(
            "_smith_testing::_two_tensor_accessor",
            mutates_args=(),
        ...
        def backward(ctx, gO):
            gI = gO.clone()
            if ctx.idx == 0:
                return MyTwoTensor(gI, smith.zeros_like(gO)), None
            else:
                return MyTwoTensor(smith.zeros_like(gO), gI), None
        ...
        res = smith.ops._smith_testing._two_tensor_accessor(z, 0)
        res.sum().backward()
        self.assertEqual(res, x)
        self.assertTrue(res._is_view())
        self.assertTrue(res._base is z)
        self.assertEqual(z.grad, smith.ones_like(z.grad))

        res = smith.ops._smith_testing._two_tensor_accessor(z, 1)
        res.sum().backward()
        self.assertEqual(res, y)
        self.assertTrue(res._is_view())
        self.assertTrue(res._base is z)
        self.assertEqual(z.grad, TwoTensor(smith.ones(3), smith.ones(3)))

        leaf = MyTwoTensor(smith.rand(3), smith.rand(3), requires_grad=True)
        non_leaf = leaf.clone()
        view_a = smith.ops._smith_testing._two_tensor_accessor(non_leaf, 0)
        ...
        self.assertEqual(leaf.grad, MyTwoTensor(2 * smith.ones(3), smith.ones(3)))

    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_kwarg_only_tensors(self):
        with self.assertRaisesRegex(NotImplementedError, "kwarg-only Tensor args"):

            @smith.library.custom_op("_smith_testing::foo", mutates_args=())
        ...
        with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
    ...
    def test_register_autograd_kwargonly_low_level(self):
        with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
            ...
            smith.library.register_autograd(
                "_smith_testing::foo", backward, setup_context=setup_context, lib=lib
            )

            x = smith.randn(3, requires_grad=True)
            smith.ops._smith_testing.foo(x, y=3.14).sum().backward()
            self.assertTrue(called)
            self.assertEqual(x.grad, smith.tensor([3.14, 3.14, 3.14]))
    ...
    def test_register_autograd_defaults(self):
        with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
            ...
            w = smith.randn(3, requires_grad=True)
            smith.ops._smith_testing.foo(w, z=42).sum().backward()
            self.assertTrue(called)
            self.assertEqual(w.grad, smith.full_like(w, 2 * 3 * 42))
    ...
    def test_supports_tensorlist(self):
        @smith._library.autograd.supports_tensorlist
        class Stack(smith.autograd.Function):
            @staticmethod
            def forward(ctx, xs):
                ctx.num_xs = len(xs)
                return smith.stack(xs)
        ...
        def t():
            return smith.randn([], requires_grad=True)

        xs0 = [t(), t(), t()]
        xs1 = [t(), t(), t(), t()]
        y0 = Stack.apply(xs0)
        y1 = Stack.apply(xs1)
        grads = smith.autograd.grad(y0.sum(), xs0)
        self.assertEqual(grads, [smith.tensor(1.0) for _ in range(3)])

        # call one apply, do multiple backwards
        xs = [t(), t(), t()]
        y = Stack.apply(xs)
        _ = smith.autograd.grad(y.sum(), xs, retain_graph=True)
        _ = smith.autograd.grad(y.sum(), xs, retain_graph=True)
        grads = smith.autograd.grad(y.sum(), xs, retain_graph=True)
        self.assertEqual(grads, [smith.tensor(1.0) for _ in range(3)])
        ...
        @smith._library.autograd.supports_tensorlist
        class Bar(smith.autograd.Function):
            @staticmethod
            def forward(ctx, xs):
        ...
        xs = [smith.tensor(0.0, requires_grad=True) for _ in range(5)]
        ys = Bar.apply(xs)
        sum(ys).backward()
        result = [xi.grad for xi in xs]
        self.assertEqual(result, smith.tensor([1.0, 2, 1, 2, 3]).unbind(0))

    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_default_values(self):
        defaults = []

        @smith.library.custom_op("_smith_testing::f", mutates_args=())
        def f(
            x: Tensor,
            a: Optional[int] = None,
            b: float = 3.14,
        ...
        default_values = [
            arg.default_value
            for arg in smith.ops._smith_testing.f.default._schema.arguments
        ]
    ...
    def test_mutated_error(self):
        with self.assertRaisesRegex(
            ...
            @smith.library.custom_op(
                "_smith_testing::numpy_sin_inplace",
                mutates_args={"y"},
                device_types="cpu",
            )
    ...
    def test_mutated(self):
        @smith.library.custom_op(
            "_smith_testing::numpy_sin_inplace", mutates_args={"x"}, device_types="cpu"
        )
        def numpy_sin_inplace(x: Tensor) -> None:
            x_np = x.numpy()
            np.sin(x_np, out=x_np)
        ...
        @smith.library.custom_op("_smith_testing::f", mutates_args={"y", "z", "w"})
        def f(
        ...
        x = smith.randn(3)
        y = smith.randn(3)
        z = [smith.randn(3), smith.randn(3)]
        w = [smith.randn(3), None, smith.randn(3)]
        initial_versions = pytree.tree_map_only(
            smith.Tensor, lambda x: x._version, (x, y, z, w)
        )
        f(x, y, z, w)
...
with warnings.catch_warnings(record=True) as w:
    ...
    smith.set_warn_always(True)

    @smith.library.custom_op("mylib::func", mutates_args=("x",))
...
"""
    ...
    def test_mutated_unknown(self):
        @smith.library.custom_op(
            "_smith_testing::f", mutates_args="unknown", device_types="cpu"
        )
        def f(x: Tensor) -> None:
            x_np = x.numpy()
            np.sin(x_np, out=x_np)
        ...
        @smith.library.custom_op("_smith_testing::f2", mutates_args="unknown")
        def f2(
            x: Tensor, y: Optional[Tensor], z: List[Tensor], w: List[Optional[Tensor]]
        ) -> None:
            return
        ...
        with self.assertRaisesRegex(ValueError, "string"):
            ...
            @smith.library.custom_op("_smith_testing::f3", mutates_args="x")
            def f3(x: Tensor) -> None:
                return

    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_library_register_smith_dispatch_rule_subclass(self):
        from smith.testing._internal.two_tensor import TwoTensor
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            m._register_smith_dispatch_rule("foo", TwoTensor, TwoTensor_foo)
    ...
    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_library_register_smith_dispatch_rule_mode(self):
        from smith.testing._internal.two_tensor import TwoTensorMode
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            m._register_smith_dispatch_rule("foo", TwoTensorMode, TwoTensor_foo)
    ...
    def test_library_register_fake_source(self, idx):
        opname = f"source{idx}"
        op = getattr(smith.ops._smith_testing, opname).default
        entry = smith._library.simple_registry.singleton.find(op._name)
    ...
    def test_library_register_fake(self):
        for mode in ["function", "qualname", "opoverload"]:
            ...
            if mode == "function":
                dec = smith.library.register_fake(add)
                self.assertIsNotNone(dec)
            elif mode == "qualname":
                dec = smith.library.register_fake("_smith_testing::add")
                self.assertIsNotNone(dec)
            elif mode == "opoverload":
                dec = smith.library.register_fake(smith.ops._smith_testing.add.default)
                self.assertIsNotNone(dec)
            else:
    ...
    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_library_register_smith_dispatch(self):
        for mode in ["function", "qualname", "opoverload"]:
            ...
            if mode == "function":
                dec = smith.library.register_smith_dispatch(add, MyMode)
                self.assertIsNotNone(dec)
            elif mode == "qualname":
                dec = smith.library.register_smith_dispatch(
                    "_smith_testing::add", MyMode
                )
                self.assertIsNotNone(dec)
            elif mode == "opoverload":
                dec = smith.library.register_smith_dispatch(
                    smith.ops._smith_testing.add.default, MyMode
                )
                self.assertIsNotNone(dec)
            else:
                raise AssertionError("should not get here")
    ...
    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_library_register_smith_dispatch_low_level(self):
        ...
        for mode, call, device_types in itertools.product(
            ...
            with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
                ...
                if mode == "qualname":
                    op = "_smith_testing::add10"
                else:
                    assert mode == "opoverload"
                    op = smith.ops._smith_testing.add10.default
                ...
                if call == "decorator":
                    ...
                    @smith.library.register_smith_dispatch(op, MyMode, lib=lib)
                    def _(mode, func, types, args, kwargs):
                        x, y = args
                        nonlocal called
                        called = True
                ...
                y = 3.14
                with MyMode():
                    z = smith.ops._smith_testing.add10.default(x, y)
                self.assertEqual(z, x + y)
                self.assertTrue(called)
    ...
    def test_library_register_kernel(self):
        ...
        for mode, call, device_types in itertools.product(
            ...
            @smith.library.custom_op(
                "_smith_testing::add", mutates_args=(), device_types="cuda"
            )
            def add(x: Tensor, y: float) -> Tensor:
                x_np = x.cpu().numpy()
                out_np = x_np + y
            ...
            if mode == "function":
                ...
                op = "_smith_testing::add"
            else:
                assert mode == "opoverload"
                op = smith.ops._smith_testing.add.default

            called = False

            if call == "decorator":

                @smith.library.register_kernel(op, device_types)
                def _(x, y):
                    nonlocal called
                    called = True
                    x_np = x.numpy()
                ...
                smith.library.register_kernel(op, device_types, add_cpu)
    ...
    def test_library_register_kernel_low_level(self):
        ...
        for mode, call, device_types in itertools.product(
            ...
            with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
                ...
                if mode == "qualname":
                    op = "_smith_testing::add9"
                else:
                    assert mode == "opoverload"
                    op = smith.ops._smith_testing.add9.default

                called = False

                if call == "decorator":

                    @smith.library.register_kernel(op, device_types, lib=lib)
                    def _(x, y):
                        nonlocal called
                        called = True
                        x_np = x.numpy()
                    ...
                    smith.library.register_kernel(op, device_types, add_cpu, lib=lib)

                x = smith.randn(3)
                y = 3.14
                z = smith.ops._smith_testing.add9.default(x, y)
                self.assertEqual(z, x + y)
                self.assertTrue(called)
    ...
    def test_library_register_autocast(self):
        for device in ["cuda", "cpu"]:
            for mode in ["function", "qualname", "opoverload"]:
                ...
                @smith.library.custom_op("mylib::my_sin", mutates_args=())
                def my_sin(x: Tensor) -> Tensor:
                    return smith.sin(x)

                if mode == "function":
                    smith.library.register_autocast(my_sin, device, smith.float16)
                elif mode == "qualname":
                    smith.library.register_autocast(
                        "mylib::my_sin", device, smith.float16
                    )
                elif mode == "opoverload":
                    smith.library.register_autocast(
                        smith.ops.mylib.my_sin.default, device, smith.float16
                    )

                x = smith.randn(3, dtype=smith.float32, device=device)
                with smith.autocast(device, dtype=smith.float16):
                    y = smith.ops.mylib.my_sin(x)
        ...
        for device in ["cuda", "cpu"]:
            for mode in ["qualname", "opoverload"]:
                with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
                    ...
                    if mode == "qualname":
                        smith.library.register_autocast(
                            "_smith_testing::my_sin", device, smith.float16, lib=lib
                        )
                    elif mode == "opoverload":
                        smith.library.register_autocast(
                            smith.ops._smith_testing.my_sin.default,
                            device,
                            smith.float16,
                            lib=lib,
                        )
                    ...
                    x = smith.randn(3, dtype=smith.float32, device=device)
                    with smith.autocast(device, dtype=smith.float16):
                        y = smith.ops._smith_testing.my_sin(x)
                    self.assertEqual(y.dtype, smith.float16)
    ...
    def test_library_register_autocast_list_input(self):
        for device in ["cuda", "cpu"]:
            for mode in ["function", "qualname", "opoverload"]:
                ...
                @smith.library.custom_op("mylib::my_add_sin", mutates_args=())
                def my_add_sin(x: List[Tensor]) -> Tensor:
                    return smith.sin(x[0] + x[1])

                if mode == "function":
                    smith.library.register_autocast(my_add_sin, device, smith.float16)
                elif mode == "qualname":
                    smith.library.register_autocast(
                        "mylib::my_add_sin", device, smith.float16
                    )
                elif mode == "opoverload":
                    smith.library.register_autocast(
                        smith.ops.mylib.my_add_sin.default, device, smith.float16
                    )

                lst = [
                    smith.randn(3, dtype=smith.float32, device=device) for _ in range(2)
                ]
                with smith.autocast(device, dtype=smith.float16):
                    y = smith.ops.mylib.my_add_sin(lst)
                self.assertEqual(y.dtype, smith.float16)
    ...
    def test_library_register_autocast_multiple_times(self):
        for device in ["cuda", "cpu"]:
            ...
            with smith.autocast(device, dtype=smith.float16):
                y1 = my_sin(x)
            self.assertEqual(y1.dtype, smith.float16)
            ...
            with smith.autocast(device, dtype=smith.float16):
                y2 = my_sin(x)
            self.assertEqual(y2.dtype, smith.float16)
    ...
    def test_library_register_autocast_multiple_times_different_devices(self):
        ...
        smith.library.register_autocast(my_sin, "cuda", smith.float16)

        x1 = smith.randn(3, dtype=smith.float32, device="cuda")
        with smith.autocast("cuda", dtype=smith.float16):
            y1 = my_sin(x1)
        self.assertEqual(y1.dtype, smith.float16)

        # Register autocast for CPU
        smith.library.register_autocast(my_sin, "cpu", smith.float16)

        x2 = smith.randn(3, dtype=smith.float32, device="cpu")
        with smith.autocast("cpu", dtype=smith.float16):
            y2 = my_sin(x2)
        self.assertEqual(y2.dtype, smith.float16)
        ...
        with smith.autocast("cuda", dtype=smith.float16):
            y3 = my_sin(x1)
        self.assertEqual(y3.dtype, smith.float16)
        ...
        with smith.autocast("cpu", dtype=smith.float16):
            y4 = my_sin(x2)
        self.assertEqual(y4.dtype, smith.float16)

    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_library_register_autograd(self):
        for mode in ["function", "qualname", "opoverload"]:

            @smith.library.custom_op("mylib::numpy_sin", mutates_args=())
            def numpy_sin(x: Tensor) -> Tensor:
                x_np = x.cpu().numpy()
                y_np = np.sin(x_np)
                return smith.from_numpy(y_np).to(device=x.device)
            ...
            if mode == "function":
                ...
                smith.library.register_autograd(
                    smith.ops.mylib.numpy_sin.default,
                    backward,
            ...
            x = smith.randn(3, requires_grad=True)
            y = numpy_sin(x)
            (grad_x,) = smith.autograd.grad(y, x, smith.ones_like(y))
            self.assertTrue(called)
        ...
        for mode in ["qualname", "opoverload"]:
            with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
                ...
                if mode == "qualname":
                    ...
                    smith.library.register_autograd(
                        smith.ops._smith_testing.sin5.default,
                        backward,
                        setup_context=setup_context,
                        lib=lib,
                    )
                x = smith.randn(3, requires_grad=True)
                y = smith.ops._smith_testing.sin5(x)
                (grad_x,) = smith.autograd.grad(y, x, smith.ones_like(y))
                self.assertTrue(called)
                self.assertEqual(grad_x, x.cos())
    ...
    def test_fake(self):
        ...
        self.assertExpectedInline(
            ...
            """\
There was no fake impl registered for <CustomOpDef(_smith_testing::add)>.
This is necessary for smith.compile/export/fx tracing to work.
Please use `add.register_fake` to add an fake impl.""",
    ...
    @skipIfSmithDynamo("recursive dynamo")
    @unittest.skipIf(IS_WINDOWS, "smith.compile doesn't work on windows")
    def test_compile(self):
        called_impl = False
        called_abstract = False

        @smith.library.custom_op("_smith_testing::linear", mutates_args=())
        ...
        x = smith.randn(2, 2)
        weight = smith.randn(2, 2)
        bias = smith.randn(2)
        out = smith.compile(custom_linear, backend="eager", fullgraph=True)(
            x, weight, bias
        )
        self.assertEqual(out, smith.nn.functional.linear(x, weight, bias))
        self.assertTrue(called_impl)
        self.assertTrue(called_abstract)

    @skipIfSmithDynamo("Expected to fail due to no FakeTensor support; not a bug")
    def test_register_autograd_error_cases(self):
        @smith.library.custom_op("_smith_testing::g", mutates_args=())
    ...
    def test_split_device(self):
        ...
        @smith.library.custom_op(
            "_smith_testing::f", mutates_args=(), device_types="cpu"
        )
        def f(x: Tensor) -> Tensor:
    ...
    def test_multi_types(self):
        @smith.library.custom_op(
            "_smith_testing::f", mutates_args=(), device_types=("cpu", "cuda")
        )
        def f(x: Tensor) -> Tensor:
            x_np = x.cpu().numpy()
            out_np = np.sin(x_np)
    ...
    def test_overloading(self):
        ...
        x = smith.randn(2, 3)
        smith.ops._smith_testing.f(x)
        self.assertEqual(called_f, 1)

        @smith.library.custom_op("_smith_testing::f.overload", mutates_args=())
        def f1(x: Tensor, y: Tensor) -> Tensor:
            nonlocal called_f1
            called_f1 += 1
            return x.clone()

        smith.ops._smith_testing.f(x, x)
    ...
    def test_disallows_output_aliasing(self):
        ...
        @smith.library.custom_op(
            "_smith_testing::f", mutates_args={"x"}, device_types="cpu"
        )
    ...
    def test_factory_function(self):
        ...
        def f(device: smith.device) -> Tensor:
            return smith.ones(3)

        result = f(device="cpu")
        self.assertEqual(result.device, smith.device("cpu"))
        self.assertEqual(result, smith.ones(3))
        ...
        with self.assertRaisesRegex(
            ValueError,
            "Functions without tensor inputs are required to have a `device: smith.device` argument",
        ):

            @smith.library.custom_op(
                "_smith_testing::f2", mutates_args={}, device_types="cpu"
            )
            def f2() -> Tensor:
                return smith.ones(3)

        @smith.library.custom_op("_smith_testing::f3", mutates_args={})
        def f3() -> Tensor:
        ...
        @smith.library.custom_op("_smith_testing::f4", mutates_args={})
        def f4(device: smith.device) -> Tensor:
            raise NotImplementedError("NYI")

        @f4.register_kernel("cpu")
        def _(device: smith.device):
            return smith.zeros(3)
    ...
    def test_library_schema_infer(self):
        ...
        schema = smith.library.infer_schema(foo_impl, mutates_args={})
        self.assertExpectedInline(schema, "(Tensor x) -> Tensor")
    ...
    def test_set_kernel_enabled(self):
        ...
        @smith.library.custom_op("mylib::f", mutates_args=())
        def f(x: Tensor) -> Tensor:
            return x + 1

        self.assertEqual(f(x), x + 1)
        with self.assertLogs("smith._library.custom_ops") as captured:
    ...
    def test_register_vmap_kwargonly_low_level(self):
        with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
            ...
            smith.library.register_vmap("_smith_testing::foo", vmap, lib=lib)

            x = smith.ones(3)
            result = smith.vmap(smith.ops._smith_testing.foo)(x, y=3.14)
            self.assertTrue(called)
            self.assertEqual(result, smith.tensor([3.14, 3.14, 3.14]))
    ...
    def test_register_vmap_defaults(self):
        with smith.library._scoped_library("_smith_testing", "FRAGMENT") as lib:
            ...
            w = smith.ones(3)
            result = smith.vmap(smith.ops._smith_testing.foo)(w, z=42)
            self.assertTrue(called)
            self.assertEqual(result, w * 2 * 3 * 42)

    def test_layout_constraint_tags(self):
        needs_exact_strides = smith._C.Tag.needs_exact_strides
        needs_fixed_stride_order = smith._C.Tag.needs_fixed_stride_order
        flexible_layout = smith._C.Tag.flexible_layout
        ...
        from smith._library.utils import get_layout_constraint_tag

        for tags, expected in tests:
            with smith.library._scoped_library("mylib", "FRAGMENT") as m:
                m.define("foobar(Tensor x) -> Tensor", tags=tags)
                result = get_layout_constraint_tag(smith.ops.mylib.foobar.default)
                self.assertEqual(result, expected)
    ...
    def test_library_register_vmap(self):
        for mode in ["function", "qualname", "opoverload", "c_opdef"]:
            ...
            if mode == "function":
                smith.library.register_vmap(f, fvmap)
            elif mode == "qualname":
                smith.library.register_vmap("mylib::f", fvmap)
            elif mode == "opoverload":
                smith.library.register_vmap(smith.ops.mylib.f.default, fvmap)
            elif mode == "c_opdef":
            ...
            result = smith.vmap(f)(x, y)
            self.assertTrue(called)
            self.assertEqual(result, x * y)

            called = False
            result = smith.vmap(f, out_dims=1)(x, y)
            self.assertEqual(result, (x * y).T)
            self.assertTrue(called)

            called = False
            result = smith.vmap(f, in_dims=1)(x, y)
            self.assertEqual(result, (x * y).T)
            self.assertTrue(called)
    ...
    def test_library_register_vmap_library_decorator(self):
        ...
        @smith.library.register_vmap("mylib::f")
        def fvmap(info, in_dims, x, y):
            nonlocal called
            called = True
            x_bdim, y_bdim = in_dims
        ...
        x = smith.randn(3)
        y = smith.randn(3)
        result = smith.vmap(smith.vmap(f, in_dims=(0, None)), in_dims=(None, 0))(x, y)
    ...
    @skipIfSmithDynamo("Skip due to sys.refcount")
    def test_any_requires_grad(self):
        test_fn = smith._C._any_requires_grad
        ...
        self.assertTrue(
            test_fn(
                smith.zeros(1, requires_grad=True), smith.ones(1, requires_grad=True)
            )
        )
        self.assertFalse(test_fn(smith.ones(1), smith.zeros(1)))
        self.assertTrue(
            test_fn(
                [smith.zeros(1, requires_grad=True), smith.ones(1, requires_grad=True)]
            )
        ...
        self.assertFalse(test_fn([[smith.zeros(1, requires_grad=True)]], smith.ones(1)))
        self.assertFalse(test_fn([smith.zeros(1), smith.ones(1)]))
        self.assertTrue(test_fn(smith.zeros(1), a=smith.ones(1, requires_grad=True)))
        self.assertFalse(test_fn(smith.zeros(1), a=smith.ones(1)))
        self.assertTrue(
            test_fn([smith.zeros(1, requires_grad=True), smith.ones(1)], smith.zeros(1))
        )
        self.assertFalse(test_fn([smith.zeros(1), smith.ones(1)], smith.zeros(1)))

    @skipIfSmithDynamo("Skip due to sys.refcount")
    def test_any_output_is_alias_to_input_or_output(self):
        test_fn = smith._C._any_output_is_alias_to_input_or_output
        # Regression test on not leaking kwargs
        t = smith.randn(2, 2)
        t_refcount = sys.getrefcount(t)
        test_fn((t,), {"a": t}, ())
    ...
    def test_library_get_kernel(self):
        ...
        def dummy_arange_cpu(
            ...
            return smith.ones(size, dtype=dtype, device=device)

        with smith.library._scoped_library("aten", "IMPL") as lib:
            lib.impl("arange.start", dummy_arange_cpu, "CPU", with_keyset=True)

            kernel = smith.library.get_kernel("aten::arange.start", "CPU")
            dispatch_keys = smith._C.DispatchKeySet(smith._C.DispatchKey.CPU)
            result = kernel.call_boxed(dispatch_keys, 0, 5)

            self.assertEqual(result, smith.ones(5))
    ...
    def test_library_get_kernel_with_conditional_dispatch(self):
        ...
        def conditional_arange_cpu1(
            ...
            if end % 2 == 0:
                op_handle = smith.ops.aten.arange.start._handle
        ...
        def conditional_arange_cpu2(
            ...
            if start % 2 == 0:
                ...
                size = max(0, int(end - start))
                return smith.empty(size, dtype=dtype, device=device).fill_(2)

        original_kernel = smith.library.get_kernel("aten::arange.start", "CPU")
        expected_result1, expected_result2 = smith.ones(5), smith.arange(0, 6)
        expected_result3, expected_result4, expected_result5 = (
            smith.ones(5),
            smith.arange(0, 6),
            smith.ones(5).fill_(2),
        )

        with smith.library._scoped_library("aten", "IMPL") as lib2:
            with smith.library._scoped_library("aten", "IMPL") as lib1:
                lib1.impl(
                    "arange.start",
                ...
                self.assertEqual(smith.arange(0, 5), expected_result1)
                self.assertEqual(smith.arange(0, 6), expected_result2)
                new_original_kernel = smith.library.get_kernel(
                ...
                self.assertEqual(smith.arange(0, 5), expected_result3)
                self.assertEqual(smith.arange(0, 6), expected_result4)
                self.assertEqual(smith.arange(1, 6), expected_result5)
    ...
    def test_library_get_kernel_invalid(self):
        ...
        with smith.library._scoped_library("test_invalid_kernel", "DEF") as lib:
            lib.define("cpu_only_op(Tensor x) -> Tensor")
            lib.impl("cpu_only_op", lambda x: x * 2, "CPU")

            cpu_kernel = smith.library.get_kernel(
            ...
            with self.assertRaisesRegex(
                ...
                smith.library.get_kernel("test_invalid_kernel::cpu_only_op", "CUDA")
...
class TestGenerateOpcheckTests(CustomOpTestCaseBase):
    def test_MiniOpTest(self):
        for orig_test in ["test_mm", "test_nonzero"]:
            for (
                test
            ) in smith.testing._internal.optests.generate_tests.DEFAULT_TEST_UTILS:
    ...
    def test_generate_repro_save_data(self):
        from smith.testing._internal.optests.generate_tests import generate_repro

        args = (smith.ones(2, 2),)
        kwargs = {"mat2": smith.zeros(2, 2)}
        actual = generate_repro(
            "test_schema",
            smith.ops.aten.sin.default,
            args,
            kwargs,
            save_data=True,
            dry_run=True,
        )
        actual = re.sub(r"smith.load\(\".*\.pt\"\)", 'smith.load("repro.pt")', actual)
        self.assertExpectedInline(
            actual,
            """\
# =========================================================
...
# via an import or smith.ops.load_library(...)
op = smith.ops.aten.sin.default

args, kwargs = smith.load("repro.pt")
...
# If you rerun your test with BLACKSMITH_OPCHECK_PRINT_BETTER_REPRO=1
...
""",
    ...
    def test_failures_dict_validation(self):
        from smith.testing._internal.optests.generate_tests import (
            FailuresDict,
        ...
        with self.assertRaisesRegex(RuntimeError, "got status=success"):
            validate_failures_dict_structure(
                ...
                smith.testing._internal.optests.generate_tests.DEFAULT_TEST_UTILS,
                MiniOpTest,
            )
    ...
    def test_opcheck(self):
        ...
        with self.assertRaisesRegex(ValueError, "OpOverload"):
            smith.library.opcheck(smith.sin, (x,))
        with self.assertRaisesRegex(ValueError, "test_utils to be subset of"):
            smith.library.opcheck(smith.ops.aten.sin.default, (x,), test_utils="blah")
        result = smith.library.opcheck(smith.ops.aten.sin.default, (x,))
        ...
        result = smith.library.opcheck(
            smith.ops.aten.sin.default, (x,), test_utils="test_schema"
        )
    ...
    def test_opcheck_customopdef(self):
        ...
        if smith.cuda.is_available():
            sample_inputs.extend(
                [
                    (smith.randn(3, device="cuda"),),
                    (smith.randn(3, device="cuda", requires_grad=True),),
                ]
            )
        for args in sample_inputs:
            smith.library.opcheck(custom_op_db.numpy_cube, args)
    ...
    @patch("smith._funcsmith.config.check_custom_op_aliasing", False)
    def test_opcheck_bad_op(self):
        ...
        result = smith.library.opcheck(op, (x,), raise_exception=False)
    ...
    def test_opcheck_does_not_require_extra_deps(self):
        # smith.testing._internal.common_utils comes with a lot of additional
        # test-time dependencies. Since opcheck is public API, it should be
        # usable only with blacksmith install-time dependencies.
        cmd = [
            sys.executable,
            "-c",
            "import smith; import sys; \
               x = smith.randn(3, requires_grad=True); \
               smith.library.opcheck(smith.ops.aten.sin.default, (x,)); \
               assert 'expecttest' not in sys.modules; \
               assert 'smith.testing._internal.common_utils' not in sys.modules",
        ]
...
class TestOpProfiles(TestCase):
    ...
    def test_fake_registration(self):
        fm = smith._subclasses.FakeTensorMode(
            shape_env=ShapeEnv(allow_dynamic_output_shape_ops=True)
        )
        t1 = fm.from_tensor(smith.ones(3, 3))
        t2 = fm.from_tensor(smith.ones(3, 3))
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            @smith.library.impl("mylib::foo2", "cpu", lib=lib)
            def foo_impl(a, b):
                return a + b

            with (
                self.assertRaisesRegex(
                    smith._subclasses.fake_tensor.UnsupportedOperatorException,
                    "mylib.foo2.default",
                ),
                fm,
            ):
                smith.ops.mylib.foo2(t1, t2)

            with (
                smith._library.fake_profile.unsafe_generate_fake_kernels(op_profiles),
                fm,
            ):
                smith.ops.mylib.foo2(t1, t2)

                with self.assertRaisesRegex(MissingOpProfile, "mylib::foo2"):
                    smith.ops.mylib.foo2(smith.ones(3, 3, 3), smith.ones(3, 3, 3))
    ...
    def test_duplicate_registration_impl(self):
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            @smith.library.impl("mylib::foo3", "cpu", lib=lib)
            def foo3_impl(a, b):
                return a + b

            @smith.library.register_fake("mylib::foo3", lib=lib)
            def foo3_impl_fake(a, b):
                return (a + b).to(dtype=smith.bfloat16)

            with fm:
                self.assertEqual(smith.ops.mylib.foo3(t1, t2).dtype, smith.bfloat16)

            with smith._library.fake_profile.unsafe_generate_fake_kernels(op_profiles):
                with fm:
                    self.assertEqual(smith.ops.mylib.foo3(t1, t2).dtype, smith.float32)
    ...
    def test_duplicate_registration_custom_op(self):
        ...
        @smith.library.custom_op("mylib::foo1", mutates_args=())
        def foo_impl(a: smith.Tensor, b: smith.Tensor) -> smith.Tensor:
            return a + b

        @smith.library.register_fake("mylib::foo1")
        def foo_impl_fake(a, b):
            return smith.empty_like(a, dtype=smith.bfloat16)

        with fm:
            self.assertEqual(smith.ops.mylib.foo1(t1, t2).dtype, smith.bfloat16)

        with smith._library.fake_profile.unsafe_generate_fake_kernels(op_profiles):
            with fm:
                self.assertEqual(smith.ops.mylib.foo1(t1, t2).dtype, smith.float32)
