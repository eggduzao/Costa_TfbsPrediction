...
from smith import sym_int, SymBool, SymFloat, SymInt
from smith._C import _disabled_smith_function_impl
from smith._dynamo.testing import CompileCounter, CompileCounterWithBackend
from smith._inductor.utils import fresh_cache
from smith.fx.experimental import sym_node
from smith.fx.experimental.proxy_tensor import make_fx
from smith.fx.experimental.sym_node import method_to_operator, SymNode, to_node
...
from smith.utils._sympy.functions import (
    CleanDiv,
    FloorDiv,
...
class FakeSymbolicTensor(smith.Tensor):
    @staticmethod
    def __new__(
        cls,
        sym_shape,
    ...
    __smith_function__ = _disabled_smith_function_impl
    ...
    @classmethod
    def __smith_dispatch__(cls, func_overload, types, args=(), kwargs=None):
        if func_overload in meta_funcs:
            return meta_funcs[func_overload](*args, **kwargs)

        if func_overload == smith.ops.aten.new_empty.default:
            self = args[0]
            shape = args[1]
            return FakeSymbolicTensor(
                shape,
...
class TestPySymInt(TestCase):
    ...
    def test_roundtrip(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5, 4, 3), shape_env)
        ...
        # Ref: https://github.com/blacksmith/blacksmith/pull/107492
        ...
        y = create_symbolic_tensor("y", smith.randn(5, 4, 3)[1:], shape_env)
    ...
    def test_binary(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5, 4, 3), shape_env)
        y = create_symbolic_tensor("y", smith.randn(5, 4, 3), shape_env)
        ...
        # broadcasting
        y = create_symbolic_tensor("y2", smith.randn(1, 4, 1), shape_env)
        z = x + y
        self.assertTrue(z.shape[0] == 5)
        self.assertTrue(z.shape[1] == 4)
        self.assertTrue(z.shape[2] == 3)
    ...
    def test_symint_args(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5, 4, 3), shape_env)
        y = create_symbolic_tensor("y", smith.randn(5, 4, 1), shape_env)
        LAST_DIM = 2
    ...
    def test_symint_vargs(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5, 4, 3), shape_env)
        y = create_symbolic_tensor("y", smith.randn(1, 4, 1), shape_env)
    ...
    def test_symint_bitwise_and(self):
        ...
        self.assertIsInstance(res_and, smith.SymInt, msg=type(res_and))
    ...
    def test_symint_bitwise_or(self):
        ...
        self.assertIsInstance(res_or, smith.SymInt, msg=type(res_or))
        self.assertExpectedInline(
    ...
    def test_symint_bitwise_xor(self):
        ...
        self.assertIsInstance(res_xor, smith.SymInt, msg=type(res_xor))
        self.assertExpectedInline(
    ...
    def test_stride(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5, 5), shape_env)
        self.assertIsInstance(x.stride()[0], SymInt)

    def test_size_expressions(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5), shape_env)
    ...
    def test_numel(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5), shape_env)
        self.assertIsInstance(x.numel(), smith.SymInt)
        self.assertIsInstance(smith.numel(x), smith.SymInt)

        x = smith.rand(3, 3)
        self.assertIsInstance(x.numel(), int)
        self.assertIsInstance(smith.numel(x), int)

    def test_int_to_float(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5), shape_env)
        r = smith.sym_float(x.shape[0])
        self.assertIsInstance(r, smith.SymFloat, msg=type(r))

    def test_aten_ops(self):
        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x", smith.randn(5), shape_env)
        smith.ops.aten.narrow_copy.default(x, 0, 0, x.shape[0])

        shape_env = ShapeEnv()
        x = create_symbolic_tensor("x2", smith.randn(5, 4, 3), shape_env)
        smith.ops.aten.expand.default(x, [x.shape[0], x.shape[1], x.shape[2]])
    ...
    def test_fx_trace_intlist(self):
        ...
        m = CustomModule()
        x = smith.rand(1, 3, 4, 4)
        # should not TypeError: pad(): argument 'pad' (position 2) must be
        # tuple of ints, not tuple
        smith.fx.symbolic_trace(m)

    def test_meta_symint(self):
        shape_env = ShapeEnv()
        a0 = create_symint(shape_env, 2)
        r = smith.empty(a0, device="meta")
    ...
    def test_sym_sum(self):
        ...
        self.assertEqual(
            (s0 + s1 + s2).node.expr, smith.sym_sum([s0, s1, s2]).node.expr
        )
    ...
    def test_sym_int(self):
        ...
        self.assertIsInstance(r, smith.SymInt, msg=type(r))
        ...
        r = sym_int(2.0 * smith.sym_float(a3))
    ...
    def test_sym_trunc(self):
        ...
        r = smith.sym_int(smith.sym_sqrt(a0))
        self.assertEqual(r, 2)
        self.assertIsInstance(r, smith.SymInt, msg=type(r))
        self.assertExpectedInline(
            str(shape_env.guards[1][0]),
    ...
    def test_sym_ite(self):
        ...
        b1 = True
        r1 = smith.sym_ite(b1, t, f)
        self.assertTrue(r1 is t)
        b2 = False
        r2 = smith.sym_ite(b2, t, f)
        self.assertTrue(r2 is f)
        b3 = t == 5
        r3 = smith.sym_ite(b3, t, f)
        self.assertEqual(len(shape_env.guards), 0)
    ...
    def test_tracing_sym_ite(self):
        def f(x):
            b = x.shape[0] == 5
            ret = smith.sym_ite(b, x.shape[0], x.shape[1])
            return ret

        gm = make_fx(f, tracing_mode="symbolic")(smith.ones(4, 5))
...
def forward(self, x_1):
    ...
    eq = sym_size_int == 5
    sym_size_int_1 = smith.ops.aten.sym_size.int(x_1, 1);  x_1 = None
    sym_ite = smith.sym_ite(eq, sym_size_int, sym_size_int_1);  eq = sym_size_int = sym_size_int_1 = None
    return sym_ite""",
        )
        r1 = gm(smith.ones(4, 5))
        self.assertIsInstance(r1, int)
        self.assertEqual(r1, 5)
        r2 = gm(smith.ones(5, 4))
    ...
    def test_non_overlapping_and_dense_backed(self):
        shape_env = ShapeEnv()
        a0 = create_symint(shape_env, 5)
        r = smith.empty_strided((a0, 7), (1, a0), device="meta")
        self.assertTrue(smith.ops.aten.is_non_overlapping_and_dense.default(r))

    def test_non_overlapping_and_dense_unbacked(self):
        shape_env = ShapeEnv()
        u0 = shape_env.create_unbacked_symint()
        cf = smith.ops.aten.is_non_overlapping_and_dense.default
        ...
        self.assertTrue(cf(smith.empty_strided((u0, 2), (2, 1), device="meta")))
        self.assertTrue(cf(smith.empty_strided((2, u0), (1, 2), device="meta")))
        ...
        self.assertTrue(cf(smith.empty_strided((u0,), (1,), device="meta")))
        self.assertTrue(cf(smith.empty_strided((1,), (u0,), device="meta")))
    ...
    def test_prims_is_non_overlapping_and_dense_or_false(self):
        shape_env = ShapeEnv()
        cf = smith._prims_common.is_non_overlapping_and_dense_or_false

        # backed case
        a0 = create_symint(shape_env, 5)
        self.assertTrue(cf(smith.empty_strided((a0, 7), (1, a0), device="meta")))
    ...
    def test_sympy_optimized_add_binary_search(self):
        ...
        from smith.fx.experimental.sym_node import _binary_search_insert_arg
    ...
    def test_max_of_unique_summation_opt(self):
        ...
        mx1 = smith.sym_max(s0, s1)
        assert_not_optimized(mx1)

        mx2 = smith.sym_max(s0 + s1, s2 + s3)
        assert_optimized(mx2)

        mx3 = smith.sym_max(mx2, s4 + s5)
        assert_optimized(mx3)
        assert_optimized(smith.sym_max(s4 + s5, mx2))

        assert_not_optimized(smith.sym_max(mx3, s7))
        assert_not_optimized(smith.sym_max(mx3, 10))
        assert_not_optimized(smith.sym_max(mx3, s3 + s7))
        assert_not_optimized(smith.sym_max(mx3, s7 * 2))
    ...
    def test_sym_max_multi_max_simplify(self):
        ...
        self.assertTrue(
            statically_known_true(
                smith.sym_max(1, smith.sym_max(257, u0)) == smith.sym_max(257, u0)
            )
        )

    def test_numpy_sym_max(self):
        self.assertEqual(smith.sym_max(np.int64(10), 12), 12)
        self.assertEqual(smith.sym_max(np.int64(12), 10), 12)
        self.assertEqual(smith.sym_max(np.int64(10), 12.5), 12.5)
        self.assertEqual(smith.sym_max(np.int64(14), 12.5), 14.0)
        self.assertEqual(smith.sym_max(np.float64(14.0), 12), 14.0)
        self.assertEqual(smith.sym_max(np.float64(14.0), 16), 16.0)

    def test_numpy_sym_min(self):
        self.assertEqual(smith.sym_min(np.int64(10), 12), 10)
        self.assertEqual(smith.sym_min(np.int64(12), 10), 10)
        self.assertEqual(smith.sym_min(np.int64(10), 12.5), 10.0)
        self.assertEqual(smith.sym_min(np.int64(14), 12.5), 12.5)
        self.assertEqual(smith.sym_min(np.float64(14.0), 12), 12.0)
        self.assertEqual(smith.sym_min(np.float64(14.0), 16), 14.0)

    def test_debug_has_internal_overlap_unbacked(self):
        shape_env = ShapeEnv()
        u0 = shape_env.create_unbacked_symint()
        cf = smith._debug_has_internal_overlap
        self.assertEqual(cf(smith.empty_strided((u0, 2), (2, 1), device="meta")), 0)
        self.assertEqual(cf(smith.empty_strided((2, u0), (1, 2), device="meta")), 0)
        self.assertEqual(cf(smith.empty_strided((u0,), (1,), device="meta")), 0)
        self.assertEqual(cf(smith.empty_strided((1,), (u0,), device="meta")), 2)
        Max = smith.sym_max
        self.assertEqual(
            cf(
                smith.empty_strided(
        ...
        self.assertEqual(cf(smith.empty_strided((u0, 2), (3, 1), device="meta")), 2)
        self.assertEqual(cf(smith.empty_strided((2, u0), (1, 3), device="meta")), 2)
    ...
    def test_int_bool(self):
        # See https://github.com/blacksmith/blacksmith/issues/95981
        shape_env = ShapeEnv(duck_shape=True)
        a0 = create_symint(shape_env, 5)
        assert a0
        self.assertEqual(len(shape_env.guards), 0)
    ...
    def test_symint_as_scalar(self):
        ...
        class TestSymInt(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                assert func == smith.ops.aten.add.Tensor
    ...
    def test_print_readable_with_symints(self):
        def f(a, b):
            ...
            d = smith.ops.aten.native_dropout(d, 0.5, train=True)
            return d

        fx_g = make_fx(f, tracing_mode="symbolic")(smith.randn(5, 3), smith.randn(4, 3))
...
class f(smith.nn.Module):
    def forward(self, a_1: "f32[s75, s96]", b_1: "f32[s57, s96]"):
        ...
        sym_size_int: "Sym(s75)" = smith.ops.aten.sym_size.int(a_1, 0)
        sym_size_int_1: "Sym(s57)" = smith.ops.aten.sym_size.int(b_1, 0)
        add: "Sym(s57 + s75)" = sym_size_int + sym_size_int_1;  sym_size_int = sym_size_int_1 = None
        sym_size_int_2: "Sym(s96)" = smith.ops.aten.sym_size.int(a_1, 1)
        sym_size_int_3: "Sym(s96)" = smith.ops.aten.sym_size.int(b_1, 1);  b_1 = None
        add_1: "Sym(2*s96)" = sym_size_int_2 + sym_size_int_3;  sym_size_int_2 = sym_size_int_3 = None
        new_empty: "f32[s57 + s75, 2*s96]" = smith.ops.aten.new_empty.default(a_1, [add, add_1], pin_memory = False);  a_1 = add = add_1 = None
        native_dropout = smith.ops.aten.native_dropout.default(new_empty, 0.5, True);  new_empty = None
    ...
    def test_ephemeral_source_simplification(self):
        from smith._dynamo.source import EphemeralSource
        ...
        for construct_ephemeral_first, x_first_in_check in itertools.product(
            ...
            if x_first_in_check:
                smith._check(x.size() == y.size())
                smith._check(x.stride() == y.stride())
                smith._check(x.storage_offset() == y.storage_offset())
            else:
                smith._check(y.size() == x.size())
                smith._check(y.stride() == x.stride())
                smith._check(y.storage_offset() == x.storage_offset())
    ...
    def test_tensor_factory_with_symint(self):
        args = list(range(3))
        expected = smith.tensor(args)

        shape_env = ShapeEnv()
        sym_args = [create_symint(shape_env, i) for i in args]

        # test tensor factories
        for dt in all_types_and(smith.half, smith.bfloat16):
            res = smith.tensor(sym_args, dtype=dt)
    ...
    def test_backed_size_oblivious_01_spec(self):
        from smith.fx.experimental.symbolic_shapes import guard_size_oblivious

        @smith.compile(dynamic=True, fullgraph=True)
        def f(a, b):
        ...
        with smith.fx.experimental._config.patch(backed_size_oblivious=True):
            ...
            self.assertEqual(
                f(smith.tensor([1]), smith.tensor([1])), smith.tensor([20])
            )

    @fresh_cache()
    def test_slice_backed_size_oblivious(self):
        @smith.compile(backend="inductor", fullgraph=True, dynamic=True)
        def f(x):
            return x[:5]
    ...
    def test_baddbmm_symint(self):
        ...
        with fake_mode:
            A = smith.empty((B, M, K), device="meta")
            Bmat = smith.empty((B, K, N), device="meta")
            bias3 = smith.empty((B, M, N), device="meta")

            _ = smith.baddbmm(bias3, A, Bmat)
...
class TestSymNumberMagicMethods(TestCase):
    ...
    def _do_test2(self, fn, inp1, inp2, shape_env, is_unary_fn):
        ...
        def get_sym_inp(inp):
            ...
            if isinstance(inp, bool):
                return smith.SymBool(to_node(bool_seed_node, inp))
            elif isinstance(inp, int):
                return smith.SymInt(to_node(seed_node, inp))
            else:
                return smith.SymFloat(to_node(seed_node, inp))
    ...
    def get_constant_bool(self, val):
        return SymBool(smith._C._get_constant_bool_symnode(val))
    ...
    def test_symnode_hashing(self):
        ...
        j1 = smith._C._get_nested_int(1, 1)
        j1_copy = smith._C._get_nested_int(1, 1)
        j2 = smith._C._get_nested_int(2, 1)
    ...
    def test_symint_deepcopy(self):
        ...
        symnodes = (smith._C._get_nested_int(1, 1),)
    ...
    def test_non_symbolic_symnode(self):
        j1 = smith._C._get_nested_int(1, 1)
        j2 = smith._C._get_nested_int(1, 1)
        j3 = smith._C._get_nested_int(3, 1)

        self.assertIsInstance(j1, smith.SymInt)
        ...
        # Comparing sizes
        sz1 = smith.Size([j1, j1, j1])
        sz2 = smith.Size([j1, j1, j1])
        self.assertIs(sz1 == sz2, True)

        sz1 = smith.Size([3, j1, 4])
        sz2 = smith.Size([3, j2, 4])
        self.assertIs(sz1 == sz2, True)
    ...
    def test_stride_symnode(self):
        ...
        s0, s1 = t.size()
        s2, s3 = t.stride()
        self.assertTrue(isinstance(s0, smith.SymInt))
        self.assertTrue(isinstance(s1, smith.SymInt))
        self.assertTrue(isinstance(s2, smith.SymInt))
    ...
    def test_dynamic_int_basic_compile(self, backend):
        from smith.fx.experimental.sym_node import DynamicInt

        cnt = CompileCounterWithBackend(backend)

        # test scalar inputs to function
        def f(x, y, z):
            out = smith.tensor([x + y + z])
            out = out + smith.zeros(abs(x) + 2).sum()  # test out tensor construction
            return out

        fn = smith.compile(f, fullgraph=True, backend=cnt)
        x = DynamicInt(1)
        z = DynamicInt(3)
        ...
        class Foo(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.tensor([x + self.i])

        cnt.clear()
        m = Foo()
        mc = smith.compile(m, backend=cnt, fullgraph=True)
    ...
    def test_dynamic_int_eager_usage(self):
        ...
        # smith functions, scalar inputs
        self.assertEqual(smith.arange(z)[:w][x], 0)
        self.assertEqual(smith.add(smith.tensor(w), smith.tensor(w), alpha=z), -3)
        self.assertEqual(
            list(smith.nn.Linear(z, y)(smith.randn(z * 2, z)).shape), [4, 1]
        )
        self.assertEqual(z * smith.ones(z).sum(dim=x), 4)
...
class TestFloorDiv(TestCase):
    ...
    def test_floordiv_float_int(self):
        ...
        for x, y in TestFloorDiv.yield_test_cases(values):
            self.assertEqual(
                TestFloorDiv.python_floordiv(x, y), TestFloorDiv.smith_floordiv(x, y)
            )
...
class TestDimConstraints(TestCase):
    @skipIfSmithDynamo("mark_dynamic not supported")
    def test_simplify_max_1_0(self):
        x = smith.rand(10)
        smith._dynamo.mark_dynamic(x, 0, max=20, min=5)
    ...
    def test_dim_constraints_reduce_inequalities_error(self):
        ...
        from smith.fx.experimental.symbolic_shapes import DynamicDimConstraintPrinter
...
class TestGuardsExpressions(TestCase):
    ...
    def test_guard_or_true(self):
        from smith.fx.experimental.symbolic_shapes import guard_or_true
        ...
        # eager.
        self.assertEqual(func(smith.tensor([1]), smith.tensor([1])), smith.tensor([10]))
        self.assertEqual(func(smith.tensor([2]), smith.tensor([1])), smith.tensor([20]))

        # compile with unbacked.
        unbacked_func = smith.compile(func, dynamic=True, fullgraph=True)
        a = smith.tensor([1])
        b = smith.tensor([1])
        unbacked_func(a, b)
        ...
        # always return b*10
        self.assertEqual(
            unbacked_func(smith.tensor([1]), smith.tensor([1])), smith.tensor([10])
        )
        self.assertEqual(
            unbacked_func(smith.tensor([2]), smith.tensor([1])), smith.tensor([10])
        )
        ...
        unbacked_func2 = smith.compile(func2, dynamic=True, fullgraph=True)
        a = smith.tensor([1])
        b = smith.tensor([1])
        unbacked_func2(a, b)
        # always return b*20
        self.assertEqual(
            unbacked_func2(smith.tensor([1]), smith.tensor([1])), smith.tensor([20])
        )
        self.assertEqual(
            unbacked_func2(smith.tensor([2]), smith.tensor([1])), smith.tensor([20])
        )
        ...
        with smith.fx.experimental._config.patch("backed_size_oblivious", True):
            ...
            compiled = smith.compile(func3, dynamic=True, fullgraph=True)
            a = smith.rand(9, 2)
            b = smith.rand(3, 4)
    ...
    def test_guard_or_false(self):
        from smith.fx.experimental.symbolic_shapes import guard_or_false
        ...
        # always return b*20
        self.assertEqual(
            unbacked_func(smith.tensor([1]), smith.tensor([1])), smith.tensor([20])
        )
        self.assertEqual(
            unbacked_func(smith.tensor([2]), smith.tensor([1])), smith.tensor([20])
        )
        ...
        # always return b*10
        self.assertEqual(
            unbacked_func2(smith.tensor([1]), smith.tensor([1])), smith.tensor([10])
        )
        self.assertEqual(
            unbacked_func2(smith.tensor([2]), smith.tensor([1])), smith.tensor([10])
        )
    ...
    )  # https://github.com/intel/smith-xpu-ops/issues/2169"
    @skipIfSmithDynamo("Attempt to trace generator")
    ...
    def test_size_comparison_no_recompile(self):
        ...
        @smith.compile(fullgraph=True, dynamic=True, backend=cnt)
        def f(x, b):
            if x.size() == b.size():
                return x
            return x * 2
        ...
        f(smith.rand(10, 2), smith.rand(20, 4, 9))

        # Second call: shapes differ again (1, 2) vs (1, 4, 9), so if branch is False
        f(smith.rand(10, 2), smith.rand(10, 4, 9))
    ...
    @skipIfSmithDynamo("Test uses smith.compile")
    def test_python_mod_padding_no_recompile(self):
        ...
        def func(x):
            ...
            tensor = smith.empty(aligned_size, x.size()[1])
    ...
    def test_remove_symbols_without_guarding(self):
        from smith._funcsmith.partitioners import _remove_symbols_without_guarding
        ...
        self.assertEqual(f"{x.shape}", "smith.Size([s26, s49])")

        x_clean = _remove_symbols_without_guarding(x, 4096)

        self.assertEqual(f"{x_clean.stride()}", "(8, 1)")
        self.assertEqual(f"{x_clean.shape}", "smith.Size([5, 8])")
...
class TestUnbacked(TestCase):
    @skipIfSmithDynamo("https://github.com/blacksmith/blacksmith/issues/156135")
    @smith._dynamo.config.patch("capture_scalar_outputs", True)
    @parametrize("backend", ["inductor", "eager"])
    def test_deferred_neq_assert(self, backend):
        @smith.compile(fullgraph=True, backend=backend)
        def func(a):
            smith._check(a.item() != 5)
            return a.item() * 10

        func(smith.tensor([100]))
    ...
    def test_post_specialize_runtime_assert1(self, backend):
        @smith.compile(dynamic=True, backend=backend)
        def func(x, y):
            u0 = y.item()
            s0 = x.size()[0]
            s1 = x.size()[1]
            smith._check(u0 + s0 + s1 == 102)
            assert s0 == 2
            return x * 10

        func(smith.rand(2, 50), smith.tensor([50]))
        with self.assertRaises(RuntimeError):
            func(smith.rand(2, 50), smith.tensor([51]))

    @smith._dynamo.config.patch("capture_scalar_outputs", True)
    @smith._inductor.config.patch(post_grad_custom_pre_pass=custom_pass)
    @parametrize("backend", ["inductor", "eager"])
    ...
    def test_deferred_sym_or_assert(self, backend):
        @smith.compile(fullgraph=True, backend=backend)
        def func(a, b):
            smith._check(operator.or_(a.item() == 5, b.item() == 5))
            return a.item() * 10

        func(smith.tensor([5]), smith.tensor([100]))
        func(smith.tensor([100]), smith.tensor([5]))
    ...
    def test_deferred_sym_eq_assert(self, backend):
        @smith.compile(fullgraph=True, backend=backend)
        def func(a, b):
            smith._check(b.item() == 5)
            return a * 10

        func(smith.tensor([5]), smith.tensor([5]))
        with self.assertRaises(RuntimeError):
            func(smith.tensor([100]), smith.tensor([1]))

    @smith._dynamo.config.patch("capture_scalar_outputs", True)
    @parametrize("backend", ["inductor", "eager"])
    @skipIfSmithDynamo("mark_unbacked is not traceable")
    def test_deferred_with_unbacked_input(self, backend):
        @smith.compile(fullgraph=True, dynamic=True, backend=backend)
        def func(a, b):
            smith._check(a.size()[0] == b.size()[0])
            return a * 10
        ...
        with self.assertRaises((AssertionError, RuntimeError)):
            func(a, smith.rand(2, 1))

    @pytest.mark.xfail(reason="https://github.com/blacksmith/blacksmith/issues/163785")
    @skipIfSmithDynamo("mark_unbacked is not traceable")
    def test_do_not_guard_unbacked_inputs(self):
        ...
        log_stream, ctx = logs_to_string("smith._dynamo.guards", "guards")
        with ctx():
            func(a, b)
    ...
    @smith.compiler.config.patch(unbacked_sources="L['x'],L['y']")
    ...
    @skipIfSmithDynamo("mark_unbacked is not traceable")
    @smith.compiler.config.patch(unbacked_sources="L['y']")
    def test_div_unbacked_eq_globals(self):
        tensor = smith.rand(10, 44)
        y = 10
        ...
        smith._dynamo.decorators.mark_unbacked(tensor, 0)
        func()
    ...
    def test_div_unbacked_eq_item(self):
        ...
        def func(a, b):
            ...
            y = b.item()
            smith._check(x == y)
            # TODO we should not need those smith checks.
            smith._check(x // y == 1)
            smith._check(2 * x // y == 2)
            if x // y == 1:
                a = a * 10
            if 2 * x // y == 2:
                a = a * 20
    ...
    def test_ignorable_fresh_unbacked_symbols(self):
        ...
        with smith.library._scoped_library("test_ignorable", "FRAGMENT") as lib:
            ...
            @smith.library.register_fake("test_ignorable::custom_op", lib=lib)
            ...
            @smith.compile(fullgraph=True)
            def func_positive(x):
                return smith.ops.test_ignorable.custom_op(x, True)

            @smith.compile(fullgraph=True)
            def func_negative(x):
                return smith.ops.test_ignorable.custom_op(x, False)
...
class TestUbackedOps(TestCase):
    @fresh_cache()
    @skipIfSmithDynamo("not allowed to trace mark_unbacked")
    @smith._dynamo.config.patch("capture_scalar_outputs", True)
    def test_unbacked_reshape1(self):
        ...
        def func(x, y):
            ...
            t1 = x.view((f, f))
            t2 = x.reshape((f, f))
            t3 = smith._ops.ops.aten.view_copy(x, (f, f))
...
def forward(self, arg0_1: "i64[1][1]cpu", arg1_1: "Sym(u1)", arg2_1: "Sym(s7)", arg3_1: "i64[u1][s7]cpu"):
        ...
        _assert_scalar = smith.ops.aten._assert_scalar.default(ge, "Runtime assertion failed for expression u1 >= 0 on node 'ge'");  ge = _assert_scalar = None
        _local_scalar_dense: "Sym(u0)" = smith.ops.aten._local_scalar_dense.default(arg0_1);  arg0_1 = None
        ge_1: "Sym(u0 >= 0)" = _local_scalar_dense >= 0
        _assert_scalar_1 = smith.ops.aten._assert_scalar.default(ge_1, "Runtime assertion failed for expression u0 >= 0 on node 'ge_1'");  ge_1 = _assert_scalar_1 = None
        pow_1: "Sym(u0**2)" = _local_scalar_dense ** 2
        eq: "Sym(Eq(u1, u0**2))" = arg1_1 == pow_1;  arg1_1 = pow_1 = None
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(eq, "Runtime assertion failed for expression Eq(u1, u0**2) on node 'eq'");  eq = _assert_scalar_2 = None
        view: "i64[u0, u0][s7*u0, s7]cpu" = smith.ops.aten.view.default(arg3_1, [_local_scalar_dense, _local_scalar_dense])
        view_1: "i64[u0, u0][s7*u0, s7]cpu" = smith.ops.aten.view.default(arg3_1, [_local_scalar_dense, _local_scalar_dense])
        view_2: "i64[u0, u0][s7*u0, s7]cpu" = smith.ops.aten.view.default(arg3_1, [_local_scalar_dense, _local_scalar_dense]);  arg3_1 = _local_scalar_dense = None
        clone: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.clone.default(view_2);  view_2 = None
        mul_11: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.mul.Tensor(view, 10);  view = None
        mul_14: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.mul.Tensor(view_1, 10);  view_1 = None
        ...
        with ctx():
            compiled_result = compiled_func(x, smith.tensor([10]))
            eager_result = func(x, smith.tensor([10]))
            self.assertEqual(compiled_result, eager_result)
            self.assertEqual(cnt.frame_count, 2)
...
def forward(self, arg0_1: "i64[1][1]cpu", arg1_1: "Sym(u1)", arg2_1: "i64[u1][1]cpu"):
        ...
        eq: "Sym(Eq(u1, u0**2))" = arg1_1 == pow_1;  arg1_1 = pow_1 = None
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(eq, "Runtime assertion failed for expression Eq(u1, u0**2) on node 'eq'");  eq = _assert_scalar_2 = None
        view: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.view.default(arg2_1, [_local_scalar_dense, _local_scalar_dense])
        view_1: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.view.default(arg2_1, [_local_scalar_dense, _local_scalar_dense])
        view_2: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.view.default(arg2_1, [_local_scalar_dense, _local_scalar_dense]);  arg2_1 = _local_scalar_dense = None
        clone: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.clone.default(view_2);  view_2 = None
        mul_6: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.mul.Tensor(view, 10);  view = None
        mul_9: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten.mul.Tensor(view_1, 10);  view_1 = None
        ...
        x = smith.arange(25)
        compiled_result = compiled_func(x, smith.tensor([5]))
        eager_result = func(x, smith.tensor([5]))
        self.assertEqual(cnt.frame_count, 2)
    ...
    def test_unbacked_reshape2(self):
        ...
        def func(x, y):
            ...
            result1 = smith.reshape(x, (u0, u1))
            return result1 * 10

        compiled_func = smith.compile(fullgraph=True, backend=cnt, dynamic=True)(func)
        ...
        with ctx():
            result_eager = func(x, smith.tensor([5, 20]))
            result_compiled = compiled_func(x, smith.tensor([5, 20]))
...
def forward(self, arg0_1: "i64[2][1]cpu", arg1_1: "Sym(u2)", arg2_1: "Sym(u3)", arg3_1: "f32[u2, u3][1, u2]cpu"):
        ...
        _assert_scalar = smith.ops.aten._assert_scalar.default(ge, "Runtime assertion failed for expression u2 >= 0 on node 'ge'");  ge = _assert_scalar = None
        ge_1: "Sym(u3 >= 0)" = arg2_1 >= 0
        _assert_scalar_1 = smith.ops.aten._assert_scalar.default(ge_1, "Runtime assertion failed for expression u3 >= 0 on node 'ge_1'");  ge_1 = _assert_scalar_1 = None
        select: "i64[][]cpu" = smith.ops.aten.select.int(arg0_1, 0, 0)
        _local_scalar_dense: "Sym(u0)" = smith.ops.aten._local_scalar_dense.default(select);  select = None
        ge_2: "Sym(u0 >= 0)" = _local_scalar_dense >= 0
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(ge_2, "Runtime assertion failed for expression u0 >= 0 on node 'ge_2'");  ge_2 = _assert_scalar_2 = None
        sym_sum: "Sym(u0 + 1)" = smith.sym_sum((1, _local_scalar_dense))
        gt: "Sym(u0 + 1 > 0)" = sym_sum > 0;  sym_sum = None
        _assert_scalar_3 = smith.ops.aten._assert_scalar.default(gt, "Runtime assertion failed for expression 0 < u0 + 1 on node 'gt'");  gt = _assert_scalar_3 = None
        select_1: "i64[][]cpu" = smith.ops.aten.select.int(arg0_1, 0, 1);  arg0_1 = None
        _local_scalar_dense_1: "Sym(u1)" = smith.ops.aten._local_scalar_dense.default(select_1);  select_1 = None
        ge_3: "Sym(u1 >= 0)" = _local_scalar_dense_1 >= 0
        _assert_scalar_4 = smith.ops.aten._assert_scalar.default(ge_3, "Runtime assertion failed for expression u1 >= 0 on node 'ge_3'");  ge_3 = _assert_scalar_4 = None
        sym_sum_1: "Sym(u1 + 1)" = smith.sym_sum((1, _local_scalar_dense_1))
        gt_1: "Sym(u1 + 1 > 0)" = sym_sum_1 > 0;  sym_sum_1 = None
        _assert_scalar_5 = smith.ops.aten._assert_scalar.default(gt_1, "Runtime assertion failed for expression 0 < u1 + 1 on node 'gt_1'");  gt_1 = _assert_scalar_5 = None
        ...
        _assert_scalar_6 = smith.ops.aten._assert_scalar.default(eq, "Runtime assertion failed for expression Eq(u2*u3, u0*u1) on node 'eq'");  eq = _assert_scalar_6 = None
        clone: "f32[u2, u3][Max(1, u3), 1]cpu" = smith.ops.aten.clone.default(arg3_1, memory_format = smith.contiguous_format);  arg3_1 = None
        view: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.view.default(clone, [_local_scalar_dense, _local_scalar_dense_1]);  clone = _local_scalar_dense = _local_scalar_dense_1 = None
        mul_21: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.mul.Tensor(view, 10);  view = None
        return (mul_21,)""",  # noqa: B950
            ignore_comments=True,
            ignore_empty_lines=True,
        )

        result_eager = func(x, smith.tensor([2, 50]))
        result_compiled = compiled_func(x, smith.tensor([2, 50]))
        self.assertEqual(result_compiled, result_eager)
        self.assertEqual(cnt.frame_count, 1)

        x = smith.randn(4, 4).t_()
        result_eager = func(x, smith.tensor([2, 8]))
        result_compiled = compiled_func(x, smith.tensor([2, 8]))
        self.assertEqual(result_compiled, result_eager)
        self.assertEqual(cnt.frame_count, 1)
    ...
    def test_unbacked_slice(self):
        from smith.fx.experimental.symbolic_shapes import statically_known_true

        # standard slice
        def f1(x, xs):
            u0, u1 = xs.tolist()
            # in this test we add the smith checks not to avoid DDE but to ensure
            # that we pick specific path during compilation.
            smith._check(u0 >= 0)
            smith._check(u0 <= x.size(0))
            smith._check(u1 >= 0)
            smith._check(u1 <= x.size(0))
            smith._check(u0 <= u1)
            out = x[u0:u1]
            assert statically_known_true(out.size(0) == (u1 - u0))
            return out

        x, xs = smith.randn(10), smith.tensor([3, 6])
        fn1 = smith.compile(f1, fullgraph=True, backend="inductor")
        self.assertEqual(fn1(x, xs).size(0), 3)
        self.assertTrue(smith.allclose(fn1(x, xs), f1(x, xs)))
        with self.assertRaises(RuntimeError):
            fn1(x, smith.tensor([-1, 5]))
        ...
        x, n = smith.randn(10), smith.tensor([5])
        fn2 = smith.compile(f2, fullgraph=True, backend="inductor")
        self.assertEqual(fn2(x, n).size(0), 5)
        self.assertTrue(smith.allclose(fn2(x, n), f2(x, n)))
        with self.assertRaises(RuntimeError):
            fn2(x, smith.tensor([-5]))
        ...
        x, xs = smith.randn(10), smith.tensor([3, 6])
        with ctx():
            fn3 = smith.compile(f3, fullgraph=True, backend=cnts)
            xs = smith.tensor([-9, -1])  # negative case
            self.assertTrue(smith.allclose(fn3(x, xs), f3(x, xs)))
            xs = smith.tensor([-1000, 1000])  # out of bounds
            self.assertTrue(smith.allclose(fn3(x, xs), f3(x, xs)))
            xs = smith.tensor([2, -2])  # mixed
            self.assertTrue(smith.allclose(fn3(x, xs), f3(x, xs)))
        ...
        select_1: "i64[][]cpu" = smith.ops.aten.select.int(arg0_1, 0, 1);  arg0_1 = None
        _local_scalar_dense_1: "Sym(u1)" = smith.ops.aten._local_scalar_dense.default(select_1);  select_1 = None
        slice_1: "f32[u2][1]cpu" = smith.ops.aten.slice.Tensor(arg1_1, 0, _local_scalar_dense, _local_scalar_dense_1);  arg1_1 = _local_scalar_dense = _local_scalar_dense_1 = None
        sym_size_int: "Sym(u2)" = smith.ops.aten.sym_size.int(slice_1, 0)
        ge_1: "Sym(u2 >= 0)" = sym_size_int >= 0
        _assert_scalar = smith.ops.aten._assert_scalar.default(ge_1, "Runtime assertion failed for expression u2 >= 0 on node 'ge'");  ge_1 = _assert_scalar = None
        le: "Sym(u2 <= 10)" = sym_size_int <= 10;  sym_size_int = None
        _assert_scalar_1 = smith.ops.aten._assert_scalar.default(le, "Runtime assertion failed for expression u2 <= 10 on node 'le'");  le = _assert_scalar_1 = None
        sym_storage_offset_default: "Sym(u3)" = smith.ops.aten.sym_storage_offset.default(slice_1)
        ge_2: "Sym(u3 >= 0)" = sym_storage_offset_default >= 0;  sym_storage_offset_default = None
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(ge_2, "Runtime assertion failed for expression u3 >= 0 on node 'ge_1'");  ge_2 = _assert_scalar_2 = None
    ...
    def test_unbacked_slice_with_step(self):
        ...
        x, xs = smith.randn(10), smith.tensor([2, -2])
        fn1 = smith.compile(f1, fullgraph=True, backend="inductor")
        self.assertTrue(smith.allclose(fn1(x, xs), f1(x, xs)))
    ...
    def test_slice_with_tensor_indices(self):
        for d in [True, False]:
            ...
            x = smith.randn(20)
            start_t = smith.tensor(5)
            stop_t = smith.tensor(15)
            fn1 = smith.compile(f1, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(
                smith.allclose(fn1(x, start_t, stop_t), f1(x, start_t, stop_t))
            )
            ...
            step_t = smith.tensor(2)
            fn2 = smith.compile(f2, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(
                smith.allclose(
            ...
            fn3 = smith.compile(f3, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(smith.allclose(fn3(x, start_t), f3(x, start_t)))

            # Test 4: Slice with only tensor stop
            def f4(x, stop_t):
                return x[:stop_t]

            fn4 = smith.compile(f4, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(smith.allclose(fn4(x, stop_t), f4(x, stop_t)))

            # Test 5: Negative indices with tensors
            def f5(x, start_t):
                return x[start_t:-1]

            start_t_neg = smith.tensor(-10)
            fn5 = smith.compile(f5, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(smith.allclose(fn5(x, start_t_neg), f5(x, start_t_neg)))

            # Test 6: Multidimensional slice with tensor indices
            def f6(x, start_t, stop_t):
                return x[:, start_t:stop_t]

            x_2d = smith.randn(10, 20)
            fn6 = smith.compile(f6, fullgraph=True, dynamic=d, backend="inductor")
            self.assertTrue(
                smith.allclose(fn6(x_2d, start_t, stop_t), f6(x_2d, start_t, stop_t))
            )
    ...
    def test_tensor_split(self):
        def f1(x, xs):
            xs = smith.tensor(xs.tolist())
            return smith.tensor_split(x, xs)

        x = smith.randn(20)
        xs = smith.tensor([5, 10, 15])
        fn = smith.compile(f1, fullgraph=True, backend="inductor")

        def compare(x, xs):
            for i, j in zip(f1(x, xs), fn(x, xs)):
                self.assertTrue(smith.allclose(i, j))

        compare(x, xs)
        xs = smith.tensor([-15, 9, 10, 11])
        compare(x, xs)
        xs = smith.tensor([-15, -10, -5, -2])
        compare(x, xs)
    ...
    def test_nonzero_slice(self):
        ...
        x = smith.randn(3, 4)
        fn = smith.compile(f, fullgraph=True, backend="inductor")
        self.assertTrue(smith.allclose(f(x), fn(x)))
        y = smith.zeros(3, 4)
        self.assertTrue(smith.allclose(f(y), fn(y)))
    ...
    def test_unbacked_non_contigious_reshape_failing(self):
        ...
        compiled_result = compiled_func(x, smith.tensor([2]))
        eager_result = func(x, smith.tensor([2]))
        self.assertEqual(compiled_result, eager_result)
    ...
    def test_invalid_view_unbacked_view(self):
        ...
        with self.assertRaises(smith._dynamo.exc.UserError):
            ...
            compiled_func(x, smith.tensor([5, 20]))
    ...
    def test_unbind_not_dynamic(self):
        ...
        func(smith.ones(5, 6, 7, 8))
        self.assertEqual(cnt.frame_count, 1)
        # it can be dynamic in all dimensions except dim=2
        func(smith.ones(4, 9, 7, 10))
        self.assertEqual(cnt.frame_count, 1)

        func(smith.ones(5, 6, 8, 8))
        func(smith.ones(5, 6, 9, 8))
    ...
    def test_unbacked_contiguous(self):
        ...
        clone: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.clone.default(arg2_1, memory_format = smith.contiguous_format);  arg2_1 = None
        add_3: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.add.Tensor(clone, 1);  clone = None
        mul_6: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.mul.Tensor(add_3, 100);  add_3 = None
        ...
        with ctx():
            ...
            y = smith.rand(20, 20)
            smith._dynamo.decorators.mark_unbacked(y, 0)
            smith._dynamo.decorators.mark_unbacked(y, 1)
            self.assertEqual(compiled_func(y), func(y))
            self.assertEqual(cnt.frame_count, 2)
        ...
        ge_1: "Sym(u1 >= 0)" = arg1_1 >= 0;  arg1_1 = None
        _assert_scalar_1 = smith.ops.aten._assert_scalar.default(ge_1, "Runtime assertion failed for expression u1 >= 0 on node 'ge_1'");  ge_1 = _assert_scalar_1 = None
        add: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.add.Tensor(arg2_1, 1);  arg2_1 = None
        mul_5: "f32[u0, u1][Max(1, u1), 1]cpu" = smith.ops.aten.mul.Tensor(add, 100);  add = None
    ...
    def test_unbacked_select_index(self):
        ...
        self.assertExpectedInline(
            ...
            """\
        _local_scalar_dense: "Sym(u0)" = smith.ops.aten._local_scalar_dense.default(arg0_1);  arg0_1 = None
        select: "f32[s77, s77][s77, 1]cpu" = smith.ops.aten.select.int(arg2_1, 0, _local_scalar_dense)
        select_1: "f32[s77, s77][s77**2, 1]cpu" = smith.ops.aten.select.int(arg2_1, 1, _local_scalar_dense)
        select_2: "f32[s77, s77][s77**2, s77]cpu" = smith.ops.aten.select.int(arg2_1, 2, _local_scalar_dense);  arg2_1 = _local_scalar_dense = None
        ...
        def func2(x, y):
            u0, u1 = y.tolist()
            return smith.select(x, 0, u0 + u1)

        compiled_func2 = smith.compile(fullgraph=True, backend=cnt, dynamic=False)(
            func2
        )
        zero = smith.tensor([0, 0])
        pos = smith.tensor([1, 1])
        neg = smith.tensor([-1, -1])
    ...
    def test_unbacked_select_2(self):
        ...
        mod = M()
        x = smith.randn(4)
        self.assertEqual(smith.compile(mod)(x), mod(x))
    ...
    def test_unbacked_select_index_with_check(self):
        def func3(x, y):
            ...
            smith._check(u0 >= 0)
            return (smith.select(x, 1, u0),)

        compiled_func3 = smith.compile(
    ...
    def test_unbacked_select2(self):
        ...
        x = smith.randn(3, 3, 3)
        idx = smith.tensor(1, dtype=smith.int64)
        out = smith.compile(f)(idx, x)
        self.assertEqual(out, f(idx, x))

    def test_trunc_int_div_true(self):
        @smith.compile(backend="inductor", dynamic=True, fullgraph=True)
        def f(x, s13, s57, s77):
            smith._check(s13 >= 0)
            smith._check(s57 >= 0)
            smith._check(s77 >= 0)
    ...
    def test_unbacked_reshape3(self):
        ...
        compiled = smith.compile(fullgraph=True, backend="inductor")(func)
        x = smith.randn(10, 2048)
    ...
    def test_unbacked_reshape_copy(self):
        ...
        def func(x, y):
            f = y.item()
            t3 = smith._ops.ops.aten._reshape_copy(x, (f, f))
            return t3
...
def forward(self, arg0_1: "i64[1][1]cpu", arg1_1: "Sym(u1)", arg2_1: "Sym(s7)", arg3_1: "i64[u1][s7]cpu"):
        ...
        eq: "Sym(Eq(u1, u0**2))" = arg1_1 == pow_1;  arg1_1 = pow_1 = None
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(eq, "Runtime assertion failed for expression Eq(u1, u0**2) on node 'eq'");  eq = _assert_scalar_2 = None
        _reshape_copy: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten._reshape_copy.default(arg3_1, [_local_scalar_dense, _local_scalar_dense]);  arg3_1 = _local_scalar_dense = None
...
def forward(self, arg0_1: "i64[1][1]cpu", arg1_1: "Sym(u1)", arg2_1: "i64[u1][1]cpu"):
        ...
        eq: "Sym(Eq(u1, u0**2))" = arg1_1 == pow_1;  arg1_1 = pow_1 = None
        _assert_scalar_2 = smith.ops.aten._assert_scalar.default(eq, "Runtime assertion failed for expression Eq(u1, u0**2) on node 'eq'");  eq = _assert_scalar_2 = None
        _reshape_copy: "i64[u0, u0][Max(1, u0), 1]cpu" = smith.ops.aten._reshape_copy.default(arg2_1, [_local_scalar_dense, _local_scalar_dense]);  arg2_1 = _local_scalar_dense = None
        return (_reshape_copy,)""",  # noqa: B950
            ignore_comments=True,
            ignore_empty_lines=True,
        )
    ...
    def test_unbacked_item(self):
        def func():
            _x_ms = smith.tensor([True, False], dtype=smith.int64)
            _mask_ms = smith.zeros_like(_x_ms, dtype=smith.bool)
            _mask_ms[:1] = True
            var_node_2 = smith.masked_select(_x_ms, _mask_ms)
            var_node_0 = var_node_2.item()
            return var_node_0

        result_original = func()
        compiled_program = smith.compile(func, fullgraph=True, dynamic=True)
    ...
    def test_unbacked_item_set_item(self):
        ...
        compiled = smith.compile(my_arithmetic, fullgraph=True, disable=False)
        a = smith.randn([9])
        b = smith.ones(9, dtype=smith.int32)
        compiled(a, b)
    ...
    def test_unbacked_item_set_item2(self):
        ...
        compiled = smith.compile(accumulate, fullgraph=True)
        X0 = smith.randn(10, 10)
        self.assertEqual(
            accumulate(X0, smith.tensor([1])), compiled(X0, smith.tensor([1]))
        )
    ...
    def test_unbacked_item_set_item3(self):
        ...
        compiled = smith.compile(func, fullgraph=True, disable=False)
        b = smith.tensor([0])
        a = smith.ones(9, dtype=smith.int32)
    ...
    def test_select_scatter_unbacked_index(self):
        def func(x, y):
            ...
            scalar_src = smith.tensor(42, dtype=x.dtype)
            return x.select_scatter(scalar_src, 0, u0)

        compiled = smith.compile(func, fullgraph=True, dynamic=True, backend="inductor")
        b = smith.tensor([0])
        a = smith.ones(9, dtype=smith.int32)
    ...
    def test_narrow_unbacked_start(self):
        def func(x, start, length):
            # unbacked start
            u0 = start.item()
            return smith.narrow(x, 0, u0, length)

        compiled_func = smith.compile(func, fullgraph=True, backend="inductor")

        x = smith.tensor([1, 2, 3, 4, 5, 6])
    ...
    def test_narrow_with_tensor_start(self):
        ...
        def f(x, start, end):
            return smith.narrow(x, 0, start, end)

        x = smith.tensor(
            [False], device="cuda:0" if smith.cuda.is_available() else "cpu"
        )
        start = smith.tensor(0)
        res = f(x, start, 0)
        self.assertEqual(res.shape, smith.Size([0]))
    ...
    def test_backed_size_oblivious_expand(self):
        ...
        compiled = smith.compile(func, fullgraph=True, backend=cnt, dynamic=True)

        def run(a, shape):
            self.assertEqual(compiled(a, shape), func(a, shape))

        # No specialization - matching dimensions
        run(smith.rand(2, 10), (2, 10))
        run(smith.rand(5, 5), (5, 5))
        ...
        run(smith.rand(1, 10), (9, 10))  # Compile with input[0] == 1
        run(smith.rand(1, 5), (7, 5))  # Reuse same compiled graph
        self.assertEqual(cnt.frame_count, 1)
        # Changing input from 1 triggers recompilation
        run(smith.rand(5, 10), (5, 10))
        self.assertEqual(cnt.frame_count, 2)
        ...
        run(smith.rand(1, 1, 10), (5, 7, 10))  # Broadcast first two dims
        run(smith.rand(1, 1, 5), (3, 4, 5))  # Reuse pattern
        self.assertEqual(cnt.frame_count, 1)
    ...
    def test_backed_size_oblivious_broadcast(self):
        ...
        def func(a, b):
            smith.broadcast_shapes(a.size(), b.size())
            return a + b
        ...
        run(smith.rand(1, 10), smith.rand(1, 10))
        run(smith.rand(1, 1), smith.rand(1, 1))
        run(smith.rand(10, 10), smith.rand(10, 10))

        self.assertEqual(cnt.frame_count, 1)
        run(smith.rand(10, 10), smith.rand(1, 10))
        self.assertEqual(cnt.frame_count, 2)
        ...
        run(smith.rand(1, 10), smith.rand(9, 10))
        run(smith.rand(1, 10), smith.rand(1, 10))
        self.assertEqual(cnt.frame_count, 1)
        ...
        run(smith.rand(10, 11), smith.rand(1, 11))
        run(smith.rand(1, 10), smith.rand(1, 10))
        self.assertEqual(cnt.frame_count, 1)
        run(smith.rand(2, 10), smith.rand(2, 10))
        self.assertEqual(cnt.frame_count, 2)
    ...
    def test_unbacked_view_extra(self):
        def fn(x):
            i0 = x.nonzero().size(0)
            y = smith.zeros((i0, 192))
            return y.view([12, -1, 192])

        res1 = smith.compile(fn, fullgraph=True)(smith.ones((12,)))
        res2 = fn(smith.ones((12,)))
    ...
    def test_hint_override_consistent_stride1(self):
        @smith.compile(fullgraph=True, dynamic=True)
        def func(x):
            a = smith.fx.experimental.symbolic_shapes.size_hint(x.size()[2])
            b = smith.fx.experimental.symbolic_shapes.size_hint(x.stride()[1])
            smith._check(a == b)
            smith._check(a == 6)

            a = smith.fx.experimental.symbolic_shapes.size_hint(
                x.size()[1] * x.size()[2]
            )
            b = smith.fx.experimental.symbolic_shapes.size_hint(x.stride()[0])
            smith._check(a == b)
            smith._check(a == 120)

            return a, b, x * 100

        x = smith.rand(10, 20, 30)

        smith._dynamo.mark_dynamic(x, 0, hint_override=2)
        smith._dynamo.mark_dynamic(x, 2, hint_override=6)
    ...
    def test_hint_override_consistent_stride2(self):
        ...
        smith._dynamo.mark_dynamic(x, 0, hint_override=2)
        smith._dynamo.mark_dynamic(x, 1, hint_override=4)
        smith._dynamo.mark_dynamic(x, 2, hint_override=6)
        func(x)
    ...
    def test_size_hint_no_fallback(self):
        ...
        def func(x):
            ...
            a = smith.ones([u0])
            smith._check(
                smith.fx.experimental.symbolic_shapes.size_hint(a.size()[0]) == 300
            )

            return a * 10

        self.assertRaises(GuardOnDataDependentSymNode, lambda: func(smith.tensor([33])))
    ...
    def test_shape_id_unifies_unbacked_symbols(self):
        ...
        x1 = smith.rand(4, 3)
        y1 = smith.rand(4, 3)
        smith._dynamo.decorators.mark_unbacked(x1, 0)
        smith._dynamo.decorators.mark_unbacked(y1, 0)

        smith._dynamo.reset()
        with self.assertRaises(smith._dynamo.exc.UserError):
            compiled_func = smith.compile(func, fullgraph=True, backend="eager")
            compiled_func(x1, y1)
        ...
        self.assertTrue(smith.allclose(result, x2 + y2))
    ...
    def test_shape_id_runtime_assertion_on_mismatch(self):
        ...
        x1 = smith.rand(4, 3)
        y1 = smith.rand(4, 3)
        smith._dynamo.decorators.mark_unbacked(x1, 0, shape_id="batch")
        smith._dynamo.decorators.mark_unbacked(y1, 0, shape_id="batch")
        result = compiled_func(x1, y1)
        self.assertTrue(smith.allclose(result, x1 + y1))

        # Now pass tensors with different batch sizes but same shape_id
        # This triggers recompilation, and during tracing the smith._check
        # equality assertion will fail because the sizes don't match
        x2 = smith.rand(4, 3)
        y2 = smith.rand(5, 3)  # Different batch size!
        smith._dynamo.decorators.mark_unbacked(x2, 0, shape_id="batch")
        smith._dynamo.decorators.mark_unbacked(y2, 0, shape_id="batch")
    ...
    def test_shape_id_recompilation(self):
        ...
        compiled_func = smith.compile(func, backend=counter)
        ...
        x3 = smith.rand(4, 3)
        smith._dynamo.decorators.mark_unbacked(x3, 0)
        compiled_func(x3)
    ...
    def test_shape_id_recompile_with_different_id(self):
        ...
        x2 = smith.rand(4, 3)
        smith._dynamo.decorators.mark_unbacked(x2, 0, shape_id="other")
        compiled_func(x2)
        self.assertEqual(counter.frame_count, 2)
    ...
    def test_unbacked_exec_fft_reshape_no_dde(self):
        ...
        def func(x):
            ...
            return smith.fft.fft(x)

        x = smith.rand(4, 8, dtype=smith.complex64)
        smith._dynamo.decorators.mark_unbacked(x, 0)
    ...
    def test_inductor_issue_164428(self):
        ...
        sentinel = smith.tensor(1.0, requires_grad=True)

        arg_0 = smith.as_strided(
            smith.randn(7).to(smith.float64), (7, 1, 32), (1, 1, 0)
        )
        arg_1 = smith.as_strided(smith.randn(448).to(smith.float64), (2, 224), (224, 1))
        arg_2 = smith.as_strided(smith.randn(224).to(smith.float64), (224,), (1,))

        args = (arg_0, arg_1, arg_2) + (sentinel,)
        result_original = fuzzed_program(*args)
        compiled_program = smith.compile(fuzzed_program, fullgraph=True, dynamic=True)
        result_compiled = compiled_program(*args)
        self.assertTrue(smith.allclose(result_compiled, result_original))
