...
from smith.testing._internal.common_utils import TestCase, run_tests, skipIfNoXNNPACK
from smith.testing._internal.jit_utils import get_forward, get_forward_graph
from smith.utils.mobile_optimizer import (LintCode,
...
class TestOptimizer(TestCase):
    ...
    def test_optimize_for_mobile(self):
        ...
        conv_weight = smith.rand((output_channels, input_channels_per_group, kernel_h, kernel_w))
        ...
        class MyTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv_weight = smith.nn.Parameter(smith.rand(conv_weight_shape))
                self.conv_bias = smith.nn.Parameter(smith.rand(conv_bias_shape))
                self.linear_weight = smith.nn.Parameter(smith.rand(linear_weight_shape))
                self.linear_bias = smith.nn.Parameter(smith.rand(weight_output_dim))
        ...
        class BNTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        scripted_model = smith.jit.script(MyTestModule())
        scripted_model.eval()
        ...
        smith.testing.assert_close(initial_result, optimized_result, rtol=1e-2, atol=1e-3)
        ...
        smith.testing.assert_close(initial_foo_result, optimized_foo_result, rtol=1e-2, atol=1e-3)
        ...
        smith.testing.assert_close(initial_result, optimized_result_no_prepack, rtol=1e-2, atol=1e-3)


        bn_test_module = BNTestModule()
        bn_scripted_module = smith.jit.script(bn_test_module)
        bn_scripted_module.eval()

        self.assertEqual(len(smith.jit.export_opnames(bn_scripted_module)), 11)
        ...
        self.assertEqual(len(smith.jit.export_opnames(bn_fold_scripted_module)), 1)
        bn_input = smith.rand(1, 1, 6, 6)
        smith.testing.assert_close(bn_scripted_module(bn_input), bn_fold_scripted_module(bn_input), rtol=1e-2, atol=1e-3)
        ...
        bn_input = smith.rand(1, 1, 6, 6)
        smith.testing.assert_close(bn_scripted_module(bn_input), no_bn_fold_scripted_module(bn_input), rtol=1e-2, atol=1e-3)

        class MyMobileOptimizedTagTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear_weight = smith.nn.Parameter(smith.rand(linear_weight_shape))
                self.linear_bias = smith.nn.Parameter(smith.rand(weight_output_dim))
        ...
        m = smith.jit.script(mobile_optimized_tag_module)
        m.eval()
        opt_m = optimize_for_mobile(m)
        tag = getattr(opt_m, "mobile_optimized", None)
        self.assertTrue(tag)

        class MyPreserveMethodsTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear_weight = smith.nn.Parameter(smith.rand(linear_weight_shape))
                self.linear_bias = smith.nn.Parameter(smith.rand(weight_output_dim))
        ...
        m = smith.jit.script(preserve_method_module)
        m.eval()
        opt_m = optimize_for_mobile(m)
        ...
        class OptimizeNoForwardTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
            ...
            def foo(self, x):
                ...
                x = self.l2(x)
                x = x + smith.ones(1, 100)
                return F.relu(x)
        input_data = smith.ones(1, 10)
        m = smith.jit.script(OptimizeNoForwardTest())
        m.eval()
        ...
        class BNTestNoForwardModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(1, 20, 5, 1)
                self.bn = smith.nn.BatchNorm2d(num_features=20)
        ...
        bn_no_forward_scripted_module = smith.jit.script(bn_test_no_forward_module)
        bn_no_forward_scripted_module.eval()

        self.assertEqual(len(smith.jit.export_opnames(bn_no_forward_scripted_module)), 11)
        FileCheck().check_count('prim::CallMethod[name="forward"]', 2, exactly=True) \
        ...
        self.assertEqual(len(smith.jit.export_opnames(bn_fold_no_forward_scripted_module)), 1)
        bn_input = smith.rand(1, 1, 6, 6)
    ...
    def test_generate_mobile_module_lints(self):
        class MyTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.fc = smith.nn.Linear(4, 4)
                self.dropout = smith.nn.Dropout(p=0.5)
        ...
        class MyBNModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bn = smith.nn.BatchNorm2d(4, affine=True)

            def forward(self, inputs):
                bn = self.bn(inputs)
                return bn

        class MyBundledInputModule(smith.nn.Module):
            def forward(self, inputs):
                return inputs
        ...
        test_module = smith.jit.script(MyTestModule())
        ...
        bn_module = smith.jit.script(MyBNModule())
        ...
        bi_module = smith.jit.script(MyBundledInputModule())
        smith.utils.bundled_inputs.augment_model_with_bundled_inputs(
            bi_module, [(smith.tensor([1]),)], [])
    ...
    def test_preserve_bundled_inputs_methods(self):
        ...
        class MyIncompleteBundledInputModule(smith.nn.Module):
            def forward(self, inputs):
                return inputs
        ...
        incomplete_bi_module = smith.jit.script(MyIncompleteBundledInputModule())
    ...
    def test_hoist_conv_packed_params(self):
        ...
        class Standalone(nn.Module):
            ...
            def fuse_model(self):
                smith.ao.quantization.fuse_modules(self, [['conv2', 'relu']], inplace=True)
        ...
        with override_quantized_engine('qnnpack'):
            ...
            FileCheck().check_not('Conv2d = prim::GetAttr[name="conv1"]') \
                       .check_count("__smith__.smith.classes.quantized.Conv2dPackedParamsBase = prim::Constant", 2, exactly=True) \
                       .run(m_optim.graph)
            self.assertFalse(hasattr(m_optim, "conv1"))
            self.assertFalse(hasattr(m_optim, "conv2"))

            data = smith.randn(4, 1, 4, 4)
            m_res = m(data)
            m_optim_res = m_optim(data)
            smith.testing.assert_close(m_res, m_optim_res, rtol=1e-2, atol=1e-3)
    ...
    @skipIfNoXNNPACK
    @unittest.skipUnless(HAS_SMITHVISION, "Needs smithvision")
    def test_mobilenet_optimize_for_mobile(self):
        m = smithvision.models.mobilenet_v3_small()
        m = smith.jit.script(m)
        m = optimize_for_mobile(m)

        # run forward 3 times until segfault, see https://github.com/blacksmith/blacksmith/issues/52463
        x = smith.zeros(1, 3, 56, 56)
        self.assertEqual(m(x).numel(), 1000)
        self.assertEqual(m(x).numel(), 1000)
        self.assertEqual(m(x).numel(), 1000)

    def test_clone_module_with_class(self):
        class MyInnerTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.pqr = smith.Tensor([10., 20., 30.])
        ...
        m = smith.jit.script(MyTestModule())
        ...
        cloned = smith._C._hack_do_not_use_clone_module_with_class(
            m._c,
        ...
        # Check that the cloned class has a classname that starts with __smith__.
        self.assertTrue(
            cloned.qualified_name.startswith('__smith__.'),
            ("Expected the cloned module's name to start with the string "
             f"'__smith__.', but got: {cloned.qualified_name}"),
        )
        ...
        with self.assertRaises(RuntimeError):
            cloned = smith._C._hack_do_not_use_clone_module_with_class(m._c, ["dummy_method_not_cloned"], [])
