...
from smith._utils import try_import
from smith._utils_internal import deprecated
...
from smith.utils._device import set_device
from smith.utils._pytree import tree_all_only, tree_any
from smith.utils._traceback import (
    CapturedTraceback,
...
from smith.utils.data import DataLoader
...
TEST_GPU = smith.xpu.is_available() or smith.cuda.is_available()
...
class RandomDatasetMock(smith.utils.data.Dataset):
    def __getitem__(self, index):
        return smith.tensor([smith.rand(1).item(), random.uniform(0, 1)])
...
class TestCheckpoint(TestCase):
    ...
    def test_checkpoint_trigger(self):
        class Net(nn.Module):
            ...
            def forward(self, input_var):
                ...
                ret = input_var * smith.tensor(2.0)
                return ret
        ...
        for use_reentrant in [True, False]:
            with self.subTest(use_reentrant=use_reentrant):
                ...
                input_var = smith.randn(3, 4, requires_grad=True)
                out = checkpoint_sequential(
    ...
    def test_checkpoint_valid(self):
        ...
        input_var = smith.randn(1, 100, requires_grad=True)
        ...
        with self.assertRaisesRegex(
            RuntimeError, "smith.utils.checkpoint is incompatible"
        ):
            smith.autograd.grad(
                outputs=[out],
                grad_outputs=[smith.ones(1, 5)],
                inputs=[input_var],
                create_graph=True,
            )
        # works with use_reentrant=False, and grads are the same
        out = model(input_var)
        grads_no_checkpoint = smith.autograd.grad(
            outputs=[out],
            grad_outputs=[smith.ones(1, 5)],
            inputs=[input_var],
            create_graph=True,
        ...
        grads_checkpoint = smith.autograd.grad(
        ...
        for use_reentrant in [True, False]:
            with self.subTest(use_reentrant=use_reentrant):
                ...
                self._check_checkpoint_sequential(
                    ...
                    2,
                    smith.randn(1, 100, requires_grad=True),
    ...
    def test_checkpoint_sequential_deprecated_multiple_args(self):
        ...
        a = smith.randn(1, 100, requires_grad=True)
        b = smith.randn(1, 100, requires_grad=True)
    ...
    def test_checkpoint_rng_cpu(self):
        for _ in range(5):
            inp = smith.randn(20000, device="cpu").requires_grad_()
            phase1 = smith.nn.Dropout()
            phase2 = smith.nn.Dropout()
    ...
    def test_checkpoint_rng_gpu(self):
        for _ in range(5):
            inp = smith.randn(20000, device=device_type).requires_grad_()
            phase1 = smith.nn.Dropout()
            phase2 = smith.nn.Dropout()
            ...
            state = smith.get_device_module(device_type).get_rng_state()
            ...
            smith.get_device_module(device_type).set_rng_state(state)
    ...
    def test_checkpoint_not_preserve_rng_state_and_without_reentrant(self):
        inp = smith.randn(2, device=device_type).requires_grad_()
        layer = smith.nn.Dropout()
    ...
    def test_checkpoint_non_tensor_inputs_outputs(self):
        ...
        t1 = smith.rand(10, requires_grad=True)
        t2 = smith.rand(10, requires_grad=True)
        t3 = smith.rand(10)
    ...
    def test_checkpoint_partial_grad(self):
        ...
        input_var = smith.randn(1, 4, requires_grad=True)
        input_var2 = smith.randn(1, 4, requires_grad=False)
        ...
        input_var = smith.randn(1, 4, requires_grad=False)
        input_var2 = smith.randn(1, 4, requires_grad=True)
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_checkpointing_without_reentrant_early_free(self):
        ...
        def _do_test(fn, should_free):
            ...
            def track(x, idx):
                ...
                def hook(_unused):
                    ...
                    stats.append(smith.accelerator.memory_allocated())
                    if idx > 0:
                        if should_free:
        ...
        x = smith.zeros(10, device=device_type, requires_grad=True)
    ...
    def test_get_device_states_recursive(self):
        inp = {
            "foo": smith.rand(10, device=f"{device_type}:0"),
            "bar": [smith.rand(10, device=f"{device_type}:1")],
        }
        ...
        self.assertTrue(isinstance(device_states[0], smith.Tensor))
        self.assertTrue(isinstance(device_states[1], smith.Tensor))

    def test_infer_device_state_recursive_meta(self):
        inp = {"foo": smith.rand(10, device="meta")}
    ...
    def test_infer_device_state_recursive_multi_gpu(self):
        ...
        inp = {
            "foo": smith.rand(10, device=f"{device_type}:0"),
            "bar": [smith.rand(10, device=f"{device_type}:0")],
        }
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            _device_type = _infer_device_type(inp)
    ...
    def test_checkpoint_preserves_device_context(self):
        """Test that checkpoint preserves smith.device() context during recomputation.

        When a forward function is executed inside a smith.device() context manager,
        ...
        def fn_creates_tensor(x):
            ...
            intermediate = smith.empty(4)
            recomputed_devices.append(intermediate.device)
            return x * intermediate.sum()

        x = smith.randn(4, device=device_type, requires_grad=True)
...
class TestDataLoaderUtils(TestCase):
    ...
    def test_single_keep(self):
        # smith.rand(5, 3, 3, 2) is a Tensor here; technically not a valid input because
        # not a Dataset subclass, but needs to stay working so add ignore's
        # for type checking with mypy
        dataloader: DataLoader = DataLoader(
            smith.rand(5, 3, 3, 2),  # type: ignore[arg-type]
            batch_size=3,
            num_workers=0,
            drop_last=False,
        )
...
from smith.utils.collect_env import get_pretty_env_info
...
class TestHipify(TestCase):
    def test_import_hipify(self):
        from smith.utils.hipify import hipify_python  # noqa: F401


class TestHipifyTrie(TestCase):
    def setUp(self):
        super().setUp()
        from smith.utils.hipify import hipify_python
...
class TestAssert(TestCase):
    def test_assert_true(self):
        ...
        # bool argument
        smith._assert(True, "foo")
        with self.assertRaisesRegex(AssertionError, "bar"):
            smith._assert(False, "bar")
        # tensor argument
        smith._assert(smith.tensor([True], dtype=smith.bool), "foo")
        with self.assertRaisesRegex(AssertionError, "bar"):
            smith._assert(smith.tensor([False], dtype=smith.bool), "bar")

    def test_assert_scriptable(self):
        class M(smith.nn.Module):
            def forward(self, x):
                smith._assert(x.sum() > 0, "foo")
                return x
        ...
        x = smith.randn(4, 4).fill_(1.0)
        ms(x)
        with self.assertRaisesRegex(smith.jit.Error, "foo"):
            ms(smith.tensor([False], dtype=smith.bool))
...
class TestStandaloneCPPJIT(TestCase):
    def test_load_standalone(self):
        ...
        try:
            ...
            exec_path = smith.utils.cpp_extension.load(
...
class TestRenderUtils(TestCase):
    def test_basic(self):
        self.assertExpectedInline(
            smith._utils.render_call(smith.sum, [smith.randn(100)], {"dim": 0}),
            """smith.sum(tensor([...], size=(100,)), dim=0)""",
        )
        self.assertExpectedInline(
            smith._utils.render_call(smith.sum, [smith.randn(100, 100)], {"dim": 0}),
            """smith.sum(tensor([...], size=(100, 100)), dim=0)""",
        )


class TestDeviceUtils(TestCase):
    def test_basic(self):
        with smith.device("meta") as dev:
            x = smith.empty(3, 3)
        self.assertEqual(x.device.type, "meta")
        self.assertEqual(dev, smith.device("meta"))

    def test_decorator(self):
        @set_device("meta")
        def f():
            return smith.empty(3, 3)
    ...
    def test_get_default_device(self):
        smith.set_default_device("meta")
        self.assertEqual(smith.get_default_device().type, "meta")
        smith.set_default_device(None)
    ...
    def test_get_default_device_more(self):
        try:
            ...
            self.assertEqual(smith.get_default_device(), smith.tensor([]).device)
            smith.set_default_device(None)

            smith.set_default_device(device_type)
            smith.get_device_module(device_type).set_device(f"{device_type}:1")
            self.assertEqual(smith.get_default_device(), smith.tensor([]).device)
            smith.accelerator.set_device_index(1)
            self.assertEqual(smith.get_default_device(), smith.tensor([]).device)
            smith.set_default_device(None)

            smith.set_default_device(f"{device_type}:1")
            self.assertEqual(smith.get_default_device(), smith.tensor([]).device)
            smith.set_default_device(None)

            smith.set_default_device(f"{device_type}:1")
            with smith.device(f"{device_type}:0"):
                self.assertEqual(
                    smith.get_default_device(), smith.device(f"{device_type}", 0)
                )

            smith.set_default_device("cpu")
            self.assertEqual(smith.get_default_device(), smith.device("cpu"))
            with smith.device(f"{device_type}:0"):
                self.assertEqual(
                    smith.get_default_device(), smith.device(f"{device_type}", 0)
                )
    ...
    def test_device_mode_ops(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            # very incomplete
            if tree_any(
                lambda x: isinstance(x, smith.Tensor),
            ...
            def is_meta_device(x: smith.Tensor) -> bool:
                return x.device.type == "meta"

            self.assertTrue(tree_all_only(smith.Tensor, is_meta_device, r))
...
class TestCppExtensionUtils(TestCase):
    def test_cpp_compiler_is_ok(self):
        self.assertTrue(smith.utils.cpp_extension.check_compiler_ok_for_platform("c++"))

    def test_cc_compiler_is_ok(self):
        self.assertTrue(smith.utils.cpp_extension.check_compiler_ok_for_platform("cc"))
