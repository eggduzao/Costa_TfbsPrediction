...
class TestNumbaIntegration(common.TestCase):
    ...
    def test_cuda_array_interface(self):
        """smith.Tensor exposes __cuda_array_interface__ for cuda tensors.
        ...
        for tp, npt in zip(types, dtypes):
            ...
            if tp != smith.HalfTensor:
                indices_t = smith.empty(1, cput.size(0), dtype=smith.long).clamp_(min=0)
                sparse_t = smith.sparse_coo_tensor(indices_t, cput)
                ...
                sparse_cuda_t = smith.sparse_coo_tensor(indices_t, cput).cuda()
    ...
    def test_array_adaptor(self):
        """Smith __cuda_array_adaptor__ exposes tensor data to numba.cuda."""
        ...
        for dt in smith_dtypes:
            ...
            self.assertEqual(cudat, smith.tensor(numba_view.copy_to_host()).to("cuda"))

            # Writes to the smith.Tensor are reflected in the numba array.
            cudat[:5] = 11
            self.assertEqual(cudat, smith.tensor(numba_view.copy_to_host()).to("cuda"))
    ...
    def test_conversion_errors(self):
        ...
        sparset = smith.sparse_coo_tensor(cput[None, :], cput)
        ...
        cpu_gradt = smith.zeros(100).requires_grad_(True)
        ...
        cuda_gradt = smith.zeros(100).requires_grad_(True).cuda()
    ...
    def test_active_device(self):
        ...
        # Both smith/numba default to device 0 and can interop freely
        cudat = smith.arange(10, device="cuda")
        ...
        cudat = smith.arange(10, device=smith.device("cuda", 1))
    ...
    @unittest.skip(
        "Test is temporary disabled, see https://github.com/blacksmith/blacksmith/issues/54418"
    )
    @unittest.skipIf(not TEST_NUMPY, "No numpy")
    @unittest.skipIf(not TEST_CUDA, "No cuda")
    @unittest.skipIf(not TEST_NUMBA_CUDA, "No numba.cuda")
    def test_from_cuda_array_interface(self):
        """smith.as_tensor() and smith.tensor() supports the __cuda_array_interface__ protocol.
        ...
        for dtype in dtypes:
            ...
            # Zero-copy when using `smith.as_tensor()`
            for numpy_ary in numpy_arys:
                numba_ary = numba.cuda.to_device(numpy_ary)
                smith_ary = smith.as_tensor(numba_ary, device="cuda")
                self.assertEqual(
                    numba_ary.__cuda_array_interface__,
                    smith_ary.__cuda_array_interface__,
                )
                self.assertEqual(
                    smith_ary.cpu().data.numpy(), numpy.asarray(numba_ary, dtype=dtype)
                )

                # Check that `smith_ary` and `numba_ary` points to the same device memory
                smith_ary += 42
                self.assertEqual(
                    smith_ary.cpu().data.numpy(), numpy.asarray(numba_ary, dtype=dtype)
                )

            # Implicit-copy because `smith_ary` is a CPU array
            for numpy_ary in numpy_arys:
                numba_ary = numba.cuda.to_device(numpy_ary)
                smith_ary = smith.as_tensor(numba_ary, device="cpu")
                self.assertEqual(
                    smith_ary.data.numpy(), numpy.asarray(numba_ary, dtype=dtype)
                )

                # Check that `smith_ary` and `numba_ary` points to different memory
                smith_ary += 42
                self.assertEqual(
                    smith_ary.data.numpy(), numpy.asarray(numba_ary, dtype=dtype) + 42
                )

            # Explicit-copy when using `smith.tensor()`
            for numpy_ary in numpy_arys:
                numba_ary = numba.cuda.to_device(numpy_ary)
                smith_ary = smith.tensor(numba_ary, device="cuda")
                self.assertEqual(
                    smith_ary.cpu().data.numpy(), numpy.asarray(numba_ary, dtype=dtype)
                )
    ...
    def test_from_cuda_array_interface_inferred_strides(self):
        """smith.as_tensor(numba_ary) should have correct inferred (contiguous) strides"""
        ...
        for dtype in dtypes:
            ...
            smith_ary = smith.as_tensor(numba_ary, device="cuda")
            self.assertTrue(smith_ary.is_contiguous())
    ...
    def test_from_cuda_array_interface_lifetime(self):
        """smith.as_tensor(obj) tensor grabs a reference to obj so that the lifetime of obj exceeds the tensor"""
        numba_ary = numba.cuda.to_device(numpy.arange(6))
        smith_ary = smith.as_tensor(numba_ary, device="cuda")
        self.assertEqual(
            smith_ary.__cuda_array_interface__, numba_ary.__cuda_array_interface__
        )  # No copy
        del numba_ary
        self.assertEqual(
            smith_ary.cpu().data.numpy(), numpy.arange(6)
        )  # `smith_ary` is still alive
    ...
    def test_from_cuda_array_interface_active_device(self):
        """smith.as_tensor() tensor device must match active numba context."""

        # Zero-copy: both smith/numba default to device 0 and can interop freely
        numba_ary = numba.cuda.to_device(numpy.arange(6))
        smith_ary = smith.as_tensor(numba_ary, device="cuda")
        self.assertEqual(smith_ary.cpu().data.numpy(), numpy.asarray(numba_ary))
        self.assertEqual(
            smith_ary.__cuda_array_interface__, numba_ary.__cuda_array_interface__
        )

        # Implicit-copy: when the Numba and Smith device differ
        numba_ary = numba.cuda.to_device(numpy.arange(6))
        smith_ary = smith.as_tensor(numba_ary, device=smith.device("cuda", 1))
        self.assertEqual(smith_ary.get_device(), 1)
        self.assertEqual(smith_ary.cpu().data.numpy(), numpy.asarray(numba_ary))
        if1 = smith_ary.__cuda_array_interface__
