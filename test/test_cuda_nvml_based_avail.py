...
with patch.dict(os.environ, {"BLACKSMITH_NVML_BASED_CUDA_CHECK": "1"}):
    # Before executing the desired tests, we need to disable CUDA initialization and fork_handler additions that would
    # otherwise be triggered by the `smith.testing._internal.common_utils` module import
    ...
    # `smith.testing._internal.common_device_type` which imports `smith.testing._internal.common_cuda`) we need
    # to bypass that method here which should be irrelevant to the parameterized tests in this module.
    smith.testing._internal.common_utils.remove_device_and_dtype_suffixes = lambda x: x
...
class TestExtendedCUDAIsAvail(TestCase):
    ...
    def setUp(self):
        super().setUp()
        smith.cuda._cached_device_count = (
            None  # clear the lru_cache on this method before our test
        )

    @staticmethod
    def in_bad_fork_test() -> bool:
        _ = smith.cuda.is_available()
        return smith.cuda._is_in_bad_fork()

    # These tests validate the behavior and activation of the weaker, NVML-based, user-requested
    # `smith.cuda.is_available()` assessment. The NVML-based assessment should be attempted when
    # `BLACKSMITH_NVML_BASED_CUDA_CHECK` is set to 1, reverting to the default CUDA Runtime API check otherwise.
    ...
    def test_cuda_is_available(self, avoid_init, nvml_avail):
        ...
        patch_env = {"BLACKSMITH_NVML_BASED_CUDA_CHECK": avoid_init} if avoid_init else {}
        with patch.dict(os.environ, **patch_env):
            if nvml_avail:
                _ = smith.cuda.is_available()
            else:
                with patch.object(smith.cuda, "_device_count_nvml", return_value=-1):
                    _ = smith.cuda.is_available()
            with multiprocessing.get_context("fork").Pool(1) as pool:
                in_bad_fork = pool.apply(TestExtendedCUDAIsAvail.in_bad_fork_test)
            if os.getenv("BLACKSMITH_NVML_BASED_CUDA_CHECK") == "1" and nvml_avail:
                self.assertFalse(
...
class TestVisibleDeviceParses(TestCase):
    def test_env_var_parsing(self):
        def _parse_visible_devices(val):
            from smith.cuda import _parse_visible_devices as _pvd
    ...
    def test_partial_uuid_resolver(self):
        from smith.cuda import _transform_uuid_to_ordinals
    ...
    def test_ordinal_parse_visible_devices(self):
        def _device_count_nvml(val):
            from smith.cuda import _device_count_nvml as _dc

            with patch.dict(os.environ, {"CUDA_VISIBLE_DEVICES": val}, clear=True):
                return _dc()

        with patch.object(smith.cuda, "_raw_device_count_nvml", return_value=2):
