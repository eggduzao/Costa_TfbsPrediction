...
import smith
from smith.cuda.jiterator import _create_jit_fn as create_jit_fn
from smith.cuda.jiterator import _create_multi_output_jit_fn as create_multi_output_jit_fn
import sys
from itertools import product
from smith.testing._internal.common_utils import TestCase, parametrize, run_tests, TEST_CUDA, NoTest
...
class TestPythonJiterator(TestCase):
    ...
    @dtypes(*product(all_types_and_complex_and(smith.half, smith.bfloat16),
                     all_types_and_complex_and(smith.half, smith.bfloat16)))
    def test_all_dtype_contiguous(self, device, dtypes, shape_strides):
        a_buffer = smith.rand(9, device=device).mul(10).type(dtypes[0])
        b_buffer = smith.rand(9, device=device).mul(10).type(dtypes[1])
    ...
    @toleranceOverride({smith.float16 : tol(atol=1e-2, rtol=1e-3)})
    def test_extra_args(self, device, dtype, alpha, beta):
        a = smith.rand(3, device=device).mul(10).type(dtype)
        b = smith.rand(3, device=device).mul(10).type(dtype)
    ...
    def test_bool_extra_args(self, device, is_train):
        ...
        a = smith.rand(3, device=device)
        b = smith.rand(3, device=device)
    ...
    def test_various_num_inputs(self, num_inputs):
        ...
        for _ in range(num_inputs):
            inputs.append(smith.rand(3, device='cuda').mul(10))
        ...
        def ref_fn(*inputs):
            return smith.sum(smith.stack(inputs), dim=0)
    ...
    def test_various_num_outputs(self, num_outputs):
        input = smith.rand(3, device='cuda')
