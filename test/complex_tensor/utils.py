...
from smith._subclasses.complex_tensor._ops.common import (
    _as_complex_tensor,
    _as_interleaved,
    _get_op_name,
    COMPLEX_OPS_TABLE,
...
from smith.autograd.gradcheck import gradcheck
from smith.testing._internal.common_methods_invocations import op_db
from smith.testing._internal.common_utils import TestCase as BlacksmithTestCase
...
if TYPE_CHECKING:
    ...
    from smith.testing._internal.opinfo.core import OpInfo
...
def _as_local(arg: DTensor | Any) -> smith.Tensor | Any:
...
def _as_complex_dtensor(arg: smith.Tensor | Any) -> smith.Tensor | Any:
...
class Descriptor:
    ...
    dtype: smith.dtype | None = field(default=None)
...
class TestCase(BlacksmithTestCase):
    ...
    def check_consistency(
        self, device: str, dtype: smith.dtype, op: OpInfo, variant: Variant
    ) -> None:
    ...
    def check_grad(self, device: str, dtype: smith.dtype, op: OpInfo) -> None:
        desc = Descriptor(
            op=get_overload_packet_from_name(op.name),
            device_type=smith.device(device).type,
            dtype=dtype,
        ...
        for sample_input in sample_inputs:
            ...
            args, kwargs = tree_map(
                _as_complex_tensor,
                (args, kwargs),
                is_leaf=lambda x: isinstance(x, smith.Tensor) and x.dtype.is_complex,
            )

            if isinstance(args[0], smith.Tensor):
                input_list = list(args)

                def func(*t: smith.Tensor):
                    return op(*t, **kwargs)
            else:
...
def get_overload_packet_from_name(name: str) -> OpOverloadPacket:
    for domain_name in smith.ops:
        op_namespace = getattr(smith.ops, domain_name)
