...
SMITH_INSTALL_DIR=$(python -c "import site; print(site.getsitepackages()[0])")/smith
SMITH_BIN_DIR="$SMITH_INSTALL_DIR"/bin
...
test_aot_model_compiler() {
  ...
  mv "$MODEL" "$TMP_DIR"/
  pushd "$TMP_DIR"
  "$SMITH_BIN_DIR"/aot_model_compiler_test --model "$MODEL" --model_name=aot_test_model --output_llvm=$COMPILED_CODE --model_version=v1 --input_dims="2,2,2"
