...
#         increment = smith.ones([2, 4], dtype=smith.float64)
#         return self.x + y + increment

# output_model_path = Path(tmpdirname, "script_module_v5.ptl")
# script_module = smith.jit.script(TestModule(1))
...
(4,
 ('__smith__.*.TestModule.forward',
  (('instructions',
   ...
   ('constants',
    (smith._utils._rebuild_tensor_v2(pers.obj(('storage', smith.DoubleStorage, '0', 'cpu', 8),),
       0,
       (2, 4),
       (4, 1),
       False,
...
(5,
 ('__smith__.*.TestModule.forward',
  (('instructions',
   ...
   ('operators', (('aten::add', 'int'), ('aten::add', 'Scalar'))),
   ('constants',
    (smith._utils._rebuild_tensor_v2(pers.obj(('storage', smith.DoubleStorage, 'constants/0', 'cpu', 8),),
       0,
       (2, 4),
       (4, 1),
       False,
...
class testVariousModelVersions(TestCase):
    def test_get_model_bytecode_version(self):
        ...
        for version, model_info in SCRIPT_MODULE_BYTECODE_PKL.items():
            model_path = blacksmith_test_dir / "cpp" / "jit" / model_info["model_name"]
    ...
    def test_bytecode_values_for_all_backport_functions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            while current_from_version > MINIMUM_TO_VERSION:
                ...
                input_model_path = blacksmith_test_dir / "cpp" / "jit" / model_name
    ...
    #     module_input = 1
    #     expected_mobile_module_result = 3 * smith.ones([2, 4], dtype=smith.float64)
    ...
    #             smith.testing.assert_close(mobile_module_result, expected_mobile_module_result)
    ...
    def test_backport_bytecode_from_file_to_file(self):
        ...
        if maximum_checked_in_model_version > MINIMUM_TO_VERSION:
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                module_input = 1
                mobile_module_result = mobile_module(module_input)
                expected_mobile_module_result = 3 * smith.ones(
    ...
    def test_get_model_ops_and_info(self):
        ...
        script_module_v6 = blacksmith_test_dir / "cpp" / "jit" / "script_module_v6.ptl"
