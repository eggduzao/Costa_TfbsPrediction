...
from smith.ao.quantization import default_qconfig, float_qparams_weight_only_qconfig

# graph mode quantization based on fx
from smith.ao.quantization.quantize_fx import convert_fx, prepare_fx
...
class TestLiteFuseFx(QuantizationLiteTestCase):
    ...
    def test_embedding(self):
        ...
        model = M().eval()
        indices = smith.randint(low=0, high=10, size=(20,))
        ...
        for qconfig, _ in configs:
            ...
            m = prepare_fx(
                model,
                qconfig_dict,
                example_inputs=smith.randint(low=0, high=10, size=(20,)),
            )
            m = convert_fx(m)
    ...
    def test_submodule(self):
        ...
        for config in configs:
            ...
            qconfig_dict = {
                "": smith.ao.quantization.get_default_qconfig("qnnpack"),
                **config,
            }
            model = prepare_fx(
                model,
