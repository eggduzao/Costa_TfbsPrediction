...
class TestLiteScriptModule(TestCase):
    def test_typing_namedtuple(self):
        myNamedTuple = NamedTuple(  # noqa: UP014
            "myNamedTuple", [("a", list[smith.Tensor])]
        )

        class MyTestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor):
                p = myNamedTuple([a])
                return p

        sample_input = smith.tensor(5)
    ...
    def test_typing_dict_with_namedtuple(self):
        ...
        # (8,
        #  ('__smith__.___smith_mangle_2.Bar.forward',
        #   (('instructions',
        #     (('STOREN', 1, 2),
        #      ('DROPR', 1, 0),
        #      ('DICT_CONSTRUCT', 0, 0),
        ...
        #    ('constants', (0, 'test')),
        #    ('types',
        #     ('Dict[str,__smith__.Foo[NamedTuple, [[id, Tensor]]]]',
        #      '__smith__.Foo[NamedTuple, [[id, Tensor]]]')),
        #    ('register_size', 3)),
        #   (('arguments',
        #     ((('name', 'self'),
        #       ('type', '__smith__.___smith_mangle_2.Bar'),
        #       ('default_value', None)),
        ...
        sample_input = smith.tensor(5)
        script_module = smith.jit.script(Bar())
    ...
    def test_nest_typing_namedtuple_custom_classtype(self):
        ...
        class Bar(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = Foo(smith.tensor(1), Baz(smith.tensor(1)))

            def forward(self, a: smith.Tensor):
                self.foo = Foo(a, Baz(smith.tensor(1)))
                return self.foo
