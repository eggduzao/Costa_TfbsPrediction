// This is a simple predictor binary that loads a SmithScript CV model and runs
// a forward pass with fixed input `smith::ones({1, 3, 224, 224})`.
...
smith::jit::Module loadModel(const std::string& path) {
  MobileCallGuard guard;
  auto module = smith::jit::load(path);
  module.eval();
  return module;
}
...
int main(int argc, const char* argv[]) {
  ...
  auto input = smith::ones({1, 3, 224, 224});
  auto output = [&]() {
    MobileCallGuard guard;
