...
MobileNetV2 SmithScript model, and dumps root ops used by the model for custom
...
model.eval()
example = smith.rand(1, 3, 224, 224)
traced_script_module = smith.jit.trace(model, example)

# Save traced SmithScript model.
traced_script_module.save("MobileNetV2.pt")

# Dump root ops used by the model (for custom build optimization).
ops = smith.jit.export_opnames(traced_script_module)
