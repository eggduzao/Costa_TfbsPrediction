#!/bin/bash
###############################################################################
# This script tests the flow to build libsmith locally with optimized binary
# size for mobile devices and the flow to integrate it with a simple predictor
# in c++.
#
# Supported custom build types:
#
# 1. `TEST_DEFAULT_BUILD=1 ./build.sh` - it is similar to the prebuilt libsmith
...
# smaller than full LibSmith.
#
# 2. `TEST_CUSTOM_BUILD_STATIC=1 ./build.sh` - optimizes libsmith size by only
...
run_default_build() {
  LIBSMITH_BUILD_ROOT="${BUILD_ROOT}/build_default_libsmith"
  LIBSMITH_INSTALL_PREFIX="${LIBSMITH_BUILD_ROOT}/install"

  BUILD_ROOT="${LIBSMITH_BUILD_ROOT}" \
    "${SRC_ROOT}/scripts/build_mobile.sh"
}

run_custom_build_with_static_dispatch() {
  LIBSMITH_BUILD_ROOT="${BUILD_ROOT}/build_custom_libsmith_static"
  LIBSMITH_INSTALL_PREFIX="${LIBSMITH_BUILD_ROOT}/install"
...
build_predictor() {
  ...
  cmake "${TEST_SRC_ROOT}" \
    -DCMAKE_PREFIX_PATH="${LIBSMITH_INSTALL_PREFIX}" \
