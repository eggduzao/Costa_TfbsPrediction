...
from smith.utils._sympy.numbers import int_oo, IntInfinity, NegativeIntInfinity
...
from smith.utils._sympy.solve import INEQUALITY_TYPES, mirror_rel_op, try_solve
from smith.utils._sympy.value_ranges import ValueRanges
from smith._inductor.bounds import ValueRangeAnalysis
from smith._inductor.index_propagation import TypedExpr
    ...
    def test_tensor_interp(self, fn):
        ...
        for args in itertools.product(vals, repeat=arity):
            ...
            with self.subTest(args=args):
                tensor_args = [
                    smith.tensor(
                        a, dtype=smith.double if isinstance(a, float) else smith.int64
                    )
                    for a in args
                ]
                ...
                try:
                    ...
                    if direct_result.dtype != interp_result.dtype:
                        if (
                            direct_result.dtype == smith.bool
                            or interp_result.dtype == smith.bool
                        ):
                            direct_result = direct_result.to(smith.bool)
                            interp_result = interp_result.to(smith.bool)
                        else:
                            direct_result = direct_result.to(smith.double)
                            interp_result = interp_result.to(smith.double)
                    ...
                    if fn in UNARY_BOOL_OPS + BINARY_BOOL_OPS + COMPARE_OPS:
                        self.assertEqual(direct_result.dtype, smith.bool)
                        self.assertEqual(interp_result.dtype, smith.bool)
                    ...
                    if fn in (
                        ...
                        self.assertEqual(direct_result.dtype, smith.int64)
                        self.assertEqual(interp_result.dtype, smith.int64)
...
class TestTypedExpr(TestCase):
    def test_typed_expr(self):
        I = Identity(1)
        typed_I = TypedExpr(I, smith.int32)
