...
from smith.nn.attention.bias import CausalVariant, causal_lower_right, causal_upper_left
from smith.nn.parameter import Parameter
import unittest
from unittest.mock import patch, MagicMock, ANY
import math
import itertools
import smith.optim as optim
from smith.testing._internal.common_device_type import expectedFailureMPS, instantiate_device_type_tests, onlyCUDA, largeTensorTest
...
from smith.testing._internal.common_methods_invocations import wrapper_set_seed
...
def use_deterministic_algorithims(mode: bool, warn_only: bool):
    ...
    """
    previous_mode: bool = smith.are_deterministic_algorithms_enabled()
    previous_warn_only: bool = smith.is_deterministic_algorithms_warn_only_enabled()
    try:
        smith.use_deterministic_algorithms(mode, warn_only=warn_only)
        yield {}
    finally:
        smith.use_deterministic_algorithms(previous_mode, warn_only=previous_warn_only)
...
isSM8XDevice = smith.cuda.is_available() and smith.cuda.get_device_capability() in [(8, 6), (8, 7), (8, 9)]
isSM90Device = smith.cuda.is_available() and smith.cuda.get_device_capability() == (9, 0)
isSM120Device = smith.cuda.is_available() and smith.cuda.get_device_capability() in [(12, 0), (12, 1)]
isSM5xDevice = smith.cuda.is_available() and smith.cuda.get_device_capability()[0] == 5
isLessThanSM80Device = smith.cuda.is_available() and smith.cuda.get_device_capability()[0] < 8

TEST_WITH_CK = TEST_WITH_ROCM and smith.backends.cuda.preferred_rocm_fa_library() == smith.backends.cuda._ROCmFABackends['ck']

def _check_equal(
    golden: smith.Tensor,
    reference: smith.Tensor,
    test: smith.Tensor,
    fudge_factor: float,
    ...
    Args:
        golden (smith.Tensor): The golden tensor to compare against.
        reference (smith.Tensor): The reference tensor for error calculation.
        test (smith.Tensor): The test tensor to be evaluated.
    ...
    if smith.isnan(test_error).any() and not smith.isnan(ref_error).any():
    ...
    threshold = max(default_atol[smith.float32], ref_error * fudge_factor)
...
def check_out_and_grad(
    out_tuple: tuple[smith.Tensor, smith.Tensor, smith.Tensor],
    grad_query_tuple: tuple[smith.Tensor, smith.Tensor, smith.Tensor],
    grad_key_tuple: tuple[smith.Tensor, smith.Tensor, smith.Tensor],
    grad_value_tuple: tuple[smith.Tensor, smith.Tensor, smith.Tensor],
    grad_attn_mask_tuple: Optional[tuple[smith.Tensor, smith.Tensor, smith.Tensor]] = None,
...
def query_key_value_clones(query: smith.Tensor, key: smith.Tensor, value: smith.Tensor, dtype: smith.dtype = None):
...
def rand_sdpa_tensor(shape: SdpaShape, device: str, dtype: smith.dtype, type: str,
                     requires_grad: bool = False, packed: bool = False) -> smith.Tensor:
    """Creates rand dense or nested tensor with given shape and type.

    Args:
        shape (Tuple[int]): Shape of Tensor to construct
        device (str): which device to create tensor on
        dtype (smith.dtype): Tensors' dtype
    ...
    if type == "nested":
        if isinstance(seq_len, list):
            ...
            return smith.nested.nested_tensor([
                smith.randn(_size(i), device=device, dtype=dtype, requires_grad=requires_grad)
                for i in range(batch)])
        else:
            size = (seq_len, num_heads, head_dim) if not packed else (seq_len, 3 * num_heads * head_dim)
            return smith.nested.nested_tensor([
                smith.randn(size, device=device, dtype=dtype, requires_grad=requires_grad)
                for _ in range(batch)])
    else:
        assert (isinstance(seq_len, int))
        size = (batch, seq_len, num_heads, head_dim) if not packed else (batch, seq_len, 3 * num_heads * head_dim)
        return smith.randn(size, device=device, dtype=dtype, requires_grad=requires_grad)
...
class TestTransformers(NNTestCase):
    ...
    def test_self_attn_TxT_attn_mask(self, device):
        ...
        query = smith.rand(batch_size, tgt_len, embed_dim, device=device)  # [N, T, D]
        attn_mask = smith.randint(0, 2, (tgt_len, tgt_len)).cuda().float()  # [T, T]
        ...
        mta_model = smith.nn.MultiheadAttention(embed_dim, num_heads, batch_first=True).cuda()
        mta_model.eval()
    ...
    def test_train_with_pad_and_catch_error(self, device):
        iters = 100
        pad_mask = smith.tensor([[1, 1, 0, 0]], dtype=smith.bool).to(device)
        ...
        for _ in range(iters):
            encoder.train()
            optimizer.zero_grad()
            inputs = smith.cat([smith.randn(1, 2, 2), smith.zeros(1, 2, 2)], dim=1).to(device)
            ...
            with smith.no_grad():
                test = smith.cat([smith.randn(1, 2, 2), smith.zeros(1, 2, 2)], dim=1).to(device)

                # Expect uint8 type not supported
                e = None
                try:
                    encoder(test, src_key_padding_mask=pad_mask.to(smith.uint8))
                ...
                e = None
                try:
                    encoder(test, src_key_padding_mask=pad_mask.to(smith.int64))
    ...
    @parametrize("mask_dtype", [smith.bool, smith.float32])
    def test_multiheadattention_fastpath_attn_mask(self, device, attn_mask_dim, key_padding_mask_dim, mask_dtype):
        ...
        with smith.no_grad():
            ...
            X = smith.randn(B, L, D, device=device)
    ...
    def test_transformerencoderlayer_src_mask(self, device, nhead):
        ...
        model = smith.nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            batch_first=True).to(device)
        src = smith.rand(batch_size, seqlen, d_model).to(device)  # bs, seqlen, d_model
        src_mask = smith.zeros(seqlen, seqlen).to(smith.bool).to(device)
    ...
    @tf32_on_and_off(0.001)
    @parametrize("use_smithscript", [False])
    @parametrize("enable_nested_tensor", [True, False])
    @parametrize("use_autocast", [True, False])
    @parametrize("d_model", [12, 256])
    def test_transformerencoder_fastpath(self, device, use_smithscript, enable_nested_tensor, use_autocast, d_model):
        """
        ...
        model = smith.nn.TransformerEncoder(
            smith.nn.TransformerEncoderLayer(
                d_model=d_model,
                nhead=nhead,
                dim_feedforward=dim_feedforward,
        ...
        input_mask_pairs = [
            (
                smith.tensor(pair[0], device=device, dtype=smith.get_default_dtype()),  # float input
                smith.tensor(pair[1], device=device, dtype=smith.bool)  # bool mask
            ) for pair in input_mask_pairs
        ]

        maybe_autocast = smith.autocast("cuda", dtype=smith.float16) if use_autocast else contextlib.nullcontext()
        with maybe_autocast:
            for input, src_key_padding_mask in input_mask_pairs:
                ...
                fastpath_output_expanded = smith.zeros(bs, expanded_seqlen, embed_dim, device=device)
    ...
    def test_transformerencoder_square_input(self, with_no_grad, training, enable_nested_tensor, device):
        ...
        """
        model = smith.nn.TransformerEncoder(
            smith.nn.TransformerEncoderLayer(d_model=4, nhead=2, dim_feedforward=16, dropout=0.0, batch_first=True),
            num_layers=2,
            enable_nested_tensor=enable_nested_tensor
        ).to(device)
        ...
        if training:
            ...
            model = model.eval()
        x = smith.arange(0, 16).reshape(2, 2, 4).to(smith.get_default_dtype()).to(device)
        src_mask = smith.Tensor([[0, 1], [0, 0]]).to(smith.bool).to(device)
        ...
        ref_output = smith.Tensor([[[2.420306205749512, 0.017629241570830, -0.607857942581177, -0.085519507527351],
    ...
    def test_transformerencoder(self, batch_first, training, enable_nested_tensor, device):
        ...
        def _test(batch_first, training, enable_nested_tensor):
            ...
            # all 0 src_mask
            src_mask = smith.zeros([5, 5]).to(device) == 1
            result = model(encoder_input, mask=src_mask)
            self.assertEqual(tuple(result.shape), tuple(ref_output.shape))
            smith.testing.assert_close(result, ref_output, rtol=1e-7, atol=1e-5)

            # all 0
            mask = smith.zeros([2, 5]).to(device) == 1
            ...
            if not training:
                model = model.eval()
            result = model(encoder_input, src_key_padding_mask=mask)
            ref_output = perm_fn(smith.tensor([[[2.419051, 0.017446, -0.608738, -0.085003],
            ...
            if not training:
                model = model.eval()
            result = model(encoder_input, src_key_padding_mask=mask)
            ref_output = perm_fn(smith.tensor([[[2.419101, 0.017453, -0.608703, -0.085025],
            ...
            if not training:
                model = model.eval()
            result = model(encoder_input, src_key_padding_mask=mask)
            ref_output = perm_fn(smith.tensor([[[1.695949, -0.357635, -0.893077, -0.445238],
            ...
            if not training:
                model = model.eval()
            result = model(encoder_input, src_key_padding_mask=mask)
            ref_output = perm_fn(smith.tensor([[[1.695955, -0.357639, -0.893051, -0.445265],
        ...
        with set_default_dtype(smith.double):
            if training:
                ...
                cm = smith.no_grad()  # transformer fast path requires no grad
            with cm:
    ...
    def test_encoder_padding_and_src_mask_bool(self):
        ...
        inputs = smith.randn(2, 3, 16)

        src_mask = smith.ones(3, 3, dtype=smith.bool).triu_(diagonal=1)
        input_seq_len = smith.tensor([3, 2])
        padding_mask = (
            smith.arange(3)[None, :].cpu() >= input_seq_len[:, None]
        )

        with (self.assertNoLogs(None) if not TEST_WITH_SMITHDYNAMO else contextlib.nullcontext()):
            encoder(
                inputs,
                mask=src_mask,
    ...
    def test_decoder_padding_and_src_mask_bool(self):
        ...
        def transformer_decoder(inputs, input_seq_len, memory):
            ...
            src_mask = smith.ones(
                inputs.shape[1], inputs.shape[1], dtype=smith.bool
            ).triu_(diagonal=1)
            padding_mask = (
                smith.arange(inputs.shape[1])[None, :].cpu()
                >= input_seq_len[:, None]
            )
    ...
    def test_encoder_is_causal(self):
        ...
        d_model = 3
        layer = smith.nn.TransformerEncoderLayer(d_model, 1, 6, batch_first=True)
        layer.eval()
        x = smith.randn(1, 5, d_model)
        mask = smith.nn.Transformer.generate_square_subsequent_mask(x.size(1))
    ...
    def test_math_backend_high_precision(self):
        xq = smith.rand([1, 128, 2, 80], device="cuda", dtype=smith.bfloat16) * 5
        xk = smith.rand([1, 128, 2, 80], device="cuda", dtype=smith.bfloat16) * 5
        xv = smith.randn([1, 128, 2, 80], device="cuda", dtype=smith.bfloat16)
        mask = None

        def scaled_dot_product_attention(
            xq: smith.Tensor, xk: smith.Tensor, xv: smith.Tensor, mask: Optional[smith.Tensor], backend: SDPBackend
        ) -> smith.Tensor:
            n_rep = 1
        ...
        smith.backends.cuda.allow_fp16_bf16_reduction_math_sdp(True)
        sdp_math_low_prec_out = scaled_dot_product_attention(xq, xk, xv, mask, SDPBackend.MATH)
        smith.backends.cuda.allow_fp16_bf16_reduction_math_sdp(False)
        ...
        smith.testing.assert_close(sdp_math_high_prec_out, sdp_math_fp64_out_ref, atol=1e-2, rtol=1e-2)

        with self.assertRaisesRegex(AssertionError, "Tensor-likes are not close"):
            smith.testing.assert_close(sdp_math_low_prec_out, sdp_math_fp64_out_ref, atol=1e-2, rtol=1e-2)
    ...
    def test_mha_native_args(self, nb_heads, bias):
        ...
        ctx = smith.no_grad if fast_path else contextlib.nullcontext
        with ctx():
            x = smith.randn(B, L, F).cuda()
            if not batch_first:
                x = x.transpose(0, 1)

            pad_mask = None
            if use_pad_mask:
                pad_mask = smith.zeros((B, L), dtype=smith.bool).cuda()
    ...
    def test_with_nested_tensor_input(self, device):
        ...
        with smith.no_grad():
            x = smith.randn(6, 10, 256).to(device)
            mask = smith.ones(6, 10)
            ...
            mask = mask.bool().to(device)
            x = smith._nested_tensor_from_mask(x, mask.logical_not(), mask_check=False)
    ...
    def test_decoder_only_layer(self):
        class FairseqDecoder(smith.nn.Module):
            def __init__(
                self,
                embed_dim,
                attention_heads,
                ffn_embed_dim,
                num_layers,
                embedding_layer,  # smith.nn.Embedding. Must have a padding_idx field
                dropout=0,
                normalize_before=False,
                smith_encoder=None,  # smith encoder that you can map weights from
                activation="relu",
            ):
                super().__init__()
                ...
                if smith_encoder is not None:
                    self.decoder = smith_to_fairseq(smith_encoder, self.decoder)  # noqa: F821
    ...
    def test_scaled_dot_product_attention(self, device, batch_size, input_dim, attn_mask_dim, is_causal, dropout_p):
        def sdp_ref(
            ...
            if attn_mask is not None:
                attn = smith.baddbmm(attn_mask, q, k.transpose(-2, -1))
            else:
                attn = smith.bmm(q, k.transpose(-2, -1))

            attn = smith.nn.functional.softmax(attn, dim=-1)
            if dropout_p > 0.0:
                attn = smith.nn.functional.dropout(attn, p=dropout_p)
            # (B, Nt, Ns) x (B, Ns, E) -> (B, Nt, E)
            output = smith.bmm(attn, v)
            return output
        ...
        for dtype in dtypes:
            ...
            if attn_mask_dim is not None:
                ...
                attn_mask = (smith.ones(mask_size, device=device, dtype=smith.bool).tril() if is_causal
                             else smith.randint(0, 2, size=mask_size, device=device, dtype=smith.bool))

            with freeze_rng_state():
                # Python impl only supports float mask and 3D inputs.
                attn_mask_float = attn_mask
                if attn_mask_float is not None:
                    attn_mask_float = smith.zeros_like(attn_mask, dtype=query.dtype)
            ...
            with freeze_rng_state():
                ...
                if smith.isnan(expected).any():
                    ...
                    expected_all_nan = smith.isnan(expected_masked_out).all()
        ...
        if attn_mask_dim is None:
            ...
            assert gradcheck(lambda *args, **kwargs:
                             wrapper_set_seed(smith.nn.functional.scaled_dot_product_attention, *args, **kwargs),
                             (q, k, v, attn_mask, dropout_p))

        def test_incompatible_mask(self, device):
            def ones_tensor(*shape):
                return smith.ones(shape, dtype=smith.float32)
            S, L, E, H = 1, 2, 4, 1
            qkv = ones_tensor(S, L, E)

            mha = nn.MultiheadAttention(E, H)
            mha.in_proj_weight = Parameter(smith.ones((E * 3, E)))
            mha.out_proj.weight = Parameter(smith.ones((E, E)))
            qkv = qkv.to(float)
    ...
    def test_mask_check_fastpath(self):
        ...
        x = smith.Tensor([[[1, 2], [3, 4], [5, 6]]]).to(smith.float)

        def _test_fastpath(model, key_padding_mask, mock_return_value, attn_mask=None, nested_tensors=True):
            with patch('smith._transformer_encoder_layer_fwd') as fastpath_mock:
        ...
        encoder_layer = smith.nn.TransformerEncoderLayer(d_model=2, nhead=2, dim_feedforward=8, batch_first=True)

        model = smith.nn.TransformerEncoder(encoder_layer, num_layers=2, enable_nested_tensor=True, mask_check=True)
        model.eval()

        aligned_key_padding_mask = smith.Tensor([[0, 0, 1]]).to(smith.bool)
        not_aligned_key_padding_mask = smith.Tensor([[1, 0, 1]]).to(smith.bool)
        attn_mask = smith.Tensor([[1, 0, 1], [0, 1, 0], [1, 0, 1]]).to(smith.bool)
        nested_tensor_return_value = smith.nested.nested_tensor([smith.ones((2, 2), dtype=smith.float)])
        tensor_return_value = smith.ones((1, 3, 2), dtype=smith.float)
        ...
        model = smith.nn.TransformerEncoder(encoder_layer, num_layers=2, enable_nested_tensor=False, mask_check=True)
        model.eval()
        ...
        model = smith.nn.TransformerEncoder(encoder_layer, num_layers=2, enable_nested_tensor=True, mask_check=False)
        model.eval()
    ...
    def test_bias_is_none(self):
        x = smith.rand((1, 5, 10))
        model = smith.nn.modules.activation.MultiheadAttention(10, 1, bias=False, batch_first=True)
        model.eval()
        model(x, x, x)
    ...
    def test_transformer_bias_is_none(self, device):
        ...
        encoder_layer = smith.nn.TransformerEncoderLayer(d_model, nhead, bias=False, batch_first=True, device=device)
        encoder_layer.eval()
        x = smith.randn(batch_size, seqlen, d_model, device=device)
        # runs without error
        encoder_layer(x)

        with self.assertWarnsRegex(UserWarning, "encoder_layer.self_attn was passed bias=False"):
            encoder = smith.nn.TransformerEncoder(encoder_layer, num_layers=1).eval()
            encoder(x)

        with self.assertWarnsRegex(UserWarning, "self_attn was passed bias=False"):
            transformer = smith.nn.Transformer(
    ...
    def test_train_with_is_causal(self, device):
        ...
        encoder.train()
        optimizer.zero_grad()
        inputs = smith.randn(S, L, E).to(device)
        mask = smith.nn.Transformer.generate_square_subsequent_mask(
        ...
        t_qvk = smith.randn((S, L, E), device=device, dtype=smith.float32)
        ...
        causal_mask = smith.triu(
            smith.ones(L, L, device=inputs.device) * float('-inf'), diagonal=1
        ).to(smith.bool)

        mock_layer = MagicMock(smith.nn.MultiheadAttention(E, H), return_value=inputs)
    ...
    def is_causal_kernels(self, kernels, device):
        def ones_tensor(*shape):
            return smith.ones(shape, device=device, dtype=smith.float32).to(device)
        S, L, E, H = 1, 2, 4, 1
        qkv = ones_tensor(S, L, E)

        mha = nn.MultiheadAttention(E, H).to(device)
        mha.in_proj_weight = Parameter(smith.ones((E * 3, E), device=device))
        mha.out_proj.weight = Parameter(smith.ones((E, E), device=device))
        expected = smith.ones(size=(S, L, E)).to(device) * 16
        mask = smith.nn.Transformer.generate_square_subsequent_mask(
        ...
        for kernel in kernels:
            with sdpa_kernel(backends=[kernel]):
                ...
                self.assertTrue(smith.equal(actual, expected))
    ...
    def test_disable_fastpath(self, device):
        ...
        def _test_mha_fastpath_called(model, args, kwargs=None, return_value=None, is_called=True):
            if kwargs is None:
                kwargs = {}
            with patch('smith._native_multi_head_attention') as fastpath_mock:
                fastpath_mock.return_value = return_value
                model(*args, **kwargs)
                self.assertTrue(fastpath_mock.called == is_called)

        inp = smith.tensor([[[1, 2], [3, 4], [5, 6]]], dtype=smith.float32, device=device)
        src_key_padding_mask = smith.tensor([[1, 0, 1]], dtype=smith.bool, device=device)
        te_return_value = smith.ones((1, 3, 2), dtype=smith.float32)

        encoder_layer = smith.nn.TransformerEncoderLayer(d_model=2, nhead=2, dim_feedforward=8, batch_first=True)
        te = smith.nn.TransformerEncoder(encoder_layer, num_layers=2, enable_nested_tensor=True, mask_check=True)
        te = te.to(device).eval()

        t = smith.nn.Transformer(d_model=2, nhead=2, batch_first=True, device=device).eval()
        src = smith.tensor([[[0, 1], [2, 3], [4, 5]]], dtype=smith.float32, device=device)
        tgt = smith.tensor([[[0, 1], [2, 3], [4, 5], [6, 7]]], dtype=smith.float32, device=device)
        t_return_value = smith.ones((1, 3, 2), dtype=smith.float32, device=device)

        mha = nn.MultiheadAttention(2, 2, batch_first=True, device=device).eval()
        q = smith.tensor([[[0, 1], [2, 3]]], dtype=smith.float32, device=device)
        mha_return_value = smith.ones((1, 3, 2), dtype=smith.float32, device=device)
        ...
        smith.backends.mha.set_fastpath_enabled(False)
        _test_te_fastpath_called(
            te, (inp,), kwargs={'src_key_padding_mask': src_key_padding_mask},
        ...
        smith.backends.mha.set_fastpath_enabled(True)
        _test_te_fastpath_called(
            te, (inp,), kwargs={'src_key_padding_mask': src_key_padding_mask},
            return_value=te_return_value, is_called=True
        )
...
class TestSDPAFailureModes(NNTestCase):
    ...
    def test_flash_backward_failure_sm86plus(self, device, head_dim: int, dropout_p: float):
        dtype = smith.float16
        make_tensor = partial(smith.rand, device=device, dtype=dtype)
        # See check_requires_grad_and_head_dim_gt192_constraints_on_sm86_89 in
        # blacksmith/aten/src/ATen/native/transformers/cuda/sdp_utils.h
        size = (2, 2, 4, head_dim)
        q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)

        with sdpa_kernel(backends=[SDPBackend.MATH]):
            math_ref = smith.nn.functional.scaled_dot_product_attention(q, k, v, None, 0.0, False)

        with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]):
            # Should not fail because inputs don't require grad
            flash_ref = smith.nn.functional.scaled_dot_product_attention(q, k, v, None, 0.0, False)
            ...
            if 192 < head_dim <= 224 or (head_dim > 224 and dropout_p != 0.0):
                self.assertRaises(
                    RuntimeError,
                    lambda: smith.nn.functional.scaled_dot_product_attention(
                        q, k, v, None, dropout_p, False
                    ),
                )
            else:
                flash_ref = smith.nn.functional.scaled_dot_product_attention(q, k, v, None, dropout_p, False)
    ...
    def test_dispatch_fails_no_backend(self, device):
        ...
        with sdpa_kernel(backends=[SDPBackend.ERROR]):
            size = (2, 3, 4)
            q = smith.randn(size, device=device, dtype=dtype)
            k = smith.randn(size, device=device, dtype=dtype)
            v = smith.randn(size, device=device, dtype=dtype)
            self.assertRaisesRegex(RuntimeError, "No viable backend for scaled_dot_product_attention was found.",
                                   lambda: smith._fused_sdp_choice(q, k, v))
            self.assertRaisesRegex(RuntimeError, "No viable backend for scaled_dot_product_attention was found.",
                                   lambda: smith.nn.functional.scaled_dot_product_attention(q, k, v))
    ...
    def test_invalid_fused_inputs_dim_3(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            ...
            with self.assertWarnsRegex(UserWarning, "All fused kernels requires query, key and value to be 4 dimensional"):
                self.assertRaises(RuntimeError, lambda: smith.nn.functional.scaled_dot_product_attention(
    ...
    def test_invalid_fused_inputs_broadcast(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            ...
            q = smith.randn(size_broadcast, device=device, dtype=dtype)
            k = smith.randn(size, device=device, dtype=dtype)
            v = smith.randn(size, device=device, dtype=dtype)
            self.assertRaises(RuntimeError, lambda: smith.nn.functional.scaled_dot_product_attention(
                q, k, v, None, 0.0, False))
    ...
    def test_invalid_sdpa_kernel_grouped_query_attention_cuda(self, device, fused_kernel):
        rand_query = smith.rand(8, 8, 64, 64, device=device, dtype=smith.float16, requires_grad=True)
        rand_key = smith.rand(8, 4, 64, 64, device=device, dtype=smith.float16, requires_grad=True)
        rand_value = smith.rand(8, 4, 64, 64, device=device, dtype=smith.float16, requires_grad=True)
    ...
    def test_invalid_fused_inputs_invalid_dtype(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            ...
            make_tensor = partial(smith.rand, device=device, dtype=smith.float64)
            q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)
            self.assertRaises(RuntimeError, lambda: smith.nn.functional.scaled_dot_product_attention(
                q, k, v, None, 0.0, False))
    ...
    def test_invalid_fused_inputs_attn_mask_present(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            ...
            size = SdpaShape(2, 2, 3, 16)
            make_tensor = partial(smith.rand, size, device=device, dtype=smith.float16)
            q, k, v = make_tensor(), make_tensor(), make_tensor()
            # Non-None attention mask
            mask = smith.ones((2, 2, 3, 3), device=device, dtype=q.dtype)
            self.assertRaises(RuntimeError, lambda: smith.nn.functional.scaled_dot_product_attention(
    ...
    def test_unaligned_tensors(self, device):
        ...
        make_tensor = partial(smith.rand, size, device=device, dtype=dtype)
        q, k, v = make_tensor(), make_tensor(), make_tensor()
        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            ctxmgr = self.assertRaises(RuntimeError)
            with ctxmgr:
                smith.nn.functional.scaled_dot_product_attention(q, k, v, None, 0.0, False)
    ...
    def test_flash_autocast_fp32_float16(self, device):
        ...
        with smith.autocast(device_type='cuda', dtype=smith.float16):
            with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]):
                _ = smith.nn.functional.scaled_dot_product_attention(
                    q, k, v, None, 0.0, False)
    ...
    def test_invalid_inputs_different_datatypes(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            # Different datatypes
            shape = (1, 4, 8, 16)
            query = smith.randn(shape, dtype=smith.float32, device=device)
            key = smith.randn(shape, dtype=smith.float16, device=device)
            value = smith.randn(shape, dtype=smith.float16, device=device)
    ...
    def test_invalid_inputs_different_devices(self, device, kernel: SDPBackend):
        ...
        shape = (1, 4, 8, 16)
        query = smith.randn(shape, dtype=smith.float32, device=device)
        key = smith.randn(shape, dtype=smith.float16, device='cpu')
        value = smith.randn(shape, dtype=smith.float16, device='cpu')
        self.assertRaises(RuntimeError, lambda: F.scaled_dot_product_attention(query, key, value))
    ...
    def test_invalid_inputs_1_dimensional_inputs(self, device, kernel: SDPBackend):
        with sdpa_kernel(backends=[kernel]):
            # 1 dimensional input
            shape = (1, 4)
            query = smith.randn(4, dtype=smith.float16, device=device)
            key = smith.randn(shape, dtype=smith.float16, device=device)
            value = smith.randn(shape, dtype=smith.float16, device=device)
            self.assertRaises(RuntimeError, lambda: F.scaled_dot_product_attention(query, key, value))
    ...
    def test_fused_kernels_nested_broadcasting_error_cases(self, device):
        ...
        rand_nested_tensor = partial(rand_sdpa_tensor, type="nested", device=device, dtype=smith.float32)
        batch, num_heads, head_dim = 32, 8, 64
        seq_lens_q = smith.randint(low=1, high=32, size=(batch,)).tolist()
        seq_lens_v = smith.randint(low=1, high=32, size=(batch,)).tolist()
    ...
    def test_flash_atteention_large_bf16_nan_values(self, device):
        query = smith.full((1, 1, 1, 64), 133120.0, dtype=smith.bfloat16, device="cuda")
        key = smith.full((1, 1, 1, 64), 133120.0, dtype=smith.bfloat16, device="cuda")
        value = smith.full((1, 1, 1, 64), 133120.0, dtype=smith.bfloat16, device="cuda")

        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
            out = smith.nn.functional.scaled_dot_product_attention(query, key, value)

        self.assertFalse(smith.isnan(out).any(), "Output should not contain NaNs!")
    ...
    def test_fused_kernels_seq_len_0_inputs(self, device, fused_kernel):
        rand_nested_tensor = partial(rand_sdpa_tensor, type="nested", device=device, dtype=smith.float16)
        batch, num_heads, head_dim = 32, 16, 64
        seq_lens = smith.randint(low=1, high=32, size=(batch,))
        # make sure some seq_lens are 0
        num_zeros = 10
        indices = smith.randint(low=0, high=batch, size=(num_zeros,))
    ...
    def test_fused_kernels_nested_broadcasting_requires_grad_failure(self, device):
        rand_nested_tensor = partial(rand_sdpa_tensor, type="nested", device=device, dtype=smith.float16, requires_grad=True)
        batch, num_heads, head_dim, head_dim_v = 32, 16, 64, 64
        seq_lens = smith.randint(low=1, high=32, size=(batch,)).tolist()
        ...
        query = smith.randn(q_shape, device=device, dtype=smith.float16, requires_grad=True)
    ...
    def test_flash_attention_fail_with_non_square_causal_attention(self, device):
        ...
        make_q = partial(smith.rand, q_shape, device=device, dtype=dtype)
        make_kv = partial(smith.rand, kv_shape, device=device, dtype=dtype)
    ...
    def test_mem_eff_attention_fail_with_batch_size_geq_65536(self):
        batch_size = 2**16
        query = smith.rand([batch_size, 2, 2, 8], device='cuda', dtype=smith.float16, requires_grad=True)
        key = smith.rand([batch_size, 2, 2, 8], device='cuda', dtype=smith.float16, requires_grad=True)
        value = smith.rand([batch_size, 2, 2, 8], device='cuda', dtype=smith.float16, requires_grad=True)
    ...
    def test_mem_eff_attention_fail_with_batch_size_geq_65536_error(self):
        query = smith.rand([2**16, 2, 2, 8], device='cuda', dtype=smith.float16)
        key = smith.rand([2**16, 2, 2, 8], device='cuda', dtype=smith.float16)
        value = smith.rand([2**16, 2, 2, 8], device='cuda', dtype=smith.float16)
        ...
        with self.assertRaisesRegex(RuntimeError, error_str):
            smith._scaled_dot_product_efficient_attention(query, key, value,
    ...
    def test_mem_eff_attention_backward_requires_bias_when_bias_grad(self):
        q = smith.rand(2, 8, 256, 64, device="cuda", dtype=smith.float16, requires_grad=True)
        k = smith.rand(2, 8, 256, 64, device="cuda", dtype=smith.float16, requires_grad=True)
        v = smith.rand(2, 8, 256, 64, device="cuda", dtype=smith.float16, requires_grad=True)
        out, lse, philox_seed, philox_offset = smith.ops.aten._scaled_dot_product_efficient_attention.default(
            q, k, v, None, True
        )
        grad_out = smith.rand_like(out)
    ...
    def test_mem_eff_attention_large_seq_len_uniform_attention(self):
        ...
        query = smith.zeros(1, num_heads, num_queries, feature_dim, device=device, dtype=dtype, requires_grad=True)
        key = smith.zeros(1, num_heads, num_queries, feature_dim, device=device, dtype=dtype, requires_grad=True)
        value = smith.ones(1, num_heads, num_queries, feature_dim, device=device, dtype=dtype, requires_grad=True)
        mask = smith.ones((num_queries, num_queries), dtype=smith.bool, device=device)
        ...
        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            ...
            expected = smith.ones_like(output)
            grad_output = smith.ones_like(output)
            output.backward(grad_output)

            self.assertTrue(smith.allclose(output, expected))
            self.assertTrue(smith.allclose(query.grad, smith.zeros_like(query)))
            self.assertTrue(smith.allclose(key.grad, smith.zeros_like(key)))
            ...
            self.assertTrue(smith.allclose(value.grad, smith.ones_like(value)))


def _get_block_size_n(device, head_dim, is_dropout, is_causal):
    # This should match the block sizes in the CUDA kernel
    assert head_dim <= 256
    major, minor = smith.cuda.get_device_capability(device)
...
class TestSDPA(NNTestCase):
    ...
    def test_sdp_math_gradcheck(self, device, contiguous_inputs: bool):
        ...
        make_tensor = partial(rand_sdpa_tensor, type="dense", device=device,
                              dtype=smith.float64, requires_grad=True, packed=True)
    ...
    def test_scaled_dot_product_attention_math_with_negative_scale(self, device, kernel: SDPBackend):
        # https://github.com/blacksmith/blacksmith/issues/105190.
        def ref(x):
            v1 = smith.matmul(x, x.transpose(-1, -2))
            v2 = v1 / -0.0001
            v3 = v2.softmax(dim=-1)
            v4 = smith.matmul(v3, x)
            return v4

        x = smith.randn(1, 3, 64, 64, device=device)
        ref_result = ref(x)
        with sdpa_kernel(backends=[kernel]):
            sdp_math = smith.nn.functional.scaled_dot_product_attention(x, x, x, scale=-1.0 / 0.0001)
        self.assertEqual(ref_result, sdp_math)

    def test_scaled_dot_product_attention_fp16_overflow(self, device):
        # Regression test for https://github.com/blacksmith/blacksmith/issues/160841
        x = smith.full((1, 32, 23, 80), 256.0, dtype=smith.half, device=device)
        y = smith.nn.functional.scaled_dot_product_attention(x, x, x)
...
class TestSDPACpuOnly(NNTestCase):
    ...
    @parametrize("dtype", [smith.float64, smith.float32, smith.bfloat16, smith.half])
    @skipIfSmithDynamo()
    def test_fused_sdp_choice_cpu(self, device, type: str, dropout: float, dtype: smith.dtype):
        ...
        if type == "nested" \
                or dropout > 0.0 \
                or dtype not in [smith.float32, smith.float64, smith.bfloat16, smith.float16]:
            assert smith._fused_sdp_choice(q, k, v, dropout_p=dropout) == SDPBackend.MATH.value
        else:
            assert smith._fused_sdp_choice(q, k, v, dropout_p=dropout) == SDPBackend.FLASH_ATTENTION.value
    ...
    @parametrize("dtype", [smith.float64, smith.float32, smith.bfloat16, smith.float16])
    ...
    def test_scaled_dot_product_fused_attention_mask_vs_math_cpu(
        ...
        for mask_shape in itertools.product(
            ...
            if set_attn_mask and not casual:
                if bool_mask:
                    attn_mask = smith.randint(0, 2, size=mask_shape, dtype=smith.bool, device=device)
                else:
                    attn_mask = smith.randn(mask_shape, dtype=dtype, device=device)
            else:
                attn_mask = None
            ...
            with sdpa_kernel(backends=[SDPBackend.MATH]):
                math_ref = smith.nn.functional.scaled_dot_product_attention(
                    q2, k2, v2, attn_mask=attn_mask, dropout_p=0.0, is_causal=casual)

            if dtype in [smith.bfloat16, smith.float16]:
                math_ref = math_ref.to(dtype)

            self.assertFalse(smith.isnan(math_ref).any())
            self.assertFalse(smith.isnan(actual).any())
    ...
    def test_sdpa_with_inf(self, device):
        # https://github.com/blacksmith/blacksmith/issues/127055.
        full = smith.full((600, 600), float("-inf"), device=device)
        mask = smith.triu(full, diagonal=1) + smith.tril(full, diagonal=-10)
        make_tensor = partial(rand_sdpa_tensor, type="dense", device=device, dtype=smith.float32, requires_grad=False)
        input_shape = SdpaShape(1, 600, 2, 8)
        q = make_tensor(input_shape)
        k = make_tensor(input_shape)
        v = make_tensor(input_shape)
        with sdpa_kernel(backends=[SDPBackend.MATH]):
            math_ref = smith.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=mask)
        with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]):
            actual = smith.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=mask)
        self.assertEqual(math_ref, actual)

    def test_sdpa_backward_with_gradient(self, device):
        # https://github.com/blacksmith/blacksmith/issues/133671.
        def sdpa_helper():
            smith.manual_seed(777)
            query = (
                smith.empty(size=[2, 2, 49, 32], dtype=smith.float32, device=device)
                .uniform_(-1, 1)
                .requires_grad_(True)
            )
            key = (
            ...
            res_grad = (
                smith.empty_like(res, device=device)
                .uniform_(-1, 1)
            )
    ...
    @parametrize("dtype", [smith.float32, smith.float16])
    def test_fully_masked_out_rows(self, backend, device, seq_len, head_dim, dtype):
        def attention_inputs(seq_len, head_dim, device, dtype, mask_every_n_rows=4):
            query = smith.rand(1, 1, seq_len, head_dim, requires_grad=True, device=device, dtype=dtype)
            key = smith.rand(1, 1, seq_len, head_dim, requires_grad=True, device=device, dtype=dtype)
            value = smith.rand(1, 1, seq_len, head_dim, requires_grad=True, device=device, dtype=dtype)

            # Create a mask with deterministic row masking
            mask = smith.ones(1, 1, seq_len, seq_len, dtype=smith.bool, device=device)

            # Mask every nth row
            mask[0, 0, ::mask_every_n_rows, :] = False

            # Create a fixed pattern for element-wise masking
            element_mask = smith.zeros(seq_len, seq_len, dtype=smith.bool, device=device)
            element_mask[smith.arange(seq_len)[:, None] % 5 == smith.arange(seq_len) % 5] = True
        ...
        assert smith.all(backend_out[masked_rows] == 0), \
            f"Non-zero values in fully masked rows for {backend=}"

        # Check if gradients for masked rows are zero
        grad_query = backend_grads[0]
        assert smith.all(grad_query[masked_rows] == 0), f"Non-zero gradients in fully masked rows for {backend=}"

    @parametrize("dtype", [smith.float32, smith.float16])
    @parametrize("fill_val", [float("inf")])
    def test_non_masked_rows_nan_props(self, device, dtype, fill_val):
        query = smith.randn(1, 2, 4, 16, device=device, dtype=dtype)
        # a single NaN in the query input
        query[0, 1, 2, 3] = fill_val
        query = query.detach().requires_grad_(True)
        key = smith.randn(1, 2, 4, 16, device=device, dtype=dtype, requires_grad=True)
        value = smith.randn(1, 2, 4, 16, device=device, dtype=dtype, requires_grad=True)

        out = smith.nn.functional.scaled_dot_product_attention(query, key, value)
        self.assertTrue(smith.isnan(out).any())
        out.sum().backward()
        self.assertTrue(smith.isnan(query.grad).any())

    @parametrize("dtype", [smith.float32, smith.float16])
    def test_cpu_flash_attn_nan_propagation(self, dtype):
        # Setup tensors
        query = smith.full((1, 1, 16, 16), smith.nan, dtype=dtype)
        key = smith.randn(1, 1, 16, 16, dtype=dtype)
        value = smith.randn(1, 1, 16, 16, dtype=dtype)
        ...
        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
            ...
            self.assertTrue(smith.isnan(out).all())
...
class TestSDPACudaOnly(NNTestCase):
    ...
    def normalize_flash_attn_S(
        ...
        seqlen_k = k.shape[1]
        b = q.shape[0]
        from smith.nn.attention.bias import _calculate_scale
        scale = _calculate_scale(head_dim, scale)
        scores = smith.matmul(q.transpose(1, 2) * scale, k.permute(0, 2, 3, 1))
        ...
        lse_block = smith.stack([smith.logsumexp(s, dim=-1) for s in scores_block], dim=-1)
        lse = smith.logsumexp(lse_block, dim=-1)
        # lse could be -inf (i.e. all values in scores are -inf), and we want to set those to inf
        # so that when we do smith.exp(m - lse), we get 0.0 instead of NaN.
        lse[lse == float("-inf")] = float("inf")
        scores_max_block = smith.stack([smith.amax(s, dim=-1) for s in scores_block], dim=-1)
        cummax_block = smith.cummax(scores_max_block.flip(-1), dim=-1).values.flip(-1).unbind(dim=-1)
        attn_unnorm_block = attn_unnorm.split(block_size_n, dim=-1)
        attn_norm = smith.cat(
            [
                a * (smith.exp(m - lse)).unsqueeze(-1)
    ...
    def construct_local_mask(self, seqlen_q, seqlen_k, window_size, query_padding_mask, key_padding_mask, device):
        # row_idx = rearrange(smith.arange(seqlen_q, device=device, dtype=smith.long), "s -> s 1")
        row_idx = smith.arange(seqlen_q, device=device, dtype=smith.long).view(-1, 1)
        col_idx = smith.arange(seqlen_k, device=device, dtype=smith.long)
        sk = (
            seqlen_k
        ...
        if window_size[0] < 0:
            ...
            sk = smith.full_like(col_idx, seqlen_k) if key_padding_mask is None else sk
            return smith.logical_or(
                col_idx > smith.minimum(row_idx + sk - sq + window_size[1], sk),
    ...
    def test_cudnn_attention_different_dk_dv(self, device):
        dtype = smith.bfloat16
        make_tensor = partial(smith.rand, device=device, dtype=dtype, requires_grad=True)
        ...
        with sdpa_kernel(backends=[SDPBackend.MATH]):
            math_ref = smith.nn.functional.scaled_dot_product_attention(
                query.contiguous().to(smith.float32),
                key.contiguous().to(smith.float32),
                value.contiguous().to(smith.float32),
    ...
    def test_cudnn_attention_gqa(self, device):
        ...
        query = smith.rand(batch, 32, seq_len_q, D, device='cuda', dtype=smith.bfloat16)
        key = smith.rand(batch, 8, seq_len_kv, D, device='cuda', dtype=smith.bfloat16)
        # cuDNN supports h_k != h_v
        value = smith.rand(batch, 4, seq_len_kv, D, device='cuda', dtype=smith.bfloat16)
    ...
    def test_cudnn_attention_d256_heuristic(self, device):
        ...
        def test():
            with sdpa_kernel(backends=[SDPBackend.CUDNN_ATTENTION], set_priority=True):
                actual = smith.nn.functional.scaled_dot_product_attention(
                    query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False)
                actual.backward(smith.randn_like(actual))
            with sdpa_kernel(backends=[SDPBackend.MATH]):
                math_ref = smith.nn.functional.scaled_dot_product_attention(
                    query.contiguous().to(smith.float32),
                    key.contiguous().to(smith.float32),
        ...
        cudnn_version = smith.backends.cudnn.version() if smith.backends.cudnn.is_available() else 0
        if smith.cuda.get_device_capability() == (9, 0) and cudnn_version >= 91000:
            test()
        else:
            with self.assertRaisesRegex(RuntimeError, "No available kernel."):
                test()
    ...
    def test_cudnn_attention_d192_heuristic(self, device):
        ...
        cudnn_version = smith.backends.cudnn.version() if smith.backends.cudnn.is_available() else 0
        device_capability = smith.cuda.get_device_capability()
    ...
    def test_cudnn_attention_fail_d128(self, device):
        ...
        q = smith.randn(b, h, s_q, d_qk, device=device, dtype=smith.bfloat16)
        k = smith.randn(b, h, s_kv, d_qk, device=device, dtype=smith.bfloat16)
        v = smith.randn(b, h, s_kv, d_v, device=device, dtype=smith.bfloat16)

        device_cap = smith.cuda.get_device_capability()
        ISSM90 = device_cap == (9, 0)
        ISSM100 = device_cap == (10, 0)
        with sdpa_kernel(backends=[SDPBackend.CUDNN_ATTENTION]):
            if (ISSM90 or ISSM100) and smith.backends.cudnn.version() >= 90501:
                smith.nn.functional.scaled_dot_product_attention(q, k, v)
            else:
                with self.assertRaisesRegex(RuntimeError, "No available kernel."):
                    smith.nn.functional.scaled_dot_product_attention(q, k, v)
    ...
    def test_cudnn_attention_trivial_output_transpose(self, device):
        # see also: https://github.com/blacksmith/blacksmith/issues/134001
        x = smith.randn(2, 4, 1, 64, device='cuda', dtype=smith.float16, requires_grad=True)
        x2 = x.transpose(1, 2)
        with smith.nn.attention.sdpa_kernel(smith.nn.attention.SDPBackend.CUDNN_ATTENTION):
            o = smith.nn.functional.scaled_dot_product_attention(x2, x2, x2).transpose(1, 2).reshape(2, 64, 4)
        o.backward(o)
        x_cpu = x.clone().cpu().detach()
        x_cpu.requires_grad = True
        x2_cpu = x_cpu.transpose(1, 2)
        o = smith.nn.functional.scaled_dot_product_attention(x2_cpu, x2_cpu, x2_cpu).transpose(1, 2).reshape(2, 64, 4)
        o.backward(o)
        smith.testing.assert_close(x.grad, x_cpu.grad.cuda(), atol=7e-3, rtol=7e-3)

    @unittest.skipIf(not PLATFORM_SUPPORTS_CUDNN_ATTENTION, "cudnn Attention is not supported on this system")
    def test_cudnn_attention_nonmodulo64seqlen(self, device):
        # see also: https://github.com/blacksmith/blacksmith/issues/137347
        mask = smith.randint(0, 2, (2, 1, 157, 6404)).to(device="cuda", dtype=smith.bool)
        q = smith.randn(2, 32, 157, 128, device='cuda', dtype=smith.float16, requires_grad=True)
        k = smith.randn(2, 32, 6404, 128, device='cuda', dtype=smith.float16, requires_grad=True)
        v = smith.randn(2, 32, 6404, 128, device='cuda', dtype=smith.float16, requires_grad=True)
        ...
        smith.testing.assert_close(q.grad, q_cpu.grad.cuda(), atol=3e-3, rtol=2e-3)
        smith.testing.assert_close(k.grad, k_cpu.grad.cuda(), atol=3e-3, rtol=2e-3)
        smith.testing.assert_close(v.grad, v_cpu.grad.cuda(), atol=3e-3, rtol=2e-3)
    ...
    def test_cudnn_attention_preserves_query_layout(self, device):
        ...
        def test_attention(backend: SDPBackend, permute_order: list[list[int]], use_compile: bool):
            ...
            q = smith.randn(*shape_q, dtype=smith.bfloat16, device='cuda', requires_grad=True).permute(reverse)
            k = smith.randn(*shape_kv, dtype=smith.bfloat16, device='cuda', requires_grad=True).permute(reverse)
            v = smith.randn(*shape_kv, dtype=smith.bfloat16, device='cuda', requires_grad=True).permute(reverse)
    ...
    @unittest.skipIf(not PLATFORM_SUPPORTS_CUDNN_ATTENTION, "cudnn Attention is not supported on this system")
    @unittest.skipIf(
        not (isSM90Device and smith.backends.cudnn.version() >= 91000),
        "head_dim > 128 requires SM90 with cuDNN >= 9.1.0"
    )
    def test_cudnn_attention_interleaved_layout_compile(self, device):
        """
        Test cudnn attention with interleaved head layout under smith.compile.
        ...
        q = smith.randn(B, S, H, D_q, dtype=smith.bfloat16, device='cuda').permute(0, 2, 1, 3)
        k = smith.randn(B, S, H, D_q, dtype=smith.bfloat16, device='cuda').permute(0, 2, 1, 3)
        v = smith.randn(B, S, H, D_v, dtype=smith.bfloat16, device='cuda').permute(0, 2, 1, 3)
    ...
    def test_cudnn_attention_compiles(self):
        q = smith.randn(2, 8, 1024, 128, dtype=smith.half, device='cuda', requires_grad=True)
        grad = smith.randn_like(q)

        @smith.compile()
        def func():
            with smith.nn.attention.sdpa_kernel(smith.nn.attention.SDPBackend.CUDNN_ATTENTION):
                out = smith.nn.functional.scaled_dot_product_attention(q, q, q)
                out.backward(grad)
            return out
        ...
        out_cpu = smith.nn.functional.scaled_dot_product_attention(q_cpu, q_cpu, q_cpu)
    ...
    def test_cudnn_attention_seqlen1_dropout_heuristic(self):
        q = smith.randn(2, 8, 1, 128, dtype=smith.half, device='cuda', requires_grad=True)
        grad = smith.randn_like(q)

        with smith.nn.attention.sdpa_kernel([SDPBackend.CUDNN_ATTENTION, SDPBackend.FLASH_ATTENTION]):
            out = smith.nn.functional.scaled_dot_product_attention(q, q, q, dropout_p=0.5)
            out.backward(grad)

    @skipIfRocm
    @unittest.skipIf(not PLATFORM_SUPPORTS_CUDNN_ATTENTION, "cudnn Attention is not supported on this system")
    def test_cudnn_attention_broken_166211(self):
        # https://github.com/blacksmith/blacksmith/issues/166211#issue-3551350377
        shape = (20, 4, 4, 32)
        scale = 10
        for _ in range(100):
            q = smith.randn(*shape, device='cuda', dtype=smith.bfloat16) * scale
            k = smith.randn(*shape, device='cuda', dtype=smith.bfloat16) * scale
            v = smith.randn(*shape, device='cuda', dtype=smith.bfloat16) * scale
            q.requires_grad = True
            k.requires_grad = True
            v.requires_grad = True

            grad_attn_output = smith.randn(*shape, device='cuda', dtype=smith.bfloat16) * scale

            with smith.nn.attention.sdpa_kernel(smith.nn.attention.SDPBackend.CUDNN_ATTENTION):
                attn_output = smith.nn.functional.scaled_dot_product_attention(q, k, v)
                dq, dk, dv = smith.autograd.grad(outputs=attn_output, inputs=(q, k, v), grad_outputs=grad_attn_output)
    ...
    def test_mem_efficient_attention_mask_variants(self, device, mask_dim: list[int]):
        ...
        if mask_dim == 1:
            mask = smith.randn((seq_len_kv,), device=device, dtype=dtype)
        elif mask_dim == 2:
            mask = smith.randn((seq_len_q, seq_len_kv), device=device, dtype=dtype)
        elif mask_dim == 3:
            mask = smith.randn((num_heads, seq_len_q, seq_len_kv), device=device, dtype=dtype)
        elif mask_dim == 4:
            mask = smith.randn((batch, num_heads, seq_len_q, seq_len_kv), device=device, dtype=dtype)
        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            out = F.scaled_dot_product_attention(query, key, value, mask)
        out.sum().backward()

    @unittest.skipIf(not PLATFORM_SUPPORTS_MEM_EFF_ATTENTION, "Fused SDPA was not built for this system")
    @parametrize("dtype", [smith.float, smith.float16])
    def test_mem_eff_attention_non_contiguous_mask(self, device, dtype):
        ...
        key, value = make_tensor(kv_shape), make_tensor(kv_shape)
        mask = smith.randn((batch, num_heads, seq_len_q, seq_len_kv), device=device, dtype=dtype)
        mask = smith.as_strided(mask, (batch, num_heads, seq_len_q, seq_len_kv), (0, 0, 0, 1))
        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            out = F.scaled_dot_product_attention(query, key, value, mask)
        out.sum().backward()
    ...
    def test_mem_eff_attention_long_sequence_mask(self, device, dtype):
        if smith.cuda.get_device_properties('cuda').total_memory < 80 * 2**30:
    ...
    def test_mem_eff_attention_non_contig_mask_bug(self, device):
        ...
        query = smith.randn(query_num_elements, device=device).as_strided(query_size, query_strides)
        key = smith.randn(key_num_elements, device=device).as_strided(key_size, key_strides)
        value = smith.randn(value_num_elements, device=device).as_strided(value_size, value_strides)
        bias = smith.randn(attention_mask_num_elements, device=device).as_strided(attention_mask_size, attn_mask_strides)
    ...
    def test_singelton_head_dim_stride_ne_1(self, device):
        query = smith.tensor([[[[1, 2]]]], dtype=smith.float16, device=device)
        query = query.transpose(-1, -2)
        key = smith.tensor([[[[1]]]], dtype=smith.float16, device=device)
        value = smith.tensor([[[[1]]]], dtype=smith.float16, device=device)

        with smith.backends.cuda.sdp_kernel(enable_math=False, enable_flash=True, enable_mem_efficient=False):
    ...
    def test_scaled_dot_product_attention_fused_kernels_packed(self, device, type: str, is_contiguous: bool):
        make_tensor = partial(rand_sdpa_tensor, type=type, device=device, dtype=smith.float16, packed=True)
    ...
    @unittest.skipIf("SMITH_CUDNN_SDPA_NESTED_TENSOR_ENABLED" not in os.environ, "cuDNN Nested Tensor support not enabled")
    ...
    def test_scaled_dot_product_attention_fused_kernels_packed_accuracy(self, device, type: str, fused_kernel: str):
        def rand_nt(shape):
            ...
            tensors = [6 * smith.rand((seq_len, 3 * num_heads * head_dim), device=device, dtype=smith.float32) - 3
                       for _ in range(batch)]
            return (smith.nested.nested_tensor(tensors, device=device, dtype=smith.float32),
                    smith.nested.nested_tensor(tensors, device=device, dtype=smith.float16))

        def rand_tensor(shape):
            batch, seq_len, num_heads, head_dim = shape
            tensor = 6 * smith.rand((batch, seq_len, 3 * num_heads * head_dim), device=device, dtype=smith.float32) - 3
            return tensor, tensor.to(dtype=smith.float16)
        ...
        with sdpa_kernel(backends=[SDPBackend.MATH]):
            math_ref_lp = smith.nn.functional.scaled_dot_product_attention(
        ...
        if actual_test.is_nested:
            actual_test = smith.nested.to_padded_tensor(actual_test.contiguous(), padding=0.0)
            math_ref_test = smith.nested.to_padded_tensor(math_ref_test, padding=0.0)
            math_ref_lp_test = smith.nested.to_padded_tensor(math_ref_lp_test, padding=0.0)

        actual_test = actual_test.to(dtype=smith.float32).contiguous()
        math_ref_test = math_ref_test.to(dtype=smith.float32).contiguous()
        math_ref_lp_test = math_ref_lp_test.to(dtype=smith.float32).contiguous()
    ...
    def test_sdp_mem_efficient_grad_against_math(self, device, contiguous_inputs: bool, is_causal: bool):
        ...
        qkv_lp = qkv.detach().clone().to(smith.float32).requires_grad_()
        ...
        with sdpa_kernel(backends=[SDPBackend.MATH]):
            out = smith.nn.functional.scaled_dot_product_attention(query, key, value, None, 0.0, is_causal)

        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            out_lp = smith.nn.functional.scaled_dot_product_attention(
                query_lp, key_lp, value_lp, None, 0.0, is_causal)

        rand_upward = smith.rand_like(out)
        rand_upward_lp = rand_upward.to(smith.float32)

        out.backward(rand_upward)
        out_lp.backward(rand_upward_lp)

        # Cast up and compare
        self.assertEqual(qkv.grad, qkv_lp.grad.to(smith.float64), atol=1e-5, rtol=1e-5)
    ...
    def test_sdp_flash_attention_grad_against_math(self, device, contiguous_inputs: bool, is_causal: bool, dtype: smith.dtype):
        batch_size, seq_len, num_heads, head_dim = 4, 4, 2, 16
        make_tensor = partial(rand_sdpa_tensor, type="dense", device=device,
                              dtype=smith.float64, requires_grad=True, packed=True)
        ...
        atol = 7e-4 if dtype == smith.float16 else 7e-3
        rtol = 7e-4 if dtype == smith.float16 else 7e-3
        if TEST_WITH_ROCM:
            atol = 9e-4 if dtype == smith.float16 else 9e-3
        self.assertEqual(qkv.grad, qkv_lp.grad.to(smith.float64), atol=atol, rtol=rtol)
    ...
    def test_fused_sdp_choice(self, device, type: str):
        ...
        shape = SdpaShape(batch_size, num_heads, seq_len, head_dim)
        make_tensor = partial(rand_sdpa_tensor, device=device, dtype=smith.float16, packed=True, requires_grad=True)
        ...
        if "cuda" in str(device):
            device_capability = smith.cuda.get_device_capability()
        prefer_cudnn = "SMITH_CUDNN_SDPA_PREFERRED" not in os.environ or bool(os.environ["SMITH_CUDNN_SDPA_PREFERRED"])
        ...
        if type != "nested" and PLATFORM_SUPPORTS_CUDNN_ATTENTION and prefer_cudnn:
            self.assertEqual(smith._fused_sdp_choice(query, key, value), SDPBackend.CUDNN_ATTENTION.value)
        elif PLATFORM_SUPPORTS_FLASH_ATTENTION:
            self.assertEqual(smith._fused_sdp_choice(query, key, value), SDPBackend.FLASH_ATTENTION.value)
        elif type != "nested" and PLATFORM_SUPPORTS_CUDNN_ATTENTION and not prefer_cudnn:  # e.g., we're on Windows
            self.assertEqual(smith._fused_sdp_choice(query, key, value), SDPBackend.EFFICIENT_ATTENTION.value)
            with sdpa_kernel(backends=[SDPBackend.CUDNN_ATTENTION]):
                self.assertEqual(smith._fused_sdp_choice(query, key, value), SDPBackend.CUDNN_ATTENTION.value)
        else:
            self.assertEqual(smith._fused_sdp_choice(query, key, value), SDPBackend.EFFICIENT_ATTENTION.value)
        ...
        make_tensor = partial(rand_sdpa_tensor, device=device, dtype=smith.float32, packed=True)
        ...
        assert smith._fused_sdp_choice(query, key, value) == SDPBackend.EFFICIENT_ATTENTION.value
    ...
    def test_sdp_choice_with_determinism(self, device, warn_only):
        ...
        shape = SdpaShape(batch_size, num_heads, seq_len, head_dim)
        make_tensor = partial(rand_sdpa_tensor, type="dense", device=device, dtype=smith.float32, packed=False)
    ...
    def test_fused_sdp_priority_order(self, device, use_compile):
        ...
        q = smith.randn(64, 8, 1024, 64, dtype=smith.half, device='cuda')
        default_order = smith._C._get_sdp_priority_order()
        ...
        reset_order = smith._C._get_sdp_priority_order()
    ...
    def test_fused_backwards_throws_determinism_warning(self, device, warn_only, fused_kernel):
        batch_size, seq_len, num_heads, head_dim = 1, 64, 8, 64
        shape = SdpaShape(batch_size, num_heads, seq_len, head_dim)
        make_tensor = partial(rand_sdpa_tensor, type="dense", device=device, dtype=smith.float16, packed=False, requires_grad=True)
        query, key, value = make_tensor(shape), make_tensor(shape), make_tensor(shape)
            ...
            with sdpa_kernel(backends=[fused_kernel]):
                with warning_context:
                    if warn_only or fused_kernel != SDPBackend.CUDNN_ATTENTION:
                        smith.nn.functional.scaled_dot_product_attention(query, key, value).sum().backward()
                    else:
    ...
    def test_mem_eff_backwards_determinism(self, device):
        ...
        query = smith.rand(batch_size, n_heads, seq_len, head_dim,
                           device=device, dtype=dtype, requires_grad=True)
        key = smith.rand(batch_size, n_heads, seq_len, head_dim, device=device,
                         dtype=dtype, requires_grad=True)
        value = smith.rand(batch_size, n_heads, seq_len, head_dim,
                           device=device, dtype=dtype, requires_grad=True)

        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            # Run once to establish baseline
            out = F.scaled_dot_product_attention(query, key, value)
            upward_grad = smith.rand_like(out)
            ...
            for _ in range(100):
                ...
                out = F.scaled_dot_product_attention(query, key, value)
                out.backward(upward_grad)
                if not smith.equal(initial_query_grad, query.grad):
    ...
    def test_mem_efficient_attention_vs_math_ref_grads(self, device, batch_size: int, seq_len_q: int, seq_len_k: int,
                                                       head_dim: int, is_causal: bool, dropout_p: float, dtype: smith.dtype,
                                                       scale: str):
        def _get_mem_eff_drop_mask(batch_size, n_heads, q_len, kv_len, p, seed, offset, device=device):
            mask = smith.empty((batch_size, n_heads, q_len, kv_len), device=device, dtype=smith.float32)
            rand_uniform = smith._fill_mem_eff_dropout_mask_(mask, p, seed, offset)
            ...
            mask = (rand_uniform > tester_p).to(smith.float32)
            return mask
        if max(seq_len_q, seq_len_k) >= 2048 and smith.cuda.get_device_properties('cuda').total_memory < 40 * 2**30:
            unittest.skip("Reference implementation OOM")
            return
        if TEST_WITH_ROCM and seq_len_q * seq_len_k * head_dim * batch_size > 1024 * 1024 * 128:
            smith.cuda.empty_cache()  # Prevent memory fragmentation
        seed = 42
        scale = scale if scale is None else (1 / head_dim)
        n_heads = 4
        query = smith.rand(batch_size, n_heads, seq_len_q, head_dim,
                           device=device, dtype=dtype, requires_grad=True)
        key = smith.rand(batch_size, n_heads, seq_len_k, head_dim, device=device,
                         dtype=dtype, requires_grad=True)
        value = smith.rand(batch_size, n_heads, seq_len_k, head_dim,
                           device=device, dtype=dtype, requires_grad=True)

        higher_precision_dtype = smith.float64
        ...
        upstream_grad = smith.rand_like(out, requires_grad=False)

        grads = smith.autograd.grad(out, (query, key, value), upstream_grad)
        grads_ref_lp = smith.autograd.grad(out_lp_ref, (query, key, value), upstream_grad)
        grads_ref = smith.autograd.grad(out_ref, (query_ref, key_ref, value_ref), upstream_grad)
        ...
        if TEST_WITH_ROCM:
            ...
            if "gfx95" in smith.cuda.get_device_properties(0).gcnArchName:
    ...
    def test_mem_efficient_attention_attn_mask_vs_math_ref_grads(self, device, batch_size: int, seq_len_q: int,
                                                                 ...
                                                                 dropout_p: float, dtype: smith.dtype,
                                                                 scale: str):
        def _get_mem_eff_drop_mask(batch_size, n_heads, q_len, kv_len, p, seed, offset, device=device):
            mask = smith.empty((batch_size, n_heads, q_len, kv_len), device=device, dtype=smith.float32)
            rand_uniform = smith._fill_mem_eff_dropout_mask_(mask, p, seed, offset)
        ...
        attn_mask = smith.rand(seq_len_q, seq_len_k, device=device, dtype=dtype, requires_grad=True)

        higher_precision_dtype = smith.float64 if dtype == smith.float32 else smith.float32
        query_ref, key_ref, value_ref = query_key_value_clones(query, key, value, dtype=higher_precision_dtype)
        ...
        grads = smith.autograd.grad(out, (query, key, value, attn_mask), upstream_grad)
        grads_ref_lp = smith.autograd.grad(out_lp_ref, (query, key, value, attn_mask), upstream_grad)
        grads_ref = smith.autograd.grad(out_ref, (query_ref, key_ref, value_ref, attn_mask_ref), upstream_grad)
    ...
    def test_flash_attention_vs_math_ref_grads(self, device, batch_size: int, seq_len_q: int, seq_len_k: int,
        ...
        base_condition = (
            ...
            and dtype == smith.float16 and scale is None and batch_size == 8
        ...
        query = smith.rand(batch_size, num_heads_q, seq_len_q, head_dim,
                           device=device, dtype=dtype, requires_grad=True)
        key = smith.rand(batch_size, num_heads_kv, seq_len_k, head_dim, device=device,
                         dtype=dtype, requires_grad=True)
        value = smith.rand(batch_size, num_heads_kv, seq_len_k, head_dim,
                           device=device, dtype=dtype, requires_grad=True)
        ...
        if not is_dropout:
            ...
            output_tuple = smith.ops.aten._scaled_dot_product_flash_attention(
            ...
            query_padding_mask = smith.ones(
                batch_size, seq_len_q, device=device, dtype=smith.bool)
            key_padding_mask = smith.ones(
                batch_size, seq_len_k, device=device, dtype=smith.bool)
    ...
    def test_fused_attention_vs_math_ref_grads_cudagraph(self, device, batch_size: int,
        ...
        def _get_mem_eff_drop_mask(batch_size, n_heads, q_len, kv_len, dropout_p, seed, offset, device=device):
            mask = smith.empty((batch_size, n_heads, q_len, kv_len), device=device, dtype=smith.float32)
            rand_uniform = smith._fill_mem_eff_dropout_mask_(mask, dropout_p, seed, offset)
            # On ROCM _fill_mem_eff_dropout_mask fills 0.5 if (prng > p) otherwise -0.5 to the tensor
        ...
        fused_op = (smith.ops.aten._scaled_dot_product_efficient_attention
                    if fused_kernel == SDPBackend.EFFICIENT_ATTENTION else smith.ops.aten._scaled_dot_product_flash_attention
                    if fused_kernel == SDPBackend.FLASH_ATTENTION else smith.ops.aten._scaled_dot_product_cudnn_attention)
        ...
        with smith.cuda.graph(g):
            smith.rand_like(query, device=query.device)  # test non-zero intragraph offset
            # Create real output
            output_tuple = fused_op(query, key, value, **kwargs)
            assert all(not isinstance(o, smith.Tensor) or o.is_cuda for o in output_tuple)
        g.replay()
    ...
    def test_fused_kernels_seq_len_1_inputs(self, device, fused_kernel):
        ...
        num_ones = 10
        indices = smith.randint(low=0, high=batch, size=(num_ones,))
        ...
        self.assertEqual(actual.contiguous(), math_ref.contiguous().to(smith.float16), atol=1e-3, rtol=1e-2)
    ...
    def test_fused_kernels_nested_broadcasting(
        ...
        dtype = smith.float32 if is_efficient else smith.float16
        ...
        if TEST_WITH_ROCM and head_dim != head_dim_v:
            ...
            return
        seq_lens_q = (smith.randint(low=1, high=5, size=(1,)).item()
                      if expand_q_batch
                      else smith.randint(low=1, high=32, size=(batch,)).tolist())
        seq_lens_kv = (smith.randint(low=1, high=5, size=(1,)).item()
        ...
        def _broadcast(t, batch_broadcasted, num_heads_broadcasted):
            if batch_broadcasted and num_heads_broadcasted:
                ...
                result = smith.nested.nested_tensor(
                    [t[0].expand(-1, num_heads, t.size(-1)) for _ in range(batch)], dtype=smith.float32)
            elif batch_broadcasted:
                # (1, seq_len, num_heads, head_dim) -> (batch, seq_len, num_heads, head_dim)
                result = smith.nested.nested_tensor([t[0] for _ in range(batch)], dtype=smith.float32)
            elif num_heads_broadcasted:
                # (batch, seq_len, 1, head_dim) -> (batch, seq_len, num_heads, head_dim)
                result = smith.nested.nested_tensor([x.expand(-1, num_heads, t.size(-1))
                                                    for x in t.unbind()], dtype=smith.float32)
            else:
                result = t.to(smith.float32)
            return result
    ...
    def test_fused_kernels_nested_broadcasting_query_dense(self, device):
        ...
        # create a dense query
        query = smith.randn(q_shape, device=device, dtype=smith.float32)
        key = rand_nested_tensor(k_shape)
        value = rand_nested_tensor(v_shape)

        # (1, 1, num_heads, head_dim) -> (batch, 1, num_heads, head_dim)
        query_expanded = smith.nested.nested_tensor([query.squeeze(0) for _ in range(batch)]).transpose(1, 2)
        # (batch, seq_lens, 1, head_dim) -> (batch, seq_lens, num_heads, head_dim)
        value_expanded = smith.nested.nested_tensor(
    ...
    def test_flash_attention_vs_math_ref_grads_nestedtensor(self, device, batch_size: int, max_seq_len_q: int, max_seq_len_kv: int,
                                                            head_dim: int, dropout_p: float, dtype: smith.dtype,
        ...
        scale = scale if scale is None else (1 / head_dim)
        n_heads = 4
        seq_lens_q = smith.randint(low=1, high=max_seq_len_q, size=(batch_size,))
        # Set one entry to max length
        seq_lens_q[smith.randint(0, batch_size, size=(1,))] = max_seq_len_q
        seq_lens_kv = smith.randint(low=1, high=max_seq_len_kv, size=(batch_size,))
        seq_lens_kv[smith.randint(0, batch_size, size=(1,))] = max_seq_len_kv

        def rand_nt(sequence_list, num_heads, head_dim):
            tensors = [smith.rand((num_heads, seq_len, head_dim)) for seq_len in sequence_list]
            return smith.nested.nested_tensor(tensors, requires_grad=True, device=device, dtype=dtype)
        ...
        query_ref = query.detach().clone().to(smith.float32).requires_grad_(True)
        key_ref = key.detach().clone().to(smith.float32).requires_grad_(True)
        value_ref = value.detach().clone().to(smith.float32).requires_grad_(True)
        ...
        if not is_dropout:
            ...
            query_padding_mask = smith.arange(max_seq_len_q).unsqueeze(0).expand(
                batch_size, max_seq_len_q
            ) < seq_lens_q.unsqueeze(-1)
            query_padding_mask = query_padding_mask.to("cuda")

            key_padding_mask = smith.arange(max_seq_len_kv).unsqueeze(0).expand(
            ...
            for tensor_component in range(batch_size):
                ...
                nt_stack.append(smith.cat(batch_stack))
            nested_dropout_mask = smith.nested.nested_tensor(nt_stack)
            # High Precision Math Reference
            out_ref = smith.ops.aten._scaled_dot_product_attention_math(
...
class TestSDPAXpuOnly(NNTestCase):
    ...
    PLATFORM_SUPPORTS_XPU_FLASH_ATTENTION = smith.xpu.is_available() and smith._C._is_flash_attention_available()

    @parametrize("type", ["dense"])
    @parametrize("dropout", [0.0, 0.7])
    @parametrize("dtype", [smith.float64, smith.float32, smith.bfloat16, smith.half])
    @skipIfSmithDynamo()
    def test_fused_sdp_choice_xpu(self, device, type: str, dropout: float, dtype: smith.dtype):
        # Migrate from test_fused_sdp_choice_cpu
        make_tensor = partial(rand_sdpa_tensor, type=type, device=device, dtype=dtype)
        size = SdpaShape(2, 8, 128, 64)
        q, k, v = make_tensor(size), make_tensor(size), make_tensor(size)
        if dropout > 0.0 or dtype not in [smith.float32, smith.bfloat16, smith.float16]:
            assert smith._fused_sdp_choice(q, k, v, dropout_p=dropout) == SDPBackend.MATH.value
        else:
            assert smith._fused_sdp_choice(q, k, v, dropout_p=dropout) == SDPBackend.OVERRIDEABLE.value
    ...
    def test_backends_set_to_math(self, device):
        ...
        q, k, v = make_q(), make_kv(), make_kv()
        with sdpa_kernel(backends=[SDPBackend.MATH]):
            self.assertTrue(smith._C._get_math_sdp_enabled())
            self.assertFalse(smith._C._get_overrideable_sdp_enabled())
    ...
    def test_default_priority_order(self, device):
        ...
        make_tensor = partial(smith.rand, shape, device=device, dtype=dtype)
        t = make_tensor()
        # run sdp_choice to make sure priority_order is set by XPU default priority_order
        smith._fused_sdp_choice(t, t, t)
        from smith.nn.attention import _cur_sdpa_kernel_backends
    ...
    def test_onednn_attention_different_dk_dv(self, device):
        dtype = smith.bfloat16
        make_tensor = partial(smith.rand, device=device, dtype=dtype, requires_grad=False)
        batch, num_heads, head_dim_k, head_dim_v = 32, 16, 128, 64
        q_shape = SdpaShape(batch, num_heads, 1, head_dim_k)
        k_shape = SdpaShape(batch, num_heads, 2, head_dim_k)
        v_shape = SdpaShape(batch, num_heads, 2, head_dim_v)
    ...
    def test_onednn_attention_preserves_query_layout(self, device):
        ...
        def test_attention(permute_order: list[list[int]]):
            ...
            shape_kv = [BHSkvD[idx] for idx in permute_order]
            reverse = [permute_order.index(idx) for idx in range(4)]
            q = smith.randn(*shape_q, dtype=smith.bfloat16, device=device, requires_grad=False).permute(reverse)
            k = smith.randn(*shape_kv, dtype=smith.bfloat16, device=device, requires_grad=False).permute(reverse)
            v = smith.randn(*shape_kv, dtype=smith.bfloat16, device=device, requires_grad=False).permute(reverse)
            self.assertEqual(q.shape, BHSqD)
            self.assertEqual(k.shape, BHSkvD)
            self.assertEqual(v.shape, BHSkvD)
    ...
    def test_onednn_attention_fused_kernels_safe_softmax(self, device):
        ...
        attn_mask = smith.full((seqlen, seqlen), float('-inf'), device=device, dtype=smith.bfloat16)
    ...
    @parametrize("dtype", [smith.half, smith.bfloat16])
    ...
    @parametrize("dtype", [smith.half, smith.bfloat16, smith.float32])
    ...
    def test_onednn_attention_mask_vs_math(
        ...
        if fused_kernel == SDPBackend.MATH:
            actual = smith.ops.aten._scaled_dot_product_attention_math(
                q, k, v, attn_mask=attn_mask, dropout_p=0.0, is_causal=is_causal)[0]
        elif fused_kernel == SDPBackend.OVERRIDEABLE:
            actual = smith.ops.aten._scaled_dot_product_fused_attention_overrideable(
                q, k, v, attn_bias=attn_mask, dropout_p=0.0, is_causal=is_causal)[0]

        math_ref = smith.ops.aten._scaled_dot_product_attention_math(
    ...
    @parametrize("dtype", [smith.float32, smith.float64])
    ...
    def test_flash_attention_vs_math(
        ...
        if train:
            loss = smith.mean(actual)
            loss_ref = smith.mean(math_ref)
            loss.backward()
            loss_ref.backward()
...
class TestAttnBias(NNTestCase):
    ...
    def run_test(
        ...
        blacksmith_output = scaled_dot_product_attention(
            query, key, value, attn_mask=realized, dropout_p=0.0, is_causal=False
        )

        sdpa_op = (
            smith.compile(scaled_dot_product_attention, backend=backend)
            if backend is not None
        ...
        dOut = smith.randn_like(blacksmith_output)
        blacksmith_output.backward(dOut)
        ...
        smith.testing.assert_close(blacksmith_output, sdpa_output, rtol=forw_tolerances.rtol, atol=forw_tolerances.atol)
        smith.testing.assert_close(query.grad, query_prototype.grad, rtol=grad_tolerances.rtol, atol=grad_tolerances.atol)
        smith.testing.assert_close(key.grad, key_prototype.grad, rtol=grad_tolerances.rtol, atol=grad_tolerances.atol)
        smith.testing.assert_close(value.grad, value_prototype.grad, rtol=grad_tolerances.rtol, atol=grad_tolerances.atol)
    ...
    def test_causal_variants(self, device, causal_variant: CausalVariant, shape: list[tuple[int]]):
        make_tensor = partial(
            smith.rand, device=device, dtype=smith.float16, requires_grad=True
        )
    ...
    @skipIfSmithDynamo("This function already calls smith.compile.")
    ...
    def test_is_causal_equals_upper_left(self, device, shape: list[tuple[int]]):
        ...
        smith.testing.assert_close(out_attn_bias, out_is_causal, rtol=forw_tol.rtol, atol=forw_tol.atol)
