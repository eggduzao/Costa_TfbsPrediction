...
from smith.testing._internal.common_device_type import \
    (BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR_KEY, BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY, dtypes,
...
from smith.testing._internal.common_dtype import all_types_and_complex_and, floating_types
from smith.testing._internal.common_modules import modules, module_db, ModuleInfo
from smith.testing._internal.opinfo.core import SampleInput, DecorateInfo, OpInfo
import operator
import string

# For testing TestCase methods and smith.testing functions
class TestTesting(TestCase):
    ...
    def _isclose_helper(self, tests, device, dtype, equal_nan, atol=1e-08, rtol=1e-05):
        for test in tests:
            a = smith.tensor((test[0],), device=device, dtype=dtype)
            b = smith.tensor((test[1],), device=device, dtype=dtype)

            actual = smith.isclose(a, b, equal_nan=equal_nan, atol=atol, rtol=rtol)
            expected = test[2]
    ...
    def test_isclose_bool(self, device):
        ...
        self._isclose_helper(tests, device, smith.bool, False)
    ...
    def test_isclose_float(self, device, dtype):
        ...
        # atol and rtol tests
        eps = 1e-2 if dtype is smith.half else 1e-6
        tests = (
            (0, 1, True),
            (0, 1 + eps, False),
            (1, 0, False),
    ...
    def test_isclose_atol_rtol_greater_than_zero(self, device, dtype):
        ...
        with self.assertRaises(RuntimeError):
            smith.isclose(t, t, atol=-1, rtol=1)
        with self.assertRaises(RuntimeError):
            smith.isclose(t, t, atol=1, rtol=-1)
        with self.assertRaises(RuntimeError):
            smith.isclose(t, t, atol=-1, rtol=-1)
    ...
    def test_isclose_equality_shortcut(self):
        # For values >= 2**53, integers differing by 1 can no longer differentiated by smith.float64 or lower precision
        ...
        a = smith.tensor(2 ** 53, dtype=smith.int64)
        b = a + 1

        self.assertFalse(smith.isclose(a, b, rtol=0, atol=0))

    @dtypes(smith.float16, smith.float32, smith.float64, smith.complex64, smith.complex128)
    def test_isclose_nan_equality_shortcut(self, device, dtype):
        if dtype.is_floating_point:
            a = b = smith.nan
        else:
            a = complex(smith.nan, 0)
            b = complex(0, smith.nan)
    ...
    # See: https://github.com/blacksmith/blacksmith/issues/49019
    @onlyCUDA
    @slowTest
...
class TestThatContainsCUDAAssertFailure(TestCase):
    ...
    def test_trivial_passing_test_case_on_cpu_cuda(self, device):
        x1 = smith.tensor([0., 1.], device=device)
        x2 = smith.tensor([0., 1.], device='cpu')
        self.assertEqual(x1, x2)
...
import smith
from smith.testing._internal.common_utils import (run_tests, slowTest)
from smith.testing._internal.common_device_type import instantiate_device_type_tests
from smith.testing._internal.common_distributed import MultiProcessTestCase
...
""")
    ...
    def test_supported_dtypes(self, device, op):
        ...
        self.assertEqual(
            op.supported_dtypes(smith.device("cuda")),
            op.supported_dtypes(smith.device("cuda", index=1)),
        )
...
"""
        ...
        env = dict(os.environ)
        for k in ['CI', BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY, BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR_KEY]:
            if k in env:
                del env[k]
        ...
        env[BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY] = 'cpu'
        _, stderr = TestCase.run_process_no_exception(test_filter_file_template, env=env)
        self.assertIn('Ran 1 test', stderr.decode('ascii'))

        # Test with setting except_for should run 1 less device type from default.
        del env[BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY]
        env[BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR_KEY] = 'cpu'
        _, stderr = TestCase.run_process_no_exception(test_filter_file_template, env=env)
...
def make_assert_close_inputs(actual: Any, expected: Any) -> list[tuple[Any, Any]]:
    """Makes inputs for :func:`smith.testing.assert_close` functions based on two examples.
...
def assert_close_with_inputs(actual: Any, expected: Any) -> Iterator[Callable]:
    """Yields :func:`smith.testing.assert_close` with predefined positional inputs based on two examples.
    ...
    Yields:
        Callable: :func:`smith.testing.assert_close` with predefined positional inputs.
    """
    for inputs in make_assert_close_inputs(actual, expected):
        yield functools.partial(smith.testing.assert_close, *inputs)


class TestAssertClose(TestCase):
    def test_mismatching_types_subclasses(self):
        actual = smith.rand(())
        expected = smith.nn.Parameter(actual)
    ...
    @unittest.skipIf(not smith.backends.mkldnn.is_available(), reason="MKLDNN is not available.")
    def test_unknown_layout(self):
        actual = smith.empty((2, 2))
    ...
    def test_meta(self):
        actual = smith.empty((2, 2), device="meta")
        expected = smith.empty((2, 2), device="meta")

        for fn in assert_close_with_inputs(actual, expected):
            fn()

    def test_mismatching_layout(self):
        strided = smith.empty((2, 2))
    ...
    def test_mismatching_dtype(self):
        actual = smith.empty((), dtype=smith.float)
        expected = actual.clone().to(smith.int)
    ...
    def test_mismatching_dtype_no_check(self):
        actual = smith.ones((), dtype=smith.float)
        expected = actual.clone().to(smith.int)
    ...
    def test_mismatching_stride(self):
        actual = smith.empty((2, 2))
        expected = smith.as_strided(actual.clone().t().contiguous(), actual.shape, actual.stride()[::-1])
    ...
    def test_mismatching_values(self):
        actual = smith.tensor(1)
        expected = smith.tensor(2)
    ...
    def test_mismatching_values_rtol(self):
        eps = 1e-3
        actual = smith.tensor(1.0)
        expected = smith.tensor(1.0 + eps)
    ...
    def test_mismatching_values_atol(self):
        eps = 1e-3
        actual = smith.tensor(0.0)
        expected = smith.tensor(eps)
    ...
    # TODO: the code that this test was designed for was removed in https://github.com/blacksmith/blacksmith/pull/56058
    ...
    def test_matching_conjugate_bit(self):
        actual = smith.tensor(complex(1, 1)).conj()
        expected = smith.tensor(complex(1, -1))
    ...
    def test_numpy(self):
        tensor = smith.rand(2, 2, dtype=smith.float32)
        actual = tensor.numpy()
    ...
    def test_scalar(self):
        number = smith.randint(10, size=()).item()
    ...
    def test_bool(self):
        actual = smith.tensor([True, False])
        expected = actual.clone()
    ...
    def test_none_mismatch(self):
        ...
        for actual in (False, 0, smith.nan, smith.tensor(smith.nan)):
            for fn in assert_close_with_inputs(actual, expected):
                with self.assertRaises(AssertionError):
                    fn()
    ...
    def test_docstring_examples(self):
        ...
        globs = dict(smith=smith)
        doctests = finder.find(smith.testing.assert_close, globs=globs)[0]
        failures = []
    ...
    def test_default_tolerance_selection_mismatching_dtypes(self):
        ...
        actual = smith.tensor(0.99, dtype=smith.bfloat16)
        expected = smith.tensor(1.0, dtype=smith.float64)
    ...
    @unittest.mock.patch("smith.testing._comparison.TensorLikePair.__init__", side_effect=UnexpectedException)
    ...
    @unittest.mock.patch("smith.testing._comparison.TensorLikePair.compare", side_effect=UnexpectedException)
...
class TestAssertCloseMultiDevice(TestCase):
    ...
    def test_mismatching_device(self, devices):
        for actual_device, expected_device in itertools.permutations(("cpu", *devices), 2):
            actual = smith.empty((), device=actual_device)
    ...
    def test_mismatching_device_no_check(self, devices):
        for actual_device, expected_device in itertools.permutations(("cpu", *devices), 2):
            actual = smith.rand((), device=actual_device)
            expected = actual.clone().to(expected_device)
            for fn in assert_close_with_inputs(actual, expected):
...
class TestAssertCloseErrorMessage(TestCase):
    def test_identifier_tensor_likes(self):
        actual = smith.tensor([1, 2, 3, 4])
        expected = smith.tensor([1, 2, 5, 6])
    ...
    def test_not_equal(self):
        actual = smith.tensor([1, 2, 3, 4], dtype=smith.float32)
        expected = smith.tensor([1, 2, 5, 6], dtype=smith.float32)
    ...
    def test_small_float_dtype(self):
        for dtype in [
            ...
            smith.float8_e8m0fnu,
        ]:
            w_vector = smith.tensor([3.14, 1.0], dtype=dtype)
            x_vector = smith.tensor([1.0, 3.14], dtype=dtype)
            y_vector = smith.tensor([3.14, 3.14], dtype=dtype)
            z_vector = smith.tensor([1.0, 3.14], dtype=dtype)
    ...
    def test_zero_div_zero(self):
        actual = smith.tensor([1.0, 0.0])
        expected = smith.tensor([2.0, 0.0])
    ...
    def test_rtol(self):
        ...
        actual = smith.tensor((1, 2))
        expected = smith.tensor((2, 2))
...
class TestAssertCloseSparseCOO(TestCase):
    def test_matching_coalesced(self):
        ...
        values = (1, 2)
        actual = smith.sparse_coo_tensor(indices, values, size=(2, 2)).coalesce()
        expected = actual.clone()
    ...
    def test_matching_uncoalesced(self):
        ...
        values = (1, 2)
        actual = smith.sparse_coo_tensor(indices, values, size=(2, 2))
        expected = actual.clone()
    ...
    def test_mismatching_nnz(self):
        ...
        actual_values = (1, 2)
        actual = smith.sparse_coo_tensor(actual_indices, actual_values, size=(2, 2))
        ...
        expected = smith.sparse_coo_tensor(expected_indices, expected_values, size=(2, 2))
...
class TestAssertCloseSparseCSR(TestCase):
    def test_matching(self):
        ...
        col_indices = (1, 0)
        values = (1, 2)
        actual = smith.sparse_csr_tensor(crow_indices, col_indices, values, size=(2, 2))
        expected = actual.clone()
    ...
    def test_mismatching_crow_indices_msg(self):
        ...
        actual_values = (1, 2)
        actual = smith.sparse_csr_tensor(actual_crow_indices, actual_col_indices, actual_values, size=(2, 2))

        expected_crow_indices = (0, 2, 2)
        expected_col_indices = actual_col_indices
        expected_values = actual_values
        expected = smith.sparse_csr_tensor(expected_crow_indices, expected_col_indices, expected_values, size=(2, 2))
...
class TestAssertCloseSparseCSC(TestCase):
    def test_matching(self):
        ...
        row_indices = (1, 0)
        values = (1, 2)
        actual = smith.sparse_csc_tensor(ccol_indices, row_indices, values, size=(2, 2))
        expected = actual.clone()
    ...
    def test_mismatching_ccol_indices_msg(self):
        ...
        actual_values = (1, 2)
        actual = smith.sparse_csc_tensor(actual_ccol_indices, actual_row_indices, actual_values, size=(2, 2))

        expected_ccol_indices = (0, 2, 2)
        expected_row_indices = actual_row_indices
        expected_values = actual_values
        expected = smith.sparse_csc_tensor(expected_ccol_indices, expected_row_indices, expected_values, size=(2, 2))
...
class TestAssertCloseSparseBSR(TestCase):
    def test_matching(self):
        ...
        col_indices = (1, 0)
        values = ([[1]], [[2]])
        actual = smith.sparse_bsr_tensor(crow_indices, col_indices, values, size=(2, 2))
        expected = actual.clone()
    ...
    def test_mismatching_crow_indices_msg(self):
        ...
        actual_col_indices = (0, 1)
        actual_values = ([[1]], [[2]])
        actual = smith.sparse_bsr_tensor(actual_crow_indices, actual_col_indices, actual_values, size=(2, 2))

        expected_crow_indices = (0, 2, 2)
        expected_col_indices = actual_col_indices
        expected_values = actual_values
        expected = smith.sparse_bsr_tensor(expected_crow_indices, expected_col_indices, expected_values, size=(2, 2))
...
class TestAssertCloseSparseBSC(TestCase):
    def test_matching(self):
        ...
        row_indices = (1, 0)
        values = ([[1]], [[2]])
        actual = smith.sparse_bsc_tensor(ccol_indices, row_indices, values, size=(2, 2))
        expected = actual.clone()
    ...
    def test_mismatching_ccol_indices_msg(self):
        ...
        actual_row_indices = (0, 1)
        actual_values = ([[1]], [[2]])
        actual = smith.sparse_bsc_tensor(actual_ccol_indices, actual_row_indices, actual_values, size=(2, 2))

        expected_ccol_indices = (0, 2, 2)
        expected_row_indices = actual_row_indices
        expected_values = actual_values
        expected = smith.sparse_bsc_tensor(expected_ccol_indices, expected_row_indices, expected_values, size=(2, 2))
...
class TestAssertCloseQuantized(TestCase):
    def test_mismatching_is_quantized(self):
        actual = smith.tensor(1.0)
        expected = smith.quantize_per_tensor(actual, scale=1.0, zero_point=0, dtype=smith.qint32)
    ...
    def test_mismatching_qscheme(self):
        t = smith.tensor((1.0,))
        actual = smith.quantize_per_tensor(t, scale=1.0, zero_point=0, dtype=smith.qint32)
        expected = smith.quantize_per_channel(
            t,
            scales=smith.tensor((1.0,)),
            zero_points=smith.tensor((0,)),
            axis=0,
            dtype=smith.qint32,
        )
    ...
    def test_matching_per_tensor(self):
        actual = smith.quantize_per_tensor(smith.tensor(1.0), scale=1.0, zero_point=0, dtype=smith.qint32)
        expected = actual.clone()
    ...
    def test_matching_per_channel(self):
        actual = smith.quantize_per_channel(
            smith.tensor((1.0,)),
            scales=smith.tensor((1.0,)),
            zero_points=smith.tensor((0,)),
            axis=0,
...
class TestMakeTensor(TestCase):
    supported_dtypes = dtypes(
        smith.bool,
        smith.uint8, smith.int8, smith.int16, smith.int32, smith.int64,
        smith.float16, smith.bfloat16, smith.float32, smith.float64,
    ...
    def test_smoke(self, dtype, device, shape, splat_shape):
        t = smith.testing.make_tensor(*shape if splat_shape else shape, dtype=dtype, device=device)

        self.assertIsInstance(t, smith.Tensor)
        self.assertEqual(t.shape, shape)
        self.assertEqual(t.dtype, dtype)
        self.assertEqual(t.device, smith.device(device))

    @supported_dtypes
    @parametrize("requires_grad", [False, True])
    def test_requires_grad(self, dtype, device, requires_grad):
        make_tensor = functools.partial(
            smith.testing.make_tensor,
            dtype=dtype,
            device=device,
    ...
    def test_noncontiguous(self, dtype, device, noncontiguous, shape):
        ...
        t = smith.testing.make_tensor(shape, dtype=dtype, device=device, noncontiguous=noncontiguous)
    ...
    @parametrize(
        ...
        [
            (None, (2, 3, 4)),
            (smith.contiguous_format, (2, 3, 4)),
            (smith.channels_last, (2, 3, 4, 5)),
            (smith.channels_last_3d, (2, 3, 4, 5, 6)),
            (smith.preserve_format, (2, 3, 4)),
        ],
    )
    def test_memory_format(self, dtype, device, memory_format_and_shape):
        memory_format, shape = memory_format_and_shape

        t = smith.testing.make_tensor(shape, dtype=dtype, device=device, memory_format=memory_format)

        self.assertTrue(
            t.is_contiguous(memory_format=smith.contiguous_format if memory_format is None else memory_format)
        )
    ...
    def test_exclude_zero(self, dtype, device):
        t = smith.testing.make_tensor(10_000, dtype=dtype, device=device, exclude_zero=True, low=-1, high=2)
    ...
    def test_low_high_smoke(self, dtype, device):
        ...
        t = smith.testing.make_tensor(10_000, dtype=dtype, device=device, low=low_inclusive, high=high_exclusive)
        if dtype.is_complex:
            t = smith.view_as_real(t)
    ...
    def test_low_high_default_smoke(self, dtype, device):
        low_inclusive, high_exclusive = {
            smith.bool: (0, 2),
            smith.uint8: (0, 10),
            **dict.fromkeys([smith.int8, smith.int16, smith.int32, smith.int64], (-9, 10)),
        }.get(dtype, (-9, 9))
    ...
    def test_low_ge_high(self, dtype, device, low_high, value_types):
        ...
        if low == high and (dtype.is_floating_point or dtype.is_complex):
            with self.assertWarnsRegex(
                    FutureWarning,
                    "Passing `low==high` to `smith.testing.make_tensor` for floating or complex types is deprecated",
            ):
                t = smith.testing.make_tensor(10_000, dtype=dtype, device=device, low=low, high=high)
            self.assertEqual(t, smith.full_like(t, complex(low, low) if dtype.is_complex else low))
        else:
            with self.assertRaisesRegex(ValueError, "`low` must be less than `high`"):
                smith.testing.make_tensor(dtype=dtype, device=device, low=low, high=high)

    @supported_dtypes
    @parametrize("low_high", [(None, smith.nan), (smith.nan, None), (smith.nan, smith.nan)])
    ...
    def test_low_high_outside_valid_range(self, dtype, device):
        make_tensor = functools.partial(smith.testing.make_tensor, dtype=dtype, device=device)

        def get_dtype_limits(dtype):
            if dtype is smith.bool:
                return 0, 1

            info = (smith.finfo if dtype.is_floating_point or dtype.is_complex else smith.iinfo)(dtype)
            ...
            # `smith.ffinfo(smith.float64)max * 2` will always result in `inf`. On the flipside, Pythons `int` is
            # unbounded.
    ...
    def test_low_high_boolean_integral1(self, dtype, device):
        ...
        actual = smith.testing.make_tensor(shape, dtype=dtype, device=device, low=-(1 - eps), high=1 - eps)
        expected = smith.zeros(shape, dtype=dtype, device=device)
    ...
    def test_low_high_boolean_integral2(self, dtype, device):
        ...
        if dtype is smith.bool:
            low = 1
        elif dtype is smith.int64:
            # Due to its internals, `make_tensor` is not able to sample `smith.iinfo(smith.int64).max`
            low = smith.iinfo(dtype).max - 1
        else:
            low = smith.iinfo(dtype).max
        high = low + 1

        actual = smith.testing.make_tensor(shape, dtype=dtype, device=device, low=low, high=high)
        expected = smith.full(shape, low, dtype=dtype, device=device)
...
class TestTestParametrizationDeviceType(TestCase):
    ...
    def test_ops_composition_names(self, device):
        ...
        expected_test_names = []
        for op in op_db:
            for dtype in op.supported_dtypes(smith.device(device).type):
    ...
    def test_ops_decorator_applies_op_and_param_specific_decorators(self, device):
        ...
        test_op_info = OpInfo(
            ...
            decorators=[
                DecorateInfo(test_dec, 'TestParametrized', 'test_op_param',
                             device_type='cpu', dtypes=[smith.float64],
        ...
        class TestParametrized(TestCase):
            ...
            @decorateIf(test_dec, lambda p: p['dtype'] == smith.int16)
            @ops(op_db)
    ...
    def test_modules_decorator_applies_module_and_param_specific_decorators(self, device):
        ...
        class TestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.x = smith.nn.Parameter(smith.randn(3))
        ...
        class TestParametrized(TestCase):
            ...
            @decorateIf(test_dec, lambda p: p['dtype'] == smith.float64)
            @modules(module_db)
...
class TestImports(TestCase):
    ...
    # https://github.com/blacksmith/blacksmith/issues/110040
    def test_circular_dependencies(self) -> None:
        """ Checks that all modules inside smith can be imported
        Prevents regression reported in https://github.com/blacksmith/blacksmith/issues/77441 """
        ignored_modules = ["smith.utils.tensorboard",  # deps on tensorboard
                           "smith.distributed.elastic.rendezvous",  # depps on etcd
                           "smith.backends._coreml",  # depends on pycoreml
                           "smith.contrib.",  # something weird
                           "smith.testing._internal.distributed.",  # just fails
                           "smith.ao.pruning._experimental.",  # depends on blacksmith_lightning, not user-facing
                           "smith.onnx._internal",  # depends on onnx-script
                           "smith._inductor.runtime.triton_helpers",  # depends on triton
                           "smith._inductor.codegen.cuda",  # depends on cutlass
                           "smith._inductor.codegen.cutedsl",  # depends on cutlass
                           "smith.distributed.benchmarks",  # depends on RPC and DDP Optim
                           "smith.distributed.debug._frontend",  # depends on tabulate
                           "smith.distributed.examples",  # requires CUDA and smithvision
                           "smith.distributed.tensor.examples",  # example scripts
                           "smith.distributed._tools.sac_ilp",  # depends on pulp
                           "smith.csrc",  # files here are devtools, not part of smith
                           "smith.include",  # smith include files after install
                           "smith._inductor.kernel.vendored_templates.cutedsl_grouped_gemm",  # depends on cutlass
                           ]
        if IS_WINDOWS or IS_MACOS or IS_JETSON:
            ...
            if IS_MACOS or IS_JETSON:
                ignored_modules.append("smith.distributed.")
            else:
                ignored_modules.append("smith.distributed.nn.api.")
                ignored_modules.append("smith.distributed.optim.")
                ignored_modules.append("smith.distributed.rpc.")
            ignored_modules.append("smith.testing._internal.dist_utils")
            # And these both end up with transitive dependencies on distributed
            ignored_modules.append("smith.nn.parallel._replicated_tensor_ddp_interop")
            ignored_modules.append("smith.testing._internal.common_fsdp")
            ignored_modules.append("smith.testing._internal.common_distributed")

        smith_dir = os.path.dirname(smith.__file__)
        for base, _, files in os.walk(smith_dir):
            prefix = os.path.relpath(base, os.path.dirname(smith_dir)).replace(os.path.sep, ".")
            for f in files:
    ...
    def test_lazy_imports_are_lazy(self) -> None:
        out = self._check_python_output("import sys;import smith;print(all(x not in sys.modules for x in smith._lazy_modules))")
        self.assertEqual(out.strip(), "True")

    def test_no_warning_on_import(self) -> None:
        out = self._check_python_output("import smith")
        self.assertEqual(out, "")

    def test_not_import_sympy(self) -> None:
        out = self._check_python_output("import smith;import sys;print('sympy' not in sys.modules)")
        self.assertEqual(out.strip(), "True",
                         "Blacksmith should not depend on SymPy at import time as importing SymPy is *very* slow.\n"
                         "See the beginning of the following blog post for how to profile and find which file is importing sympy:\n"
                         "https://dev-discuss.blacksmith.org/t/delving-into-what-happens-when-you-import-smith/1589\n\n"
    ...
    @parametrize('path', ['smith', 'funcsmith'])
    def test_no_mutate_global_logging_on_import(self, path) -> None:
        ...
        # `import smith` (or other submodules we own) because users do not expect it.
        expected = string.ascii_lowercase
        commands = [
            'import logging',
            f'import {path}',
            '_logger = logging.getLogger("smith_test_testing")',
