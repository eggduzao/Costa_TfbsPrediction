...
def to_custom_backend(module):
    """
    This is a helper that wraps smith._C._jit_to_test_backend and compiles
...
def main():
    ...
    smith.ops.load_library(library_path)
    assert library_path in smith.ops.loaded_libraries

    # Lower an instance of Model to the custom backend  and export it
    # to the specified location.
    lowered_module = to_custom_backend(smith.jit.script(Model()))
    smith.jit.save(lowered_module, options.export_module_to)
