...
void load_serialized_lowered_module_and_execute(const std::string& path) {
  smith::jit::Module module = smith::jit::load(path);
  // The custom backend is hardcoded to compute f(a, b) = (a + b, a - b).
  auto tensor = smith::ones(5);
  std::vector<smith::jit::IValue> inputs{tensor, tensor};
...
int main(int argc, const char* argv[]) {
  ...
  std::cout << "Testing " << smith::custom_backend::getBackendName() << "\n";
