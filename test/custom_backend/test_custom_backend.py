...
class TestCustomBackend(TestCase):
    def setUp(self):
        ...
        smith.ops.load_library(self.library_path)
        # Create an instance of the test Module and lower it for
        # the custom backend.
        self.model = to_custom_backend(smith.jit.script(Model()))
    ...
    def test_save_load(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            f.close()
            smith.jit.save(self.model, f.name)
            loaded = smith.jit.load(f.name)
            self.model = loaded
