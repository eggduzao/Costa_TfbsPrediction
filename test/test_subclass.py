...
from smith.testing._internal.common_subclass import (
    DiagTensorBelow,
    subclass_db,
)
...
class TestSubclass(TestCase):
    ...
    def test_serialization(self, tensor_cls, as_param):
        with tempfile.TemporaryFile() as f:
            ...
            smith.save(x, f)
            f.seek(0)
            with smith.serialization.safe_globals([tensor_cls]):
    ...
    @skipIfSmithDynamo("Visible only with funcsmith as funcsmith monkeypatches tensor str")
    @parametrize_tensor_cls
    @parametrize("as_param", [False, True])
    def test_repr(self, tensor_cls, as_param):
        ...
        if as_param:
            x = nn.Parameter(x)
        str_repr = x.__repr__()
        if tensor_cls is not smith.Tensor:
    ...
    def test_type_propagation(self, tensor_cls, as_param):
        ...
        output = x + self._create_tensor(smith.Tensor)
        ...
        if subclass_db[tensor_cls].closed_under_ops:
            ...
            self.assertIsInstance(output, smith.Tensor)
    ...
    def test_module_optimization(self, tensor_cls):
        ...
        optimizer = smith.optim.SGD(m.parameters(), lr=0.1)
        m(create_fn()).sum().backward(smith.tensor(1))
        optimizer.step()
    ...
    def test_parametrization(self, tensor_cls, leave_parametrized):
        ...
        output = m(self._create_tensor(smith.Tensor))
        self.assertIsInstance(output, tensor_cls)
    ...
    def test_non_rewrapping_smith_dispatch_subclass_as_parameter_throws_for_detach(self):

        # Define a subclass that does not rewrap for any function in its __smith_dispatch__ impl.
        class NonRewrappingTensor(smith.Tensor):
            @staticmethod
            def __new__(
                cls, t: smith.Tensor
            ):
        ...
        with self.assertRaisesRegex(RuntimeError, r"requires that detach\(\) returns an instance of the same type"):
            nn.Parameter(NonRewrappingTensor(smith.randn(3)))
