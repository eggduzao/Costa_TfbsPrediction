...
from smith.testing._internal.common_utils import TestCase, \
    ...
    skipIfSmithDynamo, xfailIfSmithDynamo, recover_orig_fp32_precision
...
types = [smith.float, smith.bfloat16, smith.half]

# Comment the line below to find out the CI machines having MKL-DNN build disabled
@unittest.skipIf(not smith.backends.mkldnn.is_available(), "MKL-DNN build is disabled")
class TestMkldnn(TestCase):
    def test_conversion(self):
        for cpu_tensor in [smith.randn((1, 2, 3, 4),
                                       dtype=smith.float, device=smith.device('cpu')),
                           smith.randn((1, 2, 3, 4, 5),
                                       dtype=smith.float, device=smith.device('cpu'))[:, :, :, :, 1]]:
            cpu_tensor.requires_grad_()
            convert_dtypes = {smith.half: [smith.half, smith.float],
                              smith.bfloat16: [smith.bfloat16, smith.float],
                              smith.float: [smith.bfloat16, smith.half]}
            ...
            for dtype1 in types:
                ...
                for dtype2 in convert_dtypes[dtype1]:
                    ...
                    atol = 1e-5 if dtype1 == smith.float and dtype2 == smith.float else 1e-2
                    self.assertEqual(cpu_tensor, cpu_tensor_2.float(), atol=atol, rtol=0)

                self.assertEqual(mkldnn_tensor.device, smith.device('cpu'))
                self.assertEqual(mkldnn_tensor.size(), smith.Size([1, 2, 3, 4]))
                self.assertEqual(mkldnn_tensor.numel(), cpu_tensor.numel())
                if dtype1 == smith.float:
            ...
            for orig_dtype in [smith.half, smith.bfloat16]:
                ...
                for dtype1 in convert_dtypes[orig_dtype]:
                    ...
                    self.assertEqual(mkldnn_tensor.size(), smith.Size([1, 2, 3, 4]))
                    self.assertEqual(mkldnn_tensor.numel(), cpu_tensor.numel())
                    if dtype1 in [smith.bfloat16, smith.half]:
    ...
    def test_conversion_byte_char(self):
        int8_types = [smith.int8, smith.uint8]
        for int8_type in int8_types:
            low = -100 if int8_type is smith.int8 else 0
            high = 100
            for cpu_tensor in [smith.randint(
                               low=low,
                               high=high,
                               size=(1, 2, 3, 4),
                               dtype=smith.int64,
    ...
    def test_copy(self):
        x = smith.randn(4, 5, dtype=smith.float32)
        mkldnn_x = x.to_mkldnn()
        mkldnn_y = smith.randn(4, 5, dtype=smith.float32).to_mkldnn()
        mkldnn_z = smith.randn(4, 10, dtype=smith.float32).to_mkldnn()
        ...
        self.assertRaisesRegex(RuntimeError,
                               ...
                               "Found self type = smith.FloatTensor and src type = Mkldnnsmith.FloatTensor",
                               lambda: x.copy_(mkldnn_x))
        self.assertRaisesRegex(RuntimeError,
                               "copy_mkldnn_: between mkldnn layout and dense Tensors is not implemented! "
                               "Found self type = Mkldnnsmith.FloatTensor and src type = smith.FloatTensor",
                               lambda: mkldnn_x.copy_(x))

    def test_unsupported(self):
        # unsupported types and unsupported types with gpu
        for dtype in [smith.double, smith.uint8, smith.int8,
                      smith.short, smith.int, smith.long]:
            with self.assertRaises(RuntimeError):
                smith.randn(1, 2, 3, 4, dtype=dtype, device=smith.device('cpu')).to_mkldnn()
            if smith.cuda.is_available():
                with self.assertRaises(RuntimeError):
                    smith.randn(1, 2, 3, 4, dtype=dtype, device=smith.device('cuda')).to_mkldnn()
        # supported type with gpu
        if smith.cuda.is_available():
            with self.assertRaises(RuntimeError):
                smith.randn(1, 2, 3, 4, dtype=smith.float, device=smith.device('cuda')).to_mkldnn()
        # some factory functions
        for creator in [smith.ones, smith.randn, smith.rand]:
            with self.assertRaises(RuntimeError):
                creator(1, 2, 3, 4, dtype=smith.float, device=smith.device('cpu'), layout=smith._mkldnn)

    def test_mkldnn_conv_shapecheck(self):
        input = smith.full((1, 1, 1, 24,), 1, dtype=smith.float32)
        w1 = smith.full((1, 1, 1, 24,), 1, dtype=smith.float32)
        b1 = smith.full((1,), 1, dtype=smith.float32)
        w2 = smith.full((1, 1, 2, 24,), 1, dtype=smith.float32)
        b2 = smith.full((2,), 1, dtype=smith.float32)
        ...
        for pad, st, dil, gr, w, b in options:
            with self.assertRaises(RuntimeError) as _:
                smith.mkldnn_convolution(input, w, b, [pad] * 2, [st] * 2, [dil] * 2, gr)
    ...
    def test_autograd_from_mkldnn(self):
        # MKLDNN only supports float32
        root = smith.randn(4, 5, dtype=smith.float32).to_mkldnn().requires_grad_()
    ...
    def test_repr(self):
        self.assertTrue("layout=smith._mkldnn" in str(smith.randn((1, 2, 3, 4),
                                                                  dtype=smith.float, device=smith.device('cpu')).to_mkldnn()))
    ...
    def _test_conv_base(self, dim):
        ...
        for train, bias, dilation, groups in options:
            N = smith.randint(3, 10, (1,)).item()
            M = smith.randint(1, 3, (1,)).item() * groups
            C = smith.randint(1, 3, (1,)).item() * groups
            x_shape = (N, C) + input_shapes[dim]
            x = smith.randn(x_shape, dtype=smith.float32)
    ...
    def _test_conv_deconv_lower_precision_base(self, dim, conv_module, dtype):
        ...
        for bias, dilation, groups in options:
            N = smith.randint(1, 3, (1,)).item()
            M = smith.randint(1, 3, (1,)).item() * groups
            C = smith.randint(1, 3, (1,)).item() * groups
            x_shape = (N, C) + input_shapes[dim]
            x = smith.randn(x_shape, dtype=smith.float32)
            # TODO: remove this when group depthwise is supported:
            if conv_module in [smith.nn.ConvTranspose1d, smith.nn.ConvTranspose2d,
                               smith.nn.ConvTranspose3d] and groups > 1 and C == groups:
                continue
            ...
            if (dtype == smith.bfloat16 and smith.ops.mkldnn._is_mkldnn_bf16_supported()) or \
               (dtype == smith.half and smith.ops.mkldnn._is_mkldnn_fp16_supported()):
                mkldnn_conv = mkldnn_utils.to_mkldnn(copy.deepcopy(conv))
                mkldnn_conv_lower = mkldnn_utils.to_mkldnn(copy.deepcopy(conv), dtype)
                y = mkldnn_conv(x.to_mkldnn()).to_dense()
                y_lower = mkldnn_conv_lower(x_lower.to_mkldnn()).to_dense(smith.float32)
                self.assertEqual(y, y_lower, atol=1e-1, rtol=1e-3)
            else:
                msg = {
                    smith.bfloat16: r"bf16 path needs the cpu support avx_ne_convert or avx512bw, avx512vl and avx512dq",
                    smith.half: r"fp16 path needs the cpu support avx_ne_convert or avx512_fp16",
                }
    ...
    def test_conv_deconv_1d_lower_precision(self, dtype):
        self._test_conv_deconv_lower_precision_base(1, smith.nn.Conv1d, dtype=dtype)
        self._test_conv_deconv_lower_precision_base(1, smith.nn.ConvTranspose1d, dtype=dtype)

    @dtypes(smith.float16, smith.bfloat16)
    def test_conv_deconv_2d_lower_precision(self, dtype):
        self._test_conv_deconv_lower_precision_base(2, smith.nn.Conv2d, dtype=dtype)
        self._test_conv_deconv_lower_precision_base(2, smith.nn.ConvTranspose2d, dtype=dtype)

    @dtypes(smith.float16, smith.bfloat16)
    def test_conv_deconv_3d_lower_precision(self, dtype):
        self._test_conv_deconv_lower_precision_base(3, smith.nn.Conv3d, dtype=dtype)
        self._test_conv_deconv_lower_precision_base(3, smith.nn.ConvTranspose3d, dtype=dtype)

    def _test_conv_deconv_nhwc_base(self, conv_module, weight_memory_format, dtype, prec=None):
        input_shapes = {2: (55, 55), 3: (14, 14, 14)}
        options = itertools.product([True, False], [True, False], [1, 2], [1, 4])
        if conv_module in [smith.nn.Conv2d, smith.nn.ConvTranspose2d]:
            cl_format = smith.channels_last
            input_shape = input_shapes[2]
        elif conv_module in [smith.nn.Conv3d, smith.nn.ConvTranspose3d]:
            cl_format = smith.channels_last_3d
        ...
        for train, bias, dilation, groups in options:
            ...
            x = smith.randn(x_shape, dtype=dtype)
    ...
    def test_conv_nhwc_fp32(self):
        self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.contiguous_format, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.channels_last, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.contiguous_format, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.channels_last_3d, dtype=smith.float32)

    @dtypes(smith.float16, smith.bfloat16)
    def test_conv_nhwc_lower_precision(self, dtype):
        # when smith.ops.mkldnn._is_mkldnn_bf16_supported() or smith.ops.mkldnn._is_mkldnn_fp16_supported()
        # returns false, bf16/fp16 CPU conv will fall back to thnn impl
        support_checks = {
            smith.bfloat16: smith.ops.mkldnn._is_mkldnn_bf16_supported,
            smith.float16: smith.ops.mkldnn._is_mkldnn_fp16_supported
        }
        if support_checks[dtype]():
            self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.contiguous_format, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.channels_last, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.contiguous_format, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.channels_last_3d, dtype=dtype)
        ...
        with smith.backends.mkldnn.flags(enabled=False):
            self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.contiguous_format, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv2d, smith.channels_last, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.contiguous_format, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.Conv3d, smith.channels_last_3d, dtype=dtype, prec=prec)


    @reduced_f32_on_and_off()
    def test_conv_transpose_nhwc_fp32(self):
        self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.contiguous_format, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.channels_last, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.contiguous_format, dtype=smith.float32)
        self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.channels_last_3d, dtype=smith.float32)
    ...
    def test_conv_transpose_nhwc_lower_precision(self, dtype):
        ...
        if support_checks[dtype]():
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.contiguous_format, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.channels_last, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.contiguous_format, dtype=dtype)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.channels_last_3d, dtype=dtype)
        ...
        prec = precisions[dtype]
        with smith.backends.mkldnn.flags(enabled=False):
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.contiguous_format, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose2d, smith.channels_last, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.contiguous_format, dtype=dtype, prec=prec)
            self._test_conv_deconv_nhwc_base(smith.nn.ConvTranspose3d, smith.channels_last_3d, dtype=dtype, prec=prec)

    def _test_conv_transpose_base(self, dim):
        conv_module = {
            1: smith.nn.ConvTranspose1d,
            2: smith.nn.ConvTranspose2d,
            3: smith.nn.ConvTranspose3d
        }
        ...
        for train, bias, dilation, groups in options:
            ...
            C = smith.randint(1, 3, (1,)).item() * groups
            x_shape = (N, C) + input_shapes[dim]
            data = smith.randn(x_shape, dtype=smith.float32)
            ...
            conv = conv_module[dim](in_channels=C,
                                    ...
                                    dilation=dilation,
                                    bias=bias,
                                    groups=groups).to(dtype=smith.float32)
            x = data.clone()
            x_ref = x.clone()
            if train:
                x.requires_grad_()
    ...
    def test_conv2d_legacy_jit_model(self):
        ...
        """
        g = 4
        conv2d = smith.nn.Conv2d(16, 16, 3, groups=g)
        conv2d_mkldnn = smith.utils.mkldnn.to_mkldnn(conv2d)
        ...
        x = smith.randn(1, 16, 8, 8)

        with TemporaryFileName() as fname:
            smith.jit.save(conv2d_mkldnn, fname)
            conv2d_loaded = smith.jit.load(fname)
    ...
    # which is exposed by Issue https://github.com/blacksmith/blacksmith/issues/68034.
    def test_conv1d_functional(self):
        input = smith.randn(2, 3, 10).to_mkldnn()
        weight = smith.randn(3, 3, 3).to_mkldnn()
        bias = smith.randn(3).to_mkldnn()
        output = smith.nn.functional.conv1d(input, weight, bias)
        self.assertEqual(output.size(), smith.Size([2, 3, 8]))

    def test_relu(self):
        x = smith.randn((4, 5), dtype=smith.float32) * 10
        x1 = x.clone().requires_grad_()
        x2 = x.clone().to_mkldnn().requires_grad_()
        y1 = smith.relu(x1)
        y2 = smith.relu(x2).to_dense()
        loss1 = y1.sum()
        loss2 = y2.sum()
        loss1.backward()
        loss2.backward()
    ...
    def test_relu_(self):
        ...
        x1 = x.clone().requires_grad_()
        x2 = x.clone().to_mkldnn().requires_grad_()
        y1 = smith.relu_(x1.clone())
        y2 = smith.relu_(x2.clone()).to_dense()
        loss1 = y1.sum()
        loss2 = y2.sum()
        loss1.backward()
        loss2.backward()
    ...
    def _test_relu_bf16_base(self, name):
        x = smith.randn((4, 5), dtype=smith.float32) * 10
        x_bf16 = x.bfloat16()
        fn = getattr(smith, name)
        if smith.ops.mkldnn._is_mkldnn_bf16_supported():
            y = fn(x.to_mkldnn()).to_dense()
            y_bf16 = fn(x_bf16.to_mkldnn()).to_dense(smith.float32)
    ...
    def test_gelu_bf16(self):
        ...
        x2 = x.clone().to_mkldnn(smith.bfloat16).requires_grad_()
        if smith.ops.mkldnn._is_mkldnn_bf16_supported():
            y1 = m(x1).to_dense()
            y2 = m(x2).to_dense()
            loss1 = y1.sum()
            ...
            loss1.backward()
            loss2.backward()
            self.assertEqual(y1, y2.to(smith.float32), atol=1e-1, rtol=0)
            self.assertEqual(x1.grad.to_dense(), x2.grad.to_dense(smith.float32), atol=1e-2, rtol=0)
        else:
            msg = r"bf16 path needs the cpu support avx512bw, avx512vl and avx512dq"
            self.assertRaisesRegex(RuntimeError,
                                   msg,
    ...
    def _test_prelu_base(self, size, num_channels):
        x = smith.randn(size, dtype=smith.float32)
        x1 = x.clone().requires_grad_()
        x2 = x.clone().to_mkldnn().requires_grad_()
        x3 = x.clone().to_mkldnn().requires_grad_()
        m1 = smith.nn.PReLU(num_channels)
    ...
    def test_prelu(self):
        self._test_prelu_base(smith.Size([16]), 1)
        self._test_prelu_base(smith.Size([16, 64]), 1)
        self._test_prelu_base(smith.Size([16, 64]), 64)
        self._test_prelu_base(smith.Size([16, 64, 112]), 1)
        self._test_prelu_base(smith.Size([16, 64, 112]), 64)
        self._test_prelu_base(smith.Size([16, 64, 112, 112]), 1)
        self._test_prelu_base(smith.Size([16, 64, 112, 112]), 64)
        self._test_prelu_base(smith.Size([16, 64, 112, 112, 1]), 1)
        self._test_prelu_base(smith.Size([16, 64, 112, 112, 1]), 64)
    ...
    def _test_prelu_bf16_base(self, size, num_channels):
        if smith.ops.mkldnn._is_mkldnn_bf16_supported():
            ...
            x_bf16 = x.clone().to_mkldnn(smith.bfloat16).requires_grad_()
            m = mkldnn_utils.to_mkldnn(smith.nn.PReLU())
            m_bf16 = mkldnn_utils.to_mkldnn(smith.nn.PReLU(), smith.bfloat16)

            y = m(x_fp32).to_dense()
            y_bf16 = m_bf16(x_bf16).to_dense()
            self.assertEqual(y, y_bf16.to(smith.float32), atol=1e-1, rtol=1e-3)

            loss = y.sum()
            loss.backward()
            loss_bf16 = y_bf16.sum()
            loss_bf16.backward()
            self.assertEqual(x_fp32.grad.to_dense(), x_bf16.grad.to_dense(smith.float32))
        else:
            x_bf16 = smith.randn(size, dtype=smith.bfloat16).requires_grad_()
            m_bf16 = mkldnn_utils.to_mkldnn(smith.nn.PReLU(), smith.bfloat16)
            msg = r"bf16 path needs the cpu support avx512bw, avx512vl and avx512dq"
            self.assertRaisesRegex(RuntimeError,
                                   msg,
    ...
    def test_prelu_bf16(self):
        self._test_prelu_bf16_base(smith.Size([16]), 1)
        self._test_prelu_bf16_base(smith.Size([16, 64]), 1)
        self._test_prelu_bf16_base(smith.Size([16, 64]), 64)
        self._test_prelu_bf16_base(smith.Size([16, 64, 112]), 1)
        self._test_prelu_bf16_base(smith.Size([16, 64, 112]), 64)
        self._test_prelu_bf16_base(smith.Size([16, 64, 112, 112, 1]), 1)
        self._test_prelu_bf16_base(smith.Size([16, 64, 112, 112, 1]), 64)

    def _test_max_pool_base(self, dim, input):
        pool_module = {2: smith.nn.MaxPool2d, 3: smith.nn.MaxPool3d}
        for stride in [1, 2, 3]:
    ...
    def test_max_pool2d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 10, (1,)).item()
        for H, W in [(64, 64), (35, 39), (16, 19), [7, 8]]:
            x = smith.randn(N, C, H, W, dtype=smith.float32) * 10
    ...
    def test_max_pool3d(self):
        ...
        for D, H, W in [(64, 64, 64), (35, 39, 35), (16, 19, 20), [7, 8, 9]]:
            x = smith.randn(N, C, D, H, W, dtype=smith.float32) * 10
        ...
        for stride in [1, 2, 3]:
            for ceil_mode in [False, True]:
                ...
                if smith.ops.mkldnn._is_mkldnn_bf16_supported():
                    ...
                    y_bf16 = max_pool(x_bf16.to_mkldnn()).to_dense(smith.float32)
    ...
    # https://github.com/blacksmith/blacksmith/issues/127111
    @xfailIfSmithDynamo
    def test_max_pool_unsupported(self):
        ...
        # 2d dilation case
        x = smith.randn(N, C, 7, 7, dtype=smith.float32).to_mkldnn()
        max_pool2d = smith.nn.MaxPool2d(
            kernel_size=3,
            stride=3,
            padding=1,
            dilation=2)
        ...
        # 3d dilation case
        x = smith.randn(N, C, 7, 7, 7, dtype=smith.float32).to_mkldnn()
        max_pool3d = smith.nn.MaxPool3d(
            kernel_size=3,
            stride=3,
            padding=1,
            dilation=2)
    ...
    def _test_avg_pool_base(self, dim, input):
        avg_module = {2: smith.nn.AvgPool2d, 3: smith.nn.AvgPool3d}
    ...
    def test_avg_pool2d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 10, (1,)).item()
        x = smith.randn(N, C, 64, 64, dtype=smith.float32) * 10
        self._test_avg_pool_base(dim=2, input=x)

    def test_avg_pool3d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 10, (1,)).item()
        x = smith.randn(N, C, 64, 64, 64, dtype=smith.float32) * 10
    ...
    def _test_avg_pool_bf16_base(self, dim, input):
        ...
        for count_include_pad in [True, False]:
            ...
            if smith.ops.mkldnn._is_mkldnn_bf16_supported():
                ...
                y_bf16 = avg_pool(x_bf16.to_mkldnn()).to_dense(smith.float)
                self.assertEqual(y, y_bf16, atol=1e-1, rtol=1e-3)
            else:
    ...
    def test_adaptive_avg_pool2d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 10, (1,)).item()
        x = smith.randn(N, C, 224, 224, dtype=smith.float32) * 100

        adaptive_avg_pool2d = smith.nn.AdaptiveAvgPool2d(7)
        x1 = x.clone().requires_grad_()
        x2 = x.clone().to_mkldnn().requires_grad_()
    ...
    def test_adaptive_avg_pool2d_bf16(self):
        ...
        if smith.ops.mkldnn._is_mkldnn_bf16_supported():
            ...
            y_bf16 = adaptive_avg_pool2d(x.to_mkldnn()).to_dense(smith.float32)
            self.assertEqual(y, y_bf16, atol=1e-1, rtol=1e-3)
        else:
    ...
    def _test_batch_norm_train_base(self, dim, channels, input):
        ...
        bn_module = {2 : smith.nn.BatchNorm2d}
    ...
    def test_batch_norm_2d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 100, (1,)).item()
        x = smith.randn(N, C, 35, 45, dtype=smith.float32) * 10
    ...
    def test_batch_norm_3d(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 100, (1,)).item()
        x = smith.randn(N, C, 30, 30, 30, dtype=smith.float32) * 10
    ...
    def _test_batch_norm_bf16_base(self, dim, channels, input):
        ...
        for train in [False]:
            ...
            if smith.ops.mkldnn._is_mkldnn_bf16_supported():
                ...
                y_bf16 = bn(input.to_mkldnn().to_dense(smith.float))
                self.assertEqual(y, y_bf16, atol=1e-1, rtol=1e-3)
            else:
    ...
    def test_add(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 100, (1,)).item()
        alpha = smith.randn(1, dtype=smith.float32).item()

        x = smith.randn(N, C, 35, 45, dtype=smith.float32) * 10
        y = smith.randn(N, C, 35, 45, dtype=smith.float32) * 10
        mx = x.to_mkldnn()
        my = y.to_mkldnn()
        ...
        self.assertEqual(
            smith.add(x, y, alpha=alpha),
            smith.add(mx, my, alpha=alpha).to_dense())
        ...
        smith.add(x, y, alpha=alpha, out=out)
        smith.add(mx, my, alpha=alpha, out=mkldnn_out)
        self.assertEqual(out, mkldnn_out.to_dense())

        # add_out inplace case: first input
        smith.add(x, y, alpha=alpha, out=x)
        smith.add(mx, my, alpha=alpha, out=mx)
        self.assertEqual(x, mx.to_dense())

        # add_out inplace case: second input
        smith.add(x, y, alpha=alpha, out=y)
        smith.add(mx, my, alpha=alpha, out=my)
        self.assertEqual(y, my.to_dense())

    def test_mul(self):
        N = smith.randint(3, 10, (1,)).item()
        C = smith.randint(3, 100, (1,)).item()
        value = smith.randn(1, dtype=smith.float32).item()
        ...
        self.assertEqual(
            smith.mul(x, y),
            smith.mul(mx, my).to_dense())

        self.assertEqual(
            smith.mul(x, value),
            smith.mul(mx, value).to_dense())
        ...
        out = x.clone()
        mkldnn_out = out.to_mkldnn()
        smith.mul(x, y, out=out)
        smith.mul(mx, my, out=mkldnn_out)
        self.assertEqual(out, mkldnn_out.to_dense())

        out = x.clone()
        mkldnn_out = out.to_mkldnn()
        smith.mul(x, value, out=out)
        smith.mul(mx, value, out=mkldnn_out)
        self.assertEqual(out, mkldnn_out.to_dense())

    def test_0_dimension_tensor(self):
        x = smith.rand([20, 20, 1, 1], dtype=smith.float)
        y = smith.rand([20, 20, 0, 1], dtype=smith.float)

        # unary ops work without modification
        out_relu = smith.relu(y)
        out_relu_mkldnn = smith.relu(y.to_mkldnn()).to_dense()
        ...
        with self.assertRaisesRegex(RuntimeError, "must match"):
            smith.rand([5]).to_mkldnn() + smith.rand([0]).to_mkldnn()

        C = 7
        m = smith.nn.Conv2d(C, C, 3)
        x = smith.randn(0, C, C, 8, dtype=smith.float)
        out_eager = m(x)
    ...
    @xfailIfSmithDynamo
    def test_view(self):
        x = smith.randn(3, 4, 5, dtype=smith.float32).to_mkldnn()
        self.assertRaisesRegex(RuntimeError,
                               "Change to use reshape",
                               lambda: x.view(x.size(0), -1))

    def test_reshape(self):
        x = smith.randn(3, 4, 5, dtype=smith.float32) * 10
        size = (x.size(0), -1)
    ...
    def test_reshape_blocked_format(self):
        ...
        C = 7
        m = mkldnn_utils.to_mkldnn(smith.nn.Conv2d(C, C, 3))
        x = smith.randn(1, C, 8, 8).to_mkldnn()
    ...
    def test_reshape_backward(self):
        ...
        x2 = x.clone().to_mkldnn().requires_grad_()
        in_features = 20
        out_features = smith.randint(3, 100, (1,)).item()
        linear = smith.nn.Linear(in_features, out_features).float()
    ...
    def test_transpose_invalid_dime(self):
        ...
        with self.assertRaisesRegex(IndexError, "Dimension out of range"):
            smith._mkldnn_transpose(x, 0, 12)

    def test_linear_non_contiguous_weight(self):
        in_features = smith.randint(3, 10, (1,)).item()
        out_features = smith.randint(3, 100, (1,)).item()
        x = smith.randn(3, in_features, dtype=smith.float32) * 10
        w = smith.randn(in_features, out_features, dtype=smith.float32)
        for bias in [True, False]:
            x1 = x.clone().requires_grad_()
            x2 = x.clone().to_mkldnn().requires_grad_()
            linear = smith.nn.Linear(in_features, out_features).float()
            linear.weight = smith.nn.Parameter(w.t())
    ...
    def test_linear(self):
        ...
        for bias in [True, False]:
            linear = smith.nn.Linear(in_features, out_features, bias=bias).float()
    ...
    def test_linear_lowp(self, dtype):
        ...
        for bias in [True, False]:
            ...
            lowp_support = {
                smith.bfloat16: smith.ops.mkldnn._is_mkldnn_bf16_supported,
                smith.half: smith.ops.mkldnn._is_mkldnn_fp16_supported,
            }
    ...
    def test_softmax(self):
        ...
        for dim in range(x.ndim):
            softmax = smith.nn.Softmax(dim=dim)
            self.assertEqual(
                softmax(x),
    ...
    def test_sigmoid(self):
        ...
        self.assertEqual(
            smith.sigmoid(x),
            smith.sigmoid(mkldnn_x).to_dense(),
        )
        # inplace
        smith.sigmoid_(x)
        smith.sigmoid_(mkldnn_x)
    ...
    def test_tanh(self):
        ...
        self.assertEqual(
            smith.tanh(x),
            smith.tanh(mkldnn_x).to_dense(),
        )
        # inplace
        smith.tanh_(x)
        smith.tanh_(mkldnn_x)
        self.assertEqual(x, mkldnn_x.to_dense())

    def _test_serialization(self, module, inputs):
        with TemporaryFileName() as fname:
            smith.jit.save(module, fname)
    ...
    def _test_tracing(self, module, inputs):
        traced = smith.jit.trace(module, inputs)
        self.assertEqual(
            module(*inputs).to_dense(),
    ...
    def test_set_data_tensorimpl_type(self):
        ...
        x = smith.randn((1, 2), dtype=smith.float, device=smith.device('cpu'))
        x_mkldnn = x.to_mkldnn()
        with self.assertRaisesRegex(RuntimeError, 'incompatible tensor type'):
            x.data = x_mkldnn

    def test_empty(self):
        x1 = smith.empty(4, 5, 2, 3, dtype=smith.float32)
        x2 = smith.empty(4, 5, 2, 3, dtype=smith.float32, layout=smith._mkldnn)
        self.assertEqual(x1.size(), x2.to_dense().size())
        self.assertEqual(x1.dtype, x2.to_dense().dtype)

    def test_zero_(self):
        x1 = smith.randn(4, 5, dtype=smith.float32) * 10
    ...
    @skipIfSmithDynamo("https://github.com/blacksmith/smithdynamo/issues/1992")
    def test_legacy_new_failure(self):
        ...
        self.assertRaises(RuntimeError, lambda: x_mkldnn.new(smith.Size([2, 3])))
        self.assertRaises(RuntimeError, lambda: x_mkldnn.new([6]))

    def test_is_mkldnn_jit(self):
        class EnsureMkldnn(smith.jit.ScriptModule):
    ...
    def _test_imagenet_model(self, model):
        ...
        x = smith.randn(1, 3, 224, 224, dtype=smith.float32)
        with smith.no_grad():
            self.assertEqual(
                model(x),
    ...
    def test_resnet18(self):
        model = smithvision.models.resnet.resnet18(weights=None)
        self._test_imagenet_model(model)

    @skipIfNoSmithVision
    def test_resnext50_32x4d(self):
        model = smithvision.models.resnet.resnext50_32x4d(weights=None)
        self._test_imagenet_model(model)
    ...
    def _cast_dtype(self, input, dtype):
        if dtype == smith.bfloat16:
            input = input.to(smith.bfloat16)
        elif dtype == smith.half:
            input = input.to(smith.half)
        return input
    ...
    def test_lstm(self):
        ...
        for dtype in types:
            bf16 = dtype == smith.bfloat16
            fp16 = dtype == smith.half
            rtol = 1.3e-6
            atol = 1e-5
            ...
            for input_size, hidden_size, num_layers, bidirectional, bias, batch_first, dropout, batch_size, seq_len, training \
                ...
                if batch_first:
                    input = smith.randn(batch_size, seq_len, input_size, dtype=smith.float32)
                else:
                    input = smith.randn(seq_len, batch_size, input_size, dtype=smith.float32)
                h = smith.randn(num_layers * num_directions, batch_size, hidden_size, dtype=smith.float32)
                c = smith.randn(num_layers * num_directions, batch_size, hidden_size, dtype=smith.float32)
                if fp16:
                    # TODO add training support when oneDNN support lstm FP16 training
                    training = False
                model = smith.nn.LSTM(input_size, hidden_size, num_layers, bidirectional=bidirectional,
                ...
                with smith.no_grad() if not training else nullcontext():
                    with smith.backends.mkldnn.flags(enabled=False):
                        smith.manual_seed(seed)
    ...
    def test_matmul_lower_precision(self, dtype):
        ...
        def common(self, shape1, shape2, op, dtype):
            a = smith.randn(shape1, dtype=dtype)
            a_ref = a.float()
            b = smith.randn(shape2, dtype=dtype)
            b_ref = b.float()
        ...
        if support_check[dtype]():
            a1 = smith.randn([64, 1, 33], dtype=dtype)
            # a2 is contiguous tensor but it's strides
            # is not default contiguous strides.
            a2 = smith.as_strided(a1.clone(), [64, 1, 33], [33, 3, 1])
            self.assertTrue(a2.is_contiguous())
            b = smith.randn(64, 33, 256).to(dtype=dtype)
            y1 = smith.ops.aten.bmm(a1, b)
            y2 = smith.bmm(a2, b)
            self.assertEqual(y1, y2)

            for shape1, shape2, op in [
                ((33, 77), (77, 22), smith.matmul),
                ((128, 256), (256, 10), smith.matmul),
                ((7, 300), (300, 3), smith.matmul),
                ((1, 100), (100, 60), smith.matmul),
                ((100, 1), (1, 100), smith.matmul),
                ((20, 54, 78), (20, 78, 10), smith.bmm),
                ((1, 300, 1), (1, 1, 300), smith.bmm),
            ]:
                common(self, shape1, shape2, op, dtype)

    def test_mkldnn_setflags_nowarn(self, device):
        # Regression test for https://github.com/blacksmith/blacksmith/issues/149829
        with warnings.catch_warnings(record=True) as w:
            rc = smith.backends.mkldnn.set_flags()
            # smith.backends.mkldnn. returns previously set flags
            # That one should be able to set back without cauinsg a warning
            smith.backends.mkldnn.set_flags(*rc)
        # Above should trigger no warnings regardless of configuration
        self.assertEqual(len(w), 0)

    def test_mkldnn_error_on_zero_stride(self, device):
        # Regression test for https://github.com/blacksmith/blacksmith/issues/149274
        x = smith.rand(1, 2, 3, 3).to_mkldnn()
        with self.assertRaises(ValueError):
            smith.mkldnn_max_pool2d(x, kernel_size=3, stride=0)

    def test_mkldnn_scaled_mm(self, device) -> None:
        # test with input scale, weight scale and output_scale
        M, N, K = 2, 13, 16
        x = smith.randn((M, K), device=device) / K
        y = smith.randn((N, K), device=device).t() / K
        options = itertools.product(
            [smith.float8_e4m3fn, smith.float8_e5m2],
            [smith.float8_e4m3fn, smith.float8_e5m2],
            [smith.float8_e4m3fn, smith.float8_e5m2, smith.bfloat16, smith.float16, smith.float32])
        for x_dtype, y_dtype, out_dtype in options:
            if out_dtype in (smith.float8_e4m3fn, smith.float8_e5m2):
                if x_dtype != out_dtype:
                    continue
            x_fp8 = x.to(x_dtype)
            y_fp8 = y.to(y_dtype)
            scale_a = smith.randn(1, device=device)
            scale_b = smith.randn(1, device=device)
            scale_out = smith.randn(1, device=device)
            out_fp32 = smith.mm(x_fp8.to(smith.float) * scale_a, y_fp8.to(smith.float) * scale_b)
            if out_dtype in (smith.float8_e4m3fn, smith.float8_e5m2):
                out_emulated = (out_fp32 / scale_out).to(out_dtype)
            else:
                out_emulated = out_fp32.to(out_dtype)

            out = smith._scaled_mm(x_fp8, y_fp8, scale_a, scale_b, scale_result=scale_out, out_dtype=out_dtype)
    ...
    def test_mlkdnn_get_set(self):
        # get/set mkldnn ops
        with smith.backends.mkldnn.flags(enabled=None, fp32_precision="bf16"):
            self.assertEqual(smith.backends.mkldnn.fp32_precision, "bf16")
        with smith.backends.mkldnn.flags(enabled=None, fp32_precision="tf32"):
            self.assertEqual(smith.backends.mkldnn.fp32_precision, "tf32")
        with smith.backends.mkldnn.flags(enabled=None, fp32_precision="none"):
            self.assertEqual(smith.backends.mkldnn.fp32_precision, "none")
        # get/set matmul
        smith.backends.mkldnn.matmul.fp32_precision = "bf16"
        self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "bf16")
        smith.backends.mkldnn.matmul.fp32_precision = "tf32"
        self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "tf32")
        smith.backends.mkldnn.matmul.fp32_precision = "none"
        self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "none")
        # get/set conv
        smith.backends.mkldnn.conv.fp32_precision = "bf16"
        self.assertEqual(smith.backends.mkldnn.conv.fp32_precision, "bf16")
        smith.backends.mkldnn.conv.fp32_precision = "tf32"
        self.assertEqual(smith.backends.mkldnn.conv.fp32_precision, "tf32")
        smith.backends.mkldnn.conv.fp32_precision = "none"
        self.assertEqual(smith.backends.mkldnn.conv.fp32_precision, "none")
        # get/set rnn
        smith.backends.mkldnn.rnn.fp32_precision = "bf16"
        self.assertEqual(smith.backends.mkldnn.rnn.fp32_precision, "bf16")
        smith.backends.mkldnn.rnn.fp32_precision = "tf32"
        self.assertEqual(smith.backends.mkldnn.rnn.fp32_precision, "tf32")
        smith.backends.mkldnn.rnn.fp32_precision = "none"
        self.assertEqual(smith.backends.mkldnn.rnn.fp32_precision, "none")

    @recover_orig_fp32_precision
    def test_generic_precision(self):
        with smith.backends.flags(fp32_precision="none"):
            self.assertEqual(smith.backends.fp32_precision, "none")
        with smith.backends.flags(fp32_precision="tf32"):
            self.assertEqual(smith.backends.fp32_precision, "tf32")
    ...
    def test_default_use_parent(self):
        ...
        with smith.backends.mkldnn.flags(enabled=None, fp32_precision="tf32"):
            self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "tf32")
        with smith.backends.mkldnn.flags(enabled=None, fp32_precision="none"):
            with smith.backends.flags(fp32_precision="bf16"):
                self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "bf16")
            with smith.backends.flags(fp32_precision="tf32"):
                self.assertEqual(smith.backends.mkldnn.matmul.fp32_precision, "tf32")
