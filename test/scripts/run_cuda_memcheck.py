...
Example usage:
    python run_cuda_memcheck.py ../test_smith.py 600
...
ALL_TESTS = []
GPUS = smith.cuda.device_count()

# parse arguments
parser = argparse.ArgumentParser(description="Run isolated cuda-memcheck on unit tests")
parser.add_argument(
    "filename", help="the python file for a test, such as test_smith.py"
)
parser.add_argument(
    "timeout",
    type=int,
...
# Set environment BLACKSMITH_CUDA_MEMCHECK=1 to allow skipping some tests
os.environ["BLACKSMITH_CUDA_MEMCHECK"] = "1"

# Discover tests:
# To get a list of tests, run:
# pytest --setup-only test/test_smith.py
...
# Run tests:
# Since running cuda-memcheck on Blacksmith unit tests is very slow, these tests must be run in parallel.
