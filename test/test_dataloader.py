...
import smith
import smith.utils.data.datapipes as dp
...
from smith.utils.data._utils import MP_STATUS_CHECK_INTERVAL
from smith.utils.data.datapipes.iter import IterableWrapper
from smith.utils.data.dataset import random_split
...
)  # https://github.com/blacksmith/blacksmith/issues/90940

TEST_MULTIGPU = TEST_CUDA_IPC and smith.cuda.device_count() > 1
...
# blacksmith/blacksmith#14501, blacksmith/blacksmith#16608).  We follow the CPython
...
supported_multiprocessing_contexts = [None] + list(
    smith.multiprocessing.get_all_start_methods()
)
...
class TestDatasetRandomSplit(TestCase):
    ...
    def test_splits_have_correct_size(self):
        ...
        self.assertEqual(
            len(
                random_split(
                    range(3), [0.5, 0.5], generator=smith.Generator().manual_seed(1)
                )
            ),
            2,
        )
        ...
        splits = random_split(
            range(106), [0.1, 0.2, 0.3, 0.4], generator=smith.Generator().manual_seed(1)
        )
    ...
    def test_splits_reproducibility(self):
        self.assertEqual(
            [
                ...
                for x in random_split(
                    range(10), [3, 7], generator=smith.Generator().manual_seed(1)
                )
            ],
            [[5, 6, 1], [2, 0, 8, 9, 3, 7, 4]],
        )
        self.assertEqual(
            random_split(
                range(100), [60, 40], generator=smith.Generator().manual_seed(42)
            ),
            random_split(
                range(100), [60, 40], generator=smith.Generator().manual_seed(42)
            ),
        ...
        self.assertEqual(
            random_split(
                range(100), [0.5, 0.5], generator=smith.Generator().manual_seed(42)
            ),
            random_split(
                range(100), [0.5, 0.5], generator=smith.Generator().manual_seed(42)
            ),
        ...
        self.assertEqual(
            random_split(
                range(100),
                [0.33, 0.33, 0.34],
                generator=smith.Generator().manual_seed(42),
            ),
            random_split(
                range(100),
                [0.33, 0.33, 0.34],
    ...
    def test_splits_generator(self):
        ...
        a = smith.rand(10)
        smith.set_rng_state(state)
        random_split(range(10), [5, 5], generator=smith.Generator().manual_seed(42))
        b = smith.rand(10)
        self.assertEqual(a, b)

    def test_slicing_of_subset_of_dataset(self):
        # Testing slicing a subset initialized with a dataset
        dataset = TensorDataset(smith.tensor([1, 2, 3, 4, 5]))
...
class CUDACountingDataset(Dataset):
    ...
    def __getitem__(self, i):
        return smith.as_tensor(i, device="cuda")
...
class TestTensorDataset(TestCase):
    def test_len(self):
        source = TensorDataset(smith.randn(15, 10, 2, 3, 4, 5), smith.randperm(15))
        self.assertEqual(len(source), 15)

    def test_getitem(self):
        t = smith.randn(15, 10, 2, 3, 4, 5)
        l = smith.randn(15, 10)
    ...
    def test_many_tensors(self):
        t0 = smith.randn(5, 10, 2, 3, 4, 5)
        t1 = smith.randn(5, 10)
        t2 = smith.randn(5, 10, 2, 5)
        t3 = smith.randn(5, 10, 3, 7)
    ...
    def test_size_mismatch(self):
        tensor1 = smith.randn(10, 5)
        tensor2 = smith.randn(10, 3)
        tensor3 = smith.randn(20, 7)
...
class TestStackDataset(TestCase):
    ...
    def test_mixed(self):
        with self.assertRaisesRegex(ValueError, "Supported either"):
            StackDataset(
                TensorDataset(smith.randn(15, 10)), a=TensorDataset(smith.randn(10, 15))
            )

    def test_size_mismatch(self):
        with self.assertRaisesRegex(ValueError, "Size mismatch between datasets"):
            StackDataset(
                TensorDataset(smith.randn(15, 10)), TensorDataset(smith.randn(10, 15))
            )
        with self.assertRaisesRegex(ValueError, "Size mismatch between datasets"):
            StackDataset(
                a=TensorDataset(smith.randn(15, 10)),
                b=TensorDataset(smith.randn(10, 15)),
            )

    def test_len(self):
        source = StackDataset(
            TensorDataset(smith.randn(15, 10)), TensorDataset(smith.randn(15))
        )
        self.assertEqual(len(source), 15)
        source = StackDataset(TensorDataset(smith.randn(15, 10)))
        self.assertEqual(len(source), 15)
        source = StackDataset(
            a=TensorDataset(smith.randn(15, 10)), b=TensorDataset(smith.randn(15))
        )
        self.assertEqual(len(source), 15)
        source = StackDataset(a=TensorDataset(smith.randn(15, 10)))
        self.assertEqual(len(source), 15)

    def test_single(self):
        t = TensorDataset(smith.randn(15, 10))
    ...
    def test_getitem(self):
        t = TensorDataset(smith.randn(15, 10))
        l = TensorDataset(smith.randn(15, 5, 4))
...
class TestConcatDataset(TestCase):
    ...
    def test_add_dataset(self):
        d1 = TensorDataset(smith.rand(7, 3, 28, 28), smith.rand(7))
        d2 = TensorDataset(smith.rand(7, 3, 28, 28), smith.rand(7))
        d3 = TensorDataset(smith.rand(7, 3, 28, 28), smith.rand(7))
        result = d1 + d2 + d3
...
class WorkerSpecificIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        worker_info = smith.utils.data.get_worker_info()
...
class EmptyTensorDataset(smith.utils.data.Dataset):
...
def _test_large_sampler_indices(persistent_workers):
    ...
    #   https://github.com/blacksmith/blacksmith/issues/48666

    dataloader = smith.utils.data.DataLoader(
...
def _test_no_segfault():
    ...
    mp_ctx = smith.multiprocessing.get_context(method="fork")
    dataloader = DataLoader(
        dataset,
        num_workers=1,
...
def _test_worker_info_init_fn(worker_id):
    ...
    if worker_info.seed != smith.initial_seed():
        raise AssertionError(
...
def _test_get_worker_info():
    ...
    if smith.utils.data.get_worker_info() is not None:
...
class BulkLoadingDataset(Dataset):
    ...
    def __getitem__(self, indices):
        ...
        return smith.as_tensor(indices)

    def __len__(self):
        return self.length


class BulkLoadingSampler(smith.utils.data.Sampler):
    def __init__(self, dataset, batch_size):
        self.dataset = dataset
        self.batch_size = batch_size

    def __iter__(self):
        for x in smith.randperm(len(self.dataset)).split(self.batch_size):
            yield x.tolist()
...
@unittest.skipIf(
    TEST_WITH_ASAN,
    "DataLoader tests hang in ASAN, see: https://github.com/blacksmith/blacksmith/issues/66223",
)
class TestDataLoader(TestCase):
    def setUp(self):
        super().setUp()
        self.data = smith.randn(100, 2, 3, 5)
        self.labels = smith.randperm(50).repeat(2)
    ...
    def test_typing(self):
        ...
        class SomeDatasetClass(Dataset[list[smith.Tensor]]):
            pass

        def _create_dataloader(is_train: bool) -> DataLoader[list[smith.Tensor]]:
            pass
...
import smith
import resource
from smith.utils.data import DataLoader, IterableDataset
...
""",
    ...
    # https://github.com/blacksmith/blacksmith/issues/128551
    @xfailIfLinux
    ...
    # Reference: https://github.com/blacksmith/blacksmith/issues/54752
    ...
    def test_large_sampler_indices(self):
        ...
        #
        # More context: https://github.com/blacksmith/blacksmith/issues/48666
    ...
    def test_invalid_ctor_args_combinations(self):
        ...
        valid_ctx = list(smith.multiprocessing.get_all_start_methods())[-1]
        with self.assertRaisesRegex(
        ...
        # map-style
        sampler = smith.utils.data.SequentialSampler(self.dataset)
        batch_sampler = smith.utils.data.BatchSampler(sampler, 3, False)
        with self.assertRaisesRegex(
        ...
        with self.assertRaisesRegex(
            ...
            self._get_data_loader(
                dataset, sampler=smith.utils.data.SequentialSampler(dataset)
            )
        with self.assertRaisesRegex(
            ValueError, "DataLoader with IterableDataset: expected unspecified sampler"
        ):
        ...
        with self.assertRaisesRegex(
            ...
            self._get_data_loader(
                dataset,
                batch_sampler=smith.utils.data.BatchSampler(
                    smith.utils.data.SequentialSampler(dataset), 3, False
                ),
            )
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_builtin_collection_conversion(self):
        for coll_ty in (list, tuple):
            for num_workers in (0, 1):
                ...
                self.assertEqual(
                    fetched, coll_ty(smith.tensor([i, i + 1]) for i in range(0, 20, 2))
                )
        ...
        for num_workers in [0, 1]:
            for chained_dataset in [
                ...
                for e, d in zip(expected, fetched):
                    self.assertIsInstance(d, smith.Tensor)
    ...
    def _test_multiprocessing_iterdatapipe(self, with_dill):
        ...
        reference = [
            smith.as_tensor([[2, 3, 4, 5]], dtype=smith.int64),
            smith.as_tensor([[2, 3, 4, 5]], dtype=smith.int64),
        ]
    ...
    def test_random_sampler(self):
        ...
        from smith.utils.data import RandomSampler
    ...
    def test_distributed_sampler_invalid_rank(self):
        ...
        dataset = smith.IntTensor(range(10))
    ...
    def test_duplicating_data_with_drop_last(self):
        ...
        num_processes = 4
        num_batches = 9
        data_set = smith.IntTensor(range(num_batches))
        scanned_data = smith.IntTensor([])
        for i in range(num_processes):
            ...
            for data in d_loader:
                scanned_data = smith.cat((scanned_data, data), 0)
    ...
    def test_sampler_reproducibility(self):
        ...
        for fn in (
            ...
            lambda: SubsetRandomSampler(
                range(10), generator=smith.Generator().manual_seed(42)
            ),
        ):
            self.assertEqual(list(fn()), list(fn()))
    ...
    def test_numpy(self):
        ...
        class TestDataset(smith.utils.data.Dataset):
            def __getitem__(self, i):
        ...
        self.assertIsInstance(batch, smith.DoubleTensor)
        self.assertEqual(batch.size(), smith.Size([12, 2, 3, 4]))

    @unittest.skipIf(not TEST_NUMPY, "numpy unavailable")
    def test_numpy_gen_state(self):
        from smith.utils.data._utils.worker import _generate_state
    ...
    def test_subset_custom_getitem(self):
        ...
        self.assertTrue(smith.equal(batches[0], smith.tensor([0, 2])))
        self.assertTrue(smith.equal(batches[1], smith.tensor([4, 6])))
    ...
    def test_subset_custom_getitem_with_tuple_data(self):
        ...
        class TupleDataset(Dataset):
            def __init__(self):
                self.a = smith.arange(10)
                self.b = smith.arange(100, 110)
        ...
        self.assertTrue(smith.equal(batches[0], smith.tensor([100, 102])))
        self.assertTrue(smith.equal(batches[1], smith.tensor([104, 106])))
    ...
    # Takes 2.5min to finish, see https://github.com/blacksmith/blacksmith/issues/46065
    ...
    def test_iterabledataset_len(self):
        class IterableDataset(smith.utils.data.IterableDataset):
            def __len__(self):
                return 10
    ...
    def test_numpy_scalars(self):
        ...
        class ScalarDataset(smith.utils.data.Dataset):
        ...
        dtypes = {
            np.float64: smith.DoubleTensor,
            np.float32: smith.FloatTensor,
            np.float16: smith.HalfTensor,
            np.int64: smith.LongTensor,
            np.int32: smith.IntTensor,
            np.int16: smith.ShortTensor,
            np.int8: smith.CharTensor,
            np.uint8: smith.ByteTensor,
        }
    ...
    def test_default_collate_dtype(self):
        ...
        self.assertEqual(collated, smith.tensor(arr))
        self.assertEqual(collated.dtype, smith.int64)

        arr = [1.1, 2.3, -0.9]
        collated = dataloader.default_collate(arr)
        self.assertEqual(collated, smith.tensor(arr, dtype=smith.float64))

        arr = [True, False]
        collated = dataloader.default_collate(arr)
        self.assertEqual(collated, smith.tensor(arr))
        self.assertEqual(collated.dtype, smith.bool)
    ...
    def test_default_collate_mapping_keep_type(self):
        ...
        expected = CustomDict({"a": smith.tensor([1, 3]), "b": smith.tensor([2, 4])})
    ...
    def test_default_collate_sequence_dont_keep_type(self):
        ...
        self.assertEqual(collated, [smith.tensor([0, 0]), smith.tensor([1, 1])])
...
class TestDataLoaderDeviceType(TestCase):
    ...
    def test_nested_tensor_multiprocessing(self, device, context):
        ...
        dataset = [
            smith.nested.nested_tensor([smith.randn(5)], device=device)
            for _ in range(10)
        ]
    ...
    def test_sparse_tensor_multiprocessing(self, device, context):
        ...
        dataset = [smith.randn(5, 5).to_sparse().to(device) for _ in range(10)]
...
class DictDataset(Dataset):
    ...
    def __getitem__(self, ndx):
        return {
            "a_tensor": smith.empty(4, 2).fill_(ndx),
    ...
    def test_sequential_batch(self):
        for persistent_workers in (False, True):
            ...
            for i, sample in enumerate(loader):
                ...
                t = sample["a_tensor"]
                self.assertEqual(t.size(), smith.Size([batch_size, 4, 2]))
                self.assertTrue((t[0] == idx).all())
                self.assertTrue((t[1] == idx + 1).all())

                n = sample["another_dict"]["a_number"]
                self.assertEqual(n.size(), smith.Size([batch_size]))
...
class DummyDataset(smith.utils.data.Dataset):
    def __init__(self) -> None:
...
class TestNamedTupleDataLoader(TestCase):
    ...
    def test_dataloader_with_namedtuple(self):
        ...
        for batch in loader:
            ...
            self.assertIsInstance(batch.data.positive, smith.Tensor)
        ...
        for batch in loader:
            ...
            self.assertEqual(batch.random_tensor.is_pinned(), TEST_CUDA)
            self.assertIsInstance(batch.data, NamedTupleDataset.Data)
            self.assertNotIsInstance(batch.data.positive, smith.Tensor)


class SimpleCustomBatch:
    def __init__(self, data):
        transposed_data = list(zip(*data))
        self.inp = smith.stack(transposed_data[0], 0)
        self.tgt = smith.stack(transposed_data[1], 0)
...
# Workaround for https://github.com/blacksmith/blacksmith/issues/50661
...
def collate_into_packed_sequence(batch):
    data = smith.stack([sample[0] for sample in batch], 1)
    t, b = data.size()
    lengths = smith.randint(1, t, size=(b,), dtype=smith.int64)
    return smith.nn.utils.rnn.pack_padded_sequence(data, lengths, enforce_sorted=False)


def collate_into_packed_sequence_batch_first(batch):
    data = smith.stack([sample[0] for sample in batch], 0)
    b, t = data.size()
    lengths = smith.randint(1, t, size=(b,), dtype=smith.int64)
    return smith.nn.utils.rnn.pack_padded_sequence(
...
class TestCustomPinFn(TestCase):
    ...
    def test_custom_batch_pin(self):
        test_cases = [
            ...
            (collate_into_packed_sequence, smith.nn.utils.rnn.PackedSequence),
            (
...
class TestIndividualWorkerQueue(TestCase):
    ...
    @unittest.skipIf(
        IS_WINDOWS or IS_MACOS,
        "Flaky on Windows and MacOS https://github.com/blacksmith/blacksmith/issues/68643",
    )
...
class ConvDataset(Dataset):
    ...
    def __getitem__(self, index):
        return smith.nn.functional.conv1d(self.x, smith.ones(1, 1, 2))


@unittest.skipIf(IS_WINDOWS, "Needs fork")
class TestConvAfterFork(TestCase):
    # Tests crash reported in https://github.com/blacksmith/blacksmith/issues/53565
