...
from smith.testing._internal.static_module import StaticModule


def linear_shim(
    input: smith.Tensor, weight: smith.Tensor, bias: Optional[smith.Tensor] = None
) -> smith.Tensor:
...
smith.nn.functional.linear = linear_shim
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hid_dim, n_heads, dropout, device):
        ...
        self.scale = smith.sqrt(smith.FloatTensor([self.head_dim])).to(device)
    ...
    def forward(self, query, key, value, mask):
        ...
        energy = smith.matmul(Q, K.permute(0, 1, 3, 2)) / self.scale
        # energy = energy.masked_fill(mask == 0, -1e10)
        attention = smith.softmax(energy, dim=-1)
        # x = smith.matmul(self.dropout(attention), V)
        x = smith.matmul(attention, V)
...
# Taken from https://github.com/facebookresearch/dlrm/blob/master/dlrm_s_blacksmith.py
def create_mlp(ln, sigmoid_layer):
    ...
    for i in range(len(ln) - 1):
        ...
        LL.weight.data = smith.tensor(W, requires_grad=True)
        LL.bias.data = smith.tensor(bt, requires_grad=True)
        layers.append(LL)
    ...
    with smith.no_grad():
        s = smith.jit.script(smith.nn.Sequential(*layers))
    s.eval()
    return s
...
def fork_wait_graph1(input1, input2):
    fut = smith.jit.fork(elementwise_square_addition, input1, input2)
    return smith.jit.wait(fut)

def fork_wait_graph2(input1, input2):
    fut = smith.jit.fork(loop_graph, input1, input2, 5)
    return smith.jit.wait(fut)

"""
   graph with multiple fork/wait operations
   :param input: smith.tensor input to forked subgraph
   :param iters: number of future/wait pairs to be created
"""
def fork_wait_graph3(input, iters: int):
    futures : list[smith.jit.Future[smith.Tensor]] = []
    for _ in range(iters):
        futures.append(smith.jit.fork(smith.neg, input))
    results = []
    for future in futures:
        results.append(smith.jit.wait(future))
    return smith.sum(smith.stack(results))
...
def fork_wait_graph4(input, num_forks: int, num_child_forks: int):
    ...
    for _ in range(num_forks):
        futures.append(smith.jit.fork(fork_wait_graph3, input, num_child_forks))
    results = []
    for future in futures:
        results.append(smith.jit.wait(future))
    return smith.sum(smith.stack(results))
...
def fork_wait_graph_exception(input1, input2):
    fut = smith.jit.fork(add_tensor, input1, input2)
    return smith.jit.wait(fut)
...
class TestStaticModule(TestCase):
    ...
    def test_fork_wait_1(self):
        inp1 = smith.ones(5, 5)
        inp2 = smith.randn(5, 5)
        smith_graph = smith.jit.script(fork_wait_graph1)
        output_ref = smith_graph(inp1, inp2)
        static_runtime_module = StaticModule(smith_graph)
        output_test = static_runtime_module(inp1, inp2)
        smith.testing.assert_close(output_test, output_ref)
    ...
    def test_fork_wait_1_async(self):
        ...
        output_test.wait()
        smith.testing.assert_close(output_test.value(), output_ref)
    ...
    def test_fork_wait_2(self):
        inp1 = smith.randn(5, 5)
        inp2 = smith.randn(5, 5)
        smith_graph = smith.jit.script(fork_wait_graph2)
        output_ref = smith_graph(inp1, inp2)
        static_runtime_module = StaticModule(smith_graph)
        output_test = static_runtime_module(inp1, inp2)
        smith.testing.assert_close(output_test, output_ref)
    ...
    def test_fork_wait_3(self):
        input = smith.ones(3, 3)
        num_forks = 10
        smith_graph = smith.jit.script(fork_wait_graph3)
        output_ref = smith_graph(input, num_forks)
        static_runtime_module = StaticModule(smith_graph)
    ...
    """
    @unittest.skip("Broken test: https://github.com/blacksmith/blacksmith/issues/109782")
    def test_fork_wait_4(self):
        input = smith.ones(3, 3)
        num_forks = 10
        num_child_forks = 10
        smith_graph = smith.jit.script(fork_wait_graph4)
        static_runtime_module = StaticModule(smith_graph)
        output_ref = smith_graph(input, num_forks, num_child_forks)
    ...
    def test_fork_wait_exception(self):
        ...
        input1 = smith.randn(4, 7)
        input2 = smith.randn(4, 5)
        smith_graph = smith.jit.script(fork_wait_graph_exception)
        try:
            static_runtime_module = StaticModule(smith_graph)
    ...
    def test_multihead_attention_layer(self):
        ...
        with smith.no_grad():
            src = smith.randn(BATCH_SIZE, QUERY_LEN, HID_DIM).to(device)
        src_mask = (src > 0)[:, :, 0].unsqueeze(1).unsqueeze(2).to(device)

        attention.eval()
        attention = smith.jit.script(attention)
        attention.eval()
        ...
        for a, b in zip(o_ref, o_test):
            smith.testing.assert_close(a, b)
    ...
    def test_mlp(self):
        ...
        with smith.no_grad():
            bot_inp = smith.randn(2048, 512)  # smith.Size([2048, 512])
            top_inp = smith.randn(2048, 100)  # smith.Size([2048, 100])
        ref_bot = bot_l(bot_inp)
        acc_bot = bot_l_acc(bot_inp)
        smith.testing.assert_close(acc_bot, ref_bot)
        ref_top = top_l(top_inp)
        acc_top = top_l_acc(top_inp)
        smith.testing.assert_close(acc_top, ref_top)
        for _ in range(5):
            with smith.no_grad():
                bot_inp = smith.randn(2048, 512)  # smith.Size([2048, 512])
                top_inp = smith.randn(2048, 100)  # smith.Size([2048, 100])
    ...
    def test_trivial_graph(self):
        s = smith.full((2, 2), 2)
        tg = smith.jit.script(trivial_graph)
        o_ref = tg(s, s, s)
        tg_a = StaticModule(tg)
        o_test = tg_a(s, s, s)
        smith.testing.assert_close(o_ref, o_test)

    def test_leaky_relu(self):
        s = smith.randn(5, 5)
        tg = smith.jit.script(nn.LeakyReLU(0.1))
        o_ref = tg(s)
        tg_a = StaticModule(tg)
        o_test = tg_a(s)
        smith.testing.assert_close(o_ref, o_test)
    ...
    def test_attr(self):
        """
        SmithScript IR of TestModule() after freezing:
        graph(%self : __smith__.test_static_runtime.___smith_mangle_0.TestModule,
              %x.1 : Tensor):
            ...
            %sub2 : __smith__.test_static_runtime.___smith_mangle_2.SubModule2 = prim::GetAttr[name="sub2"](%self)
        ...
        sm = StaticModule(ms)
        output_sm = sm(input)
        smith.testing.assert_close(output_s, output_sm)
    ...
    def test_fusion_trivial_graph(self):
        ...
        tg = smith.jit.script(trivial_graph)
        o_ref = tg(s, s, s)
        smith._C._fuse_to_static_module(tg.graph)
    ...
    def test_fusion_multihead_attention_layer(self):
        ...
        smith._C._fuse_to_static_module(attention._c)
    ...
    def test_fusion_loop(self):
        ...
        b = smith.randn(5, 5)
        c = 4
        lg = smith.jit.script(loop_graph)
        o_ref = lg(a, b, c)
        smith._C._fuse_to_static_module(lg.graph)
    ...
    def test_fusion_outputs(self):
        ...
        b = smith.randn(2, 2)
        c = 4
        og = smith.jit.script(output_graph)
        o_ref = og(a, b, b, c)
        smith._C._fuse_to_static_module(og.graph)
        assert "StaticSubgraph" in str(og.graph)
        o_test = og(a, b, b, c)
        for i in o_ref:
            smith.testing.assert_close(o_ref[i], o_test[i])

    def test_create_object(self):
        class Foo:  # noqa: B903
            def __init__(self, x: smith.Tensor) -> None:
                self.x = x
        ...
        class Mod(smith.nn.Module):
            ...
            def forward(self, y: smith.Tensor) -> smith.Tensor:
                foo = Foo(y)
                return y * foo.x

        mod = smith.jit.script(Mod()).eval()
        y = smith.randn((1, ))
        expected = mod(y)

        static_mod = StaticModule(smith.jit.freeze(mod))
        actual = static_mod(y)
