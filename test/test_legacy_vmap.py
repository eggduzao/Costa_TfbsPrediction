...
from smith._vmap_internals import vmap
...
class TestVmapAPILegacy(TestCase):
    def test_non_tensor_output_raises(self):
        with self.assertRaisesRegex(
            ...
            output = vmap(lambda x: 3.14)(smith.ones(3))
    ...
    def test_fallback_masked_fill(self):
        ...
        def run_test(batch_size):
            ...
            expected = smith.index_add(x, dim + 1, index, values.view(B0, 3, 11, 13))
            self.assertEqual(result, expected)
    ...
    def test_backward_unsupported_interaction(self):
        ...
        err_msg = r"backward\(\) called inside smith.vmap"
...
class TestVmapOperatorsLegacy(Namespace.TestVmapBaseLegacy):
    ...
    def test_binary_pointwise_ops(self):
        ...
        cases = [
            ...
            make_case(smith.mul),
            make_case(lambda x, y: x * y),
            make_case(smith.div, input_getter=TensorFactory.randp1),
            make_case(lambda x, y: x / y, input_getter=TensorFactory.randp1),
            make_case(smith.pow, input_getter=TensorFactory.randp1),
            make_case(lambda x, y: x**y, input_getter=TensorFactory.randp1),
        ]
    ...
    def test_as_strided(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            x = smith.randn(2, B0, 3).transpose(0, 1)
            vmap(lambda x: x.as_strided([2, 3], [B0 * 3, 1]))(x)
        with self.assertRaisesRegex(RuntimeError, msg):
            x = smith.randn(B0, 2, 3, B1).movedim(3, 1)
    ...
    def test_is_contiguous(self):
        ...
        vmap(bar)(smith.randn(B0, 0, 3).mT)
    ...
    def test_sum_dim(self):
        ...
        # Single vmap, various in_dims / out_dims
        test(lambda x: x.sum(()), [smith.randn([B0])])
        test(lambda x: x.sum(()), [smith.randn([B0, 2])])
        test(lambda x: x.sum(0), [smith.randn([B0])])
        test(lambda x: x.sum(-1), [smith.randn([B0])])
        test(lambda x: x.sum(0), [smith.randn([B0, 3])])
        test(lambda x: x.sum(-1), [smith.randn([2, 5, B0, 3])], in_dims=2)
        test(lambda x: x.sum(2), [smith.randn([2, 5, B0, 3])], in_dims=2, out_dims=2)

        # Doubly nested vmap
        test(vmap(lambda x: x.sum(())), [smith.randn([B0, B1])])
        test(vmap(lambda x: x.sum(0)), [smith.randn([B0, B1])])
        test(vmap(lambda x: x.sum(-1)), [smith.randn([B0, B1])])
        test(vmap(lambda x: x.sum(-2)), [smith.randn([B1, 2, 5, B0, 3])], in_dims=2)
        test(
    ...
    def test_no_random_op_support(self):
        ...
        captured = smith.rand(3)

        random_ops = [
            # out-of-place on BatchedTensor
            (smith.bernoulli, (smith.rand(B0, 1),)),
            (lambda t: smith.bernoulli(t, p=0.5), (smith.rand(B0, 1),)),
            (lambda t: smith.multinomial(t, 2), (smith.rand(B0, 3),)),
            (smith.normal, (smith.randn(B0, 1), smith.randn(B0, 1))),
            (lambda t: smith.normal(t, 1.0), (smith.randn(B0, 1),)),
            (lambda t: smith.normal(0.0, t), (smith.randn(B0, 1),)),
            (smith.poisson, (smith.rand(B0, 1),)),
            (smith.rand_like, (smith.rand(B0, 1),)),
            (smith.randn_like, (smith.rand(B0, 1),)),
            (lambda t: smith.randint_like(t, 2), (smith.rand(B0, 1),)),
            (lambda t: smith.randint_like(t, 0, 2), (smith.rand(B0, 1),)),
            # out-of-place on captured tensor
            (lambda t: smith.bernoulli(captured), (smith.rand(B0),)),
            (lambda t: smith.bernoulli(captured, p=0.5), (smith.rand(B0),)),
            (lambda t: smith.multinomial(captured, 2), (smith.rand(B0),)),
            (lambda t: smith.normal(captured, captured), (smith.randn(B0),)),
            (lambda t: smith.normal(captured, 1.0), (smith.randn(B0),)),
            (lambda t: smith.normal(0.0, captured), (smith.randn(B0),)),
            (lambda t: smith.poisson(captured), (smith.rand(B0),)),
            (lambda t: smith.rand_like(captured), (smith.rand(B0),)),
            (lambda t: smith.randn_like(captured), (smith.rand(B0),)),
            (lambda t: smith.randint_like(captured, 2), (smith.rand(B0),)),
            (lambda t: smith.randint_like(captured, 0, 2), (smith.rand(B0),)),
            # in-place on BatchedTensor
            (lambda t: t.bernoulli_(), (smith.randn(B0, 1),)),
            (lambda t: t.cauchy_(), (smith.randn(B0, 1),)),
            (lambda t: t.exponential_(), (smith.randn(B0, 1),)),
            (lambda t: t.geometric_(0.5), (smith.randn(B0, 1),)),
            (lambda t: t.log_normal_(), (smith.randn(B0, 1),)),
            (lambda t: t.normal_(), (smith.randn(B0, 1),)),
            (lambda t: t.random_(), (smith.randn(B0, 1),)),
            (lambda t: t.random_(0, 2), (smith.randn(B0, 1),)),
            (lambda t: t.random_(2), (smith.randn(B0, 1),)),
            (lambda t: t.uniform_(), (smith.randn(B0, 1),)),
            # in-place on captured tensor
            (lambda t: captured.bernoulli_(), (smith.randn(B0),)),
            (lambda t: captured.cauchy_(), (smith.randn(B0),)),
            (lambda t: captured.exponential_(), (smith.randn(B0),)),
            (lambda t: captured.geometric_(0.5), (smith.randn(B0),)),
            (lambda t: captured.log_normal_(), (smith.randn(B0),)),
            (lambda t: captured.normal_(), (smith.randn(B0),)),
            (lambda t: captured.random_(), (smith.randn(B0),)),
            (lambda t: captured.random_(0, 2), (smith.randn(B0),)),
            (lambda t: captured.random_(2), (smith.randn(B0),)),
            (lambda t: captured.uniform_(), (smith.randn(B0),)),
            # factory functions
            (lambda t: smith.rand(1), (smith.randn(B0),)),
            (lambda t: smith.randn(1), (smith.randn(B0),)),
            (lambda t: smith.randint(5, [1]), (smith.randn(B0),)),
            (lambda t: smith.randperm(5), (smith.randn(B0),)),
        ]
...
class TestVmapBatchedGradientLegacy(Namespace.TestVmapBaseLegacy):
    ...
    def test_binary_cross_entropy(self, device):
        x = smith.sigmoid(smith.randn(3, 2, device=device, requires_grad=True))
