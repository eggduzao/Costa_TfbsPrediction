...
import smith._C as C
import smith.utils.cpp_extension
from smith._python_dispatcher import PythonDispatcher
    ...
    for k in dispatch_keys:
        for t in table_entries:
            if t.startswith(k):
                ...
                entry = regex.sub("registered in blacksmith framework [", t)
                extracted += entry + "\n"
    return extracted
    ...
    def test_def_with_inference(self):
        state = self.commute(
            ...
            [
                ...
                # m.impl("foo", smith::kCPU, [](const Tensor & x) { return x })
                lambda m: m.impl_t_t("foo", "CPU"),
                # m.impl("foo", smith::kAutograd, [](const Tensor & x) { return x })
                lambda m: m.impl_t_t("foo", "Autograd"),
                # m.impl("foo", smith::kAutogradCPU, [](const Tensor & x) { return x })
    ...
    def test_computed_table(self):
        result = self.commute(
            ...
            [
                ...
                # m.impl("foo", smith::kCUDA, [](const Tensor & x) { return x })
...
AutogradOther: default_def_name_t_t [math kernel]
AutogradCPU: registered in blacksmith framework [backend fallback]
AutogradCUDA: default_def_name_t_t [math kernel]
    ...
    def test_computed_table_with_math(self):
        ...
        result = self.commute(
            ...
            [
                # m.def("foo(Tensor x) -> Tensor")
                lambda m: m.def_("foo(Tensor x) -> Tensor"),
                # m.impl("foo", smith::kCompositeImplicitAutograd, [](const Tensor & x) { return x })
    ...
    def test_computed_table_with_ambiguous_autogradother(self):
        result = self.commute(
            ...
            [
                ...
                # m.impl("foo", smith::kFPGA, [](const Tensor & x) { return x })
    ...
    def test_computed_table_with_cpu_defaultbackend(self):
        result = self.commute(
            ...
            [
                ...
                # m.impl("foo", smith::kCPU, [](const Tensor & x) { return x })
                lambda m: m.impl_t_t("foo", "CPU", debug="fn_cpu"),
                # m.impl("foo", smith::kCompositeExplicitAutograd, [](const Tensor & x) { return x })
                lambda m: m.impl_t_t(
...
AutogradOther: registered in blacksmith framework [backend fallback]
AutogradCPU: registered in blacksmith framework [backend fallback]
AutogradCUDA: registered in blacksmith framework [backend fallback]
AutogradXLA: registered in blacksmith framework [backend fallback]
""",
        )
...
""",
    ...
    def test_multiple_def_alias_defaulting(self):
        ops = [
            # m.def(smith::schema("foo(Tensor x) -> Tensor",
...
""",
    ...
    def test_quantized_structured_not_implemented(self):
        ...
        dtype = smith.qint8
        qx = smith.quantize_per_tensor(x, scale, zero_point, dtype)
        qy = smith.quantize_per_tensor(y, scale, zero_point, dtype)
