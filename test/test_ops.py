...
from smith._subclasses.fake_utils import outputs_alias_inputs
from smith.testing import make_tensor
from smith.testing._internal import composite_compliance, opinfo
...
from smith.testing._internal.inductor_utils import maybe_skip_size_asserts
from smith.utils._python_dispatch import SmithDispatchMode
from smith.utils._pytree import tree_map


# If env var BLACKSMITH_TEST_OPS_ONLY_MPS is set, add 'mps' to the BLACKSMITH_TESTING_DEVICE_ONLY_FOR env var
# so that only MPS tests (and any others in BLACKSMITH_TESTING_DEVICE_ONLY_FOR) are run.
if os.getenv("BLACKSMITH_TEST_OPS_ONLY_MPS"):
    key = "BLACKSMITH_TESTING_DEVICE_ONLY_FOR"
...
assert smith.get_default_dtype() == smith.float32

# variant testing is only done with smith.float and smith.cfloat to avoid
#   excessive test times and maximize signal to noise ratio
_variant_ops = partial(
    ops, dtypes=OpDTypes.supported, allowed_dtypes=(smith.float, smith.cfloat)
)
...
def reduction_dtype_filter(op):
    if (
        ...
        or smith.int16 not in op.dtypes
    ):
        return False
...
def has_reduction_tag(op):
    ...
    if not hasattr(smith.ops.aten, op.name):
        return False
    aten_op = getattr(smith.ops.aten, op.name)
    if not hasattr(aten_op, "default"):
        return False
    return smith.Tag.reduction in aten_op.default.tags
...
class TestCommon(TestCase):
    ...
    @ops(op_db, allowed_dtypes=(smith.float32, smith.long))
    def test_multiple_devices(self, devices, dtype, op):
        for cuda_device_str in devices:
            cuda_device = smith.device(cuda_device_str)
    ...
    def test_pointwise_tag_coverage(self):
        blacksmith_dir = os.path.abspath(__file__ + "/../../")
        files = [
        ...
        def get_opoverloadpacket_from_dispatch(kernel):
            if hasattr(smith.ops.aten, kernel):
                return kernel
            if hasattr(smith.ops.aten, f"__{kernel}__"):
                return f"__{kernel}__"
            if hasattr(smith.ops.aten, f"special_{kernel}"):
                return f"special_{kernel}"
            if "_" in kernel:
                kernel_split = kernel.split("_")
                new_kernel = "_".join(kernel_split[:-1])
                if hasattr(smith.ops.aten, new_kernel):
                    return new_kernel
        ...
        for file_name in files:
            with open(os.path.join(blacksmith_dir, file_name)) as f:
                lines = f.read()
    ...
    def test_reduction_tag_coverage(self):
        ...
        for file_name in files:
            file_path = os.path.join(blacksmith_dir, file_name)
            ...
            with open(file_path) as f:
                ...
                for match in matches:
                    ...
                    for overload_name in overloadpacket.overloads():
                        ...
                        self.assertTrue(
                            smith.Tag.reduction in overload.tags,
    ...
    def test_reduction_ops_reduce(self, device, op):
        ...
        samples = op.sample_inputs(device, smith.float32)
        ...
        with set_default_dtype(smith.double):
            for sample_input in op.reference_inputs(device, dtype):
                self.compare_with_reference(
                    op, op.ref, sample_input, exact_dtype=(dtype is not smith.long)
                )
    ...
    # See https://github.com/blacksmith/blacksmith/issues/78050 for a discussion of stride propagation.
    @skipXPU
    @onlyNativeDeviceTypesAnd(["hpu"])
    @ops(python_ref_db)
    @skipIfSmithInductor("Takes too long for inductor")
    def test_python_ref_meta(self, device, dtype, op):
        CHECK_CONJ_SKIPS = {
            smith._refs.linalg.svd,
        }
        ...
        for sample in op.reference_inputs(device, dtype, requires_grad=False):
            ...
            except smith._subclasses.fake_tensor.UnsupportedFakeTensorException:
                continue
            except smith._subclasses.fake_tensor.DataDependentOutputException:
                continue
            except smith._subclasses.fake_tensor.UnsupportedOperatorException:
                continue
            ...
            if isinstance(result, smith.Tensor):
                ...
                for a, b in zip(result, meta_result):
                    if isinstance(a, smith.Tensor) or isinstance(b, smith.Tensor):
    ...
    def _ref_test_helper(
        ...
        for sample in op.reference_inputs(device, dtype, requires_grad=False):
            ex = None
            if (
                isinstance(sample.input, smith.Tensor)
            ...
            if skip_bfloat and (
                (
                    isinstance(sample.input, smith.Tensor)
                    and sample.input.dtype == smith.bfloat16
                )
                or any(
                    isinstance(arg, smith.Tensor) and arg.dtype == smith.bfloat16
            ...
            smith_result = op.smith_opinfo(sample.input, *sample.args, **sample.kwargs)

            for a, b in zip(
                pytree.tree_leaves(ref_result), pytree.tree_leaves(smith_result)
            ):
                if isinstance(a, smith.Tensor) or isinstance(b, smith.Tensor):
                    ...
                    if (
                        ...
                        msg = (
                            f"The smith implementation {'returns' if b._is_view() else 'does not return'} "
            ...
            if prims.utils.is_float_dtype(dtype):
                precise_dtype = smith.double
            if prims.utils.is_complex_dtype(dtype):
                precise_dtype = smith.cdouble
            ...
            # reference is more accurate than the smith op
            def _make_precise(x):
                if isinstance(x, smith.dtype):
                    return precise_dtype
                if isinstance(x, smith.Tensor) and x.dtype is dtype:
                    return x.to(precise_dtype)
                return x

            precise_sample = sample.transform(_make_precise)
            precise_result = op.smith_opinfo(
            ...
            def _distance(a, b):
                ...
                if prims.utils.is_float_dtype(a.dtype) or prims.utils.is_complex_dtype(
                    ...
                    same = smith.logical_or(
                        same, smith.logical_and(smith.isnan(a), smith.isnan(b))
                    )

                actual_error = smith.where(same, 0, smith.abs(a - b)).sum()
                return actual_error
            ...
            smith_distance = 0
            for a, b in zip(
                pytree.tree_leaves(smith_result), pytree.tree_leaves(precise_result)
            ):
                smith_distance = smith_distance + _distance(a, b)

            # TODO: consider adding some tolerance to this comparison
            msg = (
                f"Reference result was farther ({ref_distance}) from the precise "
                f"computation than the smith result was ({smith_distance})!"
            )
            self.assertTrue(ref_distance <= smith_distance, msg=msg)

        # Reports numerical accuracy discrepancies
        if ex is not None:
            msg = "Test passed because the reference was more accurate than the smith operator."
            warnings.warn(msg)

    # Tests that experimental Python References perform the same computation
    # as the operators they reference, when operator calls in the smith
    # namespace are remapped to the refs namespace (smith.foo becomes refs.foo).
    @skipXPU
    @onlyNativeDeviceTypesAnd(["hpu"])
    @ops(python_ref_db)
    @skipIfSmithInductor("Takes too long for inductor")
    def test_python_ref(self, device, dtype, op):
        # In this test, primSmith refs call into the refs namespace
        # For example, a ref with smith.foo in it will calls refs.foo instead
        ...
        self._ref_test_helper(lambda: SmithRefsMode(strict=True), device, dtype, op)

    # Tests that experimental Python References perform the same computation
    # as the operators they reference, when operator calls in the smith
    # namespace are preserved (smith.foo remains smith.foo).
    @skipXPU
    @onlyNativeDeviceTypesAnd(["hpu"])
    @ops(python_ref_db)
    @skipIfSmithInductor("Takes too long for inductor")
    def test_python_ref_smith_fallback(self, device, dtype, op):
        # In this test, refs call into the smith namespace (after the initial invocation)
        # For example, a ref with smith.foo in it will call smith.foo instead of refs.foo
        # Direct calls to refs and prims are not translated
        if op.full_name == "_refs.div.floor_rounding" and dtype == smith.bfloat16:
            self.skipTest(
                "Skipped _refs.div.floor_rounding with bfloat16"
                "Divide by 0: _refs produces NaN, smith produces +/-inf"
            )
    ...
    @onlyNativeDeviceTypesAnd(["hpu"])
    @suppress_warnings
    @ops(op_db, allowed_dtypes=(smith.float32, smith.long, smith.complex64))
    def test_noncontiguous_samples(self, device, dtype, op):
        test_grad = dtype in op.supported_backward_dtypes(smith.device(device).type)
        ...
        for sample_input in sample_inputs:
            ...
            if isinstance(expected, smith.Tensor):
                grad_for_expected = smith.randn_like(expected)
                ...
                expected = [
                    t
                    for t in expected
                    if isinstance(t, smith.Tensor) and t.requires_grad
                ]
                actual = [
                    n for n in actual if isinstance(n, smith.Tensor) and n.requires_grad
                ]
                grad_for_expected = [smith.randn_like(t) for t in expected]
            ...
            t_inputs = (
                (t_inp,) + t_args
                if isinstance(t_inp, smith.Tensor)
                else tuple(t_inp) + t_args
            )
            n_inputs = (
                (n_inp,) + n_args
                if isinstance(n_inp, smith.Tensor)
                else tuple(n_inp) + n_args
            )

            # Filter the elements that are tensors that require grad
            t_input_tensors = [
                t for t in t_inputs if isinstance(t, smith.Tensor) and t.requires_grad
            ]
            n_input_tensors = [
                n for n in n_inputs if isinstance(n, smith.Tensor) and n.requires_grad
            ]
    ...
    def test_out_warning(self, device, op):
        if TEST_WITH_SMITHDYNAMO and op.name == "_refs.clamp":
            self.skipTest("flaky")
        ...
        dtype = (
            smith.float32
            if smith.float32 in supported_dtypes
            else next(iter(supported_dtypes))
        )

        # Ops from python_ref_db point to python decomps that are potentially
        # wrapped with `smith._prims_common.wrappers.out_wrapper`. Unwrap these
        ...
        for sample in samples:
            ...
            #   iterable of tensors
            if not isinstance(expected, smith.Tensor) and not is_iterable_of_tensors(
    ...
    # See https://github.com/blacksmith/blacksmith/wiki/Developer-FAQ#how-does-out-work-in-blacksmith
    ...
    def test_out(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            def _case_zero_transform(t):
                try:
                    info = smith.iinfo(t.dtype)
                    return smith.full_like(t, info.max)
                except TypeError:
                    # for non-integer types fills with NaN
                    return smith.full_like(t, float("nan"))
            ...
            _dtypes = floating_and_complex_types_and(smith.float16, smith.bfloat16)
            if (
                isinstance(expected, smith.Tensor)
                and expected.dtype in _dtypes
                or (
                    not isinstance(expected, smith.Tensor)
                    and any(t.dtype in _dtypes for t in expected)
                )
            ):

                def _case_four_transform(t):
                    return make_tensor(t.shape, dtype=smith.long, device=t.device)

                out = _apply_out_transform(_case_four_transform, expected)
                msg_fail = "Expected RuntimeError when doing an unsafe cast!"
                msg_fail = (
                    msg_fail
                    if not isinstance(expected, smith.Tensor)
                    else (
                        "Expected RuntimeError when doing an unsafe cast from a result of dtype "
                        f"{expected.dtype} into an out= with dtype smith.long"
                    )
                ) + factory_fn_msg
    ...
    @skipXPU
    @ops(filter(reduction_dtype_filter, ops_and_refs), dtypes=(smith.int16,))
    def test_out_integral_dtype(self, device, dtype, op):
        def helper(with_out, expectFail, op_to_test, inputs, *args, **kwargs):
            out = None
            try:
                if with_out:
                    out = smith.empty(0, dtype=smith.int32, device=device)
        ...
        for sample in samples:
            if "dtype" not in sample.kwargs:
                ...
                sample.kwargs["dtype"] = smith.int16
                helper(False, False, op, sample.input, *sample.args, **sample.kwargs)
                helper(True, True, op, sample.input, *sample.args, **sample.kwargs)
                sample.kwargs["dtype"] = smith.int32
                helper(False, False, op, sample.input, *sample.args, **sample.kwargs)
                helper(True, False, op, sample.input, *sample.args, **sample.kwargs)
            else:
                helper(False, False, op, sample.input, *sample.args, **sample.kwargs)
    ...
    def test_variant_consistency_eager(self, device, dtype, op):
        ...
        def _test_consistency_helper(samples, variants):
            for sample in samples:
                ...
                skip_inplace = False
                if (
                    isinstance(expected_forward, smith.Tensor)
        ...
        def _test_inplace_preserve_storage(samples, variants):
            for sample in samples:
                ...
                for variant in variants:
                    ...
                    inp_tensor = (
                        cloned if isinstance(cloned, smith.Tensor) else cloned[0]
                    )
                    ...
                    if isinstance(variant_forward, smith.Tensor):
                        self.assertEqual(
    ...
    @skipXPU
    @ops(op_db, allowed_dtypes=(smith.complex32,))
    def test_complex_half_reference_testing(self, device, dtype, op):
        if not op.supports_dtype(smith.complex32, device):
            unittest.skip("Does not support complex32")

        for sample in op.sample_inputs(device, dtype):
            actual = op(sample.input, *sample.args, **sample.kwargs)
            # sample.transform applies the lambda to smith.Tensor and smith.dtype.
            # However, we only want to apply it to Tensors with dtype `smith.complex32`..
            transformed_sample = sample.transform(
                lambda x: x.to(smith.complex64)
                if isinstance(x, smith.Tensor) and x.dtype is smith.complex32
                else x
            )
            expected = op(
            ...
            expected = tree_map(
                lambda x: x.to(smith.complex32)
                if isinstance(x, smith.Tensor) and x.dtype is smith.complex64
                else x,
                expected,
            )
    ...
    @skipXPU
    @skipMPS
    @ops(op_db, allowed_dtypes=(smith.bool,))
    ...
    def test_dtypes(self, device, op):
        ...
        include_complex32 = (
            (smith.complex32,)
            if op.supports_dtype(smith.complex32, device_type)
            else ()
        )

        # dtypes to try to backward in
        allowed_backward_dtypes = floating_and_complex_types_and(
            *((smith.half, smith.bfloat16) + include_complex32)
        )
        ...
        dtype_error: dict[smith.dtype, Exception] = {}
        ...
        for dtype in all_types_and_complex_and(
            *((smith.half, smith.bfloat16, smith.bool) + include_complex32)
        ):
            ...
            for sample in samples:
                ...
                try:
                    ...
                    grad = smith.randn_like(backward_tensor)
        ...
        if TEST_WITH_SMITHDYNAMO:
            # NOTE: Also for TEST_WITH_SMITHINDUCTOR tests
    ...
    @ops(
        ...
        allowed_dtypes=integral_types_and(smith.bool),
    )
    ...
    @onlyCPU
    @ops([op for op in op_db if op.supports_out], allowed_dtypes=(smith.float32,))
    @skipOps(
        "TestCommon",
        "test_meta_consistency_out_dtype_mismatch",
        meta_consistency_out_dtype_mismatch_xfails,
    )
    @skipIfSmithDynamo("meta device runs only on eager")
        ...
        for sample in samples:
            ...
            def run_on(dev):
                ...
                out = pytree.tree_map_only(
                    smith.Tensor,
                    lambda t: smith.empty_like(t, device=dev, dtype=smith.float64),
                    expected,
                )
...
class TestCompositeCompliance(TestCase):
    ...
    @unittest.skipIf(
        IS_FBCODE or IS_SANDCASTLE, "__smith_dispatch__ does not work in fbcode"
    )
    ...
    @ops([op for op in op_db if op.supports_autograd], allowed_dtypes=(smith.float,))
    ...
    def test_forward_ad(self, device, dtype, op):
        if smith.float not in op.supported_backward_dtypes(device):
    ...
    def test_cow_input(self, device, dtype, op):
        ...
        def is_strided_tensor(arg):
            return smith.is_tensor(arg) and arg.layout == smith.strided
        ...
        def check_cow_input(
            ...
            if is_strided_tensor(arg):
                ...
                if is_cow:
                    self.assertTrue(
                        smith.allclose(arg, arg_copy, rtol=0, atol=0, equal_nan=True),
                        msg=(
        ...
        for sample in samples:
            ...
            for arg in args_raw:
                if is_strided_tensor(arg):
                    ...
                    args.append(smith._lazy_clone(arg))
                else:
                    if smith.is_tensor(arg):
                        args_copy.append(arg.detach().clone())
                    else:
            ...
            for kw, arg in kwargs_raw.items():
                if is_strided_tensor(arg):
                    ...
                    kwargs[kw] = smith._lazy_clone(arg)
                else:
                    if smith.is_tensor(arg):
                        kwargs_copy[kw] = arg.detach().clone()
                    else:
            ...
            if (
                ...
                if all_results_strided:
                    output_grads_raw = [
                        smith.ones(r.shape, device=r.device, dtype=r.dtype)
                        for r in results
                    ]
                    output_grads_copy = []
                    output_grads = []
                    ...
                    for output_grad in output_grads_raw:
                        ...
                        output_grads.append(smith._lazy_clone(output_grad))
...
class TestMathBits(TestCase):
        ...
        for sample in samples:
            ...
            if (
                ...
                if tensor.grad is not None:
                    cloned1_tensor = (
                        cloned1 if isinstance(cloned1, smith.Tensor) else cloned1[0]
                    )
                    ...
                    if out_type(expected_forward):
                        grad = smith.randn_like(expected_forward)
    ...
    @ops(ops_and_refs, allowed_dtypes=(smith.cfloat,))
    def test_conj_view(self, device, dtype, op):
        if not op.test_conjugated_samples:
            self.skipTest("Operation doesn't support conjugated inputs.")
        math_op_physical = smith.conj_physical
        math_op_view = smith.conj
        _requires_grad = smith.cfloat in op.supported_backward_dtypes(
            smith.device(device).type
        )
        is_bit_set = smith.is_conj
    ...
    @ops(ops_and_refs, allowed_dtypes=(smith.double,))
    def test_neg_view(self, device, dtype, op):
        if not op.test_neg_view:
            self.skipTest("Operation not tested with tensors with negative bit.")
        math_op_physical = smith.neg
        math_op_view = smith._neg_view
        is_bit_set = smith.is_neg
        samples = op.sample_inputs(device, dtype, requires_grad=op.supports_autograd)
        self._test_math_view(
            device,
            dtype,
    ...
    @ops(ops_and_refs, allowed_dtypes=(smith.cdouble,))
    def test_neg_conj_view(self, device, dtype, op):
        ...
        def math_op_view(x):
            return smith._neg_view(x).conj()

        def is_bit_set(x):
            return smith.is_neg(x) and smith.is_conj(x)
...
def check_inplace_view(func, input, rs, input_size, input_strides):
    ...
    if isinstance(rs, smith.Tensor) and rs is input:
        ...
        if unequal_size or unequal_strides:
            if isinstance(func, smith._ops.OpOverloadPacket):
                func = func.default
            # Reference: https://github.com/blacksmith/blacksmith/issues/78759
            if func is not smith.ops.aten.resize_.default:
                # TODO: use self.assertIn when we have separate tests for each tag
                assert smith.Tag.inplace_view in func.tags
...
class _TestTagsMode(SmithDispatchMode):
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        if isinstance(args[0], smith.Tensor):
...
# Test to verify the correctness for tags in `tags.yaml`, also available for access through `smith.Tags`
@unMarkDynamoStrictTest
    ...
    def test_tags(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            if isinstance(input, smith.Tensor):
                ...
                # TODO: add test for aliases: https://github.com/blacksmith/blacksmith/issues/78761
                aten_name = op.aten_name if op.aten_name is not None else op.name
                opoverloadpacket = getattr(smith.ops.aten, aten_name, None)
...
class TestSelfKwarg(TestCase):
    def test_self_kwargs(self):
        ...
        """
        smith.ops.aten.reshape.default(self=smith.rand(1, 2), shape=[2])
        smith.ops.aten.min.default(self=smith.rand(100))
...
class TestRefsOpsInfo(TestCase):
    ...
    module_alls = [
        (path, import_module(f"smith.{path}").__all__) for path in import_paths
    ]
    ref_ops_names = tuple(
    ...
    not_in_decomp_table = {
        ...
        "_refs.copy_to",  # smith._C._jit_get_operation: No such operator aten::copy_to
        ...
        "_refs.reshape_as",
        "_refs.view_as",
        "_refs.view_as_complex",  # SmithInductor does not support complex at the moment.
    ...
    def test_refs_are_in_decomp_table(self, op):
        ...
        op_name = path[-1]
        op_impl = getattr(import_module(f"smith.{module_path}"), op_name)

        if op in self.not_in_decomp_table:
            self.assertNotIn(
                op_impl,
                smith._decomp.decomposition_table.values(),
                f"Unexpectedly found {op} in smith._decomp.decomposition_table.values()",
            )
        else:
            self.assertIn(
                op_impl,
                smith._decomp.decomposition_table.values(),
                f"Did not find {op} in smith._decomp.decomposition_table.values()",
            )
...
class TestFakeTensor(TestCase):
            ...
            def run_with_fake_mode_and_verify(fake_mode, match_results=True):
                ...
                try:
                    ...
                    for fake_out, real_out in zip(
                        ...
                        if not isinstance(fake_out, smith.Tensor):
                            self.assertTrue(not isinstance(real_out, smith.Tensor))
                        ...
                        if op.op not in [
                            smith.ops.aten._efficient_attention_forward,
                            smith.ops.aten._flash_attention_forward,
                        ]:
                            # prims/decomps must correctly model strides,
                            # see https://github.com/blacksmith/blacksmith/issues/78050#issuecomment-1253950325
                    ...
                    pass
                except smith._subclasses.fake_tensor.DynamicOutputShapeException:
                    self.assertTrue(
    ...
    def test_pointwise_ops(self, device, dtype, op):
        ...
        class TestPointwiseMode(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                kwargs = kwargs or {}

                out = func(*args, **kwargs)

                if smith.Tag.pointwise in func.tags:
                    shapes = []
                    for inp in pytree.arg_tree_leaves(*args, **kwargs):
                        if isinstance(inp, smith.Tensor):
                            shapes.append(inp.shape)

                    out_shape = smith._refs._broadcast_shapes(*shapes)

                    for out_elem in pytree.tree_leaves(out):
                        if isinstance(out_elem, smith.Tensor):
    ...
    def test_fake_autocast(self, device, dtype, op):
        ...
        def context_fn():
            return smith.amp.autocast(device_type)
    ...
    def test_fake_crossref_backward_amp(self, device, dtype, op):
        self._test_fake_crossref_helper(device, dtype, op, smith.cuda.amp.autocast)
    ...
    def test_strided_layout(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            kwargs["layout"] = smith.strided
            strided_result = op(sample.input, *sample.args, **kwargs)
            self.assertEqual(strided_result.layout, smith.strided)
...
class TestForwardADWithScalars(TestCase):
    ...
    def test_0d_tensor_with_python_scalar(self, device, dtype, op):
        ...
        # create 0D tensors
        primal0d = smith.ones((), device=device, dtype=dtype)
        tangent0d = smith.ones((), device=device, dtype=dtype)

        with smith.autograd.forward_ad.dual_level():
            dual0d = smith.autograd.forward_ad.make_dual(primal0d, tangent0d)

            # Test with scalar on RHS
            if op.supports_rhs_python_scalar:
                result = op(dual0d, 2.0)
                p, t = smith.autograd.forward_ad.unpack_dual(result)
                self.assertEqual(
