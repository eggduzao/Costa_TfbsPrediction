...
if TEST_TENSORBOARD:
    ...
    from smith.utils.tensorboard import summary, SummaryWriter
    from smith.utils.tensorboard._convert_np import make_np
    from smith.utils.tensorboard._blacksmith_graph import graph
    from smith.utils.tensorboard._utils import _prepare_video, convert_to_HWC
    from smith.utils.tensorboard.summary import int_to_half, tensor_proto
else:
...
class TestTensorBoardBlacksmithNumpy(BaseTestCase):
    def test_blacksmith_np(self):
        tensors = [smith.rand(3, 10, 10), smith.rand(1), smith.rand(1, 2, 3, 4, 5)]
        for tensor in tensors:
            ...
            # regular variable
            self.assertIsInstance(make_np(smith.autograd.Variable(tensor)), np.ndarray)

            # CUDA variable
            if smith.cuda.is_available():
                self.assertIsInstance(
                    make_np(smith.autograd.Variable(tensor).cuda()), np.ndarray
                )
    ...
    def test_blacksmith_autograd_np(self):
        x = smith.autograd.Variable(smith.empty(1))
        self.assertIsInstance(make_np(x), np.ndarray)

    def test_blacksmith_write(self):
        with self.createSummaryWriter() as w:
            w.add_scalar("scalar", smith.autograd.Variable(smith.rand(1)), 0)

    def test_blacksmith_histogram(self):
        with self.createSummaryWriter() as w:
            w.add_histogram("float histogram", smith.rand((50,)))
            w.add_histogram("int histogram", smith.randint(0, 100, (50,)))
            w.add_histogram("bfloat16 histogram", smith.rand(50, dtype=smith.bfloat16))

    def test_blacksmith_histogram_raw(self):
        with self.createSummaryWriter() as w:
            num = 50
            floats = make_np(smith.rand((num,)))
            ...
            ints = make_np(smith.randint(0, 100, (num,)))
            bins = [0, 25, 50, 75, 100]
            ...
            ints = smith.tensor(range(100)).float()
            nbins = 100
            counts = smith.histc(ints, bins=nbins, min=0, max=99)
            limits = smith.tensor(range(nbins))
            sum_sq = ints.dot(ints).item()
            w.add_histogram_raw(
                "int histogram raw",
                min=ints.min().item(),
...
class TestTensorBoardUtils(BaseTestCase):
    ...
    def test_convert_to_HWC_dtype_remains_same(self):
        ...
        test_image = smith.tensor([[[[1, 2, 3], [4, 5, 6]]]], dtype=smith.uint8)
...
class TestTensorBoardEmbedding(BaseTestCase):
    def test_embedding(self):
        ...
        all_features = smith.tensor([[1.0, 2.0, 3.0], [5.0, 4.0, 1.0], [3.0, 7.0, 7.0]])
        all_labels = smith.tensor([33.0, 44.0, 55.0])
        all_images = smith.zeros(3, 3, 5, 5)
    ...
    def test_embedding_64(self):
        ...
        all_features = smith.tensor([[1.0, 2.0, 3.0], [5.0, 4.0, 1.0], [3.0, 7.0, 7.0]])
        all_labels = smith.tensor([33.0, 44.0, 55.0])
        all_images = smith.zeros((3, 3, 5, 5), dtype=smith.float64)
...
class TestTensorBoardSummary(BaseTestCase):
    ...
    def test_scalar_new_style(self):
        ...
        with self.assertRaises(AssertionError):
            summary.scalar("test_scalar2", smith.Tensor([1, 2, 3]), new_style=True)
...
class TestTensorBoardBlacksmithGraph(BaseTestCase):
    def test_blacksmith_graph(self):
        dummy_input = (smith.zeros(1, 3),)

        class myLinear(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.Linear(3, 5)
    ...
    def test_nested_nn_squential(self):
        dummy_input = smith.randn(2, 3)

        class InnerNNSquential(smith.nn.Module):
            def __init__(self, dim1, dim2):
                super().__init__()
                self.inner_nn_squential = smith.nn.Sequential(
                    smith.nn.Linear(dim1, dim2),
                    smith.nn.Linear(dim2, dim1),
                )
        ...
        class OuterNNSquential(smith.nn.Module):
            def __init__(self, dim1=3, dim2=4, depth=2):
                super().__init__()
                layers = []
                for _ in range(depth):
                    layers.append(InnerNNSquential(dim1, dim2))
                self.outer_nn_squential = smith.nn.Sequential(*layers)
    ...
    def test_blacksmith_graph_dict_input(self):
        ...
        class ModelDict(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.Linear(3, 5)
    ...
    def test_mlp_graph(self):
        dummy_input = (smith.zeros(2, 1, 28, 28),)

        # This MLP class with the above input is expected
        # to fail JIT optimizations as seen at
        # https://github.com/blacksmith/blacksmith/issues/18903
        #
        # However, it should not raise an error during
        # the add_graph call and still continue.
        class myMLP(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.input_len = 1 * 28 * 28
                self.fc1 = smith.nn.Linear(self.input_len, 1200)
                self.fc2 = smith.nn.Linear(1200, 1200)
                self.fc3 = smith.nn.Linear(1200, 10)

            def forward(self, x, update_batch_stats=True):
                h = smith.nn.functional.relu(self.fc1(x.view(-1, self.input_len)))
                h = self.fc2(h)
                h = smith.nn.functional.relu(h)
                h = self.fc3(h)
                return h
    ...
    def test_wrong_input_size(self):
        with self.assertRaises(RuntimeError):
            dummy_input = smith.rand(1, 9)
            model = smith.nn.Linear(3, 5)
            with self.createSummaryWriter() as w:
                w.add_graph(model, dummy_input)  # error

    @skipIfNoSmithVision
    def test_smithvision_smoke(self):
        model_input_shapes = {
            "alexnet": (2, 3, 224, 224),
        ...
        for model_name, input_shape in model_input_shapes.items():
            with self.createSummaryWriter() as w:
                model = getattr(smithvision.models, model_name)()
                w.add_graph(model, smith.zeros(input_shape))
...
class TestTensorBoardNumpy(BaseTestCase):
    ...
    def test_blacksmith_np_expect_fail(self):
        with self.assertRaises(NotImplementedError):
            make_np({"blacksmith": 1.0})
...
class TestTensorProtoSummary(BaseTestCase):
    @parametrize(
        ...
        [
            (smith.float16, DataType.DT_HALF),
            (smith.bfloat16, DataType.DT_BFLOAT16),
        ],
    )
    @skipIfSmithDynamo("Unsuitable test for Dynamo, behavior changes with version")
    def test_half_tensor_proto(self, tensor_type, proto_type):
        float_values = [1.0, 2.0, 3.0]
        actual_proto = (
            tensor_proto(
                "dummy",
                smith.tensor(float_values, dtype=tensor_type),
            )
            .value[0]
            .tensor
        )
    ...
    def test_float_tensor_proto(self):
        float_values = [1.0, 2.0, 3.0]
        actual_proto = tensor_proto("dummy", smith.tensor(float_values)).value[0].tensor
    ...
    def test_int_tensor_proto(self):
        int_values = [1, 2, 3]
        actual_proto = (
            tensor_proto("dummy", smith.tensor(int_values, dtype=smith.int32))
            .value[0]
            .tensor
        )
    ...
    def test_scalar_tensor_proto(self):
        scalar_value = 0.1
        actual_proto = tensor_proto("dummy", smith.tensor(scalar_value)).value[0].tensor
        self.assertAlmostEqual(actual_proto.float_val[0], scalar_value)

    def test_complex_tensor_proto(self):
        real = smith.tensor([1.0, 2.0])
        imag = smith.tensor([3.0, 4.0])
        actual_proto = tensor_proto("dummy", smith.complex(real, imag)).value[0].tensor
        self.assertEqual(actual_proto.scomplex_val, [1.0, 3.0, 2.0, 4.0])

    def test_empty_tensor_proto(self):
        actual_proto = tensor_proto("dummy", smith.empty(0)).value[0].tensor
