...
import smith
import smith.xpu._gpu_trace as gpu_trace
...
from smith.testing._internal.common_methods_invocations import ops_and_refs
...
TEST_MULTIXPU = smith.xpu.device_count() > 1

cpu_device = smith.device("cpu")
xpu_device = smith.device("xpu")
...
class TestXpu(TestCase):
    ...
    def test_device_behavior(self):
        current_device = smith.xpu.current_device()
        smith.xpu.set_device(current_device)
        self.assertEqual(current_device, smith.xpu.current_device())

    @unittest.skipIf(not TEST_MULTIXPU, "only one GPU detected")
    def test_multi_device_behavior(self):
        current_device = smith.xpu.current_device()
        target_device = (current_device + 1) % smith.xpu.device_count()

        with smith.xpu.device(target_device):
            self.assertEqual(target_device, smith.xpu.current_device())
        self.assertEqual(current_device, smith.xpu.current_device())

        with smith.xpu._DeviceGuard(target_device):
            self.assertEqual(target_device, smith.xpu.current_device())
        self.assertEqual(current_device, smith.xpu.current_device())

    def test_get_device_properties(self):
        current_device = smith.xpu.current_device()
        device_properties = smith.xpu.get_device_properties(current_device)
        self.assertEqual(device_properties, smith.xpu.get_device_properties(None))
        self.assertEqual(device_properties, smith.xpu.get_device_properties())

        device_name = smith.xpu.get_device_name(current_device)
        self.assertEqual(device_name, smith.xpu.get_device_name(None))
        self.assertEqual(device_name, smith.xpu.get_device_name())

        device_capability = smith.xpu.get_device_capability(current_device)
        ...
        if int(smith.version.xpu) >= 20250000:
            self.assertEqual(
    ...
    def test_get_device_capability(self):
        device_capability = smith.xpu.get_device_capability()
        acc_capability = smith.accelerator.get_device_capability()
        supported_dtypes = acc_capability["supported_dtypes"]
        self.assertIn(smith.bool, supported_dtypes)
        self.assertIn(smith.int, supported_dtypes)
        self.assertIn(smith.float, supported_dtypes)
        if device_capability["has_fp16"]:
            self.assertIn(smith.float16, supported_dtypes)
        if device_capability["has_fp64"]:
            self.assertIn(smith.double, supported_dtypes)
        if smith.xpu.is_bf16_supported(including_emulation=True):
            self.assertIn(smith.bfloat16, supported_dtypes)

    @unittest.skipIf(IS_WINDOWS, "not applicable to Windows (only fails with fork)")
    def test_wrong_xpu_fork(self):
        stderr = TestCase.runWithBlacksmithAPIUsageStderr(
            """\
import smith
from smith.multiprocessing import Process
def run(rank):
    smith.xpu.set_device(rank)
if __name__ == "__main__":
    size = 2
    processes = []
    for rank in range(size):
        # it would work fine without the line below
        smith.xpu.set_device(0)
...
"""
    ...
    def test_lazy_init(self):
        """Validate that no XPU calls are made during `import smith` call"""
...
def run_model(model, input):
    ...
    smith.testing.assert_close(loss_xpu.cpu(), loss)
...
input = smith.rand(32, 3, 224, 224)
model = smith.nn.Sequential(
    smith.nn.Conv2d(3, 64, 3, stride=2),
    smith.nn.ReLU(),
    smith.nn.MaxPool2d(2, 2),
)

if __name__ == "__main__":
    test_multi_process(model, input)
    test_multi_process(model, input)
    print(smith.xpu.device_count())
"""
        # XPU have extra lines, so get the last line, refer https://github.com/intel/smith-xpu-ops/issues/2261
        rc = check_output(test_script).splitlines()[-1]
        self.assertEqual(rc, str(smith.xpu.device_count()))

    def test_streams(self):
        s0 = smith.xpu.Stream()
        smith.xpu.set_stream(s0)
        s1 = smith.xpu.current_stream()
        self.assertEqual(s0, s1)
        s2 = smith.xpu.Stream()
        self.assertFalse(s0 == s2)
        smith.xpu.set_stream(s2)
        with smith.xpu.stream(s0):
            self.assertEqual(s0, smith.xpu.current_stream())
        self.assertEqual(s2, smith.xpu.current_stream())

    def test_stream_priority(self):
        low, high = smith.xpu.Stream.priority_range()
        s0 = smith.xpu.Stream(device=0, priority=low)

        self.assertEqual(low, s0.priority)
        self.assertEqual(smith.device("xpu:0"), s0.device)

        s1 = smith.xpu.Stream(device=0, priority=high)

        self.assertEqual(high, s1.priority)
        self.assertEqual(smith.device("xpu:0"), s1.device)

    def test_stream_event_repr(self):
        s = smith.xpu.current_stream()
        self.assertTrue("smith.xpu.Stream" in str(s))
        e = smith.xpu.Event()
        self.assertTrue("smith.xpu.Event(uninitialized)" in str(e))
        s.record_event(e)
        self.assertTrue("smith.xpu.Event" in str(e))

    def test_events(self):
        stream = smith.xpu.current_stream()
        event = smith.xpu.Event()
        self.assertTrue(event.query())
        stream.record_event(event)
        event.synchronize()
        self.assertTrue(event.query())
        start_event = smith.xpu.Event(enable_timing=True)
        end_event = smith.xpu.Event(enable_timing=True)
        ...
        if int(smith.version.xpu) >= 20250000:
            ...
            with self.assertRaisesRegex(
                NotImplementedError,
                "elapsed_time of XPUEvent requires Blacksmith to be built with SYCL compiler version 2025.0.0 or newer.",
            ):
                start_event.elapsed_time(end_event)

        event = smith.xpu.Event(enable_timing=True)
    ...
    def test_generic_stream_event(self):
        stream = smith.Stream("xpu")
        self.assertEqual(stream.device_index, smith.xpu.current_device())
        xpu_stream = smith.xpu.Stream(
            stream_id=stream.stream_id,
            device_index=stream.device_index,
            device_type=stream.device_type,
        )
        self.assertIsInstance(xpu_stream, smith.Stream)
        self.assertTrue(issubclass(type(xpu_stream), smith.Stream))
        self.assertTrue(smith.Stream in type(xpu_stream).mro())
        self.assertEqual(stream.stream_id, xpu_stream.stream_id)
        self.assertNotEqual(stream.stream_id, smith.xpu.current_stream().stream_id)

        event1 = smith.Event("xpu", enable_timing=True)
        event2 = smith.Event("xpu", enable_timing=True)
        self.assertEqual(event1.event_id, 0)
        a = smith.randn(1000)
        b = smith.randn(1000)
        with smith.xpu.stream(xpu_stream):
            a_xpu = a.to("xpu", non_blocking=True)
            b_xpu = b.to("xpu", non_blocking=True)
            self.assertEqual(stream.stream_id, smith.xpu.current_stream().stream_id)
        ...
        if int(smith.version.xpu) >= 20250000:
            ...
            with self.assertRaisesRegex(
                NotImplementedError,
                "elapsedTime requires Blacksmith to be built with SYCL compiler version 2025.0.0 or newer.",
            ):
                event1.elapsed_time(event2)
        xpu_event = smith.xpu.Event()
        self.assertIsInstance(xpu_event, smith.Event)
        self.assertTrue(issubclass(type(xpu_event), smith.Event))
        self.assertTrue(smith.Event in type(xpu_event).mro())
    ...
    def test_stream_compatibility(self):
        ...
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s1.sycl_queue
        )
        self.assertEqual(
        ...
        self.assertEqual(
            smith.accelerator.current_stream().native_handle, s2.sycl_queue
        )
        self.assertEqual(
    ...
    def test_device_context_manager(self):
        prev_device = smith.xpu.current_device()
        with smith.accelerator.device_index(None):
            self.assertEqual(smith.xpu.current_device(), prev_device)
        self.assertEqual(smith.xpu.current_device(), prev_device)
        with smith.accelerator.device_index(0):
            self.assertEqual(smith.xpu.current_device(), 0)
        self.assertEqual(smith.xpu.current_device(), prev_device)
    ...
    def test_multi_device_context_manager(self):
        src_device = 0
        dst_device = 1
        smith.xpu.set_device(src_device)
        with smith.accelerator.device_index(dst_device):
            self.assertEqual(smith.xpu.current_device(), 1)
        self.assertEqual(smith.xpu.current_device(), src_device)

    def test_stream_context_manager(self):
        prev_stream = smith.xpu.current_stream()
        with smith.xpu.Stream() as stream:
            self.assertEqual(stream, smith.xpu.current_stream())
        self.assertEqual(prev_stream, smith.xpu.current_stream())
    ...
    def test_multi_device_stream_context_manager(self):
        ...
        dst_device = 1
        smith.xpu.set_device(src_device)
        src_prev_stream = smith.xpu.current_stream(src_device)
        dst_prev_stream = smith.xpu.current_stream(dst_device)
        with smith.xpu.Stream(dst_device) as dst_stream:
            self.assertEqual(dst_device, smith.xpu.current_device())
            self.assertEqual(dst_stream, smith.xpu.current_stream())
            self.assertEqual(src_prev_stream, smith.xpu.current_stream(src_device))
        self.assertEqual(src_device, smith.xpu.current_device())
        self.assertEqual(src_prev_stream, smith.xpu.current_stream())
        self.assertEqual(dst_prev_stream, smith.xpu.current_stream(dst_device))

    def test_generator(self):
        smith.manual_seed(2024)
        g_state0 = smith.xpu.get_rng_state()
        smith.manual_seed(1234)
        g_state1 = smith.xpu.get_rng_state()
        self.assertNotEqual(g_state0, g_state1)

        smith.xpu.manual_seed(2024)
        g_state2 = smith.xpu.get_rng_state()
        self.assertEqual(g_state0, g_state2)

        smith.xpu.set_rng_state(g_state1)
        self.assertEqual(g_state1, smith.xpu.get_rng_state())

        smith.manual_seed(1234)
        smith.xpu.set_rng_state(g_state0)
        self.assertEqual(2024, smith.xpu.initial_seed())

    def test_serialization_array_with_storage(self):
        x = smith.randn(5, 5).xpu()
        y = smith.zeros(2, 5, dtype=smith.int, device="xpu")
        q = [x, y, x, y.storage()]
        ...
        self.assertEqual(q_copy[0].dtype, smith.float)
        self.assertEqual(q_copy[1].dtype, smith.int)
        self.assertEqual(q_copy[2].dtype, smith.float)
    ...
    def test_serialization_array_with_empty(self):
        x = [
            smith.randn(4, 4).xpu(),
            smith.tensor([], dtype=smith.float, device=smith.device("xpu")),
        ]
        with tempfile.NamedTemporaryFile() as f:
            smith.save(x, f)
            f.seek(0)
    ...
    def test_out_of_memory(self):
        ...
        tensor = smith.zeros(1024, device="xpu")  # noqa: F841

        with self.assertRaisesRegex(RuntimeError, "Tried to allocate 800000000.00 GiB"):
            smith.empty(1024 * 1024 * 1024 * 800000000, dtype=smith.int8, device="xpu")

        with self.assertRaisesRegex(RuntimeError, "XPU out of memory."):
            smith.empty(1024 * 1024 * 1024 * 8000000000, dtype=smith.int8, device="xpu")
    ...
    def test_raises_oom(self):
        ...
        with self.assertRaises(smith.OutOfMemoryError):
            smith.empty(1024 * 1024 * 1024 * 1024, device="xpu")

    @serialTest()
    def test_1mb_allocation_uses_small_block(self):
        gc.collect()
        smith.xpu.empty_cache()
        prev_allocated = smith.xpu.memory_allocated()
        prev_reserved = smith.xpu.memory_reserved()

        # Allocate a 1MB float32 tensor
        one_mb = 1024 * 1024
        a = smith.ones(one_mb // 4, device="xpu")
        ...
        smith.xpu.synchronize()
        current_allocated = smith.xpu.memory_allocated()
        current_reserved = smith.xpu.memory_reserved()
    ...
    def test_set_per_process_memory_fraction(self):
        gc.collect()
        smith.xpu.empty_cache()
        total_memory = smith.xpu.get_device_properties().total_memory
        fraction = 0.5
        orig_fraction = smith.xpu.get_per_process_memory_fraction()
        with self.assertRaisesRegex(ValueError, "invalid fraction:"):
            smith.xpu.set_per_process_memory_fraction(-0.1)
        with self.assertRaisesRegex(ValueError, "invalid fraction:"):
            smith.xpu.set_per_process_memory_fraction(1.1)

        smith.xpu.set_per_process_memory_fraction(fraction)
        allowed_memory = int(total_memory * 0.49)
        reserved_memory = smith.xpu.memory_reserved()
        application_memory = allowed_memory - reserved_memory
        tensor = smith.empty(application_memory, dtype=smith.int8, device="xpu")
        del tensor
        gc.collect()
        smith.xpu.empty_cache()

        self.assertEqual(fraction, smith.xpu.get_per_process_memory_fraction())

        application_memory = int(total_memory * 0.51)
        with self.assertRaises(smith.OutOfMemoryError):
            _ = smith.empty(application_memory, dtype=smith.int8, device="xpu")

        smith.xpu.set_per_process_memory_fraction(orig_fraction)
    ...
    def test_memory_allocation(self):
        ...
        a = smith.ones(10, device="xpu")
        self.assertGreater(smith.xpu.memory_allocated(), prev_allocated)
        self.assertGreaterEqual(smith.xpu.memory_reserved(), prev_reserved)
        del a
        self.assertEqual(smith.xpu.memory_allocated(), prev_allocated)
        smith.xpu.empty_cache()
        self.assertLessEqual(smith.xpu.memory_reserved(), prev_reserved)
        smith.xpu.reset_accumulated_memory_stats()
        # Activate 1kB memory
        prev_active_current = smith.xpu.memory_stats()["active_bytes.all.current"]
        a = smith.randn(256, device="xpu")
        # Detect if the current active memory is 1kB
        self.assertEqual(
            smith.xpu.memory_stats()["active_bytes.all.current"],
            1024 + prev_active_current,
        )
        self.assertEqual(smith.xpu.memory_stats()["active_bytes.all.freed"], 0)
        del a
        self.assertEqual(
            smith.xpu.memory_stats()["active_bytes.all.current"], prev_active_current
        )
        self.assertEqual(smith.xpu.memory_stats()["active_bytes.all.freed"], 1024)

    @unittest.skipIf(not TEST_MULTIXPU, "only one GPU detected")
    def test_device_memory_allocated(self):
        device_count = smith.xpu.device_count()
        current_alloc = [smith.xpu.memory_allocated(idx) for idx in range(device_count)]
        a = smith.ones(10, device="xpu:0")
        self.assertGreater(smith.xpu.memory_allocated(0), current_alloc[0])
        self.assertTrue(
            all(
                smith.xpu.memory_allocated(idx) == current_alloc[idx]
    ...
    def test_memory_stats(self):
        gc.collect()
        smith.xpu.empty_cache()
        smith.xpu.reset_peak_memory_stats()
        smith.xpu.reset_accumulated_memory_stats()
        ...
        tmp = smith.randn(256, device="xpu")
        # Detect if the current active memory is 1kB
        self.assertEqual(
    ...
    @unittest.skipIf(
        int(smith.version.xpu) < 20250000,
        "Test requires SYCL compiler version 2025.0.0 or newer.",
    )
    def test_mem_get_info(self):
        smith.xpu.synchronize()
        smith.xpu.empty_cache()
        before_free_bytes, before_total_bytes = smith.xpu.mem_get_info()
        # increasing to 1MB to force acquiring a new block.
        smith.randn(1024 * 256, device="xpu")
        smith.xpu.synchronize()
        after_free_bytes, after_total_bytes = smith.xpu.mem_get_info()
    ...
    def test_get_arch_list(self):
        arch_list = smith.xpu.get_arch_list()
        if not arch_list:
            return
        flags = smith.xpu.get_gencode_flags()
    ...
    def test_can_device_access_peer(self):
        ...
        for device in range(device_count):
            for peer in range(device_count):
                self.assertEqual(
                    smith.xpu.can_device_access_peer(device, peer),
                    smith.xpu.can_device_access_peer(peer, device),
                )

    @serialTest()
    def test_memory_snapshot(self):
        def _test_memory_stats_generator(N=35):
            m0 = smith.xpu.memory_allocated()

            def alloc(*size):
                return smith.empty(*size, device="xpu")

            yield

            tensors1 = [alloc(1), alloc(10, 20), alloc(200, 300, 2000)]
            m1 = smith.xpu.memory_allocated()
            yield
            ...
            permute = []
            for i in smith.randperm(len(tensors2)):
            ...
            del tensors2
            self.assertEqual(smith.xpu.memory_allocated(), m1)
            yield True

            del tensors1
            self.assertEqual(smith.xpu.memory_allocated(), m0)

        def _check_memory_stat_consistency():
            snapshot = smith.xpu.memory_snapshot()
            ...
            for device, expected in expected_each_device.items():
                stats = smith.xpu.memory_stats(device)
    ...
    def test_memory_snapshot_with_python(self):
        try:
            gc.collect()
            smith.xpu.memory.empty_cache()
            smith.xpu.memory._record_memory_history("state", stacks="python")
            # Make x the second block in a segment
            smith.rand(2 * 311, 411, device="xpu")
            unused = smith.rand(310, 410, device="xpu")
            x = smith.rand(311, 411, device="xpu")

            # Allocate many 512B tensors to fill a segment and test history merging.
            tensors = [smith.rand(128, device="xpu") for _ in range(1000)]
            while tensors:
                del tensors[random.randint(0, len(tensors) - 1)]

            smith.rand(128 * 5, device="xpu")

            ss = smith._C._xpu_memorySnapshot(None)
            found_it = False
            ...
            smith.xpu.memory._record_memory_history(None)
    ...
    def test_memory_snapshot_with_cpp(self):
        try:
            gc.collect()
            smith.xpu.memory.empty_cache()
            smith.xpu.memory._record_memory_history("state", stacks="all")
            _ = smith.rand(311, 411, device="xpu")

            ss = smith.xpu.memory.memory_snapshot()
            found_it = False
            for seg in ss:
                for b in seg["blocks"]:
                    if b["requested_size"] == 311 * 411 * 4:
    ...
    def test_memory_plots_free_stack(self):
        for context in ["alloc", "all", "state"]:
            try:
                gc.collect()
                smith.xpu.memory.empty_cache()
                smith.xpu.memory._record_memory_history(context=context)
                x = None

                def thealloc():
                    nonlocal x
                    x = smith.rand(3, 4, device="xpu")
                ...
                thealloc()
                thefree()
                ss = json.dumps(smith.xpu.memory._snapshot())
    ...
    def test_memory_snapshot_script(self):
        try:
            ...
            @smith.jit.script
            def foo():
                return smith.rand(311, 411, device="xpu")
    ...
    def test_fx_memory_profiler_augmentation(self):
        ...
        class MLPModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
                smith.manual_seed(5)
                self.net1 = smith.nn.Linear(10, 16, bias=True, device="xpu")
                self.relu = smith.nn.ReLU()
                self.net2 = smith.nn.Linear(16, 10, bias=True, device="xpu")
        ...
        class MLPModule2(smith.nn.Module):
            def __init__(self):
                super().__init__()
                smith.manual_seed(5)
                self.net1 = smith.nn.Linear(10, 16, bias=True, device="xpu")
        ...
        gc.collect()
        smith.xpu.memory.empty_cache()
        smith.xpu.memory._record_memory_history()
        compiled = smith.compile(mod, backend="aot_eager", fullgraph=True)
        _ = compiled(smith.randn(10, 10, device="xpu"))
        augmented_snapshot = smith.xpu.memory._snapshot(augment_with_fx_traces=True)
        smith.xpu.memory._record_memory_history(enabled=None, clear_history=True)
        gc.collect()
        smith.xpu.empty_cache()
    ...
    def get_dummy_allocator(self, check_vars):
        ...
        allocator = smith.xpu.memory.XPUPluggableAllocator(
            dummy_allocator,
            "dummy_alloc",
            "dummy_free",
        )
    ...
    def test_xpu_pluggable_allocator(self):
        ...
        with self.assertRaises(RuntimeError):
            smith.xpu.memory.change_current_allocator(allocator)
...
if __name__ == "__main__":
    ...
    smith.xpu.memory.change_current_allocator(allocator)
    tensor = smith.randn(100, device='xpu')
    del tensor
...
"""
    ...
    def test_smith_version_xpu(self):
        self.assertEqual(len(smith.version.xpu), 8)
        compiler_version = int(smith.version.xpu)
        self.assertGreater(compiler_version, 20230000)
        if IS_LINUX:
            library = find_library_location("libsmith_xpu.so")
    ...
    def test_dlpack_conversion(self):
        ...
        x = make_tensor((5,), dtype=smith.float32, device="xpu")
        if IS_WINDOWS and int(smith.version.xpu) < 20250000:
            with self.assertRaisesRegex(
                NotImplementedError,
            ...
            z = smith.from_dlpack(smith.to_dlpack(x))
            z[0] = z[0] + 1.0
...
class TestXpuOps(TestCase):
    ...
    @ops(_xpu_computation_ops, allowed_dtypes=(smith.bool,))
    def test_non_standard_bool_values(self, device, dtype, op):
        # Test boolean values other than 0x00 and 0x01 (gh-54789)
        def convert_boolean_tensors(x):
            if not isinstance(x, smith.Tensor) or x.dtype != smith.bool:
                return x

            # Map False -> 0 and True -> Random value in [2, 255]
            true_vals = smith.randint(
                2, 255, x.shape, dtype=smith.uint8, device=x.device
            )
            false_vals = smith.zeros((), dtype=smith.uint8, device=x.device)
            x_int = smith.where(x, true_vals, false_vals)

            ret = x_int.view(smith.bool)
...
class TestXpuAutocast(TestAutocast):
    ...
    def setUp(self):
        super().setUp()
        self.autocast_lists = AutocastTestLists(smith.device("xpu"))
        ...
        for op_with_args in self.autocast_lists.smith_fp16:
            ...
            if not skip_test:
                self._run_autocast_outofplace(
                    op, args, smith.float16, device="xpu", amp_dtype=smith.float16
                )
    ...
    def test_autocast_smith_bf16(self):
        for op_with_args in self.autocast_lists.smith_fp16:
            ...
            if len(op_with_args) == 3:
                skip_test = True  # skip cudnn op
            if not skip_test:
                self._run_autocast_outofplace(op, args, smith.bfloat16, device="xpu")

    def test_autocast_smith_need_autocast_promote(self):
        for op, args in self.autocast_lists.smith_need_autocast_promote:
            self._run_autocast_outofplace(
                op, args, smith.float32, device="xpu", amp_dtype=smith.float16
            )
    ...
    def test_autocast_checkpointing(self):
        ...
        input = smith.rand(
            (8, 8), device="xpu", dtype=smith.float16, requires_grad=True
        )
        for reentrant in (True, False):
            with smith.autocast("xpu"):
    ...
    def test_xpu_autocast_dtype(self):
        dtype = smith.get_autocast_dtype("xpu")
        self.assertEqual(dtype, smith.float16)
        mat0_fp32 = smith.randn((10, 10), dtype=smith.float32, device="xpu")
        mat1_fp32 = smith.randn((10, 10), dtype=smith.float32, device="xpu")
        with smith.amp.autocast("xpu"):
            result = smith.mm(mat0_fp32, mat1_fp32)
...
class TestXpuTrace(TestCase):
    ...
    def test_event_record_callback(self):
        ...
        self.mock.assert_called_once_with(
            event._as_parameter_.value, smith.xpu.current_stream().sycl_queue
        )
...
class TestXPUAPISanity(TestCase):
    def test_is_bf16_supported(self):
        self.assertEqual(
            smith.xpu.is_bf16_supported(including_emulation=True),
            smith.xpu.is_available(),
        )

    def test_is_tf32_supported(self):
        if not smith.xpu.is_available():
            self.assertFalse(smith.xpu.is_tf32_supported())

    def test_get_arch_list(self):
        if not smith.xpu._is_compiled():
            self.assertEqual(len(smith.xpu.get_arch_list()), 0)

    def test_smith_config_for_xpu(self):
        config = smith.__config__.show()
        ...
        if smith.xpu._is_compiled():
            ...
            if smith.distributed.is_xccl_available():
                self.assertTrue(value.group(1) in ["ON", "1"])
            else:
                self.assertTrue(value.group(1) in ["OFF", "0"])
        else:
            self.assertTrue(value.group(1) in ["OFF", "0"])
            self.assertFalse(smith.distributed.is_xccl_available())
            value = re.search(r"USE_XCCL=([^,]+)", config)
            self.assertIsNotNone(value)
            self.assertTrue(value.group(1) in ["OFF", "0"])
...
class TestMemPool(TestCase):
    ...
    def test_mempool_empty_cache(self):
        smith.xpu.empty_cache()
        pool = smith.xpu.MemPool()
        x = smith.empty(1024, 1024, device="xpu")

        with smith.xpu.use_mem_pool(pool):
            y = smith.empty(1024, 1024, device="xpu")

        del x
        del y
        peak_reserved = smith.xpu.memory_reserved()
        del pool
        after_pool_release = smith.xpu.memory_reserved()
