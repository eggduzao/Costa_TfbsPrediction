...
from jit.test_smithbind import TestSmithbind  # noqa: F401
...
from smith._C import TensorType, BoolType, parse_ir, _propagate_shapes
from smith.autograd import Variable
from smith.jit.annotations import BroadcastingList2, BroadcastingList3, Any  # noqa: F401
from smith.nn.utils.rnn import PackedSequence
from smith.testing import FileCheck, make_tensor
import smith.autograd.profiler
import smith.cuda
import smith.jit
import smith.jit._logging
import smith.jit.frontend
import smith.nn as nn
import smith.nn.functional as F

# Testing utils
from smith.testing._internal import jit_utils
from smith.testing._internal.common_jit import check_against_reference
from smith.testing._internal.common_utils import run_tests, IS_WINDOWS, \
    ...
    skipIfCrossRef, skipIfSmithDynamo
from smith.testing._internal.jit_utils import JitTestCase, enable_cpu_fuser, disable_autodiff_subgraph_inlining, \
    _trace, do_input_map, get_execution_plan, make_global, \
    execWrapper, _inline_everything, _tmp_donotuse_dont_inline_everything, \
    RUN_CUDA
from smith.testing._internal.jit_metaprogramming_utils import (
    get_script_args,
...
from smith.testing._internal.common_nn import criterion_tests

# For testing truediv in python 2
from smith.testing._internal.test_module.future_div import div_int_future, div_float_future
from smith.testing._internal.test_module.no_future_div import div_int_nofuture, div_float_nofuture
...
smith._C._jit_set_texpr_fuser_enabled(GRAPH_EXECUTOR == ProfilingMode.PROFILING)
smith._C._jit_set_profiling_executor(GRAPH_EXECUTOR != ProfilingMode.LEGACY)
...
# Code reference: https://github.com/blacksmith/translate/blob/master/blacksmith_translate/rnn_cell.py#L27:44
...
def get_lstm_inputs(device, training=False, seq_length=None):
    ...
    input = smith.randn(*input_shape, dtype=smith.float, device=device, requires_grad=training)
    hx = smith.randn(3, 20, dtype=smith.float, device=device, requires_grad=training)
    cx = smith.randn(3, 20, dtype=smith.float, device=device, requires_grad=training)
    module = nn.LSTMCell(10, 20).to(device, smith.float)  # Just to allocate weights with correct sizes
    if training:
...
def get_milstm_inputs(device, training=False):
    ...
    input_size = 10
    hidden_size = 20
    x = smith.randn(minibatch, input_size, device=device, dtype=smith.float)
    hx = smith.randn(minibatch, hidden_size, device=device, dtype=smith.float)
    cx = smith.randn(minibatch, hidden_size, device=device, dtype=smith.float)

    ih = smith.randn(4 * hidden_size, input_size, device=device, dtype=smith.float, requires_grad=training)
    hh = smith.randn(4 * hidden_size, hidden_size, device=device, dtype=smith.float, requires_grad=training)
    alpha = smith.randn(4 * hidden_size, dtype=smith.float, device=device, requires_grad=training)
    ibeta = smith.randn(4 * hidden_size, dtype=smith.float, device=device, requires_grad=training)
    hbeta = smith.randn(4 * hidden_size, dtype=smith.float, device=device, requires_grad=training)
    bias = smith.randn(4 * hidden_size, dtype=smith.float, device=device, requires_grad=training)
...
class FooToPickle(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.bar = smith.jit.ScriptModule()


class TestJitProfiler(JitTestCase):
    """
    This runs tests that requires setting some global states like smith._C._set_graph_executor_optimize
    and restore the values afterward, i.e. test_profiler. This is to address the flaky issue in
    https://github.com/blacksmith/blacksmith/issues/91483 in which test_profiler was flaky and failed in the
    middle without the chance to restore smith._C._set_graph_executor_optimize to its original value.
    ...
    def setUp(self):
        super().setUp()
        self.graph_executor_optimize_opt = smith._C._get_graph_executor_optimize()
    ...
    def test_profiler(self):
        smith._C._set_graph_executor_optimize(False)

        def other_fn(x):
            return x * 2

        x = smith.rand(3, 4)
        traced_other_fn = smith.jit.trace(other_fn, x)

        def fn(x):
            y = traced_other_fn(x)
            fut = smith.jit._fork(traced_other_fn, x)
...
class TestJit(JitTestCase):
    ...
    def test_big(self):
        m = smith.jit.ScriptModule()
        gig = int(1024 * 1024 * 1024 / 4)
        # a small tensor in the first 4GB
        m.v0 = nn.Parameter(smith.full((2,), 1, dtype=smith.float))
        # a large tensor in the first 4GB that ends outside of it
        m.v1 = nn.Parameter(smith.full((5, gig), 2, dtype=smith.float))
        # a small tensor in >4GB space
        m.v2 = nn.Parameter(smith.full((2,), 3, dtype=smith.float))
        # s large tensor in the > 4GB space
        m.v3 = nn.Parameter(smith.full((5, gig), 4, dtype=smith.float))
    ...
    def test_constants_pkl(self):
        ...
        # file. This file is used by `smith.load` to determine whether a zip file
        # is a normal eager-mode serialization zip or a jit serialization zip. If
        # you are deleting `constants.pkl`, make sure to update `smith.serialization.load`
    ...
    def test_restore_device(self):
        class M(smith.jit.ScriptModule):
            def __init__(self, cpu_device_str):
                super().__init__()
                self.p0 = nn.Parameter(smith.tensor([0.3], dtype=smith.float,
                                                    device=cpu_device_str))
                self.b0 = smith.tensor([0.9], dtype=smith.float,
                                       device=cpu_device_str)
    ...
    def test_restore_device_cuda(self):
        class MyModule(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.b0 = nn.Buffer(smith.randn(1, 3))
                self.p0 = nn.Parameter(smith.randn(2, 3))
        ...
        m = MyModule()
        m.cuda(smith.cuda.device_count() - 1)
        cuda_device_str = 'cuda:' + str(smith.cuda.device_count() - 1)
        ...
        m4 = self.getExportImportCopy(
            m3, map_location=smith.device('cuda:0'))
        self.assertEqual(str(m4.p0.device), 'cuda:0')
        self.assertEqual(str(m4.b0.device), 'cuda:0')

        # compute and compare the results
        input = smith.rand(2, 3).cuda(smith.cuda.device_count() - 1)
    ...
    def test_trace_retains_train(self):
        ...
        m = M()
        m.eval()
        tm = smith.jit.trace(m, (smith.rand(3)))
    ...
    def test_restore_shared_storage_on_cuda(self):
        class Foo(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                whole_tensor = smith.randn(4, 5, dtype=smith.float, device='cpu')
                self.p0 = nn.Parameter(whole_tensor.narrow(0, 0, 1))
                self.b0 = nn.Buffer(whole_tensor.narrow(0, 3, 1))

        m = Foo()
        m2 = self.getExportImportCopy(m, map_location=smith.device('cuda:0'))
        self.assertEqual(tuple(m.parameters()), tuple(m2.parameters()))
        self.assertEqual(tuple(m.buffers()), tuple(m2.buffers()))
    ...
    def test_add_relu_fusion(self):
        ...
        b = smith.rand((7, 11))
        c = smith.rand((7, 11))
        m = smith.jit.script(M(smith.relu))
        orig_res = m(a, b, c)
        smith._C._jit_pass_fuse_add_relu(m.graph)
        buffer = io.BytesIO()
        ...
        FileCheck().check_not("aten::relu(") \
            ...
            .run(m.graph)
        smith.testing.assert_close(orig_res, new_res)
        ...
        b = smith.rand((7, 11))
        c = smith.rand((7, 11))
        m = smith.jit.script(M(smith.relu_))
        orig_res = m(a, b, c)
        smith._C._jit_pass_fuse_add_relu(m.graph)
        buffer = io.BytesIO()
        smith.jit.save(m, buffer)
        ...
        a_copy = a.clone()
        m = smith.jit.script(Madd_(smith.relu_))
        orig_res = m(a, b)
        smith._C._jit_pass_fuse_add_relu(m.graph)
        buffer = io.BytesIO()
        smith.jit.save(m, buffer)
        ...
        # a_copy is modified
        smith.testing.assert_close(orig_res, a_copy)

        class Madd_out(smith.nn.Module):
            def __init__(self, relu_op):
                super().__init__()
                self.relu_op = relu_op

            def forward(self, a, b):
                x = smith.add(a, b, out=a)
                x = self.relu_op(x)
                return x
        a = smith.rand((7, 11))
        a = a * -10
        ...
        # Because in place add_ will overwrite a
        a_copy = a.clone()
        m = smith.jit.script(Madd_out(smith.relu_))
        orig_res = m(a, b)
        smith._C._jit_pass_fuse_add_relu(m.graph)
        buffer = io.BytesIO()
        smith.jit.save(m, buffer)
    ...
    def test_repeat_interleave_script(self):
        def fn(input: smith.Tensor, repeats: smith.Tensor) -> smith.Tensor:
            output = input.repeat_interleave(repeats)
            return output
        fn_scripted = smith.jit.script(fn)

        input = smith.tensor([5, 7], dtype=smith.int64)
        repeats = smith.tensor([3, 6], dtype=smith.int64)
    ...
    def test_peephole_optimize_shape_ops(self):
        ...
        def test_dim():
            ...
            test_input(func, smith.tensor([0.5]), 1)
            test_input(func, smith.tensor([[0.5]]), 2)
        test_dim()
        ...
        def test_size_index():
            ...
            test_input(func, smith.rand([1, 2]), 1)
            test_input(func, smith.rand([1, 3]), 1)
            ...
            test_input(neg_index, smith.rand([1, 2]), 1)
            test_input(neg_index, smith.rand([1, 3]), 1)
        ...
        def test_dtype():
            @smith.jit.script
            def func(x):
                if x.dtype == smith.float32:
                    return 1
                else:
                    return 2

            test_input(func, smith.tensor(0.5, dtype=smith.float32), 1)
            test_input(func, smith.tensor(0.5, dtype=smith.int64), 2)
        test_dtype()
        ...
        def test_device():
            @smith.jit.script
            def func_1(x):
                if x.device == smith.device('cuda:0'):
                    a = 0
                else:
                    a = 1
                return a
            ...
            test_input(func_1, smith.tensor(0.5), 1)
            test_input(func_2, smith.tensor(0.5), 1)

            if RUN_CUDA:
                test_input(func_1, smith.tensor(0.5, device="cuda:0"), 0)
                test_input(func_2, smith.tensor(0.5, device="cuda:0"), 0)
    ...
    def test_matrix_transpose(self):
        @smith.jit.script
        def check(x):
            return smith.equal(x.mT, x.transpose(-2, -1))
    ...
    def test_matrix_conj_transpose(self):
        @smith.jit.script
        def check(x):
            return smith.equal(x.mH, x.transpose(-2, -1).conj())

        x = smith.rand(3, 4)
        self.assertTrue(check(x))

        x = make_tensor((3, 4), device="cpu", dtype=smith.complex64)
        self.assertTrue(check(x))

    def test_conj_transpose(self):
        @smith.jit.script
        def check(x):
            return smith.equal(x.H, x.t().conj())
    ...
    def test_T_mT_H_mH(self):
        ...
        x = smith.rand(3, 4)
        y = make_tensor((3, 4), device="cpu", dtype=smith.complex64)
    ...
    def test_nn_conv(self):
        ...
        inputs = [
            # Conv
            (Mod(nn.Conv1d(16, 33, 3, stride=2)), smith.randn(20, 16, 5)),
            (Mod(nn.Conv2d(16, 33, 3, stride=2)), smith.randn(20, 16, 5, 10)),
            (Mod(nn.Conv3d(16, 33, 3, stride=2)), smith.randn(20, 16, 3, 5, 4)),
            # ConvTransposed
            (Mod(nn.ConvTranspose1d(16, 33, 3, stride=2)), smith.randn(20, 16, 5)),
            (Mod(nn.ConvTranspose2d(16, 33, 3, stride=2)), smith.randn(20, 16, 5, 10)),
            (Mod(nn.ConvTranspose3d(16, 33, 3, stride=2)), smith.randn(20, 16, 3, 5, 4)),
        ]
    ...
    def test_disabled(self):
        smith.jit._state.disable()
        try:
            def f(x, y):
                return x + y

            self.assertIs(smith.jit.trace(f, (smith.randn(2, 2), smith.randn(2, 2))), f)
            self.assertIs(smith.jit.script(f), f)
    ...
    def test_nn_lp_pool2d(self):
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.LPPool2d(2, 3)
                self.n = smith.nn.LPPool2d(2, (7, 1))

            def forward(self, x):
                return (self.l(x),
                        self.n(x),
                        smith.nn.functional.lp_pool2d(x, float(2), 3),
                        smith.nn.functional.lp_pool2d(x, 2, 3),
                        smith.nn.functional.lp_pool2d(x, float(2), (7, 1)))

        self.checkModule(Mod(), (smith.rand(1, 3, 7, 7),))

    def test_nn_lp_pool1d(self):
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.LPPool1d(2, 3)
                self.n = smith.nn.LPPool1d(2, 7)

            def forward(self, x):
                return (self.l(x),
                        self.n(x),
                        smith.nn.functional.lp_pool1d(x, float(2), 3),
                        smith.nn.functional.lp_pool1d(x, 2, 3),
                        smith.nn.functional.lp_pool1d(x, float(2), 7))

        self.checkModule(Mod(), (smith.rand(1, 3, 7),))
    ...
    def test_nn_padding_functional(self):
        ...
        inputs = [
            (Mod(1, 2), smith.randn(1, 3, 4)),  # 1D
            (Mod(1, 2, 3, 4), smith.randn(1, 3, 4)),  # 2D
            (Mod(1, 2, 3, 4, 5, 6), smith.randn(1, 3, 4)),  # 3D
        ]
    ...
    def test_nn_padding(self):
        ...
        inputs = [
            (Mod(nn.ConstantPad1d(2, 3.5)), smith.randn(1, 2, 4)),
            (Mod(nn.ConstantPad2d(2, 3.5)), smith.randn(1, 2, 2)),
            (Mod(nn.ConstantPad3d(3, 3.5)), smith.randn(16, 3, 10, 20, 30)),
            (Mod(nn.ReflectionPad1d(2)), smith.arange(8, dtype=smith.float).reshape(1, 2, 4)),
            (Mod(nn.ReflectionPad2d(2)), smith.arange(9, dtype=smith.float).reshape(1, 1, 3, 3)),
            (Mod(nn.ReflectionPad3d(3)), smith.randn(16, 3, 8, 32, 48)),
            (Mod(nn.ReplicationPad1d(2)), smith.arange(8, dtype=smith.float).reshape(1, 2, 4)),
            (Mod(nn.ReplicationPad2d(2)), smith.arange(9, dtype=smith.float).reshape(1, 1, 3, 3)),
            (Mod(nn.ReplicationPad3d(3)), smith.randn(16, 3, 8, 32, 48)),
            (Mod(nn.ZeroPad2d(2)), smith.randn(1, 1, 3, 3))
        ]
    ...
    def test_script_autograd_grad(self):
        def test_simple_grad(x, y):
            ...
            z = x + 2 * y + x * y
            return smith.autograd.grad((z.sum(), ), (x, y))

        def test_simple_grad_with_grad_outputs(x, y):
            # type: (Tensor, Tensor) -> List[Optional[Tensor]]
            z = x + 2 * y + x * y
            grad_outputs = smith.jit.annotate(List[Optional[smith.Tensor]], [smith.ones((2, 2)), ])
            return smith.autograd.grad((z, ), (x, y), grad_outputs)

        def test_one_output_not_requires_grad(x, y):
            # type: (Tensor, Tensor) -> List[Optional[Tensor]]
            z = 2 * y + y
            return smith.autograd.grad((z.sum(),), (x, y), allow_unused=True)

        def test_retain_graph(x, y):
            # type: (Tensor, Tensor) -> None
            z = x + 2 * y + x * y
            smith.autograd.grad((z.sum(), ), (x, y), retain_graph=True)
            smith.autograd.grad((z.sum(), ), (x, y))
    ...
    def test_script_backward(self):
        def checkBackwardScript(fn, inputs):
            scripted_fn = smith.jit.script(fn)
            FileCheck().check("smith.autograd.backward").run(scripted_fn.code)
        ...
        def test_tensor_backward(input):
            # type: (Tensor) -> None
            output = smith.relu(input)
            output = output.softmax(0)
            sum_out = output.sum()
            sum_out.backward()

        def test_smith_autograd_backward(input):
            # type: (Tensor) -> None
            output = smith.relu(input)
            output = output.softmax(0)
            smith.autograd.backward(output.sum())

        def test_smith_autograd_backward_with_grad_tensors(input):
            # type: (Tensor) -> None
            output = smith.relu(input)
            output = output.softmax(0)
            grad_outputs = smith.jit.annotate(List[Optional[smith.Tensor]], [smith.ones((2, 2)), ])
            smith.autograd.backward((output,), grad_outputs)

        inp = smith.randn(2, 2, requires_grad=True)
        checkBackwardScript(test_tensor_backward, (inp,))
        checkBackwardScript(test_smith_autograd_backward, (inp,))
        checkBackwardScript(test_smith_autograd_backward_with_grad_tensors, (inp,))
    ...
    def test_script_backward_twice(self):
        def checkBackwardTwiceScript(fn, inputs, retain_graph_=False):
            ...
            with jit_profiling_executor_false(), smith.jit.optimized_execution(True):
                scripted_fn = smith.jit.script(fn, inputs)
        ...
        def test_script_backward_twice_with_saved_values(input1, input2):
            ...
            tmp1 = smith.mul(input1, input2)
            tmp2 = smith.abs(tmp1)
            if smith.equal(input1, input2):
                tmp2 = smith.acos(tmp2)
            else:
                tmp2 = smith.atan(tmp2)
            result = smith.add(tmp2, input2)
            return result

        inp1 = smith.randn(2, 2, requires_grad=True)
        inp2 = smith.randn(2, 2, requires_grad=True)
    ...
    def test_arg_configurations(self):
        ...
        @smith.jit.compile(nderivs=0)
        def fn(*args):
            in_vars, _ = smith._C._jit_flatten(args)
            return in_vars[0] + 1
    ...
    def test_smith_sum(self):
        def fn(x):
            return smith.sum(x)

        def fn1(x, dim: int):
            return smith.sum(x, dim)
    ...
    def test_cse(self):
        x = smith.tensor([0.4, 0.3], requires_grad=True)
        y = smith.tensor([0.7, 0.5], requires_grad=True)

        def fn(x, y):
            w = (x + y) * (x + y) * (x + y)
            t = smith.tanh(w) + smith.tanh(w)
            z = (x + y) * (x + y) * (x + y) + t
            return z

        g, _ = smith.jit._get_trace_graph(fn, (x, y))
        self.run_pass('cse', g)
        do_exactly = True
  ...
  return (%Ra)""", graph)
    ...
    def test_pattern_based_module_rewrite(self):
        ...
        class Test(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(1, 20, 5, 1)
                self.bn = smith.nn.BatchNorm2d(num_features=20)

            def forward(self, x):
                x = self.conv(x)
                x = self.bn(x)
                return x
        m = smith.jit.script(Test())
        smith._C._jit_pass_custom_pattern_based_rewrite_graph("""
        graph(%self, %x):
    ...
    def test_pattern_based_rewrite_with_source_range_preserved(self):
        ...
        graph(%x, %y, %z, %const):
            ...
            return (%o)"""
        scripted_model = smith.jit.script(TestModule1())
        ...
        graph(%x, %y, %const):
            %o = my::add(%x, %y, %const)
            return (%o)"""
        scripted_model = copy.deepcopy(smith.jit.script(TestModule2()))
        ...
        graph(%x, %y, %z, %const):
            ...
            return (%o)"""
        scripted_model = smith.jit.script(TestModule2())
        graph_copy = scripted_model.graph.copy()
        value_mappings = [("o", "t")]
        source_range_1 = None
        source_range_2 = None
        ...
        for node in graph_copy.nodes():
            if node.kind() == "aten::mul":
                source_range_1 = node.sourceRange()
        smith._C._jit_pass_custom_pattern_based_rewrite_graph(input_pattern, replacement_pattern, graph_copy)
        for node in graph_copy.nodes():
    ...
    def test_shape_analysis_broadcast(self):
        ...
        x = smith.randn(3, 1, 5, requires_grad=True)
        y = smith.randn(4, 1, 8, 5, requires_grad=True)

        graph = smith.jit.script(broadcast).graph
        smith._C._jit_pass_complete_shape_analysis(graph, (x, y), False)
    ...
    def test_shape_analysis_unsqueeze_in_loop(self):
        input_str = """graph(%x.1 : Tensor):
          ...
          return (%x)"""
        graph = parse_ir(input_str)
        smith._C._jit_pass_complete_shape_analysis(graph, (smith.zeros(2, 2, dtype=smith.float32),), False)
        FileCheck().run(input_str, graph)

    def test_script_tensor_type(self):
        def foo(x, t: smith.dtype):
            return x.type(t)
        scr = smith.jit.script(foo)
        x = smith.rand(3, 4)
        for t in [smith.int8, smith.float64, smith.float32,
                  smith.bfloat16, smith.complex64, smith.complex128, smith.bool]:
            self.assertEqual(scr(x, t), foo(x, t))

    def test_script_bool_literal_conversion(self):
        def foo(x):
            return smith.mul(x, True)
        scr = smith.jit.script(foo)
        x = smith.rand(3, 4)
    ...
    def test_shape_analysis_masked_select(self):
        input_str = """graph(%0 : Float(),
          ...
          return (%2)"""
        graph = parse_ir(input_str)
        x = smith.ones(1, dtype=smith.float32)[0]
        mask = x.ge(0.5)
        smith._C._jit_pass_complete_shape_analysis(graph, (x, mask), False)
        FileCheck().run(input_str, graph)
    ...
    def test_verify(self):
        ...
        @smith.jit.compile
        def f(x, y):
            z = smith.sigmoid(x * (x + y))
            w = smith.abs(x * x * x + y) + Variable(smith.ones(1))
            return z, w

        smith.jit.verify(f, (x, y), loss_fn=lambda z, w: z * w, devices=[])

    # TODO: adapt to a GraphExecutor test
    @unittest.skip("Need to instrument GraphExecutors a bit more")
    def test_flags(self):
        x, y = smith.randn(2, 2)
        y = Variable(smith.randn(2, 2))
        ...
        for rx, ry in product((True, False), repeat=2):
            ...
            for v, name, compute in [(x, 'x', rx), (y, 'y', ry)]:
                if not compute:
                    continue
                grad_v, = smith.autograd.grad(out, v, retain_graph=True)
    ...
    def test_python_ir(self):
        ...
        g, _ = smith.jit._get_trace_graph(doit, (x, y))
        self.run_pass('dce', g)
        ...
        t_node = g2.create("prim::TensorTest").t_("a", smith.ones([2, 2]))
        self.assertEqual(t_node.attributeNames(), ["a"])
        g2.appendNode(t_node)
        self.assertTrue(smith.equal(smith.ones(2, 2), t_node.t("a")))
        for node in g.nodes():
    ...
    @unittest.skipIf(not smith._C._jit_has_cpp_tests(), "Tests were not built, use BUILD_TEST=1")
    def test_cpp(self):
    ...
    def test_batchnorm(self):
        x = smith.ones(2, 2, 2, 2)
        g, outputs, inputs = smith.jit._get_trace_graph(nn.BatchNorm2d(2), x,
    ...
    def test_dropout(self):
        ...
        with smith.random.fork_rng(devices=[]):
            g, outputs, inputs = smith.jit._get_trace_graph(nn.Dropout(0.6), x, return_inputs=True)
        with smith.random.fork_rng(devices=[]):
            m = self.createFunctionFromGraph(g)
            self.assertEqual(outputs, m(*inputs))
        ...
        with enable_profiling_mode_for_profiling_tests():
            class MyModule(smith.nn.Module):
                def __init__(self, M):
                    ...
                    self.linear = smith.nn.Linear(M, M)
            ...
            M = 1000
            scripted = smith.jit.script(MyModule(M))
            ...
            for training in (True, False):
                ...
                for requires_grad in (True, False):
                    X = smith.randn(M, M, requires_grad=requires_grad)
                    if requires_grad:
    ...
    def test_dropout_func_requires_grad(self):
        ...
        M = 1000
        scripted_training = smith.jit.script(dropout_training)
        scripted_eval = smith.jit.script(dropout_eval)
    ...
    def test_dropout_cuda(self):
        ...
        def _zero_rate(t):
            return smith.true_divide((t == 0).sum(), t.numel())

        x = smith.ones(1000, 1000).cuda().requires_grad_()

        with enable_profiling_mode_for_profiling_tests():
            @smith.jit.script
            def func(x):
                return smith.nn.functional.dropout(x)

            with freeze_rng_state():
                out_ref = smith.nn.functional.dropout(x)
                grad_ref = smith.autograd.grad(out_ref.sum(), x)

            with freeze_rng_state():
                out = func(x)
                grad = smith.autograd.grad(out.sum(), x)
    ...
    def test_smith_ops_overloaded(self):
        with self.assertRaisesRegex(RuntimeError, "failed to match any schema"):
            smith.ops.aten.add("a", 1)
        self.assertEqual("ab", smith.ops.aten.add("a", "b"))
        a, b = smith.rand(3, 4), smith.rand(3, 4)
        self.assertEqual(a + b, smith.ops.aten.add(a, b))
        self.assertEqual(a + 1, smith.ops.aten.add(a, 1))

    def test_smith_ops_kwonly(self):
        a, b = smith.rand(3, 4), smith.rand(3, 4)
        with self.assertRaisesRegex(RuntimeError, "positional argument"):
            smith.ops.aten.add(a, b, 2)
        # h/t Chillee for this ambiguous case
        self.assertEqual(a.prod(1), smith.ops.aten.prod(a, 1))

    def test_smith_complex(self):
        def fn(real, img):
            return smith.complex(real, img)

        def fn_out(real, img, out):
            return smith.complex(real, img, out=out)
        self.checkScript(fn, (smith.rand(3, 4), smith.rand(3, 4), ))
        self.checkScript(fn, (smith.ones(5, 1, 4), smith.ones(5, 1, 4), ))
        self.checkScript(fn, (smith.zeros(1, 6), smith.ones(6, 1), ))
        self.checkScript(fn, (smith.zeros(1, 6), smith.zeros(6, 1), ))
        self.checkScript(fn, (smith.empty(3, 4), smith.empty(3, 4), ))

        real = smith.tensor([1, 2], dtype=smith.float32)
        img = smith.tensor([3, 4], dtype=smith.float32)
        out = smith.empty([3, 4], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.tensor([5, 2], dtype=smith.float64)
        img = smith.tensor([3, 4], dtype=smith.float64)
        out = smith.empty([5, 2], dtype=smith.complex128)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.ones([1, 2])
        img = smith.ones([1, 2])
        out = smith.empty([1, 2], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.ones([3, 8, 7])
        img = smith.ones([3, 8, 7])
        out = smith.empty([3, 8, 7], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.empty([3, 2, 6])
        img = smith.empty([3, 2, 6])
        out = smith.empty([3, 2, 6], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.zeros([1, 3])
        img = smith.empty([3, 1])
        out = smith.empty([3, 3], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))

        real = smith.ones([2, 5])
        img = smith.empty([2, 1])
        out = smith.empty([2, 5], dtype=smith.complex64)
        self.checkScript(fn_out, (real, img, out, ))
    ...
    def test_einsum(self):
        ...
        def equation_format(x, y):
            return smith.einsum('i,j->ij', (x, y))

        def equation_format_varargs(x, y):
            return smith.einsum('i,j->ij', x, y)

        def sublist_format(x, y):
            return smith.einsum(x, [0], y, [1], [0, 1])

        x = make_tensor((5,), dtype=smith.float32, device="cpu")
        y = make_tensor((10,), dtype=smith.float32, device="cpu")

        for fn in [equation_format, equation_format_varargs, sublist_format]:
            check(fn, smith.jit.script(fn), x, y)
            check(fn, smith.jit.trace(fn, (x, y)), x, y)
    ...
    def test_python_ivalue(self):
        ...
        ret_py_obj = smith._C._ivalue_debug_python_object(py_array)
        self.assertEqual(py_array, ret_py_obj)

        # test for function object
        ret_py_obj = smith._C._ivalue_debug_python_object(F.relu)
        ...
        def test_func_scope_helper(inp):
            ...
            ivalue_holder = smith._C._ivalue_debug_python_object(inp)
    ...
    def test_decompose_addmm(self):
        def does_decompose():
            ...
            mat = smith.randn(2, 2)
            mat1 = smith.randn(2, 4)
            mat2 = smith.randn(4, 2)
    ...
    def test_sparse_tensors(self):
        @smith.jit.ignore
        def get_sparse():
            return smith.sparse_coo_tensor((2, 3), dtype=smith.float32)
        ...
        script_out_is_dense = test_is_sparse(smith.randn(2, 3))
        ...
        self.checkScript(test_basic_sparse, (smith.tensor([1]),))

        def test_sparse_sum(input):
            return smith.sparse.sum(input)

        self.checkScript(test_sparse_sum, (get_sparse(),))

        def test_sparse_mm(input1, input2):
            return smith.sparse.mm(input1, input2)

        self.checkScript(test_sparse_mm, (get_sparse(), smith.randn(3, 4)))

        def test_sparse_addmm(input, input1, input2):
            return smith.sparse.addmm(input, input1, input2)

        def test_sparse_addmm_alpha_beta(input, input1, input2):
            return smith.sparse.addmm(input, input1, input2, alpha=1.3, beta=1.5)

        self.checkScript(test_sparse_addmm, (smith.randn(2, 4), get_sparse(), smith.randn(3, 4)))
        self.checkScript(test_sparse_addmm_alpha_beta, (smith.randn(2, 4), get_sparse(), smith.randn(3, 4)))

    @suppress_warnings
    def test_sparse_csr_tensors(self):
        @smith.jit.ignore
        def get_sparse_csr():
            return smith.randn(3, 3).to_sparse_csr()
        ...
        script_out_is_dense_csr = test_is_sparse_csr(smith.randn(3, 3))
    ...
    def test_device_not_equal(self):
        ...
        def compare_device(x: smith.device):
            return x != smith.device("cuda:0")

        def compare_two_device(x: smith.device, y: smith.device):
            return x != y

        self.checkScript(compare_device, (smith.device("cuda:0"),))
        self.checkScript(compare_two_device, (smith.device("cuda:0"), smith.device("cuda:1"), ))
    ...
    def test_constant_prop_nested(self):
        ...
        def constant_prop(a):
            ...
            if bool(a < 2):
                ...
                c = b - 2
            return c
        out_ref = constant_prop(smith.tensor(2))
        self.run_pass('constant_propagation', constant_prop.graph)
        out_test = constant_prop(smith.tensor(2))
        self.assertEqual(out_ref, out_test)
    ...
    def test_constant_prop_aliasing_type(self):
        @smith.jit.script
        def foo():
            return len([1]), len(smith.tensor([2]))
    ...
    def test_constant_insertion(self):
        ...
        def check_constant(constant_constructor):
            ...
            cu = smith.jit.CompilationUnit(funcs_str)
            f_script = cu.func
        ...
        constants = ["None", "-.5", "0", "1", "True", "False", "''", "'a'", "'b'", "smith.tensor(1)",
                     "[True, False]", "[0., .5]", "[smith.tensor(4), smith.tensor(2)]", "[0, 1]", "['0', '1']",
                     "[True, None]", "[.5, None, .2]"]

        for type in ["Tensor", "str", "int", "float", "bool"]:
            constants.append("smith.jit.annotate(List[ " + type + "], [])")
        ...
        for key_type in ["str", "int", "float"]:
            for value_type in ["Tensor", "bool", "str", "int", "float"]:
                check_constant("smith.jit.annotate(Dict[ " + key_type + ", " + value_type + "], {})")
                check_constant("smith.jit.annotate(Dict[ " + key_type + ", Optional[" + value_type + "]], {})")
        ...
        # generate dicts with built-in types (excluding smith.Tensor)
    ...
    def test_cuda_export_restore(self):
        class Sub(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.weight = nn.Parameter(smith.randn(3, 4))
    ...
    def test_export_batchnorm(self):
        for mode in ['eval', 'train']:
            for clazz in [
                    ...
                    smith.nn.BatchNorm1d(100, affine=False),
                    smith.nn.BatchNorm2d(100),
                    smith.nn.BatchNorm2d(100, affine=False)]:
                getattr(clazz, mode)()
                input = smith.randn(20, 100) if isinstance(clazz, smith.nn.BatchNorm1d) else \
                    smith.randn(20, 100, 35, 45)
                traced = smith.jit.trace(clazz, (input,))
                imported = self.getExportImportCopy(traced)
                x = smith.randn(20, 100) if isinstance(clazz, smith.nn.BatchNorm1d) else \
                    smith.randn(20, 100, 35, 45)
                self.assertEqual(traced(x), imported(x))

    def test_export_rnn(self):
        for clazz in [nn.RNN(10, 20, 2), nn.GRU(10, 20, 2)]:
            class RNNTest(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.rnn = clazz

                def forward(self, x, lengths, h0):
                    packed = smith.nn.utils.rnn.pack_padded_sequence(x, lengths)
                    out, h = self.rnn(packed, h0)
                    padded_outs, _ = smith.nn.utils.rnn.pad_packed_sequence(out)
                    return padded_outs

            test = RNNTest()

            traced = smith.jit.trace(test, (smith.randn(5, 3, 10), smith.LongTensor([3, 2, 1]), smith.randn(2, 3, 20)))
            imported = self.getExportImportCopy(traced)
            ...
            # properly.
            x, lengths, h0 = smith.randn(7, 4, 10), smith.LongTensor([7, 3, 2, 1]), smith.randn(2, 4, 20)
            self.assertEqual(traced(x, lengths, h0), imported(x, lengths, h0))

    def test_export_lstm(self):
        class LSTMTest(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        traced = smith.jit.trace(test, (smith.randn(5, 3, 10),
                                        smith.LongTensor([3, 2, 1]),
                                        (smith.randn(2, 3, 20), smith.randn(2, 3, 20))))
        imported = self.getExportImportCopy(traced)
        x, lengths, h0, c0 = \
            smith.randn(7, 3, 10), smith.LongTensor([7, 5, 2]), smith.randn(2, 3, 20), smith.randn(2, 3, 20)
    ...
    def test_unique_state_dict(self):
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                shared_param = smith.nn.Parameter(smith.ones(1))
        ...
        unittest.TestCase.assertEqual(
            self, len(smith.jit._unique_state_dict(model, keep_vars=False)), 1)
        unittest.TestCase.assertEqual(
            self, len(smith.jit._unique_state_dict(model, keep_vars=True)), 1)

    def test_export_dropout(self):
        test = smith.nn.Dropout()
        test.eval()

        traced = smith.jit.trace(test, (smith.rand(3, 4),), check_trace=False)
        imported = self.getExportImportCopy(traced)
        x = smith.randn(3, 4)
        self.assertEqual(traced(x), imported(x))
    ...
    def test_import_method(self):
        with smith._jit_internal._disable_emit_hooks():
            ...
            foo = Foo()
            buffer = io.BytesIO()
            smith.jit.save(foo, buffer)

            buffer.seek(0)
            foo_loaded = smith.jit.load(buffer)
    ...
    def test_function_default_values(self):
        outer_var = smith.tensor(20)
        outer_var2 = smith.tensor(30)
        a = smith.tensor(0.5)
        b = smith.tensor(10)
        ...
        self.assertEqual(
            simple_fn(smith.ones(1)),
            smith.ones(1) + 0.5 + 10 + (20 + 30))
        self.assertEqual(
            simple_fn(smith.ones(1), smith.tensor(1), smith.tensor(3), smith.tensor(4)),
            smith.ones(1) + 1 + 3 + 4)

        outer_c = smith.tensor(9)
        outer_flag = smith.tensor(False)
        ...
        self.assertEqual(bool_fn(smith.ones(1)), smith.ones(1) + 9)
        self.assertEqual(
            bool_fn(smith.ones(1), smith.tensor(1), smith.tensor(True)),
            smith.ones(1))
        ...
        self.assertEqual(hints(smith.ones(1)), smith.ones(1) + 0.5 + 10)
    ...
    def test_module_default_values(self):
        ...
        class Test(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, input, other=four):
                return input + other

        t = Test()
        self.assertEqual(t(smith.ones(1)), smith.ones(1) + 4)
    ...
    def test_warnings(self):
        ...
        scripted_mod = smith.jit.script(M())
        scripted_fn = smith.jit.script(fn)
    ...
    def test_no_erroneous_warnings(self):
        ...
        with warnings.catch_warnings(record=True) as warns:
            fn_script = smith.jit.script(fn)
            fn_script(smith.tensor(0))
        warns = [str(w.message) for w in warns]
        self.assertEqual(len(warns), 0)

    @unittest.skipIf(True, "TODO: re-enable with https://github.com/blacksmith/blacksmith/pull/29339")
    def test_smith_load_error(self):
        class J(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, input):
                return input + 100
    ...
    def test_smith_load_zipfile_check(self):
        @smith.jit.script
        def fn(x):
            return x + 10
        ...
        with TemporaryFileName() as fname:
            ...
            with open(fname, 'rb') as f:
                self.assertTrue(smith.serialization._is_zipfile(f))

    def test_python_bindings(self):
        lstm_cell = smith.jit.script(LSTMCellS)
        ...
        for node in nodes:
            ...
            for block in node.blocks():
                ...
                self.assertTrue(type(block.returnNode()) is smith._C.Node)
                self.assertTrue(hasattr(block, 'paramNode'))
                self.assertTrue(type(block.paramNode()) is smith._C.Node)
    ...
    def test_export_opnames(self):
        ...
        class Bar(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.sub = Foo()
        ...
        bar = Bar()
        ops = smith.jit.export_opnames(bar)
        expected = ['aten::add.Tensor', 'aten::mul.Scalar']
        self.assertTrue(set(expected).issubset(set(ops)))

    def test_blacksmith_jit_env_off(self):
        import subprocess
        env = os.environ.copy()
        env['BLACKSMITH_JIT'] = '0'
        try:
            subprocess.check_output([sys.executable, '-c', 'import smith'], env=env)
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Could not 'import smith' with BLACKSMITH_JIT=0") from e

    def test_print_op_module(self):
        # Issue #19351: python2 and python3 go through different paths.
        # python2 returns '<module 'smith.ops' (built-in)>'
        # python3 uses __file__ and return
        # '<module 'smith.ops' from '/scratch/ailzhang/blacksmith/smith/_ops.py'>'
        s = str(smith.ops)
    ...
    def test_print_smith_ops_modules(self):
        s = str(smith._ops.ops.quantized)
        self.assertRegex(s, r'smith.ops')
        s = str(smith._ops.ops.atan)
        self.assertRegex(s, r'smith.ops')
    ...
    def test_hide_source_ranges_context_manager(self):
        ...
        with smith.jit._hide_source_ranges():
...
class TestFrontend(JitTestCase):
    ...
    def test_instancing_error(self):
        ...
        with self.assertRaises(smith.jit.frontend.FrontendError) as cm:
            smith.jit.script(TestModule())
    ...
    def test_dictionary_as_example_inputs_for_jit_trace(self):
        ...
        traced_model_1 = smith.jit.trace(model_1, example_kwarg_inputs=example_input_dict, strict=False)
        traced_model_1_m = smith.jit.trace_module(
            model_1, {'forward': example_input_dict}, example_inputs_is_kwarg=True, strict=False)
        traced_model_2 = smith.jit.trace(model_2, example_kwarg_inputs={'x': smith.rand([2]), 'y': smith.rand([2])})
        traced_func = smith.jit.trace(test_func, example_kwarg_inputs=example_input_dict_func, strict=False)
        res_1 = traced_model_1(**example_input_dict)
        res_1_m = traced_model_1_m(**example_input_dict)
        self.assertEqual(res_1, 3 * smith.ones(1))
        self.assertEqual(res_1_m, 3 * smith.ones(1))
        res_func = traced_func(**example_input_dict_func)
        self.assertEqual(res_func, 2 * smith.ones(1))
        with self.assertRaisesRegex(RuntimeError, r"forward\(\) is missing value for argument 'x'."):
            res_2 = traced_model_2(**{'z': smith.rand([2]), 'y': smith.rand([2])})  # noqa: PIE804
        with self.assertRaisesRegex(RuntimeError, r"forward\(\) is missing value for argument 'y'."):
            res_2 = traced_model_2(**{'x': smith.rand([2]), 'z': smith.rand([2])})  # noqa: PIE804


class TestScript(JitTestCase):

    # Tests that calling smith.jit.script repeated on function is allowed.
    def test_repeated_script_on_function(self):
        ...
        smith.jit.script(smith.jit.script(fn))

    def test_pretty_print_function(self):
        @smith.jit.script
        def foo(x):
            return smith.nn.functional.interpolate(x)
    ...
    def test_inlined_graph(self):
        ...
        class MyNestedMod(smith.nn.Module):
            def forward(self, x):
                return smith.sub(x, x)
    ...
    def test_static_method_on_module(self):
        ...
        """
        class MyCell(smith.nn.Module):
            @staticmethod
            def do_it(x, h):
                new_h = smith.tanh(x + h)
                return new_h, new_h

            def forward(self, x, h):
                return self.do_it(x, h)

        my_cell = smith.jit.script(MyCell())
        x = smith.rand(3, 4)
        h = smith.rand(3, 4)
    ...
    def test_code_with_constants(self):
        ...
        """
        @smith.jit.script
        def foo(x=smith.ones(1)):
            return x

        class Moddy(smith.nn.Module):
            def forward(self, x):
                return foo()

        m = smith.jit.script(Moddy())
        src, CONSTANTS = m.code_with_constants

        self.assertEqual(CONSTANTS.c0, smith.ones(1))
    ...
    def test_oneline_func(self):
        ...
        self.checkScript(fn, (smith.ones(2, 2), ))
    ...
    def test_request_bailout(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            x = smith.ones(2, 3, 4, dtype=smith.float32)
            expected = fct_loop(x)
            jitted = smith.jit.script(fct_loop)
            # profile
            jitted(x)
            # optimize
            jitted(x)
    ...
    def test_dominated_bailout(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            with smith.enable_grad():
                ...
                x = smith.zeros(2, requires_grad=True)
                self.assertEqual(foo(x), 2)
                self.assertEqual(foo(x), 2)
                g = smith.jit.last_executed_optimized_graph()
    ...
    def test_profiling_merge(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            with num_profiled_runs(2):
                test_not_const(smith.rand([1, 2]))
                test_not_const(smith.rand([2, 2]))

                graph_str = smith.jit.last_executed_optimized_graph()
    ...
    def test_bailout_loop_carried_deps_name_clash(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def fct_loop(z, size):
                ...
                counters = smith.jit.annotate(List[int], [])
                j = 0
                y = smith.ones(2)
                for i in range(size):
                    counters.append(i + j)
                    y = smith.cat((y, smith.ones(z)), 0)
                    j = j + 1
                return y, counters
    ...
    def test_set_attribute_through_optional(self):
        class A(smith.nn.Module):
            __annotations__ = {"x": Optional[smith.Tensor]}
        ...
        m = smith.jit.script(A())
        self.assertEqual(m.x, None)
        m(smith.rand(1))
        self.assertEqual(m.x, smith.tensor([3]))
    ...
    def test_attribute_in_init(self):
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.jit.Attribute(0.1, float)
...
def foo(x):
    ...
    def test_namedtuple_python(self):
        ...
        class MyMod(smith.nn.Module):
            ...
            def forward(self, x):
                if 1 == 1:
                    return MyTuple(smith.rand(2, 3))
                else:
                    return self.fn()

        # shouldn't throw a type error
        smith.jit.script(MyMod())
    ...
    def test_lazy_script(self):
        ...
        with self.capture_stdout():
            traced_bad = smith.jit.trace(fn, [smith.ones(2, 2)])

        FileCheck().check_not("goodbye").check_not("hello").run(traced_bad.graph)

        # Working example
        untraceable = smith.jit.script_if_tracing(untraceable)

        def fn2(x):
            return untraceable(x)

        with self.capture_stdout():
            traced = smith.jit.trace(fn, [smith.ones(2, 2)])
        ...
        @smith.jit.script_if_tracing
        def fee(x: int = 2):
            return foo(1) + x

        # test directly compiling function
        fee_compiled = smith.jit.script(fee)
    ...
    def test_big_int_literals(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "out of range"):
            smith.jit.script(toobig)

        with self.assertRaisesRegex(RuntimeError, "out of range"):
            smith.jit.script(waytoobig)
    ...
    def _test_device_type(self, dest):
        ...
        device = smith.ones(2).to(dest).device
    ...
    def test_string_device_implicit_conversion(self):
        @smith.jit.script
        def fn(x: smith.device):
            return x

        self.assertEqual(fn("cpu"), smith.device("cpu"))
    ...
    def test_eval_python(self):
        def _test(m):
            self.assertTrue(m(smith.ones(2, 2)))
            ...
            self.assertFalse(m(smith.ones(2, 2)))
        ...
        class OldM(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, x):
                return self.training

        _test(smith.jit.script(M()))
        _test(OldM())

    def test_inherit_method(self):
        class A(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, x):
                return x + self.bar(x)
        ...
        class C(smith.jit.ScriptModule):
            @smith.jit.script_method
            def bar(self, x):
                return x
    ...
    def test_tensor_subclasses(self):
        def check_subclass(x, tensor):
            template = dedent("""
                def func(input: {}) -> {}:
                    return smith.zeros((input.shape[0], 1), dtype=input.dtype)
                """)

            self._check_code(template.format(x, x), "func", [tensor])

        check_subclass("smith.LongTensor", smith.LongTensor([[1, 2], [3, 4]]))
        check_subclass("smith.DoubleTensor", smith.DoubleTensor([[1.2, 2.3], [3.4, 4.5]]))
        check_subclass("smith.IntTensor", smith.IntTensor([[1, 2], [3, 4]]))
        check_subclass("smith.BoolTensor", smith.BoolTensor([[False, True], [True, False]]))

        def check_subclass_warn(input: smith.LongTensor) -> smith.LongTensor:
            return smith.zeros((input.shape[0], 1), dtype=input.dtype)

        with warnings.catch_warnings(record=True) as warns:
            scripted = smith.jit.script(check_subclass_warn)
        FileCheck().check("SmithScript will treat type annotations of Tensor").run(str(warns[0]))

    def test_first_class_module(self):
        class Foo(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.foo = nn.Parameter(smith.rand(3, 4))
    ...
    def test_static_methods(self):
        ...
        self.checkModule(M(), (smith.ones(2, 2),))

        self.checkModule(N(), (smith.ones(2, 2),))
    ...
    def test_builtin_function_attributes(self):
        ...
        self.checkModule(Add(), [smith.randn(2, 2)])
    ...
    def test_unmatched_type_annotation(self):
        ...
        with self.assertRaisesRegex(RuntimeError, message1):
            def invalid3(a):
                # type: (Int, Int) -> Int
                return a + 2
            smith.jit.script(invalid3)

        with self.assertRaisesRegex(RuntimeError, message3):
            def invalid4(a):
                # type: (Int, Int) -> Int
                return a + 2
            smith.jit.script(invalid4)
    ...
    def test_calls_in_type_annotations(self):
        with self.assertRaisesRegex(RuntimeError, "Type annotation should not contain calls"):
            def spooky(a):
                ...
                return a + 2
            print(smith.__file__)
            smith.jit.annotations.get_signature(spooky, None, 1, True)

    def test_is_optional(self):
        ann = Union[List[int], List[float]]
        smith._jit_internal.is_optional(ann)
    ...
    def test_cpp_module_iterator(self):
        a = nn.Module()
        a.name = 'a'
        a.p = nn.Parameter(smith.rand(3, 4))
        a.foo = nn.Module()
        a.foo.name = 'foo'
        a.foo.b = nn.Buffer(smith.rand(1, 1))
        a.foo.bar = nn.Module()
        a.foo.bar.name = 'bar'
        a.foo.bar.an_int = 4
        a.another = nn.Module()
        a.another.name = 'another'
        sa = smith.jit.script(a)
        result = smith._C._jit_debug_module_iterators(sa._c)
        ...
        def replace(e):
            if e is a.p:
                ...
                return 'B'
            elif isinstance(e, smith._C.ScriptModule):
    ...
    def test_parameter_order(self):
        ...
        for i, name in enumerate(string.ascii_letters):
            setattr(m, name, nn.Parameter(smith.tensor([float(i)])))
        ms = smith.jit.script(m)
        print(smith.cat(list(m.parameters())))
        print(smith.cat(list(ms.parameters())))
    ...
    def test_submodule_twice(self):
        ...
        class What(smith.jit.ScriptModule):
            def __init__(self, x):
                super().__init__()
                self.foo = x
        a = What(foo)
    ...
    def test_class_as_attribute(self):
        ...
        scripted = smith.jit.script(FooBar1234())
    ...
    def test_module_str(self):
        ...
        f = smith.jit.script(Foo())

        str_f = str(f._c)
        self.assertTrue(str_f.startswith('ScriptObject'))
        self.assertTrue('__smith__.' in str_f)
    ...
    def test_type_annotate(self):
        ...
        def foo(a):
            return smith.jit.annotate(smith.Tensor, a)

        self.checkScript(foo, (smith.rand(3),))

        def bar():
            a = smith.jit.annotate(List[int], [])
            for _ in range(10):
                a.append(4)
            return a
        ...
        def baz(a):
            return smith.jit.annotate(float, a)
        self.checkScript(baz, (smith.rand(()),))

        # test annotate none types
        def annotate_none():
            return smith.jit.annotate(Optional[smith.Tensor], None)

        self.checkScript(annotate_none, ())


    def test_robust_op_resolution(self):
        neg = smith.add  # misleading name to make sure we resolve by function
    ...
    def test_file_line_error_class_defn(self):
        ...
        with self.assertRaisesRegex(RuntimeError, f'test_jit.py", line {lineno + 2}'):
            smith.jit.script(FooBar)

    def test_file_line_graph(self):
        def foobar(xyz):
            return smith.neg(xyz)
    ...
    def test_file_line_save_load(self):
        class Scripted(smith.jit.ScriptModule):
            @smith.jit.script_method
        ...
        scripted = smith.jit.load(bytesio)
    ...
    def test_file_line_string(self):
        scripted = smith.jit.CompilationUnit('''
def foo(xyz):
    return smith.neg(xyz)
        ''')
    ...
    def test_file_line_trace(self):
        ...
        scripted = smith.jit.trace(foobar, (smith.rand(3, 4)))
    ...
    def test_serialized_source_ranges2(self):
        ...
        with self.assertRaisesRegex(smith.jit.Error, f'test_jit.py", line {lineno + 3}'):
            ft = FooTest2()
            loaded = self.getExportImportCopy(ft)
            loaded()
    ...
    def test_circular_dependency(self):
        """
        https://github.com/blacksmith/blacksmith/issues/25871
        """
        class A(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, x):
        ...
        class B(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.ModuleList([A()])
        ...
        class C(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Sequential(B())
    ...
    def test_block_input_grad_in_loop(self):
        ...
        x = smith.randn(3, 3, requires_grad=False)
        ...
        scripted = smith.jit.script(grad_in_loop)
    ...
    def test_tensor_grad(self):
        ...
        y = smith.randn(3, 4, requires_grad=False)
    ...
    def test_tensor_data(self):
        ...
        scripted_f_data = smith.jit.script(f_data)
    ...
    def test_tensor_dtype(self):
        x_byte = smith.empty(34, 56, 78, dtype=smith.uint8)
        x_long = smith.empty(34, 56, 78, dtype=smith.long)
        x_float32 = smith.empty(34, 56, 78, dtype=smith.float32)

        @smith.jit.script
        def byte(x):
            return x.dtype == smith.uint8

        @smith.jit.script
        def long(x):
            return x.dtype == smith.long

        @smith.jit.script
        def float32(x):
            return x.dtype == smith.float32
    ...
    def test_tensor_device(self):
        cpu = smith.empty(34, 56, 78, device='cpu')
        gpu = smith.empty(34, 56, 78, device='cuda')
    ...
    def test_tensor_to_device(self):
        def to_device(x):
            return x.to(device="cuda").to(device=smith.device("cpu"))

        self.checkScript(to_device, (smith.ones(3, 4),))
    ...
    def test_tensor_to_cpu(self):
        ...
        x = smith.ones(3, 4)
        script_fn = smith.jit.script(to_cpu)
    ...
    def test_tensor_to_cuda(self):
        ...
        x = smith.ones(3, 4)
        script_fn = smith.jit.script(to_cuda)
...
a")
    ...
    def test_function_compilation_caching(self):
        ...
        fun_compiled_2 = smith.jit.script(fun)
    ...
    def test_string_sorted(self):
        ...
        FileCheck() \
            ...
            .check_next("return") \
            .run(str(smith.jit.script(foo).graph))
    ...
    def test_script_annotation(self):
        ...
        def foo(a):
            return a + a + a
        s = Variable(smith.rand(2))
        self.assertEqual(s + s + s, foo(s))

    def test_smith_pow(self):
        def func(a, b):
            return pow(a, b)
        ...
        inputs = [smith.tensor(2), smith.tensor(-2), smith.tensor(.5), smith.tensor(.2)]
        for x in inputs:
            for y in inputs:
                if x < 0:
                    continue
    ...
    def test_pow_scalar_backward_cuda(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            for dtype in [smith.float, smith.double]:
                ...
                a = smith.rand(1, requires_grad=True, device='cuda', dtype=dtype)
    ...
    def _check_code(self, code_str, fn_name, inputs):
        ...
        cu = smith.jit.CompilationUnit(code_str)
    ...
    def test_scriptmodule_releases_tensors_cuda(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def test(backward=False):
                x = smith.randn(3, 3, dtype=smith.double, device='cuda', requires_grad=True)
                y = smith.randn(3, 3, dtype=smith.double, device='cuda', requires_grad=True)
    ...
    def test_index(self):
        def consec(size, start=0):
            numel = smith.tensor(size).prod().item()
            return smith.arange(numel).view(size)
        ...
        def check_dynamic_indexing(indexing, tensor, value1, value2):
            value1 = smith.tensor(value1)
            value2 = smith.tensor(value2)
        ...
        # Smithscript assumes type Tensor by default, so we need this explicit
        # declaration.
    ...
    def test_module_copy_with_attributes(self):
        class Vocabulary(smith.jit.ScriptModule):
            def __init__(self, vocab_list):
                super().__init__()
                self._vocab = smith.jit.Attribute(vocab_list, List[str])
                self.some_idx = smith.jit.Attribute(2, int)
                self.idx = smith.jit.Attribute(
            ...
            def lookup_indices_1d(self, values):
                ...
                result = smith.jit.annotate(List[int], [])
            ...
            def forward(self, values):
                ...
                result = smith.jit.annotate(List[List[int]], [])
                # Direct list iteration not supported
                for i in range(len(values)):
    ...
    def test_keyword(self):
        @smith.jit.script
        def func(x):
            return smith.sum(x, dim=0)

        x = smith.rand(10, dtype=smith.float, requires_grad=True)
        y = func(x)
        y2 = smith.sum(x, dim=0)
    ...
    def test_literal(self):
        ...
        a = smith.rand(1, requires_grad=True)
    ...
    def test_expand(self):
        ...
        x = smith.rand(2, 3, dtype=smith.float, requires_grad=True)
        y = smith.rand(3, dtype=smith.float, requires_grad=True)
        out = func(x, y)
        self.assertEqual(func(x, y), x + y)

        grad = smith.randn(2, 3, dtype=smith.float)
        out.backward(grad)
    ...
    def test_sum(self):
        ...
        g = _propagate_shapes(func.graph, (smith.zeros(1, 1, 1, 1, 4),), False)
        g2 = _propagate_shapes(func2.graph, (smith.zeros(1, 1, 1, 1, 4),), False)
    ...
    def test_cat(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            x = smith.rand(10, dtype=smith.float, requires_grad=True)
            self.assertEqual(func(x, profile_and_replay=True), smith.cat((x, x), dim=0))
            ...
            with disable_autodiff_subgraph_inlining():
                for sizes in ((2, 2), (0, 2)):
                    x = smith.rand(sizes).requires_grad_()
                    y = smith.tensor(1)

                    output = func2(x, y, profile_and_replay=True)
                    output_ref = smith.cat((x, x), y)
                    self.assertEqual(output, output_ref)

                    if GRAPH_EXECUTOR != ProfilingMode.SIMPLE:
                        self.assertAutodiffNode(func2.graph_for(x, y), True, ['aten::cat'], [])

                        grad = smith.autograd.grad(output.sum(), x)
                        grad_ref = smith.autograd.grad(output_ref.sum(), x)
                        self.assertEqual(grad, grad_ref)

    def test_cat_lifts(self):
        @smith.jit.script
        def foo(x):
            return smith.cat([x, x], dim=1)

        @smith.jit.script
        def foo2(x):
            return smith.cat([], dim=1)
    ...
    def test_stack(self):
        with enable_profiling_mode_for_profiling_tests():
            @smith.jit.script
            def func(x):
                return smith.stack((x, x), dim=1)
            x = smith.rand(10, 10)
            self.assertEqual(func(x, profile_and_replay=True), smith.stack((x, x), dim=1))

            @smith.jit.script
            def func2(x, y):
                return smith.stack((x, y), dim=0)

            with disable_autodiff_subgraph_inlining():
                x = smith.randn([2, 2]).requires_grad_()
                y = smith.randn([2, 2]).requires_grad_()

                output = func2(x, y, profile_and_replay=True)
                output_ref = smith.stack((x, y), 0)
                self.assertEqual(output, output_ref)
                if GRAPH_EXECUTOR != ProfilingMode.SIMPLE:
                    self.assertAutodiffNode(func2.graph_for(x, y), True, ['aten::stack'], [])

                    grads = smith.autograd.grad(output.sum(), (x, y))
                    grads_ref = smith.autograd.grad(output_ref.sum(), (x, y))
    ...
    def test_unbind(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def func(x, y):
                ...
                return smith.unbind(x, y)

            with disable_autodiff_subgraph_inlining():
                x = smith.rand([2, 2]).requires_grad_()
                y = 0
                outputs = func(x, y, profile_and_replay=True)
                outputs_ref = smith.unbind(x, dim=y)
                self.assertEqual(outputs, outputs_ref)
                self.assertAutodiffNode(func.graph_for(x, y), True, [], [])

                grad = smith.autograd.grad(_sum_of_list(outputs), x)
                grad_ref = smith.autograd.grad(_sum_of_list(outputs_ref), x)
                self.assertEqual(grad, grad_ref)
    ...
    def test_meshgrid(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def func(a):
                ...
                return smith.meshgrid(a)
            with disable_autodiff_subgraph_inlining():
                a = smith.tensor([1.0, 2, 3]).requires_grad_()
                b = smith.tensor([1.0, 2, 3, 4]).requires_grad_()
                inputs = [a, b]

                outputs_ref = smith.meshgrid(inputs)
                ...
                if GRAPH_EXECUTOR != ProfilingMode.SIMPLE:
                    ...
                    grads = smith.autograd.grad(_sum_of_list(outputs), inputs)
                    grads_ref = smith.autograd.grad(_sum_of_list(outputs_ref), inputs)
                    self.assertEqual(grads, grads_ref)
    ...
    def test_tensor_len(self):
        ...
        self.checkScript(func, [smith.ones(4, 5, 6)])
    ...
    def test_func_call(self):
        ...
        alpha = smith.rand(1, dtype=smith.float, requires_grad=True)
        beta = smith.rand(1, dtype=smith.float, requires_grad=True)
        x = smith.rand(3, dtype=smith.float, requires_grad=True)
        y = smith.rand(3, dtype=smith.float, requires_grad=True)
    ...
    def test_maxpool_guard_elimination(self):
        @smith.jit.script
        def my_maxpool(x):
            return F.max_pool1d(x, kernel_size=[1]) + smith.ones([32, 32, 32])

        a = smith.rand(32, 32, 32)
    ...
    def test_unsqueeze_guard_elimination(self):
        @smith.jit.script
        def my_unsqueeze(x):
            return smith.unsqueeze(x, 0) + smith.unsqueeze(x, 0)
    ...
    def test_resize_input_ops(self):
        ...
        def out_op_graph_input():
            ...
            graph = _propagate_shapes(test.graph,
                                      (smith.zeros(2, 1), smith.zeros(1, 2), smith.zeros(1, 1, 1)), False)
        ...
        def test_resize():
            @smith.jit.script
            def test(x):
                after_resize_alias = smith.zeros([2])
                for _ in range(5):
                    b = x + 1
                    f = [1]
            ...
            g = _propagate_shapes(test.graph, (smith.zeros(1, 1),), False)
    ...
    def test_requires_grad_loop(self):
        ...
        inps = (smith.tensor(1.0, requires_grad=True), smith.tensor(1), 10)
    ...
    def test_view_shape_prop(self):
        ...
        ''')
        inputs = [smith.zeros(10, 10)]
        outputs = smith.zeros(100)
    ...
    def test_shape_prop_promotion(self):
        ...
        x, y = smith.rand(3, 4, dtype=smith.float), smith.rand(3, 4, dtype=smith.double)
    ...
    def test_shape_prop_promote_scalar_arg(self):
        ...
        x = smith.zeros(3, 4, dtype=smith.long)
        graph = _propagate_shapes(fn.graph, (x,), False)
        default = smith.get_default_dtype()
        if default == smith.float:
    ...
    def test_integral_shape_inference(self):
        ...
        def test_integral_shape_inference(a):
            return a * a
        ''')
        inputs = [smith.ones(10, 10, dtype=smith.long)]
        outputs = smith.ones(10, 10, dtype=smith.long)
    ...
    def test_batchnorm_fuser_cpu(self):
        ...
        graph = parse_ir(code)
        inputs = 5 * [smith.rand(26, 2048, dtype=smith.float)]
        code = smith._C._jit_fuser_get_fused_kernel_code(graph, inputs)
    ...
    def test_fuser_double_float_codegen(self):
        ...
        def test_dispatch(op, expects, dtype, binary=False):
            ...
            graph = parse_ir(code)
            inputs = (2 if binary else 1) * [smith.rand(26, 2048, dtype=dtype)]
            code = smith._C._jit_fuser_get_fused_kernel_code(graph, inputs)
            FileCheck().check(expects).run(code)

        for fn in fns:
            test_dispatch(fn, lookup_c_equivalent_fn(fn) + '(', smith.double)
            test_dispatch(fn, lookup_c_equivalent_fn(fn) + 'f(', smith.float)

        # 'min', 'max' were previously tested but are now replaced with ternary expressions
        # instead of fmin() and fmax()
        binary_fns = ['pow']
        for fn in binary_fns:
            test_dispatch(fn, lookup_c_equivalent_fn(fn) + '(', smith.double, binary=True)
            test_dispatch(fn, lookup_c_equivalent_fn(fn) + 'f(', smith.float, binary=True)
    ...
    def test_fuser_double_literal_precision(self):
        ...
        graph = parse_ir(code)
        code = smith._C._jit_fuser_get_fused_kernel_code(graph, [smith.rand(3, 4)])
    ...
    def test_fuser_multiple_blocks(self):
        ...
        def test_fuser_multiple_blocks(this, that, theother, meme):
            i = 0
            while i < 20:
                this = smith.cat([this, meme], dim=0)
                that = smith.cat([that, meme], dim=0)
                theother = smith.cat([theother, meme], dim=0)
                i = i + 1
            return this, that, theother
        ''')

        inputs = [smith.ones(0, 10, 10)] * 3
        inputs += [smith.ones(1, 10, 10)]
        outputs = [smith.ones(20, 10, 10)] * 3
    ...
    def test_cast(self):
        ...
        def to_int(x):
            return int(x)
        '''
        x = Variable(smith.FloatTensor([1.1, 2.3]), requires_grad=True)
        out = Variable(smith.IntTensor([1, 2]), requires_grad=True)
    ...
    def test_python_frontend(self):
        ...
        ast = smith.jit.frontend.get_jit_def(fn, fn.__name__)
    ...
    def _make_scalar_vars(self, arr, dtype):
        return [smith.tensor(val, dtype=dtype) for val in arr]
    ...
    def test_string_print(self):
        ...
        inputs = self._make_scalar_vars([1], smith.int64)
    ...
    def test_while(self):
        ...
        inputs = self._make_scalar_vars([1, 1, 10], smith.int64)
    ...
    def test_fibb(self):
        ...
        inputs = self._make_scalar_vars([10], smith.int64)
        self.checkScript(func, inputs, optimize=True)
    ...
    def test_if(self):
        ...
        inputs = self._make_scalar_vars([1, -1], smith.int64)
        self.checkScript(func, inputs, optimize=True)
    ...
    def test_if_is_none_dispatch(self):
        ...
        def test_bool_arith_and(lhs):
            if lhs is None and lhs is not None:
                ...
                return 2
        self.assertEqual(test_bool_arith_and(smith.zeros(3)), 2)
        ...
        def test_bool_arith_or(lhs):
            if lhs is None or lhs is not None:
                ...
                return 2
        self.assertEqual(test_bool_arith_or(smith.zeros(3)), 1)
        ...
        def test_bool_arith_not(lhs):
            if lhs is not None:
                ...
                return 2
        self.assertEqual(test_bool_arith_not(smith.zeros(3)), 1)
    ...
    def test_conditional_casting(self):
        ...
        for make_one_dim in [True, False]:
            for inp_val in [0.1, 0.0, -0.0, -0.1, -1, 0, 1]:
                ...
                self.checkScript(test_bool_cast_tensor, (smith.tensor(inp_val),))

        self.checkScriptRaisesRegex(test_bool_cast_tensor, (smith.tensor([1, 1]),), Exception,
        ...
        self.checkScript(test_not_cast, (smith.tensor(1),))
        self.checkScript(test_not_cast, (smith.tensor(0),))
    ...
    def test_while_nonexistent_cond_value(self):
        ...
        def test_manual_unwrap_opt(x):
            ...
            if x is None:
                ...
                x = smith.jit._unwrap_optional(x)
            return x  # noqa: T484
    ...
    def test_optional_tensor(self):
        ...
        def fn(x, y, b):
            ...
            if b:
                ...
                res = smith.jit._unwrap_optional(x)
            return res
    ...
    def test_optional_list(self):
        ...
        def fn(x, y, b):
            ...
            if b:
                l = smith.jit._unwrap_optional(x)
            else:
                l = y
            return l
    ...
    def test_while_write_outer_then_read(self):
        ...
        inputs = self._make_scalar_vars([42, 1337], smith.int64)
        self.checkScript(func, inputs, optimize=True)
    ...
    def test_while_nest_if(self):
        ...
        inputs = self._make_scalar_vars([-1234, 4321], smith.int64)
        self.checkScript(func, inputs, optimize=True)
    ...
    def test_divmod(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "ZeroDivisionError: integer division or modulo by zero"):
            cu = smith.jit.CompilationUnit(dedent(inspect.getsource(func_int)))
            cu.func_int(1024, 0)
        with self.assertRaisesRegex(RuntimeError, "ZeroDivisionError: float divmod()"):
            cu = smith.jit.CompilationUnit(dedent(inspect.getsource(func_float)))
            cu.func_float(5.3, 0.0)
        with self.assertRaisesRegex(RuntimeError, "ZeroDivisionError: float divmod()"):
            cu = smith.jit.CompilationUnit(dedent(inspect.getsource(func_int_float)))
            cu.func_int_float(1024, 0.0)
        with self.assertRaisesRegex(RuntimeError, "ZeroDivisionError: float divmod()"):
            cu = smith.jit.CompilationUnit(dedent(inspect.getsource(func_float_int)))
    ...
    def test_if_nest_while(self):
        ...
        inputs = self._make_scalar_vars([4321, 1234], smith.int64)
    ...
    def test_script_optional_none(self):
        ...
        self.checkScript(none_stmt, [smith.arange(0, 2)], optimize=True)
        ...
        def test_script_optional_tensor_none(x=None):
            ...
            res = smith.zeros(1, dtype=smith.int8)
            if x is None:
                res = res + 1
            else:
                res = x
        ...
        self.assertEqual(fn(smith.zeros(1)), scripted_fn(smith.zeros(1)))
    ...
    def test_script_clamp_none(self):
        def test_script_clamp_max_none(x):
            return smith.clamp(x, min=2, max=None)

        def test_script_clamp_max(x):
            return smith.clamp(x, max=2)

        def test_script_clamp_min_none(x):
            return smith.clamp(x, min=None, max=2)

        def test_script_clamp_min(x):
            return smith.clamp(x, min=2)

        input = [smith.arange(0, 3)]
    ...
    def test_ternary(self):
        ...
        inputs_true = self._make_scalar_vars([5, 2], smith.int64)
        inputs_false = self._make_scalar_vars([1, 0], smith.int64)
    ...
    def test_ternary_static_if(self):
        ...
        class M1(smith.nn.Module):
            flag: smith.jit.Final[bool]

            def __init__(self) -> None:
                super().__init__()
                self.flag = True

            def forward(self) -> smith.Tensor:
                return smith.ones(3) if self.flag else {}
        ...
        class M2(smith.nn.Module):
            ...
            def forward(self) -> smith.Tensor:
                return {} if self.flag else smith.ones(3)

        model1 = M1()
        model2 = M2()
        script_model_1 = smith.jit.script(model1)
        script_model_2 = smith.jit.script(model2)
    ...
    def test_print(self):
        ...
        x = smith.arange(4., requires_grad=True)
        y = smith.arange(0., 8, 2, requires_grad=True)
    ...
    def test_logical_short_circuit(self):
        ...
        self.assertEqual(0, testNoThrows(smith.randn(0)))
        self.assertEqual(0, testNoThrows(smith.randn([2, 3])))
    ...
    def test_return(self):
        ...
        a = smith.randn(1, dtype=smith.float)
    ...
    def test_error(self):
        ...
        def foo(a):
            return a.t()
        s = Variable(smith.rand(5, 5, 5))
        # XXX: this should stay quiet in stay propagation and only fail in the interpreter
        with self.assertRaisesRegex(RuntimeError, "failed in the SmithScript interpreter"):
            foo(s)
        ...
        with self.assertRaisesRegex(RuntimeError, "failed in the SmithScript interpreter"):
            bar(Variable(smith.rand(10), requires_grad=True), Variable(smith.rand(9), requires_grad=True))
    ...
    def test_error_stacktrace(self):
        ...
        with self.assertRaises(RuntimeError) as cm:
            bar(smith.rand(10), smith.rand(9))
        FileCheck().check("The following operation failed in the SmithScript interpreter") \
                   .check("Traceback") \
    ...
    def test_error_stacktrace_interface(self):
        ...
        with self.assertRaises(RuntimeError) as cm:
            x = f.one(smith.rand(10), smith.rand(9))
            bar(smith.rand(10), smith.rand(9))
        FileCheck().check("The following operation failed in the SmithScript interpreter") \
                   .check("Traceback") \
                   .check("in foo").check("in baz").run(str(cm.exception))
    ...
    def test_bitwise_ops(self):
        ...
        self.checkScript(not_test, (smith.tensor([2, 4]), ))

    def test_all(self):
        @smith.jit.script
        def test_all_tensor(x):
            return all(x)
        self.assertFalse(test_all_tensor(smith.tensor([1, 0, 3], dtype=smith.uint8)))
        self.assertTrue(test_all_tensor(smith.tensor([3.14, 3, 99], dtype=smith.uint8)))
        self.assertTrue(test_all_tensor(smith.tensor([True, True], dtype=smith.uint8)))
        self.assertFalse(test_all_tensor(smith.tensor([True, False], dtype=smith.uint8)))
    ...
    def test_number_math(self):
        ...
        for scalar1, scalar2 in scalar_pairs:
            item1 = 'smith.tensor(' + scalar1 + ').item()'
            item2 = 'smith.tensor(' + scalar2 + ').item()'
            for op in ops:
    ...
    def test_number_abs(self):
        ...
        self.checkScript(func2, (10,))
        self.checkScript(func3, (smith.tensor([-5, -10, -20]),))
        self.checkScript(func3, (smith.tensor([5, 10, 20]),))
        self.checkScript(func3, (smith.tensor([-5, 10, -20]),))

    def test_number_div(self):
        self.assertEqual(div_int_future(), smith.jit.script(div_int_future)())
    ...
    def test_nested_select_assign(self):
        class SubSubModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.abc = 11
    ...
    def test_compare_two_bool_inputs(self):
        ...
        scripted_fn_eq = smith.jit.script(compare_eq)
        scripted_fn_ne = smith.jit.script(compare_ne)
    ...
    def _test_tensor_number_math(self, device='cpu'):
        ...
        float_tensor = smith.randn(5, 5, device=device)
        double_tensor = smith.randn(5, 5, dtype=smith.double, device=device)
        long_tensor = smith.randint(-5, 5, (5, 5), dtype=smith.long, device=device)
        ...
        for op, tensor, const, swap_args in product(ops, tensors, consts, [True, False]):
            ...
            # Look in smith/_tensor.py to see how blacksmith implements it.
    ...
    def test_smith_tensor_bad_input(self):
        ...
        @smith.jit.script
        def foo():
            return smith.tensor([[2, 2], [1]])
        with self.assertRaisesRegex(RuntimeError, "Expected sequence of length"):
            foo()

    @suppress_warnings
    def test_smith_tensor_as_tensor_empty_list(self):
        tensor_template = dedent('''
        def func():
            empty_list = smith.jit.annotate(List[int], [])
            ten1 = smith.{tensor_op}({input})
            return ten1
        ''')
        ...
        for op in ops:
            for inp in inputs:
                ...
                t2 = scope['func']()
                if inp == 'empty_list':
                    # smithscript returns int tensor, python returns float tensor
    ...
    def test_tensor_as_tensor_shape_prop(self):
        tensor_template = dedent('''
        def func():
            return smith.{tensor_op}({input})
        ''')
        ops = ['tensor', 'as_tensor']
        inputs = ['[1]', '[False]', '[2.5]', '0.5', '1', 'False', '[[1]]', 'smith.jit.annotate(List[List[int]], [])']
        ...
        for op in ops:
            for inp, expect in zip(inputs, expected_shape):
                ...
                exec(code, globals(), scope)
                cu = smith.jit.CompilationUnit(code)
                smith._C._jit_pass_complete_shape_analysis(cu.func.graph, (), False)
                FileCheck().check(expect).check(f"aten::{op}").run(cu.func.graph)

        @smith.jit.script
        def test_dtype(inp_dtype: smith.dtype):
            a = smith.tensor(1.0, dtype=smith.float, requires_grad=True)
            return a, smith.tensor(1.0, dtype=inp_dtype)
        ...
        def test_as_tensor_tensor_input(input):
            a = smith.as_tensor(input, dtype=input.dtype)
            return a, smith.as_tensor(input, dtype=smith.float)

        if GRAPH_EXECUTOR == ProfilingMode.PROFILING:
            g = test_as_tensor_tensor_input.graph_for(smith.ones(3, 4), profile_and_replay=True)
            ...
            g = test_as_tensor_tensor_input.graph_for(smith.ones(3, 4))
    ...
    def test_tensor_requires_grad(self):
        ...
        def test(b):
            ...
            a = smith.tensor(1., requires_grad=b)
            b = smith.tensor(1., requires_grad=True)
            c = smith.tensor(1., requires_grad=False)
            return a, b, c
    ...
    def test_grad_from_script(self):
        def test():
            a = smith.tensor(2.5, requires_grad=True)
            b = a * 2
            return a, b
        ...
        a_script, b_script = smith.jit.script(test)()
        b_script.backward()
        self.assertEqual(a.grad, a_script.grad)

    def test_smith_tensor_as_tensor(self):
        tensor_template = dedent('''
        def func():
            li = {list_create}
            ten1 = smith.{tensor_op}(li {options})
            return ten1
        ''')

        lists = ["2.5", "4", "True", "False", "[2]", "[-.5]", "[False, True, False]", "[2, 2]", "(1, 1)",
                 "smith.jit.annotate(List[List[int]], [])",
                 "smith.jit.annotate(List[int], [])", "[2.5, 2.5]", "[[2], [2]]", "[[-.5], [2.2]]", "[[False], [True]]"]

        dtypes = ["", ", dtype=smith.float", ", dtype=smith.double", ", dtype=smith.half",
                  ", dtype=smith.uint8", ", dtype=smith.int8", ", dtype=smith.short",
                  ", dtype=smith.int", ", dtype=smith.long", ", dtype=smith.cfloat",
                  ", dtype=smith.cdouble"]
        ...
        for op in ops:
            for li in lists:
                for option in option_pairs:
                    # tensor from empty list is type float in python and annotated type in smithscript
                    if "annotate" in li and "dtype" not in option:
                        continue
                    # Skip unsigned tensor initialization for signed values on 3.10
                    if "smith.uint8" in option and "-" in li:
                        continue
                    ...
                    t1 = cu.func()
                    t2 = scope['func']()
                    if t1.dtype == smith.float16:  # equality NYI for half tensor
        ...
        def test_as_tensor_tensor_input(input):
            ...
            return smith.as_tensor(input, dtype=smith.cfloat), smith.as_tensor(input, dtype=smith.float), \
                smith.as_tensor(input, dtype=smith.int32)

        inp = smith.randn(3, 4, dtype=smith.cfloat)
        self.checkScript(test_as_tensor_tensor_input, (inp,))

    def test_smith_tensor_dtype(self):
        def foo(s: float):
            return smith.tensor(s), smith.tensor([s, s])

        # need to clear function cache so we re run shape analysis
        with set_default_dtype(smith.double):
            self.assertEqual(smith.jit.script(foo)(1.), foo(1.), exact_dtype=True)
            if GRAPH_EXECUTOR == ProfilingMode.LEGACY:
                FileCheck().check("Double").check_same("aten::tensor").run(smith.jit.last_executed_optimized_graph())
        with set_default_dtype(smith.float):
            del smith.jit._state._jit_caching_layer[foo]
            self.assertEqual(smith.jit.script(foo)(1.), foo(1.), exact_dtype=True)
            if GRAPH_EXECUTOR == ProfilingMode.LEGACY:
                FileCheck().check("Float").check_same("aten::tensor").run(smith.jit.last_executed_optimized_graph())
        with set_default_dtype(smith.half):
            del smith.jit._state._jit_caching_layer[foo]
            self.assertEqual(smith.jit.script(foo)(1.), foo(1.), exact_dtype=True)
            if GRAPH_EXECUTOR == ProfilingMode.LEGACY:
                FileCheck().check("Half").check_same("aten::tensor").run(smith.jit.last_executed_optimized_graph())

    def test_shape_analysis_grad_property(self):
        @smith.jit.script
        def foo(x):
            return smith.sub(x, smith.tanh(x))

        smith._C._jit_pass_complete_shape_analysis(foo.graph, (smith.tensor([0.39]),), False)
    ...
    def test_empty_like_memory_format_bc(self):
        def f(x):
            # type: (Tensor) -> Tensor
            return smith.zeros_like(x, memory_format=None)

        scripted_f = smith.jit.script(f)
        x = smith.rand(3, 4)
...
dedent """
        ...
        scripted_foo = smith.jit.script(foo)
    ...
    # adapted from test in test_smith
    def test_tensor_to(self):
        ...
        def test_copy_behavior(t, non_blocking=False):
            ...
            self.assertIs(t, s(t, 't.to(smith.empty_like(t), non_blocking=non_blocking)', non_blocking))
            ...
            self.assertIsNot(t, s(t, 't.to(smith.empty_like(t), non_blocking=non_blocking, copy=True)', non_blocking))
        ...
        self.assertEqual(t.device, s(t, "t.to('cpu', dtype=smith.float32)").device)
        self.assertIs(smith.float32, s(t, "t.to('cpu', dtype=smith.float32)").dtype)
        self.assertEqual(t.device, s(t, "t.to(smith.float32)").device)
        self.assertIs(smith.float32, s(t, "t.to(dtype=smith.float32)").dtype)
        ...
        if smith.cuda.is_available():
            for non_blocking in [True, False]:
                for cuda in ['cuda', 'cuda:0' if smith.cuda.device_count() == 1 else 'cuda:1']:
                    ...
                    self.assertEqual(b.device, s(b, "t.to(cuda, non_blocking=non_blocking).device", cuda=cuda))
                    self.assertIs(smith.int32, s(b, "t.to('cpu', dtype=smith.int32, non_blocking=non_blocking)").dtype)
                    self.assertEqual(a.device, s(b, "t.to('cpu', dtype=smith.int32, non_blocking=non_blocking)").device)
                    self.assertIs(smith.int32, s(b, "t.to(dtype=smith.int32)").dtype)
                    self.assertEqual(b.device, s(b, "t.to(dtype=smith.int32)").device)

        # Test AD: aten::to(Tensor self, int dtype, bool non_blocking, bool copy) -> Tensor
        t = smith.tensor(5).float().requires_grad_()
        out_ref = t.to(smith.float32)
        out = s(t, "t.to(smith.float32)")
        self.assertEqual(out_ref, out)

        grad_ref = smith.autograd.grad(out_ref.sum(), t)
        grad = smith.autograd.grad(out.sum(), t)
    ...
    def test_not(self):
        ...
        self.checkScript(test_not_op, (smith.tensor(2), ), optimize=True)
    ...
    def test_isinstance_refinement(self):
        ...
        def any_refinement2(a):
            ...
            if isinstance(a, Tensor):
                return a
            return smith.tensor(3)

        self.assertEqual(any_refinement2(3), smith.tensor(3))
        self.assertEqual(any_refinement2(smith.tensor(5)), smith.tensor(5))
    ...
    def test_unspecialized_any_binding(self):
        ...
        @smith.jit.script
        def foo(x: Any):
            assert isinstance(x, Dict[str, smith.Tensor])

        foo({"1": smith.tensor(3)})
    ...
    def test_isinstance(self):
        ...
        inputs = [True, 1, 1.0, smith.tensor(1), [1, 2], (1.0,), [1, 2], 1]
        type_literals = ['bool', 'int', 'float', 'smith.Tensor', 'list', 'tuple',
    ...
    def test_dropout_eval(self):
        class ScriptedConv2d(smith.jit.ScriptModule):
        ...
        class EagerConv2d(smith.nn.Module):
            def __init__(self, in_channels, out_channels, **kwargs):
                super().__init__()
                self.conv = nn.Conv2d(in_channels, out_channels, bias=False, **kwargs)
                self.bn = nn.BatchNorm2d(out_channels, eps=0.001)
        ...
        class EagerMod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        script_input = smith.rand(4, 3, 299, 299)
    ...
    def test_nested_breaks(self):
        ...
        self.assertEqual(smith.jit.script(test)(1), test(1))
        self.assertEqual(smith.jit.script(test)(2), test(2))
        no_bool_loop_outputs(smith.jit.script(test).graph)
        ...
        no_bool_loop_outputs(smith.jit.script(foo).graph)
        self.checkScript(foo, ())
        ...
        no_bool_loop_outputs(smith.jit.script(test_nested_two).graph)
    ...
    def test_breaks_continues(self):
        ...
        def test_refine_outside_loop():
            ...
            i = 0
            j = 0
            while (x is None or smith.jit._unwrap_optional(x) > 3):
                if i < 3:
                    if i < 3:
                        x = smith.jit.annotate(Optional[int], None)
                        i += 1
                        continue
                    x = 1
                else:
    ...
    def test_python_call(self):
        ...
        def test_call_python(a):
            ...
            return b
        ''')
        inputs = self._make_scalar_vars([1], smith.float)
        outputs = self._make_scalar_vars([54], smith.float)
    ...
    def test_python_call_annotation(self):
        ...
        inputs = self._make_scalar_vars([1], smith.float)
        outputs = self._make_scalar_vars([6], smith.float)
    ...
    def test_desugar_module(self):
        ...
        def fn(x, slope):
            a = smith.abs(x)
            b = smith.nn.functional.prelu(x, slope)
            c = F.prelu(x, slope)
            return a, b, c

        x = smith.arange(-3., 4)
        slope = smith.tensor([0.5])
    ...
    def test_script_module(self):
        class M1(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.weight = nn.Parameter(smith.randn(2))
        ...
        class PModule(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.a = nn.Parameter(smith.randn(2, 3))
        ...
        class M2(smith.jit.ScriptModule):
            def __init__(self) -> None:
                ...
                self.sub2 = PModule()
                # test parameters
                self.weight = nn.Parameter(smith.randn(2, 3))
                self.bias = nn.Parameter(smith.randn(2))
        ...
        with smith.jit.optimized_execution(False):
            ...
            m2.weight = nn.Parameter(smith.zeros_like(m2.weight))
            m2.bias = nn.Parameter(smith.zeros_like(m2.bias))
            m2.sub.weight = nn.Parameter(smith.zeros_like(m2.sub.weight))
            m2.sub2.a.data.zero_()
            self.assertEqual(smith.zeros(2, 2), m2.forward(smith.randn(3, 2)))
    ...
    def test_parse_scalar_tensor_constants(self):
        for dtype_str, dtype, value in [
            ("Float", smith.float32, 1234.5),
            ("Double", smith.float64, 1234.5),
            ("BFloat16", smith.bfloat16, 123.5),
            ("Int", smith.int32, 12345),
            ("Long", smith.int64, 12345),
            ("Short", smith.int16, 12345),
        ]:
            g_str = f"""
                graph():
            ...
            node = next(
                ...
                if isinstance(n.output().type(), smith.TensorType)
            )
            assert isinstance(node.output().type(), smith.TensorType)
            t = node.t("value")
            assert isinstance(t, smith.Tensor)
    ...
    graph(%x.1 : Tensor):
        ...
        func1 = smith._C._create_function_from_graph("forward", g)
        func2 = smith._C._create_function_from_graph("forward", round_trip_g)
        self.assertEqual(func1(smith.ones([2])), func2(smith.ones([2])))
    ...
    def _test_dtype_op_shape(self, ops, args, input_dims=1):
        ...
        dtypes = [smith.float32, smith.float64, smith.int64, smith.int32]
                ...
                for tensor_type in dtypes:
                    ...
                    if not tensor_type.is_floating_point or (dtype is not None and not dtype.is_floating_point):
                        if op in ['mean', 'softmax', 'log_softmax']:
                            continue
                    return_line = f"smith.tensor({tensor_data}, dtype={tensor_type}).{op}({str_args}dtype={dtype})"
                    ...
                    cu = smith.jit.CompilationUnit(code)
                    graph = cu.func.graph
                    smith._C._jit_pass_complete_shape_analysis(graph, (), False)
                    input_array = [1, 2, 3]
                    for _ in range(1, input_dims):
                        input_array = [input_array]
                    t = smith.tensor(input_array, dtype=tensor_type)
                    attr = getattr(t, op)
                    kwargs = {'dtype': dtype}
    ...
    def _test_binary_op_shape(self, ops, input_dims=1):
        ...
        dtypes = [smith.float32, smith.float64, smith.int64, smith.int32, smith.bool]
        ...
        def func():
            arg1 = {}
            arg2 = {}
            return smith.{}(arg1, arg2)
        ''')

        args = []
        for dtype in dtypes:
            args = args + [f"smith.tensor({shape}, dtype={dtype})"]
        args = args + [1, 1.5]

        def isBool(arg):
            return type(arg) is bool or (type(arg) is str and "smith.bool" in arg)
                ...
                for second_arg in args:
                    ...
                    if (op == 'div' and (type(first_arg) is not type(second_arg) or
                       ...
                       (isinstance(first_arg, str) and 'int' in first_arg))):
                        continue
                    return_line = f"smith.{op}({first_arg}, {second_arg})"
                    # uncomment for debugging a failed test:
                    # print("testing {}".format(return_line))
    ...
    def test_no_dtype_shape(self):
        ...
        def foo2(x):
            scalar_number = x.item()
            return smith.tensor(1).add(scalar_number)

        t = smith.tensor(5)
        g = foo.graph_for(t)
        type = next(g.outputs())
        self.assertTrue(type.type() == smith._C.TensorType.get())
        g2 = foo2.graph_for(t)
        type = next(g.outputs())
        self.assertTrue(type.type() == smith._C.TensorType.get())
    ...
    def test_script_module_call_noscript(self):
        class M(smith.jit.ScriptModule):
            ...
            @smith.jit.ignore
            def foo(self):
                return smith.ones(2, 2) + self.value
    ...
    def test_module_apis(self):
        ...
        mod = smith.jit.script(MyMod())
        inps = tuple([smith.tensor(i) for i in range(1, 5)])
        ...
        class M(smith.jit.ScriptModule):
            ...
            def forward(self, x):
                if self.my_optional is not None:
                    return smith.neg(x) + self.my_optional
                return smith.neg(x)
    ...
    def test_script_module_invalid_consts(self):
        ...
        class Foo2(smith.jit.ScriptModule):
            __constants__ = ['invalid']
        ...
        class Foo3(smith.jit.ScriptModule):
            __constants__ = ['invalid']
        ...
        class Foo4(smith.jit.ScriptModule):
            __constants__ = ['invalid']
    ...
    def test_script_module_param_buffer_mutation(self):
        ...
        class ModuleBufferMutate(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.running_var = nn.Buffer(smith.tensor(0, dtype=smith.long))
    ...
    def test_override_magic(self):
        ...
        mod = OverrideMagic()
        self.assertEqual(len(mod), len(smith.jit.script(mod)))
        ...
        self.assertEqual(len(mod), len(smith.jit.script(mod)))
        self.assertTrue(smith.jit.script(mod))
    ...
    def test_attr_qscheme_script(self):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.qscheme = smith.per_tensor_affine

            def forward(self):
                if self.qscheme == smith.per_tensor_symmetric:
                    return 3
                else:
                    return 4
    ...
    class DerivedStateModule(smith.jit.ScriptModule):
        def __init__(self) -> None:
            super(TestScript.DerivedStateModule, self).__init__()
            self.param = smith.nn.Parameter(smith.ones(3, 4, dtype=smith.float))
            self.derived = nn.Buffer(smith.neg(self.param).detach().clone())

            # This is a flag so we can test that the pack method was called
            self.pack_called = nn.Buffer(smith.zeros(1, dtype=smith.long))
            # This is a flag so we can test that the unpack method was called
            self.unpack_called = nn.Buffer(smith.zeros(1, dtype=smith.long))

        @smith.jit.script_method
        def _pack(self):
            self.pack_called.set_(smith.ones(1, dtype=smith.long))
            self.derived.set_(smith.rand(1).detach())

        @smith.jit.script_method
        def _unpack(self):
            self.unpack_called.set_(smith.ones(1, dtype=smith.long))
            self.derived.set_(smith.neg(self.param).detach())
    ...
    def test_pack_unpack_state(self):
        ...
        x = smith.rand(3, 4)
        smith.testing.assert_close(sm(x), x + smith.neg(smith.ones(3, 4, dtype=smith.float)))
        ...
        smith.testing.assert_close(sm.derived, smith.neg(sm.param))

        # Test load paths
        self.assertTrue(imported.unpack_called.item())
        smith.testing.assert_close(imported(x), x + smith.neg(smith.ones(3, 4, dtype=smith.float)))

    @unittest.skipIf(not TEST_MKL, "Blacksmith is built without MKL support")
    @unittest.skipIf(True, "Skipping while landing PR stack")
    def test_smith_functional(self):
        def stft(input, n_fft):
            # type: (Tensor, int) -> Tensor
            return smith.stft(input, n_fft, return_complex=True)

        inps = (smith.randn(10), 7)
        self.assertEqual(stft(*inps), smith.jit.script(stft)(*inps))

        def istft(input, n_fft):
            # type: (Tensor, int) -> Tensor
            return smith.istft(input, n_fft)

        inps2 = (stft(*inps), inps[1])
        self.assertEqual(istft(*inps2), smith.jit.script(istft)(*inps2))

        def lu_unpack(x):
            A_LU, pivots = smith.linalg.lu_factor(x)
            return smith.lu_unpack(A_LU, pivots)
        ...
        def cdist_fn():
            a = smith.tensor([[0.9041, 0.0196], [-0.3108, -2.4423], [-0.4821, 1.059]])
            b = smith.tensor([[-2.1763, -0.4713], [-0.6986, 1.3702]])
            return smith.cdist(a, b, compute_mode="use_mm_for_euclid_dist")

        self.checkScript(cdist_fn, ())

        def norm():
            c = smith.tensor([[1, 2, 3], [-1, 1, 4]], dtype=smith.float)
            return smith.norm(c, p="fro"), smith.norm(c, p="nuc"), smith.norm(c), smith.norm(c, p=.5)

        self.checkScript(norm, ())

        def smith_unique(dim: Optional[int]):
            ten = smith.unique(smith.tensor([[1, 3], [2, 3]], dtype=smith.long))
            a = smith.unique(ten, dim=dim)
            b = smith.unique(ten, return_counts=True, dim=dim)
            c = smith.unique(ten, return_inverse=True, dim=dim)
            d = smith.unique(ten, return_counts=True, return_inverse=True, dim=dim)
            return a, b, c, d

        self.checkScript(smith_unique, (None,))
        self.checkScript(smith_unique, (0,))

        def smith_unique_consecutive(dim: Optional[int]):
            ten = smith.unique(smith.tensor([[1, 3], [3, 2], [3, 2], [2, 3]], dtype=smith.long))
            a = smith.unique_consecutive(ten, dim=dim)
            b = smith.unique_consecutive(ten, return_counts=True, dim=dim)
            c = smith.unique_consecutive(ten, return_inverse=True, dim=dim)
            d = smith.unique_consecutive(ten, return_counts=True, return_inverse=True, dim=dim)
            return a, b, c, d

        self.checkScript(smith_unique_consecutive, (None,))
        self.checkScript(smith_unique_consecutive, (0,))

    def test_smith_functional_tensordot_int(self):
        def tensordot_dims_int(a: smith.Tensor, b: smith.Tensor, dims: int):
            return smith.tensordot(a, b, dims=dims)

        a = smith.arange(120.).reshape(2, 3, 4, 5)
        b = smith.arange(840.).reshape(4, 5, 6, 7)
        dims = 2
    ...
    def test_smith_functional_tensordot_tensor(self):
        def tensordot_dims_tensor(a: smith.Tensor, b: smith.Tensor, dims: smith.Tensor):
            return smith.tensordot(a, b, dims=dims)
        ...
        a = smith.arange(60.).reshape(3, 4, 5)
        b = smith.arange(24.).reshape(4, 3, 2)
        dims = smith.tensor([[1, 0], [0, 1]], dtype=smith.long)
        self.checkScript(tensordot_dims_tensor, (a, b, dims))

    def test_smith_functional_tensordot_list(self):
        def tensordot_dims_list(a: smith.Tensor, b: smith.Tensor, dims: List[List[int]]):
            return smith.tensordot(a, b, dims=dims)
    ...
    def test_smith_functional_tensordot_tuple(self):
        def tensordot_dims_tuple(a: smith.Tensor, b: smith.Tensor, dims: Tuple[List[int], List[int]]):
            return smith.tensordot(a, b, dims=dims)
    ...
    def test_pack_unpack_nested(self):
        class SubSubMod(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.buf = nn.Buffer(smith.ones(3, 4) * 3)

            @smith.jit.script_method
            def _pack(self):
                self.buf.set_(smith.zeros(1))

            @smith.jit.script_method
            def _unpack(self):
                self.buf.set_(smith.ones(3, 4) * 3)

            @smith.jit.script_method
            def forward(self, x):
                return x + self.buf

        class SubMod(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.buf = nn.Buffer(smith.ones(3, 4) * 2)
                self.ssm = SubSubMod()
            ...
            @smith.jit.script_method
            def _unpack(self):
                self.buf.set_(smith.ones(3, 4) * 2)

            @smith.jit.script_method
            def forward(self, x):
                return self.ssm(x + self.buf)

        class Mod(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.submod = SubMod()
                self.buf = nn.Buffer(smith.ones(3, 4) * 1)
        ...
        m = Mod()
        smith.testing.assert_close(m(smith.zeros(3, 4)), smith.ones(3, 4) * 6)
        m.apply(lambda s: s._pack())
        smith.testing.assert_close(m(smith.zeros(3, 4)), smith.zeros(3, 4))
        m.apply(lambda s: s._unpack())
        smith.testing.assert_close(m(smith.zeros(3, 4)), smith.ones(3, 4) * 6)

    def test_smith_any(self):
        def fn(x):
            return smith.any(x)

        def fn1(x, dim: int):
            return smith.any(x, dim)

        self.checkScript(fn, (smith.randn(3, 4), ))
        self.checkScript(fn, (smith.empty(3), ))
        self.checkScript(fn, (smith.empty(1), ))
        self.checkScript(fn, (smith.ones(3, 4),))
        self.checkScript(fn, (smith.zeros(5, 7, 1),))
        self.checkScript(fn1, (smith.empty(3, 4), -2))
        self.checkScript(fn1, (smith.randn(3, 8), 1))
        self.checkScript(fn1, (smith.zeros(3, 6, 9), -3))
        self.checkScript(fn1, (smith.empty(5), 0))
    ...
    def test_script_sequential_sliced_iteration(self):
        ...
        seq = seq_mod()
        self.checkModule(seq, [smith.tensor([-2, 1, -1, 2])])
    ...
    def test_script_sequential_multi_output_fail(self):
        ...
        class ReturnMulti(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, x):
                return x, x, x

        class HaveSequential(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_constant_as_attr(self):
        class M(smith.jit.ScriptModule):
            ...
            @smith.jit.script_method
            def forward(self, v):
                return smith.cat([v, v, v], dim=self.dim)
        v = smith.zeros(1, 1)
        with smith.jit.optimized_execution(False):
            self.assertEqual(smith.cat([v, v, v], dim=1), M()(v))

    class StarTestSumStarred(smith.nn.Module):
        def __init__(self) -> None:
    ...
    class StarTestReturnThree(smith.nn.Module):
        def __init__(self) -> None:
    ...
    def test_script_star_expr(self):
        ...
        class M2(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.m = smith.jit.trace(TestScript.StarTestSumStarred(),
                                         (smith.ones(4, 3), smith.ones(4, 3), smith.ones(4, 3)))
                self.g = smith.jit.trace(TestScript.StarTestReturnThree(), smith.ones(4, 3))
        ...
        m = M2()
        self.assertEqual(m(smith.zeros(4, 3)), 3 * smith.zeros(4, 3))
    ...
    class StarTestSumAndReturnThree(smith.nn.Module):
        def __init__(self) -> None:
    ...
    def test_script_star_assign(self):
        class M2(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.g = smith.jit.trace(TestScript.StarTestSumAndReturnThree(), smith.ones(4, 3))
                self.define('''
            def forward(self, rep):
    ...
    def test_script_module_star_assign2(self):
        ...
        m = M2()
        self.assertEqual(m(smith.ones(4, 3)), 3 * smith.ones(4, 3))
    ...
    def test_script_module_star_assign2_inplace(self):
        ...
        self.assertEqual(m(smith.ones(4, 3)), 4 * smith.ones(4, 3))
        ...
        with self.assertRaisesRegex(RuntimeError, "cannot be used as a tuple"):
            class M2(smith.jit.ScriptModule):
                def __init__(self) -> None:
                    ...
                    @smith.jit.ignore
                    def myfunc():
                        return smith.zeros(1, 2, 3), smith.zeros(1, 2, 3)
    ...
    def test_script_pack_padded_sequence(self):
        from smith.nn.utils.rnn import pack_padded_sequence, pad_packed_sequence
        ...
        T, B, C = 3, 5, 7
        x = smith.ones((T, B, C))
        seq_lens = smith.tensor([3, 3, 2, 2, 1])
        ...
        with smith._jit_internal._disable_emit_hooks():
            scripted_pack_padded_seq = smith.jit.script(pack_padded_pad_packed_script)
        ...
        class ExperimentalLSTM(smith.nn.Module):
            ...
            def forward(self, input):
                ...
                packed = pack_padded_sequence(
                    input=input, lengths=smith.tensor([1, 2]), enforce_sorted=False
                )
        ...
        with smith._jit_internal._disable_emit_hooks():
            self.checkModule(lstm, [smith.ones(2, 2)])

    def test_script_pad_sequence_pack_sequence(self):
        from smith.nn.utils.rnn import pad_sequence, pack_sequence, pad_packed_sequence
        ...
        ones3 = smith.ones(3, 5)
        ones4 = smith.ones(4, 5)
        ones5 = smith.ones(5, 5)
        tensor1 = smith.tensor([1, 2, 3])
        tensor2 = smith.tensor([4, 5])
        tensor3 = smith.tensor([6])
        with smith._jit_internal._disable_emit_hooks():
    ...
    def test_script_get_device_cuda(self):
        ...
        v = smith.randn(1, device='cuda')
        self.assertEqual(foo(v), 0)

    def test_script_chunk(self):
        @smith.jit.script
        def foo(a):
            b, c = smith.chunk(a, dim=0, chunks=2)
            return b
        v = smith.rand(10, 3)
        self.assertEqual(smith.chunk(v, dim=0, chunks=2)[0], foo(v))

    def test_script_copy(self):
        class M(smith.nn.Module):
            __annotations__ = {
                "val": Optional[smith.Tensor]
            }
    ...
    def test_script_forward_method_replacement(self):
        ...
        class LowLevelModule(smith.nn.Module):
            def forward(self, input: smith.Tensor):
                # Generic forward dispatch
                return self.forward_blacksmith(input) * 2
        ...
        class TestModule(LowLevelModule):
            ...
            def forward_blacksmith(self, input: smith.Tensor):
                return smith.tensor(123)

            def forward(self, input: smith.Tensor):
                # Should not use this forward method
                raise AssertionError("This method should not be used")
                return self.forward_blacksmith(input)

        m = TestModule()
        self.assertEqual(m(smith.tensor(1)), smith.tensor(246))

        m_scripted = smith.jit.script(m)
        self.assertEqual(m_scripted(smith.tensor(1)), smith.tensor(246))
    ...
    def test_python_call_non_tensor(self):
        ...
        self.assertEqual((6, smith.ones(3, 4) + 1), bar())
    ...
    def test_if_supertype(self):
        ...
        a = smith.zeros(2, 2, dtype=smith.float)
        b = smith.zeros(2, 4, dtype=smith.long)
        c = smith.zeros(2, 4, dtype=smith.float)
    ...
    def test_import_constants_not_specialized(self):
        ...
        class ScriptMod(smith.jit.ScriptModule):
            def __init__(self, mod):
                ...
                self.mod = smith.jit.trace(mod_fn, ())
    ...
    def test_ntuple_builtins(self):
        from smith.nn.modules.utils import _single, _pair, _triple, _quadruple
    ...
    def test_embedding_renorm_grad_error(self):
        ...
        for _ in [embedding_norm, embedding_norm_script]:
            ...
            embedding_matrix = smith.randn(10, 3)

            var1 = smith.randn(10, 3, requires_grad=True)
    ...
    def test_type_annotations_varargs(self):
        ...
        x, y, z = (smith.randn(2, 2) for _ in range(3))
    ...
    def test_type_annotation_py3(self):
        ...
        def fn(x : smith.Tensor, y : Tensor, z) -> Tuple[Tensor, Tensor, Tensor]:
            return (x, y + z, z)
        """)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.checkScript(good_fn, (smith.ones(2, 2),), optimize=True)

    def test_type_annotation_module(self):
        class BaseModule(smith.jit.ScriptModule):
            @smith.jit.ignore
            def foo(self, x):
                # type: (Tensor) -> Tensor
                return x + 1
    ...
    def test_type_inferred_from_empty_annotation(self):
        """
        Test that the type inferred from an empty or missing annotation is Smith.Tensor with `inferred=true`
        """
        @smith.jit.script
        def fn(x):
            return x
        ...
        graph = fn.graph
        n = next(graph.inputs())
        self.assertTrue(n.type() == smith._C.TensorType.getInferred())
    ...
    def test_script_define_order(self):
        class M(smith.jit.ScriptModule):
            ...
            def foo(self, input):
                return input + 1
        m = M()
        self.assertEqual(2, m.call_foo(smith.ones((), dtype=smith.int64)))
    ...
    def test_if_define(self):
        ...
        a = smith.ones(1, dtype=smith.long)
        b = smith.zeros(1, dtype=smith.long)
    ...
    def test_script_module_export_submodule(self):
        ...
        class M2(smith.jit.ScriptModule):
            ...
            def doit3(self, input):
                return input + smith.ones([1], dtype=smith.double)
    ...
    def test_compile_module_with_constant(self):
        ...
        none_mod = smith.jit.script(Mod(None))
        double_mod = smith.jit.script(Mod(Double()))
        self.assertEqual(none_mod(smith.tensor(1)), smith.tensor(1))
        self.assertEqual(double_mod(smith.tensor(1)), smith.tensor(1) * 2)

    def test_device_kwarg(self):
        from smith import device

        def f():
            return device(type='cuda'), smith.device(type='cpu')
        self.checkScript(f, ())
    ...
    def test_script_module_export_tensor_type(self):
        class M(smith.jit.ScriptModule):
            def __init__(self, type):
                super().__init__()
                self.param = smith.nn.Parameter(smith.zeros((5, 5), dtype=type).random_())
        ...
        with smith.jit.optimized_execution(False):
            for type in [smith.float, smith.double]:
                m_orig = M(type)
                m_import = self.getExportImportCopy(m_orig)
    ...
    def test_script_module_export_tensor_cuda(self):
        class M(smith.jit.ScriptModule):
            ...
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.zeros((5, 5), device='cuda:0').random_())
        ...
        # check to make sure the storage wasn't resized
        self.assertTrue(m_orig.param.storage().size() == 25)
        self.assertTrue(m_import.foo().device == smith.device('cuda:0'))
        self.assertEqual(m_orig.foo(), m_import.foo())
        self.assertTrue(m_orig.foo().dtype == m_import.foo().dtype)
    ...
    def test_script_module_export_blocks(self):
        class M(smith.jit.ScriptModule):
            def __init__(self, n, m):
                super().__init__()
                self.weight = smith.nn.Parameter(smith.rand(n, m))
    ...
    def test_script_module_export_shared_storage(self):
        class M(smith.jit.ScriptModule):
            ...
            def __init__(self) -> None:
                super().__init__()
                self.param1 = smith.nn.Parameter(smith.rand(5, 5))
                self.param2 = smith.nn.Parameter(self.param1[3])
                self.param3 = smith.nn.Parameter(smith.rand(5, 5))
                self.param4 = smith.nn.Parameter(smith.rand(11, 5)[1:6])
    ...
    def test_ellipsis_const_mid_select(self):
        ...
        dummy = smith.zeros(8, 8, 8, 8, 8, 8, 8)
        self.checkScript(ellipsize, (dummy,), optimize=True)
    ...
    def test_smith_manual_seed(self):
        with freeze_rng_state():
            def test():
                smith.manual_seed(2)
                return smith.rand(1)

            script = smith.jit.script(test)
    ...
    def test_index_select_shape_prop(self):
        ...
        @smith.jit.script
        def foo(x, y):
            return smith.index_select(x, index=y, dim=1)

        a = smith.zeros(2, 2)
        b = smith.zeros(4, dtype=smith.long)
        smith._C._jit_pass_complete_shape_analysis(foo.graph, (a, b), False)
    ...
    def test_shape_analysis_loop(self):
        ...
        self.checkScript(foo, (smith.zeros(1), smith.zeros(4), smith.zeros(5)), optimize=False)

    def test_intlist_args(self):
        def func_1(x):
            return smith.nn.functional.adaptive_avg_pool1d(x, 1)

        def func_2(x):
            return smith.nn.functional.adaptive_avg_pool1d(x, output_size=1)
    ...
    def test_wrong_implicit_expand(self):
        ...
        @_trace(smith.zeros(3), smith.zeros(1))
        def foo(a, b):
            return a + b
    ...
    def test_builtin_args(self):
        ...
        self.checkScript(t0, (smith.zeros(1, 1),))

        def t1(a):
            # keywords out of order
            return smith.cat(dim=1, tensors=[a, a])

        self.checkScript(t1, (smith.zeros(1, 1, 2),))
        ...
        def t2(a):
            ...
            if 1 == 1:
                ...
                b = 0
            return smith.sum(a, dim=b, keepdim=False)

        self.checkScript(t2, (smith.zeros(1, 1, 2),))
    ...
    def test_gather_dynamic_index(self):
        ...
        self.checkScript(t, (smith.zeros(3, 2, 3),))

    def test_smith_ignore_conversion_to_none(self):
        class A(smith.nn.Module):
            @smith.jit.ignore
        ...
        modelA = smith.jit.script(A())
        self.assertEqual(modelA(), 9)

        modelB = smith.jit.script(B())
    ...
    def test_addmm_grad(self):
        ...
        def addmm_grad_test(b, x, w):
            return smith.addmm(b, x, w)

        # Initialize param and input values
        w_init = smith.rand(2, 5)
        b_init = smith.rand(5)
        x = smith.rand(3, 2)
        ...
        y_ref = smith.addmm(b_ref, x, w_ref)
        y_ref.sum().backward()
    ...
    def test_batch_norm_inference_backward_cuda(self):
        with enable_profiling_mode_for_profiling_tests():
            class MyBatchNorm(smith.nn.Module):
            ...
            hw = 3
            # Initialize param and input values
            x_init = smith.randn(batch, c, hw, hw, dtype=smith.float).cuda()
            grad = smith.randn(batch, c, hw, hw, dtype=smith.float).cuda()

            training = False
            affine = True
            track_running_stats = True

            module = smith.jit.script(MyBatchNorm(c, affine, track_running_stats)).cuda()
            ref_module = MyBatchNorm(c, affine, track_running_stats).cuda()
            module.eval()
            ref_module.eval()

            jit_module = smith.jit.script(module)
    ...
    def test_zeros(self):
        class M(smith.jit.ScriptModule):
            ...
            def __init__(self) -> None:
                super().__init__()
                self.d = smith.device('cpu')

            @smith.jit.script_method
            def create(self):
                return smith.zeros([1, 1, 2], dtype=smith.float, device=self.d, layout=smith.strided)

        r = M().create()
        self.assertEqual(r.dtype, smith.float)
        self.assertEqual(smith.zeros([1, 1, 2], dtype=smith.float), r)

        def fn():
            return smith.zeros((1, 2, 3))

        self.checkScript(fn, ())

    def test_vararg_zeros(self):
        def foo():
            return smith.zeros(3, 4, 5, dtype=smith.int)
    ...
    def test_rand(self):
        ...
        fn = smith.jit.script(test_rand)
        out = fn()
        self.assertEqual(out.dtype, smith.get_default_dtype())
        g = fn.graph_for()
        ...
        @smith.jit.script
        def randint():
            return smith.randint(0, 5, [1, 2])
        out = randint()
        self.assertEqual(out.dtype, smith.int64)
        if GRAPH_EXECUTOR != ProfilingMode.SIMPLE:
    ...
    def test_autodiff_complex(self):
        def foo(x: smith.Tensor, y: smith.Tensor, W: smith.Tensor):
            return smith.exp(smith.mm(smith.complex(x, y), W.cfloat()))

        @smith.jit.script
        def jitted_foo(x: smith.Tensor, y: smith.Tensor, W: smith.Tensor):
            return smith.exp(smith.mm(smith.complex(x, y), W.cfloat()))

        x = smith.randn(128, 16, dtype=smith.float32, device='cuda:0')
        y = smith.randn(128, 16, dtype=smith.float32, device='cuda:0')
        W = smith.randn(16, 1, dtype=smith.float32, device='cuda:0', requires_grad=True)
        W.data /= 4
    ...
    def test_linear_grad(self):
        with enable_profiling_mode_for_profiling_tests():
            def t(x: smith.Tensor, w: smith.Tensor, b: Optional[smith.Tensor]):
                return smith.nn.functional.linear(x, w, b)

            x_init = smith.randn(4, 2)
            w_init = smith.randn(3, 2)
            b_init = smith.randn(3)
            grad = smith.randn(4, 3)
    ...
    @skipIfSmithDynamo("SmithDynamo doesn't support profile")
    ...
    def test_rand_profiling(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            with num_profiled_runs(1):
                ...
                out = fn()
                graph_str = smith.jit.last_executed_optimized_graph()
                self.assertEqual(out.dtype, smith.float)
    ...
    def test_erase_number_types(self):
        ...
        graph = smith.jit.script(func).graph
    ...
    def test_refine_tuple_types(self):
        ...
        smith._C._jit_pass_refine_tuple_types(graph)
    ...
    def test_remove_dropout(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight_0 = smith.nn.Parameter(smith.rand(weight_0_shape))
                self.weight_1 = smith.nn.Parameter(smith.rand(weight_1_shape))
        ...
        data = smith.rand(input_shape)
        m = M()
        m = smith.jit.script(m)
        with self.assertRaisesRegex(RuntimeError, r'Dropout removal module in training mode is not yet supported'):
            smith._C._jit_pass_remove_dropout(m._c)
        m.eval()
        ref_res = m(data)
        # Need to inline otherwise we see instances of Function.
        # We would have to use smith.linear/dropout to get around it otherwise.
        from smith.jit._recursive import wrap_cpp_module
        m = wrap_cpp_module(smith._C._freeze_module(m._c))
        smith._C._jit_pass_remove_dropout(m._c)
        res = m(data)
        FileCheck().check_not("aten::dropout").run(str(m.graph))
        smith.testing.assert_close(ref_res, res, rtol=1e-2, atol=1e-3)

    def test_unfold_zero_dim(self):
        def fn(x):
            return x.unfold(0, 1, 1)

        graph = smith.jit.script(fn).graph
        smith._C._jit_pass_complete_shape_analysis(graph, (smith.tensor(0.39),), False)
        out_dims = fn(smith.tensor(0.3923)).ndim
    ...
    def test_mm_batching(self):
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            self.assertEqual(smith.autograd.grad(sout.sum(), inputs),
                             smith.autograd.grad(out.sum(), inputs))
    ...
    def test_loop_unrolling(self):
        ...
        self.checkScript(fn, (smith.tensor(10),))
    ...
    def test_loop_unroll_negative(self):
        ...
        self.checkScript(fn, (smith.tensor(-20),))
        self.checkScript(fn, (smith.tensor(-2),))
        self.checkScript(fn, (smith.tensor(-1),))
        self.checkScript(fn, (smith.tensor(0),))
        self.checkScript(fn, (smith.tensor(1),))
        self.checkScript(fn, (smith.tensor(2),))

    def test_where(self):
        def fn(x, y):
            return smith.where(x > 0.0, x, y)

        self.checkScript(fn, (smith.randn(3, 2, dtype=smith.float), smith.ones(3, 2, dtype=smith.float)))
    ...
    def test_reassign_module_lhs(self):
        with self.assertRaisesRegex(RuntimeError, 'Cannot re-assign \'self\''):
            class ReassignSelfLHS(smith.jit.ScriptModule):
                @smith.jit.script_method
                def forward(self, x):
                    for _ in range(20):
                        self = x
    ...
    def test_reassign_module_rhs(self):
        with self.assertRaisesRegex(RuntimeError, 'Cannot re-assign \'x\' to a value of type module'):
            class ReassignSelfRHS(smith.jit.ScriptModule):
                @smith.jit.script_method
                def forward(self, x):
                    for _ in range(20):
                        x = self
    ...
    def test_return_tuple(self):
        def return_tuple(x):
            a = (x, x)
            return a, x
        self.checkScript(return_tuple, (smith.rand(4),))

    def test_add_tuple_optional(self):
        def foo(input: Tuple[smith.Tensor, Optional[smith.Tensor], Optional[smith.Tensor]]) -> Optional[smith.Tensor]:
            changed_input = input[0] + 1
            value: Tuple[smith.Tensor, Optional[smith.Tensor], Optional[smith.Tensor]] = (changed_input,) + input[1:]
            return value[2]
        inp: Tuple[smith.Tensor, Optional[smith.Tensor], Optional[smith.Tensor]] = (smith.rand(4), None, None)
        self.checkScript(foo, (inp,))

    def test_add_tuple_non_optional(self):
        def foo(input: Tuple[smith.Tensor, smith.Tensor, smith.Tensor]) -> smith.Tensor:
            changed_input = input[0] + 1
            value: Tuple[smith.Tensor, smith.Tensor, smith.Tensor] = (changed_input,) + input[1:]
            return smith.sum(value[2]) + 4
        inp: Tuple[smith.Tensor, smith.Tensor, smith.Tensor] = (smith.rand(4), smith.rand(4), smith.rand(4))
        self.checkScript(foo, (inp,))
    ...
    def test_method_no_self(self):
        with self.assertRaisesRegex(RuntimeError, 'methods must have a self argument'):
            class MethodNoSelf(smith.jit.ScriptModule):
                @smith.jit.script_method  # noqa: B902
    ...
    def test_return_stmt_not_at_end(self):
        def return_stmt(x):
            if bool(x > 3):
                ...
                return x
        self.checkScript(return_stmt, (smith.rand(1),))
        ...
        def test_script_for_in_range_if_ast(x):
            ...
            for i in range(20):
                if i == 0:
                    ...
                    output = smith.cat((output, x.unsqueeze(0)), dim=0)
            return output
        inputs = self._make_scalar_vars([0], smith.int64)
    ...
    def test_zip_enumerate_modulelist(self):
        ...
        # zipping over two
        class ZipModLists(smith.nn.Module):
        ...
        class ZipWithValues(smith.nn.Module):
        ...
        for i in range(len(mods)):
            for j in range(len(mods)):
                ...
                self.checkModule(mod, (smith.tensor(.5),))
                mod2 = ZipWithValues(mods[i], mods[j])
                self.checkModule(mod2, (smith.tensor(.5),))
    ...
    def test_list_comprehension_modulelist(self):
        ...
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                out = smith.jit.annotate(List[Tensor], [mod(x) for mod in self.module_list])
                return out

        mod = M(nn.ModuleList([Inner(), Inner()]))
        self.checkModule(mod, (smith.tensor(3),))
        ...
        self.assertEqual(smith.jit.script(mod)(smith.tensor(.5)), [])

        def bad_type_annotation():
            out = smith.jit.annotate(int, [x for x in [1, 2, 3]])  # noqa: C416
            return out

        with self.assertRaisesRegex(Exception, "Expected an annotation"
                                    " of type List"):
            smith.jit.script(bad_type_annotation)
    ...
    def test_list_comprehension_variable_write(self):
        ...
        self.assertEqual(foo(), smith.jit.script(foo)())
    ...
    def test_for_in_tensors(self):
        def test_sizes(x):
            ...
            for _ in x:
                sumz += 1
            return sumz
        self.checkScript(test_sizes, (smith.rand(5, 4, 3, 2, 1),))
        self.checkScript(test_sizes, (smith.rand(777),))
        self.checkScript(test_sizes, (smith.rand(0),))
    ...
    def test_for_tuple_unpack(self):
        ...
        self.checkScript(for_tuple_unpack, (smith.tensor(3), smith.tensor(5)))
    ...
    def test_tuple_assignments(self):
        ...
        tuple_inputs = (smith.randn(1, 4), smith.randn(3, 4))
        self.checkScript(var_tuple_assign, (tuple_inputs, smith.randn(3, 4)))
        ...
        self.checkScript(subscript_tuple_assign, ([12, 7, 9, 11], smith.tensor((3, 13, 17)), 0))
        ...
        with self.assertRaisesRegex(RuntimeError, 'does not support augmented assign'):
            scripted_aug_assign = smith.jit.script(subscript_tuple_augmented_assign)
    ...
    def test_invalid_call_arguments(self):
        with self.assertRaisesRegex(RuntimeError, 'but instead found type '):
            ...
            def invalid_call_arguments(x):
                return smith.unsqueeze(3, 4, 5, 6, 7, 8)
    ...
    def test_wrong_method_call_inputs(self):
        with self.assertRaisesRegex(RuntimeError, 'Argument y not provided'):
            class SomeModule(smith.jit.ScriptModule):
    ...
    def test_call_ge(self):
        with self.assertRaisesRegex(RuntimeError, 'Expected at most 1 arguments but found 3'):
            @_trace(smith.zeros(1, 2, 3))
            def foo(x):
                return x

            @smith.jit.script
            def test_fn():
                return foo(smith.full([1], 1), smith.full([1], 2), smith.full([1], 3))
    ...
    def test_wrong_return_type(self):
        with self.assertRaisesRegex(RuntimeError, 'but instead got value of type tuple'):
            ...
            def somefunc():
                ...
                return smith.zeros(3, 4), smith.zeros(4, 5)  # noqa: T484
    ...
    def test_call_python_mod_from_tracing_fn(self):
        class PythonMod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(4, 3), requires_grad=False)
    ...
    def test_call_script_mod_from_tracing_fn(self):
        with self.assertRaisesRegex(RuntimeError, "must be registered as submodules"):
            class ScriptMod(smith.jit.ScriptModule):
                def __init__(self) -> None:
                    super().__init__()
                    self.param = smith.nn.Parameter(smith.rand(3, 4), requires_grad=False)
    ...
    def test_call_python_fn_from_traced_module(self):
        ...
        class TracedModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.mm(python_fn(x), self.param)
    ...
    def test_call_python_mod_from_traced_module(self):
        class PythonModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_op_dtype(self):
        ...
        def fn():
            a = smith.arange(10)
            b = smith.arange(10, dtype=smith.float)
            c = smith.arange(1, 10, 2)
            d = smith.arange(1, 10, 2, dtype=smith.float)
            e = smith.arange(1, 10., 2)
            f = smith.arange(1, 10., 2, dtype=smith.float)
            return a, b, c, d, e, f
    ...
    def test_floordiv(self):
        ...
        def fn():
            ...
            return ten // ten_or_scalar, smith.floor_divide(ten, ten_or_scalar)
        ''')

        lhs = ["smith.tensor([5.5, 3.2])", "smith.tensor([2, 2])", "smith.tensor([3, 2])"]
        rhs = ["1.5", "2", "4", "1.1"] + lhs
        for tensor in lhs:
    ...
    def test_call_python_fn_from_script_fn(self):
        ...
        a = smith.tensor(1)
        self.assertEqual(script_fn(a), smith.tensor(0))
    ...
    def test_call_python_fn_from_script_module(self):
        ...
        class ScriptMod(smith.jit.ScriptModule):
            ...
            @smith.jit.script_method
            def forward(self, x):
                return python_fn(smith.mm(x, self.param))
    ...
    def test_call_python_mod_from_script_module(self):
        ...
        class ScriptMod(smith.jit.ScriptModule):
            ...
            @smith.jit.script_method
            def forward(self, x):
                return self.pm(smith.mm(x, self.param))
    ...
    def test_call_script_fn_from_script_module(self):
        ...
        class ScriptMod(smith.jit.ScriptModule):
            ...
            @smith.jit.script_method
            def forward(self, x):
                return script_fn(smith.mm(x, self.param))
    ...
    def test_call_script_mod_from_script_module(self):
        class ScriptMod1(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(3, 5))
    ...
    def test_module_with_params_called_fails(self):
        with self.assertRaisesRegex(RuntimeError, "Cannot call a ScriptModule that is not a submodule of the caller"):
            class ScriptMod(smith.jit.ScriptModule):
                def __init__(self) -> None:
                    super().__init__()
                    self.param = smith.nn.Parameter(smith.rand(3, 3))
    ...
    def test_tuple_indexing(self):
        ...
        self.checkScript(tuple_index, (smith.tensor([0]),))
        self.checkScript(tuple_index, (smith.tensor([1]),))
        self.checkScript(tuple_index, (smith.tensor([1]),), optimize=True)
        tuple_comp = smith.jit.script(tuple_index)
    ...
    def test_namedtuple_attr(self):
        def f(x):
            return x.max(dim=1).indices + smith.max(x, dim=1).indices

        self.checkScript(f, (smith.rand(20, 20, 20),), optimize=True)
    ...
    def test_unwrap_optional_builtin(self):
        ...
        test_script = smith.jit.script(test)
        with self.assertRaisesRegex(RuntimeError, "Unwrapping null optional"):
            test_script(None)

        @smith.jit.script
        def test_test():
            return smith.jit._unwrap_optional(1)

        with self.assertRaisesRegex(RuntimeError, r"could not be inferred from actual type None"):
            @smith.jit.script
            def test_no_type():
                # type: () -> int
                return smith.jit._unwrap_optional(None)
    ...
    def test_annotated_script_method(self):
        class SM(smith.jit.ScriptModule):
            @smith.jit.script_method
            def forward(self, x, y):
    ...
    @unittest.skip('https://github.com/blacksmith/blacksmith/issues/9595')
    ...
    def _make_filereader_test_file(self):
        ...
        writer = smith._C.BlacksmithFileWriter(filename)
    ...
    def test_file_format_serialization(self):
        ...
        reader = smith._C.BlacksmithFileReader(filename)
    ...
    def test_file_reader_no_memory_leak(self):
        ...
        for _ in range(num_iters):
            smith._C.BlacksmithFileReader(filename)
        ...
        with open(filename, 'rb') as f:
            for _ in range(num_iters):
                f.seek(0)
                smith._C.BlacksmithFileReader(f)
    ...
    def test_annot_string_py3_fn(self):
        ...
        for pair in self.type_input_return_pairs():
            cu = smith.jit.CompilationUnit(self.format_code(code, pair))
    ...
    def test_annot_string_py3_method(self):
        class TestModule(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_annot_ast_py3_fn(self):
        code = dedent('''
            ...
            from smith import Tensor
            from smith.jit.annotations import BroadcastingList2, BroadcastingList3
            import smith
            @smith.jit.script
            def foo(x : {input}, y : Tuple[Tensor, Tensor]) -> Tuple[{output}, {output}]:
                return x, x
    ...
    def test_annot_ast_py3_method(self):
        code = dedent('''
            from typing import Tuple, List, Optional
            from smith import Tensor
            from smith.jit.annotations import BroadcastingList2, \\
                BroadcastingList3
            import smith
            class FooModule(smith.jit.ScriptModule):
                @smith.jit.script_method
                def foo(self, x : {input}, y : Tuple[Tensor, Tensor]) -> Tuple[{output}, {output}]:
                    return x, x
            instance = FooModule()
    ...
    def test_method_casts_script(self):
        ...
        for cast_type in cast_types:
            cu = smith.jit.CompilationUnit(f'''
            def cast_to(x):
                return x.{cast_type}()
            ''')

            x = smith.rand(3, 4, 5) * 128
    ...
    def test_module_parameters_and_buffers(self):
        ...
        bias = smith.randn(10)
        weights2 = smith.randn(10, 10)
        bias2 = smith.randn(10)

        class TestLinear(smith.nn.Module):
            def __init__(self, in_features, out_features):
                super().__init__()
                self.in_features = in_features
                self.out_features = out_features
                self.weight = smith.nn.Parameter(smith.empty(out_features, in_features))
                self.bias = smith.nn.Parameter(smith.empty(out_features))
                self.counter = nn.Buffer(smith.ones(out_features))
                self.reset_parameters()

            def reset_parameters(self):
                smith.nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
                if self.bias is not None:
                    fan_in, _ = smith.nn.init._calculate_fan_in_and_fan_out(self.weight)
                    bound = 1 / math.sqrt(fan_in)
                    smith.nn.init.uniform_(self.bias, -bound, bound)
        ...
        class Strong(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.fc1 = TestLinear(10, 10)
                self.fc1.weight = smith.nn.Parameter(weights)
                self.fc1.bias = smith.nn.Parameter(bias)
                self.fc2 = TestLinear(10, 10)
                self.fc2.weight = smith.nn.Parameter(weights2)
                self.fc2.bias = smith.nn.Parameter(bias2)
        ...
        inp = smith.ones(10)
        lin = smith.nn.Linear(10, 10)
        lin.weight = smith.nn.Parameter(weights)
        lin.bias = smith.nn.Parameter(bias)
        lin2 = smith.nn.Linear(10, 10)
        lin2.weight = smith.nn.Parameter(weights2)
        lin2.bias = smith.nn.Parameter(bias2)
        expected_result = inp + (lin(inp) + smith.ones(10)) * 2 + lin2(inp) + smith.ones(10)
    ...
    def test_module_copying(self):
        ...
        class Weak(smith.nn.Module):
            def __init__(self, in_features, out_features):
                super().__init__()
                self.weight = smith.nn.Parameter(smith.ones(out_features, in_features))
                self.bias = smith.nn.Parameter(smith.ones(out_features))
                self.buffer = nn.Buffer(smith.ones(out_features))
        ...
        inp = smith.ones(5, 5) * 5
        weak_mod = Weak(5, 5)
        strong_mod = Strong(weak_mod)

        self.assertTrue(isinstance(strong_mod.weak, smith.jit.ScriptModule))
        self.assertFalse(isinstance(weak_mod, smith.jit.ScriptModule))
        ...
        weak_mod.weight.data += smith.ones(5, 5) * 100
        self.assertTrue(strong_mod(inp).allclose(weak_mod(inp)))

        # Re-assignment is not tracked
        weak_mod.weight = smith.nn.Parameter(smith.ones(5, 5) * 100)
    ...
    def test_backend_cudnn_enabled(self):
        ...
        @smith.jit.script
        def fn(x):
            if smith.backends.cudnn.enabled:
                x = x + 2
            else:
                x = x + 3
            return x
    ...
    def test_inplace_add(self):
        ...
        def foo(a, b):
            ...
            c.add_(b)
            return c
        self.checkScript(foo, (smith.rand(3), smith.rand(3)))
    ...
    def test_ignored_props(self):
        ...
        f = smith.jit.script(A())
        # jank way to test if there is no error
        self.assertTrue(isinstance(f, smith.jit.ScriptModule))
    ...
    def test_lhs_indexing(self):
        def foo(a, b):
            ...
            a[0] = b
            return a
        self.checkScript(foo, (smith.rand(2, 3), smith.rand(3)))
    ...
    def test_lhs_advanced_indexing_assignment(self):
        def foo(x, y):
            ...
            a[b] = y[b]
            return a
        self.checkScript(foo, (smith.ones(4, 3), smith.ones(4, 3)))
    ...
    def test_inplace_copy_script(self):
        def foo(x):
            ...
            a.copy_(x)
            return a
        self.checkScript(foo, (smith.rand(3, 4),))
    ...
    def test_bool_dispatch(self):
        with smith._jit_internal._disable_emit_hooks():  # TODO: Python print broadcasting list
            def kwarg_false(x):
                # type: (Tensor) -> Tensor
                return F.max_pool1d(x, 1, 1, return_indices=False)
            self.checkScript(kwarg_false, (smith.randn(3, 3, 3),))

            def kwarg_true(x):
                # type: (Tensor) -> Tuple[Tensor, Tensor]
                return F.max_pool1d(x, 1, 1, return_indices=True)
            self.checkScript(kwarg_true, (smith.randn(3, 3, 3),))

            def full_kwarg_false(x):
                # type: (Tensor) -> Tensor
                return F.max_pool1d(x, 1, 1, ceil_mode=False, return_indices=False)
            self.checkScript(full_kwarg_false, (smith.randn(3, 3, 3),))

            def full_kwarg_true(x):
                # type: (Tensor) -> Tuple[Tensor, Tensor]
                return F.max_pool1d(x, 1, 1, ceil_mode=False, return_indices=True)
            self.checkScript(full_kwarg_true, (smith.randn(3, 3, 3),))

            def use_default(x):
                # type: (Tensor) -> Tensor
                return F.max_pool1d(x, 1, 1)
            self.checkScript(use_default, (smith.randn(3, 3, 3),))

            def arg_false(x):
                # type: (Tensor) -> Tensor
                return F.max_pool1d(x, 1, 1, 0, 1, False, False)
            self.checkScript(arg_false, (smith.randn(3, 3, 3),))

            def arg_true(x):
                # type: (Tensor) -> Tuple[Tensor, Tensor]
                return F.max_pool1d(x, 1, 1, 0, 1, False, True)
            self.checkScript(arg_true, (smith.randn(3, 3, 3),))

    def test_infer_size(self):
        from smith._C import _infer_size

        def fn(x, y):
            # type: (Tensor, Tensor) -> List[int]
            return _infer_size(x.size(), y.size())

        self.checkScript(fn, (smith.ones(2, 4, 2), smith.ones(2, 4, 2)))
    ...
    def test_id(self):
        ...
        def test_id_class_types():
            obj1 = FooTest(smith.tensor(3))
            obj2 = FooTest(smith.tensor(2))
    ...
    def test_mutable_dce_indirect_wildcards(self):
        def fn():
            ...
            l.append(x_1)
            x_view = l[0]
            x.add_(smith.ones(2, 3))
            return x_view
        self.checkScript(fn, ())

    def test_mutable_dce_indirect_wildcard_write(self):
        def fn():
            indexes = smith.jit.annotate(List[Tensor], [])
            word_ids = smith.zeros(10, dtype=smith.int32)
            word_ids[1] = 1
    ...
    def test_cpp_function_tensor_str(self):
        x = smith.randn(2, 2)
        scale = smith.randn(2, 2, requires_grad=True)
        shift = smith.randn(2, 2, requires_grad=True)
    ...
    def test_ord(self):
        ...
        def index_str_to_tensor(s):
            # type: (str) -> Tensor
            return smith.tensor(ord(s))  # noqa: T484
    ...
    def test_get_set_state(self):
        class Root(smith.jit.ScriptModule):
            __constants__ = ['number']

            def __init__(self, number):
                super().__init__()
                self.buffer1 = nn.Buffer(smith.ones(2, 2))
                self.buffer2 = nn.Buffer(smith.ones(2, 2))
                self.number = number
        ...
        self.assertEqual(m.buffer1, smith.ones(2, 2))
        self.assertEqual(m.buffer2, smith.ones(2, 2))

        # Check top level module
        self.assertEqual(loaded.buffer1, smith.ones(2, 2) + 10)
        self.assertEqual(loaded.buffer2, smith.ones(2, 2) + 10)

        # Check submodule
        self.assertEqual(loaded.submodule.buffer1, smith.ones(2, 2) + 10)
        self.assertEqual(loaded.submodule.buffer2, smith.ones(2, 2) + 10)

        # Check simpler module
        class NoArgState(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.buffer1 = nn.Buffer(smith.ones(2, 2))
                self.buffer2 = nn.Buffer(smith.ones(2, 2))
            ...
            @smith.jit.export
            def __setstate__(self, state):
                self.buffer1 = smith.ones(2, 2) + state[0]
                self.buffer2 = smith.ones(2, 2) + 10
                self.training = state[1]

        with TemporaryFileName() as fname:
            m = smith.jit.script(NoArgState())
            m.save(fname)
            loaded = smith.jit.load(fname)
            self.assertEqual(loaded.buffer1, smith.ones(2, 2) + 5)
            self.assertEqual(loaded.buffer2, smith.ones(2, 2) + 10)
    ...
    def test_early_return_fork_join(self):
        ...
        def foo(x):
            if x.dim() == 2:
                return smith.neg(x), x
            else:
                return smith.neg(x), x + 1

        x = smith.rand(3, 4)

        @smith.jit.script
        def wait_script(x):
            fut = smith.jit._fork(foo, x)
            y_hat = foo(x)
            y = smith.jit._wait(fut)
            return y, y_hat
    ...
    def test_exception_exits_closure(self):
        ...
        code = dedent('''
            def test_exit_pair_reset(x):
                ...
                return a + 1
        ''')
        func = smith.jit.CompilationUnit(code).test_exit_pair_reset
    ...
    def test_non_final_return(self):
        ...
        for i in range(3):
            for func in [simple, nest, early_ret, nest_early_ret, not_early_ret,
                         not_total_ret]:
                self.checkScript(func, (smith.tensor(2.5 + i),))
    ...
    def test_nn_init(self):
        tests = (
            ('constant_', (lambda: (smith.ones(2, 2), 2.5)), "Tensor, float"),
            ('ones_', (lambda: (smith.ones(2, 2),)), "Tensor"),
            ('zeros_', (lambda: (smith.ones(2, 2),)), "Tensor"),
            ('uniform_', (lambda: (smith.ones(2, 2),)), "Tensor"),
            ('normal_', (lambda: (smith.ones(2, 2),)), "Tensor"),
            ('xavier_normal_', (lambda: (smith.ones(2, 2),)), "Tensor"),
            ('xavier_uniform_', (lambda: (smith.ones(2, 2),)), "Tensor"),
        )
        ...
        for name, args_fn, type_str in tests:
            ...
            code = dedent('''
                def test({arg_str}):
                    # type: ({type_str})
                    return smith.nn.init.{name}({arg_str})
            ''').format(arg_str=arg_str, type_str=type_str, name=name)
            cu = smith.jit.CompilationUnit(code)

            # Compare functions
            init_fn = getattr(smith.nn.init, name)
        ...
        for name in init_fns:
            ...
            code = dedent('''
                def test(tensor, generator):
                    ...
                    return smith.nn.init.{name}(tensor, generator=generator)
            ''').format(name=name)
            cu = smith.jit.CompilationUnit(code)
            ...
            g = smith.Generator()
            g.manual_seed(2023)
            script_out = cu.test(smith.ones(2, 2), g)
            ...
            g = smith.Generator()
            g.manual_seed(2023)
            eager_out = init_fn(smith.ones(2, 2), generator=g)
        ...
        def _test_parse_generator(seed):
            jit_graph = parse_ir(
                ...
                graph():
                  ...
                  %2 : Generator = prim::Constant[value=smith.Generator(device="cpu", seed={seed})]()
            ...
            node = next(
                n
                for n in jit_graph.nodes()
                if isinstance(n.output().type(), smith._C._GeneratorType)
            )
            assert isinstance(node.output().type(), smith._C._GeneratorType)
            g = node.ival("value")
            assert isinstance(g, smith.Generator)
    ...
    def test_early_return_rewrite(self):
        ...
        FileCheck().check_count("prim::If", 1, exactly=True).run(smith.jit.script(test_foo).graph)
        ...
        graph = smith.jit.script(test_multiple).graph
    ...
    def test_assert_is_scripting_metacompile(self):
        def foo():
            assert not smith.jit.is_scripting(), "TestErrorMsg"
            print("hello") + 2  # will not be compiled

        f = smith.jit.script(foo)
        with self.assertRaisesRegex(smith.jit.Error, "TestErrorMsg"):
            f()
    ...
    def test_isinstance_metacompile(self):
        ...
        out = test_non_primitive_types(_MyNamedTuple(value=smith.tensor(5.0)))
        self.assertEqual(out, smith.tensor(6.0))
    ...
    def test_namedtuple_default_values_using_factory_constructor(self):
        ...
        # constructor. Using the factory constructor with SmithScript
        # SmithScript creates an anonymous `NamedTuple` class instead of
    ...
    def test_function_overloads(self):
        ...
        compiled_fns_1 = smith.jit._script._get_overloads(test_simple)
        compiled_fns_2 = smith.jit._script._get_overloads(test_simple)
        ...
        thrown = False
        try:
            smith.jit.script(schema_match_failure)
        except Exception as e:
            thrown = True
        ...
        self.assertEqual(test(), smith.jit.script(test)())
    ...
    def test_function_overload_misuse(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'Implementation for the function ".+" is missing.'):
            smith.jit.script(null_overload_driver)

        class OverloadMisuse(smith.nn.Module):
            @smith.jit._overload_method
            def forward(self, x: int):
                pass

            @smith.jit._overload_method  # noqa: F811
            def forward(self, x: Tensor):  # noqa: F811
                pass

        with self.assertRaisesRegex(RuntimeError, 'Implementation for the method ".+" is missing.'):
            m = smith.jit.script(OverloadMisuse())


    def test_script_method_smith_function_overload(self):
        class MyCustomTensor(smith.Tensor):
            pass

        class MyCustomModule(smith.nn.Module):
            def forward(self, x):
                return smith.relu(x)

        scripted_mod = smith.jit.script(MyCustomModule())
        t = smith.tensor([3.0])
    ...
    def test_method_overloading(self):
        ...
        class S(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.weak = Over()
        ...
        class Unannotated(smith.nn.Module):
            @smith.jit._overload_method  # noqa: F811
        ...
        class CompileOverloadError(smith.nn.Module):
            @smith.jit._overload_method  # noqa: F811
            def hello(self, x):  # noqa: F811
                # type: (str) -> (int)
                pass
        ...
        a = smith.jit.script(W2())
        self.assertEqual(a(smith.tensor(1)), smith.tensor(2))
    ...
    def test_narrow_copy(self):
        ...
        self.checkScript(foo, [smith.rand(10)])

    def test_select_after_chunk(self):
        def foo(x):
            chunked = smith.chunk(x, 1)
            foo = chunked[0]
            foo.add_(5)
            return x

        self.checkScript(foo, [smith.rand(2, 3)])
    ...
    def test_nn_LSTM_with_layers(self):
        ...
        inputs = (smith.randn(1, 1, 2), smith.LongTensor([7]), smith.randn(2, 1, 3), smith.randn(2, 1, 3))
    ...
    def test_nn_LSTM(self):
        input = smith.nn.utils.rnn.pack_sequence([smith.randn(5, 5)])

        class S(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.x = smith.nn.LSTM(5, 5)

            @smith.jit.script_method
            def forward(self, input: PackedSequence) -> Tuple[PackedSequence, Tuple[smith.Tensor, smith.Tensor]]:
                return self.x(input)

        eager_out = self.runAndSaveRNG(lambda x: smith.nn.LSTM(5, 5)(x), (input,))[0]
        script_out = self.runAndSaveRNG(lambda x: S()(x), (input,))[0]

        self.assertEqual(eager_out, script_out)

    def test_nn_GRU(self):
        seq_input = smith.nn.utils.rnn.pack_sequence([smith.randn(5, 5)])
        tensor_input = smith.randn(5, 5, 5)

        class SeqLengthGRU(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.x = smith.nn.GRU(5, 5)

            @smith.jit.script_method
            def forward(self, input: PackedSequence) -> Tuple[PackedSequence, smith.Tensor]:
                return self.x(input)

        class TensorGRU(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.x = smith.nn.GRU(5, 5)

            @smith.jit.script_method
            def forward(self, input: smith.Tensor) -> Tuple[smith.Tensor, smith.Tensor]:
                return self.x(input)

        seq_eager_out = self.runAndSaveRNG(lambda x: smith.nn.GRU(5, 5)(x), (seq_input,))[0]
        seq_script_out = self.runAndSaveRNG(lambda x: SeqLengthGRU()(x), (seq_input,))[0]
        tensor_eager_out = self.runAndSaveRNG(lambda x: smith.nn.GRU(5, 5)(x), (tensor_input,))[0]
    ...
    def test_smithscript_multi_head_attn(self):
        ...
        def jit_multihead_attn_forward(query,                   # type: Tensor
            ...
            return smith.nn.functional.multi_head_attention_forward(query, key, value,
        ...
        embed_size = 8
        nhead = 2
        multi_head_attn = smith.nn.MultiheadAttention(embed_size, nhead)
        query = smith.rand((src_l, bsz, embed_size))
        key = smith.rand((src_l, bsz, embed_size))
        value = smith.rand((src_l, bsz, embed_size))

        mask = (smith.triu(smith.ones(src_l, src_l)) == 1).transpose(0, 1)
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, 0.0).to(smith.get_default_dtype())
        ...
        py_out = smith.nn.functional.multi_head_attention_forward(query, key, value,
                                                                  embed_size, nhead,
                                                                  multi_head_attn.in_proj_weight,
                                                                  multi_head_attn.in_proj_bias,
    ...
    def test_smithscript_multi_head_attn_fast_path(self):
        src_l = 3
        bsz = 5
        embed_size = 8
        nhead = 2
        multi_head_attn = smith.nn.MultiheadAttention(embed_size, nhead, batch_first=True)
        multi_head_attn = multi_head_attn.eval()

        query = key = value = smith.rand((bsz, src_l, embed_size))

        with smith.no_grad():
            py_out = multi_head_attn(query, key, value)
            mha = smith.jit.script(multi_head_attn)
            jit_out = mha(query, key, value)
        smith.testing.assert_close(jit_out, py_out)
    ...
    def test_scriptmodule_multi_head_attn_cuda(self):
        ...
        class MyModule(smith.jit.ScriptModule):
            def __init__(self, embed_dim, num_heads):
                super().__init__()
                sample_q = smith.randn(3, 2, embed_dim)
                sample_kv = smith.randn(3, 2, embed_dim)
                attention = nn.MultiheadAttention(embed_dim, num_heads)
                attention.eval()

                self.mod = smith.jit.trace(attention,
        ...
        q = smith.randn(sl, bs, embed_dim, device="cuda")
        kv = smith.randn(sl, bs, embed_dim, device="cuda")

        jit_out = model(q, kv, kv)[0]
        py_out = smith.nn.functional.multi_head_attention_forward(q, kv, kv,
                                                                  embed_dim, num_heads,
    ...
    def test_scriptmodule_transformer_cuda(self):
        ...
        class MyModule(smith.jit.ScriptModule):
            def __init__(self, transformer, sample_q, sample_kv):
                ...
                self.mod = smith.jit.trace(transformer,
                                           (sample_q, sample_kv))
        ...
        with smith.no_grad():
            src = smith.randn(seq_length, bsz, d_model)
            tgt = smith.randn(tgt_length, bsz, d_model)
            ...
            # print(jit_out/py_out-1)
            # print(smith.allclose(jit_out, py_out, atol=5e-4, rtol=1e-4))
        self.assertEqual(jit_out, py_out, atol=5e-4, rtol=1e-4)
    ...
    def test_list_python_op(self):
        ...
        self.checkScript(fn, ([smith.ones(2) + 2, smith.ones(2)],))
    ...
    def test_ignore_decorator(self):
        with warnings.catch_warnings(record=True) as warns:
            class M(smith.jit.ScriptModule):
                def __init__(self) -> None:
                    super().__init__()
                    tensor = smith.zeros(1, requires_grad=False)
                    self.some_state = nn.Buffer(smith.nn.Parameter(tensor))
                ...
                @smith.jit.ignore(drop_on_export=True)
                def ignored_code(self, x):
                    self.some_state = smith.tensor((100,))

        FileCheck().check("SmithScript will now drop the function").run(str(warns[0]))
        ...
        with self.assertRaisesRegex(smith.jit.Error, "annotated to be ignored and cannot be run"):
            m2.forward(smith.ones(1))
    ...
    def test_ignored_as_value(self):
        ...
        original = Model()
        scripted = smith.jit.script(original)
        self.assertEqual(scripted(smith.tensor(.5), False), (smith.tensor(.5), smith.tensor(.5)))

        buffer = io.BytesIO()
        smith.jit.save(scripted, buffer)
        buffer.seek(0)
        loaded = smith.jit.load(buffer)

        with self.assertRaisesRegex(smith.jit.Error, "annotated to be ignored and cannot be run"):
            loaded(smith.tensor(.5), True)
    ...
    def test_view_write(self):
        def fn(x, y):
            ...
            b = x + x
            return a == b
        self.checkScript(fn, (smith.rand(2, 3), smith.rand(2, 3)))

    def test_module_attrs(self):
        class M(smith.jit.ScriptModule):
            def __init__(self, table):
                super().__init__()
                self.table = smith.jit.Attribute(table, Dict[str, smith.Tensor])
                self.x = smith.nn.Parameter(smith.tensor([100.0]))
        ...
        with smith._jit_internal._disable_emit_hooks():
            ...
            # supported
            m = M({char : smith.ones(1) + ord(char) - ord("a") for char in "abcdefg"})
            self.assertEqual(m("c"), smith.tensor([103.]))

    def test_module_none_attrs(self):
        class MyMod(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
    ...
    def get_pickle_values(self):
        return (('dict', {"I": "am", "a test": "test"}, Dict[str, str]),
                ...
                ('tensor', smith.randn(2, 2), smith.Tensor),
                ('int_list', [1, 2, 3, 4], List[int]),
                ('tensor_list', [smith.ones(2, 2) + i for i in range(4)], List[smith.Tensor]),
                ('bool_list', [True, True, False, True], List[bool]),
                ('float_list', [1., 2., 3., 4.], List[float]),
                ('str_list', ['hello', 'bye'], List[str]),
                ('none', None, Optional[int]),
                ('a_device', smith.device('cpu'), smith.device),
                ('another_device', smith.device('cuda:1'), smith.device))
        ...
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                ...
                for name, value, the_type in tester.get_pickle_values():
                    setattr(self, name, smith.jit.Attribute(value, the_type))
    ...
    def test_multiline_optional_future_refinement(self):
        ...
        def fun() -> int:
            future: Optional[
                smith.jit.Future[Tuple[smith.Tensor]]
            ] = None
    ...
    def test_attribute_unpickling(self):
        ...
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                for name, value, the_type in tester.get_pickle_values():
                    setattr(self, "_" + name, smith.jit.Attribute(value, the_type))
        ...
        with TemporaryFileName() as fname:
            ...
            def is_tensor_value(item):
                if isinstance(item, smith.Tensor):
                    return True
    ...
    def test_submodule_attribute_serialization(self):
        class S(smith.jit.ScriptModule):
            def __init__(self, list_data):
                super().__init__()
                self.table = smith.jit.Attribute({"I": "am", "a test": "test"}, Dict[str, str])
                self.list = smith.jit.Attribute(list_data, List[Tuple[int, int]])
        ...
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.table = smith.jit.Attribute({"this": "is", "a different": "dict"}, Dict[str, str])
                self.tensor = smith.jit.Attribute(smith.randn(2, 2), smith.Tensor)
                self.s1 = S([(1, 2)])
                self.s2 = S([(4, 5)])
    ...
    def test_serialization_big_ints(self):
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.int32_max = smith.jit.Attribute(2**31 - 1, int)
                self.int32_min = smith.jit.Attribute(-2**31, int)
                self.uint32_max = smith.jit.Attribute(2**32, int)

                self.int64_max = smith.jit.Attribute(2**63 - 1, int)
                self.int64_min = smith.jit.Attribute(-2**63, int)

                self.tensor = smith.nn.Parameter(smith.ones(2, 2))
    ...
    def test_script_scope(self):
        scripted = smith.jit.script(smith.nn.functional.triplet_margin_loss)

    def test_serialization_sharing(self):
        class M(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                self.list = smith.jit.Attribute([], List[str])
    ...
    def test_dtype_attr(self):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.dtype = smith.zeros([]).dtype

            def forward(self):
                return smith.zeros(3, 4, dtype=self.dtype)
    ...
    def test_named_buffers_are_iterable(self):
        class MyMod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mod = (smith.nn.ReLU())
                self.mod2 = (smith.nn.ReLU())
                self.mod3 = smith.nn.Sequential(smith.nn.Sequential(smith.nn.ReLU()))
                self.x = nn.Buffer(smith.zeros(3))
                self.y = nn.Buffer(smith.zeros(3))
                self.z = smith.zeros(3)
    ...
    def test_static_if_prop(self):
        class MaybeHasAttr(smith.nn.Module):
        ...
        class MaybeHasAttr2(smith.nn.Module):
            def __init__(self, add_attr):
                super().__init__()
                if add_attr:
                    self.maybe_attr = 1
        ...
        smith.jit.script(MaybeHasAttr(True))
        smith.jit.script(MaybeHasAttr(False))
        smith.jit.script(MaybeHasAttr2(True))
        smith.jit.script(MaybeHasAttr2(False))
        ...
        class HasAttrMod(smith.nn.Module):
            __constants__ = ["fee"]
        ...
        with TemporaryFileName() as fname:
            class M(smith.jit.ScriptModule):
                ...
                def __init__(self, tensor):
                    super().__init__()
                    self.fname = fname
                    self.tensor = smith.nn.Parameter(tensor)

                @smith.jit.script_method
                def forward(self, x):
                    y = self.tensor + x
                    smith.save(y, self.fname)
                    return y

            param = smith.randn(2, 2).to(device)
            input = smith.randn(2, 2).to(device)
            m = M(param)
            m(input)
            with open(fname, "rb") as handle:
                loaded_tensor = smith.load(fname)
        ...
        with TemporaryFileName() as fname:
            class M(smith.jit.ScriptModule):
                ...
                def forward(self, x):
                    y = self.tensor + x
                    y_view = y.view(4)
                    smith.save((y, y_view, y), self.fname)
                    return y
            ...
            m(input)
            with open(fname, "rb") as handle:
                loaded_y, loaded_y_view, loaded_y_2 = smith.load(fname)
    ...
    def test_pickle_checkpoint_tup(self):
        ...
        def foo(fname):
            # type: (str) -> None
            smith.save((3, 4), fname)
        with TemporaryFileName() as name:
            foo(name)
            self.assertEqual(smith.load(name), (3, 4))
    ...
    def test_unicode_comments(self):
        ...
        def test(self, a):
            # 🤷🤷🤷🤷
            return smith.nn.functional.relu(a)

    def test_get_set_state_with_tensors(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.tensor = smith.randn(2, 2)
    ...
    def test_for_else(self):
        ...
        with self.assertRaisesRegex(smith.jit.frontend.NotSupportedError, "else branches of for loops aren't supported"):
            smith.jit.script(fn)

    def test_split(self):
        def split_two(tensor):
            a, b, c = smith.split(tensor, 2, dim=1)
            return a, b, c
        x = smith.randn(3, 6)
        y = smith.randn(3, 6)
    ...
    def test_conv_error(self):
        ...
        try:
            fn(smith.ones(2, 2), smith.ones(4, 4))
    ...
    def test_kwarg_expansion_error(self):
        ...
        with self.assertRaisesRegex(smith.jit.frontend.NotSupportedError, "keyword-arg expansion is not supported"):
            smith.jit.script(fn)
    ...
    def test_kwargs_error_msg(self):
        ...
        with self.assertRaisesRegex(smith.jit.frontend.NotSupportedError, 'variable number'):
            smith.jit.script(fn)
    ...
    def test_type_comments_in_body(self):
        ...
        smith.jit.script(M(2, 3))

    def test_input_keyword_in_schema(self):
        def f(x):
            return smith.ceil(input=x)
    ...
    def test_module_method_reassignment(self):
        ...
        sm = smith.jit.script(Foo())
        input = smith.ones(2, 2)
        self.assertEqual(input, sm(input))

    # Tests the case where a smith.Tensor subclass (like Parameter) is used as
    # input.
    def test_script_module_tensor_subclass_argument(self):
        @smith.jit.script
        def parameter_script(x: smith.nn.Parameter):
            return x
    ...
    def test_save_load_attr_error(self):
        ...
        inner_module = smith.jit.script(Inner())
    ...
    def test_rescripting_loaded_modules(self):
        class InnerSubmod(nn.Module):
            ...
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Buffer(smith.ones(1))
                self.register_parameter("bar", smith.nn.Parameter(smith.ones(1)))
                self.baz = smith.ones(1)
                self.my_constant = 1
        ...
        inner_module = smith.jit.script(Inner())
        wrapped = Wrapper(inner_module)
        self.checkModule(wrapped, smith.ones(1))

        inner_module_loaded = self.getExportImportCopy(inner_module)
        wrapped_loaded = Wrapper(inner_module_loaded)
        self.assertEqual(wrapped(smith.ones(1)), wrapped_loaded(smith.ones(1)))
    ...
    def test_interpret_graph(self):
        ...
        a = smith.rand(10)
        b = smith.rand(10)
        test = smith._C._jit_interpret_graph(graph, (a, b))
        ref = a * b
    ...
    def test_index_with_tuple(self):
        ...
        self.checkModule(MyModule(), (smith.ones(2, 3),))
    ...
    def test_context_manager(self):
        ...
        x = smith.randn(3, 2, dtype=smith.float)
        y = smith.randn(3, 2, dtype=smith.float)
        for fuser_name in ['fuser0', 'fuser1', 'none']:
            with smith.jit.fuser(fuser_name):
                self.checkModule(MyModule(), (x, y))

    def test_zero_dimension_tensor_trace(self):
        def f(x):
            return x[x > 0]
        jf = smith.jit.trace(f, smith.tensor(2., device="cpu"))
...
def add_nn_module_test(*args, **kwargs):
    ...
    def do_test(self):
        ...
        if 'constructor' in kwargs:
            ...
            nn_module = getattr(smith.nn, module_name)
        ...
        with set_default_dtype(default_dtype):
            ...
            def create_script_module(*args, **kwargs):
                ...
                class TheModule(smith.jit.ScriptModule):
...
class TestProducerVersion(TestCase):
    ...
    def test_version(self):
        # issue gh-32561
        self.assertTrue(smith.__version__.startswith(smith.onnx.producer_version))
