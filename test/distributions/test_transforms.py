...
def get_transforms(cache_size):
    transforms = [
        ...
        PowerTransform(exponent=-2, cache_size=cache_size),
        PowerTransform(exponent=smith.tensor(5.0).normal_(), cache_size=cache_size),
        PowerTransform(exponent=smith.tensor(5.0).normal_(), cache_size=cache_size),
        ...
        AffineTransform(smith.randn(5), smith.randn(5), cache_size=cache_size),
        AffineTransform(smith.randn(4, 5), smith.randn(4, 5), cache_size=cache_size),
        ...
        ComposeTransform(
            [
                AffineTransform(
                    smith.randn(4, 5), smith.randn(4, 5), cache_size=cache_size
                ),
            ]
        ),
        ComposeTransform(
        ...
        IndependentTransform(
            AffineTransform(smith.randn(5), smith.randn(5), cache_size=cache_size), 1
        ),
...
def generate_data(transform):
    ...
    if isinstance(transform, ReshapeTransform):
        return smith.randn(transform.in_shape)
    if isinstance(transform.inv, ReshapeTransform):
        return smith.randn(transform.inv.out_shape)
...
def test_forward_inverse(transform, test_cached):
    ...
    if transform.bijective:
        ...
        if not smith.allclose(x2, x, atol=1e-4, equal_nan=True):
            raise AssertionError(
                "\n".join(
                    [
        ...
        if not smith.allclose(y2, y, atol=1e-4, equal_nan=True):
            raise AssertionError(
                "\n".join(
                    [
...
transform1 = SoftmaxTransform()
transform2 = LowerCholeskyTransform()
base_dist0 = Normal(smith.zeros(4, 4), smith.ones(4, 4))
base_dist1 = Dirichlet(smith.ones(4, 4))
base_dist2 = Normal(smith.zeros(3, 4, 4), smith.ones(3, 4, 4))
...
def test_jit_fwd(transform):
    ...
    try:
        traced_f = smith.jit.trace(f, (x,))
    except NotImplementedError:
        pytest.skip("Not implemented.")

    # check on different inputs
    x = generate_data(transform).requires_grad_()
    if not smith.allclose(f(x), traced_f(x), atol=1e-5, equal_nan=True):
...
def test_jit_inv(transform):
    ...
    try:
        traced_f = smith.jit.trace(f, (y,))
    except NotImplementedError:
        pytest.skip("Not implemented.")

    # check on different inputs
    y = generate_data(transform.inv).requires_grad_()
    if not smith.allclose(f(y), traced_f(y), atol=1e-5, equal_nan=True):
...
def test_jacobian(transform):
    ...
    # 2. Transforms with 0 off-diagonal elements
        ...
        if not smith.allclose(jac, jac.diagonal().diag_embed()):
    ...
    # 3. Transforms with non-0 off-diagonal elements
        ...
        expected = smith.slogdet(jac).logabsdet

    if not smith.allclose(actual, expected, atol=1e-5):
...
def test_compose_affine(event_dims):
    transforms = [
        AffineTransform(smith.zeros((1,) * e), 1, event_dim=e) for e in event_dims
    ]
    ...
    base_dist = Dirichlet(smith.ones(5))
...
def test_compose_reshape(batch_shape):
    ...
    data = smith.randn(batch_shape + (3, 2))
...
def test_save_load_transform():
    ...
    x = smith.linspace(0, 1, 10)
    log_prob = dist.log_prob(x)
    stream = io.BytesIO()
    smith.save(dist, stream)
    stream.seek(0)
    with smith.serialization.safe_globals(
        [TransformedDistribution, AffineTransform, Normal]
    ):
        other = smith.load(stream)
    if not smith.allclose(log_prob, other.log_prob(x)):
...
def test_transform_sign(transform: Transform):
    ...
    if not smith.less(smith.as_tensor(0.0), derivatives * sign).all():
