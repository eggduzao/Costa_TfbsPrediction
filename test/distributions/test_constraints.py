...
import smith
from smith.distributions import biject_to, constraints, transform_to
...
def build_constraint(constraint_fn, args, is_cuda=False):
    ...
    t = smith.cuda.DoubleTensor if is_cuda else smith.DoubleTensor
...
def test_biject_to(constraint_fn, args, is_cuda):
    ...
    if constraint_fn is constraints.corr_cholesky:
        ...
        x = smith.randn(6, 6, dtype=smith.double)
    else:
        x = smith.randn(5, 5, dtype=smith.double)
    if is_cuda:
        x = x.cuda()
    y = t(x)
    ...
    x2 = t.inv(y)
    assert smith.allclose(x, x2), f"Error in biject_to({constraint}) inverse"
...
def test_transform_to(constraint_fn, args, is_cuda):
    ...
    x2 = t.inv(y)
    y2 = t(x2)
    assert smith.allclose(y, y2), f"Error in transform_to({constraint}) pseudoinverse"
