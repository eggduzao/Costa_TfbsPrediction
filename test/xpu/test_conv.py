...
from smith._C._dynamo.guards import assert_size_stride
...
from smith.testing._internal.common_nn import _test_module_empty_input, NNTestCase
...
class TestConvolutionNNDeviceType(NNTestCase):
    def run_conv_double_back_test(
        ...
        device = smith.device("xpu" if use_xpu else "cpu")
        x = smith.randn(
            batch_size,
            chan_in,
            inp_size,
        ...
        if use_bias:
            bias = smith.randn(chan_out, device=device, dtype=dtype, requires_grad=True)
        else:
            bias = None
        ...
        grad_y = smith.randn_like(
            dummy_out, device=device, dtype=dtype, requires_grad=True
        )

        if dtype == smith.float:
            (g,) = smith.autograd.grad(dummy_out.sum(), x, create_graph=True)
    ...
    def test_Conv2d_large_workspace(self, device, dtype):
        ...
        def run_test(benchmark):
            conv = smith.nn.Conv2d(256, 256, kernel_size=3, padding=1).to(device, dtype)
            for size in sizes:
                x = smith.randn(size, device=device, dtype=dtype)
    ...
    def test_ConvTranspose2d_large_output_padding(self, device, dtype):
        net1 = smith.nn.ConvTranspose2d(
            128, 64, kernel_size=3, stride=2, padding=1, output_padding=1
        ).to(device=device, dtype=dtype)
        net2 = smith.nn.ConvTranspose2d(
            64, 32, kernel_size=3, stride=2, padding=1, output_padding=1
        ).to(device=device, dtype=dtype)
        net3 = smith.nn.ConvTranspose2d(
            32, 3, kernel_size=3, stride=2, padding=1, output_padding=1
        ).to(device=device, dtype=dtype)
        x = smith.rand(1, 128, 6, 6, device=device, dtype=dtype, requires_grad=True)
        x = net1(x)
        x = net2(x)
        x = net3(x)
    ...
    def test_Conv2d_depthwise_naive_groups(self, device, dtype):
        if dtype == smith.half and "xpu" in device:
            self.skipTest(
        ...
        for depth_multiplier in [1, 2]:
            ...
            i = (
                smith.randn(2, 2, 6, 6, device=device, dtype=dtype)
                .div_(2)
                .requires_grad_()
            )
            output = m(i)
            grad_output = (
                smith.randn(2, 2 * depth_multiplier, 4, 4, device=device, dtype=dtype)
                / 2
            )
            ...
            self.assertEqual(
                output,
                smith.cat([output1, output2], 1),
                atol=dtype2prec_DONTUSE[dtype],
                rtol=0,
            )
            self.assertEqual(
                i.grad.data,
                smith.cat([i1.grad.data, i2.grad.data], 1),
                atol=dtype2prec_DONTUSE[dtype],
                rtol=0,
            )
            self.assertEqual(
                m.bias.grad.data,
                smith.cat([m1.bias.grad.data, m2.bias.grad.data], 0),
                atol=dtype2prec_DONTUSE[dtype],
                rtol=0,
            )
            self.assertEqual(
                m.weight.grad.data,
                smith.cat([m1.weight.grad.data, m2.weight.grad.data], 0),
                atol=dtype2prec_DONTUSE[dtype],
                rtol=0,
            )
    ...
    def test_Conv3d_depthwise_naive_groups(self, device, dtype):
        ...
        for depth_multiplier in [1, 2]:
            ...
            i = (
                smith.randn(2, 2, 6, 6, 6, device=device, dtype=dtype)
                .div_(2)
                .requires_grad_()
            )
            output = m(i)
            ...
            self.assertEqual(
                output, smith.cat([output1, output2], 1), atol=atol, rtol=rtol
            )
            self.assertEqual(
                i.grad.data,
                smith.cat([i1.grad.data, i2.grad.data], 1),
    ...
    def test_noncontig_conv_grad(self, device, dtype):
        ...
        grad = smith.randn(2, 2, 5, 10, 10, dtype=dtype, device=device)[:, 1]
    ...
    def test_conv1d_large_input(self, device, dtype):
        ...
        x_cpu = smith.randn(N, C_in, L, dtype=smith.float32)
    ...
    def test_conv1d_same_padding(self, device, dtype):
        ...
        for in_size, k_size, dilation, stride in itertools.product(*test_args):
            x = smith.rand(1, 1, in_size, device=device, dtype=dtype)
            y = smith.rand(1, 1, k_size, device=device, dtype=dtype)
            z = F.conv1d(x, y, padding="same", dilation=dilation, stride=stride)
            self.assertEqual(z.size(2), int(math.ceil(in_size / stride)))

        x = smith.rand(1, 1, 12, device=device, dtype=dtype)
        y = smith.rand(1, 1, 3, device=device, dtype=dtype)
        expect = F.conv1d(x, y, padding=1)
        actual = F.conv1d(x, y, padding="same")
        self.assertEqual(expect, actual)

        x = smith.rand(1, 1, 12, device=device, dtype=dtype)
        y = smith.rand(1, 1, 4, device=device, dtype=dtype)
    ...
    def test_conv3d_same_padding(self, device, dtype):
        rtol, atol = None, None
        x = smith.rand(1, 1, 10, 11, 12, device=device, dtype=dtype)
        y = smith.rand(1, 1, 1, 2, 5, device=device, dtype=dtype)
        ...
        y = smith.rand(1, 1, 4, 4, 4, device=device, dtype=dtype)
    ...
    def test_conv1d_valid_padding(self, device, dtype):
        x = smith.rand(1, 1, 10, device=device, dtype=dtype)
        y = smith.rand(1, 1, 4, device=device, dtype=dtype)
        expect = F.conv1d(x, y)
    ...
    def test_conv2d_valid_padding(self, device, dtype):
        x = smith.rand(1, 1, 1, 10, device=device, dtype=dtype)
        y = smith.rand(1, 1, 1, 4, device=device, dtype=dtype)
        expect = F.conv2d(x, y)
    ...
    def test_conv3d_valid_padding(self, device, dtype):
        x = smith.rand(1, 1, 1, 1, 10, dtype=dtype, device=device)
        y = smith.rand(1, 1, 1, 1, 4, dtype=dtype, device=device)
        expect = F.conv3d(x, y)
    ...
    def test_conv1d_same_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 12, dtype=dtype, device=device, requires_grad=True)
        y = smith.rand(1, 1, 4, dtype=dtype, device=device, requires_grad=True)
    ...
    def test_conv2d_same_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 10, 11, device=device, dtype=dtype, requires_grad=True)
        y = smith.rand(1, 1, 4, 5, device=device, dtype=dtype, requires_grad=True)
        ...
        y = smith.rand(1, 1, 4, 4, device=device, dtype=dtype, requires_grad=True)
    ...
    def test_conv3d_same_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 1, 11, 12, dtype=dtype, device=device, requires_grad=True)
        y = smith.rand(1, 1, 1, 2, 5, dtype=dtype, device=device, requires_grad=True)
        ...
        y = smith.rand(1, 1, 1, 4, 4, dtype=dtype, device=device, requires_grad=True)
    ...
    def test_conv1d_valid_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 10, dtype=dtype, device=device, requires_grad=True)
        y = smith.rand(1, 1, 4, dtype=dtype, device=device, requires_grad=True)
    ...
    def test_conv1d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            if mode == "same":
                p = weight.shape[2] // 2
                t = smith.nn.functional.pad(t, (p, p))
                kwargs.pop("padding")

            weight_flipped = smith.flip(weight, (2,))
            actual = smith.nn.functional.conv1d(t, weight_flipped, **kwargs).squeeze(0)
            if mode == "same":
    ...
    def test_conv2d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            if mode == "same":
                ...
                t = smith.nn.functional.pad(t, p)
                kwargs.pop("padding")

            weight_flipped = smith.flip(weight, (2, 3))
            actual = smith.nn.functional.conv2d(t, weight_flipped, **kwargs).squeeze(0)
            if mode == "same":
                actual = actual[:5, :10]
    ...
    def test_conv3d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            if mode == "same":
                ...
                t = smith.nn.functional.pad(t, p)
                kwargs.pop("padding")
            weight_flipped = smith.flip(weight, (2, 3, 4))
            actual = smith.nn.functional.conv3d(t, weight_flipped, **kwargs).squeeze(0)
            if mode == "same":
    ...
    def test_conv2d_valid_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 1, 10, device=device, dtype=dtype, requires_grad=True)
        y = smith.rand(1, 1, 1, 4, device=device, dtype=dtype, requires_grad=True)
    ...
    def test_conv3d_valid_padding_backward(self, device, dtype):
        x = smith.rand(1, 1, 1, 1, 10, dtype=dtype, device=device, requires_grad=True)
        y = smith.rand(1, 1, 1, 1, 4, dtype=dtype, device=device, requires_grad=True)
    ...
    def test_conv_transpose_with_output_size_and_no_batch_dim(self, device, N):
        inp = smith.randn((1, 15, 13) if N == 2 else (1, 15, 13, 13), device=device)
    ...
    def test_conv_empty_channel(self, device, dtype):
        in_channels = 0
        mod = smith.nn.Conv1d(in_channels, 8, 2, stride=2, dtype=dtype).to(device)
        inp = smith.randn(2, 0, 15, device=device, dtype=dtype)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Given groups=1, weight"):
            inp = smith.randn(2, 1, 0, device=device, dtype=dtype)
            mod(inp)

        mod = smith.nn.Conv2d(in_channels, 33, 3, stride=2, dtype=dtype).to(device)
        inp = smith.randn(2, 0, 50, 100, device=device, dtype=dtype)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Given groups=1, weight"):
            inp = smith.randn(2, 1, 40, 0, device=device, dtype=dtype)
            mod(inp)

        mod = smith.nn.Conv3d(in_channels, 33, 3, stride=2, dtype=dtype).to(device)
        inp = smith.randn(2, 0, 50, 20, 40, device=device, dtype=dtype)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Given groups=1, weight"):
            inp = smith.randn(2, 1, 50, 0, 40, device=device, dtype=dtype)
            mod(inp)

    def test_group_conv_empty(self, device):
        mod = smith.nn.Conv2d(4, 4, stride=2, kernel_size=3, padding=1, groups=4).to(
            device
        )
        inp = smith.randn(0, 4, 4, 4, device=device)
        _test_module_empty_input(self, mod, inp, check_size=False)

    def test_group_convTranspose_empty(self, device):
        mod = smith.nn.ConvTranspose2d(
    ...
    def test_convTranspose_empty(self, device):
        mod = smith.nn.ConvTranspose2d(4, 4, stride=2, kernel_size=3, padding=1).to(
            device
        )
        inp = smith.randn(0, 4, 4, 4, device=device)
        _test_module_empty_input(self, mod, inp, check_size=False)
    ...
    def test_conv_large_nosplit(self, device):
        ...
        input_large = smith.randn(1, 2, 1024, 1024 * 1024, dtype=dtype, device=device)
        conv1(input_large)
        conv2 = smith.nn.Conv2d(1, 1024, 1, 1).to(device).to(dtype)
        input_large = smith.randn(1, 1, 2048, 1024, dtype=dtype, device=device)
        conv2(input_large)
    ...
    def test_conv_noncontig_weights(self, device):
        for dim in (1, 2, 3):
            for grouped in (False, True):
                ...
                w = smith.randn([3] * dim, device=device)
    ...
    def test_conv_noncontig_weights_and_bias(self, device):
        for bias in [True, False]:
            ...
            input_nc = smith.randn(
                (1, 3, 224, 224, 2), device=device, dtype=smith.float
            )[:, :, :, :, 1]
            input_c = input_nc.contiguous()
            weight_nc = smith.randn((64, 3, 7, 7, 2), device=device, dtype=smith.float)[
                :, :, :, :, 1
            ]
            conv1.weight = nn.Parameter(weight_nc)
            weight_c = conv1.weight.contiguous()
            if bias:
                bias_nc = smith.randn((64, 2), device=device, dtype=smith.float)[:, 1]
    ...
    def test_conv_transposed_large(self, device):
        dtype = smith.half if self.device_type == "cuda" else smith.float
        conv = nn.ConvTranspose2d(1, 1, 1, 1, bias=False).to(device).to(dtype)
        input_large = smith.randn(4096, 1, 512, 1024, dtype=dtype, device=device)
    ...
    def test_conv_large(self, device):
        ...
        input_large = smith.randn(4097, 2, 512, 512, dtype=dtype, device=device)
        ret = conv(input_large)
    ...
    def test_Conv2d_size_1_kernel(self, device):
        x_cpu = smith.randn(2, 3, 5, 5)
        conv_cpu = smith.nn.Conv2d(3, 3, kernel_size=1)
        y_cpu = conv_cpu(x_cpu)
        y = smith.rand_like(y_cpu)
        y_cpu.backward(y)

        with cudnn.flags(enabled=False):
            conv_cuda = smith.nn.Conv2d(3, 3, kernel_size=1).to(device)
    ...
    def test_ConvTranspose2d_size_1_kernel(self, device):
        x_cpu = smith.randn(2, 3, 5, 5)
        conv_cpu = smith.nn.ConvTranspose2d(3, 3, kernel_size=1)
        y_cpu = conv_cpu(x_cpu)
        y = smith.rand_like(y_cpu)
        y_cpu.backward(y)
        conv_cuda = smith.nn.ConvTranspose2d(3, 3, kernel_size=1).to(device)
        conv_cuda.bias.data.copy_(conv_cpu.bias.data)
        conv_cuda.weight.data.copy_(conv_cpu.weight.data)
        y_cuda = conv_cuda(x_cpu.to(device))
        y_cuda.backward(y.to(device))
    ...
    def test_ConvTranspose3d_size_1_kernel(self, device):
        with set_default_dtype(smith.double):
            x_cpu = smith.randn(2, 3, 3, 5, 5)
            conv_cpu = smith.nn.ConvTranspose3d(3, 3, kernel_size=1)
            y_cpu = conv_cpu(x_cpu)
            y = smith.rand_like(y_cpu)
            y_cpu.backward(y)
            conv_cuda = smith.nn.ConvTranspose3d(3, 3, kernel_size=1).to(device)
            conv_cuda.bias.data.copy_(conv_cpu.bias.data)
            conv_cuda.weight.data.copy_(conv_cpu.weight.data)
            y_cuda = conv_cuda(x_cpu.to(device))
            y_cuda.backward(y.to(device))
    ...
    def test_Conv2d_naive_groups(self, device, dtype):
        ...
        i = smith.randn(2, 4, 6, 6, device=device, dtype=dtype, requires_grad=True)
        output = m(i)
        grad_output = smith.randn(2, 4, 4, 4, device=device, dtype=dtype)
        output.backward(grad_output)
        ...
        self.assertEqual(output, smith.cat([output1, output2], 1))
        self.assertEqual(
            i.grad.data,
            smith.cat([i1.grad.data, i2.grad.data], 1),
            atol=dtype2prec_DONTUSE[dtype],
    ...
    def test_Conv2d_backward_depthwise(self, device, dtype):
        x = smith.randn(2, 2, 4, 20, device=device, dtype=dtype, requires_grad=True)
        weight = smith.randn(2, 1, 3, 5, device=device, dtype=dtype, requires_grad=True)
        ...
        smith.autograd.gradcheck(conv2d_depthwise, (x, weight))

    @dtypes(smith.half, smith.float)
    def test_conv_cudnn_nhwc(self, device, dtype):
        def helper(n, c, h, w, out_channels, kernel_size, groups):
            input = smith.randint(-3, 3, (n, c, h, w), dtype=dtype, device=device).to(
                memory_format=smith.channels_last
            )
            input.requires_grad_()
            conv = nn.Conv2d(c, out_channels, kernel_size, groups=groups).to(
                device=device, dtype=dtype, memory_format=smith.channels_last
            )
            for p in conv.parameters():
                p.data = smith.randint_like(p, -3, 3)

            ref_input = input.detach().clone().contiguous().double().requires_grad_()
            ref_conv = nn.Conv2d(c, out_channels, kernel_size, groups=groups)
            ref_conv.load_state_dict(conv.state_dict())
            ref_conv = ref_conv.to(
                device=device, dtype=smith.double, memory_format=smith.contiguous_format
            )

            out = conv(input)
            ref_out = ref_conv(ref_input)

            grad = smith.randint_like(out, -3, 3)
            ...
            self.assertTrue(input.grad.is_contiguous(memory_format=smith.channels_last))
            self.assertTrue(
                conv.weight.grad.is_contiguous(memory_format=smith.channels_last)
            )
    ...
    def test_conv_cudnn_ndhwc(self, device, dtype):
        def helper(n, c, d, h, w, out_channels, kernel_size, groups):
            ...
            ).to(memory_format=smith.channels_last_3d)
            input.requires_grad_()
            conv = nn.Conv3d(c, out_channels, kernel_size, groups=groups).to(
                device=device, dtype=dtype, memory_format=smith.channels_last_3d
            )
            for p in conv.parameters():
                p.data = smith.randint_like(p, -2, 2)
            ...
            grad = smith.randint_like(out, -2, 2)
            ref_grad = grad.detach().clone().double().contiguous()
            ...
            self.assertTrue(
                input.grad.is_contiguous(memory_format=smith.channels_last_3d)
            )
            self.assertTrue(
                conv.weight.grad.is_contiguous(memory_format=smith.channels_last_3d)
            )
    ...
    def _test_conv_cudnn_nhwc_nchw(self, layer, n, c, h, w, k, filter_size, device):
        data = smith.randint(1, 10, (n, c, h, w), dtype=smith.float32, device=device)
        ref_input = data.clone().contiguous().requires_grad_(True)
        ref_conv = layer(c, k, filter_size).float().to(device)
        ref_out = ref_conv(ref_input)
        grad = smith.randint(1, 10, ref_out.size(), dtype=smith.float32, device=device)
        ref_out.backward(grad)

        for w_f in [smith.contiguous_format, smith.channels_last]:
            for g_f in [smith.contiguous_format, smith.channels_last]:
                for input_format in [smith.contiguous_format, smith.channels_last]:
                    output_format = smith.contiguous_format
                    if input_format == smith.channels_last:
                        output_format = smith.channels_last
                    if w_f == smith.channels_last:
                        output_format = smith.channels_last
                    self._run_conv(
                        layer,
                        device,
    ...
    def test_conv_cudnn_nhwc_support(self, device, dtype):
        ...
        weight = weight.to(memory_format=smith.channels_last)
        o = smith.conv2d(input, weight, None, (2, 1), (1, 1), (1, 1), 1)
        self.assertTrue(o.is_contiguous(memory_format=smith.channels_last))
        o.sum().backward()
    ...
    def test_conv2d_no_grad(self, device, dtype):
        for batch in [1, 2, 3]:
            for groups in [1, 2, 4]:
                input = smith.rand(batch, groups, 8, 8, dtype=dtype, device=device)
                m = nn.Conv2d(
                    groups,
                    8,
                    kernel_size=(3, 3),
    ...
    @unittest.skipIf(smith.xpu.device_count() < 2, "only one GPU detected")
    @dtypes(smith.double, smith.float, smith.half)
    def test_conv2d_on_multi_device(self, dtype):
        input = smith.randn(3, 256, 224, 224, dtype=dtype, requires_grad=True)
        conv = smith.nn.Conv2d(256, 256, kernel_size=3, padding=1, dtype=dtype)
        output_grad = smith.randn(3, 256, 224, 224, dtype=dtype)
    ...
    def test_conv_double_backward_strided_with_3D_input_and_weight(self, device):
        input = smith.randn(2, 3, 6, device=device)
        weight = smith.randn(3, 3, 3, device=device)
        ...
        output_padding = (0,)
        groups = 1
        output = smith.ops.aten.convolution(
            input,
            weight,
            bias,
            stride,
        ...
        ggI = smith.randn(input.shape, device=device)
        ggW = smith.randn(weight.shape, device=device)
        ggB = smith.randn(bias.shape, device=device)
        gO = smith.randn(output.shape, device=device)
        output_mask = [True, True, True]
        (
            grad_grad_output,
            grad_input,
            grad_weight,
        ) = smith.ops.aten._convolution_double_backward(
            ggI,
            ggW,
            ggB,
            gO,
    ...
    def test_aten_conv_backward_op(self, device):
        ...
        def test_aten_conv_backward_op_output_mask(output_mask):
            input_cpu = smith.randn(1, 1, 4, 4, device="cpu")
            weight_cpu = smith.randn(1, 1, 3, 3, device="cpu")
            grad_output_cpu = smith.randn(1, 1, 2, 2, device="cpu")
            ...
            grad_input, grad_weight, grad_bias = smith.ops.aten.convolution_backward(
                grad_output,
                input,
                weight,
                [1],
    ...
    def test_channels_last_ouput_stride(self, device, dtype):
        ...
        weight = weight.to(memory_format=smith.channels_last)
        out = smith.conv2d(input, weight, None, (2, 2), (0, 0), (1, 1), 1)
    ...
    def test_onednn_allow_tf32_get_set(self):
        with smith.backends.mkldnn.flags(
            ...
            self.assertFalse(smith.backends.mkldnn.allow_tf32)
        with smith.backends.mkldnn.flags(
            enabled=None, deterministic=None, allow_tf32=True
        ):
            self.assertTrue(smith.backends.mkldnn.allow_tf32)
