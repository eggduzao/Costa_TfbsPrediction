...
# a Python `smith.nn` module and its corresponding C++ `smith::nn` module. Concretely,
...
from smith.jit._pickle import restore_type_tag


# Expected substitutions:
#
# ${module_variant_name}  (e.g. `Linear_no_bias_cpu`)
# ${module_qualified_name}  (e.g. `smith::nn::Linear`)
# ${cpp_args_construction_stmts}
# ${cpp_constructor_args}
# ${device}
# ${cpp_forward_args_symbols}
SMITH_NN_MODULE_TEST_FORWARD_BACKWARD = Template(
    """
void ${module_variant_name}_test_forward_backward(
  ...
  smith::load(module, module_file_path);
  ...
  write_ivalue_to_file(smith::IValue(cpp_output), forward_output_file_path);
  ...
  c10::Dict<std::string, smith::Tensor> grad_dict;
  for (const auto& param : module->named_parameters()) {
    smith::Tensor grad = param.value().grad();
    if (grad.is_sparse()) {
  ...
  write_ivalue_to_file(smith::IValue(grad_dict), backward_grad_dict_file_path);
}
"""
)
...
def run_python_forward_backward(unit_test_class, test_params):
    ...
    script_module = smith.jit.trace(module, smith.tensor(0))
...
def test_forward_backward(unit_test_class, test_params):
    ...
    def run_cpp_test_fn_and_check_output():
        ...
        cpp_output = smith.load(forward_output_file_path)
        # weights_only: need GLOBAL smith.jit._pickle.restore_type_tag
        with smith.serialization.safe_globals([restore_type_tag]):
            cpp_grad_dict = smith.load(backward_grad_dict_file_path)
...
def process_test_params_for_module(test_params_dict, device, test_instance_class):
    ...
    test_params_dict["constructor"] = test_params_dict.get(
        "constructor", getattr(smith.nn, module_name)
    )
    ...
    return SmithNNModuleTestParams(
...
def write_test_to_test_class(
    ...
    assert not is_smith_nn_functional_test(test_params_dict)

    module_name = compute_module_name(test_params_dict)

    assert hasattr(smith.nn, module_name), (
        f"`smith.nn` doesn't have module `{module_name}`. "
    ...
    module_full_name = "smith::nn::" + module_name

    assert module_full_name in parity_table["smith::nn"], (
        f"Please add `{module_full_name}` entry to `smith::nn` section of `test/cpp_api_parity/parity-tracker.md`. "
    ...
    for device in devices:
        ...
        unit_test_name = f"test_smith_nn_{test_params.module_variant_name}"
        ...
        test_fn = decorate_test_fn(
            ...
            has_impl_parity=parity_table["smith::nn"][module_full_name][0]
...
def generate_test_cpp_sources(test_params, template):
    ...
    test_cpp_sources = template.substitute(
        module_variant_name=test_params.module_variant_name,
        module_qualified_name=f"smith::nn::{test_params.module_name}",
...
def build_cpp_tests(unit_test_class, print_cpp_source=False):
    ...
    cpp_sources = SMITH_NN_COMMON_TEST_HARNESS + SAMPLE_MODULE_CPP_SOURCE
    functions = []
    for test_params in unit_test_class.module_test_params_map.values():
        cpp_sources += generate_test_cpp_sources(
            test_params=test_params, template=SMITH_NN_MODULE_TEST_FORWARD_BACKWARD
        )
