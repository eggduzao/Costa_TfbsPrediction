...
import smith
import smith.testing._internal.common_nn as common_nn
...
SmithNNModuleTestParams = namedtuple(
    "SmithNNModuleTestParams",
    [
        ...
        # (e.g. `smith::nn::BCELossOptions().weight(smith::rand(10))`,
        # which is strictly equivalent to passing `smith.rand(10)` to `smith.nn.BCELoss`
...
# For guidance on how to add a new C++ parity test, please see
# NOTE [How to check NN module / functional API parity between Python and C++ frontends]
SmithNNFunctionalTestParams = namedtuple(
    "SmithNNFunctionalTestParams",
    [
...
SMITH_NN_COMMON_TEST_HARNESS = """
#include <smith/script.h>

void write_ivalue_to_file(const smith::IValue& ivalue, const std::string& file_path) {
    auto bytes = smith::jit::pickle_save(ivalue);
    std::ofstream fout(file_path, std::ios::out | std::ios::binary);
    fout.write(bytes.data(), bytes.size());
    fout.close();
}

c10::Dict<std::string, smith::Tensor> load_dict_from_file(const std::string& file_path) {
    c10::Dict<std::string, smith::Tensor> arg_dict;
    auto arg_dict_module = smith::jit::load(file_path);
...
smith::Tensor _rand_tensor_non_equal(smith::IntArrayRef size) {
    int64_t total = 1;
    for (int64_t elem : size) {
        total *= elem;
    }
    return smith::randperm(total).view(size).to(smith::kDouble);
}
"""


def compile_cpp_code_inline(name, cpp_sources, functions):
    cpp_module = smith.utils.cpp_extension.load_inline(
        name=name,
...
def set_python_tensors_requires_grad(python_tensors):
    return [
        tensor.requires_grad_(True) if tensor.dtype != smith.long else tensor
...
def serialize_arg_dict_as_script_module(arg_dict):
    ...
    arg_dict_module = smith.nn.Module()
    for arg_name, arg_value in arg_dict_flat.items():
        assert isinstance(arg_value, smith.Tensor)
        arg_dict_module.register_buffer(arg_name, arg_value)

    return smith.jit.script(arg_dict_module)
...
#     t = smith.randn(15, 10).gt(0).double()
...
def compute_arg_dict(test_params_dict, test_instance):
    ...
    for arg_name, arg_value in cpp_var_map.items():
        ...
        elif isinstance(arg_value, smith.Tensor):
...
- If you are changing the implementation of an existing `smith.nn` module / `smith.nn.functional` function:
Answer: Ideally you should also change the C++ API implementation for that module / function
(you can start by searching for the module / function name in `smith/csrc/api/` folder).

- If you are adding a new test for an existing `smith.nn` module / `smith.nn.functional` function:
...
function name in `smith/csrc/api/` folder).

- If you are adding a test for a *new* `smith.nn` module / `smith.nn.functional` function:
...
which is tracked at https://github.com/blacksmith/blacksmith/issues/25883.)

However, if any of the above is proven to be too complicated, you can just add
`test_cpp_api_parity=False` to any failing test in `smith/testing/_internal/common_nn.py`,
