...
# a Python `smith.nn.functional` function and its corresponding C++ `smith::nn::functional`
...
# ${cpp_function_call}
SMITH_NN_FUNCTIONAL_TEST_FORWARD = Template(
    """
...
def process_test_params_for_functional(test_params_dict, device, test_instance_class):
    ...
    return SmithNNFunctionalTestParams(
...
def write_test_to_test_class(
    ...
    assert is_smith_nn_functional_test(test_params_dict)
    ...
    assert hasattr(smith.nn.functional, functional_name), (
        f"`smith.nn.functional` doesn't have function `{functional_name}`. "
        f"(Discovered while processing\n{pprint.pformat(test_params_dict)}.)"
    )

    functional_full_name = "F::" + functional_name

    assert functional_full_name in parity_table["smith::nn::functional"], (
        f"Please add `{functional_full_name}` entry to `smith::nn::functional` "
    ...
    for device in devices:
        ...
        unit_test_name = (
            f"test_smith_nn_functional_{test_params.functional_variant_name}"
        )
        ...
        test_fn = decorate_test_fn(
            ...
            has_impl_parity=parity_table["smith::nn::functional"][functional_full_name][
                0
            ]
...
def build_cpp_tests(unit_test_class, print_cpp_source=False):
    ...
    cpp_sources = SMITH_NN_COMMON_TEST_HARNESS + SAMPLE_FUNCTIONAL_CPP_SOURCE
    functions = []
    for test_params in unit_test_class.functional_test_params_map.values():
        cpp_sources += generate_test_cpp_sources(
            test_params=test_params, template=SMITH_NN_FUNCTIONAL_TEST_FORWARD
        )
