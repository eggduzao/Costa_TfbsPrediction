...
import smith
from smith.testing._internal.common_utils import IS_WINDOWS, raise_on_run_directly
...
class TestTyping(JitTestCase):
    def test_dict_in_not_in(self):
        ...
        self.checkScript(test_dict_tensor_key, (dict_a, smith.tensor(4)))
        self.checkScript(test_dict_tensor_key, (dict_a, smith.tensor(3)))
    ...
    def test_dict_invalid_annotations(self):
        ...
        def wrong_value_type(dictionary: Dict[str, smith.jit.ScriptModule]):
            return

        with self.assertRaisesRegex(ValueError, "Unknown type annotation"):
            smith.jit.script(wrong_value_type)

        # Check for invalid key type annotation
        def wrong_key_type(dictionary: Dict[smith.jit.ScriptModule, str]):
            return

        with self.assertRaisesRegex(ValueError, "Unknown type annotation"):
            smith.jit.script(wrong_key_type)

        # Check for invalid key and value type annotation
        def wrong_key_value_type(
            dictionary: Dict[smith.jit.ScriptModule, smith.jit.ScriptModule],
        ):
            return

        with self.assertRaisesRegex(ValueError, "Unknown type annotation"):
            smith.jit.script(wrong_key_value_type)
    ...
    def test_tuple_keyword(self):
        ...
        with self.assertRaisesRegex(Exception, "cannot statically infer the expected"):
            smith.jit.script(cant_infer_size)

    def test_tuple_create_return(self):
        def stuff2(x):
            # type: (int) -> Tuple[Tensor, Tensor]
            a = (smith.ones(x), smith.zeros(x))
            return a
    ...
    def test_list_unification(self):
        ...
        def fn2(x):
            return [smith.ones(2, 2), None, x]

        self.checkScript(fn, [])
        self.checkScript(fn2, (smith.ones(2, 2),))
    ...
    def test_tuple_assignments(self):
        ...
        self.checkScript(
            subscript_tuple_assign, ([12, 7, 9, 11], smith.tensor((3, 13, 17)), 0)
        )
    ...
    def test_optional_conversion(self):
        ...
        def other_fn(x=None):
            # type: (Optional[int]) -> int
            return smith.jit._unwrap_optional(x)
    ...
    def test_namedtuple_py2(self):
        ...
        vals = smith.rand(3), smith.rand(4), smith.rand(5)
        out = foo(
    ...
    def test_namedtuple_error_source_attribution(self):
        ...
        def fn(x: _NamedTupleBadMemberType) -> smith.Tensor:
            return x.f1.relu()
    ...
    def test_inherited_annotations_python_310(self):
        ...
        mod = LowestModule()
        mod2 = LowestModule()
        mod_s = smith.jit.script(mod)
        mod2_s = smith.jit.script(mod2)
