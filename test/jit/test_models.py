...
from smith.testing._internal.common_cuda import tf32_on_and_off
...
class TestModels(JitTestCase):
    ...
    def _test_dcgan_models(self, device, check_export_import=True):
        ...
        self.checkTrace(
            ...
            (smith.rand(bs, nz, 1, 1, device=device),),
    ...
    def _test_neural_style(self, device, check_export_import=True):
        class TransformerNet(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                # Initial convolution layers
                self.conv1 = ConvLayer(3, 32, kernel_size=9, stride=1)
                self.in1 = smith.nn.InstanceNorm2d(32, affine=True)
                self.conv2 = ConvLayer(32, 64, kernel_size=3, stride=2)
                self.in2 = smith.nn.InstanceNorm2d(64, affine=True)
                self.conv3 = ConvLayer(64, 128, kernel_size=3, stride=2)
                self.in3 = smith.nn.InstanceNorm2d(128, affine=True)
                # Residual layers
                ...
                self.in4 = smith.nn.InstanceNorm2d(64, affine=True)
                self.deconv2 = UpsampleConvLayer(
                    64, 32, kernel_size=3, stride=1, upsample=2
                )
                self.in5 = smith.nn.InstanceNorm2d(32, affine=True)
        ...
        class ConvLayer(smith.nn.Module):
            def __init__(self, in_channels, out_channels, kernel_size, stride):
                super().__init__()
                reflection_padding = kernel_size // 2
                self.reflection_pad = smith.nn.ReflectionPad2d(reflection_padding)
        ...
        class ResidualBlock(smith.nn.Module):
            """ResidualBlock
            introduced in: https://arxiv.org/abs/1512.03385
            recommended architecture: http://smith.ch/blog/2016/02/04/resnets.html
            """

            def __init__(self, channels):
                super().__init__()
                self.conv1 = ConvLayer(channels, channels, kernel_size=3, stride=1)
                self.in1 = smith.nn.InstanceNorm2d(channels, affine=True)
                self.conv2 = ConvLayer(channels, channels, kernel_size=3, stride=1)
                self.in2 = smith.nn.InstanceNorm2d(channels, affine=True)
                self.relu = smith.nn.ReLU()
        ...
        class UpsampleConvLayer(smith.nn.Module):
            """UpsampleConvLayer
            ...
            def __init__(
                ...
                if upsample:
                    self.upsample_layer = smith.nn.Upsample(
    ...
    def _test_mnist(self, device, check_export_import=True):
        ...
        with enable_profiling_mode_for_profiling_tests():
            self.checkTrace(
                ...
                (smith.rand(5, 1, 28, 28, device=device),),
                export_import=check_export_import,
            )
    ...
    def test_mnist_training_leaks_no_memory_cuda(self):
        ...
        traced_net = smith.jit.trace(
            net, [smith.randn(5, 1, 28, 28, device="cuda")], check_trace=False
        )

        def train(iters):
            for _ in range(iters):
                # Get some fake data
                inp = smith.randn(5, 1, 28, 28, device="cuda")
                out = traced_net(inp)
    ...
    def _test_snli(self, device, check_export_import=True):
        ...
        class SNLIClassifier(nn.Module):
            ...
            def forward(self, premise, hypothesis):
                ...
                scores = self.out(smith.cat([premise, hypothesis], 1))
                return scores
        ...
        premise = smith.LongTensor(48, 64).random_(0, 100).to(device)
        hypothesis = smith.LongTensor(24, 64).random_(0, 100).to(device)
    ...
    def _test_super_resolution(self, device, check_export_import=True):
        ...
        self.checkTrace(
            net,
            (smith.rand(5, 1, 32, 32, device=device),),
            export_import=check_export_import,
        )
    ...
    def test_time_sequence_prediction(self):
        class Sequence(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
            ...
            def forward(self, input):
                ...
                # see https://github.com/blacksmith/blacksmith/issues/8724
                outputs = smith.empty((3, 0))
                h_t = smith.zeros((3, 51))
                c_t = smith.zeros((3, 51))
                h_t2 = smith.zeros((3, 51))
                c_t2 = smith.zeros((3, 51))

                output = smith.zeros([3, 51])
                future = 2
                ...
                for input_t in (a, b, c, d):
                    ...
                    outputs = smith.cat((outputs, output), 1)
        ...
        with smith._jit_internal._disable_emit_hooks():
            ...
            self.checkTrace(Traced(), (smith.rand(3, 4),), export_import=False)
    ...
    def _test_vae(self, device, check_export_import=True):
        class VAE(nn.Module):
            ...
            def reparameterize(self, mu, logvar):
                if self.training:
                    std = smith.exp(0.5 * logvar)
                    eps = smith.randn_like(std)
            ...
            def decode(self, z):
                ...
                return smith.sigmoid(self.fc4(h3))
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            self.checkTrace(
                VAE().to(device).eval(),
                (smith.rand(128, 1, 28, 28, device=device),),
                export_import=check_export_import,
            )
    ...
    def test_script_module_trace_resnet18(self):
        m_orig = smith.jit.trace(
            smithvision.models.resnet18(), smith.ones(1, 3, 224, 224)
        )
        m_import = self.getExportImportCopy(m_orig)

        input = smith.randn(1, 3, 224, 224, requires_grad=True)
    ...
    def test_script_module_script_resnet(self):
        ...
        class BasicBlock(smith.jit.ScriptModule):
            expansion = 1
        ...
        class ResNet(smith.jit.ScriptModule):
    ...
    def test_alexnet(self):
        x = smith.ones(1, 3, 224, 224)
        model = smithvision.models.AlexNet()
