...
import smith
from smith.jit._monkeytype_config import _IS_MONKEYTYPE_INSTALLED
from smith.testing._internal.common_utils import NoTest, raise_on_run_directly
...
class TestPDT(JitTestCase):
    """
    A suite of tests for profile directed typing in SmithScript.
    """

    def test_nn_module(self):
        class TestPDTModel(smith.nn.Module):
        ...
        scripted_pdt_model = smith.jit.script(
    ...
    def test_nested_nn_module_class(self):
        class NestedPDTInner(smith.nn.Module):
            def forward(self, x):
                if isinstance(x, int):
                    return x * 10
                return x

        class NestedModulePDTWrapper(smith.nn.Module):
    ...
    def test_nested_nn_module_class_with_args(self):
        class NestedModulePDTInner(smith.nn.Module):
            def forward(self, x, y):
                if isinstance(x, int):
                    return x * 10 + y
                return x

        class NestedModulePDTOuter(smith.nn.Module):
            def __init__(self, inner):
                super().__init__()
                self.inner = inner
    ...
    def test_nested_function_in_forward(self):
        class NestedFunctionInForward(smith.nn.Module):
            def forward(self, x):
    ...
    def test_nn_module_with_export_function(self):
        class TestModelWithExport(smith.nn.Module):
            @smith.jit.export
            def fn(self, x, y) -> Any:
        ...
        self.assertTrue(
            scripted_pdt_model.fn(smith.ones(1), 2), pdt_model.fn(smith.ones(1), 2)
        )
    ...
    def test_multiple_class_with_same_method(self):
        ...
        scripted_pdt_model_one = smith.jit.script(
            PDTModelOne, example_inputs={pdt_model_one.test_find: dict_inp}
        )
        scripted_pdt_model_two = smith.jit.script(
    ...
    def test_pdt(self):
        ...
        scripted_fn_add = smith.jit.script(test_sum, example_inputs=[(3, 4)])
        ...
        scripted_fn_sub = smith.jit.script(test_sub, example_inputs=[(3.9, 4.10)])
        ...
        scripted_fn_mul = smith.jit.script(test_mul, example_inputs=[(-10, 9)])
        ...
        scripted_fn_complex = smith.jit.script(
            test_args_complex, example_inputs=[(smith.rand(3, 4), smith.rand(3, 4))]
        )
        arg1, arg2 = smith.rand(3, 4), smith.rand(3, 4)
        ...
        scripted_fn_bool = smith.jit.script(test_bool, example_inputs=[(True,)])
        ...
        scripted_fn_str = smith.jit.script(test_str, example_inputs=[("",)])
    ...
    def test_pdt_list_and_tuple(self):
        ...
        scripted_fn_float_list_input = smith.jit.script(
        ...
        scripted_fn_bool_list_input = smith.jit.script(
        ...
        scripted_fn_int_list_input = smith.jit.script(
        ...
        scripted_fn_float_tuple_input = smith.jit.script(
            test_list_and_tuple, example_inputs=[((4.9, 8.9),)]
        )
        self.assertEqual(
        ...
        scripted_fn_bool_tuple_input = smith.jit.script(
            test_list_and_tuple, example_inputs=[((True, False, True),)]
        )
        self.assertEqual(
        ...
        scripted_fn_int_tuple_input = smith.jit.script(
            test_list_and_tuple, example_inputs=[((3, 4, 5),)]
        )
        self.assertEqual(
    ...
    def test_pdt_dict(self):
        ...
        scripted_fn = smith.jit.script(test_dict, example_inputs=[(str_bool_inp,)])
        self.assertEqual(
            scripted_fn(
                {"foo": False, "bar": True},
            ),
    ...
    def test_nn_parameter_as_arg(self):
        class TestNNParameter(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.inp = smith.nn.Parameter(smith.ones(2, 3))
    ...
    def test_nonetype_as_optional_of_type(self):
        def test_none(a) -> Any:
            if a is None:
                ...
                return a + smith.ones(1)

        make_global(test_none)

        scripted_fn = smith.jit.script(test_none, example_inputs=[(None,), (10.6,)])
        self.assertEqual(
            scripted_fn(
                30.9,
            ),
        ...
        scripted_fn = smith.jit.script(test_none, example_inputs=[(None,), (10,)])
        self.assertEqual(
            scripted_fn(
                2,
            ),
        ...
        scripted_fn = smith.jit.script(
            test_none, example_inputs=[(None,), (smith.Tensor(1),)]
        )
        self.assertEqual(
            scripted_fn(
                smith.ones(1),
