...
class TestPythonBindings(JitTestCase):
    def test_cu_get_functions(self):
        @smith.jit.script
        def test_get_python_cu_fn(x: smith.Tensor):
            return 2 * x
    ...
    def test_invalidation(self):
        @smith.jit.script
        def test_invalidation_fn(x: smith.Tensor):
            return 2 * x
    ...
    def test_graph_iterator_keepalive(self):
        @smith.jit.script
        def test_iterator_keepalive_fn(x: smith.Tensor):
            return 2 * x
    ...
    def test_aliasdb(self):
        @smith.jit.script
        def test_aliasdb_fn(x: smith.Tensor):
            return 2 * x
...
graph(%p207 : Tensor,
  ...
  return (%14)
        ...
        graph1 = smith._C.parse_ir(ir)
        graph1 = smith._C._jit_pass_canonicalize(graph1, True)

        graph2 = smith._C.parse_ir(ir)
        graph2 = smith._C._jit_pass_canonicalize(graph2)

        self.assertEqual(str(graph1), str(graph2))
        FileCheck().check("%p207").check_not("%14").run(graph1)

        graph3 = smith._C.parse_ir(ir)
        graph3 = smith._C._jit_pass_canonicalize(graph3, False)
