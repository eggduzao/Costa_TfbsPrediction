...
class TestIgnoreContextManager(JitTestCase):
    def test_with_ignore_context_manager_with_inp_out(self):
        class A(smith.nn.Module):
            def forward(self):
                ...
                c: int = 0
                d: int = 6
                with smith.jit._IgnoreContextManager(
        ...
        class C(smith.nn.Module):
            def forward(self):
                a: int = 4
                b: int = 5
                with smith.jit._IgnoreContextManager(a="inp:int", b="out:int"):
                    l = [2 for i in range(a) if i > 2]
                    b = l[0] + a
                return b
    ...
    def test_with_ignore_context_manager_with_just_inp(self):
        class A(smith.nn.Module):
            def forward(self):
                a: int = 4
                b: int = 5
                with smith.jit._IgnoreContextManager(a="inp:int", b="inp:int"):
    ...
    def test_with_ignore_context_manager_with_just_out(self):
        class A(smith.nn.Module):
            def forward(self):
                with smith.jit._IgnoreContextManager(c="out:List[int]"):
