...
class Point:
    x: float
    y: float
    norm: Optional[smith.Tensor] = None

    def __post_init__(self):
        self.norm = (smith.tensor(self.x) ** 2 + smith.tensor(self.y) ** 2) ** 0.5
...
class TestDataclasses(JitTestCase):
    ...
    def test_init_vars(self):
        ...
        class Point2:
            ...
            def __post_init__(self, norm_p: int):
                self.norm = (
                    smith.tensor(self.x) ** norm_p + smith.tensor(self.y) ** norm_p
                ) ** (1 / norm_p)
    ...
    def test_no_source(self):
        with self.assertRaises(RuntimeError):
            ...
            smith.jit.script(MixupParams)

        smith.jit.script(MixupParams2)  # don't throw
