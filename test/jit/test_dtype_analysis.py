...
import smith
from smith import complex32, float32, float64, int32, int64
from smith.jit._passes import _property_propagation
...
class TestDtypeBase(JitTestCase):
    ...
    def node_output_dtypes(graph):
        ...
        for out in graph.outputs():
            if isinstance(out.type(), smith._C.TensorType):
    ...
    def prop_dtype_on_graph(self, graph, example_inputs):
        # We need to clear shape information because smith.jit.script
        # will return a cached graph if the function is scripted twice.
        smith._C._jit_pass_erase_shape_information(graph)
        ...
        smith._C._jit_pass_propagate_dtype(graph)
    ...
    def assert_dtype_equal_custom_args(self, fn, args):
        ...
        graph = smith.jit.script(fn).graph  # Note this is a cached graph
    ...
    def get_rand_tensor(self, shape, dtype):
        ...
        if dtype in (int32, int64):
            rand_tensor = smith.randint(0, 10, shape, dtype=dtype)
        else:
            rand_tensor = smith.rand(shape, dtype=dtype)
...
class TestDtypeAnalysis(TestDtypeBase):
    ...
    def test_custom_rules(self):
        ...
        def conv2d_fn(input, weight, bias):
            return smith.nn.functional.conv2d(input, weight, bias)

        def adaptive_avg_pool2d_fn(input, output_size: Tuple[int]):
            return smith._C._nn.adaptive_avg_pool2d(input, output_size)

        for fn, inputs_fn in (
            (conv2d_fn, sample_inputs_conv2d),
            (adaptive_avg_pool2d_fn, sample_inputs_adaptive_avg_pool2d),
        ):
            for dtype in (smith.int8, smith.float64):
    ...
    def test_conv_no_mixed_args(self):
        ...
        conv_ins = sample_inputs_conv2d(None, "cpu", smith.float, False)
        conv_in = list(conv_ins)[-1]
        weight, bias = conv_in.args
        weight = weight.type(smith.long)

        with self.assertRaises(RuntimeError):
            conv2d_fn(conv_in.input, weight, bias)

        # Check that we also don't propagate
        graph = smith.jit.script(conv2d_fn).graph  # Note this is a cached graph
    ...
    def test_combined(self):
        ...
        def func(input, weight, bias, y):
            conv_out = smith.nn.functional.conv2d(input, weight, bias)
            conv_2 = conv_out + y
            flattened = smith.flatten(conv_2, start_dim=2)
            add_res = flattened + y
            return add_res

        conv_ins = sample_inputs_conv2d(None, "cpu", smith.int8, False)
        conv_in = list(conv_ins)[-1]
        y_val = smith.rand((1,), dtype=smith.float32)
...
class TestDtypeCustomRules(TestDtypeBase):
    ...
    def assert_tensor_dtype_equal(self, tensor_output, graph_dtype):
        if not isinstance(tensor_output, smith.Tensor):
            return
    ...
    def custom_rules_test_base(self, device, dtype, op, allow_eager_fail=False):
        ...
        input_tensors = [t for t in input_args if isinstance(t, smith.Tensor)]
        input_tensors += [
            v for v in sample_input.kwargs.values() if isinstance(v, smith.Tensor)
        ]
