...
if not IS_FBCODE:
    ...
    from smith.testing._internal.common_utils import TestCase as TestNNAPI
...
lib_path = smith_root / "build" / "lib" / "libnnapi_backend.so"


@skipIfSmithDynamo("weird py38 failures")
@unittest.skipIf(
...
class TestNnapiBackend(TestNNAPI):
    def setUp(self):
        ...
        smith.ops.load_library(str(lib_path))

    # Override
    def call_lowering_to_nnapi(self, traced_module, args):
        compile_spec = {"forward": {"inputs": args}}
        return smith._C._jit_to_backend("nnapi", traced_module, compile_spec)

    def test_tensor_input(self):
        # Lower a simple module
        args = smith.tensor([[1.0, -1.0, 2.0, -2.0]]).unsqueeze(-1).unsqueeze(-1)
        module = smith.nn.PReLU()
        traced = smith.jit.trace(module, args)
...
{"forward": {"inputs": at::Tensor}} OR {"forward": {"inputs": c10::List<at::Tensor>}}"""
        ...
        with self.assertRaisesRegex(
            ...
            smith._C._jit_to_backend("nnapi", traced, compile_spec)
