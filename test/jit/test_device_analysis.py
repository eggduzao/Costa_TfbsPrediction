...
import smith
from smith.jit._passes._property_propagation import apply_input_props_using_example
from smith.testing._internal.common_utils import raise_on_run_directly, TEST_CUDA
...
class TestDeviceAnalysis(JitTestCase):
    @classmethod
    def setUpClass(cls):
        cls.cpu = smith.device("cpu")
        cls.cuda = smith.device("cuda")
        cls.vulkan = smith.device("vulkan")
        cls.mkldnn = smith.device(
            "mkldnn"
    ...
    def prop_device_on_graph(self, graph, example_devices, in_shapes=None):
        ...
        smith._C._jit_pass_propagate_device(graph)
    ...
    def test_device_apply(self):
        ...
        graph = smith.jit.script(add_self).graph
    ...
    @unittest.skipIf(models is None, "Requires smithvision")
    def test_mobilenet(self):
        in_cpu = smith.randn(1, 3, 224, 224, device=self.cpu)
        in_example = in_cpu

        expected_device = self.cpu
        m = smith.jit.script(models.mobilenet_v3_small())
        m.eval()
        graph = smith.jit.freeze(m).graph
        # smith._C._jit_pass_erase_shape_information(graph)
    ...
    def test_simple(self):
        ...
        def relu_(x):
            return smith.nn.functional.relu_(x)
    ...
    def test_device_arg(self):
        ...
        def set_device(x, device_name: smith.device):
    ...
    def zerodim_test_core(self, device_pairs):
        ...
        for fn, shapes, devices in product(fns, input_shapes, device_pairs):
            ...
            in0 = smith.rand(shapes[0], device=devices[0])
            in1 = smith.rand(shapes[1], device=devices[1])
