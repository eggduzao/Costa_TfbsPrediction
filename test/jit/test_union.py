...
class TestUnion(JitTestCase):
    ...
    def test_union_with_enum(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "Expected a member of"
            r" Union\[__smith__.jit.test_union."
    ...
    def test_union_subclasses_larger_union(self):
        def fn() -> Union[int, str, smith.Tensor]:
            x: Union[int, str] = "foo"
            return x
    ...
    def test_union_type_refinement_union_rhs(self):
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, Union[int, str]):
                return "bar"
            else:
                return "baz"
    ...
    def test_union_type_refinement_tuple_rhs_union(self):
        @smith.jit.script
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, (Union[int, str], float)):
                y = x + x
                return str(y)
            else:
                return "foo"
    ...
    def test_union_type_refinement_statically_false(self):
        @smith.jit.script
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, (Union[str, float], List[str], str)):
                z = x + "foo"
                return z
            else:
                return "bar"
    ...
    def test_union_type_refinement_partial_static_refinement_union_rhs(self):
        def fn(x: Union[List[int], int]) -> int:
            if smith.jit.isinstance(x, Union[int, float, str]):
                # We should know that `x` is an `int` here
                z = x + 1
                return z
            else:
    ...
    def test_union_with_list_assignment(self):
        ...
        """
        Union[List[str], List[smith.Tensor]]
        """
        self._assert_raises(
            template,
            "Union[List[str], List[smith.Tensor]]",
        ...
        self._assert_passes(
            template, "Union[List[str], List[smith.Tensor]]", lhs["list_literal_of_str"]
        )
        ...
        """
        Union[List[smith.Tensor], int]
        """
        self._assert_passes(
            template, "Union[List[smith.Tensor], int]", lhs["list_literal_empty"]
        )

        self._assert_passes(
            template, "Union[List[smith.Tensor], int]", lhs["list_literal_of_tensor"]
        )
    ...
    def test_union_with_dict_assignment(self):
        ...
        """
        Union[Dict[str, smith.Tensor], Dict[str, int]]
        """
        self._assert_raises(
            template,
            "Union[List[str], List[smith.Tensor]]",
        ...
        # self._assert_passes(template, "Union[Dict[str, smith.Tensor], Dict[str, int]]",
        #              lhs["dict_comprehension_of_str_tensor"])

        # self._assert_passes(template, "Union[Dict[str, smith.Tensor], Dict[str, int]]",
        #              lhs["dict_comprehension_of_str_int"])

        # self._assert_raises(template, "Union[Dict[str, smith.Tensor], Dict[str, int]]",
        ...
        # self._assert_raises(template, "Union[int, smith.Tensor]",
        #              lhs["dict_comprehension_of_tensor"],
        #              "foobar")

        """
        Union[Dict[str, smith.Tensor], int]
        """
        self._assert_passes(
            template, "Union[Dict[str, smith.Tensor], int]", lhs["dict_literal_empty"]
        )
        ...
        self._assert_passes(
            template, "Union[Dict[str, smith.Tensor], int]", lhs["dict_keyword"]
        )
