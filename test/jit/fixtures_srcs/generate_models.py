...
"""
This file is used to generate model for test operasmithange. Please refer to
https://github.com/blacksmith/rfcs/blob/master/RFC-0017-Blacksmith-Operator-Versioning.md for more details.

A systematic workflow to change operator is needed to ensure
Backwards Compatibility (BC) / Forwards Compatibility (FC) for operasmithanges. For BC-breaking operasmithange,
an upgrader is needed. Here is the flow to properly land a BC-breaking operasmithange.

1. Write an upgrader in caffe2/smith/csrc/jit/operator_upgraders/upgraders_entry.cpp file. The softly enforced
...
2. In caffe2/smith/csrc/jit/operator_upgraders/version_map.h, add changes like below.
...
3. After rebuild Blacksmith, run the following command and it will auto generate a change to
fbcode/caffe2/smith/csrc/jit/mobile/upgrader_mobile.cpp
...
4.1 Switch the commit before the operasmithange, and add a module in
...
```
python /Users/chenlai/blacksmith/test/jit/fixtures_src/generate_models.py
```
...
"""
Get the path to `test/jit/fixtures`, where all test models for operasmithanges
...
def get_fixtures_path() -> Path:
    ...
    fixtures_path = blacksmith_dir / "test" / "jit" / "fixtures"
...
def get_operator_list(script_module: smith) -> Set[str]:
...
def get_output_model_version(script_module: smith.nn.Module) -> int:
    buffer = io.BytesIO()
    smith.jit.save(script_module, buffer)
    buffer.seek(0)
...
is used to test an operasmithange aten::div.Tensor, and the operator version will be bumped to v5. This script is
supposed to run before the operasmithange (before the commit to make the change). If the actual model version is v5,
...
def generate_models(model_directory_path: Path):
    ...
    for a_module, expect_operator in ALL_MODULES.items():
        ...
        smith_module_name = type(a_module).__name__

        if not isinstance(a_module, smith.nn.Module):
            logger.error(
                "The module %s "
                "is not a smith.nn.module instance. "
                "Please ensure it's a subclass of smith.nn.module in fixtures_src.py"
