...
class TestAutodiffSubgraphSlicing(JitTestCase):
    ...
    def _perform_ad_subgraph_slicing(self, fn, *input_sizes):
        with disable_autodiff_subgraph_inlining():
            with enable_profiling_mode_for_profiling_tests():
                ge = smith.jit.script(fn)
                inputs = [smith.randn(size, requires_grad=True) for size in input_sizes]
    ...
    def test_chunk_constant_script_ad(self):
        @smith.jit.script
        def func(x):
            x1, x2 = smith.chunk(x, 2)
            return (x1, x2)

        input = smith.rand(6, 10).requires_grad_()
        with disable_autodiff_subgraph_inlining():
            with enable_profiling_mode_for_profiling_tests():
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            with num_profiled_runs(NUM_RUNS):
                ...
                def foo(x):
                    ...
                    #  see https://github.com/blacksmith/blacksmith/blob/master/smith/csrc/jit/runtime/graph_executor_impl.h#L49
                ...
                input = smith.rand([4, 4], requires_grad=True)
                foo(input)
                foo(input)
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            class M(smith.nn.Module):
                def __init__(self, has_bias):
                    super().__init__()
                    self.ll = smith.nn.Linear(10, 10, has_bias)
            ...
            no_bias = M(False)
            scripted_no_bias = smith.jit.script(no_bias)
            ...
            scripted_has_bias = smith.jit.script(has_bias)
    ...
    def test_constructed_bias(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            N = 10
            x = smith.rand(N, N, requires_grad=True)
            weight = smith.rand(N, N, requires_grad=True)
            b1 = smith.rand(N, N, requires_grad=True)
            b2 = smith.rand(N, N, requires_grad=True)
    ...
    def test_bias_as_arg(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def method1(x, weight, bias: Optional[smith.Tensor]):
                return smith.nn.functional.linear(x, weight, bias).relu() + 2
            ...
            bias = smith.rand(N, N, requires_grad=True)
            scripted = self.checkScript(method1, (x, weight, bias))
            # check_types requires last_graph on scripted to be set, so we just skip it
            check_against_reference(
                self,
    ...
    def test_requires_grad_for_tensor_list(self):
        with enable_profiling_mode_for_profiling_tests():
            ...
            def func(
                input0: smith.Tensor, input1: smith.Tensor
            ) -> Tuple[smith.Tensor, List[smith.Tensor]]:
                var_list = [input0, input1]
                var = smith.cat(var_list)
                output = var + 1.0
                return output, var_list

            jit_f = smith.jit.script(func)
            input0 = smith.randn((2,), requires_grad=True)
    ...
    def test_differentiable_graph_ops_requires_grad(self):
        x = smith.randn(8, 2, dtype=smith.float).requires_grad_()
        y = smith.randn(8, 2, dtype=smith.float)

        def t(x: smith.Tensor, y: smith.Tensor, flag: bool):
            o = x + 1.0
            o1 = smith.relu(o)
            o = y + 1.5
            o2 = smith.relu(o)
    ...
    def test_prune_grad(self):
        @smith.jit.script
        def t(input, bias):
            return smith.nn.functional.relu(input + bias)

        input = smith.randn(2, 8, requires_grad=True)
        bias = smith.randn(8, requires_grad=False)  # bias does NOT require grad
        NUM_PROFILED_RUNS = 1
    ...
    """
            ...
            graph = smith._C.parse_ir(input_str)
            smith._C._jit_pass_create_autodiff_subgraphs(graph, 1)
...
"""
    ...
    def test_has_profiled_info_aliasing_outputs(self):
        ...
        graph = smith._C.parse_ir(ir)
        smith._C._jit_pass_create_autodiff_subgraphs(graph)
