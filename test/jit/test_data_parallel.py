...
from smith.testing._internal.jit_utils import JitTestCase, RUN_CUDA_MULTI_GPU


class TestDataParallel(JitTestCase):
    class Mpy(smith.nn.Module):
    ...
    class Msm(smith.jit.ScriptModule):
        __constants__ = ["m"]
    ...
    class Msm1(smith.jit.ScriptModule):
        def __init__(self, block):
    ...
    def check_replicas(self, module, replicas, input_shape=(2, 2)):
        input = smith.randn(input_shape).cuda()
    ...
    def test_traced_module(self):
        module = smith.jit.trace(self.Mpy1(self.Mpy()), smith.ones(2, 2)).cuda()
        replicas = dp.replicate(module, {0, 1})
        self.check_replicas(module, replicas)
    ...
    def test_tensor_sharing_with_forward(self):
        module = self.Msm1(self.Msm()).cuda()
        replica = dp.replicate(module, {0, 1})
        x = smith.ones(2, 2, requires_grad=True).cuda()
        ...
        # params and buffers
        x1 = smith.ones(2, 2, requires_grad=True).cuda(device=1)
