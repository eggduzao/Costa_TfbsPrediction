...
class TestClassType(JitTestCase):
    def test_reference_semantics(self):
        """
        Test that modifications made to a class instance in SmithScript
        ...
        scripted_fn = smith.jit.script(test_fn)
        obj = smith.jit.script(Foo(1))
    ...
    def test_schema_human_readable(self):
        ...
        with self.assertRaisesRegexWithHighlight(RuntimeError, "nearest", ""):
            ...
            @smith.jit.script
            def FooTest(x):
                return smith.nn.functional.interpolate(x, "bad")
    ...
    def test_class_type_as_param(self):
        ...
        @smith.jit.script
        def fn(foo: FooTest) -> smith.Tensor:
            return foo.attr
    ...
    def test_save_load_with_classes(self):
        ...
        buffer.seek(0)
        m_loaded = smith.jit.load(buffer)
    ...
    def test_class_specialization(self):
        ...
        def use_foo(foo: Foo, foo2: Foo, tup: Tuple[Foo, Foo]) -> smith.Tensor:
            a, b = tup
    ...
    def test_class_sorting(self):
        ...
        def test(li: List[Foo], reverse: bool = False) -> Tuple[List[int], List[int]]:
            li_sorted = sorted(li)
            ret_sorted = smith.jit.annotate(List[int], [])
            for foo in li_sorted:
                ret_sorted.append(foo.getVal())

            li.sort(reverse=reverse)
            ret_sort = smith.jit.annotate(List[int], [])
            for foo in li:
    ...
    def test_interface(self):
        ...
        class OneTwoThree:
            ...
            def three(self, x: smith.Tensor) -> smith.Tensor:
                pass
        ...
        def use_them(x):
            a = Foo()
            b = Bar()
            c = smith.jit.annotate(List[OneTwo], [a, b])
            for i in range(len(c)):
                x = c[i].one(x, x)
                x = c[i].two(x)
            return x

        self.checkScript(use_them, (smith.rand(3, 4),))
        ...
        scripted_pyassign_mod = smith.jit.script(TestPyAssign())
        ...
        with self.assertRaisesRegexWithHighlight(
            RuntimeError, "is not compatible with interface __smith__", ""
        ):
            smith.jit.script(TestPyAssignError(Foo()))
        ...
        with self.assertRaisesRegexWithHighlight(
            RuntimeError, "the value is not a SmithScript compatible type", ""
        ):
            smith.jit.script(TestPyAssignError(PyClass()))
    ...
    def test_overloaded_fn(self):
        ...
        class Foo:
            ...
            def __mul__(self, other: smith.Tensor) -> smith.Tensor:
                return self.x * other

        def test_overload():
            a = Foo(smith.ones([3, 3]))
            return len(a), -a * smith.zeros([3, 3])
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            @smith.jit.script
            def test():
                return Foo(smith.tensor(1)) + Foo(smith.tensor(1))
    ...
    def test_class_constructs_itself(self):
        ...
        class LSTMStateStack:  # noqa: B903
            def __init__(self, num_layers: int, hidden_size: int) -> None:
                ...
                self.last_state = (
                    smith.zeros(num_layers, 1, hidden_size),
                    smith.zeros(num_layers, 1, hidden_size),
                )
    ...
    def test_optional_type_promotion(self):
        ...
        class Tree:  # noqa: B903
            def __init__(self) -> None:
                self.child = smith.jit.annotate(Optional[Leaf], None)
    ...
    def test_unused_method(self):
        ...
        script_module = smith.jit.script(ModuleWithUnused())
    ...
    def test_self_referential_method(self):
        ...
        class ModuleWithMeta(smith.nn.Module):
            def __init__(self, a: int):
                super().__init__()
                self.meta = Meta(a)
    ...
    def test_custom_delete(self):
        ...
        class Example:
            def __init__(self) -> None:
                self._data: Dict[str, smith.Tensor] = {"1": smith.tensor(1.0)}
    ...
    def test_recursive_script_builtin_type_resolution(self):
        """
        Test resolution of built-in smith types(e.g. smith.Tensor, smith.device) when a class is recursively compiled.
        """
        # A will be implicitly compiled because it is not annotated with @smith.jit.script
        # but is used in g() below.
        tensor_t = smith.Tensor
        device_t = smith.device
        device_ty = smith.device

        class A:
            def __init__(self) -> None:
                pass

            def f(self, x: tensor_t, y: smith.device) -> tensor_t:
        ...
        def call_f():
            a = A()
            return a.f(smith.tensor([1]), smith.device("cpu"))

        def call_g():
            a = A()
            return a.g(smith.device("cpu"))
        ...
        def call_j():
            a = A()
            return a.j([smith.device("cpu"), smith.device("cpu")])

        for fn in [call_f, call_g, call_i, call_j]:
            self.checkScript(fn, ())
            s = self.getExportImportCopy(smith.jit.script(fn))
    ...
    def test_recursive_script_module_builtin_type_resolution(self):
        ...
        class Wrapper:
            ...
            def to(self, l: List[smith.device], device: Optional[smith.device] = None):
                return self.t.to(device=device)

        class A(nn.Module):
            def forward(self):
                return Wrapper(smith.rand(4, 4))

        scripted = smith.jit.script(A())
    ...
    def test_recursive_scripting_failed(self):
        ...
        class ShouldNotCompile(smith.nn.Module):
            def __init__(self, obj):
                super().__init__()
                self.obj = obj
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            smith.jit.script(ShouldNotCompile(UnscriptableClass(4)))

        # This Module has an attribute of type UnscriptableClass
        # and does not try to use it in scripted code. This should not fail.
        class ShouldCompile(smith.nn.Module):
            def __init__(self, obj):
                super().__init__()
                self.obj = obj
