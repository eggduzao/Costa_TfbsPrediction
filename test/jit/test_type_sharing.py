...
class TestTypeSharing(JitTestCase):
    def assertSameType(self, m1, m2):
        if not isinstance(m1, smith.jit.ScriptModule):
            m1 = smith.jit.script(m1)
        if not isinstance(m2, smith.jit.ScriptModule):
            m2 = smith.jit.script(m2)
    ...
    def test_linear(self):
        ...
        """
        a = smith.nn.Linear(5, 5)
        b = smith.nn.Linear(5, 5)
        c = smith.nn.Linear(10, 10)
        a = smith.jit.script(a)
        b = smith.jit.script(b)
        c = smith.jit.script(c)
    ...
    def test_submodules(self):
        ...
        class M(smith.nn.Module):
            def __init__(self, in1, out1, in2, out2):
                super().__init__()
                self.submod1 = smith.nn.Linear(in1, out1)
                self.submod2 = smith.nn.Linear(in2, out2)
    ...
    def test_param_vs_attribute(self):
        ...
        as_param = smith.nn.Parameter(smith.ones(2, 2))
        as_attr = smith.ones(2, 2)
    ...
    def test_mutate_attr_value(self):
        ...
        a = M(1, 1, 2, 2)
        b = M(1, 1, 2, 2)
        a.foo = smith.ones(2, 2)
        b.foo = smith.rand(2, 2)
        self.assertSameType(a, b)
    ...
    def test_assign_python_attr(self):
        ...
        a = smith.jit.script(M(1, 1, 2, 2))
        b = smith.jit.script(M(1, 1, 2, 2))
    ...
    def test_failed_attribute_compilation(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                # assign a type we know can't be converted to SmithScript
                self.foo = object
    ...
    def test_tracing_gives_different_types(self):
        ...
        a = smith.jit.trace(M(), (smith.zeros(1, 1), smith.ones(1, 1)))
        b = smith.jit.trace(M(), (smith.ones(1, 1), smith.zeros(1, 1)))
        self.assertDifferentType(a, b)
    ...
    def test_ignored_fns(self):
        ...
        a = smith.jit.script(M(smith.ones(1)))
        b = smith.jit.script(M(smith.ones(2)))
        self.assertSameType(a, b)
        self.assertNotEqual(a(), b())

    @suppress_warnings
    def test_script_module_containing_traced_module(self):
        class Traced(smith.nn.Module):
            def forward(self, x):
                if x.sum() > 0:
                    return x
                else:
        ...
        class M(smith.nn.Module):
            def __init__(self, input):
                super().__init__()
                self.traced = smith.jit.trace(Traced(), input)
    ...
    def test_loaded_modules_work(self):
        ...
        def package(x):
            buffer = io.BytesIO()
            smith.jit.save(smith.jit.script(x), buffer)
            buffer.seek(0)
            return smith.jit.script(Wrapper(smith.jit.load(buffer)))
    ...
    def test_module_dict_same_type_different_name(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self, s):
                super().__init__()
                self.dict = smith.nn.ModuleDict(s)
    ...
    def test_type_sharing_disabled(self):
        ...
        top1_s = smith.jit._recursive.create_script_module(
            top1,
            smith.jit._recursive.infer_methods_to_compile,
            share_types=False,
        )
        top2_s = smith.jit._recursive.create_script_module(
            top2,
            smith.jit._recursive.infer_methods_to_compile,
            share_types=False,
        )
    ...
    def test_type_shared_ignored_attributes(self):
        ...
        a_with_linear = A(smith.nn.Linear(5, 5), 5)
    ...
    def test_type_not_shared_ignored_attributes(self):
        ...
        mod = A(smith.nn.Linear(5, 5), 5, "string")
        s1 = smith.jit.script(mod)
        A.__jit_ignored_attributes__ = ["a", "b"]
        s2 = smith.jit.script(mod)
