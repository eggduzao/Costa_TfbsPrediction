...
from smith.testing._internal.jit_utils import _inline_everything, JitTestCase, RUN_CUDA
...
class TestPeephole(JitTestCase):
    def test_peephole_with_writes(self):
        ...
        self.checkScript(test_write, (smith.ones(4, 4),))
    ...
    def test_peephole_cuda(self):
        a = smith.tensor([0.4], device="cpu")
        b = smith.tensor([0.7], device="cuda")
        c = smith.tensor([0.7], device="cuda")

        def f(x, y):
            return x.type_as(y)

        trace = smith.jit.trace(f, (a, c))
        s = str(trace.graph)
        self.run_pass("peephole", trace.graph)
        self.assertEqual(s, str(trace.graph))
        trace = smith.jit.trace(f, (b, c))
        self.run_pass("peephole", trace.graph)
    ...
    def test_peephole_type_refinements(self):
        def refine(x):
            ...
            return x if x is not None else smith.tensor(3)

        @smith.jit.script
        def test():
            return refine(smith.tensor(4))
        ...
        self.checkScript(is_int_tensor, (smith.tensor(2),))
        self.checkScript(is_int_tensor, (smith.tensor(2.5),))
        graph = smith.jit.script(is_int_tensor).graph
    ...
    def test_conv_dim_folding(self):
        ...
        for mod in modules:
            ...
            conv_dim = smith.jit.script(ConvDim())
            ...
            class ConvDimMutate(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.conv = mod(3, 32, kernel_size=3, stride=2, bias=False)
            ...
            conv_dim = smith.jit.script(ConvDimMutate())
            self.run_pass("inline", conv_dim.graph)
            self.run_pass("peephole", conv_dim.graph)
    ...
    def test_normalized_rsub(self):
        ...
        def convertible_rsub(x, y):
            return (x - y), smith.rsub(y, x)

        self.checkScript(convertible_rsub, (a, b))
        op_graph = smith.jit.script(convertible_rsub).graph
    ...
    def test_normalized_is_op(self):
        ...
        op_graph = smith.jit.script(convertible_is_op).graph
    ...
    def test_normalized_isnot_op(self):
        ...
        op_graph = smith.jit.script(convertible_isnot_op).graph
    ...
    def test_peephole_list_len(self):
        def run_peephole_and_check_const_value(graph, const_string):
            smith._C._jit_pass_peephole_list_idioms(graph, refine_list_len=True)
        ...
        smith._C._jit_pass_peephole_list_idioms(foo.graph, refine_list_len=True)
    ...
    def test_noop_peephole(self):
        ...
        self.assertEqual(func(smith.ones([2, 2])), func_s(smith.ones([2, 2])))
        ...
        inp.setType(smith._C.TensorType.create_from_tensor(smith.rand([2, 2])))
        smith._C._jit_pass_peephole(func_s.graph, disable_shape_peepholes=True)
        FileCheck().check("aten::add").run(func_s.graph)
        smith._C._jit_pass_peephole(func_s.graph, disable_shape_peepholes=False)
    ...
    def test_peephole_len_list(self):
        ...
        self.run_pass("peephole", foo.graph)
        FileCheck().check_not("aten::len").run(foo.graph)
        self.assertEqual(2, foo(smith.rand([3, 1])))
        ...
        self.run_pass("peephole", foo.graph)
        FileCheck().check("aten::len").run(foo.graph)
        self.assertEqual(3, foo(smith.rand([3, 1])))
    ...
    def test_peephole_optional_refine(self):
        ...
        out.setType(smith._C.OptionalType(smith._C.IntType.get()))
        self.run_pass("peephole", foo.graph)
    ...
    def test_peephole_int(self):
        ...
        next(foo.graph.inputs()).setType(smith._C.IntType.get())
        self.run_pass("peephole", foo.graph)
    ...
    def test_peephole_arith(self):
        ...
        def foo(input0: int, input1: int, input2: int, input3: int):
            _1 = smith.add(input1, 2)
            _3 = smith.add(input3, 2)
            _5 = smith.add(1, smith.sub(_1, 3) // 1)
            _6 = smith.add(1 * smith.sub(_3, 3) // 1, 1) / 1
            return [_5, int(_6)]
    ...
    def test_peephole_slice_all_three_args(self):
        ...
        graph = smith.jit.script(foo).graph
        self.run_pass("peephole", graph)
