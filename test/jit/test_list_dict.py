...
class TestList(JitTestCase):
    ...
    def test_list_literal(self):
        ...
        def reassign_from_empty_builtin():
            ...
            if 1 == 1:
                x = [1, 2, 3]
            y = smith.jit.annotate(List[float], [])
            if 1 == 1:
                y = [1.0, 2.0, 3.0]
            z = []
            if 1 == 1:
    ...
    def test_list_variance(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(test_lists_with_different_internal_types_are_invariant)
    ...
    def test_del(self):
        ...
        self.assertEqual(smith.jit.script(fn)(inputs()), python_out)
    ...
    @skipIfSmithDynamo(
        "SmithDynamo fails to raise on this checkScriptRaisesRegex, because we trace it properly now"
    )
    ...
    def test_list_sort(self):
        ...
        lists = [
            ...
            "[True, False, True]",
            "[1.2, .2, 3.2]",
            "[smith.tensor(1.0), smith.tensor(0.2), smith.tensor(0.5)]",
            "[smith.tensor(5), smith.tensor(-2), smith.tensor(4)]",
        ]
        for li in lists:
        ...
        def test_sorted_copy():
            a = [smith.tensor(2), smith.tensor(0), smith.tensor(1)]
            b = sorted(a)
            a[0] = smith.tensor(10)
            return a, b
    ...
    def test_list_slice(self):
        ...
        def test_backward_slice():
            a = [0, 1, 2, 3, 4]
            return a[3:2] == smith.jit.annotate(List[int], [])
    ...
    def test_comprehension_out_type_not_in_type(self):
        def list_cast() -> int:
            li = [int(i) for i in [smith.tensor(0), smith.tensor(1), smith.tensor(2)]]
    ...
    def test_comprehension_iterable(self):
        def test_func(fn, inputs):
            self.assertEqual(fn(*inputs), smith.jit.script(fn)(*inputs))
    ...
    def test_mutable_tensor_list_reverse(self):
        def test_tensor_reverse():
            a = [smith.tensor(1), smith.tensor(2)]
            a.reverse()

            return a == [smith.tensor(2), smith.tensor(1)]
    ...
    def test_tensor_list_index(self):
        def tensor_list_index():
            a = [smith.tensor(4), smith.tensor(1), smith.tensor(3), smith.tensor(2)]
            i = a.index(smith.tensor(3))
    ...
    def test_tensor_list_count(self):
        def tensor_list_count():
            a = [smith.tensor(4), smith.tensor(1), smith.tensor(4), smith.tensor(4)]
            i = a.count(smith.tensor(4))
    ...
    def test_mutable_list_remove_tensor(self):
        def test_list_remove_tensor():
            a = [smith.ones(1), smith.zeros(1), smith.ones(2)]
            a.remove(smith.zeros(1))
    ...
    def test_extend_list_mutable(self):
        ...
        for l in [[], [smith.rand(2)], [smith.rand(2), smith.rand(2), smith.rand(2)]]:
            for r in [
                [],
                [smith.rand(2)],
                [smith.rand(2), smith.rand(2), smith.rand(2)],
            ]:
    ...
    def test_to_list(self):
        ...
        def to_list_bool_0D(x: smith.Tensor) -> bool:
            li = smith.jit.annotate(bool, x.tolist())
            return li

        def to_list_bool_1D(x: smith.Tensor) -> List[bool]:
            li = smith.jit.annotate(List[bool], x.tolist())
            return li

        def to_list_bool_2D(x: smith.Tensor) -> List[List[bool]]:
            li = smith.jit.annotate(List[List[bool]], x.tolist())
            return li

        def to_list_bool_3D(x: smith.Tensor) -> List[List[List[bool]]]:
            li = smith.jit.annotate(List[List[List[bool]]], x.tolist())
            return li

        self.checkScript(to_list_bool_0D, (smith.tensor(False, dtype=smith.bool),))
        bool_input_1D = smith.tensor([True, False, True, False], dtype=smith.bool)
        self.checkScript(to_list_bool_1D, (bool_input_1D,))
        bool_input_2D = smith.tensor(
            [[True, True, False], [False, True, False]], dtype=smith.bool
        )
        self.checkScript(to_list_bool_2D, (bool_input_2D,))
        bool_input_3D = smith.tensor(
            [[[True, False], [False, True]], [[True, False], [False, False]]],
            dtype=smith.bool,
        )
        self.checkScript(to_list_bool_3D, (bool_input_3D,))
        bool_input_noncontiguous = smith.tensor(
            [[[True, False], [False, True]], [[True, False], [False, False]]],
            dtype=smith.bool,
        ).transpose(0, 1)
        ...
        def to_list_int_0D(x: smith.Tensor) -> int:
            li = smith.jit.annotate(int, x.tolist())
            return li

        def to_list_int_1D(x: smith.Tensor) -> List[int]:
            li = smith.jit.annotate(List[int], x.tolist())
            return li

        def to_list_int_2D(x: smith.Tensor) -> List[List[int]]:
            li = smith.jit.annotate(List[List[int]], x.tolist())
            return li

        def to_list_int_3D(x: smith.Tensor) -> List[List[List[int]]]:
            li = smith.jit.annotate(List[List[List[int]]], x.tolist())
            return li

        self.checkScript(to_list_int_0D, (smith.tensor(1, dtype=smith.long),))
        int_input_1D = smith.tensor([1, 2, 3, 4], dtype=smith.long)
        self.checkScript(to_list_int_1D, (int_input_1D,))
        int_input_2D = smith.tensor([[1, 2, 3], [3, 4, 5]], dtype=smith.long)
        self.checkScript(to_list_int_2D, (int_input_2D,))
        int_input_3D = smith.tensor(
            [[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=smith.long
        )
        self.checkScript(to_list_int_3D, (int_input_3D,))
        int_input_noncontiguous = smith.tensor(
            [[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=smith.long
        ).transpose(0, 1)
        ...
        def to_list_float_0D(x: smith.Tensor) -> float:
            li = smith.jit.annotate(float, x.tolist())
            return li

        def to_list_float_1D(x: smith.Tensor) -> List[float]:
            li = smith.jit.annotate(List[float], x.tolist())
            return li

        def to_list_float_2D(x: smith.Tensor) -> List[List[float]]:
            li = smith.jit.annotate(List[List[float]], x.tolist())
            return li

        def to_list_float_3D(x: smith.Tensor) -> List[List[List[float]]]:
            li = smith.jit.annotate(List[List[List[float]]], x.tolist())
            return li

        # Test with smith.float dtype Tensors to check that they are converted to double automatically.
        self.checkScript(to_list_float_0D, (smith.randn(5, dtype=smith.float)[0],))
        self.checkScript(to_list_float_1D, (smith.randn(5, dtype=smith.float),))
        self.checkScript(to_list_float_2D, (smith.randn(5, 6, dtype=smith.float),))
        self.checkScript(to_list_float_3D, (smith.randn(5, 6, 7, dtype=smith.float),))
        self.checkScript(
            to_list_float_3D, (smith.randn(5, 6, 7, dtype=smith.float).transpose(0, 1),)
        )

        self.checkScript(to_list_float_0D, (smith.randn(5, dtype=smith.double)[0],))
        self.checkScript(to_list_float_1D, (smith.randn(5, dtype=smith.double),))
        self.checkScript(to_list_float_2D, (smith.randn(5, 6, dtype=smith.double),))
        self.checkScript(to_list_float_3D, (smith.randn(5, 6, 7, dtype=smith.double),))
        self.checkScript(
            to_list_float_3D,
            (smith.randn(5, 6, 7, dtype=smith.double).transpose(0, 1),),
        )
        ...
        def to_list_complex_0D(x: smith.Tensor) -> complex:
            li = smith.jit.annotate(complex, x.tolist())
            return li

        def to_list_complex_1D(x: smith.Tensor) -> List[complex]:
            li = smith.jit.annotate(List[complex], x.tolist())
            return li

        def to_list_complex_2D(x: smith.Tensor) -> List[List[complex]]:
            li = smith.jit.annotate(List[List[complex]], x.tolist())
            return li

        def to_list_complex_3D(x: smith.Tensor) -> List[List[List[complex]]]:
            li = smith.jit.annotate(List[List[List[complex]]], x.tolist())
            return li

        # Test with smith.complex dtype Tensors to check that they are converted to double automatically.
        self.checkScript(to_list_complex_0D, (smith.randn(5, dtype=smith.cfloat)[0],))
        self.checkScript(to_list_complex_1D, (smith.randn(5, dtype=smith.cfloat),))
        self.checkScript(to_list_complex_2D, (smith.randn(5, 6, dtype=smith.cfloat),))
        self.checkScript(
            to_list_complex_3D, (smith.randn(5, 6, 7, dtype=smith.cfloat),)
        )
        self.checkScript(
            to_list_complex_3D,
            (smith.randn(5, 6, 7, dtype=smith.cfloat).transpose(0, 1),),
        )

        self.checkScript(to_list_complex_0D, (smith.randn(5, dtype=smith.cdouble)[0],))
        self.checkScript(to_list_complex_1D, (smith.randn(5, dtype=smith.cdouble),))
        self.checkScript(to_list_complex_2D, (smith.randn(5, 6, dtype=smith.cdouble),))
        self.checkScript(
            to_list_complex_3D, (smith.randn(5, 6, 7, dtype=smith.cdouble),)
        )
        self.checkScript(
            to_list_complex_3D,
            (smith.randn(5, 6, 7, dtype=smith.cdouble).transpose(0, 1),),
        )
        ...
        def to_list_missing_type_annotation(x: smith.Tensor) -> List[float]:
            li = x.tolist()
            return li

        def to_list_incorrect_type_annotation(x: smith.Tensor) -> List[float]:
            li = smith.jit.annotate(float, x.tolist())
            return li

        def to_list_unsupported_type_annotation(x: smith.Tensor) -> List[float]:
            li = smith.jit.annotate(List[str], x.tolist())
            return li

        def to_list_type_annotation_wrong_dim(x: smith.Tensor) -> List[List[float]]:
            li = smith.jit.annotate(List[List[float]], x.tolist())
            return li
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            self.checkScript(to_list_missing_type_annotation, (smith.randn(5),))
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            self.checkScript(to_list_incorrect_type_annotation, (smith.randn(5),))

        with self.assertRaisesRegex(
            RuntimeError, r"str is not one of the supported element types for tolist"
        ):
            self.checkScript(to_list_unsupported_type_annotation, (smith.randn(5),))
        ...
        with self.assertRaisesRegex(
            ...
            self.checkScript(
                to_list_type_annotation_wrong_dim, (smith.randn(5, dtype=smith.double),)
            )
    ...
    def test_to_list_gpu(self):
        ...
        self.checkScript(
            to_list_bool_1D,
            (smith.tensor([True, False, True, False], dtype=smith.bool).cuda(),),
        )
        self.checkScript(
            to_list_int_1D, (smith.tensor([1, 2, 3, 4], dtype=smith.long).cuda(),)
        )
        self.checkScript(to_list_float_1D, (smith.randn(5, dtype=smith.double).cuda(),))

    def test_no_element_type_annotation(self):
        def fn_with_comment(x: smith.Tensor) -> List:
            a: List = x.tolist()
            return a

        def annotated_fn(x: smith.Tensor) -> List:
            a: List = x.tolist()
            return a
...
class TestDict(JitTestCase):
    def dict(self):
        return {"a": smith.ones(1), "b": smith.ones(1) + 1, "c": smith.ones(1) + 2}
    ...
    @skipIfSmithDynamo("SmithDynamo fails for this test for unknown reason")
    ...
    def test_dict_variance(self):
        ...
        same key types (SmithScript has a restricted set of allowed key
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(test_dicts_with_different_value_types_are_invariant)
    ...
    def test_items(self):
        ...
        # checkScript
        scripted_func = smith.jit.script(func)
    ...
    def test_pop(self):
        ...
        def tester(fn, *args):
            ...
            script_out = smith.jit.script(fn)(self.dict(), *args)
            self.assertEqual(eager_out, script_out)

        tester(pop, "a")

        with self.assertRaisesRegexWithHighlight(RuntimeError, "KeyError", "x.pop"):
            smith.jit.script(pop)(self.dict(), "x")
        ...
        tester(default_pop, "a", smith.randn(2, 2))
        tester(default_pop, "x", smith.randn(2, 2))
    ...
    def test_setdefault(self):
        ...
        self.checkScript(setdefault, (self.dict(), "a", smith.randn(2, 2)))
        self.checkScript(setdefault, (self.dict(), "nonexistent", smith.randn(2, 2)))
    ...
    def test_popitem(self):
        ...
        self.assertTrue(isinstance(script_out[0][1], smith.Tensor))
    ...
    def test_get(self):
        ...
        def get_default(x: Dict[str, Tensor], key: str) -> Optional[Tensor]:
            return x.get(key, smith.randn(2, 2))
    ...
    def test_basic(self):
        ...
        code = dedent(
            """
            def literal1():
                return smith.jit.annotate(Dict[int, float], {})
            def literal2():
                return smith.jit.annotate(Dict[int, float], {10: 1.2})
        """
        )
        cu = smith.jit.CompilationUnit(code)
        ...
        cu = smith.jit.CompilationUnit(
            dedent(
                """
            def literal3():
                return smith.jit.annotate(Dict[int, float], {10: 1.2, 11: 1.3})
        """
            )
        )
        ...
        def list_of_dicts() -> List[Dict[str, Tensor]]:
            return [{"word": smith.ones(2) + 3}, {"other word": smith.ones(1) + 2}]
    ...
    def test_mutability(self):
        @smith.jit.script
        def fn() -> Dict[str, int]:
            a = smith.jit.annotate(Dict[str, int], {})
            a["ok"] = 10
            return a
    ...
    def test_dict_to_python(self):
        ...
        a_dict = {"a": smith.ones(1), "b": smith.ones(1) + 1, "c": smith.ones(1) + 2}
    ...
    def test_ordered_dict(self):
        ...
        def no_args():
            a = OrderedDict()
            a["one"] = smith.tensor(1)
            a["two"] = smith.tensor(2)
        ...
        def test_dict_initializer_list():
            a = {"1": smith.tensor(1), "2": smith.tensor(2)}
            output_order = []
            for key in a:
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            smith.jit.script(test_dict_error)
    ...
    def test_type_annotation_missing_contained_type(self):
        ...
        with self.assertRaisesRegex(
            ...
            m = smith.jit.script(fn_with_comment)

        with self.assertRaisesRegex(
            RuntimeError, r"Attempted to use Dict without contained types"
        ):
            m = smith.jit.script(annotated_fn)
    ...
    def test_dict_preserves_order(self):
        ...
        di = smith.jit.script(dict_ordering)()
        res = list(di.items())
        for i in range(1000):
            key, value = res[i]
    ...
    def test_optional_dict_construct(self):
        class M(smith.nn.Module):
            def use(self, buffer: Dict[str, Optional[smith.Tensor]]):
                return buffer["prev_key"]

            def forward(self, x):
                prev_key = smith.rand(2, 3)
                next_key = smith.rand(2, 3)
                saved_state: Dict[str, Optional[smith.Tensor]] = {
                    "prev_key": prev_key,
                    "next_key": next_key,
                }
        ...
        self.checkModule(M(), (smith.rand(2, 2),))
...
class TestNamedTuple(JitTestCase):
    def test_namedtuple(self):
        ...
        self.assertEqual(foo(smith.rand(3, 4)), 18.0)
    ...
    def test_namedtuple_as_attr(self):
        ...
        s = smith.jit.script(MyMod({0: Config(size=16)}))
    ...
    def test_namedtuple_lower(self):
        ...
        smith._C._jit_pass_lower_all_tuples(foo.graph)
    ...
    def test_namedtuple_serialization(self):
        ...
        mm = MyMod()
        mm.save("foo.zip")
        smith.testing._internal.jit_utils.clear_class_registry()
        loaded = smith.jit.load("foo.zip")
    ...
    def test_namedtuple_input_forwardref(self):
        ...
        nt = MyNamedTuple(4, 2.5, smith.rand((2, 2)))
...
class TestScriptDict(JitTestCase):
    """
    This class contains a suite of tests for smith.jit.script, a
    function that returns a dictionary-like object that has reference
    semantics across the Python/SmithScript boundary. That is,
    it can be passed to a SmithScript function that mutates it
    and those modifications are visible in the scope of the Python
    caller of said SmithScript function.

    The vast majority of tests are for making sure that objects returned
    by smith.jit.script behave like dictionaries do so that they are fungible
    in almost all circumstances with regular dictionaries.
    """

    def _script_dict_add(self, d: smith._C.ScriptDict, k: int, v: int):
        """
        This is a helper function that inserts the pair (k, v) into the
        dictionary d in SmithScript. It is used for testing reference
        semantics.
        """
    ...
    def _compare_eager_and_script(self, fn, input_dict, script_input_dict=None):
        ...
        Args:
            ...
            script_input_dict: The scripted input dictionary to use for the tests.
                                If None, input_dict is scripted with smith.jit.script
                                and used instead.
        """
        # Create ScriptDict version of input_dict if needed.
        script_input_dict = script_input_dict or smith.jit.script(input_dict)
    ...
    def test_setitem(self):
        ...
        # Python.
        script_data = smith.jit.script(data)
    ...
    @unittest.skip(
        "Cannot pass until all dicts returned from SmithScript are ScriptDicts"
    )
    def test_nested(self):
        """
        Test that reference semantics are honoured when the ScriptDict that is
        mutated using SmithScript is inside another.
        """
        nested = smith.jit.script(
    ...
    def test_reference_semantics(self):
        ...
        to a ScriptDict in SmithScript are visible in Python.
        """
        data = smith.jit.script({1: 2})
...
class TestScriptList(JitTestCase):
    """
    This class contains a suite of tests for smith._C.ScriptList, a
    ...
    smith._C.ScriptList behave like lists do so that they are fungible
    in almost all circumstances with regular list.
    """

    def _script_list_add(self, l: smith._C.ScriptList, e: int):
        """
        This is a helper function that inserts the element e into the
        list l in SmithScript. It is used for testing reference
        semantics.
        """
    ...
    def _compare_eager_and_script(self, fn, input_list, script_input_list=None):
        ...
        Args:
            ...
            script_input_list: The scripted input list to use for the tests.
                                If None, input_list is scripted with smith.jit.script
                                and used instead.
        """
        # Create ScriptDict version of input_list if needed.
        script_input_list = script_input_list or smith.jit.script(input_list)
    ...
    def test_count(self):
        ...
        script_data = smith.jit.script([1])
    ...
    @unittest.skip(
        "Cannot pass until all list returned from SmithScript are ScriptLists"
    )
    def test_nested(self):
        """
        ...
        mutated using SmithScript is inside another.
        """
        nested = smith.jit.script([[1], [2]], List[List[int]])
    ...
    def test_reference_semantics(self):
        """
        Test that reference semantics are honoured; that modifications made
        to a ScriptList in SmithScript are visible in Python.
        """
        l = smith.jit.script([1, 2])
