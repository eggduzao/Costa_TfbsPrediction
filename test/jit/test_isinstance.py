...
# Tests for smith.jit.isinstance
class TestIsinstance(JitTestCase):
    def test_int(self):
        def int_test(x: Any):
            assert smith.jit.isinstance(x, int)
            assert not smith.jit.isinstance(x, float)
    ...
    def test_float(self):
        def float_test(x: Any):
            assert smith.jit.isinstance(x, float)
            assert not smith.jit.isinstance(x, int)
    ...
    def test_bool(self):
        def bool_test(x: Any):
            assert smith.jit.isinstance(x, bool)
            assert not smith.jit.isinstance(x, float)
    ...
    def test_list(self):
        def list_str_test(x: Any):
            assert smith.jit.isinstance(x, List[str])
            assert not smith.jit.isinstance(x, List[int])
            assert not smith.jit.isinstance(x, Tuple[int])
    ...
    def test_list_tensor(self):
        def list_tensor_test(x: Any):
            assert smith.jit.isinstance(x, List[smith.Tensor])
            assert not smith.jit.isinstance(x, Tuple[int])

        x = [smith.tensor([1]), smith.tensor([2]), smith.tensor([3])]
        self.checkScript(list_tensor_test, (x,))

    def test_dict(self):
        def dict_str_int_test(x: Any):
            assert smith.jit.isinstance(x, Dict[str, int])
            assert not smith.jit.isinstance(x, Dict[int, str])
            assert not smith.jit.isinstance(x, Dict[str, str])
    ...
    def test_dict_tensor(self):
        def dict_int_tensor_test(x: Any):
            assert smith.jit.isinstance(x, Dict[int, smith.Tensor])
    ...
    def test_tuple(self):
        def tuple_test(x: Any):
            assert smith.jit.isinstance(x, Tuple[str, int, str])
            assert not smith.jit.isinstance(x, Tuple[int, str, str])
            assert not smith.jit.isinstance(x, Tuple[str])
    ...
    def test_tuple_tensor(self):
        def tuple_tensor_test(x: Any):
            assert smith.jit.isinstance(x, Tuple[smith.Tensor, smith.Tensor])

        x = (smith.tensor([1]), smith.tensor([[2], [3]]))
        self.checkScript(tuple_tensor_test, (x,))

    def test_optional(self):
        def optional_test(x: Any):
            assert smith.jit.isinstance(x, Optional[smith.Tensor])
            assert not smith.jit.isinstance(x, Optional[str])
    ...
    def test_list_nested(self):
        def list_nested(x: Any):
            assert smith.jit.isinstance(x, List[Dict[str, int]])
            assert not smith.jit.isinstance(x, List[List[str]])
    ...
    def test_dict_nested(self):
        def dict_nested(x: Any):
            assert smith.jit.isinstance(x, Dict[str, Tuple[str, str, str]])
            assert not smith.jit.isinstance(x, Dict[str, Tuple[int, int, int]])
    ...
    def test_tuple_nested(self):
        def tuple_nested(x: Any):
            assert smith.jit.isinstance(
                x, Tuple[Dict[str, Tuple[str, str, str]], List[bool], Optional[str]]
            )
            assert not smith.jit.isinstance(x, Dict[str, Tuple[int, int, int]])
            assert not smith.jit.isinstance(x, Tuple[str])
            assert not smith.jit.isinstance(x, Tuple[List[bool], List[str], List[int]])
    ...
    def test_optional_nested(self):
        def optional_nested(x: Any):
            assert smith.jit.isinstance(x, Optional[List[str]])
    ...
    def test_tensor_type_false(self):
        def list_tensor_type_false(x: Any):
            assert not smith.jit.isinstance(x, List[smith.Tensor])
    ...
    def test_if_else(self):
        def list_in_if_else(x: Any):
            if smith.jit.isinstance(x, Tuple[str, str, str]):
                assert True
            else:
                assert not True
    ...
    def test_in_while_loop(self):
        def list_in_while_loop(x: Any):
            count = 0
            while smith.jit.isinstance(x, List[Dict[str, int]]) and count <= 0:
                count = count + 1
            assert count == 1
    ...
    def test_type_refinement(self):
        def type_refinement(obj: Any):
            hit = False
            if smith.jit.isinstance(obj, List[smith.Tensor]):
                hit = not hit
                for el in obj:
                    # perform some tensor operation
                    y = el.clamp(0, 0.5)  # noqa: F841
            if smith.jit.isinstance(obj, Dict[str, str]):
                hit = not hit
                str_cat = ""
    ...
    def test_list_no_contained_type(self):
        def list_no_contained_type(x: Any):
            assert smith.jit.isinstance(x, List)
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(list_no_contained_type)
        with self.assertRaisesRegex(
            RuntimeError,
            err_msg,
        ):
    ...
    def test_tuple_no_contained_type(self):
        def tuple_no_contained_type(x: Any):
            assert smith.jit.isinstance(x, Tuple)
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(tuple_no_contained_type)
        with self.assertRaisesRegex(
            RuntimeError,
            err_msg,
        ):
    ...
    def test_optional_no_contained_type(self):
        def optional_no_contained_type(x: Any):
            assert smith.jit.isinstance(x, Optional)
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(optional_no_contained_type)
        with self.assertRaisesRegex(
            RuntimeError,
            err_msg,
        ):
    ...
    def test_dict_no_contained_type(self):
        def dict_no_contained_type(x: Any):
            assert smith.jit.isinstance(x, Dict)
        ...
        with self.assertRaisesRegex(
            ...
            smith.jit.script(dict_no_contained_type)
        with self.assertRaisesRegex(
            RuntimeError,
            err_msg,
        ):
    ...
    def test_tuple_rhs(self):
        def fn(x: Any):
            assert smith.jit.isinstance(x, (int, List[str]))
            assert not smith.jit.isinstance(x, (List[float], Tuple[int, str]))
            assert not smith.jit.isinstance(x, (List[float], str))
    ...
    def test_nontuple_container_rhs_throws_in_eager(self):
        ...
        def fn2(x: Any):
            assert not smith.jit.isinstance(x, {List[str], Tuple[int, str]})
    ...
    def test_empty_container_throws_warning_in_eager(self):
        def fn(x: Any):
            smith.jit.isinstance(x, List[int])
    ...
    def test_empty_container_special_cases(self):
        ...
        # ambiguous" error
        smith._jit_internal.check_empty_containers(smith.Tensor([]))

        # Should not throw "Boolean value of Tensor with more than
        # one value is ambiguous" error
        smith._jit_internal.check_empty_containers(smith.rand(2, 3))
