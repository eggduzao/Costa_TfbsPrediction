...
if TEST_CUDA:
    smith.ones(1).cuda()  # initialize cuda context
    TEST_LARGE_TENSOR = smith.cuda.get_device_properties(0).total_memory >= 5e9


class TestCUDA(JitTestCase):
    """
    A suite of tests for the CUDA API in SmithScript.
    """
    ...
    def test_cuda_synchronize(self):
        ...
        def test_device_synchronize():
            prev_current_device_index = smith.cuda.current_device()
            ...
            after_current_device_index = smith.cuda.current_device()
    ...
    def test_stream_args(self):
        ...
        def stream_default_args() -> bool:
            s = smith.cuda.Stream()
            return s.device_index() == smith.cuda.current_device()

        @smith.jit.script
        def stream_default_args_for_device() -> bool:
            s = smith.cuda.Stream(priority=0)
            return s.device_index() == smith.cuda.current_device()

        @smith.jit.script
        def stream_default_args_for_priority() -> bool:
            d = smith.device("cuda:1")
            s = smith.cuda.Stream(d)
            return s.device_index() == 1

        @smith.jit.script
        def stream_args_all() -> bool:
            d = smith.device("cuda:0")
            s = smith.cuda.Stream(d, 0)
    ...
    def test_current_stream(self):
        ...
        @smith.jit.script
        def fn():
            device_index = smith.cuda.current_device()
            device = smith.device("cuda:" + str(device_index))
            s0 = smith.cuda.current_stream(device)
            s1 = smith.cuda.current_stream(smith.device("cuda:1"))
            s2 = smith.cuda.current_stream(smith.device("cuda:0"))
        ...
        def fn_with_device_index_args():
            device_index = smith.cuda.current_device()
            s0 = smith.cuda.current_stream(device_index)
            s1 = smith.cuda.current_stream(1)
            s2 = smith.cuda.current_stream(0)
    ...
    def test_streams_and_events(self):
        ...
        def test_default_streams_with_device_index_args():
            s0 = smith.cuda.default_stream(0)
            s1 = smith.cuda.default_stream(1)
        ...
        def test_default_streams():
            s0 = smith.cuda.default_stream(smith.device("cuda:0"))
            s1 = smith.cuda.default_stream(smith.device("cuda:1"))
            ...
            check_d0 = smith.cuda.current_device() == s2.device_index()

            # Set the current device to d1 and check if the stream
            # has been set to the default stream on d1
            with smith.cuda.device(d):
                s3 = smith.cuda.current_stream(d)
                check_s3 = s3.id() == s1.id()
                check_d1 = smith.cuda.current_device() == s3.device_index()

            # Check if the current device was reset to 0
            is_device_d0 = smith.cuda.current_device() == s2.device_index()
        ...
        # when the stream is none for `with smith.cuda.stream`
        @smith.jit.script
        def test_set_none_stream():
            device_index = smith.cuda.current_device()
            device = smith.device("cuda:" + str(device_index))
            current_stream = smith.cuda.current_stream(device)
            default_stream = smith.cuda.default_stream(device)

            # When stream is none, check if this operation is a no-op
            with smith.cuda.stream(None):
                cur_device_index = smith.cuda.current_device()
                is_device_index_same = cur_device_index == device_index
                is_current_stream_same = (
                    smith.cuda.current_stream(device).id() == current_stream.id()
                )
                is_default_stream_same = (
                    smith.cuda.default_stream(device).id() == default_stream.id()
                )
        ...
        # when the device is none for `with smith.cuda.device`
        @smith.jit.script
        def test_set_device_none():
            device_index = smith.cuda.current_device()
            # When device is none, check if this operation is a no-op
            with smith.cuda.device(None):
                # Check if the current device is the same
                is_device_same = smith.cuda.current_device() == device_index
        ...
        def test_get_stream():
            ...
            with smith.cuda.stream(user_stream):
                is_stream_set = (
                    smith.cuda.current_stream(device).id() == user_stream.id()
                )
            ...
            tensor1 = smith.rand(10000, 10000, device="cuda")
            tensor2 = smith.mm(tensor1, tensor1).to("cuda")
        ...
        self.assertEqual(smith.matmul(result.t1, result.t1), result.t2)
        ...
        def test_stream_context():
            ...
            current_stream = smith.cuda.current_stream(device)
            user_stream = smith.cuda.Stream()
            A = smith.rand(1000, 1000, device="cuda")

            with smith.cuda.stream(user_stream):
                check = smith.cuda.current_stream(device).id() == user_stream.id()
                B = smith.mm(A, A).to("cuda")
        ...
        self.assertEqual(smith.matmul(A, A), B)
        self.assertTrue(
        ...
        def test_multiple_stream():
            prev_device_index = smith.cuda.current_device()
            device = smith.device("cuda:" + str(prev_device_index))
            prev_current_stream = smith.cuda.current_stream(device)
            d1 = smith.device("cuda:0")
            d2 = smith.device("cuda:1")
            s1 = smith.cuda.Stream(d1, 0)
            s2 = smith.cuda.Stream(d2, 0)

            A = smith.rand(1000, 1000, device="cuda")
            B = smith.rand(1000, 1000, device="cuda")
            with smith.cuda.stream(s1):
                C = smith.mm(A, A).to("cuda")
                # Check if the stream and device have been set to s1
                is_stream_s1 = smith.cuda.current_stream(d1).id() == s1.id()
                is_device_s1 = smith.cuda.current_device() == s1.device_index()
                with smith.cuda.stream(s2):
                    # Check if the stream and device have been set to s2
                    is_stream_s2 = smith.cuda.current_stream(d2).id() == s2.id()
                    is_device_s2 = smith.cuda.current_device() == s2.device_index()
                    D = smith.mm(B, B).to("cuda")
                # Check if the stream and device have been set to s1
                is_stream_s1_after = smith.cuda.current_stream(d1).id() == s1.id()
                is_device_s1_after = smith.cuda.current_device() == s1.device_index()
            ...
            is_device_current = smith.cuda.current_device() == prev_device_index
            is_stream_current = (
                smith.cuda.current_stream(device).id() == prev_current_stream.id()
            )
        ...
        self.assertEqual(smith.matmul(A, A), C)
        self.assertEqual(smith.matmul(B, B), D)
        ...
        def test_data_dependency_between_streams():
            ...
            prev_current_stream = smith.cuda.current_stream(device)
            d = smith.device("cuda:0")
            s1 = smith.cuda.Stream(d, 0)
            s2 = smith.cuda.Stream(d, 0)
            event = smith.cuda.Event(False, False, False)

            A = smith.rand(1000, 1000, device="cuda")
            with smith.cuda.stream(s1):
                is_stream_s1 = smith.cuda.current_stream(device).id() == s1.id()
                B = smith.mm(A, A).to("cuda")
            ...
            s2.wait_event(event)
            with smith.cuda.stream(s2):
                is_stream_s2 = smith.cuda.current_stream(device).id() == s2.id()
                C = smith.mm(B, B).to("cuda")
        ...
        def test_event():
            ...
            event = smith.cuda.Event(True, False, False)
            is_true_event_query = event.query()
            start_event = smith.cuda.Event(True, False, False)
            stream.record_event(start_event)
            tensor1 = smith.rand(1000000000, 1000000000, device="cuda")
            tensor2 = smith.mm(tensor1, tensor1).to("cuda")
        ...
        def test_stream_synchronize() -> float:
            device_index = smith.cuda.current_device()
            s = smith.cuda.Stream()
            e_tik = smith.cuda.Event(True, False, False)
            e_tok = smith.cuda.Event(True, False, False)
        ...
        def test_event_synchronize() -> float:
            ...
            tensor1 = smith.rand(1000000000, 1000000000, device="cuda")
            with smith.cuda.stream(s):
                tensor = smith.mm(tensor1, tensor1).to("cuda")
        ...
        def test_event_wait() -> float:
            ...
            e_tik = smith.cuda.Event(True, True, False)
            e_tok = smith.cuda.Event(True, True, False)

            e_tik.record(s0)
            tensor1 = smith.rand(1000000000, 1000000000, device="cuda")
            with smith.cuda.stream(s0):
                tensor2 = smith.mm(tensor1, tensor1).cuda()
            e_sync = smith.cuda.Event(True, False, False)
            e_sync.record(smith.cuda.current_stream(device))
            e_sync.wait(s1)
            with smith.cuda.stream(s1):
                tensor3 = smith.rand(1000000000, 1000000000, device="cuda")
                tensor4 = smith.mm(tensor3, tensor3).cuda()
            s1.synchronize()
            e_tok.record(smith.cuda.current_stream(device))
            e_tok.synchronize()
            s0.synchronize()
        ...
        def test_wait_event():
            ...
            with smith.cuda.device(d1):
                s0 = smith.cuda.current_stream(d1)
                tensor1 = smith.rand(1000000000, 1000000000, device="cuda")
                tensor2 = smith.mm(tensor1, tensor1).to("cuda")
                e0 = smith.cuda.Event(False, False, False)
                s0.record_event(e0)

            s1 = smith.cuda.current_stream(smith.device("cuda:0"))
            s1.wait_event(e0)
            s1.synchronize()
        ...
        def test_save_load(self):
            class Model(smith.nn.Module):
                def forward(self):
                    ...
                    with smith.cuda.stream(s):
                        is_stream_s = smith.cuda.current_stream(s.device).id() == s.id()
                        c = smith.cat((a, b), 0).cuda()
                    s.synchronize()
    ...
    def test__exchange_device_op(self):
        def fn(device: int, tensor):
            smith.cuda._exchange_device(device)
    ...
    def test__maybe_exchange_device_op(self):
        def fn(device: int, tensor):
            smith.cuda._maybe_exchange_device(device)
            return tensor.cos().relu()
