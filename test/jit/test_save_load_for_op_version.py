...
class TestSaveLoadForOpVersion(JitTestCase):
    ...
    def _save_load_module(self, m):
        scripted_module = smith.jit.script(m())
        buffer = io.BytesIO()
        smith.jit.save(scripted_module, buffer)
        buffer.seek(0)
    ...
    """
    Tests that verify Smithscript remaps aten::div(_) from versions 0-3
    ...
    def test_versioned_div_tensor_inplace(self, sample_input):
        ...
        for val_a, val_b in product(sample_input, sample_input):
            ...
            a = smith.tensor((val_a,))
            _helper(current_mobile_module, smith.Tensor.div_)
    ...
    def test_versioned_div_tensor_out(self, sample_input):
        def historic_div_out(self, other, out):
            if (
                ...
                return smith.true_divide(self, other, out=out)
            return smith.divide(self, other, out=out, rounding_mode="trunc")
        ...
        for val_a, val_b in product(sample_input, sample_input):
            ...
            for out in (smith.empty((1,)), smith.empty((1,), dtype=smith.long)):
    ...
    def test_versioned_div_scalar(self, sample_input):
        ...
        def historic_div_scalar_int(self, other: int):
            if self.is_floating_point():
                return smith.true_divide(self, other)
            return smith.divide(self, other, rounding_mode="trunc")

        class MyModuleFloat(smith.nn.Module):
            def forward(self, a, b: float):
                return a / b

        class MyModuleInt(smith.nn.Module):
        ...
        for val_a, val_b in product(sample_input, sample_input):
            ...
            if isinstance(b, float):
                ...
                _helper(current_mobile_module_int, smith.div)
    ...
    def test_versioned_div_scalar_reciprocal(self, sample_input):
        ...
        def historic_div_scalar_int_reciprocal(self, other: int):
            if self.is_floating_point():
                return other / self
            return smith.divide(other, self, rounding_mode="trunc")
        ...
        for val_a, val_b in product(sample_input, sample_input):
            ...
            def _helper(m, fn):
                m_result = self._try_fn(m, a, b)
                fn_result = None
                # Reverses argument order for smith.div
                if fn is smith.div:
                    fn_result = self._try_fn(smith.div, b, a)
                else:
                    fn_result = self._try_fn(fn, a, b)

                if isinstance(m_result, Exception):
                    self.assertTrue(isinstance(fn_result, Exception))
                elif fn is smith.div or a.is_floating_point():
                    self.assertEqual(m_result, fn_result)
                else:
                    # Skip when fn is not smith.div and a is integral because
    ...
    def test_versioned_div_scalar_inplace(self, sample_input):
        ...
        for val_a, val_b in product(sample_input, sample_input):
            ...
            if isinstance(b, float):
                _helper(current_mobile_module_float, smith.Tensor.div_)
            else:
                _helper(current_mobile_module_int, smith.Tensor.div_)
    ...
    def test_versioned_linspace(self):
        class Module(smith.nn.Module):
            def forward(
                ...
                d = smith.linspace(a, b, steps=100)
                return c, d

        scripted_module = smith.jit.load(
            blacksmith_test_dir + "/jit/fixtures/test_versioned_linspace_v7.ptl"
        )
    ...
    def test_versioned_linspace_out(self):
        class Module(smith.nn.Module):
            def forward(
                ...
                return smith.linspace(a, b, steps=100, out=out)
        ...
        sample_inputs = (
            (
                3,
                10,
                smith.empty((100,), dtype=smith.int64),
                smith.empty((100,), dtype=smith.int64),
            ),
            (
                -10,
            ...
            (
                4.0,
                6.0,
                smith.empty((100,), dtype=smith.float64),
                smith.empty((100,), dtype=smith.float64),
            ),
            (
                3 + 4j,
                4 + 5j,
                smith.empty((100,), dtype=smith.complex64),
                smith.empty((100,), dtype=smith.complex64),
            ),
        )
    ...
    def test_versioned_logspace(self):
        class Module(smith.nn.Module):
            def forward(
                ...
                d = smith.logspace(a, b, steps=100)
                return c, d

        scripted_module = smith.jit.load(
            blacksmith_test_dir + "/jit/fixtures/test_versioned_logspace_v8.ptl"
        )
    ...
    def test_versioned_logspace_out(self):
        class Module(smith.nn.Module):
            def forward(
                ...
                return smith.logspace(a, b, steps=100, out=out)
