...
class TestRecursiveScript(JitTestCase):
    ...
    def test_script_function_attribute(self):
        ...
        self.checkModule(fn1_mod, (smith.randn(2, 2),))
        self.checkModule(fn2_mod, (smith.randn(2, 2),))
    ...
    def test_python_function_attribute(self):
        ...
        self.checkModule(mod, (smith.randn(2, 2),))
    ...
    def test_script_after_eval(self):
        ...
        m = M()
        sm1 = smith.jit.script(m)
        m.eval()
        sm2 = smith.jit.script(m)
    ...
    def test_constants_with_final(self):
        ...
        self.checkModule(M1(), (smith.randn(2, 2),))
        ...
        self.checkModule(M2(), (smith.randn(2, 2),))
        ...
        self.checkModule(M3(), (smith.randn(2, 2),))
    ...
    def test_class_compile(self):
        ...
        self.checkModule(N(), (smith.randn(2, 2),))
    ...
    def test_module_basic(self):
        class Other(smith.nn.Module):
            __constants__ = ["x"]

            def __init__(self, x):
                super().__init__()
                self.x = x
                self.param = smith.nn.Parameter(smith.ones(2, 2))
    ...
    def test_iterable_modules(self):
        ...
        self.checkModule(M(), (smith.randn(5, 5),))

    def test_prepare_scriptable_basic(self):
        class SeluButReluWhenScripted(smith.nn.SELU):
    ...
    def test_prepare_scriptable_escape_hatch(self):
        ...
        with self.assertRaisesRegex(
            smith.jit.frontend.NotSupportedError,
        ...
        jit_obj = smith.jit.script(NonJitableClassWithEscapeHatch())
    ...
    def test_attributes(self):
        ...
        untyped_values = (
            ...
            ("my_tuple", (1, 2, 3, 4)),
            ("my_list", [(1, 2), (3, 4)]),
            # ('my_tensor', smith.randn(2, 2)),
            ("my_int_list", [1, 2, 3, 4]),
            # ('my_tensor_list', [smith.ones(2, 2) + i for i in range(4)]),
    ...
    def test_function_attribute_in_submodule(self):
        class N(nn.Module):
            def __init__(self, norm):
                super().__init__()
                self.activation = smith.nn.functional.relu
                self.norm = norm
    ...
    def test_inner_traced_module(self):
        ...
        dummy = smith.jit.trace(Dummy(), smith.randn(1, 2))
        dummies = nn.ModuleList([dummy])
        model = Model(dummies)
        self.checkModule(model, (smith.rand(5, 5),))
    ...
    def test_script_loaded_module(self):
        ...
        dummy = smith.jit.script(Dummy())
        dummy = self.getExportImportCopy(dummy)

        class ContainsLoaded(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.encoder = dummy
        ...
        self.checkModule(ContainsLoaded(), (smith.rand(2, 3),))
    ...
    def test_optional_module(self):
        ...
        mod = Dummy()
        self.checkModule(mod, (smith.rand(2, 2),))
        mod.foo = None
        self.checkModule(mod, (smith.rand(2, 2),))
    ...
    def test_thread_safe_error_stacks(self):
        ...
        def callstack_creator():
            factory = smith._C._jit_tree_views.SourceRangeFactory(
                "source code", "a.py", 1, 0
            )
            x = smith._C.CallStack("a", factory.make_range(1, 0, 1))
            callstacks.append(x)
            del x
