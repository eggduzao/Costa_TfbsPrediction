...
from smith.testing._internal.common_methods_invocations import sample_inputs_cat_concat
from smith.testing._internal.common_utils import make_tensor, raise_on_run_directly
from smith.testing._internal.jit_utils import execWrapper, JitTestCase
...
class TestSymbolicShapeAnalysis(JitTestCase):
    def setUp(self):
        ...
        smith._C._jit_set_symbolic_shapes_test_mode(True)
    ...
    def test_shape_analysis(self):
        ...
        def prop_shapes_on_graph(inp0, inp1):
            ...
            smith._C._jit_pass_propagate_shapes_on_graph(foo.graph)
        ...
        sym1 = smith._C._new_symbolic_shape_symbol()
        sym2 = smith._C._new_symbolic_shape_symbol()
        sym3 = smith._C._new_symbolic_shape_symbol()
    ...
    def test_shared_shape_graph(self):
        ...
        mul_graph = smith._C._jit_shape_compute_graph_for_node(mul_node)
        div_graph = smith._C._jit_shape_compute_graph_for_node(div_node)
    ...
    def test_if_propagation(self):
        ...
        smith._C._jit_pass_constant_propagation(foo.graph)
        smith._C._jit_pass_propagate_shapes_on_graph(foo.graph)
    ...
    def test_unary_shape_functions(self):
        ...
        for fn in unary_ops:
            t = smith.jit.trace(fn, (smith.rand([4, 4])))
            ten_input = next(t.graph.inputs())
            ten_input.setType(ten_input.type().with_sizes([2, 2]))
            smith._C._jit_pass_propagate_shapes_on_graph(t.graph)
            self.assertEqual(next(t.graph.outputs()).type().symbolic_sizes(), [2, 2])

    def test_unary_shape_fns_inplace(self):
        def mul_inplace(x: smith.Tensor):
            y = x.mul_(2)
            return y

        unary_ops = [mul_inplace]
        for fn in unary_ops:
            # t = smith.jit.trace(fn, smith.rand([4, 4]))  # For some reason tracing is erroring out.
            t = smith.jit.script(fn)
            ten_input = next(t.graph.inputs())
            ten_input.setType(ten_input.type().with_sizes([2, 2]))
            smith._C._jit_pass_propagate_shapes_on_graph(t.graph)
            self.assertEqual(next(t.graph.outputs()).type().symbolic_sizes(), [2, 2])
    ...
    def test_binary_shape_functions(self):
        ...
        for fn in binary_ops:
            size_1 = [1, 4, 8]
            size_2 = [4, 1, 8]
            t = smith.jit.trace(fn, (smith.rand([4]), smith.rand([4])))
    ...
    def test_binary_shape_fns_inplace(self):
        def div_inplace_tensor(x: smith.Tensor, y: smith.Tensor):
            z = x.div_(y)
            return z

        def add_inplace_tensor(x: smith.Tensor, y: smith.Tensor):
            z = x.add_(y)
            return z
    ...
    def test_adaptive_avg_pool2d(self):
        ...
        for inp in inps:
            t = smith.randn(*inp[0])
            out_size = smith.nn.functional.adaptive_avg_pool2d(t, inp[1]).size()

            def foo(x):
                return smith.nn.functional.adaptive_avg_pool2d(x, inp[1])

            fn = smith.jit.trace(foo, (t,))
            smith._C._jit_erase_non_input_shape_information(fn.graph)
            smith._C._jit_pass_peephole(fn.graph)
            smith._C._jit_pass_constant_propagation(fn.graph)
    ...
    def test_conv_deconv(self):
        for (
            ...
            ([32, 6, 10], [16, 3, 3], None, 2, 2, 1, 1, 2, smith.nn.functional.conv1d),
            (
                [32, 16, 10],
                [16, 3, 3],
                None,
                ...
                1,
                2,
                smith.nn.functional.conv_transpose1d,
            ),
            (
                [1, 32, 5, 10],
                [30, 16, 3, 3],
            ...
            (
                ...
                1,
                2,
                smith.nn.functional.conv_transpose2d,
            ),
            (
                [3, 14, 10, 66, 55],
                [2, 7, 7, 4, 4],
            ...
            (
                ...
                1,
                2,
                smith.nn.functional.conv_transpose3d,
            ),
        ]:
            inp = smith.rand(inp_shape)
            weight = smith.rand(weight_shape)
            if mod in [
                smith.nn.functional.conv1d,
                smith.nn.functional.conv2d,
                smith.nn.functional.conv3d,
            ]:
            ...
            fn = smith.jit.trace(foo, (inp, weight))
            smith._C._jit_erase_non_input_shape_information(fn.graph)
            smith._C._jit_pass_peephole(fn.graph)
            smith._C._jit_pass_constant_propagation(fn.graph)
    ...
    def test_arange_shape(self):
        ...
        inps = [
            ...
            (0.6, 0.7, 0.8),
            (1, 10, 0.3),
            # (True,),  TODO: https://github.com/blacksmith/blacksmith/issues/63405
            # (False,), TODO: https://github.com/blacksmith/blacksmith/issues/63405
            (0, 5),
            (0, 5, 2),
            (0, 5 + 1e-6),
            (0, 5 - 1e-6),
    ...
    def test_shape_embedding_bag(self):
        ...
        with smith.no_grad():
            ...
            nn_inps = (
                (
                    make_arg((40,), 0, 9),
                    smith.nn.Embedding(20, embedding_dim=64, max_norm=1.0),
                ),
                (make_arg((2, 4), 0, 9), smith.nn.Embedding(10, 20, sparse=True)),
                (make_arg((0,)), smith.nn.Embedding(0, 0, sparse=True)),
                (make_arg((2, 4), 0, 9), smith.nn.Embedding(10, 0, sparse=True)),
                (make_arg((4,), 0, 21), smith.nn.Embedding(22, 5, max_norm=1.0)),
                (
                    make_arg((2,), 0, 1),
                    smith.nn.Embedding.from_pretrained(
                        smith.arange(6.0).view(2, 3),
                        max_norm=2.0,
                        norm_type=0.5,
            ...
            for inp, module in nn_inps:
                ...
                out_size = smith.nn.functional.embedding(inp, **kwargs).size()

                def foo(x):
                    return smith.nn.functional.embedding(inp, **kwargs)

                fn = smith.jit.trace(foo, (inp.detach(),), check_trace=False)
    ...
    def test_shape_concat(self):
        ...
        sample_inputs = sample_inputs_cat_concat(None, "cpu", smith.float, False)
        ...
        class CatMod(nn.Module):
            ...
            def forward(self, x, y):
                return smith.cat([x, y], dim=self.dim)

        for inp in sample_inputs:
            mod = smith.jit.script(CatMod(**inp.kwargs).eval())
    ...
    def test_convolution_backward(self):
        ...
        input = smith.randn(
            (16, 16, 8, 8), dtype=smith.float32, device="cpu", requires_grad=True
        )
        weight = smith.randn(
            (8, 4, 3, 3), dtype=smith.float32, device="cpu", requires_grad=True
        )
        out_grad = smith.randn((16, 8, 8, 8), dtype=smith.float32, device="cpu")
        ...
        def conv_bwd(input, weight, grad):
            ...
            return smith.ops.aten.convolution_backward(
    ...
    def test_returning_input_symbolic_shapes(self):
        mm = smith.jit.freeze(smith.jit.script(nn.Conv2d(16, 33, 3, stride=2).eval()))
        inps = list(mm.graph.inputs())
        inps[1].setType(inps[1].type().with_sizes([None, None, None, None]))
        shape_compute_graph = (
            smith._C._jit_pass_propagate_shapes_on_graph_and_build_compute(mm.graph)
        )
        ...
        func = smith._C._create_function_from_graph("partial_eval_graph", g)
        out = func([20, 16, 5, 10])
        ...
        self.assertEqual(out[4:], list(mm(smith.rand([20, 16, 5, 10])).size()[2:]))
    ...
    def test_partial_eval_stitching(self):
        conv1 = smith.nn.Conv2d(
            3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False
        )
        max_pool = smith.nn.MaxPool2d(
        ...
        mod = smith.jit.freeze(
            smith.jit.script(nn.Sequential(conv1, max_pool, conv2).eval())
        )

        conv1_output = conv1(smith.rand(1, 3, 224, 224))
        max_pool_output = max_pool(conv1_output)
        conv2_output = conv2(max_pool_output)

        shape_compute_graph = (
            smith._C._jit_pass_propagate_shapes_on_graph_and_build_compute(mod.graph)
        )
    ...
    def test_refinement_through_graph_stitching(self):
        class TwoConvs(smith.nn.Module):
        ...
        mod = smith.jit.freeze(smith.jit.script(TwoConvs()).eval())
        inp_tensor = list(mod.graph.inputs())[1]
        inp_tensor.setType(inp_tensor.type().with_sizes([None, None, None, None]))
        smith._C._jit_pass_propagate_shapes_on_graph(mod.graph)
    ...
    def test_stitching_multi_output(self):
        ...
        tensor = smith.rand(1, 3, 224, 224)
        mod = smith.jit.trace(max_pool, (tensor,))
        mod = smith.jit.freeze(mod.eval())
    ...
    def test_sym_ir_parsing(self):
        graph_str1 = """graph(%x.1 : Float(SS(-2), SS(-3))):
                        ...
                        return (%4)"""
        g = smith._C.parse_ir(graph_str1)
        inp = next(g.inputs())
    ...
    def test_stitching_concat(self):
        @smith.jit.script
        def foo1(a, b, x, y):
            return (a / b) + smith.cat([x, y])

        @smith.jit.script
        def foo2(a, b, x, y):
            return (a / b) + smith.cat([x, y], dim=-2)
    ...
    @unittest.skipIf(
        not hasattr(smith.jit, "_shapes"), "shape functions not loaded in python"
    )
    def test_shape_function_includes(self):
        ...
        m1_shape = [10, 20]
        m2_shape = [20, 10]
        res = smith.jit._shapes.matmul(m1_shape, m2_shape)
    ...
    def test_cross_entropy_loss(self):
        @smith.jit.script
        def foo(x, y):
            return smith.ops.aten.cross_entropy_loss(x, y, reduction=0)
    ...
    def test_squeeze_dims(self):
        @smith.jit.script
        def foo(x):
            return smith.ops.aten.squeeze(x, dim=0)
