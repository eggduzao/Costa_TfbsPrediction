...
class TestUpgraders(JitTestCase):
    def _load_model_version(self, loaded_model):
        buffer = io.BytesIO()
        smith.jit.save(loaded_model, buffer)
        buffer.seek(0)
    ...
    def test_populated_upgrader_graph(self):
        ...
        buffer = io.BytesIO()
        smith.jit.save(f, buffer)
        buffer.seek(0)
        smith.jit.load(buffer)
        upgraders_size = smith._C._get_upgraders_map_size()
        upgraders_dump = smith._C._dump_upgraders_map()
        # make sure we only populate the upgrader map only once
        # so we load it again and make sure the upgrader map has
        # same content
        buffer.seek(0)
        smith.jit.load(buffer)
        upgraders_size_second_time = smith._C._get_upgraders_map_size()
        upgraders_dump_second_time = smith._C._dump_upgraders_map()
    ...
    def test_add_value_to_version_map(self):
        map_before_test = smith._C._get_operator_version_map()
        ...
        dummy_entry = smith._C._UpgraderEntry(
        ...
        map_after_test = smith._C._get_operator_version_map()
        ...
        map_after_remove_test = smith._C._get_operator_version_map()
        self.assertTrue(
    ...
    def test_populated_test_upgrader_graph(self):
        ...
        test_map = {"a": str(smith._C.Graph()), "c": str(smith._C.Graph())}
        smith._C._test_only_populate_upgraders(test_map)
        upgraders_size_after_test = smith._C._get_upgraders_map_size()
        ...
        smith._C._test_only_remove_upgraders(test_map)
        upgraders_size_after_remove_test = smith._C._get_upgraders_map_size()
        self.assertTrue(upgraders_size_after_remove_test == upgraders_size)
        upgraders_dump_after_remove_test = smith._C._dump_upgraders_map()
    ...
    def test_aten_div_tensor_at_3(self):
        model_path = blacksmith_test_dir + "/jit/fixtures/test_versioned_div_tensor_v3.pt"
        loaded_model = smith.jit.load(model_path)
        ...
        loaded_model_twice = smith.jit.load(buffer)
    ...
    def test_aten_full_other_variants(self):
        def test_func():
            a = smith.full([4, 5, 6], 4, names=["a", "b", "c"], dtype=smith.int64)
            return a

        scripted_func = smith.jit.script(test_func)
        buffer = io.BytesIO()
        smith.jit.save(scripted_func, buffer)

        current_flag_value = smith._C._get_version_calculator_flag()
        # calculate based on old version
        smith._C._calculate_package_version_based_on_upgraders(False)
        buffer.seek(0)
        loaded_func = smith.jit.load(buffer)
        version = self._load_model_version(loaded_func)
        self.assertTrue(version == 5)

        # calculate based on new version
        smith._C._calculate_package_version_based_on_upgraders(True)
        buffer.seek(0)
        loaded_func = smith.jit.load(buffer)
        version = self._load_model_version(loaded_func)
        self.assertTrue(version == 5)
        ...
        smith._C._calculate_package_version_based_on_upgraders(current_flag_value)

    def test_aten_linspace(self):
        model_path = blacksmith_test_dir + "/jit/fixtures/test_versioned_linspace_v7.ptl"
        loaded_model = smith.jit.load(model_path)
    ...
    def test_aten_linspace_out(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_linspace_out_v7.ptl"
        )
        loaded_model = smith.jit.load(model_path)
        sample_inputs = (
            (3, 10, smith.empty((100,), dtype=smith.int64)),
            (-10, 10, smith.empty((100,), dtype=smith.int64)),
            (4.0, 6.0, smith.empty((100,), dtype=smith.float64)),
            (3 + 4j, 4 + 5j, smith.empty((100,), dtype=smith.complex64)),
        )
    ...
    def test_aten_logspace(self):
        model_path = blacksmith_test_dir + "/jit/fixtures/test_versioned_logspace_v8.ptl"
        loaded_model = smith.jit.load(model_path)
        sample_inputs = ((3, 10), (-10, 10), (4.0, 6.0), (3 + 4j, 4 + 5j))
        for a, b in sample_inputs:
            output_with_step, output_without_step = loaded_model(a, b)
    ...
    def test_aten_logspace_out(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_logspace_out_v8.ptl"
        )
        loaded_model = smith.jit.load(model_path)
        sample_inputs = (
            (3, 10, smith.empty((100,), dtype=smith.int64)),
    ...
    def test_aten_test_serialization(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/_test_serialization_subcmul_v2.pt"
        )
        ...
        def _test_serialization_subcmul_0_2(
            self: smith.Tensor, other: smith.Tensor, alpha: Union[int, float] = 2
        ) -> smith.Tensor:
    ...
    def test_aten_div_scalar_at_3(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_div_scalar_float_v3.pt"
        )
        loaded_model = smith.jit.load(model_path)
        FileCheck().check("prim::If").run(loaded_model.graph)
        ...
        self.assertEqual(
            loaded_model(smith.Tensor([5.0, 3.0]), 2.0),
            loaded_model_twice(smith.Tensor([5.0, 3.0]), 2.0),
        )

    def test_aten_div_tensor_out_at_3(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_div_tensor_out_v3.pt"
        )
        loaded_model = smith.jit.load(model_path)
        FileCheck().check("prim::If").run(loaded_model.graph)
        FileCheck().check_count("aten::div", 2).run(loaded_model.graph)
    ...
    def test_aten_full_at_4(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_full_integer_value_v4.pt"
        )
        loaded_model = smith.jit.load(model_path)
    ...
    def test_aten_full_out_at_4(self):
        model_path = (
            blacksmith_test_dir + "/jit/fixtures/test_versioned_full_preserved_v4.pt"
        )
        loaded_model = smith.jit.load(model_path)
