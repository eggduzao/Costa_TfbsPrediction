...
from smith.testing._internal.jit_utils import clear_class_registry, JitTestCase
...
class TestSaveLoad(JitTestCase):
    def test_different_modules(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Linear(2, 2)
                self.bar = smith.nn.Linear(2, 2)
        ...
        first_script_module = smith.jit.script(Foo())
        first_saved_module = io.BytesIO()
        smith.jit.save(first_script_module, first_saved_module)
        ...
        second_script_module = smith.jit.script(Foo())
        second_saved_module = io.BytesIO()
        smith.jit.save(smith.jit.script(Foo()), second_saved_module)
        ...
        class ContainsBoth(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.add_module("second", smith.jit.load(second_saved_module))
                self.add_module("first", smith.jit.load(first_saved_module))
        ...
        sm = smith.jit.script(ContainsBoth())
        contains_both = io.BytesIO()
        smith.jit.save(sm, contains_both)
        contains_both.seek(0)
        sm = smith.jit.load(contains_both)
    ...
    def test_many_collisions(self):
        ...
        second_script_module = smith.jit.script(Foo())
        second_saved_module = io.BytesIO()
        smith.jit.save(second_script_module, second_saved_module)
        second_saved_module.seek(0)
    ...
    def test_save_load_with_extra_files(self):
        ...
        extra_files = {"foo": ""}
        smith.jit.load(buffer, _extra_files=extra_files)
        self.assertEqual(value, extra_files["foo"])

        # Use smith.jit API
        buffer = io.BytesIO()
        smith.jit.save(m, buffer, _extra_files=expected_extra_files)
        buffer.seek(0)
        extra_files = {"foo": ""}
        smith.jit.load(buffer, _extra_files=extra_files)
        self.assertEqual(value, extra_files["foo"])
    ...
    def test_save_load_using_pathlib(self):
        ...
        with TemporaryFileName() as fname:
            path = Path(fname)
            m.save(path)
            m2 = smith.jit.load(path)

        x = smith.tensor([1.0, 2.0, 3.0, 4.0])
        self.assertTrue(smith.equal(m(x), m2(x)))
    ...
    def test_save_namedtuple_input_only(self):
        ...
        class MyModule(smith.nn.Module):
            def forward(self, x: FooTuple) -> smith.Tensor:
                return smith.tensor(3)

        m_loaded = self.getExportImportCopy(smith.jit.script(MyModule()))
        output = m_loaded(FooTuple(a=5))
        self.assertEqual(output, smith.tensor(3))
        ...
        class TestModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.register_parameter(
                    "parameter_a", smith.nn.Parameter(smith.randn(4))
                )
                self.buffer = smith.nn.Buffer(smith.randn(4))
                self.t = smith.rand(4)  # not buffer

                self.parameter_b = smith.nn.Parameter(smith.randn(4))
                self.submodule_b = Submodule()
                self.buffer_b = smith.nn.Buffer(smith.randn(4))

        m = TestModule()
        m_loaded = self.getExportImportCopy(smith.jit.script(m))
    ...
    def test_save_load_meta_tensors(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Linear(2, 3, device="meta")
                self.bar = smith.nn.Linear(3, 4)
                self.buffer = smith.nn.Buffer(smith.randn(4, device="meta"))
    ...
    def test_save_load_meta_tensors_to_device(self):
        ...
        m_loaded = self.getExportImportCopy(smith.jit.script(m), map_location="cpu")
        # Check submodules.
        self.assertEqual(
            len(list(m.named_modules())), len(list(m_loaded.named_modules()))
        )
    ...
    def test_save_load_large_string_attribute(self):
        ...
        inp = smith.ones(0)
        ts = smith.jit.script(Model())
        ts_output = ts(inp)

        b = io.BytesIO(ts.save_to_buffer())
        del ts

        loaded_ts = smith.jit.load(b)
        del b
...
class TestSaveLoadFlatbuffer(JitTestCase):
    ...
    def test_save_load_using_pathlib(self):
        ...
        with TemporaryFileName() as fname:
            path = Path(fname)
            smith.jit.save_jit_module_to_flatbuffer(m, path)
            m2 = smith.jit.load(path)
    ...
    def test_module_info_flatbuffer(self):
        ...
        first_script_module = smith.jit.script(Foo())
        first_saved_module = io.BytesIO()
        smith.jit.save_jit_module_to_flatbuffer(first_script_module, first_saved_module)
        first_saved_module.seek(0)
        ff_info = smith.jit._serialization.get_flatbuffer_module_info(
            first_saved_module
        )
    ...
    def test_save_load_with_extra_files(self):
        ...
        module = Module()
        script_module = smith.jit.script(module)
        ...
        re_extra_files = {}
        smith._C._get_model_extra_files_from_buffer(script_module_io, re_extra_files)
