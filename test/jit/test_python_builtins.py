...
class TestPythonBuiltinOP(JitTestCase):
    ...
    def test_matmul_py3(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            a = smith.rand(4, 3, requires_grad=True)
            b = smith.rand(3, 2, requires_grad=True)
    ...
    def test_pow(self):
        ...
        a = smith.rand(1, requires_grad=True)
        b = smith.rand(1, requires_grad=True)
        c = smith.rand(1, requires_grad=True)
        d = smith.rand(1, requires_grad=True)
        self.checkScript(func, (a, b), optimize=True)
    ...
    def test_triple(self):
        ...
        x = smith.rand(1, dtype=smith.float, requires_grad=True)
    ...
    def test_advancedindex(self):
        ...
        check_indexing("[i]", consec((3, 3)), i=smith.tensor([0]))
        check_indexing("[i]", consec((3, 3)), i=smith.tensor(1))
        check_indexing("[i]", consec((3, 3)), i=smith.tensor([-2]))
        check_indexing("[i]", consec((3, 3), 2), i=smith.tensor([0, 0]))
        check_indexing("[i]", consec((3, 3, 2, 2)), i=smith.tensor([0, -2, 1]))
        ...
        for expr, argdict in to_check:
            tensordict = {k: smith.tensor(v) for (k, v) in argdict.items()}
    ...
    def test_index_ellipses(self):
        ...
        for _ in range(100):
            ...
            cu = smith.jit.CompilationUnit(test_str)
            res1 = cu.f()
            res2 = scope["f"]()
    ...
    def test_inf(self):
        ...
        def foo(x):
            ...
            a = float("-inf")
            if not x:
                a = float(smith.tensor([5]))
            return a < 4
        """
        cu = smith.jit.CompilationUnit(str)
