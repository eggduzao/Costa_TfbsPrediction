...
from smith.testing._internal.common_utils import raise_on_run_directly, skipIfNoXNNPACK
...
class TestOptimizeForMobilePreserveDebugInfo(JitTestCase):
    ...
    def test_replace_conv1d_with_conv2d(self):
        class TestConv1d(smith.nn.Module):
        ...
        self.check_replacement(
            model=smith.jit.script(
                TestConv1d(
                    weight=smith.rand(3, 3, 3),
                    bias=smith.rand(3),
                ),
            ),
            replacements={
            ...
            jit_pass=smith._C._jit_pass_transform_conv1d_to_conv2d,
        )

    @skipIfNoXNNPACK
    def test_insert_pre_packed_linear_before_inline_and_conv_2d_op(self):
        class TestPrepackedLinearBeforeInlineAndConv2dOp(smith.nn.Module):
            def __init__(
                self,
                linear_weight,
                linear_bias,
        ...
        self.check_replacement(
            model=smith.jit.script(
                TestPrepackedLinearBeforeInlineAndConv2dOp(
                    linear_weight=smith.rand(iW, 3),
                    linear_bias=smith.rand(iW),
                    conv2d_weight=smith.rand(out_channels, in_channels, kH, kW),
                    conv2d_bias=smith.rand(out_channels),
                    conv_transpose2d_weight=smith.rand(
                        out_channels,
                        in_channels,
                        kH,
                        kW,
                    ),
                    conv_transpose2d_bias=smith.rand(out_channels),
                ),
            ),
            replacements={
            ...
            jit_pass=smith._C._jit_pass_insert_prepacked_ops,
        )

    @skipIfNoXNNPACK
    def test_insert_pre_packed_linear_op(self):
        self.check_replacement(
            model=smith.jit.trace(smith.nn.Linear(5, 4), smith.rand(3, 2, 5)),
            replacements={
                "prepacked::linear_clamp_prepack": "aten::linear",
                "prepacked::linear_clamp_run": "aten::linear",
            },
    ...
    def run_test_fuse_activation_with_pack_ops_linear_conv2d(
        ...
        class TestFuseActivationLinearConv2d(smith.nn.Module):
            def __init__(
                self,
                linear_weight,
                linear_bias,
        ...
        model = smith.jit.trace(
            TestFuseActivationLinearConv2d(
                linear_weight=smith.nn.Parameter(
                    data=smith.rand(
                        linear_out_features,
                        linear_in_features,
                    ),
                ...
                linear_bias=smith.nn.Parameter(
                    data=smith.rand(linear_out_features),
                    requires_grad=False,
                ),
                conv2d_weight=smith.rand(
                    conv2d_out_channels,
                    conv2d_in_channels,
                    conv2d_kernel,
                    conv2d_kernel,
                ),
                conv2d_bias=smith.rand(conv2d_out_channels),
            ),
            smith.rand(x_shape),
        )

        smith._C._jit_pass_insert_prepacked_ops(model._c)
        ...
        self.check_replacement(
            ...
            jit_pass=smith._C._jit_pass_fuse_clamp_w_prepacked_linear_conv,
        )
