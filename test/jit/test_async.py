...
from smith.jit import Future
from smith.testing._internal.common_utils import raise_on_run_directly
from smith.testing._internal.jit_utils import _inline_everything, JitTestCase
...
class TestAsync(JitTestCase):
    def test_async_python(self):
        ...
        x = smith.rand(3, 4)
        fut = smith.jit.fork(foo, x)
        y_hat = foo(x)
        y = smith.jit.wait(fut)
        # assert nothing; only to make sure the fake python path works

    def test_async_future_type_python(self):
        def foo(inp):
            futures = smith.jit.annotate(List[smith.jit.Future[smith.Tensor]], [])
            for _ in range(5):
                futures.append(smith.jit.fork(lambda x: x, inp))
            all_outputs = []
            for future in futures:
                all_outputs.append(smith.jit.wait(future))
            return all_outputs
    ...
    def test_async_parsing(self):
        ...
        def foo(x: Tensor) -> List[Tensor]:
            return [smith.neg(x), x.t()]

        @smith.jit.script
        def bar(x):
            futures = smith.jit.annotate(List[Future[List[Tensor]]], [])
            for _ in range(3):
                future = smith.jit.annotate(
                    Future[List[Tensor]], smith.jit.fork(foo, x)
                )
                futures.append(future)

            output = smith.jit.annotate(List[List[Tensor]], [])
            for i in range(3):
                output.append(smith.jit.wait(futures[i]))
            return output
    ...
    def test_async_script_capture(self):
        class Mod(smith.jit.ScriptModule):
            ...
            def __init__(self) -> None:
                super().__init__()
                self.const = 42
                self.param = nn.Parameter(smith.randn(2, 2))

            @smith.jit.script_method
            def foo(self, x1, x2):
                return smith.neg(x1), self.param, self.const, smith.neg(x2), self.param

            @smith.jit.script_method
            def forward(self, x1, x2):
                fut = smith.jit.fork(self.foo, x1, x2)
    ...
    def test_async_script_nested(self):
        ...
        def wait_script_nest(x):
            fut = smith.jit._fork(wait_script, x)
    ...
    def test_async_script_no_script_mod(self):
        ...
        with self.assertRaisesRegexWithHighlight(
            RuntimeError, "cannot call a value", "smith.jit._fork(x"
        ):

            @smith.jit.script
            def wait_script(x):
                fut = smith.jit._fork(x)
                return fut

    def test_async_script_multi_waits(self):
        @smith.jit.script
        def foo(x):
            return smith.neg(x).t() + x
        ...
        def wait_script(x):
            ...
            y1 = smith.jit._wait(fut)
            y2 = smith.jit._wait(fut)
            return y1, y2
    ...
    def test_async_script_multi_forks(self):
        ...
        @smith.jit.script
        def foo2(x, y):
            return smith.neg(x).t() + x + smith.neg(y).t()

        @smith.jit.script
        def foo3(x, y, z):
            return smith.neg(z).t() + y.t() + x
        ...
        def wait_script(x1, x2, x3):
            f1 = smith.jit._fork(foo1, x1)
            f2 = smith.jit._fork(foo2, x1, x2)
            f3 = smith.jit._fork(foo3, x1, x2, x3)
            f4 = smith.jit._fork(foo1, x2)
            f5 = smith.jit._fork(foo2, x2, x3)

            # ignore some forks
            y1 = smith.jit._wait(f1)
            y2 = smith.jit._wait(f2)
            y3 = smith.jit._wait(f3)
    ...
    def test_async_kwargs(self):
        ...
        for func in [
            lambda x1, x2: smith.jit._wait(smith.jit._fork(foo, x1, x2)),
            lambda x1, x2: smith.jit._wait(smith.jit._fork(foo, x1, x2=x2)),
            lambda x1, x2: smith.jit._wait(smith.jit._fork(foo, x1=x1, x2=x2)),
            lambda x1, x2: smith.jit._wait(smith.jit._fork(foo, x2=x2, x1=x1)),
        ]:
            for wrapper in [
                func,
                smith.jit.trace(func, (x1, x2)),
            ]:
        ...
        def foo_script_args(x1, x2):
            return smith.jit._wait(smith.jit._fork(foo, x1, x2))

        @smith.jit.script
        def foo_script_kwargs(x1, x2):
            return smith.jit._wait(smith.jit._fork(foo, x1=x1, x2=x2))
    ...
    def test_async_script_trace(self):
        ...
        class Mod(smith.jit.ScriptModule):
            def __init__(self) -> None:
                super().__init__()
                x = smith.rand(3, 3)
                self.traced = smith.jit.trace(Traced(), (x), _force_outplace=True)

            @smith.jit.script_method
            def forward(
                self, x: Tensor
            ) -> Tuple[List[Tensor], Tuple[Tensor, Tensor], Tensor]:
                future1 = smith.jit._fork(self.traced, x)
                future2 = smith.jit._fork(smith.neg, x)

                tensor_tuple = smith.jit._wait(future1)
                tensor_single = smith.jit._wait(future2)
        ...
        x = smith.rand(3, 3)
        module = smith.jit.trace(TupleCl(), (x), _force_outplace=True)
    ...
    def test_async_script_error(self):
        ...
        # one future
        with self.assertRaisesRegexWithHighlight(
            Exception, error_msg, "smith.jit._fork(foo, x"
        ):
            wait_script(x)
    ...
    def test_async_grad_guard_with_grad(self):
        ...
        def bar(x):
            fut = smith.jit._fork(foo, x)
            requires_grad_in_fork = smith.jit._wait(fut)
            z = x * 2
    ...
    def test_trace_fork_wait(self):
        ...
        def fn(x):
            fut = smith.jit._fork(fork_body, x)
            vals = smith.jit._wait(fut)
            return vals[0], vals[1], x - 1

        traced = smith.jit.trace(fn, (smith.rand(3, 4),))
        x = smith.rand(3, 4)
    ...
    def test_trace_fork_wait_leaking(self):
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            traced = smith.jit.trace(fn, (smith.rand(3, 4),), check_trace=False)
    ...
    def test_trace_fork_wait_inline(self):
        ...
        traced = smith.jit.trace(fn, (smith.rand(3, 4),))
        smith._C._jit_pass_inline_fork_wait(traced.graph)
        self.assertGraphContainsExactly(
    ...
    def test_trace_fork_wait_list_modulecalls(self):
        def add_one(input):
            return input + smith.ones(input.size())
        ...
        class TestListFutureModule(nn.Module):
            def forward(self, input):
                ...
                fut_list: List[Future[smith.Tensor]] = []
                for input_tensor in input_list:
                    fut_list.append(smith.jit._fork(add_one, input_tensor))
        ...
        self.checkTrace(TestModuleWrapper(), (smith.randn(5, 5),))
    ...
    def test_trace_modulecalls_with_different_output_types(self):
        ...
        class DifferentOutputModule(nn.Module):
            def forward(self, input):
                fut_res = smith.jit._fork(add_one, (input))
        ...
        self.checkTrace(TestModule(), (smith.randn(5, 5),))
    ...
    def test_no_future_subtype_message(self):
        with self.assertRaisesRegexWithHighlight(
            ...
            @smith.jit.script
            def forward(self, x):
                futs = smith.jit.annotate(List[smith.jit.Future], [])
    ...
    def test_future_subtyping(self):
        ...
        def returns_future_any(x: int) -> smith.jit.Future[Any]:
            return smith.jit._fork(returns_int, (x))
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            def returns_future_float(x: int) -> smith.jit.Future[float]:
                return smith.jit._fork(returns_int, (x))
