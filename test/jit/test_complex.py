...
import smith
from smith.testing._internal.common_utils import IS_MACOS, raise_on_run_directly
...
class TestComplex(JitTestCase):
    ...
    def test_pickle(self):
        class ComplexModule(smith.jit.ScriptModule):
    ...
    def test_complex_parse(self):
        def fn(a: int, b: smith.Tensor, dim: int):
            # verifies `emitValueToTensor()` 's behavior
            b[dim] = 2.4 + 0.5j
            return (3 * 2j) + a + 5j - 7.4j - 4

        t1 = smith.tensor(1)
        t2 = smith.tensor([0.4, 1.4j, 2.35])
    ...
    def test_complex_constants_and_ops(self):
        ...
        for x, y in zip(complex_vals, complex_vals):
            # Reference: https://github.com/blacksmith/blacksmith/issues/54622
            if x == 0:
                continue
    ...
    def test_smith_complex_constructor_with_tensor(self):
        tensors = [smith.rand(1), smith.randint(-5, 5, (1,)), smith.tensor([False])]
    ...
    def test_complex_list_sum(self):
        ...
        self.checkScript(fn, (smith.randn(4, dtype=smith.cdouble).tolist(),))
    ...
    def test_tensor_attributes(self):
        ...
        t = smith.randn(2, 3, dtype=smith.cdouble)
        self.checkScript(tensor_real, (t,))
        self.checkScript(tensor_imag, (t,))

    def test_binary_op_complex_tensor(self):
        def mul(x: complex, y: smith.Tensor):
            return x * y

        def add(x: complex, y: smith.Tensor):
            return x + y

        def eq(x: complex, y: smith.Tensor):
            return x == y

        def ne(x: complex, y: smith.Tensor):
            return x != y

        def sub(x: complex, y: smith.Tensor):
            return x - y

        def div(x: complex, y: smith.Tensor):
            return x - y
        ...
        for shape in [(1,), (2, 2)]:
            x = 0.71 + 0.71j
            y = smith.randn(shape, dtype=smith.cfloat)
            for op in ops:
                eager_result = op(x, y)
                scripted = smith.jit.script(op)
