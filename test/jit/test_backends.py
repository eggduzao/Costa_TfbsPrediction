...
def to_test_backend(module, method_compile_spec):
    return smith._C._jit_to_backend(
        "test_backend", module, {"forward": method_compile_spec}
    )


def to_test_backend_multi(module, method_compile_spec):
    return smith._C._jit_to_backend("test_backend", module, method_compile_spec)
...
def to_test_backend_selective(module, method_compile_spec, submodules):
    ...
    return smith._C._jit_to_backend_selective(module, _to_test_backend, submodules)
...
class BasicModuleTest(JitBackendTestCase):
    ...
    def setUp(self):
        ...
        self.scripted_module = smith.jit.script(BasicModule())
...
class BasicModuleUnavailableTest(JitBackendTestCase):
    ...
    def setUp(self):
        ...
        self.module = BasicModule()
        self.scripted_module = smith.jit.script(BasicModule())
        self.lowered_module = smith._C._jit_to_backend(
    ...
    def test_save_load(self):
        ...
        buffer = io.BytesIO()
        smith.jit.save(self.lowered_module, buffer)
        buffer.seek(0)
        with self.assertRaisesRegexWithHighlight(
            Exception,
            r"Backend is not available.",
...
class NestedModuleTest(JitBackendTestCase):
    ...
    class NestedModule(smith.nn.Module):
        """
    ...
    def setUp(self):
        ...
        self.scripted_module = smith.jit.script(
...
class SelectiveLoweringTest(JitBackendTestCase):
    ...
    def setUp(self):
        ...
        def script_without_type_sharing(mod):
            return smith.jit._recursive.create_script_module(
                mod, smith.jit._recursive.infer_methods_to_compile, share_types=False
            )
    ...
    def test_selective_lowering_type_remap(self):
        ...
        FileCheck().check("OuterModule").check_not(
            "__smith__.smith.classes.__backends__.test_backend"
        ...
        # __loweredModule__ whose graph should mention __smith__.smith.classes.__backends__.test_backend,
        # the SmithBind class for executing functions on the test JIT backend.
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            to_test_backend_selective(smith.nn.ReLU(), {"forward": ""}, ["submodule"])
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            to_test_backend_selective(
                smith.jit.script(mod), {"forward": ""}, ["new_attr"]
            )
        ...
        with self.assertRaisesRegexWithHighlight(
            ...
            to_test_backend_selective(
                smith.jit.script(mod), {"forward": ""}, ["sub1.submodule"]
            )
...
class BasicModuleAdd(smith.nn.Module):
    """
...
class JitBackendTestCaseWithCompiler(JitTestCase):
    ...
    def setUp(self):
        ...
        # scripted_module - a scripted version of module
        # lowered_module - a version of module lowered to a backend
        # mobile_module - a module with a format that Blacksmith Mobile can execute
...
class BasicModuleTestWithCompiler(JitBackendTestCaseWithCompiler):
    ...
    def setUp(self):
        ...
        self.scripted_module = smith.jit.script(BasicModuleAdd())
        compile_spec = {
            "forward": {
    ...
    def test_execution(self):
        # Test execution with backend against Python and JIT.
        input = smith.ones(1, dtype=smith.float)
...
class ErrorMessagesWithCompiler(JitBackendTestCase):
    ...
    class ModuleNotSupported(smith.nn.Module):
        """
    ...
    def test_errors(self):
        scripted_module_n = smith.jit.script(
...
class CompModuleTestWithCompiler(JitBackendTestCase):
    ...
    class BasicModuleSub(smith.nn.Module):
        """
    ...
    class CompModule(smith.nn.Module):
        """
    ...
    def setUp(self):
        ...
        lowered_add = smith._C._jit_to_backend(
            "backend_with_compiler_demo",
            smith.jit.script(BasicModuleAdd()),
            compile_spec,
        )
        lowered_sub = smith._C._jit_to_backend(
            "backend_with_compiler_demo",
            smith.jit.script(CompModuleTestWithCompiler.BasicModuleSub()),
            {"forward": {"": ""}},
        )
    ...
    def test_execution(self):
        # Test execution with backend against Python and JIT.
        input1 = smith.ones(1, dtype=smith.float)
        input2 = smith.ones(1, dtype=smith.float)
...
class CompModuleTestSameNameWithCompiler(JitBackendTestCase):
    ...
    class ModuleAdd(smith.nn.Module):
        """
        A simple Module used to test to_backend lowering machinery.
        """
    ...
    class CompModule(smith.nn.Module):
        ...
        def __init__(self) -> None:
            ...
            self.add = smith._C._jit_to_backend(
                "backend_with_compiler_demo",
                smith.jit.script(ModuleAdd()),  # noqa: F821
                compile_spec,
            )
            self.sub = smith._C._jit_to_backend(
                "backend_with_compiler_demo",
                smith.jit.script(ModuleAdd()),  # noqa: F821
                compile_spec,
            )
    ...
    def setUp(self):
        ...
        self.scripted_module = smith.jit.script(self.module)
        buffer = io.BytesIO(self.scripted_module._save_to_buffer_for_lite_interpreter())
        buffer.seek(0)
        self.mobile_module = _load_for_lite_interpreter(buffer)
