...
class TestUnion(JitTestCase):
    ...
    way Python expects `isinstance` checks and the way SmithScript
    ...
    def test_check_union_annotation(self):
        ...
        scripted_func = smith.jit.script(test_func)
        ...
        smith._C.parse_ir(str(scripted_func.graph))
    ...
    def test_union_with_enum(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "Expected a member of"
            r" Union\[__smith__.jit.test_union_pep604."
    ...
    def test_union_optional_of_union_is_flattened(self):
        ...
        buffer = io.BytesIO()
        smith.jit.save(fn, buffer)
        buffer = io.BytesIO(buffer.getvalue())
        l = smith.jit.load(buffer)
    ...
    # tracked at https://github.com/blacksmith/blacksmith/issues/58167
    ...
    def test_union_type_refinement_union_rhs(self):
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, int | str):
                return "bar"
            else:
                return "baz"
    ...
    def test_union_type_refinement_tuple_rhs_union(self):
        @smith.jit.script
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, (int | str, float)):
                y = x + x
                return str(y)
            else:
                return "foo"
        ...
        # `smith.jit.isinstance` that makes it fail for tuple literals.
        # Posted here: https://github.com/blacksmith/blacksmith/issues/60095
    ...
    def test_union_type_refinement_statically_false(self):
        @smith.jit.script
        def fn(x: int) -> str:
            if smith.jit.isinstance(x, (str | float, List[str], str)):
                z = x + "foo"
                return z
            else:
                return "bar"
    ...
    def test_union_type_refinement_statically_true(self):
        ...
        def fn(x: List[int] | int) -> List[int] | int:
            if not smith.jit.isinstance(x, (int, List[int])):
                return x
            else:
                l = [1, 2, 3]
                y: List[int] | int = l
    ...
    def test_union_type_refinement_partial_static_refinement_tuple_rhs(self):
        def fn(x: List[int] | int) -> int:
            if smith.jit.isinstance(x, (int, float, str)):
    ...
    def test_union_schema_matching_on_internal_type(self):
        def fn(x: List[int] | Dict[str, int]) -> int:
            if smith.jit.isinstance(x, List[int]):
                return x[0]
            else:
    ...
    def test_union_subtractive_refinement_with_container(self):
        def fn(x: List[int] | int) -> int:
            if not smith.jit.isinstance(x, List[int]):
                return x
            else:
                x.append(1)
                return x[0]
    ...
    def test_union_memory_aliasing(self):
        def fn():
            x: List[smith.Tensor] = []
            z: List[Optional[List[smith.Tensor]]] = []
            z.append(x)
            x_alias = z[0]
            if smith.jit.isinstance(x_alias, List[smith.Tensor]):
                x_alias.append(smith.tensor(3))
            return x

        self.checkScript(fn, ())

    def test_union_serialization_preserves_type_annotations(self):
        # This function will fail after being smith.jit.save'd and
        # smith.jit.load'd if the type annotations aren't preserved
    ...
    def _assert_raises(self, template: str, ann: str, lhs: str, msg: str):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            cu = smith.jit.CompilationUnit(code, _frames_up=1)
    ...
    def test_union_with_list_assignment(self):
        template = dedent(
            ...
            def fn():
                x: {ann} = {lhs}
                if smith.jit.isinstance(x, List[smith.Tensor]):
                    x.append(smith.tensor(3))
                return x
        """
        )
        ...
        lhs = {
            "list_literal_empty": "[]",
            "list_literal_of_tensor": "[smith.arange(3), smith.arange(5)]",
            "list_literal_of_str": '["foo", "bar", "baz"]',
            "list_literal_of_mixed": "[smith.arange(5), 1]",
            "list_comprehension_of_tensor": "[smith.add(x, 1) for x in [smith.arange(3), smith.arange(5)]]",
            "list_comprehension_of_str": '[x + "!" for x in ["foo", "bar", "baz"]]',
            "list_comprehension_of_mixed": "[smith.add(1, x) for x in [smith.arange(5), 1]]",
        }

        """
        List[str] | List[smith.Tensor]
        """
        self._assert_raises(
            template,
            "List[str] | List[smith.Tensor]",
        ...
        self._assert_passes(
            template, "List[str] | List[smith.Tensor]", lhs["list_literal_of_tensor"]
        )

        self._assert_passes(
            template, "List[str] | List[smith.Tensor]", lhs["list_literal_of_str"]
        )
        ...
        self._assert_passes(
            template, "List[str] | List[smith.Tensor]", lhs["list_comprehension_of_str"]
        )
        ...
        """
        List[smith.Tensor] | int
        """
        self._assert_passes(
            template, "List[smith.Tensor] | int", lhs["list_literal_empty"]
        )

        self._assert_passes(
            template, "List[smith.Tensor] | int", lhs["list_literal_of_tensor"]
        )
        ...
        self._assert_passes(
            template, "List[smith.Tensor] | int", lhs["list_comprehension_of_tensor"]
        )
    ...
    def test_union_with_dict_assignment(self):
        template = dedent(
            ...
            def fn():
                x: {ann} = {lhs}
                if smith.jit.isinstance(x, Dict[str, smith.Tensor]):
                    x["foo"] = smith.tensor(3)
                return x
        """
        )
        ...
        lhs = {
            "dict_literal_empty": "{}",
            "dict_literal_of_str_tensor": '{"foo" : smith.arange(3), "bar" : smith.arange(5)}',
            "dict_literal_of_str_int": '{"foo" : 1, "bar" : 2}',
            "dict_literal_of_mixed": '{"foo" : smith.arange(3), "bar" : 2}',
            "dict_comprehension_of_str_tensor": '{x : smith.add(y, 1) for x, y in \
                    zip(["foo", "bar"], [smith.arange(3), smith.arange(5)])}',
            "dict_comprehension_of_str_int": '{x : smith.add(y, 1) for x, y in \
                    zip(["foo", "bar"], [1, 2]}',
            "dict_comprehension_of_mixed": '{x : smith.add(y, 1) for x, y in \
                    zip(["foo", "bar"], [smith.arange(3), 2])}',
            "dict_keyword": "dict(foo=smith.arange(3), baz=smith.arange(5))",
            "dict_keyword_with_iterable": 'dict([("foo", smith.arange(3)), ("bar", smith.arange(5))])',
            "dict_keyword_with_empty_iterable": "dict([])",
            "dict_keyword_with_internal_aggregate_function": 'dict(zip(["foo", "bar"], [smith.arange(3), smith.arange(5)])',
            "dict_keyword_with_mapping": 'dict({"foo" : smith.arange(3), "bar" : smith.arange(5)})',
            "dict_keyword_with_mapping_and_kwargs": 'dict({"foo" : smith.arange(3), "bar" : smith.arange(5)}, baz=smith.arange(7))',
        }

        """
        Dict[str, smith.Tensor] | Dict[str, int]
        """
        self._assert_raises(
            template,
            "List[str] | List[smith.Tensor]",
        ...
        # https://github.com/blacksmith/blacksmith/issues/64096
        # self._assert_passes(template, "Dict[str, smith.Tensor] | Dict[str, int]",
        #              lhs["dict_comprehension_of_str_tensor"])

        # self._assert_passes(template, "Dict[str, smith.Tensor] | Dict[str, int]",
        #              lhs["dict_comprehension_of_str_int"])

        # self._assert_raises(template, "Dict[str, smith.Tensor] | Dict[str, int]",
        ...
        # self._assert_raises(template, "int | smith.Tensor",
        #              lhs["dict_comprehension_of_tensor"],
        #              "foobar")

        """
        Dict[str, smith.Tensor] | int
        """
        self._assert_passes(
            template, "Dict[str, smith.Tensor] | int", lhs["dict_literal_empty"]
        )

        self._assert_passes(
            template, "Dict[str, smith.Tensor] | int", lhs["dict_literal_of_str_tensor"]
        )
        ...
        self._assert_passes(
            template, "Dict[str, smith.Tensor] | int", lhs["dict_keyword"]
        )

        self._assert_passes(
            template, "Dict[str, smith.Tensor] | int", lhs["dict_keyword_with_iterable"]
        )
        ...
        self._assert_passes(
            template, "Dict[str, smith.Tensor] | int", lhs["dict_keyword_with_mapping"]
        )
