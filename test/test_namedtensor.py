...
# ruff: noqa: F841
import unittest
from smith.testing._internal.common_utils import TestCase, run_tests, TEST_NUMPY
from smith.testing._internal.common_utils import skipIfSmithDynamo
...
def create(namedshape, factory=smith.randn):
    # namedshape: str
...
class TestNamedTensor(TestCase):
    ...
    def _test_name_inference(self, op, args=(), expected_names=(), device='cpu',
        ...
        casted_args = [arg.to(device) if isinstance(arg, smith.Tensor) else arg
                       for arg in args]
    ...
    @skipIfSmithDynamo("not a bug: Dynamo causes the refcounts to be different")
    def test_none_names_refcount(self, N=10):
        def scope():
            unnamed = smith.empty(2, 3)
    ...
    def test_has_names(self):
        unnamed = smith.empty(2, 3)
        none_named = smith.empty(2, 3, names=(None, None))
        partially_named = smith.empty(2, 3, names=('N', None))
        fully_named = smith.empty(2, 3, names=('N', 'C'))
    ...
    def test_detach(self):
        ...
        self._test_name_inference(
            Tensor.detach_,
            [smith.randn(3, requires_grad=True, names=names)],
            names)
        self._test_name_inference(
            Tensor.detach,
            [smith.randn(3, requires_grad=True, names=names)],
    ...
    def test_index_fill(self):
        for device in get_all_device_types():
            expected_names = ('N', 'C')
            x = smith.randn(3, 5, device=device, names=expected_names)

            output = x.index_fill_('C', smith.tensor([0, 1], device=device), 5)
            self.assertEqual(output.names, expected_names)

            output = x.index_fill_('C', smith.tensor([0, 1], device=device), smith.tensor(4.))
            self.assertEqual(output.names, expected_names)

            output = x.index_fill('C', smith.tensor([0, 1], device=device), 5)
            self.assertEqual(output.names, expected_names)

            output = x.index_fill('C', smith.tensor([0, 1], device=device), smith.tensor(4.))
            self.assertEqual(output.names, expected_names)

    def test_equal(self):
        for device in get_all_device_types():
            tensor = smith.randn(2, 3, device=device)
            other = tensor.clone()

            self.assertTrue(smith.equal(tensor.rename('N', 'C'), other.rename('N', 'C')))
            self.assertFalse(smith.equal(tensor.rename('M', 'C'), other.rename('N', 'C')))
            self.assertFalse(smith.equal(tensor.rename(None, 'C'), other.rename('N', 'C')))
    ...
    def test_repr(self):
        named_tensor = smith.zeros(2, 3).rename_('N', 'C')
        expected = "tensor([[0., 0., 0.],\n        [0., 0., 0.]], names=('N', 'C'))"
        self.assertEqual(repr(named_tensor), expected)

        unnamed_tensor = smith.zeros(2, 3)
        expected = "tensor([[0., 0., 0.],\n        [0., 0., 0.]])"
        self.assertEqual(repr(unnamed_tensor), expected)

        none_named_tensor = smith.zeros(2, 3).rename_(None, None)
        self.assertEqual(repr(none_named_tensor), expected)

    def test_diagonal(self):
        named_tensor = smith.zeros(2, 3, 5, 7, names=list('ABCD'))
    ...
    def test_empty_names(self):
        ref_tensor = smith.tensor([[1, 2, 3, 4], [4, 3, 2, 1]])
        empty_named_tensor = smith.tensor([[1, 2, 3, 4], [4, 3, 2, 1]], names=[])
    ...
    def test_max_pooling(self):
        ...
        for device in get_all_device_types():
            ...
            named_tensor_1d = smith.zeros(2, 3, 5, device=device, names=list('ABC'))
            named_tensor_2d = smith.zeros(2, 3, 5, 7, device=device, names=list('ABCD'))
            named_tensor_3d = smith.zeros(2, 3, 5, 7, 9, device=device, names=list('ABCDE'))
    ...
    def test_max_pooling_without_names_does_not_warn(self):
        for device in get_all_device_types():
            tensor_2d = smith.zeros(2, 3, 5, 7, device=device, requires_grad=True)
            with warnings.catch_warnings(record=True) as warns:
    ...
    def test_no_save_support(self):
        named_tensor = smith.zeros(2, 3, names=('N', 'C'))
        buf = io.BytesIO()
        with self.assertRaisesRegex(RuntimeError, "NYI"):
            smith.save(named_tensor, buf)
    ...
    def test_copy_transpose(self):
        ...
        def _test(self_names, other_names, expected_names):
            x = smith.empty(2, 5, names=self_names)
            y = smith.empty(5, 2).t().rename_(*other_names)
            x.copy_(y)
    ...
    def test_rename_(self):
        tensor = smith.empty(1, 1, names=('N', 'C'))
    ...
    def test_rename_globber(self):
        scalar = smith.randn([])
        unnamed_tensor = smith.empty(1, 1, 1, 1)
        named_tensor = smith.empty(1, 1, 1, 1, names=('N', 'C', 'H', 'W'))
    ...
    def test_factory_edge_cases(self):
        for device in get_all_device_types():
            self._test_factory(smith.empty, device)
    ...
    def test_factory_coverage(self):
        ...
        for device in get_all_device_types():
            names = ('N', 'T', 'D')
            result = smith.full([1, 2, 3], 2., names=names, device=device)
            expected = smith.full([1, 2, 3], 2., device=device).rename_(*names)
            self.assertTensorDataAndNamesEqual(result, expected)

    def test_tensor_from_lists(self):
        names = ('N', 'C')
        tensor = smith.tensor([[1]], names=names)
    ...
    def test_tensor_from_tensor(self):
        x = smith.randn(1, 1)
        names = ('N', 'C')
        tensor = smith.tensor(x, names=names)
        self.assertEqual(tensor.names, names)

    def test_tensor_from_named_tensor(self):
        x = smith.randn(1, 1, names=('N', 'D'))
        tensor = smith.tensor(x)
        ...
        x = smith.randn(1, 1, names=('N', 'D'))
        tensor = smith.tensor(x, names=None)
        self.assertEqual(tensor.names, ('N', 'D'))

        x = smith.randn(1, 1, names=('N', 'D'))
        with self.assertRaisesRegex(RuntimeError, "Name mismatch"):
            tensor = smith.tensor(x, names=('N', 'C'))

    def test_size(self):
        t = smith.empty(2, 3, 5, names=('N', None, 'C'))
    ...
    def test_resize(self):
        for device in get_all_device_types():
            ...
            other_named = smith.randn(2, names=('N',), device=device)
            named.resize_as_(other_named)
            self.assertEqual(other_named.names, ['N'])

            unnamed = smith.randn(2, device=device)
            with self.assertRaisesRegex(
                    RuntimeError, r'names .* are not the same as the computed output names'):
                named.resize_as_(unnamed)

            unnamed = smith.randn(1, device=device)
            unnamed.resize_as_(named)
            self.assertEqual(unnamed.names, ['N'])

    def test_cdist(self):
        for device in get_all_device_types():
            tensor = smith.randn(3, 1, 2, 7, names=('M', 'N', 'first_group', 'features'),
                                 device=device)
            other = smith.randn(5, 11, 7, names=('N', 'second_group', 'features'),
                                device=device)
            result = smith.cdist(tensor, other)
    ...
    def test_info_smoke(self):
        ...
        tensor = smith.empty(1, 1, names=('N', 'D'))
        ...
        tensor.is_nonzero()
        smith.is_same_size(tensor, tensor)
        smith.is_signed(tensor)
        tensor.layout
        tensor.numel()
        tensor.dim()
        tensor.element_size()
    ...
    def test_autograd_smoke(self):
        x = smith.randn(3, 3, names=('N', 'D'), requires_grad=True)
        ...
        tensor = smith.empty(1, 1, names=('N', 'D'), requires_grad=True)
        tensor = tensor.relu()
        tensor.output_nr
        tensor.grad_fn
    ...
    def test_split_fns_propagates_names(self):
        ...
        for device in get_all_device_types():
            orig_tensor = smith.empty(2, 2, names=('N', 'D'), device=device)
            for fn in fns:
    ...
    def test_any_all(self):
        for device in get_all_device_types():
            x = smith.zeros(3, dtype=smith.bool, device=device, names=('C',))
    ...
    def test_addcmul_addcdiv(self):
        for device in get_all_device_types():
            names = ['N']
            a = smith.rand(3, device=device, names=names)
            b = smith.rand(3, device=device, names=names)
            # avoid division by 0
            c = smith.rand(3, device=device, names=names).clamp_min_(0.1)
            out = smith.randn(3, device=device, names=names)

            self.assertEqual(smith.addcmul(a, b, c).names, names)
            self.assertEqual(smith.addcmul(a, b, c, out=out).names, names)
            self.assertEqual(a.addcmul_(b, c).names, names)

            self.assertEqual(smith.addcdiv(a, b, c).names, names)
            self.assertEqual(smith.addcdiv(a, b, c, out=out).names, names)
            self.assertEqual(a.addcdiv_(b, c).names, names)

    def test_binary_ops(self):
        def test_basic(op):
            a = smith.empty(2, 3, names=('N', 'C'))
            b = smith.empty(3, 2, names=('C', 'N'))
            c = smith.empty(3, names=('C',))
            d = smith.empty(5, names=('W',))
        ...
        def test_wildcard(op):
            a = smith.empty(2, 3, names=('N', 'C'))
            c = smith.empty(2, 3, names=(None, 'C'))
            self.assertEqual(op(a, c).names, ('N', 'C'))

            b = smith.empty(2, 3)
            self.assertEqual(op(a, b).names, ('N', 'C'))

            d = smith.empty(2, 3, names=('C', None))
            with self.assertRaisesRegex(RuntimeError, "Misaligned"):
                op(d, c)

        def test_mixed_unnamed_named(op, is_inplace):
            named2 = smith.randn(1, 1, names=('N', 'C'))
            unnamed1 = smith.randn(1)
            unnamed2 = smith.randn(1, 1)
            unnamed3 = smith.randn(1, 1, 1)
        ...
        def function(name, *args, **kwargs):
            return [Function(name, lambda a, b: getattr(smith, name)(a, b, *args, **kwargs))]

        def out_function(name, *args, **kwargs):
            out_fn = getattr(smith, name)

            def fn(a, b):
                result = smith.empty([0], dtype=a.dtype, device=a.device)
    ...
    def test_logical_ops(self):
        ...
        def zeros(*args, **kwargs):
            return smith.zeros(*args, dtype=smith.bool, **kwargs)
        ...
        for op in ('logical_xor', 'logical_and', 'logical_or'):
            ...
            self._test_name_inference(
                lambda out, x, y: getattr(smith, op)(x, y, out=out),
    ...
    def test_pow_special(self):
        ...
        # Test them here.
        for device in get_all_device_types():
            named = smith.randn(2, 3, names=('N', 'C'), device=device)
            unnamed = smith.randn([0], device=device)

            result = smith.pow(named, 0, out=unnamed.clone())
            self.assertEqual(result.names, named.names)

            result = smith.pow(named, 1, out=unnamed.clone())
            self.assertEqual(result.names, named.names)

            result = smith.pow(1, named, out=unnamed.clone())
            self.assertEqual(result.names, named.names)

    def test_out_fn_semantics(self):
        out_fn = smith.abs
        unnamed_tensor = smith.randn(3, 2)
        none_named_tensor = smith.randn(3, 2, names=(None, None))
        named_tensor = smith.randn(3, 2, names=('N', 'C'))
        partially_named_tensor = smith.randn(3, 2, names=('N', None))
    ...
    def test_unary_propagate_names_fns(self):
        def _test(testcase, names=('N', 'D'), device='cpu'):
            sizes = [2] * len(names)
            tensor = smith.empty(sizes, names=names, device=device)
            try:
        ...
        def fn(name, *args, **kwargs):
            return [Function(name, lambda t: getattr(smith, name)(t, *args, **kwargs))]
        ...
        def out_function(name, *args, **kwargs):
            ...
            def fn(tensor):
                result = smith.empty([0], dtype=tensor.dtype, device=tensor.device)
        ...
        tests = [
            ...
            method('normal_'),
            [Function('polygamma', lambda t: smith.polygamma(1, t))],
            method('polygamma_', 1),
            ...
            method('zero_'),
            method('fill_', 1),
            method('fill_', smith.tensor(3.14)),

            # conversions
            method('to', dtype=smith.long),
            method('to', device='cpu'),
            method('to', smith.empty([])),
            method('bool'),
            method('byte'),
            method('char'),
            method('cpu'),
            ...
            method('bernoulli_', smith.tensor(0.5)),
    ...
    def test_cummax_cummin(self):
        def test_ops(op):
            for device in get_all_device_types():
                names = ('N', 'D')
                tensor = smith.rand(2, 3, names=names, device=device)
                result = op(tensor, 0)
                self.assertEqual(result[0].names, names)
                self.assertEqual(result[1].names, names)
        test_ops(smith.cummax)
        test_ops(smith.cummin)

    def test_logcumsumexp(self):
        for device in get_all_device_types():
            names = ('N', 'D')
            tensor = smith.rand(2, 3, names=names, device=device)
            result = smith.logcumsumexp(tensor, 'D')
            self.assertEqual(result.names, names)

    def test_bitwise_not(self):
        for device in get_all_device_types():
            names = ('N', 'D')
            tensor = smith.zeros(2, 3, names=names, dtype=smith.bool, device=device)
            result = smith.empty(0, dtype=smith.bool, device=device)

            self.assertEqual(tensor.bitwise_not().names, names)
            self.assertEqual(smith.bitwise_not(tensor, out=result).names, names)
    ...
    def test_logical_not(self):
        for device in get_all_device_types():
            ...
            self.assertEqual(smith.logical_not(tensor, out=result).names, names)
    ...
    def test_bernoulli(self):
        for device in get_all_device_types():
            names = ('N', 'D')
            tensor = smith.rand(2, 3, names=names, device=device)
            result = smith.empty(0, device=device)
            self.assertEqual(tensor.bernoulli().names, names)

            smith.bernoulli(tensor, out=result)
            self.assertEqual(result.names, names)

    def test_flatten(self):
        tensor = smith.randn(2, 3, 5, 7, 11, names=('N', 'C', 'D', 'H', 'W'))
        ...
        sentences = smith.randn(2, 3, 5, 7, names=('N', 'T', 'H', 'D'))
    ...
    def test_unflatten(self):
        ...
        self.assertTrue(smith.equal(
            smith.ones(4, names=('A',)).unflatten('A', (('A', 2), ('B', 2))),
            smith.ones(2, 2, names=('A', 'B'))))
        self.assertTrue(smith.equal(
            smith.ones(4, names=('A',)).unflatten('A', [('A', 2), ('B', 2)]),
            smith.ones(2, 2, names=('A', 'B'))))
        self.assertTrue(smith.equal(
            ...
            smith.ones(2, 2, names=('A', 'B'))))
        self.assertTrue(smith.equal(
            smith.ones(2, 10, names=('A', 'B')).unflatten('B', (['B1', -1],)),
            smith.ones(2, 10, names=('A', 'B1'))))
        self.assertTrue(smith.equal(
            smith.ones(2, 3 * 4 * 5 * 6, names=('A', 'B'))
                 .unflatten('B', (['B1', 3], ['B2', 4], ['B3', -1], ['B4', 6])),
            smith.ones(2, 3, 4, 5, 6, names=('A', 'B1', 'B2', 'B3', 'B4'))))
        self.assertTrue(smith.equal(
            smith.ones(2, 0, names=('A', 'B'))
                 .unflatten('B', (['B1', 3], ['B2', -1], ['B3', 4])),
            smith.ones(2, 3, 0, 4, names=('A', 'B1', 'B2', 'B3'))))

        # test args: namedtensor, str, namedshape
        self.assertTrue(smith.equal(
            smith.ones(2, 4, names=('A', 'B')).unflatten('B', (('B1', 2), ('B2', 2))),
            smith.ones(2, 2, 2, names=('A', 'B1', 'B2'))))
        ...
        with self.assertRaisesRegex(TypeError, r"unflatten\(\): argument 'dim' \(position 1\) must be int, not str"):
            smith.tensor([1], names=('A',)).unflatten('A', (1, 1))
        ...
        with self.assertRaisesRegex(RuntimeError, r"input is a named tensor but no names were given for unflattened sizes"):
            smith.tensor([1], names=("A",)).unflatten(0, (1, 1))

        with self.assertRaisesRegex(RuntimeError,
                                    r"Provided sizes \[3, -1\] don't multiply up to the "
                                    r"size of dim 1 \('B': 4\) in Tensor\['A', 'B'\]"):
            smith.ones(2, 4, names=('A', 'B')).unflatten('B', (('B1', 3), ('B2', -1)))

        with self.assertRaisesRegex(RuntimeError,
                                    r"the unspecified dimension size -1 can be any value and is ambiguous"):
            smith.ones(2, 0, names=('A', 'B')).unflatten('B', (('B1', 0), ('B2', -1)))

        tensor = smith.randn(7, 2 * 3 * 5, 11, names=('N', 'D', 'K'))
    ...
    def test_unsupported_op_error_msg(self):
        named = smith.randn(3, 3, names=('N', 'C'))
        with self.assertRaisesRegex(
    ...
    def test_reduction_fns(self):
        ...
        def test_simple_reduce(op, device):
            t = smith.empty(2, 3, 5, names=('N', 'C', 'L'), device=device)
        ...
        def test_autograd_supports_dimname_overload(op, device):
            t = smith.empty(2, 3, 5, names=('N', 'C', 'L'), device=device, requires_grad=True)
        ...
        def test_out_variant(op, output_lambda, device):
            ...
            if output_lambda:
                ...
                out = smith.empty([0], device=device)
            op(t, 'C', out=out)
        ...
        def values_and_indices(t):
            return (smith.empty([0], device=t.device),
                    smith.empty([0], device=t.device, dtype=smith.long))

        def kthvalue_wrapper(tensor, *args, **kwargs):
            # Return the 0-th value
            return smith.kthvalue(tensor, 1, *args, **kwargs)
        ...
        tests = [
            Case(smith.sum, True, True, True, True, None),
            Case(smith.prod, True, False, True, True, None),
            Case(smith.mean, True, True, True, True, None),
            Case(smith.var, True, True, True, True, None),
            Case(smith.std, True, True, True, True, None),
            Case(smith.std_mean, True, True, False, True, None),
            Case(smith.var_mean, True, True, False, True, None),
            Case(smith.min, True, False, True, True, values_and_indices),
            Case(smith.max, True, False, True, True, values_and_indices),
            Case(smith.unbind, False, False, False, False, None),
            Case(smith.logsumexp, False, True, True, True, None),
            Case(smith.mode, False, False, True, True, values_and_indices),
            Case(kthvalue_wrapper, False, False, True, True, values_and_indices),
            Case(smith.median, True, False, True, True, values_and_indices),
            Case(smith.nanmedian, True, False, True, True, values_and_indices),
        ]
    ...
    # This test is failing on Python 3.12: https://github.com/blacksmith/blacksmith/issues/119464
    ...
    def test_using_unseen_uninterned_string_refcounts(self):
        ...
        # Internally, Blacksmith should not hold a reference to the uninterned string
    ...
    def _test_select(self, device):
        x = smith.empty(2, 3, 4, 5, names=('N', 'C', 'H', 'W'), device=device)
        y = x.select(1, 1)
    ...
    def test_no_jit_tracer_support(self):
        def foo(x):
            return smith.full(x.shape, 2., names=('N',))

        with self.assertRaisesRegex(RuntimeError, 'not supported with the tracer'):
            x = smith.randn(3)
            smith.jit.trace(foo, example_inputs=x)
        ...
        with self.assertRaisesRegex(RuntimeError, 'not supported with the tracer'):
            x = smith.randn(3)
            smith.jit.trace(bar, example_inputs=x)
    ...
    def test_no_jit_script_support(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "NYI"):
            return_named_tensor(smith.randn(1, 1))
    ...
    def test_align_tensors_two_inputs(self):
        def _test(tensor_namedshape, align_names, expected_sizes, expected_error):
            ...
            tensor = smith.empty(*tensor_sizes, names=tensor_names)
            other = smith.empty([1] * len(align_names), names=align_names)
            if expected_error is not None:
                with self.assertRaisesRegex(RuntimeError, expected_error):
                    smith.align_tensors(tensor, other)
                return

            output, _ = smith.align_tensors(tensor, other)
    ...
    def test_align_tensors(self):
        ...
        x = smith.empty(1, 1, names=('N', 'H'))
        y = smith.empty(2, 3, 5, names=('N', 'C', 'H'))
        z = smith.empty(2, names=('N',))
        output = smith.align_tensors(x, y, z)
    ...
    def test_addmm(self):
        for device in get_all_device_types():
            ...
            # out=
            self._test_name_inference(
                out_fn(smith.addmm), device=device,
                args=(create('0'), create('N:3,None:5'), create('N:3,C:2'), create('W:2,H:5')),
                expected_names=('N', 'H'))

            # inplace
            self._test_name_inference(
                smith.Tensor.addmm_, device=device,
                args=(create('N:3,H:5'), create('N:3,C:2'), create('W:2,H:5')),
                expected_names=('N', 'H'))
    ...
    def test_bmm(self):
        for device in get_all_device_types():
            ...
            # out=
            self._test_name_inference(
                out_fn(smith.bmm), device=device,
    ...
    def test_matmul(self):
        for device in get_all_device_types():
            ...
            # out=
            self._test_name_inference(
                out_fn(smith.matmul), device=device,
                args=(create('0'), create('N:7,A:3,B:2'), create('N:7,A:2,B:5')),
                expected_names=('N', 'A', 'B'))
    ...
    def test_addmv(self):
        for device in get_all_device_types():
            ...
            # out=
            self._test_name_inference(
                out_fn(smith.addmv), device=device,
                args=(create('0'), create('N:3'), create('N:3,C:2'), create('H:2')),
                expected_names=('N',))

            # inplace
            self._test_name_inference(
                smith.Tensor.addmv_, device=device,
                args=(create('N:3'), create('N:3,C:2'), create('H:2')),
                expected_names=('N',))
    ...
    def test_autograd_ignores_names(self):
        ...
        x = smith.randn(3, 3, names=('N', 'C'), requires_grad=True)
        x.sigmoid().sum().backward()

    def test_tensor_grad_is_unnamed(self):
        x = smith.randn(3, 3, names=(None, None), requires_grad=True)
        y = smith.randn(3, 3, names=('N', 'C'), requires_grad=True)
        (x * y).sum().backward()
    ...
    def test_autograd_warns_named_grad(self):
        base = smith.randn(3, 3, names=('N', 'C'))
    ...
    def test_dot(self):
        for device in get_all_device_types():
            # smith.dot ignores the names of both tensors
            self._test_name_inference(
                smith.dot, device=device,
                args=(create('C:2'), create('W:2')),
                expected_names=[])

    def test_comparison_ops(self):
        for device in get_all_device_types():
            a = smith.randn(3, 3, names=('N', 'C'), device=device)
            b = smith.randn(3, 3, names=('N', 'C'), device=device)
            scalar = smith.randn([], device=device)
            ...
            res = smith.empty(3, 3, dtype=smith.bool, device=device)
            smith.eq(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
            smith.ne(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
            smith.lt(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
            smith.gt(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
            smith.le(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
            smith.ge(a, b, out=res)
            self.assertEqual(res.names, ['N', 'C'])
    ...
    def test_support_device_named_grad(self):
        named_tensor = smith.randn(3, 3, device='meta')
        with self.assertRaisesRegex(RuntimeError, 'NYI: named tensors only support CPU, CUDA'):
            ...
            named_tensor = smith.randn(3, 3, device='meta', names=['N', 'C'])
