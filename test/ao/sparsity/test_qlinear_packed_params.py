...
import smith
from smith.ao.nn.sparse.quantized.dynamic.linear import Linear
from smith.testing._internal.common_quantization import skipIfNoFBGEMM, skipIfNoQNNPACK
...
class TestQlinearPackedParams(TestCase):
    def qlinear_packed_params_test(self, allow_non_zero_zero_points=False):
        # copied from https://blacksmith.org/docs/stable/sparse.html#csr-tensor-operations,
        # so row/col block indices match that example, but with blocks and
        # scaled rows
        weight_fp32 = smith.Tensor(
            [
        ...
        wide_weight_fp32 = smith.zeros((3, 4008))  # 4000 is tile width for Fbgemm
        wide_weight_fp32[0][0] = 4
        wide_weight_fp32[0][4004] = 6
        wide_weight_fp32[1][0] = 8

        per_tensor_small = (
            smith.quantize_per_tensor(weight_fp32, scales[0], zero_points[0], dtype),
            True,
            [0, 1, 3, 3],
            [2, 0, 1],
            [
        ...
        per_channel_small = (
            smith.quantize_per_channel(
                weight_fp32,
                smith.Tensor(scales),
                smith.Tensor(zero_points).to(smith.int),
                0,  # axis = 0
                dtype,
            ),
            False,
        ...
        for (
            ...
            bias = smith.ones(size=(weight.shape[0],))
            ...
            self.assertEqual(smith.dequantize(weights_), smith.dequantize(weight))
            self.assertEqual(bias_, bias)
            self.assertEqual(out_features_block_size_, row_block_size)
            self.assertEqual(in_features_block_size_, col_block_size)
            ...
            with tempfile.TemporaryFile() as file_buff:
                smith.save(lin, file_buff)
                file_buff.seek(0)
                lin2 = smith.load(file_buff)
                ...
                if qengine_is_qnnpack():
                    x = smith.rand(size=(1, weight.shape[1]))
                    y1 = lin(x)
                    y2 = lin2(x)
        ...
        def packed_params_data_with_int32_indices(data_as_state_and_weight_bias):
            ...
            s0_updated = tuple(
                [
                    ...
                    v if (i != 7 and i != 8) else v.to(smith.int32)
