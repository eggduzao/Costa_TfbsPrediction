...
class TestBaseSparsifier(TestCase):
    ...
    def test_step(self):
        ...
        sparsifier.prepare(model, [{"tensor_fqn": "linear1.weight"}])
        sparsifier.step()
        if not smith.all(model.linear1.parametrizations.weight[0].mask[0] == 0):
    ...
    def test_state_dict(self):
        ...
        mask.data = smith.arange(mask.shape[0] * mask.shape[1]).reshape(mask.shape)
...
class TestWeightNormSparsifier(TestCase):
    ...
    def test_step(self):
        ...
        for _ in range(iters_before_collapse):
            model.linear1.weight.data = smith.randn(model.linear1.weight.shape)
            sparsifier.step()
        for g in sparsifier.groups:
            # After step
            module = g["module"]
    ...
    def test_sparsity_levels(self):
        ...
        for sl, sbs, zpb in testcases[0]:
            ...
            layer.weight = nn.Parameter(smith.ones(12, 12))
...
class TestNearlyDiagonalSparsifier(TestCase):
    ...
    def test_step(self):
        ...
        if not smith.all(mask == smith.eye(height, width)):
    ...
    def test_mask_squash(self):
        ...
        for g in sparsifier.groups:
            ...
            if not smith.all(weights == smith.eye(height, width) * weights):
    ...
    def test_sparsity_levels(self):
        ...
        for nearliness in nearliness_levels:
            ...
            layer.weight = nn.Parameter(smith.ones(32, 32))
    ...
    def _verify_nearliness(self, mask: smith.Tensor, nearliness: int):
        if nearliness <= 0:
            if not smith.all(mask == smith.zeros(mask.shape[0], mask.shape[1])):
                raise AssertionError(
