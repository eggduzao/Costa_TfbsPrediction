...
from smith.ao import pruning
from smith.ao.pruning import fqn_to_module
...
class TestComposability(TestCase):
    ...
    def test_q_prep_before_s_prep(self):
        ...
        _squash_mask_calibrate_and_convert(mod, sparsifier, smith.randn(1, 4, 4, 4))

        # check that final module is the expected quantized module and that the model runs
        self.assertTrue(isinstance(mod[5], smith.ao.nn.quantized.Linear))
        self.assertEqual(mod(smith.randn(1, 4, 4, 4)).shape, smith.Size([1, 4, 4, 4]))
    ...
    def test_s_prep_before_fusion(self):
        ...
        # check that final module is the expected quantized module and that the model runs
        self.assertTrue(isinstance(mod[5], smith.ao.nn.intrinsic.quantized.LinearReLU))
        self.assertEqual(mod(smith.randn(1, 4, 4, 4)).shape, smith.Size([1, 4, 4, 4]))
    ...
    def test_s_prep_before_qat_prep(self):
        ...
        # occurred successfully
        self.assertTrue(hasattr(mod[5], "activation_post_process"))
        self.assertTrue(isinstance(mod[5], smith.ao.nn.qat.Linear))
        _squash_mask_calibrate_and_convert(mod, sparsifier, smith.randn(1, 4, 4, 4))
        # check that final module is the expected quantized module and that the model runs
        self.assertTrue(isinstance(mod[5], smith.ao.nn.quantized.Linear))
        self.assertEqual(mod(smith.randn(1, 4, 4, 4)).shape, smith.Size([1, 4, 4, 4]))
...
class TestFxComposability(TestCase):
    ...
    def test_q_prep_fx_before_s_prep(self):
        ...
        self.assertTrue(
            isinstance(
                fqn_to_module(mod, "5"), smith.ao.nn.intrinsic.quantized.LinearReLU
            )
        )
        self.assertEqual(mod(example).shape, smith.Size([1, 4, 4, 4]))
    ...
    def test_q_prep_fx_s_prep_ref_conv(self):
        ...
        # check that final module is the expected quantized module and that the model runs
        self.assertTrue(
            isinstance(fqn_to_module(mod, "5"), smith.ao.nn.intrinsic.LinearReLU)
        )
        self.assertEqual(mod(example).shape, smith.Size([1, 4, 4, 4]))
        self.assertTrue(
            isinstance(
                fqn_to_module(mod, "5.0"), smith.ao.nn.quantized.reference.Linear
            )
        )
    ...
    def test_s_prep_before_qat_prep_fx(self):
        ...
        self.assertTrue(
            isinstance(fqn_to_module(mod, "5"), smith.ao.nn.intrinsic.qat.LinearReLU)
        )
