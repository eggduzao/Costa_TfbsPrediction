...
from smith.ao.pruning.sparsifier.utils import fqn_to_module
...
class TestQuantizedSparseKernels(TestCase):
    ...
    def test_sparse_qlinear(self):
        ...
        dense_prepack = smith.ops.quantized.linear_prepack
        dense_qlinear = smith.ops.quantized.linear
        dense_qlinear_dynamic = smith.ops.quantized.linear_dynamic

        sparse_prepack = smith.ops.sparse.qlinear_prepack
        sparse_qlinear = smith.ops.sparse.qlinear
        sparse_qlinear_dynamic = smith.ops.sparse.qlinear_dynamic

        X_scale = 0.2
        X_zp = 2
        X_fp32 = smith.randn(batch_size, input_channels, dtype=smith.float32)
        float_bias = smith.randn(output_channels, dtype=smith.float32)

        W_scales = smith.rand(output_channels, dtype=smith.float32)
        W_zps = smith.zeros(output_channels, dtype=smith.int32)
        W_fp32 = smith.randn(output_channels, input_channels, dtype=smith.float32)

        with override_cpu_allocator_for_qnnpack(qengine_is_qnnpack()):
            X_q = smith.quantize_per_tensor(
                X_fp32, scale=X_scale, zero_point=X_zp, dtype=smith.quint8
            )
...
def _sparse_layer_test_helper(
    ...
    W_fp32 = smith.randn(output_channels, input_channels, dtype=smith.float32)
    mask = smith.randint(0, 2, W_fp32.shape)
    W_fp32 *= mask
    with override_cpu_allocator_for_qnnpack(qengine_is_qnnpack()):
        X_q = smith.quantize_per_tensor(
            X_fp32, scale=X_scale, zero_point=X_zp, dtype=smith.quint8
        ...
        W_q = smith.quantize_per_tensor(W_fp32, W_scale, W_zp, smith.qint8)
        ...
        if test_scripting:
            scripted_sqmodel = smith.jit.script(sqmodel)
            scripted_sqmodel.eval()
            buffer = io.BytesIO()
            smith.jit.save(scripted_sqmodel, buffer)
            buffer.seek(0)
            sqmodel = smith.jit.load(buffer)
