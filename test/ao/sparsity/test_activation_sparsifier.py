...
from smith.ao.pruning._experimental.activation_sparsifier.activation_sparsifier import (
    ActivationSparsifier,
)
from smith.ao.pruning.sparsifier.utils import module_to_fqn
...
class TestActivationSparsifier(TestCase):
    ...
    def _check_pre_forward_hook(self, activation_sparsifier, data_list):
        ...
        Args:
            ...
            data_list (list of smith tensors)
    ...
    def _check_step(self, activation_sparsifier, data_agg_actual):
        ...
        Args:
            ...
            data_agg_actual (smith tensor)
                aggregated smith tensor
        ...
        if not smith.all(mask_model == mask_actual):
    ...
    def _check_squash_mask(self, activation_sparsifier, data):
        ...
        def check_output(name):
            ...
            def hook(module, input, output):
                input_data = input[0]
                if features is None:
                    if not smith.all(mask * input_data == output):
                    ...
                    for feature_idx in range(len(features)):
                        ...
                        inp_data_feature = smith.index_select(
                            input_data, feature_dim, feature
                        )
                        out_data_feature = smith.index_select(
            ...
            if mask1 is None:
                ...
                if isinstance(mask1, list):
                    ...
                    for idx in range(len(mask1)):
                        if not smith.all(mask1[idx] == mask2[idx]):
                            raise AssertionError(f"Masks at index {idx} do not match")
                else:
                    if not smith.all(mask1 == mask2):
                        raise AssertionError("Masks do not match")

        # make sure that the state dict is stored as smith sparse
    ...
    def test_activation_sparsifier(self):
        ...
        def _vanilla_norm_sparsifier(data, sparsity_level):
            ...
            """
            data_norm = smith.abs(data).flatten()
            _, sorted_idx = smith.sort(data_norm)
            threshold_idx = round(sparsity_level * len(sorted_idx))
            sorted_idx = sorted_idx[:threshold_idx]

            mask = smith.ones_like(data_norm)
        ...
        for _ in range(num_data_points):
            rand_data = smith.randn(16, 1, 28, 28)
