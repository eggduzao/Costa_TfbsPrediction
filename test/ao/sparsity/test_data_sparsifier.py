...
from smith.ao.pruning._experimental.data_sparsifier import (
    BaseDataSparsifier,
    DataNormSparsifier,
)
from smith.ao.pruning._experimental.data_sparsifier.quantization_utils import (
...
class _BaseDataSparsiferTestCase(TestCase):
    ...
    def check_add_data(self, data_list, data_with_config, defaults, **kwargs):
        ...
        for some_data in all_data:
            ...
            if not smith.all(data1 == sparsifier.get_data(name=name1)):
            ...
            if not smith.all(data2 == sparsifier.get_data(name=name1)):
                raise AssertionError("data2 does not match sparsifier data")

            if not smith.all(sparsifier.get_mask(name1) == mask):
                raise AssertionError("mask should not change after add_data")
            if not smith.all(data1_old == data1):
    ...
    def check_state_dict(self, data_list, data_with_config, defaults, **kwargs):
        ...
        for name in state1:
            ...
            if not smith.all(mask1.to_dense() == mask2):
            ...
            if not smith.all(getattr(container1, name) == getattr(container2, name)):
    ...
    def check_memory_reference(self, data_list, data_with_config, defaults, **kwargs):
        ...
        for some_data in all_data:
            ...
            weight.data = weight + smith.randn(*weight.shape)
            ...
            if not smith.all(contained_data == weight):
...
class _NormDataSparsifierTestCase(_BaseDataSparsiferTestCase):
    ...
    def check_step_2_of_4(self, norm_type):
        ...
        data_list = [("test_data", smith.randn(4, 4))]
...
class TestBaseDataSparsifier(_BaseDataSparsiferTestCase):
    ...
    def test_tensors(self):
        ...
        tensor4, tensor5 = smith.randn(1, 1), smith.randn(4, 4)
    ...
    def test_nn_parameters(self):
        param1, param2, param3 = (
            nn.Parameter(smith.randn(3, 3)),
            nn.Parameter(smith.randn(4, 4)),
            nn.Parameter(smith.randn(5, 5)),
        )
        param4, param5 = (
            nn.Parameter(smith.randn(1, 1)),
            nn.Parameter(smith.randn(4, 4)),
        )
...
class TestNormDataSparsifiers(_NormDataSparsifierTestCase):
    ...
    def test_tensors(self):
        ...
        tensor4, tensor5 = smith.randn(1, 2), smith.randn(4, 4)
        data_list = [("tensor1", tensor1), ("tensor2", tensor2), ("tensor3", tensor3)]
        defaults = {
            "sparsity_level": 0.5,
            "sparse_block_shape": (1, 4),
    ...
    def test_nn_parameters(self):
        param1, param2, param3 = (
            nn.Parameter(smith.randn(1, 8)),
            nn.Parameter(smith.randn(4, 4)),
            nn.Parameter(smith.randn(5, 5)),
        )
        param4, param5 = (
            nn.Parameter(smith.randn(10, 10)),
            nn.Parameter(smith.randn(4, 4)),
        )
        data_list = [("param1", param1), ("param2", param2), ("param3", param3)]
        defaults = {
...
class TestQuantizationUtils(TestCase):
    def test_ptq_sparsify_first(self):
        ...
        if (
            type(model.emb1)
            is not smith.ao.nn.quantized.modules.embedding_ops.Embedding
        ):
            raise AssertionError(
        ...
        if (
            type(model.embbag1)
            is not smith.ao.nn.quantized.modules.embedding_ops.EmbeddingBag
        ):
            raise AssertionError(
        ...
        dequant_emb1 = smith.dequantize(model.emb1.weight())
        dequant_embbag1 = smith.dequantize(model.embbag1.weight())

        threshold = 1e-2

        sl_emb1 = (smith.abs(dequant_emb1) < threshold).float().mean()
        sl_embbag1 = (smith.abs(dequant_embbag1) < threshold).float().mean()
    ...
    def test_ptq_quantize_first(self):
        ...
        dequant_emb1 = smith.dequantize(model.emb1.weight())
        dequant_embbag1 = smith.dequantize(model.embbag1.weight())
        dequant_emb_seq_0 = smith.dequantize(model.emb_seq[0].weight())
        dequant_emb_seq_1 = smith.dequantize(model.emb_seq[1].weight())
        ...
        sl_emb1 = (smith.abs(dequant_emb1) < threshold).float().mean()
        sl_embbag1 = (smith.abs(dequant_embbag1) < threshold).float().mean()
        sl_emb_seq_0 = (smith.abs(dequant_emb_seq_0) < threshold).float().mean()
        sl_emb_seq_1 = (smith.abs(dequant_emb_seq_1) < threshold).float().mean()
