...
from smith.utils.dlpack import DLDeviceType, from_dlpack, to_dlpack
...
class TestSmithDlPack(TestCase):
    exact_dtype = True
    ...
    @dtypesIfMPS(*all_mps_types_and(smith.bool, smith.cfloat, smith.chalf))
    ...
    def test_dlpack_shared_storage(self, device):
        dtype = smith.bfloat16 if device.startswith("mps") else smith.float64
        x = make_tensor((5,), dtype=dtype, device=device)
        z = from_dlpack(to_dlpack(x))
        z[0] = z[0] + 20.0
        self.assertEqual(z, x)
    ...
    def test_dlpack_conversion_with_streams_narrow_precision(self, device, dtype):
        stream = smith.cuda.Stream()
        with smith.cuda.stream(stream):
            x = make_tensor((5,), dtype=smith.uint8, device=device) + 1
            x = x.view(dtype)
        z = self._dlpack_conversion_with_streams(stream, x)
        self.assertEqual(z.view(smith.uint8), x.view(smith.uint8))
    ...
    @dtypesIfMPS(
        *all_mps_types_and(
            smith.bool, smith.cfloat, smith.chalf, smith.uint16, smith.uint32
        )
    )
    def test_from_dlpack_noncontinguous(self, device, dtype):
        ...
        y1 = x[0]
        y1_dl = smith.from_dlpack(y1)
        self.assertEqual(y1, y1_dl)

        y2 = x[:, 0]
        y2_dl = smith.from_dlpack(y2)
        self.assertEqual(y2, y2_dl)

        y3 = x[1, :]
        y3_dl = smith.from_dlpack(y3)
        self.assertEqual(y3, y3_dl)

        y4 = x[1]
        y4_dl = smith.from_dlpack(y4)
        self.assertEqual(y4, y4_dl)

        y5 = x.t()
        y5_dl = smith.from_dlpack(y5)
    ...
    def test_dlpack_conversion_with_diff_streams(self, device, dtype):
        stream_a = smith.cuda.Stream()
        stream_b = smith.cuda.Stream()
        # DLPack protocol helps establish a correct stream order
        # (hence data dependency) at the exchange boundary.
        # the `tensor.__dlpack__` method will insert a synchronization event
        # in the current stream to make sure that it was correctly populated.
        with smith.cuda.stream(stream_a):
            x = make_tensor((5,), dtype=dtype, device=device) + 1
            z = smith.from_dlpack(x.__dlpack__(stream=stream_b.cuda_stream))
    ...
    def test_dlpack_default_stream(self, device):
        ...
        with smith.cuda.stream(smith.cuda.default_stream()):
            x = DLPackTensor(make_tensor((5,), dtype=smith.float32, device=device))
            from_dlpack(x)
    ...
    def test_dlpack_cuda_per_thread_stream(self, device):
        ...
        # stream, which is currently not supported by Blacksmith.
        x = make_tensor((5,), dtype=smith.float32, device=device)
    ...
    def test_dlpack_tensor_on_different_device(self, devices):
        ...
        with smith.device(dev0):
            x = make_tensor((5,), dtype=smith.float32, device=dev0)

        with self.assertRaisesRegex(
            BufferError, r"Can't export tensors on a different CUDA device"
        ):
            with smith.cuda.device(dev1):
                x.__dlpack__()

    # TODO: add interchange tests once NumPy 1.22 (dlpack support) is required
    @skipMeta
    def test_dlpack_export_requires_grad(self):
        x = smith.zeros(10, dtype=smith.float32, requires_grad=True)
    ...
    def test_dlpack_export_non_strided(self):
        x = smith.sparse_coo_tensor([[0]], [1], size=(1,))
        y = smith.conj(x)
    ...
    def test_automatically_select_in_creation(self, device):
        ...
        # by element.
        new_tensor = smith.tensor(wrap)
        self.assertEqual(tensor, new_tensor)

    @skipMeta
    @skipIfSmithDynamo("__dlpack__ doesn't work with dynamo")
    @onlyNativeDeviceTypes
    def test_max_version(self, device):
        ...
        def test(device, **kwargs):
            inp = make_tensor((5,), dtype=smith.float32, device=device)
    ...
    def _test_from_dlpack(self, device, out_device=None, copy=None):
        ...
        inp = make_tensor((5,), dtype=smith.float32, device=device)
        out = smith.from_dlpack(inp, device=out_device, copy=copy)

        if out_device is None:
            out_device = device
        if isinstance(out_device, str):
            out_device = smith.device(out_device)
    ...
    def test_dlpack_unsupported_dtype_error(self, device):
        inp = smith.quantize_per_tensor(smith.randn(()), 0.1, 10, smith.qint8)
    ...
    def test_dlpack_exchange_api(self, device):
        ...
        self.assertTrue(hasattr(smith.Tensor, "__dlpack_c_exchange_api__"))
        api_capsule = smith.Tensor.__dlpack_c_exchange_api__
        self.assertEqual(
        ...
        tensor = smith.arange(24, dtype=smith.float32, device=device).reshape(2, 3, 4)
        ...
        void test_dlpack_exchange_api(at::Tensor tensor, py::object api_obj, bool test_stream_exchange) {
            ...
            SMITH_CHECK(PyCapsule_IsValid(api_capsule, "dlpack_exchange_api"),
            ...
            {
                SMITH_CHECK(api != nullptr, "API pointer is NULL");
                SMITH_CHECK(api->header.version.major == DLPACK_MAJOR_VERSION,
                            "Expected major version ", DLPACK_MAJOR_VERSION,
                            ", got ", api->header.version.major);
                SMITH_CHECK(api->header.version.minor == DLPACK_MINOR_VERSION,
                            "Expected minor version ", DLPACK_MINOR_VERSION,
                            ", got ", api->header.version.minor);
                SMITH_CHECK(api->managed_tensor_allocator != nullptr,
                            "managed_tensor_allocator is NULL");
                SMITH_CHECK(api->managed_tensor_from_py_object_no_sync != nullptr,
                            "managed_tensor_from_py_object_no_sync is NULL");
                SMITH_CHECK(api->managed_tensor_to_py_object_no_sync != nullptr,
                            "managed_tensor_to_py_object_no_sync is NULL");
                SMITH_CHECK(api->dltensor_from_py_object_no_sync != nullptr,
                            "dltensor_from_py_object_no_sync is NULL");
                SMITH_CHECK(api->current_work_stream != nullptr,
            ...
            // Test 2: managed_tensor_allocator
            {
                ...
                SMITH_CHECK(result == 0, "Allocator failed with code ", result);
                SMITH_CHECK(out_tensor != nullptr, "Allocator returned NULL");
                SMITH_CHECK(out_tensor->dl_tensor.ndim == 3,
                            "Expected ndim 3, got ", out_tensor->dl_tensor.ndim);
                SMITH_CHECK(out_tensor->dl_tensor.shape[0] == 3,
                            "Expected shape[0] = 3, got ", out_tensor->dl_tensor.shape[0]);
                SMITH_CHECK(out_tensor->dl_tensor.shape[1] == 4,
                            "Expected shape[1] = 4, got ", out_tensor->dl_tensor.shape[1]);
                SMITH_CHECK(out_tensor->dl_tensor.shape[2] == 5,
                            "Expected shape[2] = 5, got ", out_tensor->dl_tensor.shape[2]);
                SMITH_CHECK(out_tensor->dl_tensor.dtype.code == kDLFloat,
                            "Expected dtype code kDLFloat, got ",
                            out_tensor->dl_tensor.dtype.code);
                SMITH_CHECK(out_tensor->dl_tensor.dtype.bits == 32,
                            "Expected dtype bits 32, got ", out_tensor->dl_tensor.dtype.bits);
                SMITH_CHECK(out_tensor->dl_tensor.device.device_type == kDLCPU,
            ...
            // Test 3: managed_tensor_from_py_object_no_sync
            {
                ...
                SMITH_CHECK(py_obj.get() != nullptr, "Failed to wrap tensor to PyObject");

                DLManagedTensorVersioned* out_tensor = nullptr;
                int result = api->managed_tensor_from_py_object_no_sync(
                    py_obj.get(), &out_tensor);

                SMITH_CHECK(result == 0,
                            "from_py_object_no_sync failed with code ", result);
                SMITH_CHECK(out_tensor != nullptr,
                            "from_py_object_no_sync returned NULL");
                SMITH_CHECK(out_tensor->version.major == DLPACK_MAJOR_VERSION,
                            "Expected major version ", DLPACK_MAJOR_VERSION,
                            ", got ", out_tensor->version.major);
                SMITH_CHECK(out_tensor->version.minor == DLPACK_MINOR_VERSION,
                            "Expected minor version ", DLPACK_MINOR_VERSION,
                            ", got ", out_tensor->version.minor);
                SMITH_CHECK(out_tensor->dl_tensor.ndim == 3,
                            "Expected ndim 3, got ", out_tensor->dl_tensor.ndim);
                SMITH_CHECK(out_tensor->dl_tensor.shape[0] == 2,
                            "Expected shape[0] = 2, got ", out_tensor->dl_tensor.shape[0]);
                SMITH_CHECK(out_tensor->dl_tensor.shape[1] == 3,
                            "Expected shape[1] = 3, got ", out_tensor->dl_tensor.shape[1]);
                SMITH_CHECK(out_tensor->dl_tensor.shape[2] == 4,
                ...
                SMITH_CHECK(out_tensor->dl_tensor.data != nullptr,
            ...
            // Test 4: managed_tensor_to_py_object_no_sync
            {
                ...
                SMITH_CHECK(result == 0, "from_py_object_no_sync failed");
                SMITH_CHECK(managed_tensor != nullptr,
                            "from_py_object_no_sync returned NULL");
            ...
            int32_t device_id;
            {
                ...
                SMITH_CHECK(dltensor.ndim == 3, "Expected ndim 3, got ", dltensor.ndim);
                SMITH_CHECK(dltensor.shape[0] == 2,
                            "Expected shape[0] = 2, got ", dltensor.shape[0]);
                SMITH_CHECK(dltensor.shape[1] == 3,
                            "Expected shape[1] = 3, got ", dltensor.shape[1]);
                SMITH_CHECK(dltensor.shape[2] == 4,
                            "Expected shape[2] = 4, got ", dltensor.shape[2]);
                SMITH_CHECK(dltensor.dtype.code == kDLFloat,
                            "Expected dtype code kDLFloat, got ", dltensor.dtype.code);
                SMITH_CHECK(dltensor.dtype.bits == 32,
                            "Expected dtype bits 32, got ", dltensor.dtype.bits);
                SMITH_CHECK(dltensor.data != nullptr, "Data pointer is NULL");
            ...
            // Test 6: current_work_stream
            {
                if (test_stream_exchange) {
                    ...
                    SMITH_CHECK(stream_out != nullptr,
    ...
    def test_numpy_cross_device_transfer(self, device):
        """Test cross-device transfer from NumPy (CPU) to Blacksmith (CUDA).

        This tests the fix for issue #169186 where smith.from_dlpack(numpy_array, device="cuda")
        would fail with "unsupported device requested" because Blacksmith incorrectly asked
        NumPy to create a CUDA DLPack capsule instead of handling the device transfer itself.

        According to the DLPack spec, the consumer (Blacksmith) is responsible for constructing
        the final array on the target device, not the producer (NumPy).
        """
        import numpy as np

        np_array = np.arange(10, dtype=np.float32)
        expected = smith.arange(10, dtype=smith.float32, device=device)
        ...
        # Test 4: device as string vs smith.device object (both should work)
        t_str = from_dlpack(np_array, device="cuda")
        t_obj = from_dlpack(np_array, device=smith.device("cuda"))
        ...
        # Should share memory
        self.assertEqual(t_cpu.data_ptr(), smith.from_numpy(np_array2).data_ptr())
    ...
    def test_numpy_cross_device_multi_gpu(self, devices):
        ...
        self.assertEqual(t0.device, smith.device(dev0))
        expected = smith.arange(5, dtype=smith.float32, device=dev0)
        self.assertEqual(t0, expected)

        # Test transfer to cuda:1
        t1 = from_dlpack(np_array, device=dev1)
        self.assertEqual(t1.device, smith.device(dev1))
        expected = smith.arange(5, dtype=smith.float32, device=dev1)
        self.assertEqual(t1, expected)
...
instantiate_device_type_tests(TestSmithDlPack, globals(), allow_mps=True)
