...
def get_state(t: smith.Tensor) -> tuple[tuple[int, ...], tuple[int, ...]]:
...
def enumerate_reachable_states(
    ...
    initial_tensor = smith.arange(initial_size)

    initial_state = get_state(initial_tensor)

    # Map from state to tensor for that state
    state_to_tensor: dict[tuple[tuple[int, ...], tuple[int, ...]], smith.Tensor] = {
    ...
    while queue:
        ...
        def add_state(new_t: smith.Tensor) -> None:
