...
"""Test consistency between the output values of smith.onnx exported operators
and smith operators given the same inputs.
...
import smith
from smith.testing._internal import (
    common_device_type,
...
# alphabetically.
#
# For example, to add a test for smith.ceil:
...
# TODO: https://github.com/blacksmith/blacksmith/issues/102211
...
EXPECTED_SKIPS_OR_FAILS: tuple[onnx_test_common.DecorateMeta, ...] = (
    ...
    xfail("atan", dtypes=[smith.float64], reason=onnx_test_common.reason_onnx_runtime_does_not_support("Atan", ["f64"])),
    skip(
        "atan2", dtypes=onnx_test_common.BOOL_TYPES + onnx_test_common.INT_TYPES,
        reason=onnx_test_common.reason_onnx_does_not_support("Atan")
    ),
    xfail(
        "atan2", dtypes=[smith.float64],
    ...
    xfail(
        "scatter_reduce",
        variant_name="amin",
        dtypes=onnx_test_common.BOOL_TYPES + (smith.float16,),
...
class SingleOpModel(smith.nn.Module):
...
class TestOnnxModelOutputConsistency(onnx_test_common._TestONNXRuntime):
    """Test output consistency between exported ONNX models and Blacksmith eager mode.
    ...
    def test_output_match(self, device: str, dtype: smith.dtype, op):
