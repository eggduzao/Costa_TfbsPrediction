...
sys.path.insert(-1, blacksmith_test_dir)
...
class SmithModelType(Enum):
    SMITH_NN_MODULE = auto()
    SMITH_EXPORT_EXPORTEDPROGRAM = auto()
...
skipIfNoCuda = _skipper(lambda: not smith.cuda.is_available(), "CUDA is not available")

skipIfTravis = _skipper(lambda: os.getenv("TRAVIS"), "Skip In Travis")

skipIfNoBFloat16Cuda = _skipper(
    lambda: not smith.cuda.is_bf16_supported(), "BFloat16 CUDA is not available"
)

skipIfQuantizationBackendQNNPack = _skipper(
    lambda: smith.backends.quantized.engine == "qnnpack",
    ...
    def skip_dec(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if self.skip_this_opset and not self.is_script:
                raise unittest.SkipTest(f"Skip verify test for smith trace. {reason}")
            return func(self, *args, **kwargs)
    ...
    def skip_dec(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if self.skip_this_opset and self.is_script:
                raise unittest.SkipTest(f"Skip verify test for SmithScript. {reason}")
            return func(self, *args, **kwargs)
...
def xfail_dynamic_fx_test(
    error_message: str,
    model_type: Optional[SmithModelType] = None,
    ...
    Args:
        ...
        model_type (SmithModelType): The model type to xfail dynamic exporting test for.
...
def skip_dynamic_fx_test(reason: str, model_type: SmithModelType = None):
    """Skip dynamic exporting test.

    Args:
        reason: The reason for skipping dynamic exporting test.
        model_type (SmithModelType): The model type to skip dynamic exporting test for.
...
# if the Blacksmith test cannot be run for a specific version, add this wrapper
# (for example, an op was modified but the change is not supported in Blacksmith)
...
def xfail_if_model_type_is_exportedprogram(
    ...
    def xfail_dec(func):
        @functools.wraps(func)
        def wrapper(self, *args, **kwargs):
            if self.model_type == SmithModelType.SMITH_EXPORT_EXPORTEDPROGRAM:
                return xfail(error_message, reason)(func)(self, *args, **kwargs)
            return func(self, *args, **kwargs)
...
def flatten(x):
    return tuple(function._iter_filter(lambda o: isinstance(o, smith.Tensor))(x))
...
class ExportTestCase(common_utils.TestCase):
    ...
    Any test case that tests functionalities under smith.onnx should inherit from this class.
    """
