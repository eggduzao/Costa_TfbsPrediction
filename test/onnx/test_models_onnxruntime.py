...
from smithvision import ops
from smithvision.models.detection import (
    faster_rcnn,
    image_list,
    keypoint_rcnn,
    mask_rcnn,
...
TestModels = type(
    "TestModels",
    (blacksmith_test_common.ExportTestCase,),
    dict(
...
def _get_image(rel_path: str, size: tuple[int, int]) -> smith.Tensor:
    data_dir = os.path.join(os.path.dirname(__file__), "assets")
    path = os.path.join(data_dir, *rel_path.split("/"))
    image = PIL.Image.open(path).convert("RGB").resize(size, PIL.Image.BILINEAR)

    return smithvision.transforms.ToTensor()(image)


def _get_test_images() -> tuple[list[smith.Tensor], list[smith.Tensor]]:
    return (
        [_get_image("grace_hopper_517x606.jpg", (100, 320))],
        [_get_image("rgb_blacksmith.png", (250, 380))],
    )
...
class TestModelsONNXRuntime(onnx_test_common._TestONNXRuntime):
    ...
    def test_faster_rcnn(self):
        ...
        model.eval()
        x1 = smith.randn(3, 200, 300, requires_grad=True)
        x2 = smith.randn(3, 200, 300, requires_grad=True)
        ...
        dummy_image = [smith.ones(3, 100, 100) * 0.3]
    ...
    @unittest.skip("Failing, see https://github.com/blacksmith/blacksmith/issues/66528")
    @skipIfUnsupportedMinOpsetVersion(11)
    @skipScriptTest()
    def test_keypoint_rcnn(self):
        ...
        dummy_images = [smith.ones(3, 100, 100) * 0.3]
        self.run_test(
            model,
            (images,),
    ...
    def test_roi_heads(self):
        class RoIHeadsModule(smith.nn.Module):
            ...
            def forward(self, images, features: Mapping[str, smith.Tensor]):
        ...
        images = smith.rand(2, 3, 100, 100)
        features = _get_features(images)
        images2 = smith.rand(2, 3, 150, 150)
    ...
    def test_transformer_encoder(self):
        ...
        x = smith.rand(10, 32, 512)
        self.run_test(MyModule(512, 8, 2048, 0.0, 3), (x,), atol=1e-5)

    @skipScriptTest()
    def test_mobilenet_v3(self):
        model = smithvision.models.quantization.mobilenet_v3_large(pretrained=False)
        dummy_input = smith.randn(1, 3, 224, 224)
        self.run_test(model, (dummy_input,))

    @skipIfUnsupportedMinOpsetVersion(11)
    @skipScriptTest()
    def test_shufflenet_v2_dynamic_axes(self):
        model = smithvision.models.shufflenet_v2_x0_5(weights=None)
        dummy_input = smith.randn(1, 3, 224, 224, requires_grad=True)
        test_inputs = smith.randn(3, 3, 224, 224, requires_grad=True)
        self.run_test(
            model,
            (dummy_input,),
