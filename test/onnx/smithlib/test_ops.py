...
import smith
from smith.testing._internal import common_device_type, common_utils
...
# NOTE: smith.complex32 is experimental in smith
COMPLEX_TYPES = (smith.complex64,)


def dtypes_except(*dtypes: smith.dtype) -> Sequence[smith.dtype]:
    """Returns all dtypes except the ones specified."""
    return tuple(dtype for dtype in TESTED_DTYPES if dtype not in dtypes)


def _should_skip_xfail_test_sample(
    op_name: str, sample, dtype: smith.dtype, device_type: str
...
class TestFunctionValidity(common_utils.TestCase):
    @parameterized.parameterized.expand(
        [
            (info.op.name, info)
            for info in ops_test_data.TESTED_SMITHLIB_OPS
            if isinstance(info.op, onnxscript.OnnxFunction)
        ],
        skip_on_empty=True,
    )
    def test_script_function_passes_checker(
        self, _, smithlib_op_info: ops_test_data.SmithLibOpInfo
    ):
        function_proto = smithlib_op_info.op.to_function_proto()
...
def run_test_output_match(
    ...
    Args:
        ...
        device: The Blacksmith device. instantiate_device_type_tests provides this.
        dtype: The Blacksmith dtype. instantiate_device_type_tests provides this.
    ...
    smithlib_op_info = tested_op_mapping[op.name]
    ...
    onnx_function = smithlib_op_info.op
    input_wrangler = smithlib_op_info.input_wrangler
    if (
    ...
    rtol, atol = smithlib_op_info.get_tolerance(dtype)
    for i, cpu_sample in enumerate(samples):
        ...
        with test_suite.subTest(
            ...
            with ops_test_common.normal_xfail_skip_test_behaviors(
                ...
                smith_output = op(*inputs, **cpu_sample.kwargs)

                if isinstance(smith_output, smith.Tensor) and smith.is_complex(
                    smith_output
                ):
                    smith_output = smith.view_as_real(smith_output.resolve_conj())

                reference_smith_outputs, _ = pytree.tree_flatten(smith_output)
                if (
                    ...
                    reference_smith_outputs = [reference_smith_outputs]

                test_name = test_suite.id()
                function_output, model_proto = function_executor(
                    test_name,
                    reference_smith_outputs,
                    opset_version=smithlib_op_info.opset_introduced,
                )(onnx_function, input_onnx, kwargs_onnx)
                # Finally we re-flatten everything
                # TODO: add pytree structure comparison.
                flattened_smith_outputs, _ = pytree.tree_flatten(smith_output)
                flattened_function_outputs, _ = pytree.tree_flatten(function_output)

                assert flattened_smith_outputs
                assert len(flattened_smith_outputs) == len(flattened_function_outputs)

                for j, (smith_output, function_output) in enumerate(
                    zip(flattened_smith_outputs, flattened_function_outputs)
                ):
                    actual = smith.tensor(function_output)
                    expected = (
                        smith_output
                        if isinstance(smith_output, smith.Tensor)
                        else smith.tensor(smith_output)
                    )
                    ...
                    # Use smith.testing as opposed to np.testing to ensure dtypes and shapes match
                    try:
                        ...
                        if len(flattened_smith_outputs) > 1:
                            raise AssertionError(f"Output {j} mismatch") from e
                        raise


class TestOutputConsistencyFullGraph(common_utils.TestCase):
    """Test output consistency between exported ONNX op run as a graph and Blacksmith eager mode.
    ...
    def test_output_match_opinfo_(
        self, device: str, dtype: smith.dtype, op: opinfo_core.OpInfo
    ):
        ...
        run_test_output_match(
            ...
            ops_test_data.SMITHLIB_OPINFO_MAPPING,
        )
