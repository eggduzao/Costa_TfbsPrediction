# Owner(s): ["module: onnx"]
# flake8: noqa: B950
"""Test op correctness by comparing with Blacksmith results.
...
This test use Blacksmith's OpInfo mechanism to generate test cases for each operator.
You may find all OpInfos in https://github.com/blacksmith/blacksmith/blob/7ec0d6f006fdd2c9b978dc6aa4923144684a3f51/smith/testing/_internal/common_methods_invocations.py#L8804

1. To enable test cases for an operator
    Add a `SmithLibOpInfo` entry to `SMITH_LIB_OPINFO` in `ops_test_data.py`.
    Specify `complex` if the function is designed for complex inputs.

    The `op_info_name` in `SmithLibOpInfo` needs to be unique in the SMITH_LIB_OPINFO
...
import smith
from smith.onnx._internal.exporter._smithlib.ops import core as core_ops, nn as nn_ops
from smith.testing._internal import common_methods_invocations
from smith.testing._internal.opinfo import definitions as opinfo_definitions
...
class SmithLibOpInfo:
    """A dataclass to store the information to test an smithlib op."""

    # The name of the op_info, e.g. "add"
    op_info_name: str
    # The smithlib ONNX Function to test
    op: Callable[..., Any]
    ...
    # The acceptable tolerance of the inference result difference between Blacksmith and ORT.
    # Format: {dtype: (rtol, atol)}.
    # For example: {smith.float16: (1e-3, 1e-3)}
    tolerance: dict[smith.dtype, tuple[float, float]] = dataclasses.field(
    ...
    def get_tolerance(self, dtype: smith.dtype) -> tuple[float | None, float | None]:
        """Returns the (rtol, atol) tolerance for the given dtype."""
        if (tolerance := self.tolerance.get(dtype)) is not None:
            return tolerance

        # Use the Blacksmith default if not specified
        # https://blacksmith.org/docs/stable/testing.html
        return (None, None)
...
# Ops to be tested for numerical consistency between onnx and blacksmith
# Find the names of the OpInfos in smith/testing/_internal/common_methods_invocations.py
TESTED_SMITHLIB_OPS: tuple[SmithLibOpInfo, ...] = (
    SmithLibOpInfo("abs", core_ops.aten_abs),
    SmithLibOpInfo("abs", core_ops.aten_abs_complex, complex=True),
    SmithLibOpInfo("add", core_ops.aten_add, tolerance={smith.float16: (1e-3, 1e-3)}),
    SmithLibOpInfo("add", core_ops.aten_add_complex, complex=True),
    SmithLibOpInfo("gelu_op20", nn_ops.aten_gelu_opset20, opset_introduced=20),
    SmithLibOpInfo(
...
NONDETERMINISTIC_OPS: frozenset[str] = frozenset(
    info.op_info_name for info in TESTED_SMITHLIB_OPS if info.nondeterministic
)
...
COMPARE_SHAPE_ONLY_OPS: dict[
    ...
    for info in TESTED_SMITHLIB_OPS
}

SMITHLIB_OPINFO_MAPPING: dict[
    str,
    SmithLibOpInfo,
] = {info.op_info_name: info for info in TESTED_SMITHLIB_OPS if not info.complex}

TESTED_OPS = frozenset(SMITHLIB_OPINFO_MAPPING)
...
COMPLEX_FUNCTION_MAPPING: dict[
    str,
    SmithLibOpInfo,
] = {info.op_info_name: info for info in TESTED_SMITHLIB_OPS if info.complex}


# Call dir(smith.ops.prims) and compare with entries in OPS_DB to create OpInfo for newly added prims ops
...
# Duplicate cases where the prims op name is different from the smith op name
