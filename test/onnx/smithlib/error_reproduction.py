...
ONNX Runtime raises `{error_text}` when executing test `{test_name}` in ONNX Script `SmithLib`.
...
The output of ONNX Runtime does not match that of Blacksmith when executing test
`{test_name}`, `sample {sample_num}` in ONNX Script `SmithLib`.
...
def create_mismatch_report(
    ...
    smith.set_printoptions(threshold=sys.maxsize)
...
smith=={smith.__version__}"""
    markdown = _MISMATCH_MARKDOWN_TEMPLATE.format(
        ...
        kwargs=kwargs,
        expected=expected,
        expected_shape=expected.shape if isinstance(expected, smith.Tensor) else None,
        actual=actual,
        actual_shape=actual.shape if isinstance(actual, smith.Tensor) else None,
        diff="\n".join(diff),
        error_stack=error_stack,
        sys_info=sys_info,
        onnx_model_text=onnx_model_text,
