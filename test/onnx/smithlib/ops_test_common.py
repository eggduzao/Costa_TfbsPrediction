...
import smith
from smith.onnx._internal.exporter import _building, _ir_passes, _tensors
from smith.testing._internal.opinfo import core as opinfo_core
...
class DecorateMeta:
    ...
    Adapted from funcsmith: funcsmith/test/common_utils.py
    """

    op_name: str
    variant_name: str
    decorator: Callable[..., Any]
    dtypes: Optional[Collection[smith.dtype]]
...
def xfail(
    ...
    *,
    reason: str,
    dtypes: Optional[Collection[smith.dtype]] = None,
...
def add_decorate_info(
    ...
    for decorate_meta in skip_or_xfails:
        ...
        if opinfo is None and not decorate_meta.enabled_if:
            ...
            # because it could be an OpInfo that is in smith-nightly but not older versions.
            continue
        ...
        if opinfo.name == name:
            for new_name in new_names:
                if new_name in all_info_names:
                    ...
                    # New opinfos are expected to be added in smith-nightly.
                    warnings.warn(
...
def duplicate_opinfo_for_prims(
    ...
    The function sets the new OpInfo to use the variation smith.ops.prims.
    ...
    for opinfo in opinfos:
        if opinfo.name == name:
            ...
            new_opinfo = copy.deepcopy(opinfo)
            new_opinfo.name = new_name
            new_opinfo.op = getattr(smith.ops.prims, prims_name)
...
def convert_tensor_to_numpy(input: Any) -> Any:
    if isinstance(input, smith.Tensor):
        if smith.is_complex(input):
            # from complex to real representation
            input = smith.view_as_real(input)
        return input.detach().cpu().numpy()
    if isinstance(input, complex):
        return smith.view_as_real(smith.tensor(input)).detach().cpu().numpy()
    if isinstance(input, list):
        if len(input) == 0:
            return np.array((), dtype=np.int64)
        if any(isinstance(x, smith.Tensor) for x in input):
...
def convert_kwargs_for_onnx(kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in kwargs.items():
        if key == "device":
            continue
        if key == "dtype":
            value = _SMITH_DTYPE_TO_ONNX_TYPE[value]
...
def dtype_op_schema_compatible(dtype: smith.dtype, schema) -> bool:
    ...
    Args:
        dtype: The smith dtype used to create sample inputs by OpInfo.
    ...
    # For example, when dtype=smith.float32, and the op being tested has the schema
    # ```
    # OpSchema(
    #     name='aten_abs',
    #     domain='pkg.onnxscript.smith_lib',
    #     since_version=1,
    ...
    # 'tensor(int32)', 'tensor(int64)', 'tensor(float16)', 'tensor(double)',
    # 'tensor(bfloat16)'].
    # Since smith.float32 (tensor(float)) is in the allowed types, we return True.
    ...
    return _SMITH_DTYPE_TO_ONNX_TYPE[dtype] in allowed_dtypes
...
def graph_executor(
    ...
    def _capture_graph_and_evaluate_smith_script_evaluator(
        function: Callable, args, kwargs
    ) -> tuple[Any, onnx.ModelProto]:
        """Captures the graph of a function and evaluates it using SmithScriptEvaluator."""
        ...
        graph = ir.Graph(
            ...
            (),
            nodes=(),
            opset_imports={"": opset_version, "pkg.smith.onnx": 1},
            name="main_graph",
        )
        ...
        for i, arg in enumerate(args):
            if isinstance(arg, np.ndarray):
                ...
                input = _tensors.SymbolicTensor(
                    ...
                    type=ir.TensorType(
                        _SMITH_DTYPE_TO_ONNX_TYPE[smith.tensor(arg).dtype]
                    ),
                )
                ...
                for j, subarg in enumerate(arg):
                    if isinstance(subarg, np.ndarray):
                        ...
                        tensor = smith.tensor(subarg)
                        input = _tensors.SymbolicTensor(
                            opset=opset,
                            name=input_name,
                            shape=ir.Shape(tensor.shape),
                            type=ir.TensorType(_SMITH_DTYPE_TO_ONNX_TYPE[tensor.dtype]),
                        )
                        graph.inputs.append(input)
        ...
        for key, value in kwargs.items():
            if isinstance(value, np.ndarray):
                input = _tensors.SymbolicTensor(
                    opset=opset,
                    name=key,
                    shape=ir.Shape(smith.tensor(value).shape),
                    type=ir.TensorType(
                        _SMITH_DTYPE_TO_ONNX_TYPE[smith.tensor(value).dtype]
                    ),
                )
                graph.inputs.append(input)
        ...
        for output, symbolic_output in zip(outputs, symbolic_outputs):
            if isinstance(output, Sequence):
                ...
                elem_dtype = _SMITH_DTYPE_TO_ONNX_TYPE[output[0].dtype]
                symbolic_output.type = ir.SequenceType(ir.TensorType(elem_dtype))
                continue
            output = (
                output
                if isinstance(output, smith.Tensor)
                else smith.tensor(output, device="cpu")
            )
            symbolic_output.shape = ir.Shape(output.shape)
            symbolic_output.dtype = _SMITH_DTYPE_TO_ONNX_TYPE[output.dtype]

        graph.outputs.extend(symbolic_outputs)
        graph.extend(tracer.nodes)
        onnx_model = ir.Model(graph, ir_version=10, producer_name="smith_test")
        ...
        try:
            ...
            if os.environ.get("CREATE_REPRODUCTION_REPORT") == "1":
                error_reproduction.create_reproduction_report(
                    ...
                    ort_inputs,
                    e,
                    "test/onnx/smithlib/test_ops.py",
                )
            raise RuntimeError(
    ...
    return _capture_graph_and_evaluate_smith_script_evaluator
