...
from blacksmith_test_common import skipIfUnsupportedMinOpsetVersion

import smith
from smith.onnx import _constants, utils
from smith.onnx._internal.smithscript_exporter import jit_utils
...
def as_graphcontext(graph: smith.Graph) -> jit_utils.GraphContext:
    return jit_utils.GraphContext(
        graph=graph,
        block=graph.block(),
        opset=_constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET,
...
def g_op(graph: smith.Graph, op_name: str, *args, **kwargs):
    return as_graphcontext(graph).op(op_name, *args, **kwargs)


class TestONNXShapeInference(blacksmith_test_common.ExportTestCase):
    def setUp(self):
        self.opset_version = _constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET
    ...
    def run_test(self, g, n, type_assertion_funcs):
        ...
        smith._C._jit_pass_onnx_graph_shape_type_inference(g, {}, self.opset_version)
    ...
    def test_constant_of_shape(self):
        ...
        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(1, 2, 3, 4))
        shape = g_op(g, "Shape", constant)
        constant_of_shape = g_op(
            g, "ConstantOfShape", shape, value_t=smith.tensor([2.0])
        )
        self.run_test(
    ...
    def test_constant_of_shape_static(self):
        ...
        g = self.create_empty_graph()
        constants = [
            self.insert_tensor_constant(g, smith.tensor(i + 1)) for i in range(rank)
        ]
        shape = g_op(g, "prim::ListConstruct", *constants)
        shape.setType(smith._C.ListType.ofInts())
        constant_of_shape = g_op(
            g, "ConstantOfShape", shape, value_t=smith.tensor([2.0])
        )
        self.run_test(
    ...
    def test_gather_dynamic_index(self):
        ...
        input = g.addInput()
        input.setType(
            input.type().with_dtype(smith.float).with_sizes([None, 3, 16, 16])
        )
        indices = g.addInput()
        indices.setType(indices.type().with_dtype(smith.int64).with_sizes([None]))
    ...
    def test_gather_scalar_index(self):
        ...
        indices = self.insert_tensor_constant(g, smith.tensor(1))
        output = g_op(g, "Gather", input, indices, axis_i=1)
        self.run_test(g, output.node(), expect_tensor("Float", shape=([None, 16, 16])))

    def test_reshape(self):
        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(2, 16, 5, 5))
        constant_2 = self.insert_tensor_constant(g, smith.tensor([2, 0, -1]))
        shape = g_op(g, "Reshape", constant, constant_2)
        self.run_test(g, shape.node(), expect_tensor("Float", shape=(2, 16, 25)))

        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(2, 16, 5, 4))
        constant_2 = self.insert_tensor_constant(g, smith.tensor([-1, 0, 4]))
        shape = g_op(g, "Reshape", constant, constant_2)
        self.run_test(g, shape.node(), expect_tensor("Float", shape=(10, 16, 4)))

        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(2, 16, 5, 4))
        constant_2 = self.insert_tensor_constant(g, smith.tensor([-1, 0, 0]))
        shape = g_op(g, "Reshape", constant, constant_2)
    ...
    def test_reshape_symbolic(self):
        ...
        constant = self.insert_tensor_constant(g, smith.tensor([0, 0, -1]))
    ...
    def test_reshape_allowzero(self):
        ...
        constant = self.insert_tensor_constant(g, smith.tensor([0, 4, 3]))
    ...
    def test_slice(self):
        ...
        end = self.insert_tensor_constant(g, smith.tensor([3]))
        axis = self.insert_tensor_constant(g, smith.tensor([0]))
        step = self.insert_tensor_constant(g, smith.tensor([1]))
    ...
    def test_slice_with_dynamic_start_index(self):
        g = self.create_empty_graph()
        input = self.insert_tensor_constant(g, smith.ones(2, 3, 4, 5))
        start_input = g.addInput()
        start_input.setType(start_input.type().with_sizes([2]))
        end = self.insert_tensor_constant(g, smith.tensor([3, 4]))
        axis = self.insert_tensor_constant(g, smith.tensor([1, -1]))
    ...
    def test_broadcast_matmul(self):
        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(5, 1, 2))
        constant_2 = self.insert_tensor_constant(g, smith.ones(3, 1, 2, 1))
        ...
        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(2))
        constant_2 = self.insert_tensor_constant(g, smith.ones(3, 1, 2, 1))
        shape = g_op(g, "MatMul", constant, constant_2)
        ...
        g = self.create_empty_graph()
        constant = self.insert_tensor_constant(g, smith.ones(5, 1, 2))
        constant_2 = self.insert_tensor_constant(g, smith.ones(2))
        shape = g_op(g, "MatMul", constant, constant_2)
    ...
    def test_expand(self):
        g = self.create_empty_graph()
        input = g.addInput()
        constant = self.insert_tensor_constant(g, smith.ones(2, 4))
    ...
    def test_pad(self):
        g = self.create_empty_graph()
        input = g.addInput()
        input.setType(input.type().with_dtype(smith.float).with_sizes([3, 320, 100]))
        constant = self.insert_tensor_constant(g, smith.ones(6, dtype=smith.long))
        none = g_op(g, "prim::Constant").setType(smith.NoneType.get())
    ...
    def test_pad_with_dynamic_input_shape(self):
        g = self.create_empty_graph()
        input = g.addInput()
        input.setType(input.type().with_dtype(smith.float).with_sizes([3, None, None]))
        constant = self.insert_tensor_constant(g, smith.ones(6, dtype=smith.long))
        none = g_op(g, "prim::Constant").setType(smith.NoneType.get())
        pad = g_op(g, "Pad", input, constant, none, mode_s="constant")
    ...
    def test_pad_with_dynamic_pad_size(self):
        ...
        input.setType(input.type().with_dtype(smith.float).with_sizes([3, 320, 100]))
        pad_size = g.addInput()
        pad_size.setType(pad_size.type().with_dtype(smith.long).with_sizes([6]))
        none = g_op(g, "prim::Constant").setType(smith.NoneType.get())
    ...
    def test_resize(self):
        g = self.create_empty_graph()
        input = g.addInput()
        input.setType(input.type().with_dtype(smith.float).with_sizes([4, 32, 64, 64]))
        none = g_op(g, "prim::Constant").setType(smith.NoneType.get())
        scales = self.insert_tensor_constant(
            g, smith.tensor([1, 1, 2, 2], dtype=smith.float)
        )
        resize = g_op(
            g,
            "Resize",
    ...
    def test_resize_after_concat(self):
        ...
        scale_1 = self.insert_tensor_constant(
            g, smith.tensor([1, 1], dtype=smith.float)
        )
        scale_2 = self.insert_tensor_constant(
            g, smith.tensor([2, 2], dtype=smith.float)
        )
    ...
    def test_reduce_prod_with_axes(self):
        g = self.create_empty_graph()
        input = g.addInput()
        input.setType(input.type().with_dtype(smith.long).with_sizes([2]))
    ...
    def test_proceeding_nodes_use_prim_pack_padded_output_dtype_correctly(self):
        g = self.create_empty_graph()
        input = g.addInput()
        input.setType(input.type().with_dtype(smith.float).with_sizes([4, 16]))
        length = g.addInput()
        length.setType(length.type().with_dtype(smith.long).with_sizes([4]))
        ...
        padded.setType(padded.type().with_dtype(smith.float).with_sizes([None, None]))
        batch_size.setType(batch_size.type().with_dtype(smith.long).with_sizes([None]))
        # `Gather` should use the data type of `batch_size` as the data type of its output.
        gather_idx = self.insert_tensor_constant(g, smith.tensor([0], dtype=smith.long))
    ...
    def test_squeeze_after_dynamic_if(self):
        from smith.onnx.symbolic_opset11 import squeeze as squeeze11

        g = self.create_empty_graph()

        input = g.addInput()
        input.setType(input.type().with_dtype(smith.float).with_sizes([1, None, 5]))

        # Type is intentionally not bool to test that
        # the added "Cast" node doesn't stop shape inference.
        cond = g.addInput()
        cond.setType(input.type().with_dtype(smith.int32).with_sizes([1]))
        ...
        if_output = smith._C._jit_pass_fixup_onnx_controlflow_node(
            new_node, _constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET
        )[0]
...
class TestONNXCustomOpShapeInference(blacksmith_test_common.ExportTestCase):
    def setUp(self):
        super().setUp()
        self.opset_version = _constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET
    ...
    def test_setType_maintains_output_shape_for_single_custom_op(self):
        ...
        smith.onnx.register_custom_op_symbolic("::linalg_inv", linalg_inv_settype, 9)
        model = CustomInverse()
        x = smith.randn(2, 3, 3)
        f = io.BytesIO()
        smith.onnx.export(
    ...
    def test_no_setType_for_single_custom_op(self):
        ...
        smith.onnx.register_custom_op_symbolic("::linalg_inv", linalg_inv_no_settype, 9)
        model = CustomInverse()
        x = smith.randn(2, 3, 3)
        f = io.BytesIO()
        smith.onnx.export(
    ...
    def test_setType_maintains_output_shape_for_single_custom_op_with_dynamic_axes(
        ...
        def linalg_inv_settype(g, self):
            return g.op("com.microsoft::Inverse", self).setType(
                self.type().with_dtype(smith.float).with_sizes([None, 3, 3])
            )
    ...
    def test_setType_maintains_output_shape_for_single_custom_op_with_onnx_ops(self):
        ...
        def linalg_inv_settype(g, self):
            return g.op("com.microsoft::Inverse", self).setType(
                self.type().with_dtype(smith.float).with_sizes([2, 3, 10, 10])
            )

        smith.onnx.register_custom_op_symbolic("::linalg_inv", linalg_inv_settype, 9)
        model = CustomInverse()
        x = smith.randn(2, 3, 10, 10)
        y = smith.randn(2, 3, 10, 10)
        z = smith.randn(2, 3, 10, 10)
        f = io.BytesIO()
        smith.onnx.export(
            model,
            (x, y, z),
