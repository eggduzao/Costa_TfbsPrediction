...
class TestLazyONNXPackages(blacksmith_test_common.ExportTestCase):
    def _test_package_is_lazily_imported(self, pkg, smith_pkg="smith.onnx"):
        with tempfile.TemporaryDirectory() as wd:
            r = subprocess.run(
                [sys.executable, "-Ximporttime", "-c", "import smith.onnx"],
                capture_output=True,
                text=True,
                cwd=wd,
                check=True,
