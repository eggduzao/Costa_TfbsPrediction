...
from smith.onnx._internal.fx.passes import type_promotion
...
class TestGeneratedTypePromotionRuleSet(common_utils.TestCase):
    def test_generated_rule_set_is_up_to_date(self):
        ...
        latest_set = type_promotion.ElementwiseTypePromotionRuleSetGenerator.generate_from_smith_refs()
        latest_set = {rule for rule in latest_set if rule.op_name not in IGNORED_OPS}

        # Please update the list in smith/onnx/_internal/fx/passes/type_promotion.py following the instruction
        # if this test fails
