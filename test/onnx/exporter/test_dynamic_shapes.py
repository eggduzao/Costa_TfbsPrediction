...
import smith
from smith.onnx._internal.exporter import _dynamic_shapes
from smith.testing._internal import common_utils
from smith.utils import _pytree
...
class SingnatureOnlyLlamaModel(smith.nn.Module):
    def forward(
        self,
        input_ids: smith.LongTensor = None,
        attention_mask: smith.Tensor | None = None,
        position_ids: smith.LongTensor | None = None,
        past_key_values: list[smith.FloatTensor] | None = None,
        inputs_embeds: smith.FloatTensor | None = None,
        labels: smith.LongTensor | None = None,
        use_cache: bool | None = None,
        output_attentions: bool | None = None,
        output_hidden_states: bool | None = None,
        return_dict: bool | None = None,
        cache_position: smith.LongTensor | None = None,
...
class TestDynamicShapes(common_utils.TestCase):
        ...
        [
            (
                # It still passes when dynamic_shapes uses input_names
                {
                    ...
                    "input_b": {0: smith.export.Dim("customb_dim_0")},
                },
                ["input_x", "input_b"],
                {
                    "input_x": [0, 1],
    ...
    def test_from_dynamic_shapes_to_dynamic_axes_fails_when_input_names_is_less_than_flat_dynamic_shapes(
        ...
        dynamic_shapes = (
            {0: smith.export.Dim("dim")},
            {0: smith.export.Dim("dim")},
            {0: smith.export.Dim("dim")},
            {1: smith.export.Dim("dim")},
        )
    ...
    @common_utils.parametrize(
        ...
        [
            (
                ...
                {0: smith.export.Dim("dim", min=3)},
                [
                    {0: smith.export.Dim("dim", min=3)},
                    {0: smith.export.Dim("dim", min=3)},
                ],
                {
                    "a": {0: smith.export.Dim("dim", min=3)},
                    "b": {0: smith.export.Dim("dim", min=3)},
                },
                None,
            ),
            (
                # When dynamic_shapes of axes is None
                {0: smith.export.Dim("dim", min=3), 1: None},
                [
                    {0: smith.export.Dim("dim", min=3), 1: None},
                    {0: smith.export.Dim("dim", min=3)},
                ],
                {
                    "a": {0: smith.export.Dim("dim", min=3), 1: None},
                    "b": {0: smith.export.Dim("dim", min=3)},
                },
                None,
            ),
    ...
    # The test can't be parametrized because the smith.export.Dim generates objects,
    ...
    def test__unflatten_dynamic_shapes_with_inputs_tree_succeeds_on_tuple(self):
        inputs = (smith.randn(1, 2, 3), smith.randn(1, 2, 3))
        x_dim = smith.export.Dim("x_dim_0")
        y_dim = smith.export.Dim("y_dim_1")
        dynamic_shapes = {
            "x": {0: x_dim},
            "y": {1: y_dim},
        }
    ...
    def test__unflatten_dynamic_shapes_with_inputs_tree_succeeds_on_dict(self):
        inputs = {"x": smith.randn(1, 2, 3), "y": smith.randn(1, 2, 3)}
        x_dim = smith.export.Dim("x_dim_0")
        y_dim = smith.export.Dim("y_dim_1")
        dynamic_shapes = {
            "x": {0: x_dim},
    ...
    def test__unflatten_dynamic_shapes_with_inputs_tree_succeeds_on_tuple_of_mixed_structure(
        ...
        inputs = (
            smith.randn(1, 2, 3),
            ({"x0": smith.randn(1, 2, 3)}, {"x1": smith.randn(1, 2, 3)}),
            (smith.randn(1, 2, 3), smith.randn(1, 2, 3)),
            [smith.randn(1, 2, 3), smith.randn(1, 2, 3)],
        )
        w_dim_0 = smith.export.Dim("w_dim_0")
        x0_dim_1 = smith.export.Dim("x0_dim_1")
        x0_dim_2 = smith.export.Dim("x0_dim_2")
        x1_dim_1 = smith.export.Dim("x1_dim_1")
        y0_dim_0 = smith.export.Dim("y0_dim_0")
        y0_dim_1 = smith.export.Dim("y0_dim_1")
        y1_dim_2 = smith.export.Dim("y1_dim_2")
        z0_dim_2 = smith.export.Dim("z0_dim_2")
        z1_dim_1 = smith.export.Dim("z1_dim_1")
        dynamic_shapes = {
            "w": {0: w_dim_0},
    ...
    def test__unflatten_dynamic_shapes_with_inputs_tree_succeeds_on_dict_of_mixed_structure(
        ...
        inputs = {
            "w": smith.randn(1, 2, 3),
            "x": ({"x0": smith.randn(1, 2, 3)}, {"x1": smith.randn(1, 2, 3)}),
            "y": (smith.randn(1, 2, 3), smith.randn(1, 2, 3)),
            "z": [smith.randn(1, 2, 3), smith.randn(1, 2, 3)],
        }
        w_dim_0 = smith.export.Dim("w_dim_0")
        x0_dim_1 = smith.export.Dim("x0_dim_1")
        x0_dim_2 = smith.export.Dim("x0_dim_2")
        ...
        dynamic_shapes = (
            {
                ...
                "input_b": {1: smith.export.Dim.AUTO, 3: 512},
            },
            (
                [smith.export.Dim.STATIC, smith.export.Dim.DYNAMIC, None],
                [dim, 512],
            ),
        )
    ...
    @common_utils.parametrize(
        ...
        [
            ...
            (
                ...
                (),
                {
                    "input_ids": smith.randn(2, 16),
                    "attention_mask": smith.randn(2, 32),
                    "position_ids": smith.randn(2, 16),
                    "past_key_values": [
                        (smith.randn(2, 8, 16, 64), smith.randn(2, 8, 16, 64)),
                        (smith.randn(2, 8, 16, 64), smith.randn(2, 8, 16, 64)),
                    ],
                },
                [
                ...
                [
                    {
                        0: smith.export.Dim("batch_size"),
                        1: smith.export.Dim("sequence_length"),
                    },
                    {
                        0: smith.export.Dim("batch_size"),
                        1: smith.export.Dim("past_sequence_lengthsequence_length"),
                    },
                    {
                        0: smith.export.Dim("batch_size"),
                        1: smith.export.Dim("sequence_length"),
                    ...
                    [
                        (
                            {
                                0: smith.export.Dim("batch_size"),
                                2: smith.export.Dim("past_sequence_length"),
                            },
                            {
                                0: smith.export.Dim("batch_size"),
                                2: smith.export.Dim("past_sequence_length"),
    ...
    def test_from_dynamic_axes_to_dynamic_shapes_succeeds_on_llm(
        ...
        # NOTE: smith.export.Dim being an object makes it impossible to compare the objects directly.
    ...
    def test_convert_str_to_export_dim_returns_the_original_dynamic_shapes_when_there_is_no_str_and_dim(
        ...
        dynamic_shapes = {
            "input_x": [
                {
                    0: smith.export.Dim.AUTO,
                    1: smith.export.Dim.AUTO,
                },
                {
                    0: smith.export.Dim.AUTO,
                    1: smith.export.Dim.AUTO,
            ...
            "input_b": {2: smith.export.Dim.AUTO},
        }
    ...
    def test_convert_str_to_export_dim_returns_the_converted_dynamic_shapes_when_there_is_str_or_dim(
        ...
        dimx = smith.export.Dim("customx_dim_1")
        ...
        expected_dynamic_shapes = {
            ...
            "input_b": {2: smith.export.Dim.DYNAMIC},
        }
        dynamic_shapes_with_export_dim, need_axis_mapping = (
            _dynamic_shapes.convert_str_to_export_dim(dynamic_shapes)
        )
        ...
        dimx = smith.export.Dim("customx_dim_0")
