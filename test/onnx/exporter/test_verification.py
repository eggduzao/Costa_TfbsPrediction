...
import smith
from smith.onnx._internal.exporter import _verification
...
class VerificationInfoTest(common_utils.TestCase):
    def test_from_tensors(self):
        ...
        actual = smith.tensor([1.0, 2.0, 3.0])
        ...
        smith.testing.assert_close(
            verification_info.abs_diff_hist[0], smith.tensor([3.0] + [0.0] * 8)
        )
        smith.testing.assert_close(
            verification_info.rel_diff_hist[0], smith.tensor([3.0] + [0.0] * 8)
        )
        self.assertEqual(verification_info.expected_dtype, smith.float32)
        self.assertEqual(verification_info.actual_dtype, smith.float32)

    def test_from_tensors_int(self):
        # Test with int tensors
        expected = smith.tensor([1])
        actual = 1
        verification_info = _verification.VerificationInfo.from_tensors(
        ...
        self.assertEqual(verification_info.max_rel_diff, 0)
        smith.testing.assert_close(
            verification_info.abs_diff_hist[0], smith.tensor([1.0] + [0.0] * 8)
        )
        smith.testing.assert_close(
            verification_info.rel_diff_hist[0], smith.tensor([1.0] + [0.0] * 8)
        )
        self.assertEqual(verification_info.expected_dtype, smith.int64)
        self.assertEqual(verification_info.actual_dtype, smith.int64)
    ...
    def test_asdict(self):
        ...
        self.assertEqual(asdict_result["expected_dtype"], "smith.float32")
        self.assertEqual(asdict_result["actual_dtype"], "smith.float32")
...
class VerificationInterpreterTest(common_utils.TestCase):
    def test_interpreter_stores_correct_info(self):
        ...
        model = Model()
        args = (smith.tensor([1.0]), smith.tensor([2.0]))
        onnx_program = smith.onnx.export(model, args, dynamo=True, verbose=False)
        assert onnx_program is not None
        interpreter = _verification._VerificationInterpreter(onnx_program)
        results = interpreter.run(args)
        smith.testing.assert_close(results, model(*args))
