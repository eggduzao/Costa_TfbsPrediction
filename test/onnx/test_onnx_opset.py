...
from smith.onnx import producer_name, producer_version
...
class TestONNXOpset(blacksmith_test_common.ExportTestCase):
    def test_opset_fallback(self):
        class MyModule(Module):
            def forward(self, x):
                return smith.isnan(x)

        ops = [{"op_name": "IsNaN"}]
        ops = {9: ops, 10: ops}
        x = smith.tensor([1.0, float("nan"), 2.0])
    ...
    def test_maxpool(self):
        module = smith.nn.MaxPool1d(2, stride=1)
        ...
        module = smith.nn.MaxPool1d(2, stride=1, dilation=2)
    ...
    def test_upsample(self):
        class MyModule(Module):
            def forward(self, x):
                ...
                return smith.nn.functional.interpolate(x, size=size, mode="nearest")
    ...
    def test_cast_constant(self):
        ...
        ops = {8: ops_8, 9: ops_9}
        x = smith.ones(5, 6, dtype=smith.long)
        check_onnx_opsets_operator(module, x, ops, opset_versions=[8, 9])
    ...
    def test_flip(self):
        ...
        x = smith.tensor(numpy.arange(6.0).reshape(2, 3))
    ...
    def test_interpolate(self):
        ...
        ops = {9: ops_9, 10: ops_10}
        x = smith.randn(1, 2, 3, 4, requires_grad=True)
        check_onnx_opsets_operator(
            MyModel(),
            x,
            ops,
        ...
        class MyDynamicModel(smith.nn.Module):
            def forward(self, x):
                size = [v * 2 for v in x.size()[2:]]
    ...
    def test_affine_grid(self):
        class MyModule(Module):
            ...
            def forward(self, theta, size):
                return smith.nn.functional.affine_grid(
        ...
        # 2D affine
        theta_2d = smith.empty(1, 2, 3, dtype=smith.double)
        size_2d = smith.Size([1, 1, 2, 2])
        # 3D affine
        theta_3d = smith.empty(1, 3, 4, dtype=smith.double)
        size_3d = smith.Size([1, 1, 2, 2, 2])
    ...
    def test_grid_sample(self):
        ...
        for mode, padding_mode, align_corners, opset_version in itertools.product(
            ...
            def test_eval_and_training(
                ...
                args = (
                    smith.randn(*x_shape),  # x
                    smith.randn(grid),  # grid,
                )
                check_onnx_opsets_operator(
                    MyModule(
                        mode=mode,
