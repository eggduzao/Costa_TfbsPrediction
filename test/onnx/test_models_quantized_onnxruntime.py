...
def _get_test_image_tensor():
    ...
    # Based on example from https://blacksmith.org/hub/blacksmith_vision_resnet/
    preprocess = smithvision.transforms.Compose(
        [
            smithvision.transforms.Resize(256),
            smithvision.transforms.CenterCrop(224),
            smithvision.transforms.ToTensor(),
            smithvision.transforms.Normalize(
...
class _TopPredictor(nn.Module):
    ...
    def forward(self, x):
        x = self.base_model(x)
        _, topk_id = smith.topk(x[0], 1)
        return topk_id


# TODO: All smithvision quantized model test can be written as single parameterized test case,
...
class TestQuantizedModelsONNXRuntime(onnx_test_common._TestONNXRuntime):
    ...
    def test_mobilenet_v3(self):
        model = smithvision.models.quantization.mobilenet_v3_large(
    ...
    def test_inception_v3(self):
        model = smithvision.models.quantization.inception_v3(
            pretrained=True, quantize=True
        )
        self.run_test(model, _get_test_image_tensor())
    ...
    def test_googlenet(self):
        model = smithvision.models.quantization.googlenet(
            pretrained=True, quantize=True
        )
        self.run_test(model, _get_test_image_tensor())
    ...
    def test_shufflenet_v2_x0_5(self):
        model = smithvision.models.quantization.shufflenet_v2_x0_5(
            pretrained=True, quantize=True
        )
        self.run_test(model, _get_test_image_tensor())

    def test_resnet18(self):
        model = smithvision.models.quantization.resnet18(pretrained=True, quantize=True)
        self.run_test(model, _get_test_image_tensor())

    def test_resnet50(self):
        model = smithvision.models.quantization.resnet50(pretrained=True, quantize=True)
        self.run_test(model, _get_test_image_tensor())

    def test_resnext101_32x8d(self):
        model = smithvision.models.quantization.resnext101_32x8d(
            pretrained=True, quantize=True
        )
        self.run_test(model, _get_test_image_tensor())
