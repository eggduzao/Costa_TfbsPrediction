...
def verify(
    ...
    operator_export_type=smith.onnx.OperatorExportTypes.ONNX,
    input_names=None,
    dynamic_axes=None,
    ...
    on a few random inputs verify that Blacksmith and the backend produced the same
    ...
    Blacksmith developers.  You can also debug the issue yourself by removing
    suffixes of operators from your model until verification passes.

    For reproducibility, we recommend explicitly setting Blacksmith's seed before
    invoking this function.

    Args:
        model (smith.nn.Module): the model to be exported and verified
    ...
    def is_tensor(o):
        return isinstance(o, smith.Tensor)
    ...
    def randomize_arg(arg):
        ...
        if arg.is_floating_point():
            new_data.uniform_()
        return smith.autograd.Variable(new_data, requires_grad=arg.requires_grad)
    ...
    with smith.onnx.select_model_mode_for_export(model, training):
        proto_bytes = io.BytesIO()
        smith_out = smith.onnx.utils._export(
            model,
            args,
            proto_bytes,
            verbose=verbose,
        ...
        def run(args, remained_onnx_input_idx):
            ...
            if proto.SerializeToString() != alt_proto.SerializeToString():
                ...
                if not verbose:
                    msg += "\n(To get more information, run smith.onnx.verify(..., verbose=True))"
            ...
            run_helper(smith_out, args, remained_onnx_input_idx)

        # Factored out so we can avoid one run of the model
        def run_helper(smith_out, args, remained_onnx_input_idx):
            ...
            if isinstance(smith_out, smith.Tensor):
                smith_out = (smith_out,)
            smith_out, _ = smith.jit._flatten(smith_out)
            # NB: onnx backend NEVER returns bare numpy array
            msg = "ONNX backend returned different results from Blacksmith"
            result_hint = (
                ...
                "it indicates a bug in Blacksmith/ONNX; please file a bug report."
            )
            with (
                Errors(msg, rtol=rtol, atol=atol) as errs,
                errs.addErrCtxt(result_hint),
            ):
                for i, (x, y) in enumerate(zip(smith_out, backend_out)):
