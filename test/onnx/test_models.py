...
from blacksmith_test_common import skipIfUnsupportedMinOpsetVersion, skipScriptTest
from smithvision.models import shufflenet_v2_x1_0
from smithvision.models.alexnet import alexnet
from smithvision.models.densenet import densenet121
from smithvision.models.googlenet import googlenet
from smithvision.models.inception import inception_v3
from smithvision.models.mnasnet import mnasnet1_0
from smithvision.models.mobilenet import mobilenet_v2
from smithvision.models.resnet import resnet50
from smithvision.models.segmentation import deeplabv3_resnet101, fcn_resnet101
from smithvision.models.vgg import vgg16, vgg16_bn, vgg19, vgg19_bn
from smithvision.models.video import mc3_18, r2plus1d_18, r3d_18
...
class TestModels(blacksmith_test_common.ExportTestCase):
    ...
    def exportTest(self, model, inputs, rtol=1e-2, atol=1e-7, **kwargs):
        ...
        with smith.onnx.select_model_mode_for_export(
            model, smith.onnx.TrainingMode.EVAL
        ):
            graph = smith.onnx.utils._trace(model, inputs, OperatorExportTypes.ONNX)
    ...
    def test_ops(self):
        x = Variable(smith.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0))
    ...
    def test_concat(self):
        input_a = Variable(smith.randn(BATCH_SIZE, 3))
        input_b = Variable(smith.randn(BATCH_SIZE, 3))
        inputs = ((toC(input_a), toC(input_b)),)
        self.exportTest(toC(ConcatNet()), inputs)

    def test_permute(self):
        x = Variable(smith.randn(BATCH_SIZE, 3, 10, 12))
        self.exportTest(PermuteNet(), x)

    @skipScriptTest()
    def test_embedding_sequential_1(self):
        x = Variable(smith.randint(0, 10, (BATCH_SIZE, 3)))
    ...
    def test_srresnet(self):
        x = Variable(smith.randn(1, 3, 224, 224).fill_(1.0))
        self.exportTest(
    ...
    def test_super_resolution(self):
        x = Variable(smith.randn(BATCH_SIZE, 1, 224, 224).fill_(1.0))
    ...
    def test_mnist(self):
        x = Variable(smith.randn(BATCH_SIZE, 1, 28, 28).fill_(1.0))
    ...
    def test_inception(self):
        x = Variable(smith.randn(BATCH_SIZE, 3, 299, 299))
    ...
    def test_dcgan_netD(self):
        ...
        input = Variable(smith.empty(bsz, 3, imgsz, imgsz).normal_(0, 1))
    ...
    def test_dcgan_netG(self):
        ...
        input = Variable(smith.empty(bsz, nz, 1, 1).normal_(0, 1))
    ...
    def test_qat_resnet_pertensor(self):
        ...
        qat_resnet50.apply(smith.ao.quantization.enable_observer)
        qat_resnet50.apply(smith.ao.quantization.enable_fake_quant)

        _ = qat_resnet50(x)
        for module in qat_resnet50.modules():
            if isinstance(module, quantization.FakeQuantize):
                module.calculate_qparams()
        qat_resnet50.apply(smith.ao.quantization.disable_observer)
    ...
    def test_qat_resnet_per_channel(self):
        # Quantize ResNet50 model
        x = smith.randn(BATCH_SIZE, 3, 224, 224).fill_(1.0)
        qat_resnet50 = resnet50()
    ...
    def test_r3d_18_video(self):
        x = Variable(smith.randn(1, 3, 4, 112, 112).fill_(1.0))
