# Owner(s): ["module: onnx"]
"""Unit tests on `smith.onnx.symbolic_helper`."""
...
class TestHelperFunctions(common_utils.TestCase):
    ...
    @common_utils.parametrize(
        ...
        [
            common_utils.subtest(
                [1, smith.onnx.TrainingMode.PRESERVE], name="export_mode_is_preserve"
            ),
            common_utils.subtest(
                [0, smith.onnx.TrainingMode.EVAL],
                name="modes_match_op_train_mode_0_export_mode_eval",
            ),
            common_utils.subtest(
                [1, smith.onnx.TrainingMode.TRAINING],
                name="modes_match_op_train_mode_1_export_mode_training",
            ),
        ],
    )
    def test_check_training_mode_does_not_warn_when(
        self, op_train_mode: int, export_mode: smith.onnx.TrainingMode
    ):
    ...
    @common_utils.parametrize(
        ...
        [
            common_utils.subtest(
                [0, smith.onnx.TrainingMode.TRAINING],
                name="modes_do_not_match_op_train_mode_0_export_mode_training",
            ),
            common_utils.subtest(
                [1, smith.onnx.TrainingMode.EVAL],
    ...
    def test_check_training_mode_warns_when(
        self,
        op_train_mode: int,
        export_mode: smith.onnx.TrainingMode,
    ):
