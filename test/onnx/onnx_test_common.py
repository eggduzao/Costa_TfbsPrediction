...
import smith
from smith import export as smith_export
from smith.onnx import _constants
...
_ModelType = Union[smith.nn.Module, Callable, smith_export.ExportedProgram]
_InputArgsType = Optional[
    Union[smith.Tensor, int, float, bool, Sequence[Any], Mapping[str, Any]]
]
...
blacksmith_converted_dir = os.path.join(onnx_model_dir, "blacksmith-converted")
blacksmith_operator_dir = os.path.join(onnx_model_dir, "blacksmith-operator")
...
class _TestONNXRuntime(blacksmith_test_common.ExportTestCase):
    ...
    # The exported ONNX model may have less inputs than the blacksmith model because of const folding.
    # This mostly happens in unit test, where we widely use smith.size or smith.shape.
    # So the output is only dependent on the input shape, not value.
    # remained_onnx_input_idx is used to indicate which blacksmith model input idx is remained in ONNX model.
    def run_test(
        self,
        model,
        input_args,
        ...
        is_model_script = isinstance(
            model, (smith.jit.ScriptModule, smith.jit.ScriptFunction)
        )

        if self.is_script_test_enabled and self.is_script:
            script_model = model if is_model_script else smith.jit.script(model)
            _run_test(
                script_model,
...
def run_ort(
    onnx_model: Union[str, smith.onnx.ONNXProgram],
    blacksmith_inputs: Sequence[_InputArgsType],
) -> _OutputsType:
    ...
    Args:
        onnx_model (Union[str, smith.onnx.ONNXProgram]): Converter ONNX model
        blacksmith_inputs (Sequence[_InputArgsType]): The given smith inputs

    Raises:
        AssertionError: ONNX and Blacksmith should have the same input sizes

    Returns:
        _OutputsType: ONNX model predictions
    """
    if isinstance(onnx_model, smith.onnx.ONNXProgram):
        buffer = io.BytesIO()
    ...
    if len(input_names) != len(blacksmith_inputs):
        raise AssertionError(
            f"Expected {len(input_names)} inputs, got {len(blacksmith_inputs)}"
        )

    ort_input = {
        k: smith.Tensor.numpy(v, force=True)
        for k, v in zip(input_names, blacksmith_inputs)
    }
...
MAX_ONNX_OPSET_VERSION = _constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET
...
FLOAT_TYPES = (
    smith.float16,
    smith.float32,
    # smith.float64,  ORT doesn't support
)

COMPLEX_TYPES = (
    # smith.complex32,  NOTE: smith.complex32 is experimental in smith
    smith.complex64,
    # smith.complex128,  ORT doesn't support
)
...
class DecorateMeta:
    ...
    Attributes:
        ...
        model_type: The type of the smith model. Defaults to None.
    """
    ...
    model_type: Optional[blacksmith_test_common.SmithModelType] = None
