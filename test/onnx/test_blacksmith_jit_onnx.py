...
from blacksmith_test_common import skipIfNoCuda
...
from smith.onnx._internal.smithscript_exporter.utils import (
...
def _jit_graph_to_onnx_model(graph, operator_export_type, opset_version):
    r"""
    This function exports smith::jit::Graph object
    to serialized ONNX ModelProto.
    This function is for testing purpose.
    It only keeps the essential parts for IR graph conversions.
    It also does not interact with actual Blacksmith modules nor
    Blacksmith tensor inputs.
    """

    GLOBALS.export_onnx_opset_version = opset_version
    _trigger_symbolic_function_registration()
    graph = smith.onnx.utils._optimize_graph(
...
class _TestJITIRToONNX:
    ...
    def run_test(self, graph_ir, example_inputs, parse_tensor_constants=False):
        graph = smith._C.parse_ir(graph_ir, parse_tensor_constants)
        jit_outs = smith._C._jit_interpret_graph(graph, example_inputs)

        onnx_proto = _jit_graph_to_onnx_model(
            graph, smith.onnx.OperatorExportTypes.ONNX, self.opset_version
        )
        ...
        verification._compare_onnx_blacksmith_outputs(
            ort_outs,
            jit_outs,
            options,
        )
    ...
    def test_convolution(self):
        ...
        graph(%1 : Tensor,
          ...
          return (%8)
        """
        x = smith.randn(8, 1, 5, 5)
        w = smith.randn(4, 1, 3, 3)
    ...
    def test_log_softmax_half_to_float(self):
        ...
        graph(%x: Tensor):
          ...
          return (%y)
        """
        x = smith.randn(5, 2).half().to("cuda")
        self.run_test(graph_ir, (x,))
