...
from smith.onnx.ops import _impl, _symbolic_impl
...
class SchemaTest(common_utils.TestCase):
    def test_symbolic_has_correct_schema(self):
        smith.library.opcheck(
            _symbolic_impl._symbolic,
            ([smith.tensor(1)], "CustomOp", 1),
            dict(
                shape=[
                    1,
                ],
    ...
    def test_symbolic_multi_out_has_correct_schema(self):
        smith.library.opcheck(
            ...
            ([smith.tensor(1)], "CustomMultiOutOp", [1, 2, 10]),
            dict(
                shapes=[[1, 2], [42], []],
                attr_keys=["key"],
                attr_types=["i"],
...
class SymbolicOpsTest(common_utils.TestCase):
    def test_symbolic_accepts_valid_inputs(self):
        output = smith.onnx.ops.symbolic(
        ...
        self.assertEqual(output.shape, smith.Size([1, 2, 3]))
        self.assertEqual(output.dtype, smith.float32)
        self.assertEqual(output.device, smith.device("cpu"))
    ...
    def test_symbolic_accepts_valid_inputs_empty_shape(self):
        ...
        self.assertEqual(output.shape, smith.Size([]))
    ...
    def test_symbolic_accepts_valid_inputs_int4_type(self):
        ...
        # We use smith uint8 for int4
        self.assertEqual(output.dtype, smith.uint8)

    def test_symbolic_is_exportable(self):
        class Model(smith.nn.Module):
            def forward(self, x: smith.Tensor):
                return smith.onnx.ops.symbolic(
                    "custom_domain::CustomOp",
                    (x, None),
                    dict(
                        int_key=1,
        ...
        onnx_program = smith.onnx.export(
            Model(), (smith.tensor(1),), dynamo=True, verbose=False
        )
    ...
    def test_symbolic_preserves_dynamic_shapes(self):
        ...
        onnx_program = smith.onnx.export(
            Model(),
            (smith.zeros(2, 3), smith.zeros(1, 2)),
    ...
    def test_symbolic_multi_out_accepts_valid_inputs(self):
        outputs = smith.onnx.ops.symbolic_multi_out(
        ...
        self.assertEqual(outputs[0].shape, smith.Size([1, 2]))
        self.assertEqual(outputs[0].dtype, smith.float32)
        self.assertEqual(outputs[1].shape, smith.Size([42]))
        self.assertEqual(outputs[1].dtype, smith.int32)
        self.assertEqual(outputs[2].shape, smith.Size([]))
        self.assertEqual(outputs[2].dtype, smith.float8_e4m3fn)
        self.assertEqual(outputs[0].device, smith.device("cpu"))
        self.assertEqual(outputs[1].device, smith.device("cpu"))
        self.assertEqual(outputs[2].device, smith.device("cpu"))
    ...
    def test_symbolic_multi_out_accepts_valid_inputs_empty_shape(self):
        ...
        self.assertEqual(outputs[0].shape, smith.Size([]))
    ...
    def test_symbolic_multi_out_accepts_valid_inputs_int4_type(self):
        ...
        # We use smith uint8 for int4
        self.assertEqual(outputs[0].dtype, smith.uint8)

    def test_symbolic_multi_out_is_exportable(self):
        class Model(smith.nn.Module):
            def forward(self, x: smith.Tensor):
                return smith.onnx.ops.symbolic_multi_out(
                    "custom_domain::CustomOp",
                    (x, None),
                    dict(
                        int_key=1,
                    ...
                    dtypes=(smith.float32, smith.int32, smith.float8_e4m3fn),
                    shapes=([1, 2], [42], []),
                    version=1,
                    metadata_props={"meta_key": "meta_value"},
                )
    ...
    def test_symbolic_multi_out_raises_when_dtypes_and_shapes_differ(self):
        with self.assertRaises(RuntimeError):
            smith.onnx.ops.symbolic_multi_out(
                "custom_domain::CustomMultiOutOp",
                (smith.tensor(1),),
                dict(
                    int_key=1,
                ...
                dtypes=(smith.float32, smith.int32),
                shapes=([1, 2], [42], []),
                version=1,
                metadata_props={"meta_key": "meta_value"},
            )
...
class NativeOnnxOpsTest(common_utils.TestCase):
    ...
    def test_onnx_ops_can_be_decomposed_to_aten(self):
        ...
        position_ids_data = smith.randint(0, 50, (2, 4)).long()
        ...
        # The program can be decomposed into aten ops so it is fully compatible with the Blacksmith ecosystem
        aten_decomped = ep.run_decompositions(smith.onnx.ops.aten_decompositions())
        self.assertNotIn(
            "onnx.RotaryEmbedding.opset23",
    ...
    def test_rotary_embedding(self):
        ...
        result = smith.onnx.ops.rotary_embedding(
            input_data, cos_cache_data, sin_cache_data, position_ids_data
        )
    ...
    def test_rotary_embedding_3d(self):
        num_heads = 2
        input_data = smith.rand(2, 3, 8)
        sin_cache_data = smith.rand(2, 3, 2)
        cos_cache_data = smith.rand(2, 3, 2)
        ...
        dynamic_shapes = {
            "input_data": {0: smith.export.Dim.DYNAMIC},
            "cos_cache_data": {0: smith.export.Dim.DYNAMIC},
            "sin_cache_data": {0: smith.export.Dim.DYNAMIC},
        }
    ...
    def test_attention_without_past_kv_caches(self):
        ...
        Q = smith.rand(batch_size, q_num_heads, q_seq_len, head_size)
        K = smith.rand(batch_size, kv_num_heads, kv_seq_len, head_size)
        V = smith.rand(batch_size, kv_num_heads, kv_seq_len, head_size)

        # Test eager mode
        smith.library.opcheck(_impl.attention_23, (Q, K, V))
        output, present_key, present_value, qk_output = smith.onnx.ops.attention(
            Q, K, V
        )
    ...
    def test_attention_3d_inputs(self):
        ...
        Q = smith.rand(batch_size, q_seq_len, q_num_heads * head_size)
        K = smith.rand(batch_size, kv_seq_len, kv_num_heads * head_size)
        V = smith.rand(batch_size, kv_seq_len, kv_num_heads * head_size)
    ...
    def test_attention_kv_num_heads(self, name: str, kv_num_heads: int):
        ...
        self.assertEqual(present_key.shape, K.shape)
        self.assertEqual(present_value.shape, V.shape)
        smith.testing.assert_close(output, expected)
    ...
    def test_attention_with_2d_mask(self):
        ...
        bool_mask = smith.randint(0, 2, (q_seq_len, kv_seq_len), dtype=smith.bool)
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=bool_mask))
        output_bool, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=bool_mask)

        # Test with float mask
        float_mask = smith.randn(q_seq_len, kv_seq_len)
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=float_mask))
        output_float, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=float_mask)
    ...
    def test_attention_with_4d_mask(self):
        ...
        # Test with boolean mask
        bool_mask = smith.randint(
            0, 2, (batch_size, q_num_heads, q_seq_len, kv_seq_len), dtype=smith.bool
        )
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=bool_mask))
        output_bool, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=bool_mask)

        # Test with float mask
        float_mask = smith.randn(batch_size, q_num_heads, q_seq_len, kv_seq_len)
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=float_mask))
        output_float, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=float_mask)
    ...
    def test_attention_with_zero_float_mask(self):
        ...
        zero_mask = smith.zeros(q_seq_len, kv_seq_len)
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=zero_mask))
        output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=zero_mask)
    ...
    def test_attention_with_causal_mask_pattern(self):
        ...
        causal_mask = smith.tril(smith.ones(q_seq_len, kv_seq_len, dtype=smith.bool))
        smith.library.opcheck(
            _impl.attention_23, (Q, K, V), dict(attn_mask=causal_mask)
        )
        output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=causal_mask)
    ...
    def test_attention_with_gqa_and_mask(self):
        ...
        mask_2d = smith.randint(0, 2, (q_seq_len, kv_seq_len), dtype=smith.bool)
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=mask_2d))
        output_2d, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask_2d)

        # Test 4D mask with GQA (note: using q_num_heads for mask heads)
        mask_4d = smith.randint(
            0, 2, (batch_size, q_num_heads, q_seq_len, kv_seq_len), dtype=smith.bool
        )
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(attn_mask=mask_4d))
        output_4d, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask_4d)
    ...
    def test_attention_causal(self):
        ...
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(is_causal=True))
        output, _, _, _ = smith.onnx.ops.attention(Q, K, V, is_causal=True)
    ...
    def test_attention_with_past_kv(self):
        ...
        K = smith.rand(batch_size, kv_num_heads, kv_seq_len, head_size)
        V = smith.rand(batch_size, kv_num_heads, kv_seq_len, head_size)
        past_key = smith.rand(batch_size, kv_num_heads, past_seq_len, head_size)
        past_value = smith.rand(batch_size, kv_num_heads, past_seq_len, head_size)
        ...
        output, present_key, present_value, _ = smith.onnx.ops.attention(
    ...
    def test_attention_with_softcap(self):
        ...
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(softcap=30.0))
        output, _, _, _ = smith.onnx.ops.attention(Q, K, V, softcap=30.0)
    ...
    def test_attention_qk_output_modes(self):
        ...
        for mode in [0, 1, 2, 3]:
            ...
            output, _, _, qk_output = smith.onnx.ops.attention(
    ...
    def test_attention_custom_scale(self):
        ...
        custom_scale = 0.25
        smith.library.opcheck(_impl.attention_23, (Q, K, V), dict(scale=custom_scale))
        output, _, _, _ = smith.onnx.ops.attention(Q, K, V, scale=custom_scale)
    ...
    def test_attention_export(self):
        ...
        class AttentionModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V)
                return output
    ...
    def test_attention_export_with_dynamic_shapes(self):
        ...
        attn_mask = smith.randint(
            0, 2, (batch_size, 1, q_seq_len, kv_seq_len), dtype=smith.bool
        )

        class AttentionModel(smith.nn.Module):
            def forward(self, Q, K, V, attn_mask):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=attn_mask)
                return output
    ...
    def test_attention_3d_export(self):
        ...
        class AttentionModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, _, _, _ = smith.onnx.ops.attention(
                    Q, K, V, q_num_heads=q_num_heads, kv_num_heads=kv_num_heads
                )
                return output
        ...
        class AttentionModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, present_key, present_value, qk_output = (
                    smith.onnx.ops.attention(Q, K, V)
                )
                return output

        model = AttentionModel()

        ep = smith.export.export(model, (Q, K, V))
        self.assertIn(
    ...
    def test_attention_export_with_all_optional_inputs(self):
        ...
        V = smith.rand(batch_size, kv_num_heads, kv_seq_len, head_size)
        attn_mask = smith.randint(
            0, 2, (1, 1, q_seq_len, kv_seq_len + past_seq_len), dtype=smith.bool
        )
        past_key = smith.rand(batch_size, kv_num_heads, past_seq_len, head_size)
        past_value = smith.rand(batch_size, kv_num_heads, past_seq_len, head_size)

        class FullAttentionModel(smith.nn.Module):
            def forward(self, Q, K, V, attn_mask, past_key, past_value):
                output, present_key, present_value, qk_matmul = (
                    smith.onnx.ops.attention(
                        Q,
                        K,
                        V,
                        attn_mask=attn_mask,
    ...
    def test_attention_export_3d_with_num_heads_attributes(self):
        ...
        class Attention3DModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, _, _, _ = smith.onnx.ops.attention(
                    Q, K, V, q_num_heads=q_num_heads, kv_num_heads=kv_num_heads
                )
    ...
    def test_attention_export_with_all_attributes(self):
        ...
        class FullAttributesModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, _, _, _ = smith.onnx.ops.attention(
                    Q,
                    K,
    ...
    def test_attention_export_with_different_mask_shapes(self):
        ...
        class Mask2DModel(smith.nn.Module):
            def forward(self, Q, K, V, mask):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask)
                return output
        ...
        # Test 3D mask
        mask_3d = smith.randint(
            0, 2, (batch_size, 1, q_seq_len, kv_seq_len), dtype=smith.bool
        )

        class Mask3DModel(smith.nn.Module):
            def forward(self, Q, K, V, mask):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask)
                return output
        ...
        class Mask4DModel(smith.nn.Module):
            def forward(self, Q, K, V, mask):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask)
                return output
    ...
    def test_attention_export_with_float_mask(self):
        ...
        class FloatMaskModel(smith.nn.Module):
            def forward(self, Q, K, V, mask):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V, attn_mask=mask)
                return output
    ...
    def test_attention_export_qk_output_modes(self):
        ...
        for mode in [0, 1, 2, 3]:
            ...
            class QKOutputModel(smith.nn.Module):
    ...
    def test_attention_export_mqa(self):
        ...
        class MQAModel(smith.nn.Module):
            def forward(self, Q, K, V):
                output, _, _, _ = smith.onnx.ops.attention(Q, K, V)
                return output
    ...
    def test_attention_export_with_softmax_precision(
        ...
        class SoftmaxPrecisionModel(smith.nn.Module):
    ...
    def test_attention_export_gqa(self):
        ...
        class AttentionOutputsModel(smith.nn.Module):
            def forward(self, Q, K, V):
                result, _, _, _ = smith.onnx.ops.attention(Q, K, V)
                return result
