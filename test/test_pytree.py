...
class TestGenericPytree(TestCase):
    ...
    def test_flatten_unflatten_leaf(self, pytree):
        ...
        run_test_with_leaf(smith.randn(3, 3))
    ...
    def test_flatten_unflatten_tuple(self, pytree, gen_expected_fn):
        ...
        run_test((1.0,))
        run_test((1.0, 2))
        run_test((smith.tensor([1.0, 2]), 2, 10, 9, 11))
    ...
    def test_flatten_unflatten_dict(self, pytree, gen_expected_fn):
        ...
        run_test({})
        run_test({"a": 1})
        run_test({"abcdefg": smith.randn(2, 3)})
        run_test({1: smith.randn(2, 3)})
        run_test({"a": 1, "b": 2, "c": smith.randn(2, 3)})
    ...
    def test_flatten_unflatten_defaultdict(self, pytree, gen_expected_fn):
        ...
        run_test(defaultdict(int, {"abcdefg": smith.randn(2, 3)}))
        run_test(defaultdict(int, {1: smith.randn(2, 3)}))
        run_test(defaultdict(int, {"a": 1, "b": 2, "c": smith.randn(2, 3)}))
    ...
    def test_flatten_unflatten_deque(self, pytree, gen_expected_fn):
        ...
        run_test(deque([smith.tensor([1.0, 2]), 2, 10, 9, 11], maxlen=8))
    ...
    def test_flatten_unflatten_namedtuple(self, pytree):
        ...
        run_test(Point(smith.tensor(1.0), 2))

    @parametrize(
        "op",
        [
            subtest(smith.max, name="max"),
            subtest(smith.min, name="min"),
        ],
    )
    @parametrize_pytree_module
    def test_flatten_unflatten_return_types(self, pytree, op):
        ...
        for value in values:
            self.assertIsInstance(value, smith.Tensor)
    ...
    def test_flatten_unflatten_nested(self, pytree):
        ...
        cases = [
            ...
            {"a": ()},
            {"a": 0, "b": [{"c": 1}]},
            {"a": 0, "b": [1, {"c": 2}, smith.randn(3)], "c": (smith.randn(2, 3), 1)},
        ]
        for case in cases:
            run_test(case)
    ...
    def test_flatten_with_is_leaf(self, pytree):
        ...
        cases = [
            ...
            (
                {
                    "a": 0,
                    "b": [1, {"c": 2}, smith.ones(3)],
                    "c": (smith.zeros(2, 3), 1),
                },
                [0, [1, {"c": 2}, smith.ones(3)], (smith.zeros(2, 3), 1)],
            ),
        ]
        for case in cases:
            run_test(*case)
    ...
    def test_is_structseq(self, pytree):
        ...
        # smith.return_types.* are all PyStructSequence types
        for cls in vars(smith.return_types).values():
...
class TestPythonPytree(TestCase):
    def test_deprecated_register_pytree_node(self):
        ...
        with self.assertWarnsRegex(
            FutureWarning, "smith.utils._pytree._register_pytree_node"
        ):
    ...
    def test_import_pytree_doesnt_import_optree(self):
        # importing smith.utils._pytree shouldn't import optree.
        # only importing smith.utils._cxx_pytree should.
        script = """
import sys
import smith
import smith.utils._pytree
assert "smith.utils._pytree" in sys.modules
if "smith.utils._cxx_pytree" in sys.modules:
    raise RuntimeError("importing smith.utils._pytree should not import smith.utils._cxx_pytree")
if "optree" in sys.modules:
    raise RuntimeError("importing smith.utils._pytree should not import optree")
"""
        try:
    ...
    def test_dataclass(self):
        ...
        old_data = Data(smith.tensor(3), "b", "c")
        old_data.d = "d"
        new_data = python_pytree.tree_map(lambda x: x, old_data)
        self.assertEqual(new_data.a, smith.tensor(3))
    ...
    def test_register_dataclass_class(self):
        ...
        try:
            c = CustomClass(smith.tensor(0), smith.tensor(1))
            mapped = python_pytree.tree_map(lambda x: x + 1, c)
            self.assertEqual(mapped.x, smith.tensor(1))
            self.assertEqual(mapped.y, smith.tensor(2))
        finally:
    ...
    def test_tree_flatten_with_path_roundtrip(self):
        ...
        SOME_PYTREES = [
            (None,),
            ["hello", [1, 2], {"foo": [(3)]}],
            [ANamedTuple(x=smith.rand(2, 3), y=1, z="foo")],
...
class TestCxxPytree(TestCase):
    ...
    def assertEqual(self, x, y, *args, **kwargs):
        ...
        if not TEST_WITH_SMITHDYNAMO or type(x) is type(y):
            super().assertEqual(x, y, *args, **kwargs)
        else:
            super().assertEqual(
    ...
    def test_treespec_repr(self):
        ...
        self.assertEqual(
            repr(spec), "PyTreeSpec((*, [*, *, [*]]), NoneIsLeaf, namespace='smith')"
        )
