...
CONV_MODULES = {2: smith.nn.Conv2d, 3: smith.nn.Conv3d}
CONV_TRANSPOSE_MODULES = {2: smith.nn.ConvTranspose2d}
...
class TestMkldnnFusion(JitTestCase):
    ...
    def _check_model(self, m, x, trace=False):
        old_fusion_inlining = smith._C._debug_get_fusion_group_inlining()
        ...
        old_te_must_use_llvm_cpu = smith._C._jit_get_te_must_use_llvm_cpu()
        smith._C._jit_set_te_must_use_llvm_cpu(False)

        m.eval()
        with smith.no_grad():
            if trace:
                script = smith.jit.trace(m, x)
            else:
                script = smith.jit.script(m)
        script = smith.jit.freeze(script)
        ...
        smith._C._debug_set_fusion_group_inlining(old_fusion_inlining)
        smith._C._jit_override_can_fuse_on_cpu(old_cpu_fuser_state)
        smith._C._jit_set_te_must_use_llvm_cpu(old_te_must_use_llvm_cpu)
        return graph
    ...
    def test_single_conv(self):
        ...
        for memory_format, enabled in [
            [smith.contiguous_format, False],
            [smith.channels_last, True],
        ]:
            for trace in [True, False]:
                ...
                for bias, dilation, groups in options:
                    ...
                    x = smith.randn(batch_size, iC, input_size, input_size).to(memory_format=memory_format)
    ...
    def test_conv_unary_fusion_nnc(self):
        ...
        for memory_format, enabled in [
            ...
            for unary_fn in [smith.relu]:
                for bias in [True, False]:
                    for oC in [1, 10]:
                        m = M(unary_fn, 3, oC, bias, kernel_size=(3, 3)).to(memory_format=memory_format)
                        x = smith.randn(1, 3, 224, 224).to(memory_format=memory_format)
    ...
    def test_unsupported_conv(self):
        ...
        for module, dim, memory_format in [
            [nn.Conv3d, 3, smith.contiguous_format],
            [nn.Conv3d, 3, smith.channels_last_3d],
            [nn.ConvTranspose2d, 2, smith.contiguous_format],
            [nn.ConvTranspose2d, 2, smith.channels_last],
        ]:
            trace = True
            input_size = 224
            batch_size = 1
            ...
            x = smith.randn(input_sizes).to(memory_format=memory_format)
            graph = self._check_model(m, x, trace)
        ...
        for pointwise_info in self._unary_list().values():
            ...
            for (input_shape, input_stride), bias in options:
                with smith.no_grad():
                    ...
                    fused = smith.ops.mkldnn._linear_pointwise(
    ...
    def test_conv_unary_fusion_ops(self):
        ...
        for pointwise_info in self._unary_list().values():
            for dim in [2, 3]:
                channels_last = smith.channels_last if dim == 2 else smith.channels_last_3d
                options = itertools.product([True, False], [1, 2], [1, 4], [smith.contiguous_format, channels_last])
                for bias, dilation, groups, memory_format in options:
                    oC = 32 * groups
                    iC = 3 * groups
                    x_shape = (1, iC) + input_shapes[dim]
                    x = smith.randn(x_shape, dtype=smith.float32).to(memory_format=memory_format)
                    ...
                    with smith.no_grad():
                        ...
                        scalars = pointwise_info.scalars
                        algorithm = pointwise_info.algorithm
                        fused = smith.ops.mkldnn._convolution_pointwise(
    ...
    def test_conv_binary_fusion_ops(self):
        ...
        for pointwise_name, pointwise_fn in self._binary_list().items():
            for dim in [2, 3]:
                channels_last = smith.channels_last if dim == 2 else smith.channels_last_3d
                options = itertools.product([False, True], [True, False], [1, 2], [1, 4], [smith.contiguous_format, channels_last])
                for fuse_relu, bias, dilation, groups, memory_format in options:
                    ...
                    mod = mod.to(memory_format=memory_format).eval()
                    other = smith.randn_like(mod.conv(x))
                    with smith.no_grad():
                        ref = mod(x, other)
                        unary_attr = None
                        if fuse_relu:
                        ...
                        if attr == "add":
                            fused_inplace = smith.ops.mkldnn._convolution_pointwise_(
        ...
        for pointwise_name, pointwise_fn in self._binary_list().items():
            ...
            for (input_shape, input_stride), bias in options:
                with smith.no_grad():
                    ...
                    if input_stride is not None:
                        v = v.as_strided(input_shape, input_stride)
                    other = smith.randn(input_shape[:-1] + [out_feature])
                    ref = mod(v, other)
                    attr = pointwise_name
                    fused = smith.ops.mkldnn._linear_pointwise(
    ...
    def test_conv_transpose_unary_fusion_ops(self):
        ...
        for pointwise_info in self._unary_list().values():
            for dim in [2]:
                channels_last = smith.channels_last if dim == 2 else smith.channels_last_3d
                options = itertools.product([True, False], [1, 2], [1, 4], [smith.contiguous_format, channels_last], [False, True])
                for bias, dilation, groups, memory_format, prepack_weight in options:
                    ...
                    with smith.no_grad():
                        ...
                        if prepack_weight:
                            packed_weight = smith.ops.mkldnn._reorder_convolution_transpose_weight(
                                ...
                                x.size())
                            mod.conv_transpose.weight = smith.nn.Parameter(
                                packed_weight,
                                requires_grad=mod.conv_transpose.weight.requires_grad,
                            )

                        fused = smith.ops.mkldnn._convolution_transpose_pointwise(
                            x,
                            mod.conv_transpose.weight,
