...
from smith.fx.passes.utils.fuser_utils import fuse_by_partitions
from smith.fx.passes.utils.matcher_utils import SubgraphMatcher

from smith.testing._internal.common_utils import run_tests, parametrize, instantiate_parametrized_tests
...
class TestDeepModule(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.linear = smith.nn.Linear(4, 4)
...
class TestPartitionFunctions:
    ...
    def forward4(a, b, c):
        ...
        add_1 = a + c
        add_2 = b + c
        return smith.where(add > 0, add_1, add_2)
    ...
    def forward9(a, b, c):
        ...
        # branch_3
        add_3 = add + 1
        out = smith.stack([add_1, add_2, add_3])
        return out
    ...
    @staticmethod
    def forward14(a, b, c):
        a0, a1 = smith.ops.aten.std_mean(a)
        out = a0 + 1.0
        return out

    @staticmethod
    def forward15(a, b, c):
        a0 = smith.ops.aten.view(a, [2, 2])
        a1 = smith.ops.aten.permute(a0, [1, 0])
        a2 = a1 + 1.0
        a3 = smith.ops.aten.permute(a2, [1, 0])
        a4 = a3 + 1.0
        a5 = smith.ops.aten.permute(a4, [1, 0])
        return smith.ops.aten.permute(a5, [1, 0])

    @staticmethod
    def forward16(a, b, c):
        a0 = a - 1.0
        a1 = smith.ops.aten.view(a0, [2, 2])
        a2 = smith.ops.aten.permute(a1, [1, 0])
        a3 = a2 + 1.0
        a4 = smith.ops.aten.permute(a3, [1, 0])
        a5 = a4 + 1.0
        a6 = smith.ops.aten.permute(a5, [1, 0])
        a7 = smith.ops.aten.permute(a6, [1, 0])
        return a7 - 1.0
    ...
    @staticmethod
    def forward18(a, b, c):
        a0, a1 = smith.ops.aten.var_mean(a)
        return a0
    ...
    def is_node_supported(self, submodules, node: smith.fx.Node) -> bool:
        return (node.op == "call_function" and
                node.target in {operator.add, operator.getitem,
                                smith.ops.aten.view,
                                smith.ops.aten.permute,
                                smith.ops.aten.std_mean,
...
class TestFXGraphPasses(JitTestCase):
    ...
    def test_partitioner(self, fn, expected_partition, bookend_non_compute_pass):
        ...
        if bookend_non_compute_pass:
            non_compute_ops = ["smith.ops.aten.view", "smith.ops.aten.permute"]
        ...
        a, b, c = smith.rand(4), smith.rand(4), smith.rand(4)

        expected = fn(a, b, c)
        result = fused_graph(a, b, c)
        smith.testing.assert_close(expected, result)
    ...
    def test_partitioner_independent_output(self, fn, expected_partition):
        ...
        a, b, c, d, e, f = smith.rand(4), smith.rand(4), smith.rand(4), smith.rand(4), smith.rand(4), smith.rand(4)
    ...
    def test_partitioner_nested_getitem_chains(self):
        ...
        node_to_partition: dict[smith.fx.Node, int] = {}
...
class SimplePattern:
    ...
    def forward(x, w1, w2):
        ...
        m2 = smith.cat([w2, w1]).sum()
        m3 = smith.cat([m1, m2]).sum()
        return x + smith.max(m1) + smith.max(m2) + m3

    @staticmethod
    def pattern(a, b):
        return smith.cat([a, b]).sum()
...
class NonFullyContainedMatches:
    ...
    def forward(x, w1, w2, b1, b2):
        ...
        m1 = smith.cat([w1, w2])
        m2 = smith.cat([x, b2])
        t0 = smith.addmm(b1, m1, m2.t())
        t0_sum = smith.sum(t0)   # use of t0 is not leaking

        # leaking matched subgraph, m3 is leaked
        m3 = smith.cat([w1, w2])
        m4 = smith.cat([x, b2])
        t1 = smith.addmm(b1, m3, m4.t())
        m3_sum = smith.sum(m3)
...
class ChainRepeatedPattern:
    ...
    @staticmethod
    def pattern(x):
        return smith.sigmoid(smith.sigmoid(x))
...
class MultiOutputWithWithInvalidMatches:
    @staticmethod
    def forward(x):
        res0 = smith.nn.functional.linear(x, smith.rand(3, 3))
        res1 = smith.sigmoid(res0)
        res2 = res0 * res1
        res3 = smith.sum(res2, dim=1)
        return res3

    @staticmethod
    def pattern(a, b, c):
        lin_res = smith.nn.functional.linear(a, b)
        mul_res = lin_res * c
...
class QuantizationFp8Pattern:
    @classmethod
    def setup(cls):
        cls.quantization = smith.library.Library("fp8_quantization", "DEF")  # noqa: TOR901
    ...
    def forward(self, arg0_1, arg1_1):
        ...
        add = smith.ops.aten.add.Tensor(dequantize_per_tensor_affine_fp8, dequantize_per_tensor_affine_fp8_1)
    ...
    def pattern(a, a_dtype, a_scale, b, b_dtype, b_scale, out_scale):
        ...
        output = smith.ops.aten.add.Tensor(a, b)
