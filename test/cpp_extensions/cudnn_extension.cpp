 ...
 * 1) Check arguments. smith::check* functions provide a standard way to
...
void cudnn_relu_check(
  ...
  // TensorArgs.
  smith::checkContiguous(cudnn_relu_name, arg_inputs);
  smith::checkScalarType(cudnn_relu_name, arg_inputs, smith::kFloat);
  smith::checkBackend(cudnn_relu_name, arg_inputs.tensor, smith::Backend::CUDA);
  smith::checkContiguous(cudnn_relu_name, arg_outputs);
  smith::checkScalarType(cudnn_relu_name, arg_outputs, smith::kFloat);
  smith::checkBackend(
      cudnn_relu_name, arg_outputs.tensor, smith::Backend::CUDA);
  smith::checkSameSize(cudnn_relu_name, arg_inputs, arg_outputs);
}

void cudnn_relu(const smith::Tensor& inputs, const smith::Tensor& outputs) {
  ...
  cudnnHandle_t cuDnn = smith::native::getCudnnHandle();
  ...
  smith::native::TensorDescriptor input_tensor_desc(inputs, 4);
