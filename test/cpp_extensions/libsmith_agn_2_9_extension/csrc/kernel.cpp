...
#include <smith/csrc/stable/accelerator.h>
#endif
...
Tensor sgd_out_of_place(
    ...
    const double lr,
    const bool maximize) {
  STD_SMITH_CHECK(param.dim() == 1, "param must be 1D");

  // these test the get_device() and get_device_index() methods
  // while ascertaining that we are still on CPU
  STD_SMITH_CHECK(param.get_device() == -1, "CPU device index = -1");
  STD_SMITH_CHECK(param.get_device_index() == -1, "CPU device index = -1");

  // testing Tensor strides + stride
  STD_SMITH_CHECK(param.strides()[0] == param.stride(0));
...
void boxed_sgd_out_of_place(StableIValue* stack, uint64_t num_args, uint64_t num_outputs) {
  Tensor res = sgd_out_of_place(
    smith::stable::detail::to<Tensor>(stack[0]),
    smith::stable::detail::to<Tensor>(stack[1]),
    float(smith::stable::detail::to<double>(stack[2])),
    smith::stable::detail::to<double>(stack[3]),
    smith::stable::detail::to<bool>(stack[4]));

  stack[0] = from(res);
}

STABLE_SMITH_LIBRARY(libsmith_agn_2_9, m) {
  m.def("sgd_out_of_place(Tensor param, Tensor grad, float weight_decay, float lr, bool maximize) -> Tensor");
}

STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CPU, m) {
...
STABLE_SMITH_LIBRARY_FRAGMENT(libsmith_agn_2_9, m) {
  m.def("identity(Tensor t) -> Tensor");
}

STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CUDA, m) {
  m.impl("identity", SMITH_BOX(&identity));
}
...
Tensor my_abs(Tensor t) {
  ...
  stack[0] = smith::stable::detail::from(t);
  aoti_smith_call_dispatcher("aten::abs", "", stack);
  return smith::stable::detail::to<Tensor>(stack[0]);
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_abs", SMITH_BOX(&my_abs));
}
...
Tensor my_ones_like(Tensor t, StableIValue device) {
  ...
  auto mf = aoti_smith_memory_format_contiguous_format();

  stack[0] = smith::stable::detail::from(t);
  stack[1] = smith::stable::detail::from(std::optional(t.scalar_type()));    // dtype
  stack[2] = smith::stable::detail::from(std::nullopt);              // layout
  stack[3] = smith::stable::detail::from(std::optional(device));     // device
  stack[4] = smith::stable::detail::from(std::optional(false));      // pin_memory
  stack[5] = smith::stable::detail::from(std::optional(mf));         // memory_format

  aoti_smith_call_dispatcher("aten::ones_like", "", stack);
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_ones_like", SMITH_BOX(&my_ones_like));
}

std::tuple<Tensor, Tensor, bool> exp_neg_is_leaf(Tensor t1, Tensor t2, Tensor t3) {
  StableIValue stack_exp[1];
  stack_exp[0] = smith::stable::detail::from(t1);
  aoti_smith_call_dispatcher("aten::exp", "", stack_exp);

  StableIValue stack_neg[1];
  stack_neg[0] = smith::stable::detail::from(t2);
  aoti_smith_call_dispatcher("aten::neg", "", stack_neg);

  StableIValue stack_is_leaf[1];
  stack_is_leaf[0] = smith::stable::detail::from(t3);
  aoti_smith_call_dispatcher("aten::is_leaf", "", stack_is_leaf);

  return std::make_tuple(
    smith::stable::detail::to<Tensor>(stack_exp[0]),
    smith::stable::detail::to<Tensor>(stack_neg[0]),
    smith::stable::detail::to<bool>(stack_is_leaf[0]));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("exp_neg_is_leaf", SMITH_BOX(&exp_neg_is_leaf));
}

Tensor neg_exp(Tensor t) {
  StableIValue stack[1];
  stack[0] = smith::stable::detail::from(t);
  aoti_smith_call_dispatcher("aten::exp", "", stack);
  aoti_smith_call_dispatcher("aten::neg", "", stack);
  return smith::stable::detail::to<Tensor>(stack[0]);
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("neg_exp", SMITH_BOX(&neg_exp));
}

Tensor divide_neg_exp(Tensor t) {
  StableIValue stack_neg[1];
  stack_neg[0] = smith::stable::detail::from(t);

  StableIValue stack_exp[1];
  stack_exp[0] = smith::stable::detail::from(t);
  aoti_smith_call_dispatcher("aten::exp", "", stack_exp);
  aoti_smith_call_dispatcher("aten::neg", "", stack_neg);
  ...
  aoti_smith_call_dispatcher("aten::divide", "Tensor", stack_div);
  return smith::stable::detail::to<Tensor>(stack_div[0]);
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("divide_neg_exp", SMITH_BOX(&divide_neg_exp));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("is_contiguous", SMITH_BOX(&is_contiguous));
}
...
// This is used to test const smith::stable::Tensor& with SMITH_BOX
Tensor my_empty_like(const Tensor& t) {
  return empty_like(t);
}

// This is used to test smith::stable::Tensor& with SMITH_BOX
...
Tensor my_new_empty_dtype_variant(Tensor t) {
  ...
  auto dtype = std::make_optional(smith::headeronly::ScalarType::BFloat16);
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_transpose", SMITH_BOX(&my_transpose));
  m.impl("my_empty_like", SMITH_BOX(&my_empty_like));
  m.impl("fill_infinity", SMITH_BOX(&fill_infinity));
  m.impl("my_is_cpu", SMITH_BOX(&my_is_cpu));
  m.impl("my_new_empty_dtype_variant", SMITH_BOX(&my_new_empty_dtype_variant));
  m.impl("my_new_zeros_dtype_variant", SMITH_BOX(&my_new_zeros_dtype_variant));
  m.impl("my_copy_", SMITH_BOX(&my_copy_));
  m.impl("my_clone", SMITH_BOX(&my_clone));
}

STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeImplicitAutograd, m) {
  m.impl("my_pad", SMITH_BOX(&my_pad));
  m.impl("my_narrow", SMITH_BOX(&my_narrow));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_zero_", SMITH_BOX(&my_zero_));
  m.impl("my_amax", SMITH_BOX(&my_amax));
  m.impl("my_amax_vec", SMITH_BOX(&my_amax_vec));
  m.impl("test_default_constructor", SMITH_BOX(&test_default_constructor));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CPU, m) {
  m.impl("mv_tensor_accessor", SMITH_BOX(&mv_tensor_accessor_cpu));
}

// Test functions for smith::stable::accelerator APIs

#ifdef LAE_USE_CUDA
int64_t test_device_guard(int64_t device_index) {
  using smith::stable::accelerator::DeviceGuard;
  ...
  STD_SMITH_CHECK(err == cudaSuccess);
...
int64_t test_stream(int32_t device_index) {
  ...
  return smith::stable::accelerator::getCurrentStream(device_index).id();
}

int64_t test_get_current_device_index() {
  return smith::stable::accelerator::getCurrentDeviceIndex();
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("test_device_guard", SMITH_BOX(&test_device_guard));
  m.impl("test_device_guard_set_index", SMITH_BOX(&test_device_guard_set_index));
  m.impl("test_stream", SMITH_BOX(&test_stream));
  m.impl("test_get_current_device_index", SMITH_BOX(&test_get_current_device_index));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_flatten", SMITH_BOX(&my_flatten));
}

// Test function for const std::optional<Tensor>& with SMITH_BOX
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_optional_tensor_ref", SMITH_BOX(&my_optional_tensor_ref));
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_storage_offset", SMITH_BOX(&my_storage_offset));
  m.impl("my_element_size", SMITH_BOX(&my_element_size));
}

Tensor my_unsqueeze(const Tensor& t, int64_t dim) {
  return smith::stable::unsqueeze(t, dim);
}

Tensor my_squeeze(const Tensor& t, int64_t dim) {
  return smith::stable::squeeze(t, dim);
}

Tensor my_select(const Tensor& t, int64_t dim, int64_t index) {
  return smith::stable::select(t, dim, index);
}

Tensor my_matmul(const Tensor& self, const Tensor& other) {
  return smith::stable::matmul(self, other);
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_9, CompositeExplicitAutograd, m) {
  m.impl("my_unsqueeze", SMITH_BOX(&my_unsqueeze));
  m.impl("my_squeeze", SMITH_BOX(&my_squeeze));
  m.impl("my_select", SMITH_BOX(&my_select));
  m.impl("my_matmul", SMITH_BOX(&my_matmul));
}
