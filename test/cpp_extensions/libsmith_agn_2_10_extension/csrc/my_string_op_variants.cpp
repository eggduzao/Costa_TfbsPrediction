// This file is intended to test (const) std::string& and const std::string_view& arguments with SMITH_BOX
...
static int64_t process_accessor(Tensor t, std::string_view accessor) {
  if (accessor == "dim") {
    ...
    STD_SMITH_CHECK(false, "Unsupported accessor value: ", std::string(accessor).c_str())
  }
}
...
STABLE_SMITH_LIBRARY_IMPL(libsmith_agn_2_10, CompositeExplicitAutograd, m) {
  m.impl("my_string_op_const_string_ref", SMITH_BOX(&my_string_op_const_string_ref));
  m.impl("my_string_op_const_string_view_ref", SMITH_BOX(&my_string_op_const_string_view_ref));
  m.impl("my_string_op_string_ref", SMITH_BOX(&my_string_op_string_ref));
}
