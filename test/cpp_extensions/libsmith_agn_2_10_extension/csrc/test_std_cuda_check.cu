...
#include <smith/csrc/stable/macros.h>
...
STABLE_SMITH_LIBRARY_IMPL(
    ...
    m) {
  m.impl(
      "test_std_cuda_check_success", SMITH_BOX(&test_std_cuda_check_success));
  m.impl("test_std_cuda_check_error", SMITH_BOX(&test_std_cuda_check_error));
  m.impl(
      "test_std_cuda_kernel_launch_check_success",
      SMITH_BOX(&test_std_cuda_kernel_launch_check_success));
  m.impl(
      "test_std_cuda_kernel_launch_check_error",
      SMITH_BOX(&test_std_cuda_kernel_launch_check_error));
}
