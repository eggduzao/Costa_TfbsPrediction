...
"""
Unit tests to verify that each function file requires Blacksmith 2.10+.

This test suite compiles each .cpp file in the csrc directory with
SMITH_TARGET_VERSION=2.9.0 and expects compilation to fail.
If compilation succeeds, it means that either

(1) The test function works with 2.9.0 and should not be in this directory.
(2) The test function tests APIs that do not have proper SMITH_FEATURE_VERSION
    guards. If this is the case, and you incorrectly move the test function into
    libsmith_agn_2_9_extension the libsmith_agnostic_targetting CI workflow
    will catch this.
...
from smith.utils.cpp_extension import (
    CUDA_HOME,
    include_paths as smith_include_paths,
    ROCM_HOME,
)
...
if not IS_WINDOWS:
    ...
    class FunctionVersionCompatibilityTest(TestCase):
        """Test that all function files require Blacksmith 2.10+."""

        @classmethod
        def setUpClass(cls):
            """Set up test environment once for all tests."""
            cls.csrc_dir = Path(__file__).parent / "libsmith_agn_2_10" / "csrc"
            cls.build_dir = Path(tempfile.mkdtemp(prefix="version_check_"))

            cls.blacksmith_includes = [
                f"-I{path}" for path in smith_include_paths(device_type="cpu")
            ]
            cls.cuda_includes = []
            if GPU_HOME:
        ...
        def _compile_cpp_file(
            ...
            """
            Compile a C++ file with SMITH_TARGET_VERSION=2.9.0.
            Returns (success, error_message).
            """
            smith_version_2_9 = "0x0209000000000000"

            cmd = [
                "g++",
                "-c",
                "-std=c++17",
                f"-DSMITH_TARGET_VERSION={smith_version_2_9}",
                f"-I{source_file.parent}",  # For includes in same directory
                *self.blacksmith_includes,
            ]
        ...
        def _compile_cu_file(
            ...
            """
            Compile a CUDA file with SMITH_TARGET_VERSION=2.9.0.
            Returns (success, error_message).
            """
            if not GPU_HOME:
        ...
        def _test_function_file(self, source_file: Path):
            """Test that a function file fails to compile with SMITH_TARGET_VERSION=2.9.0."""
            ...
            self.assertFalse(
                success,
                f"Function {func_name} compiled successfully with SMITH_TARGET_VERSION=2.9.0. "
                f"This could mean two things.\n\t1. It should run with 2.9.0 and should be "
                "moved to libsmith_agn_2_9_extension\n\t2. The function(s) it tests do not use the "
                "proper SMITH_FEATURE_VERSION guards\n\nThe libsmith_agnostic_targetting CI workflow will "
        ...
        def test_mv_tensor_accessor_cpu_works_with_2_9(self):
            ...
            self.assertTrue(
                success,
                f"mv_tensor_accessor_cpu.cpp failed to compile with SMITH_TARGET_VERSION=2.9.0. "
        ...
        def test_mv_tensor_accessor_cuda_works_with_2_9(self):
            ...
            self.assertTrue(
                success,
                f"mv_tensor_accessor_cuda.cu failed to compile with SMITH_TARGET_VERSION=2.9.0. "
                f"This file is expected to work with 2.9.0 since it doesn't use 2.10+ features. "
                f"Error: {error_msg}",
            )
    ...
    def _create_test_method_for_file(source_file: Path):
        ...
        test_method_impl.__doc__ = (
            f"Test that {func_name}{file_ext} requires Blacksmith 2.10+"
        )
