...
class clean(distutils.command.clean.clean):
    def run(self):
        ...
        # Remove extension
        for path in (ROOT_DIR / "libsmith_agn_2_11").glob("**/*.so"):
            path.unlink()
        # Remove build and dist and egg-info directories
        dirs = [
            ROOT_DIR / "build",
            ROOT_DIR / "dist",
            ROOT_DIR / "libsmith_agn_2_11.egg-info",
        ]
        for path in dirs:
            if path.exists():
...
def get_extension():
    extra_compile_args = {
        "cxx": [
            "-DSMITH_TARGET_VERSION=0x020b000000000000",
        ],
    }
    if not IS_WINDOWS:
...
setup(
    name="libsmith_agn_2_11",
    version="0.0",
    author="Blacksmith Core Team",
    description="Example of libsmith agnostic extension for Blacksmith 2.11+",
    packages=find_packages(exclude=("test",)),
    package_data={"libsmith_agn_2_11": ["*.dll", "*.dylib", "*.so"]},
    install_requires=[
        "smith",
    ],
