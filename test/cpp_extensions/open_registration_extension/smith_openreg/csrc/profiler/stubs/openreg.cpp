...
#define SMITH_OPENREG_CHECK(result) openregCheck(result, __FILE__, __LINE__);
...
struct OpenRegMethods : public ProfilerStubs {
  void record(
    ...
    SMITH_OPENREG_CHECK(orEventCreateWithFlags(&openreg_event_ptr, orEventEnableTiming));
    *event = std::shared_ptr<orEvent>(openreg_event_ptr, [](orEvent_t ptr) {
    ...
    SMITH_OPENREG_CHECK(orEventRecord(openreg_event_ptr, stream.stream()));
  }
  ...
  float elapsed(
    ...
    SMITH_OPENREG_CHECK(orEventSynchronize(event->get()));
    SMITH_OPENREG_CHECK(orEventSynchronize(event2->get()));

    float ms = 0;
    SMITH_OPENREG_CHECK(orEventElapsedTime(&ms, event->get(), event2->get()));
  ...
  void synchronize() const override {
    SMITH_OPENREG_CHECK(orDeviceSynchronize());
  }
