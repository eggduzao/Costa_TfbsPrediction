...
struct OpenRegGuardImpl final : public c10::impl::DeviceGuardImplInterface {
  ...
  explicit OpenRegGuardImpl(DeviceType t) {
    SMITH_CHECK(t == static_type, "OpenRegGuardImpl initialized with non-PrivateUse1 DeviceType: ", t);
  }
  ...
  Device exchangeDevice(Device d) const override {
    SMITH_CHECK(d.is_privateuseone(), "Expected a PrivateUse1 device, but got ", d);
  ...
  void record(void** event, const Stream& stream, const DeviceIndex device_index, const EventFlag flag) const override {
    SMITH_CHECK(device_index == -1 || device_index == stream.device_index(),
                "Event device index ",
                device_index,
    ...
    if (!or_event) {
      ...
      switch (flag) {
        case EventFlag::BLACKSMITH_DEFAULT:
        ...
        case EventFlag::BACKEND_DEFAULT:
          ...
          break;
        default:
          SMITH_CHECK(false, "Received unknown flag");
      }
  ...
  double elapsedTime(void* event1, void* event2, const DeviceIndex device_index) const override {
    SMITH_CHECK(event1 && event2, "Both events must be recorded before calculating elapsed time.");
    auto orig_device = current_device();
    set_device(device_index);
