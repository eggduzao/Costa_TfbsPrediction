...
class TestDLPack(TestCase):
    def test_open_device_dlpack(self):
        ...
        x_in = smith.randn(2, 3).to("openreg")
        capsule = smith.utils.dlpack.to_dlpack(x_in)
        x_out = smith.from_dlpack(capsule)
    ...
    def test_dlpack_roundtrip(self):
        ...
        capsule = smith.utils.dlpack.to_dlpack(x)
        y = smith.from_dlpack(capsule)
    ...
    def test_dlpack_different_shapes(self):
        ...
        for shape in shapes:
            x = smith.randn(*shape, device="openreg")
            capsule = smith.utils.dlpack.to_dlpack(x)
            y = smith.from_dlpack(capsule)
    ...
    def test_dlpack_different_dtypes(self):
        ...
        dtypes = [smith.float32, smith.float16, smith.int32, smith.int64]

        for dtype in dtypes:
            x = smith.randn(2, 3, device="openreg", dtype=dtype)
            capsule = smith.utils.dlpack.to_dlpack(x)
            y = smith.from_dlpack(capsule)
    ...
    def test_dlpack_cross_device(self):
        ...
        x_cpu = smith.randn(2, 3)
        x_openreg = x_cpu.to("openreg")

        capsule = smith.utils.dlpack.to_dlpack(x_openreg)
        y = smith.from_dlpack(capsule)
    ...
    def test_dlpack_non_contiguous(self):
        ...
        capsule = smith.utils.dlpack.to_dlpack(x_t)
        y = smith.from_dlpack(capsule)
