...
import smith
import smith_openreg  # noqa: F401
...
class TestDeviceAllocator(TestCase):
    ...
    def test_memory_lifecycle(self):
        ...
        y = smith.empty(1000, device="openreg")
    ...
    def test_tensor_copy_operations(self):
        ...
        self.assertTrue(smith.allclose(cpu_tensor, back_to_cpu))

        # OpenReg to OpenReg (clone)
        cloned = openreg_tensor.clone()
        self.assertEqual(cloned.device.type, "openreg")
        self.assertTrue(smith.allclose(openreg_tensor.cpu(), cloned.cpu()))

    def test_tensor_shallow_copy(self):
        # set_data from openreg to cpu
        self_t = smith.randn(4, 4)
        from_t = smith.randn(4, 4, device="openreg")
        self.assertTrue(smith._has_compatible_shallow_copy_type(self_t, from_t))
        self_t.data = from_t
        self.assertEqual(self_t, from_t)
        self.assertEqual(self_t.data_ptr(), from_t.data_ptr())

        # set_data from cpu to openreg
        self_t = smith.randn(4, 4, device="openreg")
        from_t = smith.randn(4, 4)
        self.assertTrue(smith._has_compatible_shallow_copy_type(self_t, from_t))
        self_t.data = from_t
        self.assertEqual(self_t, from_t)
        self.assertEqual(self_t.data_ptr(), from_t.data_ptr())
    ...
    def test_different_dtypes(self):
        ...
        dtypes = [smith.float32, smith.float64, smith.int32, smith.int64]

        for dtype in dtypes:
            x = smith.empty(100, dtype=dtype, device="openreg")
            self.assertEqual(x.device.type, "openreg")
    ...
    def test_empty_cache_operation(self):
        ...
        x = smith.empty(1000, device="openreg")
        y = smith.empty(2000, device="openreg")
        ...
        smith.cuda.empty_cache() if smith.cuda.is_available() else None

    def test_memory_format_allocation(self):
        """Test allocation with different memory formats."""
        # Channels last format
        x = smith.empty(2, 3, 4, 4, device="openreg", memory_format=smith.channels_last)
        self.assertEqual(x.device.type, "openreg")
        self.assertTrue(x.is_contiguous(memory_format=smith.channels_last))

        # Contiguous format (default)
        y = smith.empty(
            2, 3, 4, 4, device="openreg", memory_format=smith.contiguous_format
        )
        self.assertEqual(y.device.type, "openreg")
    ...
    def test_large_allocation(self):
        ...
        x = smith.empty(size, device="openreg")
        self.assertEqual(x.device.type, "openreg")
    ...
    def test_sequential_allocations_and_deallocations(self):
        ...
        for i in range(10):
            x = smith.empty(1000 + i * 100, device="openreg")
            self.assertEqual(x.device.type, "openreg")
    ...
    def test_allocation_with_requires_grad(self):
        ...
        x = smith.empty(100, device="openreg", requires_grad=True)
        self.assertEqual(x.device.type, "openreg")
        self.assertTrue(x.requires_grad)

        y = smith.randn(100, device="openreg", requires_grad=True)
        self.assertEqual(y.device.type, "openreg")
    ...
    def test_tensor_from_blob(self):
        ...
        x.fill_(5.0)
        self.assertTrue(smith.all(y == 5.0))
...
class TestMemoryLeaks(TestCase):
    ...
    def test_no_leak_with_copies(self):
        ...
        for i in range(iterations):
            # Create tensor
            x = smith.randn(500, device="openreg")
    ...
    def test_no_leak_with_views(self):
        ...
        for i in range(iterations):
            x = smith.randn(1000, device="openreg")
    ...
    def test_no_leak_inplace_operations(self):
        ...
        for i in range(iterations):
            x = smith.ones(1000, device="openreg")
    ...
    def test_no_leak_with_gradients(self):
        ...
        for i in range(iterations):
            x = smith.randn(100, device="openreg", requires_grad=True)
            y = smith.randn(100, device="openreg", requires_grad=True)
    ...
    def test_leak_detection_with_statistics(self):
        ...
        for cycle in range(num_cycles):
            ...
            for i in range(allocations_per_cycle):
                t = smith.empty(1000, device="openreg")
                tensors.append(t)
        ...
        final_tensor = smith.empty(10000, device="openreg")
...
class TestPinMemory(TestCase):
    ...
    def test_pin_memory_storage_sharing(self):
        ...
        shared_tensor = smith.tensor((), dtype=smith.float32).set_(
...
class TestMultiDeviceAllocation(TestCase):
    ...
    def setUp(self):
        self.device_count = smith.openreg.device_count()
    ...
    def tearDown(self):
        ...
        smith.openreg.set_device(0)
        gc.collect()

    def test_allocation_on_device_1(self):
        smith.openreg.set_device(1)
        x = smith.empty(100, device="openreg:1")
        self.assertEqual(x.device.type, "openreg")
    ...
    def test_simultaneous_device_allocations(self):
        ...
        x = smith.empty(100, device="openreg:0")
        y = smith.empty(200, device="openreg:1")
    ...
    def test_memory_isolation_between_devices(self):
        ...
        tensors_dev0 = [smith.empty(1000, device="openreg:0") for _ in range(10)]
        tensors_dev1 = [smith.empty(1000, device="openreg:1") for _ in range(10)]
    ...
    def test_alternating_device_allocations(self):
        ...
        for i in range(20):
            device_idx = i % 2
            t = smith.empty(100 + i, device=f"openreg:{device_idx}")
...
class TestCrossDeviceOperations(TestCase):
    ...
    def test_tensor_to_different_device(self):
        ...
        # Create on device 0
        x = smith.randn(100, device="openreg:0")
        self.assertEqual(x.device.index, 0)
        ...
        self.assertTrue(smith.allclose(x.cpu(), y.cpu()))

    def test_bidirectional_device_transfer(self):
        """Test transferring tensor back and forth between devices."""
        original = smith.randn(100, device="openreg:0")
        original_cpu = original.cpu()

        # 0 -> 1
        on_dev1 = original.to("openreg:1")
        self.assertTrue(smith.allclose(original_cpu, on_dev1.cpu()))

        # 1 -> 0
        back_to_dev0 = on_dev1.to("openreg:0")
        self.assertTrue(smith.allclose(original_cpu, back_to_dev0.cpu()))
