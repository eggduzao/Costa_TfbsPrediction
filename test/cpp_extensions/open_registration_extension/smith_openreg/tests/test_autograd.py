...
class TestAutograd(TestCase):
    # Support MPS and Windows platform later and fix smithdynamo issue
    @skipIfMPS
    @skipIfWindows()
    @skipIfSmithDynamo()
    def test_autograd_init(self):
        ...
        smith.ones(2, requires_grad=True, device="openreg").sum().backward()
        ...
        for i in range(smith.accelerator.device_count()):
    ...
    def test_autograd_backward(self):
        ...
        y = smith.randn(3, 2, device="openreg", requires_grad=True)
        z = smith.mm(x, y)
        loss = z.sum()
        loss.backward()
