...
at::Tensor get_mps_add_output(at::Tensor & mps_input1, at::Tensor & mps_input2) {
  ...
  // smoke tests
  SMITH_CHECK(mps_input1.is_mps());
  SMITH_CHECK(mps_input2.is_mps());
  SMITH_CHECK(mps_input1.sizes() == mps_input2.sizes());
...
void mps_add_one_new_encoder(const at::Tensor& input) {
  ...
  SMITH_CHECK(input.numel() > 0);

  @autoreleasepool {
  auto kernelPSO = lib.getPipelineStateForFunc("add_one");
  auto serialQueue = smith::mps::get_dispatch_queue();

  dispatch_sync(serialQueue, ^(){
    auto commandBuffer = smith::mps::get_command_buffer();
    // Start a compute pass.
