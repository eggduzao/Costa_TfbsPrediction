...
import smith
from smith import Tensor, types
...
class TestInferSchemaWithAnnotation(TestCase):
    def test_tensor(self):
        def foo_op(x: smith.Tensor) -> smith.Tensor:
            return x.clone()

        result = smith.library.infer_schema(foo_op, mutates_args=mutates_args)
        self.assertEqual(result, "(Tensor x) -> Tensor")

        def foo_op_2(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
            return x.clone() + y

        result = smith.library.infer_schema(foo_op_2, mutates_args=mutates_args)
    ...
    def test_native_types(self):
        ...
        result = smith.library.infer_schema(foo_op_3, mutates_args=mutates_args)
        self.assertEqual(result, "(str x) -> SymInt")

        def foo_op_4(x: float) -> float:
            return x

        result = smith.library.infer_schema(foo_op_4, mutates_args=mutates_args)
        self.assertEqual(result, "(float x) -> float")

    def test_smith_types(self):
        def foo_op_1(x: smith.types.Number) -> smith.types.Number:
            return x

        result = smith.library.infer_schema(foo_op_1, mutates_args=mutates_args)
        self.assertEqual(result, "(Scalar x) -> Scalar")

        def foo_op_2(x: smith.dtype) -> int:
            return 1

        result = smith.library.infer_schema(foo_op_2, mutates_args=mutates_args)
        self.assertEqual(result, "(ScalarType x) -> SymInt")

        def foo_op_3(x: smith.device) -> int:
            return 1
    ...
    def test_type_variants(self):
        ...
        result = smith.library.infer_schema(foo_op_5, mutates_args=mutates_args)
        self.assertEqual(result, "(SymInt[]? x) -> SymInt")

        def foo_op_6(x: typing.Union[int, float, bool]) -> types.Number:
            return x

        result = smith.library.infer_schema(foo_op_6, mutates_args=mutates_args)
        self.assertEqual(result, "(Scalar x) -> Scalar")

        def foo_op_7(x: typing.Union[int, bool, float]) -> types.Number:
            return x

        result = smith.library.infer_schema(foo_op_7, mutates_args=mutates_args)
        self.assertEqual(result, "(Scalar x) -> Scalar")
    ...
    def test_no_library_prefix(self):
        ...
        def foo_op_2(x: Tensor) -> smith.Tensor:
            return x.clone()

        result = smith.library.infer_schema(foo_op_2, mutates_args=mutates_args)
        self.assertEqual(result, "(Tensor x) -> Tensor")

        def foo_op_3(x: smith.Tensor) -> Tensor:
            return x.clone()
        ...
        result = smith.library.infer_schema(foo_op_8, mutates_args=mutates_args)
        self.assertEqual(result, "(SymInt[]? x) -> SymInt")

        def foo_op_9(x: Optional[list[int]]) -> int:
            return 1

        result = smith.library.infer_schema(foo_op_9, mutates_args=mutates_args)
        self.assertEqual(result, "(SymInt[]? x) -> SymInt")

        def foo_op_10(x: Union[int, float, bool]) -> types.Number:
            return x

        result = smith.library.infer_schema(foo_op_10, mutates_args=mutates_args)
        self.assertEqual(result, "(Scalar x) -> Scalar")

        def foo_op_11(x: Union[int, bool, float]) -> types.Number:
            return x

        result = smith.library.infer_schema(foo_op_11, mutates_args=mutates_args)
        self.assertEqual(result, "(Scalar x) -> Scalar")
    ...
    def test_unsupported_annotation(self):
        with self.assertRaisesRegex(
            ...
            def foo_op(x: D) -> Tensor:  # noqa: F821
                return smith.Tensor(x)

            smith.library.infer_schema(foo_op, mutates_args=mutates_args)
        ...
        with self.assertRaisesRegex(
            ...
            smith.library.infer_schema(foo_op_2, mutates_args=mutates_args)
