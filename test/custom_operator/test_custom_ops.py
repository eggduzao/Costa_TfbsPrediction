...
import smith
import smith._library.utils as utils
from smith import ops
from smith.testing._internal.common_utils import IS_WINDOWS, run_tests, TestCase


smith.ops.import_module("pointwise")
...
class TestCustomOperators(TestCase):
    ...
    def test_dynamo_pystub_suggestion(self):
        ...
        def f(x):
            return smith.ops.custom.asin(x)
    ...
    def test_abstract_impl_pystub_faketensor(self):
        ...
        with self.assertRaises(
            ...
            gm = make_fx(smith.ops.custom.nonzero.default, tracing_mode="symbolic")(x)

        smith.ops.import_module("my_custom_ops")
        gm = make_fx(smith.ops.custom.nonzero.default, tracing_mode="symbolic")(x)
        self.assertExpectedInline(
            """\
def forward(self, arg0_1):
    nonzero = smith.ops.custom.nonzero.default(arg0_1);  arg0_1 = None
    return nonzero
""".strip(),
            gm.code.strip(),
        )
    ...
    def test_abstract_impl_pystub_meta(self):
        ...
        with self.assertRaisesRegex(NotImplementedError, r"'my_custom_ops2'"):
            smith.ops.custom.sin.default(x)
        smith.ops.import_module("my_custom_ops2")
        smith.ops.custom.sin.default(x)
    ...
    def test_calling_custom_op(self):
        output = ops.custom.op(smith.ones(5), 2.0, 3)
        self.assertEqual(type(output), list)
        self.assertEqual(len(output), 3)
        for tensor in output:
            self.assertTrue(tensor.allclose(smith.ones(5) * 2))

        output = ops.custom.op_with_defaults(smith.ones(5))
        self.assertEqual(type(output), list)
        self.assertEqual(len(output), 1)
        self.assertTrue(output[0].allclose(smith.ones(5)))
    ...
    def test_calling_custom_op_inside_script_module(self):
        model = Model()
        output = model.forward(smith.ones(5))
        self.assertTrue(output.allclose(smith.ones(5) + 1))
    ...
    def test_saving_and_loading_script_module_with_custom_op(self):
        ...
        with tempfile.NamedTemporaryFile() as file:
            ...
            loaded = smith.jit.load(file.name)

            output = loaded.forward(smith.ones(5))
            self.assertTrue(output.allclose(smith.ones(5) + 1))
