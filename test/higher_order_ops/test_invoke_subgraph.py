...
from funcsmith.compile import aot_function, nop
...
from smith._higher_order_ops.schema import find_hop_schema
...
nested_compile_region = smith.compiler.nested_compile_region
...
class TestInvokeSubgraph(TestCase):
    def test_simple(self):
        ...
        y = smith.randn(8, requires_grad=True)
        ref = gn(x, y)
...
class TestInvokeSubgraphCompile(TestCase):
    ...
    def test_simple(self):
        ...
        x_clone = x.detach().clone().requires_grad_(True)
        y_clone = y.detach().clone().requires_grad_(True)
        res = smith.compile(fn, backend="inductor", fullgraph=True)(x_clone, y_clone)
    ...
    def test_module_forward(self):
        class Mod(smith.nn.Module):
            ...
            def forward(self, x, y):
                return smith.mul(x, y).sin() + self.c
    ...
    def test_stack_trace(self):
        ...
        # model code, not files in smith._higher_order_ops directory.
        class Mod(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.c = 5
        ...
        smith.compile(fn, backend=backend, fullgraph=True)(x, y)

        graph = backend.graphs[0]
        invoke_subgraph_nodes = graph.graph.find_nodes(
            op="call_function", target=smith._higher_order_ops.invoke_subgraph
        )
    ...
    def test_gen_schema(self):
        ...
        y_clone = y.detach().clone().requires_grad_(True)
        backend = AotEagerAndRecordGraphs()
        res = smith.compile(fn, backend=backend, fullgraph=True)(x_clone, y_clone)
        res.sum().backward()

        self.assertEqual(len(backend.fw_graphs), 1)
        self.assertEqual(len(backend.bw_graphs), 1)
        fw_schema = find_hop_schema(
            backend.fw_graphs[0], smith.ops.higher_order.invoke_subgraph
        )
        bw_schema = find_hop_schema(
            backend.bw_graphs[0], smith.ops.higher_order.invoke_subgraph
        )
    ...
    def test_gen_schema_with_buffer_mutation(self):
        class Mod(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.c = 5
                self.register_buffer("buf", smith.ones(8, requires_grad=False))

            @nested_compile_region
            def forward(self, x, y):
                self.buf.add_(1)
                return smith.mul(x, y).sin() + self.c + self.buf
        ...
        with (
            ...
            res = smith.compile(fn, backend=backend, fullgraph=True)(
                mod, x_clone, y_clone
            )

        self.assertEqual(len(backend.graphs), 1)
        fw_schema = find_hop_schema(
            backend.graphs[0], smith.ops.higher_order.invoke_subgraph
        )
        if not TEST_WITH_CROSSREF:
            self.assertExpectedInline(
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8]", L_y_: "f32[8]", L_mod_buffers_buf_: "f32[8]"):
        ...
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_mod_buffers_buf_, l_x_, l_y_);  subgraph_0 = None
        getitem_8: "f32[8]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', l_mod_buffers_buf_, l_x_, l_y_);  subgraph_1 = l_mod_buffers_buf_ = l_x_ = l_y_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_mod_buffers_buf_: "f32[8]", l_x_: "f32[8]", l_y_: "f32[8]"):
            ...
            mul: "f32[8]" = smith.mul(l_x_, l_y_);  l_x_ = l_y_ = None
...
""",
    ...
    def test_auto_functionalize(self):
        ...
        y_clone = y.detach().clone().requires_grad_(True)
        backend = AotEagerAndRecordGraphs()
        res = smith.compile(fn, backend=backend, fullgraph=True)(mod, x_clone, y_clone)
        res.sum().backward()
        self.assertEqual(len(backend.fw_graphs), 1)
        self.assertEqual(len(backend.bw_graphs), 1)
        self.assertEqual(ref, res)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8]", primals_2: "f32[8]", primals_3: "f32[8]"):
        ...
        invoke_subgraph_4 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1, primals_2, primals_3);  partitioned_fw_subgraph_0_0 = None
        ...
        invoke_subgraph_6 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_1, 'partitioned_fw_subgraph_0_0', primals_1, primals_2, primals_3);  partitioned_fw_subgraph_0_1 = primals_1 = primals_2 = primals_3 = None
        ...
        add: "f32[8]" = smith.ops.aten.add.Tensor(getitem, getitem_1);  getitem = getitem_1 = None
        return (add, getitem_12, getitem_11, getitem_10, getitem_15, getitem_14, getitem_13)
    class partitioned_fw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8]", primals_1: "f32[8]", primals_2: "f32[8]"):
            mul: "f32[8]" = smith.ops.aten.mul.Tensor(primals_0, primals_1)
            sin: "f32[8]" = smith.ops.aten.sin.default(mul);  mul = None
            mul_1: "f32[8]" = smith.ops.aten.mul.Tensor(sin, 5);  sin = None
            mul_2: "f32[8]" = smith.ops.aten.mul.Tensor(mul_1, primals_2);  mul_1 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, getitem_12: "f32[8]", getitem_11: "f32[8]", getitem_10: "f32[8]", getitem_15: "f32[8]", getitem_14: "f32[8]", getitem_13: "f32[8]", tangents_1: "f32[8]"):
        ...
        invoke_subgraph_7 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_1, 'partitioned_bw_subgraph_0_0', getitem_13, getitem_14, getitem_15, tangents_1);  partitioned_bw_subgraph_0_1 = getitem_13 = getitem_14 = getitem_15 = None
        ...
        invoke_subgraph_5 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, 'partitioned_bw_subgraph_0_0', getitem_10, getitem_11, getitem_12, tangents_1);  partitioned_bw_subgraph_0_0 = getitem_10 = getitem_11 = getitem_12 = tangents_1 = None
        getitem_6: "f32[8]" = invoke_subgraph_5[0]
        getitem_7: "f32[8]" = invoke_subgraph_5[1];  invoke_subgraph_5 = None
        add_1: "f32[8]" = smith.ops.aten.add.Tensor(getitem_2, getitem_6);  getitem_2 = getitem_6 = None
        add_2: "f32[8]" = smith.ops.aten.add.Tensor(getitem_3, getitem_7);  getitem_3 = getitem_7 = None
        return (add_1, add_2, None)

    class partitioned_bw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8]", primals_1: "f32[8]", primals_2: "f32[8]", tangents_0: "f32[8]"):
            mul_3: "f32[8]" = smith.ops.aten.mul.Tensor(tangents_0, primals_2);  tangents_0 = primals_2 = None
            mul_4: "f32[8]" = smith.ops.aten.mul.Tensor(mul_3, 5);  mul_3 = None
            mul: "f32[8]" = smith.ops.aten.mul.Tensor(primals_0, primals_1)
            cos: "f32[8]" = smith.ops.aten.cos.default(mul);  mul = None
            mul_5: "f32[8]" = smith.ops.aten.mul.Tensor(mul_4, cos);  mul_4 = cos = None
            mul_6: "f32[8]" = smith.ops.aten.mul.Tensor(mul_5, primals_0);  primals_0 = None
            mul_7: "f32[8]" = smith.ops.aten.mul.Tensor(mul_5, primals_1);  mul_5 = primals_1 = None
...
""",
    ...
    def test_buffer_mutation_works_under_no_grad(self):
        class Mod(smith.nn.Module):
            ...
            def forward(self, x, y):
                self.buf.add_(1)
                return smith.mul(x, y).sin() * self.buf
        ...
        y_clone = y.detach().clone().requires_grad_(True)
        with smith.no_grad():
            res = smith.compile(fn, fullgraph=True)(mod, x_clone, y_clone)
        self.assertEqual(ref, res)
    ...
    def test_buffer_mutation_errors_under_training(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith.compile(fn, backend="inductor", fullgraph=True)(mod, x, y)

    def test_list(self):
        @nested_compile_region
        def gn(x, y):
            return [smith.mul(x, y), smith.add(x, y)]

        def fn(x, y):
            lst = gn(x, y)
            lst.append(smith.sin(x))
    ...
    def test_differing_strides_for_grad_outs(self):
        class CustomOp(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return smith.sin(x)
            ...
            def backward(ctx, grad_out):
                ...
                return smith.cos(smith.reshape(a, (3, 4, 5)))
        ...
        def fn(x):
            ...
            b = smith.permute(a, (0, 2, 1))
            return b

        x = smith.randn(3, 4, 5, requires_grad=True)
        ref = smith.permute(gn(x), (0, 2, 1))
    ...
    def test_sdpa(self):
        ...
        q = smith.randn(
            1, 1, 32, 32, device="cuda", dtype=smith.bfloat16, requires_grad=True
        )
        k = smith.randn(
            1, 1, 32, 32, device="cuda", dtype=smith.bfloat16, requires_grad=True
        )
    ...
    def test_symint_from_fwd_to_bwd(self):
        @nested_compile_region
        def gn(x, y):
            a = smith.sum(x, (1,), keepdim=True).view(y.shape[1], y.shape[0])
            return smith.matmul(a, y)
        ...
        x = smith.randn(64, 1, requires_grad=True)
        y = smith.randn(8, 8, requires_grad=True)
        ref = fn(x, y)
        res = opt_fn(x, y)
        self.assertEqual(ref, res)

        x = smith.randn(256, 1, requires_grad=True)
        ...
        x = smith.randn(16, 1, requires_grad=True)
    ...
    def test_dropout_checks_joint_graph(self):
        ...
        @nested_compile_region
        def gn(x):
            return smith.nn.functional.dropout(smith.sin(x), p=0.5)
        ...
        res = smith.compile(fn, backend="inductor", fullgraph=True)(x)  # noqa: F841

        smith.compiler.reset()
        backend = InductorAndRecordGraphs()
        res = smith.compile(fn, backend=backend, fullgraph=True)(x)
        res.sum().backward()
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8]"):
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_4 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1);  partitioned_fw_subgraph_0_0 = None
        ...
        invoke_subgraph_6 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_1_0, 'partitioned_fw_subgraph_1_0', primals_1);  partitioned_fw_subgraph_1_0 = primals_1 = None
    ...
    class partitioned_fw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8]"):
            sin: "f32[8]" = smith.ops.aten.sin.default(primals_0)

            inductor_seeds_default: "i64[1]" = smith.ops.prims.inductor_seeds.default(1, device(type='cpu'))

            inductor_lookup_seed_default: "i64[]" = smith.ops.prims.inductor_lookup_seed.default(inductor_seeds_default, 0);  inductor_seeds_default = None
            inductor_random_default: "f32[8]" = smith.ops.prims.inductor_random.default([8], inductor_lookup_seed_default, 'rand');  inductor_lookup_seed_default = None
            gt: "b8[8]" = smith.ops.aten.gt.Scalar(inductor_random_default, 0.5);  inductor_random_default = None
            mul: "f32[8]" = smith.ops.aten.mul.Tensor(gt, sin);  sin = None
            mul_1: "f32[8]" = smith.ops.aten.mul.Tensor(mul, 2.0);  mul = None
            return (mul_1, primals_0, gt)

    class partitioned_fw_subgraph_1_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8]"):
            sin: "f32[8]" = smith.ops.aten.sin.default(primals_0)
...
""",
    ...
    def test_dropout_checks_joint_graph_inference(self):
        ...
        smith.compile(fn, backend=backend, fullgraph=True)(x)
...
class <lambda>(smith.nn.Module):
    def forward(self, arg0_1: "f32[8]"):
        ...
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(repeated_subgraph0, 'subgraph_0', arg0_1);  repeated_subgraph0 = arg0_1 = None
    ...
    class repeated_subgraph0(smith.nn.Module):
        def forward(self, arg0_1: "f32[8]"):
            ...
            inductor_random_default: "f32[8]" = smith.ops.prims.inductor_random.default([8], inductor_lookup_seed_default, 'rand');  inductor_lookup_seed_default = None
            gt: "b8[8]" = smith.ops.aten.gt.Scalar(inductor_random_default, 0.5);  inductor_random_default = None
            sin: "f32[8]" = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
            mul: "f32[8]" = smith.ops.aten.mul.Tensor(gt, sin);  gt = sin = None
            mul_1: "f32[8]" = smith.ops.aten.mul.Tensor(mul, 2.0);  mul = None
            return (mul_1,)
""",
                ignore_empty_lines=True,
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8]", L_y_: "f32[8]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_x_, l_y_);  subgraph_0 = l_x_ = None
        a: "f32[8]" = invoke_subgraph[0];  invoke_subgraph = None

        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', a, l_y_);  subgraph_1 = a = l_y_ = None
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8]", primals_2: "f32[8]"):
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_4 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1, primals_2);  partitioned_fw_subgraph_0_0 = primals_1 = None
        getitem_9: "f32[8]" = invoke_subgraph_4[2]
        getitem_8: "f32[8]" = invoke_subgraph_4[1]
        getitem: "f32[8]" = invoke_subgraph_4[0];  invoke_subgraph_4 = None

        partitioned_fw_subgraph_0_1 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_6 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_1, 'partitioned_fw_subgraph_0_0', getitem, primals_2);  partitioned_fw_subgraph_0_1 = getitem = primals_2 = None
...
""",
    ...
    def test_no_raise_on_reparameterize_module(self):
        ...
        ref = fn(x)
        backend = AotEagerAndRecordGraphs()
        opt_fn = smith.compile(fn, backend=backend, fullgraph=True)
        with mock.patch(
            "smith._dynamo.variables.higher_order_ops.InvokeSubgraphHigherOrderVariable.allow_side_effects",
            False,
        ):
            res = opt_fn(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[10, 10]", L_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_: "f32[10, 10]", L_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_: "f32[10]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_x_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_);  subgraph_0 = None
        getitem: "f32[10, 10]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', l_x_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_);  subgraph_1 = l_x_ = l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_ = l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[10, 10]", l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_: "f32[10, 10]", l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_: "f32[10]"):
            linear: "f32[10, 10]" = smith._C._nn.linear(l_x_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_, l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_);  l_x_ = l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_weight_ = l_gn_closure_0_cell_contents_closure_0_cell_contents_parameters_bias_ = None
            return (linear,)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[10, 10]", primals_2: "f32[10, 10]", primals_3: "f32[10]"):
        ...
        add: "f32[10, 10]" = smith.ops.aten.add.Tensor(getitem, getitem_1);  getitem = getitem_1 = None
        return (add, getitem_10, getitem_11)
    class partitioned_fw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[10, 10]", primals_1: "f32[10, 10]", primals_2: "f32[10]"):
            t: "f32[10, 10]" = smith.ops.aten.t.default(primals_1);  primals_1 = None
            addmm: "f32[10, 10]" = smith.ops.aten.addmm.default(primals_2, primals_0, t);  primals_2 = t = None
    ...
    def test_raise_with_reparameterize_but_module_mutation(self):
        ...
        with mock.patch(
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                r"Higher Order Operator: smith\.ops\.higher_order\.invoke_subgraph",
            ):
                opt_fn(x)
    ...
    def test_dce(self):
        ...
        smith.compile(fn, backend=backend, fullgraph=True)(
            smith.randn(4, requires_grad=False)
        )
...
class <lambda>(smith.nn.Module):
    ...
    class repeated_subgraph0(smith.nn.Module):
        def forward(self, arg0_1: "f32[4]"):
            sin: "f32[4]" = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
            return (sin,)
""",
            )
    ...
    def test_dce_recursive(self):
        def fn1(x):
            a = smith.sin(x)
            _ = smith.cos(x)  # unused intermediate
            return a

        @nested_compile_region
        def fn1_checkpoint(x):
            return smith.utils.checkpoint.checkpoint(fn1, x, use_reentrant=False)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_x_);  subgraph_0 = l_x_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[8]"):
            ...
            tag_activation_checkpoint = smith.ops.higher_order.tag_activation_checkpoint(wrap_body_0, l_x_, use_reentrant = False);  wrap_body_0 = l_x_ = None
            getitem_2: "f32[8]" = tag_activation_checkpoint[0];  tag_activation_checkpoint = None
            return (getitem_2,)

        class wrap_body_0(smith.nn.Module):
            def forward(self, l_x_: "f32[8]"):
                a: "f32[8]" = smith.sin(l_x_)

                _: "f32[8]" = smith.cos(l_x_);  l_x_ = _ = None
                return (a,)
""",
                )
    ...
    def test_nonlocal_update(self):
        ...
        smith._dynamo.reset()
        backend = AotEagerAndRecordGraphs()
        smith.compile(fn, backend=backend, fullgraph=True)(x_clone, y_clone)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8]", L_y_: "f32[8]"):
        ...
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_1', getitem_4, l_y_);  subgraph_1 = getitem_4 = l_y_ = None
    ...
    class subgraph_1(smith.nn.Module):
        def forward(self, a: "f32[8]", l_y_: "f32[8]"):
            mul: "f32[8]" = smith.mul(a, l_y_);  a = l_y_ = None
...
class <lambda>(smith.nn.Module):
    ...
    class repeated_subgraph0(smith.nn.Module):
        def forward(self, arg0_1: "f32[8]"):
            ...
            view: "f32[1, 8]" = smith.ops.aten.reshape.default(sin, [1, 8]);  sin = None

            sin_1: "f32[1, 8]" = smith.ops.aten.sin.default(view);  view = None
            return (sin_1,)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8]", L_y_: "f32[8]"):
        ...
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', getitem_25, l_y_);  subgraph_1 = getitem_25 = None
        getitem_26: "f32[8]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
        subgraph_2 = self.subgraph_0
        invoke_subgraph_2 = smith.ops.higher_order.invoke_subgraph(subgraph_2, 'subgraph_0', getitem_26, l_y_);  subgraph_2 = getitem_26 = None
        getitem_27: "f32[8]" = invoke_subgraph_2[0];  invoke_subgraph_2 = None
        subgraph_3 = self.subgraph_0
        invoke_subgraph_3 = smith.ops.higher_order.invoke_subgraph(subgraph_3, 'subgraph_0', getitem_27, l_y_);  subgraph_3 = getitem_27 = None
        getitem_28: "f32[8]" = invoke_subgraph_3[0];  invoke_subgraph_3 = None
        subgraph_4 = self.subgraph_0
        invoke_subgraph_4 = smith.ops.higher_order.invoke_subgraph(subgraph_4, 'subgraph_0', getitem_28, l_y_);  subgraph_4 = getitem_28 = l_y_ = None
...
""",
    ...
    def test_input_mutation_mutiple_times_fake_tensor_cahche_hit(self):
        ...
        with (
            mock.patch(
                "smith._higher_order_ops.utils.registered_hop_fake_fns",
                {smith.ops.higher_order.invoke_subgraph: _mock_invoke_subgraph},
            ),
            smith.no_grad(),
        ):
            out = opt_fn(x, y)
    ...
    def test_simple_module(self):
        mod = smith.nn.Linear(8, 8)

        @nested_compile_region
        def gn(x):
            return smith.cos(x), mod(x)
        ...
        # outputs
        x = smith.randn(8, 8, requires_grad=False)
        x_clone = x.detach().clone().requires_grad_(False)
    ...
    def test_fail_with_direct_invoke_subgraph(self):
        from smith._higher_order_ops import invoke_subgraph
        ...
        with self.assertRaisesRegex(
            smith._dynamo.exc.Unsupported, "Directly using invoke_subgraph is not"
        ):
            opt_fn(x)
    ...
    def test_mod_attr_aliasing(self):
        class MutateParam(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.a = smith.ones(8)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8, 8]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_x_);  subgraph_0 = None
        getitem: "f32[8, 8]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', l_x_);  subgraph_1 = l_x_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[8, 8]"):
            sin: "f32[8, 8]" = smith.sin(l_x_);  l_x_ = None
            return (sin,)
""",
            )
    ...
    def test_module_method(self):
        class Mod(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(8, 8)
        ...
        mod = Mod()
        backend = AotEagerAndRecordGraphs()
        opt_mod = smith.compile(mod, backend=backend, fullgraph=True)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8, 8]", L_self_modules_linear_parameters_weight_: "f32[8, 8]", L_self_modules_linear_parameters_bias_: "f32[8]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', l_x_, l_self_modules_linear_parameters_weight_, l_self_modules_linear_parameters_bias_);  subgraph_0 = None
        getitem: "f32[8, 8]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', l_x_, l_self_modules_linear_parameters_weight_, l_self_modules_linear_parameters_bias_);  subgraph_1 = l_self_modules_linear_parameters_weight_ = l_self_modules_linear_parameters_bias_ = None
        getitem_1: "f32[8, 8]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[8, 8]", l_self_modules_linear_parameters_weight_: "f32[8, 8]", l_self_modules_linear_parameters_bias_: "f32[8]"):
            linear: "f32[8, 8]" = smith._C._nn.linear(l_x_, l_self_modules_linear_parameters_weight_, l_self_modules_linear_parameters_bias_);  l_x_ = l_self_modules_linear_parameters_weight_ = l_self_modules_linear_parameters_bias_ = None
            return (linear,)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8, 8]"):
        ...
        getitem: "f32[8, 8]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', l_x_);  subgraph_1 = None
        getitem_1: "f32[8, 8]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
        mul: "f32[8, 8]" = getitem * getitem_1;  getitem = getitem_1 = None
        add: "f32[8, 8]" = l_x_ + mul;  mul = None
        add_1: "f32[8, 8]" = add + l_x_;  add = l_x_ = None
...
""",
    ...
    def test_return_none(self):
        ...
        weight = smith.ones(
            1000, device="cuda:0", dtype=smith.float32, requires_grad=True
        )
        ones = smith.ones(1000, device="cuda:0", dtype=smith.float32)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8, 8]"):
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_2 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1);  partitioned_fw_subgraph_0_0 = primals_1 = None
        getitem: "f32[8, 8]" = invoke_subgraph_2[0]
        getitem_1: "f32[8, 8]" = invoke_subgraph_2[1];  invoke_subgraph_2 = None

        add: "f32[8, 8]" = smith.ops.aten.add.Tensor(getitem, getitem_1);  getitem = getitem_1 = None
        return (add,)

    class partitioned_fw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8, 8]"):
            mul: "f32[8, 8]" = smith.ops.aten.mul.Tensor(primals_0, 2)
            mul_1: "f32[8, 8]" = smith.ops.aten.mul.Tensor(primals_0, 3);  primals_0 = None
            return (mul, mul_1)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, tangents_1: "f32[8, 8]"):
        partitioned_bw_subgraph_0_0 = self.partitioned_bw_subgraph_0_0
        invoke_subgraph_3 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, 'partitioned_bw_subgraph_0_0', tangents_1, tangents_1);  partitioned_bw_subgraph_0_0 = tangents_1 = None
        getitem_2: "f32[8, 8]" = invoke_subgraph_3[0];  invoke_subgraph_3 = None
        return (getitem_2,)

    class partitioned_bw_subgraph_0_0(smith.nn.Module):
        def forward(self, tangents_0: "f32[8, 8]", tangents_1: "f32[8, 8]"):
            mul_2: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, 3)
            mul_3: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, 2);  tangents_1 = None
            add: "f32[8, 8]" = smith.ops.aten.add.Tensor(mul_2, mul_3);  mul_2 = mul_3 = None
            return (add,)
""",
            )
    ...
    def test_grad_accumulation(self):
        mod1 = smith.nn.Linear(8, 8)
        mod2 = smith.nn.Linear(8, 8)
        mod3 = smith.nn.Linear(8, 8)
        ...
        c = smith.randn((8, 8), requires_grad=True)
    ...
    def test_complex(self):
        ...
        x = smith.randn(2, 2, dtype=smith.complex64)
        ref = fn(x)
        opt_fn = smith.compile(fn, backend="inductor", fullgraph=True)
        res = opt_fn(x)
        self.assertEqual(ref, res)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8, 8]", primals_2: "f32[8, 8]"):
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_2 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1, primals_2);  partitioned_fw_subgraph_0_0 = primals_1 = primals_2 = None
        ...
        sin: "f32[8, 8]" = smith.ops.aten.sin.default(getitem)
        cos: "f32[8, 8]" = smith.ops.aten.cos.default(getitem);  getitem = None
        return (sin, getitem_6, getitem_5, getitem_4, cos)

    class partitioned_fw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8, 8]", primals_1: "f32[8, 8]"):
            mm: "f32[8, 8]" = smith.ops.aten.mm.default(primals_0, primals_1)

            sin: "f32[8, 8]" = smith.ops.aten.sin.default(mm)

            t: "f32[8, 8]" = smith.ops.aten.t.default(primals_0);  primals_0 = None
            t_1: "f32[8, 8]" = smith.ops.aten.t.default(primals_1);  primals_1 = None
            return (sin, mm, t, t_1)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, getitem_6: "f32[8, 8]", getitem_5: "f32[8, 8]", getitem_4: "f32[8, 8]", cos: "f32[8, 8]", tangents_1: "f32[8, 8]"):
        mul: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, cos);  tangents_1 = cos = None
        partitioned_bw_subgraph_0_0 = self.partitioned_bw_subgraph_0_0
        invoke_subgraph_3 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, 'partitioned_bw_subgraph_0_0', getitem_4, getitem_5, getitem_6, mul);  partitioned_bw_subgraph_0_0 = getitem_4 = getitem_5 = getitem_6 = mul = None
    ...
    class partitioned_bw_subgraph_0_0(smith.nn.Module):
        def forward(self, mm: "f32[8, 8]", t: "f32[8, 8]", t_1: "f32[8, 8]", tangents_0: "f32[8, 8]"):
            cos: "f32[8, 8]" = smith.ops.aten.cos.default(mm);  mm = None
            mul: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_0, cos);  tangents_0 = cos = None

            mm_1: "f32[8, 8]" = smith.ops.aten.mm.default(t, mul);  t = None
            mm_2: "f32[8, 8]" = smith.ops.aten.mm.default(mul, t_1);  mul = t_1 = None
            return (mm_2, mm_1)
""",
            )
    ...
    def test_const_tensor(self):
        @nested_compile_region
        def gn(x):
            return smith.tensor(64, dtype=smith.float32) * x

        def fn(x):
            return gn(x) + gn(x)

        x = smith.randn(64, requires_grad=True)
    ...
    def test_ac(self):
        ...
        def fn2_checkpoint(x):
            return smith.utils.checkpoint.checkpoint(fn2, x, use_reentrant=False)
        ...
        x_clone = x.clone().detach().requires_grad_(True)
        backend = AotEagerAndRecordGraphs()
        res = smith.compile(fn, backend=backend, fullgraph=True)(x_clone)
        ...
        res = smith.compile(fn, backend="inductor", fullgraph=True)(x_clone)
        self.assertEqual(ref, res)

    @smith._inductor.config.patch(fallback_random=True)
    def test_ac_rng(self):
        def fn1(x):
            return smith.cos(smith.nn.functional.dropout(x, p=0.5))
    ...
    def test_ac_rng_cudagraphs(self):
        ...
        def fn1_checkpoint(q, k, v):
            return smith.utils.checkpoint.checkpoint(fn1, q, k, v, use_reentrant=False)

        def fn(q, k, v):
            return fn1_checkpoint(q, k, v) + fn1_checkpoint(q.cos(), k, v)

        q = smith.randn(
            1, 1, 32, 32, device=GPU_TYPE, dtype=smith.bfloat16, requires_grad=True
        )
        k = smith.randn(
            1, 1, 32, 32, device=GPU_TYPE, dtype=smith.bfloat16, requires_grad=True
        )
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8, 8]", L_y_: "f32[16, 16]"):
        ...
        getitem: "f32[8, 8]" = invoke_subgraph[0];  invoke_subgraph = None
        subgraph_1 = self.subgraph_1
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_1', l_y_);  subgraph_1 = l_y_ = None
    ...
    class subgraph_1(smith.nn.Module):
        def forward(self, l_y_: "f32[16, 16]"):
            sin: "f32[16, 16]" = smith.sin(l_y_);  l_y_ = None
            return (sin,)
""",
            )
    ...
    def test_different_symint(self):
        ...
        def fn(x):
            ...
            b = smith.narrow(a, 0, 0, a.size()[0] // 2)
            return gn(b)
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", L_x_: "f32[s77, 8]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', s77, l_x_);  subgraph_0 = l_x_ = None
        a: "f32[s77, 8]" = invoke_subgraph[0];  invoke_subgraph = None

        floordiv: "Sym((s77//2))" = s77 // 2
        b: "f32[(s77//2), 8]" = smith.narrow(a, 0, 0, floordiv);  a = floordiv = None

        subgraph_1 = self.subgraph_1
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_1', s77, b);  subgraph_1 = s77 = b = None
        getitem_3: "f32[(s77//2), 8]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
        return (getitem_3,)

    class subgraph_0(smith.nn.Module):
        def forward(self, s77: "Sym(s77)", l_x_: "f32[s77, 8]"):
            sin: "f32[s77, 8]" = smith.sin(l_x_);  l_x_ = None
            return (sin,)

    class subgraph_1(smith.nn.Module):
        def forward(self, s77: "Sym(s77)", b: "f32[(s77//2), 8]"):
            sin: "f32[(s77//2), 8]" = smith.sin(b);  b = None
            return (sin,)
""",
            )
...
class GraphModule(smith.nn.Module):
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[8, 8]"):
            ...
            autograd_function_apply = smith.ops.higher_order.autograd_function_apply(fwd_body_0, bwd_body_0, l_x_, non_differentiable_idx = [], saved_for_backward_idx = [0]);  fwd_body_0 = bwd_body_0 = l_x_ = None
        ...
        class bwd_body_0(smith.nn.Module):
            def forward(self, grad_out: "f32[8, 8]", l_x_: "f32[8, 8]"):
                ...
                cos: "f32[8, 8]" = smith.cos(grad_out);  grad_out = None
...
""",
    ...
    def test_triton_kernel_native(self):
        ...
        t1 = smith.rand(5, device=GPU_TYPE)
        t2 = smith.rand(5, device=GPU_TYPE)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[5]", L_y_: "f32[5]"):
        ...
        subgraph_0 = self.subgraph_0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(subgraph_0, 'subgraph_0', x, y);  subgraph_0 = x = None
        getitem_4: "f32[5]" = invoke_subgraph[0];  invoke_subgraph = None

        subgraph_1 = self.subgraph_1
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_1', getitem_4, y);  subgraph_1 = getitem_4 = y = None
        getitem_5: "f32[5]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
        return (getitem_5,)

    class subgraph_0(smith.nn.Module):
        def forward(self, x: "f32[5]", y: "f32[5]"):
            o: "f32[5]" = smith.zeros_like(x)

            triton_kernel_wrapper_mutation = smith.ops.higher_order.triton_kernel_wrapper_mutation(kernel_idx = 0, constant_args_idx = 0, grid = [(5, 1, 1)], tma_descriptor_metadata = {}, kwargs = {'in_ptr0': x, 'in_ptr1': y, 'out_ptr': o});  x = y = triton_kernel_wrapper_mutation = None
    ...
    class subgraph_1(smith.nn.Module):
        def forward(self, z: "f32[5]", y: "f32[5]"):
            o: "f32[5]" = smith.zeros_like(z)

            triton_kernel_wrapper_mutation = smith.ops.higher_order.triton_kernel_wrapper_mutation(kernel_idx = 0, constant_args_idx = 1, grid = [(5, 1, 1)], tma_descriptor_metadata = {}, kwargs = {'in_ptr0': z, 'in_ptr1': y, 'out_ptr': o});  z = y = triton_kernel_wrapper_mutation = None
...
""",
    ...
    def test_unbacked_symbol(self):
        @nested_compile_region
        def gn(x):
            return smith.sin(smith.nonzero(x))
    ...
    def test_different_strides_in_backward(self):
        ...
        x = smith.randn(8, 16, requires_grad=True)
        smith._dynamo.mark_dynamic(x, 0)
        x_clone = x.detach().clone().requires_grad_(True)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "Sym(s77)", primals_2: "f32[s77, 16]"):
        ...
        invoke_subgraph_8 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_1, 'partitioned_fw_subgraph_0_1', primals_1, primals_2);  partitioned_fw_subgraph_0_1 = primals_2 = None
        ...
        invoke_subgraph_10 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_2, 'partitioned_fw_subgraph_0_1', primals_1, getitem);  partitioned_fw_subgraph_0_2 = getitem = None
        ...
        sin: "f32[s77, 16]" = smith.ops.aten.sin.default(getitem_1)

        partitioned_fw_subgraph_0_3 = self.partitioned_fw_subgraph_0_1
        invoke_subgraph_12 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_3, 'partitioned_fw_subgraph_0_1', primals_1, sin);  partitioned_fw_subgraph_0_3 = sin = None
        ...
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_14 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', primals_1, getitem_2);  partitioned_fw_subgraph_0_0 = None
        ...
        sum_1: "f32[]" = smith.ops.aten.sum.default(getitem_2);  getitem_2 = None
        sum_2: "f32[]" = smith.ops.aten.sum.default(getitem_3);  getitem_3 = None
        add_15: "f32[]" = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None

        cos: "f32[s77, 16]" = smith.ops.aten.cos.default(getitem_1);  getitem_1 = None
        return (add_15, getitem_16, getitem_18, getitem_20, getitem_22, cos, primals_1, getitem_17, getitem_19, getitem_21, getitem_23)

    class partitioned_fw_subgraph_0_1(smith.nn.Module):
        def forward(self, primals_0: "Sym(s77)", primals_1: "f32[s77, 16]"):
            cos: "f32[s77, 16]" = smith.ops.aten.cos.default(primals_1)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "Sym(s77)", getitem_17: "Sym(s77)", getitem_19: "Sym(s77)", getitem_21: "Sym(s77)", getitem_23: "Sym(s77)", getitem_16: "f32[s77, 16]", getitem_18: "f32[s77, 16]", getitem_20: "f32[s77, 16]", getitem_22: "f32[s77, 16]", cos: "f32[s77, 16]", tangents_1: "f32[]"):
        expand: "f32[s77, 16]" = smith.ops.aten.expand.default(tangents_1, [primals_1, 16]);  tangents_1 = primals_1 = None

        partitioned_bw_subgraph_0_0 = self.partitioned_bw_subgraph_0_0
        invoke_subgraph_15 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, 'partitioned_bw_subgraph_0_0', getitem_23, getitem_22, expand);  partitioned_bw_subgraph_0_0 = getitem_23 = getitem_22 = None
        getitem_5: "f32[s77, 16]" = invoke_subgraph_15[1];  invoke_subgraph_15 = None
        add_16: "f32[s77, 16]" = smith.ops.aten.add.Tensor(expand, getitem_5);  expand = getitem_5 = None

        partitioned_bw_subgraph_0_3 = self.partitioned_bw_subgraph_0_1
        invoke_subgraph_13 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_3, 'partitioned_bw_subgraph_0_1', getitem_21, getitem_20, add_16);  partitioned_bw_subgraph_0_3 = getitem_21 = getitem_20 = add_16 = None
        getitem_8: "f32[s77, 16]" = invoke_subgraph_13[1];  invoke_subgraph_13 = None

        mul_10: "f32[s77, 16]" = smith.ops.aten.mul.Tensor(getitem_8, cos);  getitem_8 = cos = None

        partitioned_bw_subgraph_0_2 = self.partitioned_bw_subgraph_0_1
        invoke_subgraph_11 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_2, 'partitioned_bw_subgraph_0_1', getitem_19, getitem_18, mul_10);  partitioned_bw_subgraph_0_2 = getitem_19 = getitem_18 = mul_10 = None
        ...
        invoke_subgraph_9 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_1, 'partitioned_bw_subgraph_0_1', getitem_17, getitem_16, getitem_11);  partitioned_bw_subgraph_0_1 = getitem_17 = getitem_16 = getitem_11 = None
    ...
    class partitioned_bw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "Sym(s77)", primals_1: "f32[s77, 16]", tangents_0: "f32[s77, 16]"):
            sin: "f32[s77, 16]" = smith.ops.aten.sin.default(primals_1);  primals_1 = None
            neg: "f32[s77, 16]" = smith.ops.aten.neg.default(sin);  sin = None
            mul_9: "f32[s77, 16]" = smith.ops.aten.mul.Tensor(tangents_0, neg);  tangents_0 = neg = None
            return (None, mul_9)

    class partitioned_bw_subgraph_0_1(smith.nn.Module):
        def forward(self, primals_0: "Sym(s77)", primals_1: "f32[s77, 16]", tangents_0: "f32[s77, 16]"):
            sin: "f32[s77, 16]" = smith.ops.aten.sin.default(primals_1);  primals_1 = None
            neg: "f32[s77, 16]" = smith.ops.aten.neg.default(sin);  sin = None
            mul_10: "f32[s77, 16]" = smith.ops.aten.mul.Tensor(tangents_0, neg);  tangents_0 = neg = None
            return (None, mul_10)
""",
                ignore_empty_lines=True,
            )
    ...
    def test_div(self):
        @nested_compile_region
        def gn(x):
            div = smith.div(1024, 256, rounding_mode="trunc")
            return div * smith.ones(64, div) * x
    ...
    def test_preserves_strides(self):
        ...
        x = smith.randn(4, 4, 2, 2, device=GPU_TYPE)
        other = smith.randn(4, 4, 2, 2, device=GPU_TYPE)
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            def meta(x, y):
                return smith.empty_like(y, memory_format=smith.contiguous_format)
            ...
            @nested_compile_region
            def gn(y, z):
                return smith.ops.mylib.add_op.default(y, z)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[8, 8]", L_y_: "f32[8, 8]"):
        ...
        subgraph_1 = self.subgraph_0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_0', getitem, l_y_);  subgraph_1 = getitem = l_y_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[8, 8]", l_y_: "f32[8, 8]"):
            a: "f32[8, 8]" = smith.sin(l_x_);  l_x_ = None

            b: "f32[8, 8]" = smith.cos(l_y_);  l_y_ = None
            return (a, b)
""",
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[8, 8]"):
        partitioned_fw_subgraph_0_1 = self.partitioned_fw_subgraph_0_1
        invoke_subgraph_4 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_1, 'partitioned_fw_subgraph_0_1', primals_1);  partitioned_fw_subgraph_0_1 = primals_1 = None
        ...
        partitioned_fw_subgraph_0_0 = self.partitioned_fw_subgraph_0_0
        invoke_subgraph_6 = smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, 'partitioned_fw_subgraph_0_0', getitem);  partitioned_fw_subgraph_0_0 = getitem = None
        ...
        add: "f32[8, 8]" = smith.ops.aten.add.Tensor(getitem_1, getitem_2);  getitem_1 = getitem_2 = None
        add_1: "f32[8, 8]" = smith.ops.aten.add.Tensor(add, getitem_3);  add = getitem_3 = None
        return (add_1, getitem_10, getitem_11)
    class partitioned_fw_subgraph_0_1(smith.nn.Module):
        def forward(self, primals_0: "f32[8, 8]"):
            sin: "f32[8, 8]" = smith.ops.aten.sin.default(primals_0)
            cos: "f32[8, 8]" = smith.ops.aten.cos.default(primals_0)
...
class GraphModule(smith.nn.Module):
    def forward(self, getitem_10: "f32[8, 8]", getitem_11: "f32[8, 8]", tangents_1: "f32[8, 8]"):
        partitioned_bw_subgraph_0_0 = self.partitioned_bw_subgraph_0_0
        invoke_subgraph_7 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, 'partitioned_bw_subgraph_0_0', getitem_11, tangents_1, tangents_1);  partitioned_bw_subgraph_0_0 = getitem_11 = None
        getitem_4: "f32[8, 8]" = invoke_subgraph_7[0];  invoke_subgraph_7 = None
        partitioned_bw_subgraph_0_1 = self.partitioned_bw_subgraph_0_1
        invoke_subgraph_5 = smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_1, 'partitioned_bw_subgraph_0_1', getitem_10, getitem_4, tangents_1);  partitioned_bw_subgraph_0_1 = getitem_10 = getitem_4 = tangents_1 = None
    ...
    class partitioned_bw_subgraph_0_0(smith.nn.Module):
        def forward(self, primals_0: "f32[8, 8]", tangents_0: "f32[8, 8]", tangents_1: "f32[8, 8]"):
            sin_1: "f32[8, 8]" = smith.ops.aten.sin.default(primals_0)
            neg: "f32[8, 8]" = smith.ops.aten.neg.default(sin_1);  sin_1 = None
            mul: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, neg);  neg = None
            cos_1: "f32[8, 8]" = smith.ops.aten.cos.default(primals_0);  primals_0 = None
            mul_1: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, cos_1);  tangents_1 = cos_1 = None
            add: "f32[8, 8]" = smith.ops.aten.add.Tensor(mul, mul_1);  mul = mul_1 = None
            return (add,)
    class partitioned_bw_subgraph_0_1(smith.nn.Module):
        def forward(self, primals_0: "f32[8, 8]", tangents_0: "f32[8, 8]", tangents_1: "f32[8, 8]"):
            sin_1: "f32[8, 8]" = smith.ops.aten.sin.default(primals_0)
            neg: "f32[8, 8]" = smith.ops.aten.neg.default(sin_1);  sin_1 = None
            mul: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_1, neg);  tangents_1 = neg = None
            cos_1: "f32[8, 8]" = smith.ops.aten.cos.default(primals_0);  primals_0 = None
            mul_1: "f32[8, 8]" = smith.ops.aten.mul.Tensor(tangents_0, cos_1);  tangents_0 = cos_1 = None
            add: "f32[8, 8]" = smith.ops.aten.add.Tensor(mul, mul_1);  mul = mul_1 = None
            return (add,)
""",
        )
...
class GraphModule(smith.nn.Module):
    def forward(self, x):
        ...
        subgraph_1 = self.subgraph_1
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(subgraph_1, 'subgraph_1', l_x_);  subgraph_1 = l_x_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, l_x_: "f32[64, 1]"):
            max_1 = smith.max(l_x_, 0);  l_x_ = None
...
class TestInvokeSubgraphExport(TestCase):
    def test_simple_func(self):
        ...
        ep = smith.export.export(M(), (x, y), strict=self.strict)
        self.assertTrue(smith.allclose(ep.module()(x, y), M()(x, y)))
...
class GraphModule(smith.nn.Module):
    def forward(self, x: "f32[8]", y: "f32[8]"):
        repeated_subgraph0 = self.repeated_subgraph0
        invoke_subgraph = smith.ops.higher_order.invoke_subgraph(repeated_subgraph0, 'subgraph_0', x, y);  repeated_subgraph0 = x = None
        getitem: "f32[8]" = invoke_subgraph[0];  invoke_subgraph = None
        repeated_subgraph0_1 = self.repeated_subgraph0
        invoke_subgraph_1 = smith.ops.higher_order.invoke_subgraph(repeated_subgraph0_1, 'subgraph_0', getitem, y);  repeated_subgraph0_1 = getitem = y = None
        getitem_1: "f32[8]" = invoke_subgraph_1[0];  invoke_subgraph_1 = None
        return (getitem_1,)
    class repeated_subgraph0(smith.nn.Module):
        def forward(self, arg0_1: "f32[8]", arg1_1: "f32[8]"):
            mul: "f32[8]" = smith.ops.aten.mul.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = None
            return (mul,)
""",
        )
    ...
    def test_pending_unbacked(self):
        ...
        ep = smith.export.export(
            ...
            (smith.randn(8),),
            strict=self.strict,
            dynamic_shapes={"x": {0: smith.export.Dim.DYNAMIC}},
        )
        ep = ep.run_decompositions()
        ...
        ep = smith.export.export(
            M(),
            (smith.randn(8, requires_grad=True),),
            strict=self.strict,
            dynamic_shapes={"x": {0: smith.export.Dim.DYNAMIC}},
        )
        ep = ep.run_decompositions()
    ...
    def test_multiple_module(self):
        ...
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.mod_list = smith.nn.ModuleList(N() for _ in range(10))
