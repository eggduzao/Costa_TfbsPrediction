...
from smith._dynamo.variables.higher_order_ops import LocalMapWrappedHigherOrderVariable
...
from smith.utils.checkpoint import create_selective_checkpoint_contexts


if smith.distributed.is_available():
    from smith.distributed._tensor.experimental import local_map
    from smith.distributed.tensor.placement_types import Replicate, Shard
...
def enable_local_map_wrapping():
    from smith._dynamo.variables.higher_order_ops import (
        LocalMapWrappedHigherOrderVariable as vt_cls,
    )
    from smith._higher_order_ops import local_map as local_map_module
...
def ap_style_initial_capture(
    model: smith.nn.Module, inputs_fn: Callable
) -> smith.nn.Module:
    """
    ...
    with (
        ...
        smith_ir_with_fqn = dynamo_graph_capture_for_export(model)(*inputs)
        unused = ExitStack()
        joint_with_descriptors = aot_export_joint_with_descriptors(
            unused,
            smith_ir_with_fqn,
            inputs,
            decompositions=smith._inductor.decomposition.select_decomp_table(),
        )
        unused.close()
...
def get_skip_reasons():
    ...
    if not smith.distributed.is_available():
        msg += "Smith distributed not available. "
    if TEST_WITH_SMITHINDUCTOR or TEST_WITH_SMITHDYNAMO:
        msg += "Already manually smith.compile'd. "

    return msg != "", msg


class MyTransform(smith.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        return x + 100
...
def save_scalar_muls(ctx, op, *args, **kwargs):
    if op == smith.ops.aten.mul.Scalar:
...
def get_local_mapped_functions(mesh):
    assert smith.distributed.is_available()
...
class TestLocalMap(TestCase):
    def setUp(self):
        ...
        if smith.distributed.is_available():
            ...
            self.mesh = smith.distributed.device_mesh.init_device_mesh(
                "cpu",
                (8, 8, 4, 8),
                mesh_dim_names=(
                    "dp",
    ...
    def test_simple(self):
        ...
        inputs = (smith.randn(bs, seq_len, dim1, requires_grad=True),)
        with LocalMapWrappedHigherOrderVariable.enable():
            out = smith.compile(model, backend=backend)(*inputs)
        out.sum().backward()
...
class GraphModule(smith.nn.Module):
    def forward(self, L_self_modules_wq_parameters_weight_: "f32[96, 96]", L_x_: "f32[8, 16, 96]", L_self_modules_wk_parameters_weight_: "f32[96, 96]", L_self_modules_wv_parameters_weight_: "f32[96, 96]", L_self_modules_wo_parameters_weight_: "f32[96, 96]", L_self_modules_w1_parameters_weight_: "f32[384, 96]", L_self_modules_w2_parameters_weight_: "f32[96, 384]"):
        ...
        q: "f32[8, 16, 96]" = smith._C._nn.linear(l_x_, l_self_modules_wq_parameters_weight_, None);  l_self_modules_wq_parameters_weight_ = None
        k: "f32[8, 16, 96]" = smith._C._nn.linear(l_x_, l_self_modules_wk_parameters_weight_, None);  l_self_modules_wk_parameters_weight_ = None
        v: "f32[8, 16, 96]" = smith._C._nn.linear(l_x_, l_self_modules_wv_parameters_weight_, None);  l_self_modules_wv_parameters_weight_ = None
        ...
        local_map_hop = smith.ops.higher_order.local_map_hop(subgraph_0, q_1, k_1, v_1);  subgraph_0 = q_1 = k_1 = v_1 = None
        ...
        o_1: "f32[8, 16, 96]" = smith._C._nn.linear(o, l_self_modules_wo_parameters_weight_, None);  o = l_self_modules_wo_parameters_weight_ = None
        o0: "f32[8, 16, 96]" = o_1 + l_x_;  o_1 = l_x_ = None
        o_2: "f32[8, 16, 384]" = smith._C._nn.linear(o0, l_self_modules_w1_parameters_weight_, None);  l_self_modules_w1_parameters_weight_ = None
        o_3: "f32[8, 16, 384]" = smith.nn.functional.relu(o_2);  o_2 = None
        o_4: "f32[8, 16, 96]" = smith._C._nn.linear(o_3, l_self_modules_w2_parameters_weight_, None);  o_3 = l_self_modules_w2_parameters_weight_ = None
    ...
    class subgraph_0(smith.nn.Module):
        def forward(self, q_1: "f32[1, 2, 4, 6]", k_1: "f32[1, 2, 16, 6]", v_1: "f32[1, 2, 16, 6]"):
            out: "f32[1, 2, 4, 6]" = smith._C._nn.scaled_dot_product_attention(query = q_1, key = k_1, value = v_1, is_causal = False);  q_1 = k_1 = v_1 = None
            return (out,)""",
    ...
    def test_sac(self):
        ...
        from smith._dynamo.testing import AotEagerAndRecordGraphs, normalize_gm
        ...
        if not TEST_WITH_CROSSREF:
            ...
            self.assertEqual(
                len(
                    backend.graphs[0].graph.find_nodes(
                        op="call_function",
                        target=smith._higher_order_ops.wrap.tag_activation_checkpoint,
                    )
                ),
                1,
            )
            ...
            for node in backend.fw_graphs[0].graph.nodes:
                if "recompute" in node.meta:
                    ...
                    if actual == smith.utils.checkpoint.CheckpointPolicy.MUST_SAVE:
                        self.assertTrue(node.name in fw_outs and node.name in bw_ins)
                    elif (
                        actual == smith.utils.checkpoint.CheckpointPolicy.MUST_RECOMPUTE
                    ):
    ...
    def test_sac_deferred(self):
        ...
        inputs = (
            smith.randn(bs, seq_len, dim1, requires_grad=True, dtype=smith.bfloat16),
        )
        try:
            ...
            self.assertTrue(
                "module 'smith._higher_order_ops.local_map' has no attribute 'call_local_map'"
                in str(e)
            )
    ...
    def test_local_map_dynamo_mismatch_placements(self):
        ...
        x = smith.randn(64, 64, 64, requires_grad=True)
        y = smith.randn(64, 64, 64, requires_grad=True)
        with (
            ...
            smith.compile(mismatch_input, backend="eager", fullgraph=True)(x, y)
        ...
        with (
            ...
            smith.compile(mismatch_outputs, backend="eager", fullgraph=True)(x)
    ...
    def test_local_map_dynamo_reordered_inputs(self):
        ...
        x = smith.randn(64, 64, 64, requires_grad=True)
        y = smith.randn(8, 64, 64, requires_grad=True)
        with (
            LocalMapWrappedHigherOrderVariable.enable(),
            self.assertRaisesRegex(
                AssertionError,
            ...
            smith.compile(reorder_inputs, backend="eager", fullgraph=True)(x, y)
    ...
    def test_local_map_with_local_shapes_hop_tracing(self):
        ...
        with FakeTensorMode():
            global_tensor = smith.randn(80, 80, requires_grad=True)
        with smith._higher_order_ops.local_map.defer_inlining():
            out = smith._higher_order_ops.local_map_hop(gm, global_tensor)
            out[0].sum().backward()
    ...
    def test_local_map_with_local_shapes_dynamo_tracing(self):
        ...
        def inputs_fn():
            return (smith.randn(80, 80, requires_grad=True),)
    ...
    def test_none_placements(self):
        ...
        def inputs_fn():
            return (smith.randn(10, 10, requires_grad=True),)
    ...
    def test_filtered_gradients(self):
        ...
        def returns_non_param(w, x):
            ...
            return smith.matmul(x, w.t()), x + 20
    ...
    def test_symint_activations(self):
        ...
        def _all_to_all(
            ...
            tensor = smith.ops._c10d_functional.all_to_all_single(
                self, output_split_sizes, input_split_sizes, group_name
            )
            res = smith.ops._c10d_functional.wait_tensor(tensor)
            return res

        class _AllToAll(smith.autograd.Function):
            @staticmethod
            def forward(
                ctx: Any,
                x: smith.Tensor,
            ...
            @staticmethod
            def backward(ctx: Any, grad_output: smith.Tensor):  # type: ignore[override]
                return _all_to_all(
                    grad_output,
        ...
        def _forward(num_tokens_per_expert, x, axis_name):
            ...
            with smith.no_grad():
                ...
                input_splits = (
                    ...
                    .sum(dim=1)
                    .to(smith.device("cpu"), non_blocking=True)
                )
                # NOTE: this would incur a device-to-host sync
                output_splits = (
                    num_tokens_per_expert_group.view(ep_size, -1)
                    .sum(dim=1)
                    .to(smith.device("cpu"), non_blocking=False)
                )
        ...
        def inputs_fn():
            x = smith.randn(8, 80, requires_grad=True)
            num_tokens_per_expert = smith.ones(1, 8, dtype=smith.int32)
            return (
                num_tokens_per_expert,
                x,
            )
    ...
    def test_fx_annotations(self):
        ...
        class MyModule(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                b = smith.utils.checkpoint.checkpoint(
        ...
        with fx_traceback.preserve_node_meta():
            ...
            interp = smith.fx.Interpreter(joint_gm_deferred)
