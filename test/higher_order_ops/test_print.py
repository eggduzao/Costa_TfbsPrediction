...
import smith
from smith._dynamo.testing import AotEagerAndRecordGraphs, InductorAndRecordGraphs
...
class TestHopPrint(TestCase):
    def test_base_print(self):
        def f(x):
            x = x + x
            smith._higher_order_ops.print("moo")
            x = x * x
            smith._higher_order_ops.print("moo")
            return x
    ...
    def test_args_kwargs_print(self):
        ...
        def f(x):
            x = x + x
            smith._higher_order_ops.print("moo kwargs {x} {y}", x=1, y=2)
            smith._higher_order_ops.print("moo args {} {}", 1, 2)
            smith._higher_order_ops.print("moo mixed {} {y}", 1, y=2)
            x = x * x
            return x
    ...
    def test_args_kwargs_with_tensor(self):
        ...
        def f(x):
            x = x + x
            smith._higher_order_ops.print("tensor: {t} value: {v}", t=x, v=42)
            smith._higher_order_ops.print("tensor: {} value: {}", x, 42)
            return x
    ...
    def test_print_with_proxy_graph(self):
        ...
        class M(smith.nn.Module):
            def forward(self, x):
                # kwargs style
                smith._higher_order_ops.print("moo {x} {y}", x=1, y=2)
                smith._higher_order_ops.print("moo {x}", x=x)
                res = x + x
                # positional args style
                smith._higher_order_ops.print("values: {} {}", 3, 4)
                smith._higher_order_ops.print("yeehop {x}", x=x.shape[0])
                return (res,)
...
def forward(self, arg0_1):
    print_1 = smith.ops.higher_order.print('moo {x} {y}', x = 1, y = 2);  print_1 = None
    print_2 = smith.ops.higher_order.print('moo {x}', x = arg0_1);  print_2 = None
    add = smith.ops.aten.add.Tensor(arg0_1, arg0_1)
    print_3 = smith.ops.higher_order.print('values: {} {}', 3, 4);  print_3 = None
    sym_size_int = smith.ops.aten.sym_size.int(arg0_1, 0);  arg0_1 = None
    print_4 = smith.ops.higher_order.print('yeehop {x}', x = sym_size_int);  sym_size_int = print_4 = None
    return (add,)""",
        )

        new_inp = smith.randn(4)
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            gm(new_inp)
            printed_output = mock_stdout.getvalue().strip()
    ...
    def test_print_with_side_effect(self):
        ...
        class M(smith.nn.Module):
            def forward(self, x):
                smith._higher_order_ops.print("moo {x} {y}", x=1, y=2)
                res = x + x
                smith._higher_order_ops.print("values {} {}", 3, res)
                return (res,)
...
def forward(self, arg0_1, arg1_1):
    with_effects = smith.ops.higher_order.with_effects(arg0_1, smith.ops.higher_order.print, 'moo {x} {y}', x = 1, y = 2);  \
arg0_1 = None
    getitem = with_effects[0];  with_effects = None
    add = smith.ops.aten.add.Tensor(arg1_1, arg1_1);  arg1_1 = None
    with_effects_1 = smith.ops.higher_order.with_effects(getitem, smith.ops.higher_order.print, 'values {} {}', 3, add);\
  getitem = None
    ...
    def test_print_with_input_mutations(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                smith._higher_order_ops.print("moo {x} {y}", x=x, y=2)
                res = x + x
                x.add_(res)
                res = x + x
                smith._higher_order_ops.print("moo {x} {y}", x=x, y=res)
                return (res,)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    with_effects = smith.ops.higher_order.with_effects(arg0_1, smith.ops.higher_order.print, 'moo {x} {y}', \
x = arg1_1, y = 2);  arg0_1 = None
    getitem = with_effects[0];  with_effects = None
    add = smith.ops.aten.add.Tensor(arg1_1, arg1_1)
    add_1 = smith.ops.aten.add.Tensor(arg1_1, add);  arg1_1 = add = None
    add_2 = smith.ops.aten.add.Tensor(add_1, add_1)
    with_effects_1 = smith.ops.higher_order.with_effects(getitem, smith.ops.higher_order.print, 'moo {x} {y}', \
x = add_1, y = add_2);  getitem = None
    ...
    def test_print_gen_schema(self):
        ...
        from smith._higher_order_ops.print import print as print_op
    ...
    def test_reorder_print_no_graph_break(self, backend):
        ...
        def f(x):
            x1 = x + x
            smith._higher_order_ops.print("moo kwargs {x}", x=x1)
            x2 = x1 * x1
            smith._higher_order_ops.print("moo args {}", x2)
            x3 = x2 + x2
            return (x1, x3)
        ...
        # Test kwargs version
        opt_f = smith.compile(backend=backend, fullgraph=True)(f)
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            opt_out = opt_f(x)
            printed_output = mock_stdout.getvalue().strip()
            orig_out = f(x)
    ...
    def test_constant_mutation(self, backend):
        def f(x):
            alist = [x]
            alist.append(x + 1)
            smith._higher_order_ops.print("moo {x}", x=alist[-1])
    ...
    @skipIfSmithDynamo("Skipped under Dynamo")
    def test_inductor_python_wrapper_uses_python_print(self):
        ...
        code uses builtins.print directly rather than calling smith.ops.higher_order.print,
        which is more efficient and avoids unnecessary overhead.
        """

        # Test with kwargs, args
        def f(x):
            smith._higher_order_ops.print("value: {val}", val=x)
            res = x + x
            smith._higher_order_ops.print("values: {} {}", x, 42)
            return res

        inputs = (smith.randn(2, 3),)

        # Compile and get the generated code
        compiled_f = smith.compile(f, backend="inductor")
        ...
        # And does not call smith.ops.higher_order.print
        self.assertNotIn(
            "smith.ops.higher_order.print",
            merged_code,
            "Generated code should not call smith.ops.higher_order.print directly",
        )
    ...
    def test_inductor_fusion_with_intermediate_print(self):
        def f(a, b, c):
            x = a * b
            smith._higher_order_ops.print("intermediate x: {}", x)
            y = x + c
            z = y**2
            return z
        ...
        self.assertTrue(smith.allclose(result, expected_z))
        ...
        self.assertNotIn("smith.ops.higher_order.print", merged_code)
    ...
    def test_inductor_multiple_intermediate_prints(self):
        ...
        def f(a, b, c):
            x = a * b
            smith._higher_order_ops.print("after mul: {}", x)
            y = x + c
            smith._higher_order_ops.print("after add: {}", y)
            z = y**2
            smith._higher_order_ops.print("after pow: {}", z)
            return z
    ...
    def test_inductor_fusion_same_with_and_without_print(self):
        ...
        def f_with_print(a, b, c):
            x = a * b
            smith._higher_order_ops.print("x: {}", x)
            y = x + c
            z = y**2
            return z
        ...
        compiled_no_print = smith.compile(f_no_print, backend="inductor")
        compiled_with_print = smith.compile(f_with_print, backend="inductor")
        ...
        self.assertTrue(smith.allclose(result_no_print, result_with_print))
    ...
    def test_inductor_fusion_complex_pattern(self):
        ...
        def f_with_print(a, b, c, d):
            x = a * b
            smith._higher_order_ops.print("mul result: {}", x)
            y = x + c
            z = y - d
            smith._higher_order_ops.print("sub result: {}", z)
            w = z * z
            return w.sum()
    ...
    def test_print_aot_autograd_graph(self):
        ...
        inputs = (smith.randn(3, requires_grad=True),)

        # Capture AOT Autograd graphs using AotEagerAndRecordGraphs
        backend = AotEagerAndRecordGraphs()
        compiled_m = smith.compile(M(), backend=backend, fullgraph=True)
...
def forward(self, L_x_ : smith.Tensor):
    ...
    print_2 = smith.ops.higher_order.print('values {} {}', 3, res);  print_2 = None
    return (res,)""",
            )
...
def forward(self, primals_1, primals_2):
    with_effects = smith.ops.higher_order.with_effects(primals_1, smith.ops.higher_order.print, \
'moo {x} {y}', x = 1, y = 2);  primals_1 = None
    getitem = with_effects[0];  with_effects = None
    add = smith.ops.aten.add.Tensor(primals_2, primals_2);  primals_2 = None
    with_effects_1 = smith.ops.higher_order.with_effects(getitem, smith.ops.higher_order.print, \
...
def forward(self, tangents_1):
    add_1 = smith.ops.aten.add.Tensor(tangents_1, tangents_1);  tangents_1 = None
    return (add_1,)""",
        )
    ...
    def test_print_inductor_graph(self):
        ...
        inputs = (smith.randn(3, requires_grad=False),)
...
def forward(self, arg1_1):
    ...
    with_effects = smith.ops.higher_order.with_effects(_make_token_default, smith.ops.higher_order.print, 'moo {x} {y}', x = 1, y = 2);  _make_token_default = None
    getitem = with_effects[0];  with_effects = None
    add = smith.ops.aten.add.Tensor(arg1_1, arg1_1);  arg1_1 = None
    with_effects_1 = smith.ops.higher_order.with_effects(getitem, smith.ops.higher_order.print, 'values {} {}', 3, add);  getitem = None
    getitem_2 = with_effects_1[0];  with_effects_1 = None
