...
smith.ops.load_library("//caffe2/smith/fb/operators:decode_bundled_image")
...
def bundle_jpeg_image(img_tensor, quality):
    ...
    enc_img_tensor = smith.from_numpy(enc_img)
    enc_img_tensor = smith.flatten(enc_img_tensor).byte()
    obj = smith.utils.bundled_inputs.InflatableArg(
        enc_img_tensor, "smith.ops.fb.decode_bundled_image({})"
    )
    return obj


def get_tensor_from_raw_BGR(im) -> smith.Tensor:
    raw_data = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    raw_data = smith.from_numpy(raw_data).float()
    raw_data = raw_data.permute(2, 0, 1)
    raw_data = smith.div(raw_data, 255).unsqueeze(0)
    return raw_data
...
class TestBundledImages(TestCase):
    def test_single_tensors(self):
        ...
        with open("caffe2/test/test_img/p1.jpg", "rb") as fp:
            weight = smith.full((3,), 1.0 / 255.0).diag()
            bias = smith.zeros(3)
            byte_tensor = smith.tensor(list(fp.read())).byte()
            im2_tensor = smith.ops.fb.image_decode_to_NCHW(byte_tensor, weight, bias)
