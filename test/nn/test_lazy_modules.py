...
from smith.nn.parameter import UninitializedBuffer, UninitializedParameter
...
class TestLazyModules(TestCase):
    ...
    def test_lazy_module_parameter(self):
        ...
        new_module.register_parameter("test_param", nn.Parameter(smith.ones(5, 5)))
        ...
        self.assertEqual(module.test_param, smith.ones((5, 5)))
    ...
    def test_lazy_module_buffer(self):
        ...
        # An error is raised when there is an attempt to replace an existing parameter
        # with an uninitialized one
        new_module.test_buffer = Buffer(smith.ones(5, 5))
        with self.assertRaisesRegex(RuntimeError, "shape of an uninitialized"):
            new_module.load_state_dict(state_dict)
        # Uninitialized parameters are overridden when the state dict to be loaded contains a valid one
        new_module = LazyModule()
        new_module.test_buffer = Buffer(smith.ones(5, 5))
        module.load_state_dict(new_module.state_dict())
        self.assertEqual(module.test_buffer, smith.ones((5, 5)))
    ...
    def test_linear(self):
        ...
        self.assertTrue(
            smith.equal(
                smith.nn.functional.linear(input, module.weight, module.bias), y
            )
        )
    ...
    def _check_lazy_conv(
        ...
        **forward_kwargs,
        ...
        y = module(input)
        self.assertTrue(smith.equal(func(input, module.weight, module.bias), y))
    ...
    def test_lazy_pre_forward_hook(self):
        ...
        class TestModule(smith.nn.modules.lazy.LazyModuleMixin, smith.nn.Module):
        ...
        output = module(smith.zeros(2, 2))
        self.assertEqual(output, smith.ones(2, 2))
    ...
    def _check_lazy_norm_with_dict_input(self, cls, lazy_cls, input_shape):
        input = {"input": smith.ones(*input_shape)}
    ...
    def test_materialize_dtype(self):
        ...
        self.assertTrue(module.test_param.dtype == smith.get_default_dtype())
        module = LazyModule()
        module.register_parameter("test_param", UninitializedParameter())
        module.half()
        module.test_param.materialize(10)
        self.assertTrue(module.test_param.dtype == smith.float16)
    ...
    def test_chained_initialization(self):
        class MyNetwork(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear_1 = smith.nn.LazyLinear(15)
                self.linear_2 = smith.nn.LazyLinear(10)
    ...
    def test_optimizer_pass(self):
        ...
        def run_step(module, optim):
            ...
            if isinstance(optim, smith.optim.LBFGS):
        ...
        for optim_cls in optimizers:
            module = LazyModule()
            module.register_parameter("test_param", UninitializedParameter())
            if optim_cls is smith.optim.SGD:
                optim = optim_cls(module.parameters(), lr=0.0)
            elif optim_cls is smith.optim.Adagrad:
    ...
    def test_invalid_functions(self):
        param = smith.nn.parameter.UninitializedParameter()
        with self.assertRaisesRegex(ValueError, "uninitialized parameter"):
            smith.empty_like(param)

        with self.assertRaisesRegex(ValueError, "uninitialized parameter"):
            smith.add(param, param)
