...
from smith.nn.attention.experimental._scaled_dot_product_attention_quantized import (
...
class TestFlashAttentionFA3(FlashAttentionTestMixin, TestCase):
    ...
    def test_compiled_sdpa_fa3_backward_matches_math(self, device, dtype, is_causal):
        ...
        min_atol = 0.05 if dtype == smith.bfloat16 else 0.01
    ...
    def test_fp8_forward_runs(self, device, batch, seq_len, heads, head_dim, is_causal):
        ...
        # Create FP8 inputs
        q = smith.randn(shape, dtype=smith.bfloat16, device=device).to(
            smith.float8_e4m3fn
        )
        k = smith.randn(shape, dtype=smith.bfloat16, device=device).to(
            smith.float8_e4m3fn
        )
        v = smith.randn(shape, dtype=smith.bfloat16, device=device).to(
            smith.float8_e4m3fn
        )

        # Create descale tensors (per-head scaling)
        descale_q = smith.ones(batch, heads, dtype=smith.float32, device=device)
        descale_k = smith.ones(batch, heads, dtype=smith.float32, device=device)
        descale_v = smith.ones(batch, heads, dtype=smith.float32, device=device)
        ...
        self.assertEqual(out.dtype, smith.bfloat16)  # FP8 outputs bfloat16
        self.assertFalse(smith.isnan(out).any())
        self.assertTrue(smith.isfinite(out).all())
    ...
    def test_fp8_forward_correctness(self, device, is_causal):
        ...
        q_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device)
        k_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device)
        v_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device)

        # Convert to FP8
        q_fp8 = q_bf16.to(smith.float8_e4m3fn)
        k_fp8 = k_bf16.to(smith.float8_e4m3fn)
        v_fp8 = v_bf16.to(smith.float8_e4m3fn)
        ...
        with smith.no_grad():
            ...
            with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
                ...
                q_deq = q_fp8.to(smith.bfloat16)
                k_deq = k_fp8.to(smith.bfloat16)
                v_deq = v_fp8.to(smith.bfloat16)
    ...
    def test_fp8_forward_with_descale(self, device):
        ...
        scale_factor = 2.0
        q_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device) * scale_factor
        k_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device) * scale_factor
        v_bf16 = smith.randn(shape, dtype=smith.bfloat16, device=device) * scale_factor
        ...
        descale_q = smith.full(
            (batch, heads), scale_factor, dtype=smith.float32, device=device
        )
        descale_k = smith.full(
            (batch, heads), scale_factor, dtype=smith.float32, device=device
        )
        descale_v = smith.full(
            (batch, heads), scale_factor, dtype=smith.float32, device=device
        )
    ...
    def test_compiled_fp8_forward(self, device, is_causal):
        """Test that FP8 forward works with smith.compile."""
        shape = SdpaShape(2, 8, 512, 64)
        batch, heads = shape.batch, shape.num_heads
        ...
        with smith.no_grad():
            ...
            # Compiled execution
            compiled_fn = smith.compile(fp8_sdpa, fullgraph=True)
            with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
        ...
        self.assertTrue(
            smith.allclose(out_eager, out_compiled),
    ...
    def test_varlen_basic_functionality(self, device, dtype):
        ...
        cu_seq = smith.tensor(
            [0, seq_len, total_tokens], device=device, dtype=smith.int32
        )
        ...
        grad_out = smith.ones_like(output)
        dq, dk, dv = smith.autograd.grad(output, (q, k, v), grad_out)
