...
from smith.nn.attention import (
...
def flash_vs_math(test_case: TestCase, q, k, v, is_causal: bool = False, rtol: int = 2):
    ...
    test_case.assertFalse(smith.isnan(out_flash).any())
    test_case.assertFalse(smith.isnan(out_math_low).any())
    test_case.assertFalse(smith.isnan(out_math_fp32).any())
    test_case.assertTrue(smith.isfinite(out_flash).all())
    test_case.assertTrue(smith.isfinite(out_math_low).all())
    test_case.assertTrue(smith.isfinite(out_math_fp32).all())
...
class FlashAttentionTestMixin:
    ...
    def _assert_flash_matches_math(
        ...
        """
        q = smith.randn(shape, dtype=dtype, device=device).requires_grad_(True)
        k = smith.randn(shape, dtype=dtype, device=device).requires_grad_(True)
        v = smith.randn(shape, dtype=dtype, device=device).requires_grad_(True)
        ...
        if test_backward:
            ...
            g = smith.randn_like(out_flash)

            # Flash gradients
            dq_flash, dk_flash, dv_flash = smith.autograd.grad(
                out_flash, (q, k, v), g, retain_graph=True
            )

            # Math fp32 gradients (reference)
            dq_math_fp32, dk_math_fp32, dv_math_fp32 = smith.autograd.grad(
                out_math_fp32, (q, k, v), g, retain_graph=True
            )

            # Math low precision gradients
            dq_math_low, dk_math_low, dv_math_low = smith.autograd.grad(
    ...
    def _test_compiled_sdpa_metadata(self, device, dtype, is_causal):
        ...
        q = smith.randn(shape, dtype=dtype, device=device)
        k = smith.randn(shape, dtype=dtype, device=device)
        v = smith.randn(shape, dtype=dtype, device=device)
        ...
        compiled_sdpa = smith.compile(sdpa_fn, fullgraph=True)
        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
    ...
    def _test_compiled_sdpa_matches_math(self, device, dtype, is_causal):
        ...
        # Sanity checks
        self.assertFalse(smith.isnan(out_compiled).any())
        self.assertTrue(smith.isfinite(out_compiled).all())
    ...
    def _test_compiled_sdpa_backward_matches_math(
        ...
        def make_tensors():
            return (
                smith.randn(shape, dtype=dtype, device=device, requires_grad=True),
                smith.randn(shape, dtype=dtype, device=device, requires_grad=True),
                smith.randn(shape, dtype=dtype, device=device, requires_grad=True),
            )
        ...
        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
            out_compiled = compiled_sdpa(q, k, v)
        grad_out = smith.randn_like(out_compiled)
        dq_compiled, dk_compiled, dv_compiled = smith.autograd.grad(
        ...
        with sdpa_kernel(SDPBackend.MATH):
            out_math_fp32 = F.scaled_dot_product_attention(
                q_ref.to(smith.float32),
                k_ref.to(smith.float32),
                v_ref.to(smith.float32),
                attn_mask=None,
                dropout_p=0.0,
                is_causal=is_causal,
            )
        dq_math_fp32, dk_math_fp32, dv_math_fp32 = smith.autograd.grad(
            out_math_fp32, (q_ref, k_ref, v_ref), grad_out.to(smith.float32)
        )
        ...
        for t in [dq_compiled, dk_compiled, dv_compiled]:
            self.assertFalse(smith.isnan(t).any())
            self.assertTrue(smith.isfinite(t).all())
    ...
    def _test_attention_preserves_query_layout(self, device):
        ...
        def test_attention(permute_order: list[int]):
            ...
            q = smith.randn(
                *shape_q, dtype=smith.bfloat16, device=device, requires_grad=False
            ).permute(reverse)
            k = smith.randn(
                *shape_kv, dtype=smith.bfloat16, device=device, requires_grad=False
            ).permute(reverse)
            v = smith.randn(
                *shape_kv, dtype=smith.bfloat16, device=device, requires_grad=False
            ).permute(reverse)
    ...
    def _test_kernel_called(self, device, dtype):
        ...
        try:
            ...
            q = smith.randn(shape, dtype=dtype, device=device, requires_grad=True)
            k = smith.randn(shape, dtype=dtype, device=device, requires_grad=True)
            v = smith.randn(shape, dtype=dtype, device=device, requires_grad=True)
