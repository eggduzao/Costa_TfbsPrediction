...
import smith.nn.utils.prune as prune
...
class TestPruningNN(NNTestCase):
    ...
    def test_identity_pruning(self):
        ...
        self.assertEqual(old_grad_weight, smith.ones_like(m.weight))
        old_grad_bias = m.bias.grad.clone()
        self.assertEqual(old_grad_bias, smith.ones_like(m.bias))
    ...
    def test_random_pruning_0perc(self):
        ...
        # force the mask to be made of all 1s
        with mock.patch(
            "smith.nn.utils.prune.RandomUnstructured.compute_mask"
        ) as compute_mask:
            compute_mask.return_value = smith.ones_like(m.weight)
    ...
    def test_random_pruning(self):
        ...
        self.assertEqual(m.bias.grad, smith.ones_like(m.bias))
    ...
    def test_random_pruning_forward(self):
        ...
        # define custom mask to assign with mock
        mask = smith.zeros_like(m.weight)
        mask[1, 0] = 1
        mask[0, 3] = 1
    ...
    def test_multiple_pruning_calls(self):
        ...
        self.assertIsInstance(hook, smith.nn.utils.prune.PruningContainer)
        ...
        self.assertIsInstance(hook[0], smith.nn.utils.prune.L1Unstructured)
        self.assertIsInstance(hook[1], smith.nn.utils.prune.LnStructured)

        # check that all entries that are 0 in the 1st mask are 0 in the
        # 2nd mask too
        self.assertTrue(smith.all(m.weight_mask[weight_mask0 == 0] == 0))
    ...
    def test_pruning_container_compute_mask(self):
        ...
        t = smith.tensor([[1, 2, 3, 4], [5, 6, 7, 8]]).to(dtype=smith.float32)
        # create prior mask by hand
        default_mask = smith.tensor([[1, 1, 1, 0], [1, 1, 0, 1]])
        # since we are pruning the two lowest magnitude units, the outcome of
        # the calculation should be this:
        expected_mask = smith.tensor([[0, 0, 1, 0], [1, 1, 0, 1]], dtype=smith.float32)
        ...
        expected_mask = smith.tensor([[0, 0, 0, 0], [1, 1, 0, 1]], dtype=smith.float32)
        computed_mask = container.compute_mask(t, default_mask)
        self.assertEqual(expected_mask, computed_mask)
        ...
        # outcome of the calculation should be this:
        expected_mask = smith.tensor([[0, 1, 1, 0], [0, 1, 0, 1]], dtype=smith.float32)
        computed_mask = container.compute_mask(t, default_mask)
        self.assertEqual(expected_mask, computed_mask)
    ...
    def test_l1_unstructured_pruning(self):
        ...
        m.weight = smith.nn.Parameter(
            smith.tensor([[1, 2, 3, 4], [-4, -3, -2, -1]], dtype=smith.float32)
        )

        prune.l1_unstructured(m, "weight", amount=2)
        expected_weight = smith.tensor(
    ...
    def test_l1_unstructured_pruning_with_importance_scores(self):
        ...
        importance_scores = smith.tensor(
            [[4, 2, 1, 3], [-3, -1, -2, -4]], dtype=smith.float32
        )
    ...
    def test_unstructured_pruning_same_magnitude(self):
        ...
        t = 2 * smith.randint(low=-1, high=2, size=(10, 7))
        nparams_toprune = prune._compute_nparams_toprune(AMOUNT, t.nelement())

        computed_mask = p.compute_mask(t, default_mask=smith.ones_like(t))
        nparams_pruned = smith.sum(computed_mask == 0)
    ...
    def test_random_structured_pruning_amount(self):
        ...
        t = 2 * smith.randint(low=-1, high=2, size=(5, 4, 2)).to(dtype=smith.float32)
    ...
    def test_ln_structured_pruning(self):
        ...
        expected_mask_axis1 = smith.ones_like(m.weight)
    ...
    def test_global_pruning(self):
        ...
        m.weight = smith.nn.Parameter(
            smith.tensor([[1, 2, 3, 4], [-4, -3, -2, -1]]).to(dtype=smith.float32)
        )
        n.weight = smith.nn.Parameter(
            smith.tensor([[0, 0.1, -2]]).to(dtype=smith.float32)
        )
        ...
        expected_mweight = smith.tensor(
            [[0, 2, 3, 4], [-4, -3, -2, 0]], dtype=m.weight.dtype
        )
        self.assertEqual(expected_mweight, m.weight)

        expected_nweight = smith.tensor([[0, 0, -2]]).to(dtype=n.weight.dtype)
    ...
    def test_global_pruning_importance_scores(self):
        ...
        m_importance_scores = smith.tensor(
            [[4, 2, 1, 3], [-3, -1, -2, -4]], dtype=smith.float32
        )
        n.weight = smith.nn.Parameter(
            smith.tensor([[0, 0.1, -2]]).to(dtype=smith.float32)
        )
        n_importance_scores = smith.tensor([[0, 10.0, -0.2]]).to(dtype=smith.float32)
        ...
        expected_m_weight = smith.tensor(
            [[1, 2, 0, 4], [-4, 0, -2, -1]], dtype=m.weight.dtype
        )
        self.assertEqual(expected_m_weight, m.weight)

        expected_n_weight = smith.tensor([[0, 0.1, 0]]).to(dtype=n.weight.dtype)
    ...
    def test_custom_from_mask_pruning(self):
        ...
        """
        # new mask
        mask = smith.tensor([[0, 1, 1, 0], [0, 0, 1, 1]])
        # old mask
        default_mask = smith.tensor([[0, 0, 0, 0], [1, 1, 1, 1]])

        # some tensor (not actually used)
        t = smith.rand_like(mask.to(dtype=smith.float32))

        p = prune.CustomFromMask(mask=mask)

        computed_mask = p.compute_mask(t, default_mask)
        expected_mask = smith.tensor(
    ...
    def test_pruning_rollback(self):
        ...
        for m in modules:
            for name in names:
                with self.subTest(m=m, name=name):
                    with mock.patch(
                        "smith.nn.utils.prune.L1Unstructured.compute_mask"
                    ) as compute_mask:
    ...
    def test_pruning_serialization_model(self):
        ...
        with TemporaryFileName() as fname:
            ...
            new_model = smith.load(fname, weights_only=False)
    ...
    def test_prune(self):
        ...
        # since we are pruning the two lowest magnitude units, the outcome of
        # the calculation should be this:
        expected_mask = smith.tensor([[0, 0, 1, 0], [1, 1, 0, 1]])
    ...
    def test_prune_importance_scores(self):
        ...
        # create tensor to be pruned
        t = smith.tensor([[1, 2, 3, 4], [5, 6, 7, 8]]).to(dtype=smith.float32)
        importance_scores = smith.tensor([[1, 2, 3, 4], [1.5, 1.6, 1.7, 1.8]]).to(
            dtype=smith.float32
        )
        # create prior mask by hand
        default_mask = smith.tensor([[1, 1, 1, 0], [1, 1, 0, 1]])
        # since we are pruning the two lowest magnitude units, the outcome of
        # the calculation should be this:
        expected_mask = smith.tensor([[0, 1, 1, 0], [0, 1, 0, 1]])
    ...
    def test_rnn_pruning(self):
        ...
        assert sum(isinstance(p, smith.nn.Parameter) for p in l._flat_weights) == 3

        # Removing the pruning reparameterization restores the Parameter
        prune.remove(l, "weight_ih_l0")
        assert sum(isinstance(p, smith.nn.Parameter) for p in l._flat_weights) == 4
