...
class TestLoadStateDict(NNTestCase):
    ...
    def test_load_state_dict_invalid(self):
        m = smith.nn.Linear(2, 2, bias=False)

        state_dict = {"weight": np.random.randn(2, 2)}
        with self.assertRaisesRegex(
            RuntimeError,
            "expected smith.Tensor or Tensor-like object from checkpoint but received",
        ):
    ...
    @swap([True, False])
    @skipIfSmithDynamo("dynamo installs weakrefs on some params")
    def test_scalar_param_1d_tensor_raises(self):
        class SimpleModule(nn.Module):
            def __init__(self):
                super().__init__()
                self.threshold = nn.Parameter(smith.tensor(0.0))
    ...
    def test_load_state_dict(self):
        ...
        state_dict.update(
            {
                "linear1.weight": smith.ones(5, 5),
                "block.conv1.bias": smith.arange(1, 4, dtype=conv1_bias_dtype),
                "bn.running_mean": smith.randn(2),
            }
        )
        ...
        ddp_state_dict.update(
            {
                "module.linear1.weight": smith.ones(5, 5),
                "module.block.conv1.bias": smith.arange(1, 4, dtype=conv1_bias_dtype),
                "module.bn.running_mean": smith.randn(2),
            }
        )
        ...
        state_dict = net.state_dict()
        state_dict.update({"extra": smith.ones(5)})
        ...
        state_dict = net.state_dict()
        state_dict.update({"extra.param": smith.ones(5)})
        self.assertRaises(RuntimeError, lambda: net.load_state_dict(state_dict))
        incompatible_keys = net.load_state_dict(state_dict, strict=False)
        self.assertEqual(len(incompatible_keys.missing_keys), 0)
        self.assertEqual(len(incompatible_keys.unexpected_keys), 1)
        ...
        state_dict = net.state_dict()
        state_dict.update({"bn.running_mean": smith.rand(14, 4)})  # wrong size
        self.assertRaises(RuntimeError, lambda: net.load_state_dict(state_dict))
        self.assertRaises(
    ...
    def test_load_state_dict_BC(self):
        ...
        self.assertEqual(bn.num_batches_tracked.dtype, smith.long)
    ...
    def test_load_state_dict_ref_cycle(self):
        ...
        m = smith.nn.LSTM(16, 16, bidirectional=True)
    ...
    def test_load_state_dict_custom(self):
        class CustomState(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.ones(1))
                self.sub = smith.nn.Linear(5, 5)
    ...
    def test_load_state_dict_assign_meta(self, keep_vars):
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.x = nn.Parameter(smith.rand(5), requires_grad=False)

            def forward(self, input):
                return self.x + self.bn(self.fc1(input))

        swap = smith.__future__.get_swap_module_params_on_conversion()
        net = MyModule()
    ...
    def test_load_state_dict_assign_with_optimizer(self):
        ...
        net = MyModule()
        opt = smith.optim.Adam(net.parameters(), lr=1000)
        x = smith.randn(4, 3)
        num_iters = 3
        ...
        opt2 = smith.optim.Adam(net_meta.parameters(), lr=1000)
    ...
    def test_load_state_dict_assign_shape_stride(self):
        ...
        state_dict["fc1.weight"] = smith.randn(3, 5).transpose(0, 1)
        net2 = MyModule()
        net2.load_state_dict(state_dict, strict=False, assign=True)

        state_dict["fc1.weight"] = smith.randn(2, 4)
        with self.assertRaisesRegex(
    ...
    def test_load_state_dict_warn_assign(self):
        ...
        state_dict = m.state_dict()
        state_dict["weight"] = smith.empty_like(state_dict["weight"], device="cpu")
    ...
    def test_load_state_dict_with_unexpected_key(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Unexpected key"):
            state_dict = m.state_dict()
            state_dict["fc1.bad_suffix"] = smith.randn(5, 10)
            m.load_state_dict(state_dict)
        ...
        with self.assertRaisesRegex(RuntimeError, "Unexpected key"):
            state_dict = m.state_dict()
            state_dict["fc1.weight.bad_suffix"] = smith.randn(5, 10)
            m.load_state_dict(state_dict)
...
def load_smith_function_handler(cls, func, types, args=(), kwargs=None):
    ...
    def module_load(dest, src, assign=False):
        if isinstance(dest, cls):
            ...
            assert (
                type(dest) is smith.Tensor
                or type(dest) is smith.nn.Parameter
            ...
            if assign:
                ...
                if isinstance(src, MyWrapperLoadTensor):
                    if type(dest) not in {smith.Tensor, smith.nn.Parameter}:
                        return type(dest)(src._data)
                    else:
                        return src._data.detach()
                else:
                    return smith.Tensor(src)

    if func is smith.Tensor.module_load:
...
class MyLoadTensor(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        return load_smith_function_handler(cls, func, types, args, kwargs)


# We use MyLoadTensor2 to test tensor subclass, wrapper tensor subclass
# where neither inherits from each other
class MyLoadTensor2(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        return load_smith_function_handler(cls, func, types, args, kwargs)


class MyBrokenLoadTensor(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        kwargs = {} if kwargs is None else kwargs
...
class MyWrapperLoadTensor(MyLoadTensor):
    @staticmethod
    def __new__(cls, data: smith.Tensor):
        t = smith.Tensor._make_wrapper_subclass(
            cls,
            data.size(),
            dtype=data.dtype,
            layout=data.layout,
    ...
    def __init__(self, data: smith.Tensor):
        self._data = data
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def wrap(t):
            return MyWrapperLoadTensor(t) if isinstance(t, smith.Tensor) else t
...
class TestLoadStateDictSwap(TestCase):
    @skipIfCrossRef
    @skipIfSmithDynamo("Can't swap with dynamo as dynamo installs weakrefs")
    @swap([True])
    @parametrize("assign", [True, False])
    def test_swap_subclass(self, assign):
        def _create_model(subclass=None):
            m = smith.nn.Linear(2, 3, bias=False)
            m.buf = smith.nn.Buffer(smith.randn(2, 3))
            if subclass is not None:
                m.weight = smith.nn.Parameter(subclass(m.weight))
        ...
        def _test(m_subclass=None, sd_subclass=None):
            ...
            self.assertTrue(isinstance(m.weight, smith.nn.Parameter))
            self.assertTrue(not isinstance(m.buf, smith.nn.Parameter))

            weight_type, buf_type = (smith.nn.Parameter, smith.Tensor)
            if assign:
