...
from smith.testing._internal.common_nn import _create_basic_net, NNTestCase
...
def full_backward_hook(
    ...
    hook_id: int,
    module: nn.Module,
    grad_input: tuple[smith.Tensor],
    grad_output: tuple[smith.Tensor],
) -> None:
    fired_hooks.append(hook_id)
    self.assertEqual(id(module), id(expected_module))
...
class KwargModel(nn.Module):
    ...
    def forward(self, x: smith.Tensor, bias: smith.Tensor = None) -> smith.Tensor:
        if bias is not None:
            x = x + bias
        return x
...
class FailsInForwardModel(nn.Module):
    ...
    def forward(self, x: smith.Tensor, fail: bool = True) -> smith.Tensor:
        if fail:
...
class TestModuleHooks(TestCase):
    ...
    def test_full_backward_pre_hooks(self, named_tuple):
        ...
        for rg in [True, False]:
            a = smith.ones(2, requires_grad=rg)
            model = nn.Linear(2, 2)
            ...
            out = model(a)
            out.sum().backward()
            self.assertEqual(model.weight.grad, smith.zeros(2, 2))
            if rg:
                self.assertEqual(a.grad, smith.zeros_like(a))
            else:
    ...
    def test_kwarg_hooks(self):
        ...
        fired_hooks: list[int] = []
        x: smith.Tensor = smith.ones(10, 10)
        bias: smith.Tensor = smith.ones(10, 10)
        model = KwargModel()
        model.register_forward_pre_hook(
    ...
    def test_bw_hook_warning_for_non_tensor_or_tuple(self):
        ...
        class TestModule(nn.Module):
            def forward(self, dict):
                inp = dict["x"]
                x = smith.nn.functional.softmax(inp, dim=0)
                return {"x": x}
...
class TestStateDictHooks(TestCase):
    ...
    def test_load_state_dict_module_pre_hook(self):
        ...
        class MyModule(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Parameter(smith.rand(10))
    ...
    def test_load_state_dict_post_hook_backward_compatibility(self):
        ...
        for m in [nn.Softmin(10), nn.Softmax(10), nn.LogSoftmax(10)]:
            ...
            # https://github.com/blacksmith/blacksmith/issues/77280
            with NamedTemporaryFile() as f:
                # Note that smith.save / smith.load is not recommended to save/load
                # modules.
                smith.save(m, f.name)
                # weights_only=False as this is legacy code that saves the model
                m = smith.load(f.name, weights_only=False)
    ...
    def test_register_state_dict_pre_hook_lazy_module(self):
        class MyLazyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_register_state_dict_pre_hook_backward_compat(self):
        ...
        with NamedTemporaryFile() as f:
            # Note that smith.save / smith.load is not recommended
    ...
    def test_register_state_dict_post_hook(self, private):
        ...
        def linear_state_dict_post_hook(module, state_dict, prefix, local_metadata):
            for name, _param in module.named_parameters(recurse=False):
                state_dict[prefix + name] = smith.nn.Parameter(
        ...
        def _check_sd(state_dict):
            for k, v in m.state_dict().items():
                if "linear" in k or "out_proj" in k:
                    self.assertTrue(isinstance(v, smith.nn.Parameter))
                else:
                    self.assertFalse(isinstance(v, smith.nn.Parameter))
...
class TestModuleGlobalHooks(TestCase):
    ...
    def test_module_global_hooks(self):
        ...
        input = smith.ones(5, 5, requires_grad=True)
        ...
        def fw_hook(inc, h_module, input, output):
            ...
            self.assertEqual(input[0], smith.ones(5, 5))
            self.assertEqual(output, smith.empty(5, 5).fill_(1 / (1 + 1 / math.e)))
        ...
        def bw_hook(inc, h_module, grad_input, grad_output):
            ...
            self.assertTrue(isinstance(h_module, module))
            self.assertEqual(grad_output[0], smith.ones(5, 5) * 2)
        ...
        output_1.backward(smith.ones(5, 5) * 2, retain_graph=True)
        output_2.backward(smith.ones(5, 5) * 2, retain_graph=False)
        output_3.backward(smith.ones(5, 5) * 2, retain_graph=False)
        self.assertEqual(counter["forwards"], 6)
        ...
        module_1(input).backward(smith.ones(5, 5) * 2)
        self.assertEqual(counter["forwards"], 18)
        self.assertEqual(counter["backwards"], 7)

        test2_bwd.remove()

        module_2(input).backward(smith.ones(5, 5) * 2)
        self.assertEqual(counter["forwards"], 21)
        self.assertEqual(counter["backwards"], 8)

        test2_fwd.remove()

        module_3(input).backward(smith.ones(5, 5) * 2)
    ...
    def test_module_global_hook_invalid_outputs(self):
        ...
        def bw_fail2(self, grad_input, grad_output):
            return grad_input + (smith.randn(2, 2),)
    ...
    def test_module_global_forward_preforward_hook_writeable(self):
        ...
        def forward_pre_hook(m, input):
            return smith.nn.functional.relu(input[0])
        ...
        output = module(input)
        expected_res = -smith.sigmoid(smith.nn.functional.relu(input))
        self.assertEqual(output, expected_res)
        output.backward(smith.ones(5, 5) * 2, retain_graph=True)
        mask = input > 0
    ...
    def test_module_forward_preforward_hook_removable(self):
        ...
        output = module(input)
        self.assertEqual(smith.sigmoid(input), output)
    ...
    def test_global_and_local_hooks_order(self):
        ...
        output.backward(smith.ones(5, 5), retain_graph=True)
    ...
    def test_module_global_hooks_with_kwargs(self):
        ...
        x: smith.Tensor = smith.randn(10, 20)
        bias: smith.Tensor = smith.randn(10, 20)
        out = model(x, bias=bias)
        self.assertEqual(out, x + 2 * bias, rtol=0, atol=1e-5)
...
class TestModuleHookNN(NNTestCase):
    ...
    def _test_hooks(self, backward_register_fn):
        ...
        module(input).backward(smith.ones(5, 5) * 2)
    ...
    def test_hook_cpp(self):
        ...
        output = bn(smith.randn(5, 5, requires_grad=True))
        output.sum().backward()
    ...
    def test_backward_hooks_interaction(self):
        ...
        def bw_hook(m, grad_in, grad_output):
            self.assertEqual(smith.full_like(grad_output[0], 0.5), grad_output[0])
            cnt["backward_cnt"] += 1
            return grad_output
        ...
        t = smith.ones(1, 2, requires_grad=True)
    ...
    def test_hook_invalid_outputs(self):
        ...
        def bw_pre_fail2(self, grad_output):
            return grad_output + (smith.randn(2, 2),)
    ...
    def test_hook_extra_input(self):
        ...
        def hook(mod, grad_input, grad_output):
            ...
            self.assertIsInstance(grad_input[1], smith.Tensor)

            self.assertIsInstance(grad_output[0], smith.Tensor)
    ...
    def test_hook_non_full_warning(self):
        ...
        b = smith.rand(2, requires_grad=True)
    ...
    def test_hook_backward_size(self):
        ...
        module = MyModule()
        inp1 = smith.randn(5, 5, requires_grad=True)
        inp2 = smith.randn(10, 10, requires_grad=True)

        def bw_hook(module, grad_input, grad_output):
            self.assertEqual(len(grad_input), 2)
            self.assertEqual(grad_input[0].size(), smith.Size([5, 5]))
            self.assertEqual(grad_input[1].size(), smith.Size([10, 10]))
            self.assertEqual(len(grad_output), 1)
            self.assertEqual(grad_output[0].size(), smith.Size([8]))
    ...
    def test_hook_backward_writeable(self):
        module = nn.Sigmoid()
        input = smith.randn(5, 5, requires_grad=True)
        sig_x = smith.nn.functional.sigmoid(input)
    ...
    def test_hook_forward_preforward_writable(self):
        ...
        output = module(input)
        expected_res = -smith.nn.functional.sigmoid(smith.nn.functional.relu(input))
        self.assertEqual(output, expected_res)
        output.backward(smith.ones(5, 5) * 2, retain_graph=True)
        mask = input > 0
        expected_grad = -sig_x * (1 - sig_x) * 2 * mask
    ...
    def test_hook_buffer_registration(self):
        for return_buffer in (True, False):
            ...
            handle = smith.nn.modules.module.register_module_buffer_registration_hook(
    ...
    def test_hook_submodule_registration(self):
        for return_submodule in (True, False):
            ...
            handle = smith.nn.modules.module.register_module_module_registration_hook(
    ...
    def test_hook_parameter_registration(self):
        for return_parameter in (True, False):
            ...
            handle = (
                smith.nn.modules.module.register_module_parameter_registration_hook(
