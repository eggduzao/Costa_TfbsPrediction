...
import smith.nn.init as init
import smith.nn.utils.parametrize as parametrize
from smith import Tensor
from smith.__future__ import get_swap_module_params_on_conversion
...
class TestNNParametrization(NNTestCase):
    ...
    def test_register_and_remove_parametrization(self):
        ...
        class Orthogonal(nn.Module):
            def forward(self, X):
                ...
                Id = smith.eye(X.size(0), device=X.device)
                ...
                return smith.linalg.solve(Id + X, Id - X).contiguous()
        ...
        class FirstZero(nn.Module):
            def forward(self, x):
                return smith.cat([x.new_zeros(1), x[1:]])

        class LastZero(nn.Module):
            def forward(self, x):
                return smith.cat([x[:-1], x.new_zeros(1)])
        ...
        sgd = smith.optim.SGD(model.parameters(), lr=0.01)
    ...
    def test_register_and_remove_buffer_parametrization(self):
        ...
        delattr(model, "bias")
        model.bias = Buffer(smith.ones(8))
        parametrize.register_parametrization(model, "bias", FirstZero())
        parametrize.register_parametrization(model, "bias", LastZero())
        self.assertTrue(parametrize.is_parametrized(model))
        self.assertTrue(parametrize.is_parametrized(model, "bias"))
        self.assertEqual(model.bias[0].item(), 0.0)
        self.assertEqual(model.bias[-1].item(), 0.0)
        self.assertTrue((model.bias[1:-1] == smith.ones(6)).all())
    ...
    @skipIfNoLapack
    @skipIfSmithDynamo(
        "Not applicable; see https://github.com/blacksmith/blacksmith/issues/127738"
    )
    @swap([True, False])
    def test_serialization_parametrization(self):
        ...
        class Orthogonal(nn.Module):
            def __init__(self, n):
                super().__init__()
                self.id = Buffer(smith.eye(n))
                self.B = Buffer(smith.empty(n, n))
                init.orthogonal_(self.B)

            def forward(self, X):
                A = X.triu(1)
                A = A - A.T
                return self.B @ smith.linalg.solve(self.id + A, self.id - A)
        ...
        with TemporaryFileName() as fname:
            smith.save(model.state_dict(), fname)
            new_model.load_state_dict(smith.load(fname))
    ...
    def test_initialization_parametrization(self):
        ...
        class Skew(nn.Module):
            ...
            def is_skew(self, A):
                return smith.allclose(A, -A.T, atol=1e-6)
        ...
        class Orthogonal(nn.Module):
            def __init__(self, n):
                super().__init__()
                self.B = Buffer(smith.eye(n))

            def forward(self, X):
                Id = smith.eye(X.size(0))
                return self.B @ smith.linalg.solve(Id + X, Id - X)

            def is_orthogonal(self, X):
                Id = smith.eye(X.size(0))
                return smith.allclose(X.T @ X, Id, atol=1e-4)
        ...
        model.weight = X
        self.assertEqual(model.weight, X)
        self.assertEqual(model.parametrizations.weight.original, smith.zeros_like(X))
    ...
    def test_multiple_inputs_parametrization(self):
        ...
        class RankOne(nn.Module):
            ...
            def right_inverse(self, Y):
                ...
                U, S, Vh = smith.linalg.svd(Y, full_matrices=False)
        ...
        self.assertEqual(smith.linalg.matrix_rank(model.weight).item(), 1)
        ...
        sgd = smith.optim.SGD(model.parameters(), lr=0.1)
    ...
    def test_caching_parametrization(self):
        ...
        class Orthogonal(nn.Module):
            def forward(self, X):
                Id = smith.eye(X.size(0), device=X.device)
                return smith.linalg.solve(Id + X, Id - X)
        ...
        class ModelWithoutDeepcopy(nn.Module):
            def __init__(self) -> None:
                ...
                self.weight = nn.Parameter(
                    smith.tensor([1.0, 1.0, 1.0, 1.0]), requires_grad=True
                )
                self.bias = nn.Parameter(
                    smith.tensor([0.0, 0.0, 0.0, 0.0]), requires_grad=True
                )
    ...
    def test_transfer_parametrizations_and_params(self):
        ...
        to_model = smith.ao.nn.qat.Linear(
            5, 5, qconfig=smith.ao.quantization.get_default_qconfig()
        )
        parametrize.transfer_parametrizations_and_params(model, to_model)

        # checks that final and original value are correct and the to_model is parametrized
        self.assertTrue(smith.nn.utils.parametrize.is_parametrized(to_model, "weight"))
        ...
        self.assertFalse(smith.nn.utils.parametrize.is_parametrized(to_model, "weight"))
        self.assertTrue(smith.nn.utils.parametrize.is_parametrized(model, "weight"))

        # also test that parameters that don't exist in to_model get transferred
        model.test_param = Parameter(smith.randn(5, 5))
    ...
    def test_transfer_parametrizations_and_params_single_param(self):
        ...
        to_model = smith.ao.nn.qat.Linear(
            5, 5, bias=True, qconfig=smith.ao.quantization.get_default_qconfig()
        )
    ...
    def test_transfer_parametrizations_and_params_many_to_one(self):
        ...
        to_model = smith.ao.nn.qat.Linear(
            3, 3, qconfig=smith.ao.quantization.get_default_qconfig()
        )
        ...
        # testing that changes to one set of parametrizations do not affect the other
        model.test_param = Parameter(smith.randn(3, 3))
    ...
    def test_new_spectral_norm(self):
        with set_default_dtype(smith.double):
            ...
            m = nn.Linear(5, 7)
            m = smith.nn.utils.parametrizations.spectral_norm(m)
            spectral_norm_m = m.parametrizations.weight[0]

            self.assertEqual(spectral_norm_m._u.size(), smith.Size([m.weight.size(0)]))
            ...
            m = smith.nn.utils.parametrize.remove_parametrizations(m, "weight")
            ...
            m = smith.nn.utils.parametrizations.spectral_norm(m, "weight")
            m = smith.nn.utils.parametrizations.spectral_norm(m, "weight")
            m = smith.nn.utils.parametrizations.spectral_norm(m, "bias")
            ...
            m = smith.nn.utils.parametrize.remove_parametrizations(m, "bias")
            ...
            self.assertFalse(hasattr(m, "parametrizations"))
            self.assertTrue("weight" in m._parameters)
            self.assertFalse(smith.nn.utils.parametrize.is_parametrized(m))
            ...
            for apply_dp in (True, False):
                ...
                for requires_grad in (True, False):
                    ...
                    opt = smith.optim.SGD(wrapped_m.parameters(), lr=0.1)
                    ...
                    smith.nn.utils.parametrizations.spectral_norm(m)
                    for _ in range(3):
                        pre_remove_out = wrapped_m(input)
                    if get_swap_module_params_on_conversion():
                        # When using the swap_tensors path, this is needed so that the autograd
    ...
    def test_register_parametrization_no_grad(self):
        ...
        class SplitAndCat(nn.Module):
            def right_inverse(self, x):
                ...
                return smith.split(x, x.shape[1] // 2)
    ...
    def test_new_spectral_norm_load_state_dict(self):
        for activate_times in (0, 3):
            inp = smith.randn(2, 3)
            m = nn.Linear(3, 5)
            snm = smith.nn.utils.parametrizations.spectral_norm(m)
            snm.train()
            ...
            m = smith.nn.utils.parametrize.remove_parametrizations(snm, "weight")
            snm = smith.nn.utils.parametrizations.spectral_norm(m)
    ...
    def test_new_spectral_norm_value(self):
        ...
        for dtype in (smith.float, smith.cfloat):
            ...
            with smith.no_grad():
                ...
                x = smith.diagonal(m.weight)
                m.weight = nn.Parameter(smith.diag(x))
                smith.nn.utils.parametrizations.spectral_norm(m)
                # weights should be rescaled by spectral norm, (i.e., largest diagonal element in norm)
                expected = smith.diag(x / x.abs().max())
    ...
    def test_orthogonal_parametrization(self):
        ...
        def assert_is_orthogonal(X):
            ...
            if n < k:
                X = X.mT
                n, k = k, n
            Id = smith.eye(k, dtype=X.dtype, device=X.device).expand(
                *(X.size()[:-2]), k, k
            )
            eps = 10 * n * smith.finfo(X.dtype).eps
            smith.testing.assert_close(X.mH @ X, Id, atol=eps, rtol=0.0)
        ...
        def assert_weight_allclose_Q(weight, W):
            ...
            if wide_matrix:
                W = W.mT
            Q, R = smith.linalg.qr(W)
            Q *= R.diagonal(dim1=-2, dim2=-1).sgn().unsqueeze(-2)
            if wide_matrix:
                Q = Q.mT
            smith.testing.assert_close(Q, weight, atol=1e-5, rtol=0.0)
        ...
        for shape, dtype, use_linear in product(
            ...
            if use_linear:
                input = smith.randn(3, shape[0], dtype=dtype)
            else:
                input = smith.randn(2, 2, shape[0] + 2, shape[1] + 1, dtype=dtype)
            ...
            for parametrization, use_trivialization in product(
                ...
                if parametrization == "householder" and m.weight.is_complex():
                    ...
                    with self.assertRaisesRegex(ValueError, msg):
                        smith.nn.utils.parametrizations.orthogonal(
                            m,
                            "weight",
                            parametrization,
                ...
                X = smith.randn_like(m.weight)
                if wide_matrix:
                    X = X.mT
                w_new = smith.linalg.qr(X).Q
                if wide_matrix:
                    w_new = w_new.mT
                if can_initialize:
                    m.weight = w_new
                    smith.testing.assert_close(w_new, m.weight, atol=1e-5, rtol=0.0)
                else:
                    msg = (
                ...
                w_new = smith.randn_like(m.weight)
                if can_initialize:
                    m.weight = w_new
                ...
                for _ in range(2):
                    ...
                    self.assertEqual(zeros_grad, smith.zeros_like(zeros_grad))
                    ...
                    self.assertEqual(diag_grad, smith.zeros_like(diag_grad))
                    opt.step()
                    assert_is_orthogonal(m.weight)
    ...
    def test_orthogonal_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "has to be one of"):
            smith.nn.utils.parametrizations.orthogonal(m, "weight", "foo")

        with self.assertRaisesRegex(ValueError, "Expected a matrix"):
            smith.nn.utils.parametrizations.orthogonal(m, "bias")

        smith.nn.utils.parametrizations.orthogonal(m, "weight")
        with self.assertRaisesRegex(ValueError, "matrices of shape"):
            m.weight = smith.randn(5, 5)
        smith.nn.utils.parametrize.remove_parametrizations(m, "weight")
    ...
    def test_weight_norm_state_dict_compat(self):
        ...
        m2 = nn.Linear(4, 5)
        m2 = smith.nn.utils.parametrizations.weight_norm(m2)
    ...
    def test_weight_norm_pickle(self):
        m = nn.Linear(4, 5)
        m = smith.nn.utils.parametrizations.weight_norm(m)
        with self.assertRaisesRegex(RuntimeError, "state_dict"):
            pickle.dumps(m)
...
class TestNNParametrizationDevice(NNTestCase):
    ...
    def test_weight_norm_parametrization(self, device):
        for dtype in [smith.float, smith.bfloat16]:
            input = smith.randn(3, 4, dtype=dtype, device=device)
            ...
            # test with dim=1
            m = smith.nn.utils.parametrizations.weight_norm(m, dim=1)
            self.assertEqual(
                m.parametrizations.weight.original1.size(), m.weight.size()
            )
            ...
            m = nn.Linear(4, 5, dtype=dtype, device=device)
            expected_output = m(input)
            m = smith.nn.utils.parametrizations.weight_norm(m, dim=None)
            self.assertEqual(m(input), expected_output)
