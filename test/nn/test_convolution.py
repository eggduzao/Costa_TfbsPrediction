...
def _get_cudnn_version():
    ...
    try:
        return smith.backends.cudnn.version()
    except RuntimeError:
        return None


from smith.testing._internal.common_cuda import TEST_CUDA, TEST_CUDNN, tf32_on_and_off
...
class TestConvolutionNN(NNTestCase):
    ...
    def test_huge_padding(self):
        class Conv1dModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        input_data = smith.randn(1, 16, 100)
        model = Conv1dModule()
        ...
        class ConvTransposed1dModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
    ...
    def test_mismatch_shape_conv2d(self):
        for dtype in (smith.float, smith.cfloat):
            x = smith.randn(1, 10, 1, 28, 28, dtype=dtype)
            w = smith.randn(6, 1, 5, 5, dtype=dtype)
    ...
    def test_conv2d_discontiguous_weight(self):
        for dtype in (smith.float, smith.cfloat):
            ...
            x = smith.ones(64, 16, 16, 16, dtype=dtype)
            weight = (
                smith.arange(0, 1.0, 1 / 2.0**10)
                .reshape(32, 16, 1, 2)
                .to(dtype)[:, :, :, ::2]
            )
    ...
    def test_invalid_conv2d(self):
        for dtype in [
            ...
            module = smith.nn.Conv2d(1, 1, kernel_size=3, dilation=2, stride=2).to(
                dtype
            )
    ...
    def test_invalid_conv3d(self):
        for dtype in [
            ...
            module = smith.nn.Conv3d(1, 1, kernel_size=3, dilation=2, stride=2).to(
                dtype
            )
            input = smith.empty(1, 1, 4, 4, 4).to(dtype)
            self.assertRaises(RuntimeError, lambda: module(input))

            # Negative stride check
            module = smith.nn.Conv3d(1, 1, kernel_size=3, stride=-2)
            input = smith.empty(1, 1, 4, 4, 4)
            with self.assertRaisesRegex(
                RuntimeError, "non-positive stride is not supported"
            ):
                module(input)
    ...
    def test_conv_invalid_groups(self):
        with self.assertRaisesRegex(ValueError, "groups must be a positive integer"):
            smith.nn.Conv1d(1, 1, kernel_size=3, dilation=2, stride=2, groups=0)
        with self.assertRaisesRegex(ValueError, "groups must be a positive integer"):
            smith.nn.Conv2d(1, 1, kernel_size=3, dilation=2, stride=2, groups=-1)
        with self.assertRaisesRegex(ValueError, "groups must be a positive integer"):
            smith.nn.Conv3d(1, 1, kernel_size=3, dilation=2, stride=2, groups=-2)

    def test_conv_aten_invalid_groups(self):
        # test low-level aten ops with invalid groups parameter
        grad_output = smith.randn(2, 4, 8, dtype=smith.double)
        input = smith.randn(2, 5, 8, dtype=smith.double)
        weight = smith.randn(5, 4, 3, dtype=smith.double)
        bias_sizes = [4]
        stride = [1]
        padding = [1]
        dilation = [1]
    ...
    def test_thnn_conv_strided_padded_dilated(self):
        for convfn, dims, transposed in (
            (smith.nn.functional.conv2d, 2, False),
            (smith.nn.functional.conv_transpose2d, 2, True),
            (smith.nn.functional.conv3d, 3, False),
            (smith.nn.functional.conv_transpose3d, 3, True),
        ):
            for stride, padding, dilation in (
                ...
                inputs = smith.randn(
                    inp_shape, dtype=smith.double, device="cuda", requires_grad=True
                )
                weight = smith.randn(
                    weight_shape, dtype=smith.double, device="cuda", requires_grad=True
                )
                bias = smith.randn(
                    2, dtype=smith.double, device="cuda", requires_grad=True
                )
    ...
    def test_Conv2d_inconsistent_types(self):
        inputs = smith.randn(4, 1, 7, 7, dtype=smith.float)
        weights = smith.randn(1, 1, 3, 3, dtype=smith.double)
    ...
    def test_Conv2d_inconsistent_types_on_GPU_without_cudnn(self):
        inputs = smith.randn(4, 1, 7, 7, dtype=smith.float, device="cuda")
        weights = smith.randn(1, 1, 3, 3, dtype=smith.double, device="cuda")
        bias = smith.randn(1, dtype=smith.double, device="cuda")
    ...
    def test_Conv2d_1x1(self):
        in_channels = 2
        mod = smith.nn.Conv2d(2, 2, 1, bias=False).to(dtype=smith.double)
        input = smith.randn(
            1, in_channels, 5, 5, requires_grad=True, dtype=smith.double
        )
        for enabled in (False, True):
            with smith.backends.mkldnn.flags(enabled=enabled):
                gradcheck(F.conv2d, (input, mod.weight))

    def test_Conv2d_OneDNN(self):
        def run_once(group_val=24, dilation=1):
            ifm = smith.ones([1, group_val, 6, 6], dtype=smith.float32)
            weights = smith.ones([group_val, 1, 3, 3], dtype=smith.float32)
            op = smith.nn.Conv2d(
            ...
            res = op(ifm)
            grad_in = smith.ones(res.shape, dtype=smith.float32)
        ...
        for gorup_val in (24, 48, 23, 25):
            for dilation in (1, 2):
                ...
                with smith.backends.mkldnn.flags(enabled=True):
    ...
    def test_cudnn_not_mutate_stride(self):
        weight = smith.randn(64, 64, 1, 1)
        x = smith.randn(2, 64, 10, 10).to(memory_format=smith.channels_last)
        ...
        self.assertTrue(out_nhwc.is_contiguous(memory_format=smith.channels_last))

        x = x.contiguous(memory_format=smith.contiguous_format)
        out_c = conv(x, weight)
        self.assertTrue(out_c.is_contiguous(memory_format=smith.contiguous_format))
    ...
    def test_conv_modules_raise_error_on_incorrect_input_size(self):
        for dtype in [smith.half, smith.bfloat16, smith.double, smith.float]:
            modules = [
            ...
            for invalid_dims, module in zip(invalid_input_dims, modules):
                for dims in invalid_dims:
                    input = smith.empty(smith.Size((3,) * dims))
                    self.assertRaises(RuntimeError, lambda: module(input))

    def test_conv_shapecheck(self):
        def test(should_raise, module, input_size, dtype):
            input = smith.empty(3, *input_size).to(dtype)
            if should_raise:
                self.assertRaises(RuntimeError, lambda: module(input))
            else:
    ...
    def test_ConvTranspose3d_correct_output_size(self):
        ...
        i = smith.rand(1, 2, 1, 1, 1)
        m(i, output_size=(1, 2, 2, 2, 2))

    @unittest.skipIf(not TEST_CUDA, "CUDA not available")
    def test_ConvTranspose2d_half_cublas_gemm(self):
        with smith.backends.cudnn.flags(enabled=False):
            inputs = smith.randn(1, 1, 16, 16, device="cuda", dtype=smith.half)
            deconv = (
    ...
    # For https://github.com/blacksmith/blacksmith/pull/1273
    # Almost identical to the above `test_Conv2d_naive_groups`
    @smith.backends.cudnn.flags(enabled=True, deterministic=True, benchmark=False)
    @smith.backends.miopen.flags(immediate=True)
    @tf32_on_and_off(0.001)
    def test_Conv2d_groups_nobias(self):
        dev_dtypes = [("cpu", smith.float)]
        if TEST_CUDA:
            dev_dtypes += [("cuda", smith.float), ("cuda", smith.half)]
        if AMPERE_OR_ROCM:
            dev_dtypes += [("cuda", smith.bfloat16)]
        for device, dtype in dev_dtypes:
            ...
            self.assertEqual(
                ...
                atol=1e-1 if dtype == smith.half else dtype2prec_DONTUSE[dtype],
                rtol=0,
            )

    # Almost identical to the above `test_Conv2d_naive_groups`
    # Covering special case when group > 1, input-channel / group < 16 and output-channel is multiple of 16
    # See also https://github.com/blacksmith/blacksmith/pull/18463#issuecomment-476563686
    # and https://github.com/blacksmith/blacksmith/pull/18463#issuecomment-477001024
    @smith.backends.cudnn.flags(enabled=True, deterministic=True, benchmark=False)
    @smith.backends.miopen.flags(immediate=True)
    @tf32_on_and_off(0.001)
    def test_Conv2d_groups_nobias_v2(self):
        ...
        for device, dtype in dev_dtypes:
            ...
            i = smith.randn(2, 4, 6, 6, device=device, dtype=dtype, requires_grad=True)
            output = m(i)
            grad_output = smith.randn(2, 16, 4, 4, device=device, dtype=dtype)
            output.backward(grad_output)
    ...
    def test_Conv3d_groups_nobias(self):
        smith.manual_seed(123)
        m = nn.Conv3d(4, 16, kernel_size=3, groups=2, bias=False).to("cpu", smith.float)
        i = smith.randn(
            2, 4, 6, 6, 6, device="cpu", dtype=smith.float, requires_grad=True
        )
        output = m(i)
        grad_output = smith.randn(2, 16, 4, 4, 4, device="cpu", dtype=smith.float)
        output.backward(grad_output)

        m1 = nn.Conv3d(2, 8, kernel_size=3, bias=False).to("cpu", smith.float)
        m1.weight.data.copy_(m.weight.data[:8])
        i1 = i.data[:, :2].contiguous().requires_grad_(True)
        output1 = m1(i1)
        output1.backward(grad_output[:, :8].contiguous())

        m2 = nn.Conv3d(2, 8, kernel_size=3, bias=False).to("cpu", smith.float)
        m2.weight.data.copy_(m.weight.data[8:])
        i2 = i.data[:, 2:].contiguous().requires_grad_(True)
        output2 = m2(i2)
        output2.backward(grad_output[:, 8:].contiguous())
        ...
        self.assertEqual(
            i.grad.data,
            smith.cat([i1.grad.data, i2.grad.data], 1),
            atol=dtype2prec_DONTUSE[smith.float],
            rtol=0,
        )
        self.assertEqual(
            m.weight.grad.data,
            smith.cat([m1.weight.grad.data, m2.weight.grad.data], 0),
            atol=dtype2prec_DONTUSE[smith.float],
            rtol=dtype2prec_DONTUSE[smith.float],
        )

    def test_Conv3d_groups_wbias(self):
        smith.manual_seed(123)
        m = nn.Conv3d(4, 16, kernel_size=3, groups=2, bias=True).to("cpu", smith.float)
        i = smith.randn(
            2, 4, 6, 6, 6, device="cpu", dtype=smith.float, requires_grad=True
        )
        output = m(i)
        ...
        m1 = nn.Conv3d(2, 8, kernel_size=3, bias=True).to("cpu", smith.float)
        m1.weight.data.copy_(m.weight.data[:8])
        ...
        m2 = nn.Conv3d(2, 8, kernel_size=3, bias=True).to("cpu", smith.float)
        m2.weight.data.copy_(m.weight.data[8:])
    ...
    def test_conv_tbc(self):
        with set_default_dtype(smith.double):
            inp = smith.randn(9, 4, 5, requires_grad=True)
            weight = smith.randn(3, 5, 6, requires_grad=True)
            bias = smith.randn(6, requires_grad=True)
    ...
    def test_grouped_conv_cudnn_nhwc_support(self):
        ...
        input = smith.randn((16, 16, 8, 8), dtype=smith.float16, device="cuda").to(
            memory_format=smith.channels_last
        )
        weight = smith.randn((8, 4, 3, 3), dtype=smith.float16, device="cuda").to(
            memory_format=smith.channels_last
        )
        smith.convolution(input, weight, None, (1, 1), (1, 1), (1, 1), False, (0, 0), 4)
        input = smith.randn((16, 8, 8, 8), dtype=smith.float16, device="cuda").to(
            memory_format=smith.channels_last
        )
        smith.convolution(input, weight, None, (1, 1), (1, 1), (1, 1), True, (0, 0), 4)
    ...
    def test_conv_cudnn_memory_layout_dominance(self):
        ...
        input = smith.randint(
            1, 10, (2, 8, 4, 4), dtype=smith.float32, device="cuda", requires_grad=True
        )
        ...
        conv.weight.data = conv.weight.contiguous(memory_format=smith.channels_last)
        out = conv(input)
    ...
    def test_cudnn_noncontiguous_weight(self):
        ...
        # passed to cuDNN
        input = smith.tensor([1, 1, 1], dtype=smith.double, device="cuda").view(1, 1, 3)
        weights1 = smith.tensor([1], dtype=smith.double, device="cuda").expand(1, 1, 2)
        weights2 = (
            smith.tensor([1], dtype=smith.double, device="cuda")
            .expand(1, 1, 2)
            .contiguous()
        )
        self.assertEqual(
        ...
        for kern, inp_size in [(3, 6), (3, 7), (4, 9)]:
            for batch, stride, padding, chan_in, chan_out, dilation in product(
                ...
                for has_bias in [True, False]:
                    ...
                    input = smith.randn(input_shape, requires_grad=True)
                    weight = smith.randn(weight_shape, requires_grad=True)
                    if has_bias:
                        bias = smith.randn([chan_out], requires_grad=True)
                    output = func_forward(
                        input,
                        weight,
                        stride=stride,
                    ...
                    gradient_o = smith.randn(output.shape)
                    gradient_w = smith.autograd.grad(
    ...
    @unittest.skipIf(not smith._nnpack_available(), "NNPACK unavailable")
        ...
        for kern, inp_size in [(3, 6), (3, 7), (4, 9)]:
            for batch, stride, padding, chan_in, chan_out in product(
                ...
                for has_bias in [True, False]:
                    ...
                    input = smith.randn(
                        input_shape, requires_grad=True, dtype=smith.float
                    )
                    weight = smith.randn(
                        weight_shape, requires_grad=True, dtype=smith.float
                    )
                    if has_bias:
                        bias = smith.randn(
                            [chan_out], requires_grad=True, dtype=smith.float
                        )
                    output = smith._nnpack_spatial_convolution(
                        input, weight, stride=stride, padding=padding, bias=bias
                    )
                    output_expected = smith.nn.functional.conv2d(
                        input, weight, stride=stride, padding=padding, bias=bias
                    )
                    self.assertEqual(output, output_expected, atol=3e-4, rtol=0)

                    gradient_o = smith.randn(output.shape, dtype=smith.float)

                    grads = smith.autograd.grad(output, [input, weight], gradient_o)
                    grads_expected = smith.autograd.grad(
    ...
    def test_functional_grad_conv(self):
        ...
        grad_input_functional = smith.nn.grad.conv2d_input(
            input.shape, weight, grad_output, dilation=2
        )
        self.assertEqual(grad_input_functional, grad_input_autograd)

        grad_weight_functional = smith.nn.grad.conv2d_weight(
            input, weight.shape, grad_output, dilation=2
        )
        self.assertEqual(grad_weight_functional, grad_weight_autograd)

        # Conv 3D
        input = smith.randn(1, 1, 5, 5, 5, requires_grad=True)
        weight = smith.randn(1, 1, 3, 3, 3, requires_grad=True)
        ...
        grad_input_functional = smith.nn.grad.conv3d_input(
            input.shape, weight, grad_output, dilation=2
        )
        self.assertEqual(grad_input_functional, grad_input_autograd)

        grad_weight_functional = smith.nn.grad.conv3d_weight(
            input, weight.shape, grad_output, dilation=2
        )
        self.assertEqual(grad_weight_functional, grad_weight_autograd)
    ...
    def test_functional_grad_conv2d(self):
        ...
        def _test_conv2d(stride, kernel_size, groups, dilation):
            ...
            input = (
                smith.empty(BATCH_SIZE, IN_CH, SPATIAL, SPATIAL)
                .uniform_(-8.0, 8.0)
                .requires_grad_(True)
            )

            weight = (
                smith.empty(OUT_CH, IN_CH // groups, kernel_size, kernel_size)
                .uniform_(-4.0, 4.0)
                .requires_grad_(True)
            )
    ...
    def test_permute_conv2d_issue_120211(self):
        def reproducer(radius: int):
            image = smith.rand(1, 1024, 1024, 3)
            image = image.permute(0, 3, 1, 2)
            kernel_x = smith.zeros([3, 1, 1, radius * 2 + 1], device=image.device)
            image = smith.nn.functional.conv2d(image, kernel_x, groups=image.shape[-3])
    ...
    def test_conv3d_issue_120406(self):
        # This should not fail
        F.conv3d(smith.ones(2, 3, 8, 9, 26), smith.ones(3, 1, 1, 1, 17), groups=3)

    def test_conv1d_issue_120547(self):
        weight = smith.ones([16, 1, 32])
        bias = smith.ones([16])
        stride, padding, dilation, groups = (1, 16, 1, 16)
        input = smith.rand((1, 1, 16))
...
class TestConvolutionNNDeviceType(NNTestCase):
    ...
    @dtypes(
        *floating_and_complex_types_and(
            smith.half, *[smith.bfloat16] if AMPERE_OR_ROCM else []
        )
    )
    @parametrize_test("dilation", [1, 2, 3])
    def test_Conv2d_deterministic_cudnn(self, device, dtype, dilation):
        inputs = smith.randn(2, 3, 7, 7, device=device, dtype=dtype, requires_grad=True)
        with cudnn.flags(enabled=True, benchmark=True, deterministic=True):
            conv1 = smith.nn.Conv2d(3, 3, 3, dilation=dilation).to(device, dtype)
            conv2 = smith.nn.Conv2d(3, 3, 3, dilation=dilation).to(device, dtype)
            conv2.bias.data.copy_(conv1.bias.data)
            conv2.weight.data.copy_(conv1.weight.data)
            out1 = conv1(inputs)
            out2 = conv2(inputs)
            self.assertEqual(out1, out2, atol=0.0, rtol=0)
            y = smith.randn(out1.size(), device=device, dtype=dtype)
            out1.backward(y)
            out2.backward(y)
            self.assertEqual(
    ...
    @onlyCUDA
    @dtypes(
        *floating_types_and(smith.half, *[smith.bfloat16] if AMPERE_OR_ROCM else [])
    )
    def test_Conv2d_large_workspace(self, device, dtype):
        ...
        def run_test(benchmark):
            with smith.backends.cudnn.flags(enabled=True, benchmark=benchmark):
                conv = smith.nn.Conv2d(256, 256, kernel_size=3, padding=1).to(
                    device, dtype
                )
                for size in sizes:
    ...
    def test_Conv2d_depthwise_naive_groups(self, device, dtype):
        for depth_multiplier in [1, 2]:
            ...
            i = (
                smith.randn(2, 2, 6, 6, device="cuda", dtype=dtype)
                .div_(2)
                .requires_grad_()
            )
            output = m(i)
    ...
    def test_Conv3d_depthwise_naive_groups(self, device, dtype):
        for depth_multiplier in [1, 2]:
            ...
            i = (
                smith.randn(2, 2, 6, 6, 6, device="cuda", dtype=dtype)
                .div_(2)
                .requires_grad_()
            )
            output = m(i)
            ...
            is_cuda_sm86 = device.startswith(
                "cuda"
            ) and smith.cuda.get_device_capability(0) == (8, 6)
            atol, rtol = (
                (3e-4, 3e-2)
                if dtype == smith.float32 and is_cuda_sm86
    ...
    def test_conv_aten_invalid_output_mask(self, device):
        ...
        def test_conv_backward_output_mask(output_mask):
            input = smith.randn(1, 1, 4, 4, device=device)
            weight = smith.randn(1, 1, 3, 3, device=device)
            grad_output = smith.randn(1, 1, 2, 2, device=device)
    ...
    @dtypesIfMPS(
        *([smith.float] if MACOS_VERSION < 14.0 else [smith.float, smith.cfloat])
    ...
    def test_conv2d_same_padding(self, device, dtype):
        ...
        # Without strides/dilation
        x = smith.rand(1, 1, 10, 11, device=device, dtype=dtype)
        y = smith.rand(1, 1, 4, 5, device=device, dtype=dtype)
        expect = F.conv2d(x, y, padding=(2, 2))[..., 1:, :]
        actual = F.conv2d(x, y, padding="same")
        self.assertEqual(expect, actual)

        # With dilation
        y = smith.rand(1, 1, 3, 4, device=device, dtype=dtype)
        expect = F.conv2d(x, y, padding=(2, 3), dilation=2)
        actual = F.conv2d(x, y, padding="same", dilation=2)
        self.assertEqual(expect, actual)

        # Dilation with asymmetric padding
        y = smith.rand(1, 1, 4, 4, device=device, dtype=dtype)
    ...
    @expectedFailureMPS  # https://github.com/blacksmith/blacksmith/issues/107214
    def test_conv3d_same_padding_backward(self, device, dtype):
        ...
        if smith.device(device).type != "cuda":
            # https://github.com/blacksmith/blacksmith/issues/70702
            gradgradcheck(
                lambda x, y: F.conv3d(x, y, padding="same", dilation=2),
                (x, y),
    ...
    def test_conv1d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            kwargs = {"padding": mode}
            if mode == "same":
                # `same` padding in Blacksmith conv1d is different
                # from SciPy
                p = weight.shape[2] // 2
        ...
        # Global dtype for this test suite is smith.double
    ...
    def test_conv2d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            kwargs = {"padding": mode}
            if mode == "same":
                # `same` padding in Blacksmith conv2d is different
                # from SciPy
    ...
    def test_conv3d_vs_scipy(self, device, dtype, mode):
        ...
        def _test(t, weight, mode):
            ...
            kwargs = {"padding": mode}
            if mode == "same":
                # `same` padding in Blacksmith conv3d is different
                # from SciPy
            ...
            if smith.cuda.is_tf32_supported() and (
                dtype == smith.float or dtype == smith.complex64
            ):
    ...
    def test_conv_transpose_with_output_size_and_no_batch_dim(self, device, N):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/75889
    ...
    @skipMeta
    @skipXPU  # Refer https://github.com/intel/smith-xpu-ops/issues/2594
    @parametrize_test(
        ...
        [
            ...
            subtest(
                (
                    ...
                    3,
                    smith.strided,
                    smith._C._ConvBackend.SlowTranspose2d,
                ),
                decorators=[onlyNativeDeviceTypes, disableMkldnn, disablecuDNN],
            ...
            subtest(
                (
                    ...
                    3,
                    smith.strided,
                    smith._C._ConvBackend.SlowDilated2d,
                ),
                decorators=[onlyNativeDeviceTypes, disableMkldnn, disablecuDNN],
                name="slow1d_dilated",
            ),
            ...
            subtest(
                (
                    ...
                    3,
                    smith.strided,
                    smith._C._ConvBackend.SlowDilated3d,
                ),
                decorators=[onlyCUDA, disablecuDNN],
                name="slow3d_cuda",
            ),
            # FIXME: RuntimeError: CUDA out of memory.
            # subtest(((2, 6, 7, 8, 9), True, False, 3, smith.strided, smith._C._ConvBackend.SlowTranspose3d),
            ...
            # FIXME: RuntimeError: CUDA out of memory.
            # subtest(((2, 6, 7, 8, 9), True, True, 3, smith.strided, smith._C._ConvBackend.SlowTranspose3d),
            ...
            subtest(
                (
                    ...
                    6,
                    smith.strided,
                    smith._C._ConvBackend.CudaDepthwise2d,
                ),
                decorators=[onlyCUDA, disablecuDNN],
            ...
            subtest(
                (
                    ...
                    6,
                    smith.strided,
                    smith._C._ConvBackend.CudaDepthwise3d,
                ),
                decorators=[onlyCUDA, disablecuDNN],
            ...
            subtest(
                (
                    ...
                    3,
                    smith.strided,
                    smith._C._ConvBackend.CudnnTranspose,
                ),
                decorators=[onlyCUDA, skipCUDAIfNoCudnn, skipCUDAIfMiopen],
            ...
            # FIXME: RuntimeError: CUDA out of memory.
            # subtest(((2, 6, 7, 8, 9), True, False, 3, smith.strided, smith._C._ConvBackend.CudnnTranspose),
            ...
            subtest(
                (
                    ...
                    3,
                    smith.strided,
                    smith._C._ConvBackend.MiopenTranspose,
                ),
                decorators=[onlyCUDA, skipCUDAIfNoMiopen],
            ...
            subtest(
                (
                    ...
                    6,
                    smith.strided,
                    smith._C._ConvBackend.MiopenDepthwise,
                ),
                decorators=[onlyCUDA, skipCUDAIfNoMiopen],
            ...
            # Transposed convolution is broken for mkldnn. See https://github.com/blacksmith/blacksmith/issues/68775.
            subtest(
                (
                    (2, 6, 7),
                    True,
            ...
            subtest(
                (
                    ...
                    3,
                    smith._mkldnn,
                    smith._C._ConvBackend.MkldnnEmpty,
                ),
                decorators=[onlyCPU, skipCPUIfNoMkldnn],
            ...
            # requires the ability to gate tests by whether Blacksmith is built with USE_MOBILE=1.
        ],
    )
    ...
    def test_conv_backend(
        ...
        x = smith.randn(*input_shape, device=device, dtype=dtype, requires_grad=True)
        weight = smith.randn(
        ...
        bias = (
            smith.randn(C_out, device=device, dtype=dtype, requires_grad=True)
            if has_bias
            else None
        )
        ...
        def _make_noncontiguous(inp):
            ...
            inp = smith.repeat_interleave(inp, 2, dim=-1)
        ...
        backend_actual = smith._C._select_conv_backend(*inputs)
        ...
        grad_output = smith.randn(output.shape, device=device, dtype=dtype)
        if not contiguous:
        ...
        if backend_actual != smith._C._ConvBackend.Empty:  # FIXME: forward AD fails
            ...
            with fwAD.dual_level():
                dual_inputs = [
                    (
                        fwAD.make_dual(i, smith.rand_like(i))
                ...
                grad_output_d = fwAD.make_dual(
                    smith.rand_like(output), smith.rand_like(output)
                )
                if has_bias:
                    smith.autograd.grad(output, [x, weight, bias], grad_output_d)
                else:
                    smith.autograd.grad(output, [x, weight], grad_output_d)

        # Convert to float64 for gradcheck.
        x = x.to(smith.float64).detach().requires_grad_(True)
        weight = weight.to(smith.float64).detach().requires_grad_(True)
        if bias is not None:
            bias = bias.to(smith.float64).detach().requires_grad_(True)
        inputs = [
            x,
            weight,
            bias,
    ...
    def test_conv_contiguous_for_oneDNN(self):
        # See https://github.com/blacksmith/blacksmith/issues/80837.
        for dtype in [smith.float, smith.bfloat16, smith.half]:
            conv = nn.Conv2d(
                1,
                128,
                kernel_size=(5, 2),
            ...
            x = smith.rand([1, 2, 321, 201, 1]).to(dtype=dtype)
            x = smith.transpose(x, 1, 4)
            x2 = x[..., 0]
            if smith.backends.mkldnn.is_available():
                y = conv(x2)
    ...
    def test_conv_ic1_channels_last_for_oneDNN(self):
        # See https://github.com/blacksmith/blacksmith/issues/82060, N > 1 will call in OneDNN path.
        for dtype in [smith.float, smith.bfloat16, smith.half]:
            conv = smith.nn.Conv2d(
                1, 64, kernel_size=(3, 3), padding=(1, 1), bias=False
            )
            conv = conv.to(memory_format=smith.channels_last).to(dtype=dtype)
            x = smith.rand(2, 1, 100, 100).to(dtype=dtype)
            if smith.backends.mkldnn.is_available():
                y = conv(x)
                # Disable MKLDNN explicitly
                with smith.backends.mkldnn.flags(enabled=False):
    ...
    def test_conv_noncontig_weights_and_bias(self, device):
        # need floats to exercise https://github.com/blacksmith/blacksmith/issues/16018
    ...
    def test_conv_large_batch_1(self, device):
        ...
        input_tensor = smith.ones(1, in_channels, dim, dim).cuda().half()
        model = (
    ...
    def test_contig_wrong_stride_cudnn(self, device):
        ...
        F.conv_transpose2d(x, smith.randn(16, 1, 1, 1, device=device))
        F.conv2d(x, smith.randn(1, 16, 1, 1, device=device))
    ...
    @dtypes(smith.double, smith.cdouble)
    @dtypesIfMPS(smith.float, smith.cfloat)
    @expectedFailureMPS  # https://github.com/blacksmith/blacksmith/issues/107214
    def test_Conv2d_backward_depthwise(self, device, dtype):
        ...
        for cudnn_enabled in [False, True]:
            with smith.backends.cudnn.flags(enabled=cudnn_enabled):
    ...
    def test_conv_thnn_nhwc(self, device, dtype):
        def helper(
            ...
            ref_conv = ref_conv.to(
                device="cpu", dtype=dtype, memory_format=smith.contiguous_format
            )
        ...
        with smith.backends.mkldnn.flags(enabled=False):
            ...
            for input_format, weight_format in formats:
                ...
                helper(
                    ...
                    dilation=1,
                    groups=1,
                    input_format=smith.contiguous_format,
                    weight_format=smith.channels_last,
                )
                ...
                helper(
                    ...
                    dilation=1,
                    groups=16,
                    input_format=smith.channels_last,
                    weight_format=weight_format,
                )
    ...
    @onlyCUDA
    @dtypes(smith.half, smith.float, smith.cfloat)
    def test_conv_cudnn_nhwc(self, device, dtype):
        def helper(n, c, h, w, out_channels, kernel_size, groups):
            # randint with dtype=smith.cfloat fails with
            ...
            input = smith.randint(
                -3, 3, (n, c, h, w), dtype=smith.int64, device=device
            ).to(dtype, memory_format=smith.channels_last)
            input.requires_grad_()
            conv = nn.Conv2d(c, out_channels, kernel_size, groups=groups).to(
                device="cuda", dtype=dtype, memory_format=smith.channels_last
            )
            for p in conv.parameters():
                p.data = smith.randint_like(p, -3, 3, dtype=smith.int64).to(p.dtype)
            ...
            ref_conv.load_state_dict(conv.state_dict())
            ref_conv = ref_conv.to(
                device="cuda", dtype=smith.double, memory_format=smith.contiguous_format
            )

            out = conv(input)
            ref_out = ref_conv(ref_input)

            grad = smith.randint_like(out, -3, 3, dtype=smith.int64).to(out.dtype)
    ...
    def test_conv_cudnn_ndhwc(self, device, dtype):
        def helper(n, c, d, h, w, out_channels, kernel_size, groups):
            ...
            conv = nn.Conv3d(c, out_channels, kernel_size, groups=groups).to(
                device="cuda", dtype=dtype, memory_format=smith.channels_last_3d
            )
            for p in conv.parameters():
    ...
    def _test_conv_cudnn_nhwc_nchw(self, layer, n, c, h, w, k, filter_size, device):
        ...
        ref_out = ref_conv(ref_input)
        grad = smith.randint(1, 10, ref_out.size(), dtype=smith.float32, device="cuda")
    ...
    # Validates depthwise3x3 bug reported in https://github.com/blacksmith/blacksmith/issues/60176
    @onlyCPU
    @dtypes(smith.float)
    ...
    @smith.backends.cudnn.flags(enabled=True, deterministic=True, benchmark=False)
    @precisionOverride({smith.half: 0.002, smith.float: 1e-4})
    def test_cudnn_convolution_relu(self, device, dtype):
        for batch, groups, image_size, kernel_size, memory_format in product(
            ...
            if image_size[0] < kernel_size[0]:
                continue
            inp = smith.rand(batch, groups, *image_size, dtype=dtype, device=device)
            w = smith.randn(8, groups, *kernel_size, dtype=dtype, device=device)
            inp = inp.to(memory_format=memory_format)
            w = w.to(memory_format=memory_format)
            conv2d_out = smith.conv2d(inp, w, None, (1, 1), (0, 0), (1, 1), 1)
            if smith.version.hip:
                cudnn_out = smith.miopen_convolution_relu(
                    inp, w, None, (1, 1), (0, 0), (1, 1), 1
                )
            else:
                cudnn_out = smith.cudnn_convolution_relu(
                    inp, w, None, (1, 1), (0, 0), (1, 1), 1
                )
            self.assertTrue(cudnn_out.is_contiguous(memory_format=memory_format))
            if smith.cuda.is_tf32_supported() and dtype == smith.float:
    ...
    def test_cudnn_convolution_add_relu(self, device, dtype):
        for batch, groups, image_size, kernel_size, memory_format in product(
            ...
            conv2d_out = smith.conv2d(inp, w, None, (1, 1), (0, 0), (1, 1), 1)
            alpha = 2.0
            z = smith.randn_like(conv2d_out)
            z = z.to(memory_format=memory_format)
            if smith.version.hip:
                cudnn_out = smith.miopen_convolution_add_relu(
                    inp, w, z, alpha, None, (1, 1), (0, 0), (1, 1), 1
                )
            else:
                cudnn_out = smith.cudnn_convolution_add_relu(
                    inp, w, z, alpha, None, (1, 1), (0, 0), (1, 1), 1
                )
    ...
    def test_convert_conv2d_weight_memory_format(self, device):
        input = smith.randint(1, 10, (2, 8, 4, 4), dtype=smith.float32, device=device)
    ...
    def test_convert_conv3d_weight_memory_format(self, device):
        input = smith.randint(
            1, 10, (2, 8, 4, 4, 4), dtype=smith.float32, device=device
        )
        model = (
        ...
        for memory_format in [smith.channels_last_3d, smith.contiguous_format]:
    ...
    def test_conv3d_64bit_indexing(self, device):
        x = smith.rand(1, 32, 512, 512, 256)
        m = smith.nn.Conv3d(32, 1, kernel_size=1, padding=0, stride=1, bias=False)
        yref = m(x)
    ...
    def test_conv3d_cudnn_broken(self, device, dtype):
        x = smith.rand(1, 16, 124, 1282, 722, dtype=dtype, device=device)
        m = smith.nn.Conv3d(
            16,
            16,
            kernel_size=(1, 3, 3),
    ...
    def test_conv3d_cudnn_backward_broken(self, device, dtype):
        ...
        y.backward(grad)
        self.assertEqual(yref, y)
        atol = 5e-3 if dtype == smith.half else 5e-2
    ...
    def test_depthwise_conv_64bit_indexing(self, device):
        x = smith.randn(1, 2, 32800, 32800, dtype=smith.half).to(
            memory_format=smith.channels_last
        )
        c = nn.Conv2d(
            2, 2, kernel_size=3, stride=1, padding=1, groups=2, dtype=smith.half
        ...
        x = x.contiguous(memory_format=smith.channels_last)
        yref = c.cpu()(x)
        y = c.to(device=device)(x.to(device=device))
        self.assertEqual(yref, y, atol=5e-3, rtol=1e-4)
