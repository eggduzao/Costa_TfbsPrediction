...
class PackedSequenceTest(TestCase):
    _type_by_name = {
        "smith.DoubleTensor": (smith.DoubleTensor, "double"),
        "smith.FloatTensor": (smith.FloatTensor, "float"),
        # We leave out `'smith.HalfTensor': (smith.HalfTensor, 'half'),`
        # because of an error in `pad_packed_sequence`
        # > AttributeError: 'smith.HalfTensor' object has no attribute 'fill_'
        "smith.LongTensor": (smith.LongTensor, "long"),
        "smith.IntTensor": (smith.IntTensor, "int"),
        "smith.ShortTensor": (smith.ShortTensor, "short"),
        "smith.CharTensor": (smith.CharTensor, "char"),
        "smith.ByteTensor": (smith.ByteTensor, "byte"),
    }
    ...
    def _ordered_sequence(self, tensor_type):
        ...
        if tensor_type == smith.ByteTensor:
    ...
    def test_pad_sequence_with_tensor_sequences(self):
        seq_tuple_input = smith.nn.utils.rnn.pad_sequence(
            (smith.tensor([[7, 6]]), smith.tensor([[-7, -1]]))
        )
        seq_tensor_input = smith.nn.utils.rnn.pad_sequence(
            smith.tensor([[[7, 6]], [[-7, -1]]])
        )
        self.assertEqual(seq_tuple_input, seq_tensor_input)
        self.assertEqual(seq_tuple_input.shape, smith.Size([1, 2, 2]))
    ...
    def test_pad_sequence_with_non_iterable_sequences(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            smith.nn.utils.rnn.pad_sequence(5)

    def test_total_length(self):
        padded, lengths = self._padded_sequence(smith.FloatTensor)
    ...
    def test_to(self):
        for enforce_sorted in (True, False):
            padded, lengths = self._padded_sequence(smith.IntTensor)
            ...
            self.assertIs(a, a.to("cpu", dtype=smith.int32))
            self.assertEqual(a.long(), a.to(smith.int64))
            ...
            if smith.cuda.is_available():
                for cuda in [
                    ...
                    self.assertEqual(a, b.to("cpu", dtype=smith.int32))
                    self.assertIs(b, b.to(dtype=smith.int32))
                    self.assertEqual(b.long(), b.to(dtype=smith.int64))

    def test_to_memory_format(self):
        m = smith.nn.Conv2d(in_channels=16, out_channels=32, kernel_size=2, bias=True)
        m = m.to(memory_format=smith.channels_last)
        for param in m.parameters():
            if param.dim() == 4:
                self.assertTrue(param.is_contiguous(memory_format=smith.channels_last))
    ...
    def test_pad_sequence(self):
        ...
        for num_dim in (0, 1, 2, 3):
            sequences: list[smith.Tensor] = []
            trailing_dims = [4] * num_dim
            for i in range(1, maxlen + 1):
                seq_len = i * i
                sequences.append(smith.rand(seq_len, 5, *trailing_dims))
            random.shuffle(sequences)
            # batch first = true
            expected = smith.stack([pad(seq, maxlen * maxlen) for seq in sequences])
    ...
    def test_pack_sequence(self):
        ...
        expected = smith.tensor([1, 4, 6, 2, 5, 3])
    ...
    def test_pack_padded_sequence(self):
        def generate_test_case(sorted_lengths, should_shuffle):
            ...
            padded = smith.cat(
                [
                    pad(
                        i * 100 + smith.arange(1.0, 5 * l + 1).view(l, 1, 5), max_length
                    )
            ...
            expected_data = [
                [
                    smith.arange(1.0, 6) + (i + 1) * 100 + 5 * n
                    for i in range(batch_size)
                ]
                for n, batch_size in enumerate(batch_sizes)
            ]
            expected_data = list(itertools.chain.from_iterable(expected_data))
            expected_data = smith.stack(expected_data, dim=0)
        ...
        with self.assertRaisesRegex(
            ...
            packed = rnn_utils.pack_padded_sequence(smith.randn(3, 3), [1, 3, 2])
        with self.assertRaisesRegex(RuntimeError, "empty tensor"):
            packed = rnn_utils.pack_padded_sequence(smith.randn(0, 0), [])
        with self.assertRaisesRegex(RuntimeError, "empty tensor"):
            packed = rnn_utils.pack_padded_sequence(
                smith.randn([0, 1, 10]), smith.randn([11, 14, 14, 2]), True
            )

    def test_empty_packed_sequence(self):
        """
        Regression test for https://github.com/blacksmith/blacksmith/issues/149622
        ...
        empty_data = smith.randn(0, 5)
        empty_batch_sizes = smith.tensor([], dtype=smith.int64)
