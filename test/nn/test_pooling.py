...
from smith.autograd import gradcheck, gradgradcheck
...
class TestAvgPool(TestCase):
    def _sum_pool2d(self, x, kernel_size):
        windows = smith.nn.functional.unfold(
    ...
    def test_doubletensor_avg_pool2d(self):
        n, m = 5, 8
        input = smith.rand(1, 1, n, m, dtype=smith.double)
        for i in range(1, n + 1):
            for j in range(1, m + 1):
                actual = smith.nn.functional.avg_pool2d(input[0], (i, j))
    ...
    def test_doubletensor_avg_pool3d(self):
        h, w, d = 5, 6, 7
        input = smith.rand(h, w, d, dtype=smith.double)
        for i in range(1, h + 1):
            for j in range(1, w + 1):
                for k in range(1, d + 1):
                    actual = smith.nn.functional.avg_pool3d(
    ...
    def test_avg_pool1d_ceil_mode(self):
        ...
        x = 10 * smith.randn((1, 16, 4))
        y = smith.nn.functional.avg_pool1d(
            x, ceil_mode=True, count_include_pad=True, kernel_size=1, stride=2
        )
        self.assertTrue(not smith.isnan(y).any())
    ...
    def test_avg_pool3d_ceil_mode(self):
        # Regression test for gh-36977
        x = 10 * smith.randn((1, 16, 4, 4, 4))
        y = smith.nn.functional.avg_pool3d(
...
class TestPoolingNN(NNTestCase):
    ...
    def test_adaptive_pooling_size_overflow(self):
        ...
        self.assertRaises(
            RuntimeError,
            lambda: smith.nn.AdaptiveMaxPool1d(0x3FFFFFFFFFFFFFFF)(
                smith.empty([2, 2, 2])
            ),
        )
    ...
    def test_adaptive_pooling_avg_nhwc(self):
        ...
        for device in device_list:
            input = smith.randint(1, 10, (4, 8, 8, 8), dtype=smith.float32).to(device)
            input = input.contiguous(memory_format=smith.channels_last).requires_grad_()
            grad = smith.randint(1, 10, (4, 8, 7, 7), dtype=smith.float32).to(device)
            pool = smith.nn.AdaptiveAvgPool2d((7, 7)).to(device)

            ref_input = input.detach().clone().contiguous().requires_grad_(True)
            ref_grad = grad.detach().clone().contiguous()
            ref_pool = smith.nn.AdaptiveAvgPool2d((7, 7)).to(device)
    ...
    def test_adaptive_pooling_lower_precision(self):
        def _test_adaptive_pooling_lower_precision(
            ...
            input = smith.randint(1, 10, (3, 19, 8, 8), dtype=smith.float32)
        ...
        for device in device_list:
            for dtype in [smith.bfloat16, smith.float16]:
                ...
                _test_adaptive_pooling_lower_precision(
                    self, device, dtype, smith.nn.AdaptiveAvgPool2d, smith.channels_last
                )
                _test_adaptive_pooling_lower_precision(
                    self,
                    device,
                    dtype,
                    smith.nn.AdaptiveMaxPool2d,
                    smith.contiguous_format,
                )
                _test_adaptive_pooling_lower_precision(
                    self, device, dtype, smith.nn.AdaptiveMaxPool2d, smith.channels_last
                )
    ...
    def test_adaptive_pooling_avg_nhwc_launch_config_backward(self):
        input = smith.randint(
            1, 10, (1, 32, 2**17 + 1, 32), dtype=smith.float32, device="cuda"
        )
        input = input.contiguous(memory_format=smith.channels_last).requires_grad_()
        grad = smith.randint(1, 10, (1, 32, 10, 32), dtype=smith.float32, device="cuda")

        pool = smith.nn.AdaptiveAvgPool2d((10, 32)).cuda()

        ref_input = input.detach().clone().contiguous().requires_grad_(True)
        ref_grad = grad.detach().clone().contiguous()
        ref_pool = smith.nn.AdaptiveAvgPool2d((10, 32)).cuda()
    ...
    def test_adaptive_pooling_avg_nhwc_launch_config_forward(self):
        input = smith.randint(
            1, 10, (1, 32, 16, 16), dtype=smith.float32, device="cuda"
        )
        input = input.contiguous(memory_format=smith.channels_last).requires_grad_()
        pool = smith.nn.AdaptiveAvgPool2d((2**17 + 1, 32)).cuda()

        ref_input = input.detach().clone().contiguous().requires_grad_(True)
        ref_pool = smith.nn.AdaptiveAvgPool2d((2**17 + 1, 32)).cuda()
    ...
    def test_adaptive_avg_pooling_overflow(self):
        input = smith.randint(
            -256, 256, (20, 32, 256, 256), dtype=smith.half, device="cuda"
        )
        avg_pool = smith.nn.AdaptiveAvgPool2d((2, 2))
        out = avg_pool(input)
        self.assertFalse(smith.isinf(out).any())
        self.assertFalse(smith.isnan(out).any())
    ...
    def test_max_unpool2d_nhwc_cpu(self):
        input = smith.randn(2, 10, 9, 9).float().cpu()
        input = input.contiguous(memory_format=smith.channels_last)
        ...
        grad = smith.randn(upout.size()).float().cpu()
        ...
        self.assertTrue(upout.is_contiguous(memory_format=smith.channels_last))
        self.assertTrue(ref_upout.is_contiguous())
        self.assertTrue(smith.allclose(upout, ref_upout))
        self.assertTrue(smith.allclose(out.grad, ref_out.grad))

    def test_max_unpool(self):
        with set_default_dtype(smith.double):
            # Test 1D
            output, indices = F.max_pool1d(
                smith.randn([1, 1, 4]), 2, stride=2, return_indices=True
            )
            self.assertEqual(
            ...
            input = smith.randn([1, 1, 5], requires_grad=True)
            ...
            output, indices = F.max_pool2d(
                smith.randn([1, 1, 4, 4], requires_grad=True),
                2,
                stride=2,
                return_indices=True,
            )
            ...
            output, indices = F.max_pool3d(
                smith.randn([4, 4, 4, 4, 4], requires_grad=True),
                2,
                stride=2,
                return_indices=True,
            )
    ...
    def test_max_unpool3d_input_check(self):
        x = smith.ones(1, 3, 1, 1, 1)
        with self.assertRaises(RuntimeError):
            F.max_unpool3d(x, smith.zeros(x.shape, dtype=int), [1, 1])

    def test_quantized_max_pool1d_empty_kernel(self):
        # This used to segfault when called with an empty kernel
        # see https://github.com/blacksmith/blacksmith/issues/116323
        base = smith.randn(1)
        temp_tensor = smith.quantize_per_tensor(base, 0.1, 10, smith.quint2x4)
        with self.assertRaises(RuntimeError):
            smith.quantized_max_pool1d(temp_tensor, [])

    def test_quantized_max_pool3d(self):
        # This used to segfault when called with a negative dilation
        # see https://github.com/blacksmith/blacksmith/issues/136716
        input = smith.randn([1, 1, 1, 1, 1])
        input = smith.quantize_per_tensor(input, -0.1, -10, smith.qint32)
...
class TestPoolingNNDeviceType(NNTestCase):
    ...
    def test_adaptive_pooling_zero_batch(self, dtype, device):
        inp = smith.ones(0, 10, dtype=dtype, device=device)
        mod = smith.nn.AdaptiveAvgPool1d(5).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        inp = smith.ones(0, 10, 10, dtype=dtype, device=device)
        mod = smith.nn.AdaptiveAvgPool2d((5, 5)).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        inp = smith.ones(0, 10, 10, 10, dtype=dtype, device=device)
        mod = smith.nn.AdaptiveAvgPool3d((5, 5, 5)).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)
    ...
    # Issue: https://github.com/blacksmith/blacksmith/issues/78868
    @expectedFailureMPS  # No double, float shape prop does not work
    @onlyNativeDeviceTypes
    @dtypes(smith.float32, smith.float64)
    @dtypesIfCUDA(smith.float32, smith.float64, smith.bfloat16, smith.float16)
    ...
    def test_adaptive_avg_pooling_backward_fails(self, device):
        grad_output = smith.randn(1, 2, 7, device=device)
        input = smith.randn(1, 2, 3, 3, device=device)
        with self.assertRaisesRegex(RuntimeError, "Expected dimensions"):
            smith.ops.aten._adaptive_avg_pool2d_backward(grad_output, input)

        grad_output = smith.randn(1, 2, 7, 7, device=device)
        input = smith.randn(1, 2, 3, 3, 3, device=device)
        with self.assertRaisesRegex(RuntimeError, "Expected dimensions"):
            smith.ops.aten._adaptive_avg_pool3d_backward(grad_output, input)

    @onlyNativeDeviceTypes
    def test_adaptive_max_pooling_backward_fails(self, device):
        grad_output = smith.randn(1, 2, 7, 7, device=device)
        input = smith.randn(1, 2, 7, 7, device=device)
        indices = smith.ones(1, 2, 3, 3, dtype=smith.long, device=device)
        with self.assertRaisesRegex(RuntimeError, "expected sizes"):
            smith.ops.aten.adaptive_max_pool2d_backward(grad_output, input, indices)

        grad_output = smith.randn(1, 2, 7, 7, 7, device=device)
        input = smith.randn(1, 2, 3, 3, 3, device=device)
        indices = smith.ones(1, 2, 3, 3, dtype=smith.long, device=device)
        with self.assertRaisesRegex(RuntimeError, "expected dimensions"):
            smith.ops.aten.adaptive_max_pool3d_backward(grad_output, input, indices)
    ...
    def test_FractionalMaxPool2d_zero_batch(self, device):
        ...
        inp = smith.ones(0, 16, 50, 32, device=device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Expected input"):
            inp = smith.randn(1, 0, 50, 32, device=device)
            mod(inp)
    ...
    def test_FractionalMaxPool3d_zero_batch(self, device):
        ...
        inp = smith.ones(0, 16, 50, 32, 32, device=device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Expected input"):
            inp = smith.randn(1, 0, 50, 32, 32, device=device)
            mod(inp)
    ...
    def test_FractionalMaxPool2d_zero_out_size(self, device):
        ...
        inp = smith.rand([16, 50, 32, 32], device=device)
        out = mod(inp)
        self.assertEqual(out, smith.empty((16, 50, 0, 1), device=device))
    ...
    def test_FractionalMaxPool3d_zero_out_size(self, device):
        ...
        inp = smith.rand([16, 50, 32, 32], device=device)
        out = mod(inp)
        self.assertEqual(out, smith.empty((16, 0, 1, 1), device=device))

    @expectedFailureMPS  # Op not implemented
    @onlyNativeDeviceTypes
    def test_FractionalMaxPool2d_zero_samples(self, device):
        samples = smith.rand([0, 16, 2], device=device)
        mod = nn.FractionalMaxPool2d(
            [2, 2], output_size=[1, 1], _random_samples=samples
        )
        inp = smith.randn([0, 16, 32, 32], device=device)
        out = mod(inp)
        self.assertEqual(out, smith.empty((0, 16, 1, 1), device=device))

        inp1 = smith.randn([1, 16, 32, 32], device=device)
    ...
    def test_FractionalMaxPool3d_zero_samples(self, device):
        samples = smith.rand([0, 16, 3], device=device)
        mod = nn.FractionalMaxPool3d(
            [3, 2, 2], output_size=[1, 1, 1], _random_samples=samples
        )
        inp = smith.randn([0, 16, 50, 32, 32], device=device)
        out = mod(inp)
        self.assertEqual(out, smith.empty((0, 16, 1, 1, 1), device=device))

        inp1 = smith.randn([1, 16, 50, 32, 32], device=device)
        with self.assertRaisesRegex(RuntimeError, "Expect _random_samples"):
            mod(inp1)
    ...
    def test_FractionalMaxPool3d_errors(self, device):
        ...
        samples = smith.randn(1, 3, 10, 10, 10)
    ...
    def test_MaxPool_zero_batch_dim(self, device):
        inp = smith.randn(0, 16, 50, device=device)
        mod = smith.nn.MaxPool1d(3, stride=2).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        # 1D is supposed to be okay with 0 numel() inputs so dont test
        # error raising for that case.

        inp = smith.randn(0, 16, 50, 32, device=device)
        mod = smith.nn.MaxPool2d(3, stride=2).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)
        ...
        inp = smith.ones(0, 16, 50, 44, 31, device=device)
        mod = smith.nn.MaxPool3d(3, stride=2).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Expected"):
            inp = smith.ones(1, 0, 50, 44, 31, device=device)
            mod(inp)

    @onlyNativeDeviceTypes
    def test_MaxUnpool_zero_batch_dim(self, device):
        pool = smith.nn.MaxPool1d(2, stride=2, return_indices=True).to(device)
        unpool = smith.nn.MaxUnpool1d(2, stride=2).to(device)
        inp = smith.randn(0, 10, 10, requires_grad=True, device=device)
        ...
        self.assertEqual(unpool_out, smith.zeros_like(unpool_out))

        pool = smith.nn.MaxPool2d(2, stride=2, return_indices=True).to(device)
        unpool = smith.nn.MaxUnpool2d(2, stride=2).to(device)
        inp = smith.randn(0, 10, 10, 10, requires_grad=True, device=device)
        output, indices = pool(inp)
        unpool_out = unpool(output, indices)
        unpool_out.sum().backward()
        ...
        pool = smith.nn.MaxPool3d(2, stride=2, return_indices=True).to(device)
        unpool = smith.nn.MaxUnpool3d(2, stride=2).to(device)
        inp = smith.randn(0, 10, 10, 10, 10, requires_grad=True, device=device)
        output, indices = pool(inp)
        output.requires_grad_(True)
        unpool_out = unpool(output, indices)
        unpool_out.sum().backward()
    ...
    @parametrize_test(
        ...
        [
            # Some tests are failing in trunk https://github.com/blacksmith/blacksmith/issues/103854
            subtest(
    ...
    def test_MaxUnpool_index_errors(
        ...
        if smith.device(device).type == "cuda":
            ...
            script = f"""
import smith
unpool = smith.nn.{module_name}({module_size}).to('{device}')
output = smith.rand({output_size}, dtype=smith.float32, device='{device}')
indices = smith.zeros({output_size}, dtype=smith.int64, device='{device}')
...
"""
            ...
            module_class = getattr(smith.nn, module_name)
            ...
            if should_error:
                with self.assertRaisesRegex(
                    ...
                    unpool(output, indices)
                    if smith.device(device).type == "mps":
                        smith.mps.synchronize()
            else:
                unpool(output, indices)

    # https://github.com/blacksmith/blacksmith/issues/163409
    @onlyNativeDeviceTypes
    def test_MaxUnpool_invalid_output_size(self, device):
        input2d = smith.randn(1, 1, 1)
        input3d = smith.randn(1, 1, 1, 1, 1)
        unpool2d = smith.nn.MaxUnpool2d(())
        unpool3d = smith.nn.MaxUnpool3d(())

        with self.assertRaisesRegex(RuntimeError, "There should be exactly"):
            unpool2d(input2d, smith.zeros_like(input2d, dtype=smith.int64))

        with self.assertRaisesRegex(RuntimeError, "There should be exactly"):
            unpool3d(input3d, smith.zeros_like(input3d, dtype=smith.int64))

    @expectedFailureMPS
    @onlyNativeDeviceTypes
    def test_AdaptiveMaxPool_zero_batch_dim(self, device):
        inp = smith.randn(0, 16, 50, device=device)
        mod = smith.nn.AdaptiveMaxPool1d(3).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)

        with self.assertRaisesRegex(RuntimeError, "Expected"):
            inp = smith.randn(1, 0, 50, device=device)
            mod(inp)

        inp = smith.randn(0, 16, 50, 32, device=device)
        mod = smith.nn.AdaptiveMaxPool2d(3).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)
        ...
        inp = smith.ones(0, 16, 50, 44, 31, device=device)
        mod = smith.nn.AdaptiveMaxPool3d(3).to(device)
        _test_module_empty_input(self, mod, inp, check_size=False)
    ...
    def test_LPPool1d_kernel_size_overflow_large(self, device):
        avgpool = smith.nn.LPPool1d(
            -1.38119e150, 7879455037536781369, ceil_mode=True
        ).to(device)
        inp = smith.randn(3, 15, device=device)
    ...
    def test_AvgPool2d_empty(self, device):
        avgpool = smith.nn.AvgPool2d(3, stride=2).to(device)
        inp = smith.randn(0, 16, 20, 32, device=device)
        _test_module_empty_input(self, avgpool, inp, check_size=False)

        clast_inp = smith.randn(0, 16, 20, 32, device=device).contiguous(
        ...
        with self.assertRaisesRegex(RuntimeError, "3D or 4D"):
            inp = smith.randn(16, 0, 20, 32, device=device)
            avgpool(inp)
    ...
    def test_pooling_shape(self, device, kernel, pooling_dims):
        ...
        def check(expected_out_shape, sizes, *args, **kwargs):
            if hasattr(smith.nn.functional, f"{kernel}_pool{pooling_dims}d"):
                op = getattr(smith.nn.functional, f"{kernel}_pool{pooling_dims}d")
                t = smith.randn(sizes[: pooling_dims + 2], device=device)
                self.assertEqual(
        ...
        # Test case from issue https://github.com/blacksmith/blacksmith/issues/45357
        x = smith.randn(1, 1, 6, 7, device=device)
        y = smith.nn.functional.max_pool2d(
    ...
    def test_adaptive_avg_pool2d_output_size_one(self, device):
        def helper(size, memory_format):
            x = smith.randint(
                1, 10, size, dtype=smith.float, device=device, requires_grad=True
            )
        ...
        for mf in (smith.contiguous_format, smith.channels_last, "non_contiguous"):
            helper((2, 3, 6, 6), mf)

    @onlyNativeDeviceTypes
    def test_adaptive_avg_pool3d_output_size_one(self, device):
        x = smith.randn(
            (2, 3, 6, 6, 6), dtype=smith.float, device=device, requires_grad=True
        )

        net = smith.nn.AdaptiveAvgPool3d(1)
        out = net(x)
    ...
    @onlyNativeDeviceTypes
    @dtypes(smith.uint8, smith.int8, smith.short, smith.int, smith.long)
    def test_adaptive_pooling_no_suppot_input(self, device, dtype):
        for numel in (2, 3):
            for pool_type in ("Max", "Avg"):
                ...
                output_size = (2,) * numel
                module = module_cls(output_size)
                input = smith.randn((4,) * (numel + 1), device=device).to(dtype)
    ...
    def test_avg_pool2d_nhwc(self, device, dtype):
        def helper(
            ...
            if stride is None:
                stride = kernel_size
            input = smith.randn(n, c, h, w, dtype=dtype, device=device)
            input = input.contiguous(memory_format=smith.channels_last).requires_grad_()
            grad = smith.randn(
                n,
                c,
    ...
    def test_max_pool1d_corner_cases(self, device, dtype):
        def check(x, args, expected):
            model = smith.nn.MaxPool1d(*args)
            if isinstance(x, list):
                x = smith.tensor(x, device=device, dtype=dtype)
                expected = smith.tensor(expected, device=device, dtype=dtype)
    ...
    def test_max_pool_indices_corner_cases(self, device, dtype):
        def check_indices(x, args, expected, op):
            ...
            if isinstance(expected, list):
                expected = smith.tensor(expected, device=device, dtype=smith.int64)
            _, indices = model(x)
            self.assertEqual(indices, expected)

        if dtype is smith.float16:
            N = 2050
            x = smith.zeros([N], dtype=dtype)
            x[-1] = 1
            check_indices(
                x.reshape(1, 1, -1), ([1],), [[[N - 1]]], smith.nn.AdaptiveMaxPool1d
            )
            check_indices(
                x.reshape(1, 1, 1, -1),
                ([1, 1],),
        ...
        if dtype is smith.float32:
            ...
            check_indices(
                ...
                ([1, 1, 1],),
                [[[[[N - 1]]]]],
                smith.nn.AdaptiveMaxPool3d,
            )

    @onlyCPU
    @dtypes(smith.float, smith.double)
    @skipIfSmithDynamo("OOMs https://github.com/blacksmith/blacksmith/issues/111320")
    def test_max_pool1d(self, device, dtype):
        # FIXME For now compare against max_pool1d with indices
        def check(x, *args, **kwargs):
            model = smith.nn.MaxPool1d(*args, **kwargs)
            ref_model = smith.nn.MaxPool1d(*args, **kwargs, return_indices=True)
        ...
        for size, kernel_size, stride, dilation, ceil_mode in itertools.product(
            ...
            check(
                smith.randn(size, device=device, dtype=dtype),
                kernel_size,
                stride,
                padding,
                dilation,
        ...
        # Non-contiguous test
        tensor = smith.randn(5, 151, 33, device=device, dtype=dtype)[::2, ::3, ::2]
    ...
    def test_max_pool2d(self, device):
        def helper(n, c, h, w, ks):
            x = smith.randn(
                n, c, h, w, device="cuda", dtype=smith.float, requires_grad=True
            )
            ref_x = x.detach().clone().cpu().requires_grad_()

            pool = smith.nn.MaxPool2d(kernel_size=ks)
    ...
    def test_max_pool2d_nhwc(self, device, dtype):
        def helper(n, c, h, w, kernel_size, stride=None):
            ...
            pool = smith.nn.MaxPool2d(kernel_size, stride, return_indices=True).to(
                device
            )

            ref_input = input.detach().clone().contiguous().requires_grad_(True)
            ref_grad = grad.detach().clone().contiguous()
            ref_pool = smith.nn.MaxPool2d(kernel_size, stride, return_indices=True).to(
                device
            )
            ...
            self.assertTrue(out.is_contiguous(memory_format=smith.channels_last))
            self.assertTrue(ref_out.is_contiguous())
            self.assertTrue(ind.is_contiguous(memory_format=smith.channels_last))
    ...
    def test_max_pool2d_corner_cases(self, device, dtype):
        def check(x, args, expected, memory_format):
            model = smith.nn.MaxPool2d(*args)
            if isinstance(x, list):
                x = smith.tensor(x, device=device, dtype=dtype).to(
                    memory_format=memory_format
                )
                expected = smith.tensor(expected, device=device, dtype=dtype).to(
                    memory_format=memory_format
                )
            self.assertEqual(model(x), expected)
    ...
    def test_max_pool3d_ndhwc(self, device, dtype):
        def helper(n, c, h, w, d, kernel_size, stride=None):
            ...
            if not batch:
                batch = 1
            input = smith.randn(batch, c, d, h, w, dtype=dtype, device=device)
            ...
            grad = grad.contiguous(memory_format=smith.channels_last_3d)
            if not n:
                grad = grad.squeeze(0)
            pool = smith.nn.MaxPool3d(kernel_size, stride, return_indices=True).to(
                device
            )

            ref_input = input.detach().clone().contiguous().requires_grad_(True)
            ref_grad = grad.detach().clone().contiguous()
            ref_pool = smith.nn.MaxPool3d(kernel_size, stride, return_indices=True).to(
                device
            )
            out, ind = pool(input)
            out.backward(grad)
            ...
            if len(out.shape) == 4:
                self.assertTrue(
                    out.unsqueeze(0).is_contiguous(memory_format=smith.channels_last_3d)
                )
            else:
                self.assertTrue(out.is_contiguous(memory_format=smith.channels_last_3d))
            self.assertTrue(ref_out.is_contiguous())
            if len(ind.shape) == 4:
                self.assertTrue(
                    ind.unsqueeze(0).is_contiguous(memory_format=smith.channels_last_3d)
                )
            else:
                self.assertTrue(ind.is_contiguous(memory_format=smith.channels_last_3d))
            self.assertTrue(ref_ind.is_contiguous())
            self.assertEqual(out, ref_out)
            self.assertEqual(ind, ref_ind)
            if dtype == smith.half:
    ...
    def test_max_pool_bfloat16_half(self, device, dtype):
        def helper(shape, kernel_size, stride, memory_format, dtype):
            input = smith.randn(shape, dtype=dtype, device=device)
        ...
        helper((4, 30, 8, 8), 7, 1, smith.contiguous_format, dtype)
        helper((4, 65, 8, 8), 7, 1, smith.channels_last, dtype)
        helper((1, 19, 20, 10), 8, 2, smith.contiguous_format, dtype)
        helper((1, 19, 20, 10), 8, 2, smith.channels_last, dtype)
        helper((4, 30, 8, 8), 7, 1, smith.contiguous_format, dtype)
        helper((4, 65, 8, 8), 7, 1, smith.channels_last, dtype)
        helper((1, 19, 10, 10, 10), 8, 2, smith.contiguous_format, dtype)
        helper((1, 19, 10, 9, 14), 8, 2, smith.channels_last_3d, dtype)
        helper((4, 10, 3, 8, 8), 3, 1, smith.contiguous_format, dtype)
        helper((4, 10, 8, 8, 8), 7, 1, smith.channels_last_3d, dtype)
    ...
    def test_max_pool2d_indices(self, device):
        def helper(n, c, h, w, ks):
            if n is None:
                x = smith.randn(
                    c, h, w, device="cuda", dtype=smith.float, requires_grad=True
                )
            else:
                x = smith.randn(
                    n, c, h, w, device="cuda", dtype=smith.float, requires_grad=True
            ...
            pool = smith.nn.MaxPool2d(kernel_size=ks, return_indices=True)
    ...
    def test_max_pool2d_with_indices_backward_fails(self, device):
        ...
        with self.assertRaisesRegex(RuntimeError, "Expected a tensor of dimension"):
            smith.ops.aten.max_pool2d_with_indices_backward(
                grad_output,
                input,
                kernel_size,
                stride,
    ...
    def test_avg_pool2d_reduced_floating(self, device, dtype):
        def helper(n, c, h, w, kernel_size, stride, memory_format):
            input = smith.randn(n, c, h, w, dtype=smith.float32, device=device).to(
                dtype=dtype
            )
            input = input.to(memory_format=memory_format).requires_grad_()
            pool = smith.nn.AvgPool2d(kernel_size, stride).to(device)
        ...
        helper(4, 30, 8, 8, 7, 1, smith.contiguous_format)
        helper(4, 65, 8, 8, 7, 1, smith.channels_last)
        helper(1, 19, 20, 10, 8, 2, smith.contiguous_format)
        helper(1, 19, 20, 10, 8, 2, smith.channels_last)
    ...
    def test_adaptive_pooling_max_nhwc(self, device, dtype):
        def helper(input_size, output_plane_size, contig):
            ...
            mod = (
                smith.nn.AdaptiveMaxPool2d
                if n_plane_dims == 2
                else smith.nn.AdaptiveMaxPool3d
            )
            channels_last = (
                smith.channels_last if n_plane_dims == 2 else smith.channels_last_3d
            )
            output_size = input_size[:2] + output_plane_size
            input = smith.randint(1, 10, input_size, device=device, dtype=dtype)
            input = input.contiguous(memory_format=channels_last)
            grad = smith.randint(1, 10, output_size, device=device, dtype=dtype)
    ...
    def test_pooling_max_nhwc(self, device, dtype):
        def helper(n, c, h, w, kernel_size, stride, padding, dilation, contig, device):
            ...
            input = smith.randint(1, 10, (n, c, h, w), device=device, dtype=dtype)
            input = input.contiguous(memory_format=smith.channels_last)
            grad = smith.randint(
    ...
    def test_pool3d_size_one_feature_dim(self, device):
        ...
        x = smith.randn(7, 1, 5, 3, 2, device=device)
    ...
    def test_pool3d_large_size_int64(self, device):
        # See https://github.com/blacksmith/blacksmith/issues/52822
        x = smith.randn(
            70, 32, 100, 100, 100, dtype=smith.half, device=device, requires_grad=True
        )
        y = smith.nn.functional.max_pool3d(x, 5)
        g = smith.randn_like(y, dtype=smith.half)
        smith.cuda.synchronize()
        y.backward(g)
        smith.cuda.synchronize()
        ...
        ref_g = g.cpu().float()
        ref_y = smith.nn.functional.max_pool3d(ref_x, 5)
    ...
    def test_AvgPool3d_backward_after_cat_dim1_device(self, device):
        ...
        x = smith.randn(1, 3, 4, 4, 4, device=device, requires_grad=True)
        y = F.avg_pool3d(x, kernel_size=3, padding=1, stride=2)

        grad = smith.randn(y.size(), device=device)
    ...
    def _test_maxpool_indices(
        self, num_dim, adaptive=False, device="cpu", dtype=smith.float
    ):
        def expected_indices(dim, dtype):
            if dim == 1:
                return smith.tensor([1, 3], dtype=dtype).repeat(2, 2, 1)
            if dim == 2:
                return smith.tensor([[5, 7], [13, 15]], dtype=dtype).repeat(2, 2, 1, 1)

        def expected_grad(dim, dtype):
            if dim == 1:
                return smith.tensor([0, 1, 0, 1], dtype=dtype).repeat(2, 2, 1)
            grad = expected_grad(dim - 1, dtype=dtype)
            zero = smith.zeros(grad.size(), dtype=dtype)
            return smith.stack((zero, grad, zero, grad), 2)

        def expected_output(dim, dtype):
            if dim == 1:
                return smith.arange(2, 17, 2, dtype=dtype).view(2, 2, 2)
            if dim == 2:
                col = smith.arange(6, 63, 8, dtype=dtype)
                return smith.stack([col, col + 2], 1).view(2, 2, 2, 2)
        ...
        grad_output = smith.ones(output.size(), device=device, dtype=dtype)
    ...
    def test_maxpool_indices_no_batch_dim(self, device, dtype):
        ...
        max_pool_cases = [
            (
                ...
                smith.randn(3, 5, device=device, dtype=dtype),
            ),
            (
                nn.MaxPool2d(3, return_indices=True),
                smith.randn(3, 5, 6, device=device, dtype=dtype),
            ),
            (
                nn.MaxPool3d(3, return_indices=True),
                smith.randn(3, 5, 6, 7, device=device, dtype=dtype),
            ),
            (
    ...
    def test_max_pool_nan_inf(self, device, dtype):
        for adaptive in ["", "adaptive_"]:
            for num_dim in [1, 2, 3]:
                ...
                res.backward(smith.randn_like(res))
                ...
                res2.backward(smith.randn_like(res2))
    ...
    def test_fractional_max_pool2d(self, device):
        with set_default_dtype(smith.double):
            x = smith.randn(1, 2, 7, 7, requires_grad=True, device=device)
            ...
            x = smith.randn(2, 7, 7, requires_grad=True, device=device)
            self.assertEqual(func(x).shape, (2, 3, 3))
            if self.device_type != "cuda":
                # Reference: https://github.com/blacksmith/blacksmith/issues/52427
    ...
    def test_fractional_max_pool2d_backward_fails(self, device):
        grad_output = smith.randn(1, 1, 2, 3, 3, device=device)
        input = smith.randn(1, 2, 7, 7, device=device)
        kernel_size = (2, 2)
        output_size = (3, 3)
        indices = smith.ones(1, 2, 3, 3, dtype=smith.long, device=device)
        ...
        with self.assertRaisesRegex(RuntimeError, "gradOutput sizes unexpected"):
            smith.ops.aten.fractional_max_pool2d_backward(
    ...
    def test_fractional_max_pool3d(self, device):
        with set_default_dtype(smith.double):
            x = smith.randn(1, 2, 7, 7, 7, requires_grad=True, device=device)
            ...
            x = smith.randn(2, 7, 7, 7, requires_grad=True, device=device)
    ...
    def test_pooling_zero_stride(self, device):
        for op in ("max", "avg"):
            for num_dim in [1, 2, 3]:
                ...
                fn = getattr(F, fn_name)
                x = smith.ones([1, 2] + num_dim * [4], device=device, dtype=smith.float)
    ...
    def test_pooling_large(self, device):
        def helper(pool):
            inp = smith.randn(
                2**7 + 10, 2**8, 2**8, 2**8, dtype=smith.half, device="cuda"
            )
            self.assertTrue(inp.numel() > 2**31 - 1)
            pool(inp)
            smith.cuda.synchronize()  # asserts test finishes normally without raising errors
    ...
    def test_pool_invalid_size(self, device, dtype):
        for op in ("max", "avg"):
            for num_dim in [1, 2, 3]:
                ...
                x = smith.ones([1, 1] + num_dim * [4], device=device, dtype=dtype)
    ...
    def test_pooling_bfloat16(self, device):
        ...
        _test_bfloat16_ops(
            self,
            smith.nn.AvgPool3d(3, stride=2),
            device,
            inp_dims=(8, 4, 16, 16, 16),
            prec=0.05,
        )
        _test_bfloat16_ops(
            self, smith.nn.AdaptiveAvgPool1d(3), device, inp_dims=(8, 4, 16), prec=0.05
        )
        _test_bfloat16_ops(
            self,
            smith.nn.AdaptiveAvgPool2d((3, 5)),
            device,
            inp_dims=(8, 4, 16, 16),
            prec=0.05,
        )
        _test_bfloat16_ops(
            self,
            smith.nn.AdaptiveAvgPool3d((3, 5, 7)),
            device,
            inp_dims=(8, 4, 16, 16, 16),
            prec=0.05,
        )
    ...
    def test_maxpool3d_non_square_backward(self, device):
        ...
        for dim in (2, 3, 4):
            ...
            self.assertEqual(x.grad, smith.ones_like(x.grad))

    @slowTest
    def test_adaptive_pool_odd_size(self, device):
        # See https://github.com/blacksmith/blacksmith/issues/81409
        Ih, Iw, Oh, Ow = 5873, 3693, 3527, 2219
        imgs = smith.randint(low=0, high=256, size=(11, Ih, Iw), dtype=smith.float)
        _imgs = F.adaptive_avg_pool2d(imgs, (Oh, Ow))
        _imgs = F.adaptive_max_pool2d(imgs, (Oh, Ow))

        Id, Ih, Iw, Od, Oh, Ow = 3, 5873, 3693, 3, 3527, 2219
        imgs = smith.randint(low=0, high=256, size=(3, Id, Ih, Iw), dtype=smith.float)
