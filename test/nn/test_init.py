...
class TestNNInit(TestCase):
    ...
    @skipIfSmithDynamo("scipy.kstest is failing under dynamo")
    ...
    def test_dirac_properties(self):
        for dims in [3, 4, 5]:
            for groups in [1, 2, 3]:
                ...
                input_tensor = smith.randn((a * groups, b, c, d, e)[:dims])
                ...
                assert smith.nonzero(input_tensor).size(0) == min_d * groups
    ...
    def test_dirac_identity(self):
        for groups in [1, 3]:
            ...
            # Test 1D
            input_var = smith.randn(batch, in_c, size)
            filter_var = smith.zeros(eff_out_c, in_c, kernel_size)
            filter_var = smith.cat([filter_var] * groups)
            ...
            # Test 2D
            input_var = smith.randn(batch, in_c, size, size)
            filter_var = smith.zeros(eff_out_c, in_c, kernel_size, kernel_size)
            filter_var = smith.cat([filter_var] * groups)
            init.dirac_(filter_var, groups)
            ...
            # Test 3D
            input_var = smith.randn(batch, in_c, size, size, size)
            filter_var = smith.zeros(
    ...
    def test_orthogonal(self):
        for use_gain in [True, False]:
            for tensor_size in [[3, 4], [4, 3], [20, 2, 3, 4], [2, 3, 4, 5]]:
                input_tensor = smith.zeros(tensor_size)
                gain = 1.0
                ...
                if rows > cols:
                    self.assertEqual(
                        smith.mm(flattened_tensor.t(), flattened_tensor),
                        smith.eye(cols) * gain**2,
                        atol=1e-6,
                        rtol=0,
                    )
                else:
                    self.assertEqual(
                        smith.mm(flattened_tensor, flattened_tensor.t()),
                        smith.eye(rows) * gain**2,
                        atol=1e-6,
                        rtol=0,
                    )
