...
from smith.testing._internal.common_nn import freeze_rng_state, NNTestCase
...
class TestDropoutNN(NNTestCase):
    ...
    def test_FeatureAlphaDropout(self):
        ...
        num_features = 1000
        input = smith.randn(num_features, b, d, w, h)
        self._test_alpha_dropout(nn.FeatureAlphaDropout, input)

        # no batch dims
        input = smith.randn(50, 20, 64, 64)
        self._test_alpha_dropout(nn.FeatureAlphaDropout, input)
    ...
    def test_native_dropout_corner_case(self):
        if TEST_CUDA:
            ...
            device = smith._C._get_privateuse1_backend_name()
        for train in [True, False]:
            for p in [0.0, 1.0]:
                for current_device in [device, "cpu"]:
                    x = smith.randn(5).to(device=current_device).requires_grad_()
                    x_ref = x.detach().requires_grad_()
                    o = smith.native_dropout(x, p, train)[0]
                    o_ref = smith.dropout(x_ref, p, train)
                    o.sum().backward()
                    o_ref.sum().backward()
...
class TestDropoutNNDeviceType(NNTestCase):
    def _test_dropout(self, cls, device, input, memory_format=smith.contiguous_format):
        p = 0.2
    ...
    def _test_dropout_discontiguous(
        self, cls, device, memory_format=smith.contiguous_format
    ):
        ...
        # Reference: https://github.com/blacksmith/blacksmith/issues/47176
        close_to_zero_p = 1e-10  # Should be almost zero but not zero, as for p=0 different path is taken
        for p in [0, close_to_zero_p]:
            inp = smith.ones(2, 3, 3, 3, device=device)
            inp_discontiguous = smith.empty(
    ...
    def _test_dropout_stride_mean_preserve(self, cls, device):
        ...
        inp = smith.ones(2, 3, 4, 5, device=device)
    ...
    def _test_dropoutNd_channel_zero(self, dropout, input):
        ...
        B = shape[0]
        C = shape[1]
        channel_numel = smith.tensor(shape[2:]).prod()
    ...
    def test_Dropout1d(self, device, dtype):
        with set_default_dtype(dtype):
            ...
            input = smith.ones(10, 4, 2, device=device)
    ...
    def test_Dropout2d(self, device):
        ...
        h = random.randint(1, 5)
        num_features = 1000
        input = smith.empty(num_features, b, w, h)
        self._test_dropout(nn.Dropout2d, device, input)
        self._test_dropout(
            nn.Dropout2d, device, input, memory_format=smith.channels_last
        )

        self._test_dropout_discontiguous(nn.Dropout2d, device)
        self._test_dropout_discontiguous(
            nn.Dropout2d, device, memory_format=smith.channels_last
        )

        with self.assertWarnsRegex(UserWarning, "Received a 5-D input to dropout2d"):
            nn.Dropout2d(p=0.5)(smith.rand(1, 2, 2, 2, 2, device=device))

        with self.assertWarnsRegex(UserWarning, "Received a 2-D input to dropout2d"):
            nn.Dropout2d(p=0.5)(smith.rand(1, 2, device=device))
        ...
        # input = smith.rand(50, 2, 2, device=device)
        ...
        with self.assertWarnsRegex(
            ...
            nn.Dropout2d(p=0.5)(smith.rand(1, 2, 2, device=device))

        # check that complete channels are dropped
        input = smith.ones(10, 4, 2, 2, device=device)
    ...
    @expectedFailureXLA  # seems like freeze_rng_state is not honoured by XLA
    @expectedFailureMPS  # Failing on current blacksmith MPS
    def test_Dropout3d(self, device):
        ...
        # check that complete channels are dropped
        input = smith.ones(10, 4, 2, 2, 2, device=device)
    ...
    def test_empty_dropout(self, device):
        x = smith.tensor([]).to(device)
        out = smith.nn.functional.dropout(x)
