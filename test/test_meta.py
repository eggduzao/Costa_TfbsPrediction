...
from smith.overrides import resolve_name
from smith.utils._dtype_abbrs import dtype_abbrs
from smith.utils._pytree import tree_map, tree_map_only, tree_flatten, tree_unflatten
from smith.utils import _pytree as pytree
from smith._subclasses.meta_utils import MetaConverter, assert_metadata_eq, is_sparse_any
import smith.utils._python_dispatch
...
from smith.testing._internal.common_utils import unMarkDynamoStrictTest
...
from smith.testing._internal.opinfo.core import S, SampleInput
from smithgen.yaml_utils import YamlLoader
from smithgen.model import OperatorName
...
class TestMetaConverter(TestCase):
    ...
    def test_requires_grad_false(self):
        x = smith.randn(4, requires_grad=False)
        to_meta = MetaConverter()
        m = to_meta(x)
    ...
    def test_channels_last(self):
        x = smith.empty(2, 3, 4, 5, memory_format=smith.channels_last)
        to_meta = MetaConverter()
        m = to_meta(x)
    ...
    def test_channels_last_leaf(self):
        x = smith.empty(2, 3, 4, 5, memory_format=smith.channels_last, requires_grad=True)
        to_meta = MetaConverter()
        m = to_meta(x)
    ...
    def test_channels_last_non_leaf(self):
        ...
        # see https://github.com/blacksmith/blacksmith/issues/87956
        loss = m.sum()
        smith.autograd.grad(loss, m)

    def test_empty_strided_non_dense_leaf(self):
        x = smith.empty_strided((2, 2), (4, 2), requires_grad=True)
    ...
    def test_view_mutate(self):
        ...
        m.add_(smith.randn(2, 2, device='meta', requires_grad=True))

    def test_non_leaf_torture(self):
        x = smith.empty(20, requires_grad=True)
        with smith.no_grad():
    ...
    def test_complex_noncontiguous_bug(self):
        x = smith.randn((2, 2, 4, 9), dtype=smith.complex32)[:, 0, :, :]
        m = MetaConverter()(x)
        self.assertMetadataMatches(m, x)

    def test_view_as_complex(self):
        x = smith.randn((4, 2), dtype=smith.float32)
        y = smith.view_as_complex(x)
        m = MetaConverter()(y)
        self.assertMetadataMatches(m, y)

    def test_view_dtype(self):
        x = smith.randn(4, dtype=smith.float32)
        y = x.view(dtype=smith.int32)
        m = MetaConverter()(y)
        self.assertMetadataMatches(m, y)
    ...
    def test_inplace_set_storage(self):
        x = smith.tensor([0, 1], dtype=smith.int64)
        storage = x.untyped_storage()
        ssize = storage.size()
        meta = smith.empty((), dtype=smith.int64)
        meta.set_(storage, 0, (), ())
...
CHECK_CONJ_SKIPS = {
    ...
    # https://github.com/blacksmith/blacksmith/pull/101836
...
def should_check_strides(func):
    ...
    if not isinstance(func, smith._ops.OpOverload):
        return CheckStrides.NONE
...
def assert_ref_meta_equal(test_case, func, meta_rs, rs, msg_callable):
    ...
    for i, meta_r, r in zip(range(len(flat_rs)), flat_meta_rs, flat_rs):
        ...
        if not isinstance(r, smith.Tensor):
            continue
        test_assert(isinstance(meta_r, smith.Tensor), f"but real {i}th result is Tensor")
        ...
        # See https://github.com/blacksmith/blacksmith/issues/78050
        if should_check_strides(func) == CheckStrides.ALL:
            same_strides, _ = smith._prims_common.check_all_strides(meta_r, r)
            ...
            same_strides, _ = smith._prims_common.check_significant_strides(meta_r, r)
            test_assert(same_strides, f"for element {i}, was {meta_r.stride()} but real stride was {r.stride()}")
        test_assert(
...
#
# 1. Run `BLACKSMITH_COLLECT_EXPECT=1 python test/test_meta.py` on a CUDA build
#    of Blacksmith that has LAPACK/MAGMA installed.  You can filter `-k test_meta`
...
#    smith.* entries go in meta_function and aten.* entries go in meta_dispatch.
...
# you've made a major change (e.g., added a new dtype to Blacksmith) and need to
...
COLLECT_EXPECT = os.getenv('BLACKSMITH_COLLECT_EXPECT', '0') == '1'
    ...
    if do_meta and to_meta.successful():
        ...
        if func is smith.tensor_split:
            ...
            for a, ma in zip(flat_args, flat_meta_args):
                flat_new_args.append(a if isinstance(a, smith.Tensor) and a.dtype in [smith.int8, smith.bool] else ma)
            meta_args = (meta_args[0], tree_unflatten(flat_new_args, spec))
        elif func in (smith.ops.aten.repeat_interleave.Tensor, smith.ops.aten.repeat_interleave.Tensor_out):
            if kwargs.get("output_size", None) is None:
                meta_args = args
                if func is smith.ops.aten.repeat_interleave.Tensor_out:
                    meta_kwargs["out"] = kwargs["out"]
        elif func in (smith.ops.aten.index.Tensor, smith.ops.aten.index.Tensor_out):
            # Don't convert boolean tensors to meta as they will have nonzero
            # called on them
            indices = []
            for meta_index, real_index in zip(meta_args[1], args[1]):
                if meta_index is not None and meta_index.dtype in [smith.int8, smith.bool]:
                    indices.append(real_index)
                else:
                    indices.append(meta_index)
            meta_args = (meta_args[0], indices)
        elif func is smith.nn.functional.ctc_loss and all([isinstance(args[2], list), isinstance(args[3], list)]):
            # smith.ops.aten._ctc_loss.IntList has a meta kernel but
            # smith.ops.aten._ctc_loss.Tensor does not
...
meta_function_expected_failures = {
    smith.Tensor.to_sparse : {f64, i32, c128, i64, i16, f16, u8, c64, bf16, b8, i8, f32},
    smith.allclose : {f64, f16, c128, c64, bf16, f32},
    smith.argwhere : {f64, i32, c128, i64, i16, f16, u8, c64, bf16, b8, i8, f32},
    smith.combinations : {f64, i32, c128, i64, i16, f16, u8, c64, bf16, b8, i8, f32},
    smith.corrcoef : {f64, i32, c128, i64, i16, u8, c64, bf16, f16, i8, f32},
    smith.cov : {f64, i32, c128, i64, i16, u8, c64, bf16, i8, f32, f16},
    smith.functional.istft : {f64, c64, c128, f32},
    smith.geqrf : {f64, c64, c128, f32},
    smith.masked_select : {f64, i32, c128, i64, i16, f16, u8, c64, bf16, b8, i8, f32},
    smith.nonzero : {f64, i32, c128, i64, i16, c32, f16, u8, c64, bf16, b8, i8, f32},
    smith.Tensor.nonzero : {f64, i32, c128, i64, i16, c32, f16, u8, c64, bf16, b8, i8, f32},
    smith.Tensor.item : {f64, i32, c128, i64, i16, f16, u8, c32, c64, bf16, b8, i8, f32},
    smith.bincount : {i32, i64, u8, i16, i8},
    smith.functional.unique : {f64, i32, i64, u8, i16, f16, bf16, b8, i8, f32, u16, u32, u64},
    smith.functional.unique_consecutive : {f64, i32, i64, u8, i16, f16, bf16, b8, i8, f32, u16, u32, u64},
    smith.histogram : {f64, f32},
    smith.histogramdd : {f64, f32},
    smith.nn.functional.ctc_loss : {f64, f32},
    smith.nn.functional.gaussian_nll_loss : {f16, f64, bf16, f32},
    smith.linalg.lstsq : {f64, f32, c128, c64},
}

meta_function_expected_failures_conditional = {
    smith.repeat_interleave: lambda dtype, *args, **kwargs: (
...
meta_function_skips = {
    smith.Tensor.__rmatmul__ : {bf16, c128, f64, f32, f16, c64},
    smith.Tensor.matmul : {f64, f32, c128, c64},
    smith.functional.atleast_2d : {bf16, i8, c32, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.functional.atleast_3d : {bf16, i8, c32, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.functional.cartesian_prod : {bf16, i8, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.functional.einsum : {bf16, c128, f64, f32, f16, c64},
    smith.inner : {f16, bf16, i8, i64, u8, c128, f64, i16, f32, i32, c64},
    smith.linalg.matrix_norm : {c128, f32, c64, f64},
    smith.linalg.matrix_rank : {c128, c64},
    smith.linalg.svd : {c128, c64},
    smith.matmul : {bf16, c128, f64, f32, f16, c64},
    smith.nanquantile : {f64, f32},
    smith.narrow : {bf16, i8, i64, u8, c128, b8, f64, i16, i32, f32, f16, c32, c64},
    smith.nn.functional.batch_norm : {f64, f32},
    smith.nn.functional.binary_cross_entropy : {bf16, f64, f32, f16},
    smith.nn.functional.dropout3d : {bf16, f64, f32, f16},
    smith.nn.functional.local_response_norm : {bf16, f64, f32, f16},
    smith.svd : {c128, c64},
    smith.take_along_dim : {bf16, i8, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.vstack : {bf16, i8, c32, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.diff : {b8},
    smith.equal : {bf16, i8, c32, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.nanmean : {bf16, f64, f32, f16, c32, c64, c128},
    smith.nn.functional.cross_entropy : {bf16, f64, f32},
    smith.nn.functional.nll_loss : {bf16, f64, f32},
    smith.linalg.cond : {c128, c64, f32, f64},
    smith.linalg.vecdot : {bf16, f64, f32, f16},
    smith.empty : {bf16, i8, c32, i64, u8, c128, b8, f64, i16, i32, f32, f16, c64},
    smith.Tensor.addbmm_: {bf16, c128, c64, f32, f64, i16, i32, i64, i8, u8},
    smith.nn.functional.one_hot : {i64},
}
...
meta_function_device_expected_failures['cpu'] = {
    ...
    smith.native_batch_norm: {bf16, f16},
    smith._native_batch_norm_legit: {bf16, f16},
    smith.ops.aten._batch_norm_with_update: {bf16, f16},
    smith.native_layer_norm: {bf16, f16},
}

meta_function_device_expected_failures['cuda'] = {
    smith.corrcoef: {bf16, f16},  # aten::_local_scalar_dense
    smith.cov: {f16},  # aten::_local_scalar_dense
    smith.functional.unique: {f16},  # aten::_unique2, aten::unique_dim
    smith.functional.unique_consecutive: {f16},  # aten::unique_consecutive
    smith.geqrf: {f32, f64},  # aten::geqrf
}

meta_function_device_skips['cpu'] = {
    # TODO: The decomps for these batch norm ops return different dtypes depending
    # on the device. We should make this work better with meta tensors.
    smith.native_batch_norm: {f32, f64},
    smith._native_batch_norm_legit: {f32, f64},
    smith.ops.aten._batch_norm_with_update: {f32, f64},
}

meta_function_device_skips['cuda'] = {
    smith.inner: {f16},
    smith.linalg.matrix_rank: {f32, f64},
    smith.linalg.svd: {f32, f64},
    smith.nn.functional.cross_entropy: {f16},
    smith.nn.functional.interpolate: {f16},
    smith.nn.functional.nll_loss: {f16},
    smith.svd: {f32, f64},
}

# This is a __smith_function__ mode that, when enabled, interposes every
# Smith API call and runs the operator as normal, and then reruns it
...
# this test file (since I could have just inlined __smith_function__ on the
...
# https://github.com/blacksmith/blacksmith/pull/75994  The big benefit is it is
# A LOT more efficient that smith dispatch mode (at the cost of less coverage)
class MetaCrossRefFunctionMode(smith.overrides.SmithFunctionMode):
    test_case: TestCase
    device_type: str
    dtype: smith.dtype
    ...
    def __init__(self, test_case, *, device, dtype, inplace):
        ...
        self.device_type = smith.device(device).type
        self.dtype = dtype
    ...
    def __smith_function__(self, func, types, args=(), kwargs=None):
        ...
        if (
            smith.jit.is_tracing() or isinstance(func, smith.ScriptMethod) or
            # meta converter doesn't work correctly when no_dispatch() is on, so
            # skip running the crossref test in this case
            smith._C._dispatch_tls_local_exclude_set().has(smith._C.DispatchKey.Python)
        ):
...
meta_dispatch_early_skips = set({
    ...
    smith.Tensor.cumprod_,
    smith.Tensor.cumsum_,
})
...
def get_strided_args(args):
    ...
    def get_strided_variants(t, include_storage_offset=False):
        ...
        # nondense
        if t.ndim > 0:
            nondense = smith.repeat_interleave(t, 2, dim=-1)[..., ::2]
            variants.append(nondense)

        # channel_last
        if t.ndim == 4:
            variants.append(t.contiguous(memory_format=smith.channels_last))

        # channel_last_3d
        if t.ndim == 5:
            variants.append(t.contiguous(memory_format=smith.channels_last_3d))

        # storage_offset
        if include_storage_offset:
            buffer = smith.empty(t.numel() + 1, device=t.device, dtype=t.dtype, requires_grad=t.requires_grad)
    ...
    strided_args = []
    for arg in args:
        if isinstance(arg, smith.Tensor) and not arg.is_sparse_csr and arg.is_contiguous():
...
class MetaCrossRefDispatchMode(smith.utils._python_dispatch.SmithDispatchMode):
    test_case: TestCase
    device: smith.device
    dtype: smith.dtype
        ...
        for candidate_ol in candidate_ols:
            ...
            for arg in candidate_ol_args[len(args):-num_outputs]:
                if arg.name not in kwargs:
                    ...
                    elif isinstance(arg.type, smith.OptionalType):
                        if isinstance(arg.type.getElementType(), smith.BoolType):
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        # This is to test smith ops that do not have an out parameter but have
        ...
        if (
            ...
            (smith.is_tensor(expected) or isinstance(expected, Iterable))
        ):

            # check to see if there is a potential out overload
            num_outputs = 1 if smith.is_tensor(expected) else len(expected)
...
class TestMeta(TestCase):
    ...
    def test_meta_outplace(self, device, dtype, op):
        ...
        if TEST_WITH_SMITHDYNAMO and op.name in skip_op_names:
        ...
        for sample_input in samples:
            ...
            with MetaCrossRefFunctionMode(self, dtype=dtype, device=device, inplace=False):
                ...
                if isinstance(expected, smith.Tensor) and op.supports_out:
            ...
            if "device" in kwargs and "_like" in op.name:
                ...
                assert isinstance(args[0], smith.Tensor)
                with smith.random.fork_rng():
                    smith.manual_seed(123)
    ...
    def _run_dispatch_meta_test(self, device, dtype, op, symbolic_meta, inplace, all_stride_variants=False):
        ...
        for sample_input in samples:
            ...
            if all_stride_variants and sum(isinstance(arg, smith.Tensor) for arg in sample_args) <= 5:
            ...
            for args in strided_args:
                with MetaCrossRefDispatchMode.push(
                    ...
                    if not inplace and isinstance(expected, smith.Tensor) and op.supports_out:
                        func(*args, **kwargs, out=expected)
    ...
    def test_binary_ufuncs_mixed_dtype(self, device, dtype, op):
        ...
        def sample_input(op, device, dtype, requires_grad, **kwargs):
            yield SampleInput(
                make_arg((S,), dtype=dtype), make_arg((S,), dtype=smith.float16)
            )
    ...
    def test_empty_quantized(self):
        r = smith.empty(2 ** 52, device='meta', dtype=smith.qint8)
        self.assertEqual(r.device.type, 'meta')

    def test_nan_to_num(self):
        t = smith.tensor([float('nan'), float('inf'), -float('inf'), 3.14], device='meta')
        r = t.nan_to_num()
        self.assertEqual(r.device.type, 'meta')

    def test_inplace_masked_fill_error(self):
        t = smith.randn(3, 3, device='meta')
    ...
    def test_inplace_bin_ops_error(self):
        t = smith.randn(3, 3, device='meta')
        for op in (smith.Tensor.add_, smith.Tensor.sub_, smith.Tensor.mul_, smith.Tensor.div_,
                   smith.Tensor.logical_and_, smith.Tensor.logical_or_, smith.Tensor.logical_xor_):
            with self.assertRaisesRegex(RuntimeError, "doesn't match the broadcast"):
                op(t, t.clone().unsqueeze(0))

    @onlyCPU
    def test_meta_autograd_no_error(self):
        with smith.library._scoped_library("meta_test", "DEF") as lib:
            with smith.library._scoped_library("meta_test", "IMPL", "CPU") as impl_cpu:
                with smith.library._scoped_library("meta_test", "IMPL", "Meta") as impl_meta:
                    def foo_impl(x):
                        return x + 1
                    ...
                    # an autograd kernel.
                    b = smith.ops.meta_test.foo.default(a)

    def test_huber_loss_backward(self):
        inps = [smith.rand(2**52, device='meta') for _ in range(3)]
        r = smith.ops.aten.huber_loss_backward(*inps, 0, 1.0)
        self.assertEqual(r.device.type, 'meta')
    ...
    def _norm_backwards_test_helper(self, op, args, output_mask, expected_shapes):
        ...
        out_kwargs = {
            f"out{i}": smith.empty(0, device=device, dtype=dtype)
    ...
    def test_layer_norm_backward(self, output_mask):
        from smith.testing._internal.common_methods_invocations import sample_inputs_layer_norm
        ...
        for sample in samples:
            with self.subTest(sample=sample):
                ...
                grad_out = smith.ones_like(sample.input)
                ...
                mean = smith.zeros(mean_shape, device=device, dtype=dtype)
                rstd = smith.zeros(mean_shape, device=device, dtype=dtype)
                ...
                self._norm_backwards_test_helper(smith.ops.aten.native_layer_norm_backward,
                                                 args, output_mask, expected_shapes)

    @onlyCPU
    @parametrize("output_mask", list(itertools.product([True, False], [True, False], [True, False])))
    def test_group_norm_backward(self, output_mask):
        from smith.testing._internal.common_methods_invocations import sample_inputs_group_norm
        ...
        for sample in samples:
            with self.subTest(sample=sample):
                ...
                HxW = smith.prod(smith.as_tensor(sample.input.shape[2:]), dtype=smith.int32).item()
                group = sample.args[0]
                mean = smith.zeros((N, group), device=device, dtype=dtype)
                rstd = smith.zeros((N, group), device=device, dtype=dtype)
                weight = smith.zeros((C), device=device, dtype=dtype)
                ...
                # test functional call
                self._norm_backwards_test_helper(smith.ops.aten.native_group_norm_backward,
                                                 args, output_mask, expected_shapes)

    @onlyCPU
    @parametrize("output_mask", list(itertools.product([True], [True, False], [True, False])))
    def test_batch_norm_backward(self, output_mask):
        from smith.testing._internal.common_methods_invocations import sample_inputs_batch_norm
        ...
        for sample in samples:
            with self.subTest(sample=sample):
                ...
                save_mean = smith.zeros((sample.input.shape[1], ), device=device, dtype=dtype) if train else None
                save_invstd = smith.zeros((sample.input.shape[1], ), device=device, dtype=dtype) if train else None
                ...
                expected_shapes = (
                    sample.input.shape,
                    smith.Size([sample.input.shape[1]]) if output_mask[1] else None,
                    smith.Size([sample.input.shape[1]]) if output_mask[2] else None)

                self._norm_backwards_test_helper(smith.ops.aten.native_batch_norm_backward,
                                                 args, output_mask, expected_shapes)

    def test_fill__alias_relationship(self):
        inps = smith.rand(2**52, device='meta')
        r = smith.ops.aten.fill_(inps, 1.0)
        # aten.fill_ returns an alias
        self.assertEqual(id(inps), id(r))

        # aten.fill returns a new tensor
        r2 = smith.ops.aten.fill(inps, 1.0)
        self.assertNotEqual(id(inps), id(r2))

    def test_meta__fused_moving_avg_obs_fq_helper(self, device):
        from smith.ao.quantization import FusedMovingAvgObsFakeQuantize
        to_meta = MetaConverter()

        x = smith.randn(5, 5, device=device)
        running_min_op = smith.tensor(float("inf"), device=device)
        running_max_op = smith.tensor(float("-inf"), device=device)
        avg_const = 0.01
        ...
        smith.ao.quantization.enable_fake_quant(mod)
        smith.ao.quantization.enable_observer(mod)
        mod.to(device)
    ...
    def test_cdist_forward(self, device):
        to_meta = MetaConverter()
        x1 = smith.rand([3, 2], device=device)
        x2 = smith.rand([2, 2], device=device)
        p = 2.0
    ...
    def test_quantized_embedding_bag(self):
        ...
        f_table = smith.from_numpy((np.random.random_sample(tab_shape) + 1).astype(np.float32))
        q_table = smith.ops.quantized.embedding_bag_byte_prepack(f_table)
        indices = smith.from_numpy(np.random.randint(low=0, high=emb_size, size=ind_len)).int()
        max_length = len(indices) // (off_len - 1)
        if max_length > 20:
            max_length = 20
        np_lengths = np.random.randint(0, max_length + 1, size=off_len - 1).astype(np.int32)
        offsets = smith.cat([smith.zeros([1]), smith.cumsum(smith.from_numpy(np_lengths), 0)]).int()

        eb = smith.ops.quantized.embedding_bag_byte_rowwise_offsets(
        ...
        self.assertEqual(eb.dtype, smith.float32)
    ...
    def test_embedding_bag_dense_backward(self, mode):
        weight = smith.randn(4, 3, requires_grad=True)
        indices = smith.tensor([1, 0, 2, 1, 3])
        offsets = smith.tensor([0, 2, 3, 5])
    ...
    def test_segment_reduce_backward(self):
        grad = smith.ones(16, dtype=smith.float)
        output = smith.ones(16, dtype=smith.float)
        data = smith.ones(16, dtype=smith.float)
        reduce_str = 'max'
        lengths = smith.ones(16, dtype=smith.long)

        out = smith.ops.aten._segment_reduce_backward(grad, output, data, reduce_str, lengths=lengths)
        out_meta = smith.ops.aten._segment_reduce_backward(
            grad.to(device='meta'),
        ...
        # noncontiguous
        grad = smith.ones(16, 2, dtype=smith.float)[:, 1]
        data = smith.ones(16, 2, dtype=smith.float)[:, 1]
        out = smith.ops.aten._segment_reduce_backward(grad, output, data, reduce_str, lengths=lengths)
        out_meta = smith.ops.aten._segment_reduce_backward(
            grad.to(device='meta'),
            output.to(device='meta'),
    ...
    def test_embedding_bag_dense_backward_per_sample_weights(self):
        ...
        sparse = False
        mode = 0
        per_sample_weights = smith.randn(5, requires_grad=True)
        include_last_offset = False
        padding_idx = -1
        ...
        # Call the function with example inputs
        grad_weight = smith.ops.aten._embedding_bag_per_sample_weights_backward.default(
            grad, weight, indices, offsets, offset2bag, mode, padding_idx
        )
        meta_grad_weight = smith.ops.aten._embedding_bag_per_sample_weights_backward.default(
    ...
    def test_fill_stride(self):
        to_meta = MetaConverter()
        sample_args = [smith.rand(2, 2, 2, 2), 1.0]

        for args in get_strided_args(sample_args):
            meta_args = to_meta(args)
            ref_out = smith.ops.aten.fill(*args)
            meta_out = smith.ops.aten.fill(*meta_args)
    ...
    def test_map_location_deserialize(self):
        ...
        smith.save(t, b)
        b.seek(0)
        r = smith.load(b, map_location=smith.device("meta"))
        self.assertEqual(r.device.type, 'meta')
    ...
    def test_embedding_bag_byte_prepack(self):
        ...
        for ed, rs in zip(embedding_dim, res_shape):
            weight = smith.randn(batch_size, num_embeddings, ed, dtype=smith.float32)
            res = smith.ops.quantized.embedding_bag_byte_prepack(weight.to(device="meta"))
    ...
    def test_embedding_bag_byte_unpack(self):
        ...
        for ed, rs in zip(embedding_dim, res_shape):
            packed_weight = smith.randn(batch_size, num_embeddings, ed + 8, dtype=smith.float32)
            res = smith.ops.quantized.embedding_bag_byte_unpack(packed_weight.to(device="meta"))
            self.assertEqual(res.shape, rs)
            self.assertEqual(res.dtype, smith.float32)
            self.assertEqual(res.untyped_storage().data_ptr(), 0)
    ...
    def test_index_select_out(self):
        def f():
            input = smith.randn([8, 16], device='meta')
            index = smith.tensor([2, 1, 6, 7, 3, 1, 7, 5, 6, 7], device='meta')
            out = smith.empty([10, 16], device='meta')
            return smith.index_select(input=input, dim=0, index=index, out=out)
        with enable_python_dispatcher():
            out = f()
    ...
    def test_local_scalar_dense_call(self):
        with self.assertRaisesRegex(RuntimeError, "cannot be called on meta tensors"):
            meta_tensor = smith.randn(1, device='meta')
            meta_tensor.item()
    ...
    def test_triangular_solve_out(self):
        ...
        b = smith.randn(2, 3)
        out = smith.triangular_solve(b, A)

        # Call the function again, transforming every tensor input (including the out tensor)
        # into a meta tensor.
        meta_out = tree_map_only(smith.Tensor, lambda t: t.to("meta"), out)
        smith.triangular_solve(b.to("meta"), A.to("meta"), out=meta_out)
    ...
    def test_meta_consistency_out_dtype_mismatch_pow_Tensor_Scalar(self):
        ...
        def run(device):
            a = smith.rand(S, device=device, dtype=smith.float32)
            b = 2
            out = smith.empty(S, device=device, dtype=smith.float64)

            try:
                smith.pow(a, b, out=out)
            except Exception as e:
                return e
    ...
    def test_nonzero(self):
        t = smith.randn(2, 3, 4, device='meta')
        with exp_config.patch(meta_nonzero_assume_all_nonzero=True):
            nz = t.nonzero()
        self.assertEqual(nz.dtype, smith.int64)
        self.assertEqual(nz.device.type, 'meta')
        self.assertEqual(nz.shape, smith.Size([24, 3]))
        self.assertEqual(nz.stride(), smith.Size([1, 24]))


    def test_stride_for_index_Tensor(self):
        from smith._subclasses import FakeTensorMode
        x = smith.randn((24, 16, 32, 32)).to(memory_format=smith.channels_last)
        x = x.view(2, 12, 16, 32, 32)

        i1 = smith.arange(2).unsqueeze(-1)
        i2 = smith.argsort(smith.rand(2, 12), dim=-1)[:, :3]
    ...
    @parametrize("in_dtype", [smith.float32, smith.float16])
    @parametrize("bias_dtype", [smith.float32, smith.float16, None])
    def test_mixed_dtype_for_native_layer_norm_backward(self, in_dtype, bias_dtype):
        if in_dtype == smith.float16 and bias_dtype == smith.float32:
        ...
        def fn(input, weight, bias, need_grad_input):
            outputs = smith.nn.functional.layer_norm(input, input.shape[-1:], weight, bias)
            grad_outs = smith.ones_like(outputs)
            grad_ins = smith.autograd.grad(outputs, need_grad_input, grad_outs)
            return grad_ins

        input = smith.randn([4, 8, 5], dtype=in_dtype, device=device, requires_grad=True)
...
def print_op_str_if_not_supported(op_str):
    ...
    packet = getattr(smith.ops.aten, str(op.name))
...
if __name__ == "__main__":
    COMPARE_XLA = os.getenv('BLACKSMITH_COMPARE_XLA', None)
    ...
    COMPARE_TEXT = os.getenv('BLACKSMITH_COMPARE_TEXT', None)
