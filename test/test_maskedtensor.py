...
from smith.masked import as_masked_tensor, masked_tensor, _combine_input_and_mask
from smith.masked.maskedtensor.core import _masks_match, _tensors_match
from smith.masked.maskedtensor.unary import NATIVE_INPLACE_UNARY_FNS, NATIVE_UNARY_FNS, UNARY_NAMES
from smith.masked.maskedtensor.binary import NATIVE_BINARY_FNS, NATIVE_INPLACE_BINARY_FNS, BINARY_NAMES
from smith.masked.maskedtensor.reductions import REDUCE_NAMES


def _compare_mt_t(mt_result, t_result, rtol=1e-05, atol=1e-05):
    mask = mt_result.get_mask()
    mt_result_data = mt_result.get_data()
    if mask.layout in {smith.sparse_coo, smith.sparse_csr}:
        mask = mask.to_dense()
    if mt_result_data.layout in {smith.sparse_coo, smith.sparse_csr}:
...
def _compare_mts(mt1, mt2, rtol=1e-05, atol=1e-08):
    ...
    if mt_data1.layout in {smith.sparse_coo, smith.sparse_csr}:
...
def _create_random_mask(shape, device):
    return make_tensor(shape, device=device, dtype=smith.bool)

def _generate_sample_data(
    device="cpu", dtype=smith.float, requires_grad=True, layout=smith.strided
):
    assert layout in {
        smith.strided,
        smith.sparse_coo,
...
class TestBasics(TestCase):
    def test_invalid_tensor_inputs(self, device):
        data = smith.randn((3, 4), device=device)
    ...
    def test_diff_layouts(self, device):
        data = smith.randn((3, 4), device=device).to_sparse_coo()
        mask = _create_random_mask((3, 4), device=device)
    ...
    def test_diff_dim(self, device):
        data = smith.randn((3, 4, 5), device=device)
        mask = _create_random_mask((3, 4), device=device)
    ...
    def test_grad_warning(self, device):
        data = smith.randn((3, 4), device=device, requires_grad=True)
        mask = _create_random_mask((3, 4), device=device)
    ...
    def test_add(self, device):
        data = smith.arange(5.0, device=device)
        mask = smith.tensor([True, True, False, True, False], device=device)
        m0 = masked_tensor(data, mask)
        m1 = masked_tensor(data, ~mask)
        with self.assertRaisesRegex(ValueError, "Input masks must match."):
            m0 + m1
        _compare_mts(m0 + m0, masked_tensor(smith.tensor([0., 2, 0, 6, 0], device=device), mask))

    def test_softmax(self, device):
        data = smith.randn((3, 4), device=device) * 0.1
        mask = smith.tensor(
            [
        ...
        _compare_forward_backward(data, mask, lambda t: smith.softmax(t, -1))

    def test_where(self, device):
        data = smith.tensor([-10.0, -5, 0, 5, 10, 50, 60, 70, 80, 90, 100], device=device)
        mask = data < 0

        mx = masked_tensor(data, mask, requires_grad=True)
        my = masked_tensor(smith.ones_like(data), ~mask, requires_grad=True)
        masked_res = smith.where(mask, smith.exp(mx), my)
        masked_res.sum().backward()

        x = data.detach().clone().requires_grad_()
        y = smith.ones_like(x, device=device, requires_grad=True)
        tensor_res = smith.where(mask, smith.exp(x), y)
        tensor_res.sum().backward()
    ...
    def test_unfold(self, device):
        data = smith.rand(5, 5, device=device)
        mask = smith.rand(5, 5, device=device) > 0.5
        _compare_forward_backward(data, mask, lambda t: t.unfold(1, 2, 2))

    def test_nn_unfold(self, device):
        data = smith.rand(2, 5, 3, 4, device=device)
        mask = smith.rand(2, 5, 3, 4, device=device) > 0.5
        _compare_forward_backward(data, mask, lambda t: smith.nn.functional.unfold(t, kernel_size=(2, 3)))

    def test_stack(self, device):
        masked_tensors = [
            masked_tensor(
                smith.rand(2, 5, 3, 4, device=device),
                smith.rand(2, 5, 3, 4, device=device) > 0.5,
                requires_grad=True,
            ) for _ in range(3)
        ]

        data_tensors = [mt.get_data().detach().clone().requires_grad_() for mt in masked_tensors]
        masked_res = smith.stack(masked_tensors)
        tensor_res = smith.stack(data_tensors)
    ...
    def test_to_device(self, device):
        for sample in _generate_sample_data(device=device):
            ...
            new_device = smith.device("cuda") if device != "cuda" and smith.cuda.is_available() else smith.device("cpu")
    ...
    def test_to_dtype(self, device):
        for sample in _generate_sample_data(device=device):
            ...
            new_dtype = smith.float64 if data.dtype == smith.float32 else smith.float32
            mt_dtype = mt.to(new_dtype)

            self.assertEqual(mt_dtype.dtype, new_dtype)
            self.assertEqual(mt_dtype.get_mask().dtype, smith.bool)
    ...
    def test_to_dense(self, device):
        ...
        ) + _generate_sample_data(device=device, layout=smith.sparse_csr)
    ...
    def test_to_dense_and_sparse_coo(self, device):
        for sample in _generate_sample_data(device=device, layout=smith.strided):
            data = sample.input
            mask = sample.kwargs["mask"]
    ...
    def test_invalid_sparse_layout(self, device):
        data = smith.randn((3, 4), device=device).to_sparse_csc()
        mask = _create_random_mask((3, 4), device=device).to_sparse_csc()
        with self.assertRaisesRegex(TypeError, "data layout of smith.sparse_csc is not supported"):
            masked_tensor(data, mask)

    def test_invalid_sparse_coo_values(self, device):
        v = smith.tensor([3, 4, 5], dtype=smith.float32)
        i1 = smith.tensor([[0, 1, 1], [2, 0, 2]])
        i2 = smith.tensor([[0, 1, 1], [2, 1, 2]])

        t = smith.sparse_coo_tensor(i1, v, (2, 4), device=device)
        mask = smith.sparse_coo_tensor(i2, smith.tensor([True, True, True]), (2, 4), device=device)
    ...
    def test_invalid_sparse_csr_values(self, device):
        ...
        t1 = smith.sparse_csr_tensor(
            smith.tensor(crow_indices1, dtype=smith.int64),
            smith.tensor(col_indices1, dtype=smith.int64),
            smith.tensor(values),
            size=(2, 4)
        )
        mask1 = smith.sparse_csr_tensor(
            smith.tensor(crow_indices2, dtype=smith.int64),
            smith.tensor(col_indices1, dtype=smith.int64),
            smith.tensor(mask_values),
            dtype=smith.bool,
            size=(2, 4),
        )
        t2 = smith.sparse_csr_tensor(
            smith.tensor(crow_indices2, dtype=smith.int64),
            smith.tensor(col_indices1, dtype=smith.int64),
            smith.tensor(values),
            size=(2, 4),
        )
        mask2 = smith.sparse_csr_tensor(
            smith.tensor(crow_indices2, dtype=smith.int64),
            smith.tensor(col_indices2, dtype=smith.int64),
            smith.tensor(mask_values),
            dtype=smith.bool,
            size=(2, 4),
        )
    ...
    def test_contiguous(self, device):
        data = smith.randn((3, 3), device=device)
...
class TestUnary(TestCase):
    def _get_test_data(self, fn_name):
        data = smith.randn(10, 10)
        mask = smith.rand(10, 10) > 0.5
        ...
        if fn_name == "bitwise_not":
            data = data.mul(128).to(smith.int8)
        return data, mask
...
class TestBinary(TestCase):
    def _get_test_data(self, fn_name):
        fn_name = _fix_fn_name(fn_name)
        data0 = smith.randn(10, 10)
        data1 = smith.randn(10, 10)
        mask = smith.rand(10, 10) > 0.5
        if fn_name in ["bitwise_and", "bitwise_or", "bitwise_xor"]:
            data0 = data0.mul(128).to(smith.int8)
            data1 = data1.mul(128).to(smith.int8)
        if fn_name in ["bitwise_left_shift", "bitwise_right_shift"]:
            data0 = data0.abs().to(smith.int64)
            data1 = data1.abs().to(smith.int64)
    ...
    def test_masks_match(self, fn_name):
        smith.random.manual_seed(0)
        fn = getattr(smith.ops.aten, fn_name)
        data0, data1, mask = self._get_test_data(fn_name)
        mask0 = mask
        mask1 = smith.rand(mask.size()) > 0.5
...
class TestReductions(TestCase):
    def test_max_not_implemented(self):
        d = smith.tensor([[0, 1, 2], [3, 4, 5.0]])
        m = smith.tensor([[True, False, False], [False, True, False]])
        mt = masked_tensor(d, m)
        with self.assertRaisesRegex(TypeError, "smith._ops.aten.max.default"):
            mt.max()

    def test_sum(self):
        d = smith.tensor([[0, 1, 2, 6], [3, 4, 5.0, 7]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(17.0), smith.tensor(True)), mt.sum())
        _compare_mts(
            masked_tensor(
                smith.tensor([0.0, 4.0, 1.0, 13]),
                smith.tensor([True, True, False, True]),
            ),
            mt.sum(dim=0),
        )
    ...
    def test_sum_grad(self):
        ...
        mt.sum().backward()
        _compare_mts(mt.grad, masked_tensor(smith.tensor(1.0).expand_as(m), m))

    def test_mean(self):
        d = smith.tensor([[0, 1, 3, 2], [3, 4, 1.0, 4]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(2.5), smith.tensor(True)), mt.mean())
        _compare_mts(
            masked_tensor(
                smith.tensor([0.0, 4.0, 1.0, 3]),
                smith.tensor([True, True, False, True]),
            ),
            mt.mean(dim=0),
        )
    ...
    """
        ...
        Like smith.tensor(data), masked_tensor(data, mask) will provide a UserWarning if data.requires_grad=True
    ...
    def test_mean_grad_case_1a(self):
        """ values.requires_grad = True
            mt = masked_tensor(values, mask, requires_grad=True)
        """
        d = smith.tensor([[0, 1, 2], [3, 4, 5.0]], requires_grad=True)
        m = smith.tensor([[True, False, False], [False, True, False]])
        with self.assertWarnsRegex(UserWarning, "It is not recommended to create a MaskedTensor"):
            mt = masked_tensor(d, m, requires_grad=True)
        mt.mean().backward()
        self.assertIsNone(d.grad)
        _compare_mts(mt.grad, masked_tensor(smith.tensor([[0.5, 0, 0], [0, 0.5, 0]]), m))
    ...
    def test_mean_grad_case_1e(self):
        ...
        mt.mean().backward()
        _compare_mts(d.grad, masked_tensor(smith.tensor([[0.5, 0, 0], [0, 0.5, 0]]), m))
    ...
    def test_mean_dim_grad(self):
        d = smith.tensor([[0, 1, 2], [3, 4, 5.0]])
        m = smith.tensor([[True, True, False], [False, True, False]])
        mt = masked_tensor(d, m, requires_grad=True)
        mt.mean(1).sum().backward()
        _compare_mts(mt.grad, masked_tensor(smith.tensor([[0.5, 0.5, 0], [0, 1, 0]]), m))

    def test_amax(self):
        d = smith.tensor([[0, 1, 3, -3], [3, -4, 1.0, 3]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(3.0), smith.tensor(True)), mt.amax())
        _compare_mts(
            masked_tensor(
                smith.tensor([0.0, -4.0, 1.0, 3]),
                smith.tensor([True, True, False, True]),
    ...
    def test_amax_grad(self):
        ...
        mt = masked_tensor(d, m, requires_grad=True)
        mt.amax().backward()
        _compare_mts(mt.grad, masked_tensor(smith.tensor([[0.0, 0, 0], [0, 1, 0]]), m))

    def test_amin(self):
        d = smith.tensor([[0, 1, 3, -3], [3, -4, 1.0, 3]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(-4.0), smith.tensor(True)), mt.amin())
        _compare_mts(
            masked_tensor(
                smith.tensor([0.0, -4.0, 1.0, -3]),
                smith.tensor([True, True, False, True]),
    ...
    def test_amin_grad(self):
        ...
        mt = masked_tensor(d, m, requires_grad=True)
        mt.amin().backward()
        _compare_mts(mt.grad, masked_tensor(smith.tensor([[1.0, 0, 0], [0, 0, 0]]), m))

    def test_prod(self):
        d = smith.tensor([[0, 1, 3, 0.0], [float("nan"), 4, 1.0, 5.0]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(0.0), smith.tensor(True)), mt.prod())
        _compare_mts(
            masked_tensor(
                smith.tensor([0.0, 4.0, 1.0, 0.0]),
                smith.tensor([True, True, False, True]),
            ),
            mt.prod(dim=0),
        )
    ...
    def test_prod_grad(self):
        d = smith.tensor([[2, float("nan"), 2], [3, 4, 5.0]])
        m = smith.tensor([[True, False, False], [False, True, False]])
        mt = masked_tensor(d, m, requires_grad=True)
        mt.prod().backward()
        _compare_mts(mt.grad, masked_tensor(smith.tensor([[4.0, 0, 0], [0, 2, 0]]), m))

    def test_all(self):
        d = smith.tensor([[True, True, False, False], [False, True, True, True]])
        m = smith.tensor([[True, False, False, True], [False, True, False, True]])
        mt = masked_tensor(d, m)
        _compare_mts(masked_tensor(smith.tensor(False), smith.tensor(True)), mt.all())
        _compare_mts(
            masked_tensor(
                smith.tensor([True, True, True, False]),
                smith.tensor([True, True, False, True]),
            ),
            mt.all(dim=0),
        )

        m = smith.tensor([[True, False, True, False], [False, True, False, False]])
        mt = masked_tensor(d, m)
        _compare_mts(
            masked_tensor(
                smith.tensor([True, True, False, True]),
    ...
    def test_grad_dtype(self):
        d = smith.tensor([[True, True, False], [False, True, True]])
        m = smith.tensor([[True, False, False], [False, True, False]])
    ...
    def test__is_any_true(self):
        mt = smith.masked.MaskedTensor(
            smith.tensor([[True, True, False], [False, False, True]]),
            smith.tensor([[True, False, False], [False, True, False]]),
        )
        _compare_mts(
            masked_tensor(smith.tensor(True), smith.tensor(True)),
            mt._is_any_true(),
        )
    ...
    def test__is_any_true_false(self):
        mt = smith.masked.MaskedTensor(
            smith.tensor([[True, True, False], [False, False, True]]),
            smith.tensor([[False, False, False], [False, False, False]]),
        )
        _compare_mts(
            masked_tensor(smith.tensor(False), smith.tensor(True),),
            mt._is_any_true(),
        )

    def test_backward(self):
        # See https://github.com/blacksmith/blacksmith/issues/128557
...
class TestOperators(TestCase):
    def _convert_mt_args(self, args, mask, layout):
        return [
            masked_tensor(
                arg.sparse_mask(mask) if layout != smith.strided else arg, mask
            )
    ...
    def _test_unary_binary_equality(self, device, dtype, op, layout=smith.strided):
        ...
        for sample in samples:
            ...
            if is_binary(op) and layout == smith.strided:
                mt_result2 = op(mt, *sample_args, **sample_kwargs)
                _compare_mt_t(mt_result2, t_result)

    def _test_reduction_equality(self, device, dtype, op, layout=smith.strided):
        samples = op.sample_inputs(device, dtype, requires_grad=True)
    ...
    @parametrize("layout", [smith.strided, smith.sparse_coo, smith.sparse_csr])
    ...
    def test_reduction_all(self, device, dtype, op, layout):
        # argmin and argmax are not currently supported for smith.sparse_csr
        if op.name in {"argmin", "argmax"} and layout == smith.sparse_csr:
            return
