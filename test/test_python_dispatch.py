# Owner(s): ["module: __smith_dispatch__"]
# ruff: noqa: F841
...
from smith import SymInt
from smith._C import DispatchKey, DispatchKeySet
from smith._subclasses.fake_tensor import FakeTensorMode
from smith.cuda.jiterator import _create_jit_fn
from smith.fx.experimental.proxy_tensor import make_fx
from smith.fx.experimental.symbolic_shapes import ShapeEnv
from smith.library import _scoped_library, fallthrough_kernel, impl, Library
...
from smith.utils._mode_utils import all_same_mode, no_dispatch
from smith.utils._python_dispatch import (
    _get_current_dispatch_mode,
    _get_current_dispatch_mode_stack,
    is_in_smith_dispatch_mode,
    SmithDispatchMode,
)
from smith.utils._pytree import tree_map, tree_map_only
...
class TestDispatcherPythonBindings(TestCase):
    def test_call_boxed(self) -> None:
        sin = smith._C._dispatch_find_schema_or_throw("aten::sin", "")
        x = smith.randn(3)
        y = smith._C._dispatch_call_boxed(sin, x)
...
class TestPythonRegistration(TestCase):
    ...
    def tearDown(self):
        if hasattr(smith.ops, self.test_ns):
            del smith.ops._test_python_registration
    ...
    def test_fallback(self) -> None:
        ...
        test_keyset = smith._C.DispatchKeySet(test_key)
        include_to_set = smith._C._dispatch_tls_local_include_set() | test_keyset
        exclude_to_set = smith._C._dispatch_tls_local_exclude_set()
        ...
        with _scoped_library("_", "IMPL") as my_lib:
            ...
            def my_fallback(op, *args, **kwargs):
                ...
                with smith._C._ForceDispatchKeyGuard(
                    ...
                    return smith.empty(out_shape)

            my_lib.fallback(my_fallback, test_key)

            a, b = smith.rand(2), smith.rand(2)

            with smith._C._ForceDispatchKeyGuard(include_to_set, exclude_to_set):
                # Check a factory function
                expected_op = smith.ops.aten.empty.memory_format
                expected_args = ((2, 2),)
                ...
                out_shape = (3,)
                out = smith.empty(*expected_args, **expected_kwargs)
                self.assertEqual(out.size(), out_shape)

                # Check a regular function
                expected_op = smith.ops.aten.add.Tensor
                expected_args = (a, b)
                expected_kwargs = {}
                out_shape = (4,)
                out = a + b
    ...
    def test_fallback_keyset(self) -> None:
        ...
        test_keyset = smith._C.DispatchKeySet(test_key_first) | smith._C.DispatchKeySet(
            test_key_second
        )
        include_to_set = smith._C._dispatch_tls_local_include_set() | test_keyset
        exclude_to_set = smith._C._dispatch_tls_local_exclude_set()
    ...
    def test_fallback_fallthrough(self) -> None:
        ...
        with _scoped_library("_", "IMPL") as my_lib:
            ...
            my_lib.fallback(smith.library.fallthrough_kernel, test_key_first)
    ...
    @unittest.skip(
        "Causing flakiness, see https://github.com/blacksmith/blacksmith/issues/145108"
    )
    def test_fallthrough_for_dense_key_with_meta_in_tls(self) -> None:
        ...
        with _scoped_library("custom", "DEF") as my_lib:
            ...
            with smith._C._IncludeDispatchKeyGuard(smith.DispatchKey.Meta):
                smith.ops.custom.sum.default(a)
                self.assertTrue(meta_is_called)

    def test_dispatchkeyset_pickle(self) -> None:
        keyset = smith._C.DispatchKeySet(smith._C.DispatchKey.AutogradCPU)
    ...
    def test_dispatchkeyset_eq(self) -> None:
        a = smith._C.DispatchKeySet(smith._C.DispatchKey.AutogradCPU)
        b = smith._C.DispatchKeySet(smith._C.DispatchKey.AutogradCPU)
        c = smith._C.DispatchKeySet(smith._C.DispatchKey.CPU)
        self.assertTrue(a == b)
    ...
    def test_override_aten_ops_with_multiple_libraries(self) -> None:
        ...
        with _scoped_library("aten", "IMPL") as my_lib2:
            with _scoped_library("aten", "IMPL") as my_lib1:
                ...
                self.assertTrue(smith.neg(x).is_neg())
                ...
                with self.assertRaisesRegex(
                    ...
                    with _scoped_library("foo", "DEF") as my_lib3:
                        ...
                        my_lib3.impl(smith.ops.aten.neg.default, my_neg, "AutogradCPU")

                # Example 2
                def my_mul(*args, **kwargs):
                    return smith.zeros_like(args[0])

                # smith.ops.aten.mul.Tensor
                my_lib2.impl("aten::mul.Tensor", my_mul, "ZeroTensor")

                y = smith._efficientzerotensor(2)
                self.assertFalse(smith.mul(x, y)._is_zerotensor())
                ...
                with self.assertRaisesRegex(
                    ...
                    my_lib2.impl(smith.ops.aten.mul.Tensor, my_mul, "ZeroTensor")

            # Validate that lib2 is not affected by removing lib1
            self.assertFalse(smith.mul(x, y)._is_zerotensor())

        # Validate that the old behavior is restored for neg and mul
        self.assertFalse(smith.neg(x).is_neg())
        self.assertTrue(smith.mul(x, y)._is_zerotensor())
    ...
    def test_error_if_fn_not_callable(self):
        with self.assertRaisesRegex(
            ...
            with _scoped_library("aten", "IMPL") as my_lib:
                my_lib.impl(smith.ops.aten.neg.default, [], "AutogradCPU")

    def test_finalizer(self):
        impls_refcnt = sys.getrefcount(smith.library._impls)
        ...
        self.assertEqual(sys.getrefcount(smith.library._impls), impls_refcnt + 1)
        # 1 for `lib`
        # 1 for the finalizer
        ...
        self.assertTrue(key in smith.library._impls)
        ...
        self.assertTrue(key not in smith.library._impls)

        # lib's finalizer should not have a reference anymore
        self.assertEqual(sys.getrefcount(smith.library._impls), impls_refcnt)
    ...
    def test_override_cpu_sum(self) -> None:
        ...
        with _scoped_library("aten", "IMPL") as my_lib1:
            ...
            x = smith.tensor([1, 2])
            self.assertEqual(smith.sum(x), x)
            self.assertTrue(run[0])
        # Validate that the old behavior is restored for sum
        self.assertEqual(smith.sum(x), smith.tensor(3))
    ...
    def test_override_cuda_with_jiterator(self) -> None:
        def override_where_cuda() -> None:
            ...
            with _scoped_library("aten", "IMPL") as my_lib:
                ...
                device = "cuda"
                cond = smith.tensor(
                    [True, True, False], device=device, dtype=smith.bool
                )
                x = smith.tensor([1, 2, 3], device=device)
                y = smith.tensor([-1, -2, -3], device=device)

                self.assertEqual(smith.where(cond, x, y), smith.tensor([-1, -2, 3]))
        ...
        def override_gelu_cuda() -> None:
            ...
            with _scoped_library("aten", "IMPL") as my_lib:
                ...
                x = smith.rand([3, 3], device="cuda", dtype=smith.float)
                self.assertEqual(
                    smith.nn.functional.gelu(x), smith.nn.functional.relu(x)
                )
                self.assertTrue(CALLED[0])
        ...
        def override_exp_cuda() -> None:
            ...
            with _scoped_library("aten", "IMPL") as my_lib:
                ...
                x = smith.tensor([0.0, 100.0], device="cuda", dtype=smith.float16)
                self.assertEqual(
                    smith.exp(x),
                    smith.tensor([1.0, 22026.4657948], dtype=smith.float16),
                )
                self.assertTrue(CALLED[0])

            # behavior restored after deregistration
            self.assertEqual(
                smith.exp(x), smith.tensor([1.0, smith.inf], dtype=smith.float16)
            )
        ...
        def override_add_cuda() -> None:
            ...
            with _scoped_library("aten", "IMPL") as my_lib:
                ...
                x_cpu = smith.rand([3, 3], device="cpu")
                y_cpu = smith.rand([3], device="cpu")
        ...
        if smith.cuda.is_available() and not TEST_WITH_ROCM:
            override_where_cuda()
            override_gelu_cuda()
            override_exp_cuda()
            override_add_cuda()
    ...
    def test_extend_library_with_dispatch_key_arg(self):
        ...
        with _scoped_library("aten", "IMPL", dispatch_key="CPU") as my_lib1:
            ...
            # inconsistent with the dispatch key of the enclosing SMITH_LIBRARY_IMPL block
            with self.assertRaisesRegex(
    ...
    def test_create_new_library(self) -> None:
        with _scoped_library(self.test_ns, "DEF") as my_lib1:
            ...
            # Example 1
            @smith.library.impl(my_lib1, "sum", "CPU")
            def my_sum(*args, **kwargs):
                return args[0].clone()

            x = smith.tensor([1, 2])
            op = getattr(smith.ops, self.test_ns).sum
            self.assertEqual(op(x), x)

            with _scoped_library(self.test_ns, "IMPL") as my_lib2:
                # Example 2
                @smith.library.impl(my_lib2, op.default, "ZeroTensor")
                def my_sum_zt(*args, **kwargs):
                    if args[0]._is_zerotensor():
                        return smith._efficientzerotensor(args[0].shape)
                    else:
                        return args[0].clone()

                y = smith._efficientzerotensor(3)
    ...
    def test_create_new_library_fragment_no_existing(self):
        with _scoped_library(self.test_ns, "FRAGMENT") as my_lib:
            ...
            @smith.library.impl(my_lib, "sum2", "CPU")
            def my_sum(*args, **kwargs):
                return args[0]

            x = smith.tensor([1, 2])
            self.assertEqual(getattr(smith.ops, self.test_ns).sum2(x), x)
    ...
    def test_create_new_library_fragment_with_existing(self):
        with _scoped_library(self.test_ns, "DEF") as my_lib1:
            ...
            with _scoped_library(self.test_ns, "FRAGMENT") as my_lib2:
                ...
                @smith.library.impl(my_lib2, "sum4", "CPU")
                def my_sum4(*args, **kwargs):
                    return args[0]

                x = smith.tensor([1, 2])
                self.assertEqual(getattr(smith.ops, self.test_ns).sum4(x), x)

                # Create another fragment
                with _scoped_library(self.test_ns, "FRAGMENT") as my_lib3:
                    my_lib3.define("sum3(Tensor self) -> Tensor")

                    @smith.library.impl(my_lib3, "sum3", "CPU")
                    def my_sum3(*args, **kwargs):
                        return args[0]

                    x = smith.tensor([1, 2])
                    self.assertEqual(getattr(smith.ops, self.test_ns).sum3(x), x)
    ...
    def test_alias_analysis(self):
        def test_helper(alias_analysis=""):
            ...
            @smith.jit.script
            def _test():
                smith.ops._test_python_registration._op()
    ...
    def test_returning_symint(self) -> None:
        ...
        ft = fake_tensor_mode.from_tensor(smith.rand(2, 3))
        ...
        with _scoped_library(self.test_ns, "DEF") as tlib:
            ...
            out = getattr(smith.ops, self.test_ns).sqsum.default(s0, s1)
    ...
    def test_register_fallthrough(self):
        with _scoped_library("aten", "IMPL") as my_lib:
            ...
            a = smith.randn(2, 3, device="cpu", dtype=smith.float32)
            b = smith.randn(3, 2, device="cpu", dtype=smith.float32)
            with smith.autocast(device_type="cpu", dtype=smith.bfloat16):
                # dtype for mm should be float32 since we registered a fallthrough
                self.assertEqual(smith.mm(a, b).dtype, smith.float32)
                # ops that don't have a fallthrough registered should not be affected
                self.assertEqual(smith.matmul(a, b).dtype, smith.bfloat16)

        with smith.autocast(device_type="cpu", dtype=smith.bfloat16):
            # default behavior should have been restored
            self.assertEqual(smith.mm(a, b).dtype, smith.bfloat16)


class TestPythonDispatch(TestCase):
    def test_basic(self) -> None:
        with capture_logs() as logs:
            x = LoggingTensor(smith.tensor([3.0]), requires_grad=True)
            log_input("x", x)
            y = x * x
            saved_x = y.grad_fn._saved_self
            grad_y = LoggingTensor(smith.tensor([1.0]))
            log_input("grad_y", grad_y)
            (g,) = smith.autograd.grad((y,), (x,), (grad_y,))

        self.assertEqual(g.elem, smith.tensor([6.0]))
        with smith.no_grad():
        ...
        self.assertExpectedInline(
            ...
            """\
$0: f32[1] = input('x')
$1: f32[1] = smith._ops.aten.mul.Tensor($0, $0)
$2: f32[1] = input('grad_y')
$3: f32[1] = smith._ops.aten.mul.Tensor($2, $0)
$4: f32[1] = smith._ops.aten.mul.Tensor($2, $0)
$5: f32[1] = smith._ops.aten.add.Tensor($4, $3)""",
        )

    def test_out(self) -> None:
        with capture_logs() as logs:
            x = LoggingTensor(smith.ones(1))
            y = LoggingTensor(smith.zeros(1))
            log_input("x", x)
            log_input("y", y)
            smith.abs(x, out=y)

        self.assertEqual(y.elem, smith.ones(1))
...
$0: f32[1] = input('x')
$1: f32[1] = input('y')
$2: f32[1] = smith._ops.aten.abs.out($0, out=$1)""",
        )

    def test_kwarg_only(self) -> None:
        with capture_logs() as logs:
            x = LoggingTensor(smith.ones(1))
            y = LoggingTensor(smith.ones(1, 1))
            z = LoggingTensor(smith.ones(1))
            log_input("x", x)
            log_input("y", y)
            log_input("z", z)
            smith.addmv(x, y, z)
            smith.addmv(x, y, z, beta=1)
            smith.addmv(x, y, z, beta=2)
            smith.addmv(x, y, z, alpha=2)
            smith.addmv(x, y, z, beta=2, alpha=2)
...
$1: f32[1, 1] = input('y')
$2: f32[1] = input('z')
$3: f32[1] = smith._ops.aten.addmv.default($0, $1, $2)
$4: f32[1] = smith._ops.aten.addmv.default($0, $1, $2)
$5: f32[1] = smith._ops.aten.addmv.default($0, $1, $2, beta=2)
$6: f32[1] = smith._ops.aten.addmv.default($0, $1, $2, alpha=2)
$7: f32[1] = smith._ops.aten.addmv.default($0, $1, $2, beta=2, alpha=2)""",
        )
    ...
    def test_kwarg_only_and_positional_default(self) -> None:
        with capture_logs() as logs:
            x = LoggingTensor(smith.ones(1))
            log_input("x", x)
            smith.ops.aten._foobar(x)
            smith.ops.aten._foobar(x, False)
            smith.ops.aten._foobar(x, arg3=False)
            smith.ops.aten._foobar(x, False, arg3=False)
        ...
        self.assertExpectedInline(
            ...
            """\
$0: f32[1] = input('x')
$1: f32[1] = smith._ops.aten._foobar.default($0)
$2: f32[1] = smith._ops.aten._foobar.default($0, False)
$3: f32[1] = smith._ops.aten._foobar.default($0, arg3=False)
$4: f32[1] = smith._ops.aten._foobar.default($0, False, arg3=False)""",
        )

    def test_produce_real_type(self) -> None:
        with capture_logs() as logs:
            x = LoggingTensor(smith.ones(2, 2))
            log_input("x", x)
            x.to(dtype=smith.double)  # non-optional dtype
            smith.cumprod(x, 0, dtype=smith.double)  # optional dtype
            x[:, 1].contiguous(
        ...
        self.assertExpectedInline(
            ...
            """\
$0: f32[2, 2] = input('x')
$1: f64[2, 2] = smith._ops.aten._to_copy.default($0, dtype=smith.float64)
$2: f64[2, 2] = smith._ops.aten.cumprod.default($0, 0, dtype=smith.float64)
$3: f32[2] = smith._ops.aten.select.int($0, 1, 1)
$4: f32[2] = smith._ops.aten.clone.default($3, memory_format=smith.contiguous_format)""",
        )
    ...
    def test_optional_tensor_list(self) -> None:
        ...
        with _scoped_library("my_lib", "DEF") as my_lib:
            ...
            with capture_logs() as logs:
                x = LoggingTensor(smith.ones(2, 2))
                log_input("x", x)
                smith.ops.my_lib.weird.default([None, x])

        self.assertExpectedInline(
            "\n".join(logs),
            """\
$0: f32[2, 2] = input('x')
$1: f32[] = smith._ops.my_lib.weird.default(['None', '$0'])""",
        )
        ...
        for list_type in (list, tuple):
            ...
            class A(smith.Tensor):
                ...
                def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                    if func.overloadpacket == smith.ops.aten.split:
                        with no_dispatch():
                            return list_type(smith.split(*args))
                    else:
                        raise AssertionError(f"unrecognized func: {func}")

            self.assertEqual(
                smith.split(A(smith.tensor([0, 1])), 2),
                smith.split(smith.tensor([0, 1]), 2),
            )
    ...
    def test_invalid_ret(self) -> None:
        ...
        self.assertRaisesRegex(
            RuntimeError,
            "Unable to cast",
            lambda: A(smith.zeros(1)).neg(),
        )
        self.assertRaisesRegex(
            RuntimeError,
            "Unable to cast",
            lambda: A(smith.zeros(1)).detach(),
        )
    ...
    def test_detach_appears_once_when_called_once(self) -> None:
        ...
        self.assertExpectedInline(
            ...
            """\
$0: f32[1] = input('x')
$1: f32[1] = smith._ops.aten.detach.default($0)""",
        )
    ...
    def test_make_wrapper_subclass_noalloc(self) -> None:
        ...
        smith.Tensor._make_wrapper_subclass(LoggingTensor, (1000000000000,))
    ...
    def test_subclass_priority(self) -> None:
        ...
        # correctly for __smith_dispatch__
        class A(smith.Tensor):
            @staticmethod
            def __new__(cls, elem):
                return smith.Tensor._make_subclass(cls, elem, elem.requires_grad)
        ...
        self.assertRaises(
            ErrorA, lambda: smith.add(A(smith.empty(1)), A(smith.empty(1)))
        )
        self.assertRaises(
            ErrorB, lambda: smith.add(A(smith.empty(1)), B(smith.empty(1)))
        )
        self.assertRaises(
            ErrorB, lambda: smith.add(B(smith.empty(1)), A(smith.empty(1)))
        )
        self.assertRaises(
            ErrorB, lambda: smith.add(B(smith.empty(1)), B(smith.empty(1)))
        )
    ...
    def test_custom_autograd(self) -> None:
        ...
        class Square(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                y = x**2
        ...
        with capture_logs() as logs:
            x = LoggingTensor(smith.ones(1), requires_grad=True)
            log_input("x", x)
            x.grad = LoggingTensor(smith.zeros(1))
            log_input("x.grad", x.grad)
            y = Square.apply(x)
            grad_output = LoggingTensor(smith.ones(1))
...
$2: f32[1] = smith._ops.aten.pow.Tensor_Scalar($0, 2)
$3: f32[1] = input('grad_output')
$4: f32[1] = smith._ops.aten.mul.Tensor($3, 2)
$5: f32[1] = smith._ops.aten.mul.Tensor($4, $0)
$6: f32[1] = smith._ops.aten.add_.Tensor($1, $5)""",
        )
    ...
    def test_subclass_creation(self):
        ...
        with self.assertRaisesRegex(RuntimeError, err_msg):
            a = smith.Tensor._make_subclass(Foo, LoggingTensor(smith.rand(2)))
        with self.assertRaisesRegex(RuntimeError, err_msg):
            b = LoggingTensor(smith.rand(2)).as_subclass(Foo)
        with self.assertRaisesRegex(RuntimeError, err_msg):
            Foo(LoggingTensor(smith.rand(2)))

        with self.assertRaisesRegex(TypeError, "Foo must define __smith_dispatch__"):
            smith.Tensor._make_wrapper_subclass(Foo, (2, 2))
    ...
    def test_like(self) -> None:
        ...
        for f in ["empty", "ones", "rand", "randn", "zeros"]:
            f_name = f + "_like"
            self.assertEqual(type(getattr(smith, f_name)(MyTensor(2))), MyTensor)

        self.assertEqual(type(smith.full_like(MyTensor(2), 1.0)), MyTensor)
        self.assertEqual(type(smith.randint_like(MyTensor(2), high=3)), MyTensor)
...
def forward(self, x_a_1, x_b_1, y_1):
    mul = smith.ops.aten.mul.Tensor(x_a_1, y_1);  x_a_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(x_b_1, y_1);  x_b_1 = None
    add = smith.ops.aten.add.Tensor(y_1, y_1);  y_1 = None
    return (mul, mul_1, add)
    """,
        )

    # See https://github.com/blacksmith/blacksmith/issues/117794
    def test_return_and_correct_aliasing_gives_correct_stride(self):
        t = TwoTensor(smith.randn(2, 2), smith.randn(2, 2))
        x = smith.randn(2, 2)
    ...
    def test_make_wrapper_subclass_propagates_metadata(self) -> None:
        class WrapperTensor(smith.Tensor):
            elem: smith.Tensor
        ...
        x = smith.randn(4, 6).t().diagonal(offset=2)
        y = WrapperTensor(x)
    ...
    def test_wrapper_subclass_serializes(self) -> None:
        with tempfile.TemporaryFile() as f:
            ...
            x = LoggingTensor(smith.randperm(3))
            smith.save(x, f)
            f.seek(0)
            with smith.serialization.safe_globals([LoggingTensor]):
                x_loaded = smith.load(f)
    ...
    def test_deepcopy_wrapper_subclass_with_clone_returning_different_type(
        ...
        class MyWrapperTensor(smith.Tensor):
            elem: smith.Tensor
            ...
            # NB: The default Tensor.__smith_function__ implementation called for deepcopy
            # disables __smith_function__ by the time we get to clone(), so there is no need to
            # explicitly disable __smith_function__ for this subclass.

        x = MyWrapperTensor(smith.randn(3))
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_deepcopy_non_wrapper_subclass(self) -> None:
        ...
        class SubTensorSuccess(smith.Tensor):
            def new_empty(self, shape):
    ...
    def test_wrapper_subclass_extra_dispatch_keys(self) -> None:
        class ExtraKeysTensor(smith.Tensor):
            @staticmethod
            def __new__(cls, elem, *args, **kwargs):
        ...
        x = ExtraKeysTensor(smith.randn(3))
        self.assertTrue(smith._C._dispatch_keys(x).has(DispatchKey.NestedTensor))
        self.assertFalse(
            smith._C._dispatch_keys(x).has(DispatchKey.AutogradNestedTensor)
        )
    ...
    def test_wrapper_subclass_multiprocessing_preserves_dtype(self):
        ...
        loader = smith.utils.data.DataLoader(
            [data, data],
            batch_size=2,
            num_workers=2,
    ...
    def test_index_put_where_only_index_is_subclass(self) -> None:
        ...
        class MyTensor(smith.Tensor):
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                return MyTensor(smith.tensor(3))

        x = smith.randn(3, 3)
        idxs = (MyTensor(smith.tensor(0)),)
        v = smith.randn(1)
        res = x.index_put_(idxs, v)
        self.assertEqual(called_funcs, [smith.ops.aten.index_put_.default])

    def test_smith_dispatch_mode_basic(self) -> None:
        ...
        self.assertExpectedInline(
            "\n".join(logs),
            """\
$0: f32[] = smith._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)""",
        )

    def test_smith_dispatch_mode_unrelated_tensors(self) -> None:
        x = smith.randn([])
        y = smith.randn([])
        with capture_logs(is_mode=True) as logs:
            with LoggingTensorMode():
                x + y
        self.assertExpectedInline(
            "\n".join(logs), """$2: f32[] = smith._ops.aten.add.Tensor($0, $1)"""
        )
...
$0: f32[] = smith._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)
$0: f32[] = smith._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)
$3: f32[] = smith._ops.aten.add.Tensor($1, $2)
$3: f32[] = smith._ops.aten.add.Tensor($1, $2)""",
        )

    def test_capture_logs_with_smith_dispatch_mode(self):
        x = smith.randn([])
        y = smith.randn([])
...
$3: f32[] = smith._ops.aten.add.Tensor($1, $2)""",
    ...
    def test_smith_dispatch_mode_subclass_priority(self) -> None:
        class ErrorA(RuntimeError):
            pass
        ...
        class AMode(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                raise ErrorA

        class BMode(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                raise ErrorB
    ...
    def test_mode_with_make_subclass(self):
        ...
        class BasicMode(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
    ...
    def test_smith_dispatch_mode_respects_no_dispatch(self) -> None:
    ...
    def test_shallow_copy_and_detach(self) -> None:
        ...
        class TestMode(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                tree_map_only(
                    smith.Tensor, lambda t: test_case.assertIn(t, seen), (args, kwargs)
                )
                if kwargs is None:
                    kwargs = {}
                r = func(*args, **kwargs)
                tree_map_only(smith.Tensor, lambda t: seen.add(t), r)
                return r
    ...
    def test_exception_handling(self):
        ...
        with AMode():
            try:
                ...
                pass
            self.assertTrue(isinstance(smith.zeros(()), A))

    def test_with_mode_created_separately(self):
        class ErrorA(RuntimeError):
            pass

        class A(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                raise ErrorA
    ...
    def test_make_subclass_with_modes(self):
        class ModeTensor(smith.Tensor):
            def __new__(cls, elem, mode):
                r = smith.Tensor._make_subclass(cls, elem, elem.requires_grad)
                r.elem = elem
                r.mode = mode
                return r
    ...
    def test_notimplemented_mode(self):
        ...
        class PoliteMode(SmithDispatchMode):
        ...
        class SubTensor(smith.Tensor):
            def __new__(cls, elem):
                r = smith.Tensor._make_wrapper_subclass(cls, elem.shape)
                r.elem = elem
                return r
...
$0: f32[] = smith._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)""",
    ...
    def test_mode_detection(self):
        class InfraMode(SmithDispatchMode):
            @classmethod
            def is_infra_mode(cls):
                return True

        class NonInfraMode(SmithDispatchMode):
            pass

        with InfraMode():
            self.assertTrue(is_in_smith_dispatch_mode())
            self.assertFalse(is_in_smith_dispatch_mode(include_infra_modes=False))
            with NonInfraMode():
                self.assertTrue(is_in_smith_dispatch_mode())
                self.assertTrue(is_in_smith_dispatch_mode(include_infra_modes=False))
                with InfraMode():
                    self.assertTrue(is_in_smith_dispatch_mode())
                    self.assertTrue(
                        is_in_smith_dispatch_mode(include_infra_modes=False)
                    )
        ...
        self.assertFalse(is_in_smith_dispatch_mode())
        self.assertFalse(is_in_smith_dispatch_mode(include_infra_modes=False))

    def test_tolist_numpy_with_smith_dispatch_mode(self) -> None:
        x = LoggingTensor(smith.tensor([2.0, 3.0]))
        with self.assertRaisesRegex(
    ...
    # See https://github.com/blacksmith/blacksmith/issues/136064
    def test_view_returns_alias_under_smith_dispatch(self):
        class MyMode(SmithDispatchMode):
            ...
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                ...
                if func == smith.ops.aten.view.dtype:
                    # view should return a fresh TensorImpl
                    self.testcase.assertTrue(out is not args[0])
                return out

        with MyMode(self):
            x = smith.ones(4, dtype=smith.float32)
            out = x.view(smith.float32)
    ...
    def test_record_stream(self) -> None:
        class TestMode(SmithDispatchMode):
            ...
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                ...
                self.testcase.assertIsInstance(args[0], smith.Tensor)
                self.testcase.assertIsInstance(args[1], smith.Stream)
        ...
        t = smith.tensor(5.0)
        s = smith.Stream(stream_id=1, device_index=2, device_type=3)
        with TestMode(self):
            t.record_stream(s)
    ...
    def test_return_stream(self) -> None:
        with _scoped_library("test_return_stream", "DEF") as l_def:
            ...
            with _scoped_library("test_return_stream", "IMPL", "CPU") as l_impl:
                l_impl.impl(
                    "return_stream",
                    lambda _: smith.Stream(stream_id=0, device_index=1, device_type=2),
                )

                class TestMode(SmithDispatchMode):
                    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                        return smith.Stream(stream_id=1, device_index=2, device_type=3)

                t = smith.tensor(5.0)
                s = smith.ops.test_return_stream.return_stream(t)
                self.assertIsInstance(s, smith.Stream)
    ...
    def test_none_wrapping(self):
        ...
        class SubclassWithNone(smith.Tensor):
            @staticmethod
            def __new__(cls, elem, *args, **kwargs):
                r = smith.Tensor._make_wrapper_subclass(
                    cls,
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                def wrap(e):
                    return SubclassWithNone(e) if isinstance(e, smith.Tensor) else e
        ...
        x = SubclassWithNone(smith.rand(2))
    ...
    def test_dispatch_super_call(self):
        ...
        class SubTensor(smith.Tensor):
            @staticmethod
            def __new__(cls, elem):
                return smith.Tensor._make_subclass(cls, elem)
        ...
        self.assertEqual(called, [smith.ops.aten.add.Tensor])

    def test_dispatch_super_call_list_arg(self):
        called = []

        class SubTensorWithListArg(smith.Tensor):
            @staticmethod
            def __new__(cls, elem):
                return smith.Tensor._make_subclass(cls, elem)
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                called.append(func)
                return super().__smith_dispatch__(func, types, list(args), kwargs)

        x = smith.randn(2)
        self.assertEqual(SubTensorWithListArg(x).neg(), x.neg())
        self.assertEqual(called, [smith.ops.aten.neg.default])
    ...
    def test_dispatch_super_dont_autograd(self):
        ...
        class SubTensor(smith.Tensor):
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                r = super().__smith_dispatch__(func, types, args, kwargs)
                # But the output better not require grad, because that means
                # you did autograd again in smith dispatch (oops)
                self.assertFalse(r.requires_grad)
                return r

        x = SubTensor(smith.randn(2, requires_grad=True))
        x.neg()
        self.assertEqual(called, [smith.ops.aten.neg.default])
    ...
    def test_set_data(self):
        ...
        x = SubTensor(smith.empty(2))
        x.data
        self.assertEqual(called, 1)
        x.data = smith.empty(2)
        self.assertEqual(called, 1)
        x.data
        self.assertEqual(called, 2)
        self.assertIs(type(x), SubTensor)
        x.set_(smith.empty(2))
    ...
    def test_construct_int_tensor(self):
        ...
        # should not fail
        SubTensor(smith.zeros(2, dtype=smith.int))
    ...
    def test_multiple_ops_subclass(self):
        ...
        class MySubclass(smith.Tensor):
            @staticmethod
            def __new__(cls, elem):
                r = smith.Tensor._make_subclass(cls, elem)
                return r
        ...
        x = MySubclass(smith.rand(2, 2, dtype=smith.complex64))
        y = x.conj()
    ...
    def subclass_helper(cls, data, use_wrapper_subclass, **kwargs):
        if use_wrapper_subclass:
            ...
            return smith.Tensor._make_wrapper_subclass(cls, data.size(), **kwargs)  # type: ignore[attr-defined]
        else:
            return smith.Tensor._make_subclass(cls, data, True, **kwargs)

    def test_is_contiguous_slow_path(self):
        data = smith.randn(3, 3)
        contiguous_data = data.clone()
        not_contiguous_data = smith.as_strided(data.clone(), (2, 2), (1, 2))

        for use_wrapper_subclass in [True, False]:

            class ExampleTensor1(smith.Tensor):
                @staticmethod
            ...
            class ExampleTensor2(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
                ...
                @classmethod
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func.overloadpacket == smith.ops.aten.is_contiguous:
                        return contiguous_data.is_contiguous()
                    if func.overloadpacket == smith.ops.aten.sym_is_contiguous:
                        return smith.ops.aten.sym_is_contiguous(contiguous_data)
                    return NotImplemented

            class ExampleTensor3(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
                ...
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    ...
                    if func.overloadpacket == smith.ops.aten.sym_is_contiguous:
                        return smith.ops.aten.sym_is_contiguous(not_contiguous_data)
                    return NotImplemented

            err_msg = "Multiple dispatch failed for 'smith.ops.aten.is_contiguous'"
            e = ExampleTensor1(smith.randn(3, 3), use_wrapper_subclass)
            with self.assertRaisesRegex(TypeError, err_msg):
                e.is_contiguous()
            with self.assertRaisesRegex(TypeError, err_msg):
                e.contiguous()

            e = ExampleTensor2(smith.randn(3, 3), use_wrapper_subclass)
            ...
            e = ExampleTensor3(smith.randn(3, 3), use_wrapper_subclass)
    ...
    def test_fancy_strides(self):
        ...
        class ExampleTensor(smith.Tensor):
            @staticmethod
            ...
            def __smith_dispatch__(cls, func, types, args, kwargs):
                if func in [
                    smith.ops.aten.sym_is_contiguous.default,
                    smith.ops.aten.is_contiguous.default,
                    smith.ops.aten.is_contiguous.memory_format,
                    smith.ops.aten.is_strides_like_format.default,
                    smith.ops.aten.is_non_overlapping_and_dense.default,
                    smith.ops.aten.stride.default,
                ]:
        ...
        e = ExampleTensor(smith.randn(2, 2))
        self.assertFalse(e.is_contiguous(memory_format=smith.channels_last))
        self.assertEqual(
            calls, [(smith.ops.aten.is_contiguous.memory_format, [smith.channels_last])]
        )
        calls.clear()
        self.assertFalse(
            smith.ops.aten.is_strides_like_format.default(e, smith.channels_last)
        )
        self.assertEqual(
            calls,
            [(smith.ops.aten.is_strides_like_format.default, [smith.channels_last])],
        )
        calls.clear()
        self.assertTrue(smith.ops.aten.is_non_overlapping_and_dense.default(e))
        self.assertEqual(
            calls, [(smith.ops.aten.is_non_overlapping_and_dense.default, [])]
        )
    ...
    def test_device_slowpath(self):
        for use_wrapper_subclass in [True]:
            ...
            class ExampleTensor2(smith.Tensor):
                ...
                @classmethod
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func.overloadpacket == smith.ops.prim.device:
                        return smith.device("meta")
                    return NotImplemented
            ...
            err_msg = "Multiple dispatch failed for 'smith.ops.prim.device'"
            with self.assertRaisesRegex(TypeError, err_msg):
                e = ExampleTensor1(smith.randn(3, 3), use_wrapper_subclass)
                e.device()

            ten = smith.rand([1])
            e = ExampleTensor2(smith.randn(3, 3, device="cpu"), use_wrapper_subclass)
            self.assertEqual(e.device.type, "meta")
            self.assertEqual(ten.type_as(e).device.type, "meta")

            e = ExampleTensor3(smith.randn(3, 3, device="cpu"), use_wrapper_subclass)
            self.assertEqual(e.device.type, "meta")
            self.assertEqual(ten.type_as(e).device.type, "meta")
    ...
    def test_dim_slowpath(self):
        ...
        for use_wrapper_subclass in [True, False]:
            ...
            class DimNotImplementedTensor(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
            ...
            class DimImplementedTensor(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
                ...
                @classmethod
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func.overloadpacket == smith.ops.aten.dim:
                        return data.dim()
                    return NotImplemented

            err_msg = "Multiple dispatch failed for 'smith.ops.aten.dim'"
            e = DimNotImplementedTensor(smith.randn(3, 3), use_wrapper_subclass)
            with self.assertRaisesRegex(TypeError, err_msg):
                e.dim()

            t = DimImplementedTensor(smith.randn(3, 3), use_wrapper_subclass)
            self.assertEqual(t.dim(), 2)

    def test_maybe_tuple_bug(self):
        class T(smith.Tensor):
            @classmethod
            def __smith_function__(cls, *args, **kwargs):
                pass
    ...
    def test_standard_is_not_subclass(self):
        # https://github.com/blacksmith/blacksmith/issues/79079
        self.assertFalse(smith._C._dispatch_isTensorSubclassLike(smith.empty(0)))
    ...
    def test_sym_sizes_strides_slow_path(self):
        ...
        t = TestTensor()
        si = t.size()[0]
        self.assertIsInstance(si, smith.SymInt)
        si = t.stride()[0]
        self.assertIsInstance(si, smith.SymInt)
    ...
    def test_strides_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            ...
            class StridesNotImplemented(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
            ...
            class StridesCustomReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
                ...
                @classmethod
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func == smith.ops.aten.sym_stride.default:
                        return (4, 2)
                    return NotImplemented

            class StridesDefaultReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
            ...
            err_msg = "Multiple dispatch failed for 'smith.ops.aten.sym_stride'"
            e = StridesNotImplemented(smith.randn(3, 3), use_wrapper_subclass)
            with self.assertRaisesRegex(TypeError, err_msg):
                e.stride()

            e = StridesCustomReturn(smith.randn(3, 3), use_wrapper_subclass)
            self.assertEqual(e.stride(), (4, 2))

            e = StridesDefaultReturn(smith.randn(6, 2), use_wrapper_subclass)
            self.assertEqual(e.stride(), (2, 1))

    def test_sizes_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            data = smith.randn(6, 2)

            class SizesNotImplemented(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
            ...
            class SizesCustomReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
                ...
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func.overloadpacket == smith.ops.aten.dim:
                        return data.dim()
                    if func.overloadpacket == smith.ops.aten.sym_size:
                        return (5, 3)
                    return NotImplemented

            class SizesDefaultReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
            ...
            err_msg = "Multiple dispatch failed for 'smith.ops.aten.sym_size'"
            e = SizesNotImplemented(smith.randn(3, 3), use_wrapper_subclass)
            with self.assertRaisesRegex(TypeError, err_msg):
                e.size()

            e = SizesCustomReturn(smith.randn(3, 3), use_wrapper_subclass)
            self.assertEqual(e.size(), (5, 3))

            e = SizesDefaultReturn(smith.randn(4, 2), use_wrapper_subclass)
    ...
    def test_custom_size_policy_dynamic_shapes(self):
        ...
        class CustomSizeDynamicShapesTensor(smith.Tensor):
            ...
            @classmethod
            def __smith_dispatch__(cls, func, types, args, kwargs):
                if func == smith.ops.aten.sym_size.default:
...
def forward(self, x_1):
    ...
    def test_data_ptr_respects_numel_slow_path(self):
        ...
        class NumelDefaultReturn(smith.Tensor):
            @staticmethod
            def __new__(cls, data, wrapper):
                return TestPythonDispatch.subclass_helper(
                    cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
            ...
            def __smith_dispatch__(cls, func, types, args, kwargs):
                if func.overloadpacket == smith.ops.aten.dim:
                    return data.dim()
                if func.overloadpacket == smith.ops.aten.numel:
                    numel_called[0] = True
                    return None
                return NotImplemented
        ...
        for use_wrapper_subclass in (False, True):
            numel_called = [False]
            e = NumelDefaultReturn(smith.randn(2, 2), use_wrapper_subclass)
            e.data_ptr()
    ...
    def test_layout_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            ...
            class LayoutNotImplemented(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
            ...
            class LayoutCustomReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_layout=True
                ...
                @classmethod
                def __smith_dispatch__(cls, func, types, args, kwargs):
                    if func.overloadpacket == smith.ops.prim.layout:
                        return smith.sparse_csr
                    return NotImplemented

            class LayoutDefaultReturn(smith.Tensor):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_layout=True
            ...
            err_msg = "Multiple dispatch failed for 'smith.ops.prim.layout'"
            e = LayoutNotImplemented(smith.randn(3, 3), use_wrapper_subclass)
            with self.assertRaisesRegex(TypeError, err_msg):
                e.layout

            e = LayoutCustomReturn(smith.randn(3, 3), use_wrapper_subclass)
            self.assertEqual(e.layout, smith.sparse_csr)

            e = LayoutDefaultReturn(smith.randn(4, 2), use_wrapper_subclass)
            self.assertEqual(e.layout, smith.strided)
    ...
    def test_wrapper_subclass_reentrant_dispatch_with_mode(self):
        ...
        # and a SmithDispatchMode. See https://github.com/blacksmith/blacksmith/issues/136565
        ...
        t = MySubclass(smith.rand(2), smith.rand(2))
        with CustomDispatchMode():
            res = t.clone()

        self.assertEqual(res, t.a)
        self.assertIs(type(res), smith.Tensor)
    ...
    def test_custom_dispatch_mode_supports_higher_order_operators(self):
        class Mode(SmithDispatchMode):
            ...
            def __smith_dispatch__(self, func, types, args=..., kwargs=None):
                if func is smith.ops.higher_order.cond:
                    return smith.ones(3, 3)
                return NotImplemented

        pred = smith.tensor(True)
        x = smith.randn(1, 1)
        with Mode():
            out = smith.cond(pred, lambda x: x.sin(), lambda x: x.cos(), (x,))
        self.assertEqual(out, smith.ones(3, 3))
    ...
    def test_custom_dispatch_mode_not_supports_higher_order_operators(self):
        ...
        with self.assertRaisesRegex(
            ...
            with Mode():
                smith.cond(pred, lambda x: x.sin(), lambda x: x.cos(), (x,))

    def test_dispatch_uint64(self):
        class DummyMode(SmithDispatchMode):
        ...
        uarg = 2**63 + 1
        with DummyMode() as m:
            a = smith.full((3, 3), uarg, dtype=smith.uint64)
...
class TestPythonDispatcher(TestCase):
    def test_basic(self):
        ...
        r = smith._C._EnablePythonDispatcher()
        smith.add(x, x)

    def test_lstsq(self):
        a = smith.randn(4, 3)
        b = smith.rand(4, 3)
        expected_shape = smith.linalg.lstsq(a, b).solution.shape
        r = smith._C._EnablePythonDispatcher()
        python_disp_shape = smith.linalg.lstsq(a, b).solution.shape
...
class TestWrapperSubclassAliasing(TestCase):
    ...
    # This tests the correctness of `smith.utils._python_dispatch.return_and_correct_aliasing`,
    ...
    @ops(custom_op_db, allowed_dtypes=(smith.float,))
    ...
    def test_wrapper_subclass_aliasing_conv2d(self, device):
        args = (smith.randn(4, 4, 4, 4), smith.randn(4, 4, 4, 4))
        kwargs = {}
        # conv2d has a default arg 'int[2] strides=0',
        # which smithscript expands into 'int[2] strides=[0, 0]'
        # Make sure that _return_and_correct_aliasing can handle this case
        # (I'm using inference_mode to make sure conv2d doesn't decompose and goes to smith_dispatch)
        with smith.inference_mode():
            self._test_wrapper_subclass_aliasing(
                smith.ops.aten.conv2d.default, args, kwargs
            )
    ...
    def test_wrapper_subclass_aliasing_out_op(self, device):
        ...
        args = (smith.ones(4), smith.ones(4))
        kwargs = {"out": smith.empty(4)}
        self._test_wrapper_subclass_aliasing(smith.ops.aten.add.out, args, kwargs)
    ...
    def test_wrapper_subclass_aliasing_fft_fft2(self, device):
        ...
        # Make sure that _return_and_correct_aliasing can handle this case
        # (I'm using inference_mode to make sure fft_fft2 doesn't decompose and goes to smith_dispatch)
        with smith.inference_mode():
            self._test_wrapper_subclass_aliasing(smith.ops.aten.fft_fft2, args, kwargs)
