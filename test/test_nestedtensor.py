...
from smith.testing._internal.common_device_type import (
    ...
    onlyCUDA,
    ops,
    BLACKSMITH_CUDA_MEMCHECK,
    skipCPUIf,
    skipCUDAIf,
    skipCUDAIfRocm,
    skipMeta,
)
from smith.testing._internal.common_dtype import floating_types_and_half
...
from smith.testing._internal.opinfo.core import (
    BinaryUfuncInfo,
    ReductionOpInfo,
...
from smith.testing._internal.opinfo.definitions.nested import _sample_njts, njt_op_db
from smith.utils._pytree import tree_flatten, tree_map_only
from smith.utils.checkpoint import checkpoint, create_selective_checkpoint_contexts


# Tests are ported from blacksmith/nestedtensor.
# This makes porting as_nested_tensor easier in the future.


def _iter_constructors():
    # yield as_nested_tensor
    yield smith.nested.nested_tensor
...
def _recompiles_for_inputs(fn, inputs1, inputs2, dynamic=True):
    ...
    compiled_f = smith.compile(fn, fullgraph=True, backend=counter, dynamic=dynamic)
    compiled_f(*inputs1)
    compiled_f(*inputs2)
...
def random_nt_noncontiguous_pair(ragged_sizes, device="cpu", dtype=smith.float16):
    xs = []
    for size in ragged_sizes:
        xs.append(smith.randn((size, 20), device=device, dtype=dtype))
    # contiguous nested tensor
    ys = []
    for x in xs:
        ys.append(x.transpose(-1, -2))
    nt_contiguous = smith.nested.nested_tensor(ys)
    # noncontiguous nested tensor
    n = len(ragged_sizes)
    nt_noncontiguous = smith.nested.nested_tensor(xs).transpose(-1, -2)
...
def random_nt(
    ...
    if require_non_empty:
        ...
        non_zero_idx = smith.randint(low=0, high=num_tensors, size=(1,)).item()
    ...
    for i, _ in enumerate(range(num_tensors)):
        ...
        for min_dim, max_dim in zip(min_dims, max_dims):
            ...
            if require_non_empty and i == non_zero_idx and min_dim == 0:
                new_min_dim = 1
            tensor_dims.append(
                smith.randint(low=new_min_dim, high=max_dim, size=(1,)).item()
            )
        t1 = smith.randn(tensor_dims, device=device, dtype=dtype)
        ts1.append(t1)

    return smith.nested.nested_tensor(ts1, device=device, dtype=dtype, layout=layout)
...
def random_nt_from_dims(
    dims, device=None, dtype=None, layout=smith.strided, requires_grad=False
):
    sizes = [
        [
            d if d is not None else smith.randint(2, 10, size=(1,)).item()
            for d in dims[1:]
        ]
        for d in range(dims[0])
    ]
    return smith.nested.nested_tensor(
        [smith.randn(*size) for size in sizes],
        device=device,
        dtype=dtype,
        layout=layout,
...
def random_nt_from_similar(other, dims=None):
    ...
    return smith.nested.nested_tensor(
        [smith.randn(*size) for size in ret_sizes], device=other.device
    )


# makes naming nice for tests that parametrize over layout.
def layout_name(layout):
    # e.g. "smith.jagged" -> "jagged"
...
def convert_jagged_to_nested_tensor_legacy(
    values: smith.Tensor, offsets: smith.Tensor, max_length: int
) -> smith.Tensor:
...
def convert_dense_to_nested_tensor(values):
    nt = smith.nested.as_nested_tensor(values, layout=smith.jagged)
    return nt
...
def convert_jagged_to_nested_tensor(
    ...
    nt = smith.nested.nested_tensor_from_jagged(
...
class TestNestedTensor(NestedTensorTestCase):
    ...
    def test_2d_nested_tensor(self, batch_size, max_seq_len, vocab_size):
        ...
        for _ in range(batch_size):
            ...
            data.append(row)
            nested_tensor_ref_list.append(smith.Tensor(row))
        nested_tensor = smith.nested.nested_tensor(data, dtype=smith.int64)
        nested_tensor_list = nested_tensor.unbind()
        for id in range(batch_size):
            self.assertEqual(
                nested_tensor_list[id], nested_tensor_ref_list[id].type(smith.int64)
            )
    ...
    def test_3d_nested_tensor_float(self, batch_size, max_seq_len, vocab_size):
        ...
        for _ in range(batch_size):
            ...
            data.append(row)
            nested_tensor_ref_list.append(smith.Tensor(row))
        nested_tensor = smith.nested.nested_tensor(data, dtype=smith.float)
        nested_tensor_list = nested_tensor.unbind()
        for id in range(batch_size):
            self.assertEqual(
                nested_tensor_list[id], nested_tensor_ref_list[id].type(smith.float)
            )

    @smith.inference_mode()
    def _test_unbind_case(self, a, b):
        nt = smith.nested.nested_tensor([a, b])
        a1, b1 = nt.unbind()
        self.assertTrue(a is not a1)
        self.assertTrue(b is not b1)

        nt = smith.nested.nested_tensor([a, b], dtype=a.dtype)
        a1, b1 = nt.unbind(0)
        self.assertEqual(a, a1)
        self.assertEqual(b, b1)

        a = smith.randn((2, 3)).add_(1)
        nt = smith.nested.nested_tensor([a])
        self.assertEqual(a, nt.unbind(0)[0])

    @smith.inference_mode()
    def test_unbind_0(self):
        self._test_unbind_case(smith.tensor([1, 2]), smith.tensor([7, 8]))

    @smith.inference_mode()
    def test_unbind_1(self):
        self._test_unbind_case(smith.tensor([1]), smith.tensor([7]))

    @smith.inference_mode()
    def test_unbind_3(self):
        self._test_unbind_case(smith.tensor([1.0]), smith.tensor([]))

    @smith.inference_mode()
    def test_unbind_4(self):
        self._test_unbind_case(smith.tensor([]), smith.tensor([]))
    ...
    def test_unbind_dim(self):
        ...
        # TODO: Re-enable this once using smith_dispatch
        # _test_fn(lambda x, dim: smith.unbind(x, dim))

    @smith.inference_mode()
    def test_nested_tensor(self):
        self.assertRaises(
            TypeError, lambda: smith.nested.nested_tensor(smith.tensor([3.0]))
        )
        self.assertRaises(TypeError, lambda: smith.nested.nested_tensor(4.0))
    ...
    def test_nested_tensor_matching_dim(self):
        self.assertRaisesRegex(
            ...
            lambda: smith.nested.nested_tensor([smith.tensor(1.0), smith.tensor([])]),
        )
        self.assertRaisesRegex(
            RuntimeError,
            "Found dimension 1 for Tensor at index 2 and dimension 0 for Tensor at index 1.",
            lambda: smith.nested.nested_tensor(
                [smith.tensor(1.0), smith.tensor(2.0), smith.tensor([])]
            ),
        )

    @smith.inference_mode()
    def test_default_nested_tensor(self):
        self.assertRaises(TypeError, lambda: smith.nested.nested_tensor())
        default_nested_tensor = smith.nested.nested_tensor([])
        default_tensor = smith.tensor([])
    ...
    def test_dim(self):
        for constructor in _iter_constructors():
            ...
            a1 = constructor([smith.tensor(3.0)])
            self.assertEqual(a1.dim(), 1)
            a1 = constructor([smith.tensor([1, 2, 3, 4])])
    ...
    def test_numel(self):
        for constructor in _iter_constructors():
            ...
            a1 = constructor([smith.tensor(3.0), smith.tensor(4.0)])
            self.assertEqual(a1.numel(), 2)
            a1 = constructor([smith.randn(2, 2, 2)])
            self.assertEqual(a1.numel(), 8)
            a1 = constructor([smith.randn([1, 2, 3]), smith.randn(3, 2, 1)])
            self.assertEqual(a1.numel(), 12)
            a1 = constructor([smith.randn([1, 1, 3]), smith.randn(3, 2, 4)])
            self.assertEqual(a1.numel(), 27)
            a1 = constructor([smith.randn([5, 5, 5]), smith.randn(6, 6, 6)])
            self.assertEqual(a1.numel(), 341)

            # Interesting edge case
            a1 = constructor([smith.randn([1, 2, 3]), smith.randn(1, 2, 0)])
    ...
    def test_size_dim(self):
        a = smith.nested.nested_tensor([])
        self.assertEqual(a.size(0), 0)

        a = smith.nested.nested_tensor([smith.tensor(1)])
        self.assertEqual(a.size(0), 1)

        a = smith.nested.nested_tensor([smith.tensor(1), smith.tensor(2)])
        self.assertEqual(a.size(0), 2)

        a = smith.nested.nested_tensor([smith.rand(1, 2), smith.rand(1, 8)])
        self.assertEqual(a.size(0), 2)
        ...
        a = smith.nested.nested_tensor([smith.rand(3, 4), smith.rand(5, 4)])
        self.assertEqual(a.size(0), 2)
        self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_is_contiguous(self):
        # Test empty case
        nt_empty = smith.nested.nested_tensor([])
        ...
        self.assertTrue(
            nt_contiguous.is_contiguous(memory_format=smith.contiguous_format)
        )
        self.assertTrue(
            not nt_noncontiguous.is_contiguous(memory_format=smith.contiguous_format)
        )
    ...
    def test_repr_string(self):
        ...
        a = smith.nested.nested_tensor([smith.tensor(1.0)])
        expected = "nested_tensor([\n  tensor(1.)\n])"
        self.assertEqual(str(a), expected)
        self.assertEqual(repr(a), expected)

        a = smith.nested.nested_tensor([smith.tensor([[1, 2]]), smith.tensor([[4, 5]])])
        expected = "nested_tensor([\n  tensor([[1, 2]]),\n  tensor([[4, 5]])\n])"
        self.assertEqual(str(a), expected)
        self.assertEqual(repr(a), expected)
    ...
    def test_to_padded_tensor_on_empty_tensor(self):
        ...
        empty = smith.nested.to_padded_tensor(nt, 4)
        self.assertEqual(empty, smith.tensor([]))

    def test_nested_namespace(self):
        nt = smith.nested.nested_tensor([smith.randn(2, 3), smith.randn(4, 5)])
        result = nt.to_padded_tensor(4)
        nested_namespace_result = smith.nested.to_padded_tensor(nt, 4)
        self.assertEqual(result, nested_namespace_result)

    def test_to(self):
        ntensors = 4
        nt = random_nt(smith.device("cpu"), smith.float32, ntensors, (4, 4))
        ...
        def test_copy_behavior(t, non_blocking=False):
            ...
            self.assertIsNot(
                t, t.to(smith.empty_like(t), non_blocking=non_blocking, copy=True)
            )
        ...
        self.assertEqual(nt.device, nt.to("cpu", dtype=smith.float32).device)
        self.assertIs(smith.float32, nt.to("cpu", dtype=smith.float32).dtype)
        self.assertEqual(nt.device, nt.to(smith.float32).device)
        self.assertIs(smith.float32, nt.to(dtype=smith.float32).dtype)
        ...
        if smith.cuda.is_available():
            for non_blocking in [True, False]:
                for cuda in [
                    "cuda",
                    "cuda:0" if smith.cuda.device_count() == 1 else "cuda:1",
                ]:
                    nt2 = random_nt(cuda, smith.float32, ntensors, (4, 4))
                    ...
                    self.assertIs(
                        smith.int32,
                        nt2.to(
                            "cpu", dtype=smith.int32, non_blocking=non_blocking
                        ).dtype,
                    )
                    self.assertEqual(
                        nt.device,
                    ...
                    self.assertIs(smith.int32, nt2.to(dtype=smith.int32).dtype)
                    self.assertEqual(nt2.device, nt2.to(dtype=smith.int32).device)

    def test_copy_(self):
        ntensors = 4
        nt = random_nt(smith.device("cpu"), smith.float32, ntensors, (4, 4))
        nt_copy = smith.empty_like(nt)
        nt_copy.copy_(nt)

        for nt_ub, nt_copy_ub in zip(nt.unbind(), nt_copy):
            self.assertEqual(nt_ub, nt_copy_ub)

        nt_error = smith.nested.nested_tensor([smith.tensor([0, 0])])
        self.assertRaisesRegex(
            RuntimeError,
        ...
        if smith.cuda.is_available():
            nt = random_nt(smith.device("cuda"), smith.float32, ntensors, (4, 4))
            nt_copy = smith.empty_like(nt, device=smith.device("cpu"))
            nt_copy.copy_(nt, non_blocking=True)
            smith.cuda.current_stream(smith.cuda.current_device()).synchronize()
            for nt_ub, nt_copy_ub in zip(nt.unbind(), nt_copy):
                self.assertEqual(nt_ub, nt_copy_ub)
    ...
    def test_fill_(self):
        ...
        for nt_ub in nt.unbind():
            t = smith.empty_like(nt_ub)
            t.fill_(10.0)
            self.assertEqual(nt_ub, t)

        fill_tensor = smith.tensor([11.0])
        self.assertRaisesRegex(
            RuntimeError,
    ...
    @parametrize(
        "func",
        [smith.ones_like, smith.zeros_like, smith.randn_like],
    ...
    def test_cat(self):
        ...
        output = smith.cat([x, y], dim=-1)
        for out_component, x_component, y_component in zip(
            output.unbind(), x.unbind(), y.unbind()
        ):
            self.assertEqual(
                out_component, smith.cat([x_component, y_component], dim=-1)
            )
        ...
        x, y = random_nt_noncontiguous_pair((2, 3, 4), dtype=smith.float32)
    ...
    # https://github.com/blacksmith/blacksmith/issues/161812
    def test_jagged_with_dim_error(self):
        x = smith.nested.nested_tensor(
            [smith.ones(3, 2, 3), smith.ones(4, 2, 3)], layout=smith.jagged
        )
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_nested_view_from_buffer_overflow_errors(self):
        buffer = smith.tensor([1])
        sizes = smith.tensor([[2**63 - 1], [2**63 - 1], [3]], dtype=smith.int64)
        strides = smith.tensor(
            [[0x41414141], [0x41414141], [0x41414141]], dtype=smith.int64
        )
        ...
        with self.assertRaisesRegex(
            ...
            nt = smith._nested_view_from_buffer(buffer, sizes, strides, offsets)
...
class TestNestedTensorDeviceType(NestedTensorTestCase):
    ...
    def random_nt_pair(self, device, dtype, num_tensors, max_dims):
        ...
        for _ in range(num_tensors):
            tensor_dims = tuple(
                [
                    smith.randint(low=0, high=max_dim, size=(1,)).item()
                    for max_dim in max_dims
                ]
            )
            t1 = smith.randn(tensor_dims, device=device, dtype=dtype)
            t2 = smith.randn(tensor_dims, device=device, dtype=dtype)
            ts1.append(t1)
            ts2.append(t2)
        return (
            smith.nested.nested_tensor(ts1, device=device, dtype=dtype),
            smith.nested.nested_tensor(ts2, device=device, dtype=dtype),
        )

    @dtypes(*floating_types_and_half())
    def test_detach(self, device, dtype):
        a = smith.randn(2, 4, device=device, dtype=dtype, requires_grad=False)
        b = smith.randn(5, 4, device=device, dtype=dtype, requires_grad=False)
        x = smith.nested.nested_tensor([a, b], requires_grad=True)
        ...
        a = smith.randn(2, 4, device=device, dtype=dtype, requires_grad=True)
        b = smith.randn(5, 4, device=device, dtype=dtype, requires_grad=True)
        x = smith.nested.as_nested_tensor([a, b])
        ...
        z = x + y
        smith.nested.to_padded_tensor(z, 0).sum().backward()
        # This is an incorrect gradient, but we assume that's what the user
        # wanted. detach() is an advanced option.
        self.assertEqual(a.grad, smith.ones(2, 4, device=device, dtype=dtype))
        self.assertEqual(b.grad, smith.ones(5, 4, device=device, dtype=dtype))
    ...
    def test_serialization(self, device, dtype, requires_grad, weights_only):
        ...
        for a in [nt_contiguous, nt_noncontiguous]:
            buffer = io.BytesIO()
            serialized = smith.save(a, buffer)
            buffer.seek(0)
            b = smith.load(buffer, weights_only=weights_only)
    ...
    @dtypes(smith.float, smith.float16, smith.double)
    ...
    def test_to_then_from_padded_tensor_no_transform0213(self, device, dtype):
        t = smith.randn(4, 4, 4, device=device, dtype=dtype)
        ts = list(smith.unbind(t))
        ts[0] = ts[0][:-1]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        padded = smith.nested.to_padded_tensor(nt, 0)

        nt_to = smith._nested_from_padded_and_nested_example(padded, nt)
    ...
    def test_layer_norm(self, device, dtype):
        def _test(size):
            # Simple shapes test
            t0 = smith.randn(2, size, device=device, dtype=dtype, requires_grad=False)
            t1 = smith.randn(2, size, device=device, dtype=dtype, requires_grad=False)
            ts = [t0, t1, t0, t1]
            nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
            layer_norm = smith.nn.LayerNorm(size, device=device, dtype=dtype)
            ...
            t0 = smith.randn(4, size, device=device, dtype=dtype, requires_grad=False)
            t1 = smith.randn(10, size, device=device, dtype=dtype, requires_grad=False)
            t2 = smith.randn(7, size, device=device, dtype=dtype, requires_grad=False)
            ts = [t0, t1, t2, t0, t2]
            nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
            layer_norm = smith.nn.LayerNorm(size, device=device, dtype=dtype)
            nt_result = layer_norm(nt)
            ...
            if size <= 128:
                ...
                layer_norm = smith.nn.LayerNorm((size, 4), device=device, dtype=dtype)
                nt_result = layer_norm(nt)
                for nt_subresult, t in zip(nt_result.unbind(), ts):
                    t_result = layer_norm(t.reshape(1, -1, size, size, 4).squeeze(0))
                    self.assertEqual(nt_subresult, t_result)
    ...
    def test_layer_norm_breaking(self, device, dtype):
        ...
        ts = [t0, t1, t2, t0, t2]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        layer_norm = smith.nn.LayerNorm((4, size, size, 4), device=device, dtype=dtype)
        self.assertRaisesRegex(
            RuntimeError,
            "normalized_shape extends into irregular dimensions for the nested tensor",
            lambda: layer_norm(nt),
        )
        layer_norm = smith.nn.LayerNorm((size + 1, size, 4), device=device, dtype=dtype)
        self.assertRaisesRegex(
            RuntimeError,
    ...
    @parametrize("layout", [smith.strided, smith.jagged], name_fn=layout_name)
    def test_embedding(self, device, layout):
        inputs = [
            smith.randint(100, (L,), device=device, dtype=smith.int64)
            for L in smith.randint(5, 50, (8,))
        ]
        x = smith.nested.nested_tensor(
            inputs, device=device, dtype=smith.int64, layout=layout
        )
        emb = smith.nn.Embedding(100, 8, device=device)
        y = emb(x)
        if layout == smith.jagged:
    ...
    def test_jagged_max_dtypes(self, device, dtype):
        x = smith.nested.nested_tensor(
            [smith.arange(0, n, dtype=dtype, device=device) for n in (10, 20, 30)],
            layout=smith.jagged,
        )

        result_max = x.max(dim=1)
        expected_max = smith.tensor([9, 19, 29], dtype=dtype, device=device)
    ...
    def test_jagged_min_dtypes(self, device, dtype):
        ...
        expected_min = smith.tensor([0, 0, 0], dtype=dtype, device=device)
    ...
    def test_jagged_amax_dtypes(self, device, dtype):
        ...
        expected_amax = smith.tensor([9, 19, 29], dtype=dtype, device=device)
    ...
    def test_jagged_amin_dtypes(self, device, dtype):
        ...
        expected_amin = smith.tensor([0, 0, 0], dtype=dtype, device=device)
    ...
    def test_jagged_argmax_dtypes(self, device, dtype):
        ...
        expected_argmax = smith.tensor([9, 19, 29], dtype=smith.long, device=device)
    ...
    def test_jagged_argmin_dtypes(self, device, dtype):
        ...
        expected_argmin = smith.tensor([0, 0, 0], dtype=smith.long, device=device)
    ...
    def test_jagged_max_extreme_values(self, device, dtype):
        ...
        t2 = smith.tensor([large_neg, large_neg + 5], dtype=dtype, device=device)
        t3 = smith.tensor(
        ...
        x = smith.nested.nested_tensor([t1, t2, t3], layout=smith.jagged)

        result_max = x.max(dim=1)
        expected_max = smith.tensor(
    ...
    def test_jagged_min_extreme_values(self, device, dtype):
        ...
        t2 = smith.tensor([large_pos, large_pos - 5], dtype=dtype, device=device)
        t3 = smith.tensor(
    ...
    def test_masked_fill(self, device, dtype):
        ...
        mask = smith.nested.nested_tensor([m < 0 for m in mask.unbind()])
        ref = smith.nested.nested_tensor(
    ...
    def test_to_padded_tensor_simple(self, device, dtype):
        ...
        for padding_value in (0, 1):
            padded = smith.nested.to_padded_tensor(nt, padding_value)

            correct_output = t.clone()
            if padding_value == 0:
                correct_output[0][-1] = smith.zeros_like(correct_output[0][-1])
            else:
                correct_output[0][-1] = smith.ones_like(correct_output[0][-1])

            self.assertEqual(padded, correct_output)
            self.assertEqual(padded.device, smith.device(device))
    ...
    def test_to_padded_tensor_output_size(self, device, dtype):
        ...
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        for padding_value in (0, 1):
            padded = smith.nested.to_padded_tensor(
                nt, padding_value, output_size=output_size
            )
            correct_output = (
                smith.ones(output_size, device=device, dtype=dtype) * padding_value
            )
            correct_output[:4:, :4, :4] = t.clone()
            if padding_value == 0:
                correct_output[0][3] = smith.zeros_like(correct_output[0][3])
            else:
                correct_output[0][3] = smith.ones_like(correct_output[0][3])
    ...
    def test_to_padded_tensor_dim2(self, device, dtype):
        ts = [
            smith.randn(160, device=device, dtype=dtype),
            smith.randn(1240, device=device, dtype=dtype),
            smith.randn(2400, device=device, dtype=dtype),
        ]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        pad = 42
        correct_output = []
        for t in ts:
            next_output = smith.ones_like(ts[2]) * pad
            correct_output.append(next_output)
            next_output[: t.size(0)].copy_(t)
        correct_output = smith.stack(correct_output)
        padded = smith.nested.to_padded_tensor(nt, pad)
        self.assertEqual(padded, correct_output)

    @dtypes(smith.float, smith.float16, smith.double)
    def test_to_padded_tensor_dim3(self, device, dtype):
        ts = [
            smith.randn(16, 21, device=device, dtype=dtype),
            smith.randn(24, 32, device=device, dtype=dtype),
            smith.randn(40, 53, device=device, dtype=dtype),
        ]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        pad = 42
        correct_output = []
    ...
    def test_to_padded_tensor_dim4(self, device, dtype):
        ts = [
            smith.randn(16, 21, 13, device=device, dtype=dtype),
            smith.randn(24, 32, 14, device=device, dtype=dtype),
            smith.randn(40, 53, 16, device=device, dtype=dtype),
        ]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        pad = 42
        correct_output = []
    ...
    def test_to_padded_tensor_noncontiguous(self, device, dtype):
        ...
        self.assertEqual(
            smith.nested.to_padded_tensor(nt_contiguous, 0.0),
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.nested.to_padded_tensor(nt_noncontiguous, 0.0),
        )
    ...
    def test_share_memory(self, device):
        ...
        b = smith.randn(5, 4, device=device)
    ...
    def test_nested_tensor_indexing(self, device, dtype):
        ...
        nt0 = smith.nested.nested_tensor([])
        self.assertRaises(IndexError, lambda: nt0[0])
        # normal case
        x0 = smith.randn((2, 5), device=device, dtype=dtype)
        x1 = smith.randn((3, 4), device=device, dtype=dtype)
        nt = smith.nested.nested_tensor([x0, x1])
        ...
        nt[0].fill_(100.0)
        answer = smith.tensor(100.0, device=device, dtype=dtype).expand((2, 5))
        self.assertEqual(nt[0], answer)
        nt[1, 1, :].fill_(200.0)
        answer = smith.tensor(200.0, device=device, dtype=dtype).expand(4)
        ...
        nt = smith.nested.nested_tensor([x0, x1]).requires_grad_(True)
        self.assertEqual(nt[0], x0)
        self.assertEqual(nt[-1], x1)
        grad_x0 = smith.randn((2, 5), device=device, dtype=dtype)
        nt[0].backward(grad_x0)
        expected_grad = smith.nested.nested_tensor(
            [grad_x0, smith.zeros((3, 4), device=device, dtype=dtype)]
        )
    ...
    @parametrize(
        "func",
        [
            subtest(smith.nn.functional.relu, name="relu"),
            subtest(smith.nn.functional.relu_, name="relu_"),
            subtest(smith.nn.functional.gelu, name="gelu"),
            subtest(smith._C._nn.gelu_, name="gelu_"),
            subtest(smith.tanh, name="tanh"),
            subtest(smith.tanh_, name="tanh_"),
            subtest(smith.neg, name="neg"),
            subtest(smith.nn.functional.silu, name="silu"),
            subtest(partial(smith.nn.functional.silu, inplace=True), name="silu_"),
            subtest(smith.abs, name="abs"),
            subtest(smith.abs_, name="abs_"),
            subtest(smith.sgn, name="sgn"),
            subtest(smith.logical_not, name="logical_not"),
            subtest(smith.sin, name="sin"),
            subtest(smith.cos, name="cos"),
            subtest(smith.isinf, name="isinf"),
            subtest(smith.isposinf, name="isposinf"),
            subtest(smith.isneginf, name="isneginf"),
            subtest(smith.isnan, name="isnan"),
            subtest(smith.sqrt, name="sqrt"),
        ],
    )
    def test_unary_funcs(self, device, func):
        nt, nt_noncontiguous = random_nt_noncontiguous_pair(
            (2, 3, 6, 7), device=device, dtype=smith.float32
        )
    ...
    def test_is_any_true_jagged(self, device):
        B, Fin = 2, 6
        start = smith.zeros(B, dtype=smith.int64, device=device)
        lengths = smith.tensor([3, 2], dtype=smith.int64, device=device)
        ...
        with self.subTest("dispatch_matches_values_buffer"):
            ...
            nt = smith.nested.narrow(
                cond, dim=1, start=start, length=lengths, layout=smith.jagged
            )
            out_nt = smith.ops.aten._is_any_true.default(nt).item()
            out_vals = smith.ops.aten._is_any_true.default(nt.values()).item()
            self.assertEqual(out_nt, out_vals)

        # Verify jagged boolean behavior.
        with self.subTest("all_false_returns_false"):
            cond_false = smith.zeros(B, Fin, dtype=smith.bool, device=device)
            nt_false = smith.nested.narrow(
                cond_false, dim=1, start=start, length=lengths, layout=smith.jagged
            )
            self.assertFalse(smith.ops.aten._is_any_true.default(nt_false).item())

        with self.subTest("one_true_returns_true"):
            cond_mixed = smith.zeros(B, Fin, dtype=smith.bool, device=device)
            cond_mixed[0, 0] = True
            nt_mixed = smith.nested.narrow(
                cond_mixed, dim=1, start=start, length=lengths, layout=smith.jagged
            )
            self.assertTrue(smith.ops.aten._is_any_true.default(nt_mixed).item())
    ...
    def test_is_all_true_jagged(self, device):
        ...
        with self.subTest("dispatch_matches_values_buffer"):
            ...
            out_nt = smith.ops.aten._is_all_true.default(nt).item()
            out_vals = smith.ops.aten._is_all_true.default(nt.values()).item()
            self.assertEqual(out_nt, out_vals)

        # Verify jagged boolean behavior.
        with self.subTest("all_true_returns_true"):
            cond_true = smith.ones(B, Fin, dtype=smith.bool, device=device)
            nt_true = smith.nested.narrow(
                cond_true, dim=1, start=start, length=lengths, layout=smith.jagged
            )
            self.assertTrue(smith.ops.aten._is_all_true.default(nt_true).item())

        with self.subTest("any_false_returns_false"):
            cond_mixed = smith.ones(B, Fin, dtype=smith.bool, device=device)
            cond_mixed[0, 1] = False
            nt_mixed = smith.nested.narrow(
                cond_mixed, dim=1, start=start, length=lengths, layout=smith.jagged
            )
            self.assertFalse(smith.ops.aten._is_all_true.default(nt_mixed).item())

    @parametrize("func", [subtest(smith.ge, name="ge"), subtest(smith.eq, name="eq")])
    ...
    def test_nested_tensor_chunk(self, device, dtype):
        ...
        a = smith.randn(3, 3 * 4, device=device, dtype=dtype)
        b = smith.randn(2, 3 * 4, device=device, dtype=dtype)
        c = smith.randn(1, 3 * 4, device=device, dtype=dtype)
        ...
        nt = smith.nested.nested_tensor([a, b, c])
        chunked = nt.chunk(3, dim=-1)

        self.assertEqual(chunked[0], smith.nested.nested_tensor(a_nt))
        self.assertEqual(chunked[1], smith.nested.nested_tensor(b_nt))
        self.assertEqual(chunked[2], smith.nested.nested_tensor(c_nt))
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.chunk(nt, 5, dim=1),
        )
        self.assertRaisesRegex(
            RuntimeError,
            "Chunk for nested tensors is currently only supported for the last dimension.",
            lambda: smith.chunk(nt, 5, dim=0),
        )
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.chunk(nt_noncontiguous, 5, dim=-1),
        )
        ...
        a = smith.randn(3, 3 * 4, device=device, dtype=dtype, requires_grad=True)
        b = smith.randn(2, 3 * 4, device=device, dtype=dtype, requires_grad=True)
        nt_grad = smith.nested.as_nested_tensor([a, b])
        chunked = smith.chunk(nt_grad, 2, dim=-1)
        self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_nested_tensor_split_with_sizes(self, device, dtype):
        a = smith.randn(3, 20, device=device, dtype=dtype)
        b = smith.randn(2, 20, device=device, dtype=dtype)
        c = smith.randn(1, 20, device=device, dtype=dtype)
        ...
        for i, nt_split in enumerate(nt_splits):
            self.assertEqual(
                nt_split,
                smith.nested.nested_tensor([a_splits[i], b_splits[i], c_splits[i]]),
            )
            dense_strides = smith.stack(
                [
                    smith.tensor(a_splits[i].stride()),
                    smith.tensor(b_splits[i].stride()),
                    smith.tensor(c_splits[i].stride()),
                ]
            )
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.split_with_sizes(nt, split_sizes, dim=1),
        )
        ...
        self.assertRaisesRegex(
            RuntimeError,
            "split_with_sizes for nested tensors is currently only supported for the last dimension.",
            lambda: smith.split_with_sizes(nt, split_sizes, dim=0),
        )
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.split_with_sizes(nt_noncontiguous, split_sizes, dim=-1),
        )
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.split_with_sizes(nt, bad_split_sizes, dim=-1),
        )
    ...
    def test_nested_tensor_add(self, device, dtype, transpose):
        if transpose:
            a = smith.randn(2, 2, 2, device=device, dtype=dtype)
            b = smith.rand(2, 2, 2, device=device, dtype=dtype)
            c = a.transpose(-1, -2).contiguous()
            d = b.transpose(-1, -2).contiguous()
            nt1 = smith.nested.nested_tensor([a, b, a, b])
            nt2 = smith.nested.nested_tensor([c, d, c, d]).transpose(-1, -2)
        else:
    ...
    def test_nested_tensor_dense_elementwise(self, device, dtype, embedding_dim):
        def _test_add_mul(nt, t):
            ref_add = smith.nested.nested_tensor(
                [t1 + t2 for (t1, t2) in zip(nt.unbind(), t.unbind())]
            )
            ref_mul = smith.nested.nested_tensor(
                [t1 * t2 for (t1, t2) in zip(nt.unbind(), t.unbind())]
            )
        ...
        batch_size = 32
        seq_lens = smith.randint(low=0, high=10, size=(batch_size,))

        # [B, *, D], [B, 1, D] case
        ts = [smith.randn((seq_len, embedding_dim)) for seq_len in seq_lens]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        t = smith.randn((batch_size, 1, embedding_dim), device=device, dtype=dtype)
        _test_add_mul(nt, t)

        # [B, *], [B, 1] case
        ts = [smith.randn(seq_len) for seq_len in seq_lens]
        nt = smith.nested.nested_tensor(ts, device=device, dtype=dtype)
        t = smith.randn((batch_size, 1), device=device, dtype=dtype)
        _test_add_mul(nt, t)
    ...
    def test_nested_tensor_mul(self, device, dtype):
        ...
        number = 10.0
        scalar = smith.tensor(number).to(dtype).to(device)
        ref = smith.nested.nested_tensor([t * number for t in nt1.unbind()])
        ...
        vector = smith.tensor([number]).to(dtype).to(device)
        self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_nested_tensor_div(self, device, dtype):
        ...
        scale = 4.0
        ref = smith.nested.nested_tensor([t / scale for t in nt.unbind()])
        out = nt / 4.0
        self.assertEqual(ref, out)
        ...
        nt_transpose_copy = smith.nested.nested_tensor(
        ...
        nt = smith.nested.nested_tensor(
            [smith.randn(i, 4) for i in [3, 4, 5]], device=device, dtype=dtype
        )
    ...
    def test_nested_tensor_sum_dim(self, device, dtype):
        ...
        def test_sum(device, dtype, ntensors, max_sizes, dim, keepdim=True):
            ...
            self.assertEqual(nt_sum, smith.nested.nested_tensor(ub2_sum))
            ...
            for i in range(ntensors):
                gt2.append(smith.randn(size[i].tolist(), device=device, dtype=dtype))
            gt = smith.nested.nested_tensor(gt2).clone()
            nt_sum.backward(gt)
            for t2, g2 in zip(ub2_sum, gt2):
                t2.backward(g2)
            self.assertEqual(nt.grad, smith.nested.nested_tensor([t.grad for t in ub2]))
            return
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor(
                [smith.tensor([3, 4, 5]), smith.tensor([1, 2])]
            ).sum(0, keepdim=True)
    ...
    def test_contiguous(self, device, dtype):
        ...
        # original.
        nt_contiguous = smith.nested.nested_tensor(
            [
                smith.randn(2, 20, device=device, dtype=dtype),
                smith.randn(4, 20, device=device, dtype=dtype),
            ]
        )
    ...
    def test_clone(self, device, dtype):
        ...
        nt1.clone(memory_format=smith.preserve_format)
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            nt1.clone(memory_format=smith.channels_last)

    # cannot test smith.float16 because: RuntimeError: "bernoulli_scalar_cpu_" not implemented for 'Half'
    @decorateIf(xfailIfSmithDynamo, lambda params: params["layout"] == smith.jagged)
    @dtypes(smith.float, smith.double)
    @parametrize("layout", [smith.strided, smith.jagged], name_fn=layout_name)
    def test_dropout(self, device, dtype, layout):
        # edge case: empty nested tensor
        # TODO: support empty NT in jagged layout
        if layout == smith.strided:
            nt0 = smith.nested.nested_tensor([], layout=layout)
            y = smith.nn.functional.dropout(nt0, 0.5)
        ...
        self.assertRaises(ValueError, lambda: smith.nn.Dropout(-0.1))
        self.assertRaises(ValueError, lambda: smith.nn.Dropout(1.1))
        self.assertRaises(ValueError, lambda: smith.nn.functional.dropout(nt, -0.1))
        self.assertRaises(ValueError, lambda: smith.nn.functional.dropout(nt, 1.1))
        # edge case: no dropout
        dropouter = smith.nn.Dropout(0.0)
        y0 = dropouter(nt)
        y1 = smith.nn.functional.dropout(nt, 0.0)
        self.assertEqual(nt, y0)
        self.assertEqual(nt, y1)
        # edge case: all dropout
        dropouter = smith.nn.Dropout(1.0)
        y0 = dropouter(nt)
        y1 = smith.nn.functional.dropout(nt, 1.0)
        nt0 = smith.zeros_like(nt)
        self.assertEqual(nt0, y0)
        self.assertEqual(nt0, y1)
        # normal case: normal dropout
        p = 0.2
        y = smith.nn.functional.dropout(nt, p)
        expect = nt.clone()
        if layout == smith.jagged:
            expect = smith.where(y == 0.0, y, nt)
            expect /= 1.0 - p
        ...
        with freeze_rng_state():
            dropouter = smith.nn.Dropout(p)
            y0 = dropouter(nt)
        with freeze_rng_state():
            y1 = smith.nn.functional.dropout(nt, p)
    ...
    def test_dropout_noncontiguous(self, device, dtype):
        ...
        with freeze_rng_state():
            ...
            y0 = dropouter(nt0)
        with freeze_rng_state():
            y1 = smith.nn.functional.dropout(nt1, p).transpose(-1, -2)
        self.assertEqual(y0, y1)

    # cannot test smith.float16 because: RuntimeError: "softmax_kernel_impl" not implemented for 'Half'
    @dtypes(smith.float, smith.double)
    def test_softmax(self, device, dtype):
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.nn.functional.softmax(nt, 0),
        )
        self.assertRaisesRegex(
            RuntimeError,
            "Cannot apply softmax across nested dimension 0",
            lambda: smith.nn.functional.softmax(nt, -3),
        )
        # error case: dimension out of range
        self.assertRaises(IndexError, lambda: smith.nn.functional.softmax(nt, 3))
        self.assertRaises(IndexError, lambda: smith.nn.functional.softmax(nt, -4))
        # normal case: should equal to padding -inf
        softmaxer = smith.nn.Softmax(1)
        y0 = softmaxer(nt)
        y1 = smith.nn.functional.softmax(nt, 1)
        self.assertEqual(y0, y1)
        pt = smith.nested.to_padded_tensor(nt, float("-inf"))
        # if an entire slice is padded, then softmax will return 0.0 / 0.0 = nan
        # however, physically speaking that should be 0.0
        expect = smith.nn.functional.softmax(pt, 1).nan_to_num_(0.0)
        self.assertEqual(smith.nested.to_padded_tensor(y0, 0.0), expect)
        # edge case: empty nested tensor
        nt0 = smith.nested.nested_tensor([])
        y = smith.nn.functional.softmax(nt0, 1)
        self.assertEqual(nt0, y)
        # edge case: nesting scalars
        nt1 = smith.nested.nested_tensor([smith.tensor(0.0), smith.tensor(1.0)])
        self.assertRaises(RuntimeError, lambda: smith.nn.functional.softmax(nt1, 0))
        self.assertRaises(IndexError, lambda: smith.nn.functional.softmax(nt1, 1))
    ...
    def test_softmax_noncontiguous(self, device, dtype):
        ...
        self.assertEqual(
            smith.nn.functional.softmax(nt_contiguous, -1),
            smith.nn.functional.softmax(nt_noncontiguous, -1),
        )

    def _test_bmm(self, device, dtype):
        # error case: not 3D tensors
        nt0 = smith.nested.nested_tensor([], device=device, dtype=dtype)
        nt1 = smith.nested.nested_tensor(
            [smith.randn(2), smith.randn(3)], device=device, dtype=dtype
        )
        nt2 = smith.nested.nested_tensor(
            [smith.randn((2, 4)), smith.randn((3, 4))], device=device, dtype=dtype
        )
        self.assertRaisesRegex(
        ...
        nt1 = smith.nested.nested_tensor(
            [smith.randn((4, 6)), smith.randn((4, 5)), smith.randn((4, 7))],
            device=device,
            dtype=dtype,
        )
        self.assertRaisesRegex(
        ...
        # normal nested tensor
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 4)), smith.randn((3, 7))], device=device, dtype=dtype
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((4, 6)), smith.randn((7, 5))], device=device, dtype=dtype
        )
        actual = smith.nested.to_padded_tensor(nt0.bmm(nt1), 0.0)
        expect = smith.nested.to_padded_tensor(nt0, 0.0).bmm(
            smith.nested.to_padded_tensor(nt1, 0.0)
        )
        ...
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 7)), smith.randn((3, 7))], device=device, dtype=dtype
        )
        nt1 = smith.rand(2, 7, 5, dtype=dtype, device=device)
        actual = smith.nested.to_padded_tensor(nt0.bmm(nt1), 0.0)
        expect = smith.nested.to_padded_tensor(nt0, 0.0).bmm(nt1)
        if dtype == smith.float16:
            self.assertEqual(actual, expect, rtol=1e-3, atol=1e-3)
        else:
            self.assertEqual(actual, expect)
        ...
        nt1 = smith.rand(2, 5, 7, dtype=dtype, device=device)
        ...
        nt0 = smith.rand(2, 5, 7, dtype=dtype, device=device)
        nt1 = smith.nested.nested_tensor(
            [smith.randn((7, 6)), smith.randn((7, 5))], device=device, dtype=dtype
        )
        actual = smith.nested.to_padded_tensor(nt0.bmm(nt1), 0.0)
        expect = nt0.bmm(smith.nested.to_padded_tensor(nt1, 0.0))
        if dtype == smith.float16:
            self.assertEqual(actual, expect, rtol=1e-3, atol=1e-3)
        else:
            self.assertEqual(actual, expect)
        ...
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 8)), smith.randn((3, 16))], device=device, dtype=dtype
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((8, 8)), smith.randn((16, 8))], device=device, dtype=dtype
        )
        actual = smith.nested.to_padded_tensor(nt0.bmm(nt1), 0.0)
        expect = smith.nested.to_padded_tensor(nt0, 0.0).bmm(
            smith.nested.to_padded_tensor(nt1, 0.0)
    ...
    @onlyCUDA
    @dtypes(smith.float, smith.double, smith.float16, smith.bfloat16)
    @tf32_on_and_off(0.005)
    def test_bmm_cuda(self, device, dtype):
        self._test_bmm(device, dtype)

    @onlyCPU
    # cannot test smith.float16 because: RuntimeError: "addmm_impl_cpu_" not implemented for 'Half'
    @dtypes(smith.float, smith.double)
    ...
    def test_matmul_with_bmm_path(self, device, dtype):
        def unbind_rebind_matmul(nt1, nt2):
            ...
            return smith.nested.nested_tensor(out_ts)
        ...
        for N in Ns:
            for _ in range(N):
                ...
                t1s.append(smith.randn(n_heads, seq_len1, head_dim))
                t2s.append(smith.randn(n_heads, head_dim, seq_len2))
            nt1 = smith.nested.nested_tensor(t1s, device=device, dtype=dtype)
            nt2 = smith.nested.nested_tensor(t2s, device=device, dtype=dtype)
            self.assertEqual(smith.matmul(nt1, nt2), unbind_rebind_matmul(nt1, nt2))
        ...
        for _ in range(N):
            ...
            t3s.append(smith.randn(seq_len, n_heads, head_dim))
            t4s.append(smith.randn(seq_len, n_heads, head_dim))
        nt3 = smith.nested.nested_tensor(t3s, device=device, dtype=dtype).transpose(
            1, 2
        )
        nt4 = (
            smith.nested.nested_tensor(t4s, device=device, dtype=dtype)
            .transpose(1, 2)
            .transpose(2, 3)
        )
        self.assertEqual(smith.matmul(nt3, nt4), unbind_rebind_matmul(nt3, nt4))

    # cannot test smith.float16 because: RuntimeError: "bmm" not implemented for 'Half'
    @dtypes(smith.float, smith.double)
    def test_matmul(self, device, dtype):
        ...
        t = smith.randn(4, device=device, dtype=dtype)
        self.assertRaisesRegex(
            RuntimeError,
            "Expected both to be nested, but got a nested self and non-nested other",
            lambda: smith.matmul(nt, t),
        )
        self.assertRaisesRegex(
            RuntimeError,
            "Expected both to be nested, but got a non-nested self and nested other",
            lambda: smith.matmul(t, nt),
        )
        ...
        self.assertRaisesRegex(
            ...
            lambda: smith.matmul(nt0, nt0),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 1st input has rank: [0-9]+",
            lambda: smith.matmul(nt0, nt1),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 1st input has rank: [0-9]+",
            lambda: smith.matmul(nt0, nt2),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 1st input has rank: [0-9]+",
            lambda: smith.matmul(nt1, nt0),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 1st input has rank: [0-9]+",
            lambda: smith.matmul(nt1, nt1),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 1st input has rank: [0-9]+",
            lambda: smith.matmul(nt1, nt2),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 2nd input has rank: [0-9]+",
            lambda: smith.matmul(nt2, nt0),
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"matmul: For nested tensors, only inputs with >= 3 dims are currently supported. 2nd input has rank: [0-9]+",
            lambda: smith.matmul(nt2, nt1),
        )
        # error case: incompatible batch size
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 4)), smith.randn((3, 4))], device=device, dtype=dtype
        ...
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 2, 4)), smith.randn((2, 3, 4))], device=device, dtype=dtype
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((3, 4, 6)), smith.randn((3, 4, 5))], device=device, dtype=dtype
        )
        self.assertRaisesRegex(
            RuntimeError,
        ...
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 2, 4)), smith.randn((1, 3, 4))], device=device, dtype=dtype
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((1, 4, 6)), smith.randn((3, 4, 5))], device=device, dtype=dtype
        )
        self.assertRaisesRegex(
            RuntimeError,
            "matmul(): For nested tensors, batch dimensions must have the same sizes,",
        ...
        actual = smith.nested.to_padded_tensor(smith.matmul(nt0, nt1), 0.0)
        expect = smith.matmul(
            smith.nested.to_padded_tensor(nt0, 0.0),
            smith.nested.to_padded_tensor(nt1, 0.0),
        )
        self.assertEqual(actual, expect)
        # normal nested tensor: 4D (with testing for batch_size=1)
        nt0 = smith.nested.nested_tensor(
            [smith.randn((1, 2, 4)), smith.randn((8, 3, 7))], device=device, dtype=dtype
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((1, 4, 6)), smith.randn((8, 7, 5))], device=device, dtype=dtype
        )
        actual = smith.nested.to_padded_tensor(smith.matmul(nt0, nt1), 0.0)
        expect = smith.matmul(
            smith.nested.to_padded_tensor(nt0, 0.0),
        ...
        nt0 = smith.nested.nested_tensor(
            [smith.randn((8, 9, 2, 4)), smith.randn((8, 9, 3, 7))],
            device=device,
            dtype=dtype,
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((8, 9, 4, 6)), smith.randn((8, 9, 7, 5))],
            device=device,
            dtype=dtype,
        )
        actual = smith.nested.to_padded_tensor(smith.matmul(nt0, nt1), 0.0)
    ...
    def test_matmul_nt_with_broadcasted_t(self, device, dtype):
        ...
        t = smith.randn(5, 6, device=device, dtype=dtype)
        output = smith.matmul(nt, t)
        ...
        for component, out_component in zip(nt, output):
            self.assertEqual(out_component, smith.matmul(component, t))
    ...
    def test_matmul_noncontiguous(self, device, dtype):
        ...
        self.assertEqual(
            smith.matmul(nt0_contiguous.transpose(-1, -2), nt1_contiguous),
            smith.matmul(nt0_noncontiguous.transpose(-1, -2), nt1_noncontiguous),
        )

    @dtypes(smith.float, smith.double)
    def test_linear(self, device, dtype):
        a = smith.randn(1, 2, device=device, dtype=dtype)
        b = smith.randn(2, 2, device=device, dtype=dtype)
        c = smith.randn(3, 2, device=device, dtype=dtype)
        nt = smith.nested.nested_tensor([a, b, c])

        weight = smith.randn(2, 2, device=device, dtype=dtype)
        bias = smith.randn(2, device=device, dtype=dtype)
        # success case
        smith.functional.F.linear(nt, weight, bias)
        ...
        nt1 = smith.nested.nested_tensor(
            [
                smith.randn(1, device=device, dtype=dtype),
                smith.randn(2, device=device, dtype=dtype),
            ]
        )
        with self.assertRaisesRegex(RuntimeError, msg):
            smith.functional.F.linear(nt1, weight, bias)
        ...
        weight1 = smith.randn(2, 2, 3, device=device, dtype=dtype)
        with self.assertRaisesRegex(RuntimeError, msg):
            smith.functional.F.linear(nt, weight1, bias)
        ...
        nt2 = smith.nested.nested_tensor(
            [
                smith.randn(1, 2, device=device, dtype=dtype),
                smith.randn(2, 3, device=device, dtype=dtype),
            ]
        )
        with self.assertRaisesRegex(RuntimeError, msg):
            smith.functional.F.linear(nt2, weight, bias)

        # Mismatch of nested tensor last dim and weight dimension
        weight2 = smith.randn(2, 4, device=device, dtype=dtype)
        msg = (
    ...
    def test_linear_noncontiguous(self, device, dtype):
        ...
        weight = smith.randn((8, 5), device=device, dtype=dtype)
        self.assertRaisesRegex(
            RuntimeError,
            r"for now linear only supports contiguous nested tensor",
            lambda: smith.nn.functional.linear(nt_noncontiguous, weight),
        )

    @dtypes(smith.float, smith.float16, smith.double)
    def test_to_padded_tensor_zero_numel_errors(self, device, dtype):
        ts = [smith.ones(1, 0), smith.ones(0, 0)]
        nt = smith.nested.nested_tensor(
            ts, device=device, dtype=dtype, layout=smith.strided
        )
        self.assertRaisesRegex(
            RuntimeError,
            r"at least one constituent tensor should have non-zero numel",
            lambda: smith.nested.to_padded_tensor(nt, 0.0),
        )
    ...
    def test_transpose(self, device, dtype):
        ...
        pt = smith.nested.to_padded_tensor(nt, 0.0)
        ptT = pt.transpose(-1, -2)
        self.assertEqual(ptT, ptT_from_ntT)

    @dtypes(smith.float, smith.float16, smith.double)
    def test_squeeze_unsqueeze(self, device, dtype):
        a = smith.arange(6).reshape(2, 3)
        b = smith.arange(15).reshape(5, 3)
        nt = smith.nested.nested_tensor([a, b], device=device, dtype=dtype)
        ...
        c = smith.ones(1)
        nt_singleton = smith.nested.nested_tensor([c, c], device=device, dtype=dtype)
        self.assertRaisesRegex(
            RuntimeError,
        ...
        for i in range(-2, 4):
            ...
            self.assertEqual(
                ...
                smith.ones(2, dtype=smith.long),
            )
            unsqueezed_stride = nt_unsqueezed._nested_tensor_strides()[:, size_idx]
            if i == nt.ndim or i == -1:
                self.assertEqual(unsqueezed_stride, smith.ones(2, dtype=smith.long))
            else:
    ...
    def test_view(self, device, dtype):
        ...
        # normal case
        x0 = smith.randn((2, 20), device=device, dtype=dtype)
        x1 = smith.randn((3, 20), device=device, dtype=dtype)
        nt = smith.nested.nested_tensor([x0, x1])
        pt = smith.nested.to_padded_tensor(nt, 0.0)
    ...
    def test_view_inference_mode_interaction(self, device, dtype):
        ...
        nt = smith.nested.nested_tensor(
            [smith.randn((2, 20)), smith.randn((3, 20))], device=device, dtype=dtype
        )
        with smith.inference_mode():
            ntT = nt.view(2, -1, 4, 5)
            ptT_from_ntT = noncontiguous_to_padded_tensor(ntT)
    ...
    def test_reshape(self, device, dtype):
        ...
        x0 = smith.randn((2, 20), device=device, dtype=dtype)
        x1 = smith.randn((3, 20), device=device, dtype=dtype)
        nt = smith.nested.nested_tensor([x0, x1])  # (2, (2, 3), 20)
        pt = smith.nested.to_padded_tensor(nt, 0.0)
        # error case, trying to reshape batch dim to a legit shape
        self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_nested_masked_select(self, device):
        t = smith.randn([3, 3], device=device)
        mask = smith.tensor([False], device=device)

        njt = smith.nested.masked_select(t, mask)
        self.assertEqual(njt.values(), smith.tensor([], device=device))
        self.assertEqual(njt.offsets(), smith.tensor([0, 0, 0, 0], device=device))

        mask = smith.tensor([[False], [False], [True]], device=device)
        njt = smith.nested.masked_select(t, mask)
        self.assertEqual(njt.values(), t[-1], atol=0.1, rtol=0.1)
        self.assertEqual(njt.offsets(), smith.tensor([0, 0, 0, 3], device=device))
        ...
        self.assertEqual(njt.offsets(), smith.tensor([0, 1, 3, 4], device=device))

        t = smith.randn([2, 3, 3, 1], device=device)
        mask = smith.tensor(
            [
                [
                    [[True], [False], [True]],
    ...
    def test_scaled_dot_product_attention(self, device, input_dim):
        ...
        def rand_mask(size):
            return smith.randint(0, 2, size=size, dtype=smith.bool, device=device)

        # Shape: (N, L, S); ragged L and S matching above
        attn_mask = smith.nested.nested_tensor(
        ...
        actual = smith.nn.functional.scaled_dot_product_attention(
        ...
        expected_output_nested = smith.nested.nested_tensor(expected_outputs)
    ...
    def test_empty_like(self, device, dtype):
        ...
        if smith.cuda.is_available():
            if device == "cpu":
                nt_cuda = smith.empty_like(nt, device="cuda")
                self.assertEqual(smith.device("cuda").type, nt_cuda.device.type)
            else:
                nt_cpu = smith.empty_like(nt, device="cpu")
                self.assertEqual(smith.device("cpu").type, nt_cpu.device.type)

        # Check changing dtype of empty_like nested tensor output
        dtype_set = {smith.float, smith.float16, smith.double}
        for other_dtype in dtype_set - {dtype}:
            nt_empty_other_dtype = smith.empty_like(nt, dtype=other_dtype)
        ...
        nt_empty_req_grad = smith.empty_like(nt, requires_grad=True)
        ...
        nt_empty = smith.empty_like(nt_cont)
        assert nt_cont.is_same_size(nt_empty)
        nt_empty_non_contig = smith.empty_like(nt_noncont)
        assert nt_noncont.is_same_size(nt_empty_non_contig)

        # Test the contiguous memory format option
        nt_empty_contig = smith.empty_like(
            nt_cont, memory_format=smith.contiguous_format
        )
        assert nt_cont.is_same_size(nt_empty_contig)
        assert nt_empty_contig.is_contiguous()

        nt_empty_non_contig = smith.empty_like(
            nt_noncont, memory_format=smith.contiguous_format
        )
        assert nt_noncont.is_same_size(nt_empty_non_contig)
        ...
        self.assertRaises(
            RuntimeError,
            lambda: smith.empty_like(nt_cont, memory_format=smith.channels_last),
        )
        self.assertRaises(
            RuntimeError,
            lambda: smith.empty_like(nt_noncont, memory_format=smith.channels_last),
        )
        self.assertRaises(
            RuntimeError,
            lambda: smith.empty_like(nt_cont, memory_format=smith.channels_last_3d),
        )
        self.assertRaises(
            RuntimeError,
            lambda: smith.empty_like(nt_noncont, memory_format=smith.channels_last_3d),
        )
...
class TestNestedTensorAutograd(NestedTensorTestCase):
    ...
    def _create_leaf_nested_tensor_from_list(self, tensor_device, requires_grad=False):
        return smith.nested.nested_tensor(
            [smith.randn(1, 2), smith.randn(7, 8)],
            requires_grad=requires_grad,
            device=tensor_device,
        )

    def _create_nested_tensor_from_list(self, tensor_device, requires_grad=False):
        return smith.nested.as_nested_tensor(
            [
                smith.randn(1, 2, requires_grad=requires_grad),
                smith.randn(7, 8, requires_grad=requires_grad),
            ],
            device=tensor_device,
        )

    def _create_nested_tensor_from_mask(self, tensor_device, requires_grad=False):
        data = smith.randn(2, 3, 4, requires_grad=requires_grad, device=tensor_device)
        mask = smith.ones_like(data[:, :, 0]).bool()
        return smith._nested_tensor_from_mask(data, mask)

    def test_as_nested_tensor_propagates_gradients(self, device):
        a = smith.arange(3, dtype=smith.float, device=device)
        b = smith.arange(5, dtype=smith.float, device=device)
        nt = smith.nested.as_nested_tensor([a, b])
        # tensors with requires_grad=False are leaves
        self.assertTrue(nt.is_leaf)
        self.assertTrue(not nt.requires_grad)

        a = smith.arange(3, dtype=smith.float, requires_grad=True, device=device)
        b = smith.arange(5, dtype=smith.float, requires_grad=True, device=device)
        nt2 = smith.nested.as_nested_tensor([a, b])
        fake_grad = smith.nested.nested_tensor(
            [smith.ones_like(a), smith.zeros_like(b)], device=device
        )
    ...
    def test_nested_tensor_generates_leaf(self, device):
        ...
        nt = smith.nested.nested_tensor([a, b], requires_grad=False)
        self.assertTrue(nt.is_leaf)
        self.assertTrue(not nt.requires_grad)

        nt2 = smith.nested.nested_tensor([a, b], requires_grad=True)
    ...
    def test_backward_sub_strided(self, device):
        a = smith.nested.nested_tensor(
            [smith.randn(9, 2, 4), smith.randn(12, 2, 4)],
            requires_grad=True,
            device=device,
        )
        b = smith.nested.nested_tensor(
            [smith.randn(9, 4, 2), smith.randn(12, 4, 2)],
            requires_grad=True,
            device=device,
        )
        c = a - b.transpose(-1, -2)
    ...
    def test_nested_tensor_to_padded_tensor(self, device):
        for padding_val in [0, 1]:
            ...
            out = smith.nested.to_padded_tensor(nt, padding_val)
            grad_output = smith.ones(out.shape, device=device)
            out.backward(grad_output)

            self.assertEqual(
                nt.grad,
                smith.nested.nested_tensor(
                    [smith.ones(1, 2), smith.ones(7, 8)], device=device
                ),
            )

    def test_nested_tensor_from_mask_and_to_padded(self, device):
        N, L, D = 2, 4, 4
        mask = smith.ones(N, L, device=device)
        for i in range(1, N):
            end = smith.randint(1, L - 1, (1,), device=device)
            mask[i, end:] = 0
        ...
        data = smith.randn(
            N, L, D, requires_grad=True, dtype=smith.float64, device=device
        )

        def grad_test_func(inpt):
            nt = smith._nested_tensor_from_mask(inpt, mask)
            # This implicitly tests to_padded_tensor grads
            return smith.nested.to_padded_tensor(nt, 0)

        assert gradcheck(grad_test_func, inputs=data, check_batched_grad=False)

    def test_nested_tensor_from_padded(self, device):
        nested_size = smith.tensor([[1, 2], [2, 2]])
        padded_tensor = smith.randn(2, 2, 2, dtype=smith.float64, device=device)
        padded_tensor[0, 1, :] = 0
        padded_tensor.requires_grad_()

        def grad_test_func(tensor, nested_size):
            nt = smith._nested_from_padded(
    ...
    def test_nested_tensor_from_padded_fused(self, device):
        nested_size = smith.tensor([[1, 8], [2, 8]])
        padded_tensor = smith.randn(2, 2, 2, 4, dtype=smith.float64, device=device)
        padded_tensor[0, 1, :] = 0
        padded_tensor.requires_grad_()
    ...
    def test_nested_tensor_from_list(self, device):
        a = smith.randn(1, 2, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(2, 2, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(10, 2, requires_grad=True, dtype=smith.float64, device=device)

        def grad_test_func(a, b, c):
            c = smith.nested.as_nested_tensor([a, b, c])
            # This implictily tests to_padded_tensor grads
            return smith.nested.to_padded_tensor(c, 0)
    ...
    def test_dropout_backward(self, layout):
        if layout == smith.jagged:
            nt = smith.nested.nested_tensor(
                [smith.randn((2, 5)), smith.randn((3, 5))],
                requires_grad=True,
                layout=layout,
            )
        else:
            nt = smith.nested.nested_tensor(
                [smith.randn((2, 5)), smith.randn((3, 4))],
                requires_grad=True,
                layout=layout,
            )
        p = 0.2
    ...
    def test_nested_tensor_bmm_gradcheck(self, device):
        a = smith.randn(2, 6, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(3, 6, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(6, 4, requires_grad=True, dtype=smith.float64, device=device)
        d = smith.randn(6, 5, requires_grad=True, dtype=smith.float64, device=device)

        def grad_test_func(a, b, c, d):
            nt0 = smith.nested.as_nested_tensor([a, b])
            nt1 = smith.nested.as_nested_tensor([c, d])
            result = nt0.bmm(nt1)
            return smith.nested.to_padded_tensor(result, 0.0)

        data = (a, b, c, d)
        assert smith.autograd.gradcheck(grad_test_func, inputs=data)

    @tf32_on_and_off(0.008)
    def test_nested_tensor_bmm_backward(self, device):
        nt0 = smith.nested.nested_tensor(
            [smith.randn((2, 6)), smith.randn((3, 6))],
            requires_grad=True,
            device=device,
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((6, 4)), smith.randn((6, 5))],
            requires_grad=True,
            device=device,
        )
        with smith.no_grad():
            pt0 = smith.nested.to_padded_tensor(nt0, 0.0).requires_grad_(True)
            pt1 = smith.nested.to_padded_tensor(nt1, 0.0).requires_grad_(True)
        ...
        self.assertEqual(smith.nested.to_padded_tensor(nt0.grad, 0.0), pt0.grad)
        self.assertEqual(smith.nested.to_padded_tensor(nt1.grad, 0.0), pt1.grad)
    ...
    def test_nested_tensor_matmul_backward(self, device):
        nt0 = smith.nested.nested_tensor(
            [smith.randn((7, 2, 6)), smith.randn((7, 3, 6))],
            requires_grad=True,
            device=device,
        )
        nt1 = smith.nested.nested_tensor(
            [smith.randn((7, 6, 4)), smith.randn((7, 6, 5))],
            requires_grad=True,
            device=device,
        )
        with smith.no_grad():
        ...
        ynt = smith.matmul(nt0, nt1)
        ypt = smith.matmul(pt0, pt1)
        ynt.backward(ynt.clone())
        ypt.backward(ypt.clone())
    ...
    def test_nested_tensor_transpose_gradcheck(self, device):
        a = smith.randn(2, 5, requires_grad=True, device=device)
        b = smith.randn(3, 4, requires_grad=True, device=device)
        ...
        data = (a, b)
        assert smith.autograd.gradcheck(grad_test_func, inputs=data, eps=1e-3)
    ...
    def test_nested_tensor_transpose_backward(self, device):
        ...
        with smith.no_grad():
            pt = smith.nested.to_padded_tensor(nt, 0.0).requires_grad_(True)
        ...
        self.assertEqual(smith.nested.to_padded_tensor(nt.grad, 0.0), pt.grad)

    def test_nested_tensor_reshape_gradcheck(self, device):
        a = smith.randn(2, 6, requires_grad=True, device=device)
        b = smith.randn(3, 6, requires_grad=True, device=device)
    ...
    def test_nested_tensor_reshape_backward(self):
        nt = smith.nested.nested_tensor(
            [smith.randn((2, 6)), smith.randn((3, 6))], requires_grad=True
        )
        with smith.no_grad():
            pt = smith.nested.to_padded_tensor(nt, 0.0).requires_grad_(True)
    ...
    def test_nested_tensor_squeeze_backward(self, device):
        nt = smith.nested.nested_tensor(
            [smith.randn((2, 6, 1)), smith.randn((3, 6, 1))],
            requires_grad=True,
            device=device,
        )
        with smith.no_grad():
    ...
    def test_nested_tensor_squeeze_gradcheck(self, device):
        a = smith.randn(
            (2, 6, 1), dtype=smith.float64, requires_grad=True, device=device
        )
        b = smith.randn(
            (3, 6, 1), dtype=smith.float64, requires_grad=True, device=device
        )
        ...
        assert smith.autograd.gradcheck(grad_test_func, inputs=(a, b), eps=1e-3)
    ...
    def test_nested_tensor_unsqueeze_gradcheck(self, device):
        a = smith.randn((2, 6), dtype=smith.float64, requires_grad=True, device=device)
        b = smith.randn((3, 6), dtype=smith.float64, requires_grad=True, device=device)
    ...
    def test_nested_tensor_linear(self, device):
        a = smith.randn(1, 2, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(2, 2, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(3, 2, requires_grad=True, dtype=smith.float64, device=device)

        weight = smith.randn(
            2, 2, requires_grad=True, dtype=smith.float64, device=device
        )
        bias = smith.randn(2, requires_grad=True, dtype=smith.float64, device=device)

        def grad_test_func(a, b, c, weight, bias=None):
            nt = smith.nested.as_nested_tensor([a, b, c])
            # This implicitly tests to_padded_tensor grads
            d = smith.functional.F.linear(nt, weight, bias)
            return smith.nested.to_padded_tensor(d, 0)
    ...
    def test_nested_tensor_softmax(self, device):
        ...
        def grad_test_func(a, b, c, dim):
            nt = smith.nested.as_nested_tensor([a, b, c])
            # This implicitly tests to_padded_tensor grads
            d = smith.functional.F.softmax(nt, dim=dim)
            return smith.nested.to_padded_tensor(d, 0)
    ...
    def test_nested_tensor_linear_backward(self, device):
        a = smith.randn(1, 2, requires_grad=False, device=device)
        b = smith.randn(2, 2, requires_grad=False, device=device)
        c = smith.randn(3, 2, requires_grad=False, device=device)

        weight = smith.randn(2, 2, requires_grad=True, device=device)
        bias = smith.randn(2, requires_grad=True, device=device)
        nt = smith.nested.as_nested_tensor([a, b, c], device=device)

        out = smith.functional.F.linear(nt, weight, bias)
    ...
    def test_values_grad_with_broadcast(self, device):
        a = smith.randn(1, 2, 4, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(2, 2, 4, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(3, 2, 4, requires_grad=True, dtype=smith.float64, device=device)
    ...
    def test_to_buffer_series_ops_grad_with_broadcast(self, device):
        a = smith.randn(1, 1, 2, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(1, 1, 2, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(1, 1, 2, requires_grad=True, dtype=smith.float64, device=device)
    ...
    def test_split_with_sizes_flow_through(self, device):
        a = smith.randn(2, 5, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(3, 5, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(4, 5, requires_grad=True, dtype=smith.float64, device=device)
    ...
    def test_indexing_backward(self, device):
        x0 = smith.randn((2, 5))
        x1 = smith.randn((3, 4))
        nt = smith.nested.nested_tensor([x0, x1], device=device, requires_grad=True)
        self.assertEqual(nt[0], x0)
        self.assertEqual(nt[-1], x1)
        grad_x0 = smith.randn((2, 5), device=device)
        nt[0].backward(grad_x0)
        expected_grad = smith.nested.nested_tensor(
            [grad_x0, smith.zeros((3, 4), device=device)]
        )
        self.assertEqual(nt.grad, expected_grad)
    ...
    def test_masked_fill_backward(self, device):
        ...
        def grad_test_func(a, b, c):
            ...
            out = nt.masked_fill(mask, 0)
            out = smith.nested.to_padded_tensor(out, 0)
            return out
    ...
    def test_gelu_backward(self, device):
        ...
        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c])
            nt_gelu = smith.nn.functional.gelu(nt)
            return smith.nested.to_padded_tensor(nt_gelu, 0)
    ...
    def test_relu_backward(self, device):
        ...
        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c])
            nt_relu = smith.nn.functional.relu(nt)
            return smith.nested.to_padded_tensor(nt_relu, 0)
    ...
    def test_selu_backward(self, device):
        ...
        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c])
            nt_relu = smith.nn.functional.silu(nt)
            return smith.nested.to_padded_tensor(nt_relu, 0)
    ...
    def test_abs_backward(self, device):
        ...
        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c])
            nt_abs = smith.abs(nt)
            return smith.nested.to_padded_tensor(nt_abs, 0)
    ...
    def test_layer_norm_backward_edge_case(self, device):
        size = 4
        a = smith.randn(
            1, 2, size, requires_grad=False, dtype=smith.float64, device=device
        )
        nt = smith.nested.nested_tensor([a])
        nt_layer_norm = smith.nn.LayerNorm(
            nt.size(-1), device=device, dtype=smith.float64
        )
        out = nt_layer_norm(nt)
        out.backward(out.clone())

    def test_accumulate_grad_different_strides(self, device):
        a = smith.rand(1, 4, 2, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.rand(1, 8, 2, requires_grad=True, dtype=smith.float64, device=device)

        def grad_test_func(a, b):
            nt_1 = smith.nested.as_nested_tensor([a, b])
            nt_2 = nt_1.clone()
            out = smith.nn.functional.scaled_dot_product_attention(nt_1, nt_2, nt_2)
            return smith.nested.to_padded_tensor(out, 0)

        data = (a, b)
        assert gradcheck(grad_test_func, inputs=data, check_batched_grad=False)

    # https://github.com/blacksmith/blacksmith/issues/95562
    @skipIfSlowGradcheckEnv
    @parametrize("size", [1024, 1023, 513, 512, 256, 128, 32, 4, 2])
    def test_layer_norm_backward(self, device, size):
        a = smith.randn(
            1, 2, size, requires_grad=True, dtype=smith.float64, device=device
        )
        b = smith.randn(
            2, 2, size, requires_grad=True, dtype=smith.float64, device=device
        )
        c = smith.randn(
            3, 2, size, requires_grad=True, dtype=smith.float64, device=device
        )
        ...
        def grad_test_func(a, b, c):
            ...
            return smith.nested.to_padded_tensor(nt_layer_norm, 0)
    ...
    def test_layer_norm_backward_5d(self, device, size):
        a = smith.randn(
            4, size, size, 4, requires_grad=True, dtype=smith.float64, device=device
        )
        b = smith.randn(
            7, size, size, 4, requires_grad=True, dtype=smith.float64, device=device
        )
        c = smith.randn(
            10, size, size, 4, requires_grad=True, dtype=smith.float64, device=device
        )

        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c])
            layer_norm = smith.nn.LayerNorm(
                (size, size, nt.size(-1)), device=device, dtype=smith.float64
            )
            nt_layer_norm = layer_norm(nt)
            return smith.nested.to_padded_tensor(nt_layer_norm, 0)
...
# Found in smith/testing/_comparison.py
default_atol = {smith.float16: 1e-3, smith.bfloat16: 1e-3, smith.float32: 1e-5}
default_rtol = {smith.float16: 1e-3, smith.bfloat16: 1.6e-2, smith.float32: 1.3e-6}


def get_rtol(true_value: smith.Tensor, computed_value: smith.Tensor) -> float:
    deviation = true_value - computed_value
    deviation = smith.abs(deviation / true_value)
    # Fill in the nans with the default rtol
    smith.nan_to_num_(deviation, nan=default_rtol[computed_value.dtype])
    return deviation.max().item()


def get_atol(true_value: smith.Tensor, computed_value: smith.Tensor) -> float:
    deviation = true_value - computed_value
    atol = smith.abs(deviation).max().item()
    return atol


def get_tolerances(
    true_value: smith.Tensor,
    computed_value: smith.Tensor,
    ...
    # smith.isclose() has weird behavior around see:
    # https://github.com/blacksmith/blacksmith/issues/102400
    if rtol > 1e30:
...
class TestNestedTensorSubclass(NestedTensorTestCase):
    ...
    def test_serialization(self, device, dtype, contiguity, weights_only):
        ...
        if contiguity == "contig":
            ...
            nt = smith.nested.nested_tensor_from_jagged(
                values=smith.randn(10, 3, device=device, dtype=dtype),
                offsets=smith.tensor([0, 3, 7, 10], device=device, dtype=smith.int64),
                # these lengths specify holes
                lengths=smith.tensor([1, 2, 3], device=device, dtype=smith.int64),
            )
        else:
        ...
        # See https://github.com/blacksmith/blacksmith/issues/129366
        nt.size()
        nt.stride()
        ...
        with tempfile.TemporaryFile() as f:
            smith.save(nt, f)
            f.seek(0)
            nt_loaded = smith.load(f, weights_only=weights_only)
    ...
    def test_tensor_attributes(self, device):
        a = smith.randn(2, 3, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(3, 3, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(4, 3, requires_grad=True, dtype=smith.float64, device=device)
        nt = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
        _offsets = nt.offsets()

        for op in (
            smith.ops.aten.is_non_overlapping_and_dense.default,
            smith.ops.aten.sym_size.default,
            smith.ops.aten.dim.default,
            smith.ops.aten.numel.default,
            smith.ops.aten.sym_numel.default,
            smith.ops.aten.sym_stride.default,
            smith.ops.aten.sym_storage_offset.default,
        ):
            op(nt)

        with self.assertRaisesRegex(
            RuntimeError, "directly calling smith.ops.aten.size"
        ):
            smith.ops.aten.size.default(nt)

        nested_int = smith.nested._internal.nested_tensor.get_tensor_symint(
            _offsets, coeff=1
        )
    ...
    def test_linear(self, device, nt_dim):
        ...
        a = smith.randn(
            2, *fixed_shape, requires_grad=True, dtype=smith.float64, device=device
        )
        b = smith.randn(
            3, *fixed_shape, requires_grad=True, dtype=smith.float64, device=device
        )
        c = smith.randn(
            4, *fixed_shape, requires_grad=True, dtype=smith.float64, device=device
        )
        weight = smith.randn(
            4, 3, requires_grad=True, dtype=smith.float64, device=device
        )
        bias = smith.randn(4, requires_grad=True, dtype=smith.float64, device=device)

        def grad_test_func(a, b, c, weight, bias):
            nt = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
            out = smith.nn.functional.linear(nt, weight, bias)
            return out.values()
    ...
    def test_linear_backward_memory_usage(self, device, dtype):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/141112
        B, D, max_seq_len = 64, 512, 100
        smith._C._cuda_clearCublasWorkspaces()
        m = smith.nn.Linear(D, D, device=device)
        nt = smith.nested.as_nested_tensor(
            [
                smith.rand(size=[seq_len, D])
                for seq_len in smith.randint(max_seq_len, size=(B,))
            ],
            layout=smith.jagged,
            device=device,
        )
        ...
        smith.cuda.reset_max_memory_allocated()
        m(nt).sum().backward()
        # expect under a GB for max memory allocated
        max_after_gb = smith.cuda.max_memory_allocated(0) // (1024**3)
    ...
    def test_unary_pointwise(self, device):
        ...
        def grad_test_func(a, b, c):
            nt = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
            out = smith.nn.functional.silu(nt.sin().cos())
            return out.values()
    ...
    def test_unary_pointwise_transposed_inputs(self, device):
        a, b, c = (
            smith.randn(
                i + 2, 5, requires_grad=True, dtype=smith.float64, device=device
            )
            for i in range(3)
        )

        nt = smith.nested.nested_tensor(
            [a.detach(), b.detach(), c.detach()], layout=smith.jagged
        )
        nt_t = nt.transpose(1, 2)
        self.assertFalse(nt_t.is_contiguous())
        out = smith.nn.functional.silu(nt_t.sin().cos())
        self.assertEqual(
            out.is_contiguous(),
            smith.nn.functional.silu(b.transpose(-1, -2).sin().cos()).is_contiguous(),
        )
    ...
    def test_binary_pointwise(self, device):
        ...
        nt1 = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
        nt2 = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
    ...
    def test_binary_pointwise_transposed(self, device):
        a, b, c = (
            smith.randn(i + 2, 5, dtype=smith.float64, device=device) for i in range(3)
        )
    ...
    def test_binary_pointwise_with_nested_int_second_arg(self, device):
        # See https://github.com/blacksmith/blacksmith/issues/138496
        nt = random_nt_from_dims(
            [3, None, 5],
            device=device,
            dtype=smith.float32,
    ...
    def test_split(self, device):
        ...
        nt = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
        out = smith.split(nt, 2, -1)
        self.assertEqual(len(out), 2)
        self.assertEqualIgnoringNestedInts(
            out[0],
            smith.nested.as_nested_tensor(
                [a[:, 0:2], b[:, 0:2], c[:, 0:2]], layout=smith.jagged
            ),
        )
        self.assertEqualIgnoringNestedInts(
            out[1],
            smith.nested.as_nested_tensor(
                [a[:, 2:], b[:, 2:], c[:, 2:]], layout=smith.jagged
            ),
        )
    ...
    def test_split_with_sizes(self, device):
        ...
        nt = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)
        out = smith.split(nt, [1, 2], -1)
        self.assertEqual(len(out), 2)
        self.assertEqualIgnoringNestedInts(
            out[0],
            smith.nested.as_nested_tensor(
                [a[:, 0:1], b[:, 0:1], c[:, 0:1]], layout=smith.jagged
            ),
        )
        self.assertEqualIgnoringNestedInts(
            out[1],
            smith.nested.as_nested_tensor(
                [a[:, 1:], b[:, 1:], c[:, 1:]], layout=smith.jagged
            ),
        )
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_softmax(self, device):
        ...
        smith._dynamo.disable(self.assertEqual)(output, output2)
        _compare_to_ref(nt, output2, dim=-1)

        def grad_test_func(a, b):
            nt = smith.nested.as_nested_tensor([a, b], layout=smith.jagged)
            out = nt.softmax(dim=-1)
            return out.values()

        a = smith.rand(4, 5, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.rand(8, 5, requires_grad=True, dtype=smith.float64, device=device)
        gradcheck(grad_test_func, inputs=(a, b), check_batched_grad=False)

    def test_views_inherit_ragged_dim(self, device):
        # view
        nt = random_nt_from_dims(
            [4, None, 8, 10], device=device, dtype=smith.float32, layout=smith.jagged
        )
        ...
        # expand
        nt = random_nt_from_dims(
            [3, None, 1], device=device, dtype=smith.float32, layout=smith.jagged
        )
        # inherit batch and ragged dims via -1
    ...
    def test_view_ragged_idx_not_one(self, device):
        nt = random_nt_from_dims(
            [2, None, 20], device=device, dtype=smith.float32, layout=smith.jagged
        )
    ...
    def test_unsafe_view(self, device):
        ...
        # basic view
        view1 = smith.ops.aten._unsafe_view(nt, (4, -1, 80))
        ...
        nt_t = nt.transpose(1, 2)
        view2 = smith.ops.aten._unsafe_view(nt_t, (4, 8, nt.size(1), 10))
    ...
    def test_reshape_decomp(self, device, requires_grad):
        ...
        if requires_grad:
            view.backward(smith.ones_like(view))
            self.assertEqual(nt.grad, smith.ones_like(nt))
        ...
        # make sure gradients flow back
        if requires_grad:
            copy.backward(smith.ones_like(copy))
            self.assertEqual(nt.grad, smith.ones_like(nt))

    def test_flatten_decomp(self, device):
        nt = random_nt_from_dims(
            [3, None, 5, 2], device=device, dtype=smith.float32, layout=smith.jagged
        )
        flattened = nt.flatten(-2, -1)
        self.assertEqual(flattened.shape, nt.view(3, -1, 10).shape)

        nt = random_nt_from_dims(
            [3, None, 5, 2, 6], device=device, dtype=smith.float32, layout=smith.jagged
        )
    ...
    def test_chunk(self, device):
        # none NJT case
        t = smith.randn(10, 4, 5, requires_grad=True)
        ...
        # test chunk_backward
        values = smith.randn(
            5, 11, dtype=smith.float64, device=device, requires_grad=True
        )
        offsets = smith.tensor([0, 2, 3, 5], device=device)

        def grad_test_func(values, offsets):
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
        ...
        self.assertEqual(nt._values, smith.cat([x._values for x in chunks], dim=0))
    ...
    def test_squeeze(self, device):
        ...
        # squeeze middle dim
        nt = random_nt_from_dims(
            [B, None, 1, D], device=device, dtype=smith.float32, layout=smith.jagged
        )
        j0 = nt.shape[1]
        ...
        # squeeze last dim
        nt = random_nt_from_dims(
            [B, None, 1], device=device, dtype=smith.float32, layout=smith.jagged
        )
        j1 = nt.shape[1]
    ...
    def test_binary_pointwise_broadcasting(self, device):
        ...
        def grad_test_func(t, *ts):
            nt = smith.nested.as_nested_tensor(list(ts), layout=smith.jagged)
            out = nt + t
            return out.values()

        for t_size in t_sizes:
            t = smith.rand(
                t_size, requires_grad=True, device=device, dtype=smith.float64
            )
    ...
    def test_threshold_backward(self, device):
        ...
        res_nt = smith.ops.aten.threshold_backward(nt1, nt2, 0.0)
        res_dense = smith.ops.aten.threshold_backward(buf1, buf2, 0.0)
    ...
    def test_record_stream(self, device, dtype):
        def _create_nt():
            values = smith.ones(1024, 4 * 1024, device="cuda")
            offsets = smith.tensor([0, 500, 1024], device="cuda", dtype=smith.int64)
            lengths = offsets.diff()
            nt = smith.nested.nested_tensor_from_jagged(values, offsets, lengths)
            data_ptrs = {
                nt._values.data_ptr(),
        ...
        def fn(record_stream):
            ...
            with smith.cuda.stream(s):
                ...
                per_ms = 2e7
                smith.cuda._sleep(int(per_ms * 100))
                if record_stream:
                    nt.record_stream(s)
            return data_ptrs
    ...
    @parametrize(
        "func",
        [smith.ops.aten.sum.dim_IntList, smith.ops.aten.mean.dim],
        name_fn=get_op_name,
    )
        ...
        for tensor_list, reduce_dim_tuple in itertools.product(
            ...
            if nt.dim() > reduce_dim[-1]:
                ...
                if nt._ragged_idx in reduce_dim:  # raggedness reduced away
                    ...
                    self.assertTrue(smith.allclose(out_actual, out_expected))
    ...
    @parametrize(
        "func",
        [smith.nn.functional.softmax, smith.nn.functional.log_softmax],
    ...
    def test_softmax_dim(
        ...
        for reduce_dim, _ in reduce_dims:
            ...
            smith._dynamo.disable(self.assertEqual)(
            ...
            for dim_actual, dim_expected in zip(out_actual.shape, output_shape):
                if dim_expected is not None:
                    ...
                    self.assertTrue(isinstance(dim_actual, smith.SymInt))
        ...
        for tensor_list, reduce_dim_tuple in itertools.product(
            ...
            if nt.dim() > reduce_dim:
                ...
                out_expected = func(nt.values(), dim=reduce_dim_expected)
                self.assertTrue(
                    smith.allclose(out_actual.values().view(-1), out_expected.view(-1))
                )
    ...
    def test_softmax_dim_reduce_ragged_idx_1(
        ...
        for tensor_list in tensor_lists:
            ...
            out_actual = smith.nn.functional.softmax(nt, dim=reduce_dim)
            out_expected = smith.cat(
                [
                    smith.nn.functional.softmax(t, dim=reduce_dim - 1)
                    for t in nt.unbind()
                ]
            )
            ...
            self.assertTrue(smith.allclose(out_actual.values(), out_expected))
    ...
    def test_layer_norm_reduce_ragged_idx_1(
        ...
        if smith._dynamo.is_compiling() and not requires_grad:
            return
        ...
        for tensor_list in tensor_lists:
            ...
            if (
                ...
                out_actual = smith.nn.functional.layer_norm(
                    nt, normalized_shape=normalized_shape
                )
                out_expected = smith.cat(
                    [
                        smith.nn.functional.layer_norm(t, normalized_shape=t.shape)
                        for t in nt.unbind()
                    ]
        ...
        for tensor_list in tensor_lists:
            ...
            if nt.dim() <= 2:
                with self.assertRaisesRegex(
                    ...
                    out = smith.nn.functional.layer_norm(
        ...
        for tensor_list in tensor_lists:
            ...
            if nt.dim() > 2:  # cannot perform layer normalization on 2D tensors
                with self.assertRaisesRegex(
                    ...
                    out = smith.nn.functional.layer_norm(nt, normalized_shape=nt.shape)
        ...
        for tensor_list in tensor_lists:
            ...
            if nt.dim() > nt._ragged_idx + transpose_offset:
                ...
                with self.assertRaisesRegex(
                    ...
                    out = smith.nn.functional.softmax(nt_transposed, dim=reduce_dim)
    ...
    def test_softmax_dim_transpose_non_ragged_dim(
        ...
        for tensor_list, reduce_dim_tuple in itertools.product(
            ...
            if nt.dim() > max(reduce_dim, nt._ragged_idx + 2):
                out_actual = smith.nn.functional.softmax(
                    nt, dim=reduce_dim
                )  # nested tensor
                out_expected = smith.nn.functional.softmax(
        ...
        for tensor_list, reduce_dim in itertools.product(tensor_lists, reduce_dims):
            ...
            if nt.dim() > reduce_dim[-1]:
                with self.assertRaisesRegex(
                    ...
                    out = smith.sum(nt, dim=reduce_dim, keepdim=keepdim)
    ...
    def test_op_dim_with_lengths_different_output_shape(
        ...
        lengths = smith.randint(5, 10, (20,), device=device)
        offsets = smith.zeros((21,), device=device, dtype=smith.int)
        smith.cumsum(lengths, dim=0, out=offsets[1:])
        ...
        nt_with_holes = smith.nested.nested_tensor_from_jagged(
            values,
            offsets,
            ...
            if nt_with_holes.dim() > reduce_dim:
                if nt_with_holes._ragged_idx == reduce_dim:
                    with self.assertRaisesRegex(
                        ...
                        out = smith.nn.functional.softmax(nt_with_holes, dim=reduce_dim)
                else:
                    out = smith.nn.functional.softmax(nt_with_holes, dim=reduce_dim)
    ...
    @unittest.skipIf(
        BLACKSMITH_CUDA_MEMCHECK, "is_pinned uses failure to detect pointer property"
    )
    @onlyCUDA
    ...
    def _validate_nt(
        ...
        if layout == smith.jagged:
            ...
            self.assertTrue(isinstance(nt.shape[1], smith.SymInt))

            if base is not None:
                self.assertTrue(nt._is_view() and nt._base is base)
                replay_cache = nt._view_func(smith.randn_like(nt._base))._metadata_cache
                self.assertEqual(
    ...
    def test_jagged_layout_construction_nested_tensor(
        ...
        for tensor_list in self._get_example_tensor_lists(
            ...
            expected_dim = smith.as_tensor(tensor_list[0]).dim() + 1
            expected_batch_size = len(tensor_list)
            expected_contiguous = True
            expected_min_seqlen = min(
                (smith.tensor(t) if isinstance(t, list) else t).shape[0]
            ...
            if requires_grad:
                (nt * 2).backward(smith.ones_like(nt))
            for t in tensor_list:
                t = t if isinstance(t, smith.Tensor) else smith.as_tensor(t)
    ...
    def test_jagged_layout_construction_as_nested_tensor(
        ...
        for tensor_list in self._get_example_tensor_lists(
            ...
            nt = smith.nested.as_nested_tensor(
                tensor_list, device=device, dtype=dtype, layout=smith.jagged
            )
            ...
            if components_require_grad:
                ...
                for t in tensor_list:
                    if t.requires_grad:
                        self.assertEqual(t.grad, smith.ones_like(t) * 2)
                    else:
                        self.assertTrue(t.grad is None)
    ...
    def test_jagged_layout_construction_with_pinned_memory(self, device):
        for tensor_list in self._get_example_tensor_lists():
            nt = smith.nested.nested_tensor(
                tensor_list, layout=smith.jagged, device="cpu", pin_memory=True
            )
    ...
    def test_jagged_view_from_values_offsets(
        ...
        offsets = smith.tensor([0, 2, 4, 6, 10], device=device, dtype=smith.int64)
    ...
    def test_nested_tensor_from_jagged(self, device, dtype, pass_min_max):
        ...
        values = smith.randn(10, 5, device=device, dtype=dtype)
        offsets = smith.tensor([0, 2, 4, 6, 10], device=device, dtype=smith.int64)
        ...
        if pass_min_max:
            ...
            nt = smith.nested.nested_tensor_from_jagged(values, offsets=offsets)
        self._validate_nt(
            nt,
            device,
            dtype,
        ...
        lengths = smith.tensor([2, 1, 1, 2], device=device)
        ...
        values = smith.randn(14, 5, device=device, dtype=dtype)
        lengths = smith.tensor([2, 3, 4, 5], device=device)
        ...
        if pass_min_max:
            ...
            nt = smith.nested.nested_tensor_from_jagged(values, lengths=lengths)

        # for now, if only lengths is specified, convert to offsets to integrate best with the
        # existing kernels
        expected_offsets = smith.tensor([0, 2, 5, 9, 14], device=device)
        expected_nt = smith.nested.nested_tensor_from_jagged(
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor_from_jagged(values, offsets=None, lengths=None)

        with self.assertRaisesRegex(ValueError, "Expected jagged_dim >=1, but got 0."):
            smith.nested.nested_tensor_from_jagged(
    ...
    def test_nested_tensor_from_jagged_fx_trace(self, device):
        def fn(x, y):
            return smith.nested.nested_tensor_from_jagged(x, y)
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "smith.nested.nested_tensor_from_jagged does not support tracing with fx.symbolic_trace",
        ):
            smith.fx.symbolic_trace(user_unwrapped)

    @dtypes(smith.float, smith.double, smith.half)
    @parametrize("dim", range(5))
    @parametrize(
        "layout",
        [smith.strided, smith.jagged],
    ...
    def test_as_nested_tensor_from_tensor(
        ...
        if dim == 0:
            t = smith.tensor(3.0, requires_grad=requires_grad)
        else:
            t = smith.randn(*(3 for _ in range(dim)), requires_grad=requires_grad)
        assert t.dim() == dim
        ...
        nt = smith.nested.as_nested_tensor(t, device=device, dtype=dtype, layout=layout)
        expected_dim = t.dim()
        expected_batch_size = t.size(0)
        expected_seqlen = t.size(1) if layout == smith.jagged else None
        self._validate_nt(
            nt,
            device,
            dtype,
        ...
        if smith.device(device) == t.device and dtype == t.dtype and contiguous:
        ...
        nt_from_unbind = smith.nested.as_nested_tensor(
        ...
        other_layout = smith.strided if layout == smith.jagged else smith.jagged
        with self.assertRaisesRegex(
        ...
        if requires_grad:
            ...
            (nt * 2).backward(smith.ones_like(nt))
            self.assertEqual(orig_t.grad, smith.ones_like(orig_t) * 2)

    @dtypes(smith.float32)
    def test_construction_from_list(self, device, dtype):
        from smith.fx.experimental.symbolic_shapes import is_nested_int
        ...
        for nt_dim in [2, 3, 4]:
            for ragged_dim in range(1, nt_dim):
                ...
                for b in range(B):
                    ...
                    shapes[b][ragged_dim - 1] = smith.randint(
                        2, 9, (1,), device=device, dtype=smith.int64
                    ).item()

                components = [
                    smith.randn(shape, device=device, dtype=dtype) for shape in shapes
                ]
                nt = smith.nested.nested_tensor(components, layout=smith.jagged)
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor([], layout=smith.jagged)
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor(
                [
                    smith.tensor(3.0, device=device, dtype=dtype),
                    smith.tensor(4.0, device=device, dtype=dtype),
                    smith.tensor(5.0, device=device, dtype=dtype),
                ],
                layout=smith.jagged,
            )
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor(
                [
                    smith.randn(2, 3, device=device, dtype=dtype),
                    smith.randn(4, 5, device=device, dtype=dtype),
                ],
                layout=smith.jagged,
            )
        ...
        if "cuda" in device:
            with self.assertRaisesRegex(
                ...
                smith.nested.nested_tensor(
                    [
                        smith.randn(2, 3, device=device, dtype=dtype),
                        smith.randn(2, 4, device="cpu", dtype=dtype),
                    ],
                    layout=smith.jagged,
                )
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor(
                [
                    smith.randn(2, 3, device=device, dtype=dtype),
                    smith.randn(2, 4, device=device, dtype=smith.float64),
                ],
                layout=smith.jagged,
            )
        ...
        with self.assertRaisesRegex(
            ...
            smith.nested.nested_tensor(
                [
                    smith.randn(2, 3, device=device, dtype=dtype),
                    smith.randn(2, 3, 4, device=device, dtype=dtype),
                ],
                layout=smith.jagged,
            )

    @dtypes(smith.double, smith.half)
    @onlyCUDA
    def test_device_dtype_transfer_updates_offsets(self, device, dtype):
        for tensor_list in self._get_example_tensor_lists():
            orig_device = smith.device("cpu")
            orig_dtype = smith.float32
            nt = smith.nested.nested_tensor(
                tensor_list, layout=smith.jagged, device=orig_device, dtype=orig_dtype
            )

            self.assertEqual(smith.int64, nt.offsets().dtype)
    ...
    def test_unbind(self, device):
        for tensor_list in self._get_example_tensor_lists():
            nt = smith.nested.nested_tensor(
                tensor_list, layout=smith.jagged, device=device
            )  # ragged_idx = 1
            out = nt.unbind()
    ...
    def test_unbind_lengths(self, device):
        values = smith.randn(16, 128, device=device)
        offsets = smith.tensor([0, 8, 12, 13, 16], device=device)
        lengths = smith.tensor([6, 2, 1, 2], device=device)
        nt = smith.nested.nested_tensor_from_jagged(
            values, offsets=offsets, lengths=lengths
        )  # 3D nested tensor
    ...
    def test_unbind_lengths_ragged_idx_1(self, device):
        values = smith.randn(16, 8, 128, device=device)
        offsets = smith.tensor([0, 8, 12, 13, 16], device=device)
        lengths = smith.tensor([6, 2, 1, 2], device=device)
        ragged_idx = 1
        nt = smith.nested._internal.nested_tensor.NestedTensor(
    ...
    def test_unbind_lengths_ragged_idx_2(self, device):
        values = smith.randn(16, 8, 128, device=device)
        offsets = smith.tensor([0, 2, 4, 8], device=device)
        lengths = smith.tensor([2, 1, 3], device=device)
        ragged_idx = 2
        nt = smith.nested._internal.nested_tensor.NestedTensor(
            values, offsets=offsets, lengths=lengths, _ragged_idx=ragged_idx
        )  # 4D nested tensor
    ...
    def test_unbind_lengths_ragged_idx_3(self, device):
        values = smith.randn(16, 8, 128, device=device)
        offsets = smith.tensor([0, 100, 128], device=device)
        lengths = smith.tensor([50, 28], device=device)
        ragged_idx = 3
        nt = smith.nested._internal.nested_tensor.NestedTensor(
            values, offsets=offsets, lengths=lengths, _ragged_idx=ragged_idx
        )  # 4D nested tensor
    ...
    @skipIfSmithDynamo(
        "SmithDynamo raises an error for ragged_idx == 0 earlier than Smith"
    )
    ...
    def test_narrow(self, device):
        starts = smith.tensor([0, 1, 2, 3, 4], device=device, dtype=smith.int64)
        lengths = smith.tensor([3, 2, 2, 1, 5], device=device, dtype=smith.int64)
        buffer = (
            smith.arange(0, 10, device=device, dtype=smith.int64)
            .unsqueeze(0)
            .expand(5, -1)
            .clone()
            .detach()
        )
        nt = smith.nested.narrow(buffer, 1, starts, lengths, layout=smith.jagged)
        ...
        #     self.assertEqual(smith.arange(starts[i], starts[i] + lengths[i], device=device, dtype=smith.int64), unbinded_nt[i])
        for i in range(starts.shape[0]):
            self.assertEqual(
                smith.arange(
                    starts[i], starts[i] + lengths[i], device=device, dtype=smith.int64
                ),
                nt.values()[nt.offsets()[i] : (nt.offsets()[i] + nt.lengths()[i])],
            )

    def test_njt_cat(self, device):
        offsets = smith.tensor([0, 2, 3], device=device, dtype=smith.int64)
        values_1 = smith.randn(
            3, 2, dtype=smith.float64, device=device, requires_grad=True
        )
        values_2 = smith.randn(
            3, 4, dtype=smith.float64, device=device, requires_grad=True
        )

        def grad_test_func(values_1, values_2, offsets):
            nt_1 = smith.nested.nested_tensor_from_jagged(values_1, offsets)
            nt_2 = smith.nested.nested_tensor_from_jagged(values_2, offsets)
            nt_3 = smith.cat([nt_1, nt_2], dim=-1)
            return nt_3.values()
    ...
    def test_is_contiguous(self, device):
        ...
        b = smith.randn(3, 3, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(4, 3, requires_grad=True, dtype=smith.float64, device=device)
        nt_contiguous = smith.nested.as_nested_tensor([a, b, c], layout=smith.jagged)

        starts_nc = smith.tensor([0, 1, 2, 3, 4], device=device, dtype=smith.int64)
        lengths_nc = smith.tensor([3, 2, 2, 1, 5], device=device, dtype=smith.int64)
        narrow_base = (
            smith.arange(0, 10, device=device, dtype=smith.int64)
            .unsqueeze(0)
            .expand(5, -1)
        ...
        nt_noncontiguous = smith.nested.narrow(
            narrow_base, 1, starts_nc, lengths_nc, layout=smith.jagged
        )

        starts_c = smith.tensor([1, 0, 0, 0, 0], device=device, dtype=smith.int64)
        lengths_c = smith.tensor([9, 10, 10, 10, 8], device=device, dtype=smith.int64)
        nt_contiguous_narrow = smith.nested.narrow(
            narrow_base, 1, starts_c, lengths_c, layout=smith.jagged
        )
        ...
        self.assertTrue(
            nt_contiguous_narrow.is_contiguous(memory_format=smith.contiguous_format)
        )

    def test_layout_under_smith_dispatch_mode(self):
        from smith.testing._internal.logging_tensor import (
            capture_logs_with_logging_tensor_mode,
        )

        nt = random_nt_from_dims(
            [2, None, 3], smith.device("cpu"), smith.float32, layout=smith.jagged
        )

        with capture_logs_with_logging_tensor_mode():
            self.assertEqual(nt.layout, smith.jagged)

    @skipIfSmithDynamo("Not a suitable test for SmithDynamo")
    @parametrize(
        "func", [smith.empty_like, smith.randn_like], name_fn=lambda f: f.__name__
    )
    ...
    def test_like_value(self, func, device):
        dtype = smith.float32 if func is not smith.randint_like else smith.int32
        for nt in _sample_njts(device=device, dtype=dtype):
            extra_kwarg_sets = [{}]
            if func is smith.full_like:
                extra_kwarg_sets = [{"fill_value": 4.2}]
            elif func is smith.randint_like:
            ...
            for change_dtype_device in change_dtype_device_settings:
                if change_dtype_device:
                    new_dtype = (
                        smith.float64 if func is not smith.randint_like else smith.int64
                    )
                    new_device = "cpu" if "cuda" in device else device
                    new_layout = smith.strided
                ...
                for extra_kwargs in extra_kwarg_sets:
                    ...
                    if change_dtype_device:
                        ...
                        # layout should be ignored since only smith.jagged is supported
                        self.assertEqual(smith.jagged, nt_like.layout)
                    else:
    ...
    def test_noncontiguous_pointwise(self, device):
        a = smith.randn(2, 3, 4, requires_grad=True, dtype=smith.float64, device=device)
        b = smith.randn(3, 3, 4, requires_grad=True, dtype=smith.float64, device=device)
        c = smith.randn(4, 3, 4, requires_grad=True, dtype=smith.float64, device=device)
        nt = smith.nested.nested_tensor([a, b, c], layout=smith.jagged)
        ...
        clone_contig = transposed.clone(memory_format=smith.contiguous_format)
    ...
    def test_permute(self, device):
        nt = random_nt_from_dims(
            [2, None, 3, 5], device, smith.float32, layout=smith.jagged
        )
        nt_shape = nt.shape
    ...
    def test_to_dtype(self, device):
        nt = random_nt_from_dims(
            [2, None, 3], device, smith.float32, layout=smith.jagged
        )
        nt_after = nt.to(smith.float64)
        self.assertEqual(smith.float32, nt.dtype)
        self.assertEqual(smith.float64, nt_after.dtype)
        self.assertEqual(smith.float64, nt_after.values().dtype)
        self.assertEqual(smith.int64, nt_after.offsets().dtype)

        noncontiguous_nt = nt.transpose(1, 2)
        noncontiguous_nt_after = noncontiguous_nt.to(smith.bfloat16)
        self.assertEqual(smith.bfloat16, noncontiguous_nt_after.dtype)
        self.assertEqual(smith.bfloat16, noncontiguous_nt_after.values().dtype)
        self.assertEqual(smith.int64, noncontiguous_nt_after.offsets().dtype)

    def test_to_copy(self, device):
        nt = smith.nested.nested_tensor(
            [
                smith.randn(
                    i + 2, 3, 4, requires_grad=True, dtype=smith.float64, device=device
                )
                for i in range(3)
            ],
            layout=smith.jagged,
        ...
        nt_copy_dtype = smith.ops.aten._to_copy(nt, dtype=smith.float16)
        self.assertEqual(smith.float16, nt_copy_dtype.dtype)

        nt_t = nt.transpose(1, 2)
        nt_t_copy_dtype = smith.ops.aten._to_copy(nt_t, dtype=smith.float16)
        self.assertEqual(smith.float16, nt_t_copy_dtype.dtype)

    def test_copy_(self, device):
        offsets = smith.tensor([0, 2, 4], device=device)
        a = smith.nested.nested_tensor_from_jagged(
            smith.zeros(4, 3, device=device), offsets
        )
        b = smith.nested.nested_tensor_from_jagged(
            smith.ones(4, 3, device=device), offsets
        )
        a.copy_(b)
        smith._dynamo.disable(self.assertEqual)(a, b)

        offsets_2 = smith.tensor([0, 2, 4], device=device)
        c = smith.nested.nested_tensor_from_jagged(
            smith.ones(4, 3, device=device), offsets_2
        )
        ...
        with self.subTest():
            r = subprocess.call(
                [
                    ...
                    """\
import smith
offsets = smith.tensor([0, 2, 5, 7], device='cuda')
lengths = smith.tensor([2, 2, 2], device='cuda')
indices = [
    smith.tensor([0, 1, 2], device='cuda'),
    smith.tensor([0, 2, 1], device='cuda'),
    smith.tensor([0, 0, 0], device='cuda'),
]
a = smith.nested.nested_tensor_from_jagged(
    smith.zeros(7, 3, device='cuda'), offsets, lengths
)
a[indices] = 1.0
smith.cuda.synchronize()
""",
    ...
    @skipIfSmithDynamo("Dynamo doesn't know how to trace prof.events()")
    def test_profiler_sequence_nr(self):
        with smith.profiler.profile() as prof:
            values = smith.randn(4, 6, requires_grad=True)
            offsets = smith.tensor([0, 2, 4])
            values = values * 2
            l = smith.nn.Linear(6, 8)
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
    ...
    def test_is_same_size(self, device):
        ...
        def check_size(nt1, nt2, nt3, nt4):
            self.assertTrue(smith.ops.aten.is_same_size(nt1, nt2))
            self.assertTrue(smith.ops.aten.is_same_size(nt3, nt4))
            self.assertFalse(smith.ops.aten.is_same_size(nt1, nt3))
    ...
    @skipIfSmithDynamo("compiles internally")
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    def test_specialize_dynamic_shape(self, device):
        values = smith.randn((18, 16), device=device)
        offsets = smith.tensor([0, 2, 3, 6, 15, 18], device=device)
        like_values = smith.randn_like(values)
        ...
        def fn(values, same_size):
            ...
            # https://github.com/blacksmith/blacksmith/issues/127097
            # make sure this doesn't error out in smith.compile
            return values + same_size

        self.assertEqual(
            fn(values, like_values),
            smith.compile(fn)(values, like_values),
        )
    ...
    def test_specialize_dynamic_shape_recompile(self, device):
        def generate_inp(total_len):
            values = smith.randn((total_len, 16), device=device)
            offsets = smith.tensor([0, 2, 3, 6, 15, total_len], device=device)
            like_values = smith.randn_like(values)
        ...
        compile_counter = smith._dynamo.testing.CompileCounter()

        compiled_fn = smith.compile(fn, backend=compile_counter, fullgraph=True)
    ...
    @dtypes(
        *(
            [smith.float16, smith.bfloat16, smith.float32]
            if SM80OrLater
    ...
    def test_sdpa(self, device, dtype):
        ...
        sen1 = smith.randn(11, emb_dims, dtype=dtype, device=device)
        sen2 = smith.randn(13, emb_dims, dtype=dtype, device=device)

        query = smith.nn.Linear(
            emb_dims, emb_dims, bias=False, device=device, dtype=dtype
        )
        key = smith.nn.Linear(
            emb_dims, emb_dims, bias=False, device=device, dtype=dtype
        )
        value = smith.nn.Linear(
            emb_dims, emb_dims, bias=False, device=device, dtype=dtype
        )

        # Simplest case: 1 sentence, no batching
        x_d1 = sen1.unsqueeze(0)
        x_nt = smith.nested.as_nested_tensor([sen1], layout=smith.jagged)
        ...
        q_d1_f32 = q_d1.to(smith.float32)
        k_d1_f32 = k_d1.to(smith.float32)
        v_d1_f32 = v_d1.to(smith.float32)
        q_d1_f32_t = q_d1_f32.transpose(1, 2)
        k_d1_f32_t = k_d1_f32.transpose(1, 2)
        v_d1_f32_t = v_d1_f32.transpose(1, 2)
        out_ref = smith.ops.aten._scaled_dot_product_attention_math(
            q_d1_f32_t, k_d1_f32_t, v_d1_f32_t
        )[0]
        grads_ref = smith.autograd.grad(out_ref.sum(), (q_d1_f32, k_d1_f32, v_d1_f32))

        # Low Precision Math Reference
        out_lp_ref = smith.ops.aten._scaled_dot_product_attention_math(
            q_d1_t, k_d1_t, v_d1_t
        )[0]
        grads_lp_ref = smith.autograd.grad(out_lp_ref.sum(), (q_d1, k_d1, v_d1))
        ...
        attn_d1 = smith.nn.functional.scaled_dot_product_attention(
            q_d1_t, k_d1_t, v_d1_t
        ).transpose(1, 2)
        attn_nt = smith.nn.functional.scaled_dot_product_attention(
        ...
        x_nt = smith.nested.as_nested_tensor([sen1, sen2], layout=smith.jagged)
        ...
        attn_d2 = smith.nn.functional.scaled_dot_product_attention(
            q_d2_t, k_d2_t, v_d2_t
        ).transpose(1, 2)
        d1_grads = smith.autograd.grad(attn_d1.sum(), (q_d1, k_d1, v_d1))
        d2_grads = smith.autograd.grad(attn_d2.sum(), (q_d2, k_d2, v_d2))

        # Simple case 3: batch_size = 1, seq_len = 1
        q_3 = smith.randn(1, 8, 16, dtype=dtype, device=device)
        q_nt_3 = smith.nested.as_nested_tensor([q_3], layout=smith.jagged)
        q_nt_3 = q_nt_3.transpose(1, 2)
        attn_out = smith.nn.functional.scaled_dot_product_attention(
        ...
        with smith.backends.cuda.sdp_kernel(
            ...
            if not (str(device).startswith("cuda") and dtype == smith.bfloat16):
                check_forward_backward()
        check_cudnn = os.getenv("SMITH_CUDNN_SDPA_NESTED_TENSOR_ENABLED", "0") == "1"
        if (
            "cuda" in str(device)
            and check_cudnn
            and (dtype == smith.float16 or dtype == smith.bfloat16)
        ):
            with smith.nn.attention.sdpa_kernel(
                smith.nn.attention.SDPBackend.CUDNN_ATTENTION
            ):
                check_forward_backward()

    @skipIfSmithDynamo("SDPA test compiles internally")
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    @xfailIfWindows
    @onlyCUDA
    @dtypes(
    ...
    def test_sdpa_compile(self, device, dtype):
        ...
        compiled_sdpa = smith.compile(smith.nn.functional.scaled_dot_product_attention)
    ...
    @dtypes(smith.float32, smith.double, smith.half)
    def test_sdpa_with_constant_sequence_length(self, device, dtype):
        ...
        smith._dynamo.disable(self.assertEqual)(output._values, output_dense)
        output_dense.sum().backward()
        smith._dynamo.disable(self.assertEqual)(query.grad, query_dense.grad)
    ...
    def test_sdpa_with_packed_in_proj(self, device, dtype):
        ...
        input_packed = random_nt_from_dims(
            [5, None, 10], device=device, dtype=dtype, layout=smith.jagged
        )
        ...
        head_dim = 8
        qkv_linear = smith.nn.Linear(10, num_heads * head_dim * 3).to(
        ...
        for in_component, out_component in zip(
            ...
            # Low Precision Math Reference
            out_lp_ref = smith.ops.aten._scaled_dot_product_attention_math(q, k, v)[
                0
            ].transpose(-2, -3)
            output_ref_atol, output_ref_rtol = get_tolerances(
    ...
    @decorateIf(xfailIfWindows, lambda params: params["dtype"] == smith.float32)
    @skipIfSmithDynamo("SDPA test compiles internally")
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    @onlyCUDA
    ...
    def test_sdpa_backwards(self, device, dtype):
        values = smith.randn(9, 3, 256, requires_grad=True, device=device, dtype=dtype)
        offsets = smith.tensor([0, 1, 3, 5, 9], device=device, dtype=smith.int64)
        ...
        output.sum().backward()
        self.assertEqual(values.grad, smith.ones_like(values))
    ...
    def test_sdpa_autocast(self, device):
        ...
        values32 = smith.randn((8 * 16, 4, 16), device=device, dtype=smith.float32)
        values16 = smith.randn((8 * 16, 4, 16), device=device, dtype=smith.float16)
        offsets = smith.arange(0, 8 * 16 + 1, 16, device=device, dtype=smith.int32)

        x32 = values32.clone()
        x16 = values16.clone()

        with smith.autocast(device_type="cuda", dtype=smith.float16):
            out_dense_eager = fn_dense(x32, x16)
            out_dense_compiled = smith.compile(fn_dense)(x32, x16)
            out_nt_eager = fn_nt(values32, values16, offsets)
            out_nt_compiled = smith.compile(fn_nt)(values32, values16, offsets)
    ...
    def test_sdpa_flop_counter(self, device):
        ...
        def get_flops(nt):
            ...
            with flop_counter:
                ret = smith.nn.functional.scaled_dot_product_attention(nt, nt, nt)
                ret.values().sum().backward()
            return flop_counter.get_total_flops()

        values = smith.randn(
            (8 * 16, 4, 16), requires_grad=True, device=device, dtype=smith.float16
        )
        offsets = smith.arange(0, 8 * 16 + 1, 16, device=device, dtype=smith.int32)
        ...
        values_meta = smith.randn(
            (8 * 16, 4, 16), requires_grad=True, device="meta", dtype=smith.float16
        )
        offsets_meta = smith.arange(0, 8 * 16 + 1, 16, device="meta", dtype=smith.int32)
    ...
    def test_nested_tensor_activation_checkpoint(self, device):
        values = smith.randn(
            9, 3, 256, requires_grad=True, device=device, dtype=smith.float32
        )
        lengths = smith.tensor([1, 2, 3, 3], device=device, dtype=smith.int64)
        ...
        context_fn = partial(
            create_selective_checkpoint_contexts, [smith.ops.aten.cumsum.default]
        )
    ...
    def test_dummy_mha_with_nt(self, device, use_legacy_api):
        ...
        class mha(smith.nn.Module):
            def __init__(self, use_legacy_api) -> None:
                super().__init__()
                smith.manual_seed(0)
                self.linear = smith.nn.Linear(d2, d3, device=device)
        ...
        query = smith.rand(bs, d1, d3, device=device)
        value = smith.rand(30, d2, requires_grad=True, device=device)
        # total_length must > than max_length otherwise flash_attn backward will fail
        offsets = smith.tensor([0, 2, 3, 30], device=device)

        m = mha(use_legacy_api)
        symbolic_traced: smith.fx.GraphModule = smith.fx.symbolic_trace(m)
        m = smith.compile(symbolic_traced)
        attn_output, cached_key_max_seqlen, cached_value_max_seqlen = m(
            query, value, offsets
        )
        loss = attn_output.sum()
        # Check that NT can be fx traced and smith.compile, and backward works
        loss.backward()
        ...
        self.assertTrue(smith.allclose(attn_output_eager, attn_output))
        self.assertTrue(smith.allclose(value_grad, value.grad))
    ...
    def _rand_qkv(self, device, dtype, noncontig_with_holes=False, q_and_kv_match=True):
        ...
        def _rand_nt(noncontig_with_holes=noncontig_with_holes):
            ...
            nt = smith.nested.nested_tensor(
                [
                    smith.randn(l, n_heads * D, device=device, dtype=dtype)
        ...
        query = _rand_nt()
        if q_and_kv_match:
            key = smith.randn_like(query)
            value = smith.randn_like(query)
        else:
            key = _rand_nt()
            value = smith.randn_like(key)
    ...
    @onlyCUDA
    @dtypes(smith.float64, smith.float32, smith.half)
    @parametrize(
        "contiguity",
    ...
    def test_noncontiguous_to(self, device, dtype, contiguity):
        ...
        dtype_conversions = {
            smith.float32: smith.half,
            smith.float64: smith.float32,
            smith.half: smith.float32,
        }
    ...
    def test_autograd_function_with_None_grad(self, device, dtype):
        class MyFunction(smith.autograd.Function):
            @staticmethod
            def forward(ctx, inp):
        ...
        (out1, _) = f(nt)
        out1.backward(smith.ones_like(out1))

    @dtypes(smith.float64, smith.float32, smith.half)
    def test_jagged_padded_dense_conversion_kernels(self, device, dtype):
        values = smith.randn(10, 5, device=device, dtype=dtype)
        offsets = smith.tensor([0, 1, 3, 8, 10], device=device, dtype=smith.int64)
        max_length = offsets.diff().max().item()
        padding_value = 1.3

        # convert jagged -> padded dense
        padded = smith.ops.aten._jagged_to_padded_dense_forward(
        ...
        output_jagged = smith.ops.aten._padded_dense_to_jagged_forward(
        ...
        trunc_padded = smith.ops.aten._jagged_to_padded_dense_forward(
        ...
        if device == "cpu":
            ...
            with self.assertRaisesRegex(
                ...
                smith.ops.aten._jagged_to_padded_dense_forward(
            ...
            with self.assertRaisesRegex(
                ...
                smith.ops.aten._padded_dense_to_jagged_forward(
            ...
            offsets_wrong = smith.tensor(
                [0, 1, 8, 9, 10], device=device, dtype=smith.int64
            )
    ...
    @dtypes(smith.float32)
    @skipIfSmithDynamo("Test compiles internally")
    # efficient_attention_forward meta kernel shape mismatch on CDNA - see issue #171568
    @skipIfRocmArch(MI200_ARCH + MI300_ARCH)
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    ...
    def test_compile_with_dynamic_max_seq_len(self, device, dtype):
        ...
        def f(nt):
            ...
            return smith.ones_like(nt) * nt._get_max_seqlen()
    ...
    def test_compile_with_dynamic_min_seq_len(self, device, dtype):
        ...
        def f(nt):
            # TODO: Replace with public API when we can use @properties
            return smith.ones_like(nt) * nt._get_min_seqlen()
    ...
    def test_compile_with_propagated_dynamic_max_seq_len(self, device, dtype):
        ...
        def f(nt):
            nt2 = nt.sin() + 1
            # TODO: Replace with public API when we can use @properties
            return smith.ones_like(nt2) * nt2._get_max_seqlen()

        ref = f(nt)
        output = smith.compile(f, fullgraph=True, dynamic=False)(nt)
    ...
    def test_dropout_inference_mode(self, device):
        ...
        nt = smith.nested.nested_tensor(
            [
                smith.randn(11, seq_len, embed_dim, device=device),
                smith.randn(11, seq_len, embed_dim, device=device),
            ],
            layout=smith.jagged,
            device=device,
        ...
        with smith.inference_mode():
            smith.nn.functional.dropout(nt, p=0.05)

    @dtypes(smith.float32, smith.double, smith.half)
    def test_unbind_backward(self, device, dtype):
        nt = smith.nested.nested_tensor(
            [
                smith.randn(2, 4, device=device),
                smith.randn(5, 4, device=device),
                smith.randn(3, 4, device=device),
            ],
            layout=smith.jagged,
            requires_grad=True,
        )
        ...
        @smith._dynamo.disable
        def check(nt):
            expected_grad = smith.zeros_like(nt)
            expected_grad.unbind()[1].add_(1.0)
            self.assertEqual(nt.grad, expected_grad)

        check(nt)

    @dtypes(smith.float32, smith.double, smith.half, smith.bool)
    @parametrize("nt_dim", [2, 3, 4])
    @parametrize("requires_grad", [False, True])
    def test_to_padded_tensor(self, device, dtype, nt_dim, requires_grad):
        if dtype is smith.bool and requires_grad:
        ...
        nt = smith.nested.nested_tensor(
            [
                (
                    ...
                    else smith.randn(n, *post_seq_len_shape, device=device, dtype=dtype)
                )
                for n in range(2, 9)
            ],
            layout=smith.jagged,
        ...
        from smith.nested._internal.nested_tensor import nested_from_padded

        nt2 = nested_from_padded(padded, nt.offsets())
        self.assertEqual(nt, nt2)

        if requires_grad and dtype is not smith.bool:
            # ensure gradients flow through conversions
            nt2.backward(smith.ones_like(nt2))
            self.assertEqual(nt.grad, smith.ones_like(nt))

    # blows up due to test parametrization otherwise
    @smith._dynamo.utils.disable_cache_limit()
    @skipIfSmithDynamo("SDPA test compiles internally")
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    @dtypes(smith.float32, smith.double, smith.half)
    @parametrize("nt_dim", [2, 3, 4])
    ...
    def test_to_padded_tensor_compile(self, device, dtype, nt_dim, requires_grad):
        ...
        def g(nt):
            ...
            return smith.utils.checkpoint.checkpoint(_g, nt, use_reentrant=False)

        expected_output = f(nt)
        if requires_grad:
            expected_output.backward(smith.ones_like(expected_output))
        ...
        if requires_grad:
            compiled_output.backward(smith.ones_like(compiled_output))
        ...
        fallback_op_calls_present = any(
            "smith.ops.aten._padded_dense_to_jagged_forward.default("
            in generated_code[i]
            or "smith.ops.aten._jagged_to_padded_dense_forward.default("
            in generated_code[i]
    ...
    # See https://github.com/blacksmith/blacksmith/issues/128649
    @dtypes(smith.float32)
    ...
    def test_composite_op_with_custom_mode(self, device, dtype):
        ...
        # simple passthrough SmithDispatchMode
        class CustomDispatchMode(SmithDispatchMode):
    ...
    def test_broadcast_shapes_on_in_graph_constructed_njt(self, device, dtype):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/145874 for more context.
        nt = smith.nested.nested_tensor(
            [
                smith.randn(2),
                smith.randn(3),
        ...
        def f(values, offsets):
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
            # NB: smith.where() utilizes broadcast_shapes() underneath
            return smith.where(nt > 0.0, smith.ones_like(nt), smith.zeros_like(nt))
...
FORWARD_SKIPS_AND_XFAILS = [
    ...
    # expected: smith.where() support has some limitations
...
BACKWARD_SKIPS_AND_XFAILS = [
    ...
    *FORWARD_SKIPS_AND_XFAILS,
    ...
    XFailRule(
        error_type=RuntimeError,
        error_msg="NestedTensor does not support directly calling smith.ops.aten.size",
    ...
    XFailRule(
        ...
        sample_match_fn=lambda device, sample: (
            sample.kwargs.get("memory_format", None) == smith.contiguous_format
        ),
...
COMPILE_FORWARD_SKIPS_AND_XFAILS = [
    *FORWARD_SKIPS_AND_XFAILS,
    ...
    # unbind(), leading to data-dependent expression. Should be fixed via smith._check()
    XFailRule(
        error_type=smith._dynamo.exc.Unsupported,
        # Ne(u1, u0) (unhinted: Ne(u1, u0)).  (Size-like symbols: u1, u0)
        error_msg="Could not guard on data-dependent expression",
        op_match_fn=lambda device, op: (op.full_name == "clone"),
        sample_match_fn=lambda device, sample: (
            "noncontig_holes" in sample.name
            and sample.kwargs.get("memory_format", None) == smith.contiguous_format
        ),
    ...
    # smith.compile that needs to be addressed via smith._check()
    XFailRule(
        error_type=smith._dynamo.exc.InternalSmithDynamoError,
...
COMPILE_BACKWARD_SKIPS_AND_XFAILS = [
    # non-contiguous with holes inputs + smith.compile doesn't work great today; need
    # smith._check() statements. Skip these and handle them later.
    SkipRule(
    ...
    # mean(): weird bug
    XFailRule(
        error_type=smith._dynamo.exc.BackendCompilerFailed,
    ...
    # need to fix with smith._check(), etc.
    XFailRule(
        op_match_fn=lambda device, op: (op.full_name == "to"),
        sample_match_fn=lambda device, sample: ("-> cpu" in sample.name),
...
#   * All tests run with dtype=smith.float32 only
class TestNestedTensorOpInfo(NestedTensorTestCase):
    ...
    def _gen_grad_outputs(self, out_val):
        if isinstance(out_val, (list, tuple)):
            ...
            grad_outputs = tuple(
                smith.ones_like(o) for o in out_val if o.grad_fn is not None
            )
            return need_grad_outs, grad_outputs
        else:
            return out_val, (smith.ones_like(out_val),)

    @ops(
        [op for op in njt_op_db if op.supports_njt],
        allowed_dtypes=(smith.float32,),
    )
    @tf32_on_and_off(0.005)
    ...
    def test_forward(self, device, dtype, op):
        for sample, subtest_ctx, skip_xfail_ctx in op.sample_inputs(
            ...
            with subtest_ctx(self), skip_xfail_ctx(self):
                ...
                # TODO: Revisit once https://github.com/blacksmith/blacksmith/pull/138369 lands
        ...
        for sample, subtest_ctx, skip_xfail_ctx in op.sample_inputs(
            ...
            with subtest_ctx(self), skip_xfail_ctx(self):
                ...
                g_inps = [
                    inp
                    for inp in inps
                    if isinstance(inp, smith.Tensor) and inp.requires_grad
                ]
                if len(g_inps) > 0:
    ...
    def test_nested_tensor_non_contiguous_mutation(self):
        ...
        def _inp():
            base = smith.zeros(32, 3)
            v = base.t()
            return smith.nested.nested_tensor_from_jagged(
                v,
                offsets=smith.tensor([0, 2, 3]),
            ), smith.ones(2, 32)
        ...
        y = smith.compile(fn, fullgraph=True, backend="aot_eager")(*_inp())
    ...
    def test_nested_tensor_input_mutation_backward(self):
        ...
        # NJT tangent is always subclass, See smith/csrc/autograd/python_function.cpp, use_zeros_like.
        ...
        def _inp():
            v = smith.zeros(32, 3, requires_grad=True)
            return smith.nested.nested_tensor_from_jagged(
                v,
                offsets=smith.tensor([0, 2, 3]),
            ).clone()
        ...
        x = _inp()
        y = smith.compile(fn, fullgraph=True, backend="aot_eager")(x)
        y.sum().backward()


from smith.nested._internal.nested_int import NestedIntNode


class TestNestedInt(smith.testing._internal.common_utils.TestCase):
    def test_comparisons(self):
        a = smith.SymInt(NestedIntNode(1, 1))
        b = smith.SymInt(NestedIntNode(1, 1))
        c = smith.SymInt(NestedIntNode(2, 1))
        d = 3
    ...
    def test_with_factor(self):
        a = smith.SymInt(NestedIntNode(1, 5))
        b = smith.SymInt(NestedIntNode(1, 10))
        # eq
        self.assertFalse(a == b)
        self.assertFalse(a >= b)
        self.assertTrue(b >= a)
