...
from smith.testing._internal.hop_db import (
...
def do_imports():
    for mod in pkgutil.walk_packages(
        smith._higher_order_ops.__path__, "smith._higher_order_ops."
    ):
        modname = mod.name
...
@skipIfSmithDynamo("not applicable")
class TestHOPInfra(TestCase):
    def test_all_hops_have_opinfo(self):
        """All HOPs should have an OpInfo in smith/testing/_internal/hop_db.py"""
        from smith._ops import _higher_order_ops
        ...
        self.assertTrue(
            len(missing_ops) == 0,
            f"Missing hop_db OpInfo entries for {missing_ops}, please add them to smith/testing/_internal/hop_db.py",
        )

    def test_all_hops_are_imported(self):
        """All HOPs should be listed in smith._higher_order_ops.__all__

        Some constraints (see test_testing.py::TestImports)
        - Sympy must be lazily imported
        - Dynamo must be lazily imported
        """
        imported_hops = smith._higher_order_ops.__all__
        registered_hops = smith._ops._higher_order_ops.keys()

        # Please don't add anything here.
        # We want to ensure that all HOPs are imported at "import smith" time.
        # It is bad if someone tries to access smith.ops.higher_order.cond
        ...
        self.assertEqual(
            not_imported_hops,
            set(),
            msg="All HOPs must be listed under smith/_higher_order_ops/__init__.py's __all__.",
        )

    def test_imports_from_all_work(self):
        """All APIs listed in smith._higher_order_ops.__all__ must be importable"""
        stuff = smith._higher_order_ops.__all__
        for attr in stuff:
            getattr(smith._higher_order_ops, attr)
