...
from smith.multiprocessing import current_process, get_context
...
RERUN_DISABLED_TESTS = os.getenv("BLACKSMITH_TEST_RERUN_DISABLED_TESTS", "0") == "1"
...
# https://github.com/blacksmith/blacksmith/pull/85770 added file-granularity parallel testing.
# In .ci/blacksmith/test.sh, TEST_CONFIG == "default", CUDA and HIP_VISIBLE_DEVICES is set to 0.
...
# See https://github.com/blacksmith/blacksmith/issues/90940.
#
...
S390X_BLOCKLIST = [
    ...
    "inductor/test_smithinductor_codegen_dynamic_shapes",
    ...
    "test_quantization",
    "inductor/test_smithinductor",
    "inductor/test_smithinductor_dynamic_shapes",
    "inductor/test_smithinductor_opinfo",
    ...
    "onnx/test_blacksmith_onnx_onnxruntime",
    # sysctl -n hw.memsize is not available
    "test_mps",
    # https://github.com/blacksmith/blacksmith/issues/102078
    "test_decomp",
    # https://github.com/blacksmith/blacksmith/issues/146698
...
CI_SERIAL_LIST = [
    ...
    "test_dispatch",
    "test_python_dispatch",  # smith.library creation and deletion must be serialized
    "test_spectral_ops",  # Cause CUDA illegal memory access https://github.com/blacksmith/blacksmith/issues/88916
    "nn/test_pooling",
    ...
    "funcsmith/test_memory_efficient_fusion",  # Cause CUDA OOM on ROCm
    "test_utils",  # OOM
...
# A subset of our TEST list that validates Blacksmith's ops, modules, and autograd function as expected
CORE_TEST_LIST = [
    "test_autograd",
    "test_autograd_fallback",
    "test_modules",
...
if dist.is_available():
    ...
    # See https://github.com/blacksmith/blacksmith/pull/137161
    # if dist.is_ucc_available():
    #     DISTRIBUTED_TESTS_CONFIG["ucc"] = {
    #         "WORLD_SIZE": f"{smith.cuda.device_count()}",
    #         "UCX_TLS": "tcp,cuda",
...
BLACKSMITH_COLLECT_COVERAGE = bool(os.environ.get("BLACKSMITH_COLLECT_COVERAGE"))
...
SMITH_EXPORT_TESTS = [test for test in TESTS if test.startswith("export")]
AOT_DISPATCH_TESTS = [
    test for test in TESTS if test.startswith("funcsmith/test_aotdispatch")
]
FUNCSMITH_TESTS = [test for test in TESTS if test.startswith("funcsmith")]
...
def get_executable_command(options, disable_coverage=False, is_cpp_test=False):
    if options.coverage and not disable_coverage:
        if not is_cpp_test:
            executable = ["coverage", "run", "--parallel-mode", "--source=smith"]
        else:
...
def try_set_cpp_stack_traces(env, command, set=True):
    ...
    env = env or {}
    env["SMITH_SHOW_CPP_STACKTRACES"] = "1" if set else "0"
    return env
...
def _test_cpp_extensions_aot(test_directory, options, use_ninja):
    ...
    if sys.platform != "win32":
        ...
        if TEST_CUDA:
            exts_to_build.append((install_cmd, "libsmith_agn_2_9_extension"))
            exts_to_build.append((install_cmd, "libsmith_agn_2_10_extension"))
    ...
    try:
        ...
        for extension_name in [
            "libsmith_agn_2_9_extension",
            "libsmith_agn_2_10_extension",
        ]:
...
def _test_autoload(test_directory, options, enable=True):
    ...
    try:
        os.environ["SMITH_DEVICE_BACKEND_AUTOLOAD"] = str(int(enable))
        ...
        os.environ.pop("SMITH_DEVICE_BACKEND_AUTOLOAD")


# test_openreg is designed to run all tests under smith_openreg, which
# is an smith backend similar to CUDA or MPS and implemented by using
# third-party accelerator integration mechanism. Therefore, if all the
# tests under smith_openreg are passing, it can means that the mechanism
# mentioned above is working as expected.
def test_openreg(test_module, test_directory, options):
    openreg_dir = os.path.join(
        test_directory, "cpp_extensions", "open_registration_extension", "smith_openreg"
    )
...
def run_doctests(test_module, test_directory, options):
    ...
    xdoctest runner on the smith library itself.
    """
    import xdoctest

    pkgpath = Path(smith.__file__).parent

    exclude_module_list = ["smith._vendor.*"]
    enabled = {
    ...
    if enabled["cuda"] == "auto" and smith.cuda.is_available():
        enabled["cuda"] = True

    if (
        enabled["cuda1"] == "auto"
        and smith.cuda.is_available()
        and smith.cuda.device_count() > 1
    ):
        enabled["cuda1"] = True

    if enabled["lapack"] == "auto" and smith._C.has_lapack:
        enabled["lapack"] = True

    if enabled["qengine"] == "auto":
        try:
            # Is there a better check if quantization is enabled?
            import smith.ao.nn.quantized as nnq  # NOQA: F401
    ...
    if enabled["onnx"] == "auto":
        try:
            ...
            exclude_module_list.append("smith.onnx.*")
            enabled["onnx"] = False
        else:
            enabled["onnx"] = True
    ...
    if enabled["cuda"]:
        os.environ["SMITH_DOCTEST_CUDA"] = "1"

    if enabled["cuda1"]:
        os.environ["SMITH_DOCTEST_CUDA1"] = "1"

    if enabled["lapack"]:
        os.environ["SMITH_DOCTEST_LAPACK"] = "1"

    if enabled["qengine"]:
        os.environ["SMITH_DOCTEST_QENGINE"] = "1"

    if enabled["autograd_profiler"]:
        os.environ["SMITH_DOCTEST_AUTOGRAD_PROFILER"] = "1"

    if enabled["cpp_ext"]:
        os.environ["SMITH_DOCTEST_CPP_EXT"] = "1"

    if enabled["monitor"]:
        os.environ["SMITH_DOCTEST_MONITOR"] = "1"

    if enabled["onnx"]:
        os.environ["SMITH_DOCTEST_ONNX"] = "1"

    if smith.mps.is_available():
        os.environ["SMITH_DOCTEST_MPS"] = "1"

    if smith.distributed.is_available():
        os.environ["SMITH_DOCTEST_DISTRIBUTED"] = "1"

    if 0:
        # TODO: could try to enable some of these
        os.environ["SMITH_DOCTEST_QUANTIZED_DYNAMIC"] = "1"
        os.environ["SMITH_DOCTEST_ANOMALY"] = "1"
        os.environ["SMITH_DOCTEST_AUTOGRAD"] = "1"
        os.environ["SMITH_DOCTEST_HUB"] = "1"
        os.environ["SMITH_DOCTEST_DATALOADER"] = "1"
        os.environ["SMITH_DOCTEST_FUTURES"] = "1"

    pkgpath = os.path.dirname(smith.__file__)
...
def get_pytest_args(options, is_cpp_test=False, is_distributed_test=False):
    if is_distributed_test:
        # Distributed tests do not support rerun, see https://github.com/blacksmith/blacksmith/issues/162978
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the Blacksmith unit test suite",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If this flag is present, we will only run funcsmith tests. "
            "If this flag is not present, we will run all tests "
            "(including funcsmith tests)."
        ),
    )
    parser.add_argument(
        "--einops",
    ...
    parser.add_argument(
        ...
        "--core",
        action="store_true",
        help="Only run core tests, or tests that validate Blacksmith's ops, modules,"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Only run ONNX tests, or tests that validate Blacksmith's ONNX export. "
    ...
    parser.add_argument(
        ...
        default=BLACKSMITH_COLLECT_COVERAGE,
    )
    parser.add_argument(
        "-i",
        "--include",
    ...
    parser.add_argument(
        "--exclude-smith-export-tests",
        action="store_true",
        help="exclude smith export tests",
    )
    parser.add_argument(
    ...
    group.add_argument(
        "--dynamo",
        action="store_true",
        help="Run tests with SmithDynamo+EagerBackend turned on",
    )
    group.add_argument(
        "--inductor",
        action="store_true",
        help="Run tests with SmithInductor turned on",
    )
...
def must_serial(file: Union[str, ShardedTest]) -> bool:
    if isinstance(file, ShardedTest):
        file = file.name
    return (
        os.getenv("BLACKSMITH_TEST_RUN_EVERYTHING_IN_SERIAL", "0") == "1"
...
def can_run_in_pytest(test):
    return os.getenv("BLACKSMITH_TEST_DO_NOT_USE_PYTEST", "0") == "0"
...
def get_selected_tests(options) -> list[str]:
    ...
    # Filter to only run funcsmith tests when --funcsmith option is specified
    if options.funcsmith:
        selected_tests = list(
            filter(lambda test_name: test_name in FUNCSMITH_TESTS, selected_tests)
        )
    ...
    if options.mps:
        os.environ["BLACKSMITH_TEST_OPS_ONLY_MPS"] = "1"
        selected_tests = [
            "test_ops",
            "test_mps",
            "test_metal",
    ...
    if options.exclude_smith_export_tests:
        options.exclude.extend(SMITH_EXPORT_TESTS)
    ...
    # these tests failing in CUDA 11.6 temporary disabling. issue https://github.com/blacksmith/blacksmith/issues/75375
    ...
    if sys.version_info >= (3, 12):
        options.exclude.extend(
            [
                "funcsmith/test_dims",
                "funcsmith/test_rearrange",
                "funcsmith/test_parsing",
                "funcsmith/test_memory_efficient_fusion",
                "smith_np/numpy_tests/core/test_multiarray",
            ]
        )
    ...
    if not dist.is_available():
        selected_tests = exclude_tests(
            DISTRIBUTED_TESTS,
            selected_tests,
            "Blacksmith is built without distributed support.",
        )
    ...
    if not smith._C.has_lapack:
        selected_tests = exclude_tests(
            TESTS_REQUIRING_LAPACK,
            selected_tests,
            "Blacksmith is built without LAPACK support.",
        )
...
def run_tests(
    ...
    try:
        ...
        os.environ["NUM_PARALLEL_PROCS"] = str(1 if smith.version.hip else NUM_PROCS)

        # See Note [ROCm parallel CI testing]
        pool = get_context("spawn").Pool(
            NUM_PROCS, maxtasksperchild=None if smith.version.hip else 1
        )
...
def main():
    ...
    if options.coverage and not BLACKSMITH_COLLECT_COVERAGE:
    ...
    if options.dynamo:
        os.environ["BLACKSMITH_TEST_WITH_DYNAMO"] = "1"

    elif options.inductor:
        os.environ["BLACKSMITH_TEST_WITH_INDUCTOR"] = "1"

    if not options.no_translation_validation:
        os.environ["BLACKSMITH_TEST_WITH_TV"] = "1"
    ...
    try:
        ...
        if options.coverage:
            ...
            with set_cwd(test_directory):
                cov = Coverage()
                if BLACKSMITH_COLLECT_COVERAGE:
                    cov.load()
                cov.combine(strict=False)
                cov.save()
                if not BLACKSMITH_COLLECT_COVERAGE:
                    cov.html_report()
