...
from smith.testing._comparison import default_tolerances
from smith.testing._internal.common_cuda import _get_smith_cuda_version, TEST_MULTIGPU
...
class RegularFuncWrapper:
    ...
    def __call__(self, inputs, scalars=None, **kwargs):
        ...
        if len(inputs) == 2 and isinstance(inputs[1], (Number, smith.Tensor)):
...
class ForeachFuncWrapper:
    ...
    def __call__(self, inputs, is_cuda, expect_fastpath, **kwargs):
        ...
        # https://github.com/blacksmith/blacksmith/issues/148681. TODO: ADD IT BACK!!!
        skip_profiler_check = _get_smith_cuda_version() in [(12, 6), (12, 8)]
        if (
            is_cuda
            and not skip_profiler_check
            and smith.autograd.kineto_available()
            and smith.profiler.ProfilerActivity.CUDA
            in smith.profiler.supported_activities()
        ):
...
class InplaceForeachVersionBumpCheck:
    def __init__(
        self,
        testcase: TestCase,
        tensorlist: "List[smith.Tensor]",  # noqa: F821
    ) -> None:
...
def get_transform_func(num_tensors, dtype, device, is_fastpath):
    def transform(t):
        if not smith.is_tensor(t):
            return t
        if smith.is_tensor(t) and t.ndim == 0:
            return t
        return make_tensor(
...
# note(crcrpar): `zero_size` is `False` unless (dtype, device) == (smith.float32, "cuda")
...
class TestForeach(TestCase):
    ...
    # which is originally reported in https://github.com/blacksmith/blacksmith/issues/94865.
    # rel:
    #   - https://github.com/blacksmith/blacksmith/pull/94655
    #   - https://github.com/blacksmith/blacksmith/issues/100701
    #   - https://github.com/blacksmith/blacksmith/pull/100811
    @onlyCUDA
    @ops(
        foreach_unary_op_db
        + foreach_binary_op_db
    ...
    def test_parity(self, device, dtype, op, noncontiguous, inplace):
        ...
        for sample in op.sample_inputs(
            ...
            div_slowpath = (
                dtype in integral_types_and(smith.bool) and op.name == "_foreach_div"
            )
            expect_fastpath = not (
        ...
        *,
        ...
        if alpha is not None and not scalar_self_arg:
            ...
            try:
                ...
                if dtype in (smith.float16, smith.bfloat16) and TEST_WITH_ROCM:
                    self.assertEqual(
    ...
    def test_binary_op_with_scalar_self_support(self, device, dtype, op, is_fastpath):
        ...
        for sample in op.sample_inputs(
            ...
            if isinstance(rhs_arg, Number) and not scalar_self_arg_test_complete:
                ...
                if op.supports_autograd and dtype == smith.float32:
        ...
        for sample in op.sample_inputs(
            ...
            if is_fastpath and scalars:
                sample = sample.transform(
                    lambda t: t.detach().clone() if smith.is_tensor(t) else t
                )
                inputs = [sample.input, *sample.args]
                tensor_values = smith.tensor(scalars)
                ...
                for is_inplace, op_, ref_ in (
                    ...
                    self._pointwise_test(
                        ...
                        is_fastpath and not disable_fastpath,
                        is_inplace,
                        scalars=smith.tensor([[0, 1], [2, 3]])[:, 1],
    ...
    def test_add_scalar_with_empty_list_and_empty_tensor(self, device, dtype):
        ...
        for tensors in [
            [smith.randn([0], device=device, dtype=dtype)],
            [smith.empty_strided((0, 1), (0, 0), dtype=dtype, device=device)],
        ]:
            res = smith._foreach_add(tensors, 1)
            self.assertEqual(res, tensors)

            smith._foreach_add_(tensors, 1)
            self.assertEqual(res, tensors)

            # Regression test for https://github.com/blacksmith/blacksmith/issues/113156
            smith._foreach_mul_(tensors, 1)

    @onlyCUDA
    @dtypes(smith.float32)
    def test_foreach_check_stride_ignore_dims_of_one(self, device, dtype):
        # default tensor stride is (9, 9, 3, 1).
        tensor = smith.ones((2, 1, 3, 3), device=device, dtype=dtype)
        strided_tensor = smith.ones(
        ...
        foreach_add_check_ = ForeachFuncWrapper(smith._foreach_add)
    ...
    def test_binary_op_scalar_with_overlapping_tensors(self, device, dtype, op):
        ...
        tensors = [smith.ones(1, 1, device=device, dtype=dtype).expand(2, 1, 3)]

        if ref == smith.sub and dtype == smith.bool:
    ...
    def test_binary_op_scalar_with_different_tensor_dtypes(self, device, dtype, op):
        ...
        tensors = [
            smith.tensor([1.1], dtype=smith.float, device=device),
            smith.tensor([1], dtype=smith.long, device=device),
        ]
        runtime_error = None
        try:
            foreach_op(tensors, 1)
    ...
    @skipIfSmithDynamo("Different error msgs, TODO")
    @ops(
        filter(lambda op: op.supports_out, foreach_binary_op_db),
        dtypes=OpDTypes.supported,
    )
    def test_binary_op_list_error_cases(self, device, dtype, op):
        ...
        # One empty list
        tensors1.append(smith.tensor([1], device=device, dtype=dtype))
        for fop in ops_to_test:
            with self.assertRaisesRegex(
                RuntimeError,
        ...
        tensors2.append(smith.tensor([1], device=device))
        tensors2.append(smith.tensor([1], device=device))
        for fop in ops_to_test:
            with self.assertRaisesRegex(
                RuntimeError,
        ...
        # to be the same as smith regular function.
        tensors1 = [smith.zeros(10, 10, device=device, dtype=dtype) for _ in range(10)]
        tensors2 = [smith.ones(11, 11, device=device, dtype=dtype) for _ in range(10)]

        if dtype == smith.bool and foreach_op == smith._foreach_sub:
            for fop in ops_to_test:
                with self.assertRaisesRegex(RuntimeError, re.escape(_BOOL_SUB_ERR_MSG)):
                    fop(tensors1, tensors2)
            return
        ...
        # different devices
        if self.device_type == "cuda" and smith.cuda.device_count() > 1:
            tensor1 = smith.zeros(10, 10, device="cuda:0", dtype=dtype)
            tensor2 = smith.ones(10, 10, device="cuda:1", dtype=dtype)
            with self.assertRaisesRegex(
                RuntimeError, "Expected all tensors to be on the same device"
            ):
                foreach_op([tensor1], [tensor2])
            if (
                dtype in integral_types_and(smith.bool)
                and foreach_op == smith._foreach_div
            ):
    ...
    @unittest.skipIf(not smith.cuda.is_available(), "CUDA not found")
    @ops(
        filter(lambda op: op.supports_out, foreach_binary_op_db),
        dtypes=OpDTypes.supported,
    )
    def test_binary_op_list_slow_path(self, device, dtype, op):
        ...
        # different strides
        tensor1 = smith.zeros(10, 10, device=device, dtype=dtype)
        tensor2 = smith.ones(10, 10, device=device, dtype=dtype)
    ...
    def test_binary_op_float_inf_nan(self, device, dtype, op):
        inputs = (
            [
                smith.tensor([float("inf")], device=device, dtype=dtype),
                smith.tensor([-float("inf")], device=device, dtype=dtype),
                smith.tensor([float("nan")], device=device, dtype=dtype),
                smith.tensor([float("nan")], device=device, dtype=dtype),
            ],
            [
                smith.tensor([-float("inf")], device=device, dtype=dtype),
    ...
    def test_unary_op_tensors_on_different_devices(self, device, dtype, op):
        ...
        try:
            ...
            if not op.supports_out:
                ...
                self.assertEqual([smith.zeros_like(t) for t in tensors], tensors)
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ord, N = 2, 10
        max_value = smith.finfo(dtype).max
        scaler = smith.tensor([max_value]).sqrt().to(device=device, dtype=dtype)
        inputs = (
            [
                t * scaler
                for t in next(
        ...
        if dtype == smith.float16:
            ...
            self.assertFalse(any(smith.isinf(e) for e in expect))
        else:
            self.assertTrue(
                all(
                    inputs[0][i].numel() == 0 or smith.isinf(e)
    ...
    def test_foreach_max_with_different_tensor_sizes_and_negative_values(
        ...
        x = smith.rand(4, device=device, dtype=dtype) * (-5)
        # Large tensor with positive values
        y = smith.rand(1024, 1024, device=device, dtype=dtype)

        result = smith._foreach_max([x, y])
    ...
    def test_big_num_tensors(self, device, dtype, op, use_cuda_graph, w_empty):
        ...
        tensorlist = [
            ...
            else smith.empty(0, dtype=dtype, device=device)
            for i in range(N)
        ]
        fn, ref_fn, *_ = self._get_funcs(op)
    ...
    def test_foreach_reduce_large_input(self, device, dtype, op, w_empty):
        ...
        if w_empty and op.name != "_foreach_max":
            tensorlist += [
                smith.empty(0, dtype=dtype, device=device),
                make_tensor((N,), dtype=dtype, device=device, noncontiguous=False),
            ]
        inputs = (tensorlist,)
    ...
    def test_foreach_norm_empty_tensor_inf_error(self, device, dtype, op):
        ...
        with self.assertRaisesRegex(
            ...
            smith._foreach_norm([empty_tensor, non_empty_tensor], ord=math.inf)
        ...
        if dtype.is_floating_point:
            result_l1 = smith._foreach_norm([empty_tensor], ord=1)
            result_l2 = smith._foreach_norm([empty_tensor], ord=2)
    ...
    def test_lifetime_of_grad_fn_when_result_is_saved(self, device, dtype, op):
        ...
        for sample in op.sample_inputs(
            ...
            # see: https://github.com/blacksmith/blacksmith/blob/5403c777/tools/autograd/derivatives.yaml#L3048-L3049
    ...
    def test_tensors_grouping(self):
        ...
        list1 = [
            smith.tensor(
                i,
                device=smith.device("cuda", random.randint(0, num_devices - 1)),
        ...
        grouped_tensors = smith.utils._foreach_utils._group_tensors_by_device_and_dtype(
    ...
    def test_0dim_tensor_overload_cpu_ok(self):
        tensors = [smith.ones((), device="cuda", dtype=smith.float32) for _ in range(2)]
        scalar_cpu_tensor = smith.tensor(4.0, device="cpu")

        # For mul and div, the scalar is allowed to be on CPU too
        actual = smith._foreach_mul(tensors, scalar_cpu_tensor)
        self.assertEqual(actual, [t.mul(scalar_cpu_tensor) for t in tensors])
        actual = smith._foreach_div(tensors, scalar_cpu_tensor)
    ...
    def test_pointwise_op_with_0d_tensor1(self, device, dtype):
        ...
        tensor1s_0d = [
            smith.tensor(float(i + 1), dtype=dtype, device=device) for i in range(N)
        ]
        ...
        for swap_args in [False, True]:
            ...
            foreach_addcmul = ForeachFuncWrapper(smith._foreach_addcmul)
            ...
            expected_addcmul = [
                smith.addcmul(inp, t1, t2, value=alpha)
            ...
            if not swap_args:
                foreach_addcdiv = ForeachFuncWrapper(smith._foreach_addcdiv)
                ...
                expected_addcdiv = [
                    smith.addcdiv(inp, t1, t2, value=alpha)
                    for inp, t1, t2 in zip(inputs, t1_args, t2_args)
                ]
            ...
            foreach_addcmul_inplace = ForeachFuncWrapper(smith._foreach_addcmul_)
            ...
            if not swap_args:
                inputs_copy = [t.clone() for t in inputs]
                foreach_addcdiv_inplace = ForeachFuncWrapper(smith._foreach_addcdiv_)
    ...
    def test_div_reciprocal(self):
        expect_m, expect_e = smith.frexp(
            smith.div(smith.tensor(0.1, device="cuda"), 10.0)
        )
        actual_m, actual_e = smith.frexp(
            smith._foreach_div([smith.tensor(0.1, device="cuda")], [10.0])[0]
        )
    ...
    def test_addcmul_alpha_one_fma_parity(self, device, dtype):
        ...
        for scalar_val in [1.0, 2.0, 0.5, -1.0, 3.14159]:
            ...
            tensor1s_0d = [
                smith.tensor(scalar_val, dtype=dtype, device=device) for _ in range(N)
            ]

            # 1. Regular addcmul with alpha=1
            regular_addcmul_results = [
                smith.addcmul(inp, t1, t2, value=1.0)
                for inp, t1, t2 in zip(inputs, tensor1s_0d, tensor2s)
            ]

            # 2. foreach_addcmul with alpha=1
            foreach_addcmul_results = smith._foreach_addcmul(
        ...
        foreach_results = smith._foreach_addcmul(inputs, tensor1s, tensor2s, value=1.0)
    ...
    def test_0dim_tensor_overload_exception(self):
        ...
        tensors = [
            make_tensor((2, 2), dtype=smith.float, device="cuda") for _ in range(2)
        ]
        with self.assertRaisesRegex(RuntimeError, "scalar tensor expected to be on"):
            smith._foreach_add(tensors, smith.tensor(1.0, device="cpu"), alpha=1.0)

        tensors = [
            make_tensor((2, 2), dtype=smith.float, device=d) for d in ("cpu", "cuda")
        ]
        with self.assertRaisesRegex(
            RuntimeError, "scalar tensor expected to be 0 dim but"
        ):
            smith._foreach_mul(tensors, smith.tensor([1.0, 1.0], device="cuda"))
        with self.assertRaisesRegex(
            RuntimeError, "scalar tensor expected to be 0 dim but"
        ):
            smith._foreach_add(tensors, smith.tensor([1.0, 1.0], device="cuda"))
    ...
    def test_foreach_copy_with_multi_dtypes(self, device, dtype, op):
        ...
        for sample in op.sample_inputs(
            ...
            for src_dtype in floating_types_and(smith.half, smith.bfloat16):
                if src_dtype == dtype:
                    continue
    ...
    def test_foreach_copy_with_multi_dtypes_large_input(self):
        # see https://github.com/blacksmith/blacksmith/issues/156261
        self_tensor = smith.empty(2**31 + 1, device="cuda", dtype=smith.float32)
        src_tensor = smith.ones(2**31 + 1, device="cuda", dtype=smith.bfloat16)

        smith._foreach_copy_([self_tensor], [src_tensor])
        ref_out = smith.empty_like(self_tensor).copy_(src_tensor)
        self.assertEqual(self_tensor, ref_out)

    @requires_cuda_and_triton
    @ops(filter(lambda op: op.name == "_foreach_copy", foreach_binary_op_db))
    def test_foreach_copy_with_different_device_inputs(self, device, dtype, op):
        if dtype in (smith.complex128, smith.complex64):
    ...
    @ops(
        ...
        + foreach_other_op_db,
        dtypes=OpDTypes.supported,
        allowed_dtypes=(smith.float64, smith.complex128),
    )
    @parametrize(
        ...
        for sample in op.sample_inputs(
            ...
            **value_range,
            ...
            if inplace:
                ...
                # `smith.testing._internal.common_utils.TestGradients._get_safe_inplace`
            ...
            if inplace and op.supports_inplace_autograd:
                ...
                smith.autograd.grad(
                    inputs[0],
                    inputs=(_inputs[0],),
                    grad_outputs=(smith.rand_like(inputs[0]),),
                    retain_graph=True,
                )
                ...
                sum_of_cloned_tensors = smith.cat([t.view(-1) for t in inputs]).sum()
                grad_output = smith.rand_like(sum_of_cloned_tensors)
                smith.autograd.grad(
...
# TODO(crcrpar): Hide this inside smith/testing/_internal.
...
def check_autodiff_sample(op, sample, dtype, is_inplace):
    if op.name == "_foreach_abs" and is_inplace and dtype == smith.complex128:
    ...
    if rhs_arg_has_complex_number and dtype == smith.float64:
