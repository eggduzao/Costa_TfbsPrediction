...
from smith.nn import CrossEntropyLoss
from smith.nn.utils._expanded_weights import ExpandedWeight
from smith.nn.utils._expanded_weights.expanded_weights_utils import (
    forward_helper,
...
from smith.nn.utils._per_sample_grad import call_for_per_sample_grads
from smith.testing._internal.common_cuda import TEST_CUDA, tf32_off
from smith.testing._internal.common_device_type import (
    instantiate_device_type_tests,
    OpDTypes,
    ops,
)
from smith.testing._internal.common_methods_invocations import op_db, SampleInput
...
class TestExpandedWeightHelperFunction(TestCase):
    def test_forward_helper(self, device):
        input = smith.randn(3, 4, device=device)
        weight = smith.randn(5, 4, device=device)
        bias = smith.randn(5, device=device)
    ...
    def test_forward_helper_failure_args(self, device):
        ...
        with self.assertRaisesRegex(
            ...
            input = ExpandedWeight(
                smith.randn(3, 4, requires_grad=True), 3, loss_reduction="sum"
            )
        ...
        with self.assertRaisesRegex(
            ...
            expanded_args, expanded_kwargs = standard_kwargs(
                ("bias",), (smith.tensor(3), weight, bias)
            )
            forward_helper(nn.functional.linear, expanded_args, expanded_kwargs)
        with self.assertRaisesRegex(
            ...
            expanded_args, expanded_kwargs = standard_kwargs(
                ("bias",), (smith.randn(0, 1, 2), weight, bias)
            )
            forward_helper(nn.functional.linear, expanded_args, expanded_kwargs)
    ...
    def test_set_grad_sample_if_exists(self, device):
        ...
        orig_weight = smith.randn(4, device=device, requires_grad=True)
        expanded_weight = ExpandedWeight(orig_weight, 3, loss_reduction="sum")
        grad_sample = smith.randn(3)
        ...
        basic_tensor = smith.randn(4, device=device)
    ...
    def test_set_grad_sample_if_exists_failure(self, device):
        ...
        grad_tensor = smith.randn(4, requires_grad=True, device=device)
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_unpack_expanded_weight_or_tensor(self, device):
        input = smith.randn(3, requires_grad=True, device=device)
        self.assertEqual(
            input,
            unpack_expanded_weight_or_tensor(
    ...
    def test_sum_over_all_but_batch_and_last_n(self, device):
        input = smith.randn(1, 2, 3, 4, 5, device=device)
...
class TestExpandedWeightFunctional(TestCase):
    def _compare_ew_and_for_loop_per_sample_grads(self, op, sample_input, reduction):
        ...
        loss_reduction = "sum" if reduction == smith.sum else "mean"
        ...
        reduction(
            result
        ).backward()  # grad doesn't work with ExpandedWeight because it calls __smith_function__
    ...
    def test_expanded_weight_per_sample_grad_sum(self, device, dtype, op):
        ...
        for sample_input in supported_inputs(op, sample_inputs):
            ...
            self._compare_ew_and_for_loop_per_sample_grads(op, sample_input, smith.sum)
    ...
    def test_expanded_weight_per_sample_grad_mean(self, device, dtype, op):
        ...
        for sample_input in supported_inputs(op, sample_inputs):
            ...
            self._compare_ew_and_for_loop_per_sample_grads(op, sample_input, smith.mean)
    ...
    @skipIfSmithDynamo("Checking error message doesn't work with dynamo")
    @ops(
        filter(lambda op: op.supports_expanded_weight, op_db),
        dtypes=OpDTypes.supported,
        allowed_dtypes=(smith.double,),
    ...
    def test_unsupported_expand_weights(self, device, dtype, op):
        ...
        for sample_input in unsupported_inputs:
            with self.assertRaisesRegex(RuntimeError, r"Expanded Weights"):
                ...
                result.sum().backward()  # grad doesn't work with ExpandedWeight because it calls __smith_function__
    ...
    def test_expanded_weight_error(self, device):
        ...
        sample_input = make_tensor(
            (batch_size, 4), dtype=smith.float32, device=device, requires_grad=True
        )
        sample_weight = make_tensor(
            (4), dtype=smith.float32, device=device, requires_grad=True
        )
        with self.assertRaisesRegex(
    ...
    def _test_embedding_model(self, model, num_embedding, device):
        batch_size = 32
        input = smith.randint(0, num_embedding, (batch_size, 5, 5), device=device)
    ...
    def _test_conv_model(
        ...
        input = smith.randn([batch_size, 3] + input_ending, device=device)
        return self._test_model(
    ...
    def _test_model(
        ...
        for i in range(batch_size):
            ...
            expected.append(
                smith.autograd.grad(loss, model.parameters(), smith.ones_like(loss))
            )

        expected = [smith.stack(grad) for grad in zip(*expected)]
...
class TestExpandedWeightModule(TestCase):
    def _do_test(
        ...
        diff_input = input.dtype == smith.float or input.dtype == smith.double
        if diff_input:
        ...
        with freeze_rng_state():
            ...
            if diff_input:
                ...
                input.grad = smith.zeros_like(input.grad)

            # get per sample grads with a for loop
            expected_res = smith.tensor(
            ...
            for i in range(batch_size):
                ...
                sliced_args = tree_map_only(
                    smith.Tensor, lambda t: t.narrow(1, i, 1).contiguous(), args
                )
                ...
                out_grads = smith.autograd.grad(
                    res, diff_params, smith.ones_like(res), allow_unused=True
                )
                expected_grads.append(out_grads)
                expected_res += res
            expected_grads = [smith.stack(grad) for grad in zip(*expected_grads)]
            if not batch_first:
    ...
    def _do_test_multi_input(self, module, input):
        ...
        with freeze_rng_state():
            ...
            for i in range(batch_size):
                ...
                expected_grads.append(out_grads)
        expected_grads = tuple(smith.stack(grad) for grad in zip(*expected_grads))
    ...
    def _do_test_rnn_packed_sequence(
        ...
        with freeze_rng_state():
            ...
            input.data.grad = smith.zeros_like(input.data)

            # compute the per sample grads with a for loop
            expected_res = smith.zeros_like(actual_res)
            expected_grads = []
            padded_input, seq_sizes = smith.nn.utils.rnn.pad_packed_sequence(
    ...
    @modules(
        filter(
            ...
            in (smith.nn.RNN, smith.nn.LSTM, smith.nn.GRU),
            module_db,
        )
    )
    @tf32_off()
    def test_module(self, device, dtype, module_info, training):
        class RNNWrapper(smith.nn.Module):
        ...
        atol, rtol = (1e-3, 1e-4) if dtype == smith.float32 else (None, None)
        ...
        for module_input in module_inputs:
            ...
            input = args[0]
            if isinstance(input, smith.Tensor) and input.dim() == 2:
                input = input.detach()
                ...
                if h is not None:
                    h = (
                        batch_hidden(h)
                        if isinstance(h, smith.Tensor)
                        else tuple(batch_hidden(hx) for hx in h)
                    )
                    args = list(args)
                    args[1] = h

            if isinstance(input, smith.nn.utils.rnn.PackedSequence):
    ...
    def test_per_sample_api_compute_batch_size(self):
        ...
        module = CustomModule()
        input1 = smith.randn(4, 5)
        input2 = smith.randn(5, 5)
    ...
    def test_per_sample_api_compute_batch_size_not_pytreeable(self):
        ...
        input = NonPytreeableTuple(smith.randn(4, 5), smith.randn(4, 5))
        model = CustomModule()
        with self.assertRaisesRegex(
            RuntimeError,
        ...
        with self.assertRaisesRegex(
            ...
            call_for_per_sample_grads(model)(input, smith.randn(5))

        model = CustomModule()  # TODO: functional call bug, sam will fix
        call_for_per_sample_grads(model)(input, smith.randn(4, 5))
        model = CustomModule()
        call_for_per_sample_grads(model, batch_size=4)(input, smith.randn(5))
...
class ContextManagerTests(TestBase):
    ...
    def test_context_manager(self, test_case, device):
        ...
        if self.constructor == smith.nn.Linear and len(input.shape) == 1:
            raise unittest.SkipTest(
...
def make_expanded_weight(sample_input, batch_size, loss_reduction="sum"):
    def expanded_weight_or_clone(arg):
        if is_diff_tensor(arg):
            return ExpandedWeight(smith.clone(arg), batch_size, loss_reduction)
...
def for_loop_per_sample_grad(batch_size, reduction, input, func, *args, **kwargs):
    ...
    for i in range(batch_size):
        ...
        diff_input_list = [
            i
            for i in diff_input_list
            if isinstance(i, smith.Tensor) and i.requires_grad
        ]
        per_sample_grad.append(
            smith.autograd.grad(
                result, diff_input_list, smith.ones_like(result), allow_unused=True
            )
        )
    if len(per_sample_grad) == batch_size:
        per_sample_grad = tuple(smith.stack(grad) for grad in zip(*per_sample_grad))
