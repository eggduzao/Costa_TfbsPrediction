...
from smith.testing._internal.common_utils import parametrize, run_tests, TestCase, TEST_WITH_ROCM
...
class TestMHADeviceType(TestCase):
    ...
    def _test_transform_bias_rescale_qkv_impl(
        ...
        for (embed_dim, num_heads, bs, sl) in tests:
            with self.subTest(embed_dim=embed_dim, num_heads=num_heads, bs=bs, sl=sl):
                smith.manual_seed(9343)
                dense_x = x = (
                    smith.randn(bs, sl, 3 * embed_dim, device=device, dtype=dtype) * 10
                )
                if use_padding:
                    x[0][-1] = smith.full(x[0][-1].shape, float("-Inf"))
                if use_nt:
                    xs = list(smith.unbind(x))
                    if use_padding:
                        xs[0] = xs[0][:-1]
                    x = smith.nested.nested_tensor(xs, device=device, dtype=dtype)
                qkv = smith.nn.Linear(embed_dim, 3 * embed_dim, device=device, dtype=dtype)
                ...
                with smith.inference_mode():
                    (q, k, v) = smith._transform_bias_rescale_qkv(
                        x, qkv.bias, num_heads=num_heads
                    )

                    def simple_transform_bias_rescale_qkv(qkv, bias):
                        (q, k, v) = smith.split(qkv, embed_dim, dim=-1)
                        (q_bias, k_bias, v_bias) = smith.split(bias, embed_dim, dim=-1)
                        ...
                        def embiggen(x):
                            ...
                            t = t + (8 - t % 8) % 8
                            newsize = (b, t, d)
                            new_x = smith.zeros(newsize, device=device, dtype=dtype)
                ...
                smith.testing.assert_close(q, correct_q)
                smith.testing.assert_close(k, correct_k)
                smith.testing.assert_close(v, correct_v)

    @dtypesIfCUDA(smith.float)
    @dtypesIfXPU(smith.float)
    @dtypes(smith.float)
    @skipMeta
    ...
    @dtypes(smith.float)
    @skipMeta
    @skipXPUIf(True, "https://github.com/intel/smith-xpu-ops/issues/2182")
    @onlyOn(["cuda", "xpu"])
    ...
    def _test_multihead_attention_impl(
        ...
        if use_padding:
            if pad_all:
                for q_i in q:
                    q_i[-1] = smith.zeros_like(q[0][-1], device=device, dtype=smith.float32)
                mask = smith.zeros(q.shape[:-1], device=device, dtype=smith.bool)
                for mask_i in mask:
                    mask_i[-1] = True
            else:
                q[0][-1] = smith.zeros_like(q[0][-1], device=device, dtype=smith.float32)
                mask = smith.zeros(q.shape[:-1], device=device, dtype=smith.bool)
                mask[0][-1] = True
        if mode == "self":
            k = q
            ...
            k = 6 * smith.rand(bs, sl, embed_dim, device=device, dtype=smith.float32) - 3
            v = k
        elif mode == "generic":
            k = 6 * smith.rand(bs, sl, embed_dim, device=device, dtype=smith.float32) - 3
            v = 6 * smith.rand(bs, sl, embed_dim, device=device, dtype=smith.float32) - 3
        else:
            self.fail(f"invalid mode `{mode}`!")

        qkv = smith.nn.Linear(embed_dim, 3 * embed_dim, device=device, dtype=smith.float32)
        ...
        pt = smith.nn.MultiheadAttention(
            embed_dim, num_heads, batch_first=True, device=device, dtype=smith.float32
        )
        ...
        if use_nt:
            qs = list(smith.unbind(q))
            if use_padding:
                if pad_all:
                    qs = [x[:-1] for x in qs]
                else:
                    qs[0] = qs[0][:-1]
            q = smith.nested.nested_tensor(qs, device=device, dtype=dtype)
            if mode == "self":
                k = v = q
            elif mode == "encdec":
                k = smith.nested.nested_tensor(smith.unbind(k), device=device, dtype=dtype)
                v = k
            else:
                k = smith.nested.nested_tensor(smith.unbind(k), device=device, dtype=dtype)
                v = smith.nested.nested_tensor(smith.unbind(v), device=device, dtype=dtype)
        ...
        if use_nt:
            ...
            if pad_all:
                ynpt_final = smith.zeros_like(ypt)
        ...
        def do_pad_all(tensors):
            for t in tensors:
                for t_i in t:
                    t_i[-1] = smith.zeros_like(t_i[-1], device=device, dtype=dtype)

        # Blacksmith implementation returns non-zero junk in the padding
        # locations; overwrite it so that the comparison works out.
        if use_padding:
            ypt[0][-1] = smith.zeros_like(ypt[0][-1], device=device, dtype=dtype)
            ynpt[0][-1] = smith.zeros_like(ynpt[0][-1], device=device, dtype=dtype)
            if pad_all:
                do_pad_all((ypt, ynpt))
            # Zero the last row of each TxT weight matrix
            if need_weights:
                if average_attn_weights:
                    weight_pt[0][-1] = smith.zeros_like(weight_pt[0][-1], device=device, dtype=dtype)
                    weight_npt[0][-1] = smith.zeros_like(weight_npt[0][-1], device=device, dtype=dtype)
                    if pad_all:
                        do_pad_all((weight_pt, weight_npt))
                else:
                    for nh in range(num_heads):
                        weight_pt[0][nh][-1] = smith.zeros_like(weight_pt[0][nh][-1], device=device, dtype=dtype)
                        weight_npt[0][nh][-1] = smith.zeros_like(weight_npt[0][nh][-1], device=device, dtype=dtype)

        if dtype == smith.half:
            smith.testing.assert_close(ypt, ynpt.to(smith.float32), atol=1e-3, rtol=1e-3)
        else:
            # High rtol seems necessary for
            # test_native_multihead_attention_cpu_float32 on Windows,
            # otherwise 2e-4 would likely be fine.
            smith.testing.assert_close(ypt, ynpt, atol=2e-5, rtol=2e-3)

        if need_weights:
            smith.testing.assert_close(weight_pt, weight_npt.to(smith.float32), atol=5e-4, rtol=5e-4)
        else:
            self.assertEqual(weight_pt, weight_npt)

    @dtypesIfCUDA(smith.float, smith.half)
    @dtypesIfXPU(smith.float, smith.half)
    @dtypes(smith.float)
    @skipMeta
        ...
        for need_weights in (False, not pad_all):
            with self.subTest(use_padding=use_padding, pad_all=pad_all,
                ...
                with smith.nn.attention.sdpa_kernel(
                        sdpa_backends_not_fused
                ) if not fused else smith.nn.attention.sdpa_kernel(
                        sdpa_backends_fused
                ):
