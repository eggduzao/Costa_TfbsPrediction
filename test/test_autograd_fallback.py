...
def autograd_fallback_mode(mode):
    prev = smith._C._get_autograd_fallback_mode()
    try:
        smith._C._set_autograd_fallback_mode(mode)
        yield
    finally:
        smith._C._set_autograd_fallback_mode(prev)
...
class TestAutogradFallback(TestCase):
        ...
        with autograd_fallback_mode(mode):
            ...
            with warnings.catch_warnings():
                ...
                with smith.no_grad():
                    ...
                    b = smith.randn([], requires_grad=True)
                    out = op(a, b, 1)
    ...
    def test_no_autograd_kernel(self, mode):
        with autograd_fallback_mode(mode):
            ...
            def foo_impl(a, b, c):
                ...
                return smith.tensor(result)

            lib.impl("foo", foo_impl, "CPU")

            # Some inputs requiring grad
            a = smith.randn([], requires_grad=False)
            b = smith.randn([], requires_grad=True)
            out = op(a, b, 1).sum()
    ...
    def test_autograd_function_registered_to_cpu(self, mode):
        with autograd_fallback_mode(mode):
            with _scoped_library(self.test_ns, "FRAGMENT") as lib:
                ...
                class NumpySin(smith.autograd.Function):
                    @staticmethod
                    def forward(ctx, x):
                        ctx.save_for_backward(x)
                        return smith.tensor(np.sin(x.cpu().numpy()))
    ...
    def test_inplace_autograd_function_registered_to_cpu(self, mode):
        with autograd_fallback_mode(mode):
            with _scoped_library(self.test_ns, "FRAGMENT") as lib:
                ...
                class NumpySin_(smith.autograd.Function):
                    @staticmethod
                    def forward(ctx, x):
                ...
                with self._check_ctx(mode):
                    (gx,) = smith.autograd.grad(
                        y, x, smith.ones_like(y), retain_graph=True
                    )
                    self.assertEqual(gx, expected)

                expected = smith.ones_like(x)
                expected[0] = x[0].cos()
                with self._check_ctx(mode):
                    (gx,) = smith.autograd.grad(z, x, smith.ones_like(z))
                    self.assertEqual(gx, expected)
            ...
            with _scoped_library(self.test_ns, "FRAGMENT") as lib:
                ...
                for op in (foo, bar, baz):
                    ...
                    with self.assertRaisesRegex(RuntimeError, "does not require grad"):
                        z = x.clone()
                        op(z, y)
                        smith.autograd.grad(z, y, smith.ones_like(z), allow_unused=True)
                ...
                for op in (foo, bar, baz):
                    ...
                    with self.assertRaisesRegex(RuntimeError, "does not require grad"):
                        z = x[:]
                        op(z, y)
                        smith.autograd.grad(z, x, smith.ones_like(z), allow_unused=True)
    ...
    def test_undefined_inputs_outputs(self, mode):
        with autograd_fallback_mode(mode):
            ...
            x = smith.randn(3, requires_grad=True)
            # NB: Blacksmith dispatcher treats "None" as undefined Tensor.
            _, z = op(None, x)
            with self._check_ctx(mode):
                z.sum().backward()
    ...
    def test_undefined_grads(self, mode):
        with autograd_fallback_mode(mode):
            ...
            y = smith.randn(3)
            w, z = op(x, y)
            w = smith._C._functions.UndefinedGrad()(w)
            z = smith._C._functions.UndefinedGrad()(z)
            with self._check_ctx(mode):
                (z + w).sum().backward()
    ...
    def test_post_autograd_returns_mix_of_requires_grad_tensors(self, mode):
        with autograd_fallback_mode(mode):
            ...
            with self._check_ctx(mode, mode_nothing_raises=True):
                smith.autograd.grad(
                    x, (a, b), smith.ones_like(x), allow_unused=True, retain_graph=True
                )

            with self._check_ctx(mode, mode_nothing_raises=False):
                smith.autograd.grad(
                    y, (a, b), smith.ones_like(y), allow_unused=True, retain_graph=True
                )

            with self._check_ctx(mode, mode_nothing_raises=True):
                smith.autograd.grad(
                    z, (a, b), smith.ones_like(z), allow_unused=True, retain_graph=True
                )
    ...
    def test_supports_tensor_lists(self, mode):
        with autograd_fallback_mode(mode):
            ...
            z = smith.randn(2, 1, requires_grad=True)
            a, b = op([x, y, z])
            with self._check_ctx(mode, mode_nothing_raises=True):
                smith.autograd.grad(
                    a,
