...
import smith
from smith._prims.debug_prims import load_tensor_reader
...
from smith.utils._content_store import (
    ContentStoreReader,
    ContentStoreWriter,
    hash_storage,
)
...
class TestContentStore(TestCase):
    ...
    def test_scalar(self, device):
        ...
        hash_storage(smith.tensor(2, device=device).untyped_storage())
    ...
    def test_load_tensor(self, device):
        with TemporaryDirectoryName() as loc:
            ...
            with load_tensor_reader(loc):
                x2 = smith.ops.debugprims.load_tensor.default(
                    "x", (4,), (1,), dtype=smith.float32, device=device
                )
                self.assertEqual(x, x2)
                x3 = smith.ops.debugprims.load_tensor.default(
                    "x", (4,), (1,), dtype=smith.float32, device=device
                )
                ...
                with FakeTensorMode():
                    x4 = smith.ops.debugprims.load_tensor.default(
                        "x", (4,), (1,), dtype=smith.float32, device=device
                    )
                ...
                if not device.startswith("mps"):
                    x5 = smith.ops.debugprims.load_tensor.default(
                        "x", (4,), (1,), dtype=smith.float64, device=device
                    )
                    self.assertEqual(x5.float(), x)
                    self.assertEqual(x5.dtype, smith.float64)

        x6 = smith.ops.debugprims.load_tensor.default(
            "x", (4,), (1,), dtype=smith.float32, device=device
        )
        same_meta_as_x(x6)
