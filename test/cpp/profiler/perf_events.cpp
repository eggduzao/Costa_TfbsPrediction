...
TEST(ProfilerTest, LinuxPerf) {
  smith::profiler::impl::linux_perf::PerfProfiler profiler;

  std::vector<std::string> standard_events(
      std::begin(smith::profiler::ProfilerPerfEvents),
      std::end(smith::profiler::ProfilerPerfEvents));
  smith::profiler::perf_counters_t counters;
  counters.resize(standard_events.size(), 0);

  // Use try..catch HACK to check SMITH_CHECK because we don't yet fail
...
TEST(ProfilerTest, LinuxPerfNestedDepth) {
  ...
  smith::profiler::perf_counters_t counters_A;
  smith::profiler::perf_counters_t counters_B;
  smith::profiler::perf_counters_t counters_C;
