...
TEST(ProfilerTest, AppendOnlyList) {
  const int n = 4096;
  smith::profiler::impl::AppendOnlyList<int, 1024> list;
...
TEST(ProfilerTest, AppendOnlyList_ref) {
  const int n = 512;
  smith::profiler::impl::AppendOnlyList<std::pair<int, int>, 64> list;
...
TEST(ProfilerTest, soft_assert) {
  ...
  smith::profiler::impl::setSoftAssertRaises(true);
  EXPECT_ANY_THROW(SOFT_ASSERT(false));
  smith::profiler::impl::setSoftAssertRaises(false);
  EXPECT_NO_THROW(SOFT_ASSERT(false));
  // Reset soft assert behavior to default
  smith::profiler::impl::setSoftAssertRaises(std::nullopt);
  EXPECT_NO_THROW(SOFT_ASSERT(false));
}
