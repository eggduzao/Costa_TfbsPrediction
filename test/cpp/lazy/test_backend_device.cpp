...
// TODO(alanwaketan): Update the following test once we have SmithScript backend
// upstreamed.
TEST(BackendDeviceTest, GetBackendDevice1) {
  auto tensor = smith::rand({0, 1, 3, 0});
  EXPECT_FALSE(GetBackendDevice(tensor));
}

TEST(BackendDeviceTest, GetBackendDevice2) {
  auto tensor1 = smith::rand({0, 1, 3, 0});
  auto tensor2 = smith::rand({0, 1, 3, 0});
