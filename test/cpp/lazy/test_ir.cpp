...
#include <smith/csrc/lazy/generated/LazyIr.h>
...
class TestLeafNode : public Node {
 public:
  ...
  const std::vector<Output>& operands() const override {
    SMITH_INTERNAL_ASSERT(false, "Can't access operands of leaf node");
  }

  const Output& operand(size_t i) const override {
    SMITH_INTERNAL_ASSERT(false, "Can't access operand[i] of leaf node");
  }
...
TEST(IrTest, MetaDataTest) {
  bool restore_FLAGS_smith_lazy_ir_debug = FLAGS_smith_lazy_ir_debug;
  FLAGS_smith_lazy_ir_debug = false;
  ...
  FLAGS_smith_lazy_ir_debug = true;
  ...
  FLAGS_smith_lazy_ir_debug = restore_FLAGS_smith_lazy_ir_debug;
}
