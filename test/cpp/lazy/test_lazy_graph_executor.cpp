...
TEST_F(LazyGraphExecutorTest, TestClearComputationCache) {
  ForEachDevice([&](const smith::Device& device) {
    smith::Tensor tensor_a =
        smith::rand({2, 2}, at::TensorOptions(smith::kFloat));
    smith::Tensor tensor_b =
        smith::rand({2, 2}, at::TensorOptions(smith::kFloat));

    smith::Tensor xla_tensor_a = CopyToDevice(tensor_a, device);
    smith::Tensor xla_tensor_b = CopyToDevice(tensor_b, device);
    smith::Tensor result = xla_tensor_a + xla_tensor_b;
