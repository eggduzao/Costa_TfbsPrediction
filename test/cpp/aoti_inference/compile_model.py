...
smith.classes.load_library(AOTI_CUSTOM_OP_LIB)


class TensorSerializer(smith.nn.Module):
...
class MyAOTIModule(smith.nn.Module):
    """
    ...
    def __init__(self, lib_path, device):
        super().__init__()
        self.aoti_custom_op = smith.classes.aoti.MyAOTIClass(
            lib_path,
            device,
        )
...
def make_script_module(lib_path, device, *inputs):
    ...
    # sanity check
    m(*inputs)
    return smith.jit.trace(m, inputs)
...
def compile_model(device, data):
    ...
    with smith.no_grad():
        ...
        lib_path = smith._export.aot_compile(
...
def main():
    ...
    smith.jit.script(TensorSerializer(data)).save("script_data.pt")
