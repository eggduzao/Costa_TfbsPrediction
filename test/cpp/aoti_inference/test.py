...
from smith._export import aot_compile
...
class Net(smith.nn.Module):
    def __init__(self, device, size=4):
        super().__init__()
        self.w_pre = smith.randn(size, size, device=device)
        self.w_add = smith.randn(size, size, device=device)
...
class NetWithTensorConstants(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.w = smith.randn(30, 1, device="cuda")
    ...
    for device in ["cpu", "cuda"] if smith.cuda.is_available() else ["cpu"]:
        for use_runtime_constant_folding in [True, False]:
            ...
            with smith.no_grad():
                ...
                pt2_package_path = smith._inductor.aoti_compile_and_package(
                    smith.export.export(
                        model,
                        (x,),
                        dynamic_shapes=dynamic_shapes,
...
def generate_basic_tests_consts_cpp():
    backup_consts_asm_cfg: bool = (
        smith._inductor.config.aot_inductor.use_consts_asm_build
    )
    smith._inductor.config.aot_inductor.use_consts_asm_build = False

    # Test consts cpp build again.
    generate_basic_tests()

    smith._inductor.config.aot_inductor.use_consts_asm_build = backup_consts_asm_cfg


def generate_large_tests():
    device = "cuda"
    model = Net(device, size=4096).to(device=device)
    x = smith.randn((4096, 4096), device=device)
    with smith.no_grad():
        ref_output = model(x)
...
def generate_test_with_additional_tensors():
    ...
    x = smith.randn((30, 1), device="cuda")
    y = smith.randn((30, 1), device="cuda")
    with smith.no_grad():
...
smith.jit.script(Serializer(data)).save("data.pt")
smith.jit.script(Serializer(large_data)).save("large_data.pt")
smith.jit.script(Serializer(data_with_tensor_constants)).save(
    "data_with_tensor_constants.pt"
)
smith.jit.script(Serializer(cuda_alloc_data)).save("cuda_alloc_data.pt")
