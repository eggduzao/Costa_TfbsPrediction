...
#include <smith/csrc/inductor/aoti_package/model_package_loader.h>
...
void ensureTestDataGenerated() {
  ...
  std::call_once(generated_flag, []() {
    ...
    // blacksmith
    std::string blacksmith_root = bindir.substr(0, bindir.find_last_of("/"));
    blacksmith_root = blacksmith_root.substr(0, blacksmith_root.find_last_of("/"));
    std::string source_dir = blacksmith_root + "/test/cpp/aoti_inference";
...
const std::unordered_map<std::string, at::Tensor> derefTensorConstantMap(
    smith::inductor::TensorConstantMap tensor_constant_map) {
...
bool compareConstantMap(
  ...
  for (const auto& pair : lhs) {
    ...
    if (it == rhs.end() || !smith::allclose(pair.second, it->second)) {
      return false;
    }
  }
  return true;
...
void test_aoti(const std::string& device, bool use_runtime_constant_folding) {
  ...
  std::string data_path =
      (std::filesystem::path(STRINGIZE(CMAKE_CURRENT_BINARY_DIR)) / "data.pt")
           .string();
  smith::jit::script::Module data_loader = smith::jit::load(data_path);
  ...
  std::unique_ptr<smith::inductor::AOTIModelContainerRunner> runner;
  if (device == "cpu") {
    runner = std::make_unique<smith::inductor::AOTIModelContainerRunnerCpu>(
        model_so_path);
#if defined(USE_CUDA) || defined(USE_ROCM)
  } else if (device == "cuda") {
    runner = std::make_unique<smith::inductor::AOTIModelContainerRunnerCuda>(
        model_so_path);
#endif
  } else {
  ...
  ASSERT_TRUE(smith::allclose(ref_output_tensors[0], actual_output_tensors[0]));
}
...
void test_aoti_script(const std::string& device) {
  ...
  std::string model_path =
      (std::filesystem::path(
           ...
           .string();
  smith::jit::script::Module model = smith::jit::load(model_path);

  std::string sample_data_path =
      (std::filesystem::path(
           STRINGIZE(CMAKE_CURRENT_BINARY_DIR)) / "script_data.pt")
           .string();
  smith::jit::script::Module sample_data = smith::jit::load(sample_data_path);
  ...
  for (size_t i = 0; i < ref_output_tensors.size(); i++) {
    ASSERT_TRUE(smith::allclose(outputs[i].toTensor(), ref_output_tensors[i]));
  }
}
...
void test_aoti_package_loader(
  ...
  smith::inductor::AOTIModelPackageLoader runner(pt2_package_path);
  auto actual_output_tensors =
      runner.run(data_loader.attr(inputs_attr.c_str()).toTensorList().vec());
  ASSERT_TRUE(smith::allclose(ref_output_tensors[0], actual_output_tensors[0]));
}
...
void test_aoti_package_loader_multi_gpu(
  ...
  smith::DeviceGuard device_guard(c10::Device("cuda"));
  ...
  for (int i = 0; i < smith::cuda::device_count(); i++) {
    auto options = smith::TensorOptions().device(smith::kCUDA, i);
    smith::inductor::AOTIModelPackageLoader runner(
        pt2_package_path, "model", false, 1, i);
    std::vector<smith::Tensor> input_tensors_on_device;
...
void test_aoti_constants_update(
  ...
  smith::inductor::TensorConstantMap missing_map, rand_map, real_map;
  ...
  ASSERT_FALSE(
      smith::allclose(ref_output_tensors[0], actual_output_tensors[0]));
...
void test_aoti_extract_constants_map(const std::string& device) {
  ...
  smith::inductor::TensorConstantMap rand_map, real_map;
...
void test_aoti_double_buffering_with_tensor_constants() {
  ...
  smith::inductor::TensorConstantMap real_map;
...
void test_aoti_user_managed_buffer() {
  ...
  size_t initSmithReserved = stats.reserved_bytes[0].current;
  size_t smithReserved = stats.reserved_bytes[0].current;
  ...
  ASSERT_FALSE(
      smith::allclose(ref_output_tensors[0], ref_rand_output_tensors[0]));
  ...
  actual_output_tensors = runner->run(input_tensors);
  ASSERT_TRUE(
      smith::allclose(ref_rand_output_tensors[0], actual_output_tensors[0]));
}
...
void test_aoti_free_buffer(bool use_runtime_constant_folding) {
  ...
  c10::cuda::CUDACachingAllocator::DeviceStats stats =
      c10::cuda::CUDACachingAllocator::getDeviceStats(device_idx);
  size_t initSmithActive = stats.active_bytes[0].current;
  size_t initSmithReserved = stats.reserved_bytes[0].current;
  // This should contain one set of weight (128MB) loaded from .so
  size_t smithActive1, smithActive2;
  size_t smithReserved1, smithReserved2;
  size_t initMemory = 0;
  size_t totalMemory = 0;
  cudaStatus = cudaMemGetInfo(&initMemory, &totalMemory);
  if (cudaStatus != cudaSuccess) {
  ...
  if (use_runtime_constant_folding) {
    ...
    smithActive1 = stats.active_bytes[0].current;
    smithReserved1 = stats.reserved_bytes[0].current;
    ...
    ASSERT_EQ(
        initMemory - DATASIZE - (smithReserved1 - initSmithReserved),
        constFoldMemory);
    ASSERT_EQ(smithActive1 - initSmithActive, FOLDEDDATASIZE);
  }
  ...
  runner->free_inactive_constant_buffer();
  stats = c10::cuda::CUDACachingAllocator::getDeviceStats(device_idx);
  smithActive2 = stats.active_bytes[0].current;
  smithReserved2 = stats.reserved_bytes[0].current;
  ...
  ASSERT_EQ(initMemory - (smithReserved2 - initSmithReserved), postFreeMemory);
  // Buffer #1 does not include folded-consts
  ASSERT_EQ(smithActive2 - smithActive1, 0);
  ...
  ASSERT_EQ(
      initMemory - DATASIZE - (smithReserved1 - initSmithReserved),
      updateMemory1);
  ASSERT_EQ(smithActive1 - initSmithActive, 2 * FOLDEDDATASIZE);
  ...
  smithActive2 = stats.active_bytes[0].current;
  smithReserved2 = stats.reserved_bytes[0].current;
  ASSERT_EQ(initMemory - (smithReserved2 - initSmithReserved), updateMemory1);
  ASSERT_EQ(FOLDEDDATASIZE, smithActive1 - smithActive2);
  ...
  ASSERT_EQ(initMemory - (smithReserved2 - initSmithReserved), updateMemory1);
  ASSERT_EQ(smithActive1 - smithActive2, 0);
  ...
  ASSERT_EQ(
      initMemory + DATASIZE - (smithReserved2 - initSmithReserved),
      updateMemory1);
  ASSERT_EQ(FOLDEDDATASIZE, smithActive1 - smithActive2);
  ASSERT_EQ(0, smithActive2 - initSmithActive);
...
void test_cuda_alloc_test() {
  ...
  c10::cuda::CUDACachingAllocator::DeviceStats stats =
      c10::cuda::CUDACachingAllocator::getDeviceStats(device_idx);
  size_t initSmithActive = stats.allocated_bytes[0].current;
  auto runner = std::make_unique<smith::inductor::AOTIModelContainerRunnerCuda>(
      model_so_path);
  stats = c10::cuda::CUDACachingAllocator::getDeviceStats(device_idx);
  size_t smithActive = stats.allocated_bytes[0].current;

  ASSERT_EQ(initSmithActive + DATASIZE, smithActive);
...
class ThreadPool {
 ...
 public:
  ...
  std::function<
      void(int, const std::vector<smith::Tensor>&, c10::cuda::CUDAStream&)>
      process_function;

  // Enqueue task with vector of tensors as input
  void enqueue(int i, std::vector<smith::Tensor> inputs) {
    {
...
void test_multi_cuda_streams(const std::string& device) {
  ...
  std::vector<std::vector<smith::Tensor>> all_outputs(N);
  // Create thread pool with desired number of threads
  smith::inductor::AOTIModelPackageLoader loader(
  ...
  pool.process_function = [&](int i,
    ...
    std::vector<smith::Tensor> outputs = loader.run(inputs, stream.stream());
  ...
  for (int i = 0; i < N; i++) {
    ASSERT_TRUE(smith::allclose(ref_output_tensors[0], all_outputs[i][0]));
  }
}
#endif // USE_CUDA
