...
TEST_F(DistAutogradTest, TestSendFunctionInvalidInputs) {
  ...
  auto in1 = smith::ones({3, 3}, options);
  auto in2 = smith::ones({3, 3}, options);
  ...
  std::vector<smith::Tensor> tensors = {in1, in2};
  ...
  // grad.
  send_function->setGrads({in1, smith::autograd::Variable()});
...
TEST_F(DistAutogradTest, TestInitializedContextCleanup) {
  ...
  auto y = smith::randn({2, 2}, smith::requires_grad());
  auto z = (x * x + y * y).sum();
...
TEST_F(DistAutogradTest, TestInitializedContextCleanupSendFunction) {
  ...
  auto options = at::TensorOptions().requires_grad(true);
  auto t = smith::ones({1}, options);
  auto tensors = std::vector<smith::Tensor>{t};
  addSendRpcBackward(
