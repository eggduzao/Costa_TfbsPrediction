...
using smith::distributed::autograd::DistAutogradContext;
...
class TestE2EBase : public ::testing::Test {
 protected:
  ...
  c10::intrusive_ptr<OwnerRRef> createRemoteRRef(
      at::Tensor t1,
      at::Tensor t2,
      std::shared_ptr<smith::jit::Operator> op) {
  ...
  void runTrainingLoop() {
    ...
    auto t1 = smith::ones({3, 3}, options);
    auto t2 = smith::ones({3, 3}, options);

    c10::OperatorName full_name("aten::add", "Tensor");
    auto matchedOp = smith::jit::findOperatorFor(full_name);
