...
#include <smith/csrc/distributed/rpc/tensorpipe_utils.h>
...
TEST(TensorpipeSerialize, Base) {
  // Sender serializes
  at::Tensor t1 = smith::ones({1024}, at::ScalarType::Int);
  at::Tensor t2 = smith::ones({1024}, at::ScalarType::Float);
  std::vector<at::Tensor> tensors{t1, t2};
  std::vector<char> payload = {'1', '2', '3'};
  std::vector<char> payloadCopy = payload; // for testing
  smith::distributed::rpc::MessageType mtype =
      smith::distributed::rpc::MessageType::UNKNOWN;
  int64_t mId = 100;
  auto sendingRpcMessage =
      c10::make_intrusive<smith::distributed::rpc::Message>(
          std::move(payload), std::move(tensors), mtype);
  sendingRpcMessage->setId(mId);
  auto [sendingTpMessage, sendingTpBuffers] =
      smith::distributed::rpc::tensorpipeSerialize(
  ...
  auto [recvingTpAllocation, recvingTpBuffers] =
      smith::distributed::rpc::tensorpipeAllocate(recvingTpDescriptor, {});
  ...
  // Mimic read() callback:
  // - Unpickle
  c10::intrusive_ptr<smith::distributed::rpc::Message> recvingRpcMessage =
      smith::distributed::rpc::tensorpipeDeserialize(
  ...
  EXPECT_TRUE(smith::equal(t1, recvingRpcMessage->tensors()[0]));
  EXPECT_TRUE(smith::equal(t2, recvingRpcMessage->tensors()[1]));
}
...
TEST(TensorpipeSerialize, RecopySparseTensors) {
  ...
  EXPECT_TRUE(smith::equal(main, tpBuffers.tensors[0]));
  EXPECT_TRUE(smith::equal(tiny, tpBuffers.tensors[1]));
  // Test cloned storage
  EXPECT_EQ(
      main.storage().data(),
...
TEST(TensorpipeSerialize, NoDeleterTensors) {
  ...
  at::Tensor t1 = smith::from_blob((float*)(blob1.data()), blob1.size());
  at::Tensor t2 = smith::from_blob((float*)(blob2.data()), blob2.size());
  std::vector<at::Tensor> tensors{t1, t2};
  std::vector<char> payload = {'1', '2', '3'};
  smith::distributed::rpc::MessageType mtype =
      smith::distributed::rpc::MessageType::UNKNOWN;
