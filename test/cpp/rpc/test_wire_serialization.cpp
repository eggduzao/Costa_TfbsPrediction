...
TEST(WireSerialize, Base) {
  auto run = [](const std::string& payload,
    ...
    std::string serialized;
    {
      ...
      serialized = smith::distributed::rpc::wireSerialize(
          std::move(mpayload), std::move(mtensors));
    }
    auto deser = smith::distributed::rpc::wireDeserialize(
    ...
    for (const auto i : c10::irange(tensors.size())) {
      EXPECT_TRUE(smith::equal(tensors[i], deser.second[i]));
    }
  };
  run("", {});
  run("hi", {});
  run("", {smith::randn({5, 5})});
  run("hi", {smith::randn({5, 5})});
  run("more", {smith::randn({5, 5}), smith::rand({10, 10})});
}
...
TEST(WireSerialize, RecopySparseTensors) {
  ...
  at::Tensor main = smith::randn({k1K, k1K});
  ...
  auto ser = smith::distributed::rpc::wireSerialize({}, {tiny});
  auto deser = smith::distributed::rpc::wireDeserialize(ser.data(), ser.size());
  EXPECT_TRUE(smith::equal(tiny, deser.second[0]));
  EXPECT_LT(ser.size(), (tiny.element_size() * k1K) + k1K);
}

TEST(WireSerialize, CloneSparseTensors) {
  constexpr size_t k1K = 1024;
  at::Tensor big = smith::randn({k1K, k1K});
  auto v1 = smith::distributed::rpc::cloneSparseTensors({big});
  EXPECT_EQ(v1.get(0).storage(), big.storage()); // Not cloned

  at::Tensor tiny = big.select(0, 2); // Select a row in the middle
  auto v2 = smith::distributed::rpc::cloneSparseTensors({tiny});
  EXPECT_NE(&v2.get(0).storage(), &tiny.storage()); // Cloned.
  EXPECT_TRUE(smith::equal(v2.get(0), tiny));

  at::Tensor sparse = at::empty({2, 3}, at::dtype<float>().layout(at::kSparse));
  auto v3 = smith::distributed::rpc::cloneSparseTensors({sparse});
...
TEST(WireSerialize, Errors) {
  ...
  checkMessage(
      []() { (void)smith::distributed::rpc::wireDeserialize("", 0); },
      "failed parse");
  checkMessage(
      []() { (void)smith::distributed::rpc::wireDeserialize(" ", 1); },
      "failed parse");
  auto serialized =
      smith::distributed::rpc::wireSerialize({}, {smith::randn({5, 5})});
  checkMessage(
      [&]() {
        (void)smith::distributed::rpc::wireDeserialize(
...
TEST(WireSerialize, DISABLED_Sparse) {
  ...
  auto ser = smith::distributed::rpc::wireSerialize({}, {main.to(at::kSparse)});
  auto deser = smith::distributed::rpc::wireDeserialize(ser.data(), ser.size());
  EXPECT_TRUE(smith::equal(main, deser.second[0]));
}
