...
#include <smith/csrc/autograd/generated/variable_factories.h> // @manual

#include <smith/nativert/executor/Executor.h> // @manual
#include <smith/nativert/executor/SerialGraphExecutor.h> // @manual
#include <smith/nativert/executor/Weights.h> // @manual
#include <smith/nativert/executor/memory/LayoutManager.h> // @manual
#include <smith/nativert/kernels/KernelFactory.h> // @manual
#include <smith/nativert/kernels/KernelHandlerRegistry.h> // @manual
#include <smith/nativert/kernels/KernelRegistry.h> // @manual

using namespace ::testing;

namespace smith::nativert /* must be same as namespace that includes TEST_FRIEND
                             declarations */
{
...
class LayoutPlannerCtorTests : public testing::Test {
 public:
  ...
  void createPlannerForModel(
      ...
      const std::unordered_map<std::string, smith::_export::TensorMeta>&
          tensorMeta = {}) {
  ...
  smith::_export::TensorMeta createSymbolicTensorMeta(
      const std::vector<int64_t>& dims,
      std::string device = "cpu",
      smith::_export::ScalarType dtype = smith::_export::ScalarType::FLOAT) {
    smith::_export::TensorMeta out_meta;

    smith::_export::Device d;
    d.set_type(std::move(device));
    out_meta.set_device(d);

    std::vector<smith::_export::SymInt> symvec;
    for (size_t i = 0; i < dims.size(); ++i) {
      smith::_export::SymInt symint;
      smith::_export::SymExpr symexpr;
    ...
    out_meta.set_layout(smith::_export::Layout::Strided);

    {
      smith::_export::SymInt i;
      i.set_as_int(0);
  ...
  smith::_export::TensorMeta createTensorMeta(
      const std::vector<int64_t>& dims,
      std::string device = "cpu",
      smith::_export::ScalarType dtype = smith::_export::ScalarType::FLOAT) {
    smith::_export::TensorMeta out_meta;
...
TEST_F(LayoutPlannerCtorTests, TestConstructOutputAlias) {
  auto model = R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
  return (%out_t))";
...
TEST_F(LayoutPlannerCtorTests, TestConstructNoOutVariant) {
  std::unordered_map<std::string, smith::_export::TensorMeta> meta = {
      {"out_t", createTensorMeta({10, 10, 10})}};

  auto model = R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %res = smith.ops.aten.clone.default(self=%out_t, memory_format=None)
  return (%res))";
...
TEST_F(LayoutPlannerCtorTests, TestConstructTensorList) {
  auto model = R"(
    graph(%y0, %y1):
      %out_t0 = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %out_t1 = smith.ops.aten.matmul.default(self=%y0, other=%y1)

      %l[] = prim.ListPack(l0=%out_t0, l1=%out_t1)
      %x0, %x1 = prim.ListUnpack(self=%l)

      %res0 = smith.ops.aten.clone.default(self=%x0, memory_format=None)
      %res1 = smith.ops.aten.clone.default(self=%x1, memory_format=None)
  return (%res0, %res1))";
...
%weight1_plannable = smith.ops.aten.clone.default(self=%weight1, memory_format=None)
%weights_list[] = prim.ListPack(l0=%weight1_plannable, l1=%weight2)
%weights_cat = smith.ops.aten.cat.default(tensors=%weights_list, dim=0)
return (%weights_cat)
)";
...
TEST_F(LayoutPlannerCtorTests, TestConstructReturnTensorList) {
  auto model = R"(
    graph(%y0, %y1):
      %y0_clone = smith.ops.aten.clone.default(self=%y0, memory_format=None)
      %y1_clone = smith.ops.aten.clone.default(self=%y1, memory_format=None)
...
TEST_F(LayoutPlannerCtorTests, TestConstruct) {
  ...
  c10::IValue tensor = c10::IValue(smith::rand({10, 10, 10}));
...
TEST_F(LayoutPlannerCtorTests, TestConstructPlanNoOverlap) {
  ...
  auto model = R"(
    graph(%y0, %y1):
      %out1_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %res1 = smith.ops.aten.clone.default(self=%out1_t, memory_format=None)
      %out2_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %res2 = smith.ops.aten.clone.default(self=%out2_t, memory_format=None)
  return (%res1, %res2))";
  createPlannerForModel(model, create_enabled_executor_config(), meta);
...
TEST_F(LayoutPlannerCtorTests, TestConstructPlanWithOverlap) {
  ...
  auto model = R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %out2_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %res = smith.ops.aten.clone.default(self=%out2_t, memory_format=None)
      %res1 = smith.ops.aten.clone.default(self=%out_t, memory_format=None)
  return (%res, %res1))";
  createPlannerForModel(model, create_enabled_executor_config(), meta);
...
TEST_F(LayoutPlannerCtorTests, TestConstructPlanWithMaybeAliasingToCopy) {
  auto model = R"(graph(%input):
          %i1 = smith.ops.aten._to_copy.default(self=%input, dtype=ScalarType::FLOAT, memory_format=None)
          %i2 = smith.ops.aten._to_copy.default(self=%input, dtype=ScalarType::FLOAT, memory_format=None)
          %out_t = smith.ops.aten.matmul.default(self=%i1, other=%i2)
          return (%out_t))";
