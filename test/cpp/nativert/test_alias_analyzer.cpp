...
TEST_F(AliasAnalyzerTests, TestSimpleAlias) {
  setModel(R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %res = smith.ops.aten.slice.Tensor(self=%out_t, dim=1, start=0, end=0, step=1)
  return (%res))");
...
TEST_F(AliasAnalyzerTests, TestDeepAlias) {
  setModel(R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %a1 = smith.ops.aten.slice.Tensor(self=%out_t, dim=1, start=0, end=0, step=1)
      %res = smith.ops.aten.slice.Tensor(self=%a1, dim=1, start=0, end=0, step=1)
  return (%res))");
...
TEST_F(AliasAnalyzerTests, TestAmbiguousSourceOfAlias) {
  setModel(R"(
    graph(%y0, %y1):
      %out_t = smith.ops.aten.matmul.default(self=%y0, other=%y1)
      %out_t2 = smith.ops.aten.matmul.default(self=%y0, other=%y1)
