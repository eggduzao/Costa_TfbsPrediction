...
TEST(ContainerAliasingTest, Conservative) {
  ...
  auto ops = smith::RegisterOperators(
      "custom::conservative", [](smith::List<at::Tensor> in) { return in; });
...
TEST(ContainerAliasingTest, MovesAcrossContainedWrites) {
  auto ops = smith::RegisterOperators().op(
      "uses::list",
      smith::RegisterOperators::options()
          .catchAllKernel(
              [](smith::List<at::Tensor> in) { return smith::rand({2, 3}); })
...
TEST(AliasRegistrationTest, ConservativeWithInferredSchema) {
  auto registry = smith::RegisterOperators().op(
      "foo::rand1",
      smith::RegisterOperators::options()
          .catchAllKernel(
...
TEST(AliasRegistrationTest, AliasMoveAtenListOp) {
  ...
  smith::jit::parseIR(graph_string, graph.get(), vmap);
  AliasDb aliasDb(graph);
...
TEST(NonDeterminismBackwardsCompatibility, BackwardsCompatibility) {
  ...
  for (const std::string& op : nondeterministic_ops) {
    const c10::FunctionSchema& schema = smith::jit::parseSchema(op);
