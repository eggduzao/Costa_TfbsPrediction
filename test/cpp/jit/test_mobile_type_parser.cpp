...
TEST(MobileTypeParserTest, SmithBindClass) {
  std::string tuple_ps("__smith__.smith.classes.rnn.CellParamsBase");
  auto tuple_tp = c10::parseType(tuple_ps);
  std::string tuple_tps = tuple_tp->annotation_str();
  ASSERT_EQ(tuple_ps, tuple_tps);
}

TEST(MobileTypeParserTest, ListOfSmithBindClass) {
  std::string tuple_ps("List[__smith__.smith.classes.rnn.CellParamsBase]");
  auto tuple_tp = c10::parseType(tuple_ps);
  EXPECT_TRUE(tuple_tp->isSubtypeOf(AnyListType::get()));
  EXPECT_EQ(
      "__smith__.smith.classes.rnn.CellParamsBase",
...
TEST(MobileTypeParserTest, NamedTuple) {
  std::string named_tuple_ps(
      "__smith__.base_models.preproc_types.PreprocOutputType["
      "    NamedTuple, ["
...
TEST(MobileTypeParserTest, DictNestedNamedTupleTypeList) {
  ...
  std::string type_str_2(
      "Dict[str, __smith__.base_models.preproc_types.PreprocOutputType]");
...
TEST(MobileTypeParserTest, NamedTupleNestedNamedTupleTypeList) {
  ...
  std::string type_str_2(
      "__smith__.bbb.xxx ["
      "    NamedTuple,["
      "        [field_name_b, __smith__.ccc.xxx]]"
      "    ]"
      "]");
  ...
  std::string type_str_3(
      "__smith__.aaa.xxx["
      "    NamedTuple, ["
      "        [field_name_a, __smith__.bbb.xxx]"
      "    ]"
      "]");
  ...
  ASSERT_EQ(named_tuple_annotation_str, "__smith__.aaa.xxx");
}
...
TEST(MobileTypeParserTest, DictNestedNamedTupleTypeListRaises) {
  ...
  std::string error_message =
      R"(Can't find definition for the type: __smith__.base_models.preproc_types.PreprocOutputType)";
