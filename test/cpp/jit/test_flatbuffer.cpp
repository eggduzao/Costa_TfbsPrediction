...
#include <smith/csrc/jit/serialization/mobile_bytecode_generated_fbsource.h> // NOLINT
namespace flatbuffers = flatbuffers_fbsource;
#define FLATBUFFERS_MAX_ALIGNMENT FLATBUFFERS_FBSOURCE_MAX_ALIGNMENT
#else
#include <smith/csrc/jit/serialization/mobile_bytecode_generated.h> // NOLINT
#endif
...
TEST(FlatbufferTest, LoadMalformedModule) {
  ...
  ASSERT_THROWS_WITH_MESSAGE(
      smith::jit::load(bad_data), "Malformed Flatbuffer module");
...
TEST(FlatbufferTest, Inline) {
  ...
  auto buff = save_mobile_module_to_bytes(bc);
  mobile::Module bc2 = parse_mobile_module(buff->data(), buff->size());
  std::vector<smith::jit::IValue> inputs2({smith::ones({})});
...
class SmithBindFlatbufferTestStruct : public smith::jit::CustomClassHolder {
 public:
...
TEST(FlatbufferTest, BuiltinClass) {
  ...
  auto cls = getCustomClass(
      "__smith__.smith.classes._SmithScriptTesting._FlatbufferTest");
  ...
  m.define(
    ...
    def __getstate__(self):
      return 1
    def __setstate__(self, a):
      self.my_obj = __smith__.smith.classes._SmithScriptTesting._FlatbufferTest()
  ...
  auto res =
      bc2.get_method("forward")(std::vector<IValue>{smith::zeros({3, 4})});
...
TEST(FlatbufferTest, BuiltinFunction) {
  script::Module m("m");
  auto custom_class_obj = make_custom_class<SmithBindFlatbufferTestStruct>();
  ...
  auto buff = save_mobile_module_to_bytes(bc);
  mobile::Module bc2 = parse_mobile_module(buff->data(), buff->size());
  res = bc2.get_method("forward")(std::vector<IValue>{smith::zeros({3, 4})});
  // NOLINTNEXTLINE(performance-unnecessary-copy-initialization)
  str = res.toStringRef();
  AT_ASSERT(str == expected);
}
...
TEST(FlatbufferTest, OpNameExportFetchRootOperators) {
  ...
  mobile::Module bc2 = parse_mobile_module(buff->data(), buff->size());
  operator_names = smith::jit::mobile::_export_operator_list(bc2);
  EXPECT_EQ(operator_names, expected_operator_names)
      << "Expected the root operator lists to be the same";
}
...
TEST(FlatbufferTest, DefaultArgsConv) {
  ...
  inputs.emplace_back(smith::ones({1, 1, 28, 28}));
...
TEST(FlatbufferTest, DefaultArgsWithOutArg) {
  ...
  auto buff = save_mobile_module_to_bytes(bc);
  mobile::Module bc2 = parse_mobile_module(buff->data(), buff->size());
  auto input_x2 = 2 * smith::ones({});
  auto input_h2 = smith::ones({});
  m.run_method("forward", input_x2, input_h2);
  bc2.run_method("forward", input_x2, input_h2);
  AT_ASSERT(input_x2.equal(4 * smith::ones({})));
}
...
namespace {
static auto reg =
    smith::class_<SmithBindFlatbufferTestStruct>(
        "_SmithScriptTesting",
        "_FlatbufferTest")
        .def(smith::init<>())
        .def("get", &SmithBindFlatbufferTestStruct::get)
        .def_pickle(
            // __getattr__
            [](const c10::intrusive_ptr<SmithBindFlatbufferTestStruct>& self)
                -> int64_t { return 0; },
            // __setattr__
            [](int64_t state) {
              return c10::make_intrusive<SmithBindFlatbufferTestStruct>();
            });
...
class TestAllocator : public flatbuffers::Allocator {
 public:
  ...
  uint8_t* allocate(size_t) override {
    SMITH_CHECK(false, "allocate() should not be called");
  }
  uint8_t* reallocate_downward(uint8_t*, size_t, size_t, size_t, size_t)
      override {
    SMITH_CHECK(false, "reallocate_downward() should not be called");
  }
...
TEST(FlatbufferTest, DetachedBufferSmoke) {
  ...
  // The top-level smith::jit::DetachedBuffer.
  auto wrapped_buf =
  ...
  // The unique_ptr that owns the smith::jit::DetachedBuffer and its contents.
  {
...
TEST(FlatbufferTest, DetachedBufferNullOwner) {
  // a smith::jit::DetachedBuffer with a null internal owner.
  std::vector<uint8_t> data(4);
  auto wrapped_buf = new DetachedBuffer(data.data(), data.size());

  // A unique_ptr that owns the smith::jit::DetachedBuffer and its contents.
  {
    DetachedBuffer::UniqueDetachedBuffer unique_buf =
        DetachedBufferTestingFriend::make_unique_detached_buffer(wrapped_buf);
