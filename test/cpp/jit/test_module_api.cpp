...
TEST(ModuleAPITest, CloneWithModuleInterface) {
  ...
  parentMod.register_attribute(
      "subMod1",
      cu->get_interface("__smith__.OneInterface"),
      subMod1._ivalue());
  parentMod.register_attribute(
      "subMod2",
      cu->get_interface("__smith__.OneInterface"),
...
TEST(ModuleAPITest, Define) {
  ...
  auto result = m.run_method("add_it", smith::ones({}));
...
TEST(ModuleAPITest, Freezing) {
  ...
  auto frozen_mod = smith::jit::freeze(m);
  ...
  auto frozen_mod2 = smith::jit::optimize_for_inference(m);
...
TEST(ModuleAPITest, OfiFreezesTraining) {
  ...
  // attribute is set
  auto frozen_mod = smith::jit::optimize_for_inference(m);
  forward_g = frozen_mod.get_method("forward").graph();
  testing::FileCheck().check_not("GetAttr")->run(*forward_g);
}
...
TEST(ModuleAPITest, OfiFreezesNoForward) {
  ...
  auto frozen_mod =
      smith::jit::optimize_for_inference(m, std::vector<std::string>{"bar"});
  ASSERT_EQ(
      m.run_method("bar", smith::ones({})).toTensor().item<float>(),
      frozen_mod.run_method("bar", smith::ones({})).toTensor().item<float>());
}
...
TEST(ModuleAPITest, To_CUDA) {
  Module m("test");
  {
    ...
    m.register_parameter("foo", smith::ones({}, at::kCUDA), false);
    m.register_buffer("bar", smith::ones({}, at::kCUDA));
  ...
  }
  {
    ...
    m.register_parameter("foo", smith::ones({}), false);
    m.register_buffer("bar", smith::ones({}));
