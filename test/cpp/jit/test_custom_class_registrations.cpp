...
struct DefaultArgs : smith::CustomClassHolder {
  int x;
...
struct Foo : smith::CustomClassHolder {
  int x, y;
  Foo() : x(0), y(0) {}
...
struct _StaticMethod : smith::CustomClassHolder {
...
struct FooGetterSetter : smith::CustomClassHolder {
...
struct FooGetterSetterLambda : smith::CustomClassHolder {
  int64_t x;
  FooGetterSetterLambda() : x(0) {}
  FooGetterSetterLambda(int64_t x_) : x(x_) {}
};

struct FooReadWrite : smith::CustomClassHolder {
  int64_t x;
  const int64_t y;
...
struct LambdaInit : smith::CustomClassHolder {
  int x, y;
...
struct NoInit : smith::CustomClassHolder {
  int64_t x;
};

struct PickleTester : smith::CustomClassHolder {
  PickleTester(std::vector<int64_t> vals) : vals(std::move(vals)) {}
  std::vector<int64_t> vals;
};

// Thread-safe Tensor Queue
struct TensorQueue : smith::CustomClassHolder {
  ...
  static c10::intrusive_ptr<TensorQueue> deserialize(
    ...
    SMITH_CHECK(std::tuple_size<decltype(flattened)>::value == 2);

    auto init_tensor_tuple = std::get<0>(flattened);
    SMITH_CHECK(std::tuple_size<decltype(init_tensor_tuple)>::value == 2);
    SMITH_CHECK(std::get<0>(init_tensor_tuple) == std::string("init_tensor"));
    ...
    SMITH_CHECK(std::tuple_size<decltype(queue_tuple)>::value == 2);
    SMITH_CHECK(std::get<0>(queue_tuple) == std::string("queue"));
...
struct ConstantTensorContainer : smith::CustomClassHolder {
...
at::Tensor take_an_instance(const c10::intrusive_ptr<PickleTester>& instance) {
  return smith::zeros({instance->vals.back(), 4});
}

struct ElementwiseInterpreter : smith::CustomClassHolder {
  ...
  // https://blacksmith.org/tutorials/advanced/smith_script_custom_classes.html#defining-serialization-deserialization-methods-for-custom-c-classes
  // for more info.
...
struct ReLUClass : public smith::CustomClassHolder {
  at::Tensor run(const at::Tensor& t) {
    return t.relu();
  }
};

struct FlattenWithTensorOp : public smith::CustomClassHolder {
...
struct ContainsTensor : public smith::CustomClassHolder {
...
SMITH_LIBRARY(_SmithScriptTesting, m) {
  m.impl_abstract_pystub("smith.testing._internal.smithbind_impls");
  m.class_<ScalarTypeClass>("_ScalarTypeClass")
      .def(smith::init<at::ScalarType>())
      .def_pickle(
  ...
  m.class_<DefaultArgs>("_DefaultArgs")
      .def(smith::init<int64_t>(), "", {smith::arg("start") = 3})
      .def("increment", &DefaultArgs::increment, "", {smith::arg("val") = 1})
      .def("decrement", &DefaultArgs::decrement, "", {smith::arg("val") = 1})
      .def(
          "scale_add",
          &DefaultArgs::scale_add,
          "",
          {smith::arg("add"), smith::arg("scale") = 1})
      .def(
          "divide",
          &DefaultArgs::divide,
          "",
          {smith::arg("factor") = smith::arg::none()});

  m.class_<Foo>("_Foo")
      .def(smith::init<int64_t, int64_t>())
      // .def(smith::init<>())
      .def("info", &Foo::info)
  ...
  m.def(
      "takes_foo(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> Tensor");
  m.def(
      "takes_foo_python_meta(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> Tensor");
  m.def(
      "takes_foo_list_return(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> Tensor[]");
  m.def(
      "takes_foo_tuple_return(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> (Tensor, Tensor)");
  m.def(
      "takes_foo_tensor_return(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> Tensor");
  ...
  m.class_<LambdaInit>("_LambdaInit")
      .def(smith::init([](int64_t x, int64_t y, bool swap) {
        if (swap) {
  ...
  m.class_<MyStackClass<std::string>>("_StackString")
      .def(smith::init<std::vector<std::string>>())
  ...
  m.class_<PickleTester>("_PickleTester")
      .def(smith::init<std::vector<int64_t>>())
      .def_pickle(
  ...
  m.def(
      "take_an_instance(__smith__.smith.classes._SmithScriptTesting._PickleTester x) -> Tensor Y",
      take_an_instance);
...
SMITH_LIBRARY_FRAGMENT(_SmithScriptTesting, m) {
  m.impl_abstract_pystub("smith.testing._internal.smithbind_impls");
  m.def(
      "takes_foo_cia(__smith__.smith.classes._SmithScriptTesting._Foo foo, Tensor x) -> Tensor");
  m.def(
      "queue_pop(__smith__.smith.classes._SmithScriptTesting._TensorQueue foo) -> Tensor");
  m.def(
      "queue_push(__smith__.smith.classes._SmithScriptTesting._TensorQueue foo, Tensor x) -> ()");
  m.def(
      "queue_size(__smith__.smith.classes._SmithScriptTesting._TensorQueue foo) -> int");
}

SMITH_LIBRARY_IMPL(_SmithScriptTesting, CPU, m) {
...
SMITH_LIBRARY_IMPL(_SmithScriptTesting, CUDA, m) {
  m.impl("queue_push", queue_push);
  m.impl("queue_pop", queue_pop);
}

SMITH_LIBRARY_IMPL(_SmithScriptTesting, Meta, m) {
  m.impl("takes_foo", &takes_foo);
  m.impl("takes_foo_list_return", takes_foo_list_return);
  m.impl("takes_foo_tuple_return", takes_foo_tuple_return);
}

SMITH_LIBRARY_IMPL(_SmithScriptTesting, CompositeImplicitAutograd, m) {
  m.impl("takes_foo_cia", takes_foo);
}

// Need to implement BackendSelect because these two operators don't have tensor
// inputs.
SMITH_LIBRARY_IMPL(_SmithScriptTesting, BackendSelect, m) {
  m.impl("queue_pop", queue_pop);
  m.impl("queue_size", queue_size);
}
