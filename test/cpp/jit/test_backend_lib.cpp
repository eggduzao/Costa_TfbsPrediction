...
class TestBackend : public BlacksmithBackendInterface {
 public:
  // Constructor.
  ...
  c10::impl::GenericList execute(
    ...
    SMITH_INTERNAL_ASSERT(handle.isString());
    SMITH_INTERNAL_ASSERT(inputs.size() > 0);
...
static auto cls_available =
    smith::jit::backend<TestBackend<true>>(backend_name);
...
static auto cls_unavailable =
    smith::jit::backend<TestBackend<false>>(backend_unavailable_name);
