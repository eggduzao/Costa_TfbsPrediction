...
#include <smith/csrc/jit/mobile/import_data.h>
#include <smith/csrc/jit/mobile/module.h>
#include <smith/csrc/jit/mobile/train/export_data.h>
#include <smith/csrc/jit/mobile/train/optim/sgd.h>
#include <smith/csrc/jit/mobile/train/random.h>
#include <smith/csrc/jit/mobile/train/sequential.h>
...
TEST(LiteTrainerTest, Params) {
  Module m("m");
  m.register_parameter("foo", smith::ones({1}, at::requires_grad()), false);
  m.define(R"(
    def forward(self, x):
      b = 1.0
      return self.foo * x + b
  ...
  ::smith::optim::SGD optimizer(
      parameters, ::smith::optim::SGDOptions(learning_rate).momentum(momentum));
  for (int epoc = 0; epoc < n_epoc; ++epoc) {
    for (auto& data : trainData) {
      ...
      auto loss = ::smith::l1_loss(output, targets);
      loss.backward();
      optimizer.step();
    }
  }
  ...
  ::smith::optim::SGD bc_optimizer(
      bc_parameters,
      ::smith::optim::SGDOptions(learning_rate).momentum(momentum));
  for (int epoc = 0; epoc < n_epoc; ++epoc) {
    for (auto& data : trainData) {
      auto source = data.first, targets = data.second;
...
TEST(MobileTest, NamedParameters) {
  ...
  Module child("m2");
  child.register_parameter("foo", 4 * smith::ones({}), false);
  child.register_parameter("bar", 4 * smith::ones({}), false);
...
TEST(MobileTest, SaveLoadParameters) {
  ...
  Module child("m2");
  child.register_parameter("foo", 4 * smith::ones({}), false);
  child.register_parameter("bar", 3 * smith::ones({}), false);
  m.register_module("child1", child);
  m.register_module("child2", child.clone());
  auto full_params = m.named_parameters();
  std::stringstream ss;
...
TEST(MobileTest, SaveLoadParametersUsingFlatbuffers) {
  ...
  input_params["four_by_ones"] = 4 * smith::ones({});
  input_params["three_by_ones"] = 3 * smith::ones({});
  ...
  {
    auto four_by_ones = 4 * smith::ones({});
    EXPECT_EQ(
        output_params["four_by_ones"].item<int>(), four_by_ones.item<int>());
  }
  {
    auto three_by_ones = 3 * smith::ones({});
    EXPECT_EQ(
...
TEST(LiteTrainerTest, SGD) {
  ...
  // Reference: Full jit and smith::optim::SGD
  std::stringstream ms;
  m.save(ms);
  auto mm = load(ms);
  std::vector<::at::Tensor> parameters;
  ...
  // Test: lite interpreter and smith::jit::mobile::SGD
  std::stringstream ss;
  m._save_for_mobile(ss);
  mobile::Module bc = _load_for_mobile(ss);
  std::vector<::at::Tensor> bc_parameters = bc.parameters();
  ::smith::jit::mobile::SGD bc_optimizer(
      bc_parameters,
      ::smith::jit::mobile::SGDOptions(learning_rate).momentum(momentum));
  for (int epoc = 0; epoc < n_epoc; ++epoc) {
    for (auto& data : trainData) {
      auto source = data.first, targets = data.second;
      bc_optimizer.zero_grad();
...
namespace {
struct DummyDataset : smith::data::datasets::Dataset<DummyDataset, int> {
...
TEST(LiteTrainerTest, SequentialSampler) {
  ...
  auto data_loader = smith::data::make_data_loader<mobile::SequentialSampler>(
