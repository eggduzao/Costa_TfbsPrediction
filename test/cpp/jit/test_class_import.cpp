...
class FooNestedTest2:
    def __init__(self, y):
        self.y = y
        self.nested = __smith__.FooNestedTest(y)

class FooTest:
    def __init__(self, x):
        self.class_attr = __smith__.FooNestedTest(x)
        self.class_attr2 = __smith__.FooNestedTest2(x)
...
static constexpr std::string_view smithbindSrc = R"JIT(
class FooBar1234(Module):
  __parameters__ = []
  f : __smith__.smith.classes._SmithScriptTesting._StackString
  training : bool
  def forward(self: __smith__.FooBar1234) -> str:
    return (self.f).top()
)JIT";
...
TEST(ClassImportTest, CustomClass) {
  ...
  import_libs(
      cu1,
      "__smith__.FooBar1234",
      std::make_shared<Source>(smithbindSrc),
      constantTable);
}
