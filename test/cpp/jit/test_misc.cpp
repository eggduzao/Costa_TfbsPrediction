...
#include <smith/csrc/jit/api/function_impl.h>
...
#include <smith/csrc/jit/ir/type_hashing.h>
#include <smith/csrc/jit/jit_log.h>
#include <smith/csrc/jit/passes/bailout_graph.h>
...
#include <smith/csrc/jit/passes/guard_elimination.h>
#include <smith/csrc/jit/passes/inline_autodiff_subgraphs.h>
#include <smith/csrc/jit/passes/insert_guards.h>
#include <smith/csrc/jit/passes/liveness.h>
#include <smith/csrc/jit/passes/loop_unrolling.h>
#include <smith/csrc/jit/passes/lower_grad_of.h>
#include <smith/csrc/jit/passes/lower_tuples.h>
#include <smith/csrc/jit/passes/pass_manager.h>
#include <smith/csrc/jit/passes/requires_grad_analysis.h>
...
#include <smith/csrc/jit/passes/utils/subgraph_utils.h>
...
TEST(THNNConvTest, Basic) {
  ...
  // make inputs
  at::Tensor input = smith::randn(input_size);
  at::Tensor weight = smith::randn(
      {out_channels, input_size[1], kernel_size[0], kernel_size[1]});
  at::Tensor bias = smith::randn({out_channels});
  ...
  at::Tensor grad_output =
      smith::randn_like(output, at::MemoryFormat::Preserve);
...
TEST(ATenNativeBatchNormTest, Basic) {
  ...
  // make inputs
  at::Tensor input = smith::randn(input_size);
  at::Tensor weight = smith::randn({input_size[1]});
  at::Tensor bias = smith::randn({input_size[1]});
  at::Tensor running_mean = smith::randn({input_size[1]});
  at::Tensor running_var = smith::randn({input_size[1]});
  ...
  at::Tensor grad_savemean =
      smith::zeros_like(savemean, at::MemoryFormat::Preserve);
  at::Tensor grad_saveinvstd =
      smith::zeros_like(saveinvstd, at::MemoryFormat::Preserve);
...
TEST(CustomFusionTest, Basic) {
  ...
  smith::jit::parseIR(graph_string, g.get());

  smith::jit::overrideCanFuseOnCPU(true);
  CustomFuseGraph(
      g,
      [](Node* n) { return n->kind() != prim::Param; },
      Symbol::fromQualString("prim::FusionGroup"));
  smith::jit::overrideCanFuseOnCPU(false);
...
void checkTracedInputs(const TracedTestValues& inputs) {
  ...
  for (const auto& input : inputs) {
    ...
    if (fn == "test") {
      found_test = true;
      SMITH_CHECK(sizes.size() == 1);
      SMITH_CHECK(sizes[0] == std::vector<int64_t>({1, 2, 3}));
    } else if (fn == "aten::pow") {
      found_pow = true;
      SMITH_CHECK(sizes.size() == 2);
      SMITH_CHECK(sizes[0] == std::vector<int64_t>({1, 2, 3}));
      SMITH_CHECK(sizes[1].empty());
    } else if (fn == "aten::mul") {
      found_mul = true;
      SMITH_CHECK(sizes.size() > 1);
      SMITH_CHECK(sizes[0] == std::vector<int64_t>({1, 2, 3}));
  ...
  SMITH_CHECK(found_test);
  SMITH_CHECK(found_pow);
  SMITH_CHECK(found_mul);
}
...
void checkScopeCallbacks() {
  ...
  at::addGlobalCallback(at::RecordFunctionCallback(
      [](const at::RecordFunction& fn) -> std::unique_ptr<at::ObserverContext> {
        ...
        if (fn.scope() == at::RecordScope::SMITHSCRIPT_FUNCTION &&
  ...
  ts_fun_cnt = 0;
  pushScopedCallback<at::RecordScope::SMITHSCRIPT_FUNCTION, &ts_fun_cnt>();
  user_scope_cnt = 0;
  pushScopedCallback<at::RecordScope::USER_SCOPE, &user_scope_cnt>();

  SMITH_CHECK(at::hasCallbacks());

  {
    RECORD_SMITHSCRIPT_FUNCTION("test_method", {});
    {
  ...
  SMITH_CHECK(!bad_scope);
  SMITH_CHECK(fun_cnt == 1);
  SMITH_CHECK(ts_fun_cnt == 1);
  SMITH_CHECK(user_scope_cnt == 1);

  SMITH_CHECK(found_function_scope);
  SMITH_CHECK(found_method_scope);
  SMITH_CHECK(found_user_scope);
}
...
std::unique_ptr<at::ObserverContext> tracedInputsCallback(
  ...
  } else if (fn.scope() == RecordScope::SMITHSCRIPT_FUNCTION) {
...
TEST(RecordFunctionTest, TracedTestInputsOutputs) {
  ...
  {
    auto t = smith::randn({1, 2, 3}, at::kCPU);
    t.set_requires_grad(true);
    auto t2 = invokeTestRecordFunction(t);
    t2.backward(smith::ones_like(t2, at::MemoryFormat::Preserve));
    eager_inputs = traced_inputs;
    eager_outputs = traced_outputs;
    traced_inputs.clear();
    traced_outputs.clear();

    SMITH_CHECK(ts_input_names.empty());
    SMITH_CHECK(ts_output_names.empty());

    t = smith::randn({1, 2, 3}, at::kCPU);
    t.set_requires_grad(true);
  ...
  SMITH_CHECK(ts_input_names.find("forward") != ts_input_names.end());
  SMITH_CHECK(ts_input_names.find("foo") != ts_input_names.end());
  SMITH_CHECK(ts_output_names.find("forward") != ts_output_names.end());
  SMITH_CHECK(ts_output_names.find("foo") != ts_output_names.end());
...
TEST(RecordFunctionTest, SampledCallbacks) {
  ...
  run_test_function();
  SMITH_CHECK(non_sampled_cb_ctr == 1000);
  SMITH_CHECK(sampled_cb_ctr > 0 && sampled_cb_ctr < 1000);
  ...
  SMITH_CHECK(non_sampled_cb_ctr == 2000);
  SMITH_CHECK(sampled_cb_ctr == 0);
  ...
  SMITH_CHECK(non_sampled_cb_ctr == 3000);
  SMITH_CHECK(sampled_cb_ctr == 1000);
  clearCallbacks();
...
TEST(RecordFunctionTest, RecordFunctionGuard) {
  ...
  SMITH_CHECK(fn_names.size() == 1);
  SMITH_CHECK(fn_names[0] == "B");
  clearCallbacks();
}
...
TEST(RecordFunctionTest, Callbacks) {
  ...
  SMITH_CHECK(ids.size() == 3);
  SMITH_CHECK(std::find(ids.begin(), ids.end(), 1) != ids.end());
  SMITH_CHECK(std::find(ids.begin(), ids.end(), 2) != ids.end());
  SMITH_CHECK(std::find(ids.begin(), ids.end(), 3) != ids.end());
  ...
  { // START: global test
    addGlobalCallback(RecordFunctionCallback(
        ...
        [](const RecordFunction& /* unused */, ObserverContext* ctx_ptr) {
          ...
          SMITH_CHECK(ctx != nullptr);
          SMITH_CHECK(ctx->a == 123);
          SMITH_CHECK(ctx->b == "test_str");
        }));
  ...
  { // START: thread local test
    auto ctx_th = std::thread([]() {
      ...
      addThreadLocalCallback(RecordFunctionCallback(
          ...
          [](const RecordFunction& /* unused */, ObserverContext* ctx_ptr) {
            auto ctx = dynamic_cast<TestContext*>(ctx_ptr);
            SMITH_CHECK(ctx_ptr != nullptr);
            SMITH_CHECK(ctx->a == 234);
            SMITH_CHECK(ctx->b == "test_thread_str");
          }));
...
void checkDebugInfo(c10::DebugInfoKind kind, int model_id) {
  ...
  SMITH_CHECK(debug_info != nullptr);
  auto* test_debug_info = dynamic_cast<TestThreadLocalDebugInfo*>(debug_info);
  SMITH_CHECK(test_debug_info != nullptr);
  SMITH_CHECK(test_debug_info->getModelId() == model_id);
}
...
TEST(TestSymIntArrayRef, BasicConversion) {
  ...
  ASSERT_TRUE(smith::allclose(b, c));
}
...
TEST(FallbackGraphsTest, Basic) {
  ...
  auto graph_string = R"IR(
    graph(%0 : Float(1),
      ...
      return (%3))IR";
  auto graph = std::make_shared<Graph>();
  smith::jit::parseIR(graph_string, graph.get());
...
//   auto input = smith::randn({seq_len, batch_size, input_size}, at::kCPU);
//   auto hx = smith::randn({batch_size, hidden_size}, at::kCPU);
//   auto cx = smith::randn({batch_size, hidden_size}, at::kCPU);
//   auto w_ih = t_def(smith::randn({4 * hidden_size, input_size}, at::kCPU));
//   auto w_hh = t_def(smith::randn({4 * hidden_size, hidden_size}, at::kCPU));
...
graph(%a):
  ...
  std::vector<IValue> stack = {IValue(smith::randn({22}, at::kCPU))};
...
TEST(JitTracing, Basic) {
...

  {
    ...
    smith::allclose(prof_out, traced_out);
  }

  {
    ...
    smith::allclose(prof_out, scripted_out);
  }
}
...
graph(%inp : Tensor,
  ...
  auto input = smith::randn({1, 2});
  auto weight = smith::randn({2, 2});
  auto bias = smith::randn({1, 2});
...
TEST(InlinedCallStackTest, SelfCallMethods) {
  ...
  for (Node* n : graph->nodes()) {
    auto hierarchy = smith::jit::utils::getNodesModuleHierarchy(*n);
...
TEST(AutogradSymbolsTest, Basic) {
  ...
  SMITH_CHECK(canRunWithAutograd(node));
...
TEST(TLSFutureCallbacksTest, Basic) {
  ...
  auto profilerEnabledCb = [](Future& /* unused */) {
    ASSERT_TRUE(smith::autograd::profiler::profilerEnabled());
  };
  // test running callbacks with propagation of TLS state.
  {
    // Enable the profiler in this thread
    smith::autograd::profiler::enableProfilerLegacy(
        smith::autograd::profiler::ProfilerConfig(
            smith::autograd::profiler::ProfilerState::CPU, false, false));
    auto s1 = c10::make_intrusive<Future>(IntType::get());
    ...
    t.join();
    smith::autograd::profiler::disableProfilerLegacy();
  }
  // then() with TLS State
  {
    // Enable the profiler in this thread
...
TEST(ProfilerDisableInCallbackTest, Basic) {
  ...
  auto verifyProfilerCb =
      wrapPropagateTLSState([&profilerEnabledCb](Future& /* unused */) {
        ...
        profilerEnabledCb();
        auto t1 = smith::ones({2, 2});
        auto t2 = smith::ones({2, 2});
        smith::add(t1, t2);
        // Don't cleanup TLSState, and just consolidate.
        auto opts =
            smith::autograd::profiler::ProfilerDisableOptions(false, true);
        auto thread_event_lists =
            // NOLINTNEXTLINE(performance-move-const-arg)
            smith::autograd::profiler::disableProfilerLegacy(std::move(opts));
  ...
  auto opts = smith::autograd::profiler::ProfilerDisableOptions(true, false);
  // NOLINTNEXTLINE(performance-move-const-arg)
  smith::autograd::profiler::disableProfilerLegacy(std::move(opts));
  ...
  opts = smith::autograd::profiler::ProfilerDisableOptions(true, false);
  // NOLINTNEXTLINE(performance-move-const-arg)
  smith::autograd::profiler::disableProfilerLegacy(std::move(opts));
}
...
TEST(RecordDebugHandles, Basic) {
  ...
  // Enable the profiler in this thread
  const std::set<smith::autograd::profiler::ActivityType> activities(
      {smith::autograd::profiler::ActivityType::CPU});
  smith::autograd::profiler::prepareProfiler(
      smith::autograd::profiler::ProfilerConfig(
          smith::autograd::profiler::ProfilerState::KINETO, false, false),
      activities);
  smith::autograd::profiler::enableProfiler(
      smith::autograd::profiler::ProfilerConfig(
          smith::autograd::profiler::ProfilerState::KINETO, false, false),
      activities);
  {
  ...
  auto profiler_results_ptr = smith::autograd::profiler::disableProfiler();
...
TEST(RecordDebugHandles, ScopedCallbacks) {
  ...
  {
    auto a = smith::rand({128, 128});
    auto b = smith::rand({128, 128});
    auto c = a + b;
  }
  auto profiler_results_ptr = smith::autograd::profiler::disableProfiler();
  ...
  profiler_results_ptr = smith::autograd::profiler::disableProfiler();
  ASSERT_TRUE(profiler_results_ptr->events().size() == 0);
...
TEST(ComputeFlopsTest, Basic) {
  ...
  flops = smith::profiler::impl::computeFlops(
  ...
  flops =
      smith::profiler::impl::computeFlops(std::string("aten::mm"), extra_args);
  ...
  extra_args["mat2_size"] = at::IValue(at::IntArrayRef(mat2_sizes));
  flops =
      smith::profiler::impl::computeFlops(std::string("aten::bmm"), extra_args);
  ...
  flops =
      smith::profiler::impl::computeFlops(std::string("aten::add"), extra_args);
  ASSERT_EQ(flops, 360);
  ...
  extra_args["mat_size"] = at::IValue(at::IntArrayRef(mat_sizes));
  flops =
      smith::profiler::impl::computeFlops(std::string("aten::mul"), extra_args);
  ASSERT_EQ(flops, 360);
}

TEST(TestConstant, TensorGrad) {
  auto graph = std::make_shared<Graph>();
  IValue ten = smith::randn({3, 5}).requires_grad_(true);
...
graph():
  ...
  smith::jit::RegisterShapeComputeGraphForSchema(schema, graph);
...
TEST(TestFunctionExecutor, RunDecompositionTest) {
  static auto* func = smith::jit::GetDecompositionExecutor(
...
class Composed : public ::testing::Test {
 public:
  void SetUp() override {
    smith::jit::tensorexpr::getTEMustUseLLVMOnCPU() = false;
  }
};
...
TEST_F(Composed, ComposedOp) {
  ...
  auto ref2 = a * ref1;
  WithCPUFuser g(true);
  bool fusable_on_device = smith::jit::tensorexpr::getTEMustUseLLVMOnCPU();
  smith::jit::tensorexpr::getTEMustUseLLVMOnCPU() = false;
  FuseTensorExprs(
      graph,
      /*min_group_size*/ 2,
  ...
  smith::jit::tensorexpr::getTEMustUseLLVMOnCPU() = fusable_on_device;
#endif
}
...
TEST(ConstantPropagation, CustomClassesCanBePropagated) {
  ...
  const auto src = R"IR(
    graph():
        ...
        %params: __smith__.smith.classes.quantized.LinearPackedParamsBase = quantized::linear_prepack(%weight_q, %none)
        return (%params)
  )IR";
  auto graph = std::make_shared<Graph>();
  std::unordered_map<std::string, Value*> vmap;
