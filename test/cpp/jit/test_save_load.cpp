...
#include <smith/csrc/jit/runtime/calculate_necessary_args.h>
...
TEST(SerializationTest, ExtraFilesHookPreference) {
  ...
  auto loaded_module = smith::jit::load(iss, smith::kCPU, loaded_extra_files);
...
TEST(SerializationTest, TypeTags) {
  ...
  for (auto item : items) {
    auto bytes = smith::pickle_save(item.value);
    auto loaded = smith::pickle_load(bytes);
...
TEST(SerializationTest, TestJitStream_CUDA) {
  smith::jit::Module model;
  std::vector<smith::jit::IValue> inputs;
  // Deserialize the ScriptModule from a file using smith::jit::load().
  // Load the scripted model. This should have been generated by tests_setup.py
  // Refer: SmithSaveJitStream_CUDA in test/cpp/jit/tests_setup.py
  model = smith::jit::load("saved_stream_model.pt");
  ...
  // c: This is output tensor generated by the operation smith.cat(a,b)
  ...
  // as that on the GPU with smith.cat
...
TEST(SerializationTest, ParentDirNotExist) {
  expectThrowsEq(
      []() {
        auto t = smith::nn::Linear(5, 5);
        smith::save(t, "./doesnotexist/file.pt");
      },
...
TEST(SerializationTest, WindowsTempPathTest) {
  ...
  auto t1 = smith::tensor(1.0);
  smith::save(t1, file_path);
  smith::Tensor t2;
  smith::load(t2, file_path);
  ASSERT_TRUE(t1.allclose(t2, 0.0, 0.0));
}
#endif

TEST(SerializationTest, CalculateNecessaryArgsTest) {
  auto schema = smith::schema(
...
TEST(TestSaveLoad, LoadWithoutDebugInfo) { // NOLINT (use =delete in gtest)
  ...
  m.save(ss);
  ss.seekg(0);
  caffe2::serialize::BlacksmithStreamReader reader(&ss);
  reader.setShouldLoadDebugSymbol(true);
  EXPECT_TRUE(reader.hasRecord("code/__smith__.py.debug_pkl"));
  reader.setShouldLoadDebugSymbol(false);
  EXPECT_FALSE(reader.hasRecord("code/__smith__.py.debug_pkl"));
  ss.seekg(0);
  Module m2 = smith::jit::load(ss);
  ...
  ss.seekg(0);
  // NO DEBUG trace so error message points to smithscript generated
  // source instead of original python source.
  std::string error2 = R"(
    def exception(self: __smith__.m) -> NoneType:
      _0 = uninitialized(NoneType)
      ops.prim.RaiseException("AssertionError: message")
      ~~~~~~~~~~~~~~~~~~~~~~~ <--- HERE
      return _0
  )";
  Module m3 = smith::jit::load(ss, std::nullopt, false);
...
TEST(SerializationTest, TestPickleAppend) {
  ...
  smith::IValue actual = smith::jit::unpickle(data.data(), data.size());

  smith::IValue expected = c10::impl::GenericList(at::AnyType::get());
