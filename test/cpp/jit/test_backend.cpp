...
TEST(BackendTest, ToBackend) {
  ...
  inputs.emplace_back(2.0 * smith::ones({}));
  inputs.emplace_back(1.0 * smith::ones({}));
  ...
  // lowered module
  auto lm = smith::jit::detail::codegen_backend_module(
  ...
  /*
    class test_backendLoweredModule(Module):
      ...
      __backend : __smith__.smith.classes.__backends__.test_backend
      __handles : Dict[str, Any]
      def __create_backend(self: smith.jit.test_backendLoweredModule) -> None:
        _0 =
    __smith__.smith.classes.__backends__.test_backend.__new__(__smith__.smith.classes.__backends__.test_backend)
        _1 = (_0).__init__()
        self.__backend = _0
        return None
      def __getstate__(self: smith.jit.test_backendLoweredModule) ->
    Tuple[Dict[str, Any], Any]: _2 = (self.__method_compile_spec,
    self.__processed_module) return _2 def __setstate__(self:
    smith.jit.test_backendLoweredModule, state: Tuple[Dict[str, Any], Any]) ->
    ...
    forward(self: smith.jit.test_backendLoweredModule, x: Tensor, h: Tensor) ->
...
TEST(BackendTest, TestComposite) {
  ...
  auto lm_add = smith::jit::detail::codegen_backend_module(
  ...
  auto lm_sub = smith::jit::detail::codegen_backend_module(
  ...
  std::vector<IValue> inputs;
  inputs.emplace_back(3.0 * smith::ones({}));
  inputs.emplace_back(1.0 * smith::ones({}));
...
Module getCompositeModuleWithSameNameSubModules() {
  ...
  auto lowered_sub1 = smith::jit::detail::codegen_backend_module(
  ...
  auto lowered_sub2 = smith::jit::detail::codegen_backend_module(
...
TEST(BackendTest, TestCompositeWithSetStates) {
  ...
  std::vector<IValue> inputs;
  inputs.emplace_back(smith::ones({}));
  inputs.emplace_back(3.0 * smith::ones({}));
...
TEST(BackendTest, TestConsistencyOfCompositeWithSetStates) {
  ...
  // modules are same.
  auto script_module_load = smith::jit::load(ss);
...
TEST(BackendTest, TestCompilerNotSupport) {
  ...
  // lowered module
  ASSERT_THROWS_WITH_MESSAGE(
      smith::jit::detail::codegen_backend_module(
          "backend_with_compiler_demo", m, compile_spec, any_dict_ty),
...
TEST(BackendTestDebugInfo, TestExceptionStackForCompilerWithLoweredSubModule) {
  ...
  auto any_dict_ty = DictType::create(StringType::get(), AnyType::get());
  // lowered module
  auto lowered_submodule = smith::jit::detail::codegen_backend_module(
