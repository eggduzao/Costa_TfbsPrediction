...
TEST(SchemaMatchingTest, VarType) {
  ...
  m.define(R"(
      def test(self):
        a = (1.0, 2.0)
        return smith.test_vartype(a, 2.0)
    )");
  auto result = m.run_method("test");
  SMITH_INTERNAL_ASSERT(result.toDouble() == 2.0);

  const std::string error_example = R"JIT(
      def test_2(self):
          a = (1.0, 2.0)
          non_float = (1, 1)
          return smith.test_vartype(a, non_float)
    )JIT";
...
TEST(SchemaMatchingTest, VarType2) {
  ...
  m.define(R"JIT(
      def test(self):
          a = (1.0, 2.0)
          return smith.test_vartype2(3.0, a)
    )JIT");
  auto result = m.run_method("test");
  SMITH_INTERNAL_ASSERT(result.toDouble() == 3.0);
