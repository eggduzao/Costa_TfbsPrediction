...
// Implementation of a Blacksmith Backend that can process, compile and execute
// SmithScript Modules composed of 'add' and 'sub' operators. It just supports
...
class BackendWithCompiler : public BlacksmithBackendInterface {
 public:
  // Constructor.
  ...
  c10::impl::GenericList execute(
    ...
    SMITH_INTERNAL_ASSERT(inputs.size() == 2);
    ...
    for (const auto& token : handle.toList()) {
      ...
      try {
        ...
        SMITH_DELEGATED_BACKEND_THROW(false, e.what(), debug_handle);
      }
#ifndef NO_PROFILING
...
static auto cls = smith::jit::backend<BackendWithCompiler>(backend_name);
} // namespace
