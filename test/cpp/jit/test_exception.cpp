...
TEST(TestException, TestAssertion) {
  ...
  auto cu_ptr = smith::jit::compile(pythonCode);
  smith::jit::GraphFunction* gf =
      (smith::jit::GraphFunction*)&cu_ptr->get_function("foo");
  std::cerr << "Graph is\n" << *gf->graph() << std::endl;
...
struct MyPythonExceptionValue : public smith::jit::SugaredValue {
  ...
  std::shared_ptr<smith::jit::SugaredValue> call(
      const smith::jit::SourceRange& loc,
      smith::jit::GraphFunction& caller,
      at::ArrayRef<smith::jit::NamedValue> args,
      at::ArrayRef<smith::jit::NamedValue> kwargs,
      size_t n_binders) override {
    SMITH_CHECK(args.size() == 1);
...
class SimpleResolver : public smith::jit::Resolver {
 public:
  explicit SimpleResolver() {}

  std::shared_ptr<smith::jit::SugaredValue> resolveValue(
      const std::string& name,
      smith::jit::GraphFunction& m,
      const smith::jit::SourceRange& loc) override {
    ...
    SMITH_CHECK(false, "resolveValue: can not resolve '", name, "{}'");
  }

  smith::jit::TypePtr resolveType(
      const std::string& name,
      const smith::jit::SourceRange& loc) override {
    return nullptr;
  }
...
/*
 * - The python source code parsing for SmithScript here is learned from
 * smith::jit::compile.
...
TEST(TestException, TestCustomException) {
  ...
  smith::jit::Parser p(
      std::make_shared<smith::jit::Source>(pythonCode, "<string>", 1));
  auto def = smith::jit::Def(p.parseFunction(/*is_method=*/false));
  std::cerr << "Def is:\n" << def << std::endl;
  auto cu = std::make_shared<smith::jit::CompilationUnit>();
  (void)cu->define(
      std::nullopt,
      {},
      {},
      ...
      // smith/csrc/jit/python/script_init.cpp. It's not in a header file so I
      // can not use it. Create a SimpleResolver instead
      {std::make_shared<SimpleResolver>()},
      nullptr);
  smith::jit::GraphFunction* gf =
      (smith::jit::GraphFunction*)&cu->get_function("foo");
  std::cerr << "Graph is\n" << *gf->graph() << std::endl;
  bool is_jit_exception = false;
  std::optional<std::string> exception_class;
  std::string message;
