...
TEST(CustomClassTest, SmithbindIValueAPI) {
  script::Module m("m");
  ...
  m.define(R"(
    def forward(self, s : __smith__.smith.classes._SmithScriptTesting._StackString):
      return s.pop(), s
  )");
...
TEST(CustomClassTest, ScalarTypeClass) {
  ...
  auto loaded_module = smith::jit::load(iss, smith::kCPU);
}

class SmithBindTestClass : public smith::jit::CustomClassHolder {
 public:
  std::string get() {
...
constexpr char class_doc_string[] = R"(
  I am docstring for SmithBindTestClass
  Args:
...
constexpr char method_doc_string[] =
    "I am docstring for SmithBindTestClass get_with_docstring method";

namespace {
static auto reg =
    smith::class_<SmithBindTestClass>(
        "_SmithBindTest",
        "_SmithBindTestClass",
        class_doc_string)
        .def("get", &SmithBindTestClass::get)
        .def("get_with_docstring", &SmithBindTestClass::get, method_doc_string);
...
TEST(CustomClassTest, TestDocString) {
  auto class_type = getCustomClass(
      "__smith__.smith.classes._SmithBindTest._SmithBindTestClass");
...
TEST(CustomClassTest, Serialization) {
  ...
  auto frozen_m = smith::jit::freeze_module(m.clone());
  ...
  auto loaded_frozen_module = smith::jit::load(iss_frozen, smith::kCPU);
}
