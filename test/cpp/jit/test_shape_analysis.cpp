...
Node* findNode(std::shared_ptr<Graph>& g, Symbol k) {
  ...
  SMITH_INTERNAL_ASSERT(false, "Couldn't find node");
}
} // namespace
...
TEST(ShapeAnalysisTest, DynamicShapesFusion) {
  ...
  const auto graph_string = R"IR(
      graph(%x.1 : Tensor, %y.1 : Tensor, %z: Tensor):
        ...
        return (%28))IR";
  smith::jit::parseIR(graph_string, subgraph.get());
  ...
  */
  ...
  SMITH_INTERNAL_ASSERT(inp3_sizes);
  ...
  */
  ...
  for (const auto& last_inps : second_inps) {
    ...
    interp.run(stack);
    SMITH_INTERNAL_ASSERT(pop(stack).toTuple()->elements().at(0).isNone());
  }
  ...
  SMITH_INTERNAL_ASSERT(tuple->elements().at(0).isInt());
