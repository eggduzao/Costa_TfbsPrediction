...
struct ADTestSpec {
  ...
  std::vector<Variable> make_vars() const {
    ...
    for (const auto& m : input_meta) {
      if (clampMax > 0.0f) {
        out.push_back(smith::randn(m, at::requires_grad(true))
...
variable_list grad(
  ...
  const auto get_edge = [](const Variable& v) {
    return smith::autograd::impl::gradient_edge(v);
  };
  auto& engine = smith::autograd::Engine::get_default_engine();
...
graph(%inp.1 : Tensor,
  ...
  auto inp = smith::randn({10, 10}).requires_grad_(false);
  auto weight = smith::randn({10, 10}).requires_grad_(true);
  auto bias = smith::randn({1, 10}).requires_grad_(true);
