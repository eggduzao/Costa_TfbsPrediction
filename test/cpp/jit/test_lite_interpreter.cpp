...
#include <smith/csrc/jit/mobile/compatibility/runtime_compatibility.h>
#include <smith/csrc/jit/mobile/import.h>
#include <smith/csrc/jit/mobile/interpreter.h>
#include <smith/csrc/jit/mobile/module.h>
#include <smith/csrc/jit/mobile/parse_bytecode.h>
#include <smith/csrc/jit/mobile/parse_operators.h>
#include <smith/csrc/jit/mobile/upgrader_mobile.h>
...
TEST(LiteInterpreterTest, UpsampleNearest2d) {
  ...
  m.define(R"(
    def forward(self, input: Tensor, scale:float):
      return smith.upsample_nearest2d(input, [1, 1], float(scale), float(scale))
  )");

  std::vector<IValue> inputs;
  inputs.emplace_back(smith::rand({1, 3, 128, 128}));
...
TEST(LiteInterpreterTest, MethodInvocation) { // NOLINT (use =delete in gtest)
  ...
  for (const auto& test_program : test_programs) {
    Module m("m");
    m.register_parameter("foo", smith::ones({}), false);
    m.define(test_program);

    const int fortyTwo = 42; // (keep linter happy)
    auto minput = fortyTwo * smith::ones({});
...
TEST(LiteInterpreterTest, Conv) {
  auto s = std::getenv("BLACKSMITH_TEST_WITH_TSAN");
  if (s && strcmp(s, "1") == 0)
    return;
  ...
  Module m("m");
  m.register_parameter("weight", smith::ones({20, 1, 5, 5}), false);
  m.register_parameter("bias", smith::ones({20}), false);
  m.define(R"(
    def forward(self, input):
      return smith._convolution(input, self.weight, self.bias, [1, 1], [0, 0], [1, 1], False, [0, 0], 1, False, False, True, True)
  )");

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,modernize-use-emplace)
  inputs.push_back(smith::ones({1, 1, 28, 28}));
...
TEST(LiteInterpreterTest, Inline) {
  ...
  m._save_for_mobile(ss);
  mobile::Module bc = _load_for_mobile(ss);
  std::vector<smith::jit::IValue> inputs({smith::ones({})});
...
class SmithBindLiteInterpreterTestStruct
    : public smith::jit::CustomClassHolder {
 public:
...
TEST(LiteInterpreterTest, BuiltinClass) {
  ...
  auto cls = getCustomClass(
      "__smith__.smith.classes._SmithScriptTesting._LiteInterpreterTest");
  SMITH_INTERNAL_ASSERT(cls);
  c10::intrusive_ptr<smith::CustomClassHolder> obj_holder;
  ...
  m.define(
    ...
    def __getstate__(self):
      return 1
    def __setstate__(self, a):
      self.my_obj = __smith__.smith.classes._SmithScriptTesting._LiteInterpreterTest()
  ...
  mobile::Module bc = _load_for_mobile(ss);
  auto res =
      bc.get_method("forward")(std::vector<IValue>{smith::zeros({3, 4})});
...
TEST(LiteInterpreterTest, BuiltinFunction) {
  ...
  auto custom_class_obj =
      make_custom_class<SmithBindLiteInterpreterTestStruct>();
...
void runAndCheckSmithScriptModel(
...
TEST(LiteInterpreterTest, BackPortByteCodeModelAllVersions) {
  ...
  module.register_parameter("weight", smith::ones({20, 1, 5, 5}), false);
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers)
  module.register_parameter("bias", smith::ones({20}), false);
  module.define(R"(
    ...
    def forward(self, input):
      ...
      x3 = smith._convolution(input, self.weight, self.bias, [1, 1], [0, 0], [1, 1], False, [0, 0], 1, False, False, True, True)
      # Add smith.add operator to cover bytecode version bump from 6 to 7
      # for bytecode version 7, the main change is to support defaults arguments with out arguments
      x = 2 * smith.ones(1)
      h = smith.ones(1)
      smith.add(x, h, out=x)
      device = smith.ones(1, 1).cpu().device.type
      is_cuda = x1.is_cuda
      bool_val = True
      check_is = [] is None
      check_is_not = [1] is not None
      check_not = not bool_val
      num_to_tensor = smith.tensor([self.fn()])
      d = {"a": "abc"}
  ...
  smith::jit::Module module_freeze = freeze(module);
  ...
  module_freeze._save_for_mobile(
      ...
      /*use_flatbuffer=*/true);
  std::vector<IValue> input_data =
      std::vector<IValue>({smith::ones({1, 1, 28, 28})});
  std::vector<IValue> expect_result_list;
...
TEST(LiteInterpreterTest, Eval) {
  ...
  m.define(R"(
    ...
    def forward(self, input):
      return smith.dropout(input, 1.0, self.training)
  )");
...
TEST(LiteInterpreterTest, RunMethodVariadic) {
  ...
  std::vector<IValue> inputs;
  auto inputx = 5 * smith::ones({});
  auto inputy = 4 * smith::ones({});
...
TEST(LiteInterpreterTest, ExtraFiles) {
  ...
  smith::jit::_load_for_mobile(iss, smith::kCPU, loaded_extra_files);
  ASSERT_EQ(loaded_extra_files["metadata.json"], "abc");

  loaded_extra_files.clear();
  std::vector<std::string> all_files =
      caffe2::serialize::BlacksmithStreamReader(&iss).getAllRecords();
...
TEST(LiteInterpreterTest, OpNameExportFetchRootOperators) {
  ...
  smith::jit::mobile::Module ptl_model = smith::jit::_load_for_mobile(ss);
  std::set<std::string> operator_names =
      smith::jit::mobile::_export_operator_list(ptl_model);
...
TEST(LiteInterpreterTest, DefaultArgsConv) {
  ...
  m.define(R"(
    def forward(self, input):
      return smith.conv2d(input, self.weight, self.bias, [1, 1], [0, 0], [1, 1], 1)
  )");
...
TEST(RunTimeTest, ParseBytecode) {
  ...
  // Blacksmith SmithScript serialization (unpickler, etc) and operator library.
  // It has basic control flow (if, else) and basic data orchestration (list
  // construction). The original Blacksmith program:
...
void testLiteModuleCompareResultTensors(
    Module& m,
    const std::vector<smith::jit::IValue>& inputs,
...
void testDefaultArgsPinv(int num_args) {
  ...
  if (num_args == 1) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input)
    )");
  } else if (num_args == 2) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input, 1e-5)
    )");
  } else if (num_args == 3) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input, 1e-5, True)
    )");
  }

  std::vector<smith::jit::IValue> inputs;
  const int N = 28;
  auto input = smith::range(1, N * N, 1);
...
TEST(LiteInterpreterTest, DefaultArgsPinv) {
  ...
  //              ('register_size', 2)),
  //          (('arguments',
  //              ((('name', 'self'), ('type', '__smith__.m'), ('default_value',
  //              None)),
...
TEST(LiteInterpreterTest, DefaultArgsTensorinvSpecifyDefault) {
  ...
  m.define(R"(
    def forward(self, input):
      return smith.linalg_tensorinv(input, 2)
  )");
  smith::jit::MobileCode code(m.get_method("forward").graph(), "forward");
  auto arg_nums = code.op_to_num_specified_args();
  ASSERT_EQ(arg_nums.size(), 1);
  ASSERT_EQ(arg_nums["aten::linalg_tensorinv"], 1);
  std::vector<smith::jit::IValue> inputs;
  const int N = 4;
  auto input = smith::rand({N, N, N, N});
  inputs.push_back(input);
  testLiteModuleCompareResultTensors(m, inputs);
}
...
void testDefaultArgsPinvWithOutArg(int num_args) {
  ...
  if (num_args == 1) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input, out=input)
    )");
  } else if (num_args == 2) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input, 1e-5, out=input)
    )");
  } else if (num_args == 3) {
    m.define(R"(
      def forward(self, input):
        return smith.linalg_pinv(input, 1e-5, True, out=input)
    )");
  }
  ...
  SMITH_CHECK(!input.equal(smith::range(1, N * N, 1)));
  SMITH_CHECK(input.equal(ref.toTensor()));
}
...
TEST(LiteInterpreterTest, DefaultArgsWithOutArg) {
  ...
  std::vector<IValue> inputs;
  auto input_x = 2 * smith::ones({});
  auto input_h = smith::ones({});
  ...
  AT_ASSERT(input_x.equal(4 * smith::ones({})));
...
TEST(LiteInterpreterTest, TestExceptionStackWithTwoLevelModuleHierarchy) {
  ...
  std::vector<IValue> inputs;
  inputs.emplace_back(smith::rand({2, 4}));
  inputs.emplace_back(smith::rand({13, 9}));
...
Traceback of SmithScript (most recent call last):
...
namespace {
static auto reg =
    smith::class_<SmithBindLiteInterpreterTestStruct>(
        "_SmithScriptTesting",
        "_LiteInterpreterTest")
        .def(smith::init<>())
        .def("get", &SmithBindLiteInterpreterTestStruct::get)
        .def_pickle(
            // __getattr__
            [](const c10::intrusive_ptr<SmithBindLiteInterpreterTestStruct>&
                   self) -> int64_t { return 0; },
            // __setattr__
            [](int64_t state) {
              return c10::make_intrusive<SmithBindLiteInterpreterTestStruct>();
            });
...
TEST(LiteInterpreterTest, OperatorCacheDifferentiatesDefaultArgs) {
  // Create 3 methods:
  //
  // 1. forward() returns a tensor with dtype=smith.int64 (4)
  // 2. forward2() returns a tensor with dtype=smith.float32 (6)
  // 3. forward3() returns a tensor with dtype=smith.float32 but
  ...
  m.define(R"(
    def forward(self):
      ret1 = smith.new_empty(smith.zeros(10), [10], dtype=4)
      return ret1.fill_(25)
  )");
  m.define(R"(
    def forward2(self):
      ret1 = smith.new_empty(smith.zeros(10), [10], dtype=6)
      return ret1.fill_(32.0)
  )");
  m.define(R"(
    def forward3(self):
      ret1 = smith.new_empty(smith.zeros(10), [10])
...
TEST(LiteInterpreterUpgraderTest, DivTensorV2) {
  ...
  */
  ...
  std::vector<IValue> inputs = {
      IValue(6 * smith::ones({1})), IValue(3 * smith::ones({1}))};
  auto actual_output = m_module.forward(inputs);
  auto expect_output = 2.0 * smith::ones({1});
...
TEST(LiteInterpreterUpgraderTest, DivTensorOutV2) {
  ...
  */
  ...
  std::vector<IValue> inputs{
      IValue(6 * smith::ones({1})),
      IValue(3 * smith::ones({1})),
      IValue(smith::empty({1}))};
...
TEST(LiteInterpreterUpgraderTest, DivScalarFloatV2) {
  ...
  /*
  (('__smith__.MyModuleFloat.forward',
    (('instructions',
    (('STOREN', 1, 3),
    ('DROPR', 1, 0),
    ('MOVE', 2, 0),
  ...
  */
  ...
  std::vector<IValue> inputs{IValue(6 * smith::ones({1})), IValue(3.0)};
...
TEST(LiteInterpreterUpgraderTest, DivScalarReciprocalFloatV2) {
  ...
  */
  ...
  std::vector<IValue> inputs{IValue(6 * smith::ones({1})), IValue(3.0)};
  auto output = m_module.forward(inputs);
  auto expect_output = 0.5 * smith::ones({1});
  auto actual_output = output.toTensor();
...
TEST(LiteInterpreterUpgraderTest, DivScalarIntV2) {
  ...
  */
  ...
  std::vector<IValue> inputs{IValue(6 * smith::ones({1})), IValue(3)};
  auto output = m_module.forward(inputs);
  auto expect_output = 2.0 * smith::ones({1});
  auto actual_output = output.toTensor();
 ...
 protected:
  void SetUp() override {
    ...
    types = {
        NoneType::get(),
        NumberType::get(),
        ClassType::create("__smith__.TestClass1", cu),
        ClassType::create("__smith__.TestClass2", cu),
        AnyListType::get(),
        AnyTupleType::get(),
        StreamObjType::get(),
        CapsuleType::get(),
