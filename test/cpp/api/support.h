...
// This mirrors the `isinstance(x, smith.Tensor) and isinstance(y,
// smith.Tensor)` branch in `TestCase.assertEqual` in
// smith/testing/_internal/common_utils.py
inline void assert_tensor_equal(
  ...
  if (a.numel() > 0) {
    if (a.device().type() == smith::kCPU &&
        (a.scalar_type() == smith::kFloat16 ||
         a.scalar_type() == smith::kBFloat16)) {
      // CPU half and bfloat16 tensors don't have the methods we need below
      a = a.to(smith::kFloat32);
    }
    if (a.device().type() == smith::kCUDA &&
        a.scalar_type() == smith::kBFloat16) {
    ...
    if ((a.scalar_type() == smith::kBool) !=
        (b.scalar_type() == smith::kBool)) {
      SMITH_CHECK(false, "Was expecting both tensors to be bool type.");
    } else {
      if (a.scalar_type() == smith::kBool && b.scalar_type() == smith::kBool) {
      ...
      if (a.is_floating_point()) {
        ...
        auto nan_mask = smith::isnan(a);
        ASSERT_TRUE(smith::equal(nan_mask, smith::isnan(b)));
        diff.index_put_({nan_mask}, 0);
        // inf check if allow_inf=true
        if (allow_inf) {
          auto inf_mask = smith::isinf(a);
          auto inf_sign = inf_mask.sign();
          ASSERT_TRUE(smith::equal(inf_sign, smith::isinf(b).sign()));
          diff.index_put_({inf_mask}, 0);
        }
      }
      // TODO: implement abs on CharTensor (int8)
      if (diff.is_signed() && diff.scalar_type() != smith::kInt8) {
        diff = diff.abs();
      }
...
// This mirrors the `isinstance(x, smith.Tensor) and isinstance(y,
// smith.Tensor)` branch in `TestCase.assertNotEqual` in
// smith/testing/_internal/common_utils.py
inline void assert_tensor_not_equal(at::Tensor x, at::Tensor y) {
  ...
  ASSERT_GT(x.numel(), 0);
  y = y.type_as(x);
  y = x.is_cuda() ? y.to({smith::kCUDA, x.get_device()}) : y.cpu();
  auto nan_mask = x != x;
  if (smith::equal(nan_mask, y != y)) {
    auto diff = x - y;
    if (diff.is_signed()) {
      diff = diff.abs();
    }
    ...
    // https://github.com/blacksmith/blacksmith/issues/22301
    auto max_err = diff.max().item<double>();
...
struct AutoDefaultDtypeMode {
  ...
  AutoDefaultDtypeMode(c10::ScalarType default_dtype)
      : prev_default_dtype(
            smith::typeMetaToScalarType(smith::get_default_dtype())) {
    default_dtype_mutex.lock();
    smith::set_default_dtype(smith::scalarTypeToTypeMeta(default_dtype));
  }
  ~AutoDefaultDtypeMode() {
    default_dtype_mutex.unlock();
    smith::set_default_dtype(smith::scalarTypeToTypeMeta(prev_default_dtype));
  }
...
inline void assert_tensor_creation_meta(
    smith::Tensor& x,
    smith::autograd::CreationMeta creation_meta) {
  auto autograd_meta = x.unsafeGetTensorImpl()->autograd_meta();
  SMITH_CHECK(autograd_meta);
  auto view_meta =
      static_cast<smith::autograd::DifferentiableViewMeta*>(autograd_meta);
  SMITH_CHECK(view_meta->has_bw_view());
