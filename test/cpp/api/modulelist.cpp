...
struct ModuleListTest : smith::test::SeedingFixture {};
...
TEST_F(ModuleListTest, IsCloneable) {
  ModuleList list(Linear(3, 4), Functional(smith::relu), BatchNorm1d(3));
...
TEST_F(ModuleListTest, RangeBasedForLoop) {
  smith::nn::ModuleList mlist(
      smith::nn::Linear(3, 4),
      smith::nn::BatchNorm1d(4),
      smith::nn::Dropout(0.5));
...
TEST_F(ModuleListTest, InvalidAt) {
  smith::nn::ModuleList m(smith::nn::Linear(1, 2));
  ASSERT_THROWS_WITH(
      m->at<smith::nn::Dropout2dImpl>(0), "Unable to cast module");
}
