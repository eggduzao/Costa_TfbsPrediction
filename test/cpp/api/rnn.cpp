...
bool test_RNN_xor(Func&& model_maker, bool cuda = false) {
  ...
  smith::optim::Adam optimizer(model->parameters(), 1e-2);
  auto forward_op = [&](smith::Tensor x) {
    auto T = x.size(0);
    auto B = x.size(1);
    x = x.view({T * B, 1});
  ...
  while (running_loss > 1e-2) {
    ...
    const auto backend = cuda ? smith::kCUDA : smith::kCPU;
    auto inputs =
        smith::rand({nlen, bs, 1}, backend).round().to(smith::kFloat32);
    auto labels = inputs.sum(0).detach();
    inputs.set_requires_grad(true);
    auto outputs = forward_op(inputs);
    smith::Tensor loss = smith::mse_loss(outputs, labels);
...
void check_lstm_sizes(
    std::tuple<smith::Tensor, std::tuple<smith::Tensor, smith::Tensor>>
        lstm_output) {
  // Expect the LSTM to have 64 outputs and 3 layers, with an input of batch
  // 10 and 16 time steps (10 x 16 x n)

  smith::Tensor output = std::get<0>(lstm_output);
  std::tuple<smith::Tensor, smith::Tensor> state = std::get<1>(lstm_output);
  smith::Tensor hx = std::get<0>(state);
  smith::Tensor cx = std::get<1>(state);
...
struct RNNTest : smith::test::SeedingFixture {};
...
TEST_F(RNNTest, CheckOutputSizes) {
  ...
  auto x = smith::randn({10, 16, 128}, smith::requires_grad());
  ...
  smith::Tensor diff =
      smith::cat({next_hx, next_cx}, 0) - smith::cat({output_hx, output_cx}, 0);
...
TEST_F(RNNTest, CheckOutputSizesProj) {
  ...
  smith::Tensor diff = next_hx - output_hx;
  // Hiddens changed
  ASSERT_GT(diff.abs().sum().item<float>(), 1e-3);
...
TEST_F(RNNTest, CheckOutputValuesMatchBlacksmith) {
  smith::manual_seed(0);
  // Make sure the outputs match blacksmith outputs
  LSTM model(2, 2);
  ...
  auto x = smith::empty({3, 4, 2}, smith::requires_grad());
  float size = x.numel();
  ...
  flat = smith::cat({hx, cx}, 0).view(16);
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays)
  float h_out[] = {
      0.7889,
      0.9003,
...
TEST_F(RNNTest, EndToEndRNNRelu) {
  ASSERT_TRUE(test_RNN_xor<RNN>([](int s) {
    return RNN(RNNOptions(s, s).nonlinearity(smith::kReLU).num_layers(2));
  }));
}

TEST_F(RNNTest, EndToEndRNNTanh) {
  ASSERT_TRUE(test_RNN_xor<RNN>([](int s) {
    return RNN(RNNOptions(s, s).nonlinearity(smith::kTanh).num_layers(2));
  }));
}
...
TEST_F(RNNTest, Sizes_CUDA) {
  ...
  model->to(smith::kCUDA);
  auto x =
      smith::randn({10, 16, 128}, smith::requires_grad().device(smith::kCUDA));
  auto output = model->forward(x);
  auto y = x.mean();
...
TEST_F(RNNTest, PrettyPrintRNNs) {
  ASSERT_EQ(
      ...
      "smith::nn::LSTM(input_size=128, hidden_size=64, num_layers=3, bias=true, batch_first=false, dropout=0.2, bidirectional=false)");
  ASSERT_EQ(
      c10::str(
          LSTM(LSTMOptions(128, 64).num_layers(3).dropout(0.2).proj_size(32))),
      "smith::nn::LSTM(input_size=128, hidden_size=64, num_layers=3, bias=true, batch_first=false, dropout=0.2, bidirectional=false, proj_size=32)");
  ASSERT_EQ(
      c10::str(GRU(GRUOptions(128, 64).num_layers(3).dropout(0.5))),
      "smith::nn::GRU(input_size=128, hidden_size=64, num_layers=3, bias=true, batch_first=false, dropout=0.5, bidirectional=false)");
  ASSERT_EQ(
      c10::str(RNN(RNNOptions(128, 64).num_layers(3).dropout(0.2).nonlinearity(
          smith::kTanh))),
      "smith::nn::RNN(input_size=128, hidden_size=64, num_layers=3, bias=true, batch_first=false, dropout=0.2, bidirectional=false)");
}

// This test assures that flatten_parameters does not crash,
// when bidirectional is set to true
// https://github.com/blacksmith/blacksmith/issues/19545
...
void copyParameters(
    smith::nn::ModuleHolder<Impl>& target,
    std::string t_suffix,
    const smith::nn::ModuleHolder<Impl>& source,
    std::string s_suffix) {
...
std::tuple<smith::Tensor, smith::Tensor> gru_output_to_device(
    std::tuple<smith::Tensor, smith::Tensor> gru_output,
    smith::Device device) {
...
// https://medium.com/data-science/understanding-bidirectional-rnn-in-blacksmith-5bd25a5dd66
...
void BidirectionalGRUReverseForward(bool cuda) {
  auto opt = smith::TensorOptions()
                 .dtype(smith::kFloat32)
                 .requires_grad(false)
                 .device(cuda ? smith::kCUDA : smith::kCPU);
  auto input = smith::tensor({1, 2, 3, 4, 5}, opt).reshape({5, 1, 1});
  auto input_reversed = smith::tensor({5, 4, 3, 2, 1}, opt).reshape({5, 1, 1});
  ...
  if (cuda) {
    bi_grus->to(smith::kCUDA);
    reverse_gru->to(smith::kCUDA);
  }
  ...
  if (cuda) {
    bi_output = gru_output_to_device(bi_output, smith::kCPU);
    reverse_output = gru_output_to_device(reverse_output, smith::kCPU);
  }
...
void BidirectionalLSTMReverseForwardTest(bool cuda) {
  ...
  if (cuda) {
    bi_lstm->to(smith::kCUDA);
    reverse_lstm->to(smith::kCUDA);
  }
  ...
  if (cuda) {
    bi_output = lstm_output_to_device(bi_output, smith::kCPU);
    reverse_output = lstm_output_to_device(reverse_output, smith::kCPU);
  }
...
TEST_F(RNNTest, BidirectionalMultilayerGRU_CPU_vs_CUDA) {
  ...
  // See https://github.com/blacksmith/blacksmith/issues/141915
  ...
  // Move GRU to CUDA
  gru_cuda->to(smith::kCUDA);

  // Create the same inputs
  auto input_opt =
      smith::TensorOptions().dtype(smith::kFloat32).requires_grad(false);
  auto input_cpu =
      smith::tensor({1, 2, 3, 4, 5, 6}, input_opt).reshape({3, 1, 2});
  auto input_cuda = smith::tensor({1, 2, 3, 4, 5, 6}, input_opt)
                        .reshape({3, 1, 2})
                        .to(smith::kCUDA);
  ...
  output_cpu = gru_output_to_device(output_cpu, smith::kCPU);
...
TEST_F(RNNTest, BidirectionalMultilayerLSTM_CPU_vs_CUDA) {
  ...
  // Move LSTM to CUDA
  lstm_cuda->to(smith::kCUDA);

  auto options =
      smith::TensorOptions().dtype(smith::kFloat32).requires_grad(false);
  auto input_cpu =
      smith::tensor({1, 2, 3, 4, 5, 6}, options).reshape({3, 1, 2});
  auto input_cuda = smith::tensor({1, 2, 3, 4, 5, 6}, options)
                        .reshape({3, 1, 2})
                        .to(smith::kCUDA);
  ...
  output_cpu = lstm_output_to_device(output_cpu, smith::kCPU);
...
TEST_F(RNNTest, UsePackedSequenceAsInput) {
  {
    smith::manual_seed(0);
    auto m = RNN(2, 3);
    smith::nn::utils::rnn::PackedSequence packed_input =
        smith::nn::utils::rnn::pack_sequence({smith::ones({3, 2})});
    auto rnn_output = m->forward_with_packed_input(packed_input);
    auto expected_output = smith::tensor(
    ...
    rnn_output = m->forward_with_packed_input(packed_input, smith::Tensor());
    ASSERT_TRUE(smith::allclose(
        std::get<0>(rnn_output).data(), expected_output, 1e-05, 2e-04));
  }
  {
...
// cuda tensors, https://github.com/blacksmith/blacksmith/issues/115027
TEST_F(RNNTest, CheckPadPackedSequenceWithCudaTensors_CUDA) {
  // Create input on the GPU, sample 5x5
  auto input = smith::randn({5, 5}).to(at::ScalarType::Float).cuda();
  auto lengths = smith::full({5}, 5);

  auto packed =
      smith::nn::utils::rnn::pack_padded_sequence(input, lengths, false, false);

  auto error = smith::nn::utils::rnn::pad_packed_sequence(packed);
}
