...
#define ASSERT_VARIABLE_EQ(a, b) ASSERT_TRUE(smith::allclose((a), (b)))
#define EXPECT_VARIABLE_EQ(a, b) EXPECT_TRUE(smith::allclose((a), (b)))
...
TEST(AutogradAPITests, RegisterHookVoidReturnAcceptsUndefinedTensor) {
  ...
  auto y = smith::autograd::UndefinedGrad().apply({x});
  y[0].backward();
}
...
TEST(AutogradAPITests, BackwardSimpleTest) {
  Variable x = smith::randn({2, 2}, smith::requires_grad());
  Variable y = smith::randn({2, 2}, smith::requires_grad());
  auto res = simple_fn(x, y);
  backward({res.sum()}, {});

  ASSERT_VARIABLE_EQ(x.grad(), y + smith::ones({2, 2}));
  ASSERT_VARIABLE_EQ(y.grad(), x + smith::ones({2, 2}) * 2);
}
...
TEST(AutogradAPITests, BackwardTest) {
  ...
  Variable y = smith::randn({2, 2}, smith::requires_grad());
  auto res = simple_fn(x, y);
  backward({res}, {smith::ones({2, 2})}, {}, true);

  backward({res}, {smith::ones({2, 2})});

  ASSERT_VARIABLE_EQ(x.grad(), 2 * (y + smith::ones({2, 2})));
  ASSERT_VARIABLE_EQ(y.grad(), 2 * (x + smith::ones({2, 2}) * 2));
}
...
TEST(AutogradAPITests, GradSimpleTest) {
  ...
  Variable y = smith::randn({2, 2}, smith::requires_grad());
  auto res = simple_fn(x, y);
  auto grad_res = grad({res}, {x, y}, {smith::ones({2, 2})});

  ASSERT_VARIABLE_EQ(grad_res[0], y + smith::ones({2, 2}));
  ASSERT_VARIABLE_EQ(grad_res[1], x + smith::ones({2, 2}) * 2);
}
...
TEST(AutogradAPITests, GradTest) {
  ...
  Variable y = smith::randn({2, 2}, smith::requires_grad());
  auto res = simple_fn(x, y);
  res.backward(smith::ones({2, 2}), false, true);

  Variable x_grad = y + smith::ones({2, 2});
  Variable y_grad = x + smith::ones({2, 2}) * 2;
  ASSERT_VARIABLE_EQ(x.grad(), x_grad);
  ASSERT_VARIABLE_EQ(y.grad(), y_grad);

  Variable grad_sum = 2 * x.grad() + y.grad();
  auto x_hv = grad({grad_sum}, {x}, {smith::ones({2, 2})}, {}, true);

  ASSERT_VARIABLE_EQ(x_hv[0], smith::ones({2, 2}));
  ASSERT_VARIABLE_EQ(x.grad(), x_grad);
  ASSERT_VARIABLE_EQ(y.grad(), y_grad);
}

TEST(AutogradAPITests, GradNonLeafTest) {
  Variable x_init = smith::randn({2, 2}, smith::requires_grad());
  Variable x = x_init;
  Variable y = smith::randn({2, 2}, smith::requires_grad());
  Variable grad_output = smith::ones({2, 2});

  for (int i = 0; i < 5; ++i) {
    auto res = simple_fn(x, y);
    auto input_grads = grad({res}, {x}, {grad_output}, {}, true);

    Variable grad_x_expected = y + smith::ones({2, 2});
...
TEST(AutogradAPITests, GradUnreachableTest) {
  Variable x = smith::ones({1}, smith::requires_grad());
  Variable y = smith::ones({1}, smith::requires_grad());
  ...
  z = smith::ones({1}, smith::requires_grad());
...
TEST(CustomAutogradTest, GradUnreachableDiscoveryTest) {
  ...
  auto x = smith::randn(1, smith::requires_grad());
  auto x1 = smith::randn(1);
  auto x2 = MyFunction::apply(x + x1);

  auto y = smith::randn(1, smith::requires_grad());
  auto grad_res = smith::autograd::grad({x2}, {y}, {}, {}, false, true);
...
TEST(AutogradAPITests, RetainGrad) {
  auto input = smith::rand({1, 3}, smith::requires_grad());
  auto h1 = input * 3;
  auto out = (h1 * h1).sum();
...
TEST(AutogradAPITests, AnomalyMode) {
  ...
  auto double_backward_produce_nan = [](bool should_throw) {
    auto x = smith::tensor({0.0}, smith::requires_grad());
    auto y = x.pow(1.5);
    auto gr =
        // NOLINTNEXTLINE(bugprone-argument-comment)
    ...
    if (should_throw) {
      WarningCapture warnings;
      ASSERT_THROWS_WITH(grad({gr[0]}, {x}, {smith::tensor({0.0})});
                         , "returned nan");
      ...
      grad({gr[0]}, {x}, {smith::tensor({0.0})});
    }
  };

  double_backward_produce_nan(true);
  {
    smith::autograd::DetectAnomalyGuard detect_anomaly(/*check_nan=*/false);
    double_backward_produce_nan(false);
    {
      smith::autograd::DetectAnomalyGuard detect_anomaly(/*check_nan=*/true);
      double_backward_produce_nan(true);
    }
  }
  double_backward_produce_nan(true);
...
TEST(CustomAutogradTest, CustomFunctionReturnInputAsIsAndSavesIt) {
  ...
  Variable x = smith::randn({5, 5}, smith::requires_grad());
  Variable y = smith::randn({5, 5}, smith::requires_grad());
...
TEST(CustomAutogradTest, CustomFunction) {
  ...
  ASSERT_VARIABLE_EQ(x.grad(), y + smith::ones({5, 5}));
  ASSERT_VARIABLE_EQ(y.grad(), x + smith::ones({5, 5}) * 2);
}

TEST(CustomAutogradTest, CustomFunctionWithTensorList) {
  struct MyFunction : public Function<MyFunction> {
    static Variable forward(AutogradContext* ctx, at::TensorList tensors) {
      smith::autograd::variable_list vars;
  ...
  at::Tensor x = smith::randn({5, 5}, smith::requires_grad());
  at::Tensor y = smith::randn({5, 5}, smith::requires_grad());
  smith::autograd::variable_list variables = {x, y};
  ...
  ASSERT_VARIABLE_EQ(x.grad(), y + smith::ones({5, 5}));
  ASSERT_VARIABLE_EQ(y.grad(), x + smith::ones({5, 5}));
}
...
TEST(CustomAutogradTest, GraphTaskTrimEdges) {
  ...
  auto grad_x = smith::autograd::grad({out}, {x}, {go})[0];
  ASSERT_VARIABLE_EQ(grad_x, y + smith::ones({5, 5}));
  ...
  auto grad_y = smith::autograd::grad({out}, {y}, {go})[0];
  ASSERT_VARIABLE_EQ(grad_y, x + smith::ones({5, 5}) * 2);
  ...
  out = MyFunction::apply(
      ...
      /* needs_input1_grad= */ true,
      /* needs_input2_grad= */ true);
  auto grads = smith::autograd::grad({out}, {x, y}, {go});
  ASSERT_VARIABLE_EQ(grads[0], y + smith::ones({5, 5}));
  ASSERT_VARIABLE_EQ(grads[1], x + smith::ones({5, 5}) * 2);
}
...
TEST(CustomAutogradTest, FunctionReturnsInput) {
  ...
  Variable x(smith::ones(1, smith::requires_grad()));
  MyFunction::apply(x).backward(smith::ones(1), true, true);
  ASSERT_VARIABLE_EQ(x.grad(), smith::full(1, 2.));
}
...
TEST(CustomAutogradTest, FunctionReturnsUndefined) {
  ...
  auto x = smith::ones(1, smith::requires_grad());
  ...
  ASSERT_FALSE(smith::autograd::grad(
...
TEST(CustomAutogradTest, MaterializeGrads) {
  struct MyFunction : public Function<MyFunction> {
    ...
    static variable_list backward(
        AutogradContext* ctx,
        variable_list grad_output) {
      EXPECT_VARIABLE_EQ(grad_output[0], smith::zeros(1));
      return grad_output;
    }
  };
...
TEST(CustomAutogradTest, NoGradCustomFunction) {
  ...
  auto x = smith::ones({5, 5}, smith::requires_grad());
  {
...
TEST(CustomAutogradTest, MarkDirty) {
  ...
  auto x = smith::randn({5, 5}, smith::requires_grad()).clone();
...
TEST(CustomAutogradTest, MarkNonDifferentiableMixed) {
  struct MyFunction : public Function<MyFunction> {
    ...
    static variable_list backward(
      ...
      EXPECT_VARIABLE_EQ(grad_a, smith::zeros({5, 5}));
      EXPECT_VARIABLE_EQ(grad_b, smith::ones({5, 5}));
      return {grad_b};
    }
  };
  ...
  out[1].sum().backward();
  ASSERT_VARIABLE_EQ(x.grad(), smith::ones({5, 5}));
}
...
TEST(CustomAutogradTest, ReturnLeafInplace) {
  ...
  Variable x = smith::randn({5, 5});
  Variable y = smith::randn({5, 5}, smith::requires_grad());

  auto out = Inplace::apply(x, y);
  auto& q = out[0];
  ASSERT_TRUE(smith::equal(q, x));
  ASSERT_TRUE(q.requires_grad());
  q.sum().backward();
  ASSERT_VARIABLE_EQ(y.grad(), smith::ones({5, 5}));
}
...
TEST(CustomAutogradTest, ReturnDuplicateInplace) {
  ...
  ASSERT_TRUE(smith::equal(out[0], out[1]));
}
...
TEST(CustomAutogradTest, InvalidGradients) {
  struct MyFunction : public Function<MyFunction> {
    ...
    static variable_list backward(
        ...
        variable_list grad_outputs) {
      return {
          smith::randn(10, smith::dtype(smith::kFloat).requires_grad(true))};
    }
  };

  auto input1 =
      smith::randn({5, 5}, smith::dtype(smith::kFloat).requires_grad(true));
  ASSERT_THROWS_WITH(
      MyFunction::apply(input1).sum().backward(), "expected shape");
  auto input2 =
      smith::randn(10, smith::dtype(smith::kDouble).requires_grad(true));
}
...
TEST(CustomAutogradTest, DepNoGrad) {
  struct F1 : public Function<F1> {
    static variable_list forward(AutogradContext* ctx, Variable input) {
      auto out = smith::randn(input.sizes());
  ...
  auto x = smith::randn(5, smith::requires_grad());
  auto out = F1::apply(x);
  ...
  auto c = F2::apply(a, b);
  c.backward(smith::ones(c.sizes()), false, false);
  ASSERT_VARIABLE_EQ(x.grad(), smith::ones(x.sizes()));
}

TEST(CustomAutogradTest, Reentrant) {
  static Variable y_data = smith::randn({2, 2});
...
TEST(CustomAutogradTest, DeepReentrant) {
  ...
  auto v =
      smith::tensor({8193}, smith::dtype(smith::kFloat).requires_grad(true));
...
TEST(CustomAutogradTest, ReentrantPriority) {
  ...
  auto a = MyFunction::apply(
      smith::tensor({6}, smith::dtype(smith::kFloat).requires_grad(true)));
  auto b = Reenter::apply(
      smith::tensor({9}, smith::dtype(smith::kFloat).requires_grad(true)));
  auto v = a * b;
  v.backward();
...
TEST(CustomAutogradTest, Hooks) {
  Variable x = smith::ones({5, 5}, smith::requires_grad());
  Variable y = smith::ones({5, 5}) * 4;
  ...
  z.backward(smith::ones({5, 5}), true, true);
  ASSERT_EQ(counter, 1);
  ...
  y.grad().zero_();
  z.backward(smith::ones({5, 5}), true, false);
  ASSERT_VARIABLE_EQ(y.grad(), (x + 1) * 2);

  y.register_hook(bw_hook_modify);
  y.grad().zero_();
  z.backward(smith::ones({5, 5}), false, false);
...
TEST(CustomAutogradTest, HooksInplace) {
  auto a = smith::ones({5, 5}, smith::requires_grad()).clone();

  int hook1_count = 0;
  auto hook1 = ([&hook1_count](Variable grad) {
    hook1_count++;
    ASSERT_VARIABLE_EQ(grad, smith::ones({5, 5}) * 2);
  });

  int hook2_count = 0;
  auto hook2 = ([&hook2_count](Variable grad) {
    hook2_count++;
    ASSERT_VARIABLE_EQ(grad, smith::ones({5, 5}));
  });
...
TEST(CustomAutogradTest, HooksInplaceWithRetainsGrad) {
  ...
  ASSERT_VARIABLE_EQ(a.grad(), smith::ones({5, 5}));
}
...
TEST(CustomAutogradTest, HooksInplaceTwiceWithRetainsGrad) {
  ...
  auto hook1 = ([&hook1_count](Variable grad) {
    hook1_count++;
    ASSERT_VARIABLE_EQ(grad, smith::ones({5, 5}) * 4);
  });
...
TEST(CustomAutogradTest, BackwardWithInputs) {
  ...
  z.backward(smith::ones({5, 5}), false, false, {x});
...
TEST(CustomAutogradTest, BackwardWithEmptyInputs) {
  ...
  Variable y_grad_expected = x + 2 * y;
  ASSERT_THROWS_WITH(
      z.backward(smith::ones({5, 5}), false, false, std::vector<Variable>{}),
      "cannot be empty");
}
...
TEST(CustomAutogradTest, BackwardWithNonLeafInputs) {
  ...
  w.backward(smith::ones({5, 5}), false, false, std::vector<Variable>{x, z});
...
TEST(CustomAutogradTest, BackwardWithCreateGraphWarns) {
  ...
  smith::Tensor x = smith::randn({5, 5}).set_requires_grad(true);
  auto z = x * x;
  {
    WarningCapture warnings;
    z.backward(smith::ones({5, 5}), std::nullopt, true);
    ASSERT_TRUE(
  ...
  {
    WarningCapture warnings;
    smith::autograd::backward({z}, {smith::ones({5, 5})}, std::nullopt, true);
    ASSERT_TRUE(
        warnings.str().find("Using backward() with create_graph=True") !=
        std::string::npos);
  }
...
smith::Tensor inplace_op(
    const smith::Tensor& self,
    const smith::Tensor& other) {
  return self.add_(other);
}

std::tuple<smith::Tensor, smith::Tensor> two_arg_inplace_op(
    const smith::Tensor& self,
    const smith::Tensor& other) {
  other.add_(self);
  self.add_(other);
  return std::tuple<smith::Tensor, smith::Tensor>(self, other);
}

std::tuple<smith::Tensor, smith::Tensor> two_pairs_of_view_op(
    const smith::Tensor& self,
    const smith::Tensor& other) {
...
std::tuple<smith::Tensor, smith::Tensor> non_first_view_op(
    const smith::Tensor& self,
    const smith::Tensor& other) {
  // This is not allowed. We test below that this calling into the boxed kernel
  // will raise an error
  return std::tuple<smith::Tensor, smith::Tensor>(self.clone(), other);
}
...
smith::Tensor my_custom_op(
    const smith::Tensor& self,
    const smith::Tensor& other) {
  return self + other;
}

std::tuple<smith::Tensor, smith::Tensor, int64_t> ret_tuple_non_tensor(
    const smith::Tensor& self,
    const smith::Tensor& other) {
  auto a = self - other;
  auto b = self + other;
  return std::tuple<smith::Tensor, smith::Tensor, int64_t>(a, b, 12);
}

smith::Tensor view_op(const smith::Tensor& self) {
  return self.alias();
}

std::vector<smith::Tensor> ret_tensor_vector_view(
    const smith::Tensor& self,
    const smith::Tensor& other) {
...
smith::Tensor tensorlist_op(const smith::Tensor& self, at::TensorList other) {
...
#define REGISTER_TEST_OP(name, schema, fn)                                 \
  auto m = MAKE_SMITH_LIBRARY(_test);                                      \
  m.def(schema);                                                           \
  auto m_autograd = MAKE_SMITH_LIBRARY_IMPL(_test, Autograd);              \
  auto m_cpu = MAKE_SMITH_LIBRARY_IMPL(_test, CPU);                        \
  auto m_inplaceorview = MAKE_SMITH_LIBRARY_IMPL(_test, ADInplaceOrView);  \
  m_cpu.impl(name, c10::DispatchKey::CPU, SMITH_FN(fn));                   \
  m_autograd.impl(                                                         \
...
void assertBasicChecks(F op) {
  auto a = smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true);
  auto b = smith::tensor({1.}, {smith::kFloat32});
  auto c = smith::tensor({1.}, {smith::kFloat32});
...
TEST(TestAutogradNotImplementedFallback, RetSingleNonTensor) {
  ...
  auto op = [&](const smith::Tensor& _1, const smith::Tensor& _2) {
    return callOpUnboxed<int64_t, const smith::Tensor&, const smith::Tensor&>(
        opHandle, _1, _2);
  };
...
TEST(TestAutogradNotImplementedFallback, InplaceOp) {
  ...
  auto op = [&](const smith::Tensor& _1, const smith::Tensor& _2) {
    return callOpUnboxed<
        smith::Tensor,
        const smith::Tensor&,
        const smith::Tensor&>(opHandle, _1, _2);
  };
  ...
  b = b.clone();
  auto c = op(a, b);
  ASSERT_TRUE(smith::allclose(c, inplace_op(a, b)));

  // Test in-place on view
  auto base =
      smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true).clone();
  auto view = base.view(-1);
  auto t = smith::tensor({1.}, {smith::kFloat32});

  smith::Tensor v_nograd;
  {
...
TEST(TestAutogradNotImplementedFallback, DoubleInplaceOp) {
  ...
  auto op = [&](const smith::Tensor& _1, const smith::Tensor& _2) {
    return callOpUnboxed<
        std::tuple<smith::Tensor, smith::Tensor>,
        const smith::Tensor&,
        const smith::Tensor&>(opHandle, _1, _2);
  };
  auto a = smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true);
...
TEST(TestAutogradNotImplementedFallback, OptOp) {
  ...
  auto op = [&](const smith::Tensor& _1,
                const std::optional<smith::Tensor>& _2) {
    return callOpUnboxed<
        smith::Tensor,
        const smith::Tensor&,
        const std::optional<smith::Tensor>&>(opHandle, _1, _2);
  };

  auto a = smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true);
  auto b = smith::tensor({1.}, {smith::kFloat32});

  ASSERT_TRUE(smith::allclose(op(a, b), opt_op(a, b)));
  ASSERT_TRUE(smith::allclose(op(a, {}), opt_op(a, {})));
}
...
TEST(TestAutogradNotImplementedFallback, RetTupleNonTensor) {
  ...
  auto op = [&](const smith::Tensor& _1, const smith::Tensor& _2) {
    auto out = callOpUnboxed<
        std::tuple<smith::Tensor, smith::Tensor, int64_t>,
        const smith::Tensor&,
        const smith::Tensor&>(opHandle, _1, _2);
...
TEST(TestAutogradNotImplementedFallback, ViewOp) {
  ...
  auto op = [&](const smith::Tensor& _1) {
    return callOpUnboxed<smith::Tensor, const smith::Tensor&>(opHandle, _1);
  };
  auto b = smith::tensor({1.}, {smith::kFloat32});
  auto v = op(b);
  ...
  auto t = smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true);
...
TEST(TestAutogradNotImplementedFallback, RetTensorVectorView) {
  ...
  auto a = smith::tensor({1.}, {smith::kFloat32});
  auto b = smith::tensor({1.}, {smith::kFloat32});
  auto out = op(a, b);
...
TEST(TestAutogradNotImplementedFallback, RetTensorVector) {
  ...
  auto op = [&](const smith::Tensor& _1, const smith::Tensor& _2) {
    return callOpUnboxed<
        std::vector<at::Tensor>,
        const smith::Tensor&,
        const smith::Tensor&>(opHandle, _1, _2)[0];
  };
  assertBasicChecks(op);
}
...
TEST(TestAutogradNotImplementedFallback, TensorlistOp) {
  ...
  auto opHandle = c10::Dispatcher::singleton().findSchemaOrThrow(
      "_test::tensorlist_op", "");
  auto op = [&](smith::Tensor _1, at::TensorList _2) {
    return callOpUnboxed<smith::Tensor, const smith::Tensor&, at::TensorList>(
        opHandle, _1, _2);
  };

  auto a = smith::tensor({1.}, {smith::kFloat32});
  auto b = smith::tensor({1.}, {smith::kFloat32});
  auto c = smith::tensor({1.}, {smith::kFloat32}).set_requires_grad(true);
  std::vector<smith::Tensor> vec = {b, c};
  auto out = op(a, vec);

  ASSERT_THROWS_WITH(
      smith::autograd::grad({out}, {vec[0]}),
      "element 0 of the input tensors does not require grad");
  ASSERT_THROWS_WITH(
      smith::autograd::grad({out}, {vec[1]}), "is not implemented");
...
TEST(TestAutogradUtils, ValidateOutputsReduce) {
  auto input = smith::ones({}, {smith::kFloat32});
  auto grad = smith::ones({2, 3}, {smith::kFloat32});
  ...
  smith::autograd::validate_outputs(input_metadata, grads, test_format_error);
  ASSERT_TRUE(at::allclose(grads[0], grad.sum()));
}

TEST(TestAutogradUtils, ValidateOutputsBasic) {
  auto input = smith::zeros({2, 3}, {smith::kFloat32});
  auto grad = smith::ones({2, 3}, {smith::kFloat32});
  ...
  smith::autograd::validate_outputs(input_metadata, grads, test_format_error);
  ASSERT_TRUE(at::allclose(grad, smith::ones({2, 3})));
}
