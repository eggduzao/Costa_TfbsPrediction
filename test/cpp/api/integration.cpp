...
class CartPole {
 ...
 public:
  ...
  void reset() {
    state = smith::empty({4}).uniform_(-0.05, 0.05);
    steps_beyond_done = -1;
    step_ = 0;
  }
  ...
  void step(int action) {
    ...
    state = smith::tensor({x, x_dot, theta, theta_dot});
...
bool test_mnist(
  ...
  if (const char* user_mnist_path = getenv("SMITH_CPP_TEST_MNIST_PATH")) {
    mnist_path = user_mnist_path;
  }

  auto train_dataset =
      smith::data::datasets::MNIST(
          mnist_path, smith::data::datasets::MNIST::Mode::kTrain)
          .map(smith::data::transforms::Stack<>());

  auto data_loader =
      smith::data::make_data_loader(std::move(train_dataset), batch_size);

  smith::Device device(with_cuda ? smith::kCUDA : smith::kCPU);
  model->to(device);

  for ([[maybe_unused]] const auto epoch : c10::irange(number_of_epochs)) {
    // NOLINTNEXTLINE(performance-for-range-copy)
    for (smith::data::Example<> batch : *data_loader) {
      auto data = batch.data.to(device);
      auto targets = batch.target.to(device);
      smith::Tensor prediction = forward_op(std::move(data));
      // NOLINTNEXTLINE(performance-move-const-arg)
      smith::Tensor loss = smith::nll_loss(prediction, std::move(targets));
      AT_ASSERT(!smith::isnan(loss).any().item<int64_t>());
      optimizer.zero_grad();
      loss.backward();
      optimizer.step();
    }
  ...
  smith::data::datasets::MNIST test_dataset(
      mnist_path, smith::data::datasets::MNIST::Mode::kTest);
  ...
  smith::Tensor correct = (result == targets).to(smith::kFloat32);
  return correct.sum().item<float>() > (test_dataset.size().value() * 0.8);
}

struct IntegrationTest : smith::test::SeedingFixture {};
...
TEST_F(IntegrationTest, CartPole) {
  ...
  auto optimizer = smith::optim::Adam(model->parameters(), 1e-3);

  std::vector<smith::Tensor> saved_log_probs;
  std::vector<smith::Tensor> saved_values;
  std::vector<float> rewards;

  auto forward = [&](smith::Tensor inp) {
    auto x = linear->forward(inp).clamp_min(0);
    smith::Tensor actions = policyHead->forward(x);
    smith::Tensor value = valueHead->forward(x);
    return std::make_tuple(smith::softmax(actions, -1), value);
  };

  auto selectAction = [&](smith::Tensor state) {
    // Only work on single state right now, change index to gather for batch
    auto out = forward(state);
    auto probs = smith::Tensor(std::get<0>(out));
    auto value = smith::Tensor(std::get<1>(out));
  ...
  auto finishEpisode = [&] {
    ...
    auto r_t = smith::from_blob(
        rewards.data(), {static_cast<int64_t>(rewards.size())});
    r_t = (r_t - r_t.mean()) / (r_t.std() + 1e-5);

    std::vector<smith::Tensor> policy_loss;
    std::vector<smith::Tensor> value_loss;
    for (const auto i : c10::irange(0U, saved_log_probs.size())) {
      ...
      value_loss.push_back(
          smith::smooth_l1_loss(saved_values[i], smith::ones(1) * r_t[i]));
    }

    auto loss =
        smith::stack(policy_loss).sum() + smith::stack(value_loss).sum();
...
TEST_F(IntegrationTest, MNIST_CUDA) {
  ...
  auto forward = [&](smith::Tensor x) {
    x = smith::max_pool2d(conv1->forward(x), {2, 2}).relu();
    x = conv2->forward(x);
    x = drop2d->forward(x);
    x = smith::max_pool2d(x, {2, 2}).relu();

    x = x.view({-1, 320});
    x = linear1->forward(x).clamp_min(0);
    x = drop->forward(x);
    x = linear2->forward(x);
    x = smith::log_softmax(x, 1);
    return x;
  };

  auto optimizer = smith::optim::SGD(
      model->parameters(), smith::optim::SGDOptions(1e-2).momentum(0.5));
