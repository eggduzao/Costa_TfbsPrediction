...
TEST(SmithScriptTest, CanCompileMultipleFunctions) {
  auto module = smith::jit::compile(R"JIT(
      def test_mul(a, b):
        return a * b
      def test_relu(a, b):
...
TEST(SmithScriptTest, TestNestedIValueModuleArgMatching) {
  auto module = smith::jit::compile(R"JIT(
      def nested_loop(a: List[List[Tensor]], b: int):
        return smith.tensor(1.0) + b
    )JIT");

  auto b = 3;

  smith::List<smith::Tensor> list({smith::rand({4, 4})});

  smith::List<smith::List<smith::Tensor>> list_of_lists;
...
TEST(SmithScriptTest, TestOptionalArgMatching) {
  auto module = smith::jit::compile(R"JIT(
  ...
  ASSERT_EQ(
      0, module->run_method("optional_tuple_op", smith::jit::IValue()).toInt());
}

TEST(SmithScriptTest, TestPickle) {
  ...
  smith::IValue ivalue = smith::jit::unpickle(data.data(), data.size());
