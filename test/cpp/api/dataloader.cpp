...
using namespace smith::data; // NOLINT
...
TEST(DataTest, NoSequencerIsIdentity) {
  using namespace smith::data::detail::sequencers; // NOLINT
...
TEST(DataTest, TensorDatasetConstructsFromSingleTensor) {
  datasets::TensorDataset dataset(smith::eye(5));
  ASSERT_TRUE(
      smith::tensor({0, 0, 1, 0, 0}, smith::kFloat32).allclose(dataset.get(2)));
}

TEST(DataTest, TensorDatasetConstructsFromInitializerListOfTensors) {
  std::vector<smith::Tensor> vector = smith::eye(5).chunk(5);
...
TEST(DataTest, StackTransformWorksForExample) {
  struct D : public datasets::Dataset<D> {
    ...
    smith::Tensor tensor{smith::eye(4)};
  };

  auto d = D().map(transforms::Stack<Example<>>());

  Example<> batch = d.get_batch({0, 1});
  ASSERT_TRUE(batch.data.allclose(smith::eye(4).slice(/*dim=*/0, 0, 2)));
  ASSERT_TRUE(batch.target.allclose(1 + smith::eye(4).slice(/*dim=*/0, 0, 2)));

  Example<> second = d.get_batch({2, 3});
  ASSERT_TRUE(second.data.allclose(smith::eye(4).slice(/*dim=*/0, 2, 4)));
  ASSERT_TRUE(second.target.allclose(1 + smith::eye(4).slice(/*dim=*/0, 2, 4)));
}

TEST(DataTest, StackTransformWorksForTensorExample) {
  auto d = datasets::TensorDataset(smith::eye(4))
...
struct T : transforms::TensorTransform<Target> {
  smith::Tensor operator()(smith::Tensor input) override {
    return input * 2;
  }
};
...
struct TensorStringDataset
    : datasets::
          Dataset<TensorStringDataset, Example<smith::Tensor, std::string>> {
  Example<smith::Tensor, std::string> get(size_t index) override {
    return {smith::tensor(static_cast<double>(index)), std::to_string(index)};
  }
...
TEST(DataTest, TensorTransformWorksForAnyTargetType) {
  ...
  std::vector<Example<smith::Tensor, std::string>> batch = d.get_batch({1, 2});

  ASSERT_EQ(batch.size(), 2);
  ASSERT_TRUE(batch[0].data.allclose(smith::tensor(2.0)));
  ASSERT_EQ(batch[0].target, "1");

  ASSERT_TRUE(batch[1].data.allclose(smith::tensor(4.0)));
...
TEST(DataTest, TensorLambdaWorksforAnyTargetType) {
  auto d = TensorStringDataset().map(transforms::TensorLambda<std::string>(
      [](smith::Tensor input) { return input * 2; }));
  std::vector<Example<smith::Tensor, std::string>> batch = d.get_batch({1, 2});
...
struct DummyTensorDataset
    : datasets::Dataset<DummyTensorDataset, Example<smith::Tensor, int>> {
  Example<smith::Tensor, int> get(size_t index) override {
    const auto channels = static_cast<int64_t>(index);
    smith::Tensor tensor =
        (channels > 0) ? smith::ones({channels, 4, 4}) : smith::ones({4, 4});
...
TEST(DataTest, NormalizeTransform) {
  ...
  std::vector<Example<smith::Tensor, int>> output = dataset.get_batch(0);
  ASSERT_EQ(output.size(), 1);
  // (1 - 0.5) / 0.1 = 5
  ASSERT_TRUE(output[0].data.allclose(smith::ones({4, 4}) * 5))
      << output[0].data;
  ...
  ASSERT_TRUE(output[0].data.allclose(smith::ones({1, 4, 4}) * 5))
      << output[0].data;
  ...
  ASSERT_TRUE(output[0]
                  ...
                  .allclose(smith::ones({1, 4, 4}) * 5))
      << output[0].data;
  ASSERT_TRUE(output[0]
                  .data.slice(/*dim=*/0, /*start=*/1)
                  .allclose(smith::ones({1, 4, 4}) * -2.5))
      << output[0].data;
  ...
  ASSERT_TRUE(output[0].data.allclose(smith::ones({3, 4, 4}) * -2.5))
      << output[0].data;
  ...
  ASSERT_TRUE(output[0]
                  ...
                  .allclose(smith::ones({1, 4, 4}) * 12.5))
      << output[0].data;
}
...
struct UnCopyableDataset : public datasets::Dataset<UnCopyableDataset> {
  ...
  Example<> get(size_t index) override {
    return {
        smith::tensor({static_cast<int64_t>(index)}),
        smith::tensor({static_cast<int64_t>(index)})};
  }
...
TEST(DataTest, MapDoesNotCopy) {
  auto dataset = UnCopyableDataset()
                     .map(transforms::TensorLambda<>(
                         [](smith::Tensor tensor) { return tensor + 1; }))
                     .map(transforms::TensorLambda<>(
                         [](smith::Tensor tensor) { return tensor + 2; }))
                     .map(transforms::TensorLambda<>(
                         [](smith::Tensor tensor) { return tensor + 3; }));
...
TEST(DataTest, QueuePushAndPopFromSameThread) {
  smith::data::detail::Queue<int> queue;
  queue.push(1);
  queue.push(2);
...
TEST(DataTest, DataShuttleCanPushAndPopJob) {
  smith::data::detail::DataShuttle<int, int> shuttle;
  shuttle.push_job(1);
  shuttle.push_job(2);
...
TEST(DataTest, SharedBatchDatasetReallyIsShared) {
  ...
  auto shared_dataset =
      smith::data::datasets::make_shared_dataset<UncopyableDataset>(
          "uncopyable");

  auto data_loader = smith::data::make_data_loader(
      shared_dataset, smith::data::DataLoaderOptions().workers(3));
...
struct TestIndex : public smith::data::samplers::CustomBatchRequest {
...
struct TestIndexSampler : public samplers::Sampler<TestIndex> {
  ...
  void save(smith::serialize::OutputArchive& archive) const override {}
  void load(smith::serialize::InputArchive& archive) override {}
  size_t index_ = 0;
  size_t size_;
};
...
struct UnsizedDataset : public datasets::Dataset<UnsizedDataset> {
  smith::data::Example<> get(size_t i) override {
    return {smith::ones(i), smith::ones(i)};
  }
...
TEST(DataLoaderTest, IteratorsCompareEqualToSentinelWhenExhausted) {
  ...
  auto data_loader =
      smith::data::make_data_loader(dataset, dataset.size().value() / 4);
  auto i = data_loader->begin();
  auto end = data_loader->end();
  ASSERT_NE(i, end);
  ++i;
...
TEST(DataLoaderTest, IteratorsShareState) {
  DummyDataset dataset;
  auto data_loader =
      smith::data::make_data_loader(dataset, dataset.size().value() / 2);
  auto i = data_loader->begin();
  auto j = i;
  auto end = data_loader->end();
  ASSERT_NE(i, end);
...
TEST(DataLoaderTest, CanDereferenceIteratorMultipleTimes) {
  DummyDataset dataset;
  auto data_loader =
      smith::data::make_data_loader<smith::data::samplers::SequentialSampler>(
          dataset,
...
TEST(DataLoaderTest, CanUseIteratorAlgorithms) {
  struct D : datasets::BatchDataset<D, int> {
    int get_batch(smith::ArrayRef<size_t> indices) override {
      // NOLINTNEXTLINE(cppcoreguidelines-narrowing-conversions,bugprone-narrowing-conversions)
...
TEST(DataLoaderTest, CallingBeginWhileOtherIteratorIsInFlightThrows) {
  DummyDataset dataset;
  auto data_loader =
      smith::data::make_data_loader(dataset, DataLoaderOptions(1).workers(2));
  auto i = data_loader->begin();
  ASSERT_THROWS_WITH(
      data_loader->begin(),
...
TEST(DataLoaderTest, IncrementingExhaustedValidIteratorThrows) {
  DummyDataset dataset;
  auto data_loader =
      smith::data::make_data_loader(dataset, dataset.size().value());
  auto i = data_loader->begin();
...
TEST(DataLoaderTest, YieldsCorrectBatchSize) {
  DummyDataset dataset;
  auto data_loader = smith::data::make_data_loader(dataset, 25);
  auto iterator = data_loader->begin();
...
struct Dataset : datasets::BatchDataset<Dataset, size_t> {
  ...
  size_t get_batch(smith::ArrayRef<size_t> indices) override {
...
TEST(DataLoaderTest, EnforcesOrderingAmongThreadsWhenConfigured) {
  auto data_loader = smith::data::make_data_loader(
      ...
      smith::data::samplers::SequentialSampler(ordering_test::kNumberOfWorkers),
      DataLoaderOptions()
          .batch_size(1)
...
TEST(DataLoaderTest, TestExceptionsArePropagatedFromWorkers) {
  ...
  try {
    (void)*iterator;
  } catch (smith::data::WorkerException& e) {
    ASSERT_EQ(
        e.what(),
...
TEST(DataLoaderTest, StatefulDatasetWithNoWorkers) {
  ...
  auto data_loader = smith::data::make_data_loader(D{});
...
TEST(DataLoaderTest, StatefulDatasetWithManyWorkers) {
  ...
  auto data_loader = smith::data::make_data_loader(
      smith::data::datasets::make_shared_dataset<D>(),
...
TEST(DataLoaderTest, StatefulDatasetWithMap) {
  ...
  auto data_loader = smith::data::make_data_loader(
      D().map(transforms::BatchLambda<int, std::string>(
          ...
          .map(transforms::BatchLambda<std::string, smith::Tensor>(
              [](const std::string& x) {
                return smith::tensor(static_cast<int64_t>(std::stoi(x)));
              })),
      DataLoaderOptions{});
  ...
  for (const smith::Tensor& t : *data_loader) {
...
TEST(DataLoaderTest, StatefulDatasetWithCollate) {
    ...
    std::optional<std::vector<Example<>>> get_batch(
      ...
      if (counter < kNumberOfExamplesAfterWhichTheDatasetExhausts) {
        ...
        std::vector<Example<>> batch(
            /*count=*/batch_size,
            Example<>{
                smith::ones(batch_size + 1), smith::zeros(batch_size - 1)});
        return batch;
      }
      return std::nullopt;
    }
  ...
  ASSERT_TRUE(batch->data[0].allclose(smith::ones(kBatchSize + 1)));
  ASSERT_TRUE(batch->target[0].allclose(smith::zeros(kBatchSize - 1)));
}
...
TEST(DataLoaderTest, ChunkDatasetLoad) {
  ...
  // Configure sampler to skip 2 chunks
  {
    ...
    // See Note [save/load ChunkDataset as ChunkSampler]
    smith::save(sampler, tempfile.name);
  }
  ...
  smith::load(*dataset, tempfile.name);
