...
TEST(CuDNNBatchNormTest, OutVariantMatchesFunctional) {
  ...
  auto device = smith::device(smith::kCUDA);

  auto input = smith::rand({2, 3, 4, 4}, device);
  auto weight = smith::randn({3}, device);
  auto bias = smith::randn({3}, device);
  auto running_mean = smith::zeros({3}, device);
  auto running_var = smith::ones({3}, device);

  bool training = true;
  double exponential_average_factor = 0.1;
  double epsilon = 1e-5;

  auto output = smith::empty_like(input);
  auto save_mean = smith::empty({3}, device);
  auto save_var = smith::empty({3}, device);
  auto reserve = smith::empty({0}, device.dtype(smith::kByte));
  ...
  ASSERT_TRUE(smith::allclose(output, std::get<0>(ref_outputs)));
  ASSERT_TRUE(smith::allclose(save_mean, std::get<1>(ref_outputs)));
  ASSERT_TRUE(smith::allclose(save_var, std::get<2>(ref_outputs)));
  ASSERT_TRUE(smith::equal(reserve, std::get<3>(ref_outputs)));
}
