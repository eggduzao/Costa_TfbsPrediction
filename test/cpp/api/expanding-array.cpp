...
struct ExpandingArrayTest : smith::test::SeedingFixture {};

TEST_F(ExpandingArrayTest, CanConstructFromInitializerList) {
  smith::ExpandingArray<5> e({1, 2, 3, 4, 5});
  ASSERT_EQ(e.size(), 5);
...
TEST_F(ExpandingArrayTest, CanConstructFromVector) {
  smith::ExpandingArray<5> e(std::vector<int64_t>{1, 2, 3, 4, 5});
  ASSERT_EQ(e.size(), 5);
  for (const auto i : c10::irange(e.size())) {
    ASSERT_EQ((*e)[i], i + 1);
  }
...
TEST_F(ExpandingArrayTest, CanConstructFromArray) {
  smith::ExpandingArray<5> e(std::array<int64_t, 5>({1, 2, 3, 4, 5}));
  ASSERT_EQ(e.size(), 5);
  for (const auto i : c10::irange(e.size())) {
    ASSERT_EQ((*e)[i], i + 1);
  }
...
TEST_F(
  ...
  ASSERT_THROWS_WITH(
      smith::ExpandingArray<5>({1, 2, 3, 4, 5, 6, 7}),
...
TEST_F(
  ...
  ASSERT_THROWS_WITH(
      smith::ExpandingArray<5>(std::vector<int64_t>({1, 2, 3, 4, 5, 6, 7})),
      "Expected 5 values, but instead got 7");
}
