...
struct ModuleDictTest : smith::test::SeedingFixture {};
...
TEST_F(ModuleDictTest, ConstructsFromordereddict) {
  ...
  smith::OrderedDict<std::string, std::shared_ptr<Module>> ordereddict = {
      {"module_1", std::make_shared<M>(1)},
      {"module_2", std::make_shared<M>(2)},
      {"module_3", std::make_shared<M>(3)},
  };
...
void iscloneable_helper(smith::Device device) {
  smith::OrderedDict<std::string, std::shared_ptr<Module>> ordereddict = {
      {"linear", Linear(2, 3).ptr()},
      {"relu", Functional(smith::relu).ptr()},
...
TEST_F(ModuleDictTest, IsCloneable) {
  iscloneable_helper(smith::kCPU);
}

TEST_F(ModuleDictTest, IsCloneable_CUDA) {
  iscloneable_helper({smith::kCUDA, 0});
}

TEST_F(ModuleDictTest, RegistersElementsAsSubmodules) {
  smith::OrderedDict<std::string, std::shared_ptr<Module>> ordereddict1 = {
      {"linear", Linear(10, 3).ptr()},
      {"conv", Conv2d(1, 2, 3).ptr()},
  ...
  // Update Existing
  smith::OrderedDict<std::string, std::shared_ptr<Module>> ordereddict2 = {
...
TEST_F(ModuleDictTest, PrettyPrintModuleDict) {
  ...
  ASSERT_EQ(
      ...
      "  (conv): smith::nn::Conv2d(1, 2, kernel_size=[3, 3], stride=[1, 1])\n"
      "  (dropout): smith::nn::Dropout(p=0.5, inplace=false)\n"
      "  (batch): smith::nn::BatchNorm2d(5, eps=1e-05, momentum=0.1, affine=true, track_running_stats=true)\n"
...
TEST_F(ModuleDictTest, InvalidAt) {
  ...
  ModuleDict dict(ordereddict);
  ASSERT_THROWS_WITH(
      dict->at<smith::nn::Dropout2dImpl>("linear"), "Unable to cast module");
}
