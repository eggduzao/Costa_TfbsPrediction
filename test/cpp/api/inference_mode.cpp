...
namespace {
smith::Tensor functional_op(smith::Tensor& x) {
  return x * x;
}

void inplace_op(smith::Tensor& x) {
  x.mul_(1);
}

smith::Tensor view_op(smith::Tensor& x) {
  return x.view({2, 3});
}
...
TEST(InferenceModeTest, TestExistingAutogradSession) {
  smith::Tensor s = smith::ones({1, 2, 3}).set_requires_grad(true);
  smith::Tensor a = s.clone();

  // Save `a` in an existing autograd session
  smith::Tensor out = a * a;
  {
    InferenceMode guard;
    inplace_op(a);
  }
...
TEST(InferenceModeTest, TestInferenceTensorInInferenceModeFunctionalOp) {
  ...
  for (bool requires_grad : {true, false}) {
    ...
    smith::Tensor func_out = functional_op(c); // go through kernels: CPU
...
TEST(InferenceModeTest, TestInferenceTensorInInferenceModeViewOp) {
  ...
  for (bool requires_grad : {true, false}) {
    ...
    smith::Tensor view_out = view_op(c); // go through kernels: CPU
...
TEST(InferenceModeTest, TestInferenceTensorInNormalModeFunctionalOp) {
  smith::Tensor inference_tensor;
  for (bool requires_grad : {true, false}) {
    {
      InferenceMode guard;
      inference_tensor =
          smith::ones({1, 2, 3}).set_requires_grad(requires_grad);
    }
...
TEST(InferenceModeTest, TestNormalTensorInplaceOutputInNormalMode) {
  for (bool requires_grad : {true, false}) {
    ...
    smith::Tensor tmp = functional_op(a); // go through kernels: VariableType,
                                          // ADInplaceOrView(fallthrough), CPU
    ASSERT_FALSE(tmp.is_inference());
...
TEST(InferenceModeTest, TestMixInferenceAndNormalTensorFunctionalOp) {
  for (bool requires_grad : {true, false}) {
    ...
    {
      InferenceMode guard;
      c = smith::ones({1, 2, 3}).set_requires_grad(requires_grad);
    }

    // add(Tensor, Tensor) is safe with inference tensor since it doesn't save
    // any variable for backward.
    smith::Tensor out = c.add(s); // go through kernels: VariableType,
                                  // ADInplaceOrView(fallthrough), CPU
    ...
    if (requires_grad) {
      ...
      out.backward(smith::ones_like(out));
      assert_tensor_equal(c.grad(), smith::ones_like(c));
    }
    ...
    if (requires_grad) {
      ...
      /*
      std::vector<smith::Tensor> inputs = {s, c};
      ASSERT_THROWS_WITH(
          smith::stack(inputs), // go through kernels: VariableType(ERROR)!,
                                // ADInplaceOrView(fallthrough), CPU
...
TEST(InferenceModeTest, TestMixInferenceAndNormalTensorInplaceOp) {
  for (bool requires_grad : {true, false}) {
    ...
    if (requires_grad) {
      ...
      ASSERT_THROWS_WITH(
          smith::mul_out(/*out=*/c, s, s), // go through kernels: VariableType,
...
TEST(InferenceModeTest, TestCreationMetaPropagationInput) {
  smith::Tensor s = smith::ones({2, 2, 3}).set_requires_grad(true);
...
TEST(InferenceModeTest, TestInplaceUpdateInferenceTensorWithNormalTensor) {
  smith::Tensor s = smith::ones({1, 2, 3});
  smith::Tensor t;
  {
    InferenceMode guard;
    t = smith::ones({1, 2, 3});
...
TEST(InferenceModeTest, TestComplexViewInInferenceMode) {
  smith::Tensor s = smith::ones({3, 3, 2});
  smith::Tensor t = smith::view_as_complex(s);
  {
    InferenceMode guard;
    smith::Tensor tmp;

    tmp = smith::view_as_real(t);
    ASSERT_FALSE(tmp.is_inference());
    tmp = smith::view_as_complex(s);
    ASSERT_FALSE(tmp.is_inference());

    smith::Tensor e = smith::ones({3, 3, 2});
    tmp = smith::view_as_complex(e);
    ASSERT_TRUE(tmp.is_inference());
    tmp = smith::view_as_real(tmp);
    ASSERT_TRUE(tmp.is_inference());
  }
}
...
TEST(InferenceModeTest, TestComplexViewInNormalMode) {
  ...
  smith::Tensor tmp = smith::view_as_complex(s);
  ASSERT_TRUE(tmp.is_inference());
  tmp = smith::view_as_real(tmp);
  ASSERT_TRUE(tmp.is_inference());
}
...
TEST(InferenceModeTest, TestCustomFunction) {
  ...
  {
    InferenceMode guard;
    smith::Tensor var1 = smith::ones({3, 3}).set_requires_grad(true);
    ...
    smith::Tensor expected = var1 + mul * var2 + var1 * var2;
