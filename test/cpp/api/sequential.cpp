...
struct SequentialTest : smith::test::SeedingFixture {};
...
TEST_F(SequentialTest, ConstructsFromConcreteType) {
  ...
  struct M : smith::nn::Module {
    ...
    M(const M& other) : smith::nn::Module(other) {
      copy_count++;
    }
    int value;
    int forward() {
...
TEST_F(SequentialTest, PushBackAddsAnElement) {
  ...
  auto a = smith::nn::AnyModule(smith::nn::Linear(1, 2));
...
TEST_F(SequentialTest, CallingForwardChainsCorrectly) {
  struct MockModule : smith::nn::Module {
...
TEST_F(SequentialTest, CallingForwardWithTheWrongReturnTypeThrows) {
  struct M : public smith::nn::Module {
    int forward() {
      return 5;
    }
  };
...
TEST_F(SequentialTest, TheReturnTypeOfForwardDefaultsToTensor) {
  struct M : public smith::nn::Module {
    smith::Tensor forward(smith::Tensor v) {
      return v;
    }
  };

  Sequential sequential(M{});
  auto variable = smith::ones({3, 3}, smith::requires_grad());
...
TEST_F(SequentialTest, IsCloneable) {
  Sequential sequential(Linear(3, 4), Functional(smith::relu), BatchNorm1d(3));
  Sequential clone =
...
TEST_F(SequentialTest, PrettyPrintSequential) {
  ...
  ASSERT_EQ(
      c10::str(sequential),
      "smith::nn::Sequential(\n"
      "  (0): smith::nn::Linear(in_features=10, out_features=3, bias=true)\n"
      "  (1): smith::nn::Conv2d(1, 2, kernel_size=[3, 3], stride=[1, 1])\n"
      "  (2): smith::nn::Dropout(p=0.5, inplace=false)\n"
      "  (3): smith::nn::BatchNorm2d(5, eps=1e-05, momentum=0.1, affine=true, track_running_stats=true)\n"
      "  (4): smith::nn::Embedding(num_embeddings=4, embedding_dim=10)\n"
      "  (5): smith::nn::LSTM(input_size=4, hidden_size=5, num_layers=1, bias=true, batch_first=false, dropout=0, bidirectional=false)\n"
      ")");
  ...
  ASSERT_EQ(
      ...
      "smith::nn::Sequential(\n"
      "  (linear): smith::nn::Linear(in_features=10, out_features=3, bias=true)\n"
      "  (conv2d): smith::nn::Conv2d(1, 2, kernel_size=[3, 3], stride=[1, 1])\n"
      "  (dropout): smith::nn::Dropout(p=0.5, inplace=false)\n"
      "  (batchnorm2d): smith::nn::BatchNorm2d(5, eps=1e-05, momentum=0.1, affine=true, track_running_stats=true)\n"
      "  (embedding): smith::nn::Embedding(num_embeddings=4, embedding_dim=10)\n"
      "  (lstm): smith::nn::LSTM(input_size=4, hidden_size=5, num_layers=1, bias=true, batch_first=false, dropout=0, bidirectional=false)\n"
      ")");
}
...
TEST_F(SequentialTest, ModuleForwardMethodOptionalArg) {
  {
    ...
    std::dynamic_pointer_cast<ConvTranspose1dImpl>(sequential[1])
        ->weight.set_data(smith::arange(18.).reshape({3, 2, 3}));
    auto x = smith::arange(30.).reshape({2, 3, 5});
    auto y = sequential->forward(x);
  ...
  }
  {
    ...
    std::dynamic_pointer_cast<ConvTranspose2dImpl>(sequential[1])
        ->weight.set_data(smith::arange(54.).reshape({3, 2, 3, 3}));
    auto x = smith::arange(75.).reshape({1, 3, 5, 5});
    auto y = sequential->forward(x);
    auto expected = smith::tensor(
  ...
  }
  {
    ...
    std::dynamic_pointer_cast<ConvTranspose3dImpl>(sequential[1])
        ->weight.set_data(smith::arange(32.).reshape({2, 2, 2, 2, 2}));
  ...
  }
  {
    ...
    auto x = smith::tensor({{1, 0}}, smith::kLong);
    auto y = sequential->forward(x);
    auto expected = smith::tensor({2.5000, 3.7000, 4.6500});
    ASSERT_TRUE(smith::allclose(y, expected));
  }
  {
    smith::manual_seed(0);
    ...
    auto query = smith::ones({batch_size, tgt_len, embed_dim});
    auto key = smith::ones({batch_size, src_len, embed_dim});
    // NOLINTNEXTLINE(performance-unnecessary-copy-initialization)
    auto value = key;

    Sequential sequential(MultiheadAttention(embed_dim, num_heads));
    auto output = sequential->forward<std::tuple<smith::Tensor, smith::Tensor>>(
        query.transpose(0, 1), key.transpose(0, 1), value.transpose(0, 1));

    auto attn_output = std::get<0>(output);
    auto attn_output_expected = smith::tensor(
        {{{0.0674, -0.0056, 0.1324, 0.0922, 0.0160, -0.0934, -0.1700, 0.1663},
          ...
          {0.0674,
           ...
           0.1663}}});
    ASSERT_TRUE(
        smith::allclose(attn_output, attn_output_expected, 1e-05, 2e-04));

    auto attn_output_weights = std::get<1>(output);
    auto attn_output_weights_expected = smith::tensor(
  ...
  }
  {
    ...
    auto x = smith::tensor({{{2, 4, 5}}}, smith::dtype(smith::kFloat));
    Sequential sequential(MaxUnpool1d(3));
    auto y = sequential->forward(x, indices);
    auto expected =
        smith::tensor({{{0, 2, 0, 4, 5, 0, 0, 0, 0}}}, smith::kFloat);
    ASSERT_TRUE(smith::allclose(y, expected));
  }
  {
  ...
  }
  {
    ...
    auto x = smith::ones({2, 3, 2});
    auto rnn_output =
        sequential->forward<std::tuple<smith::Tensor, smith::Tensor>>(x);
  ...
  }
  {
    ...
    auto rnn_output = sequential->forward<
        std::tuple<smith::Tensor, std::tuple<smith::Tensor, smith::Tensor>>>(x);
    auto expected_output = smith::tensor(
  ...
  }
  {
    ...
    auto x = smith::ones({2, 2});
    auto rnn_output = sequential->forward<smith::Tensor>(x);
    auto expected_output =
        smith::tensor({{-0.0645, -0.7274, 0.4531}, {-0.0645, -0.7274, 0.4531}});
    ASSERT_TRUE(smith::allclose(rnn_output, expected_output, 1e-05, 2e-04));
  }
  {
    smith::manual_seed(0);
    ...
    auto rnn_output =
        sequential->forward<std::tuple<smith::Tensor, smith::Tensor>>(x);
    auto expected_output =
        smith::tensor({{-0.2693, -0.1240, 0.0744}, {-0.2693, -0.1240, 0.0744}});
    ASSERT_TRUE(smith::allclose(
        std::get<0>(rnn_output), expected_output, 1e-05, 2e-04));
  }
  {
    ...
    auto rnn_output = sequential->forward<smith::Tensor>(x);
    auto expected_output =
        smith::tensor({{-0.1134, 0.0467, 0.2336}, {-0.1134, 0.0467, 0.2336}});
    ASSERT_TRUE(smith::allclose(rnn_output, expected_output, 1e-05, 2e-04));
  }
}
