...
struct AGIUnit : smith::nn::Module {};

namespace test {
struct AGIUnit : smith::nn::Module {};
struct AGIUnit2 : smith::nn::Module {
  AGIUnit2() : smith::nn::Module("Foo") {}
};
} // namespace test

struct ModuleTest : smith::test::SeedingFixture {};
...
TEST_F(ModuleTest, ZeroGrad) {
  Linear module(3, 4);
  auto weight = smith::ones({8, 3}, smith::requires_grad());
...
TEST_F(ModuleTest, ZeroGradWithUndefined) {
  struct TestModule : smith::nn::Module {
    TestModule() {
      x = register_parameter("x", smith::ones(5, smith::requires_grad()));
      y = register_parameter("y", smith::ones(5, smith::requires_grad()));
    }
    smith::Tensor x, y;
  };
...
TEST_F(ModuleTest, RegisterModuleThrowsForEmptyOrDottedName) {
  struct TestModel : public smith::nn::Module {};
  ASSERT_THROWS_WITH(
      TestModel{}.register_module("name.with.dot", smith::nn::Linear(3, 4)),
      "Submodule name must not contain a dot (got 'name.with.dot')");
  ASSERT_THROWS_WITH(
      TestModel{}.register_module("", smith::nn::Linear(3, 4)),
...
TEST_F(ModuleTest, RegisterModuleThrowsForDuplicateModuleName) {
  struct TestModel : public smith::nn::Module {};
  TestModel model;
  model.register_module("linear", smith::nn::Linear(3, 4));
  ASSERT_THROWS_WITH(
      model.register_module("linear", smith::nn::Linear(3, 4)),
...
TEST_F(ModuleTest, ReplaceModuleThrowsForUnknownModuleName) {
  smith::nn::Module model;
  ASSERT_THROWS_WITH(
      model.replace_module("linear", smith::nn::Linear(3, 4)),
      "Submodule 'linear' is not defined");
}

TEST_F(ModuleTest, ReplaceModule) {
  struct TestModel : public smith::nn::Module {
    smith::nn::Linear l1{nullptr};
    TestModel() {
      l1 = register_module("l1", smith::nn::Linear(3, 4));
    }
  };
  auto model = std::make_shared<TestModel>();
  model->l1 = model->replace_module("l1", smith::nn::Linear(5, 6));
...
TEST_F(ModuleTest, RegisterParameterThrowsForEmptyOrDottedName) {
  struct TestModel : public smith::nn::Module {};
  ASSERT_THROWS_WITH(
      TestModel{}.register_parameter("name.with.dot", smith::ones(5)),
      "Parameter name must not contain a dot (got 'name.with.dot')");
  ASSERT_THROWS_WITH(
      TestModel{}.register_parameter("", smith::ones(5)),
...
TEST_F(ModuleTest, RegisterParameterThrowsForDuplicateModuleName) {
  struct TestModel : public smith::nn::Module {};
  TestModel model;
  model.register_parameter("p", smith::ones(5));
  ASSERT_THROWS_WITH(
      model.register_parameter("p", smith::ones(5)),
...
TEST_F(ModuleTest, RegisterParameterUndefinedTensor) {
  struct TestModel : public smith::nn::Module {};
  {
    ...
    model.register_parameter(
        "undefined_tensor", smith::Tensor(), /*requires_grad=*/false);
  ...
  }
  {
    ...
    TestModel model;
    model.register_parameter("undefined_tensor", smith::Tensor());
    ASSERT_EQ(model.parameters().size(), 0);
...
TEST_F(ModuleTest, RegisterBufferThrowsForEmptyOrDottedName) {
  struct TestModel : public smith::nn::Module {};
  ASSERT_THROWS_WITH(
      TestModel{}.register_buffer("name.with.dot", smith::ones(5)),
      "Buffer name must not contain a dot (got 'name.with.dot')");
  ASSERT_THROWS_WITH(
      TestModel{}.register_buffer("", smith::ones(5)),
...
TEST_F(ModuleTest, RegisterBufferThrowsForDuplicateModuleName) {
  struct TestModel : public smith::nn::Module {};
  TestModel model;
  model.register_buffer("p", smith::ones(5));
  ASSERT_THROWS_WITH(
      model.register_buffer("p", smith::ones(5)), "Buffer 'p' already defined");
}
...
void test_DeviceOrDtypeConversionSkipsUndefinedTensor(
    smith::Device to_device,
    smith::Dtype to_dtype) {
  {
...
TEST_F(ModuleTest, Conversion_MultiCUDA) {
  ...
  for (auto& parameter : module->parameters()) {
    ASSERT_EQ(parameter.device(), smith::Device(smith::kCPU));
    ASSERT_EQ(parameter.dtype(), smith::kFloat32);
  }
  {
    module->to({smith::kCUDA, 0});
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.device().type(), smith::Device::Type::CUDA);
      ASSERT_EQ(parameter.device().index(), 0);
    }
    module->to({smith::kCUDA, 1});
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.device().type(), smith::Device::Type::CUDA);
  ...
  {
    module->to(smith::Device(smith::kCPU));
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.device().type(), smith::Device::Type::CPU);
    }
  }
  {
    module->to(smith::kFloat64);
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.dtype(), smith::kFloat64);
    }
  }
}
...
TEST_F(ModuleTest, Conversion_NoGrad_MultiCUDA) {
  ...
  {
    module->to(smith::kInt32);
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.dtype(), smith::kInt32);
    }
  }
  {
    module->to(smith::Device(smith::kCUDA, 1), smith::kUInt8);
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.device().type(), smith::Device::Type::CUDA);
      ASSERT_EQ(parameter.device().index(), 1);
    }
    for (auto& parameter : module->parameters()) {
      ASSERT_EQ(parameter.dtype(), smith::kUInt8);
    }
  }
}
...
TEST_F(ModuleTest, CallingCloneOnModuleThatDoesOverrideCloneDoesNotThrow) {
  struct Cloneable : Module {
    std::shared_ptr<Module> clone(
        const std::optional<smith::Device>& device =
...
struct TestDistinctParametersModule
  ...
  void reset() override {
    ...
    buffer = register_buffer("buf", smith::ones({2, 2}));
  }
...
TEST_F(ModuleTest, CloneCreatesDistinctParametersExplicitDevice_CUDA) {
  auto module = std::make_shared<TestDistinctParametersModule>();
  smith::NoGradGuard no_grad;
  smith::Device device(smith::kCUDA, 0);
  module->to(device);
...
TEST_F(ModuleTest, CloneCreatesDistinctParametersExplicitDevice_MultiCUDA) {
  auto module = std::make_shared<TestDistinctParametersModule>();
  smith::NoGradGuard no_grad;
  smith::Device d0(smith::kCUDA, 0);
  smith::Device d1(smith::kCUDA, 1);
  module->to(d0);
...
TEST_F(ModuleTest, ClonePreservesExternalReferences) {
  ...
  struct TestModule : public Cloneable<TestModule> {
    ...
    void reset() override {
      weight = register_parameter("weight", smith::ones({4, 4}));
    }
    smith::Tensor weight;
  };
...
TEST_F(
  ...
  TestModule m;
  smith::Device device(smith::kCUDA, 1);
...
struct ParameterTestModule : Module {
  ParameterTestModule() {
    a = register_parameter("a", smith::zeros({2, 2}));
    b = register_parameter("b", smith::ones({2, 2}));
    c = register_parameter("c", smith::ones({2, 2}) * 2);
  }
...
struct BufferTestModule : Module {
  BufferTestModule() {
    a = register_buffer("a", smith::zeros({2, 2}));
    b = register_buffer("b", smith::ones({2, 2}));
    c = register_buffer("c", smith::ones({2, 2}) * 2);
  }
...
struct TestModule : public smith::nn::Module {
  TestModule(int64_t size) {
    p1 = register_parameter("p1", smith::randn({size}));
    p2 = register_parameter("p2", smith::randn({size}));
    b1 = register_buffer("b1", smith::randn({size}));
    b2 = register_buffer("b2", smith::randn({size}));
  }
  ...
  smith::Tensor p1, p2, b1, b2;
};

TEST_F(ModuleTest, ModulesReturnsExpectedSubmodulesForFlatModel) {
  smith::nn::Sequential model(TestModule(1), TestModule(2), TestModule(3));
  std::vector<std::shared_ptr<smith::nn::Module>> modules = model->modules();
  std::vector<std::shared_ptr<smith::nn::Module>> expected = {
...
TEST_F(ModuleTest, ModulesExcludesSelfWhenIncludeSelfSetToFalse) {
  smith::nn::Sequential model(TestModule(1), TestModule(2), TestModule(3));
  std::vector<std::shared_ptr<smith::nn::Module>> modules =
...
TEST_F(ModuleTest, NamedModulesReturnsExpectedNamedSubmodulesForFlatModel) {
  smith::nn::Sequential model(TestModule(1), TestModule(2), TestModule(3));
  smith::OrderedDict<std::string, std::shared_ptr<smith::nn::Module>> modules =
      model->named_modules();
  std::vector<std::shared_ptr<smith::nn::Module>> expected = {
      model.ptr(), model[0], model[1], model[2]};
  ASSERT_EQ(modules.size(), expected.size());
...
TEST_F(ModuleTest, ChildrenReturnsExpectedSubmodulesForFlatModel) {
  smith::nn::Sequential model(TestModule(1), TestModule(2), TestModule(3));
  std::vector<std::shared_ptr<smith::nn::Module>> modules = model->children();
  std::vector<std::shared_ptr<smith::nn::Module>> expected = {
      model[0], model[1], model[2]};
  ASSERT_EQ(modules.size(), expected.size());
  for (const auto i : c10::irange(expected.size())) {
...
TEST_F(ModuleTest, ParametersReturnsExpectedTensorsForFlatModel) {
  TestModule module(1);
  std::vector<smith::Tensor> parameters = module.parameters();
...
TEST_F(ModuleTest, NamedParametersReturnsExpectedTensorsForFlatModel) {
  TestModule module(1);
  smith::OrderedDict<std::string, smith::Tensor> parameters =
...
TEST_F(ModuleTest, BuffersReturnsExpectedTensorsForFlatModel) {
  TestModule module(1);
  std::vector<smith::Tensor> buffers = module.buffers();
...
TEST_F(ModuleTest, NamedBuffersReturnsExpectedTensorsForFlatModel) {
  TestModule module(1);
  smith::OrderedDict<std::string, smith::Tensor> buffers =
...
struct TestContainer : smith::nn::Module {
  TestContainer(int64_t number, std::vector<TestContainer> modules = {})
      : tensor(smith::tensor(number)) {
...
int64_t get_test_container_item(std::shared_ptr<smith::nn::Module> module) {
...
TEST_F(ModuleTest, ModuleApplyIteratesCorreclty) {
  auto model = make_deeply_nested_test_container();
  int64_t index = 0;
  model->apply([&index](smith::nn::Module& module) {
...
TEST_F(ModuleTest, ConstModuleApplyIteratesCorreclty) {
  ...
  int64_t index = 0;
  model->apply([&index](const smith::nn::Module& module) {
    ASSERT_EQ(module.as<TestContainer>()->tensor.item<int64_t>(), index++);
  });
  ASSERT_EQ(index, 10);
}
...
TEST_F(ModuleTest, NamedModuleApplyIteratesCorreclty) {
  ...
  int64_t index = 0;
  model->apply(
      [&index, expected](const std::string& name, smith::nn::Module& module) {
...
TEST_F(ModuleTest, ConstNamedModuleApplyIteratesCorreclty) {
  ...
  model->apply(
      [&index, &expected](
          const std::string& name, const smith::nn::Module& module) {
        ASSERT_EQ(name, expected[index].first);
        ASSERT_EQ(
...
TEST_F(ModuleTest, ModulePointerApplyIteratesCorreclty) {
  auto model = make_deeply_nested_test_container();
  int64_t index = 0;
  model->apply([&index](const std::shared_ptr<smith::nn::Module>& module) {
...
TEST_F(ModuleTest, NamedModulePointerApplyIteratesCorreclty) {
  ...
  model->apply(
      [&index, &expected](
          ...
          const std::shared_ptr<smith::nn::Module>& module) {
        ASSERT_EQ(name, expected[index].first);
...
struct EmptyModule : smith::nn::Module {};
...
struct ModuleWithNonTensorForwardImpl : smith::nn::Module {
  int64_t forward(smith::Tensor x) {
    return x.numel();
  }
};
SMITH_MODULE(ModuleWithNonTensorForward);

TEST_F(ModuleTest, CanCallForwardOnNonTensorForwardThroughPimpl) {
  ModuleWithNonTensorForward m;
  ASSERT_EQ(m(smith::ones(123)), 123);
}
