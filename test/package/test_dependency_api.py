...
import smith.nn
from smith.package import EmptyMatchError, Importer, PackageExporter, PackageImporter
from smith.package.package_exporter import PackagingError
from smith.testing._internal.common_utils import IS_WINDOWS, run_tests
...
class TestDependencyAPI(PackageTestCase):
    ...
    def test_externing_c_extension(self):
        """Externing c extensions modules should allow us to still access them especially those found in smith._C."""

        buffer = BytesIO()
        # The C extension module in question is F.gelu which comes from smith._C._nn
