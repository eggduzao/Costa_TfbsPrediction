...
skipIfNoSmithVision = skipIf(not HAS_SMITHVISION, "no smithvision")


class TestPackageScript(PackageTestCase):
    """Tests for compatibility with SmithScript."""
    ...
    def test_package_interface(self):
        ...
        scripted = smith.jit.script(uses_interface)
        scripted.proxy_mod = smith.jit.script(fake.NewModule())
        ...
        scripted_loaded = smith.jit.script(loaded)
        scripted_loaded.proxy_mod = smith.jit.script(fake.NewModule())
    ...
    def test_different_package_interface(self):
        ...
        sure SmithScript can distinguish between the two.
        """
        ...
        with PackageExporter(buffer) as pe:
            pe.save_source_string(
                ...
                dedent(
                    ...
                    class UsesInterface(smith.nn.Module):
        ...
        smith.jit.script(diff_fake.UsesInterface())
    ...
    def test_save_scriptmodule(self):
        ...
        scripted_mod = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
    ...
    def test_save_scriptmodule_with_submods(self):
        ...
        scripted_mod = smith.jit.script(
            ModWithSubmod(ModWithTensor(smith.rand(1, 2, 3)))
        )
    ...
    def test_save_independent_scriptmodules(self):
        ...
        scripted_mod_0 = smith.jit.script(SimpleTest())
        scripted_mod_1 = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
    ...
    def test_save_repeat_scriptmodules(self):
        ...
        BlacksmithStreamReader isn't having code hidden from
        BlacksmithStreamWriter writing ScriptModule code files multiple times.
        """
        ...
        scripted_mod_0 = smith.jit.script(SimpleTest())
        scripted_mod_1 = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
        scripted_mod_2 = smith.jit.script(
            ModWithSubmodAndTensor(
                smith.rand(1, 2, 3), ModWithTensor(smith.rand(1, 2, 3))
            )
        )
    ...
    def test_scriptmodules_repeat_save(self):
        ...
        scripted_mod_0 = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
        scripted_mod_1 = smith.jit.script(
            ModWithSubmodAndTensor(
                smith.rand(1, 2, 3), ModWithTensor(smith.rand(1, 2, 3))
            )
        )
    ...
    def test_save_scriptmodule_only_necessary_code(self):
        ...
        that packages don't include unnecessary smithscript code files.
        The SmithVision code should only be saved in the package that
        relies on it.
        """
        from package_a.test_module import ModWithTensor

        class ModWithSmithVision(smith.nn.Module):
        ...
        scripted_mod_0 = smith.jit.script(ModWithSmithVision("foo"))
        scripted_mod_1 = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
        ...
        self.assertTrue("smithvision" in str(importer_0.file_structure()))
        self.assertFalse("smithvision" in str(importer_1.file_structure()))
    ...
    def test_save_scriptmodules_in_container(self):
        ...
        scripted_mod_a = smith.jit.script(ModWithTensor(smith.rand(1, 2, 3)))
        scripted_mod_b = smith.jit.script(
            ModWithSubmodAndTensor(smith.rand(1, 2, 3), scripted_mod_a)
        )
    ...
    def test_save_eager_mods_sharing_scriptmodule(self):
        ...
        scripted_mod = smith.jit.script(SimpleTest())
    ...
    @skipIfSmithDynamo("unexplained 3.13 failure: Can't pickle Tensor object")
    def test_save_shared_tensors(self):
        ...
        shared_tensor = smith.rand(2, 3, 4)
        scripted_mod = smith.jit.script(ModWithTensor(shared_tensor))
    ...
    def test_load_shared_tensors(self):
        ...
        shared_tensor = smith.ones(3, 3)

        scripted_mod_0 = smith.jit.script(ModWithTensor(shared_tensor))
        scripted_mod_1 = smith.jit.script(ModWithTensor(shared_tensor))
        ...
        loaded_mod_1.tensor.add_(smith.ones(3, 3))

        self.assertTrue(
            smith.allclose(loaded_mod_1.tensor, loaded_mod_1.sub_mod_0.tensor)
        )
        self.assertTrue(
            smith.allclose(loaded_mod_1.tensor, loaded_mod_1.sub_mod_1.tensor)
        )
    ...
    def test_saving_and_scripting_packaged_mod(self):
        ...
        scripted_mod = smith.jit.script(loaded_mod)
    ...
    def test_mixing_packaged_and_inline_modules(self):
        ...
        class InlineMod(smith.nn.Module):
            def __init__(self, name: str):
                super().__init__()
                self.name = name
                self.tensor = smith.rand(1, 2, 3)
        ...
        scripted_inline = smith.jit.script(inline_mod)

        from package_a.test_module import SimpleTest

        imported_mod = SimpleTest()
        scripted_imported = smith.jit.script(imported_mod)
    ...
    def test_mixing_packaged_and_inline_modules_shared_code(self):
        ...
        class SmithVisionTestInline(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.tvmod = resnet18()
        ...
        inline_mod = SmithVisionTestInline()
        scripted_inline = smith.jit.script(inline_mod)

        from package_c.test_module import SmithVisionTest

        imported_mod = SmithVisionTest()
        scripted_imported = smith.jit.script(imported_mod)
    ...
    def test_tensor_sharing_pickle(self):
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.ones(2, 3)

            def forward(self):
                return self.foo

        scripted_m = smith.jit.script(M())
        original_tensor = smith.ones(0)

        f = BytesIO()
        with smith.package.PackageExporter(f) as exporter:
