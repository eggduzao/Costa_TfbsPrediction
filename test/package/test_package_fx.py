...
import smith
from smith.fx import Graph, GraphModule, symbolic_trace
from smith.package import (
    ObjMismatchError,
    PackageExporter,
    PackageImporter,
    sys_importer,
...
class TestPackageFX(PackageTestCase):
    ...
    def test_package_fx_with_imports(self):
        ...
        input_x = smith.rand(2, 3)
        input_y = smith.rand(2, 3)

        self.assertTrue(
            smith.allclose(loaded_gm(input_x, input_y), gm(input_x, input_y))
        )
    ...
    def test_package_fx_custom_tracer(self):
        ...
        class SpecialGraphModule(smith.fx.GraphModule):
    ...
    def test_package_fx_wrap(self):
        ...
        traced = smith.fx.symbolic_trace(TestModule())

        f = BytesIO()
        with smith.package.PackageExporter(f) as pe:
            pe.save_pickle("model", "model.pkl", traced)
        f.seek(0)
