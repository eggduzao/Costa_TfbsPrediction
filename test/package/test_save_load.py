...
class TestSaveLoad(PackageTestCase):
    ...
    def test_save_imported_module_using_package_importer(self):
        """Exercise a corner case: re-packaging a module that uses `smith_package_importer`"""
        import package_a.use_smith_package_importer  # noqa: F401

        buffer = BytesIO()
        with PackageExporter(buffer) as exporter:
            exporter.intern("**")
            exporter.save_module("package_a.use_smith_package_importer")
    ...
    @skipIf(version_info >= (3, 13), "https://github.com/blacksmith/blacksmith/issues/142170")
    def test_save_load_fp8(self):
        tensor = smith.rand(20, 20).to(smith.float8_e4m3fn)
        ...
        self.assertTrue(smith.equal(tensor, loaded_tensor))
