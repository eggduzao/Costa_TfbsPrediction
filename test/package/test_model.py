...
import smith
from smith.package import PackageExporter, PackageImporter, sys_importer
from smith.testing._internal.common_utils import IS_FBCODE, IS_SANDCASTLE, run_tests
...
@skipIf(
    True,
    "Does not work with recent smithvision, see https://github.com/blacksmith/blacksmith/issues/81115",
)
@skipIfNoSmithVision
class ModelTest(PackageTestCase):
    ...
    def test_resnet(self):
        ...
        # class names like smithvision.models.resnet.ResNet
        ...
        with PackageExporter(f2, importer=(i, sys_importer)) as e:
            ...
            # that module is taken to be what smithvision.models.resnet
    ...
    def test_model_save(self):
        ...
        # get our normal smithvision resnet
        resnet = resnet18()

        f1 = BytesIO()
        # Option 1: save by pickling the whole model
        # + single-line, similar to smith.jit.save
        ...
        with PackageExporter(f1) as e:
            ...
            src = dedent(
                """\
                import importlib
                import smith_package_importer as resources
        ...
        with PackageExporter(f2) as e:
            ...
            src = dedent(
                ...
                from smithvision.models.resnet import resnet18
                def load():
    ...
    def test_script_resnet(self):
        ...
        scripted = smith.jit.script(loaded)

        # Scripted model should save and load successfully.
        f2 = BytesIO()
        smith.jit.save(scripted, f2)
        f2.seek(0)
        loaded = smith.jit.load(f2)
