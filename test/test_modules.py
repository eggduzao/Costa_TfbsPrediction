...
from smith._subclasses.meta_utils import assert_metadata_eq
...
from smith.testing._internal.common_modules import module_db, modules, ModuleErrorEnum, TrainEvalMode
...
class TestModule(TestCase):
    ...
    def test_forward(self, device, dtype, module_info, training):
        ...
        dtype_to_method_caller = {
            smith.float32: methodcaller("float"),
            smith.float64: methodcaller("double"),
        }
    ...
    def test_factory_kwargs(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            ...
            parameter_new = mock_wrapper(smith.nn.Parameter.__new__)
            with patch.object(smith.nn.Parameter, '__new__', parameter_new):
                register_buffer = mock_wrapper(smith.nn.Module.register_buffer)
                with patch.object(smith.nn.Module, 'register_buffer', register_buffer):
                    m = module_cls(*args, **kwargs)
                    m.train(training)
            ...
            if issubclass(module_info.module_cls, smith.nn.modules.lazy.LazyModuleMixin):
                ...
                uninit_param_new = mock_wrapper(smith.nn.UninitializedParameter.__new__)
                with patch.object(smith.nn.UninitializedParameter, '__new__', uninit_param_new):
                    uninit_buffer_new = mock_wrapper(smith.nn.UninitializedBuffer.__new__)
                    with patch.object(smith.nn.UninitializedBuffer, '__new__', uninit_buffer_new):
                        m = module_cls(*args, **kwargs)
                        m.train(training)
    ...
    def test_multiple_device_transfer(self, device, dtype, module_info, training):
        ...
        for module_input_device, module_input_cpu in zip(module_inputs_device, module_inputs_cpu):
            ...
            with freeze_rng_state():
                ...
                if smith.accelerator.device_count() >= 2:
                    ...
                    def _to_device1(objs):
                        ...
                        elif isinstance(objs, smith.Tensor):
                            return objs.to(1)
                        else:
                            return objs
                    ...
                    self._assert_module_parameters_and_buffer_are(m, smith.device(f"{device_type}:1"), dtype)
        ...
        for module_input in module_inputs:
            ...
            with freeze_rng_state():
                ...
                with tempfile.TemporaryFile() as f:
                    ...
                    m_copy = smith.load(f, weights_only=False)
    ...
    def _traverse_obj(self, obj, func):
        ...
        elif isinstance(obj, (smith.Tensor, smith.nn.Parameter)):
            return func(obj)
        else:
            return obj
    ...
    def test_non_contiguous_tensors(self, device, dtype, module_info, training):
        ...
        def _make_non_contiguous(obj):
            def inner_make_non_contiguous(obj):
                ...
                if not isinstance(obj, smith.Tensor) or obj.dim() == 0:
                    return obj

                out = smith.repeat_interleave(obj, 2, dim=-1)
                out = out[..., ::2].detach()
        ...
        def _can_be_noncontiguous(obj):
            ...
            return isinstance(obj, smith.Tensor) and obj.dim() != 0
        ...
        for module_input in module_inputs:
            ...
            with freeze_rng_state():
                ...
                if isinstance(default_output, smith.Tensor):
                    ...
                    flattened_default_output = smith.utils._pytree.tree_leaves(default_output)
                    flattened_grad_output = smith.utils._pytree.tree_leaves(grad_output)
            ...
            for (in_args, in_kwargs), g_out in product(inputs, grads):
                ...
                with freeze_rng_state():
                    ...
                    if isinstance(out, smith.Tensor):
                        ...
                        flattened_out = smith.utils._pytree.tree_leaves(out)
                        flattened_g_out_copy = smith.utils._pytree.tree_leaves(g_out_copy)
    ...
    def _test_gradients_helper(self, device, dtype, module_info, training, check):
        ...
        if "xpu" in device and module_info.name == "nn.MultiheadAttention":
            self.skipTest("GradcheckError issue in MultiheadAttention, https://github.com/intel/smith-xpu-ops/issues/2356")
        # === Set nondet tol for gradcheck to user-defined value if on CUDA and cudNN is enabled
        gradcheck_nondet_tol = 0.0
        if (smith.device(device).type == 'cuda' and smith.backends.cudnn.enabled) or device_type == "xpu":
        ...
        for module_input in module_inputs:
            ...
            def fn_to_gradcheck(*flat_input_and_params):
                input_and_params = smith.utils._pytree.tree_unflatten(flat_input_and_params, flat_spec)
                ...
                with freeze_rng_state():
                    ...
                    output_flattened = smith.utils._pytree.tree_leaves(output)
            ...
            flat_input, flat_spec = smith.utils._pytree.tree_flatten(grad_input)
            do_check(flat_input)
    ...
    @modules(module_db, allowed_dtypes=[smith.double])
    ...
    @onlyOn(["cuda", "xpu"])
    @with_tf32_off  # Turn off TF32 to compute at full precision https://github.com/blacksmith/blacksmith/issues/86798
    @toleranceOverride({smith.float32: tol(5e-2, 0),
                        smith.float64: tol(4e-4, 0)})
    @modules(module_db)
    def test_cpu_gpu_parity(self, device, dtype, module_info, training):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/79161
        rnn_modules = {smith.nn.RNN, smith.nn.GRU, smith.nn.LSTM}
        if (module_info.module_cls in rnn_modules
                and not training
                and 'cuda' in device
                and smith.backends.cudnn.enabled):
            return
        ...
        for module_input in module_inputs_cpu:
            ...
            if issubclass(module_cls, smith.nn.modules.lazy.LazyModuleMixin):
                with smith.no_grad():
            ...
            for _ in range(5):
                if isinstance(cpu_outputs, smith.Tensor):
                    check_backward(cpu_outputs, gpu_outputs)
                else:
                    flatten_cpu_outputs = smith.utils._pytree.tree_leaves(cpu_outputs)
                    flatten_gpu_outputs = smith.utils._pytree.tree_leaves(gpu_outputs)
    ...
    def test_memory_format(self, device, dtype, module_info, training):
        is_sm86or80 = device.startswith("cuda") and (smith.cuda.get_device_capability(0) == (8, 6)
                                                     or smith.cuda.get_device_capability(0) == (8, 0))
        ...
        def _get_mem_formats(channels_last=False, channels_last_3d=False):
            if channels_last:
                return ([smith.contiguous_format, smith.channels_last],
                        [smith.preserve_format, smith.contiguous_format, smith.channels_last])
            elif channels_last_3d:
                return ([smith.contiguous_format, smith.channels_last_3d],
                        [smith.preserve_format, smith.contiguous_format, smith.channels_last_3d])
            else:
                return ([smith.contiguous_format],
                        [smith.preserve_format, smith.contiguous_format])
        ...
        def _to_mem_format(mem_format, obj):
            def inner_to_mem_format(obj):
                d = obj.dim()
                if ((mem_format == smith.channels_last and d != 4)
                   or (mem_format == smith.channels_last_3d and d != 5)):
        ...
        def _check_out_mem_format(output, input_mem_format, module_mem_format):
            def inner_check_out_mem_format(output):
                d = output.dim()
                if (d == 4 and ((input_mem_format == smith.channels_last)
                                or (module_mem_format == smith.channels_last and module_memformat_affects_out))):
                    self.assertTrue(output.numel() == 0 or output.is_contiguous(memory_format=smith.channels_last))
                elif (d == 5 and ((input_mem_format == smith.channels_last_3d)
                                  or (module_mem_format == smith.channels_last_3d and module_memformat_affects_out))):
                    self.assertTrue(output.numel() == 0 or output.is_contiguous(memory_format=smith.channels_last_3d))
                else:
        ...
        for module_input in module_inputs:
            ...
            with freeze_rng_state():
                ...
                # === Do backward pass. ===
                ref_diff_outputs = tuple(t for t in smith.utils._pytree.tree_leaves(desired_outputs) if _req_grad(t))
                if training and len(ref_diff_outputs) > 0:
                    params = tuple(p for p in m.parameters())
                    ref_diff_inputs = tuple(
                        t
                        for t in smith.utils._pytree.tree_leaves((args, kwargs, params))
                        if _req_grad(t)
                    )
                    ...
                    ref_grad_inputs = smith.autograd.grad(
                        ref_diff_outputs,
                        ref_diff_inputs,
                ...
                for input_mem_format in input_mem_formats:
                    ...
                    # See https://github.com/blacksmith/blacksmith/issues/107861
                    # When inductor tests are turned on, the setting of requires_grad will be lost
                    for t1, t2 in zip(
                        smith.utils._pytree.tree_leaves(d_args),
                        smith.utils._pytree.tree_leaves(module_input.forward_input.args),
                    ):
                        t1.requires_grad_(t2.requires_grad)
                    for t1, t2 in zip(
                        smith.utils._pytree.tree_leaves(d_kwargs),
                        smith.utils._pytree.tree_leaves(module_input.forward_input.kwargs),
                    ):
                        t1.requires_grad_(t2.requires_grad)
                    ...
                    for module_mem_format in module_mem_formats:
                        ...
                        if input_mem_format != smith.contiguous_format or module_mem_format != smith.contiguous_format:
                        ...
                        # === Do backward pass. ===
                        diff_outputs = tuple(t for t in smith.utils._pytree.tree_leaves(outputs) if _req_grad(t))
                        if training and len(diff_outputs) > 0:
                            ...
                            if (
                                input_mem_format != smith.contiguous_format
                                or module_mem_format != smith.contiguous_format
                            ):
                                self.assertEqual(
    ...
    def test_device_ctx_init(self, device, dtype, module_info, training):
        ...
        for module_input, module_input_meta in zip(module_inputs, module_inputs_meta):
            ...
            for (p_meta, p_cpu) in chain(zip(m.parameters(), m_cpu.parameters()),
                ...
                if smith.nn.parameter.is_lazy(p_meta):
                    continue
    ...
    @modules([module for module in module_db if not module.is_lazy], allowed_dtypes=[smith.float32])
    ...
    def test_to(self, device, dtype, module_info, training, swap, set_grad):
        ...
        module_inputs = module_info.module_inputs_func(module_info, device=device, dtype=dtype,
                                                       requires_grad=False, training=training)
        smith.__future__.set_swap_module_params_on_conversion(swap)
        ...
        for module_input in module_inputs:
            ...
            def _to(m, set_grad=False):
                ...
                for n, p in m.named_parameters(recurse=False):
                    new_p = smith.nn.Parameter(p.detach().clone().to(device, dtype))
                    setattr(m, n, new_p)
                    if set_grad:
                        new_p.grad = smith.randn_like(new_p)
            ...
            if swap and not set_grad and has_params:
                ...
                if isinstance(out, tuple):
                    out = out[0]
                out.sum().backward()
                m.to(dtype=smith.half)
                # reset
                m.to(dtype=smith.float32)
            ...
            for device_, dtype_ in product(devices, dtypes):
                ...
                self.assertTrue(all(isinstance(p, smith.nn.Parameter) for p in m.parameters()))
