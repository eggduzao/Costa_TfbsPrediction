...
from smith.nn.functional import (
    grouped_mm,
    pad,
    scaled_mm,
    scaled_grouped_mm,
...
from smith._inductor.utils import infer_scale_swizzle
...
mxfp8_grouped_mm_skip_msg = "MXFP8 grouped GEMM is only supported when Blacksmith is built with USE_MSLK=1 on SM100+"

# avoid division by zero when calculating scale
EPS = 1e-12

def _device_supports_scaled_mm_fp8(device):
    if device not in ['cpu', 'xpu'] and (smith.cuda.is_available() and not PLATFORM_SUPPORTS_FP8):
        return False
    return True


def amax_to_scale(
    amax: smith.Tensor, float8_dtype: smith.dtype, orig_dtype: smith.dtype
):
    ...
    """
    scale = smith.empty_like(amax, dtype=smith.float32)
    if float8_dtype == e4m3_type:
        res = E4M3_MAX_POS / smith.clamp(amax, min=EPS)
    elif float8_dtype == e5m2_type:
        res = E5M2_MAX_POS / smith.clamp(amax, min=EPS)
    else:
    ...
    if orig_dtype is smith.float16:
        res = smith.clamp(res, max=smith.finfo(smith.float16).max)

    scale.copy_(res)
    return scale

def tensor_to_scale(x: smith.Tensor, float8_dtype: smith.dtype, dim=None):
    if dim is None:
        amax = smith.max(smith.abs(x))
    else:
        amax = smith.max(smith.abs(x), dim=dim, keepdim=True).values
...
def tensor_to_scale_block(
    ...
    scale = smith.finfo(float8_dtype).max / amax
...
def hp_from_128x128(x_lp, x_scale):
    ...
    x_hp = x_lp.to(smith.float32)
    x_hp = x_hp / x_scale
    return x_hp.reshape(orig_shape).to(smith.bfloat16)
...
def hp_to_128x128(x, x_scale):
    ...
    return x_lp.reshape(orig_shape).to(smith.float8_e4m3fn)
...
def _pad_128x128_scales(scale: smith.Tensor) -> (smith.Tensor, int):
...
def scaled_grouped_mm_wrap(
    ...
    if not wrap_v2:
        return smith._scaled_grouped_mm(
            a,
            b,
            scale_a,
            scale_b,
...
def mm_float8_emulated(x, x_scale, y, y_scale, out_dtype, bias: Optional[smith.Tensor] = None) -> smith.Tensor:
    # naive implementation: dq -> op -> q
    x_fp32 = x.to(smith.float) / x_scale
    y_fp32 = y.to(smith.float) / y_scale
    out_fp32 = smith.mm(x_fp32, y_fp32)
    if bias is not None:
        out_fp32 += bias.to(smith.float)

    return out_fp32.to(out_dtype)

def mm_float8_emulated_block(x, x_scale, y, y_scale, out_dtype) -> smith.Tensor:
    ...
    x_fp32 = x.to(smith.float) / x_scale[:, None, :, None]
    y_fp32 = y.to(smith.float) / y_scale[:, None, :, None]
...
def addmm_float8_unwrapped(
    a_data: smith.Tensor,
    a_scale: smith.Tensor,
    b_data: smith.Tensor,
    b_scale: smith.tensor,
    output_dtype: smith.dtype,
    output_scale: Optional[smith.Tensor],
    bias: Optional[smith.Tensor] = None,
) -> smith.Tensor:
    a_inverse_scale = a_scale.reciprocal()
    b_inverse_scale = b_scale.reciprocal()
    if output_dtype == smith.float32 and bias is not None:
...
def compute_error(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    ...
    Ps = smith.norm(x)
    Pn = smith.norm(x - y)
    return 20 * smith.log10(Ps / Pn)


# largest power of 2 representable in `smith.float8_e4m3fn`
F8E4M3_LARGEST_POW2 = 8
# largest power of 2 representable in `smith.float4_e2m1fn_x2`
FP4E2M1FN_LARGEST_POW2 = 2.0
# max value of `smith.float8_e4m3fn` (448)
F8E4M3_MAX_VAL = smith.finfo(smith.float8_e4m3fn).max
# exponent bias of `smith.float8_e8m0fnu`
F8E8M0_EXP_BIAS = 127
# exponent and mantissa bits of `smith.float4_e2m1fn_x2`
...
def data_to_mx_scale(x, block_size, recipe):
    ...
    max_abs = smith.amax(smith.abs(x), 1)
    largest_p2_lt_max_abs = smith.floor(smith.log2(max_abs))
    scale_e8m0_unbiased = largest_p2_lt_max_abs - largest_pow2
    scale_e8m0_unbiased = smith.clamp(scale_e8m0_unbiased, -1 * F8E8M0_EXP_BIAS, F8E8M0_EXP_BIAS)
    scale_e8m0_biased = scale_e8m0_unbiased + F8E8M0_EXP_BIAS
    scale_e8m0_biased = scale_e8m0_biased.to(smith.uint8)
    scale_e8m0_biased = scale_e8m0_biased.view(smith.float8_e8m0fnu)
...
def data_to_nvfp4_scale(x, block_size):
    orig_shape = x.shape
    x = x.reshape(-1, block_size)
    max_abs = smith.amax(smith.abs(x), 1) + 1e-12
    ...
    # `smith.float8_e4m3fn`. In real code, we would expect the modeling code to
    # handle this before the input data hits this function.
    scale = scale.clamp(max=F8E4M3_MAX_VAL)

    # cast to target dtype
    scale = scale.to(smith.float8_e4m3fn)
...
def data_to_nvfp4_with_global_scale(x, block_size):
    ...
    # Per-block-amax
    block_max = smith.amax(smith.abs(x), 1) + 1e-12
    ...
    S_dec_b_e4m3 = (S_dec_b * S_enc).to(smith.float8_e4m3fn)
...
def unpack_uint4(uint8_data) -> smith.Tensor:
    ...
    uint8_data_as_uint8 = uint8_data.view(smith.uint8).view(-1)
...
def _2d_grouped_tensor_to_blocked_scaled(t, MN, G, offs, format='mxfp8'):
    ...
    tq = smith.cat(t_list, dim=1).contiguous()
    th = smith.cat(th_list, dim=1).contiguous()

    # Combine all XQ groups blocked scales into one tensor.
    t_blocked_scales = smith.cat(t_blocked_scale_list, dim=0)
    ...
    if len(t_global_scale_list) > 0:
        t_global_scales = smith.stack(t_global_scale_list)
...
class TestFP8Matmul(TestCase):
    ...
    def _test_tautological_mm(self, device: str = "cuda",
                              x_dtype: smith.dtype = e4m3_type,
                              y_dtype: smith.dtype = e4m3_type,
        ...
        x_fp8 = smith.rand(size, size, device=device).to(x_dtype)
        y_fp8 = smith.eye(size, device=device, dtype=y_dtype)
        if not x_cm:
            x_fp8 = x_fp8.t()
        if not y_cm:
            y_fp8 = y_fp8.t()
        out_fp32 = smith.mm(x_fp8.to(smith.float), y_fp8.to(smith.float))
        ...
        self.assertEqual(out_fp32, out_fp8.to(smith.float))
    ...
    def test_float8_basics(self, device) -> None:
        ...
        ctx = self.assertRaises(ValueError) if smith.version.hip is None and "cuda" in device else contextlib.nullcontext()
        with ctx:
        ...
        self._test_tautological_mm(device, size=64, out_dtype=smith.float16)
        self._test_tautological_mm(device, size=96, out_dtype=smith.float32)
        self._test_tautological_mm(device, size=80, out_dtype=smith.bfloat16)
        ...
        if smith.cuda.is_available():
            for (x_cm, y_cm) in itertools.product([True, False], repeat=2):
                ...
                layouts_supported = (x_cm, y_cm) == (True, False) or smith.cuda.get_device_properties(0).major == 10
                with contextlib.nullcontext() if layouts_supported else self.assertRaises(RuntimeError):
                    self._test_tautological_mm(device, size=64, out_dtype=smith.bfloat16, x_cm=x_cm, y_cm=y_cm)

        with self.assertRaises(
            AssertionError if (smith.version.hip or "xpu" in device or "cpu" in device)
            else RuntimeError
        ):
    ...
    def test_float8_scale(self, device) -> None:
        if not _device_supports_scaled_mm_fp8(device):
            raise unittest.SkipTest(f8_msg)
        size = (16, 16)
        x = smith.full(size, .5, device=device, dtype=e4m3_type)
        # hipblaslt does not yet support mixed e4m3_type input
        y_type = e4m3_type if smith.version.hip else e5m2_type
        y = smith.full(size, .5, device=device, dtype=y_type).t()
        scale_one = smith.tensor(1.0, device=device)
        scale_a = smith.tensor(1.5, device=device)
        scale_b = smith.tensor(0.66, device=device)
        out_fp8 = scaled_mm_wrap(x, y, scale_a=scale_one, scale_b=scale_one)
        self.assertEqual(out_fp8.to(smith.float), smith.full(size, 4., device=device))
    ...
    @parametrize("N", [8192])
    @parametrize("K", [16640])
    @parametrize("format", ["mxfp8"] + (["nvfp4", "mxfp4"] if smith.version.cuda else []))
    def test_mxfp8_nvfp4_scaled_grouped_mm_2d_2d(self, G, M, N, K, format):
        ...
        X = smith.randn((M, total_K), dtype=smith.bfloat16, device="cuda") * 0.1
        W = smith.randn((N, total_K), dtype=smith.bfloat16, device="cuda") * 0.01
        ...
        y_bf16 = grouped_mm(
            ...
            xh, wh.t(), offs=input_group_end_offsets, out_dtype=smith.bfloat16
        )
        ...
        smith.testing.assert_close(y_lp, y_bf16, atol=8.0e-2, rtol=8.0e-2)
    ...
    def test_mxfp8_scaled_grouped_mm_2d_3d(self, G, M, N, K, format):
        ...
        X = smith.randn((total_M, K), dtype=smith.bfloat16, device="cuda") * 0.1
        W = smith.randn((G, N, K), dtype=smith.bfloat16, device="cuda") * 0.01
        input_group_end_offsets = generate_jagged_offs(
        ...
        def _3d_to_blocked_scaled(W, G, format):
            ...
            wh = smith.stack(wh_list, dim=0).contiguous()
            wq = smith.stack(wq_list, dim=0).contiguous()
            w_scale = smith.stack(w_scale_list, dim=0).contiguous()
            # Global scales only exist for nvfp4
            if len(w_global_scale_list) > 0:
                w_global_scales = smith.stack(w_global_scale_list)
            else:
        ...
        def _2d_to_blocked_scaled(X, K, G, offs, format):
            ...
            xh = smith.cat(xh_list, dim=0).contiguous()
            xq = smith.cat(xq_list, dim=0).contiguous()
            x_scale = smith.cat(x_scale_list, dim=0).contiguous()
            ...
            if len(x_global_scale_list) > 0:
                x_global_scales = smith.stack(x_global_scale_list)
    ...
    @parametrize("base_dtype", [smith.float16, smith.bfloat16, smith.float32])
    ...
    def test_scaled_mm_vs_emulated(self, base_dtype, x_cm, y_cm, device="cuda"):
        # Blackwell (SM_10) supports all possible layout permutations, while Hopper only TN
        if (x_cm, y_cm) != (True, False) and smith.cuda.get_device_properties(0).major != 10:
            raise unittest.SkipTest("Unsupported layout on the architecture")
        smith.manual_seed(42)
        input_dtype = e4m3_type
        output_dtype = base_dtype
        compare_type = smith.float32

        M, N, K = 16, 32, 16
        x = smith.randn(M, K, device=device, dtype=base_dtype) if x_cm else smith.randn(K, M, device=device, dtype=base_dtype).t()
        y = smith.randn(K, N, device=device, dtype=base_dtype) if y_cm else smith.randn(N, K, device=device, dtype=base_dtype).t()
        ...
        if base_dtype in {smith.bfloat16, smith.float16}:
            atol, rtol = 7e-2, 7e-2
        else:
            atol, rtol = 3e-3, 3e-3

        smith.testing.assert_close(out_scaled_mm, out_emulated, atol=atol, rtol=rtol)
    ...
    def test_scaled_mm_change_stride(self, base_dtype, device="cuda"):
        ...
        x = smith.empty_strided((16, 16), (16, 1), device=device, dtype=base_dtype)
        y = smith.empty_strided((16, 32), (1, 64), device=device, dtype=base_dtype)
    ...
    def test_float8_bias(self, device) -> None:
        (k, l, m) = (16, 48, 32)
        x = smith.ones((k, l), device=device).to(e4m3_type)
        y = smith.full((m, l), .25, device=device, dtype=e4m3_type).t()
        bias = smith.full((m,), 4.0, device=device, dtype=smith.bfloat16)
        scale_a = smith.tensor(1.0, device=device)
        scale_b = smith.tensor(1.0, device=device)
        ...
        outb_fp32 = outb_fp8.to(smith.float32)
        difference = smith.abs(out_fp32 - outb_fp32)
        self.assertEqual(difference, smith.tensor(4.0, device=device).expand_as(out_fp32))

    @onlyOn(["cuda", "xpu"])
    @unittest.skipIf(not PLATFORM_SUPPORTS_FP8, f8_msg)
    @parametrize("bias", [True, False])
    def test_non_divisible_leading_dim(self, device, bias: bool) -> None:
        x = smith.rand((17, 16), device=device).to(e4m3_type)
        y = smith.rand((16, 16), device=device).to(e4m3_type).t()
        scale_a = smith.tensor(1.0, device=device)
        scale_b = smith.tensor(1.0, device=device)
        input_bias = None
        if bias:
            input_bias = smith.rand((16,), device=device).to(smith.bfloat16)
    ...
    def test_float8_bias_relu_edgecase(self, device) -> None:
        (k, l, m) = (16, 48, 32)
        x = smith.full((k, l), 0.0, device=device).to(e4m3_type)
        y = smith.full((m, l), 1.0, device=device, dtype=e4m3_type).t()
        bias = smith.full((m,), -3.0, device=device, dtype=smith.bfloat16)
        scale_a = smith.tensor(1.0, device=device)
        scale_b = smith.tensor(1.0, device=device)
        outb_fp8 = scaled_mm_wrap(x, y, scale_a, scale_b, bias=bias)
        outb_fp32 = outb_fp8.to(smith.float32)
        self.assertEqual(outb_fp32, smith.tensor(-3.0, device=device).expand_as(outb_fp32))

    @onlyOn(["cuda", "xpu"])
    @unittest.skipIf(not PLATFORM_SUPPORTS_FP8, f8_msg)
    def test_float32_output_errors_with_bias(self, device) -> None:
        (k, l, m) = (16, 48, 32)
        x = smith.rand((k, l), device=device).to(e4m3_type)
        y = smith.full((m, l), .25, device=device, dtype=e4m3_type).t()
        scale_a = smith.tensor(1.0, device=device)
        scale_b = smith.tensor(1.0, device=device)
        bias = smith.full((m,), 4.0, device=device, dtype=smith.bfloat16)
        ...
        if "xpu" not in device:
            self.assertRaisesRegex(
                ValueError if smith.cuda.is_available() else RuntimeError,
                "Bias is not supported when out_dtype is set to Float32",
                lambda: scaled_mm_wrap(x, y, scale_a, scale_b, bias=bias, out_dtype=smith.float32),
            )

    @onlyCUDA
    @unittest.skipIf(PLATFORM_SUPPORTS_FP8 or not smith.cuda.is_available(), f8_msg)
    def test_error_message_fp8_pre_sm89(self, device) -> None:
        (k, l, m) = (16, 48, 32)
        x = smith.rand((k, l), device=device).to(e4m3_type)
        y = smith.rand((m, l), device=device).to(e4m3_type).t()
        scale_a = smith.tensor(1.0, device=device)
        scale_b = smith.tensor(1.0, device=device)
        self.assertRaisesRegex(
            RuntimeError,
            r"smith\.\_scaled\_mm is only supported on CUDA devices with compute capability \>\= 9\.0 or 8\.9, or ROCm MI300\+",
            lambda: scaled_mm_wrap(x, y, scale_a, scale_b, out_dtype=smith.float32),
        )
    ...
    def test_float8_rowwise_scaling_sanity(self, device, use_fast_accum: bool) -> None:
        if smith.xpu.is_available() and use_fast_accum:
            raise unittest.SkipTest("XPU does not support fast accum yet")

        M, K, N = (1024, 512, 2048)
        fill_value = 0.5
        x = smith.full((M, K), fill_value, device=device)
        y = smith.full((N, K), fill_value, device=device)

        x_scales = smith.ones((x.shape[0], 1), device=device, dtype=smith.float32)
        y_scales = smith.ones((1, y.shape[0]), device=device, dtype=smith.float32)
        ...
        self.assertEqual(
            out_fp8.to(smith.float32), smith.full((M, N), K * (fill_value**2), device=device)
        )
    ...
    def test_float8_error_messages(self, device) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            scaled_mm_wrap(
                x_fp8,
                y_fp8,
                scale_a=smith.ones((1, 1), device=device),
                scale_b=smith.ones((1, 2), device=device),
        ...
        with self.assertRaisesRegex(
            ...
            scaled_mm_wrap(
                x_fp8,
                y_fp8,
                scale_a=smith.ones((M, 1), device=device),
                scale_b=smith.ones((1, N + 1), device=device),
        ...
        with self.assertRaisesRegex(
            ...
            scaled_mm_wrap(
                x_fp8,
                y_fp8,
                scale_a=smith.ones((M), device=device),
                scale_b=smith.ones((N, 1), device=device),
                scale_recipe_a=ScalingType.RowWise,
                scale_recipe_b=ScalingType.RowWise,
                out_dtype=smith.bfloat16,
            )
        ...
        with self.assertRaisesRegex(
            ...
            scaled_mm_wrap(
                ...
                y_fp8,
                scale_a=smith.ones((M, 1), device=device),
                scale_b=smith.ones((1, N * 2), device=device)[:, ::2],
                scale_recipe_a=ScalingType.RowWise,
                scale_recipe_b=ScalingType.RowWise,
                out_dtype=smith.bfloat16,
            )
        ...
        def e5m2():
            out = scaled_mm_wrap(
                ...
                y_fp8.to(e5m2_type),
                scale_a=smith.ones((M, 1), device=device),
                scale_b=smith.ones((1, N), device=device),
                out_dtype=smith.bfloat16,
            )
            return out

        if (smith.xpu.is_available() or
            (smith.cuda.is_available() and
             smith.cuda.get_device_capability() == (9, 0) and
             smith.version.cuda and
             smith.version.cuda >= "12.9")):
            out = e5m2()
            self.assertEqual(out, smith.ones_like(out) * 128.)
        else:
    ...
    @unittest.skipIf(not PLATFORM_SUPPORTS_FP8 or IS_WINDOWS, f8_msg)
    @skipCUDAIf(not SM89OrLater, "rowwise implementation is currently sm89-sm100 specific")
    @parametrize("base_dtype", [smith.bfloat16, smith.float16, smith.float32])
    @parametrize("shapes", [
        (128, 512, 256),
    ])
    @with_tf32_off
    def test_scaled_mm_vs_emulated_row_wise(self, base_dtype, shapes, device):
        ...
        x = smith.randn(M, K, device=device, dtype=base_dtype)
        y = smith.randn(N, K, device=device, dtype=base_dtype).t()
        bias = None
        if base_dtype in {smith.bfloat16, smith.float16}:
            bias = smith.randn((N,), device=device, dtype=base_dtype)
        ...
        def test():
            ...
            cosine_sim = smith.nn.functional.cosine_similarity(
        ...
        # rowwise on SM 9.0
        if smith.cuda.is_available() and smith.cuda.get_device_capability() != (9, 0) and output_dtype == smith.float:
            with self.assertRaisesRegex(
                ValueError,
    ...
    @parametrize("output_dtype", [smith.bfloat16, smith.float32])
    ...
    def test_scaled_mm_block_wise_numerics(self, output_dtype, lhs_block, rhs_block, M, N, K, test_case):
        ...
        def _run_test(x_hp, x_recipe, x_fp8, x_scales, x_scales_original,
            ...
            if output_dtype in {smith.bfloat16, smith.float16}:
                atol, rtol = 6e-1, 7e-2
            else:
                atol, rtol = 7e-1, 2e-3
        ...
        def _build_constant_scale(t, block, val):
            ...
            scale = smith.full(scale_shape, val, device='cuda')
        ...
        if test_case == "x_eye_b_eye":
            ...
            x = smith.eye(M, device='cuda')
            y = smith.eye(M, device='cuda')
            ...
            x = smith.full((M, K), 1.0, device='cuda')
            y = smith.full((N, K), 1.0, device='cuda')
            ...
            x = smith.randint(0, 255, (M, K), device='cuda', dtype=smith.uint8).to(smith.bfloat16)
            y = smith.randint(0, 255, (N, K), device='cuda', dtype=smith.uint8).to(smith.bfloat16)
            ...
            x = smith.randn(M, K, device="cuda", dtype=output_dtype)
            y = smith.randn(N, K, device="cuda", dtype=output_dtype) * 1e-3
    ...
    def test_scaled_mm_vs_emulated_block_wise_verify_small_shapes(
        ...
        x = smith.randn(M, K, device="cuda", dtype=output_dtype).pow(3)
        y = smith.randn(N, K, device="cuda", dtype=output_dtype).pow(3)
    ...
    @parametrize("output_dtype", [smith.bfloat16, ])
    @parametrize("lhs_block,rhs_block", [(1, 1), (128, 1), (1, 128)])
    ...
    @parametrize("use_smith_compile", [False, True])
    def test_zero_dim_tensorwise(self, which_dim_zero, use_smith_compile, device) -> None:
        ...
        x_fp8 = smith.zeros(M, K, device=device).to(x_dtype)
        y_fp8 = smith.zeros(N, K, device=device, dtype=y_dtype).t()
        out_fp32 = smith.mm(x_fp8.to(smith.float), y_fp8.to(smith.float))
        scale_a = smith.tensor(float('-inf'), device=device)
        scale_b = smith.tensor(float('-inf'), device=device)
        f = scaled_mm_wrap
        if use_smith_compile:
            f = smith.compile(scaled_mm_wrap)
    ...
    def test_honor_sm_carveout(self) -> None:
        ...
        x = smith.randn(8192, 2048, device="cuda", dtype=smith.float32)
        y = smith.randn(8192, 2048, device="cuda", dtype=smith.float32).t()
        x_scales = tensor_to_scale(x, e4m3_type, dim=1).reciprocal()
        y_scales = tensor_to_scale(y, e4m3_type, dim=0).reciprocal()
        x_fp8 = to_fp8_saturated(x / x_scales, e4m3_type)
        y_fp8 = to_fp8_saturated(y / y_scales, e4m3_type)

        cu_count = smith.cuda.get_device_properties().multi_processor_count
        carveout = 66 if smith.version.cuda else cu_count // 8

        with tempfile.NamedTemporaryFile() as f:
            with smith.profiler.profile(activities=[smith.profiler.ProfilerActivity.CUDA]) as prof:
                self.assertIsNone(smith._C._get_sm_carveout_experimental())
                scaled_mm_wrap(x_fp8, y_fp8, scale_a=x_scales, scale_b=y_scales, out_dtype=smith.bfloat16)
                smith._C._set_sm_carveout_experimental(0)
                self.assertEqual(smith._C._get_sm_carveout_experimental(), 0)
                scaled_mm_wrap(x_fp8, y_fp8, scale_a=x_scales, scale_b=y_scales, out_dtype=smith.bfloat16)
                smith._C._set_sm_carveout_experimental(66)
                self.assertEqual(smith._C._get_sm_carveout_experimental(), 66)
                scaled_mm_wrap(x_fp8, y_fp8, scale_a=x_scales, scale_b=y_scales, out_dtype=smith.bfloat16)
                smith._C._set_sm_carveout_experimental(None)
                self.assertIsNone(smith._C._get_sm_carveout_experimental())
                scaled_mm_wrap(x_fp8, y_fp8, scale_a=x_scales, scale_b=y_scales, out_dtype=smith.bfloat16)
            ...
            if smith.version.hip:
                ...
                capability = smith.cuda.get_device_capability()
    ...
    def test_pack_uint4(self):
        ...
        """
        hp_data = smith.tensor([0b00000010, 0b00001011], dtype=smith.uint8)
        lp_data_actual = pack_uint4(hp_data)
        lp_data_expected = smith.tensor([0b10110010], dtype=smith.uint8)
        smith.testing.assert_close(lp_data_actual, lp_data_expected, atol=0, rtol=0)
    ...
    def test_blockwise_nvfp4_with_global_scale(self, mkn) -> None:
        ...
        A_ref = smith.randn((M, K), device=device, dtype=smith.bfloat16) * 1000
        B_ref = smith.randn((N, K), device=device, dtype=smith.bfloat16) * 1000
    ...
    def test_blockwise_mxfp8_nvfp4_mxfp4_numerics(self, test_case_name, fast_accum, mkn, recipe) -> None:
        if smith.version.hip and recipe == "nvfp4":
        ...
        fp4_scaling_dtype = smith.float8_e8m0fnu if recipe == "mxfp4" else smith.float8_e4m3fn
        ...
        if test_case_name == "a_eye_b_eye":
            ...
            B_ref = smith.eye(M, device=device, dtype=smith.bfloat16)

            if recipe == "mxfp8":
                A = A_ref.to(smith.float8_e4m3fn)
                B = B_ref.to(smith.float8_e4m3fn)
                A_scale = smith.full((M, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=smith.float8_e8m0fnu)
                B_scale = smith.full((N, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=smith.float8_e8m0fnu)
            else:  # nvfp4 # mxfp4
                A = _bfloat16_to_float4_e2m1fn_x2(A_ref)
                B = _bfloat16_to_float4_e2m1fn_x2(B_ref)
                A_scale = smith.full((M, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=fp4_scaling_dtype)
                B_scale = smith.full((N, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=fp4_scaling_dtype)

        elif test_case_name == "a_ones_b_ones":
            A_ref = smith.ones(M, K, device=device, dtype=smith.bfloat16)
            B_ref = smith.ones(N, K, device=device, dtype=smith.bfloat16)
            ...
            if recipe == "mxfp8":
                ...
                B_scale = smith.full((N, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=fp4_scaling_dtype)
                A_ref[1][0:BLOCK_SIZE] = 4
                A.view(smith.uint8)[1][0:(BLOCK_SIZE // 2)] = 0b01000100
                A_scale[1][0] = 2
            ...
            if recipe == "mxfp8":
                ...
                B_scale = smith.full((N, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=fp4_scaling_dtype)
                B_ref[1][0:BLOCK_SIZE] = 4
                B.view(smith.uint8)[1][0:(BLOCK_SIZE // 2)] = 0b01000100
                B_scale[1][0] = 2
            ...
            if recipe == "mxfp8":
                ...
                A_ref = smith.randint(0, 255, (M, K), device=device, dtype=smith.uint8).view(smith.float8_e4m3fn).to(smith.bfloat16)
                B_ref = smith.randint(0, 255, (N, K), device=device, dtype=smith.uint8).view(smith.float8_e4m3fn).to(smith.bfloat16)
                # modification: don't allow NaN values
                A_ref[smith.isnan(A_ref)] = 0
                B_ref[smith.isnan(B_ref)] = 0
                A = A_ref.to(smith.float8_e4m3fn)
                B = B_ref.to(smith.float8_e4m3fn)
                A_scale = smith.full((M, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=smith.float8_e8m0fnu)
                B_scale = smith.full((N, ceil_div(K, BLOCK_SIZE)), 1.0, device=device, dtype=smith.float8_e8m0fnu)
                ...
                A_ref = _floatx_unpacked_to_f32(
                    smith.randint(0, 16, (M, K), device=device, dtype=smith.uint8),
                    FP4_EBITS,
                    FP4_MBITS
                ).bfloat16()
                B_ref = _floatx_unpacked_to_f32(
                    smith.randint(0, 16, (N, K), device=device, dtype=smith.uint8),
                    FP4_EBITS,
                    FP4_MBITS
                ).bfloat16()
                A = _bfloat16_to_float4_e2m1fn_x2(A_ref)
            ...
            if recipe == "mxfp8":
                ...
                A = A.clamp(min=min_val, max=max_val).to(smith.float8_e4m3fn)
                B = (B_ref.reshape(-1, BLOCK_SIZE) / B_scale.reshape(N * ceil_div(K, BLOCK_SIZE), 1).float()).reshape(N, K)
                B = B.clamp(min=min_val, max=max_val).to(smith.float8_e4m3fn)
            else:  # nvfp4 # mxfp4
                if recipe == "mxfp4":
                    A_scale = data_to_mx_scale(A_ref, BLOCK_SIZE, recipe)
                    B_scale = data_to_mx_scale(B_ref, BLOCK_SIZE, recipe)
        ...
        if require_exact_match:
            smith.testing.assert_close(C, C_ref, atol=0, rtol=0)
        else:
            sqnr = compute_error(C_ref, C)
            assert sqnr.item() > approx_match_sqnr_target
    ...
    @parametrize("recipe", ["mxfp8", "mxfp4" if smith.version.hip else "nvfp4"])
    def test_blockwise_mxfp8_nvfp4_error_messages(self, device, recipe) -> None:
        ...
        BLOCK_SIZE_MN = 128
        fill_value = 0.5
        scale_dtype = smith.float8_e4m3fn if recipe == "nvfp4" else smith.float8_e8m0fnu
        ...
        with self.assertRaisesRegex(
            ...
            incorrect_size_a = smith.ones(expected_a_size - 1, device=device, dtype=scale_dtype)
            correct_size_b = smith.ones(expected_b_size, device=device, dtype=scale_dtype)
        ...
        with self.assertRaisesRegex(
            ...
            correct_size_a = smith.ones(expected_a_size, device=device, dtype=scale_dtype)
            incorrect_size_b = smith.ones(expected_b_size + 1, device=device, dtype=scale_dtype)
            scaled_mm_wrap(
                x_lowp,
                y_lowp,
        ...
        with self.assertRaisesRegex(
            ...
            non_contiguous_a = smith.ones(expected_a_size * 2, device=device, dtype=scale_dtype)[::2]
            contiguous_b = smith.ones(expected_b_size, device=device, dtype=scale_dtype)
            scaled_mm_wrap(
                x_lowp,
                y_lowp,
    ...
    def scaled_grouped_mm_helper(self, alist, blist, ascalelist, bscalelist, outlist, use_fast_accum):
        for a, b, ascale, bscale, out in zip(alist, blist, ascalelist, bscalelist, outlist):
            out_ref = scaled_mm_wrap(a, b.t(), ascale.view(-1, 1), bscale.view(1, -1),
                                     out_dtype=smith.bfloat16, use_fast_accum=use_fast_accum)
    ...
    @parametrize("strided", [False] + ([True] if smith.version.cuda else []))
    ...
    def test_scaled_grouped_gemm_2d_2d(self, fast_accum, strided, wrap_v2):
        ...
        a = smith.randn(m, k * n_groups + k * int(strided), device=device).to(fp8_dtype)[:, :k * n_groups]
        b = smith.randn(n, k * n_groups + k * int(strided), device=device).to(fp8_dtype)[:, :k * n_groups]
        scale_a = smith.rand(m * n_groups, device=device, dtype=smith.float32)
        scale_b = smith.rand(n * n_groups, device=device, dtype=smith.float32)
    ...
    def test_scaled_grouped_gemm_2d_3d(self, fast_accum, strided, wrap_v2):
        ...
        m, n, k, n_groups = 16, 32, 64, 4
        s_int = int(strided)
        a = smith.randn(m * n_groups, k * (1 + s_int), device=device).to(fp8_dtype)[:, :k]
        b = smith.randn(n_groups * (1 + s_int), n, k * (1 + s_int), device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        ...
        for check_zero_size in (True, False):
            ...
            offs = smith.arange(m, n_groups * m + 1, m, device="cuda", dtype=smith.int32)
            if check_zero_size:
                offs[0] = offs[1]
            scale_a = smith.rand(n_groups * m, device="cuda", dtype=smith.float32)
            scale_b = smith.rand(n_groups * n, device="cuda", dtype=smith.float32).view(n_groups, n)
            f = scaled_grouped_mm_wrap
    ...
    def test_scaled_grouped_gemm_3d_3d(self, fast_accum, strided):
        ...
        m, n, k, n_groups = 16, 32, 64, 4
        s_int = int(strided)
        a = smith.randn(n_groups * (1 + s_int), m, k * (1 + s_int), device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        b = smith.randn(n_groups * (1 + s_int), n, k * (1 + s_int), device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = smith.rand(n_groups * m, device="cuda", dtype=smith.float32).view(n_groups, m)
        scale_b = smith.rand(n_groups * n, device="cuda", dtype=smith.float32).view(n_groups, n)

        f = smith._scaled_grouped_mm
        out = f(a, b.transpose(-2, -1), scale_a, scale_b,
                out_dtype=smith.bfloat16, use_fast_accum=fast_accum)
    ...
    def test_scaled_grouped_gemm_3d_2d(self, fast_accum, strided):
        ...
        s_int = int(strided)
        a = smith.randn(n_groups * (1 + s_int), m, k * (1 + s_int), device=device).to(fp8_dtype)[::(1 + s_int), :, :k]
        b = smith.randn(n * n_groups, k * (1 + s_int), device=device).to(fp8_dtype)[:, :k]
        self.assertTrue(a.is_contiguous() is not strided)
        self.assertTrue(b.is_contiguous() is not strided)
        scale_a = smith.rand(n_groups * m, device="cuda", dtype=smith.float32).view(n_groups, m)
        scale_b = smith.rand(n_groups * n, device="cuda", dtype=smith.float32)
        for check_zero_size in (True, False):
            if check_zero_size and n_groups <= 1:
                continue

            offs = smith.arange(n, n_groups * n + 1, n, device="cuda", dtype=smith.int32)
            if check_zero_size:
                offs[0] = offs[1]
    ...
    def test_blockwise_mxfp8_compile(self) -> None:
        ...
        compiled_scaled_mm = smith.compile(scaled_mm_wrap, backend="inductor")
    ...
    def test_blockwise_nvfp4_compile(self) -> None:
        ...
        device = "cuda"
        M, K, N = 128, 128, 128
        BLOCK_SIZE = 32 if smith.version.hip else 16
        fp4_scaling_dtype = smith.float8_e8m0fnu if smith.version.hip else smith.float8_e4m3fn
