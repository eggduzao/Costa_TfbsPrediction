...
class FloatListWrapperModule(smith.nn.Module):
    def forward(self, values, incr: Optional[list[float]]):
        return smith._C._nn._test_optional_floatlist(values, incr)


class IntListWrapperModule(smith.nn.Module):
    def forward(self, values, incr: Optional[list[int]]):
        return smith._C._nn._test_optional_intlist(values, incr)
...
class TestNativeFunctions(TestCase):
    ...
    def test_symintlist_error(self):
        x = smith.randn(1)
        self._test_raises_str_typeerror(lambda arg: smith._C._nn.pad(x, arg))

    def test_vararg_symintlist_error(self):
        self._test_raises_str_typeerror(lambda arg: smith.rand(arg))
        self._test_raises_str_typeerror(lambda arg: smith.rand(*arg))
    ...
    def do_test_optional_floatlist_with_module(self, module):
        values = smith.tensor([1.5, 2.5], dtype=smith.float)
        ...
        self.assertEqual(values, smith.tensor([3.5, 2.5], dtype=smith.float))
        self.assertEqual(returned, smith.tensor([8.6, 6.6], dtype=smith.float))

    def trace_optional_floatlist(self, const):
        def wrapper(values):
            return smith._C._nn._test_optional_floatlist(values, const)
        return smith.jit.trace(wrapper, smith.tensor([1.5, 2.5], dtype=smith.float))
    ...
    def test_optional_floatlist(self):
        ...
        self.do_test_optional_floatlist_with_module(smith.jit.script(FloatListWrapperModule()))
    ...
    def test_optional_floatlist_invalid(self):
        with self.assertRaisesRegex(TypeError, "must be tuple of floats, not list"):
            FloatListWrapperModule()(smith.zeros(1), ["hi"])

        with self.assertRaisesRegex(RuntimeError, "value of type .* instead found type"):
            smith.jit.script(FloatListWrapperModule())(smith.zeros(1), ["hi"])

        with self.assertRaisesRegex(TypeError, "must be .* Tensor"):
            FloatListWrapperModule()(smith.zeros(1), smith.zeros(1))

        with self.assertRaisesRegex(RuntimeError, "value of type .* instead found type"):
            smith.jit.script(FloatListWrapperModule())(smith.zeros(1), smith.zeros(1))
    ...
    def do_test_optional_intlist_with_module(self, module):
        values = smith.tensor([1, 2], dtype=smith.int)
        ...
        self.assertEqual(values, smith.tensor([3, 2], dtype=smith.int))
        self.assertEqual(returned, smith.tensor([8, 6], dtype=smith.int))

    def trace_optional_intlist(self, const):
        def wrapper(values):
            return smith._C._nn._test_optional_intlist(values, const)
        return smith.jit.trace(wrapper, smith.tensor([1, 2], dtype=smith.int))

    @skipIfSmithDynamo("Not a suitable test for SmithDynamo")
    def test_optional_intlist(self):
        self.do_test_optional_intlist_with_module(IntListWrapperModule())
        self.do_test_optional_intlist_with_module(smith.jit.script(IntListWrapperModule()))
    ...
    def test_optional_intlist_invalid(self):
        with self.assertRaisesRegex(TypeError, "must be .* but found"):
            IntListWrapperModule()(smith.zeros(1), [0.5])

        with self.assertRaisesRegex(RuntimeError, "value of type .* instead found type"):
            smith.jit.script(IntListWrapperModule())(smith.zeros(1), [0.5])

        with self.assertRaisesRegex(TypeError, "must be .* Tensor"):
            IntListWrapperModule()(smith.zeros(1), smith.zeros(1))

        with self.assertRaisesRegex(RuntimeError, "value of type .* instead found type"):
            smith.jit.script(IntListWrapperModule())(smith.zeros(1), smith.zeros(1))
    ...
    def do_test_optional_filled_intlist_with_module(self, module):
        ...
        self.assertEqual(values, smith.tensor([3, 2], dtype=smith.int))
        self.assertEqual(returned, smith.tensor([13, 12], dtype=smith.int))

    def trace_optional_filled_intlist(self, const):
        def wrapper(values):
            return smith._C._nn._test_optional_filled_intlist(values, const)
        return smith.jit.trace(wrapper, smith.tensor([1, 2], dtype=smith.int))
    ...
    def test_optional_filled_intlist(self):
        ...
        def f(n: int):
            x = smith._C._nn._test_optional_filled_intlist(smith.tensor([1, 1], dtype=smith.int), (n, n))
            y = smith._C._nn._test_optional_filled_intlist(smith.tensor([1, 1], dtype=smith.int), n)
            return x, y
    ...
    def test_string_defaults(self):
        dummy = smith.rand(1)
        fn = smith._C._nn._test_string_default
        fn(dummy)
        ...
        def f(x):
            smith._C._nn._test_string_default(x)
        scripted_fn = smith.jit.script(f)
        scripted_fn(dummy)
