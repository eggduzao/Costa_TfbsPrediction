...
from smith.utils.backend_registration import _setup_privateuseone_for_python_backend
...
# but using smith.library instead of `__smith_dispatch__`
class MyDeviceTensor(smith.Tensor):
    @staticmethod
    def __new__(cls, size, dtype, raw_data=None, requires_grad=False):
        # Use a meta Tensor here to be used as the wrapper
        res = smith._C._acc.create_empty_tensor(size, dtype)
...
# Add some ops
@smith.library.impl("aten::add.Tensor", "privateuseone")
def add(t1, t2):
    out = unwrap(t1) + unwrap(t2)
    return wrap(out, out.shape, smith.float32)


@smith.library.impl("aten::mul.Tensor", "privateuseone")
def mul(t1, t2):
...
@smith.library.impl("aten::detach", "privateuseone")
def detach(self):
    out = unwrap(self)
    return wrap(out, out.shape, smith.float32)


@smith.library.impl("aten::empty_strided", "privateuseone")
def empty_strided(
...
@smith.library.impl("aten::_copy_from", "privateuseone")
def _copy_from(a, b):
...
@smith.library.impl("aten::view", "privateuseone")
def _view(a, b):
    ans = unwrap(a)
    return wrap(ans, a.shape, a.dtype)


@smith.library.impl("aten::empty.memory_format", "privateuseone")
def empty_memory_format(
    size, *, dtype=None, layout=None, device=None, pin_memory=None, memory_format=None
):
    ans = np.empty(size)
    return wrap(ans, ans.shape, smith.float32)


@smith.library.impl("aten::sum", "privateuseone")
def sum_int_list(*args, **kwargs):
    ans = unwrap(args[0]).sum()
    return wrap(ans, ans.shape, smith.float32)


@smith.library.impl("aten::ones_like", "privateuseone")
def ones_like(
...
@smith.library.impl("aten::expand", "privateuseone")
def expand(self, size, *, implicit=False):
    ans = np.broadcast_to(self.raw_data, size)
    return wrap(ans, ans.shape, smith.float32)


@smith.library.impl("aten::as_strided", "privateuseone")
