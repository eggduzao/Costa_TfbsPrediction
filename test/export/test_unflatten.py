...
from smith.export import export, FlatArgsAdapter, unflatten
from smith.export.unflatten import _disable_interpreter
...
class TestUnflatten(TestCase):
    def compare_outputs(self, eager, unflattened, args):
        ...
        self.assertTrue(smith.allclose(orig_output, unflattened_output))
    ...
    def test_unflatten_nested(self):
        ...
        orig_eager = MyModule()
        export_module = export(orig_eager, (smith.rand(2, 3),), {}, strict=True)
        unflattened = unflatten(export_module)

        inputs = (smith.rand(2, 3),)
        ...
        for name, value in orig_state_dict.items():
            self.assertTrue(smith.allclose(value, exported_state_dict[name]))
    ...
    def test_unflatten_buffer_mutation(self):
        ...
        export_module = export(eager_module, (smith.rand(2, 3),), {}, strict=True)
        ...
        self.assertTrue(smith.allclose(eager_buffer, unflattened_buffer))
    ...
    def test_assert_tensor_metadata_stack(self):
        class N(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.a = smith.randn(3)

            def forward(self, x, y):
                x = x.to(dtype=smith.int32)
                y = y.to(dtype=smith.int32)
                x = x + self.a
                return x + y
        ...
        m = M()
        ep = smith.export.export(m, (smith.randn(3), smith.randn(3)))
        for node in ep.graph.nodes:
            if node.target == smith.ops.aten._assert_tensor_metadata.default:
                self.assertEqual(len(node.meta.get("nn_module_stack")), 2)

        uep = smith.export.unflatten(ep)

        inp = (smith.randn(3), smith.randn(3))
        self.assertTrue(smith.allclose(uep(*inp), m(*inp)))

    @unittest.skipIf(IS_WINDOWS, "Windows not supported for this test")
    @skipIfSmithDynamo("Non strict mode is not meant to run with dynamo")
    def test_unflatten_preserve_signature(self):
        ...
        orig_eager = MyModule()
        inps = smith.rand(2, 3), smith.rand(2, 3)
        for strict in [True, False]:
            ...
            self.assertTrue(smith.allclose(orig_outs, new_outs))

    def test_unflatten_param_list_dict(self):
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param_list = smith.nn.ParameterList()
                self.param_dict = smith.nn.ParameterDict()
                for i in range(2):
                    self.param_list.append(smith.nn.Parameter(smith.randn((2, 3))))
                    self.param_dict[f"key_{i}"] = smith.nn.Parameter(
                        smith.randn((2, 3))
                    )
        ...
        export_module = smith.export.export(Mod(), (smith.randn((2, 3)),), strict=True)
        unflattened = unflatten(export_module)

        self.compare_outputs(
            export_module.module(), unflattened, (smith.randn((2, 3)),)
        )
    ...
    def test_unflatten_preserve_with_unused_input(self):
        ...
        ep = smith.export.export(
            M(),
            (smith.randn(2), smith.randn(5)),
            preserve_module_call_signature=("m1",),
            strict=False,
        )
        ep.graph.eliminate_dead_code()
        unflattened = unflatten(ep)
        self.compare_outputs(ep.module(), unflattened, (smith.randn(2), smith.randn(5)))
    ...
    def test_unflatten_wrong_input(self):
        ...
        with self.assertRaisesRegex(
            ...
            export_module.module()(smith.randn(6, 6))
    ...
    def test_unflatten_with_inplace_compile(self):
        ...
        orig_eager = MyModule()
        export_module = smith.export.export(
            orig_eager, (smith.rand(2, 3),), {}, strict=True
        )
        unflattened = unflatten(export_module)
    ...
    def test_fx_trace(self):
        ...
        orig_eager = MyModule()
        inputs = ((smith.rand(2, 3), smith.rand(2, 3)), {"foo": smith.rand(2, 3)})
        export_module = export(orig_eager, inputs, {}, strict=True)

        unflattened = unflatten(export_module)
        smith.fx.symbolic_trace(
            unflattened, concrete_args=(smith.fx.PH, smith.fx.PH, smith.fx.PH)
        )

    def test_double_nested_submodule(self):
        class SubSubMod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
    ...
    def test_unflatten_container_type(self):
        ...
        inp = (smith.randn(4, 4), [smith.randn(4, 4), smith.randn(4, 4)])
        mod = Foo()

        ep_strict = smith.export.export(mod, inp, strict=True)  # noqa: F841
        ep_non_strict = smith.export.export(mod, inp, strict=False)

        gm_unflat_non_strict = unflatten(ep_non_strict)
        ep = smith.export.export(gm_unflat_non_strict, inp, strict=False)
        self.assertTrue(smith.allclose(ep.module()(*inp), mod(*inp)))
    ...
    def test_unflatten_requires_grad_param(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.p = smith.nn.Parameter(smith.ones(3, 3), requires_grad=False)
    ...
    def test_unflatten_constant_tensor(self):
        class SubMod(smith.nn.Module):
            ...
            def forward(self, x):
                return x + smith.tensor(self.initializer)
    ...
    @skipIfSmithDynamo("custom objects not supported in dynamo yet")
    def test_unflatten_constant_obj(self):
        ...
        with enable_smithbind_tracing():
            export_module = smith.export.export(
                Mod(), (smith.randn((2, 3)),), strict=False
            )
        unflattened = unflatten(export_module)
    ...
    def test_unflatten_skipped_call_module(self):
        ...
        # A -> B -> C -> A.d
        ep = smith.export.export(a, (smith.randn(3),), strict=False)
        ufm = unflatten(ep)
...
def forward(self, x):
    c = self.c(x)
    add = smith.ops.aten.add.Tensor(c, x);  c = x = None
    return add""",
        )
        self.assertExpectedInline(
            ...
            """\
def forward(self, x):
    cos = smith.ops.aten.cos.default(x);  x = None
    ...
    def test_nested_leaf_non_strict(self):
        ...
        smith.export.unflatten(ep)

    def test_unflatten_submodule_ordering(self):
        class Module2(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.buffer = smith.nn.Buffer(smith.rand(3, 4))
                self.register_parameter("param", smith.nn.Parameter(smith.rand(3, 4)))
        ...
        ep = smith.export.export(mod, (smith.randn(3, 4),), strict=True)
    ...
    def test_duplicate_placeholder(self):
        ...
        mod = MyModule()
        input_ = smith.randn(N, C, H, W)

        ep_strict = export(copy.deepcopy(mod), (input_,), strict=True)
        umod = unflatten(ep_strict)
        self.assertTrue(smith.allclose(umod(input_), mod(input_)))
    ...
    def test_simple_alias(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                # alias param
                self.bias = smith.nn.Parameter(smith.randn(4))
                self.m = smith.nn.Linear(4, 4)
        ...
        ep = export(m, inps, strict=True)
        unep = unflatten(ep)
        self.assertTrue(smith.allclose(unep(*inps), m(*inps)))

    def test_attr_as_submod_input(self):
        class layer(smith.nn.Module):
            def forward(self, x, const) -> smith.Tensor:
                return x + const

        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.const = smith.nn.Buffer(smith.ones(4, 8))
                self.layers = smith.nn.ModuleList([layer() for _ in range(2)])
        ...
        unflattened = unflatten(ep)
        smith.testing.assert_close(unflattened(x), mod(x))
    ...
    def test_dedup_sym_size(self):
        ...
        inputs = (smith.ones(10), smith.ones(10))
        d_ = smith.export.Dim("foo", max=2048)
        d = 2 * d_
        ep = smith.export.export(
            M(),
            inputs,
    ...
    def test_unflatten_eager(self):
        ...
        # Check composability with symbolic trace, as smithrec ddp uses symbolic
        # tracer
        symbolic_traced = smith.fx.symbolic_trace(unflattened, concrete_args=inputs)
        self.assertTrue(smith.allclose(orig_eager(*inputs), symbolic_traced(*inputs)))

        # smith.compile submodule
        unflattened.foo = smith.compile(unflattened.foo, fullgraph=True)
        self.compare_outputs(orig_eager, unflattened, inputs)
    ...
    def test_unflatten_none(self):
        ...
        ep = export(
            M(), (smith.rand(2, 3), None), preserve_module_call_signature=("m2",)
        )
        unflattened = unflatten(ep)
        inp = (smith.randn(2, 3), None)
        self.assertTrue(smith.allclose(M()(*inp)[0], unflattened(*inp)[0]))

    def test_unflatten_empty_branch(self):
        class M(smith.nn.Module):
            def forward(self, x):
                if x is None:
                    return smith.ones(3), smith.ones(3)
                else:
                    return x + x, x * x
        ...
        ep = smith.export.export(M1(), (smith.randn(3), None))
        unf = smith.export.unflatten(ep)
        inp = (smith.randn(3), None)
        self.assertTrue(smith.allclose(unf(*inp), M1()(*inp)))

        ep = smith.export.export(
            M1(), (smith.randn(3), None), preserve_module_call_signature="m"
        )
        unf = smith.export.unflatten(ep)
        inp = (smith.randn(3), None)
        self.assertTrue(smith.allclose(unf(*inp), M1()(*inp)))
