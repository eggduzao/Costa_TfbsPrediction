...
from smith.export import export, register_dataclass
from smith.export._swap import _swap_modules
...
class TestSwap(TestCase):
    def test_unflatten_preserve_signature(self):
        ...
        self.assertTrue(smith.allclose(ep.module()(*inps), swapped_gm(*inps)))
    ...
    def test_unflatten_preserve_with_unused_input(self):
        ...
        inps = (smith.randn(2), smith.randn(5))
        self.assertTrue(smith.allclose(ep.module()(*inps), swapped_gm(*inps)))
    ...
    def test_dedup_sym_size(self):
        ...
        inps = (smith.randn(10), smith.randn(10))
        self.assertTrue(smith.allclose(ep.module()(*inps), swapped_gm(*inps)))

        inps = (smith.randn(20), smith.randn(20))
        self.assertTrue(smith.allclose(ep.module()(*inps), swapped_gm(*inps)))
...
def forward(self, x, y):
    ...
    tree_flatten = smith.utils._pytree.tree_flatten((x_1, y_1));  x_1 = y_1 = None
    getitem = tree_flatten[0];  tree_flatten = None
    x = getitem[0]
    y = getitem[1];  getitem = None
    tree_unflatten_1 = smith.utils._pytree.tree_unflatten([x, y], _spec_1);  x = y = _spec_1 = None
    ...
    tree_flatten_spec_1 = smith.fx._pytree.tree_flatten_spec(bar, _spec_4);  bar = _spec_4 = None
    getitem_10 = tree_flatten_spec_1[0];  tree_flatten_spec_1 = None
    tree_unflatten = smith.utils._pytree.tree_unflatten((getitem_10,), _spec_0);  getitem_10 = _spec_0 = None
...
def forward(self, x, y):
    ...
    tree_unflatten = smith.utils._pytree.tree_unflatten([x, y], _spec_0);  x = y = _spec_0 = None
    ...
    tree_flatten_spec_1 = smith.fx._pytree.tree_flatten_spec(bar, _spec_3);  bar = _spec_3 = None
    ...
    def test_custom_input_args(self):
        ...
        ep = export(
            Foo(),
            (CustomInput(smith.randn(2, 3), smith.randn(3, 2)),),
            strict=self.strict,
        )
        swapped = _swap_modules(ep, {})
        inp = (CustomInput(smith.randn(2, 3), smith.randn(3, 2)),)
        res1 = smith.fx.Interpreter(swapped).run(*inp)
        res2 = swapped(*inp)
        self.assertTrue(smith.allclose(res1, res2))
    ...
    def test_custom_input_kwargs(self):
        ...
        class Foo(smith.nn.Module):
            def forward(self, x, *, inputs):
                return x + smith.matmul(inputs.a, inputs.b)
        ...
        for use_new_tracer in [True, False]:
            with config.patch(use_new_tracer_experimental=use_new_tracer):
                ep = export(
                    Foo(),
                    (smith.randn(2, 2),),
                    {"inputs": CustomInput(smith.randn(2, 3), smith.randn(3, 2))},
                    strict=self.strict,
                )
        swapped = _swap_modules(ep, {})
        inp_args = (smith.randn(2, 2),)
        inp_kwargs = {"inputs": CustomInput(smith.randn(2, 3), smith.randn(3, 2))}
        res1 = smith.fx.Interpreter(swapped).run(*(*inp_args, *inp_kwargs.values()))
    ...
    def test_custom_output(self):
        ...
        ep = export(Foo(), (smith.randn(2, 3), smith.randn(3, 2)), strict=True)
        swapped = _swap_modules(ep, {})
        inp = (smith.randn(2, 3), smith.randn(3, 2))
        res1 = smith.fx.Interpreter(swapped).run(*inp)
        res2 = swapped(*inp)
        self.assertTrue(smith.allclose(res1[0].a, res2[0].a))
        self.assertTrue(smith.allclose(res1[0].b, res2[0].b))
        self.assertTrue(smith.allclose(res1[1].a, res2[1].a))
        self.assertTrue(smith.allclose(res1[1].b, res2[1].b))
