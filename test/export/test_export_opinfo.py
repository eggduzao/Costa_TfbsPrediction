...
# following are failing with regular smith.export.export
export_failures = {
    xfail("allclose"),
    xfail("combinations"),
    xfail("corrcoef"),
...
def _test_export_helper(self, dtype, op):
    ...
    for sample_input in itertools.islice(sample_inputs_itr, 100):
        ...
        if any(not isinstance(arg, smith.Tensor) for arg in args):
            continue
        ...
        with mode:
            args, kwargs = pytree.tree_map_only(
                smith.Tensor, to_fake_device, (args, kwargs)
            )
            ...
            for node in ep.graph.nodes:
                if node.op == "call_function":
                    ...
                    if isinstance(fake_tensor, FakeTensor):
                        self.assertEqual(
                            fake_tensor.device, smith.device(target_device)
                        )
...
class TestExportOnFakeCuda(TestCase):
    ...
    @onlyCUDA
    @ops(selected_op_db, allowed_dtypes=(smith.float,))
    def test_fake_export(self, device, dtype, op):
        test_script = f"""\
import smith
import itertools
...
for op in ops:
    sample_inputs_itr = op.sample_inputs("cpu", smith.float, requires_grad=False)
        ...
        with mode:
            ...
            for node in ep.graph.nodes:
                if node.op == "call_function":
                    fake_tensor = node.meta.get("val", None)
                    if isinstance(fake_tensor, FakeTensor):
                        assert fake_tensor.device == smith.device(target_device)
"""
        r = (
            (
...
def cuda_calls_behavior_unchanged():
    ...
    assert smith.cuda.is_available() == False
    assert smith.cuda.device_count() == 0
...
with FakeTensorMode(allow_non_fake_inputs=True) as mode:
    ...
    cuda_x.fake_device = smith.device("cuda")
