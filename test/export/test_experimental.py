...
from smith.export.experimental import _export_forward_backward, _sticky_export
from smith.export.graph_signature import OutputKind
from smith.testing import FileCheck
from smith.testing._internal.common_utils import TEST_CUDA
...
def _register_blockmask_pytree():
    ...
    from smith.nn.attention.flex_attention import BlockMask
    from smith.utils._pytree import register_pytree_node, SUPPORTED_NODES
    ...
    if BlockMask not in SUPPORTED_NODES:
        register_pytree_node(
            ...
            serialized_type_name="smith.nn.attention.flex_attention.BlockMask",
        )
...
@unittest.skipIf(not smith._dynamo.is_dynamo_supported(), "dynamo isn't supported")
class TestExperiment(TestCase):
    def test_joint_basic(self) -> None:
        class Module(smith.nn.Module):
            ...
            def forward(self, x):
                return self.loss(
                    self.linear(x).softmax(dim=0), smith.tensor([1.0, 0.0, 0.0])
                )
        ...
        with smith._export.config.patch(use_new_tracer_experimental=True):
            ep = smith.export.export(m, example_inputs, strict=True)
...
def forward(self, p_linear_weight, p_linear_bias, c_lifted_tensor_0, x):
    view = smith.ops.aten.view.default(x, [1, 3]);  x = None
    permute = smith.ops.aten.permute.default(p_linear_weight, [1, 0]);  p_linear_weight = None
    addmm = smith.ops.aten.addmm.default(p_linear_bias, view, permute);  p_linear_bias = permute = None
    view_1 = smith.ops.aten.view.default(addmm, [3]);  addmm = None
    _softmax = smith.ops.aten._softmax.default(view_1, 0, False);  view_1 = None
    alias = smith.ops.aten.alias.default(_softmax)
    clone = smith.ops.aten.clone.default(c_lifted_tensor_0);  c_lifted_tensor_0 = None
    _log_softmax = smith.ops.aten._log_softmax.default(_softmax, 0, False);  _softmax = None
    alias_1 = smith.ops.aten.alias.default(_log_softmax)
    mul = smith.ops.aten.mul.Tensor(_log_softmax, clone);  _log_softmax = None
    sum_1 = smith.ops.aten.sum.dim_IntList(mul, []);  mul = None
    neg = smith.ops.aten.neg.default(sum_1);  sum_1 = None
    div = smith.ops.aten.div.Scalar(neg, 1);  neg = None
    full_like = smith.ops.aten.full_like.default(div, 1, pin_memory = False, memory_format = smith.preserve_format)
    div_1 = smith.ops.aten.div.Scalar(full_like, 1);  full_like = None
    neg_1 = smith.ops.aten.neg.default(div_1);  div_1 = None
    expand = smith.ops.aten.expand.default(neg_1, [3]);  neg_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(expand, clone);  expand = clone = None
    alias_2 = smith.ops.aten.alias.default(alias_1);  alias_1 = None
    exp = smith.ops.aten.exp.default(alias_2);  alias_2 = None
    sum_2 = smith.ops.aten.sum.dim_IntList(mul_1, [0], True)
    mul_2 = smith.ops.aten.mul.Tensor(exp, sum_2);  exp = sum_2 = None
    sub = smith.ops.aten.sub.Tensor(mul_1, mul_2);  mul_1 = mul_2 = None
    alias_3 = smith.ops.aten.alias.default(alias);  alias = None
    mul_3 = smith.ops.aten.mul.Tensor(sub, alias_3);  sub = None
    sum_3 = smith.ops.aten.sum.dim_IntList(mul_3, [0], True)
    mul_4 = smith.ops.aten.mul.Tensor(alias_3, sum_3);  alias_3 = sum_3 = None
    sub_1 = smith.ops.aten.sub.Tensor(mul_3, mul_4);  mul_3 = mul_4 = None
    view_2 = smith.ops.aten.view.default(sub_1, [1, 3]);  sub_1 = None
    permute_1 = smith.ops.aten.permute.default(view_2, [1, 0])
    mm = smith.ops.aten.mm.default(permute_1, view);  permute_1 = view = None
    permute_2 = smith.ops.aten.permute.default(mm, [1, 0]);  mm = None
    sum_4 = smith.ops.aten.sum.dim_IntList(view_2, [0], True);  view_2 = None
    view_3 = smith.ops.aten.view.default(sum_4, [3]);  sum_4 = None
    permute_3 = smith.ops.aten.permute.default(permute_2, [1, 0]);  permute_2 = None
...
def forward(self, p_linear_weight, p_linear_bias, c_lifted_tensor_0, x):
    ...
    def _test_export_blockmask_with_mask_fn(self, make_mask_fn):
        from smith.nn.attention.flex_attention import create_block_mask
        ...
        x = smith.randn(2, 128, device="cuda")
    ...
    def test_export_with_default_kwargs(self):
        class FunctionalWrapper(smith.nn.Module):
...
def forward(self, args_0):
    ...
    l_args_0_ = L_args_0_
    out = smith._C._nn.linear(l_args_0_, l_self_modules_module_parameters_weight_, l_self_modules_module_parameters_bias_);  l_args_0_ = l_self_modules_module_parameters_weight_ = l_self_modules_module_parameters_bias_ = None
    ...
    def test_joint_dynamic(self) -> None:
        ...
        class Module(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.y = smith.nn.Parameter(smith.randn(3))

            def forward(self, x):
                x = smith.ones(x.shape[0], 3)
                return (self.y + x).sum()
    ...
    def test_joint_cifar10_backwards(self) -> None:
        ...
        # From Blacksmith's CIFAR10 example:
        # https://blacksmith.org/tutorials/beginner/blitz/cifar10_tutorial.html
        class Net(nn.Module):
            def __init__(self):
                super().__init__()
            ...
            def forward(self, x, labels):
                ...
                x = smith.flatten(x, 1)  # flatten all dimensions except batch
                x = F.relu(self.fc1(x))
                x = F.relu(self.fc2(x))
                x = self.fc3(x)
        ...
        net = Net()
        x = smith.randn(4, 3, 32, 32)
        labels = smith.ones(4, dtype=smith.int64)
        inputs = (x, labels)
    ...
    def test_joint_buffer_input_mutations(self):
        class Foo(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.l = smith.nn.Linear(4, 4)
                self.register_buffer("buf", smith.randn(4))
                self.loss = smith.nn.CrossEntropyLoss()
    ...
    def test_sticky_export(self):
        ...
        res2 = p.generate(inp)
        self.assertTrue(smith.allclose(res, res2))
    ...
    def test_sticky_export_dynamic(self):
        ...
        def callback(*args, **kwargs):
            ...
            flat_args, _ = smith.utils._pytree.tree_flatten((args, kwargs))
            collections = smith.export.ShapesCollection()
            for arg in flat_args:
                if isinstance(arg, smith.Tensor):
                    collections[arg] = {
                        i: smith.export.Dim.AUTO for i in range(len(arg.shape))
                    }
            return collections
...
def forward(self, x):
    ...
    def test_side_effect(self):
        ...
        with smith._dynamo.config.patch(replay_side_effects=False):
            _ = dynamo_graph_capture_for_export(Foo())(smith.randn(4, 4))
    ...
    def test_export_add_in_out_info(self):
        ...
        dct = {"a": smith.randn(2, 3), "b": smith.randn(2, 3)}
        lst = [smith.randn(2, 3), [smith.randn(2, 3), smith.randn(2, 3)]]
    ...
    def test_dynamo_graph_capture(self):
        ...
        def make_inputs():
            return (
                {"a": smith.randn(2, 3), "b": smith.randn(2, 3)},
                [smith.randn(2, 3), (smith.randn(2, 3),)],
                smith.randn(2, 3),
            )
    ...
    def test_dynamo_graph_capture_with_call_override(self):
        class _InterestingModule(smith.nn.Module):
            def __init__(self, module):
                super().__init__()
                self._module = module
    ...
    def test_dynamo_graph_capture_custom_pytree_type(self):
        ...
        def make_inputs():
            return (Bar(smith.randn(2, 3), smith.randn(2, 3)),)
...
def forward(self, args_0):
    ...
    stacked = smith.stack([y, y, y], dim = 0);  y = None
    ...
    def test_dynamo_graph_capture_full_tracing_context(self) -> None:
        ...
        def make_inputs(b: int):
            ret = (smith.randn(b, 3),)
            smith._dynamo.mark_dynamic(ret[0], 0)
            return ret
    ...
    def test_dynamo_graph_capture_dict_keys_getitem(self):
        ...
        block_mask = BlockMask(smith.randn(4))
...
def forward(self, args_0):
    ...
    def test_dynamo_graph_capture_side_effects(self):
        ...
        trace_inputs = make_inputs()
        with (
            smith._dynamo.config.patch(replay_side_effects=False),
    ...
    def test_dynamo_graph_capture_fx_graph_annotate_overlap_pass(self):
        class DummyOp(smith.autograd.Function):
            @staticmethod
        ...
        def input_fn():
            inputs = (smith.rand(2, 128, device="cuda", requires_grad=True),)
            grad_ins = (smith.rand(2, 128, device="cuda"),)
            return (
                *inputs,
                *grad_ins,
            )
        ...
        def forward(self, args_0, args_1):
            ...
            fw_out = smith.ops.higher_order.autograd_function_apply(fwd_body_0, bwd_body_0, l_fw_in_, args_tensor_mask = [True, False], non_differentiable_idx = []);  fwd_body_0 = bwd_body_0 = l_fw_in_ = None
    ...
    def test_dynamo_graph_capture_default_args(self):
        ...
        m = Module()
        ep = dynamo_graph_capture_for_export(m)(smith.randn(2, 3))
        test_inputs = (smith.randn(2, 3),)
    ...
    def test_restore_state_dict_basic(self):
        ...
        from smith.export import _restore_state_dict
        ...
        model = Model()
        gm = dynamo_graph_capture_for_export(model)(smith.randn(2, 4))
    ...
    def test_restore_state_dict_nested_modules(self):
        ...
        class SubModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.fc = smith.nn.Linear(8, 8)
                self.register_buffer("scale", smith.tensor(2.0))
        ...
        model = Model()
        gm = dynamo_graph_capture_for_export(model)(smith.randn(2, 8))
    ...
    def test_restore_state_dict_with_bound_method(self):
        ...
        model = Model()
        gm = dynamo_graph_capture_for_export(model.forward)(smith.randn(2, 4))
