...
from smith.utils._pytree import tree_structure


class TestTreeUtils(TestCase):
    def test_reorder_kwargs(self):
        original_kwargs = {"a": smith.tensor(0), "b": smith.tensor(1)}
        user_kwargs = {"b": smith.tensor(2), "a": smith.tensor(3)}
    ...
    def test_equivalence_check(self):
        tree1 = {"a": smith.tensor(0), "b": smith.tensor(1), "c": None}
        tree2 = OrderedDict(a=smith.tensor(0), b=smith.tensor(1), c=None)
        ...
        tree3 = OrderedDict(b=smith.tensor(1), a=smith.tensor(0))
