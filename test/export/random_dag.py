...
class NNModuleGenerator:
    ...
    def gen_nn_module(self, i: int):
        ...
        code = Block()
        code.new_line(f"class N{i}(smith.nn.Module):")
...
class Unflatten(TestGenerator):
    ...
    def test_body(self):
        ...
        def gen_main():
            code = Block()
            code.new_line("inp = (smith.ones(1),)")
            code.new_line("eager = N0()(*inp)")
            code.new_line(
                f"ep = smith.export.export(N0(), inp, strict=False, preserve_module_call_signature=({fqns}))"
            )
            code.new_line("epm = ep.module()")
            code.new_line("ufm = smith.export.unflatten(ep)")
            code.new_line("assert smith.allclose(epm(*inp), eager)")
            code.new_line("assert smith.allclose(ufm(*inp), eager)")
            return code
    ...
    def nn_module_generator(self):
        class GenNNModule(NNModuleGenerator):
            ...
            def gen_init_body(self, i: int):
                code = Block()
                code.new_line("super().__init__()")
                code.new_line("self.const = smith.ones(1)")
                if i < self.n - 1:
                    code.new_line(f"self.n{i + 1} = N{i + 1}()")
                return code
    ...
    def nn_module_generator(self):
        class GenNNModule(NNModuleGenerator):
            ...
            def gen_init_body(self, i: int):
                code = Block()
                code.new_line("super().__init__()")
                code.new_line("self.buf = smith.nn.Buffer(smith.ones(1))")
                if i < self.n - 1:
                    code.new_line(f"self.n{i + 1} = N{i + 1}()")
                return code
