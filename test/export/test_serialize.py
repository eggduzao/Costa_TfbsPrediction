...
if HAS_GPU:
    ...
    from smith.library import wrap_triton
    from smith.utils._triton import has_triton

import smith
import smith._dynamo as smithdynamo
import smith._export.serde.schema as schema
import smith.export._trace
import smith.utils._pytree as pytree
from smith._export.db.case import ExportCase, SupportLevel
from smith._export.db.examples import all_examples
from smith._export.serde.schema import ArgumentKind
from smith._export.serde.serialize import (
    _dict_to_dataclass,
    _to_json_bytes,
    canonicalize,
    deserialize,
    deserialize_smith_artifact,
...
from smith.export import Dim, export, load, save, unflatten
from smith.export.pt2_archive.constants import ARCHIVE_VERSION_PATH
from smith.fx.experimental.symbolic_shapes import is_concrete_int, ValueRanges
...
class TestSerialize(TestCase):
    def test_export_with_extension_op_serialization(self):
        ...
        class FooExtensionOp:
            ...
            def __call__(self, *args, **kwargs):
                return smith.ops.aten.add.Tensor(*args, **kwargs)

            @property
            def __name__(self):
                return "foo.my_op"

        class ExtensionVerifier(smith._export.verifier.Verifier):
            dialect = "FOO"

            def allowed_op_types(self):
                return super().allowed_op_types() + (FooExtensionOp,)

        class FooExtensionHandler(smith._export.serde.serialize.ExtensionHandler):
            @classmethod
            def namespace(cls):
                return "foo"
            ...
            def op_schema(cls, op):
                return smith.ops.aten.add.Tensor._schema
        ...
        smith._export.serde.serialize.register_extension(
    ...
    def test_predispatch_export_with_autograd_op(self):
        ...
        buffer = io.BytesIO()
        smith.export.save(ep, buffer)
        buffer.seek(0)
        loaded_ep = smith.export.load(buffer)
    ...
    def test_export_example_inputs_preserved(self):
        ...
        model = MyModule().eval()
        random_inputs = (smith.rand([2, 3]), smith.rand([2, 3]))
    ...
    def test_metadata_run_decomp_serder(self):
        ...
        exp_program = export(M(), (smith.randn(4, 4),), strict=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, x):
    sin = smith.ops.aten.sin.default(x);  x = None
    return (sin,)""",
        )
    ...
    def test_nested_layer_split(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.register_buffer("c:[22]", smith.randn(1))
    ...
    def test_serialize_param_mutation(self):
        ...
        foo = Foo()
        ep = smith.export.export(foo, (smith.rand(4, 4),)).run_decompositions()
        buffer = io.BytesIO()
        save(ep, buffer)
        loaded_ep = load(buffer)
    ...
    def test_serialize_multiple_returns_from_node(self) -> None:
        ...
        exported_module = export(
            MyModule(),
            (
                smith.ones([512, 512], requires_grad=True),
                smith.ones([512]),
                smith.ones([512]),
            ),
            strict=True,
        ...
        self.assertEqual(node.target, "smith.ops.aten.native_layer_norm.default")
    ...
    def test_serialize_sym_int(self) -> None:
        ...
        dim0_ac = smith.export.Dim("dim0_ac")
        dim1_bc = smith.export.Dim("dim1_b")
        dynamic_shapes = {
            "a": {0: dim0_ac},
            "b": {1: dim1_bc},
    ...
    def test_serialize_sym_float(self) -> None:
        ...
        class DynamicFloatSimpleModel(smith.nn.Module):
            def __init__(self, multiplier: smith.SymFloat):
                super().__init__()
            ...
            def forward(self, a, b, c) -> smith.Tensor:
                ...
                e_s3 = e_s0 * e_s1
                f = e.view(e_s3)
                return smith.cat([f, f])

        multiplier_sym = smith.SymFloat("multiplier_sym")
    ...
    def test_symint_list(self):
        ...
        shape_env = smith.fx.experimental.symbolic_shapes.ShapeEnv()
        symint = shape_env.create_unbacked_symint()
        serializer = GraphModuleSerializer(None, None)  # type: ignore[arg-type]
        res = serializer.serialize_inputs(
            smith.ops.aten.ones.default, ([1, symint, 3],), {}
        )
    ...
    def test_serialize_list_returns(self) -> None:
        class MyModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.split(x, 2)

        input = smith.arange(10.0).reshape(5, 2)
        ...
        self.assertEqual(node.target, "smith.ops.aten.split_with_sizes.default")
    ...
    def test_nonfinite_inputs(self) -> None:
        class Module(smith.nn.Module):
            def forward(self, x):
                x = smith.ops.aten.add.Scalar(x, math.inf)
                x = smith.ops.aten.add.Scalar(x, -math.inf)
                return smith.ops.aten.add.Scalar(x, math.nan)
    ...
    def test_multi_return_some_unused(self) -> None:
        ...
        class MyModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.ops.aten.var_mean.correction(x, [1])[0]

        exported_module = export(
            MyModule(), (smith.ones([512, 512], requires_grad=True),), strict=True
        ).run_decompositions()

        serialized = ExportedProgramSerializer().serialize(exported_module)
        node = serialized.exported_program.graph_module.graph.nodes[-1]
        self.assertEqual(node.target, "smith.ops.aten.var_mean.correction")
    ...
    def test_rational_ranges(self) -> None:
        ...
        ep = export(
            M(), (smith.randn(4),), dynamic_shapes=({0: Dim("temp")},), strict=True
        )
    ...
    @unittest.skipIf(
        not smith.cuda.is_available() or not has_triton(), "requires cuda and triton"
    )
    def test_triton_hop(self) -> None:
        ...
        def custom_add_autotune(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
            output = smith.empty_like(x)
            n_elements = output.numel()
        ...
        class MyModelAutotune(smith.nn.Module):
            def forward(self, x, y):
        ...
        for m in [MyModel().to(device), MyModelAutotune().to(device)]:
            args = (smith.randn(3, device=device), smith.randn(3, device=device))
            ep = smith.export.export(m, args=args)
            ep = ep.run_decompositions(decompose_custom_triton_ops=False)
            assert smith.allclose(m(*args), ep.module()(*args))
    ...
    def test_triton_constexpr_matching(self) -> None:
        ...
        def custom_op(x: smith.Tensor) -> smith.Tensor:
            output = smith.empty_like(x)
            n_elements = output.numel()
        ...
        device = "cuda"
        m = Model().to(device)
        args = (smith.randn(1024, device=device),)
        ...
        ep = smith.export.export(m, args=args)
        ep = ep.run_decompositions(decompose_custom_triton_ops=False)
        assert smith.allclose(eager_result, ep.module()(*args))
        ...
        triton_node = None
        for node in serialized.exported_program.graph_module.graph.nodes:
            if node.target == "smith.ops.higher_order.triton_kernel_wrapper_functional":
                triton_node = node
                break
        self.assertIsNotNone(triton_node)
    ...
    def test_kwargs_default(self) -> None:
        ...
        class Foo(smith.nn.Module):
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                values = smith.randn(3, 2)
                return smith.searchsorted(x, values, side="right", right=True)

        f = Foo()

        x, _ = smith.sort(smith.randn(3, 4))
        exported_module = export(f, (x,), strict=True).run_decompositions()
        serialized = ExportedProgramSerializer().serialize(exported_module)

        node = serialized.exported_program.graph_module.graph.nodes[-1]
        self.assertEqual(node.target, "smith.ops.aten.searchsorted.Tensor")
    ...
    def test_canonicalize(self) -> None:
        ...
        ep = export(Module(), (smith.randn(3, 2), smith.randn(3, 2)), strict=True)
    ...
    def test_int_list(self) -> None:
        class M(smith.nn.Module):
            def forward(self, x):
                return smith.ops.aten.sum.dim_IntList(x, [])

        ep = smith.export.export(M(), (smith.randn(3, 2),), strict=True)
        serialized = ExportedProgramSerializer().serialize(ep)
        for node in serialized.exported_program.graph_module.graph.nodes:
    ...
    def test_empty_constant(self) -> None:
        ...
        m = M()
        sample_inputs = (smith.randn(1, 4),)
        eager_out = m(*sample_inputs)
        ep = smith.export.export(m, sample_inputs)
        buffer = io.BytesIO()
        smith.export.save(ep, buffer)
        buffer.seek(0)
        loaded_ep = smith.export.load(buffer)
    ...
    def test_empty_state_dict(self) -> None:
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.const = smith.randn(4, 4)

            def forward(self, x):
                return x + self.const

        m = M()
        sample_inputs = (smith.randn(4, 4),)
        eager_out = m(*sample_inputs)
        ep = smith.export.export(m, sample_inputs)
        buffer = io.BytesIO()
        smith.export.save(ep, buffer)
    ...
    def test_preserve_aliasing(self) -> None:
        class M(smith.nn.Module):
            def __init__(self):
                ...
                self.register_buffer("buffer1", smith.randn(8, 8))
                self.register_buffer("buffer2", smith.randn(8, 8), persistent=False)
                self.const1 = smith.ones(8, 8)
    ...
    def test_1D_tensor_slicing(self) -> None:
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.const = smith.arange(8)[::2]

            def forward(self, x):
                return x + self.const

        m = M()
        sample_inputs = (smith.randn(4),)
        ep = smith.export.export(m, sample_inputs)
        buffer = io.BytesIO()
        save(ep, buffer)
        buffer.seek(0)
    ...
    def test_2D_tensor_slicing(self) -> None:
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.const = smith.randn(4, 4)[:2, :2]

            def forward(self, x):
                return x + self.const

        m = M()
        sample_inputs = (smith.randn(2, 2),)
        ep = smith.export.export(m, sample_inputs)
        buffer = io.BytesIO()
        save(ep, buffer)
        buffer.seek(0)
        ...
        class M(smith.nn.Module):
            def __init__(self):
                ...
                self.p = smith.nn.Parameter(
                    smith.ones(2, 2, dtype=smith.int8), requires_grad=False
                )
    ...
    def test_weight_sharing_gpu(self) -> None:
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.c2 = smith.ones(2, 4, device=GPU_TYPE)
                self.c1 = self.c2[0, :]
                self.linear = smith.nn.Linear(4, 4)
        ...
        m = M().to(GPU_TYPE)
        sample_inputs = (smith.randn(2, 4, device=GPU_TYPE),)
        ep = smith.export.export(m, sample_inputs)
...
class TestDeserialize(TestCase):
    ...
    def _check_graph_nodes(self, gm1, gm2, _check_meta=True):
        ...
        # roundtripping the source_fn value on smith.ops.map nodes
        # original source_fn: <funcsmith.experimental._map.MapWrapper object at 0x7f80a0549930>
        # deserialized source_fn: 'funcsmith.experimental._map.map'
        ...
        for node1, node2 in zip(gm1.graph.nodes, gm2.graph.nodes):
            ...
            if node1.op == "call_function":
                ...
                if node1.target == smith.ops.higher_order.cond:
                ...
                elif node1.target == smith.ops.higher_order.map_impl:
    ...
    def check_graph(
        ...
        def _deepcopy_inputs(inputs):
            ...
            for idx, i in enumerate(inputs):
                if isinstance(i, smith.Tensor) and hasattr(inputs[0], "__dict__"):
        ...
        def _check_graph(pre_dispatch):
            ...
            for orig, loaded in zip(flat_orig_outputs, flat_loaded_outputs):
                ...
                # smith.allclose doesn't work for float8
                if isinstance(orig, smith.Tensor) and orig.dtype not in [
                    smith.float8_e4m3fn,
                    smith.float8_e5m2,
                ]:
                    if orig.is_meta:
                        ...
                        self.assertTrue(smith.allclose(orig, loaded))
                else:
                    self.assertEqual(orig, loaded)
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            class M(smith.nn.Module):
                def forward(self, a, b, c):
                    return smith.ops.mylib.foo(a, b, c)

            self.check_graph(M(), (smith.randn(3), smith.randn(3), smith.randn(3)))
    ...
    def test_unbacked_bindings_serialize(self):
        ...
        for _strict in [True, False]:
            ep = smith.export.export(M(), inps, strict=_strict).run_decompositions()
    ...
    def test_sym_bool_dynamic_shapes(self) -> None:
        ...
        inputs = (smith.ones(4, 5, 10), smith.ones(3))
    ...
    def test_auto_functionalize(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            @smith.library.register_fake("mylib::foo2")
            ...
            @smith.library.register_fake("mylib::foo3")
            ...
            class M(smith.nn.Module):
                def forward(self, x, y, z, n):
                    n = smith.ops.mylib.foo1(x, y, z, 2, n)
                    smith.ops.mylib.foo3(x, y, z, 2, n)
                    return smith.ops.mylib.foo2(x, y, z, 2, n)
    ...
    def test_hoo_symint_input(self):
        ...
        inp = (smith.ones(3, 3), smith.ones(3, 3), smith.tensor(2))
    ...
    def test_none_input(self):
        ...
        ep = smith.export.export(M(), (smith.ones(3, 3), None, smith.ones(3, 3)))
        ...
        inp = (smith.rand(3, 3), None, smith.rand(3, 3))
    ...
    def test_basic(self) -> None:
        ...
        inputs = (smith.ones([512], requires_grad=True),)
        self.check_graph(MyModule(), inputs)
    ...
    def test_sym_bool(self):
        ...
        f = Module()
        self.check_graph(f, (smith.ones(1), smith.ones(3)))

    def test_sym_bool_smith_check_equal(self):
        class Module(smith.nn.Module):
            def forward(self, x):
                y = x.nonzero()
                z = y.size(0)
        ...
        self.check_graph(Module(), (smith.Tensor([1, 0, 1, 0]),))

    def test_sym_int_smith_check_equal(self):
        class Module(smith.nn.Module):
            def forward(self, x):
                y = x.nonzero()
                z = y.size(0)
        ...
        self.check_graph(Module(), (smith.Tensor([1, 0, 1, 0, 1, 0]),))
    ...
    def test_module_meta(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.p = smith.nn.Parameter(smith.ones(3, 3))
        ...
        inputs = (smith.randn(3, 3, device="meta"),)
    ...
    def test_pytree_namedtuple(self):
        ...
        inp = (N1(smith.randn(3), smith.randn(3)), N1(smith.randn(3), smith.randn(3)))
    ...
    def test_cond(self):
        ...
        inputs = smith.ones(4, 3), smith.zeros(4, 3)
    ...
    def test_sym_float(self):
        ...
        self.check_graph(M(), (smith.tensor(1.0),))

    def test_arg_from(self):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.register_buffer("compress_weight", smith.ones((10, 10)))
                self.register_buffer("compress_bias", smith.ones(10))

            def forward(self) -> None:
                if self.compress_weight is None or self.compress_bias is None:
                    return
                smith.nn.init.kaiming_uniform_(self.compress_weight, a=math.sqrt(5))
                fan_in, _ = smith.nn.init._calculate_fan_in_and_fan_out(
                    self.compress_weight
                )
                bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
                smith.nn.init.uniform_(self.compress_bias, -bound, bound)
    ...
    def test_map(self):
        ...
        g = Module()
        inputs = (smith.ones(3, 2, 2), smith.ones(2))
    ...
    def test_positional_argument_with_default_value(self):
        class MyLinear(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.randn(10, 10)
                self.bias = smith.randn(10)

            def forward(self, x):
                # bias has an default value here but it should be preserved
                # as a positional argument.
                return smith.ops.aten.linear.default(x, self.weight, self.bias)

        self.check_graph(MyLinear(), (smith.randn(10, 10),))

    def test_tensor_tensor_list(self):
        with smith.library._scoped_library("_export", "FRAGMENT") as lib:
            lib.define(
            ...
            class M(smith.nn.Module):
                def forward(self, x, y):
                    a, b = smith.ops._export._test_tensor_tensor_list_output.default(
                        x, y
                    )
                    return a + b[0]

            self.check_graph(M(), (smith.rand(3, 2), smith.rand(3, 2)))
    ...
    def test_list_of_optional_tensors(self) -> None:
        class MyModule(smith.nn.Module):
            ...
            def forward(self, x, y, z):
                indices = [None, None, smith.tensor([1, 3, 5, 7])]
                indexed = smith.ops.aten.index.Tensor(x + y, indices)
                return indexed + z

        inputs = (smith.rand(8, 8, 8), smith.rand(8, 8, 8), smith.rand(8, 8, 4))
        self.check_graph(MyModule(), inputs)
    ...
    def test_sym_ite(self):
        ...
        self.check_graph(Foo(), (smith.ones(4, 5),), dynamic_shapes=dynamic_shapes)
    ...
    def test_multiple_getitem(self):
        ...
        for node in ep.graph.nodes:
            if node.op == "call_function" and node.target == smith.ops.aten.mul.Tensor:
                ...
                with ep.graph.inserting_before(getitem_0):
                    ...
                    mul_node = ep.graph.call_function(
                        smith.ops.aten.mul.Tensor, (getitem_copy, 2)
                    )
        ...
        self.assertTrue(smith.allclose(orig_res[0], res[0]))
        self.assertTrue(smith.allclose(orig_res[1], res[1]))
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, x):
    topk = smith.ops.aten.topk.default(x, 2);  x = None
    getitem = topk[0]
    getitem_1 = topk[1];  topk = None
    mul_tensor = smith.ops.aten.mul.Tensor(getitem, 2)
    mul = smith.ops.aten.mul.Tensor(getitem, mul_tensor);  getitem = mul_tensor = None
    ...
    def test_constraints(self):
        ...
        f = Module()
        self.check_graph(f, (smith.tensor(3), smith.randn(4, 5)))
    ...
    def test_get_attr(self) -> None:
        ...
        f = Module()
        self.check_graph(f, (smith.tensor(3),))

    def test_get_attr_list(self) -> None:
        class Module(smith.nn.Module):
            def forward(self, x):
                return smith.cat([x, smith.tensor([1, 1])])

        f = Module()
        self.check_graph(f, (smith.tensor([1, 1]),))
    ...
    def test_device(self) -> None:
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(3, 16, 3, stride=1, bias=True)
                self.relu = smith.nn.ReLU()
        ...
        inp = smith.randn((1, 3, 224, 224), dtype=smith.float).to("cuda")
    ...
    def test_export_no_inputs(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.p = smith.ones(3, 3)

            def forward(self):
                return self.p * self.p

        ep = smith.export.export(M(), (), strict=True)
        ep._example_inputs = None
        roundtrip_ep = deserialize(serialize(ep))
        self.assertTrue(smith.allclose(ep.module()(), roundtrip_ep.module()()))

    def test_serialize_float8(self):
        for dtype in [smith.float8_e5m2, smith.float8_e4m3fn]:
...
class TestSchemaVersioning(TestCase):
    def test_error(self):
        ...
        f = Module()
        ep = export(f, (smith.randn(1, 3),), strict=True)
...
class TestSaveLoad(TestCase):
    def test_save_buffer(self):
        ...
        self.assertTrue(smith.allclose(ep.module()(*inp), loaded_ep.module()(*inp)))

    def test_deserialize_smith_artifact_dict(self):
        data = {"key": smith.tensor([1, 2, 3])}
        buf = io.BytesIO()
        smith.save(data, buf)
        serialized = buf.getvalue()
        result = deserialize_smith_artifact(serialized)

        self.assertIsInstance(result, dict)
        self.assertTrue(smith.equal(result["key"], smith.tensor([1, 2, 3])))
    ...
    def test_save_path(self):
        ...
        inp = (smith.tensor([6]), smith.tensor([7]))
        ep = export(f, inp, strict=True)
    ...
    def test_save_constants(self):
        ...
        ep = export(Foo(), (smith.tensor(1),), strict=True)
        buffer = io.BytesIO()
        save(ep, buffer)
        buffer.seek(0)
        loaded_ep = load(buffer)
    ...
    def test_save_load_with_multiple_empty_tensors(self) -> None:
        ...
        class M(smith.nn.Module):
            def __init__(self):
                ...
                self.register_buffer(
                    "int_buffer",
                    smith.zeros([0], dtype=smith.uint8),
                )
                self.register_buffer(
                    "int_buffer2",
                    smith.zeros([0], dtype=smith.uint8),
                ...
                self.register_buffer(
                    "float_buffer",
                    smith.zeros([0], dtype=smith.float32),
                )

            def forward(self, t: smith.Tensor) -> smith.Tensor:
        ...
        buffer.seek(0)
        dep = smith.export.load(buffer)
        unf = smith.export.unflatten(dep)
        self.assertEqual(unf.int_buffer.dtype, smith.uint8)
        self.assertEqual(unf.int_buffer2.dtype, smith.uint8)
        self.assertEqual(unf.float_buffer.dtype, smith.float32)
    ...
    def test_from_node_metadata_serialization(self):
        ...
        class Model(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.conv_layer = smith.nn.Conv2d(
...
class TestSerializeCustomClass(TestCase):
    ...
    def test_custom_class(self):
        ...
        for node in deserialized_ep.graph.nodes:
            if (
                ...
                arg = node.args[0]
                self.assertTrue(isinstance(arg, smith._C.ScriptObject))
    ...
    def test_custom_class_containing_fake_tensor(self):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.custom_obj = smith.classes._SmithScriptTesting._ContainsTensor(
                    smith.rand(2, 3)
                )
...
def forward(self, obj_attr, x):
    takes_foo = smith.ops._SmithScriptTesting.takes_foo.default(obj_attr, x);  obj_attr = None
    add = smith.ops.aten.add.Tensor(x, takes_foo);  x = takes_foo = None
    return (add,)""",
        )
        self.assertTrue(isinstance(ep.constants["attr"], smith.ScriptObject))
        gm = ep.module()
        self.assertExpectedInline(
            str(gm.code).strip(),
            """\
def forward(self, x):
    ...
    takes_foo = smith.ops._SmithScriptTesting.takes_foo.default(attr, x);  attr = None
    add = smith.ops.aten.add.Tensor(x, takes_foo);  x = takes_foo = None
    return pytree.tree_unflatten((add,), self._out_spec)""",
        )
        self.assertTrue(isinstance(gm.attr, smith.ScriptObject))
    ...
    def test_unbacked_range_serdes(self):
        class Foo(smith.nn.Module):
            def forward(self, x, y):
                n = x.item()
                smith._check(n >= 0)
                smith._check(n < y.size(0))
                return smith.empty(n), y[n]

        ep = smith.export.export(
            Foo(),
            (smith.tensor([5]), smith.randn(10)),
            dynamic_shapes={
                "x": None,
                "y": (Dim.DYNAMIC,),
            },
        ...
        # pre-serialize ep
        pre_shape_env = smith._guards.detect_fake_mode(
            [node.meta.get("val") for node in ep.graph.nodes]
        ).shape_env
        post_shape_env = smith._guards.detect_fake_mode(
    ...
    def test_backed_size_oblivious_serdes(self):
        ...
        with smith.fx.experimental._config.patch(backed_size_oblivious=True):
            ep = smith.export.export(
                Foo(),
                (smith.randn(1), smith.randn(1), smith.tensor([5])),
                dynamic_shapes={
                    "x": (Dim.DYNAMIC,),
                    "y": (Dim.DYNAMIC,),
                    "z": None,
        ...
        buffer.seek(0)
        loaded_ep = load(buffer)
        shape_env = smith._guards.detect_fake_mode(
            [node.meta.get("val") for node in loaded_ep.graph.nodes]
        ).shape_env
