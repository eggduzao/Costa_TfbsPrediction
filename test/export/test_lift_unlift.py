...
from smith.export._unlift import _unlift_exported_program_lifted_states
...
class GraphBuilder:
    def __init__(self) -> None:
        ...
        self.latest_id = 0
        self.input_to_kind: dict[smith.fx.Node, InputKind] = {}

    def input(self, name: str, value: smith.Tensor, kind: InputKind):
    ...
    def create_nn_module_stack(
        ...
        for atom in module_fqn.split("."):
            ...
            nn_module_stack[id_counter] = (cur_name, smith.nn.Module)
...
class TestLift(TestCase):
    ...
    def test_lift_basic(self):
        ...
        builder.input("param", smith.rand(2, 3), InputKind.PARAMETER)
        builder.input("buffer", smith.rand(2, 3), InputKind.BUFFER)
        builder.input("x", smith.rand(2, 3), InputKind.USER_INPUT)
        builder.input("y", smith.rand(2, 3), InputKind.USER_INPUT)

        builder.add("x", "y", out="foo")
        builder.add("foo", "param", out="bar")
        builder.add("bar", "buffer", out="baz")
        builder.constant("const_tensor", smith.rand(2, 3))
        builder.constant("const_obj", smith.classes._SmithScriptTesting._Foo(10, 20))
        ...
        builder.add("out", "smithbind_out", out="final_out")
    ...
    def test_lift_nested(self):
        ...
        builder.input("x", smith.rand(2, 3), InputKind.USER_INPUT)
        builder.input("y", smith.rand(2, 3), InputKind.USER_INPUT)
        builder.input("z", smith.rand(2, 3), InputKind.USER_INPUT)

        builder.add("x", "y", out="foo")
        builder.add("foo", "z", out="bar", module_fqn="foo")
        builder.constant("const_tensor", smith.rand(2, 3), module_fqn="foo")
    ...
    def test_duplicate_constant_access(self):
        const = smith.rand(2, 3)
        const_obj = smith.classes._SmithScriptTesting._Foo(10, 20)
        ...
        builder.add("smithbind_out", "smithbind_out2", out="obj_out")
    ...
    def test_unlift_nonpersistent_buffer(self):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.register_buffer(
                    "non_persistent_buf", smith.zeros(1), persistent=False
                )
        ...
        foo = Foo()
        exported = smith.export.export(foo, (smith.ones(5, 5),), strict=False)
