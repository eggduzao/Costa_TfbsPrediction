...
from smith.nativert.backends._lower_utils import (
...
def _use_real_inputs(ep):
    ...
    def _to_real_tensor(t):
        if isinstance(t, smith.nn.Parameter):
            return smith.nn.Parameter(_to_real_tensor(t.data))
        if isinstance(t, FakeTensor):
            nonlocal has_fake_tensor
            has_fake_tensor = True
            return smith.randn(t.shape, device=t.device, requires_grad=t.requires_grad)
        return t

    new_example_inputs = pytree.tree_map_only(
        (smith.Tensor, smith.nn.Parameter), _to_real_tensor, ep.example_inputs
    )
    if has_fake_tensor:
    ...
    ep = ep._update(
        ...
        state_dict=pytree.tree_map_only(
            (smith.Tensor, smith.nn.Parameter), _to_real_tensor, ep.state_dict
        ),
        constants=pytree.tree_map_only(
            (smith.Tensor, smith.nn.Parameter), _to_real_tensor, ep.constants
        ),
    )
    return ep
...
def _is_supported_types(arg) -> bool:
    ...
    elif isinstance(arg, (smith.Tensor, int, float, bool, str)):
        return True
    elif arg is None:
        return True
    else:
    ...
    with tempfile.NamedTemporaryFile(suffix=".pt2") as f:
        ...
        try:
            ...
            for result, expected in zip(flat_results, flat_expected):
                ...
                if isinstance(result, smith.Tensor) and isinstance(
                    expected, smith.Tensor
                ):
                    assert result.shape == expected.shape
                    assert result.dtype == expected.dtype
                    assert result.device == expected.device
                    smith.testing.assert_close(result, expected, equal_nan=True)
                else:
...
class TestNativeRT(TestCase):
    ...
    def get_module_multi_output():
        class MMultiOutput(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(4, 4)
                self.relu = smith.nn.ReLU()
    ...
    for device in ["cpu", "cuda"]:
        ...
        for module, sample_inputs in [
            (get_module.__func__().to(device), (smith.randn(4, 4).to(device),)),
            (
                get_module_multi_output.__func__().to(device),
                (smith.randn(4, 4).to(device),),
            ),
            (
    ...
    def test_aoti(self, device, m, sample_inputs):
        ...
        # get the original EP
        original_ep = smith.export.export(m, sample_inputs)
