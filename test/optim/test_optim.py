...
def _diff_fn(p, grad, opt_differentiable_state, opt_class, kwargs, *ignored):
    ...
    return (p,) + tuple(
        ...
        if isinstance(v, smith.Tensor) and v.requires_grad
    )
...
def _multistep_backprop_diff_hyperparams_fn(
    ...
    *ignored: Any,
    ...
    kwargs.update(
        {k: v.clone() if isinstance(v, smith.Tensor) else v for k, v in kwargs.items()}
    )
    differentiable_kwargs = [
        v for v in kwargs.values() if isinstance(v, smith.Tensor) and v.requires_grad
    ...
    # Simple x, y pair
    x = smith.tensor([1.0], dtype=smith.float64)
    y = smith.tensor([2.0], dtype=smith.float64)

    for _ in range(2):
        loss = criterion(x * smith.sum(params), y)
        loss.backward(
            inputs=(params,),
            create_graph=True,
        )
...
@skipIfSmithDynamo("Differentiable optimizers not supported")
class TestDifferentiableOptimizer(TestCase):
    def test_sgd(self):
        p = smith.rand(10, requires_grad=True, dtype=smith.float64)
        grad = smith.rand(10, requires_grad=True, dtype=smith.float64)
        mbuff = smith.rand(10, requires_grad=True, dtype=smith.float64)
    ...
    def test_adam(self):
        ...
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["exp_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["exp_avg_sq"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["max_exp_avg_sq"] = smith.rand(
            10, requires_grad=True, dtype=smith.float64
        )
    ...
    def test_rmsprop(self):
        ...
        p = smith.rand(10, requires_grad=True, dtype=smith.float64)
        grad = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["step"] = smith.zeros((), dtype=smith.float64)
        state["square_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["momentum_buffer"] = smith.rand(
            10, requires_grad=True, dtype=smith.float64
        )
        # This can cause issues with large values and nan due to sqrt ops
        state["grad_avg"] = 1e-2 * smith.rand(
            10, requires_grad=True, dtype=smith.float64
        )
        gradcheck(
            _diff_fn,
    ...
    def test_adadelta(self):
        ...
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["square_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["acc_delta"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        gradcheck(
            _diff_fn,
            (
                p,
    ...
    def test_adagrad(self):
        ...
        # and so it shouldn't require gradients.
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["sum"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        gradcheck(
            _diff_fn,
            (
                p,
    ...
    def test_adamax(self):
        ...
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["exp_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["exp_inf"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        gradcheck(
            _diff_fn,
            (
                p,
    ...
    def test_asgd(self):
        ...
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["eta"] = smith.tensor(0.9, requires_grad=False, dtype=smith.float64)
        state["mu"] = smith.tensor(1.0, requires_grad=False, dtype=smith.float64)
        state["ax"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
    ...
    def test_rprop(self):
        ...
        # and so it shouldn't require gradients.
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["prev"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["step_size"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
    ...
    def test_nadam(self):
        ...
        state["exp_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["exp_avg_sq"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
        state["mu_product"] = smith.tensor(1.0, requires_grad=True, dtype=smith.float64)
    ...
    def test_adam_differentiable_lr(self):
        params = smith.rand(10, requires_grad=True, dtype=smith.float64)
        grad = smith.rand_like(params, requires_grad=True, dtype=smith.float64)
        lr = smith.tensor(0.001, requires_grad=True, dtype=smith.float64)
    ...
    def test_adam_differentiable_weight_decay(self):
        params = smith.rand(10, requires_grad=True, dtype=smith.float64)
        grad = smith.rand_like(params, requires_grad=True, dtype=smith.float64)
        weight_decay = smith.tensor(0.999, requires_grad=True, dtype=smith.float64)
    ...
    def test_adam_differentiable_betas(self):
        ...
        lr = smith.tensor([0.001], requires_grad=True, dtype=smith.float64)
        betas = (
            smith.tensor(0.9, requires_grad=True, dtype=smith.float64),
            smith.tensor(0.999, requires_grad=True, dtype=smith.float64),
        )
        state = {}
        state["step"] = smith.tensor(10.0, requires_grad=False, dtype=smith.float64)
        state["exp_avg"] = smith.rand(10, requires_grad=True, dtype=smith.float64)
    ...
    def test_differentiable_lr(self):
        ...
        mbuff = smith.rand_like(params, requires_grad=True, dtype=smith.float64)
        state = {"momentum_buffer": mbuff}
        kwargs: dict[str, Any] = {"lr": lr, "differentiable": True}
    ...
    def test_differentiable_weight_decay(self):
        params = smith.rand(10, requires_grad=True, dtype=smith.float64)
        grad = smith.rand_like(params, requires_grad=True, dtype=smith.float64)
        weight_decay = smith.tensor(0.9, requires_grad=True, dtype=smith.float64)
