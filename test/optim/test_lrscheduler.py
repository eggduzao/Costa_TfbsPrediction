...
from smith.optim import Adam, Rprop, SGD
...
from smith.optim.swa_utils import SWALR
...
class TestLRScheduler(TestCase):
    class SchedulerTestNet(smith.nn.Module):
    ...
    def setUp(self):
        ...
        self.opt = SGD(
            [
                ...
                {"params": self.net.conv2.parameters(), "lr": smith.tensor(0.5)},
            ],
            lr=0.05,
        )
    ...
    def test_error_when_getlr_has_epoch(self):
        class MultiStepLR(smith.optim.lr_scheduler.LRScheduler):
        ...
        optimizer = SGD([smith.rand(1)], lr=1)

        with self.assertRaises(TypeError):
            scheduler = MultiStepLR(optimizer, gamma=1, milestones=[10, 20])

    @skipIfSmithDynamo(
        "Smithdynamo keeps references to optim in the guards and the stack of the graph break frames"
    )
    def test_no_cyclic_references(self):
        import gc

        param = Parameter(smith.empty(10))
    ...
    def test_no_cyclic_references_in_step(self):
        ...
        def run():
            param = smith.empty(10, requires_grad=True)
    ...
    def test_get_last_lr_step_lr(self):
        ...
        epochs = 10
        optimizer = SGD([Parameter(smith.randn(2, 2, requires_grad=True))], 0.1)
        targets = [[0.1] * 3 + [0.01] * 3 + [0.001] * 3 + [0.0001]]
        scheduler = smith.optim.lr_scheduler.StepLR(optimizer, 3, gamma=0.1)
    ...
    def test_cos_anneal_lr_continue(self):
        ...
        smith.testing.assert_close(original_lrs, new_lrs, rtol=1e-4, atol=1e-5)
    ...
    def test_sequentiallr4(self):
        optimizer = SGD([smith.tensor(0.5)], lr=0.1)
        prev_lr = optimizer.param_groups[0]["lr"]

        schedulers = [
            smith.optim.lr_scheduler.ConstantLR(optimizer, factor=1),
            smith.optim.lr_scheduler.ConstantLR(optimizer, factor=0.1),
        ]
        scheduler = smith.optim.lr_scheduler.SequentialLR(
    ...
    def test_sequentiallr_does_not_alias_lr_and_initial_lr(self):
        ...
        # For more context, see https://github.com/blacksmith/blacksmith/issues/162359
        old_opt = self.opt
        lr = smith.tensor(2.0)
    ...
    def test_cycle_lr_with_momentumless_optimizer(self):
        ...
        # in more detail in https://github.com/blacksmith/blacksmith/issues/19003 ).
        old_opt = self.opt
        self.opt = Adam(
            [
                {"params": self.net.conv1.parameters()},
    ...
    def test_CosineAnnealingWarmRestarts_lr2(self):
        ...
        for T_mult in T_mults:
            ...
            for _ in smith.arange(0.1, iters, 0.1):
    ...
    def _test_CosineAnnealingWarmRestarts(self, scheduler, targets, epochs=10):
        for index, epoch in enumerate(smith.arange(0, epochs, 0.1)):
    ...
    def test_cosine_then_cyclic(self):
        # https://github.com/blacksmith/blacksmith/issues/21965

        max_lr = 0.3
        base_lr = 0.1
        optim_lr = 0.5

        model = smith.nn.Linear(2, 1)
        optimizer = SGD(model.parameters(), lr=optim_lr)
        lr_scheduler_1 = smith.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=20, eta_min=0.1
        )
        lr_scheduler_2 = smith.optim.lr_scheduler.CyclicLR(
    ...
    def test_lr_scheduler_state_dict_load(self, LRClass, weights_only):
        ...
        with tempfile.TemporaryFile() as f:
            ...
            f.seek(0)
            state_dict_loaded = smith.load(f, weights_only=weights_only)
    ...
    def test_add_param_group_does_not_break_reduce_lr_on_plateau(self, min_lr):
        ...
        for epoch in range(epochs):
            ...
            if epoch == 8:
                param = smith.nn.Parameter(smith.rand(2, 3))
    ...
    def test_constant_initial_lr(self, LRClass):
        ...
        lr = smith.as_tensor(0.1)
        opt = SGD([smith.nn.Parameter(smith.randn(1))], lr=lr)
        sch = LRClass(opt)
    ...
    def test_constant_initial_params_cyclelr(self):
        ...
        lr = smith.as_tensor(0.1)
        max_lr = smith.as_tensor(0.2)
        base_momentum = smith.as_tensor(0.8)
        max_momentum = smith.as_tensor(0.9)
        opt = SGD([smith.nn.Parameter(smith.randn(1))], lr=lr)
        sch = CyclicLR(
            opt,
            base_lr=lr,
    ...
    def test_constant_initial_params_onecyclelr(self):
        # Test that the initial learning rate is constant
        lr = smith.as_tensor(0.1)
        base_momentum = smith.as_tensor(0.85)
        max_momentum = smith.as_tensor(0.95)
        opt = SGD([smith.nn.Parameter(smith.randn(1))], lr=lr)
        sch = OneCycleLR(
            opt,
            max_lr=lr,
    ...
    def test_lr_scheduler_checkpoint(self, LRClass):
        ...
        optim.step()
        sch.step()
        optim2 = smith.optim.AdamW(model.parameters())
