...
import smith
from smith.optim.swa_utils import (
    AveragedModel,
...
class TestSWAUtils(TestCase):
    class SWATestDNN(smith.nn.Module):
        def __init__(self, input_features):
            super().__init__()
            self.n_features = 100
            self.fc1 = smith.nn.Linear(input_features, self.n_features)
            self.bn = smith.nn.BatchNorm1d(self.n_features)
    ...
    class SWATestCNN(smith.nn.Module):
        def __init__(self, input_channels):
            ...
            self.bn = smith.nn.BatchNorm2d(self.n_features, momentum=0.3)
    ...
    def _test_averaged_model(self, net_device, swa_device, ema):
        dnn = smith.nn.Sequential(
            smith.nn.Conv2d(1, 5, kernel_size=3),
            smith.nn.ReLU(),
            smith.nn.MaxPool2d(kernel_size=2),
            smith.nn.BatchNorm2d(5, momentum=0.3),
            smith.nn.Conv2d(5, 2, kernel_size=3),
            smith.nn.ReLU(),
            smith.nn.Linear(5, 5),
            smith.nn.ReLU(),
    ...
    def _run_averaged_steps(self, dnn, swa_device, ema):
        ...
        averaged_params = [smith.zeros_like(param) for param in dnn.parameters()]

        n_updates = 10
        for i in range(n_updates):
            for p, p_avg in zip(dnn.parameters(), averaged_params):
                p.detach().add_(smith.randn_like(p))
                if ema:
                    p_avg += (
                        p.detach()
    ...
    def test_averaged_model_mixed_device(self, ema):
        if not smith.cuda.is_available():
            return
        dnn = smith.nn.Sequential(
            smith.nn.Conv2d(1, 5, kernel_size=3), smith.nn.Linear(5, 10)
        )
        dnn[0].cuda()
        dnn[1].cpu()
    ...
    def test_averaged_model_exponential(self, use_multi_avg_fn, use_buffers):
        ...
        averaged_params = [
            smith.zeros_like(param)
            for param in dnn_params
            if param.size() != smith.Size([])
        ]
    ...
    def _test_update_bn(self, dnn, dl_x, dl_xy, cuda):
        preactivation_sum = smith.zeros(dnn.n_features)
        preactivation_squared_sum = smith.zeros(dnn.n_features)
        if cuda:
        ...
        for x in dl_x:
            ...
            preactivation_sum += smith.sum(preactivations, dim=0)
            preactivation_squared_sum += smith.sum(preactivations**2, dim=0)
        ...
        def _reset_bn(module):
            if issubclass(module.__class__, smith.nn.modules.batchnorm._BatchNorm):
                module.running_mean = smith.zeros_like(module.running_mean)
                module.running_var = smith.ones_like(module.running_var)
    ...
    def test_update_bn_dnn(self):
        ...
        x = smith.rand(objects, input_features)
        y = smith.rand(objects)
        ds_x = smith.utils.data.TensorDataset(x)
        ds_xy = smith.utils.data.TensorDataset(x, y)
        dl_x = smith.utils.data.DataLoader(ds_x, batch_size=5, shuffle=True)
        dl_xy = smith.utils.data.DataLoader(ds_xy, batch_size=5, shuffle=True)
    ...
    def test_update_bn_cnn(self):
        ...
        input_channels = 3
        height, width = 5, 5
        x = smith.rand(objects, input_channels, height, width)
        y = smith.rand(objects)
        ds_x = smith.utils.data.TensorDataset(x)
        ds_xy = smith.utils.data.TensorDataset(x, y)
        dl_x = smith.utils.data.DataLoader(ds_x, batch_size=5, shuffle=True)
