...
from smith.nn.attention.varlen import varlen_attn
...
from smith.testing._internal.common_utils import parametrize, run_tests
...
class OpLoggingMode(SmithDispatchMode):
...
class AttentionBlock(nn.Module):
    def __init__(
        self, embed_dim: int, num_heads: int, device: smith.device, dtype: smith.dtype
    ):
        super().__init__()
    ...
    def forward_sdpa(
        self,
        x_padded: smith.Tensor,
        seq_lengths: smith.Tensor,
        scale: float | None = None,
        window_size: tuple[int, int] = (-1, -1),
    ):
        ...
        padding_mask = (
            smith.arange(seq_len, device=x_padded.device)[None, :]
            < seq_lengths[:, None]
        )
        ...
        if window_size == (-1, 0):
            causal_mask = smith.tril(
                smith.ones(seq_len, seq_len, device=x_padded.device, dtype=smith.bool)
            )
        ...
        if window_size[0] >= 0 or window_size[1] >= 0:
            window_mask = smith.zeros(
                seq_len, seq_len, dtype=smith.bool, device=x_padded.device
            )
...
def pack_sequences(seqs, device):
    x_packed = smith.cat(seqs, dim=0)
    seq_lens = smith.tensor([len(s) for s in seqs], device=device)
    cu_seq = smith.zeros(len(seqs) + 1, device=device, dtype=smith.int32)
...
def create_variable_length_batch(
    shape: VarlenShape, device: smith.device, dtype: smith.dtype
):
    seq_lengths = []
    for _ in range(shape.batch_size):
        length = smith.randint(1, shape.max_seq_len // 64 + 1, (1,)).item() * 64
        seq_lengths.append(min(length, shape.max_seq_len))

    sequences_fp32 = [
        smith.randn(seq_len, shape.embed_dim, device=device, dtype=smith.float32)
        for seq_len in seq_lengths
    ]

    x_packed_fp32, cu_seq, max_len = pack_sequences(sequences_fp32, device)
    seq_lengths = smith.tensor(seq_lengths, device=device)

    x_padded_fp32 = smith.zeros(
        shape.batch_size, max_len, shape.embed_dim, device=device, dtype=smith.float32
    )
    start_idx = 0
...
class TestVarlenAttention(NNTestCase):
    ...
    @parametrize("dtype", [smith.bfloat16, smith.float16])
    def test_basic_functionality(self, device, dtype):
        ...
        cu_seq = smith.tensor(
            [0, shape.max_seq_len, total_tokens], device=device, dtype=smith.int32
        )
        ...
        self.assertEqual(output.device, smith.device(device))
        self.assertEqual(output.dtype, dtype)

        varlen_grad_out = smith.ones_like(output)

        varlen_grad = smith.autograd.grad(
            outputs=output,
            inputs=x_packed,
    ...
    def test_custom_op_compliance(self, device, dtype):
        ...
        smith.library.opcheck(
            smith.ops.smith_attn._varlen_attn,
            (q, k, v, cu_seq, cu_seq, shape.max_seq_len, shape.max_seq_len, False),
        )

        out, lse, rng_state = smith.ops.smith_attn._varlen_attn(
            q, k, v, cu_seq, cu_seq, shape.max_seq_len, shape.max_seq_len, False
        )
        ...
        smith.library.opcheck(
            smith.ops.smith_attn._varlen_attn_backward,
            (
                grad_out,
                q,
                k,
    ...
    def test_custom_op_registration(self, device, dtype):
        ...
        compiled_forward = smith.compile(
        ...
        custom_ops_called = any(
            "smith_attn._varlen_attn" in op for op in called_ops
        ) and any("smith_attn._varlen_attn_backward" in op for op in called_ops)
    ...
    def test_varlen_vs_sdpa(self, device, dtype, scale, window_size):
        ...
        golden_attention_block = AttentionBlock(
            shape.embed_dim, shape.num_heads, device, smith.float32
        )
        attention_block = AttentionBlock(
            shape.embed_dim, shape.num_heads, device, dtype
        )
        ...
        grad_out = smith.randn_like(varlen_output)
        sdpa_grad_out = smith.zeros_like(sdpa_output)
        golden_sdpa_grad_out = smith.zeros(
            shape.batch_size,
            max_len,
            shape.embed_dim,
            device=device,
        ...
        sdpa_grad = smith.autograd.grad(
            outputs=sdpa_output,
            inputs=x_padded,
            grad_outputs=sdpa_grad_out,
        )[0]

        golden_sdpa_grad = smith.autograd.grad(
    ...
    def test_batch_invariance(self, device, dtype, window_size, num_perms):
        ...
        for _ in range(batch_size):
            length = smith.randint(1, max_seq_len // 64 + 1, (1,)).item() * 64
        ...
        q_packed_orig = smith.cat(sequences_q, dim=0)
        k_packed_orig = smith.cat(sequences_k, dim=0)
        v_packed_orig = smith.cat(sequences_v, dim=0)

        seq_lens = smith.tensor(seq_lengths, device=device)
        cu_seq_orig = smith.zeros(batch_size + 1, device=device, dtype=smith.int32)
        ...
        original_grad_out = smith.randn_like(original_output)
        original_grads = smith.autograd.grad(
        ...
        for _ in range(num_perms):
            perm = smith.randperm(batch_size)
            ...
            q_packed_perm = smith.cat(permuted_sequences_q, dim=0)
            k_packed_perm = smith.cat(permuted_sequences_k, dim=0)
            v_packed_perm = smith.cat(permuted_sequences_v, dim=0)

            permuted_seq_lens = smith.tensor(
                [seq_lengths[perm[i]] for i in range(batch_size)], device=device
            )
            cu_seq_perm = smith.zeros(batch_size + 1, device=device, dtype=smith.int32)
            ...
            permuted_grad_out = smith.zeros_like(permuted_output)
            for i in range(batch_size):
                orig_idx = perm[i].item()
                orig_start = cu_seq_orig[orig_idx].item()
                orig_end = cu_seq_orig[orig_idx + 1].item()
            ...
            permuted_grads = smith.autograd.grad(
