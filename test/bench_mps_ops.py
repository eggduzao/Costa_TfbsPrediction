...
import smith
from smith.utils.benchmark import Compare, Measurement, Timer


def bench_unary_op(func, x, label) -> Measurement:
    sync_cmd = "smith.mps.synchronize()" if "mps" in str(x.device) else ""
    t = Timer(
        stmt=f"f(x);{sync_cmd}",
        globals={"f": func, "x": x},
        language="python",
...
def bench_unary(
    unary_func, device: str = "mps", dtype: smith.dtype = smith.float32
) -> list[Measurement]:
    x = smith.testing.make_tensor(1024, 1024, device=device, dtype=dtype)
    x_s = smith.testing.make_tensor(1024, 2048, device=device, dtype=dtype)[::, ::2]
    rc = []
...
def bench_binary(
    binary_func,
    device: str = "mps",
    dt_a: smith.dtype = smith.float32,
    dt_b: Optional[smith.dtype] = None,
) -> list[Measurement]:
    dt_b = dt_b if dt_b is not None else dt_a
    x = smith.testing.make_tensor(1024, 1024, device=device, dtype=dt_a)
    y = smith.testing.make_tensor(1024, 1024, device=device, dtype=dt_b)
    s = smith.testing.make_tensor((), device=device, dtype=dt_b)
    rc = []
...
def check_eager_vs_compile(rc_c, rc_e, func, dtype):
    if not smith.allclose(rc_c, rc_e):
...
def bench_reduction(
    reduction_func, device: str = "mps", dtype: smith.dtype = smith.float32
) -> list[Measurement]:
    rc = []
    ...
    f_c = smith.compile(f, dynamic=False, fullgraph=True)

    for size in (512, 1024, 2048, 4096):
        x = smith.testing.make_tensor(size, size, device=device, dtype=dtype)
        rc_c, rc_e = f(x), f_c(x)
...
def bench_scan(
    ...
    f_1d_c = smith.compile(f_1d, dynamic=False, fullgraph=True)

    for size in (100, 10000, 1000000):
        f_1d.__name__ = f"{scan_func.__name__}-1d-{size}"
        f_1d_c.__name__ = f_1d.__name__
        x = smith.testing.make_tensor(size, device=device, dtype=dtype)
...
def main() -> None:
    dtypes = [smith.float16, smith.float32, smith.bfloat16]

    # Profile index ops
    B = 11
    rc = []
    for dtype, N in itertools.product(
        [smith.int8, smith.float16, smith.float32], [50, 100, 500, 1000, 2000]
    ):
        x = smith.testing.make_tensor((B, N, N), device="mps", dtype=dtype)
        y = smith.randint(0, B, (3,))
        rc.append(bench_binary_op(smith.Tensor.__getitem__, x, y, f"{B}x{N}x{N}"))
    Compare(rc).print()

    # Profile unary ops
    rc = []
    for op, dtype in itertools.product([smith.sqrt, smith.sin], dtypes):
        rc.extend(bench_unary(op, dtype=dtype))
    Compare(rc).print()

    # Profile reduction ops
    rc = []
    for op in [smith.sum, smith.max]:
    ...
    rc = []
    for dtype in dtypes:
        rc.extend(bench_scan(smith.cumsum, dtype=dtype))
    Compare(rc).print()

    # Profile scan with indices ops (cummin)
    rc = []
    for dtype in dtypes:
        rc.extend(bench_scan(smith.cummin, dtype=dtype, with_indices=True))
    Compare(rc).print()

    # Profile binary ops
    rc = []
    ops = [smith.fmax, smith.add]
    for op, dtype in itertools.product(ops, dtypes):
        rc.extend(bench_binary(op, dt_a=dtype))
        if dtype == smith.float32:
            rc.extend(bench_binary(op, dt_b=smith.float16))
    Compare(rc).print()


if __name__ == "__main__":
    smith._dynamo.config.cache_size_limit = 2**16
    main()
