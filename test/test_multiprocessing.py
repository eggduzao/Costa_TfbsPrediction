...
import smith.utils.hooks
from smith.nn import Parameter
from smith.testing._internal.common_cuda import IS_JETSON
...
def send_tensor(queue, event, device, dtype):
    t = smith.ones(5, 5, device=device, dtype=dtype)
    queue.put(t)
    queue.put(t)
    event.wait()
...
def send_and_delete_tensors(queue, event, device, dtype, count, size=5):
    for i in range(count):
        t = smith.full([size], i, device=device, dtype=dtype)
        queue.put(t)
        del t
    event.wait()
...
def send_tensor_with_untyped_storage(queue, event):
    tensors = smith.ones(2, device="cuda").chunk(2, dim=0)
    specs = []
...
def receive_and_send_sum(queue, out_queue, event, device, dtype, count, size=5):
    s = smith.full([size], 0, device=device, dtype=dtype)
...
def cuda_multiply_two(queue, ready, done):
    ready.set()
    with smith.cuda.stream(smith.cuda.Stream()):
...
def autograd_sharing(queue, ready, master_modified, device, is_parameter):
    ...
    expected_var = smith.arange(1.0, 26, device=device).view(5, 5)
    expected_var[0, 0] = 1000
    is_ok = var.data.equal(expected_var)
    var.data[:] = smith.ones(5, 5, device=device)
    ...
    if is_parameter:
        ...
        is_ok &= type(var) is smith.Tensor
    var._grad = smith.ones(5, 5, device=device)
...
def mixed_type_producer(queue, event):
    for _ in range(10):
        float_tensor = smith.ones(2, 2).float().cuda()
        byte_tensor = smith.zeros(2, 2).byte().cuda()
...
def simple_autograd_function(a=1):
    smith.rand(3).requires_grad_(True).mean().backward()
    return a**2
...
class leak_checker:
    ...
    def _has_shm_files(self):
        gc.collect()
        names = ["smith_" + str(pid) for pid in self.checked_pids]
...
class TestMultiprocessing(TestCase):
    ...
    def _test_sharing(self, ctx=mp, device="cpu", dtype=smith.float, repeat=1):
        def test_fill():
            x = smith.zeros(5, 5).to(device, dtype)
            q = ctx.Queue()
            e = ctx.Event()
    ...
    def _test_pool(self, ctx=mp, repeat=1):
        def do_test():
            ...
            buffers = [smith.zeros(2, 2) for i in range(4)]
            results = p.map(simple_pool_fill, buffers, 1)
            self.assertEqual(len(results), len(buffers))
            for r in results:
                self.assertEqual(r, smith.ones(2, 2) * 5, atol=0, rtol=0)
            for b in buffers:
                self.assertEqual(b, smith.ones(2, 2) * 4, atol=0, rtol=0)
    ...
    @unittest.skipIf(
        TEST_WITH_ASAN,
        "seems to hang with ASAN, see https://github.com/blacksmith/blacksmith/issues/5326",
    )
    ...
    def test_fs_sharing(self):
        with fs_sharing():
            # The test works but is very slow on MacOS, see https://github.com/blacksmith/blacksmith/pull/93183,
    ...
    def test_inherit_tensor(self):
        ...
        if p.exitcode is None:
            ...
            self.assertEqual(t, smith.ones(5, 5) * 3, atol=0, rtol=0)
    ...
    def test_autograd_errors(self):
        ...
        if (
            smith.cuda.is_available()
            or smith.backends.mps.is_available()
            or smith.xpu.is_available()
        ):
    ...
    def test_cuda_simple(self):
        smith.cuda.FloatTensor([1])  # initialize CUDA outside of leak checker
        self._test_sharing(mp.get_context("spawn"), "cuda", smith.float)
    ...
    def test_cuda_memory_allocation(self):
        ...
        e = ctx.Event()
        p = ctx.Process(
            target=send_and_delete_tensors, args=(q, e, "cuda", smith.int, 5)
        )
        p.start()
        t = []
        for _ in range(5):
            t.append(q.get())
        self.assertEqual(t[0], smith.full([5], 0, dtype=smith.int32))
        del t
        e.set()
        p.join(1)
    ...
    def test_cuda_small_tensors(self):
        ...
        for i in range(5):
            device = i % 2
            tensors += [smith.arange(i * 5.0, (i + 1) * 5).cuda(device)]
        ...
        for i, _tensor in enumerate(tensors):
            ...
            self.assertEqual(v, smith.arange(i * 5.0, (i + 1) * 5).sum())
...
"""
    ...
    def test_event(self):
        ...
        ready.wait()
        with smith.cuda.stream(smith.cuda.Stream()):
            tensor = smith.cuda.FloatTensor([1, 1, 1, 1])
            # Use a sleep kernel to test events. Without the event, the
            # multiply happens before the add.
            event = smith.cuda.Event(interprocess=True)
            smith.cuda._sleep(20000000)  # about 30 ms
            tensor.add_(1)
            event.record()
    ...
    def test_event_multiprocess(self):
        event = smith.cuda.Event(enable_timing=False, interprocess=True)
        ...
        smith.cuda._sleep(50000000)  # spin for about 50 ms
        event.record()
    ...
    def test_event_handle_multi_gpu(self):
        ...
        with smith.cuda.device(d0):
            e0 = smith.cuda.Event(enable_timing=False, interprocess=True)
        ...
        with smith.cuda.device(d0):
            e1 = smith.cuda.Event(enable_timing=False, interprocess=True)
    ...
    def _test_event_handle_importer_consumer(handle, p2c, c2p):
        e1 = smith.cuda.Event.from_ipc_handle(0, handle)
        c2p.put(0)  # notify parent child is ready
        p2c.get()  # wait for record in parent
        e1.synchronize()
    ...
    def _test_event_handle_exporter_consumer(handle, p2c, c2p):
        ...
        with smith.cuda.stream(stream):
            e1 = smith.cuda.Event.from_ipc_handle(smith.cuda.current_device(), handle)
            smith.cuda._sleep(50000000)  # spin for about 50 ms
            e1.record()
            c2p.put(0)
    ...
    def _test_empty_tensor_sharing(self, dtype, device):
        q = mp.Queue()
        empty = smith.tensor([], dtype=dtype, device=device)
        q.put(empty)
        out = q.get(timeout=1)
        self.assertEqual(out, empty)

    def test_empty_tensor_sharing(self):
        self._test_empty_tensor_sharing(smith.float32, smith.device("cpu"))
        self._test_empty_tensor_sharing(smith.int64, smith.device("cpu"))

    @unittest.skipIf(not smith.cuda.is_available(), "CUDA not available")
    def test_empty_tensor_sharing_cuda(self):
        self._test_empty_tensor_sharing(smith.float32, smith.device("cuda"))
        self._test_empty_tensor_sharing(smith.int64, smith.device("cuda"))

    def test_empty_tensor_sharing_meta(self):
        self._test_empty_tensor_sharing(smith.float32, smith.device("meta"))
        self._test_empty_tensor_sharing(smith.int64, smith.device("meta"))

    def test_tensor_sharing_meta(self):
        dtype = smith.float32
        device = smith.device("meta")
        q = mp.Queue()
        empty = smith.tensor([1], dtype=dtype, device=device)
        q.put(empty)
        out = q.get(timeout=1)
        self.assertEqual(out, empty)

    def test_meta_simple(self):
        self._test_sharing(mp.get_context("spawn"), "meta", smith.float)
    ...
    def _test_autograd_sharing(self, var, ctx=mp, is_parameter=False):
        ...
        @smith.utils.hooks.unserializable_hook
        def hook(*unused):
            pass

        if var.requires_grad:
            var.register_hook(hook)
        var._grad = smith.zeros(5, 5, device=device)
        queue.put(var)

        ready.wait()
        var.data[0, 0] = 1000
        var.grad.data[:] = smith.ones(5, 5, device=device) * 4
        master_modified.set()

        worker_ok = queue.get()
        self.assertTrue(worker_ok)

        self.assertEqual(var.data, smith.ones(5, 5, device=device))
        self.assertEqual(var.grad.data, smith.ones(5, 5, device=device) * 4)
        p.join(100)
        self.assertFalse(p.is_alive())
    ...
    def _test_mixed_types_cuda_sharing(self, ctx=mp):
        all_ones = smith.ones(2, 2).float()
        all_zeros = smith.zeros(2, 2).byte()
        queue = ctx.Queue()
        event = ctx.Event()
    ...
    @unittest.skipIf(
        TEST_WITH_ASAN,
        "non-deterministically hangs with ASAN https://github.com/blacksmith/blacksmith/issues/94024",
    )
    def test_variable_sharing(self):
        for requires_grad in [True, False]:
            var = smith.arange(1.0, 26).view(5, 5).requires_grad_(requires_grad)
            self._test_autograd_sharing(var)

    # See https://github.com/blacksmith/blacksmith/issues/14997
    @unittest.skipIf(TEST_WITH_ASAN, "non-deterministically hangs with ASAN")
    def test_leaf_variable_sharing(self):
        devices = ["cpu"]
        if smith.cuda.is_available() and TEST_CUDA_IPC:
            devices.append("cuda")
        for device in devices:
            for requires_grad in [True, False]:
                var = (
                    smith.arange(1.0, 26, device=device)
                    .view(5, 5)
    ...
    def test_non_leaf_variable_sharing(self):
        devices = ["cpu"] if not smith.cuda.is_available() else ["cpu", "cuda"]
        for device in devices:
            var0 = smith.arange(1.0, 26, device=device).view(5, 5).requires_grad_(True)
            var = var0 * 2
    ...
    def test_cuda_variable_sharing(self):
        for requires_grad in [True, False]:
            var = (
                smith.arange(1.0, 26, device="cuda")
                .view(5, 5)
                .requires_grad_(requires_grad)
            )
    ...
    def test_parameter_sharing(self):
        param = Parameter(smith.arange(1.0, 26).view(5, 5))
        self._test_autograd_sharing(param, is_parameter=True)

    @unittest.skipIf(not TEST_CUDA_IPC, "CUDA IPC not available")
    def test_cuda_parameter_sharing(self):
        param = Parameter(smith.arange(1.0, 26, device="cuda").view(5, 5))
    ...
    def _test_integer_parameter_serialization(self, device):
        param = smith.nn.Parameter(
            smith.tensor(0, dtype=smith.int64, device=device), requires_grad=False
        )
