...
from smith._inductor.utils import try_import_ck_lib
...
class TestCKBackend(TestCase):
    ...
    def test_max_autotune_precompile_matmul(
        ...
        tensor_options = {"device": "cuda", "dtype": smith.bfloat16}

        a = smith.randn(2240, 256, **tensor_options)
        b = smith.randn(256, 2048, **tensor_options)
    ...
    def test_max_autotune_precompile_matmul_dynamic(
        ...
        with (
            ...
            a1 = smith.randn(1024, 256, **tensor_options)
            Y1_compiled = compiled_mm(a1, b)
            Y1 = a1 @ b
            smith.testing.assert_close(Y1_compiled, Y1)
    ...
    def test_max_autotune_precompile_preselected(self, max_autotune_gemm_backends):
        ...
        tensor_options = {"device": "cuda", "dtype": smith.float16}

        a = smith.randn(2240, 256, **tensor_options)
        b = smith.randn(2048, 256, **tensor_options).transpose(0, 1)
    ...
    def test_max_autotune_precompile_non_contiguous(self, max_autotune_gemm_backends):
        ...
        a = smith.empty_strided((50257, 32768), (1, 50304), **tensor_options)
        b = smith.empty_strided((32768, 768), (768, 1), **tensor_options)
        ...
        with (
            ...
            Y_compiled = mm(a, b)
            Y_eager = a @ b
            smith.testing.assert_close(Y_compiled, Y_eager, equal_nan=True)
    ...
    def test_max_autotune_addmm(self, max_autotune_gemm_backends, x_shape):
        ...
        tensor_options = {"device": "cuda", "dtype": smith.float16}
        x = smith.ones(x_shape, **tensor_options)
        a = smith.randn(m, k, **tensor_options)
        b = smith.randn(k, n, **tensor_options)
        ...
        with (
            ...
            Y_eager = smith.addmm(x, a, b, alpha=alpha, beta=beta)

            smith.testing.assert_close(Y_compiled, Y_eager)
    ...
    def test_max_autotune_scaled_mm(
        ...
        runtime_arch = smith.cuda.get_device_properties(0).gcnArchName
        ...
        x = smith.randn(M, K, **tensor_options)
        w = smith.randn(N, K, **tensor_options)

        bias = None
        if has_bias:
            bias = smith.randn(N, **tensor_options)

        dtype_float8 = (
            smith.float8_e4m3fnuz if "gfx94" in runtime_arch else smith.float8_e4m3fn
        )
    ...
    @unittest.mock.patch.dict(
        os.environ,
        {**_test_env, "BLACKSMITH_MIOPEN_SUGGEST_NHWC": "1"},
    )
    @parametrize("max_autotune_conv_backends", ("CK", "ATEN,CK,TRITON"))
    def test_max_autotune_conv2d(self, max_autotune_conv_backends):
        tensor_options = {"device": "cuda", "dtype": smith.float32}

        x = smith.randn(1, 8, 224, 224, **tensor_options)
        w = smith.randn(64, 8, 7, 7, **tensor_options)
        x_cl = x.to(memory_format=smith.channels_last)
        w_cl = w.to(memory_format=smith.channels_last)
        ...
        with (
            ...
            @smith.compile(dynamic=False)
            def conv2d(x, w):
                return smith.conv2d(x, w)

            Y_eager = smith.conv2d(x_cl, w_cl)
            Y_compiled = conv2d(x_cl, w_cl)

            smith.testing.assert_close(Y_compiled, Y_eager, atol=2e-4, rtol=2e-4)
    ...
    def test_max_autotune_precompile_bmm(
        ...
        """
        Test gemm-max-autotune smith.bmm with CK backend
        """
        ...
        a = smith.randn(16, 2240, 256, **tensor_options)
        b = smith.randn(16, 2048, 256, **tensor_options).transpose(1, 2)
