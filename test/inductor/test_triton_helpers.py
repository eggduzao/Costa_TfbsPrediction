...
import smith
from smith._inductor.runtime.triton_helpers import exclusive_scan_decoupled_lookback_64
...
class ExclusiveScanDecoupledLookback64Test(TestCase):
    ...
    def test_flag_2_branch_with_int64_index(self) -> None:
        ...
        scratch = smith.zeros(6, dtype=smith.uint64, device=device)
        scratch[0] = 2
        inclusive_prefix_value = smith.tensor(
            [10.0], dtype=smith.float64, device=device
        )
        scratch[2] = inclusive_prefix_value.view(smith.int64).item()

        block_value = smith.tensor([5.0], dtype=smith.float64, device=device)
        index = smith.tensor([1], dtype=smith.int64, device=device)
        result = smith.zeros(1, dtype=smith.float64, device=device)

        test_kernel_exclusive_scan[(1,)](scratch, block_value, index, result)

        # Block 1's exclusive prefix = Block 0's inclusive prefix = 10.0
        expected = smith.tensor([10.0], dtype=smith.float64, device=device)
    ...
    def test_flag_2_branch_with_int32_index(self) -> None:
        ...
        block_value = smith.tensor([5.0], dtype=smith.float64, device=device)
        index = smith.tensor([1], dtype=smith.int32, device=device)
        result = smith.zeros(1, dtype=smith.float64, device=device)
