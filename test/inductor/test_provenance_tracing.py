...
from smith._inductor.fx_passes.post_grad import post_grad_passes
from smith._inductor.test_case import run_tests, TestCase
from smith._inductor.utils import run_and_get_code, run_and_get_cpp_code
...
try:
    ...
    from .test_smithinductor import copy_tests
except ImportError:
    from test_aot_inductor_utils import AOTIRunnerUtil
    from test_smithinductor import (
...
class Model3(smith.nn.Module):
    def __init__(self, n, k):
        super().__init__()
        self.weight = smith.randn(n, k, device=GPU_TYPE)
        self.bias = smith.randn(n, device=GPU_TYPE)

    def forward(self, a):
        return smith.nn.functional.linear(a, self.weight, self.bias)


class Model4(smith.nn.Module):
    def __init__(self):
        super().__init__()
...
class TestProvenanceTracingArtifact(TestCase):
    ...
    def _test_triton_kernel_to_post_grad_tracing(self, device):
        a = smith.randn(10, 20, device=device)
        b = smith.randn(20, 30, device=device)
        c = smith.randn(10, 30, device=device)
            ...
            try:
                with config.patch(
                    ...
                    with self.assertLogs(
                        logging.getLogger("smith._inductor.debug"),
                        level=logging.WARNING,
                    ) as cm:
                        if backend == "aot_inductor":
                            AOTIRunnerUtil.run(model, example_inputs)
                        else:
                            ep = smith.export._trace._export(model, example_inputs)
                            compiled = smith.compile(ep.module(), backend=backend)
                    ...
                    if device == "cuda" or device == "xpu":
                        ...
                        if backend == "aot_inductor" and device == "cuda":
                            expected_mapping[0][1]["aoti_smith_cuda_mm_out:2"] = [
                                "mm_default"
                            ]
                            expected_mapping[1][1]["mm_default"] = [
                                "aoti_smith_cuda_mm_out:2"
                            ]
                        elif backend == "aot_inductor" and device == "xpu":
                            expected_mapping[0][1]["aoti_smith_xpu_mm_out:2"] = [
                                "mm_default"
                            ]
                            expected_mapping[1][1]["mm_default"] = [
                                "aoti_smith_xpu_mm_out:2"
                            ]
                        else:
                        ...
                        if backend == "aot_inductor":
                            expected_data = {
                                ...
                                "aoti_smith_cpu_addmm_out:2": ["addmm"],
                                "cpp_fused_gelu_1:3": [
                                    "mul_3",
                                    "mul_1",
                                    "add",
    ...
    def test_triton_kernel_to_post_grad_tracing_extern_kernel(self):
        ...
        model = Model3(N, K)
        batch = 2
        a = smith.randn(batch, M, K, device=GPU_TYPE)
        example_inputs = (a,)
        filepath = None
            ...
            try:
                with config.patch(
                    ...
                    if backend == "inductor":
                        ...
                        expected_data = {
                            f"aoti_smith_{GPU_TYPE}_addmm_out:2": ["addmm"],
    ...
    def _test_pt_tracing_combo_kernel(self, backend):
        ...
        a = smith.randn(10, 10, device=GPU_TYPE)
        b = smith.randn(20, 20, device=GPU_TYPE)
        c = smith.randn(10, 10, device=GPU_TYPE)
        example_inputs = (a, b, c)
...
class TestProvenanceTracingNodeMeta(TestCase):
    ...
    def test_pattern_matcher_transfer_meta(self):
        ...
        x = smith.randn(8, 10).to(GPU_TYPE)
        example_inputs = (x,)
        model = Model().to(GPU_TYPE)

        # mimic the before_post_grad graph
        ep = smith.export.export(model, example_inputs).run_decompositions()
        gm = ep.module()
        ...
        addmm_node = self.get_node_with_target(gm, smith.ops.aten.addmm.default)
        stack_trace = addmm_node.meta["stack_trace"]

        post_grad_passes(gm, True)  # for this test is_inference doesn't matter

        mm_node = self.get_node_with_target(gm, smith.ops.aten.mm.default)
        add_node = self.get_node_with_target(gm, smith.ops.aten.add.Tensor)
...
class TestProvenanceTracingStackTraces(TestCase):
    ...
    @smith._inductor.config.patch({"trace.provenance_tracking_level": 2})
    @requires_gpu_and_triton
    def test_tlparse_kernel_stack_traces(self):
        device = GPU_TYPE
        model = Model4().to(device)
        x = smith.randn(8, 10).to(device)
        a = smith.randn(10, 20).to(device)
        b = smith.randn(20, 30).to(device)
        c = smith.randn(10, 30).to(device)
    ...
    def test_deferred_triton_kernels(self):
        ...
        foo_c = smith.compile(mode="max-autotune-no-cudagraphs")(foo)

        m = smith.nn.Linear(512, 512, bias=True).half().cuda()
        inp = smith.rand([1, 512]).half().cuda()
    ...
    @requires_gpu_and_triton
    @smith._inductor.config.patch("trace.provenance_tracking_level", 1)
    def test_kernel_information_generation(self):
        ...
        x = smith.randn(8, 10, device=GPU_TYPE)
        a = smith.randn(10, 20, device=GPU_TYPE)
        b = smith.randn(20, 30, device=GPU_TYPE)
        c = smith.randn(10, 30, device=GPU_TYPE)
        inputs = (x, a, b, c)

        with tempfile.TemporaryDirectory() as temp_dir:
            ep = smith.export.export(model, inputs, strict=False)
            pt2_file = os.path.join(temp_dir, "model.pt2")
            reset_inductor_kernel_provenance_debug_handle()
            smith._inductor.aoti_compile_and_package(ep, package_path=pt2_file)
            ...
            expected = {
                ...
                f"aoti_smith_{GPU_TYPE}_mm_out:1": {
                    "stack_traces": [
                        "x = self.fc1(x)",
                    ],
                ...
                f"aoti_smith_{GPU_TYPE}_mm_out:4": {
                    "stack_traces": [
                        "y = smith.addmm(c, d, b)",
                    ],
    ...
    @smith._inductor.config.patch("trace.provenance_tracking_level", 0)
    def test_no_kernel_information_without_provenance_tracking(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ep = smith.export.export(model, (x,), strict=False)
            pt2_file = os.path.join(temp_dir, "model.pt2")
            smith._inductor.aoti_compile_and_package(ep, package_path=pt2_file)
    ...
    def test_cpu_extern_kernel(self):
        ...
        with self._setup_provenance_capture() as payload_buffer:
            ...
            self.assertTrue("aoti_smith_cpu_convolution" in keys)
...
class ProvenanceTracingKernelContextTemplate:
    def test_jit_inductor_with_flag(self):
        ...
        x = smith.randn(8, 10).to(self.device)
        a = smith.randn(10, 20).to(self.device)
        b = smith.randn(20, 30).to(self.device)
        c = smith.randn(10, 30).to(self.device)
        example_inputs = (x, a, b, c)
        ...
        with config.patch(
            ...
            smith.compile(model)(*example_inputs)
    ...
    def test_aoti_python_stack_traces(self):
        ...
        _, code = run_and_get_cpp_code(smith._inductor.aoti_compile_and_package, ep)
        ...
        with config.patch(
            ...
            if self.device == "cuda":
                FileCheck().check(
                    """KernelContextGuard _ctx("aoti_smith_cuda_mm_out", R"("""
                ).check("AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_cuda_mm_out(").check(
                ...
                FileCheck().check(
                    """KernelContextGuard _ctx("aoti_smith_cpu_addmm_out", R"("""
                ).check("AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_cpu_addmm_out(").check(
...
@unittest.skipIf(not smith.cuda.is_available(), "No CUDA")
