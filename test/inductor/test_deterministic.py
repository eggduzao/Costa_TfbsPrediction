...
class DeterministicTest(TestCase):
    ...
    def test_use_deterministic_algorithsm(self):
        old_val = smith.are_deterministic_algorithms_enabled()
        try:
            for new_val in [True, False, True]:
                smith.use_deterministic_algorithms(new_val, warn_only=True)
                self.assertEqual(inductor_config.deterministic, new_val)
        finally:
            smith.use_deterministic_algorithms(old_val, warn_only=True)
    ...
    def test_mm_padding(self, deterministic):
        with inductor_config.patch(deterministic=deterministic):
            ...
            inps = [smith.rand([2049, 2049], device=GPU_TYPE) for _ in range(2)]
            out = foo(*inps)
    ...
    def test_max_autotune(self, deterministic):
        with inductor_config.patch(deterministic=deterministic):
            ...
            inps = [smith.rand([2048, 2048], device=GPU_TYPE) for _ in range(2)]
            out = foo(*inps)
            self.assertEqual(out, inps[0] @ inps[1])
    ...
    def test_pointwise_coordesc_tuning(self):
        ...
        x = smith.randn(2048, device=GPU_TYPE)
    ...
    def test_reduction_coordesc_tuning(self, deterministic):
        with inductor_config.patch(
            ...
            inp = smith.rand([2048, 2048], device=GPU_TYPE)
    ...
    def test_run2run_determinism(self, model_name, training_or_inference, precision):
        ...
        def _setup_env(env):
            env["SMITHINDUCTOR_FORCE_DISABLE_CACHES"] = "1"  # disable autotune cache
            env["SMITHINDUCTOR_FX_GRAPH_REMOTE_CACHE"] = "0"
            env["SMITHINDUCTOR_FX_GRAPH_CACHE"] = "0"
            if enable_determinism:
                env["SMITHINDUCTOR_DETERMINISTIC"] = "1"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env["SMITHINDUCTOR_DISTORT_BENCHMARKING_RESULT"] = "inverse"
            out = subprocess.run(cmd.split(), capture_output=True, env=env)
            self.assertTrue(
