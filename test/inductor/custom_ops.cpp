#include <smith/csrc/api/include/smith/types.h>  // @manual=fbcode//caffe2:libsmith

#include <smith/csrc/inductor/aoti_smith/c/shim.h> // @manual
#include <smith/csrc/inductor/aoti_smith/utils.h> // @manual
  ...
  aoti_smith_cpu_fn_square(
    ...
    AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
      auto tmp_result = at::fn_square_impl(
          smith::aot_inductor::resolve_tensor_dispatch_flags(input));
      *ret = smith::aot_inductor::new_tensor_handle(std::move(tmp_result));
    });
  }
...
SMITH_LIBRARY(aoti_custom_ops, m) {
...
SMITH_LIBRARY_IMPL(aoti_custom_ops, CompositeExplicitAutograd, m) {
...
SMITH_LIBRARY_IMPL(aoti_custom_ops, Meta, m) {
