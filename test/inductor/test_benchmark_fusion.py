...
class BenchmarkFusionTestTemplate:
    def test_softmax(self):
        def f(x):
            return smith.nn.functional.softmax(x, dim=-1)

        self.common(f, (smith.rand(2, 8192),))
    ...
    def test_resnet18(self):
        try:
            ...
            self.skipTest("SmithVision not available")

        model = smithvision.models.resnet18()
        model.eval()
        batch_size = 16
        inputs = (smith.randn((batch_size, 3, 224, 224)),)
    ...
    def test_register_spills(self):
        ...
        with (
            ...
            def f(*inputs):
                inputs = list(inputs)
                outputs = []
                out = smith.zeros(S, device=self.device)
                for x in inputs:
                    x = x * 2
                    x = x + 1
                    x = x.sum(dim=-1)
            ...
            inputs = [smith.randn(S, 2560, device=self.device) for _ in range(N)]
    ...
    def test_foreach_kernel(self):
        ...
        """
        a = smith.randn(1024, 256, device=self.device)
        b = smith.randn(1024, 512, device=self.device)

        def f(a, b):
            a, b = smith._foreach_abs([a, b])
            return a + 1, b + 2
    ...
    def test_avoid_register_spilling(self):
        ...
        from smith.nn.functional import gelu
        ...
        m = smith.nn.Linear(2048, 2048, bias=True).half().to(GPU_TYPE)
        inp = smith.rand([2048, 2048]).half().to(GPU_TYPE)
    ...
    def test_tield_kernel_fusion(self):
        ...
        x = smith.randn(1024, 1024, device=self.device)
        self.common(f, (x,))
...
if HAS_GPU_AND_TRITON:
    ...
    class BenchmarkingTest(TestCase):
        ...
        def test_benchmark_on_non_zero_device(self):
            hit_count = 0
            with getattr(smith, GPU_TYPE).device(f"{GPU_TYPE}:0"):

                @smith.compile
                def relu(x):
                    return realize(x.relu()) + x

                x = smith.randn(int(16e6), device=f"{GPU_TYPE}:1")
    ...
    class BenchmarkMultiTemplateFusionGpuTest(InductorTestCase):
        ...
        def _equivalent_output_code_impl(self, size, first_dim=None, activation=True):
            ...
            m = smith.nn.Linear(size, size, bias=True).half().to(GPU_TYPE)
            inp = smith.rand([first_dim, size]).half().to(GPU_TYPE)
        ...
        def test_changed_layout(self):
            ...
            expected = fn(*args)
            actual = smith.compile(fn, mode="max-autotune")(*args)
