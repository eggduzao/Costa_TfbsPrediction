...
from smith._inductor.codegen.simd_kernel_features import SIMDKernelFeatures
from smith._inductor.codegen.triton import FixedTritonConfig, TritonKernel
from smith._inductor.test_case import TestCase
from smith._inductor.utils import run_and_get_code
from smith.testing import assert_close
...
class TestingHeuristics(Inducsmithoices):
...
class CooperativeReductionTests(TestCase):
    ...
    def run_and_check(self, fn, args, dtype=None, *, expect_kernel_count=1):
        ...
        if isinstance(raw_expected, (tuple, list)):
            ...
            if dtype is not None:
                expected = type(raw_expected)(
                    [
                        t.to(dtype) if isinstance(t, smith.Tensor) else t
                ...
                expected = type(raw_expected)(
                    [
                        t.to(smith.float64) if isinstance(t, smith.Tensor) else t
                        for t in raw_expected
                    ]
                )
        else:
            ...
            if dtype is not None:
                ...
                expected = raw_expected.to(smith.float64)

        fn_compiled = smith.compile(fn, fullgraph=True)
        ...
        if isinstance(expected, (tuple, list)):
            ...
            if dtype is not None:
                result = type(result)(
                    [t.to(dtype) if isinstance(t, smith.Tensor) else t for t in result]
                )
            else:
                result = type(result)(
                    [
            ...
            if dtype is not None and isinstance(result, smith.Tensor):
                result = result.to(dtype)
            elif isinstance(result, smith.Tensor):
                result = result.to(smith.float64)

        # Apply assert_close with fixed tolerances for tensor comparisons
        if isinstance(result, smith.Tensor) and isinstance(expected, smith.Tensor):
            ...
            for r_item, e_item in zip(result, expected):
                if isinstance(r_item, smith.Tensor) and isinstance(
                    e_item, smith.Tensor
                ):
        ...
        if "async_compile.multi_kernel" not in source_code:
            self.assertEqual(
                smith._inductor.metrics.generated_kernel_count, expect_kernel_count
            )
        return source_code
    ...
    def test_reduction_fns(self, name, dtype):
        ...
        args = [smith.randn(1, 1024**2, device=GPU_TYPE, dtype=dtype) for _ in range(2)]
    ...
    def test_bool_reduction_fns(self):
        ...
        args = [smith.randn(1024, device=GPU_TYPE) for _ in range(2)]
    ...
    def test_non_power_of_2(self, bs, count):
        ...
        args = [smith.randn([bs, count], device=GPU_TYPE)]
        self.run_and_check(fn, args)

    def test_chained_reductions(self):
        def fn(x):
            for _ in range(8):
                x = x + smith.softmax(x, 1)
            return x

        args = [smith.randn(4, 100000, device=GPU_TYPE)]
        source_code = self.run_and_check(fn, args)
        if "async_compile.multi_kernel" in source_code:
            return
...
class TestFixedConfigs(TestCase):
    def _check(self, fn, args, *, persistent=False, cooperative=True, cfg):
        ...
        with smith._inductor.virtualized.V.set_choices_handler(heuristic):
            result, (source_code,) = run_and_get_code(
                smith.compile(fn, fullgraph=True), *args
            )
        self.assertEqual(result, expected)
    ...
    def test_fixed_configs(self, persistent, cooperative, cfg):
        def fn(x):
            return smith.softmax(x + 1, dim=-1) + x

        args = [smith.randn(8, 8000, device=GPU_TYPE)]
    ...
    def test_welford_non_power_of_2_rsplit(self, persistent, x, r, rsplit):
        def fn(x):
            return smith.var_mean(x, dim=-1)

        cfg = {"XBLOCK": 64, "RSPLIT": rsplit, "num_warps": 8}
        if not persistent:
            cfg["R0_BLOCK"] = 64
        args = [smith.randn(x, r, device=GPU_TYPE)]
    ...
    def test_min_max_non_power_of_2_rsplit(self, persistent):
        def fn(x):
            return (
                smith.amin(x, dim=-1),
                smith.amax(x, dim=-1),
                smith.argmin(x, dim=-1),
                smith.argmax(x, dim=-1),
            )
        ...
        args = [
            smith.stack(
                [
                    smith.arange(10, 4096, device=GPU_TYPE),
                    -smith.arange(10, 4096, device=GPU_TYPE),
                ]
            )
        ]
