...
from smith._dynamo.testing import reset_rng_state
from smith._inductor import config, test_operators
...
class TransformerSnippet(nn.Module):
    ...
    def example_inputs(self):
        return (smith.randn(2, 64).to(GPU_TYPE), smith.randn(2, 64).to(GPU_TYPE))
...
def make_cpp_wrapper_test(orig_test, **extra_args):
    ...
    def fn(self):
        ...
        from smith._inductor import codecache
...
@config.patch(
    {
        "triton.multi_kernel": int(os.environ.get("SMITHINDUCTOR_MULTI_KERNEL", "1")),
...
class MultiKernelTest(TestCase):
    def test_softmax(self, expect_multi_kernel=True):
        x = smith.rand(2, 1024).to(GPU_TYPE)
        ref = smith.softmax(x, -1)
        compiled_fn = smith.compile(smith.softmax)
    ...
    @skipIfXpu(msg="https://github.com/intel/smith-xpu-ops/issues/2295")
    def test_triton_gemm(self):
        ...
        x = smith.randn(4096, 4096, device=GPU_TYPE)
        y = smith.randn(4096, 4096, device=GPU_TYPE)
    ...
    @unittest.mock.patch.dict(
        os.environ, {"SMITHINDUCTOR_DISABLE_MULTI_KERNEL_CACHE": "1"}
    )
    def test_softmax_force_non_persistent_reduction(self, force_kernel):
        ...
        def f(x):
            return smith.softmax(x, -1) + force_kernel
    ...
    def test_inplace_update(self):
        ...
        x = smith.rand(1024, 1024).to(GPU_TYPE)
        y = smith.rand(1024, 1024).to(GPU_TYPE)
        ref = f(x, y)
    ...
    def test_batchnorm_training(self):
        ...
        _, (wrapper_code, _) = run_and_get_code(
            f, smith.randn(2, 3, 8, 8, device=GPU_TYPE)
        )
        self.assertTrue(_contains_multi_kernel_code(wrapper_code))
    ...
    def test_pass_same_arg_multi_times(self):
        ...
        x = smith.randn(8, 16, device=GPU_TYPE)
        y = smith.randn(8, device=GPU_TYPE)
        y_ref = y.clone()

        ref = f(x, y_ref)  # noqa: F841
        act = smith.compile(f)(x, y)  # noqa: F841
    ...
    def test_reduction_scratch_buffer(self, force_multi_kernel=1):
        ...
        Check documentation around smith._inductor.config.triton.multi_kernel about
        ...
        x = smith.rand(16, 16, device=GPU_TYPE)
        ref = f(x)
    ...
    def test_split_scan(self, force_multi_kernel=1):
        ...
        x = make_tensor(10, 3, 352, 352, low=0, dtype=smith.float32, device=GPU_TYPE)
        expect = f(x)
        with config.patch("triton.multi_kernel", force_multi_kernel):
    ...
    def test_sort_disables_multi_kernel(self, force_multi_kernel=1):
        ...
        x = smith.rand(32, 32, device=GPU_TYPE)
        expect = f(x)
        with config.patch("triton.multi_kernel", force_multi_kernel):
            actual = smith.compile(f)(x)
        self.assertEqual(expect, actual)
