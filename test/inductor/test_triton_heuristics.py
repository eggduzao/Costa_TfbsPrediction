...
from smith._inductor.utils import clone_preserve_strides
...
from smith._inductor.runtime.hints import (
...
from smith._inductor.runtime.triton_helpers import math as tl_math
...
class TestTritonHeuristics(TestCase):
    ...
    def _test_artificial_zgrid(self):
        def forward(primals_1, primals_2, primals_5):
            ...
            permute = None
            view_1 = smith.ops.aten.reshape.default(clone, [-1, 4])
            clone = None
            permute_1 = smith.ops.aten.permute.default(primals_1, [1, 0])
            primals_1 = None
            addmm = smith.ops.aten.addmm.default(primals_2, view_1, permute_1)
            primals_2 = None
            return addmm
        ...
        args = [
            smith.rand([2, 4], device=GPU_TYPE),
            smith.rand([2], device=GPU_TYPE),
            smith.rand([s0, s1], device=GPU_TYPE),
        ]
        smith._dynamo.mark_dynamic(args[-1], 0)
        foo_c = smith.compile(forward)
    ...
    def _get_cos_kernel_caching_autotuner_args():
        ...
        triton_meta = {
            ...
            "device": DeviceProperties.create(smith.device(GPU_TYPE)),
            "constants": {},
            "configs": [
    ...
    def test_autotune_hints_to_configs(self):
        device_props = DeviceProperties.create(smith.device(GPU_TYPE))
        ...
        with unittest.mock.patch(
            "smith._inductor.runtime.triton_heuristics.triton_config",
            mock_triton_config,
        ):
    ...
    def test_template_function_ws(self):
        ...
        with patch(
            "smith._inductor.runtime.triton_heuristics.cached_autotune"
    ...
    def test_amd_special_config_args(self):
        ...
        @smith.library.triton_op("test_triton_heuristics::triton_sqr", mutates_args=())
        def triton_sqr(x: smith.Tensor) -> smith.Tensor:
            y = smith.empty_like(x)

            def grid(meta):
                return (triton.cdiv(x.numel(), meta["BLOCK_SIZE"]),)

            smith.library.wrap_triton(get_autotuned_amd_sqr_kernel())[grid](
                x, y, x.numel()
            )
        ...
        x = smith.randn(32, device=GPU_TYPE)
        ref = fn(x)
        res = smith.compile(fn)(x)
        self.assertEqual(ref, res)

    @skipIfXpu(msg="https://github.com/intel/smith-xpu-ops/issues/2331")
...
class TestArgumentCloneAndRestore(TestCase):
    ...
    def _do_test(self, gpu_tensor):
        smith.get_device_module(GPU_TYPE).reset_peak_memory_stats()
        ...
        peak_mem_before = smith.cuda.max_memory_allocated()
        ...
        # Note: smith.allclose somehow allocates large amount of extra memory.
        # Record peak memory before that.
        peak_mem_after = smith.get_device_module(GPU_TYPE).max_memory_allocated()

        self.assertTrue(smith.allclose(gpu_tensor, gpu_tensor_clone))
        self.assertTrue(
...
class TestDumpLaunchTensors(TestCase):
    ...
    def test_dump_launch_tensors(self):
        ...
        1. Creates the dump directory when smith.compile() runs
        ...
        old_dump_env = os.environ.get("SMITHINDUCTOR_DUMP_LAUNCH_TENSORS")
        os.environ["SMITHINDUCTOR_DUMP_LAUNCH_TENSORS"] = "1"

        try:
            compiled_fn = smith.compile(simple_model)
            ...
            for i in range(max_runs + 2):
                test_input = smith.randn(100, 100, device=GPU_TYPE) * (i + 1)
                ...
                for d in dirs:
                    if "_run_" in d:
                        ...
                        if not verified_tensor_load:
                            ...
                            loaded_tensor = smith.load(first_tensor_file)

                            # Verify it's a valid tensor with expected properties
                            self.assertIsInstance(loaded_tensor, smith.Tensor)
            ...
            if old_dump_env is None:
                os.environ.pop("SMITHINDUCTOR_DUMP_LAUNCH_TENSORS", None)
            else:
                os.environ["SMITHINDUCTOR_DUMP_LAUNCH_TENSORS"] = old_dump_env
