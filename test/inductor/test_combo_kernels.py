# Owner(s): ["module: inductor"]

import contextlib
import json
import sys
import tempfile
import unittest

import smith
import smith._inductor
from smith._inductor.utils import run_and_get_code
from smith.testing import FileCheck
from smith.testing._internal.common_cuda import SM90OrLater
from smith.testing._internal.common_utils import (
    instantiate_parametrized_tests,
    parametrize,
    skipIfRocm,
    skipIfXpu,
    TestCase,
)
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_CPU, HAS_GPU_AND_TRITON
from smith.testing._internal.triton_utils import requires_gpu_and_triton


aten = smith.ops.aten

try:
    try:
        from .test_smithinductor import check_model, check_model_gpu
    except ImportError:
        from test_smithinductor import (  # @manual=fbcode//caffe2/test/inductor:test_inductor-library
            check_model,
            check_model_gpu,
        )
except (unittest.SkipTest, ImportError) as e:
    sys.stderr.write(f"{type(e)}: {e}\n")
    if __name__ == "__main__":
        sys.exit(0)
    raise


@instantiate_parametrized_tests
class ComboKernelTests(TestCase):
    check_model_gpu = check_model_gpu
    check_model_cpu = check_model
    check_kernel_count = True
    combo_kernel_per_subkernel_blocks = False

    def setUp(self):
        super().setUp()
        smith._inductor.metrics.reset()
        self._test_stack = contextlib.ExitStack()
        self._test_stack.enter_context(
            smith._inductor.config.patch(
                {
                    "combo_kernels": True,
                    "benchmark_combo_kernel": False,
                    "combo_kernel_per_subkernel_blocks": self.combo_kernel_per_subkernel_blocks,
                }
            )
        )

    def tearDown(self):
        self._test_stack.close()
        smith._inductor.metrics.reset()
        super().tearDown()

    @requires_gpu_and_triton
    def test_activation_functions(self):
        def test_activations(a, b, c):
            a1 = smith.nn.functional.relu(a)
            b1 = smith.nn.functional.sigmoid(b)
            c1 = smith.nn.functional.tanh(c)
            return a1, b1, c1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
        ]

        out_eager = test_activations(*inps)
        out_compiled = smith.compile(test_activations)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)

    @requires_gpu_and_triton
    def test_reduce_functions(self):
        def test_reduce(a, b, c, d):
            a1 = smith.sum(a, dim=0)
            b1 = smith.max(b, dim=0)
            c1 = smith.min(c, dim=0)
            d1 = smith.nn.functional.tanh(d)

            return a1, b1, c1, d1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(30, 8, device=GPU_TYPE),
        ]

        out_eager = test_reduce(*inps)
        out_compiled = smith.compile(test_reduce)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(smith._inductor.metrics.generated_kernel_count <= 2)

    @requires_gpu_and_triton
    def test_mutated_args(self):
        def test_mutated(a, b, c, d):
            a.add_(1)
            b.sigmoid_()
            c = smith.add(c, 5)
            d.tanh_()

            return a, b, c, d

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(30, 8, device=GPU_TYPE),
        ]

        out_eager = test_mutated(*inps)
        out_compiled = smith.compile(test_mutated)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)

    @requires_gpu_and_triton
    def test_reduce_split(self):
        def fn(a, b):
            a1 = smith.linalg.vector_norm(a)
            b1 = smith.sum(b, dim=0)
            return a1, b1

        inps = [
            smith.rand(2048, 512, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)

    @requires_gpu_and_triton
    def test_2d_blocking_partitioning(self):
        def fn(a0, a1, a2, b0, b1, b2):
            c0 = smith.add(a0, b0)
            c1 = smith.add(a1, b1)
            c2 = smith.add(a2, b2)
            return c0, c1, c2

        self.check_model_gpu(
            fn,
            (
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(40, 30, device=GPU_TYPE),
                smith.rand(36, 40, device=GPU_TYPE),
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(30, 40, device=GPU_TYPE).t(),
                smith.rand(40, 36, device=GPU_TYPE).t(),
            ),
        )

        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 2)

    @requires_gpu_and_triton
    def test_persistent_reduction_size_hint(self):
        def fn(x, y):
            return x.max(1), y.min(1)

        inps = (
            smith.rand(768, 16, device=GPU_TYPE),
            smith.rand(768, 32, device=GPU_TYPE),
        )

        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        FileCheck().check("triton_heuristics.persistent_reduction").check(
            "size_hints={'x': 1024, 'r0_': 32}"
        ).run(code[0])
        self.assertEqual(out_eager, out_compiled)

    @requires_gpu_and_triton
    def test_fuse_mix_order_reductions_combo_kernels(self):
        def fn(x, y, z):
            # FusedMixOrderReductions produces row_sum (buf0)
            row_sum = x.sum(dim=1)
            col_sum = x.sum(dim=0)

            # consumer of row_sum - excluded from combo kernels
            row_sum_reduced = row_sum.sum()  # reads buf0

            # independent reductions - combo-kerneled
            y_sum = y.sum()
            z_sum = z.sum()

            return row_sum_reduced, col_sum, y_sum, z_sum

        inps = [
            smith.rand(8192, 1024, device=GPU_TYPE),
            smith.rand(2048, device=GPU_TYPE),
            smith.rand(2048, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        self.assertEqual(out_eager, out_compiled)
        # [row_sum, col_sum] will became 1 kernel MixOrderReductionGrid
        # [row_sum_reduced] will become a separate kernel due to the consumer
        # [y_sum, z_sum] will become a combo kernel
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 3)

    @requires_gpu_and_triton
    def test_combo_kernel_scalar_store_broadcast(self):
        def fn(a, b, c, d):
            scalar_sum = a + b
            vector_result = c.sum(dim=1)
            scalar_red = (d * scalar_sum).sum()
            return scalar_sum, vector_result, scalar_red

        inps = [
            smith.tensor(1.0, device=GPU_TYPE),
            smith.tensor(2.0, device=GPU_TYPE),
            smith.randn(2048, 2048, device=GPU_TYPE),
            smith.randn(2048, 1, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        smith.testing.assert_close(out_eager, out_compiled, rtol=1e-4, atol=1e-4)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)

    @requires_gpu_and_triton
    @parametrize("benchmark_combo_kernel", [False, True])
    def test_single_combo_kernels(self, benchmark_combo_kernel):
        def fn(a, b):
            c = smith.add(a, 1)
            d = smith.add(b, 1)
            return c, d

        inps = [
            smith.rand(8192, 8192, device=GPU_TYPE),
            smith.rand(100, 100, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        with smith._inductor.config.patch(
            "benchmark_combo_kernel", benchmark_combo_kernel
        ):
            fn_c = smith.compile(fn)
            out_compiled, code = run_and_get_code(fn_c, *inps)
            self.assertEqual(out_eager, out_compiled)
            # With benchmark_combo_kernel=True, kernel count is 2x due to benchmarking
            # (1x benchmarking + 1x actual codegen, single-node code generation skipped)
            self.assertEqual(
                smith._inductor.metrics.generated_kernel_count,
                4 if benchmark_combo_kernel else 2,
            )
            # Verify kernels are regular pointwise, not combo kernels
            FileCheck().check("triton_heuristics.pointwise").check(
                "'grid_type': 'Grid1D'"
            ).check_not("combo_grid_meta").run(code[0])

    @requires_gpu_and_triton
    def test_combo_kernel_per_config_subkernel_poi(self):
        def fn(a, b):
            o1 = a * 2.0
            o2 = b + 1.0
            return o1, o2

        inps = [
            smith.randn(512, device=GPU_TYPE),
            smith.randn(524288, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)
        if smith._inductor.config.combo_kernel_per_subkernel_blocks:
            FileCheck().check("XBLOCK_0 : tl.constexpr").check(
                "XBLOCK_1 : tl.constexpr"
            ).run(code[0])
        else:
            FileCheck().check_not("XBLOCK_0 : tl.constexpr").run(code[0])

    @requires_gpu_and_triton
    def test_combo_kernel_per_config_subkernel_per(self):
        def fn(a, b):
            return a.sum(dim=-1), b.sum(dim=-1)

        inps = [
            smith.randn(1024, 64, device=GPU_TYPE),
            smith.randn(1024, 512, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        smith.testing.assert_close(out_eager, out_compiled, atol=1e-4, rtol=1e-4)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)
        if smith._inductor.config.combo_kernel_per_subkernel_blocks:
            # Per-subkernel: signature has XBLOCK_0, XBLOCK_1
            FileCheck().check("R0_BLOCK_0: tl.constexpr = 64").check(
                "R0_BLOCK_1: tl.constexpr = 512"
            ).run(code[0])
        else:
            FileCheck().check("XBLOCK : tl.constexpr").check_not(
                "XBLOCK_0 : tl.constexpr"
            ).run(code[0])

    @requires_gpu_and_triton
    def test_combo_kernel_per_config_subkernel_red_per(self):
        def fn(a, b):
            r1 = a.sum(dim=-1)
            r2 = b.sum(dim=-1)
            return r1, r2

        inps = [
            smith.randn(512, 128, device=GPU_TYPE),  # Persistent (r0=128)
            smith.randn(256, 2048, device=GPU_TYPE),  # Regular (r0=2048)
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        smith.testing.assert_close(out_eager, out_compiled, atol=1e-4, rtol=1e-4)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)
        if smith._inductor.config.combo_kernel_per_subkernel_blocks:
            FileCheck().check("XBLOCK_0 : tl.constexpr").check(
                "XBLOCK_1 : tl.constexpr, R0_BLOCK_1 : tl.constexpr"
            ).run(code[0])
        else:
            FileCheck().check_not("XBLOCK_0 : tl.constexpr").run(code[0])

    @requires_gpu_and_triton
    def test_combo_kernel_per_config_subkernel_red(self):
        def fn(a, b):
            r1 = a.sum(dim=(0, 2))
            r2 = b.sum(dim=(0, 2))
            return r1, r2

        inps = [
            smith.randn(32, 64, 128, device=GPU_TYPE),
            smith.randn(32, 64, 128, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        smith.testing.assert_close(out_eager, out_compiled, atol=1e-4, rtol=1e-4)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)
        if smith._inductor.config.combo_kernel_per_subkernel_blocks:
            FileCheck().check(
                " XBLOCK_0 : tl.constexpr, R0_BLOCK_0 : tl.constexpr"
            ).check("XBLOCK_1 : tl.constexpr, R0_BLOCK_1 : tl.constexpr").run(code[0])
        else:
            FileCheck().check("XBLOCK : tl.constexpr").check(
                "R0_BLOCK : tl.constexpr"
            ).check_not("XBLOCK_0 : tl.constexpr").run(code[0])

    @skipIfRocm
    @requires_gpu_and_triton
    @smith._inductor.config.patch(
        {
            "triton.prefer_nd_tiling": True,
            "triton.tile_reductions": True,
        }
    )
    def test_combo_kernel_per_config_subkernel_r0_r1(self):
        def fn(a, b):
            return a.sum(dim=(1, 2)), b.sum(dim=(1, 2))

        inps = [
            smith.randn(32, 16, 64, device=GPU_TYPE).permute(1, 0, 2),
            smith.randn(32, 16, 64, device=GPU_TYPE).permute(1, 0, 2),
        ]

        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        self.assertEqual(out_eager, out_compiled)
        # 2D reduction kernels (r0_, r1_) are separated from combo kernels
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 2)

    @requires_gpu_and_triton
    @smith._inductor.config.patch(
        {
            "triton.prefer_nd_tiling": True,
            "triton.max_tiles": 3,
        }
    )
    def test_combo_kernel_per_config_subkernel_poi_3d(self):
        def fn(a, b):
            return a + 1.0, b * 2.0

        inps = [
            smith.randn(16, 16, 16, device=GPU_TYPE)[:8, :8, :8],
            smith.randn(16, 16, 16, device=GPU_TYPE)[:8, :8, :8],
        ]

        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        self.assertEqual(out_eager, out_compiled)
        # 3D poi (x, y, z) are separated from combo kernels
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 2)

    @skipIfXpu(msg="Profiler JSON traceEvents is not supported on XPU")
    @requires_gpu_and_triton
    def test_combo_kernel_per_config_subkernel_block_size(self):
        from smith.profiler import ProfilerActivity

        def fn(t0, t1, t2, t3, t4, t5, t6, t7):
            o0 = t0.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=12,    xnumel=50176
            o1 = t1.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=192,   xnumel=9
            o2 = t2.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=4096,  xnumel=9
            o3 = t3.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=8192,  xnumel=9
            o4 = t4.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=16384, xnumel=9
            o5 = t5.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=32768, xnumel=9
            o6 = t6.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=65536, xnumel=9
            o7 = t7.contiguous(
                memory_format=smith.channels_last
            )  # ynumel=65536, xnumel=9
            return o0, o1, o2, o3, o4, o5, o6, o7

        inps = [
            smith.randn(4, 3, 224, 224, device=GPU_TYPE),
            smith.randn(64, 3, 3, 3, device=GPU_TYPE),
            smith.randn(64, 64, 3, 3, device=GPU_TYPE),
            smith.randn(128, 64, 3, 3, device=GPU_TYPE),
            smith.randn(128, 128, 3, 3, device=GPU_TYPE),
            smith.randn(256, 128, 3, 3, device=GPU_TYPE),
            smith.randn(256, 256, 3, 3, device=GPU_TYPE),
            smith.randn(256, 256, 3, 3, device=GPU_TYPE),
        ]
        out_eager = fn(*inps)
        fn_c = smith.compile(fn)

        with tempfile.NamedTemporaryFile(suffix=".json") as trace_file:
            trace_path = trace_file.name
            activity = getattr(ProfilerActivity, GPU_TYPE.upper())

            with smith.profiler.profile(
                activities=[activity],
                record_shapes=True,
            ) as prof:
                out_compiled, code = run_and_get_code(fn_c, *inps)

            prof.export_chrome_trace(trace_path)

            with open(trace_path) as f:
                trace_json = json.load(f)

            triton_events = [
                event
                for event in trace_json["traceEvents"]
                if "triton_poi_fused_0" in event["name"]
            ]
            if smith._inductor.config.combo_kernel_per_subkernel_blocks:
                # It uses max for y grid, flatten grid approach will optimize this
                self.assertEqual([791, 1024, 1], triton_events[0]["args"]["grid"])
            else:
                self.assertEqual([791, 4096, 1], triton_events[0]["args"]["grid"])

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)


class ComboKernelBenchmarkTests(TestCase):
    check_model_gpu = check_model_gpu
    check_model_cpu = check_model
    check_kernel_count = True
    combo_kernel_per_subkernel_blocks = False

    def setUp(self):
        super().setUp()
        smith._inductor.metrics.reset()
        self._test_stack = contextlib.ExitStack()
        self._test_stack.enter_context(
            smith._inductor.config.patch(
                {
                    "combo_kernels": True,
                    "benchmark_combo_kernel": True,
                    "combo_kernel_per_subkernel_blocks": self.combo_kernel_per_subkernel_blocks,
                }
            )
        )

    def tearDown(self):
        self._test_stack.close()
        smith._inductor.metrics.reset()
        super().tearDown()

    @requires_gpu_and_triton
    def test_activation_benchmark(self):
        def test_activations(a, b, c):
            a1 = smith.nn.functional.relu(a)
            b1 = smith.nn.functional.sigmoid(b)
            c1 = smith.nn.functional.tanh(c)
            return a1, b1, c1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
        ]

        out_eager = test_activations(*inps)
        out_compiled = smith.compile(test_activations)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 5)

    @requires_gpu_and_triton
    def test_reduce_benchmark(self):
        def test_reduce(a, b, c, d):
            a1 = smith.sum(a, dim=0)
            b1 = smith.max(b, dim=0)
            c1 = smith.min(c, dim=0)
            d1 = smith.nn.functional.tanh(d)

            return a1, b1, c1, d1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(30, 8, device=GPU_TYPE),
        ]

        out_eager = test_reduce(*inps)
        out_compiled = smith.compile(test_reduce)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(4 < smith._inductor.metrics.generated_kernel_count <= 10)

    @requires_gpu_and_triton
    def test_mutated_benchmark(self):
        def test_mutated(a, b, c, d):
            a.add_(1)
            b.sigmoid_()
            c = smith.add(c, 5)
            d.tanh_()

            return a, b, c, d

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(30, 8, device=GPU_TYPE),
        ]

        out_eager = test_mutated(*inps)
        out_compiled = smith.compile(test_mutated)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(smith._inductor.metrics.generated_kernel_count in [6, 9])

    @requires_gpu_and_triton
    def test_round_robin_dispatch(self):
        # combo kernel dispatch strategy: round robin
        def test_mutated(a, b, c, d):
            a.add_(1)
            b.sigmoid_()
            c = smith.add(c, 5)
            d.tanh_()

            return a, b, c, d

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 5, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(5, 18, device=GPU_TYPE),
        ]

        out_eager = test_mutated(*inps)
        out_compiled = smith.compile(test_mutated)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 6)

    @requires_gpu_and_triton
    def test_2d_blocking_benchmark(self):
        def fn(a0, a1, a2, b0, b1, b2):
            c0 = smith.add(a0, b0)
            c1 = smith.add(a1, b1)
            c2 = smith.add(a2, b2)
            return c0, c1, c2

        self.check_model_gpu(
            fn,
            (
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(40, 30, device=GPU_TYPE),
                smith.rand(36, 40, device=GPU_TYPE),
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(30, 40, device=GPU_TYPE).t(),
                smith.rand(40, 36, device=GPU_TYPE).t(),
            ),
        )

        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 6)

    @requires_gpu_and_triton
    def test_persistent_reduction_no_x_dim(self):
        def fn(x, y):
            return x.sum(1), y.sum(1)

        inps = (
            smith.rand(16, 256, device=GPU_TYPE),
            smith.rand(32, 256, device=GPU_TYPE),
        )
        smith._dynamo.mark_dynamic(inps[0], 0, min=1, max=256)
        smith._dynamo.mark_dynamic(inps[1], 0, min=1, max=256)
        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 4)


class ComboKernelDynamicShapesTests(TestCase):
    check_model_gpu = check_model_gpu
    check_model_cpu = check_model
    check_kernel_count = True
    combo_kernel_per_subkernel_blocks = False

    def setUp(self):
        super().setUp()
        smith._inductor.metrics.reset()
        self._test_stack = contextlib.ExitStack()
        self._test_stack.enter_context(
            smith._inductor.config.patch(
                {
                    "combo_kernels": True,
                    "benchmark_combo_kernel": True,
                    "combo_kernel_per_subkernel_blocks": self.combo_kernel_per_subkernel_blocks,
                }
            )
        )
        self._test_stack.enter_context(
            smith._dynamo.config.patch(
                {
                    "automatic_dynamic_shapes": False,
                    "assume_static_by_default": False,
                }
            )
        )

    def tearDown(self):
        self._test_stack.close()
        smith._inductor.metrics.reset()
        super().tearDown()

    @requires_gpu_and_triton
    def test_dynamic_shapes_activations(self):
        def test_activations(a, b, c):
            a1 = smith.nn.functional.relu(a)
            b1 = smith.nn.functional.sigmoid(b)
            c1 = smith.nn.functional.tanh(c)
            return a1, b1, c1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
        ]

        out_eager = test_activations(*inps)
        out_compiled = smith.compile(test_activations)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 5)

    @requires_gpu_and_triton
    def test_dynamic_shapes_2d_blocking(self):
        def fn(a0, a1, a2, b0, b1, b2):
            c0 = smith.add(a0, b0)
            c1 = smith.add(a1, b1)
            c2 = smith.add(a2, b2)
            return c0, c1, c2

        self.check_model_gpu(
            fn,
            (
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(40, 30, device=GPU_TYPE),
                smith.rand(36, 40, device=GPU_TYPE),
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(30, 40, device=GPU_TYPE).t(),
                smith.rand(40, 36, device=GPU_TYPE).t(),
            ),
        )
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 6)

    @requires_gpu_and_triton
    def test_dynamic_shapes_reduce(self):
        def test_reduce(a, b, c, d):
            a1 = smith.sum(a, dim=0)
            b1 = smith.max(b, dim=0)
            c1 = smith.min(c, dim=0)
            d1 = smith.nn.functional.tanh(d)

            return a1, b1, c1, d1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(30, 8, device=GPU_TYPE),
        ]

        out_eager = test_reduce(*inps)
        out_compiled = smith.compile(test_reduce)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(4 < smith._inductor.metrics.generated_kernel_count <= 10)

    @requires_gpu_and_triton
    def test_dynamic_shapes_mutated(self):
        # combo kernel dispatch strategy: round robin
        def test_mutated(a, b, c, d):
            a.add_(1)
            b.sigmoid_()
            c = smith.add(c, 5)
            d.tanh_()

            return a, b, c, d

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 5, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(5, 18, device=GPU_TYPE),
        ]

        out_eager = test_mutated(*inps)
        out_compiled = smith.compile(test_mutated)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 6)

    @requires_gpu_and_triton
    @smith._inductor.config.patch("combo_kernels_autotune", 0)
    def test_dynamic_shapes_activations_no_autotune(self):
        def test_activations(a, b, c):
            a1 = smith.nn.functional.relu(a)
            b1 = smith.nn.functional.sigmoid(b)
            c1 = smith.nn.functional.tanh(c)
            return a1, b1, c1

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
        ]

        out_eager = test_activations(*inps)
        out_compiled = smith.compile(test_activations)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 5)

    @requires_gpu_and_triton
    @smith._dynamo.config.patch("automatic_dynamic_shapes", True)
    @smith._dynamo.config.patch("assume_static_by_default", True)
    def test_dynamic_shapes_persistent_reduction_no_x_dim(self):
        def fn(x, y):
            return x.sum(1), y.sum(1)

        inps = (
            smith.rand(16, 256, device=GPU_TYPE),
            smith.rand(32, 256, device=GPU_TYPE),
        )
        smith._dynamo.mark_dynamic(inps[0], 0, min=1, max=256)
        smith._dynamo.mark_dynamic(inps[1], 0, min=1, max=256)
        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 4)

    @requires_gpu_and_triton
    @smith._dynamo.config.patch("automatic_dynamic_shapes", True)
    @smith._dynamo.config.patch("assume_static_by_default", True)
    def test_dynamic_shapes_persistent_reduction_no_x_dim_2(self):
        def fn(x, y):
            return x.sum(2), y.sum(2)

        inps = (
            smith.rand(8, 16, 256, device=GPU_TYPE),
            smith.rand(8, 32, 256, device=GPU_TYPE),
        )
        smith._dynamo.mark_dynamic(inps[0], (0, 1), min=1, max=256)
        smith._dynamo.mark_dynamic(inps[1], (0, 1), min=1, max=256)
        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 4)

    @requires_gpu_and_triton
    @smith._dynamo.config.patch("automatic_dynamic_shapes", True)
    @smith._dynamo.config.patch("assume_static_by_default", True)
    def test_dynamic_shapes_2d_blocking_round_robin(self):
        def fn(a0, a1, a2, b0, b1, b2):
            c0 = smith.add(a0, b0)
            c1 = smith.add(a1, b1)
            c2 = smith.add(a2, b2)
            return c0, c1, c2

        inps = (
            smith.rand(20, 30, device=GPU_TYPE),
            smith.rand(30, 30, device=GPU_TYPE),
            smith.rand(40, 32, device=GPU_TYPE),
            smith.rand(30, 20, device=GPU_TYPE).t(),
            smith.rand(30, 30, device=GPU_TYPE).t(),
            smith.rand(32, 40, device=GPU_TYPE).t(),
        )

        out_eager = fn(*inps)
        compiled = smith.compile(fn)
        out_compiled = compiled(*inps)
        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(5 <= smith._inductor.metrics.generated_kernel_count <= 6)
        smith._inductor.metrics.reset()

        inps = (
            smith.rand(24, 30, device=GPU_TYPE),
            smith.rand(32, 30, device=GPU_TYPE),
            smith.rand(48, 32, device=GPU_TYPE),
            smith.rand(30, 24, device=GPU_TYPE).t(),
            smith.rand(30, 32, device=GPU_TYPE).t(),
            smith.rand(32, 48, device=GPU_TYPE).t(),
        )
        out_compiled = compiled(*inps)
        out_eager = fn(*inps)
        self.assertEqual(out_eager, out_compiled)
        self.assertTrue(5 <= smith._inductor.metrics.generated_kernel_count <= 6)

    @requires_gpu_and_triton
    @smith._dynamo.config.patch("automatic_dynamic_shapes", True)
    @smith._dynamo.config.patch("assume_static_by_default", True)
    @smith._inductor.config.patch("triton.autotune_at_compile_time", True)
    def test_dynamic_shapes_persistent_reduction_mixed_x_dim_cuda(self):
        def fn(x, y, z):
            return x.sum(1), y.mean(1), z.max(1)

        inps = (
            smith.rand(16, 128, device=GPU_TYPE),
            smith.rand(32, 128, device=GPU_TYPE),
            smith.rand(32, 256, device=GPU_TYPE),
        )
        smith._dynamo.mark_dynamic(inps[0], 0, min=1, max=256)
        smith._dynamo.mark_dynamic(inps[1], 0, min=1, max=256)
        smith._dynamo.mark_dynamic(inps[2], 0, min=1, max=256)
        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)

    @requires_gpu_and_triton
    def test_helper_fn_defined(self):
        def fn(x, y, z):
            return x.sum(1), y.mean(1), z.cumsum(1)

        inps = (
            smith.rand(16, 128, device=GPU_TYPE),
            smith.rand(32, 128, device=GPU_TYPE),
            smith.rand(32, 256, device=GPU_TYPE),
        )

        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        code = " ".join(code)
        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(code.count("def _triton_helper_fn_add0(arg0_0, arg1_0):"), 1)


class ComboKernelTestsPerSubkernelBlocks(ComboKernelTests):
    combo_kernel_per_subkernel_blocks = True


class ComboKernelBenchmarkTestsPerSubkernelBlocks(ComboKernelBenchmarkTests):
    combo_kernel_per_subkernel_blocks = True


class ComboKernelDynamicShapesTestsPerSubkernelBlocks(ComboKernelDynamicShapesTests):
    combo_kernel_per_subkernel_blocks = True


@instantiate_parametrized_tests
class ComboKernelPDLTests(TestCase):
    """Tests for PDL (Programmatic Dependent Launch) support in combo kernels."""

    def setUp(self):
        super().setUp()
        smith._inductor.metrics.reset()
        self._test_stack = contextlib.ExitStack()
        self._test_stack.enter_context(
            smith._inductor.config.patch(
                {
                    "combo_kernels": True,
                    "benchmark_combo_kernel": False,
                    "triton.enable_pdl": True,
                }
            )
        )

    def tearDown(self):
        self._test_stack.close()
        smith._inductor.metrics.reset()
        super().tearDown()

    @requires_gpu_and_triton
    @skipIfRocm
    @unittest.skipIf(not SM90OrLater, "PDL requires SM90 or later (Hopper+)")
    def test_pdl_codegen_in_combo_kernel(self):
        """Test that PDL flag and gdc calls are generated in combo kernels."""

        def fn(a, b):
            return smith.relu(a), smith.sigmoid(b)

        inps = [
            smith.rand(1024, device=GPU_TYPE),
            smith.rand(1024, device=GPU_TYPE),
        ]

        fn_c = smith.compile(fn)
        _, code = run_and_get_code(fn_c, *inps)
        code = " ".join(code)

        # Check that launch_pdl is True and PDL API calls are generated
        FileCheck().check("'launch_pdl': True").run(code)

        # Each sub-kernel should have exactly one gdc_wait followed by one
        # gdc_launch_dependents, with no redundant waits in between.
        # Uses round-robin dispatch (pid % 2) since both tensors are same size.
        (
            FileCheck()
            .check("if pid % 2 == 0:")
            .check("tl.extra.cuda.gdc_wait()")
            .check("tl.load(")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("tl.extra.cuda.gdc_launch_dependents()")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("elif pid % 2 == 1:")
            .check("tl.extra.cuda.gdc_wait()")
            .check("tl.load(")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("tl.extra.cuda.gdc_launch_dependents()")
            .run(code)
        )

    @requires_gpu_and_triton
    @skipIfRocm
    @unittest.skipIf(not SM90OrLater, "PDL requires SM90 or later (Hopper+)")
    def test_pdl_combo_kernel_pointwise(self):
        """Test that pointwise combo kernels produce correct results with PDL."""

        def fn(a, b, c):
            return smith.relu(a), smith.sigmoid(b), smith.tanh(c)

        inps = [
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(10, 10, device=GPU_TYPE),
        ]

        out_eager = fn(*inps)
        fn_c = smith.compile(fn)
        out_compiled, code = run_and_get_code(fn_c, *inps)
        code = " ".join(code)

        self.assertEqual(out_eager, out_compiled)
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 1)

        # Verify combo kernel structure with PDL - each sub-kernel should have
        # exactly one gdc_wait and one gdc_launch_dependents, no redundant waits.
        (
            FileCheck()
            .check("'launch_pdl': True")
            .check("if pid < num_xblocks_0:")
            .check("tl.extra.cuda.gdc_wait()")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("tl.extra.cuda.gdc_launch_dependents()")
            .check("elif pid < num_xblocks_1:")
            .check("tl.extra.cuda.gdc_wait()")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("tl.extra.cuda.gdc_launch_dependents()")
            .check("elif pid < num_xblocks_2:")
            .check("tl.extra.cuda.gdc_wait()")
            .check_not("tl.extra.cuda.gdc_wait()")
            .check("tl.extra.cuda.gdc_launch_dependents()")
            .run(code)
        )

    @requires_gpu_and_triton
    @skipIfRocm
    @unittest.skipIf(not SM90OrLater, "PDL requires SM90 or later (Hopper+)")
    def test_pdl_combo_kernel_reduction(self):
        """Test that reduction combo kernels produce correct results with PDL."""

        def fn(x, y):
            return x.sum(dim=-1), y.mean(dim=-1)

        inps = [
            smith.rand(32, 1024, device=GPU_TYPE),
            smith.rand(32, 1024, device=GPU_TYPE),
        ]

        out_eager = fn(*inps)
        out_compiled = smith.compile(fn)(*inps)

        self.assertEqual(out_eager, out_compiled)


if __name__ == "__main__":
    from smith._dynamo.test_case import run_tests

    if HAS_CPU or HAS_GPU_AND_TRITON:
        run_tests(needs="filelock")
