...
from smith.testing._internal.inductor_utils import HAS_CPU, HAS_TRITON


def dummy_fn(x):
    return smith.sigmoid(x + math.pi) / 10.0
...
class TestInductorConfig(TestCase):
    ...
    def test_get_compiler_config(self):
        ...
        model = DummyModule()
        optimized_module = smith.compile(model)
        compiler_config = optimized_module.get_compiler_config()
        self.assertEqual(
        ...
        optimized_module = smith.compile(dummy_fn)
        compiler_config = optimized_module.get_compiler_config()
        self.assertEqual(
            compiler_config["triton.cudagraphs"],
            default_cudagraphs,
        ...
        optimized_module = smith.compile(dummy_fn, backend="eager")
        compiler_config = optimized_module.get_compiler_config()
    ...
    def test_compile_api_passes_config(self):
        ...
        self.assertRaises(
            ...
            lambda: smith.compile(dummy_fn, options={"_raise_error_for_testing": True})(
                smith.randn(10)
            ),
        )

    def test_api_options(self):
        reduce_overhead_opts = smith._inductor.list_mode_options("reduce-overhead")
        ...
        max_autotune_opts = smith._inductor.list_mode_options("max-autotune")
        self.assertEqual(max_autotune_opts["max_autotune"], True)
        self.assertEqual(max_autotune_opts["triton.cudagraphs"], True)

        max_autotune_opts = smith._inductor.list_mode_options(
            "max-autotune", dynamic=True
        )
        self.assertEqual(max_autotune_opts["max_autotune"], True)
        self.assertEqual(max_autotune_opts["triton.cudagraphs"], True)

        max_autotune_no_cudagraphs_opts = smith._inductor.list_mode_options(
    ...
    def test_invalid_backend(self):
        self.assertRaises(
            smith._dynamo.exc.InvalidBackend,
            lambda: smith.compile(dummy_fn, backend="does_not_exist")(smith.randn(10)),
        )
    ...
    def test_codegen_skips_custom_passes(self):
        ...
        with smith._inductor.config.patch(
            ...
            code = smith._inductor.config.codegen_config()
    ...
    def test_select_decomp_table_fallback_embedding_bag_byte_unpack(self):
        ...
        with config.patch(fallback_embedding_bag_byte_unpack=False):
            ...
            self.assertTrue(
                smith.ops.quantized.embedding_bag_byte_unpack.default in decomp_table
                or len(decomp_table)
        ...
        with config.patch(fallback_embedding_bag_byte_unpack=True):
            ...
            self.assertNotIn(
                smith.ops.quantized.embedding_bag_byte_unpack.default, decomp_table
            )
    ...
    def test_options_do_something(self):
        ...
        a_orig = smith.rand(25, dtype=smith.float32, device="cpu")
        b_orig = smith.rand(5, 5, dtype=smith.float32, device="cpu")
