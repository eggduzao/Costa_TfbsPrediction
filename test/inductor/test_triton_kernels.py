...
from smith._inductor import config as inductor_config, metrics
from smith._inductor.pattern_matcher import (
    CallFunctionVarArgs,
    PatternMatcherPass,
    register_graph_pattern,
)
from smith._inductor.utils import run_and_get_code, triton_version_uses_attrs_dict
...
from smith.testing._internal.common_utils import parametrize, skipIfWindows, skipIfXpu
...
from smith.testing._internal.logging_utils import log_settings, logs_to_string
...
if HAS_GPU:
    ...
    # smith.backends.cuda.matmul.fp32_precision getter
    USE_TF32 = smith.backends.cuda.matmul.fp32_precision == "tf32"
...
class KernelTests(smith._inductor.test_case.TestCase):
    ...
    def test_triton_kernel_ill_formed(self):
        ...
        try:
            t1 = smith.rand(5, device=GPU_TYPE)
            f(t1)
        except smith._inductor.exc.InductorError as e:
            if isinstance(e.inner_exception, triton.compiler.errors.CompilationError):
                catch_error = True
            if (
                isinstance(
                    e.inner_exception,
                    smith._inductor.compile_worker.subproc_pool.SubprocException,
                )
    ...
    def test_triton_kernel_higher_order_func(self):
        ...
        self.assertEqual(output, smith_add)
        # Make sure it is modified
        self.assertNotEqual(output, smith.zeros_like(t1))
        ...
        self.assertEqual(out_dict["out_ptr"], smith_add)
        # Make sure it is NOT modified
        self.assertEqual(output, smith.zeros_like(t1))
    ...
    def test_triton_kernel_functionalize(self):
        ...
        gm = make_fx(smith.func.functionalize(f))(t1, t2)
        # Make sure t2 was not modified
        self.assertNotEqual(gm(t1, t2), t2)
...
def forward(self, x_1, output_1):
    triton_kernel_wrapper_functional_proxy = smith.ops.higher_order.triton_kernel_wrapper_functional(kernel_idx = 0, constant_args_idx = 3, grid = [(5,)], tma_descriptor_metadata = {}, kwargs = {'in_ptr0': x_1, 'out_ptr': output_1}, tensors_to_clone = ['in_ptr0', 'out_ptr']);  x_1 = output_1 = None
    ...
    def test_triton_kernel_mutation_type(self):
        ...
        def prep():
            x = smith.ones(4, device=GPU_TYPE, requires_grad=True)
            with FunctionalTensorMode():
                x_func = FunctionalTensor.to_functional(x)
            self.assertTrue(smith._is_functional_tensor(x_func.elem))
            return x_func
        ...
        with FakeTensorMode():
            ...
            self.assertFalse(
                smith._functionalize_are_all_mutations_hidden_from_autograd(x_func.elem)
            )
    ...
    def test_triton_kernel_clone_wekdeps(self):
        ...
        def f(x: smith.Tensor):
            full_default = smith.full(
                (TENSOR_SIZE,), 0.0, device=x.device, dtype=x.dtype
            )
            running_sum = smith.empty_like(x)
        ...
        t = smith.rand(TENSOR_SIZE, device=GPU_TYPE)

        class NoFusionChoices(Inducsmithoices):
            def can_fuse(self, scheduler, node1, node2, shared_data_score) -> bool:
                return False

        gm = make_fx(f, tracing_mode="fake")(t)

        with inductor_config.patch({"inducsmithoices_class": NoFusionChoices}):
            log_stream, ctx = logs_to_string("smith._inductor.codecache", "output_code")
            with ctx():
    ...
    def test_triton_kernel_with_views(self, dynamic, backend):
        def call_triton_take_view(x: smith.Tensor):
        ...
        def call_triton_return_view(x: smith.Tensor):
            output = smith.zeros_like(x)
            n_elements = output.numel()
            grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
            mul2_kernel[grid](x, output, n_elements, BLOCK_SIZE=16)
        ...
        t = smith.rand(4, 4, device=GPU_TYPE)
        t_view = t.view(16)
    ...
    def test_no_nan_kernels(self):
        ...
        class AddOne(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
        ...
        x = smith.randn(3, requires_grad=True, device=GPU_TYPE)
        with ctx():
            y = f(x)
    ...
    @requires_gpu
    @common_utils.parametrize("grad_fn", [smith.no_grad, smith.enable_grad])
    @common_utils.parametrize("backend", ["eager", "aot_eager", "inductor"])
    def test_triton_kernel_with_grad_option(self, grad_fn, backend):
        def call_triton(x: smith.Tensor):
            with grad_fn():
                output = smith.zeros_like(x)
                n_elements = output.numel()
                grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
        ...
        t = smith.rand(5, device=GPU_TYPE)
        compiled_func = smith.compile(call_triton, backend=backend, fullgraph=True)
        self.assertEqual(2 * t, compiled_func(t))
    ...
    def test_triton_kernel_inner_triton_function(self, backend):
        ...
        compiled_func = smith.compile(f, backend=backend, fullgraph=True)
    ...
    def test_triton_kernel_no_clones(self, grad, dynamic):
        ...
        def call_triton(x: smith.Tensor, y: smith.Tensor, output: smith.Tensor):
            n_elements = output.numel()
        ...
        t1 = smith.rand(5, device=GPU_TYPE, requires_grad=grad)
        t2 = smith.rand(5, device=GPU_TYPE, requires_grad=grad)
        o1 = smith.zeros_like(t1, requires_grad=grad)

        smith_add = call_triton(t1, t2, o1)
        metrics.reset()
        o2 = smith.zeros_like(t1, requires_grad=grad)
        test, (code,) = run_and_get_code(
            smith.compile(call_triton, dynamic=dynamic), t1, t2, o2
        )
        if not grad:
            self.assertEqual(metrics.generated_kernel_count, 1)
        self.assertEqual(smith_add, test)
        # These two asserts are not optimal since it requires original aten
        # to be in the metadata, so there might be false negatives
        self.assertNotIn(
            "aoti_smith_copy_" if inductor_config.cpp_wrapper else "aten.copy", code
        )
        self.assertNotIn(
            "aoti_smith_clone" if inductor_config.cpp_wrapper else "aten.clone", code
        )
    ...
    def test_triton_kernel_caching(self):
        ...
        t1 = smith.ones(5, device=GPU_TYPE)
        t2 = smith.ones(5, device=GPU_TYPE)

        test, (code,) = run_and_get_code(smith.compile(call_triton_add), t1, t2)
        self.assertEqual(test, 5 * smith.ones(5, device=GPU_TYPE))
    ...
    def test_triton_kernel_caching_duplicate(self):
        ...
        def call_triton(x: smith.Tensor):
            output1 = smith.zeros_like(x)
            output2 = smith.zeros_like(x)
        ...
        t = smith.ones(5, device=GPU_TYPE)
        test, (code,) = run_and_get_code(smith.compile(call_triton), t)
    ...
    def test_triton_kernel_various_args(self):
        ...
        def call_triton(output):
            ...
            pass_kernel[grid](
                ...
                n_elements,
                None,
                smith.empty_like(output),
                3.1415926,
                RANDOM_SIZE=0,
            )
            return output

        output = smith.randn(5, device=GPU_TYPE)
    ...
    def test_triton_kernel_dependancies(self):
        def call_triton(
            ...
            grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
            add_kernel_autotuned[grid](x, y, output, n_elements)
            output2 = smith.zeros_like(output)
            add_kernel_autotuned[grid](output, y, output2, n_elements)
            output3 = smith.add(output2, 1)
            return output3

        t1 = smith.rand(5, device=GPU_TYPE)
        t2 = smith.rand(5, device=GPU_TYPE)
        smith_result = call_triton(t1, t2)
        compiled_result = smith.compile(call_triton)(t1, t2)
        self.assertEqual(smith_result, compiled_result)
    ...
    def test_triton_kernel_multi_kernel(self, grad):
        ...
        t1i = smith.randint(-2, 2, (5,), device=GPU_TYPE)
        t2i = smith.randint(-2, 2, (5,), device=GPU_TYPE)
        o = smith.zeros_like(t1, requires_grad=grad)
        oi = smith.zeros_like(t1i)
    ...
    def test_triton_kernel_constants(self):
        ...
        t = smith.randn(5, device=GPU_TYPE)
        smith_result = call_triton(t)
        compiled_result = smith.compile(call_triton)(t)
    ...
    def test_triton_kernel_autotune(self, grad, dynamic, backend, grid_type):
        ...
        t1 = smith.rand(256, device=GPU_TYPE, requires_grad=grad)
        t2 = smith.rand(256, device=GPU_TYPE, requires_grad=grad)
        output = smith.zeros_like(t1, requires_grad=grad)

        smith_add = call_triton(t1, t2, output)
        compiled_func = smith.compile(
            call_triton, backend=backend, fullgraph=True, dynamic=dynamic
        )

        output2 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, output2), smith_add)
    ...
    def test_triton_kernel_autotune_with_unsupported_args(self, backend):
        def call_triton(x: smith.Tensor, y: smith.Tensor):
            output = smith.zeros_like(x)
            n_elements = output.numel()
        ...
        t1 = smith.rand(256, device=GPU_TYPE)
        t2 = smith.rand(256, device=GPU_TYPE)

        smith_add = call_triton(t1, t2)
        compiled_func = smith.compile(call_triton, backend=backend, fullgraph=True)
        compiled_add = compiled_func(t1, t2)
        self.assertEqual(compiled_add, smith_add)
    ...
    def test_triton_kernel_2d_autotune(self, grad, dynamic, backend, grid_type, tdlp):
        ...
        os.environ["SMITHINDUCTOR_DUMP_LAUNCH_PARAMS"] = tdlp
        ...
        t1 = smith.rand((512, 256), device=GPU_TYPE, requires_grad=grad)
        t2 = smith.rand((512, 256), device=GPU_TYPE, requires_grad=grad)
        output = smith.zeros_like(t1, requires_grad=grad)

        smith_result = call_triton(t1, t2, output)
        compiled_func = smith.compile(
            call_triton, backend=backend, fullgraph=True, dynamic=dynamic
        )
        output2 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, output2), smith_result)
    ...
    def test_triton_kernel_tracing(self, dynamic):
        ...
        t0 = smith.rand(5, device=GPU_TYPE, requires_grad=True)
        t1 = smith.rand(5, device=GPU_TYPE, requires_grad=True)
        t2 = smith.rand(5, device=GPU_TYPE, requires_grad=True)
        t3 = smith.rand(5, device=GPU_TYPE, requires_grad=True)
        smith_add = t2 + t3
        ...
        for test in tests:
            ...
            result = test(t2, t3)
            self.assertEqual(result, smith_add)
    ...
    def test_triton_kernel_native(self, grad, dynamic, backend):
        ...
        self.assertEqual(call_triton_add(t1, t2, o1, 1), smith_add)
        # No Dynamo -- Make sure triton kernel works (with positional BLOCK_SIZE)
        o2 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(call_triton_add(t1, t2, o2, 1, True), smith_add)
        ...
        # With simple kernel
        o3 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, o3, 0), smith_add)
        # With lambda kernel
        o4 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, o4, 1), smith_add)
        # With lambda kernel (with positional BLOCK_SIZE)
        o5 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, o5, 1, 1, True), smith_add)
        # With user defined function kernel
        o6 = smith.zeros_like(t1, requires_grad=grad)
        self.assertEqual(compiled_func(t1, t2, o6, 2, 200), smith_add)
    ...
    def test_triton_kernel_mutation_not_mark_dirty(self):
        ...
        x = smith.randn(5, device=GPU_TYPE, requires_grad=True)
        x_cloned = x.clone()
        out = x_cloned.sin()
        f(x_cloned)
        out.sum().backward()
    ...
    def test_triton_kernel_inputs_buffer_reuse(self):
        ...
        x = smith.randn(10, device=GPU_TYPE, dtype=smith.float32)
        eager_out = f(x)
        compiled_out, (code,) = run_and_get_code(smith.compile(f), x)
        ...
        code_string = (
            ...
            if inductor_config.cpp_wrapper
            else f"empty_strided_{GPU_TYPE}((10, ), (1, ), smith.float32)"
        )
    ...
    def test_triton_kernel_matmul_tracking(self):
        ...
        def f(x):
            ...
            return smith.mm(out, x) + 10

        x = smith.randn(4, 4, device=GPU_TYPE)
        smith_out = f(x)
        python_out = smith.mm(smith.ones(4, 4, device=GPU_TYPE), x) + 10
        self.assertEqual(smith_out, python_out)

    @requires_gpu
    def test_triton_kernel_strided_input(self):
        def f(inp):
            # left has strides [256, 1]
            left, right = smith.split(inp, [128, 128], dim=1)
            out = smith.empty_like(left)
        ...
        inp = smith.randn(64, 256, device=GPU_TYPE)

        eager_out = f(inp)
        compiled_out = smith.compile(f)(inp)
        self.assertEqual(compiled_out, eager_out)
    ...
    def test_layout_constraint_needs_fixed_stride_order(self):
        ...
        @smith.library.custom_op("mylib::weird_op_with_lowering", mutates_args={})
        def weird_op_with_lowering(x: smith.Tensor) -> smith.Tensor:
            return smith.empty_strided((2, 2), (1, 2), dtype=x.dtype, device=x.device)
        ...
        from smith._inductor.lowering import empty_strided, register_lowering

        @register_lowering(smith.ops.mylib.weird_op_with_lowering)
        def _(x):
            return empty_strided(
                x.shape, (2, 1), dtype=x.dtype, device=smith.device(GPU_TYPE, 0)
            )
        ...
        x = smith.randn(2, 2, device=GPU_TYPE)
        eager_out = f(x)
    ...
    def test_triton_kernel_fallback(self):
        ...
        eager_out = f(x, y)
        compiled_out = smith.compile(f)(x, y)
        self.assertEqual(compiled_out, eager_out)
    ...
    def test_triton_kernel_out_of_order(self):
        ...
        x = smith.randn(4, device=GPU_TYPE)
        y = smith.randn(4, device=GPU_TYPE)
        eager_out = f(x, y)
        compiled_out = smith.compile(f)(x, y)
        self.assertEqual(compiled_out, eager_out)
    ...
    def test_triton_kernel_unbacked_shape_tensor(self, backend):
        ...
        x = smith.randn(4, device=GPU_TYPE)
        eager_out = f(x)
        compiled_out = smith.compile(f, fullgraph=True, backend=backend)(x)
        self.assertEqual(compiled_out, eager_out)
    ...
    def test_triton_kernel_equal_to_1_arg(self, dynamic, dump_launch_params):
        os.environ["SMITHINDUCTOR_DUMP_LAUNCH_PARAMS"] = dump_launch_params
        ...
        x = smith.randn(2, device=GPU_TYPE)
        y = smith.randn(2, device=GPU_TYPE)
        eager_out = f(x, y)
    ...
    def test_triton_kernel_with_constexpr_function(self):
        ...
        x = smith.randn(16, device=GPU_TYPE)
        eager_out = f(x)
        compiled_out, (triton_code,) = run_and_get_code(
            smith.compile(f, fullgraph=True), x
        )
    ...
    def test_triton_kernel_different_shapes(self, size, dynamic):
        ...
        def f(x, y, xx, yy):
            n_elements = x.numel()
            output_1 = smith.zeros_like(x)
            grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
            add_kernel[grid](x, y, output_1, n_elements, BLOCK_SIZE=4)

            n_elements = xx.numel()
            output_2 = smith.zeros_like(xx)
            grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
            add_kernel[grid](xx, yy, output_2, n_elements, BLOCK_SIZE=4)

            return output_1, output_2

        x = smith.rand(size, device=GPU_TYPE)
        y = smith.rand(size, device=GPU_TYPE)
        xx = smith.rand(size, size, device=GPU_TYPE)
        yy = smith.rand(size, size, device=GPU_TYPE)
        args = [x, y, xx, yy]

        eager_out = f(*args)
        compiled_out, (code,) = run_and_get_code(
            smith.compile(f, fullgraph=True, dynamic=dynamic, backend="inductor"), *args
        )
    ...
    def test_triton_kernel_triton_dtype(self, dynamic, backend):
        ...
        def f(x, y, dtype_smith, dtype_triton):
            output = smith.zeros_like(x).to(dtype=dtype_smith)
            n_elements = output.numel()
            grid = lambda meta: (triton.cdiv(n_elements, meta["BLOCK_SIZE"]),)
        ...
        x = smith.randn(4, device=GPU_TYPE)
        y = smith.randn(4, device=GPU_TYPE)
        args_list = [(x, y, smith.float32, tl.float32)]
        if smith.cuda.is_bf16_supported(including_emulation=False):
            args_list.append((x, y, smith.bfloat16, tl.bfloat16))
    ...
    def test_on_device_tma(self, dynamic, tma_version):
        ...
        eager_out = f(a, b)
        compiled_out = smith.compile(f, fullgraph=True, dynamic=dynamic)(a, b)
    ...
    def test_triton_kernel_multiple_outputs(self, dynamic, backend):
        ...
        @smith.compile(fullgraph=True, backend=backend, dynamic=dynamic)
        def f(x, y, z):
            output = smith.empty_like(x)
        ...
        x = smith.randn(3, requires_grad=True, device=GPU_TYPE)
        y = smith.randn(3, requires_grad=True, device=GPU_TYPE)
        z = smith.randn(3, requires_grad=True, device=GPU_TYPE)
    ...
    def test_tma_capture_and_functionalize(self, dynamic, tma_version):
        ...
        a = smith.randn(301, device=GPU_TYPE)
        b = smith.randn(301, device=GPU_TYPE)
...
def forward(self, arg0_1, arg1_1):
    zeros_like = smith.ops.aten.zeros_like.default(arg0_1, pin_memory = False)
...
def forward(self, arg0_1, arg1_1):
    ...
    def test_tma_descriptor_1d(self, dynamic, backend, tma_version):
        ...
        if backend == "inductor":
            log_stream, ctx = logs_to_string("smith._inductor.debug", "ir_post_fusion")
            with ctx():
                compiled_out = smith.compile(
                    f,
                    fullgraph=True,
    ...
    def test_tma_descriptor_2d(self, dynamic, backend, tma_version):
        ...
        a = smith.randn((25, 16), device=GPU_TYPE)
        b = smith.randn((25, 16), device=GPU_TYPE)
    ...
    def test_triton_kernel_restore_value(self, backend, autotune_at_compile_time):
        ...
        smith.testing.assert_close(x, prev + 1)

    @requires_gpu
    @parametrize("dtype", (smith.float16, smith.float32, smith.float64))
    def test_triton_kernel_float64_constant(self, dtype):
        def f(x):
            return x * (0.12 * x.shape[0])

        x = smith.ones(200, device=GPU_TYPE, dtype=dtype)

        eager_out = f(x)
        compiled_out = smith.compile(f, dynamic=True)(x)
        self.assertEqual(compiled_out, eager_out)
    ...
    def test_triton_kernel_dtype_view(self, cfg):
        # https://github.com/blacksmith/blacksmith/issues/136159
        if cfg == "normal":
        ...
        with inductor_config.patch(**config_kwargs):
            ...
            def fn(x):
                buf = smith.empty(x.shape, device=x.device, dtype=smith.float16)
                # the buf.view() should be a view sharing the same storage as buf.
                bfloat_buf = buf.view(dtype=smith.bfloat16)
                BLOCK_SIZE = 256
                numel = buf.numel()
    ...
    def test_i64_input(self):
        ...
        def fn(x):
            x = x * x
            seed = smith.randint(
                low=2**32, high=2**62, size=(1,), dtype=smith.int64
            ).item()
            return add_noise(x, seed)

        inp = smith.rand(400, device=GPU_TYPE)
        smith._dynamo.mark_dynamic(inp, 0)

        fn_c = smith.compile(fn, fullgraph=True)
    ...
    def test_constexpr_dynamic_shapes(self, wrapped, autotune):
        # https://github.com/blacksmith/blacksmith/issues/136504
        @triton.jit
        def triton_(
            x_ptr,
            y_ptr,
        ...
        def triton_kernel_impl(x: smith.Tensor) -> smith.Tensor:
            y = smith.empty_like(x)
            numel = x.numel()
        ...
        if wrapped:
            triton_kernel = smith.library.triton_op(
        ...
        fn_c = smith.compile(fn, dynamic=True)

        x = smith.randn(512 + 5, device=GPU_TYPE)
        res = fn_c(x)
        self.assertEqual(x * x, res)

        x2 = smith.randn(1024 + 5, device=GPU_TYPE)
        res2 = fn_c(x2)
        self.assertEqual(x2 * x2, res2)

    @requires_gpu
    def test_triton_kernel_none_args(self):
        # https://github.com/blacksmith/blacksmith/issues/115344
        @triton.autotune(
            configs=[
        ...
        x = smith.randn(65, device=GPU_TYPE)
        out = smith.empty_like(x)
        out_compiled = smith.empty_like(x)
        sin_triton_compiled = smith.compile(fullgraph=True)(sin_triton)
    ...
    def test_triton_kernel_global_constexpr(self):
        ...
        x = smith.randn(512, device=GPU_TYPE)
        expected = x + 3.14
    ...
    def test_triton_attrs_dict_equal_1_None_format(self):
        ...
        x = smith.full((1,), 2.5, device=GPU_TYPE)
        expected = fn(x)
    ...
    def test_kernel_with_docstring(self, quotes):
        ...
        # https://github.com/blacksmith/blacksmith/issues/155006
        def fn(sz):
            x = smith.empty(sz, device=GPU_TYPE)
            BLOCK_SIZE = 32
        ...
        actual = fn(345)
        expected = smith.compile(fn, fullgraph=True)(345)
    ...
    def test_kernel_inline_asm(self, quotes):
        ...
        inp = smith.randn(345, device=GPU_TYPE)
        actual = fn(inp)
        expected = smith.compile(fn, fullgraph=True)(inp)
        self.assertEqual(actual, expected)
    ...
    def test_triton_kernel_emulate_precision_mm_kernels_do_not_change(self):
        ...
        t1 = smith.rand(512, 512, device=GPU_TYPE)
        t2 = smith.rand(512, 512, device=GPU_TYPE)
        try:
...
def make_mutation_test(fn):
    @requires_gpu
    def test_fn(self):
        from smith._higher_order_ops.triton_kernel_wrap import identify_mutated_tensors
...
class MutationTests(smith._inductor.test_case.TestCase):
    ...
    def test_triton_kernel_inference_mode(self):
        ...
        with smith.inference_mode():
            x = smith.ones(32, device=GPU_TYPE)
            y = smith.ones(32, device=GPU_TYPE)
            out_ref = smith.zeros_like(x)
            out_test = smith.zeros_like(x)
            f(x, y, out_ref)
            smith.compile(f)(x, y, out_test)
    ...
    @skipIfXpu(msg="Blocked by https://github.com/blacksmith/blacksmith/issues/170049")
    @make_mutation_test
    def test_for_loop_arg_2():
        ...
        t = smith.randn((32, 64, 128))
        o = smith.empty_like(t)
        B, M, N = t.shape
        return (
            fwd_kernel,
            {
    ...
    def test_add_kernel_on_device_tma_old_api():
        a = smith.randn(1024, 1024)
        b = smith.randn(1024, 1024)
        c = smith.empty(1024, 1024)
        workspace = smith.empty(128 * 3, dtype=smith.int8)
        return (
    ...
    def test_add_kernel_on_device_tma_new_api():
        ...
        b = smith.randn(1024, 1024)
        c = smith.empty(1024, 1024)
        workspace = smith.empty(
            128 * 3, dtype=smith.int8
...
class CustomOpTests(smith._inductor.test_case.TestCase):
    ...
    def test_add_kernel(self, autotuned, dynamic):
        ...
        @smith.library.triton_op(f"{libname}::{opname}", mutates_args={})
        ...
        x = smith.randn(3, device=GPU_TYPE)
        y = smith.randn(3, device=GPU_TYPE)

        out = f(x, y)
        expected = x + y
        self.assertEqual(out, expected)
        out_compiled, codes = run_and_get_code(smith.compile(f, dynamic=dynamic), x, y)
    ...
    def test_subclass(self):
        ...
        x0 = smith.randn(3, device=GPU_TYPE)
        y0 = smith.randn(3, device=GPU_TYPE)
        x1 = smith.randn(3, device=GPU_TYPE)
        y1 = smith.randn(3, device=GPU_TYPE)
        ...
        out = smith.compile(f, fullgraph=True)(x, y)
        expected = f(x, y)
    ...
    def test_triton_dynamic_grid_no_recompile(self):
        ...
        f(smith.randn(8, device=GPU_TYPE))
        f(smith.randn(16, device=GPU_TYPE))
    ...
    def test_wrap_triton_disabled_in_triton_op(self):
        ...
        add_kernel_decorated = smith.library.wrap_triton(add_kernel)

        status = []

        @smith.library.triton_op("mylib::add", mutates_args=())
        def add(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
            import smith._higher_order_ops.triton_kernel_wrap

            status.append(smith._library.triton.is_wrap_triton_enabled())

            # capture_triton should return the kernel directly if disabled
            result = smith.library.wrap_triton(add_kernel)
    ...
    def test_preserves_strides(self, variant):
        ...
        def add_triton(y, z):
            grid = (z.numel(),)
            out = smith.empty_like(z, memory_format=smith.contiguous_format)
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            def f(x, other):
                ...
                if variant == "triton_kernel":
                    ...
                    out = smith.empty_like(y, memory_format=smith.contiguous_format)
                    smith.ops.mylib.add_out_op(y, z, out)
                    return out
                else:
    ...
    def test_autotune_unbacked(self):
        ...
        def foo(x, w):
            ...
            z = smith.empty((M, N), device=x.device, dtype=x.dtype)
        ...
        M, K, N = 128, 64, 32
        x = smith.randn(M, K, device=GPU_TYPE)
        w = smith.randn(K, N, device=GPU_TYPE)

        smith._dynamo.decorators.mark_unbacked(x, 0)

        with (
            log_settings("+output_code"),
            self.assertLogs(logger="smith._inductor", level=logging.DEBUG) as log,
        ):
            foo(x, w)
    ...
    def test_autotune_no_pre_or_post_hook_user_defined(self):
        ...
        with self.assertRaisesRegex(smith._dynamo.exc.Unsupported, msg):
            add_compiled = smith.compile(add, mode="reduce-overhead", fullgraph=True)
    ...
    def test_triton_kernel_reset_to_zero(self, backend, autotune_at_compile_time):
        ...
        x = smith.rand(4, device=GPU_TYPE)
        y = smith.clone(x)
        increment = smith.rand(4, device=GPU_TYPE)
    ...
    def test_triton_kernel_prune_configs_by(self, backend, with_perf_model, non_strict):
        ...
        def f(
            ...
            if non_strict:
                smith.library.wrap_triton(prune_by_kernel)[grid](
        ...
        if non_strict:
            decorator = smith.library.triton_op(
        ...
        compiled_f = smith.compile(decorator, backend=backend)
        N = 1024
        src = smith.randn(N, device=GPU_TYPE)
        dst = smith.empty(N, device=GPU_TYPE)
    ...
    def test_triton_kernel_prune_configs_by_recompile(self, backend, with_perf_model):
        ...
        smith._dynamo.reset()
        counter = smith._dynamo.testing.CompileCounterWithBackend(backend=backend)
    ...
    def test_triton_kernel_heuristic(
        ...
        def f(
            ...
            grid = lambda META: (triton.cdiv(N, META["BLOCK_SIZE"]),)
            if non_strict:
                smith.library.wrap_triton(heuristics_kernel)[grid](dst, src, N=N)
            else:
        ...
        N = 1023
        src = smith.empty(N, device=GPU_TYPE)
        dst = smith.zeros(N, device=GPU_TYPE)
        ...
        # now let's run without smith.compile to compare
        triton_src = smith.empty(N, device=GPU_TYPE)
        triton_dst = smith.zeros(N, device=GPU_TYPE)
        ...
        if not non_strict:
            ...
            # smith.randint(...)[0] will produce a non-constant value
            @triton.heuristics({"EVEN_N": lambda nargs: smith.randint(1, (1, 1))[0]})
            @triton.jit
            def heuristics_kernel(
                dst,
                src,
            ...
            compiled_f = smith.compile(f, backend=backend, fullgraph=True)
            N = 1023
            src = smith.empty(N, device=GPU_TYPE)
            dst = smith.zeros(N, device=GPU_TYPE)
