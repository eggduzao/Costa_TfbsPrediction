...
from smith._inductor.augmented_graph_helper import AugmentedGraphHelper
...
class TestAugmentedGraphHelper(TestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        # Create a smith.fx.Graph with multiple nodes
        self.graph = fx.Graph()
        ...
        self.node_a = self.graph.call_function(
            smith.add, args=(self.x, self.y), name="A"
        )
        self.node_b = self.graph.call_function(
            smith.mul, args=(self.node_a, self.x), name="B"
        )
        self.node_c = self.graph.call_function(
            smith.sub, args=(self.node_a, self.y), name="C"
        )
        self.node_d = self.graph.call_function(
            smith.div, args=(self.node_b, self.node_c), name="D"
        )
        self.node_e = self.graph.call_function(
            operator.neg, args=(self.node_d,), name="E"
        )
        self.node_f = self.graph.call_function(smith.abs, args=(self.node_e,), name="F")
        self.node_g = self.graph.call_function(
            smith.relu, args=(self.node_f,), name="G"
        )
        self.node_h = self.graph.call_function(
            smith.sigmoid, args=(self.node_g,), name="H"
        )
    ...
    def test_has_path_through_merge(self):
        ...
        a2 = graph2.call_function(smith.neg, args=(x2,), name="A2")
        b2 = graph2.call_function(smith.abs, args=(a2,), name="B2")
        c2 = graph2.call_function(smith.relu, args=(x2,), name="C2")
        d2 = graph2.call_function(smith.sigmoid, args=(c2,), name="D2")
        graph2.output(d2)
    ...
    def test_simple_cycle_detection(self):
        ...
        a3 = graph3.call_function(smith.neg, args=(x3,))
        b3 = graph3.call_function(smith.abs, args=(a3,))
        c3 = graph3.call_function(smith.relu, args=(b3,))
        graph3.output(c3)
    ...
    def test_cycle_through_merge(self):
        ...
        a4 = graph4.call_function(smith.neg, args=(x4,))
        b4 = graph4.call_function(smith.abs, args=(a4,))
        c4 = graph4.call_function(smith.relu, args=(x4,))
        d4 = graph4.call_function(smith.sigmoid, args=(c4,))
        graph4.output(d4)
    ...
    def test_transfer_with_cross_deps(self):
        ...
        old_start = graph.call_function(smith.relu, args=(x,), name="old_start")
        old_wait = graph.call_function(smith.abs, args=(x,), name="old_wait")
        compute = graph.call_function(smith.neg, args=(old_wait,), name="compute")
        ...
        # Create replacements
        new_start = graph.call_function(smith.sigmoid, args=(x,), name="new_start")
        new_wait = graph.call_function(smith.tanh, args=(x,), name="new_wait")
    ...
    def test_transfer_preserves_external_deps(self):
        ...
        graph = fx.Graph()
        x = graph.placeholder("x")
        external1 = graph.call_function(smith.relu, args=(x,), name="external1")
        old1 = graph.call_function(smith.abs, args=(x,), name="old1")
        old2 = graph.call_function(smith.neg, args=(x,), name="old2")
        external2 = graph.call_function(smith.sigmoid, args=(x,), name="external2")
        ...
        # Create new nodes
        new1 = graph.call_function(smith.tanh, args=(x,), name="new1")
        new2 = graph.call_function(smith.exp, args=(x,), name="new2")
    ...
    def test_transfer_with_merge_sets(self):
        ...
        graph = fx.Graph()
        x = graph.placeholder("x")
        old_a = graph.call_function(smith.relu, args=(x,), name="old_a")
        old_b = graph.call_function(smith.abs, args=(x,), name="old_b")
        dep = graph.call_function(smith.neg, args=(x,), name="dep")
        user = graph.call_function(smith.sigmoid, args=(x,), name="user")
        graph.output(user)
        ...
        # Create new node
        new = graph.call_function(smith.tanh, args=(x,), name="new")
    ...
    def test_transfer_multiple_merge_sets_with_chain(self):
        ...
        # Create nodes
        node1 = graph.call_function(smith.relu, args=(x,), name="node1")
        node2 = graph.call_function(smith.abs, args=(x,), name="node2")
        node3 = graph.call_function(smith.neg, args=(x,), name="node3")
        other_node = graph.call_function(smith.sigmoid, args=(x,), name="other_node")
        node4 = graph.call_function(smith.tanh, args=(x,), name="node4")
        node5 = graph.call_function(smith.exp, args=(x,), name="node5")
        ...
        new_2_3 = graph.call_function(smith.sin, args=(x,), name="new_2_3")
        new_4_5 = graph.call_function(smith.cos, args=(x,), name="new_4_5")
