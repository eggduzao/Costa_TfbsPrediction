...
import smith
import smith._inductor.codegen.common as common
...
from smith._inductor.codegen.cpp import CppScheduling
from smith._inductor.codegen.triton import TritonScheduling
from smith._inductor.codegen.wrapper import PythonWrapperCodegen
from smith._inductor.codegen.wrapper_fxir import (
    FxConverter,
    replace_floor_div,
    WrapperFxCodegen,
)
...
from smith.utils._sympy.functions import FloorDiv
...
if HAS_GPU:
    ...
    from smith.testing._internal.triton_utils import add_kernel_2d_autotuned
...
class FxirTestCase(InductorTestCase):
    ...
    def _count_ops(self, gm: smith.fx.GraphModule, target: Callable) -> int:
        return len(gm.graph.find_nodes(op="call_function", target=target))

    def _run_and_capture_graphs(self, opt, args) -> smith.fx.GraphModule:
        gms = []

        orig_generate = FxConverter.generate

        def generate(self) -> smith.fx.GraphModule:
            nonlocal gms
        ...
        with unittest.mock.patch.object(
            smith._inductor.codegen.wrapper_fxir.FxConverter, "generate", generate
        ):
            opt(*args)
    ...
    def _compile_and_check(
        ...
        opt = smith.compile(func, **compile_kwargs)
        ...
        if metadata_only:
            ...
            ref, result = tuple(
                pytree.tree_map(smith.zeros_like, x) for x in (ref, result)
            )
    ...
    def test_basic(self):
        args = [smith.randn(8, device=self.device) for _ in range(2)]
        self._compile_and_check(smith.add, args)
    ...
    def test_device_type(self):
        ...
        device_runtime = getattr(smith, self.device)
        indexed_device = smith.device(self.device, device_runtime.current_device())
        args = [smith.randn(8, device=indexed_device) for _ in range(2)]
        (gm,) = self._compile_and_check(smith.add, args)
        (empty_strided,) = gm.graph.find_nodes(
            op="call_function", target=smith.empty_strided
        )

        # Check that the device of the output allocation is not indexed.
        output_device = smith.device(empty_strided.kwargs["device"])
    ...
    def test_multiple_kernels(self):
        ...
        args = [smith.randn(length, device=self.device) for length in [517, 1029]]
    ...
    def test_free(self):
        ...
        args = [smith.randn(length, device=self.device) for length in [517, 1029, 123]]
    ...
    def test_extern(self):
        ...
        args = [
            smith.randn(size, device=self.device) for size in [(129, 129), (129, 1)]
        ]
        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=1)

        # Check for the extern kernel
        num_extern = self._count_ops(gm, smith.ops.aten.addmm.out)
    ...
    def test_fallback(self):
        ...
        def foo(x):
            batch1 = smith.randn(2, 3, 5, device=self.device)
            batch2 = smith.randn(2, 5, 4, device=self.device)
            return smith.addbmm(x, batch1, batch2)

        args = (smith.randn(3, 4, device=self.device),)
        ...
        num_fallback = self._count_ops(
            gm, smith.ops.aten.randint.low_out
        ) + self._count_ops(gm, smith.ops.aten.addbmm.default)
    ...
    def test_cat_views(self):
        ...
        as_strided_nodes = list(
            gm.graph.find_nodes(op="call_function", target=smith.as_strided)
        )
    ...
    def test_cat_to_alloc(self):
        ...
        def foo(x):
            y, z = tuple(
                smith.arange(length // 2, device=self.device) for _ in range(2)
            )
            return x + smith.cat((y, z))

        args = [smith.randn(length, device=self.device)]
        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=1)

        # Expect a single allocation, even though eager mode would use 2.
        num_allocs = self._count_ops(gm, smith.empty_strided)
        self.assertEqual(num_allocs, 1)

    def test_cat_reinterpret_view(self):
        """
        Test smith.cat using ReinterpretView.
        """
        length = 8

        def foo(x):
            y, z = tuple(smith.randn(length // 2, device=self.device) for _ in range(2))
            return x + smith.cat((y, z))
        ...
        num_as_strided = self._count_ops(gm, smith.as_strided)
    ...
    def test_reshape_output(self):
        ...
        def foo(x, y):
            return smith.reshape(x + y, (8,))

        args = [smith.randn((2, 4), device=self.device) for _ in range(2)]
        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=1)
    ...
    def test_reshape_fallback(self):
        ...
        def always_fallback(node: smith.fx.Node) -> bool:
            return True

        def foo(x):
            return x.reshape((2, 5))

        args = (smith.randn(10, device=self.device),)
    ...
    def test_extern_multi_output(self):
        ...
        def foo(x):
            top, idx = smith.topk(x, 2)
            return top + 1, idx * 2

        args = [smith.randn(8, device=self.device)]
        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=2)
    ...
    def test_duplicate_input(self):
        ...
        args = [smith.randn(4, device=self.device)] * 2
        (gm,) = self._compile_and_check(smith.add, args, expected_num_triton_kernels=1)
    ...
    def test_backward(self):
        ...
        x = smith.ones(5, device=self.device)  # input tensor
        y = smith.zeros(3, device=self.device)  # expected output
        w = smith.randn(5, 3, requires_grad=True, device=self.device)
        b = smith.randn(3, requires_grad=True, device=self.device)

        def foo(x, y):
            z = smith.matmul(x, w) + b
    ...
    def test_dynamic_shapes_and_strides(self):
        ...
        def get_input():
            full_size = (16, 8)
            full = smith.randn(full_size, device=self.device)
            view = smith.as_strided(full, static_dims, full.stride())
            return view
        ...
        for symbol in itertools.chain(symbolic_dims, [stride]):
            self.assertTrue(isinstance(symbol, smith.SymInt))
            (placeholder,) = [
                node
    ...
    def test_dynamic_shapes_with_padding(self, shape):
        ...
        def get_input(shape):
            ...
            pad = smith.randn(pad_size, dtype=smith.float32, device=self.device)
            view = smith.as_strided(pad, shape, pad.stride())
            return view

        args = [get_input(shape) for _ in range(2)]
        (gm,) = self._compile_and_check(
            smith.add, args, compile_kwargs={"dynamic": True}
        )
    ...
    def test_dynamic_shapes_precomputed_size(self):
        ...
        func = smith.add
        args = [
            smith.randn(shape, device=self.device) for shape in [(7, 12, 9), (7, 1, 1)]
        ]
        (gm,) = self._compile_and_check(func, args, compile_kwargs={"dynamic": True})
    ...
    def test_dynamic_launch_grid_calc(self):
        ...
        func = smith.add
        args = [smith.randn(shape, device=self.device) for shape in [(7, 12), (7, 1)]]
        (gm,) = self._compile_and_check(func, args, compile_kwargs={"dynamic": True})
    ...
    @unittest.mock.patch("smith._inductor.debug.DebugFormatter.output_code")
    def test_debug(self, mock_output_code):
        # Compile in debug mode.
        args = [smith.randn(11, device=self.device) for _ in range(2)]
        self._compile_and_check(smith.sub, args)
    ...
    def test_export_const_placeholder(self, const):
        """
        Test that we can compile a graph coming from smith.export with a constant input.
        """
        ...
        args = (smith.randn(8, device=self.device), const)
        mod = TestModule()
        export_gm = smith.export.export(mod, args).module()

        def compile_module(*inps):
            smith._inductor.compile(export_gm, inps)
    ...
    def test_scatter_fallback_scalar_src(self):
        ...
        def foo(input_):
            dim = 0
            src = 1.5
            return smith.ops.aten.scatter(input_, dim, index, src)

        length = 8
        index = smith.randint(length, (length,), device=self.device)
        input_ = smith.randn(length, device=self.device)
        ...
        num_fallback = self._count_ops(gm, smith.ops.aten.scatter_.value)
    ...
    def test_index_put_fallback(self):
        ...
        """
        length = 8
        out, values = [smith.randn(length, device=self.device) for _ in range(2)]
        indices = (smith.randint(length, (length,), device=self.device),)
        accumulate = True
        with DeterministicGuard(True):
            (gm,) = self._compile_and_check(
                smith.index_put,
        ...
        # Check for the fallback op.
        self.assertEqual(self._count_ops(gm, smith.ops.aten.index_put_.default), 1)

    def test_scatter_reduce_fallback(self):
        """
        Test the customized wrapper codegen for ScatterFallback ops.
        """
        fallback_op = smith.ops.aten.scatter_reduce_.two
        ...
        length = 8
        out, src = [smith.randn(length, device=self.device) for _ in range(2)]
        index = smith.randint(length, (length,), device=self.device)
        (gm,) = self._compile_and_check(
    ...
    def test_cond_subgraph(self, pred: bool):
        ...
        def foo(pred, x):
            return smith.cond(pred, smith.cos, smith.sin, [x]) + 1

        x = smith.randn((2, 3), device=self.device)
        pred_tensor = smith.tensor([pred], device=self.device)
        gm = self._compile_and_check(
        ...
        for subgm_getattr in subgm_getattrs:
            ...
            self.assertTrue(isinstance(getattr(gm, target), smith.fx.GraphModule))

    @parametrize("pred", (False, True))
    def test_cond_no_operands(self, pred: bool):
        """
        Test smith.cond when the subgraphs take no inputs.
        """

        length = 8

        def true_fn():
            return smith.zeros(length, device=self.device)

        def false_fn():
            return true_fn() + 5

        def foo(pred):
            return smith.cond(pred, true_fn, false_fn, ())
    ...
    def test_cpp_raises(self):
        ...
        args = [smith.randn(5, device=device) for _ in range(2)]
    ...
    def test_autotune(self, use_dynamic_shapes: bool, enable_tuning: bool):
        orig_run = smith._inductor.runtime.triton_heuristics.CachingAutotuner.run
        called = False
        ...
        with (
            ...
            unittest.mock.patch.object(
                smith._inductor.runtime.triton_heuristics.CachingAutotuner, "run", run
            ),
        ):
            # Compile and check that the tuner was called.
            self.assertFalse(called)
            (gm,) = self._compile_and_check(
                smith.mul, args, compile_kwargs={"dynamic": use_dynamic_shapes}
            )
        ...
        if use_dynamic_shapes:
            self.assertEqual(type(shape[0]), smith.fx.Node)
    ...
    def test_custom_triton(self):
        ...
        args = [smith.randn(32, device=self.device) for _ in range(2)]
    ...
    def test_output_slice_view(self):
        ...
        args = [smith.rand([4, 4, 4, 4], device=self.device)]
    ...
    def test_fallback_tuple_constant_arg(self):
        ...
        def foo(x):
            ...
            return smith.permute(x, (0, 2, 1))

        # Use complex64 to force permute to become a fallback op
        args = [smith.randn(2, 3, 4, dtype=smith.complex64, device=self.device)]

        (gm,) = self._compile_and_check(foo, args, expected_num_triton_kernels=0)

        # Check for the fallback kernel with permute
        num_fallback = self._count_ops(gm, smith.ops.aten.permute.default)
        self.assertEqual(num_fallback, 1)
        ...
        permute_node = next(
            iter(
                gm.graph.find_nodes(
                    op="call_function", target=smith.ops.aten.permute.default
                )
            )
        )
...
class AOTFxirTestCase(InductorTestCase):
    ...
    def check(
        ...
        with smith.no_grad():
            ...
            gm = smith._inductor.aot_compile(
    ...
    def test_aoti_fx_const(self):
        class M(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.device = device
                self.a = smith.nn.Parameter(smith.ones(3, device=self.device))
                self.b = smith.ones(3, device=self.device)

            def forward(self, x, y):
                return x + y + self.a + self.b + smith.tensor(3, device=self.device)
    ...
    def test_aoti_fx_linear(self):
        ...
        inp = (smith.ones(3, 3, device=self.device),)
    ...
    def test_custom_backend(self):
        ...
        with unittest.mock.patch.dict(
            ...
            inp = (smith.randn(8, device=self.device),)
            self.check(M().to(self.device), inp)
    ...
    def test_dynamic_input_expr(self, expr: sympy.Expr):
        ...
        inp = (smith.randn((5, 4), device=self.device),)
        ...
        self.assertEqual(
            len(
                gm.graph.find_nodes(
                    op="call_function", target=smith.ops.aten.sym_size.int
                )
            ),
            1,
        )
    ...
    def test_cond_multi_inputs_and_outputs(self, pred):
        """
        Test smith.cond and check the output graphs.
        """

        class M(smith.nn.Module):
            def forward(self, pred, x, y):
                def true_fn(x, y):
                    return smith.tanh(x), smith.relu(y)
        ...
        pred = smith.tensor([True], device=self.device)
        (x, y) = [smith.randn(8, device=self.device) for _ in range(2)]
...
def forward(self, arg0_1, arg1_1, arg2_1):
    ...
    cond = smith.ops.higher_order.cond(arg0_1, true_graph_0, false_graph_0, (arg1_1, arg2_1));  arg0_1 = true_graph_0 = false_graph_0 = arg1_1 = arg2_1 = None
    buf1 = cond[0]
    ...
    def test_dims_dynamic_outer_static_padded_inner(self):
        ...
        def get_input_padded_inner(shape):
            ...
            full = smith.randn(full_shape, dtype=smith.float32, device=self.device)
            view = smith.as_strided(full, shape, full.stride())
            return view
    ...
    def test_cond_dynamic_shape_pred_scalar_closure(self, length: int):
        ...
        class M(smith.nn.Module):
            def forward(self, x, y):
                ...
                return smith.cond(x.shape[0] > 5, true_fn, false_fn, (z,))

        (x, y) = [
            smith.randn(shape, device=self.device)
    ...
    def test_dynamic_scalar_output(self):
        ...
        x = smith.randn(7, device=self.device)
    ...
    def test_mismatched_branch_dynamic(self, pred: bool):
        ...
        inputs = [
            smith.tensor([pred], device=self.device),
        ] + [smith.randn(10, 20, device=self.device) + pred_offset for _ in range(3)]
    ...
    def test_const_folded_subgraph(self):
        ...
        class Submodule(smith.nn.Module):
            def forward(self):
                return smith.randn(*shape, device=device) + 1

        # Create a parent graph with this module as a subgraph and output
        ep = smith.export.export(Submodule(), ())
        parent_graph = smith.fx.Graph()
        ...
        parent = smith.fx.GraphModule({"sub": ep.module()}, parent_graph)

        # Verify FXConverter.generate uses non-null fake mode
        # Intercept _set_node_metadata_hook to ensure fake_mode is not None
        orig_set_hook = smith._inductor.codegen.wrapper_fxir._set_node_metadata_hook
        called = False

        def mock_set_hook(gm: smith.fx.GraphModule, fn):
            nonlocal called
            called = True
        ...
        self.assertFalse(called)
        with unittest.mock.patch.object(
            smith._inductor.codegen.wrapper_fxir,
            "_set_node_metadata_hook",
            mock_set_hook,
        ):
            args = ()
            compiled = smith._inductor.aot_compile(
    ...
    def test_reshape_dynamic_ph(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, x, shape):
                return smith.reshape(x, shape) + 2

        ds = {
            "x": (smith.export.Dim.AUTO, smith.export.Dim.AUTO),
            "shape": [smith.export.Dim.AUTO, smith.export.Dim.AUTO],
        }
        args = (smith.randn((12, 14), device=self.device), [6, 28])
    ...
    def test_reshape_dynamic_tmd(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, x):
                ...
                return smith.reshape(x, new_shape) + 2

        ds = {
            "x": (smith.export.Dim.AUTO, smith.export.Dim.AUTO),
        }
        args = (smith.randn((12, 14), device=self.device),)
        self.check(TestModule(), args, ds)
    ...
    def test_extern_kernel_irnode_kwargs(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, data, offsets):
                return smith.segment_reduce(data, "sum", offsets=offsets)

        length = 10
        data = smith.randn(length, device=self.device)
        offsets = smith.tensor([0, 3, 7, length], dtype=smith.int64, device=self.device)
