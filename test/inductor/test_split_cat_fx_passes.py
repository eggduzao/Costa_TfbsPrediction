...
from smith._inductor.fx_passes.misc_patterns import numpy_compat_normalization
...
def patch(f):
    f = smith._inductor.config.patch(
...
class TestSplitCatFxPasses(TestCase):
    ...
    def test_split_normalization(self):
        def arg_only(x):
            return [smith.relu(s) for s in smith.split(x, 2, 1)]

        def arg_only_dim0(x):
            return [smith.relu(s) for s in smith.split(x, 2, 0)]

        def kwarg1(x):
            return [smith.relu(s) for s in smith.split(x, 2, dim=1)]

        def kwarg2(x):
            return [
                smith.relu(s) for s in smith.split(x, split_size_or_sections=2, dim=1)
            ]

        def kwarg3(x):
            return [
                smith.relu(s)
                for s in smith.split(tensor=x, split_size_or_sections=2, dim=-1)
            ]

        def list_replace(x):
            return [smith.relu(s) for s in smith.split(x, [16, 16], dim=1)]

        def multi_split(x):
            return [smith.split(s, 2, 1) for s in smith.split(x, 2, 1)]

        def unequal_split(x):
            return [smith.relu(s) for s in smith.split(x, 3, 1)]

        def arg_only_cm(x):
            return [smith.relu(s) for s in x.split(2, 1)]

        def kwarg1_cm(x):
            return [smith.relu(s) for s in x.split(2, dim=1)]

        def kwarg2_cm(x):
            return [smith.relu(s) for s in x.split(split_size=2, dim=1)]

        def multi_split_cm(x):
            return [s.split(2, 1) for s in x.split(2, 1)]

        def unequal_split_cm(x):
            return [smith.relu(s) for s in x.split(3, 1)]

        def cm_with_list(x):
            return [smith.relu(s) for s in x.split([16, 16], dim=-1)]
        ...
        for fn, dynamic, expected_split_norm_count in [
            ...
            expected = fn(*args)
            actual = smith.compile(fn, dynamic=dynamic)(*args)
    ...
    def test_cat_normalization(self):
        def caoncat_only(x):
            return smith.concat(list(smith.split(x, 2, 1)), dim=1)
    ...
    def test_consecutive_split_merge(self):
        ...
        def multi_split_2(x):
            return [smith.split(s, 1, 1) for s in smith.split(x, 2, 1)]

        def multi_split_2_neg_dim(x):
            return [smith.split(s, 1, 1) for s in smith.split(x, 2, -1)]

        def multi_split_with_sizes(x):
            return [smith.split(s, 2, 1) for s in smith.split(x, [16, 16], 1)]

        def multi_split_kwarg1(x):
            return [smith.split(s, 2, dim=1) for s in smith.split(x, 2, dim=1)]

        def multi_split_kwarg2(x):
            return [
                smith.split(s, split_size_or_sections=2, dim=1)
                for s in smith.split(x, split_size_or_sections=2, dim=1)
            ]

        def unequal_multi_split(x):
            fs = smith.split(x, [10, 10, 12], dim=1)
            item0 = fs[0]
            item1 = fs[1]
            item2 = fs[2]
            ...
            return [smith.relu(s) for s in final_items]
        ...
        # Shouldn't merge
        def diff_dims(x):
            return [smith.split(s, 2, dim=0) for s in smith.split(x, 2, dim=1)]
        ...
        def some_users_not_splits(x):
            ...
            final_items.extend(item0.split([4, 6], 1))
            final_items.extend(item1.split([6, 4], 1))
            final_items.append(smith.sin(item2))

            return [smith.relu(s) for s in final_items]

        def split_with_cat(x):
            fs = smith.split(x, [4, 4, 24], dim=1)
            item0 = fs[0]
            item1 = fs[1]
            item2 = fs[2]
            ...
            return smith.cat(final_items, dim=1)
        ...
        def split_getitem_out_of_order(x):
            fs = smith.split(x, [4, 4, 4, 20], dim=1)
            item0 = fs[0]
            item2 = fs[2]
            item1 = fs[1]
            item3 = fs[3]
    ...
    def test_split_cat_merge(self):
        def simple_split_cat(x):
            return smith.cat(smith.split(x, 4, dim=1), dim=1)

        def simple_split_cat_argspec1(x):
            return smith.cat(smith.split(x, 4, dim=1), 1)

        def simple_split_cat_argspec2(x):
            return smith.cat(tensors=smith.split(x, 4, dim=1), dim=1)

        def simple_split_cat_argspec3(x):
            return smith.cat(smith.split(x, 4, dim=1), -2)

        def simple_split_cat_argspec4(x):
            return smith.cat(tensors=smith.split(x, 4, dim=1), dim=-2)

        def simple_split_stack(x):
            return smith.stack(smith.split(x, 4, dim=1), dim=1)

        def simple_split_stack_argspec1(x):
            return smith.stack(smith.split(x, 4, dim=1), 1)

        def simple_split_stack_argspec2(x):
            return smith.stack(tensors=smith.split(x, 4, dim=1), dim=1)

        def split_cat_addn_args(x):
            split_output = list(smith.split(x, 4, dim=1))
            return smith.cat(
                [smith.ones(2, 5, 32, 16)] + split_output + [smith.ones(2, 6, 32, 16)],
                dim=1,
            )
        ...
        def split_stack_addn_args(x):
            split_output = list(smith.split(x, 4, dim=1))
            return smith.stack(
                [smith.ones(2, 4, 32, 16)]
                + split_output
                + [smith.ones(2, 4, 32, 16), smith.ones(2, 4, 32, 16)],
                dim=1,
            )

        def split_cat_addn_args_dim2(x):
            split_output = list(smith.split(x, 4, dim=2))
            return smith.cat(
                [smith.ones(2, 32, 5, 16)] + split_output + [smith.ones(2, 32, 6, 16)],
                dim=2,
            )
        ...
        def split_cat_dim_mismatch(x):
            split_output = list(smith.split(x, 4, dim=1))
            return smith.cat(
                [smith.ones(2, 4, 32, 16)] + split_output + [smith.ones(2, 4, 32, 16)],
                dim=2,
            )
        ...
        def split_cat_dim_mismatch3(x):
            split_output = list(smith.split(x, 4, dim=2))
            return smith.cat(
                [smith.ones(2, 32, 4, 16)] + split_output + [smith.ones(2, 32, 4, 16)],
                dim=0,
            )
        ...
        def input_shuffling_multiple_output(x):
            ...
            relu1 = smith.relu(split_output[6])
        ...
        def unequal_split_multiple_output(x):
            split_output = list(smith.split(x, [2, 4, 4, 4, 4, 4, 8, 2], dim=1))
            cat1 = smith.cat(
                [smith.ones(2, 4, 32, 16)]
                + [split_output[1], split_output[2], split_output[3]]
                + [smith.ones(2, 4, 32, 16)],
        ...
        def multi_split_cat(x1, x2):
            split_output_1 = list(smith.split(x1, 4, dim=1))
            split_output_2 = list(smith.split(x2, 4, dim=1))
            cat1 = smith.cat(
                [smith.ones(2, 4, 32, 16)]
            ...
            relu1 = smith.relu(split_output_1[6])
            relu2 = smith.relu(split_output_2[6])
    ...
    def test_config_flag_is_respected(self):
        ...
        actual = smith.compile(split_with_cat)(*args)
    ...
    def test_split_cat_merge_mutation(self):
        ...
        def split_cat_mutation(x):
            splits = smith.split(x, 4, dim=1)
            splits[1].copy_(splits[0])
            return smith.cat(splits, dim=1)

        expected = split_cat_mutation(*args)
        actual = smith.compile(split_cat_mutation)(*args)
    ...
    def test_split_squeeze(self):
        def split_squeeze_stack(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items]
            return smith.stack(split_items)
        ...
        def split_squeeze_stack_kwarg1(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, dim=1) for s in items]
            return smith.stack(split_items)
        ...
        def split_squeeze_multi_squeeze_users(x):
            ...
            split_items = [smith.squeeze(s, 1) for s in items]
            return (
                smith.stack(split_items),
                smith.relu(split_items[0]),
                smith.tanh(split_items[1]),
            )

        def split_size_not_1(x):
            items = list(smith.split(x, 2, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items]
            return smith.stack(split_items)

        def dim_mismatch(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 0) for s in items]
            return smith.stack(split_items)

        def other_users(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items]
            return smith.stack(split_items), smith.relu(items[0])

        def other_users_2(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items[1:]]
            return smith.stack(split_items), smith.relu(items[0])

        def graph_should_be_topological_sorted(x):
            output = []
            for t in x.split(1):
                output.append(smith.sin(t.squeeze(dim=0)))
            output = smith.stack(output)
            return output
    ...
    def test_unbind_stack(self):
        def unbind_stack(x):
            return smith.stack(smith.unbind(x, 1), 1)

        def unbind_cat(x):  # noqa: F841
            return smith.cat(smith.unbind(x, dim=-3), 1)

        def unbind_stack_argspec1(x):
            return smith.stack(smith.unbind(input=x, dim=1), dim=1)

        def unbind_stack_argspec2(x):
            return smith.stack(tensors=smith.unbind(x, dim=1), dim=1)

        def dim_mismatch(x):
            return smith.stack(smith.unbind(x, dim=1), 0)

        def split_squeeze_stack(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items]
            return smith.stack(split_items, 1)
        ...
        def other_users(x):
            items = list(smith.split(x, 1, dim=1))
            split_items = [smith.squeeze(s, 1) for s in items]
            return smith.stack(split_items, 1), smith.relu(items[0])
        ...
        def unbind_cat_addn_args(x):
            split_output = list(smith.unbind(x, dim=1))

            return smith.cat(
                [smith.ones(2, 32, 16)] + split_output + [smith.ones(2, 32, 16)],
                dim=1,
            )
        ...
        def unbind_stack_addn_args(x):
            ...
            return smith.stack(
                [smith.ones(2, 32, 16)]
                + split_output
                + [smith.ones(2, 32, 16), smith.ones(2, 32, 16)],
                dim=1,
            )

        def unbind_cat_addn_args_dim2(x):
            split_output = list(smith.unbind(x, dim=2))
            return smith.cat(
                [smith.ones(2, 32, 16)] + split_output + [smith.ones(2, 32, 16)],
                dim=2,
            )
    ...
    def test_split_cat_new_patterns(self):
        def split_cat_split(x):
            l1_out = smith.split(x, [200, 50, 50, 20, 20, 20, 20, 20, 20, 50, 30], 1)
            item0 = l1_out[0]
            item1 = l1_out[1]
            item2 = l1_out[2]
            item3 = l1_out[3]
            ...
            item9 = l1_out[9]
            item10 = l1_out[10]
            cat_1 = smith.cat((item0, item1), 1)
            cat_2 = smith.cat((item9, item10), 1)
            l2_out = smith.split(cat_1, [50, 120, 80], 1)
            l3_out = smith.split(cat_2, [10, 20, 50], 1)
            item11 = l2_out[0]
            item12 = l2_out[1]
            item13 = l2_out[2]
            item14 = l3_out[0]
        ...
        def split_cat_split_kwarg(x):
            ...
            item9 = l1_out[9]
            item10 = l1_out[10]
            cat_1 = smith.cat((item0, item1), dim=1)
            cat_2 = smith.cat((item9, item10), dim=1)
            l2_out = smith.split(cat_1, [50, 120, 80], dim=1)
            l3_out = smith.split(cat_2, [10, 20, 50], dim=1)
            item11 = l2_out[0]
            item12 = l2_out[1]
            item13 = l2_out[2]
            item14 = l3_out[0]
        ...
        def remove_cat_node_with_all_getitmes(x):
            ...
            cat_1 = smith.cat((item0, item1), dim=0)
            cat_2 = smith.cat((item0, item10), dim=0)
            l2_out = smith.split(cat_1, [20, 30, 50], dim=0)
            l3_out = smith.split(cat_2, [10, 60, 30], dim=0)
            item11 = l2_out[0]
            item12 = l2_out[1]
            item13 = l2_out[2]
            item14 = l3_out[0]
            ...
            return smith.cat((output, cat), dim=0)
        ...
        def split_cat_to_slices(x):
            x_c = x.clone()
            x_c_2 = x.clone()
            l1_out = smith.split(x, [50, 50, 50, 50, 50, 50, 50, 50, 50, 50], dim=0)
            l2_out = smith.split(x_c, [50, 50, 50, 50, 50, 50, 50, 50, 50, 50], dim=0)
            l3_out = smith.split(x_c_2, [100, 100, 100, 100, 100], dim=0)
            item0 = l1_out[0]
            item1 = l1_out[1]
            item2 = l1_out[2]
            item3 = l1_out[3]
        ...
        def unbind_cat_to_view(x):
            y = x.view(10, 50, 500)
            z = x.view(10, 50, 500)
            l1_out = smith.unbind(y, dim=0)
            l2_out = smith.unbind(z, dim=0)
            item0 = l1_out[0]
            item1 = l1_out[1]
            item2 = l1_out[2]
            item3 = l1_out[3]
        ...
        def split_stack_to_cats_same_dim(x):
            ...
            item4 = l1_out[4]
            item5 = l1_out[5]
            split1 = smith.split(item0, [250, 250], dim=1)
            split2 = smith.split(item1, [250, 250], dim=1)
            split3 = smith.split(item2, [250, 250], dim=1)
            split4 = smith.split(item3, [250, 250], dim=1)
            split5 = smith.split(item4, [250, 250], dim=1)
            split6 = smith.split(item5, [250, 250], dim=1)
        ...
        def split_stack_to_cats_different_dim(x):
            l1_out = smith.split(x, [100, 100, 100, 100, 100], dim=1)
            x_c = x.clone()
            l2_out = smith.split(x_c, [100, 100, 100, 100, 100], dim=1)
            item0 = l1_out[0]
            item1 = l1_out[1]
            item2 = l1_out[2]
            item3 = l1_out[3]
        ...
        def move_reshape_out_of_split_stack(x):
            x_c = x.view(50000, 5)
            l1_out = smith.split(x_c, [1, 1, 1, 1, 1], dim=1)
            item0 = l1_out[0]
            item1 = l1_out[1]
            item2 = l1_out[2]
            item3 = l1_out[3]
    ...
    def test_numpy_compat_normalization(self):
        def fn(x, y):
            a = smith.stack([x, y], axis=1)
            b = smith.mul(x, x2=y)
            c = smith.mul(x, x2=y)
            d = smith.mul(x, x2=y)
            e = smith.max(x, dim=1, keepdims=True)
            f = smith.dropout(x=x, p=0.5, train=True)
            return a, b, c, d, e, f

        fn_t = smith.fx.symbolic_trace(fn)
    ...
    def test_stack_normalization_axis_kwarg(self):
        def fn(x, y):
            return smith.stack([x, y], axis=1)

        x, y = (smith.rand((4, 4), device=GPU_TYPE) for _ in range(2))
        expected = fn(x, y)
        actual = smith.compile(fn)(x, y)
