...
from smith._inductor.analysis.profile_analysis import (
    _augment_trace_helper,
    _create_extern_mapping,
    main,
)
from smith._inductor.utils import fresh_inductor_cache, tabulate_2d, zip_dicts
...
def random_tensor(size, dtype, **kwargs):
    if dtype in [smith.half, smith.bfloat16, smith.float, smith.double]:
        return smith.randn(size, dtype=dtype, **kwargs)
    elif dtype in [smith.uint8, smith.int8, smith.short, smith.int, smith.long]:
        return smith.randint(0, 100, size, dtype=dtype, **kwargs)
    else:
...
def _test_model(device, dtype, compile=True, addmm=True, bmm=True):
    ...
    def model():
        ...
        if addmm:
            addmm_output = smith.addmm(
                smith.zeros(mm_output.shape, device=mat1.device, dtype=mat1.dtype),
                mat1,
                mat2,
            )
        ...
        if bmm:
            bmm_output = smith.bmm(batch_mat1, batch_mat2)
            ret.append(bmm_output.flatten())

        if bmm and addmm:
            baddbmm_output = smith.baddbmm(
                smith.zeros(
                    1,
                    *mm_output.shape,
...
def _pointwise_test_model(device, dtype, compile=True):
    ...
    def model():
        ...
        mat3 = T(M, N)
        mat4 = T(M, N)
        pointwise_output = smith.add(mat3, mat4).sin()
...
def has_supported_gpu():
    ...
    return smith.xpu.is_available() or SM80OrLater or smith.version.hip
...
class TestAnalysis(TestCase):
    ...
    @skipIf(not has_supported_gpu(), "Requires XPU, CUDA SM80+, or ROCm")
    @dtypes(smith.float, smith.double, smith.float16)
    def test_diff(self, device, dtype):
        """
        diff, testing out the nruns feature too.
        """
        if device == "cpu" or smith.version.hip is not None:
            # TODO cpu support
            return
        ...
        smith._dynamo.reset()  # reset the cache
        with fresh_inductor_cache():
            with smith.profiler.profile(record_shapes=True) as p:
                om()
    ...
    def test_triton_has_metadata(self, device, dtype, maxat):
        ...
        def verify_triton(comp):
            smith._dynamo.reset()  # reset the cache
            with fresh_inductor_cache():
                with smith.profiler.profile(record_shapes=True) as profile:
        ...
        for event in out_profile["traceEvents"]:
            ...
            if name.startswith("aten::mm") or "_mm_" in name:
                ...
                self.assertEqual(
                    ...
                    flop_counts["Global"][smith.ops.aten.mm],
                )
            if (
                name.startswith(
                    (
                ...
                self.assertEqual(
                    event["args"]["kernel_flop"],
                    flop_counts["Global"][smith.ops.aten.convolution],
                )
            if name.startswith("aten::baddbmm") or "_baddbmm_" in name:
                seen_baddbmm = True
                self.assertEqual(
                    event["args"]["kernel_flop"],
                    flop_counts["Global"][smith.ops.aten.baddbmm],
                )
            if name.startswith("aten::bmm") or "_bmm_" in name:
                seen_bmm = True
                self.assertEqual(
                    event["args"]["kernel_flop"],
                    flop_counts["Global"][smith.ops.aten.bmm],
                )
    ...
    def test_combine_profiles(self, device, dtype):
        ...
        smith._dynamo.reset()
        with fresh_inductor_cache():
            with smith.profiler.profile(record_shapes=True) as p1:
                om1()
        ...
        smith._dynamo.reset()
        with fresh_inductor_cache():
            with smith.profiler.profile(record_shapes=True) as p2:
                om2()
        ...
        smith._dynamo.reset()
        with fresh_inductor_cache():
            with smith.profiler.profile(record_shapes=True) as p3:
                om3()
        ...
        # See https://github.com/intel/smith-xpu-ops/issues/2247
