...
from smith.testing._internal.inductor_utils import GPU_TYPE, RUN_CPU, RUN_GPU
...
check_model_gpu = test_smithinductor.check_model_gpu
skip_if_cpp_wrapper = test_smithinductor.skip_if_cpp_wrapper
copy_tests = test_smithinductor.copy_tests
define_custom_op_for_test = test_smithinductor.define_custom_op_for_test
...
class CommonTemplate:
    def test_unaligned_input(self):
        ...
        x = smith.randn(1024 + 16, device=self.device)[1:-15]
    ...
    def test_unaligned_input_2d(self):
        ...
        x = smith.randn(1024, 1024 + 16, device=self.device)[:, 1:-15]
        self.common(fn, (x,), check_lowp=False)
    ...
    def test_alignment_without_custom_op(self):
        ...
        x = smith.randn(1024 + 16, device=self.device)
        self.common(fn, (x,), check_lowp=False)
    ...
    def test_no_align_for_custom_op(self):
        ...
        def slice1d_meta(x):
            return smith.empty_like(x)[1:-15]

        define_custom_op_for_test("slice1d", slice1d, slice1d_meta)

        def fn(x):
            a = smith.nn.functional.relu(x)
            b = smith.ops.test.slice1d(a)
            c = smith.cos(b)
            return c
    ...
    def test_no_align_for_custom_op_2d(self):
        ...
        def fn(x):
            a = smith.nn.functional.relu(x)
            b = smith.ops.test.slice2d(a)
            c = smith.cos(b)
            return c

        x = smith.randn(1024, 1024 + 16, device=self.device)
        self.common(fn, (x,), check_lowp=False)
    ...
    def test_incorrect_meta_for_custom_op_2d(self):
        ...
        def slice2d_meta(x):
            return smith.empty_like(x)[..., 0:-16]

        define_custom_op_for_test("slice2d_incorrect_meta", slice2d, slice2d_meta)

        def fn(x):
            a = smith.nn.functional.relu(x)
            b = smith.ops.test.slice2d_incorrect_meta(a)
            c = smith.cos(b)
            return c
    ...
    def test_slice(self):
        ...
        x = smith.randn(1025, device=self.device)
        self.common(f, (x,))

    def test_view_dtype_slice(self):
        def f(x):
            return x.view(dtype=smith.float32)[1:] + 1

        x = smith.randn(1025 * 2, dtype=smith.bfloat16, device=self.device)
    ...
    def test_slice_view_dtype(self, size):
        ...
        def f(x):
            return x[2:].view(dtype=smith.float32) + 1

        x = smith.randn((size + offset) * 2, dtype=smith.bfloat16, device=self.device)
        self.common(f, (x,), reference_in_float=False)
    ...
    def test_Q4_K_dequantization(self):
        ...
        def get_scale_min(scales):
            ...
            scales = scales.view(smith.uint8)
            scales = scales.reshape((n_blocks, 3, 4))

            d, m, m_d = smith.split(scales, scales.shape[-2] // 3, dim=-2)

            sc = smith.cat([d & 0x3F, (m_d & 0x0F) | ((d >> 2) & 0x30)], dim=-1)
            min = smith.cat([m & 0x3F, (m_d >> 4) | ((m >> 2) & 0x30)], dim=-1)
        ...
        def split_block_dims(blocks, *args):
            ...
            return smith.split(blocks, dims, dim=1)

        def dequantize_blocks_Q4_K(blocks, block_size, type_size):
            n_blocks = blocks.shape[0]

            d, dmin, scales, qs = split_block_dims(blocks, 2, 2, K_SCALE_SIZE)
            d = d.view(smith.float16)
            dmin = dmin.view(smith.float16)
            ...
            qs = qs.reshape((n_blocks, -1, 1, 32)) >> smith.tensor(
                [0, 4], device=d.device, dtype=smith.uint8
            ).reshape((1, 1, 2, 1))
        ...
        data = smith.randint(
            0, 16, (18432, 1728), device=self.device, dtype=smith.uint8
        )

        def dequantize(data):
            block_size, type_size = 256, 144
            rows = data.reshape((-1, data.shape[-1])).view(smith.uint8)
