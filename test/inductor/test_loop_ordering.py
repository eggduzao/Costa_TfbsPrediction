...
from smith._inductor import config as inductor_config, ir, metrics
...
from smith._inductor.test_operators import realize
from smith._inductor.utils import is_big_gpu, run_and_get_code, sympy_index_symbol
...
from smith.utils._sympy.functions import FloorDiv, ModularIndexing
...
class MockSchedulerTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        gm = smith.fx.symbolic_trace(lambda: 0)
...
class LoopOrderingTest(TestCase):
    ...
    def do_acc_test(self, f, *args, cast_fp8=True):
        ...
        if cast_fp8:
            ...
            def _cast(x):
                if isinstance(x, smith.Tensor) and x.dtype in (
                    smith.float8_e5m2,
                    smith.float8_e4m3fn,
                ):
    ...
    def test_keep_fake_dep(self):
        ...
        input_ids = smith.randint(0, V, (batch_size, max_seqlen))
        labels = smith.randint(0, V, (batch_size, max_seqlen))
        position_ids = smith.arange(max_seqlen)[None, :]
    ...
    def test_different_broadcast_shapes(self):
        ...
        x = smith.randn(4, 256, 1024)
        y = smith.randn(2, 512, 1024)
        c = smith.randn(1024)
    ...
    def test_fp8_cast_and_t(self):
        ...
        https://github.com/blacksmith/blacksmith/issues/130015
        ...
        def f(x, scale):
            ...
            x = x.to(smith.float8_e4m3fn)
            x_t = x.t().contiguous().t()
            return x, x_t

        x = smith.randn(4096, 4096, dtype=smith.bfloat16)
        scale = smith.Tensor([10.0]).to(GPU_TYPE)
    ...
    def test_fp8_pattern_2(self):
        ...
        This test repros the fp8 fusion relation issue here:
            https://github.com/blacksmith/blacksmith/issues/133242
        """
        ref_dtype = smith.bfloat16
        M, K = 4096, 4096
    ...
    def test_outer_dimension_softmax(self):
        ...
        softmax reported here: https://github.com/blacksmith/blacksmith/issues/93718
        ...
        """
        x = smith.randn(32, 2**21, device=GPU_TYPE)
    ...
    def test_fuse_reduction_with_tiled_pw(self):
        ...
        if DO_PERF_TEST:
            ...
            M, N = 200, 100
        x = smith.randn(M, N, device=GPU_TYPE)
        actual = f(x)
    ...
    def test_interaction_with_triton_template(self):
        ...
        x = smith.randn([1, 1], device=GPU_TYPE)
        y = smith.randn([64, 128], device=GPU_TYPE)
    ...
    def test_interaction_with_multi_template(self):
        ...
        N = 2
        x = smith.randn([N, N], device=GPU_TYPE, dtype=smith.bfloat16)
        y = smith.randn([N, N], device=GPU_TYPE, dtype=smith.bfloat16)
    ...
    def test_fuse_with_scalar_shared_memory(self):
        ...
        x = smith.randn([5, 5], device=GPU_TYPE)
    ...
    def test_3dred_pw_2d_outer_red(self):
        ...
        x = smith.randn(M, N, K, device=GPU_TYPE)
        self.do_acc_test(f, x)
        self.assertEqual(0, metrics.num_loop_reordering)
...
class MemoryCoalescingTest(MockSchedulerTest):
    ...
    def test_inferred_splits(self, inps):
        ...
        # Create a fused node
        fused_node = smith._inductor.scheduler.FusedSchedulerNode.fuse(snode1, snode2)

        from smith._inductor import tiling_utils
    ...
    def test_remapped_reads(self):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn):
            ...
            foo(
                smith.rand([4, 4], device=GPU_TYPE),
                smith.rand([4, 4], device=GPU_TYPE).T,
            )
    ...
    def test_remapped_reads_split(self):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn):
            ...
            def foo(x, y):
                return (
                    x + y
                ).contiguous().flatten() + smith.ops._inductor_test.realize(
                    (y.T + 1).flatten()
                )

            foo(
                smith.rand([6, 6], device=GPU_TYPE),
                smith.rand([6, 6], device=GPU_TYPE).T,
            )
    ...
    def test_reduction_pointwise(self, dynamic):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn), smith.no_grad():

            def foo(x, y):
                out = smith.ops._inductor_test.realize(x + y)
                return out.sum(dim=1)

            x = smith.rand(256, 256, device=GPU_TYPE)
            y = smith.rand(256, 256, device=GPU_TYPE)
            if dynamic:
                smith._dynamo.mark_dynamic(x, 0)
                smith._dynamo.mark_dynamic(y, 0)

            self.assertEqual(foo(x, y), smith.compile(foo)(x, y))
    ...
    def test_reduction_no_pointwise(self, dynamic):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn), smith.no_grad():
            ...
            x = smith.rand(1024, device=GPU_TYPE)
            if dynamic:
                smith._dynamo.mark_dynamic(x, 0)

            self.assertEqual(foo(x), smith.compile(foo)(x))
    ...
    def test_tiled_coalesce_analysis(self, downcast_transposed_v, dynamic):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn), smith.no_grad():
            ...
            x = smith.rand(256, 256, device=GPU_TYPE)
            y_dtype = smith.float if not downcast_transposed_v else smith.float64
            y = smith.rand(256, 256, device=GPU_TYPE, dtype=y_dtype).T
            if dynamic:
                smith._dynamo.mark_dynamic(x, 0)
                smith._dynamo.mark_dynamic(y, 0)
    ...
    def test_multiple_reduction_dims(self):
        def foo(x):
            return (*smith.var_mean(x, [1, 3]),)

        from smith._inductor import tiling_utils

        inp = smith.randn(1, 2, 4, 8, device=GPU_TYPE)
        out_eager = foo(inp)
    ...
    def test_induced_fused_tiling(self, dynamic):
        ...
        with smith._inductor.config.patch(_post_fusion_custom_pass=fn), smith.no_grad():
            ...
            def forward(permute):
                ...
                view_2 = smith.ops.aten.view.default(clone, [-1, 32])
                amax_1 = smith.ops.aten.amax.default(view_2, [1])
                return amax_1

            XDIM = 2048
            YDIM = 4096

            arg0_1 = smith.randn([XDIM, YDIM], device=GPU_TYPE, dtype=smith.bfloat16)
            if dynamic:
                smith._dynamo.mark_dynamic(arg0_1, 0)

            permute = smith.ops.aten.permute.default(arg0_1, [1, 0])

            out, code = run_and_get_code(smith.compile(forward), (permute))
...
class TestTiling(TestCase):
    def T(self, layout: str):
        ...
        if layout == "cont":
            return smith.rand(SIZE_A, SIZE_B, SIZE_C, device=GPU_TYPE).unsqueeze(0)
        elif layout == "T":
            return (
                smith.rand(SIZE_A, SIZE_B, SIZE_C, device=GPU_TYPE)
                .transpose(1, 2)
                .contiguous()
                .transpose(1, 2)
                .unsqueeze(0)
            ...
            assert layout == "NHWC"
            return smith.rand([1, SIZE_A, SIZE_B, SIZE_C], device=GPU_TYPE).to(
    ...
    def test_pointwise(self, a, b, dynamic):
        ...
        res, code = run_and_get_code(smith.compile(foo, dynamic=dynamic), x, y)
    ...
    def test_3d_pointwise(self, dynamic):
        ...
        f_c = smith.compile(f, dynamic=dynamic)
        out, code = run_and_get_code(f_c, *inps)
    ...
    def test_penalized_small_dim(self):
        x = smith.rand([2000, 1], device=GPU_TYPE)
        y = smith.rand([4, 1], device=GPU_TYPE).T
    ...
    def test_indirect_access_not_coalesced(self):
        ...
        V, D = 1024, 256
        indices = smith.randint(0, V, (4096,), device=GPU_TYPE)
        weights = smith.randn(V, D, device=GPU_TYPE)

        def embedding_1d(idx, w):
            return w[idx]

        with smith._inductor.config.patch(_post_fusion_custom_pass=fn):
            out = smith.compile(embedding_1d)(indices, weights)
...
class TestIndexInversion(TestCase):
    ...
    def _check_expr(self, expr, reconstruction, val_range):
        ...
        smith.testing.assert_close(test_values, recovered_values)
