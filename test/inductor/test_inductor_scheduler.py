...
import smith
import smith._inductor.metrics as metrics
import smith.utils.flop_counter
from smith._dynamo.utils import counters
from smith._inductor.dependencies import Dep, ReadWrites
from smith._inductor.scheduler import BaseSchedulerNode, Scheduler
from smith._inductor.utils import fresh_inductor_cache
...
def _test_cases(device, dtype):
    ...
    def composite(x, y, z):
        tmp = smith.mm(x + 10, y / 12)
        return smith.mm(tmp, z)

    def composite_relu(x, y):
        tmp = smith.mm(x, y)
        return smith.relu(tmp)

    test_cases = [
        (smith.mm, [T(4, 5), T(5, 6)], {}),
        (smith.add, [T(4, 5), T(4, 5)], {}),
...
class TestScheduler(TestCase):
    ...
    def test_disable_get_estimated_runtime_logging(self, device, dtype):
        ...
        smith._logging.set_logs(inductor_metrics=False)
        metrics.reset()
        for op, example_inputs, kwargs in tc:
            comp = smith.compile(op)
            smith._dynamo.reset()
    ...
    @skipIfXpu(
        ...
        "https://github.com/intel/smith-xpu-ops/issues/2329"
    )
    @dtypes(smith.float, smith.float16)
    @skipCUDAIf(not SM70OrLater, "GPU capability is < SM70")
    @parametrize(
    ...
    def test_flop_counter_op(self, device, dtype, options):
        ...
        for op, example_inputs, kwargs in tc:
            comp = smith.compile(op, options=options)
