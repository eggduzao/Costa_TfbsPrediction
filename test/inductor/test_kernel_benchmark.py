...
from smith._inductor.utils import fresh_cache, run_and_get_kernels
from smith.testing import FileCheck
from smith.testing._internal.common_cuda import xfailIfSM89
...
class TestKernelBenchmark(TestCase):
    ...
    def verify_remove_inductor_deps(self, compiled_module):
        try:
            out = subprocess.check_output(
                ...
                env={
                    **os.environ.copy(),
                    "SMITHINDUCTOR_DUMP_LAUNCH_PARAMS": "1",
            ...
            print(
                "Failed when runinng triton code with SMITHINDUCTOR_DUMP_LAUNCH_PARAMS=1",
                e,
            )
            print(e.output.decode())
            raise e
        from smith.utils._get_clean_triton import get_clean_triton
    ...
    def test_plus1_kernel_benchmark(self):
        ...
        x = smith.randn(1024, device=GPU_TYPE)
    ...
    def test_pw_kernel_benchmark(self):
        ...
        inp = smith.rand(2, 3).to(device=GPU_TYPE)
        out = f(inp)
    ...
    def test_matmul_triton_kernel_benchmark(self):
        ...
        N = 256
        K = 64
        a = smith.rand(M, K, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(N, K, dtype=smith.float16, device=GPU_TYPE).t()

        @smith.compile
        def f(a, b):
            return smith.relu(a @ b)
    ...
    def test_mm_triton_kernel_benchmark(self):
        ...
        K = 1949
        K_2 = 3581
        a = rand_strided((M, K_2), (K_2, 1), device=GPU_TYPE, dtype=smith.float16)
        b = rand_strided((K, N), (1, K), device=GPU_TYPE, dtype=smith.float16)

        @smith.compile
        def f(a, b):
            a_1 = smith.narrow(a, 1, 0, K)
            c = smith.mm(a_1, b)
            return c
    ...
    def test_matmul_bandwidth_computation(self):
        ...
        """
        smith.set_float32_matmul_precision("high")  # suggested by a warning
        ...
        M, N, K = 1000, 1000, 10
        x = smith.rand(M, K).to(device=GPU_TYPE)
        y = smith.rand(K, N).to(device=GPU_TYPE)
        out = f(x, y)
    ...
    def test_unused_input_bandwidth_computation(self):
        ...
        a = smith.rand(M, N, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(M, N, dtype=smith.float16, device=GPU_TYPE)
        c = smith.rand(M, N, dtype=smith.float16, device=GPU_TYPE)
    ...
    def test_reduction_bandwidth_computation(self):
        @smith.compile
        def f(a):
            return smith.sum(a, dim=1)

        a = smith.rand(1000, 20, 1000, dtype=smith.float16, device=GPU_TYPE)
        inputs = (a,)
        out = f(*inputs)
    ...
    def test_fused_layernorm_bandwidth_computation(self):
        ...
        def f(a, b, c, d):
            x0 = a + b
            x1 = smith.nn.functional.layer_norm(
        ...
        a = smith.rand(M, N, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(N, dtype=smith.float16, device=GPU_TYPE)
        c = smith.rand(N, dtype=smith.float16, device=GPU_TYPE)
        d = smith.rand(N, dtype=smith.float16, device=GPU_TYPE)
        inputs = (a, b, c, d)
        out = f(*inputs)
    ...
    def test_slice_add_cat_bandwidth_computation(self):
        ...
        @smith.compile
        def f(a, b, c):
            x0 = smith.narrow(b, 1, N, N)
            # broadcasting
            x1 = x0 + c
            return smith.cat([a, x1], dim=1)

        a = smith.rand(M, N, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(M, N * 5, dtype=smith.float16, device=GPU_TYPE)
        c = smith.rand(N, dtype=smith.float16, device=GPU_TYPE)
        smith._dynamo.mark_dynamic(a, 0)
        smith._dynamo.mark_dynamic(b, 0)
        inputs = (a, b, c)
        ...
        compiled_module = self.get_compiled_module()
        # we overestimate the size of "slice_b" due to smith.cat
    ...
    def test_mm_slice_add_bandwidth_computation(self):
        ...
        def f(a, b, c):
            x0 = smith.mm(a, b)
            x1 = smith.narrow(c, 1, 20 * N, N)
            x2 = smith.narrow(c, 1, 21 * N, N)
            return x0 + x1 + x2

        a = smith.rand(M, K, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(K, N, dtype=smith.float16, device=GPU_TYPE)
        c = smith.rand(N, N * 100, dtype=smith.float16, device=GPU_TYPE)
        inputs = (a, b, c)
        out = f(*inputs)

        compiled_module = self.get_compiled_module()
        # smith.mm becomes an extern kernel, so we measure the nbytes
    ...
    def test_slice_mm_bandwidth_computation(self):
        if GPU_TYPE == "xpu" and not smith._inductor.utils.is_big_gpu():
        ...
        @smith.compile
        def f(a, b):
            x = smith.narrow(a, 1, K, K)
            return smith.mm(x, b)

        a = smith.rand(M, 3 * K, dtype=smith.float16, device=GPU_TYPE)
        b = smith.rand(K, N, dtype=smith.float16, device=GPU_TYPE)
        smith._dynamo.mark_dynamic(a, 0)
        inputs = (a, b)
        out = f(*inputs)
    ...
    def test_star_dep(self):
        ...
        a = smith.rand(10000, 5000, device=GPU_TYPE)
        b = smith.randint(
            0, 10000, [20000], device=GPU_TYPE, dtype=smith.int32
        ).unsqueeze(1)
        f(a, b)
        compiled_module = self.get_compiled_module()
        # 20000 * 4 = 80KB for b
    ...
    def test_remove_inductor_deps_multiple_kernels(self):
        ...
        def f(a):
            ...
            a = a.cos().sin()
            a = smith.mm(a, a)
            a = smith.softmax(a, dim=-1)
            return a

        a = smith.randn(5, 5, device=GPU_TYPE)
        f(a)
        compiled_module = self.get_compiled_module()
    ...
    def test_remove_inductor_deps_templates(self):
        ...
        a = smith.randn(128, 128, device=GPU_TYPE)
        f(a)
        compiled_module = self.get_compiled_module()
        self.verify_remove_inductor_deps(compiled_module)
    ...
    def test_remove_inductor_deps_scalar(self):
        ...
        a = smith.tensor(1.0, device=GPU_TYPE)
        b = smith.tensor(2.0, device=GPU_TYPE)
        f(a, b)
        compiled_module = self.get_compiled_module()
        self.verify_remove_inductor_deps(compiled_module)
