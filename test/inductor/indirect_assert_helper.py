...
def upper1(x):
    b = smith.arange(4, device=x.device)
    return x[b]


def lower1(x):
    b = x.new_full((), -4, dtype=smith.int64)
    return x[b]
...
def lower2(x):
    b = x.new_zeros((), dtype=smith.int64)
    return x[b - 4]
...
if __name__ == "__main__":
    ...
    x = smith.randn(shape_x, device=GPU_TYPE)
    y = smith.arange(4, device=GPU_TYPE)
    fn = vars()[fn_name]
    fn = smith.compile(dynamic=dynamic_shapes)(fn)
    if fn_name == "store":
        shape = (y.numel(),) + x.shape[2:]
        z = smith.randn(shape, device=GPU_TYPE)
        fn(x, y, z)
