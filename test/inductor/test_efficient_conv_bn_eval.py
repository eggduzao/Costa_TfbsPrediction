...
class EfficientConvBNEvalTemplate(TestCase):
    ...
    def test_basic(self):
        def test_conv_bn_eval(
            ...
            mod_optimized = smith.compile(mod_optimized)

            original_value = counters["inductor"]["efficient_conv_bn_eval"]

            optim_eager = smith.optim.SGD(mod_eager.parameters(), lr=1e-3)
            optim_optimized = smith.optim.SGD(mod_optimized.parameters(), lr=1e-3)
