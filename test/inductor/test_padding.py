...
from smith._dynamo.convert_frame import maybe_cprofile
from smith._dynamo.device_interface import get_interface_for_device
from smith._dynamo.testing import rand_strided, reduce_to_scalar_loss
from smith._inductor import config, ir, metrics
from smith._inductor.fx_passes import pad_mm as pad_mm_pass
from smith._inductor.runtime.benchmarking import benchmarker
from smith._inductor.test_case import run_tests, TestCase
from smith._inductor.utils import ceildiv, run_and_get_code
...
def get_optim(m):
    return smith.optim.Adam(m.parameters(), lr=0.01, capturable=True, foreach=True)


def gen_transformer_inputs(vocab_size, bs, seq_length):
    def geninp():
        return smith.randint(
            0, vocab_size, (bs, seq_length), dtype=smith.int64, requires_grad=False
        )
...
class LinearAndSoftmax(nn.Module):
    ...
    def get_example_inputs(self, batch_size=16):
        return smith.randn(batch_size, 512, 768), smith.randint(
...
class TestCaseBase(TestCase):
    ...
    def setUpClass(cls):
        if HAS_GPU:
            cls.prior_float32_matmul_precision = smith.get_float32_matmul_precision()
            cls.prior_default_device = smith.get_default_device()
    ...
    def tearDownClass(cls):
        if HAS_GPU:
            smith.set_float32_matmul_precision(cls.prior_float32_matmul_precision)
            smith.set_default_device(cls.prior_default_device)
    ...
    def check_close(self, ref, act, tol=1e-3):
        ...
        if isinstance(ref, dict) and "loss" in ref:
            ...
            act = act["loss"]
        self.assertTrue(
            smith.allclose(ref, act, atol=tol, rtol=tol), f"ref:\n{ref}\nact:\n{act}"
        )
    ...
    def do_profiling(
        ...
        with smith.profiler.profile(with_stack=WITH_STACK) as p:
            niter = 3
            for _ in range(niter):
                with smith.profiler.record_function(tag_lhs):
                    f_lhs(*args, **kwargs)

                with smith.profiler.record_function(tag_rhs):
                    f_rhs(*args, **kwargs)
            device_interface.synchronize()
...
class PerfTestBetweenGoodAndBadShape(TestCaseBase):
    ...
    def test_LinearAndSoftmax_both_shapes(self, bias=True):
        ...
        m_bad_shape_opt = smith.compile(m_bad_shape)
        m_good_shape_opt = smith.compile(m_good_shape)
...
class PerfTestWithAndWithoutPadding(TestCaseBase):
    ...
    def run_acc_and_perf_test(self, model, inputs, perf_inputs=None, tol=1e-3):
        ...
        if DO_PERF_TEST:
            ...
            with config.patch(comprehensive_padding=True):
                ...
                opt_f_with_padding = smith.compile(
            ...
            with config.patch(comprehensive_padding=False):
                ...
                opt_f_without_padding = smith.compile(
    ...
    def test_nvidia_deeprecommender(self):
        ...
        m = Model()
        perf_inputs = smith.randn(256, layer_sizes[0])
...
class PaddingTest(TestCaseBase):
    ...
    def test_padmm(self):
        ...
        """
        mat1_pad = smith.randn(8192, 30522, dtype=smith.float16)
        mat2_pad = smith.randn(30522, 768, dtype=smith.float16)
        ...
        def pad_dim(x: Tensor, padded_length: int, dim: int) -> Tensor:
            ...
            return smith.cat([x, pad], dim=dim)

        @smith.compile(fullgraph=True, options={"triton.cudagraphs": False})
        def g():
            mat1 = mat1_pad
            mat2 = mat2_pad
            mat1 = pad_dim(mat1, 6, 1)
            mat2 = pad_dim(mat2, 6, 0)
            return smith.ops.aten.mm(mat1, mat2)
    ...
    def test_matmul(self):
        ...
        """
        x_good_shape = smith.randn(8192, 30528, dtype=smith.float16)
        weight_good_shape = smith.randn(30528, 768, dtype=smith.float16)
        out_good_shape = smith.randn(8192, 768, dtype=smith.float16)

        # Using stride (30522, 1) does not make a difference here.
        x_bad_shape = rand_strided(
            (8192, 30522), (30528, 1), device=GPU_TYPE, dtype=smith.float16
        )
        weight_bad_shape = smith.randn(30522, 768, dtype=smith.float16)
        out_bad_shape = smith.randn(8192, 768, dtype=smith.float16)

        def f(x, weight, out):
            smith.mm(x, weight, out=out)
            return out
    ...
    def test_LinearAndSoftmax_codegen(self, bias=True):
        ...
        m_bad_shape_opt = smith.compile(copy.deepcopy(m_bad_shape))
    ...
    def test_attention(self):
        ...
        class Attention(nn.Module):
            ...
            def forward(self, x):
                ...
                return self.cancel_reshape(smith.matmul(weights, self.reshape(value)))
    ...
    def test_pad_strides(self):
        ...
        out_strides = list(ir.Layout._pad_strides(in_strides, sizes, smith.float32))
    ...
    def test_pad_3d_tensor(self):
        ...
        x = smith.randn(2, 16, 2047)
        y = smith.randn(2047, 2047)
    ...
    def test_conv(self):
        ...
        x1 = smith.randn(*x_shape)

        padded_stride = ir.Layout._pad_strides(x1.stride(), x1.shape, smith.float32)
        x2 = rand_strided(x_shape, padded_stride, device=GPU_TYPE)
        x2.copy_(x1)

        weight = smith.randn(64, 128, 3, 3)
    ...
    def test_cat(self):
        ...
        """
        x = smith.randn(8192, 30522, dtype=smith.float16)

        def f(x):
            pad = x.new_zeros(x.size(0), 6)
            return smith.cat([x, pad], dim=1)
    ...
    def test_pad_channels_last(self):
        t = smith.randn(2, 3, 5, 1025)
        in_strides = t.stride()
        out_strides = ir.Layout._pad_strides(in_strides, t.shape, smith.float32)
        self.assertTrue(in_strides != out_strides)

        t = t.to(memory_format=smith.channels_last)
        in_strides = t.stride()
        out_strides = ir.Layout._pad_strides(in_strides, t.shape, smith.float32)
        self.assertTrue(in_strides == out_strides)
    ...
    def test_pad_outputs(
        self, dtype: smith.dtype, shape: tuple[int], alignment_bytes: int
    ):
        """
        ...
        """
        func = smith.add
        inputs = tuple(smith.randn(*shape, dtype=dtype) for input_idx in range(2))
        ...
        element_size = smith.tensor([], dtype=dtype).element_size()
    ...
    def test_noop_concat_output_padding(self, shape, alignment_bytes, pad_output):
        ...
        def get_input(size: tuple[int], alignment_bytes: int) -> smith.Tensor:
            ...
            full = smith.randn(size_padded, dtype=smith.float32)
        ...
        with config.patch(config_patches):
            compiled = smith.compile(smith.cat)
        ...
        output_line = f"buf12 = empty_strided_{GPU_TYPE}({output_shape}, {output_stride}, smith.float32)"
    ...
    def test_outer_dynamic_shape_padding(self, shape, alignment_bytes, enable_pad):
        ...
        num_inputs = 2
        input_tensors = [
            smith.randn(shape, dtype=smith.float32) for _ in range(num_inputs)
        ]
        ...
        with config.patch(config_patches):
            smith._dynamo.mark_dynamic(input_tensors[0], 0)
            smith._dynamo.mark_dynamic(input_tensors[1], 0)
            compiled = smith.compile(smith.add)
    ...
    def test_perm_outer_dynamic_shape_padding(
        ...
        def permute_contig(x):
            return smith.permute(x, perm).contiguous()
        ...
        with config.patch(config_patches):
            smith._dynamo.mark_dynamic(input_tensors[0], 2)
            compiled = smith.compile(permute_contig)
            result, _ = run_and_get_code(compiled, *input_tensors)
    ...
    def test_dynamic_shape_padding(self, shape, alignment_bytes, enable_pad):
        ...
        with config.patch(config_patches):
            compiled = smith.compile(smith.add, dynamic=True)
            result, _ = run_and_get_code(compiled, *input_tensors)
