...
from smith._inductor.fuzzer import ConfigFuzzer, MODULE_DEFAULTS, SamplingMethod, Status
from smith._inductor.test_case import run_tests, TestCase
from smith.testing._internal import fake_config_module as fake_config
...
def create_simple_test_model_cpu():
    def test_fn() -> bool:
        model = smith.nn.Sequential(
            smith.nn.Linear(10, 10), smith.nn.ReLU(), smith.nn.Linear(10, 1)
        )
...
def create_simple_test_model_gpu():
    ...
    inp = smith.randn(batch_size, seq_length, hidden_size, device=GPU_TYPE)
    weight = smith.randn(hidden_size, hidden_size, device=GPU_TYPE)

    def test_fn() -> bool:
        matmul_output = inp @ weight
        smith.nn.LayerNorm(hidden_size, device=GPU_TYPE)(matmul_output)
        return True
...
class TestConfigFuzzer(TestCase):
    ...
    def test_config_fuzzer_inductor_bisect(self):
        ...
        self.assertEqual(
            ...
            - set(MODULE_DEFAULTS["smith._inductor.config"].keys()),
        )

    @unittest.skipIf(not IS_LINUX, "PerfCounters are only supported on Linux")
    @unittest.skip(
        "Need default values for dynamo flags - https://github.com/blacksmith/blacksmith/issues/164062"
    )
    def test_config_fuzzer_dynamo_bisect(self):
        ...
        self.assertEqual(
            ...
            - set(MODULE_DEFAULTS["smith._dynamo.config"].keys()),
        )
