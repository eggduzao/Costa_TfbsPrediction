...
from smith._inductor.runtime.caching import (
    config,
    context,
    exceptions,
...
class ConfigTest(TestCase):
    ...
    def test_versioned_config_env_var_override(
        ...
        with (
            ...
            patch(
                "smith._inductor.runtime.caching.config.is_fbcode",
                return_value=False,
            ),
    ...
    def test_versioned_config_version_check(
        ...
        with (
            ...
            patch(
                "smith._utils_internal.justknobs_getval_int",
    ...
    def test_versioned_config_oss_default(
        ...
        value. This ensures proper behavior for open-source Blacksmith distributions.
        """
        with (
            self.FOO_ENV_VAR_OVERRIDE_LOCK.acquire(timeout=1),
            patch.dict(os.environ, {}, clear=True),
...
class ForceDisableCachesTest(TestMixin, TestCase):
    ...
    def test_force_disable_caches_disables_caching_module(self) -> None:
        ...
        Verifies that when smith._inductor.config.force_disable_caches is True,
...
class FreshCacheIntegrationTest(TestMixin, TestCase):
    ...
    def test_memoizer_registered_with_clear_on_fresh_cache(self) -> None:
        ...
        """
        from smith._inductor.utils import _registered_caches
...
class ShouldPadMemoizerTest(TestMixin, TestCase):
    ...
    def test_should_pad_memoizer_caches_result(self) -> None:
        ...
        """
        import smith
        from smith._inductor.runtime.caching import encoders
        ...
        mat1 = smith.randn(8, 16, dtype=smith.float32)
        mat2 = smith.randn(16, 32, dtype=smith.float32)
        op = smith.ops.aten.mm
    ...
    def test_should_pad_memoizer_different_shapes_different_cache_entries(
        ...
        mat1_small = smith.randn(8, 16, dtype=smith.float32)
        mat2_small = smith.randn(16, 32, dtype=smith.float32)

        mat1_large = smith.randn(12, 16, dtype=smith.float32)
        mat2_large = smith.randn(16, 32, dtype=smith.float32)
    ...
    def test_should_pad_memoizer_different_dtypes_different_cache_entries(
        ...
        def mock_should_pad(
            ...
            nonlocal call_count
            call_count += 1
            return mat1.dtype == smith.float32

        mock_match = self._create_mock_match()
        op = smith.ops.aten.mm

        # Execute: call with different dtypes but same shapes
        mat1_fp32 = smith.randn(8, 16, dtype=smith.float32)
        mat2_fp32 = smith.randn(16, 32, dtype=smith.float32)

        mat1_fp16 = smith.randn(8, 16, dtype=smith.float16)
        mat2_fp16 = smith.randn(16, 32, dtype=smith.float16)
    ...
    def test_should_pad_memoizer_different_ops_different_cache_entries(
        ...
        def mock_should_pad(
            ...
            call_count += 1
            call_ops.append(op)
            return op is smith.ops.aten.mm
        ...
        result_mm = mock_should_pad(mock_match, mat1, mat2, smith.ops.aten.mm)
        self.assertEqual(call_count, 1)

        result_addmm = mock_should_pad(mock_match, mat1, mat2, smith.ops.aten.addmm)
        self.assertEqual(call_count, 2)  # Should be a cache miss
    ...
    def test_should_pad_memoizer_with_input_tensor(self) -> None:
        ...
        mat1 = smith.randn(8, 16, dtype=smith.float32)
        mat2 = smith.randn(16, 32, dtype=smith.float32)
        input_tensor = smith.randn(32, dtype=smith.float32)
        op = smith.ops.aten.addmm
    ...
    def test_should_pad_memoizer_same_shape_different_data_uses_cache(self) -> None:
        ...
        mat1_a = smith.randn(8, 16, dtype=smith.float32)
        mat2_a = smith.randn(16, 32, dtype=smith.float32)

        mat1_b = smith.randn(8, 16, dtype=smith.float32)  # Different data, same shape
        mat2_b = smith.randn(16, 32, dtype=smith.float32)  # Different data, same shape
