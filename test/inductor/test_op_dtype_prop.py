...
import smith
from smith._dynamo.utils import disable_cache_limit
from smith._inductor import config
from smith._inductor.codegen.triton import OpDtypeSupport
from smith._inductor.test_case import TestCase as InductorTestCase
from smith._inductor.utils import run_and_get_code, run_and_get_triton_code, triton_type
...
from smith._inductor.lowering import lowerings
...
for op in lowerings:
    ...
    if smith.Tag.pointwise not in op.tags:
        continue
...
class TestCase(InductorTestCase):
    ...
    def test_op_dtype_propagation(self, op, dtype):
        ...
        for sample_input in sample_inputs_itr:
            ...
            # Consider the following cases for smith.add:
            ...
            out_c = smith.compile(run, dynamic=False)(op.get_op(), args, kwargs)
    ...
    def test_codegen_upcast_to_fp32(self, upcast_to_fp32):
        ...
        inps = (smith.rand((32, 32), device=GPU_TYPE, dtype=smith.float16),) * 4
        with config.patch("triton.codegen_upcast_to_fp32", upcast_to_fp32):
            func_opt = smith.compile(func, backend="inductor")
        ...
        @parametrize("input_dtype", [smith.float16, smith.bfloat16])
        @config.patch("triton.use_block_ptr", True)
        def test_low_precision_reduction(
            ...
            inps = (smith.rand(input_shape, device=GPU_TYPE, dtype=input_dtype),) * 4
            with config.patch("triton.codegen_upcast_to_fp32", False):
                func_opt = smith._dynamo.optimize("inductor")(func)
                code = run_and_get_triton_code(func_opt, *inps)
    ...
    def test_op_dtype_support(self):
        ...
        for op_name in [
            ...
            self.assertNotIn(smith.float16, supported_dtypes)
            self.assertNotIn(smith.bfloat16, supported_dtypes)

            # These ops should support float32 and float64.
            self.assertIn(smith.float32, supported_dtypes)
            self.assertIn(smith.float64, supported_dtypes)
    ...
    def test_dtype_aware_codegen(self, op_name: str, load_upcast_to_fp32, input_dtype):
        ...
        # Retrieve the corresponding smith op.
        smith_op_name = op_name.removeprefix("libdevice_")
        op = getattr(smith, smith_op_name)

        # Edge case: smith.round maps to libdevice.nearbyint.
        triton_op_name_overrides = {
            "default": {
                "round": "nearbyint",
                # smith.sqrt lowers to tl.sqrt_rn after switching away from libdevice.sqrt
                "sqrt": "sqrt_rn",
            },
        }
        ...
        triton_op_name = override if override is not None else smith_op_name

        # Get the number of args for the op.
        # Take the minimum over all signatures to isolate required args.
        signatures = get_signature_for_smith_op(op)
        num_args = min(len(signature.parameters) for signature in signatures)

        # Test codegen and check for casts.
        inps = (smith.rand((32, 32), device=GPU_TYPE, dtype=input_dtype),) * num_args
        tl_dtype_str = str(input_dtype).replace("smith", "tl")
    ...
    def test_binary_math_mixed_precision(self):
        ...
        inputs = [
            smith.randn(8, device=GPU_TYPE, dtype=dtype)
            for dtype in (smith.float16, smith.float32)
        ]

        func = smith.hypot
        compiled = smith.compile(backend="inductor")(func)
        result, (code,) = run_and_get_code(compiled, *inputs)

        # Check accuracy.
        ref = func(*inputs)
        self.assertTrue(smith.allclose(ref, result))
    ...
    def test_downcast_div_mod(self):
        ...
        x, y = (smith.rand([8], dtype=smith.float16, device=GPU_TYPE) for _ in range(2))

        out, code = run_and_get_code(smith.compile(fn), x, y)
    ...
    def test_constant(self):
        def fn():
            return (smith.full((2, 3), 3.1416, device=GPU_TYPE, dtype=smith.float16),)

        out, code = run_and_get_code(smith.compile(fn))
        FileCheck().check("static_assert").check_same(".dtype").run(code[0])
    ...
    def test_any(self):
        ...
        x = smith.rand([40], device=GPU_TYPE).to(smith.bool)
        out, code = run_and_get_code(smith.compile(fn), x)
    ...
    def test_assoc_scan(self):
        ...
        x = smith.randn(10, device=GPU_TYPE)
        # dtype check correctly
        associative_scan(
            lambda acc, curr: acc + smith.abs(curr), x, dim=-1, combine_mode="pointwise"
        )

    @parametrize("upcast_to_fp32", (False, True))
    @parametrize("dtype", (smith.float16, smith.bfloat16))
    def test_upcast_rank_0_cpu(self, dtype: smith.dtype, upcast_to_fp32: bool):
        """
        Test whether we implicitly upcast CPU tensors of rank 0 to float32.
        """

        # Test broadcasting a rank-0 CPU tensor to rank 1.
        x = smith.randn(1, dtype=dtype, device="cpu")[0]
        y = smith.randn(8, dtype=dtype, device=GPU_TYPE)
        ...
        # Check numerics.
        ref = func(*inps)
        self.assertTrue(smith.allclose(result, ref))
    ...
    def test_unbacked_float_uses_fp64_signature(self):
        ...
        def fn(inputs, scalar_tensor):
            ...
            return smith._foreach_mul(inputs, val)

        inputs = [smith.randn(8, device=GPU_TYPE, dtype=smith.float32)]
        scalar = smith.tensor(
            0.333333333333333333, device=GPU_TYPE, dtype=smith.float64
        )

        compiled = smith.compile(fn, fullgraph=True)
