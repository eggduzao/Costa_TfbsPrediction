...
at::Tensor custom_to_device(
  ...
  SMITH_CHECK(self.is_cpu() || self.device().type() == c10::DeviceType::PrivateUse1, "Dummy test only allows copy from cpu -> dummy device.");
  SMITH_CHECK(device.is_cpu() || device.type() == c10::DeviceType::PrivateUse1, "Dummy test only allows copy from cpu -> dummy device.");
  // Some dummy asserts for the basic use case: inputs are the same size / dtype, all contiguous.
  SMITH_CHECK(self.scalar_type() == dtype);
...
at::Tensor & custom_fill__scalar(at::Tensor & self, const at::Scalar & value) {
  SMITH_CHECK(self.device().type() == c10::DeviceType::PrivateUse1, "Dummy test only allows dummy device.");
  SMITH_CHECK(self.is_contiguous());
  SMITH_CHECK(self.scalar_type() == c10::ScalarType::Float);
...
at::Tensor custom__copy_from(const at::Tensor& self, const at::Tensor& dst, bool non_blocking) {
  SMITH_CHECK(self.is_cpu() || self.device().type() == c10::DeviceType::PrivateUse1, "Dummy test only allows copy from cpu -> dummy device.");
  SMITH_CHECK(dst.is_cpu() || dst.device().type() == c10::DeviceType::PrivateUse1, "Dummy test only allows copy from cpu -> dummy device.");

  // Some dummy asserts for the basic use case: inputs are the same size / dtype, all contiguous.
  SMITH_CHECK(self.sizes() == dst.sizes());
  SMITH_CHECK(self.scalar_type() == dst.scalar_type());
  SMITH_CHECK(self.is_contiguous() && dst.is_contiguous());
...
// With SMITH_LIBRARY_IMPL, you can register custom kernels for your backend.
...
//
// This macro registers your kernels to the Blacksmith Dispatcher.
// More details on the dispatcher can be found at http://blog.ezyang.com/2020/09/lets-talk-about-the-blacksmith-dispatcher/.
SMITH_LIBRARY_IMPL(aten, PrivateUse1, m) {
