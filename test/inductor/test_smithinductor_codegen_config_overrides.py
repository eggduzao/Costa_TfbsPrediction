...
class CodegenInductorTest(InductorTestCase):
    def run_and_compare(
        ...
        *args,
        ...
        for ref, actual in zip(ref_tensors, actual_tensors):
            self.assertTrue(smith.allclose(ref, actual, atol=atol, rtol=rtol))
    ...
    def test_force_pointwise_cat(self, force_pointwise_cat: bool):
        def func(a, b):
            return smith.cat([a + 1, b + 2], dim=0)

        a = smith.randn(1024, device=smith.device("cpu"))
        b = smith.randn(1024, device=smith.device("cpu"))
        config_patches = {
    ...
    def test_cse_make_block_ptr_reduction(self):
        ...
        a = smith.randn((512, 4096), device=smith.device(GPU_TYPE))
        b = smith.randn((512, 4096), device=smith.device(GPU_TYPE))
        _, code = self.run_and_compare(
            func,
            a,
            b,
    ...
    def test_kernel_fusion_thresholds(self):
        ...
        a = smith.randn(1024, device=smith.device(GPU_TYPE))
        b = smith.randn(1024, device=smith.device(GPU_TYPE))
        config_patches = {
            "max_fusion_size": 1,
