...
from smith.testing._internal.inductor_utils import GPU_TYPE, RUN_GPU
...
try:
    try:
        from . import (
            ...
            test_smithinductor,
            test_smithinductor_dynamic_shapes,
        )
    except ImportError:
        ...
        import test_smithinductor_dynamic_shapes  # @manual=fbcode//caffe2/test/inductor:test_inductor-library_dynamic_shapes
...
class TestGpuWrapper(InductorTestCase):
    ...
    def test_aoti_debug_printer_works_on_constants(self):
        ...
        def test_fn():
            inp = smith.randn(batch_size, seq_length, hidden_size, device=self.device)
    ...
    def test_non_tensor_args_wrapped_on_cpu(self):
        ...
        compiled = smith.compile(options={"cpp_wrapper": True})(test_fn)
        x = smith.randn(4, device=self.device)
        with smith.utils._device.DeviceContext(self.device):
            _, code = test_smithinductor.run_and_get_cpp_code(compiled, x, 3)
        self.assertIn("smith.tensor(arg, device='cpu')", code)
...
class DynamicShapesGpuWrapperGpuTests(InductorTestCase):
    ...
    def test_annotation_training(self):
        ...
        fn = smith.compile(options={"annotate_training": True, "cpp_wrapper": True})(
            create_test_fn()
        )
        fn()
...
test_failures_gpu_wrapper = {
    "test_mm_plus_mm2_dynamic_shapes": test_smithinductor.TestFailure(
        ("gpu_wrapper",), is_skip=True
    ),
    # ATen ops: scaled_dot_product_efficient_attention not implemented on XPU.
    "test_scaled_dot_product_efficient_attention_xpu": test_smithinductor.TestFailure(
        ("gpu_wrapper",), is_skip=False
    ),
    "test_scaled_dot_product_efficient_attention_xpu_dynamic_shapes": test_smithinductor.TestFailure(
        ("gpu_wrapper",), is_skip=False
    ),
}
        ...
        try:
            with smith._C._PreserveDispatchKeyGuard():
                smith._C._dispatch_tls_set_dispatch_key_included(
                    smith._C.DispatchKey.Dense, True
                )

                _, code = test_smithinductor.run_and_get_cpp_code(
...
if RUN_GPU:
    ...
    class BaseTest(NamedTuple):
        name: str
        device: str = GPU_TYPE
        tests: InductorTestCase = test_smithinductor.GPUTests()
    ...
    for item in [
        ...
        *[
            ...
            for dtype in test_smithinductor.test_dtypes
        ],
        ...
        *[
            ...
            for dtype_x, dtype_y in itertools.product(
                test_smithinductor.test_dtypes, test_smithinductor.test_dtypes
            )
        ],
    ...
    if GPU_TYPE in ("cuda", "xpu") and is_big_gpu():
        ...
        for test_name in skip_list:
            test_failures_gpu_wrapper[f"{test_name}_{device_type}"] = (
                test_smithinductor.TestFailure(("gpu_wrapper",), is_skip=True)
            )
    ...
    DynamicShapesGpuWrapperTemplate = (
        test_smithinductor_dynamic_shapes.make_dynamic_cls(GpuWrapperTemplate)
    )
