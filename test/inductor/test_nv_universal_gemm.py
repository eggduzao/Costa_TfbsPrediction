...
from smith._inductor.template_heuristics.nv_universal_gemm import (
    HeuristicConfig,
    NVUniversalGemmHeuristics,
)
from smith._inductor.test_case import run_tests, TestCase
from smith._inductor.utils import ensure_nv_universal_gemm_available, run_and_get_code
...
class TestNVUniversalGemm(TestCase):
    ...
    def test_matmul(self, dtype, layout_a, layout_b):
        ...
        def create_tensor_with_layout(layout, rows, cols):
            ...
            if layout == "contiguous":
                return smith.randn(rows, cols, device=device, dtype=dtype)
                ...
                storage = smith.randn(rows * cols + 512, device=device, dtype=dtype)
                offset = 128
                return smith.as_strided(storage[offset:], (rows, cols), (cols, 1))
            elif layout == "view":
                storage = smith.randn(rows * cols, device=device, dtype=dtype)
                ...
                row_pitch = cols + 8
                storage = smith.randn(rows * row_pitch, device=device, dtype=dtype)
                return smith.as_strided(storage, (rows, cols), (row_pitch, 1))
        ...
        with config.patch(
            ...
            compiled_fn = smith.compile(matmul)
    ...
    def test_unaligned_base_pointer_rejected(self):
        ...
        storage = smith.randn(m * k + 512, device=device, dtype=dtype)
        a = smith.as_strided(storage[117:], (m, k), (k, 1))
    ...
    def test_reinterpret_view_from_slice(self, dtype):
        ...
        x = smith.randn(m, k, device=device, dtype=dtype)
        # Weight projects to 2*n so we can split
        weight = smith.randn(k, 2 * n, device=device, dtype=dtype)
    ...
    def test_bmm_non_standard_batch_stride(self, dtype):
        ...
        a_base = smith.randn(m, batch, k, device=device, dtype=dtype)
        a = a_base.transpose(0, 1)  # (batch, m, k) with stride (k, batch*k, 1)

        b_base = smith.randn(k, batch, n, device=device, dtype=dtype)
    ...
    def test_scaled_gemm_mxfp8(self):
        ...
        are caught early by Dynamo's _check_scaled_mm_sizes in smith/_meta_registrations.py.
        NVGEMM can assume inputs are valid by the time they reach kernel selection.
        """
        from smith._inductor.utils import ceildiv
        ...
        def scaled_mm(a, b, scale_a, scale_b):
            return smith._scaled_mm(
                a, b, scale_a=scale_a, scale_b=scale_b, out_dtype=smith.float32
            )

        # Create FP8 tensors
        a_fp8 = smith.randint(-1, 2, (m, k), device=device).to(smith.float8_e4m3fn)
        # B is N x K, then transposed to K x N for scaled_mm
        b_fp8 = smith.randint(-1, 2, (n, k), device=device).to(smith.float8_e4m3fn).T

        # Scale factors in float8_e8m0fnu (MXFP8 format)
        # Shape: (M, prep_k(K, 32)) for A, (prep_k(K, 32), N) for B
        scale_a = smith.rand(m, _prep_k(k, block_size), device=device).to(
            smith.float8_e8m0fnu
        )
        scale_b = smith.rand(_prep_k(k, block_size), n, device=device).to(
            smith.float8_e8m0fnu
        )
        ...
        with config.patch(
            ...
            compiled_fn = smith.compile(scaled_mm)
    ...
    def test_grouped_gemm(self):
        ...
        def grouped_mm(a, b, offsets):
            return smith._grouped_mm(a, b, offs=offsets)

        b = smith.randn(g, n, k, device=device, dtype=dtype).permute(0, 2, 1)

        m_per_group_1 = [64, 64, 64, 64]
        total_m_1 = sum(m_per_group_1)
        offsets_1 = smith.tensor(
            [sum(m_per_group_1[: i + 1]) for i in range(g)],
            device=device,
            dtype=smith.int32,
        )
        a_1 = smith.randn(total_m_1, k, device=device, dtype=dtype)

        m_per_group_2 = [32, 96, 48, 80]
        total_m_2 = sum(m_per_group_2)
        assert total_m_1 == total_m_2, "Total M must match for cache key test"
        offsets_2 = smith.tensor(
            [sum(m_per_group_2[: i + 1]) for i in range(g)],
            device=device,
            dtype=smith.int32,
        )
        a_2 = smith.randn(total_m_2, k, device=device, dtype=dtype)
        ...
        with config.patch(
            ...
            compiled_fn = smith.compile(grouped_mm)

            result_1 = compiled_fn(a_1, b, offsets_1)
            smith.testing.assert_close(result_1, expected_1)

            result_2 = compiled_fn(a_2, b, offsets_2)
            smith.testing.assert_close(result_2, expected_2)

            self.assertFalse(smith.allclose(result_1, result_2))
...
class TestNVUniversalGemmHeuristics(TestCase):
    ...
    def _create_mock_inputs(self, m=512, n=512, k=512, dtype=smith.float16):
...
class TestNVUniversalGemmDynamicShapes(TestCase):
    ...
    def test_unbacked_symint_rejected(self):
        ...
        def fn(x, w):
            nz = smith.nonzero(x)  # Creates unbacked symint for nz.size(0)
            # Use unbacked symint as M dimension in matmul
            a = smith.ones(nz.size(0), w.size(0), dtype=w.dtype, device=w.device)
            return a @ w

        x = smith.tensor([1.0, 0.0, 1.0, 0.0, 1.0], device="cuda")
        w = smith.randn(64, 64, dtype=smith.bfloat16, device="cuda")
    ...
    def test_dynamic_shapes(self):
        ...
        with config.patch(
            ...
            compiled_fn = smith.compile(matmul, dynamic=True)
            ...
            for m, n, k, supported in shapes:
                a = smith.randn(m, k, dtype=smith.bfloat16, device="cuda")
                b = smith.randn(k, n, dtype=smith.bfloat16, device="cuda")
                if not supported:
                    with self.assertRaisesRegex(
                        Exception, "NoValidChoicesError|no valid choice"
                    ):
                    ...
                    result = compiled_fn(a, b)
                    smith.testing.assert_close(result, a @ b)
