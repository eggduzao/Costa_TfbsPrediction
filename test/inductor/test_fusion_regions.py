...
HAS_GPU = smith.cuda.is_available()
...
class TestFusionRegionDetection(InductorTestCase):
    ...
    def test_fusion_region_grouping(self):
        ...
        from smith._inductor.fx_passes.fusion_regions import build_fusion_regions

        with FakeTensorMode():
            a = smith.ones(64, 64, device=self.device)
    ...
    def test_mm_not_in_fusion_region(self):
        ...
        with FakeTensorMode():
            a = smith.ones(64, 64, device=self.device)
            traced = make_fx(lambda a: smith.mm(a + 1, a) * 2)(a)
    ...
    def test_strict_local_fusion_no_cross_mm(self):
        ...
        with FakeTensorMode():
            a = smith.ones(64, 64, device=self.device)
            y = smith.ones(64, 64, device=self.device)
    ...
    def test_collapse_and_expand_fusion_regions(self):
        ...
        with FakeTensorMode():
            a = smith.ones(64, 64, device=self.device)
            b = smith.ones(64, 64, device=self.device)
    ...
    def test_is_fusible_node(self):
        ...
        from smith._inductor.fx_passes.fusion_regions import is_fusible_node

        with FakeTensorMode():
            a = smith.randn(64, 64, device=self.device)
            traced = make_fx(lambda a: smith.linalg.qr(smith.mm(a + 1, a)))(a)
    ...
    def test_collapse_expand_preserves_correctness(self):
        ...
        a = smith.randn(64, 64, device=self.device)
        b = smith.randn(64, 64, device=self.device)
        ...
        with FakeTensorMode():
            a_fake = smith.ones(64, 64, device=self.device)
            b_fake = smith.ones(64, 64, device=self.device)
