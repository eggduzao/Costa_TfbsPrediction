...
import smith
from smith._dynamo.repro.aoti import (
    AOTIMinifierError,
...
class MinifierUtilsTests(TestCase):
    def test_invalid_output(self):
        ...
        # it can also obtained from running funcsmith.compile.minifier on an exported graph.
        traced = smith.fx.symbolic_trace(model)
        for strict in [True, False]:
            gm = export_for_aoti_minifier(traced, (smith.randn(2, 2),), strict=strict)
    ...
    def test_non_exportable(self):
        ...
        inputs = (smith.randn(2), smith.randn(2))
        for strict in [True, False]:
    ...
    def test_convert_module_to_string(self):
        class M(smith.nn.Module):
            def forward(self, x, flag):
                ...
                return smith.cond(flag > 0, true_fn, true_fn, [x])
        ...
        model = M()
        gm = smith.export.export(model, inputs, strict=False).module(check_guards=False)
...
#         item = smith.ops.aten.item.default(flag);  flag = None
#         gt = item > 0;  item = None
#         true_graph_0 = self.true_graph_0
#         false_graph_0 = self.false_graph_0
#         cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (x,));  gt = true_graph_0 = false_graph_0 = x = None
