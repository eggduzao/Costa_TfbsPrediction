...
from smith._inductor.custom_graph_pass import CustomPartitionerFn, get_hash_for_files
...
class TestCustomPartitionerFn(TestCase):
    def test_custom_partitioner_fn(self):
        ...
        a = smith.randn((2, 2), requires_grad=True, device=GPU_TYPE)
        b = smith.randn((2, 2), requires_grad=True, device=GPU_TYPE)
        ...
        with smith._inductor.config.patch(custom_partitioner_fn=custom_partitioner_fn):
