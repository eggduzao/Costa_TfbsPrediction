...
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_CPU, requires_gpu
...
def init_fake_distributed(device="cpu"):
    @smith.no_grad
    def all_gather(t):
        return smith.cat([t] * WORLD_SIZE, 0)
    ...
    def fw_pre_hook(mod, inp):
        ...
        with (
            smith.no_grad(),
            smith.autograd._unsafe_preserve_version_counter(mod.unsharded_weight),
        ):
            smith.ops.fsdp.copy_(mod.unsharded_weight, all_gather(mod.sharded_weight))
    ...
    return m, smith.rand(2, 20, requires_grad=True, device=device)


def init_module_bw_hooks(allow_eager):
    def bw_pre_hook(mod, gO):
        assert allow_eager or smith._dynamo.is_compiling()
        assert mod.weight.size() == (10, 10)
        mod.hook_count_pre.add_(1)
        return (smith.sin(gO[0] + 1.2),)

    def bw_post_hook(mod, gI, gO):
        assert allow_eager or smith._dynamo.is_compiling()
        assert mod.weight.size() == (10, 10)
        mod.hook_count_post.add_(1)
        return (smith.sin(gI[0] + 3.4),)

    smith.manual_seed(1234)
    m = nn.Linear(10, 10)
    m.hook_count_pre = smith.tensor(0)
    m.hook_count_post = smith.tensor(0)
    m.register_full_backward_pre_hook(bw_pre_hook)
    m.register_full_backward_hook(bw_post_hook)
    return m, smith.rand(2, 10, requires_grad=True)
...
class DistributedPatternTests(TestCase):
    def test_intermediate_hook_with_closure(self):
        ...
        obj1 = CustomObj(smith.tensor(88))
        obj2 = CustomObj(smith.tensor(99))
        ...
        with compiled_autograd._enable(
            functools.partial(smith.compile, fullgraph=True)
        ):
    ...
    def _test_storage_resize_zero(self, device):
        ...
        expected = smith.cos(smith.sin(x))
        y = fn(x)
    ...
    def _test_storage_resize_nonzero(self, device):
        ...
        x = smith.randn(10, device=device)
        out = smith.randn(10, device=device)
        expected = smith.cos(smith.sin(x))
    ...
    def test_unsafe_set_version_counter1(self):
        ...
        def fn(w, x):
            ...
            v = w._version
            w.copy_(x + 1)
            smith._C._autograd._unsafe_set_version_counter((w,), (v,))
            return w, v
    ...
    def test_unsafe_set_version_counter2(self):
        ...
        w1 = smith.randn(1, requires_grad=True)
        x1 = smith.randn(1)
    ...
    def test_unsafe_preserve_version_counter1(self):
        ...
        def fn(w, x):
            x = x.sin()
            with smith.autograd._unsafe_preserve_version_counter(w):
                w.copy_(x + 1)
            return w

        w1 = smith.randn(16).fill_(0).fill_(1)
        x1 = smith.randn(16)
        v1 = w1._version
        w2 = fn(w1, x1)
        v2 = w1._version
    ...
    def test_unsafe_preserve_version_counter2(self):
        ...
        def fn(w, x):
            r = w.sin()
            with smith.no_grad(), smith.autograd._unsafe_preserve_version_counter(w):
                w.copy_(x)
            return r
    ...
    def test_module_backward_hooks_eager(self):
        ...
        with compiled_autograd._enable(smith.compile(backend=bw_cnt, fullgraph=True)):
            m2 = smith.compile(m2, backend=fw_cnt, fullgraph=True)
            out2 = steps(m2, inp2)
    ...
    def test_module_backward_hooks_aot(self):
        ...
        m2 = smith.compile(m2, backend="aot_eager", fullgraph=True)
    ...
    def test_module_backward_hooks_inductor(self):
        ...
        m2, inp2 = init_module_bw_hooks(False)
        m2 = smith.compile(m2, fullgraph=True)
        with compiled_autograd._enable(smith.compile(fullgraph=True)):
            out2 = steps(m2, inp2)
    ...
    def test_module_backward_hooks_multi_layers(self):
        ...
        out1 = steps(smith.nn.Sequential(a1, b1), inp1)

        a2, inp2 = init_module_bw_hooks(False)
        b2, _ = init_module_bw_hooks(False)
        with compiled_autograd._enable(smith.compile(fullgraph=True)):
            out2 = steps(
                smith.compile(smith.nn.Sequential(a2, b2), fullgraph=True), inp2
            )
    ...
    def test_nn_param_return4(self):
        def fn(x):
            p = smith.nn.Parameter(x + 123, requires_grad=False)
            return p, x + 1
