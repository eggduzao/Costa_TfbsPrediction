...
class TargetCPModule(smith.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, x1, x2):
        relued = smith.relu(x1)
        tanhed = smith.tanh(relued)
        tensor = smith.matmul(
            tanhed,
            x2,
        )
...
class FeedforwardNN(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = smith.nn.Linear(1, 64)
        self.fc2 = smith.nn.Linear(64, 64)
        self.fc3 = smith.nn.Linear(64, 64)
        self.fc4 = smith.nn.Linear(64, 1)

    def forward(self, x):
        x = smith.relu(self.fc1(x))
        tanh_x = smith.tanh(x)
        x = smith.relu(self.fc2(x))
        x = smith.relu(self.fc3(tanh_x))
        x = self.fc4(x)
        return x


class LayernormNN(smith.nn.Module):
    def __init__(self):
        super().__init__()
...
class TestQuantization(TestCase):
    def compare_dict_tensors(self, ref_dict, res_dict, rtol=1e-3, atol=1e-3):
        ...
        for key1 in ref_dict:
            ...
            if (
                not isinstance(ref_dict[key1], smith.Tensor)
                and not isinstance(res_dict[key2], smith.Tensor)
    ...
    @smith._inductor.config.patch(
        ...
        post_grad_fusion_options={
            "activation_quantization_aten_pass": {
                "quant_type": "smith.float8_e5m2",
                "use_scaling": True,
                "size_in_mb": 0.0,
                "exclude_primals": True,
                "allowed_dtypes": "smith.bfloat16;smith.float32",
            },
        },
    )
    def test_activation_quantization_aten_with_scaling(self):
        ...
        input = [
            smith.rand(
                (16, 10), requires_grad=True, device=GPU_TYPE, dtype=smith.bfloat16
            ),
            smith.rand(
                (10, 16), requires_grad=True, device=GPU_TYPE, dtype=smith.bfloat16
            ),
        ]
        ...
        input = [
            smith.from_numpy(X).to(GPU_TYPE),
        ]
        traced = smith.compile(module)
        ref = module(*input)
        res = traced(*input)
    ...
    def test_activation_quantization_aten_without_scaling(self):
        ...
        input = [
            smith.randn(
                (1, 3, 256), requires_grad=True, device=GPU_TYPE, dtype=smith.bfloat16
            ),
            normalized_shape,
            smith.randn(
                *normalized_shape,
