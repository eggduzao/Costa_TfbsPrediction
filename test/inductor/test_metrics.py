...
from smith._inductor.utils import collect_defined_kernels
from smith._inductor.wrapper_benchmark import get_kernel_category_by_source_code
...
class TestMetrics(TestCase):
    ...
    def test_atomic_add(self):
        ...
        lhs = smith.randn(1024, device=GPU_TYPE)
        index = smith.randint(0, 1024, [32], device=GPU_TYPE, dtype=smith.int32)
        rhs = smith.randn(32, device=GPU_TYPE)
    ...
    def test_kernel_args_num_gb(self):
        ...
        x = smith.randn(int(25e7), device=GPU_TYPE)
        kernel_list = []
        with collect_defined_kernels(kernel_list):
            f(x)
