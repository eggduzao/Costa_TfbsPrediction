...
from smith._dynamo.debug_utils import InputReader
from smith._inductor import config
from smith._inductor.choices import Inducsmithoices
from smith._inductor.codegen.triton import FixedTritonConfig
from smith._inductor.runtime.hints import TRITON_MAX_BLOCK
from smith._inductor.runtime.runtime_utils import get_max_y_grid, is_power_of_2
...
TMA_XFAIL = test_smithinductor.TestFailure(GPU_TYPE, is_skip=False)
...
class BlockDescriptorTestBase(InductorTestCase):
    ...
    def _discontiguous_tensor(
        self, view_size: tuple[int, ...], device: Union[smith.device, str]
    ) -> smith.Tensor:
        """
    ...
    def _run_and_compare(
        ...
        *args,
        ...
        with config.patch(config_patches):
            compiled = smith.compile(func, backend="inductor", **compile_kwargs)
        ...
        for ref, actual in zip(ref_tensors, actual_tensors):
            ...
            self.assertTrue(smith.allclose(ref, actual, **tol))
...
class CommonTemplate:
    ...
    def test_expected_num_block_pointers(
        ...
        device = smith.device(self.device)
        inputs = [smith.randn(8).to(device) for arg_idx in range(2)]
    ...
    @parametrize(
        ...
        [
            ...
            subtest(
                ...
                decorators=[
                    test_smithinductor.skip_if_triton_cpu("Triton CPU: slow test")
                ],
            ),
        ],
    )
    def test_pointwise(
        ...
        def get_input() -> smith.Tensor:
            ...
            return smith.as_strided(full, view_size, view_stride, storage_offset=offset)
    ...
    def test_broadcast_with_singleton_dims(self):
        ...
        def forward(expand_4, permute_4, mul_7):
            clone = smith.ops.aten.clone.default(
                expand_4, memory_format=smith.contiguous_format
            )
            expand_4 = None
            view_4 = smith.ops.aten.view.default(clone, [1, 4, 64])
            clone = None
            cos = smith.ops.aten.cos.default(view_4)
            view_4 = None
            mul = smith.ops.aten.mul.Tensor(cos, 1.0)
            cos = None
            unsqueeze_4 = smith.ops.aten.unsqueeze.default(mul, 1)
            mul = None
            mul_6 = smith.ops.aten.mul.Tensor(permute_4, unsqueeze_4)
            permute_4 = unsqueeze_4 = None
            add_3 = smith.ops.aten.add.Tensor(mul_6, mul_7)
            mul_6 = mul_7 = None
            unsqueeze_6 = smith.ops.aten.unsqueeze.default(add_3, 2)
            add_3 = None
            return (unsqueeze_6,)
    ...
    def test_expand_broadcast(self, x_size: tuple[int, ...], y_size: tuple[int, ...]):
        ...
        def get_input(size: tuple[int, ...]) -> smith.Tensor:
            device = smith.device(self.device)
            full = smith.randn(size).to(device)
            view = smith.as_strided(full, size, full.stride())
            return view
    ...
    def test_pointwise_broadcast_nonzero_strides(self, prefer_nd_tiling: bool):
        ...
        device = smith.device(self.device)
        full = smith.randn(full_shape).to(device)
        col = smith.as_strided(full, col_shape, full.stride())
    ...
    def test_reduction(
        ...
        """
        if view_size == (2, 3 * max_block) and smith.version.hip is not None:
            view_size = (4, 6 * max_block)

        if view_size == (128, 128) and smith.version.hip is not None:
            view_size = (256, 256)
    ...
    def test_dynamic_shapes_pointwise_multiple_max_block(self):
        ...
        device = smith.device(self.device)
        x_size = (1, 1)
        x = smith.randn(x_size).to(device)
    ...
    @parametrize(
        ...
        [
            ((15, 15), 1, 1, smith.sum),  # Non-power-of 2 shapes.
            ((129, 129), 3, 2, smith.sum),  # Large size, with loops.
            ((3, 3), 1, 1, smith.argmax),
            ((129, 129), 1, 1, smith.argmax),
            ((5, 5), 1, 1, smith.var_mean),  # Reduction + pointwise fusion.
        ],
    )
    def test_2d_reduction_odd_shapes(
        ...
        """
        if reduction_op == smith.sum and smith.version.hip is not None:
    ...
    def test_2d_welford_reduction(
        ...
        """
        if smith.version.hip is not None and expected_num_triton_kernels == 2:
            size = (256, 256)
    ...
    @test_smithinductor.skip_if_triton_cpu  # Illegal instruction  File; cannot xfail because it crashes process
    ...
    def test_fused_2d_reduction(
        ...
        def foo(x):
            return smith.sum(x) + smith.argmax(x)
    ...
    @parametrize("reduction_op", [smith.sum, smith.argmax])
    ...
    def test_complex_reshape_block_ptr(self):
        def func(x, y):
            ...
            clone_0 = reshape_1.clone(memory_format=smith.contiguous_format)
            permute_1 = clone_0.permute([1, 0])
            clone_1 = permute_1.clone(memory_format=smith.contiguous_format)

            return clone_0, clone_1

        inps = (smith.rand((8, 2048), device=self.device, dtype=smith.float32),) * 2
        result, code = self._run_and_compare(
            func,
            *inps,
            expected_num_triton_kernels=2,
    ...
    def test_3d_permute_tiling(self):
        ...
        inps = (smith.rand((51, 51, 51), device=self.device, dtype=smith.float32),) * 3
        result, (code,) = self._run_and_compare(
            foo,
            *inps,
            expected_num_triton_kernels=1,
    ...
    def test_unbacked_size_on_non_contig_dim(self, num_tile_candidates: int):
        ...
        inps = (
            smith.rand((64, 33, 1), device=self.device, dtype=smith.float32),
            smith.scalar_tensor(16, device=self.device, dtype=smith.int32),
        )
    ...
    @test_smithinductor.xfail_if_triton_cpu
    ...
    def test_removed_buffers(self):
        ...
        result, code = self._run_and_compare(
            fn,
            *[smith.ones(200, 200, device=self.device) * p],
            expected_num_triton_kernels=1,
            expected_num_block_pointers=1,
            atol=p * 0.06,
            rtol=0.06,
    ...
    def test_expand_clone_broadcast(self):
        ...
        inps = [smith.randn(base_size, device=self.device)]
        result, (triton_code,) = self._run_and_compare(
            foo,
            *inps,
            expected_num_triton_kernels=1,
    ...
    def test_mul_broadcast_multi_output(self):
        ...
        inps = [
            smith.randn((8, 11, 128), device=self.device),
            smith.randn((128,), device=self.device),
            smith.randn((8, 11, 128), device=self.device),
        ]
        result, (triton_code,) = self._run_and_compare(
            foo,
    ...
    def test_ensure_integral_dims_and_strides(self):
        def model(data, *args):
            return smith.nn.functional.unfold(data, *args)

        data = smith.zeros(
            [2, 3, 5, 5], dtype=smith.float16, requires_grad=True, device=self.device
        )
        args = [2, 1, 0, 1]
        self._run_and_compare(
            model,
    ...
    @parametrize(
        ...
        [
            ...
            subtest(
                [False, False, True], decorators=[test_smithinductor.skip_if_triton_cpu]
            ),
        ],
    )
    @xfail_if_use_tensor_descriptor
    def test_boundary_check(self, block_multiple, ynumel_exceed_ygrid_size, include_z):
        ...
        class FixedBlockSizeChoices(Inducsmithoices):
        ...
        def func(a: smith.Tensor, b: smith.Tensor) -> smith.Tensor:
            return a + b
...
test_smithinductor.copy_tests(CommonTemplate, TritonBlockPointerTestGPU, GPU_TYPE)


@unittest.skipIf(
    not (HAS_CUDA_AND_TRITON and smith.cuda.get_device_capability()[0] >= 9)
...
class TritonTensorDescriptorTestCUDA(BlockDescriptorTestBase):
    ...
    def test_rms_norm_backward_does_not_crash_with_tma(self):
        B, S, D = 1, 1024, 40096
        with smith.device(self.device):
            x = smith.randn(B, S, D, dtype=smith.bfloat16, requires_grad=True)
            w = smith.randn(D, dtype=smith.bfloat16, requires_grad=True)

        def f(x: smith.Tensor, w: smith.Tensor) -> smith.Tensor:
            y = smith.rms_norm(x, (D,), w, 1e-5)
            return (y * 0.1).sum()

        compiled_f = smith.compile(f, backend="inductor", fullgraph=True)
        loss = compiled_f(x, w)
        loss.backward()
    ...
    def test_tiling_split_valid(self):
        ...
        class GraphModule(smith.nn.Module):
            def forward(
                ...
                linear = smith._C._nn.linear(input_feat, sub_dense0_w, sub_dense0_b)
                ...
                # Dense_1
                hidden_states_5 = smith._C._nn.linear(
                    hidden_states_4, sub_dense1_w, sub_dense1_b
                )

                # Compute output sequence length: ((s70 - 5) // 4) - 1
                # Note: In the dynamo graph, smith.sym_sum is used, but we use direct arithmetic here
                sym_sum = s70 - 5
                floordiv = sym_sum // 4
                sym_sum_1 = floordiv - 1

                # Create position ids
                arange = smith.arange(sym_sum_1, device=GPU_TYPE)
                ...
                inv_freq_expanded = expand.to(smith.device(GPU_TYPE, index=0))
                ...
                emb = smith.cat((freqs, freqs), dim=-1)
                ...
                cos_2 = cos_1.to(dtype=smith.float16)
                sin_2 = sin_1.to(dtype=smith.float16)
                ...
                cache_position = smith.arange(
                    sym_sum_1, device=GPU_TYPE, dtype=smith.int64
                )
                arange_4 = smith.arange(sym_sum_1, device=GPU_TYPE)
                ...
                linear_2 = smith._C._nn.linear(layer_norm, l0_ff1_lin1_w, None)
                hidden_states_7 = F.silu(linear_2)
                hidden_states_8 = F.dropout(hidden_states_7, p=0.1, training=False)
                hidden_states_9 = smith._C._nn.linear(
                ...
                linear_4 = smith._C._nn.linear(normalized_hidden_states, l0_q_w, None)
                view = linear_4.view((s25, sym_sum_1, -1, 16))
                query_states = view.transpose(1, 2)

                linear_5 = smith._C._nn.linear(normalized_hidden_states, l0_k_w, None)
                view_1 = linear_5.view((s25, sym_sum_1, -1, 16))
                key_states = view_1.transpose(1, 2)

                linear_6 = smith._C._nn.linear(normalized_hidden_states, l0_v_w, None)
                ...
                neg_1 = -x2_1
                cat_2 = smith.cat((neg_1, x1_1), dim=-1)
                mul_7 = cat_2 * sin_4
                k_embed = mul_6 + mul_7

                # SDPA
                attn_output = smith._C._nn.scaled_dot_product_attention(
                    q_embed,
                    k_embed,
                    value_states,
                ...
                attn_output_3 = smith._C._nn.linear(attn_output_2, l0_o_w, None)
                ...
                linear_8 = smith._C._nn.linear(layer_norm_3, l0_ff2_lin1_w, None)
                hidden_states_21 = F.silu(linear_8)
                hidden_states_22 = F.dropout(hidden_states_21, p=0.1, training=False)
                hidden_states_23 = smith._C._nn.linear(
                ...
                linear_10 = smith._C._nn.linear(layer_norm_5, l1_ff1_lin1_w, None)
                hidden_states_26 = F.silu(linear_10)
                hidden_states_27 = F.dropout(hidden_states_26, p=0.1, training=False)
                hidden_states_28 = smith._C._nn.linear(
                ...
                neg_2 = -x2_2
                cat_3 = smith.cat((neg_2, x1_2), dim=-1)
                mul_15 = cat_3 * sin_5
                ...
                neg_3 = -x2_3
                cat_4 = smith.cat((neg_3, x1_3), dim=-1)
                mul_17 = cat_4 * sin_5
                k_embed_1 = mul_16 + mul_17

                # SDPA
                attn_output_4 = smith._C._nn.scaled_dot_product_attention(
                    q_embed_1,
                    k_embed_1,
                    value_states_1,
                    attn_mask=attention_mask_1,
                ...
                attn_output_7 = smith._C._nn.linear(attn_output_6, l1_o_w, None)
                ...
                linear_16 = smith._C._nn.linear(layer_norm_8, l1_ff2_lin1_w, None)
                hidden_states_40 = F.silu(linear_16)
                hidden_states_41 = F.dropout(hidden_states_40, p=0.1, training=False)
                hidden_states_42 = smith._C._nn.linear(
        ...
        def create_parameters(device="cuda", dtype=smith.float16):
            """Create all the parameters needed by the GraphModule."""
            params = {}

            # Subsampler parameters
            params["sub_dense0_w"] = smith.randn(64, 80, device=device, dtype=dtype)
            params["sub_dense0_b"] = smith.randn(64, device=device, dtype=dtype)
            params["sub_conv0_w"] = smith.randn(64, 64, 5, device=device, dtype=dtype)
            params["sub_conv0_b"] = smith.randn(64, device=device, dtype=dtype)
            params["sub_conv1_w"] = smith.randn(32, 64, 5, device=device, dtype=dtype)
            params["sub_conv1_b"] = smith.randn(32, device=device, dtype=dtype)
            params["sub_dense1_w"] = smith.randn(64, 32, device=device, dtype=dtype)
            params["sub_dense1_b"] = smith.randn(64, device=device, dtype=dtype)

            # Rotary embedding
            params["rot_inv_freq"] = smith.randn(8, device=device, dtype=smith.float32)
            params["rot_attn_scale"] = smith.tensor(
                1.0, device="cpu", dtype=smith.float64
            )

            # Layer 0 parameters
            params["l0_norm_ff1_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_ff1_lin1_w"] = smith.randn(256, 64, device=device, dtype=dtype)
            params["l0_ff1_lin2_w"] = smith.randn(64, 256, device=device, dtype=dtype)
            params["l0_ff_res0"] = smith.tensor(0.5, device="cpu", dtype=smith.float64)
            params["l0_ff_res1"] = smith.tensor(0.5, device="cpu", dtype=smith.float64)
            params["l0_norm_attn_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_q_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l0_k_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l0_v_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l0_o_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l0_norm_conv_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_pw_conv1_w"] = smith.randn(
                128, 64, 1, device=device, dtype=dtype
            )
            params["l0_dw_conv_w"] = smith.randn(64, 1, 8, device=device, dtype=dtype)
            params["l0_bn_mean"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_bn_var"] = (
                smith.abs(smith.randn(64, device=device, dtype=dtype)) + 0.1
            )
            params["l0_bn_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_bn_b"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_pw_conv2_w"] = smith.randn(64, 64, 1, device=device, dtype=dtype)
            params["l0_conv_res0"] = smith.tensor(
                0.5, device="cpu", dtype=smith.float64
            )
            params["l0_conv_res1"] = smith.tensor(
                0.5, device="cpu", dtype=smith.float64
            )
            params["l0_norm_ff2_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l0_ff2_lin1_w"] = smith.randn(256, 64, device=device, dtype=dtype)
            params["l0_ff2_lin2_w"] = smith.randn(64, 256, device=device, dtype=dtype)
            params["l0_norm_out_w"] = smith.randn(64, device=device, dtype=dtype)

            # Layer 1 parameters
            params["l1_norm_ff1_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_ff1_lin1_w"] = smith.randn(256, 64, device=device, dtype=dtype)
            params["l1_ff1_lin2_w"] = smith.randn(64, 256, device=device, dtype=dtype)
            params["l1_norm_attn_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_q_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l1_k_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l1_v_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l1_o_w"] = smith.randn(64, 64, device=device, dtype=dtype)
            params["l1_norm_conv_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_pw_conv1_w"] = smith.randn(
                128, 64, 1, device=device, dtype=dtype
            )
            params["l1_dw_conv_w"] = smith.randn(64, 1, 8, device=device, dtype=dtype)
            params["l1_bn_mean"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_bn_var"] = (
                smith.abs(smith.randn(64, device=device, dtype=dtype)) + 0.1
            )
            params["l1_bn_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_bn_b"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_pw_conv2_w"] = smith.randn(64, 64, 1, device=device, dtype=dtype)
            params["l1_norm_ff2_w"] = smith.randn(64, device=device, dtype=dtype)
            params["l1_ff2_lin1_w"] = smith.randn(256, 64, device=device, dtype=dtype)
            params["l1_ff2_lin2_w"] = smith.randn(64, 256, device=device, dtype=dtype)
            params["l1_norm_out_w"] = smith.randn(64, device=device, dtype=dtype)

            # Output norm
            params["out_norm_w"] = smith.randn(64, device=device, dtype=dtype)
        ...
        batch_size = 13
        seq_len = 1024
        input_features = smith.randn(
            batch_size, seq_len, 80, device=device, dtype=dtype
        )
        compiled_model = smith.compile(model, fullgraph=True, dynamic=True)
