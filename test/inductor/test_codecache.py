...
from smith._dynamo.package import DynamoCache
...
from smith._inductor.codecache import (
    BypassFxGraphCache,
...
from smith.compiler._cache import (
    CacheArtifact,
...
if HAS_TRITON:
    ...
    from smith.testing._internal.triton_utils import add_kernel, sub_kernel
...
class MyModelConv2d(smith.nn.Module):
    def __init__(self, dim=512):
        super().__init__()
        self.conv1 = smith.nn.Conv2d(3, dim, kernel_size=3, stride=2, bias=False)
        self.conv2 = smith.nn.Conv2d(dim, dim, kernel_size=3, stride=2, bias=False)
...
class TestPyCodeCache(TestCase):
    ...
    def test_editable_cached_wrapper(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            env = os.environ.copy()
            env["SMITHINDUCTOR_CACHE_DIR"] = tmpdir
            ...
            step1 = textwrap.dedent(
                ...
                f(smith.ones(2))
                cache_dir = os.environ["SMITHINDUCTOR_CACHE_DIR"]
...
class TestFxGraphCache(TestCase):
    ...
    def test_cache_load_function(
        ...
        if (
            device == "cuda"
            and smith.version.hip is None
            and dtype == smith.bfloat16
            and not SM80OrLater
        ):
        ...
        a_orig = smith.rand(25, dtype=dtype, device=device)
        b_orig = smith.rand(5, 5, dtype=dtype, device=device)
    ...
    def test_remote_cache_load_function(
        ...
        a = smith.rand(25, dtype=dtype, device=device)
        b = smith.rand(5, 5, dtype=dtype, device=device)
    ...
    @parametrize("dtype", (smith.float32, smith.bfloat16))
    @parametrize("dynamic", (False, True))
    @smith._funcsmith.config.patch({"enable_autograd_cache": False})
    def test_cache_hot_load(self, device, dtype, dynamic):
        ...
        a = smith.rand(100, 100, dtype=dtype, device=device)
        b = smith.rand(100, 100, dtype=dtype, device=device)
    ...
    def test_cache_hot_load_caching_precompile(self, device, dtype, dynamic):
        ...
        with fresh_cache():
            eager_result = fn(a, b)
            # With caching precompile, we have to re smith.compile the function
        ...
        # Hot load and hit
        with fresh_cache(), smith.compiler.set_stance("fail_on_recompile"):
            cache_info = smith.compiler.load_cache_artifacts(artifact_bytes)
            self.assertEqual(len(cache_info.inductor_artifacts), 2)
            self.assertEqual(len(cache_info.autotune_artifacts), autotune_expect)
            self.assertEqual(len(cache_info.aot_autograd_artifacts), 1)
    ...
    def test_cache_hot_load_repeat(self):
        ...
        self.assertFalse(smith.compiler._cache.CacheArtifactManager.need_serialize())
        self.assertIsNotNone(artifacts)
        ...
        self.assertTrue(smith.compiler._cache.CacheArtifactManager.need_serialize())
    ...
    def test_cache_hot_load_pgo(self):
        ...
        # Record artifacts
        with smith.compiler.config.patch(job_id=self.id()), fresh_cache():
            f(smith.randn(2, 3))
            f(smith.randn(2, 4))
    ...
    def test_cache_hot_load_pgo_swap_file_names(self):
        ...
        # Record artifacts
        with mock.patch(
            "smith._utils_internal.get_mast_job_name_version", return_value=("foo", 5)
        ):
            with fresh_cache():
                f(smith.randn(2, 3))
                f(smith.randn(2, 4))
        ...
        with (
            mock.patch(
                "smith._utils_internal.get_mast_job_name_version",
                return_value=("bar", 10),
            ),
            fresh_cache(),
        ):
    ...
    def test_cache_hot_load_empty(self):
        self.assertIsNone(smith.compiler.save_cache_artifacts())
    ...
    def test_cache_load_model(self, device, dtype, dynamic):
        ...
        inp = smith.randn(2, 3, 16, 32, device=device, dtype=dtype)
    ...
    def test_cache_load_with_guards_static_bounds(self, device, dtype):
        ...
        def fn(x):
            return smith.nn.functional.adaptive_avg_pool2d(x, [5, 7])
    ...
    def test_tensor_device_guards_cpu_tensor(self):
        ...
        with smith.cuda._DeviceGuard(0):
            smith.cuda.set_device(0)
            result = f(smith.randn(3, device="cpu"))
    ...
    def test_constant_handling(self, device):
        ...
        def fn1(x):
            return x + smith.tensor(list(range(12)), device=device)

        def fn2(x):
            return x + smith.tensor(list(range(1, 13)), device=device)

        a = smith.rand(12, device=device)

        compiled_fn1 = smith.compile(fn1)
        compiled_fn2 = smith.compile(fn2)
    ...
    def test_auto_functionalized_caching(self, variant):
        if variant == "v1":
            patch = smith._inductor.config.patch(enable_auto_functionalized_v2=False)
        else:
            assert variant == "v2"
            patch = smith._inductor.config.patch(enable_auto_functionalized_v2=True)

        @smith.library.custom_op("mylib::sin_inplace", mutates_args=["x"])
        def sin_inplace(x: smith.Tensor) -> None:
            x.sin_()

        @smith.library.custom_op("mylib::cos_inplace", mutates_args=["x"])
        def cos_inplace(x: smith.Tensor) -> None:
            x.cos_()
    ...
    def test_flex_attention_caching(self):
        ...
        a, b, c = (smith.randn(1, 4, 512, 64).to(GPU_TYPE) for _ in range(3))
    ...
    def test_higher_order_op_bypass(self, bundle_triton):
        ...
        def fn(x):
            ...
            return smith.cond(x.shape[0], true_fn, false_fn, (x,))

        with config.patch(
            bundle_triton_into_fx_graph_cache=bundle_triton,
        ):
            compiled_fn = smith.compile(fn, dynamic=True, fullgraph=True)
    ...
    def test_triton_higher_order_op(self, bundle_triton):
        ...
        with config.patch(bundle_triton_into_fx_graph_cache=bundle_triton):
            ...
            compiled_fn2 = smith.compile(fn2, fullgraph=True)
    ...
    def test_triton_op(self, bundle_triton, use_static_triton_launcher):
        ...
        @smith._library.triton_op(f"{libname}::{opname}", mutates_args={})
        ...
        with config.patch(
            ...
            compiled_fn = smith.compile(f, fullgraph=True)
    ...
    def test_inductor_counters(self):
        ...
        a = smith.rand(8, 32, device="cpu")
        b = smith.rand(32, 8, device="cpu")
    ...
    def test_cache_with_nt(self):
        def gen_nt(r):
            values = smith.randn(r, 16)
            offsets = smith.tensor([0, 2, 3, 6, 13, r])
            return smith.nested.nested_tensor_from_jagged(values, offsets)
        ...
        counters.clear()
        smith.compile(fn)(inp1)
        smith.compile(fn)(inp2)
        self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 1)
        self.assertEqual(counters["inductor"]["fxgraph_cache_hit"], 0)
    ...
    def test_cache_with_symint_non_arg_guard(self):
        ...
        counters.clear()
        smith.compile(fn, fullgraph=True, dynamic=True)(x, 2)
        self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 1)
        self.assertEqual(counters["inductor"]["fxgraph_cache_hit"], 0)
    ...
    def test_cache_guard(self):
        ...
        x = smith.ones(2)
        a = smith.compile(f, dynamic=True)(x, 6)
        self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 1)
        self.assertEqual(counters["inductor"]["fxgraph_cache_hit"], 0)

        self.reset()
        counters.clear()
        b = smith.compile(f, dynamic=True)(x, 4)
        self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 1)
        self.assertEqual(counters["inductor"]["fxgraph_cache_hit"], 0)
    ...
    def test_async_compile_cache(self):
        class SimpleFunction(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return x * 2
        ...
        x = smith.rand([10], requires_grad=True, device="cuda")
        counters.clear()

        sf = SimpleFunction
        out = smith.compile(sf.apply)(x)
        out.sum().backward()
    ...
    def test_cache_guard_overspec(self):
        b = smith.tensor([0, 2, 4, 6, 8])

        @smith.compile
        class MyModel(smith.nn.Module):
            def forward(self, x):
                return smith.isin(x, b)
    ...
    def test_freezing(self, device, inlinable):
        ...
        class MM(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(shape))
        ...
        with smith.no_grad():
            x = smith.rand(shape).to(device=device, dtype=dtype)
            out0 = mod1(x)
            out1 = smith.compile(mod1)(x)
...
class TestStandaloneCompile(TestCase):
    ...
    def capture(self, fn, dynamic=None):
        def inner(*args):
            ...
            _ = smith.compile(fn, fullgraph=True, backend=backend, dynamic=dynamic)(
                *args
            )
    ...
    def test_basic(
        ...
        mod = smith.nn.Linear(1, 3, device=device)
        x = smith.randn(4, 1, device=device)
        if dynamic:
        ...
        with (
            ...
            with fresh_cache():
                ...
                compiled_artifact = smith._inductor.standalone_compile(
                    gm, args, aot=is_aot
                )
            ...
            with fresh_cache():
                loaded = smith._inductor.CompiledArtifact.load(path=path, format=format)
                if dynamic:
                    concrete_args = [
                        4 if isinstance(a, smith.SymInt) else a for a in args
                    ]
                else:
                    concrete_args = args
    ...
    def test_is_saveable(self) -> None:
        ...
        with fresh_cache():
            ...
            compiled_artifact = smith._inductor.standalone_compile(gm, args)
    ...
    def test_call_in_backend(self, dynamic: bool, is_aot: bool) -> None:
        ...
        def backend(gm, args, **kwargs):
            return smith._inductor.standalone_compile(gm, args, aot=is_aot)
    ...
    def test_save_in_new_path(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with fresh_cache():
                loaded = smith._inductor.CompiledArtifact.load(
    ...
    @unittest.skipIf(IS_FBCODE, "smith import error")
    @config.patch({"fx_graph_cache": True})
    @config.patch({"fx_graph_remote_cache": False})
    @funcsmith_config.patch({"enable_autograd_cache": True})
...
arg = smith.ones(4, 1)
with fresh_cache():
    loaded = smith._inductor.CompiledArtifact.load(path="{path}")
    compiled_result = loaded(arg)[0]

eager_result = arg.sin() * 2

if not smith.allclose(eager_result, compiled_result, atol=0.1, rtol=0.01):
...
"""
    ...
    @config.patch({"fx_graph_remote_cache": False})
    @funcsmith_config.patch({"enable_autograd_cache": True})
    @funcsmith_config.patch({"autograd_cache_normalize_inputs": True})
    @parametrize("is_aot", (False, True))
    def test_split_module(self, is_aot):
        ...
        split = smith.fx.passes.split_module.split_module(gm, gm, split)

        # Each of the split graphs only has one output.
        ca0 = smith._inductor.standalone_compile(
            split.submod_0, (a0, x, a1), aot=is_aot
        )
        ca1 = smith._inductor.standalone_compile(
            split.submod_1, (b0, x, b1), aot=is_aot
        )
        ca2 = smith._inductor.standalone_compile(
    ...
    def test_backend(self, dynamic_shapes, is_aot):
        ...
        smith._dynamo.reset()
        _ = smith.compile(f, backend=backend)(x)
    ...
    def test_backend_dynamic_shapes_from_example_inputs(self, is_aot):
        ...
        def backend(gm, args, **kwargs):
            compiled_artifact = smith._inductor.standalone_compile(
                gm, [5, smith.ones(4)], dynamic_shapes="from_example_inputs", aot=is_aot
            )
            y = smith.ones(4)
            (result,) = compiled_artifact(4, y)
            # 5 was baked in
    ...
    def test_backend_static_shapes(self, dynamic_shapes):
        ...
        def backend(gm, args, **kwargs):
            return smith._inductor.standalone_compile(
                gm, args, dynamic_shapes=dynamic_shapes
            )

        result = smith.compile(f, backend=backend)(static_x)
    ...
    def test_custom_pass_handling(self):
        ...
        class TestCustomGraphPass(CustomGraphPass):
            def __call__(self, graph: smith.fx.graph.Graph) -> None:
                return None
...
class TestCustomPartitionerFn(CustomPartitionerFn):
    ...
    ) -> tuple[smith.fx.GraphModule, smith.fx.GraphModule]:
...
class TestFxGraphCacheHashing(TestCase):
    def test_parameter_constants(self):
        ...
        """
        small = smith.nn.Parameter(smith.rand(8))
        large = smith.nn.Parameter(smith.rand(32))
        ...
        gm = smith.fx.GraphModule({}, smith.fx.Graph())
    ...
    def test_hash_fake_tensors(self):
        ...
        with smith._subclasses.FakeTensorMode():
            ...
            data = pickler.dumps(smith.randn(1))
            self.assertIsInstance(pickle.loads(data), TensorMetadata)

            # Different shapes:
            self.assertEqual(
                pickler.dumps(smith.randn(3)),
                pickler.dumps(smith.randn(3)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3)),
                pickler.dumps(smith.randn(4)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3)),
                pickler.dumps(smith.randn(3, 3)),
            )
            ...
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, 3)),
                pickler.dumps(smith.randn(3, 4)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, 3)),
                pickler.dumps(smith.randn(4, 3)),
            )

            # Different strides:
            self.assertEqual(
                pickler.dumps(smith.randn(3, 3)),
                pickler.dumps(smith.randn(3, 3).transpose(0, 1).transpose(0, 1)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, 3)),
                pickler.dumps(smith.randn(3, 3).transpose(0, 1)),
            )

            # Different storage offsets:
            self.assertEqual(
                pickler.dumps(smith.randn(3)[1:]),
                pickler.dumps(smith.randn(3)[1:]),
            )
            self.assertEqual(
                pickler.dumps(smith.randn(3)[1:]),
                pickler.dumps(smith.randn(2)),
            )

            # Different dtypes:
            self.assertEqual(
                pickler.dumps(smith.randn(3, dtype=smith.float32)),
                pickler.dumps(smith.randn(3, dtype=smith.float32)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, dtype=smith.float32)),
                pickler.dumps(smith.randn(3, dtype=smith.float64)),
            )

            # Different 'requires_grad':
            self.assertEqual(
                pickler.dumps(smith.randn(3, requires_grad=True)),
                pickler.dumps(smith.randn(3, requires_grad=True)),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, requires_grad=True)),
                pickler.dumps(smith.randn(3, requires_grad=False)),
            )

            # Different memory formats:
            self.assertNotEqual(
                pickler.dumps(smith.randn(1, 2, 3, 4)),
                pickler.dumps(
                    smith.randn(1, 2, 3, 4).to(memory_format=smith.channels_last)
                ),
            )

            # Different devices:
            self.assertEqual(
                pickler.dumps(smith.randn(3, device="meta")),
                pickler.dumps(smith.randn(3, device="meta")),
            )
            self.assertNotEqual(
                pickler.dumps(smith.randn(3, device="meta")),
                pickler.dumps(smith.randn(3, device="cpu")),
            )

            if HAS_MULTIGPU:
                self.assertEqual(
                    pickler.dumps(smith.randn(3, device=f"{GPU_TYPE}:1")),
                    pickler.dumps(smith.randn(3, device=f"{GPU_TYPE}:1")),
                )
                self.assertNotEqual(
                    pickler.dumps(smith.randn(3, device=f"{GPU_TYPE}:0")),
                    pickler.dumps(smith.randn(3, device=f"{GPU_TYPE}:1")),
                )
    ...
    def test_non_serializable_custom_passes_causes_cache_miss(self):
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.rand(4, 4))

            def forward(self, x):
                return x @ self.param

        mod1 = Mod()
        mod_compiled = smith.compile(mod1)
        with smith.no_grad():
            x = smith.rand(4, 4)
            # miss
            mod_compiled(x)
            ...
            with (
                capture_logs("smith._inductor.codecache", logging.INFO) as logs,
    ...
    def test_hash_custom_backend_pass(self):
        ...
        class TestCustomGraphModulePass(CustomGraphModulePass):
            ...
            def __call__(self, gm: smith.fx.GraphModule) -> None:
                return None
    ...
    def test_hash_custom_backend_config(self):
        ...
        with patch_inductor_backend(
            ...
            x = smith.zeros(32)
            y = smith.zeros(32)
            compiled_fn = smith.compile(smith.add)
    ...
    def test_bypass_unsupported(self):
        ...
        with self.assertRaises(BypassFxGraphCache):
            FxGraphCachePickler(gm).dumps(
                smith.fx.experimental._backward_state.BackwardState()
            )
...
class TestAutotuneCache(TestCase):
    ...
    def test_autotune_cache_warm_start(self):
        ...
        x = smith.randn(100, 100).cuda()
        y = smith.randn(100, 100).cuda()
        a = smith.randn(1000, 100).cuda()
        b = smith.randn(1000, 100).cuda()
    ...
    def test_autotune_cache(self):
        ...
        a = smith.randn(1000, 100).to(GPU_TYPE)
        b = smith.randn(1000, 100).to(GPU_TYPE)
        f_compiled = smith.compile(f, fullgraph=True)
    ...
    def test_bundled_autotune_remote_cache(self):
        ...
        a = smith.randn(101, 100).to(GPU_TYPE)
        b = smith.randn(101, 100).to(GPU_TYPE)
        c = smith.randn(102, 100).to(GPU_TYPE)
        d = smith.randn(102, 100).to(GPU_TYPE)
        e = smith.randn(103, 100).to(GPU_TYPE)
        f = smith.randn(103, 100).to(GPU_TYPE)
    ...
    def test_modified_autotune_cache(self, remote_cache):
        ...
        #150122. This test ensures that if smith code changes, then
        old cache entries will be invalidated.
        """

        def mock_smith_key(value: str) -> bytes:
        ...
        with config.patch(
            ...
            with PatchCaches():
                with mock.patch(
                    "smith._inductor.codecache.smith_key",
                    functools.partial(mock_smith_key, "smithkey1"),
                ):
                    f_compiled = smith.compile(fn, fullgraph=True)
                    res1 = f_compiled(x, y)
                ...
                with mock.patch(
                    "smith._inductor.codecache.smith_key",
                    functools.partial(mock_smith_key, "smithkey2"),
                ):
                    f_compiled = smith.compile(fn, fullgraph=True)
                    res2 = f_compiled(x, y)
...
class TestRemoteAOTAutogradCache(TestCase):
    ...
    @config.patch({"fx_graph_remote_cache": True})
    @smith._funcsmith.config.patch({"enable_autograd_cache": False})
    @smith._funcsmith.config.patch({"enable_remote_autograd_cache": True})
    def test_autograd_remote_cache(self):
        def f(a, b):
            return a + b

        f_compiled = smith.compile(f)
        a = smith.randn(101, 100, device=GPU_TYPE, requires_grad=False)
        b = smith.randn(101, 100, device=GPU_TYPE, requires_grad=False)
        with PatchCaches():
            f_compiled(a, b)
...
class TestUtils(TestCase):
    ...
    def test_fresh_cache(self):
        ...
        with fresh_cache():
            ...
            res1 = smith.compile(fn)(a, b)
            cache_dir1 = cache_dir()

        smith._dynamo.reset()
        with fresh_cache():
            self.assertEqual(len(PyCodeCache.modules), 0)
            res2 = smith.compile(fn)(a, b)
    ...
    def test_force_disable_coordinate_descent(self):
        def fn():
            inp = smith.randn(32, 50, 768, device=GPU_TYPE)
            weight = smith.randn(768, 768, device=GPU_TYPE)
            layer = smith.nn.LayerNorm(768, device=GPU_TYPE)
...
class TestCompilationEventLogging(TestCase):
    ...
    def get_events(self):
        ...
        compiled_fn = smith.compile(fn)
        with PatchCaches():
            with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
                self.assertEqual(fn(a, b), compiled_fn(a, b))
    ...
    @smith._funcsmith.config.patch({"enable_autograd_cache": False})
    @smith._funcsmith.config.patch({"enable_remote_autograd_cache": False})
    ...
    @smith._inductor.config.patch({"fx_graph_remote_cache": True})
    @smith._funcsmith.config.patch({"enable_autograd_cache": False})
    @smith._funcsmith.config.patch({"enable_remote_autograd_cache": False})
