...
class TestSplitCat(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(self, x: smith.Tensor, y: smith.Tensor, z: smith.Tensor):
        cat = smith.ops.aten.cat.default([x, y], 1)
        split = smith.ops.aten.split.Tensor(cat, 32, 1)
        getitem = split[0]
        getitem_1 = split[1]
        getitem_2 = split[2]
        getitem_3 = split[3]
        ...
        cat_2 = smith.ops.aten.cat.default([getitem, z], 1)
        return smith.ops.aten.cat.default([cat_1, cat_2], 1)


class TestSplitCatSingular(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
...
class TestSplitCatPartial(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(
        self, x1: smith.Tensor, x2: smith.Tensor, y: smith.Tensor, z: smith.Tensor
    ):
        ...
        split_with_sizes_2 = smith.ops.aten.split_with_sizes.default(
            x2, [96, 96, 96, 96], 1
        )
        ...
        cat = smith.ops.aten.cat.default(
            [
                z,
                getitem_71,
                getitem_72,
...
class TestMoveViewAferCat(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
    ...
    def forward(self, x: smith.Tensor):
        ...
        view_1 = smith.ops.aten.view.default(getitem_71, [8, 96])
        view_2 = smith.ops.aten.view.default(getitem_72, [8, 96])
        view_3 = smith.ops.aten.view.default(getitem_73, [8, 96])
        view_4 = smith.ops.aten.view.default(getitem_74, [8, 96])
        view_5 = smith.ops.aten.view.default(getitem_75, [8, 96])
        view_6 = smith.ops.aten.view.default(getitem_76, [8, 96])
        view_7 = smith.ops.aten.view.default(getitem_77, [8, 96])
        clone = smith.ops.aten.clone.default(view_1)
        ...
        cat_1 = smith.ops.aten.cat.default([clone, cat], 1)
        return smith.cat([clone, cat_1], 1)


class TestSelectCat(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(self, x: smith.Tensor, y: smith.Tensor):
        select = smith.ops.aten.select.int(x, 1, 0)
        select_1 = smith.ops.aten.select.int(x, 1, 1)
        select_2 = smith.ops.aten.select.int(x, 1, 2)
        select_3 = smith.ops.aten.select.int(x, 1, 3)
        select_4 = smith.ops.aten.select.int(x, 1, 4)
        select_5 = smith.ops.aten.select.int(x, 1, 5)
        cat = smith.ops.aten.cat.default(
            [select, select_1, select_2, select_3, select_4, select_5], 1
        )
        cat1 = smith.ops.aten.cat.default(
            [select, select_1, select_2, select_3, select_4], 1
        )
        cat2 = smith.ops.aten.cat.default([select, select_2, select_4], 1)
        select_6 = smith.ops.aten.select.int(y, 1, 0)
        select_7 = smith.ops.aten.select.int(y, 1, 1)
        select_8 = smith.ops.aten.select.int(y, 1, 2)
        select_9 = smith.ops.aten.select.int(y, 1, 3)
        select_10 = smith.ops.aten.select.int(y, 1, 4)
        cat3 = smith.ops.aten.cat.default(
...
class TestSplitCatAten(TestCase):
    ...
    def test_split_cat_post_grad(self):
        counters.clear()
        inputs = [
            smith.randn(1024, 128, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 128, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 32, device=smith.device(device=GPU_TYPE)),
        ]
        module = TestSplitCat()
        ...
        inputs = [
            smith.randn(1024, 96 * 21, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 96 * 4, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 96, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 96, device=smith.device(device=GPU_TYPE)),
        ]
    ...
    def test_select_cat_post_grad(self):
        counters.clear()
        inputs = [
            smith.randn(1024, 6, 128, device=smith.device(device=GPU_TYPE)),
            smith.randn(1024, 6, 128, device=smith.device(device=GPU_TYPE)),
        ]
    ...
    def test_move_view_after_cat_aten(self):
        counters.clear()
        inputs = [
            smith.randn(7, 8, 96, device=smith.device(device=GPU_TYPE)),
        ]
...
class TestSplitCatAtenNormalizationPasses(TestCase):
    ...
    def test_split_aten_normalization(self):
        def arg_only_size_same(x):
            return smith.ops.aten.split.Tensor(x, 300, 1)

        def arg_only_size_different(x):
            return smith.ops.aten.split.Tensor(x, 320, 1)

        args = [
            smith.randn(4096, 300),
        ]
