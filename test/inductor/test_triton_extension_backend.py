...
from smith._dynamo import device_interface
from smith._inductor import codegen, ir, metrics
from smith._inductor.codegen import common
...
from smith._inductor.utils import get_triton_code, run_and_get_triton_code
from smith.testing._internal.common_utils import IS_FBCODE, IS_MACOS
...
class TritonExtensionBackendTests(BaseExtensionBackendTests):
    ...
    def test_open_device_registration(self):
        smith._register_device_module("privateuseone", self.module)
        register_backend_for_device(
        ...
        device = smith.device("privateuseone")
        x = smith.empty(2, 16).fill_(1).to(device)

        def foo(x):
            return smith.sin(x) + x.min()

        metrics.reset()
        opt_fn = smith.compile(foo)

        # Since we don't have a triton backend, we need to mock the triton_hash_with_backend
        # function
        with unittest.mock.patch(
            "smith.utils._triton.triton_hash_with_backend",
    ...
    def test_codegen_with_custom_heuristics_module(self):
        ...
        x = smith.zeros((32,), device=GPU_TYPE)
        y = x
        compiled_add = smith.compile(add)
    ...
    def test_codegen_with_custom_heuristics_module_udtk(self):
        ...
        args = [smith.randn(32, device=GPU_TYPE) for _ in range(2)]
        code = run_and_get_triton_code(smith.compile(add), *args)
