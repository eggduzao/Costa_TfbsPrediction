...
from smith._inductor.kernel_inputs import MMKernelInputs
from smith._inductor.lookup_table.choices import LookupTableChoices
...
from smith._inductor.utils import fresh_cache, get_num_sms, TMA_DESCRIPTOR_SIZE
from smith._inductor.virtualized import V
from smith.testing._internal.common_utils import (
    instantiate_parametrized_tests,
    parametrize,
)
from smith.testing._internal.inductor_utils import HAS_CPU, HAS_CUDA_AND_TRITON, HAS_GPU
from smith.utils._triton import has_triton_stable_tma_api, has_triton_tma_device
...
class BaseLookupTableTest(TestCase):
    ...
    def tearDown(self):
        ...
        V.set_choices_handler(Inducsmithoices())
        super().tearDown()

    def create_mock_mm_kernel_inputs(
        self,
        shapes: Optional[list[tuple[int, ...]]] = None,
        device: smith.device = smith.device("cuda"),
        ...
        for shape in shapes:
            ...
            tensor = smith.randn(shape, device=device, dtype=dtype)
...
class TestLookupTable(BaseLookupTableTest):
    ...
    def test_cpu_input_returns_empty(self):
        ...
        kernel_inputs = self.create_mock_mm_kernel_inputs(device=smith.device("cpu"))
...
class UnifiedModel(nn.Module):
    ...
    def forward(self, *args):
        if self.operation == "mm":
            return smith.mm(args[0], args[1])
        elif self.operation == "addmm":
            return smith.addmm(args[0], args[1], args[2])
        elif self.operation == "bmm":
            return smith.bmm(args[0], args[1])
        elif self.operation == "mm_plus_mm":
            return smith.mm(args[0], args[1]) + smith.mm(args[2], args[3])
        else:
...
class BaseE2ELookupTableTest(BaseLookupTableTest):
    ...
    def create_tensors(self, operation, b=8, m=64, n=64, k=32):
        ...
        if operation in ["mm", "addmm", "mm_plus_mm"]:
            A = smith.randn(m, k, device=self.device, dtype=smith.float16)
            B = smith.randn(k, n, device=self.device, dtype=smith.float16)
            if operation == "mm":
                return [A, B]
            if operation == "addmm":
                return [
                    smith.randn((m, n), device=self.device, dtype=smith.float16),
                    A,
                    B,
                ]
                ...
                return [
                    A,
                    B,
                    smith.randn(m, k, device=self.device, dtype=smith.float16),
                    smith.randn(k, n, device=self.device, dtype=smith.float16),
                ]
        elif operation == "bmm":
            return [
                smith.randn(b, m, k, device=self.device, dtype=smith.float16),
                smith.randn(b, k, n, device=self.device, dtype=smith.float16),
            ]
        else:
    ...
    def create_basic_config(self, template_id):
        ...
        configs = {
            ...
            smith._inductor.kernel.mm_plus_mm.mm_plus_mm_template.uid: {
                "BLOCK_M": 64,
                "BLOCK_N": 64,
                "BLOCK_K": 32,
                "num_stages": 2,
            ...
            smith._inductor.kernel.bmm.bmm_template.uid: {
                "BLOCK_M": 64,
                "BLOCK_N": 64,
                "BLOCK_K": 64,
                "num_stages": 2,
            ...
            smith._inductor.kernel.mm.persistent_tma_mm_template.uid: {
                "BLOCK_M": 64,
                "BLOCK_N": 64,
                "BLOCK_K": 32,
                "num_stages": 2,
            ...
            smith._inductor.kernel.mm.aten_bias_addmm.uid: {},
            smith._inductor.kernel.mm.decompose_k_subgraph_template.uid: {"k_split": 4},
        }
    ...
    def _create_test_inputs(self, device="cuda"):
        ...
        return [
            smith.randn(512, 512, device=device, dtype=smith.float32),
            smith.randn(512, 512, device=device, dtype=smith.float32),
        ]
...
class TestLookupTableE2E(BaseE2ELookupTableTest):
    ...
    def test_no_lookup_table_entry_autotune_modes(self, max_autotune):
        ...
        self.setup_lookup_table(
            "mm",
            [
                smith.randn(64, 64, device=self.device),
                smith.randn(64, 64, device=self.device),
            ],
            [],
        )
    ...
    def test_valid_lookup_table_entry(self, operation):
        ...
        template_mapping = {
            "mm": smith._inductor.kernel.mm.mm_template.uid,
            "addmm": smith._inductor.kernel.mm.mm_template.uid,
            "bmm": smith._inductor.kernel.bmm.bmm_template.uid,
            "mm_plus_mm": smith._inductor.kernel.mm_plus_mm.mm_plus_mm_template.uid,
        }
    ...
    def test_decompose_k_lookup_table_entry(self):
        ...
        with inductor_config.patch(_DECOMPOSE_K_PATCH_ROCM):
            ...
            config = self.create_basic_config(
                smith._inductor.kernel.mm.decompose_k_subgraph_template.uid
            )
    ...
    def test_bias_addmm_lookup_table_entry(self):
        ...
        bias_unexpanded = smith.randn(64, device=self.device, dtype=smith.float16)
        expanded_bias = bias_unexpanded.expand(64, 64)
        tensors = [
            expanded_bias,
            smith.randn(64, 32, device=self.device, dtype=smith.float16),
            smith.randn(32, 64, device=self.device, dtype=smith.float16),
        ]

        config = self.create_basic_config(smith._inductor.kernel.mm.aten_bias_addmm.uid)
        ...
        with inductor_config.patch(
            ...
            compiled_model = smith.compile(model.to(self.device), mode="max-autotune")
    ...
    def test_template_hash_filtering_e2e(self):
        ...
        actual_hash = smith._inductor.kernel.mm.mm_template.src_hash
