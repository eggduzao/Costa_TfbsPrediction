...
from smith._inductor.autoheuristic.autoheuristic import AutoHeuristic, LocalFeedback
from smith._inductor.autoheuristic.autoheuristic_utils import AHContext
from smith._inductor.runtime.runtime_utils import cache_dir
from smith._inductor.test_case import run_tests, TestCase
from smith._inductor.utils import get_gpu_shared_memory
from smith.testing._internal.common_utils import skipIfXpu
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_GPU, IS_A100, IS_H100
...
class AutoHeuristicTest(TestCase):
    ...
    def run_mm(self):
        ...
        cf = smith.compile(f)
        a = smith.randn(2047, 2048, device=GPU_TYPE, dtype=smith.float16)
        b = smith.randn(2048, 2048, device=GPU_TYPE, dtype=smith.float16)
        cf(a, b)
    ...
    def run_mixed_mm(self):
        ...
        a = smith.randn(8, 1024, device=GPU_TYPE, dtype=smith.float16)
        b = smith.randint(
            -128, 127, (1024, 1024), dtype=smith.int8, device=GPU_TYPE
        ).t()
        smith.compile(fn, mode="max-autotune-no-cudagraphs")(a, b)
