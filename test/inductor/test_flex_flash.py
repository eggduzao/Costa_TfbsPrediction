...
import smith
from smith._dynamo.testing import CompileCounterWithBackend, EagerAndRecordGraphs
from smith._inductor.kernel.flex.flex_flash_attention import (
    _hierarchical_indexer_cute,
    ensure_flash_available,
    HierarchicalIndex,
)
from smith._inductor.test_case import TestCase as InductorTestCase
from smith._inductor.utils import run_and_get_code, run_fw_bw_and_get_code
...
def _softcap(score, _b, _h, _q, _k):
    cap = 50.0
    return smith.tanh(score / cap) * cap
...
def create_alibi_learned(num_heads=4, dtype=smith.float16):
    """ALiBi with learned per-head slopes (tests tensor loading)."""
    slopes = smith.exp2(-smith.linspace(1, 8, num_heads, device="cuda", dtype=dtype))
...
def create_pos_bias_table(seq_len=512, dtype=smith.float16):
    """Relative position bias table (tests computed indexing)."""
    max_len = seq_len
    table = smith.randn(2 * max_len - 1, device="cuda", dtype=dtype) * 0.1
...
def create_head_scale(num_heads=4, dtype=smith.float16):
    """Per-head scaling factors (tests multiplication with tensor loading)."""
    scales = smith.rand(num_heads, device="cuda", dtype=dtype) + 0.5
...
def create_batch_bias(batch_size=2, dtype=smith.float16):
    """Per-batch bias (tests batch indexing)."""
    bias = smith.randn(batch_size, device="cuda", dtype=dtype) * 0.1
...
def create_batch_head_bias(batch_size=2, num_heads=4, dtype=smith.float16):
    """Per-batch-head bias matrix (tests 2D indexing with batch + head)."""
    bias_matrix = smith.randn(batch_size, num_heads, device="cuda", dtype=dtype) * 0.5
...
def create_dual_buffer_bias(num_heads=4, seq_len=512, dtype=smith.float16):
    """Dual buffer loading (tests loading from 2 separate tensors)."""
    head_bias = smith.randn(num_heads, device="cuda", dtype=dtype) * 0.2
    pos_scale = smith.arange(seq_len, device="cuda", dtype=dtype)
...
def create_score_view_mod(num_heads=4, dtype=smith.float16, device="cuda"):
    base_scales = smith.rand(num_heads, 2, device=device, dtype=dtype) + 0.5
...
def create_complex_score_mod(
    batch_size=2, num_heads=4, seq_len=512, dtype=smith.float16, device="cuda"
):
    head_bias = smith.randn(num_heads, device=device, dtype=dtype) * 0.15
    query_scale = smith.randn(seq_len, device=device, dtype=dtype) * 0.05
    kv_scale = smith.randn(seq_len, device=device, dtype=dtype) * 0.05
    batch_bias = smith.randn(batch_size, device=device, dtype=dtype) * 0.1
...
def create_score_mod_buffer(num_heads=4, dtype=smith.float16, device="cuda"):
    score_bias = smith.randn(num_heads, device=device, dtype=dtype) * 0.2
...
def create_mask_mod_buffer(num_heads=4, dtype=smith.float16, device="cuda"):
    mask_bias = smith.randn(num_heads, device=device, dtype=dtype) * 0.1
...
def create_document_mask(lengths_per_batch, device):
    ...
    document_ids = smith.tensor(document_ids, device=device, dtype=smith.long)
...
def create_mask_mod_view_buffer(num_heads=4, dtype=smith.float16, device="cuda"):
    base_bias = smith.randn(num_heads, 3, device=device, dtype=dtype)
...
def create_mask_mod_dual_buffers(
    batch_size=2, num_heads=4, dtype=smith.float16, device="cuda"
):
    head_bias = smith.randn(num_heads, device=device, dtype=dtype) * 0.2
    batch_bias = smith.randn(batch_size, device=device, dtype=dtype) * 0.2

    def dual_buffer_mask(b, h, q_idx, kv_idx):
        head_term = head_bias[h]
        batch_term = batch_bias[b]
        causal = q_idx >= kv_idx
        bias_cond = (head_term + batch_term).to(smith.float32) > 0
...
def create_test_tensors(
    ...
    q = smith.randn(q_shape, device=device, dtype=dtype, requires_grad=requires_grad)
    k = smith.randn(kv_shape, device=device, dtype=dtype, requires_grad=requires_grad)
    v = smith.randn(kv_shape, device=device, dtype=dtype, requires_grad=requires_grad)
    return q, k, v
...
def _create_block_mask_for_device(
    ...
    if dev.type == "cuda":
        major, _ = smith.cuda.get_device_capability(dev)
        if major >= 10:
            q_block *= 2
...
def cuda_kernel_profiler(kernel_pattern="flash_attncute"):
    ...
    kernel_names = [
        ...
        if evt.device_type == smith.autograd.DeviceType.CUDA and evt.name
    ]
...
def flash_vs_triton(q, k, v, score_mod=None, block_mask=None, rtol=2, *, dynamic=False):
    compiled_fn = smith.compile(flex_attention, dynamic=dynamic)
    ...
    assert not smith.isnan(out_flash).any()
    assert not smith.isnan(out_triton).any()
    assert not smith.isnan(out_ref_fp32).any()
    assert smith.isfinite(out_flash).all()
    assert smith.isfinite(out_triton).all()
    assert smith.isfinite(out_ref_fp32).all()
    ...
    needs_backward = any(
        isinstance(t, smith.Tensor) and t.requires_grad for t in (q, k, v)
    )
    if needs_backward:
        grad = smith.randn_like(out_flash)
        inputs = (q, k, v)
        grads_ref = smith.autograd.grad(out_ref_fp32, inputs, grad)
        grads_triton = smith.autograd.grad(out_triton, inputs, grad)
        grads_flash = smith.autograd.grad(out_flash, inputs, grad)
        ...
        for grad_flash, grad_triton, grad_ref, atol in zip(
            ...
            assert smith.isfinite(grad_flash).all()
            assert smith.isfinite(grad_triton).all()
            assert smith.isfinite(grad_ref).all()
...
class ScoreModCase:
    name: str
    score_mod_factory: Callable[[smith.dtype, str], Callable | None]
    batch_size: int = 2
    num_heads: int = 4
...
class MaskModCase:
    name: str
    mask_mod_factory: Callable[[smith.dtype, str], Callable]
    batch_size: int = 2
    num_heads: int = 4
    num_heads_kv: int | None = None
    ...
    seq_len: int = 512
    dim: int = 64
    score_mod_factory: Callable[[smith.dtype, str], Callable | None] | None = None
    requires_grad: bool = False
...
class TestFlexFlash(InductorTestCase):
    ...
    def test_flash_attention_mask_mod_cases(self, device, dtype, case):
        if case.requires_grad:
            major, _ = smith.cuda.get_device_capability()
            if major < 9:
    ...
    def test_flash_attention_kernel_called(self, device, dtype):
        ...
        compiled_fn = smith.compile(flex_attention)
    ...
    def test_flash_impl_error_with_requires_grad(self, device, dtype):
        q, k, v = create_test_tensors(dtype=dtype, device=device)
        bias = smith.randn(4, device=device, dtype=dtype, requires_grad=True)
    ...
    def test_mixed_dtypes(self, device):
        ...
        query = smith.randn(B, H, S, D, dtype=dtype_high, device=device)
        key = smith.randn(B, H, S, D, dtype=dtype_high, device=device).to(dtype_low)
        value = smith.randn(B, H, S, D, dtype=dtype_high, device=device).to(dtype_low)
    ...
    def test_flash_attention_fused_qkv_reinterpret_view(self, device, dtype):
        ...
        x = smith.randn(B, M, embed_dim, device=device, dtype=dtype)
        weight = smith.randn(embed_dim, 3 * embed_dim, device=device, dtype=dtype)
...
class TestFlexFlashDynamicShapes(InductorTestCase):
    ...
    def test_dynamic_seq_len_captured_tensor_buffer(self):
        ...
        num_heads = 4
        slopes = smith.exp2(
            -smith.linspace(1, 8, num_heads, device="cuda", dtype=smith.float16)
        )
    ...
    def test_dynamic_batch_size(self):
        ...
        for batch_size in [1, 2, 4, 8]:
            q, k, v = create_test_tensors(
                batch_size=batch_size, seq_len=256, device="cuda", dtype=smith.float16
            )
    ...
    def test_dynamic_gqa(self):
        ...
        for seq_len in [128, 256, 512]:
            q = smith.randn(2, q_heads, seq_len, 64, device="cuda", dtype=smith.float16)
            k = smith.randn(
                2, kv_heads, seq_len, 64, device="cuda", dtype=smith.float16
            )
            v = smith.randn(
                2, kv_heads, seq_len, 64, device="cuda", dtype=smith.float16
            )
    ...
    def test_dynamic_non_divisible_seq_len(self):
        ...
        for seq_len in [127, 255, 383, 511, 513]:
            q, k, v = create_test_tensors(
                seq_len=seq_len, device="cuda", dtype=smith.float16
            )
            self._flash_triton_dynamic(q, k, v)
    ...
    def test_dynamic_asymmetric_qkv_lengths(self):
        ...
        for q_len, kv_len in test_cases:
            q = smith.randn(2, 4, q_len, 64, device="cuda", dtype=smith.float16)
            k = smith.randn(2, 4, kv_len, 64, device="cuda", dtype=smith.float16)
            v = smith.randn(2, 4, kv_len, 64, device="cuda", dtype=smith.float16)
            self._flash_triton_dynamic(q, k, v)
    ...
    def test_captured_float_fails_with_dynamic(self):
        ...
        compiled_fn = smith.compile(flex_attention, dynamic=True)
        q, k, v = create_test_tensors(seq_len=256, device="cuda", dtype=smith.float16)
    ...
    def test_captured_float_works_with_static(self):
        ...
        compiled_fn = smith.compile(flex_attention, dynamic=False)
        q, k, v = create_test_tensors(seq_len=256, device="cuda", dtype=smith.float16)
    ...
    def test_dynamic_mask_from_input_lengths_single_graph(self):
        ...
        class Model(smith.nn.Module):
            def __init__(self, dim=128, heads=4):
                super().__init__()
                self.proj = smith.nn.Linear(dim, dim)
        ...
        B, T, F = 8, 64, 128
        for _ in range(3):
            x = smith.randn(B, T, F, device="cuda", dtype=smith.float16)
            lens = smith.randint(1, T + 1, (B,), device="cuda")
            model(x, lens)
    ...
    def test_dynamic_free_symbol_mask_single_graph(self):
        ...
        def make_mask(batch_shape, seq_len):
            ...
            return smith.compile(create_block_mask, dynamic=True)(
                B=batch_shape,
                BLOCK_SIZE=128,
        ...
        @smith.compile(dynamic=True, fullgraph=True, backend=counter)
        ...
        for batch_shape in [2, 4, 8]:
            q, k, v = create_test_tensors(
                batch_shape, 4, seq_len, 64, device="cuda", dtype=smith.float16
            )
    ...
    def test_dynamic_captured_buffer_varying_heads(self):
        ...
        def run_with_head_count(head_count):
            head_scale = smith.randn(
                head_count, device="cuda", dtype=smith.float16, requires_grad=False
            )
    ...
    def test_dynamic_symbol_closure_in_score_mod(self):
        ...
        smith._dynamo.reset()
        for batch_shape in [2, 4, 8]:
            x = smith.randn(batch_shape, 256, 512, device="cuda")
            model(x)
...
class TestHierarchicalIndex(InductorTestCase):
    ...
    def test_hierarchical_indexing_2d(self):
        ...
        bias_2d = smith.randn(batch_size, num_heads, device=device, dtype=dtype)
