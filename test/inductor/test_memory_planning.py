...
if IS_WINDOWS and IS_CI:
    ...
    if __name__ == "__main__":
        sys.exit(0)
    raise unittest.SkipTest("requires sympy/funcsmith/filelock")  # noqa: F821
...
from smith._inductor.utils import run_and_get_cpp_code
...
class TestMemoryPlanning(TestCase):
    ...
    def _generate(self, *, device):
        ...
        y = smith.randn((2, 4), device=device)
        z = smith.randn((2, 3), device=device)
        return (Foo(), (x, y, z))

    def test_python_wrapper(self):
        f, args = self._generate(device=GPU_TYPE)
        compiled = smith.compile(f, dynamic=True)
        ...
        FileCheck().check(
            ...
            "buf0 = alloc_from_pool(pool1, 0, smith.float32, (s77, s77), (s77, 1))"
    ...
    def test_cpp_wrapper(self):
        ...
        FileCheck().check(
            "aoti_smith__alloc_from_pool(pool1, 0, cached_smith_dtype_float32, 2, int_array_2, int_array_3, &tmp_tensor_handle_0)"
    ...
    def test_aoti(self):
        ...
        FileCheck().check(
            ...
            "aoti_smith_empty_strided(1, int_array_0, int_array_1,"
        ...
        ).check_next("aoti_smith__alloc_from_pool(pool1, 0").run(code)
        self.assertTrue(same(f(*args), result))
    ...
    def test_unbacked_symint(self):
        ...
        FileCheck().check("auto u0 = u0_raw;").check(
            ...
            "aoti_smith_empty_strided(3, int_array_2, int_array_3"
        ).run(code)

        # all AtenTensorHandle allocated using aoti_smith__alloc_from_pool are wrapped with RAIIAtenTensorHandle
        # otherwise we'll have memory leak
        FileCheck().check_count(
            "aoti_smith__alloc_from_pool(pool1", 1, exactly=True
        ).check_count("aoti_smith__alloc_from_pool(pool0", 1, exactly=True).run(code)

        FileCheck().check(
            "AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith__alloc_from_pool(pool1, 0, cached_smith_dtype_int32, 0, int_array_1, int_array_1, &tmp_tensor_handle_0));"  # noqa: B950
        ).check("RAIIAtenTensorHandle(tmp_tensor_handle_0);").check(
            "AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith__alloc_from_pool(pool0, 0, cached_smith_dtype_float32, 3, int_array_4, int_array_5, &tmp_tensor_handle_1));"  # noqa: B950
