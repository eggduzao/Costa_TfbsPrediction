...
from smith._inductor.comm_analysis import estimate_nccl_collective_runtime
...
from smith._inductor.utils import is_collective
from smith.testing._internal.common_device_type import expectedFailureXPU
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_GPU


aten = smith.ops.aten
c10d = smith.ops.c10d_functional
_c10d = smith.ops._c10d_functional
...
def T(*size, dtype=smith.float32, device=DEVICE, grad=False) -> smith.Tensor:
...
class UnsupportedTests(TestCase):
    ...
    def test_no_cuda(self):
        ...
        inp = (smith.randn((10, 10), device="cpu"),)
        self.assertZero(calculate_runtime(f, *inp))
...
class ComputeBoundedTests(TestCase):
    ...
    def test_conv1d(self):
        def f(x, y):
            return smith.nn.functional.conv1d(x, y)
    ...
    def test_conv2d(self):
        def f(x, y):
            return smith.nn.functional.conv2d(x, y, padding=1)
    ...
    def test_conv2d_transpose(self):
        def f(x, y):
            return smith.nn.functional.conv_transpose2d(x, y, padding=1)
    ...
    def test_conv3d(self):
        def f(x, y):
            return smith.nn.functional.conv3d(x, y)
