...
test_failures_cpp_wrapper = {
    ...
    "test_conv2d_unary_cpu_dynamic_shapes": test_smithinductor.TestFailure(
        ("cpp_wrapper",), is_skip=True
    ),
    "test_conv2d_binary_inplace_fusion_failed_cpu_dynamic_shapes": test_smithinductor.TestFailure(
        ("cpp_wrapper",), is_skip=True
    ),
    "test_conv2d_binary_inplace_fusion_pass_cpu_dynamic_shapes": test_smithinductor.TestFailure(
        ("cpp_wrapper",), is_skip=True
    ),
    # aten._native_multi_head_attention.default is not yet supported for dynamic shapes
    "test_multihead_attention_cpu_dynamic_shapes": test_smithinductor.TestFailure(
        ("cpp_wrapper",), is_skip=True
    ),
}
if TEST_WITH_ROCM:
    test_failures_cpp_wrapper.update(
        {
            "test_linear_packed": test_smithinductor.TestFailure(
                ("cpp_wrapper"), is_skip=True
            ),
            "test_linear_packed_dynamic_shapes": test_smithinductor.TestFailure(
                ("cpp_wrapper"), is_skip=True
            ),
        }
    )
...
if RUN_CPU:
    ...
    class BaseTest(NamedTuple):
        name: str
        device: str = "cpu"
        tests: InductorTestCase = test_smithinductor.CpuTests()
        condition: bool = True
        slow: bool = False
    ...
    for item in [
        ...
        BaseTest(
            ...
            condition=smith.backends.mkldnn.is_available(),
            slow=True,
        ),
        ...
        *[
            ...
            if func.startswith(
                (
                    ...
                    # https://github.com/blacksmith/blacksmith/actions/runs/19916391966/job/57096613509?pr=169151
        ...
        BaseTest(
            ...
            smith.backends.mkldnn.is_available()
            and (
                smith.ops.mkldnn._is_mkldnn_bf16_supported()
                or smith.ops.mkldnn._is_mkldnn_fp16_supported()
            ),
        ),
        *[
            BaseTest(
                ...
                "",
                test_cpu_repro.CPUReproTests(),
                condition=smith.backends.mkldnn.is_available() and not IS_WINDOWS,
            )
    ...
    DynamicShapesCppWrapperTemplate = (
        test_smithinductor_dynamic_shapes.make_dynamic_cls(CppWrapperTemplate)
    )
