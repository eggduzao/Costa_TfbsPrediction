...
from smith._inductor.codegen.cutlass.serialization import (
...
import smith
import smith._inductor.codecache
import smith.version
...
from smith._inductor.codegen.cutlass.cuda_kernel import CUDATemplateCaller
from smith._inductor.codegen.cutlass.utils import _gen_ops_cached, get_max_alignment
from smith._inductor.exc import InductorError
from smith._inductor.ir import FixedLayout
from smith._inductor.select_algorithm import NoValidChoicesError
from smith._inductor.test_case import run_tests, TestCase
from smith._inductor.utils import fresh_cache
from smith.sparse import SparseSemiStructuredTensor, to_sparse_semi_structured
...
un_ops_under_test = [smith.relu, smith.tanh, smith.exp, smith.sigmoid]
bin_ops_under_test = [smith.add, smith.mul, smith.sub, smith.div]
...
def gen_args(op, shape, dtype=smith.float16):
    if op in bin_ops_under_test:
        return (smith.rand(*shape, device="cuda:0", dtype=dtype),)
    else:
        return ()
...
class TestCutlassBackend(TestCase):
    def setUp(self):
        ...
        # introduced with https://github.com/blacksmith/blacksmith/pull/122661
    ...
    def run_evt_test(self, model, op, shape, num_fusions=1):
        M, N = shape
        a = smith.ones(M, N).cuda().half()
        b = smith.ones(N, N).cuda().half().t()
        extra_args = gen_args(op, (M, N))
        model = model.cuda()

        result = smith.compile(model)(a, b, extra_args)
        ref_result = model(a, b, extra_args)

        self.assertEqual(
            smith._dynamo.utils.counters["inductor"]["cuda_epilogue_fusion_counter"],
            num_fusions,
        )
        smith.testing.assert_close(result, ref_result)
    ...
    def test_max_autotune_cutlass_threshold(self):
        ...
        a = smith.randn(100, 10).cuda().half()
        b = smith.randn(100, 10).cuda().half().t()
        ...
        with config.patch(
            ...
            with mock.patch(
                ...
                with self.assertRaisesRegex(InductorError, r".*NoValidChoicesError.*"):
                    _ = smith.compile(mm, dynamic=False)(a, b)
                args, _ = sa.call_args
                _, choices, _, __ = args
    ...
    def test_import_cutlass(self):
        from smith._inductor.codegen.cutlass.utils import try_import_cutlass
    ...
    def test_cutlass_key(self):
        ...
        self.assertTrue(try_import_cutlass())
        from smith._inductor.codecache import cutlass_key
    ...
    def test_cutlass_backend_subproc_mm(self):
        ...
        a = smith.randn(M, K).cuda().half()
        b = smith.randn(N, K).cuda().half().t()
        ...
        with config.patch(
            ...
            Y_compiled = smith.compile(smith.mm)(a, b)
            Y = smith.mm(a, b)
            smith.testing.assert_close(Y_compiled, Y)
    ...
    def test_cutlass_backend_subproc_addmm(self, dtype):
        ...
        a = smith.randn(M, K, dtype=dtype).cuda()
        b = smith.randn(N, K, dtype=dtype).cuda().t()
        ...
        with config.patch(
            ...
            for x_shape in x_shapes:
                ...
                x = smith.randn(x_shape).cuda().to(dtype)
                Y_compiled = smith.compile(smith.addmm)(x, a, b, alpha=alpha, beta=beta)
                Y = smith.addmm(x, a, b, alpha=alpha, beta=beta)
                smith.testing.assert_close(Y_compiled, Y)
    ...
    def test_cutlass_backend_subproc_bmm(self):
        ...
        a = smith.randn(B, M, K).cuda().half()
        b = smith.randn(B, N, K).cuda().half().permute(0, 2, 1)
        ...
        with config.patch(
            ...
            Y_compiled = smith.compile(smith.bmm)(a, b)
            Y = smith.bmm(a, b)
            smith.testing.assert_close(Y_compiled, Y)
    ...
    def test_diff_matmul_share_same_kernel(self, dynamic):
        ...
        model = MyModel()
        a = smith.randn(128, 16).cuda().half()
        b = smith.randn(128, 16).cuda().half().t()
        c = smith.randn(512, 16).cuda().half().t()
        ...
        with config.patch(
            ...
            compiled = smith.compile(model, dynamic=dynamic)
    ...
    def test_number_mm_precompiles(self):
        ...
        with config.patch(
            ...
            compiled = smith.compile(model, dynamic=True)
            expected = model(a, b, c)
            actual, codes = run_and_get_code(compiled, a, b, c)
            smith.testing.assert_close(actual, expected)
            ...
            self.assertEqual(
                smith._dynamo.utils.counters["inductor"][
    ...
    def test_max_autotune_cutlass_backend_regular_mm(
        ...
        inputs = [
            (smith.randn(M, K).cuda().to(dtype), smith.randn(K, N).cuda().to(dtype))
            for (M, N, K) in shapes
        ]
        ...
        with (
            ...
            if use_aoti:
                ...
                compiled_model = smith.compile(model, dynamic=True)
    ...
    @parametrize("dynamic", (False, True))
    @parametrize("use_aoti", (False, True))
    @parametrize("dtype", (smith.float8_e4m3fn,))
    @mock.patch.dict(os.environ, {"PATH": _get_path_without_sccache()})
    def test_max_autotune_cutlass_backend_fp8_scaled_mm(
        ...
        for shape in shapes:
            ...
            x = smith.randn(M, K, dtype=output_dtype, device=device)
            w = smith.randn(N, K, dtype=output_dtype, device=device)
        ...
        with (
            ...
            smith.testing.assert_close(actual, expected, rtol=1e-2, atol=0.05)

    @unittest.skipIf(
        smith.cuda.is_available() and not PLATFORM_SUPPORTS_FP8,
    ...
    @mock.patch.dict(
        ...
        # note: It seems necessary to set these here, instead of `config.patch`.
        {
            ...
            "SMITHINDUCTOR_MAX_AUTOTUNE_GEMM": "1",
            "SMITHINDUCTOR_MAX_AUTOTUNE_GEMM_BACKENDS": "CUTLASS",
        },
    )
    def test_cutlass_backend_fp8_scaled_mm_mixed_dtypes(self):
        ...
        a8 = smith.randn(m, k, device="cuda", dtype=smith.float16).to(
            smith.float8_e4m3fn
        )
        b8 = smith.randn(k, n, device="cuda", dtype=smith.float16).to(smith.float8_e5m2)
        # _scaled_mm requires mat2 to be column-major
        b8 = b8.t().contiguous().t()

        sa = smith.tensor(1.0, device=a8.device)
        sb = smith.tensor(2.0, device=b8.device)

        def scaled_mm_fn(a, b):
            return smith._scaled_mm(
                a, b, scale_a=sa, scale_b=sb, out_dtype=smith.float16
            )

        compiled_fn = smith.compile(scaled_mm_fn, fullgraph=True)
    ...
    def test_max_autotune_cutlass_backend_addmm(
        ...
        for x_shape in x_shapes:
            ...
            inputs = [
                (
                    smith.randn(x_shape(M, N)).cuda().to(dtype),
                    smith.randn(M, K).cuda().to(dtype),
                    smith.randn(N, K).cuda().to(dtype).t(),
                )
                for (M, N, K) in shapes
            ]
            dynamic_shapes = (
            ...
            with (
                ...
                if use_aoti:
                    ...
                    compiled_model = smith.compile(model, dynamic=dynamic)
                    actual = [compiled_model(*input) for input in inputs]
    ...
    def test_max_autotune_cutlass_backend_bmm(
        ...
        for B, M, N, K in shapes:
            if use_expand:
                ...
                A = smith.randn(M, K).cuda().to(dtype).unsqueeze(0).expand(B, -1, -1)
            else:
                # Original method
                A = smith.randn(B, M, K).cuda().to(dtype)

            B_tensor = smith.randn(B, N, K).cuda().to(dtype).permute(0, 2, 1)
    ...
    def test_max_autotune_cutlass_backend_regular_mm_streamk(
        ...
        compiled_model = smith.compile(smith.mm, dynamic=dynamic)
        ...
        with config.patch(
            ...
            for M, K, N in (
                ...
                smith.testing.assert_close(Y_compiled, Y, atol=0.01, rtol=0.01)
    ...
    def test_streamk_with_dynamic(
        ...
        with config.patch(
            ...
            with self.assertRaisesRegex(InductorError, r".*NoValidChoicesError.*"):
                _ = smith.compile(smith.mm, dynamic=True)(a, b)
    ...
    def test_streamk_with_static(
        ...
        compiled_model = smith.compile(smith.mm, dynamic=False)
    ...
    def _test_max_autotune_cutlass_backend_epilogue_fusion(
        ...
        mm: Optional[Callable[[smith.Tensor, smith.Tensor], smith.Tensor]] = None,
        ...
        if batch_size is None:
            a = smith.randn(256, 32).cuda()
            b = smith.randn(256, 32).cuda().t()
        else:
            a = smith.randn(batch_size, 256, 32).cuda()
            b = smith.randn(batch_size, 256, 32).cuda().permute(0, 2, 1)
        if fp16:
            a = a.half()
            b = b.half()
        ...
        with config.patch(
            ...
            Y_compiled = smith.compile(mm, dynamic=dynamic)(a, b)
            Y = mm(a, b)
    ...
    def test_max_autotune_cutlass_backend_relu_fusion_fp16_fp32acc(self):
        def mm(a, b):
            return smith.nn.functional.relu((a @ b) * 3.3 - 1.234)
    ...
    def test_max_autotune_cutlass_backend_relu6_fusion_fp16_fp32acc(self):
        def mm(a, b):
            return smith.clamp(smith.nn.functional.relu(a @ b), max=6.0)
    ...
    def test_max_autotune_cutlass_backend_no_fusion_dtype_mismatch(self):
        def mm(a, b):
            ...
            return (a @ b).to(smith.float32) * 0.00001
    ...
    def test_max_autotune_cutlass_backend_int_mm(
        ...
        a = smith.randint(0, 5, (100, 16), dtype=smith.int8).cuda()
        b = smith.randint(0, 5, (32, 16), dtype=smith.int8).cuda().T
    ...
    def test_force_cutlass_backend_aoti_dynamic(self):
        ...
        with config.patch(
            ...
            x = smith.randn(M, K).cuda().half()
            w = smith.randn(N, K).cuda().half().t()
    ...
    def test_aoti_workspace_ptr(self):
        ...
        with config.patch(
            ...
            expected = model(x, w)
            smith.testing.assert_close(expected, actual, atol=0.01, rtol=0.01)
    ...
    def test_max_autotune_cutlass_backend_sparse_semi_structured_mm(
        ...
        m, n, k = 32, 8, 64
        mask = smith.tensor([0, 0, 1, 1]).tile(m, k // 4).cuda().half()
        a = smith.rand(m, k).cuda().half() * mask
        a_sparse = to_sparse_semi_structured(a)
        b = smith.rand(k, n).cuda().half()
        ...
        with config.patch(
            ...
            Y_compiled = smith.compile(mm, dynamic=dynamic)(a_sparse, b)
            Y = mm(a, b)
            smith.testing.assert_close(Y_compiled, Y)

        cache = smith._inductor.codecache.LocalCache().lookup(
            "sparse_semi_structured_mm"
        )
        assert cache is not None
        high = cache[
            f"[('cuda', 'smith.float16', {m}, {k // 2}, {k // 2}, 1, 0), "
            f"('cuda', 'smith.int16', {m}, {k // 16}, {k // 16}, 1, 0), "
            f"('cuda', 'smith.float16', {k}, {n}, {n}, 1, 0)]"
        ]["high"]
    ...
    def test_cutlass_backend_op_denylist(
        ...
        def my_addmm(x, a, b, alpha, beta):
            return smith.addmm(x, a, b, alpha=beta, beta=alpha)

        x = smith.randn((128, 128)).cuda().half()
        a = smith.randn(128, 128).cuda().half()
        b = smith.randn(128, 128).cuda().half().t()
            ...
            with config.patch(
                ...
                with mock.patch(
                    ...
                    with self.assertRaisesRegex(
                        ...
                        smith.compile(my_addmm, dynamic=False)(x, a, b, 1.0, 2.0)
                    args, _ = sa.call_args
    ...
    def test_cutlass_backend_op_allowlist(
        ...
        def addmm(x, a, b, alpha, beta):
            return smith.addmm(x, a, b, alpha=alpha, beta=beta)
            ...
            with config.patch(
                ...
                with mock.patch(
                    ...
                    with self.assertRaisesRegex(
                        ...
                        smith.compile(addmm, dynamic=False)(x, a, b, 1.0, 1.0)
                    args, _ = sa.call_args
                    op_name, choices, _, __ = args
                    assert op_name == "addmm"
                    cuda_template_count = 0
    ...
    def test_cutlass_backend_fp8_scaled_mm_fast_accum_filtering(
        ...
        float8_dtype = smith.float8_e4m3fn
        # Only bf16 output type is supported for row-wise scaling, not fp32
        output_dtype: smith.dtype = smith.bfloat16
        device = "cuda"
        ...
        linear_compiled = smith.compile(linear, backend="inductor")
    ...
    def test_cutlass_backend_shape_coverage_mm(
        ...
        inputs = [
            (smith.randn(128, 500).cuda().half(), smith.randn(500, 576).cuda().half()),
            (
                smith.randn(500, 128).cuda().half(),
                smith.randn(128, 576).cuda().half(),
            ),
            (smith.randn(128, 250).cuda().half(), smith.randn(250, 576).cuda().half()),
            (
                smith.randn(250, 128).cuda().half(),
                smith.randn(128, 576).cuda().half(),
            ),
            (
                smith.randn(125, 128).cuda().half(),
                smith.randn(128, 576).cuda().half(),
            ),
        ]
        ...
        with (
            ...
            for input in inputs:
                ...
                with self.assertRaisesRegex(InductorError, r".*NoValidChoicesError.*"):
                    smith.compile(smith.mm, dynamic=False)(*input)
    ...
    def test_get_max_alignment(self):
        l4 = FixedLayout(
            smith.device("cpu"), smith.half, size=[1, 2, 4], stride=[0, 4, 1]
        )
        ...
        l4_2 = FixedLayout(
            smith.device("cpu"), smith.half, size=[1, 4, 2], stride=[0, 1, 4]
        )
        ...
        l1 = FixedLayout(
            smith.device("cpu"), smith.half, size=[2, 4, 2], stride=[23, 1, 4]
        )
        ...
        l2 = FixedLayout(
            smith.device("cpu"), smith.half, size=[1, 2, 4], stride=[0, 4, 1], offset=6
        )
    ...
    def test_standalone_runner(self):
        ...
        with config.patch(
            ...
            from smith._inductor.codegen.cutlass.utils import (
            ...
            with CUDACompileSourceCapturingContext() as ctx:
                compiled = smith.compile(smith.mm, dynamic=False)
    ...
    def test_cutlass_backend_integration(self):
        ...
        with config.patch(
            ...
            with (
                ...
                self.assertLogs(
                    logger="smith._inductor.codegen.cutlass", level=logging.DEBUG
                ) as test_log,
            ):
                Y_compiled = smith.compile(mm, dynamic=False)(a, b)
                Y = mm(a, b)
                smith.testing.assert_close(Y_compiled, Y)
    ...
    def test_maybe_append_choice_caching(self):
        ...
        A = smith.randn(1024, 1024, dtype=smith.bfloat16, device="cuda")
        B = smith.randn(1024, 1024, dtype=smith.bfloat16, device="cuda").t()

        expected = model(A, B)

        # Track render calls
        from smith._inductor.codegen.cutlass.gemm_template import CUTLASSGemmTemplate
        ...
        with mock.patch.object(CUTLASSGemmTemplate, "render", counting_render):
            with config.patch(
                ...
                compiled_model = smith.compile(model, fullgraph=True)
    ...
    def test_multiple_mm(self):
        ...
        class MultipleMMModel(smith.nn.Module):
        ...
        a = smith.randn(128, 64).cuda().half()
        b = smith.randn(32, 64).cuda().half().t()
        c = smith.randn(256, 128).cuda().half()
        d = smith.randn(64, 128).cuda().half().t()
    ...
    def test_multiple_mm_with_dynamic_shape(self):
        ...
        class MultipleMMDynamicModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.c = smith.randn(64, 256).cuda().half()
                self.d = smith.randn(128, 256).cuda().half().t()
    ...
    def test_cutlass_backend_matmul_same_tensor(self):
        ...
        with config.patch(
            ...
            compiled = smith.compile(smith.mm)

            smith.testing.assert_close(A @ A.t(), compiled(A, A.t()))
    ...
    def test_flexible_layout(self):
        class TestModel(smith.nn.Module):
            def forward(self, B):
                A = smith.zeros_like(B)
                return A @ B.t()

        M = 1024
        B = smith.randn(M, M).cuda().half()
    ...
    def test_evt_flexible_layout(self):
        ...
        self.assertEqual(
            smith._dynamo.utils.counters["inductor"]["cuda_epilogue_fusion_counter"], 1
        )
    ...
    def test_compilation_time(self, use_aoti):
        ...
        with config.patch(
            ...
            if use_aoti:
                ...
                actual = smith.compile(model, fullgraph=True)(A, B)
    ...
    def test_evt_mixed_dtypes(self, op):
        ...
        fp32_tensor = smith.ones(M, N).cuda().float()

        class TestModel(smith.nn.Module):
            def forward(self, a, b, extra_args):
                acc = a @ b
                out0 = op(acc.relu(), *extra_args)
                out1 = smith.add(out0, fp32_tensor)
                return out1

        model = TestModel().cuda()
        a = smith.ones(M, N).cuda().half()
        b = smith.ones(N, N).cuda().half().t()
        extra_args = gen_args(op, (M, N), dtype=smith.float16)
        ...
        with config.patch({"cutlass.cutlass_epilogue_fusion_enabled": False}):
            ref_result = smith.compile(model)(a, b, extra_args)

        self.assertEqual(
            smith._dynamo.utils.counters["inductor"]["cuda_epilogue_fusion_counter"], 0
        )
    ...
    def test_evt_multi_op(self, op):
        class TestModel(smith.nn.Module):
            def forward(self, a, b, extra_args):
                acc = a @ b
                return smith.add(op(acc.relu(), *extra_args).relu(), acc)
    ...
    def test_evt_reuse_matmul_input(self, op):
        class TestModel(smith.nn.Module):
            def forward(self, a, b, extra_args):
                acc = a @ b
                return smith.add(op(acc.relu(), *extra_args).relu(), a)
    ...
    @unittest.skipIf(not SM90OrLater, "need sm_90")
    @fp8_config
    @parametrize("float8_dtype", (smith.float8_e4m3fn,))
    @parametrize(
        "shape",
        (
            (
    ...
    @parametrize("input_dtype", (smith.bfloat16, smith.float16))
    def test_fp8_rowwise_scaling(
        ...
        x = smith.randn(M, K, dtype=input_dtype, device=device)
        w = smith.randn(N, K, dtype=input_dtype, device=device)
        bias = None
        if has_bias:
            bias = smith.randn(N, device=device, dtype=input_dtype).to(smith.bfloat16)
    ...
    def test_fp8_rowwise_scaling_multiple_linear(
        ...
        x = smith.randn(M, N, dtype=output_dtype, device=device)
        w1 = smith.randn(N, N, dtype=output_dtype, device=device)
        w2 = smith.randn(N, N, dtype=output_dtype, device=device)
        ...
        if use_aoti:
            ...
            compiled_model = smith.compile(model, fullgraph=True, dynamic=dynamic)
            actual = compiled_model(x)

        smith.testing.assert_close(expected, actual, rtol=1e-2, atol=0.05)
    ...
    def test_fp8_tensorwise_scaling(
        ...
        bias = None
        if has_bias:
            bias = smith.randn(N, device=device, dtype=input_dtype)
    ...
    def test_config_number_post_filtering(self) -> None:
        ...
        for layout in layouts:
            for dtype in dtypes:
                a = smith.randn(128, 128, dtype=dtype).cuda()
                b = smith.randn(128, 128, dtype=dtype).cuda()
                if layout[0] == "c":
                    a = a.t()
                if layout[1] == "c":
                    b = b.t()
                ...
                with config.patch(
                    ...
                    with mock.patch(
                        ...
                        with self.assertRaisesRegex(
                            ...
                            _ = smith.compile(smith.mm, dynamic=False)(a, b)
                        args, _ = sa.call_args
                        _, choices, _, __ = args
