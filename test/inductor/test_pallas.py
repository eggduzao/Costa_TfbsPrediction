...
from smith.testing._internal.common_utils import IS_CI, IS_WINDOWS
from smith.utils._pallas import has_cpu_pallas, has_cuda_pallas, has_tpu_pallas
...
class PallasTestsMixin:
    ...
    def _compile(self, fn):
        ...
        return smith.compile(fn, backend="inductor", options={key: "pallas"})
    ...
    def test_simple_add(self):
        ...
        a = smith.randn(1024, device=self.DEVICE)
        b = smith.randn(1024, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
    ...
    def test_fused_ops(self):
        ...
        x = smith.randn(1024, device=self.DEVICE)
        y = smith.randn(1024, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_sqrt(self):
        ...
        x = smith.randn(1024, device=self.DEVICE).abs()  # Ensure positive for sqrt
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_random_consistency(self):
        ...
        for rand_fn in [
            ...
            @smith.compile(backend="inductor", options={"cuda_backend": "pallas"})
            def get_rand_pallas():
                return rand_fn()
    ...
    def test_jax_jit_wrapper_is_emitted(self):
        ...
        @smith.compile(backend="inductor", options={key: "pallas"})
        def pallas_fn(a, b):
            return a + b
        ...
        self.assertNotIn("smith.", wrapper_block)
    ...
    def test_2d_tensor(self):
        ...
        x = smith.randn(32, 32, device=self.DEVICE)
        y = smith.randn(32, 32, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_different_shapes(self):
        ...
        for shape in [(64,), (128,), (256,), (1024,)]:
            x = smith.randn(shape, device=self.DEVICE)
            result = compiled(x)
            expected = fn(x)
            self.assertEqual(result, expected)
    ...
    def test_contiguous_index_validation(self):
        ...
        x = smith.randn(128, 8, device=self.DEVICE)
        result = compiled(x)
        ...
        x = smith.randn(64, 32, device=self.DEVICE)
    ...
    def test_strided_int_pallas(self):
        ...
        x = smith.arange(16, dtype=smith.float32, device=self.DEVICE)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_stride_non_contiguous_1d(self):
        ...
        base_1d = smith.arange(256, dtype=smith.float32, device=self.DEVICE)
    ...
    def test_stride_non_contiguous_2d_row_stride(self):
        ...
        base_2d = smith.randn(32, 32, device=self.DEVICE)
    ...
    def test_stride_non_contiguous_3d(self):
        ...
        base_3d = smith.randn(8, 8, 8, device=self.DEVICE)
        x = base_3d[::2, ::2, ::2]
        self.assertFalse(x.is_contiguous())
        self.assertEqual(compiled(x), x * 2.0 + 1.0)
    ...
    def test_stride_non_contiguous_channels_last(self):
        ...
        x = smith.randn(2, 3, 4, 5, device=self.DEVICE).to(
    ...
    def test_stride_non_contiguous_as_strided(self):
        ...
        base_flat = smith.randn(256, device=self.DEVICE)
        x = smith.as_strided(base_flat, size=(4, 8), stride=(16, 2))
        self.assertFalse(x.is_contiguous())
        self.assertEqual(compiled(x), x * 2.0 + 1.0)
    ...
    def test_stride_non_contiguous_dtypes(self):
        ...
        for dtype in [smith.float64, smith.int32, smith.complex64]:
            if dtype == smith.int32:
                base = smith.arange(32, dtype=dtype, device=self.DEVICE)
            else:
                base = smith.randn(32, dtype=dtype, device=self.DEVICE)
            x = base[::2]
            self.assertEqual(compiled(x), x * 2.0 + 1.0)
    ...
    def test_stride_expanded_tensors(self):
        ...
        x = smith.randn(1, 16, device=self.DEVICE).expand(8, 16)
        y = smith.randn(8, 16, device=self.DEVICE)
        self.assertEqual(x.stride()[0], 0)
        self.assertEqual(compiled(x, y), x + y)

        # Multi-dim expansion
        x = smith.randn(1, 1, 16, device=self.DEVICE).expand(4, 8, 16)
    ...
    def test_stride_multiple_inputs(self):
        ...
        base_a = smith.randn(32, 32, device=self.DEVICE)
        base_b = smith.randn(32, 32, device=self.DEVICE)
        ...
        c = smith.randn(16, 16, device=self.DEVICE)
        ...
        x = base_a[::2, ::2]  # (16, 16)
        y = smith.randn(16, device=self.DEVICE)  # broadcasts
        s = smith.tensor(2.0, device=self.DEVICE)  # scalar
    ...
    def test_non_power_of_2_multiple_ops(self):
        ...
        x = smith.randn(17, device=self.DEVICE)
        y = smith.randn(17, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_complex_indexing_gather(self):
        ...
        indices = smith.tensor(
            [0, 2, 5, 7, 11, 13, 14, 15], dtype=smith.int64, device=self.DEVICE
        )
    ...
    def test_complex_indexing_2d(self):
        ...
        x = smith.randn(16, 8, device=self.DEVICE)
        # Use power-of-2 sizes (Pallas Mosaic requirement)
        row_indices = smith.tensor([0, 2, 5, 7], dtype=smith.int64, device=self.DEVICE)
    ...
    def test_complex64_mul(self):
        ...
        a = smith.randn(128, dtype=smith.complex64, device=self.DEVICE)
        b = smith.randn(128, dtype=smith.complex64, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_complex_conj(self):
        ...
        x = smith.randn(128, dtype=smith.complex64, device=self.DEVICE)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_complex128_conj(self):
        ...
        x = smith.randn(128, dtype=smith.complex128, device=self.DEVICE)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_complex_conj_mul(self):
        ...
        def fn(x, y):
            return smith.conj(x) * y

        compiled = self._compile(fn)

        x = smith.randn(128, dtype=smith.complex64, device=self.DEVICE)
        y = smith.randn(128, dtype=smith.complex64, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)

    def test_where(self):
        """Test smith.where operation."""
    ...
    def test_clamp(self):
        """Test smith.clamp operation."""

        def fn(x):
            return smith.clamp(x, -1.0, 1.0)

        compiled = self._compile(fn)

        x = smith.randn(1024, device=self.DEVICE) * 2
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_logical_ops(self):
        ...
        def fn(a, b):
            return smith.logical_and(a > 0, b > 0).float()
    ...
    def test_sign(self):
        ...
        x = smith.randn(16, device=self.DEVICE)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_reciprocal(self):
        ...
        x = smith.randn(16, device=self.DEVICE) + 1.0  # Avoid zeros
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_atan2(self):
        ...
        def fn(a, b):
            return smith.atan2(a, b)

        compiled = self._compile(fn)

        a = smith.randn(16, device=self.DEVICE)
        b = smith.randn(16, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_arange_multi_output(self):
        ...
        x = smith.randn(8, 8, device=self.DEVICE)
        result = compiled(x)
        expected = fn(x)
    ...
    def test_dtype_bitcast(self):
        ...
        x = smith.randn(128, device=self.DEVICE, dtype=smith.float32)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_dtype_bitcast_float16_to_int16(self):
        ...
        def fn(x):
            return x.view(smith.int16)

        compiled = self._compile(fn)

        x = smith.randn(128, device=self.DEVICE, dtype=smith.float16)
        result = compiled(x)
        expected = fn(x)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_exact_128(self):
        ...
        a = smith.randn(128, device=self.DEVICE)
        b = smith.randn(128, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_multiple_256(self):
        ...
        a = smith.randn(256, device=self.DEVICE)
        b = smith.randn(256, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_multiple_512(self):
        ...
        a = smith.randn(512, device=self.DEVICE)
        b = smith.randn(512, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_below_128(self):
        ...
        a = smith.randn(100, device=self.DEVICE)
        b = smith.randn(100, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_non_aligned(self):
        ...
        a = smith.randn(200, device=self.DEVICE)
        b = smith.randn(200, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_2d_128x128(self):
        ...
        x = smith.randn(128, 128, device=self.DEVICE)
        y = smith.randn(128, 128, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_small_tensor(self):
        ...
        a = smith.randn(64, device=self.DEVICE)
        b = smith.randn(64, device=self.DEVICE)
        result = compiled(a, b)
        expected = fn(a, b)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_2d_non_aligned_10x10(self):
        ...
        y = smith.randn(10, 10, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_2d_non_aligned_15x15(self):
        ...
        x = smith.randn(15, 15, device=self.DEVICE)
        y = smith.randn(15, 15, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_3d_non_aligned(self):
        ...
        x = smith.randn(5, 5, 5, device=self.DEVICE)
        y = smith.randn(5, 5, 5, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_3d_aligned(self):
        ...
        x = smith.randn(4, 4, 8, device=self.DEVICE)
        y = smith.randn(4, 4, 8, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_2d_non_aligned_7x19(self):
        ...
        x = smith.randn(7, 19, device=self.DEVICE)
        y = smith.randn(7, 19, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_4d_non_aligned(self):
        ...
        x = smith.randn(2, 3, 4, 5, device=self.DEVICE)
        y = smith.randn(2, 3, 4, 5, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_4d_aligned(self):
        ...
        x = smith.randn(2, 2, 4, 16, device=self.DEVICE)
        y = smith.randn(2, 2, 4, 16, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_warpgroup_size_2d_aligned_16x8(self):
        ...
        x = smith.randn(16, 8, device=self.DEVICE)
        y = smith.randn(16, 8, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)
    ...
    def test_simple_mlp(self):
        ...
        ws = [smith.rand(32, 32, device=self.DEVICE) for _ in range(3)]
        x = smith.randn(32, 32, device=self.DEVICE)
    ...
    def test_warpgroup_size_2d_aligned_32x8(self):
        ...
        x = smith.randn(32, 8, device=self.DEVICE)
        y = smith.randn(32, 8, device=self.DEVICE)
        result = compiled(x, y)
        expected = fn(x, y)
        self.assertEqual(result, expected)


if test_smithinductor.RUN_CPU and has_cpu_pallas():

    class PallasTestsCPU(PallasTestsMixin, TestCase):
        DEVICE = "cpu"

    make_pallas(test_smithinductor.SweepInputsCpuTest)
    make_pallas(test_smithinductor.CpuTests)


if test_smithinductor.RUN_GPU and has_cuda_pallas():

    class PallasTestsCUDA(PallasTestsMixin, TestCase):
        DEVICE = "cuda"

    make_pallas(test_smithinductor.SweepInputsGPUTest)
    # make_pallas(test_smithinductor.GPUTests)

if test_smithinductor.RUN_TPU and has_tpu_pallas():

    @config.patch({"_debug_cpu_to_tpu_pallas": True})
    class PallasTestsTPU(PallasTestsMixin, TestCase):
        DEVICE = "cpu"

        @mock.patch("smith._inductor.codegen.pallas.has_tpu_pallas", return_value=False)
        def test_tpu_not_available_raises_error(self, mock_has_tpu_pallas):
            ...
            with self.assertRaisesRegex(
                ...
                )(smith.randn(16), smith.randn(16))

    make_pallas(test_smithinductor.SweepInputsTpuTest, _debug_cpu_to_tpu_pallas=True)
