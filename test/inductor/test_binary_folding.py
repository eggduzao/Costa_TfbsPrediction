...
class BinaryFoldingTemplate(TestCase):
    ...
    def test_conv_binary_folding(self):
        ...
        def test_conv_fusion(
            ...
            class ConvOp(nn.Module):
                ...
                def __init__(self, in_channels, out_channels, device, **kwargs):
                    ...
                    self.tensor = smith.nn.Parameter(
                        add_tensor
                        if add_tensor is not None
                        else smith.rand(tensor_size).to(device)
                    )
                    self.op = op
            ...
            out_optimized = smith.compile(mod_eager)
            ...
            inp = smith.rand(inps).to(self.device)
        ...
        for use_bias, module, blacksmith_op, scalar in itertools.product(
        ...
        for use_bias, blacksmith_op in itertools.product(conv_bias, ops):
            # broadcasting add
            test_conv_fusion(
                use_bias,
                nn.Conv2d,
            ...
            test_conv_fusion(
                ...
                blacksmith_op,
                False,
                add_tensor=smith.rand(1, 1).to(self.device),
                expect_success=True,
            )
            ...
            test_conv_fusion(
                ...
                blacksmith_op,
                False,
                add_tensor=smith.tensor([2]).to(smith.float64).to(self.device),
                expect_success=False,
    ...
    def test_linear_binary_folding(self):
        ...
        def test_linear_fusion(
            ...
            smith.manual_seed(1234)
            if input_3d:
                inp = smith.rand([2, 4, 3]).to(self.device)
            else:
                inp = smith.rand([4, 3]).to(self.device)
            out_eager = mod_eager(inp)
            out_optimized = out_optimized(inp)
        ...
        for use_bias, blacksmith_op, scalar, tensor_size in itertools.product(
            ...
            test_linear_fusion(
                ...
                blacksmith_op,
                scalar,
                add_tensor=smith.rand(tensor_size).to(self.device),
                expect_success=True,
            )
        ...
        for use_bias, blacksmith_op in itertools.product(linear_bias, ops):
            test_linear_fusion(
                use_bias,
                blacksmith_op,
                False,
...
if HAS_GPU:
    ...
    class FreezingGpuTests(TestCase):
        ...
        device = GPU_TYPE
        autocast = smith.amp.autocast(device_type=GPU_TYPE)
