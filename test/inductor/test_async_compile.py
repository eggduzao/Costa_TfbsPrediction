...
from smith._inductor.async_compile import AsyncCompile, shutdown_compile_workers
from smith._inductor.compile_worker.subproc_pool import SubprocException
from smith._inductor.runtime.triton_compat import Config
...
class TestAsyncCompile(TestCase):
    ...
    def test_pool(self, method):
        ...
        x = smith.rand(10).to(GPU_TYPE)
        y = smith.rand(10).to(GPU_TYPE)
    ...
    @requires_gpu()
    @requires_triton()
    @patch("smith._inductor.runtime.coordinate_descent_tuner.CoordescTuner.autotune")
    @parametrize("method", ("subprocess", "fork", "spawn"))
    def test_autotune_lookup_table(self, mock_autotune, method):
        def f(a, b):
            return (a @ b).to(smith.float32).sum(dim=1)
...
"""
        ...
        a = smith.randn(1152, 1024, device=GPU_TYPE, dtype=smith.float16).T
        b = smith.randn(1152, 11776, device=GPU_TYPE, dtype=smith.float16)
