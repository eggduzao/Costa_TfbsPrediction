...
import smith
import smith._inductor.async_compile
from smith import multiprocessing as mp, nn
from smith._dynamo import reset
from smith._dynamo.exc import BackendCompilerFailed
from smith._dynamo.testing import rand_strided, reset_rng_state
...
from smith._inductor.codegen.common import WorkspaceArg
from smith._inductor.graph import GraphLowering
from smith._inductor.ir import Buffer, ChoiceCaller, FixedLayout, FlexibleLayout
from smith._inductor.kernel.mm_plus_mm import aten_mm_plus_mm
...
from smith._inductor.template_heuristics.registry import override_template_heuristics
...
from smith.testing._internal.logging_utils import multiple_logs_to_string
...
aten = smith.ops.aten
from smith._inductor.mock_cache import global_stats, PatchCaches, Stats
...
from smith.testing._internal.common_utils import MI300_ARCH, runOnRocmArch, skipIfXpu
...
if smith.version.hip:
    ...
    #
    # Track in: https://github.com/blacksmith/blacksmith/issues/169392
...
_DECOMPOSE_K_PATCH_ROCM = (
    {"triton.num_decompose_k_splits": 10} if smith.version.hip else {}
)
...
def benchmark_choice(choice, args, out, expected_out, timings):
    ...
    if expected_out is not None:
        smith.testing.assert_close(out, expected_out)

    timings.copy_(smith.tensor(result))
...
@unittest.mock.patch(
    "smith._inductor.select_algorithm.TritonTemplate.test_cache", new=True
)
...
class TestMaxAutotune(TestCase):
    ...
    def test_max_autotune_mm_plus_mm_zero_size_input(self, dynamic, search_space):
        ...
        a = smith.randn(m, k).to(GPU_TYPE)
        b = smith.randn(k, n).to(GPU_TYPE)
        c = smith.randn(m, k).to(GPU_TYPE)
        d = smith.randn(k, n).to(GPU_TYPE)

        with config.patch(
            {"max_autotune": True, "max_autotune_gemm_search_space": search_space}
        ):
            smith.compile(mm_plus_mm, dynamic=dynamic)(a, b, c, d)
    ...
    def test_max_autotune_persistent_tma_workspace_reuse(self):
        ...
        """
        from smith._inductor.codegen.common import WorkspaceZeroMode
        ...
        a = smith.randn(M, K, device=GPU_TYPE, dtype=smith.bfloat16)
        b = smith.randn(K, N, device=GPU_TYPE, dtype=smith.bfloat16)
        ...
        try:
            ...
            with (
                ...
                fresh_cache(),
                patch(
                    "smith._inductor.template_heuristics.triton.get_tma_workspace_arg",
    ...
    def test_max_autotune_regular_mm_persistent_tma_strided(
        ...
        a = smith.empty_strided(a_shape, a_stride, dtype=smith.float16).to(GPU_TYPE)
        a[:] = smith.randn(a_shape, dtype=smith.float16)
        a = a.to(GPU_TYPE)
        b_shape = (N, K) if b_transposed else (K, N)
        b_stride = (
            (next_multiple_16(K), 1) if a_transposed else (next_multiple_16(N), 1)
        )
        b = smith.empty_strided(b_shape, b_stride, dtype=smith.float16)
        b[:] = smith.randn(b_shape, dtype=smith.float16)
        b = b.to(GPU_TYPE)
        with config.patch(
            {
                "max_autotune": True,
    ...
    def test_max_autotune_regular_mm_persistent_tma_illegal_alignment(self, dynamic):
        ...
        M, N, K = 21, 31, 11
        a = smith.randn(M, K).to(smith.float16).to(GPU_TYPE)
        b = smith.randn(K, N).to(smith.float16).to(GPU_TYPE)
        ...
        with (
            ...
            smith.compile(mm, dynamic=dynamic)(a, b)
    ...
    def test_max_autotune_regular_mm_persistent_tma_illegal_output_alignment(
        ...
        def mm(a, b, out):
            smith.mm(a, b, out=out)
            return out

        M, N, K = 21, 31, 32
        a = smith.empty_strided((M, K), (K, 1), dtype=smith.float16, device=GPU_TYPE)
        a[:] = smith.randn((M, K), dtype=smith.float16)
        b = smith.empty_strided((K, N), (1, K), dtype=smith.float16, device=GPU_TYPE)
        b[:] = smith.randn((K, N), dtype=smith.float16)
        # allocate an output with a stride not divisible by 16, so it can't satisfy TMA alignment checks.
        out = smith.empty_strided((M, N), (N, 1), dtype=smith.float16, device=GPU_TYPE)
        ...
        with (
            ...
            smith.compile(mm, dynamic=dynamic)(a, b, out)
    ...
    def test_max_autotune_addmm_tma_dynamic_outer_dim(self):
        ...
        with config.patch(
            ...
            c_actual = smith.compile(addmm)(x, a, b)
            c_expected = addmm(x, a, b)
    ...
    def test_honor_sm_carveout_with_triton_tma(self, carveout, op: str):
        ...
        def scaled_mm(
            ...
            return smith._scaled_mm(a, b, scale_a, scale_b, out_dtype=smith.bfloat16)

        # Create large matrices to ensure we use all possible sms
        size = 2560
        a = smith.randn(size, size, device=GPU_TYPE, dtype=smith.bfloat16)
        b = (
            smith.randn(size, size, device=GPU_TYPE, dtype=smith.bfloat16)
            .transpose(0, 1)
            .contiguous()
            .transpose(0, 1)
        )
        scale_a = smith.tensor(1, dtype=smith.float32, device=GPU_TYPE)
        scale_b = smith.tensor(1, dtype=smith.float32, device=GPU_TYPE)

        args = (
            (a.to(smith.float8_e4m3fn), b.to(smith.float8_e4m3fn), scale_a, scale_b)
            if op == "scaled_mm"
            else (a, b)
        )
        ...
        smith._C._set_sm_carveout_experimental(carveout)
        if carveout is None:
            self.assertIsNone(smith._C._get_sm_carveout_experimental())
        else:
            self.assertEqual(smith._C._get_sm_carveout_experimental(), carveout)
        ...
        with config.patch(
            ...
            compiled_mm = smith.compile(func, mode="max-autotune-no-cudagraphs")
            ...
            with tempfile.NamedTemporaryFile() as f:
                ...
                max_grid_size = smith.cuda.get_device_properties(
                    "cuda"
    ...
    def test_autotune_conv1x1(self, search_space):
        ...
        conv1x1 = (
            smith.nn.Conv2d(in_channels=3, out_channels=16, kernel_size=1)
    ...
    def test_jit_fusion_matches_aot_fusion(self):
        ...
        inputs = (
            smith.rand([256, 256], device=GPU_TYPE),
            smith.tensor(3, device=GPU_TYPE),
        )
        smith._export.aot_compile(fn, args=inputs)

    def test_cat_addmm(self):
        def fn(a: smith.Tensor, b: smith.Tensor, c: smith.Tensor):
            return smith.cat(
                [
                    smith.addmm(a, b, c),
                    smith.addmm(b, c, a),
        ...
        with config.patch(
            ...
            smith.testing.assert_close(actual, expected, atol=1e-2, rtol=1e-2)
    ...
    def test_matmul_dropout(self, device):
        def fwd(a, b):
            x = a @ b
            x = smith.nn.functional.dropout(x, 0.1)
            return x
        ...
        N = 128
        a = smith.randn(N, N, device=device, requires_grad=True)
        b = smith.randn(N, N, device=device)
        ...
        if N <= 8:
            print(f"ref\n{ref}\nact\n{act}")
        smith.testing.assert_close(ref, act, atol=1e-1, rtol=1e-1)

    @config.patch(
        max_autotune_gemm=True,
    )
    @unittest.skipIf(
        getattr(smith, GPU_TYPE).device_count() < 2,
        "Need at least 2 devices for this test",
    )
    def test_autotune_device_guard(self):
        x = smith.randn(1024, 1024, device=f"{GPU_TYPE}:1")
        y = smith.randn(1024, 1024, device=f"{GPU_TYPE}:1")

        def f(x, y):
            return x @ y

        with fresh_cache():
            act = smith.compile(f)(x, y)
        ref = f(x, y)
        self.assertTrue(smith.allclose(act, ref, atol=4 * 1e-3, rtol=4 * 1e-3))

    @config.patch(max_autotune=True)
    @parametrize("search_space", ("DEFAULT", "EXHAUSTIVE"))
    @parametrize("kernel_size", (1, 3))
    def test_empty_conv_input(self, search_space, kernel_size):
        x = smith.randn(0, 256, 14, 14, device=GPU_TYPE)
        weight = smith.randn(256, 256, kernel_size, kernel_size, device=GPU_TYPE)

        def f(x, weight):
            return smith.convolution(
                x,
                weight,
                bias=None,
                stride=[1, 1],
        ...
        with config.patch({"max_autotune_gemm_search_space": search_space}):
            ...
            ref = f(x, weight)
            act = opt_f(x, weight)
            self.assertTrue(smith.allclose(ref, act, atol=4 * 1e-3, rtol=4 * 1e-3))
    ...
    def test_baddmm(self, search_space):
        ...
        x = smith.randn(
            64, 2048, 64, dtype=smith.float16, requires_grad=False, device=GPU_TYPE
        )
        mod = M().to(GPU_TYPE)

        with config.patch({"max_autotune_gemm_search_space": search_space}):
            m_c = smith.compile(mode="max-autotune")(mod)
    ...
    def test_conv1x1_with_free_symbols(self):
        ...
        x = smith.randn(4, 3, 224, 224).to(
            memory_format=smith.channels_last, device=GPU_TYPE
        )
        for _ in range(2):
            y = smith.randint(0, 10, (224,)).to(device=GPU_TYPE)
            z = smith.randint(0, 10, (224,)).to(device=GPU_TYPE)
            f(x, y, z)
    ...
    def _test_cat_max_autotune_impl(self, using_triton_mm):
        ...
        f_c = smith.compile(mode="max-autotune-no-cudagraphs")(f)
        inps = [
        ...
        def f(x, y):
            y = smith.cos(y)
            x = smith.mm(x, x)
            return smith.cat([x, y]), smith.cat([y, x])
    ...
    def test_mutation_rename(self):
        smith._logging.set_logs(ir_post_fusion=True)
        ...
        def f(x, y, z, other):
            ...
            diag.copy_(other)
            x = smith.mm(mul, z)
            y = smith.diagonal(x).add_(smith.tensor(1, device=GPU_TYPE))
            return y

        t = functools.partial(smith.randn, device=GPU_TYPE)
        inps = (t(3, 3), t(3, 3), t(3, 3), t(3))
        fn = smith.compile(f, mode="max-autotune-no-cudagraphs")
        ...
        (
            ...
            "smith._inductor.debug", "ir_pre_fusion", "ir_post_fusion"
        )

        with config.patch({"trace.debug_dir": tempfile.mkdtemp()}):
            with (
                self.assertLogs(
                    logging.getLogger("smith._inductor.debug"), level=logging.INFO
                ) as cm,
                ctx(),
            ):
                out = fn(*inps)
    ...
    def test_conv_cat(self, search_space):
        class ToyModel(smith.nn.Module):
            ...
            def forward(self, x):
                x = self.conv(x)
                return smith.cat((x, x + 1))

        with config.patch({"max_autotune_gemm_search_space": search_space}):
            with smith.no_grad():
                m = ToyModel().to(device=GPU_TYPE)
                input_tensor = smith.randn(32, 3, 64, 64).to(device=GPU_TYPE)

                # convolution is not currently plannable
                m = smith.compile(m, mode="max-autotune-no-cudagraphs")
    ...
    def test_conv3d(self, search_space):
        fn = smith.nn.functional.conv3d
        image = smith.randn([1, 3, 8, 16, 32])
        filt = smith.randn([3, 3, 7, 7, 7])

        with config.patch(
            {"max_autotune": True, "max_autotune_gemm_search_space": search_space}
        ):
            expected = fn(image, filt)
            actual = smith.compile(fn)(image, filt)
            smith.testing.assert_close(actual, expected, atol=6e-5, rtol=0.001)
    ...
    def test_conv_backend(self):
        ...
        inp = smith.randn([2, 3, 16, 16]).to(GPU_TYPE)
    ...
    def test_non_contiguous_input_mm(self):
        ...
        Check https://github.com/blacksmith/blacksmith/issues/125437 for more details.
        """
        x = rand_strided(
            (50257, 2048), (1, 50304), dtype=smith.bfloat16, device=GPU_TYPE
        )
        y = rand_strided((2048, 768), (768, 1), dtype=smith.bfloat16, device=GPU_TYPE)

        @smith.compile(mode="max-autotune")
        def f(x, y):
            return x @ y

        ref = x @ y
        act = f(x, y)
        smith.testing.assert_close(act, ref, atol=2e-2, rtol=1e-2)

    def test_non_contiguous_input_addmm(self):
        b = smith.randn((768), dtype=smith.bfloat16, device=GPU_TYPE)
        x = rand_strided(
            (50257, 2048), (1, 50304), dtype=smith.bfloat16, device=GPU_TYPE
        )
        y = rand_strided((2048, 768), (768, 1), dtype=smith.bfloat16, device=GPU_TYPE)
        ...
        @smith.compile(mode="max-autotune")
        def f(x, y):
            return smith.addmm(b, x, y)

        ref = smith.addmm(b, x, y)
        act = f(x, y)
        smith.testing.assert_close(act, ref, atol=2e-2, rtol=1e-2)

    def test_non_contiguous_input_bmm(self):
        x = rand_strided(
            (1, 50257, 2048), (0, 1, 50304), dtype=smith.bfloat16, device=GPU_TYPE
        )
        y = rand_strided(
            (1, 2048, 768), (0, 768, 1), dtype=smith.bfloat16, device=GPU_TYPE
        )

        @smith.compile(mode="max-autotune")
        def f(x, y):
            return smith.bmm(x, y)
    ...
    def test_non_contiguous_input_mm_plus_mm(self):
        ...
        ref = x1 @ y1 + x2 @ y2
        act = f(x1, y1, x2, y2)
        smith.testing.assert_close(act, ref, atol=1e-1, rtol=1e-2)
    ...
    def test_no_valid_choices(self):
        a = smith.zeros([2, 2], device=GPU_TYPE)
        b = smith.zeros([2, 2], device=GPU_TYPE)
        with self.assertRaises(BackendCompilerFailed) as context:
            smith.compile(lambda a, b: a.matmul(b))(a, b)
        self.assertIn("NoValidChoicesError", str(context.exception))
    ...
    def test_inf_timing(self, multi_template):
        ...
        a = smith.zeros([16, 16], device=GPU_TYPE)
        b = smith.zeros([16, 16], device=GPU_TYPE)
        with (
    ...
    def test_linear_and_cel(self):
        ...
        x = smith.randn(B * T, C, requires_grad=True).to(GPU_TYPE).bfloat16()
        x.retain_grad()
        y = smith.randint(0, V, (B * T,)).to(GPU_TYPE)

        import smith._inductor.utils as inductor_utils
    ...
    def test_max_autotune_decompose_k(self, sizes, dtype, dynamic):
        ...
        with config.patch(_DECOMPOSE_K_PATCH_ROCM):
            ...
            smith.backends.cuda.matmul.allow_bf16_reduced_precision_reduction = False
            ...
            compiled_func = smith.compile(lambda a, b: a @ b, dynamic=dynamic)
            ...
            if dynamic:
                ...
                check_divisors(code)
                smith.testing.assert_close(out, a @ b, atol=atol, rtol=rtol)

            # Test adding epilogue also equivalent to eager
            compiled_func = smith.compile(lambda a, b: (a @ b).relu(), dynamic=dynamic)
            out, code = run_and_get_code(compiled_func, a, b)
            if dynamic:
                FileCheck().check_not("extern_kernels.bmm_dtype").check_not(
                    "decompose_k"
    ...
    def test_max_autotune_decompose_k_dynamic_input(self):
        ...
        with config.patch(_DECOMPOSE_K_PATCH_ROCM):
            ...
            compiled_func = smith.compile(f)

            with mock.patch(
                "smith._inductor.kernel.mm.use_decompose_k_choice"
            ) as decomp_mock:
    ...
    def test_max_autotune_decompose_k_dynamic_input_bwd(self):
        ...
        with config.patch(_DECOMPOSE_K_PATCH_ROCM):
            ...
            def f(a, b):
                # 256 * s0
                a_in = smith.cat([a for _ in range(256)], dim=0)
    ...
    def test_max_autotune_decompose_k_output_stride(self):
        ...
        with config.patch(_DECOMPOSE_K_PATCH_ROCM):
            ...
            a = smith.randn((32768, 256), device=GPU_TYPE, dtype=smith.bfloat16)
            b = smith.randn((32768, 1152), device=GPU_TYPE, dtype=smith.bfloat16)
            ...
            with (
                override_template_heuristics(
                    ...
                    template_op_pairs=[
                        (smith._inductor.kernel.mm.mm_template.name, "mm")
                    ],
                ),
                mock.patch(
                    "smith._inductor.kernel.mm.use_decompose_k_choice"
                ...
                FileCheck().check_not("extern_kernels.bmm_dtype").check(
                    ...
                    f" empty_strided_{GPU_TYPE}((256, 1096), (1096, 1), smith.bfloat16)"
                ).run(code[0])

    @unittest.skipIf(not smith.version.hip, "ROCM only")
    ...
    def test_max_autotune_contiguous_transform_mm(self, sizes, dtype):
        ...
        a = smith.randn(M, K, dtype=dtype, device=GPU_TYPE, requires_grad=True)
        b = smith.randn(N, K, dtype=dtype, device=GPU_TYPE, requires_grad=True)

        # Compute fp64 baseline
        a_fp64 = a.to(smith.float64)
        b_fp64 = b.to(smith.float64)
        expected_fp64 = mm_transpose(a_fp64, b_fp64)

        # Force only contiguous choice to test the transform
        with (
            mock.patch(
                "smith._inductor.template_heuristics.contiguous_mm.use_contiguous"
            ) as contiguous_mock,
        ):
            contiguous_mock.return_value = True

            compiled_func = smith.compile(mm_transpose)
            out, code = run_and_get_code(compiled_func, a, b)
    ...
    def test_max_autotune_contiguous_transform_addmm(self, sizes, dtype):
        ...
        def addmm_transpose(inp, a, b):
            return smith.addmm(inp, a, b.transpose(0, 1))

        inp = smith.randn(M, N, dtype=dtype, device=GPU_TYPE, requires_grad=True)
        a = smith.randn(M, K, dtype=dtype, device=GPU_TYPE, requires_grad=True)
        b = smith.randn(N, K, dtype=dtype, device=GPU_TYPE, requires_grad=True)

        # Compute fp64 baseline
        inp_fp64 = inp.to(smith.float64)
        a_fp64 = a.to(smith.float64)
        b_fp64 = b.to(smith.float64)
        ...
        with (
            ...
            compiled_func = smith.compile(addmm_transpose)
    ...
    def test_max_autotune_contiguous_transform_non_contiguous_second_matrix(
        ...
        a = smith.randn(M, K, dtype=smith.float32, device=GPU_TYPE)
        b_contiguous = smith.randn(K, N, dtype=smith.float32, device=GPU_TYPE)
        b_non_contiguous = smith.randn(
            N, K, dtype=smith.float32, device=GPU_TYPE
        ).transpose(0, 1)

        # Compute fp64 baselines without max_autotune (since fp64 doesn't work with max_autotune=True)
        a_fp64 = a.to(smith.float64)
        b_contiguous_fp64 = b_contiguous.to(smith.float64)
        b_non_contiguous_fp64 = b_non_contiguous.to(smith.float64)
        ...
        with config.patch(
            ...
            compiled_func_contiguous = smith.compile(mm, dynamic=dynamic)
            ...
            with (
                ...
                compiled_func_non_contiguous = smith.compile(mm, dynamic=dynamic)
        ...
        smith.testing.assert_close(
            out1, expected1_fp64.to(smith.float32), atol=1e-2, rtol=1e-2
        )
        smith.testing.assert_close(
            out2, expected2_fp64.to(smith.float32), atol=1e-2, rtol=1e-2
        )
    ...
    def test_max_autotune_contiguous_transform_with_epilogue(self):
        ...
        a = smith.randn(M, K, dtype=smith.float32, device=GPU_TYPE)
        b = smith.randn(N, K, dtype=smith.float32, device=GPU_TYPE)
        ...
        with (
            ...
            compiled_func = smith.compile(mm_transpose_relu)
            out, code = run_and_get_code(compiled_func, a, b)

            # Verify correctness against fp64 baseline
            smith.testing.assert_close(
                out, expected_fp64.to(smith.float32), atol=1e-2, rtol=1e-2
            )
    ...
    def test_override_template_heuristics_with_custom_class(self):
        ...
        """
        from smith._inductor.kernel.mm import MMKernelInputs
        from smith._inductor.template_heuristics.registry import (
            get_registered_heuristic_class,
            get_template_heuristic,
        )

        template_uid = smith._inductor.kernel.mm.mm_template.uid
        ...
        gm = make_fx(lambda: smith.zeros(2, 3))()
    ...
    def test_bmm_out_dtype(self):
        def f(a, b):
            return smith.bmm(a, b, out_dtype=smith.float32)

        a = smith.randn(2, 3, 4, device=GPU_TYPE, dtype=smith.float16)
        b = smith.randn(2, 4, 5, device=GPU_TYPE, dtype=smith.float16)
        with config.patch(
            max_autotune=True,
            max_autotune_gemm_backends="TRITON",
        ):
    ...
    def test_triton_template_generated_code_cache_key(self):
        generate_and_load_args = len(
            inspect.signature(
                smith._inductor.select_algorithm.TritonTemplate.generate_and_load
            ).parameters
        )
        make_key_args = len(
            inspect.signature(
                smith._inductor.select_algorithm.GeneratedCodeCache.make_key
            ).parameters
        )
    ...
    def test_triton_template_generated_code_cache_strategy(self):
        ...
        a = smith.rand(10, 22, device=GPU_TYPE)
        b = smith.rand(22, 30, device=GPU_TYPE)
        # Test that the testing strategy works by overriding input_dependent_preserved_state and simulate a cache hit.
        with unittest.mock.patch(
            "smith._inductor.select_algorithm.TritonTemplateKernel.input_dependent_preserved_state",
            ...
            with self.assertRaisesRegex(
                ...
                smith.compile(func_test1, dynamic=False)(a, b, a, b)
    ...
    def test_triton_template_generated_code_caching(self):
        ...
        def hits():
            return smith._dynamo.utils.counters["inductor"][
        ...
        with fresh_cache():
            reset_counters()
            compile_results = smith.compile(func_test1, dynamic=False)(a, b, a, b)
            ...
            if not TEST_WITH_ROCM:
                expected = """{
                        'input_nodes':[
                            "[[10,22],[22,1],smith.float32,device(type='cuda',index=0),0]",
                            "[[22,30],[30,1],smith.float32,device(type='cuda',index=0),0]"],
                        'num_stages':1,'num_warps':2,'prefix_args':0,'suffix_args':0,'call_sizes':[10,30],
                        'layout':"[[10,30],[30,1],smith.float32,device(type='cuda',index=0),0]",
        ...
        with fresh_cache():
            ...
            c = smith.rand(9, 21, device=GPU_TYPE)
            d = smith.rand(21, 30, device=GPU_TYPE)
            reset_counters()
            compiled_results = smith.compile(func_test1, dynamic=True)(a, b, c, d)
            ...
            if not TEST_WITH_ROCM:
                expected = """{
                    'input_nodes':[
                        "[[s77,s27],[s27,1],smith.float32,device(type='cuda',index=0),0]",
                        "[[s27,s94],[s94,1],smith.float32,device(type='cuda',index=0),0]"],
                    'num_stages':1,'num_warps':2,'prefix_args':0,'suffix_args':0,'call_sizes':[s77,s94],
                    'layout':"[[s77,s94],[s94,1],smith.float32,device(type='cuda',index=0),0]",'num_consumer_groups':0,
        ...
        with fresh_cache():
            ...
            compile_results = smith.compile(func_test1, dynamic=True)(a, b, a, b)
            eager_results = func_test1(a, b, a, b)
            self.assertEqual(compile_results, eager_results, atol=0.05, rtol=0.05)
        ...
        with fresh_cache():
            reset_counters()
            input = smith.rand(10, 10, device=GPU_TYPE)

            compile_results = smith.compile(test_func2, dynamic=False)(input)
        ...
        with fresh_cache():
            ...
            compile_results = smith.compile(test_func2, dynamic=True)(input)
            eager_results = test_func2(input)
            self.assertEqual(compile_results, eager_results, atol=0.05, rtol=0.05)
        ...
        def test_func3(x, y, z, m, l):
            a = smith.matmul(x, y)
            b = smith.matmul(smith.cat([x, z], 1), smith.cat([y, m, l], 0))
            return a, b

        with fresh_cache():
            a = smith.rand(10, 22, device=GPU_TYPE)
            b = smith.rand(22, 30, device=GPU_TYPE)
            c = smith.rand(10, 11, device=GPU_TYPE)
            d = smith.rand(8, 30, device=GPU_TYPE)
            e = smith.rand(3, 30, device=GPU_TYPE)

            compile_results = smith.compile(test_func3, dynamic=True)(a, b, c, d, e)
    ...
    def test_triton_template_generated_code_caching_bmm(self):
        ...
        a = smith.rand(10, 10, 22, device=GPU_TYPE)
        b = smith.rand(10, 22, 30, device=GPU_TYPE)
    ...
    def test_triton_template_generated_code_caching_mm_plus_mm(self):
        ...
        a = smith.rand(10, 40, device=GPU_TYPE)
        b = smith.rand(40, 30, device=GPU_TYPE)
    ...
    def test_max_autotune_decompose_k_envvars(
        ...
        for M, N, K in shapes:
            ...
            a = smith.randn(M, K, dtype=smith.float16, device=GPU_TYPE)
            b = smith.randn(K, N, dtype=smith.float16, device=GPU_TYPE)
            ...
            with config.patch(
                ...
                compiled_func = smith.compile(lambda a, b: a @ b)
    ...
    def test_max_autotune_exhaustive(self):
        ...
        a = smith.randn(M, K, dtype=smith.float16, device=GPU_TYPE, requires_grad=True)
        b = smith.randn(K, N, dtype=smith.float16, device=GPU_TYPE, requires_grad=True)

        with mock.patch(
            "smith._inductor.template_heuristics.registry.get_template_heuristic"
        ) as config_mock:
            config_heuristics = (
                XPUMMTemplateConfigHeuristic()
                if GPU_TYPE == "xpu"
    ...
    def test_mm_k_1(self):
        ...
        for i in range(90, 100):
            smith._dynamo.reset()
            a = smith.randn((i, 1), device=GPU_TYPE, dtype=smith.float32)
            b = smith.randn((1, i), device=GPU_TYPE, dtype=smith.float32)
            compiled_f = smith.compile(mm)

            out, code = run_and_get_code(compiled_f, a, b)
            smith.testing.assert_close(out, mm(a, b), atol=1e-2, rtol=1e-2)
    ...
    def test_autotune_gemm_choice_validation(self, op, max_autotune):
        def generate_inputs_and_func(op_name):
            ...
            base_inputs = [
                smith.randn(128, 256, device=GPU_TYPE),
                smith.randn(256, 128, device=GPU_TYPE),
            ]
            func = smith.mm
            if op_name == "mm":
                # default
                ...
                # Add bias for addmm
                base_inputs = [smith.randn(128, device=GPU_TYPE)] + base_inputs
                func = smith.addmm
            elif op_name in ["bmm", "baddbmm"]:
                # Override for batch dimensions
                base_inputs[0] = smith.randn(4, 128, 256, device=GPU_TYPE)
                base_inputs[1] = smith.randn(4, 256, 128, device=GPU_TYPE)
                func = smith.bmm
                if op_name == "baddbmm":
                    # Add batch bias
                    base_inputs = [
                        smith.smith.randn(4, 128, 128, device=GPU_TYPE)
                    ] + base_inputs
                    func = smith.baddbmm
                ...
                def mmpmm(x, w, x2, w2):
                    return smith.mm(x, w) + smith.mm(x2, w2)
        ...
        try:
            with config.patch({"max_autotune": max_autotune}):
                compiled_fn = smith.compile(fn, dynamic=False)
                compiled_fn(*inputs)
    ...
    def test_autotune_layout_optimization(self, max_autotune_enabled):
        ...
        try:
            bias = smith.randn(64, device=GPU_TYPE)
            x = smith.randn(32, 128, device=GPU_TYPE)
            w = smith.randn(128, 64, device=GPU_TYPE)

            with config.patch({"max_autotune": max_autotune_enabled}):
                compiled_fn = smith.compile(lambda b, x, w: smith.addmm(b, x, w))
    ...
    def test_fixed_layout_at_lowering(self):
        ...
        def mm_func(a, b) -> smith.Tensor:
            a_t = smith.permute(a, [1, 0]).to(smith.bfloat16)
            b_dtype = b.to(smith.bfloat16)
            # Add .to() to make sure that mm could be potentially padded
            # Strides for output are not padded
            return (a_t @ b_dtype).to(smith.float32)

        def addmm_func(a, b, bias) -> smith.Tensor:
            a_t = smith.permute(a, [1, 0]).to(smith.bfloat16)
            b_dtype = b.to(smith.bfloat16)
            bias_dtype = bias.to(smith.bfloat16)
            return smith.addmm(bias_dtype, a_t, b_dtype).to(smith.float32)

        def bmm_func(a, b) -> smith.Tensor:
            a_t = smith.permute(a, [2, 0, 1]).to(smith.bfloat16)
            b_dtype = b.to(smith.bfloat16)
            return smith.bmm(a_t, b_dtype).to(smith.float32)

        def mm_plus_mm_func(a1, b1, a2, b2) -> smith.Tensor:
            a1_t = smith.permute(a1, [1, 0]).to(smith.bfloat16)
            b1_dtype = b1.to(smith.bfloat16)
            a2_t = smith.permute(a2, [1, 0]).to(smith.bfloat16)
            b2_dtype = b2.to(smith.bfloat16)
            return (a1_t @ b1_dtype + a2_t @ b2_dtype).to(smith.float32)

        a = smith.randn((4608, 512), device=GPU_TYPE, dtype=smith.bfloat16)
        b = smith.randn((4608, 1490), device=GPU_TYPE)
        bias = smith.randn(1490, device=GPU_TYPE)

        a_bmm = smith.randn((512, 4608, 8), device=GPU_TYPE, dtype=smith.bfloat16)
        b_bmm = smith.randn((8, 4608, 1490), device=GPU_TYPE)

        # Test mm_plus_mm
        a2 = smith.randn((4608, 512), device=GPU_TYPE, dtype=smith.bfloat16)
        b2 = smith.randn((4608, 1490), device=GPU_TYPE)
        ...
        for f, args in funcs_and_args:
            c_f = smith.compile(f, mode="max-autotune-no-cudagraphs")
...
class TestTemplateConfigPruning(TestCase):
    ...
    def create_test_tensors(
        ...
        if mat1_transposed:
            mat1 = smith.randn(K, M, dtype=dtype, device=GPU_TYPE).t()
        else:
            mat1 = smith.randn(M, K, dtype=dtype, device=GPU_TYPE)

        if mat2_transposed:
            mat2 = smith.randn(N, K, dtype=dtype, device=GPU_TYPE).t()
        else:
            mat2 = smith.randn(K, N, dtype=dtype, device=GPU_TYPE)

        if include_bias:
            bias_1d = smith.randn(N, dtype=dtype, device=GPU_TYPE)
    ...
    def test_max_autotune_prune_choices(self):
        ...
        x = smith.rand([M, K], device=GPU_TYPE, dtype=smith.float32)
        y = smith.rand([K, N], device=GPU_TYPE, dtype=smith.float32)

        compiled_f = smith.compile(mm, mode="max-autotune")
        compiled_f(x, y)
    ...
    def test_shared_memory_pruning_addmm(
        ...
        if use_tma and (dtype == smith.float32 or not has_triton_tma_device()):
            return

        def addmm_op(bias, mat1, mat2):
            return smith.addmm(bias, mat1, mat2)
    ...
    def run_op_shared_mem_pruning_check(
        ...
        for c in self.gemm_configs:
            ...
            with (
                ...
                smith._dynamo.reset()
                counters.clear()
                compiled_fn = smith.compile(op, mode="max-autotune")
...
class TestMaxAutotunePrecompile(TestCase):
    ...
    def test_filled_cache_precompile(self):
        def fn(a, b, c):
            a = (a @ b) @ c
            a, b, c = (t.to(smith.float16) for t in [a, b, c])
            return (a @ b) @ c

        fn_c = smith.compile(mode="max-autotune-no-cudagraphs")(fn)
        inputs = [smith.rand([256, 256], device=GPU_TYPE) for _ in range(3)]
        from smith._dynamo.utils import counters
    ...
    def test_precompilations(self):
        ...
        smith.testing.assert_close(fn_c(*inputs), fn(*inputs), atol=1e-2, rtol=1e-2)
...
class TestMaxAutotuneSubproc(TestCase):
    def _create_buffer(self, name, shape):
        return Buffer(
            name=name,
            layout=FixedLayout(
                smith.device(f"{GPU_TYPE}:0"), dtype=smith.float32, size=shape
            ),
        )
    ...
    def test_benchmark_choice_in_subproc(self):
        ...
        with V.set_graph_handler(graph):
            ...
            layout = FixedLayout(smith.device(f"{GPU_TYPE}:0"), smith.float32, (2, 2))
            ...
            timings = smith.zeros(3, dtype=smith.float32)
    ...
    def test_max_autotune_mm_plus_mm(self, autotune_in_subproc, autotune_multi_device):
        ...
        with config.patch(
            ...
            smith.compile(mm_plus_mm)(a, b, c, d)
    ...
    def test_max_autotune_regular_mm(self, dynamic: bool):
        ...
        a = smith.randn(100, 10).to(GPU_TYPE)
        b = smith.randn(10, 100).to(GPU_TYPE)
    ...
    def test_max_autotune_addmm(self, search_space, dynamic=False):
        ...
        with config.patch(
            ...
            Y_compiled = smith.compile(addmm, dynamic=dynamic)(x, a, b)
            Y = addmm(x, a, b)
            smith.testing.assert_close(Y_compiled, Y, atol=1e-2, rtol=1e-2)

    def test_triton_template_with_epilogues_and_dynamic_shape(self):
        def fn(
            x: smith.Tensor, w: smith.Tensor, bias: smith.Tensor, mul: smith.Tensor
        ) -> smith.Tensor:
            return (
                smith.nn.functional.relu(
                    smith.matmul(smith.transpose(x, 0, 1), smith.transpose(w, 0, 1))
                    + bias
                )
                * mul
            )
        ...
        K = 4
        N = 3
        w = smith.rand(N, K).to(GPU_TYPE).half()
        b = smith.rand(N).to(GPU_TYPE).half()
        ...
        with config.patch(
            ...
            x0 = smith.rand(K, M0).to(GPU_TYPE).half()
            mul0 = smith.rand(M0, N).to(GPU_TYPE).half()
            y0 = compiled_fn(x0, w, b, mul0)
            y0_expected = fn(x0, w, b, mul0)
            smith.testing.assert_close(y0, y0_expected)

            x1 = smith.rand(K, M1).to(GPU_TYPE).half()
            mul1 = smith.rand(M1, N).to(GPU_TYPE).half()
            y1 = compiled_fn(x1, w, b, mul1)
            y1_expected = fn(x1, w, b, mul1)
            smith.testing.assert_close(y1, y1_expected)
...
class TestMaxAutotuneRemoteCache(TestCase):
    ...
    def test_max_autotune_remote_caching(self, dynamic: bool):
        ...
        x = smith.randn(100, 100).to(GPU_TYPE)
        y = smith.randn(100, 100).to(GPU_TYPE)
        ...
        with (
            ...
            with config.patch({"max_autotune": True}):
                for _ in range(4):
                    ...
                    reset()
                with (
                    smith.compiler.config.patch({"cache_key_tag": "test"}),
                    fresh_cache(),
                ):
                    smith.compile(mm, dynamic=dynamic)(a, b)
                    reset()
            ...
            for _ in range(4):
                with fresh_cache():
                    smith.compile(f, dynamic=dynamic)(x, y)
                reset()
            with smith.compiler.config.patch({"cache_key_tag": "test"}), fresh_cache():
                smith.compile(mm, dynamic=dynamic)(a, b)
                reset()
            global_stats.report()
            self.assertEqual(global_stats.autotune_remote, Stats(2, 3, 2))
...
class TestTuningProcessPool(TestCase):
    ...
    def test_tuning_pool_multiple_devices(self):
        ...
        if CUDA_VISIBLE_DEVICES in os.environ:
            ...
            visible_devices = [str(d) for d in range(smith.cuda.device_count())]
    ...
    def test_add_feedback_saver(self):
        ...
        from smith._inductor.select_algorithm import get_algorithm_selector_cache
    ...
    def test_feedback_saver_integration(self):
        ...
        a = smith.randn(32, 32, device=GPU_TYPE)
        b = smith.randn(32, 32, device=GPU_TYPE)
...
class TestPrologueFusion(TestCase):
    ...
    def test_upcast(self, sizes):
        ...
        x = smith.rand([M, K], dtype=smith.float16, device=GPU_TYPE)
        y = smith.rand([K, N], dtype=smith.float, device=GPU_TYPE)

        def foo(x, y):
            return x.to(y.dtype) @ y

        out, code = run_and_get_code(smith.compile(foo), x, y)
    ...
    def test_gather_fusion(self):
        M, K, N = (64, 128, 256)
        x = smith.rand([M, K], dtype=smith.float16, device=GPU_TYPE)
        y = smith.rand([K, N], dtype=smith.float16, device=GPU_TYPE)

        index = smith.randperm(M, device=GPU_TYPE)

        def foo(x, y, index):
            return (x[index]) @ y

        out, code = run_and_get_code(smith.compile(foo), x, y, index)
    ...
    def test_low_precision(self):
        ...
        x = smith.rand([M, K], device=GPU_TYPE).to(smith.float8_e4m3fn)
        y = smith.rand([K, N], dtype=smith.bfloat16, device=GPU_TYPE)
    ...
    def test_downcast(self):
        ...
        M, K, N = (64, 128, 256)
        x = smith.rand([M, K], dtype=smith.float, device=GPU_TYPE)
        y = smith.rand([K, N], dtype=smith.float16, device=GPU_TYPE)
    ...
    def test_multiple_fusions(self, sizes, use_async_compile: bool):
        ...
        if use_async_compile:
            smith._inductor.async_compile.AsyncCompile.wait_pool_ready()
    ...
    def test_pending_fusions_multiple(self, use_async_compile: bool):
        ...
        x = smith.rand([128, 16], device=GPU_TYPE)
        y = smith.rand([128, 32], device=GPU_TYPE)

        out, code = run_and_get_code(smith.compile(multi_use), x, y)
        ...
        x = smith.rand([128, 128], device=GPU_TYPE)
        out, code = run_and_get_code(smith.compile(resolve_pending), x)
        FileCheck().check(get_func_call()).check_count(
    ...
    def test_pending_fusion_pro_and_epi(self, use_async_compile: bool):
        ...
        x = smith.rand([128, 128], dtype=smith.float16, device=GPU_TYPE)
        out, code = run_and_get_code(smith.compile(test_multiple_fusions), x)
        FileCheck().check(get_func_call()).check_count(
            get_kernel_launch(), 1, exactly=True
        ).run(code[0])
    ...
    def test_multiple_inputs(self, sizes, use_async_compile: bool):
        ...
        def foo(x, y, z):
            return (x + y).to(smith.float) @ z

        if use_async_compile:
            smith._inductor.async_compile.AsyncCompile.wait_pool_ready()

        x = smith.rand([M, K], dtype=smith.float16, device=GPU_TYPE)
        y = smith.rand([M, K], dtype=smith.float16, device=GPU_TYPE)
        z = smith.rand([K, N], dtype=smith.float, device=GPU_TYPE)
        out_eager = foo(x, y, z)
        out, code = run_and_get_code(smith.compile(foo), x, y, z)
    ...
    def test_storage_offset_prologue(self):
        def foo(a):
            q = a[:64, :]
            k = a[64:, :]
            return smith.mm(q + 2, k - 2)

        inp = smith.randn(128, 64, device=GPU_TYPE)
    ...
    def test_broadcast_x(self, K):
        ...
        x = smith.rand([1, 1], dtype=smith.float, device=GPU_TYPE)
        y = smith.rand([K, 128], dtype=smith.float, device=GPU_TYPE)

        out, code = run_and_get_code(smith.compile(foo, dynamic=True), x, y)
        self.assertEqual(out, foo(x, y), atol=0.05, rtol=0.05)
        self.check_code(code[0], num_kernels=1, num_allocs=1, num_deallocs=2)
    ...
    def test_preserves_zero_analysis(self):
        ...
        for fn, should_mask in fns:
            x = smith.rand([64, 127], dtype=smith.float, device=GPU_TYPE)
            y = smith.rand([127, 64], dtype=smith.float, device=GPU_TYPE)

            out, code = run_and_get_code(smith.compile(foo), x, y, fn)
    ...
    def test_prologue_read_into_both_inputs(
        ...
        with config.patch(benchmark_epilogue_fusion=benchmark_fusion):
            ...
            out, code = run_and_get_code(smith.compile(foo), x)
    ...
    def test_mismatched_prologue_group(self):
        ...
        x = smith.rand([1, 256], device=GPU_TYPE)
        y = smith.rand([256, 256], device=GPU_TYPE)
        z = smith.rand([256, 128], device=GPU_TYPE)
    ...
    def test_lazy_template_fusion_multiple_candidates(self, use_async_compile: bool):
        ...
        def foo(a, b, c, d):
            ...
            a_transformed = a.to(smith.float) + 1.0
            b_transformed = b.to(smith.float) * 2.0

            # Prologues for second matmul: transform inputs c and d
            c_transformed = c.to(smith.float) - 0.5
            d_transformed = d.to(smith.float) / 2.0
        ...
        M, K, N = 64, 128, 64
        a = smith.rand([M, K], dtype=smith.bfloat16, device=GPU_TYPE)
        b = smith.rand([K, N], dtype=smith.bfloat16, device=GPU_TYPE)
        c = smith.rand([M, K], dtype=smith.bfloat16, device=GPU_TYPE)
        d = smith.rand([K, N], dtype=smith.bfloat16, device=GPU_TYPE)

        _, code = run_and_get_code(smith.compile(foo), a, b, c, d)
    ...
    def test_prologue_masked_load(self, sizes):
        ...
        x = smith.rand([250, 245], device=GPU_TYPE)
        y = smith.rand([245, 128], device=GPU_TYPE)
...
class TestEpilogueFusionStaticAnalysis(TestCase):
    ...
    def get_common_patches(self, async_compile: bool, persistent_tma: bool):
        common_patches = (
            ...
            mock.patch(
                "smith._inductor.kernel.mm.autotune_select_algorithm",
    ...
    def test_template_bad_epilogue_fusion(self, use_async_compile: bool):
        def f(a, b):
            return (a @ b).to(smith.float32)

        a = smith.randn(512, 1152, device=GPU_TYPE, dtype=smith.bfloat16)
        b = smith.randn(1152, 7680, device=GPU_TYPE, dtype=smith.bfloat16)
        ...
        for gemm_config in [bad_tma_config, good_tma_config]:
            ...
            try:
                with (
                    ...
                    mock.patch(
                        "smith._inductor.autotune_process.run_autotune_in_subprocess",
                    ...
                    compiled_f = smith.compile(f, mode="max-autotune")
                    out, code = run_and_get_code(compiled_f, a, b)
                    ...
                    if not config.cpp_wrapper:
                        smith.testing.assert_close(out, f(a, b), atol=1e-2, rtol=1e-2)
            finally:
                # Restore original configs
    ...
    def test_template_epilogue_fusion_static_analysis(
        ...
        def f(a, b):
            return (a @ b).to(smith.float32) + 1.0

        a = smith.randn(512, 1024, device=GPU_TYPE, dtype=smith.bfloat16)
        b = smith.randn(1024, 2048, device=GPU_TYPE, dtype=smith.bfloat16)
    ...
    def test_template_epilogue_fusion_extra_reads(
        ...
        x = smith.randn(512, 1024, device=GPU_TYPE, dtype=smith.bfloat16)
        w = smith.randn(1024, 2048, device=GPU_TYPE, dtype=smith.bfloat16)
        bias = smith.randn(512, 2048, device=GPU_TYPE, dtype=smith.bfloat16)
        scale = smith.randn(512, 2048, device=GPU_TYPE, dtype=smith.bfloat16)
        ...
        from smith._inductor.scheduler import BaseSchedulerNode
...
class TestMaxAutotuneAsyncPipelined(TestMaxAutotune, TestEpilogueFusionStaticAnalysis):
    ...
    def test_async_autotuner_cache_same_inputs(self):
        ...
        def three_matmuls(a1, b1, a2, b2, a3, b3):
            return smith.mm(a1, b1), smith.mm(a2, b2), smith.mm(a3, b3)

        # Same shapes for first two matmuls
        a1 = smith.randn(M, K, device=GPU_TYPE, dtype=smith.bfloat16)
        b1 = smith.randn(K, N, device=GPU_TYPE, dtype=smith.bfloat16)
        a2 = smith.randn(M, K, device=GPU_TYPE, dtype=smith.bfloat16)
        b2 = smith.randn(K, N, device=GPU_TYPE, dtype=smith.bfloat16)

        # Different shapes for third matmul
        a3 = smith.randn(M2, K2, device=GPU_TYPE, dtype=smith.bfloat16)
        b3 = smith.randn(K2, N2, device=GPU_TYPE, dtype=smith.bfloat16)

        compiled_fn = smith.compile(three_matmuls)
        result = compiled_fn(a1, b1, a2, b2, a3, b3)

        # Verify correctness
        expected = three_matmuls(a1, b1, a2, b2, a3, b3)
        for r, e in zip(result, expected):
            smith.testing.assert_close(r, e, atol=1e-2, rtol=1e-2)
