...
from smith._inductor.ir import ComputedBuffer, FixedLayout, PermuteView, Pointwise
...
if try_import_cutlass():
    ...
    from smith._inductor.codegen.cutlass.lib_extensions.evt_extensions import (
        ...
        for name, tensor in example_tensors.items():
            if isinstance(tensor, CutlassTensor):
                name_to_buffer[name] = MockComputedBuffer(
                    name, None, smith.float32, tensor.shape, tensor.stride
                )
...
class TestCutlassEVT(TestCase):
    ...
    def test_py_codegen_accumulator_return(self):
        from smith._inductor.codegen.cutlass.python_evt import CutlassEVTCodegen
        from smith._inductor.virtualized import V

        size = (100, 300, 200)
        buf0 = MockComputedBuffer("buf0", None, smith.float32, size)
        buf1 = MockComputedBuffer("buf1", None, smith.float32, size)
        buf2 = MockComputedBuffer("buf2", None, smith.float32, size)
        ...
        buf3 = MockComputedBuffer("buf3", inner_fn_buf3, smith.float32, size)
        buf4 = MockComputedBuffer("buf4", inner_fn_buf4, smith.float32, size)
...
index strides [200, 60000, 1], and layout stride [60000, 200, 1]""",
    ...
    def test_py_codegen_broadcasting(self):
        ...
        buf4 = MockComputedBuffer(
            "buf4", inner_fn_buf4, smith.float32, (100, 300, 1)
        )  # broadcast
        with V.set_graph_handler(
            MockGraphHandler(
                {"buf0": buf0, "buf1": buf1, "buf2": buf2, "buf3": buf3, "buf4": buf4}
    ...
    def test_example_tensor_creation(self):
        ...
        with V.set_graph_handler(MockGraphHandler({})):
            row_major_buf0 = MockComputedBuffer(
                "buf0", None, smith.float32, (3, 4, 1), (4, 1, 0)
            )
            col_major_buf1 = MockComputedBuffer(
                "buf1", None, smith.float32, (3, 2, 1), (1, 3, 0)
            )
            ...
            self.assertEqual(
                result["acc"].element, smith_dtype_to_cutlass_type(smith.float32)
            )

            self.assertEqual(result["buf1"].shape, (3, 2, 1))
            self.assertEqual(result["buf1"].stride, (1, 3, 0))
            self.assertEqual(
                result["buf1"].element, smith_dtype_to_cutlass_type(smith.float32)
            )
