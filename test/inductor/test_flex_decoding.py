...
if TEST_ON_CUDA:
    ...
    test_dtypes_fast = [smith.float16]
    SKIP_UT_ON_CPU = False
elif TEST_ON_XPU:
    smith._C._set_onednn_allow_tf32(True)
    test_device = ("xpu",)
    test_dtypes = [smith.float32, smith.bfloat16, smith.float16]
    test_dtypes_fast = [smith.float16]
    SKIP_UT_ON_CPU = False
else:
    test_device = ("cpu",)
...
def _generate_windowed(offset):
    def _windowed(score, b, h, q, kv):
        return smith.where(q + offset >= kv, score, float("-inf"))

    return _windowed


def _get_windowed_sdpa_mask(Mq, Mkv, offset):
    return smith.tril(smith.ones(Mkv, Mkv, dtype=smith.bool, device=test_device[0]))[
        offset : offset + Mq
    ]
...
def _head_offset(dtype: smith.dtype):
    """Captured Buffer"""
    head_offset = smith.rand(Hq, device=test_device[0], dtype=dtype)
...
class TestFlexDecoding(InductorTestCase):
    ...
    def _check_equal(
        ...
        if smith.isnan(compiled_error).any() and not smith.isnan(ref_error).any():
            self.assertTrue(False, "Output/Grad with NaN")
        if ref_error < (1e-4) * golden_out.abs().mean():
            print(
                "very small ref error of ",
                (ref_error.to(smith.float64) * (1e5) / golden_out.abs().mean()),
            )
    ...
    def preprocess_paged_attention(
        ...
        batch_reserve(
            paged_attention,
            smith.tensor([KV_S // 4, KV_S // 2, KV_S // 4, KV_S // 3], device=device),
        )
        batch_reserve(
            paged_attention,
            smith.tensor([KV_S // 4, KV_S // 2, KV_S // 2, KV_S // 2], device=device),
        )
        batch_reserve(
            paged_attention,
            smith.tensor([KV_S // 2, KV_S, KV_S // 2, KV_S], device=device),
        )
        batch_reserve(
            paged_attention, smith.tensor([KV_S, KV_S, KV_S, KV_S], device=device)
        )
    ...
    def run_test_with_call_paged_attention(
        ...
        golden_call = functools.partial(
            smith.nn.functional.scaled_dot_product_attention, attn_mask=sdpa_mask
        )
        golden_out = golden_call(q_gold, k_gold, v_gold)
        ref_out = golden_call(q_ref, k_ref, v_ref)
    ...
    def test_builtin_score_mods(
        self, device, dtype: smith.dtype, score_mod: Callable, head_dims
    ):
        Hq, Hkv = head_dims
        assert Hq % Hkv == 0
    ...
    def test_paged_attention_page_size(
        ...
        def generate_causal_offset(offset: smith.Tensor):
        ...
        mod = generate_causal_offset(
            smith.tensor(192, device=device, dtype=smith.int32)
        )
        block_mask = create_block_mask(
    ...
    @common_utils.parametrize("dtype", test_dtypes_fast)
    def test_tma_decoding(self, device, dtype: smith.dtype):
    ...
    @common_utils.parametrize("head_dims", test_Hq_Hkv)
    def test_strided_inputs(self, device, dtype: smith.dtype, k_s, v_s, head_dims):
        Hq, Hkv = head_dims
        assert Hq % Hkv == 0
        q1 = smith.randn((B * Hq * D), dtype=dtype, device=device)
        k1 = smith.randn((B * Hkv * S * D * 4), dtype=dtype, device=device)
        v1 = smith.randn((B * Hkv * S * D * 4), dtype=dtype, device=device)
        ...
        k = smith.as_strided(k1, k_shape, k_strides, k_offset)
        ...
        v = smith.as_strided(v1, v_shape, v_strides, v_offset)
    ...
    @supported_platform
    @common_utils.parametrize("dtype", test_dtypes)
    def test_captured_buffers(self, device, dtype: smith.dtype):
        head_offset = smith.rand(Hq, device=device, dtype=dtype)
    ...
    def test_captured_buffers_all_dims(self, device, dtype: smith.dtype):
        head_scale = smith.randn(Hq, device=device)
        batch_scale = smith.randn(B, device=device)
        kv_scale = smith.randn(S, device=device)
        q_scale = smith.randn(1, device=device)
    ...
    def test_non_divisible_offset_mask(self, device):
        KV_S = S - 3
        offset_tensor = smith.tensor(S // 2 - 3, device=device, dtype=smith.int32)
    ...
    def test_non_divisible_multi_token_offset_mask(self, device):
        KV_S = S - 3
        Q_S = 3
        offset_tensor = smith.tensor(S // 2 - 1, device=device, dtype=smith.int32)
    ...
    def test_load_from_bias_seq_only(self, device, dtype):
        bias = smith.randn(1, S, device=device, dtype=dtype)
    ...
    def test_load_from_bias_seq_batch(self, device, dtype):
        bias = smith.randn(B, 1, S, device=device, dtype=dtype)
    ...
    def test_head_dependent_mask_mod(
        self, device, dtype: smith.dtype, score_mod, head_dims
    ):
        Hq, Hkv = head_dims
        assert Hq % Hkv == 0

        def head_attention_mod(kv_head_num):
            head_type = smith.tensor(
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1):
    ...
    def test_multiple_score_mod_calls(self, device):
        query = smith.randn((1, 8, 4, 64), dtype=smith.float32, device=device)
        keys = [
    ...
    def test_multiple_score_mod_calls_paged_attention(self, device):
        ...
        compiled_out = smith.compile(paged_f)(
    ...
    def test_mixed_dtypes(self, device):
        ...
        query = smith.randn((1, 1, 8, 64), dtype=dtype_high, device=device)
    ...
    def test_mixed_dtypes_sqnr_per_tensor(self, device):
        query_ref = smith.testing.make_tensor(
            (1, 1, 8, 64), dtype=smith.bfloat16, device=device
        )
    ...
    def test_mixed_dtypes_sqnr_per_head(self, device):
        query_ref = smith.testing.make_tensor(
            (1, 4, 8, 64), dtype=smith.bfloat16, device=device
        )
        key_ref = smith.testing.make_tensor(
    ...
    def test_fully_masked_out_rows_0_check_gqa(self, device):
        ...
        flex = smith.compile(flex_attention, dynamic=False)
        if not self.test_inference_only:
            out, lse = flex(
                query,
                key,
    ...
    def test_not_pw_of_two(self, device):
        query = smith.randn(1, 12, 1, 16, device=device)
        key = smith.randn(1, 2, 128, 16, device=device)
        value = smith.randn(1, 2, 128, 16, device=device)
    ...
    def test_do_not_trigger_dynamic_shapes_on_empty_block_mask(self, device):
        smith._dynamo.reset()
        H = Hq
        q = smith.randn(B, H, 1, D, device=device)
        for i in range(5):
            k = smith.randn(B, H, S + i, D, device=device)
            v = smith.randn(B, H, S + i, D, device=device)
            compiled_flex_attention = smith.compile(flex_attention)
            ...
            if i == 0:
                self.assertEqual(smith._dynamo.utils.counters["frames"]["ok"], 1)
            else:
                self.assertEqual(smith._dynamo.utils.counters["frames"]["ok"], 2)
    ...
    def test_multi_block_m(self, q_seq_len, device):
        ...
        flex_attention_compiled = smith.compile(flex_attention)

        dtype = smith.float
        q = smith.randn(B, NUM_Q_HEAD, q_seq_len, HEAD_DIM, device=device, dtype=dtype)
        k = smith.randn(
        ...
        smith.testing.assert_close(eager, out, atol=5e-3, rtol=5e-3)
    ...
    def test_larger_block_mask_bug(self, device, dtype):
        ...
        flex_attention_compiled = smith.compile(flex_attention, dynamic=False)

        # Create input tensors
        shape = (2, 1, 2, 16)
        q = smith.normal(0.0, 3.0, shape, device=device, dtype=dtype)
        k = smith.normal(0.0, 3.0, shape, device=device, dtype=dtype)
        v = smith.normal(0.0, 3.0, shape, device=device, dtype=dtype)
    ...
    def test_decode_at_different_input_position(
        ...
        ref_outs = smith.cat(ref_outs)
        golden_outs = smith.cat(golden_outs)
        ...
        for i, seq_len in enumerate(prefill_length):
            batch_idx = smith.tensor([i], device=device, dtype=smith.int32)
            input_pos = smith.arange(seq_len, device=device, dtype=smith.int32).view(
                1, seq_len
            )
            paged_cache.assign(
        ...
        input_pos = smith.tensor(prefill_length, device=device, dtype=smith.int32).view(
            max_batch_size, 1
        )
        ...
        paged_out = compiled_sdpa(
            smith.cat(queries, 0), k_cache, v_cache, block_mask=new_block_mask
        )
