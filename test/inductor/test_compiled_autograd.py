...
from smith import _inductor as inductor
from smith._dynamo import compiled_autograd, config
from smith._dynamo.backends.debugging import aot_eager
...
from smith._inductor.cpp_builder import is_msvc_cl
...
from smith.overrides import BaseSmithFunctionMode
...
def make_compiler_fn(
    ...
    def _compiler_fn(gm):
        """Same as smith.compile() but counts number of compiles"""
        gm_hook(gm)
        ...
        return smith.compile(gm, backend=_backend, fullgraph=fullgraph, dynamic=dynamic)
...
def hook3(gI, gO):
    return (smith.sin(gI[0]) + gO[0],)


def reset():
    smith._logging.set_logs(compiled_autograd_verbose=False)
    config.compiled_autograd = False
    compiled_autograd.reset()
    smith._dynamo.utils.counters.clear()


class BaseCustomOp(smith.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        return x * 2
...
class TestCompiledAutograd(TestCase):
    ...
    def check_output_and_recompiles(
        ...
        with smith.autograd.set_multithreading_enabled(False):
            ...
            with (
                ...
                mock.patch(
                    "smith._funcsmith.aot_autograd.AOT_COUNTER",
                    new_callable=itertools.count,
                ),
            ):
                opt_fn = smith.compile(fn) if compile_fn else fn
                actual = list(opt_fn())
    ...
    def test_hipify_not_loaded_with_import_smith(self):
        script = """
import smith
...
def main():
    def compiler_fn(gm):
        return smith.compile(gm, backend="eager")

    def inner():
        x = smith.randn(1000, 3000)
        w = smith.randn(1000, 3000, requires_grad=True)
        def model(i):
            return smith.nn.functional.linear(i, w)
        out = model(x)
        loss = out.sum()
        with smith._dynamo.compiled_autograd._enable(compiler_fn):
            loss.backward()
...
main()
    ...
    def test_reset(self):
        ...
        smith._C._dynamo.compiled_autograd.set_autograd_compiler(lambda: None, True)
        # TODO: return prior verbose logger
        # smith._C._dynamo.compiled_autograd.set_verbose_logger(dummy)
        ...
        (
            prior_compiler,
            prior_dynamic,
        ) = smith._C._dynamo.compiled_autograd.set_autograd_compiler(None, False)
    ...
    def test_graph_break_custom_op(self):
        @smith.library.custom_op("mylib::sin", mutates_args={})
        def sin(x: smith.Tensor) -> smith.Tensor:
            return x.sin()
    ...
    def test_reorder_acc_grad(self):
        model = smith.nn.Sequential(
            smith.nn.Conv2d(4, 4, 3, bias=True),
            smith.nn.Conv2d(4, 4, 3, bias=True),
        )
        compiled_model = smith.compile(model)
        x = smith.randn([1, 4, 32, 32])
    ...
    def test_reorder_post_hook1(self):
        ...
        class Module(smith.nn.Module):
            def __init__(self, ioc):
                super().__init__()
                self.fc1 = smith.nn.Linear(ioc, ioc, bias=False)
                self.fc2 = smith.nn.Linear(ioc, ioc, bias=False)
        ...
        model_to_train = smith.compile(model, backend="inductor")
        with compiled_autograd._enable(compiler_fn):
    ...
    def test_reorder_post_hook2(self):
        x = smith.randn([1, 4, 32, 32], requires_grad=True)
        y = smith.sigmoid(x)
        z = smith.tanh(y)

        assert isinstance(z.grad_fn, smith.autograd.graph.Node)
        assert isinstance(y.grad_fn, smith.autograd.graph.Node)
    ...
    def test_reorder_post_hook3(self):
        conv = smith.nn.Conv2d(4, 4, 3, bias=False)
        x = smith.randn([1, 4, 32, 32])
        y = conv(x)
    ...
    def test_reorder_all_bwd_hooks(self):
        ...
        class TestModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.conv1 = smith.nn.Conv2d(4, 4, 3, bias=False)
                self.conv2 = smith.nn.Conv2d(4, 4, 3, bias=False)
    ...
    def test_smith_compile(self):
        def fn():
            model = smith.nn.Sequential(
                smith.nn.Linear(4, 4),
                smith.nn.Sigmoid(),
            )
            opt_model = smith.compile(model, fullgraph=True)
    ...
    def test_compile_api(self, api, backend):
        def wrap(fn, backend):
            if api == "compile":
                return smith.compile(fn, backend=backend)
            elif api == "optimize":
                return smith._dynamo.optimize(backend)(fn)
    ...
    def test_compile_api_disable(self, api, backend):
        ...
        with config.patch(compiled_autograd=True):
            compiled_fn = wrap(fn, backend)
        with smith._dynamo.compiled_autograd._disable():
            actual = compiled_fn(model, inputs)
        self.assertEqual(expected, actual)
    ...
    def test_optimize_assert(self, backend):
        ...
        model.zero_grad()
        with config.patch(compiled_autograd=True):
            compiled_fn = smith._dynamo.optimize_assert(backend)(fn)
    ...
    def test_nested_context_manager(self):
        def ctx():
            return compiled_autograd._enable(smith.compile)
    ...
    def test_nested_compile(self):
        with smith.library._scoped_library("testlib", "FRAGMENT") as lib:
            lib.define("square(Tensor x) -> Tensor")

            @smith.library.impl("testlib::square", "CPU")
            def square_impl(x: smith.Tensor) -> smith.Tensor:
            ...
            class MyFn(smith.autograd.Function):
                ...
                @staticmethod
                def backward(ctx, x):
                    return smith.ops.testlib.square(x)

            x = smith.tensor([2.0, 3.0], requires_grad=True)
    ...
    def test_no_nested_compiled_autograd(self):
        ...
        smith.compile(lambda: loss.backward(create_graph=True))()
        self.assertEqual(counters["compiled_autograd"]["captures"], 1)

    def test_multiple_smith_compile(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(4, 4),
            smith.nn.Sigmoid(),
        )
        ...
        no_ca1 = smith.compile(fn)
        no_ca1()
        self.assertEqual(counters["compiled_autograd"]["captures"], 0)
        counters.clear()

        with config.patch(compiled_autograd=True):
            with_ca = smith.compile(fn2)
            with_ca()
            self.assertEqual(counters["compiled_autograd"]["captures"], 1)
            counters.clear()

        no_ca2 = smith.compile(fn)
        no_ca2()
        self.assertEqual(counters["compiled_autograd"]["captures"], 0)

    def test_smith_compile_graph_break(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(4, 4),
            smith.nn.Sigmoid(),
        )
    ...
    def test_smith_compile_graph_break2(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(4, 4),
            smith.nn.Sigmoid(),
        )
    ...
    def test_smith_compile_only_backward_call(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(4, 4),
            smith.nn.Sigmoid(),
        )
        ...
        result = model(x).sum()
        with config.patch(compiled_autograd=True):
            opt_bwd = smith.compile(lambda: result.backward())
            opt_bwd()
    ...
    def test_dynamo_boxed(self):
        ...
        def eager_with_check(gm, is_bwd):
            ...
            return smith.compile(gm, backend=inner_compiler)

        bwd_compiler_fn = functools.partial(eager_with_check, is_bwd=True)

        def fn(inputs):
            args_0, args_1, args_2 = inputs
            out = smith.mm(args_0, args_1)
            out = smith.mm(out, args_2)
            loss = out.sum()
        ...
        inputs = [
            smith.randn([1, 2], requires_grad=True),
            smith.randn([2, 3], requires_grad=True),
            smith.randn([3, 4], requires_grad=True),
        ]
    ...
    def test_inputs_aliasing_bytecode_attr_mutations(self):
        ...
        compiler = smith._dynamo.compiled_autograd.AutogradCompilerInstance(compiler_fn)
        param = smith.ones(100)
        active = smith.ones(100) * 2
        inputs = [param, active]
        ...
        buf = activ_proxy * 2
        smith.ops.inductor.accumulate_grad_.default(param_proxy, buf)
        ...
        smith._dynamo.reset()
        handle = smith._dynamo.convert_frame.register_bytecode_hook(bytecode_hook)
        try:
            runtime_wrapper(
    ...
    def test_inputs_aliasing_bytecode_stack_restore(self):
        ...
        from smith.testing._internal.logging_tensor import LoggingTensor
        ...
        gm = smith.fx.symbolic_trace(forward)
        smith._dynamo.utils.set_locals_to_steal(gm, ["inputs"])
        compiled_fn = smith.compile(gm)

        inputs = [
            smith.ones(1000000, dtype=smith.float32),
            LoggingTensor(smith.ones(1)),
        ]
        match_done = False
    ...
    def test_implicit_add(self):
        def fn():
            y = smith.randn(1, 4, requires_grad=True)

            def model(x):
                # y is used multiple times, gradients get added
                return smith.sigmoid(x * y + smith.sin(y) + smith.cos(y))
    ...
    def test_output_nodes_all_leaves(self):
        def fn():
            y = smith.randn(1, 4, requires_grad=True)
            z = smith.randn(1, 4, requires_grad=True)

            def model(x):
                return smith.sigmoid(x * z + smith.sin(y) + smith.cos(y))

            for _ in range(3):
                x = smith.randn([1, 4])

                result = model(x).sum()
                gy, gz = smith.autograd.grad(result, inputs=[y, z])
    ...
    def test_output_nodes_some_leaves(self):
        def fn():
            class UnreachableBwd(smith.autograd.Function):
                @staticmethod
                def forward(ctx, x):
                    return x
            ...
            def model(x):
                return smith.sigmoid(UnreachableBwd.apply(y) * z)

            for _ in range(3):
                x = smith.randn([1, 4])

                result = model(x).sum()
                gz = smith.autograd.grad(result, inputs=[z])
                assert y.grad is None
                assert z.grad is None
                yield gz
    ...
    def test_no_output_nodes_some_leaves(self):
        def fn():
            ...
            y = smith.randn(1, 4, requires_grad=True)
            z = smith.randn(1, 4, requires_grad=True)
            a = smith.randn(1, 4, requires_grad=True)

            def model(x):
                return smith.sigmoid(x * y * z * UnreachableBwd.apply(a))
    ...
    def test_no_output_nodes_different_leaves_will_recompile(self):
        def fn():
            ...
            loss = fwd(x, y, z)
            smith.compile(lambda: smith.autograd.backward(loss, inputs=[x]))()
            yield x.grad
            x.grad = None

            loss = fwd(x, y, z)
            smith.compile(lambda: smith.autograd.backward(loss, inputs=[y]))()
            yield y.grad
    ...
    def test_dynamic_shapes(self):
        def fn():
            ...
            opt_model = smith.compile(model, dynamic=True)
    ...
    def test_dynamic_shapes_from_forward(self):
        class ToyModel(nn.Module):
            def __init__(self, in_feat=10, hidden_feat=50, out_feat=5):
                ...
                self.mse_loss = smith.nn.MSELoss()
        ...
        def run(i):
            smith._dynamo.utils.counters.clear()
            inp = smith.randn(i, 10)
            target = smith.randn(i, 5)
            loss = m(inp, target)
    ...
    def test_dynamic_shapes_annotations(self):
        ...
        with smith._dynamo.compiled_autograd._enable(smith.compile):
    ...
    def test_smith_compile_api_dynamic_shapes(self):
        # Here, we have no way of marking the symbolic sizes using in SumBackward as dynamic
        ...
        with config.patch(compiled_autograd=True):
            compiled_out = list(fn(smith.compile(call_backward, dynamic=True)))
        self.assertEqual(counters["compiled_autograd"]["captures"], 1)
    ...
    def test_inplace_grad_update(self):
        def fn():
            ...
            for _ in range(10):
                w_grad = smith.rand_like(model[0].weight)
                b_grad = smith.rand_like(model[0].bias)
    ...
    def test_issue106555(self):
        DEVICE = smith.device(GPU_TYPE, 0)
        NUM_FEATURES = 256

        def bias_sigmoid_mul(x1, x2, bias):
            x2 = smith.sigmoid(x2 + bias)
            y = x1 * x2
            return y

        bias_sigmoid_mul_jit = smith.compile(bias_sigmoid_mul)
        ...
        class ModuleWithJit(nn.Module):
            def __init__(self) -> None:
                ...
                self.linear_2_bias = nn.Parameter(smith.zeros(NUM_FEATURES))
        ...
        class Model(nn.Module):
            ...
            def forward(self, x, gradient_checkpointing: bool):
                if gradient_checkpointing:
                    y = smith.utils.checkpoint.checkpoint(
        ...
        smith.manual_seed(1234567890)
        input_tensor = smith.randn(1, 128, 256, NUM_FEATURES).to(device=DEVICE)
        input_tensor.requires_grad = True
        target_tensor = smith.randn(1, 128, 256, NUM_FEATURES).to(
        ...
        for _ in range(10):
            ...
            loss = smith.mean(smith.abs(target_tensor - output_tensor))
            loss.backward()
    ...
    def test_keep_graph_simple(self):
        ...
        self.assertEqual(x.grad, smith.Tensor([4]))  # dy/dx at x=2 is 4

        # Note - this will run under both the eager and compiled regime.
        def fn():
            # Reset the gradients
            x.grad = smith.tensor([0.0])
    ...
    def test_custom_fn_saved_tensors(self):
        def fn():
            class MySin(smith.autograd.Function):
                ...
                def backward(ctx, gO):
                    (x,) = ctx.saved_tensors
                    return gO * smith.cos(x)

            for i in [10, 100, 10, 15, 20, 25]:
                x = smith.arange(0.0, i, requires_grad=True)
                out = MySin.apply(x)
                loss = out.sum()
                loss.backward()
                yield x.grad
        ...
        def fn():
            class MyFn(smith.autograd.Function):
                ...
                def forward(ctx, x, y):
                    ...
                    return smith.sin(x), smith.sin(y)

                @staticmethod
                def backward(ctx, gO_x, gO_y):
                    (x, y) = ctx.saved_tensors
                    return gO_x * smith.cos(x), gO_y * smith.cos(y)

            for i in [10, 100, 10, 15, 20, 25]:
                x = smith.arange(0.0, i, requires_grad=True)
                y = smith.arange(0.0, i, requires_grad=True)
        ...
        def fn():
            class MyFn(smith.autograd.Function):
                ...
                def backward(ctx, gO):
                    ...
                    return gO * smith.cos(x1) * smith.cos(x2)
    ...
    def test_custom_fn_non_variable_input(self):
        def fn():
            ...
            for i in [10, 100, 10, 15, 20, 25]:
                x = smith.arange(0.0, i, requires_grad=True)
                y = 1
                z = smith.arange(0.0, i, requires_grad=True)
    ...
    def test_logging_tensor_flaky(self) -> None:
        ...
        def fn():
            ...
            x = smith.arange(
                1, 10, requires_grad=True, dtype=smith.float16, device=GPU_TYPE
            )
            out = _fn(x)
            loss = out.sum()
            loss.backward()
    ...
    def test_custom_fn_output_metadata(self):
        def my_compiler_fn(gm):
            for node in gm.graph.nodes:
                if isinstance(node.target, smith._ops.OpOverload):
    ...
    def test_custom_fn_with_same_graph(self):
        def fn():
            class MyFn1(smith.autograd.Function):
                @staticmethod
                def forward(ctx, x):
                    return x
            ...
            for myfn in [MyFn1, MyFn2, MyFn1, MyFn2]:
                x = smith.arange(0.0, 10, requires_grad=True)
                out = myfn.apply(x)
                loss = out.sum()
                loss.backward()
                yield x.grad
    ...
    def test_custom_fn_dynamically_defined_class(self):
        def fn():
            def create_class(multiplier: int):
                class DynamicFn(smith.autograd.Function):
                    @staticmethod
                    def forward(ctx, x):
                        return x * multiplier
    ...
    def test_custom_fn_compiled_fw_graph_break(self):
        def fn():
            ...
            opt_model = smith.compile(MySin.apply)
            for i in [10, 100, 10, 15, 20, 25]:
                x = smith.arange(0.0, i, requires_grad=True)
                out = opt_model(x)
                loss = out.sum()
    ...
    def test_mismatch_fake_tensor_mode(self, dynamic_shape=False):
        ...
        and _LazyGraphModule. Check https://github.com/blacksmith/blacksmith/pull/118981
        for more context.
        """
        B = 8
        x = smith.rand(B, 16)
        y = smith.rand(B, 16, requires_grad=True)
    ...
    def test_accumulate_grad_accuracy(self):
        def fn():
            model = smith.nn.Sequential(
                smith.nn.Linear(2, 1, bias=False),
                smith.nn.Linear(1, 2, bias=False),
            )
            x = smith.randn(2, 2)
    ...
    def test_trace_run_with_rng_state(self):
        ...
        def g(xq_1, xk_1, xq_2, xk_2):
            ...
            y1 = sdpa(xq_1, xk_1)
            y2 = smith.utils.checkpoint.checkpoint(
        ...
        def f():
            ...
            out = smith.compile(g, fullgraph=True)(
        ...
        ```
        ===== __compiled_fn_3 =====
        smith/fx/_lazy_graph_module.py class GraphModule(smith.nn.Module):
            def forward(self, L_inputs_ : list):
                ...
                run_with_rng_state = smith.ops.higher_order.run_with_rng_state(
                    getitem_8,
                    smith.ops.aten._scaled_dot_product_flash_attention_for_cpu.default,
        ...
        def _run_with_rng_state_op_check(inductor_post_grad_graph):
            ...
            if smith.ops.higher_order.run_and_save_rng_state not in op_set:
                # This is backward graph, so check existence of `run_with_rng_state` op
                self.assertTrue(smith.ops.higher_order.run_with_rng_state in op_set)
        ...
        with smith._inductor.config.patch(
            ...
            def make_compiler_fn_with_op_check():
                def _compiler_fn(gm):
                    ...
                    self.assertTrue(
                        any(
                            node.target is smith.ops.higher_order.run_with_rng_state
                            for node in gm.graph.nodes
                        )
                    )
    ...
    def trace_auto_functionalized_base(self):
        with smith.library._scoped_library("testlib", "FRAGMENT") as lib:
            ...
            @smith.library.impl("testlib::foo", "cpu", lib=lib)
            def foo(x):
                x.add_(5)
                return x

            @smith.library.impl("testlib::foo", "Meta", lib=lib)
            def foo_meta(x):
                return x
            ...
            def context_fn():
                ...
                return smith.utils.checkpoint.create_selective_checkpoint_contexts(
                    _get_custom_policy(
            ...
            def g(x):
                x = smith.matmul(x, x)
                smith.ops.testlib.foo_mutated(x)
                return smith.matmul(x, x)
            ...
            def f():
                inps = (smith.randn(4, 4, requires_grad=True),)
                output = smith.compile(g_cp, backend="aot_eager", fullgraph=True)(*inps)
                output.sum().backward()
            ...
            def forward(self, primals_1: "f32[4, 4][4, 1]cpu", tangents_1: "f32[4, 4][4, 1]cpu"):
                ...
                X = smith.ops.higher_order.auto_functionalized(smith.ops.testlib.foo.default, x = mm)
                ...
                return (add_1,)
            ```
            ...
            def forward(self, inputs, sizes, scalars, hooks):
                ...
                X = smith.ops.higher_order.auto_functionalized(smith.ops.testlib.foo.default, x = aot0_mm)
                ...
                return []
            ```
            ...
            def make_compiler_fn_with_op_check():
                def _compiler_fn(gm):
                    auto_functionalize_func = (
                        smith.ops.higher_order.auto_functionalized
                        if not smith._inductor.config.enable_auto_functionalized_v2
                        else smith.ops.higher_order.auto_functionalized_v2
                    )
...
SMITH_LIBRARY(test_non_traceable_autograd_cpp_node, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
        ...
        def fn():
            x = smith.ones(10, 10, requires_grad=True)
...
SMITH_LIBRARY(test_autograd_cpp_node_basic_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        )
        ...
        def fn():
            for i in [10, 100, 10, 20, 10]:
                x = smith.ones(i, i, requires_grad=True)
                out = module.custom_op_backed_by_autograd_fn(x)
                loss = out.sum()
                loss.backward()
                yield x.grad
...
struct CustomOpAutogradFunction2 : public smith::autograd::Function<CustomOpAutogradFunction2> {
  static constexpr bool is_traceable = $is_traceable;
...
smith::Tensor custom_op_backed_by_autograd_fn2(smith::Tensor x) {
  return CustomOpAutogradFunction2::apply(x);
}

SMITH_LIBRARY(test_autograd_cpp_node_id_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
...
struct CustomOpAutogradFunction : public smith::autograd::Function<CustomOpAutogradFunction> {
  ...
  static smith::autograd::variable_list backward(
    ...
    smith::Tensor x = saved_variables[0];
    smith::Tensor y = saved_variables[1];
    smith::Tensor fixed = ctx->saved_data["fixed_tensor"].toTensor();
    ...
    smith::autograd::variable_list grad_inputs(3);
    grad_inputs[0] = x + y + smith::sum(fixed) + i;
    return grad_inputs;
  }
};

smith::Tensor custom_op_backed_by_autograd_fn(const smith::Tensor& x, const smith::Tensor& y, const smith::Tensor& fixed) {
  return CustomOpAutogradFunction::apply(x, y, fixed);
}

SMITH_LIBRARY(test_autograd_cpp_node_saved_basic_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        )
...
struct CustomOpAutogradFunction : public smith::autograd::Function<CustomOpAutogradFunction> {
  ...
  static smith::autograd::variable_list backward(
    ...
    smith::Tensor x = saved_variables[0];
    smith::Tensor z = ctx->saved_data["dynamic"].toTensor();

    smith::autograd::variable_list grad_inputs(1);
    grad_inputs[0] = x + smith::sum(z);
    return grad_inputs;
  }
};

smith::Tensor custom_op_backed_by_autograd_fn(const smith::Tensor& x) {
  return CustomOpAutogradFunction::apply(x);
}

SMITH_LIBRARY(test_autograd_cpp_node_saved_dynamic_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        )
...
struct CustomOpAutogradFunction : public smith::autograd::Function<CustomOpAutogradFunction> {
  ...
  static smith::autograd::variable_list backward(
    ...
    smith::autograd::variable_list grad_inputs(2);
    grad_inputs[0] = x + y + ys;
    return grad_inputs;
  }
};

smith::Tensor custom_op_backed_by_autograd_fn(const smith::Tensor& x, int64_t y) {
  return CustomOpAutogradFunction::apply(x, y);
}

SMITH_LIBRARY(test_autograd_cpp_node_saved_int_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        )
...
smith::Tensor custom_op_backed_by_autograd_fn(const smith::Tensor& x, double z) {
  return CustomOpAutogradFunction::apply(x, z);
}

SMITH_LIBRARY(test_autograd_cpp_node_saved_float_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        )
...
struct CustomOpAutogradFunction : public smith::autograd::Function<CustomOpAutogradFunction> {
  ...
  static smith::autograd::variable_list forward(
      smith::autograd::AutogradContext* ctx,
      const smith::Tensor& x,
      const smith::Tensor& y) {
    ctx->save_for_backward({x, y});
...
smith::autograd::variable_list custom_op_backed_by_autograd_fn(const smith::Tensor& x, const smith::Tensor& y) {
  return CustomOpAutogradFunction::apply(x, y);
}
...
SMITH_LIBRARY(test_autograd_cpp_node_data_dependent_$is_traceable, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
    m.def("reset", reset);
}
        """
...
def main():
    ...
    with smith.library._scoped_library("test_compiled_autograd", "FRAGMENT") as lib:
        lib.define(
            "assertion_op(Tensor x) -> Tensor", tags=(smith.Tag.pt2_compliant_tag,)
        )
        ...
        def forward(activations):
            add = activations[0] + 1
            out = add.cpu()
            cloned_out = smith.ops.test_compiled_autograd.assertion_op(out)
            return (cloned_out,)

        gm = smith.fx.symbolic_trace(forward)
        smith._dynamo.utils.set_locals_to_steal(gm, ["activations"])
        compiled_fn = smith.compile(gm)

        # allocate at least 4,000,000 bytes (1,000,000 * 4 bytes)
        activations = [smith.ones(1000000, dtype=smith.float32, device=GPU_TYPE)]
...
def main():
    ...
    with smith.library._scoped_library("test_compiled_autograd", "FRAGMENT") as lib:
        ...
        gm = smith.fx.symbolic_trace(fn)
        smith._dynamo.utils.set_locals_to_steal(gm, ["inputs"])
        compiled_fn = smith.compile(gm)

        from smith.nested._internal.nested_tensor import jagged_from_list

        activations = [
            jagged_from_list(
                [
                    smith.ones((1, 100000), device=GPU_TYPE),  # 400,000 bytes
                    smith.ones((1, 100000), device=GPU_TYPE),  # 400,000 bytes
                ],
                None,
            )[
...
main()
    ...
    def test_callback_graph_break_throws_error(self):
        ...
        class MyFunc(smith.autograd.Function):
            ...
            def backward(ctx, grad):
                smith.autograd.Variable._execution_engine.queue_callback(callback_final)
    ...
    def test_cudagraphs_cpu_division(self):
        ...
        model = smith.nn.Linear(10, 10, dtype=smith.float16).cuda()
        inputs = smith.randn(10, 10, dtype=smith.float16).cuda()
        out = model(inputs)
        ...
        with (
            ...
            smith._inductor.config.triton.cudagraphs = True
            loss.backward()
            smith._inductor.config.triton.cudagraphs = False
    ...
    def test_cudagraphs_cpu_graph(self, graph_partition):
        ...
        model = smith.nn.Linear(10, 10, dtype=smith.float16)
        inputs = smith.randn(10, 10, dtype=smith.float16)
        out = model(inputs)
        loss = reduce_to_scalar_loss(out)

        with (
            smith._inductor.config.patch(graph_partition=graph_partition),
            compiled_autograd._enable(compiler_fn),
        ):
            smith._inductor.config.triton.cudagraphs = True
            loss.backward()
    ...
    def test_cudagraphs_sdpa(self):
        query = smith.rand(
            32, 8, 128, 64, dtype=smith.float16, device=GPU_TYPE, requires_grad=True
        )
        key = smith.rand(32, 8, 128, 64, dtype=smith.float16, device=GPU_TYPE)
        value = smith.rand(32, 8, 128, 64, dtype=smith.float16, device=GPU_TYPE)
        ...
        with (
            ...
            opt_bwd = smith.compile(lambda: out.sum().backward())
            opt_bwd()
    ...
    def test_cudagraphs_cpu_scalar_used_in_python_custom_op(self):
        class MyFn(smith.autograd.Function):
            ...
            def backward(ctx, gO):
                ...
                expand = gO * smith.ones_like(x)
                return expand * cpu_tensor * ctx.cpu_scalar

        x = smith.randn(10, requires_grad=True, device="cuda")
        out = MyFn.apply(x)
        with (
            config.patch(compiled_autograd=True),
            inductor_config.patch("triton.cudagraphs", True),
        ):
            opt_bwd = smith.compile(lambda: out.backward())
            opt_bwd()
...
struct CustomOpAutogradFunction : public smith::autograd::Function<CustomOpAutogradFunction> {
  ...
  static smith::Tensor forward(
      smith::autograd::AutogradContext* ctx,
      const smith::Tensor& x) {
    const auto& cpu_tensor = smith::tensor(1);
  ...
  static smith::autograd::variable_list backward(
    ...
    assert(saved_variables.size() == 2);
    smith::Tensor x = saved_variables[0];
    smith::Tensor cpu_tensor = saved_variables[1];
    int cpu_scalar = ctx->saved_data["cpu_scalar"].toInt();
    auto expand = grad_output[0] * smith::ones_like(x);
    smith::autograd::variable_list grad_inputs(1);
...
SMITH_LIBRARY(test_cudagraphs_cpu_scalar_used_in_cpp_custom_op, m) {
    m.def("custom_op_backed_by_autograd_fn", custom_op_backed_by_autograd_fn);
}
        """
        ...
        x = smith.randn(2, 2, requires_grad=True, device="cuda")
        with (
            config.patch(compiled_autograd=True),
            inductor_config.patch("triton.cudagraphs", True),
        ):
            out = smith.ops.test_cudagraphs_cpu_scalar_used_in_cpp_custom_op.custom_op_backed_by_autograd_fn(
                x
            )
            opt_bwd = smith.compile(lambda: out.sum().backward())
            opt_bwd()
    ...
    def test_logs(self):
        logs, ctx = logs_to_string(
            smith._dynamo.compiled_autograd.__name__, "compiled_autograd"
        )
        with compiled_autograd._enable(compiler_fn), ctx():
            smith.randn(4, 4, requires_grad=True).sum().backward()

        self.assertEqual(counters["compiled_autograd"]["captures"], 1)
        self.assertEqual(counters["compiled_autograd"]["compiles"], 1)
        assert "smith::autograd::AccumulateGrad (NodeCall" in logs.getvalue()
        assert (
            self.gen_cache_miss_log_prefix() + "smith::autograd::GraphRoot"
    ...
    def test_logs_aot_bwd_reuse(self):
        ...
        with compiled_autograd._enable(compiler_fn):
            ...
            z = smith.randn(4, 4, requires_grad=True)
    ...
    def test_verbose_logs_graph(self):
        ...
        logs, ctx = logs_to_string(
            smith._dynamo.compiled_autograd.__name__, "compiled_autograd_verbose"
        )
        with ctx():
            self.check_output_and_recompiles(fn)

        expected_logs = [
            "smith::autograd::GraphRoot (NodeCall 0)",
            "ReluBackward0 (NodeCall 2)",
            "AddmmBackward0 (NodeCall 3)",
            "ReluBackward0 (NodeCall 5)",
            "TBackward0 (NodeCall 6)",
            "smith::autograd::AccumulateGrad (NodeCall 7)",
            "smith::autograd::AccumulateGrad (NodeCall 9)",
            "TBackward0 (NodeCall 10)",
            "smith::autograd::AccumulateGrad (NodeCall 11)",
            "SumBackward0 (NodeCall 1)",
            "ReluBackward0 (NodeCall 2)",
            "AddmmBackward0 (NodeCall 3)",
            "smith::autograd::AccumulateGrad (NodeCall 11)",
            "TBackward0 (NodeCall 4)",
            "smith::autograd::AccumulateGrad (NodeCall 5)",
            "ReluBackward0 (NodeCall 6)",
            "AddmmBackward0 (NodeCall 7)",
            "smith::autograd::AccumulateGrad (NodeCall 10)",
    ...
    @mock.patch(
        "smith._funcsmith.aot_autograd.AOT_COUNTER", new_callable=itertools.count
    )
    @mock.patch("smith._dynamo.config.inline_inbuilt_nn_modules", True)
    ...
    def test_verbose_logs_cpp(self):
        smith._logging.set_logs(compiled_autograd_verbose=True)
        ...
        patterns1 = [
            ...
            + r"smith::autograd::GraphRoot \(NodeCall 0\) with key size (\d+), previous key sizes=\[\]\n",
        ]
    ...
    def test_verbose_logs_dynamic_shapes(self):
        ...
        for i, j in zip([10, 11, 12], [10, 10, 11]):
            ...
            with ctx(), compiled_autograd._enable(smith.compile(backend="eager")):
                result.backward()
        ...
        expected_logs = [
            self.gen_cache_miss_log_prefix()
            + "smith::autograd::GraphRoot (NodeCall 0) with key size 39, previous key sizes=[]",
        ]
    ...
    def test_verbose_logs_snapshot(self):
        ...
        unexpected_logs = [
            self.gen_cache_miss_log_prefix() + "smith::autograd::GraphRoot (NodeCall 0)"
        ]
    ...
    def test_tensor_subclass_basic(self):
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            def to_twotensor_backward(ctx, grad):
                return smith.ops.mylib.from_twotensor(grad)
            ...
            param1 = smith.randn(4, 4, requires_grad=True)
            param2 = smith.randn(4, 4, requires_grad=True)
            with TwoTensorMode():
                x = smith.ops.mylib.to_twotensor(param1, param2)
            ...
            self.assertExpectedInline(
                graph_code,
                """\
class CompiledAutograd0(smith.nn.Module):
    def forward(self, inputs, sizes, scalars, hooks, packed_data):
        ...
        unwrap_maybe_dynamic_int = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_5);  getitem_5 = None
        unwrap_maybe_dynamic_int_1 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_6);  getitem_6 = None
        unwrap_maybe_dynamic_int_2 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_7);  getitem_7 = None
        unwrap_maybe_dynamic_int_3 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_8);  getitem_8 = None
        unwrap_maybe_dynamic_int_16 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_21);  getitem_21 = None
        unwrap_maybe_dynamic_int_17 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_22);  getitem_22 = None
        unwrap_maybe_dynamic_int_18 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_23);  getitem_23 = None
        unwrap_maybe_dynamic_int_19 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_24);  getitem_24 = None

        validate_outputs = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem], [((None, None, device(type='cpu'), 6, 0, None), [], True, 6)]);  getitem = None
        getitem_25 = validate_outputs[0];  validate_outputs = None

        sum_backward0 = smith__dynamo_compiled_autograd_ops_SumBackward0([getitem_25], [True], [unwrap_maybe_dynamic_int, unwrap_maybe_dynamic_int_1]);  getitem_25 = unwrap_maybe_dynamic_int = unwrap_maybe_dynamic_int_1 = None
        getitem_26 = sum_backward0[0];  sum_backward0 = None
        validate_outputs_1 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_26], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_2, unwrap_maybe_dynamic_int_3], True, 6)]);  getitem_26 = unwrap_maybe_dynamic_int_2 = unwrap_maybe_dynamic_int_3 = None
        getitem_27 = validate_outputs_1[0];  validate_outputs_1 = None

        getitem_28 = hooks[0];  getitem_28 = None
        call_aot_bwd_prologue = smith__dynamo_compiled_autograd_call_aot_bwd_prologue((getitem_1, getitem_2), [], [], getitem_27);  getitem_1 = getitem_2 = getitem_27 = None
        ...
        aot0_mul_2 = smith.ops.aten.mul.Tensor(aot0_tangents_1, aot0_primals_1);  aot0_tangents_1 = aot0_primals_1 = None
        aot0_mul_3 = smith.ops.aten.mul.Tensor(aot0_tangents_2, aot0_primals_2);  aot0_tangents_2 = aot0_primals_2 = None
        aot0_add_2 = smith.ops.aten.add.Tensor(aot0_mul_2, aot0_mul_2);  aot0_mul_2 = None
        aot0_add_3 = smith.ops.aten.add.Tensor(aot0_mul_3, aot0_mul_3);  aot0_mul_3 = None

        make_subclass = smith__dynamo_compiled_autograd_make_subclass(aot0_add_2, aot0_add_3);  aot0_add_2 = aot0_add_3 = None

        getitem_33 = hooks[1];  hooks = None
        call_backward = smith__dynamo_external_utils_call_backward(getitem_33, (), make_subclass);  getitem_33 = make_subclass = None
        getitem_36 = call_backward[0]
        getitem_37 = call_backward[1];  call_backward = None
        validate_outputs_2 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_36, getitem_37], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_16, unwrap_maybe_dynamic_int_17], False, 6), ((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_18, unwrap_maybe_dynamic_int_19], False, 6)]);  getitem_36 = getitem_37 = unwrap_maybe_dynamic_int_16 = unwrap_maybe_dynamic_int_17 = unwrap_maybe_dynamic_int_18 = unwrap_maybe_dynamic_int_19 = None
        getitem_39 = validate_outputs_2[0]

        call_accumulate_grad_1 = smith__dynamo_external_utils_call_accumulate_grad(getitem_4, getitem_39, False);  getitem_4 = getitem_39 = call_accumulate_grad_1 = None

        getitem_40 = validate_outputs_2[1];  validate_outputs_2 = None

        call_accumulate_grad = smith__dynamo_external_utils_call_accumulate_grad(getitem_3, getitem_40, False);  getitem_3 = getitem_40 = call_accumulate_grad = None

        _exec_final_callbacks_stub = smith__dynamo_external_utils__exec_final_callbacks_stub();  _exec_final_callbacks_stub = None
        return []
""",  # noqa: B950
            )

    # https://github.com/blacksmith/blacksmith/issues/138920
    ...
    def test_compiled_autograd_does_not_specialize_on_bw_symints(self):
        class Mod(smith.nn.Module):
            def __init__(self, a, b, c):
                ...
                self.c = c
                self.b = b
                self.lin1 = smith.nn.Linear(b * a, b * c, device="cpu")
        ...
        class Mod2(smith.nn.Module):
            ...
            def forward(self, s, tensor_dict):
                args = tensor_dict[s]
                x = smith.cat(list(args))
                out = self.mod(x)
                return out

        class Mod3(smith.nn.Module):
            ...
            def forward(self, strs, tensor_dict, x):
                ...
                for i, m in enumerate(self.mods):
                    ...
                    out = m(s, tensor_dict)
                    outs.append(out)
                return smith.cat(outs).sum(0)

        def gen_tensor_dict(sizes):
            tensor_dict = {
                "a": [smith.randn(sizes[0], 48, device="cpu") for _ in range(4)],
                "b": [smith.randn(sizes[1], 48, device="cpu") for _ in range(7)],
            }
            return tensor_dict
        ...
        x = smith.zeros(100, 48, device="cpu")
        tensor_dict = gen_tensor_dict([101, 102])
        out = m(strs, tensor_dict, x)

        with smith._dynamo.compiled_autograd._enable(compiler_fn) as ctx:
            out.sum().backward()

        x = smith.zeros(103, 48, device="cpu")
        ...
        view_nodes = graphs[1].graph.find_nodes(
            op="call_function", target=smith.ops.aten.reshape.default
        )
        # First 2 view nodes have a first argument that is a SymInt, not an int burned into the graph
        self.assertTrue(isinstance(view_nodes[0].args[1][0], smith.fx.Node))
        self.assertTrue(isinstance(view_nodes[1].args[1][0], smith.fx.Node))
    ...
    def test_saved_tensor_unpack_hook_ordering(self):
        ...
        with (
            smith.autograd.graph.saved_tensors_hooks(pack_hook, unpack_hook),
    ...
    def test_checkpointing_simple(self, reentrant):
        def fn():
            ...
            out = smith.utils.checkpoint.checkpoint(_fn, inp, use_reentrant=reentrant)
            out.backward()
            yield inp.grad
...
class CompiledAutograd0(smith.nn.Module):
    def forward(self, inputs, sizes, scalars, hooks, packed_data):
        ...
        unwrap_maybe_dynamic_int = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_2);  getitem_2 = None
        unwrap_maybe_dynamic_int_1 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_3);  getitem_3 = None
        unwrap_maybe_dynamic_int_2 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_4);  getitem_4 = None
        unwrap_maybe_dynamic_int_3 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_5);  getitem_5 = None
        unwrap_maybe_dynamic_int_4 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_6);  getitem_6 = None
        unwrap_maybe_dynamic_int_5 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_7);  getitem_7 = None
        unwrap_maybe_dynamic_int_6 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_8);  getitem_8 = None
        unwrap_maybe_dynamic_int_7 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_9);  getitem_9 = None
        unwrap_maybe_dynamic_int_8 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_10);  getitem_10 = None
        unwrap_maybe_dynamic_int_9 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_11);  getitem_11 = None
        unwrap_maybe_dynamic_int_10 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_12);  getitem_12 = None
        unwrap_maybe_dynamic_int_11 = smith__dynamo_external_utils_unwrap_maybe_dynamic_int(getitem_13);  getitem_13 = None

        validate_outputs = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem], [((None, None, device(type='cpu'), 6, 0, None), [], False, 6)]);  getitem = None
        getitem_14 = validate_outputs[0];  validate_outputs = None

        sum_backward0 = smith__dynamo_compiled_autograd_ops_SumBackward0([getitem_14], [True], [unwrap_maybe_dynamic_int, unwrap_maybe_dynamic_int_1]);  getitem_14 = unwrap_maybe_dynamic_int = unwrap_maybe_dynamic_int_1 = None
        getitem_15 = sum_backward0[0];  sum_backward0 = None
        validate_outputs_1 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_15], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_2, unwrap_maybe_dynamic_int_3], False, 6)]);  getitem_15 = unwrap_maybe_dynamic_int_2 = unwrap_maybe_dynamic_int_3 = None
        ...
        call_hook = smith__dynamo_external_utils_call_hook(getitem_17, getitem_18, hook_type = 'unpack_hook');  getitem_17 = getitem_18 = None
        call_hook_1 = smith__dynamo_external_utils_call_hook(getitem_19, getitem_20, hook_type = 'unpack_hook');  getitem_19 = getitem_20 = None
        mul_backward0 = smith__dynamo_compiled_autograd_ops_MulBackward0([getitem_16], [True, True], call_hook, 6, call_hook_1, 6);  getitem_16 = call_hook = call_hook_1 = None
        getitem_21 = mul_backward0[0]
        getitem_22 = mul_backward0[1];  mul_backward0 = None
        validate_outputs_2 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_21, getitem_22], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_4, unwrap_maybe_dynamic_int_5], False, 6), ((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_6, unwrap_maybe_dynamic_int_7], False, 6)]);  getitem_21 = getitem_22 = unwrap_maybe_dynamic_int_4 = unwrap_maybe_dynamic_int_5 = unwrap_maybe_dynamic_int_6 = unwrap_maybe_dynamic_int_7 = None
        getitem_23 = validate_outputs_2[0]
        getitem_24 = validate_outputs_2[1];  validate_outputs_2 = None

        getitem_25 = hooks[2]
        getitem_26 = packed_data[2]
        call_hook_2 = smith__dynamo_external_utils_call_hook(getitem_25, getitem_26, hook_type = 'unpack_hook');  getitem_25 = getitem_26 = None
        cos_backward0 = smith__dynamo_compiled_autograd_ops_CosBackward0([getitem_24], [True], call_hook_2);  getitem_24 = call_hook_2 = None
        getitem_27 = cos_backward0[0];  cos_backward0 = None
        validate_outputs_3 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_27], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_8, unwrap_maybe_dynamic_int_9], False, 6)]);  getitem_27 = unwrap_maybe_dynamic_int_8 = unwrap_maybe_dynamic_int_9 = None
        getitem_28 = validate_outputs_3[0];  validate_outputs_3 = None
        add = smith.add(getitem_23, getitem_28);  getitem_23 = getitem_28 = None

        getitem_29 = hooks[3];  hooks = None
        getitem_30 = packed_data[3];  packed_data = None
        call_hook_3 = smith__dynamo_external_utils_call_hook(getitem_29, getitem_30, hook_type = 'unpack_hook');  getitem_29 = getitem_30 = None
        sin_backward0 = smith__dynamo_compiled_autograd_ops_SinBackward0([add], [True], call_hook_3);  add = call_hook_3 = None
        getitem_31 = sin_backward0[0];  sin_backward0 = None
        validate_outputs_4 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_31], [((None, None, device(type='cpu'), 6, 0, None), [unwrap_maybe_dynamic_int_10, unwrap_maybe_dynamic_int_11], False, 6)]);  getitem_31 = unwrap_maybe_dynamic_int_10 = unwrap_maybe_dynamic_int_11 = None
        getitem_32 = validate_outputs_4[0];  validate_outputs_4 = None

        call_accumulate_grad = smith__dynamo_external_utils_call_accumulate_grad(getitem_1, getitem_32, False);  getitem_1 = getitem_32 = call_accumulate_grad = None
        _exec_final_callbacks_stub = smith__dynamo_external_utils__exec_final_callbacks_stub();  _exec_final_callbacks_stub = None
        return []
""",  # noqa: B950
                )
    ...
    def test_cpu_offloading(self):
        def fn():
            ...
            class MyMatMul(smith.autograd.Function):
                @staticmethod
                def forward(ctx, x):
                    ctx.save_for_backward(x)
                    return smith.matmul(x, x)
            ...
            with smith.autograd.graph.saved_tensors_hooks(pack, unpack):
                for i in [10, 100, 10, 20, 30]:
                    x = smith.randn(i, requires_grad=True).cuda()
        ...
        def check(gm):
            ...
            self.assertExpectedInline(
                graph_code,
                """\
class CompiledAutograd1(smith.nn.Module):
    def forward(self, inputs, sizes, scalars, hooks, packed_data):
        getitem = inputs[0]
        getitem_1 = inputs[1];  inputs = None
        ...
        validate_outputs = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem], [((None, None, device(type='cuda', index=0), 6, 0, None), [], False)]);  getitem = None
        getitem_5 = validate_outputs[0];  validate_outputs = None

        sum_backward0 = smith__dynamo_compiled_autograd_ops_SumBackward0([getitem_5], [True], []);  getitem_5 = None
        getitem_6 = sum_backward0[0];  sum_backward0 = None
        validate_outputs_1 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_6], [((None, None, device(type='cuda', index=0), 6, 0, None), [], False)]);  getitem_6 = None
        ...
        call_hook = smith__dynamo_external_utils_call_hook(getitem_8, getitem_9, hook_type = 'unpack_hook');  getitem_8 = getitem_9 = None
        call_backward = smith__dynamo_external_utils_call_backward(getitem_10, (call_hook,), getitem_7);  getitem_10 = call_hook = getitem_7 = None
        getitem_12 = call_backward[0];  call_backward = None
        validate_outputs_2 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_12], [((None, None, device(type='cuda', index=0), 6, 0, None), [getitem_3], False)]);  getitem_12 = getitem_3 = None
        getitem_13 = validate_outputs_2[0];  validate_outputs_2 = None

        to_copy_backward0 = smith__dynamo_compiled_autograd_ops_ToCopyBackward0([getitem_13], [True], (None, None, device(type='cpu'), 6, 0, None));  getitem_13 = None
        getitem_14 = to_copy_backward0[0];  to_copy_backward0 = None
        validate_outputs_3 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_14], [((None, None, device(type='cpu'), 6, 0, None), [getitem_4], False)]);  getitem_14 = getitem_4 = None
        getitem_15 = validate_outputs_3[0];  validate_outputs_3 = None

        accumulate_grad__default = smith.ops.inductor.accumulate_grad_.default(getitem_1, getitem_15);  getitem_1 = getitem_15 = accumulate_grad__default = None
        _exec_final_callbacks_stub = smith__dynamo_external_utils__exec_final_callbacks_stub();  _exec_final_callbacks_stub = None
        return []
""",  # noqa: B950
            )
    ...
    def test_disk_offloading(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            def fn():
                ...
                with smith.autograd.graph.saved_tensors_hooks(pack, unpack):
                    for i in [10, 100, 10, 20, 30]:
                        x = smith.randn(i, requires_grad=True)
                        MyMatMul.apply(x).sum().backward()
                        yield x.grad
...
class CompiledAutograd1(smith.nn.Module):
    def forward(self, inputs, sizes, scalars, hooks, packed_data):
        ...
        validate_outputs = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem], [((None, None, device(type='cpu'), 6, 0, None), [], False)]);  getitem = None
        getitem_4 = validate_outputs[0];  validate_outputs = None

        sum_backward0 = smith__dynamo_compiled_autograd_ops_SumBackward0([getitem_4], [True], []);  getitem_4 = None
        getitem_5 = sum_backward0[0];  sum_backward0 = None
        validate_outputs_1 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_5], [((None, None, device(type='cpu'), 6, 0, None), [], False)]);  getitem_5 = None
        ...
        call_hook = smith__dynamo_external_utils_call_hook(getitem_7, getitem_8, hook_type = 'unpack_hook');  getitem_7 = getitem_8 = None
        call_backward = smith__dynamo_external_utils_call_backward(getitem_9, (call_hook,), getitem_6);  getitem_9 = call_hook = getitem_6 = None
        getitem_11 = call_backward[0];  call_backward = None
        validate_outputs_2 = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem_11], [((None, None, device(type='cpu'), 6, 0, None), [getitem_3], False)]);  getitem_11 = getitem_3 = None
        getitem_12 = validate_outputs_2[0];  validate_outputs_2 = None

        accumulate_grad__default = smith.ops.inductor.accumulate_grad_.default(getitem_1, getitem_12);  getitem_1 = getitem_12 = accumulate_grad__default = None
        _exec_final_callbacks_stub = smith__dynamo_external_utils__exec_final_callbacks_stub();  _exec_final_callbacks_stub = None
        return []
""",  # noqa: B950
                )

            # 1 graph break on smith.load -> 2 dynamo graphs
            self.check_output_and_recompiles(
                fn,
                count=[1, 2],
    ...
    def test_backward_hook_relative_ordering_partial(self):
        ...
        def fn():
            ...
            class MyModule(nn.Module):
                def __init__(self):
                    super().__init__()
                    self.linear = smith.nn.Linear(10, 10, bias=False)
    ...
    def test_checkpointing_sac(self):
        ...
        def fn():
            ...
            recompute_list = [smith.ops.aten.addmm.default]
    ...
    def test_dont_dce_side_effects(self):
        class SideEffectfulBackward(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return x
        ...
        smith._inductor.config.fallback_random = True
    ...
    def test_anomaly_mode_already_nan(self):
        def fn():
            with smith.autograd.detect_anomaly():
                a = smith.randn(5, 5, requires_grad=True)
                a.grad = smith.full((5, 5), float("nan"))
                b = smith.randn(5, 5)
                out = smith.matmul(a, b)
                loss = out.sum()
                with smith._dynamo.compiled_autograd._enable(lambda gm: gm):
                    loss.backward()
    ...
    def test_anomaly_mode_backward(self):
        def fn():
            class MyFn(smith.autograd.Function):
                ...
                @staticmethod
                def backward(ctx, gO):
                    return smith.full(gO.size(), float("nan"))
    ...
    def test_anomaly_mode_grad(self):
        def fn():
            ...
            with smith.autograd.detect_anomaly():
                ...
                loss = out.sum()
                with smith._dynamo.compiled_autograd._enable(lambda gm: gm):
                    smith.autograd.grad(loss, inputs=a)
    ...
    def test_higher_order_gradients(self):
        ...
        def fn(fwd_compiler, ca_compiler):
            ...
            try:
                with compiled_autograd._enable(ca_compiler):
                    first = smith.autograd.grad(
                        fwd_compiler(f)(x), x, create_graph=True
                    )[0]
                    second = smith.autograd.grad(first, x, create_graph=True)[0]
                    third = smith.autograd.grad(second, x, create_graph=True)[0]
                    fourth = smith.autograd.grad(third, x, create_graph=True)[0]
        ...
        def eager():
            return smith.compile(backend="eager")

        def aot_eager():
            return smith.compile(backend="aot_eager")
    ...
    def test_ddp_cpp_reducer_error(self):
        ...
        try:
            model = smith.nn.Sequential(nn.Linear(10, 10), nn.ReLU(), nn.Linear(10, 10))
            model = DDP(model)
            inputs = smith.randn(10, 10)
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ...
                    (
                        ...
                        r'please use smith._dynamo.config.optimize_ddp="python_reducer"'
                    ),
                ),
            ):
                loss.backward()
    ...
    def test_accumulate_grad_polyfill_case_1_1(self):
        def fn():
            class StealableDenseOp(BaseCustomOp):
                @staticmethod
                def backward(ctx, grad_output):
                    return smith.ones_like(grad_output, requires_grad=False) * 5
            ...
            var = smith.randn(2, 2, requires_grad=True)
            ...
            assert smith.equal(var.grad, smith.ones_like(var) * 5), (
        ...
        def fn():
            class StealableSparseOp(BaseCustomOp):
                ...
                def backward(ctx, grad_output):
                    ...
                    indices = smith.tensor([[0, 1], [0, 1]], dtype=smith.int64)
                    values = smith.tensor([5.0, 5.0])
            ...
            expected_dense_grad = smith.tensor([[5.0, 0.0], [0.0, 5.0]])
            assert smith.equal(var.grad.to_dense(), expected_dense_grad), (
    ...
    def test_accumulate_grad_polyfill_case_1_3(self):
        def fn():
            ...
            output.backward(
                smith.ones_like(output), create_graph=True
    ...
    def test_accumulate_grad_polyfill_case_1_5_1(self):
        def fn():
            class NotStealableRefsOp(BaseCustomOp):
                @staticmethod
                def backward(ctx, grad_output):
                    return smith.ones_like(grad_output, requires_grad=False) * 10.0
            ...
            assert var.grad is not None, "Grad should be defined"
            assert smith.equal(var.grad, smith.ones_like(var) * 10.0), (
                "Grad content should be as returned by backward"
            )
            assert (
    ...
    def test_accumulate_grad_polyfill_case_1_5_2(self):
        def fn():
            class SimpleDenseGradOp(BaseCustomOp):
                @staticmethod
                def backward(ctx, grad_output):
                    return smith.ones_like(grad_output, requires_grad=False) * 7.0

            # Create a non-contiguous variable
            base_tensor = smith.randn(4, 4)
            var = base_tensor[::2, ::2]
            ...
            assert smith.equal(var.grad, smith.ones_like(var) * 7.0), (
                "Grad content should be as returned by backward"
            )
            assert (
                grad_ref_holder[0].untyped_storage() is not var.grad.untyped_storage()
    ...
    def test_accumulate_grad_polyfill_case_2_1(self):
        def fn():
            class SparseVarGradDenseNewGradOp(BaseCustomOp):
                @staticmethod
                def backward(ctx, grad_output):
                    return smith.ones_like(grad_output) * 3.0

            var = smith.randn(2, 2, requires_grad=True)
            indices = smith.tensor([[0, 1], [0, 1]], dtype=smith.int64)
            values = smith.tensor([1.0, 1.0])
            var.grad = smith.sparse_coo_tensor(
                indices, values, var.size(), requires_grad=False
            )
            initial_grad_ref = var.grad
            output = SparseVarGradDenseNewGradOp.apply(var)

            expected_sum = (smith.ones_like(var) * 3.0) + initial_grad_ref.to_dense()
            output.backward(smith.ones_like(output))

            assert var.grad is not None, "Grad should be defined"
            assert not var.grad.is_sparse, "Resulting grad should be dense"
            assert smith.equal(var.grad, expected_sum), "Grad content should be the sum"
        ...
        self.check_output_and_recompiles(
            fn,
            compiler_fn=lambda gm: gm,  # https://github.com/blacksmith/blacksmith/issues/154161
            count=[1, 0],
        )
    ...
    def test_accumulate_grad_polyfill_case_2_3_1(self):
        def fn():
            ...
            var = smith.randn(2, 2, requires_grad=True)
            var.grad = smith.ones_like(var) * 1.0
            initial_grad_ref = var.grad
            output = DenseVarGradDenseNewGradOp.apply(var)
            expected_sum = initial_grad_ref + (smith.ones_like(var) * 3.0)
            output.backward(smith.ones_like(output))
        ...
        def fn():
            class SparseVarGradSparseNewGradOp(BaseCustomOp):
                ...
                def backward(ctx, grad_output):
                    size = grad_output.size()
                    indices = smith.tensor([[0, 1], [0, 1]], dtype=smith.int64)
                    values = smith.tensor([3.0, 3.0])
                    return smith.sparse_coo_tensor(
                        indices, values, size, requires_grad=False
                    )

            var = smith.randn(2, 2, requires_grad=True)
            indices_v = smith.tensor([[0, 0], [0, 1]], dtype=smith.int64)
            values_v = smith.tensor([1.0, 2.0])
            var.grad = smith.sparse_coo_tensor(
            ...
            new_grad_for_sum = smith.sparse_coo_tensor(
                smith.tensor([[0, 1], [0, 1]], dtype=smith.int64),
                smith.tensor([3.0, 3.0]),
                var.size(),
            )
            expected_sum_dense = (
            ...
            assert smith.equal(var.grad.to_dense(), expected_sum_dense), (
        ...
        def fn():
            class DenseVarGradSparseNewGradOp(BaseCustomOp):
                ...
                def backward(ctx, grad_output):
                    size = grad_output.size()
                    indices = smith.tensor([[0, 1], [0, 1]], dtype=smith.int64)
                    values = smith.tensor([3.0, 3.0])  # New sparse values
                    return smith.sparse_coo_tensor(
                        indices, values, size, requires_grad=False
                    )

            var = smith.randn(2, 2, requires_grad=True)
            var.grad = smith.ones_like(var) * 1.0  # Initial value
            initial_grad_ref = var.grad
    ...
    def test_accumulate_grad_polyfill_case_3_1(self):
        def fn():
            class SparseVarGradDenseNewGradDoubleBackwardOp(BaseCustomOp):
                @staticmethod
                def backward(ctx, grad_output):
                    return smith.ones_like(grad_output, requires_grad=True) * 3.0

            var = smith.randn(2, 2, requires_grad=True)
            indices = smith.tensor([[0, 1], [0, 1]], dtype=smith.int64)
            values = smith.tensor([1.0, 1.0], requires_grad=True)
            var.grad = smith.sparse_coo_tensor(
            ...
            expected_sum = (
                smith.ones_like(var, requires_grad=True) * 3.0
            ) + initial_grad_ref.to_dense()

            output.backward(smith.ones_like(output), create_graph=True)
    ...
    def test_smith_function_mode(self):
        called_funcs = []

        class LoggingSmithFunctionMode(BaseSmithFunctionMode):
        ...
        class MyLoss(smith.autograd.Function):
            @staticmethod
            def forward(ctx, out):
            ...
            def backward(ctx, grad_output):
                ...
                return smith.ones_like(saved) * grad_output
...
_set_multithreading_enabled""",
    ...
    def test_smith_dispatch_mode(self):
        called_funcs = []

        class LoggingSmithDispatchMode(SmithDispatchMode):
...
known_graph_breaks_tests = {
    ...
    "test_backward_dict_grad_for_nontensor",  # smith/_custom_op/autograd.py in skip files
    "test_backward_dict_invalid_keys",  # smith/_custom_op/autograd.py in skip files
    "test_backward_dict_requires_keys_for_input_optional_tensors",  # smith/_custom_op/autograd.py in skip files
    "test_backward_dict_requires_keys_for_input_tensors",  # smith/_custom_op/autograd.py in skip files
    "test_backward_grads_are_tensor_or_none",  # smith/_custom_op/autograd.py in skip files
    "test_backward_impl_on_existing_op",  # smith/_custom_op/autograd.py in skip files
    "test_backward_returns_dict",  # smith/_custom_op/autograd.py in skip files
    "test_backward_tensorlist_input_requires_list_grads",  # smith/_custom_op/autograd.py in skip files
    "test_backward_tensorlist_input_requires_list_grads_none_or_Tensor",  # smith/_custom_op/autograd.py in skip files
    "test_backward_tensorlist_input_requires_list_grads_with_same_numel",  # smith/_custom_op/autograd.py in skip files
    "test_save_for_backward_inputs_are_namedtuple",  # smith/_custom_op/autograd.py in skip files
    ...
    "test_dtensor_basic",  # smith._dynamo.exc.Unsupported: Failed to convert args/kwargs to proxy
    ...
    "test_checkpoint_compile_no_recompile",  # smith._dynamo.exc.Unsupported: Attempted to call function marked as skipped
    "test_checkpoint_device_context_fn",  # smith._dynamo.exc.Unsupported: Attempted to call function marked as skipped
...
xfail_by_backend = {
    "ca_eager": {  # xfail
        ...
        "test_node_ordering_when_none_returned",  # smith._dynamo.exc.Unsupported: TypeError <built-in method clone
        ...
        "test_backward_twice_without_saved_values",  # https://github.com/blacksmith/blacksmith/issues/129938
    ...
    "eager": {  # will be run without smith.compiling the CA graph
        ...
        "test_custom_function_exception",  # smith.no_grad(), smith._dynamo.exc.Unsupported: missing: WITH_EXCEPT_START
        ...
        "test_checkpointing_without_reentrant_saved_object_identity",  # same as https://github.com/blacksmith/blacksmith/issues/136193
        ...
        "test_graph_save_on_cpu",  # smith.save should no-op and be recorded in the graph
        "test_saving_variable_to_disk",  # smith.save should no-op and be recorded in the graph
        "test_nested_checkpoint_early_stop_False",  # AOT backward higher order gradients
        # Slow tests, these tests are close to CI timeout if we try to smith.compile them
        "test_checkpointing",
    ...
    "aot_eager": {  # will be run with smith.compile(backend="eager")
        # Category: FakeTensor
    ...
    "inductor": {},  # will be run with smith.compile(backend="aot_eager")
    # tests not present in this dict will be run with smith.compile(backend="inductor")
}
...
xfail_divergence_from_eager = {
    ...
    "test_grad_call_compiled_backward_fn",  # different funcsmith error
    "test_vjp_call_compiled_backward_fn",  # different funcsmith error
    "test_vmap_call_compiled_backward_fn",  # different funcsmith error
...
FuncSmithHigherOrderOpTestsWithCompiledAutograd = wrap_test_class(
    test_higher_order_ops.FuncSmithHigherOrderOpTests
)
ActivationCheckpointingTestsWithCompiledAutograd = wrap_test_class(
    test_higher_order_ops.ActivationCheckpointingTests
)

if smith.distributed.is_available() and HAS_CUDA_AND_TRITON:
...
class TestCompiledAutogradOpInfo(TestCase):
    ...
    def test_hops_in_bwd(self, device, dtype, op):
        ...
        for inp in op.sample_inputs(device, dtype, requires_grad=False):
            ...
            # 1. Run eager
            smith.manual_seed(123)
            dummy = smith.randn(2, 2, dtype=dtype, device=device, requires_grad=True)
