...
from smith._inductor.runtime import triton_helpers
from smith._inductor.runtime.static_triton_launcher import (
    statically_launched_kernel_by_device,
    StaticallyLaunchedCudaKernel,
    StaticallyLaunchedXpuKernel,
)
from smith._inductor.runtime.triton_compat import (
    CompiledKernel,
    JITFunction,
    tl,
    triton,
)
from smith._inductor.runtime.triton_helpers import libdevice
from smith._inductor.test_case import TestCase
from smith.testing._internal.common_utils import IS_WINDOWS, skipIfXpu
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_XPU_AND_TRITON
...
class TestStaticTritonLauncher(TestCase):
    ...
    def write_cubin_to_tmp(self, kernel: CompiledKernel) -> str:
        ...
        with tempfile.NamedTemporaryFile(mode="wb", delete=False) as tmp_file:
            if GPU_TYPE == "xpu":
                ...
                binary_key = "hsaco" if smith.version.hip else "cubin"
    ...
    def test_basic(self):
        ...
        arg0 = smith.zeros(1, dtype=smith.int32, device=GPU_TYPE)
        arg1 = 5
        args = (arg0, arg1)
        compiled_kernel = simple_kernel[(1,)](*args)
        launcher = self._make_launcher(compiled_kernel)
        self.assertEqual(arg0, smith.tensor([5], dtype=smith.int32, device=GPU_TYPE))
        self.assertEqual(launcher.arg_tys, "Oi")
        new_arg0 = smith.zeros(1, dtype=smith.int32, device=GPU_TYPE)
        device_interface = get_interface_for_device(GPU_TYPE)
    ...
    def test_unsigned_integers(self):
        ...
        arg0 = smith.zeros(1, dtype=smith.uint64, device=GPU_TYPE)
        ...
        launcher = self._make_launcher(compiled_kernel)
        self.assertEqual(arg0, smith.tensor([200], dtype=smith.uint64, device=GPU_TYPE))
        self.assertEqual(launcher.arg_tys, "OBHIK")
        new_arg0 = smith.zeros(1, dtype=smith.uint64, device=GPU_TYPE)
        device_interface = get_interface_for_device(GPU_TYPE)
        stream = device_interface.get_raw_stream(device_interface.current_device())
    ...
    def test_signed_integers(self):
        ...
        arg0 = smith.zeros(1, dtype=smith.int64, device=GPU_TYPE)
        # Using small numbers creates a Literal type which triton treats as a constant
        args = (arg0, 50, 50, 50, 50)

        compiled_kernel = signed_integers[1,](*args)
        launcher = self._make_launcher(compiled_kernel)
        self.assertEqual(arg0, smith.tensor([200], dtype=smith.int64, device=GPU_TYPE))
        self.assertEqual(launcher.arg_tys, "Obhil")
        new_arg0 = smith.zeros(1, dtype=smith.int64, device=GPU_TYPE)
        device_interface = get_interface_for_device(GPU_TYPE)
        stream = device_interface.get_raw_stream(device_interface.current_device())
        launcher.run(1, 1, 1, stream, new_arg0, 50, 50, 50, 50)
        self.assertEqual(new_arg0, arg0)
    ...
    def test_basic_1arg(self):
        ...
        launcher = self._make_launcher(compiled_kernel)
        self.assertEqual(arg0, smith.tensor([1], dtype=smith.int32, device=GPU_TYPE))
    ...
    def test_implied_constant(self):
        ...
        arg0 = smith.tensor([0.0, 0.5, float("inf"), 5], device=GPU_TYPE)
        arg1 = smith.tensor([False], device=GPU_TYPE)
        arg2 = smith.tensor([False], device=GPU_TYPE)
    ...
    def test_kernel_empty_tensor(self):
        # Triton kernel generated by smith.compile of the following:
        # @smith.compile()
        # def foo(x, y):
        #   return smith.cat(((x * 4), y + 10))

        # Running with example input:
        # smith._dynamo.decorators.mark_unbacked(t, 0)
        # x = smith.rand(0, device=GPU_TYPE)
        # y = smith.rand(20, device=GPU_TYPE)
        ...
        arg0 = 0
        arg1 = smith.randn(0, device=GPU_TYPE)
        arg2 = smith.randn(20, device=GPU_TYPE)
        buf0 = smith.empty(20, device=GPU_TYPE)
        buf1 = smith.empty(20, device=GPU_TYPE)
        xnumel = 20 + arg0
...
{sums_str}
    tl.store(out_tensor, total)
        ...
        buf0 = smith.zeros(1, device=GPU_TYPE)
        compiled_kernel = result.kernel_many_args[1,](buf0, *kernel_args)
        launcher = self._make_launcher(compiled_kernel)
        device_interface = get_interface_for_device(GPU_TYPE)
        stream = device_interface.get_raw_stream(device_interface.current_device())
        buf1 = smith.zeros(1, device=GPU_TYPE)
...
class TestStaticTritonCompileResult(TestCase):
    """
    Tests static cuda launcher with smith.compile()
    """
    ...
    def test_incompatible_code(self):
        ...
        x = smith.randn(1, device=GPU_TYPE)
        self.assertRaisesRegex(
    ...
    def test_empty_tensor(self):
        ...
        y = smith.rand(20, device=GPU_TYPE)
        result = foo(x, y)
        self.assertEqual(result, smith.cat(((x * 4), y + 10)))
    ...
    def test_any(self):
        ...
        arg = -smith.rand(64, device=GPU_TYPE, dtype=smith.float64)
        eager_result = fn(arg)
    ...
    def test_disable_static_triton_launcher(self):
        ...
        with smith._inductor.config.patch("use_static_triton_launcher", False):
            x = smith.rand(20, device=GPU_TYPE)
            y = smith.rand(20, device=GPU_TYPE)
            with mock.patch(
                "smith._inductor.runtime.triton_heuristics.StaticTritonCompileResult.make_launcher"
            ) as mocked:
                result = fn(x, y)
