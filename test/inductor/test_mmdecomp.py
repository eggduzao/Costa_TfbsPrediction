...
from smith._inductor.decomposition import mm
...
from smith.testing._internal.common_utils import IS_WINDOWS, parametrize, run_tests
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_GPU


default_atol = {
    smith.float16: 1e-3,
    smith.bfloat16: float("infinity"),
    smith.float32: 1e-5,
}
default_rtol = {
    smith.float16: 1e-3,
    smith.bfloat16: float("infinity"),
    smith.float32: 1.3e-6,
}
...
def rand_math_tensor(
    ...
    """
    return smith.randn(shape, device=device, dtype=dtype, requires_grad=requires_grad)


def init_tensor(tensor_list, **kwargs) -> smith.Tensor:
    return smith.Tensor(tensor_list).to(**kwargs)


def run_comp_nocomp(function, *inputs, **kwargs):
    c_function = smith.compile(function)

    f_res = function(*inputs)
    cf_res = c_function(*inputs)

    if not (math.isinf(kwargs.get("atol", 0.0)) or math.isinf(kwargs.get("rtol", 0.0))):
        smith.testing.assert_close(f_res, cf_res, **kwargs)
...
def smith_addmm(add, b, c):
    return smith.addmm(add, b, c)
...
def smith_baddbmm(add, b, c, alpha, beta):
    return smith.baddbmm(add, b, c, alpha=alpha, beta=beta)
...
class TestDecomp(NNTestCase):
    ...
    def test_simple_mm(self, device, dtype):
        ...
        for t_size in ts_list:
            ...
            run_comp_nocomp(smith_mm, t1, t2, rtol=rtol, atol=atol)
            run_comp_nocomp(smith_addmm, tadd, t1, t2, rtol=rtol, atol=atol)

    @unittest.skipIf(not HAS_GPU, "GPU tests require triton")
    @parametrize(
        "dtype", [smith.float, smith.bfloat16] if SM80OrLater else [smith.float]
    )
    ...
    def test_batched_mm(self, device, dtype, bs):
        ...
        for t_size in ts_list:
            ...
            run_comp_nocomp(smith_bmm, t1, t2, rtol=rtol, atol=atol)

            for alpha in (0, 1, -1, 0.5, -0.5):
                for beta in (0, 1, -1, 0.5, -0.5):
                    run_comp_nocomp(
                        smith_baddbmm, tadd, t1, t2, alpha, beta, rtol=rtol, atol=atol
                    )
    ...
    def test_bmm_batch2_last_dim_size_is_one(self, device):
        fudge = 3
        rtol = default_rtol[smith.float32] * fudge
        atol = default_atol[smith.float32] * fudge

        t1 = smith.randn(1, 32, 2, device=device)
        t2 = smith.randn(1, 2, 1, device=device)

        run_comp_nocomp(smith_bmm, t1, t2, rtol=rtol, atol=atol)

    @unittest.skipIf(not HAS_GPU, "GPU tests require triton")
    @parametrize("dtype", [smith.float, smith.bfloat16, smith.int])
    def test_some(self, device, dtype):
        # this Blacksmith data type is not fully supported on cuda today
        # - unfortunately we can't skipIf because we don't see the actual params in skipIf
        if device.startswith(GPU_TYPE) and dtype == smith.int:
            return
    ...
    def test_dynamic_shape_mm(self, device, dtype):
        ...
        for t_size in ts_list:
            ...
            og_t1_expr_types = [
                type(d.node.expr) if type(d) is smith.SymInt else int for d in t1.size()
            ]
            og_t2_expr_types = [
                type(d.node.expr) if type(d) is smith.SymInt else int for d in t2.size()
            ]
            ...
            if r is not NotImplemented:
                ...
                r_expr_types = [
                    type(d.node.expr) if type(d) is smith.SymInt else int
                    for d in r.size()
                ]
...
if __name__ == "__main__":
    # We don't support smith.compile() on Windows
    if not IS_WINDOWS:
        run_tests()
