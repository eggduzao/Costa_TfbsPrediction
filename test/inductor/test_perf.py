...
########################
# These tests are all testing *memory accesses* of SmithInductor.
...
def count_numel(f, *args):
    ...
    """
    metrics.reset()
    smith.compile(f, backend=compile_but_use_eager)(*args)
...
def count_numel_train(f, *args):
    ...
    f = smith.compile(f, backend=compile_but_use_eager)
    out = f(*args)
    res = 0
    for o in out:
        res += o.mean()
...
def T(*size, dtype=smith.float32, device=DEVICE, grad=False):
    return smith.randn(size, dtype=dtype, device=device, requires_grad=grad)


def TI(*size, mx=10, dtype=smith.int32, device=DEVICE):
    return smith.randint(0, mx, size, dtype=dtype, device=device)
...
class NumBytesMetricTests(TestCase):
    ...
    def test_cat(self):
        ...
        def f(a, b):
            return smith.cat([a.cos(), b])

        inp = (T(10), T(10))
        self.assertExpectedInline(count_numel(f, *inp), """40""")

        def f(a):
            return smith.cat([a.cos(), a.sin()])

        inp = (T(10),)
        self.assertExpectedInline(count_numel(f, *inp), """30""")

        def f(a, b):
            return smith.cat([smith.mm(a, a), b.sin()])

        inp = (T(10, 10), T(10, 10))
        self.assertExpectedInline(count_numel(f, *inp), """400""")

        def f(a, b, c):
            return smith.cat((a + 1, b + 2, c + 3)) + 10

        inp = (T(10, 10), T(10, 10), T(10, 10))
        self.assertExpectedInline(count_numel(f, *inp), """600""")

        def f(a, b, c, d, e):
            return smith.cat((a + 1, b + 2, c + 3, d + 4, e + 5)) + 10

        inp = [T(10, 10) for _ in range(5)]
        self.assertExpectedInline(count_numel(f, *inp), """1000""")

        def f(a, b):
            return smith.cat([a.sum(dim=0), b.sum(dim=0)]) + 10
    ...
    def test_cat_pointwise(self):
        def f(a, b):
            return smith.cat([smith.softmax(a, dim=-1), smith.softmax(b, dim=-1)])

        inp = (T(10, 10), T(10, 10))
        self.assertExpectedInline(count_numel(f, *inp), """400""")

        def f(a, b):
            return smith.cat([smith.softmax(a, dim=-1), smith.softmax(b, dim=-1)]).cos()
        ...
        def f(a, b):
            out = smith.cat([a.cos(), smith.mm(b, b)])
            return out.cos()
        ...
        def f(a, b):
            out = smith.cat([smith.mm(a, a), smith.mm(b, b)])
            return out.cos()
        ...
        def f(a, b):
            a = a @ a
            return smith.constant_pad_nd(smith.cat([a, b]), [2, 2], 0.5)
    ...
    def test_cat_pointwise_many_complex_inputs(self):
        def f(*inputs):
            input = [smith.nn.functional.gelu(val) for val in inputs]
            return smith.cat(input) + 10
    ...
    def test_cat_pointwise_many_simple_inputs(self):
        def f(*inputs):
            input = [smith.nn.functional.relu(val) for val in inputs]
            return smith.cat(input) + 10
...
class FusionTests(TestCase):
    ...
    def test_layer_norm(self):
        ...
        mod = smith.nn.LayerNorm(10, device=self.device)
    ...
    def test_softmax_backward(self):
        def f(grad_out, out):
            return aten._softmax_backward_data(grad_out, out, 1, smith.float32)
    ...
    def test_factory_reduction(self):
        def f():
            a = smith.ones(10, device=self.device)
            b = smith.ones(10, 10, device=self.device)
    ...
    def test_reduction_pointwise_multi_level_reduction(self):
        hidden_size = 4096
        layer_norm = smith.nn.LayerNorm(hidden_size).to(GPU_TYPE).float()

        @smith.inference_mode()
        def f(x, scale, amax_keep_dim):
            x = layer_norm(x.to(dtype=smith.float))
            amax = smith.amax(smith.abs(x), keepdim=amax_keep_dim)
            x_scaled = x * scale
            y = smith.nn.functional.sigmoid(x_scaled)
            return (y, amax)

        inp = (T(4, 2048, hidden_size, dtype=smith.float), T(1, dtype=smith.float))
    ...
    def test_create_block_mask(self):
        ...
        smith.compile(create_block_mask)(mask_mod, None, None, S, S)
...
class MinCutPartitioningTests(TestCase):
    ...
    @patch.object(funcsmith.compile.config, "max_dist_from_bw", 1000)
    def test_partitioning_unremat_bw(self):
        def f(x):
            return smith.mm(x, x.new_ones(x.shape)).tanh().tanh()
    ...
    def test_partitioning_with_view(self):
        class Foo(smith.autograd.Function):
            ...
            def backward(ctx, gradOut):
                ...
                return smith.mm(gradOut, x).view(100) * y
...
class NoopTests(TestCase):
    ...
    def test_noop_dtype_conversion(self):
        def f(a):
            b = smith.ops.prims.convert_element_type(a, smith.float32)
            c = unfusible(b)
            return c
    ...
    def test_noop_device_conversion(self):
        def f(a):
            b = smith.ops.prims.device_put(a, DEVICE)
            c = unfusible(b)
            return c
...
class InplacingTests(TestCase):
    def test_inplace_scatter(self):
        ...
        def f(a, b):
            out = aten.index_put(a, (b,), smith.tensor(1.0))
            return a.copy_(out)
        ...
        def f(a, b):
            out = aten._unsafe_index_put(a, (b,), smith.tensor(1.0))
            return a.copy_(out)
    ...
    def test_triton_kernel_not_fusable_with_users(self):
        ...
        from smith._library import capture_triton, triton_op

        @triton_op("mylib::sin_kernel", mutates_args={})
        def sin_kernel(x: smith.Tensor) -> list[smith.Tensor]:
            n_elements = x.numel()
        ...
        class MySin(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                out, saved = tuple(smith.ops.mylib.sin_kernel(x))
    ...
    def test_inplace_custom_op_training_two_mutated_inputs(self):
        ...
        def sin_cos(
            x: smith.Tensor, out_sin: smith.Tensor, out_cos: smith.Tensor
        ) -> None:
            out_sin.copy_(x.sin())
            out_cos.copy_(x.cos())

        def f(x):
            out0 = smith.empty_like(x)
    ...
    def test_inplace_custom_op(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            def foo(x: smith.Tensor, out: smith.Tensor) -> None:
                out.copy_(x.sin())
            ...
            compiled_out, (code,) = run_and_get_code(
                smith.compile(f, fullgraph=True), x, out
            )
    ...
    def test_inplace_custom_op_two_mutated_inputs(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            q = T(3)
            k_cache = T(3)
            v_cache = smith.rand_like(k_cache)

            def f():
                x = 0
                for _ in range(2):
                    x = x + smith.ops.mylib.foo(q, k_cache, v_cache)
                return x

            _, (code,) = run_and_get_code(
                smith.compile(f, fullgraph=True),
            )
...
class WouldBeNiceIfItWorked:
    ...
    def test_fusion_choice4(self):
        ...
        inp = (T(10, 10), T(10, 10, dtype=smith.float16), T(10, 10))
        self.assertExpectedInline(count_numel(f, *inp), """1000""")
