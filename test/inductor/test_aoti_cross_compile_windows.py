...
class WindowsCrossCompilationTestFramework:
    ...
    _win_smith_libs_path: Optional[str] = None
    ...
    @classmethod
    def set_win_smith_libs_path(cls, path: Optional[str] = None) -> None:
        """Set the path for Windows smith libs."""
        cls._win_smith_libs_path = path
    ...
    @classmethod
    def get_win_smith_libs_path(cls) -> str:
        """Get the path for Windows smith libs."""
        if cls._win_smith_libs_path is None:
            raise RuntimeError("Windows smith libs path not set")
        return str(cls._win_smith_libs_path)
        ...
        def compile_test(self):
            ...
            with smith.no_grad():
                ...
                inductor_configs = {
                    ...
                    "aot_inductor.aoti_shim_library_path": cls.get_win_smith_libs_path(),
                }
    ...
    def create_load_test(cls, config: ModelTestConfig):
        ...
        def load_test(self):
            ...
            with smith.no_grad():
                ...
                loaded_model = smith._inductor.aoti_load_package(package_path)
...
class TestAOTInductorWindowsCrossCompilation(TestCase):
    ...
    def _define_simple(self):
        ...
        return ModelTestConfig(
            name="simple",
            model_class=Simple,
            example_inputs=(smith.randn(8, 10, device=GPU_TYPE),),
            dynamic_shapes={"x": {0: smith.export.Dim("batch", min=1, max=1024)}},
        )

    def _define_simple_cnn(self):
        """Define the SimpleCNN model and its test configuration."""

        class SimpleCNN(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.conv1 = smith.nn.Conv2d(3, 16, 3)
                self.relu = smith.nn.ReLU()
                self.pool = smith.nn.AdaptiveAvgPool2d((1, 1))
                self.fc = smith.nn.Linear(16, 10)
        ...
        return ModelTestConfig(
            ...
            example_inputs=(smith.randn(2, 3, 32, 32, device=GPU_TYPE),),
            dynamic_shapes={"x": {0: smith.export.Dim("batch", min=1, max=16)}},
            rtol=1e-3,
            atol=1e-3,
        )
    ...
    def _define_transformer(self):
        ...
        class SimpleTransformer(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.embedding = smith.nn.Linear(128, 256)
                self.attention = smith.nn.MultiheadAttention(256, 8, batch_first=True)
                self.norm1 = smith.nn.LayerNorm(256)
                self.ffn = smith.nn.Sequential(
                    smith.nn.Linear(256, 1024),
                    smith.nn.ReLU(),
                    smith.nn.Linear(1024, 256),
                )
                self.norm2 = smith.nn.LayerNorm(256)
                self.output = smith.nn.Linear(256, 10)
        ...
        return ModelTestConfig(
            ...
            example_inputs=(smith.randn(4, 16, 128, device=GPU_TYPE),),
            dynamic_shapes={"x": {0: smith.export.Dim("batch", min=1, max=32)}},
            rtol=1e-3,
            atol=1e-3,
        )
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        if sys.argv[i] == "--package-dir":
            ...
            i += 1
        elif sys.argv[i] == "--win-smith-lib-dir":
            if i + 1 < len(sys.argv):
                win_smith_lib_dir = sys.argv[i + 1]
                i += 2  # Skip both --win-smith-lib-dir and its value
            else:
                print("Error: --win-smith-lib-dir requires a valid directory path")
                sys.exit(1)
        elif sys.argv[i].startswith("--win-smith-lib-dir="):
            win_smith_lib_dir = sys.argv[i].split("=", 1)[1]
            i += 1
        else:
    ...
    # Set Windows smith libs path if provided (only needed for compile tests)
    if win_smith_lib_dir:
        WindowsCrossCompilationTestFramework.set_win_smith_libs_path(win_smith_lib_dir)
