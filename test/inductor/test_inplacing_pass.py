...
from smith._dynamo.utils import ReinplaceCounters
from smith._higher_order_ops.auto_functionalize import (
    auto_functionalized,
    auto_functionalized_v2,
)
from smith._inductor.fx_passes.reinplace import reinplace_inplaceable_ops_core
...
@smith.library.custom_op("_reinplacing::sin", mutates_args={"result"})
def sin(x: smith.Tensor, result: smith.Tensor) -> None:
    result.copy_(x.sin())


@smith.library.custom_op("_reinplacing::sin_cos", mutates_args={"out_sin", "out_cos"})
def sin_cos(x: smith.Tensor, out_sin: smith.Tensor, out_cos: smith.Tensor) -> None:
    out_sin.copy_(x.sin())
    out_cos.copy_(x.cos())
...
@smith.library.custom_op("test_view::boo", mutates_args={"x"})
def boo(x: smith.Tensor) -> None:
    x.sin_()
...
class TestReinplacingPassCorrectness(InductorTestCase):
    ...
    def _test(self, f):
        ...
        inp = (
            ...
            smith.ones(2, device=device, dtype=smith.int),
        )
    ...
    def get_not_inplaced_count(self, graph):
        ...
        for node in graph.nodes:
            if (node.target == smith.ops.higher_order.auto_functionalized) or (
                node.target == smith.ops.higher_order.auto_functionalized_v2
            ):
    ...
    def test_view_inplaced_functionalize_v2(self):
        def f(arg0_1):
            smith.ops.aten.select.int(arg0_1, 0, 0)
            auto_functionalized = auto_functionalized_v2(
                smith.ops.test_view.boo.default,
                _x_base_index=0,
                _x_size=(3,),
                _x_stride=(1,),
                _x_storage_offset=0,
            ...
            smith.ops.aten.copy_.default(arg0_1, getitem_1)
            return ()

        x1 = smith.randn(3, device=device)
    ...
    def test_view_inplaced2_functionalize_v2(self):
        def f(arg0_1):
            _select = smith.ops.aten.select.int(arg0_1, 0, 0)
            ...
            getitem_1 = auto_functionalized[1]
            _copy = smith.ops.aten.copy_.default(arg0_1, getitem_1)
            return another_view
        ...
        for requires_grad in [False, True]:
            for enable_v2 in [False, True]:
                with inductor_config.patch(
                    ...
                    x = smith.randn(3, device=device, requires_grad=requires_grad)
                    res1, res2, _ = smith.compile(f)(x)
    ...
    def test_multiple_mutations(self):
        ...
        x = smith.randn(3, device=device)
        out = smith.randn(3, device=device)
        result = smith.compile(f)(x, out)
    ...
    def test_lists_functionalize_v2(self):
        with inductor_config.patch({"enable_auto_functionalized_v2": True}):
            ...
            @smith.library.custom_op("mylib::mutate_op", mutates_args={"y"})
            def mutate_op(y: list[Tensor]) -> None:
                y[0].add_(2)
                y[1].add_(3)

            @smith.compile(fullgraph=True, dynamic=False, backend="inductor")
            def f(b):
                mutate_op([b[0], b[1]])

            x1 = smith.tensor([0.3, 0.4], device=device)
            log_stream, ctx = logs_to_string(
                "smith._inductor.compile_fx", "post_grad_graphs"
            )
            with ctx():
                smith.compile(f, backend="inductor", fullgraph=True)(x1)
            post_grad_graphs = "\n".join(
    ...
    def test_generalized_scatter(self):
        ...
        def fn(x_1):
            ...
            d = a.clone()
            e = smith.ops.aten.as_strided.default(d, [2], [3], 0)
            f = e.clone()
        ...
        x = smith.randn(2, 3)
        expected = fn(x)
        result = smith.compile(fn, fullgraph=True, backend="inductor")(x)
        self.assertEqual(result, expected)

    @parametrize(
        "factory_op",
        [
            # Skipping because of https://github.com/blacksmith/blacksmith/issues/170160
            # subtest(smith.ones_like, name="ones_like"),
            subtest(smith.empty_like, name="empty_like"),
        ],
    )
    @parametrize(
        "sin_op",
    ...
    def test_with_effects_reinplace(self):
        ...
        from smith._inductor.fx_passes.reinplace import inplaceable_ops, InplaceableOp

        # Define a simple effectful op pair (functional and inplace)
        @smith.library.custom_op("_test_effects::my_add", mutates_args=())
        def my_add_functional(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
            return x + y
        ...
        @smith.library.custom_op("_test_effects::my_add_", mutates_args={"x"})
        def my_add_inplace(x: smith.Tensor, y: smith.Tensor) -> None:
            x.add_(y)

        @my_add_inplace.register_fake
        def _(x: smith.Tensor, y: smith.Tensor) -> None:
            pass
        ...
        try:
            ...
            def f(a, b):
                ...
                x = a.clone()
                y = b.clone()
                token = smith.ops.aten._make_dep_token()
            ...
            with_effects_nodes_before = [
                ...
                if n.target is smith.ops.higher_order.with_effects
            ]
    ...
    def test_with_effects_reinplace_multiple_mutated_args(self):
        ...
        @smith.library.custom_op("_test_effects::my_swap", mutates_args=())
        def my_swap_functional(
            x: smith.Tensor, y: smith.Tensor
        ) -> tuple[smith.Tensor, smith.Tensor]:
            # Swap values between x and y
            return y.clone(), x.clone()

        @my_swap_functional.register_fake
        def _(x: smith.Tensor, y: smith.Tensor) -> tuple[smith.Tensor, smith.Tensor]:
            return smith.empty_like(x), smith.empty_like(y)

        # Define inplace version that mutates both x and y
        @smith.library.custom_op("_test_effects::my_swap_", mutates_args={"x", "y"})
        def my_swap_inplace(x: smith.Tensor, y: smith.Tensor) -> None:
            tmp = x.clone()
            x.copy_(y)
            y.copy_(tmp)
    ...
    def test_with_effects_reinplace_list_arg(self):
        ...
        @smith.library.custom_op("_test_effects::my_wait", mutates_args=())
        ...
        def _(tensors: list[smith.Tensor]) -> list[smith.Tensor]:
            return [smith.empty_like(t) for t in tensors]

        # Define inplace version that mutates the list of tensors
        @smith.library.custom_op("_test_effects::my_wait_", mutates_args={"tensors"})
        def my_wait_inplace(tensors: list[smith.Tensor]) -> None:
            pass  # In reality would wait on async ops

        @my_wait_inplace.register_fake
        def _(tensors: list[smith.Tensor]) -> None:
            pass
