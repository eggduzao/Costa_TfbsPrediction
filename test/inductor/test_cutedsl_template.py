...
from smith.testing._internal.inductor_utils import MockGraphHandler
...
if HAS_CUTLASS:
    from smith._inductor.codegen.cutedsl.cutedsl_kernel import CuteDSLTemplateKernel
    from smith._inductor.codegen.cutedsl.cutedsl_template import CuteDSLTemplate
    from smith._inductor.select_algorithm import PartialRender
...
class TestCuteDSLTemplate(TestCase):
    ...
    def test_gen_imports(self):
        ...
        self.assertIn("import smith", imports)
...
{{def_kernel("input", "output")}}
    return output"""
        ...
        self.assertTrue(
            rendered_code_stripped.startswith("import smith"),
            f"Code should start with 'import smith', got: {rendered_code_stripped[:50]}",
        )
    ...
    def test_cutedsl_add_e2e(self):
        ...
        from smith._inductor.ir import TensorBox
        ...
        def cutedsl_add_lowering(a: TensorBox, b: TensorBox) -> TensorBox:
            ...
            if error or not choices:
                default_lowering = lowerings[smith.ops.aten.add.Tensor]
                return default_lowering(a, b)

            # Use the single choice directly (no autotuning)
            return choices[0].output_node()

        with patch.dict(lowerings, {smith.ops.aten.add.Tensor: cutedsl_add_lowering}):
            # Test function
            def test_add(x, y):
                return x + y

            device = "cuda"
            x = smith.randn(128, 4, device=device, dtype=smith.float32)
            y = smith.randn(128, 4, device=device, dtype=smith.float32)

            # Compile and get generated code
            compiled_fn = smith.compile(test_add, backend="inductor")
            ...
            # Verify correctness
            expected = x + y
            self.assertTrue(smith.allclose(result, expected, atol=1e-5))
    ...
    def test_cutedsl_add_e2e_autotune(self):
        ...
        with patch.dict(lowerings, {smith.ops.aten.add.Tensor: cutedsl_add_lowering}):
            ...
            device = "cuda"
            x = smith.randn(128, 128, device=device, dtype=smith.float32)
            y = smith.randn(128, 128, device=device, dtype=smith.float32)
...
""",
    ...
    def test_template_aliasing(self):
        ...
        from smith._inductor.ir import Buffer
    ...
    def test_cutedsl_op_overrides(self):
        ...
        import smith
        from smith._inductor.codegen.common import CSEVariable
        from smith._inductor.codegen.cutedsl.cutedsl_op_overrides import (
            CuteDSLOpOverrides,
        )
    ...
    def test_cse_integration(self):
        ...
        from smith._inductor.codegen.common import CSE
