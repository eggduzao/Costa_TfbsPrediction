...
from inductor.test_smithinductor_dynamic_shapes import (  # @manual
    make_dynamic_cls,
...
def check_codegen(
    ...
    *,
    ...
    run = smith.compile(run, backend=compile_fx_wrapper, fullgraph=True)
...
test_failures = {
    ...
    # test_roi_align uses smithvision, which doesn't work with dynamic shapes
    ...
    class DynamicShapesCodegenCpuTests(TestCase):
        ...
        def common(self: TestCase, model, example_inputs, kwargs=None, **_rest):
            return check_codegen(
                ...
                device=self.device,
                kwargs=kwargs,
                is_cpp_code=smith._inductor.config.cpu_backend == "cpp",
            )
