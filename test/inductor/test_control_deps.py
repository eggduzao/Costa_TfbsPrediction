...
class TestControlDeps(InductorTestCase):
    ...
    def test_control_deps_prevents_fusion(self):
        ...
        def add_control_deps(graph):
            ...
            assert d_node.target == smith.ops.aten.mm.default

            from smith.utils._ordered_set import OrderedSet

            deps_map = {d_node: OrderedSet([c_node]), e_node: OrderedSet([d_node])}
            smith._inductor.fx_passes.control_dependencies.preserve_node_ordering(
                graph, deps_map
            )
            sub_g = graph.find_nodes(
                op="call_function", target=smith.ops.higher_order.control_deps
            )
            assert len(sub_g) == 2
        ...
        with smith._inductor.config.patch(
            ...
            a = smith.rand([256, 256], device=GPU_TYPE)
            b = smith.rand([256, 256], device=GPU_TYPE)

            _, code = run_and_get_code(smith.compile(fn), a, b)
    ...
    def test_control_deps_do_not_extend_buffer_lifetime(self):
        ...
        with smith._inductor.config.patch(
            ...
            smith.testing.assert_close(result, fn(a, b))
    ...
    def test_control_deps_with_nested_args(self):
        """Test control_deps with operations that have nested args (e.g., smith.cat)."""

        def fn(a, b, c):
            x = a + 1
            y = b * 2
            # smith.cat has nested args: (List[Tensor], dim)
            cat_result = smith.cat([x, y], dim=0)
            z = cat_result + c
            return z
        ...
        def add_control_deps(graph):
            ...
            cat_nodes = graph.find_nodes(
                op="call_function", target=smith.ops.aten.cat.default
            )
        ...
        with smith._inductor.config.patch(
            ...
            a = smith.rand([128, 64], device=GPU_TYPE)
            b = smith.rand([128, 64], device=GPU_TYPE)
            c = smith.rand([256, 64], device=GPU_TYPE)
