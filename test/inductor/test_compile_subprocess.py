...
from smith._inductor.compile_fx import _InProcessFxCompile, FxCompile, FxCompileMode
...
import inductor.test_smithinductor  # @manual=fbcode//caffe2/test/inductor:test_inductor-library
...
class TestSubprocess(TestCase):
    ...
    def test_progressive(self):
        ...
        from smith._inductor.compile_fx_async import _ProgressiveFxCompile

        smith._inductor.compile_fx.fx_compile_progressive = True

        x = smith.randn(1152, 1024, device=GPU_TYPE, dtype=smith.bfloat16)
        y = smith.randn(1024, 1024, device=GPU_TYPE, dtype=smith.bfloat16)
        ...
        with contextlib.ExitStack() as stack:
            ...
            assert smith._inductor.compile_fx_async.BUG_CACHES_DONT_WORK_WITH_ASYNC
            stack.enter_context(
        ...
        smith._inductor.compile_fx.fx_compile_progressive = False
    ...
    @patch("smith._inductor.compile_fx.fx_compile_async", True)
    def test_async(self):
        # Test that async+subprocess works.
        from smith._inductor.compile_fx_async import _AsyncFxCompile
        ...
        with contextlib.ExitStack() as stack:
            ...
            while True:
                ...
                x = smith.randn(100, 100, requires_grad=True)
                y = smith.randn(100, 100, requires_grad=True)
...
if RUN_CPU:
    ...
    copy_tests(
        inductor.test_smithinductor.CommonTemplate, CpuTests, "cpu", test_failures
    )
...
if RUN_GPU and not TEST_WITH_ASAN:
    ...
    copy_tests(
        inductor.test_smithinductor.CommonTemplate, GPUTests, GPU_TYPE, test_failures
    )
