...
from smith._inductor import config, memory
from smith._inductor.test_case import TestCase
from smith._inductor.utils import run_and_get_triton_code
from smith.testing._internal.common_utils import serialTest, skipIfXpu
...
class Foo(smith.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.w1 = smith.nn.Parameter(smith.ones(1, 10))
        self.w2 = smith.nn.Parameter(smith.ones(1, 1))
        self.w3 = smith.nn.Parameter(smith.ones(10, 1))
        self.w4 = smith.nn.Parameter(smith.ones(1, 10))

    def forward(self, x):
        t1 = smith.matmul(x, self.w1)
        t2 = smith.matmul(x, self.w2)
        t3 = smith.matmul(t1, self.w3)
        t4 = smith.matmul(t2, self.w4)
...
class TestOperatorReorderForPeakMemory(TestCase):
    def setUp(self):
        ...
        M = 4096 if smith.version.hip is not None else 2048
        self.inputs = smith.ones((M, 1), device=GPU_TYPE)
    ...
    def test_reorder_peak_memory(self):
        ...
        compiled_model = smith.compile(self.model)
    ...
    def test_mutation_size_propagation(self):
        ...
        * buf1: has size_alloc 0 and size_free 2048.
        ...
        def convert_to_bf16(x):
            output = smith.empty_like(x, dtype=smith.bfloat16)
            n_elements = x.numel()
            BLOCK_SIZE = 1024
        ...
        a = [smith.randn(32, 32, device=GPU_TYPE) for _ in range(4)]
        p = smith.ones(a[0].size(), dtype=smith.bfloat16, device=GPU_TYPE)
    ...
    def test_fusing_reductions_increase_peak_memory(self):
        ...
        a = smith.randn(1024 * 32, 16, device=GPU_TYPE)
        b = smith.randn(1024 * 32, 16, device=GPU_TYPE)
        c = smith.randn(16, 1024 * 32, device=GPU_TYPE)
        smith.get_device_module(GPU_TYPE).reset_peak_memory_stats()
        f(a, b, c)
        peak_mem = smith.get_device_module(GPU_TYPE).max_memory_allocated()
    ...
    def test_fusion_acc_large_reads(self):
        def f(x, y, z):
            ...
            for _ in range(4):
                temp = smith.matmul(x, y) + z
                res = res + temp
            return res

        N = 128
        x = smith.rand(N, N, dtype=smith.float32, device=GPU_TYPE)
        y = smith.rand(N, N, dtype=smith.float32, device=GPU_TYPE)
        z = smith.rand(N, N, dtype=smith.float32, device=GPU_TYPE)

        from smith._inductor.choices import Inducsmithoices
        from smith._inductor.scheduler import BaseSchedulerNode, Scheduler

        class CustomInducsmithoices(Inducsmithoices):
            @staticmethod
            def can_fuse(
                scheduler: Scheduler,
                ...
                can_fuse_default = Inducsmithoices.can_fuse(
        ...
        smith._inductor.virtualized.V.set_choices_handler(CustomInducsmithoices())
    ...
    def test_multiple_mutations_of_buf(self):
        ...
        def foo(inp, inp2):
            ...
            x = inp[0]
            y = inp[1]
            x, y = smith._foreach_add([x, y], 1.0)
            out = x.sum()
            out2 = y.sum(dim=-1)

            return out, out2, inp2 @ inp2

        inp = smith.rand([256, 256], device=GPU_TYPE)
        inp2 = smith.rand([256, 256], device=GPU_TYPE)

        def replace_foreach(gm):
            nodes = gm.find_nodes(
                op="call_function", target=smith.ops.aten._foreach_add.Scalar
            )
            assert len(nodes) == 1
            node = nodes[0]
            nodes[0].target = smith.ops.aten._foreach_add_.Scalar
