...
import smith._inductor.cpu_vec_isa
import smith._inductor.select_algorithm as select_algorithm
from smith._dynamo.utils import counters
from smith._inductor import test_operators
from smith._inductor.cpu_vec_isa import VecAMX
...
try:
    try:
        from . import test_cpu_repro, test_smithinductor
    except ImportError:
...
def _get_epilogue(epilogue: str, other: Optional[smith.Tensor] = None):
    if epilogue == "none":
        return lambda x: x
    elif epilogue == "relu":
        return smith.nn.ReLU()
    elif epilogue == "gelu":
        return smith.nn.GELU()
    elif epilogue == "silu":
        return smith.nn.SiLU()
    elif epilogue == "sigmoid":
        return smith.nn.Sigmoid()
    elif epilogue == "tanh":
        return smith.nn.Tanh()
    elif epilogue == "hardswish":
        return smith.nn.Hardswish()
    elif epilogue == "hardsigmoid":
        return smith.nn.Hardsigmoid()
    elif epilogue == "leaky_relu":
        return smith.nn.LeakyReLU()
    elif epilogue == "hardtanh":
        return smith.nn.Hardtanh()
    elif epilogue == "add":
...
class BaseTestSelectAlgorithm(TestCase):
    ...
    def _check_brgemm_counter(self, vec_amx):
        if vec_amx and smith.cpu._is_amx_fp16_supported():
...
class TestSelectAlgorithm(BaseTestSelectAlgorithm):
    ...
    @dtypes(smith.float, smith.bfloat16, smith.half)
    def test_linear_static_shapes(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.linear = smith.nn.Linear(in_features, out_features, bias)
        ...
        v = smith.randn(*B, in_features).to(dtype=dtype)
    ...
    def test_linear_wgt_multi_users(self, in_features, out_features, bias, dtype):
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.embeddings = smith.nn.Embedding(out_features, in_features)
                self.linear = smith.nn.Linear(in_features, out_features, bias)
        ...
        counters.clear()
        mod = M(bias=bias).to(dtype=dtype).eval()
        v = smith.LongTensor([[1, 2, 4, 5], [4, 3, 2, 9]])
        with verify(dtype) as (atol, rtol):
            self.common(mod, (v,), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_linear_input_transpose(self, bias, dtype):
        ...
        counters.clear()
        mod = M(bias=bias).to(dtype=dtype).eval()
        v = smith.randn(in_features, batch_size).to(dtype=dtype)
    ...
    def test_linear_with_pointwise(
        ...
        counters.clear()
        v = smith.randn(batch_size, in_features).to(dtype=dtype)
        u = smith.randn(batch_size, out_features).to(dtype=dtype)
        ...
        if (
            (
                ...
                or (
                    dtype == smith.float16
                    and smith.ops.mkldnn._is_mkldnn_fp16_supported()
                )
                or (
                    dtype == smith.float32
            ...
            and epilogue != "div"
            or (
                dtype in (smith.float16, smith.bfloat16)
                and epilogue == "add"
                and not bias
            )
        ):
    ...
    def test_linear_with_transpose(
        ...
        counters.clear()
        v = smith.randn(batch_size, in_features).to(dtype=dtype)
        u = smith.randn(out_features, batch_size).to(dtype=dtype)
        other = smith.randn(batch_size, out_features).to(dtype=dtype)
    ...
    def test_linear_with_permute(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias, has_non_epilogue_users):
                super().__init__()
                self.linear = smith.nn.Linear(in_features, out_features, bias)
                self._frozen_param398 = smith.randn(batch_size, out_features, 1, 1)
                ...
                self.linear2 = smith.nn.Linear(out_features, out_features, bias)
                self._frozen_param400 = smith.randn(batch_size, out_features, 1, 1)
                self.has_non_epilogue_users = has_non_epilogue_users

            def forward(self, mul_272, _convolution_pointwise_default_31):
                out1 = smith.ops.prims.convert_element_type.default(
                    mul_272, smith.bfloat16
                )
                mul_272 = None

                _linear_pointwise_default_131 = self.linear(out1)
                permute_188 = smith.ops.aten.permute.default(
                    _linear_pointwise_default_131, [0, 3, 1, 2]
                )

                mul_273 = smith.ops.aten.mul.Tensor(permute_188, self._frozen_param398)
                add_187 = smith.ops.aten.add.Tensor(
                    mul_273, _convolution_pointwise_default_31
                )
                convert_element_type_847 = smith.ops.prims.convert_element_type.default(
                    add_187, smith.bfloat16
                )
                _convolution_pointwise_default_29 = self.conv(convert_element_type_847)
                permute_189 = smith.ops.aten.permute.default(
                    _convolution_pointwise_default_29, [0, 2, 3, 1]
                )
                permute_189 = self.linear2(permute_189)
                permute_189 = smith.ops.aten.permute.default(permute_189, [0, 3, 1, 2])
                permute_189 = smith.ops.aten.mul.Tensor(
                ...
                if self.has_non_epilogue_users:
                    add_191 = smith.ops.aten.add.Tensor(permute_189, add_187)
                    return add_191
                return permute_189

        view_12 = smith.randn(batch_size, image_size, image_size, in_features)
        _convolution_pointwise_default_31 = smith.randn(
        ...
        with verify(dtype) as (atol, rtol), smith.cpu.amp.autocast():
            self.common(
                mod,
                (
                    view_12,
    ...
    def test_linear_with_input_of_flexible_layout(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                ...
                self._frozen_param151 = smith.randn(1, 1, linear_in_features)
                self._frozen_param3 = smith.randn(1, 1, linear_in_features)
                self._frozen_param2 = smith.randn(linear_in_features)

                self.linear = smith.nn.Linear(out_features, out_features, bias)

            def forward(self, arg150_1):
                _convolution_pointwise_default = self.conv(arg150_1)
                view_73 = smith.ops.aten.reshape.default(
                    _convolution_pointwise_default,
                    [batch_size, linear_in_features, out_features],
                )
                _convolution_pointwise_default = None
                permute_62 = smith.ops.aten.permute.default(view_73, [0, 2, 1])
                view_73 = None
                mul_111 = smith.ops.aten.mul.Tensor(self._frozen_param151, permute_62)
                add_73 = smith.ops.aten.add.Tensor(self._frozen_param3, mul_111)
                permute_63 = smith.ops.aten.permute.default(add_73, [0, 2, 1])
                add_73 = None
                view_74 = smith.ops.aten.reshape.default(
                    permute_63, [flatten_BS, out_features]
                )
                permute_63 = None
                _mkl_linear_36 = self.linear(view_74)
                view_75 = smith.ops.aten.reshape.default(
                    _mkl_linear_36, [batch_size, linear_in_features, out_features]
                )
                _mkl_linear_36 = None
                permute_65 = smith.ops.aten.permute.default(view_75, [0, 2, 1])
                view_75 = None
                mul_112 = smith.ops.aten.mul.Tensor(self._frozen_param2, permute_65)
                _frozen_param2 = permute_65 = None
                add_74 = smith.ops.aten.add.Tensor(permute_62, mul_112)
                permute_62 = mul_112 = None
                return add_74

        v = smith.randn(batch_size, in_features, 224, 224).to(dtype=dtype)
        mod = M(bias=bias).to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (v,), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_epilogue_fusion_counter"], 1)
    ...
    def test_linear_unsupported_epilogue_fusion(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.linear1 = smith.nn.Linear(in_features, in_features, bias=bias)
                self.linear2 = smith.nn.Linear(out_features, in_features, bias=bias)
                self.conv = smith.nn.Conv2d(
                    in_features,
                    out_features_conv,
                    kernel_size=3,
            ...
            def forward(self, mul_239, view_425, add_184):
                ...
                view_426 = smith.ops.aten.reshape.default(
                    _mkl_linear_91, [batch_size, img_size_0, img_size_1, in_features]
                )
                _mkl_linear_91 = None
                add_187 = smith.ops.aten.add.Tensor(add_184, view_426)
                add_184 = view_426 = None
                view_429 = smith.ops.aten.reshape.default(
                ...
                if self.epilogue:
                    _mkl_linear_89 = smith.pow(_mkl_linear_89, 2)
                    _mkl_linear_89 = test_operators.realize(_mkl_linear_89)

                view_430 = smith.ops.aten.reshape.default(
                    _mkl_linear_89, [batch_size, img_size_0, img_size_1, in_features]
                )
                _mkl_linear_89 = None

                add_191 = smith.ops.aten.add.Tensor(add_187, view_430)
                add_187 = view_430 = None

                view_431 = smith.ops.aten.reshape.default(
                    add_191, [batch_size, size_0, size_0, size_1, size_1, in_features]
                )
                add_191 = None
                permute_203 = smith.ops.aten.permute.default(
                    view_431, [0, 1, 3, 2, 4, 5]
                )
                view_431 = None
                clone_188 = smith.ops.aten.clone.default(
                    permute_203, memory_format=smith.contiguous_format
                )
                permute_203 = None
                view_432 = smith.ops.aten.reshape.default(
                    clone_188, [batch_size, conv_shape, conv_shape, in_features]
                )
                clone_188 = None
                permute_204 = smith.ops.aten.permute.default(view_432, [0, 3, 1, 2])
                view_432 = None
        ...
        mul_239 = smith.randn(batch_size, img_size_0, img_size_1, out_features)
        view_425 = smith.randn(flatten_BS, in_features)
        add_184 = smith.randn(batch_size, img_size_0, img_size_1, in_features)
        mod = M(bias=bias).eval()
        with (
            verify(dtype) as (atol, rtol),
            smith.cpu.amp.autocast(enabled=dtype == smith.bfloat16),
        ):
            self.common(
                mod,
                (
    ...
    def test_linear_with_binary_input_3d(
        ...
        B = (2, batch_size)
        v = smith.randn(*B, in_features).to(dtype=dtype)
        u = smith.randn(*B, out_features).to(dtype=dtype)
    ...
    def test_linear_local_and_global_buffer_dynamic_shapes(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.linear = smith.nn.Linear(in_features, out_features, bias)
                self.linear1 = smith.nn.Linear(out_features, out_features, bias)
                self.linear2 = smith.nn.Linear(out_features, out_features2, bias)

            def forward(self, arg7_1):
                addmm_3 = self.linear(arg7_1)
                relu_2 = smith.ops.aten.relu.default(addmm_3)

                addmm_4 = self.linear1(relu_2)
                relu_3 = smith.ops.aten.relu.default(addmm_4)

                addmm_5 = self.linear2(relu_3)

                split_1 = smith.ops.aten.split.Tensor(addmm_5, 1, 1)
                getitem_2 = split_1[0]
                getitem_3 = split_1[1]

                tanh_1 = smith.ops.aten.tanh.default(getitem_3)

                add_62 = smith.ops.aten.add.Tensor(tanh_1, 1)

                mul_36 = smith.ops.aten.mul.Tensor(add_62, 6.0)
                add_69 = smith.ops.aten.add.Tensor(mul_36, -10.0)

                exp_1 = smith.ops.aten.exp.default(add_69)
    ...
    @unittest.skipIf(
        not smith.cpu._is_amx_tile_supported(), "AMX ISA support is required"
    )
    @inductor_config.patch({"freezing": True})
    @patches
    @smith.no_grad
    ...
    def test_linear_amx(self, batch_size, in_features, out_features, bias, dtype):
        ...
        if dtype == smith.half and not vec_amx.is_amx_fp16_supported():
    ...
    def test_linear_with_multiple_reindexers(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                ...
                self.linear = smith.nn.Linear(in_features, out_features, bias=False)
                self._frozen_param221 = smith.randn(out_features)
                self._frozen_param389 = smith.randn(out_features)
                self._frozen_param20 = smith.randn(out_features)
                self._frozen_param21 = smith.randn(out_features)

            def forward(self, view_368):
                _mkl_linear_57 = self.linear(view_368)
                view_369 = smith.ops.aten.reshape.default(
                    _mkl_linear_57, [batch_size, in_features_2, out_features]
                )
                _mkl_linear_57 = None

                view_370 = smith.ops.aten.reshape.default(
                    view_369, [flatten_BS, out_features]
                )
                view_369 = None
                sub_85 = smith.ops.aten.sub.Tensor(view_370, self._frozen_param221)
                view_370 = _frozen_param221 = None
                mul_261 = smith.ops.aten.mul.Tensor(sub_85, self._frozen_param389)
                sub_85 = _frozen_param389 = None
                mul_262 = smith.ops.aten.mul.Tensor(mul_261, self._frozen_param20)
                mul_261 = _frozen_param20 = None
                add_219 = smith.ops.aten.add.Tensor(mul_262, self._frozen_param21)
                mul_262 = _frozen_param21 = None
                view_371 = smith.ops.aten.reshape.default(
                    add_219, [batch_size, in_features_2, out_features]
                )
                add_219 = None

                add_220 = smith.ops.aten.add.Tensor(view_371, 3)
                clamp_min_35 = smith.ops.aten.clamp_min.default(add_220, 0)
                add_220 = None
                clamp_max_35 = smith.ops.aten.clamp_max.default(clamp_min_35, 6)
                clamp_min_35 = None
                mul_263 = smith.ops.aten.mul.Tensor(view_371, clamp_max_35)
                view_371 = clamp_max_35 = None
                div_51 = smith.ops.aten.div.Tensor(mul_263, 6)
                mul_263 = None

                return div_51

        view_368 = smith.randn(flatten_BS, in_features)
    ...
    def test_linear_with_embedding(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.linear = smith.nn.Linear(in_features, out_features, bias).to(
                    dtype=dtype
                )
                self.emb = smith.nn.Embedding(64, out_features)

            def forward(self, idx, x):
                return self.emb(idx) + self.linear(x)

        idx = smith.randint(0, 64, (batch_size,))
        x = smith.randn(batch_size, in_features).to(dtype=dtype)
        mod = M(bias=bias).eval()
        with verify(dtype) as (atol, rtol):
    ...
    def test_linear_with_indirect_indexing(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                super().__init__()
                self.wte = smith.nn.Embedding(128, seq_lens)
                self.wpe = smith.nn.Embedding(in_features, seq_lens)
                self.linear = smith.nn.Linear(out_features, seq_lens, bias)

            def forward(self, view_12, input_ids, view_9):
                inputs_embeds = self.wte(input_ids)

                position_ids = smith.arange(0, in_features, dtype=smith.long)
                ...
                view_13 = smith.ops.aten.reshape.default(
                    _linear_pointwise_default_45, [batch_size, in_features, seq_lens]
                )
                out = smith.ops.aten.add.Tensor(add_4, view_13)

                return out

        view_12 = smith.randn(batch_size * in_features, out_features)
        input_ids = smith.randint(0, 128, (batch_size, in_features))
        view_9 = smith.randn(batch_size, in_features, seq_lens)
        mod = M(bias=bias).eval()
        with verify(dtype) as (atol, rtol), smith.cpu.amp.autocast():
            self.common(
                mod,
    ...
    def test_linear_with_in_out_buffer(
        ...
        class M(smith.nn.Module):
            def __init__(self, bias):
                ...
                self.linear = smith.nn.Linear(out_features, in_features2, bias)

                self.linear2 = smith.nn.Linear(out_features, out_features, bias)
                self._frozen_param2 = smith.randn(out_features)
                self._frozen_param3 = smith.randn(out_features)
                self._frozen_param7 = smith.randn(out_features)
                self._frozen_param8 = smith.randn(out_features)
                self._frozen_param153 = smith.randn(batch_size, 1, out_features)

            def forward(self, arg152_1):
                _convolution_pointwise_default_35 = self.conv(arg152_1)
                arg152_1 = None

                view_168 = smith.ops.aten.reshape.default(
                    _convolution_pointwise_default_35, [8, 64, 3136]
                )
                _convolution_pointwise_default_35 = None
                permute_97 = smith.ops.aten.permute.default(view_168, [0, 2, 1])
                view_168 = None
                clone_65 = smith.ops.aten.clone.default(
                    permute_97, memory_format=smith.contiguous_format
                )
                permute_97 = None
                var_mean_21 = smith.ops.aten.var_mean.correction(
                    clone_65, [2], correction=0, keepdim=True
                )
                getitem_90 = var_mean_21[0]
                getitem_91 = var_mean_21[1]
                var_mean_21 = None
                add_82 = smith.ops.aten.add.Tensor(getitem_90, 1e-05)
                getitem_90 = None
                rsqrt_21 = smith.ops.aten.rsqrt.default(add_82)
                add_82 = None
                sub_29 = smith.ops.aten.sub.Tensor(clone_65, getitem_91)
                clone_65 = getitem_91 = None
                mul_82 = smith.ops.aten.mul.Tensor(sub_29, rsqrt_21)
                sub_29 = rsqrt_21 = None
                mul_83 = smith.ops.aten.mul.Tensor(mul_82, self._frozen_param2)
                mul_82 = None
                add_83 = smith.ops.aten.add.Tensor(mul_83, self._frozen_param3)
                mul_83 = None
                _frozen_param153 = self._frozen_param153
                cat_20 = smith.ops.aten.cat.default([_frozen_param153, add_83], 1)
                _frozen_param153 = add_83 = None
                slice_111 = smith.ops.aten.slice.Tensor(cat_20, 1, 0, 1)
                slice_113 = smith.ops.aten.slice.Tensor(
                    cat_20, 1, 1, 9223372036854775807
                )
                cat_20 = None
                permute_98 = smith.ops.aten.permute.default(slice_113, [0, 2, 1])
                slice_113 = None
                view_169 = smith.ops.aten.reshape.default(permute_98, [8, 64, 56, 56])
                permute_98 = None
                _convolution_pointwise_default_34 = self.conv2(view_169)

                add_84 = smith.ops.aten.add.Tensor(
                    _convolution_pointwise_default_34, view_169
                )
                _convolution_pointwise_default_34 = view_169 = None
                view_170 = smith.ops.aten.reshape.default(add_84, [8, 64, 3136])
                add_84 = None
                permute_99 = smith.ops.aten.permute.default(view_170, [0, 2, 1])
                view_170 = None
                cat_21 = smith.ops.aten.cat.default([slice_111, permute_99], 1)
                slice_111 = permute_99 = None
                var_mean_22 = smith.ops.aten.var_mean.correction(
                    cat_21, [2], correction=0, keepdim=True
                )
                getitem_92 = var_mean_22[0]
                getitem_93 = var_mean_22[1]
                var_mean_22 = None
                add_85 = smith.ops.aten.add.Tensor(getitem_92, 1e-06)
                getitem_92 = None
                rsqrt_22 = smith.ops.aten.rsqrt.default(add_85)
                add_85 = None
                sub_30 = smith.ops.aten.sub.Tensor(cat_21, getitem_93)
                getitem_93 = None
                mul_84 = smith.ops.aten.mul.Tensor(sub_30, rsqrt_22)
                sub_30 = rsqrt_22 = None
                mul_85 = smith.ops.aten.mul.Tensor(mul_84, self._frozen_param7)
                mul_84 = None
                add_86 = smith.ops.aten.add.Tensor(mul_85, self._frozen_param8)
                mul_85 = None
                view_171 = smith.ops.aten.reshape.default(add_86, [25096, 64])
                add_86 = None

                _mkl_linear_32 = self.linear(view_171)
                view_171 = None

                view_172 = smith.ops.aten.reshape.default(
                    _mkl_linear_32, [8, 3137, 192]
                )
                _mkl_linear_32 = None
                view_173 = smith.ops.aten.reshape.default(view_172, [8, 3137, 3, 8, 8])
                view_172 = None
                permute_101 = smith.ops.aten.permute.default(view_173, [2, 0, 3, 1, 4])
                view_173 = None
                unbind_8 = smith.ops.aten.unbind.int(permute_101)
                permute_101 = None
                getitem_94 = unbind_8[0]
                getitem_95 = unbind_8[1]
                getitem_96 = unbind_8[2]
                unbind_8 = None
                clone_66 = smith.ops.aten.clone.default(
                    getitem_95, memory_format=smith.contiguous_format
                )
                getitem_95 = None
                amax_8 = smith.ops.aten.amax.default(clone_66, [2], True)
                sub_31 = smith.ops.aten.sub.Tensor(clone_66, amax_8)
                clone_66 = amax_8 = None
                exp_8 = smith.ops.aten.exp.default(sub_31)
                sub_31 = None
                sum_9 = smith.ops.aten.sum.dim_IntList(exp_8, [2], True)
                div_8 = smith.ops.aten.div.Tensor(exp_8, sum_9)
                exp_8 = sum_9 = None
                permute_102 = smith.ops.aten.permute.default(div_8, [0, 1, 3, 2])
                div_8 = None
                expand_37 = smith.ops.aten.expand.default(permute_102, [8, 8, 8, 3137])
                permute_102 = None
                view_174 = smith.ops.aten.reshape.default(expand_37, [64, 8, 3137])
                expand_37 = None
                expand_38 = smith.ops.aten.expand.default(getitem_96, [8, 8, 3137, 8])
                clone_67 = smith.ops.aten.clone.default(
                    expand_38, memory_format=smith.contiguous_format
                )
                expand_38 = None
                view_175 = smith.ops.aten.reshape.default(clone_67, [64, 3137, 8])
                clone_67 = None
                bmm_16 = smith.ops.aten.bmm.default(view_174, view_175)
                view_174 = view_175 = None
                view_176 = smith.ops.aten.reshape.default(bmm_16, [8, 8, 8, 8])
                bmm_16 = None
                expand_39 = smith.ops.aten.expand.default(getitem_94, [8, 8, 3137, 8])
                clone_68 = smith.ops.aten.clone.default(
                    expand_39, memory_format=smith.contiguous_format
                )
                expand_39 = None
                view_177 = smith.ops.aten.reshape.default(clone_68, [64, 3137, 8])
                clone_68 = None
                expand_40 = smith.ops.aten.expand.default(view_176, [8, 8, 8, 8])
                view_176 = None
                view_178 = smith.ops.aten.reshape.default(expand_40, [64, 8, 8])
                expand_40 = None
                bmm_17 = smith.ops.aten.bmm.default(view_177, view_178)
                view_177 = view_178 = None
                view_179 = smith.ops.aten.reshape.default(bmm_17, [8, 8, 3137, 8])
                bmm_17 = None
                slice_116 = smith.ops.aten.slice.Tensor(
                    getitem_94, 2, 1, 9223372036854775807
                )
                getitem_94 = None
                slice_120 = smith.ops.aten.slice.Tensor(
                    getitem_96, 2, 1, 9223372036854775807
                )
                getitem_96 = None
                permute_103 = smith.ops.aten.permute.default(slice_120, [0, 1, 3, 2])
                slice_120 = None
                view_180 = smith.ops.aten.reshape.default(permute_103, [8, 64, 56, 56])
                permute_103 = None
                split_with_sizes_8 = smith.ops.aten.split_with_sizes.default(
                    view_180, [16, 24, 24], 1
                )
                view_180 = None
                ...
                cat_22 = smith.ops.aten.cat.default(
                    [
                ...
                view_181 = smith.ops.aten.reshape.default(cat_22, [8, 8, 8, 3136])
                cat_22 = None
                permute_104 = smith.ops.aten.permute.default(view_181, [0, 1, 3, 2])
                view_181 = None

                mul_86 = smith.ops.aten.mul.Tensor(slice_116, permute_104)
                slice_116 = permute_104 = None
                constant_pad_nd_8 = smith.ops.aten.constant_pad_nd.default(
                    mul_86, [0, 0, 1, 0, 0, 0], 0.0
                )
                mul_86 = None
                mul_87 = smith.ops.aten.mul.Tensor(view_179, 0.3535533905932738)
                view_179 = None
                add_87 = smith.ops.aten.add.Tensor(mul_87, constant_pad_nd_8)
                mul_87 = constant_pad_nd_8 = None
                return add_87

        view_12 = smith.randn(batch_size, in_features, image_size, image_size)
    ...
    def test_qlinear_pointwise_int8_layout(self, dtype):
        class M(smith.nn.Module):
            def __init__(self, N, K):
                super().__init__()
                qw = smith.randint(-128, 127, (N, K), dtype=smith.int8)
                self.w_scales = smith.Tensor([0.8] * N)
                self.w_zps = smith.zeros(N).to(dtype=smith.int)
                self.qw_packed = smith.ops.onednn.qlinear_prepack(qw, None)
                self.b = smith.rand((N,), dtype=dtype)
                self.y_scale = 0.5
                self.y_zp = 0
                # set int8 output to check int8 layout path
                self.output_dtype = smith.int8
                self.post_op = "none"
                self.unary_post_op_args = ()
                self.post_op_algo = "none"

            def forward(self, qx, x_scale, x_zp):
                return smith.ops.onednn.qlinear_pointwise(
                    qx,
                    x_scale,
                    x_zp,
                    self.qw_packed,
        ...
        x = smith.rand((32, 64), dtype=dtype)
        x_scale, x_zp = smith.ops.quantized_decomposed.choose_qparams.tensor(
            x, -128, 127, smith.Tensor([smith.finfo(smith.float32).eps]), smith.int8
        )
        qx = smith.ops.quantized_decomposed.quantize_per_tensor.tensor(
            x, x_scale, x_zp, -128, 127, smith.int8
        )
        mod = M(64, 64).eval()
        counters.clear()
    ...
    def test_int8_woq_mm(self, dtype, batch_size, mid_dim, in_features, out_features):
        def _convert_weight_to_int8pack(w):
            scale, zp = _calculate_dynamic_per_channel_qparams(
                w.to(smith.float), smith.int8
            )
            scale = smith.from_numpy(scale)
            zp = smith.from_numpy(zp)
            w_int8 = smith.ao.quantization.fx._decomposed.quantize_per_channel(
                input=w,
                scales=scale,
                zero_points=zp,
                axis=0,
            ...
            return w_int8, scale.to(smith.bfloat16)

        class M(smith.nn.Module):
            def __init__(self, w):
                super().__init__()
                self.linear_weight = smith.nn.Parameter(w, requires_grad=False)

            def forward(self, x, scale):
                return (
                    smith.nn.functional.linear(x, self.linear_weight.to(x.dtype))
                    * scale
                )

        counters.clear()
        # Currently, the corresponding smith.fx pattern only supports 3D x
        # Add 2D X case once the corresponding pattern-matcher pattern is added
        x = smith.rand((batch_size, mid_dim, in_features), dtype=dtype)
        w = smith.rand((out_features, in_features), dtype=dtype)
    ...
    def test_int8_woq_mm_concat(
        ...
        class M(smith.nn.Module):
            def __init__(self, w1, w2, w3):
                super().__init__()
                self.w1 = smith.nn.Parameter(w1, requires_grad=False)
                self.w2 = smith.nn.Parameter(w2, requires_grad=False)
                self.w3 = smith.nn.Parameter(w3, requires_grad=False)

            def forward(self, x, scale1, scale2, scale3):
                # Ref: _linear_fp_act_int8_weight_impl in smithao/dtypes/uintx/plain_layout.py
                y1 = (
                    smith.mm(x.reshape(-1, x.shape[-1]), self.w1.t().to(x.dtype))
                    * scale1
                )
                y2 = (
                    smith.mm(x.reshape(-1, x.shape[-1]), self.w2.t().to(x.dtype))
                    * scale2
                )
                y3 = (
                    smith.mm(x.reshape(-1, x.shape[-1]), self.w3.t().to(x.dtype))
                    * scale3
                )
                return (
        ...
        # Add 2D X case once the corresponding pattern-matcher pattern is added
        x = smith.rand((batch_size, mid_dim, in_features), dtype=dtype)
        w1 = smith.rand((out_features, in_features), dtype=dtype)
        w2 = smith.rand((out_features, in_features), dtype=dtype)
        w3 = smith.rand((out_features, in_features), dtype=dtype)
    ...
    @parametrize("dtype", [smith.float, smith.bfloat16])
    ...
    def test_da8w8_sym_act_sym_wgt_with_int_mm(
        ...
        r"""
        This testcase check if we can match the int8_dynamic_activation_int8_weight int8 linear pattern from smithao,
        ...
        """
        if dtype == smith.bfloat16 and not smith.ops.mkldnn._is_mkldnn_bf16_supported():
            return
        in_feature = 48
        out_feature = 64
        q_min, q_max = -32, 31
        ...
        class Mod(smith.nn.Module):
            def __init__(self, dtype: smith.dtype, has_bias: bool):
                super().__init__()
                self.dtype = dtype
                self.has_bias = has_bias
                self.b = smith.randint(
                    q_min, q_max, [in_feature, out_feature], dtype=smith.int8
                )
                self.per_channel_quant = per_channel_quant
                a_scale_per_tensor = smith.rand([1], dtype=dtype) * 0.01 + 0.01
                a_scale_per_channel = smith.rand([M, 1], dtype=dtype) * 0.01 + 0.01
                self.a_scale = (
                    a_scale_per_channel if per_channel_quant else a_scale_per_tensor
                )
                self.b_scale = smith.rand([out_feature]) * 0.01 + 0.01
                self.b_scale = self.b_scale.to(dtype)
                self.bias = smith.rand([out_feature], dtype=dtype) if has_bias else None
            ...
            def forward(self, a):
                if reshape_a:
                    ...
                    a_reshaped = a
                c = smith._int_mm(a_reshaped, self.b)
                c = c.to(self.dtype)
        ...
        a = smith.randint(q_min, q_max, [M, in_feature], dtype=smith.int8)
        if dynamic:
            smith._dynamo.mark_dynamic(a, 0)
            smith._dynamo.mark_static(a, 1)
        self.common(
            mod,
            (a,),
            atol=1e-2 if dtype is smith.bfloat16 else None,
            rtol=1e-2 if dtype is smith.bfloat16 else None,
        )

        vec_amx = VecAMX()
        self._check_amx_counter(vec_amx)
        if smith.cpu._is_amx_tile_supported():
    ...
    def test_int4_woq_mm_avx512(
        ...
        class M(smith.nn.Module):
            def __init__(self, K, N, group_size):
                super().__init__()
                self.linear_weight = smith.randint(
                    0, 15, (N, K // 2), dtype=smith.uint8
                )
                self.qscale_and_zeros = smith.rand(K // group_size, N, 2, dtype=dtype)
                self.group_size = group_size

            def forward(self, x):
                x_shape = x.shape
                x = x.reshape(-1, x_shape[-1])
                y = smith._weight_int4pack_mm_for_cpu(
        ...
        counters.clear()
        seq_len = 4
        x = smith.rand((batch_size, seq_len, in_features), dtype=dtype)
        mod = M(in_features, out_features, group_size).eval()
        self.common(mod, (x,), reference_in_float=False)
        available_isa = smith._inductor.cpu_vec_isa.pick_vec_isa()
    ...
    def test_int4_woq_mm_amx_Nc_larger_than_one(
        ...
        """
        Note:
        `smith._weight_int4pack_mm_for_cpu` computes with float32, while the AMX-based GEMM
        ...
        class M2(smith.nn.Module):
            ...
            def forward(self, x):
                x_eye = smith.eye(x.shape[-1], device=x.device, dtype=x.dtype)
                dq_w = self.mod(x_eye).T.contiguous()
                return smith.nn.functional.linear(x, dq_w)
        ...
        mod2 = M2(mod)
        # Skip VERIFY during smith.compile and don't use self.common. See explanation above.
    ...
    def test_int4_woq_mm_with_small_buffer_config(
        ...
        with patch.object(select_algorithm, "VERIFY", None):
            ...
            smith._C.FileCheck().check(_target_code_check).run(code)
    ...
    def test_int4_concat_woq_mm(
        ...
        class M1(smith.nn.Module):
            def __init__(self, K, out_features, group_size):
                super().__init__()
                self.linear_weight = [
                    smith.randint(0, 255, (N, K // 2), dtype=smith.uint8)
                    for N in out_features
                ]
                self.qscale_and_zeros = [
                    smith.rand(K // group_size, N, 2, dtype=dtype) for N in out_features
                ]
                self.group_size = group_size
        ...
        class M2(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                for idx in range(len(self.mod.out_features)):
                    x_shape = x_eye.shape
                    dq_w = smith._weight_int4pack_mm_for_cpu(
                        x_eye,
                ...
                return [smith.nn.functional.linear(x, dq_w) for dq_w in dq_w_list]
    ...
    def test_aoti_linear(self, batch_size, in_features, out_features, bias, dtype):
        ...
        class M(smith.nn.Module):
            def __init__(self, bias=bias) -> None:
                ...
                self.mlp = smith.nn.Sequential(
                    smith.nn.Linear(in_features, out_features, bias=bias),
                    smith.nn.ReLU(),
                )
        ...
        assert smith._inductor.config.freezing is False
        ...
        smith.manual_seed(0)
        with verify(dtype) as (atol, rtol), smith.no_grad():
            expected = mod(v)
    ...
    def test_grouped_linear_invalid(
        ...
        class M(smith.nn.Module):
            def __init__(self, in_feature, out_feature, gemm_num):
                super().__init__()
                self.linears = [
                    smith.nn.Linear(in_feature, out_feature + gemm_idx, bias=False)
        ...
        for dtype in dtypes:
            ...
            v = smith.randn(batch_size, in_features).to(dtype)
            with (
                verify(dtype) as (atol, rtol),
                smith.autocast(device_type="cpu", dtype=dtype),
                smith.no_grad(),
            ):
                self.common(mod, (v,), atol=atol, rtol=rtol)
    ...
    def test_grouped_linear(
        ...
        class M(smith.nn.Module):
            def __init__(self, in_feature, out_feature, gemm_num):
                super().__init__()
                self.linears = [
                    smith.nn.Linear(in_feature, out_feature, bias=False)
        ...
        if smith.ops.mkldnn._is_mkldnn_fp16_supported():
            dtypes.append(smith.float16)
        for dtype in dtypes:
            if dtype == smith.float16 and input_3d:
            ...
            mod = M(in_features, out_features, gemm_num).eval()
            B = (2, batch_size) if input_3d else (batch_size,)
            v = smith.randn(*B, in_features).to(dtype)
            with (
                verify(dtype) as (atol, rtol),
                smith.autocast(device_type="cpu", dtype=dtype),
                smith.no_grad(),
    ...
    def test_grouped_linear_epilogue(
        ...
        class M(smith.nn.Module):
            def __init__(self, in_feature, out_feature, bias, epilogue):
                super().__init__()
                self.linear0 = smith.nn.Linear(in_feature, out_feature, bias=bias[0])
                self.linear1 = smith.nn.Linear(in_feature, out_feature, bias=bias[1])
            ...
            def forward(self, x):
                ...
                if self.epilogue0 == "silu" and self.epilogue1 == "mul":
                    return smith.nn.functional.silu(res0) * res1
                else:
                    if self.epilogue0 == "relu":
                        res0 = smith.nn.functional.relu(res0)
                    if self.epilogue1 == "relu":
                        res1 = smith.nn.functional.relu(res1)
                    return res0, res1
        ...
        if smith.ops.mkldnn._is_mkldnn_fp16_supported():
            dtypes.append(smith.float16)
        for dtype in dtypes:
            if input_3d and dtype == smith.float16:
    ...
    def test_aoti_linear_multi_view_operations(
        ...
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.bias = smith.randn(out_features)
                self.weight = smith.randn(out_features // 2, 2, in_features)
    ...
    def test_cpp_coordinate_descent_tuning(self):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(512, 1024, bias=False)
        ...
        smith._inductor.metrics.reset()
        counters.clear()
        with verify(smith.bfloat16) as (atol, rtol), smith.autocast(device_type="cpu"):
            self.common(mod, (v,), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_linear_to_lowp_fp(self, batch_size, in_features, out_features, bias):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                return self.linear(x).to(smith.float16)
    ...
    def test_cpp_weight_prune(self):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(32, 128, bias=False)

            def forward(self, x):
                return self.linear(x)

        v = smith.randn(2, 32).to(smith.bfloat16)
        mod = M().eval().to(smith.bfloat16)
        smith._dynamo.reset()
        smith._inductor.metrics.reset()
        counters.clear()
        with verify(smith.bfloat16) as (atol, rtol):
            self.common(mod, (v,), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_bmm(self, dtype, bs, Mdim, Kdim, Ndim):
        ...
        counters.clear()
        u = smith.randn(bs, Mdim, Kdim).to(dtype=dtype)
        v = smith.randn(bs, Kdim, Ndim).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
    ...
    def test_bmm_amp(self, dtype, bs, Mdim, Kdim, Ndim):
        ...
        v = smith.randn(bs, Kdim, Ndim).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol), smith.amp.autocast("cpu"):
            self.common(mod, (u, v), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_bmm_freezing(self, dtype, bs, Mdim, Kdim, Ndim):
        class M(smith.nn.Module):
            def __init__(self, w):
                super().__init__()
                self.w = smith.nn.Parameter(w, requires_grad=False)
    ...
    @dtypes(smith.float, smith.bfloat16, smith.half)
    @unittest.skip(
        "Skip for now since it's flaky: https://github.com/blacksmith/blacksmith/actions/runs/19916391966/job/57096613514?pr=169151"
    )
    def test_bmm_2d_permute(self, Ndim, order, dtype):
        ...
        x_args = (bs, Mdim, Kdim)
        w_args = (bs, Kdim, Ndim)
        inverse_order = [smith.argsort(smith.tensor(o)).tolist() for o in order]
    ...
    @parametrize("Mdim", (64,))
    @parametrize("Kdim", (96,))
    @dtypes(smith.float, smith.float16, smith.bfloat16)
    ...
    def test_bmm_self_square(self, bs, Mdim, dtype):
        ...
        counters.clear()
        u = smith.randn(bs, Mdim, Mdim).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (u,), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_bmm_with_broadcasted_mat1(self, bs, Mdim, Kdim, Ndim, dtype):
        ...
        counters.clear()
        u = smith.randn(Mdim, Kdim).to(dtype=dtype)
        v = smith.randn(bs, Kdim, Ndim).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (u, v), atol=atol, rtol=rtol)
    ...
    @dtypes(smith.float32, smith.bfloat16, smith.half)
    def test_bmm_with_pointwise(self, bs, Mdim, Kdim, Ndim, epilogue, dtype):
        ...
        counters.clear()
        x = smith.randn(bs, Mdim, Kdim).to(dtype=dtype)
        w = smith.randn(bs, Kdim, Ndim).to(dtype=dtype)
        other = smith.randn(bs, Mdim, Ndim).to(dtype=dtype)
    ...
    def test_bmm_with_fused_epilogues(self, dtype):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.mul = smith.randn(8, 8, 3136, 8).as_strided(
                    (8, 8, 3136, 8), (200704, 8, 64, 1)
                )

            def forward(self, x, w):
                x = smith.ops.aten.reshape.default(x, [64, 3137, 8])
                w = smith.ops.aten.reshape.default(w, [64, 8, 8])
                bmm = smith.ops.aten.bmm.default(x, w)
                bmm = smith.ops.aten.reshape.default(bmm, [8, 8, 3137, 8])
                constant_pad_nd = smith.ops.aten.constant_pad_nd.default(
                    self.mul, [0, 0, 1, 0, 0, 0], 0.0
                )
                mul_2 = smith.ops.aten.mul.Tensor(bmm, 0.3535533905932738)
                add = smith.ops.aten.add.Tensor(mul_2, constant_pad_nd)
                return add

        counters.clear()
        x = smith.randn(8, 8, 3137, 8).to(dtype=dtype)
        w = smith.randn(8, 8, 8, 8).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (x, w), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_aoti_bmm_unique_identifiers(self, dtype):
        ...
        counters.clear()
        x = smith.randn(3, 64, 64).to(dtype=dtype)
        w = smith.randn(3, 64, 64).to(dtype=dtype)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol), smith.no_grad():
            expected = mod(x, w)
            actual = test_aot_inductor_utils.AOTIRunnerUtil.run(
    ...
    def test_local_and_global_accumulator(self, out_features1, dtype):
        ...
        class M(smith.nn.Module):
            def __init__(self):
                ...
                self.linear = smith.nn.Linear(in_features, out_features, bias)
                self.linear1 = smith.nn.Linear(in_features1, out_features1, bias)

            def forward(self, x):
                y = self.linear(x)
                view = smith.ops.aten.view.default(y, [-1, in_features1])
    ...
    def test_bmm_flexible_layout(self):
        class M(smith.nn.Module):
            ...
            def forward(self, u, v):
                view_3 = smith.ops.aten.reshape.default(u, [-1, 512, 64])
                clone_1 = smith.ops.aten.clone.default(
                    v, memory_format=smith.contiguous_format
                )
                view_7 = smith.ops.aten.reshape.default(clone_1, [-1, 512, 64])
                permute_6 = smith.ops.aten.permute.default(view_7, [0, 2, 1])
                div = smith.ops.aten.div.Tensor(permute_6, 8.0)
                # view_3 is a ReinterpretView and div is a FlexibleLayout which will become FixedLayout
                bmm = smith.ops.aten.bmm.default(view_3, div)
                return bmm

        mod = M().eval()
        u = smith.randn(2, 24, 512, 64)
        v = smith.randn(48, 512, 64)
    ...
    def test_linear_reuse_kernels(self, batch_size, in_features, out_features, dtype):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear_x = smith.nn.Linear(in_features, out_features)
                self.linear_y = smith.nn.Linear(out_features, in_features)
                self.linear_z = smith.nn.Linear(in_features, out_features)
...
class TestSelectAlgorithmDynamicShapes(_DynamicShapesTestBase):
    ...
    def test_bmm_with_pointwise_dynamic_shapes(self, bs, Mdim, Kdim, Ndim, dtype):
        ...
        u = smith.randn(bs, Mdim, Kdim).to(dtype=dtype)
        v = smith.randn(bs, Kdim, Ndim).to(dtype=dtype)
        smith._dynamo.mark_dynamic(u, 0)
        smith._dynamo.mark_dynamic(u, 1)
        smith._dynamo.mark_static(u, 2)
        smith._dynamo.mark_static(v, 2)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (u, v), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
    ...
    def test_bmm_with_pointwise_with_reshape_dynamic_shapes(
        ...
        u = smith.randn(bs, 8, Mdim, Kdim).to(dtype=dtype)
        v = smith.randn(bs, 8, Kdim, Ndim).to(dtype=dtype)
        noise = smith.randn(bs * 8, Mdim, Ndim).to(dtype=dtype)
        smith._dynamo.mark_dynamic(u, 0)
        smith._dynamo.mark_dynamic(u, 1)
        smith._dynamo.mark_static(u, 2)
        smith._dynamo.mark_static(u, 3)
        smith._dynamo.mark_static(v, 2)
        smith._dynamo.mark_static(v, 3)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
    ...
    def test_bmm_epilogue_dynamic_reshape(self, dtype):
        ...
        class M(smith.nn.Module):
            ...
            def forward(self, x, w, arg5_1):
                arg131_1 = x.shape[0]
                mul_91 = arg131_1 * 8
                view_422 = smith.ops.aten.reshape.default(x, [mul_91, 512, 64])
                view_423 = smith.ops.aten.reshape.default(w, [mul_91, 64, 512])
                bmm_36 = smith.ops.aten.bmm.default(view_422, view_423)
                view_424 = smith.ops.aten.reshape.default(
                    bmm_36, [arg131_1, 8, 512, 512]
                )
                abs_2 = smith.ones(512, 512, dtype=smith.int64)
                lt_562 = smith.ops.aten.lt.Scalar(abs_2, 8)
                add_5084 = smith.ones(512, 512, dtype=smith.int64)
                add_5085 = smith.ones(512, 512, dtype=smith.int64)
                full_default_1 = smith.ops.aten.full.default(
                    [512, 512], 15, dtype=smith.int64, layout=smith.strided
                )
                minimum_3 = smith.ops.aten.minimum.default(add_5085, full_default_1)
                where_2 = smith.ops.aten.where.self(lt_562, abs_2, minimum_3)
                add_5086 = smith.ops.aten.add.Tensor(add_5084, where_2)
                embedding_5 = smith.ops.aten.embedding.default(arg5_1, add_5086)
                permute_196 = smith.ops.aten.permute.default(embedding_5, [2, 0, 1])
                unsqueeze_21 = smith.ops.aten.unsqueeze.default(permute_196, 0)
                full_default = smith.ops.aten.full.default(
                    [arg131_1, 1, 1, 512],
                    -0.0,
                    dtype=smith.float32,
                    layout=smith.strided,
                )
                add_5087 = smith.ops.aten.add.Tensor(unsqueeze_21, full_default)
                add_5103 = smith.ops.aten.add.Tensor(view_424, add_5087)
                return add_5103

        counters.clear()
        u = smith.randn(bs, 8, 512, 64).to(dtype=dtype)
        v = smith.randn(bs, 8, 64, 512).to(dtype=dtype)
        arg5 = smith.randn(32, 8)
        smith._dynamo.mark_dynamic(u, 0)
        smith._dynamo.mark_static(u, 1)
        smith._dynamo.mark_static(u, 2)
        smith._dynamo.mark_static(u, 3)
        smith._dynamo.mark_static(v, 2)
        smith._dynamo.mark_static(v, 3)
    ...
    def test_bmm_dynamic_bm_stride(self):
        ...
        counters.clear()
        u = smith.randn(bs, Mdim, Kdim).to(dtype=dtype)
        v = smith.randn(Kdim, Mdim, bs).to(dtype=dtype)
        smith._dynamo.mark_dynamic(u, 0)
        smith._dynamo.mark_dynamic(u, 1)
        smith._dynamo.mark_static(u, 2)
        smith._dynamo.mark_static(v, 0)
        smith._dynamo.mark_static(v, 1)
        mod = M().to(dtype=dtype).eval()
        with verify(dtype) as (atol, rtol):
            self.common(mod, (u, v), atol=atol, rtol=rtol)
        self.assertEqual(counters["inductor"]["cpp_templated_kernel_counter"], 1)
