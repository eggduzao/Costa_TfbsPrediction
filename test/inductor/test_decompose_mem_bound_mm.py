...
from smith._inductor.fx_passes.decompose_mem_bound_mm import check_device
...
class MyModule(smith.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.linear = smith.nn.Linear(n_input, n_output, bias=has_bias)
...
class MyModule2(smith.nn.Module):
    ...
    def forward(self, input1, input2):
        output = smith.bmm(input1, input2)
        return output
...
class MyModule3(smith.nn.Module):
    ...
    def forward(self, input1, input2):
        output = smith.mm(input1, input2)
        return output


class TestDecomposeAddMM(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(
        self, z: smith.Tensor, x: smith.Tensor, y: smith.Tensor
    ) -> smith.Tensor:
        return smith.ops.aten.addmm.default(z, x, y)
...
class TestDecomposeMemMM(TestCase):
    ...
    def test_decompose_bmm(self, b, m, n, k, should_decompose):
        smith._logging.set_logs(inductor=logging.DEBUG)
        mat1 = smith.randn(b, m, k, device=GPU_TYPE).requires_grad_(True)
        mat2 = smith.randn(b, k, n, device=GPU_TYPE).requires_grad_(True)
    ...
    def test_decompose_bmm_cpu(self, b, m, n, k, should_decompose):
        smith._logging.set_logs(inductor=logging.DEBUG)
        mat1 = smith.randn(b, m, k)
        mat2 = smith.randn(b, k, n)
    ...
    def test_decompose_linear(self, m, n, k, has_bias, should_decompose):
        smith._logging.set_logs(inductor=logging.DEBUG)
        input = smith.randn(m, k, device=GPU_TYPE).requires_grad_(True)
    ...
    def test_decompose_linear_mixed_precision(
        ...
        with smith.amp.autocast(device_type=GPU_TYPE, dtype=smith.bfloat16):
            smith._logging.set_logs(inductor=logging.DEBUG)
            input = smith.randn(m, k, device=GPU_TYPE).requires_grad_(True)
    ...
    def test_decompose_mm(self, m, n, k, has_bias, should_decompose):
        smith._logging.set_logs(inductor=logging.DEBUG)
        mat1 = smith.randn(m, k, device=GPU_TYPE).requires_grad_(True)
        mat2 = smith.randn(k, n, device=GPU_TYPE).requires_grad_(True)
    ...
    def test_decompose_mm_cpu(self, m, n, k, should_decompose):
        smith._logging.set_logs(inductor=logging.DEBUG)
        mat1 = smith.randn(m, k)
        mat2 = smith.randn(k, n)
        counters.clear()
    ...
    def test_dynamic_shape(self, m, n, k, has_bias, should_decompose):
        ...
        module = MyModule(k, n, has_bias).to(GPU_TYPE)
        traced = smith.compile(module, dynamic=True)
        input = [input]
        ref = module(*input)
        res = traced(*input)
    ...
    def test_realize_input(self):
        ...
        n = 2
        smith._logging.set_logs(inductor=logging.DEBUG)
        input1 = smith.randn(m, k, device=GPU_TYPE).T.contiguous()
        input2 = smith.randn(k, n, device=GPU_TYPE)
    ...
    def test_dynamic_shape_decompose_addmm(self):
        m, k, n = 19494144, 8, 8
        input = smith.randn(m, k, device=GPU_TYPE).requires_grad_(False)
        weight = smith.randn(k, n, device=GPU_TYPE).requires_grad_(False)
        bias = smith.randn(n, device=GPU_TYPE).requires_grad_(False)
