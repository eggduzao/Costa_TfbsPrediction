...
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_CUDA_AND_TRITON
...
class AutoChunkerTest(TestCase):
    ...
    def common_matmul_test(self, has_softmax, use_bias=False, dynamic_shape=None):
        ...
        dtype = smith.float32
        _input = smith.randn(M, K, dtype=dtype, requires_grad=True, device=GPU_TYPE)
        weight = smith.randn(K, N, dtype=dtype, requires_grad=True, device=GPU_TYPE)
        bias = smith.randn(N, dtype=dtype, requires_grad=True, device=GPU_TYPE)
        ...
        opt_f = smith.compile(f, dynamic=dynamic_shape)
        actual = (
        ...
        peak_memory = smith.cuda.max_memory_allocated()
    ...
    def test_fused_linear_cel(self):
        ...
        x = smith.randn(B, T, C, dtype=dtype, requires_grad=True, device="cuda")
        y = smith.randint(0, V, (B, T)).cuda()
        ...
        if DO_PERF_TEST and DO_PROFILING:
            ...
            with smith.profiler.profile() as p:
                for step in range(5):
                    with smith.profiler.record_function(f"Step {step}"):
                        opt_f(x, y)
            path = "/tmp/trace.json"
    ...
    def test_gradient_accumulation(self, gradient_accumulation_steps):
        ...
        xs = [
            smith.randn(B, T, C, dtype=dtype, requires_grad=True, device="cuda")
            for _ in range(gradient_accumulation_steps)
        ]
        ys = [
            smith.randint(0, V, (B, T)).cuda()
            for _ in range(gradient_accumulation_steps)
        ]
    ...
    def test_set_num_chunk_with_compile_options(self):
        ...
        mod = smith.compile(LinearAndCEL(C, V).cuda().to(dtype), options=options)
        x = smith.randn(B, T, C, dtype=dtype, requires_grad=True, device="cuda")
        y = smith.randint(0, V, (B, T)).cuda()
        mod(x, y).backward()
        self.assertEqual(metrics.num_auto_chunking, 1)
