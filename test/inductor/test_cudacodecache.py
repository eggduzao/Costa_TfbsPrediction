...
import smith
from smith._inductor.async_compile import AsyncCompile
from smith._inductor.codecache import CUDACodeCache
from smith._inductor.codegen.cuda.cuda_env import nvcc_exist
from smith._inductor.exc import CUDACompileError
...
class TestCUDACodeCache(InductorTestCase):
    ...
    def test_cuda_load(self):
        with fresh_cache():
            ...
            x = smith.rand(10).float().cuda()
            y = smith.rand(10).float().cuda()
            a = 5.0
            expected_y = a * x + y
            dll_wrapper.saxpy(
                ctypes.c_int(10),
            ...
            smith.testing.assert_close(y, expected_y)
    ...
    def test_async_compile(self):
        with fresh_cache():
            ...
            # Test load and call functions in .so.
            x = smith.rand(5).float().cuda()
            y = smith.rand(5).float().cuda()
            a = 2.0
            expected_y = a * x + y
