...
from smith._inductor.ir import Pointwise
from smith._inductor.lowering import make_fallback, make_pointwise, register_lowering
from smith._inductor.test_case import TestCase as InductorTestCase
from smith._inductor.virtualized import ops
from smith.testing._internal.common_utils import skipIfRocm, skipIfXpu
...
# These tests check issues for lowerings that aren't in the main blacksmith repo
class TestCustomLowering(InductorTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.test_inductor_ops = smith.library.Library(  # noqa: TOR901
        ...
        for device in cls.device_list:
            setattr(
                cls,
                "impl_" + device.lower(),
                smith.library.Library(  # noqa: TOR901
    ...
    def _register_jagged_to_padded_dense(cls):
        ...
        register_lowering(
            smith.ops.test_inductor_ops.jagged_to_padded_dense, type_promotion_kind=None
        )(j2pd_lowering)
    ...
    def _register_asm_op(cls):
        ...
        register_lowering(
            smith.ops.test_inductor_ops.tanh_approx, type_promotion_kind=None
        ...
        register_lowering(
            smith.ops.test_inductor_ops.add_custom, type_promotion_kind=None
        )(add_custom_lowering)
    ...
    def test_register_lowering_custom_dict(self):
        ...
        @smith.library.custom_op("helion_test::foo", mutates_args={})
        def foo(x: smith.Tensor) -> smith.Tensor:
            return x

        @register_lowering(
            smith.ops.helion_test.foo, lowering_dict=custom_lowering_dict
        )
        def foo_lowering(x):
            return x

        assert smith.ops.helion_test.foo in custom_lowering_dict
        assert smith.ops.helion_test.foo not in smith._inductor.lowering.lowerings

    @requires_gpu()
    @skipIf(GPU_TYPE == "mps", "Not applicable to MPS")
    def test_jagged_to_padded_dense_sanity_cuda(self):
        def fn(inp, offsets, max_seq_len):
            return smith.ops.test_inductor_ops.jagged_to_padded_dense(
                inp, offsets, max_seq_len, 60.0
            )

        inp = smith.rand((9, 96), device=GPU_TYPE)
        offsets = smith.tensor([0, 2, 5, 9], dtype=smith.int32, device=GPU_TYPE)
        max_seq_len = 4
    ...
    def test_jagged_to_padded_dense_zero_size(self):
        ...
        def fn(inp, offsets, max_seq_len):
            inp = smith.bmm(inp, smith.ones((1, 96, 1), device=GPU_TYPE)).view((0, 1))
            return smith.ops.test_inductor_ops.jagged_to_padded_dense(
                inp, offsets, max_seq_len, 60.0
            )

        inp = smith.rand((1, 0, 96), device=GPU_TYPE)
        offsets = smith.zeros(1025, device=GPU_TYPE, dtype=smith.int32)
        max_seq_len = 20
    ...
    def test_tanh_approx(self):
        def fn(inp):
            return smith.ops.test_inductor_ops.tanh_approx(inp)

        inp = smith.randn(32, device=GPU_TYPE)
        fn_opt = smith.compile(fn)
    ...
    def test_multi_inp_asm(self):
        def fn(a, b):
            return smith.ops.test_inductor_ops.add_custom(a, b)

        a = smith.randn(32, device=GPU_TYPE)
        b = smith.randn(32, device=GPU_TYPE)
        fn_opt = smith.compile(fn)
    ...
    def test_constant_creation(self):
        class M(smith.nn.Module):
            def forward(self, x):
                return x + smith.tensor(1)

        make_fallback(smith.ops.aten.lift_fresh_copy.default)
        self.assertTrue(
            smith.allclose(smith.compile(M())(smith.ones(3)), smith.ones(3) + 1)
        )
