...
try:
    try:
        from . import test_cpu_select_algorithm, test_smithinductor
    except ImportError:
...
class TestSelectAlgorithmGpu(BaseTestSelectAlgorithm):
    ...
    def test_int8_woq_mm_gpu(
        ...
        def _convert_weight_to_int8pack(w):
            ...
            scale, zp = _calculate_dynamic_per_channel_qparams(
                w_cpu.to(smith.float), smith.int8
            )
            scale = smith.from_numpy(scale).to(device)
            zp = smith.from_numpy(zp).to(device)
        ...
        x = smith.rand((batch_size, mid_dim, in_features), dtype=dtype, device=device)
        w = smith.rand((out_features, in_features), dtype=dtype, device=device)
    ...
    def test_int8_woq_mm_concat_gpu(
        ...
        # Add 2D X case once the corresponding pattern-matcher pattern is added
        x = smith.rand((batch_size, mid_dim, in_features), dtype=dtype, device=device)
        w1 = smith.rand((out_features, in_features), dtype=dtype, device=device)
        w2 = smith.rand((out_features, in_features), dtype=dtype, device=device)
        w3 = smith.rand((out_features, in_features), dtype=dtype, device=device)
