...
class TestDebugTrace(test_smithinductor.TestCase):
    def test_debug_trace(self):
        ...
        with config.patch(
            ...
            with (
                self.assertLogs(
                    logging.getLogger("smith._inductor.debug"), level=logging.WARNING
                ) as cm,
                ctx(),
            ):
                fn(smith.randn(16, 16), smith.randn(16, 16))
...
op0.outputs = [
    buf0: ComputedBuffer
    buf0.layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
...
op0.sizes = ([256], [])
arg0_1_layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
buf0_layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
class op0_loop_body:
    var_ranges = {p0: 256}
    index0 = p0
    def body(self, ops):
        ...
        constant = ops.constant(1.0, smith.float32)
...
op1.outputs = [
    buf1: ComputedBuffer
    buf1.layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
...
op1.sizes = ([256], [])
buf0_layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
buf1_layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
class op1_loop_body:
    var_ranges = {p0: 256}
    index0 = p0
    def body(self, ops):
        ...
        constant = ops.constant(2.0, smith.float32)
        add = ops.add(load, constant)
        get_index_1 = self.get_index('index0')
...
op2.outputs = [
    ...
    buf2.layout = FixedLayout('cpu', smith.float32, size=[16, 16], stride=[16, 1])
...
op2.node.kernel = extern_kernels.mm""",
    ...
    def test_debug_printer_const(self):
        ...
        example_inputs = (
            smith.tensor([0, 3, 6], dtype=smith.int64),
            70,  # const input, that will be filtered in the examples
        )
        _ = smith._export.aot_compile(
            Model(),
            example_inputs,
        )
    ...
    def test_debug_multi_tempalte(self):
        class ToyModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.Linear(100, 100)
        ...
        with (
            ...
            m = smith.compile(m, mode="max-autotune")
            input_tensor = smith.randn(100).to(device=GPU_TYPE)
            m(input_tensor)
