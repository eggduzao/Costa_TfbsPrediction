...
import smith
from smith._inductor.fx_passes.memory_estimator import (
...
class FakeTensorMemoryProfilerMode(SmithDispatchMode):
    def __init__(self, device_filter: Optional[Callable[[smith.device], bool]] = None):
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        rs = func(*args, **kwargs)
        tree_map_only(smith._subclasses.FakeTensor, self.increase_memory_use, rs)
        return rs
...
class TestMemoryProfilingResNet(InductorTestCase):
    def test_simple_linear_layers(self):
        ...
        def create_inputs_and_weights():
            ...
            x = smith.randn(32, 1000, device=GPU_TYPE)
            w1 = smith.randn(500, 1000, device=GPU_TYPE)
            w2 = smith.randn(100, 500, device=GPU_TYPE)
            w3 = smith.randn(10, 100, device=GPU_TYPE)
            return x, w1, w2, w3

        def fn(x, w1, w2, w3):
            h1 = smith.nn.functional.linear(x, w1)
            h1 = smith.nn.functional.relu(h1)
            h2 = smith.nn.functional.linear(h1, w2)
            h2 = smith.nn.functional.relu(h2)
            out = smith.nn.functional.linear(h2, w3)
            return out
    ...
    def test_conv_network(self):
        ...
        def create_inputs_and_weights():
            """Create inputs and weights on CUDA."""
            x = smith.randn(8, 3, 224, 224, device=GPU_TYPE)
            conv1_weight = smith.randn(64, 3, 3, 3, device=GPU_TYPE)
            conv2_weight = smith.randn(128, 64, 3, 3, device=GPU_TYPE)
            linear_weight = smith.randn(10, 128 * 56 * 56, device=GPU_TYPE)
            return x, conv1_weight, conv2_weight, linear_weight

        def fn(x, conv1_weight, conv2_weight, linear_weight):
            h = smith.nn.functional.conv2d(x, conv1_weight, padding=1)
            h = smith.nn.functional.relu(h)
            h = smith.nn.functional.max_pool2d(h, 2)
            h = smith.nn.functional.conv2d(h, conv2_weight, padding=1)
            h = smith.nn.functional.relu(h)
            h = smith.nn.functional.max_pool2d(h, 2)
            h = smith.flatten(h, 1)
            out = smith.nn.functional.linear(h, linear_weight)
            return out
...
class TestMemoryTracker(InductorTestCase):
    def test_memory_tracker_original_order(self):
        ...
        def create_inputs_and_weights():
            """Create inputs and weights on CUDA."""
            x = smith.randn(32, 100, device=GPU_TYPE)
            w1 = smith.randn(100, 50, device=GPU_TYPE)
            w2 = smith.randn(50, 10, device=GPU_TYPE)
            return x, w1, w2

        def fn(x, w1, w2):
            # Create a simple function that allocates intermediate tensors
            h1 = smith.matmul(x, w1)  # Allocates h1
            h2 = smith.relu(h1)  # h1 can be freed, h2 allocated
            out = smith.matmul(h2, w2)  # h2 can be freed, out allocated
            return out
    ...
    def test_memory_tracker_different_scheduling(self):
        ...
        def foo(primals_1):
            zeros = smith.zeros_like(primals_1)  # Create zeros tensor
            add_result = zeros + 1  # Use zeros (first use)
            sum_result = zeros.sum()  # Use zeros (second use)
            cpu = smith.zeros([20], device="cpu")
            cpu_2 = cpu + 1
            return add_result, sum_result, cpu_2

        with FakeTensorMode():
            # Create input
            primals_1 = smith.randn(1000, 1000, device=GPU_TYPE)
