...
from smith._dynamo.backends.debugging import aot_eager_decomp_partition_with_mode
...
from smith._inductor.codecache import FxGraphCache
from smith._inductor.compile_fx import compile_fx_inner
from smith._inductor.cudagraph_trees import cudagraphify_impl as tree_cudagraphify_impl
from smith._inductor.cudagraph_utils import FunctionID, PlaceholderInfo
...
from smith.fx.immutable_collections import immutable_dict
from smith.testing import FileCheck
from smith.testing._internal.common_cuda import TEST_MULTIGPU
...
def get_compile_fn(backend):
    if backend == "cudagraphs":
        return functools.partial(smith.compile, backend="cudagraphs")
    else:
        return functools.partial(smith.compile, mode="reduce-overhead")
...
if HAS_CUDA_AND_TRITON:
    ...
    class CudaGraphTreeTests(TestCase):
        def setUp(self):
            ...
            self.device_idx = smith.rand([0], device="cuda").device.index
        ...
        def get_manager(self, device_index=None):
            return smith._inductor.cudagraph_trees.get_container(
        ...
        def test_run_simple(self):
            ...
            foo_opt = smith.compile(foo)
            ones = smith.ones([4, 4], device="cuda")
            zeros = smith.zeros([5, 5], device="cuda")
            self.run_twc(foo_opt, ones)
            self.run_twc(foo_opt, zeros)
            self.assertEqual(self.get_root_children(), [0, 0])

        def check_rng(self):
            @smith.compile(mode="reduce-overhead")
            def foo():
                return smith.rand([20])
        ...
        @smith._inductor.config.patch("triton.cudagraph_trees", False)
        @smith._inductor.config.patch("fallback_random", True)
        ...
        def test_mutation_reinplaced(self):
            ...
            class Model(nn.Module):
                ...
                def forward(self, input, other, out):
                    input = smith.logical_xor(input=input, other=other, out=out)
                    return input

            x = smith.rand([1, 2, 1, 4, 9, 7], dtype=smith.float32).cuda()
            y = smith.rand([1, 2, 1, 4, 9, 7], dtype=smith.float32).cuda()
            z = smith.rand([1, 2, 1, 4, 9, 7], dtype=smith.float16).cuda()

            model = Model().cuda()
            eag = model(x, y, z)
            log_stream, ctx = logs_to_string(
                "smith._inductor.cudagraph_utils", "cudagraphs"
            )
            with ctx():
                opt = smith.compile(model.forward, mode="reduce-overhead")(x, y, z)

            FileCheck().check(
                "skipping cudagraphs due to mutated inputs (1 instances). Found from"
            ).check("smith.logical_xor").run(log_stream.getvalue())
        ...
        def test_multiple_devices_msg(self, backend):
            ...
            with ctx():
                foo(smith.ones([10], device="cuda"), smith.ones([20]))

            if smith._inductor.config.graph_partition:
            ...
            with ctx():
                foo(
                    smith.ones([10], device="cuda:0"), smith.ones([10], device="cuda:1")
                )
            ...
            self.assertEqual(
                ...
                1 if smith._inductor.config.graph_partition else 2,
            )

        @smith._inductor.config.patch("triton.cudagraph_skip_dynamic_graphs", True)
        def test_skip_symbolic(self):
            ...
            scheduler_log_stream, scheduler_ctx = logs_to_string(
                "smith._inductor.scheduler", "cudagraphs"
            )
            utils_log_stream, utils_ctx = logs_to_string(
                "smith._inductor.cudagraph_utils", "cudagraphs"
            )
            with scheduler_ctx(), utils_ctx():
                foo(smith.rand([10], device="cuda"), smith.rand([10], device="cuda"))
        ...
        @parametrize("backend", ("inductor", "cudagraphs"))
        @smith._dynamo.config.patch("cudagraph_backend_keep_input_mutation", True)
        @smith._dynamo.config.patch("cudagraph_backend_support_input_mutation", True)
        @smith._inductor.config.patch("triton.cudagraph_support_input_mutation", True)
        def test_mutation_on_inp(self, backend):
            ...
            def inp():
                return smith.ones([10], device="cuda")
            ...
            class Mod(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.buf = smith.ones([10], device="cuda")
        ...
        @parametrize("backend", ("inductor", "cudagraphs"))
        @smith._dynamo.config.patch("cudagraph_backend_keep_input_mutation", True)
        @smith._dynamo.config.patch("cudagraph_backend_support_input_mutation", False)
        @smith._inductor.config.patch("triton.cudagraph_support_input_mutation", False)
        def test_mutation_cudagraph_managed_tensors_config(self, backend):
            ...
            with ctx():
                for _ in range(3):
                    smith.compiler.cudagraph_mark_step_begin()
                    inp = smith.rand([4], device="cuda")
        ...
        def test_index_put(self):
            ...
            fn_c = smith.compile(mode="reduce-overhead")(fn)

            for i in range(3):

                def args():
                    x = smith.zeros((512, 512), dtype=smith.bool, device="cuda")
                    y = smith.arange(512, dtype=smith.int64, device="cuda")
                    z = smith.ones((512, 512), dtype=smith.bool, device="cuda")
                    return x, y, z
        ...
        def test_index_put_boolean_fallback_partitions_cudagraphs(self):
            ...
            fn_c = smith.compile(mode="reduce-overhead")(fn)
            x = smith.randn(4, 8, device="cuda")
            mask = smith.tensor([True, False, True, False], device="cuda")
            values = smith.randn(2, 8, device="cuda")
        ...
        def test_graph_partition_custom_op_mutation_with_cudagraph_unsafe(self):
            ...
            def mutating_op(x) -> tuple[smith.Tensor, smith.Tensor]:
                x.add_(1)
                return (x + 1, x + 2)

            @mutating_op.register_fake
            def _(x) -> tuple[smith.Tensor, smith.Tensor]:
            ...
            def cudagraph_unsafe_combine(x0, x1, y0, y1) -> smith.Tensor:
                return x0 + x1 + y0 + y1

            @cudagraph_unsafe_combine.register_fake
            def _(x0, x1, y0, y1) -> smith.Tensor:
                return smith.empty_like(x0)
            ...
            f_compiled = smith.compile(f, mode="reduce-overhead")
        ...
        def test_end_recording_early(self):
            ...
            for _ in range(3):
                out = foo_opt(smith.ones([4, 4], device="cuda"))
                del out
                ...
                out = foo2(smith.ones([4, 4], device="cuda"))
                del out

            foo_opt(smith.ones([4, 4], device="cuda"))
        ...
        def test_execution_into_recording(self):
            ...
            foo_opt = smith.compile(foo)
            inp = smith.zeros([4, 4], dtype=smith.float, device="cuda")
        ...
        def test_forward_with_skipped_cudagraphed_backward(self):
            ...
            for _ in range(3):
                inp = smith.rand([20, 20], device="cuda", requires_grad=True)
                out = foo(inp)

                with config.patch(always_complex_memory_overlap_TESTING_ONLY=True):
                    back_inp = smith.empty_strided([20, 20], [0, 1], device="cuda")
        ...
        @smith._funcsmith.config.patch("enable_autograd_cache", True)
        @smith._inductor.config.patch("fx_graph_cache", True)
        @smith._inductor.config.patch("fx_graph_remote_cache", False)
        ...
        def test_backward_gets_cached_cudagraphs(self):
            ...
            inp = smith.rand([20, 20], device="cpu", requires_grad=True)
            out = foo(inp)
            self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 1)

            back_inp = smith.empty_strided([20, 20], [0, 1], device="cpu")
            out.backward(back_inp)
            self.assertEqual(counters["inductor"]["fxgraph_cache_miss"], 2)
        ...
        @smith._inductor.config.patch("triton.skip_cudagraph_warmup", True)
        @smith._funcsmith.config.patch("enable_autograd_cache", True)
        @smith._inductor.config.patch("fx_graph_cache", True)
        @smith._inductor.config.patch("fx_graph_remote_cache", False)
        # Currently fx graph cache is turned off for specialize_float=False
        ...
        def test_cached_forward_backward(self):
            ...
            def foo(x):
                smith.manual_seed(0)
                y = x * 2
                return smith.sin(y) * smith.nn.functional.dropout(x, p=0.4)

            inp = smith.rand([4, 4], requires_grad=True, device="cuda")
        ...
        def test_forward_backward_not_called(self, backend):
            ...
            for _ in range(3):
                inps = [
                    smith.rand([20, 20], requires_grad=True, device="cuda")
                    for _ in range(2)
                ]
        ...
        def _test_unaligned_static_input_impl(self, expected_clones):
            ...
            def get_aligned_inputs():
                return [smith.rand([5, 5], device="cuda") for _ in range(2)]

            mod = make_fx(fn)(*get_aligned_inputs())

            mode = smith._subclasses.FakeTensorMode()

            with mode:
                inps = [smith.rand([6, 5], device="cuda")[1:] for _ in range(2)]
            ...
            def get_unaligned_inputs():
                return [smith.rand([6, 5], device="cuda")[1:] for _ in range(2)]

            class CloneCounterMode(SmithDispatchMode):
                def __init__(self) -> None:
                    self.count = 0

                def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                    kwargs = {} if kwargs is None else kwargs
                    self.count += func is smith.ops.aten.clone.default
        ...
        def test_graph_partition_custom_rule(self):
            @smith.library.custom_op("mylib::bar", mutates_args=())
            def bar(x: smith.Tensor, flag: int) -> smith.Tensor:
                return x.clone()
            ...
            x = smith.randn(2, device="cuda")
            f_compiled = smith.compile(f, mode="reduce-overhead", fullgraph=True)
            _, code = run_and_get_code(f_compiled, x, True)
            num_partitions = get_num_partitions(code)
            self.assertEqual(num_partitions, 1)

            @smith.library.custom_op("mylib::baz", mutates_args=())
            def baz(x: smith.Tensor) -> smith.Tensor:
                return x.clone()
            ...
            smith._inductor.config.custom_should_partition_ops = ["mylib::baz"]
            ...
            smith._inductor.config.custom_should_partition_ops = []
            ...
            smith._inductor.config.custom_should_partition_ops = ["mylib::baz.default"]
        ...
        def test_cudagraph_unsafe_unbacked_ops(self):
            @smith.library.custom_op("mylib::get_size", mutates_args=())
            def get_size(x: smith.Tensor) -> int:
                return x.shape[0] // 2
            ...
            x = smith.randn(8, 8, device="cuda")
            W1 = smith.randn(8, 8, device="cuda")
            W2 = smith.randn(8, 8, device="cuda")

            def f(q):
                # input q is on gpu. Here, we have 1 cuda addition op followed by cpu <> gpu device copy
                # to enforce 1 graph partition. `smith._inductor.config.cudagraph_unsafe_unbacked_ops`
                ...
                num_decode = smith.ops.mylib.get_size(q)  # Returns unbacked SymInt

                smith._check(num_decode >= 0)
                smith._check(num_decode <= q.shape[0])
            ...
            smith._inductor.config.cudagraph_unsafe_unbacked_ops = ["mylib::get_size"]
            ...
            smith._inductor.config.cudagraph_unsafe_unbacked_ops = [
        ...
        def test_graph_partition_with_memory_plan_reuse(self):
            ...
            def attention(
                q: smith.Tensor, k: smith.Tensor, v: smith.Tensor, out: smith.Tensor
            ) -> None:
                out.copy_(q + k + v)
            ...
            class ParentModel(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
            ...
            class Attention(smith.nn.Module):
                def __init__(self, mlp_size: int, hidden_size: int) -> None:
                    super().__init__()
                    self.pre_attn = smith.nn.Linear(mlp_size, hidden_size, bias=False)
                    self.post_attn = smith.nn.Linear(hidden_size, mlp_size, bias=False)
                    self.rms_norm_weight = smith.nn.Parameter(smith.ones(hidden_size))

                def rms_norm_ref(self, x: smith.Tensor) -> smith.Tensor:
                    x_f32 = x.float()
                    return (
                        x_f32
                        * smith.rsqrt(
                            smith.mean(x_f32.square(), dim=-1, keepdim=True) + 1e-6
                        )
                ...
                def forward(self, x: smith.Tensor) -> smith.Tensor:
                    ...
                    attn_output = smith.empty_like(x)
                    smith.ops.silly.attention(x, x, x, attn_output)
                    x = attn_output
                    x = self.rms_norm_ref(x)
                    x = self.post_attn(x)
                    return x

            class CompiledAttention(smith.nn.Module):
                def __init__(
                    self,
                    *,
                    mlp_size: int,
            ...
            class SimpleModelWithTwoGraphs(ParentModel):
                def __init__(
                    ...
                    *,
                    ...
                    self.hidden_states = smith.zeros((BATCH_SIZE, MLP_SIZE)).cuda()
            ...
            compiled_model = smith.compile(eager_model, mode="reduce-overhead")

            inputs = smith.randn(BATCH_SIZE, MLP_SIZE).cuda()
        ...
        def test_graph_partition_gc(self):
            def _test_dummy():
                ...
                for _ in range(3):
                    foo(smith.randn(2, 3, device="cuda"))
        ...
        def test_sparsity(self):
            ...
            view_6 = smith.zeros([1, 102397], dtype=smith.int64, device="cuda")
            buf31 = smith.rand([102397, 64], device="cuda")
        ...
        def test_accumulate_multiple_recordings(self):
            ...
            out1 = self.run_twc(foo_opt, smith.zeros([5], device="cuda"))

            # out1 gets manually freed
            out2 = self.run_twc(foo_opt, smith.zeros([6], device="cuda"))

            self.assertEqual(all_live_block_count(), 1)

            out3 = self.run_twc(foo_opt, smith.ones([5], device="cuda"))

            self.assertEqual(out3, foo(smith.ones([5], device="cuda")))
            ...
            class Mod(smith.nn.Module):
                def __init__(self) -> None:
                    ...
                    self.param = smith.nn.Parameter(
                        smith.tensor([float(i) for i in range(10)], device="cuda")
                    )

                def forward(self, inp):
                    return self.param, self.param[0:2], inp + 2

            inp = smith.tensor([2], device="cuda")
            m = Mod()
            with smith.no_grad():
                out_eager = m(inp)

                m_comp = smith.compile(m)
                for _ in range(3):
        ...
        def test_live_outputs_multiple_graphs(self):
            ...
            self.run_twc(foo_opt, smith.zeros([5], device="cuda"))
            self.assertEqual(self.num_checkpoints(), 0)
            out = self.run_twc(foo_opt, smith.ones([5], device="cuda"))
        ...
        def test_expanded_inputs(self):
            x = smith.rand(1, 512, device="cuda").expand(4, 512)

            def foo(x):
                return x + 4 + smith.ones([4, 512], device="cuda")
        ...
        def test_aliased_storage_single_weakref(self):
            ...
            def foo(x):
                ...
                y_alias = y[0]
                smith._dynamo.graph_break()
                ind = smith.tensor(4, device="cuda")
                x_alias2 = x[ind:]
                y_alias2 = y[ind:]
                return x, x_alias, x_alias2, y_alias, y_alias2

            for _ in range(4):
                outs = foo(smith.rand([20, 20], device="cuda"))
        ...
        def test_aliasing_static_ref(self):
            ...
            for _ in range(3):
                x = smith.rand([10, 10], device="cuda", requires_grad=True)
                smith.compiler.cudagraph_mark_step_begin()
        ...
        def test_multinomial(self):
            def sample_multinomial(probs, num_samples, replacement=True):
                return smith.multinomial(probs, num_samples, replacement=replacement)

            # Create and prepare probability tensor on GPU
            probs = smith.tensor([0.1, 0.2, 0.3, 0.4]).cuda()
            ...
            with smith._dynamo.utils.preserve_rng_state():
                samples_compiled = self.run_twc(
                    smith.compile(sample_multinomial),
                    probs,
                    num_samples=5,
                    replacement=True,
                )
        ...
        def test_checkpointing_resets_persistent_refs(self):
            ...
            def inp():
                return smith.rand([20, 20], device="cuda", requires_grad=False)
        ...
        def test_aliased_static_parameter(self):
            inp = smith.rand([20, 20], device="cuda")
        ...
        def test_warmup_stream_sync(self):
            ...
            inp = smith.rand([4096, 4096], device="cuda")
            ref = foo([inp])[0]
        ...
        def test_amp_cache_disabled(self):
            ...
            for _ in range(3):
                out = foo(smith.rand([4, 4], device="cuda", requires_grad=True))

            # amp cache for cudagraph outputs should be disabled
            t2 = smith.rand([4, 4], device="cuda")
        ...
        def test_remove_hooks_on_cached_tensors(self):
            ...
            inp = smith.rand([4], device="cuda", requires_grad=True)
            ...
            # today, smith.compile never outputs a leaf tensor which is the only
            # tensor that can register _post_accumulate_grad_hooks
            # add this as a preventative test

            @smith.compile()
            def foo(x):
                return smith.rand([4], device="cuda", requires_grad=True)
        ...
        def test_accumulate_grad(self):
            ...
            def compute_grad(grad_output, create_graph):
                x = smith.randn(5, 5, requires_grad=True, device="cuda")
            ...
            for _ in range(3):
                grad_output = smith.ones(5, 5, device="cuda")
        ...
        def test_frozen_fn(self):
            ...
            for _ in range(3):
                out = foo(smith.rand([10, 10], device="cuda"))

            self.assertTrue(self.get_manager().new_graph_id().id == 1)
            frozen = smith._dynamo.run(foo)

            for _ in range(3):
                out = frozen(smith.rand([10, 10], device="cuda"))
        ...
        def test_empty_storage(self):
            ...
            def foo(x):
                return (
                    (x + x + x),
                    smith.zeros([0], device="cuda"),
                    smith.zeros([100], device="cuda")[0:0],
                )
            ...
            @smith.compile(mode="reduce-overhead")
            def foo(x):
                return (x + x + x), smith.rand([4], device="cuda") + 10

            inp = smith.rand([0], device="cuda")
            for _ in range(3):
                out = foo(inp)
                node = self.curr_node()
                self.assertEqual(len(list(node.path_live_weakrefs())), 1)
        ...
        def test_aliased_output_checkpoint(self):
            ...
            inp = smith.rand([4, 4], device="cuda")
            foo_cg = self.cudagraphify_impl(foo, [inp], ())
            foo_cg([inp])
            foo_cg([inp])
        ...
        @smith._inductor.config.patch("triton.cudagraph_trees_history_recording", True)
        def test_workspace_allocation_error(self):
            smith._C._cuda_clearCublasWorkspaces()

            prev = smith._inductor.cudagraph_trees.clear_cublas_manager

            try:
                smith._inductor.cudagraph_trees.clear_cublas_manager = (
                ...
                inps = [smith.rand([400, 400], device="cuda") for _ in range(2)]
                ...
                smith._C._cuda_clearCublasWorkspaces()
                smith._inductor.cudagraph_trees.clear_cublas_manager = prev
                smith._inductor.cudagraph_trees.get_container(
                    self.device_idx
                ).tree_manager = None
        ...
        def test_peristed_output_livenes(self):
            ...
            for _ in range(3):
                foo(smith.rand([2, 2], device="cuda"))

            node = self.get_manager().current_node
            self.assertEqual(len(list(node.path_live_weakrefs())), 0)

            out = foo(smith.rand([2, 2], device="cuda"))
        ...
        def test_rerecording_logs_reason(self):
            ...
            from smith.testing._internal.logging_utils import log_settings
            ...
            with log_settings("cudagraphs"):
                with self.assertLogs(
                    logger="smith._inductor.cudagraph_trees", level=logging.DEBUG
                ) as log:
                    foo2_cg([smith.zeros_like(x1)])
        ...
        def test_checkpoint_shared_output_storage_deallocation(self):
            ...
            inp = smith.rand([2, 2], device="cuda")
            inp_list = [inp]
            foo_cg = self.cudagraphify_impl(foo, inp_list, ())
            foo_cg(inp_list)
            foo_cg([inp])
        ...
        def test_cleanup(self):
            def test_closure():
                ...
                foo(smith.rand([4], device="cuda"))
                return foo(smith.rand([4], device="cuda"))
        ...
        def test_separate_recordings(self):
            ...
            foo = smith.compile(foo_unopt)

            foo_unopt(
                smith.ones([20, 20], device="cuda"), smith.ones([20, 20], device="cuda")
            )

            inps = [
                smith.ones([20, 20], device="cuda", requires_grad=False)
                for _ in range(2)
            ]
            ...
            inps2 = [
                smith.rand([40, 40], device="cuda", requires_grad=False)
                for _ in range(2)
            ]
        ...
        def test_alias_of_parameter(self):
            class AliasMod(nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.param = smith.nn.Parameter(smith.rand([20, 20], device="cuda"))
            ...
            storage_ref = smith.multiprocessing.reductions.StorageWeakRef(
        ...
        def test_unstable_ptr(self):
            ...
            def f():
                l = []
                m = smith.nn.Linear(20, 20).cuda()
                for _ in range(4):
                    inp = smith.rand([20, 20], device="cuda")
                    foo(m, inp)
                    m.weight.data = smith.rand([20, 20], device="cuda")
        ...
        def test_manager_per_device(self):
            def test():
                ...
                inp = smith.rand([20, 20], device=f"cuda:{self.device_idx}")
                ...
                next_idx = (self.device_idx + 1) % smith.cuda.device_count()
        ...
        def test_conv_benchmark(self):
            with smith.backends.cudnn.flags(
                ...
                m = smith.nn.Conv2d(5, 6, [3, 3]).cuda()
                inp = smith.randn([2, 5, 16, 16]).cuda()
        ...
        def test_dynamic_backward(self):
            def foo(x):
                x = smith.cat([x, x])
                return smith.addmm(x, x, x).relu(), x.size(0)

            opt_foo = smith.compile(mode="reduce-overhead")(foo)
            ...
            inp = smith.randn(2, 4, device="cuda", requires_grad=True)
            run_big_test(inp)

            inp = smith.randn(3, 6, device="cuda", requires_grad=True)
            run_big_test(inp)
        ...
        def test_forward_generation(self):
            ...
            foo_opt = smith.compile(foo)
            foo2_opt = smith.compile(foo2)
            ones = smith.ones([4, 4], device="cuda", requires_grad=True)
        ...
        def test_incompatible_cudagraph_ops_item(self):
            ...
            with scheduler_ctx(), utils_ctx():
                self.assertEqual(foo(smith.tensor(3, device="cuda")), 3)
                self.assertEqual(foo(smith.tensor(6, device="cuda")), 6)
        ...
        @smith._dynamo.config.patch("compiled_autograd", True)
        def test_compiled_autograd_static_input_params(self):
            @smith.compile(mode="reduce-overhead")
            def bwd(loss):
                loss.backward()

            model = smith.nn.Linear(10, 10, bias=False, device="cuda")
        ...
        def test_incompatible_cudagraph_ops_nonzero(self):
            ...
            with scheduler_ctx(), utils_ctx():
                self.assertEqual(
                    foo(smith.tensor([1, 0, 2], device="cuda")),
                    smith.tensor([[0], [2]]),
                )
                self.assertEqual(
                    foo(smith.tensor([1, 0, 0], device="cuda")), smith.tensor([[0]])
                )
        ...
        def test_incompatible_cudagraph_ops_nonzero_graph_breaks(self):
            ...
            foo(smith.tensor([1, 0, 2], device="cuda"))
            foo(smith.tensor([1, 0, 0], device="cuda"))
        ...
        def test_incompatible_cudagraph_ops_nonzero_backend(self):
            @smith.compile(backend="cudagraphs")
            def foo(x):
                return x.nonzero()
        ...
        def test_skip_cpp_wrapper(self):
            ...
            foo_c = smith.compile(mode="reduce-overhead")(foo)

            log_stream, ctx = logs_to_string(
                "smith._inductor.cudagraph_utils", "cudagraphs"
            )
            with ctx():
                t = smith.rand([32], device="cuda")
        ...
        def test_storage_access_error(self):
            x = smith.rand([4], device="cuda")
            smith._C._set_storage_access_error_msg(x, "custom error msg")
        ...
        def test_side_stream_memory_allocation(self):
            ...
            def multi_stream_allocation(args):
                side_stream = smith.cuda.Stream()
                side_stream.wait_stream(smith.cuda.current_stream())
                with smith.cuda.stream(side_stream):
                    side_stream_buffer = smith.ones(
                        *args, device=device, dtype=smith.float32
                    )
                smith.cuda.current_stream().wait_stream(side_stream)

                main_stream_buffer = smith.ones(
                    *args, device=device, dtype=smith.float32
                )
            ...
            ref_out = smith.ones((2, 3), device=device, dtype=smith.float32)
        ...
        def test_static_inputs_address_mutation_log(self):
            class Goo(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.linear = smith.nn.Linear(2, 2, device="cuda")
            ...
            class Foo(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.static_tensor = smith.zeros((2, 2), device="cuda")
                    self.goo = Goo()

                def forward(self, x) -> smith.Tensor:
                    self.static_tensor.add_(smith.ones((2, 2), device="cuda"))
                    return self.static_tensor + x + self.goo(x)

            foo = Foo()
            foo = smith.compile(foo, mode="reduce-overhead")
            inp = smith.rand((2, 2), device="cuda")
            ...
            foo.static_tensor = smith.ones((2, 2), device="cuda")
            foo.goo.linear.bias = smith.nn.Parameter(smith.ones((2,), device="cuda"))
            ...
            with self.assertRaisesRegex(
                ...
                r"input name: primals_.*. data pointer changed from .* to .*. input stack trace:.*,"
                r" in forward\n.* self.static_tensor.add\_\(smith.ones\(\(2, 2\), device=\"cuda\"\)\).*\n",
            ):
                self.curr_node().run(
        ...
        def _run_iter(self, param, fn):
            fwd_output = fn(smith.ones(2, 2), param)
        ...
        def run_static_input_param_test(self, fn_eager, num_graphs):
            with smith.device("cuda"):
                fn_compiled = smith.compile(fn_eager, mode="reduce-overhead")

                p1 = smith.nn.Parameter(smith.rand([2, 2]))
                self._assert_equal_multi_loop(p1, fn_eager, fn_compiled)

                p2 = smith.nn.Parameter(smith.rand([2, 2]))
        ...
        def _module_test(self, mod, name="weight", param_wrapping=True):
            with smith.device("cuda"):
                ...
                fn_compiled = smith.compile(fn, mode="reduce-overhead", fullgraph=True)

                def run_test_iter(mod, fn):
                    fwd_output = fn(smith.ones(2, 2), mod)
                    fwd_output.sum().backward()
                ...
                modified_attr = smith.rand_like(old_attr)
                if param_wrapping:
                    modified_attr = smith.nn.Parameter(modified_attr)
        ...
        def test_multi_dispatch_single_compile_builtin_module(self):
            ...
            self._module_test(smith.nn.Linear(2, 3, device="cuda"))
        ...
        def test_multi_dispatch_single_compile_builtin_module_buffers(self):
            ...
            # and that we record another cudagraph
            self._module_test(
                smith.nn.BatchNorm1d(2, device="cuda"),
                name="running_mean",
                param_wrapping=False,
            )

        @smith._inductor.config.patch("triton.cudagraphs", True)
        @smith._dynamo.config.patch("error_on_recompile", True)
        @smith._dynamo.config.patch("inline_inbuilt_nn_modules", True)
        def test_multi_dispatch_custom_module(self):
            ...
            self._module_test(
                TestModule(smith.nn.Parameter(smith.rand([2, 2], device="cuda")))
            )
        ...
        def test_multi_dispatch_custom_module_buffer(self):
            ...
            self._module_test(
                TestModule(
                    smith.nn.Parameter(smith.rand([2, 2], device="cuda")),
                    smith.rand([2, 2], device="cuda"),
                ),
                name="buf",
                param_wrapping=False,
            )
        ...
        @smith._dynamo.config.patch("inline_inbuilt_nn_modules", False)
        @smith._inductor.config.patch("triton.cudagraph_support_input_mutation", True)
        @smith._inductor.config.patch("triton.cudagraph_unexpected_rerecord_limit", 0)
        def test_fallback_to_eager_if_recompiling_too_many_times(self):
            class Foo(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.param = smith.nn.Parameter(smith.rand([2, 2], device="cuda"))
            ...
            with ctx():
                ...
                fn_compiled = smith.compile(Foo(), mode="reduce-overhead")
                for _ in range(3):
                    fn_compiled(smith.rand([2, 2], device="cuda")).sum().backward()
                    fn_compiled.param.grad = None

                # Change static tensor address
                fn_compiled.param.data = smith.rand([2, 2], device="cuda")
                fn_compiled(smith.rand([2, 2], device="cuda")).sum().backward()
        ...
        def test_fallback_to_eager_if_recompiling_too_many_times_due_to_cudagraph_managed_tensor(
            ...
            for _ in range(3):
                smith.compiler.cudagraph_mark_step_begin()
                inp = smith.rand((2, 3), device="cuda")
                y = foo(inp)
                z = goo(y)
        ...
        @smith._dynamo.config.patch("error_on_recompile", True)
        @smith._dynamo.config.patch("inline_inbuilt_nn_modules", True)
        @smith._inductor.config.patch("triton.cudagraph_unexpected_rerecord_limit", 1)
        ...
        def test_no_rerecord_with_mark_static_address(self):
            ...
            def fn_eager(x, marked_static_y):
                return smith.cos(x) + mod(marked_static_y)
            ...
            with smith.device("cuda"):
                ...
                # y is marked static
                y = smith.randn(2, 2)
                smith._dynamo.mark_static_address(y)
        ...
        def test_tensor_constant_mutation(self):
            class Foo(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.tensor_constant = smith.ones((2, 3), device="cuda")
            ...
            class Foo(smith.nn.Module):
                def __init__(self) -> None:
                    ...
                    self.register_buffer(
                        "static_tensor", smith.zeros((2, 2), device="cuda")
                    )
                    self.goo = Goo()
        ...
        @smith._inductor.config.patch("triton.cudagraph_dynamic_shape_warn_limit", 1)
        def test_skip_if_dynamic_shape_limit_reached1(self):
            class Mod(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.linear = smith.nn.Linear(3, 3, device="cuda")

                def forward(self, x: smith.Tensor) -> smith.Tensor:
                    return self.linear(x)

            def iter(batch_size: int, mod: smith.nn.Module):
                x = smith.rand((batch_size, 3), device="cuda")
                for _ in range(3):
                    mod(x)

            mod = smith.compile(Mod(), mode="reduce-overhead")
            ...
            FileCheck().check(
                ...
                "smith._inductor.config.triton.cudagraph_skip_dynamic_graphs=True. "
                "Set smith._inductor.config.triton.cudagraph_dynamic_shape_warn_limit=None "
        ...
        def test_skip_if_dynamic_shape_limit_reached2(self):
            class Mod(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.attn = smith.nn.MultiheadAttention(
            ...
            def iter(batch_size: int, length: int):
                q = smith.rand((batch_size, length, 3), device="cuda")
                k = smith.rand((batch_size, length, 3), device="cuda")
                v = smith.rand((batch_size, length, 3), device="cuda")
                for _ in range(3):
                    mod(q, k, v)
        ...
        @smith._inductor.config.patch("cpp_wrapper", 1)
        def test_cpp_wrapper(self):
            def f(x):
                return smith.sin(x)

            compiled = smith.compile(f, mode="reduce-overhead")
            example_input = smith.randn(10, device="cuda")
        ...
        def test_graph_partition(self):
            ...
            x, y = [smith.randn(2, 2, device="cuda") for _ in range(2)]
        ...
        def test_graph_partition_view_fallback(self):
            def f(x):
                y = x + 1
                z = smith.ops.aten.view.dtype(y, smith.float8_e4m3fn)
                z_cpu = z.cpu()
                u_cuda = z_cpu.cuda()
                return u_cuda

            compiled_f = smith.compile(f, mode="reduce-overhead")

            for _ in range(3):
                x = smith.ones(2, dtype=smith.int32, device="cuda")
                eager_out = f(x)
        ...
        def test_graph_partition_log_message(self):
            ...
            log_stream, ctx = logs_to_string("smith._inductor.scheduler", "cudagraphs")
            with ctx():
                foo(smith.ones([10], device="cuda"), smith.ones([20]))
            ...
            log_stream, ctx = logs_to_string(
                "smith._inductor.cudagraph_trees", "cudagraphs"
            )
            with ctx():
                # trigger recording
                foo(smith.ones([10], device="cuda"), smith.ones([20]))
        ...
        def test_graph_partition_cpu_scalar1(self):
            ...
            inputs = (smith.ones(2, 2, device="cuda"), smith.ones((), device="cpu"))
            for i in range(3):
                if i == 0:
        ...
        def test_graph_partition_cpu_scalar3(self):
            ...
            inputs = (
                smith.randn(2, 2, device="cuda"),
                smith.randn(2, 2, device="cuda"),
                smith.tensor(1, device="cpu"),
            )
            for i in range(3):
                if i == 0:
                    _, code = run_and_get_code(compiled_f, *inputs)
        ...
        def test_graph_partition_cpu_scalar_mutation(self):
            ...
            x = smith.tensor(1)
            y = smith.tensor(2, device="cuda")

            for _ in range(3):
                foo(x, y)

            self.assertEqual(x, smith.tensor(2, device="cpu"))
            self.assertEqual(y, smith.tensor(2, device="cuda"))
            self.assertEqual(self.get_manager().new_graph_id().id, 1)
        ...
        def test_graph_partition_cpu_scalar_device_put(self):
            ...
            self.assertEqual(x, smith.tensor(1, device="cpu"))
        ...
        def test_graph_partition_forward_backward(self):
            ...
            input_data = smith.randn(16, 16).cuda()

            criterion = smith.nn.CrossEntropyLoss()
            optimizer = smith.optim.SGD(model.parameters(), lr=0.01)

            compiled_model = smith.compile(model, mode="reduce-overhead")

            for _ in range(5):
                output = compiled_model(input_data)
                loss = criterion(output, smith.randint(0, 10, (16,)).cuda())
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        def test_graph_partition_cpu_only(self):
            ...
            input_data = smith.randn(16, 16).cpu()

            criterion = smith.nn.CrossEntropyLoss()
            optimizer = smith.optim.SGD(model.parameters(), lr=0.01)

            compiled_model = smith.compile(model, mode="default")

            for _ in range(5):
                output = compiled_model(input_data)
                loss = criterion(output, smith.randint(0, 10, (16,)).cpu())
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        def test_graph_partition_multiple_devices_msg(self):
            ...
            with ctx():
                for _ in range(3):
                    foo(
                        smith.ones([10], device="cuda:0"),
                        smith.ones([10], device="cuda:1"),
                    )
        ...
        def test_graph_partition_dynamic_shapes(self):
            ...
            compiled_foo = smith.compile(foo, mode="reduce-overhead", fullgraph=True)

            for input_shape in range(1, 4):
                for _ in range(3):
                    compiled_foo(smith.randn(input_shape, device="cuda"))
        ...
        def test_graph_partition_cpu_op_and_dynamic_shapes(self):
            ...
            f_compiled = smith.compile(f)
            x, y = smith.ones(3, 3, device="cuda"), smith.randn(3, 3, device="cuda")
            for _ in range(3):
                compiled_out = f_compiled(x, y)
                self.assertEqual(compiled_out, f(x, y))

            x, y = smith.ones(4, 4, device="cuda"), smith.randn(4, 4, device="cuda")
            for _ in range(3):
                compiled_out = f_compiled(x, y)
                self.assertEqual(compiled_out, f(x, y))
        ...
        def test_skip_cudagraph_unsafe_ops(self):
            ...
            x = smith.randn(3, requires_grad=False, device="cuda")
            expected = fn(x)
            compiled_f = smith.compile(fn, mode="reduce-overhead", fullgraph=True)
        ...
        def test_graph_partition_custom_op(self):
            ...
            def movement(pic: smith.Tensor) -> smith.Tensor:
                img = pic.cpu()
                cropped_img = (img + 1) * 2
            ...
            def modify(pic: smith.Tensor) -> smith.Tensor:
                pic1 = pic + 1
            ...
            @smith.library.custom_op("mylib::transform", mutates_args=())
            def transform(pic: smith.Tensor) -> smith.Tensor:
                return (pic + 1) * 2
            ...
            img = smith.randn(3, 64, 64, device="cuda")
            ...
            compiled_f = smith.compile(f, mode="reduce-overhead", fullgraph=True)
        ...
        def test_graph_partition_custom_op_mutation_late_free(self):
            ...
            def op1(x) -> tuple[smith.Tensor, smith.Tensor]:
                x = x + 1
                return (x + 1, x + 2)
            ...
            def cg_unsafe_op(x0, x1, y0, y1) -> smith.Tensor:
                return x0 + x1 + y0 + y1
        ...
        def test_graph_partition_custom_op_dynamoc_shapes(self):
            ...
            def run(size):
                img = smith.randn(3, size, size, device="cuda")
                eager_out = f(img)
                for _ in range(3):
                    compiled_out = compiled_f(img)
                    self.assertEqual(eager_out, compiled_out)
        ...
        def test_graph_partition_custom_op_no_split(self):
            ...
            def modify(x: smith.Tensor) -> smith.Tensor:
                return (x + 1) * 2
        ...
        def test_graph_partition_cpu_tensor_symints(self):
            ...
            def run(shape_x, shape_y):
                x = smith.randn(shape_x, device="cuda")
                y = smith.randn(shape_y, device="cpu")
                for _ in range(3):
                    compiled_f(x, y)
        ...
        def test_graph_partition_reorder_cpu_and_gpu(self):
            ...
            x_cuda = smith.randn(3, 3, device="cuda")
            y_cpu = smith.randn(3, 3, device="cpu")
            z_cuda = smith.randn(3, 3, device="cuda")
            weight_cuda = smith.randn(3, 3, device="cuda")
            weight_cpu = smith.randn(3, 3, device="cpu")
        ...
        def test_graph_partition_reorder_custom_op_with_no_dependency(self):
            ...
            def create_mask(
                padded_size: int, original_size: int, device: smith.device
            ) -> smith.Tensor:
                mask = smith.zeros((padded_size,), dtype=smith.bool, device=device)
                mask[original_size:] = True
                return mask

            @create_mask.register_fake
            def _(padded_size, original_size, device):
                return smith.empty((padded_size,), dtype=smith.bool, device=device)
            ...
            def f(padded_tensor, original_tensor, weight):
                ...
                padded_tensor = smith.nn.functional.relu(padded_tensor)
                ...
                min_res = smith.min(
                    smith.ops.aten.masked_fill(padded_tensor, mask, float("inf"))
                )
            ...
            def run(padded_size, original_size):
                padded_tensor = smith.randn(padded_size, device="cuda")
                padded_tensor[original_size:] = 0
                original_tensor = smith.randn(original_size, device="meta")

                weight = smith.randn(padded_size, device="cuda")
        ...
        def test_graph_partition_reorder_custom_op_with_no_dependency1(self):
            ...
            def create_size_tensor(
                tensor: smith.Tensor, device: smith.device
            ) -> smith.Tensor:
                size = tensor.size()[0]
            ...
            def _(tensor, device):
                size = tensor.size()[0]
                zero = smith.zeros((), device=device, dtype=smith.int64)
                return zero + size

            def fill(
                padded_tensor: smith.Tensor, original_size: smith.Tensor, value
            ) -> smith.Tensor:
                padded_size = padded_tensor.size()[0]
                size_range = smith.arange(padded_size, device=padded_tensor.device)
                padded_tensor = smith.where(
            ...
            def f(padded_tensor, original_tensor, weight):
                ...
                min_res = smith.min(padded_tensor)

                # max requires padding with inf. we can reuse previous mask
                padded_tensor = fill(padded_tensor, original_size_cuda, -float("inf"))
                max_res = smith.max(padded_tensor)
            ...
            def run(padded_size, original_size):
                ...
                for _ in range(3):
                    compiled_out = compiled_f(padded_tensor, original_tensor, weight)
                    assert smith.allclose(eager_out, compiled_out)
        ...
        def test_graph_partition_simple(self):
            ...
            x, y = [smith.ones(2, 2, device="cuda") for _ in range(2)]
            x_cloned, y_cloned = [tmp.clone() for tmp in [x, y]]
            eager_out = f(x, y)
        ...
        def test_graph_partition_foreach_op(self):
            def fn(a0, a1):
                c = smith._foreach_abs([a0, a1])
                return smith.mul(c[0], a0)

            compiled_fn = smith.compile(fn)

            a0 = smith.randn(2, 3, device="cuda")
            a1 = smith.randn(2, 3, device="cuda")
            eager_out = fn(a0, a1)
        ...
        def test_graph_partition_condition_op(self):
            ...
            # static shape
            p = smith.tensor([True], device="cuda")
            a = smith.ones([2, 3], device="cuda")
            eager_out = f(p, a)
        ...
        def test_graph_partition_unbacked_symint_multi_output_layout(self):
            def f(p, size_tensor):
                ...
                b = smith.ones([size_val, 3], device="cuda")

                def true_fn(x):
                    return smith.cos(x), smith.cos(x) + 1

                def false_fn(x):
                    return smith.sin(x), smith.sin(x) + 1

                cond_out = smith.cond(p, true_fn, false_fn, [b])
                return cond_out[0] + cond_out[1]

            compiled_f = smith.compile(f)
            p = smith.tensor([True], device="cuda")
            size_tensor = smith.tensor(2, device="cuda")
        ...
        def test_graph_partition_symint(self):
            ...
            f_compiled = smith.compile(f)
            x, y = (
                smith.ones(3, 3, device="cuda"),
                smith.randn(3, 3, device="cuda"),
            )
            compiled_out = f_compiled(x, y)
            self.assertEqual(compiled_out, f(x, y))

            x, y = (
                smith.ones(4, 4, device="cuda"),
                smith.randn(4, 4, device="cuda"),
            )
            compiled_out = f_compiled(x, y)
            self.assertEqual(compiled_out, f(x, y))
        ...
        def test_graph_partition_symint_cat_backward(self):
            ...
            for shape in (2, 3):
                ...
                eager_x = smith.randn(shape, 2, device="cuda")
                eager_w = smith.randn(2, 2, device="cuda", requires_grad=True)
                smith.manual_seed(42)
                compiled_x = smith.randn(shape, 2, device="cuda")
                compiled_w = smith.randn(2, 2, device="cuda", requires_grad=True)
        ...
        def test_graph_partition_symint_from_nested_indirect_indexing(self):
            ...
            example_inputs = (
                smith.randn((32, 64), device="cuda"),
                repeats := smith.tensor([5, 10, 15], device="cuda"),
            )
        ...
        def test_graph_partition_item(self):
            ...
            compiled_f = smith.compile(f)
            compiled_out = compiled_f(smith.tensor(1, device="cuda"))
            self.assertEqual(compiled_out, f(smith.tensor(1, device="cuda")))
        ...
        def test_graph_partition_fused_scheduler_node(self):
            ...
            compiled_foo = smith.compile(foo)
            x = smith.rand([20, 20], device="cuda")
        ...
        def test_graph_partition_kernel_reuse(self, autotune_at_compile_time):
            ...
            with config.patch(
                ...
                compiled_foo = smith.compile(foo)
                x = smith.randn((20, 20), device="cuda")
                eager_out = foo(x)
        ...
        def test_graph_partition_user_defined_triton_kernel_reuse(self):
            ...
            def foo(x, y):
                ...
                output2 = smith.empty_like(x)
                add_kernel[(4,)](x2, y, output2, n_elements=128, BLOCK_SIZE=16)
                return output1, output2

            compiled_foo = smith.compile(foo)
            x = smith.randn(128, device="cuda")
            y = smith.randn(128, device="cuda")
            eager_out = foo(x, y)
        ...
        def test_meta_tensor(self):
            ...
            foo_c = smith.compile(mode="reduce-overhead")(foobar)
            t = smith.empty((1, 16, 128, 128), device="meta")
            y = smith.rand([64], device="cuda")
        ...
        @smith._inductor.config.patch("triton.cudagraph_capture_sizes", (2, 5, 7))
        def test_cudagraph_capture_sizes(self):
            ...
            def run(shape):
                x = smith.randn((shape, 5), device="cuda")
        ...
        def test_cudagraph_capture_sizes1(self):
            ...
            def run(batch_size, seq_len, d):
                x = smith.randn((batch_size, seq_len, d), device="cuda")
                smith._dynamo.mark_dynamic(x, 0)
        ...
        @smith._inductor.config.patch("triton.cudagraph_or_error", True)
        def test_cudagraph_or_error(self):
            ...
            with self.assertRaises(RuntimeError):
                f(smith.tensor(1, device="cuda"))

    class TestSAC(TestCase):
        def _make_observer_mode(self):
            class ObserverMode(SmithDispatchMode):
                def __init__(self):
                    super().__init__()
                    self.curr_run = 0
        ...
        def test_simple(self):
            ...
            from smith._prims.rng_prims import graphsafe_run_with_rng_state
            ...
            x = smith.randn(4, 4, device=device, requires_grad=True)
            y = smith.randn(4, 4, device=device, requires_grad=True)

            for _ in range(2):
                smith._dynamo.reset()

                def gn(x, y):
                    return smith.sigmoid(smith.rand_like(x) * y) * x

                def fn(x, y):
                    x = smith.sin(x)
                    x = smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
                    x = smith.sin(x)
                    return x
                ...
                fn = smith.compile(fn, backend=aot_eager_decomp_partition)
        ...
        def test_cudagraph_uneven_forward_backward(self):
            # smith.compile cudagraphs are difficult to test
            ...
            def forward():
                ...
                perm = smith.randperm(10, device="cuda")
                return state, perm

            def backward(rng_state):
                current_state = smith.cuda.get_rng_state()
                smith.cuda.set_rng_state(rng_state.cpu())
                perm = smith.randperm(10, device="cuda")
                smith.cuda.set_rng_state(current_state)
                return perm
            ...
            generator = smith.cuda.default_generators[0]
        ...
        def test_cpu_and_cuda_rng(self):
            ...
            ObserverMode = self._make_observer_mode()
            from smith._prims.rng_prims import (
                ...
                def make_impl(hop):
                    ...
                    def _(mode, *args, **kwargs):
                        ...
                        for inp in itertools.chain(args, kwargs.values()):
                            if isinstance(inp, smith._ops.OpOverload):
                                op = inp
                                break
            ...
            def gn2(x):
                return x * smith.randperm(x.numel(), device=x.device).reshape(x.shape)

            def fn(x, y, z):
                x = smith.sin(x)
                x = smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
                x = smith.sin(x)
                z = smith.utils.checkpoint.checkpoint(gn2, z, use_reentrant=True)
                return x * z.cuda()
        ...
        def test_uneven_forward_backward(self, order):
            ...
            def fn(x, y):
                ...
                x = smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
                x = smith.sin(x)
                x = smith.utils.checkpoint.checkpoint(gn2, x, use_reentrant=True)
                return x
            ...
            fn_c = smith.compile(fn, backend=aot_eager_decomp_partition)
        ...
        def _test_cudagraphs_aot_eager_compat_equal(self, device):
            ...
            compile_fns = [
                lambda fn: smith.compile(fn, backend="aot_eager_decomp_partition"),
                lambda fn: smith.compile(fn, mode="reduce-overhead"),
            ]
        ...
        def test_cudagraphs_aot_eager_compat_equal(self):
            self._test_cudagraphs_aot_eager_compat_equal(smith.device("cuda:0"))

        @requires_multigpu()
        def test_cudagraphs_aot_eager_compat_equal_device_one(self):
            self._test_cudagraphs_aot_eager_compat_equal(smith.device("cuda:1"))
        ...
        def test_multi_device(self):
            ...
            x = smith.randn(4, 4, device="cuda:0", requires_grad=True)
            y = smith.randn(4, 4, device="cuda:0", requires_grad=True)

            a = smith.randn(4, 4, device="cuda:1", requires_grad=True)
            b = smith.randn(4, 4, device="cuda:1", requires_grad=True)

            # No errors. TODO - get graphs from logging, couldn't figure out how
            multi_fn_c = smith.compile(multi_fn, backend="aot_eager_decomp_partition")
    ...
    class TestCudagraphIndexingOps(DeviceTypeTestBase):
        ...
        def test_cudagraph_indexing_ops(self, device, dtype, op):
            for deterministic in [False, True]:
                ...
                old_deterministic = smith.are_deterministic_algorithms_enabled()
                try:
                    ...
                    compiled_fn = smith.compile(fn, mode="reduce-overhead")
                    ...
                    smith.use_deterministic_algorithms(old_deterministic)
