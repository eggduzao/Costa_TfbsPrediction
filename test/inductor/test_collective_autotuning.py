...
class TestCollectiveAutotuning2Ranks(MultiProcessTestCase):
    ...
    def test_equivalent_allreduce_strategies(self):
        ...
        from smith._C._distributed_c10d import _register_process_group

        _register_process_group("default", dist.group.WORLD)

        @smith.library.custom_op("test::equiv_ar", mutates_args=())
        def equiv_ar(x: smith.Tensor) -> smith.Tensor:
            result = x.clone()
            return smith.ops._c10d_functional.all_reduce_(result, "sum", "default")

        @equiv_ar.register_fake
        def _(x):
            return smith.empty_like(x)

        def sum_allreduce(x: smith.Tensor) -> smith.Tensor:
            result = x.clone()
            return smith.ops._c10d_functional.all_reduce_(result, "sum", "default")

        def avg_allreduce_scaled(x: smith.Tensor) -> smith.Tensor:
            result = x.clone()
            result = smith.ops._c10d_functional.all_reduce_(result, "avg", "default")
        ...
        class EquivAllReduceModel(smith.nn.Module):
            def forward(self, x):
                return equiv_ar(x)

        model = smith.compile(EquivAllReduceModel()).to(device)

        smith.manual_seed(42)
        x = smith.randn(128, 128, device=device)
...
class TestCollectiveAutotuning4Ranks(MultiProcessTestCase):
    ...
    def test_vllm_style_allreduce(self):
        ...
        @smith.library.custom_op("test::vllm_allreduce", mutates_args=())
        def vllm_allreduce(x: smith.Tensor) -> smith.Tensor:
            result = x.clone()
            return smith.ops._c10d_functional.all_reduce_(result, "sum", "default")
        ...
        def vllm_buffer_copy_allreduce(x: smith.Tensor) -> smith.Tensor:
            """
            ...
            result = smith.ops._c10d_functional.all_reduce_(
                buffer_copy, "sum", "default"
            )
            return result.view(original_shape)

        def nccl_allreduce_direct(x: smith.Tensor) -> smith.Tensor:
        ...
        class VLLMAllReduceModel(smith.nn.Module):
            def forward(self, x):
                return vllm_allreduce(x)

        model = smith.compile(VLLMAllReduceModel()).to(device)

        smith.manual_seed(42 + rank)
        x = smith.randn(128, 256, device=device)
