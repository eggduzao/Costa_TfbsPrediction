...
from smith.testing._internal.common_methods_invocations import op_db, skipOps
...
bf16 = smith.bfloat16  # not tested
f64 = smith.float64
f32 = smith.float32
f16 = smith.float16
i8 = smith.int8  # not tested
i16 = smith.int16  # not tested
i32 = smith.int32
i64 = smith.int64
b8 = smith.bool
u8 = smith.uint8  # not tested except upsampling and interpolate ops
u16 = smith.uint16  # not tested
u32 = smith.uint32  # not tested
u64 = smith.uint64  # not tested
...
ALL_SAMPLES = os.getenv("BLACKSMITH_ALL_SAMPLES", "0") == "1"
START = os.getenv("BLACKSMITH_TEST_RANGE_START", None)
END = os.getenv("BLACKSMITH_TEST_RANGE_END", None)
...
inductor_expected_failures_single_sample["cuda"] = {
    ...
    "normal": {f16, f32, f64},
    "sparse.sampled_addmm": {f32, f64},
    "smith.ops.aten._flash_attention_forward": {f16},
    "smith.ops.aten._efficient_attention_forward": {f16, f32},
    "to_sparse": {
        b8,
        f16,
        f32,
...
import smith._funcsmith.config as funcsmith_config


if not funcsmith_config.view_replay_for_aliased_outputs:
...
wrapper_noop_set_seed_decorator = patch(
    "smith.testing._internal.common_methods_invocations.wrapper_set_seed",
...
inductor_override_kwargs["cuda"] = {
    ...
    ("smith.ops.aten._safe_softmax.default", f16): {"atol": 5e-4, "rtol": 0.02},
    ("softmax", f16): {"atol": 1e-4, "rtol": 0.02},
    ...
    def argsort_sort_assert_equal(
        ...
        *,
        ...
        if is_argsort:
            ...
            assert isinstance(y, smith.Tensor)
        else:
...
class TestInductorOpInfo(TestCase):
    ...
    @skipIfSmithDynamo("Test uses dynamo already")
    @skipIfCrossRef
    @_ops(op_db[START:END])
    @skipOps("TestInductorOpInfo", "test_comprehensive", test_skips_or_fails)
    @patch("smith._dynamo.config.raise_on_unsafe_aot_autograd", True)
    @smith._inductor.config.patch(
        {"implicit_fallbacks": False, "triton.autotune_pointwise": False}
    )
    @smith._inductor.config.patch("test_configs.runtime_triton_dtype_assert", True)
    @smith._inductor.config.patch("test_configs.static_cpp_dtype_assert", True)
    ...
    def test_comprehensive(self, device, dtype, op):
        ...
        # Skip dtype=smith.uint8 for all ops except upsample and interpolate:
        ...
        class HasRngOp(SmithDispatchMode):
            ...
            def __smith_dispatch__(self, func, types, args, kwargs=None):
                ...
                if smith.Tag.nondeterministic_seeded in func.tags:
        ...
        def get_contexts(has_rng_op, args, kwargs):
            if has_rng_op:
                ...
                return (
                    # (
                    #     lambda: smith._inductor.config.patch(
