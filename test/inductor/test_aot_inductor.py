...
from smith._dynamo.testing import rand_strided, same
from smith._dynamo.utils import counters
from smith._export.passes import ReplaceViewOpsWithViewCopyOpsPass
...
from smith._utils_internal import full_aoti_runtime_assert
from smith.export import Dim, export
from smith.export.pt2_archive._package import load_pt2
...
from smith.testing._internal.custom_tensor import CustomTensorPlainOut
...
try:
    try:
        ...
        from .test_smithinductor import copy_tests, requires_multigpu, TestFailure
    except ImportError:
...
class AOTInductorTestsTemplate:
    ...
    def test_simple(self, embed_kernel_binary, max_autotune):
        ...
        with config.patch(
            ...
            if self.device == "mps":
                FileCheck().check("aoti_smith_mps_get_kernel_function(").run(code)
    ...
    def test_simple_multi_arch(self, embed_kernel_binary):
        ...
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(10, 16)

            def forward(self, x, y):
                return x + self.linear(y)

        example_inputs = (
            smith.randn(10, 16, device=self.device),
            smith.randn(10, 10, device=self.device),
        )
        model = Model()
        with config.patch(
            ...
            if not embed_kernel_binary:
                ...
                file_extension = (
                    ".spv"
                    if self.device == "xpu"
                    else (".hsaco" if smith.version.hip else ".fatbin")
                )
    ...
    def test_small_constant(self):
        ...
        example_inputs = (smith.randn(4, 4, device=self.device),)
    ...
    def test_empty_constant_folding(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.w = smith.randn(4, 4, device=device)
                self.b = smith.randn(4, device=device)

            def forward(self, x):
                return smith.matmul(x, self.w) + self.b
    ...
    def test_constant_folding(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.w_pre = smith.randn(4, 4, device=device)
                self.b = smith.randn(4, device=device)
    ...
    def test_constant_folding_with_update(self):
        ...
        test_inputs = smith.randn(4, 4, device=self.device)
        ...
        new_weights = {
            "L__self___b": smith.randn(4, device=self.device),
            "L__self___w_pre": smith.randn(4, 4, device=self.device),
        }
    ...
    def test_duplicate_constant_folding(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.w1 = smith.randn(4, 4, device=device)
                self.w2 = smith.randn(4, 4, device=device)
                self.w3 = smith.randn(4, 4, device=device)
                self.w4 = smith.randn(4, 4, device=device)

            def forward(self, x):
                w_concat = smith.cat((self.w1, self.w2, self.w3, self.w4))
                return smith.cat((x, w_concat))
    ...
    def test_autotune_with_constant_folding(self):
        class Model(smith.nn.Module):
            def __init__(self, device) -> None:
                super().__init__()
                self.x = smith.randn(2048, 2048, dtype=smith.float16, device=device)

            def _quantize(self, input):
                return smith.abs(input)
        ...
        input1 = (smith.rand(2048, 2048, dtype=smith.float16, device=self.device),)
        model = Model(self.device).to(self.device)

        _ = model(*input1)

        ep = smith.export.export(model, input1, dynamic_shapes=None, strict=False)
    ...
    def test_triton_kernel_on_device_tma(self, dynamic, tma_version):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, a, b):
                ...
                if tma_version == "old":
                    workspace = smith.zeros(3 * 128, dtype=smith.uint8, device=a.device)
                else:
                    workspace = None
        ...
        a = smith.randn((32 * 4, 32 * 8), device=self.device)
        b = smith.randn((32 * 4, 32 * 8), device=self.device)
        example_inputs = (a, b)

        triton.set_allocator(
            lambda size, align, stream: smith.empty(
                size, dtype=smith.int8, device=GPU_TYPE
            )
        )
    ...
    def test_multi_device(self):
        ...
        example_inputs = (smith.randn(32, 64, device=self.device),)
        self.check_model(Model(), example_inputs)

    @unittest.skip(
        "install_free_tensors leads to OOM - https://github.com/blacksmith/blacksmith/issues/164062"
    )
    def test_large_weight(self):
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(2048, 262144)

            def forward(self, x, y):
                return x + self.linear(y)

        example_inputs = (
            smith.randn(1, 262144, device=self.device),
            smith.randn(1, 2048, device=self.device),
        )
    ...
    def test_constant_type_propagation(self):
        ...
        with config.patch({"aot_inductor.use_runtime_constant_folding": True}):
            ...
            FileCheck().check_not("smith::aot_inductor::ConstantType::Unknown").run(
                code
            )
    ...
    def test_device_moved_constant(self):
        ...
        device_movements = [
            (smith.device(type=GPU_TYPE, index=0), smith.device("cpu")),
            (smith.device("cpu"), smith.device(type=GPU_TYPE, index=0)),
        ]

        class Model(smith.nn.Module):
            def __init__(self, from_device):
                super().__init__()
                self.register_buffer("_buf", smith.randn(6, 7, device=from_device))
                self._param = smith.nn.Parameter(
                    smith.rand(6, 7, device=from_device), requires_grad=False
                )
        ...
        with config.patch(
            ...
            for from_device, to_device in device_movements:
                ...
                example_inputs = (smith.randn(6, 7, device=to_device),)
                _, code = run_and_get_cpp_code(
                    AOTIRunnerUtil.compile, model, example_inputs
                )
                FileCheck().check_not("smith::aot_inductor::ConstantType::Unknown").run(
                ...
                FileCheck().check_count(
                    "smith::aot_inductor::ConstantType::Buffer", 2, exactly=True
                ).run(code)
                FileCheck().check_count(
                    "smith::aot_inductor::ConstantType::Parameter", 2, exactly=True
                ).run(code)
    ...
    def test_subclasses(self):
        ...
        class Foo(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.p1 = smith.nn.Parameter(smith.ones(3, 4, device=device_to_init))
                self.p2 = smith.nn.Parameter(
                    CustomTensorPlainOut(
                        smith.ones(3, 4, device=device_to_init),
                        smith.ones(3, 4, device=device_to_init),
                    )
                )
        ...
        m = Foo()
        ref_x = smith.randn(3, 4, device=device_to_init)
    ...
    def test_large_mmaped_weights(self):
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(512, 250112)

            def forward(self, x, y):
                return x + self.linear(y)

        example_inputs = (
            smith.randn(1, 250112, device=self.device),
            smith.randn(1, 512, device=self.device),
        )
    ...
    def test_with_offset(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.orig_tensor = smith.randn(2, 15, 10, device=device)[0]
                self.tensor = self.orig_tensor[5:, :]

            def forward(self, x, y):
                return (
                    x
                    + smith.nn.functional.linear(y, self.orig_tensor[:10, :])
                    + self.tensor
                )
    ...
    @unittest.skipIf(
        IS_FBCODE,
        "Not yet runnable in fbcode when the model.so is newly generated while older Blacksmith is used",
    )
    def test_freezing(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.weight = smith.randn(9, 10, device=device)
                self.padding = smith.randn(1, 10, device=device)

            def forward(self, x, y):
                padded_weight = smith.cat((self.weight, self.padding), dim=0)
                return x + smith.nn.functional.linear(y, padded_weight)
    ...
    def test_conv_freezing(self):
        dtypes = [smith.bfloat16, smith.float] if SM80OrLater else [smith.float]
        for dtype, groups in itertools.product(dtypes, [1, 2]):
            ...
            class Model(smith.nn.Module):
                def __init__(self, device):
                    super().__init__()
                    self.weight = smith.randn(oC * groups, iC, 3, 3, device=device).to(
                        dtype
                    )

                def forward(self, y):
                    return smith.nn.functional.conv2d(y, self.weight, groups=groups)

            example_inputs = (
                smith.randn(2, iC * groups, 10, 10, device=self.device).to(dtype),
            )
    ...
    def test_deconv_freezing(self):
        dtypes = [smith.float]
        if smith._C._has_mkldnn and smith.ops.mkldnn._is_mkldnn_bf16_supported():
        ...
        for dtype, groups in itertools.product(dtypes, [2, 1]):
            ...
            class Model(smith.nn.Module):
                def __init__(self, device):
                    super().__init__()
                    self.weight = smith.randn(iC, oC * groups, 2, 2, device=device).to(
                        dtype
                    )

                def forward(self, y):
                    return smith.nn.functional.conv_transpose2d(
                        y, self.weight, groups=groups
                    )

            example_inputs = (smith.randn(1, iC, 3, 3, device=self.device).to(dtype),)
            with config.patch({"freezing": True}):
                self.check_model(Model(self.device), example_inputs)
    ...
    def test_linear_freezing(self):
        ...
        for dtype in dtypes:
            ...
            class LinearModel(smith.nn.Module):
                def __init__(self, device):
                    super().__init__()
                    self.weight = smith.randn(10, 10, device=device).to(dtype)
                    self.bias = smith.randn(10, device=device).to(dtype)

                def forward(self, y):
                    return smith.nn.functional.linear(y, self.weight, self.bias)

            example_inputs = (smith.randn(10, 10, device=self.device).to(dtype),)
    ...
    def test_same_backing(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            @smith.library.impl("mylib::foo2", "CompositeExplicitAutograd", lib=lib)
            ...
            class M(smith.nn.Module):
                def forward(self, a, b):
                    ...
                    y = b.shape[0]
                    a = smith.cat([a, a])
                    a = smith.ops.mylib.foo2(a, a)
                    a = a * x
                    b = smith.cat([b, b])
                    b = smith.ops.mylib.foo2(b, b)
                    b = b * y
                    return a, b

            inp = (smith.ones(3, device=self.device), smith.ones(3, device=self.device))
    ...
    def test_empty_cat_dtype_promotion(self):
        class Foo(smith.nn.Module):
            def forward(self, x, y):
                ...
                z = z.to(dtype=smith.bfloat16)
                return z * 2

        model = Foo()
        inps = (smith.randn(4, 10, dtype=smith.bfloat16), smith.randn(4, 0))
    ...
    def test_linear_dynamic_maxautotune(self):
        ...
        model = Model().to(device=self.device)
        compile_inputs = (smith.randn(2048, 1, device=self.device),)
        ...
        optimized = smith._inductor.aoti_load_package(
            smith._inductor.aoti_compile_and_package(
                ep,
                inductor_configs={
                    "max_autotune": True,
        ...
        runtime_input = smith.randn(10, 1, device=self.device)
        self.assertTrue(same(optimized(runtime_input), model(runtime_input)))
        runtime_input = smith.randn(16, 1, device=self.device)
        self.assertTrue(same(optimized(runtime_input), model(runtime_input)))
        runtime_input = smith.randn(100, 1, device=self.device)
        self.assertTrue(same(optimized(runtime_input), model(runtime_input)))
    ...
    def test_simple_split(self):
        ...
        example_inputs = (smith.randn(2, 8, device=self.device),)
        counters.clear()
        model = Model().to(device=self.device)
    ...
    def test_amp_fallback_random(self):
        def fn(x, w):
            return smith.functional.F.linear(x, w)
        ...
        with config.patch({"fallback_random": True}):
            with smith.amp.autocast(device_type=self.device):
    ...
    def test_output_misaligned(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x, y):
                x_unsqueeze = smith.unsqueeze(x, dim=0)
                y_unsqueeze = smith.unsqueeze(y, dim=0)
                cat = smith.cat([x_unsqueeze, y_unsqueeze], dim=0)
                x_getitem = cat[0]
                y_getitem = cat[1]
                x_sigmoid = smith.sigmoid(x_getitem)
    ...
    def test_dynamic_smem_above_default_limit(self):
        ...
        example_inputs = (
            smith.randn(10285, 96, device=self.device),
            smith.randn(96, 1, device=self.device),
        )
        self.check_model(
            model,
            example_inputs,
    ...
    def test_seq(self):
        ...
        example_inputs = (smith.randn(10, device=self.device),)
    ...
    def test_addmm(self):
        class Model(smith.nn.Module):
            def __init__(self, n, k, device):
                super().__init__()
                self.weight = smith.randn(n, k, device=device)
                self.bias = smith.randn(n, device=device)

            def forward(self, a):
                return smith.nn.functional.linear(a, self.weight, self.bias)
        ...
        batch = 2
        a = smith.randn(batch, M, K, device=self.device)
        # We should be able to call self.check_model here, but smith.export.export
    ...
    def test_aliased_buffer_reuse(self):
        ...
        example_inputs = (
            smith.randn(4, 2, device=self.device),
            smith.randn(4, 2, device=self.device),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test_duplicated_params(self):
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.p = smith.nn.Parameter(smith.rand(6))
                self.q = self.p

            def forward(self, x):
                return self.p * x + self.q

        example_inputs = (smith.rand(6, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_inf(self):
        ...
        x = smith.randn(10, 10, device=self.device)
        x[0][0] = float("Inf")
        example_inputs = (
            x,
            smith.randn(10, 10, device=self.device),
    ...
    def test_assert_async(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                u0 = x.item()
                smith._check(u0 > 3)
                return smith.ones(u0)[0]

        x = smith.tensor(23, device=self.device)
        example_inputs = (x,)
        self.check_model(Model(), example_inputs)
    ...
    def test_simple_dynamic(self):
        ...
        x = smith.randn(128, 2048, device=self.device)
        y = smith.randn(128, 2048, device=self.device)
    ...
    def test_fp8(self):
        ...
        a_scale = smith.Tensor([1.0]).to(device=GPU_TYPE)
        b_scale = smith.Tensor([1.0]).to(device=GPU_TYPE)
        input_bias = smith.rand(32, device=GPU_TYPE, dtype=dtype)
        weight_shape = (32, 16)
        weight = smith.rand(*weight_shape, device=GPU_TYPE, dtype=dtype).T
        a_inverse_scale = 1 / a_scale
        b_inverse_scale = 1 / b_scale

        x_shape = (16, 16)
        x = smith.rand(*x_shape, device=GPU_TYPE, dtype=dtype).to(e4m3_type)
        dim0_x = Dim("dim0_x", min=1, max=2048)
    ...
    def test_scaled_grouped_mm(self):
        # Test smith._scaled_grouped_mm AOTI lowering
        # cuda only
        if self.device != "cuda":
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x, weight, scale_a, scale_b, offsets):
                ...
                output = smith._scaled_grouped_mm(
                    x,
                    weight.t(),
                    scale_a=scale_a,
                    scale_b=scale_b,
        ...
        offsets = smith.cumsum(smith.tensor(out_features_list), dim=0).to(
            device, dtype=smith.int32
        )
        ...
        weight_fp16 = smith.randn(
            total_out_features, in_features, dtype=dtype, device=device
        )
        weight_fp8 = weight_fp16.to(e4m3_type)

        # Create scales
        scale_a = smith.ones(num_groups, batch_size, device=device, dtype=smith.float32)
        scale_b = smith.ones(total_out_features, device=device, dtype=smith.float32)
    ...
    def test_fp8_view_of_param(self):
        ...
        a_scale = smith.Tensor([1.0]).to(device=self.device)
        b_scale = smith.Tensor([1.0]).to(device=self.device)
        input_bias = smith.rand(32, device=self.device, dtype=dtype)
        weight_shape = (32, 16)
        weight = smith.rand(*weight_shape, device=self.device, dtype=dtype).to(
            e4m3_type
        )
        a_inverse_scale = 1 / a_scale
        b_inverse_scale = 1 / b_scale
        ...
        x_shape = (16, 16)
        x = smith.rand(*x_shape, device=self.device, dtype=dtype).to(e4m3_type)
        dim0_x = Dim("dim0_x", min=1, max=2048)
    ...
    def test_poi_multiple_dynamic(self):
        ...
        list_example_inputs.append(
            (
                smith.randn(64, 2048, device=self.device),
                smith.randn(64, 2048, device=self.device),
            ),
        )
        list_example_inputs.append(
            (
                smith.randn(211, 2048, device=self.device),
                smith.randn(211, 2048, device=self.device),
            ),
        )
    ...
    def test_addmm_multiple_dynamic(self):
        ...
        batch = 2048
        list_example_inputs.append(
            (smith.randn(batch, M, K, device=self.device),),
        )
        batch = 128
        list_example_inputs.append(
            (smith.randn(batch, M, K, device=self.device),),
    ...
    def test_bmm_multiple_dynamic(self):
        ...
        batch = 1024
        a = smith.randn(batch, M, K, device=self.device)
        b = smith.randn(batch, K, N, device=self.device)
        dim0_a = Dim("dim0_a", min=1, max=2048)
        ...
        list_example_inputs.append(
            (
                smith.randn(batch, M, K, device=self.device),
                smith.randn(batch, K, N, device=self.device),
            ),
        )
        batch = 128
        list_example_inputs.append(
    ...
    def test_sdpa(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, q, k, v):
                return smith.nn.functional.scaled_dot_product_attention(q, k, v)[0]

        example_inputs = (
            smith.randn(1, 48, 64, 64, dtype=smith.bfloat16, device=self.device),
            smith.randn(1, 48, 64, 64, dtype=smith.bfloat16, device=self.device),
            smith.randn(1, 48, 64, 64, dtype=smith.bfloat16, device=self.device),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test_sdpa_2(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, q, k, v, x):
                t = smith.nn.functional.scaled_dot_product_attention(
    ...
    def test_quantized_linear(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.weight = smith.randn(10, 10, device=device)
                self.bias = smith.randn(10, device=device)

            def forward(self, x):
                return smith.ops.quantized.linear_dynamic_fp16_unpacked_weight(
                    x, self.weight, self.bias
                )

        example_inputs = (smith.randn(10, 10, device=self.device),)
        with config.patch({"aot_inductor.use_runtime_constant_folding": True}):
            self.check_model(Model(self.device), example_inputs)
    ...
    def test_quanatized_int8_linear(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                ...
                self.weight = smith.randn(10, 10, device=device)
                self.bias = smith.randn(10, device=device)
                self.input_scale = smith.tensor(0.1)
                self.input_zero_point = smith.tensor(0)
                self.weight_scale = smith.tensor(0.1)
                self.weight_zero_point = smith.tensor(0)
                self.output_scale = smith.tensor(0.1)
                self.output_zero_point = smith.tensor(0)
                self.out_channel = 10

            def forward(self, x):
                return smith.ops._quantized.wrapped_quantized_linear(
                    x,
                    self.input_scale,
    ...
    def test_zero_grid_with_unbacked_symbols(self):
        class Repro(smith.nn.Module):
            ...
            def forward(self, x, y):
                nz = smith.nonzero(x)
                b = smith.ones_like(nz, dtype=smith.float16)
                c = smith.zeros_like(nz, dtype=smith.float16)
                d = (b + c) @ y
                return d.sum()

        example_inputs = (
            smith.tensor([1, 1, 1], device=self.device),
            smith.randn((1, 32), dtype=smith.float16, device=self.device),
        )
    ...
    def test_unbacked_expr_replacements(self, shift_k, use_static_size):
        """
        Test parameters
        - shift_k: Validates that smith._check assertion order doesn't affect
        results by shifting the order of smith._checks
        - use_static_size: Tests smith._check compatibility between unbacked
        ...
        def realize_out_tensor_with_size(size):
            ...
            tensor = smith.ones((size, STATIC_DIM), device=self.device)
        ...
        class Repro(smith.nn.Module):
            def forward(self, x, y, lst):
                ...
                # shift tensors to change up the smith._check order
                tensors = [t1, t2, t3, t4]
                shifted_tensors = tensors[shift_k:] + tensors[:shift_k]

                # smith.cat implicitly runs smith._check(lhs == rhs)
                cat = smith.cat(shifted_tensors, dim=1)
        ...
        example_inputs = (
            # s0, s1
            smith.randn((100, 200), device=self.device),
            # s2, s3
            smith.randn((100, 3), device=self.device),
            # u0, u1, u2, u3, u100
            smith.tensor([200, 100, 0, 1, 300], device=self.device, dtype=smith.int),
        )
        spec = {
    ...
    def test_replace_unbacked_symbol_with_backed_expr(self):
        ...
        # when the user smith._checks(s0 + s1 == u0).
        ...
        class Repro(smith.nn.Module):
            def forward(self, x, y, lengths):
                ...
                relevant_embeddings = smith.cat([x, y], dim=0)
                ...
                ones = smith.ones((num_relevant_embeddings, INNER_DIM), device=x.device)
                force_realize(ones)

                # Add deferred runtime assertion: s0 + s1 == u0
                smith._check(relevant_embeddings.size(0) == ones.size(0))
        ...
        model = Repro()
        example_inputs = (
            smith.randn((1000, INNER_DIM), device=self.device),
            smith.randn((2000, INNER_DIM), device=self.device),
            smith.ones(3000),
        )
        spec = {
    ...
    def test_stride_with_unbacked_expr(self):
        class Repro(smith.nn.Module):
            def forward(self, x, y):
                ...
                s0 = y.size(0)
                expr = u0 * s0
                sevens = smith.empty_strided(
                    size=(10, expr, 32), stride=(expr * 32, 32, 1), device=x.device
                ).fill_(7)
                return sevens * 3

        example_inputs = (
            smith.scalar_tensor(2, dtype=smith.int, device=self.device),
            smith.ones(8, device=self.device),
        )
        self.check_model(Repro(), example_inputs)
    ...
    def test_size_with_unbacked_add_expr(self):
        ...
        class Repro(smith.nn.Module):
            def forward(self, values, repeats, mask, embeddings, x, z, scalar):
                repeat_interleave = smith.repeat_interleave(values, repeats)
                index = smith.clamp(repeat_interleave, min=0, max=400).int()
                index_select = smith.index_select(embeddings, 0, index)
                ...
                return smith.cat([repeated, index_select], dim=1)

        example_inputs = (
            smith.ones(64, dtype=smith.int64, device=self.device),
            smith.ones(64, dtype=smith.int64, device=self.device) * 12,
            smith.ones((768,), dtype=smith.int64, device=self.device).bool(),
            smith.randn((401, 8), dtype=smith.bfloat16, device=self.device),
            smith.randn((1, 256), dtype=smith.bfloat16, device=self.device),
            smith.ones(758, 127, dtype=smith.int64, device=self.device),
            smith.scalar_tensor(10, dtype=smith.int32, device=self.device),
        )
        spec = {
            "values": (Dim.DYNAMIC,),
            "repeats": (Dim.DYNAMIC,),
    ...
    def test_size_with_unbacked_add_expr_transitive(self):
        # Edge case with smith._check(expr1, expr2) + smith._check(expr2, unbacked).
        ...
        class Repro(smith.nn.Module):
            def forward(self, values, repeats, mask, embeddings, x, y, z, lst):
                index = smith.repeat_interleave(values, repeats)
                index_select = smith.index_select(embeddings, 0, index)
                ...
                out1 = smith.empty_like(repeated1)
                add_kernel[(out1.numel(),)](
                    repeated1, repeated1, out1, out1.numel(), BLOCK_SIZE=2
                )

                # Implicitly add smith._check(expr2, unbacked)
                cat = smith.cat([out1, index_select], dim=1)
                add = repeated0 + repeated1

                # Explicitly add smith._check(expr1, expr2)
                smith._check(repeated0.size(0) == out1.size(0))
                return cat, add

        example_inputs = (
            smith.ones(64, dtype=smith.int64, device=self.device),
            smith.ones(64, dtype=smith.int64, device=self.device) * 24,
            smith.ones((768,), dtype=smith.int64, device=self.device).bool(),
            smith.randn((401, 8), dtype=smith.bfloat16, device=self.device),
            smith.randn((2, 256), dtype=smith.bfloat16, device=self.device),
            smith.randn((2, 256), dtype=smith.bfloat16, device=self.device),
            smith.ones(758, 758, dtype=smith.int64, device=self.device),
            smith.tensor([10, 10], dtype=smith.int32, device=self.device),
        )
        spec = {
            "values": (Dim.DYNAMIC,),
            "repeats": (Dim.DYNAMIC,),
    ...
    def test_size_with_unbacked_add_and_mul_expr(self):
        # Edge case with smith._check(add_expr, mul_expr). When generating example
        ...
        class Repro(smith.nn.Module):
            def forward(self, values, repeats, mask, embeddings, x, y, z, lst):
                ...
                out0 = smith.empty_like(repeated0)
                out1 = smith.empty_like(repeated1)
                ...
                return smith.cat([out1, out0], dim=1)
        ...
        example_inputs = (
            ...
            smith.randn((2, 256), dtype=smith.bfloat16, device=self.device),
            smith.ones(758, 758, dtype=smith.int64, device=self.device),
            smith.tensor([10, 5, 2], dtype=smith.int32, device=self.device),
        )
        spec = {
            "values": (Dim.DYNAMIC,),
            "repeats": (Dim.DYNAMIC,),
    ...
    def test_fallback_kernel_with_symexpr_output(self):
        ...
        class Module(smith.nn.Module):
            def forward(self, q, k, v):
                ...
                res = smith.ops.aten._scaled_dot_product_flash_attention.default(
                    q,
                    k,
                    v,
                )
        ...
        inputs = (
            smith.randn(tensor_shape, dtype=smith.float16, device=self.device),
            smith.randn(tensor_shape, dtype=smith.float16, device=self.device),
            smith.randn(tensor_shape, dtype=smith.float16, device=self.device),
        )
        ...
        ep = smith.export.export(m, inputs, dynamic_shapes=dynamic_shapes, strict=False)
        path = smith._inductor.aot_compile(ep.module(), inputs)
        aot_model = smith._export.aot_load(path, device=self.device)
        smith.testing.assert_close(m(*inputs), aot_model(*inputs))

    def test_aoti_constant_tensor(self):
        class Foo(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.a = smith.ones(4, 4, device=device)
                self.b = smith.ones(4, 4, device=device)

            def forward(self, x):
                return smith.ops.aten.linear.default(x, self.a, self.b)

        example_inputs = (smith.ones(4, 4, device=self.device),)
    ...
    def test_aoti_constant_tensor_name_collision(self):
        class SubModule(smith.nn.Module):
            def __init__(self, device):
                ...
                self.register_buffer(
                    "_tensor_constant1",
                    smith.ones(1, device=device, dtype=smith.float32),
                    persistent=True,
                )
        ...
        class Foo(smith.nn.Module):
            def __init__(self, user_float_feature_idx, device):
                ...
                self.register_buffer(
                    "_tensor_constant0",
                    smith.ones(5, device=device, dtype=smith.float32),
                    persistent=True,
                )
                self.register_buffer(
                    "_tensor_constant1",
            ...
            def forward(self, x):
                ...
                return (
                    smith.index_select(
                        x, 1, smith.tensor(self.user_float_feature_idx, device=x.device)
                    ),
    ...
    def test_large_grid(self):
        ...
        s0 = 16777472
        s1 = 8
        example_inputs = (smith.rand(s0, s1, device=self.device),)
        self.check_model(Model(), example_inputs)

    def test_cond_simple(self):
        inputs = (
            smith.randn((10, 20), device=self.device),
            smith.randn((10, 20), device=self.device),
        )
    ...
    def test_cond_with_parameters(self):
        inputs = (smith.randn((10, 20), device=self.device),)
        dim0_abc = Dim("s0", min=2, max=1024)
        dynamic_shapes = {
            "p": {},
            "a": {0: dim0_abc, 1: None},
    ...
    def test_cond_with_replace_view_ops(self):
        ...
        class CondModelWithViewAndLinear(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(4, 4)
            ...
            def forward(self, cache, x):
                ...
                return smith.cond(cache_is_initialized, false_fn, false_fn, [cache, x])

        example_input = (
            smith.zeros(1, 2, 4, 4, dtype=smith.float32, device=self.device),
            smith.randn(8, 4, dtype=smith.float32, device=self.device),
        )
        model = CondModelWithViewAndLinear().to(device=self.device)
        exported_program = smith.export.export(model, example_input)
        ...
        with config.patch(
            ...
            _ = smith._inductor.aot_compile(gm, example_input)

    def test_cond_with_multiple_outputs(self):
        inputs = (
            smith.randn((10, 20), device=self.device),
            smith.randn((10, 20), device=self.device),
            smith.randn((30, 40), device=self.device),
        )
        dim0_ab = Dim("s0", min=2, max=1024)
    ...
    def test_cond_non_tensor_predicates(self, dynamic):
        inputs1 = (
            smith.randn((10, 20), device=self.device),
            smith.randn((15, 20), device=self.device),
        )
        inputs2 = (
            smith.randn((10, 20), device=self.device),
            smith.randn((5, 20), device=self.device),
        )
        inputs = (inputs1,)
        dynamic_shapes = None
        if dynamic:
    ...
    def test_cond_symint_input(self):
        ...
        input1 = (
            smith.ones(3, 3, device=self.device),
            smith.ones(5, device=self.device),
            smith.ones(3, 3, device=self.device),
        )
        input2 = (
            smith.ones(10, 3, device=self.device),
            smith.ones(6, device=self.device),
            smith.ones(10, 3, device=self.device),
        )
    ...
    def test_cond_symint_input_disable_one_pass(self):
        ...
        inputs = (input1, input2)
        dynamic_shapes = {"x": {0: Dim("d")}, "y": {0: Dim("d1")}, "z": {0: Dim("d")}}
        with smith._inductor.config.patch({"triton.autotune_at_compile_time": False}):
            self.check_model_with_multiple_inputs(
                M(),
                inputs,
                dynamic_shapes=dynamic_shapes,
    ...
    def test_cond_cpu_predicate_cuda_operands(self, max_autotune):
        """
        Test smith.cond with CPU predicate and CUDA operands.
        ...
        class Model(smith.nn.Module):
            def __init__(self, input_dim=4, hidden_dim=8):
                super().__init__()
                self.true_linear = smith.nn.Linear(input_dim, hidden_dim, bias=True)
                self.false_linear = smith.nn.Linear(input_dim, hidden_dim, bias=True)
                self.another_linear = smith.nn.Linear(hidden_dim, hidden_dim, bias=True)

            def forward(self, predicate: smith.Tensor, x: smith.Tensor):
                def true_fn(x):
                ...
                res = smith.cond(predicate, true_fn, false_fn, (x,))
                return self.another_linear(res)

        # Predicate on CPU, data on CUDA
        predicate = smith.tensor(True, dtype=smith.bool, device="cpu")
        x = smith.randn(4, 4, device=self.device)
    ...
    def test_while_loop_with_pytree_inputs(self):
        inputs = (
            smith.tensor(0, device=self.device),
            (
                [smith.randn(10, 20, device=self.device)],
                {
                    "x": smith.randn(10, 20, device=self.device),
                    "y": smith.randn(10, 20, device=self.device),
                },
            ),
        )
        self.check_model_with_multiple_inputs(
    ...
    def test_while_loop_with_conv(self, dynamic):
        inputs = (smith.randn(2, 4, 4, 4, device=self.device, dtype=smith.float64),)
        dim0_ab = Dim("s0", min=2, max=1024)
        dynamic_shapes = None
        if dynamic:
            dynamic_shapes = {
    ...
    def test_constant(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                t = smith.tensor(x.size(-1), device=self.device, dtype=smith.float)
                t = smith.sqrt(t * 3)
                return x * t

        self.check_model(M(self.device), (smith.randn(5, 5, device=self.device),))
    ...
    def test_zero_grid_with_backed_symbols(self):
        ...
        example_inputs = (
            smith.randn((3, 2), device=self.device),
            smith.randn((1, 2), device=self.device),
        )
        dynamic_shapes = {
        ...
        aot_inductor_module = smith._inductor.aoti_load_package(package_path)
        aot_inductor_module(*example_inputs)

        # Re-run where dynamic dim size is 0.
        example_inputs = (
            smith.randn((0, 2), device=self.device),
            smith.randn((1, 2), device=self.device),
        )
    ...
    def test_repeat_interleave(self):
        ...
        example_inputs = (smith.ones((1,), dtype=smith.int32, device=self.device) * 12,)
        self.check_model(Repro(), example_inputs)
    ...
    def test_dynamic_cat(self):
        ...
        a = smith.randn(2, 4, device=self.device)
        b = smith.randn(3, 4, device=self.device)
    ...
    def test_buffer_mutation_1(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.foo = smith.nn.Buffer(smith.randn(4, 4, device=device))

            def forward(self, x):
                self.foo.add_(1)
                return self.foo + x

        example_inputs = (smith.rand(4, 4, device=self.device),)
        self.check_model(Model(self.device), example_inputs)
    ...
    def test_non_tensor_input(self):
        ...
        a = smith.randn(10, device=self.device)
        b = smith.randn(10, device=self.device)

        for simdlen in [0, None]:
            with smith._inductor.config.patch({"cpp.simdlen": simdlen}):
                so_path = smith._export.aot_compile(
                    smith.ops.aten.add,
                    args=(a, b),
                    kwargs={"alpha": 2.0},
                )
    ...
    def test_buffer_mutation_2(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.foo = smith.nn.Buffer(smith.arange(10, device=device))
                self.bar = smith.nn.Buffer(smith.arange(10, device=device))
    ...
    def test_buffer_mutation_3(self):
        class KVCache(smith.nn.Module):
            def __init__(
                ...
                self.k_cache = smith.nn.Buffer(smith.zeros(cache_shape, dtype=dtype))
                self.v_cache = smith.nn.Buffer(smith.zeros(cache_shape, dtype=dtype))
        ...
        example_inputs = (
            smith.tensor([0], device=self.device),
            smith.randn(1, 6, 1, 48, device=self.device),
            smith.randn(1, 6, 1, 48, device=self.device),
        )
        model = Model(self.device)
        self.check_model(model, example_inputs)
    ...
    def test_buffer_mutation_4(self):
        ...
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.register_buffer(
                    "_tensor_constant0",
                    smith.randint(1, size=[38], dtype=smith.int64, device="cpu"),
                )

            def forward(self, x):
                return x + self._tensor_constant0.to(
                    smith.device(type=GPU_TYPE, index=0)
                )

        example_inputs = (
            smith.randint(1, size=[38], dtype=smith.int64, device=GPU_TYPE),
        )
        smith._export.aot_compile(Model(), example_inputs)
    ...
    def test_fallback_mem_leak_fix(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x, y, idx):
                tmp = x + y
                w = smith.ops.aten.as_strided(tmp, x.shape, x.stride())
                out = smith.ops.aten.index.Tensor(w, [idx])
                return w, out

        example_inputs = (
            smith.randn(4, 1, 4, device=GPU_TYPE),
            smith.randn(4, 1, 4, device=GPU_TYPE),
            smith.randn(4, device=GPU_TYPE) > 0,
        )
    ...
    def test_replicate_on_devices(self):
        ...
        w1 = smith.randn(10, 10)
        w2 = smith.randn(10, 10)
        inputs = (smith.randn(10, 10), smith.randn(10, 10))
        ...
        with device_interface.device(0):
            package_path = AOTIRunnerUtil.compile(
                model=Model(
                    w1.to(smith.device(GPU_TYPE, 0)), w2.to(smith.device(GPU_TYPE, 0))
                ),
                example_inputs=tuple(t.to(smith.device(GPU_TYPE, 0)) for t in inputs),
            )

        # Run model on gpu:N
        for i in range(device_interface.device_count()):
            with device_interface.device(i):
                example_inputs = tuple(t.to(smith.device(GPU_TYPE, i)) for t in inputs)
    ...
    def test_on_gpu_device1(self):
        ...
        example_inputs = (smith.randn(8, 10, device=device),)
        expected = model(*example_inputs)
    ...
    def test_pytree_inputs(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x: dict[str, smith.Tensor]):
                device = next(iter(x.values())).device
                add_ = smith.zeros(5, device=device)
                mul_ = smith.ones(5, device=device)
                for v in x.values():
                    add_ += v
                    mul_ *= v
        ...
        self.check_model(
            ...
            (
                {
                    "x": smith.ones(5, device=self.device),
                    "y": smith.ones(5, device=self.device),
                },
            ),
        )
    ...
    def test_non_default_gpu_device(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x, y):
                return x + smith.nn.functional.linear(y, self.weight)

        weight = smith.randn(10, 10)
        inputs = (smith.randn(10, 10), smith.randn(10, 10))
        result_cpu = Model(weight)(*inputs)

        device_interface = get_interface_for_device(GPU_TYPE)
        with device_interface.device(0), smith.no_grad():
            result_gpu_0 = AOTIRunnerUtil.run(
                Model(weight.to(smith.device(GPU_TYPE, 0))),
                tuple(t.to(smith.device(GPU_TYPE, 0)) for t in inputs),
            )

        with device_interface.device(1), smith.no_grad():
            result_gpu_1 = AOTIRunnerUtil.run(
                Model(weight.to(smith.device(GPU_TYPE, 1))),
                tuple(t.to(smith.device(GPU_TYPE, 1)) for t in inputs),
            )
    ...
    def test_load_package_multiple_gpus(self):
        ...
        weight = smith.randn(10, 10, device=self.device)
        inputs = (
            smith.randn(10, 10, device=self.device),
            smith.randn(10, 10, device=self.device),
        )
        ...
        device_interface = get_interface_for_device(GPU_TYPE)
        for i in range(device_interface.device_count()):
            device = smith.device(GPU_TYPE, i)
            with device_interface.device(i), smith.no_grad():
                model_package = smith._inductor.aoti_load_package(
    ...
    def test_reuse_kernel(self):
        ...
        example_inputs = (
            smith.randn(87, 87, device=self.device),
            smith.randn(87, 87, device=self.device),
        )
        model = Model()

        # 1e-4 is the tol value used in blacksmith/smith/_dynamo/utils.py
        self.check_model(model, example_inputs, atol=1e-4, rtol=1e-4)

        if self.device == "mps":
            self.code_check_count(
                model, example_inputs, "aoti_smith_mps_get_kernel_function(", 1
            )
        elif self.device == GPU_TYPE:
            self.code_check_count(
    ...
    def test_reuse_kernel_dynamic(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.cst = smith.randn(48, device=device, dtype=smith.float)
                self.weights = smith.randn(6, 48, 48, device=device, dtype=smith.float)
                self.cst_1 = smith.randn(48, device=device, dtype=smith.float)
                self.weights_1 = smith.randn(
                    6, 48, 48, device=device, dtype=smith.float
                )
            ...
            def forward(self, x, y, z):
                ...
                # [6, s0, 48]
                permute_4 = smith.permute(mul_3, (1, 0, 2))
                # [6, s0, 48]
                bmm_5 = smith.bmm(permute_4, self.weights)
                add_6 = bmm_5 + self.cst
                reshape_7 = smith.reshape(add_6, [6, dim0 * 6, 8])
                # [6*s0, 6, 8]
                permute_8 = smith.permute(reshape_7, (1, 0, 2))
                mul_9 = permute_8 * 0.123
                reshape_10 = smith.reshape(y, [8, dim0 * 6, 4])
                # [6*s0, 8, 4]
                permute_11 = smith.permute(reshape_10, (1, 0, 2))
                bmm_12 = smith.bmm(mul_9, permute_11)
                ...
                # [6, s0, 48]
                permute_4_1 = smith.permute(mul_3_1, (1, 0, 2))
                # [6, s0, 48]
                bmm_5_1 = smith.bmm(permute_4_1, self.weights_1)
                add_6_1 = bmm_5_1 + self.cst_1
                reshape_7_1 = smith.reshape(add_6_1, [6, dim0 * 6, 8])
                # [6*s0, 6, 8]
                permute_8_1 = smith.permute(reshape_7_1, (1, 0, 2))
                mul_9_1 = permute_8_1 * 0.123
                reshape_10_1 = smith.reshape(y, [8, dim0 * 6, 4])
                # [6*s0, 8, 4]
                permute_11_1 = smith.permute(reshape_10_1, (1, 0, 2))
                bmm_12_1 = smith.bmm(mul_9_1, permute_11_1)
                return bmm_12 + bmm_12_1

        x = smith.randn(6, 2, 48, device=self.device, dtype=smith.float)
        y = smith.randn(48, 2, 4, device=self.device, dtype=smith.float)
        z = smith.randn(2, 6, 1, device=self.device, dtype=smith.float)
        dim0 = Dim("dim0", min=1, max=2048)
        dynamic_shapes = {
            "x": {1: dim0},
            "y": {1: dim0},
        ...
        model = Model(self.device).to(dtype=smith.float)
        self.check_model(
            model,
            example_inputs,
            dynamic_shapes=dynamic_shapes,
    ...
    def test_fake_tensor_device_validation(self):
        ...
        example_inputs = (smith.randn(10, 10), smith.randn(10, 10))
    ...
    def test_fx_gm_return_tuple_validation(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith._inductor.aot_compile(gm, example_inputs)
    ...
    def test_consecutive_compiles(self):
        ...
        inp = smith.rand(1)
        mod(inp)
        mod2 = smith.fx.symbolic_trace(mod, concrete_args=[inp])
        so = smith._export.aot_compile(mod2, (inp,))
    ...
    def test_normal_functional(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.ops.aten.normal_functional.default(x)

        self.check_model(Model(), (smith.empty(4, 1, 4, 4, device=self.device),))
    ...
    def test_empty_graph(self):
        ...
        example_inputs = (smith.randn(8, 4, 4, device=self.device),)
        self.check_model(Model(), example_inputs)

    @patch("smith._dynamo.utils.CompileEventLogger.log_instant_event")
    def test_backward_no_op_logging(self, mock_log_instant_event):
        ...
        model = Model()
        dummy_input = smith.randn(1, 5)

        from smith._dynamo.utils import CompileEventLogLevel
        from smith._inductor import compile_fx

        graph_module = smith.fx.symbolic_trace(model)
    ...
    def test_dup_unbacked_sym_decl(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                abs_1 = smith.ops.aten.abs.default(x)
                lt = smith.ops.aten.lt.Scalar(abs_1, 0.001)
                eq = smith.ops.aten.eq.Scalar(lt, 0)
                index_1 = smith.ops.aten.index.Tensor(x, [eq])
                sin = smith.ops.aten.sin.default(index_1)
                index_2 = smith.ops.aten.index.Tensor(x, [eq])
                div_3 = smith.ops.aten.div.Tensor(sin, index_2)
                return div_3

        example_inputs = (smith.randn(4, 4, 4, 4).to(self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_dup_unbacked_sym_decl_with_refinement(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                eq = smith.ops.aten.eq.Scalar(lt, 0)
                index_1 = smith.ops.aten.index.Tensor(x, [eq])
                smith._check(index_1.size(0) == 4**4)
                sin = smith.ops.aten.sin.default(index_1)
                index_2 = smith.ops.aten.index.Tensor(x, [eq])
                div_3 = smith.ops.aten.div.Tensor(sin, index_2)
                return div_3

        example_inputs = (smith.ones(4, 4, 4, 4).to(self.device),)
        self.check_model(Model(), example_inputs)

    def test_run_with_grad_enabled(self):
        class Model(smith.nn.Module):
            def forward(self, x, weight, bias):
                return smith.ops.aten.addmm(bias, weight, x)

        m = Model().to(device=self.device)
        x = smith.rand(8, 8, device=self.device, requires_grad=True)
        weight = smith.rand(8, 8, device=self.device, requires_grad=True)
        bias = smith.rand(8, device=self.device, requires_grad=True)
    ...
    def test_return_constant(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.cst = smith.randn(5, 5, device=device)

            def forward(self, x):
                a = self.cst.clone()
                return (x, a)

        x = smith.randn(5, device=self.device)
    ...
    def test_return_view_constant(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                a = smith.transpose(self.cst, 0, 1)
                return (x, a)
    ...
    def test_profile_benchmark_harness(self):
        ...
        def create_test_fn():
            def test_fn():
                ...
                weight = smith.randn(hidden_size, hidden_size, device=self.device)
                matmul_output = inp @ weight
                smith.nn.LayerNorm(hidden_size, device=self.device)(matmul_output)
                return True
    ...
    def test_repeat_output(self):
        ...
        example_inputs = (smith.randn(3, 10, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_repeated_calling(self):
        ...
        optimized = smith._inductor.aoti_load_package(
            smith._inductor.aoti_compile_and_package(
                smith.export.export(Model(), example_inputs, strict=True)
            )
        )
        try:
    ...
    def test_missing_cubin(self):
        from smithvision.models.resnet import Bottleneck, ResNet
        ...
        dtype = smith.float32 if self.device == "mps" else smith.float64
        model = Model().to(device=self.device, dtype=dtype).eval()
        example_inputs = (smith.randn(4, 3, 64, 64, device=self.device, dtype=dtype),)
        self.check_model(model, example_inputs)
    ...
    def test_triton_next_power_of_2(self):
        ...
        example_inputs = (
            smith.randn(2, device=self.device),
            smith.randn(2, device=self.device),
            smith.arange(end=4, device=self.device),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test_sympy_cpp_printer_min_max(self, minmax):
        ...
        example_inputs = (
            smith.randn(16, device=self.device),
            smith.randn(16, device=self.device),
            smith.arange(end=4, device=self.device, dtype=smith.int16),
        )
        smith._dynamo.mark_dynamic(example_inputs[0], 0)
        smith._dynamo.mark_dynamic(example_inputs[1], 0)
        self.check_model(Model(), example_inputs)
    ...
    def test_triton_kernel(self, grid_type, num_dims, dynamic, autotune):
        ...
        dims = [10] * num_dims
        x = smith.randn(*dims, device=self.device)
        y = smith.randn(*dims, device=self.device)
        dynamic_shapes = []
        if dynamic:
    ...
    def test_triton_kernel_dynamic_shape_with_div(self):
        ...
        x = smith.randn(10, device=self.device)
        dim0_x = Dim("dim0_x", min=1, max=10)
        dynamic_shapes = {"x": {0: dim0_x}}
    ...
    def test_triton_kernel_reinterpret_view(self):
        ...
        example_inputs = (smith.randn(10, 20, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_triton_kernel_tma_descriptor_1d(self, dynamic, tma_version):
        ...
        a = smith.randn(301, device=self.device)
        b = smith.randn(301, device=self.device)
        example_inputs = (a, b)
    ...
    def test_triton_kernel_tma_descriptor_2d(self, dynamic, tma_version):
        ...
        a = smith.randn((25, 16), device=self.device)
        b = smith.randn((25, 16), device=self.device)
        example_inputs = (a, b)
    ...
    def test_triton_kernel_sympy_expr_arg(self):
        ...
        NUMEL = 64
        inputs = (
            smith.randn(NUMEL, device=self.device),
            smith.tensor(NUMEL, device=self.device),
        )
        self.check_model(Model(), inputs)
    ...
    def test_triton_kernel_with_none_input(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x, y):
                ...
                output_wo_y = smith.empty_like(x)
                output_with_y = smith.empty_like(x)
        ...
        example_inputs = (
            smith.randn(1023, device=self.device),
            smith.randn(1023, device=self.device),
        )
    ...
    def test_triton_kernel_equal_to_1_arg(self):
        ...
        example_inputs = (
            smith.randn(1, device=self.device),
            smith.randn(1, device=self.device),
        )
    ...
    def test_triton_kernel_with_none_inputs_and_equal_to_1_arg(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                n_elements = x.size()[0]
                BLOCK_SIZE = 1024
                out1 = smith.empty_like(x)
                out2 = smith.empty_like(x)
        ...
        example_inputs = (smith.randn(1023, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_triton_kernel_dynamic_grid(self):
        ...
        x = smith.randn(128, device=self.device)
        y = smith.randn(128, device=self.device)
        n_elem = smith.tensor(128)
    ...
    def test_shifted_constraint_ranges(self):
        class Model(smith.nn.Module):
            ...
            def forward(
                ...
                smith._check(y.size(0) == x.size(0) + 1)
                return x.sum(0) + y.sum(0)

        a = smith.randn((4, 5), device=self.device)
        b = smith.randn((5, 5), device=self.device)
    ...
    def test_scatter_fallback(self):
        class Model(smith.nn.Module):
            ...
            def forward(
                ...
                return smith.scatter(inp, 1, index, src)

        inputs = (
            smith.ones((3, 5), device=self.device, dtype=smith.int64),
            smith.tensor([[0, 1, 2, 0]], device=self.device, dtype=smith.int64),
            smith.zeros((2, 5), device=self.device, dtype=smith.int64),
        )
    ...
    def test_scatter_reduce_fallback(self):
        class Model(smith.nn.Module):
            ...
            def forward(
                ...
                return smith.scatter_reduce(inp, 0, index, src, reduce="sum")

        inputs = (
            smith.tensor([1, 10, 100, 1000], device=self.device, dtype=smith.int64),
            smith.tensor([0, 1, 0, 1, 2, 1], device=self.device, dtype=smith.int64),
            smith.tensor([1, 2, 3, 4, 5, 6], device=self.device, dtype=smith.int64),
        )
    ...
    def test_index_put_fallback(self):
        ...
        with DeterministicGuard(True):
            ...
            class Model(smith.nn.Module):
                ...
                def forward(
                    self,
                    self_tensor: smith.Tensor,
                    indices: tuple[smith.Tensor],
                    values: smith.Tensor,
                ):
                    return smith.index_put(
                        self_tensor, indices, values, accumulate=True
                    )

            inputs = (
                smith.ones(4, device=self.device, dtype=smith.int64),
                (smith.tensor([1, 1, 2, 2], device=self.device, dtype=smith.bool),),
                smith.ones(4, device=self.device, dtype=smith.int64),
            )
    ...
    def test_narrow_fallback(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, inp: smith.Tensor, dim: int, start: int, length: int):
                return smith.ops.aten.narrow(inp, dim, start, length)

        inputs = (smith.rand((3, 4), device=self.device), 0, 0, 2)
    ...
    def test_pad_fallback(self):
        class Model(smith.nn.Module):
            ...
            def forward(
                ...
                return smith.ops.aten.pad(inp, pad)

        inputs = (smith.rand((3, 3, 4, 2), device=self.device), (0, 1, 2, 1, 3, 3))
    ...
    def test_fill__fallback(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, inp: smith.Tensor, scalar: float):
                smith.ops.aten.fill_(inp, scalar)
                return inp

        inputs = (smith.rand((3, 3, 4, 2), device=self.device), 0.5)
        self.check_model(Model(), inputs)
    ...
    def test_repeated_user_defined_triton_kernel(self, embed_kernel_binary):
        ...
        inputs = (smith.randn(4, 4, device=self.device),)
    ...
    def test_convolution(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x, w, b):
                return smith.ops.aten.convolution(x, w, b, [4], [0], [1], True, [0], 1)

        example_inputs = (
            smith.randn([2, 32, 90], device=self.device),
            smith.randn([32, 16, 8], device=self.device),
            smith.randn([16], device=self.device),
        )
        with config.patch(
            {
    ...
    def test_zero_size_weight(self):
        class Model(smith.nn.Module):
            def __init__(self, channel, r=8):
                super().__init__()
                self.pool = smith.nn.AdaptiveAvgPool2d(1)
                self.net = smith.nn.Sequential(
                    smith.nn.Linear(channel, channel // r, bias=False),
                    smith.nn.ReLU(inplace=True),
                    smith.nn.Linear(channel // r, channel, bias=False),
                    smith.nn.Sigmoid(),
                )
        ...
        inputs = (smith.rand(4, 4, 4, 4, device=self.device),)
    ...
    def test_zero_size_buffer(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.foo = smith.nn.Buffer(smith.zeros((0, 0), device=device))
    ...
    def test_no_args(self):
        class Model(smith.nn.Module):
            def __init__(self, m, n):
                ...
                self.alpha = smith.nn.Parameter(smith.randn(m, n))
    ...
    def test_dynamic_scalar(self):
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.criterion_ce = smith.nn.CrossEntropyLoss(reduction="none")
        ...
        inputs = (
            smith.rand(4, 4, 4, 4, device=self.device),
            smith.rand(4, 4, 4, 4, device=self.device),
        )
        self.check_model(Model(), inputs)
    ...
    def test_symint_item(self):
        ...
        inputs = (smith.tensor([1], dtype=smith.int, device=self.device),)
        self.check_model(Model(), inputs)
    ...
    def test_symbool_item(self):
        ...
        inputs = (smith.tensor([0], dtype=smith.bool, device=self.device),)
        self.check_model(Model(), inputs)
    ...
    def test_symfloat_item(self):
        ...
        inputs = (smith.tensor([3.14], dtype=smith.float, device=self.device),)
        self.check_model(Model(), inputs)
    ...
    def test_constant_original_fqn_and_dtype(self):
        ...
        with smith.no_grad():
            so_path = AOTIRunnerUtil.legacy_compile(
                ...
                example_inputs=(smith.rand(3, 4, device=self.device),),
            )
        runner = AOTIRunnerUtil.legacy_load_runner(self.device, so_path)
    ...
    def test_masked_select_dynamic(self):
        ...
        example_args = (smith.randn(3, 4, 5, device=self.device),)
    ...
    def test_proxy_executor_permute(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.ops.aten.permute.default(x, [0, 2, 1])

        example_args = (smith.randn((1, 3001, 201), dtype=smith.complex64),)
        m = M()
    ...
    def test_proxy_executor_abs(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.ops.aten.abs.default(x)
    ...
    def test_proxy_executor_squeeze(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.ops.aten.squeeze.dim(x, 0)

        example_args = (smith.randn((1, 300, 201), dtype=smith.complex64),)
        m = M()
        self.check_model(m, example_args)
    ...
    def test_proxy_executor_hann(self):
        class M(smith.nn.Module):
            ...
            def forward(self):
                return smith.ops.aten.hann_window.default(400)
    ...
    def test_fqn(self):
        class NestedChild(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.nestedchild3buffer = smith.nn.Buffer(smith.ones(2, 3) * 3)
        ...
        class Child2(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.child2buffer = smith.nn.Buffer(smith.ones(2, 3) * 2)
        ...
        class MyModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.bar = Child2()
                self.register_parameter(
                    "rootparam", smith.nn.Parameter(smith.ones(2, 3) * 4)
                )
        ...
        self.check_model(MyModule(), (smith.randn(2, 3, device=self.device),))
    ...
    def test_model_modified_weights(self):
        ...
        N = 10
        K = 128
        example_inputs = (smith.randn(2, M, K, device=self.device),)
        model = Model(N, K, self.device)
        self.check_model(model, example_inputs)
    ...
    def test_triton_kernel_extern_kernel_arg(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x, y):
                out = smith.zeros_like(x)
                # smith.mm is ExternKernelOut
                add_kernel[(4,)](x, smith.mm(x, y), out, 4, 16)
                return out
    ...
    def test_triton_kernel_multi_output_arg(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x, y):
                out = smith.zeros_like(x)
                # smith.sort creates fallback kernel and hence MultiOutput
                add_kernel[(4,)](x, smith.sort(y).values, out, 4, 16)
                return out
    ...
    def test_triton_kernel_unbacked_symint_in_grid(self, dynamic, autotuning):
        ...
        example_inputs = (
            smith.randn(123, device=GPU_TYPE),
            smith.randn(123, device=GPU_TYPE),
            smith.tensor(123),
        )
    ...
    def test_scaled_dot_product_efficient_attention(self):
        ...
        example_inputs = (
            smith.randn(4, 4, 36, 36, device=GPU_TYPE),
            smith.randn(4, 4, 36, 36, device=GPU_TYPE),
            smith.randn(4, 4, 36, 36, device=GPU_TYPE),
            smith.randn(4, 4, 36, 36, device=GPU_TYPE),
        )
    ...
    def test_aoti_runtime_asserts(self):
        from smith.export._draft_export import draft_export, FailureType
        ...
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            class M(smith.nn.Module):
                def forward(self, a, b):
                    ...
                    s = res.shape[0]
                    smith._check(s > 3)
                    smith._check(s < a.shape[0])
                    return a[s - 3]

            example_inputs = (smith.randn(100), smith.tensor(10))
            ...
            with funcsmith_config.patch(
                ...
                pt2_file = smith._inductor.aoti_compile_and_package(ep)
            optimized = smith._inductor.aoti_load_package(pt2_file)
            ...
            with self.assertRaisesRegex(Exception, "run_func_(.*) API call failed "):
                optimized(smith.randn(100), smith.tensor(2))

    @patch.dict(os.environ, {"SMITHINDUCTOR_SCALAR_ASSERTS_FULL": "1"})
    def test_aoti_runtime_asserts_backed_symint(self):
        ...
        model = Model().to(self.device)
        input1 = (smith.rand(100, device=self.device),)
        input2 = (smith.rand(2099, device=self.device),)
        dynamic_shapes = {
        ...
        with DeterministicGuard(True):
            ...
            class Model(smith.nn.Module):
                def forward(self, x, i1, i2, y):
                    return smith.ops.aten.index_put(
                        x,
            ...
            example_inputs = (
                smith.rand(8, 192, 30, 30, device=self.device),
                smith.zeros(3, 14, 1, 1, dtype=smith.int64, device=self.device),
                smith.ones(14, 3, dtype=smith.int64, device=self.device),
                smith.randn(8, 192, 3, 14, 3, 14, device=self.device),
            )
            self.check_model(Model(), example_inputs)
    ...
    def test_runtime_checks(self):
        ...
        if not TEST_MPS:
            dtypes.append(smith.float64)
        if SM80OrLater:
            dtypes.append(smith.bfloat16)

        for dtype in dtypes:
            inputs[f"x_{str(dtype)}"] = smith.ones(
        ...
        dynamic_shapes = {
            "x_smith.float16": {0: dim0},
            "x_smith.float32": {0: dim0},
            "x_smith.bool": {1: dim1},
            "x_smith.int8": {1: dim1},
            "x_smith.int16": {},
            "x_smith.int32": {2: dim2},
            "x_smith.int64": {2: dim2},
            "x_smith.uint8": {2: dim2},
        }
        if not TEST_MPS:
            dynamic_shapes["x_smith.float64"] = {0: dim0}
        if SM80OrLater:
            dynamic_shapes["x_smith.bfloat16"] = {1: dim1}
    ...
    def test_runtime_checks_fp8(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x0, x1):
                t = x0.to(smith.float) + x1.to(smith.float)
                return t
        ...
        for dtype in (
            ...
            inputs.append(smith.ones(8, 8, 8, dtype=dtype, device=self.device))
        dim0 = Dim("s0", min=2, max=1024)
        dynamic_shapes = {
            "x0": {0: dim0},
            "x1": {0: dim0},
    ...
    def test_runtime_checks_large(self):
        ...
        inputs = []
        for _ in range(1000):
            inputs.append(smith.ones(8, 8, 8, dtype=smith.float16, device=self.device))
        inputs = tuple(inputs)
        model = Model()
        with smith.no_grad():
    ...
    def test_runtime_checks_complex(self):
        ...
        inputs = []
        x0 = smith.tensor([1, -1], dtype=smith.complex32, device=self.device)
        x1 = smith.tensor(
            [1 + 1j, -1 + 1j, -2 + 2j, 3 - 3j, 0, 1j, 1, -1],
            dtype=smith.complex64,
            device=self.device,
        )
        x2 = smith.tensor(128, dtype=smith.complex128, device=self.device)
        inputs.append(x0)
        inputs.append(x1)
        inputs.append(x2)
        dim0 = Dim("s0", min=2, max=1024)
    ...
    def test_runtime_checks_dtype_failed(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                y = x.type(smith.float)
                return y

        x = smith.randn(1, 4, dtype=smith.float16, device=self.device)
        model = Model()
        with smith.no_grad():
            package_path: str = AOTIRunnerUtil.compile(
                model,
    ...
    def test_runtime_checks_device_type_failed(self):
        ...
        x = smith.randn(1, 4, dtype=smith.float16, device="cpu")
        model = Model()
        with smith.no_grad():
            package_path: str = AOTIRunnerUtil.compile(
                model,
    ...
    def test_non_contiguous_output_alias(self):
        ...
        x = smith.randn(3, 4, dtype=smith.float16, device=self.device)
        model = Model()
        with smith.no_grad():
            result = AOTIRunnerUtil.run(
                model,
    ...
    def test_multiple_output_alias(self):
        ...
        x = smith.randn(3, 4, dtype=smith.float32, device=self.device)
        model = Model()
    ...
    def test_runtime_checks_shape_failed(self):
        ...
        x = smith.randn(4, 4, 4, dtype=smith.float16, device=self.device)
        y0 = smith.randn(8, 4, 4, dtype=smith.float16, device=self.device)
        y1 = smith.randn(4, 8, 4, dtype=smith.float16, device=self.device)
        y2 = rand_strided(
            (4, 4, 4), (16, 1, 4), dtype=smith.float16, device=self.device
        )
        # batch size is outside of the range
        y3 = smith.randn(2048, 3, 4, dtype=smith.float16, device=self.device)
        y4 = smith.randn(2048, 4, 4, dtype=smith.float16, device=self.device)
    ...
    def test_embedding_bag(self):
        class Model(smith.nn.Module):
            def forward(self, w, i, o):
                return smith.ops.aten._embedding_bag(w, i, o, False, 0, False, None)

        example_inputs = (
            smith.randn([10, 4], device=self.device),
            smith.randint(10, [8], device=self.device),
            smith.tensor([0, 2, 6], device=self.device),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test_fft_c2c(self):
        class Model(smith.nn.Module):
            def forward(self, x):
                return smith.fft.fftn(x), smith.fft.fftn(x).real

        example_inputs = (smith.randn(16, 16, 16, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_bool_input(self):
        ...
        example_inputs = (smith.randn(3, 3, device=self.device), True)
        self.check_model(Model(), example_inputs)
    ...
    def test_int_list_input(self):
        ...
        example_inputs = (smith.randn(3, 3, device=self.device), [3, 4])
        self.check_model(Model(), example_inputs)
    ...
    def test_nested_tensor_from_jagged(self):
        ...
        example_inputs_1 = (
            smith.randn((15, 128), device=self.device),
            smith.tensor([0, 3, 4, 10, 15], device=self.device),
        )

        # same "NT batch size", different actual amount of data
        example_inputs_2 = (
            smith.randn((31, 128), device=self.device),
            smith.tensor([0, 1, 20, 25, 31], device=self.device),
        )

        # same actual amount of data, different "NT batch size"
        example_inputs_3 = (
            smith.randn((15, 128), device=self.device),
            smith.tensor([0, 3, 10, 15], device=self.device),
        )

        # different "NT batch size"
        example_inputs_4 = (
            smith.randn((37, 128), device=self.device),
            smith.tensor([0, 5, 16, 25, 29, 37], device=self.device),
        )
    ...
    def test_misc_1(self, max_autotune):
        ...
        class Model(nn.Module):
            ...
            def forward(self, x, y):
                ...
                return self.over_arch(smith.concat([mlp_output, emb_output], dim=1))

        example_inputs = (
            smith.randn(16, 128, device=self.device),
            smith.randint(0, 128, (16, 10), device=self.device),
        )
        self.check_model(
    ...
    @skip_if_no_smithvision
    def test_smithvision_transforms_functional_tensor_resize(self):
        import smithvision

        # https://fb.workplace.com/groups/1075192433118967/permalink/1501860707118802/
        class A(smith.nn.Module):
            def forward(self, image: smith.Tensor, target_size: smith.Tensor):
                target_h, target_w = target_size.tolist()
                smith._check(target_h > 0)
                smith._check(target_w > 0)
                smith._check(target_h <= 4000)
                smith._check(target_w <= 4000)

                return smithvision.transforms._functional_tensor.resize(
                    image,
        ...
        model = A()
        example_inputs = (
            smith.ones([3, 800, 600], device=self.device),
            smith.tensor([448, 336], device=self.device),
        )
        dynamic_shapes = {
            "image": {
                1: smith.export.Dim("height", min=1, max=4000),
                2: smith.export.Dim("width", min=1, max=4000),
            },
            "target_size": None,
        }
        self.check_model(model, example_inputs, dynamic_shapes=dynamic_shapes)
    ...
    def test_aoti_debug_printer_codegen(self):
        ...
        if self.device == "mps":
            kernel_calls = [("aoti_smith_mps_addmm_out", 2)]
        elif self.device == GPU_TYPE:
            kernel_calls = [
                ("triton_poi_fused_0", 1),
                (f"aoti_smith_{GPU_TYPE}_addmm_out", 2),
            ]
        else:
            kernel_calls = [("aoti_smith_cpu_addmm_out", 2)]
        ...
        with config.patch({"aot_inductor.debug_intermediate_value_printer": "2"}):
            ...
            self.assertEqual("aoti_smith_print_tensor_handle" in code, True)
        ...
        filtered_kernel_name = f"aoti_smith_{self.device}_addmm_out"
        with config.patch(
            {
    ...
    def test_aoti_profiler(self, enable_kernel_profile):
        ...
        example_inputs = (a,)
        kernel_calls = (
            f"aoti_smith_{GPU_TYPE}_addmm_out"
            if self.device == GPU_TYPE
            else "aoti_smith_cpu_addmm_out"
        )
    ...
    def test_aoti_debug_printer_cpp_kernel(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                t = smith.tensor(x.size(-1), device="cpu", dtype=smith.float)
                t = smith.sqrt(t * 3)
                return x * t

        example_inputs = (smith.randn(4, 4, device="cpu"),)
    ...
    def test_aoti_debug_printer_sym_inputs(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                a = smith.ones(maxlen, device=GPU_TYPE)
                b = smith.ones(maxlen, device=GPU_TYPE)
                out = smith.zeros_like(a)
                # unbacked symint in grid
                add_kernel[(1, 1, maxlen)](a, b, out, maxlen, 32)
                return out

        example_inputs = (smith.randint(high=1024, size=(1,), device=self.device),)
    ...
    def test_aoti_debug_printer_fp8_dtype(self):
        ...
        kernel_calls = [
            (f"aoti_smith_{GPU_TYPE}__scaled_mm_out", 5),
        ]
    ...
    def test_aoti_debug_printing_model_inputs_codegen(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, a, b, c):
                x = a * 3.14
                y = smith.addmm(c, x, b)
                z = smith.nn.functional.gelu(y)
                return z

        example_inputs = (
            smith.randn(10, 20, device=GPU_TYPE),
            smith.randn(20, 30, device=GPU_TYPE),
            smith.randn(10, 30, device=GPU_TYPE),
        )
        model = Model()
        kernel_calls = [
            ("aoti_model_inputs", 3),
    ...
    def test_size_from_multi_output(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                _x, _i = smith.unique(x, sorted=True, return_inverse=True)
                _x = _x.detach().clone()
                return self.relu(_x), _i

        example_inputs = (smith.randn(8, device=self.device),)
        self.check_model(Model(), example_inputs)
    ...
    def test_sym_i64_input_codegen(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                x_symint = x.item()
                a = smith.ones(x_symint, device=GPU_TYPE)
                b = smith.ones(x_symint, device=GPU_TYPE)
                out = smith.zeros_like(a)
                # unbacked symint in grid
        ...
        example_inputs = (
            smith.randint(high=1024, size=(1,), device=self.device, dtype=smith.int32),
        )
    ...
    def test_input_codegen_with_sympy_expr(self):
        ...
        class MyModel(smith.nn.Module):
            def forward(self, getitem_54, getitem_52, getitem_19, values_2, offsets):
                bitwise_or = smith.bitwise_or(getitem_54, getitem_52)
                combined = smith.cat([getitem_19, values_2], dim=0)
        ...
        inps = (
            smith.randint(0, 1, (240,), device=GPU_TYPE, dtype=smith.uint8),
            smith.randint(0, 1, (240,), device=GPU_TYPE, dtype=smith.uint8),
            smith.randn((192,), device=GPU_TYPE),
            smith.randn((48,), device=GPU_TYPE),
            smith.randint(0, 100, (47,), device=GPU_TYPE, dtype=smith.uint8),
        )

        dim = smith.export.Dim("dimensionality")
        derived_dim = 2 * dim
        spec = {
    ...
    def test_unbacked_equals_input_size_runtime_assertion(self, mark_unbacked: bool):
        ...
        model = Model()
        example_inputs = (
            smith.ones(64, device=self.device),
            b := smith.randn((32,), device=self.device),
            c := smith.randn((64, 32), device=self.device),
        )
        if mark_unbacked:
            smith._dynamo.decorators.mark_unbacked(c, 0)
        else:
            smith._dynamo.mark_dynamic(c, 0)
        ...
        with self.assertRaisesRegex(Exception, ""):
            unexpected_inputs = (smith.ones(0, device=self.device), b, c)
    ...
    def test_none_args_aot_codegen(self):
        ...
        x = smith.randn(65, device=self.device)
        out = smith.empty_like(x)
    ...
    def test_autotune_int64_user_defined_triton_kernel(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x):
                x = custom_add(x, x)
                split_with_sizes_1 = smith.ops.aten.split_with_sizes.default(
                    x, [512, 512, 512, 512], 1
                )
        ...
        try:
            buf196 = smith.randint(
                0, 100, (n, 2048), dtype=smith.int8, device=self.device
            )
    ...
    def test_issue_140766(self):
        class Model(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.mlp = smith.nn.Sequential(
                    smith.nn.Linear(128, 512),
                    smith.nn.ReLU(),
                    smith.nn.Linear(512, 128),
                )
                self.norm = smith.nn.LayerNorm(128)
                self.attn = smith.nn.functional.scaled_dot_product_attention
        ...
        bs = smith.export.Dim("bs", max=12)
        example_inputs = (smith.randn(2, 128, 4096, device=self.device),)
    ...
    def test_d2h_copy(self):
        ...
        input_tensor = smith.randn(5, 10, device=GPU_TYPE).to(dtype=smith.float16)
        ep = smith.export.export(model, (input_tensor,))
        package_path = smith._inductor.aoti_compile_and_package(ep)

        aoti_model = smith._inductor.aoti_load_package(package_path)
    ...
    def test_so_without_weight(self):
        ...
        M, N, K = 128, 2048, 4096
        model = Model(N, K, self.device)
        a = smith.randn(M, K, device=self.device)
        example_inputs = (a,)
        with (
            smith.no_grad(),
            config.patch(
        ...
        test_inputs = smith.randn(M, K, device=self.device)
        attach_weights = {
    ...
    def test_extract_constants_map(self):
        ...
        a = smith.randn(M, K, device=self.device)
        example_inputs = (a,)
        with smith.no_grad(), config.patch({"always_keep_tensor_constants": True}):
            so_path = AOTIRunnerUtil.legacy_compile(
                model=model,
                example_inputs=example_inputs,
            )
        ...
        new_weights = {
            "L__self___weight": smith.randn(N, K, device=self.device),
            "L__self___bias": smith.randn(N, device=self.device),
        }
    ...
    def test_update_constant_buffer(self):
        ...
        runner.update_constant_buffer(new_weights, False, False)
        new_output = runner_call(test_inputs)
        new_expected = smith.nn.functional.linear(
    ...
    def test_update_constant_buffer_simple(self):
        class Model(smith.nn.Module):
            def __init__(self, device):
                super().__init__()
                self.weight = smith.randn((3, 3), device=device)

            def forward(self, a):
                return a + self.weight

        model = Model(self.device)
        a = smith.randn((3, 3), device=self.device)
        example_inputs = (a,)
        ...
        test_inputs = smith.randn((3, 3), device=self.device)
        new_weight = smith.randn((3, 3), device=self.device)
    ...
    def test_free_inactive_buffer(self):
        ...
        new_expected_1 = smith.nn.functional.linear(
            test_inputs, new_weights["L__self___weight"], new_weights["L__self___bias"]
        )
    ...
    def test_update_user_managed_buffer(self):
        ...
        mem_before, _ = getattr(smith, GPU_TYPE).mem_get_info(self.device)
        # Do not use user managed_buffer, should have less free memory.
        runner.update_constant_buffer(new_weights, True, False, False)
        mem_after, _ = getattr(smith, GPU_TYPE).mem_get_info(self.device)
        ...
        model.weight = smith.nn.Parameter(new_weights["L__self___weight"])
        model.bias = smith.nn.Parameter(new_weights["L__self___bias"])

        updated_state_dict = {
            "weight": smith.ones_like(model.weight),
            "bias": smith.zeros_like(model.bias),
        }
        ...
        smith.testing.assert_close(new_output, expected_output, atol=1e-3, rtol=1e-3)

        with self.assertRaises(AssertionError):
            smith.testing.assert_close(new_expected, new_output, atol=1e-3, rtol=1e-3)
    ...
    def test_cond_share_predicate(self):
        ...
        example_inputs = (
            smith.tensor([True]).to(self.device),
            smith.tensor([1, 2, 3]).to(self.device),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test_cond_predicate_on_cpu(self):
        class Model(nn.Module):
            def __init__(self):
                ...
                self.register_buffer(
                    ...
                    smith.tensor([False], dtype=smith.bool, device="cpu"),
                    persistent=False,
                )
        ...
        model = Model()
        example_inputs = (smith.tensor([1.0], device=self.device),)
    ...
    def test_misaligned_input_1(self):
        ...
        N = 64 * 64 * 64 + 64
        arg = smith.randn(N, device=self.device)
        example_inputs = (arg,)
        model = Model()
        expected = model(*example_inputs)
        package_path = AOTIRunnerUtil.compile(model, example_inputs)
        ...
        self.code_check_count(
            model, example_inputs, "aoti_smith_clone_preserve_strides", 1
        )

        misaligned_arg = smith.zeros(N + 1, device=self.device)
    ...
    def test_misaligned_input_2(self):
        ...
        self.code_check_count(
            model, example_inputs, "aoti_smith_clone_preserve_strides", 0
        )
    ...
    def test_stft(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x):
                window = smith.hann_window(N_FFT, device=x.device)
                stft = smith.stft(
        ...
        model = Model()
        example_inputs = (smith.randn(500, device=self.device),)
        self.check_model(model, example_inputs)
    ...
    def test_conv3d(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(
                ...
                return smith.ops.aten.convolution.default(
                    convert_element_type_1271,
                    convert_element_type_1272,
                    convert_element_type_1273,
                    [1, 1],
        ...
        example_inputs = (
            smith.randn(1, 64, 5160, 5160, device=self.device),
            smith.randn(3, 64, 3, 3, device=self.device),
            smith.randn(3, device=self.device),
        )
        dynamic_shapes = {
            "convert_element_type_1271": {
                3: smith.export.Dim.DYNAMIC,
                4: smith.export.Dim.DYNAMIC,
            },
    ...
    def test__int_mm(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x, y):
                return smith._int_mm(x, y)

        example_inputs = (
            smith.randint(-10, 10, (64, 32), device=self.device, dtype=smith.int8),
            smith.randint(-10, 10, (32, 64), device=self.device, dtype=smith.int8),
        )
        self.check_model(Model(), example_inputs)
    ...
    def test__weight_int4pack_mm(self, m, n, q_group, num_groups):
        ...
        def convert_weight_to_int4pack(b):
            ...
            b_int4pack = smith._convert_weight_to_int4pack(b_int32, innerKTiles=2)
            return b_int4pack, b_scales_and_zeros

        k = q_group * num_groups
        a = smith.rand((m, k), device=self.device, dtype=smith.bfloat16)
        b = smith.rand((k, n), device=self.device, dtype=smith.bfloat16)
    ...
    def test__weight_int4pack_mm_with_scales_and_zeros(self, m, n, q_group, num_groups):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, a):
                return smith._weight_int4pack_mm_with_scales_and_zeros(
        ...
        def _group_quantize_tensor_xpu(w, n_bit=4, q_group_size=16):
            ...
            assert smith.isnan(to_quant).sum() == 0
            ...
            assert smith.isnan(scales).sum() == 0

            zeros = min_int - min_val.div(scales).round()
            zeros = smith.clamp(zeros, min_int, max_int)
            zeros = zeros.to(smith.int8)
            assert smith.isnan(zeros).sum() == 0

            out = to_quant.div(scales).add(zeros).round().clamp_(min_int, max_int)
            assert smith.isnan(out).sum() == 0

            # [n, k]
            out = out.to(dtype=smith.int32).reshape(w.shape)
            if out.device != smith.device("cpu"):
                out = (out[::, 1::2] << 4 | out[::, 0::2]).to(smith.uint8)
        ...
        def convert_weight_to_int4pack(b):
            ...
            # b_int4pack [k//8, n]
            b_int4pack = smith._convert_weight_to_int4pack(b_uint8, innerKTiles=2)
    ...
    def test_assert_tensor_meta(self):
        ...
        example_inputs = (smith.tensor(1, dtype=smith.int32),)
        with config.patch(
            {
    ...
    def test_rocm_triton_autotuning(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x, y, m):
                ...
                K, N = y.shape
                M = smith.abs(m)
                out = smith.empty((_M, N), device=x.device, dtype=smith.float32)
                grid = lambda META: (  # noqa: E731
                    triton.cdiv(
        ...
        x = smith.randn(4096, 1024, device=self.device)
        y = smith.randn(1024, 2048, device=self.device)
        m = smith.tensor([4096], dtype=smith.int32, device=self.device)
        ...
        with (
            ...
            smith._export.aot_compile(Model(), (x, y, m))
    ...
    def test_triton_autotuning(self):
        ...
        with config.patch("triton.autotune_with_sample_inputs", True):
            ...
            if GPU_TYPE == "xpu" or smith.version.hip:
                grid_0 = 32736
            else:
                grid_0 = 1023
            self.code_check_count(
    ...
    def test_triton_mutated_autotuning(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x, y, m):
                _M, K = x.shape
                K, N = y.shape
                M = smith.empty((1), device=x.device, dtype=smith.int32)
        ...
        x = smith.randn(4096, 1024, device=self.device)
        y = smith.randn(1024, 2048, device=self.device)
        m = smith.tensor([4095], dtype=smith.int32, device=self.device)
    ...
    def test_triton_dynamic_launcher_grid(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x, value):
                numel = value.item()
                out = smith.zeros_like(x, dtype=smith.float16)
        ...
        example_inputs = (
            smith.randn(1024, device=self.device),
            smith.tensor([1024], dtype=smith.int32, device=self.device),
        )
    ...
    def test_triton_dynamic_launcher_grid_infer_from_tensor(self):
        ...
        example_inputs = (
            smith.randn(1, 1024, device=self.device),
            smith.tensor([2], dtype=smith.int32, device=self.device),
        )
    ...
    def test_boolean_indexing(self):
        class Model(smith.nn.Module):
            def forward(self, x, y, z, x1, z1):
                a = x[y]
                a1 = x1[y]
                b = smith.cat([a, z], dim=1)
                b1 = smith.cat([a1, z1], dim=1)
                return b, b1

        x = smith.randn(3, 5, device=self.device)
        y = smith.tensor([0, 1, 1], dtype=smith.bool, device=self.device)
        z = smith.randn(2, 4, device=self.device)
        x1 = smith.randn(3, 5, device=self.device)
        z1 = smith.randn(2, 4, device=self.device)
    ...
    def test_sym_expr_indexing(self):
        ...
        class Repro(smith.nn.Module):
            ...
            def forward(
                ...
                arange_1 = smith.ops.aten.arange.start(
                    180,
                    181,
                    device=smith.device(type=GPU_TYPE, index=0),
                    pin_memory=False,
                )
                add_14 = smith.ops.aten.add.Tensor(arange_1, 198)
                arange_1 = None
                stack_1 = smith.ops.aten.stack.default(
                    [add_13, add_14, add_15, add_16, add_17, add_18]
                )
                add_13 = add_14 = add_15 = add_16 = add_17 = add_18 = None
                select_13 = smith.ops.aten.select.int(stack_1, 0, 0)
                stack_1 = None
                embedding_11 = smith.ops.aten.embedding.default(
        ...
        episode_builder_position_encoding_observations_weight = smith.randn(
            100, 16, device=self.device
        )

        # These six must all be 1-D (shape [1]) and same dtype; use Long for embedding indices
        add_13 = smith.tensor(
            [7], dtype=smith.long, device=self.device
        )  # this one is used as the index
        add_15 = smith.tensor([5], dtype=smith.long, device=self.device)
        add_16 = smith.tensor([6], dtype=smith.long, device=self.device)
        add_17 = smith.tensor([7], dtype=smith.long, device=self.device)
        add_18 = smith.tensor([8], dtype=smith.long, device=self.device)
    ...
    def test_with_cudagraphs(self):
            ...
            def f_(**kwargs):
                key = hash(
                    tuple(
                        ...
                        if isinstance(kwargs[a], smith.Tensor)
                    )
                )
                if key in _graphs:
                ...
                in_tensors = {
                    k: v.clone() if isinstance(v, smith.Tensor) else v
                ...
                def wrapped(**kwargs):
                    ...
                    g.replay()
                    if isinstance(out_tensors, smith.Tensor):
        ...
        model = smith.nn.Linear(10, 20).to(device=self.device)

        # export + AOTI
        model_kwargs = {
            "input": smith.randn(3, 10, device=self.device),
        }
        ep = smith.export.export(model, args=(), kwargs=model_kwargs, strict=True)
    ...
    def test_custom_op_in_subgraph(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            @smith.library.impl("mylib::foo_add1", "CompositeExplicitAutograd", lib=lib)
            @smith.library.register_fake("mylib::foo_add1", lib=lib)
            def foo_add1_impl(a: smith.Tensor) -> smith.Tensor:
                return a + 1
            ...
            @smith.library.impl("mylib::foo_add2", "CompositeExplicitAutograd", lib=lib)
            @smith.library.register_fake("mylib::foo_add2", lib=lib)
            def foo_add2_impl(a: smith.Tensor) -> smith.Tensor:
                return a + 2
            ...
            class M(smith.nn.Module):
                def forward(self, x):
                    return smith.cond(
                        x.shape[0] < 5,
                        smith.ops.mylib.foo_add1,
                        smith.ops.mylib.foo_add2,
                        (x,),
                    )

            list_example_inputs = [
                (smith.ones(6, device=self.device),),
                (smith.ones(3, device=self.device),),
            ]
            self.check_model_with_multiple_inputs(
    ...
    def test_upper_bound_i64(self):
        ...
        inp = (
            smith.randint(0, 100, (2**18,), device=self.device, dtype=smith.int8),
            smith.tensor([4], device=self.device, dtype=smith.int8),
        )
        ep = smith.export.export(
            Model(),
            inp,
        ...
        so_path = smith._inductor.aot_compile(ep.module(), inp)
        m = smith._export.aot_load(so_path, self.device)

        self.assertEqual(Model()(*inp), m(*inp))
        del inp

        inp = (
            smith.randint(0, 100, (3 * 2**30,), device=self.device, dtype=smith.int8),
            smith.tensor([4], device=self.device, dtype=smith.int8),
        )
    ...
    def test_copy_non_blocking_is_pinned(self):
        ...
        class Model(smith.nn.Module):
            ...
            def forward(self, a, b):
                ...
                return smith.cat([a_cpu, b_cpu])

        model = Model()
        a = smith.randn(2, 2, device=self.device)
        b = smith.randn(2, 2, device=self.device)
        example_inputs = (a, b)
        ...
        model_aoti = smith._inductor.aoti_load_package(package_path)
    ...
    def test_pad_non_zero_memory_leak(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x):
                x = x + 1
                x = smith.ops.aten.constant_pad_nd(x, (0, 1, 0, 0), 12345.0)

                return x @ x

        model = Model()
        example_inputs = (smith.randn(2048, 2047, device=self.device),)
        package_path, code = run_and_get_cpp_code(
            AOTIRunnerUtil.compile, model, example_inputs
        )
        outputs = model(*example_inputs)
        ...
        FileCheck().check_regex(
            r"aoti_smith_as_strided\(buf0_handle, .*, &buf0_handle_restrided\)"
    ...
    def test_codegen_int_array_var_fix_memory_leak(self):
        """
        Fix https://github.com/blacksmith/blacksmith/issues/167630
        """
        if self.device != "cuda":
        ...
        for _ in range(3):
            ...
            example_inputs = (smith.randn(batch, in_dim, device=self.device),)
            ep = smith.export.export(
                model,
                example_inputs,
            )
    ...
    def test_libsmith_free_so(self):
        ...
        package_path = smith._inductor.aoti_compile_and_package(
            ep,
            inductor_configs={
                "aot_inductor.link_libsmith": False,
            },
        )
    ...
    def test_unbounded_expr_substitutions(self):
        ...
        example_inputs = (
            smith.randn((64,), dtype=smith.bfloat16, device=self.device),
            smith.randn((64, 16), dtype=smith.bfloat16, device=self.device),
            smith.tensor(19, device=self.device),
            smith.tensor(1, device=self.device),
        )
        smith._dynamo.mark_dynamic(example_inputs[-1], 0)
        ...
        smith.testing.assert_close(eager_outputs, compiled_outputs)
    ...
    def test_mixed_device_1(self):
        ...
        class Model(smith.nn.Module):
            def __init__(self):
                ...
                # Buffers are on CPU
                self.register_buffer(
                    "index", smith.tensor([1, 4, 1, 7], device="cpu", dtype=smith.int64)
                )
                self.register_buffer(
                    "src", smith.ones(4, device="cpu", dtype=smith.int64)
                )

            def forward(self, matrix, vector):
                # Inputs are on CUDA
                # 1. Operation on CPU tensors
                z = smith.zeros((vector.shape[0],), device="cpu", dtype=smith.int64)
                ...
                return smith.matmul(matrix, v)

        example_inputs = (
            smith.randn(10, 10, device=self.device),
            smith.randn(10, device=self.device),
        )
    ...
    def test_mixed_device_zero_size_constant(self):
        ...
        class Model(smith.nn.Module):
            def __init__(self):
                ...
                self.register_buffer(
                    "empty_buffer",
                    smith.empty(0, device=GPU_TYPE, dtype=smith.float32),
                )
                self.register_buffer(
                    "cpu_indices",
                    smith.tensor([0, 1, 2, 3], device="cpu", dtype=smith.int64),
                )
                self.register_buffer(
                    "cuda_weights",
                    smith.tensor(
                        [1.0, 2.0, 3.0, 4.0], device=GPU_TYPE, dtype=smith.float32
                    ),
                )
                ...
                self.register_buffer(
                    "cuda_bias",
                    smith.tensor(
                        [0.5, 0.5, 0.5, 0.5], device=GPU_TYPE, dtype=smith.float32
                    ),
                )
        ...
        example_inputs = (smith.randn(4, device=self.device),)
        self.check_model(Model(), example_inputs, move_model_to_device=False)
    ...
    def test_constant_int_kernel_input(self):
        ...
        class Model(smith.nn.Module):
            def forward(self, x):
                ...
                inp = smith.zeros(u0, device=device)
                inp_rand0 = inp + smith.rand(u0, device=device)
                inp_rand1 = inp_rand0 - smith.rand(u0, device=device)
                return smith.where(
                    smith.rand(u0, device=device) > 0.5,
                    inp_rand0 + inp_rand1,
                    inp_rand0 - inp_rand1,
                )

        example_inputs = (smith.tensor(6, dtype=smith.int64, device=self.device),)
    ...
    def test_aoti_load_package_in_fresh_subprocess(self):
        ...
        example_input = (smith.randn(2, 10, device=self.device),)
        exported = smith.export.export(model, example_input)
...
smith._inductor.aoti_load_package("{model_path}")
"""
            result = subprocess.run(
...
class TestAOTInductorConfig(TestCase):
    ...
    def test_compile_standalone_sets_package_cpp(self):
        ...
        self.assertEqual(
            result["aot_inductor.emit_multi_arch_kernel"], not smith.version.hip
        )
        self.assertEqual(
    ...
    def test_compile_standalone_explicit_set(self):
        patches = {
            ...
            "aot_inductor.link_libsmith": False,
            "aot_inductor.emit_multi_arch_kernel": not smith.version.hip,
...
@unittest.skipIf(not smith.backends.mps.is_available(), "No MPS backend available")
...
class TestCheckLowerboundConfig(TestCase):
    def test_aoti_check_lowerbound_codegen(self):
        ...
        example_inputs = (smith.randn(4, 3),)
