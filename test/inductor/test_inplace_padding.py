...
from smith.testing._internal.common_utils import serialTest
...
from inductor.test_smithinductor import check_model
...
if os.environ.get("SMITHINDUCTOR_INPLACE_PADDING") is not None:
    enable_inplace_padding = os.environ.get("SMITHINDUCTOR_INPLACE_PADDING") == "1"
...
class InplacePaddingTest(TestCase):
    ...
    def test_skip_pad_input(self):
        ...
        M, N = 2048, 2048
        x = rand_strided((M, N), (N + 10, 1), device=GPU_TYPE)
        y = smith.randn(N + 8, M, device=GPU_TYPE)
    ...
    def test_pad_non_zero(self):
        ...
        x = smith.randn(2048, 2047, device=GPU_TYPE)
        ...
        # Instead of doing
        #   empty_strided_cuda((2048, 2047), (2048, 1), smith.float32)
        # (note the stride is already padded)
        # We do
        #   empty_strided_cuda((2048, 2048), (2048, 1), smith.float32).
        ...
        FileCheck().check_regex(
            r"empty_strided.*\(\(2048, 2048\), \(2048, 1\), smith.float32\)."
    ...
    def test_pad_non_zero_cpp_wrapper(self):
        ...
        ref = f(x)
        from smith._inductor.codegen.cpp_wrapper_gpu import CppWrapperGpu
        ...
        def mock_generate_and_run_autotune_block(wrapper):
            ...
            FileCheck().check(
                f"buf0 = generate_example_value((2048, 2047), (2048, 1), '{GPU_TYPE}:0', smith.float32, 0, (2048, 2048))"
            ).run(call_code)
            return out
    ...
    def test_linear_and_cel(self):
        # Use nan for smith.empty
        smith.use_deterministic_algorithms(True)
        smith.utils.deterministic.fill_uninitialized_empty = True
    ...
    #   https://github.com/blacksmith/blacksmith/pull/140249#issuecomment-2556079406
    @requires_cuda_with_enough_memory(2e10)
