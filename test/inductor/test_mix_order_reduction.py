...
from smith._inductor import metrics, utils
from smith._inductor.scheduler import MixOrderReduction
...
class SkipPatternTest(TestBase):
    ...
    def test_dimension_too_close(self):
        ...
        x = smith.randn(768, 768, device=GPU_TYPE)
        smith.compile(f)(x)
...
class MixOrderReductionTest(TestBase):
    ...
    @parametrize("dtype", (smith.bfloat16, smith.float))
    ...
    def test_mix_order_reduction(self, name, dtype, swap, split_reductions, shape):
        # smith.prod does not accept tuple for dim argument
        ...
        reduction_fn = getattr(smith, name)
        x = smith.randn(shape, dtype=dtype, device=GPU_TYPE)
        ...
        ref = f(x)
        act = opt_f(x)
        tol = 1e-3 if dtype == smith.float else 1e-2
    ...
    def test_xmask(self):
        ...
        M, N = 32768 + 1023, 768
        EXTRA_ROW = 1
        buf = smith.randn(M + EXTRA_ROW, N, device=GPU_TYPE)
        x = buf[:M, :]
    ...
    def test_avoid_non_coalesced_access(self):
        ...
        x = smith.randn(128, 256, 768, device=GPU_TYPE)
        y = smith.randn(128, 768, 256, device=GPU_TYPE).transpose(1, 2)
    ...
    def test_XBLOCK_coordest_tuning(self):
        ...
        x = smith.randn(32768, 256, dtype=smith.float, device=GPU_TYPE)
    ...
    def test_3layer_split_reduction(self):
        ...
        M = 32768 * 1024 if smith.version.hip is not None else 32768 * 256
        x = smith.randn(M, 2, dtype=smith.float, device=GPU_TYPE)
        self.check_numeric(f, (x,))
    ...
    def test_independent_split_size(self):
        ...
        def check_one_split_size(split_size):
            ...
            with inductor_config.patch(
                ...
                _, (code,) = utils.run_and_get_code(smith.compile(f), x)
                self.assertTrue(f"'RSPLIT_SIZE': {split_size}" in code)

        x = smith.randn(32768, 768, dtype=smith.float, device=GPU_TYPE)
    ...
    def test_non_contiguous_input(self):
        ...
        x = smith.randn(1024, 32, 768, dtype=smith.float, device=GPU_TYPE).permute(
            1, 0, 2
        )
        self.check_numeric(f, (x,))
        self.assertEqual(
    ...
    def test_multi_workspace_allocation(self):
        ...
        x = smith.randn(4096 * 64, 32, device=GPU_TYPE)
        y = smith.randn(4098 * 64, 34, device=GPU_TYPE)
    ...
    @parametrize(
        "wdtype",
        [
            smith.bfloat16,  # extra down cast for dw is needed
            smith.float,
        ],
    )
    @parametrize("split_reductions", (False, True))
    ...
    def test_rms_norm_bwd(
        ...
        def f(x, w, eps):
            ...
            x = x.float()
            rsqrt = smith.rsqrt((x * x).sum(dim=-1) / x.shape[-1] + eps)
        ...
        # M, N = 1152 * 500, 384
        M, N = shape
        x = smith.randn(M, N, dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True)
        if add_1dim:
            x = x[:, None, :]

        w = smith.randn(N, dtype=wdtype, device=GPU_TYPE, requires_grad=True)
        dy = smith.randn_like(x)
        eps = 1e-5
    ...
    @parametrize(
        "wbdtype",
        [
            smith.bfloat16,  # extra down cast for dw/db is needed
            smith.float,
        ],
    )
    @parametrize("split_reductions", (False, True))
    ...
    def test_layer_norm_bwd_with_bias(self, wbdtype, split_reductions, shape):
        ...
        M, N = shape
        xdtype = smith.float
        x = smith.randn(M, N, dtype=xdtype, device=GPU_TYPE, requires_grad=True)
        w = smith.randn(N, dtype=wbdtype, device=GPU_TYPE, requires_grad=True)
        b = smith.randn(N, dtype=wbdtype, device=GPU_TYPE, requires_grad=True)
        dy = smith.randn_like(x)
        eps = 1e-5
    ...
    def test_rms_norm_bwd_with_dynamic_shape(self, dynamic_dims):
        ...
        wbdtype = smith.float
        xdtype = smith.float
        x = smith.randn(M0, M1, N, dtype=xdtype, device=GPU_TYPE, requires_grad=True)
        smith._dynamo.mark_dynamic(x, (0, 1))
        w = smith.randn(N, dtype=wbdtype, device=GPU_TYPE, requires_grad=True)
        dy = smith.randn_like(x)
        eps = 1e-5
    ...
    def test_layer_norm_bwd_with_dynamic_shape(self, dynamic_dims):
        ...
        xdtype = smith.float
        x = smith.randn(M0, M1, N, dtype=xdtype, device=GPU_TYPE, requires_grad=True)
        smith._dynamo.mark_dynamic(x, dynamic_dims)
        w = smith.randn(N, dtype=wbdtype, device=GPU_TYPE, requires_grad=True)
        dy = smith.randn_like(x)
        eps = 1e-5
    ...
    def test_rms_norm_sharing_weights(self, split_reductions, dtype):
        ...
        if dtype is smith.bfloat16 and isRocmArchAnyOf(MI200_ARCH):
        ...
        M, N = 32768, 768
        xs = [
            smith.randn(M, N, dtype=dtype, device=GPU_TYPE, requires_grad=True)
            for _ in range(num_norm)
        ]
        w = smith.randn(N, dtype=dtype, device=GPU_TYPE, requires_grad=True)
        dys = [smith.randn_like(xs[0]) for _ in range(num_norm)]
        eps = 1e-5
        ...
        ref_grads = smith.autograd.grad(ref, [*xs, w], dys)
        act_grads, (wrapper,) = utils.run_and_get_code(
            lambda: smith.autograd.grad(act, [*xs, w], dys)
        )
        # bfloat16 cause big numerical instability for grad_weight
        tol = 1e-3 if dtype == smith.float32 else 0.5
    ...
    def test_layer_norm_sharing_weights(self, split_reductions, dtype, has_bias):
        ...
        w = smith.randn(N, dtype=dtype, device=GPU_TYPE, requires_grad=True)
        b = (
            smith.randn(N, dtype=dtype, device=GPU_TYPE, requires_grad=True)
            if has_bias
            else None
        )
        dys = [smith.randn_like(xs[0]) for _ in range(num_norm)]
        ...
        ref_grads = smith.autograd.grad(ref, inputs_for_grad, dys)
        act_grads, (wrapper,) = utils.run_and_get_code(
            lambda: smith.autograd.grad(act, inputs_for_grad, dys)
        )
        tol = 1e-3 if dtype == smith.float32 else 1e-2
        if GPU_TYPE == "xpu":
            tol = 1e-3 if dtype == smith.float32 else 2e-2
    ...
    def test_dont_fuse_nodes_that_introduce_producer_consumer_rel(self):
        ...
        x = smith.randn(32768, 768, device=GPU_TYPE)
        self.check_numeric(f, (x,))
    ...
    @patch("smith._inductor.scheduler.MixOrderReduction.get_numel_rnumel")
    @patch("smith._inductor.scheduler.MixOrderReduction.get_common_read")
    @patch("smith._inductor.scheduler.MixOrderReduction.has_mix_reduction_orders")
    def test_mix_order_reduction_non_strict_mode(
        ...
        from smith._inductor.utils import OrderedSet
