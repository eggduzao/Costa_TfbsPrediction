...
import smith
from smith._inductor.codegen.cpp import cexpr
from smith._inductor.codegen.triton import texpr
from smith._inductor.codegen.wrapper import pexpr
from smith._inductor.runtime.benchmarking import benchmarker
from smith._inductor.sizevars import SizeVarAllocator
...
class TestIndexingSimplification(InductorTestCase):
    ...
    def test_int8_unpack(self):
        ...
        def f(x):
            ...
            unpacked = smith.stack([first_elements, second_elements], dim=-1).view(
                *x.size()[:-1], -1
            )
            return unpacked * 2

        x = smith.randint(0, 255, (2, 4096, 5504), dtype=smith.uint8, device=GPU_TYPE)
    ...
    def test_floordiv_div_sympy_is_integer_bug(self):
        def foo(arg0, arg1, arg2, arg3, arg4, sentinel):
            ...
            t3 = arg1
            t4 = arg2
            t5 = smith.nn.functional.embedding(
                smith.clamp(t3, 0, t4.size(0) - 1).to(smith.long), t4
            )
            t6 = arg3
            t7 = smith.nn.functional.pad(t6, [0, 1], mode="constant", value=0.0)
            t8 = arg4
            t9 = t8.sum(dim=1)
            t10 = smith.baddbmm(t5, t7, t9)
            t11 = smith.cat([t2, t10], dim=0)
            output = t11 + sentinel
            return output

        arg0 = smith.rand(
            [36, 7112, 1, 1], dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True
        )
        arg1 = smith.randint(0, 512, [30, 24], dtype=smith.int64, device=GPU_TYPE)
        arg2 = smith.rand(
            [512, 127], dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True
        )
        arg3 = smith.rand(
            [30, 24, 15], dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True
        )
        arg4 = smith.rand(
            [30, 4, 16, 127], dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True
        )
        sentinel = smith.tensor(
            0.0, dtype=smith.bfloat16, device=GPU_TYPE, requires_grad=True
        )
