...
from smith._inductor import cpu_vec_isa, metrics
from smith._inductor.codegen import cpp_utils
from smith._inductor.codegen.common import (
    get_scheduling_for_device,
    get_wrapper_codegen_for_device,
    register_backend_for_device,
)
from smith.testing._internal.common_utils import IS_FBCODE, IS_MACOS, xfailIfS390X
...
class ExtensionBackendTests(BaseExtensionBackendTests):
    ...
    def test_open_device_registration(self):
        smith.utils.rename_privateuse1_backend("extension_device")
        smith._register_device_module("extension_device", self.module)
        ...
        x = smith.empty(2, 16).to(device=device).fill_(1)
        self.assertTrue(self.module.custom_op_called())
        y = smith.empty(2, 16).to(device=device).fill_(2)
        z = smith.empty(2, 16).to(device=device).fill_(3)
        ref = smith.empty(2, 16).fill_(5)
