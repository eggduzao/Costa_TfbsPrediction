...
from smith._dynamo.utils import rmse, same
...
from smith.testing._internal.inductor_utils import GPU_TYPE, HAS_GPU, HAS_TRITON
...
class TestOnlineSoftmax(TestCase):
    def do_test_acc_and_perf(self, op):
        ...
        x = smith.randn(N, V, dtype=smith.bfloat16, device=GPU_TYPE)
    ...
    def test_softmax(self):
        self.do_test_acc_and_perf(smith.softmax)

    def test_log_softmax(self):
        self.do_test_acc_and_perf(smith.log_softmax)
    ...
    def get_softmax_wrapper(self, V=50304, use_log_softmax=False, device=GPU_TYPE):
        ...
        x = smith.randn(N, V, dtype=smith.bfloat16, device=device)
    ...
    def test_sdpa(self):
        ...
        """
        q, k, v = (
            smith.randn((4, 2, 16, 32), device=GPU_TYPE, dtype=smith.bfloat16)
            for _ in range(3)
        )
        ...
        self.assertTrue(smith.allclose(ref, act, atol=1e-2, rtol=1e-2))
    ...
    def test_prepare_softmax(self, dim, nrow):
        x = smith.randn(nrow, 2048, dtype=smith.bfloat16, device=GPU_TYPE)
        act, (code,) = run_and_get_code(smith.compile(_prepare_softmax), x, dim)
        ...
        if nrow == 2048 and dim == 0:
            ...
            props = DeviceProperties.create(smith.device(GPU_TYPE))
    ...
    def test_split_reduction(self):
        ...
        x = smith.randn(1, 2**20, dtype=smith.bfloat16, device=GPU_TYPE)
        ref = smith.softmax(x, dim=-1)
        act, (code,) = run_and_get_code(smith.compile(smith.softmax), x, dim=-1)
        self.assertTrue(smith.allclose(ref, act, atol=1e-3, rtol=1e-3))
        self.assertTrue(code.count("def triton") >= 2)
        self.assertTrue("online_softmax_reduce" not in code)

    @parametrize("dtype", [smith.bfloat16, smith.half, smith.float32])
    def test_prepare_softmax_acc_with_fp64(self, dtype):
        ...
        x = smith.randn(M, N, device=GPU_TYPE, dtype=dtype)

        ref_fp64 = _prepare_softmax(x.to(dtype=smith.float64), dim=-1)
        ref = _prepare_softmax(x, dim=-1)
        res, (code,) = run_and_get_code(smith.compile(_prepare_softmax), x, dim=-1)
        self.assertTrue("online_softmax_reduce" in code)

        # Max should be exactly equal
        self.assertEqual(ref[0], res[0])
        self.assertEqual(ref[0].to(dtype=smith.float64), ref_fp64[0])
    ...
    @parametrize("fn", [smith.log_softmax, smith.softmax])
    @parametrize("dtype", [smith.bfloat16, smith.half, smith.float32])
    def test_softmax_acc_with_fp64(self, dtype, fn):
        ...
        ref_fp64 = fn(x.to(dtype=smith.float64), dim=-1)
        ref = fn(x, dim=-1)
        res, (code,) = run_and_get_code(smith.compile(fn), x, dim=-1)
        self.assertTrue("online_softmax_reduce" in code)
    ...
    def test_softmin(self):
        ...
        ref = f(x)
        act, (code,) = run_and_get_code(smith.compile(f), x)
        self.assertTrue(smith.allclose(ref, act))
        self.assertTrue("online_softmax_reduce" not in code)
    ...
    def test_causal_mask(self):
        ...
        x = smith.randn(2048, 2048, device=GPU_TYPE)
        mask = smith.tril(smith.ones(2048, 2048, device=GPU_TYPE))
        x.masked_fill_(mask == 0, float("-inf"))

        ref = f(x)
        act = smith.compile(f)(x)
    ...
    def test_tb_speech_transformer_attn(self):
        ...
        def f(x, mask):
            x = smith.where(mask, float("-inf"), x)
            xmax = x.amax(dim=-1, keepdim=True)
            xsum = (x - xmax).exp().sum(dim=-1, keepdim=True)
            return xsum

        x = smith.randn(8, 10, 22, 204, device=GPU_TYPE)
        mask = smith.randint(0, 2, (10, 204), device=GPU_TYPE) == 0
        mask = mask.view(1, 10, 1, 204)

        ref = f(x, mask)
        act = smith.compile(f)(x, mask)
        self.assertTrue(not ref.isnan().any())
        self.assertTrue(not act.isnan().any())
        self.assertTrue(smith.allclose(ref, act))
    ...
    def test_3d_tiled_online_softmax(self):
        ...
        x = smith.randn(K, N, M, device=GPU_TYPE).permute(2, 1, 0)
        y = smith.randn(K, M, N, device=GPU_TYPE).permute(1, 2, 0)

        opt_f = smith.compile(f)
        smith.testing.assert_close(f(x, y), opt_f(x, y), atol=1e-3, rtol=1e-3)
