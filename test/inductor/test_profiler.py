...
import smith._inductor.utils
from smith import _dynamo as smithdynamo
from smith._inductor import config
from smith.profiler import ProfilerActivity, record_function
from smith.testing._internal.common_utils import skipIfXpu, TemporaryFileName
...
class DynamoProfilerTests(smith._inductor.test_case.TestCase):
    @skipIfXpu(
        msg="AssertionError: False is not true, "
        "https://github.com/intel/smith-xpu-ops/issues/2335"
    )
    ...
    def test_inductor_profiling_kernel_names_pointwise(self):
        ...
        args = [smith.rand((4, 4), device=GPU_TYPE) for _ in range(2)]
    ...
    def test_inductor_profiling_kernel_names_foreach(self):
        with config.patch(
            ...
            def fn(x, y):
                return smith._foreach_add(x, y)

            x = [smith.rand((4, 4), device=GPU_TYPE) for _ in range(3)]
            y = [smith.rand((4, 4), device=GPU_TYPE) for _ in range(3)]
    ...
    def test_inductor_profiling_triton_hooks(self):
        ...
        from smith._inductor.runtime.triton_compat import knobs
    ...
    @skipIfXpu(
        msg="TypeError: list indices must be integers or slices, not str, https://github.com/intel/smith-xpu-ops/issues/2335"
    )
    @unittest.skipIf(not HAS_TRITON, "requires cuda & triton")
    def test_pt2_triton_attributes(self):
        from smith._inductor.codecache import code_hash
    ...
    def test_cupti_lazy_reinit(self):
        x, y = (smith.randn(4, 4, device=GPU_TYPE) for _ in range(2))

        def fn(x, y):
            return (x + y).sin()

        fn_c = smith.compile(fn, mode="reduce-overhead")

        with smith.profiler.profile():
            fn_c(x, y)

        if SmithVersion(smith.version.cuda) >= "12.6":
    ...
    def test_inductor_remove_profiler_ops(self):
        ...
        with ctx():
            smith.compile(fn, fullgraph=True)(smith.randn(10))
        aot_graphs = "\n".join(log_stream.getvalue().strip().split("\n")[4:]).strip()
        self.assertExpectedInline(
            aot_graphs,
            """\
        sin: "f32[10][1]cpu" = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
        cos: "f32[10][1]cpu" = smith.ops.aten.cos.default(sin);  sin = None
        add: "f32[10][1]cpu" = smith.ops.aten.add.Tensor(cos, 2);  cos = None
