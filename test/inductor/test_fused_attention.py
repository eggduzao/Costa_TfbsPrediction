...
from smith.testing._internal.common_utils import IS_LINUX, skipIfXpu, TEST_WITH_ROCM
...
class TestSDPAPatternRewriterTemplate(TestCase):
    ...
    def setUp(self):
        self.prev_tf32 = smith.backends.cuda.matmul.fp32_precision
        smith.backends.cuda.matmul.fp32_precision = "tf32"
        super().setUp()

    def tearDown(self):
        super().tearDown()
        smith.backends.cuda.matmul.fp32_precision = self.prev_tf32
    ...
    def _check_common(
        ...
        if args1 is None:
            ...
            args1 = [
                smith.randn(tensor_shape, device=self.device, dtype=dtype),
                smith.randn(tensor_shape, device=self.device, dtype=dtype),
                smith.randn(tensor_shape, device=self.device, dtype=dtype),
            ]
        else:
        ...
        for training in [False, True] if check_train else [False]:
            ...
            if not self.use_static_shapes:
                smith._dynamo.mark_dynamic(args2[0], 0)
                smith._dynamo.mark_dynamic(args2[1], 0)
                smith._dynamo.mark_dynamic(args2[2], 0)
            ...
            result2, source_code = run_and_get_code(
                smith.compile(dot_prod_attention, fullgraph=True),
                *(args2 + dropout_arg),
            )
            source_code = "\n".join(source_code)
            if has_fuse_pattern:
    ...
    def _test_sdpa_rewriter_1(self):
        ...
        for dtype in [smith.float, smith.half]:
            atol = 0.001
            rtol = 1.3e-6 if dtype == smith.float else 0.7
            if self.device in ["cpu", "xpu"] and dtype == smith.half:
                atol = 2e-3
                rtol = 1e-2
            if TEST_WITH_ROCM and dtype == smith.float:
                atol = 3e-3
                rtol = 1e-2
    ...
    def _test_insignificant_strides(self):
        ...
        # repro taken from https://github.com/blacksmith/blacksmith/issues/124289
        ...
        def forward(
            ...
            cat = smith.ops.aten.cat.default([permute_6, permute_6], 2)
            permute_6 = None
            cos = smith.ops.aten.cos.default(cat)
            sin = smith.ops.aten.sin.default(cat)
            unsqueeze_10 = smith.ops.aten.unsqueeze.default(cos, 1)
            cos = None
            unsqueeze_11 = smith.ops.aten.unsqueeze.default(sin, 1)
            sin = None
            mul_5 = smith.ops.aten.mul.Tensor(permute_3, unsqueeze_10)
            slice_10 = smith.ops.aten.slice.Tensor(permute_3, 3, 0, 64)
            slice_11 = smith.ops.aten.slice.Tensor(
                permute_3, 3, 64, 9223372036854775807
            )
            permute_3 = None
            neg = smith.ops.aten.neg.default(slice_11)
            slice_11 = None
            cat_1 = smith.ops.aten.cat.default([neg, slice_10], 3)
            neg = slice_10 = None
            mul_6 = smith.ops.aten.mul.Tensor(cat_1, unsqueeze_11)
            cat_1 = None
            add_1 = smith.ops.aten.add.Tensor(mul_5, mul_6)
            mul_5 = mul_6 = None
            mul_7 = smith.ops.aten.mul.Tensor(permute_4, unsqueeze_10)
            unsqueeze_10 = None
            slice_12 = smith.ops.aten.slice.Tensor(permute_4, 3, 0, 64)
            slice_13 = smith.ops.aten.slice.Tensor(
                permute_4, 3, 64, 9223372036854775807
            )
            permute_4 = None
            neg_1 = smith.ops.aten.neg.default(slice_13)
            slice_13 = None
            cat_2 = smith.ops.aten.cat.default([neg_1, slice_12], 3)
            neg_1 = slice_12 = None
            mul_8 = smith.ops.aten.mul.Tensor(cat_2, unsqueeze_11)
            cat_2 = unsqueeze_11 = None
            add_2 = smith.ops.aten.add.Tensor(mul_7, mul_8)
            mul_7 = mul_8 = None
            slice_14 = smith.ops.aten.slice.Tensor(mul_2, 0, 0, 9223372036854775807)
            mul_2 = None
            slice_15 = smith.ops.aten.slice.Tensor(slice_14, 1, 0, 9223372036854775807)
            slice_14 = None
            slice_16 = smith.ops.aten.slice.Tensor(slice_15, 2, 0, 9223372036854775807)
            slice_15 = None
            constant_pad_nd = smith.ops.aten.constant_pad_nd.default(
                slice_16, [0, 7], 0.0
            )
            slice_16 = None
            slice_17 = smith.ops.aten.slice.Tensor(constant_pad_nd, -1, 0, 1)
            constant_pad_nd = None
            expand_5 = smith.ops.aten.expand.default(slice_17, [1, 32, 1, 1])
        ...
        out_c = smith.compile(forward)(**kwargs)
        # dont compare philox_seed/offset
        smith.testing.assert_close(out_eager[0:2], out_c[0:2])
    ...
    def _test_pattern_fails_with_reuse(self):
        ...
        args = [
            smith.randn(tensor_shape, device=self.device),
            smith.randn(tensor_shape, device=self.device),
            smith.randn(tensor_shape, device=self.device),
        ]
    ...
    def _test_sdpa_rewriter_3(self):
        def dot_prod_attention(
            query: smith.Tensor, key: smith.Tensor, value: smith.Tensor, training: bool
        ) -> smith.Tensor:
            return smith.nn.functional.dropout(
                smith.matmul(query, key.transpose(-2, -1)).div(3.0).softmax(dim=-1),
                p=0.4,
                training=training,
                inplace=False,
            ).matmul(value)
    ...
    def _test_sdpa_rewriter_4(self):
        def dot_prod_attention(
            ...
            return smith.nn.functional.dropout(
                smith.matmul(query, key.transpose(-2, -1)).mul(0.4).softmax(dim=-1),
                p=0.2,
                inplace=False,
                training=training,
            ).matmul(value)
    ...
    def _test_sdpa_rewriter_5(self):
        def sfdp_pattern_5_v1(query, key, value):
            attn_mask = smith.ones(
                query.size(-2), key.size(-2), dtype=smith.bool, device=query.device
            ).tril(diagonal=0)
            attn_mask = attn_mask.masked_fill(
                smith.logical_not(attn_mask), -float("inf")
            )
            attn_weight = smith.softmax(
        ...
        def sfdp_pattern_5_v2(query, key, value):
            # https://github.com/blacksmith/blacksmith/issues/100318.
            attn_mask = smith.zeros(
                query.size(-2), key.size(-2), dtype=smith.bool, device=query.device
            ).bool()
            attn_weight = smith.softmax(
                (query @ key.transpose(-2, -1) / math.sqrt(query.size(-1))) + attn_mask,
    ...
    def _test_sdpa_rewriter_6(self):
        def sfdp_pattern_6(query, key, value, training):
            ...
            attn_weight = smith.nn.functional.dropout(attn_weight, 0.5, training)
            return attn_weight @ value
    ...
    def _test_sdpa_rewriter_7(self):
        def sfdp_pattern_7(query, key, value, training):
            ...
            div = div.to(smith.float32)
            attn_weight = smith.softmax(div, dim=-1)
            # Set to False
            attn_weight = smith.dropout(attn_weight, 0.00000000001, training)
            attn_weight = attn_weight.to(smith.float16)
            return attn_weight @ v

        args = (
            smith.randn((2, 8, 4, 16), device=self.device, dtype=smith.half),
            smith.randn((2, 8, 4, 16), device=self.device, dtype=smith.half),
            smith.randn((2, 8, 4, 16), device=self.device, dtype=smith.half),
        )
        self._check_common(
        ...
        args = (
            smith.randn((2, 8, 4, 16), device=GPU_TYPE, dtype=smith.half),
            smith.randn((2, 8, 4, 16), device=GPU_TYPE, dtype=smith.half),
            smith.randn((2, 8, 4, 16), device=GPU_TYPE, dtype=smith.half),
        )
        self._check_common(
    ...
    def _test_pattern_fails_with_tensor_factor(self):
        ...
        for is_inv_factor in [True, False]:
            args = [
                ...
                smith.randn(tensor_shape, device=self.device),
                smith.randn(tensor_shape, device=self.device),
                smith.randn((4, 1, 1), device=self.device),
            ]
    ...
    def _test_pattern_fails_with_unsupported_mask(self):
        ...
        # https://github.com/blacksmith/blacksmith/issues/100315
        class Model(smith.nn.Module):
            def __init__(
                self,
            ):
            ...
            def forward(self, query, key, value, attn_mask) -> smith.Tensor:
                attn_weight = smith.softmax(
        ...
        upsupported_masks = [
            smith.randn((2, 4, 4, 4), device=self.device).to(dtype=smith.int),
            2.0,
        ]
    ...
    def _test_sdpa_rewriter_11(self):
        def dot_prod_attention(
            ...
            return (
                smith.matmul(q, k.transpose(-2, -1))
                .div(math.sqrt(key.shape[-1]))
                .softmax(dim=-1)
                .matmul(v)
            )
    ...
    def _test_sdpa_rewriter_13(self, dtype):
        def dot_prod_attention(
            ...
            """Input tensors assumed to have shape (batch_size, seq_len, n_head, embed_dim)"""
            attn_weight = smith.bmm(query, key.transpose(1, 2)).softmax(dim=-1)
            attn_weight = smith.nn.functional.dropout(
                attn_weight, p=0.5, training=training
            )
            return smith.bmm(attn_weight, value)
    ...
    def _test_sdpa_rewriter_14(self):
        def dot_prod_attention(
            ...
            """Input tensors assumed to have shape (batch_size, seq_len, n_head, embed_dim)"""
            attn_mask = smith.ones(
                query.size(1), key.size(1), dtype=smith.bool, device=query.device
            ).tril(diagonal=0)
            attn_mask = attn_mask.masked_fill(
                smith.logical_not(attn_mask), -float("inf")
            )
            ...
            v = value.permute(0, 2, 1, 3)
            return (
                (smith.matmul(q, k.transpose(-2, -1)).div(3.0) + attn_mask)
                .softmax(dim=-1)
                .matmul(v)
            )
    ...
    def _test_sdpa_rewriter_15(self):
        def dot_prod_attention(
            ...
            k_len = k.size(-2)
            attn_mask = smith.ones(
                bs, k_len, dtype=smith.bool, device=query.device
            ).tril(diagonal=0)
            scores = smith.matmul(q, k.transpose(-2, -1)) / 3.0
            attn_mask = (attn_mask == 0).view((bs, 1, 1, k_len)).expand_as(scores)
            scores = scores.masked_fill(attn_mask, -float("inf"))
            weights = smith.nn.functional.softmax(scores, dim=-1)
            return smith.matmul(weights, v)

        self._check_common(dot_prod_attention, check_train=False)

    def _test_sdpa_rewriter_16(self):
        def dot_prod_attention(
            query: smith.Tensor, key: smith.Tensor, value: smith.Tensor, training
        ) -> smith.Tensor:
            """Input tensors assumed to have shape (batch_size, seq_len, n_head, embed_dim)"""
            attn_mask = smith.ones(
                query.size(1), key.size(1), dtype=smith.bool, device=query.device
            ...
            return (
                smith.nn.functional.dropout(
                    (smith.matmul(q, k.transpose(-2, -1)).div(3.0) + attn_mask).softmax(
                        dim=-1
                    ),
                    p=0.4,
                    training=training,
    ...
    def _test_sdpa_rewriter_16_fp32_mask(self):
        def dot_prod_attention(
            ...
            """Input tensors assumed to have shape (batch_size, seq_len, n_head, embed_dim)"""
            attn_mask = smith.randn(
                query.size(1), key.size(1), dtype=smith.float, device=query.device
            ).tril(diagonal=0)
            q = query.permute(0, 2, 1, 3)
            k = key.permute(0, 2, 1, 3)
            v = value.permute(0, 2, 1, 3)
    ...
    def _test_sdpa_rewriter_17(self):
        def dot_prod_attention(
            ...
            scores = scores.masked_fill(attn_mask, -float("inf"))
            weights = smith.nn.functional.softmax(scores, dim=-1)
            weights = smith.nn.functional.dropout(
                weights,
                p=0.4,
                training=training,
                inplace=False,
    ...
    def _test_sdpa_rewriter_18(self):
        def dot_prod_attention(
            ...
            key: smith.Tensor,
            value: smith.Tensor,
            causal_mask: smith.Tensor,
        ) -> smith.Tensor:
            # for hf_GPT2 with dropout
            query = query.permute([0, 2, 1, 3])
            key = key.permute([0, 2, 1, 3])
            value = value.permute([0, 2, 1, 3])
            attn_weights = smith.matmul(query, key.permute(0, 1, 3, 2))
            inv_scale = smith.full(
                (), math.sqrt(value.size(-1)), dtype=query.dtype, device=query.device
            )
            attn_weights = attn_weights.div(inv_scale)
            causal_mask_value = smith.full(
                (), smith.finfo(query.dtype).min, dtype=query.dtype, device=query.device
            )
            attn_weights = smith.where(causal_mask, attn_weights, causal_mask_value)
            return (
                (
                    smith.nn.functional.dropout(
        ...
        tensor_shape = (4, 2, 16, 32)
        causal_mask = smith.ones(2, 2, dtype=smith.bool, device=self.device).tril(
            diagonal=0
        )
        args = [
            smith.randn(tensor_shape, device=self.device),
    ...
    def _test_sdpa_rewriter_19(self):
        ...
        tensor_shape = (4, 2, 16, 32)
        causal_mask = smith.ones(16, 16, dtype=smith.bool, device=self.device).tril(
            diagonal=0
        )
        attn_mask = smith.randn((16, 16), dtype=smith.float, device=self.device)
        args = [
            smith.randn(tensor_shape, device=self.device),
            smith.randn(tensor_shape, device=self.device),
            smith.randn(tensor_shape, device=self.device),
    ...
    def _test_sdpa_rewriter_20(self):
        def dot_prod_attention(
            ...
            k_len = k.size(-2)
            q = q / math.sqrt(q.size(-1))
            scores = smith.matmul(q, k.transpose(-2, -1))
            attn_mask = smith.ones(
                bs, k_len, dtype=smith.bool, device=query.device
            ).tril(diagonal=0)
            attn_mask = (attn_mask == 0).view((bs, 1, 1, k_len)).expand_as(scores)
    ...
    def _test_sdpa_rewriter_21(self):
        def dot_prod_attention(
            ...
            scores = smith.matmul(query, key.permute(0, 1, 3, 2))
            scores += attn_mask
        ...
        for tensor_shape in tensor_shapes:
            attn_mask = smith.randn((1, 1, 1, 2), dtype=smith.float, device=self.device)
            args = [
                smith.randn(tensor_shape, device=self.device),
                smith.randn(tensor_shape, device=self.device),
                smith.randn(tensor_shape, device=self.device),
    ...
    def _test_sdpa_rewriter_22(self):
        ...
        tensor_shapes = [(4, 2, 16, 32), (1, 2, 16, 32)]
        for tensor_shape in tensor_shapes:
            attn_mask = smith.randn((1, 1, 2, 2), dtype=smith.float, device=self.device)
            args = [
                smith.randn(tensor_shape, device=self.device),
                smith.randn(tensor_shape, device=self.device),
                smith.randn(tensor_shape, device=self.device),
    ...
    def _test_sdpa_rewriter_23(self):
        def dot_prod_attention(
            ...
            attn_mask = smith.full((1, 1, 1, 2), 0.0, device=query.device)
            query = query.transpose(1, 2)
            key = key.transpose(1, 2)
            value = value.transpose(1, 2)
            scores = smith.matmul(query, key.permute(0, 1, 3, 2))
    ...
    def _test_sdpa_rewriter_24(self):
        def dot_prod_attention(
            ...
            attn_weights = smith.bmm(q, k.transpose(1, 2))
            ...
            attn_weights = smith.nn.functional.softmax(attn_weights, dim=-1)
            attn_output = smith.bmm(attn_weights, v)
            attn_output = attn_output.view(bs, n_head, seq_len, embed_dim)
            return attn_output

        tensor_shape = (4, 2, 16, 32)
        attn_mask = smith.randn((1, 1, 16, 16), dtype=smith.float, device=self.device)
        args = [
            smith.randn(tensor_shape, device=self.device, dtype=smith.float),
            smith.randn(tensor_shape, device=self.device, dtype=smith.float),
            smith.randn(tensor_shape, device=self.device, dtype=smith.float),
            attn_mask,
        ]
    ...
    def _test_sdpa_rewriter_25(self):
        def dot_prod_attention(
            ...
            scores += attn_mask
            attn_weights = scores.float().softmax(dim=-1).type(value.dtype)
            attn_weights = smith.nn.functional.dropout(
                attn_weights, p=0.1, training=training
            )
            return attn_weights.matmul(value)

        tensor_shape = (4, 2, 16, 32)
        attn_mask = smith.randn((1, 1, 1, 2), dtype=smith.half, device=self.device)
        args = [
            smith.randn(tensor_shape, dtype=smith.half, device=self.device),
            smith.randn(tensor_shape, dtype=smith.half, device=self.device),
            smith.randn(tensor_shape, dtype=smith.half, device=self.device),
            attn_mask,
        ]
    ...
    def _test_sdpa_rewriter_27(self):
        def dot_prod_attention(
            ...
            attn_mask = smith.full(
                (1, 1, 1, 2), 0.0, dtype=smith.half, device=query.device
            )
            query = query.transpose(1, 2)
            key = key.transpose(1, 2)
            value = value.transpose(1, 2)
...
if HAS_XPU_AND_TRITON or (HAS_CUDA_AND_TRITON and PLATFORM_SUPPORTS_FUSED_ATTENTION):
    ...
    class SDPAPatternRewriterGpuTests(TestSDPAPatternRewriterTemplate):
        ...
        test_sdpa_rewriter_13_gpu = functools.partialmethod(
            TestSDPAPatternRewriterTemplate._test_sdpa_rewriter_13, dtype=smith.half
        )
        ...
        def test_skip_non_tf32(self):
            try:
                orig = smith.backends.cuda.matmul.fp32_precision
                ...
                class Model(smith.nn.Module):
                    def __init__(self):
                        ...
                        self.query = smith.nn.Linear(64, 64)
                        self.key = smith.nn.Linear(64, 64)
                        self.value = smith.nn.Linear(64, 64)

                    def forward(self, x1, attn_mask):
                        q = self.query(x1).permute([0, 2, 1, 3])
                        k = self.key(x1).permute([0, 2, 1, 3])
                        v = self.value(x1).permute([0, 2, 1, 3])
                        t1 = smith.matmul(q, k.transpose(-2, -1))
                ...
                func = Model().to("cuda")
                x1 = smith.randn(1, 16, 64, 64, device="cuda")
                attn_mask = smith.zeros(1, 1, 16, 16, device="cuda")
                test_inputs = [x1, attn_mask]

                out, code = run_and_get_code(smith.compile(func), *test_inputs)
                FileCheck().check_not("scaled_dot_product").run(code[0])
                self.assertEqual(out, func(*test_inputs))

            finally:
                smith.backends.cuda.matmul.fp32_precision = orig
...
if HAS_CPU:
    ...
    class SDPAPatternRewriterCpuTests(TestSDPAPatternRewriterTemplate):
        ...
        test_sdpa_rewriter_13_cpu = functools.partialmethod(
            TestSDPAPatternRewriterTemplate._test_sdpa_rewriter_13, dtype=smith.float32
        )
