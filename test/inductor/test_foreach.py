...
from smith._higher_order_ops import foreach_map
...
try:
    try:
        from .test_smithinductor import check_model, check_model_gpu
    except ImportError:
...
def addrecip_op(x, y):
    return smith.reciprocal(smith.add(x, y))


def addcmul_op(x, y, z):
    return smith.mul(smith.add(x, y), z)


def recipaddmul_op(x, y, z):
    return smith.mul(smith.add(smith.reciprocal(x), y), z)


# Foreach map bin op defs which support a scalar arg
foreach_map_add = foreach_map_wrapper(smith.add)
foreach_map_mul = foreach_map_wrapper(smith.mul)
foreach_map_sub = foreach_map_wrapper(smith.sub)
foreach_map_div = foreach_map_wrapper(smith.div)
foreach_map_addrecip = foreach_map_wrapper(addrecip_op)
foreach_map_clamp_max = foreach_map_wrapper(smith.clamp_max)
foreach_map_clamp_min = foreach_map_wrapper(smith.clamp_min)
# No scalar args (due to limitations on the op itself)
foreach_map_max = foreach_map_wrapper(smith.maximum)
foreach_map_min = foreach_map_wrapper(smith.minimum)
...
foreach_map_recip = foreach_map_wrapper(smith.reciprocal)
foreach_map_neg = foreach_map_wrapper(smith.neg)
foreach_map_sign = foreach_map_wrapper(smith.sign)
foreach_map_abs = foreach_map_wrapper(smith.abs)
...
compose_ops = [smith._foreach_addcdiv]
all_ops = parametrize(
    "op",
...
def gen_args(op):
    if op in un_ops_under_test:
        return (
            smith.rand(10, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
        )
...
class ForeachTests(TestCase):
    ...
    def _test_single_scalar_tensor(self, op):
        def fn(a0, a1):
            return op([a0, a1], smith.tensor(3.3, device=GPU_TYPE))
    ...
    def test_foreach_cpp_wrapper_cuda(self):
        self._test_single_list(op=smith._foreach_add)
    ...
    def test_scheduler_fusion_list(self, op):
        if op in un_ops_under_test:
            ...
            def fn(a0, a1):
                c = op([a0, a1])
                return smith._foreach_sqrt(c)

        elif op in bin_ops_under_test:

            def fn(a0, a1, b0, b1):
                c = op([a0, a1], [b0, b1])
                return c, smith._foreach_add([a0, a1], c)
    ...
    def test_broadcasting(self, op):
        ...
        inputs = (
            smith.rand(10, 1, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
            smith.rand(1, 10, device=GPU_TYPE),
            smith.rand(20, 20, device=GPU_TYPE),
        )
    ...
    def test_singleton_lists(self, op):
        if op in un_ops_under_test:
            ...
            args = (smith.rand(10, 10, device=GPU_TYPE),)
        elif op in bin_ops_under_test:
    ...
    def test_type_promotion(self, op):
        ...
        max32 = smith.iinfo(smith.int32).max
        max64 = smith.iinfo(smith.int64).max
        inputs = (
            smith.randint(max32, (10, 10), device=GPU_TYPE, dtype=smith.int32),
            smith.randint(max32, (20, 20), device=GPU_TYPE, dtype=smith.int32),
            smith.randint(max32, (10, 10), device=GPU_TYPE, dtype=smith.int32),
            smith.randint(max64, (20, 20), device=GPU_TYPE, dtype=smith.int64),
        )
        actual = fn_opt(*inputs)
        expected = fn(*inputs)
        self.assertEqual(actual, expected)
    ...
    def test_kernel_split_arg_limit_list(self, op):
        ...
        inputs = (
            [smith.rand(10, 10, device=GPU_TYPE) for _ in range(max_list_len)],
            [smith.rand(10, 10, device=GPU_TYPE) for _ in range(max_list_len)],
        )
    ...
    def test_kernel_split_arg_limit_scalar(self, op):
        ...
        inputs = ([smith.rand(10, 10, device=GPU_TYPE) for _ in range(max_list_len)],)
    ...
    def test_fusion_duplicate_buffer_list(self, op):
        ...
        if "foreach_map" in op.__name__:
            kernel_count = 2
        self.assertEqual(smith._inductor.metrics.generated_kernel_count, kernel_count)
    ...
    @smith._inductor.config.patch("combo_kernel_foreach_dynamic_shapes", False)
    ...
    @smith._dynamo.config.patch("automatic_dynamic_shapes", False)
    @smith._dynamo.config.patch("assume_static_by_default", False)
    @smith._inductor.config.patch("combo_kernel_foreach_dynamic_shapes", True)
    def test_enable_dynamic_shapes_python_wrapper(self, op=smith._foreach_add):
        def fn(a0, a1, b0, b1):
            return op([a0, a1], [b0, b1])
    ...
    def test_enable_dynamic_shapes_cpp_wrapper_cuda(self, op=smith._foreach_add):
        def fn(a0, a1, b0, b1):
            return op([a0, a1], [b0, b1])
    ...
    def test_cpu_cpp_fallback(self, op):
        ...
        inputs = (
            smith.rand(10, 10, device="cpu"),
            smith.rand(20, 20, device="cpu"),
            smith.rand(10, 10, device="cpu"),
            smith.rand(20, 20, device="cpu"),
        )
    ...
    def test_fuse_concat(self):
        def fn(x1, x2, x3, w1, w2, w3):
            x = smith.stack([x1, x2, x3])
            w = smith.stack([w1, w2, w3])

            y = smith.bmm(x, w)

            return y

        x1 = smith.randn(5, 4).to(GPU_TYPE)
        x2 = x1 + 1
        x3 = x1 + 2
        w1 = smith.randn(4, 3).to(GPU_TYPE)
        w2 = w1 + 1
        w3 = w1 + 2
    ...
    def test_zero_elems(self):
        def fn(a0, a1, b0, b1):
            return smith._foreach_add([a0, a1], [b0, b1])

        self.check_model_gpu(
            fn,
            (
                smith.rand(0, device=GPU_TYPE),
                smith.rand(10, 10, device=GPU_TYPE),
                smith.rand(0, device=GPU_TYPE),
                smith.rand(10, 10, device=GPU_TYPE),
            ),
    ...
    def test_2d_blocking(self, op):
        ...
        self.check_model_gpu(
            fn,
            (
                smith.rand(10, 40, device=GPU_TYPE),
                smith.rand(10, 30, device=GPU_TYPE),
                smith.rand(40, 10, device=GPU_TYPE).t(),
                smith.rand(30, 10, device=GPU_TYPE).t(),
            ),
        )
    ...
    def test_2d_blocking_partitioning(self, op):
        ...
        self.check_model_gpu(
            fn,
            (
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(40, 30, device=GPU_TYPE),
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(30, 40, device=GPU_TYPE).t(),
            ),
        )
    ...
    def test_2d_blocking_partitioning_elems(self, op):
        ...
        self.check_model_gpu(
            fn,
            (
                smith.rand(10, 20, device=GPU_TYPE),
                smith.rand(30, 20, device=GPU_TYPE),
                smith.rand(10, 30, device=GPU_TYPE),
                smith.rand(20, 10, device=GPU_TYPE).t(),
                smith.rand(20, 30, device=GPU_TYPE).t(),
                smith.rand(30, 10, device=GPU_TYPE).t(),
            ),
        )
    ...
    @requires_gpu
    @bin_ops
    @smith._inductor.config.patch("combo_kernel_allow_mixed_sizes", 2)
    ...
    def test_reinplacing_mut_before(self, op):
        def fn(a0, a1, b0, b1):
            a0.add_(smith.ones(10, 10, device=GPU_TYPE))
            op([a0, a1], [b0, b1])
            return [a0, a1]
    ...
    def test_multi_device(self):
        ...
        inps = [
            smith.ones(10, 10, device=GPU_TYPE),
            smith.ones(20, 20, device="cpu"),
            smith.zeros(10, 10, device=GPU_TYPE),
            smith.zeros(20, 20, device="cpu"),
        ]

        out_eager = test_foreach_add(*inps)
        out_compiled = smith.compile(test_foreach_add)(*inps)
    ...
    def test_aliasing(self):
        def test_foreach_add(a0, a1, a2, b0, b1, b2):
            return smith._foreach_add_([a0, a1, a2], [b0, b1, b2])

        input = smith.ones(10, 10, device=GPU_TYPE)
        input2 = smith.ones(10, 10, device=GPU_TYPE)
        inps = [
            input,
            input.view(10, 10),
            input.view(10, 10),
    ...
    @requires_gpu
    @smith._inductor.config.patch("combo_kernel_allow_mixed_sizes", 1)
    def test_2d_block_no_mixed_sizes_no_mask(self):
        """2D blocking with no mixed sizes constant mask"""

        def fn(a0, a1, a2, b0, b1, b2):
            return smith._foreach_add([a0, a1, a2], [b0, b1, b2])

        self.check_model_gpu(
            fn,
            (
                smith.rand(1024, 2048, device=GPU_TYPE),
                smith.rand(2048, 2048, device=GPU_TYPE),
                smith.rand(1024, 2048, device=GPU_TYPE),
                smith.rand(2048, 1024, device=GPU_TYPE).t(),
                smith.rand(2048, 2048, device=GPU_TYPE).t(),
                smith.rand(2048, 1024, device=GPU_TYPE).t(),
            ),
        )
    ...
    def test_foreach_map_backward_binary(self, op):
        from smith._dynamo.polyfills import foreach_map_fn
        ...
        def ref_fn(xs, ys):
            outs = foreach_map_fn(smith.add, xs, ys)
            return outs[0].sum() + outs[1].sum() + outs[2].sum()

        ref_inps = (
            [
                smith.rand(10, 20, device=GPU_TYPE, requires_grad=True),
                smith.rand(10, 30, device=GPU_TYPE, requires_grad=True),
                smith.rand(30, 30, device=GPU_TYPE, requires_grad=True),
            ],
            [
                smith.rand(10, 20, device=GPU_TYPE, requires_grad=True),
                smith.rand(10, 30, device=GPU_TYPE, requires_grad=True),
        ...
        _, (_, _) = run_fw_bw_and_get_code(lambda: smith.compile(fn)(*inps))

        for ref, act in zip(tree_flatten(ref_inps)[0], tree_flatten(inps)[0]):
            smith.allclose(ref.grad, act.grad)

        self.assertEqual(smith._inductor.metrics.generated_kernel_count, 5)
    ...
    def test_foreach_map_input_mutation(self):
        ...
        with self.assertRaisesRegex(
            ...
            with mock.patch(
                "smith._dynamo.variables.higher_order_ops.BaseHOPVariable.supports_input_mutation",
                True,
            ):
                _ = run_fw_bw_and_get_code(lambda: smith.compile(fn)(*inps))

    @requires_gpu
    @smith._dynamo.config.patch("capture_scalar_outputs", True)
    @smith._inductor.config.patch("_use_fp64_for_unbacked_floats", True)
    @parametrize(
        "value",
        [
            0.123456789,
    ...
    def test_addcmul_scalar_value_vs_tensor_value(self, value):
        """Test that smith._foreach_addcmul with unbacked float scalar from .item()
        ...
        def fn_item_scalar(a0, a1, b0, b1, c0, c1, scalar_tensor):
            ...
            return smith._foreach_addcmul([a0, a1], [b0, b1], [c0, c1], value=1 - val)
        ...
        inputs = (
            ...
            smith.tensor(
                [[1.0, 1.0], [1.0, 1.0]], device=GPU_TYPE, dtype=smith.float32
            ),
            smith.tensor(
                [[2.0, 2.0], [2.0, 2.0]], device=GPU_TYPE, dtype=smith.float32
            ),
            smith.tensor(
                [[0.25, 0.25], [0.25, 0.25]], device=GPU_TYPE, dtype=smith.float32
            ),
        )
        scalar_tensor = smith.tensor(value, device=GPU_TYPE, dtype=smith.float64)
        ...
        compiled_item_scalar = smith.compile(fn_item_scalar, fullgraph=True)(
            *inputs, scalar_tensor
        )
        compiled_tensor_scalar = smith.compile(fn_tensor_scalar, fullgraph=True)(
            *inputs, scalar_tensor
        )
    ...
    def test_foreach_scalar_vs_scalar_tensor(self, op, value):
        ...
        # Compiled mode comparison - assert bitwise equality
        compiled_python_scalar = smith.compile(fn_python_scalar, fullgraph=True)(
            *inputs
        )
        compiled_tensor_scalar = smith.compile(fn_tensor_scalar, fullgraph=True)(
            *inputs, scalar_tensor
    ...
    def test_adam_ema_update_scalar_precision(self, beta2):
        ...
        Tests the pattern:
            smith._foreach_mul_(exp_avg_sqs, beta2)
            smith._foreach_addcmul_(exp_avg_sqs, grads, grads, value=1-beta2)
        """
        ...
        # Compiled execution
        compiled_result = smith.compile(fn, fullgraph=True)(*inputs)
    ...
    def test_foreach_map_backward_unary(self, op):
        ...
        # unpacking result, (fw_code, bw_code)
        _, (_, _) = run_fw_bw_and_get_code(lambda: smith.compile(fn)(inp))
    ...
    def test_foreach_addcmul_fma_bitwise_equal(self):
        ...
        self_tensors = [
            smith.randn(64, 64, device=GPU_TYPE),
        ...
        eager_result = smith._foreach_addcmul(self_tensors, tensor1_list, tensor2_list)

        @smith.compile
        def fn(s, t1, t2):
            return smith._foreach_addcmul(s, t1, t2)
        ...
        # Test with value != 1
        eager_result2 = smith._foreach_addcmul(
            self_tensors, tensor1_list, tensor2_list, value=2.5
        )

        @smith.compile
        def fn2(s, t1, t2):
            return smith._foreach_addcmul(s, t1, t2, value=2.5)
    ...
    def test_foreach_addcmul_uses_fma_instruction(self):
        ...
        @smith.compile
        def fn(s, t1, t2):
            return smith._foreach_addcmul(s, t1, t2, value=2.0)
