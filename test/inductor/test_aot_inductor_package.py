...
from smith._inductor.codecache import get_kernel_bin_format, WritableTempFile
from smith._inductor.package import load_package, package_aoti
from smith._inductor.test_case import TestCase
from smith._inductor.utils import fresh_cache
from smith.export import Dim
from smith.export.experimental import _ExportPackage
from smith.export.pt2_archive._package import (
    AOTICompiledModel,
    load_pt2,
...
from smith.testing._internal.common_utils import IS_FBCODE, TEST_CUDA
...
class TestAOTInductorPackage(TestCase):
    ...
    def cmake_compile_and_run(self, base_dir):
        ...
        custom_env["CMAKE_PREFIX_PATH"] = ":".join(
            [str(Path(smith.__file__).parent)]
    ...
    def test_remove_intermediate_files(self):
        ...
        with smith.no_grad():
            ...
            smith.manual_seed(0)
            with WritableTempFile(suffix=".pt2") as f:
                ep = smith.export.export(model, example_inputs, strict=True)
                with fresh_cache():
                ...
                loaded = smith._inductor.aoti_load_package(package_path)
    ...
    @unittest.skipIf(
        TEST_CUDA and _get_smith_cuda_version() < TRITON_PTXAS_VERSION,
    ...
    def test_compile_after_package(self):
        ...
        with smith.no_grad():
            ...
            with (
                ...
                optimized = smith._export.aot_load(str(so_path), self.device)
    ...
    @unittest.skipIf(IS_FBCODE, "cmake won't work in fbcode")
    @requires_triton_ptxas_compat
    @smith._inductor.config.patch("test_configs.use_libsmith", True)
    ...
    def test_compile_standalone_cos(self):
        ...
        with smith.no_grad():
            example_inputs = (smith.randn(8, 32, device=self.device),)
            model = Model().to(device=self.device)
    ...
    def test_compile_with_exporter_weights(self):
        ...
        example_inputs = (smith.ones(3, 3).to(self.device),)
    ...
    def test_metadata(self):
        ...
        with smith.no_grad():
            ...
            with WritableTempFile(suffix=".pt2") as f:
                ep = smith.export.export(model, example_inputs, strict=False)
                package_path = smith._inductor.aoti_compile_and_package(
                ...
                loaded_metadata = (
                    smith._C._aoti.AOTIModelPackageLoader.load_metadata_from_package(
                        package_path, "model"
                    )
                )
                ...
                current_device_info = smith._inductor.codecache.get_device_information(
                    device
                )
    ...
    def test_multiple_methods(self):
        ...
        example_inputs1 = (
            smith.randn(2, 4, device=self.device),
            smith.randn(3, 4, device=self.device),
        )
        ep1 = smith.export.export(
            Model1(), example_inputs1, dynamic_shapes=dynamic_shapes, strict=True
        )
        aoti_files1 = smith._inductor.aot_compile(
        ...
        example_inputs2 = (smith.randn(5, 5, device=self.device),)
        ep2 = smith.export.export(Model2(self.device), example_inputs2, strict=True)
        aoti_files2 = smith._inductor.aot_compile(
    ...
    def test_duplicate_calls(self):
        ...
        self.assertTrue(
            smith.allclose(loaded1(*example_inputs1), ep1.module()(*example_inputs1))
        )
        self.assertTrue(
            smith.allclose(loaded2(*example_inputs2), ep2.module()(*example_inputs2))
        )
    ...
    def test_specified_output_dir(self):
        ...
        self.assertTrue(
            smith.allclose(loaded(*example_inputs), ep.module()(*example_inputs))
        )
    ...
    def test_save_buffer(self):
        ...
        buffer = io.BytesIO()
        buffer = smith._inductor.aoti_compile_and_package(ep, package_path=buffer)  # type: ignore[arg-type]
        for _ in range(2):
    ...
    def test_package_without_weight(self):
        class Model(smith.nn.Module):
            def __init__(self, n, k, device):
                super().__init__()
                self.linear = smith.nn.Linear(k, n, device=device)
        ...
        example_inputs = (smith.randn(M, K, device=self.device),)
    ...
    def test_update_weights(self):
        ...
        new_state_dict = {
            "linear.weight": smith.randn(N, K, device=self.device),
            "linear.bias": smith.randn(N, device=self.device),
        }
    ...
    def test_package_shared_weights(self):
        ...
        x = smith.randn(3, 4)
        y = smith.randn(3, 4)
        buffer = smith.nn.Buffer(x.clone())
        buffer2 = smith.nn.Buffer(y.clone())
        bar1 = Bar(buffer, buffer2)
        bar2 = Bar2(buffer, buffer2)
        ep1 = smith.export.export(bar1, ())
        ep2 = smith.export.export(bar2, ())
        aoti_files1 = smith._inductor.aot_compile(ep1.module(), (), options=options)
        aoti_files2 = smith._inductor.aot_compile(ep2.module(), (), options=options)
    ...
    def test_package_weights_on_disk_nested_module(self):
        ...
        x = smith.randn(3, 3).to(self.device)
        bar1 = Bar().to(self.device)
        ep = smith.export.export(bar1, (x,))
        package_path = smith._inductor.aoti_compile_and_package(
            ep, inductor_configs=options
        )
        pt2_contents = load_pt2(package_path, load_weights_from_disk=True)
