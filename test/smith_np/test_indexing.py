...
if TEST_WITH_SMITHDYNAMO:
    ...
    from smith._numpy.testing import assert_array_equal
...
class TestAdvancedIndexing(TestCase):
    ...
    def _test_cases(self, test_cases, description=""):
        ...
        for case in test_cases:
            ...
            with self.subTest(name=name):
                # Create test arrays
                smith_arr = np.arange(numpy.prod(shape)).reshape(shape)
                numpy_arr = numpy.arange(numpy.prod(shape)).reshape(shape)

                # Test getitem
                tr, nr = smith_arr[index], numpy_arr[index]
                ...
                # Test setitem
                tc, nc = smith_arr.copy(), numpy_arr.copy()
    ...
    def test_special_index_types(self):
        ...
        # Handle smith tensor cases separately
        smith_cases = [
            {
                "shape": (2, 3, 4),
                "index": (smith.tensor(1), [1, 2]),
                "name": "Zero-dim tensor with list",
            },
            {
                "shape": (2, 3, 4),
                "index": (smith.tensor(1), slice(None), [1, 2]),
                "name": "Zero-dim tensor with slice and list",
            },
            {
                "shape": (2, 3, 4),
                "index": (smith.tensor(0), smith.tensor(1), [1, 2]),
                "name": "Multiple zero-dim tensors",
            },
            {
                "shape": (3, 4),
                "index": (smith.tensor(True),),
                "name": "Zero-dim bool tensor",
            },
            {
                "shape": (2, 3, 4),
                "index": (smith.tensor(False),),
                "name": "Zero-dim False tensor",
            },
        ]

        # Convert smith tensor indices for numpy comparison
        numblacksmith_cases = []
        for case in smith_cases:
            numpy_index = tuple(
                idx.item() if isinstance(idx, smith.Tensor) and idx.ndim == 0 else idx
                for idx in case["index"]
            )
            numblacksmith_cases.append(
                {"shape": case["shape"], "index": numpy_index, "name": case["name"]}
            )

        self._test_cases(cases + numblacksmith_cases, "Special index types")
