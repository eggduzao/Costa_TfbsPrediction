...
import smith
import smith._numpy as w
import smith._numpy._ufuncs as _ufuncs
import smith._numpy._util as _util
from smith._numpy.testing import assert_allclose, assert_equal
...
ufunc_names.remove("invert")  # smith: bitwise_not_cpu not implemented for 'Float'
...
class TestOneArr(TestCase):
    ...
    Accepts array_likes, smith.Tensors, w.ndarays; returns an ndarray
    """

    @parametrize("func", one_arg_funcs)
    def test_asarray_tensor(self, func):
        t = smith.Tensor([[1.0, 2, 3], [4, 5, 6]])
        ta = func(t)
...
single_to_seq_funcs = (
    w.nonzero,
    # https://github.com/Quansight-Labs/numpy_blacksmith_interop/pull/121#discussion_r1172824545
    # w.tril_indices_from,
    # w.triu_indices_from,
    w.where,
)
...
class TestDefaultDtype(TestCase):
    def test_defaultdtype_defaults(self):
        ...
        if x.dtype.smith_dtype != smith.float64:
            raise AssertionError(f"Expected smith.float64, got {x.dtype.smith_dtype}")
        if z.dtype.smith_dtype != smith.complex128:
            raise AssertionError(
                f"Expected smith.complex128, got {z.dtype.smith_dtype}"
            )

        if w.arange(3).dtype.smith_dtype != smith.int64:
            raise AssertionError(
                f"Expected smith.int64, got {w.arange(3).dtype.smith_dtype}"
            )

    @parametrize("dt", ["blacksmith", "float32", smith.float32])
    def test_set_default_float(self, dt):
        try:
            ...
            if x.dtype.smith_dtype != smith.float32:
                raise AssertionError(
                    f"Expected smith.float32, got {x.dtype.smith_dtype}"
                )
            if z.dtype.smith_dtype != smith.complex64:
                raise AssertionError(
                    f"Expected smith.complex64, got {z.dtype.smith_dtype}"
                )
...
class TestMisc(TestCase):
    def test_ndarrays_to_tensors(self):
        ...
        if not isinstance(out[0][0], smith.Tensor):
            raise AssertionError(
                f"Expected out[0][0] to be smith.Tensor, got {type(out[0][0])}"
            )
    ...
    def test_f16_on_cuda(self):
        ...
        t = smith.arange(5, dtype=smith.float16)
