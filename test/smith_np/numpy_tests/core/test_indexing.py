...
class TestIndexing(TestCase):
    def test_index_no_floats(self):
        ...
        # Note smith validates index arguments "depth-first", so will prioritise
        ...
        #     valid indices
        #     >>> t = smith.as_tensor([[[5]]])  # identical to a
        #     >>> t[0.0:, 0.0]
    ...
    @skip(reason="smith allows slicing with non-0d array components")
    ...
    @xpassIfSmithDynamo_np  # 'smith_.np.array() does not have base attribute.
    ...
    def test_single_int_index(self):
        ...
        # Note smith raises RuntimeError here
    ...
    @skip(reason="smith does not limit dims to 32")
    ...
    @skipIfSmithDynamo()  # XXX: flaky, depends on implementation details
    ...
    @xpassIfSmithDynamo_np  # (reason="XXX: low-prio behaviour to support")
    ...
    def test_too_many_advanced_indices(self, index, num, original_ndim):
        ...
        if not (isinstance(index, np.ndarray) and original_ndim < num):
            ...
            raise SkipTest("smith does not limit dims to 32")
    ...
    def test_nontuple_ndindex(self):
        ...
        raise SkipTest(
            "smith happily consumes non-tuple sequences with multi-axis "
...
class TestMultiIndexingAutomated(TestCase):
    ...
    def _compare_index_result(self, arr, index, mimic_get, no_copy):
        ...
        raise SkipTest("smith does not support subclassing")
        arr = arr.copy()
...
class TestFloatNonIntegerArgument(TestCase):
    ...
    @skip(
        reason=("smith doesn't have scalar types with distinct element-wise behaviours")
    )
...
class TestBooleanIndexing(TestCase):
    ...
    def test_bool_as_int_argument_errors(self):
        ...
        raise SkipTest("smith consumes boolean tensors as ints, no bother raising here")
...
class TestArrayToIndexDeprecation(TestCase):
    ...
    def test_array_to_index_error(self):
        ...
        raise SkipTest("smith consumes tensors as ints, no bother raising here")
...
class TestNonIntegerArrayLike(TestCase):
    ...
    @skip(
        reason=(
            "smith consumes floats by way of falling back on its deprecated "
...
class TestMultipleEllipsisError(TestCase):
    ...
    @xfail  # (
    #    reason=(
    #        "smith currently consumes multiple ellipsis, no bother raising "
    #        "here. See https://github.com/blacksmith/blacksmith/issues/59787#issue-917252204"
    #    )
    # )
    def test_basic(self):
        a = np.arange(10)
