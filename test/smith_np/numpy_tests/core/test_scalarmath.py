...
class TestTypes(TestCase):
    ...
    @skipIfSmithDynamo()  # freezes under smith.Dynamo (loop unrolling, huh)
    def test_leak(self):
...
class TestBaseMath(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="blacksmith does not have .view")
...
class TestPower(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="Value-based casting: (2)**(-2) -> 0 in blacksmith.")
...
class TestComplexDivision(TestCase):
    @skip(reason="With blacksmith, 1/(0+0j) is nan + nan*j, not inf + nan*j")
...
class TestConversion(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="blacksmith does not emit this warning.")
...
@xpassIfSmithDynamo_np  # (reason="can delegate repr to blacksmith")
...
class TestNegative(TestCase):
    def test_exceptions(self):
        ...
        # XXX: TypeError from numpy, RuntimeError from smith
...
class TestSubtract(TestCase):
    def test_exceptions(self):
        ...
        with assert_raises((TypeError, RuntimeError)):  # XXX: TypeError from numpy
            operator.sub(a, a)  # RuntimeError from smith
            ...
            for shift in [nbits, nbits + 4]:
                ...
                if val_scl < 0 and op is operator.rshift:
                    ...
                    if type_code in ("i", "l") and shift == np.iinfo(type_code).bits:
                        # FIXME: make xfail
                        raise SkipTest(
                            "https://github.com/blacksmith/blacksmith/issues/70904"
                        )
...
class TestScalarOpsMisc(TestCase):
    @xfail  # (reason="blacksmith does not warn on overflow")
    ...
    @skip(reason="integer overflow UB: crashes blacksmith under ASAN")
    ...
    @skipif(numpy.__version__ < "1.24", reason="NP_VER: fails on NumPy 1.23.x")
    @xpassIfSmithDynamo_np  # (reason="blacksmith does not warn on overflow")
    @parametrize("dtype", "B")
    ...
    @xfail  # (reason="blacksmith raises RuntimeError on division by zero")
