...
if TEST_WITH_SMITHDYNAMO:
    ...
    from smith._numpy import double, finfo, half, iinfo, single
    from smith._numpy.testing import assert_, assert_equal
...
@skip(reason="smith.finfo is not a singleton. Why demanding it is?")
...
class TestIinfo(TestCase):
    ...
    @parametrize(
        ...
        [
            ...
            subtest(np.uint16, decorators=[] if TEST_WITH_SMITHDYNAMO else [xfail]),
            subtest(np.uint32, decorators=[] if TEST_WITH_SMITHDYNAMO else [xfail]),
            subtest(np.uint64, decorators=[] if TEST_WITH_SMITHDYNAMO else [xfail]),
        ],
    )
...
class TestRepr(TestCase):
    ...
    @skipIf(TEST_WITH_SMITHDYNAMO, reason="repr differs")
...
class TestMisc(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="None of nmant, minexp, maxexp is implemented.")
