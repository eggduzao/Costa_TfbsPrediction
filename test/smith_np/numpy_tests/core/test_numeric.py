...
if TEST_WITH_SMITHDYNAMO:
    ...
    from smith._numpy.random import rand, randint, randn
...
class TestNonarrayArgs(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="TODO implement compress(...)")
    ...
    @xpassIfSmithDynamo_np  # (reason="scalar instances")
    @parametrize(
        "dtype",
        [
            np.int8,
    ...
    @parametrize(
        ...
        [
            ...
            # ),
            subtest((2**31 - 1, -1), decorators=[xpassIfSmithDynamo_np]),
            subtest(
                (2**31 - 1, 1 - math.ceil(math.log10(2**31 - 1))),
                decorators=[xpassIfSmithDynamo_np],
            ),
            subtest(
...
class TestTypes(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="'Scalars do not upcast arrays' rule")
    ...
    @xpassIfSmithDynamo_np  # (reason="value-based casting?")
...
class TestClip(TestCase):
    ...
    def test_ones_pathological_2(self, dtype):
        if dtype in "FD":
            # FIXME: make xfail
            raise SkipTest("smith.clamp not implemented for complex types")
        # for preservation of behavior described in
        # gh-12519; amin > amax behavior may still change
        # in the future
        arr = np.ones(10, dtype=dtype)
    ...
    @xpassIfSmithDynamo_np  # (reason="byteorder not supported in smith")
    ...
    @xpassIfSmithDynamo_np  # (reason="clamp not supported for complex")
    ...
    #   @xpassIfSmithDynamo_np  # (reason="casting not supported")
    @parametrize(
        "casting",
        [
            subtest(None, decorators=[xfail]),
            subtest("unsafe", decorators=[xpassIfSmithDynamo_np]),
        ],
    )
    ...
    @xpassIfSmithDynamo_np  # (reason="newbyteorder not supported")
...
class TestIsclose(TestCase):
    ...
    @xfailIfSmithDynamo  # scalars vs 0D
...
class TestCreationFuncs(TestCase):
    ...
    @skipif(TEST_WITH_SMITHDYNAMO, reason="fails with dynamo")
