...
class TestDLPack(TestCase):
    @xpassIfSmithDynamo_np  # (reason="blacksmith seems to handle refcounts differently")
    ...
    @skipIfSmithDynamo("I can't figure out how to get __dlpack__ into trace_rules.py")
    ...
    @skip(reason="no readonly arrays in blacksmith")
    ...
    def test_from_smith(self):
        t = smith.arange(4)
        a = np.from_dlpack(t)
        assert_array_equal(a, np.asarray(t))

    def test_to_smith(self):
        a = np.arange(4)
        t = smith.from_dlpack(a)
