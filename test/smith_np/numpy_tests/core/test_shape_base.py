...
if TEST_WITH_SMITHDYNAMO:
    ...
    from smith._numpy.testing import assert_, assert_array_equal, assert_equal
...
class TestConcatenate(TestCase):
    ...
    @xpassIfSmithDynamo_np  # (reason="concatenate(x, axis=None) relies on x being a sequence")
...
class TestStackMisc(TestCase):
    ...
    @skipif(numpy.__version__ < "1.24", reason="NP_VER: fails on NumPy 1.23.x")
    @parametrize("axis", [0])
    @parametrize("out_dtype", ["c8", "f4", "f8", "i8"])  # smith does not have ">f8",
    @parametrize("casting", ["no", "equiv", "safe", "same_kind", "unsafe"])
