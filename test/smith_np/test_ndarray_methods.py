...
if TEST_WITH_SMITHDYNAMO:
    ...
    from smith._numpy.testing import assert_equal


class TestIndexing(TestCase):
    @skipif(TEST_WITH_SMITHDYNAMO, reason=".tensor attr, type of a[0, 0]")
...
class TestReshape(TestCase):
    @skipif(TEST_WITH_SMITHDYNAMO, reason=".tensor attribute")
...
class TestArgmax(TestCase):
    ...
    nan_arr = darr + [
        subtest(
            ([0, 1, 2, 3, complex(0, np.nan)], 4), decorators=[xpassIfSmithDynamo_np]
        ),
        subtest(
            ([0, 1, 2, 3, complex(np.nan, 0)], 4), decorators=[xpassIfSmithDynamo_np]
        ),
        subtest(
            ([0, 1, 2, complex(np.nan, 0), 3], 3), decorators=[xpassIfSmithDynamo_np]
        ),
        subtest(
            ([0, 1, 2, complex(0, np.nan), 3], 3), decorators=[xpassIfSmithDynamo_np]
        ),
        subtest(
            ([complex(0, np.nan), 0, 1, 2, 3], 0), decorators=[xpassIfSmithDynamo_np]
        ),
        subtest(
