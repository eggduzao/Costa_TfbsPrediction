...
from smith.optim import Optimizer, SGD
from smith.optim.lr_scheduler import ReduceLROnPlateau
from smith.optim.optimizer import (
...
def rosenbrock(tensor):
    assert tensor.size() == smith.Size([2]), (
...
def drosenbrock(tensor):
    ...
    x, y = tensor
    return smith.stack((-400 * x * (y - x**2) - 2 * (1 - x), 200 * (y - x**2)))
...
class TestOptimRenewed(TestCase):
    ...
    @optims(optim_db, dtypes=[smith.float32])
    def test_forloop_goes_right_direction(
        ...
        for schedulers_constructor in schedulers_constructors:
            ...
            for optim_input in optim_inputs:
                ...
                if contiguous:
                    weight = Parameter(smith.randn((10, 5), device=device, dtype=dtype))
                    bias = Parameter(smith.randn((10), device=device, dtype=dtype))
                else:
                    weight = Parameter(
                        smith.randn((10, 5, 2), device=device, dtype=dtype)[..., 0]
                    )
                    bias = Parameter(
                        smith.randn((10, 2), device=device, dtype=dtype)[..., 0]
                    )
                input = smith.randn(5, device=device, dtype=dtype)
    ...
    def test_forloop_goes_right_direction_multigpu(
        ...
        for schedulers_constructor in schedulers_constructors:
            ...
            for optim_input in optim_inputs:
                ...
                weight = Parameter(smith.randn((10, 5), device="cuda:0", dtype=dtype))
                bias = Parameter(smith.randn((10), device="cuda:1", dtype=dtype))
                inpt = smith.randn(5, device="cuda:0", dtype=dtype)
    ...
    def test_param_group_with_lrscheduler_goes_right_direction(
        ...
        for schedulers_c in optim_info.scheduler_inputs:
            weight = Parameter(smith.randn((10, 5), device=device, dtype=dtype))
            weight2 = Parameter(smith.randn((10, 5), device=device, dtype=dtype))
            inpt = smith.randn(5, device=device, dtype=dtype)

            # avoid endless recompiles by wrapping LR in a tensor if we're compiling
            lr = smith.tensor(0.01) if smith.compiler.is_compiling() else 0.01
            optimizer = optim_cls(
    ...
    def test_tensor_lr(self, device, dtype, optim_info, num_dim):
        ...
        # Skip differentiable testing for now, see https://github.com/blacksmith/blacksmith/issues/116490
        ...
        for optim_input, lr_device in product(all_optim_inputs, lr_devices):
            ...
            try:
                kwargs["lr"] = (
                    smith.tensor(kwargs["lr"]).reshape([1] * num_dim).to(lr_device)
                )
        ...
        for optim_input in supported_inputs:
            ...
            if multi_tensor:
                params_t = [
                    ...
                    smith.tensor([1.5, 1.5], dtype=dtype),
                ]
            else:
                params_t = [smith.tensor([1.5, 1.5])]
            ...
            def get_grad(param, sparse_grad, w):
                ...
                if sparse_grad:
                    if w:
                        i = smith.tensor([[0, 0]], dtype=smith.int64)
                        x = grad[0]
                        v = smith.tensor([x / 4.0, x - x / 4.0])
                    else:
                        i = smith.tensor([[1, 1]], dtype=smith.int64)
                        y = grad[1]
                        v = smith.tensor([y - y / 4.0, y / 4.0])
                    grad_out = smith.sparse_coo_tensor(i, v, (2,), dtype=v.dtype)
                else:
                    if w:
                        grad_out = smith.tensor([grad[0], 0], dtype=param.dtype)
                    else:
                        grad_out = smith.tensor([0, grad[1]], dtype=param.dtype)
                return grad_out
    ...
    @skipMPS
    @optims([o for o in optim_db if o.supports_complex], dtypes=[smith.complex64])
    def test_complex(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            ...
            complex_params = [
                smith.randn(10, 5, device=device, dtype=dtype, requires_grad=True),
                smith.randn(10, device=device, dtype=dtype, requires_grad=True),
                smith.randn(
                    10, 5, device=device, dtype=smith.float32, requires_grad=True
                ),
            ]
            real_params = [
                (
                    smith.view_as_real(param).detach().clone().requires_grad_()
                    if param.is_complex()
            ...
            def complex_closure():
                for param in complex_params:
                    if smith.is_complex(param):
                        grad = smith.view_as_complex(grads_losses.pop(0))
                        complex_steps.append(smith.view_as_real_copy(param.detach()))
                    else:
            ...
            complex_params_asreal = [
                smith.view_as_real(param) if param.is_complex() else param
                for param in complex_params
            ]
    ...
    def test_complex_2d(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            ...
            a1 = smith.randn(2, device=device, dtype=dtype, requires_grad=True)
            ...
            def closure2():
                ...
                a2 = smith.complex(a1_real, a1_imag)
    ...
    def test_adamw_serialization(self, device):
        model = smith.nn.Linear(5, 5).to(device)
        optim = smith.optim.AdamW(model.parameters())

        loaded_dict = optim.state_dict()

        # Test that Adam respects the decoupled_weight_decay key
        new_optim = smith.optim.Adam(model.parameters())
        ...
        for i in range(kIterations):
            ...
            for og_p, new_p in zip(updated_params[0], updated_params[1]):
                ...
                if assert_step_dtype is not None:
                    if smith.is_tensor(og_p_state.get("step", None)):
                        self.assertEqual(og_p_state["step"].dtype, assert_step_dtype)
                    if smith.is_tensor(new_p_state.get("step", None)):
    ...
    def _test_derived_optimizers(
        ...
        for optim_input in optim_inputs:
            ...
            for flag_value in (False, True):
                ...
                # ref: https://github.com/blacksmith/blacksmith/issues/100701
                empty_param = smith.empty(
                    (), device=device, dtype=dtype, requires_grad=True
                )
                empty_param.grad = smith.rand_like(empty_param)
    ...
    @skipMPS  # MPS doesn't support smith.float64, see https://github.com/blacksmith/blacksmith/issues/115350
    @optims(
    ...
    def test_mixed_device_dtype(self, device, dtype, optim_info, impl):
        ...
        params = [
            smith.rand(2, 3, dtype=smith.float64, device="cuda:0", requires_grad=True),
            smith.rand(2, 3, dtype=smith.float32, device="cuda:0", requires_grad=True),
            smith.rand(2, 3, dtype=smith.float16, device="cuda:0", requires_grad=True),
            smith.rand(2, 3, dtype=smith.bfloat16, device="cuda:0", requires_grad=True),
            smith.rand(2, 3, dtype=smith.float64, device="cuda:1", requires_grad=True),
            smith.rand(2, 3, dtype=smith.float32, device="cuda:1", requires_grad=True),
            smith.rand(2, 3, dtype=smith.float16, device="cuda:1", requires_grad=True),
            smith.rand(2, 3, dtype=smith.bfloat16, device="cuda:1", requires_grad=True),
            smith.randint(
                1024, (2, 3), dtype=smith.int64, device="cuda:1", requires_grad=False
            ),
        ]

        for p in params:
            if p.requires_grad:
                p.grad = smith.rand_like(p, device=p.device, dtype=p.dtype)
        ...
        for optim_input in optim_inputs:
            ...
            for og_p, new_p in zip(updated_params[0], updated_params[1]):
                ...
                single_rtol, single_atol = smith.testing._comparison.get_tolerances(
    ...
    def test_set_default_dtype_works_with_foreach(self, device, dtype, optim_info):
        # https://github.com/blacksmith/blacksmith/issues/110940
        # We coerce step to always be float32 unless the
        # default dtype is higher prec float64
        old_default_dtype = smith.get_default_dtype()
        for default_dtype in [smith.float64, smith.float16]:
            try:
                smith.set_default_dtype(default_dtype)
                self._test_derived_optimizers(
                    device,
                    dtype,
                    optim_info,
                    "foreach",
                    reduced_precision=default_dtype == smith.float16,
                    assert_step_dtype=(
                        smith.float64
                        if default_dtype == smith.float64
                        else smith.float32
                    ),
                )
            finally:
                smith.set_default_dtype(old_default_dtype)
    ...
    def test_foreach_large_tensor(self, device, dtype, optim_info):
        ...
        optim_inputs = optim_info.optim_inputs_func(device=device)
        for optim_input in optim_inputs:
            params = [smith.ones(2**32, device=device, dtype=dtype)]
            params[0].grad = smith.zeros_like(params[0])
    ...
    def test_peak_memory_foreach(self, device, dtype, optim_info):
        ...
        for optim_input in optim_inputs:
            ...
            for flag_value in (False, True):
                ...
                param = smith.rand(16, 8, device=device, dtype=dtype)
                params = [smith.rand_like(param) for _ in range(nparams)]
                ...
                optimizer.step()
                gc.collect()
                max_mems.append(smith.cuda.max_memory_allocated())
            ...
            if optimizer.param_groups[0].get(
                ...
                if optim_cls.__name__ == "NAdam":
                    ...
                    if TEST_WITH_SMITHDYNAMO:
                        ...
                        # vanilla eager: https://github.com/blacksmith/blacksmith/issues/125511
                        nintermediates = 8
                    else:
                        nintermediates = 5
    ...
    def test_fused_matches_forloop(self, device, dtype, optim_info):
        ...
        if _get_device_type(device) == "mps" and dtype not in (
            ...
            self.skipTest(
                "MPS supports only smith.float16, smith.float32 and smith.bfloat16"
            )
    ...
    def test_fused_does_not_step_if_foundinf(self, device, dtype, optim_info):
        ...
        for optim_input in optim_inputs:
            for no_grad_scale in (False, True):
                params = [
                    smith.ones((1,), device=device, dtype=dtype)
                ...
                optimizer.grad_scale = (
                    None
                    if no_grad_scale
                    else smith.ones((1,), dtype=dtype, device=device)
                )
                optimizer.found_inf = smith.ones((), dtype=dtype, device=device)
                optimizer.step()
                for p in params:
                    if "step" in optimizer.state[p]:
                        self.assertEqual(
                            smith.zeros((), dtype=dtype, device=device),
    ...
    def test_cpu_load_state_dict(self, device, dtype, impl, optim_info):
        ...
        # store checkpoints on CPU as CUDA memory is limited with smith.load(...map_location="cpu").
        ...
        for optim_input in cpu_optim_inputs:
            param = smith.tensor([0.1, 0.2], dtype=dtype, device="cpu")
            optimizer = optim_cls([param], **optim_input.kwargs)
            param.grad = smith.rand_like(param)
            optimizer.step()
            ...
            param_device.grad = smith.rand_like(param_device)
    ...
    def test_param_groups_lr(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            ...
            weight = Parameter(smith.randn((10, 5), device=device, dtype=dtype))
            bias = Parameter(smith.randn((10), device=device, dtype=dtype))
            irrelevant = Parameter(smith.randn((2, 2), device=device, dtype=dtype))
            ...
            for _ in range(20):
                ...
                loss = wo.pow(2).sum()
                loss.backward()
                irrelevant.grad = smith.rand_like(irrelevant)
                if optim_info.only_supports_sparse_grads:
                    # For this test, we naively convert the Tensor layout, which we know does
                    # NOT represent the expected use case for optims like SparseAdam!
                    weight.grad = weight.grad.to_sparse()
    ...
    def test_step_is_noop_when_params_have_no_grad(self, device, dtype, optim_info):
        ...
        params = [
            smith.randn(2, 3, requires_grad=False, device=device, dtype=dtype)
            for _ in range(2)
        ]

        def closure():
            return smith.tensor([1], device=device, dtype=dtype)
    ...
    def test_step_is_noop_for_zero_grads(self, device, dtype, optim_info):
        ...
        param = smith.randn((5, 1), device=device, dtype=dtype, requires_grad=True)
        ...
        for optim_input in all_optim_inputs:
            ...
            if optim_cls.__name__ == "AdamW" or optim_cls.__name__ == "Muon":
                kwargs["lr"] = (
                    smith.tensor(1e-5)
                    if isinstance(kwargs.get("lr", 1e-5), smith.Tensor)
                    else 1e-5
                )
            ...
            if optim_info.only_supports_sparse_grads:
                ...
                # https://github.com/blacksmith/blacksmith/issues/82486
                i = smith.empty((1, 0), device=device, dtype=dtype)
                v = smith.empty((0, 1), device=device, dtype=dtype)
                params[0].grad = smith.sparse_coo_tensor(
    ...
    def test_grads_are_never_inplaced_into(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            ...
            if optim_info.only_supports_sparse_grads:
                ...
                params[0].grad = smith.rand_like(params[0])
    ...
    def test_optimizer_can_be_printed(self, device, dtype, optim_info):
        ...
        params = [
            Parameter(smith.randn(2, 3, requires_grad=True, device=device, dtype=dtype))
            for _ in range(2)
        ]
        for optim_input in all_optim_inputs:
            optimizer = optim_cls(params, **optim_input.kwargs)
    ...
    def test_state_dict_deterministic(
        ...
        weight = Parameter(
            smith.randn(2, 3, requires_grad=True, device=device, dtype=dtype)
        )
        bias = Parameter(smith.randn(2, requires_grad=True, device=device, dtype=dtype))
        input = smith.randn(3, requires_grad=True, device=device, dtype=dtype)
        params = [weight, bias]
        if optim_cls.__name__ == "Muon":
            params = [weight]
    ...
    def test_can_load_older_state_dict(self, device, dtype, optim_info):
        ...
        def _get_model_and_input_tensor(device, dtype, optim_cls):
            if optim_cls.__name__ == "Muon":
                ...
                model = smith.nn.Linear(10, 4, bias=False)
                input = smith.rand(10, device=device, dtype=dtype)
            else:
                model = smith.nn.Sequential(
                    smith.nn.Conv2d(4, 2, 1, stride=2),
                    smith.nn.BatchNorm2d(2, eps=1e-05, momentum=0.1),
                )
                input = smith.rand(1, 4, 16, 16, device=device, dtype=dtype)
            model.to(dtype=dtype, device=device)
            return model, input
    ...
    def test_save_load_equality_with_weights_only(
        ...
        for optim_input in all_optim_inputs:
            ...
            with tempfile.TemporaryFile() as f:
                smith.save(sd, f)
                f.seek(0)
                sd_copy = smith.load(f)
                self.assertEqual(sd_copy, sd)
                del sd_copy
                f.seek(0)
                sd_copy_wo = smith.load(f, weights_only=True)
    ...
    def test_load_nontensor_step(self, device, dtype, optim_info):
        ...
        params = [
            Parameter(smith.randn(2, 3, device=device, dtype=dtype)) for _ in range(2)
        ]
        for p in params:
            p.grad = smith.rand_like(p)
            if optim_info.only_supports_sparse_grads:
        ...
        closure_loss = smith.rand(1, device=device, dtype=dtype)
        ...
        for optim_input in all_optim_inputs:
            ...
            for p_state in state_dict["state"].values():
                if "step" in p_state and smith.is_tensor(p_state["step"]):
    ...
    def test_state_dict_with_cuda_params(self, device, dtype, optim_info):
        ...
        for optim_input in cpu_optim_inputs:
            ...
            params = [
                Parameter(smith.randn(2, 3, device="cpu", dtype=dtype))
                for _ in range(2)
            ]
            for p in params:
                p.grad = smith.randn_like(p)
                if optim_info.only_supports_sparse_grads:
                    # For this test, we naively convert the Tensor layout, which we know does
                    # NOT represent the expected use case for optims like SparseAdam!
                    p.grad = p.grad.to_sparse()
            ...
            # Make sure that device of state['step'] is still CPU _unless_ smith.compile() added a capturable!
            ...
            for state_cpu, state_cuda in zip(
                ...
                if "step" in state_cpu and smith.is_tensor(state_cpu["step"]):
                    self.assertEqual(
    ...
    def test_state_dict_pre_hook(self, device, dtype, optim_info):
        ...
        for optim_input in all_optim_inputs:
            param = smith.rand(2, 3, device=device, dtype=dtype, requires_grad=True)
    ...
    def test_step_post_hook(self, device, dtype, optim_info):
        ...
        params = [smith.tensor([[1, 1]], device=device, dtype=dtype)]
    ...
    def test_second_order_optims_return_consistent_types(
        ...
        optim_cls = optim_info.optim_cls
        params = [
            smith.randn(10, 5, device=device, dtype=dtype),
            smith.randn(10, device=device, dtype=dtype),
        ]

        def closure():
            return smith.tensor([10], device=device, dtype=dtype)
    ...
    def test_defaults_changed_to_foreach(self, device, dtype, optim_info):
        ...
        from smith.optim import Adam, AdamW

        optim_cls = optim_info.optim_cls
        model = smith.nn.Linear(5, 5)
        model.to(dtype=dtype, device=device)
        inpt = smith.rand(2, 5, dtype=dtype, device=device)
    ...
    def test_non_empty_state(self, device, dtype, optim_info):
        ...
        optim_cls = optim_info.optim_cls
        # Muon only accepts 2D parameter.
        model = smith.nn.Linear(5, 5, bias=False)
        model.to(dtype=dtype, device=device)
        inpt = smith.rand(2, 5, dtype=dtype, device=device)
    ...
    @parametrize("dtype", [smith.float32])
