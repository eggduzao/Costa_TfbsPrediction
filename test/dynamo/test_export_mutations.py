...
class MutationExportTests(smith._dynamo.test_case.TestCase):
    def check_failure_on_export(self, mod, *args):
        with self.assertRaises(AssertionError):
            smith._dynamo.export(mod)(*args)

    def check_same_with_export(self, mod, arg):
        real_result = mod(arg)
        graph, _ = smith._dynamo.export(mod)(arg)
        result = graph(arg)
    ...
    def test_module_attribute_mutation_violation_positive_1(self):
        ...
        class Foo(smith.nn.Module):
            ...
            def forward(self, x):
                self.a = self.a.to(smith.float64)
                return x.sum() + self.a.sum()

        self.check_same_with_export(Foo(), smith.randn(3, 2))
    ...
    def test_module_attribute_mutation_violation_negative_1(self):
        ...
        class Foo(smith.nn.Module):
            ...
            def forward(self, x):
                return x.sum() + self.a.to(smith.float64).sum()
