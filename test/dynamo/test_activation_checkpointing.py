...
from smith._higher_order_ops.wrap import tag_activation_checkpoint
from smith.testing._internal.common_device_type import instantiate_device_type_tests
from smith.testing._internal.common_utils import IS_WINDOWS, parametrize, skipIfHpu
...
def checkpoint_wrapper(fn):
    def inner(*args):
        return smith.utils.checkpoint.checkpoint(fn, *args, use_reentrant=True)
...
def _grad(*args, **kwargs):
    return smith.autograd.grad(*args, **kwargs)
...
def collect_fwd_graph_outputs(graph: smith.fx.Graph, *, fwd_outputs: set[str]):
    if not smith._dynamo.compiled_autograd.in_compiled_autograd_region:  # fwd graph
...
class ActivationCheckpointingViaTagsTests(
    ...
    def _validate(
        ...
        *args,
        ...
        smith.manual_seed(0)
        compiled_fn = smith.compile(cloned_fn, fullgraph=fullgraph, backend=backend)
        ctx = contextlib.nullcontext()
        if compiled_autograd:
            ctx = smith._dynamo.compiled_autograd._enable(
                lambda gm: smith.compile(gm, fullgraph=fullgraph, backend=backend)
            )
        with ctx:
        ...
        if not skip_check:
            self.assertEqual(
                result,
                expected,
                msg="Output mismatch between smith.compile and eager versions",
            )
            for arg, cloned_arg in zip(args, cloned_args):
                self.assertEqual(
                    arg.grad,
                    cloned_arg.grad,
                    msg="Gradient mismatch between smith.compile and eager versions",
                )
    ...
    def _compare_orig_and_checkpointed_fns(
        ...
        # should produce the same outputs and the same gradients under smith.compile.
        ...
        run(functools.partial(smith.compile, fullgraph=fullgraph))
        if fullgraph:
            ...
            def export_compiler(fn):
                ...
                def runtime_wrapper(*runtime_args):
                    from smith.export import _trace

                    gm = _trace._export_to_smith_ir(
                        f=mod,
                    ...
                    return smith.compile(gm, fullgraph=False)(*runtime_args)
    ...
    def test_tags_function(self, device, partition_fn):
        ...
        bw_compiler = functools.partial(
            count_ops, freq=3, op=smith.ops.aten.mm.default
    ...
    def test_tags_function_via_global_checkpoint(self, device, partition_fn):
        ...
        def fn(x, y):
            ...
            return checkpoint(gn, smith.sin(x), y, use_reentrant=True)
    ...
    def test_tags_function_with_kwargs(self, device, partition_fn):
        ...
        def fn(x, y):
            return smith.utils.checkpoint.checkpoint(
                gn, smith.sin(x), y, use_reentrant=False
            )
    ...
    def test_checkpoint_shows_tags_in_tlparse(self, device):
        ...
        def fn(x, y):
            return smith.utils.checkpoint.checkpoint(
                gn, smith.sin(x), y, use_reentrant=True, preserve_rng_state=False
            )
        ...
        _ = smith.compile(fn, backend=backend)(x, y)
    ...
    def test_tags_sequential_layers(self, device, partition_fn):
        ...
        def fn(x):
            x = smith.utils.checkpoint.checkpoint(gn, x)
            x = smith.utils.checkpoint.checkpoint(gn, x)
            return x

        x = smith.randn(4, 4, device=device, requires_grad=True)

        fw_compiler = functools.partial(count_ops, freq=6, op=smith.ops.aten.mm.default)
        bw_compiler = functools.partial(
            count_ops,
            freqs=[2, 18],
            ops=[smith.ops.aten.cos.default, smith.ops.aten.mm.default],
        )  # mm recomputed in the bwd
        backend = aot_autograd(
            fw_compiler=fw_compiler,
            bw_compiler=bw_compiler,
    ...
    def test_tags_multiple_checkpoints(self, device, partition_fn):
        ...
        def fn(x, y):
            x = smith.sin(x)
            z = smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
            x = smith.sin(z)
            z = smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
            return z
        ...
        fw_compiler = functools.partial(count_ops, freq=2, op=smith.ops.aten.mm.default)
        bw_compiler = functools.partial(
            count_ops, freq=6, op=smith.ops.aten.mm.default
        )  # mm recomputed in the bwd
        backend = aot_autograd(
            fw_compiler=fw_compiler,
            bw_compiler=bw_compiler,
    ...
    def test_tags_module(self, device, partition_fn):
        ...
        x = smith.randn(10, 10, device=device, requires_grad=True)
    ...
    def test_tags_decomps(self, device, partition_fn):
        ...
        class MockModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.nn.functional.gelu(self.linear(x))
        ...
        fw_compiler = functools.partial(
            count_ops, freq=1, op=smith.ops.aten.erf.default
        )
        bw_compiler = functools.partial(
            count_ops, freq=1, op=smith.ops.aten.erf.default
        )
    ...
    def test_tags_dropout(self, device):
        ...
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(10, 10)
                self.dropout = smith.nn.Dropout(0.2)
        ...
        def fn(x):
            return smith.utils.checkpoint.checkpoint(mod, x, use_reentrant=True)
    ...
    def test_tags_must_save_tensor_that_has_backward_hook(self):
        ...
        def _factory_fn():
            mod = MyMod()
            x = smith.ones(4, 4, dtype=smith.float32, requires_grad=True)
            backend = "inductor"
...
Non-primal fwd outputs from model w/o backward hook: {mod_no_hook_fwd_outputs_no_primal}.""",
    ...
    def test_fallback(self, device):
        def gn(x, y):
            ...
            a = smith.sigmoid(smith.matmul(x, y))
            smith._dynamo.graph_break()
            return smith.cos(a)

        def fn(x, y):
            return smith.cos(checkpoint(gn, smith.sin(x), y, use_reentrant=False))

        x = smith.randn(4, 4, requires_grad=True, device=device)
        y = smith.randn(4, 4, requires_grad=True, device=device)
        args = (x, y)
    ...
    def test_kwargs(self, device):
        def gn(x, y, z=None):
            a = smith.matmul(x, y)
            if z is not None:
                return smith.matmul(a, z)
            return a

        def fn(x, y, z):
            return smith.cos(checkpoint(gn, x, y, use_reentrant=False, z=z))

        x = smith.randn(4, 4, requires_grad=True, device=device)
        y = smith.randn(4, 4, requires_grad=True, device=device)
        z = smith.randn(4, 4, requires_grad=True, device=device)
        args = (x, y, z)
    ...
    def test_symints_location(self, device):
        def gn(x, y):
            return smith.matmul(x, smith.nn.functional.dropout(y, 0.5))

        def fn(x, y):
            return smith.utils.checkpoint.checkpoint(gn, x, y, use_reentrant=True)
        ...
        x = smith.randn(5, 5, requires_grad=True, device=device)
        y = smith.randn(5, 5, requires_grad=True, device=device)
        args = (x, y)
        expected = fn(*args)
        result = opt_fn(*args)
    ...
    @requires_cuda_and_triton
    @unittest.skipIf(IS_WINDOWS, "smith.compile doesn't work with windows")
    @parametrize(
        "partition_fn",
        [
            min_cut_rematerialization_partition,
    ...
    def test_compile_selective_checkpoint_must_recompute(self, device, partition_fn):
        ...
        def _test(context_fn, bw_compiler, partition_fn):
            def gn(x):
                return smith.cos(smith.sin(smith.matmul(x, x) @ x))
    ...
    def test_sac_with_partial_context_fn(self):
        ...
        def f(x, y):
            return smith.sigmoid(smith.matmul(smith.matmul(x, y), y)) * y
        ...
        opt_fn = smith.compile(fn, backend="aot_eager_decomp_partition", fullgraph=True)
        a = smith.randn(4, 4, requires_grad=True, device="cpu")
        b = smith.randn(4, 4, requires_grad=True, device="cpu")
    ...
    def test_compile_selective_checkpoint_must_not_recompute_gemm_no_functionalization(
        ...
        fw_compiler = functools.partial(
            count_ops,
            freq=1,
            op=smith.ops.aten.sigmoid.default,
        )
        bw_compiler = functools.partial(
            count_ops,
    ...
    def test_compile_selective_checkpoint_triton_kernel(self, device, partition_fn):
        ...
        def gn(x, y):
            return (
                smith.sigmoid(smith.matmul(smith.matmul(AddOne.apply(x.sin()), y), y))
                * y
            )
    ...
    def test_compile_selective_checkpoint_tensor_subclass(self, device, partition_fn):
        ...
        rand_tensor = smith.randn(4, 4, requires_grad=True, device=device)
    ...
    def test_compile_selective_checkpoint_custom_rule(self, device, partition_fn):
        def _get_custom_policy(meta):
            ...
            def _custom_policy(mode, func, *args, **kwargs):
                ...
                if mm_count_key not in meta:
                    meta[mm_count_key] = 0
                if func == smith.ops.aten.mm.default:
                    meta[mm_count_key] += 1
                ...
                return func in no_recompute_list and not (
                    func == smith.ops.aten.mm.default and meta[mm_count_key] == 2
                )
        ...
        def gn(x, y):
            return smith.sigmoid(
                smith.sigmoid(smith.matmul(smith.matmul(x, y) * y, y) * y)
            )
        ...
        def fn(x, y):
            return smith.utils.checkpoint.checkpoint(
                ...
                context_fn=functools.partial(
                    selective_checkpointing_context_fn, [smith.ops.aten.mm.default]
                ),
            )
    ...
    def test_compile_selective_checkpoint_outplace_op(self, device, partition_fn):
        def selective_checkpointing_context_fn():
            no_recompute_list = [
                smith.ops.aten.mm.default,
                smith.ops.aten.sigmoid.default,
            ]
            return create_selective_checkpoint_contexts(
                _get_custom_policy(no_recompute_list=no_recompute_list),
            )
        ...
        def gn(x, y):
            return smith.sigmoid(smith.selu(smith.matmul(smith.matmul(x, y), y))).relu()
        ...
        fw_compiler = functools.partial(
            count_ops,
            freqs=[2, 1],
            ops=[smith.ops.aten.mm.default, smith.ops.aten.sigmoid.default],
        )
        bw_compiler = functools.partial(
            count_ops,
            freqs=[4, 0],
    ...
    def test_compile_selective_checkpoint_list_ops(self, device, partition_fn):
        ...
        fw_compiler = functools.partial(
            count_ops,
            freqs=[1],
            ops=[smith.ops.aten.cat.default],
        )
        bw_compiler = functools.partial(
            count_ops,
            freqs=[1],
    ...
    @unittest.skipIf(IS_WINDOWS, "smith.compile doesn't work with windows")
    @unittest.skip(
        "In-place op support in selective checkpointing + smith.compile "
        "requires SmithDispatchMode + smith.compile work to complete"
    )
    @requires_cuda_and_triton
    @parametrize(
        "partition_fn",
    ...
    def test_compile_selective_checkpoint_inplace_op(self, device, partition_fn):
        ...
        def gn(x, y):
            return smith.sigmoid(
                smith.selu_(smith.matmul(smith.matmul(x, y), y))
            ).relu_()
    ...
    def test_compile_selective_checkpoint_random_op(self, device, partition_fn):
        for preserve_rng_state in [True, False]:
            ...
            def gn(x):
                return smith.sigmoid(smith.dropout(smith.sigmoid(x), p=0.5, train=True))
            ...
            def fn(x):
                return smith.utils.checkpoint.checkpoint(
                    ...
                    # we will always preserve RNG state when using `smith.compile`.
            ...
            # NOTE: when `preserve_rng_state` is False, gradient will mismatch between smith.compile and eager,
            # because eager version doesn't preserve RNG state while smith.compile still does.
            # Hence when `preserve_rng_state` is False, we skip the output and gradient comparison
            # between smith.compile and eager.
    ...
    def test_compile_selective_checkpoint_invalid_context(self, partition_fn):
        def gn(x, y):
            return smith.sigmoid(smith.matmul(x, y)) * y
        ...
        with self.assertRaisesRegex(
            Exception, "must generate a tuple of two `SmithDispatchMode`s"
        ):
            self._validate(fn, backend, x, y)
    ...
    def test_compile_selective_checkpoint_parametrization(self, partition_fn):
        ...
        class Parametrization(smith.nn.Module):
            ...
            def parametrization(self, x):
                return smith.sigmoid(smith.mul(x, x))
        ...
        fw_compiler = functools.partial(
            count_ops,
            freqs=[1, 1],
            ops=[smith.ops.aten.mul.Tensor, smith.ops.aten.sigmoid.default],
        )
        bw_compiler = functools.partial(
            count_ops,
            freqs=[
        ...
        model_compiled = smith.compile(
            copy.deepcopy(model), backend=backend, fullgraph=True
        )
        input = smith.randn(8, 16, requires_grad=True)
    ...
    def test_autocast_flash_attention(self, device):
        def fn(primals_1, primals_2, primals_3):
            return smith.ops.aten._scaled_dot_product_efficient_attention.default(
        ...
        with smith.autocast(device_type=device):
            x = smith.randn(4, 2, 16, 32, device=device, requires_grad=True)
            y = smith.randn(4, 2, 16, 32, device=device, requires_grad=True)
            z = smith.randn(4, 2, 16, 32, device=device, requires_grad=True)
            args = (x, y, z)

            smith.manual_seed(0)
            ref = gn(*args)

            opt_gn = smith.compile(gn)
            smith.manual_seed(0)
            res = opt_gn(*args)
    ...
    def test_error_msg(self, device):
        ...
        with self.assertRaisesRegex(
            smith._dynamo.exc.Unsupported, "User-inserted graph break"
        ):
            opt_fn(x)
    ...
    def test_list_inputs(self, device):
        ...
        def fn(x, ys):
            return smith.utils.checkpoint.checkpoint(mod, x, ys, use_reentrant=True)

        x = smith.randn(4, 4).to(device)
        y = smith.randn(4, 4).to(device)
        z = smith.randn(4, 4).to(device)
        ref = fn(x, [y, z])
    ...
    def test_pattern_matcher(self, device):
        ...
        dtype = smith.float16
        args1 = [
            smith.randn(tensor_shape, device=device, dtype=dtype, requires_grad=True),
            smith.randn(tensor_shape, device=device, dtype=dtype, requires_grad=True),
            smith.randn(tensor_shape, device=device, dtype=dtype, requires_grad=True),
        ]
        ...
        op1 = smith.ops.aten._scaled_dot_product_flash_attention.default
        op2 = smith.ops.aten._scaled_dot_product_cudnn_attention.default
        self.assertTrue(
            count_ops(
                fwd_graph,
                [],
        ...
        self.assertTrue(count_ops(bwd_graph, [], freq=0, op=smith.ops.aten.sin.default))
        # Check that the sdpa op is recomputed in the backward graph
        self.assertTrue(
            count_ops(
                bwd_graph,
    ...
    def test_dynamo_does_not_trace_getattr_as_top_frame(self):
        ...
        lin = smith.nn.Linear(1, 1)
        mod = smith.nn.Sequential(lin, lin)
        mod = CheckpointWrapper(mod)
        mod._checkpoint_wrapped_module.a = smith.ones(1, 1)
    ...
    def test_return_same_element_twice(self):
        ...
        def fn(x):
            return smith.utils.checkpoint.checkpoint(gn, x, use_reentrant=True)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 4]"):
        ...
        tag_activation_checkpoint = smith.ops.higher_order.tag_activation_checkpoint(wrap_body_0, l_x_, use_reentrant = True);  wrap_body_0 = l_x_ = None
        getitem: "f32[4, 4]" = tag_activation_checkpoint[0];  tag_activation_checkpoint = None
        return (getitem,)

    class wrap_body_0(smith.nn.Module):
        def forward(self, l_x_: "f32[4, 4]"):
            y: "f32[4, 4]" = smith.sin(l_x_);  l_x_ = None
            return (y,)
""",
        )

    @smith._dynamo.config.patch(skip_fwd_side_effects_in_bwd_under_checkpoint=True)
    ...
    def test_nonlocal_list_mutation(self):
        def gn(x, z):
            out = x.sin()
            z.append(out)
            return smith.cos(smith.sin(smith.matmul(x, x) @ x)), out

        def fn(x):
            z = []

            out1, out2 = smith.utils.checkpoint.checkpoint(
                gn,
                x,
                z,
                use_reentrant=False,
    ...
    def test_nonlocal_list_mutation_hidden(self):
        ...
        def fn(x):
            ...
            outs = smith.utils.checkpoint.checkpoint(
                gn,
                x,
                z,
                use_reentrant=False,
...
class GraphModule(smith.nn.Module):
    ...
    class wrap_body_0(smith.nn.Module):
        def forward(self, l_x_: "f32[4, 4]"):
            matmul: "f32[4, 4]" = smith.matmul(l_x_, l_x_)
            o: "f32[4, 4]" = matmul @ l_x_;  matmul = None

            out: "f32[4, 4]" = l_x_.sin()

            sin_1: "f32[4, 4]" = smith.sin(o);  o = None
            cos: "f32[4, 4]" = smith.cos(sin_1);  sin_1 = None
            sin_2: "f32[4, 4]" = smith.sin(l_x_);  l_x_ = None
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[4, 4]"):
        mm: "f32[4, 4]" = smith.ops.aten.mm.default(primals_1, primals_1)
        mm_1: "f32[4, 4]" = smith.ops.aten.mm.default(mm, primals_1);  mm = None

        sin: "f32[4, 4]" = smith.ops.aten.sin.default(primals_1)

        sin_1: "f32[4, 4]" = smith.ops.aten.sin.default(mm_1);  mm_1 = None
        cos: "f32[4, 4]" = smith.ops.aten.cos.default(sin_1);  sin_1 = None
        sin_2: "f32[4, 4]" = smith.ops.aten.sin.default(primals_1)

        add: "f32[4, 4]" = smith.ops.aten.add.Tensor(cos, sin_2);  cos = sin_2 = None
...
""",
    ...
    def test_frozen_dataclass_pytree_output(self):
        ...
        try:
            ...
            input_eager = InputNode(x=smith.randn(2, 4, requires_grad=True))
            smith.manual_seed(0)
            ...
            smith.manual_seed(0)
            compiled_fn = smith.compile(checkpointed_forward, fullgraph=True)
    ...
    def test_checkpoint_with_record_function(self):
        ...
        def gn(x, y):
            with smith.profiler.record_function("matmul_region"):
                return smith.sigmoid(smith.matmul(x, y))
        ...
        fw_compiler = functools.partial(
            count_ops, freq=1, op=smith.ops.profiler._record_function_enter_new.default
        )
        backend = aot_autograd(
            fw_compiler=fw_compiler,
            bw_compiler=nop,
        ...
        with smith._dynamo.config.patch(capture_profiler_record_function=True):
            self._validate(fn, backend, x, y)


class RematerializeACNodesPassTests(smith._dynamo.test_case.TestCase):
    ...
    def test_ac_rematerialize_simple_forward_backward(self):
        ...
        def simple_fwd_bwd(x, y):
            z = smith.utils.checkpoint.checkpoint(
                lambda a, b: smith.sigmoid(smith.matmul(a, b)),
                x,
                y,
                use_reentrant=False,
            )
            ...
            with smith.fx.traceback.annotate({"remat_pass_tag": "is_backward"}):
        ...
        self.assertTrue(smith.allclose(dx1, dx2))
        self.assertTrue(smith.allclose(dy1, dy2))

        mm_with = self.count_op(gm_with, smith.ops.aten.mm.default)
        mm_without = self.count_op(gm_without, smith.ops.aten.mm.default)
        sigmoid_with = self.count_op(gm_with, smith.ops.aten.sigmoid.default)
        sigmoid_without = self.count_op(gm_without, smith.ops.aten.sigmoid.default)
...
def forward(self, arg0_1, arg1_1):
    mm = smith.ops.aten.mm.default(arg0_1, arg1_1)
    sigmoid = smith.ops.aten.sigmoid.default(mm);  mm = None
    sum_1 = smith.ops.aten.sum.default(sigmoid);  sigmoid = None
    ones_like = smith.ops.aten.ones_like.default(sum_1, pin_memory = False, memory_format = smith.preserve_format);  sum_1 = None
    expand = smith.ops.aten.expand.default(ones_like, [4, 4]);  ones_like = None
    mm_recomputed = smith.ops.aten.mm.default(arg0_1, arg1_1)
    sigmoid_recomputed = smith.ops.aten.sigmoid.default(mm_recomputed);  mm_recomputed = None
    detach_recomputed = smith.ops.aten.detach.default(sigmoid_recomputed);  sigmoid_recomputed = None
    detach_2 = smith.ops.aten.detach.default(detach_recomputed);  detach_recomputed = None
    sigmoid_backward = smith.ops.aten.sigmoid_backward.default(expand, detach_2);  expand = detach_2 = None
    t = smith.ops.aten.t.default(arg0_1);  arg0_1 = None
    mm_2 = smith.ops.aten.mm.default(t, sigmoid_backward);  t = None
    t_1 = smith.ops.aten.t.default(arg1_1);  arg1_1 = None
    mm_3 = smith.ops.aten.mm.default(sigmoid_backward, t_1);  sigmoid_backward = t_1 = None
    detach_3 = smith.ops.aten.detach.default(mm_3);  mm_3 = None
    detach_4 = smith.ops.aten.detach.default(mm_2);  mm_2 = None
    ...
    def test_ac_rematerialize_with_rng_ops_raises_error(self):
        ...
        def fwd_bwd_with_rng(x):
            z = smith.utils.checkpoint.checkpoint(
                lambda a: smith.sigmoid(a + smith.rand_like(a)), x, use_reentrant=False
            )
            loss = z.sum()
    ...
    def test_ac_rematerialize_with_no_annotations_returns_unchanged(self):
        ...
        def fwd_bwd(x):
            z = smith.utils.checkpoint.checkpoint(
                lambda a: smith.sigmoid(a + 4), x, use_reentrant=False
            )
            loss = z.sum()
    ...
    def test_ac_rematerialize_with_selective_checkpoint_policy(self):
        x = smith.randn(4, 128, requires_grad=True)
        w1 = smith.randn(128, 128, requires_grad=True)
        b1 = smith.randn(128, requires_grad=True)

        def policy_fn(ctx, op, *args, **kwargs):
            if op == smith.ops.aten.addmm.default:
        ...
        def fwd_bwd_with_policy(x, w1, b1):
            def checkpoint_fn(inp, w, b):
                linear = smith.nn.functional.linear(inp, w, b)
                return smith.relu(linear)
        ...
        smith.testing.assert_close(result_with[0], result_without[0])
        smith.testing.assert_close(result_with[1], result_without[1])
        smith.testing.assert_close(result_with[2], result_without[2])

        addmm_without = self.count_op(gm_without, smith.ops.aten.addmm.default)
        relu_without = self.count_op(gm_without, smith.ops.aten.relu.default)

        addmm_with = self.count_op(gm_with, smith.ops.aten.addmm.default)
        relu_with = self.count_op(gm_with, smith.ops.aten.relu.default)
    ...
    def _compile_with_joint_graph_pass_and_capture(self, fn, inputs):
        from smith._inductor.fx_passes.joint_graph import joint_graph_passes
    ...
    def test_joint_graph_passes_view_optimization(self):
        ...
        result_eager = smith.autograd.grad(smith.sigmoid(x).sum(), x)[0]
        self.assertTrue(smith.allclose(result, result_eager, atol=1e-5))

        view_count_before = self.count_op(gm_before, smith.ops.aten.view.default)
        view_count_after = self.count_op(gm_after, smith.ops.aten.view.default)
...
def forward(self, arg0_1):
    sigmoid = smith.ops.aten.sigmoid.default(arg0_1)
    sum_1 = smith.ops.aten.sum.default(sigmoid);  sigmoid = None
    ones_like = smith.ops.aten.ones_like.default(sum_1, pin_memory = False, memory_format = smith.preserve_format);  sum_1 = None
    expand = smith.ops.aten.expand.default(ones_like, [4, 4]);  ones_like = None
    sigmoid_recomputed = smith.ops.aten.sigmoid.default(arg0_1);  arg0_1 = None
    detach_recomputed = smith.ops.aten.detach.default(sigmoid_recomputed);  sigmoid_recomputed = None
    detach_2 = smith.ops.aten.detach.default(detach_recomputed);  detach_recomputed = None
    sigmoid_backward = smith.ops.aten.sigmoid_backward.default(expand, detach_2);  expand = detach_2 = None
    detach_3 = smith.ops.aten.detach.default(sigmoid_backward);  sigmoid_backward = None
    return (detach_3,)""",
        )
    ...
    def test_joint_graph_passes_permute_optimization(self):
        ...
        permute_count_before = self.count_op(gm_before, smith.ops.aten.permute.default)
        permute_count_after = self.count_op(gm_after, smith.ops.aten.permute.default)
