...
from smith._dynamo.symbolic_convert import SpeculationLog, SpeculationLogDivergence
...
class ExceptionTests(smith._dynamo.test_case.TestCase):
    ...
    def test_autocast_with_exception(self):
        class Optimizer(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                raise NotImplementedError("Not implemented")
        ...
        def f(x: smith.Tensor):
            try:
                with smith.autocast(device_type="cpu", dtype=None):
                    Optimizer.apply(x)
            except NotImplementedError:
                return x + 1
        ...
        inp = smith.ones(3)
        out = f(inp)
        self.assertTrue(smith.equal(out, inp + 1))
    ...
    def test_custom_getattr_on_module_exception(self):
        class Foo(smith.nn.Module):
            def __init__(self, a=3):
                super().__init__()
                self.register_parameter("a", smith.nn.Parameter(smith.ones(4) * 2))
    ...
    def test_nn_reraise(self):
        ...
        opt_call = smith.compile(lambda x: m(x), backend="eager")
        self.assertRaises(ValueError, lambda: opt_call(smith.randn(3)))
    ...
    def test_stack_trace_from_observed_exception(self):
        class Model(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                return smith.nn.functional.linear(x, weight)

        x = (smith.randn(4, 16, requires_grad=True),)

        with self.assertRaisesRegex(Exception, "weight = self.linear.w"):
            smith._dynamo.functional_export.dynamo_graph_capture_for_export(Model())(x)

    def test_context_manager_preserves_exception_stack(self):
        # Regression test for https://github.com/blacksmith/blacksmith/issues/167900
