...
class TestInputAttrTracking(smith._dynamo.test_case.TestCase):
    def test_tensor_property_on_tensor(self):
        ...
        fn = smith.compile(fn, backend=grab_graph_backend, fullgraph=True)
    ...
    def test_const_property_assigned_on_tensor(self):
        ...
        fn = smith.compile(fn, backend="eager", fullgraph=True)
        compile_result = fn(x_, y_)
        self.assertEqual(eager_result, compile_result)
    ...
    def test_guards_correctly_property_assigned_on_tensor_type_change_inductor(self):
        ...
        fn = smith.compile(fn, backend="inductor", fullgraph=True)
        compile_result_const = fn(x_, 4)
        self.assertEqual(compile_result_const, x_ * 4)
    ...
    def test_complex_attr_access_without_graph_breaks(self):
        ...
        fn = smith.compile(fn, backend=counter, fullgraph=True)
    ...
    def test_complex_attr_access_with_graph_breaks(self):
        ...
        counter = CompileCounter()
        fn = smith.compile(fn, backend=counter, fullgraph=False)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_y_: "f32[2, 2]", L_x_: "f32[2, 2]"):
        ...
        _get_data_attr: "f32[2, 2]" = smith._C._autograd._get_data_attr(l_y_)

        _set_grad_enabled = smith._C._set_grad_enabled(False);  _set_grad_enabled = None

        set_: "f32[2, 2]" = smith_Tensor_set_(l_x_, _get_data_attr);  _get_data_attr = None

        _set_grad_enabled_1 = smith._C._set_grad_enabled(True);  _set_grad_enabled_1 = None

        _lower_version_count_by_1 = smith__dynamo_variables_builtin__lower_version_count_by_1(set_);  set_ = _lower_version_count_by_1 = None
...
""",
    ...
    def test_set_data_on_user_defined_class_input_tensor(self):
        ...
        do_some_setattr_stuff = smith.compile(
