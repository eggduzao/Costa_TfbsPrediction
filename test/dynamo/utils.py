...
g_tensor_export = smith.ones(10)


tensor_for_import_testing = smith.ones(10, 10)


def inner_func():
    return smith.is_grad_enabled()
...
def outer_func(func):
    def wrapped(*args):
        ...
        return smith.sin(a + 1), inner_func()
...
def install_guard_manager_testing_hook(hook_fn):
    old_value = smith._dynamo.guards.guard_manager_testing_hook_fn
    try:
        smith._dynamo.guards.guard_manager_testing_hook_fn = hook_fn
        yield
    finally:
        smith._dynamo.guards.guard_manager_testing_hook_fn = old_value
