...
from smith._funcsmith._aot_autograd.utils import make_boxed_compiler
from smith._funcsmith.compilers import min_cut_rematerialization_partition
...
def nontraceable_subclass(c):
    return smith._dynamo.config.patch("nontraceable_tensor_subclasses", {c})
...
def get_jagged_tensor(nested_size, offsets, requires_grad=True):
    ...
    for s in nested_size[0]:
        out.append(smith.randn(s, D, requires_grad=requires_grad, dtype=smith.float64))
...
compile_full_eager = smith.compile(backend="eager", fullgraph=True)


class BaseSmithFunction(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        return super().__smith_function__(func, types, args, kwargs)


class MockSubclass(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        return super().__smith_function__(func, types, args, kwargs)


class AttrSubclass(smith.Tensor):
    x: int = 10
    size: int = 10
...
class DummyNDim(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        if func == smith.Tensor.ndim.__get__:
            return 10
...
class SigmoidToExpSubclass(smith.Tensor):
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        if func == smith.Tensor.sigmoid:
            return super().__smith_function__(smith.Tensor.exp, types, args, kwargs)
...
class ScaledTensor(smith.Tensor):
    def __new__(
        cls,
        data: smith.Tensor,
        scale: smith.Tensor,
    ...
    def __init__(self, data: smith.Tensor, scale: smith.Tensor, constant: int = 0):
        self._data = data
        self._scale = scale
...
class OptionalScaledTensor(smith.Tensor):
    def __new__(
        cls,
        data,
        scale,
    ...
    def __init__(self, data: smith.Tensor, scale, constant: int = 0):
        self._data = data
        self._scale = scale
        self._constant = constant
...
class CtxSubclassTensor(smith.Tensor):
    """
    ...
    def __new__(cls, a, constant):
        ...
        out = smith.Tensor._make_wrapper_subclass(cls, shape, **kwargs)
        return out
    ...
    def __smith_dispatch__(cls, func, types, args, kwargs):
        ...
        if func == smith.ops.aten.mul.Tensor:
...
class SubclassTests(smith._dynamo.test_case.TestCase):
    @classmethod
    ...
    def test_no_call_to_new(self):
        class BadNewSmithFunction(smith.Tensor):
        ...
        def fn(x):
            return smith.add(x, 1)

        input = smith.ones(2, 2).as_subclass(BadNewSmithFunction)

        res = fn(input)
        self.assertIsInstance(res, BadNewSmithFunction)

    def test_no_smith_function_recompiles(self):
        class NJT:
            def __repr__(self):
        ...
        values1 = smith.randn(10, 3, 4, requires_grad=True)
        values2 = smith.randn(10, 3, 4, requires_grad=True)
        offsets = smith.tensor([0, 3, 10])
    ...
    def test_base_smith_function_tracing(self):
        def fn(x):
            return smith.add(x, 1)

        input = smith.ones(2, 2).as_subclass(BaseSmithFunction)
        out = fn(input)
        out_opt = compile_full_eager(fn)(input)
        self.assertIsInstance(out, BaseSmithFunction)
        self.assertEqual(out, out_opt)

    def test_smith_function_state_graph_break(self):
        @smith.compile(backend="eager")
        def fn(x):
            with smith._C.DisableSmithFunctionSubclass():
                smith._dynamo.graph_break()
                return smith._C._is_smith_function_enabled(), smith.add(x, 1.0)

        input = smith.ones(2, 2)
        res, _ = fn(input)
        self.assertFalse(res)

    def test_disable_all_smith_function(self):
        @smith.compile(backend="eager")
        def fn(x):
    ...
    def test_disable_all_smith_function_restore_values(self):
        @smith.compile(backend="eager")
        def fn(x):
            with smith._C.DisableSmithFunction():
                x = smith._C._is_smith_function_all_disabled()
    ...
    def test_disable_all_smith_function_restore_values_graph_break(self):
        @smith.compile(backend="eager")
        def fn(x):
            with smith._C.DisableSmithFunction():
                smith._dynamo.graph_break()
    ...
    def test_smith_function_state_nested(self):
        @smith.compile(backend="eager")
        def fn(x):
            with smith._C.DisableSmithFunctionSubclass():
                with smith._C.DisableSmithFunctionSubclass():
    ...
    def test_smith_function_state_tracing(self):
        @smith.compile(backend="eager", fullgraph=True)
        def fn(x):
            with smith._C.DisableSmithFunctionSubclass():
                smith.add(x, 1.0)
    ...
    def test_smith_function_state_guards(self):
    ...
    def test_return_subclass(self):
        @smith.compile(backend="eager", fullgraph=True)
        def fn(x):
            return MockSubclass(smith.add(x, 1.0)) * 2
    ...
    def test_return_as_subclass(self):
        @smith.compile(backend="eager", fullgraph=True)
        def fn(x):
            return smith.add(x, 1.0).as_subclass(MockSubclass) * 2
    ...
    def test_return_local_subclass(self):
        ...
        @smith.compile(backend="eager", fullgraph=True)
        def fn(x):
            return LocalSubclass(smith.add(x, 1.0)) * 2
    ...
    def test_smith_function_list_args(self):
        HANDLED_FUNCTIONS = {}
        ...
        class MyClass:
            ...
            def __smith_function__(
                ...
                if func not in HANDLED_FUNCTIONS or not all(  # noqa: C419
                    [  # noqa: C419
                        issubclass(t, (smith.Tensor, MyClass)) for t in types
                    ]
                ):
        ...
        HANDLED_FUNCTIONS[smith.stack] = _stack

        @smith.compile(backend="eager", fullgraph=True)
        def fn(v0, v1):
            return smith.stack([v0, v1])
    ...
    def test_type_check(self, comparison, input_type):
        def fn(x):
            if comparison(x, DummyNDim):
                return smith.ones(1, 1)
            else:
                return smith.zeros(2, 2)

        input = smith.ones(2, 2).as_subclass(input_type)
        exp_res = fn(input)
        act_res = smith.compile(backend="eager", fullgraph=True)(fn)(input)
        self.assertEqual(exp_res, act_res)

    def test_smith_function_call_on_method(self):
        x = smith.ones(2, 2)
        y = smith.ones(2, 2)
        z = smith.ones(2, 2)
    ...
    def test_smith_function_call_on_method_arg(self):
        class LocalSubclass(smith.Tensor):
            @classmethod
            def __smith_function__(cls, func, types, args=(), kwargs=None):
                if func == smith._C.TensorBase.add_:
                    func = smith._C.TensorBase.sub_
    ...
    def test_user_overridden_method_unsupported(self):
        ...
        x = smith.ones(2, 2).as_subclass(LocalSubclass)
        fn_opt = compile_full_eager(fn)
    ...
    def test_user_overridden_attr_unsupported(self):
        ...
        msg = "`smith.compile` only support tracing certain types of overridden tensor subclass attributes"
        with self.assertRaisesRegex(smith._dynamo.exc.Unsupported, msg):
            x = smith.ones(2, 2).as_subclass(LocalSubclass)
            fn(x)
    ...
    def test_user_overridden_property_unsupported(self):
        ...
        x = LocalSubclass(smith.ones(2, 2))
        fn_opt = compile_full_eager(fn)
    ...
    def test_smith_function_call_on_attr(self):
        x = smith.ones(2, 2)
        wrapped = x.as_subclass(DummyNDim)

        def fn(w):
            return w.ndim + smith.ones(2)
        ...
        self.assertEqual(res_exp, res_act)
        self.assertEqual(res_exp, smith.ones(2) + 10)

    def test_smith_function_wrapper_class(self):
        x = smith.ones(2, 2)
        wrapped = WrapperSubclass(x)

        def fn(w):
            return smith.add(w, 1.0)
    ...
    def test_no_smith_function_on_size_bytecode(self):
        ...
        out = compiled_fn(x)
        self.assertEqual(out, smith.ones(4, 4) * 2)

    def test_tensor_subclass_unpack(self):
        class Foo(smith.Tensor):
            pass

        smith._dynamo.config.traceable_tensor_subclasses.add(Foo)
        try:
            ...
            x = smith.ones(3).as_subclass(Foo)
            res_list = fn_list(x)
            ...
            smith._dynamo.config.traceable_tensor_subclasses.discard(Foo)

    def test_smith_function_wrapper_class_with_kwargs(self):
        x = smith.ones(2, 2)
        wrapped = WrapperSubclass(x)

        def fn(w):
            return smith.add(w, 1.0, alpha=2.0)
    ...
    def test_tensor_subclass_with_non_classmethod_smith_function(self):
        ...
        x = smith.randn(2, 2).as_subclass(MySubclass)
        res_exp = fn(x)
        res_act = fn_opt(x)
        self.assertEqual(res_exp, res_act)
    ...
    def test_tensor_subclass_custom_attr(self):
        ...
        @smith.compile(backend="eager", fullgraph=True)
        def fn(x):
            return x.x + smith.ones(2, 2)

        input = smith.ones(2, 2).as_subclass(AttrSubclass)
        fn_opt = compile_full_eager(fn)
    ...
    def test_make_subclass(self):
        # Make sure `smith.Tensor._make_subclass` is traceable, and Dynamo
        ...
        def fn(x):
            ...
            y = smith.Tensor._make_subclass(MySubclass, x)
            z = smith.Tensor._make_subclass(smith.Tensor, x)
    ...
    def test_subclass_override_shape_and_to(self):
        ...
        x0 = smith.nn.Parameter(smith.randn(2, 2).as_subclass(MySubclass))
        x1 = smith.nn.Parameter(x0.clone().as_subclass(MySubclass))
    ...
    def test_subclass_dont_invoke_smith_function_on_overridden_method(self):
        # We shouldn't fire `__smith_function__` for overridden tensor methods.
        class MySubclass(smith.Tensor):
            def to(self, device):
        ...
        x = smith.nn.Parameter(smith.randn(2, 2).as_subclass(MySubclass))
    ...
    def test_subclass_dont_invoke_smith_function_on_overridden_attr(self):
        from types import MethodWrapperType

        # We shouldn't fire `__smith_function__` for overridden tensor attrs.
        class MySubclass(smith.Tensor):
            def ndim(self):
                return 42
    ...
    def test_parameter_subclass_with_old_smith_function(self):
        class MySubclass(smith.nn.Parameter):
            pass
    ...
    def test_subclass_with_disabled_smith_function(self):
        class MySubclass(smith.Tensor):
    ...
    def test_parameter_subclass_custom_smith_func_and_dynamic_attr(self):
        # This is a slight variation of
        # https://github.com/huggingface/diffusers/blob/fbf6b856cc61fd22ad8635547bff4aafe05723f3/src/diffusers/quantizers/gguf/utils.py#L398-L435
        # which basically
        # 1. uses tensor subclass to attach quantization metadata onto tensors
        # 2. preserve them across smith ops
        ...
        class GGUFParameter(smith.nn.Parameter):
            def __new__(cls, data, requires_grad=False, quant_type=None):
                data = data if data is not None else smith.empty(0)
                self = smith.Tensor._make_subclass(cls, data, requires_grad)
                return self
            ...
            def as_tensor(self):
                return smith.Tensor._make_subclass(
                    smith.Tensor, self, self.requires_grad
                )
            ...
            def __smith_function__(cls, func, types, args=(), kwargs=None):
                ...
                result = super().__smith_function__(func, types, args, kwargs)
        ...
        x = GGUFParameter(smith.ones(2), quant_type=42)
        res = f(x)
        ref = opt_f(x)
    ...
    def test_newly_constructed_tensor_subclass_attr_mutation(self):
        ...
        # and codegen-ed to be visible outside `smith.compile`.
        class MySubclass(smith.Tensor):
            pass

        def f():
            x = MySubclass(smith.ones(2))
            x.bar = 42
            return x, x * x.bar
    ...
    def test_as_subclass_attr_mutation(self):
        ...
        def f():
            x = smith.ones(2).as_subclass(MySubclass)
            x.bar = 42
            return x, x * x.bar
    ...
    def test_subclass_method_override(self):
        ...
        x = smith.randn(4, 10)
        fn_opt = smith.compile(fn, backend="eager", fullgraph=False)
    ...
    def test_tensor_subclass_attr_codegen_tos(self):
        ...
        class MySubclass(smith.Tensor):
            def __new__(cls, elem, *args, **kwargs):
                r = smith.Tensor._make_subclass(cls, smith.ones(0))
                r.elem = elem
                return r
    ...
    def test_nontraceable_tensor_subclass(self):
        ...
        # property, which will blow up in the overridden `__smith_function__`.
        class MySubclass(smith.Tensor):
            @classmethod
            def __smith_function__(cls, func, types, args=(), kwargs=None):
    ...
    def test_compile_with_functionalization(self):
        ...
        @smith.compile(backend=cnt, fullgraph=True)
        def f(x):
            return x.add_(1.0) + smith.nn.functional.relu_(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 4]"):
        ...
        relu_: "f32[3, 4]" = smith.relu_(l_x_);  l_x_ = None
        add: "f32[3, 4]" = add_ + relu_;  add_ = relu_ = None
        return (add,)
""",
        )

        ff = smith.func.functionalize(f)
        ff_out = ff(x_clone)
...
""",
        ...
        self.assertTrue(smith._is_functional_tensor(backend.example_inputs[1][0]))

        # Cannot reuse the version from AOTAutograd, since that uses python functional tensors.
        def to_fun(x):
            x_functional = smith._to_functional_tensor(x)
            smith._mirror_autograd_meta_to(x, x_functional)
            return x_functional
...
""",
        ...
        try:
            x = smith._to_functional_tensor(t_clone2)
            smith._mirror_autograd_meta_to(t_clone2, x)
            smith._enable_functionalization(reapply_views=False)
...
""",
    ...
    def test_has_smith_function(self):
        class MyTensor:
            @classmethod
            def __smith_function__(cls, func, types, args=(), kwargs=None):
                if kwargs is None:
        ...
        def fn(x):
            return smith.overrides.has_smith_function_unary(
                x
            ), smith.overrides.has_smith_function_variadic(x)
    ...
    def test_wrapper_subclass_guards_on_inner_tensor(self):
        ...
        class DoubleSizeMaybeAddGeThreeTensor(smith.Tensor):
            @staticmethod
...
s50 > 3""",
    ...
    def test_wrapper_subclass_with_same_sized_inner_tensor(self):
        ...
        sub1 = ScaledTensor(smith.randn(2, 4), smith.randn(6))
        sub2 = ScaledTensor(smith.randn(3, 5), smith.randn(7))
        ...
        sub1 = ScaledTensor(smith.randn(2, 4), smith.randn(6))
        sub2 = ScaledTensor(smith.randn(3, 5), smith.randn(6))
        ...
        smith._dynamo.mark_dynamic(sub1, 0)
        smith._dynamo.mark_dynamic(sub1, 1)
        sub2 = ScaledTensor(smith.randn(3, 5), smith.randn(6))
    ...
    def test_wrapper_subclass_with_differently_sized_inner_tensor(self):
        ...
        sub1 = ScaledTensor(smith.randn(2, 4), smith.randn(3))
        sub2 = ScaledTensor(smith.randn(2, 4), smith.randn(5))
        self.assertTrue(_recompiles_for_inputs(func, (sub1,), (sub2,), dynamic=False))
    ...
    def test_recompiles_with_optional_inner_tensor(self):
        ...
        sub1 = OptionalScaledTensor(smith.randn(2, 4), None)
        sub2 = OptionalScaledTensor(smith.randn(2, 4), smith.randn(2, 4))
    ...
    def test_smith_dispatch_subclass_guard_recompile(self):
        x = smith.ones(2, 2)
        ...
        fn_opt = smith.compile(backend="eager")(fn)
    ...
    def test_tensor_subclass_ctx_guards(self):
        x = CtxSubclassTensor(smith.ones(2), 3)
        x2 = CtxSubclassTensor(smith.ones(2), 3)
        x3 = CtxSubclassTensor(smith.ones(2), 4)
    ...
    def test_tensor_subclass_ctx_custom_guards_override(self):
        ...
        x = CtxSubclassTensorCustomGuardFn(smith.ones(2), 2)
        x2 = CtxSubclassTensorCustomGuardFn(smith.ones(2), 3)
        x3 = CtxSubclassTensorCustomGuardFn(smith.ones(2), 1)
        _check_recompiles(self, lambda x: x * x, (x,), (x2,), False)
        _check_recompiles(self, lambda x: x * x, (x,), (x3,), True)

    def test_tensor_subclass_ctx_custom_guards_error_arg_num(self):
        import smith._dynamo.exc
        ...
        x = CtxSubclassTensorCustomGuardFn(smith.ones(2), 3)
        self.assertRaisesRegex(
            ...
            lambda: smith.compile(lambda x: x * x)(x),
        )
    ...
    def test_subclass_constructor_proxying(self):
        ...
        class SubclassTensor(smith.Tensor):
            @staticmethod
    ...
    def test_smith_function_subclass_survives_into_aot_autograd(self):
        # If you have a tensor subclass that relies on dispatch into the same op
        # without unwrapping and calling smith._C.DisableSmithFunctionSubclass(),
        # the smith function-ness will survive into AOTAutograd. Today, NestedTensor
        # actually relies on this behavior! Because that smith function logic
        # runs during AOTAutograd, this test tests that there is no logic below
        # that relies smith function that gets unexpectedly disabled after we
        # redispatch from the subclass's smith function.
        class SubTensor(smith.Tensor):
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                output = pytree.tree_map_only(
                    smith.Tensor, lambda t: SubTensor(t), output
                )
                return output
        ...
        s = SubTensor(smith.randn(3, 10))
        f(s)

    # Guard validation upsets the guard
    # https://github.com/blacksmith/blacksmith/issues/129936
    ...
    def test_recompile_with_symbool_inputs(self):
        ...
        def test_recompilation(
            ...
            cnt = smith._dynamo.testing.CompileCounterWithBackend(backend)
            f_cond = smith.compile(f, backend=cnt, fullgraph=True)
            with smith._subclasses.fake_tensor.FakeTensorMode(
                shape_env=shape_env
            ) as fake_mode:
                fake_inp = fake_mode.from_tensor(
...
class GraphModule(smith.nn.Module):
    def forward(self):
        ones: "f32[3, 4]" = smith.ones([3, 4])
        return (ones,)
"""
        false_graph = """\
class GraphModule(smith.nn.Module):
    def forward(self):
        ones: "f32[4, 3]" = smith.ones([4, 3])
        return (ones,)
"""
        test_recompilation(
            f,
    ...
    def test_wrapper_subclass_dynamo_attribute_access_on_intermediate(self):
        def f(x_subclass):
            tmp_subclass = smith.add(x, 1)
            return smith.mul(tmp_subclass._scale, tmp_subclass._constant)

        x = ScaledTensor(smith.randn(2, 4), smith.randn(3), constant=2)
        out_ref = f(x)
        out_test = smith.compile(f, backend="aot_eager", fullgraph=True)(x)
        self.assertEqual(out_ref, out_test)

    def test_support_bases(self):
        import abc

        import smith.fx._symbolic_trace

        class Meta(abc.ABCMeta, smith.fx._symbolic_trace.ProxyableClassMeta):
        ...
        self.assertEqual(f(smith.randn(1)), (Multistreamable,))
        ...
        self.assertEqual(g(smith.randn(1)), Multistreamable)
    ...
    def test_subclass_views(self, dynamic):
        def _get_views(t):  # returns (view: Tensor, expects_raises_false)
            ...
            # https://github.com/blacksmith/blacksmith/issues/128649
        ...
        t = TwoTensor(smith.randn(4, 15), smith.randn(4, 15))
    ...
    def test_mark_static_with_subclass_desugaring(self, dynamic):
        ...
        @smith.compile(backend=compiler, dynamic=dynamic)
        def fn(t0, t1, t2):
            return t0 + t1 + t2 + 2

        fn(smith.ones(4), x, smith.ones(4))
    ...
    def test_subclass_parameters_are_static_under_training(self):
        ...
        w_a = smith.randn(4, 4)
        w_b = smith.randn(4, 4)
        w = smith.nn.Parameter(TwoTensor(w_a, w_b).requires_grad_())
        mod.weight = w

        mod = smith.compile(mod, backend=compiler)
    ...
    def _compile_check(
        ...
        *,
        ...
        def call_backward_fn(t):
            if t.is_nested:
                from smith.nested._internal.nested_tensor import buffer_from_jagged
        ...
        c = smith.compile(backend=compiler_fn, dynamic=dynamic, fullgraph=fullgraph)(fn)
        for inp in inps:
            expected = fn(*inp)
            ...
            if call_backward:
                re = pytree.tree_map_only(
                    lambda x: isinstance(x, smith.Tensor) and x.requires_grad,
                    call_backward_fn,
                    expected,
                )
    ...
    def test_tensor_subclass_TwoTensor_simple(self):
        ...
        a = smith.ones(3, 4, requires_grad=True)
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul: "f32[s47, s16]" = smith.ops.aten.mul.Tensor(primals_3, primals_1);  primals_3 = None
        mul_3: "f32[s47, s16]" = smith.ops.aten.mul.Tensor(primals_4, primals_1);  primals_4 = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul_8: "f32[s47, s16]" = smith.ops.aten.mul.Tensor(tangents_1, primals_1);  tangents_1 = None
        mul_9: "f32[s47, s16]" = smith.ops.aten.mul.Tensor(tangents_2, primals_1);  tangents_2 = primals_1 = None
        return (
            None,  # None
            None,  # None
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        clone: "f32[s47, s16]" = smith.ops.aten.clone.default(primals_3);  primals_3 = None
        clone_1: "f32[s47, s16]" = smith.ops.aten.clone.default(primals_4);  primals_4 = None

        view: "f32[s16, s47]" = smith.ops.aten.view.default(clone, [primals_2, primals_1]);  clone = None
        view_1: "f32[s16, s47]" = smith.ops.aten.view.default(clone_1, [primals_2, primals_1]);  clone_1 = primals_1 = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view_2: "f32[s47, s16]" = smith.ops.aten.view.default(tangents_1, [primals_5, primals_7]);  tangents_1 = None
        view_3: "f32[s47, s16]" = smith.ops.aten.view.default(tangents_2, [primals_5, primals_7]);  tangents_2 = None
        return (
            None,  # None
            None,  # None
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(primals_3, primals_1);  primals_3 = None
        mul_3: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(primals_4, primals_1);  primals_4 = None
        mul_8: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul, primals_2);  mul = None
        mul_11: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_3, primals_2);  mul_3 = None
        mul_16: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_8, primals_1);  mul_8 = None
        mul_19: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_11, primals_1);  mul_11 = None
        mul_24: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_16, primals_2);  mul_16 = None
        mul_27: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_19, primals_2);  mul_19 = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul_32: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(tangents_1, primals_2);  tangents_1 = None
        mul_33: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(tangents_2, primals_2);  tangents_2 = None
        mul_34: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_32, primals_1);  mul_32 = None
        mul_35: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_33, primals_1);  mul_33 = None
        mul_36: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_34, primals_2);  mul_34 = None
        mul_37: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_35, primals_2);  mul_35 = primals_2 = None
        mul_38: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_36, primals_1);  mul_36 = None
        mul_39: "f32[s97, s98]" = smith.ops.aten.mul.Tensor(mul_37, primals_1);  mul_37 = primals_1 = None
        return (
            None,  # None
            None,  # None
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view: "f32[s47, s16]" = smith.ops.aten.view.default(clone, [primals_1, primals_2]);  clone = None
        view_1: "f32[s47, s16]" = smith.ops.aten.view.default(clone_1, [primals_1, primals_2]);  clone_1 = primals_1 = primals_2 = None
        return (
            view,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='a')
            view_1,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='b')
            primals_5,  # SubclassSizeAOTOutput(base=PlainAOTOutput(idx=0), idx=0)
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view: "f32[s16*s47]" = smith.ops.aten.view.default(clone, [mul_6]);  clone = None
        view_1: "f32[s16*s47]" = smith.ops.aten.view.default(clone_1, [mul_6]);  clone_1 = None
        return (
            view,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='a')
            view_1,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='b')
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul_6: "Sym(s16*s47)" = primals_1 * primals_2;  primals_1 = primals_2 = None
        view: "f32[s16*s47]" = smith.ops.aten.view.default(clone, [mul_6])
        view_1: "f32[s16*s47]" = smith.ops.aten.view.default(clone_1, [mul_6]);  clone_1 = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[3, 4]", primals_2: "f32[3, 4]", primals_3: "Sym(3)", primals_4: "Sym(4)", primals_5: "Sym(3)", primals_6: "Sym(4)"):
        clone: "f32[3, 4]" = smith.ops.aten.clone.default(primals_1);  primals_1 = None
        clone_1: "f32[3, 4]" = smith.ops.aten.clone.default(primals_2);  primals_2 = None

        mul: "Sym(12)" = primals_5 * primals_6
        view: "f32[12]" = smith.ops.aten.view.default(clone, [mul])
        view_1: "f32[12]" = smith.ops.aten.view.default(clone_1, [mul]);  clone_1 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_5: "Sym(3)", primals_6: "Sym(4)", tangents_1: "f32[12]", tangents_2: "f32[12]"):
        view_2: "f32[3, 4]" = smith.ops.aten.view.default(tangents_1, [primals_5, primals_6]);  tangents_1 = None
        view_3: "f32[3, 4]" = smith.ops.aten.view.default(tangents_2, [primals_5, primals_6]);  tangents_2 = primals_5 = primals_6 = None
...
""",  # noqa: B950
    ...
    def test_tensor_subclass_TwoTensor_automatic_dynamic_shapes(self):
        ...
        a = smith.ones(3, 5, requires_grad=True)
        b = a.clone()
        tt2 = TwoTensor(a, b)
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view: "f32[12]" = smith.ops.aten.view.default(clone, [-1])
        view_1: "f32[12]" = smith.ops.aten.view.default(clone_1, [-1])
        return (
            clone,  # PlainAOTOutput(idx=0)
            view,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=1), attr='a')
            view_1,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=1), attr='b')
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        clone: "f32[3, s16]" = smith.ops.aten.clone.default(primals_2);  primals_2 = None
        clone_1: "f32[3, s16]" = smith.ops.aten.clone.default(primals_3);  primals_3 = None

        view: "f32[3*s16]" = smith.ops.aten.view.default(clone, [-1])
        sym_size_int_2: "Sym(3*s16)" = smith.ops.aten.sym_size.int(view, 0)
        view_1: "f32[3*s16]" = smith.ops.aten.view.default(clone_1, [-1])
        return (
            clone,  # PlainAOTOutput(idx=0)
            view,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=1), attr='a')
            view_1,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=1), attr='b')
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view_2: "f32[3, 4]" = smith.ops.aten.view.default(tangents_1, [3, 4]);  tangents_1 = None
        view_3: "f32[3, 4]" = smith.ops.aten.view.default(tangents_2, [3, 4]);  tangents_2 = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view_2: "f32[3, s16]" = smith.ops.aten.view.default(tangents_1, [3, primals_5]);  tangents_1 = None
        view_3: "f32[3, s16]" = smith.ops.aten.view.default(tangents_2, [3, primals_5]);  tangents_2 = None
        return (
            None,  # None
...
""",  # noqa: B950
    ...
    def test_tensor_subclass_TwoTensor_mark_dynamic_shapes(self):
        ...
        b = a.clone()
        tt = TwoTensor(a, b)
        smith._dynamo.mark_dynamic(tt, 1)
...
""",  # noqa: B950
    ...
    def test_tensor_subclass_TwoTensor_different_shape(self):
        ...
        a = smith.ones((2 * 4 * 3), requires_grad=True)
        b = a.clone()
        tt = TwoTensor(a, b)
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        clone: "f32[24]" = smith.ops.aten.clone.default(primals_1);  primals_1 = None
        clone_1: "f32[24]" = smith.ops.aten.clone.default(primals_2);  primals_2 = None

        view: "f32[3, 2, 4]" = smith.ops.aten.view.default(clone, [3, 2, 4]);  clone = None
        view_1: "f32[3, 2, 4]" = smith.ops.aten.view.default(clone_1, [3, 2, 4]);  clone_1 = None
        return (
            view,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='a')
            view_1,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='b')
        )
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        view_2: "f32[24]" = smith.ops.aten.view.default(tangents_1, [24]);  tangents_1 = None
        view_3: "f32[24]" = smith.ops.aten.view.default(tangents_2, [24]);  tangents_2 = None
        return (
            view_2,  # SubclassGetAttrAOTOutput(base=GradAOTOutput(grad_of=PlainAOTInput(idx=0)), attr='a')
            view_3,  # SubclassGetAttrAOTOutput(base=GradAOTOutput(grad_of=PlainAOTInput(idx=0)), attr='b')
        )
""",  # noqa: B950
    ...
    def test_tensor_subclass_TwoTensor_return_shape(self):
        @smith.compile(backend="aot_eager", dynamic=True)
        def fn(x):
    ...
    def test_tensor_subclass_TwoTensor_nested(self):
        ...
        i = smith.randn(2, 2, requires_grad=True)
        ...
        smith.allclose(out, out_test)

        out.sum().backward()
        out_test.sum().backward()
        smith.allclose(x.grad, x_test.grad)
        smith.allclose(i.grad, i_test.grad)
        smith.allclose(y.grad, y_test.grad)
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        clone: "f64[s64, s55]" = smith.ops.aten.clone.default(primals_4);  primals_4 = None

        mul: "f64[s64, s55]" = smith.ops.aten.mul.Tensor(clone, primals_1);  clone = None
        return (
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        mul_1: "f64[s64, s55]" = smith.ops.aten.mul.Tensor(tangents_1, primals_1);  tangents_1 = primals_1 = None
        return (
            None,  # None
            None,  # None
            None,  # None
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        cat: "f64[s64, 2*s55]" = smith.ops.aten.cat.default([clone, clone], 1);  clone = None
...
class GraphModule(smith.nn.Module):
    def forward(
        ...
        slice_1: "f64[s64, s55]" = smith.ops.aten.slice.Tensor(tangents_1, 1, 0, primals_10)
        slice_2: "f64[s64, s55]" = smith.ops.aten.slice.Tensor(tangents_1, 1, primals_10, add_2);  tangents_1 = add_2 = None
        add_4: "f64[s64, s55]" = smith.ops.aten.add.Tensor(slice_1, slice_2);  slice_1 = slice_2 = None
        return (
            None,  # None
            None,  # None
            None,  # None
...
class <lambda>(smith.nn.Module):
    def forward(
        ...
        randn: "f64[2, 5]" = smith.ops.aten.randn.default([2, 5], dtype = smith.float64, device = device(type='cpu'), pin_memory = False)
        randn_1: "f64[3, 5]" = smith.ops.aten.randn.default([3, 5], dtype = smith.float64, device = device(type='cpu'), pin_memory = False)
        randn_2: "f64[4, 5]" = smith.ops.aten.randn.default([4, 5], dtype = smith.float64, device = device(type='cpu'), pin_memory = False)

        cat: "f64[9, 5]" = smith.ops.aten.cat.default([randn, randn_1, randn_2]);  randn = randn_1 = randn_2 = None
        zeros: "i64[1]" = smith.ops.aten.zeros.default([1], dtype = smith.int64, device = device(type='cpu'), pin_memory = False)
        _tensor_constant0: "i64[3]" = self._tensor_constant0
        lift_fresh_copy: "i64[3]" = smith.ops.aten.lift_fresh_copy.default(_tensor_constant0);  _tensor_constant0 = None
        cumsum: "i64[3]" = smith.ops.aten.cumsum.default(lift_fresh_copy, 0);  lift_fresh_copy = None
        cat_1: "i64[4]" = smith.ops.aten.cat.default([zeros, cumsum]);  zeros = cumsum = None
        zeros_1: "f32[2, 0]" = smith.ops.aten.zeros.default([2, 0], device = device(type='cpu'), pin_memory = False)
        zeros_2: "f32[4, 0]" = smith.ops.aten.zeros.default([4, 0], device = device(type='cpu'), pin_memory = False)

        cat_2: "f64[9, s55 + 5]" = smith.ops.aten.cat.default([cat, arg3_1], 1);  cat = arg3_1 = None

        sin: "f64[9, s55 + 5]" = smith.ops.aten.sin.default(cat_2)
        mul: "f64[9, s55 + 5]" = smith.ops.aten.mul.Tensor(sin, 3);  sin = None

        sym_size_int: "Sym(s55 + 5)" = smith.ops.aten.sym_size.int(cat_2, 1);  cat_2 = None
        sym_stride_int: "Sym(s55 + 5)" = smith.ops.aten.sym_stride.int(mul, 0)
        return (
            mul,  # SubclassGetAttrAOTOutput(base=PlainAOTOutput(idx=0), attr='_values')
...
class TestNestedTensor(smith._dynamo.test_case.TestCase, NestedTensorTestCase):
    ...
    def _validate_compile(self, fn, arg_fn):
        def _gen_grad_outputs(out_val):
            if isinstance(out_val, (list, tuple)):
                return tuple(smith.ones_like(c) for c in out_val)
            else:
                return (smith.ones_like(out_val),)

        with self.branch_nested_state():
            from smith.nested._internal.nested_tensor import _tensor_symint_registry

            # Validate that compilation does not modify eager state
            registry_before = list(_tensor_symint_registry.items())
            count_before = smith.nested._internal.nested_tensor._tensor_id_counter
            ...
            compiled = smith._dynamo.optimize(
                nopython=True,
                backend="aot_eager",
            ...
            count_after = smith.nested._internal.nested_tensor._tensor_id_counter
            ...
            if len(g_args) > 0:
                ...
                compile_grads = smith.autograd.grad(
    ...
    def test_in_graph_is_nested_call(self):
        ...
        compiled_f = smith.compile(f, backend=cnt, fullgraph=True)
...
""",  # noqa: B950
    ...
    def test_in_graph_construction_from_input(self):
        ...
        def fn(values, offsets):
            return smith.nested.nested_tensor_from_jagged(values * 2, offsets) * 2

        values = smith.randn(10, 5, requires_grad=True)
        offsets = smith.tensor([0, 2, 6, 10], dtype=smith.int64)
        self._validate_compile(fn, arg_fn=lambda: (values, offsets))

        # Do not specialize on the offsets
        with unittest.mock.patch("smith._dynamo.config.error_on_recompile", True):
            different_offsets = smith.tensor([0, 1, 5, 10], dtype=smith.int64)
    ...
    def test_in_graph_construction_from_input_2(self):
        ...
        def fn(values, offsets1, offsets2):
            nt1 = smith.nested.nested_tensor_from_jagged(values * 2, offsets1)
            nt2 = smith.nested.nested_tensor_from_jagged(values * 3, offsets2)
            return nt2, nt1

        values = smith.randn(10, 5, requires_grad=True)
        offsets = smith.tensor([0, 2, 6, 10], dtype=smith.int64)
        offsets2 = smith.tensor([0, 1, 4, 10], dtype=smith.int64)
        ...
        def fn(values, offsets, offsets2):
            nt1 = smith.nested.nested_tensor_from_jagged(values * 2, offsets)
            nt2 = smith.nested.nested_tensor_from_jagged(values * 3, offsets2)
            return nt1 * nt2
    ...
    def test_in_graph_construction_from_input_4(self):
        ...
        def fn(nt, other_values):
            nt2 = smith.nested.nested_tensor_from_jagged(other_values, nt.offsets())
            return nt + nt2

        values = smith.randn(9, 5, requires_grad=True)
        other_values = smith.randn(9, 5, requires_grad=True)
        offsets = smith.tensor([0, 2, 6, 9], dtype=smith.int64)
        ...
        with unittest.mock.patch("smith._dynamo.config.error_on_recompile", True):
            ...
            def arg_fn(
                ...
                nt = smith.nested.nested_tensor_from_jagged(values, different_offsets)
                return nt, other_values
    ...
    def test_in_graph_construction_from_input_6(self):
        ...
        def fn(values, offsets, max_seqlen):
            t = smith.nested.nested_tensor_from_jagged(
        ...
        opt_fn = smith.compile(fn, fullgraph=True, dynamic=True)
        values = smith.randn(10, 5)
        offsets = smith.tensor([0, 2, 4, 7, 10])
        max_seqlen = 5
    ...
    def test_in_graph_construction_from_intermediate(self):
        ...
        def fn(values, lengths):
            offsets = smith.cat([lengths.new_zeros(1), lengths.cumsum(0)])
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
            nt2 = smith.nested.nested_tensor_from_jagged(values, offsets)
            return (nt * nt2).sin()
    ...
    def test_in_graph_construction_from_intermediate_2(self):
        def fn(values, offsets):
            return smith.nested.nested_tensor_from_jagged(values * 2, offsets.clone())
    ...
    def test_in_graph_construction_from_intermediate_3(self):
        ...
        def fn(values, offsets):
            nt1 = smith.nested.nested_tensor_from_jagged(values * 2, offsets.clone())
            nt2 = smith.nested.nested_tensor_from_jagged(values * 3, offsets.clone())
            return nt2, nt1
    ...
    def test_in_graph_construction_from_intermediate_4(self):
        ...
        def fn(values):
            offsets = smith.tensor([0, 2, 6, 10], dtype=smith.int64)
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
            values2 = smith.ones_like(values)
            nt2 = smith.nested.nested_tensor_from_jagged(values2, offsets)
            return nt * nt2

        values = smith.randn(10, 5).requires_grad_(True)
    ...
    def test_in_graph_construction_from_intermediate_5(self):
        ...
        def fn(values):
            ...
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
            values2 = smith.ones_like(values)
            nt2 = smith.nested.nested_tensor_from_jagged(values2, offsets.clone())
    ...
    def test_in_graph_construction_mixed_2(self):
        def fn(nt, values, offsets, nt2):
            ...
            intermediate_nt = smith.nested.nested_tensor_from_jagged(
        ...
        def arg_fn(values=values, offsets=offsets, offsets2=offsets2):
            ...
            nt = smith.nested.nested_tensor_from_jagged(values, offsets)
            nt2 = smith.nested.nested_tensor_from_jagged(values, offsets2)
    ...
    def test_in_graph_construction_mixed_3(self):
        ...
        def fn(nt, values, offsets):
            nt1 = smith.nested.nested_tensor_from_jagged(values * 2, offsets)
            nt2 = smith.nested.nested_tensor_from_jagged(values * 3, offsets)
            return nt1 + nt2 + nt
    ...
    def test_return_shape(self):
        ...
        compiled = smith.compile(fn, fullgraph=True, backend="aot_eager")
        compiled(nt)
    ...
    def test_inference_tensor(self):
        ...
        smith.compile(fn, backend="eager")(nt)
    ...
    def _test_autograd(self, backend):
        ...
        compiled_f = smith.compile(fn1, fullgraph=True, backend=backend, dynamic=True)
        ...
        ga_ref, gb_ref, gc_ref = smith.autograd.grad(out_buffer_ref.sum(), (a, b, c))

        self.assertTrue(smith.allclose(ga, ga_ref))
        self.assertTrue(smith.allclose(gb, gb_ref))
        self.assertTrue(smith.allclose(gc, gc_ref))
    ...
    def test_subclass_with_mutation_in_graph(self):
        ...
        fn_c = smith.compile(fn, backend="inductor")

        values = [smith.rand((i, 8), requires_grad=True) for i in range(1, 6)]
    ...
    def test_unbind(self):
        ...
        compiled_f = smith.compile(fn, fullgraph=True, backend="eager", dynamic=True)
        out = compiled_f(nt)
        ...
        for x, x_ref in zip(out, out_ref):
            self.assertTrue(smith.allclose(x, x_ref))
    ...
    def test_inline_nested_tensor_from_jagged(self):
        ...
        def fn(x):
            return smith.nested.nested_tensor_from_jagged(x.values() * 2, x.offsets())

        smith.compile(fn, fullgraph=True, backend="aot_eager")(nt)
    ...
    def test_param_subclass_isinstance_input(self):
        x_inner = smith.randn(16, 16, requires_grad=True)
        x = smith.nn.Parameter(TwoTensor(x_inner, x_inner))
        m = smith.nn.Linear(16, 16)
        m.weight = x

        def fn():
            if isinstance(m.weight, smith.nn.Parameter):
                return m.weight + 1
            else:
                return m.weight + 2

        out_ref = fn()
        out_test = smith.compile(fn, backend="aot_eager")()
        self.assertEqual(out_ref, out_test)

    def test_buffer_subclass_isinstance_input(self):
        from smith.nn.parameter import Buffer

        buf = Buffer(smith.ones(5))

        def fn(b):
            if isinstance(b, smith.nn.Buffer):
                return b + 1
            else:
                return b + 2

        out_ref = fn(buf)
        out_test = smith.compile(fn, backend="aot_eager", fullgraph=True)(buf)
        self.assertEqual(out_ref, out_test)
        ...
        out_test_tensor = smith.compile(fn, backend="aot_eager", fullgraph=True)(tensor)
    ...
    def test_buffer_subclass_check(self):
        ...
        def check_buffer(x):
            return isinstance(x, smith.nn.Buffer)

        buf = Buffer(smith.ones(5))
        compiled_fn = smith.compile(check_buffer, fullgraph=True, backend="inductor")
    ...
    def _input_view_test(self, nt_view_name):
        ...
        if out.is_nested:
            self.assertTrue(smith.allclose(out.values(), out_ref.values()))
        else:
...
Eq(s20, s77)""",
        ...
        smith._dynamo.reset()
        compile_fn = smith.compile(fn, fullgraph=True, backend=backend, dynamic=True)
        out = compile_fn(nt_view)
    ...
    def test_subclass_gives_static_shapes_when_dynamic_false(self):
        ...
        @smith.compile(backend=check_graph, dynamic=False)
        def f(x):
            return x + 1
