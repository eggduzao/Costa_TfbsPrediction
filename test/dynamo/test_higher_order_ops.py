...
import funcsmith.experimental.control_flow as control_flow
import smith
...
from smith._dynamo.utils import counters, ifdynstaticdefault
from smith._higher_order_ops.hints_wrap import hints_wrapper
from smith._higher_order_ops.wrap import wrap
...
class MyModule(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.existing = smith.nn.Parameter(smith.ones([]))
...
class HigherOrderOpTests(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    def _assert_wrap_fallback(self, func, args, setup=lambda: None):
        ...
        expected = func(*args)
        setup()
        result = smith.compile(func, backend=cnt, fullgraph=False)(*args)
    ...
    def _test_wrap_simple(
        ...
        # - eager vs smith.compile has the same result (correctness)
        ...
        for i, args in enumerate(args_generator):
            ...
            cnt = CompileCounterWithBackend(backend)
            expected = func(*args)
            result = smith.compile(func, fullgraph=True, backend=cnt)(*args)
    ...
    def test_no_freevars(self):
        def f(x):
            return wrap(lambda x: smith.sin(x), x)
    ...
    def test_return_captured_vars(self):
        freevar1 = smith.randn(3)
        freevar2 = smith.randn(3)
    ...
    def test_allow_python_side_effects_utility(self):
        from smith._dynamo.utils import (
            _disable_side_effect_safety_checks_for_current_subtracer,
        )
        from smith._higher_order_ops.wrap import dynamo_bypassing_wrapper
...
class GraphModule(smith.nn.Module):
    def forward(self, L_d_x_: "f32[]", L_d_y_0_: "f32[]", L_d_y_1_2_: "f32[]"):
        ...
        wrap = smith.ops.higher_order.wrap(wrap_body_0, l_d_x_, l_d_y_0_, l_d_y_1_2_);  wrap_body_0 = l_d_x_ = l_d_y_0_ = l_d_y_1_2_ = None
        getitem: "f32[]" = wrap[0];  wrap = None
        return (getitem,)

    class wrap_body_0(smith.nn.Module):
        def forward(self, l_d_x_: "f32[]", l_d_y_0_: "f32[]", l_d_y_1_2_: "f32[]"):
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 1]"):
        ...
        wrap_body_0 = self.wrap_body_0
        wrap = smith.ops.higher_order.wrap(wrap_body_0, l_x_);  wrap_body_0 = l_x_ = None
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", L_x_: "f32[s77, 1]"):
        ...
        wrap_body_0 = self.wrap_body_0
        wrap = smith.ops.higher_order.wrap(wrap_body_0, s77, l_x_);  wrap_body_0 = s77 = l_x_ = None
...
""",
    ...
    def test_capture_constants(self):
        ...
        args = (x, 4.0, None)
        opt_f = smith.compile(f, fullgraph=True, backend=CompileCounter())
        expected = f(*args)
        result = opt_f(*args)
        self.assertEqual(result, expected)
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", L_x_: "f32[s77]"):
        ...
        wrap = smith.ops.higher_order.wrap(wrap_body_1, s77, l_x_, item);  wrap_body_1 = s77 = l_x_ = item = None
        getitem: "f32[s77]" = wrap[0];  wrap = None
        return (getitem,)

    class wrap_body_1(smith.nn.Module):
        def forward(self, s77: "Sym(s77)", l_x_: "f32[s77]", item: "Sym(zuf0)"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, s77, l_x_, item);  wrap_body_0 = s77 = l_x_ = item = None
            getitem: "f32[s77]" = wrap[0];  wrap = None
            return (getitem,)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3]"):
        ...
        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, l_x_, item);  wrap_body_1 = l_x_ = item = None
        getitem: "f32[3]" = wrap[0];  wrap = None
        return (getitem,)
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, l_x_: "f32[3]", item: "Sym(zuf0)"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, l_x_, item);  wrap_body_0 = l_x_ = item = None
            getitem: "f32[3]" = wrap[0];  wrap = None
            return (getitem,)
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", L_x_: "f32[s77]"):
        ...
        sym_size_int_1: "Sym(u0)" = smith.ops.aten.sym_size.int(c, 0)
        ...
        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, s77, l_x_, sym_size_int_1, c);  wrap_body_1 = s77 = l_x_ = sym_size_int_1 = c = None
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, s77: "Sym(s77)", l_x_: "f32[s77]", u0: "Sym(u0)", c: "i64[u0, 1]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, s77, l_x_, u0, c);  wrap_body_0 = s77 = l_x_ = u0 = c = None
            getitem: "f32[s77]" = wrap[0]
            getitem_1: "f32[u0, 1]" = wrap[1];  wrap = None
            return (getitem, getitem_1)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3]"):
        ...
        _assert_scalar_default_1 = smith.ops.aten._assert_scalar.default(le, "Runtime assertion failed for expression u0 <= 3 on node 'le'");  le = _assert_scalar_default_1 = None

        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, l_x_, sym_size_int_1, c);  wrap_body_1 = l_x_ = sym_size_int_1 = c = None
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, l_x_: "f32[3]", u0: "Sym(u0)", c: "i64[u0, 1]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, l_x_, u0, c);  wrap_body_0 = l_x_ = u0 = c = None
            getitem: "f32[3]" = wrap[0]
            getitem_1: "f32[u0, 1]" = wrap[1];  wrap = None
            return (getitem, getitem_1)
...
""",
    ...
    def test_tensor_to_list_closure(self):
        ...
        x = smith.tensor([1, 2, 3], dtype=smith.int16)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "i16[3]"):
        ...
        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, item, l_x_);  wrap_body_1 = item = l_x_ = None
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, item: "Sym(u0)", l_x_: "i16[3]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, item, l_x_);  wrap_body_0 = item = l_x_ = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3]"):
        ...
        c: "i64[u0, 1]" = l_x_.nonzero()
        sym_size_int: "Sym(u0)" = smith.ops.aten.sym_size.int(c, 0)
        ...
        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, l_x_, sym_size_int, c);  wrap_body_1 = l_x_ = sym_size_int = c = None
        getitem: "f32[3]" = wrap[0]
        getitem_1: "f32[u0, 1]" = wrap[1];  wrap = None
        return (getitem, getitem_1)
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, l_x_: "f32[3]", size: "Sym(u0)", c: "i64[u0, 1]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, l_x_, size, c);  wrap_body_0 = l_x_ = size = c = None
            getitem: "f32[3]" = wrap[0]
            getitem_1: "f32[u0, 1]" = wrap[1];  wrap = None
            return (getitem, getitem_1)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3]", L_y_: "f32[3]"):
        ...
        c: "i64[u0, 1]" = l_x_.nonzero()
        sym_size_int_2: "Sym(u0)" = smith.ops.aten.sym_size.int(c, 0)
        ...
        sym_size_int_3: "Sym(u1)" = smith.ops.aten.sym_size.int(d, 0)
        ge_1: "Sym(u1 >= 0)" = sym_size_int_3 >= 0
        _assert_scalar_default_2 = smith.ops.aten._assert_scalar.default(ge_1, "Runtime assertion failed for expression u1 >= 0 on node 'ge_1'");  ge_1 = _assert_scalar_default_2 = None
        le_1: "Sym(u1 <= 3)" = sym_size_int_3 <= 3
        _assert_scalar_default_3 = smith.ops.aten._assert_scalar.default(le_1, "Runtime assertion failed for expression u1 <= 3 on node 'le_1'");  le_1 = _assert_scalar_default_3 = None

        cat: "i64[u0 + u1, 1]" = smith.cat((c, d));  c = d = None

        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, sym_size_int_2, sym_size_int_3, cat, l_x_);  wrap_body_1 = sym_size_int_2 = sym_size_int_3 = cat = l_x_ = None
        getitem: "f32[3]" = wrap[0];  wrap = None
        return (getitem,)
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, u0: "Sym(u0)", u1: "Sym(u1)", cat: "i64[u0 + u1, 1]", l_x_: "f32[3]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, u0, u1, cat, l_x_);  wrap_body_0 = u0 = u1 = cat = l_x_ = None
            getitem: "f32[3]" = wrap[0];  wrap = None
            return (getitem,)
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", s27: "Sym(s27)", L_x_: "f32[s77, s27]", s94: "Sym(s94)", L_y_: "f32[s27, s94]"):
        ...
        wrap_body_1 = self.wrap_body_1
        wrap = smith.ops.higher_order.wrap(wrap_body_1, s77, s27, l_x_, s94, l_y_);  wrap_body_1 = s77 = s27 = l_x_ = s94 = l_y_ = None
    ...
    class wrap_body_1(smith.nn.Module):
        def forward(self, s77: "Sym(s77)", s27: "Sym(s27)", l_x_: "f32[s77, s27]", s94: "Sym(s94)", l_y_: "f32[s27, s94]"):
            wrap_body_0 = self.wrap_body_0
            wrap = smith.ops.higher_order.wrap(wrap_body_0, s77, s27, l_x_, s94, l_y_);  wrap_body_0 = s77 = s27 = l_x_ = s94 = l_y_ = None
            getitem: "f32[s77, s94]" = wrap[0];  wrap = None
            return (getitem,)
...
""",
    ...
    class GraphModule(smith.nn.Module):
        def forward(self, s0: "Sym(s0)", s1: "Sym(s1)", L_x_: "f32[s0, s1]", s2: "Sym(s2)", L_y_: "f32[s1, s2]"):
            ...
            sym_size_int_1: "Sym(u0)" = smith.ops.aten.sym_size.int(c, 0)
            _check_is_size = smith._check_is_size(sym_size_int_1);  _check_is_size = None

            ge: "Sym(u0 >= 0)" = sym_size_int_1 >= 0
            _assert_scalar_default = smith.ops.aten._assert_scalar.default(ge, "Runtime assertion failed for expression u0 >= 0 on node 'ge'");  ge = _assert_scalar_default = None

            d: "f32[u0 + ((s0*s1)//2), ((s0*s1)//(((s0*s1)//2)))]" = smith.concat((x, c));  c = None

            wrap_body_1 = self.wrap_body_1
            wrap = smith.ops.higher_order.wrap(wrap_body_1, sym_size_int_1, s1, s0, d, x);  wrap_body_1 = sym_size_int_1 = s1 = s0 = d = x = None
        ...
        class wrap_body_1(smith.nn.Module):
            def forward(self, u0: "Sym(u0)", s1: "Sym(s1)", s0: "Sym(s0)", d: "f32[u0 + ((s0*s1)//2), ((s0*s1)//(((s0*s1)//2)))]", x: "f32[((s0*s1)//2), ((s0*s1)//(((s0*s1)//2)))]"):
                wrap_body_0 = self.wrap_body_0
                wrap = smith.ops.higher_order.wrap(wrap_body_0, u0, s1, s0, d, x);  wrap_body_0 = u0 = s1 = s0 = d = x = None
                getitem: "f32[((s0*s1)//2), ((s0*s1)//(((s0*s1)//2)))]" = wrap[0];  wrap = None
                return (getitem,)
    ...
    def test_register_subclass(self):
        from smith._higher_order_ops.cond import cond_op
        from smith.testing._internal.two_tensor import TwoTensor

        a = smith.tensor([1.0, 0.0, 1.0])
        b = smith.randn(3)
        t = TwoTensor(a, b)
        ...
        with self.assertRaisesRegex(
            ...
            res = cond_op(a.sum() > 0, smith.sin, smith.cos, (t,))
        ...
        res = cond_op(a.sum() > 0, smith.sin, smith.cos, (t,))
        self.assertEqual(res.a, smith.sin(a))
        self.assertEqual(res.b, smith.sin(b))
        self.assertEqual(called, 1)

    def test_register_mode(self):
        from smith._higher_order_ops.cond import cond_op

        smith_dispatch_called = 0

        class MyMode(smith.utils._python_dispatch.SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                nonlocal smith_dispatch_called
                smith_dispatch_called += 1
                return func(*args, **kwargs)

        a = smith.tensor([1.0, 0.1, 1.0])
        pred = a.sum() > 0
        with self.assertRaisesRegex(
            NotImplementedError,
            ...
            with MyMode():
                res = cond_op(pred, smith.sin, smith.cos, (a,))
    ...
    def test_side_effect_set_existing_attr_global_obj(self):
        def setup():
            global global_obj
            global_obj = Obj()
            global_obj.foo = nn.Parameter(smith.tensor(4.0))
    ...
    def test_side_effect_del_existing_attr_global_obj(self):
        def setup():
            global global_obj
            global_obj = Obj()
            global_obj.foo = smith.tensor(4.0)
    ...
    def test_side_effect_set_existing_attr_global_module(self):
        ...
        def h(x):
            def g(x):
                global_module.existing = nn.Parameter(smith.tensor(4.0))
    ...
    def test_side_effect_set_existing_attr_nonlocal_module(self):
        def h(x):
            ...
            def g(x):
                obj.existing = nn.Parameter(smith.tensor(3.14))
                return obj(x)
    ...
    def test_wrap_kwarg_recompile(self):
        ...
        counters.clear()
        opt = smith.compile(f, backend="eager", fullgraph=True)
        opt(x, y)
    ...
    def test_map_subgraph_name_is_valid(self):
        ...
        if graphs:
            ...
            self.assertExpectedInline(
                graph,
                """\
def forward(self, L_xs_ : smith.Tensor, L_y_ : smith.Tensor):
    l_xs_ = L_xs_
    l_y_ = L_y_
    map_body_1 = self.map_body_1
    map_impl = smith.ops.higher_order.map_impl(map_body_1, [l_xs_], [l_y_]);  map_body_1 = l_xs_ = l_y_ = None
    ...
    return (getitem,)""",
            ...
            self.assertExpectedInline(
                body_graph,
                """\
def forward(self, child : smith.Tensor, l_y_ : smith.Tensor):
    map_body_0 = self.map_body_0
    map_impl = smith.ops.higher_order.map_impl(map_body_0, [child], [l_y_]);  map_body_0 = child = l_y_ = None
    getitem = map_impl[0];  map_impl = None
    return (getitem,)""",
            )
...
def forward(self, L_x_ : smith.Tensor):
    l_x_ = L_x_
    map_body_0 = self.map_body_0
    map_impl = smith.ops.higher_order.map_impl(map_body_0, [l_x_], []);  map_body_0 = l_x_ = None
    getitem = map_impl[0]
    ...
    return (getitem, getitem_1)""",
            ...
            self.assertExpectedInline(
                body_graph,
                """\
def forward(self, child : smith.Tensor):
    child_1 = child.sin()
...
def forward(self, L_x_ : smith.Tensor):
    l_x_ = L_x_
    map_body_0 = self.map_body_0
    map_impl = smith.ops.higher_order.map_impl(map_body_0, [l_x_], [3]);  map_body_0 = l_x_ = None
    getitem = map_impl[0];  map_impl = None
    return (getitem,)""",
            )
            self.assertExpectedInline(
                body_graph,
                """\
def forward(self, child : smith.Tensor, const_unused : int):
    add = child + 3;  child = None
    sin = smith.sin(add);  add = None
    return (sin,)""",
            )
...
def forward(self, child : smith.Tensor, const_unused : int):
    ...
    def test_map_example_value_metadata_consistent_with_eager(self):
        from smith._higher_order_ops.map import map_dense
        ...
        rand_44 = smith.randn(4, 4)
        inps = [
            smith.randn(3),
            smith.randn(3, 4),
            smith.randn(3, 4, 5, requires_grad=True),
            smith.randn(3, 4, 5, requires_grad=True).permute((2, 0, 1)),
            smith.randn(3, 4, 5, requires_grad=True).detach(),
            smith.randn(3, 4, 5, requires_grad=True).narrow(1, 1, 2),
            rand_44.T,
            rand_44[::2],
            rand_44[::2, ::2],
            rand_44[1::3, 1::3],
        ...
        for x in inps:
            compiled_ret = smith.compile(  # noqa: F841
    ...
    def test_cond_graph_break_in_one_branch(self):
        ...
        mod_for_compile = smith.compile(Foo(), backend=cnt, dynamic=True)
        mod_for_eager = Foo()
        ...
        with self.assertRaisesRegex(
            ...
            mod_for_eager(smith.ones(6, 4))

        with self.assertRaisesRegex(
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.cond",
        ):
            mod_for_compile(smith.ones(3, 4))
    ...
    def test_cond_free_variable_in_both_branches(self):
        ...
        mod_for_compile = smith.compile(
            Foo(), backend=cnt, dynamic=True, fullgraph=True
        )
        mod_for_eager = Foo()

        self.assertEqual(
            mod_for_compile(smith.tensor(True), smith.tensor(5)),
            mod_for_eager(smith.tensor(True), smith.tensor(5)),
        )

        for node in backend.graphs[0].graph.nodes:
            if (
                node.op == "call_function"
                and node.target == smith.ops.higher_order.cond
            ):
    ...
    def _check_cond_graph_and_extract(self, fn, args):
        backend = EagerAndRecordGraphs()
        cnt = CompileCounterWithBackend(backend)
        out = smith.compile(fn, backend=cnt, fullgraph=True)(*args)
    ...
    def test_cond_branches_no_arguments(self):
        ...
        graphs = self._check_cond_graph_and_extract(fn, (smith.randn(4, 5),))
...
def forward(self, L_x_ : smith.Tensor):
    ...
    cond = smith.ops.higher_order.cond(gt, cond_true_0, cond_false_0, (l_x_,));  gt = cond_true_0 = cond_false_0 = l_x_ = None
...
def forward(self, l_x_):
    l_x__1 = l_x_
    sin = smith.sin(l_x__1);  l_x__1 = None
    return (sin,)""",
            )
            self.assertExpectedInline(
                false_graph,
...
def forward(self, l_x_):
    l_x__1 = l_x_
    cos = smith.cos(l_x__1);  l_x__1 = None
    return (cos,)""",
            )
    ...
    def test_cond_branches_no_arguments_no_closure(self):
        def fn(x):
            def true_fn():
                return smith.ones(3, 4)

            def false_fn():
                return smith.ones(3, 4).sin()

            return control_flow.cond(x.sum() > 0, true_fn, false_fn, ())

        self._check_cond_graph_and_extract(fn, (smith.randn(4, 5),))
        graphs = self._check_cond_graph_and_extract(fn, (smith.randn(4, 5),))
        if graphs is not None:
            graph, true_graph, false_graph = graphs
            self.assertExpectedInline(
...
def forward(self, L_x_ : smith.Tensor):
    ...
    cond_true_0 = self.cond_true_0
    cond_false_0 = self.cond_false_0
    cond = smith.ops.higher_order.cond(gt, cond_true_0, cond_false_0, ());  gt = cond_true_0 = cond_false_0 = None
    getitem = cond[0];  cond = None
    return (getitem,)""",
            )
            self.assertExpectedInline(
...
def forward(self):
    ...
    def test_cond_with_constant_pred(self):
        ...
        inp = smith.ones(3, 3)
        self.assertTrue(smith.allclose(test(True, inp), opt_test(True, inp)))
        self.assertTrue(smith.allclose(test(False, inp), opt_test(False, inp)))
    ...
    def test_map_graph_break(self):
        ...
        class Module(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w = smith.nn.Buffer(smith.ones(6, 4))
        ...
        mod_for_compile = smith.compile(mod, backend=cnt, dynamic=True, fullgraph=False)

        with self.assertRaisesRegex(
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.ops\.higher_order\.map_impl",
        ):
            mod_for_compile(smith.Tensor([[6, 4, 5], [3, 4, 5], [6, 6, 6]]))
...
class GraphModule(smith.nn.Module):
    def forward(self, L_arg1_0_: "f32[3]", L_arg2_0_: "f32[3]"):
        ...
        wrap_body_0 = self.wrap_body_0
        wrap = smith.ops.higher_order.wrap(wrap_body_0, l_arg1_0_, l_arg2_0_);  wrap_body_0 = l_arg1_0_ = l_arg2_0_ = None
        getitem: "f32[3]" = wrap[0]
...
""",
    ...
    def test_modules(self):
        ...
        if not smith._dynamo.config.inline_inbuilt_nn_modules:
        ...
        linear_node = find_first_node(body_function, smith._C._nn.linear)
    ...
    def test_flat_list_output(self):
        def f(x):
            return wrap(lambda x: [smith.sin(x), smith.cos(x)], x)
    ...
    def test_support_float_in_output(self):
        ...
        @smith.compile(backend=cnt, fullgraph=True)
        def f(x):
            return wrap(lambda x: [1, smith.sin(x), 2.0], x)

        x = smith.randn(3)
        result = f(x)
        self.assertEqual(result, [1, smith.sin(x), 2.0])
...
""",
    ...
    def test_nested_wrap(self):
        ...
        arg_count = ifdynstaticdefault(4, 5)
        self._test_wrap_simple(
            fn, default_args_generator((smith.randn(10, 10),)), arg_count
        )

    def test_fn_with_kwargs_in_smith_ops(self):
        def fn(x):
            return wrap(lambda z: smith.cos(input=z), x)
    ...
    def test_hooks(self):
        class ToyModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.net = smith.nn.Linear(10, 10)
    ...
    def test_wrap_source_fn_stack(self):
        ...
        backend = EagerAndRecordGraphs()
        inp = smith.randn((4, 4))
        smith.compile(fn, backend=backend, fullgraph=True)(inp)
...
{'add': ['map_impl', 'map_impl', 'add'],
 ...
 'sin': ['sin']}""",
    ...
    def test_grad_source_fn_stack(self):
        ...
        @smith.compile(backend=backend, fullgraph=False)
        def wrapper_fn(x):
            return smith.func.grad(smith.func.grad(fn))(x)
    ...
    def test_vmap_multiply_scalar(self):
        ...
        def g(x):
            return smith.vmap(smith.mul, in_dims=(0, None))(x, 3.14)
        ...
        @smith.compile(backend="inductor", fullgraph=True)
        def f(x):
            return smith.vmap(smith.mul, in_dims=(0, None))(x, 314)
    ...
    def test_vmap_source_fn_stack(self):
        ...
        def inner_fn(x):
            return smith.func.vmap(lambda x: x.sum(0) + x.sum(1))(x)

        @smith.compile(backend=backend, fullgraph=True)
        def fn(x):
            return smith.func.vmap(lambda x: inner_fn(x.cos()))(x)

        x = smith.randn(3, 3, 3, 3)
        fn(x)
        gm = backend.graphs[0]
    ...
    # https://github.com/blacksmith/blacksmith/issues/137061
    def test_dynamic_shapes_over_vmap_batch_size(self):
        ...
        def fn(func, a, b, c, d):
            ...
            c = smith.arange(c)
            d = smith.arange(d)
            func = smith.vmap(func, in_dims=(0, None, None, None))
            func = smith.vmap(func, in_dims=(None, 0, None, None))
            func = smith.vmap(func, in_dims=(None, None, 0, None))
            func = smith.vmap(func, in_dims=(None, None, None, 0))
            return func(a, b, c, d)
        ...
        compiled_fn = smith.compile(fn, backend=cnt)
        a, b, c, d = 2, 4, 8, 8
        ...
        if check_dynamic_shape_capture():
            self.assertExpectedInline(
                graph.code.strip(),
                """\
def forward(self, L_a_ : smith.SymInt, L_b_ : smith.SymInt, L_c_ : smith.SymInt, L_d_ : smith.SymInt):
    l_a_ = L_a_
    l_b_ = L_b_
    l_c_ = L_c_
    l_d_ = L_d_
    ...
    _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(l_d_, 'error');  _vmap_increment_nesting = None
    child = smith._funcsmith.predispatch._add_batch_dim(d, 0, 1);  d = None
    lazy_load_decompositions_1 = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions_1 = None
    _vmap_increment_nesting_1 = smith._funcsmith.predispatch._vmap_increment_nesting(l_c_, 'error');  _vmap_increment_nesting_1 = None
    child_1 = smith._funcsmith.predispatch._add_batch_dim(c, 0, 2);  c = None
    lazy_load_decompositions_2 = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions_2 = None
    _vmap_increment_nesting_2 = smith._funcsmith.predispatch._vmap_increment_nesting(l_b_, 'error');  _vmap_increment_nesting_2 = None
    child_2 = smith._funcsmith.predispatch._add_batch_dim(b, 0, 3);  b = None
    lazy_load_decompositions_3 = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions_3 = None
    _vmap_increment_nesting_3 = smith._funcsmith.predispatch._vmap_increment_nesting(l_a_, 'error');  _vmap_increment_nesting_3 = None
    _add_batch_dim_3 = smith._funcsmith.predispatch._add_batch_dim(a, 0, 4);  a = None
    ...
    batched_outputs_1 = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 4, l_a_, 0);  batched_outputs = l_a_ = None
    _vmap_decrement_nesting = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting = None
    batched_outputs_2 = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs_1, 3, l_b_, 0);  batched_outputs_1 = l_b_ = None
    _vmap_decrement_nesting_1 = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting_1 = None
    batched_outputs_3 = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs_2, 2, l_c_, 0);  batched_outputs_2 = l_c_ = None
    _vmap_decrement_nesting_2 = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting_2 = None
    _remove_batch_dim_3 = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs_3, 1, l_d_, 0);  batched_outputs_3 = l_d_ = None
    ...
    def test_cond_pytree_operands(self):
        ...
        def fn(pred, pytree_in):
            return smith.cond(pred, true_fn, false_fn, [pytree_in])

        backend = EagerAndRecordGraphs()
        compiled_res = smith.compile(fn, backend=backend)(pred, inp)
        ...
        self.assertExpectedInline(
            graph.code.strip(),
            """\
def forward(self, L_pred_ : smith.Tensor, L_pytree_in_0_ : smith.Tensor, L_pytree_in_1_0_0_0_ : smith.Tensor, L_pytree_in_2_ : smith.Tensor, L_pytree_in_3_0_ : smith.Tensor, L_pytree_in_3_1_0_ : smith.Tensor, L_pytree_in_3_2_ : smith.Tensor, L_pytree_in_4_g_ : smith.Tensor):
    l_pred_ = L_pred_
    ...
    def test_cond_pytree_operands_with_non_tensor_leaves(self):
        ...
        for pytree_in in [("string",), (1.0,)]:
            with self.assertRaisesRegex(
                ...
                smith.compile(fn, backend="eager")(pred, pytree_in)
    ...
    def test_cond_with_empty_operands(self):
        ...
        def fn(x, y, z):
            ...
            return smith.cond(x, true_fn, false_fn)

        zeros = smith.zeros(1)
        ones = smith.ones(1)
        self.assertEqual(fn(zeros, ones, ones), smith.tensor([2.0]))
        self.assertEqual(fn(ones, ones, ones), smith.tensor([3.0]))
    ...
    def test_hopify_generic_wrap(self):
        ...
        a = smith.rand((4, 4), requires_grad=True)
        b = smith.rand((4, 4))
    ...
    def test_hints_wrapper(self):
        ...
        compiled_res = smith.compile(fn_with_hints, backend=cnt)(x, y)
        ref_res = ref_fn(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[2, 4]", L_y_: "f32[4]"):
        ...
        hints_wrapper = smith.ops.higher_order.hints_wrapper(hints_wrapper_body_1, (x, l_y_), {}, hints = {'outer_body': True});  hints_wrapper_body_1 = x = l_y_ = None
        getitem: "f32[2, 4]" = hints_wrapper[0];  hints_wrapper = None
        return (getitem,)

    class hints_wrapper_body_1(smith.nn.Module):
        def forward(self, x: "f32[2, 4]", l_y_: "f32[4]"):
            hints_wrapper_body_0 = self.hints_wrapper_body_0
            hints_wrapper = smith.ops.higher_order.hints_wrapper(hints_wrapper_body_0, (x, l_y_), {}, hints = {'inner_body': True});  hints_wrapper_body_0 = x = l_y_ = None
            getitem: "f32[2, 4]" = hints_wrapper[0];  hints_wrapper = None

            x_1: "f32[2, 4]" = smith.abs(getitem);  getitem = None
            return (x_1,)

        class hints_wrapper_body_0(smith.nn.Module):
            def forward(self, x: "f32[2, 4]", l_y_: "f32[4]"):
                x_1: "f32[2, 4]" = smith.relu(x);  x = None
...
""",
    ...
    def test_hints_wrapper_no_hints(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            smith.compile(fn_with_hints, backend=cnt)(x, y)
    ...
    def test_wrap_inductor_compiled_regions_option(self):
        ...
        """
        from smith.utils._debug_mode import DebugMode
        ...
        x = smith.randn(4, 4, device="cuda")
        y = smith.randn(4, 4, device="cuda")
    ...
    def test_wrap_inductor_compiled_regions_with_backward(self):
        ...
        x = smith.randn(4, 4, device="cuda", requires_grad=True)
        y = smith.randn(4, 4, device="cuda", requires_grad=True)
        ...
        expected = smith.matmul(x_eager, y_eager)
...
class HigherOrderOpVmapGuardTests(
    smith._dynamo.test_case.TestCaseWithNestedGraphBreaks, LoggingTestCase
):
    @make_logging_test(recompiles=True)
    def test_vmap_grad_guard_ok(self, records):
        vmap = smith.vmap
        grad = smith.func.grad

        def g(x):
            return vmap(grad(smith.sin))(x)
    ...
    def test_grad_guard_fail(self, records):
        ...
        record = self.getRecord(records, "pyfuncsmith")
        self.assertIn(
            """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([])""",
            munge_exc(record.getMessage()),
        )

    @make_logging_test(recompiles=True)
    def test_dual_level_guard(self, records):
        fwAD = smith.autograd.forward_ad
        ...
        with self.assertRaises(smith._dynamo.exc.InternalSmithDynamoError):
            with fwAD.dual_level():
                fn(foo, tangent)
        self.assertGreater(len(records), 0)
        record = self.getRecord(records, "forward_ad")
        self.assertIn(
            """smith.autograd.forward_ad._current_level == -1""",
            munge_exc(record.getMessage()),
        )
    ...
    def test_jvp_guard_fail(self, records):
        jvp = smith.func.jvp
        vmap = smith.func.vmap

        @smith.compile(backend="eager")
        def fn(x):
            return jvp(smith.sin, (x,), (x,))
        ...
        self.assertGreater(len(records), 0)
        if self.hasRecord(records, "pyfuncsmith"):
            record = self.getRecord(records, "pyfuncsmith")
            self.assertIn(
                """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([])""",
                munge_exc(record.getMessage()),
    ...
    def test_vmap_guard_ok(self, records):
        @smith.compile(backend="eager")
        def fn(x):
            return smith.vmap(lambda x: x.sin())(x)

        x = smith.randn(3, 3, 4, 5)
        y = fn(x)
        # sanity check
        self.assertEqual(len(records), 0)
    ...
    def test_vmap_guard_fail_different_state(self, records):
        ...
        x = smith.zeros(3, 4)
        y = smith.vmap(fn, randomness="same")(x)
        self.assertEqual(x.sin(), y)
        self.assertEqual(len(records), 0)

        # call vmap(vmap(fn))(x) should retrigger compilation
        y = smith.vmap(fn, randomness="different")(x)
        self.assertEqual(x.sin(), y)
        self.assertGreater(len(records), 0)
        record = self.getRecord(records, "pyfuncsmith")
        self.assertIn(
            """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([('Vmap', 1, 'same')])""",
            record.getMessage(),
        )
    ...
    def test_vmap_guard_fail(self, records):
        ...
        x = smith.zeros(3, 3, 4, 5)
        y = smith.vmap(fn)(x)
        self.assertEqual(x.sin(), y)
        self.assertEqual(len(records), 0)

        # call vmap(vmap(fn))(x) should retrigger compilation as
        # _funcsmith.current_level() is not the same
        x = smith.zeros(3, 3, 3, 4, 5)
        y = smith.vmap(smith.vmap(fn))(x)
        self.assertEqual(x.sin(), y)
        self.assertGreater(len(records), 0)
        record = self.getRecord(records, "pyfuncsmith")
        self.assertIn(
            """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([('Vmap', 1, 'error')])""",
            record.getMessage(),
        )
    ...
    def test_vmap_grad_vmap_guard_fail(self, records):
        ...
        def g(x):
            y = vmap(smith.sin, randomness="same")(x)
            return y.sum(0)
    ...
    def test_vmap_recompile_different_states(self, records):
        ...
        x = smith.zeros(3, 3, 4, 5)
        smith.vmap(fn, randomness="same")(x)
        self.assertEqual(len(records), 0)  # sanity check

        smith.vmap(fn, randomness="different")(x)
        self.assertGreater(len(records), 0)
        record = self.getRecord(records, "pyfuncsmith")
        self.assertIn(
            """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([('Vmap', 1, 'same')])""",
    ...
    def test_emit_funcsmith_guard_if_active(self, records):
        @smith.compile(backend="eager")
        def fn(x):
            return smith.sin(x)
        ...
        x = smith.randn(3, 4)
        _ = fn(x)
        self.assertFalse(self.hasRecord(records, "pyfuncsmith"))  # sanity check

        _ = smith.vmap(fn)(x)
        self.assertTrue(self.hasRecord(records, "pyfuncsmith"))
        record = self.getRecord(records, "pyfuncsmith")
        self.assertIn(
            """smith._funcsmith.pyfuncsmith.compare_funcsmith_state([('Vmap', 1, 'error')])""",
            munge_exc(record.getMessage()),
    ...
    def test_linearize_recompiles(self, records):
        @smith.compile(backend="eager")
        def fn(x):
            out, jvp_fn = smith.func.linearize(smith.sin, x)
            return out, jvp_fn(x)
...
class FuncSmithHigherOrderOpTests(
    ...
    def tearDown(self):
        ...
        # i.e. test_vmap_free_tensor fails when BLACKSMITH_TEST_WITH_DYNAMO=1
        # and the call to increment nesting is not undone
        try:
            if TEST_WITH_SMITHDYNAMO:
                warn = False
                while ci := smith._C._funcsmith.peek_interpreter_stack():
                    if ci.key() == smith._C._funcsmith.TransformType.Vmap:
                        warn = True
                        smith._C._funcsmith._vmap_decrement_nesting()
                    else:
                        break
    ...
    def test_teardown_resets_nested_graph_breaks(self):
        expected_nested_state = getattr(
            self, "prev_nested_graph_breaks", smith._dynamo.config.nested_graph_breaks
        )

        def _check_flag():
            self.assertEqual(
                smith._dynamo.config.nested_graph_breaks, expected_nested_state
            )

        self.addCleanup(_check_flag)
        # Sanity check: these tests always run with nested graph breaks enabled.
        self.assertTrue(smith._dynamo.config.nested_graph_breaks)

    def _compile_check(self, fn, inputs, fullgraph=True, graph_idx=0):
        backend = EagerAndRecordGraphs()
        actual = fn(*inputs)
        expected = smith.compile(fn, backend=backend, fullgraph=fullgraph)(*inputs)
    ...
    def test_hessian(self):
        ...
        def wrapper_fn(x):
            return smith.func.hessian(smith.sin)(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]"):
        ...
        _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(12, 'error');  _vmap_increment_nesting = None

        child_1: "f32[4, 3]" = smith._funcsmith.predispatch._add_batch_dim(child, 0, 1);  child = None

        _jvp_increment_nesting = smith._C._funcsmith._jvp_increment_nesting();  _jvp_increment_nesting = None
        _set_fwd_grad_enabled = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled = None
        _enter_dual_level = smith._C._enter_dual_level();  _enter_dual_level = None

        _maybe_load_decompositions = smith.autograd.forward_ad._maybe_load_decompositions();  _maybe_load_decompositions = None

        child_2: "f32[4, 3]" = smith._make_dual(l_x_, child_1, level = 0);  child_1 = None

        _wrap_for_grad: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 2);  l_x_ = _wrap_for_grad = None

        _saved_tensors_hooks_disable = smith._C._autograd._saved_tensors_hooks_disable("smith.func.{grad, vjp, jacrev, hessian} don't yet support saved tensor hooks. Please open an issue with your use case.");  _saved_tensors_hooks_disable = None
        _grad_increment_nesting = smith._C._funcsmith._grad_increment_nesting();  _grad_increment_nesting = None

        diff_primals: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(child_2, 3);  child_2 = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[4, 3]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(diff_primals);  _set_tensor_requires_grad = None

        set_inplace_requires_grad_allowed_1 = smith._C._funcsmith.set_inplace_requires_grad_allowed(False);  set_inplace_requires_grad_allowed_1 = None

        primals_out: "f32[4, 3]" = smith.sin(diff_primals)

        results: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 3)

        _grad_decrement_nesting = smith._C._funcsmith._grad_decrement_nesting();  _grad_decrement_nesting = None
        _saved_tensors_hooks_enable = smith._C._autograd._saved_tensors_hooks_enable();  _saved_tensors_hooks_enable = None

        tensor_1: "i64[1]" = smith.tensor((12,))
        ...
        _vmap_increment_nesting_1 = smith._funcsmith.predispatch._vmap_increment_nesting(12, 'error');  _vmap_increment_nesting_1 = None

        _add_batch_dim_1: "f32[4, 3]" = smith._funcsmith.predispatch._add_batch_dim(basis, 0, 3);  basis = None

        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([primals_out], [diff_primals], [_add_batch_dim_1], retain_graph = True, create_graph = True);  primals_out = diff_primals = _add_batch_dim_1 = None
        batched_outputs: "f32[4, 3]" = _autograd_grad[0];  _autograd_grad = None

        chunked_result: "f32[12, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 3, 12, 0);  batched_outputs = None
        ...
        _unpack_dual = smith._unpack_dual(output_input, level = 0);  output_input = None
        primal: "f32[4, 3, 4, 3]" = _unpack_dual[0]
        dual: "f32[4, 3, 4, 3]" = _unpack_dual[1];  _unpack_dual = None

        primals_out_unflatten: "f32[4, 3, 4, 3]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = primals_out_unflatten = None
        tangents_out_unflatten: "f32[4, 3, 4, 3]" = smith._C._funcsmith._unwrap_for_grad(dual, 2);  dual = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        results_1: "f32[12, 4, 3, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(tangents_out_unflatten, 1, 12, 0);  tangents_out_unflatten = None
...
""",
    ...
    def test_hessian_argnums(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.hessian(fn, argnums=(1,))(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        child_1: "f32[3, 4]" = smith._funcsmith.predispatch._add_batch_dim(child, 0, 1);  child = None
        ...
        child_3: "f32[3, 4]" = smith._make_dual(l_y_, child_1, level = 0);  child_1 = None

        child_2: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 2);  l_x_ = None
        _wrap_for_grad_1: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(l_y_, 2);  l_y_ = _wrap_for_grad_1 = None

        _saved_tensors_hooks_disable = smith._C._autograd._saved_tensors_hooks_disable("smith.func.{grad, vjp, jacrev, hessian} don't yet support saved tensor hooks. Please open an issue with your use case.");  _saved_tensors_hooks_disable = None
        _grad_increment_nesting = smith._C._funcsmith._grad_increment_nesting();  _grad_increment_nesting = None

        _wrap_for_grad_2: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(child_2, 3);  child_2 = None
        child_4: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(child_3, 3);  child_3 = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[3, 4]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(child_4);  _set_tensor_requires_grad = None
        ...
        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([primals_out], [child_4], [_add_batch_dim_1], retain_graph = True, create_graph = True);  primals_out = child_4 = _add_batch_dim_1 = None
        child_5: "f32[3, 4]" = _autograd_grad[0];  _autograd_grad = None

        child_6: "f32[12, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(child_5, 3, 12, 0);  child_5 = None
        ...
        _unpack_dual = smith._unpack_dual(child_7, level = 0);  child_7 = None
        primal: "f32[4, 3, 3, 4]" = _unpack_dual[0];  _unpack_dual = None

        tangent: "f32[4, 3, 3, 4]" = smith.zeros_like(primal)

        child_8: "f32[4, 3, 3, 4]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = child_8 = None
        child_9: "f32[4, 3, 3, 4]" = smith._C._funcsmith._unwrap_for_grad(tangent, 2);  tangent = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        child_10: "f32[12, 4, 3, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(child_9, 1, 12, 0);  child_9 = None
    ...
    def test_jacrev(self):
        ...
        def wrapper_fn(x):
            return smith.func.jacrev(smith.sin)(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]"):
        ...
        diff_primals: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None
        ...
        results: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 1)
        ...
        _add_batch_dim: "f32[4, 3]" = smith._funcsmith.predispatch._add_batch_dim(basis, 0, 1);  basis = None

        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([primals_out], [diff_primals], [_add_batch_dim], retain_graph = True, create_graph = True);  primals_out = diff_primals = _add_batch_dim = None
        batched_outputs: "f32[4, 3]" = _autograd_grad[0];  _autograd_grad = None

        chunked_result: "f32[12, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 1, 12, 0);  batched_outputs = None
...
""",
    ...
    def test_jacrev_two_tensors_argnums(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.jacrev(fn, argnums=1)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        _wrap_for_grad: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = _wrap_for_grad = None
        diff_primals: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(l_y_, 1);  l_y_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[3, 4]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(diff_primals);  _set_tensor_requires_grad = None

        set_inplace_requires_grad_allowed_1 = smith._C._funcsmith.set_inplace_requires_grad_allowed(False);  set_inplace_requires_grad_allowed_1 = None

        primals_out: "f32[3, 4]" = diff_primals.sin()

        results: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 1)
        ...
        _add_batch_dim: "f32[3, 4]" = smith._funcsmith.predispatch._add_batch_dim(basis, 0, 1);  basis = None

        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([primals_out], [diff_primals], [_add_batch_dim], retain_graph = True, create_graph = True);  primals_out = diff_primals = _add_batch_dim = None
        batched_outputs: "f32[3, 4]" = _autograd_grad[0];  _autograd_grad = None

        chunked_result: "f32[12, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 1, 12, 0);  batched_outputs = None
...
""",
    ...
    def test_jacrev_has_aux(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.jacrev(fn, argnums=1, has_aux=True)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        aux: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None
        diff_primals: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(l_y_, 1);  l_y_ = None
        ...
        aux_1: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(aux, 1);  aux = None
        results: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 1)
...
""",
    ...
    def test_vjp(self):
        ...
        def wrapper_fn(x, v):
            (out, vjpfunc) = smith.func.vjp(fn, x)
            return out
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[5]"):
        ...
        _wrap_for_grad: "f32[5]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        child: "f32[5]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(_wrap_for_grad);  child = None
        ...
        results: "f32[]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 1);  primals_out = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[5]", L_v_: "f32[5]"):
        ...
        child_2: "f32[5]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(_wrap_for_grad)
        ...
        _unwrap_for_grad: "f32[5]" = smith._C._funcsmith._unwrap_for_grad(child, 1)
        _unwrap_for_grad_1: "f32[5]" = smith._C._funcsmith._unwrap_for_grad(child_1, 1)

        _grad_decrement_nesting = smith._C._funcsmith._grad_decrement_nesting();  _grad_decrement_nesting = None
        _saved_tensors_hooks_enable = smith._C._autograd._saved_tensors_hooks_enable();  _saved_tensors_hooks_enable = None

        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([child, child_1], [child_2], [l_v_, l_v_], retain_graph = True, create_graph = True);  child = child_1 = child_2 = l_v_ = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[5]", L_v_: "f32[5]"):
        ...
        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad([child, child_1], [child_2], [l_v_, child_3], retain_graph = True, create_graph = True);  child = child_1 = child_2 = l_v_ = child_3 = None
        getitem: "f32[5]" = _autograd_grad[0];  _autograd_grad = None
        return (_unwrap_for_grad, _unwrap_for_grad_1, getitem)
""",
        )
    ...
    def test_vjp_has_aux(self):
        ...
        def wrapper_fn(x, v):
            (out, vjpfunc, _) = smith.func.vjp(fn, x, has_aux=True)
            return out
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[5]"):
        ...
        aux: "f32[5]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        child: "f32[5]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(aux);  child = None
        ...
        aux_1: "f32[5]" = smith._C._funcsmith._unwrap_for_grad(aux, 1);  aux = aux_1 = None
        results: "f32[]" = smith._C._funcsmith._unwrap_for_grad(primals_out, 1);  primals_out = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_model_parameters_weight_: "f32[3, 3]", L_model_parameters_bias_: "f32[3]", L_inputs_: "f32[64, 3]", L_targets_: "f32[64, 3]"):
        ...
        prediction: "f32[64, 3]" = smith._C._nn.linear(l_inputs_, l_model_parameters_weight_, l_model_parameters_bias_);  l_inputs_ = l_model_parameters_weight_ = l_model_parameters_bias_ = None

        mse_loss: "f32[]" = smith.nn.functional.mse_loss(prediction, l_targets_);  prediction = l_targets_ = None
        return (mse_loss,)
""",
            )
        else:
...
""",
    ...
    def test_functional_call_sequential_params_and_buffers(self):
        ...
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l1 = smith.nn.Linear(1, 1)
                self.register_buffer("buffer", smith.ones(1))
                self.foo = 0.0
        ...
        def wrapper_fn(model, params, buffers, inputs):
            ...
            return smith.func.functional_call(model, (params, buffers), inputs)

        model = MockModule()
        params = dict(model.named_parameters())
        buffers = dict(model.named_buffers())
        inputs = smith.tensor([[1.5]])
...
class GraphModule(smith.nn.Module):
    def forward(self, L_inputs_: "f32[1, 1]", L_model_modules_l1_parameters_weight_: "f32[1, 1]", L_model_modules_l1_parameters_bias_: "f32[1]", L_model_buffers_buffer_: "f32[1]"):
        ...
        linear: "f32[1, 1]" = smith._C._nn.linear(l_inputs_, l_model_modules_l1_parameters_weight_, l_model_modules_l1_parameters_bias_);  l_inputs_ = l_model_modules_l1_parameters_weight_ = l_model_modules_l1_parameters_bias_ = None
...
""",
    ...
    def test_functional_call_disable_inline_nn_module(self):
        ...
        expected = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=False)(
            model, params, inputs, targets
        )
        self.assertEqual(len(counters["graph_break"]), 1)
        self.assertIn(
            "smith.func.functional_call capture is disabled",
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]"):
        ...
        diff_args: "f32[3, 3, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[3, 3, 3]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(diff_args);  _set_tensor_requires_grad = None
        ...
        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad((output,), [diff_args], create_graph = True);  diff_args = None
        grad_input: "f32[3, 3, 3]" = _autograd_grad[0];  _autograd_grad = None

        grad_input_1: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(grad_input, 1);  grad_input = None
        output_1: "f32[]" = smith._C._funcsmith._unwrap_for_grad(output, 1);  output = output_1 = None
...
""",
    ...
    def test_grad_freevar_tensor(self):
        ...
        actual = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=True)(x)
        self.assertEqual(actual, expected)
...
""",
    ...
    def test_grad_has_aux(self):
        ...
        def wrapper_fn(x):
            return smith.func.grad(fn, has_aux=True)(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]"):
        ...
        grad_input_1: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(grad_input, 1);  grad_input = None
        output_1: "f32[]" = smith._C._funcsmith._unwrap_for_grad(output, 1);  output = output_1 = None
        aux_1: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(aux, 1);  aux = None
...
""",
    ...
    def test_grad_two_tensor_has_aux(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.grad(fn, has_aux=True)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]", L_y_: "f32[3, 3, 3]"):
        ...
        diff_args: "f32[3, 3, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None
        _wrap_for_grad_1: "f32[3, 3, 3]" = smith._C._funcsmith._wrap_for_grad(l_y_, 1);  l_y_ = None
...
""",
    ...
    def test_grad_two_tensor_all_grad_has_aux(self):
        ...
        def wrapper_fn_const_var(x, y):
            return smith.func.grad(fn, argnums=(0, 1), has_aux=True)(x, y)

        def wrapper_fn_tuple_var(x, y):
            return smith.func.grad(fn, argnums=nums, has_aux=True)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]", L_y_: "f32[3, 3, 3]"):
        ...
        child: "f32[3, 3, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None
        child_1: "f32[3, 3, 3]" = smith._C._funcsmith._wrap_for_grad(l_y_, 1);  l_y_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[3, 3, 3]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(child);  _set_tensor_requires_grad = None

        set_inplace_requires_grad_allowed_1 = smith._C._funcsmith.set_inplace_requires_grad_allowed(False);  set_inplace_requires_grad_allowed_1 = None
        set_inplace_requires_grad_allowed_2 = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed_2 = None

        _set_tensor_requires_grad_1: "f32[3, 3, 3]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(child_1);  _set_tensor_requires_grad_1 = None

        set_inplace_requires_grad_allowed_3 = smith._C._funcsmith.set_inplace_requires_grad_allowed(False);  set_inplace_requires_grad_allowed_3 = None
        ...
        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad((output,), [child, child_1], create_graph = True);  child = child_1 = None
        getitem: "f32[3, 3, 3]" = _autograd_grad[0]
        getitem_1: "f32[3, 3, 3]" = _autograd_grad[1];  _autograd_grad = None

        _unwrap_for_grad: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(getitem, 1);  getitem = None
        _unwrap_for_grad_1: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(getitem_1, 1);  getitem_1 = None
        output_1: "f32[]" = smith._C._funcsmith._unwrap_for_grad(output, 1);  output = output_1 = None
        aux_1: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(aux, 1);  aux = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[]"):
        ...
        diff_args: "f32[]" = smith._C._funcsmith._wrap_for_grad(l_x_, 1);  l_x_ = None

        set_inplace_requires_grad_allowed = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed = None

        _set_tensor_requires_grad: "f32[]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(diff_args);  _set_tensor_requires_grad = None

        set_inplace_requires_grad_allowed_1 = smith._C._funcsmith.set_inplace_requires_grad_allowed(False);  set_inplace_requires_grad_allowed_1 = None
        _saved_tensors_hooks_disable_1 = smith._C._autograd._saved_tensors_hooks_disable("smith.func.{grad, vjp, jacrev, hessian} don't yet support saved tensor hooks. Please open an issue with your use case.");  _saved_tensors_hooks_disable_1 = None
        _grad_increment_nesting_1 = smith._C._funcsmith._grad_increment_nesting();  _grad_increment_nesting_1 = None

        diff_args_1: "f32[]" = smith._C._funcsmith._wrap_for_grad(diff_args, 2)

        set_inplace_requires_grad_allowed_2 = smith._C._funcsmith.set_inplace_requires_grad_allowed(True);  set_inplace_requires_grad_allowed_2 = None

        _set_tensor_requires_grad_1: "f32[]" = smith._funcsmith.eager_transforms._set_tensor_requires_grad(diff_args_1);  _set_tensor_requires_grad_1 = None
        ...
        _autograd_grad = smith._funcsmith.eager_transforms._autograd_grad((output,), [diff_args_1], create_graph = True);  diff_args_1 = None
        grad_input: "f32[]" = _autograd_grad[0];  _autograd_grad = None

        grad_input_1: "f32[]" = smith._C._funcsmith._unwrap_for_grad(grad_input, 2);  grad_input = None
        output_1: "f32[]" = smith._C._funcsmith._unwrap_for_grad(output, 2);  output = output_1 = None

        _grad_decrement_nesting = smith._C._funcsmith._grad_decrement_nesting();  _grad_decrement_nesting = None
        _saved_tensors_hooks_disable_2 = smith._C._autograd._saved_tensors_hooks_disable("smith.func.{grad, vjp, jacrev, hessian} don't yet support saved tensor hooks. Please open an issue with your use case.");  _saved_tensors_hooks_disable_2 = None

        _autograd_grad_1 = smith._funcsmith.eager_transforms._autograd_grad((grad_input_1,), [diff_args], create_graph = True);  diff_args = None
        grad_input_2: "f32[]" = _autograd_grad_1[0];  _autograd_grad_1 = None

        grad_input_3: "f32[]" = smith._C._funcsmith._unwrap_for_grad(grad_input_2, 1);  grad_input_2 = None
        output_2: "f32[]" = smith._C._funcsmith._unwrap_for_grad(grad_input_1, 1);  grad_input_1 = output_2 = None

        _grad_decrement_nesting_1 = smith._C._funcsmith._grad_decrement_nesting();  _grad_decrement_nesting_1 = None
        _saved_tensors_hooks_enable = smith._C._autograd._saved_tensors_hooks_enable();  _saved_tensors_hooks_enable = None
        return (grad_input_3,)
""",
        )
    ...
    def test_grad_with_graph_break(self):
        ...
        x = smith.randn(3, 3, 3)
        actual = wrapper_fn(x)
        expected = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=False)(x)
        self.assertEqual(len(counters["graph_break"]), 1)
        self.assertEqual(actual, expected)
...
""",
    ...
    def test_grad_fn_with_kwargs(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.grad(fn)(x, y=y)

        x = smith.randn(3, 3)
        y = smith.randn(3, 3)
        actual = wrapper_fn(x, y)
        expected = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=False)(x, y)
        self.assertEqual(len(counters["graph_break"]), 0)
        self.assertEqual(actual, expected)
    ...
    def test_jacfwd(self):
        ...
        def wrapper_fn(x):
            return smith.func.jacfwd(smith.sin)(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]"):
        ...
        _make_dual: "f32[4, 3]" = smith._make_dual(l_x_, child_1, level = 0);  child_1 = None

        _wrap_for_grad: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 2);  l_x_ = _wrap_for_grad = None

        result_duals: "f32[4, 3]" = smith.sin(_make_dual);  _make_dual = None

        _unpack_dual = smith._unpack_dual(result_duals, level = 0);  result_duals = None
        primal: "f32[4, 3]" = _unpack_dual[0]
        dual: "f32[4, 3]" = _unpack_dual[1];  _unpack_dual = None

        primals_out_unflatten: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = primals_out_unflatten = None
        tangents_out_unflatten: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(dual, 2);  dual = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        results: "f32[12, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(tangents_out_unflatten, 1, 12, 0);  tangents_out_unflatten = None
...
""",
    ...
    def test_jacfwd_two_tensors_argnums(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.jacfwd(fn, argnums=1)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        _make_dual: "f32[3, 4]" = smith._make_dual(l_y_, child_1, level = 0);  child_1 = None
        ...
        primals_out_unflatten: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = primals_out_unflatten = None
        tangents_out_unflatten: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(dual, 2);  dual = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        results: "f32[12, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(tangents_out_unflatten, 1, 12, 0);  tangents_out_unflatten = None
...
""",
    ...
    def test_jacfwd_has_aux(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.jacfwd(fn, argnums=1, has_aux=True)(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        aux: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 2);  l_x_ = None
        _wrap_for_grad_1: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(l_y_, 2);  l_y_ = _wrap_for_grad_1 = None

        result_duals: "f32[3, 4]" = _make_dual.sin();  _make_dual = None

        aux_1: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(aux, 2);  aux = None
        ...
        results: "f32[12, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(tangents_out_unflatten, 1, 12, 0);  tangents_out_unflatten = None
        aux_2: "f32[12, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(aux_1, 1, 12, 0);  aux_1 = None
...
""",
    ...
    def test_jacfwd_randomness(self):
        ...
        def wrapper_fn(x, y):
            return smith.func.jacfwd(fn, randomness="same")(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4, 3]", L_y_: "f32[3, 4]"):
        ...
        _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(12, 'same');  _vmap_increment_nesting = None
        ...
        child_3: "f32[4, 3]" = smith._make_dual(l_x_, child_1, level = 0);  child_1 = None

        _wrap_for_grad: "f32[4, 3]" = smith._C._funcsmith._wrap_for_grad(l_x_, 2);  l_x_ = _wrap_for_grad = None
        _wrap_for_grad_1: "f32[3, 4]" = smith._C._funcsmith._wrap_for_grad(l_y_, 2);  l_y_ = None

        child_2: "f32[3, 4]" = _wrap_for_grad_1.sin();  _wrap_for_grad_1 = None

        _unpack_dual = smith._unpack_dual(child_2, level = 0);  child_2 = None
        primal: "f32[3, 4]" = _unpack_dual[0];  _unpack_dual = None

        tangent: "f32[3, 4]" = smith.zeros_like(primal)

        _unpack_dual_1 = smith._unpack_dual(child_3, level = 0);  child_3 = None
        primal_1: "f32[4, 3]" = _unpack_dual_1[0]
        dual: "f32[4, 3]" = _unpack_dual_1[1];  _unpack_dual_1 = None

        child_4: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = child_4 = None
        child_5: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(primal_1, 2);  primal_1 = child_5 = None
        child_6: "f32[3, 4]" = smith._C._funcsmith._unwrap_for_grad(tangent, 2);  tangent = None
        child_7: "f32[4, 3]" = smith._C._funcsmith._unwrap_for_grad(dual, 2);  dual = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        child_8: "f32[12, 3, 4]" = smith._funcsmith.predispatch._remove_batch_dim(child_6, 1, 12, 0);  child_6 = None
        child_9: "f32[12, 4, 3]" = smith._funcsmith.predispatch._remove_batch_dim(child_7, 1, 12, 0);  child_7 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3]", L_v_: "f32[3, 3]"):
        ...
        _make_dual: "f32[3, 3]" = smith._make_dual(l_x_, l_v_, level = 0);  l_x_ = l_v_ = None
        ...
        primals_out_unflatten: "f32[]" = smith._C._funcsmith._unwrap_for_grad(primal, 1);  primal = None
        tangents_out_unflatten: "f32[]" = smith._C._funcsmith._unwrap_for_grad(dual, 1);  dual = None
...
""",
    ...
    def test_jvp_has_aux(self):
        ...
        def wrapper_fn(x, v):
            return smith.func.jvp(fn, (x,), (v,), has_aux=True)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3]", L_v_: "f32[3, 3]"):
        ...
        aux: "f32[3, 3]" = smith._make_dual(l_x_, l_v_, level = 0);  l_x_ = l_v_ = None

        sin: "f32[3, 3]" = aux.sin()
        result_duals: "f32[]" = sin.sum();  sin = None

        aux_1: "f32[3, 3]" = smith._C._funcsmith._unwrap_for_grad(aux, 1);  aux = None
...
""",
    ...
    def test_jvp_two_tensors_has_aux(self):
        ...
        def wrapper_fn(x, y, v):
            return smith.func.jvp(fn, (x, y), (v, v), has_aux=True)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3]", L_y_: "f32[3, 3]", L_v_: "f32[3, 3]"):
        ...
        aux: "f32[3, 3]" = smith._make_dual(l_x_, l_v_, level = 0);  l_x_ = None

        _maybe_load_decompositions_1 = smith.autograd.forward_ad._maybe_load_decompositions();  _maybe_load_decompositions_1 = None

        _make_dual_1: "f32[3, 3]" = smith._make_dual(l_y_, l_v_, level = 0);  l_y_ = l_v_ = None
        ...
        primals_out_unflatten: "f32[3, 3]" = smith._C._funcsmith._unwrap_for_grad(primal, 1);  primal = None
        tangents_out_unflatten: "f32[3, 3]" = smith._C._funcsmith._unwrap_for_grad(dual, 1);  dual = None
...
""",
    ...
    def test_jvp_two_tensors_disable_grad(self):
        ...
        def wrapper_fn(x, v):
            with smith.autograd.forward_ad._set_fwd_grad_enabled(False):
                return smith.func.jvp(fn, (x,), (v,))
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3]", L_v_: "f32[3, 3]"):
        ...
        _set_fwd_grad_enabled = smith._C._set_fwd_grad_enabled(False);  _set_fwd_grad_enabled = None
        _jvp_increment_nesting = smith._C._funcsmith._jvp_increment_nesting();  _jvp_increment_nesting = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None
        _enter_dual_level = smith._C._enter_dual_level();  _enter_dual_level = None
        ...
        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_2 = smith._C._set_fwd_grad_enabled(False);  _set_fwd_grad_enabled_2 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None
        _set_fwd_grad_enabled_3 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_3 = None
        return (primals_out_unflatten, tangents_out_unflatten)
""",
        )
    ...
    def test_jvp_two_tensors_disable_enable_disable_grad(self):
        ...
        def wrapper_fn(x, v):
            with smith.autograd.forward_ad._set_fwd_grad_enabled(False):  # (1)
                with smith.autograd.forward_ad._set_fwd_grad_enabled(True):  # (2)
                    with smith.autograd.forward_ad._set_fwd_grad_enabled(False):  # (3)
                        return smith.func.jvp(fn, (x,), (v,))  # (4)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3]", L_v_: "f32[3, 3]"):
        ...
        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_4 = smith._C._set_fwd_grad_enabled(False);  _set_fwd_grad_enabled_4 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None
        _set_fwd_grad_enabled_5 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_5 = None
        _set_fwd_grad_enabled_6 = smith._C._set_fwd_grad_enabled(False);  _set_fwd_grad_enabled_6 = None
        _set_fwd_grad_enabled_7 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_7 = None
        return (primals_out_unflatten, tangents_out_unflatten)
""",
        )
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]"):
        ...
        child: "f32[3, 3, 3]" = smith._make_dual(l_x_, l_x_, level = 0);  l_x_ = None

        _jvp_increment_nesting_1 = smith._C._funcsmith._jvp_increment_nesting();  _jvp_increment_nesting_1 = None
        _set_fwd_grad_enabled_1 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_1 = None

        _maybe_load_decompositions_1 = smith.autograd.forward_ad._maybe_load_decompositions();  _maybe_load_decompositions_1 = None

        _make_dual_1: "f32[3, 3, 3]" = smith._make_dual(child, child, level = 0);  child = None

        result_duals: "f32[3, 3, 3]" = smith.sin(_make_dual_1);  _make_dual_1 = None

        _unpack_dual = smith._unpack_dual(result_duals, level = 0);  result_duals = None
        primal: "f32[3, 3, 3]" = _unpack_dual[0]
        dual: "f32[3, 3, 3]" = _unpack_dual[1];  _unpack_dual = None

        primals_out_unflatten: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(primal, 2);  primal = None
        tangents_out_unflatten: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(dual, 2);  dual = None

        _set_fwd_grad_enabled_2 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_2 = None
        _jvp_decrement_nesting = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting = None

        _unpack_dual_1 = smith._unpack_dual(primals_out_unflatten, level = 0);  primals_out_unflatten = None
        primal_1: "f32[3, 3, 3]" = _unpack_dual_1[0]
        dual_1: "f32[3, 3, 3]" = _unpack_dual_1[1];  _unpack_dual_1 = None
        _unpack_dual_2 = smith._unpack_dual(tangents_out_unflatten, level = 0);  tangents_out_unflatten = None
        primal_2: "f32[3, 3, 3]" = _unpack_dual_2[0]
        dual_2: "f32[3, 3, 3]" = _unpack_dual_2[1];  _unpack_dual_2 = None

        _unwrap_for_grad_2: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(primal_1, 1);  primal_1 = None
        _unwrap_for_grad_3: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(primal_2, 1);  primal_2 = None
        _unwrap_for_grad_4: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(dual_1, 1);  dual_1 = None
        _unwrap_for_grad_5: "f32[3, 3, 3]" = smith._C._funcsmith._unwrap_for_grad(dual_2, 1);  dual_2 = None

        _exit_dual_level = smith._C._exit_dual_level(0);  _exit_dual_level = None
        _set_fwd_grad_enabled_3 = smith._C._set_fwd_grad_enabled(True);  _set_fwd_grad_enabled_3 = None
        _jvp_decrement_nesting_1 = smith._C._funcsmith._jvp_decrement_nesting();  _jvp_decrement_nesting_1 = None
...
""",
    ...
    def test_linearize_jvp_fn(self):
        ...
        def wrapper_fn(x):
            output, jvp_fn = smith.func.linearize(smith.sin, x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_self_buffers_tensor_constant0_: "f32[3, 3, 3]"):
        ...
        alias_default: "f32[3, 3, 3]" = smith.ops.aten.alias.default(l_self_buffers_tensor_constant0_);  l_self_buffers_tensor_constant0_ = None

        sin_default: "f32[3, 3, 3]" = smith.ops.aten.sin.default(alias_default)

        alias_default_1: "f32[3, 3, 3]" = smith.ops.aten.alias.default(alias_default)

        cos_default: "f32[3, 3, 3]" = smith.ops.aten.cos.default(alias_default_1);  alias_default_1 = None

        alias_default_2: "f32[3, 3, 3]" = smith.ops.aten.alias.default(sin_default);  alias_default_2 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_self_modules_FX_CONST_FOLDED_ATTRS_parameters_0_: "f32[3, 3, 3]", L_self_modules_FX_CONST_FOLDED_ATTRS_parameters_1_: "f32[3, 3, 3]", L_flat_tangents_1_: "f32[3, 3, 3]"):
        ...
        _new_zeros_with_same_feature_meta_default: "f32[3, 3, 3]" = smith.ops.aten._new_zeros_with_same_feature_meta.default(l_flat_tangents_1_, l_self_modules_fx_const_folded_attrs_parameters_0_);  l_self_modules_fx_const_folded_attrs_parameters_0_ = None

        copy__default: "f32[3, 3, 3]" = smith.ops.aten.copy_.default(_new_zeros_with_same_feature_meta_default, l_flat_tangents_1_);  _new_zeros_with_same_feature_meta_default = l_flat_tangents_1_ = None

        mul_tensor: "f32[3, 3, 3]" = smith.ops.aten.mul.Tensor(copy__default, l_self_modules_fx_const_folded_attrs_parameters_1_);  copy__default = l_self_modules_fx_const_folded_attrs_parameters_1_ = None
        return (mul_tensor,)
""",
        )
    ...
    def test_vmap_recompile(self):
        ...
        x = smith.zeros(3, 3, 4, 5)
        smith.vmap(fn)(x)
        # should not recompile on second call. See Blacksmith issue #118493
        smith.vmap(fn)(x)
    ...
    def test_vmap_recompile_same_config(self):
        ...
        x = smith.zeros(3, 3, 4, 5)
        smith.vmap(smith.vmap(fn, randomness="same"), randomness="same")(x)
        with self.assertRaises(smith._dynamo.exc.RecompileError):
            smith.vmap(smith.vmap(fn, randomness="same"), randomness="error")(x)
    ...
    def test_vmap_call_smith_compile_fn(self):
        def wrapped_fn(x):
            return x.sin()

        x = smith.randn(3, 4)
        fn = smith.compile(backend="aot_eager", fullgraph=True)(wrapped_fn)

        with self.assertRaisesRegex(
            smith._dynamo.exc.Unsupported,
            "Calling smith.func.vmap\\(compiled_fn\\) function from eager mode is not supported",
        ):
            smith.func.vmap(fn)(x)

    def test_vmap_call_compiled_backward_fn(self):
        # See Blacksmith issue #138422
        @smith.compile
        def f(x):
            return x**2
    ...
    def test_grad_call_compiled_backward_fn(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith.func.grad(get_vjp)(x)

    def test_grad_call_smith_compile_fn(self):
        def wrapped_fn(x):
            return x.sin().sum()
        ...
        with self.assertRaisesRegex(
            smith._dynamo.exc.Unsupported,
            "Calling smith.func.grad\\(compiled_fn\\) function from eager mode is not supported",
        ):
            smith.func.grad(fn)(x)

    def test_jvp_call_smith_compile_fn(self):
        def wrapped_fn(x):
            return x.sin().sum()
        ...
        with self.assertRaisesRegex(
            smith._dynamo.exc.Unsupported,
            "Calling smith.func.jvp\\(compiled_fn\\) function from eager mode is not supported",
        ):
            smith.func.jvp(fn, (x,), (x,))

    @config.patch(error_on_recompile=True)
    def test_grad_recompile(self):
        @smith.compile(backend="eager")
        def fn(x):
            return smith.func.grad(smith.sin)(x)
    ...
    def test_vmap_with_graph_break(self):
        ...
        x = smith.randn(3, 4)
        opt = smith.compile(fn, backend="aot_eager", fullgraph=False)
        expected = fn(x)
        got = opt(x)
        self.assertEqual(len(counters["graph_break"]), 1)
        self.assertEqual(expected, got)
    ...
    def test_vmap_with_graph_break_2(self):
        ...
        def fn(x):
            return smith.vmap(g, randomness="same")(x)
    ...
    def test_vmap_with_graph_break_lambda(self):
        ...
        def fn(x):
            return smith.vmap(lambda x: sin(x))(x)
    ...
    def test_vmap_scalar_tensor_indexing(self):
        data = smith.arange(20).reshape(2, 10)
        b_indices = smith.arange(2)
        n_indices = smith.arange(10)

        def vmap_index_fn(data_in, b_indices, n_indices):
            def index_fn(b, n):
                return data_in[b, n]

            return smith.func.vmap(index_fn, in_dims=(None, 0))(b_indices, n_indices)

        eager_result = vmap_index_fn(data, b_indices, n_indices)

        compiled_result = smith.compile(vmap_index_fn, backend="eager", fullgraph=True)(
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]"):
        ...
        _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(3, 'error');  _vmap_increment_nesting = None

        _add_batch_dim: "f32[3, 3]" = smith._funcsmith.predispatch._add_batch_dim(l_x_, 0, 1);  l_x_ = None
        ...
        _remove_batch_dim: "f32[3, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 1, 3, 0);  batched_outputs = None
...
""",
    ...
    def test_vmap_free_const(self):
        ...
        def fn(x):
            return smith.func.vmap(lambda x: x.sum(0) + x.sum(1) + y)(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_y_: "f32[3, 3]", L_x_: "f32[3, 3, 3]"):
        ...
        _remove_batch_dim: "f32[3, 3, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 1, 3, 0);  batched_outputs = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]", L_y_: "f32[3, 3]"):
        ...
        _add_batch_dim: "f32[3, 3]" = smith._funcsmith.predispatch._add_batch_dim(l_x_, 0, 1);  l_x_ = None
        _add_batch_dim_1: "f32[3]" = smith._funcsmith.predispatch._add_batch_dim(l_y_, 1, 1);  l_y_ = None
...
""",
    ...
    def test_vmap_over_vmap_two_inputs(self):
        def fn(x, y):
            return smith.func.vmap(smith.func.vmap(lambda x, y: x + y, in_dims=1))(x, y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[3, 3, 3]", L_y_: "f32[3, 3, 3]"):
        ...
        child: "f32[3, 3]" = smith._funcsmith.predispatch._add_batch_dim(l_x_, 0, 1);  l_x_ = None
        child_1: "f32[3, 3]" = smith._funcsmith.predispatch._add_batch_dim(l_y_, 0, 1);  l_y_ = None

        lazy_load_decompositions_1 = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions_1 = None

        _vmap_increment_nesting_1 = smith._funcsmith.predispatch._vmap_increment_nesting(3, 'error');  _vmap_increment_nesting_1 = None

        _add_batch_dim_2: "f32[3]" = smith._funcsmith.predispatch._add_batch_dim(child, 1, 2);  child = None
        _add_batch_dim_3: "f32[3]" = smith._funcsmith.predispatch._add_batch_dim(child_1, 1, 2);  child_1 = None

        batched_outputs: "f32[3]" = _add_batch_dim_2 + _add_batch_dim_3;  _add_batch_dim_2 = _add_batch_dim_3 = None

        batched_outputs_1: "f32[3, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 2, 3, 0);  batched_outputs = None

        _vmap_decrement_nesting = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting = None

        _remove_batch_dim_1: "f32[3, 3, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs_1, 1, 3, 0);  batched_outputs_1 = None
...
""",
    ...
    def test_vmap_over_vmap_captured(self):
        ...
        def fn(x):
            return smith.func.vmap(smith.func.vmap(lambda y: x * y))(y)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[2, 3]", L_y_: "f32[5, 3]"):
        ...
        _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(5, 'error');  _vmap_increment_nesting = None

        child: "f32[3]" = smith._funcsmith.predispatch._add_batch_dim(l_y_, 0, 1);  l_y_ = None

        lazy_load_decompositions_1 = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions_1 = None

        _vmap_increment_nesting_1 = smith._funcsmith.predispatch._vmap_increment_nesting(3, 'error');  _vmap_increment_nesting_1 = None

        _add_batch_dim_1: "f32[]" = smith._funcsmith.predispatch._add_batch_dim(child, 0, 2);  child = None

        batched_outputs: "f32[2, 3]" = l_x_ * _add_batch_dim_1;  l_x_ = _add_batch_dim_1 = None

        batched_outputs_1: "f32[3, 2, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs, 2, 3, 0);  batched_outputs = None

        _vmap_decrement_nesting = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting = None

        _remove_batch_dim_1: "f32[5, 3, 2, 3]" = smith._funcsmith.predispatch._remove_batch_dim(batched_outputs_1, 1, 5, 0);  batched_outputs_1 = None
...
""",
    ...
    def test_vmap_multiple_outputs(self):
        ...
        def fn(x):
            return smith.vmap(lambda x: (x.sum(0), x.sum(1)))(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[2, 4, 3]"):
        ...
        _add_batch_dim: "f32[4, 3]" = smith._funcsmith.predispatch._add_batch_dim(l_x_, 0, 1);  l_x_ = None

        child: "f32[3]" = _add_batch_dim.sum(0)
        child_1: "f32[4]" = _add_batch_dim.sum(1);  _add_batch_dim = None

        _remove_batch_dim: "f32[2, 3]" = smith._funcsmith.predispatch._remove_batch_dim(child, 1, 2, 0);  child = None
        _remove_batch_dim_1: "f32[2, 4]" = smith._funcsmith.predispatch._remove_batch_dim(child_1, 1, 2, 0);  child_1 = None
...
""",
    ...
    def test_vmap_multiple_outputs_diff_dims(self):
        ...
        def fn(x):
            return smith.vmap(lambda x: (x.sum(0), x.sum(1)), out_dims=(1, 0))(x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[2, 4, 3]"):
        ...
        _remove_batch_dim: "f32[3, 2]" = smith._funcsmith.predispatch._remove_batch_dim(child, 1, 2, 1);  child = None
        _remove_batch_dim_1: "f32[2, 4]" = smith._funcsmith.predispatch._remove_batch_dim(child_1, 1, 2, 0);  child_1 = None
...
""",
    ...
    def test_vmap_multiple_outputs_out_dims_tuple(self):
        ...
        def fn(x):
            return smith.vmap(lambda x: (x.sum(0), x.sum(1)), out_dims=out_dims)(x)
...
""",
    ...
    def test_vmap_kwargs(self):
        ...
        def fn(x, y):
            return smith.func.vmap(lambda x, y: x + y)(x, y=y)

        actual = fn(x, y)
        expected = smith.compile(fn, backend="aot_eager", fullgraph=False)(x, y)
        self.assertEqual(len(counters["graph_break"]), 0)
        self.assertEqual(actual, expected)
    ...
    def test_vmap_pytree_inputs(self):
        ...
        def fn(x, y):
            return smith.func.vmap(vmap_fn)({"x": x, "y": y})
    ...
    def test_vmap_previous_illegal_op_no_graph_break(self):
        ...
        def wrapper_fn(x):
            return smith.func.vmap(bad_fn)(x)
    ...
    def test_vmap_multiple_invocation_in_dims(self):
        ...
        def wrapper_fn(x, in_dims):
            return smith.func.vmap(smith.sum, in_dims)(x)

        x = smith.randn(3, 3, 3, 3)
        cnt = CompileCounter()
        opt = smith.compile(wrapper_fn, backend=cnt, fullgraph=False, dynamic=True)
    ...
    def test_vmap_multiple_invocation_out_dims(self):
        ...
        def wrapper_fn(x, out_dims):
            return smith.func.vmap(lambda x: smith.sum(x, 0), out_dims=out_dims)(x)
    ...
    def test_vmap_out_dims_None(self):
        # issue https://github.com/blacksmith/blacksmith/issues/149509
        def fn(x, y):
            return x, y * 2

        def wrapper_fn(x, y):
            return smith.func.vmap(fn, in_dims=(None, 0), out_dims=(None, 0))(x, y)

        x, y = smith.randn(4), smith.randn(3, 4)
        expected = wrapper_fn(x, y)
        got = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=True)(x, y)
        self.assertEqual(expected, got)
    ...
    def test_vmap_new_tensor_in_body(self):
        ...
        def wrapper_fn(x):
            return smith.func.vmap(fn)(x)

        x = smith.randn(
            3,
        )
        opt = smith.compile(wrapper_fn, backend="aot_eager", fullgraph=True)
        expected = wrapper_fn(x)
        actual = opt(x)
        self.assertEqual(expected, actual)
    ...
    def test_vmap_new_tensor_implicit_via_op(self):
        def wrapper_fn(x):
            return smith.func.vmap(lambda t: smith.add(t, 0.5))(x)
...
class ActivationCheckpointingTests(
    ...
    def _validate(self, fn, backend, *args, skip_check=False, fullgraph=True):
        ...
        opt_fn = smith.compile(fn, fullgraph=fullgraph, backend=backend)
        smith.manual_seed(0)
    ...
    @requires_cuda_and_triton
    @smith._funcsmith.config.patch(functionalize_rng_ops=True)
    def test_function(self):
        def gn(x, y):
            return smith.sigmoid(smith.matmul(x, y))

        def fn(x, y):
            return smith.utils.checkpoint.checkpoint(
                gn, smith.sin(x), y, use_reentrant=True
            )

        x = smith.randn(4, 4, requires_grad=True)
        y = smith.randn(4, 4, requires_grad=True)

        fw_compiler = functools.partial(count_ops, freq=1, op=smith.ops.aten.mm.default)
        bw_compiler = functools.partial(count_ops, freq=2, op=smith.ops.aten.mm.default)
    ...
    def test_dropout(self):
        def gn(x, y):
            return smith.nn.functional.dropout(smith.matmul(x, y), p=0.2)
        ...
        fw_compiler = functools.partial(
            count_ops, freq=1, op=smith.ops.rngprims.philox_rand.default
        )
        # philox_rand is passed from fwd
        bw_compiler = functools.partial(
            count_ops, freq=0, op=smith.ops.rngprims.philox_rand.default
        )
        backend = aot_autograd(fw_compiler=fw_compiler, bw_compiler=bw_compiler)
        self._validate(
    ...
    def test_fallback(self):
        ...
        def fn(x, y):
            return smith.cos(
                smith.utils.checkpoint.checkpoint(
                    gn, smith.sin(x), y, use_reentrant=True
                ),
            )
        ...
        expected = fn(*args)
        result = smith.compile(fn, backend=cnt)(*args)

        self.assertEqual(result, expected)

        # One graph for smith.sin on the input, and other for smith.cos.
        self.assertEqual(cnt.frame_count, 2)
        self.assertEqual(cnt.op_count, 2)
    ...
    def test_module(self):
        class MockModule(smith.nn.Module):
            ...
            def forward(self, x):
                return smith.sigmoid(self.linear(x))

        mod = MockModule()

        def fn(x):
            return smith.utils.checkpoint.checkpoint(
                mod, smith.sin(x), use_reentrant=True
            )

        x = smith.randn(10, 10, requires_grad=True)

        fw_compiler = functools.partial(
            count_ops, freq=1, op=smith.ops.aten.sigmoid.default
        )
        # sigmoid passed from fwd
        bw_compiler = functools.partial(
            count_ops, freq=0, op=smith.ops.aten.sigmoid.default
        )
        backend = aot_autograd(fw_compiler=fw_compiler, bw_compiler=bw_compiler)
        self._validate(fn, backend, x)

    def test_override_fallthrough_dispatch_key(self):
        class _FallthroughTestOnly(smith._ops.HigherOrderOperator):
            def __init__(self):
        ...
        default_keys = smith._ops._HIGHER_ORDER_OP_DEFAULT_FALLTHROUGH_DISPATCH_KEYS
        self.assertTrue(
    ...
    def test_cond_with_kwargs(self):
        ...
        cnt = CompileCounter()
        opt_test = smith.compile(test, backend=cnt, fullgraph=True)
        inp = smith.ones(3, 3)
        true_pred = smith.Tensor([True])
        false_pred = smith.Tensor([False])
        self.assertTrue(smith.allclose(test(true_pred, inp), opt_test(true_pred, inp)))
        self.assertEqual(cnt.frame_count, 1)
        self.assertTrue(
            smith.allclose(test(false_pred, inp), opt_test(false_pred, inp))
        )
        self.assertEqual(cnt.frame_count, 1)
    ...
    def test_cond_with_invalid_kwargs(self):
        ...
        cnt = CompileCounter()
        opt_test = smith.compile(test, backend=cnt)
        inp = smith.ones(3, 3)
    ...
    def test_cond_with_mismatched_output(self):
        def output_mismatch_test(x):
            def true_fn():
                return smith.concat([x, x])

            def false_fn():
                return x.sin()

            return smith.cond(x.sum() > 0, true_fn, false_fn)

        x = smith.randn(2, 3)
        output_mismatch_test(x)

        smith.compile(output_mismatch_test, backend="eager")(x)

    def test_non_aliasing_util(self):
        from smith._dynamo.variables.higher_order_ops import _assert_tensors_nonaliasing

        a = [smith.tensor(1), {"a": smith.tensor(1)}]
        b = (smith.tensor(1),)
    ...
    def test_flop_counter_for_cond(self):
        ...
        self.assertEqual(
            ...
            {
                "Global": {smith.ops.aten.addmm: 256},
                "Mod": {smith.ops.aten.addmm: 256},
                "Mod.linear": {smith.ops.aten.addmm: 256},
            },
        )
    ...
    def test_flop_counter_for_nested_cond(self):
        ...
        class Mod(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear1 = smith.nn.Linear(4, 4)
                self.linear2 = smith.nn.Linear(4, 4)
            ...
            def forward(self, x):
                ...
                return smith.cond(smith.tensor(True), true_branch, false_branch, (x,))
        ...
        self.assertEqual(
            mode.get_flop_counts(),
            {
                "Global": {smith.ops.aten.addmm: 256},
                "Mod": {smith.ops.aten.addmm: 256},
                "Mod.linear1": {smith.ops.aten.addmm: 128},
                "Mod.linear2": {smith.ops.aten.addmm: 128},
            },
        )
    ...
    def test_flop_counter_for_cond_unbalanced_branches(self):
        ...
        self.assertEqual(
            mode.get_flop_counts(),
            {
                "Global": {smith.ops.aten.addmm: 128},
                "Mod": {smith.ops.aten.addmm: 128},
                "Mod.linear": {smith.ops.aten.addmm: 128},
            },
        )
...
class TestHigherOrderOpsOpInfo(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    @requires_cuda_and_triton
    @parametrize("backend", ("aot_eager", "inductor"))
    @ops(
        list(filter(lambda op: op.name not in xfail_hops_compile, hop_db)),
        allowed_dtypes=(smith.float,),
    )
