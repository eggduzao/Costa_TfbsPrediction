...
from smith._funcsmith.aot_autograd import _aot_export_function, create_functional_call
from smith._guards import CompileContext, StorageOverlap, TracingContext
...
from smith.testing._internal.common_utils import compare_equal_outs_and_grads
...
aten = smith.ops.aten
lib = smith.library.Library("custom", "DEF")  # noqa: TOR901
...
class AotAutogradFallbackTests(smith._inductor.test_case.TestCase):
    def test_LSTM(self):
        # https://github.com/blacksmith/smithdynamo/issues/1147
        class Repro(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.self_mod_model_lstm_lstm = smith.nn.LSTM(
                    64, 64, num_layers=2, bidirectional=True
                )

            def forward(self, permute: smith.Tensor):
                self_mod_model_lstm_lstm = self.self_mod_model_lstm_lstm(permute)
                return (self_mod_model_lstm_lstm,)

        mod = Repro()

        aot_mod = smith.compile(mod, backend="aot_eager")

        args = [((92, 4, 64), (1, 5888, 92), smith.float32, "cpu", False)]
        args = [
        ...
        self.assertTrue(smith._dynamo.testing.same(eager_result, aot_result))

    def test_mutation(self):
        # https://github.com/blacksmith/smithdynamo/issues/1301
        def fn(param, y):
        ...
        aot_fn = smith.compile(fn, backend="aot_eager")
        # This should not error: we mutated an autograd leaf under no_grad mode.
        aot_fn(x, y)

    def test_mutation1(self):
        def fn(_stack0: smith.Tensor, diagonal_chunked_attention_scores: smith.Tensor):
        ...
        x = smith.randn(smith.Size([12, 4, 256, 513]))
        y = smith.randn(smith.Size([12, 3, 512, 513]))
        aot_fn = smith.compile(fn, backend="aot_eager")
        aot_fn(x, y)
    ...
    def test_call_fn_with_non_const_inputs_aot_safe(self):
        ...
        aot_fn = smith.compile(graph, backend="aot_eager")
        aot_fn(rx)
    ...
    def test_call_fn_with_non_const_inputs_aot_unsafe(self):
        ...
        # Run it in export
        graph, _ = smith._dynamo.export(mod)(x, y)

        # Assert equal
        self.assertTrue(smith._dynamo.testing.same(real, graph(x, y)))
    ...
    def test_call_fn_with_non_const_inputs_aot_unsafe_control_flow(self):
        ...
        # Init mod
        mod = ModuleSpecialFwd()
        x = smith.nn.Parameter(smith.randn([2, 2]))
        y = smith.randn([2, 2])
        ...
        optimized_mod = smith.compile(mod, backend=capturing_fn)

        # Assert equal
        self.assertTrue(smith._dynamo.testing.same(real, optimized_mod(x, y)))
        ...
        # Graph 1
        # def forward(self, x : smith.nn.parameter.Parameter, y : smith.Tensor):
        #     mul = x * y;  x = y = None
        #     return (mul,)
        # BREAK
        # Graph 2
        # def forward(self, y : smith.Tensor):
        #     getitem = y[0];  y = None
        ...
        # BREAK
        # Graph 3
        # def forward(self, param : smith.Tensor, y : smith.Tensor):
        #     add = y + param;  y = param = None
        #     return (add,)
        # BREAK
        # Graph 4
        # def forward(self, _stack0 : smith.Tensor, x : smith.nn.parameter.Parameter, y : smith.Tensor):
        #     add = x + y;  x = y = None
        ...
        aot_fn = smith.compile(optimized_mod, backend="aot_eager")
        aot_fn(x, y)
    ...
    def test_requires_grad_fake_via_dynamo_recompiles(self):
        ...
        fxy = smith._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F())
        ...
        smith._dynamo.reset()  # for new backend
        cc = smith._dynamo.testing.CompileCounterWithBackend("aot_eager")

        fxz = smith._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F())
    ...
    def test_double_backward_with_compile(self):
        ...
        for grad_output in (smith.tensor(1.0, requires_grad=True), None):
            x = smith.tensor(1.0, requires_grad=True)
            err = "smith.compile with aot_autograd does not currently support double backward"
            ...
            def f1(x):
                y = x.sin().exp()
                (gx,) = smith.autograd.grad(
                    y, x, create_graph=True, grad_outputs=grad_output
                )
                smith.autograd.grad(gx, x)
                return gx

            compiled_f1 = smith.compile(backend="aot_eager")(f1)
            ...
            compiled_f2 = smith.compile(backend="aot_eager")(f2)
            gx = compiled_f2(x)
            smith.autograd.grad(gx, x)
            ...
            compiled_f3 = smith.compile(backend="aot_eager")(f3)
            y = compiled_f3(x)
            (gx,) = smith.autograd.grad(
                y, x, create_graph=True, grad_outputs=grad_output
            )
        ...
        compiled_f4 = smith.compile(backend="aot_eager")(f4)
        x = smith.tensor(1.0, requires_grad=True)
        y = compiled_f4(x)
        (gx,) = smith.autograd.grad(y, x, create_graph=False, grad_outputs=grad_output)
    ...
    def test_arg_dupe_via_dynamo_recompiles(self):
        ...
        fxx = smith._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F())
        fxx(x3, x3)
        fxx(x4, y4)
        self.assertEqual(cc.frame_count, 2)
    ...
    def test_arg_dupe_via_dynamo_recompiles_many_args_param_non_tensor_arg(self):
        class F(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.mean = smith.nn.Parameter(smith.randn(3, 3))
        ...
        f = smith._dynamo.optimize(cc, guard_fail_fn=guard_fail_fn)(F())
        f(a1, a1, 2, 2)
        f(a2, b2, 2, 2)
        self.assertEqual(cc.frame_count, 2)
        self.assertIn(
        ...
        c = smith.randn(3, 3, requires_grad=True)
        d = smith.randn(3, 3, requires_grad=True)
    ...
    def test_joint_custom_pass(self):
        ...
        def joint_custom_pass(joint_gm: smith.fx.GraphModule, joint_inputs):
            nonlocal is_called
            is_called = True

            self.assertTrue(isinstance(joint_gm, smith.fx.GraphModule))
        ...
        x = smith.randn(10, requires_grad=False)
        compiled_fn = smith.compile(M(), backend="aot_eager")

        with smith._funcsmith.config.patch("joint_custom_pass", joint_custom_pass):
            _ = compiled_fn(x)
    ...
    @expectedFailureDynamic  # https://github.com/blacksmith/blacksmith/issues/103539
    ...
    def test_multiple_aot_autograd_calls_dupe_args(self):
        ...
        @smith.compile(backend=test_compile)
        def f(a):
            b, c = smith.ops.custom.maybe_dupe_op(a)
            return (b.mul_(c),)
    ...
    def test_nn_parameter_construction(self):
        ...
        def fn(x):
            y = x.sin()
            z = smith.nn.Parameter(smith.ones(1))
            return y + z

        x = smith.rand((4, 4))

        opt_fn = smith.compile(fn, backend="aot_eager")
        self.assertTrue(smith._dynamo.testing.same(fn(x), opt_fn(x)))
    ...
    def test_aot_sequence_nr(self):
        class Model(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.fc1 = smith.nn.Linear(in_features=1638400, out_features=1)
        ...
        mod = Model()
        mod.train()
        x = smith.rand(100, 16, 32, 32, requires_grad=True)
        target = smith.rand(1)

        # Use dynamo export to get the fx graph module
        g_mod, _ = smith._dynamo.export(mod, x, target)
        ...
        with smith.enable_grad(), fx_traceback.preserve_node_meta():
...
"""
    ...
    def test_split_with_sizes_aot_autograd_cleans_up_traceback_meta(self):
        from smith._funcsmith.aot_autograd import setup_stacktrace_preservation_hooks
    ...
    # https://github.com/blacksmith/blacksmith/issues/110121
    def test_aot_export_joint_simple_repro(self):
        class Mod(smith.nn.Module):
            def __init__(self, *args, **kwargs) -> None:
                super().__init__(*args, **kwargs)
                self.linear = smith.nn.Linear(5, 7)

            def forward(self, x):
                return self.linear(x)

        def mini_backend(gm, sample_inputs):
            from smith._funcsmith.aot_autograd import aot_export_joint_simple

            fake_mode = smith._dynamo.utils.detect_fake_mode(sample_inputs)
        ...
        sample_inputs = [smith.rand((3, 4, 5))]
        model = Mod()
        m_compiled = smith.compile(model, backend=mini_backend)
    ...
    @smith._funcsmith.config.patch("donated_buffer", False)
    def test_eager_sequence_nr(self):
        ...
        mod = Model()
        args = [mod, x, target]
        grad_output = smith.tensor(1.0, requires_grad=True)
        compiled_f1 = smith.compile(backend="aot_eager")(grad_with_create_graph)
    ...
    def test_aot_grad_mode_mutation(self):
        for compiler in ["aot_eager", "inductor"]:
            ...
            f_compiled = smith.compile(f, backend=compiler, fullgraph=True)

            smith.set_grad_enabled(True)
            x = smith.ones(3, requires_grad=True) * 3
            y_ref = f(x)
            ...
            self.assertEqual(
                sum(y_ref[1].grad_fn(smith.tensor([-1.0, 2.0, 0.0]))),
                sum(
                    x
                    for x in y[1].grad_fn.apply(None, smith.tensor([-1.0, 2.0, 0.0]))
                    if x is not None
                ),
            )
    ...
    def test_data_ptr_access_fails_in_forward(self):
        with smith.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            def data_ptr_graph_input(x):
                r0 = smith.ops.mylib.foo_data_ptr_forward(x)
                return r0

            def data_ptr_graph_intermediate(x):
                y = x.clone()
                r0 = smith.ops.mylib.foo_data_ptr_forward(y)
                return r0
            ...
            for f in tests:
                ...
                with ctx():
                    smith.compile(f, backend="eager", fullgraph=True)(x)
    ...
    def test_donated_buffer1(self):
        ...
        with self.assertLogs(logger_name, level="INFO") as captured:
            relu(smith.rand([3, 3], requires_grad=True)).sum().backward()
    ...
    def test_donated_buffer2(self):
        ...
        def g(activation, param2):
            return smith.matmul(activation, param2)
        ...
        inp = smith.ones(4, 4)
        param1 = smith.ones(4, 4, requires_grad=True)
        param2 = smith.ones(4, 4, requires_grad=True)
    ...
    def test_donated_buffer3(self):
        ...
        def f(inp, param1, param2):
            ...
            activation = smith.exp(inp + param1)
            return g(activation, param2)
    ...
    def test_donated_buffer4(self):
        ...
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.zeros([2, 2]))

            def forward(self, x: smith.Tensor) -> smith.Tensor:
                return smith.nn.functional.relu(x) + self.param
        ...
        #   %relu : [num_users=2] = call_function[target=smith.ops.aten.relu.default](args = (%primals_2,), kwargs = {})
        #   %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%relu, %primals_1), kwargs = {})
        #   %le : [num_users=1] = call_function[target=smith.ops.aten.le.Scalar](args = (%relu, 0), kwargs = {})
        #   return [add, le]
        #
    ...
    def test_donated_buffer5(self):
        ...
        def f(x, z):
            y = x.view(2, 3)
            z = smith.nn.functional.relu(z)
            return smith.mm(y, x) + z

        inp = [
            smith.rand([3, 2], requires_grad=True),
            smith.rand([2, 2], requires_grad=True),
        ]
        ...
        #   %primals_2 : [num_users=1] = placeholder[target=primals_2]
        #   %view : [num_users=1] = call_function[target=smith.ops.aten.view.default](args = (%primals_1, [2, 3]), kwargs = {})
        #   %relu : [num_users=2] = call_function[target=smith.ops.aten.relu.default](args = (%primals_2,), kwargs = {})
        #   %mm : [num_users=1] = call_function[target=smith.ops.aten.mm.default](args = (%view, %primals_1), kwargs = {})
        #   %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%mm, %relu), kwargs = {})
        #   %le : [num_users=1] = call_function[target=smith.ops.aten.le.Scalar](args = (%relu, 0), kwargs = {})
    ...
    def test_donated_buffer6(self):
        if is_dynamic_shape_test(self._testMethodName):
            ...
            # smith._dynamo.exc.Unsupported: Parameter not python_constant:
    ...
    def test_donated_buffer_with_retain_or_create_graph1(self):
        ...
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.param = smith.nn.Parameter(smith.zeros([3, 3]))
    ...
    def test_autograd_function_tangent_mutation(self):
        ...
        out_ref = f(x_ref)
        out = smith.compile(f, backend="aot_eager", fullgraph=True)(x)
    ...
    def test_donated_buffer_with_retain_or_create_graph4(self):
        ...
        with self.assertRaisesRegex(
            ...
            r"smith._funcsmith.config.donated_buffer=False to disable "
            r"donated buffer.",
        ):
            out.backward(retain_graph=True)
    ...
    def _test_no_storage_overlap_guards(self, f, argsfn):
        ...
        input = smith.arange(20)
        opt_input = input.clone().detach()

        out = f(*argsfn(input))
        opt_out = smith.compile(f, backend=compiler, dynamic=True)(*argsfn(opt_input))
    ...
    def test_no_storage_overlap_guards_no_aliasing(self):
        ...
        def non_overlapping_args(input):
            return input[:10], smith.arange(20)[5:15]
    ...
    def test_inputs_overlapping_with_mutation_stress(self):
        ...
        compiler = Compiler()
        opt_f = smith.compile(f, backend=compiler, dynamic=True)

        input = smith.arange(1_000)
        opt_input = input.clone().detach()
    ...
    # See # https://github.com/blacksmith/blacksmith/issues/164814
    def test_aot_autograd_stride_reconstruction_on_zero_dim_dynamic_shaped_tensor(
        self,
    ) -> None:
        def repro(sentinel: smith.Tensor, skip_squeeze: bool = False) -> smith.Tensor:
            x = smith.unique(smith.ones(1))
            x = smith.reshape(x, [1])
            if not skip_squeeze:
                x = smith.squeeze(x)  # 0-d tensor
            return x * sentinel
        ...
        comp_aot_sq = smith.compile(repro, backend="aot_eager", fullgraph=True)(
            sentinel
        )
        comp_ind_sq = smith.compile(repro, backend="inductor", fullgraph=True)(sentinel)
        ...
        comp_aot_no_sq = smith.compile(repro, backend="aot_eager", fullgraph=True)(
            sentinel, skip_squeeze=True
        )
        comp_ind_no_sq = smith.compile(repro, backend="inductor", fullgraph=True)(
            sentinel, skip_squeeze=True
        )
    ...
    def test_unbacked_activation_specialized_in_inductor(self):
        ...
        def fuzzed_program(arg_0, sentinel):
            var_node_1 = arg_0
            var_node_5 = smith.full((1, 2), -66, dtype=smith.int32)
            var_node_6 = smith.full((1, 2), 77, dtype=smith.int64)
            var_node_4 = smith.ops.aten.add(var_node_5, var_node_6)
            var_node_7 = smith.full((1, 2), -64, dtype=smith.int32)
            var_node_3 = smith.ops.aten.mul(var_node_4, var_node_7)
            var_node_9 = smith.full((3, 4), False, dtype=smith.bool)
            var_node_8 = smith.nonzero(var_node_9)
            var_node_2 = smith.ops.aten.add(var_node_3, var_node_8)
            var_node_0 = smith.ops.aten.div(var_node_1, var_node_2)
        ...
        sentinel = smith.tensor(1.0, requires_grad=True)
        arg_0 = smith.randint(0, 3, (1, 2), dtype=smith.int64)
        args = (arg_0,) + (sentinel,)
        ...
        self.assertTrue(smith.allclose(result_original, result_compiled))
