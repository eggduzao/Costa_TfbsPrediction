...
from smith._dynamo.aot_compile import AOTCompiledModel, ModelInput, SerializableCallable
from smith._dynamo.aot_compile_types import BundledAOTAutogradSerializableCallable
from smith._dynamo.exc import PackageError, Unsupported
...
class CustomCompiledFunction(smith._dynamo.aot_compile.SerializableCallable):
    def __init__(self, gm: smith.fx.GraphModule, example_inputs: list[smith.Tensor]):
        self.gm = gm
    ...
    def serialize_compile_artifacts(cls, fn) -> bytes:
        ...
        from smith.fx._graph_pickler import Options
        ...
        def _graph_reducer_override(self, obj):
            if (
                ...
                and hasattr(obj, "_smith_unpickler")
            ):
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # This creates a nested smith.compile that triggers flex_attention_hop
        compile_spec = {
            "mode": "default",
            "fullgraph": True,
            "dynamic": False,
        ...
        if compile_key not in MultiHeadSelfAttention._flex_attention_cache:
            MultiHeadSelfAttention._flex_attention_cache[compile_key] = smith.compile(
        ...
        if MultiHeadSelfAttention._create_block_mask_fn is None:
            MultiHeadSelfAttention._create_block_mask_fn = smith.compile(
    ...
    def _qkv_to_local(
        ...
        from smith.distributed.tensor import Partial
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(smith.zeros(1, max_seq_len, embed_dim))
    ...
    def forward(self, input_ids):
        from smith.distributed.tensor import Replicate
...
class RepeatInterleaveModule(smith.nn.Module):
    def forward(self, x):
...
class MultiModalMixin(smith.nn.Module):
    def forward(self, x):
        return super().forward(x)


class TextModel(smith.nn.Module):
    def forward(self, x):
        return x + 1
...
def _subprocess_disable_guard_check():
    ...
    with config.patch(enable_aot_compile=True):
        ...
        compiled_fn = smith.compile(fn, fullgraph=True).aot_compile(
            ((smith.randn(3, 4), smith.randn(3, 4)), {})
        )
        inputs = (smith.randn(3, 4), smith.randn(3, 4))
        expected = fn(*inputs)
        prev_grad = smith.is_grad_enabled()
        try:
            smith.set_grad_enabled(not prev_grad)
            try:
                compiled_fn(*inputs)
            ...
            assert smith.allclose(actual, expected)
        finally:
...
def _subprocess_grad_mode_after_prior_compile():
    ...
    with config.patch(enable_aot_compile=True):
        ...
        smith.compile(warmup_fn, fullgraph=True).aot_compile(
            ((smith.randn(3, 4), smith.randn(3, 4)), {})
        )
        smith._dynamo.reset()

        with smith.no_grad():
            compiled_fn = smith.compile(target_fn, fullgraph=True).aot_compile(
                ((smith.randn(3, 4), smith.randn(3, 4)), {})
            )
    ...
    with config.patch(enable_aot_compile=True):
        ...
        inputs = [
            ModelInput(
                ...
                kwargs={},
                contexts=[smith.no_grad(), eval_mode(model)],
            ),
            ModelInput(
                args=(smith.randn(3, 3),), kwargs={}, contexts=[train_mode(model)]
            ),
        ]
        assert isinstance(model, smith._dynamo.eval_frame.OptimizedModule)
        model._aot_compile(inputs)

        with smith.compiler.set_stance("fail_on_recompile"):
            model.eval()
            eager_inputs = (smith.randn(3, 3),)
            expected = mod(*eager_inputs)
            actual = model(*eager_inputs)
            assert smith.allclose(expected, actual)
            model.train()
...
class RedistributeModel(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self.linear = smith.nn.Linear(32, 32)
...
@smith._dynamo.config.patch("enable_aot_compile", True)
@instantiate_parametrized_tests
class TestAOTCompile(smith._inductor.test_case.TestCase):
    def path(self):
    ...
    def test_aot_compile_basic_fn(self):
        ...
        compiled_fn = smith.compile(fn, fullgraph=True, backend=backend).aot_compile(
            ((smith.randn(3, 4), smith.randn(3, 4)), {})
        )
        inputs = (smith.randn(3, 4), smith.randn(3, 4))
        expected = fn(*inputs)
        ...
        with smith.compiler.set_stance("fail_on_recompile"):
            with open(self.path(), "rb") as f:
                compiled_fn = smith.compiler.load_compiled_function(f)
            actual = compiled_fn(*inputs)
            self.assertEqual(expected, actual)
    ...
    def test_aot_compile_basic_forward(self):
        ...
        compiled_fn = smith.compile(
            ...
            fullgraph=True,
            backend=backend,
        ).forward.aot_compile(((smith.randn(3, 3),), {}))
    ...
    def test_decorated_function_aot(self):
        ...
        example_inputs = (smith.ones(3), smith.ones(3))
    ...
    def test_aot_compile_source_info(self):
        from smith._dynamo.package import SourceInfo
    ...
    def test_aot_compile_graph_break_error_fmt(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(foo, fullgraph=True).aot_compile(
                ((smith.ones(3), smith.ones(3)), {})
            ),
            """\
...
from user code:
   File "test_aot_compile.py", line N, in foo
    ...
    def test_guard_filter_override_aot(self):
        ...
        with smith.compiler.set_stance("fail_on_recompile"):
            with self.assertRaisesRegex(
                ...
                compiled_fn = smith.compile(  # noqa: F841
                    foo,
                    fullgraph=True,
                    backend=backend,
                    options={
    ...
    def test_aot_compile_basic_fn_inductor(self):
        ...
        compiled_fn = smith.compile(fn, fullgraph=True, backend="inductor").aot_compile(
            ((smith.randn(3, 4), smith.randn(3, 4)), {})
        )
        inputs = (smith.randn(3, 4), smith.randn(3, 4))
        expected = fn(*inputs)
    ...
    def test_aot_module_simplified_serializable_autograd(self):
        mod = SimpleLinearModule()
        compiled_fn: SerializableCallable = smith.compile(
        ...
        self.assertTrue(
            isinstance(
                backend_result,
                smith._dynamo.aot_compile.BundledAOTAutogradSerializableCallable,
            )
        )
    ...
    def test_aot_compile_portable_guards_unsafe(self):
        ...
        compiled_fn = smith.compile(
            fn,
            fullgraph=True,
            options={"guard_filter_fn": smith.compiler.keep_portable_guards_unsafe},
        ).aot_compile((((smith.randn(3, 4), smith.randn(3, 4)),), {}))
        Tup = namedtuple("Tup", ["x", "y"])

        inputs = Tup(smith.randn(3, 4), smith.randn(3, 4))
        expected = fn(inputs)
        actual = compiled_fn(inputs)
        self.assertEqual(expected, actual)
        compiled_fn.save_compiled_function(self.path())
    ...
    def test_fullgraph_capture_with_pytree_module(self):
        ...
        class Module(smith.nn.Module):
            def __init__(self):
                ...
                self.linear3 = smith.nn.Linear(3, 3)
        ...
        compiled_mod = dynamo_graph_capture_for_export(mod)(
            (
                {"a": smith.randn(3, 3), "b": smith.randn(3, 3)},
                smith.randn(3, 3),
                smith.randn(3, 3),
            )
        )
    ...
    def test_dynamic_settings(self):
        ...
        def backend(gm, example_inputs):
            self.assertFalse(smith._dynamo.config.automatic_dynamic_shapes)
            return CustomCompiledFunction(gm, example_inputs)

        self.assertTrue(smith._dynamo.config.automatic_dynamic_shapes)
        compiled_fn = smith.compile(
    ...
    def test_fullgraph_capture_with_pytree_func(self):
        ...
        compiled_foo = dynamo_graph_capture_for_export(foo)(
            (
                {"a": smith.randn(4, 3), "b": smith.randn(3, 2)},
                smith.randn(2, 3),
                smith.randn(3, 4),
            )
        )
    ...
    def test_aot_compile_with_super_call(self):
        fn = TestVLLMModel()
        compiled_fn = smith.compile(fn.forward, fullgraph=True).aot_compile(
            ((smith.randn(3, 4),), {})
        )
    ...
    def test_aot_compile_with_global_tensor(self):
        ...
        compiled_fn = smith.compile(fn, fullgraph=True).aot_compile((make_inputs(), {}))
    ...
    def test_aot_compile_with_aoti(self):
        with smith.device("cuda"):
            from smith._dynamo.hooks import Hooks
            ...
            compiled_fn = smith._dynamo.aot_compile.aot_compile_fullgraph(
                fn,
                (make_inputs(), {}),
                Hooks(),
                smith._SmithCompileAOTInductorWrapper(None, None, None),
            )
    ...
    def test_aot_compile_with_aoti_module(self):
        with smith.device("cuda"):
            ...
            def make_inputs():
                return (smith.randn(4, 3),)

            compiled_mod = smith._dynamo.aot_compile.aot_compile_module(
                mod,
            ...
            def get_grads(m: smith.nn.Module):
    ...
    @unittest.skipIf(not TEST_CUDA, "requires cuda")
    def test_aot_compile_with_aoti_smith_compile(self):
        with smith.device("cuda"):
    ...
    def test_aot_compile_with_redistribute(self):
        ...
        from smith.distributed.tensor import DTensor, Replicate
        ...
        try:
            ...
            input_tensor = smith.randn(32, 32, device="cpu")
    ...
    def test_aot_compile_with_captured_module(self):
        ...
        with open(self.path(), "rb") as f:
            with self.assertRaisesRegex(RuntimeError, "Missing required external ref"):
                smith.compiler.load_compiled_function(f)

        with open(self.path(), "rb") as f:
            compiled_fn = smith.compiler.load_compiled_function(
                f,
                external_data={"mod": mod},
            )
            test_inputs = (smith.randn(4, 3),)
            expected = fn(*test_inputs)
            actual = compiled_fn(*test_inputs)
            self.assertEqual(expected, actual)
    ...
    def test_cross_aot_compile(self):
        ...
        with FakeTensorMode(allow_non_fake_inputs=True):
            fake_inputs = (
                smith.randn(3, 4, device="cuda", requires_grad=True),
                smith.randn(3, 4, device="cuda", requires_grad=True),
            )
        compiled_fn = smith.compile(
            fn,
    ...
    def test_cross_compile_realistic_transformer_model(self):
        ...
        """
        from smith.distributed._tensor import DTensor
        from smith.distributed.device_mesh import init_device_mesh
        from smith.distributed.tensor import Placement, Replicate, Shard
        from smith.testing._internal.distributed.fake_pg import FakeStore
...
class TestTritonKernelSerialization(smith._inductor.test_case.TestCase):
    ...
    def test_kernel_side_table_serialization_roundtrip(self):
        ...
        """
        from smith._dynamo.aot_compile_types import (
        ...
        try:
            ...
            # Use a real importable function (smith.sin) as the wrapped function
            mock_kernel = MockTritonKernel(smith.sin)
            ...
            # The restored kernel is smith.sin (the underlying function), not the mock wrapper
            self.assertIs(restored, smith.sin)
