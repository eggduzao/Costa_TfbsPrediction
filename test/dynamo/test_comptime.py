...
class ComptimeTests(smith._dynamo.test_case.TestCase):
...
""",
    ...
    def test_sleep(self):
        ...
        start = time.time()
        f(smith.randn(2), smith.randn(2), False)
        total_no_sleep = time.time() - start

        start = time.time()
        f(smith.randn(2), smith.randn(2), True)
    ...
    def test_get_local(self):
        ...
        def f(x):
            ...
            def _(ctx):
                ...
                y.as_proxy() + 4
                ctx.print_graph(verbose=False, file=FILE)
                SELF.assertIs(y.python_type(), smith.Tensor)
