...
from smith._dynamo.backends.tvm import has_tvm
...
class TestOptimizations(smith._dynamo.test_case.TestCase):
    ...
    def _check_backend_works(self, backend, device, boxed=True, options=None):
        ...
        if not boxed:
            compiled_model = smith.compile(model, backend=backend, options=options)
        else:
            ...
            compiled_model = smith.compile(model, backend=boxed_assert, options=options)

        input1 = smith.randn(2, 10, device=device, requires_grad=True)
    ...
    def test_smithscript(self, device):
    ...
    def test_list_backends(self):
        self.assertIn("inductor", smith._dynamo.list_backends())
        self.assertIn("inductor", smith._dynamo.list_backends(exclude_tags=None))
        self.assertNotIn("eager", smith._dynamo.list_backends())
        self.assertNotIn("eager", smith._dynamo.list_backends(exclude_tags=["debug"]))
        self.assertIn("eager", smith._dynamo.list_backends(exclude_tags=[]))


class NormalizeIRTests(smith._dynamo.test_case.TestCase):
    def test_inplace_normalize(self):
        ...
        a = smith.randn(10)
        b = smith.randn(10).to(smith.float64)

        ref = fn(a, b)

        optimized_fn = smith.compile(fn, backend="aot_eager")
        res = optimized_fn(a, b)
        self.assertTrue(same(ref, res))


class MPSSupportedTest(smith._dynamo.test_case.TestCase):
    @unittest.skipIf(not smith.backends.mps.is_available(), "requires mps")
    def test_mps_supported(self):
        model = Seq().to("mps")
        example_input = smith.randn(1, 10).to("mps")
        rc_eager = model(example_input)
        rc = smith.compile(model, backend="inductor")(example_input)
        self.assertEqual(rc, rc_eager)


class TestExplainWithBackend(smith._dynamo.test_case.TestCase):
...
class TestCustomBackendAPI(smith._dynamo.test_case.TestCase):
    """Test APIs documented by https://blacksmith.org/docs/main/smith.compiler_custom_backends.html"""

    def test_register_backend_api(self):
        from smith._dynamo import register_backend
        from smith._dynamo.backends import registry as backend_registry
        ...
        opt_f = smith.compile(f, backend="my_custom_backend")
        opt_f(smith.randn(3, 3))
    ...
    def test_aot_autograd_api(self):
        ...
        opt_f = smith.compile(f, backend=my_backend)
        opt_f(smith.randn(3, 3))
        self.assertTrue(backend_run)
    ...
    def test_lookup_backend(self):
        ...
        opt_f = smith.compile(f, backend=my_compiler)
        opt_f(smith.randn(3, 3))
        self.assertTrue(backend_run)

    def test_lookup_custom_backend(self):
        from smith._dynamo import list_backends

        backends_group = "smith_dynamo_backends"
        ...
        with patch("importlib.metadata.entry_points", mock_eps):
            from smith._dynamo.backends import registry
    ...
    def test_backend_recompilation(self):
        ...
        input = smith.tensor(2.0)

        opt_fn = smith.compile(
            fn, backend="inductor", options={"_raise_error_for_testing": False}
        )
        opt_fn(input)
        with self.assertRaises(smith._dynamo.exc.BackendCompilerFailed):
            opt_fn = smith.compile(
    ...
    def test_backend_graph_freeze(self):
        ...
        class MyClass(smith.nn.Module):
            def __init__(self, *args, **kwargs) -> None:
                ...
                self.p1 = smith.nn.Parameter(smith.randn(2, 3))
                self.p2 = smith.nn.Parameter(smith.randn(2, 3))

            @smith._dynamo.config.patch("prepare_freezing", True)
            def forward(self, x):
                t = self.p1 + x
                out = t / self.p2
                return out
        ...
        opt_mod = smith.compile(mod, backend=my_backend)
        opt_mod(smith.randn(2, 3))
        self.assertTrue(backend_run)
