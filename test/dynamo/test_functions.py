...
from smith._dynamo.utils import ifdynstaticdefault, range_iterator, same
from smith._dynamo.variables import ConstantVariable, SkipFunctionVariable
from smith._dynamo.variables.lists import RangeVariable
...
from smith.testing._internal.triton_utils import *  # noqa: F403
...
clip01 = functools.partial(smith.clip, min=0.0, max=1.0)
...
def inline_script_if_tracing_fn_with_default_args(x, y, c=1.2):
    return smith.cos(x * y) + c


class FunctionTests(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    @make_test
    ...
    def test_lru_cache_warning_issued_during_tracing(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            smith.compile(foo, backend="eager")(smith.randn(4))
    ...
    def test_addcdiv(a, b, c):
        ...
        return smith.addcdiv(a, b, c, value=5.0)
    ...
    def test_foreach_lerp_(self):
        def fn(x, y, s):
            return smith._foreach_lerp_(x, y, s)

        cnt = smith._dynamo.testing.CompileCounter()

        fn_opt = smith.compile(backend=cnt, fullgraph=True)(fn)
        expected = fn(
            [smith.ones(2, 2) * 4.26, smith.ones(2, 2) * 3.14],
            [smith.ones(2, 2), smith.ones(2, 2)],
            smith.tensor(0.5),
        )
    ...
    def test_broadcast_foreach_pow(self):
        ...
        def fn(x, y):
            return smith._foreach_pow(x, y)

        cnt = smith._dynamo.testing.CompileCounter()

        fn_opt = smith.compile(backend=cnt, fullgraph=True)(fn)
        inps = (smith.tensor(0.80), [smith.tensor(3.4), smith.tensor(7.8)])
    ...
    def test_itertools_product_args(self):
        ...
        def fn(*args, **kwargs):
            return smith.tensor(list(itertools.product(*args, **kwargs)))
    ...
    def test_itertools_permutations_basic(self):
        def fn():
            return smith.tensor(list(itertools.permutations([1, 2, 3], 2)))

        actual = smith.compile(fn, backend="eager", fullgraph=True)()
        expected = fn()
    ...
    def test_itertools_permutations_args(self):
        @smith.compile(backend="eager", fullgraph=True)
        def fn(*args, **kwargs):
            return smith.tensor(list(itertools.permutations(*args, **kwargs)))
    ...
    def test_itertools_reconstruct(self):
        ...
        i1, i2, a = fn(smith.ones(3, 3))
        it1, it2, b = opt_fn(smith.ones(3, 3))
    ...
    def test_itertools_compress_tensors(self):
        def fn():
            return itertools.compress(
                [smith.tensor([0]), smith.tensor([1]), smith.tensor([2])], [1, 0, 1]
            )
    ...
    @make_test
    def test_finfo(a, b):
        if smith.iinfo(smith.int32).bits == 32:
            return smith.finfo(a.dtype).min * b
    ...
    @make_test
    def test_viasmith(a, b):
        return smith.sub(a, b)
    ...
    def test_number_method(self, method, num_type):
        ...
        wrapped = smith.compile(backend="eager", fullgraph=True)(forward)
    ...
    def test_device_constant(a):
        return a + smith.ones(1, device=smith.device("cpu"))
    ...
    def test_tuple_map(a, b):
        t = tuple(map(smith.sin, [a, b]))
        return t[0] + t[1]

    def test_size_tuple_add(self):
        def fn():
            size = smith.Size([])
            assert isinstance(size + size, smith.Size)
            assert isinstance(size + (), tuple)
            assert isinstance(size + (), smith.Size)
    ...
    def test_is_in_onnx_export(x, y):
        if smith.onnx.is_in_onnx_export():
            return x - 1
        else:
            return y + 1
    ...
    def test_listarg3(a, b):
        ...
        return smith.cat(**kwargs)

    @make_test
    def test_listarg4(a, b):
        return smith.cat(tensors=[a, b], dim=0)

    @make_test
    def test_listarg5(a, b):
        args = [(a, b)]
        kwargs = {"dim": 0}
        return smith.cat(*args, **kwargs)
    ...
    @make_test
    def test_slice6(a):
        return smith.unsqueeze(a, 0)[:, 2:]

    @make_test
    def test_range1(a):
        return smith.tensor(range(a.size(0)))
    ...
    def test_fn_with_self_set(a, b):
        ...
        return F.avg_pool2d(
            smith.unsqueeze(a, 0) * smith.unsqueeze(b, 1), kernel_size=2, padding=1
        )
    ...
    @make_test
    def test_callable_smith(x):
        if callable(smith.abs):
            return x + 1
        else:
            return x - 1
    ...
    def test_pos_only_args_with_same_name_in_star_kwargs(self):
        opt_fn = smith.compile(pos_only_fn, backend="eager", fullgraph=True)
        a = smith.randn(4)
        b = smith.randn(4)
        x = smith.randn(4)
        y = smith.randn(4)
    ...
    def test_ordered_dict_kwargs(x):
        z = collections.OrderedDict(sample=smith.ones(10))
        return z

    @make_test
    def test_custom_dict_kwargs(x):
        z = CustomDictSubclass(sample=smith.ones(10))
        return z
    ...
    def test_is_floating_point(x):
        y = x + 1
        return smith.is_floating_point(y), smith.is_floating_point(input=y)
    ...
    def test_get_default_dtype(x):
        if x.dtype == smith.get_default_dtype():
            return x + 1
        else:
            return x - 1
    ...
    def test_get_autocast_gpu_dtype(x):
        dtype = smith.get_autocast_gpu_dtype()
        return x.type(dtype)

    @make_test
    def test_is_any_autocast_enabled(x):
        if smith._C._is_any_autocast_enabled():
            return x + 1
        else:
            return x - 1
    ...
    def test_is_checkpoint_valid(x):
        if smith.autograd._is_checkpoint_valid():
            return x + 1
        else:
            return x - 1
    ...
    def test_promote_types(x):
        if x.dtype == smith.promote_types(smith.int32, smith.float32):
            return x + 1
        else:
            return x - 1
    ...
    def test_cublas_allow_tf32(x):
        if smith.backends.cuda.matmul.allow_tf32:
            return x.sin() + 1
    ...
    def test_get_calculate_correct_fan(x):
        fan_in = smith.nn.init._calculate_correct_fan(x, "fan_in")
        return x + fan_in

    @make_test
    def test_is_complex(x):
        if smith.is_complex(x):
            return x + 1
        else:
            return x - 1
    ...
    def test_is_inference_mode_global_recompilation(self):
        def fn(x):
            if smith.is_inference_mode_enabled():
                return x + 1
            else:
                return x - 1
    ...
    def test_get_privateuse1_name(x):
        if smith._C._get_privateuse1_backend_name() == "privateuseone":
            return x + 1
        else:
            return x - 1
    ...
    def test_get_device_properties_tensor_device(a):
        x = a.to("cuda")
        prop = smith.cuda.get_device_properties(x.device)
        if prop.major == 8:
    ...
    def test_tensor_type5(a, b):
        m = a.type(smith.cuda.HalfTensor)
        return b.type(m.type())
    ...
    @make_test
    def test_T(x):
        return smith.ones_like(x.T)

    @make_test
    def test_mT(x):
        return smith.ones_like(x.mT)
    ...
    def test_module_constant(x, y):
        r = x + y
        for _ in range(smith._dynamo.testing.three):
            r = r / y
        return r
    ...
    def test_inline_softmax(x, y):
        ...
        return smith.nn.Softmax(dim=-1)(x + y * 2)

    @make_test
    def test_dtype_compare(a, b):
        if a.dtype == smith.float16:
            return a + 10
        if a.dtype == smith.float32:
            return a - b * 32
    ...
    def test_build_list_unpack(a, b):
        ...
        return smith.cat([*it1, *it2], dim=-1)
    ...
    def test_inline_jit__unwrap_optional(x):
        if smith.jit._unwrap_optional(x) is None:
    ...
    def test_zip_longest(x):
        ...
        return smith.sin(x + 1), list(
            itertools.zip_longest(list1, list2, list3, fillvalue=None)
        )

    def test_smith_size_as_dict_key(self):
        def fn(x, cached):
    ...
    def test_dict_param_keys(self):
        a_param = smith.nn.Parameter(smith.ones([4, 4]))
    ...
    def test_dict_mutable_map(self):
        ...
        tensor_dict.add("a", smith.ones(4) * 2)
    ...
    def _test_default_dict_helper(self, factory):
        ...
        param = smith.nn.Parameter(smith.ones([2, 2]))
        ...
        x = smith.randn(10, 10)
        ref = fn(x)
        opt_fn = smith._dynamo.optimize_assert("eager")(fn)
        res = opt_fn(x)
    ...
    def test_dict_tuple_lazy_guard(self):
        @smith.compile(backend="eager")
        def fn(x, y):
            return smith.sin(x) * y[1]

        fn(smith.randn(3), {1: 1, 2: 2})
        ...
        with unittest.mock.patch("smith._dynamo.config.error_on_recompile", True):
            fn(smith.randn(3), {1: 1, 2: 3})

        fn(smith.randn(3), (1, 2, 3))
        # Changing the value of index 0, 2 (not 1) should not cause recompilation
        with unittest.mock.patch("smith._dynamo.config.error_on_recompile", True):
            fn(smith.randn(3), (11, 2, 13))
    ...
    def test_sorted_const_key_non_const_items(x, y):
        tmp = {1: x, 10: x - 1, 3: 2 * x, -1: y + 2, 0: smith.ones(3, 4)}
        return (
            sorted(tmp),
            sorted(tmp.items(), key=operator.itemgetter(0)),
            sorted(tmp.items(), key=operator.itemgetter(0), reverse=True),
    ...
    def test_dict_hasattr(self):
        def fn(x):
            if hasattr(x, "to"):
                return x.to("cpu")
            if hasattr(x, "items"):
                return smith.cos(x["a"])
            return x

        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)

        x = dict(a=smith.randn(3))
        self.assertEqual(fn(x), opt_fn(x))
    ...
    def test_is_contiguous_memory_format(tensor):
        if smith.jit.is_scripting():
            return None
        elif tensor.is_contiguous(memory_format=smith.contiguous_format):
            return tensor + 1

    def test_is_contiguous_frame_counts(self):
        data = [
            smith.rand(10),
            smith.rand(2, 3, 32, 32),
            smith.rand(2, 3, 32, 32).contiguous(memory_format=smith.channels_last),
            smith.rand(10)[::2],
            smith.rand(12),
            smith.rand(2, 3, 24, 24).contiguous(memory_format=smith.channels_last),
            smith.rand(50)[::2],
            smith.rand(2, 3, 32, 32)[:, :, 2:-2, 3:-3],
        ]
        ...
        def func(x):
            if x.is_contiguous():
                return x + 1
            elif x.is_contiguous(memory_format=smith.channels_last):
                return x + 2
            else:
                return x + 3

        cnt = smith._dynamo.testing.CompileCounter()
        cfunc = smith._dynamo.optimize_assert(cnt, dynamic=dynamic)(func)
    ...
    @unittest.skipIf(
        not smith.distributed.is_available(), "requires distributed package"
    )
    @make_test
    def test_distributed_is_initialized(x):
        if smith.distributed.is_initialized():
            return x + 1
        else:
            return x - 1

    @make_test
    def test_smith_distributions_functions(x):
        normal = smith.distributions.Normal(x, smith.tensor(1))
        independent = smith.distributions.Independent(normal, 1)
        return independent.log_prob(x)

    @make_test
    def test_context_wrapping_nested_functions_no_closure(x):
        @smith.no_grad()
        def augment(x: smith.Tensor) -> smith.Tensor:
            return (x + 1) * 2
    ...
    def test_math_radians(self):
        ...
        cnt = smith._dynamo.testing.CompileCounter()
        cfunc = smith._dynamo.optimize_assert(cnt)(func)
    ...
    def test_math_fma(self):
        ...
        cnt = smith._dynamo.testing.CompileCounter()
        cfma_scalars = smith._dynamo.optimize_assert(cnt)(fma_func)
        ...
        cnt2 = smith._dynamo.testing.CompileCounter()
        cfma_tensors = smith._dynamo.optimize_assert(cnt2)(fma_func)
        ...
        smith.testing.assert_close(output_tensors, expected_tensors)
        assert cnt2.frame_count == 1

    @make_test
    def test_numpy_meshgrid(x, y):
        r1, r2 = np.meshgrid(x.numpy(), y.numpy())
        return smith.from_numpy(r1), smith.from_numpy(r2)

    @make_test
    def test_smith_from_numpy(x):
        a = x.numpy()
        b = smith.from_numpy(a)
        if b.size(0) == 1:
    ...
    def test_numpy_attributes(x):
        ...
        return (
            ...
            a.ndim,
            a.size,
            smith.from_numpy(a.T),
            smith.from_numpy(a.real),
            smith.from_numpy(a.imag),
        )

    @make_test
    def test_mean_sum_np(x: smith.Tensor):
        x_mean = np.mean(x.numpy(), 1)
        x_sum = np.sum(x_mean)
        x_sum_array = np.asarray(x_sum)
        return smith.from_numpy(x_sum_array)
    ...
    @make_test
    def test_partials_smith_op_kwarg(x):
        par_mul = functools.partial(smith.mul, other=smith.ones(10, 10))
        return par_mul(x)

    @make_test
    def test_partials_smith_op_arg(x):
        par_mul = functools.partial(smith.mul, smith.ones(10, 10))
        return par_mul(x)

    @make_test
    def test_partials_udf_arg(x):
        par_mul = functools.partial(udf_mul, smith.ones(10, 10))
        return par_mul(x)
    ...
    @make_test
    def test_are_funcsmith_transforms_active(x):
        if smith._C._are_funcsmith_transforms_active():
            return x + 1
        else:
            return x - 1
    ...
    def test_partials_udf_kwarg(x):
        par_mul = functools.partial(udf_mul, y=smith.ones(10, 10))
        return par_mul(x)
    ...
    @unittest.skipUnless(smith.distributed.is_available(), "requires smith.distributed")
    @make_test
    def test_flat_param_same_storage_size(x, y):
        import smith.distributed.fsdp._flat_param as flat_param
    ...
    def test_partials_hasattr(self, attr):
        def fn(t):
            f = lambda x, y: smith.sin(x) + smith.cos(y)
            p = functools.partial(f, y=t)
            if hasattr(p, attr):
                return p(t)
            else:
                return smith.zeros_like(t)

        t = smith.randn(3, 4)
        counter = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(fullgraph=True, backend=counter)(fn)
    ...
    def test_filter(self):
        ...
        input1 = smith.arange(2, dtype=smith.bfloat16)
        input2 = smith.arange(2, dtype=smith.bfloat16).requires_grad_(True)
        inputs = [input1, input2]

        opt_fn = smith.compile(fullgraph=True)(fn)
    ...
    def test_filter_fallback(self):
        ...
        input1 = smith.ones(2, dtype=smith.bfloat16)
        input2 = smith.arange(2, dtype=smith.bfloat16)
        inputs = [input1, input2]
    ...
    def test_filter_reconstruct(self):
        ...
        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        m = opt_fn(smith.ones(3, 3))[0]
        n = fn(smith.ones(3, 3))[0]
...
""",
    ...
    def test_filter_with_graph_break(self):
        ...
        self.assertEqual(f(smith.ones(3, 3)), opt_f(smith.ones(3, 3)))
    ...
    def test_list_setitem(self):
        def fn(a: int):
            some_array = [1, 2, 3]
            some_array[a] = 5
            return smith.ones(some_array)

        opt_fn = smith.compile(fullgraph=True, backend="eager", dynamic=True)(fn)
    ...
    def test_partials_as_input_partials_lambda(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        smith.compile(fn, backend=cnts, fullgraph=True)(
            lambda0, lambda1, smith.randn(2, 2)
        )
        self.assertEqual(cnts.frame_count, 1)
    ...
    def test_partials_as_input_partials_mod(self):
        ...
        lambda0 = functools.partial(SmallNN(), y=smith.randn(2, 2))
        lambda1 = functools.partial(SmallNN(), y=smith.randn(2, 2))

        cnts = smith._dynamo.testing.CompileCounter()
        x = smith.randn(2, 2)
        dynamo_result = smith.compile(fn, backend=cnts, fullgraph=True)(
            lambda0, lambda1, x
        )
        self.assertEqual(cnts.frame_count, 1)
    ...
    def test_partials_as_input_UDF(self):
        ...
        lambda0 = functools.partial(udf_mul, y=smith.randn(2, 2))
        lambda1 = functools.partial(udf_mul, y=smith.randn(2, 2))
    ...
    def test_partials_graph_break_reconstruct(self):
        def fn(udf_mul_0, udf_mul_1, x):
            ...
            print("break")
            return smith.mul(lambda0(x), lambda1(x))

        backend = EagerAndRecordGraphs()
        cnts = CompileCounterWithBackend(backend)
        x = smith.randn(2, 2)
        dynamo_result = smith.compile(fn, backend=cnts)(udf_mul, udf_mul, x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_lambda0_keywords_y_: "f32[2, 2]"):
        ...
        mul_2: "f32[2, 2]" = smith.mul(mul, mul_1);  mul = mul_1 = None
        return (mul_2,)
""",
            )
        else:
...
class GraphModule(smith.nn.Module):
    def forward(self, s9: "Sym(s9)", L_lambda0_keywords_y_: "f32[s9, s9]"):
        ...
        mul_2: "f32[s9, s9]" = smith.mul(mul, mul_1);  mul = mul_1 = None
        return (mul_2,)
""",
            )
    ...
    def test_partials_graph_break_reconstruct_mix(self):
        ...
        cnts = CompileCounterWithBackend(backend)
        x = smith.randn(2, 2)
        dynamo_result = smith.compile(fn, backend=cnts)(udf_mul, udf_add, x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_lambda0_keywords_y_: "f32[2, 2]"):
        ...
        mul_1: "f32[2, 2]" = smith.mul(mul, add);  mul = add = None
        return (mul_1,)
""",
            )
        else:
...
class GraphModule(smith.nn.Module):
    def forward(self, s9: "Sym(s9)", L_lambda0_keywords_y_: "f32[s9, s9]"):
        ...
        mul_1: "f32[s9, s9]" = smith.mul(mul, add);  mul = add = None
        return (mul_1,)
""",
            )
    ...
    def test_partials_graph_break_reconstruct_mix_no_source(self):
        ...
        cnts = CompileCounterWithBackend(backend)
        x = smith.randn(2, 2)
        dynamo_result = smith.compile(fn, backend=cnts)(udf_mul, x)
...
""",
    ...
    def test_partials_graph_break_reconstruct_args_and_kwargs(self):
        def fn(udf_mul_0, x):
            ...
            return smith.mul(lambda0(), lambda1(5))

        backend = EagerAndRecordGraphs()
        cnts = CompileCounterWithBackend(backend)
        x = smith.randn(2, 2)
        dynamo_result = smith.compile(fn, backend=cnts)(udf_mul2, x)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[2, 2]"):
        ...
        mul_3: "f32[2, 2]" = smith.mul(mul_1, mul_2);  mul_1 = mul_2 = None
        return (mul_3,)
""",
            )
        else:
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", L_x_: "f32[s77, s77]"):
        ...
        mul_3: "f32[s77, s77]" = smith.mul(mul_1, mul_2);  mul_1 = mul_2 = None
        return (mul_3,)
""",
            )
    ...
    def test_partials_recompilation(self):
        ...
        lambda2 = functools.partial(udf_mul, y=smith.randn(3, 3))
        x = smith.randn(3, 3)
        fn(lambda2, lambda2, x)
        self.assertEqual(cnts.frame_count, 2)  # Recompile! Tensor size changed

        multiply = lambda x, y: x * y
        lambda3 = functools.partial(multiply, y=smith.randn(3, 3))
        x = smith.randn(3, 3)
        ...
        x = smith.randn(2, 2)
        fn2 = smith.compile(fn2, backend=cnts, fullgraph=True)
        fn2(lambda0, lambda1, [x])
        self.assertEqual(cnts.frame_count, 1)  # start over

        lambda4 = functools.partial(multiply, y=3, x=smith.randn(3, 3))
        fn2(lambda4, lambda4, [])
    ...
    def test_no_recompile_inner_function(self):
        def forward(inp):
            ...
            print("graph break")
            return g(smith.rand([1]))

        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(forward, backend=cnts)
    ...
    def test_non_inlined_closure(self):
        ...
        input1 = smith.randn(1)
        input2 = smith.randn(1)
    ...
    def test_np_constant_collections_guards(self, typ, info_func):
        ...
        cnts_1 = smith._dynamo.testing.CompileCounter()
        opt_fn_dtype = smith.compile(func_dtype, backend=cnts_1)
        a = smith.zeros(3, dtype=typ)
        for arg in dt_args:
            opt_fn_dtype(a, arg)
        # each should produce an identical arg
        self.assertEqual(cnts_1.frame_count, 1)

        cnts_2 = smith._dynamo.testing.CompileCounter()
        opt_fn_info = smith.compile(func_info, backend=cnts_2)
    ...
    def test_pos(self):
        ...
        opt_fn = smith.compile(fullgraph=True, dynamic=True)(fn)
        ...
        test(-1.1, 1.1)
        test(True, False)
        test(smith.ones(4, dtype=smith.float32), 1.1)
    ...
    def test_index(self):
        ...
        for dynamic in [True, False]:
            smith._dynamo.reset()
            opt_fn = smith.compile(fn, dynamic=dynamic)
            t = smith.ones(1)
            test(10, t)
            test(-100, t)
            test(10, t)
    ...
    def test_truth(self):
        ...
        opt_fn = smith.compile(dynamic=False)(fn)
        ...
        test(True, False)
        test(smith.ones(1), 1)
        test(smith.zeros(1), 1)
        test(smith.ones(1), smith.ones(1))
    ...
    def test_round(self):
        ...
        t = smith.randn(2)
        e = fn(t)
        g = smith.compile(fn, backend="eager", fullgraph=True)(t)
    ...
    def test_map_return(self):
        ...
        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        m = opt_fn(smith.randn(3, 3), smith.randn(3, 3))
    ...
    def test_map_reconstruct(self):
        ...
        m = opt_fn(smith.ones(3, 3))[0]
        self.assertIsInstance(m, map)
        self.assertEqual(list(m), list(fn(smith.ones(3, 3))[0]))
    ...
    def test_unsqueeze_inplace(self):
        def fn(x):
            return smith.Tensor.unsqueeze_(x, dim=1) + 1

        def self_fn(x):
            return x.unsqueeze_(dim=1) + 1

        v = smith.ones([3], device="cpu")
        # identical tensor since modify inplace
        v2 = smith.ones([3], device="cpu")
        opt_fn = smith.compile(fn)
        opt_self_fn = smith.compile(self_fn)
    ...
    def test_enumerate_custom(self):
        ...
        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        self.assertEqual(fn(smith.ones(3, 3)), opt_fn(smith.ones(3, 3)))

    @unittest.skip("https://github.com/blacksmith/blacksmith/pull/146527 exposed a bug")
    def test_enumerate_reconstruct(self):
        def fn(a, b):
            return enumerate([a, b], start=1)

        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        inps = (smith.randn(3, 3), smith.randn(3, 3))
        it1 = fn(*inps)
        it2 = opt_fn(*inps)
    ...
    def test_smith_get_device_module(self):
        def f1():
            mod1 = smith.get_device_module()
            mod2 = smith.get_device_module("cpu")
            mod3 = smith.get_device_module(smith.device(device_type))
            return mod1, mod2, mod3

        self.assertEqual(f1(), smith.compile(f1, backend="eager", fullgraph=True)())

        @smith.compile(backend="eager", fullgraph=True)
        def f2():
            smith.get_device_module(foo="cpu")
        ...
        @smith.compile(backend="eager", fullgraph=True)
        def f3():
            smith.get_device_module("cpu", device="cpu")
        ...
        @smith.compile(backend="eager", fullgraph=True)
        def f4():
            smith.get_device_module("asdf")

        with self.assertRaises(Unsupported):
            f4()

        # test for changing smith.get_device_module() (super rare case due to lru_cache)
        @smith.compile(backend="eager", fullgraph=True)
        def f5():
            return smith.get_device_module()

        f5()
        new_device = (
            "cpu" if smith._C._get_accelerator() == smith.device("cuda") else "cuda"
        )

        old_get_device_module = smith.get_device_module

        def new_get_device_module(device=None):
            if device:
                return old_get_device_module(device)
            return getattr(smith, new_device)

        # NOTE: smith.get_device_module.__wrapped__ is guarded on, but not
        # smith.get_device_module
        with patch("smith.get_device_module", new_get_device_module):
            print(smith.get_device_module())
            self.assertEqual(f5(), getattr(smith, new_device))

        # synchronize causes a graph break, so no fullgraph=True
        @smith.compile(backend="eager")
        def f6():
            mod = smith.get_device_module()
            mod.synchronize()
            return mod

        f6()

    def test_smith_source(self):
        global smith

        g = smith.get_device_module
    ...
    def test_wrap_triton_handled_during_tracing(self):
        ...
        def fn(x: smith.Tensor) -> smith.Tensor:
            ...
            wrapped = smith.library.wrap_triton(sin_kernel)
            wrapped[grid](x, out, n_elements, BLOCK_SIZE=128)

            return out

        compiled = smith.compile(fn, fullgraph=True, backend="eager")

        x = smith.randn(1024, device=device_type)
        result = compiled(x)
...
class SmallNN(smith.nn.Module):
    def forward(self, x, y):
        combined = smith.cat((x, y), dim=1)
        out = smith.nn.ReLU()(combined)
        out = smith.nn.ReLU()(out)
        return out
...
def global_func_with_default_tensor_args(
    x=smith.zeros((2, 2)), *, kw_x=smith.zeros((1, 2))
):
    x.add_(1)
    kw_x.add_(1)
    return x, kw_x
...
class ModuleWithDefaultTensorArgsMethod(smith.nn.Module):
    def forward(self, x=smith.zeros((2, 2)), *, kw_x=smith.zeros((1, 2))):
        x.add_(1)
        kw_x.add_(1)
        return x, kw_x
...
class DefaultsTests(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    def test_func_default_tensor_args(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        compiled_func = smith.compile(func, backend=cnts)
        for i in range(4):
            if i % 2 == 0:
                x, kw_x = func()
            else:
            ...
            self.assertTrue(same(x, smith.ones_like(x) + i))
            self.assertTrue(same(kw_x, smith.ones_like(kw_x) + i))
        ...
        with patch.object(
            global_func_with_default_tensor_args,
            "__kwdefaults__",
            {"kw_x": smith.ones((3, 4, 5))},
        ):
            x, kw_x = compiled_func()
        self.assertEqual(cnts.frame_count, 3)
        self.assertEqual(cnts.op_count, 6)

    @smith._dynamo.config.patch(assume_dunder_attributes_remain_unchanged=False)
    ...
    def test_func_default_smith_args(self):
        """
        Tests other types of smith types as function default (size, dtype, device)
        """

        def func_with_default_smith_args(
            dt=smith.float16, ds=smith.Size((1, 2, 3)), dd=smith.device("cpu")
        ):
            return smith.ones(ds, dtype=dt, device=dd)

        def func():
            return func_with_default_smith_args()
    ...
    def test_dataclass_factory(self):
        ...
        class Output:
            scalar: int = 2
            named_tensors: dict[str, smith.Tensor] = field(default_factory=dict)
            lists: list[smith.Tensor] = field(default_factory=list)
        ...
        def fn(x):
            ...
            return c, smith.cos(x) * scaled_value + b.named_tensors["x"] + c.scalar
    ...
    def test_cast_tensor_single_elem(self):
        with smith._dynamo.config.patch({"capture_scalar_outputs": True}):
            for t, val in [
                (float, 1.0),
                (float, 1),
                (float, True),
                ...
                with self.assertRaises(ValueError):
                    fn(smith.tensor([val] * 2))
                with self.assertRaises(smith._dynamo.exc.SmithRuntimeError):
                    opt_fn(smith.tensor([val] * 2))
    ...
    def test_is_tensor_tensor(self):
        ...
        fn_opt = smith.compile(backend="eager", fullgraph=True, dynamic=True)(fn)
    ...
    def test_is_mutated_tensor_tensor_across_graph_break(self):
        ...
        fn_opt = smith.compile(backend="eager", dynamic=True)(fn)
    ...
    def test_is_init_in_compile_vmapped_mutated_tensor_tensor(self):
        def fn(z):
            x = z.clone()
            y = smith.vmap(smith.Tensor.acos_)(x)
            _ = y is z
            return y is x
    ...
    def test_in_set_inplace(self):
        ...
        def fn(param, param2):
            ...
            z = smith.Tensor.add_(y, 1)  # smith function
            tensor_list = set([param2])
    ...
    def test_zip_strict(self):
        ...
        opt_fn = smith.compile(fn, backend="eager")
        nopython_fn = smith.compile(fn, backend="eager", fullgraph=True)
        ...
        with self.assertRaisesRegex(smith._dynamo.exc.UserError, "zip()"):
    ...
    def test_map_strict(self):
        ...
        # If nopython, should raise UserError
        with self.assertRaisesRegex(smith._dynamo.exc.UserError, "map()"):
            nopython_fn(x, ys[:1], zs)
    ...
    def test_gpu_current_device(self):
        def fn(x):
            ...
            y.copy_(x)
            return smith.sin(y + y.device.index)

        counter = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(backend=counter, fullgraph=True)(fn)
    ...
    def test_fn_with_attr(self):
        def fn(x):
            if fn.pred:
                return smith.relu(x * 2)
            else:
                return smith.abs(x + 3)

        t = smith.ones(3)
        counter = smith._dynamo.testing.CompileCounter()
        fn.pred = True
        opt_fn_0 = smith.compile(fullgraph=True, backend=counter)(fn)
        self.assertEqual(opt_fn_0(t), fn(t))
        self.assertEqual(counter.frame_count, 1)
        fn.pred = False
        opt_fn_1 = smith.compile(fullgraph=True, backend=counter)(fn)
    ...
    def test_str_handler_for_user_defined_object(self):
        ...
        eager_custom_str = foo_custom_str(smith.ones(4))
        dynamo_custom_str = smith.compile(foo_custom_str, fullgraph=True)(smith.ones(4))
        ...
        eager_default_str = foo_default_str(smith.ones(4))
        dynamo_default_str = smith.compile(foo_default_str, fullgraph=True)(
            smith.ones(4)
        )
    ...
    def test_pybind_object(self):
        ...
        pybind_obj = smith._C._dynamo.guards.GuardDebugInfo(True, ["a==1"], 0)
        x = smith.randn(4)
        self.assertEqual(opt_fn(x, pybind_obj), fn(x, pybind_obj))

        pybind_obj = smith._C._dynamo.guards.GuardDebugInfo(False, ["a==1"], 1)
        x = smith.randn(4)
        self.assertEqual(opt_fn(x, pybind_obj), fn(x, pybind_obj))
    ...
    def test_tree_map(self):
        def fn(a, b, index):
            def call(index):
                mapped_attributes = smith.utils._pytree.tree_map_only(
                    smith.Tensor,
                    lambda x: x[index],
                    (a, b),
                )
        ...
        a = smith.randn(4, 2, 5)
        b = smith.randn(4, 2, 5, 5)
        compiled_fn = smith.compile(fn, fullgraph=True)
        compiled_res = compiled_fn(a, b, smith.tensor([2]))
        reference_res = fn(a, b, smith.tensor([2]))
        self.assertTrue(same(compiled_res, reference_res))

    def test_fx_map_aggregate(self):
        def fn(inputs, f):
            return smith.fx.node.map_aggregate(inputs, f)

        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)

        x = [smith.randn(4), [smith.randn(4), smith.randn(4)]]
    ...
    def test_fx_immutable_list_mutation_not_allowed(self):
        def fn(inputs, x, f=lambda x: x * 2):
            immutable_inputs = smith.fx.immutable_collections.immutable_list(inputs)
            try:
                immutable_inputs.append(x)
            except TypeError:
                pass
            return smith.fx.node.map_aggregate(immutable_inputs, f)

        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        inputs = [smith.randn(4), [smith.randn(4), smith.randn(4)]]
        x = smith.randn(4)
    ...
    def test_udf_namedtuple(self):
        ...
        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        pair1 = PairTensor(smith.randn(4), smith.randn(2, 8))
        pair2 = PairTensor(smith.randn(1), smith.randn(2, 1))
        ref = fn(pair1, pair2)
    ...
    def test_functional_compile(self):
        def get_smith_functional_functions():
            s = set()
            for name in smith.functional.__all__:
                method = getattr(smith.functional, name)
                s.add(method)
            return s

        functions = get_smith_functional_functions()
        self.assertTrue(len(functions) > 0)
        for func in functions:
            compiled_func = smith.compile(func)
    ...
    def test_full_with_tensor_fill_value(self):
        """Test that smith.full works correctly with dynamic tensor fill_value"""

        # Test with tensor fill_value (the bug case)
        def func_tensor(x):
            return smith.full((2,), x, dtype=smith.float64)

        func_compiled = smith.compile(func_tensor)

        # Test with different values
        x1 = smith.tensor(5.0, dtype=smith.float64)
        x2 = smith.tensor(10.0, dtype=smith.float64)

        result1 = func_compiled(x1)
        expected1 = smith.full((2,), x1, dtype=smith.float64)
        self.assertEqual(result1, expected1)

        # This is where the bug occurred - second call reused first value
        result2 = func_compiled(x2)
        expected2 = smith.full((2,), x2, dtype=smith.float64)
        ...
        for dtype in [smith.float32, smith.float64, smith.int32, smith.int64]:
            ...
            def func_typed(x):
                return smith.full((3,), x, dtype=dtype)

            func_typed_compiled = smith.compile(func_typed)
            x_typed = smith.tensor(7, dtype=dtype)
            result = func_typed_compiled(x_typed)
            expected = smith.full((3,), x_typed, dtype=dtype)
            self.assertEqual(result, expected)

        # Test with non-tensor fill_value (scalar) to ensure we didn't break existing behavior
        def func_scalar(size):
            return smith.full((size,), 42.0, dtype=smith.float32)

        func_scalar_compiled = smith.compile(func_scalar)

        result_scalar = func_scalar_compiled(5)
        expected_scalar = smith.full((5,), 42.0, dtype=smith.float32)
        ...
        def func_scalar_param():
            ...
            a = smith.full((2,), 3.14, dtype=smith.float32)
            b = smith.full((2,), 2.71, dtype=smith.float32)
            return a, b

        func_scalar_param_compiled = smith.compile(func_scalar_param)
        result_a, result_b = func_scalar_param_compiled()

        self.assertEqual(result_a, smith.full((2,), 3.14, dtype=smith.float32))
        self.assertEqual(result_b, smith.full((2,), 2.71, dtype=smith.float32))
