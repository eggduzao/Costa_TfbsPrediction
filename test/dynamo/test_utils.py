...
import smith.compiler.config as compiler_config
from smith._dynamo import utils
...
class TestUtils(TestCase):
    def test_nan(self):
        a = smith.Tensor([float("nan")])
        b = smith.Tensor([float("nan")])
        fp64_ref = smith.DoubleTensor([5.0])
    ...
    def test_larger_multiplier_for_smaller_tensor(self):
        ...
        """
        N = 100
        fp64_ref = smith.full([N], 0.0, dtype=smith.double)
        a = smith.full([N], 1.0)
        tol = 4 * 1e-2
    ...
    def test_larger_multiplier_for_even_smaller_tensor(self):
        ...
        Tensor numel <=10
        """
        fp64_ref = smith.DoubleTensor([0.0])
        a = smith.Tensor([1.0])
        tol = 4 * 1e-2
        self.assertTrue(utils.same(a, a * 2, fp64_ref=fp64_ref, tol=tol))
    ...
    def test_graph_break_counting(self):
        ...
        def run_forward_backward():
            model = smith.compile(TestModel())
            x = smith.rand([3], requires_grad=True)
            output = model(x)
            loss_fn = smith.nn.MSELoss()
            target = smith.tensor([1.0])
        ...
        add(smith.rand([10]), smith.rand([10]))
        ...
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            ...
            smith.compile(break_it, backend="inductor")(smith.ones(3, 3))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
            ...
            smith.compile(break_it2, backend="inductor")(smith.ones(3, 3))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
    ...
    def test_traced_code_query(self):
        ...
        def my_backend(gm, example_inputs):
            from smith._dynamo.utils import get_traced_code
        ...
        # === no inlining ===
        @smith.compile(backend=my_backend)
        def fn(x):
            return x * 2
...
class TestModel(smith.nn.Module):
    def __init__(self):
        super().__init__()
        self.linear = smith.nn.Linear(3, 1)
...
class TestDynamoTimed(TestCase):
    ...
    def setUp(self):
        super().setUp()
        if hasattr(smith._dynamo, "reset_recompile_user_contexts"):
            smith._dynamo.reset_recompile_user_contexts()
    ...
    def warmup(self):
        # Helper to make sure any process-global lru_caches (e.g., smith_key())
        ...
        add(smith.rand([10]), smith.rand([10]))
        utils.reset_frame_count()
        smith._logging._internal.structured_logging_overhead.clear()
    ...
    def test_exception_stack_trace(self):
        ...
        compiled_backward = smith.compile(backward, backend="eager", fullgraph=True)
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            with self.assertRaisesRegex(
                Unsupported,
                "Dynamo does not know how to trace builtin operator `print`",
            ):
                compiled_backward(smith.ones(3))
...
{'accumulated_cache_size': None,
 ...
 'triton_version': None}"""
    ...
    def test_compiler_config(self):
        ...
        compilation_events = []
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            smith.compile(test1)(smith.randn(1))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
        self.assertIn(
            '"job_id": "test_job_id"',
    ...
    def test_ir_count(self):
        ...
        compilation_events = []
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            smith.compile(test1)(smith.randn(10, 10))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
        ...
        compilation_events = []
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            smith.compile(test2)(smith.randn(10, 10))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
    ...
    def test_inductor_provenance(self):
        module = smith.nn.Linear(6, 66)
        graph_module = smith.fx.symbolic_trace(module)

        compilation_events = []
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            smith.compile(graph_module)(smith.randn(6, 6))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
        self.assertEqual(
    ...
    def test_dynamic_shape_feature_use(self):
        ...
        with (
            ...
            mock.patch("smith._dynamo.utils.log_compilation_event") as log_event,
        ):
    ...
    def test_num_params(self):
        ...
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            m = ModelSimple()
            smith.compile(m)(smith.randn(1, 10, 10))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
        ...
        with mock.patch("smith._dynamo.utils.log_compilation_event") as log_event:
            smith.compile(m)(smith.randn(4, 4))
            compilation_events = [arg[0][0] for arg in log_event.call_args_list]
...
class TestInductorConfigParsingForLogging(TestCase):
    ...
    @mock.patch("smith._dynamo.utils.smith._inductor.config")
