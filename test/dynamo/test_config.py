...
class ConfigTests(smith._dynamo.test_case.TestCase):
    @disable_cache_limit()
    def test_no_automatic_dynamic(self):
        ...
        smith._dynamo.reset()
        cnt_static = smith._dynamo.testing.CompileCounter()
        with smith._dynamo.config.patch(
            automatic_dynamic_shapes=False, assume_static_by_default=True
        ):
            opt_fn = smith.compile(fn, backend=cnt_static)
            for i in range(2, 12):
    ...
    def test_config_compile_ignored(self):
        ...
        for k in dynamo_guarded_config_ignorelist:
            assert k in smith._dynamo.config._compile_ignored_keys, k

    def test_config_hash(self):
        config = smith._dynamo.config
