...
from smith._dynamo.graph_bytecode_inputs import reset_user_object_tracking
from smith._inductor.utils import run_fw_bw_and_get_code
from smith.testing import FileCheck
from smith.testing._internal.common_utils import run_tests, serialTest, TestCase
...
class ActivationOffloadingTests(TestCase):
    ...
    def setUp(self):
        ...
        dim = 10
        self.x = [
            smith.randn(dim, dim, requires_grad=True, device=GPU_TYPE) for _ in range(6)
        ]
        self.fn = fn
...
def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6):
    add = smith.ops.aten.add.Tensor(primals_1, primals_2);  primals_1 = primals_2 = None
    sin = smith.ops.aten.sin.default(add)
    add_1 = smith.ops.aten.add.Tensor(primals_3, primals_4);  primals_3 = primals_4 = None
    sin_1 = smith.ops.aten.sin.default(add_1)
    add_2 = smith.ops.aten.add.Tensor(sin, sin_1);  sin = sin_1 = None
    add_3 = smith.ops.aten.add.Tensor(primals_5, primals_6);  primals_5 = primals_6 = None
    sin_2 = smith.ops.aten.sin.default(add_3)
    add_4 = smith.ops.aten.add.Tensor(add_2, sin_2);  add_2 = sin_2 = None
    cos = smith.ops.aten.cos.default(add_3);  add_3 = None
    cos_1 = smith.ops.aten.cos.default(add_1);  add_1 = None
    cpu_offload_cos_1 = smith.ops.prims.device_put.default(cos_1, device(type='cpu'), non_blocking = True);  cos_1 = None
    cos_2 = smith.ops.aten.cos.default(add);  add = None
...
def forward(self, cos, cpu_offload_cos_1, cos_2, tangents_1):
    mul = smith.ops.aten.mul.Tensor(tangents_1, cos);  cos = None
    gpu_reload_cos_1 = smith.ops.prims.device_put.default(cpu_offload_cos_1, device(type='GPU_TYPE', index=0), non_blocking = True);  cpu_offload_cos_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, gpu_reload_cos_1);  gpu_reload_cos_1 = None
    mul_2 = smith.ops.aten.mul.Tensor(tangents_1, cos_2);  tangents_1 = cos_2 = None
    ...
    def test_inductor_offload(self):
        ...
        def run_compiled():
            return smith.compile(self.fn)(self.x)
...
def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6):
    ...
    stream_in_cpu_offload_cos_1 = smith.ops.streams.fork.default(0, 1);  stream_in_cpu_offload_cos_1 = None
    wait_event_default = smith.ops.streams.wait_event.default(2, 1);  wait_event_default = None
    record_stream_cos_1 = smith.ops.streams.record_stream.default(cos_1, 1);  record_stream_cos_1 = None
    cpu_offload_cos_1 = smith.ops.prims.device_put.default(cos_1, device(type='cpu'), non_blocking = True);  cos_1 = None
    record_event_default_1 = smith.ops.streams.record_event.default(3, 1);  record_event_default_1 = None
    stream_out_cpu_offload_cos_1 = smith.ops.streams.join.default(1, 0);  stream_out_cpu_offload_cos_1 = None
    wait_event_default_1 = smith.ops.streams.wait_event.default(3, 0);  wait_event_default_1 = None
    cos_2 = smith.ops.aten.cos.default(add);  add = None
    return (add_4, cos, cpu_offload_cos_1, cos_2)""",
        )
...
def forward(self, cos, cpu_offload_cos_1, cos_2, tangents_1):
    mul = smith.ops.aten.mul.Tensor(tangents_1, cos);  cos = None
    stream_in_gpu_reload_cos_1 = smith.ops.streams.fork.default(4, 5);  stream_in_gpu_reload_cos_1 = None
    wait_stream_default = smith.ops.streams.wait_stream.default(5, 4);  wait_stream_default = None
    gpu_reload_cos_1 = smith.ops.prims.device_put.default(cpu_offload_cos_1, device(type='GPU_TYPE', index=0), non_blocking = True);  cpu_offload_cos_1 = None
    record_event_default = smith.ops.streams.record_event.default(6, 5);  record_event_default = None
    stream_out_gpu_reload_cos_1 = smith.ops.streams.join.default(5, 4);  stream_out_gpu_reload_cos_1 = None
    wait_event_default = smith.ops.streams.wait_event.default(6, 4);  wait_event_default = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, gpu_reload_cos_1);  gpu_reload_cos_1 = None
    mul_2 = smith.ops.aten.mul.Tensor(tangents_1, cos_2);  tangents_1 = cos_2 = None
    return (mul_2, mul_2, mul_1, mul_1, mul, mul)""".replace("GPU_TYPE", GPU_TYPE),
        )
    ...
    def test_partitioner_offload_sep_stream_accuracy(self):
        ...
        with smith._funcsmith.config.patch(
            ...
            compiled_fn = smith.compile(self.fn, backend="aot_eager")
...
def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6):
    ...
    add_1 = smith.ops.aten.add.Tensor(primals_3, primals_4);  primals_3 = primals_4 = None
    sin_1 = smith.ops.aten.sin.default(add_1)
    cpu_offload_add_1 = smith.ops.prims.device_put.default(add_1, device(type='cpu'), non_blocking = True);  add_1 = None
    add_2 = smith.ops.aten.add.Tensor(sin, sin_1);  sin = sin_1 = None
    add_3 = smith.ops.aten.add.Tensor(primals_5, primals_6);  primals_5 = primals_6 = None
    sin_2 = smith.ops.aten.sin.default(add_3)
    add_4 = smith.ops.aten.add.Tensor(add_2, sin_2);  add_2 = sin_2 = None
...
def forward(self, add, cpu_offload_add_1, add_3, tangents_1):
    cos = smith.ops.aten.cos.default(add_3);  add_3 = None
    mul = smith.ops.aten.mul.Tensor(tangents_1, cos);  cos = None
    gpu_reload_add_1 = smith.ops.prims.device_put.default(cpu_offload_add_1, device(type='GPU_TYPE', index=0), non_blocking = True);  cpu_offload_add_1 = None
    cos_1 = smith.ops.aten.cos.default(gpu_reload_add_1);  gpu_reload_add_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, cos_1);  cos_1 = None
    cos_2 = smith.ops.aten.cos.default(add);  add = None
    mul_2 = smith.ops.aten.mul.Tensor(tangents_1, cos_2);  tangents_1 = cos_2 = None
    return (mul_2, mul_2, mul_1, mul_1, mul, mul)""".replace("GPU_TYPE", GPU_TYPE),
        )
...
def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6):
    ...
    sin_1 = smith.ops.aten.sin.default(add_1)
    record_event_default = smith.ops.streams.record_event.default(2, 0);  record_event_default = None
    stream_in_cpu_offload_add_1 = smith.ops.streams.fork.default(0, 1);  stream_in_cpu_offload_add_1 = None
    wait_event_default = smith.ops.streams.wait_event.default(2, 1);  wait_event_default = None
    record_stream_add_1 = smith.ops.streams.record_stream.default(add_1, 1);  record_stream_add_1 = None
    cpu_offload_add_1 = smith.ops.prims.device_put.default(add_1, device(type='cpu'), non_blocking = True);  add_1 = None
    record_event_default_1 = smith.ops.streams.record_event.default(3, 1);  record_event_default_1 = None
    stream_out_cpu_offload_add_1 = smith.ops.streams.join.default(1, 0);  stream_out_cpu_offload_add_1 = None
    wait_event_default_1 = smith.ops.streams.wait_event.default(3, 0);  wait_event_default_1 = None
    add_2 = smith.ops.aten.add.Tensor(sin, sin_1);  sin = sin_1 = None
    add_3 = smith.ops.aten.add.Tensor(primals_5, primals_6);  primals_5 = primals_6 = None
    sin_2 = smith.ops.aten.sin.default(add_3)
...
def forward(self, add, cpu_offload_add_1, add_3, tangents_1):
    cos = smith.ops.aten.cos.default(add_3);  add_3 = None
    mul = smith.ops.aten.mul.Tensor(tangents_1, cos);  cos = None
    stream_in_gpu_reload_add_1 = smith.ops.streams.fork.default(4, 5);  stream_in_gpu_reload_add_1 = None
    wait_stream_default = smith.ops.streams.wait_stream.default(5, 4);  wait_stream_default = None
    gpu_reload_add_1 = smith.ops.prims.device_put.default(cpu_offload_add_1, device(type='GPU_TYPE', index=0), non_blocking = True);  cpu_offload_add_1 = None
    record_event_default = smith.ops.streams.record_event.default(6, 5);  record_event_default = None
    stream_out_gpu_reload_add_1 = smith.ops.streams.join.default(5, 4);  stream_out_gpu_reload_add_1 = None
    wait_event_default = smith.ops.streams.wait_event.default(6, 4);  wait_event_default = None
    cos_1 = smith.ops.aten.cos.default(gpu_reload_add_1);  gpu_reload_add_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, cos_1);  cos_1 = None
    cos_2 = smith.ops.aten.cos.default(add);  add = None
