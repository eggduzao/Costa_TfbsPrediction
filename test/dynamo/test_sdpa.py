...
from smith.nn.attention import _cur_sdpa_kernel_backends, sdpa_kernel, SDPBackend
...
def allow_in_graph_sdpa_params():
    ...
    try:
        old = SDPAParams
        SDPAParams = smith._dynamo.allow_in_graph(SDPAParams)
        yield
    finally:
        SDPAParams = old


class TestSDPA(smith._dynamo.test_case.TestCase):
    ...
    def test_sdpa_kernel_decorator_with_compile(self):
        ...
        x = smith.rand(128, 64, 64, 256, dtype=smith.float16)
        result = f(x)
