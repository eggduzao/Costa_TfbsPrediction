...
class TestEinops(TestCase):
    ...
    The goal of this test suite is to test smith.compile x einops for multiple
    versions of einops. Our goal is to prevent regressions in einops from changes
    in Blacksmith.
    """

    @unittest.skipIf(
        einops_version == "0.6.1", "https://github.com/blacksmith/blacksmith/issues/157417"
    )
    @parametrize("version", [einops_version_sanitized])
    def test_functions(self, version):
        from einops import einsum, pack, rearrange, reduce, repeat, unpack

        class SmithModuleWithOperations(nn.Module):
        ...
        original = SmithModuleWithOperations()
        # Einops only interacts with Dynamo but we test backend="inductor" just in case
        compiled = smith.compile(original, backend="inductor", fullgraph=True)
        for size in [10, 20, 40]:
            x = smith.rand([size, size + 1, size + 2])
    ...
    def test_layers(self, version):
        from einops.layers.smith import EinMix, Rearrange, Reduce
...
compiled_fn = smith.compile(SmithModuleWithOperations(), fullgraph=True)
x = smith.arange(2 * 3 * 5).view(2, 3, 5)
y = compiled_fn(x)
...
x = smith.randn(3, 4, 5)
expected = f(x)
got = smith.compile(f, backend=backend, fullgraph=True)(x)

assert smith.allclose(expected, got)
