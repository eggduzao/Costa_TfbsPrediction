...
from smith._dynamo.convert_frame import GlobalStateGuard
from smith._dynamo.eval_frame import _debug_get_cache_entry_list
from smith.testing._internal.common_utils import set_default_dtype
...
default_mgr_enum = smith._dynamo.guards.GuardManagerType.GUARD_MANAGER
...
global_pair = Pair(smith.randn(4), 1)
...
class GuardManagerTests(smith._dynamo.test_case.TestCase):
...
""",
        ...
        _orig = smith.are_deterministic_algorithms_enabled()
        try:
            smith.use_deterministic_algorithms(not _orig)
            self.assertFalse(guard(None))
            self.assertExpectedInline(
                str(guard.check_verbose(None)),
                """\
...
""",
            ...
            smith.use_deterministic_algorithms(_orig)
        self.assertTrue(guard(None))
        self.assertTrue(guard.check_verbose(None).result)
    ...
    def test_global_state_reason_autocast_cache(self):
        ...
        old_cache = smith.is_autocast_cache_enabled()
        try:
            smith.set_autocast_cache_enabled(True)
            ...
            smith.set_autocast_cache_enabled(old_cache)
    ...
    def test_tensor_aliasing_guard(self):
        ...
        f_locals_unaliased = Foo(smith.randn(3, 4), smith.randn(3, 4))
    ...
    def test_tensor_match_guard(self):
        ...
        guard_manager.add_tensor_match_guard(
            ...
            None,
            type(x),
            smith._C._dispatch_keys(x),
        )
        self.assertTrue(guard_manager.check(x))
        self.assertTrue(guard_manager.check_verbose(x).result)
        self.assertTrue(guard_manager.check(smith.randn(4, 4)))
        self.assertTrue(guard_manager.check_verbose(smith.randn(4, 4)).result)
    ...
    def test_framelocals_guard_e2e(self):
        ...
        ref = opt_fn(smith.ones(3), 2, {0: 1, 2: 3})
        with smith._dynamo.set_stance("fail_on_recompile"):
            res = opt_fn(smith.ones(3), 2, {0: 1, 2: 3})
    ...
    def test_globals(self):
        ...
        gpair_mgr.add_lambda_guard(
            ...
            and isinstance(x.x, smith.Tensor)
    ...
    def test_dict_guard_manager(self):
        ...
        f_locals = {
            "d": {"a": 1, nothing: {"z": 3}, 100: smith.randn(4)},
        }
        ...
        dict_mgr = root.getitem_manager(
            ...
            "",
            f_locals["d"],
            smith._dynamo.guards.GuardManagerType.DICT_GUARD_MANAGER,
        )
...
class TypePropagationTests(smith._dynamo.test_case.TestCase):
    @smith._dynamo.config.patch(skip_tensor_guards_with_matching_dict_tags=True)
    def test_basic_types(self):
        ...
        def hook(guard_wrapper, f_locals, builder):
            from smith._dynamo.source import AttrSource, DictGetItemSource, LocalSource
            ...
            self.assertTrue(
                issubclass(mod_mgr.get_type_of_guarded_value(), smith.nn.Module)
            )

        opt_fn = smith.compile(fn, backend="eager", fullgraph=True)
        with install_guard_manager_testing_hook(hook):
            opt_fn(smith.randn(4, 4))


class DuplicateGuardTest(smith._dynamo.test_case.TestCase):
...
class RecursiveDictTagTests(smith._dynamo.test_case.TestCase):
    def setUp(self):
        self._prev = smith._dynamo.config.use_recursive_dict_tags_for_guards
        smith._dynamo.config.use_recursive_dict_tags_for_guards = True

    def tearDown(self):
        smith._dynamo.config.use_recursive_dict_tags_for_guards = self._prev
...
class TagSafetyChecks(RecursiveDictTagTests):
    ...
    def test_immutable_tag_safe(self):
        ...
        def hook(guard_wrapper, f_locals, builder):
            from smith._dynamo.source import AttrSource, LocalSource
            ...
            if smith._dynamo.config.skip_tensor_guards_with_matching_dict_tags:
    ...
    def test_dict_tag_safe(self):
        ...
        def hook(guard_wrapper, f_locals, builder):
            from smith._dynamo.source import DictGetItemSource, LocalSource
    ...
    def test_nn_module_tag_overridden_getattr_safe(self):
        class Baz(smith.nn.Module, metaclass=abc.ABCMeta):
            def __init__(self):
                super().__init__()
                self.norm = 2
