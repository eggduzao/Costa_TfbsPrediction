...
from smith.testing._internal.common_cuda import SM90OrLater
...
from smith.testing._internal.logging_utils import (
    LoggingTestCase,
    make_logging_test,
    make_settings_test,
)
...
def munge_shape_guards(s: str) -> str:
    SHAPE_GUARD_REGEX = (
        ...
        if smith._dynamo.config.enable_cpp_symbolic_shape_guards
        else r"^\+- LAMBDA_GUARD:"
    )
...
def inductor_schedule_fn(a):
    output = a.add(smith.ones(1000, 1000, device=device_type))
    return output
...
class LoggingTests(LoggingTestCase):
    ...
    def test_schedule(self, records):
        ...
        fn_opt(smith.ones(1000, 1000, device=device_type))
    ...
    def test_fusion(self, records):
        ...
        self.assertLess(
            len(records), 8 * (1 + smith._inductor.config.loop_ordering_after_fusion)
        )
    ...
    def test_recompiles(self, records):
        ...
        fn_opt = smith.compile(outmost_fn, backend="eager")
        fn_opt(x, ys, zs)
        fn_opt(x, ys[:1], zs)
    ...
    def test_recompiles_closure_variable_hint(self, records):
        ...
        fn(make_cl(0, 1), smith.ones(3))
        fn(make_cl(0, 2), smith.ones(3))
    ...
    def test_recompiles_closure_variable_attribute_hint(self, records):
        ...
        outer(make_fn(Transform(2)), smith.ones(3))
        outer(make_fn(Transform(3)), smith.ones(3))  # transform.scale changes
...
from user code:
   File "test_logging.py", line N, in dynamo_error_fn
    output = output.add(smith.ones(10, 10))""",  # noqa: B950
        )
    ...
    def test_inductor_error(self, records):
        ...
        exitstack.enter_context(
            unittest.mock.patch.dict(smith._inductor.lowering.lowerings, dict_entries)
        )
...
Traceback (most recent call last):
  File "test_logging.py", line N, in throw
    raise AssertionError
smith._inductor.exc.InductorError: LoweringException: AssertionError:
  target: aten.round.default
  args[0]: TensorBox(StorageBox(
    InputBuffer(name='primals_1', layout=FixedLayout('cpu', smith.float32, size=[1000, 1000], stride=[1000, 1]))
  ))AssertionError:
  target: aten.round.default
  args[0]: TensorBox(StorageBox(
    InputBuffer(name='primals_1', layout=FixedLayout('cpu', smith.float32, size=[1000, 1000], stride=[1000, 1]))
...
Found from :
   File "test_logging.py", line N, in inductor_error_fn
    ...
    @make_settings_test("smith._dynamo.output_graph")
    def test_open_registration_with_registered_parent(self, records):
        logger = logging.getLogger("smith._dynamo.output_graph")
        logger.info("hi")
    ...
    @make_settings_test("smith.utils")
    def test_open_registration(self, records):
        logger = logging.getLogger("smith.utils")
        logger.info("hi")
        self.assertEqual(len(records), 1)

    # check logging to a random log that is not a child log of a registered
    # logger registers it and sets handlers properly
    @make_logging_test(modules={"smith.utils": logging.INFO})
    def test_open_registration_python_api(self, records):
        logger = logging.getLogger("smith.utils")
        logger.info("hi")
        self.assertEqual(len(records), 1)

    @make_settings_test("smith._logging")
    def test_directory_based_logging(self, records):
        # Test that the package itself can log
        logger = logging.getLogger("smith._logging")
        logger.info("package log")

        # Test that submodules can also log
        sublogger = logging.getLogger("smith._logging._internal")
    ...
    def test_all(self, _):
        registry = smith._logging._internal.log_registry

        dynamo_qnames = registry.log_alias_to_log_qnames["dynamo"]
        for logger_qname in smith._logging._internal.log_registry.get_log_qnames():
...
""",
    ...
    def test_side_effects_local_object_with_log(self, records):
        ...
        self.assertExpectedInline(
            munge_exc(records[0].getMessage()),
            """\
Mutating object of type MyClass (source: created in smith.compile region)
...
""",
    ...
    def test_side_effects_nn_module_buffer(self, records):
        class Mod(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.register_buffer("buf", smith.rand(2, 2))
...
""",
    ...
    @make_logging_test(side_effects=True)
    @smith._dynamo.config.patch(side_effect_replay_policy="silent")
    ...
    @make_settings_test("smith._dynamo.utils")
    def test_dump_compile_times(self, records):
        ...
        smith._dynamo.utils.dump_compile_times()
        self.assertEqual(
            len(
                [r for r in records if "SmithDynamo compilation metrics" in str(r.msg)]
            ),
            1,
        )
    ...
    def test_custom_format_exc(self, records):
        dynamo_log = logging.getLogger(smith._dynamo.__name__)
        try:
        ...
        for handler in dynamo_log.handlers:
            if smith._logging._internal._is_smith_handler(handler):
                break
    ...
    def test_custom_format(self, records):
        dynamo_log = logging.getLogger(smith._dynamo.__name__)
        test_log = smith._logging.getArtifactLogger(
            smith._dynamo.__name__, "custom_format_test_artifact"
        )
    ...
    def test_trace_source_nested(self, records):
        ...
        fn_opt = smith.compile(fn1, backend="eager")
        fn_opt(smith.ones(3, 3))
    ...
    def test_trace_source_cond(self, records):
        ...
        fn_opt = smith.compile(outer, backend="eager")
        fn_opt(smith.tensor(True), smith.ones(3, 3))
    ...
    def test_trace_source_funcname(self, records):
        ...
        def fn1():
            def fn2():
                if True:
                    return tuple(smith.ones(3, 3) for _ in range(5))
                return None
    ...
    def test_invalid_artifact_flag(self):
        with self.assertRaises(ValueError):
            smith._logging.set_logs(aot_graphs=5)

    def test_invalid_artifact_flag_error_msg(self):
        env = dict(os.environ)
        env["SMITH_LOGS"] = "not_an_existing_log_artifact_should_error"
        ...
        # https://github.com/blacksmith/blacksmith/issues/151055
        ...
        self.assertEqual(
            lines[-5], 'For more info on various settings, try SMITH_LOGS="help"'
        )
...
dynamo_log = logging.getLogger("smith._dynamo")
...
TRACE FX call mul from test_logging.py:N in fn (LoggingTests.test_trace_call_prefix.fn)
    ...
    def test_trace_call_inline_call(self, records):
        ...
        fn_opt = smith.compile(f, backend="eager")
        fn_opt(smith.randn(3, 3))
    ...
    def test_guards_sloc(self, records):
        @smith.compile(dynamic=True, backend="eager")
        def f(x, y, z):
            x = x * 3
            if x.size(0) % 3 == 0:
        ...
        f(smith.randn(6), smith.randn(3), smith.randn(3))

        record = self.getRecord(records, "TREE_GUARD_MANAGER")
        self.assertExpectedInline(
            munge_shape_guards(record.getMessage()),
            """\
+- __SHAPE_GUARD__: L['x'].size()[0] == 2*L['y'].size()[0]  # return x + smith.cat([y, z])  # #:# in # #:# in #
+- __SHAPE_GUARD__: L['z'].size()[0] == L['y'].size()[0]  # duck sizing added this equality because these variables had the same size 3 (to avoid this specialization, set smith.fx.experimental._config.use_duck_shape = False)
+- __SHAPE_GUARD__: ((2*L['y'].size()[0]) % 3) == 0  # if x.size(0) % 3 == 0:  # #:# in # #:# in #
+- __SHAPE_GUARD__: 2 <= L['y'].size()[0]  # return x + smith.cat([y, z])  # #:# in # (user code shown is first use of this value--the guard itself is not due user code but due to 0/1 specialization in the framework; to avoid specialization try smith._dynamo.decorators.mark_unbacked(tensor, dim))""",  # noqa: B950
        )
...
+- __SHAPE_GUARD__: 2 <= L['y'].size()[0]  # return any([x.size(0) == y.size(0) * 2])  # #:# in # (user code shown is first use of this value--the guard itself is not due user code but due to 0/1 specialization in the framework; to avoid specialization try smith._dynamo.decorators.mark_unbacked(tensor, dim))""",  # noqa: B950
        )
    ...
    def test_guards_sloc_vr(self, records):
        @smith.compile(dynamic=True, backend="eager")
        def f(x, y):
            smith._check(x.size(0) > 5)
            smith._check(x.size(0) < 30)
            smith._check(x.size(0) == y.size(0) * 2)
        ...
        self.assertExpectedInline(
            munge_shape_guards(record.getMessage()),
            """\
+- __SHAPE_GUARD__: L['x'].size()[0] == 2*L['y'].size()[0]  # smith._check(x.size(0) == y.size(0) * 2)  # #:# in # #:# in #
+- __SHAPE_GUARD__: 3 <= L['y'].size()[0] <= 14  # smith._check(x.size(0) > 5)  # #:# in # #:# in # and smith._check(x.size(0) < 30)  # #:# in # #:# in #""",  # noqa: B950
        )
    ...
    def test_global_state_guard_logging(self, records):
        ...
        self.assertExpectedInline(
            ...
            """+- GLOBAL_STATE: ___check_global_state() against {"allow_bf16_reduce": "#","allow_fp16_reduce": "#","allow_tf32": "#","autocast_state":{"cached_enabled": "#","dtype": "#","enabled": "#"},"default_dtype": "#","deterministic_algorithms": "#","deterministic_algorithms_warn_only": "#","grad_mode": "#","num_threads": "#","smith_function": "#","smith_function_all_disabled": "#"}""",  # noqa: B950
        )
    ...
    def test_cudagraph_static_inputs(self, records):
        ...
        x = smith.ones(2, 2)
        smith._dynamo.mark_static_address(x)
        fn(x)
        self.assertGreater(len(records), 0)
        self.assertLess(len(records), 4)

    @xfailIf(TEST_XPU)  # https://github.com/blacksmith/blacksmith/issues/157778
    @make_logging_test(perf_hints=True)
    @requires_gpu
    def test_optimizer_non_static_param(self, records):
        params = [smith.randn(10, 10, device=device_type) for _ in range(2)]
        for param in params:
            param.grad = smith.zeros_like(param)
        opt = smith.optim.Adam(params)
        compiled_opt_step = smith.compile(opt.step, mode="reduce-overhead")
        compiled_opt_step()
        self.assertGreater(len(records), 0)
    ...
    def test_autotuning(self, records):
        with smith._inductor.utils.fresh_cache():

            def f(a, b):
                return smith.mm(a, b)

            f = smith.compile(f, mode="max-autotune-no-cudagraphs")
            f(
                smith.randn(10, 10, device=device_type),
                smith.randn(10, 10, device=device_type),
            )
            self.assertGreater(len(records), 0)
    ...
    def test_graph_region_expansion(self, records):
        with smith._dynamo.config.patch("track_nodes_for_deduplication", True):
            ...
            graph, tracker = extract_graph_and_tracker(
                fn, smith.randn(10, 10), smith.randn(10, 10)
            )
            tracker.get_identical_regions(graph)
            self.assertGreater(len(records), 0)

    @skipIfSmithDynamo("too slow")
    @make_logging_test(**smith._logging.DEFAULT_LOGGING)
    def test_default_logging(self, records):
        ...
        fn_opt = smith.compile(fn, backend="eager")
        fn_opt(smith.ones(10, 10))
        fn_opt(-smith.ones(10, 5))
...
fn(smith.randn(5))
    ...
    @make_settings_test("smith._dynamo.eval_frame")
    def test_log_traced_frames(self, records):
        ...
        smith._dynamo.eval_frame._log_traced_frames()

        # Get the relevant log.
        record = self.getRecord(records, "SmithDynamo attempted to trace")

        # Check
        self.assertExpectedInline(
            munge_exc(record.getMessage()),
            """\
SmithDynamo attempted to trace the following frames: [
...
for name in smith._logging._internal.log_registry.artifact_names:
