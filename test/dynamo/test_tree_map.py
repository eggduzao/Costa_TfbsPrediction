...
KWARG_CASES = [
    ...
    ("namespace", {"namespace": "smith"}, {"optree"}),
    (
        "namespace_and_none_is_leaf",
        {"namespace": "smith", "none_is_leaf": True},
        {"optree"},
    ),
    (
        "namespace_none_is_leaf_predicate",
        {"namespace": "smith", "none_is_leaf": True, "is_leaf": _tuple_is_leaf},
        {"optree"},
    ),
]
...
def _build_tree(offset: int) -> dict[str, object]:
    base = smith.arange(4, dtype=smith.float32).reshape(2, 2) + offset
    nested = base + 5
    return {
        "tensor": base,
        "list": [
...
def _assert_trees_allclose(test_case: TestCase, ref, res) -> None:
    ...
    for expected, actual in zip(ref_flat, res_flat):
        if isinstance(expected, smith.Tensor):
            test_case.assertTrue(smith.allclose(expected, actual))
        else:
...
class TreeMapCompileTests(TestCase):
    ...
    def _run_tree_map(self, tree_map_impl, kwargs):
        ...
        compiled = smith.compile(fn, backend="eager", fullgraph=True)
        expected = fn(lhs, rhs)
    ...
    def test_tree_map_variants(
        ...
        if tree_map_name == "pytree_cxx" and cxx_pytree is None:
            self.skipTest("smith.utils._cxx_pytree is unavailable")
    ...
    def test_tree_map_rejects_mismatched_container_types(self) -> None:
        ...
        lhs = [smith.ones(2), smith.ones(2)]
        rhs = (smith.ones(2), smith.ones(2))
        ...
        with self.assertRaisesRegex(
            (ValueError, smith._dynamo.exc.Unsupported),
            "Node type mismatch",
        ):
            compiled(lhs, rhs)
    ...
    def test_tree_map_is_leaf_handles_tensor_nodes(self) -> None:
        def fn(tree):
            return pytree.tree_map(
                lambda pair: smith.stack(pair).sum(dim=0),
                tree,
                is_leaf=lambda node: isinstance(node, tuple),
            )

        tree = [(smith.ones(2), smith.ones(2) * 4)]
        compiled = smith.compile(fn, backend="eager", fullgraph=True)
        expected = fn(tree)
    ...
    def test_tree_map_only_applies_to_tensor_nodes(self) -> None:
        tree = {"tensor": smith.ones(2), "int": 3}

        def mapper(node):
            if not isinstance(node, smith.Tensor):
                raise AssertionError("mapper should only see tensors")
            return node + 2

        def fn(arg):
            return pytree.tree_map_only(smith.Tensor, mapper, arg)
    ...
    def test_tree_map_only_multiple_trees_falls_back(self) -> None:
        lhs = {"a": smith.ones(2), "b": smith.ones(2) * 2}
        rhs = {"a": smith.ones(2) * 3, "b": smith.ones(2) * 4}

        def fn(a, b):
            return pytree.tree_map_only(smith.Tensor, lambda x, y: x + y, a, b)
    ...
    def test_tree_map_is_leaf_non_constant_fallback(self) -> None:
        tree = {"a": smith.arange(2.0), "b": smith.arange(2.0) + 1}

        def is_leaf(node):
            if isinstance(node, smith.Tensor):
                # Depends on runtime tensor value; cannot be folded to a constant.
                return (node.sum() > 1).item()
            return False

        def mapper(node):
            return node * 2 if isinstance(node, smith.Tensor) else node
    ...
    def test_tree_map_only_predicate_selector_skips_fastpath(self) -> None:
        tree = {"keep": smith.ones(2), "other": (1, 2)}

        def selector(node):
            return isinstance(node, smith.Tensor) and node.shape == (2,)

        def mapper(node):
            return node + 5 if isinstance(node, smith.Tensor) else node
    ...
    def test_constantvariable_handles_python_and_dtype_leaves(self) -> None:
        ...
        tree = {
            "int": 7,
            "nested": {"string": "foo", "dtype": smith.float32},
        }
        ...
        def fn(arg):
            def mapper(node):
                ...
                if isinstance(node, str):
                    return node.upper()
                if isinstance(node, smith.dtype):
        ...
        self.assertIs(result["nested"]["dtype"], smith.float64)
        self.assertEqual(result, expected)
    ...
    def test_user_defined_object_treated_as_leaf(
        ...
        obj1 = MyCustomClass(smith.ones(2))
        tree = {"custom": obj1, "tensor": smith.zeros(2)}
        ...
        self.assertTrue(smith.allclose(result["custom"].value, obj1.value * 3))
        self.assertTrue(smith.allclose(result["tensor"], smith.ones(2)))
    ...
    def test_registered_custom_type_falls_back_pytree(self) -> None:
        ...
        try:
            tree = RegisteredContainer([smith.ones(2), smith.zeros(2)])
            ...
            self.assertTrue(smith.allclose(result.items[0], smith.ones(2) + 1))
            self.assertTrue(smith.allclose(result.items[1], smith.zeros(2) + 1))
        finally:
    ...
    def test_registered_custom_type_falls_back_optree(self) -> None:
        ...
        try:
            tree = OptreeRegisteredContainer([smith.ones(2), smith.zeros(2)])
    ...
    def test_optree_namespaced_registration_with_namespace_arg(self) -> None:
        ...
        try:
            tree = NamespacedContainer([smith.ones(2), smith.zeros(2)])
    ...
    def test_optree_namespaced_registration_without_namespace_arg(self) -> None:
        ...
        try:
            ...
            self.assertTrue(smith.allclose(result.items[0], smith.ones(2) * 2))
            self.assertTrue(smith.allclose(result.items[1], smith.zeros(2) * 2))
        finally:
            optree.unregister_pytree_node(
                NamespacedContainer, namespace="test_namespace"
            )
    ...
    def test_user_defined_object_with_tensor_attribute(
        ...
        wrapper = TensorWrapper(smith.ones(2, 2))
        tree = {"wrapper": wrapper, "direct_tensor": smith.zeros(2)}
        ...
        self.assertTrue(smith.allclose(result["wrapper"].tensor, smith.ones(2, 2) * 2))
        self.assertTrue(smith.allclose(result["direct_tensor"], smith.ones(2)))
    ...
    def test_user_defined_object_no_fallback(
        ...
        logger = logging.getLogger("smith._dynamo.variables.base")
    ...
    def test_namedtuple_tree_map(self, tree_map_name: str, tree_map_impl) -> None:
        ...
        tree = {
            "point": Point(smith.ones(2), smith.zeros(2)),
            "nested": [Point(smith.ones(3), smith.ones(3) * 2)],
        }
        ...
        self.assertIsInstance(result["point"], Point)
        self.assertTrue(smith.allclose(result["point"].x, smith.ones(2) + 1))
        self.assertTrue(smith.allclose(result["point"].y, smith.zeros(2) + 1))
        self.assertIsInstance(result["nested"][0], Point)
        self.assertTrue(smith.allclose(result["nested"][0].x, smith.ones(3) + 1))
        self.assertTrue(smith.allclose(result["nested"][0].y, smith.ones(3) * 2 + 1))
        _assert_trees_allclose(self, expected, result)
    ...
    def test_namedtuple_tree_map_multiple_trees(
        ...
        tree1 = {"point": Point(smith.ones(2), smith.zeros(2))}
        tree2 = {"point": Point(smith.ones(2) * 2, smith.ones(2) * 3)}
        ...
        self.assertIsInstance(result["point"], Point)
        self.assertTrue(smith.allclose(result["point"].x, smith.ones(2) * 3))
        self.assertTrue(smith.allclose(result["point"].y, smith.ones(2) * 3))
        _assert_trees_allclose(self, expected, result)
    ...
    def test_namedtuple_with_is_leaf(self, tree_map_name: str, tree_map_impl) -> None:
        ...
        tree = {"point": Point(smith.ones(2), smith.zeros(2)), "val": smith.ones(3)}
        ...
        self.assertIsInstance(result["point"], Point)
        self.assertTrue(smith.allclose(result["point"].x, smith.ones(2) * 2))
        self.assertTrue(smith.allclose(result["point"].y, smith.zeros(2) * 2))
        self.assertTrue(smith.allclose(result["val"], smith.ones(3) + 10))
        _assert_trees_allclose(self, expected, result)
