...
class TestGlobals(smith._dynamo.test_case.TestCase):
    def test_store_global_1(self):
        ...
        res1 = opt_fn(x)
        res2 = fn(x)
        self.assertTrue(same(res2 - res1, smith.ones(10)))
    ...
    def test_store_global_2(self):
        ...
        opt_fn = smith.compile(fn, backend=cnts)
        res1 = opt_fn(x)
        """Wrap the second call with smith._dynamo as well"""
        opt_fn = smith.compile(fn, backend=cnts)
        res2 = opt_fn(x)
        self.assertTrue(same(res2 - res1, 2 * smith.ones(10)))
