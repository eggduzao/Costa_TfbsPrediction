...
class GeneratorTestsBase(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    def setUp(self):
        super().setUp()
        self._old = smith._dynamo.config.enable_faithful_generator_behavior
        smith._dynamo.config.enable_faithful_generator_behavior = True
        self._unittest_old = smith._dynamo.config.enable_trace_unittest
        smith._dynamo.config.enable_trace_unittest = True

    def tearDown(self):
        super().tearDown()
        smith._dynamo.config.enable_faithful_generator_behavior = self._old
        smith._dynamo.config.enable_trace_unittest = self._unittest_old

    def _compile_check(self, fn, args=None, fullgraph=True):
        eager = EagerAndRecordGraphs()
        if args is None:
            args = (smith.randn(2),)
        r = smith.compile(fn, backend=eager, fullgraph=fullgraph)(*args)
...
""",
    ...
    def test_dict_tuple_list_generator(self, container):
        ...
        t = smith.randn(2)
        expected = fn(t)
        got = smith.compile(backend="eager", fullgraph=True)(fn)(t)
    ...
    def test_list_extend(self):
        ...
        self.assertEqual(
            f(smith.ones(3)),
            smith.compile(f, backend="eager", fullgraph=True)(smith.ones(3)),
        )
...
class TestGeneratorSend(GeneratorTestsBase):
    ...
    def test_send_stop_iteration(self, fullgraph):
        ...
        @smith.compile(backend="eager", fullgraph=fullgraph)
        def fn(t):
            gen = double()
            next(gen)
            a = gen.send(t)
