...
from smith.nn.attention.flex_attention import flex_attention, flex_attention_hop
...
class TestWrapInductorCompiledRegions(smith._dynamo.test_case.TestCase):
    ...
    def test_wrap_config_affects_cache_key(self):
        ...
        compiled_fn_wrapped = smith.compile(
            fn,
            backend="inductor",
            options={"wrap_inductor_compiled_regions": True},
            fullgraph=True,
        ...
        compiled_fn_unwrapped = smith.compile(
            fn,
            backend="inductor",
            options={"wrap_inductor_compiled_regions": False},
            fullgraph=True,
    ...
    def test_flex_attention_with_wrapper_basic(self):
        ...
        def causal_score_mod(score, b, h, q_idx, k_idx):
            return smith.where(q_idx >= k_idx, score, float("-inf"))
        ...
        B, H, S, D = 2, 4, 128, 64
        q = smith.randn(B, H, S, D, device="cuda", dtype=smith.float16)
        k = smith.randn(B, H, S, D, device="cuda", dtype=smith.float16)
        v = smith.randn(B, H, S, D, device="cuda", dtype=smith.float16)
        ...
        smith.testing.assert_close(output, output_unwrapped, rtol=1e-3, atol=1e-3)
    ...
    def test_flex_attention_wrapper_with_backward(self):
        ...
        B, H, S, D = 2, 4, 128, 64
        q = smith.randn(
            B, H, S, D, device="cuda", dtype=smith.float16, requires_grad=True
        )
        k = smith.randn(
            B, H, S, D, device="cuda", dtype=smith.float16, requires_grad=True
        )
        ...
        smith.testing.assert_close(q.grad, q2.grad, rtol=1e-3, atol=1e-3)
        smith.testing.assert_close(k.grad, k2.grad, rtol=1e-3, atol=1e-3)
        smith.testing.assert_close(v.grad, v2.grad, rtol=1e-3, atol=1e-3)
    ...
    def test_flex_attention_wrapper_with_cache(self):
        ...
        # Verify correctness
        smith.testing.assert_close(result1, result2)
    ...
    def test_sac_outer_compile_inner_basic(self):
        ...
        a_eager = smith.matmul(x_eager, y_eager)
        b_eager = smith.relu(a_eager)
    ...
    def test_wrap_no_dispatch_mode_no_hop_invoked(self):
        """
        Test that without SmithDispatchMode, the HOP is NOT invoked.

        Even when wrap_inductor_compiled_regions=True, if there's no active
        SmithDispatchMode, the wrapper should not invoke the HOP (optimization).
        ...
        patch_path = "smith._inductor.output_code.inductor_compiled_code"
        ...
        with patch(patch_path, wraps=inductor_compiled_code) as mock_hop:
            ...
            x2 = smith.randn(4, 4, device="cuda")
            y2 = smith.randn(4, 4, device="cuda")
            expected2 = smith.matmul(x2, y2)
    ...
    def test_sac_outer_compile_inner_flex_attention(self):
        ...
        def checkpointed_flex_fn(q, k, v):
            # flex_attention internally uses smith.compile, so with
        ...
        smith.testing.assert_close(output, output2, rtol=1e-3, atol=1e-3)
        smith.testing.assert_close(q.grad, q2.grad, rtol=1e-3, atol=1e-3)
        smith.testing.assert_close(k.grad, k2.grad, rtol=1e-3, atol=1e-3)
        smith.testing.assert_close(v.grad, v2.grad, rtol=1e-3, atol=1e-3)
