...
from smith._dynamo.package import CompilePackage, DiskDynamoStore, DynamoCache
...
@instantiate_parametrized_tests
class TestPackage(smith._inductor.test_case.TestCase):
    def path(self):
    ...
    def test_nn_module(self):
        class MyModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(10, 10, device="cuda")
        ...
        compiled_fn = smith._dynamo.optimize("inductor", package=package)(fn)
    ...
    def test_basic_fn(self, backend, device):
        ...
        compiled_fn = smith._dynamo.optimize(backend, package=package)(fn)
        ...
        with smith.compiler.set_stance("fail_on_recompile"):
            ...
            compiled_fn = smith._dynamo.optimize(package=package)(fn)
    ...
    def test_graph_break_bomb(self, backend, device):
        ...
        # Saving
        package = CompilePackage(fn)
        compiled_fn = smith._dynamo.optimize(
            backend=backend, package=package, guard_filter_fn=guard_filter_fn
        )(fn)
        N = 10
        args_list = [(smith.tensor(x, device=device), 0, N - 1) for x in range(N)]
        ...
        with smith.compiler.set_stance("fail_on_recompile"):
            ...
            with self.assertRaisesRegex(
                ...
                compiled_fn(smith.tensor(N), 0, N - 1)
    ...
    def test_dynamic_shape(self, backend, device):
        ...
        args = (smith.randn(3, 2, device=device),)
        args1 = (smith.randn(5, 2, device=device),)
        args2 = (smith.randn(7, 2, device=device),)
        expected1 = fn(*args1)

        smith._dynamo.mark_dynamic(args[0], 0, min=3, max=5)

        # Saving
        package = CompilePackage(fn)
        compiled_fn = smith._dynamo.optimize(backend=backend, package=package)(fn)
        compiled_fn(*args)
        if backend == "eager":
            for backend_id, backend in package.cached_backends.items():
                ctx.record_eager_backend(backend_id, backend)
...
"""
    ...
    def test_dynamo_cache_manual_load(self, device):
        ...
        compiled_fn1 = smith._dynamo.optimize(backend="inductor", package=package1)(fn)
        compiled_fn2 = smith._dynamo.optimize(backend="inductor", package=package2)(fn2)
        arg1 = smith.randn(3, 2, device=device)
        arg2 = smith.randn(5, 2, device=device)
        expected = [compiled_fn1(arg1), compiled_fn2(arg2)]

        DynamoCache.save(package1)
        DynamoCache.save(package2)
        total_frames = smith._dynamo.convert_frame.FRAME_COUNTER
        ...
        self.assertEqual(smith._dynamo.convert_frame.FRAME_COUNTER, total_frames)
    ...
    def test_automatic_dynamo_recompiles(self, device):
        ...
        with smith.compiler.set_stance("fail_on_recompile"):
            ...
            arg3 = smith.randn(7, 2, device=device)
            compiled_fn(arg3)
    ...
    def test_automatic_dynamo_lazy_backward(self, device):
        ...
        arg1 = smith.randn(3, 2, device=device, requires_grad=True)
    ...
    def test_graph_break_partial_backend(self, device):
        ...
        self.assertEqual(smith._dynamo.convert_frame.FRAME_COUNTER, total_frames + 1)
    ...
    def test_call_function_from_resume(self, device):
        ...
        if device == "xpu" and not HAS_XPU_AND_TRITON:
            raise unittest.SkipTest("Requires XPU/Triton")
        mod = smith.nn.Linear(2, 3, device=device)
        ...
        args = (smith.randn(3, 2, device=device), mod)
        compiled_fn = smith.compile(foo)
        compiled_fn(*args)
        total_frames = smith._dynamo.convert_frame.FRAME_COUNTER
    ...
    def test_code_with_generator(self, device):
        ...
        def foo(set_of_x):
            if not all(isinstance(s, smith.Tensor) for s in set_of_x):
                raise TypeError(
                    f"Expected all elements of set_of_x to be tensors, got {set_of_x}"
                )

            return smith.cat(set_of_x, dim=0)

        args = ([smith.randn(3, 2, device=device) for _ in range(3)],)
        compiled_fn = smith.compile(foo)
        compiled_fn(*args)
        self._save_and_reload(expected_backends=1, expected_dynamo=1)
