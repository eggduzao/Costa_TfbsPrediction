...
from smith.testing._internal.common_utils import make_dynamo_test, munge_exc
...
class _BaseSetTests(smith._dynamo.test_case.TestCase):
    def setUp(self):
        self.old = smith._dynamo.config.enable_trace_unittest
        smith._dynamo.config.enable_trace_unittest = True
        super().setUp()

    def tearDown(self):
        smith._dynamo.config.enable_trace_unittest = self.old
...
class MiscTests(smith._dynamo.test_case.TestCase):
...
class TestSetGuards(LoggingTestCase):
    def test_set_with_function(self):
        ...
        def fn(x, s):
            if smith.amp._exit_autocast in s:
                return x.sin()
            return x.cos()
        ...
        s.remove(smith.amp._exit_autocast)
        s.add(smith._C._set_fwd_grad_enabled)
        y = fn(x, s)
...
Attempted to wrap a set with tensors
  ...
  Developer debug context: Python set containing smith.Tensor elements

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0222.html
...
from user code:
   File "test_sets.py", line N, in fn
    ...
    def test_set_recompile_on_key_change(self):
        ...
        s.add(smith._C._set_autograd_fallback_mode)
    ...
    def test_set_guard_on_keys_change(self):
        ...
        s.add(smith.amp._exit_autocast)
