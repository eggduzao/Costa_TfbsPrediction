...
from smith._dynamo.testing import extract_graph_and_tracker
...
class GraphRegionTrackerTests(TestCase):
 ...
 ['x1_2', 'y1_2', 'sum_7', 'sum_8', 'z_2']], [['a', 'b', 'cos_1', 'sum_1', 'c'], ['a_1', 'b_1', 'cos_2', 'sum_6', 'c_1']]]""",
    ...
    def test_no_single_node_regions(self):
        ...
        self.assertExpectedInline(self.get_result(fn, smith.ones(10, 10)), """[]""")
...
['x1_2', 'sum_7', 'z_2']], [['b', 'cos_1', 'sum_1'], ['b_1', 'cos_2', 'sum_6']]]""",
    ...
    def test_mismatched_dtypes(self):
        ...
        def fn(x, y):
            ...
            o4 = inner_fn(x0, y0)
            o5 = inner_fn(x0, y0)
            o1 = inner_fn(x0.to(smith.bfloat16), y0.to(smith.bfloat16))
            o3 = o1 + o2
            return o3 * o0 + o4 + o5
...
['x1_2', 'y1_2', 'sum_3', 'o4'], ['x1_3', 'y1_3', 'sum_4', 'o5']]]""",
    ...
    def test_nested_args(self):
        ...
        def fn(x, y, z):
            ...
            o1 = inner_fn(
                [x0.to(smith.bfloat16), z0.to(smith.bfloat16)],
                [x0.to(smith.bfloat16), y0.to(smith.bfloat16)],
            )
            o3 = o1 + o2
            return o3 * o0 + o4 + o5
...
'o4'], ['_foreach_add_3', 'getitem_6', 'getitem_7', 'sum_4', 'o5']]]""",
    ...
    def test_mismatched_global_state(self):
        ...
        def create_toggle_fns(property):
            old_value = getattr(smith.backends.cuda.matmul, property)

            def toggle_property():
                setattr(smith.backends.cuda.matmul, property, not old_value)

            def reset_property():
                setattr(smith.backends.cuda.matmul, property, old_value)
        ...
        for ctx in [
            lambda: smith.set_grad_enabled(False),
            smith.autograd.grad_mode.inference_mode,
            lambda: smith.autograd.graph.disable_saved_tensors_hooks(
                "This is not supported"
            ),
            # lambda: smith.set_num_threads(2), : Unsupported
            (set_default_dtype_bfloat16, reset_default_dtype),
            (
                lambda: smith.use_deterministic_algorithms(True),
                lambda: smith.use_deterministic_algorithms(False),
            ),
            # (lambda: smith.use_deterministic_algorithms(True, warn_only=True),
            # lambda: smith.use_deterministic_algorithms(False)), : Unsupported
            ...
            self.assertExpectedInline(
                self.get_result(fn, smith.rand(10, 10), smith.ones(10, 20), ctx),
...
[['x1', 'y1', 'sum_1', 'o4'], ['x1_1', 'y1_1', 'sum_2', 'o5']]]""",
    ...
    def test_mutation_tracking_setitem(self):
        ...
        self.assertExpectedInline(
            self.get_mutation_tracking(fn, smith.rand(10, 10)),
    ...
    def test_non_tensor_arg_hashing(self):
        def inner(x, w, t):
            y = x + x
            return smith.conv2d(y, w, None, *t)
        ...
        self.assertExpectedInline(
            self.get_result(
                fn,
                smith.rand(32, 256, 56, 56),
                smith.nn.Parameter(smith.rand(512, 256, 1, 1)),
            ),
            """[[['y', 'o1'], ['y_1', 'o2'], ['y_2', 'o3']]]""",
        )

    def test_region_sorting(self):
        from smith._dynamo.graph_region_tracker import _sort_with_ref_region
    ...
    def test_no_duplicate_tracking(self):
        ...
        graph, tracker = extract_graph_and_tracker(
            fn, smith.rand(10, 10), smith.ones(10, 20)
        )
        self.assertExpectedInline(
