...
from smith._dynamo.testing import unsupported
from smith._dynamo.utils import ifdynstaticdefault


globalmod = smith.nn.ReLU()
...
class SubGraphTests(smith._dynamo.test_case.TestCase):
    def _common(self, fn, frame_count, op_count):
        smith._dynamo.reset()
        v1 = smith.ones(10)
        v2 = smith.ones(10) * -2.0
        correct1 = fn(v1, v2)
        correct2 = fn(v2, v1)
        ...
        r1 = opt_fn(v1, v2)
        r2 = opt_fn(v2, v1)
        self.assertTrue(smith._dynamo.testing.same(r1, correct1))
        self.assertTrue(smith._dynamo.testing.same(r2, correct2))
        self.assertEqual(
            cnt.frame_count,
            frame_count,
    ...
    def test_capi_call3(self):
        def fn(a, b):
            c1 = a - b
            c2 = b - a
            return smith._dynamo.testing.unsupported(c1, c2)
    ...
    def test_start4(self):
        ...
        v1 = smith.randn(10)
        v2 = smith.randn(10)
        f = smith.zeros(1, dtype=smith.int32)
        t = smith.ones(1, dtype=smith.int32)
        correct1 = fn(v1, v2, t)
        correct2 = fn(v1, v2, f)
        cnt = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(fn, backend=cnt)
    ...
    def test_dynamic_getitem(self):
        ...
        opt_fn = smith.compile(fn, backend=cnt)
        for i in range(3, 12):
            opt_fn(smith.randn(i), smith.randn(i))
        # just one graph
    ...
    def test_dynamic_kwarg(self):
        ...
        smith._dynamo.reset()
        cnt_dynamic = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(fn, backend=cnt_dynamic, dynamic=True)
        start = 2
        end = 12
    ...
    def test_dynamic_order_dependence(self):
        ...
        smith._dynamo.reset()
        cnt_dynamic = smith._dynamo.testing.CompileCounter()
        opt_fn = smith.compile(fn, backend=cnt_dynamic)
        x = smith.randn(2)
        y = smith.randn(3)
        self.assertEqual(opt_fn(x, y), fn(x, y))
        self.assertEqual(opt_fn(x, x), fn(x, x))
