...
from smith._dynamo.testing import CompileCounter, same
...
@zdevito.  It represents a rather challenging test case for SmithDynamo
...
class Variable:
    def __init__(self, value: smith.Tensor, name: Optional[str] = None):
        self.value = value
    ...
    @staticmethod
    def constant(value: smith.Tensor, name: Optional[str] = None):
...
def grad(L, desired_results: list[Variable]) -> list[Variable]:
    ...
    dL_d[L.name] = Variable(smith.ones(()))
    # print(f'd{L.name} ------------------------')
...
def operator_sum(self: Variable, name: Optional[str]) -> "Variable":
    r = Variable(smith.sum(self.value), name=name)
...
class TestPythonAutograd(TestCase):
    def _common(self, fn, expected_ops):
        ...
        cnt = CompileCounter()
        fn_dynamo = smith._dynamo.optimize_assert(cnt)(fn)
        reset_tape()
    ...
    def test_split(self):
        v1 = Variable.constant(smith.randn(10), name="a")
        v2 = Variable.constant(smith.randn(10), name="b")
        cnt = CompileCounter()
        ...
        reset_tape()
        opt_forward = smith._dynamo.optimize_assert(cnt)(forward)
        opt_grad = smith._dynamo.optimize_assert(cnt)(grad)
