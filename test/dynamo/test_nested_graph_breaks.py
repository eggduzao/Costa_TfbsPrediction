...
global1, global2, global3, global4 = (smith.zeros(3),) * 4


class NestedGraphBreakTests(smith._dynamo.test_case.TestCaseWithNestedGraphBreaks):
    def test_single_graph_break(self):
        ...
        opt_fn = smith._dynamo.optimize(backend=cnts)(f3)
        x = smith.zeros(3)
        res = f3(x)
        ref = opt_fn(x)
    ...
    def test_single_graph_break_repeat(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(f3)
        x = smith.zeros(3, dtype=smith.long)
        res = f3(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
    ...
    def test_differing_arg_nums(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(f4)
        x = smith.zeros(3)
        res = f4(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
    ...
    def test_counters(self):
        ...
        self.assertEqual(f4(smith.zeros(3)), smith.zeros(3) + 255)
        self.assertEqual(len(smith._dynamo.utils.counters["graph_break"]), 2)
    ...
    def test_supported_ctx_manager(self):
        ...
        def check_disabled(value):
            assert smith.is_grad_enabled() == value
    ...
    def test_inactive_ctx_manager(self):
        ...
        def f1(x, ctx1):
            x = x + 1
            ctx2 = smith.no_grad()
            # smith.no_grad() is a stack value at the time of graph break
            ctx3 = (smith.no_grad(), smith._dynamo.graph_break())[0]
            x = x + 64
            smith._dynamo.graph_break()
            with ctx1:
                check(False)
    ...
    def test_ctx_manager_nested_step_graph_break(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(f2)
        x = smith.zeros(3)
        res = f2(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
    ...
    @smith._dynamo.config.patch(recompile_limit=1, fail_on_recompile_limit_hit=True)
    ...
    def test_cells(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(outer)
        x = smith.zeros(3)
        res = outer(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
    ...
    def test_side_effects_cells(self):
        cell1, cell2, cell3, cell4 = (smith.zeros(3),) * 4
        ...
        cell1 = smith.zeros(3)
        cell2 = smith.zeros(3) + 4
        cell3 = smith.zeros(3)
        cell4 = smith.zeros(3) + 8
        res = f3()
    ...
    def test_side_effects_globals(self):
        ...
        global1 = smith.zeros(3)
        global2 = smith.zeros(3) + 4
        global3 = smith.zeros(3)
        global4 = smith.zeros(3) + 8
    ...
    def test_nested_graph_break_in_loop(self):
        ...
        opt_fn = smith._dynamo.optimize(backend=cnts, dynamic=True)(f5)
        x = smith.zeros(3)
        res = f5(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
        ...
        self.assertEqual(smith._dynamo.utils.counters["frames"]["total"], 6)
    ...
    def test_nested_graph_break_in_try_block(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(f5)
        x = smith.zeros(3)
        res = f5(x)
        ref = opt_fn(x)
        self.assertEqual(ref, res)
        ...
        self.assertEqual(smith._dynamo.utils.counters["frames"]["total"], 4)
    ...
    def test_nested_step_unsupported(self):
        ...
        # all ops except + 4
        self.assertEqual(cnts.op_count, 4)
        self.assertEqual(smith._dynamo.utils.counters["frames"]["total"], 3)
    ...
    def test_generator_nested_graph_break(self):
        ...
        cnts = smith._dynamo.testing.CompileCounter()
        opt_fn = smith._dynamo.optimize(backend=cnts)(fn)
        x = smith.zeros(3)
        res = fn(x)
    ...
    def test_return_after_graph_break_nested(self):
        ...
        def inner1(x):
            x += 1
            return smith._dynamo.graph_break()

        ref = f3(inner1, x)
        self.assertEqual(ref, smith.zeros(3) + 7)
        ...
        def inner2(x):
            x += 1
            return smith._dynamo.step_unsupported()
        ...
        new_inst = smith._dynamo.bytecode_transformation.create_instruction
        insts = [
        ...
        code_keys = smith._dynamo.bytecode_transformation.get_code_keys()
        ...
        x = smith.zeros(3)
        ref = f3(inner3, x)
        self.assertEqual(ref, smith.zeros(3) + 1006)
        # make sure we're actually STORE_ATTR graph breaking
        self.assertEqual(len(smith._dynamo.utils.counters["graph_break"]), 1)
        ...
        x = smith.zeros(3)
        ref = f4(x)
        self.assertEqual(ref, smith.zeros(3) + 15)
    ...
    def test_return_after_graph_break_deep_nested(self):
        ...
        x = smith.zeros(3)
        ref = f8(x)
        self.assertEqual(ref, smith.zeros(3) + 1023)

        # check that only 2 resume functions are created
        self.assertEqual(len(smith._dynamo.utils.counters["resumes"]), 2)
        for name in ("resume_in_f4", "resume_in_f7"):
            self.assertTrue(
                any(name in key for key in smith._dynamo.utils.counters["resumes"])
            )
    ...
    def test_disable_nested_graph_breaks(self):
        ...
        @smith._dynamo.disable_nested_graph_breaks
        def f3(x):
            return f2(x + 16) + 32
    ...
    def test_funcsmith_with_nested_graph_break(self):
        def f1(x):
            x = x * 2
            smith._dynamo.graph_break()
            return x * 4
        ...
        def f3(x):
            return smith.func.grad(f2)(x * 32) * 64
        ...
        x = smith.randn(3)
        actual = f4(x)
        expected = smith.compile(f4, backend=cnts, fullgraph=False)(x)
