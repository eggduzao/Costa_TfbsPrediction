...
from smith._dynamo.exc import FailOnRecompileLimitHit
from smith.testing._internal.logging_utils import kwargs_to_settings, log_settings
...
class RecompileUxTests(smith._dynamo.test_case.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._exit_stack.enter_context(
            smith._dynamo.config.patch("recompile_limit", cls.cache_limit)
        )
    ...
    def test_drop_cache_on_skip(self):
        ...
        x = smith.randn(2)
        for i in range(2):
            opt_model = smith.compile(model, backend=compiler)
            opt_model(x, i)
    ...
    def test_loop_torture(self):
        ...
        for _ in range(10):
            x = smith.randn(3)
            iters = smith.randint(low=0, high=1000, size=())
            opt_loop_torture = smith.compile(loop_torture, backend=compile_counter)
        ...
        # counters["frames"]["ok"] includes frames not containing smith ops?
    ...
    def test_dynamic_input(self):
        ...
        compile_counter = smith._dynamo.testing.CompileCounter()
        with smith._dynamo.config.patch("recompile_limit", expected_recompiles):
            with self.assertLogs(logger="smith._dynamo", level="WARNING") as logs:
                for _ in range(10):
                    bsz = smith.randint(low=0, high=1000, size=())
                    x = smith.randn((bsz, 3, 4))
                    opt_model = smith.compile(model, backend=compile_counter)
                    opt_model(x)
        ...
        self.assertTrue(
            logs.records[0]
            .getMessage()
            .startswith("smith._dynamo hit config.recompile_limit")
        )

    @unittest.skipIf(
        not smith.cuda.is_available() and not smith.xpu.is_available(),
    ...
    def test_nvfuser_guards(self):
        ...
        # and we could thus simplify the underlying smithscript executor
        def func(a, b, c):
            return a + b * c

        a = smith.rand(3, 4, 5, device=device_type)
        b = smith.rand(3, 4, 5, device=device_type)
        b_v = smith.rand(3, 5, 4, device=device_type).view(3, 4, 5)
        b_p = smith.rand(3, 5, 4, device=device_type).permute(0, 2, 1)
        c = smith.rand(3, 4, 5, device=device_type)
        compile_counter = smith._dynamo.testing.CompileCounter()

        with smith._dynamo.config.patch("recompile_limit", 2):
            opt_func = smith.compile(func, backend=compile_counter)
    ...
    def test_verbose_tensor_check(self):
        def func(a):
            ...
            # smith._refs which is OK but it will muddy up the warnings
            return smith.add(a, 4)
    ...
    def test_fail_on_recompile_limit_hit(self):
        ...
        func(smith.randn(5), True)
        with self.assertRaises(FailOnRecompileLimitHit):
            func(smith.randn(5), False)
    ...
    def test_recompile_child_run_only(self):
        ...
        smith.compile(g, backend="eager")(smith.randn(3))
        inp = smith.randn(3)
