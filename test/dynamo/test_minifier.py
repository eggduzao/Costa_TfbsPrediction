...
from smith.testing._internal.common_utils import skipIfNNModuleInlined


requires_gpu = unittest.skipUnless(
    smith.cuda.is_available() or smith.xpu.is_available(), "requires cuda or xpu"
)
...
class MinifierTests(MinifierTestBase):
    ...
    def _test_after_dynamo(self, device, backend, expected_error):
        run_code = f"""\
@smith.compile(backend={backend!r})
def inner(x):
    for _ in range(10):
        x = smith.sin(x)
    x = smith.relu(x)
...
"""
    ...
    def test_after_dynamo_non_leaf_compile_error(self):
        run_code = """\
@smith.compile(backend="non_leaf_compile_error_TESTING_ONLY")
def inner(x):
    return x + 1

inner(smith.randn(20, 20, requires_grad=True) + 1)
"""
        self._run_full_test(
    ...
    def test_cpu_cuda_module_after_dynamo(self, device):
        ...
        run_code = f"""\
class CpuCudaModule(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.m_x = smith.nn.Linear(20, 20).to(device)
        self.m_y = smith.nn.Linear(20, 20)
        self.p_x = smith.nn.Parameter(smith.randn(20, 20).to(device))
        self.p_y = smith.nn.Parameter(smith.randn(20, 20))
        self.b_x = smith.nn.Buffer(smith.ones(20, 20).to(device))
        self.b_y = smith.nn.Buffer(smith.ones(20, 20))
...
@smith.compile(backend={backend_name!r})
def inner(x1, y1):
    x2 = smith.randn(20, 20).to(device)
    y2 = smith.randn(20, 20)
    x3, y3 = mod(x1 + x2, y1 + y2)
    return smith.relu(x3.cpu() + y3)

inner(smith.randn(20, 20).to(device), smith.randn(20, 20))
"""
...
class Repro(smith.nn.Module):
    def __init__(self) -> None:
        ...
        self.register_buffer('G__mod___b_x', smith.randn([20, 20], dtype=smith.float32).to(device))
        self.register_buffer('G__mod___b_y', smith.randn([20, 20], dtype=smith.float32))
        self.G__mod___p_x = smith.nn.Parameter(smith.randn([20, 20], dtype=smith.float32, device=device))
        self.G__mod___p_y = smith.nn.Parameter(smith.randn([20, 20], dtype=smith.float32))

    def forward(self, L_x1_ : smith.Tensor, L_y1_ : smith.Tensor):
        l_x1_ = L_x1_
        l_y1_ = L_y1_
        randn = smith.randn(20, 20)
        ...
        relu = smith.relu(add_6);  add_6 = None
        return (relu,)""",
        )
...
class Repro(smith.nn.Module):
    ...
    def forward(self, x_19):
        x_20 = smith.relu(x_19);  x_19 = None
        return (x_20,)""",
        )
