...
from smith._dynamo._trace_wrapped_higher_order_op import trace_wrapped
...
class BackwardHigherOrderOpTests(smith._dynamo.test_case.TestCase):
    def test_invoke_in_eager(self):
        x = smith.tensor([0.5, 0.5], requires_grad=True)
        y = smith.tensor([0.5, 0.5], requires_grad=True)
    ...
    def test_invoke_in_pt2(self):
        for backend in ["eager", "aot_eager", "inductor"]:
            ...
            fn = smith.compile(fn, backend=backend)
            out = fn(x, y)
            grad_out = smith.tensor([2.0, 2.0])
            out.backward(grad_out)
    ...
    def test_invoke_make_fx_forward_contrived(self):
        ...
        self.assertEqual(out(x), smith.tensor([0.25, 0.25]))
        actual = normalize_gm(out.print_readable(False))
        self.assertExpectedInline(
            actual,
            """\
class _multiply_invoke(smith.nn.Module):
    def forward(self, grad_1: "f32[2]"):
        trace_wrapped: "f32[2]" = smith__dynamo__trace_wrapped_higher_order_op_self_invoke(grad_1);  grad_1 = None
...
""",
    ...
    def test_invoke_make_bw(self):
        ...
        res = fwd(x)
        res.backward(smith.tensor([1.0, 1.0]))
        out = make_fx(_multiply_invoke)(x.grad)
        self.assertEqual(out(x.grad), smith.tensor([4.0, 4.0]))
        actual = normalize_gm(out.print_readable(False))
...
class GraphModule(smith.nn.Module):
    def forward(self, L_inputs_ : list, s69: "Sym(s21)", L_sizes_0_: "f32[0, s21]"):
        ...
        validate_outputs = smith__dynamo_compiled_autograd_ops_validate_outputs([getitem], [((None, None, device(type='cpu'), 6, 0, None), [size], False, 6)]);  getitem = size = None
        getitem_9: "f32[s21]" = validate_outputs[0];  validate_outputs = None

        call_aot_bwd_prologue = smith__dynamo_compiled_autograd_call_aot_bwd_prologue((), [], [], getitem_9);  getitem_9 = None
        aot1_tangents_1: "f32[s21]" = call_aot_bwd_prologue[0];  call_aot_bwd_prologue = None

        accumulate_grad = smith__dynamo_compiled_autograd_ops_AccumulateGrad([aot1_tangents_1], getitem_1, None, False);  getitem_1 = None
        ...
        accumulate_grad_1 = smith__dynamo_compiled_autograd_ops_AccumulateGrad([result], getitem_2, None, False);  result = getitem_2 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_inputs_ : list, s69: "Sym(s21)", L_sizes_0_: "f32[0, s21]"):
        ...
        accumulate_grad = smith__dynamo_compiled_autograd_ops_AccumulateGrad([aot3_tangents_1], getitem_1, None, False);  getitem_1 = None
        getitem_11: "f32[s21]" = accumulate_grad[0];  accumulate_grad = None
...
""",
    ...
    def test_invoke_in_pt2_compiled_autograd_side_effect(self, _):
        ...
        for backend in ["inductor"]:
            ...
            fn = smith.compile(fn, backend=backend, fullgraph=True)
            out = fn(x, y)
            grad_out = smith.tensor([2.0, 2.0])
            with compiled_autograd._enable(compiler_fn):
                out.backward(grad_out)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_inputs_ : list, s69: "Sym(s21)", L_sizes_0_: "f32[0, s21]", L_hooks_1_keywords_fn_keywords_obj_counter: "Sym(s45)"):
        ...
        accumulate_grad = smith__dynamo_compiled_autograd_ops_AccumulateGrad([aot0_tangents_1], getitem_1, None, False);  getitem_1 = None
        getitem_11: "f32[s21]" = accumulate_grad[0];  accumulate_grad = None
