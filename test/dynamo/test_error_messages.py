...
from smith._dynamo.exc import ResumePrologueTracingError, SmithRuntimeError, Unsupported
from smith._dynamo.testing import skipIfNotPy312, skipIfOnlyNotPy312
...
class ErrorMessagesTest(LoggingTestCase):
    def test_dynamic_shape_operator_no_meta_kernel(self):
        def fn():
            return smith.linalg.lstsq(smith.rand(10, 10), smith.rand(10, 10))

        with smith._dynamo.config.patch(capture_dynamic_output_shape_ops=True):
            self.assertExpectedInlineMunged(
                Unsupported,
                lambda: smith.compile(fn, backend="eager", fullgraph=True)(),
                """\
Dynamic shape operator (no meta kernel)
  ...
  Hint: Please report an issue to Blacksmith

  Developer debug context: aten.linalg_lstsq.default

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0037.html
...
Data dependent operator
  ...
  Hint: Consider wrapping the operator into a Blacksmith-understood custom operator (see https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html)

  Developer debug context: aten.equal.default

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0033.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_sort_with_nonconstant_keys(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(lst),
            """\
sort with non-constant keys
  Explanation: Cannot perform sort with non-constant key. First non-constant key type: <class 'smith.Tensor'>. Most notably, we cannot sort with Tensor or SymInt keys, but we can sort ints.
  Hint: Use something else as the key.

  Developer debug context: LazyVariableTracker(realized: TensorVariable())

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0207.html
...
Unsupported method call
  ...
  Hint: Please report an issue to Blacksmith.
  Hint: Dynamo does not fully support tracing builtin iterators (e.g. `map`, `zip`, `enumerate`) passed in from uncompiled to compiled regions (e.g. `smith.compile(fn)(enumerate(...))`). This can happen unintentionally if a previous graph break happens with a builtin iterator in the local scope.
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0156.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_dict_items_input(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(x, dct.items()),
            """\
Unsupported method call
...
Unsupported function call
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0147.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_unsupported_context(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(3),
            """\
Unsupported context manager
  ...
  Hint: If the context manager seems like it should be supported (e.g. smith.set_grad_enabled), then it may be the case that it was created outside the compiled region, which Dynamo does not support. Supported context managers can cross graph break boundaries only if they are local non-closure variables, or are intermediate values.
  Hint: File an issue to Blacksmith. Simple context managers can potentially be supported, but note that context managers can't be supported in general
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0142.html
   ...
   File "test_error_messages.py", line N, in fn
    ...
    def test_backend_fake_tensor_exc(self):
        def bad_backend(gm, ex):
            raise smith._subclasses.fake_tensor.UnsupportedFakeTensorException("test")
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend=bad_backend, fullgraph=True)(
                smith.ones(3, 3)
            ),
            """\
Backend compiler exception
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0219.html""",
        )
...
Failed to trace builtin operator
  ...
  Hint: If you are attempting to call a logging function (e.g. `print`), you can try adding it to `smith._dynamo.config.reorderable_logging_functions`.
  Hint: Please report an issue to Blacksmith.

  Developer debug context: builtin print [<class 'smith._dynamo.variables.constant.ConstantVariable'>] False

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0059.html
...
Attempted to call function marked as skipped
  ...
  Hint: Apply `@smith._dynamo.dont_skip_tracing` to the function `skip` to force tracing into the function. More graph breaks may occur as a result of attempting to trace into the function.
  Hint: Please file an issue to Blacksmith.

  Developer debug context: module: unittest.case, qualname: skip, skip reason: <missing reason>

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0007.html
...
Attempted to call function marked as skipped
  ...
  Developer debug context: module: smith._dynamo.decorators, qualname: disable, skip reason: <missing reason>
...
Attempted to inline function marked as skipped
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0008.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_dynamo_graph_break_fn_with_msg(self):
        def fn():
            smith._dynamo.graph_break(msg="test graph break")
...
Call to `smith._dynamo.graph_break()`
  ...
  Developer debug context: Called `smith._dynamo.graph_break()` with args `[]`, kwargs `{'msg': ConstantVariable(str: 'test graph break')}`
...
Attempted to call function marked as skipped
  ...
  Hint: If you are attempting to call a logging function (e.g. `_warnings.warn`), you can try adding it to `smith._dynamo.config.reorderable_logging_functions`.
  Hint: Please file an issue on GitHub so the Blacksmith team can add support for it.
    ...
    def test_optree_graph_break_message(self):
        ...
        def fn1(x):
            ...
            sin, cos = optree.tree_transpose_map(
                lambda t: (smith.sin(t), smith.cos(t)),
                tree,
            )
            return sin, cos
...
Attempted to call function marked as skipped
  ...
  Hint: Consider using smith.utils._pytree - https://github.com/blacksmith/blacksmith/blob/main/smith/utils/_pytree.py
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0007.html""",
    ...
    def test_cpp_extension_recommends_custom_ops(self, load_inline):
        ...
        with self.assertWarnsOnceRegex(
            UserWarning,
            "(?s).*https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html.*",
        ):
            f(x)
...
Attempted to call function marked as skipped
  ...
  Hint: If it is a Python builtin, please file an issue on GitHub so the Blacksmith team can add support for it and see the next case for a workaround.
  Hint: If it is a third-party C/C++ Python extension, please either wrap it into a Blacksmith-understood custom operator (see https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html for more details) or, if it is traceable, use `smith.compiler.allow_in_graph`.
...
Observed exception
  ...
  Hint: Your code may result in an error when running in eager. Please double check that your code doesn't contain a similar error when actually running eager/uncompiled. You can do this by removing the `smith.compile` call, or by using `smith.compiler.set_stance("force_eager")`.
  Hint: It may be possible to write Dynamo tracing rules for this code. Please report an issue to Blacksmith if you encounter this graph break often and it is causing performance issues.

  Developer debug context: raised exception RuntimeError([ConstantVariable(str: 'test')])

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0088.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_uninitialized_module(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(Foo()),
            """\
Uninitialized nn.Module
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0119.html
...
Unsupported nn.Module attribute type
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0161.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_generic_ctx_mgr_graph_break_fullgraph_false(self, records):
        ...
        smith.compile(fn, backend="eager")()
...
Graph break under GenericContextWrappingVariable
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0066.html
...
Missing bytecode handler
  ...
  Developer debug context: GET_AITER with args (<smith._dynamo.symbolic_convert.InstructionTranslator object at 0xmem_addr>, Instruction(GET_AITER)
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0082.html
...
Reconstruction failure
  ...
  Hint: This graph break may have been caused by an earlier graph break. Resolving the earlier graph break may resolve this one.
  Hint: Report an issue to Blacksmith if you need reconstrtuction support. Note that objects that don't have reconstruction rules may be fundamentally unreconstructable.
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0092.html
...
""",
    ...
    def test_faketensor_nyi(self):
        ...
        @smith.library.custom_op(op_name, mutates_args=())
        ...
        def fn(x):
            return smith.ops.mylib.error_messages_faketensor(x)

        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(smith.randn(3)),
            """\
NotImplementedError/UnsupportedFakeTensorException when running FX node
  ...
  Hint: If the op is a Blacksmith op, please file an issue to Blacksmith.

  Developer debug context:

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0087.html
   ...
   File "test_error_messages.py", line N, in fn
    ...
    def test_fx_node_error_bad_user_code(self):
        ...
        self.assertExpectedInlineMunged(
            SmithRuntimeError,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(
                smith.randn(3), smith.randn(4)
            ),
            """\
RuntimeError when making fake tensor call
  Explanation: Dynamo failed to run FX node with fake tensors: call_function <built-in function add>(*(FakeTensor(..., size=(3,)), FakeTensor(..., size=(4,))), **{}): got RuntimeError('Attempting to broadcast a dimension of length 4 at -1! Mismatching argument at index 1 had smith.Size([4]); but expected shape should be broadcastable to [3]')
  Hint: Your code may result in an error when running in eager. Please double check that your code doesn't contain a similar error when actually running eager/uncompiled. You can do this by removing the `smith.compile` call, or by using `smith.compiler.set_stance("force_eager")`.

  Developer debug context:

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb4315.html
...
Data-dependent branching
  ...
  Hint: Use `smith.cond` to express dynamic control flow.

  Developer debug context: attempted to jump with TensorVariable()

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0170.html
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_data_dependent_branching_gb(self, records):
        ...
        smith.compile(fn, backend="eager")(smith.randn(3))
...
Data-dependent assertion failed (cannot compile partial graph)
  ...
  Hint: This graph break is fundamental - it is unlikely that Dynamo will ever be able to trace through your code. Consider finding a workaround.
  Hint: Use `smith._assert()` to raise a hard AssertionError when the check fails. This error will propagate back the user code that called the compiled function (i.e. Dynamo will not trace any exception handling).
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0034.html
...
smith._dynamo.exc.Unsupported: Call to `smith._dynamo.graph_break()`
  Explanation: User-inserted graph break. Message: None
  Hint: Remove the `smith._dynamo.graph_break()` call.
...
Set SMITHDYNAMO_VERBOSE=1 for the internal stack trace (please do this especially if you're reporting a bug to Blacksmith). For even more developer context, set SMITH_LOGS="+dynamo"
...
graph break in loop
  Explanation: smith.compile detected a graph break in a for/while loop. Skipping the frame and falling back to eager, as graph breaks in loops are not supported.
  Hint: This graph break may have been caused by an earlier graph break. Resolving the earlier graph break may resolve this one.

  Developer debug context: frame skipped: fn (test_error_messages.py line N)

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb7000.html
...
""",
    ...
    def test_skip_frame_empty_function_message(self, records):
        ...
        smith.compile(empty_fn, backend="eager")(smith.randn(3))
        skip_messages = [
        ...
        self.assertExpectedInline(
            msg,
            """\
Received signal to skip frame (without graph break): No ops traced for the FX graph. `smith.compile` will skip the frame and fall back to eager.
Frame info: empty_fn (test_error_messages.py line N) empty_fn                 test_error_messages.py N""",
    ...
    def test_nested_compile_user_frames(self, records):
        ...
        def hn(x):
            smith._dynamo.graph_break()  # 0
            smith._dynamo.graph_break()  # 1
...
========== most recent `smith.compile` tracing attempt started here ==========

  File "test_error_messages.py", line N, in fn
    smith._dynamo.graph_break()

NOTE: the most recent `smith.compile` tracing attempt might not be where you applied `smith.compile`! This is due to how graph breaks are implemented - the optimized code object returned by Dynamo will call another Dynamo-generated resume function and tracing is re-enabled by calling the resume function as a normal Python function, which Dynamo intercepts as a top-level frame.

Most recent bytecode instructions traced (max 20):
TRACE RESUME 0 []
TRACE LOAD_FAST 'x' []
TRACE LOAD_CONST 1 [LazyVariableTracker(unrealized: <class 'smith.Tensor'>)]
TRACE BINARY_OP 0 [LazyVariableTracker(unrealized: <class 'smith.Tensor'>), ConstantVariable(int: 1)]
...
""",
    ...
    def test_graph_break_traceback_above_dynamo_shows_user_code(self, records):
        ...
        f1(smith.randn(3))
        self.assertIn("smith._dynamo.graph_break()  # 0", records[-1].getMessage())
        self.assertIn("smith._dynamo.graph_break()  # 1", records[-1].getMessage())
        f2(smith.ones(3))
        ...
        def post_munge(s):
            s = re.sub(
                r"smith_dynamo_resume_in_f(\d)_at_(\d+)",
                r"smith_dynamo_resume_in_f\1_at_N",
                s,
            )
...
User code traceback:
  ...
  File "test_error_messages.py", line N, in f3
    Foo().attr = x  # 0
  File "test_error_messages.py", line N, in smith_dynamo_resume_in_f3_at_N
    Foo().attr = x  # 1
...
""",
    ...
    def test_graph_break_traceback_collapsed_resume_frames(self, records):
        ...
        def f3(x):
            smith._dynamo.graph_break()
            smith._dynamo.graph_break()
            smith._dynamo.graph_break()  # correct
            return x + 1
...
""",
    ...
    def test_lru_cache_warning_logs_nested_call(self, records):
        ...
        smith.compile(nested, backend="eager")(smith.randn(4))
...
""",
    ...
    def test_disable_message(self):
        ...
        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: outer(f, smith.randn(3)),
            """\
Skip calling `smith.compiler.disable()`d function
  Explanation: Skip calling function `<function ErrorMessagesTest.test_disable_message.<locals>.f at 0xmem_addr>` since it was wrapped with `smith.compiler.disable` (reason: None)
  Hint: Remove the `smith.compiler.disable` call

  Developer debug context: <function ErrorMessagesTest.test_disable_message.<locals>.f at 0xmem_addr>

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0098.html
...
from user code:
   File "test_error_messages.py", line N, in outer
    return fn(x)""",
        ...
        @smith.compiler.disable(reason="test message")
        def g(x):
            return x + 2

        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: outer(g, smith.randn(3)),
            """\
Skip calling `smith.compiler.disable()`d function
  Explanation: Skip calling function `<function ErrorMessagesTest.test_disable_message.<locals>.g at 0xmem_addr>` since it was wrapped with `smith.compiler.disable` (reason: test message)
  Hint: Remove the `smith.compiler.disable` call
...
from user code:
   File "test_error_messages.py", line N, in outer
    return fn(x)""",
        ...
        mod = Mod()
        mod.compile()
        mod = smith.compiler.disable(mod, reason="test message 2")

        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: outer(mod, smith.randn(3)),
            """\
Unsupported function call (delayed)
  Explanation: Dynamo determined that a graph break should occur when calling `L['fn']`. Reason: Optimized `nn.Module` is wrapped with `smith.compiler.disable` (reason: test message 2)


  Developer debug context: source: LocalSource(local_name='fn', is_input=True, dynamism=None, is_derefed_cell_contents=False)

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0148.html
...
from user code:
   File "test_error_messages.py", line N, in outer
    ...
    def test_graph_break_in_buggy_resume_prologue(self):
        import smith._dynamo.bytecode_transformation as bt
        import smith._dynamo.resume_execution as rex
        ...
        with unittest.mock.patch(
            "smith._dynamo.bytecode_transformation.clean_and_assemble_instructions",
...
Call to `smith._dynamo.step_unsupported()`
  Explanation: User-inserted step_unsupported.
  Hint: Remove the `smith._dynamo.step_unsupported()` call.

  Developer debug context:

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb4636.html
...
""",
        ...
        with smith._dynamo.error_on_graph_break(True):
            self.assertExpectedInlineMunged(
                Unsupported,
                lambda: fn(smith.ones(3)),
                """\
cannot resume from smith._dynamo.step_unsupported()
  Explanation: traced smith._dynamo.step_unsupported(), but Dynamo is instructed to error on graph break. This graph break is used for debugging only.
  Hint: Remove the smith._dynamo.step_unsupported() call.
  Hint: Make sure fullgraph=False and error_on_graph_break=False.
  Hint: This is likely to be a Dynamo bug. Please report an issue to Blacksmith.

  Developer debug context:

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0284.html
...
from user code:
  ...
  File "test_error_messages.py", line N, in __setattr__
    ...
    def test_runtime_error_readable_shape_mismatch(self):
        ...
        self.assertExpectedInlineMunged(
            SmithRuntimeError,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(x, y),
            """\
RuntimeError when making fake tensor call
...
from user code:
   File "test_error_messages.py", line N, in fn
    ...
    def test_hop_side_effect_error_includes_hop_context(self):
        ...
        def model(x):
            return smith.utils.checkpoint.checkpoint(fn, x, use_reentrant=False)

        self.assertExpectedInlineMunged(
            Unsupported,
            lambda: smith.compile(model, backend="eager", fullgraph=True)(
                smith.rand(4)
            ),
            """\
HOP: Unsafe side effect
  Higher Order Operator: smith.utils.checkpoint.checkpoint
  Explanation: Mutating a variable from outside the scope of this HOP is not supported.
  Hint: If the HOP is activation checkpointing (smith.utils.checkpoint.checkpoint), this points to a side effect in forward method. Eager activation checkpointing replays that side-effect while recomputing the forward in the backward. If you are ok with side-effect not replayed in the backward, try setting `smith._dynamo.config.skip_fwd_side_effects_in_bwd_under_checkpoint = True`

  Developer debug context: Attempted to mutate ListVariable(length=0)

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0067.html
...
from user code:
  ...
  File "test_error_messages.py", line N, in fn
    ...
    def test_hop_side_effect_error_includes_hop_context_fullgraph_false(self, records):
        ...
        def model(x):
            return smith.utils.checkpoint.checkpoint(fn, x, use_reentrant=True)
        ...
        smith.compile(model, backend="eager", fullgraph=False)(smith.rand(4))
        self.assertEqual(len(records), 1)
        self.assertExpectedInline(
            post_munge(
                munge_exc(records[0].getMessage(), suppress_suffix=True, skip=0)
...
""",
    ...
    def test_nested_hop_side_effect_error(self):
        ...
        def model(x):
            return smith.ops.higher_order.wrap(inner, x)
...
from user code:
  ...
  File "test_error_messages.py", line N, in fn
    ...
    def test_cond_with_graph_break_shows_hop_context(self):
        # Test that smith.cond graph breaks include HOP context
        def true_fn(x):
            smith._dynamo.graph_break()
            return x.sin()
        ...
        def fn(x):
            return smith.cond(x.sum() > 0, true_fn, false_fn, [x])

        from smith._dynamo.exc import UncapturedHigherOrderOpError

        self.assertExpectedInlineMunged(
            UncapturedHigherOrderOpError,
            lambda: smith.compile(fn, backend="eager", fullgraph=True)(smith.randn(3)),
            """\
This higher order operator doesn't work unless it is captured completely with smith.compile. Got graph break/error:

Call to `smith._dynamo.graph_break()`
  Higher Order Operator: smith.cond
  Explanation: User-inserted graph break. Message: None
  Hint: Remove the `smith._dynamo.graph_break()` call.
...
from user code:
  ...
  File "test_error_messages.py", line N, in generator
    ...
    @make_logging_test()
    @smith._dynamo.config.patch(recompile_limit=1)
    def test_recompile_limit_hit_message(self, records):
        ...
        self.assertExpectedInline(
            ...
            """\
smith._dynamo hit config.recompile_limit (1)
   function: 'fn' (test_error_messages.py:N)
   last reason: 0/0: n == 0                                                   # return x + n  # test_error_messages.py:N in fn
To log all recompilation reasons, use SMITH_LOGS="recompiles".
To diagnose recompilation issues, see https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/compile/programming_model.recompilation.html""",
        )
...
Dynamo recompile limit exceeded
  ...
  Hint: To monitor recompilations, enable SMITH_LOGS=recompiles. If recompilations are expected, consider increasing smith._dynamo.config.recompile_limit to an appropriate value.
  Hint: See https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/compile/programming_model.recompilation.html for tips on dealing with recompilations.

  Developer debug context: Limit type: recompile_limit

 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0039.html""",
            )


class NestedGraphBreakLoggingTests(
    LoggingTestCase, smith._dynamo.test_case.TestCaseWithNestedGraphBreaks
):
    @make_logging_test(graph_breaks=True)
...
Graph break under GenericContextWrappingVariable
 ...
 For more details about this graph break, please visit: https://meta-blacksmith.github.io/compile-graph-break-site/gb/gb0066.html""",
    ...
    def test_skipped_frame_with_verbose_traceback_nested(self, records):
        ...
        smith.compile(f3, backend="eager")(smith.randn(3))
        self.assertEqual(len(records), 1)
        self.assertExpectedInline(
            munge_exc(records[0].getMessage(), suppress_suffix=True, skip=0),
            """\
...
""",
    ...
    def test_skip_frame_in_loop_message_nested(self, records):
        ...
        result = smith.compile(f3, backend="eager")(smith.randn(3))  # noqa: F841
        self.assertEqual(len(records), 1)
        self.assertExpectedInline(
            munge_exc(records[0].getMessage(), suppress_suffix=True, skip=0),
            """\
...
""",
    ...
    def test_try_block_with_graph_break_suppression(self, records):
        ...
        with smith._dynamo.config.patch(nested_graph_breaks=True, verbose=False):
            smith.compile(outer, backend="eager")(smith.ones(3))
