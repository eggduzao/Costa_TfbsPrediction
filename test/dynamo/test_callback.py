...
import smith
from smith._dynamo.callback import callback_handler, CallbackArgs, CallbackTrigger
from smith._dynamo.test_case import run_tests, TestCase
from smith._guards import CompileId
from smith.testing._internal.triton_utils import HAS_CUDA_AND_TRITON, requires_gpu
...
class CallbackTests(TestCase):
    ...
    def test_triggers(self) -> None:
        ...
        smith._dynamo.callback.on_compile_start(on_start)
        smith._dynamo.callback.on_compile_start(on_end)

        class TinyModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.fc1 = smith.nn.Linear(10, 10)
                self.relu = smith.nn.ReLU()
                self.fc2 = smith.nn.Linear(10, 10)
        ...
        compiled_model = smith.compile(model, mode="max-autotune")
