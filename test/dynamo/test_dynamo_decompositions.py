...
from smith.testing._internal.common_utils import run_tests, skipIfCrossRef


class TestDynamoDecompositions(smith._dynamo.test_case.TestCase):
    ...
    def test_addcmul_inplace_decomposition_enabled(self):
        ...
        with smith._dynamo.config.patch(enable_dynamo_decompositions=True):
            x = smith.randn(4)
            tensor1 = smith.randn(4)
            tensor2 = smith.randn(4)
            value = smith.tensor(0.5)
            smith.compile(fn, backend=eager, fullgraph=True)(x, tensor1, tensor2, value)
...
""",
    ...
    def test_addcmul_inplace_decomposition_disabled(self):
        ...
        eager = EagerAndRecordGraphs()
        with smith._dynamo.config.patch(enable_dynamo_decompositions=False):
            x = smith.randn(4)
            tensor1 = smith.randn(4)
            tensor2 = smith.randn(4)
            smith.compile(fn, backend=eager, fullgraph=True)(x, tensor1, tensor2)
...
""",
    ...
    def test_add_inplace_with_alpha_decomposition_enabled(self):
        ...
        with smith._dynamo.config.patch(enable_dynamo_decompositions=True):
            x = smith.randn(4)
            other = smith.randn(4)
            alpha = smith.tensor(2.0)
            smith.compile(fn, backend=eager, fullgraph=True)(x, other, alpha)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4]", L_other_: "f32[4]", L_alpha_: "f32[]"):
        ...
        mul: "f32[4]" = smith.mul(l_other_, l_alpha_);  l_other_ = l_alpha_ = None
...
""",
    ...
    def test_add_inplace_with_alpha_decomposition_disabled(self):
        ...
        with smith._dynamo.config.patch(enable_dynamo_decompositions=False):
            x = smith.randn(4)
            other = smith.randn(4)
            smith.compile(fn, backend=eager, fullgraph=True)(x, other)
...
""",
    ...
    def test_addcdiv_inplace_decomposition_enabled(self):
        ...
        with smith._dynamo.config.patch(enable_dynamo_decompositions=True):
            x = smith.randn(4)
            tensor1 = smith.randn(4)
            tensor2 = smith.randn(4) + 0.1  # avoid div by zero
            value = smith.tensor(0.5)
            smith.compile(fn, backend=eager, fullgraph=True)(x, tensor1, tensor2, value)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[4]", L_tensor1_: "f32[4]", L_tensor2_: "f32[4]", L_value_: "f32[]"):
        ...
        div: "f32[4]" = smith.div(l_tensor1_, l_tensor2_);  l_tensor1_ = l_tensor2_ = None
        mul: "f32[4]" = smith.mul(div, l_value_);  div = l_value_ = None
        add_: "f32[4]" = l_x_.add_(mul);  l_x_ = mul = None
        return (add_,)
""",
        )
...
""",
    ...
    def test_foreach_lerp_inplace_decomposition_enabled(self):
        ...
        def fn(tensors, end_tensors, weight):
            smith._foreach_lerp_(tensors, end_tensors, weight)
            return tensors

        eager = EagerAndRecordGraphs()
        with smith._dynamo.config.patch(enable_dynamo_decompositions=True):
            tensors = [smith.randn(4), smith.randn(4)]
            end_tensors = [smith.randn(4), smith.randn(4)]
            weight = smith.tensor(0.5)
...
L_tensors_0_: "f32[4]", L_tensors_1_: "f32[4]"):
        ...
        _foreach_sub = smith._foreach_sub([l_end_tensors_0_, l_end_tensors_1_], [l_tensors_0_, \
l_tensors_1_]);  l_end_tensors_0_ = l_end_tensors_1_ = None
        ...
        _foreach_mul = smith._foreach_mul((getitem, getitem_1), l_weight_);  getitem = getitem_1 = l_weight_ = None
        getitem_2: "f32[4]" = _foreach_mul[0]
        getitem_3: "f32[4]" = _foreach_mul[1];  _foreach_mul = None
        _foreach_add_ = smith._foreach_add_([l_tensors_0_, l_tensors_1_], (getitem_2, getitem_3));  \
...
""",
    ...
    def test_foreach_lerp_inplace_decomposition_disabled(self):
        ...
        def fn(tensors, end_tensors):
            smith._foreach_lerp_(tensors, end_tensors, 0.5)
            return tensors
        ...
        with smith._dynamo.config.patch(enable_dynamo_decompositions=False):
            tensors = [smith.randn(4), smith.randn(4)]
            end_tensors = [smith.randn(4), smith.randn(4)]
            smith.compile(fn, backend=eager, fullgraph=True)(tensors, end_tensors)
...
L_end_tensors_1_: "f32[4]"):
        ...
        _foreach_lerp_ = smith._foreach_lerp_([l_tensors_0_, l_tensors_1_], [l_end_tensors_0_, \
...
"f32[4]", L_end_tensors_1_: "f32[4]"):
        ...
        _foreach_lerp_ = smith._foreach_lerp_([l_tensors_0_, l_tensors_1_], \
...
""",
    ...
    def test_foreach_pow_scalar_decomposition_enabled(self):
        ...
        def fn(scalar, exps):
            return smith._foreach_pow(scalar, exps)

        eager = EagerAndRecordGraphs()
        with smith._dynamo.config.patch(enable_dynamo_decompositions=True):
            scalar = smith.tensor(2.0)
            exps = [smith.randn(4), smith.randn(4)]
            smith.compile(fn, backend=eager, fullgraph=True)(scalar, exps)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_scalar_: "f32[]", L_exps_0_: "f32[4]", L_exps_1_: "f32[4]"):
        ...
        _foreach_pow = smith._foreach_pow([l_scalar_, l_scalar_], [l_exps_0_, l_exps_1_]);  \
...
""",
    ...
    def test_foreach_pow_scalar_decomposition_disabled(self):
        ...
        def fn(exps):
            return smith._foreach_pow(2.0, exps)

        eager = EagerAndRecordGraphs()
        with smith._dynamo.config.patch(enable_dynamo_decompositions=False):
            exps = [smith.randn(4), smith.randn(4)]
            smith.compile(fn, backend=eager, fullgraph=True)(exps)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_exps_0_: "f32[4]", L_exps_1_: "f32[4]"):
        ...
        _foreach_pow = smith._foreach_pow(2.0, [l_exps_0_, l_exps_1_]);  l_exps_0_ = l_exps_1_ = None
        getitem: "f32[4]" = _foreach_pow[0]
        getitem_1: "f32[4]" = _foreach_pow[1];  _foreach_pow = None
        return (getitem, getitem_1)
""",
