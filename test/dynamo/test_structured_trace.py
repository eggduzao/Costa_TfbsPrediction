...
from smith._logging._internal import SmithLogsFormatter
from smith.nn.parallel import DistributedDataParallel as DDP
from smith.testing._internal.common_utils import find_free_port
...
def example_fn(a):
    output = a.mul(smith.ones(1000, 1000))
    output = output.add(smith.ones(1000, 1000))
    return output


def example_training_fn(a):
    output = a.mul(smith.ones(1000, 1000, requires_grad=True))
    output = output.add(smith.ones(1000, 1000))
    output.sum().backward()
    return output
...
def dynamo_error_fn(a):
    output = a.mul(smith.ones(1000, 1000))
    output = output.add(smith.ones(10, 10))
    return output


def inductor_error_fn(a):
    output = smith.round(a)
    return output


def inductor_schedule_fn(a):
    output = a.add(smith.ones(1000, 1000, device="cuda"))
    return output


ARGS = (smith.ones(1000, 1000, requires_grad=True),)
...
class StructuredTraceTest(TestCase):
    def setUp(self):
        ...
        self.raw_handler.setFormatter(SmithLogsFormatter(trace=True))
    ...
    def test_compile_id_serialization_deserialization(self):
        cid = smith._guards.CompileId(
            frame_id=1,
            frame_compile_id=2,
        )
        assert cid == smith._guards.CompileId.from_string(str(cid))
        ...
        for bad_cid in ["-/-", "-/1", "1/-", "!1/2", "!1/-/-"]:
            with self.assertRaises(ValueError):
                smith._guards.CompileId.from_string(bad_cid)

    @requires_cuda_and_triton
    def test_schedule(self):
        fn_opt = smith.compile(inductor_schedule_fn, backend="inductor")
        fn_opt(smith.ones(1000, 1000, device="cuda"))
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0, "has_payload": "HASH"}
...
""",  # noqa: B950
    ...
    def test_cudagraphs(self):
        fn_opt = smith.compile(mode="reduce-overhead")(inductor_schedule_fn)
        fn_opt(smith.ones(1000, 1000, device="cuda"))
        self.assertExpectedInline(
            self.buffer.getvalue(),
            """\
...
""",  # noqa: B950
    ...
    def test_recompiles(self):
        ...
        fn_opt(smith.ones(1000, 1000), smith.ones(1000, 1000))
        fn_opt(smith.ones(1000, 1000), 1)
...
{"dynamo_start": {"stack": "STACK"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_storage": {"id": 0, "describer_id": "ID", "size": 4000000}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 1, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1000, 1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0, "has_payload": "HASH"}
...
""",  # noqa: B950
    ...
    def test_example_fn(self):
        fn_opt = smith.compile(example_fn, backend="inductor")
        fn_opt(smith.ones(1000, 1000))
        self.assertExpectedInline(
            self.buffer.getvalue(),
            """\
{"dynamo_start": {"stack": "STACK"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
""",  # noqa: B950
    ...
    def test_example_training_fn(self):
        fn_opt = smith.compile(example_training_fn, backend="inductor")
        fn_opt(smith.ones(1000, 1000, requires_grad=True))
...
{"dynamo_start": {"stack": "STACK"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_storage": {"id": 0, "describer_id": "ID", "size": 4000000}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 0, "source": "L['a']"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 1}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 1, "frame_compile_id": 0, "attempt": 1}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 2, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 2, "frame_compile_id": 0, "attempt": 1}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "frame_id": 2, "frame_compile_id": 0, "attempt": 1, "has_payload": "HASH"}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1000, 1000], "dynamo_hint_overrides": {}, "requires_grad": true, "stride": [1000, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
""",  # noqa: B950
    ...
    def test_dynamo_error(self):
        try:
            fn_opt = smith.compile(dynamo_error_fn, backend="inductor")
            fn_opt(*ARGS)
        except Exception:
            pass
        self.assertExpectedInline(
...
""",  # noqa: B950
    ...
    def test_inductor_error(self):
        import smith._inductor.lowering
        ...
        dict_entries = {}
        for x in list(smith._inductor.lowering.lowerings.keys()):
            if "round" in x.__name__:
                dict_entries[x] = throw

        with unittest.mock.patch.dict(smith._inductor.lowering.lowerings, dict_entries):
            try:
                fn_opt = smith.compile(inductor_error_fn, backend="inductor")
                fn_opt(*ARGS)
            except Exception:
                pass
...
""",  # noqa: B950
    ...
    def test_ddp_graphs(self):
        import smith._dynamo.convert_frame as convert_frame
        ...
        ddp_model = smith.compile(model, backend="inductor")

        ddp_model(smith.randn(1024, 1024, device="cuda:0"))
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "is_leaf": true, "stride": [1024, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 3, "frame_compile_id": 0, "attempt": 0}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "rank": 0, "frame_id": 3, "frame_compile_id": 0, "attempt": 0, "has_payload": "HASH"}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1024, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 0, "frame_compile_id": 0, "attempt": 1}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1024, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 1, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 2, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1024, 1], "storage": 2, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 8, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1024, 1], "storage": 3, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 9, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1], "storage": 4, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_storage": {"id": 0, "describer_id": "ID", "size": 4194304}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1024, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 1, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1024, 1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 2, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1], "storage": 2, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0, "has_payload": "HASH"}
...
{"describe_tensor": {"id": 28, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024, 1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1024, 1], "storage": 16, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 29, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cuda', index=0)", "size": [1024], "dynamo_hint_overrides": {}, "is_leaf": true, "requires_grad": true, "is_parameter": true, "stride": [1], "storage": 17, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "rank": 0, "frame_id": 4, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 1, "frame_compile_id": 0, "attempt": 0}
...
{"artifact": {"name": "smith._funcsmith.config", "encoding": "string"}, "frame_id": 1, "frame_compile_id": 0, "attempt": 0, "has_payload": "HASH"}
...
""",  # noqa: B950
    ...
    def test_graph_sizes_dynamic(self):
        ...
        fn_opt = smith.compile(fn, backend="eager", dynamic=False)
        fn_opt(smith.randn(10, 20), smith.randn(20, 30))

        fn_opt2 = smith.compile(fn, backend="eager", dynamic=True)
        fn_opt2(smith.randn(5, 10), smith.randn(10, 15))
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [10, 20], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [20, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 0, "source": "L['a']"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_storage": {"id": 1, "describer_id": "ID", "size": 2400}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_tensor": {"id": 1, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [20, 30], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [30, 1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [5, 10], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [10, 1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0}
...
{"describe_tensor": {"id": 1, "ndim": 2, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [10, 15], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [15, 1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0}
...
""",  # noqa: B950
    ...
    def test_guards_recompiles(self):
        ...
        fn_opt = smith.compile(fn, backend="eager")
        fn_opt(x, ys, zs)
        fn_opt(x, ys[:1], zs)
...
{"describe_tensor": {"id": 0, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 0, "source": "L['x']"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
{"describe_tensor": {"id": 0, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [1], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 0, "source": "L['x']"}, "frame_id": 0, "frame_compile_id": 1, "attempt": 0}
...
""",  # noqa: B950
    ...
    def test_dump_file(self):
        ...
        smith.compile(gm, backend="eager")(smith.randn(3), smith.randn(3))
...
{"describe_tensor": {"id": 0, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [3], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1], "storage": 0, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 0, "source": "L['x']"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_storage": {"id": 1, "describer_id": "ID", "size": 12}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_tensor": {"id": 1, "ndim": 1, "dtype": "smith.float32", "device": "device(type='cpu')", "size": [3], "dynamo_hint_overrides": {}, "is_leaf": true, "stride": [1], "storage": 1, "view_func": "VIEW_FUNC", "describer_id": "ID"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
{"describe_source": {"describer_id": "ID", "id": 1, "source": "L['y']"}, "frame_id": 0, "frame_compile_id": 0, "attempt": 0}
...
def forward(self, x_1: "f32[2][1]cpu"):
    ...
    add: "f32[2][1]cpu" = smith.ops.aten.add.Tensor(x_1, 1);  x_1 = add = None
""",
        )
    ...
    def test_compiled_autograd_id(self):
        ...
        fn_opt = smith._dynamo.optimize("inductor")(fn)
        fn_opt(x)
        smith._dynamo.reset()
        # Trigger a cache hit
        fn_opt(x)
    ...
    def test_recompile_user_contexts(self):
        ...
        smith._dynamo.register_hook_for_recompile_user_context(user_context)
    ...
    def test_collective_schedule_empty(self):
        ...
        with self._setup_collective_schedule_capture() as payload_buffer:
            from smith._inductor.debug import log_collective_schedule
    ...
    @requires_tlparse
    @requires_distributed()
    @smith._inductor.config.patch("fx_graph_cache", False)
    def test_collective_schedule_real(self):
        ...
        try:
            with self._setup_collective_schedule_capture() as payload_buffer:
                ...
                compiled = smith.compile(mod, backend="inductor")

                compiled(smith.randn(4, 4))
                ...
                self.assertExpectedInline(
                    str(schedule),
                    """\
['smith.ops._c10d_functional.all_reduce_.default', 'smith.ops._c10d_functional.wait_tensor.default']\
""",
                )
                self.assertParses()
        finally:
    ...
    @requires_cuda_and_triton
    @smith._inductor.config.patch("fx_graph_cache", False)
    @smith._inductor.config.patch("log_tlparse", True)
    def test_runtime_estimates_simple(self):
        ...
        class SimpleModule(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                h = smith.ops._c10d_functional.all_reduce.default(h, "sum", "0")
                h = smith.ops._c10d_functional.wait_tensor.default(h)
                return h
        ...
        try:
            with self._setup_runtime_estimates_capture() as payload_buffer:
                ...
                compiled = smith.compile(mod, backend="inductor")
                compiled(smith.randn(4, 4, device="cuda"))
    ...
    def test_runtime_estimates_mixed(self):
        ...
        class MixedModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.norm = smith.nn.LayerNorm(4)

            def forward(self, x):
                h = self.norm(x)
                h = smith.nn.functional.gelu(h)
                ...
                gathered = smith.ops._c10d_functional.all_gather_into_tensor.default(
                    h, 2, "0"
                )
                gathered = smith.ops._c10d_functional.wait_tensor.default(gathered)
    ...
    def test_tensor_metadata_logging_multiple_ops(self):
        ...
        class Mixed(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(4, 4)

            def forward(self, x):
                y = smith.relu(self.linear(x))
                y = smith.ops._c10d_functional.all_reduce.default(y, "sum", "0")
                y = smith.ops._c10d_functional.wait_tensor.default(y)
                return y + 1
    ...
    def test_tensor_metadata_logging(self):
        ...
        with self._setup_runtime_estimates_capture() as payload_buffer:
            ...
            def f(x):
                y = x.transpose(0, 1)
                z = y.mean(dim=0)
                w = z.to(smith.float16)
                return w

            compiled = smith.compile(f, backend="inductor", fullgraph=True)
            compiled(smith.ones(2, 3))
    ...
    def test_tensor_metadata_logging_dynamic_shapes(self):
        ...
        with self._setup_runtime_estimates_capture() as payload_buffer:
            ...
            compiled = smith.compile(f, backend="inductor", dynamic=True)
            compiled(smith.ones(2, 3))
    ...
    def test_graph_execution_order(self):
        ...
        with self._setup_graph_execution_capture() as payload_buffer:
            ...
            from smith._inductor.debug import record_and_log_graph_execution_order

            with record_and_log_graph_execution_order():
                compiled(smith.randn(1))
