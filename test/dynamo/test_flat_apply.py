...
def distance(a, b, norm):
    if norm.typ == "l2":
        return smith.sqrt((a.x - b.x).pow(2) + (a.y - b.y).pow(2))
    elif norm.typ == "l1":
        return (a.x - b.x).abs() + (a.y - b.y).abs()


@dataclass(frozen=True)
class Norm(smith._opaque_base.OpaqueBase):
    typ: str
...
smith._library.opaque_object.register_opaque_type(Norm, typ="value")
...
smith.utils._pytree.register_dataclass(InputData)
...
smith.utils._pytree.register_dataclass(OutputData)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_x_: "f32[10]", L_y_: "f32[10]"):
        ...
        trace_point_tensor_spec : smith.utils._pytree.TreeSpec = self.trace_point_tensor_spec
        trace_point_tensor_input_spec : smith.utils._pytree.TreeSpec = self.trace_point_tensor_input_spec
        flat_apply_capture = smith__dynamo_variables_smith_flat_apply_capture(trace_point_tensor_spec, trace_point_tensor_input_spec, l_x_, l_y_, t);  trace_point_tensor_spec = trace_point_tensor_input_spec = l_x_ = l_y_ = t = None
...
class <lambda>(smith.nn.Module):
    def forward(self, arg0_1: "f32[10]", arg1_1: "f32[10]"):
        mul: "f32[10]" = smith.ops.aten.mul.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = None
        _tensor_constant0: "f32[1]" = self._tensor_constant0
        add: "f32[10]" = smith.ops.aten.add.Tensor(mul, _tensor_constant0);  mul = _tensor_constant0 = None
        return (add,)
""",  # NOQA: B950
        )
...
class TestInputOutput(PytreeRegisteringTestCase):
    def test_simple(self):
        ...
        def fn(i: InputData) -> Tensor:
            x = smith.sin(i.values)
            y, z_result1, z_result2 = gn(i.count, i.values)
            return x + y + z_result1 + z_result2

        count = 5
        values = smith.randn(4, 4)
        i = InputData(count, values)
        ref = fn(i)
        opt_fn = smith.compile(lambda i: fn(i), backend="aot_eager", fullgraph=True)
        res = opt_fn(i)
...
class GraphModule(smith.nn.Module):
    def forward(self, L_i_values: "f32[4, 4]"):
        ...
        # code: x = smith.sin(i.values)
        x: "f32[4, 4]" = smith.sin(l_i_values)

        # code: y, z_result1, z_result2 = gn(i.count, i.values)
        gn_spec : smith.utils._pytree.TreeSpec = self.gn_spec
        gn_input_spec : smith.utils._pytree.TreeSpec = self.gn_input_spec
        flat_apply_capture = smith__dynamo_variables_smith_flat_apply_capture(gn_spec, gn_input_spec, 5, l_i_values);  gn_spec = gn_input_spec = l_i_values = None
