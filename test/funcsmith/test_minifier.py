...
from smith._funcsmith.compile_utils import get_outputs, get_placeholders
...
class TestMinifier(TestCase):
    def test_has_mul_minifier(self):
        ...
        inps = [smith.randn(3), smith.randn(3)]
        failing_f = make_fx(failing_f)(*inps)

        def has_mul(fx_g, inps):
            return smith.ops.aten.mul.Tensor in (i.target for i in fx_g.graph.nodes)
    ...
    def test_has_add_mul(self):
        ...
        def has_nans(fx_g, inps):
            ...
            for i in inps:
                if smith.isnan(i).any():
                    return False
            return smith.isnan(fx_g(*inps)[0]).any()
    ...
    def test_input_returned(self):
        ...
        inps = [smith.randn(3) for _ in range(3)]
    ...
    def test_tup_use(self):
        def f(a, b):
            tup = smith.std_mean(a)
            return (tup[0] + b * tup[1],)

        inps = [smith.randn(3), smith.randn(3)]

        def has_add(fx_g, inps):
            return smith.ops.aten.add.Tensor in (i.target for i in fx_g.graph.nodes)
    ...
    def test_module(self):
        ...
        mod = MockModule()
        failing_f = smith.fx.symbolic_trace(mod)
