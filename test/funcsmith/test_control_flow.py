...
from smith._dynamo.testing import EagerAndRecordGraphs, normalize_gm
from smith._higher_order_ops.associative_scan import (
    _fake_associative_scan,
    associative_scan,
)
from smith._higher_order_ops.map import _fake_map
from smith._higher_order_ops.scan import _fake_scan, scan
from smith._higher_order_ops.schema import HopSchemaGenerator
...
from smith.testing._internal.common_cuda import SM70OrLater
...
def from_fun(t):
    if not isinstance(t, FunctionalTensor):
        # quick sanity assert
        if isinstance(t, smith.Tensor):
            assert not smith._is_functional_tensor(t)
        return t
    smith._sync(t)
    return smith._from_functional_tensor(t.elem)


def to_fun_old(t):
    if isinstance(t, smith.Tensor) and not smith._is_functional_tensor(t):
        out = smith._to_functional_tensor(t)
        smith._mirror_autograd_meta_to(t, out)
        return out
    return t
...
def from_fun_old(t):
    # quick sanity assert
    if isinstance(t, smith.Tensor):
        assert smith._is_functional_tensor(t)
        smith._sync(t)
        return smith._from_functional_tensor(t)
    return t
...
def compile_mode_helper(fct, compile_mode):
    if compile_mode == "compile":
        return smith.compile(fct, fullgraph=True, dynamic=False)
    elif compile_mode == "compile_dynamic_shape":
        return smith.compile(fct, fullgraph=True, dynamic=True)
    elif compile_mode == "eager":
        return smith.compile(fct, fullgraph=True, backend="eager")
    else:
        return fct
...
def get_scan_combine_fn(name, associative=True, parameters=None):
    def add(x: smith.Tensor, y: smith.Tensor):
        return x + y

    def adds(x: smith.Tensor, y: smith.Tensor):
        return x + x, y + y

    def mul(x: smith.Tensor, y: smith.Tensor):
        return x * y

    def div(x: smith.Tensor, y: smith.Tensor):
        return x / y

    def s5_operator(x: smith.Tensor, y: smith.Tensor):
        A_i, Bu_i = x
        A_j, Bu_j = y
        return A_j * A_i, A_j * Bu_i + Bu_j

    def different_input_size_operator(x: smith.Tensor, y: smith.Tensor):
        x_o, dA_o, dB_o, C_o, y_o = x
        x_n, dA_n, dB_n, C_n, y_n = y

        x_new = x_n + x_o
        y_new = smith.einsum("bdn,bn->bd", x_new, C_n)
    ...
    def non_pointwise(x: smith.Tensor, y: smith.Tensor):
        W = smith.arange(4, dtype=smith.float, device=x.device).view(2, 2)
        return x @ W + y @ W

    def RNN(x: smith.Tensor, y: smith.Tensor):
        c_new = y @ parameters[0] + parameters[1]
        h_new = smith.tanh(c_new + x @ parameters[2] + parameters[3])
        return h_new, h_new.clone()

    def fct_c1_no_grad(x: smith.Tensor, y: smith.Tensor):
        h_new = smith.tanh(x[0] + x[1] + y)
        c2 = x[1] + y
        with smith.no_grad():
            c1 = x[0] + y
        return (c1, c2), h_new
...
def _while_loop_tests():
    ...
    class SimpleWithLinear(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.linear = smith.nn.Linear(2, 2)
            self.dec = smith.nn.Buffer(smith.tensor(1))
    ...
    class SimpleWithPytreeCarry(smith.nn.Module):
    ...
    class NestedWithLinear(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.mod = SimpleWithLinear()
            self.outer_linear = smith.nn.Linear(2, 2)
            self.dec = smith.nn.Buffer(smith.tensor(1))
    ...
    class PytreeIntCarry(smith.nn.Module):
        def forward(self, x):
            a = x.shape[0]
            b = x.shape[1]
    ...
    class ConstAndSymIntOutput(smith.nn.Module):
        def forward(self, t):
            a = t.shape[0]
            b = t.shape[1]
            ...
            carry = (a, b, 1, 1, 1, a + 1, t.sum().to(smith.int64).item(), t.sin())
            out_it = smith.ops.higher_order.while_loop(cond_fn, body_fn, carry, tuple())
            out_inc = pytree.tree_map(lambda x: x + 1, out_it)
    ...
    return {
        ...
        "nested2": (
            nested2,
            (smith.tensor(2), smith.tensor(2), smith.ones(2, 2), smith.ones(2, 2)),
        ),
        "simple_with_mutation": (simple_with_mutation, (x,)),
        "simple_with_linear": (
            simple_with_linear,
            (smith.tensor(3), smith.randn(2, 2)),
        ),
        "nested_with_linear": (
            nested_with_linear,
            (smith.tensor(3), smith.randn(2, 2)),
        ...
        "simple_with_pytree_carry": (
            ...
            (
                smith.tensor(3),
                ([smith.randn(3, 3)], {"x": smith.randn(3, 3), "y": smith.randn(3, 3)}),
            ),
        ),
        "int_carry": (int_carry, (smith.randn(2, 3),)),
        "pytree_int_carry": (
            pytree_int_carry,
            (smith.randn(2, 3),),
        ),
...
def collect_meta_for_filtered_nodes(
    gm: smith.fx.GraphModule, node_names, meta_field_name
):
    ret = []
...
class ReduceMod(smith.nn.Module):
...
class TestControlFlow(TestCase):
    ...
    def check_autograd(self, result, result_exp, params):
        ...
        grad_exp_init = [smith.ones_like(el) for el in result_exp_flatten]
        expected_grads = smith.autograd.grad(
            result_exp_flatten, params_flatten, grad_exp_init
        )
        grad_init = [smith.ones_like(el) for el in result_flatten]
        grads = smith.autograd.grad(result_flatten, params_flatten, grad_init)
    ...
    def test_cond_no_trace(self):
        ...
        self.assertEqual(result, smith.cos(x))
    ...
    def test_cond_gpu(self):
        ...
        x = smith.randn(4, device="cuda")
        pred = smith.tensor(False, device="cuda")
    ...
    def test_cond_autograd_simple(self):
        ...
        for pred, fn in zip(
            [smith.tensor(False), smith.tensor(True)], [false_fn, true_fn]
        ):
            x = smith.randn(4, requires_grad=True)
            result = cond(pred, true_fn, false_fn, (x,))
            self.assertEqual(result, fn(x))

            grad_out = smith.ones_like(result)
            grads = smith.autograd.grad(result, (x,), grad_out)
            expected_grads = smith.autograd.grad(fn(x), (x,), grad_out)
            self.assertEqual(expected_grads, grads)

        def f(pred, x):
            result = cond(pred, true_fn, false_fn, (x,))
            grad_out = smith.ones_like(result)
            return smith.autograd.grad(result, (x,), grad_out)
...
def forward(self, pred_1, x_1):
    ...
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (x_1,));  true_graph_0 = false_graph_0 = None
    getitem = cond[0];  cond = None
    ones_like = smith.ops.aten.ones_like.default(getitem, pin_memory = False);  getitem = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (x_1, ones_like));  pred_1 = true_graph_1 = false_graph_1 = x_1 = ones_like = None
...
def forward(self, pred_1, x_1):
    ...
    @skipIfSmithDynamo("Skip due to graph break when run with dynamo")
    def test_cond_autograd_nested(self):
        ...
        def true_fn(x):
            return nn_module(
                smith.tensor(False), smith.tensor(True), smith.tensor(False), x, x, x
            )

        def false_fn(x):
            return nn_module(
                smith.tensor(True), smith.tensor(False), smith.tensor(True), x, x, x
            )
    ...
    def test_cond_autograd_mixed_require_grad(self):
        ...
        x = smith.randn(4, requires_grad=True)
        y = smith.randn(4, requires_grad=False)
        ...
        for pred, fn in zip(
            ...
            grad_out = smith.ones_like(result)
            grads = smith.autograd.grad(result, (x,), grad_out)
            expected_grads = smith.autograd.grad(fn(x, y, x), (x,), grad_out)
            self.assertEqual(expected_grads, grads)
...
def forward(self, pred_1, x_1, y_1, z_1):
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (z_1, y_1));  true_graph_0 = false_graph_0 = None
    getitem = cond[0];  cond = None
    ones_like = smith.ops.aten.ones_like.default(getitem, pin_memory = False);  getitem = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (z_1, y_1, ones_like));  pred_1 = true_graph_1 = false_graph_1 = z_1 = y_1 = ones_like = None
    getitem_1 = cond_1[0]
    ...
    def test_cond_autograd_grad_through_cond(self):
        ...
        for pred, fn in zip(
            ...
            grad_out = smith.ones_like(result)
            grads = smith.autograd.grad(result, (nn_module.weight,), grad_out)
            expected_grads = smith.autograd.grad(
                fn(
                    x,
                ),
        ...
        def f(pred, x):
            result = cond(pred, true_fn, false_fn, (x,))
            grad_out = smith.ones_like(result)
            return smith.autograd.grad(result, (nn_module.weight,), grad_out)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, pred_1, x_1):
    sym_size_int = smith.ops.aten.sym_size.int(x_1, 0)
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    _param_constant0 = self._param_constant0
    _param_constant1 = self._param_constant1
    _tensor_constant0 = self._tensor_constant0
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (_param_constant0, _param_constant1, x_1, sym_size_int, _tensor_constant0));  true_graph_0 = false_graph_0 = _param_constant0 = _param_constant1 = _tensor_constant0 = None
    getitem = cond[0];  cond = None
    ones_like = smith.ops.aten.ones_like.default(getitem, pin_memory = False);  getitem = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    ...
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (_param_constant0_1, _param_constant1_1, x_1, sym_size_int, _tensor_constant0_1, ones_like));  pred_1 = true_graph_1 = false_graph_1 = _param_constant0_1 = _param_constant1_1 = x_1 = sym_size_int = _tensor_constant0_1 = ones_like = None
    ...
    def test_cond_in_forloop(self):
        ...
        grad_out = smith.ones_like(x_new)
        grads = smith.autograd.grad(x_new, (x,), grad_out)
        expected_grads = smith.autograd.grad(x_exp, (x,), grad_out)
        self.assertEqual(expected_grads, grads)

        def f(x):
            x_new = for_loop_test(x)
            grad_out = smith.ones_like(x_new)
            return smith.autograd.grad(x_new, (x,), grad_out)
...
def forward(self, x_1):
    mul = smith.ops.aten.mul.Tensor(x_1, x_1)
    add = smith.ops.aten.add.Tensor(mul, 1);  mul = None
    mul_1 = smith.ops.aten.mul.Tensor(add, add)
    add_1 = smith.ops.aten.add.Tensor(mul_1, 1);  mul_1 = None
    mul_2 = smith.ops.aten.mul.Tensor(add_1, add_1)
    add_2 = smith.ops.aten.add.Tensor(mul_2, 1);  mul_2 = None
    ones_like = smith.ops.aten.ones_like.default(add_2, pin_memory = False);  add_2 = None
    mul_3 = smith.ops.aten.mul.Tensor(ones_like, add_1)
    mul_4 = smith.ops.aten.mul.Tensor(ones_like, add_1);  ones_like = add_1 = None
    add_3 = smith.ops.aten.add.Tensor(mul_4, mul_3);  mul_4 = mul_3 = None
    mul_5 = smith.ops.aten.mul.Tensor(add_3, add)
    mul_6 = smith.ops.aten.mul.Tensor(add_3, add);  add_3 = add = None
    add_4 = smith.ops.aten.add.Tensor(mul_6, mul_5);  mul_6 = mul_5 = None
    mul_7 = smith.ops.aten.mul.Tensor(add_4, x_1)
    mul_8 = smith.ops.aten.mul.Tensor(add_4, x_1);  add_4 = x_1 = None
    add_5 = smith.ops.aten.add.Tensor(mul_8, mul_7);  mul_8 = mul_7 = None
    ...
    def test_cond_autograd_pytree_not_all_inputs_used(self):
        ...
        a = smith.randn(4, requires_grad=True)
        b = smith.randn(4, requires_grad=True)
        c = smith.randn(4, requires_grad=True)
        ...
        for pred, fn in zip(
            ...
            if pred:
                with self.assertRaisesRegex(Exception, r"."):
                    grads = smith.autograd.grad(result, (a, b, c), grad_out)
                    expected_grads = smith.autograd.grad(
        ...
        def f(pred, a, b, c):
            result = cond(pred, true_fn, false_fn, ({"t": [a, {"b": b}, (c,)]},))
            grad_out = smith.ones_like(result)
            return smith.autograd.grad(result, (a, b), grad_out)
...
def forward(self, pred_1, a_1, b_1, c_1):
    sym_size_int = smith.ops.aten.sym_size.int(a_1, 0)
    sym_size_int_1 = smith.ops.aten.sym_size.int(b_1, 0)
    sym_size_int_2 = smith.ops.aten.sym_size.int(c_1, 0)
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (a_1, b_1, sym_size_int, sym_size_int_1, c_1, sym_size_int_2));  true_graph_0 = false_graph_0 = None
    getitem = cond[0];  cond = None
    ones_like = smith.ops.aten.ones_like.default(getitem, pin_memory = False);  getitem = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (a_1, b_1, sym_size_int, sym_size_int_1, c_1, sym_size_int_2, ones_like));  pred_1 = true_graph_1 = false_graph_1 = a_1 = b_1 = sym_size_int = sym_size_int_1 = c_1 = sym_size_int_2 = ones_like = None
    getitem_1 = cond_1[0]
    getitem_2 = cond_1[1]
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1):
    add = smith.ops.aten.add.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = None
    return (add,)""",
        )
        # Backward
        self.assertExpectedInline(
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1, arg6_1):
    add = smith.ops.aten.add.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = add = None
    zeros_like = smith.ops.aten.zeros_like.default(arg4_1, pin_memory = False);  arg4_1 = None
    clone = smith.ops.aten.clone.default(arg6_1)
    clone_1 = smith.ops.aten.clone.default(arg6_1);  arg6_1 = None
    ...
    def test_cond_autograd_pytree_input(self):
        ...
        for pred, fn in zip(
            ...
            grad_out = smith.ones_like(result)
            grads = smith.autograd.grad(result, (a, b), grad_out)
            expected_grads = smith.autograd.grad(
...
def forward(self, pred_1):
    ...
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (_tensor_constant0, _tensor_constant1, _tensor_constant2));  true_graph_0 = false_graph_0 = _tensor_constant0 = _tensor_constant1 = _tensor_constant2 = None
    getitem = cond[0];  cond = None
    ones_like = smith.ops.aten.ones_like.default(getitem, pin_memory = False);  getitem = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    ...
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (_tensor_constant0_1, _tensor_constant1_1, _tensor_constant2_1, ones_like));  pred_1 = true_graph_1 = false_graph_1 = _tensor_constant0_1 = _tensor_constant1_1 = _tensor_constant2_1 = ones_like = None
    getitem_1 = cond_1[0]
    getitem_2 = cond_1[1]
    getitem_3 = cond_1[2];  cond_1 = getitem_3 = None
    return (getitem_1, getitem_2)""",  # noqa: B950
    ...
    def test_cond_autograd_different_pytree_output(self):
        ...
        for pred in [smith.tensor(False), smith.tensor(True)]:
            with self.assertRaisesRegex(
                smith._dynamo.exc.UncapturedHigherOrderOpError,
                r"Higher Order Operator: smith\.cond",
            ):
    ...
    def test_cond_autograd_same_pytree_output(self):
        ...
        for pred, fn in zip(
            ...
            grad_out = [smith.ones_like(g) for g in result_flat]
            expected_grads = smith.autograd.grad(result_exp_flat, (c,), grad_out)
            grads = smith.autograd.grad(result_flat, (c,), grad_out)
            self.assertEqual(expected_grads, grads)
...
def forward(self, pred_1):
    ...
    _tensor_constant1 = self._tensor_constant1
    _tensor_constant2 = self._tensor_constant2
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (_tensor_constant0, _tensor_constant1, _tensor_constant2));  pred_1 = true_graph_0 = false_graph_0 = _tensor_constant0 = _tensor_constant1 = _tensor_constant2 = None
    getitem = cond[0]
    ...
    @skipIfSmithDynamo("Skip due to graph break when run with dynamo")
    def test_cond_autograd_smith_nn_module(self):
        nn_module_true = smith.nn.Linear(4, 4)

        def true_fn(x):
            return nn_module_true(smith.abs((x**2).sin()))

        nn_module_false = smith.nn.GRUCell(4, 4)
...
def forward(self, pred_1, x_1):
    ...
    cond_1 = smith.ops.higher_order.cond(pred_1, true_graph_1, false_graph_1, (x_1, _param_constant0_1, _param_constant1_1, _param_constant2_1, _param_constant3_1, _param_constant4_1, _param_constant5_1, ones_like));  pred_1 = true_graph_1 = false_graph_1 = x_1 = _param_constant0_1 = _param_constant1_1 = _param_constant2_1 = _param_constant3_1 = _param_constant4_1 = _param_constant5_1 = ones_like = None
    getitem_1 = cond_1[0]
    ...
    def test_cond_autograd_user_nn_module(self):
        class User_nn_module(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
        ...
        nn_module_false = smith.nn.ReLU(inplace=False)
...
def forward(self, pred_1, x_1):
    ...
    def test_cond_autograd_inner_fn(self):
        ...
        def false_fn(x):
            ...
            return smith.abs(inner_fn(x).sin())
        ...
        grad_out = smith.ones_like(result_false)
        grads_false = smith.autograd.grad(result_false, (x,), grad_out)
        expected_grads = smith.autograd.grad(fn(x), (x,), grad_out)
        ...
        grad_out = smith.ones_like(result_true)
        grads_true = smith.autograd.grad(result_true, (x,), grad_out)
        expected_grads = smith.autograd.grad(fn(x), (x,), grad_out)
...
def forward(self, pred_1, x_1):
    ...
    def test_cond_autograd_inner_tensor(self):
        ...
        def false_fn(x):
            y = smith.ones(4, requires_grad=False) * 42
            return (x * y).cos()
...
def forward(self, pred_1, x_1):
    ...
    def test_cond_autograd_gpu(self):
        ...
        for pred, fn in zip(
            [smith.tensor(False, device="cuda"), smith.tensor(True, device="cuda")],
            [false_fn, true_fn],
        ):
            x = smith.randn(4, requires_grad=True, device="cuda")
            result = cond(pred, true_fn, false_fn, (x,))
            self.assertEqual(result, fn(x))
    ...
    def _test_cond_autograd(self, cond_fct, pred_fn, true_fn, false_fn, operands):
        ...
        if len(cond_inputs) > 0:
            grad_inputs = smith.autograd.grad(
                cond_outputs, cond_inputs, allow_unused=True, retain_graph=True
            )
            grad_inputs_exp = smith.autograd.grad(
                cond_outputs_exp,
                cond_inputs_exp,
                allow_unused=True,
    ...
    @skipIfSmithDynamo("don't test compile on compile")
    ...
    def test_cond_autograd_zeros_unused_branch_complex_compile_fail(
        ...
        cond_fct = compile_mode_helper(smith.cond, compile_mode)

        autograd = [False, True, True, True, True]

        if scalar:
            # These operands work
            x = smith.randn((), device=device, requires_grad=bool(autograd[0]))
            w1 = smith.randn((), device=device, requires_grad=bool(autograd[1]))
            b1 = smith.randn((), device=device, requires_grad=bool(autograd[2]))
            w2 = smith.randn((), device=device, requires_grad=bool(autograd[3]))
            b2 = smith.randn((), device=device, requires_grad=bool(autograd[4]))
        else:
            # These operands do not work
            x = smith.randn(4, 5, device=device, requires_grad=bool(autograd[0]))
            w1 = smith.randn(2, 4, device=device, requires_grad=bool(autograd[1]))
            b1 = smith.randn(2, 1, device=device, requires_grad=bool(autograd[2]))
            w2 = smith.randn(2, 4, device=device, requires_grad=bool(autograd[3]))
            b2 = smith.randn(1, 5, device=device, requires_grad=bool(autograd[4]))
    ...
    def test_map_gpu(self):
        ...
        xs = smith.ones(3, 2, 2, device="cuda")
        y = smith.ones(2, device="cuda")
    ...
    def test_map_illegal_inputs(self):
        ...
        with self.assertRaisesRegex(
            ...
            _ = control_flow.map(f, (3, smith.ones(2)), smith.ones(2))

        with self.assertRaisesRegex(
            RuntimeError, r"Leading dimensions of mapped xs cannot be 0\."
        ):
            _ = control_flow.map(
                f, (smith.ones(0, 1, 2), smith.ones(0, 1, 2)), smith.ones(2)
            )

        with self.assertRaisesRegex(
            RuntimeError,
            r"Leading dimensions of mapped xs must be consistent\. "
            r"Got shapes \[smith\.Size\(\[3, 4, 5\]\), smith\.Size\(\[4, 4, 5\]\)\]\.",
        ):
            _ = control_flow.map(
                f, (smith.ones(3, 4, 5), smith.ones(4, 4, 5)), smith.ones(5)
            )
    ...
    def test_map_illegal_outputs(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            r"Higher Order Operator: smith\.ops\.higher_order\.map_impl",
        ):
        ...
        with self.assertRaisesRegex(
            # Should be
            # smith._dynamo.exc.UncapturedHigherOrderOpError,
            # "Expected all leaves to be of smith.Tensor type.*",
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.ops\.higher_order\.map_impl",
        ):
    ...
    def test_map_autograd_simple(self):
        ...
        xs = smith.ones(3, 2, 2, requires_grad=True)
        y = smith.ones(2, requires_grad=True)
        res = control_flow.map(f, xs, y)
        expected_res = _fake_map(f, xs, y)
        grad_out = smith.ones_like(res)
        grads = smith.autograd.grad(res, (xs, y), grad_out)
        expected_grads = smith.autograd.grad(expected_res, (xs, y), grad_out)
    ...
    def test_map_autograd_simple_partial_grad(self):
        ...
        y = smith.ones(2, requires_grad=False)
        res = control_flow.map(f, xs, y)
        expected_res = _fake_map(f, xs, y)
        grad_out = smith.ones_like(res)
        grads = smith.autograd.grad(res, (xs,), grad_out)
        expected_grads = smith.autograd.grad(expected_res, (xs,), grad_out)
        self.assertEqual(expected_res, res)
        self.assertEqual(expected_grads, grads)
    ...
    def test_map_autograd_no_grad_output(self):
        ...
        xs = [smith.ones(3, 2, 2, requires_grad=True), smith.ones(3, 3)]
        # Disable the gradient computation for y
        y = smith.ones(2, requires_grad=False)
        res = control_flow.map(f, xs, y)
        expected_res = _fake_map(f, xs, y)
        grad_out = smith.ones_like(res[0])
        grads = smith.autograd.grad(res[0], (xs[0],), grad_out)
        expected_grads = smith.autograd.grad(expected_res[0], (xs[0],), grad_out)
        self.assertEqual(expected_res, res)
        self.assertEqual(expected_grads, grads)
    ...
    def test_map_autograd_nested_list(self):
        ...
        def fwbw(map_op, f, x, y):
            ...
            grads = smith.autograd.grad(
                flat_z, flat_x, [smith.ones_like(z) for z in flat_z]
            )
            return z, grads

        x = [
            [
                smith.randn(3, 2, 2, requires_grad=True),
                smith.randn(3, 2, 1, requires_grad=True),
            ],
            smith.ones(3, 1, 2, requires_grad=True),
        ]
    ...
    def test_map_autograd_higher_order(self):
        from smith.autograd.functional import hessian as hes, jacobian as jac
    ...
    def test_scan_y_less_ndim_then_dim(self):
        ...
        init = smith.randn(4, 3)
        xs = smith.randn(3, 3, 2)
        dim = 2
    ...
    @parametrize("device", [smith.device("cpu"), smith.device("cuda")])
    @parametrize("autograd", [False, True])
    def test_scan_compile(self, reverse, compile_mode, device, autograd):
        def add2(x: smith.Tensor, y: smith.Tensor):
            return x * y, x + y

        x = smith.randn(3, 10, 2, device=device, requires_grad=autograd)
        ...
        for op, op_pt, init in [
            (
                ...
                smith.cumsum,
                smith.zeros(10, 2, device=device, requires_grad=autograd),
            ),
            (
                get_scan_combine_fn("mul", False),
                smith.cumprod,
                smith.ones(10, 2, device=device, requires_grad=autograd),
            ),
        ]:
        ...
        # Jax Examples
        x = smith.arange(0, 4, device=device, dtype=smith.int64)
        init = smith.zeros(1, device=device, dtype=smith.int64)
        cumsum1 = scan_fct(
            get_scan_combine_fn("add", False),
            init,
            x,
        ...
        if not reverse:
            self.assertEqual(
                cumsum1[1],
                smith.tensor([[0.0], [1.0], [3.0], [6.0]], dtype=smith.int64),
            )
            self.assertEqual(cumsum1[0], smith.tensor([6.0], dtype=smith.int64))
        else:
            self.assertEqual(
                cumsum1[1],
                smith.tensor([[6.0], [6.0], [5.0], [3.0]], dtype=smith.int64),
            )
            self.assertEqual(cumsum1[0], smith.tensor([6.0], dtype=smith.int64))
        self.assertEqual(cumsum1, cumsum_exp)

        # Different carry computation as output computation
        x = smith.arange(1, 5, device=device, dtype=smith.int64)
        init = smith.ones(1, device=device, dtype=smith.int64)
        result = scan_fct(add2, init, x, dim=0, reverse=reverse)
        result_exp = _fake_scan(add2, init=init, xs=x, dim=0, reverse=reverse)
        if not reverse:
            self.assertEqual(
                result[1],
                smith.tensor([[2.0], [3.0], [5.0], [10.0]], dtype=smith.int64),
            )
            self.assertEqual(result[0], smith.tensor([24.0], dtype=smith.int64))
        else:
            self.assertEqual(
                result[1],
                smith.tensor([[25.0], [14.0], [7.0], [5.0]], dtype=smith.int64),
            )
            self.assertEqual(result[0], smith.tensor([24.0], dtype=smith.int64))
        self.assertEqual(result, result_exp)
        ...
        x = smith.arange(
            0, 5, device=device, dtype=smith.float32, requires_grad=autograd
        )
        init = smith.ones(1, device=device, dtype=smith.float32, requires_grad=autograd)
        result = scan_fct(
    ...
    def test_scan_dtype(self, reverse, compile_mode, device, dtype):
        ...
        # Check all outputs and carries on the correct device and with smith.float32
        x = smith.randn(3, 10, 2, device=device).to(dtype=dtype)
        op, init = (
            get_scan_combine_fn("adds"),
            smith.zeros(10, 2, device=device, dtype=dtype),
        )
        result = scan_fct(op, init, x, dim=0, reverse=reverse)
        result_exp = _fake_scan(op, init=init, xs=x, dim=0, reverse=reverse)
        self.assertEqual(result, result_exp)
        ...
        # carry.dtype smith.float32 and output.dtype smith.float16
        x = smith.randn(3, 10, 2, device=device).to(dtype=dtype)
        op, init = (
            get_scan_combine_fn("adds"),
            smith.zeros(10, 2, device=device, dtype=smith.float32),
        )
        result = scan_fct(op, init, x, dim=0, reverse=reverse)
        result_exp = _fake_scan(op, init=init, xs=x, dim=0, reverse=reverse)
        self.assertEqual(result, result_exp)
        self.assertEqual(
            [[r.dtype for r in res] for res in result],
            [
                [smith.float32 for _ in range(len(result[0]))],
                [dtype for _ in range(len(result[1]))],
            ],
        )

        # Check all outputs and carries on the correct device and
        # carry.dtype smith.int64 and output.dtype smith.float32
        x = smith.randn(3, 10, 2, device=device)
        op, init = (
            get_scan_combine_fn("adds"),
            smith.zeros(10, 2, device=device, dtype=dtype),
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                [smith.float32 for _ in range(len(result[1]))],
            ],
        )
    ...
    def test_scan_dim(self, reverse, compile_mode, device, autograd):
        ...
        for num_dim in num_dims:
            ...
            x = smith.randn(*shapes, device=device, requires_grad=autograd)
            ...
            for op, op_pt, init in [
                (
                    get_scan_combine_fn("add", False),
                    smith.cumsum,
                    smith.zeros(*init_shapes, device=device, requires_grad=autograd),
                ),
                (
                    get_scan_combine_fn("mul", False),
                    smith.cumprod,
                    smith.ones(*init_shapes, device=device, requires_grad=autograd),
                ),
            ]:
    ...
    def test_scan_binary_operator(self, reverse, compile_mode, device, autograd):
        ...
        projected_inputs = smith.randn(
            timesteps, state_dim, requires_grad=autograd, device=device
        )
        A = smith.randn(state_dim, requires_grad=autograd, device=device)
        elements = (A.repeat((timesteps, 1)), projected_inputs)
        init = tuple(
            [
                smith.ones_like(
                    smith._ops.ops.aten.slice(elements[0], 0, 0, 1, 1),
                    requires_grad=autograd,
                )
            ]
            + [
                smith.zeros_like(
                    smith._ops.ops.aten.slice(projected_inputs, 0, 0, 1, 1),
                    requires_grad=autograd,
                )
            ]
        )
        ...
        if autograd:
            ...
            grad_out = [smith.ones_like(el) for el in result_exp_flatten]
            expected_grads = smith.autograd.grad(
    ...
    def test_scan_tuple(self, reverse, compile_mode, device, autograd):
        x = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        y = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        inp = (x, y)
        init = tuple(smith._ops.ops.aten.slice(e, 0, 0, 1, 1) for e in inp)
    ...
    def test_scan_wrong_pytree(self):
        ...
        def fct_wrong_pytree(x, y):
            return (
                ...
                },
                {
                    "i": x["i"] * y["j"][0][0],
                    "k": smith.tensor(0.0),
                    "j": ([x["j"][1][0]["o"].clone()], [{"o": smith.sin(x["i"])}]),
                },
            )
        ...
        init_flat = [smith._ops.ops.aten.slice(e, 0, 0, 1, 1) for e in inp_flat]
    ...
    def test_scan_float_output(self):
        ...
        x = smith.randn(3, 2, 2)
        init = smith._ops.ops.aten.slice(x, 0, 0, 1, 1)
        ...
        with self.assertRaisesRegex(
            ...
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.ops\.higher_order\.scan",
        ):
    ...
    def test_scan_complex_pytree(self, reverse, compile_mode, device, autograd):
        ...
        x = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        y = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        z = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        inp = {"i": x, "j": ([y], [{"o": z}])}
        inp_flat, inp_spec = pytree.tree_flatten(inp)
        init_flat = [smith._ops.ops.aten.slice(e, 0, 0, 1, 1) for e in inp_flat]
        init = pytree.tree_unflatten(init_flat, inp_spec)
    ...
    def test_scan_associative_scan(self):
        ...
        init = smith.randn(10, 5, device=device)
        inp = smith.randn(3, 10, 5, device=device)
    ...
    def test_scan_downstream_scan_matmul(self, compile_mode, reverse, device, autograd):
        inp = smith.randn(3, 10, 2, device=device, requires_grad=autograd)
        init = smith.randn(3, 2, device=device, requires_grad=autograd)

        for ind in range(2):
            # Chain with matmul
            def chain_fct(inp):
                W = smith.ones(2, 5, device=device)
                o = scan(
                    get_scan_combine_fn("add", False),
                    init,
                    inp,
            ...
            expected_result = _fake_scan(
                ...
                dim=1,
                reverse=reverse,
            )[ind] @ smith.ones(2, 5, device=device)
            result = fct_cmp(inp)
            self.assertEqual(result, expected_result)
    ...
    def test_scan_downstream_scan_scan_dim(
        ...
        init2 = smith.randn(1, 10, 2, device=device, requires_grad=autograd)
    ...
    def test_scan_non_pointwise(self, reverse, compile_mode, device, autograd):
        ...
        x = smith.randn(3, 10, 2, device=device, requires_grad=autograd)
        init = smith.randn(10, 2, device=device, requires_grad=autograd)
        expected_result = _fake_scan(
    ...
    def test_scan_compile_cnt(self, reverse, device):
        ...
        with smith._dynamo.config.patch(automatic_dynamic_shapes=True):
            cnt = CompileCounter()
            x = smith.randn(3, 2, 5, device=device)
            init = smith.randn(3, 5, device=device)
            # First compilation step
            smith.compile(scan, backend=cnt)(
                get_scan_combine_fn("add", False),
                init,
                x,
                dim=dim,
            ...
            x = smith.randn(3, 20, 5, device=device)
            init = smith.randn(3, 5, device=device)
            ...
            x = smith.randn(3, 40, 5, device=device)
            init = smith.randn(3, 5, device=device)
            ...
            x = smith.randn(3, 40, 5, device=device)
            init = smith.randn(3, 40, device=device)
            ...
            x = smith.randn(3, 40, 20, device=device)
            init = smith.randn(3, 40, device=device)
            ...
            x = smith.randn(3, 40, 40, device=device)
            init = smith.randn(3, 40, device=device)
            ...
            x = smith.randn(3, 60, 40, device=device)
            init = smith.randn(3, 40, device=device)
            # Recompilation because of dim change
            smith.compile(scan, backend=cnt)(
                get_scan_combine_fn("add", False),
            ...
            x = smith.randn(3, 120, 80, device=device)
            init = smith.randn(3, 80, device=device)
    ...
    def test_scan_init_scanned_0(self):
        ...
        x = smith.randn(3, 1, 2, device=smith.device("cpu"))
        init = smith.randn(3, 2, device=smith.device("cpu"))
        dim = 1

        # Scan dimension is 0
        init = smith._ops.ops.aten.slice(x, dim, 0, 1, 1)
        inp = smith._ops.ops.aten.slice(x, dim, 1, None, 1)
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def test_scan_init_wrong_pytree_init_longer_carry(self):
        def init_longer_carry(x: smith.Tensor, y: smith.Tensor):
        ...
        # Init wrong pytree
        init = (
            smith._ops.ops.aten.slice(x, dim, 0, 1, 1),
            smith._ops.ops.aten.slice(x, dim, 0, 1, 1),
        )
    ...
    def test_scan_init_wrong_pytree_init_shorter_carry(self):
        def init_shorter_carry(x: smith.Tensor, y: smith.Tensor):
            return (x + 1, x + 2), x + 3
    ...
    def test_scan_init_wrong_pytree_carry_shape(self):
        def wrong_carry_shape(x: smith.Tensor, y: smith.Tensor):
            return x[0, :], x + 3
    ...
    def test_scan_one_return(self):
        def no_carry(x: smith.Tensor, y: smith.Tensor):
            return x + 3
    ...
    def test_scan_init(self, reverse, compile_mode, device, autograd):
        ...
        # Only init and no input
        x = smith.randn(3, 1, 2, device=device, requires_grad=autograd)
        dim = 1
        op, op_pt = (get_scan_combine_fn("add", False), smith.cumsum)
        ...
        x = smith.randn(3, 5, 2, device=device, requires_grad=autograd)
        dim = 0
        ...
        # Init tensor scalar
        init = smith.ones(1, device=device, requires_grad=autograd)

        def add_scalar_carry(x: smith.Tensor, y: smith.Tensor):
            return x + 1.0, x + y
        ...
        self.assertEqual(result_init, result_exp)
        self.assertEqual(result_init[0], smith.tensor([3.0], device=device))

        if autograd:
            self.check_autograd(result_init, result_exp, (init, inp))

        # Init tensor entirely different shape than inp
        init = smith.randn(7, 8, device=device, requires_grad=autograd)

        def add_scalar_carry2(x: smith.Tensor, y: smith.Tensor):
        ...
        # to be 4x5x2
        init = smith._ops.ops.aten.slice(x, dim, 0, 2, 1)
        ...
        self.assertEqual(result_init, result_exp)
        self.assertEqual(result_init[0].shape, smith.Size([2, 5, 2]))

        if autograd:
            self.check_autograd(result_init, result_exp, (init, inp))

        init = smith.tile(init, (1, 2, 1))

        def add_scalar_carry_sliced_out(x: smith.Tensor, y: smith.Tensor):
            return x + 1.0, x[:, :1, :] + y
        ...
        self.assertEqual(result_init, result_exp)
        self.assertEqual(result_init[0].shape, smith.Size([2, 10, 2]))
        self.assertEqual(result_init[1].shape, smith.Size([2, 2, 5, 2]))
        ...
        op, op_pt = (get_scan_combine_fn("add", False), smith.cumsum)
        x = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
        init = smith.zeros(3, 2, device=device, requires_grad=autograd)
        dim = 2
        ...
        if not reverse:
            ...
            result = list(result)
            result[1] = pytree.tree_map(lambda t: smith.movedim(t, 0, dim), result[1])
            self.assertEqual(result[1], result_exp_PT)
    ...
    def test_scan_init_wrong_pytree_complex(self, reverse, device):
        x = smith.randn(3, 2, 2, device=device)
        y = smith.randn(3, 2, 2, device=device)
        z = smith.randn(3, 2, 2, device=device)

        # Wrong pytree fed to the function
        init = {
            "i": smith._ops.ops.aten.slice(x, 0, 0, 1, 1),
            "j": (
                {"a": smith._ops.ops.aten.slice(x, 0, 0, 1, 1)},
                [smith._ops.ops.aten.slice(y, 0, 0, 1, 1)],
                [{"o": smith._ops.ops.aten.slice(z, 0, 0, 1, 1)}],
            ),
        }
        inp = {
            "i": smith._ops.ops.aten.slice(x, 0, 0, None, 1),
            "j": (
                [smith._ops.ops.aten.slice(y, 0, 0, None, 1)],
                [{"o": smith._ops.ops.aten.slice(z, 0, 0, None, 1)}],
            ),
        }
        with self.assertRaisesRegex(
            Exception,
    ...
    def test_scan_init_pytree_complex(self, reverse, compile_mode, device, autograd):
        ...
        # Regular case
        init = {
            "i": smith._ops.ops.aten.slice(x, 0, init_start, init_end, 1),
            "j": (
                [smith._ops.ops.aten.slice(y, 0, init_start, init_end, 1)],
                [{"o": smith._ops.ops.aten.slice(z, 0, init_start, init_end, 1)}],
            ),
        }
        inp = {
            "i": smith._ops.ops.aten.slice(x, 0, inp_start, inp_end, 1),
            "j": (
                [smith._ops.ops.aten.slice(y, 0, inp_start, inp_end, 1)],
                [{"o": smith._ops.ops.aten.slice(z, 0, inp_start, inp_end, 1)}],
            ),
        }
        result = scan_fct(
            get_scan_combine_fn("complex_pointwise", False),
        ...
        init = {
            "i": smith._ops.ops.aten.slice(x, 0, init_start, init_end, 1),
            "j": (
                smith._ops.ops.aten.slice(x, 0, init_start, init_end, 1),
                [smith._ops.ops.aten.slice(y, 0, init_start, init_end, 1)],
                [{"o": smith._ops.ops.aten.slice(z, 0, init_start, init_end, 1)}],
            ),
        }
    ...
    def test_scan_pytree_output(self):
        x = smith.randn(3, 10, 2, device=smith.device("cpu"))
        init = smith.randn(1, 10, 2, device=smith.device("cpu"))
        ...
        smith.compile(f, backend=backend)(combine_fn, (init, init.clone()), x)
        gm = backend.graphs[0]
...
class GraphModule(smith.nn.Module):
    def forward(self, L_init_0_: "f32[1, 10, 2]", L_init_1_: "f32[1, 10, 2]", L_xs_: "f32[3, 10, 2]"):
        ...
        flip: "f32[3, 10, 2]" = smith.flip(l_xs_, [0]);  l_xs_ = None
        scan_combine_fn_0 = self.scan_combine_fn_0
        scan = smith.ops.higher_order.scan(scan_combine_fn_0, [l_init_0_, l_init_1_], [flip], []);  scan_combine_fn_0 = l_init_0_ = l_init_1_ = flip = None
    ...
    class scan_combine_fn_0(smith.nn.Module):
...
""",  # noqa: B950
    ...
    @skipIfSmithDynamo("Graph is not captured by backend if test with dynamo")
    @unittest.skipIf(not SM70OrLater, "triton")
    @requires_cuda
    @parametrize("compile_mode", ["none", "eager"])
    @parametrize("autograd", [False, True])
    def test_scan_closure_RNN(self, compile_mode, autograd):
        ...
        x = smith.randn(3, 10, 5, device=device, requires_grad=autograd)
        h = smith.randn(3, 7, device=device, requires_grad=autograd)
        ...
        expected_result = rnn(x, smith.unsqueeze(h, 0))
        ...
        result_cmp = [result[0], smith.movedim(result[1], 0, dim)]
        ...
        if autograd:
            ...
            grad_out_expected = [smith.ones_like(r) for r in result_exp_flat]
            expected_grads = smith.autograd.grad(
                result_exp_flat,
                (
                    h,
            ...
            grad_out = [smith.ones_like(r) for r in result]
            grads = smith.autograd.grad(
    ...
    def test_scan_closure_RNN_partial_autograd(
        ...
        for autograd in autograds:
            x = smith.randn(3, 10, 5, device=device, requires_grad=autograd[0])
            x1 = smith.randn(3, 10, 5, device=device, requires_grad=autograd[1])
            h = smith.randn(3, 7, device=device, requires_grad=autograd[2])
            h_1 = smith.randn(3, 7, device=device, requires_grad=autograd[3])
            W_ih = smith.randn(5, 7, device=device, requires_grad=autograd[4])
            b_ih = smith.randn(7, device=device, requires_grad=autograd[5])
            W_hh = smith.randn(7, 7, device=device, requires_grad=autograd[6])
            b_hh = smith.randn(7, device=device, requires_grad=autograd[7])
            ...
            def RNN(x: smith.Tensor, y: smith.Tensor):
                ...
                c_new_1 = x[1] + 1
                h_new = (
                    smith.tanh(c_new_1 + x[0] @ W_hh + b_hh)
                    + y[0] @ W_ih
                    + y[1] @ W_ih
                    + b_ih
                    + x[1]
    ...
    @requires_cuda
    @skipIfSmithDynamo("not a dynamo test")
    @unittest.skipIf(not SM70OrLater, "triton")
    ...
    def test_scan_multiple_layers_gradient(self, layers, device):
            ...
            def forward(self, initial, inputs_sequence):
                ...
                for t in range(T):
                    ...
                    for li, layer in enumerate(self.layers):
                        input_concat = smith.cat((hs_list[li], input), dim=-1)
                ...
                return smith.stack(all_out, dim=1)
            ...
            def forward(self, initial, input_sequence):
                def step(carry, input):
                    ...
                    for li, layer in enumerate(self.layers):
                        ...
                        input_concat = smith.cat((h_prev_li, input), dim=-1)
                        update = layer(input_concat)
        ...
        class RNNScanTensor(nn.Module):
            ...
            def forward(self, initial, input_sequence):
                ...
                hs_stacked = smith.stack(initial, dim=1)
        ...
        initial_hs_template = [
            smith.zeros(
                BATCH_SIZE, FEATURE_DIM, requires_grad=True, dtype=smith.float32
            ).to(DEVICE)
            for _ in range(LAYERS)
        ]
        inputs_template = smith.randn(
            BATCH_SIZE, SEQ_LEN, FEATURE_DIM, requires_grad=True, dtype=smith.float32
        ).to(DEVICE)
        ...
        for model_class in models:
            ...
            compiled_model = smith.compile(model_to_compile)
    ...
    def test_scan_closure_combine_fn_with_no_grad_init_carries_unequal_grad(
        ...
        dim = 1
        scan_fct = compile_mode_helper(scan, compile_mode)
        x = smith.randn(3, 10, 7, device=device, requires_grad=autograd)
        h1 = smith.randn(3, 7, device=device, requires_grad=autograd)
        h2 = smith.randn(3, 7, device=device, requires_grad=autograd)
    ...
    def test_scan_closure_combine_fn_with_no_grad_init_carries_equal_grad(
        ...
        scan_fct = compile_mode_helper(scan, compile_mode)
        x = smith.randn(3, 10, 7, device=device, requires_grad=autograd)
        h1 = smith.randn(3, 7, device=device, requires_grad=False)
        h2 = smith.randn(3, 7, device=device, requires_grad=autograd)
    ...
    def test_scan_closure_combine_fn_with_no_grad_for_out(
        ...
        def fct_ys_no_grad(x: smith.Tensor, y: smith.Tensor):
            c1 = x[0] + y
            c2 = x[1] + y
            with smith.no_grad():
                h_new = smith.tanh(x[0] + x[1] + y)
    ...
    def test_scan_closure_combine_fn_with_no_grad_additional_inputs_partial(
        ...
        x = smith.randn(3, 10, 7, device=device, requires_grad=autograd)
        h = smith.randn(3, 7, device=device, requires_grad=autograd)
        W_ih = smith.randn(7, 7, device=device, requires_grad=autograd)
        b_ih = smith.randn(7, device=device, requires_grad=autograd)
        W_hh = smith.randn(7, 7, device=device, requires_grad=autograd)
        b_hh = smith.randn(7, device=device, requires_grad=autograd)

        def fct_no_grad_bhh_Whh(x: smith.Tensor, y: smith.Tensor):
            c_new = y @ W_ih + b_ih + x

            h_new = c_new + 1
            with smith.no_grad():
                h_new_no_grad = smith.tanh(x @ W_hh + b_hh)
    ...
    def test_scan_closure_combine_fn_with_no_grad_additional_inputs_all(
        ...
        def fct_no_grad_bih_Wih_bhh_Whh(x: smith.Tensor, y: smith.Tensor):
            c_new = x + y
            h_new = c_new + x
            with smith.no_grad():
    ...
    def test_scan_closure_nested(self, reverse, compile_mode, device, autograd):
        ...
        x = smith.randn(3, 20, 5, device=device, requires_grad=autograd)
        h = smith.randn(3, 7, device=device, requires_grad=autograd)
        W = smith.randn(5, 7, device=device, requires_grad=autograd)
        b = smith.randn(7, device=device, requires_grad=autograd)

        def f1(x: smith.Tensor, y: smith.Tensor):
            c_new = y @ W + b
            h_new = smith.tanh(c_new + x)
            return c_new, h_new
        ...
        x1 = smith.ones(3, 20, 5, device=device, requires_grad=autograd)
        h1 = smith.zeros(3, 7, device=device, requires_grad=autograd)
        h2 = smith.zeros(3, 3, device=device, requires_grad=autograd)
        W_1 = smith.randn(5, 7, device=device, requires_grad=autograd)
        b_1 = smith.randn(7, device=device, requires_grad=autograd)
        W_2 = smith.randn(7, 3, device=device, requires_grad=autograd)
        b_2 = smith.randn(3, device=device, requires_grad=autograd)
        ...
        def f2(x: smith.Tensor, y: smith.Tensor):
            c_new = y @ W_2 + b_2
            h_new = smith.tanh(c_new + x)
            return c_new, h_new
        ...
        # Complex case
        x1 = smith.randn(3, 20, 3, device=device, requires_grad=autograd)
        h1 = smith.randn(3, 3, device=device, requires_grad=autograd)
        h2 = smith.randn(3, 3, device=device, requires_grad=autograd)
        W_1 = smith.randn(3, 3, device=device, requires_grad=autograd)
        b_1 = smith.randn(3, device=device, requires_grad=autograd)
        W_2 = smith.randn(3, 3, device=device, requires_grad=autograd)
        b_2 = smith.randn(3, device=device, requires_grad=autograd)
    ...
    def test_scan_simple_graph_wrong_dtype(self):
        def add_wrong_dtype(x: smith.Tensor, y: smith.Tensor):
            return smith.ones_like(x + y, dtype=smith.int64), x + y
...
def forward(self, fct_1, init_1, xs_1):
    flip = smith.ops.aten.flip.default(xs_1, [0])
    sym_size_int_1 = smith.ops.aten.sym_size.int(init_1, 1)
    sym_size_int_2 = smith.ops.aten.sym_size.int(init_1, 2)
    sym_size_int_3 = smith.ops.aten.sym_size.int(xs_1, 1)
    sym_size_int_4 = smith.ops.aten.sym_size.int(xs_1, 2);  xs_1 = None
    scan_combine_graph_0 = self.scan_combine_graph_0
    scan = smith.ops.higher_order.scan(scan_combine_graph_0, [init_1], [flip], (sym_size_int_1, sym_size_int_2, sym_size_int_3, sym_size_int_4));  scan_combine_graph_0 = init_1 = flip = sym_size_int_1 = sym_size_int_2 = sym_size_int_3 = sym_size_int_4 = None
    getitem = scan[0]
    getitem_1 = scan[1];  scan = None
    flip_1 = smith.ops.aten.flip.default(getitem_1, [0]);  getitem_1 = None
    return (getitem, flip_1)""",  # noqa: B950
        )

        # Check graph
        backend = EagerAndRecordGraphs()
        smith.compile(f, backend=backend)(get_scan_combine_fn("add", False), init, x)
        gm = backend.graphs[0]

        self.assertExpectedInline(
            gm.code.strip(),
            """\
def forward(self, L_init_ : smith.Tensor, L_xs_ : smith.Tensor):
    l_init_ = L_init_
    l_xs_ = L_xs_
    flip = smith.flip(l_xs_, [0]);  l_xs_ = None
    scan_combine_fn_0 = self.scan_combine_fn_0
    scan = smith.ops.higher_order.scan(scan_combine_fn_0, [l_init_], [flip], []);  scan_combine_fn_0 = l_init_ = flip = None
    carry = scan[0]
    ...
    def test_scan_input_mutation(self):
        ...
        x = smith.randn(3, 2, 2, device=device)
        init = smith.randn(2, 2, device=device)
    ...
    def test_scan_input_carry_alias(self):
        ...
        y = smith.randn(3, 2, 2, device=device)
        inp = (x, y)
        init = (smith.randn(2, 2, device=device), smith.randn(2, 2, device=device))
...
class AssociativeScanModels:
    ...
    class CombineFn(smith.nn.Module):
    ...
    class NestedFn(smith.nn.Module):
...
class AssociativeScanTests(TestCase):
    ...
    def _check_autograd(self, result, result_exp, autograd_param):
        ...
        grad_init = [smith.ones_like(el) for el in result_flatten]
        grads = smith.autograd.grad(result_flatten, grad_param, grad_init)
    ...
    @decorateIf(
        ...
        lambda params: (
            ...
            and (
                params["device"] == smith.device("cpu")
    ...
    #         and params["device"] == smith.device("cpu")
    #         and params["autograd"]
    #     ),
    # )
    def test_associative_scan_compile(
        ...
        if not reverse:
            results_smith = []
            for op_pt in [smith.cumsum, smith.cumprod]:
                results_smith.append(op_pt(x, 0))
            self.assertEqual(results, results_smith)

        # Jax Examples
        x = smith.arange(
            0, 4, device=device, dtype=smith.float32, requires_grad=autograd
        )
        kwargs = {
            "dim": 0,
            "reverse": reverse,
        ...
        if not reverse:
            results_smith = smith.tensor([0.0, 1.0, 3.0, 6.0], dtype=smith.float32)
        else:
            results_smith = smith.tensor([6.0, 6.0, 5.0, 3.0], dtype=smith.float32)

        self.assertEqual(result, results_smith)
    ...
    def test_associative_scan_dim_shape_failure(self, compile_mode, combine_mode):
        ...
        for num_dim in num_dims:
            ...
            rnd_scan_dim = 0
            x = smith.randn(*shapes, device=smith.device("cuda"))
    ...
    def test_associative_scan_expand_in_combine_fn(
        ...
        def combine_fn(x, y):
            return x * smith.sum(y, -1).expand(x.shape)
    ...
    def test_associative_scan_non_contiguous_tensor(
        ...
        x = (
            smith.arange(30, device=device, dtype=smith.float32, requires_grad=autograd)
            .view(10, 3)
            .t()
        )
    ...
    def test_associative_scan_pytree_output(self):
        x = (
            (
                smith.randn(3, 10, 2, device=smith.device("cpu")),
                (smith.randn(3, 10, 2, device=smith.device("cpu")),),
            ),
            smith.randn(3, 10, 2, device=smith.device("cpu")),
        )
        ...
        def combine_fn(x: smith.Tensor, y: smith.Tensor):
            a, b = (x[0][0] + y[1], x[0][1][0] - y[1])
            return (a, (b,)), a - b

        # Check graph
        backend = EagerAndRecordGraphs()
        smith.compile(f, backend=backend)(combine_fn, x)
        gm = backend.graphs[0]
...
class GraphModule(smith.nn.Module):
    def forward(self, L_xs_0_0_: "f32[3, 10, 2]", L_xs_0_1_0_: "f32[3, 10, 2]", L_xs_1_: "f32[3, 10, 2]"):
        ...
        elem: "f32[3, 10, 2]" = smith.movedim(l_xs_0_0_, 0, 0);  l_xs_0_0_ = None
        elem_1: "f32[3, 10, 2]" = smith.movedim(l_xs_0_1_0_, 0, 0);  l_xs_0_1_0_ = None
        elem_2: "f32[3, 10, 2]" = smith.movedim(l_xs_1_, 0, 0);  l_xs_1_ = None
        elem_3: "f32[3, 10, 2]" = smith.flip(elem, [0]);  elem = None
        elem_4: "f32[3, 10, 2]" = smith.flip(elem_1, [0]);  elem_1 = None
        elem_5: "f32[3, 10, 2]" = smith.flip(elem_2, [0]);  elem_2 = None
        child: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_3, 0, 0, -1, 2)
        child_1: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_4, 0, 0, -1, 2)
        child_2: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_5, 0, 0, -1, 2)
        child_3: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_3, 0, 1, None, 2)
        child_4: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_4, 0, 1, None, 2)
        child_5: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_5, 0, 1, None, 2)

        lazy_load_decompositions = smith._funcsmith.predispatch.lazy_load_decompositions();  lazy_load_decompositions = None

        _vmap_increment_nesting = smith._funcsmith.predispatch._vmap_increment_nesting(1, 'error');  _vmap_increment_nesting = None

        _add_batch_dim: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child, 0, 1);  child = None
        _add_batch_dim_1: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_1, 0, 1);  child_1 = None
        _add_batch_dim_2: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_2, 0, 1);  child_2 = _add_batch_dim_2 = None
        _add_batch_dim_3: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_3, 0, 1);  child_3 = _add_batch_dim_3 = None
        _add_batch_dim_4: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_4, 0, 1);  child_4 = _add_batch_dim_4 = None
        _add_batch_dim_5: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_5, 0, 1);  child_5 = None
        ...
        child_7: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(a, 1, 1, 0);  a = None
        child_8: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(b, 1, 1, 0);  b = None
        child_9: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(child_6, 1, 1, 0);  child_6 = None

        _vmap_decrement_nesting = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting = None

        child_10: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_3, 0, 2, None, 2)
        child_11: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_4, 0, 2, None, 2)
        child_12: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_5, 0, 2, None, 2)
        ...
        _add_batch_dim_6: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_7, 0, 1)
        _add_batch_dim_7: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_8, 0, 1)
        _add_batch_dim_8: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_9, 0, 1);  _add_batch_dim_8 = None
        _add_batch_dim_9: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_10, 0, 1);  child_10 = _add_batch_dim_9 = None
        _add_batch_dim_10: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_11, 0, 1);  child_11 = _add_batch_dim_10 = None
        _add_batch_dim_11: "f32[10, 2]" = smith._funcsmith.predispatch._add_batch_dim(child_12, 0, 1);  child_12 = None
        ...
        child_14: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(a_1, 1, 1, 0);  a_1 = None
        child_15: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(b_1, 1, 1, 0);  b_1 = None
        child_16: "f32[1, 10, 2]" = smith._funcsmith.predispatch._remove_batch_dim(child_13, 1, 1, 0);  child_13 = None

        _vmap_decrement_nesting_1 = smith._funcsmith.predispatch._vmap_decrement_nesting();  _vmap_decrement_nesting_1 = None

        slice_10: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_3, 0, 0, 1);  elem_3 = None
        cat: "f32[2, 10, 2]" = smith.cat([slice_10, child_14], dim = 0);  slice_10 = child_14 = None
        slice_11: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_4, 0, 0, 1);  elem_4 = None
        cat_1: "f32[2, 10, 2]" = smith.cat([slice_11, child_15], dim = 0);  slice_11 = child_15 = None
        slice_12: "f32[1, 10, 2]" = smith.ops.aten.slice(elem_5, 0, 0, 1);  elem_5 = None
        cat_2: "f32[2, 10, 2]" = smith.cat([slice_12, child_16], dim = 0);  slice_12 = child_16 = None

        b_2: "f32[2, 10, 2]" = smith._C._nn.pad(child_7, [0, 0, 0, 0, 0, 1], 'constant', None);  child_7 = None

        stacked: "f32[2, 2, 10, 2]" = smith.stack([cat, b_2], dim = 1);  cat = b_2 = None
        interleaved: "f32[4, 10, 2]" = smith.flatten(stacked, start_dim = 0, end_dim = 1);  stacked = None
        interleaved_1: "f32[3, 10, 2]" = smith.ops.aten.slice(interleaved, 0, 0, 3);  interleaved = None

        b_3: "f32[2, 10, 2]" = smith._C._nn.pad(child_8, [0, 0, 0, 0, 0, 1], 'constant', None);  child_8 = None

        stacked_1: "f32[2, 2, 10, 2]" = smith.stack([cat_1, b_3], dim = 1);  cat_1 = b_3 = None
        interleaved_2: "f32[4, 10, 2]" = smith.flatten(stacked_1, start_dim = 0, end_dim = 1);  stacked_1 = None
        interleaved_3: "f32[3, 10, 2]" = smith.ops.aten.slice(interleaved_2, 0, 0, 3);  interleaved_2 = None

        b_4: "f32[2, 10, 2]" = smith._C._nn.pad(child_9, [0, 0, 0, 0, 0, 1], 'constant', None);  child_9 = None

        stacked_2: "f32[2, 2, 10, 2]" = smith.stack([cat_2, b_4], dim = 1);  cat_2 = b_4 = None
        interleaved_4: "f32[4, 10, 2]" = smith.flatten(stacked_2, start_dim = 0, end_dim = 1);  stacked_2 = None
        interleaved_5: "f32[3, 10, 2]" = smith.ops.aten.slice(interleaved_4, 0, 0, 3);  interleaved_4 = None
        ...
        movedim_3: "f32[3, 10, 2]" = smith.movedim(flip_3, 0, 0);  flip_3 = None
        movedim_4: "f32[3, 10, 2]" = smith.movedim(flip_4, 0, 0);  flip_4 = None
        movedim_5: "f32[3, 10, 2]" = smith.movedim(flip_5, 0, 0);  flip_5 = None
...
""",  # noqa: B950
    ...
    def test_associative_scan_nested(self):
        ...
        inp = smith.randn(3, 10, 2, device=device)
    ...
    def test_associative_scan_loop_in_combine_fn(
        ...
        def combine_fn(x, y):
            ...
            if loop_type == "while":
                ...
                def body_fn(ind, loop_val):
                    return ind + 1, loop_val + smith.abs(ind)

                new_ind, cnt = smith.while_loop(
                    cond_fn=cond_fn,
                    body_fn=body_fn,
                    carried_inputs=(
                        smith.zeros(1, dtype=smith.int32, device=cnt.device),
                        cnt,
                    ),
                )
            else:
        ...
        inp = smith.randn(3, 10, 1, device=device, requires_grad=autograd) * 2
    ...
    def test_associative_scan_loop_in_combine_fn_failure(self):
        ...
        inp = smith.randn(3, 10, 1, device=device) * 2
    ...
    def test_associative_scan_cond_in_combine_fn(
        ...
        def combine_fn(x, y):
            val = cond(smith.sum(y) > 0.0, lambda y: y.clone(), lambda y: 1.0 - y, (y,))
            return x * val

        inp = smith.randn(3, 10, 1, device=device, requires_grad=autograd)
    ...
    def test_associative_scan_map_in_combine_fn(self):
        ...
        inp = smith.randn(3, 10, 1, device=device)
    ...
    def test_associative_scan_vmap_in_combine_fn(
        ...
        def combine_fn(x, y):
            ...
            mapped_body = smith.vmap(body, 0, 0)
            y_new = mapped_body(y)
            return x + y_new
    ...
    @decorateIf(
        unittest.skip,
        lambda params: (params["device"] == smith.device("cpu")),
    )
    ...
    def test_associative_scan_binary_operator(
        ...
        A = smith.randn(state_dim, device=device, requires_grad=autograd)
        elements = (A.repeat((timesteps, 1)), projected_inputs)
    ...
    def test_associative_scan_different_input_size(self, compile_mode, reverse, device):
        ...
        deltaB_u = smith.randn(
            (batch, hidden_dim, length, dstate), requires_grad=True, device=device
        )
        C = smith.randn((batch, dstate, length), requires_grad=True, device=device)
        x = smith.randn(
            (batch, hidden_dim, length, dstate), requires_grad=True, device=device
        )
        y = smith.randn((batch, hidden_dim, length), requires_grad=True, device=device)
    ...
    def test_associative_scan_different_input_size_wrong_dim(self):
        ...
        deltaA = smith.randn(
            (batch, hidden_dim, length, dstate), device=smith.device("cuda")
        )
        deltaB_u = smith.randn(
            (batch, hidden_dim, length, dstate), device=smith.device("cuda")
        )
        C = smith.randn((batch, dstate, length), device=smith.device("cuda"))
        x = smith.randn(
            (batch, hidden_dim, length, dstate), device=smith.device("cuda")
        )
        y = smith.randn(
    ...
    def test_associative_scan_freevars_simple(
        ...
        H = smith.rand(2, device=device, requires_grad=autograd)

        def fct_freevars1(x: smith.Tensor, y: smith.Tensor):
            return x * H + y * 2

        def fct_freevars2(x: smith.Tensor, y: smith.Tensor):
            return x * H + y * H

        H1 = smith.rand(1, device=device, requires_grad=autograd)
        H2 = smith.rand(1, device=device, requires_grad=autograd)

        def fct_freevars3(x: smith.Tensor, y: smith.Tensor):
            return x * H1 + y * H2

        inp = smith.randn(3, 2, 2, device=device, requires_grad=autograd)
    ...
    def test_associative_scan_freevars_nested(
        ...
        H1 = smith.rand(4, 5, device=device, requires_grad=autograd)
        H2 = smith.rand(4, 1, device=device, requires_grad=autograd)

        def fct_nested_outside(x: smith.Tensor, y: smith.Tensor):
            def inner(xi):
                return xi * H2
        ...
        def fct_nested_outside_fake(x: smith.Tensor, y: smith.Tensor):
            def inner(xi):
                return xi * H2
        ...
        def fct_nested_inside(x: smith.Tensor, y: smith.Tensor):
            H2_i = smith.ones(4, 1, device=device) * 42
        ...
        def fct_nested_inside_fake(x: smith.Tensor, y: smith.Tensor):
            H2_i = smith.ones(4, 1, device=device) * 42
        ...
        inp = smith.randn(3, 4, 5, device=device, requires_grad=autograd)
    ...
    def test_associative_scan_freevars_shape_check(
        ...
        H = smith.eye(2, device=device, requires_grad=True)

        def fct_freevars(x: smith.Tensor, y: smith.Tensor):
            return x @ H + y

        inp = smith.randn(2, 2, 3, device=device, requires_grad=True)
    ...
    def test_associative_scan_freevars_pytree(
        ...
        xf = smith.randn(2, 2, device=device, requires_grad=autograd)
        yf = smith.randn(2, 2, device=device, requires_grad=autograd)
        zf = smith.randn(2, 2, device=device, requires_grad=autograd)
        inpf = {"i": xf, "j": ([yf], [{"o": zf}])}
    ...
    def test_associative_scan_partial_grad(
        ...
        for a_grads in autograds:
            inp = tuple(
                [
                    smith.randn(10, 3, 2, device=device, requires_grad=a_grads[n])
    ...
    def test_associative_scan_partial_grad_no_grad(
        ...
        inp = tuple(
            [smith.randn(10, 3, 2, device=device, requires_grad=True) for n in range(2)]
        )
    ...
    def test_associative_scan_combine_fn_wrong_meta_in_combine_fn(self):
        device = smith.device("cuda")
        B, N, C, H, W = 3, 3, 2, 3, 3
        x = smith.randn(B, N, C, H, W, device=device)

        def fct_wrong_dtype(x, y):
            return (x + y).to(smith.int64)

        def fct_wrong_device(x, y):
            return (x + y).to(
                smith.device("cpu") if device.type == "cuda" else smith.device("cuda")
            )

        def fct_wrong_stride(x, y):
            return (x + y).to(memory_format=smith.channels_last)
    ...
    def test_associative_scan_input_mutation(self):
        ...
        with self.assertRaisesRegex(
            ...
            # "Encountered aliasing during higher order op tracing for HOP.*"
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.ops\.higher_order\.associative_scan",
        ):
...
class TestControlFlowTraced(TestCase):
    ...
    def _check_compile(self, fn, args, *, dynamic=False, backend="eager"):
        eager_res = fn(*args)
        compiled_fn = smith.compile(fn, backend=backend, dynamic=dynamic)
    ...
    def test_cond_traced_not_nested(self):
        ...
        x = smith.randn(4)
        graph = make_fx(f)(x, smith.tensor(False))
        result_true = graph.forward(x, smith.tensor(True))
        result_false = graph.forward(x, smith.tensor(False))
        self.assertFalse(smith.allclose(result_true, result_false))
        self.assertEqual(result_true, smith.sin(x))
        self.assertEqual(result_false, smith.cos(x))

        graph = make_fx(f, tracing_mode="symbolic")(x, smith.tensor(False))
        self.assertEqual(graph(x, smith.tensor(True)), f(x, smith.tensor(True)))
    ...
    def test_cond_simple_with_linear_compile_check_graph(self):
        ...
        backend = EagerAndRecordGraphs()
        smith.compile(f, backend=backend)(smith.tensor(False), x)
        ...
        self.assertExpectedInline(
            gm.code.strip(),
            """\
def forward(self, L_pred_ : smith.Tensor, L_x_ : smith.Tensor):
    l_pred_ = L_pred_
    l_x_ = L_x_
    cond_true_0 = self.cond_true_0
    cond_false_0 = self.cond_false_0
    cond = smith.ops.higher_order.cond(l_pred_, cond_true_0, cond_false_0, (l_x_,));  l_pred_ = cond_true_0 = cond_false_0 = None
    result = cond[0];  cond = None
    grad_out = smith.ones_like(result)
    grad = smith.autograd.grad(result, (l_x_,), grad_out);  result = l_x_ = grad_out = None
...
def forward(self, out_iter_1, it_1, y_1):
    ...
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (out_iter_1, it_1, y_1), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = out_iter_1 = it_1 = y_1 = None
...
def forward(self, arg0_1, arg1_1, arg2_1):
    sum_1 = smith.ops.aten.sum.default(arg0_1);  arg0_1 = None
    lt = smith.ops.aten.lt.Scalar(sum_1, 2);  sum_1 = None
    return lt
    """,
        )
        self.assertExpectedInline(
...
def forward(self, arg0_1, arg1_1, arg2_1):
    while_loop_cond_graph_0 = self.while_loop_cond_graph_0
    while_loop_body_graph_0 = self.while_loop_body_graph_0
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (arg0_1, arg1_1, arg2_1), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = arg0_1 = arg1_1 = arg2_1 = None
    getitem = while_loop[0]
    getitem_1 = while_loop[1]
    getitem_2 = while_loop[2];  while_loop = None
    add = smith.ops.aten.add.Tensor(getitem, 1);  getitem = None
    ...
    def test_while_loop_pytree_carry(self):
        ...
        backend = EagerAndRecordGraphs()
        expected_res = fn(*inp)
        compiled_res = smith.compile(fn, backend=backend)(*inp)
        self.assertEqual(expected_res, compiled_res)
        # When test with smith dynamo, the graph is not captured because
        # it's traced together with the code before smith.compile
        if not TEST_WITH_SMITHDYNAMO:
            self.assertEqual(len(backend.graphs), 1)
            self.assertExpectedInline(
                backend.graphs[0].code.strip(),
                """\
def forward(self, L_it_ : smith.Tensor, L_pytree_input_0_0_ : smith.Tensor, L_pytree_input_1_x_ : smith.Tensor, L_pytree_input_1_y_ : smith.Tensor):
    l_it_ = L_it_
    ...
    while_loop = smith.ops.higher_order.while_loop(cond_fn_0, body_fn_0, (l_it_, l_pytree_input_0_0_, l_pytree_input_1_x_, l_pytree_input_1_y_), ());  cond_fn_0 = body_fn_0 = l_it_ = l_pytree_input_0_0_ = l_pytree_input_1_x_ = l_pytree_input_1_y_ = None
    getitem = while_loop[0]
    getitem_1 = while_loop[1]
    value = while_loop[2]
    ...
    def _wrap_with_functionalize(self, fn, func_type):
        ...
        elif func_type == "funcsmith":
            fn = smith.func.functionalize(fn)
        else:
            assert func_type == "no"
        return fn, mode

    @parametrize("func_type", ["no", "cpp", "python", "funcsmith"])
...
def forward(self, x_1):
    while_loop_cond_graph_0 = self.while_loop_cond_graph_0
    while_loop_body_graph_0 = self.while_loop_body_graph_0
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (x_1,), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = x_1 = None
...
def forward(self, arg0_1):
    clone = smith.ops.aten.clone.default(arg0_1);  arg0_1 = None
    add_ = smith.ops.aten.add_.Tensor(clone, 1);  clone = None
    add__1 = smith.ops.aten.add_.Tensor(add_, -1);  add_ = None
    sum_1 = smith.ops.aten.sum.default(add__1);  add__1 = None
    lt = smith.ops.aten.lt.Scalar(sum_1, 10);  sum_1 = None
    return lt
    """,
            )
            self.assertExpectedInline(
...
def forward(self, arg0_1):
    ...
    add_ = smith.ops.aten.add_.Tensor(clone, 1);  clone = None
    add__1 = smith.ops.aten.add_.Tensor(add_, -1);  add_ = None
    add = smith.ops.aten.add.Tensor(add__1, 1);  add__1 = None
    return (add,)
    """,
            )
        elif func_type == "python":
...
def forward(self, arg0_1):
    while_loop_cond_graph_0 = self.while_loop_cond_graph_0
    while_loop_body_graph_0 = self.while_loop_body_graph_0
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (arg0_1,), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = arg0_1 = None
    getitem = while_loop[0];  while_loop = None
    return (getitem,)
    """,  # noqa: B950
            )
...
def forward(self, arg0_1):
    clone = smith.ops.aten.clone.default(arg0_1);  arg0_1 = None
    add = smith.ops.aten.add.Tensor(clone, 1);  clone = None
    add_1 = smith.ops.aten.add.Tensor(add, -1);  add = None
    sum_1 = smith.ops.aten.sum.default(add_1);  add_1 = None
    lt = smith.ops.aten.lt.Scalar(sum_1, 10);  sum_1 = None
    return lt
    """,
            )
...
def forward(self, arg0_1):
    ...
    add = smith.ops.aten.add.Tensor(clone, 1);  clone = None
    add_1 = smith.ops.aten.add.Tensor(add, -1);  add = None
    add_2 = smith.ops.aten.add.Tensor(add_1, 1);  add_1 = None
    return (add_2,)
    """,
            )
        else:
...
def forward(self, arg0_1):
    ...
    def test_while_loop_simple_with_linear_compile_check_graph(self):
        ...
        backend = EagerAndRecordGraphs()
        smith.compile(fn, backend=backend)(*inp)
        self.assertEqual(len(backend.graphs), 1)
        gm = backend.graphs[0]
...
class GraphModule(smith.nn.Module):
    ...
    class body_fn_0(smith.nn.Module):
        def forward(self, child_2: "i64[]", child_3: "f32[2, 2]", l_self_buffers_dec__cond_fn: "i64[]", l_self_modules_linear_parameters_bias__body_fn: "f32[2]", l_self_modules_linear_parameters_weight__body_fn: "f32[2, 2]"):
            ...
            child_4: "f32[2, 2]" = smith._C._nn.linear(child_3, l_self_modules_linear_parameters_weight__body_fn, l_self_modules_linear_parameters_bias__body_fn);  child_3 = l_self_modules_linear_parameters_weight__body_fn = l_self_modules_linear_parameters_bias__body_fn = None
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1):
    sym_size_int = smith.ops.aten.sym_size.int(arg3_1, 1)
    sym_size_int_1 = smith.ops.aten.sym_size.int(arg2_1, 1)
    sym_size_int_2 = smith.ops.aten.sym_size.int(arg2_1, 0)
    sym_size_int_3 = smith.ops.aten.sym_size.int(arg3_1, 0)
    while_loop_cond_graph_0 = self.while_loop_cond_graph_0
    while_loop_body_graph_0 = self.while_loop_body_graph_0
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (arg0_1, arg1_1, arg2_1, arg3_1), (sym_size_int, sym_size_int_1, sym_size_int_2, sym_size_int_3));  while_loop_cond_graph_0 = while_loop_body_graph_0 = arg0_1 = arg1_1 = arg2_1 = arg3_1 = sym_size_int = sym_size_int_1 = sym_size_int_2 = sym_size_int_3 = None
    getitem = while_loop[0]
    getitem_1 = while_loop[1]
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1, arg6_1, arg7_1):
    while_loop_cond_graph_0 = self.while_loop_cond_graph_0
    while_loop_body_graph_0 = self.while_loop_body_graph_0
    while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (arg0_1, arg1_1, arg2_1, arg3_1), (arg7_1, arg7_1, arg7_1, arg7_1));  while_loop_cond_graph_0 = while_loop_body_graph_0 = arg0_1 = arg1_1 = arg2_1 = arg3_1 = arg7_1 = None
    getitem = while_loop[0]
    getitem_1 = while_loop[1]
    getitem_2 = while_loop[2]
    getitem_3 = while_loop[3];  while_loop = None
    sub = smith.ops.aten.sub.Tensor(getitem, 1);  getitem = None
    clone = smith.ops.aten.clone.default(getitem_1);  getitem_1 = None
    mul = smith.ops.aten.mul.Tensor(getitem_2, 2);  getitem_2 = None
    div = smith.ops.aten.div.Tensor(getitem_3, 2);  getitem_3 = None
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1, arg6_1, arg7_1):
    clone = smith.ops.aten.clone.default(arg0_1);  arg0_1 = None
    sub = smith.ops.aten.sub.Tensor(arg1_1, 1);  arg1_1 = None
    add = smith.ops.aten.add.Tensor(arg2_1, 3.14);  arg2_1 = None
    sub_1 = smith.ops.aten.sub.Tensor(arg3_1, 2.71);  arg3_1 = None
    ...
    """,
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1, arg6_1, arg7_1):
    gt = smith.ops.aten.gt.Scalar(arg1_1, 0);  arg1_1 = None
    return gt
    """,
        )
    ...
    def test_cond_nested_traced(self):
        ...
        x = smith.randn(4)
        graph = make_fx(f)(x, smith.tensor(False), smith.tensor(False))

        result_true_true = graph.forward(
            x, smith.tensor(True), smith.tensor(True)
        )  # True + True -> x * x
        result_true_false = graph.forward(
            x, smith.tensor(True), smith.tensor(False)
        )  # True + True -> x + x
        result_false_true = graph.forward(
            x, smith.tensor(False), smith.tensor(True)
        )  # False + either -> cos
        result_false_false = graph.forward(
            x, smith.tensor(False), smith.tensor(False)
        )  # False + either -> cos

        self.assertNotEqual(result_true_true, result_true_false)
        self.assertFalse(smith.allclose(result_false_true, result_true_true))
        ...
        self.assertEqual(result_false_true, smith.cos(x))

        graph = make_fx(f, tracing_mode="symbolic")(
            x, smith.tensor(False), smith.tensor(False)
        )
        self.assertEqual(
            graph(x, smith.tensor(True), smith.tensor(True)),
            f(x, smith.tensor(True), smith.tensor(True)),
        )
    ...
    def test_cond_functionalized(self):
        ...
        example_inputs = (smith.ones(4, 5),)
        functional_f = smith.func.functionalize(f)
        self.assertEqual(functional_f(*example_inputs), f(*example_inputs))

        graph_module = make_fx(smith.func.functionalize(f), tracing_mode="symbolic")(
            *example_inputs
        )
    ...
    def test_cond_accepts_smith_function_as_inputs(self):
        a = smith.randn(3, 4)
        b = smith.randn(3, 4)

        def f(a, b):
            return cond(a.sum() > 0, smith.add, smith.mul, (a, b))
...
def forward(self, a_1, b_1):
    sum_1 = smith.ops.aten.sum.default(a_1)
    gt = smith.ops.aten.gt.Scalar(sum_1, 0);  sum_1 = None
    sym_size_int = smith.ops.aten.sym_size.int(a_1, 1)
    sym_size_int_1 = smith.ops.aten.sym_size.int(b_1, 0)
    sym_size_int_2 = smith.ops.aten.sym_size.int(b_1, 1)
    sym_size_int_3 = smith.ops.aten.sym_size.int(a_1, 0)
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (a_1, b_1, sym_size_int, sym_size_int_1, sym_size_int_2, sym_size_int_3));  gt = true_graph_0 = false_graph_0 = a_1 = b_1 = sym_size_int = sym_size_int_1 = sym_size_int_2 = sym_size_int_3 = None
    getitem = cond[0];  cond = None
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1):
    ...
    return (add,)""",
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1):
    mul = smith.ops.aten.mul.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = None
    return (mul,)""",
        )
    ...
    def test_cond_retrace_functionalized(self):
        ...
        gm_functional = make_fx(
            smith.func.functionalize(gm_non_functional), tracing_mode="real"
        )(inp)
        self.assertEqual(gm_functional(smith.zeros(1, 2)), f(smith.zeros(1, 2)))
    ...
    def test_cond_subgraph_same_shape_env_as_parent(self):
        ...
        symbolic_traced_graph = self._check_tracing(
            f, (smith.ones(4), smith.Tensor([True]))
        )["symbolic"]
        ...
        def _node_shape_env_iter(gm):
            for node in symbolic_traced_graph.graph.nodes:
                if node.op == "call_function":
                    ...
                    elif isinstance(val, smith.SymInt):
    ...
    def test_cond_functionalized_data_dependent_pred(self):
        ...
        graph_module = make_fx(smith.func.functionalize(f))(*example_inputs)
        self.assertEqual(graph_module(*example_inputs), f(*example_inputs))
    ...
    def test_cond_functionalized_input_mutation_on_true_branch(self):
        ...
        example_inputs = (smith.ones(4, 5),)
        # smith.cond inlines into one of the branches because the predicate
        # is a constant.
        gm = make_fx(smith.func.functionalize(f))(*example_inputs)
        self.assertExpectedInline(
            gm.code.strip(),
            """\
def forward(self, x_1):
    view = smith.ops.aten.view.default(x_1, [4, 5])
    add = smith.ops.aten.add.Tensor(view, 1);  view = None
    view_1 = smith.ops.aten.view.default(add, [4, 5]);  add = None
    view_2 = smith.ops.aten.view.default(view_1, [4, 5])
    sin = smith.ops.aten.sin.default(view_2);  view_2 = None
    sum_1 = smith.ops.aten.sum.default(sin);  sin = None
    copy_ = smith.ops.aten.copy_.default(x_1, view_1);  x_1 = view_1 = copy_ = None
    return sum_1""",
        )

        # smith.cond triggers the check of the branches because the predicate
        # is a SymBool.
        with self.assertRaisesRegex(
            smith._dynamo.exc.SmithRuntimeError,
            "cond_true might be modifying the input!",
        ):
            make_fx(smith.func.functionalize(f), tracing_mode="symbolic")(
                *example_inputs
            )
    ...
    def test_cond_functionalized_input_mutation_on_false_branch(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, x_1):
    view = smith.ops.aten.view.default(x_1, [5, 5])
    add = smith.ops.aten.add.Tensor(view, 1);  view = None
    view_1 = smith.ops.aten.view.default(add, [5, 5]);  add = None
    view_2 = smith.ops.aten.view.default(view_1, [5, 5])
    cos = smith.ops.aten.cos.default(view_2);  view_2 = None
    sum_1 = smith.ops.aten.sum.default(cos);  cos = None
    copy_ = smith.ops.aten.copy_.default(x_1, view_1);  x_1 = view_1 = copy_ = None
    return sum_1""",
        )
    ...
    def test_cond_functionalized_output_alias_input(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, x_1):
    view = smith.ops.aten.view.default(x_1, [5, 5]);  x_1 = None
    return view""",
        )
    ...
    def test_cond_functionalized_nested_input_mutation_with_aot_func(self):
        ...
        example_input = smith.ones(4, 5)
        try:
            example_input_func = to_fun_old(example_input)
            smith._enable_functionalization(reapply_views=False)
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    func_args = pytree.tree_map(
                        lambda x: smith._to_functional_tensor(x)
    ...
    def test_cond_functionalized_aot_func_check_functional(self):
        ...
        def f_wrapper(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    func_args = pytree.tree_map(
                        lambda x: to_fun_old(x) if isinstance(x, smith.Tensor) else x,
                        args,
                    )
                    func_kwargs = pytree.tree_map(
                        lambda x: to_fun_old(x) if isinstance(x, smith.Tensor) else x,
        ...
        self.assertEqual(result_gm(smith.ones(5, 5)), f(smith.ones(5, 5)))
    ...
    def test_cond_nested_traced_other_inputs(self):
        ...
        def true_fn(k, pred2):
            ...
            return smith.add(smith.tensor([0.25, 0.25]), z)
        ...
        a = smith.tensor([1.0, 1.0])
        result_true_true = graph.forward(a, smith.tensor(True), smith.tensor(True))
        self.assertEqual(result_true_true, (a * a) + smith.tensor([0.25, 0.25]))

        b = smith.tensor([2.0, 2.0])
        result_true_true = graph.forward(b, smith.tensor(True), smith.tensor(True))
        self.assertEqual(result_true_true, (b * b) + smith.tensor([0.25, 0.25]))
...
def forward(self, x_1, pred_1, pred2_1):
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(pred_1, true_graph_0, false_graph_0, (x_1,));  pred_1 = true_graph_0 = false_graph_0 = None
    getitem = cond[0];  cond = None
    true_graph_1 = self.true_graph_1
    false_graph_1 = self.false_graph_1
    cond_1 = smith.ops.higher_order.cond(pred2_1, true_graph_1, false_graph_1, (x_1,));  pred2_1 = true_graph_1 = false_graph_1 = x_1 = None
    getitem_1 = cond_1[0];  cond_1 = None
    add = smith.ops.aten.add.Tensor(getitem, getitem_1);  getitem = getitem_1 = None
    return add""",  # noqa: B950
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1):
    mul = smith.ops.aten.mul.Tensor(arg0_1, arg0_1);  arg0_1 = None
    return (mul,)""",
        )
    ...
    def test_raise_error_on_mismatch_type_size(self):
        ...
        with self.assertRaisesRegex(
            ...
            make_fx(f)(x, smith.tensor(False))
    ...
    def test_raise_error_on_mismatch_tensor_size(self):
        ...
        with self.assertRaisesRegex(
            smith._dynamo.exc.SmithRuntimeError,
            "When merging two branches' output in smith.cond",
        ):
            make_fx(f)(x, smith.tensor(False))
    ...
    def test_cond_traced_not_nested_fake_tensor(self):
        ...
        x = smith.randn(4)
        graph = make_fx(f, tracing_mode="fake")(x, smith.tensor(False))
        result_true = graph.forward(x, smith.tensor(True))
        result_false = graph.forward(x, smith.tensor(False))
        self.assertFalse(smith.allclose(result_true, result_false))
        self.assertEqual(result_true, smith.sin(x))
...
def forward(self, arg0_1):
    ...
    def test_raise_error_on_mismatch_type_size_fake_tensor(self):
        ...
        with self.assertRaisesRegex(
            ...
            make_fx(f, tracing_mode="fake")(x, smith.tensor(False))
        ...
        for m in gm.modules():
            for node in m.graph.nodes:
                if (
                    ...
                    and node.target == smith.ops.higher_order.map_impl
                ):
                    i += 1
    ...
    def test_tracing_map_real(self):
        ...
        gm = make_fx(g, tracing_mode="real")(smith.ones(3, 2, 2), smith.ones(2))
        x = smith.randn(3, 2, 2)
        y = smith.randn(2)
        res = gm(x, y)
    ...
    def test_tracing_map_symbolic_simple(self):
        ...
        gm = make_fx(g, tracing_mode="symbolic")(smith.ones(3, 2, 4), smith.ones(4))
        x = smith.randn(3, 2, 2)
        y = smith.randn(2)
        res = gm(x, y)
        self.assertEqual(res, g(x, y))
    ...
    def test_tracing_map_symbolic_list(self):
        ...
        example_x = [[smith.ones(3, 4, 5)], smith.ones(3, 4, 5)]
        gm = make_fx(g, tracing_mode="symbolic")(
            example_x, smith.ones(5), smith.ones(5)
        )
        x = [[smith.randn(4, 5, 6)], smith.ones(4, 5, 6)]
        y = smith.randn(6)
        z = smith.ones(6)
        res = gm(x, y, z)
    ...
    def test_tracing_map_symbolic_dict(self):
        ...
        example_x = {"b": {"a": smith.ones(3, 4, 5)}, "c": smith.ones(3, 4, 5)}
        gm = make_fx(g, tracing_mode="symbolic")(
            example_x, smith.ones(5), smith.ones(5)
        )
        x = {"b": {"a": smith.randn(4, 5, 6)}, "c": smith.ones(4, 5, 6)}
        y = smith.randn(6)
        z = smith.ones(6)
        res = gm(x, y, z)
        self.assertEqual(res, g(x, y, z))
    ...
    def test_tracing_map_autograd_symbolic_simple(self):
        ...
        def g(xs, y):
            out = control_flow.map(f, xs, y)
            return smith.autograd.grad(out, (xs, y), smith.ones_like(out))

        gm = make_fx(g, tracing_mode="symbolic")(
            smith.ones(3, 4, 5, requires_grad=True), smith.ones(5, requires_grad=True)
        )
        x = smith.randn(4, 5, 6, requires_grad=True)
        y = smith.randn(6, requires_grad=True)
        res = gm(x, y)
        self.assertEqual(res, g(x, y))
    ...
    def test_tracing_map_autograd_symbolic_list(self):
        ...
        def g(xs, y):
            ...
            return smith.autograd.grad(
                flat_out, requires_grad_inp, [smith.ones_like(out) for out in flat_out]
            )

        gm = make_fx(g, tracing_mode="symbolic")(
            [smith.ones(3, 4, 5), smith.ones(3, 4, 5, requires_grad=True)],
            smith.ones(5, requires_grad=True),
        )
        x = [smith.randn(4, 5, 6), smith.ones(4, 5, 6, requires_grad=True)]
        y = smith.randn(6, requires_grad=True)
        res = gm(x, y)
        self.assertEqual(res, g(x, y))
        self.check_map_count(gm, 2)
    ...
    def test_tracing_map_autograd_symbolic_dict(self):
        ...
        traced_x = {
            "a": smith.ones(3, 4, 5, requires_grad=True),
            "b": smith.ones(3, 4, 5, requires_grad=True),
        }
        gm = make_fx(g, tracing_mode="symbolic")(
            traced_x, smith.ones(5, requires_grad=True)
        )
        x = {
            "a": smith.randn(4, 5, 6, requires_grad=True),
            "b": smith.ones(4, 5, 6, requires_grad=True),
        }
        y = smith.randn(6, requires_grad=True)
        res = gm(x, y)
        self.assertEqual(res, g(x, y))
    ...
    def test_tracing_map_autograd_aot_functionalized(self):
        ...
        def f(xs, y):
            ...
            grads = smith.autograd.grad(res, (xs, y), smith.ones_like(res))
            return grads
        ...
        example_inputs = (
            smith.ones(3, 2, 4, requires_grad=True),
            smith.ones(2, 4, requires_grad=True),
        )
        gm = make_fx(f, tracing_mode="symbolic")(*example_inputs)
        fgm = make_fx(f_wrapper(f), tracing_mode="symbolic")(*example_inputs)
        xs = smith.ones(3, 4, 5, requires_grad=True)
        y = smith.ones(4, 5, requires_grad=True)
        ...
        def count_mutable(gm):
            ...
            for node in gm.graph.nodes:
                if node.op == "call_function":
                    if node.target == smith.ops.higher_order.map_impl:
    ...
    def test_map_functionalized(self):
        ...
        example_inputs = (smith.ones(3, 2, 4), smith.ones(4))
        functional_f = smith.func.functionalize(f)
        self.assertEqual(functional_f(*example_inputs), f(*example_inputs))
        ...
        gm = make_fx(smith.func.functionalize(f), tracing_mode="symbolic")(
            *example_inputs
        )
        self.assertEqual(gm(*example_inputs), f(*example_inputs))
    ...
    def test_map_functionalized_elem_mutation(self):
        ...
        functional_f = smith.func.functionalize(f)
        with self.assertRaisesRegex(
            smith._dynamo.exc.SmithRuntimeError, "map might be modifying the input!"
        ):
            functional_f(*example_inputs)
    ...
    def test_cond_autograd_backward(self):
        ...
        # Ensure no error is thrown when not running backward
        res_compiled = smith.compile(f)(*example_inputs)
        self.assertEqual(res, res_compiled)

    @skipIfSmithDynamo("Skip because we're testing export")
    def test_cond_autograd_backward_inp_out_aliasing(self):
        ...
        example_inputs = (
            smith.ones(3, 4, requires_grad=True),
            smith.ones(3, 4, requires_grad=True),
        )
        res = f(*example_inputs)
        res.sum().backward()
        res_compiled = smith.compile(f, backend=backend)(*example_inputs)
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[3, 4]", primals_2: "f32[3, 4]", gt: "b8[]", tangents_1: "f32[3, 4]"):
        true_graph_1 = self.true_graph_1
        false_graph_1 = self.false_graph_1
        cond_1 = smith.ops.higher_order.cond(gt, true_graph_1, false_graph_1, (primals_1, primals_2, tangents_1));  gt = true_graph_1 = false_graph_1 = primals_1 = primals_2 = tangents_1 = None
        getitem_1: "f32[3, 4]" = cond_1[0]
        getitem_2: "f32[3, 4]" = cond_1[1];  cond_1 = None
        return (getitem_1, getitem_2)

    class true_graph_1(smith.nn.Module):
        def forward(self, arg0_1: "f32[3, 4]", arg1_1: "f32[3, 4]", arg2_1: "f32[3, 4]"):
            clone: "f32[3, 4]" = smith.ops.aten.clone.default(arg2_1)
            clone_1: "f32[3, 4]" = smith.ops.aten.clone.default(arg2_1);  arg2_1 = None
            return [clone, clone_1]

    class false_graph_1(smith.nn.Module):
        def forward(self, arg0_1: "f32[3, 4]", arg1_1: "f32[3, 4]", arg2_1: "f32[3, 4]"):
            clone: "f32[3, 4]" = smith.ops.aten.clone.default(arg2_1)
            clone_1: "f32[3, 4]" = smith.ops.aten.clone.default(arg2_1);  arg2_1 = None
            return [clone, clone_1]
...
class GraphModule(smith.nn.Module):
    ...
    class true_graph_1(smith.nn.Module):
        def forward(self, arg0_1: "f32[3, 4]", arg1_1: "f32[3, 4]", arg2_1: "f32[3, 4]"):
            add: "f32[3, 4]" = smith.ops.aten.add.Tensor(arg0_1, arg1_1);  arg0_1 = arg1_1 = None
            cos: "f32[3, 4]" = smith.ops.aten.cos.default(add);  add = None
            mul: "f32[3, 4]" = smith.ops.aten.mul.Tensor(arg2_1, cos);  arg2_1 = cos = None
            clone: "f32[3, 4]" = smith.ops.aten.clone.default(mul)
            return [mul, clone]
...
""",  # noqa: B950
    ...
    def test_map_functionalized_elem_alias(self):
        ...
        example_inputs = (smith.ones(3, 2, 4),)
        functional_f = smith.func.functionalize(f)
        with self.assertRaisesRegex(
            # Should be
            # smith._dynamo.exc.Unsupported,
    ...
    def test_nested_map_cond_real(self):
        ...
        gm = make_fx(g, tracing_mode="real")(
            smith.tensor(True), smith.ones(3, 2, 4), smith.ones(4)
        )
        pred = smith.tensor(False)
        x = smith.randn(3, 2, 4)
        y = smith.randn(4)
    ...
    def test_nested_cond_map_cond_symbolic(self):
        ...
        gm = make_fx(main, tracing_mode="symbolic")(
            smith.tensor(True), smith.tensor(True), smith.ones(3, 2, 4), smith.ones(4)
        )
        p = smith.tensor(False)
        pred = smith.tensor(False)
        xs = smith.randn(3, 2, 2)
        y = smith.randn(2)
        res = gm(p, pred, xs, y)
    ...
    def test_cond_with_sym_pred(self):
        ...
        gm = make_fx(foo, tracing_mode="symbolic")(smith.ones(3, 2, 1))
...
def forward(self, x_1):
    sym_size_int = smith.ops.aten.sym_size.int(x_1, 0)
    eq = sym_size_int == 4
    sym_size_int_1 = smith.ops.aten.sym_size.int(x_1, 1)
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(eq, true_graph_0, false_graph_0, (x_1, sym_size_int_1, sym_size_int));  eq = true_graph_0 = false_graph_0 = x_1 = sym_size_int_1 = sym_size_int = None
    getitem = cond[0];  cond = None
    return getitem""",  # noqa: B950
        )
    ...
    def test_cond_with_unbacked_sym_pred(self):
        ...
        gm = make_fx(foo, tracing_mode="symbolic")(smith.randn(7))
        self.assertExpectedInline(
            gm.code.strip(),
            """\
def forward(self, x_1):
    nonzero = smith.ops.aten.nonzero.default(x_1)
    sym_size_int = smith.ops.aten.sym_size.int(nonzero, 0);  nonzero = None
    gt = sym_size_int > 3;  sym_size_int = None
    sym_size_int_1 = smith.ops.aten.sym_size.int(x_1, 0)
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (x_1, sym_size_int_1));  gt = true_graph_0 = false_graph_0 = x_1 = sym_size_int_1 = None
    getitem = cond[0];  cond = None
    return getitem""",  # noqa: B950
        )
...
def forward(self, x_1):
    ...
    _tensor_constant0 = self._tensor_constant0
    _tensor_constant1 = self._tensor_constant1
    cond = smith.ops.higher_order.cond(eq, true_graph_0, false_graph_0, (x_1, _tensor_constant0, sym_size_int_1, sym_size_int, _tensor_constant1));  eq = true_graph_0 = false_graph_0 = x_1 = _tensor_constant0 = sym_size_int_1 = sym_size_int = _tensor_constant1 = None
    getitem = cond[0];  cond = None
    return getitem""",  # noqa: B950
        )
        self.assertExpectedInline(
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1):
    ...
    def test_map_unfunc_boolean_tensor_for_nested_map_cond(self):
        ...
        gm = make_fx(f_wrapper(map_fn))(
            smith.tensor(True), smith.ones([2, 3], requires_grad=False)
        )
        self.assertExpectedInline(
            gm.code.strip(),
            """\
def forward(self, pred_1, x_1):
    select_copy = smith.ops.aten.select_copy.int(x_1, 0, 0);  select_copy = None
    body_graph_0 = self.body_graph_0
    map_impl = smith.ops.higher_order.map_impl(body_graph_0, [x_1], [pred_1]);  body_graph_0 = x_1 = pred_1 = None
...
def forward(self, arg0_1, arg1_1):
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(arg1_1, true_graph_0, false_graph_0, (arg0_1,));  arg1_1 = true_graph_0 = false_graph_0 = arg0_1 = None
    getitem = cond[0];  cond = None
    return (getitem,)""",  # noqa: B950
        )
    ...
    def test_cond_make_fx_preserve_stack_trace_for_nodes_in_subgraph(self):
        ...
        inp = smith.randn([4, 3])
        gm, _ = smith._dynamo.export(foo)(inp)

        def run_with_interpreter(*args):
            with smith.fx.traceback.preserve_node_meta():
                return smith.fx.Interpreter(gm).run(*args)
    ...
    def test_cond_no_dynamo_cache_limit(self):
        smith._dynamo.reset()
        counters = smith._dynamo.utils.counters
        counters.clear()
        ...
        inp = smith.ones(3, 4)
        exp_out = inp.sin()
        iter_n = smith._dynamo.config.recompile_limit + 1
    ...
    def test_cond_with_consecutive_make_fx_symbolic(self):
        ...
        inps = (smith.ones(3, 4), smith.ones(3, 5), smith.ones(5, 4), smith.ones(5, 3))
        for inp in inps:
...
def forward(self, x_1):
    ...
    return getitem""",  # noqa: B950
            ...
            self.assertExpectedInline(
                ...
                """\
def forward(self, arg0_1, arg1_1, arg2_1):
    cos = smith.ops.aten.cos.default(arg0_1)
    sub = smith.ops.aten.sub.Tensor(arg0_1, cos);  arg0_1 = cos = None
    return (sub,)""",
            )
            ...
            self.assertExpectedInline(
                ...
                """\
def forward(self, arg0_1, arg1_1, arg2_1):
    sin = smith.ops.aten.sin.default(arg0_1)
    add = smith.ops.aten.add.Tensor(arg0_1, sin);  arg0_1 = sin = None
    return (add,)""",
            )
    ...
    def test_cond_tracing_with_valid_inputs(
        ...
        operands = [smith.ones(2, 3) + i for i in range(nOperands)]
        closure = [smith.ones(2, 3) - i for i in range(nClosure)]
    ...
    def test_cond_vmap_simple(self):
        ...
        a = smith.arange(15).reshape((3, 5))
        res = smith.vmap(fn, in_dims=(0,))(a)
    ...
    def test_cond_vmap_multiple_inputs(self):
        ...
        a = smith.arange(15).reshape(3, 5)
        b = smith.ones_like(a) + 3
        res = smith.vmap(fn, in_dims=(0, 0))(a, b)
    ...
    def test_cond_vmap_multiple_args_with_closure(self):
        a = smith.ones((3, 5), dtype=smith.int64) + 3
        b = smith.arange(15).reshape(3, 5)
        c = smith.arange(5)
    ...
    def test_vmap_vmap(self, boolcond):
        def fn(x):
            return smith.cond(
                pred=smith.tensor([True]) if not boolcond else True,
        ...
        def wrapper(x):
            return smith.vmap(fn)(x)

        a = smith.ones((3, 4, 5))
        res = smith.vmap(wrapper)(a)
    ...
    def test_cond_trace_set__and_mutate_input(self):
        ...
        # graph break: smith._dynamo.exc.Unsupported: call_function DelayGraphBreakVariable() [TensorVariable()] {}
        # due to set_
        with self.assertRaisesRegex(
            smith._dynamo.exc.UncapturedHigherOrderOpError,
            r"Higher Order Operator: smith\.cond",
        ):
            smith.cond(inp.sum() > 0, f, f, (inp, tmp))
    ...
    def test_cond_trace_set__and_mutate_intermediate(self):
        ...
        class Mod(smith.nn.Module):
            def forward(self, inp: smith.Tensor, tmp: smith.Tensor) -> smith.Tensor:
                return smith.cond(inp.sum() > 0, f, f, (inp, tmp))

        with self.assertRaisesRegex(
            RuntimeError, "cannot mutate tensors with frozen storage"
        ):
            out = smith.compile(Mod(), backend="aot_eager")(inp, tmp)

        with self.assertRaisesRegex(
            RuntimeError, "cannot mutate tensors with frozen storage"
        ):
            out = smith.compile(Mod(), backend="inductor")(inp, tmp)

        backend = EagerAndRecordGraphs()
        out = smith.compile(Mod(), backend=backend)(inp, tmp)
        self.assertExpectedInline(
...
def forward(self, l_inp_, l_tmp_):
    ...
    def test_cond_symint_operands(self, requires_grad):
        ...
        a = smith.ones(3, 3, requires_grad=requires_grad)
        b = smith.ones(3, 3, requires_grad=requires_grad)
        out = smith.compile(Mod(), backend=backend, dynamic=True)(a, b)
        self.assertEqual(out, Mod()(a, b))
        self.assertEqual(len(backend.graphs), 1)
        self.assertExpectedInline(
            backend.graphs[0].code.strip(),
            """\
def forward(self, s97 : smith.SymInt, L_a_ : smith.Tensor, L_b_ : smith.Tensor):
    l_a_ = L_a_
    l_b_ = L_b_
    tensor = smith.tensor([True])
    cond_true_0 = self.cond_true_0
    cond_false_0 = self.cond_false_0
    cond = smith.ops.higher_order.cond(tensor, cond_true_0, cond_false_0, (l_a_, l_b_, s97));  tensor = cond_true_0 = cond_false_0 = l_a_ = l_b_ = s97 = None
    getitem = cond[0];  cond = None
    return (getitem,)""",  # noqa: B950
        )

    def test_two_hops_not_sharing_code_obj(self):
        pred, args = smith.tensor(True), (smith.ones(3, 3),)
        ...
        with smith._dynamo.config.patch(automatic_dynamic_shapes=True):
            cnt = CompileCounter()
            smith.compile(smith.cond, backend=cnt)(pred, fn1, fn2, args)
            self.assertEqual(cnt.frame_count, 1)
            ...
            smith.compile(smith.while_loop, backend=cnt)(cond_fn, fn2, args)
            # recompilation
            self.assertEqual(cnt.frame_count, 2)
            ...
            smith.compile(smith.cond, backend=cnt)(pred, fn1, fn2, (smith.randn(4, 4),))
            self.assertEqual(cnt.frame_count, 3)

    def test_hop_raises_if_not_overriding_call(self):
        class WrongHop(smith._ops.HigherOrderOperator):
            pass
    ...
    def test_scan_functionalized(self):
        ...
        example_inputs = smith.ones(5, 7, 4)
        example_init = smith.ones(5, 4)
        functional_f = smith.func.functionalize(f)
        self.assertEqual(
    ...
    def test_scan_functionalized_elem_mutation(self):
        ...
        with self.assertRaisesRegex(
            ...
            # This is the Exception with BLACKSMITH_TEST_WITH_DYNAMO=0
            # RuntimeError,
            # "smith.scan might be modifying the input!",
            # This is the Exception with BLACKSMITH_TEST_WITH_DYNAMO=1
            # smith._dynamo.exc.SmithDynamoException,
    ...
    def test_scan_pytree_closure(self):
        param_buffer = ({"param": smith.randn(3, 3)}, (smith.randn(3),))
        ...
        init = smith.randn(4, 3)
        xs = smith.randn(3, 3, 3)

        backend = EagerAndRecordGraphs()
        eager_out = f(init, xs)
        compiled_out = smith.compile(f, backend=backend)(init, xs)
        ...
        if TEST_WITH_CROSSREF:
            self.assertExpectedInline(
                backend.graphs[0].code.strip(),
                """\
def forward(self, L_init_ : smith.Tensor, L_xs_ : smith.Tensor, L_add_closure_0_cell_contents_0_param_ : smith.Tensor, L_add_closure_0_cell_contents_1_0_ : smith.Tensor):
    l_init_ = L_init_
    l_xs_ = L_xs_
    ...
    scan_combine_fn_0 = self.scan_combine_fn_0
    scan = smith.ops.higher_order.scan(scan_combine_fn_0, [l_init_], [l_xs_], [l_add_closure_0_cell_contents_0_param_, l_add_closure_0_cell_contents_1_0_]);  scan_combine_fn_0 = l_init_ = l_xs_ = l_add_closure_0_cell_contents_0_param_ = l_add_closure_0_cell_contents_1_0_ = None
    carry = scan[0]
    out = scan[1];  scan = None
...
def forward(self, L_init_ : smith.Tensor, L_xs_ : smith.Tensor, L_add_closure_0_cell_contents_0_param_ : smith.Tensor, L_add_closure_0_cell_contents_1_0_ : smith.Tensor):
    ...
    def test_scan_in_vmap_simple(self):
        ...
        def fn(scan_op, x, y):
            ...
            return smith.vmap(inner_fn, in_dims=(1, 0, None))(zeros, x, y)

        out = fn(scan, x, y)
        compile_out = smith.compile(fn)(scan, x, y)
    ...
    def test_scan_in_vmap_complex_ops(self):
        ...
        x = smith.randn(4, 5, 3, 2)
        init = smith.randn(4, 3, 2)
        weight = smith.randn(3, 3)

        def combine_fn(carry, xs):
            # carry: (3, 2), xs: (3, 2)
            intermediate = smith.nn.functional.relu(carry)
            xs_t = xs.transpose(0, 1)  # (2, 3)
            result = xs_t @ weight  # (2, 3)
            new_carry = intermediate + result.transpose(0, 1)  # Back to (3, 2)
            output = smith.sin(carry).sum() + smith.cos(xs).mean()
        ...
        def fn(scan_op, x, init):
            ...
            return smith.vmap(inner_fn, in_dims=(0, 0))(x, init)

        out = fn(scan, x, init)
        compile_out = smith.compile(fn)(scan, x, init)
    ...
    def test_scan_in_vmap_unbatched_x(self):
        ...
        def fn(scan_op, x, init):
            ...
            return smith.vmap(inner_fn, in_dims=(None, 0))(x, init)
    ...
    def test_scan_in_vmap_unbatched_init_error(self):
        ...
        def vmap_fn(x, init):
            ...
            return smith.vmap(fn, in_dims=(0, None))(x, init)
    ...
    @skipIfSmithDynamo("a vmap test, not a dynamo test")
    def test_vmap_closure_weight_error(self):
        init_batched = smith.randn(7, 2, 3)
        xs_batched = smith.randn(7, 5, 4)
        weight = smith.randn(7, 4, 3)
        ...
        def expected_fn(init, xs, weight):
            ...
            return smith.vmap(fn, in_dims=(0, 0, 0))(init, xs, weight)
    ...
    def test_scan_in_vmap_mixed_batch_dims(self):
        init = smith.randn(8, 5, 6)
        xs_batched = smith.randn(3, 6, 5, 8)
        scale = smith.randn([])
        ...
        def fn(scan_op, init, xs):
            ...
            return smith.vmap(inner_fn, in_dims=(2, 1))(init, xs)

        out = fn(scan, init, xs_batched)
        compile_out = smith.compile(fn)(scan, init, xs_batched)
    ...
    def test_vmap_scan_vmap_scan_nested(self):
        ...
        init = smith.randn(3, 4, 2, 8)
        xs_outer = smith.randn(3, 5, 4, 6, 2)
        ...
        def fn(scan_op, init, xs):
            ...
            def outer_combine(init, xs):
                ...
                inner_results = smith.vmap(inner_fn)(init, xs)  # (4, 8)
            ...
            return smith.vmap(vmap_inner_fn)(init, xs)

        out = fn(scan, init, xs_outer)
        compile_out = smith.compile(fn)(scan, init, xs_outer)
    ...
    def test_scan_vmap_scan_nested(self):
        xs_outer = smith.randn(5, 3, 4, 2)
        init_outer = smith.randn(3, 8)
        ...
        def fn(scan_op, init, xs):
            ...
            def outer_combine_fake(carry, xs):
                ...
                # result: (8,)
                # next_carry, (3, 8))
                result, next_carry = smith.vmap(inner_fn, in_dims=(0, 0))(carry, xs)
        ...
        compile_out = smith.compile(fn)(scan, init_outer, xs_outer)
    ...
    def test_while_loop_op_int_carry_export(self, strict, dynamic):
        ...
        dynamic_shapes = {"x": {0: smith.export.Dim("dim_x")}} if dynamic else None
...
class GraphModule(smith.nn.Module):
    def forward(self, x):
        ...
        sym_size_int_1: "Sym(s77)" = smith.ops.aten.sym_size.int(x, 0)

        while_loop_cond_graph_0 = self.while_loop_cond_graph_0
        while_loop_body_graph_0 = self.while_loop_body_graph_0
        while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (0, x), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = x = None
        getitem_2: "Sym(u1)" = while_loop[0]
        ge: "Sym(u1 >= 1)" = getitem_2 >= 1
        _assert_scalar_default = smith.ops.aten._assert_scalar.default(ge, "Runtime assertion failed for expression u1 >= 1 on node 'ge'");  ge = _assert_scalar_default = None
        gt_1: "Sym(u1 > 0)" = getitem_2 > 0
        _assert_scalar_default_1 = smith.ops.aten._assert_scalar.default(gt_1, "Runtime assertion failed for expression 0 < u1 on node 'gt_1'");  gt_1 = _assert_scalar_default_1 = None
        getitem_1: "f32[s77, 3]" = while_loop[1];  while_loop = None

        add: "Sym(u1 + 1)" = getitem_2 + 1

        add_1: "f32[s77, 3]" = smith.ops.aten.add.Tensor(getitem_1, getitem_2);  getitem_1 = None

        lt: "Sym(u1 < s77)" = getitem_2 < sym_size_int_1;  sym_size_int_1 = None

        mul: "Sym(2*u1)" = getitem_2 * 2;  getitem_2 = None
        ones: "f32[2*u1]" = smith.ops.aten.ones.default([mul], device = device(type='cpu'), pin_memory = False);  mul = None
        return pytree.tree_unflatten((add, add_1, lt, ones), self._out_spec)

    class while_loop_cond_graph_0(smith.nn.Module):
        def forward(self, it_1: "Sym(u0)", x_1: "f32[s77, 3]"):
            sym_size_int_1: "Sym(s77)" = smith.ops.aten.sym_size.int(x_1, 0);  x_1 = None

            lt: "Sym(u0 < s77)" = it_1 < sym_size_int_1;  it_1 = sym_size_int_1 = None
            return lt

    class while_loop_body_graph_0(smith.nn.Module):
        def forward(self, it_1: "Sym(u0)", x_1: "f32[s77, 3]"):
            clone: "f32[s77, 3]" = smith.ops.aten.clone.default(x_1);  x_1 = None
            select: "f32[3]" = smith.ops.aten.select.int(clone, 0, it_1)
            select_1: "f32[3]" = smith.ops.aten.select.int(clone, 0, it_1)
            add: "f32[3]" = smith.ops.aten.add.Tensor(select_1, it_1);  select_1 = None
            copy_: "f32[3]" = smith.ops.aten.copy_.default(select, add);  select = add = copy_ = None
            add_1: "Sym(u0 + 1)" = it_1 + 1;  it_1 = None
            return (add_1, clone)
""",  # noqa: B950
            )

    @skipIfSmithDynamo("Graph is not captured correctly when test with dynamo")
    @parametrize("dynamic", [True, False])
    @parametrize("backend", ["eager", "aot_eager"])
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", s27: "Sym(s27)", L_x_: "f32[s77, s27]"):
        ...
        cond_fn_0 = self.cond_fn_0
        body_fn_0 = self.body_fn_0
        while_loop = smith.ops.higher_order.while_loop(cond_fn_0, body_fn_0, (0, l_x_), (s27, s77));  cond_fn_0 = body_fn_0 = l_x_ = s27 = None
        getitem_4: "Sym(u2)" = while_loop[0]
        ge: "Sym(u2 >= 1)" = getitem_4 >= 1
        _assert_scalar_default = smith.ops.aten._assert_scalar.default(ge, "Runtime assertion failed for expression u2 >= 1 on node 'ge'");  ge = _assert_scalar_default = None
        gt_1: "Sym(u2 > 0)" = getitem_4 > 0
        _assert_scalar_default_1 = smith.ops.aten._assert_scalar.default(gt_1, "Runtime assertion failed for expression 0 < u2 on node 'gt_1'");  gt_1 = _assert_scalar_default_1 = None
        out_x: "f32[s77, s27]" = while_loop[1];  while_loop = None

        gt: "Sym(u2 > 0)" = getitem_4 > 0
        _check = smith._check(gt);  gt = _check = None
        ...
        ones: "f32[2*u2]" = smith.ones(mul);  mul = None
    ...
    class cond_fn_0(smith.nn.Module):
        def forward(self, unbacked_symint: "Sym(u0)", child: "f32[s77, s27]", s27: "Sym(s27)", s77: "Sym(s77)"):
            ...
            sym_size_int: "Sym(s77)" = smith.ops.aten.sym_size.int(child, 0)
...
""",  # noqa: B950
    ...
    def test_while_loop_op_constant_and_symint_output_export(self, strict, dynamic):
        ...
        dynamic_shapes = {"t": {0: smith.export.Dim("dim_t")}} if dynamic else None
        ep = self._check_export(m, args, strict=strict, dynamic_shapes=dynamic_shapes)
...
class GraphModule(smith.nn.Module):
    def forward(self, t):
        ...
        sum_1: "f32[]" = smith.ops.aten.sum.default(t)
        _assert_tensor_metadata_default = smith.ops.aten._assert_tensor_metadata.default(sum_1, dtype = smith.float32, device = device(type='cpu'), layout = smith.strided);  _assert_tensor_metadata_default = None
        to: "i64[]" = smith.ops.aten.to.dtype(sum_1, smith.int64);  sum_1 = None
        item: "Sym(u0)" = smith.ops.aten.item.default(to);  to = None
        sin: "f32[2, 3]" = smith.ops.aten.sin.default(t)

        while_loop_cond_graph_0 = self.while_loop_cond_graph_0
        while_loop_body_graph_0 = self.while_loop_body_graph_0
        while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (2, 3, 1, 1, 1, 3, item, sin), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = item = sin = None
        ...
        add_7: "f32[2, 3]" = smith.ops.aten.add.Tensor(getitem_7, 1)

        add_8: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_8);  getitem_8 = None
        add_9: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_9);  getitem_9 = None
        add_10: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_10);  getitem_10 = None
        add_11: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_11);  getitem_11 = None
        add_12: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_12);  getitem_12 = None
        add_13: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_13);  getitem_13 = None
        add_14: "f32[2, 3]" = smith.ops.aten.add.Tensor(t, getitem_14);  getitem_14 = None
        add_15: "f32[2, 3]" = smith.ops.aten.add.Tensor(getitem_7, t);  getitem_7 = t = None
    ...
    class while_loop_body_graph_0(smith.nn.Module):
        def forward(self, a_1: "Sym(u1)", b_1: "Sym(u2)", c1_1: "Sym(u3)", c2_1: "Sym(u4)", c3_1: "Sym(u5)", c0_1: "Sym(u6)", u0_1: "Sym(u7)", x_1: "f32[2, 3]"):
            ...
            add_1: "f32[2, 3]" = smith.ops.aten.add.Tensor(x_1, 1);  x_1 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, L_t_: "f32[2, 3]"):
        ...
        sum_1: "f32[]" = l_t_.sum()
        to: "i64[]" = sum_1.to(smith.int64);  sum_1 = None
        item: "Sym(u0)" = to.item();  to = None
        sin: "f32[2, 3]" = l_t_.sin()

        cond_fn_0 = self.cond_fn_0
        body_fn_0 = self.body_fn_0
        while_loop = smith.ops.higher_order.while_loop(cond_fn_0, body_fn_0, (2, 3, 1, 1, 1, 3, item, sin), ());  cond_fn_0 = body_fn_0 = item = sin = None
...
class GraphModule(smith.nn.Module):
    def forward(self, x):
        ...
        x, = fx_pytree.tree_flatten_spec(([x], {}), self._in_spec)
        _guards_fn = self._guards_fn(x);  _guards_fn = None
        sym_size_int_1: "Sym(s6)" = smith.ops.aten.sym_size.int(x, 0)

        sin: "f32[s6, 3]" = smith.ops.aten.sin.default(x);  x = None

        while_loop_cond_graph_0 = self.while_loop_cond_graph_0
        while_loop_body_graph_0 = self.while_loop_body_graph_0
        while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_0, while_loop_body_graph_0, (sym_size_int_1, 3, 2, 2, 3, sin), ());  while_loop_cond_graph_0 = while_loop_body_graph_0 = sym_size_int_1 = sin = None
        ...
        add_3: "f32[s6, 3]" = smith.ops.aten.add.Tensor(getitem_5, getitem_8);  getitem_8 = None
        add_4: "f32[s6, 3]" = smith.ops.aten.add.Tensor(getitem_5, getitem_9);  getitem_9 = None
        add_5: "f32[s6, 3]" = smith.ops.aten.add.Tensor(getitem_5, getitem_10);  getitem_10 = None
    ...
    class while_loop_body_graph_0(smith.nn.Module):
        def forward(self, arg0_1: "Sym(u15)", arg1_1: "Sym(u16)", arg2_1: "Sym(u17)", arg3_1: "Sym(u18)", arg4_1: "Sym(u19)", arg5_1: "f32[s6, 3]"):
            ...
            add_5: "f32[s6, 3]" = smith.ops.aten.add.Tensor(arg5_1, 1);  arg5_1 = None
...
class GraphModule(smith.nn.Module):
    def forward(self, s77: "Sym(s77)", s27: "Sym(s27)", L_x_: "f32[s77, s27]"):
        ...
        cond_fn_0 = self.cond_fn_0
        body_fn_0 = self.body_fn_0
        while_loop = smith.ops.higher_order.while_loop(cond_fn_0, body_fn_0, (s77, s27, 2, 2, 3, child), (s27, s77));  cond_fn_0 = body_fn_0 = s77 = s27 = child = None
...
""",  # noqa: B950
    ...
    def test_compile_while_loop_stack_output(self, dynamic, backend):
        class Mod(smith.nn.Module):
            ...
            def forward(self, x):
                c = smith.tensor(0, dtype=smith.int64)
        ...
        x = smith.randn(3, 3)
        mod = Mod()
        compiled_out = smith.compile(mod, backend=backend, dynamic=dynamic)(x)
    ...
    def test_while_loop_autograd_simple(self):
        ...
        class ModEager(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.linear = smith.nn.Linear(3, 3)
        ...
        class Mod(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                return smith._higher_order_ops.while_loop(cond_fn, body_fn, (x,))
        ...
        compiled_out = smith.compile(mod, backend=backend, fullgraph=True)(x)
        ...
        self.assertEqual(
            len(
                backend.fw_graphs[0].graph.find_nodes(
                    op="call_function",
                    target=smith.ops.higher_order.while_loop_stack_output,
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                backend.bw_graphs[0].graph.find_nodes(
                    op="call_function", target=smith.ops.higher_order.while_loop
                )
            ),
            1,
        )
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_1: "f32[3, 3]", primals_2: "f32[3, 3]", primals_3: "f32[3]"):
        while_loop_cond_graph_0 = self.while_loop_cond_graph_0
        while_loop_body_graph_0 = self.while_loop_body_graph_0
        while_loop_stack_output = smith.ops.higher_order.while_loop_stack_output(while_loop_cond_graph_0, while_loop_body_graph_0, (primals_1,), (primals_3, primals_2));  while_loop_cond_graph_0 = while_loop_body_graph_0 = None
        getitem: "f32[u2, 3, 3]" = while_loop_stack_output[0];  while_loop_stack_output = None
        select: "f32[3, 3]" = smith.ops.aten.select.int(getitem, 0, -1)
        unsqueeze: "f32[1, 3, 3]" = smith.ops.aten.unsqueeze.default(primals_1, 0);  primals_1 = None
        slice_1: "f32[u2 - 1, 3, 3]" = smith.ops.aten.slice.Tensor(getitem, 0, 0, -1);  getitem = None
        cat: "f32[u2, 3, 3]" = smith.ops.aten.cat.default([unsqueeze, slice_1]);  unsqueeze = slice_1 = None
        return (select, primals_2, primals_3, cat)

    class while_loop_cond_graph_0(smith.nn.Module):
        def forward(self, arg0_1: "f32[3, 3]", arg1_1: "f32[3]", arg2_1: "f32[3, 3]"):
            sum_1: "f32[]" = smith.ops.aten.sum.default(arg0_1);  arg0_1 = None
            lt: "b8[]" = smith.ops.aten.lt.Scalar(sum_1, 2);  sum_1 = None
            return lt

    class while_loop_body_graph_0(smith.nn.Module):
        def forward(self, arg0_1: "f32[3, 3]", arg1_1: "f32[3]", arg2_1: "f32[3, 3]"):
            mul: "f32[3, 3]" = smith.ops.aten.mul.Tensor(arg0_1, arg0_1)
            add: "f32[3, 3]" = smith.ops.aten.add.Tensor(mul, 1);  mul = None
            t: "f32[3, 3]" = smith.ops.aten.t.default(arg2_1);  arg2_1 = None
            addmm: "f32[3, 3]" = smith.ops.aten.addmm.default(arg1_1, arg0_1, t);  arg1_1 = arg0_1 = t = None
            add_1: "f32[3, 3]" = smith.ops.aten.add.Tensor(add, addmm);  add = addmm = None
            return (add_1,)
""",  # noqa: B950
            )
...
class GraphModule(smith.nn.Module):
    def forward(self, primals_2: "f32[3, 3]", primals_3: "f32[3]", cat: "f32[u2, 3, 3]", tangents_1: "f32[3, 3]"):
        zeros: "i64[]" = smith.ops.aten.zeros.default([], dtype = smith.int64, device = device(type='cpu'), pin_memory = False)
        zeros_like: "f32[3]" = smith.ops.aten.zeros_like.default(primals_3, pin_memory = False)
        zeros_like_1: "f32[3, 3]" = smith.ops.aten.zeros_like.default(primals_2, pin_memory = False)
        while_loop_cond_graph_1 = self.while_loop_cond_graph_1
        while_loop_body_graph_1 = self.while_loop_body_graph_1
        while_loop = smith.ops.higher_order.while_loop(while_loop_cond_graph_1, while_loop_body_graph_1, (zeros, tangents_1, zeros_like, zeros_like_1), (cat, primals_3, primals_2));  while_loop_cond_graph_1 = while_loop_body_graph_1 = zeros = tangents_1 = zeros_like = zeros_like_1 = cat = primals_3 = primals_2 = None
    ...
    class while_loop_cond_graph_1(smith.nn.Module):
        def forward(self, arg0_1: "i64[]", arg1_1: "f32[3, 3]", arg2_1: "f32[3]", arg3_1: "f32[3, 3]", arg4_1: "f32[u2, 3, 3]", arg5_1: "f32[3]", arg6_1: "f32[3, 3]"):
            sym_size_int_1: "Sym(u2)" = smith.ops.aten.sym_size.int(arg4_1, 0);  arg4_1 = None

            lt: "b8[]" = smith.ops.aten.lt.Scalar(arg0_1, sym_size_int_1);  arg0_1 = sym_size_int_1 = None
            return lt

    class while_loop_body_graph_1(smith.nn.Module):
        def forward(self, arg0_1: "i64[]", arg1_1: "f32[3, 3]", arg2_1: "f32[3]", arg3_1: "f32[3, 3]", arg4_1: "f32[u2, 3, 3]", arg5_1: "f32[3]", arg6_1: "f32[3, 3]"):
            sym_size_int_1: "Sym(u2)" = smith.ops.aten.sym_size.int(arg4_1, 0)

            rsub: "i64[]" = smith.ops.aten.rsub.Scalar(arg0_1, sym_size_int_1);  sym_size_int_1 = None
            sub_1: "i64[]" = smith.ops.aten.sub.Tensor(rsub, 1);  rsub = None
            _local_scalar_dense: "Sym(u7)" = smith.ops.aten._local_scalar_dense.default(sub_1);  sub_1 = None
            select: "f32[3, 3]" = smith.ops.aten.select.int(arg4_1, 0, _local_scalar_dense);  arg4_1 = _local_scalar_dense = None
            t: "f32[3, 3]" = smith.ops.aten.t.default(arg6_1);  arg6_1 = None
            t_1: "f32[3, 3]" = smith.ops.aten.t.default(t);  t = None
            mm: "f32[3, 3]" = smith.ops.aten.mm.default(arg1_1, t_1);  t_1 = None
            t_2: "f32[3, 3]" = smith.ops.aten.t.default(arg1_1)
            mm_1: "f32[3, 3]" = smith.ops.aten.mm.default(t_2, select);  t_2 = None
            t_3: "f32[3, 3]" = smith.ops.aten.t.default(mm_1);  mm_1 = None
            sum_1: "f32[1, 3]" = smith.ops.aten.sum.dim_IntList(arg1_1, [0], True)
            view: "f32[3]" = smith.ops.aten.view.default(sum_1, [3]);  sum_1 = None
            t_4: "f32[3, 3]" = smith.ops.aten.t.default(t_3);  t_3 = None
            mul_4: "f32[3, 3]" = smith.ops.aten.mul.Tensor(arg1_1, select)
            mul_5: "f32[3, 3]" = smith.ops.aten.mul.Tensor(arg1_1, select);  arg1_1 = select = None
            add_7: "f32[3, 3]" = smith.ops.aten.add.Tensor(mm, mul_5);  mm = mul_5 = None
            add_8: "f32[3, 3]" = smith.ops.aten.add.Tensor(add_7, mul_4);  add_7 = mul_4 = None
            add_9: "i64[]" = smith.ops.aten.add.Tensor(arg0_1, 1);  arg0_1 = None
            add_10: "f32[3]" = smith.ops.aten.add.Tensor(view, arg2_1);  view = arg2_1 = None
            add_11: "f32[3, 3]" = smith.ops.aten.add.Tensor(t_4, arg3_1);  t_4 = arg3_1 = None
...
""",  # noqa: B950
    ...
    def test_input_output_alias(self):
        def fn(f, *args):
            return smith.cond(args[0].sum() > 0, f, f, args)
    ...
    def test_input_input_alias(self):
        def fn(view_f, arg):
            ...
            return smith.cond(arg.sum() > 0, f, f, (arg, view_f(arg)))
    ...
    def test_input_mutation(self, inference_mode):
        def fn(view_f, *args):
            ...
            return smith.cond(args[0].sum() > 0, mutate_f, mutate_f, args)
        ...
        for f in ALIAS_FN:
            ...
            with self.assertRaisesRegex(
                ...
                with smith.inference_mode(inference_mode):
                    smith.compile(fn)(f, x)
    ...
    def _check_export_ret_graph_str(self, fn, args, dynamic_shapes=None) -> str:
        strict_ep = smith.export.export(
            fn, args, dynamic_shapes=dynamic_shapes, strict=True
        )
        non_strict_ep = smith.export.export(
    ...
    @skipIfSmithDynamo("Skip because dynamo cannot trace smith.export.")
    def test_cond_eager_run_with_item(self):
        ...
        model = M()
        smith.export.export(model, args, strict=True)
...
class GraphModule(smith.nn.Module):
    def forward(self, a, b1, b2, c):
        ...
        true_graph_0 = self.true_graph_0
        false_graph_0 = self.false_graph_0
        cond = smith.ops.higher_order.cond(a, true_graph_0, false_graph_0, (c, b1, b2));  a = true_graph_0 = false_graph_0 = c = b1 = b2 = None
        getitem: "f32[10]" = cond[0];  cond = None

        mul: "f32[10]" = smith.ops.aten.mul.Tensor(getitem, 2);  getitem = None
    ...
    class true_graph_0(smith.nn.Module):
        def forward(self, c: "f32[10]", b1: "i64[1]", b2: "i64[1]"):
            item: "Sym(u0)" = smith.ops.aten.item.default(b1);  b1 = None

            mul: "f32[10]" = smith.ops.aten.mul.Tensor(c, item);  c = item = None
            return (mul,)

    class false_graph_0(smith.nn.Module):
        def forward(self, c: "f32[10]", b1: "i64[1]", b2: "i64[1]"):
            item: "Sym(u1)" = smith.ops.aten.item.default(b2);  b2 = None
    ...
    def test_cond_merge_graph_preserves_ph_meta(self):
        class M(smith.nn.Module):
            def forward(self, x, y, z):
                ...
                return smith.cond(x.sum() > 5, true_fn, false_fn, (x,))

        backend = EagerAndRecordGraphs()
        _ = smith.compile(M(), backend=backend)(
            smith.randn(3, 4), smith.randn(3, 4), smith.randn(3, 4)
        )
        self.assertEqual(len(backend.graphs), 1)
        gm = backend.graphs[0]
    ...
    def test_cond_symint_closure(self):
        ...
        class M(smith.nn.Module):
            def forward(self, x, y, z):
                ...
                # so smith.compile need to wrap and trace symint inputs.
                return smith.cond(x.shape[0] > 5, true_fn, false_fn, (x,))

        args = (smith.ones(3, 3), smith.ones(5), smith.ones(3, 3))
        model = M()
...
class GraphModule(smith.nn.Module):
    def forward(self, x, y, z):
        ...
        sym_size_int_4: "Sym(s17)" = smith.ops.aten.sym_size.int(y, 0);  y = None
        sym_size_int_5: "Sym(s68)" = smith.ops.aten.sym_size.int(z, 0)

        gt: "Sym(s68 > 5)" = sym_size_int_5 > 5

        true_graph_0 = self.true_graph_0
        false_graph_0 = self.false_graph_0
        cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (x, sym_size_int_4, sym_size_int_5, z));  gt = true_graph_0 = false_graph_0 = x = sym_size_int_4 = sym_size_int_5 = z = None
    ...
    class true_graph_0(smith.nn.Module):
        def forward(self, x: "f32[s68, 3]", sym_size_int_4: "Sym(s17)", sym_size_int_5: "Sym(s68)", z: "f32[s68, 3]"):
            add: "f32[s68, 3]" = smith.ops.aten.add.Tensor(x, sym_size_int_4);  x = sym_size_int_4 = None
            return (add,)

    class false_graph_0(smith.nn.Module):
        def forward(self, x: "f32[s68, 3]", sym_size_int_4: "Sym(s17)", sym_size_int_5: "Sym(s68)", z: "f32[s68, 3]"):
            mul: "f32[s68, 3]" = smith.ops.aten.mul.Tensor(z, sym_size_int_5);  z = sym_size_int_5 = None

            add: "f32[s68, 3]" = smith.ops.aten.add.Tensor(x, mul);  x = mul = None
            return (add,)
""",  # noqa: B950
        )
    ...
    def test_cond_unbacked_symint_closure(self):
        ...
        args = (smith.ones(3, 3), smith.ones(5, dtype=smith.int32), smith.ones(3, 3))
        model = M()
        dynamic_shapes = {"x": {0: Dim("d")}, "y": {0: Dim("d1")}, "z": {0: Dim("d")}}
    ...
    def test_merge_output(self):
        from smith._higher_order_ops.cond import _merge_output
        ...
        def _inner(case):
            ...
            with fake_mode:
                t1 = smith.empty_strided(size1, stride1)
                t2 = smith.empty_strided(size2, stride2)
    ...
    def test_cond_mismatched_branch_output(self, dynamic, backend):
        class M(smith.nn.Module):
            def forward(self, x, y, z):
                ...
                ret = smith.cond(x.sum() > 0, true_fn, false_fn, (x,))
                return y.sum() - ret

        m = M()
        x, y, z = smith.randn(5, 4), smith.randn(5, 4), smith.randn(5, 4)
        out = m(x, y, z)
        if not (backend == "eager" and dynamic and not TEST_WITH_CROSSREF):
            compiled_out = smith.compile(
...
class GraphModule(smith.nn.Module):
    def forward(self, s17: "Sym(s17)", s94: "Sym(s94)", L_y_: "f32[s17, s94]", L_z_: "f32[s17, s94]", L_x_: "f32[s17, s94]"):
        ...
        cond_true_0 = self.cond_true_0
        cond_false_0 = self.cond_false_0
        cond = smith.ops.higher_order.cond(gt, cond_true_0, cond_false_0, (l_x_, s94, s17, s17, l_z_));  gt = cond_true_0 = cond_false_0 = l_x_ = s94 = s17 = l_z_ = None
        getitem_5: "f32[u0, s94]" = cond[0]
        sym_size_int: "Sym(u0)" = smith.ops.aten.sym_size.int(getitem_5, 0);  getitem_5 = None
    ...
    class cond_true_0(smith.nn.Module):
    ...
    class cond_false_0(smith.nn.Module):
        def forward(self, l_x_: "f32[s17, s94]", s94: "Sym(s94)", s17_true_branch: "Sym(s17)", getitem_2_false_branch: "Sym(s17)", l_z__false_branch: "f32[s17, s94]"):
            l_x__1 = l_x_
            s94_1 = s94
            ...
            def forward(self, x, y):
                ...
                def false_fn(x, y):
                    return (
                        ...
                        smith.empty_strided((4, 5), (0, 1)),
                    )

                ret = smith.cond(x.sum() > 0, true_fn, false_fn, (x, y))
                return y.sum() + ret[0]

        m = M()
        x, y = smith.randn(1, 6, 1, 5, 4, 3), smith.randn(1, 4, 5, 1, 3, 8)
        out = m(x, y)
        compiled_out = smith.compile(
            m, backend=backend, dynamic=dynamic, fullgraph=True
        )(x, y)
...
@skipIfSmithDynamo("We don't expect users to smith.compile hop schema generation.")
@unittest.skipIf(IS_WINDOWS, "Windows not supported for this test")
class TestHopSchema(TestCase):
    def _get_example_val(self, ty: str):
        from smith.fx.experimental.sym_node import SymNode
        from smith.fx.experimental.symbolic_shapes import ShapeEnv

        def create_symtype(cls, pytype, shape_env, val):
            from smith._dynamo.source import ConstantSource
        ...
        if ty == "bool":
            ...
            shape_env = ShapeEnv()
            return create_symtype(smith.SymInt, int, shape_env, 1)
        elif ty == "SymBool":
            shape_env = ShapeEnv()
            return create_symtype(smith.SymBool, bool, shape_env, True)
        elif ty == "GraphModule":

            def f(x):
                return x.sin()

            return make_fx(f)(smith.ones(1))
        elif ty == "ScriptObj":
            ...
            init_smithbind_implementations()
            foo = smith.classes._SmithScriptTesting._Foo(3, 4)
            return foo
        else:
    ...
    def test_type_gen(self, schema_type):
        from smithgen.gen_schema_utils import TypeGen
    ...
    def test_function_schema_gen(self):
        from smithgen.gen_schema_utils import FunctionSchemaGen

        inps = [
            (schema_type + "_v", self._get_example_val(schema_type))
            for schema_type in _hop_schema_test_schema_types
        ]
        schema1 = FunctionSchemaGen.from_example("test_op1", inps, smith.ones(1))
        ...
        schema3 = FunctionSchemaGen.from_example(
            "test_op3", inps, [smith.ones(1), smith.ones(1)]
        )
        self.assertExpectedInline(
            str(schema1),
            """test_op1(bool bool_v, int int_v, float float_v, str str_v, Tensor Tensor_v, SymInt SymInt_v, SymBool SymBool_v, GraphModule GraphModule_v, __smith__.smith.classes._Foo ScriptObj_v) -> Tensor""",  # noqa: B950
        )
        self.assertExpectedInline(
            str(schema2),
            """test_op2(bool bool_v, int int_v, float float_v, str str_v, Tensor Tensor_v, SymInt SymInt_v, SymBool SymBool_v, GraphModule GraphModule_v, __smith__.smith.classes._Foo ScriptObj_v) -> Tensor""",  # noqa: B950
        )
        self.assertExpectedInline(
            str(schema3),
            """test_op3(bool bool_v, int int_v, float float_v, str str_v, Tensor Tensor_v, SymInt SymInt_v, SymBool SymBool_v, GraphModule GraphModule_v, __smith__.smith.classes._Foo ScriptObj_v) -> (Tensor, Tensor)""",  # noqa: B950,
        )
    ...
    def test_schema_tree_spec(self):
        schema_gen = HopSchemaGenerator(smith.ops.higher_order.cond)
        args = (smith.randn(3, 4), smith.randn(2, 3))
        with self.assertRaisesRegex(
    ...
    def test_cond_gen_schema_tensor_inputs(self):
        schema = smith.ops.higher_order.cond.gen_schema(
            smith.tensor(True),
            lambda x: x.sin(),
            lambda x: x.cos(),
            (smith.randn(3, 4),),
    ...
    def test_cond_gen_schema_symbool_inputs(self):
        ...
        with fake_mode, fake_mode.shape_env.ignore_fresh_unbacked_symbols():
            sym_bool = smith.randn(3, 4).nonzero().size(0) == 0
    ...
    def test_while_loop_gen_schema_tensor_inputs(self):
        ...
        schema = smith.ops.higher_order.while_loop.gen_schema(
            cond_fn,
            body_fn,
            (smith.randn(3, 4), smith.randn(2, 3)),
            (),
        )
        self.assertExpectedInline(
            str(schema),
    ...
    def test_scan_gen_schema_tensor_inputs(self):
        ...
        schema = smith.ops.higher_order.scan.gen_schema(
            combine_fn,
            (smith.randn(3, 4),),
            (smith.randn(5, 3, 4),),
            (),
    ...
    def test_scan_gen_schema_multiple_inputs(self):
        ...
        schema = smith.ops.higher_order.scan.gen_schema(
            combine_fn,
            (smith.randn(3, 4), smith.randn(2, 3)),
            (smith.randn(5, 3, 4), smith.randn(5, 2, 3)),
            (),
        )
        self.assertExpectedInline(
            str(schema),
    ...
    def test_associative_scan_gen_schema_tensor_inputs(self):
        ...
        schema = smith.ops.higher_order.associative_scan.gen_schema(
            combine_fn,
            (smith.randn(5, 3, 4),),
            (),
        )
    ...
    def test_while_loop_gen_schema_with_input_mutation(self):
        ...
        schema = smith.ops.higher_order.while_loop.gen_schema(
            cond_fn,
            body_fn,
            (smith.randn(3, 3), smith.randn(3, 3), smith.randn(3, 3)),
            (c,),
        )
        self.assertExpectedInline(
            str(schema),
