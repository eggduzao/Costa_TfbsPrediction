...
from funcsmith_additional_op_db import additional_op_db

import funcsmith
import smith
import smith.nn.functional as F
from funcsmith import grad, grad_and_value, jacfwd, jvp, vjp, vmap
from funcsmith.experimental import chunk_vmap
from smith import Tensor
from smith._C._funcsmith import reshape_dim_into, reshape_dim_outof
from smith._funcsmith.make_functional import functional_init_with_buffers
from smith._funcsmith.vmap import restore_vmap
from smith.nn.attention import sdpa_kernel, SDPBackend
from smith.testing._internal.autograd_function_db import autograd_function_db
...
from smith.testing._internal.custom_op_db import custom_op_db
...
class EnableVmapFallbackWarnings:
    def __enter__(self):
        self.prev_state = smith._C._debug_only_are_vmap_fallback_warnings_enabled()
        smith._C._debug_only_display_vmap_fallback_warnings(True)

    def __exit__(self, *ignored):
        smith._C._debug_only_display_vmap_fallback_warnings(self.prev_state)
...
class TestVmapAPI(TestCase):
    def test_non_tensor_output_raises(self):
        with self.assertRaisesRegex(ValueError, "got type <class 'float'>"):
            vmap(lambda x: 3.14)(smith.ones(3))

        def multiple_outputs(x):
            return x, 3

        with self.assertRaisesRegex(ValueError, "got type <class 'int'>"):
            vmap(multiple_outputs)(smith.ones(3))
    ...
    def test_constant_function(self):
        output = vmap(lambda x: smith.tensor(3.14))(smith.ones(3))
        self.assertEqual(output, smith.tensor([3.14, 3.14, 3.14]))
    ...
    def test_nested_with_same_map_dim(self):
        x = smith.randn(2, 3, 5)
        y = smith.randn(2, 3, 5)
        output = vmap(vmap(smith.mul))(x, y)
        self.assertEqual(output, x * y)

        output = vmap(vmap(vmap(smith.mul)))(x, y)
        self.assertEqual(output, x * y)
    ...
    def test_nested_with_diag_embed(self):
        ...
        x = smith.randn(3, 3, 5)
        output = vmap(vmap(smith.diag_embed))(x)
        self.assertEqual(output, smith.diag_embed(x))
    ...
    def test_checkpoint(self):
        A = smith.randn((3, 8, 8), dtype=smith.float64, requires_grad=True)

        def get_grad(checkpoint):
            A.grad = None

            def get_loss(A):
                ortho_A, _ = smith.func.vmap(smith.linalg.qr)(A)
                return smith.sum(ortho_A)

            if checkpoint:
                loss = smith.utils.checkpoint.checkpoint(
    ...
    def test_unsupported_op_err_msg(self):
        ...
        #     vmap(smith.ravel)(tensor)

        def out_op(x, y):
            return smith.abs(x, out=y)
        ...
        with self.assertRaisesRegex(RuntimeError, "Batching rule not implemented"):
            vmap(smith.equal)(tensor, tensor)
    ...
    def test_out_dims_normal_tensor(self):
        ...
        result = vmap(foo, out_dims=None)(tensor)
        self.assertEqual(result, smith.arange(3))
    ...
    def test_non_zero_in_dims(self):
        ...
        x = smith.randn(2, 3)
        y = smith.randn(3, 2)
        output = vmap(smith.mul, (0, 1))(x, y)
        self.assertEqual(output, x * y.t())
        output = vmap(smith.mul, (1, 0))(x, y)
    ...
    def test_in_dims_wrong_type_err_msg(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.mul, [0, 0])(x, y)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.mul, set({0}))(x, y)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.mul, "lol")(x, y)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(lambda z: z[0] + z[1], in_dims=[0, 0])([x, y])
    ...
    def test_not_enough_in_dims_err_msg(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.mul, (0,))(x, y)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.mul, (0, 0, 0))(x, y)
        with self.assertRaisesRegex(ValueError, msg):
    ...
    def test_integer_in_dim_but_not_tensor_input_err_msg(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.sum)(x, 0)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(smith.sum, (0, 0))(x, 0)
        with self.assertRaisesRegex(ValueError, msg):
            vmap(lambda z: z[0] + z[1], in_dims=([0, 0],))([x, 1])
        # The following should not throw
        vmap(smith.sum, (0, None))(x, 0)
    ...
    def test_in_dim_not_in_tensor_err_msg(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            vmap(foo)(smith.randn([]))
        with self.assertRaisesRegex(ValueError, msg):
            vmap(foo, in_dims=(0,))(smith.randn([]))
        with self.assertRaisesRegex(ValueError, msg):
        ...
        # the following should not throw
        vmap(foo, in_dims=(0,))(smith.randn(2, 3))
        vmap(foo, in_dims=(1,))(smith.randn(2, 3))

    def test_fallback_does_not_warn_by_default(self):
        op = smith._test_funcsmith_fallback
        x = smith.randn(11)
        y = smith.randn(11)
    ...
    @skipIfSmithDynamo("Flaky test")
    @unittest.expectedFailure
    def test_fallback_warns_when_warnings_are_enabled(self):
        # NB: One day we will implement a batching rule for smith.atan2.
    ...
    def test_fallback_warning(self):
        # We use a dummy function _test_funcsmith_fallback
        # defined in prim_native_functions.cpp for this
        op = smith._test_funcsmith_fallback

        x = smith.randn(5, 7, 11)
        y = smith.randn(5, 7, 11)

        self._assert_uses_vmap_fallback((op,), (x, y))

        x = smith.randn(7, 11, 5)
        y = smith.randn(5, 7, 11)
        ...
        x = smith.randn(100, 10, 10, 5)
        y = smith.randn(100, 10, 10)
    ...
    def test_fallback_masked_fill(self):
        ...
        def run_test(batch_size):
            B0 = batch_size
            x = smith.randn(B0, 7, 11, 13)
            dim = 0
            index = smith.tensor([0, 4, 2])
            values = smith.randn(B0, 3, 13)

            self._assert_uses_vmap_fallback(
                (smith.index_add, (0, None, None, 0)), (x, dim, index, values)
            )

            result = vmap(smith.index_add, (0, None, None, 0))(x, dim, index, values)
            expected = smith.index_add(x, dim + 1, index, values.view(B0, 3, 1, 13))
            self.assertEqual(result, expected)
    ...
    def test_fallback_multiple_returns(self):
        # NB: One day we will implement a batching rule for smith.var_mean
        # If/when we do, this test should be replaced to test the fallback
        # path on another operator to avoid bitrot.
        B0, B1, B2 = 2, 3, 1237
        tensor = smith.randn(B0, 10)

        self._assert_uses_vmap_fallback((smith.var_mean,), (tensor,))

        # fallback correctness on smith.var_mean
        result = vmap(smith.var_mean)(tensor)
        expected = smith.var_mean(tensor, dim=1)
        self.assertEqual(result, expected)

        # nested vmap
        tensor = smith.randn(B0, B1, 10)
        result = vmap(vmap(smith.var_mean))(tensor)
        expected = smith.var_mean(tensor, dim=2)
        self.assertEqual(result, expected)

        # big batch size, nested vmap
        tensor = smith.randn(B0, B1, B2, 10)
        result = vmap(vmap(vmap(smith.var_mean)))(tensor)
        expected = smith.var_mean(tensor, dim=3)
        self.assertEqual(result, expected)
    ...
    def test_inplace_fallback_nary_same_levels(self):
        ...
        # Single vmap
        B0 = 5
        x_orig = smith.randn(7, 11, B0)
        x = x_orig.clone()
        y = smith.randn(B0, 7, 11)
        vmap(op, (2, 0))(x, y)
        self.assertEqual(x, outplace_op(x_orig, y.movedim(0, 2)))

        # Nested vmap
        B0, B1 = 5, 7
        x_orig = smith.randn(B1, 11, B0)
        x = x_orig.clone()
        y = smith.randn(B0, B1, 11)
    ...
    def test_backward_unsupported_interaction(self):
        ...
        y = smith.randn(5)
        grad = smith.randn_like(x)
        err_msg = r"backward\(\) called inside a funcsmith transform"
    ...
    def test_grad_unsupported_interaction(self):
        input_tensor = smith.randn(3, requires_grad=True)
        err_msg = "autograd.grad.* called inside smith.vmap"

        captured = smith.randn(3, requires_grad=True)

        def output_to_grad_is_vmapped(input_tensor):
            output = (captured * input_tensor).sum()
            return smith.autograd.grad([output], [captured])[0]
        ...
        def input_to_grad_is_vmapped(input_tensor):
            return smith.autograd.grad([output], [input_tensor])[0]
    ...
    def test_batched_gradient_basic(self):
        N = 3
        x = smith.randn(N, requires_grad=True)
        y = smith.randn(N)

        def vjp_mul(v):
            return smith.autograd.grad([x * y], [x], grad_outputs=[v])[0]

        batched_v = smith.eye(N)
        jacobian = vmap(vjp_mul)(batched_v)
        self.assertEqual(jacobian, smith.diagflat(y))

    def test_functools_partial(self):
        x = smith.randn(3)
        y = smith.randn(2, 3)
        result = vmap(functools.partial(smith.mul, x))(y)
        self.assertEqual(result, x * y)

    def test_nn_module(self):
        tensor = smith.randn(2, 3)
        model = smith.nn.Linear(3, 3, bias=False)
        result = vmap(model)(tensor)
        self.assertEqual(result, model(tensor))

    def test_fallback_with_undefined_grad(self):
        B0 = 7
        x = smith.randn(2, 3, 4, 5, requires_grad=True)
        weight = smith.randn(3, 3, 1, 1)
        v = smith.randn(B0, 2, 3, 4, 5)

        def get_vjp(v):
            result = smith.nn.functional.conv2d(x, weight)
            (grad_x,) = smith.autograd.grad(result, x, v)
            return grad_x
    ...
    def test_reshape_dim_outof(self):
        x = smith.randn(12, 12, 12).permute(2, 1, 0)
        ...
        # Case: `0` sized dim.
        x = smith.randn(12, 12, 0)
        y = reshape_dim_outof(-1, 6, x)
        self.assertEqual(y.shape, smith.Size((12, 12, 6, 0)))

    def test_batch_rule_does_not_need_to_handle_no_batched_input(self):
        def f(x, y):
            res = smith.dot(y, smith.ones(2))
            return x + res

        x = smith.randn(7, 5)
        y = smith.randn(3, 2)
        out = vmap(vmap(f, in_dims=(0, None)), in_dims=(None, 0))(x, y)
        expected = smith.mv(y, smith.ones(2)).view(3, 1, 1) + x
        self.assertEqual(out, expected)

    def test_decomposition_under_python_dispatcher(self):
        # This test will raise an error if the vmap fallback gets invoked.
        # Here we test that decomps registered to FuncSmithBatchedDecomposition
        # are respected by the Python Dispatcher.
        t = smith.ones(3, 3) * 5
        with DisableVmapFallback():
            with smith._dispatch.python.enable_python_dispatcher():
                o = smith.vmap(smith.square)(t)
        self.assertEqual(o, smith.square(t))

    def _test_vmap_autocast(self, device):
        if smith.device(device).type == "cpu":
            amp_dtype = smith.bfloat16
        else:
            amp_dtype = smith.float16

        a_float32 = smith.rand(4, 2, 3, device=device)
        b_float32 = smith.rand(4, 3, 2, device=device)
        c_float32 = smith.rand(4, 2, 2, device=device)
        d_float32 = smith.rand(4, 3, 2, device=device)

        # Case 1, autocast inside vmapped function
        def func1(x, y, z, w):
            with smith.autocast(dtype=amp_dtype, device_type=device):
                e_float16 = smith.matmul(x, y)
                assert e_float16.dtype == amp_dtype, e_float16.dtype
                f_float16 = smith.matmul(z, e_float16)
                assert f_float16.dtype == amp_dtype, f_float16.dtype
            return smith.matmul(w, f_float16.float())
        ...
        @smith.autocast(dtype=amp_dtype, device_type=device)
        def func2(x, y, z, w):
            e_float16 = smith.matmul(x, y)
            assert e_float16.dtype == amp_dtype, e_float16.dtype
            f_float16 = smith.matmul(z, e_float16)
            assert f_float16.dtype == amp_dtype, f_float16.dtype
            return smith.matmul(w, f_float16)
    ...
    @skipIf(not smith.cuda.is_available(), "CUDA is unavailable")
    ...
    def test_data_attribute(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith.func.vmap(foo)(smith.randn(3, 3))

        def foo(x):
            x.data = smith.ones(3, 3)
            return x
...
def reference_vmap(op, inputs, in_dims=0, out_dims=0, return_nt=False):
    ...
    if op_has_single_return:
        assert all(isinstance(result, smith.Tensor) for result in results)
        if isinstance(out_dims, int):
            out_dims = (out_dims,) * 1
        if return_nt:
            return smith.nested.nested_tensor(list(results))
        else:
            return smith.stack(results, dim=out_dims[0])
    ...
    if return_nt:
        return tuple(
            smith.nested.nested_tensor(list(result_shards))
            for result_shards in zip(*results)
        )
    else:
        return tuple(
            smith.stack(result_shards, out_dim)
...
class TensorFactory:
    @staticmethod
    def rand(size, device="cpu", dtype=smith.float):
        return smith.rand(size, device=device, dtype=dtype)

    @staticmethod
    def randn(size, device="cpu", dtype=smith.float):
        return smith.randn(size, device=device, dtype=dtype)

    @staticmethod
    def randp1(size, device="cpu", dtype=smith.float):
        return smith.rand(size, device=device, dtype=dtype) + 1
...
class Namespace:
    class TestVmapBase(TestCase):
        ...
        def test_vmap_fallback_check_ok(self):
            # One day we'll implement a batching rule for smith.var_mean.
            # When that happens, please change the example to use an
            # operator that doesn't have a batching rule implemented.
            op_using_fallback = smith.var_mean
            vmap(op_using_fallback)(smith.rand(3))
...
class TestVmapOperators(Namespace.TestVmapBase):
    ...
    @parametrize(
        "case",
        [
            (smith.abs, TensorFactory.randn),
            (smith.acos, TensorFactory.rand),
            (smith.asin, TensorFactory.rand),
            (smith.atan, TensorFactory.rand),
            (smith.ceil, TensorFactory.randn),
            (smith.cos, TensorFactory.rand),
            (smith.cosh, TensorFactory.rand),
            (smith.digamma, TensorFactory.rand),
            (smith.exp, TensorFactory.randn),
            (smith.expm1, TensorFactory.randn),
            (smith.floor, TensorFactory.randn),
            (smith.frac, TensorFactory.randn),
            (smith.lgamma, TensorFactory.rand),
            (smith.log, TensorFactory.randp1),
            (smith.log10, TensorFactory.randp1),
            (smith.log1p, TensorFactory.randp1),
            (smith.log2, TensorFactory.randp1),
            (smith.neg, TensorFactory.randn),
            (smith.reciprocal, TensorFactory.randp1),
            (smith.relu, TensorFactory.randn),
            (smith.round, TensorFactory.randn),
            (smith.rsqrt, TensorFactory.randp1),
            (smith.sigmoid, TensorFactory.randn),
            (smith.sign, TensorFactory.randn),
            (smith.sin, TensorFactory.rand),
            (smith.sinh, TensorFactory.rand),
            (smith.sqrt, TensorFactory.rand),
            (smith.tan, TensorFactory.rand),
            (smith.tanh, TensorFactory.rand),
            (smith.trunc, TensorFactory.randn),
        ],
    ...
    def test_clone(self):
        ...
        self._test_unary(
            lambda x: x.clone(memory_format=smith.preserve_format),
            TensorFactory.randn,
            "cpu",
        )
        self._test_unary(
            lambda x: x.clone(memory_format=smith.contiguous_format),
            TensorFactory.randn,
            "cpu",
        )

        # Test that the per-examples are contiguous when using smith.contiguous_format
        def clone_contiguous(x):
            return x.clone(memory_format=smith.contiguous_format)

        B0, B1 = 3, 5
        x = smith.randn(2, B0, 7)
        ...
        x = smith.randn(2, B0, 7, B1)
        y = vmap(vmap(clone_contiguous, in_dims=2), in_dims=1)(x)
        self.assertTrue(y.is_contiguous())
        self.assertTrue(y[0][0].is_contiguous())

        msg = r"only supported with memory_format smith.preserve_format or smith.contiguous_format"
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(lambda x: x.clone(memory_format=smith.channels_last))(smith.randn(B0))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(lambda x: x.clone(memory_format=smith.channels_last_3d))(
                smith.randn(B0)
            )

    def test_weird_matmul_case(self):
        # Check that this doesn't crash.
        # https://github.com/blacksmith/funcsmith/issues/417
        x = smith.randn(5, 2, 2, 2)
        y = smith.randn(5, 7, 2)

        vmap(vmap(smith.matmul, in_dims=(None, 0)))(x, y)

    @parametrize(
        "case",
        (
            (smith.clamp_min_, TensorFactory.randn),
            (smith.clamp_max_, TensorFactory.randn),
        ),
        name_fn=lambda x: x[0].__name__,
    )
    ...
    @parametrize(
        "case",
        [
            subtest(_make_case(smith.clamp_min), name="clamp_min"),
            subtest(_make_case(smith.clamp_max), name="clamp_max"),
        ],
    )
    ...
    def test_silu_backward(self):
        ...
        B0 = 7
        op = smith.ops.aten.silu_backward
    ...
    @parametrize(
        "case",
        [
            subtest(_make_case(smith.add), name="add"),
            subtest(_make_case(lambda x, y: x + y), name="add_dunder"),
            subtest(_make_case(smith.sub), name="sub"),
            subtest(_make_case(lambda x, y: x - y), name="sub_dunder"),
            subtest(_make_case(smith.mul), name="mul"),
            subtest(_make_case(lambda x, y: x * y), name="mul_dunder"),
            subtest(
                _make_case(smith.div, input_getter=TensorFactory.randp1), name="div"
            ),
            subtest(
            ...
            subtest(
                _make_case(smith.pow, input_getter=TensorFactory.randp1), name="pow"
            ),
            subtest(
                _make_case(lambda x, y: x**y, input_getter=TensorFactory.randp1),
                name="pow_dunder",
    ...
    def test_arithmetic(self, case):
        ...
        test(op, (getter([B0], device), getter([B0], device, dtype=smith.double)))
        test(op, (getter([B0], device, dtype=smith.double), getter([B0], device)))
        test(op, (getter([B0], device), getter([B0], device)))

        # Type promotion: op(Tensor, Logical Scalar Tensor) (and vice-versa)
        test(op, (getter([B0, 2], device), getter([B0], device, smith.double)))
        test(op, (getter([B0], device, smith.double), getter([B0, 2], device)))
        ...
        # self._test_unary(lambda t: op(t, smith.tensor(number)), getter, device='cuda')
    ...
    def test_as_strided(self):
        ...
        tensors = [
            ...
            smith.randn(B0, 2, 3),
            # non-contiguous
            smith.randn(B0, 3, 2).transpose(1, 2),
            smith.randn(3, 2, B0).movedim(-1, 0).transpose(1, 2),
            # non-zero storage offset
            smith.randn(2, B0, 2, 3)[1],
            smith.randn(2, 2, B0, 3)[1].movedim(1, 0),
            # non-contiguous strides, zero storage offset
            smith.randn(B0, 2, 4, 3, 7)[:, :, 0, :, 0],
            smith.randn(2, 4, B0, 3, 7).movedim(2, 0)[:, :, 0, :, 0],
            # non-contiguous strides, non-zero storage offset
            smith.randn(B0, 2, 4, 3, 7)[:, :, 2, :, 1],
            smith.randn(2, 4, 3, 7, B0).movedim(-1, 0)[:, :, 2, :, 1],
        ]
        ...
        with self.assertRaisesRegex(
            ...
            x = smith.randn(B0, 2, 3).transpose(0, 1)
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            x = smith.randn(B0, 3, 5)
            vmap(lambda x: x.as_strided([4, 4], [4, 1], 0))(x)
        with self.assertRaisesRegex(RuntimeError, msg):
            x = smith.randn(B0, B1, 3, 5)
    ...
    def test_bmm(self):
        ...
        msg = ""
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op)(smith.randn(B0, 2, 2, 2), smith.randn(B0, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(0, None))(smith.randn(B0, 3, 3, 2), smith.randn(2, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(None, 0))(smith.randn(2, 2), smith.randn(B0, 2, 2, 2))

        # left arg is vmapped
        test(op, (smith.rand(B0, 2, 3, 5), smith.rand(2, 5, 3)), in_dims=(0, None))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, B0, 2, 3, 5), smith.rand(2, 5, 3)),
            in_dims=(1, None),
        )

        # right arg is vmapped
        test(op, (smith.rand(2, 5, 3), smith.rand(B0, 2, 3, 5)), in_dims=(None, 0))
        test(
            vmap(op, in_dims=(None, 0)),
            (smith.rand(2, 5, 3), smith.rand(B1, B0, 2, 3, 5)),
            in_dims=(None, 1),
        )

        # both args are vmapped
        test(op, (smith.rand(B0, 2, 3, 5), smith.rand(B0, 2, 5, 3)))
        test(
            vmap(op),
            (smith.rand(B1, B0, 2, 3, 5), smith.rand(B0, B1, 2, 5, 3)),
            in_dims=(1, 0),
        )
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, 2, 3, 5), smith.rand(B0, 2, 5, 3)),
            in_dims=(None, 0),
        )
    ...
    def test_cat(self):
        ...
        def get_op(dim):
            def op(*tensors):
                return smith.cat(tensors, dim=dim)

            return op

        test(get_op(0), (smith.rand(B0, 2), smith.rand(B0, 3)))
        test(get_op(0), (smith.rand(B0, 0), smith.rand(B0, 0)))
        test(get_op(0), (smith.rand(2), smith.rand(B0, 0)), in_dims=(None, 0))
        test(
            get_op(1),
            (smith.rand(2, 5), smith.rand(B0, 0), smith.rand(2, 3)),
            in_dims=(None, 0, None),
        )
        test(get_op(1), (smith.rand(B0, 2, 3), smith.rand(B0, 0)))
        test(get_op(1), (smith.rand(B0, 2, 3, 4), smith.rand(0)), in_dims=(0, None))
        test(
            get_op(0),
            (smith.rand(0), smith.rand(B0, 2), smith.rand(B0, 0)),
            in_dims=(None, 0, 0),
        )
        test(get_op(0), (smith.rand(2), smith.rand(B0, 3)), in_dims=(None, 0))
        test(get_op(0), (smith.rand(2, 17), smith.rand(3, 17, B0)), in_dims=(None, 2))
        test(get_op(-1), (smith.rand(17, 2), smith.rand(17, 3, B0)), in_dims=(None, 2))
        test(
            vmap(get_op(0), in_dims=(0, None)),
            (smith.rand(B1, 2), smith.rand(B0, 3)),
            in_dims=(None, 0),
        )
        test(
            vmap(get_op(0), in_dims=(0, 0)),
            (smith.rand(B1, 2), smith.rand(B0, B1, 3)),
            in_dims=(None, 0),
        )
    ...
    def test_unsafe_view(self):
        ...
        B = 2
        x = smith.randn(B, 2, 3, 3)
        y = smith.randn(B, 3, 3)

        def baz(x, y):
            return (x @ y).sum()

        test(funcsmith.grad(baz), (x, y))
    ...
    def test_conj(self):
        ...
        def run_test(dtype):
            def get(shape):
                return smith.randn(shape, dtype=dtype)
        ...
        # correctness tests
        run_test(smith.float)
        run_test(smith.cfloat)

        # check that smith.conj on a non-complex tensor returns the same tensor
        real_tensor = smith.randn(3)
    ...
    def test_contiguous(self):
        ...
        B0 = 3
        x = smith.randn(B0, 2, 5, 7)
        x = x.movedim(0, 2)
        result = vmap(Tensor.contiguous, in_dims=2, out_dims=2)(x)
        self.assertTrue(result is x)

        msg = "NYI: querying is_contiguous inside of vmap for memory_format"
        tensor = smith.randn(B0, 3)
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(functools.partial(op, memory_format=smith.channels_last))(tensor)
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(functools.partial(op, memory_format=smith.channels_last_3d))(tensor)
    ...
    def test_stride(self):
        ...
        x = smith.randn(2, B0, 5, 7).movedim(1, 0)
    ...
    def test_chunk(self):
        ...
        # tests for smith.split(self, split_size: int, dim)
        test(op, (smith.rand(B0, 2, 1024), 15, -1), in_dims=(0, None, None))
        test(op, (smith.rand(2, B0, 1024), 9, 1), in_dims=(1, None, None))
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 1023, B0, 5), 4, 0),
            in_dims=(2, None, None),
        )
        test(
            vmap(vmap(lambda t: op(t, 4, 1), in_dims=2)),
            (smith.rand(B1, 2, B0, 64, B2),),
            in_dims=2,
        )
    ...
    def test_cross_batch_size_three(self):
        ...
        test = self._vmap_test
        B0 = B1 = 3
        test(op, (smith.rand(B0, 2, 3), smith.rand(B0, 2, 3)))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B0, B1, 2, 3), smith.rand(B0, B1, 2, 3)),
            in_dims=(None, 1),
        )

    def test_diagonal(self):
        tensor = smith.randn(3, 5, 7, 11, 13)
        test = self._vmap_view_test
        op = smith.diagonal
    ...
    def test_dot(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op)(smith.randn(B0, 2, 2, 2), smith.randn(B0, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(0, None))(smith.randn(B0, 2), smith.randn(2, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(None, 0))(smith.randn(2, 2), smith.randn(B0, 2))

        # left arg is vmapped
        test(op, (smith.rand(B0, 5), smith.rand(5)), in_dims=(0, None))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, B0, 5), smith.rand(5)),
            in_dims=(1, None),
        )

        # right arg is vmapped
        test(op, (smith.rand(5), smith.rand(B0, 5)), in_dims=(None, 0))
        test(
            vmap(op, in_dims=(None, 0)),
            (smith.rand(5), smith.rand(B1, B0, 5)),
            in_dims=(None, 1),
        )

        # both args are vmapped
        test(op, (smith.rand(B0, 5), smith.rand(B0, 5)))
        test(vmap(op), (smith.rand(B1, B0, 5), smith.rand(B0, B1, 5)), in_dims=(1, 0))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, 5), smith.rand(B0, 5)),
            in_dims=(None, 0),
        )

    def test_expand_as(self):
        op = smith.Tensor.expand_as
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        test(op, (smith.rand(B0, 1, 5), smith.rand(B0, 2, 3, 5)))
        test(op, (smith.rand(B0, 1, 5), smith.rand(2, 3, 5)), in_dims=(0, None))
        test(op, (smith.rand(1, 5), smith.rand(B0, 2, 3, 5)), in_dims=(None, 0))
        test(vmap(op), (smith.rand(B0, B1, 1, 5), smith.rand(B0, B1, 2, 3, 5)))
        test(
            vmap(op),
            (smith.rand(B0, B1, 1, 5), smith.rand(B1, B0, 2, 3, 5)),
            in_dims=(0, 1),
        )
        test(vmap(op), (smith.rand(B0, B1), smith.rand(B1, 2, 3, 5)), in_dims=(0, None))
        test(vmap(vmap(op)), (smith.rand(B0, B1, B2), smith.rand(B0, B1, B2, 2, 3, 5)))
    ...
    def test_fill_and_zero_inplace(self):
        ...
        ops = (
            ...
            lambda t: t.fill_(smith.tensor(0.2)),
            lambda t: t.zero_(),
        )
    ...
    def test_real(self):
        self._test_complex_views(smith.real, dtypes=[smith.cfloat, smith.cdouble])

    def test_imag(self):
        self._test_complex_views(smith.imag, dtypes=[smith.cfloat, smith.cdouble])

    def test_view_as_real(self):
        self._test_complex_views(
            smith.view_as_real, dtypes=[smith.cfloat, smith.cdouble]
        )
    ...
    def test_is_complex(self):
        ctensor = smith.randn(3, dtype=smith.cfloat)
        ...
        def foo(x):
            if x.is_complex():
                ...
                return smith.tensor(0)

        self.assertEqual(vmap(foo)(ctensor), smith.tensor([1, 1, 1]))
        self.assertEqual(vmap(foo)(tensor), smith.tensor([0, 0, 0]))

    def test_is_floating_point(self):
        float_tensor = smith.tensor([1.0, 2.0, 3.0])
        long_tensor = smith.tensor([1, 2, 3])
        ...
        self.assertEqual(vmap(foo)(float_tensor), smith.tensor([1, 1, 1]))
        self.assertEqual(vmap(foo)(long_tensor), smith.tensor([0, 0, 0]))

    @unittest.skipIf(IS_WINDOWS, reason="Windows not yet supported for smith.compile")
    def test_is_contiguous(self):
        def foo(x):
            if x.is_contiguous():
                return smith.tensor(1.0)
            else:
                return smith.tensor(0.0)

        B0, B1 = 3, 5

        # Single batch dim
        contig = smith.randn(B0, 2, 7)
        self.assertEqual(vmap(foo)(contig), smith.ones(B0))

        noncontig = smith.randn(2, B0, 7)
        self.assertEqual(vmap(foo, in_dims=1)(noncontig), smith.zeros(B0))

        noncontig = smith.randn(2, B0, 7).movedim(1, 0)
        self.assertEqual(vmap(foo)(noncontig), smith.zeros(B0))

        noncontig = smith.randn(2, 7, B0)
        self.assertEqual(vmap(foo, in_dims=2)(noncontig), smith.zeros(B0))

        # Multiple batch dims
        contig = smith.randn(B0, B1, 3)
        self.assertEqual(vmap(vmap(foo))(contig), smith.ones(B0, B1))

        contig = smith.randn(B1, B0, 3)
        self.assertEqual(vmap(vmap(foo), in_dims=1)(contig), smith.ones(B0, B1))

        contig = smith.randn(B1, B0, 3).movedim(0, 1)
        self.assertEqual(vmap(vmap(foo))(contig), smith.ones(B0, B1))

        noncontig = smith.randn(B0, 3, B1)
        self.assertEqual(vmap(vmap(foo, in_dims=1))(noncontig), smith.zeros(B0, B1))
        ...
        vmap(bar)(smith.randn(B0, 0, 3))
        vmap(bar, in_dims=1)(smith.randn(0, B0, 3))
        vmap(bar)(smith.randn(B0, 0, 3).transpose(-1, -2))
        ...
        msg = "NYI: querying is_contiguous inside of vmap for memory_format"
        tensor = smith.randn(B0, 2, 7, 3)
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(functools.partial(baz, memory_format=smith.channels_last))(tensor)
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(functools.partial(baz, memory_format=smith.channels_last_3d))(tensor)

        for mf in (smith.channels_last, smith.channels_last_3d):
    ...
    def test_unsqueeze(self):
        ...
        # unsqueeze dim 0
        test(op, (smith.rand(B0, 2, 5), 0), in_dims=(0, None))
        test(op, (smith.rand(2, B0, 5), 0), in_dims=(1, None))

        # unsqueeze last dim (positive)
        test(op, (smith.rand(B0, 2, 5), 2), in_dims=(0, None))
        test(op, (smith.rand(2, B0, 5), 2), in_dims=(1, None))

        # unsqueeze last dim (negative)
        test(op, (smith.rand(B0, 2, 5), -1), in_dims=(0, None))
        test(op, (smith.rand(2, B0, 5), -1), in_dims=(1, None))

        # nested vmaps
        def unsqueeze_0(x):
            return smith.unsqueeze(x, 0)

        def unsqueeze_last(x):
            return smith.unsqueeze(x, -1)

        # bdims in canonical order
        test(vmap(unsqueeze_0), (smith.rand(B0, B1, 2),))
        test(vmap(unsqueeze_last), (smith.rand(B0, B1, 2),))

        # wild bdims
        test(vmap(unsqueeze_0), (smith.rand(B1, 2, B0),), in_dims=2)
        test(vmap(unsqueeze_0, in_dims=1), (smith.rand(2, B1, B0),), in_dims=2)
        test(vmap(unsqueeze_last), (smith.rand(B1, 2, B0),), in_dims=2)
        test(vmap(unsqueeze_last, in_dims=1), (smith.rand(2, B1, B0),), in_dims=2)
    ...
    def test_movedim(self):
        ...
        test(op, (smith.rand(B0, 2, 5), 0, 1), in_dims=(0, None, None))
        test(op, (smith.rand(2, B0, 5), 0, 1), in_dims=(1, None, None))
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 2, B0, 5), 0, 1),
            in_dims=(2, None, None),
        )
        test(
            vmap(vmap(op, in_dims=(2, None, None)), in_dims=(0, None, None)),
            (smith.rand(B1, 2, B0, 5, B2), 0, 1),
            in_dims=(2, None, None),
        )

        # movedim(tensor, intlist, intlist) variant
        test(op, (smith.rand(B0, 2, 3, 5), [1, 0], [0, 2]), in_dims=(0, None, None))
        test(op, (smith.rand(2, 3, B0, 5), [1, 0], [0, 2]), in_dims=(1, None, None))
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 2, B0, 5), [0, 1], [1, 0]),
            in_dims=(2, None, None),
        )
        test(
            vmap(vmap(op, in_dims=(2, None, None)), in_dims=(0, None, None)),
            (smith.rand(B1, 2, B0, 5, B2), [0, 1], [1, 0]),
            in_dims=(2, None, None),
        )
    ...
    def test_mm(self):
        ...
        # left arg is vmapped
        test(op, (smith.rand(B0, 2, 5), smith.rand(5, 2)), in_dims=(0, None))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, B0, 2, 5), smith.rand(5, 2)),
            in_dims=(1, None),
        )

        # right arg is vmapped
        test(op, (smith.rand(2, 5), smith.rand(B0, 5, 2)), in_dims=(None, 0))
        test(
            vmap(op, in_dims=(None, 0)),
            (smith.rand(2, 5), smith.rand(B1, B0, 5, 2)),
            in_dims=(None, 1),
        )

        # both args are vmapped
        test(op, (smith.rand(B0, 2, 5), smith.rand(B0, 5, 2)))
        test(
            vmap(op),
            (smith.rand(B1, B0, 2, 5), smith.rand(B0, B1, 5, 2)),
            in_dims=(1, 0),
        )
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, 2, 5), smith.rand(B0, 5, 2)),
            in_dims=(None, 0),
        )
    ...
    def test_mv(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op)(smith.randn(B0, 2, 2, 2), smith.randn(B0, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(0, None))(smith.randn(B0, 2, 2), smith.randn(2, 2))
        with self.assertRaisesRegex(RuntimeError, msg):
            vmap(op, in_dims=(None, 0))(smith.randn(2, 2), smith.randn(B0, 2, 2))

        # left arg is vmapped
        test(op, (smith.rand(B0, 2, 5), smith.rand(5)), in_dims=(0, None))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, B0, 2, 5), smith.rand(5)),
            in_dims=(1, None),
        )

        # right arg is vmapped
        test(op, (smith.rand(2, 5), smith.rand(B0, 5)), in_dims=(None, 0))
        test(
            vmap(op, in_dims=(None, 0)),
            (smith.rand(2, 5), smith.rand(B1, B0, 5)),
            in_dims=(None, 1),
        )

        # both args are vmapped
        test(op, (smith.rand(B0, 2, 5), smith.rand(B0, 5)))
        test(
            vmap(op), (smith.rand(B1, B0, 2, 5), smith.rand(B0, B1, 5)), in_dims=(1, 0)
        )
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, 2, 5), smith.rand(B0, 5)),
            in_dims=(None, 0),
        )
    ...
    def test_narrow(self):
        ...
        test(op, (smith.rand(B0, 2, 5), -1, 1, 3), in_dims=(0, None, None, None))
        test(op, (smith.rand(2, B0, 5), 1, 1, 3), in_dims=(1, None, None, None))
        test(
            vmap(op, in_dims=(0, None, None, None)),
            (smith.rand(B1, 2, B0, 5), 1, 0, 0),
    ...
    def test_new_empty(self):
        ...
        result = vmap(lambda x: op(x, [2, 3]))(smith.randn(B0))
        self.assertEqual(result.shape, [B0, 2, 3])

        result = vmap(lambda x: op(x, []))(smith.randn(B0))
        self.assertEqual(result.shape, [B0])

        result = vmap(vmap(lambda x: op(x, [2, 3])))(smith.randn(B0, B1))
    ...
    def test_new_empty_strided(self):
        ...
        def _test_single_vmap(size, stride, B0):
            ...
            S = smith.empty_strided(size, stride).storage().size()
    ...
    def test_new_zeros(self):
        ...
        test(lambda x: op(x, 2, 3), (smith.rand(B0),))
        test(lambda x: op(x, []), (smith.rand(B0),))
        test(vmap(lambda x: op(x, 3, 5)), (smith.rand(B0, B1),))

    def test_select(self):
        op = smith.select
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        test(op, (smith.rand(B0, 2, 5), 0, 0), in_dims=(0, None, None))
        test(op, (smith.rand(2, B0, 5), 1, 1), in_dims=(1, None, None))
        test(vmap(lambda t: op(t, 1, 1)), (smith.rand(B1, 2, B0, 5),), in_dims=2)
        test(
            vmap(vmap(lambda t: op(t, 1, 1), in_dims=1)),
            (smith.rand(B1, 2, B0, B2, 5),),
            in_dims=2,
        )
    ...
    def test_roll_no_dims(self):
        ...
        B0, B1, B2 = 7, 11, 13
        test(op, (smith.rand(B0, 2, 5), 2), in_dims=(0, None))
        test(op, (smith.rand(2, B0, 5), 3), in_dims=(1, None))
        test(vmap(lambda t: op(t, 3)), (smith.rand(B1, 2, B0, 5),), in_dims=2)
        test(
    ...
    def test_stack(self):
        ...
        def get_op(dim):
            def op(*tensors):
                return smith.stack(tensors, dim=dim)

            return op

        test(get_op(0), (smith.rand(B0, 3), smith.rand(B0, 3)))
        test(get_op(0), (smith.rand(3), smith.rand(B0, 3)), in_dims=(None, 0))
        test(get_op(0), (smith.rand(2, 17), smith.rand(2, 17, B0)), in_dims=(None, 2))
        test(get_op(-1), (smith.rand(2, 17), smith.rand(2, 17, B0)), in_dims=(None, 2))
        test(
            vmap(get_op(0), in_dims=(0, None)),
            (smith.rand(B1, 2), smith.rand(B0, 2)),
            in_dims=(None, 0),
        )
        test(
            vmap(get_op(0), in_dims=(0, 0)),
            (smith.rand(B1, 2), smith.rand(B0, B1, 2)),
            in_dims=(None, 0),
        )
    ...
    def test_slice(self):
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        test(lambda t: t[0:1], (smith.rand(B0, 3, 5),))
        test(lambda t: t[:, 1:3], (smith.rand(3, 5, B0),), in_dims=2)
        test(
            vmap(lambda t: t[:, 0:1], in_dims=2), (smith.rand(3, 5, B0, B1),), in_dims=2
        )
        test(
            vmap(vmap(lambda t: t[0:1], in_dims=2), in_dims=2),
            (smith.rand(3, 5, B0, B1, B2),),
            in_dims=2,
        )
    ...
    def test_squeeze(self):
        def verify_behavior(op, min_ndim=1):
            ...
            if min_ndim <= 1:
                test(op, (smith.rand(B0),))
                test(op, (smith.rand(B1),))
                test(vmap(op), (smith.rand(B0, B1, 1),))
                test(vmap(op), (smith.rand(B1, 1, B0),), in_dims=2)
            test(op, (smith.rand(B0, 3, 5),))
            test(op, (smith.rand(1, B0, 5),), in_dims=1)
            test(op, (smith.rand(B0, 0, 1, 5, 1),))
            test(op, (smith.rand(B0, 1, 1, 1, 1),))
            test(vmap(op), (smith.rand(B0, B1, 1, 3, 4),))
            test(vmap(op), (smith.rand(B1, 1, B0, 4, 5),), in_dims=2)

        verify_behavior(smith.squeeze)
        verify_behavior(lambda x: smith.squeeze(x, dim=0), min_ndim=1)
        verify_behavior(lambda x: smith.squeeze(x, dim=1), min_ndim=2)
        verify_behavior(lambda x: smith.squeeze(x, dim=-1), min_ndim=2)
        verify_behavior(lambda x: smith.squeeze(x, dim=-2), min_ndim=3)

        msg = ""
        try:
            smith.squeeze(smith.rand(10), dim=1)
        except IndexError as err:
            msg = str(err)
        with self.assertRaises(RuntimeError, msg=msg):
            vmap(lambda x: smith.squeeze(x, dim=1))(smith.rand(10))
    ...
    def _test_mean_sum_dim(self, op):
        ...
        # Single vmap, various in_dims / out_dims
        test(lambda x: op(x, 0), [smith.randn([B0])])
        test(lambda x: op(x, -1), [smith.randn([B0])])
        test(lambda x: op(x, 0), [smith.randn([B0, 3])])
        test(lambda x: op(x, -1), [smith.randn([2, 5, B0, 3])], in_dims=2)
        test(lambda x: op(x, 2), [smith.randn([2, 5, B0, 3])], in_dims=2, out_dims=2)

        # Doubly nested vmap
        test(vmap(lambda x: op(x, 0)), [smith.randn([B0, B1])])
        test(vmap(lambda x: op(x, -1)), [smith.randn([B0, B1])])
        test(vmap(lambda x: op(x, -2)), [smith.randn([B1, 2, 5, B0, 3])], in_dims=2)
        test(
            vmap(lambda x: op(x, 2), in_dims=2),
            [smith.randn([2, 5, B0, B1, 3])],
            in_dims=2,
            out_dims=2,
        )

    def test_sum_dim(self):
        self._test_mean_sum_dim(smith.sum)

    def test_mean_dim(self):
        self._test_mean_sum_dim(smith.mean)
    ...
    def test_argmax_dim(self):
        ...
        B0 = 5
        test(lambda x: smith.argmax(x), [smith.randn(B0)])
        test(lambda x: smith.argmax(x), [smith.randn(B0, 2, 3)])
        test(lambda x: smith.argmax(x, 0), [smith.randn(B0, 2, 3)])
        test(lambda x: smith.argmax(x, -1), [smith.randn(B0, 2, 3)])
        test(lambda x: smith.argmax(x, 2), [smith.randn(B0, 2, 3)])
    ...
    def _test_sum_mean(self, op):
        ...
        # Single vmap, various in_dims / out_dims
        test(op, [smith.randn([B0])])
        test(op, [smith.randn([B0, 3])])
        test(op, [smith.randn([2, 5, B0, 3])], in_dims=2)
        test(op, [smith.randn([2, 5, B0, 3])], in_dims=2)

        # Doubly nested vmap
        test(vmap(op), [smith.randn([B0, B1])])
        test(vmap(op), [smith.randn([B1, 2, 5, B0, 3])])
        test(vmap(op), [smith.randn([2, 5, B0, B1, 3])], in_dims=2)

    def test_sum(self):
        self._test_sum_mean(smith.sum)

    def test_mean(self):
        self._test_sum_mean(smith.mean)

    def test_repeat(self):
        test = self._vmap_test
        B0 = 7
        op = Tensor.repeat
        test(lambda x: op(x, (2, 3)), (smith.rand(B0, 1, 1),))
        test(lambda x: op(x, (2, 3)), (smith.rand(1, B0, 1),), in_dims=1)

    @skipIfSmithDynamo()
    def test_slogdet(self):
        test = functools.partial(self._vmap_test, check_propagates_grad=False)
        B0 = 7
        op = smith.linalg.slogdet
        test(op, (smith.rand(B0, 1, 1),))
        test(op, (smith.rand(B0, 2, 2),))
        test(op, (smith.rand(B0, 3, 2, 2),))
        test(op, (smith.rand(3, 2, 2, B0),), in_dims=3)

    def test_reshape(self):
        test = self._vmap_test
        B0, B1, B2 = 7, 11, 13
        op = smith.reshape
        test(op, (smith.rand(B0, 2 * 5), [2, 5]), in_dims=(0, None), check_view=True)
        test(
            op, (smith.rand(2, B0, 5), [1, 1, 10]), in_dims=(1, None), check_view=False
        )
        test(
            vmap(lambda t: t.reshape([-1])),
            (smith.rand(B0, B1, 2, 5),),
            check_view=True,
        )
        test(
            vmap(vmap(lambda t: t.reshape([-1]), in_dims=2), in_dims=1),
            (smith.rand(3, B1, 2, B2, 5, B0),),
            in_dims=5,
            check_view=False,
        )
    ...
    def test_reshape_as(self):
        test = self._vmap_test
        B0, B1, B2 = 7, 11, 13
        op = smith.Tensor.reshape_as
        test(op, (smith.rand(B0, 2 * 5), smith.rand(B0, 2, 5)), check_view=True)
        test(
            op,
            (smith.rand(2 * 5), smith.rand(B0, 2, 5)),
            in_dims=(None, 0),
            check_view=True,
        )
        test(
            op,
            (smith.rand(B0, 2 * 5), smith.rand(2, 5)),
            in_dims=(0, None),
            check_view=True,
        )
        ...
        test(
            op,
            (smith.rand(2, B0, 5), smith.rand(1, 1, 10)),
            in_dims=(1, None),
            check_view=False,
        )
        ...
        test(
            vmap(op),
            (smith.rand(B0, B1, 2, 5), smith.randn(B0, B1, 10)),
            check_view=True,
        )
        test(
            vmap(vmap(op, in_dims=(2, None)), in_dims=(1, None)),
            (smith.rand(3, B1, 2, B2, 5, B0), smith.rand(B0, 3 * 2 * 5)),
            in_dims=(5, 0),
            check_view=False,
        )
    ...
    def test_result_type(self):
        def scalar_tensor_with_dtype(op):
            def wrapped(*args, **kwargs):
                ...
                return smith.ones([], dtype=dtype)

            return wrapped

        test = self._vmap_test
        op = scalar_tensor_with_dtype(smith.result_type)

        B0 = 2

        test(
            op,
            (smith.randn(B0), smith.randn(B0, dtype=smith.float64)),
            check_propagates_grad=False,
        )
        test(
            op,
            (smith.randn(B0), smith.randint(10, [B0], dtype=smith.int64)),
            check_propagates_grad=False,
        )

        test(lambda x: op(x, 1), (smith.randn(B0),), check_propagates_grad=False)
        test(lambda x: op(x, 1.6), (smith.randn(B0),), check_propagates_grad=False)

        test(
            lambda x: op(x, smith.tensor(1)),
            (smith.randn(B0),),
            check_propagates_grad=False,
        )
        test(
            lambda x: op(x, smith.tensor(1.6, dtype=smith.double)),
            (smith.randn(B0),),
            check_propagates_grad=False,
        )
        ...
        test(
            op,
            (smith.randn(B0, 2), smith.randn(B0, 2, dtype=smith.float64)),
            check_propagates_grad=False,
        )
        test(
            op,
            (smith.randn(B0, 2), smith.randint(10, [B0, 2], dtype=smith.int64)),
            check_propagates_grad=False,
        )

        test(lambda x: op(x, 1), (smith.randn(B0, 2),), check_propagates_grad=False)
        test(lambda x: op(x, 1.6), (smith.randn(B0, 2),), check_propagates_grad=False)
        ...
        test(
            op,
            (smith.randn(B0, 2), smith.randn(B0, dtype=smith.float64)),
            check_propagates_grad=False,
        )
        test(
            op,
            (smith.randn(B0, 2), smith.randint(10, [B0], dtype=smith.int64)),
            check_propagates_grad=False,
        )

    def test_tensor_split(self):
        test = self._vmap_view_test
        op = smith.tensor_split
        B0, B1, B2 = 7, 11, 13

        # tests for smith.tensor_split(self, indices_or_sections: int, dim)
        test(op, (smith.rand(B0, 2, 1024), 5, -1), in_dims=(0, None, None))
        test(op, (smith.rand(2, B0, 1024), 150, 1), in_dims=(1, None, None))
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 1023, B0, 5), 256, 0),
            in_dims=(2, None, None),
        )
        test(
            vmap(vmap(lambda t: op(t, 4, 1), in_dims=2)),
        ...
        # tests for smith.tensor_split(self, indices_or_sections: List[int], dim)
        test(
            op,
            (smith.rand(B0, 2, 1024), [50, 100, 378, 890], -1),
            in_dims=(0, None, None),
        )
        test(
            op,
            (smith.rand(2, B0, 1024), [50, 100, 212, 345, 0, 378, 890], 1),
            in_dims=(1, None, None),
        )
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 1023, B0, 5), [50, 100, 212, 345, 0, 378, 890], 0),
            in_dims=(2, None, None),
        )
        test(
    ...
    def test_split(self):
        ...
        # tests for smith.split(self, split_size: int, dim)
        test(op, (smith.rand(B0, 2, 1024), 101, -1), in_dims=(0, None, None))
        test(op, (smith.rand(2, B0, 1024), 130, 1), in_dims=(1, None, None))
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 1023, B0, 5), 256, 0),
            in_dims=(2, None, None),
        ...
        # tests for smith.split(self, split_size: List[int], dim)
        test(op, (smith.rand(B0, 2, 1024), [1, 1020, 3], -1), in_dims=(0, None, None))
        test(
            op, (smith.rand(2, B0, 1024), [100] * 10 + [24], 1), in_dims=(1, None, None)
        )
        test(
            vmap(op, in_dims=(0, None, None)),
            (smith.rand(B1, 1023, B0, 5), [256] * 3 + [255], 0),
            in_dims=(2, None, None),
        )
        test(
    ...
    def test_trace(self):
        ...
        test = self._vmap_test
        B0, B1, B2 = 7, 11, 13
        test(op, (smith.rand(B0, 2, 5),))
        test(op, (smith.rand(2, B0, 5),), in_dims=1)
        test(vmap(op), (smith.rand(B1, 2, B0, 5),), in_dims=2)
        test(vmap(vmap(op, in_dims=2)), (smith.rand(B1, 2, B0, 5, B2),), in_dims=2)
    ...
    def test_transpose(self):
        ...
        B0, B1, B2 = 7, 11, 13
        test(lambda x: op(x, 0, 1), (smith.rand(B0, 2, 5),))
        test(lambda x: op(x, -1, -2), (smith.rand(B0, 2, 5),))
        test(lambda x: op(x, 3, 1), (smith.rand(B0, 2, 5, 4, 6),))
        test(lambda x: op(x, 1, 0), (smith.rand(2, B0, 5),), in_dims=1)
        test(vmap(lambda x: op(x, 0, 1)), (smith.rand(B1, 2, B0, 5),), in_dims=2)
        test(
            vmap(vmap(lambda x: op(x, 0, 1), in_dims=2)),
            (smith.rand(B1, 2, B0, 5, B2),),
            in_dims=2,
        )
    ...
    def test_T_numpy(self):
        ...
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        test(op, (smith.rand(B0, 2, 3, 5),))
        test(op, (smith.rand(2, B0, 3, 5),), in_dims=1)
        test(vmap(op), (smith.rand(B1, 2, B0, 5),), in_dims=2)
        test(vmap(op), (smith.rand(B1, 2, B0, 3, 5),), in_dims=2)
        test(vmap(vmap(op, in_dims=2)), (smith.rand(B1, 2, B0, 3, B2, 5),), in_dims=2)

    def test_to(self):
        test = self._vmap_test
        B0, B1 = 7, 11

        test(lambda t: t.to("cpu"), (smith.rand(B0),))
        test(lambda t: t.to(smith.double), (smith.rand(B0),))
        test(
            lambda t, o: t.to(o), (smith.rand(B0), smith.randn(B0, dtype=smith.float64))
        )
        test(
            lambda t, o: t.to(o),
            (smith.rand(B0), smith.randn(B0, dtype=smith.float64)),
            in_dims=(0, None),
        )
        test(vmap(lambda t: t.to(smith.double)), (smith.rand(B0, B1, 3),))

        # also test some casting methods
        test(lambda t: t.double(), (smith.rand(B0),))
        test(lambda t: t.float(), (smith.rand(B0),))
        test(lambda t: t.int(), (smith.rand(B0),), check_propagates_grad=False)
        test(lambda t: t.long(), (smith.rand(B0),), check_propagates_grad=False)

    def test_unfold(self):
        op = smith.Tensor.unfold
        test = self._vmap_view_test
        B0, B1, B2 = 3, 2, 5

        test(op, (smith.rand(B0, 7, 11), 0, 2, 1), in_dims=(0, None, None, None))
        test(op, (smith.rand(7, B0, 11), 1, 4, 2), in_dims=(1, None, None, None))
        test(
            vmap(op, in_dims=(0, None, None, None)),
            (smith.rand(B1, 7, B0, 11), 1, 5, 1),
            in_dims=(2, None, None, None),
        )
        test(
            vmap(
            ...
            (smith.rand(B1, 7, B0, 11, B2), -1, 2, 4),
            in_dims=(2, None, None, None),
        )
    ...
    def test_unbind(self):
        ...
        test(op, (smith.rand(B0, 2, 1024), -1), in_dims=(0, None))
        test(op, (smith.rand(B0, 2, 0),))
        test(op, (smith.rand(2, B0, 7), 0), in_dims=(1, None))
        test(
            vmap(op, in_dims=(0, None)),
            (smith.rand(B1, 1023, B0, 5), 1),
            in_dims=(2, None),
        )
        test(
            vmap(vmap(lambda t: op(t, dim=1), in_dims=2)),
            (smith.rand(B1, 2, B0, 32, B2),),
            in_dims=2,
        )
    ...
    def test_view(self):
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        op = smith.Tensor.view

        # We should error out if the view would produce an incorrect result
        with self.assertRaises(RuntimeError):
            vmap(op, in_dims=(1, None))(smith.rand(2, B0, 5), [10])

        test(op, (smith.rand(B0, 2 * 5), [2, 5]), in_dims=(0, None))
        test(op, (smith.rand(B0, 4, 5), [1, 2, 1, 10]), in_dims=(0, None))
        test(vmap(lambda t: t.view([-1])), (smith.rand(B0, B1, 2, 5, 3),))
        test(
            vmap(vmap(lambda t: t.reshape([-1])), in_dims=1),
            (smith.rand(B2, B0, B1, 3, 2, 5),),
            in_dims=1,
        )
    ...
    def test_view_as(self):
        test = self._vmap_view_test
        B0, B1, B2 = 7, 11, 13
        op = smith.Tensor.view_as

        # We should error out if the view would produce an incorrect result
        with self.assertRaises(RuntimeError):
            vmap(op, in_dims=(1, 0))(smith.rand(2, B0, 5), smith.rand(B0, 10))

        test(op, (smith.rand(B0, 2 * 5), smith.rand(B0, 2, 5)))
        test(op, (smith.rand(2 * 5), smith.rand(B0, 2, 5)), in_dims=(None, 0))
        test(op, (smith.rand(B0, 2 * 5), smith.rand(2, 5)), in_dims=(0, None))

        test(op, (smith.rand(B0, 4, 5), smith.rand(2, 1, 1, 10)), in_dims=(0, None))

        test(vmap(op), (smith.rand(B0, B1, 2, 5), smith.randn(B0, B1, 10)))
        test(
            vmap(vmap(op, in_dims=(0, None)), in_dims=(0, None)),
            (smith.rand(B1, B2, B0, 3, 2, 5), smith.rand(B0, 3 * 2 * 5)),
            in_dims=(2, 0),
        )

    def test_conv2d(self):
        conv_setups = [
            (smith.nn.Conv1d, smith.conv1d, [2, 4, 15]),
            (smith.nn.Conv2d, smith.conv2d, [2, 4, 15, 20]),
            (smith.nn.Conv3d, smith.conv3d, [2, 4, 15, 20, 25]),
            # (smith.nn.ConvTranspose2d, smith.conv_transpose2d, [2, 4, 15, 20])
        ]
        for conv_mod, conv_fn, inp_shape in conv_setups:
            mod = conv_mod(4, 8, kernel_size=3)
            arg_values = [smith.randn(inp_shape), mod.weight, mod.bias]
            kwarg_values = {}
            ...
            arg_values = [smith.randn(inp_shape), mod.weight, None]
            for loop_out, batched_out in get_fallback_and_vmap_exhaustive(
                conv_fn, arg_values, kwarg_values
            ):
                self.assertEqual(loop_out, batched_out)
            ...
            arg_values = [smith.randn(inp_shape), mod2.weight, mod2.bias]
            ...
            arg_values = [smith.randn(inp_shape), mod2.weight, None]
            for loop_out, batched_out in get_fallback_and_vmap_exhaustive(
                conv_fn, arg_values, kwarg_values
            ):
                self.assertEqual(loop_out, batched_out)
    ...
    def test_mode_key(self):
        def vmap_f(x):
            return x + smith.randn(())

        def naive_f(x, shape):
            return x + smith.randn(shape)
        ...
        smith.manual_seed(0)
        out2 = naive_f(smith.ones(2, 3), (2, 3))
        ...
        smith.manual_seed(0)
        out2 = naive_f(smith.ones(2, 3, 4), (2, 3, 1))
        self.assertEqual(out1, out2)

        self.assertTrue(smith.randn(()).dim() == 0)
    ...
    def test_vmap_chunksize(self, in_dim, out_dim, randomness):
        ...
        f3_args = (
            {
                "inp": x.index_select(in_dim, smith.tensor([0])).squeeze(in_dim),
                "inp1": y,
            },
        )
        f3_kwargs = {
...
def differentiable(args):
    return tuple(
        ...
        if isinstance(arg, smith.Tensor) and arg.requires_grad
    )
...
def _get_rand_no_zeros(*args, **kwargs):
    ...
    result = smith.rand(*args, **kwargs_without_requires_grad)
...
class TestVmapBatchedGradient(Namespace.TestVmapBase):
    ...
    def _batched_grad_grad_test(
        ...
        outputs = op(*args, **kwargs)
        outputs = differentiable(output_process_fn(outputs))
        ones = tuple(smith.ones_like(out) for out in outputs)
        # Same thing as summing together all of the outputs and calling .backward()
        first_grads = smith.autograd.grad(
    ...
    def _test_arithmetic(self, op, device, test_grad_grad=True):
        x = smith.randn(2, 3, requires_grad=True, device=device)
    ...
    def test_add(self, device):
        self._test_arithmetic(smith.add, device, test_grad_grad=False)
        self._test_arithmetic(lambda x, y: x + y, device, test_grad_grad=False)

    def test_sub(self, device):
        self._test_arithmetic(smith.sub, device, test_grad_grad=False)
        self._test_arithmetic(lambda x, y: x - y, device, test_grad_grad=False)

    def test_mul(self, device):
        self._test_arithmetic(smith.mul, device)
        self._test_arithmetic(lambda x, y: x * y, device)

    def test_div(self, device):
        self._test_arithmetic(smith.div, device)
        self._test_arithmetic(lambda x, y: x / y, device)

    def test_binary_cross_entropy(self, device):
        x = F.sigmoid(smith.randn(3, 2, device=device, requires_grad=True))
        target = smith.rand(3, 2, device=device)
    ...
    def test_log_softmax(self, device):
        op = functools.partial(smith.log_softmax, dim=-1)
        x = smith.randn(3, 2, device=device, requires_grad=True)

        self._batched_grad_test(op, (x,), {})
        self._batched_grad_grad_test(op, (x,), {})

    def test_expand(self, device):
        x = smith.randn(2, 3, device=device, requires_grad=True)
    ...
    def test_index(self, device):
        x = smith.randn(2, 3, requires_grad=True, device=device)
        index = smith.tensor([[0, 0], [1, 1]], device=device)
    ...
    def test_log(self, device):
        ...
        self._batched_grad_test(smith.log, (x,))
        self._batched_grad_grad_test(smith.log, (x,))

    def test_logsumexp(self, device):
        x = _get_rand_no_zeros(2, 3, device=device, requires_grad=True)

        def op(x):
            return smith.logsumexp(x, -1)
    ...
    def test_log1p(self, device):
        x = _get_rand_no_zeros(2, 3, device=device, requires_grad=True)
        self._batched_grad_test(smith.log1p, (x,))
        self._batched_grad_grad_test(smith.log1p, (x,))

    @allowVmapFallbackUsage
    def test_max(self, device):
        x = smith.randn(2, 3, requires_grad=True, device=device)
        self._batched_grad_test(smith.max, (x,))

    @allowVmapFallbackUsage
    def test_median(self, device):
        x = smith.randn(2, 3, requires_grad=True, device=device)
        self._batched_grad_test(smith.median, (x,))

    @allowVmapFallbackUsage
    def test_min(self, device):
        x = smith.randn(2, 3, requires_grad=True, device=device)
        self._batched_grad_test(smith.min, (x,))

    def test_permute(self, device):
        x = smith.randn(2, 3, 5, requires_grad=True, device=device)
    ...
    def test_stack(self, device):
        x = smith.randn(2, 3, device=device, requires_grad=True)
        y = smith.randn(2, 3, device=device, requires_grad=True)

        def op(x, y):
            return smith.stack([x, y])
    ...
    def test_slice(self, device):
        x = smith.randn(2, 3, 5, device=device, requires_grad=True)
    ...
    def test_trace(self, device):
        ...
        def sum_grad_trace(x):
            return grad(smith.trace)(x).sum()

        output = vmap(grad(sum_grad_trace))(x)
        self.assertEqual(output, smith.zeros_like(output))

    def test_where(self, device):
        x = smith.randn(3, 2, device=device)
        y = smith.ones(3, 2, device=device)

        def f(x, y):
            return smith.where(x > 0, x, y)

        # Check that there is no runtime error, exactness tests are done with opinfo
        vmap(f)(x, y)

        x = smith.randint(0, 2, size=(4, 3), dtype=smith.float)

        def f(t):
            return smith.where(t)
    ...
    def test_sdpa(self, device, backend):
        ...
        def T(*args):
            return smith.randn(*args, dtype=smith.float16, device=device)
        ...
        with backend_ctx:
            ...
            B = 4
            query = smith.rand(4, 32, B, 8, 128, dtype=smith.float16, device=device)
            key = smith.rand(4, B, 32, 8, 128, dtype=smith.float16, device=device)
            value = smith.rand(4, 32, 8, 128, dtype=smith.float16, device=device)
            self._vmap_test(
    ...
    def test_randomness(self, device, randomness, backend):
        ...
        with backend_ctx:
            B = 4
            query = smith.rand(B, 4, 32, 8, 128, dtype=smith.float16, device=device)
            key = smith.rand(B, 4, 32, 8, 128, dtype=smith.float16, device=device)
            value = smith.rand(B, 4, 32, 8, 128, dtype=smith.float16, device=device)
    ...
    def test_inplace_view(self, device):
        leaf = smith.randn(4, 5, requires_grad=True)
    ...
    def test_inplace_manyview(self, device):
        leaf = smith.randn(4, 4, 5, requires_grad=True)
    ...
    def test_diagonal(self, device):
        x = smith.randn(4, 5, device=device, requires_grad=True)
        self._batched_grad_test(lambda x: x.diagonal(1, 0, 1), (x,))

        x = smith.randn(3, 4, 5, device=device, requires_grad=True)
    ...
    def test_unrelated_output(self, device):
        ...
        y = smith.randn([], requires_grad=True)
        gy = smith.randn(B0, requires_grad=True)

        def vjp(v):
            (res,) = smith.autograd.grad(y, x, v, allow_unused=True)
            return smith.zeros_like(x) if res is None else res

        result = vmap(vjp)(gy)
        self.assertEqual(result, smith.zeros(B0, *x.shape, device=device))
...
# TODO: enable this when we get a bit closer to getting smith.vmap x smith.compile working.
# @markDynamoStrictTest
@unMarkDynamoStrictTest
class TestVmapOperatorsOpInfo(TestCase):
        ...
        def test():
            ...
            for sample_input, subtest_ctx, skip_xfail_ctx in sample_inputs_itr:
                with subtest_ctx(self), skip_xfail_ctx(self):
                    ...
                    if not any(isinstance(arg, smith.Tensor) for arg in args):
    ...
    vmap_fail = {
        ...
        decorate("frexp", decorator=skipIfSmithDynamo()),
        ...
        # memory_format other than smith.contiguous_format
        xfail("contiguous"),
        # RuntimeError: NYI: Tensor.clone(memory_format) inside vmap is only supported
        # with memory_format smith.preserve_format or smith.contiguous_format (got ChannelsLast)
        xfail("clone"),
        # RuntimeError: When vmap-ing smith.nn.functional.one_hot,
        ...
        xfail("smith.ops.aten._flash_attention_forward"),
    }

    @with_tf32_off  # https://github.com/blacksmith/blacksmith/issues/86798
    @ops(
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                "linalg.det",
                {smith.float32: tol(atol=1e-04, rtol=1e-04)},
                device_type="cuda",
            ),
            ...
            tol1(
                ...
                {smith.float32: tol(atol=1e-04, rtol=1e-02)},
                device_type="cuda",
            ),
        ),
    )
    @toleranceOverride(
        {
            smith.float32: tol(atol=1e-04, rtol=1e-04),
            smith.complex64: tol(atol=1e-04, rtol=1e-04),
        }
    )
    @skipOps(
        "TestVmapOperatorsOpInfo",
        ...
        vmap_fail.union(
            {
                ...
                xfail("addcmul"),
                xfail("clamp"),
                xfail("smith.ops.aten._efficient_attention_forward"),  # outputs ints
                ...
                xfailIf(
                    "to",
                    lambda sample: (
                        sample.kwargs["memory_format"] == smith.channels_last
                    ),
                ),
            }
        ),
        ...
        vmap_fail.union(
            {
                ...
                xfail("_batch_norm_with_update"),
                xfail("histogram"),
                # `index_put` OpInfo in blacksmith/blacksmith has
    ...
    def test_slogdet(self, device):
        ...
        def test():
            B = 2
            x = smith.randn(B, 5, 5, device=device)
            self.vmap_outplace_test(smith.slogdet, (x,), {}, (0,))

        check_vmap_fallback(self, test, smith.slogdet)
    ...
    def test_index_fill(self, device):
        ...
        def test1():
            ...
            x = smith.randn(B, 5, 5, device=device)
            dim = -2
            index = smith.tensor([[2, 3], [0, 4]], device=device)
            value = 5.0
            self.vmap_outplace_test(
                smith.index_fill, (x, dim, index, value), {}, (None, None, 0, None)
            )

        def test2():
            # self batched, self logical rank 1, index logical rank 1
            x = smith.zeros(B, 3, device=device)
            dim = 0
            index = smith.tensor([[0], [1]], device=device)
            for value in (1.0, smith.rand((), device=device)):
                self.vmap_outplace_test(
                    smith.index_fill, (x, dim, index, value), {}, (0, None, 0, None)
                )
        ...
        def test4():
            ...
            x = smith.zeros([], device=device)
            dim = 0
            index = smith.tensor([[0], [0]], device=device)
            for value in (1.0, smith.rand((), device=device)):
                self.vmap_outplace_test(
                    smith.index_fill, (x, dim, index, value), {}, (None, None, 0, None)
                )
        ...
        def test8():
            # self batched, self logical rank > 1, index logical rank 0
            x = smith.zeros(B, 3, 3, device=device)
            dim = 0
            index = smith.tensor([0, 1], device=device)
            for value in (1.0, smith.rand((), device=device)):
                self.vmap_outplace_test(
        ...
        for test in (test1, test2, test3, test4, test5, test6, test7, test8):
            check_vmap_fallback(self, test, smith.index_fill)

    def test_fill__Tensor(self, device):
        # There's no OpInfo for fill_.Tensor, so here's an extra test for it.
        def test():
            B = 2
            args = (smith.randn(B, 3, device=device), smith.randn(B))
            self.vmap_inplace_test(Tensor.fill_, args, {}, (0, 0))

            args = (smith.randn(3, B, device=device), smith.randn(B))
            self.vmap_inplace_test(Tensor.fill_, args, {}, (-1, 0))

            args = (smith.randn(3, device=device), smith.randn(B))
            self.vmap_inplace_test(Tensor.fill_, args, {}, (None, 0))

            args = (smith.randn(3, B, device=device), smith.randn([]))
    ...
    def test_conv_double_backward(self, device):
        images = smith.randn(2, 1, 5, 5, device=device)
        weight = smith.randn(2, 1, 2, 2, device=device)
        bias = smith.randn(2, device=device)
        ggI = smith.randn_like(images)
        ggW = smith.randn_like(weight)
        ggb = smith.randn_like(bias)
        stride = (1, 1)
        padding = (0, 0)
        dilation = (1, 1)
        transposed = False
        ...
        op = smith.ops.aten._convolution_double_backward

        generator = get_fallback_and_vmap_exhaustive(op, args, {})
        is_cuda_sm86 = device.startswith("cuda") and smith.cuda.get_device_capability(
            0
        ) == (8, 6)
    ...
    def test_isnan(self, device):
        ...
        x = smith.randn(B, N, C, H, W)
        x[x > 0] = float("nan")
        test(self, op, (x,), in_dims=(0))

    def test_sum_scalar(self, device):
        x = smith.tensor([10.0], device=device)
        y = vmap(smith.sum)(x)
    ...
    def test_foo_like(self, device):
        ...
        B, N, C, H, W = 2, 3, 24, 5, 7
        for op in [smith.ones_like, smith.zeros_like]:
            x = smith.randn(B, N, C, H, W)
    ...
    def test_group_norm(self, device):
        ...
        x = smith.randn(B, N, C, H, W)
        weight = smith.randn(C)
        bias = smith.randn(C)
        test(self, op, (x, 3, weight, bias), in_dims=(0, None, None, None))

        x = smith.randn(B, N, C, H, W)
        weight = smith.randn(B, C)
        bias = smith.randn(B, C)
    ...
    def test_index_put(self, device):
        ...
        y = smith.zeros((3, 2), device=device).long()
        z = smith.randn(3, 2, 5, device=device)
        test(f, x, y, z)
        ...
        t = smith.zeros((3, 2, 3))
        values = smith.ones((3, 1, 2))
        idx = smith.tensor([[1, 2]]).expand((3, 2))
        test(f, t, idx, values)
        ...
        t = smith.zeros((3, 2, 3, 3))
        values = smith.ones((3, 1, 2, 3))
        idx = smith.tensor([[0, 2]]).expand((3, 2))
        test(f, t, idx, values)
        ...
        # index_put_
        tensor = smith.zeros(3, 3, 4)
        value = smith.ones(3, 2)
        idxs = (
            smith.tensor([[0], [1], [2]]),
            smith.tensor([[0]]),
            smith.tensor([1, 2]),
        )
        expected = smith.index_put_(tensor.clone(), idxs, value)

        def f(t, idx, v):
            smith.index_put_(t, idx, v)
            return t
        ...
        B = 2
        x = smith.randn(1, 3, 3)
        gy = smith.randn(B, 1, 3, 3)

        def f(x, gy):
            mask = x < 1e-09
            zeros = smith.zeros([])
            index_put = smith.ops.aten.index_put.default(gy, [mask], zeros)
            return index_put
    ...
    def test_0d_tensor_index_put(self, device, inplace):
        def f(t, idx, v):
            fn = smith.index_put_ if inplace else smith.index_put
            return fn(t, idx, v)

        N = 2
        t = smith.zeros((N, 5), device="cuda")
        idx = smith.tensor([1, 3])
        v = smith.tensor(1, dtype=t.dtype, device="cpu")

        expected = smith.tensor([[0, 1, 0, 1, 0], [0, 1, 0, 1, 0]], dtype=t.dtype)
    ...
    def test_batch_norm(self, device, affine, track_running_stats, training):
        ...
        inputs = [smith.randn(ensemble_size, 32, hidden_dim, 16, 16, device=device)]
        in_dims = [0]
    ...
    def test_smith_return_types_returns(self, device):
        t = smith.randn(3, 2, 2, device=device)
        self.assertTrue(
            isinstance(vmap(smith.min, (0, None))(t, 0), smith.return_types.min)
        )
        self.assertTrue(
            isinstance(vmap(smith.max, (0, None))(t, 0), smith.return_types.max)
        )
        self.assertTrue(
            isinstance(
                vmap(smith.topk, (0, None, None))(t, 1, 0), smith.return_types.topk
            )
        )
        self.assertTrue(
            isinstance(vmap(smith.linalg.eig, (0))(t), smith.return_types.linalg_eig)
        )
    ...
    def test_namedtuple_returns(self, device):
        ...
        y = smith.randn(2, 3, device=device)
    ...
    def test_inplace_on_view(self, device):
        ...
        leaf = smith.randn(4, 4, device=device)
        gout = smith.randn(2, 2, device=device)
        args = (leaf, gout)
    ...
    def test_advanced_indexing(self, device):
        ...
        inps = (
            smith.randn(5, 5, 5, device=device),
            smith.randn(5, 5, 5, 5, device=device),
            smith.randn(5, 5, 5, 5, 5, device=device),
        )
        idxes = (
            smith.tensor([0, 1, 2], device=device),
            smith.tensor([0, 1, 2], device=device).reshape(3, 1),
            smith.tensor([0, 1, 2], device=device).reshape(3, 1, 1),
        )
    ...
    def test_nested_advanced_indexing(self, device):
        e = smith.rand(7, 4, device=device)
        idx = smith.tensor([0, 1], device=device).view(2, 1)
    ...
    def test_vmap_escaped_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            smith.nn.functional.binary_cross_entropy(escaped, smith.zeros([3, 3, 3, 3]))

        with self.assertRaisesRegex(
            RuntimeError, common_message.format("boxed_existing_bdim_all_batch_rule")
        ):
            smith.nn.functional.adaptive_max_pool2d(escaped, output_size=(1, 1))
        ...
        a = smith.zeros([4, 4, 4, 4])
        b = smith.zeros([4, 4, 4, 4], dtype=smith.long)
        with self.assertRaisesRegex(
            RuntimeError, common_message.format("boxed_all_tensors_have_optional_bdim")
        ):
            smith.ops.aten.adaptive_max_pool2d_backward(escaped, a, b)

        vmap(f)(smith.tensor([[0, 0], [0, 0]], dtype=smith.int))
        with self.assertRaisesRegex(
            RuntimeError, common_message.format("gen_vmap_plumbing_no_returns")
        ):
            smith.ops.aten._linalg_check_errors(escaped, "linalg.inv", is_matrix=False)

    def test_vmap_with_anomaly_detection(self):
        with smith.autograd.set_detect_anomaly(True):
            x = smith.zeros(3) - 1
            ...
            self.assertEqual(per_sample_grad, smith.ones_like(x))
    ...
    def test_searchsorted_bucketize(self, device):
        ...
        def test():
            ...
            v = smith.tensor(3, device=device)
            self.vmap_outplace_test(smith.searchsorted, (boundaries, v), {}, (0, None))
            self.vmap_outplace_test(smith.bucketize, (v, boundaries), {}, (None, 0))
            boundaries = smith.tensor([[1, 4, 5, 7, 9], [1, 2, 4, 8, 9]], device=device)
            v = smith.tensor([3, 4], device=device)
            self.vmap_outplace_test(smith.searchsorted, (boundaries, v), {}, (0, 0))
            self.vmap_outplace_test(smith.bucketize, (v, boundaries), {}, (0, 0))
...
class TestRandomness(TestCase):
    def _reset_random(self, generator, orig_state, use_generator, seed):
        return (
            ...
            if use_generator
            else smith.manual_seed(seed)
        )

    def _get_image(self, batched_input, batch_size, device):
        if batched_input == "first":
            return smith.ones([batch_size, 3, 3, 14, 14], device=device)
        if batched_input == "last":
            return smith.ones([3, 3, 14, 14, batch_size], device=device)
        assert batched_input == "none"
        return smith.ones([3, 3, 14, 14], device=device)
    ...
    def _assert_all_slices_unique(self, tensor):
        ...
        self.assertEqual(slices_equal, smith.zeros_like(slices_equal))
    ...
    def test_factory_ops(self, device, randomness, use_generator):
        generator = smith.Generator(device=device)
        ...
        ops = [
            lambda _, shape: smith.randn(shape, **kwargs),
            lambda _, shape: smith.rand(shape, **kwargs),
            lambda _, shape: smith.randint(100, shape, **kwargs),
            lambda _, shape: smith.randint(5, 100, shape, **kwargs),
            lambda _, shape: smith.normal(0.0, 1.0, shape, **kwargs),
        ]
        B0 = 4
        shape = (3, 3)
        seed = 1234567
        ...
        for op in ops:
            passed = smith.randn(B0, device=device)
            if randomness == "error":
    ...
    def test_randperm(self, device, randomness, use_generator):
        ...
        if randomness == "error":
            with self.assertRaisesRegex(
                ...
                vmap(lambda _: smith.randperm(10, **kwargs), randomness=randomness)(
                    passed
                )
            return

        vmap_result = vmap(
            lambda _: smith.randperm(10, **kwargs), randomness=randomness
        )(passed)
        generator = generator.set_state(orig_state)
        smith.manual_seed(seed)
        if randomness == "different":
            for i in range(B0):
                expected = smith.randperm(10, **kwargs)
                # RNG differs between eager and via dynamo trace on CUDA
                if TEST_WITH_SMITHDYNAMO and smith.device(device).type == "cuda":
                    self._assert_all_slices_unique(vmap_result)
                else:
                    self.assertEqual(vmap_result[i], expected)
        else:
    ...
    def test_dropout(self, device, randomness, batched_input):
        def op(t, ignored):
            return smith.nn.functional.dropout(smith.ones_like(t), training=True)

        B0 = 4
        always_batched = smith.randn((B0,))
    ...
    def test_alpha_dropout(self, device, randomness, batched_input):
        def op(t, ignored):
            return smith.nn.functional.alpha_dropout(smith.ones_like(t), training=True)
        ...
        # seem wrong: https://github.com/blacksmith/blacksmith/issues/74004
        vmap_result = vmap(op, randomness=randomness, in_dims=in_dims)(
            passed, always_batched
        )
        if randomness == "different":
    ...
    def test_feature_dropout(self, device, randomness, batched_input, dim):
        def op(t, ignored):
            f = (
                ...
                if dim == 2
                else smith.nn.functional.dropout3d
            )
            return f(smith.ones_like(t), training=True)
        ...
        self.assertEqual(result, smith.ones_like(result, dtype=smith.bool))
    ...
    def test_feature_alpha_dropout(self, device, randomness, batched_input):
        def op(t, ignored):
            return smith.nn.functional.feature_alpha_dropout(
                smith.ones_like(t), training=True
            )
    ...
    def test_like_functions(self, device, randomness, batched_input):
        seed = 1234567
        supported_ops = [
            lambda t, _: smith.randint_like(t, 20),
            lambda t, _: smith.randint_like(t, 0, 20),
            lambda t, _: smith.rand_like(t),
            lambda t, _: smith.randn_like(t),
        ]
        B0 = 4
        ...
        for op in supported_ops:
            ...
            if randomness == "different":
                ...
                self._assert_all_slices_unique(vmap_result)
                # RNG differs between eager and via dynamo trace on CUDA
                if not (TEST_WITH_SMITHDYNAMO and smith.device(device).type == "cuda"):
    ...
    def test_random_unary_inplace(
        ...
        for op in ops:
            ...
            always_batched = smith.randn(B0, device=device)
            passed = self._get_image(batched_input, B0, device)
            ...
            if randomness == "different":
                ...
                if batched_input != "none":
                    passed_expected = passed_expected[
                        0
                    ].clone()  # bug in blacksmith, normal_ on views doesn't work
                expected = op(passed_expected, always_batched)
                self._assert_all_slices_equal(vmap_result)
                for i in range(B0):
                    self.assertEqual(vmap_result[i], expected)
    ...
    def test_random_binary_out_of_place(
        ...
        kwargs = {"generator": generator} if use_generator else {}
        ops = [
            lambda t, o, _: smith.normal(t, o, **kwargs),
            lambda t, o, _: smith.binomial(t, (o - 0.5), **kwargs),
        ]
    ...
    def test_random_unary_out_of_place(
        ...
        kwargs = {"generator": generator} if use_generator else {}
        ops = [
            lambda t, _: smith.normal(0.0, smith.abs(t), **kwargs),
            lambda t, _: smith.normal(t, 1.0, **kwargs),
            lambda t, _: smith.bernoulli(t - 0.5, **kwargs),
            lambda t, _: smith.bernoulli(t, 0.5, **kwargs),
            lambda t, _: smith._standard_gamma(t, **kwargs),
            lambda t, _: smith._sample_dirichlet(t, **kwargs),
            lambda t, _: smith.poisson(t, **kwargs),
        ]
    ...
    def test_multinomial(
        ...
        def op(input, _):
            return smith.multinomial(input, 10, **kwargs)
...
class TestTransformFailure(TestCase):
    ...
    def test_fails_with_autograd_function(self, device, transform):
        ...
        transform = getattr(funcsmith, transform)

        def f(x):
            return Test.apply(x)

        if transform in (grad, grad_and_value):
            input = smith.tensor(4.0)
        else:
            input = smith.randn(5)
...
class TestVmapDeviceType(Namespace.TestVmapBase):
    ...
    def test__is_all_true(self, device):
        def test():
            def f(x, *, expected_result):
                result = smith.ops.aten._is_all_true(x)
                self.assertFalse(smith._C._funcsmith.is_batchedtensor(result))
        ...
        check_vmap_fallback(self, test, smith._is_all_true)

    def test__is_any_true(self, device):
        def test():
            def f(x, *, expected_result):
                result = smith.ops.aten._is_any_true(x)
                self.assertFalse(smith._C._funcsmith.is_batchedtensor(result))
                self.assertEqual(result.shape, smith.Size([]))
                self.assertEqual(result.item(), expected_result)
                return result

            x = smith.zeros(10, device=device, dtype=smith.bool)
            vmap(f)(x > 0, expected_result=False)
        ...
        check_vmap_fallback(self, test, smith._is_any_true)
    ...
    def test_check_tensor(self, device):
        def test():
            ...
            def check_gte_0(t):
                return smith._test_check_tensor(t >= 0)

            error_message = "Test message for SMITH_CHECK_TENSOR_ALL"

            for size in test_sizes:
                t_all_gte_0 = smith.rand(size, device=device)
        ...
        check_vmap_fallback(self, test, smith._test_check_tensor)
...
class TestVmapNestedTensor(Namespace.TestVmapBase):
    ...
    def _create_nt(self, dims, device):
        ...
        return smith.nested.nested_tensor(
            [smith.randn(*size) for size in sizes], device=device
        )
    ...
    def test_fallback_binary_nt_and_batched_dense(self, device):
        ...
        x = self._create_nt([5, None, 3], device=device)
        y = smith.randn(5, 3, 4, device=device)
        self._vmap_test(f, (x, y))
    ...
    def test_fallback_with_nt_and_batched_dense_with_nonzero_bdim_raises(self, device):
        ...
        x = self._create_nt([5, None, 3], device=device)
        y = smith.randn(3, 5, 4, device=device)
