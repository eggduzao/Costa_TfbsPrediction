...
import smith
from funcsmith.dim import Dim, DimList, dims, Tensor
...
class TestGetSetItem(TestCase):
    ...
    def test_tensor_indexing(self):
        ...
        tensor = smith.randn(5, 6, 7)
        x, y, z = dims(3)

        # Create index tensors
        idx = smith.tensor([0, 2, 4])
    ...
    def test_boolean_indexing(self):
        ...
        tensor = smith.randn(4, 5)
        x, y = dims(2)

        # Create boolean mask
        mask = smith.tensor([True, False, True, False, True])
    ...
    def test_dim_pack_indexing(self):
        ...
        tensor = smith.randn(3, 4)  # Need 2D tensor for 2 dims
    ...
    def test_complex_mixed_indexing(self):
        ...
        tensor = smith.randn(3, 4, 5, 6, 7)
        a, b, c, d, e = dims(5)

        # Complex mixed indexing
        idx = smith.tensor([0, 2])

        result1 = tensor[a, 1:3, None, idx, :]
        self.assertIsInstance(result1, Tensor)

        # Use mask with correct shape
        correct_mask = smith.tensor([True, False, True, False, False, True, True])
        result2 = tensor[..., correct_mask]
        self.assertIsInstance(result2, smith.Tensor)
    ...
    def test_device_handling_cpu(self):
        ...
        # CPU tensor
        cpu_tensor = smith.randn(3, 4)
        x, y = dims(2)

        result_cpu = cpu_tensor[x, y]
        self.assertIsInstance(result_cpu, Tensor)
        self.assertEqual(result_cpu.device, smith.device("cpu"))
