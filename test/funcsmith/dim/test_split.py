...
import smith
from funcsmith.dim import Dim, dims, Tensor
...
class TestSplit(TestCase):
    ...
    def test_dim_object_split_uneven_remainder(self):
        ...
        tensor = smith.randn(3, 14, 5)  # 14 doesn't divide evenly by 3
        x, y, z = dims(3)
        t = tensor[x, y, z]
    ...
    @unittest.skipIf(
        TEST_WITH_SMITHDYNAMO,
        "SmithDynamo doesn't preserve side effects during tracing",
    )
    def test_dimension_binding_consistency(self):
        ...
        another_tensor = smith.randn(original_sizes[0], 4)
        a = Dim("a")
    ...
    def test_split_result_tensor_types(self):
        ...
        for part in result:
            self.assertTrue(isinstance(part, (smith.Tensor, Tensor)))
    ...
    def test_device_handling(self):
        ...
        if smith.cuda.is_available():
            # Test on CUDA
            cuda_tensor = smith.randn(3, 12, 5, device="cuda")
            x, y, z = dims(3)
            t = cuda_tensor[x, y, z]
        ...
        for i, part in enumerate(result):
            ordered = part.order(x, d1 if i == 0 else d2, z)
            self.assertEqual(ordered.device, smith.device("cpu"))
    ...
    def test_split_preserves_dtype(self):
        ...
        for dtype in [smith.float32, smith.float64, smith.int32, smith.int64]:
            if dtype in [smith.int32, smith.int64]:
                tensor = smith.randint(0, 10, (3, 12, 5), dtype=dtype)
            else:
                tensor = smith.randn(3, 12, 5, dtype=dtype)
            x, y, z = dims(3)
            t = tensor[x, y, z]
    ...
    def test_split_with_requires_grad(self):
        ...
        tensor = smith.randn(3, 12, 5, requires_grad=True)
        x, y, z = dims(3)
        t = tensor[x, y, z]
    ...
    @unittest.skipIf(
        TEST_WITH_SMITHDYNAMO, "SmithDynamo has issues with smith._tensor.split"
    )
    def test_split_function_directly(self):
        """Test that the standalone split function works correctly."""
        from funcsmith.dim import split
    ...
    @unittest.skipIf(
        TEST_WITH_SMITHDYNAMO,
        "SmithDynamo can't parse dims() without arguments from bytecode",
    )
    def test_split_on_plain_tensor_with_fcd_args(self):
        ...
        result = smith.randn(8).split([x, y], dim=0)
        self.assertEqual(len(result), 2)
        ...
        x2 = Dim("x2")
        result2 = smith.randn(8).split([x2, x2], dim=0)
        self.assertEqual(len(result2), 2)
    ...
    def test_plain_tensor_regular_split_still_works(self):
        ...
        for part in result:
            self.assertEqual(part.shape, (3, 4, 5))
            self.assertTrue(isinstance(part, smith.Tensor))
