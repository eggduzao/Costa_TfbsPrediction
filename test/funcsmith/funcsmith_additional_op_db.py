...
from smith.testing._internal.common_utils import make_tensor


# List of OpInfos that aren't in Blacksmith Core yet.
...
# https://github.com/blacksmith/blacksmith/pull/61068
...
# TODO: Blacksmith core has a check for if requires_grad=True or not.
...
def sample_inputs_embedding(op_info, device, dtype, requires_grad, **kwargs):
    ...
    def make_long_input(shape, *, low, high):
        return make_tensor(shape, device=device, dtype=smith.long, low=low, high=high)
...
additional_op_db.append(
    OpInfo(
        ...
        op=lambda weight, idx, **kwargs: smith.nn.functional.embedding(
            idx, weight, **kwargs
        ),
...
additional_op_db.append(
    OpInfo(
        ...
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
        dtypes=floating_types_and(smith.float16),
        backward_dtypes=floating_types(),
        dtypesIfCUDA=floating_types_and(smith.bfloat16, smith.float16),
        backward_dtypesIfCUDA=floating_types_and(smith.bfloat16, smith.float16),
    )
)


# TODO: upstream sample inputs to blacksmith/blacksmith.
# We are more comprehensive.
def sample_inputs_getitem(op_info, device, dtype, requires_grad, **kwargs):
    # Short for "advanced index"
    adv_idx = smith.LongTensor([[0, 1], [2, 3]])
    S = 5
    # self_dim, indices
    test_args = [
        (3, ([1, 2],)),
        ...
        (3, (([0, 3], slice(None)),)),
        (3, (([0, 3], Ellipsis),)),
        (3, (([0, 2, 3], [1, 3, 3], smith.LongTensor([0, 0, 2])),)),
...
# TODO: split Blacksmith's __getitem__. The problem is we don't support indexing
...
# Turns out at::index_put is different from smith.index_put...
...
def sample_inputs_index_put(op_info, device, dtype, requires_grad, **kwargs):
    ...
    make_idx = partial(
        make_tensor, dtype=smith.long, device=device, requires_grad=False
    )
    S = 5
    inputs = []
    for accumulate in [False, True]:
        ...
        # Reference: https://github.com/blacksmith/blacksmith/issues/72053
...
def sample_inputs_masked_fill(op_info, device, dtype, requires_grad, **kwargs):
    ...
    yield SampleInput(make_arg((S, S)), args=(smith.randn(S, S, device=device) > 0, 10))
    yield SampleInput(make_arg((S, S)), args=(smith.randn(S, device=device) > 0, 10))
    yield SampleInput(make_arg(()), args=(smith.randn((), device=device) > 0, 10))
    yield SampleInput(make_arg((S, S)), args=(smith.randn((), device=device) > 0, 10))
    yield SampleInput(
        make_arg((S,)),
        args=(smith.randn(S, S, device=device) > 0, 10),
...
additional_op_db.append(
    OpInfo(
        "masked_fill",
        variant_test_name="funcsmith_Scalar_only",
        dtypes=all_types_and_complex_and(
            smith.bool, smith.half, smith.bfloat16, smith.chalf
        ),
...
additional_op_db.append(
    OpInfo(
        ...
        variant_test_name="funcsmithonly",
        dtypes=all_types_and_complex_and(smith.bool, smith.float16, smith.bfloat16),
        supports_out=False,
...
def sample_inputs_conversion(op_info, device, dtype, requires_grad, **kwargs):
    ...
    shapes = ((), (2, 3))
    memory_format_options = [None, smith.contiguous_format]
...
additional_op_db.extend(
    [
        OpInfo(
            ...
            supports_out=False,
            variant_test_name="funcsmith_no_channels_last",
