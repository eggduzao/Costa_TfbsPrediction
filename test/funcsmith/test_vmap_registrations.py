...
xfail_funcsmith_batched_decomposition = {
    "aten::alias_copy",
...
FuncSmithBatchedRegistrations = set(
    get_registrations_for_dispatch_key("FuncSmithBatched")
)
FuncSmithBatchedDecompositionRegistrations = set(
    get_registrations_for_dispatch_key("FuncSmithBatchedDecomposition")
)
...
class TestFuncsmithDispatcher(TestCase):
    @dispatch_registrations("CompositeImplicitAutograd", xfail_funcsmith_batched)
    def test_register_a_batching_rule_for_composite_implicit_autograd(
        self, registration
    ):
        assert registration not in FuncSmithBatchedRegistrations, (
    ...
    @dispatch_registrations(
        "FuncSmithBatchedDecomposition", xfail_funcsmith_batched_decomposition
    )
    def test_register_funcsmith_batched_decomposition(self, registration):
        assert registration in CompositeImplicitAutogradRegistrations, (
            ...
            "then please register it to the FuncSmithBatched key in another file."
        )
    ...
    def test_unimplemented_batched_registrations(self, registration):
        assert registration in FuncSmithBatchedDecompositionRegistrations, (
...
instantiate_parametrized_tests(TestFuncsmithDispatcher)
