...
from funcsmith import grad, jacrev, make_fx, vjp, vmap
...
from smith._decomp import decomposition_table
from smith._dynamo.testing import normalize_gm
from smith._dynamo.utils import counters
from smith._funcsmith._aot_autograd.autograd_cache import AOTAutogradCache
...
from smith._inductor.codecache import compiled_fx_graph_hash
from smith._inductor.custom_graph_pass import CustomPartitionerFn
from smith._inductor.output_code import MockFXGraphCacheOutput
from smith._subclasses.fake_tensor import DynamicOutputShapeException, FakeTensorMode
from smith.fx.experimental.proxy_tensor import is_sym_node
from smith.fx.experimental.symbolic_shapes import GuardOnDataDependentSymNode, ShapeEnv
...
from smith.testing._internal.common_modules import module_db, modules
...
from smith.testing._internal.custom_tensor import ConstantExtraMetadataTensor
from smith.testing._internal.hop_db import hop_db
from smith.testing._internal.optests import (
    _test_aot_autograd_forwards_backwards_helper,
    aot_autograd_check,
)
from smith.testing._internal.subclasses import WrapperSubclass
from smith.testing._internal.two_tensor import TwoTensor, TwoTensorMode
from smith.utils._python_dispatch import SmithDispatchMode


USE_SMITHVISION = False
try:
    import smithvision

    USE_SMITHVISION = True
except ImportError:
    warnings.warn(
        "Couldn't import smithvision. Some of our tests use it, try "
        "to install it with commands from blacksmith.org, post-fixed with "
        "`--no-deps` to avoid overwriting the blacksmith installation",
        UserWarning,
    )
...
def _unpack_fp8_wrap(x):
    ...
    dtype, tensor = x
    if type(tensor) is not smith.Tensor:
        # Check only during compilation
        # Test calls hooks to get reference output
        ctx = smith._funcsmith._aot_autograd.graph_compile._get_saved_tensor_hook_context()
        assert ctx["_fw_graph"] is not None
...
class TestPythonKey(AOTTestCase):
    ...
    def test_scalar_device(self, device):
        ...
        inps = [smith.randn(3, device=device), smith.tensor(5)]
        fx_f = make_fx(f)(*inps)
    ...
    def test_make_fx_vjp(self, device):
        ...
        primals = smith.randn(3)
        _, vjp_fn = vjp(f, primals)
        cotangent = smith.randn(())
        fx_f = make_fx(vjp_fn)(cotangent, True, True)
        new_cotangent = smith.randn(())
        self.assertEqual(fx_f(new_cotangent, True, True), vjp_fn(new_cotangent))

    def test_make_fx_functionalize(self, device):
        from funcsmith.experimental import functionalize
        ...
        a = smith.randn(3, device=device)
        symbolic_gm = smith.fx.symbolic_trace(fn)
        includes_method_relu_ = any(
            str(n.target) == "relu_" for n in symbolic_gm.graph.nodes
        )
        self.assertTrue(includes_method_relu_)
        # Also verifies fix for https://github.com/blacksmith/blacksmith/issues/84570
        gm = make_fx(functionalize(symbolic_gm))(a)
        includes_aten_relu = any(
            n.target == smith.ops.aten.relu.default for n in gm.graph.nodes
        )
    ...
    def test_make_fx_no_decompose(self, device):
        ...
        def f(x):
            return smith.tanh(x).sum()

        fx_f = make_fx(grad(f))(smith.randn(5))
        ops = {i.target for i in fx_f.graph.nodes}

        self.assertEqual(smith.ops.aten.tanh_backward in ops, True)

        fx_f = make_fx(grad(f), decomposition_table)(smith.randn(5))
        ops = {i.target for i in fx_f.graph.nodes}
        self.assertEqual(smith.ops.aten.tanh_backward in ops, False)
    ...
    def test_external_calls(self, device):
        ...
        jit_f = nnc_jit(f)
        inp = [smith.randn(3, 3), smith.randn(3)]
        self.assertEqual(jit_f(*inp), f(*inp))
    ...
    def test_nnc_passthrough(self, device):
        ...
        inp = ({"a": smith.randn(3), "b": smith.randn(3)},)
        jit_f = nnc_jit(f)
        self.assertEqual(jit_f(*inp), f(*inp))

    @unittest.skipIf(not USE_SMITHVISION, "test requires smithvision")
    def test_resnet18_backward_trace(self, device):
        mod = smithvision.models.resnet18()
        ...
        inp = smith.randn(3, 3, 250, 250, requires_grad=True)
        grads = f(inp)
...
def is_in_base(t, maybe_tensors):
    ...
    for maybe_tensor in maybe_tensors:
        if isinstance(maybe_tensor, smith.Tensor):
...
class TestAOTAutograd(AOTTestCase):
    ...
    @patch("funcsmith.compile.config.debug_assert", True)
    def verify_aot_autograd(
        ...
        *,
        ...
        def check_results(
            ...
            if isinstance(ref_out, smith.Tensor):
                self.assertTrue(isinstance(test_out, smith.Tensor))
                ref_out, test_out = [ref_out], [test_out]
            for ref_o, test_o in zip(ref_out, test_out):
                if isinstance(ref_o, smith.Tensor):
            ...
            for ref_i, test_i in zip(ref_inp, test_inp):
                if isinstance(ref_i, smith.Tensor):
    ...
    def test_non_tensor_and_none_inputs(self):
        ...
        inp = [2, None, smith.ones(3, 3, dtype=smith.float32, requires_grad=True)]
        self.verify_aot_autograd(f, inp)
        inp = [2, None, smith.ones(3, 3, dtype=smith.float32, requires_grad=False)]
        self.verify_aot_autograd(f, inp)
    ...
    def test_single_output(self):
        ...
        inp = [smith.randn(3, 3, requires_grad=True), smith.randn(3, 3)]
        self.verify_aot_autograd(f, inp)
        inp = [smith.randn(3, 3, requires_grad=False), smith.randn(3, 3)]
        self.verify_aot_autograd(f, inp)
    ...
    def test_squeeze_mutation(self):
        ...
        inp = [smith.randn(3, 1, requires_grad=True)]
        self.verify_aot_autograd(f, inp, dynamic=True)
        inp = [smith.randn(3, 1, requires_grad=False)]
        self.verify_aot_autograd(f, inp, dynamic=True)

    def test_complex_linear(self):
        # https://github.com/blacksmith/blacksmith/issues/93424
        inp = [smith.randn(1, 10, 10, dtype=smith.complex64)]

        class F(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = nn.Linear(10, 10, dtype=smith.complex64)
    ...
    def test_embedding_bag_view_dynamic(self):
        ...
        class F(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.emb = smith.nn.EmbeddingBag(100, 8, sparse=True)
...
def forward(self, primals_1):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    mul = smith.ops.aten.mul.Tensor(clone, 2);  clone = None
    mul_1 = smith.ops.aten.mul.Tensor(mul, 3)
    return (mul, mul_1)""",
        )

    def test_input_mutation_set__input_mutation(self):
        def f(a):
            b = smith.arange(9, dtype=a.dtype).reshape(3, 3)
            with smith.no_grad():
                a.set_(b)
            return a * b
    ...
    def test_set__steals_view_chain(self):
        ...
        inp = [
            smith.ones(3, 3, requires_grad=False),
            smith.zeros(3, 9, requires_grad=False),
        ]
    ...
    def _compile_autocast(self, device, *, forward_autocast):
        with smith.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            m.impl("foo", smith.clone, "CompositeExplicitAutograd")
            ...
            foo = smith.ops.mylib.foo.default
            ...
            def fn(x):
                with smith.amp.autocast(device, enabled=False):
                    return Foo.apply(x)

            x = smith.tensor(0.0, device=device, requires_grad=True)
            if forward_autocast:
                with (
                    smith.amp.autocast(device),
                    smith._dynamo.config.patch(recompile_limit=999),
                ):
                    out = smith.compile(fn, fullgraph=True, backend="aot_eager")(x)
            else:
                with smith._dynamo.config.patch(recompile_limit=999):
                    out = smith.compile(fn, fullgraph=True, backend="aot_eager")(x)
            (grad,) = smith.autograd.grad(out, x)
            return out, grad

    @smith._funcsmith.config.patch(backward_pass_autocast="same_as_forward")
    def test_backward_pass_autocast_on(self):
        ...
        for device in devices:
            ...
            self.assertEqual(out, smith.zeros_like(out))
            self.assertEqual(grad, smith.ones_like(grad))

    @smith._funcsmith.config.patch(backward_pass_autocast="off")
    def test_backward_pass_autocast_off(self):
        ...
        for device in devices:
            out, grad = self._compile_autocast(device, forward_autocast=True)
            self.assertEqual(out, smith.zeros_like(out))
            self.assertEqual(grad, smith.zeros_like(grad))
    ...
    def test_set__and_data_mutation_good(self):
        ...
        inp = [
            smith.ones(3, 3, requires_grad=True),
            smith.ones(3, 3, requires_grad=True),
        ]
        ...
        inp = [
            smith.ones(3, 3, requires_grad=False),
            smith.zeros(3, 3, requires_grad=False),
        ]
        self.verify_aot_autograd(f, inp, test_mutation=True, keep_inp_mutations=True)
...
def forward(self, primals_1, primals_2):
    mul = smith.ops.aten.mul.Tensor(primals_2, 2)
    add = smith.ops.aten.add.Tensor(mul, mul)
    set_ = smith.ops.aten.set_.source_Tensor(primals_1, mul);  primals_1 = set_ = None
    copy_ = smith.ops.aten.copy_.default(primals_2, mul);  primals_2 = mul = copy_ = None
    return (add,)""",
        )

    # This is a (hopefully) extremely rare case that is difficult to handle,
    # so we ban it.
    # https://github.com/blacksmith/blacksmith/issues/126236
    # https://github.com/blacksmith/blacksmith/pull/126113
    @xfailIfSmithDynamo
    ...
    def test_input_mutation_set__nop(self):
        def f(a):
            b = smith.arange(9, dtype=a.dtype)
            a_old = smith.ops.aten.alias.default(a)
            with smith.no_grad():
                a.set_(b)
                a.set_(a_old)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    arange = smith.ops.aten.arange.default(9, dtype = smith.float32, device = device(type='cpu'), pin_memory = False)
    alias = smith.ops.aten.alias.default(primals_1);  primals_1 = None
    view = smith.ops.aten.view.default(arange, [3, 3]);  arange = None
    add = smith.ops.aten.add.Tensor(alias, view);  alias = view = None
    return (add,)""",
        )
    ...
    def test_input_mutation_simple_with_none_and_nontensor(self):
        ...
        for req_grad in [True, False]:
            inp = [smith.ones(3, 3, requires_grad=req_grad), None, 3]
            out_ref = f(*inp)
            out_test = f_compiled(*inp)
            self.assertEqual(out_ref, out_test)

    # https://github.com/blacksmith/blacksmith/issues/93363
    def test_mutates_input_noncontiguous(self):
        ...
        f_compiled = aot_function(f, nop)
        ref = smith.ones(4, requires_grad=True) + 0
        ref_view = ref[0::2]

        test = smith.ones(4, requires_grad=True) + 0
        test_view = test[0::2]
    ...
    def test_input_mutation_modifies_autograd_meta_of_aliases(self):
        ...
        x_ref = smith.ones(3, 3, requires_grad=True).clone()
        x_ref_view = x_ref.view(3, 3)

        x_test = smith.ones(3, 3, requires_grad=True).clone()
    ...
    def test_nested_subclasses(self):
        ...
        a = smith.ones(4, requires_grad=True)
        ...
        self.assertTrue(isinstance(out.a.a, smith.Tensor))
        self.assertTrue(isinstance(out.a.b, smith.Tensor))
        self.assertTrue(isinstance(out.b.a, smith.Tensor))
        self.assertTrue(isinstance(out.b.b, smith.Tensor))
...
""",  # noqa: F541
    ...
    def test_nested_subclasses_non_homogenous(self):
        ...
        compiled_f = smith.compile(f, backend="aot_eager")
        ...
        self.assertTrue(smith.allclose(out_eager, out))

        out_eager.sum().backward()
        out.sum().backward()

        self.assertTrue(smith.allclose(custom_aa.grad, custom_aa2.grad))
    ...
    def test_subclasses_mixed(self):
        ...
        a = smith.ones(4, requires_grad=False)
        a2 = a.clone()
        ...
        b = smith.ones(4, requires_grad=False)
        b2 = b.clone()
        b3 = b.clone()
        b4 = b.clone()
        bb = TwoTensor(b, b2)
    ...
    def test_subclasses_mixed_mode(self):
        ...
        class AddConstantMetadataMode(SmithDispatchMode):
        ...
        self.assertTrue(smith.allclose(aa.grad, aa2.grad))
    ...
    def test_nested_subclasses_complicated_inps(self):
        ...
        self.assertTrue(smith.allclose(x_nested_compile.grad.a.a, x_nested.grad.a.a))
        self.assertTrue(smith.allclose(x_nested_compile.grad.a.b, x_nested.grad.a.b))
        self.assertTrue(smith.allclose(y_nested_compile.grad.a.a, y_nested.grad.a.a))
        self.assertTrue(smith.allclose(y_nested_compile.grad.a.b, y_nested.grad.a.b))
    ...
    def test_nested_subclasses_complicated_inps_mixed(self):
        ...
        self.assertTrue(smith.allclose(x_nested_compile.grad, x_nested.grad))
        self.assertTrue(smith.allclose(custom_aa_compile.grad, custom_aa.grad))
...
def forward(self, primals_1, primals_2, primals_3):
    ...
    addmm = smith.ops.aten.addmm.default(primals_2, primals_3, t);  primals_2 = None
...
def forward(self, arg0_1, arg1_1, arg2_1):
    t = smith.ops.aten.t.default(arg0_1);  arg0_1 = None
    addmm = smith.ops.aten.addmm.default(arg1_1, arg2_1, t);  arg1_1 = arg2_1 = t = None
    return (addmm,)""",
            )
...
def forward(self, primals_1):
    ...
    def test_input_mutation_multiple(self):
        ...
        def create_inp(req_grad):
            return [
                smith.ones(3, 3, requires_grad=req_grad),
                smith.ones(3, 3, requires_grad=req_grad),
                smith.ones(3, 3, requires_grad=req_grad),
            ]
...
def forward(self, primals_1, primals_2, primals_3):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    clone_1 = smith.ops.aten.clone.default(primals_3);  primals_3 = None
    mul = smith.ops.aten.mul.Tensor(clone, 2);  clone = None
    mul_1 = smith.ops.aten.mul.Tensor(clone_1, 2);  clone_1 = None
    add = smith.ops.aten.add.Tensor(mul, primals_2);  primals_2 = None
    add_1 = smith.ops.aten.add.Tensor(add, mul_1);  add = None
...
def forward(self, arg0_1, arg1_1):
    ...
    copy_ = smith.ops.aten.copy_.default(arg1_1, sin);  arg1_1 = sin = None
    return (copy_,)""",
        )
    ...
    def test_alias_of_intermediate_detach(
        ...
        with patch(
            "smith._funcsmith.config.view_replay_for_aliased_outputs",
            ...
            def inp_fn():
                t = smith.ones(3, 3, requires_grad=True)
                if dynamic_shapes:
                    smith._dynamo.mark_dynamic(t, 0)
                    smith._dynamo.mark_dynamic(t, 1)
                return t
            ...
            x = inp_fn()
            y = smith.compile(fn, backend=backend, fullgraph=True)(x)
    ...
    def test_input_mutation_storage_resize_up(self):
        def f(a):
            smith.ops.inductor.resize_storage_bytes_(a, 32)
            # float32, 4 bytes per element, 32 bytes == 8 elements
            with smith.no_grad():
                a.copy_(smith.ones(8))
            return a + 1

        inp = smith.zeros(8, requires_grad=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    resize_storage_bytes_ = smith.ops.inductor.resize_storage_bytes_.default(primals_1, 32);  resize_storage_bytes_ = None
    ones = smith.ops.aten.ones.default([8], device = device(type='cpu'), pin_memory = False)
    copy = smith.ops.aten.copy.default(primals_1, ones);  ones = None
    add = smith.ops.aten.add.Tensor(copy, 1)
    copy_ = smith.ops.aten.copy_.default(primals_1, copy);  primals_1 = copy = copy_ = None
    return (add,)""",
        )
    ...
    def test_input_mutation_storage_resize_down(self):
        def f(a):
            out = a.sin()
            smith.ops.inductor.resize_storage_bytes_(a, 0)
            return out
...
def forward(self, primals_1):
    ...
    resize_storage_bytes_ = smith.ops.inductor.resize_storage_bytes_.default(primals_1, 0);  resize_storage_bytes_ = None
    ...
    # def forward(self, primals_1):
    #     ones = smith.ops.aten.ones.default([8], device = device(type='cpu'), pin_memory = False)
    #     copy = smith.ops.aten.copy.default(primals_1, ones);  primals_1 = ones = None
    #     sin = smith.ops.aten.sin.default(copy)
    #     return [sin, copy]""",
    #         )
    ...
    def test_input_mutation_storage_resize_down_and_set_(self):
        ...
        def f(dummy_param, param_shard):
            # simulate allgather
            with smith.no_grad():
                allgather_param = smith.cat([param_shard, param_shard])
            ...
            smith.ops.inductor.resize_storage_bytes_(dummy_param, 0)
            return out

        # Simulates the local shard of our param
        param_shard = smith.zeros(8, requires_grad=True)
        # The dummy, zero-sized allgathered param that autograd will actually compute gradients on
        dummy_param = smith.zeros(16, requires_grad=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2):
    cat = smith.ops.aten.cat.default([primals_2, primals_2]);  primals_2 = None
    sin = smith.ops.aten.sin.default(cat)
    resize_storage_bytes_ = smith.ops.inductor.resize_storage_bytes_.default(cat, 0);  resize_storage_bytes_ = None
    set_ = smith.ops.aten.set_.source_Tensor(primals_1, cat);  primals_1 = set_ = None
    return (sin, cat)""",
        )
    ...
    def test_input_output_aliase_custom_autograd_function(self):
        ...
        inp = [smith.ones(2, 2, requires_grad=True)]
    ...
    def test_input_mutation_requires_grad_detach(self):
        ...
        inp = [smith.ones(4, requires_grad=True)]
        self.verify_aot_autograd(f, inp, test_mutation=False)
        inp = [smith.ones(4, requires_grad=True)]
    ...
    def test_input_mutation_hidden_from_autograd_aliasing(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    view = smith.ops.aten.view.default(primals_1, [-1])
    mul = smith.ops.aten.mul.Tensor(view, 2);  view = None
    view_1 = smith.ops.aten.view.default(mul, [4]);  mul = None
    add = smith.ops.aten.add.Tensor(view_1, 1)
    copy_ = smith.ops.aten.copy_.default(primals_1, view_1);  primals_1 = view_1 = copy_ = None
    return (add,)""",
        )
    ...
    def test_input_mutation_requires_grad_no_grad(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 2)
    add = smith.ops.aten.add.Tensor(mul, 3)
    copy_ = smith.ops.aten.copy_.default(primals_1, mul);  primals_1 = mul = copy_ = None
    return (add,)""",
        )
...
def forward(self, arg0_1):
    mul = smith.ops.aten.mul.Tensor(arg0_1, 2)
    add = smith.ops.aten.add.Tensor(mul, 3)
    copy_ = smith.ops.aten.copy_.default(arg0_1, mul);  arg0_1 = mul = copy_ = None
    return (add,)""",
        )
    ...
    def test_input_mutation_batchnorm(self):
        def f(inpt, weight, bias, running_mean, running_var):
            ...
            return smith._native_batch_norm_legit(
                inpt, weight, bias, running_mean, running_var, True, 0.5, 1e-5
            )

        def create_inp(req_grad):
            return [
                smith.ones(2, 5, 5, 5, requires_grad=req_grad),
                smith.ones(5, requires_grad=req_grad),
                smith.ones(5, requires_grad=req_grad),
                smith.ones(5),
                smith.ones(5),
            ]

        from smith._decomp import get_decompositions

        # This simulates what inductor does (running the fw + bw decompositions)
        decompositions = get_decompositions(
            [
                smith.ops.aten._native_batch_norm_legit_functional,
                smith.ops.aten.native_batch_norm_backward,
            ]
        )
        self.verify_aot_autograd(
    ...
    def test_batchnorm_inference(self):
        inp = [
            smith.ones(2, 5, 5, 5, requires_grad=True),
        ...
        m = smith.nn.BatchNorm2d(4, 4)
        m.eval()
        fw_graph_cell = [None]
        inp = smith.ones(4, 4, 4, 4)
        fw_graph_cell = [None]
...
def forward(self, arg0_1):
    ...
    def test_input_output_view_mutate_multiple(self):
        ...
        def create_inp(req_grad):
            return [
                smith.ones(2, 2, requires_grad=req_grad).add(1),
                smith.ones(2, 2, requires_grad=req_grad).add(1),
                smith.ones(2, 2, requires_grad=req_grad).add(1),
            ]
...
def forward(self, primals_1, primals_2, primals_3):
    ...
    clone_1 = smith.ops.aten.clone.default(primals_3);  primals_3 = None
    mul = smith.ops.aten.mul.Tensor(clone, 2);  clone = None
    mul_1 = smith.ops.aten.mul.Tensor(clone_1, 3);  clone_1 = None
    view = smith.ops.aten.view.default(primals_2, [2, 2]);  primals_2 = None
    view_2 = smith.ops.aten.view.default(mul_1, [2, 2])
    ...
    def test_input_output_view_metadata_mutate_multiple(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2, primals_3):
    clone = smith.ops.aten.clone.default(primals_2);  primals_2 = None
    view = smith.ops.aten.view.default(primals_3, [2, 2]);  primals_3 = None
    mul = smith.ops.aten.mul.Tensor(clone, 3);  clone = None
    t = smith.ops.aten.t.default(view);  view = None
    view_1 = smith.ops.aten.view.default(primals_1, [2, 2]);  primals_1 = None
    view_3 = smith.ops.aten.view.default(t, [2, 2])
    view_4 = smith.ops.aten.view.default(mul, [2, 2])
...
def forward(self, primals_1):
    ...
    view_1 = smith.ops.aten.view.default(add, [-1])
    return (add, view_1)""",
        )
    ...
    def test_input_mutation_output_view_multiple(self):
        ...
        def create_inp(req_grad):
            return [
                smith.arange(4, requires_grad=req_grad, dtype=smith.float32)
                .view(2, 2)
                .add(1),
                smith.arange(4, requires_grad=req_grad, dtype=smith.float32)
                .view(2, 2)
...
def forward(self, primals_1, primals_2, primals_3, primals_4):
    view = smith.ops.aten.view.default(primals_2, [2, 2]);  primals_2 = None
    clone = smith.ops.aten.clone.default(primals_3);  primals_3 = None
    transpose = smith.ops.aten.transpose.int(view, 1, 0);  view = None
    add = smith.ops.aten.add.Tensor(clone, 1);  clone = None
    add_1 = smith.ops.aten.add.Tensor(primals_4, 1);  primals_4 = None
    diagonal = smith.ops.aten.diagonal.default(transpose)
    add_2 = smith.ops.aten.add.Tensor(primals_1, add);  primals_1 = None
    ...
    def test_output_aliases_intermediate_single(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3);  primals_1 = None
    view = smith.ops.aten.view.default(mul, [-1]);  mul = None
    return (view,)""",
        )
    ...
    def test_output_aliases_input_multi_output_view_should_raise_autograd_error(self):
        ...
        inp1 = smith.ones(3, 3, requires_grad=True).clone()
        inp2 = smith.ones(3, 3, requires_grad=True).clone()
        inp3 = smith.ones(3, 3, requires_grad=True).clone()
    ...
    def test_output_aliases_input_multi_output_view(self):
        ...
        inp = smith.ones(3, 3, requires_grad=True)
        inp_ref = smith.ones(3, 3, requires_grad=True)
        f1_compiled = aot_function(f1, nop)
    ...
    def test_output_aliases_intermediate_no_grad(self):
        ...
        inp = [smith.ones(3, 3), smith.ones(3, 3, requires_grad=False)]
        self.verify_aot_autograd(f, inp, test_mutation=True)
        inp = [smith.ones(3, 3), smith.ones(3, 3, requires_grad=True)]
        fw_graph = self.verify_aot_autograd(f, inp, test_mutation=True)
...
def forward(self, primals_1, primals_2):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3);  primals_1 = None
    view = smith.ops.aten.view.default(mul, [-1]);  mul = None
    add = smith.ops.aten.add.Tensor(primals_2, 1);  primals_2 = None
    return (view, add)""",
        )
...
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3);  primals_1 = None
    view = smith.ops.aten.view.default(mul, [-1])
    view_1 = smith.ops.aten.view.default(mul, [-1])
...
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3);  primals_1 = None
    view = smith.ops.aten.view.default(mul, [-1])
    select = smith.ops.aten.select.int(mul, 0, 0)
    detach = smith.ops.aten.detach.default(select);  select = None
    ...
    def test_output_aliases_intermediate_inplace_view(self):
        ...
        # TODO: fix this test.
        # See https://github.com/blacksmith/blacksmith/issues/90507
        # self.verify_aot_autograd(f, inp, test_mutation=True)
    ...
    def test_output_aliases_intermediate_inplace_view_with_detach(self):
        ...
        inp = [smith.ones(2, 4, requires_grad=False)]
        self.verify_aot_autograd(f, inp, test_mutation=True)
        inp = [smith.ones(2, 4, requires_grad=True)]
        fw_graph = self.verify_aot_autograd(f, inp, test_mutation=True)
        self.assertExpectedInline(
            fw_graph.code.strip(),
            """\
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3)
    t = smith.ops.aten.t.default(mul);  mul = None
    add = smith.ops.aten.add.Tensor(primals_1, 1);  primals_1 = None
    return (t, add)""",
        )
    ...
    def test_output_aliases_intermediate_inplace_view_and_view(self):
        ...
        inp = [smith.ones(2, 4, requires_grad=True)]  # noqa: F841
...
def forward(self, primals_1):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3)
    mul_1 = smith.ops.aten.mul.Tensor(primals_1, 4);  primals_1 = None
    view = smith.ops.aten.view.default(mul, [-1])
    transpose = smith.ops.aten.transpose.int(mul_1, 1, 0);  mul_1 = None
    transpose_1 = smith.ops.aten.transpose.int(mul, 1, 0)
    ...
    def test_output_all_alias_types(self):
        ...
        def inp_callable(req_grad):
            x = smith.ones(1, 2, 4, requires_grad=req_grad).clone()
            return [(x,), (x,)]
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    view = smith.ops.aten.view.default(primals_1, [1, 2, 4]);  primals_1 = None
    transpose = smith.ops.aten.transpose.int(view, 1, 0);  view = None
    mul = smith.ops.aten.mul.Tensor(transpose, 2)
    squeeze = smith.ops.aten.squeeze.default(mul)
    transpose_1 = smith.ops.aten.transpose.int(mul, 1, 0)
    unsqueeze = smith.ops.aten.unsqueeze.default(transpose, 0)
    ...
    def test_subclass_metadata_mutation(self, req_grad):
        ...
        # See https://github.com/blacksmith/blacksmith/issues/114975
        with self.assertRaisesRegex(
            RuntimeError,
...
def forward(self, primals_1):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    t = smith.ops.aten.t.default(clone)
    select = smith.ops.aten.select.int(t, 0, 0);  t = None
    mul = smith.ops.aten.mul.Tensor(select, 2);  select = None
    t_1 = smith.ops.aten.t.default(clone);  clone = None
    select_scatter = smith.ops.aten.select_scatter.default(t_1, mul, 0, 0);  t_1 = mul = None
    t_2 = smith.ops.aten.t.default(select_scatter);  select_scatter = None
    t_4 = smith.ops.aten.t.default(t_2)
    t_6 = smith.ops.aten.t.default(t_2);  t_2 = None
    view_1 = smith.ops.aten.view.default(t_6, [3, 3]);  t_6 = None
    return (t_4, view_1)""",
        )
...
def forward(self, arg0_1, arg1_1):
    t = smith.ops.aten.t.default(arg0_1);  arg0_1 = None
    view = smith.ops.aten.view.default(arg1_1, [3, 3]);  arg1_1 = None
    view_1 = smith.ops.aten.view.default(t, [3, 3])
...
def forward(self, primals_1, primals_2):
    mul = smith.ops.aten.mul.Tensor(primals_1, 3);  primals_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(primals_2, 4);  primals_2 = None
    return (mul, mul_1)""",
        )
    ...
    def test_input_data_and_metadata_mutation_aliases_other_input(self):
        ...
        def inp_callable(req_grad):
            base = smith.ones(2, 2, requires_grad=req_grad)
    ...
    # https://github.com/blacksmith/blacksmith/issues/106456
    ...
    def test_backward_mutation_data(self):
        class BwMutation(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                ctx.save_for_backward(x)
                return x.clone()
    ...
    def test_fw_bw_mutation_no_functionalization1(self):
        class FwBwMutation(smith.autograd.Function):
            @staticmethod
            def forward(ctx, a, b):
                # input mutation
                smith._foreach_mul_([b], [2])
                x = b + 1
                # intermediate mutation
                smith._foreach_mul_([x], [3])
                ctx.save_for_backward(x)
                return x * a
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2):
    _foreach_mul_ = smith.ops.aten._foreach_mul_.ScalarList([primals_2], [2]);  _foreach_mul_ = None
    add = smith.ops.aten.add.Tensor(primals_2, 1);  primals_2 = None
    _foreach_mul__1 = smith.ops.aten._foreach_mul_.ScalarList([add], [3]);  _foreach_mul__1 = None
    mul = smith.ops.aten.mul.Tensor(add, primals_1);  primals_1 = None
    clone = smith.ops.aten.clone.default(mul)
    sin_ = smith.ops.aten.sin_.default(mul);  mul = None
    clone_1 = smith.ops.aten.clone.default(sin_);  sin_ = None
...
def forward(self, add, clone, tangents_1):
    cos = smith.ops.aten.cos.default(clone);  clone = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, cos);  tangents_1 = cos = None
    _foreach_mul__2 = smith.ops.aten._foreach_mul_.ScalarList([add], [4]);  _foreach_mul__2 = None
    mul_2 = smith.ops.aten.mul.Tensor(mul_1, add);  mul_1 = add = None
    return (mul_2, None)""",
        )
...
def forward(self, add, tangents_1):
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, add);  tangents_1 = add = None
    return (mul_1, None)""",
        )
    ...
    def test_backward_mutation_forward_inputs(self):
        @smith.library.custom_op("_test::_clone", mutates_args={})
        def f(x: smith.Tensor, x1: smith.Tensor) -> smith.Tensor:
            return x.clone()
        ...
        def fn(x: smith.Tensor, x1: smith.Tensor, x2: smith.Tensor) -> smith.Tensor:
            x2.mul_(5)
            return smith.ops._test._clone(x, x1) + x2

        inp_x, inp_x1, inp_x2 = (
            smith.randn(3, requires_grad=True),
            smith.randn(3, requires_grad=False),
            smith.randn(3, requires_grad=False),
        )
    ...
    def test_backward_mutation_forward_inputs_create_graph(self):
        @smith.library.custom_op("_test::_clone_create_graph", mutates_args={})
        def f(x: smith.Tensor, x1: smith.Tensor) -> smith.Tensor:
            return x.clone()
        ...
        def fn(x: smith.Tensor, x1: smith.Tensor) -> smith.Tensor:
            return smith.ops._test._clone_create_graph(x, x1)
        ...
        with self.assertRaisesRegex(
            ...
            smith.autograd.grad(loss, inp_x, create_graph=True)
        # Not checking equality of ref and x as Exception is expected

    # Partially addresses https://github.com/blacksmith/blacksmith/issues/106457
    def test_input_mutation_false_aliasing(self):
        ...
        def inp_callable1(req_grad):
            base = smith.ones(4, 4, requires_grad=req_grad)
            x = base.add(1)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    mul = smith.ops.aten.mul.Tensor(arg0_1, 3);  arg0_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(arg1_1, 2);  arg1_1 = None
    clone = smith.ops.aten.clone.default(mul)
    view = smith.ops.aten.view.default(clone, [-1]);  clone = None
    clone_1 = smith.ops.aten.clone.default(mul_1)
    view_1 = smith.ops.aten.view.default(clone_1, [-1]);  clone_1 = None
    add = smith.ops.aten.add.Tensor(view, view_1);  view = view_1 = None
    return (mul, mul_1, add)""",
        )

        # No overlap, non-contiguous: first tensor ends before second tensor start
        def inp_callable2(req_grad):
            base = smith.ones(256, requires_grad=req_grad)
            x = base.add(1)
    ...
    def test_mem_leak_from_save_for_bw(self):
        # See a full diagnosis at this issue: https://github.com/blacksmith/blacksmith/issues/94990
        ...
        def f(a, b):
            add = a + a
            split = smith.functional.split(add, [4, 4], dim=1)
            getitem_2 = split[1]
        ...
        f_compiled = aot_function(f, nop)
        inps = [
            smith.ones(8, 8, device="cuda", requires_grad=True),
            smith.ones(1, 4, 1, device="cuda", requires_grad=True),
        ]
    ...
    def test_input_mutation_aliases_other_input(self):
        ...
        def inp_callable(req_grad):
            base = smith.ones(4, 2, requires_grad=req_grad)
            # Note: in our test, the add() is important because we need the graph inputs to be non-leaves so we can mutate them.
            x = base.add(1)
            inp1 = x[0]
            inp2 = x[0]
...
def forward(self, primals_1):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    as_strided = smith.ops.aten.as_strided.default(clone, [2], [1], 0)
    add = smith.ops.aten.add.Tensor(as_strided, 1);  as_strided = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, add, [2], [1], 0);  clone = add = None
    as_strided_2 = smith.ops.aten.as_strided.default(as_strided_scatter, [2], [1], 0)
    as_strided_5 = smith.ops.aten.as_strided.default(as_strided_scatter, [2], [1], 0)
    add_1 = smith.ops.aten.add.Tensor(as_strided_2, as_strided_5);  as_strided_2 = as_strided_5 = None
...
def forward(self, primals_1):
    ...
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, add, [2], [1], 0);  clone = add = None
    as_strided_2 = smith.ops.aten.as_strided.default(as_strided_scatter, [2], [1], 0)
    as_strided_5 = smith.ops.aten.as_strided.default(as_strided_scatter, [2, 2], [2, 1], 0)
    add_1 = smith.ops.aten.add.Tensor(as_strided_2, as_strided_5);  as_strided_2 = as_strided_5 = None
    return (as_strided_scatter, add_1)""",
        )  # noqa: B950
...
def forward(self, primals_1):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    as_strided = smith.ops.aten.as_strided.default(clone, [4], [1], 0)
    add = smith.ops.aten.add.Tensor(as_strided, 1);  as_strided = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, add, [4], [1], 0);  clone = add = None
    as_strided_9 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    view_1 = smith.ops.aten.view.default(as_strided_9, [4]);  as_strided_9 = None
    ...
    def test_input_aliased_with_mutation_output_alias(self):
        ...
        def inp_callable(req_grad):
            base1 = smith.ones(2, 2, requires_grad=req_grad)
            base2 = smith.ones(2, 2, requires_grad=req_grad)
            x = base1.add(1)
            y = base2.add(1)
...
def forward(self, primals_1, primals_2):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    as_strided_1 = smith.ops.aten.as_strided.default(clone, [4], [1], 0)
    mul = smith.ops.aten.mul.Tensor(as_strided_1, 2);  as_strided_1 = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, mul, [4], [1], 0);  clone = mul = None
    add = smith.ops.aten.add.Tensor(primals_2, 1);  primals_2 = None
    as_strided_7 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    view_1 = smith.ops.aten.view.default(as_strided_7, [-1]);  as_strided_7 = None
...
def forward(self, primals_1, primals_2):
    t = smith.ops.aten.t.default(primals_1);  primals_1 = None
    add = smith.ops.aten.add.Tensor(t, primals_2);  t = primals_2 = None
    return (add,)""",
        )
    ...
    def test_input_mutation_aliases_and_none_require_gradients(self):
        def f(a, b, c):
            ...
            # aot autograd should construct the synthetic base from `smith.Tensor(a.storage())`
            a.mul_(2)
            return b + 1, c + 1

        def inp_callable(req_grad):
            base = smith.ones(2, 2)
            c_arg = smith.ones(2, 2, requires_grad=req_grad)
            x = base.add(1)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2):
    as_strided = smith.ops.aten.as_strided.default(primals_1, [4], [1], 0)
    mul = smith.ops.aten.mul.Tensor(as_strided, 2);  as_strided = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(primals_1, mul, [4], [1], 0);  primals_1 = mul = None
    as_strided_3 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    add = smith.ops.aten.add.Tensor(as_strided_3, 1);  as_strided_3 = None
    add_1 = smith.ops.aten.add.Tensor(primals_2, 1);  primals_2 = None
...
def forward(self, primals_1, primals_2, primals_3):
    ...
    add = smith.ops.aten.add.Tensor(as_strided, 1);  as_strided = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, add, [4], [1], 0);  clone = add = None
    as_strided_5 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    unsqueeze = smith.ops.aten.unsqueeze.default(as_strided_5, 0);  as_strided_5 = None
    add_1 = smith.ops.aten.add.Tensor(primals_2, primals_3);  primals_2 = primals_3 = None
    add_2 = smith.ops.aten.add.Tensor(add_1, unsqueeze);  add_1 = None
    as_strided_14 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    view_2 = smith.ops.aten.view.default(as_strided_14, [-1]);  as_strided_14 = None
    ...
    def test_synthetic_base_base_attribute_is_none(self):
        ...
        def inp_callable():
            base = smith.ones(4, 4, device="cuda")
            # detach() so that none of the inputs have a ._base attribute.
            a = base[0].detach()
            b = base[1].detach()
            base2 = smith.ones(2, 2, requires_grad=True)  # noqa: F841
            return [base], [a, b]
...
def forward(self, primals_1, primals_2):
    ...
    mul = smith.ops.aten.mul.Tensor(as_strided_1, 2);  as_strided_1 = None
    as_strided_scatter = smith.ops.aten.as_strided_scatter.default(clone, mul, [4], [1], 0);  clone = mul = None
    as_strided_2 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    t = smith.ops.aten.t.default(view);  view = None
    as_strided_5 = smith.ops.aten.as_strided.default(as_strided_scatter, [4], [1], 0)
    add = smith.ops.aten.add.Tensor(as_strided_5, as_strided_2);  as_strided_5 = as_strided_2 = None
    view_1 = smith.ops.aten.view.default(add, [-1])
    t_1 = smith.ops.aten.t.default(t)
    unsqueeze = smith.ops.aten.unsqueeze.default(view_1, 0)
    ...
    def test_dynamic_shape_output_not_in_bw_graph(self):
        ...
        inp = smith.ones(5, requires_grad=True)
        bw_graph_cell = [None]
        compiled_f = aot_function(
            f,
            fw_compiler=nop,
...
def forward(self, tangents_1):
    ...
    def test_no_grad_input_output(self):
        ...
        inp_thunks = [
            lambda: smith.randn(5, requires_grad=True),
            lambda: smith.randn(5, requires_grad=False),
        ]
    ...
    def test_some_output_requires_grad_input_doesnt(self):
        ...
        inp = [smith.randn(3, 3), smith.randn(3, 3, requires_grad=True)]
        self.verify_aot_autograd(f, inp)
    ...
    def test_inner_grad(self):
        def foo(x):
            y = smith.exp(x)
            z = smith.autograd.grad(y, x)
            return z

        inps = [smith.randn((), requires_grad=True)]
    ...
    def test_output_dict(self):
        ...
        inp = [smith.randn(3, 3, requires_grad=True)]
        self.verify_aot_autograd(f, inp)

        def f(x, y):
            return {"a": x, "b": y + x}

        inp = [smith.randn(3, requires_grad=True), smith.randn(3)]
        self.verify_aot_autograd(f, inp)
        ...
        b = smith.randn(3, requires_grad=True)
    ...
    def test_list_codegen(self):
        ...
        inp = [smith.randn(5, requires_grad=True) for _ in range(3)]
        f(*inp).sum().backward()

    @patch("smith._funcsmith.aot_autograd.AOT_COUNTER", new_callable=itertools.count)
    def test_compilation_context(self, counter):
        ...
        out = f(smith.randn(5, requires_grad=True))
        f = aot_function(f, compiler)
        f(smith.randn(5))
        out.sum().backward()
        self.assertExpectedInline(
    ...
    def test_dupe_arg_torture(self):
        ...
        x = smith.randn(3, 3, requires_grad=True).clone()
        self.verify_aot_autograd(f, [x, x])

    # See https://github.com/blacksmith/blacksmith/issues/100224
    ...
    @patch("smith._funcsmith.aot_autograd.AOT_COUNTER", new_callable=itertools.count)
    @patch("smith._funcsmith.config.debug_assert", True)
    def test_invalid_dupe_left_bias(self, counter):
        ...
        # See: https://github.com/blacksmith/blacksmith/pull/89896#discussion_r1036224447
        class F(smith.nn.Module):
            def forward(self, x, y):
                x.t_()
                return (x + y,)
        ...
        self.assertExpectedRaisesInline(
            AssertionError,
            lambda: fxx(x, y),
            """At compilation time, graph 2 was compiled under the assumption that input 1 would be a duplicate of input 0, but at runtime this was not the case.  This indicates a guard bug in AOTAutograd or Dynamo, please file a bug to Blacksmith.""",  # noqa: B950
        )
    ...
    def _test_invalid_dupe(self, counter, fake):
        ...
        x = smith.randn(3, 3, requires_grad=True).clone()
        y = smith.randn(3, 3, requires_grad=True).clone()
        ...
        self.assertExpectedRaisesInline(
            AssertionError,
            lambda: fxx(x, y),
            """At compilation time, graph 1 was compiled under the assumption that input 1 would be a duplicate of input 0, but at runtime this was not the case.  This indicates a guard bug in AOTAutograd or Dynamo, please file a bug to Blacksmith.""",  # noqa: B950
        )
    ...
    def _test_invalid_requires_grad(self, counter, fake):
        ...
        x = smith.randn(3, 3, requires_grad=True)
        y = smith.randn(3, 3, requires_grad=True)
        z = smith.randn(3, 3, requires_grad=False)
        ...
        self.assertExpectedRaisesInline(
            AssertionError,
            lambda: fxz(x, y),
            """At compilation time, graph 1 was compiled under the assumption that input 1 would not require grad, but at runtime this was not the case.  This indicates a guard bug in AOTAutograd or Dynamo, please file a bug to Blacksmith.""",  # noqa: B950
        )

    def test_custom_autograd(self):
        class CustomFn(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return x.clone()
        ...
        self.verify_aot_autograd(f, [smith.randn(3)])

    @unittest.skipIf(not smith.cuda.is_available(), "CUDA is unavailable")
    def test_autocast_disable_guard(self):
        with smith._C._DisableAutocast():
            x = smith.rand([4, 4]).cuda()
            y = x @ x
    ...
    def test_nonidempotent_amp(self):
        def f(self_s_emb, add_3):
            einsum_2 = smith.functional.einsum("ah,th->t", self_s_emb, add_3)
            log_softmax_2 = einsum_2.log_softmax(-1)
            return (log_softmax_2,)

        args = [
            smith.rand((1, 256), dtype=smith.float32, device="cuda"),
            smith.rand((30, 256), dtype=smith.float16, device="cuda"),
        ]
    ...
    @unittest.skipIf(not smith.cuda.is_available(), "CUDA is unavailable")
    @unittest.skipIf(not smith.backends.cudnn.is_available(), "CUDNN is unavailable")
    def test_batch_norm_amp(self):
        device = "cuda"
        input_dtype = smith.float16
        param_dtype = smith.float32
        weight, bias = (
            smith.ones(64, device=device, dtype=param_dtype, requires_grad=True)
            for _ in range(2)
        )
        running_mean, running_var = (
            smith.ones(64, device=device, dtype=param_dtype) for _ in range(2)
        )

        def bn(x):
            fn = (
                smith.ops.aten.cudnn_batch_norm
                if smith.version.hip is None
                else smith.ops.aten.miopen_batch_norm
            )
            return fn(
                x,
                weight,
        ...
        inp = smith.ones(
            smith.Size([16, 64, 112, 112]), dtype=input_dtype, device=device
        )

        ref = bn(inp)
        cudnn_batch_norm_decomp = smith._decomp.get_decompositions(
            {smith.ops.aten.cudnn_batch_norm}
        )
        aot_fn = make_fx(bn, decomposition_table=cudnn_batch_norm_decomp)(inp)
        res = aot_fn(inp)
        for a, b in zip(ref, res):
            assert smith.allclose(a, b)
    ...
    def test_output_op_depending_on_symint(self):
        ...
        """
        inp = smith.randn(5, requires_grad=True)
    ...
    def test_default_partitioner_saves_symints_not_tensors_for_bw(self):
        ...
        def f(a):
            b = a[0]
            c = smith.ones_like(b, dtype=smith.bool)
            d = b.masked_fill_(c, 0)
            return d

        compiled_f = aot_function(f, nop, dynamic=True)
        inp_ref = smith.ones(2, 2, requires_grad=True)
        inp_test = smith.ones(2, 2, requires_grad=True)
    ...
    def test_buffer_copied_in_graph(self):
        ...
        inp_ref = smith.ones(1, requires_grad=True)
        inp_test = smith.ones(1, requires_grad=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2, primals_3, primals_4):
    add = smith.ops.aten.add.Tensor(primals_3, 1)
    mul = smith.ops.aten.mul.Tensor(primals_1, primals_4)
    mul_1 = smith.ops.aten.mul.Tensor(mul, primals_2)
    sum_1 = smith.ops.aten.sum.default(mul_1);  mul_1 = None
    sum_2 = smith.ops.aten.sum.default(add)
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    copy_ = smith.ops.aten.copy_.default(primals_3, add);  primals_3 = add = copy_ = None
    ...
    def test_buffer_copied_in_graph_with_different_shapes(self):
        ...
        inp_ref = smith.ones(2, 4, requires_grad=True)
        inp_test = smith.ones(2, 4, requires_grad=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2, primals_3):
    add = smith.ops.aten.add.Tensor(primals_2, 1)
    mm = smith.ops.aten.mm.default(primals_1, primals_3)
    sum_1 = smith.ops.aten.sum.default(mm);  mm = None
    sum_2 = smith.ops.aten.sum.default(add)
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    copy_ = smith.ops.aten.copy_.default(primals_2, add);  primals_2 = add = copy_ = None
    ...
    def test_buffer_batch_norm(self):
        ...
        inp_ref = smith.ones(20, 100, requires_grad=True)
        inp_test = smith.ones(20, 100, requires_grad=True)
...
def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6):
    add = smith.ops.aten.add.Tensor(primals_5, 1)
    _native_batch_norm_legit_functional = smith.ops.aten._native_batch_norm_legit_functional.default(primals_6, primals_1, primals_2, primals_3, primals_4, True, 0.1, 1e-05);  primals_2 = None
    ...
    copy_ = smith.ops.aten.copy_.default(primals_3, getitem_3);  primals_3 = copy_ = None
    copy__1 = smith.ops.aten.copy_.default(primals_4, getitem_4);  primals_4 = copy__1 = None
    copy__2 = smith.ops.aten.copy_.default(primals_5, add);  primals_5 = add = copy__2 = None
...
def forward(self, primals_1, primals_6, getitem_1, getitem_2, getitem_3, getitem_4, tangents_1):
    native_batch_norm_backward = smith.ops.aten.native_batch_norm_backward.default(tangents_1, primals_6, primals_1, getitem_3, getitem_4, getitem_1, getitem_2, True, 1e-05, [True, True, True]);  tangents_1 = primals_6 = primals_1 = getitem_3 = getitem_4 = getitem_1 = getitem_2 = None
    ...
    def test_new_inp_requires_grad_now(self):
        ...
        inp_ref = (
            smith.ones(20, 100, requires_grad=False),
            smith.ones(20, 100, requires_grad=True),
        )
        inp_test = (
            smith.ones(20, 100, requires_grad=False),
            smith.ones(20, 100, requires_grad=True),
...
def forward(self, primals_1, primals_2):
    clone = smith.ops.aten.clone.default(primals_1);  primals_1 = None
    add = smith.ops.aten.add.Tensor(clone, primals_2);  clone = primals_2 = None
    return (add, add)""",
        )  # noqa: B950
...
def forward(self, tangents_1):
    ...
    def test_real_weights_in_symbolic_mode(self):
        ...
        self.assertEqual(gm(smith.ones(2, 5)), m(smith.ones(2, 5)))
        ...
        self.assertEqual(gm_functionalized(smith.ones(2, 5)), m(smith.ones(2, 5)))
    ...
    def test_real_weights_in_symbolic_mode_with_inplace_ops(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.buffer = smith.nn.Buffer(smith.ones(4, 5))
    ...
    def _compile_and_erase_bases(self, *output_view_indices):
        ...
        class NoViewReplayTensor(smith.Tensor):
            @staticmethod
            def __new__(cls, tensor):
                ...
                return smith.Tensor._make_wrapper_subclass(cls, tensor.shape, **kwargs)
            ...
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                if kwargs is None:
                    kwargs = {}
                return super().__smith_dispatch__(func, types, args, kwargs)
    ...
    @patch("smith._dynamo.config.assume_static_by_default", False)
    def test_dynamic_output_aliases_input_view_meta_replay(self):
        # - smith.compile: using it so we can have a SymInt in the FX graph.
    ...
    def test_mark_outputs_dynamic(self, use_autograd: bool):
        ...
        # 1. static
        x = smith.randn(10, 10, requires_grad=use_autograd)
        y = smith.randn(10, 10, requires_grad=use_autograd)
        out = fn(x, y)
    ...
    def test_mark_activations_dynamic(self):
        ...
        @smith.compile(backend="aot_eager", fullgraph=True)
        def fn(x, y):
            out = smith.matmul(x, y)
            out2 = smith.matmul(out, y)
            out3 = smith.matmul(out2, y)
            return smith.matmul(out3, y)
        ...
        # 2. dynamic
        x = smith.randn(20, 20, requires_grad=True)
        y = smith.randn(20, 20, requires_grad=True)
        with smith.autograd.graph.saved_tensors_hooks(
    ...
    @unittest.skipIf(not smith.cuda.is_available(), "CUDA is unavailable")
    @smith._funcsmith.config.patch(saved_tensors_hooks_filtering_mode="no_static")
    @smith._funcsmith.config.patch(recompute_views=True)
    def test_saved_tensors_hooks_mutations_raise(self):
        ctx = smith.autograd.graph.saved_tensors_hooks
        device = "cuda"

        class SAF(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                ctx.save_for_backward(x)
                return x
        ...
        with self.assertRaisesRegex(
            ...
            try:
                with ctx(*saved_tensors_hooks_to_gm(mutate, mutate, None, None)):
                    x = inp_fn()
                    y = smith.compile(fn, backend="aot_eager", fullgraph=True)(x)
                    y.sum().backward()
            except smith._dynamo.exc.BackendCompilerFailed as e:
    ...
    def test_mark_activations_dynamic_with_nested(self):
        ...
        x = smith.randn(10, 10, requires_grad=True)
        y = smith.randn(10, 10, requires_grad=True)
        a = smith.randn(2, 3, requires_grad=True, dtype=smith.float64)
        b = smith.randn(3, 3, requires_grad=True, dtype=smith.float64)
        c = smith.randn(4, 3, requires_grad=True, dtype=smith.float64)
...
class TestMod(smith.nn.Module):
    def __init__(self, fn):
        super().__init__()
        self.p = smith.nn.Parameter(smith.ones(2, requires_grad=True))
        self.fn = fn
...
class TestAOTExport(AOTTestCase):
    ...
    def test_aot_export_ban_dropout_mut_pre_dispatch(self):
        def fn(p, x):
            y = smith.ops.aten.dropout.default(x, 0.1, train=False)
            y.add_(1)
            return (y,)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    clone = smith.ops.aten.clone.default(arg1_1);  arg1_1 = None
    add = smith.ops.aten.add.Tensor(clone, 1);  clone = None
    return (add,)""",
        )
...
def forward(self, arg0_1, arg1_1):
    ...
    def test_aot_export_predispatch_func_simple(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    add = smith.ops.aten.add.Tensor(arg1_1, 2)
    _set_grad_enabled = smith._C._set_grad_enabled(False);  _set_grad_enabled = None
    add_1 = smith.ops.aten.add.Tensor(add, 2);  add = None
    ...
    add_2 = smith.ops.aten.add.Tensor(mul, add_1);  mul = add_1 = None
    return (add_2,)""",
        )
...
def forward(self, arg0_1, arg1_1):
    ...
    matmul = smith.ops.aten.matmul.default(arg1_1, arg1_1)
    _set_grad_enabled_1 = smith._C._set_grad_enabled(False);  _set_grad_enabled_1 = None
    add = smith.ops.aten.add.Tensor(matmul, 2);  matmul = None
    sum_1 = smith.ops.aten.sum.default(arg1_1);  arg1_1 = None
    sum_2 = smith.ops.aten.sum.default(add);  add = None
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    return (add_1,)""",
        )
    ...
    def test_aot_export_predispatch_composite_implicit_inplace(self):
        def fn(x, p):
            return (smith.ops.aten.absolute_.default(x.clone()),)
...
def forward(self, arg0_1, arg1_1):
    ...
    abs_1 = smith.ops.aten.abs.default(clone);  clone = None
    return (abs_1,)""",
        )
    ...
    def test_aot_export_predispatch_composite_implicit_linear(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1, arg2_1):
    linear = smith.ops.aten.linear.default(arg2_1, arg0_1, arg1_1);  arg2_1 = arg0_1 = arg1_1 = None
    return (linear,)""",
        )
    ...
    def test_aot_export_predispatch_outdtype(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                y.add_(5)
                return (
                    out_dtype(smith.ops.aten.mm.default, smith.int32, y, self.weight),
                )

        weight = smith.randint(-128, 127, (5, 5), dtype=smith.int8)
        mod = M(weight)
        inp = smith.randint(-128, 127, (5, 5), dtype=smith.int8)
...
def forward(self, arg0_1, arg1_1):
    _set_grad_enabled = smith._C._set_grad_enabled(True);  _set_grad_enabled = None
    mm = smith.ops.aten.mm.default(arg1_1, arg1_1)
    _set_grad_enabled_1 = smith._C._set_grad_enabled(False);  _set_grad_enabled_1 = None
    add = smith.ops.aten.add.Tensor(mm, 2);  mm = None
    sum_1 = smith.ops.aten.sum.default(arg1_1);  arg1_1 = None
    sum_2 = smith.ops.aten.sum.default(add);  add = None
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    return (add_1,)""",
...
def forward(self, arg0_1, arg1_1):
    ...
    add = smith.ops.aten.add.Tensor(matmul, 2);  matmul = None
    sum_1 = smith.ops.aten.sum.default(arg1_1);  arg1_1 = None
    view_1 = smith.ops.aten.view.default(add, [1, 4]);  add = None
    sum_2 = smith.ops.aten.sum.default(view_1);  view_1 = None
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    return (add_1,)""",
        )
    ...
    def test_aot_export_predispatch_buffer_mutation_metadata(self):
        class Foo(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.foo = smith.nn.Buffer(smith.zeros(2, 2))
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    add = smith.ops.aten.add.Tensor(arg0_1, 4);  arg0_1 = None
    sum_1 = smith.ops.aten.sum.default(arg1_1);  arg1_1 = None
    sum_2 = smith.ops.aten.sum.default(add)
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    return (add, add_1)""",
        ...
        eager_mod = Foo()
        output_1, output_2 = gm(smith.zeros(2, 2), inp)
        eager_output = eager_mod(inp)
        self.assertTrue(smith.allclose(output_2, eager_output[0]))
...
def forward(self, arg0_1, arg1_1):
    _set_grad_enabled = smith._C._set_grad_enabled(True);  _set_grad_enabled = None
    add = smith.ops.aten.add.Tensor(arg1_1, 5)
    add_1 = smith.ops.aten.add.Tensor(add, 5);  add = None
    add_2 = smith.ops.aten.add.Tensor(add_1, 7);  add_1 = None
    cos = smith.ops.aten.cos.default(arg1_1);  arg1_1 = None
    sin = smith.ops.aten.sin.default(add_2);  add_2 = None
    add_3 = smith.ops.aten.add.Tensor(cos, sin);  cos = sin = None
    _set_grad_enabled_1 = smith._C._set_grad_enabled(False);  _set_grad_enabled_1 = None
    return (add_3,)""",
        )
    ...
    @unittest.skipIf(IS_WINDOWS, "Windows isn't supported for this case")
    @unittest.skipIf(
        not smithdynamo.is_dynamo_supported(), "SmithDynamo is not supported"
    )
    def test_aot_export_predispatch_with_cond_nested(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                ...
                a = smith.cond(x.sum() > 4, true_fn, false_fn, [x])
                return (a + 3, a + 4)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1):
    sum_1 = smith.ops.aten.sum.default(arg0_1)
    gt = smith.ops.aten.gt.Scalar(sum_1, 4);  sum_1 = None
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (arg0_1,));  gt = true_graph_0 = false_graph_0 = arg0_1 = None
    getitem = cond[0];  cond = None
    add = smith.ops.aten.add.Tensor(getitem, 3)
    add_1 = smith.ops.aten.add.Tensor(getitem, 4);  getitem = None
...
def forward(self, arg0_1):
    sin = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
    add = smith.ops.aten.add.Tensor(sin, 5);  sin = None
    cos = smith.ops.aten.cos.default(add)
    sum_1 = smith.ops.aten.sum.default(cos);  cos = None
    gt = smith.ops.aten.gt.Scalar(sum_1, 5);  sum_1 = None
    cos_1 = smith.ops.aten.cos.default(add);  add = None
    true_graph_0 = self.true_graph_0
    false_graph_0 = self.false_graph_0
    cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (cos_1,));  gt = true_graph_0 = false_graph_0 = cos_1 = None
    getitem = cond[0];  cond = None
...
def forward(self, arg0_1):
    sin = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
    add = smith.ops.aten.add.Tensor(sin, 7);  sin = None
    sin_1 = smith.ops.aten.sin.default(add);  add = None
    return (sin_1,)""",
        )
    ...
    def test_aot_export_predispatch_map_1(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x, y):
                ...
                a = smith.cond(x.sum() > 4, true_fn, false_fn, [x, y])
                return (a + 3, a + 4)

        inps = [smith.randn(2, 2), smith.ones(2)]
...
class <lambda>(smith.nn.Module):
    def forward(
        ...
        gt: "b8[]" = smith.ops.aten.gt.Scalar(sum_1, 4);  sum_1 = None

        true_graph_0 = self.true_graph_0
        false_graph_0 = self.false_graph_0
        cond = smith.ops.higher_order.cond(gt, true_graph_0, false_graph_0, (arg0_1, arg1_1));  gt = true_graph_0 = false_graph_0 = arg0_1 = arg1_1 = None
        getitem: "f32[2, 2]" = cond[0];  cond = None

        add: "f32[2, 2]" = smith.ops.aten.add.Tensor(getitem, 3)
        add_1: "f32[2, 2]" = smith.ops.aten.add.Tensor(getitem, 4);  getitem = None
        return (
    ...
    class true_graph_0(smith.nn.Module):
        def forward(self, arg0_1: "f32[2, 2]", arg1_1: "f32[2]"):
            sin: "f32[2, 2]" = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None

            add: "f32[2, 2]" = smith.ops.aten.add.Tensor(sin, 5);  sin = None

            cos: "f32[2, 2]" = smith.ops.aten.cos.default(add);  add = None

            sum_1: "f32[]" = smith.ops.aten.sum.default(arg1_1);  arg1_1 = None

            add_1: "f32[2, 2]" = smith.ops.aten.add.Tensor(cos, sum_1);  cos = sum_1 = None
            return (add_1,)

    class false_graph_0(smith.nn.Module):
        def forward(self, arg0_1: "f32[2, 2]", arg1_1: "f32[2]"):
            cos: "f32[2, 2]" = smith.ops.aten.cos.default(arg0_1);  arg0_1 = None

            body_graph_0 = self.body_graph_0
            map_impl = smith.ops.higher_order.map_impl(body_graph_0, [cos], [arg1_1]);  body_graph_0 = None
            getitem: "f32[2, 2]" = map_impl[0];  map_impl = None

            sum_1: "f32[]" = smith.ops.aten.sum.default(getitem);  getitem = None

            add: "f32[2, 2]" = smith.ops.aten.add.Tensor(cos, sum_1);  sum_1 = None

            body_graph_1 = self.body_graph_1
            map_impl_1 = smith.ops.higher_order.map_impl(body_graph_1, [cos], [arg1_1]);  body_graph_1 = cos = arg1_1 = None
            getitem_1: "f32[2, 2]" = map_impl_1[0];  map_impl_1 = None

            sum_2: "f32[]" = smith.ops.aten.sum.default(getitem_1);  getitem_1 = None

            add_1: "f32[2, 2]" = smith.ops.aten.add.Tensor(add, sum_2);  add = sum_2 = None
            return (add_1,)

        class body_graph_0(smith.nn.Module):
            def forward(self, arg0_1: "f32[2]", arg1_1: "f32[2]"):
                cos: "f32[2]" = smith.ops.aten.cos.default(arg0_1);  arg0_1 = None

                add: "f32[2]" = smith.ops.aten.add.Tensor(cos, 5);  cos = None

                add_1: "f32[2]" = smith.ops.aten.add.Tensor(add, arg1_1);  add = arg1_1 = None
                return (add_1,)

        class body_graph_1(smith.nn.Module):
            def forward(self, arg0_1: "f32[2]", arg1_1: "f32[2]"):
                cos: "f32[2]" = smith.ops.aten.cos.default(arg0_1);  arg0_1 = None
...
class <lambda>(smith.nn.Module):
    def forward(
        ...
        body_graph_0 = self.body_graph_0
        map_impl = smith.ops.higher_order.map_impl(body_graph_0, [cos], [arg1_1]);  body_graph_0 = arg1_1 = None
        getitem: "f32[2, 2]" = map_impl[0];  map_impl = None

        sum_1: "f32[]" = smith.ops.aten.sum.default(getitem);  getitem = None
        add: "f32[2, 2]" = smith.ops.aten.add.Tensor(cos, sum_1);  cos = sum_1 = None
        return (
            add,  # PlainAOTOutput(idx=0)
        )
    ...
    def test_aot_export_predispatch_with_cond(self):
        class M(smith.nn.Module):
            ...
            def forward(self, x):
                def true_fn(x):
                    y = x.sin()
                    z = smith.ops.aten.linear.default(y, smith.randn(2, 2))
                    z.add_(5)
                    return z.cos()
...
def forward(self, arg0_1):
    sin = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
    randn = smith.ops.aten.randn.default([2, 2], device = device(type='cpu'), pin_memory = False)
    linear = smith.ops.aten.linear.default(sin, randn);  sin = randn = None
    add = smith.ops.aten.add.Tensor(linear, 5);  linear = None
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1, arg5_1, arg6_1, arg7_1):
    conv2d = smith.ops.aten.conv2d.default(arg7_1, arg0_1, arg1_1);  arg7_1 = arg0_1 = arg1_1 = None
    add = smith.ops.aten.add.Tensor(arg6_1, 1);  arg6_1 = None
    _native_batch_norm_legit_functional = smith.ops.aten._native_batch_norm_legit_functional.default(conv2d, arg2_1, arg3_1, arg4_1, arg5_1, True, 0.1, 1e-05);  conv2d = arg2_1 = arg3_1 = arg4_1 = arg5_1 = None
    getitem = _native_batch_norm_legit_functional[0]
    getitem_3 = _native_batch_norm_legit_functional[3]
    getitem_4 = _native_batch_norm_legit_functional[4];  _native_batch_norm_legit_functional = None
    ...
    def test_aot_export_predispatch_reshape(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1):
    view = smith.ops.aten.view.default(arg0_1, [4, 4]);  arg0_1 = None
    sum_1 = smith.ops.aten.sum.default(view);  view = None
    return (sum_1,)""",
        )  # noqa: B950

    def test_aot_export_predispatch_contiguous(self):
        class Cont(smith.nn.Module):
            def forward(self, x):
                y = smith.ops.aten.contiguous.default(x)
                return (y.sum(),)
...
def forward(self, arg0_1):
    ...
    def test_aot_export_module_joint(self):
        class ConvBatchnormRelu(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.conv = smith.nn.Conv2d(1, 3, 1, 1)
                self.bn = smith.nn.BatchNorm2d(3)
            ...
            def forward(self, x):
                x = self.conv(x)
                x = self.bn(x)
                user_out = smith.nn.functional.relu(x)
                loss = user_out.sum()
...
class <lambda>(smith.nn.Module):
    def forward(
        ...
        convolution: "f32[1, 3, 3, 3]" = smith.ops.aten.convolution.default(arg7_1, arg0_1, arg1_1, [1, 1], [0, 0], [1, 1], False, [0, 0], 1);  arg1_1 = None
        add: "i64[]" = smith.ops.aten.add.Tensor(arg6_1, 1);  arg6_1 = None
        _native_batch_norm_legit_functional = smith.ops.aten._native_batch_norm_legit_functional.default(convolution, arg2_1, arg3_1, arg4_1, arg5_1, True, 0.1, 1e-05);  arg3_1 = arg4_1 = arg5_1 = None
        ...
        relu: "f32[1, 3, 3, 3]" = smith.ops.aten.relu.default(getitem);  getitem = None
        detach: "f32[1, 3, 3, 3]" = smith.ops.aten.detach.default(relu);  detach = None
        detach_1: "f32[1, 3, 3, 3]" = smith.ops.aten.detach.default(relu)
        sum_1: "f32[]" = smith.ops.aten.sum.default(relu)
        detach_2: "f32[1, 3, 3, 3]" = smith.ops.aten.detach.default(relu);  relu = None
        ones_like: "f32[]" = smith.ops.aten.ones_like.default(sum_1, pin_memory = False, memory_format = smith.preserve_format)
        expand: "f32[1, 3, 3, 3]" = smith.ops.aten.expand.default(ones_like, [1, 3, 3, 3]);  ones_like = None
        detach_3: "f32[1, 3, 3, 3]" = smith.ops.aten.detach.default(detach_1);  detach_1 = None
        threshold_backward: "f32[1, 3, 3, 3]" = smith.ops.aten.threshold_backward.default(expand, detach_3, 0);  expand = detach_3 = None
        native_batch_norm_backward = smith.ops.aten.native_batch_norm_backward.default(threshold_backward, convolution, arg2_1, getitem_3, getitem_4, getitem_1, getitem_2, True, 1e-05, [True, True, True]);  threshold_backward = convolution = arg2_1 = getitem_1 = getitem_2 = None
        ...
        convolution_backward = smith.ops.aten.convolution_backward.default(getitem_5, arg7_1, arg0_1, [3], [1, 1], [0, 0], [1, 1], False, [0, 0], 1, [False, True, True]);  getitem_5 = arg7_1 = arg0_1 = None
...
class <lambda>(smith.nn.Module):
    def forward(
        ...
        # No stacktrace found for following nodes
        convolution: "f32[1, 3, 3, 3]" = smith.ops.aten.convolution.default(arg7_1, arg0_1, arg1_1, [1, 1], [0, 0], [1, 1], False, [0, 0], 1);  arg7_1 = arg0_1 = arg1_1 = None
        add: "i64[]" = smith.ops.aten.add.Tensor(arg6_1, 1);  arg6_1 = None
        _native_batch_norm_legit_functional = smith.ops.aten._native_batch_norm_legit_functional.default(convolution, arg2_1, arg3_1, arg4_1, arg5_1, True, 0.1, 1e-05);  convolution = arg2_1 = arg3_1 = arg4_1 = arg5_1 = None
        getitem: "f32[1, 3, 3, 3]" = _native_batch_norm_legit_functional[0]
        getitem_3: "f32[3]" = _native_batch_norm_legit_functional[3]
        getitem_4: "f32[3]" = _native_batch_norm_legit_functional[4];  _native_batch_norm_legit_functional = None
        relu: "f32[1, 3, 3, 3]" = smith.ops.aten.relu.default(getitem);  getitem = None
        sum_1: "f32[]" = smith.ops.aten.sum.default(relu)
        detach: "f32[1, 3, 3, 3]" = smith.ops.aten.detach.default(relu);  relu = None
        return (
...
""",  # noqa: B950
    ...
    def test_aot_export_simplified_basic(self):
        ...
        grad_outs = [smith.ones_like(x) for x in out_ref2]
        grads_ref = smith.autograd.grad(out_ref2, [x2, y2], grad_outputs=grad_outs)
    ...
    def test_aot_export_forward_mutation_no_buffer_mut(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.buffer1 = smith.nn.Buffer(smith.ones(6, 4))
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1, arg1_1):
    add = smith.ops.aten.add.Tensor(arg1_1, 4);  arg1_1 = None
    cos = smith.ops.aten.cos.default(add)
    sum_1 = smith.ops.aten.sum.default(cos);  cos = None
    sum_2 = smith.ops.aten.sum.default(arg0_1);  arg0_1 = None
    add_1 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    return (add, add_1)""",
        )  # noqa: B950
    ...
    def test_aot_export_forward_mutation_multiple_mut(self):
        ...
        mod = M()
        inp = [smith.ones(6, 4), smith.zeros(6, 4)]
        gm, sig = aot_export_module(mod, inp, trace_joint=False)
        self.assertExpectedInline(
            str(gm.code).strip(),
            """\
def forward(self, arg0_1, arg1_1, arg2_1):
    add = smith.ops.aten.add.Tensor(arg2_1, 4);  arg2_1 = None
    add_1 = smith.ops.aten.add.Tensor(arg0_1, 5);  arg0_1 = None
    cos = smith.ops.aten.cos.default(arg1_1);  arg1_1 = None
    sum_1 = smith.ops.aten.sum.default(cos);  cos = None
    sin = smith.ops.aten.sin.default(add)
    sum_2 = smith.ops.aten.sum.default(sin);  sin = None
    add_2 = smith.ops.aten.add.Tensor(sum_1, sum_2);  sum_1 = sum_2 = None
    sum_3 = smith.ops.aten.sum.default(add_1)
    ...
    def test_aot_export_input_mutation_on_input_requiring_grad_banned(self):
        ...
        mod = M()
        inp = smith.randn(2, requires_grad=True)
...
graph():
    ...
    %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%arg0_1, 4), kwargs = {})
    return (add, add)""",
        )
...
graph():
    ...
    %mul : [num_users=2] = call_function[target=smith.ops.aten.mul.Tensor](args = (%arg0_1, 2), kwargs = {})
    %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%mul, %arg1_1), kwargs = {})
    return (mul, add)""",
        )
    ...
    @unittest.skipIf(IS_WINDOWS, "Windows isn't supported for this case")
    @unittest.skipIf(
        not smith._dynamo.is_dynamo_supported(), "Cond needs dynamo to run"
    )
    def test_aot_export_with_smith_cond(self):
        class M(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
...
def forward(self, arg0_1):
    ...
    return (add, add_1)""",  # noqa: B950
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, arg0_1):
    cos = smith.ops.aten.cos.default(arg0_1);  arg0_1 = None
    return (cos,)""",
        )
...
def forward(self, arg0_1):
    ...
    def test_aot_export_functionalized_rng_banned(self):
        ...
        inp = smith.randn(2)
        with (
            patch("funcsmith.compile.config.functionalize_rng_ops", True),
    ...
    def test_aot_export_unbacked_arg(self):
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self):
    full = smith.ops.aten.full.default([], 11, device = device(type='cpu'), pin_memory = False)
    _local_scalar_dense = smith.ops.aten._local_scalar_dense.default(full);  full = None
    full_1 = smith.ops.aten.full.default([_local_scalar_dense], 0, device = device(type='cpu'), pin_memory = False);  _local_scalar_dense = None
    ...
    def test_aot_export_input_mutation(self):
        ...
        x = smith.randn(2, requires_grad=True)
        buf = smith.zeros(2, requires_grad=False)
...
def forward(self, primals, tangents):
    ...
    add = smith.ops.aten.add.Tensor(primals_2, 1)
    mul = smith.ops.aten.mul.Tensor(add, primals_1);  primals_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, add);  tangents_1 = None
    copy_ = smith.ops.aten.copy_.default(primals_2, add);  primals_2 = add = copy_ = None
...
class TestPartitioning(AOTTestCase):
    ...
    def test_recompute_partitioning(self):
        def fn(a, b):
            return smith.sin(smith.sin(a)) + b

        # Reference calculation
        ref_a = smith.rand(10, 10, requires_grad=True)
        ref_b = smith.rand(10, 10, requires_grad=True)
        ref = fn(ref_a, ref_b)
        ref.sum().backward()
        ...
        assert smith.allclose(ref_a.grad, res_a.grad, atol=1e-3, rtol=1e-3)
        assert smith.allclose(ref_b.grad, res_b.grad, atol=1e-3, rtol=1e-3)
    ...
    def test_meta_tensor_inplace_op(self):
        ...
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.weight = smith.nn.Parameter(
                    smith.randn(3072, 768, requires_grad=True)
                )
                self.bias = smith.nn.Parameter(smith.randn(3072, requires_grad=True))

            def forward(self, add_4):
                linear_4 = smith.nn.functional.linear(
                    add_4, self.weight, bias=self.bias
                )
                gelu = smith.nn.functional.gelu(linear_4)
                return gelu
        ...
        inp0 = smith.randn(16, 128, 768, requires_grad=True)
        inputs = [
            inp0,
        ]
    ...
    def test_default_partitioner_getitem(self):
        ...
        fw_graph, bw_graph = get_fw_bw_graph(
            f,
            [smith.randn(3, 10, requires_grad=True), mod.weight, mod.bias],
    ...
    def test_min_cut_partitioner_raise_getitems(self):
        def f(x):
            y = smith.split(x, x.size(0) // 2, dim=0)
            a = y[0].sin()
            b = y[1].cos()
            return a + b

        _, bw_graph = get_fw_bw_graph(f, [smith.randn(4, 4, requires_grad=True)])

        self.assertExpectedInline(
            bw_graph.code.strip(),
            """\
def forward(self, primals_1, tangents_1):
    split = smith.ops.aten.split.Tensor(primals_1, 2);  primals_1 = None
    getitem_1 = split[1]
    getitem = split[0];  split = None
    sin_1 = smith.ops.aten.sin.default(getitem_1);  getitem_1 = None
    neg = smith.ops.aten.neg.default(sin_1);  sin_1 = None
    mul = smith.ops.aten.mul.Tensor(tangents_1, neg);  neg = None
    cos_1 = smith.ops.aten.cos.default(getitem);  getitem = None
    mul_1 = smith.ops.aten.mul.Tensor(tangents_1, cos_1);  tangents_1 = cos_1 = None
    cat = smith.ops.aten.cat.default([mul_1, mul]);  mul_1 = mul = None
    return (cat,)""",
        )
    ...
    def test_custom_partitioner_fn(self):
        ...
        inp = [smith.randn((4, 4), requires_grad=True)]
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1):
    cos = smith.ops.aten.cos.default(primals_1)
    cos_1 = smith.ops.aten.cos.default(cos);  cos = None
...
def forward(self, primals_1, tangents_1):
    cos = smith.ops.aten.cos.default(primals_1)
    sin = smith.ops.aten.sin.default(cos);  cos = None
    neg = smith.ops.aten.neg.default(sin);  sin = None
    mul = smith.ops.aten.mul.Tensor(tangents_1, neg);  tangents_1 = neg = None
    sin_1 = smith.ops.aten.sin.default(primals_1);  primals_1 = None
    neg_1 = smith.ops.aten.neg.default(sin_1);  sin_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(mul, neg_1);  mul = neg_1 = None
    return (mul_1,)""",
        )
    ...
    def test_min_cut_partitioner_save_shape(self):
        ...
        inp = [smith.ones([10, 10], requires_grad=True)]
        ...
        inp = [
            ...
            smith.randn((3, 10), requires_grad=True),
            smith.randn((2, 10), requires_grad=True),
        ]
        ...
        def f(a, b, c):
            ...
            sb = smith.ops.aten.sym_size(b)
            sc = c.size()
            x = sb[0] + sc[0]
            a_sz = (x, a.size(0))
            return smith.cat([a.expand(a_sz), b, c])
    ...
    def test_default_partitioner_output_tensor_shape_tensor(self):
        inp = [
            ...
            smith.randn((3, 10), requires_grad=True),
            smith.randn((2, 10), requires_grad=True),
            smith.randn((10, 1), requires_grad=True),
        ]
        ...
        def f(a, b, c, d):
            ...
            x = sb[0] + sc[0]
            a_sz = (x, a.size(0))
            cat = smith.cat([a.expand(a_sz), b, c])
            mm = smith.mm(cat, d)
            mm2 = smith.mm(
        ...
        self.assertTrue(isinstance(real_outs[1], smith.Size))

        # TODO(whc) we should learn to return smith.Sizes
        self.assertFalse(isinstance(compiled_outs[1], smith.Size))
    ...
    def test_min_cut_partitioner(self):
        ...
        fw_graph, bw_graph = get_fw_bw_graph(f, [smith.randn(3, requires_grad=True)])
        ...
        fw_graph, bw_graph = get_fw_bw_graph(
            f, [smith.randn(3, requires_grad=True) for _ in range(4)]
        )
    ...
    def test_contiguous(self):
        ...
        # https://discuss.blacksmith.org/t/error-on-transpose-and-view/434
        def f(x):
            return x.view(2, 3).t()

        inp = smith.randn(6, requires_grad=True)
        out = aot_function(f, nop)(inp)
        smith.autograd.grad(out, inp, smith.randn(3, 2))

    def test_preserve_random(self):
        def fn(x):
            return smith.nn.functional.dropout(x, 0.5) + x
        ...
        assert smith.allclose(ref, res)

    # https://github.com/blacksmith/blacksmith/issues/110666
    ...
    def test_autocast(self):
        mod = smithvision.models.resnet18().cuda()
        mod.train()

        x = smith.randn(16, 3, 32, 32, device="cuda")
        aot_mod = memory_efficient_fusion(mod)

        # Ensure that AOT Autograd works with AMP
        with smith.cuda.amp.autocast(True):
            res = aot_mod(x)
        res.sum().backward()
    ...
    def test_quantize_activation_duplicate_nodes(self):
        ...
        from smith._subclasses.fake_tensor import extract_tensor_metadata

        # Mock the inductor config
        with patch.dict(
            "smith._inductor.config.post_grad_fusion_options",
            {
                "activation_quantization_aten_pass": {
                    "allowed_dtypes": "smith.bfloat16",
                    "size_in_mb": 1,
                    "use_scaling": True,
                    "exclude_primals": False,
                    "skip_dynamo_guards": True,
                    "quantize_dynamic_shape": False,
                    "quant_type": "smith.float16",  # float8_e5m2 must be GPU
                }
            },
        ):
            ...
            x.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            x.meta["tensor_meta"] = extract_tensor_metadata(x.meta["val"])

            y = fwd_graph.placeholder("y")
            y.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            y.meta["tensor_meta"] = extract_tensor_metadata(y.meta["val"])

            # Create a computation node that will be duplicated in outputs
            mul_node = fwd_graph.call_function(smith.ops.aten.mul.Tensor, (x, y))
            mul_node.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            mul_node.meta["tensor_meta"] = extract_tensor_metadata(mul_node.meta["val"])
            mul_node.meta["saved_for_quantization"] = True

            # Create another node
            add_node = fwd_graph.call_function(smith.ops.aten.add.Tensor, (x, y))
            add_node.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            ...
            scale_input2.meta["val"] = smith.randn(100, 100, dtype=smith.float32)
            ...
            grad_output1.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            grad_output1.meta["tensor_meta"] = extract_tensor_metadata(
                grad_output1.meta["val"]
            )

            grad_output2 = bwd_graph.placeholder("tangents_2")
            grad_output2.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            ...
            mul_bwd1 = bwd_graph.call_function(
                smith.ops.aten.mul.Tensor, (quant_input1, grad_output1)
            )
            mul_bwd1.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
            mul_bwd1.meta["tensor_meta"] = extract_tensor_metadata(mul_bwd1.meta["val"])

            mul_bwd2 = bwd_graph.call_function(
                smith.ops.aten.mul.Tensor, (quant_input2, grad_output2)
            )
            mul_bwd2.meta["val"] = smith.randn(100, 100, dtype=smith.bfloat16)
    ...
    def test_min_cut_partitioner_unbounded_error_message(self):
        ...
        from smith._funcsmith.partitioners import _find_infinite_capacity_path
    ...
    def test_min_cut_partitioner_getitem_of_banned_multi_output(self):
        ...
        class GetitemAsGradOutput(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                var, mean = smith.var_mean(smith.stack([x, x]), dim=0)
        ...
        context_fn = partial(
            create_selective_checkpoint_contexts,
            [smith.ops.aten.var_mean.correction],
        )
    ...
    def test_force_save_effectful_ops(self):
        ...
        from smith._higher_order_ops.effects import _register_effectful_op, with_effects
        from smith._library.effects import EffectType

        @smith.library.custom_op("test::effectful_op", mutates_args=())
        def effectful_op(x: smith.Tensor) -> smith.Tensor:
            return x * 2
        ...
        try:
            ...
            @smith.compile(backend=graph_capture_backend)
            def fn(x, weight):
                a = smith.ops.test.effectful_op(x)
                return a.sum() * weight

            x = smith.randn(4, 4)
            weight = smith.randn(4, 4)
            fn(x, weight)
    ...
    def test_force_save_effectful_ops_nested_tuple(self):
        ...
        @smith.library.custom_op("test::effectful_tuple_op", mutates_args=())
        def effectful_tuple_op(x: smith.Tensor) -> tuple[smith.Tensor, smith.Tensor]:
            return x * 2, x + 1

        @effectful_tuple_op.register_fake
        def _(x: smith.Tensor) -> tuple[smith.Tensor, smith.Tensor]:
        ...
        try:
            ...
            @smith.compile(backend=graph_capture_backend)
            def fn(x):
                a, b = smith.ops.test.effectful_tuple_op(x)
                return a.sum() + b.sum()
            ...
            for node in getitem_nodes:
                ...
                if isinstance(val, smith.Tensor):
...
class TestAOTDispatch(AOTTestCase):
    ...
    def test_aot_dispatch_simple(self):
        ...
        a1_ref = smith.ones(3, 3, requires_grad=True)
        a2_ref = smith.ones(3, 3, requires_grad=True)
        a_ref = TwoTensor(a1_ref, a2_ref)
        b_ref = smith.ones(3, 3, requires_grad=True)
        ...
        self.assertExpectedInline(
            ...
            """\
def forward(self, primals_1, primals_2, primals_3):
    mul = smith.ops.aten.mul.Tensor(primals_1, 6);  primals_1 = None
    mul_1 = smith.ops.aten.mul.Tensor(primals_2, 6);  primals_2 = None
    div = smith.ops.aten.div.Tensor(primals_3, 2);  primals_3 = None
    add = smith.ops.aten.add.Tensor(mul, div);  mul = None
    add_1 = smith.ops.aten.add.Tensor(mul_1, div);  mul_1 = div = None
    return (add, add_1)""",
        )
...
def forward(self, tangents_1, tangents_2):
    div_1 = smith.ops.aten.div.Tensor(tangents_1, 2)
    div_2 = smith.ops.aten.div.Tensor(tangents_2, 2)
    mul_2 = smith.ops.aten.mul.Tensor(tangents_1, 6);  tangents_1 = None
    mul_3 = smith.ops.aten.mul.Tensor(tangents_2, 6);  tangents_2 = None
    ...
    def test_aot_dispatch_inference(self):
        ...
        a1_ref = smith.ones(3, 3)
        a2_ref = smith.ones(3, 3)
        a_ref = TwoTensor(a1_ref, a2_ref)
        b_ref = smith.ones(3, 3)
    ...
    def test_aot_dispatch_incorrect_backward(self):
        ...
        def f(a, b):
            aa = smith.mul(a, 2)
            bb = smith.add(b, 3)
            out_subclass = smith.div(aa, bb)
            out_reg = smith.add(b, b)
        ...
        # We compiled our graph assuming type(grad_out[1]) is smith.Tensor,
...
""",  # noqa: F541
    ...
    def test_aot_dispatch_output_alias(self):
        ...
        b1_ref = smith.ones(3, 3, requires_grad=True)
        b2_ref = smith.ones(3, 3, requires_grad=True)
        b_ref = TwoTensor(b1_ref, b2_ref)
        a_ref = smith.ones(3, 3, requires_grad=True)
    ...
    def test_aot_dispatch_input_mutation(self):
        ...
        a_ref_base = smith.ones(3, 3, requires_grad=True)
        b_ref = b_ref_base + 1
        a_ref = a_ref_base + 1
    ...
    def test_aot_dispatch_input_metadata_mutation(self):
        ...
        b1_ref = smith.arange(9, requires_grad=True, dtype=smith.float32).reshape(3, 3)
        b2_ref = smith.arange(9, requires_grad=True, dtype=smith.float32).reshape(3, 3)
        b_ref_base = TwoTensor(b1_ref, b2_ref)
        a_ref_base = (
            smith.arange(9, dtype=smith.float32)
            .reshape(3, 3)
            .detach()
            .requires_grad_(True)
        )
    ...
    def test_aot_dispatch_output_requires_grad_in_no_grad(self):
        ...
        inp_fns = [
            lambda: smith.ones(10, requires_grad=True),
            lambda: smith.ones(10, requires_grad=False),
        ]
        ...
        for inp_fn in inp_fns:
            ...
            if ref_x.requires_grad:
                with smith.enable_grad():
                    ...
                    self.assertEqual(ref_x.grad, x.grad)
                    assert smith.allclose(ref_x.grad, x.grad, atol=1e-3, rtol=1e-3)
    ...
    def test_aot_dispatch_output_requires_grad_in_no_grad_views(self):
        ...
        inp_x0 = smith.ones(2, 3, requires_grad=True)
...
class GradsNoForceContiguousContextManager(ContextDecorator):
    def __enter__(self):
        # flake8: noqa: TOR901
        self.lib = smith.library.Library("_test_aotdispatch_lib", "FRAGMENT")
        self.d = {
            smith.channels_last: 0,
            smith.contiguous_format: 0,
        }
        ...
        def log_tangents_memory_format_log_impl(x):
            self.d[smith._prims_common.suggest_memory_format(x)] += 1
        ...
        def log_tangents_memory_format_bwd(ctx, grad):
            smith.ops._test_aotdispatch_lib.log_tangents_memory_format_log(grad)
            return grad.clone()
        ...
        from smith._higher_order_ops.effects import _EffectType, _register_effectful_op

        _register_effectful_op(
            smith.ops._test_aotdispatch_lib.log_tangents_memory_format.default,
            _EffectType.ORDERED,
        )
        _register_effectful_op(
            smith.ops._test_aotdispatch_lib.log_tangents_memory_format_log.default,
            _EffectType.ORDERED,
        )
...
class TestAOTModuleSimplified(AOTTestCase):
    def test_aot_module_simplified(self):
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(20, 30)
        ...
        x = smith.randn(128, 20, requires_grad=True)
        y = smith.randn(128, 30, requires_grad=True)
        inputs = [x, y]
        ...
        assert smith.allclose(ref[0], res[0])
        assert smith.allclose(inputs[0].grad, cloned_inputs[0].grad)
        assert smith.allclose(inputs[1].grad, cloned_inputs[1].grad)
 ...
 - Eq(s70, 30)""",
    ...
    # https://github.com/blacksmith/blacksmith/issues/105327
    def test_lift_fresh_copy_in_graph(self):
        class MyMod(smith.nn.Module):
            def forward(self, x):
                _tensor_constant0 = smith.tensor([1])
    ...
    def test_inference_python_dispatcher(self):
        ...
        class MockModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.upsample = smith.nn.Upsample(
        ...
        shape_env = ShapeEnv()
        fake_mode = FakeTensorMode(shape_env=shape_env)
        x = smith.randn(2, 512, 40, 59)  # NB: must not require grad
        inputs = [x]
        fake_inputs = [fake_mode.from_tensor(x) for x in inputs]
    ...
    def test_aot_module_simplified_preserves_stack_trace(self):
        ...
        tracer = smith.fx.Tracer()
        tracer.record_stack_traces = True
        graph = tracer.trace(MockModule())
        mod = smith.fx.GraphModule(tracer.root, graph)
        ...
        def assert_compiler(gm: smith.fx.GraphModule, _):
            for node in gm.graph.nodes:
    ...
    def test_aot_module_simplified_preserves_stack_trace_from_mutation(self):
        ...
        def assert_compiler(gm: smith.fx.GraphModule, _):
            assert smith.ops.aten.copy_.default in [x.target for x in gm.graph.nodes]
            for node in gm.graph.nodes:
                if node.target == smith.ops.aten.copy_.default:
                    assert "stack_trace" in node.meta
                    assert "x_view.mul_(2)" in node.meta["stack_trace"]
            return gm.forward  # return a python callable

        x = smith.randn(128, 20)
        inputs = [x]
    ...
    def test_aot_module_simplified_fake_tensor_gm_raises(self):
        ...
        real_x = smith.randn(4, requires_grad=True)
        fake_x = fake_mode.from_tensor(real_x)
        real_z = smith.randn(4)
    ...
    def test_aot_test_subclasses_with_tensor_factories(self):
        from smith.testing._internal.common_subclass import SubclassWithTensorFactory

        inp = SubclassWithTensorFactory(smith.zeros(3, 5))
    ...
    # https://github.com/blacksmith/blacksmith/issues/134644
        ...
        with GradsNoForceContiguousContextManager() as ctx:
            ...
            class M(smith.nn.Module):
                ...
                def forward(self, x, y, cont_inp):
                    ...
                    y.mul_(2)
                    r = self.conv(x)
                    r = smith.ops._test_aotdispatch_lib.log_tangents_memory_format(r)
                    return (
                        r,
                        r.transpose(0, 1),
                        z.view(-1),
            ...
            def dense_inps():
                return (
                    smith.randn(2, 3, 5, 5, requires_grad=True).to(
                        memory_format=smith.channels_last
                    ),
                    smith.randn(3, 2, 1, 1, requires_grad=True).to(
                        memory_format=smith.channels_last
                    ),
                    smith.randn(3, 2, 1, 1, requires_grad=True),
                )
            ...
            ctx.reset_counters()
            inps = dense_inps()
            outs = smith.compile(m, backend="inductor", fullgraph=True)(*inps)
            outs[0].sum().backward()

            self.assertEqual(ctx.d[smith.channels_last], 1)
            self.assertEqual(ctx.d[smith.contiguous_format], 0)
        ...
        with GradsNoForceContiguousContextManager() as ctx:
            ...
            def inps_fn():
                return (
                    ...
                    smith.randn(3, 2, requires_grad=True).clone(),
                )
            ...
            ctx.reset_counters()
            mc = M()
            mc.to(memory_format=smith.channels_last)
            mc.train()
            outs = smith.compile(mc, backend="aot_eager", fullgraph=True)(*inps_fn())
            outs[0].sum().backward()

            self.assertEqual(ctx.d[smith.channels_last], 2)
            self.assertEqual(ctx.d[smith.contiguous_format], 0)
    ...
    def test_grads_no_force_contiguous_nested_subclass(self):
        with GradsNoForceContiguousContextManager() as ctx:
            ...
            x = smith.randn(2, 3, 5, 5, requires_grad=True).to(
                memory_format=smith.channels_last
            )
            ref_inps = inps_fn(x)
            ref_outs = m(*ref_inps)
            ...
            inps = inps_fn(x)
            outs = smith.compile(mc, backend="aot_eager", fullgraph=True)(*inps)
            outs[0].sum().backward()
            self.assertEqual(ctx.d[smith.channels_last], 4)
            self.assertEqual(ctx.d[smith.contiguous_format], 0)
    ...
    def test_grads_no_force_contiguous_nested_tensor_tangent(self):
        ...
        out = smith.compile(fn, backend="aot_eager", fullgraph=True)(nt)
        out_buffer = out.values()
        ga, gb, gc = smith.autograd.grad(out_buffer.sum(), (a, b, c))
...
""",  # noqa: F541
            ...
            y2.backward(gradient=smith.randn(2, 3))

    def test_tangent_type_coercion(self):
        def fn(x):
            return x.clone()

        ref_y = fn(WrapperSubclass(smith.randn(2, 3, requires_grad=True)))
        ref_y.sum().backward()
        ...
        y2.backward(gradient=WrapperSubclass(smith.randn(2, 3)))

        y3 = smith.compile(fn, fullgraph=True)(smith.randn(2, 3, requires_grad=True))
        # Test coercion WrapperSubclass -> Tensor
        y3.backward(gradient=WrapperSubclass(smith.randn(2, 3)))
    ...
    def test_inductor_freezing_with_subclasses(self):
        class M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.w = TwoTensor(smith.randn(3, 4), smith.randn(3, 4))
                self.wt = smith.randn(3, 4)

            def forward(self, x):
                return (
                    x.index_select(
                        dim=0, index=smith.tensor([0, 2, 1], dtype=smith.int64)
                    )
                    + self.w
                    + self.wt
                )
        ...
        inp = smith.randn(3, 4)
        with smith.no_grad():
            smith.compile(m, fullgraph=True)(inp)

    def test_rrelu(self):
        def fn(x):
            return smith.rrelu(x, training=True)

        def fn_(x):
            smith.rrelu_(x, training=True)
            return x

        x = smith.randn(4, 4)
        smith.compile(fn, backend="inductor", fullgraph=True)(x)
        smith.compile(fn_, backend="inductor", fullgraph=True)(x)
    ...
    def test_layer_norm(self):
        ...
        x = smith.randn(2, 4, 8)
        eager = fn(x)
        aot_eager = smith.compile(backend="aot_eager")(fn)(x)
    ...
    def test_rms_norm(self):
        ...
        x = smith.randn(2, 4, 8, device="cuda")
        eager = fn(x)
        aot_eager = smith.compile(backend="aot_eager")(fn)(x)
        self.assertEqual(eager, aot_eager, atol=0, rtol=0)

    def test_subclass_parameters(self):
        class _M(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.p = smith.nn.Parameter(
                    TwoTensor(
                        TwoTensor(smith.zeros(3, 4), smith.randn(3, 4)),
                        smith.ones(3, 4),
                    )
                )
        ...
        class M(smith.nn.Module):
            def __init__(self):
                ...
                self.p2 = smith.nn.Parameter(
                    TwoTensor(
                        smith.ones(3, 4),
                        TwoTensor(smith.randn(3, 4), smith.randn(3, 4)),
                    )
                )
                self._m = _M()
        ...
        from smith._funcsmith._aot_autograd.subclass_parametrization import (
        ...
        comp_fn = smith.compile(m, backend="aot_eager", fullgraph=True)
        out = comp_fn(x)
        self.assertEqual(ref_out, out)
        out.sum().backward()
        self.assertEqual(ref_x.grad, x.grad)
            ...
            def __init__(self):
                ...
                self.p2 = smith.nn.Parameter(
                    TwoTensor(
                        ...
                        TwoTensor(
                            TwoTensor(smith.randn(3, 4), smith.randn(3, 4)),
                            TwoTensor(smith.ones(3, 4), smith.randn(3, 4)),
                        ),
                    )
                )
    ...
    def test_rrelu_with_noise_mutation(self):
        def fn_functional(x):
            noise = smith.ones_like(x)
            result, noise_out = smith.ops.aten.rrelu_with_noise_functional(
        ...
        def fn_mutation(x):
            noise = smith.ones_like(x)
            result = smith.ops.aten.rrelu_with_noise(x, noise, 0.2, 0.8, True)
            return result, noise

        def fn_inplace(x):
            noise = smith.ones_like(x, requires_grad=False)
            smith.ops.aten.rrelu_with_noise_(x, noise, 0.2, 0.8, True)
            return x, noise

        def _test_fn(fn, check_backward=True):
            x = -smith.abs(smith.randn(4, 4, dtype=smith.bfloat16, requires_grad=True))

            ref_y, ref_noise = fn(x)
            self.assertTrue(smith.all(ref_noise < smith.ones_like(ref_noise)).item())

            comp_y, comp_noise = smith.compile(fn, backend="inductor", fullgraph=True)(
                x
            )

            if check_backward:
                comp_y.sum().backward()
            self.assertTrue(smith.all(comp_noise < smith.ones_like(comp_noise)).item())
    ...
    @patch("smith._funcsmith.config.guess_tangent_strides_as_outputs", True)
    def test_noncontig_nonmemformat_tangents(
        ...
        def _inp_dense():
            t = smith.randn(B, T, E, device=device, requires_grad=True)
            if dynamic_shapes:
                for i in range(t.ndim):
                    smith._dynamo.mark_dynamic(t, i)
            return t
        ...
        comp_fn = smith.compile(fn, backend="aot_eager", fullgraph=True)
        ...
        TEST_CASES = [
            (_inp, lambda y: smith.ones(y.shape, dtype=y.dtype, device=y.device)),
            # Memory overlap, dense tangent
            (
                _inp,
                lambda y: smith.tensor([1], dtype=y.dtype, device=y.device).as_strided(
                    y.shape, (0,) * y.ndim
                ),
            ),
    ...
    def test_flex_attn_noncontiguous_tangents(self):
        with GradsNoForceContiguousContextManager() as ctx:
            ...
            def attn_fn(q, k, v):
                ...
                y = smith.ops._test_aotdispatch_lib.log_tangents_memory_format(y)
                return y

            class M(smith.nn.Module):
                def __init__(self):
                    super().__init__()
                    self.c_attn = smith.nn.Linear(E, 3 * E)
            ...
            def _inp():
                return smith.randn(B, T, E, requires_grad=True, device="cuda")

            x = _inp()
            y = m(x)
            y.backward(smith.ones_like(y).contiguous())
    ...
    def _test_pack_hooks(
        ...
        with ExitStack() as stack:
            ...
            x = pytree.tree_map_only(smith.Tensor, _f, ref_x)
        ...
        with ExitStack() as stack:
            ...
            ref_x_grad = pytree.tree_map_only(smith.Tensor, lambda t: t.grad, ref_x)
            x_grad = pytree.tree_map_only(smith.Tensor, lambda t: t.grad, x)
    ...
    def test_saved_tensors_hooks_base(self, saved_tensors_hooks_filtering_mode):
        with patch(
            "smith._funcsmith.config.saved_tensors_hooks_filtering_mode",
            ...
            class AF(smith.autograd.Function):
                @staticmethod
                def forward(ctx, x):
                    ctx.save_for_backward(x)
                    ctx.d1 = x.size(1)
            ...
            def inp_fn():
                x = smith.ones(2, 2, device=device, requires_grad=True)
                smith._dynamo.mark_dynamic(x, 0)
                smith._dynamo.mark_dynamic(x, 1)
                y = smith.zeros(2, 2, device=device, dtype=smith.int64)
                return x, y
            ...
            def pack_bf16(x):
                return x.dtype, x.to(dtype=smith.bfloat16)
    ...
    def test_saved_tensors_hooks_params(self):
        lib = smith.library.Library("_test_aotdispatch_lib", "FRAGMENT")
        logged_shapes = []
        logged_dtypes = []
        ...
        def pack_fp8_with_scale_and_log(x):
            smith.ops._test_aotdispatch_lib.log(x)
            return _pack_fp8_with_scale_wrap(x)
        ...
        def m_inp_fn():
            x = smith.ones(
                2, 2, 2, device=device, dtype=smith.float64, requires_grad=True
            )
            smith._dynamo.mark_dynamic(x, 0)
            smith._dynamo.mark_dynamic(x, 1)
            return (x,)

        class SAF0(smith.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                ctx.save_for_backward(x)
                return x
        ...
        with patch(
            "smith._funcsmith.config.saved_tensors_hooks_filtering_mode", "donated"
        ):
            _reset_logged()
            _test_m()
            ...
            # input excluded
            self.assertFalse(smith.float64 in logged_dtypes)

        with patch(
            "smith._funcsmith.config.saved_tensors_hooks_filtering_mode", "no_static"
        ):
            _reset_logged()
            _test_m()
            # Check that hooks were not applied to Parameters
            ...
            self.assertFalse([2, 2] in logged_shapes)
            self.assertTrue([2, 2, 2] in logged_shapes)
            self.assertTrue(smith.float64 in logged_dtypes)

        with patch("smith._funcsmith.config.saved_tensors_hooks_filtering_mode", "all"):
            _reset_logged()
            _test_m()
    ...
    @unittest.skipIf(not smith.cuda.is_available(), "CUDA is unavailable")
    @unittest.skipIf(not SM80OrLater, "bfloat16, float8")
    @smith._funcsmith.config.patch(saved_tensors_hooks_filtering_mode="all")
    def test_saved_tensors_hooks_recompile(self):
        ctx = smith.autograd.graph.saved_tensors_hooks

        def pack_bf16(x):
            return x.to(dtype=smith.bfloat16)

        def unpack_bf16(x):
            return x.to(dtype=smith.float)
        ...
        def _test(hooks, inline, expected_compile_count):
            ...
            cnt = CompileCounter()
            x = inp_fn()
            y = smith.compile(fn, backend=cnt, fullgraph=True)(x)
            y.sum().backward()
    ...
    def test_saved_tensors_hooks_donated_buffers(self):
        ...
        logger_name = "smith._funcsmith._aot_autograd.graph_compile"
        ...
        def fn(x):
            x0 = x
            x = SAF.apply(x)
            return x0, smith.nn.functional.relu(x)

        inp = smith.rand([3, 3], requires_grad=True)
        # 1. No donated buffers without hooks, as relu saves input which is also user output.
        with self.assertLogs(logger_name, level="INFO") as captured:
            out = smith.compile(fn, backend="aot_eager", fullgraph=True, dynamic=False)(
                inp
            )
            out[1].sum().backward()
        ...
        inp = smith.rand([3, 3], requires_grad=True)
        with smith.autograd.graph.saved_tensors_hooks(pack_gm, unpack_gm):
            with self.assertLogs(logger_name, level="INFO") as captured:
                out = smith.compile(
...
aot_autograd_failures = {
    ...
    decorate(
        "svd_lowrank",
        decorator=toleranceOverride({smith.float32: tol(atol=1e-04, rtol=1e-05)}),
    ),
    decorate(
    ...
    decorate(
        ...
        decorator=toleranceOverride({smith.float32: tol(atol=1e-02, rtol=1e-02)}),
    ),
    decorate(
    ...
    aot_autograd_failures.update(
        {
            decorate(
                ...
                decorator=toleranceOverride(
                    {smith.float32: tol(atol=6e-05, rtol=4e-06)}
                ),
            ),
            decorate(
                "__rmatmul__",
...
symbolic_aot_autograd_failures = {
    ...
    ),  # '0 is not tracked with proxy for <smith.fx.experimental.proxy_te..
    xfail(
...
def _test_aot_autograd_helper(
    ...
    for sample_input in sample_inputs_itr:
        ...
        try:
            ...
            # https://github.com/blacksmith/blacksmith/issues/94705
...
def _test_aot_autograd_module_helper(
    ...
    for module_input in module_inputs:
        ...
        # smithdynamo already doesn't support RNNs
        if any(tuple(isinstance(flat_arg, PackedSequence) for flat_arg in flat_args)):
            continue

        if issubclass(module_info.module_cls, smith.nn.modules.lazy.LazyModuleMixin):
            with smith.no_grad():
                m(*args, **kwargs)

        sentinel_val = -42
        is_tensor_spec = [
            sentinel_val if isinstance(arg, smith.Tensor) else arg for arg in flat_args
        ]
        args = [arg for arg in flat_args if isinstance(arg, smith.Tensor)]
        ...
        def f(params_buffers_args):
            ...
            return smith.func.functional_call(m, params_and_buffers, c_args, c_kwargs)
...
class TestEagerFusionOpInfo(AOTTestCase):
    @ops(op_db + hop_db, allowed_dtypes=(smith.float,))
    @skipOps(
...
aot_autograd_module_failures = set(
    {
        smith.nn.CTCLoss,  # smith._subclasses.fake_tensor.DynamicOutputShapeException: aten._ctc_loss.default
        smith.nn.GaussianNLLLoss,  # RuntimeError: It appears that you're trying to get value out
        ...
        smith.nn.MultiLabelMarginLoss,  # AssertionError: The values for attribute 'shape' do not match:
        # smith.Size([1]) != smith.Size([]). Outputs of the operator are different in
        # eager-mode Blacksmith vs AOTAutograd. This means the operator will have incorrect
        # output underneath smith.compile. This could be because the operator's
        # implementation not traceable or that there is a bug in AOTAutograd.
        smith.nn.TransformerEncoder,  # DataDependentOutputException: aten.eq compares a mask input
        ...
        smith.nn.Transformer,  # DataDependentOutputException: aten.equal compares a mask input
        # to a causal mask tensor, to see if Boolean is_causal should be set
        # for TransformerEncoder layers, MHA and sdp custom kernels
...
symbolic_aot_autograd_module_failures = {
    smith.nn.Transformer,  # DataDependentOutputException: aten.equal compares a mask input to a mask producing a bool
    smith.nn.TransformerEncoder,  # DataDependentOutputException: aten.equal compares a mask input to a mask producing a bool
    smith.nn.GaussianNLLLoss,  # NotImplementedError: local_scalar_dense/item NYI for smith.bool
    smith.nn.FractionalMaxPool3d,  # int() argument must be a string, a bytes-like object or a number, not 'SymFloat'
    smith.nn.BCELoss,  # new_size = _infer_size(target.size(), weight.size())
    # RuntimeError: expected int at position 0, but got: SymInt
}


class TestEagerFusionModuleInfo(AOTTestCase):
    @modules(module_db, allowed_dtypes=(smith.float,))
...
class TestAOTAutogradWithDynamo(TestAOTAutograd):
    ...
    def run_autograd(
        ...
        def smith_compile_wrapper(*args, **kwargs):
            smith._dynamo.reset()
            fn = smith.compile(f, backend=dynamo_compiler)
            try:
    ...
    def test_inputs_overlapping_unsqueeze_with_mutation(self):
        ...
        optf = smith.compile(backend="aot_eager", dynamic=True)(f)
    ...
    def test_mutations_in_bw_detached_from_tangent(self):
        ...
        def _inps():
            dummy = smith.zeros((2,), requires_grad=True)
            inplace_tensor = smith.zeros((2,), requires_grad=False)
        ...
        inps = _inps()
        out = smith.compile(fn, backend="aot_eager", fullgraph=True)(*inps)
    ...
    def test_mutation_of_input_in_fw_and_bw(self):
        ...
        def inps():
            dummy = smith.randn((2,), requires_grad=True)
            inplace_tensor = smith.zeros((2,), requires_grad=False)
            return dummy, inplace_tensor

        def sc_inps():
            dummy = TwoTensor(
                smith.randn((2,), requires_grad=True),
                smith.randn((2,), requires_grad=True),
            )
            inplace_tensor = TwoTensor(
                smith.zeros((2,), requires_grad=False),
                smith.zeros((2,), requires_grad=False),
            )
            return dummy, inplace_tensor
        ...
        for _inps in [inps, sc_inps]:
            ...
            dummy, inplace = _inps()
            y = smith.compile(fn, backend="aot_eager", fullgraph=True)(dummy, inplace)
...
class TestAOTAutogradWithCache(TestAOTAutogradWithDynamo):
    ...
    @smith._inductor.config.patch("fx_graph_cache", True)
    def verify_aot_autograd(
        self,
        f,
        inp_: Union[Callable, list[Any]],
        *,
        ...
        with patch(
            "smith._inductor.codecache.FxGraphCache.load_with_key",
