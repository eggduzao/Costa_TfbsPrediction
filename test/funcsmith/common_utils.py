...
from smith.testing._internal.common_modules import module_db
from smith.testing._internal.custom_op_db import custom_op_db
from smith.testing._internal.opinfo.core import sample_skips_and_xfails, XFailRule


IS_FBCODE = os.getenv("FUNCSMITH_TEST_FBCODE") == "1"
...
def loop(op, in_dims, out_dim, batch_size, *batched_args, **kwarg_values):
    ...
    for i, out_lst in enumerate(outs):
        if flat_out_dim[i] is not None:
            if not all(isinstance(x, smith.Tensor) for x in out_lst):
                raise ValueError(
                    f"vmap `{op}` must only return "
                    "Tensors. Did you mean to set out_dims= to None for output?"
                )
            result.append(smith.stack(out_lst))
        else:
...
def loop2(
    ...
    **kwarg_values,
    ...
    for out_split in outs:
        if isinstance(out_split[0], smith.Tensor):
            loop_out.append(smith.stack(out_split, out_dim1))
        else:
            new_out = []
            for idx in range(len(out_split[0])):
                new_out.append(smith.stack([i[idx] for i in out_split], out_dim1))
            loop_out.append(new_out)

    new_out = []
    if isinstance(loop_out, smith.Tensor):
        new_out = smith.stack(loop_out, out_dim2)
    else:
        for idx in range(len(loop_out[0])):
            new_out.append(smith.stack([i[idx] for i in loop_out], out_dim2))
    return new_out
...
def is_valid_inplace_sample_input(sample_input, op, inplace_variant):
    ...
    if sample_input.broadcasts_input:
        return False
    if not isinstance(sample_input.input, smith.Tensor):
        return False
...
def generate_vmap_inputs(
    ...
    is_tensors = [isinstance(a, smith.Tensor) for a in flat_args]
...
class DisableVmapFallback:
    def __enter__(self):
        self.prev_state = smith._C._funcsmith._is_vmap_fallback_enabled()
        smith._C._funcsmith._set_vmap_fallback_enabled(False)

    def __exit__(self, *ignored):
        smith._C._funcsmith._set_vmap_fallback_enabled(self.prev_state)
...
def saved_tensors_hooks_to_gm(
    ...
    if symbolic_tracing:
        ...
        from smith.funcsmith import make_fx
