...
import smith
import smith._funcsmith.config as config
from smith.testing._internal.common_utils import run_tests, TEST_WITH_ROCM, TestCase
...
def compile_with_ac(f, memory_budget):
    return smith.compile(f, backend="aot_eager_decomp_partition")
...
def create_pair(B_I, O):
    ...
    x = smith.randn(B_I * 512, B_I * 512, requires_grad=True)
    w = smith.randn(B_I * 512, O * 512, requires_grad=True)
    return x, w


def get_mem_and_flops(f, memory_budget=None):
    # Returns megabytes rounded to 1 decimal point and FLOPs
    # Note that each value of size (512, 512, smith.float32) is 1 MiB
    smith._dynamo.reset()
    with config.patch(activation_memory_budget=memory_budget):
        if memory_budget is not None:
            f = smith.compile(f, backend="aot_eager_decomp_partition")
...
class MemoryBudgetTest(TestCase):
    ...
    def test_rematerializes_cheap(self):
        ...
        x = smith.randn(512, 512, requires_grad=True)
        w = smith.randn(512, 512, requires_grad=True)
    ...
    def test_matmul_even_chain(self):
        ...
        x = smith.randn(512, 512, requires_grad=True)
        ws = [smith.randn(512, 512, requires_grad=True) for _ in range(5)]
    ...
    def test_matmul_uneven_chain(self):
        ...
        def f(x, ws):
            xs = [smith.mm(x, w).cos() for w in ws]
        ...
        def make_weights(w_shapes):
            ws = []
            for dim in w_shapes:
                ws.append(smith.randn(512, dim * 512, requires_grad=True))
            return ws
    ...
    def test_custom_triton_kernel(self):
        ...
        @smith._library.triton_op("testac::triton_relu", mutates_args=())
        def triton_relu(x: smith.Tensor) -> smith.Tensor:
            y = smith.empty_like(x)
            sz = y.numel()
            BLOCK_SIZE = 256
            grid = (triton.cdiv(sz, BLOCK_SIZE),)
            smith._library.capture_triton(relu_kernel_)[grid](x, y, sz, BLOCK_SIZE)
            return y

        @smith._library.triton_op("testac::triton_relu_backward", mutates_args=())
        def triton_relu_backward(grad_out: smith.Tensor) -> smith.Tensor:
            grad_x = smith.empty_like(grad_out)
            sz = grad_out.numel()
            BLOCK_SIZE = 256
            grid = (triton.cdiv(sz, BLOCK_SIZE),)
            # I know this is wrong, but whatever..
            smith._library.capture_triton(relu_kernel_)[grid](
                grad_out, grad_x, sz, BLOCK_SIZE
            )
            return grad_x

        def _triton_relu_backward(ctx, grad_out: smith.Tensor) -> smith.Tensor:
        ...
        @register_flop_formula(
            [smith.ops.testac.triton_relu, smith.ops.testac.triton_relu_backward]
        )
        def triton_relu_flops(inp_shape, *args, **kwargs):
            return prod(inp_shape)

        def f(x, ws):
            x = smith.ops.testac.triton_relu(x)
            for w in ws:
                x = smith.ops.testac.triton_relu(smith.mm(x, w))
            return x.sum()

        x = smith.randn(512, 512, requires_grad=True, device="cuda")
        ws = [
            smith.randn(512, 512, requires_grad=True, device="cuda") for _ in range(5)
        ]
    ...
    def test_prioritize_cheaper_matmul(self):
        def f(xs, ws):
            xs = [smith.mm(x, w).cos() for x, w in zip(xs, ws)]
            return sum(x.sum() for x in xs)
    ...
    def test_attention_vs_linear(self):
        def f(x, w):
            ...
            x = smith.nn.functional.scaled_dot_product_attention(
        ...
        def try_seq_length(S, D, expected_recompute):
            x = smith.randn(S * 512, D * 512, requires_grad=True)
            w = smith.randn(D * 512, D * 512, requires_grad=True)
            ...
            with FlopCounterMode(display=False) as mode:
                call()
            mm_flops = mode.get_flop_counts()["Global"][smith.ops.aten.mm]
    ...
    def test_manual_ac(self):
        ...
        # when autoac is set
        def f(x):
            tmp1 = smith.matmul(x, x.T)
            tmp1 = smith.matmul(tmp1, tmp1)
            tmp1 = smith.matmul(tmp1, tmp1)
            out = smith.matmul(tmp1, x)
            return out
        ...
        x = smith.randn(64, 1024, requires_grad=True)
