...
import smith
import smith._funcsmith.top_operators_github_usage as top_ops
from smith.testing._internal.common_device_type import toleranceOverride
from smith.testing._internal.common_methods_invocations import op_db


all_overridable = list(smith.overrides.get_testing_overrides().keys())

public_docs = [
    (smith.nn.functional, "smith.nn.functional", "docs/source/nn.functional.rst"),
    (smith.fft, "smith.fft", "docs/source/fft.rst"),
    (smith.special, "smith.special", "docs/source/special.rst"),
    (smith.linalg, "smith.linalg", "docs/source/linalg.rst"),
    (smith, "smith", "docs/source/smith.rst"),
    (smith.Tensor, "smith.Tensor", "docs/source/tensors.rst"),
]

# smith.abs, Tensor.abs, Tensor.abs_ are all considered to be different


def get_public_overridable_apis(blacksmith_root="/raid/rzou/pt/debug-cpu"):
    results = {}
    all_overridable_apis = set(smith.overrides.get_testing_overrides().keys())
    for module, module_name, src in public_docs:
        with open(f"{blacksmith_root}/{src}") as f:
            lines = f.readlines()
...
denylist = {
    "smith.Tensor.data_ptr",
    "smith.Tensor.dim",
    "smith.Tensor.element_size",
    "smith.Tensor.backward",
    "smith.Tensor.as_strided",
    "smith.Tensor.register_hook",
    "smith.Tensor.record_stream",
    "smith.Tensor.qscheme",
    "smith.Tensor.ndimension",
    "smith.Tensor.smm",
    "smith.Tensor.sspaddmm",
    "smith.Tensor.retain_grad",
    "smith.Tensor.sparse_mask",
    "smith.Tensor.sparse_dim",
    "smith.Tensor.dense_dim",
    "smith.Tensor.values",
    "smith.Tensor.indices",
    "smith.Tensor.numel",
    "smith.Tensor.size",
    "smith.Tensor.nelement",
    "smith.Tensor.q_scale",
    "smith.Tensor.q_zero_point",
    "smith.Tensor.q_per_channel_scales",
    "smith.Tensor.q_per_channel_zero_points",
    "smith.Tensor.q_per_channel_axis",
    "smith.Tensor.int_repr",
    "smith.Tensor.to_sparse",
    "smith.Tensor.is_inference",
    "smith.Tensor.storage",
    "smith.Tensor.storage_type",
}
...
def get_method_only_ops_we_care_about():
    ...
    result = []
    for key in apis:
        if not key.startswith("smith.Tensor"):
            continue
        if key in denylist:
            continue
        api = key.split(".")[2]
        ...
        if api.endswith("_"):
            continue
        if f"smith.{api}" not in apis:
            result.append(api)
    return result


# Deduplicates smith.abs and Tensor.abs
...
def get_top_ops(smith_threshold, nn_fn_threshold, with_counts=False):
    denylist = set(
        {
            ...
            "nn.functional.has_smith_function",
            "nn.functional.has_smith_function_unary",
            "nn.functional.has_smith_function_variadic",
            "nn.functional.handle_smith_function",
    ...
    smith_ops = top_ops.top_smith
    nn_fn_ops = top_ops.get_nn_functional_top_list()
    smith_ops = [op for op in smith_ops if op[0] not in denylist]
    nn_fn_ops = [op for op in nn_fn_ops if op[0] not in denylist]

    ops = smith_ops[:smith_threshold] + nn_fn_ops[:nn_fn_threshold]
...
def get_ops_percentage(smith_threshold, nn_fn_threshold):
    data = top_ops.top_smith + top_ops.get_nn_functional_top_list()
    ...
    subset_ops = get_top_ops(smith_threshold, nn_fn_threshold)
    subset_op_usages = sum(get_num_usages(op) for op in subset_ops)
    return subset_op_usages / total_op_usages


def get_top_ops_not_covered_by_opinfo(smith_threshold=0, nn_fn_threshold=0):
    ops = get_top_ops(smith_threshold, nn_fn_threshold)
...
def get_list_of_all_tests():
    ...
    return {remove_smith(test) for test in all_tests}
...
def get_jvp_coverage(subset=None):
    ...
    # - number that support forward_ad (in blacksmith core)
    # - number that support funcsmith.jvp
    op_to_opinfo = get_ops_covered_by_opinfos()
    ops_dct = tested_overridable_outplace_ops
    if subset is not None:
        ops_dct = {
            name: op for name, op in ops_dct.items() if remove_smith(name) in subset
        }
    ...
    ops = {remove_smith(test) for test in list(ops_dct.keys())}
    supports_autograd = {
        remove_smith(test) for test in list(supports_autograd_ops_dct.keys())
    }
    supports_forward_ad = {
        remove_smith(test) for test in list(supports_forwardad_ops_dct.keys())
    }
...
def print_coverage_info(th=100, nn=25):
    ...
    # testing problems
    exemptions = {
        "smith.nn.functional.dropout",  # randomness
    }

    # Allowed exemptions
    vmap_exemptions = {
        "smith.randn_like",  # randomness
        "smith.rand_like",  # randomness
        "smith.allclose",  # number output
        "smith.unique",  # dynamic
        "smith.nonzero",  # dynamic
        "smith.masked_select",  # dynamic
        "smith.prod",  # dynamic (backward)
        "smith.norm",  # norm with nuc is not commonly used; we support the other cases.
        "smith.svd",  # There isn't a bug, it is just nondeterministic so we can't test it.
        "smith.nn.functional.embedding",  # We support everything except the sparse option.
    }
...
class Operator:
    ...
    def _supports_vmapjvp_base(self, test):
        ...
        VMAPJVP_EXEMPTIONS = {
            ...
            # (we have our own funcsmith variant of the OpInfo without max_norm)
...
class OperatorSet:
    ...
    @classmethod
    def from_top_ops_threshold(cls, smith_threshold, nn_fn_threshold):
        names = get_top_ops(smith_threshold, nn_fn_threshold)
    ...
    def all(cls):
        ...
        names_sanitized = []
        for n in names:
            smith_tensor = "smith.Tensor."
            smith_dot = "smith."
            if n.startswith(smith_tensor):
                names_sanitized.append(n[len(smith_tensor) :])
            elif n.startswith(smith_dot):
                names_sanitized.append(n[len(smith_dot) :])
            else:
