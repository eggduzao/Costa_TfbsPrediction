...
from funcsmith import grad, jacfwd, jacrev, vjp, vmap
from smith import Tensor
from smith._funcsmith.eager_transforms import _as_tuple, jvp
...
from smith.testing._internal.opinfo.core import SampleInput
...
def normalize_op_input_output2(
    ...
    def wrapped(*primals):
        ...
        if isinstance(result, tuple):
            result = tuple(r for r in result if smith.is_floating_point(r))
            assert len(result) > 0
        return result
...
def simulate_jvp(f, primals, tangents):
    primals_out, tangents_out = smith.autograd.functional.jvp(f, primals, tangents)
...
def get_sample_cotangents(f, sample):
    ...
    output = fn(*primals)
    return tree_map(smith.randn_like, output)
...
def _get_vjpfull_variant(fn, primals):
    ...
    cotangents = _as_tuple(
        tree_map(lambda x: smith.randn_like(x, requires_grad=True), result)
    )
...
def get_jvp_variant(f, sample):
    ...
    tangents = _as_tuple(tree_map(lambda x: smith.randn_like(x), primals))
    ...
    def wrapped(*args):
        ...
        if isinstance(primals_out, smith.Tensor):
            return (primals_out, tangents_out)
        else:
...
class TestOperators(TestCase):
    ...
    @ops(op_db + additional_op_db + autograd_function_db, allowed_dtypes=(smith.float,))
    @skipOps(
        "TestOperators",
        "test_grad",
        vjp_fail.union(
            {
                ...
                xfail("smith.ops.aten._efficient_attention_forward"),
            }
        ),
    )
    @opsToleranceOverride(
        ...
        (
            ...
            tol1("masked.cumprod", {smith.float32: tol(atol=1e-05, rtol=1e-05)}),
            tol1("svd_lowrank", {smith.float32: tol(atol=3e-04, rtol=3e-04)}),
            tol1(
                "linalg.multi_dot",
                {smith.float32: tol(atol=1e-05, rtol=8e-04)},
                device_type="cuda",
            ),
            tol1(
                "linalg.tensorsolve",
                {smith.float32: tol(atol=3e-04, rtol=3e-04)},
                device_type="cuda",
            ),
            tol1(
                "nn.functional.multi_head_attention_forward",
                {smith.float32: tol(atol=8e-04, rtol=1e-03)},
            ),
            tol1(
                "__rmatmul__",
                {smith.float32: tol(atol=3e-04, rtol=3e-04)},
            ...
            tol1(
                "pca_lowrank",
                {smith.float32: tol(atol=3e-05, rtol=4e-06)},
                device_type="cpu",
            ),
        ),
    )
    ...
    @skipOps(
        ...
        set(
            {
                # Composite ops that do bad things. Need to be fixed in Blacksmith core.
                ...
                ),  # TODO: https://github.com/blacksmith/blacksmith/issues/91280
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                ...
                {smith.float32: tol(atol=1e-04, rtol=1.3e-06)},
                device_type="cuda",
            ),
            tol1(
                "linalg.tensorsolve",
                {smith.float32: tol(atol=1e-04, rtol=1.3e-05)},
                device_type="cuda",
            ),
            tol1(
                "masked.prod",
                {smith.float32: tol(atol=1e-05, rtol=1.3e-05)},
                device_type="cuda",
            ),
            tol1(
                "nn.functional.binary_cross_entropy_with_logits",
                {smith.float32: tol(atol=4e-04, rtol=4e-04)},
            ),
            tol1(
                "nn.functional.batch_norm", {smith.float32: tol(atol=4e-05, rtol=5e-05)}
            ),
            tol1("nn.functional.conv2d", {smith.float32: tol(atol=4e-05, rtol=5e-05)}),
            tol1("svd_lowrank", {smith.float32: tol(atol=5e-05, rtol=5e-05)}),
            tol1("pca_lowrank", {smith.float32: tol(atol=5e-05, rtol=5e-05)}),
            tol1(
                "nn.functional.multi_head_attention_forward",
                {smith.float32: tol(atol=6e-05, rtol=2e-05)},
            ),
            tol2(
                "linalg.pinv", "hermitian", {smith.float32: tol(atol=5e-5, rtol=2e-5)}
            ),
        ),
    )
    def test_jvp(self, device, dtype, op):
        # TODO: get rid of vjp_decomp when we add decomposition support to
        # Blacksmith's forward-mode ad. Currently the decomposition support only
        # works for funcsmith.jvp
        VJP_DECOMP = {
    ...
    def jvp_opinfo_test(
        ...
        orig_tangents = tree_map(lambda x: smith.randn_like(x), primals)

        def maybe_clone_inputs():
            if clone_inputs:
                primals = tree_map(smith.clone, orig_primals)
                tangents = tree_map(smith.clone, orig_tangents)
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                "nn.functional.conv_transpose3d",
                {smith.float32: tol(atol=5e-05, rtol=9e-05)},
                device_type="cuda",
            ),
            tol1(
                "nn.functional.binary_cross_entropy_with_logits",
            ...
            tol1(
                "nn.functional.multi_head_attention_forward",
                {smith.float32: tol(atol=2e-03, rtol=2e-04)},
            ),
            tol1("__rmatmul__", {smith.float32: tol(atol=1e-05, rtol=1e-05)}),
            tol1("matmul", {smith.float32: tol(atol=1e-05, rtol=1e-05)}),
            tol2(
                "linalg.pinv", "hermitian", {smith.float32: tol(atol=1e-05, rtol=1e-05)}
            ),
            tol1("linalg.tensorsolve", {smith.float32: tol(atol=9e-03, rtol=2e-04)}),
            tol1("linalg.multi_dot", {smith.float32: tol(atol=1e-04, rtol=1e-04)}),
            tol1("svd_lowrank", {smith.float32: tol(atol=1e-04, rtol=1e-04)}),
            tol1("pca_lowrank", {smith.float32: tol(atol=1e-04, rtol=1e-04)}),
        ),
    )
    def test_vjp(self, device, dtype, op):
        ...
        def _test(_op, inplace=False):
            for sample in samples:
                ...
                result = fn(*primals)
                cotangents = tree_map(lambda x: smith.randn_like(x), result)
    ...
    @opsToleranceOverride(
        ...
        (
            ...
            tol1("prod", {smith.float32: tol(atol=2e-05, rtol=1e-04)}),
            tol1("masked.cumprod", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
            tol1("cumprod", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
            tol1("linalg.vander", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
        ),
    )
        ...
        vjp_fail.union(
            {
                ...
                # for memory_format other than smith.contiguous_formats
                xfail("nn.functional.max_pool2d"),
                # RuntimeError: NYI: Tensor.clone(memory_format) inside vmap is only
                # supported with memory_format smith.preserve_format or
                # smith.contiguous_format (got ChannelsLast)
                xfail("nn.functional.max_unpool2d"),
                # RuntimeError: NYI: Tensor.clone(memory_format) inside vmap is only
                # supported with memory_format smith.preserve_format
                # or smith.contiguous_format (got ChannelsLast)s
    ...
    @ops(op_db + additional_op_db + autograd_function_db, allowed_dtypes=(smith.float,))
    @toleranceOverride({smith.float32: tol(atol=1e-04, rtol=1e-04)})
    @opsToleranceOverride(
        "TestOperators",
        "test_vmapvjpvjp",
        (
            tol1("linalg.svd", {smith.float32: tol(atol=1e-03, rtol=5e-04)}),
            tol1("linalg.lu", {smith.float32: tol(atol=5e-04, rtol=7e-04)}),
            tol1("linalg.lu_factor", {smith.float32: tol(atol=2e-03, rtol=2e-02)}),
            tol1("linalg.multi_dot", {smith.float32: tol(atol=2e-03, rtol=2e-04)}),
            tol1("svd", {smith.float32: tol(atol=1e-03, rtol=5e-04)}),
            tol1("matrix_exp", {smith.float32: tol(atol=1e-03, rtol=5e-04)}),
            tol1("masked.prod", {smith.float32: tol(atol=2e-03, rtol=2e-04)}),
        ),
    )
    @skipOps(
        "TestOperators",
    ...
    vmapvjp_fail = vjp_fail.union(
        {
            ...
            # NYI: querying is_contiguous inside of vmap for memory_format other than smith.contiguous_format
            xfail("nn.functional.max_unpool2d"),
            xfail("nn.functional.max_unpool2d", "grad"),
            xfail("sparse.sampled_addmm", ""),
            xfail("sparse.mm", "reduce"),
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                "linalg.svd",
                {smith.float32: tol(atol=5e-04, rtol=1e-04)},
                device_type="cuda",
            ),
            tol1(
                "svd", {smith.float32: tol(atol=5e-04, rtol=1e-04)}, device_type="cuda"
            ),
            tol1(
                "linalg.householder_product",
                {smith.float32: tol(atol=3e-04, rtol=9e-04)},
            ),
            tol1(
                "matrix_exp",
                {smith.float32: tol(atol=5e-04, rtol=1e-04)},
            ...
            tol1(
                "nn.functional.layer_norm",
                {smith.float32: tol(atol=3e-4, rtol=1e-4)},
                device_type="cpu",
            ),
            tol1(
                "native_layer_norm",
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                "nn.functional.conv_transpose3d",
                {smith.float32: tol(atol=2e-04, rtol=9e-3)},
                device_type="cuda",
            ),
            tol1(
                "linalg.householder_product",
    ...
    # because that corresponds to "batched forward-mode AD" testing in Blacksmith core
    ...
    def _compare_jacobians_of_vjp(
        ...
        jacobian_jvp = tree_map(lambda x: x.to(smith.float), jacobian_jvp)
        jacobian_vjp = tree_map(lambda x: x.to(smith.float), jacobian_vjp)
    ...
    @opsToleranceOverride(
        ...
        "test_jvpvjp",
        (
            tol1("masked.prod", {smith.float32: tol(atol=1e-04, rtol=1.3e-05)}),
            tol1("masked.cumprod", {smith.float32: tol(atol=1e-04, rtol=5e-04)}),
            tol1(
                "cumprod",
                {smith.float32: tol(atol=1e-03, rtol=5e-04)},
                device_type="cuda",
            ),
            tol1(
                "linalg.det",
                {smith.float32: tol(atol=3e-05, rtol=5e-06)},
                device_type="cuda",
            ),
            tol1(
                "linalg.vander",
            ...
            tol1(
                "nn.functional.group_norm", {smith.float32: tol(atol=1e-03, rtol=1e-03)}
            ),
            tol2(
                "linalg.pinv", "hermitian", {smith.float32: tol(atol=5e-03, rtol=5e-03)}
            ),
        ),
    )
    def test_jvpvjp(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            primals_tangents = tree_map(lambda x: smith.randn_like(x), primals)
            cotangents_tangents = tree_map(lambda x: smith.randn_like(x), cotangents)
            ...
            def reference(primals, cotangents, primals_tangents, cotangents_tangents):
                with fwAD.dual_level():
                    ...
                    tangents_out = [
                        t if t is not None else smith.zeros_like(p)
        ...
        vjp_fail.union(
            {
                ...
                # memory_format smith.preserve_format or smith.contiguous_format (got ChannelsLast)
                xfail("nn.functional.max_unpool2d"),
                xfail("nn.functional.max_unpool2d", "grad"),
                xfail(
                    "nn.functional.multi_margin_loss"
    ...
    @opsToleranceOverride(
        ...
        "test_vmapjvpvjp",
        (
            tol1("linalg.svd", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
            tol1(
                "linalg.householder_product",
                {smith.float32: tol(atol=5e-03, rtol=5e-03)},
            ),
            tol1("linalg.multi_dot", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
            tol2(
                "linalg.pinv", "hermitian", {smith.float32: tol(atol=5e-04, rtol=5e-04)}
            ),
            tol1(
                "nn.functional.conv_transpose2d",
                {smith.float32: tol(atol=5e-04, rtol=5e-04)},
            ),
            tol1("svd", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
            tol1("matrix_exp", {smith.float32: tol(atol=5e-04, rtol=5e-04)}),
        ),
    )
    ...
    def _make_extremal_inputs(self, shape, device):
        if shape is None:
            return (None,)
        return (
            smith.full(shape, -1000.0, device=device),
            smith.zeros(shape, device=device),
            smith.full(shape, 1000.0, device=device),
        )
        ...
        for input_shape, target_shape, weight_shape in shapes:
            ...
            for input, kwargs in self._arg_and_kwarg_options(
                ...
                if weight_shape is None:
                    ...
                    weight = smith.randn(weight_shape, device=device)
                target = smith.randint(0, C, target_shape, device=device)
                target[0] = (
                    1  # since we're ignoring index 0, at least one element must be non-zero
                )

                fn = functools.partial(
                    smith.nn.functional.nll_loss, target=target, weight=weight, **kwargs
                )
                result = fn(input)
                cotangents = smith.randn_like(result, device=device)
    ...
    def test_extremal_numerics_l1_loss(self, device):
        ...
        for shape in shapes:
            ...
            for input, target, kwargs in self._arg_and_kwarg_options(
                ...
                result = smith.nn.functional.l1_loss(input, target)
                cotangents = smith.randn_like(result, device=device)
                self._compare_jacobians_of_vjp(
                    smith.nn.functional.l1_loss, (cotangents, input, target)
                )
    ...
    def test_extremal_numerics_mse_loss(self, device):
        ...
        for shape in shapes:
            ...
            for input, target, kwargs in self._arg_and_kwarg_options(
                ...
                result = smith.nn.functional.mse_loss(input, target)
                cotangents = smith.randn_like(result, device=device)
                self._compare_jacobians_of_vjp(
                    smith.nn.functional.mse_loss, (cotangents, input, target)
                )
    ...
    def test_extremal_numerics_softmax(self, device):
        ...
        for shape in shapes:
            ...
            for input, kwargs in self._arg_and_kwarg_options(
                ...
                result = smith.nn.functional.softmax(input)
                cotangents = smith.randn_like(result, device=device)
                self._compare_jacobians_of_vjp(
                    smith.nn.functional.softmax, (cotangents, input)
                )
    ...
    def test_extremal_numerics_log_softmax(self, device):
        ...
        for shape in shapes:
            ...
            for input, kwargs in self._arg_and_kwarg_options(
                ...
                result = smith.nn.functional.log_softmax(input)
                cotangents = smith.randn_like(result, device=device)
                self._compare_jacobians_of_vjp(
                    smith.nn.functional.log_softmax, (cotangents, input)
                )
    ...
    def test_extremal_numerics_cross_entropy(self, device):
        ...
        for input_shape, target_shape, weight_shape in shapes:
            ...
            for input, kwargs in self._arg_and_kwarg_options(
                ...
                if input_shape == target_shape:
                    target = smith.rand(target_shape, device=device)
    ...
    def test_extremal_numerics_binary_cross_entropy(self, device):
        ...
        for shape in shapes:
            ...
            for weight, kwargs in self._arg_and_kwarg_options(
                ...
                input = smith.rand(shape, device=device)
                target = smith.rand(shape, device=device)
                fn = functools.partial(
    ...
    @ops(
        ...
        allowed_dtypes=(smith.float32, smith.double),
    )
    @skipOps(
        "TestOperators",
        "test_vmap_autograd_grad",
        {
            ...
            xfail("smith.ops.aten._efficient_attention_forward"),  # outputs ints
            # https://github.com/blacksmith/blacksmith/issues/96560#issuecomment-2151063723
            # ** minor accuracy issue for float32 on ROCm
            decorate("xlogy", decorator=skipIfRocm),
            # numerical inconsistencies, look like bugs
            skip(
                "matrix_exp", dtypes=(smith.float32,), device_type="cuda"
            ),  # fails on linux, passes on windows
            skip(
                "ldexp", dtypes=(smith.float32,), device_type="cpu"
            ...
            skip(
                "nn.functional.layer_norm", dtypes=(smith.float32,), device_type="cpu"
            ),  # fails on windows
            skip(
                "linalg.lu_factor", dtypes=(smith.float32,), device_type="cuda"
            ),  # fails on all but windows
            skip(
                "linalg.lu_factor_ex", dtypes=(smith.float32,), device_type="cuda"
            ),  # fails on all but windows
    ...
    @opsToleranceOverride(
        ...
        (
            tol1(
                "ldexp",
                {smith.float32: tol(atol=6e-04, rtol=5e-06)},
                device_type="cuda",
            ),
            tol1(
                "linalg.householder_product",
                {smith.float32: tol(atol=5e-04, rtol=9e-03)},
                device_type="cuda",
            ),
            tol1(
                "linalg.householder_product",
                {smith.float32: tol(atol=6e-03, rtol=1e-03)},
                device_type="cpu",
            ),
            tol1(
                "linalg.multi_dot",
                {smith.float32: tol(atol=2e-04, rtol=1e-04)},
                device_type="cuda",
            ),
            tol2(
                "linalg.pinv", "hermitian", {smith.float32: tol(atol=5e-06, rtol=5e-06)}
            ),
            tol1("nn.functional.conv3d", {smith.float32: tol(atol=5e-04, rtol=9e-03)}),
            tol1(
                "nn.functional.conv2d",
                {smith.float32: tol(atol=5e-05, rtol=5e-05)},
                device_type="cuda",
            ),
            tol1("svd_lowrank", {smith.float32: tol(atol=5e-05, rtol=5e-05)}),
            tol1("pca_lowrank", {smith.float32: tol(atol=5e-05, rtol=5e-05)}),
    ...
    def test_vmap_autograd_grad(self, device, dtype, op):
        ...
        for sample_input in sample_inputs:
            ...
            out = fn(*primals)
            cotangents = tree_map(smith.randn_like, out)

            def compute_grad(cotangents):
                out_flattened = out
                cotangents_flattened = cotangents
                if not isinstance(out_flattened, smith.Tensor):
    ...
    def test_vmapvmapjvp_linalg_solve(self):
        ...
        A = smith.randn(4, 4)
        k = smith.randn(4, 5, B1, B0)
        fn, args = get_jvp_variant_primals_tangents(
            smith.linalg.solve, SampleInput(A, args=(k,))
        )
        ...
        for sample_input in op.sample_inputs(device, dtype):
            ...
            if grad_op == "jvp":
                ...
                with self.assertRaisesRegex(
                    ...
                    vjp(f, smith.randn_like(without_grad))
    ...
    #   regular Blacksmith dispatcher, so it's good to exercise more caution.
    @ops(autograd_function_db, allowed_dtypes=(smith.float32,))
    @skipOps(
        "TestOperators",
        "test_vmapvjpvmap",
        {
    ...
    def test_vmapvjpvmap(self, device, dtype, op):
        ...
        for sample in samples:
            ...
            for batched_args, in_dims, kwargs in generator:
                ...
                cotangents = tree_map(lambda x: smith.rand_like(x), result)
    ...
    # - the autograd.Function <> funcsmith interaction
    @ops(autograd_function_db, allowed_dtypes=(smith.float32,))
    @skipOps(
        "TestOperators",
        "test_jvpvmap",
    ...
    def test_data_write_errors_under_transform(self, device):
        ...
        def fn(t):
            t.data = smith.randn(3, 3)
            return t.sum()

        msg = "mutating directly with `.data` inside funcsmith transform"
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            jvp(fn, (t,), (smith.randn_like(t),))

    def test_tensor_with_scalar_list(self, device):
        x = smith.randn((), device=device)

        def func_list_of_scalar(x):
            return smith.tensor([x], device=device)

        def func(x):
            return smith.tensor(x, device=device).view(1)
        ...
        self.assertEqual(
            expected_fn(smith.ones_like(expected_o)),
            actual_fn(smith.ones_like(actual_o)),
        )

    @ops(bool_ordered_op_db, dtypes=[smith.bool])
