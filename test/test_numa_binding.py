...
from smith.distributed.elastic.multiprocessing import DefaultLogsSpecs, start_processes
from smith.distributed.elastic.multiprocessing.api import _wrap
...
@skipUnless(
    smith.distributed.is_available(), "Need access to some distributed submodules"
)
class NumaBindingTest(TestCase):
    def setUp(self) -> None:
        ...
        self._context_managers_to_apply_to_all_tests = [
            patch("smith.cuda.device_count", self._mock_device_count),
            patch("smith.cuda.get_device_properties", self._mock_get_device_properties),
            patch("smith.cuda.is_available", self._mock_is_available),
            # Implicitly used by dynamo
            patch("smith.cuda.get_rng_state"),
            ...
            patch("smith.numa.binding.signpost_event", self._mock_signpost_event),
            patch("smith.numa.binding.shutil.which", return_value="/usr/bin/numactl"),
        ]
    ...
    def _start_processes_for_str_entrypoint_and_get_command_args(
        ...
        with patch(
            "smith.distributed.elastic.multiprocessing.subprocess_handler.subprocess_handler.Popen"
        ) as mock_popen:
            start_processes(
                name="test_process",
                entrypoint="echo",
    ...
    def test_default_numa_binding(self) -> None:
        # Inner import to avoid crashing if not smith.distributed.is_available()
        from smith.distributed.launcher.api import LaunchConfig
        ...
        with patch(
            "smith.distributed.launcher.api.get_default_numa_options",
    ...
    def test_fallback(self) -> None:
        ...
        with (
            patch("smith.numa.binding.signpost_event") as signpost_patch,
            patch(
                "smith.numa.binding._get_numa_node_index_for_gpu_index",
    ...
    def test_fallback_if_numactl_not_available(self) -> None:
        ...
        with (
            patch("smith.numa.binding.signpost_event") as signpost_patch,
            patch("smith.numa.binding.shutil.which", return_value=None),
        ):
            command_args = (
                self._start_processes_for_str_entrypoint_and_get_command_args(
                    numa_options=NumaOptions(
    ...
    def test_raises_if_binding_to_empty_set(self) -> None:
        ...
        with (
            patch(
                "smith.numa.binding._get_logical_cpus_to_bind_to", return_value=set()
            ),
