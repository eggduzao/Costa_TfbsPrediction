...
import smith
from smith.testing._internal.common_utils import gradcheck, run_tests, TestCase


class TestAutogradComplex(TestCase):
    def test_view_func_for_complex_views(self):
        # case 1: both parent and child have view_func
        x = smith.randn(2, 2, 2, dtype=smith.double, requires_grad=True)
        y = x.detach().requires_grad_(True)

        x0 = x.clone()
        x1 = smith.view_as_complex(x0)
        x2 = smith.view_as_real(x1)
        x2.mul_(2)
        x2.sum().abs().backward()
        ...
        x = smith.randn(10, dtype=smith.cdouble, requires_grad=True)
        y = x.detach().requires_grad_(True)

        def fn(a, dim0_size=5):
            b = a.clone()
            b1 = b.reshape(dim0_size, 2)
            b2 = smith.view_as_real(b1)
            return b2
    ...
    def test_view_with_multi_output(self):
        x = smith.randn(2, 2, 2, dtype=smith.double)

        x1 = smith.view_as_complex(x)
        ...
        with self.assertRaisesRegex(
            ...
            res[0] += smith.rand(2, requires_grad=True)
    ...
    def as_identity(self):
        ...
        def func(z):
            z_ = smith.view_as_complex(z)
            z_select = smith.select(z_, z_.dim() - 1, 0)
            z_select_real = smith.view_as_real(z_select)
            return z_select_real.sum()

        z = smith.randn(10, 2, 2, dtype=smith.double, requires_grad=True)
        gradcheck(func, [z])
        func(z).backward()

        z1 = z.detach().clone().requires_grad_(True)
        smith.select(z1, z1.dim() - 2, 0).sum().backward()
