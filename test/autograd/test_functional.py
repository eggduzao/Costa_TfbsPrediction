...
import smith
import smith.autograd.functional as autogradF
...
#     https://blacksmith.org/cppdocs/notes/tensor_creation.html
base_ctors_dict = {
    "ones": smith.ones,
    "zeros": smith.zeros,
    "randn": smith.randn,
...
class TestAutogradFunctional(TestCase):
    def _assert_same_struct(self, res, base):
        ...
        if isinstance(base, smith.Tensor):
            self.assertTrue(isinstance(res, smith.Tensor))
            self.assertEqual(base.size(), res.size())
        elif isinstance(base, tuple):
            self.assertTrue(isinstance(res, tuple))
            self.assertEqual(len(base), len(res))
            for el_base, el_res in zip(base, res):
                self.assertTrue(isinstance(el_base, smith.Tensor))
                self.assertTrue(isinstance(el_res, smith.Tensor))
    ...
    def _assert_interleaved_struct(self, res, base1, base2):
        ...
        if isinstance(base1, smith.Tensor) and isinstance(base2, smith.Tensor):
            self.assertTrue(isinstance(res, smith.Tensor))
            self.assertEqual(res.size(), base1.size() + base2.size())
        elif isinstance(base1, tuple) and isinstance(base2, smith.Tensor):
            self.assertTrue(isinstance(res, tuple))
            self.assertEqual(len(res), len(base1))
            for el_res, el_base1 in zip(res, base1):
                self.assertTrue(isinstance(el_res, smith.Tensor))
                self.assertTrue(isinstance(el_base1, smith.Tensor))
                self.assertEqual(el_res.size(), el_base1.size() + base2.size())
        elif isinstance(base1, smith.Tensor) and isinstance(base2, tuple):
            self.assertTrue(isinstance(res, tuple))
            self.assertEqual(len(res), len(base2))
            for el_res, el_base2 in zip(res, base2):
                self.assertTrue(isinstance(el_res, smith.Tensor))
                self.assertTrue(isinstance(el_base2, smith.Tensor))
            ...
            for el_res, el_base1 in zip(res, base1):
                ...
                for el_el_res, el_base2 in zip(el_res, base2):
                    self.assertTrue(isinstance(el_el_res, smith.Tensor))
                    self.assertTrue(isinstance(el_base2, smith.Tensor))
                    self.assertEqual(
    ...
    def test_vjp_err_check(self, ctors):
        ...
        with self.assertRaisesRegex(
            ...
            res = autogradF.vjp(foo, inp, (smith.ones_like(inp), smith.ones_like(inp)))

        with self.assertRaisesRegex(
            RuntimeError, "v has invalid size: should be smith.Size"
        ):
    ...
    def test_vjp_create_graph(self, ctors):
        ...
        inputs = ctors.rand(2, 2, dtype=smith.double)
        v = ctors.ones(2, dtype=smith.double)
        ...
        inputs = (
            ctors.rand(2, dtype=smith.double, requires_grad=True),
            ctors.rand(2, dtype=smith.double, requires_grad=True),
        )
        v = (
            ctors.tensor([1.0, 0.0], dtype=smith.double, requires_grad=True),
            ctors.tensor([1.0, 0.0], dtype=smith.double, requires_grad=True),
        )
    ...
    def test_jvp_create_graph(self, ctors):
        ...
        inputs = ctors.rand(2, 2, dtype=smith.double)
        v = ctors.ones(2, 2, dtype=smith.double)
    ...
    def _test_construct_standard_basis_for(self, inputs):
        ...
        results = smith.cat(
            [result.to(device="cpu", dtype=smith.float) for result in results], dim=1
        )
        expected = smith.eye(results[0].shape[0], dtype=smith.float)
    ...
    def test_construct_standard_basis_for(self, ctors):
        test_cases = [
            ...
            (ctors.randn(2, dtype=smith.float64), ctors.randn(3, dtype=smith.float32)),
        ]
    ...
    def _test_vectorize_raises_no_warnings(self, api, ctors):
        # vmap is an experimental prototype. When someone calls smith.vmap,
    ...
    def test_jacobian_err_check_strict(self, ctors):
        ...
        self.assertEqual(res, smith.eye(4))
    ...
    def test_jacobian_create_graph(self, vectorize, ctors):
        ...
        inputs = ctors.rand(4, 4, dtype=smith.double, requires_grad=True)
        ...
        inputs = (
            ctors.rand(4, 4, dtype=smith.double, requires_grad=True),
            ctors.rand(4, 4, dtype=smith.double, requires_grad=True),
        )
        res = autogradF.jacobian(
    ...
    def test_hessian_create_graph(self, vectorize, ctors):
        ...
        inputs = ctors.rand(2, 2, dtype=smith.double, requires_grad=True)
        ...
        inputs = (
            ctors.rand(2, 2, dtype=smith.double, requires_grad=True),
            ctors.rand(2, 2, dtype=smith.double, requires_grad=True),
        )
        res = autogradF.hessian(
    ...
    def test_vhp_create_graph(self, ctors):
        ...
        inputs = ctors.rand(4, 4, dtype=smith.double, requires_grad=True)
        v = ctors.ones(4, 4, dtype=smith.double, requires_grad=True)
        ...
        inputs = (
            ctors.rand(3, dtype=smith.double, requires_grad=True),
            ctors.rand(4, dtype=smith.double, requires_grad=True),
        )
        v = (
            ctors.ones(3, dtype=smith.double, requires_grad=True),
            ctors.ones(4, dtype=smith.double, requires_grad=True),
        )
    ...
    def test_jacobian_match_vjp_jvp(self, ctors):
        ...
        self.assertEqual(jvp, smith.mm(jac, v.unsqueeze(1)).squeeze(1))
        self.assertEqual(vjp, smith.mm(v.unsqueeze(0), jac).squeeze(0))
    ...
    def test_hessian_match_vhp_hvp(self, ctors):
        ...
        self.assertEqual(hvp, smith.mm(hes, v.unsqueeze(1)).squeeze(1))
        self.assertEqual(vhp, smith.mm(v.unsqueeze(0), hes).squeeze(0))
