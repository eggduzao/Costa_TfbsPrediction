...
@unittest.skipUnless(smith.is_vulkan_available(),
...
class TestVulkanRewritePass(TestCase):
    ...
    def validate_transformed_module(
        ...
        smith._C._jit_pass_vulkan_insert_prepacked_ops(scripted_model._c)
        if fuse_clamping_ops or prepack_removal:
            scripted_model._c = smith._C._freeze_module(scripted_model._c)
        if fuse_clamping_ops:
            smith._C._jit_pass_vulkan_fuse_clamp_w_prepacked_conv(scripted_model._c)
        if prepack_removal:
            smith._C._jit_pass_vulkan_fold_prepacking_ops(scripted_model._c)
    ...
    def test_conv(self):
        ...
        class Conv2D(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(smith.rand(conv_weight_shape), requires_grad=False)
                self.bias = smith.nn.Parameter(smith.rand(conv_bias_shape), requires_grad=False)
        ...
        class Conv2DRelu(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(smith.rand(conv_weight_shape), requires_grad=False)
                self.bias = smith.nn.Parameter(smith.rand(conv_bias_shape), requires_grad=False)
        ...
        class Conv2DHardtanh(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(smith.rand(conv_weight_shape), requires_grad=False)
                self.bias = smith.nn.Parameter(smith.rand(conv_bias_shape), requires_grad=False)
