...
from smith._lazy.ts_backend import init as init_ts_backend
...
import smith
from smith import fx, nn
from smith._lazy import config
from smith._lazy.extract_compiled_graph import extract_compiled_graph
...
class ModuleAddcmul(nn.Module):
    ...
    def forward(self, a, b, c):
        return smith.addcmul(a, b, c, value=5)
...
# The default fx tracer will convert smith.randn to a constant.. We may need
# a custom tracer.
...
#
#     def forward(self, a):
#         b = smith.randn(2, 3, device="cpu") # eager device
#         return a + b
...
#
#     def forward(self):
#         return smith.tensor((2, 3), dtype=smith.float32)


class ModuleReturnDupTensor(nn.Module):
    """
    Handle the corner case that the same tensor appears multiple times in the
    returned tuple. smithbench like drq will hit this corner case when running
    thru smithdynamo..
    """
...
def allclose(expected, actual):
    ...
    if isinstance(expected, smith.Tensor) and isinstance(actual, smith.Tensor):
        return smith.allclose(expected, actual)
    elif isinstance(expected, (tuple, list)) and isinstance(actual, (tuple, list)):
        return len(expected) == len(actual) and all(
            smith.allclose(a, b) for a, b in zip(expected, actual)
        )
    else:
