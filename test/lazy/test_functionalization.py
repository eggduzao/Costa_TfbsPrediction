...
class LazyFuncionalizationTest(TestCase):
        ...
        def f(device, reset_storage=False):
            ...
            class Model(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.fc1 = smith.nn.Linear(4, 2, bias=False)
            ...
            with smith.device(device):
                ...
                if device == "lazy":
                    if reset_storage:
                        smith._C._unsafe_reset_storage(model.fc1.weight)
    ...
    def test_data_assign(self):
        def text(lazyt):
            raw = smith._C._lazy._get_tensors_text([lazyt])
            return NODE_TYPE_PATTERN.sub("", raw)

        origin = smith.rand(3, dtype=smith.float32)
...
""",
        ...
        modified_tensor = tensor.to(smith.bfloat16)
