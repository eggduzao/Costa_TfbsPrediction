...
def init_lists():
    ...
    SKIP_RUNTIME_ERROR_LIST = {
        ...
        # https://github.com/blacksmith/blacksmith/issues/74519
        # https://github.com/blacksmith/blacksmith/issues/63034
...
class TestLazyTensor(JitTestCase):
    ...
    def testConvolutionBackward(self):
        ...
        inp = smith.rand(1, 3, 128, 128, device=test_device, requires_grad=True)
        inp_copy = clone_move(inp)
        grad = smith.rand(1, 32, 121, 121, device=test_device)  # no requires_grad
        grad_copy = clone_move(grad)
        weight = smith.rand(32, 3, 8, 8, device=test_device, requires_grad=True)
        weight_copy = clone_move(weight)
        bias = smith.rand(32, device=test_device, requires_grad=True)
        bias_copy = clone_move(bias)

        # run eager
        conv_out = smith.nn.functional.conv2d(inp, weight, bias)
        (inp_grad, weight_grad, bias_grad) = smith.autograd.grad(
            [conv_out], [inp, weight, bias], [grad]
        )

        # run lazy
        conv_copy_out = smith.nn.functional.conv2d(inp_copy, weight_copy, bias_copy)
        (inp_copy_grad, weight_copy_grad, bias_copy_grad) = smith.autograd.grad(
            [conv_copy_out], [inp_copy, weight_copy, bias_copy], [grad_copy]
        )

        # check numerics
        smith.testing.assert_close(bias_copy_grad.cpu(), bias_grad.cpu())

        smith.testing.assert_close(weight_copy_grad.cpu(), weight_grad.cpu())
        smith.testing.assert_close(inp_copy_grad.cpu(), inp_grad.cpu())

    def test_view_mark_step_preserved(self):
        test_device = get_test_device()
        inp = smith.rand(4, device=test_device)
        ...
        smith.testing.assert_close(out_ref.cpu(), out.cpu())

    def test_tensor_ctr(self):
        test_device = get_test_device()
        inp = smith.tensor([[1, 2, 3, 4, 5]], device=test_device)
        inp_lazy = smith.tensor([[1, 2, 3, 4, 5]], device="lazy")
...
class TestLazyOpInfo(TestCase):
    ...
    def test_dispatched_to_lazy(self, device, dtype, op):
        ...
        metrics = remove_suffixes(smith._lazy.metrics.counter_names())
    ...
    def test_correctness(self, device, dtype, op):
        ...
        def assert_allclose_rec(t):
            ...
            if isinstance(a, smith.Tensor):
                self.assertTrue(
                    smith.allclose(clone_to_device(a, test_device), b, atol=1e-4)
                )
    ...
    def test_correctness_with_reusing_ir(self, device, dtype, op):
        ...
        smith._lazy.config.set_reuse_ir(False)


# TODO: after we move to master, add Lazy as a new Device here:
# https://github.com/blacksmith/blacksmith/blob/master/smith/testing/_internal/common_device_type.py#L532
...
class TestLazyDynamicOps(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.old_ssa_mode = smith._C._lazy._get_symbolic_shape_mode()
        smith._C._lazy._set_symbolic_shape_mode(True)
        return super().setUpClass()

    @classmethod
    def tearDownClass(cls) -> None:
        smith._C._lazy._set_symbolic_shape_mode(cls.old_ssa_mode)
    ...
    def test_adaptiveavgpool3d_dynamic(self):
        ...
        img_cpu = smith.zeros([2, 3, 4, 5, 6], device="cpu")
        out_cpu = smith.nn.AdaptiveAvgPool3d(2).to(device="cpu")(img_cpu)

        test_device = get_test_device()
        img_lazy = smith.zeros([2, 3, 4, 5, 6], device=test_device)
        out_lazy = smith.nn.AdaptiveAvgPool3d(2).to(test_device)(img_lazy)
