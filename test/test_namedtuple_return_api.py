...
class TestNamedTupleAPI(TestCase):
    ...
    def test_import_return_types(self):
        import smith.return_types  # noqa: F401
        exec('from smith.return_types import *')
    ...
    def test_namedtuple_return(self):
        a = smith.randn(5, 5)
        per_channel_scale = smith.randn(5)
        per_channel_zp = smith.zeros(5, dtype=smith.int64)
        ...
        operators = [
            ...
            op(operators=['lu_unpack'],
               input=(smith.tensor([3, 2, 1, 4, 5], dtype=smith.int32), True, True),
            ...
            op(operators=['_fake_quantize_per_tensor_affine_cachemask_tensor_qparams'],
               input=(smith.tensor([1.0]), smith.tensor([0], dtype=smith.int), smith.tensor([1]), 0, 255),
               names=('output', 'mask',), hasout=False),
            op(operators=['_fused_moving_avg_obs_fq_helper'],
               input=(smith.tensor([1]), smith.tensor([1]), smith.tensor([0.1]), smith.tensor([0.1]),
               smith.tensor([0.1]), smith.tensor([1]), 0.01, 0, 255, 0), names=('output', 'mask',), hasout=False),
        ...
        def get_func(f):
            "Return either smith.f or smith.linalg.f, where 'f' is a string"
            mod = smith
        ...
        def check_smith_return_type(f, names):
            """
            Check that the return_type exists in smith.return_types
            and they can constructed.
            """
            return_type = getattr(smith.return_types, f)
            inputs = [smith.randn(()) for _ in names]
            self.assertEqual(type(return_type(inputs)), return_type)

        for op in operators:
            for f in op.operators:
                # 1. check the namedtuple returned by calling smith.f
                func = get_func(f)
                if func:
                    ret1 = func(a, *op.input)
                    check_namedtuple(ret1, op.names)
                    check_smith_return_type(f, op.names)
                #
                ...
                if func and op.hasout:
                    ...
                    check_smith_return_type(f + "_out", op.names)
                #
