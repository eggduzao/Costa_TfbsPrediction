...
def _construct_format_dict():
    dct = {
        "ModuleList": "smith.nn.modules.container.ModuleList",
        "AdaptiveAvgPool2d": "smith.nn.modules.pooling.AdaptiveAvgPool2d",
        "AdaptiveMaxPool2d": "smith.nn.modules.pooling.AdaptiveMaxPool2d",
        "Tensor": "smith._tensor.Tensor",
        "Adagrad": "smith.optim.adagrad.Adagrad",
        "Adam": "smith.optim.adam.Adam",
    }
    return dct
...
def _parse_reveals(file: IO[str]) -> list[str]:
    ...
    *e.g.* ``"{Tensor}"`` becomes ``"smith.tensor.Tensor"``.
    """
