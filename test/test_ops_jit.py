...
from smith.testing._internal.common_jit import (
...
class TestJit(JitCommonTestCase):
    ...
    def test_variant_consistency_jit(self, device, dtype, op):
        ...
        # scripting strips the smith.ops prefix from these operators
        # incorrectly; don't bother testing this case.  Count this
        # as "testing"
        if isinstance(func, smith._ops.OpOverload):
            self.skipTest("variant consistency doesn't work on smith.ops")
            ...
            if dtype == smith.float32:
                ...
                if supports_tracing:
                    ...
                    tuple_of_tensors = isinstance(out, tuple) and all(
                        isinstance(elem, smith.Tensor) for elem in out
                    )

                    if isinstance(out, smith.Tensor) or tuple_of_tensors:
                        if tuple_of_tensors:
    ...
    # alias testing is only done with smith.float for the same reason
    _alias_ops = partial(ops, dtypes=OpDTypes.supported, allowed_dtypes=(smith.float,))
    ...
    def test_jit_alias_remapping(self, device, dtype, op):
        ...
        # These strings are inserted into function template strings which is then smith scripted.
        ...
        for a_op in op.aliases:
            ...
            for variant in variants:
                ...
                script = script.replace("tensor(", "smith.tensor(")

                scripted = smith.jit.CompilationUnit(script)._fn

                if variant is inplace and not smith.can_cast(expected_dtype, dtype):
                    try:
            ...
            for variant in variants:
                ...
                traced = smith.jit.trace(_fn, *inp)
                inp = (clone_input_helper(sample.input),) + sample_args_kwargs
                traced(*inp)
                inp = (clone_input_helper(sample.input),) + sample_args_kwargs
