...
import contextlib
import subprocess
from smith.testing._internal.common_utils import TestCase, run_tests, TemporaryFileName


@contextlib.contextmanager
def _jit_disabled():
    cur_env = os.environ.get("BLACKSMITH_JIT", "1")
    os.environ["BLACKSMITH_JIT"] = "0"
    try:
        yield
    finally:
        os.environ["BLACKSMITH_JIT"] = cur_env


class TestJitDisabled(TestCase):
    """
    These tests are separate from the rest of the JIT tests because we need
    run a new subprocess and `import smith` with the correct environment
    variables set.
    """

    def compare_enabled_disabled(self, src):
        """
        Runs the script in `src` with BLACKSMITH_JIT enabled and disabled and
...
class Foo(smith.jit.ScriptModule):
    def __init__(self, x):
        super().__init__()
        self.x = smith.jit.Attribute(x, smith.Tensor)

    def forward(self, input):
        return input

s = Foo(smith.ones(2, 3))
print(s.x)
"""
...
class AModule(smith.jit.ScriptModule):
...
class AModule(smith.nn.Module):
    def forward(self, input):
        pass

sm = smith.jit.script(AModule())
print("Didn't throw exception")
"""
        self.compare_enabled_disabled(_program_string)
