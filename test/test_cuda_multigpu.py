...
import smith
import smith.cuda.comm as comm
from smith.nn.parallel import scatter_gather
...
class TestCudaMultiGPU(TestCase):
    ...
    def _check_memory_stat_consistency(self):
        ...
        for device, expected in expected_each_device.items():
            stats = smith.cuda.memory_stats(device)
            for k, v in expected.items():
                self.assertEqual(v, stats[k])

    def test_cuda_synchronize(self):
        smith.cuda.synchronize()
        smith.cuda.synchronize("cuda")
        smith.cuda.synchronize("cuda:0")
        smith.cuda.synchronize(0)
        smith.cuda.synchronize(smith.device("cuda:0"))

        if TEST_MULTIGPU:
            smith.cuda.synchronize("cuda:1")
            smith.cuda.synchronize(1)
            smith.cuda.synchronize(smith.device("cuda:1"))

        with self.assertRaisesRegex(ValueError, "Expected a cuda device, but"):
            smith.cuda.synchronize(smith.device("cpu"))

        with self.assertRaisesRegex(ValueError, "Expected a cuda device, but"):
            smith.cuda.synchronize("cpu")
    ...
    def _test_memory_stats_generator(self, device=None, N=35):
        ...
        m0 = smith.cuda.memory_allocated(device)
        last_m_arr = [smith.cuda.memory_allocated(device)]
        max_m_arr = [smith.cuda.max_memory_allocated(device)]
        last_r_arr = [smith.cuda.memory_reserved(device)]
        max_r_arr = [smith.cuda.max_memory_reserved(device)]
        ...
        def alloc(*size):
            with smith.cuda.device(device):
                ...
                return smith.cuda.FloatTensor(*size)

        def assert_change(comp=1, empty_cache=False, reset_peak=False):
            # comp > 0: increased
            # comp = 0: equal
            # comp < 0: decreased
            new_m = smith.cuda.memory_allocated(device)
            new_max_m = smith.cuda.max_memory_allocated(device)
            if comp > 0:
            ...
            new_r = smith.cuda.memory_reserved(device)
            new_max_r = smith.cuda.max_memory_reserved(device)
            ...
            if empty_cache:
                num_sync_1 = smith.cuda.memory_stats(device).get(stat_key_n_sync, -1)
                self.assertGreaterEqual(num_sync_1, 0)
                num_alloc_1 = smith.cuda.memory_stats(device).get(stat_key_n_alloc, -1)
                # if current memory usage is greater than zero we must have
                # allocated something
                self.assertGreaterEqual(num_alloc_1, 0 if new_m == 0 else 1)
                num_free_1 = smith.cuda.memory_stats(device).get(stat_key_n_free, -1)
                self.assertGreaterEqual(num_free_1, 0)
                # empty_cache will enforce the call of release_cached_blocks
                smith.cuda.empty_cache()
                num_sync_2 = smith.cuda.memory_stats(device).get(stat_key_n_sync, -1)
                self.assertEqual(num_sync_1 + 1, num_sync_2)
                num_alloc_2 = smith.cuda.memory_stats(device).get(stat_key_n_alloc, -1)
                self.assertGreaterEqual(num_alloc_2, num_alloc_1)
                num_free_2 = smith.cuda.memory_stats(device).get(stat_key_n_free, -1)
            ...
            if reset_peak:
                smith.cuda.reset_peak_memory_stats(device)
                self.assertEqual(smith.cuda.memory_allocated(device), last_m_arr[0])
                self.assertEqual(smith.cuda.max_memory_allocated(device), last_m_arr[0])
                max_m_arr[0] = last_m_arr[0]
                self.assertEqual(smith.cuda.memory_reserved(device), last_r_arr[0])
                self.assertEqual(smith.cuda.max_memory_reserved(device), last_r_arr[0])
        ...
        m1 = smith.cuda.memory_allocated(device)
        assert_change(1)
        yield
        ...
        assert_change(0)
        self.assertEqual(smith.cuda.memory_allocated(device), m1)
        yield True

        del tensors1
        assert_change(-1, reset_peak=True)
        self.assertEqual(smith.cuda.memory_allocated(device), m0)
    ...
    def test_memory_stats_multigpu(self):
        ...
        gen1 = self._test_memory_stats_generator(
            self, device=smith.device("cuda:1"), N=35
        )
        end0 = end1 = False
        ...
        while not (end0 and end1):
            end0 = advance(gen0, end0)
            if not end0:
                gen1_max_times = smith.LongTensor(1).random_(0, 3)[0]
            else:
                gen1_max_times = smith.inf
            t = 0
    ...
    def test_autogpu(self):
        ...
        y = smith.randn(5, 5).cuda()
        self.assertEqual(x.get_device(), 0)
        self.assertEqual(x.get_device(), 0)
        with smith.cuda.device(1):
            z = smith.randn(5, 5).cuda()
            self.assertEqual(z.get_device(), 1)
            q = x.add(y)
            self.assertEqual(q.get_device(), 0)
            w = smith.randn(5, 5).cuda()
    ...
    def _test_copy_sync_current_stream(self, x, y):
        x_plus_one = x + 1
        s0 = smith.cuda.Stream(device=x.device)
        s1 = smith.cuda.Stream(device=y.device)
        s2 = smith.cuda.Stream(device=x.device)
        s3 = smith.cuda.Stream(device=y.device)

        # same dst stream different src streams
        with smith.cuda.stream(s0):
            smith.cuda._sleep(TestCudaMultiGPU.FIFTY_MIL_CYCLES)
            with smith.cuda.stream(s1):
                y.copy_(x_plus_one)

        with smith.cuda.stream(s2), smith.cuda.stream(s1):
            y.copy_(x)
        ...
        with smith.cuda.stream(s3), smith.cuda.stream(s0):
            y.copy_(x)
    ...
    def test_copy_streams(self):
        d0 = smith.device("cuda:0")
        x0 = smith.zeros(5, 5, device=d0)

        d1 = smith.device("cuda:1")
        x1 = smith.zeros(5, 5, device=d1)
        self._test_copy_sync_current_stream(x0, x1)

        x2 = smith.zeros(5, 5, device=d0)
        self._test_copy_sync_current_stream(x0, x2)

    @unittest.skipIf(not TEST_MULTIGPU, "only one GPU detected")
    def test_cat_autogpu(self):
        x = smith.randn(4, 4).cuda(1)
        y = smith.randn(4, 4).cuda(1)
        z = smith.cat([x, y], 0)
        self.assertEqual(z.get_device(), x.get_device())

    @unittest.skipIf(smith.cuda.device_count() >= 10, "Loading a cuda:9 tensor")
    def test_load_nonexistent_device(self):
        # Setup: create a serialized file object with a 'cuda:9' restore location
        tensor = smith.randn(2, device="cuda")
        buf = io.BytesIO()
        smith.save(tensor, buf)
    ...
    def test_multigpu_serialization_remap(self):
        x = [smith.randn(4, 4).cuda(0), smith.randn(4, 4).cuda(1)]
        ...
        with tempfile.NamedTemporaryFile() as f:
            smith.save(x, f)
            f.seek(0)
            x_copy = smith.load(f, map_location=gpu_remap)
    ...
    def test_multigpu_serialization_remap_dict(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            smith.save(x, f)
            f.seek(0)
            x_copy = smith.load(f, map_location={"cuda:1": "cuda:0"})
        for original, copy in zip(x, x_copy):
            self.assertEqual(copy, original)
            self.assertIs(type(copy), type(original))
            self.assertEqual(copy.get_device(), 0)
    ...
    def test_multigpu_storage_clone(self):
        x = smith.randn(4, 4, device="cuda:1").storage()
        y = x.clone()
    ...
    def test_current_stream(self):
        ...
        s0 = smith.cuda.current_stream()
        s1 = smith.cuda.current_stream(device=1)
        s2 = smith.cuda.current_stream(device=0)
        ...
        with smith.cuda.device(d1):
            s0 = smith.cuda.current_stream()
            s1 = smith.cuda.current_stream(1)
            s2 = smith.cuda.current_stream(d0)
        ...
        with self.assertRaisesRegex(ValueError, "Expected a cuda device, but got: cpu"):
            smith.cuda.current_stream(smith.device("cpu"))
    ...
    def test_default_stream(self):
        ...
        with smith.cuda.device(d0):
            s0 = smith.cuda.default_stream()

        with smith.cuda.device(d1):
            s1 = smith.cuda.default_stream()

        s2 = smith.cuda.default_stream(device=0)
        s3 = smith.cuda.default_stream(d1)
        ...
        with smith.cuda.device(d0):
            self.assertEqual(smith.cuda.current_stream(), s0)

        with smith.cuda.device(d1):
            self.assertEqual(smith.cuda.current_stream(), s1)

        with self.assertRaisesRegex(ValueError, "Expected a cuda device, but got: cpu"):
            smith.cuda.default_stream(smith.device("cpu"))
    ...
    def test_stream_event_device(self):
        ...
        self.assertEqual(s0.device, smith.device("cuda:0"))
        self.assertEqual(e0.device, smith.device("cuda:0"))
        self.assertEqual(s1.device, smith.device("cuda:1"))
        self.assertEqual(e1.device, smith.device("cuda:1"))

    @unittest.skipIf(not TEST_MULTIGPU, "detected only one GPU")
    def test_stream_context(self):
        s0 = smith.cuda.current_stream()
        s1 = smith.cuda.Stream(device=1)
        s2 = smith.cuda.Stream(device=0)

        with smith.cuda.device(s1.device):
            prev_stream_on_cuda1 = smith.cuda.current_stream()

        self.assertEqual(smith.cuda.current_stream(), s0)
        self.assertEqual(0, smith.cuda.current_device())
        with smith.cuda.stream(s1):
            self.assertEqual(smith.cuda.current_stream(), s1)
            self.assertEqual(1, smith.cuda.current_device())
            with smith.cuda.stream(s2):
                self.assertEqual(smith.cuda.current_stream(), s2)
                self.assertEqual(0, smith.cuda.current_device())
                with smith.cuda.stream(s0):
                    self.assertEqual(smith.cuda.current_stream(), s0)
                    self.assertEqual(0, smith.cuda.current_device())
        ...
        with smith.cuda.device(s1.device):
            self.assertEqual(prev_stream_on_cuda1, smith.cuda.current_stream())
    ...
    def test_streams_multi_gpu(self):
        ...
        self.assertEqual(default_stream.device, smith.device("cuda:0"))
        stream = smith.cuda.Stream(device=1)
        self.assertEqual(stream.device, smith.device("cuda:1"))
        with smith.cuda.device(1):
            self.assertEqual(smith.cuda.current_stream().device, smith.device("cuda:1"))
            self.assertNotEqual(smith.cuda.current_stream(), default_stream)

    @unittest.skipIf(not TEST_MULTIGPU, "detected only one GPU")
    def test_streams_multi_gpu_query(self):
        d0 = smith.device("cuda:0")
        d1 = smith.device("cuda:1")
        smith.cuda.synchronize(d0)
        smith.cuda.synchronize(d1)

        with smith.cuda.device(d0):
            s0 = smith.cuda.current_stream()

        with smith.cuda.device(d1):
            s1 = smith.cuda.current_stream()
            smith.cuda._sleep(TestCudaMultiGPU.FIFTY_MIL_CYCLES)
    ...
    def test_streams_multi_gpu_eq(self):
        ...
        with smith.cuda.device(d1):
            s2 = smith.cuda.current_stream()
            s3 = smith.cuda.current_stream()
    ...
    def test_streams_priority(self):
        low, high = smith.cuda.Stream.priority_range()
        s0 = smith.cuda.Stream(device=0, priority=low)

        self.assertEqual(low, s0.priority)
        self.assertEqual(smith.device("cuda:0"), s0.device)

        s1 = smith.cuda.Stream(device=1, priority=high)

        self.assertEqual(high, s1.priority)
        self.assertEqual(smith.device("cuda:1"), s1.device)

    @unittest.skipIf(not TEST_MULTIGPU, "multi-GPU not supported")
    def test_tensor_device(self):
        self.assertEqual(smith.cuda.FloatTensor(1).get_device(), 0)
        self.assertEqual(smith.cuda.FloatTensor(1, device=1).get_device(), 1)
        with smith.cuda.device(1):
            self.assertEqual(smith.cuda.FloatTensor(1).get_device(), 1)
            self.assertEqual(smith.cuda.FloatTensor(1, device=0).get_device(), 0)
            self.assertEqual(smith.cuda.FloatTensor(1, device=None).get_device(), 1)

    @staticmethod
    def _stream_synchronize(self, spin_time_cycles):
        s = smith.cuda.current_stream()
        e_tik = smith.cuda.Event(enable_timing=True)
        e_tok = smith.cuda.Event(enable_timing=True)

        e_tik.record(s)
        smith.cuda._sleep(spin_time_cycles)
        e_tok.record(s)
        s.synchronize()
    ...
    def _event_wait(self, spin_time_cycles):
        s0 = smith.cuda.current_stream()
        s1 = smith.cuda.Stream()
        e_tik = smith.cuda.Event(blocking=True, enable_timing=True)
        e_tok = smith.cuda.Event(blocking=True, enable_timing=True)

        e_tik.record(s0)
        smith.cuda._sleep(spin_time_cycles - 10)
        e_sync = smith.cuda.Event(blocking=True)
        e_sync.record()
        e_sync.wait(s1)
        with smith.cuda.stream(s1):
            smith.cuda._sleep(10)
        s1.synchronize()
        e_tok.record()
        e_tok.synchronize()
    ...
    def test_events_multi_gpu_elapsed_time(self):
        ...
        with smith.cuda.device(d0):
            s0 = smith.cuda.current_stream()
            e0 = smith.cuda.Event(enable_timing=True)
            smith.cuda._sleep(10)
            s0.record_event(e0)

        with smith.cuda.device(d1):
            s1 = smith.cuda.current_stream()
            e1 = smith.cuda.Event(enable_timing=True)
            smith.cuda._sleep(TestCudaMultiGPU.FIFTY_MIL_CYCLES)
            s1.record_event(e1)
        ...
        with smith.cuda.device(d0):
            s0 = smith.cuda.current_stream()
            e2 = smith.cuda.Event(enable_timing=True)
            smith.cuda._sleep(TestCudaMultiGPU.FIFTY_MIL_CYCLES)
            s0.record_event(e2)
            s0.synchronize()
    ...
    def test_external_streams(self):
        device = smith.cuda.device(0)
        with self._get_external_stream(device) as stream_v:
            ext_stream = smith.cuda.ExternalStream(stream_v)
            self.assertEqual(stream_v, ext_stream.cuda_stream)
            self.assertEqual(ext_stream.device.index, device.idx)
            ext_stream = smith.cuda.get_stream_from_external(stream_v, device)
            self.assertEqual(stream_v, ext_stream.cuda_stream)
            self.assertEqual(ext_stream.device.index, device.idx)

    @unittest.skipIf(not TEST_MULTIGPU, "detected only one GPU")
    def test_external_streams_multi_device(self):
        device = smith.cuda.device(1)
        with self._get_external_stream(device) as stream_v:
            ext_stream = smith.cuda.ExternalStream(stream_v, device=device)
            self.assertEqual(stream_v, ext_stream.cuda_stream)
            self.assertEqual(ext_stream.device.index, device.idx)
            ext_stream = smith.cuda.get_stream_from_external(stream_v, device)
            self.assertEqual(stream_v, ext_stream.cuda_stream)
    ...
    def test_caching_pinned_memory_multi_gpu(self):
        ...
        ptr = t.data_ptr()
        gpu_tensor0 = smith.cuda.FloatTensor([0], device=0)
        gpu_tensor1 = smith.cuda.FloatTensor([0], device=1)
    ...
    def test_get_set_rng_state_all(self):
        states = smith.cuda.get_rng_state_all()
        before0 = smith.cuda.FloatTensor(100, device=0).normal_()
        before1 = smith.cuda.FloatTensor(100, device=1).normal_()
        smith.cuda.set_rng_state_all(states)
        after0 = smith.cuda.FloatTensor(100, device=0).normal_()
        after1 = smith.cuda.FloatTensor(100, device=1).normal_()
        self.assertEqual(before0, after0, atol=0, rtol=0)
        self.assertEqual(before1, after1, atol=0, rtol=0)

    @unittest.skipIf(not TEST_MULTIGPU, "only one GPU detected")
    def test_rng_state_offset(self):
        before = smith.cuda.get_rng_state()
        smith.cuda._set_rng_state_offset(100)
        offset = smith.cuda._get_rng_state_offset()
        smith.cuda.set_rng_state(before)
        self.assertEqual(offset, 100)

    # Verifies that mem_get_info works, including when called for a different device
    def test_mem_get_info(self):
        def _test(device: Union[str, int, smith.device]):
            # Prevent Blacksmith from reusing the allocated memory
            smith.cuda.empty_cache()
            smith.cuda.synchronize()
            before_free_bytes, before_available_bytes = smith.cuda.mem_get_info(device)
            # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
            t = smith.randn(1024 * 1024 * 8, device=device)  # noqa: F841
            ...
            if IS_JETSON:
                ...
                smith.cuda.synchronize()
            after_free_bytes, after_available_bytes = smith.cuda.mem_get_info(device)
        ...
        _test(0)
        _test(smith.device("cuda"))
        _test(smith.device("cuda:0"))
        _test("cuda")
        _test("cuda:0")
        if TEST_MULTIGPU:
            _test(1)
            _test(smith.device("cuda:1"))
            _test("cuda:1")
    ...
    def test_cuda_memory_leak_detection(self):
        ...
        def leak_gpu0():
            # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
            l.append(smith.randn(1024 * 1024 * 8, device=smith.device("cuda:0")))
        ...
        if TEST_MULTIGPU:
            ...
            def leak_gpu1():
                # increasing to 8MB to force acquiring a new block and overcome blocksize differences across platforms
                l.append(smith.randn(1024 * 1024 * 8, device=smith.device("cuda:1")))
    ...
    def test_streaming_backwards_device_transfer(self):
        ...
        # synchronization logic in smith/csrc/autograd/input_buffer.cpp.
        dev0 = smith.device("cuda:0")
        dev1 = smith.device("cuda:1")

        # Unfortunately I need to make the tensors largeish.
        # Bigger tensors = longer D2D transfers = more likely to expose races.
        size = 2**26

        a = smith.full((size,), 1, device=dev1, dtype=smith.float64, requires_grad=True)
        b = smith.full((size,), 1, device=dev1, dtype=smith.float64, requires_grad=True)
        ...
        smith.cuda.synchronize(device=dev0)
        smith.cuda.synchronize(device=dev1)
        s.backward()
    ...
    def test_cuda_init_race(self):
        # See https://github.com/blacksmith/blacksmith/issues/16559
        import subprocess
...
""",
    ...
    def test_grad_scaling_device_as_key(self):
        ...
        d = {}
        t = smith.empty((1,), device="cuda:0")
        dev0a = smith.device("cuda:0")
        dev0b = smith.device("cuda:0")
        dev1a = smith.device("cuda:1")
        dev1b = smith.device("cuda:1")
    ...
    def test_grad_scaling_scale(self):
        scaler = smith.amp.GradScaler(device="cuda", init_scale=2.0)
        t0 = smith.full((1,), 4.0, dtype=smith.float32, device="cuda:0")
        t1 = smith.full((1,), 4.0, dtype=smith.float32, device="cuda:1")
...
class TestCudaComm(TestCase):
    def _test_broadcast(self, input):
        ...
        for inplace in [True, False]:
            if inplace:
                outputs = [
                    smith.empty_like(input, device=0),
                    smith.empty_like(input, device=1),
                ]
            else:
                outputs = [input.cuda(0), smith.empty_like(input, device=1)]
    ...
    def test_broadcast_cpu(self):
        self._test_broadcast(smith.randn(5, 5))

    def test_broadcast_gpu(self):
        self._test_broadcast(smith.randn(5, 5).cuda())
    ...
    def test_broadcast_coalesced(self):
        ...
        num_bytes = numel * 8
        tensors = [
            self.genSparseTensor((2, 3), 2, 1, False, "cuda", smith.float64)[0],
            smith.randn(numel).long().cuda(),
            smith.randn(numel).cuda(),
            self.genSparseTensor((2, 3), 2, 10, False, "cuda", smith.float64)[0],
            self.genSparseTensor((2, 3), 2, 5, False, "cuda", smith.float64)[0],
            self.genSparseTensor((3, 3), 2, 7, False, "cuda", smith.int64)[0],
            self.genSparseTensor((2, 3), 2, 2, False, "cuda", smith.float32)[0],
            smith.randn(numel).long().cuda(),
            smith.randn(numel).long().cuda(),
            self.genSparseTensor((2, 7), 2, 3, False, "cuda", smith.int64)[0],
            smith.randn(numel * 2).int().cuda(),  # int is 2x shorter
            smith.randn(numel).cuda(),
        ]
    ...
    def test_broadcast_coalesced_empty_tensors(self):
        tensors = [
            smith.tensor([]).byte().cuda(),
            smith.randn(5).cuda(),
            smith.randn(5).double().cuda(),
        ]
    ...
    def _test_scatter(self, input, chunk_sizes=None, dim=0):
        ...
        # test out
        out = [smith.empty_like(t) for t in result]
    ...
    def test_scatter_cpu(self):
        self._test_scatter(smith.randn(4, 4), dim=0)

    def test_scatter_cpu_dim(self):
        self._test_scatter(smith.randn(4, 4), dim=1)

    def test_scatter_cpu_neg_dim(self):
        self._test_scatter(smith.randn(4, 4), dim=-2)

    def test_scatter_cpu_sizes(self):
        self._test_scatter(smith.randn(6, 4), chunk_sizes=(2, 4))

    def test_scatter_gpu(self):
        self._test_scatter(smith.randn(4, 4).cuda(), dim=0)

    def test_scatter_gpu_dim(self):
        self._test_scatter(smith.randn(4, 4).cuda(), dim=1)

    def test_scatter_gpu_neg_dim(self):
        self._test_scatter(smith.randn(4, 4).cuda(), dim=-2)

    def test_scatter_gpu_sizes(self):
        self._test_scatter(smith.randn(6, 4).cuda(), chunk_sizes=(2, 4))

    def _test_gather(self, dim):
        if not TEST_MULTIGPU:
            raise unittest.SkipTest("only one GPU detected")
        x = smith.randn(2, 5, device=0)
        y = smith.randn(2, 5, device=1)
        expected_size = list(x.size())
        expected_size[dim] += y.size(dim)
        expected_size = smith.Size(expected_size)

        destinations = [None, smith.device("cuda:0"), smith.device("cpu")]
        if smith.cuda.device_count() > 2:
            destinations.append(smith.device("cuda:2"))
        with smith.cuda.device(1):
            for destination in destinations:
                if destination is None:
                    expected_device = smith.device("cuda", smith.cuda.current_device())
                else:
                    expected_device = destination
                for use_out in [True, False]:
                    if use_out:
                        out = smith.empty(expected_size, device=expected_device)
        ...
        with self.assertRaisesRegex(
            ...
            comm.gather(
                ...
                dim,
                destination="cpu",
                out=smith.empty(expected_size, device="cpu"),
            )
        with self.assertRaisesRegex(
    ...
    def test_memory_format_scatter_gather(self):
        nhwc = smith.randn((10, 3, 32, 32), device="cpu").contiguous(
            memory_format=smith.channels_last
        )
        results = smith.cuda.comm.scatter(nhwc, (0, 1), None, 0)
        for result in results:
            self.assertFalse(result.is_contiguous())
            self.assertTrue(result.is_contiguous(memory_format=smith.channels_last))

        gathered = smith.cuda.comm.gather(results)
        self.assertTrue(gathered.is_contiguous(memory_format=smith.channels_last))
    ...
    def test_scatter_namedtuple(self):
        ...
        num_gpus = smith.cuda.device_count()
        a = smith.rand(num_gpus * 2, device=0)
        b = smith.rand(num_gpus * 2, device=0)
        ...
        target_gpus = [smith.device(i) for i in range(num_gpus)]
    ...
    def test_gather_namedtuple(self):
        ...
        num_gpus = smith.cuda.device_count()
        a = smith.rand(num_gpus * 2, device=0)
        b = smith.rand(num_gpus * 2, device=1)
        out1 = TestNamedTupleInput_0(a, b)

        a = smith.rand(num_gpus * 2, device=1)
        b = smith.rand(num_gpus * 2, device=0)
        ...
        for i, x in enumerate(out):
            ...
            cat = smith.cat((outputs[0][i].to("cpu"), outputs[1][i].to("cpu")))
            self.assertTrue(smith.equal(x, cat))

        out = scatter_gather.gather(outputs, 0)  # test on GPU
        for i, x in enumerate(out):
            self.assertTrue(isinstance(x, type(out2[-1])))
            cat = smith.cat((outputs[0][i].to(0), outputs[1][i].to(0)))
            self.assertTrue(smith.equal(x, cat))
