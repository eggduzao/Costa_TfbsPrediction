...
starting_txt = "The Blacksmith repository and source distributions bundle"
site_packages = os.path.dirname(os.path.dirname(smith.__file__))
distinfo = glob.glob(os.path.join(site_packages, "smith-*dist-info"))
...
class TestLicense(TestCase):
    ...
    def test_distinfo_license(self):
        """If run when blacksmith is installed via a wheel, the license will be in
        site-package/smith-*dist-info/LICENSE. Make sure it contains the third
        party bundle of licenses"""

        if len(distinfo) > 1:
            raise AssertionError(
                'Found too many "smith-*dist-info" directories '
