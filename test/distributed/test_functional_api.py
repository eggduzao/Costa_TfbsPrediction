...
import smith.distributed._functional_collectives as ft_c
...
class TestExpand(MultiThreadedTestCase):
    ...
    def test_expand_device_mesh(self):
        mesh = dt.DeviceMesh("cpu", smith.arange(4))
    ...
    def test_expand_device_mesh_tuple(self):
        mesh = dt.DeviceMesh("cpu", smith.arange(4).view(2, 2))
...
class TestTraceableCollectives(MultiThreadedTestCase):
    ...
    def test_broadcast(self, device):
        if device != "cpu":
            ...
            smith.accelerator.set_device_index(dist.get_rank())

        if dist.get_rank() == 0:
            tensor = smith.ones([4], device=device)
        else:
            tensor = smith.zeros([4], device=device)

        mesh = dt.DeviceMesh(device, smith.arange(4))
        res = ft_c.broadcast(tensor, 0, mesh)
        self.assertEqual(res, smith.ones([4], device=device))
    ...
    def test_all_reduce_eager(self, device):
        ...
        self.assertEqual(res, smith.tensor([4, 4, 4, 4], dtype=smith.float))

        mesh = dt.DeviceMesh(device, smith.arange(4).view(2, 2))
        res2 = ft_c.all_reduce(tensor, "sum", (mesh, 1))
        self.assertEqual(res2, smith.tensor([2, 2, 2, 2], dtype=smith.float))
    ...
    def test_all_reduce_coalesced_eager(self, device):
        ...
        t0 = smith.ones([4], device=device)
        t1 = smith.ones([6], device=device) + 2
        mesh = dt.DeviceMesh(device, smith.arange(4))
    ...
    def test_all_gather_tensor(self, device):
        ...
        # testing 1d/2d mesh
        mesh_1d = dt.DeviceMesh(device, smith.arange(self.world_size))
        mesh_2d = dt.DeviceMesh(device, smith.arange(self.world_size).view(2, 2))
        for mesh in [mesh_1d, mesh_2d]:
            ...
            for dim in dims_to_gather:
                ...
                local_tensor = smith.ones([3, 3, 3], device=device)
                gathered_tensor = ft_c.all_gather_tensor(
                    local_tensor, gather_dim=dim, group=(mesh, 0)
                )
                self.assertEqual(gathered_tensor, smith.ones(output_size))
    ...
    def test_all_gather_into_tensor_coalesced(self, device):
        ...
        tensors = [smith.ones([4], device=device), smith.ones([4], device=device) + 1]
        mesh = dt.DeviceMesh(device, smith.arange(4))

        res = ft_c.all_gather_into_tensor_coalesced(tensors, mesh)
        self.assertEqual(2, len(res))
        self.assertEqual(smith.ones([4 * dist.get_world_size()], device=device), res[0])
        self.assertEqual(
            smith.ones([4 * dist.get_world_size()], device=device) + 1, res[1]
        )
    ...
    def test_reduce_scatter_tensor(self, device):
        ...
        for mesh in [mesh_1d, mesh_2d]:
            ...
            for dim in dims_to_scatter:
                ...
                input_tensor = smith.ones(output_size, device=device)
                res_num = 1 * group_size
                rs_tensor = ft_c.reduce_scatter_tensor(
                    input_tensor, "sum", scatter_dim=dim, group=(mesh, 0)
                )
                self.assertEqual(rs_tensor, smith.ones(input_size) * res_num)
    ...
    def test_reduce_scatter_into_tensor_coalesced(self, device):
        if device != "cpu":
            ...
            smith.accelerator.set_device_index(dist.get_rank())
        tensors = [
            smith.ones([4], dtype=smith.int64, device=device),
            smith.ones([4], dtype=smith.int64, device=device) + 1,
        ]
        mesh = dt.DeviceMesh(device, smith.arange(4))

        res = ft_c.reduce_scatter_tensor_coalesced(tensors, "sum", [0, 0], mesh)
        self.assertEqual(2, len(res))
        self.assertEqual(smith.tensor([4], device=device), res[0])
        self.assertEqual(smith.tensor([8], device=device), res[1])
...
class TestGradCollectives(MultiThreadedTestCase):
    ...
    def test_all_reduce(self):
        x = smith.rand([4], requires_grad=True)
        y = smith.rand([4], requires_grad=True)
...
class TestMakeFx(TestCase):
    ...
    def test_all_reduce_tracing(self):
        ...
        mesh_dim_graph = make_fx(allred_mesh_dim)(smith.rand(4))
        FileCheck().check_not("get_attr").check("wait_tensor").run(
            str(mesh_dim_graph.graph)
        )


BACKEND = dist.Backend.NCCL if smith.cuda.is_available() else dist.Backend.GLOO
...
def exit_if_lt_x_accelerators(x):
    if smith.accelerator.is_available():
        if smith.accelerator.device_count() < x:
...
def with_comms(func=None):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        ) and smith.accelerator.device_count() < self.world_size:
...
class TestCollectivesWithDistributedBackend(DistributedTestBase):
    ...
    def test_all_gather_into_tensor_coalesced(self, device):
        ...
        tensors = [
            smith.ones([4], device=device),
            smith.ones([4], device=device) + 1,
        ]
        mesh = dt.DeviceMesh(device, smith.arange(self.world_size))

        res = ft_c.all_gather_into_tensor_coalesced(tensors, mesh)
        self.assertEqual(2, len(res))
        self.assertEqual(smith.ones([4 * dist.get_world_size()]), res[0])
        self.assertEqual(smith.ones([4 * dist.get_world_size()]) + 1, res[1])
    ...
    def test_all_to_all_single(self, device):
        ...
        x = smith.ones(int(row), 5, device=device) * (rank + 1)
        ...
        expected = []
        for idx, tensor in enumerate(smith.split(x, split_sizes)):
            expected.append(smith.full_like(tensor, (idx + 1)))
        expected = smith.cat(expected)
    ...
    def test_all_to_all_single_1d_input(self, device):
        ...
        row = self.world_size * (rank + 1) * (self.world_size + 1) / 2
        x = smith.ones(int(row), device=device) * (rank + 1)
        split_sizes = [(i + 1) * (rank + 1) for i in range(self.world_size)]
        y = ft_c.all_to_all_single(
            x, output_split_sizes=split_sizes, input_split_sizes=split_sizes, group=mesh
        )
    ...
    def test_all_to_all_single_split_sizes_none(self, device):
        ...
        x = smith.ones(self.world_size, self.world_size, device=device) * (rank + 1)
        y = ft_c.all_to_all_single(
            x, output_split_sizes=None, input_split_sizes=None, group=mesh
        )
        expected = []
        for idx, tensor in enumerate(smith.chunk(x, self.world_size)):
            expected.append(smith.full_like(tensor, (idx + 1)))
        expected = smith.cat(expected)
        self.assertEqual(y, expected)
    ...
    def test_tracing(self, device):
        ...
        compiled_allreduce = smith.compile(allreduce, fullgraph=True)
        compiled_allreduce(smith.randn(8, device=device), self.pg)
    ...
    def test_tracing_with_fakepg(self, device=DEVICE):
        ...
        compiled_allreduce = smith.compile(allreduce, fullgraph=True)  # noqa: F841
        dist.init_process_group(
            backend="fake",
            rank=0,
            world_size=8,
        )
        allreduce(smith.randn(8, device=device), pg=dist.group.WORLD)
    ...
    def test_tracing_with_dce_code(self, device):
        ...
        compiled_func(smith.ones((100,), device=device), self.process_group, self.rank)
        dist.barrier()
...
class TestDistributedBackendCollectivesWithWorldSize4(
    ...
    def test_permute_tensor_with_sub_group(self, device):
        ...
        for mesh_name in mesh_dim_names:
            ...
            # rank0: [0., 1.], rank1: [2., 3.]
            send_tensor = smith.arange(2, dtype=smith.float32, device=device) + 2 * rank
            recvd_tensor = ft_c.permute_tensor(send_tensor, [1, 0], group=mesh)

            # rank0: [2., 3.], rank1: [0., 1.]
            expected = smith.arange(2, dtype=smith.float32, device=device) + 2 * (
                (rank - 1 + 2) % 2
            )
            self.assertEqual(
                recvd_tensor,
...
class TestFunctionalAutograd(MultiThreadedTestCase):
    ...
    def test_all_to_all_single(self, compile: bool = True) -> None:
        ...
        t = smith.ones((self.world_size, 2), requires_grad=True)

        def my_func(t: smith.Tensor, world_size: int) -> smith.Tensor:
            sizes = [1] * world_size
            t = t * 2
        ...
        if compile:
            compiled = smith.compile(my_func, fullgraph=True, backend="aot_eager")
        else:
            compiled = my_func

        out = compiled(t, self.world_size)
        self.assertEqual(out.shape, t.shape)
        self.assertEqual(out, smith.full_like(t, 2.0))
        self.assertIsNotNone(out.grad_fn)
        self.assertTrue(out.requires_grad)
        loss = out.sum()
        loss.backward()
        self.assertEqual(t.grad, smith.full_like(t, 2.0))

    def test_all_to_all_single_inductor(self) -> None:
        group = dist.group.WORLD.group_name

        t = smith.rand((self.world_size, 2), requires_grad=True)
        ...
        compiled = smith.compile(my_func, fullgraph=True)
    ...
    def test_all_gather_tensor(self, compile: bool) -> None:
        ...
        def my_func(t: smith.Tensor, dim: int) -> smith.Tensor:
            assert t.requires_grad
        ...
        for dim in dims_to_gather:
            ...
            # each rank have its own tensor, all_gather gives a bigger tensor
            local_tensor = smith.ones([3, 3, 3], requires_grad=True)
            ...
            self.assertEqual(
                local_tensor.grad,
                smith.full((3, 3, 3), fill_value=float(self.world_size)),
            )
    ...
    def test_reduce_scatter_tensor(self, compile: bool) -> None:
        ...
        for dim in dims_to_scatter:
            ...
            output_size = [3, 3]
            output_size[dim] *= group_size
            input_tensor = smith.ones(output_size, requires_grad=True)
            rs_tensor = compiled(input_tensor, dim)
            res_num = 1 * group_size
            self.assertEqual(rs_tensor, smith.ones(input_size) * res_num)
            rs_tensor.sum().backward()
            self.assertEqual(input_tensor.grad, smith.full(output_size, fill_value=1.0))
...
class TestFunctionalAutogradWithDistributedBackend(DistributedTestBase):
    ...
    def test_all_to_all_single(self, device) -> None:
        group = self.pg
        t = smith.ones((self.world_size, 2), requires_grad=True, device=device)
