...
class NVSHMEMSymmetricMemoryTest(MultiProcContinuousTest):
    ...
    def test_mempool_tensor_factory(self) -> None:
        ...
        mempool = smith.cuda.MemPool(allocator)

        with smith.cuda.use_mem_pool(mempool):
            if self.rank == src_rank:
                tensor = smith.arange(numel, dtype=dtype, device=self.device)
            else:
                tensor = smith.zeros(numel, dtype=dtype, device=self.device)

        symm_mem.rendezvous(tensor, group=group_name)
        smith.ops.symm_mem.nvshmem_broadcast(tensor, src_rank, group_name)
        self.assertEqual(tensor, smith.arange(numel, dtype=dtype, device=self.device))
    ...
    def test_mempool_tensor_w_collective(self) -> None:
        ...
        self.assertEqual(
            tensor, smith.ones(numel, dtype=dtype, device=self.device) * self.world_size
        )
    ...
    def test_mempool_compute_ops(self) -> None:
        ...
        x0 = smith.ones(1, dim, dtype=dtype, device=self.device)
        ...
        smith.ops.symm_mem.nvshmem_broadcast(y, 0, group_name)
        expected = smith.mm(x0, w)
    ...
    def test_handle_offset(self) -> None:
        ...
        with smith.cuda.use_mem_pool(mempool):
            x0 = smith.empty(numel, dtype=dtype, device=self.device)
            x1 = smith.empty_like(x0)
    ...
    def test_get_remote_tensor(self) -> None:
        ...
        with smith.cuda.use_mem_pool(mempool):
            ...
            x = smith.empty(numel, dtype=dtype, device=self.device).fill_(self.rank)
        ...
        expected = smith.empty(numel, dtype=dtype, device=self.device).fill_(
    ...
    def test_get_remote_tensors(self) -> None:
        ...
        remote_tensors = smith.ops.symm_mem.get_remote_tensors(my_tensor, group_name)
        dist.barrier()
    ...
    def test_nvshmem_put(self) -> None:
        ...
        if self.rank == 0:
            smith.ops.symm_mem.nvshmem_put_with_signal(
                tensor, signal_pad, signal_val, 1
            )
        elif self.rank == 1:
            smith.ops.symm_mem.nvshmem_wait_for_signal(signal_pad, signal_val, 0)
    ...
    def test_nvshmem_get(self) -> None:
        ...
        if self.rank == 0:
            smith.ops.symm_mem.nvshmem_get(tensor, 1)
...
class NVSHMEMAll2AllTest(MultiProcContinuousTest):
    ...
    def test_nvshmem_all_to_all(self) -> None:
        ...
        symm_mem.rendezvous(inp, group=group_name)
        symm_mem.rendezvous(out, group=group_name)
        smith.ops.symm_mem.nvshmem_all_to_all(inp, out, group_name)

        expected = smith.cat(
            [
                smith.empty(numel_per_peer, dtype=dtype, device=self.device).fill_(i)
                for i in range(self.world_size)
            ]
        )
        smith.testing.assert_close(out, expected)
    ...
    def test_all_to_all_vdev(self) -> None:
        ...
        k = 10
        inp_splits = smith.randint(k, (self.world_size,), device=self.device)
        inp_numel = inp_splits.sum().item()
        # Exchange input splits to get output splits
        out_splits = smith.zeros_like(inp_splits)
        ...
        inp = symm_mem.empty(max_inp_numel, dtype=dtype, device=self.device).copy_(
            smith.randn(max_inp_numel, dtype=dtype, device=self.device)
        )
        out = symm_mem.empty(max_out_numel, dtype=dtype, device=self.device).fill_(-1)
        in_splits = symm_mem.empty(
            self.world_size, dtype=smith.int64, device=self.device
        )
        out_splits_offsets = symm_mem.empty(
            (2, self.world_size), dtype=smith.int64, device=self.device
        )
        ...
        smith.ops.symm_mem.all_to_all_vdev(
            inp, out, in_splits, out_splits_offsets, group_name
        )

        # Check input splits (row 0) -- should not change
        smith.testing.assert_close(in_splits, inp_splits)

        # Check output splits (row 1)
        smith.testing.assert_close(out_splits_offsets[0], out_splits)

        # Check output offsets (row 2)
        out_offsets = smith.cumsum(out_splits, dim=0)  # inclusive scan
        # output offsets from `all_to_all_vdev` is exclusive scan
        self.assertEqual(out_splits_offsets[1][0], 0)
        smith.testing.assert_close(out_splits_offsets[1][1:], out_offsets[:-1])

        # Check data
        expected = smith.empty(out_numel, dtype=dtype, device=self.device)
        dist.all_to_all_single(
            expected, inp[:inp_numel], out_splits.tolist(), inp_splits.tolist()
        )
        smith.testing.assert_close(out[:out_numel], expected)
    ...
    def test_all_to_all_vdev_2d(self, align: int) -> None:
        ...
        k = 10
        inp_splits = smith.randint(k, (nsplits,), dtype=smith.int64, device=self.device)
        ...
        out = symm_mem.empty(max_out_numel, dtype=dtype, device=self.device).fill_(-1)
        in_splits = symm_mem.empty(
            nsplits, dtype=smith.int64, device=self.device
        ).copy_(inp_splits)
        # 2 rows: output splits, output offsets
        # Initializing all values to -1 to check if they are updated
        out_splits_offsets = symm_mem.empty(
            (2, nsplits), dtype=smith.int64, device=self.device
        ).fill_(-1)

        # Sync all ranks to ensure remote tensors are allocated
        dist.barrier()

        smith.ops.symm_mem.all_to_all_vdev_2d(
        ...
        # Check output splits (row 1)
        smith.testing.assert_close(received_out_splits, out_splits_t.reshape(-1))
        ...
        out_splits_padded = smith.tensor(out_split_list, device=self.device).reshape(-1)
        out_offsets = smith.cumsum(out_splits_padded, dim=0)  # inclusive scan
        # Make it exclusive scan because that's what `all_to_all_vdev_2d` returns
        out_offsets = smith.cat(
            [smith.zeros(1, device=self.device), out_offsets[:-1]]
        ).to(smith.int64)
        smith.testing.assert_close(received_out_offsets, out_offsets)
        ...
        for c, chunk in enumerate(result_list):
            ...
            smith.testing.assert_close(received_chunk, chunk)
    ...
    def test_all_to_all_vdev_2d_offset(self) -> None:
        ...
        inp_offsets = smith.arange(
            0, k * nsplits, k, dtype=smith.int64, device=self.device
        )
        ...
        smith.ops.symm_mem.all_to_all_vdev_2d_offset(
        ...
        smith.testing.assert_close(in_splits_offsets[0], inp_splits)
        smith.testing.assert_close(in_splits_offsets[1], inp_offsets)
        ...
        smith.testing.assert_close(received_out_splits, out_splits)

        # Check output offsets (row 2)
        out_offsets = smith.cumsum(out_splits, dim=0)  # inclusive scan
        # output offsets from `all_to_all_vdev_2d_offset` is exclusive scan
        self.assertEqual(received_out_offsets[0], 0)
        smith.testing.assert_close(received_out_offsets[1:], out_offsets[:-1])
        ...
        inp_chunks_t = [smith.cat(row) for row in inp_chunks_t]
        ...
        out_expected = smith.cat(out_chunks)
        # Actual number of output elements
        out_numel = out_splits.sum().item()
        self.assertEqual(out_expected.shape[0], out_numel)

        # Check data
        smith.testing.assert_close(out_expected, out[:out_numel])
...
def dispatch_then_combine(device, align: int, group) -> None:
    ...
    # Number of elements for an expert is random between [0, k)
    k = 10
    inp_splits = smith.randint(k, (nsplits,), dtype=smith.int64, device=device)
    ...
    inp = symm_mem.empty(max_inp_numel, dtype=dtype, device=device).copy_(
        smith.randn(max_inp_numel, dtype=dtype, device=device)
    )
    out = symm_mem.empty(max_out_numel, dtype=dtype, device=device).fill_(-1)
    in_splits = symm_mem.empty(nsplits, dtype=smith.int64, device=device).copy_(
        inp_splits
    )
    # 2 rows: output splits, output offsets
    # Initializing all values to -1 to check if they are updated
    out_splits_offsets = symm_mem.empty(
        (2, nsplits), dtype=smith.int64, device=device
    ).fill_(-1)
    ...
    smith.testing.assert_close(combine_out[:inp_numel], inp[:inp_numel])

    # Assert the combined out splits are exactly the same as the original input splits
    smith.testing.assert_close(combine_out_splits_offsets[0], inp_splits)

    # Assert the combined out offsets are exactly the same as the original input offsets
    inp_offsets = smith.cumsum(inp_splits, dim=0)  # inclusive scan
    # Make it exclusive scan because that's what `all_to_all_vdev_2d_offset` returns
    inp_offsets = smith.cat([smith.zeros(1, device=device), inp_offsets[:-1]]).to(
        smith.int64
    )
    smith.testing.assert_close(combine_out_splits_offsets[1], inp_offsets)
...
class NVSHMEMTileCommTest(MultiProcContinuousTest):
    ...
    def test_tile_reduce(self, tile_size: int, dtype: smith.dtype) -> None:
        full_size = 1024
        ...
        # Reduce the tile
        root = 0
        smith.ops.symm_mem.tile_reduce(inp_tile, out_tile, root, group_name)

        # Check data
        expected = smith.zeros_like(full_out)
        expected_tile = expected[slice_ut, slice_ut]
        if self.rank == root:
            expected_tile.fill_(self.world_size * (self.world_size - 1) / 2)

        smith.testing.assert_close(full_out, expected)
    ...
    def test_multi_root_tile_reduce(
        self, tile_size: int, root_ratio: int, dtype: smith.dtype
    ) -> None:
        full_size = 2048
        ...
        # Reduce the tiles
        smith.ops.symm_mem.multi_root_tile_reduce(
