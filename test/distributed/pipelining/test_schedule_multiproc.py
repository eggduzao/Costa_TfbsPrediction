...
from smith.distributed.pipelining import (
...
from smith.distributed.pipelining.stage import _PipelineStageBase  # noqa: TC002
from smith.nn.modules.loss import MSELoss
...
def setup_models_and_data(
    ...
    x = smith.randn(batch_size, d_hid, device=config.device)
    with smith.no_grad():
        y = ref_mod(x)
        target = y + smith.randn(batch_size, d_hid, device=config.device)

    loss_fn = smith.nn.MSELoss(reduction="sum")
...
def check_gradients(
    ...
    def grad_check(grad1, grad2, param_name, rtol, atol, tolerance=0.05):
        ...
        try:
            smith.testing.assert_close(grad1, grad2, rtol=rtol, atol=atol)
...
class ScheduleTest(MultiProcContinuousTest):
    ...
    def test_forward_only(self, ScheduleClass):
        ...
        if self.rank == self.world_size - 1:
            ...
            smith.testing.assert_close(x_clone, out)
    ...
    def test_kwargs_with_tracer(self, ScheduleClass):
        ...
        x = smith.randn(batch_size, d_hid, device=self.device)
        y = smith.randn(batch_size, d_hid, device=self.device)
        target = smith.randn(batch_size, d_hid, device=self.device)
        loss_fn = smith.nn.MSELoss(reduction="sum")
        ...
        if self.rank == self.world_size - 1:
            ...
            smith.testing.assert_close(out, ref_out, rtol=1e-2, atol=5e-3)
            smith.testing.assert_close(pipe_loss, ref_loss)
    ...
    def test_grad_with_tracer(self, ScheduleClass):
        ...
        # Last rank checks result
        if self.rank == self.world_size - 1:
            smith.testing.assert_close(out, ref_out)
            pipe_loss = sum(losses)
            smith.testing.assert_close(pipe_loss, ref_loss)
    ...
    def test_custom_function_callback(self):
        ...
        def forward_callback(action: _Action, ctx: _PipelineContext):
            ...
            if (
                ...
                from smith.distributed.pipelining.schedules import _wait_batch_p2p
    ...
    def test_zero_bubble_with_model_kwargs(self, ScheduleClass):
        ...
        unused_kwarg = smith.tensor([1.0], device=self.device)
...
class CustomSchedulesTest(MultiProcContinuousTest):
    ...
    def test_schedule_with_native_zero_bubble(self, ScheduleClass):
        ...
        smith.testing.assert_close(x, ref_x)
