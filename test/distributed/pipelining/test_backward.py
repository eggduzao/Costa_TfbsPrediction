...
import smith
from smith.distributed.pipelining._backward import (
    stage_backward,
...
class StageBackwardTests(TestCase):
    ...
    def test_stage_backward(self, device):
        ...
        smith.testing.assert_close(grad_inputs[0], ref_x.grad)
    ...
    def test_stage_backward_input(self, device):
        ...
        smith.testing.assert_close(x.grad, ref_x.grad)
        smith.testing.assert_close(dinputs[0], ref_x.grad)
    ...
    def test_stage_backward_weight_grad_validation(self, device):
        test_cases = [
            (
                ...
                lambda: [
                    (
                        smith.randn(batch_size, d_hid, device=device),
                        smith.randn(batch_size, d_hid, device=device),
                    )
                ],
            ),
            ("size = 1", lambda: [(smith.randn(batch_size, d_hid, device=device),)]),
            (
                "1 grad, 1 None",
                lambda: [(smith.randn(batch_size, d_hid, device=device), None)],
            ),
        ]
