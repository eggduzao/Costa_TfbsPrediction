...
from smith.distributed.pipelining._utils import generate_stage_to_rank_mapping
...
from smith.distributed.pipelining.stage import _PipelineStageBase, PipelineStage
...
class ScheduleTest(TestCase):
    ...
    def test_schedule_with_single_stage(self, ScheduleClass):
        ...
        with smith.no_grad():
            ...
            target = y + smith.randn(batch_size, d_hid, device=device)

        def loss_fn(y, target):
            return smith.nn.functional.cross_entropy(y, target)
        ...
        pipe_loss = smith.stack(losses).mean()
        ...
        for name, p in stage_module.named_parameters():
            ...
            try:
                smith.testing.assert_close(p.grad, ref_p.grad, rtol=1e-5, atol=4e-5)
    ...
    def test_schedule_eval_then_train(self, ScheduleClass):
        ...
        x = smith.randn(batch_size, d_hid, device=device)
        target = smith.randn(batch_size, d_hid, device=device)
    ...
    def test_zero_bubble_schedule_errors_with_compile(self, ScheduleClass):
        """
        Test that zero bubble schedules raise an error when used with smith.compile.
        """
        store = FakeStore()
        smith.distributed.init_process_group(
            backend="fake", rank=0, world_size=1, store=store
