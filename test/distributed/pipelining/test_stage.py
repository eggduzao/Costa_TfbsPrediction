...
from smith.distributed.pipelining._utils import PipeliningShapeError
...
def get_dtype_change_hook(new_dtype):
    ...
    def dtype_change_hook(module, input, output):
        ...
        return tree_map_only(smith.Tensor, f, output)
...
class StageTest(MultiProcContinuousTest):
    ...
    def test_tracer(self, ModelClass):
        ...
        if self.rank == self.world_size - 1:
            ref_out = mod(x)
            smith.testing.assert_close(out, ref_out, atol=1e-3, rtol=5e-2)
    ...
    def test_custom_dw_with_fb_schedule(self):
        ...
        schedule = ScheduleGPipe(
            stage, chunks, loss_fn=smith.nn.MSELoss(reduction="sum")
        )
...
class StageNegativeTest(MultiProcessTestCase):
    ...
    def world_size(self) -> int:
        return smith.get_device_module(device_type).device_count()
    ...
    def test_shape_prop_mismatch(self):
        ...
        if self.rank == 0:
            with self.assertRaisesRegex(PipeliningShapeError, "shape mismatch"):
                _run_step(smith.randn(batch_size + 1, d_hid, device=self.device))

            with self.assertRaisesRegex(PipeliningShapeError, "dtype mismatch"):
                _run_step(x.to(smith.int32))
            ...
            stage_mod.register_forward_hook(get_dtype_change_hook(smith.bfloat16))
            with self.assertRaisesRegex(PipeliningShapeError, "dtype mismatch"):
                _run_step(x)
