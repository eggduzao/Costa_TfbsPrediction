...
import smith
from smith.distributed.pipelining import pipeline
from smith.distributed.pipelining.microbatch import (
    merge_chunks,
...
class MicrobatchTests(TestCase):
    def test_split_and_merge(self):
        x0 = smith.randn(128, d_hid)
        x1 = smith.randn(256, d_hid)
        x2 = smith.randn(512, d_hid)
        ...
        assert arg_chunks[0][0].shape == smith.Size([64, d_hid])
        assert arg_chunks[1][0].shape == smith.Size([64, d_hid])
        assert arg_chunks[0][1].shape == smith.Size([128, d_hid])
        assert arg_chunks[0][2].shape == smith.Size([256, d_hid])
        assert kwarg_chunks[0]["x0"].shape == smith.Size([64, d_hid])
        assert kwarg_chunks[0]["x1"].shape == smith.Size([128, d_hid])
        assert kwarg_chunks[1]["x2"].shape == smith.Size([256, d_hid])
        ...
        smith.testing.assert_close(merged_args, args)
        ...
        smith.testing.assert_close(merged_kwargs, kwargs)
    ...
    def test_split_block_mask(self, device):
        ...
        def create_block_causal_mask(batch, eos_id: int):
            mask = batch == eos_id
            mask[:, -1] = True
            acc_mask = smith.cumsum(smith.where(mask, 1, 0), dim=1)
            seq_idx = smith.zeros_like(acc_mask, dtype=smith.int32)
        ...
        batch = smith.arange(total_elements, device=device) % DOC_LEN
        batch = batch.reshape(B, SEQ_LEN)
        q, k, v = (
            smith.randn(B, H, SEQ_LEN, DIM, device=device, requires_grad=True)
            for i in range(3)
        )
        block_mask_fn = smith.compile(create_block_mask, fullgraph=True)
        ...
        if device == "cuda":
            flex_fn = smith.compile(flex_attention)
        else:
            # It's unclear why CPU + smith.compile + flex_attention can cause an issue.
        ...
        concat_q = smith.cat(q_total_chunks, dim=0)
        concat_dq = smith.cat(dq_total_chunks, dim=0)
        concat_k = smith.cat(k_total_chunks, dim=0)
        concat_dk = smith.cat(dk_total_chunks, dim=0)
        concat_v = smith.cat(v_total_chunks, dim=0)
        concat_dv = smith.cat(dv_total_chunks, dim=0)
        concat_kv_indices = smith.cat(
            [bm.kv_indices for bm in block_mask_total_chunks], dim=0
        )
        concat_kv_num_blocks = smith.cat(
            [bm.kv_num_blocks for bm in block_mask_total_chunks], dim=0
        )
        concat_kv_full_num_blocks = smith.cat(
            [bm.full_kv_num_blocks for bm in block_mask_total_chunks], dim=0
        )
        concat_kv_full_indices = smith.cat(
            [bm.full_kv_indices for bm in block_mask_total_chunks], dim=0
        )
        concat_out = smith.cat(out_total_chunks, dim=0)
    ...
    def test_split_block_mask_batch_size_one(self, device):
        ...
        q, k, v = (smith.randn(B, H, SEQ_LEN, DIM, device=device) for i in range(3))
        block_mask_fn = smith.compile(create_block_mask, fullgraph=True)
        block_mask = block_mask_fn(
            create_causal_mask(),
            B=1,
    ...
    @skipXPUIf(True, "https://github.com/intel/smith-xpu-ops/issues/1682")
    def test_chunk_spec(self, device):
        mod = ModelWithKwargs().to(device)
        batch_size = ModelWithKwargs.DEFAULT_BATCH_SIZE

        x = smith.randn(batch_size, d_hid, device=device)
        y = smith.randn(batch_size, d_hid, device=device)
