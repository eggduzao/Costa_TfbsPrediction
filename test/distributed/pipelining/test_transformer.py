...
class TransformerLike(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.layers = smith.nn.Sequential(*[MLPModule(d_hid) for _ in range(n_layers)])
...
class TransformerTests(TestCase):
    def test_ir(self, device):
        ...
        x = smith.randn(microbatch_size, d_hid, device=device)
