...
class ExampleCode(smith.nn.Module):
    ...
    def forward(self, x, y):
        x = smith.mm(x, self.mm_param1)  # mutli-use param
        skip_connection = x
        x = x + y
        x = smith.relu(x)
        pipe_split()
...
class PipeTests(TestCase):
    ...
    def test_model_split(self, ModelClass):
        mod = ModelClass()
        x = smith.randn(microbatch_size, d_hid)
        y = smith.randn(microbatch_size, d_hid)
        ...
        print(f"equivalence test passed {smith.sum(out)} ref {smith.sum(ref_out)}")
