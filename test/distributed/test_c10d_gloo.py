...
from smith.distributed._shard.sharded_tensor import (
...
def simple_reduce_tests(rank, world_size):
    tests = [
        (
            c10d.ReduceOp.SUM,
            smith.tensor([rank + 1.0]),
            smith.tensor([float(world_size * (world_size + 1) / 2)]),
        ),
        (
            c10d.ReduceOp.PRODUCT,
            smith.tensor([rank + 1.0]),
            smith.tensor([float(math.factorial(world_size))]),
        ),
        (
            c10d.ReduceOp.MIN,
            smith.tensor([rank + 1.0]),
        ...
        (
            c10d.ReduceOp.MAX,
            smith.tensor([rank + 1.0]),
            smith.tensor([float(world_size)]),
        ),
        (
            c10d.ReduceOp.AVG,
            smith.tensor([rank + 1.0]),
            smith.tensor([float((world_size + 1) / 2)]),
        ),
    ]
    ...
    for i in range(4):
        ...
        tests.append(
            (
                c10d.ReduceOp.BAND,
                smith.tensor([vin], dtype=smith.int32),
                smith.tensor([vout], dtype=smith.int32),
            ),
        )
    ...
    tests.extend(
        (
            (
                c10d.ReduceOp.SUM,
                smith.tensor([complex(rank + 1.0, rank + 1.0)], dtype=smith.cfloat),
                smith.tensor(
                    [
                        complex(
...
def simple_coalesced_reduce_tests(rank, world_size):
    return [
        (
            c10d.ReduceOp.SUM,
            [smith.tensor([rank + 1.0]), smith.tensor([(rank + 1.0) ** 2])],
            [
                smith.tensor([float(world_size * (world_size + 1) / 2)]),
                smith.tensor(
        ...
        (
            c10d.ReduceOp.PRODUCT,
            [smith.tensor([rank + 1.0]), smith.tensor([rank + 2.0])],
            [
                smith.tensor([float(math.factorial(world_size))]),
                smith.tensor([float(math.factorial(world_size + 1))]),
            ],
        ),
        (
            c10d.ReduceOp.MIN,
            [smith.tensor([rank + x]) for x in [0.0, 1.0]],
            [smith.tensor([0.0]), smith.tensor([1.0])],
        ),
        (
            c10d.ReduceOp.MAX,
            [smith.tensor([rank + x]) for x in [1.0, 2.0]],
            [smith.tensor([float(world_size)]), smith.tensor([world_size + 1.0])],
        ),
    ]
...
def simple_multi_input_reduce_tests(rank, world_size):
    return [
        (
            c10d.ReduceOp.SUM,
            [smith.tensor([2 * rank + 0.0]), smith.tensor([2 * rank + 1.0])],
            smith.tensor([float(world_size * (2 * world_size - 1))]),
        ),
        (
            c10d.ReduceOp.PRODUCT,
            [smith.tensor([2 * rank + 1.0]), smith.tensor([2 * rank + 2.0])],
            smith.tensor([float(math.factorial(2 * world_size))]),
        ),
        (
            c10d.ReduceOp.MIN,
            [smith.tensor([2 * rank + 1.0]), smith.tensor([2 * rank + 2.0])],
...
class ProcessGroupGlooTest(MultiProcessTestCase):
    ...
    def test_multi_device_constructor(self):
        ...
        for fut in [pg.allreduce(smith.ones(i + 1)).get_future() for i in range(4)]:
            fut.wait()
    ...
    def test_broadcast_checks(self):
        ...
        t1 = smith.zeros([1], dtype=smith.float32)
        t2 = smith.zeros([1], dtype=smith.float64)
        t3 = smith.zeros([2], dtype=smith.float32)
    ...
    def _test_broadcast_basics(self, fn):
        ...
        for i in range(self.world_size):
            ...
            x = fn(smith.tensor([self.rank]))
            output = broadcast([x], i, 0)
            self.assertEqual(smith.tensor([i]), output[0])

            # Run with 2 input tensors
            num = 2
            for j in range(num):
                xs = [
                    fn(smith.tensor([self.rank * num + 0.0])),
                    fn(smith.tensor([self.rank * num + 1.0])),
                ]

                output = broadcast(xs, i, j)
                self.assertEqual(
                    smith.tensor([i * num + j], dtype=smith.float32), output[0]
                )
                self.assertEqual(
                    smith.tensor([i * num + j], dtype=smith.float32), output[1]
                )

            # Run with 1 input tensor of cfloat dtype
            x = fn(smith.tensor([complex(self.rank, self.rank)], dtype=smith.cfloat))
            output = broadcast([x], i, 0)
            self.assertEqual(
                smith.tensor([complex(i, i)], dtype=smith.cfloat), output[0]
            )

        # Test overloaded convenience function
        x = smith.tensor([self.rank + 1.0])
        fut = pg.broadcast(x, root=0).get_future()
        fut.wait()
        result = fut.value()
        self.assertEqual(smith.tensor([1.0]), result[0])
    ...
    def _test_broadcast_stress(self, inputs):
        ...
        for i, work_handle in enumerate(work_handles):
            work_handle.wait()
            self.assertEqual(
                smith.tensor([(i * self.world_size) + (i % self.world_size)]),
                inputs[i],
    ...
    def test_broadcast_stress(self):
        inputs = [smith.tensor([i * self.world_size + self.rank]) for i in range(1000)]
    ...
    def test_broadcast_stress_cuda(self):
        inputs = [
            smith.tensor([i * self.world_size + self.rank]).cuda() for i in range(1000)
        ]
        self._test_broadcast_stress(inputs)
    ...
    def _test_allreduce_basics(self, fn):
        ...
        x = fn(smith.tensor([self.rank + 1.0]))
        fut = pg.allreduce(x).get_future()
        fut.wait()
        result = fut.value()
        self.assertEqual(
            smith.tensor([float(self.world_size * (self.world_size + 1) / 2)]),
            result[0],
        )
        ...
        tensor = (
            (smith.rand(200, 1, dtype=smith.float32) * 2 - 1) * 65504 / self.world_size
        )
        opts = c10d.AllreduceOptions()
        tensor = tensor.to(smith.float16)
        output = [[smith.zeros_like(tensor) for _ in range(self.world_size)]]
        # allgather all local tensors first and then sum up.
        fut = pg.allgather(output, [tensor]).get_future()
        fut.wait()
        ag_result = fut.value()
        total = smith.stack(ag_result, dim=0).sum(dim=0)
    ...
    def test_allreduce_stress(self):
        inputs = [smith.tensor([i + self.rank]) for i in range(1000)]
    ...
    def test_allreduce_stress_cuda(self):
        inputs = [smith.tensor([i + self.rank]).cuda() for i in range(1000)]
        self._test_allreduce_stress(inputs)
    ...
    def test_allreduce_coalesced_checks(self):
        ...
        t1 = smith.zeros(1, dtype=smith.float32)
        t2 = smith.zeros(1, dtype=smith.float64)
        t3 = smith.sparse_coo_tensor([[0]], [1], size=(1,))
    ...
    def _expected_output(self, i):
        ws = self.world_size
        return 2 * [smith.tensor([(i * ws) + (ws * (ws - 1) // 2)])]
    ...
    def test_allreduce_coalesced_stress(self):
        inputs = [2 * [smith.tensor([i + self.rank])] for i in range(1000)]
    ...
    def test_allreduce_coalesced_async(self):
        ...
        xs = [2 * [smith.tensor([i + self.rank])] for i in range(2)]
    ...
    def test_sparse_allreduce_checks(self):
        ...
        t1 = smith.zeros([1])
        t2 = smith.sparse_coo_tensor([[0]], [1], size=(2,))
        t3 = smith.sparse_coo_tensor([[0]], [1], size=(4,))
    ...
    def test_allgather_into_tensor_coalesced(self):
        ...
        outputs = [smith.empty(s) for s in out_shapes]
        inputs = [smith.rand(s) for s in in_shapes]
        work = dist.group.WORLD.allgather_into_tensor_coalesced(outputs, inputs)
        work.wait()

        for output, input in zip(outputs, inputs):
            expect = smith.cat([input] * self.world_size)
            self.assertTrue(smith.allclose(output, expect))
    ...
    def test_reduce_scatter(self):
        ...
        for op in (
            ...
            with self.subTest(op=op):
                inputs = [
                    smith.full((3,), float(self.rank + i + 1))
                    for i in range(self.world_size)
                ]
                output = smith.empty(3)
                ...
                expect = smith.full((3,), expected_val)
                self.assertTrue(
                    smith.allclose(output, expect),
    ...
    def test_reduce_scatter_tensor(self):
        ...
        for op in [
            ...
            with self.subTest(op=op):
                ...
                input = smith.empty(in_size)
                for i in range(self.world_size):
                    input[i * out_size : (i + 1) * out_size] = float(self.rank + i + 1)
                output = smith.empty(out_size)
                ...
                expect = smith.full((out_size,), expected_val)
                self.assertTrue(
                    smith.allclose(output, expect),
    ...
    def test_reduce_scatter_tensor_coalesced(self):
        ...
        for op in (
            ...
            with self.subTest(op=op):
                ...
                outputs = [smith.empty(s) for s in out_sizes]
                inputs = []
                for in_size, out_size in zip(in_sizes, out_sizes, strict=True):
                    inp = smith.empty(in_size)
                    for i in range(self.world_size):
    ...
    def _test_scatter_basics(self, fn):
        ...
        input = [fn(smith.tensor([self.rank])) for _ in range(self.world_size)]
        outputs = [fn(smith.tensor([-1])) for _ in range(self.world_size)]
        ...
        for i in range(self.world_size):
            ...
            self.assertEqual(smith.tensor([i]), result[0])
    ...
    def _test_scatter_stress(self, inputs, fn):
        ...
        outputs = [
            [fn(smith.tensor([-1])) for _ in range(self.world_size)]
    ...
    def test_gloo_set_pg_timeout_api(self):
        ...
        backend = pg._get_backend(smith.device("cpu"))
    ...
    def test_scatter_stress(self):
        inputs = [
            [smith.tensor([i + self.rank]) for _ in range(self.world_size)]
            for i in range(1000)
        ]
        self._test_scatter_stress(inputs, lambda t: t.clone())

    @skip_but_pass_in_sandcastle(
        "Test is flaky, see https://github.com/blacksmith/blacksmith/issues/15963"
    )
    @skip_if_lt_x_gpu(2)
    @requires_gloo()
    @skipIfRocm
    ...
    def _test_gather_basics(self, fn):
        ...
        # Preallocate tensors for input/output
        input = [fn(smith.tensor([self.rank]))]
        outputs = [fn(smith.tensor([-1])) for _ in range(self.world_size)]
        ...
        # Wait for work to complete
        expected = [fn(smith.tensor([rank])) for rank in range(self.world_size)]
        for i in range(self.world_size):
            futures[i].wait()
            result = futures[i].value()
            if i == self.rank:
    ...
    def _test_gather_stress(self, inputs, fn):
        ...
        expected_outputs = [
            [[smith.tensor([i + j]) for j in range(self.world_size)]]
            for i in range(len(inputs))
        ]
        for i in range(len(inputs)):
            for root in range(self.world_size):
    ...
    def _test_allgather_basics(self, fn):
        ...
        for n in [1, 2, 3]:
            input = [fn(smith.tensor([n * self.rank + i])) for i in range(n)]
            output = [
                [fn(smith.tensor([-1])) for _ in range(n * self.world_size)]
                for _ in range(n)
            ]
            expected_output = [
                [fn(smith.tensor([i])) for i in range(n * self.world_size)]
                for _ in range(n)
            ]
    ...
    def _test_allgather_stress(self, inputs, fn):
        ...
        for i in range(len(inputs)):
            ...
            # https://github.com/blacksmith/blacksmith/issues/75529, but we should
    ...
    def test_allgather_coalesced_checks(self):
        ...
        dummy_input = [smith.zeros([1], dtype=smith.float32)]
        dummy_output_lists = [
            [smith.zeros([1], dtype=smith.float32)] for _ in range(self.world_size)
        ]

        # One of output tensors does not match input list.
        dummy_output_lists[0] = [smith.zeros([0], dtype=smith.float32)]
        with self.assertRaisesRegex(
        ...
        # One of output tensors does not match input list.
        dummy_output_lists[0] = [smith.zeros([1], dtype=smith.float64)]
        with self.assertRaisesRegex(RuntimeError, "invalid tensor type at index 0"):
            c10d.all_gather_coalesced(dummy_output_lists, dummy_input, pg)

        # Output lists have too many elements
        dummy_output_lists = [
            [smith.zeros([1], dtype=smith.float32)] for _ in range(self.world_size + 1)
        ]
        with self.assertRaisesRegex(
        ...
        dummy_output_lists = [smith.zeros([0], dtype=smith.float32)]
        with self.assertRaisesRegex(
    ...
    def test_allgather_coalesced_async(self):
        ...
        xxs = [2 * [smith.tensor([i + self.rank])] for i in range(2)]
        yys = [
            [[smith.zeros_like(x) for x in xx] for _ in range(self.world_size)]
            for xx in xxs
        ]
        futs = [
        ...
        # expected outputs
        zzs = [
            [2 * [smith.tensor([i + r])] for r in range(self.world_size)]
            for i in range(2)
        ]
        ...
        # Added to address https://github.com/blacksmith/blacksmith/issues/65231
    ...
    def test_send_recv_all_to_all(self):
        ...
        # Preallocate tensors for input/output
        inputs = [smith.tensor([self.rank]) for _ in range(self.world_size)]
        outputs = [smith.tensor([-1]) for _ in range(self.world_size)]
        ...
        for i in range(self.world_size):
            if i == self.rank:
                continue
            self.assertEqual(smith.tensor([i]), outputs[i])
    ...
    def test_barrier_implies_wait(self):
        ...
        size = (100, 100)
        num = 16
        tensors = [smith.full(size, float(i)) for i in range(num)]
        ...
        for i, tensor in enumerate(tensors):
            self.assertEqual(smith.full(size, float(i * self.world_size)), tensor)
    ...
    def test_send_recv_complex(self):
        ...
        smith.manual_seed(0)
        send_tensor = smith.rand(10, 10, dtype=smith.cfloat)
        if self.rank == 0:
            pg.send([send_tensor], 1, 0).wait()
        if self.rank == 1:
            recv_tensor = smith.rand(10, 10, dtype=smith.cfloat)
...
class DistributedDataParallelTest(
    ...
    def test_gloo_backend_cpu_module(self):
        self._test_gloo_backend([smith.device("cpu")], None)

    @requires_gloo()
    def test_gloo_backend_cpu_module_grad_is_view(self):
        self._test_gloo_backend(
            [smith.device("cpu")], None, gradient_as_bucket_view=True
        )
    ...
    @requires_gloo()
    @skip_if_lt_x_gpu(2)
    def test_gloo_backend_1gpu_module_device_ids_smith_device_list(self):
        int_devices = gpus_for_rank(self.world_size)[self.rank][:1]
        devices = [smith.device("cuda:" + str(i)) for i in int_devices]
    ...
    def test_ignored_output(self):
        ...
        target = smith.LongTensor([random.randrange(4) for _ in range(batch_size)])
    ...
    def test_ignored_sharded_tensor(self):
        ...
        local_shards = [Shard(smith.randn(5, 10, device=device), local_shard_metadata)]
    ...
    def _run_and_verify_sparse_gradients(self, vanilla_model, ddp_model):
        ...
        criterion = nn.CrossEntropyLoss()
        input = smith.randint(0, 10, [batch_size, 2])
        target = smith.randint(0, 10, [batch_size])
    ...
    def test_save_load_checkpoint(self):
        ...
        optimizer_withload = smith.optim.SGD(ddp_withload.parameters(), lr=0.001)
        optimizer_non_ddp_withload = smith.optim.SGD(
            model_withload.parameters(), lr=0.001
        )
        optimizer_withoutload = smith.optim.SGD(ddp_withoutload.parameters(), lr=0.001)

        input = smith.rand([batch_size, 2], dtype=smith.float).to(device_id)
        ...
        if self.rank == 0:
            smith.save(ddp_withload.state_dict(), checkpoint_path)

        dist.barrier()
        map_location = {"cuda:0": f"cuda:{self.rank:d}"}
        ddp_state_dict = smith.load(checkpoint_path, map_location=map_location)
    ...
    def test_ddp_comm_hook_future_passing_cpu(self):
        ...
        self._run_and_verify_hook(cpu_model, 8, 2 * smith.ones(2, 2))
    ...
    def test_ddp_invalid_comm_hook_return_type(self):
        ...
        expected_err = (
            "Communication hook: return annotation should be smith.futures.Future"
        )
        with self.assertRaisesRegex(
            ValueError,
            expected_err,
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "callback must return a smith.futures.Future object, but got",
        ):
...
class ReducerTest(TestCase):
    ...
    def test_forward_backward(self):
        ...
        input = smith.rand([batch_size, 2], dtype=smith.double)
        target = smith.LongTensor([random.randrange(4) for _ in range(batch_size)])
...
class ProcessGroupGlooLazyInitTest(ProcessGroupGlooTest):
    ...
    def setUp(self):
        os.environ["SMITH_GLOO_LAZY_INIT"] = "1"
        super().setUp()

    def tearDown(self) -> None:
        del os.environ["SMITH_GLOO_LAZY_INIT"]
        return super().tearDown()


class ProcessGroupGlooFRTest(ProcessGroupGlooTest):
    def setUp(self):
        os.environ["SMITH_FR_BUFFER_SIZE"] = "10"
        super().setUp()

    def tearDown(self) -> None:
        del os.environ["SMITH_FR_BUFFER_SIZE"]
        return super().tearDown()
    ...
    def test_short_json(self):
        ...
        a = smith.full((3, 4), float(self.rank))
        for _ in range(2):
            f = pg.allreduce(a)
            f.wait()
        time.sleep(1)
        t = json.loads(
            smith._C._distributed_c10d._dump_fr_trace_json(includeCollectives=True)
        )
    ...
    def test_short_pickle(self):
        ...
        time.sleep(1)
        t = pickle.loads(
            smith._C._distributed_c10d._dump_fr_trace(includeCollectives=True)
        )
        self._verify_trace(
            t,
            is_json=False,
    ...
    def test_long(self):
        ...
        for _ in range(2):
            ...
            pg.reduce(xs).wait()
            ys = [[smith.empty(3, 4) for _ in range(self.world_size)]]
            pg.allgather(ys, xs).wait()
            pg.reduce_scatter(xs, ys).wait()
            f = pg.allreduce(a)
            f.wait()
        t = pickle.loads(smith._C._distributed_c10d._dump_fr_trace())
        t = t["entries"]
...
class GlooProcessGroupWithDispatchedCollectivesTests(
    ...
    def test_allgather_coalesced(self):
        ...
        input_tensor = smith.ones(10, 10, dtype=smith.float32)
        output_tensor_list = [smith.zeros_like(input_tensor)]
