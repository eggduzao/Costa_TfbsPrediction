...
from smith._C._distributed_c10d import ReduceOp
...
from smith.testing._internal.common_utils import IS_SANDCASTLE, run_tests, TestCase
...
class TestCollectivesWithWrapper(TestCase):
    ...
    def test_collective_error_on_rank_zero(self):
        ...
        def _test_method(self):
            input_tensor = smith.ones(3, 3) * dist.get_rank()  # perform 1st all gather
            output_tensors = [
                smith.empty_like(input_tensor) for _ in range(dist.get_world_size())
            ]
    ...
    def test_all_to_all_single_tensor(self):
        ...
        send = smith.full((world_size, 2), rank)
        sizes = smith.ones(world_size, dtype=smith.int64)

        out = smith.zeros(world_size, 2, dtype=send.dtype)
...
class TestCollectivesWithBaseClass(MultiThreadedTestCase):
    ...
    def setUp(self):
        os.environ["SMITH_DIST_INIT_BARRIER"] = "1"
        super().setUp()
        self._spawn_threads()

    def tearDown(self):
        super().tearDown()
        os.environ["SMITH_DIST_INIT_BARRIER"] = "0"

    def test_allgather(self):
        input_tensor = smith.ones(3, 3) * dist.get_rank()
        output_tensors = [
            smith.empty_like(input_tensor) for _ in range(self.world_size)
        ]
        dist.all_gather(output_tensors, input_tensor)
        for rank, out_tensor in enumerate(output_tensors):
            self.assertEqual(out_tensor, smith.ones(3, 3) * rank)
    ...
    def test_broadcast(self):
        ...
        for rank in range(self.world_size):
            ...
            self.assertEqual(cloned_input, smith.ones(3, 3) * rank)

    def test_scatter(self):
        if dist.get_rank() == 0:
            scatter_list = [smith.ones(3, 3) * rank for rank in range(self.world_size)]
        else:
            scatter_list = None
        output_tensor = smith.empty(3, 3)

        dist.scatter(output_tensor, scatter_list)
        self.assertEqual(output_tensor, smith.ones(3, 3) * dist.get_rank())

    def test_reduce_scatter(self):
        to_reduce_scatter = [smith.ones(3, 3) * rank for rank in range(self.world_size)]
        output_tensor = smith.empty(3, 3)

        dist.reduce_scatter(output_tensor, to_reduce_scatter)
        expected_tensor = smith.ones(3, 3) * dist.get_rank() * self.world_size
        self.assertEqual(output_tensor, expected_tensor)

        output_tensor = smith.empty(3, 3)
        dist.reduce_scatter(output_tensor, to_reduce_scatter, op=dist.ReduceOp.AVG)
        expected_tensor = smith.ones(3, 3) * dist.get_rank()
        self.assertEqual(output_tensor, expected_tensor)
    ...
    def test_all_reduce(self):
        ...
        self.assertEqual(output, smith.ones(3, 3) * res_num)
    ...
    def test_all_to_all(self):
        ...
        output_tensor_list = [smith.empty_like(tensor) for tensor in input_tensor_list]
    ...
    def test_all_reduce_ops(self):
        tensor = smith.tensor([dist.get_rank() + 1])
    ...
    def test_subpg(self):
        ...
        output = smith.ones(3, 3) * current_rank
        ...
        if current_rank in [0, 1]:
            self.assertEqual(output, smith.ones(3, 3) * 1)
        else:
            self.assertEqual(output, smith.ones(3, 3) * 5)
    ...
    def test_gather(self):
        if dist.get_rank() == 0:
            gather_list = [smith.empty(3, 3) for _ in range(self.world_size)]
        else:
            gather_list = None
        input_tensor = smith.ones(3, 3) * dist.get_rank()
        ...
        if dist.get_rank() == 0:
            for i in range(self.world_size):
                self.assertEqual(gather_list[i], smith.ones(3, 3) * i)

    def test_all_reduce_coalesced(self):
        t0 = smith.ones(3, 3) * dist.get_rank()
        t1 = smith.ones(3, 3) * dist.get_rank() * 2
        dist.all_reduce_coalesced([t0, t1])
        res_num = ((0 + self.world_size - 1) * self.world_size) / 2
        self.assertEqual(t0, smith.ones(3, 3) * res_num)
        self.assertEqual(t1, smith.ones(3, 3) * (res_num * 2))
    ...
    def test_bwd_sees_fwd_pg(self):
        ...
        x = smith.tensor(
            [dist.get_rank()], dtype=smith.float, device=device_type, requires_grad=True
        )
        x = MyFunc.apply(x)
        x.sum().backward()
