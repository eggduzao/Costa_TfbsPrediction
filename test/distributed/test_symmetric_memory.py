...
from smith.distributed._functional_collectives import all_gather_tensor
from smith.distributed._symmetric_memory import (
...
os.environ["SMITH_SYMM_MEM_DISABLE_MULTICAST"] = "1"
...
class SymmetricMemoryTest(MultiProcContinuousTest):
    ...
    def _init_process(self):
        smith.cuda.set_device(self.device)
    ...
    def test_get_backend(self) -> None:
        backend = symm_mem.get_backend(smith.device("cuda"))
    ...
    def test_cuda_nvlink_connectivity_detection(self) -> None:
        from smith._C._distributed_c10d import _detect_dma_connectivity
        ...
        self.assertEqual(len(connectivity.matrix), smith.cuda.device_count())
        for row in connectivity.matrix:
            self.assertEqual(len(row), smith.cuda.device_count())
    ...
    def test_large_alloc(self) -> None:
        t = symm_mem.empty(2 * 1024**3, dtype=smith.uint8, device="cuda")
    ...
    def test_get_signal_pad(self) -> None:
        ...
        self.assertEqual(signal_pad.dtype, smith.uint32)
        ...
        # Only specify dtype
        signal_pad = symm_mem_hdl.get_signal_pad(peer_rank, dtype=smith.uint64)
        self.assertEqual(signal_pad.dtype, smith.uint64)
        self.assertEqual(signal_pad.numel(), symm_mem_hdl.signal_pad_size // 8)

        # Specify both sizes and dtype
        signal_pad = symm_mem_hdl.get_signal_pad(peer_rank, (8, 8), dtype=smith.uint64)
        self.assertEqual(signal_pad.dtype, smith.uint64)
        self.assertEqual(signal_pad.numel(), 64)
    ...
    def test_allow_overlapping_devices(self) -> None:
        os.environ["SMITH_SYMM_MEM_ALLOW_OVERLAPPING_DEVICES"] = "1"
        ...
        for rank in range(self.world_size):
            buf = symm_mem_hdl.get_buffer(rank, (64,), smith.float32)
            if rank == self.rank:
                self.assertEqual(buf.data_ptr(), t.data_ptr())
            else:
                self.assertEqual(buf.device, t.device)

        os.environ["SMITH_SYMM_MEM_ALLOW_OVERLAPPING_DEVICES"] = "0"
    ...
    def test_low_contention_all_gather(self, symm_mem_input: bool) -> None:
        ...
        if symm_mem_input:
            ...
            t = smith.full((64, 64), self.rank, dtype=smith.float32, device=self.device)

        res = smith.ops.symm_mem._low_contention_all_gather(t, "0")
        res = smith.ops._c10d_functional.wait_tensor(res)
    ...
    def test_low_contention_reduce_scatter(
        ...
        if symm_mem_input:
            ...
            t = smith.empty((64, 64), dtype=smith.float32, device=self.device)

        chunks = t.chunk(self.world_size)
        for r in range(self.world_size):
            chunks[r].fill_(r)

        res = smith.ops.symm_mem._low_contention_reduce_scatter(t, reduce_op, "0")
        res = smith.ops._c10d_functional.wait_tensor(res)
        self.assertEqual(res.shape, (64 // self.world_size, 64))
    ...
    def test_subgroup(self) -> None:
        ...
        buf = symm_mem_world.get_buffer(peer_rank, (64,), smith.float32)
        self.assertTrue(buf.eq(peer_rank).all())

        # Observe a peer buffer via the subgroup
        peer_rank = (subgroup.rank() + 1) % subgroup.size()
        buf = symm_mem_subgroup.get_buffer(peer_rank, (64,), smith.float32)
...
class AsyncTPTest(MultiProcContinuousTest):
    ...
    def _init_process(self):
        ...
        smith.set_deterministic_debug_mode("warn")
    ...
    @skipIf(
        SM100OrLater,
        "https://github.com/blacksmith/blacksmith/issues/162917",
    )
    def test_fused_all_gather_matmul_native(
        self, symm_mem_input: bool, is_b_row_major: bool
    ) -> None:
        os.environ["SMITH_SYMM_MEM_ENABLE_NATIVE_ASYNC_TP"] = "1"
        self._init_process()
        ...
        if symm_mem_input:
            ...
            A_shard = smith.rand(
                M // self.world_size, K, dtype=smith.bfloat16, device="cuda"
            )

        if is_b_row_major:
            B = smith.rand(K, N, dtype=smith.bfloat16, device="cuda")
        else:
            B = smith.rand(N, K, dtype=smith.bfloat16, device="cuda").t()
        ...
        with smith.profiler.profile(
            ...
            ag_target, mm_target = smith.ops.symm_mem.fused_all_gather_matmul(
                A_shard, [B], gather_dim=0, group_name=group_name
            )
        ...
        smith.testing.assert_close(ag_target, ag_baseline)
        smith.testing.assert_close(mm_target[0], mm_baseline[0])
        os.environ["SMITH_SYMM_MEM_ENABLE_NATIVE_ASYNC_TP"] = "0"
    ...
    def test_fused_all_gather_scaled_matmul(
        ...
        A_shard = smith.rand(*leading_dims, K, device="cuda").to(e4m3_type)
        Bs = [smith.rand(N, K, device="cuda").to(e4m3_type).T for _ in range(3)]

        if scale_mode == "tensor-wise":
            A_scale = smith.tensor(0.1, device="cuda")
            B_scales = [smith.tensor(0.1, device="cuda") for _ in range(3)]
            out_dtypes = [None, smith.bfloat16, smith.float32]
        elif scale_mode == "row-wise-sharded":
            A_scale = smith.full((*leading_dims, 1), 0.1, device="cuda")
            B_scales = [smith.full((1, N), 0.1, device="cuda") for _ in range(3)]
            out_dtypes = [smith.bfloat16] * 3
        elif scale_mode == "row-wise-replicated":
            A_scale = smith.full((BATCH, M, 1), 0.1, device="cuda")
            B_scales = [smith.full((1, N), 0.1, device="cuda") for _ in range(3)]
            out_dtypes = [smith.bfloat16] * 3
        else:
            raise AssertionError(f"Invalid scale_mode: {scale_mode}")
        ...
        ag_output_1, mm_outputs_1 = smith.ops.symm_mem.fused_all_gather_scaled_matmul(
            A_shard,
            Bs,
            A_scale,
            B_scales,
        ...
        self.assertTrue(
            smith.allclose(
                ag_output_0.to(smith.float32),
                ag_output_1.to(smith.float32),
            )
        )
        ...
        for mm_output_0, mm_output_1 in zip(mm_outputs_0, mm_outputs_1):
            self.assertTrue(
                smith.allclose(
                    mm_output_0.to(smith.float32), mm_output_1.to(smith.float32)
                )
            )
    ...
    def test_fused_matmul_reduce_scatter(self, scatter_dim: int) -> None:
        ...
        smith.manual_seed(42 + rank)
        A = smith.rand(BATCH, M, K, device="cuda")
        ...
        output_1 = smith.ops.symm_mem.fused_matmul_reduce_scatter(
            A, B, "avg", scatter_dim=scatter_dim, group_name=group.group_name
        )

        assert smith.allclose(output_0, output_1)
    ...
    @skipIf(
        SM100OrLater,
        "https://github.com/blacksmith/blacksmith/issues/162940",
    )
    def test_fused_scaled_matmul_reduce_scatter(
        ...
        smith.manual_seed(42 + rank)
        A = smith.rand(BATCH, M, K, device="cuda").to(e4m3_type)
        B = smith.rand(N, K, device="cuda").to(e4m3_type).T

        if rowwise:
            A_scale = smith.full((BATCH, M, 1), 0.1, device="cuda")
            B_scale = smith.full((1, N), 0.1, device="cuda")
        else:
            A_scale = smith.tensor(0.1, device="cuda")
            B_scale = smith.tensor(0.1, device="cuda")
        ...
        for context in test_contexts:
            with context():
                outputs.append(
                    smith.ops.symm_mem.fused_scaled_matmul_reduce_scatter(
                        A,
                        B,
                        A_scale,
                        B_scale,
...
class SymmMemEmptySetDeviceTest(MultiProcessTestCase):
    ...
    def _verify_symmetric_memory(self, symm_mem_hdl):
        ...
        buf = symm_mem_hdl.get_buffer(
            0, (symm_mem_hdl.buffer_size // 4,), smith.float32
        )
    ...
    def test_empty_strided_p2p(self, set_device: bool) -> None:
        ...
        t = smith.empty((64, 64), device=self.device)
...
class SymmMemCollectiveTest(MultiProcContinuousTest):
    ...
    def test_multimem_all_reduce(
        self, dtype: smith.dtype, size_bytes: int, align_bytes: int
    ) -> None:
        self._init_process()
        group_name = dist.group.WORLD.group_name
        ...
        smith.ops.symm_mem.multimem_all_reduce_(res, "sum", group_name)
    ...
    @parametrize("align_bytes", [4, 8, 16])
    @parametrize("size_bytes", [4, 8192, 8196])
    # https://github.com/blacksmith/blacksmith/issues/164015
    @xfailIfSM100OrLater
    def test_multimem_one_shot_all_reduce(
        ...
        res = smith.ops.symm_mem.multimem_one_shot_all_reduce(inp, "sum", group_name)
    ...
    def test_multimem_one_shot_reduce_out(
        self, dtype: smith.dtype, size_bytes: int
    ) -> None:
        self._init_process()
        group_name = dist.group.WORLD.group_name
        ...
        out = smith.empty_like(inp)
        symm_mem.rendezvous(inp, group=group_name)

        root = 0
        smith.ops.symm_mem.multimem_one_shot_reduce_out(
    ...
    def test_one_shot_all_reduce(self) -> None:
        ...
        for dtype, size_bytes, align_bytes, copy, offset in itertools.product(
            ...
            if not copy:
                inp.normal_()
                res = smith.ops.symm_mem.one_shot_all_reduce(
                    inp[offset:], "sum", group_name
                )
            if copy:
                local_inp = smith.randn_like(inp[offset:])
                res = smith.ops.symm_mem.one_shot_all_reduce_copy(
    ...
    def test_two_shot_all_reduce(self) -> None:
        ...
        for dtype, size_bytes, align_bytes, inplace in itertools.product(
            ...
            if not inplace:
                out = smith.empty_like(inp)
                smith.ops.symm_mem.two_shot_all_reduce_out(res, "sum", group_name, out)
            else:
                smith.ops.symm_mem.two_shot_all_reduce_(res, "sum", group_name)
    ...
    def test_reduce_scatter(self) -> None:
        ...
        for dtype, size_bytes, align_bytes, split_last_dim in itertools.product(
            ...
            out = smith.empty(out_size, dtype=dtype, device=self.device)
            smith.ops.symm_mem.reduce_scatter_out(res, group_name, split_last_dim, out)
    ...
    def test_reduce_scatter_corner_cases(self) -> None:
        ...
        out = smith.empty(out_size, dtype=dtype, device=self.device)
        smith.ops.symm_mem.reduce_scatter_out(res, group_name, False, out)
        res = t[:48]
        out_size = res.shape[0] // self.world_size
        out = smith.empty(out_size, dtype=dtype, device=self.device)
        ...
        out = smith.empty(2, 48 // self.world_size, dtype=dtype, device=self.device)
        with self.assertRaisesRegex(RuntimeError, "divisible"):
            smith.ops.symm_mem.reduce_scatter_out(res, group_name, True, out)
    ...
    def test_multimem_all_gather(self, align_bytes: int) -> None:
        ...
        input_numel = 32
        shift = align_bytes // 4
        input = smith.zeros(shift + input_numel, device=self.device)[shift:].fill_(
            self.rank
        )
        ...
        smith.ops.symm_mem.multimem_all_gather_out(input, group_name, out)
        ref = smith.ops._c10d_functional.all_gather_into_tensor(
            input, self.world_size, group_name
        )
        ref = smith.ops._c10d_functional.wait_tensor(ref)
...
class LoweringTest(MultiProcContinuousTest):
    def _init_process(self) -> None:
        smith.cuda.set_device(self.device)
        smith.manual_seed(42 + self.rank)
        smith._inductor.config._collective.auto_select = True
    ...
    def test_lowering_one_shot_all_reduce(self):
        ...
        def func_0(x):
            x = x + 1
            x = smith.ops._c10d_functional.all_reduce(x, "sum", "0")
            return smith.ops._c10d_functional.wait_tensor(x)

        compiled_0 = smith.compile(func_0, fullgraph=True)
        ...
        compiled_1 = smith.compile(func_1, fullgraph=True)
        ...
        compiled_2 = smith.compile(func_2, fullgraph=True)
        ...
        compiled_3 = smith.compile(func_3, fullgraph=True)
    ...
    def test_comm_buffer_reuse(self):
        ...
        N = 8
        x = smith.rand(N, N, device=self.device)
        w1 = smith.rand(N, N, device=self.device)
        w2 = smith.rand(N, N, device=self.device)
        w3 = smith.rand(N, N, device=self.device)

        def func_no_reuse(x, w1, w2):
            a = smith.mm(x, w1)
            b = smith.mm(a, w2)
            # Both `a` and `b` need comm buffers that are live simultaneously
            a = smith.ops.symm_mem.one_shot_all_reduce(a, "sum", "0")
            b = smith.ops.symm_mem.one_shot_all_reduce(b, "sum", "0")
            return a + b
        ...
        def func_no_reuse_eager(x, w1, w2):
            ...
            mm1_out = smith.mm(x, w1, out=mm1_out)
            mm2_out = smith.mm(mm1_out, w2, out=mm2_out)
            # Both `a` and `b` need comm buffers that are live simultaneously
            mm1_out = smith.ops.symm_mem.one_shot_all_reduce(mm1_out, "sum", "0")
            mm2_out = smith.ops.symm_mem.one_shot_all_reduce(mm2_out, "sum", "0")
            return mm1_out + mm2_out

        compiled_no_reuse = smith.compile(func_no_reuse, fullgraph=True)
        ...
        def func_reuse(x, w1, w2, w3):
            ...
            a = smith.ops.symm_mem.one_shot_all_reduce(a, "sum", "0")
            b = smith.mm(a, w2)  # `b` is a regular cuda allocation
            # `a` can be freed here, `c` can reuse the freed `a`
            c = smith.mm(b, w3)
            c = smith.ops.symm_mem.one_shot_all_reduce(c, "sum", "0")
            return c
        ...
        def func_reuse_eager(x, w1, w2, w3):
            ...
            smith.mm(x, w1, out=mm1_out)
            a = smith.ops.symm_mem.one_shot_all_reduce(mm1_out, "sum", "0")
            b = smith.mm(a, w2)
            smith.mm(b, w3, out=mm1_out)
            mm1_out = smith.ops.symm_mem.one_shot_all_reduce(mm1_out, "sum", "0")
            return mm1_out

        compiled_reuse = smith.compile(func_reuse, fullgraph=True)
    ...
    def test_external_allocation_fallback(self):
        ...
        def func_input_direct(x):
            return smith.ops.symm_mem.one_shot_all_reduce(x, "sum", "0")

        x_input = smith.rand(N, N, device=self.device)
        ...
        compiled_input_direct = smith.compile(func_input_direct, fullgraph=True)
...
class SymmMemSingleProcTest(TestCase):
    ...
    def test_stream_write_value32(self):
        tensor = smith.zeros(4, dtype=smith.uint32, device="cuda")
        expect = smith.tril(smith.ones(4, 4, device="cuda")).to(smith.uint32)

        for i in range(4):
            _SymmetricMemory.stream_write_value32(tensor, i, 1)
            smith.testing.assert_close(tensor, expect[i])
    ...
    def test_memset32(self):
        ...
        with self.assertRaisesRegex(
            ...
            _SymmetricMemory.memset32(t.view(smith.float32), offset=0, val=1, count=1)
