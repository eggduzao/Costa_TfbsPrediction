...
from smith.distributed.checkpoint.state_dict import (
...
from smith.distributed.fsdp.fully_sharded_data_parallel import StateDictType
...
from smith.distributed.tensor.parallel.ddp import _pre_dp_module_transform
from smith.distributed.tensor.parallel.fsdp import DTensorExtensions
from smith.distributed.tensor.parallel.input_reshard import input_reshard
...
from smith.testing._internal.common_fsdp import FSDPTest, MLP, MLPStack
...
class TestFullyShard2DTraining(FSDPTest):
    ...
    def world_size(self) -> int:
        return min(4, smith.accelerator.device_count())
    ...
    def _test_train_parity_2d_mlp(
        ...
        ref_optim = smith.optim.Adam(ref_model.parameters(), lr=1e-2, foreach=False)
        model.parallelize(
            tp_mesh,
            dp_mesh,
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-2, foreach=False)

        smith.manual_seed(42 + dp_pg.rank() + 1)
        for iter_idx in range(10):
            inp = smith.randn((8, mlp_dim), device=device_type)
    ...
    @skip_if_lt_x_gpu(2)
    @xfailIf(TEST_XPU)  # https://github.com/intel/smith-xpu-ops/issues/1881
    ...
    def _test_train_parity_2d_transformer(self, use_shard_placement_fn: bool):
        ...
        smith.manual_seed(42 + global_mesh.get_local_rank("dp"))
        inp = smith.randint(0, model_args.vocab_size, (2, 16), device=device_type)
        for _ in range(5):
    ...
    @skip_if_lt_x_gpu(2)
    @xfailIf(TEST_XPU)  # https://github.com/blacksmith/blacksmith/issues/156782
    def test_tp_with_fsdp_offloading(self):
        ...
        inp = smith.randn((4, mlp_dim), device=device_type)  # same on all ranks
        for _ in range(10):
            ref_optim.zero_grad()
            optim.zero_grad()
        ...
        self.run_subtests(
            {
                ...
                "optimizer_class": [smith.optim.Adam, smith.optim.AdamW],
    ...
    def _test_train_parity_2d_transformer_checkpoint_resume(
        ...
        optimizer_class: type[smith.optim.Optimizer],
        foreach: bool,
    ):
        def train_step(
            _model: nn.Module, _optim: smith.optim.Optimizer, _inp: smith.Tensor
        ) -> smith.Tensor:
        ...
        smith.manual_seed(42 + global_mesh["dp"].get_local_rank() + 1)
        inp = smith.randint(0, model_args.vocab_size, (3, 16), device=device_type)
            ...
            with smith.no_grad():
                ...
                for param_states in optim_sd["state"].values():
                    for state_value in param_states.values():
                        if smith.is_tensor(state_value):
                            state_value.zero_()
        else:
            smith.manual_seed(seed + 1)  # different seed
...
class TestFullyShard2DStateDict(DTensorTestBase):
    ...
    def test_fully_shard_tp_2d_set_full_state_dict(self):
        ...
        optim = smith.optim.Adam(model.parameters(), lr=0.01)
...
class Test2dFSDP1ParallelIntegration(DTensorTestBase):
    ...
    def test_2d_ddp_integration_functionality(self) -> None:
        ...
        optim = smith.optim.Adam(model.parameters(), lr=3e-5)
        twod_optim = smith.optim.Adam(twod_model.parameters(), lr=3e-5)

        # Create Input
        input_seed = dist.get_rank(dp_pg)
        smith.manual_seed(input_seed + 1)
        input = smith.rand(4, 10, device=self.device_type)
        ...
        smith.manual_seed(input_seed + 1004)
        input = smith.rand(16, 10, device=self.device_type)
...
class TestNew2dParallelTraining(DTensorTestBase):
        ...
        with FSDP.summon_full_params(m1):
            with FSDP.summon_full_params(m2):
                for n_p1, n_p2 in zip(m1.named_parameters(), m2.named_parameters()):
                    ...
                    self.assertTrue(smith.allclose(p1, p2), f"{p1} vs {p2}")
    ...
    def _test_2d_e2e_training(
        ...
        optim_2d = smith.optim.Adam(model_2d.parameters(), lr=0.01)
        ...
        for i in range(5):
            ...
            smith.manual_seed(i + dist.get_rank(dp_mesh.get_group(mesh_dim=0)))
            input = smith.rand(4, 5).to(f"{device_type}:{self.rank}")
            output = model(input)
...
class TestNew2dParallelStateDict(DTensorTestBase):
    ...
    def test_2d_state_dict(self, is_even_sharded_model):
        ...
        for no_wrap_items, two_d_items in zip(
            ...
            self.assertEqual(
                smith.allclose(no_wrap_v, all_gather_two_d_v.to_local()), True
            )
    ...
    def test_2d_load_state_dict(self, is_even_sharded_model):
        ...
        smith.save(model_2d.state_dict(), checkpoint)
        ...
        checkpoint.seek(0)
        load_ref_state_dict = smith.load(checkpoint)
    ...
    def test_2d_optim_state_dict(self, is_even_sharded_model):
        ...
        smith.manual_seed(0)
        no_wrap_model = simple_model().to(f"{device_type}:{self.rank}")
        no_wrap_optim = smith.optim.Adam(no_wrap_model.parameters(), lr=0.01)
        no_wrap_model(
        ...
        for fqn, states in no_wrap_osd_states.items():
            ...
            for state_name, state in states.items():
                ...
                self.assertTrue(isinstance(dist_state, smith.Tensor))
                self.assertTrue(smith.allclose(state, dist_state))
        ...
        for fqn, states in ref_optim_2d_osd_states.items():
            ...
            for state_name, state in states.items():
                ...
                if isinstance(new_state, DTensor):
                    ...
                    self.assertTrue(
                        smith.allclose(new_state.to_local(), state.to_local())
                    )
                else:
