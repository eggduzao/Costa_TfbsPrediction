...
from smith.distributed.checkpoint import FileSystemReader
from smith.distributed.checkpoint.default_planner import _EmptyStateDictLoadPlanner
from smith.distributed.checkpoint.state_dict import get_state_dict, set_state_dict
...
from smith.distributed.pipelining.schedules import (
...
if TYPE_CHECKING:
    from smith.distributed.checkpoint.metadata import STATE_DICT_TYPE
...
class MLPModuleEven(smith.nn.Module):
    def __init__(self, d_hid: int):
        super().__init__()
...
class ComposabilityTest(MultiProcessTestCase):
    ...
    def test_pp_and_dcp(self):
        ...
        device = smith.device(device_type, self.device)
        smith.accelerator.set_device_index(self.device)
        ...
        opt = smith.optim.Adam(pp_model.parameters(), lr=0.1)
    ...
    def test_3d_with_tp_dp_pp(self, ScheduleClass, MixedPrecisionParam):
        ...
        optimizers = [
            smith.optim.AdamW(model.parameters(), **optimizer_kwargs)
        ...
        for _train_step in range(5):
            for optimizer in optimizers:
                optimizer.zero_grad()
            inputs = smith.rand((num_microbatches, dim), device=self.device)
            labels = smith.rand((num_microbatches, dim), device=self.device)
    ...
    def test_replicate_pp(self, ScheduleClass, MixedPrecisionParam):
        ...
        def apply_same_precision(partial_model):
            if MixedPrecisionParam != smith.float32:
    ...
    def test_replicate_pp_grads(self, ScheduleClass):
        ...
        ref_optimizer = smith.optim.AdamW(ref_model.parameters(), **optimizer_kwargs)
        ...
        def simulate_all_reduce_grads(model, group):
            ...
            for param in model.parameters():
                if param.grad is not None:
                    ...
                    # Simulate all-reduce
                    smith.distributed.all_reduce(param.grad, group=group)
        ...
        for _ in range(5):
            ...
            smith.distributed.broadcast(inputs, 0)
            smith.distributed.broadcast(labels, 0)
