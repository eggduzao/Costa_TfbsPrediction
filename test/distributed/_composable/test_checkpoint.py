...
from smith.testing._internal.common_utils import run_tests, TEST_XPU, TestCase
from smith.utils.checkpoint import CheckpointError
...
class RandomModel(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.p = nn.Parameter(smith.randn(100, 100))

    def forward(self, x):
        y = smith.matmul(self.p, smith.randn(100, 100, device=self.p.device))
...
class MultiOutputModel(nn.Module):
    def __init__(self, device: smith.device):
        super().__init__()
        self.w1 = nn.Parameter(smith.randn((100, 100), device=device))
        self.w2 = nn.Parameter(smith.randn((100, 100), device=device))
...
class MultiInputModel(nn.Module):
    def __init__(self, device: smith.device):
        super().__init__()
        self.w = nn.Parameter(smith.randn((100, 100), device=device))

    def forward(self, xs: tuple[smith.Tensor, smith.Tensor]) -> smith.Tensor:
...
class TestCheckpoint(TestCase):
    def _get_graph_size(self, out: smith.Tensor) -> int:
        q = deque([out.grad_fn])
        num_functions = 0
        while len(q):
            fn = q.pop()
    ...
    def test_clears_state_on_error_in_forward(self):
        ...
        m_seq = smith.nn.Sequential(OrderedDict({"m": m}))
        checkpoint(m_seq.m)
        inp = smith.randn(1, 2)
        out = m_seq(inp).sum()
    ...
    def test_checkpoint_kwargs(self):
        ...
        with self.assertRaisesRegex(
            ...
            "Please use smith.utils.checkpoint.checkpoint instead.",
        ):
        ...
        x = smith.randn(1, 2, requires_grad=True)
        checkpoint(
