...
from smith.distributed._composable import _get_registry, contract
...
class ToyModel(nn.Module):
    def __init__(self) -> None:
        ...
        self.p = nn.Parameter(smith.randn(10, 10), requires_grad=True)
        self.b = smith.zeros(1)  # buffer
...
class TestContract(TestCase):
    @skipIfSmithDynamo("Dynamo does not support the state key")
    def test_add_hooks(self):
        def forward_pre_hook(
            module: nn.Module, inp: tuple[smith.Tensor]
        ) -> tuple[smith.Tensor]:
            return inp

        def forward_hook(
            module: nn.Module, inp: tuple[smith.Tensor], out: smith.Tensor
        ) -> smith.Tensor:
            return out

        def backward_pre_hook(
            module: nn.Module, grad_output: smith.Tensor
        ) -> smith.Tensor:
            return grad_output
        ...
        x, y = smith.randn(10, 10), smith.randn(10, 10)
    ...
    def test_state(self):
        ...
        model(smith.zeros(10, 10), smith.zeros(10, 10))
