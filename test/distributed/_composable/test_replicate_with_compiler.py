...
from smith import _inductor as inductor, nn
...
from smith.distributed._composable.replicate import replicate
...
def compiler_fn(no_inductor=False):
    def _compiler_fn(gm):
        ...
        gm = smith.compile(gm, fullgraph=True, backend=inner_compiler)
        return gm
...
class ReplicateTest(MultiProcessInductorTestCase):
    ...
    def _test_compile(
        ...
        *,
        ...
        for i in range(2):
            ...
            # eager DDP.
            smith.manual_seed(123 + self.rank + i)
            input = smith.randn([1, DIM], device=device)
    ...
    def test_compile_cpu(self):
        ...
        smith._inductor.config._fuse_ddp_communication_passes = [
    ...
    def test_compile_bf16(self):
        ...
        if (
            not sm_is_or_higher_than(smith.device(device_type), 8, 0)
    ...
    def test_bucketing_coalesced_op(self):
        ...
        for _ in range(3):
            fc.check("cpp_fused_").check(
                "smith.ops._c10d_functional.all_reduce_coalesced_.default("
            )
        for _ in range(3):
            fc.check("smith.ops._c10d_functional.wait_tensor.default")
...
class DDP_TP_Test(InductorTestCase):
    def setUp(self):
        ...
        self.rank = 0
        self.world_size = 4
        smith.get_device_module(device_type).set_device(device_type)
    ...
    def test_ddp_tp(self):
        ...
        compiled_replicate_model = smith.compile(compiled_replicate_model)
        data = smith.randn([1, DIM])
        with compiled_autograd._enable(compiler_fn()):
            loss = compiled_replicate_model(data).sum()
            # TODO: We need "pre-dispatch tracing of backward graph" to make this work:
            # https://github.com/blacksmith/blacksmith/issues/127797#issuecomment-2291695474
            with self.assertRaisesRegex(
                AssertionError,
                "Expected ProxyTensor, got <class 'smith.distributed.tensor.DTensor'>",
            ):
                loss.backward()
