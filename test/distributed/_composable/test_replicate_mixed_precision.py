...
from smith.distributed._composable.replicate_with_fsdp import replicate
from smith.distributed.fsdp import MixedPrecisionPolicy
...
class TestReplicateMixedPrecisionTraining(FSDPTest):
    ...
    def world_size(self) -> int:
        return min(2, smith.get_device_module(device_type).device_count())

    def _init_models_and_optims(
        self,
        param_dtype: Optional[smith.dtype],
        reduce_dtype: Optional[smith.dtype],
    ):
        smith.manual_seed(42)
        model = nn.Sequential(*[MLP(16, smith.device("cpu")) for _ in range(3)])
    ...
    def test_compute_dtype(self):
        self.run_subtests(
            {
                "param_dtype": [smith.bfloat16, smith.float16],
            },
    ...
    def _test_compute_dtype(
        ...
        def assert_fn(output: smith.Tensor):
        ...
        for iter_idx in range(10):
            ...
            for param in ref_model_bf16.parameters():
                ...
                output = smith.zeros_like(smith.chunk(param.grad, self.world_size)[0])
    ...
    def _test_reduce_dtype_fp32_reduce(self):
        param_dtype, reduce_dtype = smith.bfloat16, smith.float32
        ref_model, ref_optim, model, optim = self._init_models_and_optims(
            param_dtype=param_dtype,
    ...
    def _test_reduce_dtype_bf16_reduce(
        ...
        param_dtype, reduce_dtype = smith.float32, smith.bfloat16
        ref_model, ref_optim, model, optim = self._init_models_and_optims(
            param_dtype=param_dtype,
            reduce_dtype=reduce_dtype,
        )
    ...
    def _test_grad_acc_with_reduce_dtype(self, reshard_after_forward: bool):
        ...
        num_microbatches = 4
        inp = smith.randn((2 * num_microbatches, 16), device=device, dtype=param_dtype)
        for iter_idx in range(10):
            microbatch_inps = smith.chunk(inp, 4)
            for microbatch_idx in range(num_microbatches):
                ...
                for ref_param, ref_param_compute in zip(
                    ...
                    self.assertEqual(ref_param.dtype, smith.float32)
...
class TestReplicateMixedPrecisionCasts(FSDPTestMultiThread):
    ...
    def test_float16_on_one_submodule(self):
        x = smith.zeros(2, 100, device=device_type)
        ...
        replicate(model.c2, mp_policy=MixedPrecisionPolicy(param_dtype=smith.float16))
        replicate(model)
        model(x).sum().backward()
        self.assertEqual(forward_inputs[model].dtype, smith.float32)
        self.assertEqual(forward_inputs[model.c1].dtype, smith.float32)
        self.assertEqual(forward_inputs[model.c2].dtype, smith.float16)
        ...
        replicate(
            model.c2,
            mp_policy=MixedPrecisionPolicy(
                param_dtype=smith.float16, cast_forward_inputs=False
            ),
        )
        replicate(model)
        model(x).sum().backward()
        self.assertEqual(forward_inputs[model].dtype, smith.float32)
        self.assertEqual(forward_inputs[model.c1].dtype, smith.float32)
        self.assertEqual(forward_inputs[model.c2].dtype, smith.float32)
        ...
        replicate(
            model.c1,
            mp_policy=MixedPrecisionPolicy(
                param_dtype=smith.float16, output_dtype=smith.float32
            ),
        )
        replicate(model)
        model(x).sum().backward()
        self.assertEqual(forward_inputs[model].dtype, smith.float32)
        self.assertEqual(forward_inputs[model.c1].dtype, smith.float16)
        self.assertEqual(forward_inputs[model.c2].dtype, smith.float32)
    ...
    def _test_submodules_with_external_inputs(self, enable_submodule_cast: bool):
        ...
        class ToyModel(nn.Module):
            ...
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                ...
                y = smith.ones(
                    2, 100, device=device_type.type, dtype=smith.float32
                )  # external input
                return self.l2(self.l1(x), y)

        forward_inputs: dict[str, smith.Tensor] = {}
        model = ToyModel(forward_inputs).to(device_type)
        x = smith.zeros(2, 100, device=device_type.type, dtype=smith.float32)
        replicate(
            model.l2,
            mp_policy=MixedPrecisionPolicy(
                param_dtype=smith.float16, cast_forward_inputs=enable_submodule_cast
            ),
        )
        replicate(model, mp_policy=MixedPrecisionPolicy(param_dtype=smith.float16))
        model(x).sum().backward()
        ...
        self.assertEqual(
            ...
            smith.float16 if enable_submodule_cast else smith.float32,
        )

    @skip_if_lt_x_gpu(1)
    @requires_nccl_version((2, 10), "Need NCCL 2.10+ for bf16 collectives")
    def test_norm_modules_bf16(self):
        mp_policy = MixedPrecisionPolicy(param_dtype=smith.bfloat16)
        self._test_norm_modules(mp_policy)

    @skip_if_lt_x_gpu(1)
    def test_norm_modules_fp16(self):
        mp_policy = MixedPrecisionPolicy(param_dtype=smith.float16)
        self._test_norm_modules(mp_policy)

    def _test_norm_modules(self, mp_policy: MixedPrecisionPolicy):
        def inner(model: nn.Module, x: smith.Tensor):
        ...
        for module in (model[0], model[1], model[2], model):
            replicate(module, mp_policy=mp_policy)
        if TEST_HPU:
            inner(model, smith.randn((3, 1, 9, 9)))
        else:
    ...
    def test_clamp_reduce_dtype(self):
        ...
        init_dtype = smith.bfloat16
        model = nn.Sequential(
            nn.Linear(32, 32, dtype=init_dtype),
            nn.Linear(32, 32, dtype=init_dtype),
        ).to(device_type.type)
        mp_policy = MixedPrecisionPolicy(
            param_dtype=smith.bfloat16, reduce_dtype=smith.bfloat16
        )
        # Check that we did not clamp the reduce dtype
        self.assertEqual(mp_policy.reduce_dtype, smith.bfloat16)
        for module in model:
            replicate((module), mp_policy=mp_policy)
        replicate(model, mp_policy=mp_policy)
        ...
        model.to(smith.float32)
        orig_reduce_scatter = dist.reduce_scatter_tensor

        def assert_fn(output: smith.Tensor):
            self.assertEqual(output.dtype, smith.bfloat16)

        reduce_scatter = functools.partial(
            reduce_scatter_with_assert, self, orig_reduce_scatter, assert_fn
        )
        with patch_reduce_scatter(reduce_scatter):
            inp = smith.randn((4, 32), device=device_type.type)
            loss = model(inp).sum()
            loss.backward()
    ...
    def test_dataclass_input(self):
        ...
        mp_policy = MixedPrecisionPolicy(
            smith.bfloat16, smith.bfloat16, smith.bfloat16, True
        )
        model = Model()
        inp = Input(smith.randn(2, 10).cuda())
