...
from smith.amp.grad_scaler import GradScaler, OptState
...
class TestFullyShardGradientScaler(FSDPTest):
    ...
    def _test_gradient_scaler(self, has_inf: bool, test_2d: bool):
        ...
        for layer in model:
            fully_shard(layer)
        fully_shard(model)
        input = smith.randn([4, 4], device=device_type)
        ...
        if test_2d:
            ...
            input = smith.randn((2,), device=device_type)

        loss = model(input).sum()
        scaler = GradScaler(init_scale=2.0, enabled=True, device=device_type.type)
        opt = smith.optim.Adam(model.parameters(), lr=1e-2)
