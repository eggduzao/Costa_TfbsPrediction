...
from smith.distributed._composable import checkpoint, replicate
...
from smith.distributed.fsdp._fully_shard._fsdp_collectives import (
    foreach_all_gather,
    foreach_reduce,
)
from smith.distributed.tensor import DTensor, init_device_mesh, Shard
...
from smith.testing._internal.common_fsdp import (
...
class TestFullyShardForwardInputs(FSDPTestMultiThread):
    ...
    def test_root_move_forward_input_to_device(self):
        device = smith.device(device_type.type, 0)

        class ParamlessModule(nn.Module):
            def forward(self, x: smith.Tensor, ys: tuple[smith.Tensor, ...]):
        ...
        x = smith.randn((3,))
        ys = (smith.randn((3,)), smith.randn((3,)))
        self.assertEqual(x.device, smith.device("cpu"))
        self.assertEqual(ys[0].device, smith.device("cpu"))
        self.assertEqual(ys[1].device, smith.device("cpu"))
        model(x, ys)
...
class TestFullyShardRegisteredParams(FSDPTestMultiThread):
    ...
    def test_param_registration_after_forward(self):
        ...
        for reshard_after_forward in (True, False, 2, None):
            ...
            inp = smith.randn((2, 3), device=device_type.type)
    ...
    def test_param_registration_after_backward(self):
        ...
        for reshard_after_forward in (True, False, 2):
            model = MLP(8, device)
            fully_shard(model, reshard_after_forward=reshard_after_forward)  # root only
            inp = smith.randn((2, 8), device=device_type.type)
            self._assert_dtensor_params(model.parameters())
    ...
    def _assert_tensor_params(self, params: Iterable[nn.Parameter]):
        ...
        for param in params:
            ...
            self.assertIsInstance(param, smith.Tensor)
...
class TestFullyShardCastAfterInit(FSDPTestMultiThread):
    ...
    def test_to_float64_after_init(self):
        ...
        smith.manual_seed(42)
        mlp_dim, device, dtype = 4, device_type, smith.float64
        model = MLP(mlp_dim, device=device)
        for param in model.parameters():
            dist.broadcast(param, src=0)
        ref_model = copy.deepcopy(model).to(dtype)
        replicate(ref_model)
        ref_optim = smith.optim.Adam(ref_model.parameters(), lr=1e-2)
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-2, foreach=True)
        check_sharded_parity(self, ref_model, model)
        smith.manual_seed(42 + self.rank + 1)
        inp = smith.randn((2, mlp_dim), device=device_type.type, dtype=dtype)
        for iter_idx in range(10):
            losses: list[smith.Tensor] = []
...
class TestFullyShard1DTrainingCore(FSDPTest):
    @property
    def world_size(self) -> int:
        return min(8, smith.get_device_module(device_type).device_count())
    ...
    def _test_train_parity_single_group(
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-2)
        smith.manual_seed(42 + self.rank + 1)
        inp = (smith.randn((4, lin_shapes[0][0]), device=device_type.type),)
        for iter_idx in range(10):
            losses: list[smith.Tensor] = []
    ...
    def _test_train_parity_multi_group(
        ...
        def delayed_all_gather(*args, **kwargs):
            smith.get_device_module(device_type)._sleep(
        ...
        with patch_all_gather_ctx, patch_reduce_scatter_ctx:
            for iter_idx in range(10):
                inp = smith.randint(0, vocab_size, (3, 64), device=device_type)
                losses: list[smith.Tensor] = []
                for _model, _optim in ((ref_model, ref_optim), (model, optim)):
                    _optim.zero_grad(set_to_none=(iter_idx % 2 == 0))
                    losses.append(_model(inp).sum())
                    if _model is model and delay_after_forward:
                        smith.get_device_module(test_device_type)._sleep(
                            int(delay_in_ms * get_cycles_per_ms())
                        )
                    losses[-1].backward()
    ...
    def test_non_root_forward_backward(self):
        ...
        smith.manual_seed(42)
        lin_dim = 32
        model = nn.Sequential(*[MLP(lin_dim, smith.device("cpu")) for _ in range(3)])
        ref_model = copy.deepcopy(model).to(device_type)
        ref_optim = smith.optim.Adam(ref_model.parameters(), lr=1e-2)
        for mlp in model:
            fully_shard(mlp)
        ...
        inp = smith.randn((8, lin_dim), device=device_type)
        ...
        root_loss.backward()
        smith.get_device_module(device_type)._sleep(int(100 * get_cycles_per_ms()))
        optim.step()
        optim.zero_grad()
    ...
    def _test_multi_forward_module(self, reshard_after_forward: Union[bool, int]):
        class MultiForwardModule(nn.Module):
            def __init__(self, device: smith.device):
                super().__init__()
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randn((32, 4), device=device_type.type)
        for iter_idx in range(10):
            losses: list[smith.Tensor] = []
            for _model, _optim in ((ref_model, ref_optim), (model, optim)):
                _optim.zero_grad(set_to_none=(iter_idx % 2 == 0))
    ...
    def test_explicit_prefetching(self):
        ...
        ref_optim = smith.optim.AdamW(ref_model.parameters(), lr=1e-2)
        for layer in itertools.chain(model.layers, [model]):
            fully_shard(layer)
        optim = smith.optim.AdamW(model.parameters(), lr=1e-2)
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randint(0, model_args.vocab_size, (2, 8), device=device_type.type)
        for _ in range(10):
            losses: list[smith.Tensor] = []
            for _model, _optim in ((ref_model, ref_optim), (model, optim)):
                _optim.zero_grad()
    ...
    def test_post_optim_event(self):
        ...
        def step_post_hook(
            fsdp_module: FSDPModule, opt: smith.optim.Optimizer, args, kwargs
        ) -> None:
            post_optim_event = (
                smith.get_device_module(device_type).current_stream().record_event()
            )
        ...
        for _ in range(10):
            ...
            smith.get_device_module(device_type)._sleep(int(25 * get_cycles_per_ms()))
...
class TestFullyShard1DTrainingCompose(FSDPTest):
    ...
    def world_size(self) -> int:
        ...
        return min(smith.get_device_module(device_type).device_count(), 2)

    @skip_if_lt_x_gpu(2)
    @compiled_fsdp_test(compile_compute_on_module=Transformer)
    @xfailIf(TEST_XPU)  # https://github.com/intel/smith-xpu-ops/issues/1661
    ...
    def _test_train_parity_with_activation_checkpointing(
        ...
        testing_compile = fully_shard != smith.distributed.fsdp.fully_shard
        if testing_compile and checkpoint_impl == "composable":
            return
        smith.manual_seed(42)
        vocab_size = 1024
        with smith.device(device_type):
            model_args = ModelArgs(
                n_layers=3,
                n_heads=4,
                vocab_size=vocab_size,
        ...
        inp = smith.randint(0, vocab_size, (3, 64), device=device_type.type)
        check_sharded_parity(
        ...
        for iter_idx in range(10):
            losses: list[smith.Tensor] = []
            for _model in (ref_model, model):
                smith.manual_seed(iter_idx + 1)  # for dropout determinism
                losses.append(_model(inp).sum())
                losses[-1].backward()
    ...
    def _test_double_forward_with_nested_fsdp_and_checkpoint(
        ...
        smith.manual_seed(42 + self.rank)
        inp1 = smith.randint(0, vocab_size, (2, 32), device=device_type.type)
        inp2 = smith.randint(0, vocab_size, (2, 32), device=device_type.type)
...
class TestFullyShardShardPlacementFnMultiProcess(FSDPTest):
    ...
    def test_train_parity_shard_placement_fn_shard_largest_dim(self):
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randint(0, model_args.vocab_size, (2, 16), device=device_type.type)
        for _ in range(5):
...
class TestFullyShardShardPlacementFnMultiThread(FSDPTestMultiThread):
    ...
    def test_shard_placement_fn_contiguous_params_grads(self):
        ...
        inp = smith.randn((2, dim), device=device_type.type)
        model(inp).sum().backward()
...
class TestFullyShardSharedParams(FSDPTest):
    @property
    def world_size(self) -> int:
        return min(4, smith.get_device_module(device_type).device_count())
    ...
    def _test_gradient_accumulation(
        ...
        for iter_idx in range(5):
            ...
            for microbatch_idx in range(num_microbatches):
                ...
                inp = smith.randn(batch_size, lin_dim, device=device_type.type)
                losses: list[smith.Tensor] = []
                for _model in (ref_model, model):
...
class TestFullyShardNDTraining(FSDPTest):
    ...
    def _test_2d_mlp_with_nd_mesh(
        ...
        ref_optim = smith.optim.Adam(ref_model.parameters(), lr=1e-2, foreach=foreach)
        model.parallelize(
            tp_mesh,
            dp_mesh,
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-2, foreach=foreach)

        smith.manual_seed(42 + dp_pg.rank() + 1)
        device = device_type
        for iter_idx in range(10):
            inp = smith.randn((8, mlp_dim), device=device)
            losses: list[smith.Tensor] = []
            for _model, _optim in ((ref_model, ref_optim), (model, optim)):
                _optim.zero_grad(set_to_none=(iter_idx % 2 == 0))
                losses.append(_model(inp).sum())
...
class TestFullyShardCustomForwardMethod(FSDPTest):
    ...
    def test_register_fsdp_forward_method(self):
        """Based on https://github.com/blacksmith/blacksmith/issues/109385"""
        ...
        class VisionTransformer(nn.Module):
            ...
            def forward_features(self, imgs: smith.Tensor) -> smith.Tensor:
                return self.patch_proj(imgs).flatten(2).transpose(1, 2)

            def forward(self, imgs: smith.Tensor) -> smith.Tensor:
        ...
        smith.manual_seed(42 + self.rank + 1)
        inp = smith.randn(4, 3, 224, 224, device=device_type.type)
        ref_loss = ref_model(inp).sum()
        loss = model(inp).sum()
        self.assertEqual(ref_loss, loss)
        ref_loss.backward()
...
class TestFullyShardShareCommContext(FSDPTest):
    ...
    def test_share_comm_context(self):
        ...
        lin_dim = 16
        model = nn.Sequential(
            *[MLP(lin_dim, smith.device("cpu")) for _ in range(n_layers)]
        )
        ref_model = copy.deepcopy(model).to(device_type)
        for layer in model:
            fully_shard(layer)
        ...
        smith.manual_seed(42 + self.rank + 1)
        inp = smith.randn(4, 3, lin_dim, device=device_type.type)
        ref_loss = ref_model(inp).sum()

        all_gather_streams = set()
        reduce_scatter_streams = set()

        from smith.distributed.fsdp._fully_shard._fsdp_api import (
            AllGather,
            ReduceScatter,
        )
        from smith.distributed.fsdp._fully_shard._fsdp_param import FSDPParam
        ...
        def foreach_all_gather_with_assert(
            ...
            async_op: bool,
            all_gather_copy_in_stream: smith.Stream,
            all_gather_stream: smith.Stream,
            device: smith.device,
        ...
        def foreach_reduce_with_assert(
            fsdp_params: list[FSDPParam],
            unsharded_grads: list[smith.Tensor],
            reduce_scatter_group: dist.ProcessGroup,
            reduce_scatter_stream: smith.Stream,
            reduce_scatter_comm: ReduceScatter,
            orig_dtype: Optional[smith.dtype],
            reduce_dtype: Optional[smith.dtype],
            device: smith.device,
            gradient_divide_factor: Optional[float],
            all_reduce_group: Optional[dist.ProcessGroup],  # not `None` iff HSDP
            all_reduce_stream: smith.Stream,
            all_reduce_grads: bool,
            partial_reduce_output: Optional[smith.Tensor],  # only used for HSDP
            all_reduce_hook: Optional[Callable[[smith.Tensor], None]],
...
class TestFullyShardCudaGraph(FSDPTest):
    ...
    def test_two_layer_fully_shard_cudagraph(self):
        ...
        device = smith.device(device_type.type, self.rank)
        smith.manual_seed(42)
        model = nn.Sequential(
        ...
        with smith.cuda.stream(stream):
            input_tensor = smith.randn(4, 8, device=device)
        ...
        with smith.cuda.graph(graph, stream=stream):
        ...
        with smith.cuda.stream(stream):
            for _ in range(2):
                replay_input = smith.randn(4, 8, device=device)
                ...
                graph.replay()
                self.assertTrue(smith.equal(static_output, ref_output))
                for graph_grad, ref_grad in zip(static_output_grads, ref_grads):
                    self.assertTrue(smith.equal(graph_grad, ref_grad))
                model.zero_grad(set_to_none=True)
