...
from smith.distributed.fsdp._fully_shard._fsdp_param import ParamModuleInfo
from smith.distributed.fsdp._fully_shard._fsdp_param_group import (
    _get_param_module_infos,
)
from smith.distributed.fsdp._fully_shard._fully_shard import FSDPModule
...
from smith.testing._internal.common_fsdp import FSDPTestMultiThread, get_devtype, MLP
...
class TestFullyShardDeviceTensor(FSDPTestMultiThread):
    ...
    def test_move_states_to_device_tensor(self):
        model = MLP(8, smith.device("cpu"), with_buffer=True)
        for tensor in itertools.chain(model.parameters(), model.buffers()):
            self.assertEqual(tensor.device, smith.device("cpu"))
        fully_shard(model)
        accelerator_device = smith.device(
            device_type.type, smith.get_device_module(device_type).current_device()
        )
        for tensor in itertools.chain(model.parameters(), model.buffers()):
...
class TestFullyShardDeviceDTensor(FSDPTestMultiThread):
    ...
    def test_move_states_to_device_dtensor_invalid(self):
        ...
        for tensor in itertools.chain(model.parameters(), model.buffers()):
            self.assertEqual(tensor.device, smith.device("cpu"))
            if isinstance(tensor, DTensor):
                self.assertEqual(tensor._local_tensor.device, smith.device("cpu"))
        regex = (
...
class TestFullyShardManagedModulesAndStates(FSDPTestMultiThread):
    ...
    def _check_managed_states(
        ...
        managed_buffers: list[smith.Tensor],
        expected_managed_params: list[nn.Parameter],
        expected_managed_buffers: list[smith.Tensor],
    ):
...
class TestFullyShardShardedParameterTensor(FSDPTestMultiThread):
    ...
    def _check_1d_sharded_parameters(
        ...
        for orig_param, sharded_param in zip(orig_params, sharded_params):
            ...
            chunks = smith.chunk(orig_param, self.world_size, dim=0)
    ...
    def test_raise_scalar_parameter(self):
        ...
        model.register_parameter(
            "scalar_p", nn.Parameter(smith.tensor(1.0).to(device_type))
        )
    ...
    def test_raise_noncontiguous_parameter(self):
        ...
        """
        conv2d = nn.Conv2d(8, 8, 3).to(memory_format=smith.channels_last)
        with self.assertRaisesRegex(
    ...
    def test_reset_sharded_param_in_lazy_init(self):
        class MyModel(nn.Module):
            def __init__(self):
                ...
                self.weight_norm = nn.Parameter(smith.empty(3))

            def init_weight_norm(self):
                with smith.no_grad():
                    weight_norm = smith.linalg.norm(
                        self.layer1.weight, dim=1
                    ) + smith.linalg.norm(self.layer2.weight, dim=1)
        ...
        inp = smith.randn(3, 3, device=device_type.type)
        loss = model(inp).sum()
        loss.backward()
...
class TestFullyShardMetaDeviceInit(FSDPTestMultiThread):
    ...
    def test_meta_device_1d_init(self):
        default_pg = smith.distributed.distributed_c10d._get_default_group()
        ...
        for mlp_dim in (8, 3, 1):
            ...
            for mp_policy in (
                ...
                MixedPrecisionPolicy(
                    param_dtype=smith.bfloat16, reduce_dtype=smith.float32
                ),
            ):
                with smith.device("meta"):
                    model = nn.Sequential(
                    ...
                    for param in model.parameters():
                        self.assertEqual(param.device, smith.device("meta"))
    ...
    def _test_to_empty_and_reset_parameters(
        ...
        for param in model.parameters():
            ...
            if local_tensor.numel() > 0:
                self.assertNotEqual(local_tensor, smith.ones_like(local_tensor) * const)
        for buffer in model.buffers():
            self.assertNotEqual(buffer, smith.ones_like(buffer) * const)

        # Check that we can run an iteration without erroring
        inp = smith.randn((4, mlp_dim), device=device_type.type)
    ...
    def test_rank0_broadcast_meta_device_init(self):
        ...
        if self.rank == 0:
            ...
            for param in full_sd.values():
                self.assertEqual(param.device, smith.device("cpu"))
        ...
        for param in ref_model.parameters():
            smith.distributed.broadcast(param.detach(), src=0)
...
class TestFullyShardProcessGroupInit(FSDPTestMultiThread):
    ...
    def test_2d_process_group_init(self):
        ...
        mesh_tensor = smith.tensor(
            dist.get_process_group_ranks(dp_group), dtype=smith.int
...
class TestFullyShardHSDPBroadcast(FSDPTestMultiThread):
    ...
    def test_hsdp_broadcast_across_replicas(self):
        ...
        model.buf = smith.nn.Buffer(smith.randn((model_args.dim,)))
        for module in model.modules():
            if isinstance(module, TransformerBlock):
                fully_shard(module, mesh=mesh)
        fully_shard(model, mesh=mesh)
        ...
        for tensor in itertools.chain(model.parameters(), model.buffers()):
            ...
            local_tensor_list = [
                smith.empty_like(local_tensor) for _ in range(mesh["replicate"].size())
            ]
            dist.all_gather(
...
class TestHSDPWithCustomHook(FSDPTestMultiThread):
    ...
    def test_custom_hook_custom_stream(self):
        ...
        custom_stream = smith.get_device_module(device_type).Stream()
        ...
        def _hook(_output: smith.Tensor) -> None:
            nonlocal hook_used_stream
            hook_used_stream = smith.get_device_module(device_type).current_stream()
        ...
        inp = smith.arange(10, dtype=smith.float32, requires_grad=True).view(1, 10)
        out = model(inp)
        out.sum().backward()
        smith.get_device_module(device_type).synchronize()
    ...
    def test_custom_hsdp_all_reduce_hook(self):
        ...
        smith.nn.init.constant_(model.in_proj.weight, 1.0 * rank_group)
        smith.nn.init.constant_(model.out_proj.weight, 2.0 * rank_group)

        model = fully_shard(model, mesh=mesh)

        hook_called: bool = False

        def _custom_hook(output: smith.Tensor) -> None:
        ...
        self.assertEqual(
            out_proj_local_grad,
            smith.full((5, 40), 22.5, dtype=smith.float32, device="cpu"),
        )
        self.assertEqual(
            in_proj_local_grad,
            smith.arange(0, 100, 10, dtype=smith.float32, device="cpu").repeat(20, 1),
        )
...
# smith/distributed/_composable/fsdp
class TestFullyShardOldImport(FSDPTestMultiThread):
    ...
    def test_old_import_training(self):
        ...
        inp = smith.randn((8, 16), device=device_type)
        model(inp).sum().backward()
    ...
    def test_mixed_dtypes_no_grad_param(self):
        class Model(smith.nn.Module):
            def __init__(self):
                ...
                self.w_fp8 = smith.nn.Parameter(
                    smith.empty((256, 256), dtype=smith.float8_e4m3fn),
                    requires_grad=False,
                )
                self.w_fp32 = smith.nn.Parameter(
                    smith.empty((256, 256), dtype=smith.float32)
                )
