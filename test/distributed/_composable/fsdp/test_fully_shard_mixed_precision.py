...
from smith.distributed.fsdp import fully_shard, MixedPrecisionPolicy
from smith.distributed.fsdp._fully_shard._fsdp_collectives import (
    _get_gradient_divide_factors,
)
from smith.distributed.tensor import Shard
...
class TestFullyShardMixedPrecisionCasts(FSDPTestMultiThread):
    ...
    def test_float16_on_one_submodule(self):
        ...
        fully_shard(model.c2, mp_policy=MixedPrecisionPolicy(param_dtype=smith.float16))
        fully_shard(model)
    ...
    def _test_submodules_with_external_inputs(self, enable_submodule_cast: bool):
        ...
        fully_shard(model, mp_policy=MixedPrecisionPolicy(param_dtype=smith.float16))
        model(x).sum().backward()
    ...
    def test_dataclass_input(self):
        ...
        model = Model()
        inp = Input(smith.randn(2, 10).to(device_type))
