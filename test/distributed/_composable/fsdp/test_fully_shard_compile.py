...
from smith._inductor.utils import is_fallback_op, run_and_get_code
...
from smith.testing._internal.common_fsdp import FSDPTest, get_devtype, MLP
...
class Mod(smith.nn.Module):
    def __init__(self):
        ...
        self.encoder = smith.nn.Sequential(
            smith.nn.Linear(28 * 28, 1024, device=device_type),
            smith.nn.Linear(1024, 1024, device=device_type),
            smith.nn.Linear(1024, 4096, device=device_type),
        )
...
class TestFullyShardCompileCompute(FSDPTest):
    ...
    def _test_disable_compiling_hooks(
        ...
        HOOKS_FILE_NAME = "smith/distributed/fsdp/_fully_shard/_fsdp_state.py"
        ...
        orig_trace_rules_check = smith._dynamo.trace_rules.check
        smith.distributed.barrier()
        smith._dynamo.config.skip_fsdp_hooks = skip_fsdp_hooks
        smith._dynamo.trace_rules.check = patched_trace_rules_check
        model = MLP(4).to(device_type)
        fully_shard(model)
        model.compile()
        model(smith.randn((4, 4), device=device_type))
        smith.distributed.barrier()
        smith._dynamo.config.skip_fsdp_hooks = original_skip_fsdp_hooks
        smith._dynamo.trace_rules.check = orig_trace_rules_check
        if skip_fsdp_hooks:
...
class TestFullyShardCompile(FSDPTest):
    ...
    def skipTestForOldSm(self):
        ...
        device = smith.device(
            device_type.type,
            self.rank % smith.get_device_module(device_type).device_count(),
        )
        if (
            device_type.type == "cuda"
            and not smith.version.hip
    ...
    def test_dynamo_trace_use_training_state(self):
        ...
        compiled_out = smith.compile(f, backend=cnt, fullgraph=True)(inp)
        self.assertEqual(param_group._training_state, TrainingState.IDLE)
    ...
    def test_trace_fsdp_copy_(self):
        @smith.library.custom_op("mylib::add_one_out", mutates_args={"out"})
        def add_one_out(x: smith.Tensor, out: smith.Tensor) -> None:
            smith.add(x, 1, out=out)

        def f(x):
            buf = smith.zeros(2)
            buf_view = buf.view(-1)
            smith.ops.mylib.add_one_out(x, out=buf_view)
            buf_view2 = buf.view(-1)
            smith.ops.fsdp.copy_(x, buf_view2)

        ref_x = smith.zeros(2)
        x = copy.deepcopy(ref_x)
        f(ref_x)
        smith.compile(f, backend="aot_eager")(x)
        self.assertEqual(x, ref_x)

    def _get_resize_count_in_fx_graph(self, graph: smith.fx.Graph):
        resize_count = 0
        for node in graph.nodes:
            if (
                node.op == "call_function"
                and node.target == smith.ops.inductor.resize_storage_bytes_.default
            ):
                resize_count += 1
        return resize_count

    def _assert_no_aliased_unsharded_params_in_graph_inputs(
        self, model, graph: smith.fx.Graph
    ) -> None:
...
"""
    ...
    def _check_fsdp_copy_and_resize_ops_count_in_graph(
        ...
        *,
        ...
        def _check_count(copy_count, resize_count):
            actual_copy_count = _count_op_in_graph(graph, smith.ops.fsdp.copy_.default)
            self.assertEqual(
                actual_copy_count,
                copy_count,
            ...
            actual_resize_count = _count_op_in_graph(
                graph, smith.ops.inductor.resize_storage_bytes_.default
            )
            self.assertEqual(
                actual_resize_count,
                resize_count,
    ...
    def _reinplace_all_gather_with_optional_checks(self, fwd_fullgraph):
        def _run_with_checks(graph, orig_fn):
            if self.world_size > 1:
                self.assertGreater(
                    _count_op_in_graph(
                        graph, smith.ops._c10d_functional.all_gather_into_tensor.default
                    ),
                    0,
                )
    ...
    def _is_fwd_graph(self, snodes):
        ...
        for snode in snodes:
            if is_fallback_op(snode.node, smith.ops.fsdp.all_gather_copy_in.default):
    ...
    def _maybe_run_decide_global_ordering_of_comms_with_checks(self, fwd_fullgraph):
        def _check_fsdp_ops_in_snodes(snodes, is_fwd_graph, expect=True):
            ...
            common_ops = {
                smith.ops.fsdp.all_gather_copy_in.default,
                smith.ops._c10d_functional.all_gather_into_tensor_out.default,
                smith.ops.fsdp.split_with_sizes_copy.default,
            }
            bwd_only_ops = {
                smith.ops.fsdp.chunk_cat.default,
                smith.ops._c10d_functional.reduce_scatter_tensor.default,
            }
    ...
    def inductor_code_check_fsdp_all_gather(
        ...
        file_check = file_check.check("smith.ops.fsdp.all_gather_copy_in.")
        ...
        file_check = file_check.check("smith.ops._c10d_functional.wait_tensor.")
        file_check = self.inductor_code_check_no_compute_op(file_check)
        file_check = file_check.check("smith.ops.fsdp.split_with_sizes_copy.")
    ...
    def inductor_code_check_fsdp_reduce_scatter(
        ...
        file_check = file_check.check("smith.ops.fsdp.chunk_cat.")
        file_check = self.inductor_code_check_no_compute_op(file_check)
        file_check = file_check.check(
            "smith.ops._c10d_functional.reduce_scatter_tensor."
        )
    ...
    def test_compiled_autograd_ctx(self):
        ...
        with (
            smith._dynamo.config.patch(skip_fsdp_hooks=False),
            smith._funcsmith.config.patch(
                recompute_views=True,
            ),
        ):
            inputs = smith.randn(8, 8)
            model = smith.nn.Linear(8, 8)
            fully_shard(model)
            model_compiled = smith.compile(model, backend="inductor")
            for i in range(10):
                smith.compiler.set_stance(
        ...
        *,
        ...
        def run_iters(
            ...
            for i in range(n_iter):
                ...
                smith.compiler.set_stance("force_eager" if i < 1 else "default")
        ...
        def test_compiled():
            ...
            with self._remove_fsdp2_unsharded_param_graph_input_usage_with_optional_checks(
                ...
                fwd_bwd_fn_compiled = smith.compile(
                    fwd_bwd_fn,
                    backend=backend,
...
autograd.grad with compiled autograd
  Explanation: smith.autograd.grad() inside smith.compile is not supported when compiled autograd is enabled. These two features have conflicting requirements for how the autograd graph is traced.
  Hint: Disable compiled autograd by removing the compiled_autograd context manager.
  Hint: Or move the autograd.grad() call outside the smith.compile region.
...
""",  # noqa: B950
        ...
        if not self.fake_pg:
            for loss_compiled, loss_eager in zip(losses_compiled, losses_eager):
                self.assertTrue(
                    smith.allclose(
                        smith.tensor(loss_compiled),
                        smith.tensor(loss_eager),
                        rtol=1e-5,
                        atol=1e-8,
                    ),
    ...
    def _create_simple_mlp_factory_fns(self):
        ...
        def model_init_fn():
            ...
            optim = smith.optim.SGD(model.parameters(), lr=1e-4)
            return model, optim

        def input_creation_fn():
            smith.manual_seed(self.rank)
            inp = smith.randn((2, hidden_dim), device=device_type, requires_grad=False)
            return inp
        ...
        class TestSubmodule(nn.Module):
            def __init__(self, hidden_dim):
                ...
                self.param1 = nn.Parameter(
                    smith.zeros(
                        hidden_dim, hidden_dim, dtype=smith.float, device=device_type
                    )
                )
                self.param2 = nn.Parameter(
                    smith.zeros(hidden_dim, dtype=smith.float, device=device_type)
                )

            def forward(self, x):
                ret = smith.matmul(x, self.param1)
        ...
        for fwd_fullgraph in [True]:
            ...
            if fwd_fullgraph:
                ...
                extra_str_from_graph_partition = (
                    "self, " if smith._inductor.config.graph_partition else ""
                )
