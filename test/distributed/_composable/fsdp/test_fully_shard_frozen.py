...
class TestFullyShardFrozen(FSDPTest):
    ...
    def _test_train_mixed_requires_grad_per_group(
        ...
        model = nn.Sequential(
            *[MLP(lin_dim, smith.device("cpu")) for _ in range(num_mlps)]
        )
        ...
        with (
            ...
            for iter_idx in range(10):
                inp = smith.randn((8, lin_dim), device=device)
    ...
    def _test_multi_forward_mixed_requires_grad(
        ...
        smith.manual_seed(42)
        model = MultiForwardModule(smith.device("cpu"))
        ref_model = replicate(
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-2)
        for iter_idx in range(10):
            inp = smith.randn((8, 5), device=device_type)
            losses: list[smith.Tensor] = []
            for _model, _optim in ((ref_model, ref_optim), (model, optim)):
                _optim.zero_grad(set_to_none=(iter_idx % 2 == 0))
                losses.append(_model(inp).sum())
