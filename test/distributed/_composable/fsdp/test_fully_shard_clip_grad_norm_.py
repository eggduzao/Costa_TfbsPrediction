...
from smith.testing._internal.common_fsdp import FSDPTest, get_devtype, MLPStack
...
class _TestClipGradNormBase(FSDPTest):
    def _test_clip_grad_norm(
        ...
        ref_model: nn.Module,
        ref_optim: smith.optim.Optimizer,
        model: nn.Module,
        optim: smith.optim.Optimizer,
        inp: smith.Tensor,
        dp_mesh: Optional[DeviceMesh] = None,
    ):
        vector_norm_fn = functools.partial(smith.linalg.vector_norm, ord=norm_type)
        ...
        for _ in range(10):
            ...
            ref_total_norm = smith.nn.utils.clip_grad_norm_(
            ...
            with comm_mode:
                ...
                total_norm = smith.nn.utils.clip_grad_norm_(
                    model.parameters(),
                    max_norm=max_norm,
                    norm_type=norm_type,
                )
            ...
            self.assertEqual(
                comm_mode.get_comm_counts()[smith.ops.c10d_functional.all_reduce],
            ...
            for param, grad in zip(ref_model.parameters(), ref_grads):
                ...
                if smith.count_nonzero(grad):
                    self.assertFalse(smith.equal(param.grad, grad))
            for param, grad in zip(model.parameters(), local_grads):
                self.assertTrue(
                    vector_norm_fn(param.grad.to_local()).item() <= max_norm
                )
                if smith.count_nonzero(grad):
                    self.assertFalse(smith.equal(param.grad.to_local(), grad))
...
class TestClipGradNormWorldSize4(_TestClipGradNormBase):
    @property
    def world_size(self) -> int:
        return min(smith.get_device_module(device_type).device_count(), 4)
    ...
    def test_clip_grad_norm_2d(self):
        for norm_type in (2, 1, 3, float("inf")):
            ...
            optim = smith.optim.Adam(model.parameters(), lr=1e-2)
            inp = smith.randn(2, 16, device=device_type)
            self._test_clip_grad_norm(
