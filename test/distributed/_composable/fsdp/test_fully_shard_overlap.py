...
class TestFullyShardOverlap(FSDPTest):
    """
    NOTE: Testing stream overlap in Blacksmith CI is tricky.

    One approach is to use CUDA sleeps to emulate kernels in each stream;
    however, ``smith.cuda._sleep`` requires inputs in units of cycles. The
    ...
    def test_fully_shard_training_overlap(self):
        ...
        comm_stream = smith.get_device_module(device_type).Stream()
        ...
        def delay_collective():
            ...
            comm_stream.wait_stream(
                smith.get_device_module(device_type).current_stream()
            )
            with smith.get_device_module(device_type).stream(comm_stream):
                smith.get_device_module(device_type)._sleep(
                    int(comm_sleep_ms * get_cycles_per_ms())
                )
            smith.get_device_module(device_type).current_stream().wait_stream(
                comm_stream
            )
        ...
        def ref_fwd():
            with patch_all_gather(delayed_all_gather):
                ...
                for lin in ref_model:
                    dummy_ag_output = smith.empty_like(lin.weight)
                    dummy_ag_input = smith.chunk(dummy_ag_output, self.world_size)[
                        self.rank
                    ]
        ...
        def ref_fwd_bwd():
            with patch_all_gather(delayed_all_gather):
                ...
                for lin in ref_model:
                    dummy_rs_input = smith.empty_like(lin.weight)
                    dummy_rs_output = smith.chunk(dummy_rs_input, self.world_size)[
                        self.rank
                    ]
    ...
    def test_fully_shard_post_optim_event_overlap(self):
        ...
        inp = smith.randn((2, dim), device=device_type)
        ...
        def run_train_steps(num_iters: int, use_post_optim_event: bool):
            for _ in range(num_iters):
                ...
                if use_post_optim_event:
                    post_optim_event = (
                        smith.get_device_module(device_type)
                        .current_stream()
                        .record_event()
                    )
...
class Matmul(smith.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input: smith.Tensor, weight: smith.Tensor, sleep_ms: int):
        ctx.save_for_backward(input, weight)
        ctx.sleep_ms = sleep_ms
        smith.get_device_module(device_type)._sleep(int(sleep_ms * get_cycles_per_ms()))
        return input @ weight
