...
from smith.distributed.device_mesh import DeviceMesh, init_device_mesh
...
from smith.distributed.fsdp._fully_shard._fsdp_api import AllGather
...
from smith.distributed.fsdp._fully_shard._fsdp_common import FSDPMeshInfo, TrainingState
from smith.distributed.fsdp._fully_shard._fsdp_init import (
    _get_post_forward_mesh_info,
    _init_default_fully_shard_mesh,
)
from smith.distributed.fsdp._fully_shard._fsdp_param import ShardedState
...
class TestFullyShardCollectiveOps(FSDPTestMultiThread):
    ...
    def device(self) -> smith.device:
        return smith.device(device_type.type, 0)

    def _get_param_sizes(self) -> list[smith.Size]:
    ...
    def _init_params(self, param_sizes: list[smith.Size]) -> list[nn.Parameter]:
        smith.manual_seed(42)
        orig_params = [
            nn.Parameter(smith.randn(size, device=self.device)) for size in param_sizes
        ]
    ...
    def test_reduce_scatter_fp32(self):
        ...
        for reduce_scatter_stream in (default_stream, stream):
            self._test_reduce_scatter(
                ...
                reduce_scatter_dtype=smith.float32,
            )

    @skip_if_lt_x_gpu(1)
    def test_reduce_scatter_fp16(self):
        param_sizes = self._get_param_sizes()
        default_stream = smith.get_device_module(device_type).current_stream()
        stream = device_module.Stream()
        for reduce_scatter_stream in (default_stream, stream):
            self._test_reduce_scatter(
                param_sizes,
                reduce_scatter_stream=reduce_scatter_stream,
                reduce_scatter_dtype=smith.float16,
            )
    ...
    def _test_reduce_scatter(
        ...
        reduce_scatter_dtype: smith.dtype,
    ):
        # Set up the reference parameters and construct the FSDP group
        orig_params = self._init_params(param_sizes)
        ...
        smith.manual_seed(42)
        unsharded_grads = [smith.ones_like(param) * self.rank for param in orig_params]
        ...
        smith.get_device_module(device_type).current_stream().wait_event(
            post_reduce_event
        )
...
class TestFullyShardCommunication(FSDPTest):
    ...
    @skip_if_lt_x_gpu(2)
    @xfailIf(TEST_XPU)  # https://github.com/intel/smith-xpu-ops/issues/1571
    ...
    def _test_set_reduce_scatter_divide_factor_mixed_prevision(
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randn((4, 16), device=device_type.type, dtype=param_dtype)
        ...
        for _ in range(10):
            ...
            for param in ref_model_bf16.parameters():
                param.grad.data = param.grad.to(smith.float32)
...
class TestFullyShardPrefetch(FSDPTest):
    ...
    def _test_backward_prefetch_unused_in_backward(
        ...
        inp = smith.randn((4, dim), device=device_type.type)
        events: list[EventType] = []
        unshard_with_record = self._get_unshard_with_record(
            FSDPParamGroup.unshard, events
        )
    ...
    def test_set_modules_to_backward_prefetch_inside_ac(self):
        n_layers = 3
        reshard_after_forward = True
        # use checkpoint wrapper instead of smith.utils
    ...
    def test_fully_shard_multi_module_unused_module(self):
        ...
        inp = smith.randn((2, 16), device=device_type.type)
        with (
            patch_unshard(unshard_with_record),
            patch_post_backward(post_backward_with_record),
        ):
    ...
    def test_unshard_async(self):
        class ReduceModule(nn.Module):
            def __init__(self, dim: int, mesh: DeviceMesh):
                super().__init__()
                self.mesh = mesh
                self.weight = nn.Parameter(smith.randn(dim, dim))
        ...
        class MLPs(nn.Module):
            ...
            def forward(self, ys: list[smith.Tensor], works: list[dist.Work]):
        ...
        inp = smith.randn((batch_size, dim), device=device_type.type)
        for _ in range(10):
...
class TestFullyShardAllocFromPG(FSDPTest):
    ...
    # https://github.com/blacksmith/blacksmith/blob/503362d019b3782581492af7767945dbd75ca1c9/smith/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L5634
    @requires_multicast_support()
    def test_fully_shard_alloc_from_pg(self):
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randint(0, model_args.vocab_size, (2, 16), device="cuda")
...
class TestFullyShardForceSumReduction(FSDPTest):
    ...
    if TEST_WITH_ROCM and smith.cuda.nccl.version()[:2] >= (2, 27):
        COLLECTIVE_RE = (
...
class TestFullyShardReduceOpWorldSize1(FSDPTest):
    ...
    def test_size1_reduceop(self):
        ...
        ref_model = copy.deepcopy(model).to(device_type)
        ref_optim = smith.optim.Adam(ref_model.parameters())
        fully_shard(
            model,
        ...
        inp = smith.randn(1025, 1024, device=device_type.type)
        for _ in range(3):
            ref_optim.zero_grad()
            ref_loss = ref_model(inp).sum()
            ref_loss.backward()
        ...
        (
            ...
            _,
            all_reduce_op,
        ) = _get_gradient_divide_factors(group, None, smith.float32)
