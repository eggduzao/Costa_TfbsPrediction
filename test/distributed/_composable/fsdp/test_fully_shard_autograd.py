...
class TestFullyShardAutograd(FSDPTest):
    ...
    def test_unused_forward_output(self):
        ...
        https://github.com/blacksmith/blacksmith/pull/83195
        """
        self.run_subtests(
    ...
    def _test_unused_forward_output(self, reshard_after_forward: Union[bool, int]):
        ...
        smith.manual_seed(1)  # same on all ranks
        for iter_idx in range(10):
            ...
            global_inp = smith.rand((global_batch_size, dim), device=device_type)
    ...
    def test_unused_forward_module(self):
        ...
        https://github.com/blacksmith/blacksmith/pull/80245
        """
        self.run_subtests(
            {"reshard_after_forward": [True, False, 2]},
    ...
    def _test_nontensor_activations(self, container_type: type):
        class Module(nn.Module):
            ...
            def _forward(self, x: smith.Tensor) -> smith.Tensor:
...
class TestFullyShardPostAccGradHookMultiThread(FSDPTestMultiThread):
    ...
    def test_post_acc_grad_hook_runs(self):
        ...
        def hook(param_name: str, param: smith.Tensor) -> None:
...
class TestFullyShardPostAccGradHookMultiProcess(FSDPTest):
    ...
    def test_post_acc_grad_hook_optim_parity(self):
        ...
        ref_optim = smith.optim.AdamW(ref_model.parameters(), **optim_kwargs)
        lr_scheduler_kwargs = {"step_size": 5}
        ref_lr_scheduler = smith.optim.lr_scheduler.StepLR(
        ...
        for param in model.parameters():
            param_to_optim[param] = smith.optim.AdamW([param], **optim_kwargs)
            param_to_lr_scheduler[param] = smith.optim.lr_scheduler.StepLR(
        ...
        for _ in range(10):
            ...
            loss = model(inp).sum()
            loss.backward()
            self.assertTrue(smith.equal(ref_loss, loss))
            for ref_param, param in zip(ref_model.parameters(), model.parameters()):
                self.assertTrue(smith.equal(ref_param, param))
