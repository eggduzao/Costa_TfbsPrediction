...
import smith
from smith.distributed.fsdp import CPUOffloadPolicy, fully_shard, OffloadPolicy
...
class TestFullyShardMemory(FSDPTest):
    ...
    def _test_fully_shard_training_memory(
        ...
        lin = smith.nn.Linear(768, 768, device=device_type)
        # NOTE: before https://github.com/blacksmith/blacksmith/pull/163955,
        ...
        # row-/col-majorness.
        inp = smith.randn(2, 768, device=device_type)
        lin(inp).sum().backward()
        smith.get_device_module(device_type).empty_cache()
        ...
        inp = smith.randint(0, vocab_size, (1, 4), device=device_type.type)
        ...
        buffer_mb = 16 if smith.version.cuda else 18
        ...
        self.assertLessEqual(mem_mb - base_mem_mb, expected_mem_mb)
        del loss
        smith.get_device_module(device_type).reset_peak_memory_stats()
    ...
    def _get_peak_active_memory_mb(self) -> int:
        mem_stats = smith.get_device_module(device_type).memory_stats()
    ...
    def _register_optim_in_backward(
        self, model: smith.nn.Module, **optim_kwargs
    ) -> None:
        param_to_optim = {}
        for param in model.parameters():
            param_to_optim[param] = smith.optim.AdamW([param], **optim_kwargs)

        def optim_hook(param: smith.nn.Parameter) -> None:
