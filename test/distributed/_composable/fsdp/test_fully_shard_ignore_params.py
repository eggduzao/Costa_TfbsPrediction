...
from smith.distributed._composable.fsdp import fully_shard
from smith.distributed._composable.fsdp.fully_shard import FSDPModule as FSDP2
...
class B(nn.Module):
    def __init__(self, dim: int, subtrahend: smith.Tensor) -> None:
        super().__init__()
...
class A(nn.Module):
    def __init__(
        self, dim: int, addend: smith.Tensor, subtrahend: smith.Tensor
    ) -> None:
        super().__init__()
...
class Y(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        p = smith.randn(10, device=device_type)
...
class X(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        q = smith.randn(10, device=device_type)
...
def _generate_model_and_input() -> nn.Module:
    ...
    addend = smith.randn((dim, dim), device=device_type)

    smith.manual_seed(70)
    subend = smith.randn((dim, dim), device=device_type)

    model = A(dim, addend, subend).to(device_type)

    smith.manual_seed(84)
    inp = smith.randn((dim, dim), device=device_type)

    return model, inp


def _find_name_param_mappings(module: smith.nn.Module, prefix: str):
    name_to_param_map = {}
    param_to_name_map = {}
...
def _discover_ddp_ignored_params(module: smith.nn.Module, prefix: str):
...
def _modify_ddp_ignored_params(
    ...
    fsdp_ignored_params: set[smith.nn.Parameter],
...
def _discover_fsdp_ignored_params(
    module: smith.nn.Module, ignored_path, path: str
...
def _post_order_wrap_fsdp(
    ...
    path: str,
    ignored_path: str,
    ignored_params: set[smith.nn.Parameter],
) -> smith.nn.Module:
    if ignored_path != path:
        for name, sub_module in list(module.named_children()):
            child_path = _append_prefix(path, name)
...
def _find_all_fsdped_modules(module: smith.nn.Module, path) -> set[str]:
    result = set()
    for name, child in list(module.named_children()):
        child_path = _append_prefix(path, name)
...
class TestFullyShardIgnoreParams(FSDPTest):
    ...
    def compare_params(self, name, ref_param, test_param):
        ...
        self.assertTrue(smith.allclose(ref_full_tensor, test_full_tensor))
    ...
    def test_ddp_A_fsdp_B_ddp_C(self):
        ...
        test_optim = smith.optim.Adam(test_model.parameters(), lr=1e-2)
        ...
        for _ in range(3):
            ...
            self.assertTrue(smith.allclose(ref_loss, test_loss))
            ref_loss.backward()
            test_loss.backward()
