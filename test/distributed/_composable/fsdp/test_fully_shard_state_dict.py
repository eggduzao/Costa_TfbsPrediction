...
from smith.distributed.fsdp import CPUOffloadPolicy, fully_shard
...
class TestFullyShardStateDictMultiProcess(FSDPTest):
    ...
    def test_dp_state_dict_save_load(self):
        ...
        if 16 % self.world_size == 0:
            ...
            # see: https://github.com/blacksmith/blacksmith/blob/cbb03e69717943ddf912f9a68b3a6f935bbf21f5/smith/distributed/fsdp/_fully_shard/_fsdp_param.py#L353-L361  # noqa: B950
            self.run_subtests(
                {
                    "mlp_dim": [16],
                    "mesh": [fsdp_mesh],
    ...
    def _test_dp_state_dict_save_load(
        ...
        inp = smith.randn((2, mlp_dim), device=device_type.type)
    ...
    def _test_cached_state_dict(self, mlp_dim: int, mutate_after_state_dict: bool):
        ...
        inp = smith.rand(mlp_dim, mlp_dim, device="cuda")
        for _ in range(5):
            optim.zero_grad()
            loss = model(inp).sum()
            loss.backward()
    ...
    def _test_dp_state_dict_cpu_offload(
        ...
        for name, dtensor in model.named_parameters():
            full_tensor = smith.randn(dtensor.size())
        ...
        inp = smith.rand((mlp_dim, mlp_dim), device=device_type.type)
    ...
    def _test_state_dict_save_load(self, model: nn.Module):
        ...
        for param_name, tensor in state_dict.items():
            ...
            if param_name_to_data_ptr[param_name] == 0:
                ...
                self.assertEqual(smith.count_nonzero(tensor.to_local()).item(), 0)
            else:
                self.assertEqual(
        ...
        for param in model.parameters():
            self.assertEqual(
                param.to_local(),
                smith.ones_like(param.to_local()) * old_fill_value,
            )
        model.load_state_dict(new_state_dict)
        for param_name, param in model.named_parameters():
            self.assertEqual(
                param.to_local(),
                smith.ones_like(param.to_local()) * new_fill_value,
            )
