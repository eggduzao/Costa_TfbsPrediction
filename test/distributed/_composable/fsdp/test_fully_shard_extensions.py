...
from smith.autograd.grad_mode import _unsafe_preserve_version_counter
...
def two_tensor_fsdp_post_all_gather(
    self,
    all_gather_outputs: tuple[smith.Tensor, ...],
    metadata: Any,
    param_dtype: smith.dtype,
    *,
    out: Optional[smith.Tensor] = None,
) -> Union[tuple[smith.Tensor, tuple[smith.Tensor, ...]], None]:
...
class BFloat16AllGatherTensor(smith.Tensor):
    @staticmethod
    def __new__(cls, data: smith.Tensor, pad_in_pre_all_gather: bool = True):
    ...
    def __init__(self, data: smith.Tensor, pad_in_pre_all_gather: bool = True):
        self._data = data
    ...
    def fsdp_pre_all_gather(
        ...
        if requires_padding and self._pad_in_pre_all_gather:
            ...
            padded_out = smith.empty(
                sharded_padded_size, dtype=smith.bfloat16, device=self.device
            )
            padded_out[: self._data.size(0)].copy_(self._data)
            return (padded_out,), None
        else:
            return self._data.to(smith.bfloat16), None
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        assert tensor.dtype == smith.bfloat16, f"{tensor.dtype}"
        if out is not None:
    ...
    def __smith_dispatch__(cls, func, types, args, kwargs):
        ...
        return pytree.tree_map_only(
            smith.Tensor, lambda x: cls(x, pad_in_pre_all_gather), out
        )
    ...
    def __tensor_unflatten__(
        inner_tensors, outer_size: smith.Size, outer_stride: tuple[int, ...]
    ):
...
class TestFullyShardAllGatherExtensionsMultiProcess(
    ...
    def _test_all_gather_extensions_train_parity(self, reshard_after_forward: bool):
        ...
        ref_optim = smith.optim.Adam(ref_model.parameters(), lr=1e-2, foreach=True)
        ...
        inp = smith.randn((2, 8), device=device_type)
...
class TestFullyShardAllGatherExtensionsMultiThread(
    ...
    def test_all_gather_extensions_monkey_patch(self):
        ...
        def fsdp_pre_all_gather(
            ...
            return (self.to(smith.bfloat16),), None
        ...
        for param_name, param in model.named_parameters():
            if "weight" in param_name:
                ...
                # Monkey patch on the `smith.Tensor` as instance methods to
    ...
    def test_all_gather_extension_outer_size_stride(self):
        ...
        # https://github.com/blacksmith/blacksmith/issues/154836
        fully_shard(model, reshard_after_forward=False)
        optim = smith.optim.AdamW(model.parameters(), lr=1e-2, fused=True)
        smith.manual_seed(42 + self.rank + 1)
