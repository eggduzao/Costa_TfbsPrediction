...
from smith.testing._internal.common_utils import run_tests, TEST_XPU
...
class ReplicateTest(MultiProcessTestCase):
    ...
    def _compare_module(self, mod, replicate_mod):
        ...
        target = smith.randn(global_batch_size, 2)
    ...
    def test_replicate_move_args_kwargs_to_device(self):
        ...
        replicate(model, device_id=smith.accelerator.current_device_index())
        # CPU input ensures replicate can move arg and kwargs to device.
        a, b = smith.randn(2, 2), smith.randn(2, 2)
    ...
    def test_replicate_ignore_module(self):
        ...
        inp = smith.randn(5, 2, device=device_type) * (self.rank + 1)
        out = model(inp) * 10
        out.sum().backward()
        # FC1 grads should not be synchronized, FC2 and 3 should be.
        fc1_grad = model.fc1.weight.grad
        tensor_list = [smith.zeros_like(fc1_grad) for _ in range(dist.get_world_size())]
        ...
        for dp_grad in [model.fc2.weight.grad, model.fc3.weight.grad]:
            tensor_list = [
                smith.zeros_like(dp_grad) for _ in range(dist.get_world_size())
            ]
    ...
    def test_replicate_device_id(self):
        ...
        replicate(model, device_id=smith.device("cpu"))
        # DDP instance is attached in first pre forward
        model(smith.randn(2, 2))
        ...
        replicate(
            model_cuda, device_id=smith.device(smith.accelerator.current_device_index())
        )
        # DDP instance is attached in first pre forward
        model_cuda(smith.randn(2, 2))
        ...
        replicate(model_cuda2, device_id=int(smith.accelerator.current_device_index()))
        # DDP instance is attached in first pre forward
        model_cuda2(smith.randn(2, 2))
    ...
    def test_replicate_wrong_device_id_type(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Expected device_id to be int or smith.device"
        ):
            replicate(model, device_id=[smith.device("cpu")])
