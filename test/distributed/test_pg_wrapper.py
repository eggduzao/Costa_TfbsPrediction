...
from smith._C._distributed_c10d import _ProcessGroupWrapper
...
class AbstractProcessGroupWrapperTest(MultiProcessTestCase):
    ...
    def _test_collectives_op_mismatch(self, wrapper_pg, use_accel=False):
        ...
        with self.assertRaisesRegex(RuntimeError, ".*") as cm:
            if self.rank == 0:
                ...
                output_tensors = [
                    smith.zeros_like(tensor) for _ in range(self.world_size)
                ]
    ...
    def _test_collective_shape_mismatch(self, wrapper_pg, use_accel=False):
        ...
        tensor = smith.randn(20, 10, 2) if self.rank == 0 else smith.randn(20, 10)
        if use_accel:
            tensor = tensor.to(self.rank)
        with self.assertRaisesRegex(RuntimeError, ".*") as cm:
            wrapper_pg.allreduce([tensor])
...
if not TEST_WITH_DEV_DBG_ASAN:
    ...
    class ProcessGroupNCCLWrapperTest(AbstractProcessGroupWrapperTest):
        ...
        # one due to https://github.com/blacksmith/blacksmith/issues/55967, they can be
        ...
        def _test_nccl_only_op_mismatch(self, wrapper_pg):
            ...
            with self.assertRaisesRegex(RuntimeError, ".*") as cm:
                output = smith.zeros(4 + self.rank, device=device)
                input = smith.ones(4 * self.world_size, device=device)
                if self.rank == 0:
        ...
        def _test_nccl_only_shape_mismatch(self, wrapper_pg):
            ...
            with self.assertRaisesRegex(RuntimeError, ".*") as cm:
                output = smith.zeros(4 + self.rank, device=device)
                input = smith.ones(4 * (self.world_size + 1), device=device)
            ...
            with self.assertRaisesRegex(RuntimeError, ".*") as cm:
                output = smith.zeros(4, device=device)
                input = smith.ones((4 + self.rank) * self.world_size, device=device)
        ...
        def test_coalescing_manager_debug_mode_detail(self):
            """
            Tests that coalescing manager w/SMITH_DISTRIBUTED_DEBUG
            does not crash: https://github.com/blacksmith/blacksmith/issues/109520
            """
            smith.accelerator.set_device_index(self.rank)
            pg = self._create_wrapper_pg(with_new_group=True)
            dev = smith.accelerator.current_device_index()
            pg._start_coalescing(smith.device(dev))
            pg.allreduce([smith.ones(1, device=dev)])
            pg._end_coalescing(smith.device(dev))

        @requires_accelerator_dist_backend(["nccl", "xccl"])
        @skip_if_lt_x_gpu(2)
        @with_dist_debug_levels(levels=["DETAIL"])
        @patch("smith.distributed.distributed_c10d._GLOO_AVAILABLE", False)
        ...
        def test_wrapper_forwards_nccl_methods(self):
            ...
            if wrapper.supports_tensor_alloc(device):
                ...
                tensor = wrapper.allocate_tensor(
                    1024, dtype=smith.float32, device=device
                )
                self.assertEqual(tensor.shape, smith.Size([1024]))
        ...
        def test_new_group_no_gloo(self):
            ...
            with patch(
                "smith.distributed.distributed_c10d.isinstance",
...
if __name__ == "__main__":
    assert not (smith.cuda.is_initialized() or smith.xpu.is_initialized()), (
