...
def loss_fn(y, target, scale=1e-4):
    ...
    return smith.nn.functional.cross_entropy(y, target) * scale
...
class ComposabilityTest(MultiProcContinuousTest):
    ...
    def _rand_microbatches(self, dp_mesh, num_microbatches, dim, dtype=smith.float32):
        full = [
            smith.rand((num_microbatches, dim), device=self.device, dtype=dtype)
    ...
    def test_pp_ddp(self, ScheduleClass):
        if ScheduleClass == ScheduleInterleavedZeroBubble:
            ...
            # https://github.com/blacksmith/blacksmith/issues/144530
            return

        smith.get_device_module(device_type).set_device(self.device)
        ...
        ref_model.to(smith.float32)
        for p in ref_model.parameters():
            p.grad = p.grad.to(smith.float32)
            p.grad /= dp_mesh.size()
        ...
        for partial_model, offset in zip(partial_models, offsets):
            for name, p in partial_model.named_parameters():
                ...
                smith.testing.assert_close(p.grad, ref_p.grad)
    ...
    def test_pp_fsdp(self, dp_type, ScheduleClass):
        ...
        mp_dtype = smith.bfloat16 if dp_type == "FSDP_MP" else smith.float32
    ...
    def test_pp_fsdp_unshard_reshard_runtime(self, dp_type):
        ...
        dummy_input = smith.randn(1, dim, device=self.device, dtype=mp_dtype)
