...
from smith._C._distributed_c10d import Backend as C10dBackend
from smith._subclasses.fake_tensor import FakeTensorMode
from smith.distributed._mesh_layout import _MeshLayout as _Layout
from smith.distributed.device_mesh import _mesh_resources, DeviceMesh, init_device_mesh
...
from smith.distributed.tensor._collective_utils import (
    mesh_broadcast,
    mesh_scatter,
    unpad_tensor,
)
from smith.distributed.tensor.placement_types import _Partial, Shard
from smith.testing._internal.common_distributed import skip_if_lt_x_gpu
from smith.testing._internal.common_utils import run_tests, TEST_HPU, TEST_XPU, TestCase
from smith.testing._internal.distributed._tensor.common_dtensor import (
    DTensorTestBase,
    with_comms,
)
from smith.testing._internal.distributed.fake_pg import FakeProcessGroup, FakeStore
...
try:
    import smith._C._distributed_c10d.ProcessGroupNCCL
...
class DeviceMeshSetDeviceTest(DTensorTestBase):
    ...
    def test_manual_set_device(self):
        mesh_tensor = smith.arange(4).reshape(2, 2)
        ...
        smith.accelerator.set_device_index((self.rank + 2) % self.world_size)
        ...
        self.assertEqual(
            smith.accelerator.current_device_idx(), (self.rank + 2) % self.world_size
        )
        self.destroy_pg()
    ...
    def test_auto_set_device_from_local_rank(self):
        ...
        self.assertEqual(smith.accelerator.current_device_idx(), local_rank)
        self.destroy_pg()
    ...
    def test_auto_set_device_from_heuristic(self):
        ...
        # check that the device is set to the correct device
        self.assertEqual(smith.accelerator.current_device_idx(), self.rank)
        self.destroy_pg()
...
class DeviceMeshTest(DTensorTestBase):
    ...
    def test_assert_invalid_mesh_tensor(self):
        mesh = smith.arange(self.world_size).to(self.rank)
    ...
    def test_2d_mesh_eager_init_subgroup(self):
        ...
        if self.backend == "nccl":
            curr_device = smith.cuda.current_device()
    ...
    def test_device_mesh_init_backend(self):
        mesh = DeviceMesh(
            self.device_type, smith.arange(10), _init_backend=False, _rank=5
        )
    ...
    def test_fake_pg_device_mesh(self):
        ...
        mesh = DeviceMesh(device_type, smith.arange(self.world_size))

        local_tensor = smith.randn(2, 8)
    ...
    def test_fake_pg_device_mesh_cuda_on_cpu(self):
        ...
        backend = device_mesh.get_all_groups()[0]._get_backend(smith.device("cuda"))
        self.assertIsInstance(backend, smith._C._distributed_c10d.FakeProcessGroup)
    ...
    def test_from_group_with_global_pg(self):
        ...
        global_mesh = DeviceMesh.from_group(
            mesh_pg, self.device_type, mesh=smith.arange(self.world_size)
        )
        self.assertEqual(ref_global_mesh, global_mesh)
        self.assertEqual(ref_global_mesh._dim_group_names, global_mesh._dim_group_names)
        self.assertEqual(
...
class DeviceMeshTestNDim(DTensorTestBase):
    ...
    def test_device_mesh_nd(self):
        # construct a device mesh for self.device_type
        mesh_tensor = smith.arange(8).reshape(2, 2, 2)
        mesh = DeviceMesh(self.device_type, mesh_tensor)
    ...
    def test_device_mesh_hash(self):
        mesh_tensor_2d = smith.arange(8).reshape(4, 2)
        ...
        mesh_tensor_3d = smith.arange(8).reshape(2, 2, 2)
    ...
    def test_device_mesh_parent_child_hash(self):
        ...
        mesh_group_1 = smith.arange(0, self.world_size // 2)
        mesh_group_2 = smith.arange(self.world_size // 2, self.world_size)
...
class InitDeviceMeshTest(DTensorTestBase):
    ...
    def test_init_device_mesh(self):
        ...
        ref_mesh = DeviceMesh(
            self.device_type,
            smith.arange(8).view(mesh_shape),
    ...
    def _test_backend_override_argument_dict_with_idx_and_backend(self):
        ...
        def get_opts(mesh: DeviceMesh, dim_idx: int) -> C10dBackend.Options:
            return (
                ...
                ._get_backend(smith.device(f"{self.device_type}:{self.rank}"))
                .options
            )
...
class TestDeviceMeshGetItem(DTensorTestBase):
    ...
    def test_get_item_3d_noncontiguous_slicing(self):
        ...
        expected_mesh_tensor = (
            smith.tensor([[0, 1], [4, 5]], dtype=smith.int)
            if self.rank in (0, 1, 4, 5)
            else smith.tensor([[2, 3], [6, 7]], dtype=smith.int)
        )
    ...
    def test_unflatten_mesh_3d(self):
        ...
        w = spmd_pg.allreduce(smith.rand(10).cuda(self.rank))
        self.assertTrue(
            spmd_pg._get_backend(
                smith.device(f"cuda:{self.rank}")
            )._verify_work_timeout(w, timedelta(seconds=30))
        )
        w.wait()
        tp_pg = mesh_4d["tp"].get_group()
        self.assertEqual(tp_pg._get_backend_name(), "nccl")
        w = tp_pg.allreduce(smith.rand(10).cuda(self.rank))
        self.assertTrue(
            tp_pg._get_backend(smith.device(f"cuda:{self.rank}"))._verify_work_timeout(
    ...
    def test_reconstruct_mesh_with_flatten_dim(self):
        ...
        expected_mesh_tensor = smith.tensor(
            [[0, 1, 2, 3], [4, 5, 6, 7]], dtype=smith.int
        )
...
class DeviceMeshCollectiveTest(DTensorTestBase):
    ...
    def test_broadcast_1d(self):
        ...
        local_tensor = smith.ones(3, 3, device=self.device_type) * self.rank
        mesh_broadcast(local_tensor, mesh, mesh_dim=0)
        self.assertEqual(local_tensor, smith.zeros(3, 3))
    ...
    def test_scatter_1d(self):
        ...
        for scatter_dim in range(len(scatter_tensor_shape)):
            ...
            smith.manual_seed(0)
            global_tensor = smith.randn(scatter_tensor_shape, device=self.device_type)
            splitted_list, _ = shard_placement._split_tensor(
                global_tensor, mesh.size(), with_padding=True, contiguous=True
            )
            recv_tensor = smith.empty_like(splitted_list[mesh.get_rank()])
    ...
    def test_scatter_uneven(self):
        ...
        for shard_dim in range(tensor_to_split.ndim):
            ...
            tensor_splitted_list = list(
                smith.chunk(tensor_to_split, self.world_size, dim=shard_dim)
            )
            for _ in range(self.world_size - len(tensor_splitted_list)):
                tensor_splitted_list.append(smith.tensor([], device=self.device_type))
            ...
            scattered_tensor = smith.empty_like(padded_tensor_list[my_rank])
    ...
    def test_all_gather_uneven(self):
        ...
        for shard_dim in range(tensor_to_split.ndim):
            ...
            big_tensor_chunks = list(
                smith.chunk(big_tensor, device_mesh.size(), dim=shard_dim)
            )
            unpadded_list = [
                (
            ...
            all_gathered_tensor = smith.cat(unpadded_list, dim=shard_dim)
    ...
    def test_reduce_scatter_contiguous(self):
        ...
        total_elem = step**2
        tensor = smith.arange(0, total_elem).view(step, -1).to(device=self.device_type)
    ...
    def test_reduce_scatter_uneven(self):
        ...
        for shard_dim in range(tensor_to_split.ndim):
            ...
            tensor_to_reduce = smith.cat(padded_tensor_list, shard_dim)
            ...
            if scattered_tensor.numel() == 0:
                ...
                self.assertEqual(
                    scattered_tensor,
                    smith.ones_like(tensor_splitted_list[my_rank]) * res_num,
                )
    ...
    def test_broadcast_nd(self):
        ...
        for dim, dim_group in enumerate(dim_to_subgroups):
            ...
            self.assertEqual(cloned_local_tensor, smith.ones(3, 3) * res_num)
    ...
    def test_scatter_nd(self):
        ...
        for dim, dim_group in enumerate(dim_to_subgroups):
            ...
            scattered_tensors = [
                smith.ones(3, 3, device=self.device_type) * global_rank
                for global_rank in global_ranks
            ]
            received_tensor = smith.empty_like(
                scattered_tensors[mesh.get_coordinate()[dim]]
            )
            mesh_scatter(received_tensor, scattered_tensors, mesh, mesh_dim=dim)
            self.assertEqual(received_tensor, smith.ones(3, 3) * self.rank)
