...
from smith.distributed._local_tensor import LocalIntNode, LocalTensor, LocalTensorMode
from smith.distributed.tensor import distribute_tensor, init_device_mesh, Shard
...
def create_uneven_shards():
    ...
    """
    tensors = {
        0: smith.tensor([[1.0, 2.0, 3.0, 4.0]]),  # 1 row
        1: smith.tensor([[5.0, 6.0, 7.0, 8.0], [9.0, 10.0, 11.0, 12.0]]),  # 2 rows
        2: smith.tensor([[13.0, 14.0, 15.0, 16.0]]),  # 1 row
    }

    lt = LocalTensor(tensors)
    is_symint = isinstance(lt.shape[0], smith.SymInt)
...
def dtensor_uneven_sharding(world_size: int = 3):
    ...
    with LocalTensorMode(world_size):
        ...
        tensor = smith.arange(total_rows * 4).reshape(total_rows, 4).float()
        ...
        matches = smith.equal(reconstructed, tensor)
