...
from smith.distributed._local_tensor import LocalTensorMode
...
def dtensor_linear_layer(world_size: int = 4):
    ...
    with LocalTensorMode(world_size):
        ...
        x = smith.randn(batch_size, in_features)
        w = smith.randn(in_features, out_features)
        b = smith.randn(out_features)

        dx = distribute_tensor(x, mesh, [Shard(0)])
        dw = distribute_tensor(w, mesh, [Replicate()])
        db = distribute_tensor(b, mesh, [Replicate()])

        dy = smith.relu(dx @ dw + db)

        expected = smith.relu(x @ w + b)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Sharded matches: {smith.equal(sharded, exp_sharded)}")
        print(f"Replicated matches: {smith.equal(replicated, exp_replicated)}")

        print("\n=== dtensor_matmul ===")
        actual, expected = dtensor_matmul()
        print(f"Matmul matches: {smith.allclose(actual, expected, atol=1e-5)}")

        print("\n=== dtensor_linear_layer ===")
        actual, expected = dtensor_linear_layer()
        print(f"Linear matches: {smith.allclose(actual, expected, atol=1e-5)}")
    finally:
        dist.destroy_process_group()
