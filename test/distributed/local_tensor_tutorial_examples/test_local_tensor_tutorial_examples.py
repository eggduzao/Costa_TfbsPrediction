...
class TestExample01BasicOperations(TestCase):
    ...
    def test_create_local_tensor(self):
        ...
        self.assertTrue(smith.equal(lt._local_tensors[0], exp_rank_0))
        self.assertTrue(smith.equal(lt._local_tensors[1], exp_rank_1))

    def test_arithmetic_operations(self):
        """Test arithmetic operations on LocalTensor."""
        (doubled, added), (exp_d0, exp_d1, exp_a0) = arithmetic_operations()

        self.assertTrue(smith.equal(doubled._local_tensors[0], exp_d0))
        self.assertTrue(smith.equal(doubled._local_tensors[1], exp_d1))
        self.assertTrue(smith.equal(added._local_tensors[0], exp_a0))
    ...
    def test_reconcile_identical_shards(self):
        ...
        self.assertTrue(smith.equal(result, expected))
    ...
    def test_access_individual_shards(self):
        ...
        self.assertTrue(smith.equal(shard_0, exp_0))
        self.assertTrue(smith.equal(shard_1, exp_1))
...
class TestExample02CollectiveOperations(TestCase):
    ...
    def test_all_gather_tensors(self):
        ...
        for actual, exp in zip(results, expected):
            self.assertTrue(smith.equal(actual, exp))
    ...
    def test_reduce_scatter_tensors(self):
        ...
        for rank in rank_outputs:
            self.assertTrue(smith.equal(rank_outputs[rank], expected[rank]))
...
class TestExample03DTensorIntegration(TestCase):
    ...
    def test_distribute_and_verify(self):
        ...
        self.assertTrue(smith.equal(sharded_actual, sharded_exp))
        self.assertTrue(smith.equal(replicated_actual, replicated_exp))

    def test_dtensor_matmul(self):
        """Test DTensor matrix multiplication."""
        actual, expected = dtensor_matmul()
        self.assertTrue(smith.allclose(actual, expected, atol=1e-5))
