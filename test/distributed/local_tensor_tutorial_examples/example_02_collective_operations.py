...
from smith.distributed._local_tensor import LocalTensor, LocalTensorMode
...
def all_reduce_sum(process_group):
    ...
    tensors = {
        ...
        2: smith.tensor([[9.0, 10.0], [11.0, 12.0]]),
    }
...
def broadcast_from_rank(process_group, src_rank: int = 0):
    ...
    """
    tensors = {
        0: smith.tensor([10.0, 20.0, 30.0]),
        1: smith.tensor([40.0, 50.0, 60.0]),
        2: smith.tensor([70.0, 80.0, 90.0]),
    }
...
def all_gather_tensors(process_group):
    ...
    """
    tensors = {
        0: smith.tensor([[1.0, 2.0]]),
        1: smith.tensor([[3.0, 4.0]]),
        2: smith.tensor([[5.0, 6.0]]),
    }
    ...
    with LocalTensorMode(frozenset(tensors.keys())):
        ...
        output_list = [smith.zeros_like(lt) for _ in range(num_ranks)]
...
def reduce_scatter_tensors(process_group):
    ...
    with LocalTensorMode(frozenset(tensors.keys())):
        ...
        lt_output = smith.zeros(output_shape, dtype=lt_input.dtype)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, (r, e) in enumerate(zip(results, expected)):
            print(f"  [{i}] {smith.equal(r, e)}")

        print("\n=== reduce_scatter_tensors ===")
        results, expected = reduce_scatter_tensors(pg)
        for rank in results:
            print(f"  Rank {rank}: {smith.equal(results[rank], expected[rank])}")
    finally:
        dist.destroy_process_group()
