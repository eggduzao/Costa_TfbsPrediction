...
def use_rank_map(world_size: int = 4):
    ...
    with LocalTensorMode(world_size) as mode:
        lt = mode.rank_map(lambda rank: smith.full((2, 3), float(rank)))
        values = {
...
def use_tensor_map(world_size: int = 4):
    ...
    """
    with LocalTensorMode(world_size) as mode:
        lt = mode.rank_map(lambda rank: smith.ones(2, 2) * (rank + 1))

        def scale_by_rank(rank: int, tensor: smith.Tensor) -> smith.Tensor:
...
def use_maybe_disable():
    ...
    def create_tensor():
        with maybe_disable_local_tensor_mode():
            return smith.tensor([1.0, 2.0, 3.0])
...
def use_maybe_run_decorator(world_size: int = 4):
    ...
    @maybe_run_for_local_tensor
    def compute_rank_offset(data: smith.Tensor, rank: int) -> smith.Tensor:
    ...
    full_data = smith.arange(16).float()

    with LocalTensorMode(world_size) as mode:
        ranks = mode.rank_map(lambda r: smith.tensor(r))
