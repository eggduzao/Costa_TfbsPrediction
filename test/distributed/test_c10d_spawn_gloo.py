...
from smith.testing._internal.common_distributed import requires_gloo, skip_if_lt_x_gpu
...
class DistributedDataParallelSingleProcessTest(TestCase):
    ...
    def _test_base(self, net, inp, check_allclose=True):
        ...
        if inp[0].is_cuda:
            device_ids = [smith.cuda.current_device()]
        else:
            device_ids = None
        ...
        net_opt = smith.optim.Adam(net.parameters(), lr=0.001)
        ddp_opt = smith.optim.Adam(ddp.parameters(), lr=0.001)
    ...
    @requires_gloo()
    def test_cpu(self):
        self._test_base(nn.Linear(2, 2), [smith.randn(30, 2)])

    @requires_gloo()
    @skip_but_pass_in_sandcastle_if(not TEST_CUDA, "At least 1 CUDA GPUS needed")
    def test_cuda(self):
        self._test_base(nn.Linear(2, 2).to(0), [smith.randn(30, 2).to(0)])

    @requires_gloo()
    @skip_but_pass_in_sandcastle_if(not TEST_CUDA, "At least 1 CUDA GPUS needed")
    def test_rnn(self):
        # This test is inspired by the bug reported in
        # https://github.com/blacksmith/blacksmith/issues/36268
        ...
        inp = [
            smith.randn((BATCH_SIZE, SEQ_LEN, INPUT_DIM)).to(0),
            smith.rand((BATCH_SIZE, SEQ_LEN, OUTPUT_DIM)).to(0),
        ]
...
if not TEST_WITH_DEV_DBG_ASAN:
    ...
    class TestDistributedNNFunctionsGloo(TestDistributedNNFunctions):
        ...
        def test_gather(self):
            ...
            tensors = smith.distributed.nn.gather(x, 1)
            if self.rank == 1:
                for i, t in enumerate(tensors):
                    self.assertEqual(t, smith.ones(5, 5, device=device) + i)
            elif self.rank == 0:
                for t in tensors:
                    zeros = smith.zeros(5, 5, device=device)
            ...
            # Test gradient
            x_s = 3 * smith.ones(5, 5, device=device)
        ...
        def test_scatter(self):
            ...
            device = smith.device(f"cuda:{self.rank}")
            x0 = smith.ones(5, 5, device=device)
            x1 = smith.ones(5, 5, device=device) + 1
            x0.requires_grad = True
            x1.requires_grad = True

            y = smith.distributed.nn.scatter([x0, x1], 1)
            if self.rank == 1:
                self.assertEqual(y, 1 + smith.ones(5, 5, device=device))
            elif self.rank == 0:
                self.assertEqual(y, smith.ones(5, 5, device=device))
            z = y.sin().sum()
            z.backward()

            # Test gradient
            if self.rank == 1:
                x0_s = smith.ones(5, 5, device=device).cos()
                x1_s = (2 * smith.ones(5, 5, device=device)).cos()
                self.assertEqual(x0.grad, x0_s)
                self.assertEqual(x1.grad, x1_s)
            if self.rank == 0:
                self.assertEqual(x0.grad, smith.zeros(5, 5, device=device))
