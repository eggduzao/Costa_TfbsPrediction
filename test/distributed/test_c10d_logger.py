...
from smith.distributed.c10d_logger import _c10d_logger, _exception_logger
...
from smith.testing._internal.common_distributed import DistributedTestBase, TEST_SKIPS
...
WORLD_SIZE = min(4, max(2, smith.get_device_module(device_type).device_count()))
...
def with_comms(func=None):
    ...
    def wrapper(self, *args, **kwargs):
        if smith.get_device_module(device_type).device_count() < self.world_size:
    ...
    def test_exception_logger(self) -> None:
        ...
        with self.assertLogs(_c10d_logger, level="DEBUG") as captured:
            ...
            if self.backend(device_type) == dist.Backend.NCCL:
                ...
                nccl_ver = smith.cuda.nccl.version()
                self.assertEqual(
