...
from smith.distributed.distributed_c10d import _rank_not_in_group
...
class TestFSDPHybridShard(FSDPTest):
    @property
    def world_size(self):
        return max(smith.accelerator.device_count(), 2)
    ...
    def test_hsdp_save_load_state_dict(self):
        model = MyModel().to(device_type)
        num_node_devices = smith.accelerator.device_count()
        shard_rank_lists = (
        ...
        model(smith.randn(2, 10)).sum().backward()
        optim.step()
        ...
        load_optim = smith.optim.AdamW(load_model.parameters())
    ...
    def _test_fsdp_hybrid_shard_basic_setup(
        ...
        for fsdp_module in hsdp_model.fsdp_modules(hsdp_model):
            ...
            # https://github.com/blacksmith/blacksmith/issues/90857
        ...
        with (
            ...
            inp = hsdp_model.get_input(device=smith.accelerator.current_device_index())
    ...
    def _test_fsdp_hybrid_shard_parity(
        ...
        hsdp_optim = smith.optim.Adam(hsdp_model.parameters(), lr=1e-2)
        smith.manual_seed(global_pg.rank() + 1)
        for _ in range(5):
