...
from smith.distributed.fsdp import ShardingStrategy
...
class TestClipGradNorm(FSDPTest):
    ...
    def test_non_root(self, device):
        ...
        # fsdp_model(smith.randn((2, 5), device=smith.device(self.device_type))).sum().backward()
        fsdp_model(smith.randn((2, 5), device=device_type)).sum().backward()
    ...
    def test_ddp_parity(self, device):
        ...
        DDP with ``smith.nn.utils.clip_grad_norm_()` when using full precision.
        """
        self.run_subtests(
            {
                "device": [device],
    ...
    def _test_ddp_parity(
        ...
        device = smith.device(self.device_type)
        LARGE_FACTOR = 100
        ...
        ddp_total_norm = smith.nn.utils.clip_grad_norm_(
        ...
        for param, orig_grad in zip(ddp_model.parameters(), orig_ddp_grads):
            assert not smith.equal(param.grad, orig_grad)
        ...
        if offload_params:
            ...
            # https://github.com/blacksmith/blacksmith/issues/89133
            return
        # Run a few more iterations
        # TODO: We cannot run too many iterations, or else there is drift:
        # https://github.com/blacksmith/blacksmith/issues/89136
        for i in range(3):
    ...
    def _test_low_precision_grads(
        ...
        inp = fsdp_model.module.get_input(smith.device(self.device_type))
        out = fsdp_model(*inp)
        out.sum().backward()
        for param in fsdp_model.parameters():
        ...
        self.assertEqual(total_norm.dtype, smith.float16)
        ...
        for param in fsdp_model.parameters():
            if param.grad is not None:
                self.assertTrue(
                    smith.linalg.vector_norm(param.grad, norm_type).item() <= max_norm,
                )
    ...
    def _test_no_gradients(self, device, use_orig_params: bool):
        ...
        inp = smith.randn(32, 24, device=self.device_type)
        fsdp_module(inp)
        with self.assertWarnsRegex(
            expected_warning=UserWarning,
            expected_regex="on rank "
            rf"{self.rank} with no gradients -- returning the total "
            "norm in the default dtype smith.float32",
        ):
            total_norm = fsdp_module.clip_grad_norm_(1)
        self.assertEqual(total_norm.dtype, smith.float32)
        self.assertEqual(total_norm, smith.tensor(0.0, device=self.device_type))
