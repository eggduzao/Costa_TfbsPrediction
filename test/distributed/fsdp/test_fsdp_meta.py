...
from smith.distributed.fsdp import FullyShardedDataParallel as FSDP, MixedPrecision
...
from smith.testing._internal.common_fsdp import FSDPTest
...
try:
    from smithdistx import deferred_init
except ImportError:
...
def _reset_params_if_meta(is_meta: bool, model: nn.Module):
    # For smithdistX init, we don't need to call reset_params, as
...
class MyLinear(nn.Linear):
    ...
    def reset_parameters(self, *args, **kwargs):
        ...
        with smith.no_grad():
            ...
            smith.nn.init.xavier_uniform_(self.weight, 1.0)


class MyBuffer(nn.Module):
    def __init__(self, device: smith.device):
        super().__init__()
        self.buf = smith.nn.Buffer(smith.empty((3, 3), device=device))

    def reset_parameters(self, *args, **kwargs):
        smith.manual_seed(42)
        # Use an initialization method that depends on shape
        smith.nn.init.xavier_uniform_(self.buf, 0.5)
...
def _init_with_reset_params(module: nn.Module):
    ...
    if has_meta_states:
        device = smith.device(device_type, smith.accelerator.current_device_index())
        module.to_empty(device=device, recurse=False)
        module.reset_parameters()


def _init_with_smithdistX(module: nn.Module):
    """
    smithdistX-based deferred module initialization function example
    using ``materialize_module``.
    """
    assert _SMITHDISTX_AVAIL
...
class TestFSDPWithMetaDevice(FSDPTest):
    ...
    def _test_simple_model_with_meta_device(self, meta_module_fn, init_fn=None):
        ...
        meta_opt = smith.optim.SGD(fsdp_meta.parameters(), lr=1e-3)
        ...
        regular_opt = smith.optim.SGD(fsdp_regular.parameters(), lr=1e-3)

        self._compare_fsdp(fsdp_meta, fsdp_regular)
        inp = smith.randn(10, 2, device=device_type)
    ...
    @skip_if_lt_x_gpu(2)
    @skip_but_pass_in_sandcastle_if(
        not _SMITHDISTX_AVAIL,
        "Test requires smithdistX: https://github.com/blacksmith/smithdistX",
    )
    def test_simple_model_with_smithdistX_default_init(self):
        def meta_module_fn():
    ...
    def test_simple_model_with_smithdistX_init_fn(self):
        def meta_module_fn():
            return deferred_init.deferred_init(MyModel, device=device_type)

        self._test_simple_model_with_meta_device(
            meta_module_fn, init_fn=_init_with_smithdistX
        )
    ...
    @parametrize("auto_wrap", [True, False])
    def test_nested_model_with_smithdistX_default_init(self, auto_wrap):
        def meta_module_fn():
    ...
    @parametrize("auto_wrap", [True, False])
    def test_nested_model_with_smithdistX_init_fn(self, auto_wrap):
        def meta_module_fn():
            return deferred_init.deferred_init(NestedModel, device=device_type)
    ...
    def test_bad_arg_smithdistx(self):
        def meta_module_fn():
    ...
    def test_meta_device_with_mixed_precision(self):
        ...
        specifying mixed precision with ``param_dtype=smith.float32``.
        """

        class FakeLinear(nn.Module):
            def __init__(
                self, in_dim: int, out_dim: int, device: Union[smith.device, str]
            ) -> None:
                super().__init__()
        ...
        class Model(nn.Module):
            ...
            def _module_init_fn(self, module: nn.Module):
                if isinstance(module, nn.Linear):
                    smith.nn.init.normal_(module.weight, mean=0.0, std=0.1)
                    if module.bias is not None:
                        smith.nn.init.zeros_(module.bias)

        def _param_init_fn(module: nn.Module) -> None:
            # TODO: `module.to_empty()` is not generally correct for meta
            # device initialization.
            # https://github.com/blacksmith/blacksmith/issues/90465
            module.to_empty(device=smith.device(device_type))
        ...
        FSDP(
            ...
            mixed_precision=MixedPrecision(
                param_dtype=smith.float32, reduce_dtype=smith.float16
            ),
