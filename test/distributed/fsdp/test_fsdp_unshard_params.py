...
from smith.distributed.fsdp.fully_sharded_data_parallel import FLAT_PARAM
...
class TestUnshardParamsBase(FSDPTest):
    ...
    def _test_unshard_params_writeback(
        ...
        **fsdp_kwargs: dict[str, Any],
        ...
        with FSDP.summon_full_params(model, writeback=writeback), smith.no_grad():
    ...
    def _get_test_unshard_params_writeback_config(self) -> dict[str, list[Any]]:
        return {
            ...
            "mixed_precision": [MixedPrecision(param_dtype=smith.float16), None],
            "cpu_offload": [
            ...
            if not rank0_only or self.rank == 0:
                for p1, (n2, p2) in zip(
                    ...
                    if (
                        ...
                        self.assertEqual(smith.device("cpu"), p2.device)
                    else:
...
class TestUnshardParams(TestUnshardParamsBase):
    ...
    def _test_unshard_params_respects_reshard(
        ...
        # backward
        output = model(smith.zeros(5, device=device_type.type))
        self.assertEqual(
            expected_outer_flat_param_unsharded_numel,
    ...
    def _test_named_parameters_and_buffers(self, prefix: str, recurse: bool):
        ...
        model.buffer = nn.Buffer(smith.ones(1))
        ...
        fsdp_model.buffer = nn.Buffer(smith.ones(1))
    ...
    def _test_with_grads_core(
        ...
        def _check_grads(
            ddp_model: DDP,
            fsdp_model: FSDP,
            old_fsdp_grads: Optional[list[smith.Tensor]],
        ):
            """
            ...
            for old_grad, new_grad in zip(old_fsdp_grads, new_fsdp_grads):
                if writeback_persists:
                    smith.testing.assert_close(old_grad * WRITEBACK_FACTOR, new_grad)
                else:
                    smith.testing.assert_close(old_grad, new_grad)
            if writeback_persists:
        ...
        with FSDP.summon_full_params(fsdp_model):
            for p1, p2 in zip(ddp_model.module.parameters(), fsdp_model.parameters()):
                assert smith.all(smith.isclose(p1, p2))
...
class TestUnshardParamsErrors(TestUnshardParamsBase):
    ...
    def test_unshard_params_from_forward_raises(self):
        class MyModule(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.a = nn.Parameter(smith.zeros(5))
    ...
    def test_unshard_params_from_backward_raises(self):
        ...
        output = model(smith.ones(2, device=device_type.type))
