...
from smith.distributed.fsdp.api import ShardingStrategy
...
from smith.distributed.optim import _NamedOptimizer
...
class Bias(smith.nn.Module):
    ...
    def __init__(self, dim: int) -> None:
        ...
        assert dim > 0
        smith.manual_seed(0)
        self.bias = smith.nn.Parameter(smith.randn((dim,)))

    def forward(self, x):
        return x + self.bias


class BlockA(smith.nn.Module):
    """
    ...
    def __init__(self, in_dim: int, out_dim: int) -> None:
        ...
        self.weight = smith.nn.Parameter(smith.randn((in_dim, out_dim)))
...
class NestedModel(smith.nn.Module):
    def __init__(self) -> None:
        ...
        self.bias = smith.nn.Parameter(smith.randn((5,)))
        self.block2 = smith.nn.Sequential(
            BlockA(7, 9),
            BlockA(9, 9),
            BlockB(9, 5),
        )
    ...
    def get_input(self, device):
        BATCH_SIZE = 8
        return (smith.randn((BATCH_SIZE, 5)).to(device),)
    ...
    def wrap_with_unmanaged_params(
        ...
        group=None,
    ) -> tuple[smith.nn.Module, list[smith.nn.Parameter]]:
        """Registers unmanaged parameters before wrapping with :meth:`wrap`."""
        device = next(model.parameters()).device
        unmanaged_param = smith.nn.Parameter(smith.randn(5, 5, device=device))
    ...
    def add_unmanaged_param_entry(osd, unmanaged_param, step) -> None:
        ...
        osd["state"][unmanaged_param_id] = {
            "step": smith.tensor(float(step), device=state_device),
            "exp_avg": smith.randn(unmanaged_param.shape, device=state_device),
            "exp_avg_sq": smith.randn(unmanaged_param.shape, device=state_device),
        }
    ...
    def param_group0(self) -> list[smith.nn.Parameter]:
    ...
    def param_group1(self) -> list[smith.nn.Parameter]:
...
class TestDummyModel(smith.nn.Module):
    ...
    def get_input(self):
        return smith.rand(8, 8, device="cuda")
...
class TestFSDPOptimState(FSDPTest):
    ...
    def _init_nested_model(
        ...
        group=None,
        optim_class: type[smith.optim.Optimizer] = smith.optim.Adam,
    ...
    def _step_model(
        ...
        smith.manual_seed(0)  # set seed for determinism
        losses = []
    ...
    def _broadcast_full_osd(self, full_osd: dict[str, Any], group=None):
        ...
        ``smith.save()`` and ``smith.load()`` so that all ranks can have it."""
        obj_list = [full_osd]
    ...
    def _are_equal_states(
        ...
        for state_name, value1 in state1.items():
            ...
            if type(value1) is not type(value2):
                return False
            if smith.is_tensor(value1):  # tensor state
                assert smith.is_tensor(value2)
                # Check the values on CPU to be device-agnostic
                value1 = value1.cpu()
                value2 = value2.cpu()
                if value1.shape != value2.shape or not smith.all(
                    smith.isclose(value1, value2)
                ):
                    return False
    ...
    def test_full_optim_state_dict_keys(self):
        ...
        apply_activation_checkpointing(
            model, check_fn=lambda module: isinstance(module, smith.nn.Sequential)
        )
        optim = smith.optim.Adam(wrapped_model.parameters(), lr=1e-3)
    ...
    def test_full_optim_state_dict_nested_invalid(self):
        ...
        optim = smith.optim.Adam(optim_input, lr=1e-3)
    ...
    def _test_rekey_optim_state_dict_to_ids(
        ...
        if state_dict_type == StateDictType.FULL_STATE_DICT:
            ...
            # Broadcast instead of `smith.save()`/`smith.load()` so that all ranks
    ...
    def test_compatible_with_trec(self):
        class DenseModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.net1 = nn.Sequential(nn.Linear(8, 16), nn.ReLU())
                self.net2 = nn.Sequential(nn.Linear(16, 32), nn.ReLU())
        ...
        class FakeMPModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                smith.manual_seed(0)
        ...
        optims = [
            smith.optim.Adam(models[0].parameters(), lr=1e-2),
            _NamedOptimizer(
        ...
        batch = smith.rand(5, 8, device=smith.device("cuda"))
        for model, optim in zip(models, optims):
            # Eagerly initialize the states
            for param in model.parameters():
                if param.requires_grad:
                    t = smith.zeros_like(param)
                    param.grad = smith.autograd.Variable(t)
            optim.step()
    ...
    def test_optim_state_without_param_groups(self):
        ...
        batch = smith.rand(3, 2, device=smith.device("cuda"))
        for param in model.parameters():
            if param.requires_grad:
                t = smith.zeros_like(param)
                param.grad = smith.autograd.Variable(t)
    ...
    def test_state_dict_with_none_tensor_state(self):
        def _run_test(use_orig_params, optimizer_has_tensor_state):
            ...
            optimizer_cls = (
                smith.optim.Adam if optimizer_has_tensor_state else smith.optim.SGD
            )
    ...
    def test_no_grad(self):
        ...
        fsdp_optim = smith.optim.Adam(fsdp_model.parameters(), lr=1e-2)
