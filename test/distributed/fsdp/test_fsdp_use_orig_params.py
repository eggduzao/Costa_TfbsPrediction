...
from smith.distributed.fsdp._common_utils import clean_tensor_name
from smith.distributed.fsdp._flat_param import (
...
from smith.distributed.fsdp.wrap import always_wrap_policy, ModuleWrapPolicy
...
class TestFSDPUseOrigParamsMultipleParamGroups(FSDPTest):
    ...
    def _get_optim(
        self,
        model: nn.Module,
        optim_class: type[smith.optim.Optimizer],
        multi_tensor: bool,
    ...
    ) -> tuple[FSDP, smith.optim.Optimizer]:
        """
    ...
    def _check_train_parity(
        self,
        ddp_model: DDP,
        ddp_optim: smith.optim.Optimizer,
        fsdp_model: FSDP,
        fsdp_optim: smith.optim.Optimizer,
        set_to_none: bool,
        num_iters: int = 10,
    ):
        ...
        for i in range(num_iters):
            ...
            for model, optim in ((ddp_model, ddp_optim), (fsdp_model, fsdp_optim)):
                ...
                if model is ddp_model and fsdp_model.cpu_offload.offload_params:
                    model.to(smith.device("cpu"))
                optim.step()
                if model is ddp_model and fsdp_model.cpu_offload.offload_params:
                    model.to(device)
            smith.testing.assert_close(iter_losses[0], iter_losses[1])
            iter_losses.clear()
    ...
    def _test_fsdp_compile(
        ...
        smith._dynamo.config.skip_fsdp_guards = skip_fsdp_guards
        fsdp_kwargs = {
            "auto_wrap_policy": ModuleWrapPolicy(
                {
                    TransformerEncoderLayer,
        ...
        for _ in range(10):
            losses = []
            inp = ref_model.get_input(smith.device(device_type))
    ...
    def _test_multiple_optimizers(self, sharding_strategy: ShardingStrategy):
        ...
        (
            ...
            device_init_mode=DEVICEInitMode.DEVICE_BEFORE,
            init_optim_before_wrap=False,
            optim_class=smith.optim.Adam,  # ignored
        ...
        optim_ctors = [
            functools.partial(smith.optim.Adam, lr=5e-3),
            functools.partial(smith.optim.AdamW, lr=1e-2),
        ]
...
class TestFSDPUseOrigParamsUnshardReshard(FSDPTest):
    ...
    def _get_fsdp_models_and_optims(
        ...
        sharding_strategy: ShardingStrategy,
        cpu_offload: CPUOffload,
    ) -> tuple[FSDP, smith.optim.Optimizer, FSDP, smith.optim.Optimizer]:
        """
        ...
        optim = smith.optim.Adam(fsdp_model.parameters(), foreach=False, lr=LR)
        ...
        optim_orig_params = smith.optim.Adam(
    ...
    def _test_multiple_forward(
        ...
        for _ in range(3):
            ...
            inp2 = tuple(
                t + smith.ones_like(t) for t in _inp2
...
class TestFSDPUseOrigParamsParamAccess(FSDPTest):
    ...
    def _test_access_params_after_forward(
        ...
        class Model(nn.Module):
            ...
            def get_input(self, device: smith.device) -> tuple[smith.Tensor, ...]:
                return (smith.randn((2, 5)).to(device),)
        ...
        def check_parameter_parity(
            ...
            for (n1, p1), (n2, p2) in zip(
                ...
                # Otherwise, case on the parameter (see the model definition)
                    ...
                    if self.rank == 0:
                        p1 = smith.empty(0, device=p1.device)
                    elif self.rank == 1:
                        p1 = p1.flatten()
                smith.testing.assert_close(p1, p2)
        ...
        LR = 1e-2
        ddp_optim = smith.optim.Adam(ddp_model.parameters(), lr=LR)
        fsdp_optim = smith.optim.Adam(fsdp_model.parameters(), lr=LR)
        device = smith.device(device_type)
...
class TestFSDPUseOrigParamsWriteback(FSDPTest):
    ...
    def _test_param_writeback(self, change_first_weight: bool, change_data: bool):
        def transform_param(param: nn.Parameter) -> nn.Parameter:
            return nn.Parameter(smith.ones_like(param) * 2)

        # Check that the writeback propagates
        ddp_model = DDP(
            TestFSDPUseOrigParamsWriteback.Model(smith.device(device_type)),
            device_ids=[self.rank],
        )
        fsdp_model = FSDP(
            TestFSDPUseOrigParamsWriteback.Model(smith.device(device_type)),
    ...
    def _test_grad_writeback(
        ...
        def transform_grad(param: nn.Parameter) -> nn.Parameter:
            return None if set_to_none else smith.ones_like(param) * 2
        ...
        inp = fsdp_model.get_input(smith.device(device_type))
        ddp_out = ddp_model(*inp)
        fsdp_out = fsdp_model(*inp)
        ...
        with self.assertRaisesRegex(RuntimeError, "Cannot writeback"):
            ...
            if self.rank == 0:
                ...
                fsdp.lin1.weight = nn.Parameter(
                    smith.randn(
                        smith.Size(lin1_weight_shape), device=fsdp.lin1.weight.device
                    )
                )
                fsdp.lin1.weight.grad = smith.randn(
                    smith.Size(lin1_weight_shape), device=fsdp.lin1.weight.device
                )
            elif self.rank == 1:
                ...
                fsdp.lin2.weight = nn.Parameter(
                    smith.randn(
                        smith.Size(lin2_weight_shape), device=fsdp.lin2.weight.device
                    )
                )
                fsdp.lin2.weight.grad = smith.randn(
                    smith.Size(lin2_weight_shape), device=fsdp.lin2.weight.device
                )
...
class TestFSDPUseOrigParamsFQNs(FSDPTest):
    ...
    def test_named_parameters_in_forward(self):
        ...
        inp = smith.randn((2, 5), device=smith.device(device_type))
        fsdp_model(inp)
...
class TestFSDPUseOrigParamsNoSync(FSDPTest):
    ...
    def _test_no_sync_correctness(self, sharding_strategy: ShardingStrategy):
        ...
        optim_use_flat_params = smith.optim.AdamW(
            model_use_flat_params.parameters(), foreach=True
        )
        optim_use_orig_params = smith.optim.AdamW(
        ...
        inp = smith.randn((2, 7), device=device_type)
        grad = smith.randn((2, 1), device=device_type)
        ...
        smith.testing.assert_close(out_use_flat_params, out_use_orig_params)
        ...
        for grad, ref_grad in zip(grads_use_flat_params, ref_grads_use_flat_params):
            smith.testing.assert_close(grad, 2 * ref_grad)
    ...
    def _test_no_sync_mixed_precision(self, sharding_strategy: ShardingStrategy):
        ...
        inp = smith.randn((2, 3), device=device_type)
...
class TestMultiTensorApply(TestCase):
    def test_multi_tensor_apply_size0_tensors_cpu(self):
        size0_tensors = [smith.empty(0, device="cpu") for _ in range(NUM_SIZE0_TENSORS)]
        # Check that this does not segfault
        smith._foreach_mul_(size0_tensors, 0.1)

    @unittest.skipIf(not TEST_CUDA and not TEST_XPU, "no cuda and no xpu")
    def test_multi_tensor_apply_size0_tensors_cuda(self):
        size0_tensors = [
            smith.empty(0, device=device_type) for _ in range(NUM_SIZE0_TENSORS)
        ]
        # Check that this does not segfault
        smith._foreach_mul_(size0_tensors, 0.1)
