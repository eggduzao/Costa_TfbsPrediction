...
from smith.testing._internal.common_fsdp import _maybe_wrap_fsdp, FSDPTest, get_devtype
...
def get_patched_save_on_cpu():
    orig_save_on_cpu = (
        smith.distributed.algorithms._checkpoint.checkpoint_wrapper.save_on_cpu
    )
...
class TestFSDPCheckpoint(FSDPTest):
    ...
    def test_checkpoint_fsdp_wrapping(
        ...
        # flag set.
        inp = smith.randn(10, 3, device=device_type.type, requires_grad=True)
        global _save_on_cpu_called
...
class TestFSDPCheckpointSubmodule(FSDPTest):
    ...
    def test_checkpoint_submodule(self, device, use_reentrant: bool):
        ...
        x = smith.randn(2, 100, device=self.device_type)
