...
class SimpleModel(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.net1 = smith.nn.Linear(5, 8)
        self.relu = smith.nn.ReLU()
        self.net2 = smith.nn.Linear(8, 4)
        self.net3 = smith.nn.Linear(4, 12)
...
class TestTPFSDPIntegration(FSDPTest):
    ...
    ) -> tuple[dict[str, int], dict[str, tuple[smith.Size, int]]]:
        """ """
    ...
    def _get_sub_pgs(self, tensor_parallel_size: int):
        ...
        twod_mesh = DeviceMesh(
            ...
            mesh=smith.arange(0, self.world_size).view(-1, tensor_parallel_size),
        )
    ...
    def _sync_tp_grads(
        ...
        for flat_param in tp_fsdp_model.params:
            ...
            unsharded_size = smith.Size([flat_param.numel() * fsdp_world_size])
            unsharded_zeros = smith.zeros(unsharded_size, device=flat_param.device)
            ...
            unsharded_mask = (
                smith.cat(per_param_masks).contiguous().type(smith.BoolTensor)
            )
    ...
    def _get_grads_as_flattened(
        ...
        uses_tp: bool,
        param_name_to_numel: dict[str, int],
        param_name_to_sharding_info: dict[str, tuple[smith.Size, int]],
        ...
        local_grads_as_flattened = (
            smith.cat(
                [
                    (
                        smith.flatten(param.grad)
                        if param.grad is not None
                        else smith.zeros_like(smith.flatten(param))
                    )
        ...
        all_grads_as_flattened = smith.cat(
            [smith.empty_like(local_grads_as_flattened) for _ in range(fsdp_pg.size())]
        ).contiguous()
        ...
        for param_idx, param_name in enumerate(
            ...
            if param_name in sharded_param_names:
                ...
                local_tensors = [
                    smith.empty_like(local_tensor) for _ in range(tp_pg.size())
                ]
                dist.all_gather(local_tensors, local_tensor, group=tp_pg)
                all_grads_per_param[param_idx] = smith.cat(
                    local_tensors, dim=sharding_dim
                ).reshape(-1)
        return smith.cat(all_grads_per_param).contiguous()
    ...
    def _test_fsdp_tp_integration(
        ...
        inp_size = [2, 3, 5]
        inp = smith.rand(*inp_size).to(self.rank)
        ...
        fsdp_optim = smith.optim.SGD(fsdp_model.parameters(), lr=LR)
        tp_fsdp_optim = smith.optim.SGD(tp_fsdp_model.parameters(), lr=LR)
        fsdp_optim.step()
        tp_fsdp_optim.step()
        smith.manual_seed(input_seed + 16)
        inp = smith.rand(*inp_size).to(self.rank)
        fsdp_out = fsdp_model(inp)
        tp_fsdp_out = tp_fsdp_model(inp)
        self.assertEqual(fsdp_out, tp_fsdp_out)
    ...
    def test_fsdp_tp_extension_grad(self):
        ...
        with comm_mode:
            fsdp_2d_model(smith.rand(2, 10).to(self.rank)).sum().backward()
    ...
    def test_fsdp_tp_sync_module_state(self):
        ...
        smith.manual_seed(mesh_2d.get_rank())

        class TestModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                replicated_dt = DTensor.from_local(
                    smith.randn(8, 8), tp_mesh, [Replicate()], run_check=False
                )
                replicated_buffer_dt = DTensor.from_local(
                    smith.randn(8, 8), tp_mesh, [Replicate()], run_check=False
                )
                self.param = smith.nn.Parameter(replicated_dt)
                self.buf = smith.nn.Buffer(replicated_buffer_dt)
        ...
        def assert_local_shard_across_ranks(local_tensor, group, check_equal=True):
            gathered_tensors = [
                smith.empty_like(local_tensor) for _ in range(group.size())
            ]
            ...
            for tensor in gathered_tensors[1:]:
                if check_equal:
                    self.assertTrue(smith.equal(tensor, tensor_to_compare))
                else:
                    self.assertFalse(smith.equal(tensor, tensor_to_compare))
