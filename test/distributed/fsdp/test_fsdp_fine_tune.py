...
from smith.distributed.fsdp import BackwardPrefetch, CPUOffload, MixedPrecision
...
class ModelUnusedInput(nn.Module):
    ...
    def forward(self, frozen_input, learnable_input):
        ...
        return smith.concat([x, y, z, learnable_input])
...
class TestFSDPFineTune(FSDPTest):
    ...
    def _test_backward_reshard_hooks(
        ...
        orig_post_backward_reshard = (
            smith.distributed.fsdp._runtime_utils._post_backward_reshard
        )
    ...
    def _test_hooks_multi_traversal(
        ...
        fsdp_optim = smith.optim.Adam(fsdp_seq.parameters(), lr=1e-2)
        ddp_optim = smith.optim.Adam(ddp_seq.parameters(), lr=1e-2)
        ...
        for _ in range(6):
            ...
            for seq, optim in ((fsdp_seq, fsdp_optim), (ddp_seq, ddp_optim)):
                ...
                optim.step()
                optim.zero_grad()
            smith.testing.assert_close(losses[0], losses[1])
            losses.clear()
    ...
    def _test_parity_with_ddp(
        ...
        losses = []
        for _ in range(6):
            inp = smith.randn((8, 5), device=device_type.type)
            for seq, optim in ((fsdp_seq, fsdp_optim), (ddp_seq, ddp_optim)):
                loss = seq(inp).sum()
                losses.append(loss)
                loss.backward()
            ...
            if TEST_CUDA:
                ...
                smith.testing.assert_close(losses[0], losses[1], atol=1e-03, rtol=1e-03)
            losses.clear()
    ...
    def _test_parity_with_non_frozen_fsdp(
        ...
        model_optim = smith.optim.Adam(model.parameters(), lr=1e-2)
        ref_model_optim = smith.optim.Adam(
            [
                param
        ...
        losses = []
        for _ in range(6):
            frozen_input = smith.randn((4, 4), device=device_type, requires_grad=False)
