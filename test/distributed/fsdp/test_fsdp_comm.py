...
class TestCommunication(FSDPTest):
    ...
    def test_communication(
        ...
        num_iters = 3
        with (
            patch("smith.distributed.all_gather_into_tensor") as mock_all_gather,
            patch("smith.distributed.reduce_scatter_tensor") as mock_reduce_scatter,
        ):
...
class TestExplicitUnshard(FSDPTest):
    ...
    def test_unshard_async(self, device, use_orig_params: bool):
        ...
        inp = smith.randn((batch_size, dim), device=device_type)
        for _ in range(10):
