...
from smith.distributed.fsdp import CPUOffload, FullyShardedDataParallel as FSDP
...
class TestGradAcc(FSDPTest):
    ...
    def _test_grad_acc(
        ...
        # but permuted
        def permute_tensor(x: smith.Tensor):
            return x.view(-1)[smith.randperm(x.numel())].view_as(x)

        batch: tuple[smith.Tensor, ...] = fsdp_model.module.get_input(device)
        batches: list[tuple[smith.Tensor, ...]] = [batch]
        ...
        for batch1, batch2 in itertools.combinations(batches, r=2):
            for t1, t2 in zip(batch1, batch2):
                assert not smith.all(t1 == t2), (
                    "Check the test to make sure that batches are distinct"
                )

        # Concatenate the batches along the given batch dimension
        concat_batch: tuple[smith.Tensor, ...] = tuple(
            smith.cat(ts, dim=batch_dim) for ts in zip(*batches)
        )
        ...
        smith.testing.assert_close(ref_loss, acc_loss)
        ...
        for ref_grad, acc_grad in zip(ref_grads, acc_grads):
            ...
            smith.testing.assert_close(ref_grad, acc_grad)
