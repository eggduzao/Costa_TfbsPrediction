...
from smith.distributed.fsdp._flat_param import _FSDP_USE_UNSAFE_SETATTR
from smith.distributed.fsdp._runtime_utils import HOMOGENEOUS_ATTR_NAMES
...
class TestFSDPMiscMultiProcess(FSDPTest):
    ...
    def test_fsdp_device_id(self, use_index):
        ...
        Tests the FSDP ``device_id`` argument:
          ...
          without specifying a device ID (i.e. ``smith.device("cuda")``) warns
        """
        dev_id = (
            smith.accelerator.current_device_index()
            if use_index
            else smith.device(device_type, smith.accelerator.current_device_index())
        )
        ...
        def _check_device_matches(module, device_id):
            ...
            if use_index and not isinstance(device_id, smith.device):
                device = smith.device(device_type, device_id)
            else:
                device = device_id
        ...
        _check_device_matches(nested_wrapped_module, dev_id)
        # Check that passing in `smith.device("cuda")` for a GPU module warns
        ...
        with context:
            nested_wrapped_module = NestedWrappedModule.init(
                ...
                FSDPInitMode.RECURSIVE,
                DEVICEInitMode.DEVICE_BEFORE,
                fsdp_kwargs={"device_id": smith.device(device_type)},
            )
        _check_device_matches(
            nested_wrapped_module,
            smith.device(device_type, smith.accelerator.current_device_index()),
        )
    ...
    def test_fsdp_zero2_eval_with_prefetch(self):
        ...
        class Mnist(nn.Module):
            ...
            def forward(self, x, y):
                ...
                x = self.conv2(x)
                x = smith.nn.functional.relu(x)
                x = smith.nn.functional.max_pool2d(x, 2)
                x = self.dropout1(x)
                x = smith.flatten(x, 1)
                x = self.ln(x)
                x = self.fc1(x)
                ...
                x = self.dropout2(x)
                x = self.fc2(x)
                output = smith.nn.functional.log_softmax(x, dim=1)
                loss = smith.nn.functional.cross_entropy(output, y)
                return loss
        ...
        fsdp_opt = smith.optim.SGD(fsdp_model.parameters(), lr=1e-4)
        ddp_opt = smith.optim.SGD(ddp_model.parameters(), lr=1e-4)

        seed = self.rank + 20231010
        smith.manual_seed(seed)
        smith.get_device_module(device_type).manual_seed(seed)

        losses = []
        grads = []
        for i in range(5):
            x = smith.randn(8, 1, 28, 28, device=device_type).requires_grad_()
            y = smith.randint(low=0, high=9, size=(8,), device=device_type)
            for model, opt in ((fsdp_model, fsdp_opt), (ddp_model, ddp_opt)):
                ...
                grads.append(x.grad)
                opt.zero_grad()
            assert smith.allclose(losses[0], losses[1])
            assert smith.allclose(grads[0], grads[1])
            losses.clear()
            grads.clear()
        ...
        with smith.no_grad():
            ...
            for _ in range(5):
                ...
                assert smith.allclose(fsdp_loss, ddp_loss)
    ...
    def test_fsdp_module_no_compute_grad(self, use_second_layer, sharding_strategy):
        ...
        x = smith.randn(10, 10, device=device_type)
        y = smith.randn(10, 10, device=device_type)
        for _ in range(4):
            if use_second_layer:
                a, _ = fsdp(x, y)
            else:
    ...
    def _test_fsdp_not_all_outputs_used_in_loss(
        ...
        def _check_equal(local, fsdp):
            with FSDP.summon_full_params(fsdp):
                for p1, p2 in zip(fsdp.parameters(), local.parameters()):
                    smith.testing.assert_close(p1, p2)
        ...
        opt = smith.optim.SGD(m.parameters(), lr=1e-3)
        opt_local = smith.optim.SGD(local_m.parameters(), lr=1e-3)

        for i in range(6):
            t = smith.ones(4, device=device_type)
            a, b = m(t)
    ...
    def test_fsdp_optim_overlap_no_use_orig_params_error(self):
        ...
        inp = smith.randn(10, 10, device=device_type)
    ...
    def test_fsdp_optimizer_overlap(self):
        ...
        for cpu_offload in [True, False]:
            ...
            for i in range(6):
                inp = smith.randn(2, 2, device=device_type)
                with smith.no_grad():
                    inp_clone = inp.clone()
    ...
    def test_fsdp_cpu_init_stays_on_cpu(self):
        ...
        self.assertEqual(1, len(devices))
        self.assertEqual(smith.device("cpu"), devices.pop())
        ...
        # CUDA device.
        inp = fsdp_model.module.get_input(device=smith.device("cpu"))
    ...
    def test_cpu_init_with_sync_module_states(self):
        ...
        nested_wrapped_module.buf = nn.Buffer(
            smith.ones((2, 2), device="cpu") * self.rank
        )
        nested_wrapped_module.module[0].buf = nn.Buffer(
            smith.ones((3, 2), device="cpu") * self.rank
        )
        nested_wrapped_module = FSDP(
            nested_wrapped_module,
            self.process_group,
            auto_wrap_policy=ModuleWrapPolicy({nn.Linear}),
            device_id=smith.accelerator.current_device_index(),
            sync_module_states=True,
        )
        ...
        self.assertEqual(nested_wrapped_module.buf, smith.zeros((2, 2)))
        self.assertEqual(
            nested_wrapped_module.module.module[0].buf.device,
            smith.device(device_type, smith.accelerator.current_device_index()),
        )
        self.assertEqual(
            nested_wrapped_module.module.module[0].buf, smith.zeros((3, 2))
        )
...
class TestFSDPMiscMultiThread(FSDPTestMultiThread):
    ...
    def test_fsdp_namedtuple(self):
        ...
        m = MyModule().to(device=device_type)
        m = FSDP(m)
        t = smith.ones(1, device=device_type, requires_grad=True)
        ...
        # https://github.com/blacksmith/blacksmith/issues/83107 and
        # https://github.com/blacksmith/blacksmith/issues/83129
    ...
    def test_cpu_gpu_module(self):
        ...
        fsdp = FSDP(cpu_gpu, device_id=smith.accelerator.current_device_index())
        for param in fsdp.parameters():
            self.assertEqual(
                param.device, smith.device(smith.accelerator.current_device_index())
            )
    ...
    def test_fsdp_ignored_module_meta(self):
        ...
        m = FSDP(
            ...
            param_init_fn=lambda m: m.to_empty(
                device=smith.accelerator.current_device_index(), recurse=False
            ),
        )
        self.assertEqual(meta_device, next(m.a.parameters()).device)
    ...
    def test_fsdp_device_id_no_move_ignored_params_and_bufs(self):
        class CPUGPUModule(nn.Module):
            def __init__(self) -> None:
                ...
                self.a = nn.Linear(1, 1)
                self.b = nn.Linear(1, 1)
                self.a.buf = smith.nn.Buffer(smith.ones(1))
        ...
        for t in chain(ignored_params, ignored_bufs):
            self.assertEqual(smith.device("cpu"), t.device)
    ...
    def test_no_params(self):
        ...
        # Test CPU + device_id
        no_params = nn.ReLU()
        FSDP(no_params, device_id=smith.accelerator.current_device_index())
        ...
        # is computed as smith.cuda.current_device when there are no params.
        no_params = nn.ReLU().to(device=device_type)
        context = (
            (
                self.assertRaisesRegex(
    ...
    def test_fsdp_same_model_across_ranks(self):
        ...
        class MyModel(nn.Module):
            def __init__(self, rank):
                ...
                smith.get_device_module(device_type).manual_seed(rank)
                self.lin = nn.Linear(10, 10, bias=False)
                self.buffer = nn.Buffer(smith.ones(1) * rank)
    ...
    def _test_homogeneous_attributes(self, attr_name_and_values: tuple[str, Any, Any]):
        ...
        with self.assertRaisesRegex(
            ...
            inp = fsdp_model.module.get_input(smith.device(device_type))
            fsdp_model(*inp)
    ...
    def test_fsdp_unsupported_module_cls(self):
        ...
        model = nn.ModuleList([MLP(8, smith.device("cpu")) for _ in range(3)])
        with self.assertWarnsRegex(UserWarning, regex):
            FSDP(model, device_id=device_type)
        model = nn.ModuleDict(
            {"1": MLP(8, smith.device("cpu")), "2": MLP(8, smith.device("cpu"))}
        )
        with self.assertWarnsRegex(UserWarning, regex):
            FSDP(model)
...
class TestFSDPMiscWorldSize1(FSDPTestMultiThread):
    ...
    def test_training_device_mismatch_errors(self):
        ...
        fsdp_model.to(smith.device(device_type))
        inp = smith.randn((2, 10))
        with self.assertRaisesRegex(
            RuntimeError,
    ...
    def _test_unsafe_setattr(self, use_orig_params: bool):
        ...
        class SetattrLinear(nn.Module):
            def __init__(self, in_dim: int, out_dim: int, device: smith.device) -> None:
                super().__init__()
                self.weight = nn.Parameter(
                    smith.randn((in_dim, out_dim), device=device)
                )
        ...
        module = SetattrLinear(5, 5, smith.device(device_type))
        fsdp_module = FSDP(module, use_orig_params=use_orig_params)
        inp = smith.randn((8, 5), device=smith.device(device_type))
        called_setattr_override = False
        fsdp_module(inp)
