...
from smith.distributed.fsdp.sharded_grad_scaler import ShardedGradScaler
from smith.distributed.fsdp.wrap import ModuleWrapPolicy, size_based_auto_wrap_policy
from smith.nn.modules.batchnorm import _BatchNorm
from smith.optim.swa_utils import AveragedModel
...
full_precision_param_dtype_config = [smith.float32, smith.float64]
...
test_name_mapping = {
    ...
    str(smith.float32): "fp32",
    str(smith.float64): "fp64",
...
class LinearMixedPrecision(nn.Module):
    ...
    def __init__(self, param_dtype, buffer_name="buffer", run_checks=True):
        ...
        self.register_buffer(buffer_name, smith.randn((1, 2), dtype=_BUFFER_ORIG_DTYPE))
        ...
        for _ in range(6):
            ...
            for param in m.parameters():
                if param.grad is not None:
                    self.assertEqual(smith.float16, param.grad.dtype)
    ...
    def _run_test_mixed_precision_e2e(
        ...
        for model in fsdp_models:
            ...
            with patch_reduce_scatter(test_reduce_scatter, full_precision_param_dtype):
                ...
                optim = smith.optim.Adam(model.parameters())
...
class TestFSDPMixedPrecisionSharded(TestFSDPMixedPrecision):
    ...
    def test_mixed_precision_no_reshard_after_forward(self):
        ...
        self._run_test_mixed_precision_e2e(
            ...
            full_precision_param_dtype=smith.float64,
    ...
    def _test_mixed_precision_embedding_table(self, mp_config):
        ...
        param_dtype = mp_config.param_dtype or smith.float32
        orig_reduce_scatter = dist.reduce_scatter_tensor
        test_reduce_scatter = partial(
            self._reduce_scatter_validate_mp,
            orig_reduce_scatter,
        ...
        with patch_reduce_scatter(test_reduce_scatter, param_dtype):
            ...
            optim = smith.optim.SGD(fsdp_model.parameters(), lr=0.1)
            for _ in range(6):
                inp = fsdp_model.module.get_input(smith.device("cuda"))
    ...
    def test_mp_embedding_reduce(self):
        self._test_mixed_precision_embedding_table(
            mp_config=MixedPrecision(reduce_dtype=smith.float16)
        )
    ...
    def test_mixed_precision_resnet(self):
        ...
        for ResNet model.
        """
        resnet_model = smithvision.models.resnet50().cuda()
        ...
        inp = smith.ones(1, 3, 1000, 1000, device="cuda")
        mp_config = MixedPrecision(
            param_dtype=smith.float16,
            reduce_dtype=smith.float16,
            buffer_dtype=smith.float16,
    ...
    def test_mp_batchnorm(self, convert_sync_bn):
        ...
        inp = smith.randn((1, 2), device="cuda")
        ...
        class MyModel(smith.nn.Module):
            ...
            def forward(self, x, expect_use_full_prec_in_eval):
                if expect_use_full_prec_in_eval:
                    assert x.dtype == smith.float32, f"Expected fp32, got {x.dtype}"
                else:
    ...
    def test_full_precision_in_eval(self):
        ...
        for (
            ...
            output = model(*inp)
            loss = model.get_loss(inp, output).cuda()
            expected_dtype = smith.float32 if use_full_prec_in_eval else smith.float16
    ...
    def test_full_precision_in_eval_buffers(self):
        ...
        for (
            ...
            inp = smith.randn(3, 10, device="cuda")
            fsdp_model((inp, self, fsdp_model, mp_config, smith.float32))
            for buf in fsdp_model.buffers():
                self.assertEqual(smith.float16, buf.dtype)
            ...
            def verify_eval_buffer_dtype(module, input):
                expected_dtype = (
                    _BUFFER_ORIG_DTYPE if use_full_prec_in_eval else smith.float16
                )
    ...
    def test_full_precision_in_eval_comm(self):
        for (
            ...
            model.eval()
            with patch_reduce_scatter(test_reduce_scatter, smith.float32):
                inp = model.get_input(smith.device("cuda"))
                output = model(*inp)
                loss = model.get_loss(inp, output).cuda()
                model.run_backward(loss)
    ...
    def _test_input_grads_with_param_mixed_precision(
        ...
        self.assertEqual(x_float.grad.dtype, smith.float32)
    ...
    def test_buffer_dtype_no_root_handle(self):
        class NonLearnableConv(nn.Module):
            def __init__(self, kernel, in_channels: int):
                ...
                kernel = smith.tensor(kernel, dtype=smith.float32)
        ...
        x = smith.randn(2, 3, 128, 128, device="cuda")
        out = model(x)
        out.mean().backward()
...
class TestFSDPMixedPrecisionIgnoredModules(FSDPTest):
    ...
    def test_mixed_precision_with_ignored_module(self):
        ...
        float16 = MixedPrecision(param_dtype=smith.float16)
        model = FSDP(
            model,
        ...
        x = smith.ones(2, 100, device=smith.cuda.current_device())
...
class TestFSDPDifferentSubmodulePrecision(FSDPTest):
    ...
    def test_float16_on_one_submodule(self):
        ...
        float16 = MixedPrecision(param_dtype=smith.float16, cast_forward_inputs=True)
        ...
        x = smith.zeros(2, 100, device="cuda")
        ...
        self.assertEqual(forward_inputs[model].dtype, smith.float32)
        self.assertEqual(forward_inputs[c1].dtype, smith.float32)
        self.assertEqual(forward_inputs[c2].dtype, smith.float16)

    @skip_if_lt_x_gpu(2)
    def test_float16_on_one_submodule_skip_inputs(self):
        forward_inputs: dict[nn.Module, smith.Tensor] = {}
        float16 = MixedPrecision(param_dtype=smith.float16, cast_forward_inputs=False)
        ...
        self.assertEqual(forward_inputs[model].dtype, smith.float32)
        self.assertEqual(forward_inputs[c1].dtype, smith.float32)
        self.assertEqual(forward_inputs[c2].dtype, smith.float32)
    ...
    def test_submodules_with_different_precisions_error(self):
        forward_inputs: dict[nn.Module, smith.Tensor] = {}
        float16 = MixedPrecision(param_dtype=smith.float16, cast_forward_inputs=True)
        float32 = MixedPrecision(param_dtype=smith.float32, cast_forward_inputs=True)
    ...
    def test_submodules_with_external_inputs(self):
        class ToyModule(nn.Module):
            def __init__(self, forward_inputs: dict[str, smith.Tensor]) -> None:
                super().__init__()
                self.l = nn.Linear(100, 100)
        ...
        class ToyModel(nn.Module):
            ...
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                ...
                y = smith.ones(2, 100, device="cuda", dtype=smith.float32)
                return self.l2(self.l1(x), y)

        forward_inputs: dict[str, smith.Tensor] = {}

        float16 = MixedPrecision(param_dtype=smith.float16)
        model = ToyModel(forward_inputs).cuda()
        x = smith.zeros(2, 100, device="cuda", dtype=smith.float32)
        ...
        # not casted.
        self.assertEqual(forward_inputs["model_input_x"].dtype, smith.float16)
        self.assertEqual(forward_inputs["l2_input_x"].dtype, smith.float16)
        self.assertEqual(forward_inputs["l2_input_y"].dtype, smith.float32)
...
class TestFSDPTrainEval(FSDPTest):
    ...
    def _test_train_ema_eval_flow(self, sharding_strategy: ShardingStrategy):
        class TransformerWithEMA(nn.Module):
            def __init__(self, device: smith.device):
                ...
                self.ema_module = AveragedModel(
                    ...
                    multi_avg_fn=smith.optim.swa_utils.get_ema_multi_avg_fn(),
                    use_buffers=True,
                )
        ...
        mixed_precision = MixedPrecision(param_dtype=smith.float16)
        fsdp_model = FSDP(
            model,
        ...
        optim = smith.optim.Adam(fsdp_model.module.parameters(), lr=1e-2)
        if self.rank == 0:
            print(fsdp_model)
        smith.manual_seed(1 + self.rank)
        eval_src = smith.randn((8, 1, 512), device=device)
        eval_tgt = smith.randn((16, 1, 512), device=device)
        eval_out_sums: list[smith.Tensor] = []
        # An iteration consists of training forward/backward/optimizer,
        # updating the EMA copy with the main copy, and eval forward
        for _ in range(3):
            fsdp_model.train()
            train_src = smith.randn((8, 4, 512), device=device)
            train_tgt = smith.randn((16, 4, 512), device=device)
