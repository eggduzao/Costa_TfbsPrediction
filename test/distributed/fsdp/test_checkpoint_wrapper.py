...
class CheckpointWrapperTest(TestCase):
    def test_load_activation_checkpointed_module(self):
        ...
        for p1, p2 in zip(lin.parameters(), lin_new.parameters()):
            ...
            self.assertTrue(smith.allclose(p1, p2))
    ...
    def test_checkpoint_wrapper_kwarg_support(self):
        ...
        for wrapper in [
            ...
            with self.subTest(wrapper=wrapper):
                ...
                inp = smith.ones(4, 10, requires_grad=True)
    ...
    def test_checkpoint_wrapper_args_kwargs(self):
        ...
        # kwargs test
        smith_utils_checkpoint = smith.utils.checkpoint.checkpoint
        m = checkpoint_wrapper(
            smith.nn.Linear(1, 1),
            checkpoint_fn=smith_utils_checkpoint,
            use_reentrant=False,
            context_fn=get_ctx_mgrs,
        )
        m(smith.randn(2, 1)).sum().backward()
    ...
    def test_checkpoint_wrapper_parity(self):
        ...
        smith.utils.checkpoint (with the same reentrant config)
        ...
        def test(use_checkpointing, use_wrapper, use_reentrant):
            ...
            x = smith.randn(10000, 256, requires_grad=True).to(device_type.type)
            smith.get_device_module(device_type.type).reset_peak_memory_stats()
            loss = a(x).sum()
            loss.backward()
            return smith.get_device_module(device_type.type).max_memory_allocated()
    ...
    def test_apply_activation_checkpointing(self):
        ...
        for i, wrapper in enumerate(
            ...
            with self.subTest(wrapper=wrapper):
                ...
                inp = smith.randn(4, 10, requires_grad=True)
                for _ in range(6):
                    # Kwarg input
    ...
    def test_checkpoint_wrapper_cpu_offload(self):
        ...
        orig_init = smith.autograd.graph.saved_tensors_hooks.__init__
        smith.autograd.graph.saved_tensors_hooks.__init__ = patched_init

        model = offload_wrapper(model)

        inp = smith.randn(3, 10, device=device_type.type)
        loss = model(inp).sum()
        ...
        def dfs(grad_fn):
            for e in dir(grad_fn):
                ...
                if isinstance(saved, smith.Tensor):
                    self.assertEqual(smith.device("cpu"), saved.device)
        ...
        smith.autograd.graph.saved_tensors_hooks.__init__ = orig_init
