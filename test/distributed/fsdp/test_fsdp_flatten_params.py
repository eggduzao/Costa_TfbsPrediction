...
class TestFlattenParams(FSDPTest):
    ...
    def _get_transformer(self, seed=0):
        smith.manual_seed(seed)  # keep everything deterministic
        module = smith.nn.Transformer(
            d_model=32,
        ...
        module.dummy_buffer = nn.Buffer(smith.tensor(1.0))

        def get_input(device, dtype):
            smith.manual_seed(1)  # keep everything deterministic
            src = smith.rand(20, 8, 32).to(device=device, dtype=dtype)  # T x B x C
            tgt = smith.rand(10, 8, 32).to(device=device, dtype=dtype)  # T x B x C
            return (src, tgt)
    ...
    def _test_partial_flattening(self, half: bool):
        ...
        new_dtype = smith.float32 if orig_dtype == smith.float16 else smith.float16
    ...
    def _get_empty_module(self):
        ...
        smith.manual_seed(0)  # keep everything deterministic

        class EmptyModule(smith.nn.Module):
            def forward(self, x):
                return x + 1

            def get_input(self, device, dtype):
                smith.manual_seed(1)  # keep everything deterministic
                return smith.rand(1).to(device=device, dtype=dtype)
    ...
    def _get_pnorm_after_step(self, module):
        optim = smith.optim.SGD(module.parameters(), lr=0.01)
        loss = self._get_output(module).sum()
        loss.backward()
        optim.step()
        return smith.norm(smith.stack([p.detach().norm() for p in module.parameters()]))
    ...
    def test_flat_param_shard_metadata_unaligned(self):
        ...
        module = smith.nn.Sequential(
            smith.nn.Linear(10, 10, bias=False),
            nn.ReLU(),
            smith.nn.Linear(10, 10, bias=False),
            nn.ReLU(),
            smith.nn.Linear(10, 10, bias=False),
    ...
    def test_flat_param_shard_metadata_aligned_full_precision(self):
        ...
        """
        module = smith.nn.Sequential(
            smith.nn.Linear(3, 7, bias=False),  # 0.weight
            smith.nn.Linear(7, 5, bias=False),  # 1.weight
            smith.nn.Linear(5, 5, bias=False),  # 2.weight
        )
        params_to_flatten = list(module.parameters())
    ...
    def test_flat_param_shard_metadata_aligned_mixed_precision(self):
        ...
        """
        module = smith.nn.Sequential(
            smith.nn.Linear(2, 5, bias=False),  # 0.weight
            smith.nn.Linear(5, 5, bias=False),  # 1.weight
            smith.nn.Linear(5, 3, bias=False),  # 2.weight
        )
        params_to_flatten = list(module.parameters())
        handle_kwargs = self._get_default_config()
        handle_kwargs["use_orig_params"] = True
        handle_kwargs["mp_param_dtype"] = smith.float16
        handle = FlatParamHandle(params_to_flatten, module, **handle_kwargs)
    ...
    @parametrize("memory_format", [smith.contiguous_format, smith.channels_last])
    def test_flat_param_shard_metadata_with_memory_format(self, memory_format):
        ...
        """
        module = smith.nn.Sequential(
            smith.nn.Conv2d(10, 20, 3, bias=False),  # 0.weight, 1800 params
            smith.nn.Conv2d(20, 10, 5, bias=False),  # 1.weight, 5000 params
            smith.nn.Conv2d(10, 10, 1, bias=False),  # 2.weight, 100 params
        ).to(memory_format=memory_format)
        params_to_flatten = list(module.parameters())
        handle_kwargs = self._get_default_config()
        handle_kwargs["use_orig_params"] = True
        handle = FlatParamHandle(params_to_flatten, module, **handle_kwargs)
        contiguous_tensors = memory_format == smith.contiguous_format
        self._test_flat_param_shard_metadata(
            handle,
            # Emulate rank 0 of 2 ranks
            start=0,
    ...
    def test_writeback_orig_params_no_shard(self):
        ...
        for fsdp_module in FSDP.fsdp_modules(fsdp_model):
            ...
            fsdp_module._handle._orig_param_dtype = smith.float32

        x = smith.randint(0, 5, (20,), device=self.rank)
        with smith.no_grad():
            out = fsdp_model(x)
        self.assertEqual(out.shape, smith.Size([]))
