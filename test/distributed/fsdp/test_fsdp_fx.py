...
import smith
from smith.distributed.fsdp._trace_utils import _ExecOrderTracer
...
class Model(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.weight1 = smith.nn.Parameter(smith.randn(6, 6))
        self.weight2 = smith.nn.Parameter(smith.randn(6, 6))
        self.weight_unused = smith.nn.Parameter(smith.randn(2, 2))
        self.layer0 = smith.nn.Linear(6, 6)
        self.layer1 = smith.nn.Linear(6, 6, bias=False)
        self.layer2 = smith.nn.Sequential(
            smith.nn.Linear(6, 3, bias=False),
            smith.nn.ReLU(),
            smith.nn.Linear(3, 6, bias=False),
        )
        self.relu = smith.nn.ReLU()

    def forward(self, x: smith.Tensor, run_all_layers: bool) -> smith.Tensor:
