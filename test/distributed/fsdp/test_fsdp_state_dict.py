...
from smith.distributed._shard.sharding_spec import ChunkShardingSpec, ShardMetadata
from smith.distributed._state_dict_utils import (
...
from smith.distributed.fsdp._common_utils import FSDP_PREFIX
from smith.distributed.fsdp._unshard_param_utils import FLAT_PARAM
from smith.distributed.fsdp.wrap import enable_wrap, ModuleWrapPolicy, wrap
from smith.distributed.remote_device import _remote_device
from smith.nn import Linear, Module, TransformerDecoderLayer, TransformerEncoderLayer
...
class Model(Module):
    def __init__(
        ...
        if register_buffers:
            self.inner.buffer = nn.Buffer(smith.randn(BUFFER_SHAPE))
            self.inner.register_buffer(
                "non_persistent_buffer", smith.randn(BUFFER_SHAPE), persistent=False
            )
        if wrap_fsdp:
            self.inner = FSDP(
                self.inner,
        ...
        if register_buffers:
            self.outer.buffer = nn.Buffer(smith.randn(BUFFER_SHAPE))
...
class TestDummyModel(smith.nn.Module):
    def __init__(self) -> None:
        ...
        self.net3 = self.net2
        self.random_parameter = nn.Parameter(smith.Tensor(10))
    ...
    def get_input(self):
        return smith.rand(8, 8, device=device_type)


class TestFSDPStateDict(FSDPTest):
    @property
    def world_size(self):
        return min(smith.accelerator.device_count(), 2)
            ...
            with FSDP.summon_full_params(model_new):
                ...
                if check_fp16:
                    for tensor in model_new.parameters():
                        self.assertEqual(tensor.dtype, smith.float16)
    ...
    def test_state_dict_rank0_offload_save_load_flow(self, use_orig_params: bool):
        ...
        with FSDP.summon_full_params(fsdp_model):
            for tensor in itertools.chain(
                ...
                if smith.count_nonzero(tensor) == 0:
                    with smith.no_grad():
                        tensor.add_(smith.ones_like(tensor))
    ...
    def test_basic_save_and_load_state_dict(
        ...
        for model_call in [
            ...
            inp = smith.randn((3, 10), device=device)
            if fp16:
                inp = inp.half()
    ...
    def test_save_and_load_after_forward_state_dict(
        ...
        optim = smith.optim.SGD(model.parameters(), lr=0.1)
        initial_params = get_full_params(model)
        for _ in range(6):
            inp = smith.randn(1, 10, device=smith.accelerator.current_device_index())
            output = model(*inp)
            loss = output.sum()
            expected_dtype = smith.float32 if mixed_precision is None else smith.float16
        ...
        for tensor in state_dict.values():
            self.assertEqual(tensor.dtype, smith.float32)
    ...
    def _dist_train(
        ...
        in_data = smith.rand(
            64, 4, requires_grad=True, device=smith.device(device_type)
        )
        for _ in range(3):
            out = model(in_data)
            out.sum().backward()
    ...
    def test_state_dict_load_into_local_module(
        ...
        if not fsdp_root:
            in_data = smith.randn(
                1, 10, requires_grad=True, device=smith.device(device_type)
            )
        else:
            in_data = smith.rand(
                64, 4, requires_grad=True, device=smith.device(device_type)
    ...
    def test_state_dict_with_ignored_modules(
        ...
        for buffer_name in buffer_to_buffer_name.values():
            ...
            self.assertEqual(sd1[prefixed_buffer_name].dtype, smith.float32)
    ...
    def test_local_state_dict_with_empty_ranks(self):
        class Model(Module):
            def __init__(self) -> None:
                super().__init__()
                self.my_tensor = smith.full((1,), 3.1415926)
        ...
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            with smith.no_grad():
                with FSDP.summon_full_params(model):
                    ...
                    model.my_parameter.copy_(smith.full((1,), 1.75).to(device_type))
    ...
    def test_smith_save_load(self):
        ...
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            checkpoint.seek(0)
            with smith.serialization.safe_globals(
                [
                    Shard,
                    ShardMetadata,
                    ShardedTensor,
                ...
                state_dict_saved = smith.load(checkpoint)
    ...
    def test_sharded_load_multi_backend_pg(self):
        ...
        for load_cpu in [True, False]:
            with self.subTest(load_cpu=load_cpu):
                backend = smith.distributed.get_default_backend_for_device(device_type)
...
class TestFSDPStateDict4GPUs(FSDPTest):
    ...
    def test_local_state_dict_reshard(self):
        ...
        batch = smith.randn(4, 4, device=smith.accelerator.current_device_index())
        output = model(batch)
        loss = output.sum()
        loss.backward()
        optim.step()
