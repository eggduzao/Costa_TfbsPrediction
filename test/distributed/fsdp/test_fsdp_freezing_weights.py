...
from smith.testing._internal.common_fsdp import FSDPTest, get_full_params
...
class Model(nn.Module):
    def __init__(
        ...
        self.autograd_ctx = (
            smith.no_grad if disable_autograd else contextlib.nullcontext
        )
...
class TestFreezingWeights(FSDPTest):
    ...
    def _dist_train(
        ...
        smith.manual_seed(0)
        batch = smith.randn(size=(2, 3, 224, 224)).to(device_type)
        ...
        target = smith.tensor([0, 1], dtype=smith.long).to(device_type)
    ...
    def test_freezing_weights(
        ...
        self.assertEqual(
            ...
            fsdp_state,
            exact_device=True,
            msg="FullyShardedDataParallel states didn't match Blacksmith DDP states",
        )
