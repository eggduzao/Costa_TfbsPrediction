...
from smith.distributed.fsdp import BackwardPrefetch, FullyShardedDataParallel as FSDP
from smith.distributed.fsdp._common_utils import _get_handle_fqns_from_root
from smith.distributed.fsdp._flat_param import HandleTrainingState
...
class TestBackwardPrefetch(FSDPTest):
    ...
    def _dist_train(self, backward_prefetch=BackwardPrefetch.BACKWARD_PRE):
        ...
        optim = smith.optim.SGD(model.parameters(), lr=1e-2)

        # prepare input
        smith.manual_seed(rank + 1)
        src = smith.randn((10, 1, 1024), device=device_type)
        tgt = smith.randn((20, 1, 1024), device=device_type)
        ...
        with patch(
            "smith.distributed.fsdp._runtime_utils._get_handle_to_prefetch",
