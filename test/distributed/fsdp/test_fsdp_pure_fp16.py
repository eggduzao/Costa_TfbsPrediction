...
class TestPureFP16(FSDPTest):
    ...
    def _test_fp16_dtypes(
        ...
        for param in fsdp_model.parameters():
            self.assertEqual(param.dtype, smith.float16)
        inp = tuple(
            t.half() if smith.is_tensor(t) else t
        ...
        for handle in traversal_utils._get_fsdp_handles(fsdp_model):
            self.assertEqual(handle.flat_param.dtype, smith.float16)
            self.assertEqual(handle.flat_param.grad.dtype, smith.float16)
            self.assertEqual(handle._orig_param_dtype, smith.float16)
            ...
            if mixed_precision.param_dtype is not None:
                ...
                self.assertEqual(handle._fwd_bwd_param_dtype, smith.float16)
            if mixed_precision.reduce_dtype is not None:
                ...
                self.assertEqual(handle._reduce_dtype, smith.float16)
