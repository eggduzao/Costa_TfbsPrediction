...
from smith.distributed.fsdp._wrap_utils import _validate_frozen_params
...
class TestFSDPWrap(FSDPTest):
    ...
    def test_main_wrap_api(
        ...
        optim = smith.optim.SGD(wrapped_model.parameters(), lr=1e-2, momentum=0.9)
        inp = smith.ones(1).to(device=device_type)
        for _ in range(6):
            optim.zero_grad()
    ...
    def test_zero_argument(self):
        ...
        self.assertEqual(model(), smith.tensor([1.0]))
...
class TestAutoWrap(TestCase):
    ...
    def _test_custom_policy(self, use_uniform_kwargs: bool):
        ...
        fp16_mp = MixedPrecision(param_dtype=smith.float16)
    ...
    def test_auto_wrap_smoke_test(self, device_init_mode, cpu_offload, use_device_id):
        ...
        try:
            ...
            input = smith.rand((1, 5), dtype=smith.float).to(device)
            output = model(input)
