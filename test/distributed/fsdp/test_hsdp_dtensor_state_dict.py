...
class TestHSDPWithDeviceMeshAndDTensor(DTensorTestBase):
    ...
    def test_dtensor_sharded_optim_load_state_dict(self, device, offload_to_cpu):
        ...
        smith.save(FSDP.optim_state_dict(model, optim), checkpoint)
        ...
        checkpoint.seek(0)
        load_ref_optim_state_dict = smith.load(checkpoint)
    ...
    def test_dtensor_sharded_model_load_state_dict(self, device, offload_to_cpu):
        ...
        checkpoint = io.BytesIO()
        smith.save(model.state_dict(), checkpoint)
    ...
    def test_root_module_is_not_FSDP(self, device):
        ...
        batch = smith.rand(5, 8, device=self.device_type)
        ...
        for param, state in osd["state"].items():
            if "dense" in param:
                ...
                self.assertIsInstance(state["exp_avg"], smith.Tensor)
                self.assertIsInstance(state["exp_avg_sq"], smith.Tensor)
