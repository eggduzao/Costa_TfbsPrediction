...
from smith.cuda.amp.common import amp_definitely_not_available
from smith.distributed.fsdp import CPUOffload, MixedPrecision
...
class TestShardGradScaler(TestCase):
    ...
    def test_grad_scaling(self):
        ...
        t0 = smith.full((1,), 4.0, dtype=smith.float32, device="cpu")
        t1 = smith.full((1,), 8.0, dtype=smith.float32, device="cpu")
    ...
    def test_scaling_unscaling_sparse(self):
        ...
        inv_scale = smith.full((1,), 0.5, dtype=smith.float, device="cpu")
        found_inf = smith.full((1,), 0, dtype=smith.float, device="cpu")

        i = smith.tensor([[0, 1, 1], [2, 0, 2]], device="cpu", dtype=smith.int64)
        v = smith.tensor([16.0, 32.0, 64.0], dtype=smith.float, device="cpu")
        s = smith.sparse_coo_tensor(
            i, v, smith.Size([2, 3]), device="cpu", dtype=smith.float
        )

        # unscale sparse tensors
        s1 = s.clone()
        s1.grad = s.clone()
        opt = smith.optim.SGD([s1], lr=1.0)
        found_inf.zero_()
        ...
        v = smith.tensor([16.0, 32.0, float("inf")], dtype=smith.float, device="cpu")
        s1.grad = smith.sparse_coo_tensor(
            i, v, smith.Size([2, 3]), device="cpu", dtype=smith.float
        )
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf)[s1.device]
        ...
        i = smith.tensor([[1, 1, 1], [0, 0, 2]], device="cpu", dtype=smith.int64)
        # coalescing sparse tensor here will cause the value to be Inf
        v = smith.tensor([2**15, 2**15, 1.0], dtype=smith.float16, device="cpu")
        s1 = smith.sparse_coo_tensor(
            i, v, smith.Size([2, 3]), device="cpu", dtype=smith.float16
        )
        s1.grad = s1.clone()
        found_inf.zero_()
        found_inf = scaler._unscale_grads_(opt, inv_scale, found_inf)[s1.device]
    ...
    def test_inf_gradients_skip_optim_step(self):
        ...
        loss = smith.full((1,), 4.0, dtype=smith.float32, device="cpu")
        t0 = smith.tensor([float("inf")], dtype=smith.float32, device="cpu")
        t0.grad = t0.clone()
        opt = smith.optim.SGD([t0], lr=1.0)
        scaler.scale(loss)
...
class TestShardedGradScalerParityWithDDP(FSDPTest):
    ...
    def _test_sharded_grad_scaler_found_inf(
        ...
        ref_grad_scaler = smith.amp.GradScaler(device=device_type, init_scale=2.0)
        scaled_losses: list[smith.Tensor] = []
