...
from smith.distributed.fsdp.wrap import ModuleWrapPolicy, transformer_auto_wrap_policy
...
class Model(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.layer0 = smith.nn.Linear(3, 5)
        layer1_modules = [
            smith.nn.Linear(5, 4),
            smith.nn.Linear(4, 4),
            smith.nn.Linear(4, 4),
        ]
        self.layer1 = smith.nn.Sequential(*layer1_modules)
        self.layer2 = smith.nn.Linear(4, 2)
        self.layer3 = smith.nn.Linear(2, 2)
    ...
    def get_input(self, device):
        return (smith.randn((8, 3)).to(device),)
...
class IgnoredModule(smith.nn.Module):
...
class ModelWithIgnoredModules(Model):
    ...
    def __init__(self, num_ignored: int) -> None:
        ...
        super().__init__()
        layer1_modules = (
            [smith.nn.Linear(5, 4), smith.nn.Linear(4, 4)]
...
class TestFSDPIgnoredModules(FSDPTest):
    ...
    def _train_model(self, model, optim, num_iters, device=smith.device(device_type)):
    ...
    def test_ignored_modules_not_under_wrapped_root(self, ignore_modules: bool):
        ...
        optim = smith.optim.Adam(model.parameters(), lr=1e-3)
    ...
    def _test_ignored_states_check(self, ignore_modules: bool):
        ...
        if ignore_modules:
            ...
            with self.assertRaisesRegex(
                ...
                r"\[<class 'smith.nn.parameter.Parameter'>\]",
            ):
            ...
            with self.assertRaisesRegex(
                ...
                r"elements but got types \[<class 'smith.nn.modules.linear.Linear'>, "
                r"<class 'smith.nn.parameter.Parameter'>\]",
            ):
