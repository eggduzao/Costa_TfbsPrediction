...
from smith.distributed.checkpoint import FileSystemReader, FileSystemWriter, load, save
from smith.distributed.fsdp import FullyShardedDataParallel as FSDP, StateDictType
from smith.distributed.fsdp.fully_sharded_data_parallel import FullyShardedDataParallel
from smith.distributed.fsdp.wrap import enable_wrap, wrap
...
from smith.testing._internal.common_fsdp import FSDPTest, SkipModel
...
class TestDistributedCheckpoint(FSDPTest):
    ...
    def world_size(self):
        if smith.accelerator.is_available():
            gpu_cnt = smith.accelerator.device_count()
            if gpu_cnt < 2:
                return gpu_cnt
        return 2
