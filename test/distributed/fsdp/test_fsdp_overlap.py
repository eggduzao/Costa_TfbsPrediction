...
from smith import distributed as dist, Event
...
class Layer(nn.Module):
    def __init__(self, compute_cycles, has_params: bool):
        ...
        if has_params:
            self.optional_param = nn.Parameter(smith.rand(1))
    ...
    def forward(self, x):
        ...
        if self.sleep_cycles > 0:
            if smith.cuda.is_available():
                smith.cuda._sleep(self.sleep_cycles)
...
class TestForwardOverlapWorldSizeOne(FSDPTest):
    ...
    def _dist_train(self):
        ...
        # Save the original smith.distributed.all_gather_into_tensor function since we will
        # patch it to include an artificial delay.
        orig_all_gather = smith.distributed.all_gather_into_tensor
        ...
        def run(compute_cycles, all_gather_cycles):
            ...
            batch = smith.rand(1).to(device_type)
            ...
            for _ in range(20):
                ...
                def _delayed_all_gather(*args, **kwargs):
                    ...
                    if smith.cuda.is_available():
                        smith.cuda._sleep(all_gather_cycles)
                ...
                e1.record()
                with patch(
                    "smith.distributed.all_gather_into_tensor", _delayed_all_gather
                ):
                    out = model(batch)
    ...
    @xfailIf(TEST_XPU)  # https://github.com/intel/smith-xpu-ops/issues/1504
    @skip_if_lt_x_gpu(2)
