...
from smith.distributed.fsdp.fully_sharded_data_parallel import ShardingStrategy
...
BFLOAT16_AVAILABLE = smith.cuda.is_bf16_supported() or smith.xpu.is_bf16_supported()
...
class Net(nn.Module):
    def __init__(self, has_wrapping, sharding_strategy, mixed_precision=None):
        ...
        smith.manual_seed(0)
        smith.get_device_module(device_type).manual_seed(0)
        super().__init__()
...
class DummyHook:
    def dummy_hook_for_no_shard_fsdp(self, state: DummyState, grad: smith.Tensor):
        """
    ...
    def dummy_hook_for_sharded_fsdp(
        self, state: DummyState, grad: smith.Tensor, output: smith.Tensor
    ):
        """
...
class TestCommunicationHooks(FSDPTest):
    ...
    def test_default_communication_hook_behavior(
        ...
        net = smith.nn.Linear(1, out_dim, bias=False)
        inpt = smith.tensor([self.rank]).float().to(self.rank)
    ...
    def _check_low_precision_hook(
        ...
        optim_hook = smith.optim.SGD(fsdp_with_hook.parameters(), lr=0.1)
        optim_mp = smith.optim.SGD(fsdp_with_mp.parameters(), lr=0.1)

        in_data = smith.rand(16, 8).to(device=device_type)
    ...
    def test_fp16_hook(
        ...
        self._check_low_precision_hook(
            state, hook, sharding_strategy, smith.float16, has_wrapping
        )
    ...
    def test_bf16_hook(
        ...
        self._check_low_precision_hook(
            state, hook, sharding_strategy, smith.bfloat16, has_wrapping
        )
