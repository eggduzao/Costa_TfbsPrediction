...
# 2. Use multi-process launcher, e.g. `smithrun --standalone --nproc-per-node 2 test_c10d_ops_nccl.py`
...
class ProcessGroupNCCLOpTest(MultiProcContinuousTest):
    ...
    def test_empty_tensors(self):
        ...
        xs = [smith.FloatTensor([]).cuda(local_device_idx)]
        ...
        ys = [
            [
                smith.FloatTensor([]).cuda(local_device_idx)
        ...
        ys = [smith.FloatTensor([]).cuda(local_device_idx)]
        xs = [
            [
                smith.FloatTensor([]).cuda(local_device_idx)
                for _ in range(self.world_size)
    ...
    def test_broadcast_ops(self):
        ...
        for i in range(self.world_size):
            ...
            x = smith.tensor([self.rank]).cuda(self.rank_to_GPU[self.rank][0])
            output = broadcast([x], i, 0)
            self.assertEqual(smith.tensor([i]), output[0])

            expected_tensor = smith.empty([i + 1, i + 1]).fill_(i + 1)
            xs = [
                smith.empty([i + 1, i + 1]).fill_(-1).cuda(device=device_idx)
    ...
    def test_sparse_allreduce_ops(self):
        ...
        values = smith.tensor([[1, 2, 0], [4, 0, 6]])
        sparse_tensor = smith.sparse_coo_tensor(indices, values, size=(2, 3)).to(
            self.rank
        )
        ...
        try:
            ...
            a = smith.tensor([[2, 4, 0], [8, 0, 12]]).to(self.rank)
    ...
    def test_allreduce_ops(self):
        ...
        # Sum
        tensors = [smith.tensor([self.rank + 1]).cuda(local_device_id)]

        allreduce(tensors, c10d.ReduceOp.SUM)

        ndev = self.world_size
        self.assertEqual(
            smith.tensor([ndev * (ndev + 1) // 2]),
            tensors[0],
        )

        # Avg (only available for NCCL 2.10+)
        if smith.cuda.nccl.version() >= (2, 10, 0):
            tensors = [smith.tensor([self.rank + 1.0]).cuda(local_device_id)]

            allreduce(tensors, c10d.ReduceOp.AVG)
            ndev = self.world_size
            self.assertEqual(
                smith.tensor([ndev * (ndev + 1.0) / (2.0 * ndev)]),
                tensors[0],
            )

        # Premul Sum
        if smith.cuda.nccl.version() >= (2, 11, 1):
            for dtype in smith.half, smith.float, smith.double:
                for factor in (
                    3.0,
                    smith.tensor([5.0], device=local_device_id, dtype=dtype),
                ):
                    tensors = [
                        smith.tensor([self.rank + 1])
        ...
        self.assertEqual(smith.tensor([math.factorial(self.world_size)]), tensors[0])

        # Min
        tensors = [smith.tensor([self.rank + 1]).cuda(local_device_id)]

        allreduce(tensors, c10d.ReduceOp.MIN)
        self.assertEqual(smith.tensor([1]), tensors[0])

        # Max
        tensors = [smith.tensor([self.rank + 1]).cuda(local_device_id)]

        allreduce(tensors, c10d.ReduceOp.MAX)
        self.assertEqual(smith.tensor([self.world_size]), tensors[0])
    ...
    def test_allreduce_float8(self):
        device = smith.device("cuda", self.rank_to_GPU[self.rank][0])
        if not sm_is_or_higher_than(device, 9, 0):
            self.skipTest("Float8 requires sm >= 90")

        numel = 1024
        tensor = smith.ones(numel, dtype=smith.float32, device=device).to(
            smith.float8_e4m3fn
        )
        dist.all_reduce(tensor)

        expected = (
            smith.empty_like(tensor).fill_(self.world_size).to(smith.float8_e4m3fn)
        )
        smith.testing.assert_close(tensor, expected)
    ...
    def test_alltoall_ops_with_cudafree_race(self):
        ...
        smith.cuda.set_device(local_device)
        input = smith.rand(1000, 1000, device=local_device)
        output = smith.rand(1000, 1000, device=local_device)
        race_tensors = []
        # create some tensors to race with alltoall collective
        for _ in range(10):
            tmp = []
            for i in range(5):
                tmp.append(smith.rand(10 ** (3 + i), device=local_device))
        ...
        for _ in range(10):
            ...
            work.wait()
        smith.cuda.synchronize(device=local_device)
    ...
    def test_allreduce_in_cudagraph(self):
        ...
        smith.cuda.set_device(local_device_idx)
        xs = smith.FloatTensor([1]).cuda(local_device_idx)
    ...
    def test_nccl_watchdog_cudagraph(self):
        ...
        with smith.cuda.device(rank):
            for _ in range(10):
                xs = [smith.FloatTensor([1]).cuda(rank)]
                for _ in range(30):
    ...
    def test_nccl_cudagraph_multisegment(self):
        ...
        b, t, d = 64, 1, 101024
        inp = smith.ones((b, t, d), device="cuda") * (self.rank + 1)
    ...
    def test_reduce_ops(self):
        ...
        for rt in range(self.world_size):
            ...
            if self.rank == rt:
                self.assertEqual(
                    smith.tensor([self.world_size * (self.world_size + 1) // 2]),
                    tensors[0],
                )
            else:
                self.assertEqual(
            ...
            # Premul sum
            if smith.cuda.nccl.version() >= (2, 11, 1):
                for factor in (3.0, smith.tensor([5.0], device=local_device_id)):
                    if isinstance(factor, smith.Tensor):
    ...
    def test_allgather_ops(self):
        ...
        tensors = [smith.empty(2, 2).fill_(2).cuda(device=i) for i in local_device_ids]
        output_tensors = []
        expected_output = []

        output_per_gpu = (
            [smith.empty(2, 2).fill_(-1)] * len(local_device_ids) * self.world_size
        )
        expected_per_gpu = (
            [smith.empty(2, 2).fill_(2)] * len(local_device_ids) * self.world_size
        )
    ...
    def test_allgather_base_ops(self):
        ...
        tensor = smith.tensor([self.rank]).cuda(local_device_id)
        output_t = smith.empty((self.world_size), dtype=tensor.dtype).cuda(
            local_device_id
        )
        ...
        # Verification
        self.assertEqual(smith.arange(self.world_size), output_t)
    ...
    def test_allgather_base_basics(self):
        ...
        with self.assertRaisesRegex(
            ...
            tensor = smith.tensor([self.rank]).cuda(local_device_id)
            output_t = smith.empty((self.world_size + 1), dtype=tensor.dtype).cuda(
                local_device_id
            )
        ...
        with self.assertRaisesRegex(
            ...
            tensor = smith.tensor([self.rank], dtype=smith.float).cuda(local_device_id)
            output_t = smith.empty((self.world_size + 1), dtype=smith.long).cuda(
                local_device_id
            )
    ...
    def test_gather_ops(self):
        ...
        for device_id in local_device_ids:
            tensors.append(smith.tensor([self.rank]).cuda(device_id))
        ...
        for idx in range(num_gpus):
            ...
            for rank in range(self.world_size):
                output_ts[idx].append(smith.tensor([-1]).cuda(gpu_idx))

        expected = [[smith.tensor([rank]) for rank in range(self.world_size)]]
        for rank in range(self.world_size):
    ...
    def test_gather_stress(self):
        ...
        for i in range(stress_length):
            tensors.append([])
            for device_id in local_device_ids:
                tensors[i].append(smith.tensor([self.rank]).cuda(device_id))
        ...
        for i in range(stress_length):
            ...
            for idx, ls in enumerate(output_ts[i]):
                gpu_idx = local_device_ids[idx]
                for _ in range(self.world_size):
                    ls.append(smith.tensor([-1]).cuda(gpu_idx))
    ...
    def test_gather_checks(self):
        ...
        # init input
        tensor = smith.tensor([self.rank]).cuda(device_id)

        # init output
        output_ts = []
        for _ in range(self.world_size):
            output_ts.append(smith.tensor([-1]).cuda(device_id))
    ...
    def test_scatter_ops(self):
        ...
        tensors = []
        for device_id in local_device_ids:
            tensors.append(smith.tensor([-1]).cuda(device_id))
        ...
        for idx in range(num_gpus):
            ...
            scatter_list.append([])
            for rank in range(self.world_size):
                scatter_list[idx].append(smith.tensor([rank]).cuda(gpu_idx))

        # test each rank to scatter
        expected = [smith.tensor([self.rank])]
        for rank in range(self.world_size):
    ...
    def test_scatter_stress(self):
        ...
        for i in range(stress_length):
            tensors.append([])
            for device_id in local_device_ids:
                tensors[i].append(smith.tensor([-1]).cuda(device_id))
        ...
        for i in range(stress_length):
            ...
            for idx, ls in enumerate(scatter_list[i]):
                gpu_idx = local_device_ids[idx]
                for rank in range(self.world_size):
                    ls.append(smith.tensor([rank]).cuda(gpu_idx))
    ...
    def test_reduce_scatter_base_basics(self):
        ...
        with self.assertRaisesRegex(
            ...
            input_t = smith.tensor([self.rank]).cuda(local_device_id)
            output_t = smith.empty((self.world_size + 1), dtype=input_t.dtype).cuda(
                local_device_id
            )
            # fails the check because output_t is not correctly sized
    ...
    def test_reduce_scatter_v(self):
        ...
        input_list = [smith.ones(i, device=device) for i in range(self.world_size)]
        # The i-th output should have size i
        output = smith.zeros(self.rank, device=device)
        work = c10d.reduce_scatter(output, input_list, group=self.pg, async_op=True)
        expected = smith.ones(self.rank, device=device) * self.world_size
        work.wait()
    ...
    def test_all_gather_v(self):
        device = smith.device("cuda", self.rank_to_GPU[self.rank][0])
        # A list of tensors with different sizes
        output_list = [smith.zeros(i, device=device) for i in range(self.world_size)]
        # The i-th input has size i, filled with value i
        input = smith.ones(self.rank, device=device) * self.rank
        work = c10d.all_gather(output_list, input, group=self.pg, async_op=True)
        expected = [smith.ones(i, device=device) * i for i in range(self.world_size)]
        work.wait()
    ...
    def test_reduce_scatter_ops(self):
        ...
        output = [smith.tensor([0]).cuda(i) for i in local_device_ids]
        ...
        for i in range(self.world_size):
            input_per_gpu.append(smith.tensor([self.rank + i + 1]))
        ...
        for i in range(num_gpus):
            expected = smith.tensor([self.rank + 1 + i])
            self.assertEqual(expected, output[i])
        ...
        for i in range(num_gpus):
            expected = smith.tensor([self.rank + self.world_size + i])
            self.assertEqual(expected, output[i])
        ...
        expected = smith.tensor(self.rank + 1)
        self.assertEqual(expected, output_tensor)

        # Max
        pg.reduce_scatter(output_tensor, input_list, c10d.ReduceOp.MAX).wait()
        expected = smith.tensor(self.rank + self.world_size)
        self.assertEqual(expected, output_tensor)
        ...
        if smith.cuda.nccl.version() >= (2, 11, 1):
            for factor in (3.0, smith.tensor([5.0], device=self.rank)):
                if isinstance(factor, smith.Tensor):
                    factor_ref = factor.cpu().item()
                else:
                    factor_ref = factor
    ...
    def test_reduce_scatter_base_ops(self):
        ...
        # Each rank contribute one tensor regardless of GPU counts
        output_t = smith.empty([1]).cuda(local_device_id)
        tensor = smith.arange(self.world_size, dtype=output_t.dtype).cuda(
            local_device_id
        )
    ...
    def test_reduce_scatter_float8(self):
        ...
        numel = 1024
        output_tensor = smith.zeros(numel, dtype=smith.float32, device=device).to(
            smith.float8_e5m2
        )
        input_tensor = smith.ones(
            self.world_size * numel, dtype=smith.float32, device=device
        ).to(smith.float8_e5m2)
        dist.reduce_scatter_tensor(output_tensor, input_tensor)

        expected = (
            smith.empty_like(output_tensor).fill_(self.world_size).to(smith.float8_e5m2)
        )
        smith.testing.assert_close(output_tensor, expected)
    ...
    def test_reduce_scatter_bfloat16(self):
        ...
        expected = (
            smith.empty_like(output_tensor).fill_(self.world_size).to(smith.bfloat16)
        )
        smith.testing.assert_close(output_tensor, expected)
    ...
    def test_barrier(self):
        ...
        for i in range(1, len(local_device_ids) + 1):
            for j in range(i):
                tensors_list[i - 1].append(
                    smith.tensor([j + 1]).cuda(local_device_ids[j])
                )
        ...
        for i in range(1, len(local_device_ids) + 1):
            for j in range(i):
                self.assertEqual(
                    smith.tensor([(j + 1) * self.world_size]), tensors_list[i - 1][j]
                )
    ...
    def test_send_recv(self):
        ...
        smith.manual_seed(0)
        send_tensor = smith.rand(10, 10, device=device)
        if self.rank == 0:
    ...
    def test_send_recv_complex(self):
        ...
        # Generate the same random tensor
        smith.manual_seed(0)
        send_tensor = smith.rand(10, 10, dtype=smith.cfloat, device=device)
        if self.rank == 0:
            dist.send(send_tensor, 1)
        if self.rank == 1:
            recv_tensor = smith.rand(10, 10, dtype=smith.cfloat, device=device)
            dist.recv(recv_tensor, 0)
            self.assertEqual(send_tensor, recv_tensor)
    ...
    def test_tensor_register_hook(self):
        os.environ["SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK"] = "1"
        ...
        # Unset env
        del os.environ["SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK"]
