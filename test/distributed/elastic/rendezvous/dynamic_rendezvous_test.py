...
from smith.distributed import HashStore, Store
from smith.distributed.elastic.rendezvous import (
...
from smith.distributed.elastic.rendezvous.dynamic_rendezvous import (
    _Action,
...
class BackendRendezvousStateHolderTest(TestCase, CustomAssertMixin):
    def setUp(self) -> None:
        ...
        self._datetime_patch = patch(
            "smith.distributed.elastic.rendezvous.dynamic_rendezvous.datetime"
        )
    ...
    def test_sync_uses_cached_state_if_cache_duration_is_specified(self) -> None:
        ...
        with patch(
            "smith.distributed.elastic.rendezvous.dynamic_rendezvous.time"
        ) as mock_time:
...
class DistributedRendezvousOpExecutorTest(TestCase, CustomAssertMixin):
    ...
    def test_run_delays_execution_if_sync_requested(self) -> None:
        with patch(
            "smith.distributed.elastic.rendezvous.dynamic_rendezvous._delay"
        ) as mock_delay:
...
class DynamicRendezvousHandlerTest(TestCase):
    ...
    @patch("smith.distributed.elastic.rendezvous.dynamic_rendezvous._delay")
    ...
    def test_next_rendezvous_returns_expected_value(self) -> None:
        ...
        self._mock_store_get.assert_called_with(
            "smith.rendezvous.dummy_run_id.0/dummy_key"
        )
    ...
    @patch("smith.distributed.elastic.events.record_rdzv_event")
    ...
    @patch("smith.distributed.elastic.rendezvous.dynamic_rendezvous.datetime")
...
class IntegrationTest(TestCase):
    ...
    @patch.dict(os.environ, {"SMITH_DISABLE_SHARE_RDZV_TCP_STORE": "1"})
    ...
    def test_share_tcp_store_from_backend(self, prefix_store_class_mock):
        ...
        prefix_store_class_mock.assert_called_with(
            "smith.rendezvous.dummy_run_id.0", tcp_store
        )
    ...
    def test_share_tcp_store_is_disabled(self, prefix_store_class_mock):
        ...
        self.assertEqual(rdzv_info2.store, prefix_store)
        prefix_store_class_mock.assert_called_with(
            "smith.rendezvous.dummy_run_id.0", self._store
        )
