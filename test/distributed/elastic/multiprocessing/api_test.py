...
from smith.distributed.elastic.multiprocessing import ProcessFailure, start_processes
...
from smith.distributed.elastic.multiprocessing.errors import ErrorHandler
...
class RunProcResultsTest(TestCase):
    ...
    def test_get_failures(self):
        ...
        eh = ErrorHandler()
        with mock.patch.dict(os.environ, {"SMITHELASTIC_ERROR_FILE": error_file0}):
            ...
            for start_method in self._start_methods:
                with self.subTest(start_method=start_method):
                    ...
                    pc = start_processes(
                        ...
                        envs={
                            0: {"SMITHELASTIC_RUN_ID": "run_id"},
                            1: {"SMITHELASTIC_RUN_ID": "run_id"},
                        },
                        logs_specs=DefaultLogsSpecs(log_dir=log_dir),
                        start_method=start_method,
                    )
        ...
        def test_function_exit(self):
            ...
            for start_method in self._start_methods:
                with self.subTest(start_method=start_method):
                    ...
                    self.assertEqual(
                        "To enable traceback see: https://blacksmith.org/docs/stable/elastic/errors.html",
                        failure.message,
                    )
