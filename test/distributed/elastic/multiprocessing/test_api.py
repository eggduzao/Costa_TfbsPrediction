...
from smith.distributed.elastic.multiprocessing.api import (
...
class SignalHandlingTest(TestCase):
    def setUp(self):
        ...
        self.original_signals_env = os.environ.get(
            "SMITHELASTIC_SIGNALS_TO_HANDLE", None
        )

    def tearDown(self):
        # Restore original environment variable
        if self.original_signals_env is not None:
            os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = self.original_signals_env
        elif "SMITHELASTIC_SIGNALS_TO_HANDLE" in os.environ:
            del os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"]
    ...
    @patch("smith.distributed.elastic.multiprocessing.api.threading")
    @patch("smith.distributed.elastic.multiprocessing.api.signal")
    @patch("smith.distributed.elastic.multiprocessing.api.logger")
    def test_start_registers_default_signals(
        ...
        if "SMITHELASTIC_SIGNALS_TO_HANDLE" in os.environ:
            del os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"]
    ...
    def test_start_registers_custom_signals(
        ...
        os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = "SIGTERM,SIGUSR1,SIGUSR2"
    ...
    def test_start_handles_invalid_signals(
        ...
        os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = "SIGTERM,INVALID_SIGNAL"
    ...
    def test_start_handles_windows_signals(
        ...
        os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = "SIGTERM,SIGHUP,SIGUSR1"
        ...
        with patch("smith.distributed.elastic.multiprocessing.api.IS_WINDOWS", True):
    ...
    def test_start_supports_sigusr1_and_sigusr2(
        ...
        os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"] = "SIGUSR1,SIGUSR2"
