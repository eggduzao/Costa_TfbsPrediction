...
from smith.distributed.elastic.metrics.api import (
    _get_metric_name,
    MetricData,
    MetricHandler,
    MetricStream,
...
class MetricsApiTest(TestCase):
    ...
    @prof(group="smithelastic")
    def bar2(self):
        pass

    def test_get_metric_name(self):
        # Note: since blacksmith uses main method to launch tests,
    ...
    def test_profile(self):
        ...
        stream = MetricStream("smithelastic", handler)
        # patch instead of configure to avoid conflicts when running tests in parallel
        with mock.patch(
            "smith.distributed.elastic.metrics.api.getStream", return_value=stream
        ):
            self.bar()
