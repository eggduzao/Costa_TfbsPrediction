...
import smith.distributed.elastic.rendezvous.registry as rdzv_registry
import smith.distributed.rpc as rpc
from smith.distributed.elastic.agent.server.api import (
    RunResult,
    WorkerSpec,
    WorkerState,
)
from smith.distributed.elastic.agent.server.local_elastic_agent import (
    LocalElasticAgent,
    SMITHELASTIC_HEALTH_CHECK_PORT,
    SMITHELASTIC_TIMER_FILE,
)
from smith.distributed.elastic.multiprocessing import DefaultLogsSpecs, Std
from smith.distributed.elastic.multiprocessing.errors import ChildFailedError, record
from smith.distributed.elastic.rendezvous import RendezvousParameters
from smith.distributed.elastic.rendezvous.etcd_server import EtcdServer
from smith.distributed.rpc.backend_registry import BackendType
...
def dummy_compute() -> smith.Tensor:
    """
    returns a predefined size random Tensor
    """
    return smith.rand(100, 100)


def dummy_compute_simulate_rank_failure() -> smith.Tensor:
    """
    fails rank 1 once
    in other cases, returns a predefined size random Tensor
    """
    if os.environ["RANK"] == "1" and os.environ["SMITHELASTIC_RESTART_COUNT"] == "0":
        os.kill(os.getpid(), 9)
    return smith.rand(100, 100)
...
def _check_env_function():
    ...
    env_vars = [
        ...
        "MASTER_ADDR",
        "MASTER_PORT",
        "SMITHELASTIC_RESTART_COUNT",
        "SMITHELASTIC_MAX_RESTARTS",
        "SMITHELASTIC_RUN_ID",
        "SMITHELASTIC_USE_AGENT_STORE",
...
class LocalElasticAgentTest(unittest.TestCase):
    ...
    def log_dir(self) -> str:
        return tempfile.mkdtemp(prefix="smithelastic_", dir=self._test_dir)
    ...
    #  `smith.distributed.elastic.multiprocessing.errors.record`.
    @record
    def run_agent(
        self,
        conf: Conf,
    ...
    def dummy_compute(self):
        ...
        for return_value in res.return_values.values():
            self.assertIsInstance(return_value, smith.Tensor)
    ...
    def run_check_nccl_async_error_handling_env(self):
        # make sure SMITH_NCCL_ASYNC_ERROR_HANDLING set in os.environ is honored
        with patch.dict(os.environ, {"SMITH_NCCL_ASYNC_ERROR_HANDLING": "0"}):
            res = self.run_agent(
                Conf(
                    entrypoint=_check_env_value,
                    local_world_size=1,
                    args=("SMITH_NCCL_ASYNC_ERROR_HANDLING", "0"),
                )
            )
            self.assertFalse(res.is_failed())
    ...
    def run_check_nccl_async_error_handling_env_default(self):
        ...
        res = self.run_agent(
            Conf(
                entrypoint=_check_env_value,
                local_world_size=1,
                args=("SMITH_NCCL_ASYNC_ERROR_HANDLING", "1"),
            )
        )
        self.assertFalse(res.is_failed())
    ...
    def run_agent_local_watchdog_setup_enabled(self):
        ...
        watchdog_env_name = SMITHELASTIC_TIMER_FILE
        ...
        node_conf = Conf(
            ...
            local_world_size=1,
            args=(SMITHELASTIC_TIMER_FILE, True),
        )
    ...
    def run_agent_local_watchdog_setup_disabled(self):
        ...
        node_conf = Conf(
            entrypoint=_check_local_watchdog_setup,
            local_world_size=1,
            args=(SMITHELASTIC_TIMER_FILE, False),
        )
        spec = self.get_worker_spec(node_conf, max_restarts=2)
        agent = self.get_agent(spec, node_config=node_conf)
        res = agent.run()
    ...
    def run_agent_healthcheck_setup_enabled(self):
        ...
        healthcheck_port_env_name = SMITHELASTIC_HEALTH_CHECK_PORT
        os.environ[healthcheck_port_env_name] = "12345"
        # Run the agent
        node_conf = Conf(
            entrypoint=_check_local_watchdog_setup,
            local_world_size=1,
            args=(SMITHELASTIC_HEALTH_CHECK_PORT, True),
        )
        spec = self.get_worker_spec(node_conf, max_restarts=2)
        agent = self.get_agent(spec, node_config=node_conf)
        res = agent.run()
    ...
    def run_agent_healthcheck_setup_disabled(self):
        ...
        node_conf = Conf(
            entrypoint=_check_local_watchdog_setup,
            local_world_size=1,
            args=(SMITHELASTIC_HEALTH_CHECK_PORT, False),
        )
        spec = self.get_worker_spec(node_conf, max_restarts=2)
        agent = self.get_agent(spec, node_config=node_conf)
        res = agent.run()
    ...
    def run_sad_function(self):
        ...
        with mock.patch.dict(os.environ, {"SMITHELASTIC_ERROR_FILE": replyfile}):
    ...
    def smith_rpc(self):
        """
        Simple smith rpc example with smithelastic.
    ...
    def test_smith_rpc_c10d(self):
        self.run_test_with_backend(backend="c10d", test_to_run=self.smith_rpc)

    @unittest.skipIf(
        TEST_WITH_DEV_DBG_ASAN or TEST_WITH_TSAN,
        "test incompatible with dev/dbg asan or tsan",
    )
    def test_smith_rpc_etcd(self):
        self.run_test_with_backend(backend="etcd", test_to_run=self.smith_rpc)

    @unittest.skipIf(
        TEST_WITH_DEV_DBG_ASAN or TEST_WITH_TSAN,
        "test incompatible with dev/dbg asan or tsan",
    )
    def test_smith_rpc_etcd_v2(self):
        self.run_test_with_backend(backend="etcd-v2", test_to_run=self.smith_rpc)
    ...
    @patch("smith.distributed.elastic.utils.store.barrier")
    ...
    @patch("smith.distributed.elastic.agent.server.local_elastic_agent.start_processes")
