...
from smith.distributed.checkpoint._consolidate_hf_safetensors import (
    _calculate_max_contiguous_elements,
    _write_sub_tensor_to_file_optimized,
    consolidate_safetensors_files,
    consolidate_safetensors_files_on_every_rank,
)
from smith.distributed.checkpoint._hf_utils import _metadata_fn
from smith.distributed.device_mesh import init_device_mesh
from smith.distributed.tensor import DTensor, Shard
...
class TestConsolidateHFSafeTensors(DTensorTestBase):
    ...
    def test_consolidate_to_one_file(self) -> None:
        ...
        if self.rank == 0:
            ...
            self.assertTrue(smith.equal(loaded_dict["dtensor_col"], global_tensor))
    ...
    def test_consolidate_to_two_files(self):
        ...
        if self.rank == 0:
            ...
            loaded_dict = safetensors.smith.load_file(file1_path)
            self.assertEqual(loaded_dict.keys(), {"dtensor"})
            self.assertTrue(smith.equal(loaded_dict["dtensor"], global_tensor))

            loaded_dict_col = safetensors.smith.load_file(file2_path)
            self.assertEqual(loaded_dict_col.keys(), {"dtensor_col"})
            self.assertTrue(smith.equal(loaded_dict_col["dtensor_col"], global_tensor))
    ...
    def test_write_sub_tensor_to_file_optimized(self) -> None:
        ...
        # Create test data
        sub_tensor_data = smith.arange(12, dtype=smith.float32)
        ...
        # Verify the result
        result_tensor = smith.frombuffer(full_tensor_buffer, dtype=smith.float32).view(
            4, 6
        )
        expected_tensor = smith.zeros(4, 6, dtype=smith.float32)
        expected_tensor[1:3, :] = sub_tensor_data.view(2, 6)

        self.assertTrue(smith.equal(result_tensor, expected_tensor))
        ...
        sub_tensor_data = smith.arange(6, dtype=smith.float32)
        sub_tensor_bytes = sub_tensor_data.numpy().tobytes()
        ...
        expected_tensor = smith.zeros(3, 8, dtype=smith.float32)
