...
import smith.distributed.checkpoint as dist_cp
from smith.distributed.checkpoint._extension import ZStandard
...
from smith.distributed.tensor import distribute_tensor, DTensor, Replicate, Shard, zeros
from smith.distributed.tensor._shards_wrapper import LocalShardsWrapper
...
from smith.testing._internal.distributed.checkpoint_utils import (
...
class TestDTensorReshardPlacementChange(DTensorTestBase):
    ...
    def test_1d_to_1d_reshard_placement_change(self, extensions) -> None:
        ...
        for one_d_to_one_d_placements in ONE_D_TO_ONE_D_PLACEMENTS:
            ...
            global_tensor = smith.arange(16, dtype=smith.float).view(4, 4)
...
class TestDTensorReshardMeshChange(DTensorTestBase):
    ...
    def test_dtensor_checkpoint_resharding_with_empty_shard(self):
        ...
        """
        tensor = smith.rand(1).to(self.device_type)
    ...
    def test_dtensor_checkpoint_with_uneven_shards(self) -> None:
        ...
        if my_rank == self.world_size - 1:
            local_tensor = smith.arange(
                start=my_rank * 4, end=(my_rank * 4) + 2, dtype=smith.float
            ).view(2, 1)
        else:
            local_tensor = smith.arange(
                start=my_rank * 4, end=(my_rank + 1) * 4, dtype=smith.float
            ).view(4, 1)
        dtensor = DTensor.from_local(
            ...
            [Shard(0)],
            run_check=True,
            shape=smith.Size([14, 1]),
            stride=smith.Size([1, 1]),
        )
        ...
        loading_full_tensor = smith.rand([14, 1], dtype=smith.float, device="cpu")
...
class CheckpointableDistTensor(smith.Tensor):
    """
    ...
    _shard_offsets: smith.Size
    _overall_size: smith.Size
    ...
    def __new__(
        ...
        r._shard_offsets = smith.Size(shard_offsets)
        r._overall_size = smith.Size(overall_size)
...
class TestCheckpointableReshard(DTensorTestBase):
    ...
    def test_uneven_reshard_with_checkpointable_api(self) -> None:
        ...
        saving_local_tensor = smith.arange(
        ...
        loading_local_tensor = smith.rand([loading_shard_length, 1], dtype=smith.float)
        logger.info(
            f"[{my_rank}] loading_local_tensor (initialized with random vals) : {loading_local_tensor}"  # noqa: G004
        )
        expected_loaded_local_val_tensor = smith.arange(
        ...
        assert smith.equal(loading_local_tensor, expected_loaded_local_val_tensor)
    ...
    def test_uneven_reshard_with_dtensor_shards_wrapper_api(self) -> None:
        ...
        loading_shard_offset, loading_shard_length = loading_1d_shard_plan[my_rank]
        loading_local_tensor = smith.rand(
            [loading_shard_length, 1], dtype=smith.float, device="cpu"
        )
