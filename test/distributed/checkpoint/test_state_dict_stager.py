...
from smith.distributed._tensor.placement_types import Replicate, Shard
from smith.distributed.checkpoint._state_dict_stager import StateDictStager
from smith.distributed.checkpoint.staging import _ReplicationStager
from smith.distributed.checkpoint.state_dict_saver import async_save
from smith.distributed.tensor import DeviceMesh, distribute_tensor
...
def compare_state_dicts(gpu_state_dict, cpu_state_dict, rtol=1e-5, atol=1e-8):
    ...
    def compare_objects(gpu_obj, cpu_obj, path=""):
        ...
        if isinstance(gpu_obj, smith.Tensor) and isinstance(cpu_obj, smith.Tensor):
            ...
            if not smith.equal(gpu_obj.cpu(), cpu_obj):
                return (
                    False,
...
class TestStateDictStager(TestCase):
    ...
    def test_views(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                tensor1 = smith.randn(4, 4).to(device_type)
                tensor2 = tensor1.view(16)
                tensor3 = smith.randn(4, 4).to(device_type)
                state_dict = {
                    "tensor1": tensor1,
                    "tensor2": tensor2,
                    "recursive": {
                ...
                assert cpu_state_dict["tensor1"].device == smith.device("cpu")
                assert cpu_state_dict["tensor2"].device == smith.device("cpu")
                assert (
                ...
                assert recursive["tensor3"].device == smith.device("cpu")
                assert recursive["type"].tensor1.device == smith.device("cpu")
                assert (
    ...
    def test_caching(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                assert smith.all(cpu_state_dict3["tensor1"] == 42.0), (
                    "Updated values should be reflected in the cached state dict"
                )
                assert smith.all(cpu_state_dict3["tensor2"] == 42.0), (
                    "Updated values should be reflected in the cached state dict"
                )
    ...
    def test_different_dtypes(self):
        ...
        tensors = {
            "float32": smith.randn(4, 4, dtype=smith.float32).to(device_type),
            "float64": smith.randn(4, 4, dtype=smith.float64).to(device_type),
            "int32": smith.randint(-100, 100, (4, 4), dtype=smith.int32).to(
                device_type
            ),
            "int64": smith.randint(-100, 100, (4, 4), dtype=smith.int64).to(
                device_type
            ),
            "bool": smith.randint(0, 2, (4, 4), dtype=smith.bool).to(device_type),
        }
        ...
        for dtype_name, original_tensor in tensors.items():
            ...
            self.assertTrue(
                smith.allclose(cpu_tensor, original_tensor.cpu()),
    ...
    def test_empty_tensors(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                tensors = {
                    "empty_0d": smith.tensor([], dtype=smith.float32).to(device_type),
                    "empty_1d": smith.tensor([], dtype=smith.float32)
                    .reshape(0)
                    .to(device_type),
                    "empty_2d": smith.tensor([], dtype=smith.float32)
                    .reshape(0, 0)
                    .to(device_type),
                    "empty_3d": smith.tensor([], dtype=smith.float32)
                    .reshape(0, 0, 0)
                    .to(device_type),
                    "zero_dim": smith.tensor(0.0).to(device_type),  # scalar tensor
                }
    ...
    def test_complex_storage_sharing(self):
        ...
        base_tensor = smith.randn(10, 10).to(device_type)
        ...
        self.assertTrue(
            smith.all(cpu_state_dict["view1"] == 42.0),
            "view1 should reflect changes to base",
        )
        self.assertTrue(
            smith.all(cpu_state_dict["view2"] == 42.0),
            "view2 should reflect changes to base",
        )
        self.assertTrue(
            smith.all(cpu_state_dict["slice1"] == 42.0),
            "slice1 should reflect changes to base",
        )
        self.assertTrue(
            smith.all(cpu_state_dict["slice2"] == 42.0),
            "slice2 should reflect changes to base",
        )
        self.assertTrue(
            smith.all(cpu_state_dict["slice3"] == 42.0),
    ...
    def test_dataclasses(self):
        # Create tensors
        tensor1 = smith.randn(4, 4).to(device_type)
        tensor2 = smith.randn(8, 8).to(device_type)
        tensor3 = smith.randn(2, 6).to(device_type)
        tensor4 = smith.randn(3, 5).to(device_type)
        ...
        self.assertTrue(smith.allclose(cpu_state_dict["regular_tensor"], tensor2.cpu()))
        ...
        self.assertTrue(smith.allclose(complex_cpu.tensor, tensor1.cpu()))
        ...
        self.assertTrue(smith.allclose(nested_cpu.tensor, tensor3.cpu()))
        ...
        self.assertTrue(smith.allclose(frozen_cpu.tensor, tensor4.cpu()))
        ...
        self.assertFalse(smith.allclose(complex_cpu.tensor, tensor1.cpu()))
        self.assertFalse(smith.allclose(nested_cpu.tensor, tensor3.cpu()))
        self.assertFalse(smith.allclose(frozen_cpu.tensor, tensor4.cpu()))
    ...
    def test_cpu_storage_independence(self):
        ...
        """
        # Create test tensors
        tensor1 = smith.randn(4, 4)
        tensor2 = smith.randn(8, 8)
        ...
        self.assertTrue(
            smith.allclose(tensor1, cpu_tensor1),
            "CPU tensor should have the same values as the original tensor",
        )
        self.assertTrue(
            smith.allclose(tensor2, cpu_tensor2),
            "CPU tensor should have the same values as the original tensor",
        )
        ...
        self.assertFalse(smith.allclose(cloned_orginial1, tensor1))
        self.assertTrue(
            smith.allclose(cloned_orginial1, cpu_tensor1),
            "CPU tensor should have the same values as the original tensor",
        )
        self.assertTrue(
            smith.allclose(cloned_orginia2, cpu_tensor2),
            "CPU tensor should have the same values as the original tensor",
        )
    ...
    def test_tensor_pinned_and_shared(self):
        ...
        for pin_memory, share_memory in test_configs:
            with self.subTest(pin_memory=pin_memory, share_memory=share_memory):
                ...
                self.assertTrue(
                    smith.allclose(cpu_tensor1, tensor1.cpu()),
                    "CPU tensor should have the same values as the original tensor",
                )
                self.assertTrue(
                    smith.allclose(cpu_tensor2, tensor2.cpu()),
                    "CPU tensor should have the same values as the original tensor",
                )
...
class TestDTensorStateDictStager(DTensorTestBase):
    ...
    def test_dtensor(self):
        ...
        tensor = smith.randn(3, 3, device=self.device_type)
        ...
        self.assertTrue(smith.allclose(dtensor.to_local(), (tensor + 1) * 2))
        self.assertTrue(
            smith.allclose(
    ...
    def test_async_save_no_memory_leak(self):
        ...
        tensor = smith.randn(num_elements, device=device_type).to(smith.float32)
...
class TestReplicationStager(DTensorTestBase):
    ...
    def _create_simple_state_dict(self, rank: int) -> dict:
        ...
        smith.manual_seed(42 + rank)  # Different seed per rank

        return {
            "layer1.weight": smith.randn(64, 128, device="cpu"),
            "layer1.bias": smith.randn(64, device="cpu"),
            "layer2.weight": smith.randn(32, 64, device="cpu"),
            "layer2.bias": smith.randn(32, device="cpu"),
            "nested": {
                "param": smith.randn(16, 16, device="cpu"),
                "buffer": smith.randn(8, device="cpu"),
            },
            "scalar": smith.tensor(float(rank), device="cpu"),
        }
    ...
    def _verify_simple_state_dict_replication(
        ...
        def compare_tensors(actual, expected, path=""):
            if isinstance(actual, dict) and isinstance(expected, dict):
            ...
            elif isinstance(actual, smith.Tensor) and isinstance(
                ...
                self.assertTrue(
                    smith.equal(actual, expected), f"Values mismatch at {path}"
                )
            else:
    ...
    def _create_dtensor_state_dict(self, rank: int, device_mesh: DeviceMesh) -> dict:
        ...
        global_size = 128
        global_tensor = smith.arange(0, global_size * 16, dtype=smith.float32).reshape(
            global_size, 16
        )
        ...
        replicated_global = smith.full(
            (8, 8), float(global_size * 100), dtype=smith.float32, device="cpu"
        )
        ...
        return {
            ...
            "rank_scalar": smith.tensor(float(rank), device="cpu"),
        }
    ...
    def _verify_dtensor_replication(
        ...
        if "sharded_param" in replicated_dict:
            ...
            expected_global = smith.arange(0, 128 * 16, dtype=smith.float32).reshape(
                128, 16
            )
            ...
            self.assertTrue(
                smith.equal(replicated_local, expected_local),
        ...
        if "replicated_param" in replicated_dict:
            ...
            expected_tensor = smith.full(
                (8, 8), expected_value, dtype=smith.float32, device="cpu"
            )

            self.assertTrue(
                smith.equal(replicated_local, expected_tensor),
    ...
    def _create_sharded_tensor_state_dict(self, rank: int, world_size: int) -> dict:
        ...
        local_tensor = smith.arange(
            start_idx * 8, end_idx * 8, dtype=smith.float32, device="cpu"
    ...
    def test_replication_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as _:
            ...
            loaded_state_dict = smith.load(expected_path)
            self._verify_simple_state_dict_replication(
