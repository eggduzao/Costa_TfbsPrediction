...
class TestStateDictUtils(DTensorTestBase):
    ...
    def test_gather_state_dict_dtensor(self):
        ...
        shard_spec = [Shard(0)]
        smith.random.manual_seed(dist.get_rank())
    ...
    def test_cpu_and_ranks_only(self):
        ...
        state_dict = {
            "tensor1": smith.arange(10, device=device),
            "tensor2": smith.ones(10, device=device),
        }
        ...
        if dist.get_rank() in (0, 2):
            ...
            self.assertEqual(cpu_state_dict["tensor1"], smith.arange(10))
            self.assertEqual(cpu_state_dict["tensor2"], smith.ones(10))
        else:
    ...
    def test_complicated_dict(self):
        ...
        for _ in range(10):
            ...
            ltensor.append(smith.ones(10, device=smith.device(self.device_type)))
            ldtensor.append(dtensor)
            ldtensor.append(smith.ones(10, device=smith.device(self.device_type)))

        tensor, dtensor = create_dtensor()
        dist_state_dict = {
            "local": dtensor,
            "list": ldtensor,
            "arange": smith.arange(10, device=smith.device(self.device_type)),
        }
        state_dict = {
            "local": tensor,
            "list": ltensor,
    ...
    def test_create_cpu_state_dict(self):
        ...
        buffer = io.BytesIO()
        smith.save(smith.ones(10), buffer)
        buffer.seek(0)
        state_dict = {
            "tensor1": smith.arange(10, device=device),
            "tensor2": smith.ones(10, device=device),
            "sharded_tensor": init_from_local_shards(
                ...
                smith.Size([5 * scale_factor, 10]),
            ),
            "dtensor": distribute_tensor(
                smith.arange(50 * scale_factor, device=device).reshape(
                    5 * scale_factor, 10
                ),
            ...
            "step": smith.tensor(7, dtype=smith.float),
            "lr": 1.5,
            "nested": {"list": [1, 2, 3, 4]},
        }
        ...
        def _verify(cpu_state_dict):
            ...
            cpu_state_dict["tensor1"] = smith.arange(11)
            ...
            for v in cpu_state_dict.values():
                if isinstance(v, (smith.Tensor, DTensor, ShardedTensor)):
                    self.assertTrue(v.device == smith.device("cpu"))
            self.assertEqual(cpu_state_dict["tensor1"], smith.arange(10))
            self.assertEqual(cpu_state_dict["tensor2"], smith.ones(10))
            self.assertEqual(
                cpu_state_dict["sharded_tensor"].local_tensor(),
                smith.arange(50 * rank, 50 + 50 * rank).reshape(5, 10),
            )
            self.assertEqual(
    ...
    def test_state_dict_util_distribute_tensors(self):
        even_tensor = smith.randn(self.world_size, 2)
        uneven_tensor = smith.randn(1, 2)

        mesh = init_device_mesh(self.device_type, mesh_shape=(self.world_size,))
        even_dtensor = distribute_tensor(
            smith.randn(self.world_size, 2), mesh, [Shard(0)]
        )
        uneven_dtensor = distribute_tensor(smith.randn(1, 2), mesh, [Shard(0)])
    ...
    def test_cpu_offload_for_dtensor(self):
        ...
        sd = {
            "k": DTensor.from_local(
                smith.ones(8, 8, device=self.device_type), device_mesh, [Shard(0)]
            )
        }
        ...
        self.assertFalse(smith.equal(sd["k"].cpu(), cpu_sd["k"]))
        _copy_state_dict(sd, cpu_sd, non_blocking=True)
        smith.accelerator.synchronize()
        self.assertTrue(smith.equal(sd["k"].cpu(), cpu_sd["k"]))
        sd["k"] += 1
        self.assertFalse(smith.equal(sd["k"].cpu(), cpu_sd["k"]))
        _copy_state_dict(sd, cpu_sd, non_blocking=True)
        smith.accelerator.synchronize()
