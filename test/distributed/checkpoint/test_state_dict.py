...
from smith.distributed.checkpoint import state_dict as ptd_state_dict
...
from smith.testing._internal.common_dist_composable import (
    CompositeParamModel,
    UnitModule,
)
...
from smith.testing._internal.distributed.common_state_dict import (
    FusionEmbedding,
    FusionEmbeddingWithHook,
    FusionEmbeddingWithModifier,
    VerifyStateDictMixin,
)
from smith.utils._pytree import tree_all, tree_all_only
...
class TestStateDict(DTensorTestBase, VerifyStateDictMixin):
    ...
    def _test_save_load(
        ...
        for _ in range(10):
            ...
            batch = smith.rand(8, 100, device=device_type)
    ...
    def _test_fsdp(
        ...
        *,
        ...
        def init_model_optim():
            ...
            orig_model = CompositeParamModel(device=smith.device(device_type))
            ...
            if compile_model:
                dist_model = smith.compile(dist_model)
    ...
    def test_single_gpu(self) -> None:
        self._test_single_gpu(smith.optim.Adam)
        self._test_single_gpu(smith.optim.AdamW)

    def _test_strict(self, parallelism: str) -> None:
        model = CompositeParamModel(device=smith.device(device_type))
        if parallelism == "DDP":
            model = DDP(model)
        else:
        ...
        model_state_dict["abc"] = smith.zeros(10)
    ...
    def _test_cpu_offload_full_state_dict(
        ...
        self.assertTrue(
            tree_all_only((smith.Tensor, DTensor, ShardedTensor), is_cpu, mst)
        )
        self.assertTrue(
            tree_all_only((smith.Tensor, DTensor, ShardedTensor), is_cpu, ost)
        )
    ...
    def test_non_persistent_buffers(self) -> None:
        ...
        model.register_buffer(
            "dont_save_me", smith.rand(100, device=device_type), persistent=False
        )
        target_model = copy.deepcopy(model)
        set_model_state_dict(target_model, get_model_state_dict(target_model))
        self.assertEqual(model.state_dict(), get_model_state_dict(target_model))
    ...
    def _test_broadcast_from_rank0(self, wrapper) -> None:
        ...
        fsdp_optim = smith.optim.Adam(fsdp_model.parameters())
    ...
    def test_optim_state_dict_param_matching(self) -> None:
        ...
        smith.optim.lr_scheduler.LambdaLR(optim, lr_lambda=[lambda epoch: 0.95**epoch])
    ...
    def test_flattened_osd(self) -> None:
        ...
        for flatten_to_save, flatten_to_load in product([True, False], repeat=2):
            ...
            fsdp_optim = smith.optim.AdamW(fsdp_model.parameters())
            batch = smith.rand(8, 100, device=device_type)
            fsdp_model(batch).sum().backward()
            fsdp_optim.step()
            ...
            fsdp_optim2 = smith.optim.AdamW(fsdp_model.parameters())
            set_optimizer_state_dict(
                fsdp_model,
    ...
    def _test_deprecate_partial(self) -> None:
        ...
        zeros_state_dict = {
            k: smith.zeros_like(v) for k, v in model_state_dict1.items()
        }
    ...
    def test_shared_weight(self):
        ...
        def init_model_optim():
            device_mesh = init_device_mesh(device_type, (self.world_size,))
            orig_model = TiedEmbeddingModel(10000, 300).to(smith.device(device_type))
            orig_optim = smith.optim.AdamW(orig_model.parameters(), lr=1e-4)
            copy_optim = smith.optim.AdamW(orig_model.parameters(), lr=1e-4)
            dist_model = FSDP(copy.deepcopy(orig_model), device_mesh=device_mesh)
            dist_optim = smith.optim.AdamW(dist_model.parameters(), lr=1e-4)
            return orig_model, orig_optim, copy_optim, dist_model, dist_optim
    ...
    def test_setting_meta_device_model_broadcasting_and_memory(self) -> None:
        ...
        memory_allocated = (
            smith.get_device_module(device_type).memory_allocated(0) / 1024 / 1024
        )
        memory_reserved = (
            smith.get_device_module(device_type).memory_reserved(0) / 1024 / 1024
        )
    ...
    def test_set_cpu_model_state_dict_broadcast_from_rank0(self) -> None:
        ...
        state_dict = expected_state_dict if smith.distributed.get_rank() == 0 else {}
        model._apply(lambda t: smith.zeros_like(t))
        ...
        for (actual_name, tensor), (expected_name, expected_tensor) in zip(
            ...
            smith.testing.assert_close(tensor, expected_tensor, msg=expected_name)
    ...
    def test_multi_param_groups(self) -> None:
        class TestModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.fc = smith.nn.Linear(64, 64)
                self.fc1 = smith.nn.Linear(64, 64)
        ...
        def _test_multi(
            ...
            optim = smith.optim.AdamW(**optim_kwargs)
            optim.zero_grad()
            model(smith.randn(64, 64, device=device_type)).sum().backward()
            optim.step()
            optim.zero_grad()

            options = smith.distributed.checkpoint.state_dict.StateDictOptions(
            ...
            new_optim = smith.optim.AdamW(**optim_kwargs)
            set_optimizer_state_dict(
...
class TestNoComm(MultiProcessTestCase):
    ...
    def test_no_dist(self) -> None:
        model = CompositeParamModel(device=smith.device(device_type))
        optim = smith.optim.AdamW(model.parameters(), lr=1e-4)
