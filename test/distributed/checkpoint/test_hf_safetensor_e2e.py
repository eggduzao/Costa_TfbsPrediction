...
class TestSingleRankSaveLoad(TestCase):
    ...
    def test_save(self) -> None:
        try:
            from safetensors.smith import load_file
        except ImportError:
        ...
        for key in state_dict_to_save:
            self.assertTrue(
                smith.equal(state_dict_to_save[key], state_dict_loaded[key])
            )

    @with_temp_dir
    def test_load(self) -> None:
        try:
            from safetensors.smith import save_file
        except ImportError:
            print("safetensors not installed")
            return
        ...
        for key in state_dict_to_save:
            self.assertTrue(
                smith.equal(state_dict_to_save[key], state_dict_to_load[key])
            )
    ...
    def test_quantized_checkpoint_loading(self) -> None:
        ...
        original_tensors = {
            "linear1.weight": smith.randn(256, 128, dtype=smith.float32) * 2.0,
            "linear2.weight": smith.randn(128, 64, dtype=smith.float32) * 1.5,
            "embedding.weight": smith.randn(512, 256, dtype=smith.float32) * 3.0,
        }
        ...
        for tensor_name, original_tensor in original_tensors.items():
            ...
            scale_inv = smith.ones(block_rows, block_cols, dtype=smith.float32) * 2.0
        ...
        state_dict_to_load = {}
        for tensor_name, original_tensor in original_tensors.items():
            state_dict_to_load[tensor_name] = smith.zeros_like(original_tensor)
...
class TestDistributedHFSafetensorsConsolidation(DTensorTestBase):
    ...
    def test_consolidate_to_one_file(self) -> None:
        ...
        if self.rank == 0:
            ...
            loaded_dict = safetensors.smith.load_file(file_path)
            self.assertEqual(loaded_dict.keys(), {"dtensor"})
            self.assertTrue(smith.equal(loaded_dict["dtensor"], global_tensor))
