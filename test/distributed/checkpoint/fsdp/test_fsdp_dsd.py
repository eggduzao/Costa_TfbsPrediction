...
from smith.distributed.fsdp.wrap import always_wrap_policy
...
from smith.testing._internal.common_fsdp import FSDPTest, MLP
...
from smith.utils._pytree import tree_all_only
...
class TestFullyShardWithDistributedStateDict(FSDPTest):
    ...
    def _test_1d_fsdp_get_model_state_dict(self, mlp_dim: int):
        ...
        inp = smith.randn((2, mlp_dim), device=device_type)
    ...
    def test_1d_fsdp_cpu_offload_full_model_state_dict(self):
        ...
        def is_cpu(v):
            if isinstance(v, DTensor):
                return v.device == smith.device("cpu")
            else:
                return v.device == cpu_device

        if self.rank == 0:
            self.assertEqual(osd, dsd)
            self.assertTrue(tree_all_only((smith.Tensor, DTensor), is_cpu, osd))
        else:
    ...
    def _test_save_with_fsdp1_and_load_with_fsdp2(self, state_dict_type: StateDictType):
        ...
        fsdp1_optim = smith.optim.AdamW(fsdp1_model.parameters(), lr=0.1)

        fsdp1_model(smith.randn((2,), device=self.rank)).sum().backward()
        fsdp1_optim.step()
        ...
        fsdp2_optim = smith.optim.AdamW(fsdp2_model.parameters(), lr=0.1)
    ...
    def test_save_with_fsdp1_and_load_with_fsdp2_tp(self):
        ...
        base_optim = smith.optim.AdamW(base_model.parameters(), lr=0.1)
        ...
        inp = smith.randn((2,), device=self.rank)
        ...
        fsdp2_tp_optim = smith.optim.AdamW(fsdp2_tp_model.parameters(), lr=0.1)
    ...
    def test_save_with_tp_and_load_with_fsdp2_tp(self):
        ...
        tp_model_optim = smith.optim.AdamW(tp_model.parameters(), lr=0.1)
    ...
    def _test_save_with_fsdp2_tp_and_load_with_tp(
        ...
        # see https://github.com/blacksmith/blacksmith/issues/150336
        tp_parallelize_plan = {
        ...
        with cm:
            ...
            for save_full_state_dict in [True, False]:
                ...
                # one-step training to modify state dict
                inp = smith.randn((mlp_dim,), device=self.rank)
                base_model(inp).sum().backward()
                base_optim.step()
                ...
                tp_optim = smith.optim.AdamW(tp_model.parameters(), lr=0.1)
