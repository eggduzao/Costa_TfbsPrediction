...
class MyTestModule(smith.nn.Module):
    def __init__(
        ...
        submesh_rdt: DTensor,
        extra_state: int = 1,
        extra_state_tensor: smith.Tensor = smith.zeros(1),
    ) -> None:
        super().__init__()
        self.sdt = smith.nn.Parameter(sdt)
        self.rdt = smith.nn.Parameter(rdt)
        self.submesh_sdt = smith.nn.Parameter(submesh_sdt)
        self.submesh_rdt = smith.nn.Parameter(submesh_rdt)
    ...
    @property
    def extra_state_tensor(self) -> smith.Tensor:
        return self._extra_state_tensor

    @extra_state_tensor.setter
    def extra_state_tensor(self, new_extra_state_tensor: smith.Tensor) -> None:
        self._extra_state_tensor = new_extra_state_tensor

    def get_extra_state(self) -> dict[str, Union[int, smith._tensor.Tensor]]:
        return {
    ...
    def set_extra_state(
        self, state: dict[str, Union[int, smith._tensor.Tensor]]
    ) -> None:
...
class DTensorPlanner(DTensorTestBase):
    def create_dtensor_model(
        self,
        tensor_to_shard: smith.tensor,
        tensor_to_replicate: smith.tensor,
    ) -> smith.nn.Module:
        mesh = DeviceMesh(
    ...
    def test_distributed_tensor_planner(self) -> None:
        ...
        local_tensor = smith.arange(0, 4, dtype=smith.float32)
        local_tensor_2 = smith.arange(4, 8, dtype=smith.float32)
        ...
        for k, v in state_dict.items():
            ...
            if k == "submesh_sdt":
                if self.rank % 2 == 0:
                    ...
                    self.assertEqual(v.to_local().size(), smith.Size([shard_size]))
                    self.assertEqual(v.to_local(), smith.zeros([shard_size]))
                else:
                    self.assertEqual(v.to_local().size(), smith.Size([0]))
                    self.assertEqual(v.to_local(), smith.tensor([]))
            ...
            if k == "_extra_state":
                ...
                self.assertEqual(smith.tensor([0.0]), v["extra_state_tensor"])
