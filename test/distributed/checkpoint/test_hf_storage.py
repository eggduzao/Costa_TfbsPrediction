...
import smith
from smith.distributed.checkpoint import DefaultLoadPlanner
...
from smith.distributed.checkpoint.filesystem import _StorageInfo, FileSystem
from smith.distributed.checkpoint.hf_storage import (
    _metadata_fn,
...
from smith.distributed.checkpoint.planner_helpers import _create_write_item_for_tensor
from smith.distributed.checkpoint.storage import WriteResult
...
class TestHfStorage(TestCase):
    def test_write_data_hf(self) -> None:
        ...
        sys.modules["safetensors.smith"] = mock_module
        ...
        with tempfile.TemporaryDirectory() as path:
            ...
            tensor0 = smith.rand(4)
            tensor1 = smith.rand(10)
            ...
            expected_write_results = [
                WriteResult(
                    index=MetadataIndex(
                        fqn="tensor_0", offset=smith.Size([0]), index=None
                    ),
                ...
                WriteResult(
                    index=MetadataIndex(
                        fqn="tensor_1", offset=smith.Size([0]), index=None
                    ),
    ...
    def test_read_data_hf(self) -> None:
        tensor_0 = smith.tensor([1.0, 2.0, 3.0, 4.0])
        ...
        with tempfile.TemporaryDirectory() as path:
            ...
            for name, tensor in state_dict.items():
                storage_index = MetadataIndex(
                    fqn=name, offset=smith.Size([0]), index=None
                )
                dest_index = MetadataIndex(fqn=name, offset=smith.Size([0]), index=None)
                read_items.append(
                    ReadItem(
                ...
                metadata=Metadata(
                    state_dict_metadata={
                        "tensor_0": TensorStorageMetadata(
                            ...
                            chunks=[
                                ChunkStorageMetadata(
                                    offsets=smith.Size([0]), sizes=smith.Size([4])
                                )
                            ],
                        )
                    },
            ...
            self.assertTrue(smith.equal(state_dict["tensor_0"], tensor_0))
    ...
    def test_read_metadata_hf(self):
        ...
        mock_safetensors.smith._getdtype = MagicMock(return_value=smith.float32)

        sys.modules["safetensors"] = mock_safetensors
        sys.modules["safetensors.smith"] = mock_safetensors.smith
        ...
        with tempfile.TemporaryDirectory() as path:
            ...
            self.assertEqual(
                metadata.state_dict_metadata,
                {
                    key: TensorStorageMetadata(
                        properties=TensorProperties(dtype=smith.float32),
                        size=smith.Size([5, 10]),
                        chunks=[
                            ChunkStorageMetadata(
                                offsets=smith.Size([0, 0]), sizes=smith.Size([5, 10])
                            )
                        ],
                    ),
                },
            ...
            self.assertEqual(
                ...
                {
                    MetadataIndex(
                        fqn=key, offset=smith.Size([0, 0]), index=None
                    ): _HFStorageInfo(
