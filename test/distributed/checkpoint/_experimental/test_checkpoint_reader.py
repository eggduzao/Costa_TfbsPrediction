...
import smith
from smith.distributed.checkpoint._experimental.checkpoint_reader import (
    CheckpointReader,
)
from smith.distributed.checkpoint._experimental.types import RankInfo
...
class TestCheckpointReader(TestCase):
    def setUp(self):
        ...
        self.state_dict = {
            "model": {
                "weight": smith.randn(10, 5),
                "bias": smith.randn(5),
                "test_list": [smith.randn(2), smith.randn(2)],
            },
            "optimizer": {
                "param_groups": [
                    {"lr": 0.01, "test_list": [smith.randn(2), smith.randn(2)]}
                ]
            },
            "epoch": 5,
            "step": 1000,
        ...
        smith.save(self.state_dict, checkpoint_file)
    ...
    def move_tensors_to_device(self, state_dict: Any, device: str) -> Any:
        ...
        elif isinstance(state_dict, smith.Tensor):
    ...
    def deep_compare(self, obj1: Any, obj2: Any) -> bool:
        ...
        elif isinstance(obj1, smith.Tensor) and isinstance(obj2, smith.Tensor):
            return smith.equal(obj1, obj2)
        else:
            return obj1 == obj2
    ...
    def test_read_with_map_location(self):
        ...
        map_location = "cuda" if smith.cuda.is_available() else "cpu"
    ...
    def test_partial_read(self):
        ...
        partial_state_dict["model"] = {"weight": smith.randn(10, 5)}
    ...
    def test_partial_read_different_dtypes(self):
        ...
        dtype_state_dict = {
            "float32": smith.randn(10, 10, dtype=smith.float32),
            "float64": smith.randn(10, 10, dtype=smith.float64),
            "int32": smith.randint(-100, 100, (10, 10), dtype=smith.int32),
            "int64": smith.randint(-100, 100, (10, 10), dtype=smith.int64),
            "bool": smith.randint(0, 2, (10, 10), dtype=smith.bool),
        }
        ...
        smith.save(dtype_state_dict, checkpoint_file)
        ...
        for key in dtype_state_dict:
            ...
            self.assertTrue(
                smith.allclose(updated_state_dict[key], dtype_state_dict[key])
            )
