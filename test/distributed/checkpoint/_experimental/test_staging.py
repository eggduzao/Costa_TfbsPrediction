...
class TestDefaultStager(TestCase):
    def setUp(self) -> None:
        ...
        self.state_dict = {
            ...
            "nested": {"inner_tensor": smith.ones(2, 2), "inner_value": 42},
        }
    ...
    def test_different_option_combinations(self) -> None:
        ...
        if smith.cuda.is_available():
            ...
            test_cases.append(
                CheckpointStagerConfig(
                    use_pinned_memory=smith.accelerator.is_available(),
                    use_shared_memory=False,
            ...
            test_cases.append(
                CheckpointStagerConfig(
                    ...
                    use_shared_memory=False,
                    use_async_staging=False,
                    use_non_blocking_copy=smith.accelerator.is_available(),
                )
            )
        ...
        for options in test_cases:
            with self.subTest(options=options):
                ...
                if options.use_async_staging and smith.accelerator.is_available():
                    result = stager.stage(self.state_dict)
                    self.assertIsInstance(result, Future)
                    staged_dict = result.result()
                else:
    ...
    def test_cuda_tensors_staging(self) -> None:
        ...
        cuda_state_dict = {
            "cuda_tensor": smith.randn(3, 4).cuda(),
            "cpu_tensor": smith.randn(2, 3),
            "mixed_model": {
                "weight": smith.randn(5, 5).cuda(),
                "bias": smith.randn(5).cuda(),
            },
        }
    ...
    def test_multiple_staging_operations(self) -> None:
        ...
        state_dicts = [
            {"model1": smith.nn.Linear(5, 3).state_dict()},
            {"model2": smith.nn.Conv2d(3, 16, 3).state_dict()},
