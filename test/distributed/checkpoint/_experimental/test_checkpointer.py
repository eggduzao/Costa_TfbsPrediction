...
import smith
from smith.distributed.checkpoint._experimental.checkpoint_process import (
    CheckpointProcess,
...
from smith.distributed.checkpoint._experimental.checkpoint_writer import (
    CheckpointWriter,
    CheckpointWriterConfig,
)
from smith.distributed.checkpoint._experimental.checkpointer import (
    AsyncCheckpointer,
    Checkpointer,
    SyncCheckpointer,
)
from smith.distributed.checkpoint._experimental.staging import (
...
class TestCheckpointer(TestCase):
    ...
    def setUp(self):
        ...
        self.state_dict = {
            "model": smith.nn.Linear(10, 5).state_dict(),
        ...
        for checkpointer_type, checkpointer in self._get_checkpointers():
            with self.subTest(checkpointer_type=checkpointer_type):
                try:
                    ...
                    limited_state_dict = {"model": smith.nn.Linear(10, 5).state_dict()}
                    stage_future, write_future = self._save_checkpoint(
            ...
            with self.subTest(checkpointer_type=checkpointer_type):
                try:
                    ...
                    nested_state_dict = {
                        "model": {
                            "layer1": {
                                "weight": smith.randn(5, 10),
                                "bias": smith.randn(5),
                            },
                            "layer2": {
                                "weight": smith.randn(2, 5),
                                "bias": smith.randn(2),
                            },
                        },
...
class TestAsyncCheckpointerSpecific(TestCase):
    ...
    def test_async_sequential_saves_wait(self):
        ...
        try:
            ...
            loaded1 = smith.load(checkpoint_file1)
            loaded2 = smith.load(checkpoint_file2)
    ...
    def test_async_multiple_saves_ordering(self):
        ...
        try:
            ...
            state_dicts = [
                {"epoch": 1, "model": smith.nn.Linear(5, 3).state_dict()},
                {"epoch": 2, "model": smith.nn.Linear(5, 3).state_dict()},
                {"epoch": 3, "model": smith.nn.Linear(5, 3).state_dict()},
            ]
            ...
            for i, checkpoint_path in enumerate(checkpoint_paths, 1):
                ...
                loaded = smith.load(checkpoint_file)
