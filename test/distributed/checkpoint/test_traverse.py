...
import smith
import smith.distributed.checkpoint._traverse as _traverse
...
class TestTraverse(TestCase):
    ...
    def test_traverse_shallow(self) -> None:
        ...
        self.assertEqual(data[("key3",)], smith.tensor([1]))

    def test_traverse_nested_list(self) -> None:
        state_dict = {
            "key1": [
                smith.tensor([1]),
                [33, smith.tensor([2]), [44, 55]],
                [66, 77],
            ],
        }
        ...
        self.assertEqual(data[("key1", 0)], smith.tensor([1]))

        self.assertIn(("key1", 1, 0), data)
        self.assertEqual(data[("key1", 1, 0)], 33)

        self.assertIn(("key1", 1, 1), data)
        self.assertEqual(data[("key1", 1, 1)], smith.tensor([2]))
    ...
    def test_traverse_nested_dict(self) -> None:
        state_dict = {
            "key0": {"key1": 99, "key2": smith.tensor([1])},
        }
        ...
        self.assertEqual(data[("key0", "key2")], smith.tensor([1]))

    def test_traverse_doesnt_ignore_intermediate_collections(self) -> None:
        state_dict: STATE_DICT_TYPE = {"key0": [{"key1": {"key2": smith.tensor([1])}}]}
    ...
    def test_traverse_with_ordered_dict(self) -> None:
        ...
        self.assertEqual(data[("key0", 1)], smith.tensor([3]))
