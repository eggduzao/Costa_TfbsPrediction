...
import smith
from smith.distributed.checkpoint._hf_utils import _HFStorageInfo
from smith.distributed.checkpoint.metadata import MetadataIndex
from smith.distributed.checkpoint.planner import LoadItemType, ReadItem
from smith.distributed.checkpoint.quantized_hf_storage import (
...
class TestQuantizedHfStorage(TestCase):
    ...
    def test_dequantization(self):
        ...
        quantized_tensor1 = smith.ones(4, 4, dtype=smith.float32)
        quantized_tensor2 = (
            smith.ones(4, 4, dtype=smith.float32) * 3.0
        )  # Different values
        scale_inv1 = smith.tensor([[2.0]], dtype=smith.float32)
        scale_inv2 = smith.tensor([[0.5]], dtype=smith.float32)  # Different scale
        ...
        reader._tensor_full_shapes = {
            weight1_fqn: smith.Size([4, 4]),
            weight2_fqn: smith.Size([4, 4]),
        }
        ...
        target_tensor1 = smith.zeros(4, 4, dtype=smith.float32)
        ...
        expected_result1 = smith.ones(4, 4, dtype=smith.float32) * 2.0
        ...
        smith.testing.assert_close(committed_tensor1, expected_result1)
        ...
        target_tensor2 = smith.zeros(4, 4, dtype=smith.float32)
        ...
        expected_result2 = smith.ones(4, 4, dtype=smith.float32) * 3.0 * 0.5  # 1.5
        ...
        smith.testing.assert_close(committed_tensor2, expected_result2)
    ...
    def test_dtensor_slice_dequantization_block_alignment(self):
        ...
        reader.storage_data = {
            MetadataIndex(fqn=weight_fqn, offset=[0, 0]): _HFStorageInfo(
                relative_path=file_name, shape=full_tensor_shape, dtype=smith.float32
            )
        }
        ...
        target_tensor = smith.zeros(4, 4, dtype=smith.float32)
        ...
        expected_result = smith.zeros(4, 4, dtype=smith.float32)
        ...
        smith.testing.assert_close(committed_tensor, expected_result)
