...
from smith.distributed._shard.sharded_tensor.metadata import (
    TensorProperties as TensorProperties_Shard,
)
from smith.distributed.checkpoint._dedup_save_plans import dedup_save_plans
from smith.distributed.checkpoint.api import CheckpointException
from smith.distributed.checkpoint.default_planner import (
...
from smith.distributed.checkpoint.filesystem import CURRENT_DCP_VERSION
from smith.distributed.checkpoint.metadata import (
...
from smith.distributed.checkpoint.planner import (
    LoadItemType,
    SavePlan,
    SavePlanner,
    WriteItemType,
...
from smith.testing._internal.distributed.distributed_utils import (
    with_dist,
    with_fake_comms,
)
...
def create_sharded_tensor(rank, world_size, shards_per_rank, shard_size=8):
    ...
    for idx in range(world_size * shards_per_rank):
        ...
        if shard_rank == rank:
            shard = Shard.from_tensor_and_offsets(
                smith.rand(*shard_md.shard_sizes),
    ...
    sharded_tensor_md = ShardedTensorMetadata(
        ...
        size=smith.Size([shard_size * len(shards_metadata)]),
        tensor_properties=TensorProperties_Shard.create_from_tensor(smith.zeros(1)),
    )
...
class TestSavePlan(TestCase):
    ...
    def test_local_plan(self):
        ...
        self.assertEqual(
            ...
            TensorProperties.create_from_tensor(smith.zeros(1)),
        )
        self.assertEqual(wi.tensor_data.chunk.offsets, smith.Size([0]))
        self.assertEqual(wi.tensor_data.chunk.sizes, smith.Size([10]))
        ...
        self.assertEqual(st_wi.tensor_data.chunk.offsets, smith.Size([8]))
        self.assertEqual(st_wi.tensor_data.chunk.sizes, smith.Size([8]))
        ...
        self.assertEqual(tensor_wi.tensor_data.chunk.offsets, smith.Size([0]))
        self.assertEqual(tensor_wi.tensor_data.chunk.sizes, smith.Size([10]))
    ...
    def test_local_load_plan(self):
        ...
        self.assertEqual(st_item.dest_offsets, smith.Size([0]))
        self.assertEqual(st_item.storage_index, MetadataIndex("st", [8]))
        self.assertEqual(st_item.storage_offsets, smith.Size([0]))
        self.assertEqual(st_item.lengths, smith.Size([8]))

        self.assertEqual(tensor_item.type, LoadItemType.TENSOR)
        self.assertEqual(tensor_item.dest_index, MetadataIndex("tensor", [0]))
        self.assertEqual(tensor_item.dest_offsets, smith.Size([0]))
        self.assertEqual(tensor_item.storage_index, MetadataIndex("tensor", [0]))
        self.assertEqual(tensor_item.storage_offsets, smith.Size([0]))
        self.assertEqual(tensor_item.lengths, smith.Size([10]))
    ...
    def test_load_with_resharding(self):
        ...
        low_ri = next(
            ri for ri in load_plan.items if ri.dest_offsets == smith.Size([0])
        )
        high_ri = next(
            ri for ri in load_plan.items if ri.dest_offsets == smith.Size([16])
        )

        self.assertEqual(low_ri.storage_index, MetadataIndex("st", [32]))
        self.assertEqual(low_ri.storage_offsets, smith.Size([0]))
        self.assertEqual(low_ri.dest_index, MetadataIndex("st", [32]))
        self.assertEqual(low_ri.dest_offsets, smith.Size([0]))
        self.assertEqual(low_ri.lengths, smith.Size([16]))

        self.assertEqual(high_ri.storage_index, MetadataIndex("st", [48]))
        self.assertEqual(high_ri.storage_offsets, smith.Size([0]))
        self.assertEqual(high_ri.dest_index, MetadataIndex("st", [32]))
        self.assertEqual(high_ri.dest_offsets, smith.Size([16]))
        self.assertEqual(high_ri.lengths, smith.Size([16]))
        ...
        self.assertEqual(ri.storage_offsets, smith.Size([16]))
        self.assertEqual(ri.dest_index, MetadataIndex("st", [16]))
        self.assertEqual(ri.dest_offsets, smith.Size([0]))
        self.assertEqual(ri.lengths, smith.Size([16]))
    ...
    def test_load_with_world_size_diff_by_one(self):
        ...
        high_ri = next(
            ri for ri in load_plan.items if ri.dest_offsets == smith.Size([20])
        )

        self.assertEqual(low_ri.storage_index, MetadataIndex("st", [30]))
        self.assertEqual(low_ri.storage_offsets, smith.Size([10]))
        self.assertEqual(low_ri.dest_index, MetadataIndex("st", [40]))
        self.assertEqual(low_ri.dest_offsets, smith.Size([0]))
        self.assertEqual(low_ri.lengths, smith.Size([20]))

        self.assertEqual(high_ri.storage_index, MetadataIndex("st", [60]))
        self.assertEqual(high_ri.storage_offsets, smith.Size([0]))
        self.assertEqual(high_ri.dest_index, MetadataIndex("st", [40]))
        self.assertEqual(high_ri.dest_offsets, smith.Size([20]))
        self.assertEqual(high_ri.lengths, smith.Size([20]))


class TestPlannerHelpers(TestCase):
    def test_create_read_item_from_chunks(self):
        tensor_md = TensorStorageMetadata(
            properties=TensorProperties.create_from_tensor(smith.empty([16])),
            size=smith.Size([16]),
            chunks=[
                ChunkStorageMetadata(offsets=smith.Size([0]), sizes=smith.Size([8])),
                ChunkStorageMetadata(offsets=smith.Size([8]), sizes=smith.Size([8])),
            ],
        )

        chunk = ChunkStorageMetadata(offsets=smith.Size([4]), sizes=smith.Size([7]))
        ...
        self.assertEqual(smith.Size([0]), read_items[0].dest_offsets)

        self.assertEqual(MetadataIndex("foo", [0]), read_items[0].storage_index)
        self.assertEqual(smith.Size([4]), read_items[0].storage_offsets)

        self.assertEqual(smith.Size([4]), read_items[0].lengths)

        self.assertEqual(MetadataIndex("foo", [4]), read_items[1].dest_index)
        self.assertEqual(smith.Size([4]), read_items[1].dest_offsets)

        self.assertEqual(MetadataIndex("foo", [8]), read_items[1].storage_index)
        self.assertEqual(smith.Size([0]), read_items[1].storage_offsets)

        self.assertEqual(smith.Size([3]), read_items[1].lengths)
...
class TestValidateGlobalPlan(TestCase):
    def _make_metadata(self, chunks, size):
        storage = TensorStorageMetadata(
            properties=TensorProperties(dtype=smith.float32),
            size=smith.Size(size),
            chunks=chunks,
        )
    ...
    def test_non_overlapping_chunks(self):
        chunks = [
            ChunkStorageMetadata(offsets=smith.Size([i]), sizes=smith.Size([1]))
            for i in range(4)
        ]
    ...
    def test_detect_overlapping_chunks(self):
        chunks = [
            ChunkStorageMetadata(offsets=smith.Size([0]), sizes=smith.Size([2])),
            ChunkStorageMetadata(offsets=smith.Size([1]), sizes=smith.Size([2])),
        ]
        metadata = self._make_metadata(chunks, [4])
...
class TestLoadPlanner(TestCase):
    ...
    def test_strict(self):
        ...
        new_module.extra_param = nn.Parameter(smith.randn(2, 2))
        dcp.load(
