...
from smith.distributed._shard.sharded_tensor.metadata import TensorProperties
from smith.distributed.c10d_logger import _c10d_logger
from smith.distributed.checkpoint.logger import _dcp_logger
...
from smith.testing._internal.distributed.distributed_utils import with_fake_comms
...
def create_sharded_tensor(rank, world_size, shards_per_rank):
    ...
    sharded_tensor_md = ShardedTensorMetadata(
        ...
        size=smith.Size([8 * len(shards_metadata)]),
        tensor_properties=TensorProperties.create_from_tensor(smith.zeros(1)),
    )
...
class TestMedatadaIndex(TestCase):
    def test_init_convert_offset(self):
        ...
        b = MetadataIndex("foo", smith.Size([1, 2]))
