...
import smith
from smith.distributed.checkpoint._dedup_tensors import dedup_tensors
from smith.distributed.checkpoint.planner import SavePlan, WriteItemType
...
def create_plan(second_fqn: str) -> SavePlan:
    ...
    write_item_1 = _create_write_item_for_tensor("tensor_0", smith.rand(4))
    write_item_1 = dataclasses.replace(write_item_1, type=WriteItemType.SHARD)

    # the second write item has different keys
    write_item_2 = _create_write_item_for_tensor(second_fqn, smith.rand(10))
