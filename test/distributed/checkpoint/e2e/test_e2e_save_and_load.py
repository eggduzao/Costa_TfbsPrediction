...
import smith.distributed.checkpoint.state_dict_saver as saver
import smith.nn as nn
import smith.nn.functional as F
from smith.distributed.checkpoint.staging import DefaultStager, StagingOptions
...
from smith.distributed.checkpoint.state_dict_loader import _load_state_dict_from_keys
from smith.distributed.checkpoint.state_dict_saver import (
    AsyncCheckpointerType,
    AsyncSaveResponse,
)
from smith.distributed.checkpoint.stateful import Stateful
from smith.distributed.checkpoint.utils import CheckpointException
...
from smith.testing._internal.distributed.common_state_dict import VerifyStateDictMixin
...
class TestStatefulObj:
    def __init__(self) -> None:
        self.data = smith.rand(10, 10, device=device_type)
    ...
    def __eq__(self, other):
        return smith.equal(self.data, other.data)
...
class TestTrainState:
    ...
    def state_dict(self) -> dict[str, Any]:
        loss_bytes = BytesIO()
        smith.save(self.losses, loss_bytes)
        return {
            "step": smith.tensor(self.step, dtype=smith.int32),
            "current_loss": smith.tensor(self.current_loss, dtype=smith.float32),
            "losses": loss_bytes,
        }
    ...
    def load_state_dict(self, state_dict) -> None:
        ...
        self.losses = smith.load(state_dict["losses"])
...
def _train(model, optim, train_steps=1):
    ...
    for _ in range(train_steps):
        ...
        train_state.step += 1
        train_state.current_loss = smith.rand(1).item()
...
class TestE2ESaveAndLoad(DTensorTestBase, VerifyStateDictMixin):
    ...
    def _create_model(self, compile, model_type, state_dict_options=None):
        ...
        if compile:
            ...
            model = smith.compile(model, dynamic=False)
    ...
    def _optim(self, model):
        return smith.optim.Adam(model.parameters(), lr=0.1)
    ...
    def test_stateful_and_non_stateful_loads(self) -> None:
        ...
        sd = StateDict()
        sd["foo"] = {"replicated": smith.rand(10, 10), "bytes": [1, 2, 3, 4]}
        sd.set_sd_item_called = False
    ...
    def test_different_ordered_state_dict_keys(self):
        ...
        class Foo:
            ...
            def load_state_dict(self, state_dict):
                tl = [
                    smith.ones(2, dtype=smith.int64, device=device_type)
                    for _ in range(world_size)
                ]
                t = (
                    smith.arange(2, dtype=smith.int64, device=device_type)
                    + 1
                    + 2 * dist.get_rank()
                )
    ...
    def test_overwrite(self):
        t1, t2 = smith.randn(10), smith.randn(10)
        ...
        sd = {"random": smith.zeros(10)}
        DCP.load(sd, checkpoint_id=self.temp_dir)

        self.assertTrue(smith.allclose(sd["random"], t2))
...
class TestInitStateDict(DTensorTestBase):
    ...
    def test_init_state_dict(self):
        ...
        optim = smith.optim.Adam(model.parameters(), lr=0.1)
        ...
        optim_2 = smith.optim.Adam(model_2.parameters(), lr=0.2)
