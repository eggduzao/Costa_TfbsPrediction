...
class TestDistributedReshardOnLoad(ShardedTensorTestBase):
    ...
    def load_tensor(self, tensor: ShardedTensor) -> smith.Tensor:
        res = smith.zeros(tensor.shape, device="cpu") if dist.get_rank() == 0 else None
        ...
        for save_spec in specs:
            for load_spec in specs:
                save_dict = {
                    ...
                    "replicated": smith.rand(tensor_size, device="cpu"),
                }
                ...
                load_dict = {
                    "sharded": smith.zeros(tensor_size, device="cpu"),
