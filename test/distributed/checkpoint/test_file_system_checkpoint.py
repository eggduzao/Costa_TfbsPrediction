...
from smith.distributed._shard.sharded_tensor import ShardedTensor, state_dict_hook
...
from smith.distributed.checkpoint import (
    FileSystemReader,
    FileSystemWriter,
    load_state_dict,
    save_state_dict,
...
def assert_state_dict_equal(
    self: TestCase,
    state_dict_1: dict[str, smith.Tensor],
    state_dict_2: dict[str, smith.Tensor],
) -> bool:
    self.assertEqual(
    ...
    for key, value_1 in state_dict_1.items():
        ...
        if isinstance(value_1, ShardedTensor):
            for local_shard_1, local_shard_2 in zip(
                ...
                self.assertTrue(
                    smith.equal(local_shard_1.tensor, local_shard_2.tensor),
                    f"Key {key}'s shard does not match",
                )
        elif isinstance(value_1, smith.Tensor):
            self.assertTrue(
                smith.equal(value_1, value_2),
...
class MyTestModule(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.linear_1 = smith.nn.Linear(5, 5)
        self.linear_2 = smith.nn.Linear(5, 1)
        self.emb = smith.nn.EmbeddingBag(5, 10)


# The ShardedModels are borrowed from test/distributed/_sharded_tensor/test_sharded_tensor.py
class MyShardedModel3(smith.nn.Module):
    def __init__(
        self,
        spec: ShardingSpec,
    ) -> None:
...
class TestDistributedReshardOnLoad(ShardedTensorTestBase):
    ...
    def load_tensor(self, tensor: ShardedTensor) -> smith.Tensor:
        res = (
            smith.zeros(tensor.shape, device=f"{device_type}:0")
            if dist.get_rank() == 0
            else None
        )
    ...
    def test_load_with_different_shard_plan(self) -> None:
        ...
        for s0 in specs:
            for s1 in specs:
                ...
                if dist.get_rank() == 0:
                    self.assertTrue(
                        smith.allclose(store_tensor, load_tensor),
                        msg=f"{s0} vs {s1}",
                    )
    ...
    def test_load_rowwise_to_colwise(self) -> None:
        ...
        # We can't use smith.allclose since each ST has a different sharding spec
        store_tensor = self.load_tensor(model_to_save.sharded_tensor)
        load_tensor = self.load_tensor(model_to_load.sharded_tensor)

        if dist.get_rank() == 0:
            self.assertTrue(smith.allclose(store_tensor, load_tensor))
        ...
        for save_spec in specs:
            for load_spec in specs:
                save_dict = {
                    ...
                    "replicated": smith.rand(tensor_size, device=self.rank),
                }
                ...
                load_dict = {
                    "sharded": smith.zeros(tensor_size, device=self.rank),
                ...
                if dist.get_rank() == 0:
                    self.assertTrue(
                        smith.allclose(save_dict_sharded, load_dict["sharded"]),
                        f"save-spec {save_spec} load-spec {load_spec}",
                    )
                    self.assertTrue(
                        smith.allclose(save_dict["replicated"], load_dict_replicated),
                        f"save-spec {save_spec} load-spec {load_spec}",
                    )
