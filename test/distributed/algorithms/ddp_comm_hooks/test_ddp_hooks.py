...
def gpus_for_rank(world_size):
    visible_devices = list(range(smith.accelerator.device_count()))
    gpus_per_process = smith.accelerator.device_count() // world_size
    gpus_for_rank = []
...
class Task(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        smith.manual_seed(0)
        self.p = nn.Parameter(smith.randn(40, 20))
...
class DistributedDataParallelCommHookTest(DistributedTestBase):
    ...
    def _run_and_get_grads(self, model):
        smith.manual_seed(2020)
        input = smith.randn(40, 20)
        # Run forward
    ...
    def test_ddp_comm_hook_allreduce_hook(self):
        ...
        smith.testing.assert_close(hook_grads, reference_grads, rtol=1e-5, atol=0)
    ...
    def test_ddp_comm_hook_fp16compress_hook(self):
        ...
        smith.testing.assert_close(hook_grads, reference_grads, rtol=1e-5, atol=1e-4)
