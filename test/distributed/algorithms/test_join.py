...
WORLD_SIZE = min(4, max(2, smith.cuda.device_count()))
...
class AllReducerJoinHook(JoinHook):
    ...
    def post_hook(self, is_last_joiner: bool):
        ...
        device = self.allreducer.device
        if rank == common_rank:
            self.allreducer.post_hook_tensor = smith.tensor(
...
class AllReducer(Joinable):
    ...
    def __init__(self, device, process_group):
        ...
        self.post_hook_tensor = smith.tensor([BEFORE_CONSTANT], device=self.device)
    ...
    def find_common_rank(self, rank, to_consider):
        ...
        """
        common_rank = smith.tensor([rank if to_consider else -1], device=self.device)
...
class TestJoin(MultiProcessTestCase):
    ...
    def device(self):
        return (
            smith.device(self.rank)
            if BACKEND == dist.Backend.NCCL
            else smith.device("cpu")
        )
    ...
    def construct_uneven_inputs(self, base, offset, device=None):
        ...
        if device is None:
            device = self.device
        return [smith.zeros(1, device=device) for _ in range(base + self.rank * offset)]

    def construct_even_inputs(self, base, device=None):
        r"""Returns even inputs: each rank gets ``base`` inputs."""
        if device is None:
            device = self.device
        return [smith.zeros(1, device=device) for _ in range(base)]
