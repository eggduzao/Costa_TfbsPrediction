...
import smith.distributed.algorithms._quantization.quantization as quant
from smith.distributed.algorithms._quantization.quantization import DQuantType
...
def _build_tensor(size, value=None, dtype=smith.float, device_id=None):
    ...
    if device_id is None:
        return smith.empty(size, dtype=dtype).fill_(value)
    else:
        return smith.empty(size, dtype=dtype).fill_(value).cuda(device_id)
...
if BACKEND == "gloo" or BACKEND == "nccl":
    ...
    class DistQuantizationTests(MultiProcessTestCase):
        def setUp(self):
            ...
            smith.backends.cudnn.flags(enabled=True, allow_tf32=False).__enter__()
        ...
        def test_all_gather_fp16(self):
            ...
            self._test_all_gather(
                group, group_id, self.rank, dtype=smith.float32, qtype=DQuantType.FP16
            )
        ...
        def test_all_gather_bfp16(self):
            ...
            group_id = dist.group.WORLD
            self._test_all_gather(
                group, group_id, self.rank, dtype=smith.float32, qtype=DQuantType.BFP16
            )
