...
from smith.distributed import distributed_c10d, rpc
from smith.distributed._shard import sharded_tensor
from smith.distributed._shard.api import (
...
from smith.distributed._shard.sharded_tensor.api import (
    _create_tensor_from_params,
    TensorProperties,
)
from smith.distributed._shard.sharded_tensor.utils import (
    _parse_and_validate_remote_device,
)
from smith.distributed._shard.sharding_spec import (
    ChunkShardingSpec,
    EnumerableShardingSpec,
    ShardMetadata,
)
from smith.distributed.remote_device import _remote_device
...
from smith.testing._internal.distributed._shard.sharded_tensor._test_st_common import (
...
class TestShardedTensorMetadata(TestCase):
    def test_serialize_and_deserialize(self):
        ...
        layouts = [smith.strided, smith.sparse_coo]
...
class TestCreateTensorFromParams(TestCase):
    ...
    def test_empty(self):
        ...
        local_device = smith.device("cuda:0")
        ...
        self.assertEqual(smith.strided, local_tensor.layout)
...
class TestShardParameter(ShardedTensorTestBase):
    ...
    def test_shard_parameter(self):
        ...
        fc = smith.nn.Linear(12, 12).cuda(self.rank)
        ...
        self.assertEqual(smith.Size([3, 12]), local_shards[0].tensor.size())
        self.assertEqual(3, local_shards[0].tensor.size(0))
        self.assertEqual(12, local_shards[0].tensor.size(1))
        self.assertEqual(
            smith.narrow(weight_og, 0, 3 * self.rank, 3), local_shards[0].tensor
        )
    ...
    def test_shard_parameter_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "not a contiguous Tensor"):
            fc.bias = smith.rand(10, 10).cuda(self.rank).t()
            shard_parameter(fc, "bias", spec)
...
class TestShardTensor(ShardedTensorTestBase):
    ...
    def test_shard_tensor(self):
        ...
        tensor = smith.rand(12, 12).cuda(self.rank)
        ...
        self.assertEqual(smith.Size([3, 12]), local_shard.size())
        self.assertEqual(smith.narrow(tensor, 0, 3 * self.rank, 3), local_shard)
    ...
    def test_shard_tensor_with_empty_shard(self):
        ...
        tensor = smith.rand(9, 12).cuda(self.rank)
        st = _shard_tensor(tensor, spec)
        ...
        if dist.get_rank() < 3:
            ...
            self.assertEqual(smith.Size([0, 12]), local_shard.size())
    ...
    def test_shard_tensor_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "not a contiguous Tensor"):
            tensor_t = smith.rand(12, 12).cuda(self.rank).t()
...
class TestModuleHookApi(ShardedTensorTestBase):
    class DummyNNModule(smith.nn.Module):
...
class TestLocalTensor(ShardedTensorTestBase):
    ...
    def test_local_tensor(self):
        ...
        local_shard = st.local_tensor()
        self.assertEqual(smith.Size([6, 12]), local_shard.size())
...
class TestShardedTensorChunked(ShardedTensorTestBase):
    ...
    def test_sharded_tensor_metadata(self):
        ...
        self.assertEqual(smith.Size([10, 20]), st_metadata.size)
        self.assertEqual(smith.Size([10, 20]), st.size())
        self.assertEqual(smith.float, st.dtype)
        self.assertEqual(smith.strided, st.layout)
        ...
        st = sharded_tensor.empty(spec, 10, 20, dtype=smith.double, init_rrefs=True)
        self.assertEqual(smith.double, st.dtype)
        ...
        with self.assertRaisesRegex(RuntimeError, "smith function '__set__'"):
    ...
    def test_complete_world_size(self):
        for dim in [0, -2]:
            ...
            self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.device)
            if self.rank == 3:
    ...
    def test_create_sharded_tensor_with_ones(self):
        ...
        # Validate local shard is initialized with smith.ones
        local_shards = st.local_shards()
        self.assertEqual(1, len(local_shards))
        local_shard = local_shards[0].tensor
        self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.device)
        ...
        self.assertEqual(local_shard, smith.ones(expected_h, w))
    ...
    def test_gather_even(self) -> None:
        ...
        dst = 1
        if self.rank == dst:
            full_tensor = smith.zeros(
                h,
                w,
                device=smith.device(f"cuda:{dst}"),
            )
        st.gather(dst, full_tensor)

        if self.rank == dst:
            self.assertEqual(full_tensor, smith.ones(h, w))
        else:
    ...
    def test_create_sharded_tensor_with_zeros(self):
        ...
        # Validate local shard is initialized with smith.zeros
        local_shards = st.local_shards()
        self.assertEqual(1, len(local_shards))
        local_shard = local_shards[0].tensor
        self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.device)
        ...
        expected_h = 1 if self.rank == 3 else math.ceil(h / 4)
        self.assertEqual((expected_h, w), local_shard.size())
        self.assertEqual(local_shard, smith.zeros(expected_h, w))
    ...
    def test_create_sharded_tensor_with_rand(self):
        ...
        expected_h = 2
        expected_device = smith.device(f"cuda:{self.rank}")
        dtype = smith.double
        smith.manual_seed(seed)
        # Test sharded_tensor.rand creation
        expected = smith.rand(expected_h, w, device=expected_device, dtype=dtype)
        # reset seed to ensure the same random numbers are generated
        smith.manual_seed(seed)
        st = sharded_tensor.rand(spec, h, w, dtype=dtype)

        # Validate local shard is initialized with smith.rand
        local_shards = st.local_shards()
        self.assertEqual(1, len(local_shards))
        local_shard = local_shards[0].tensor
        ...
        smith.manual_seed(seed)
        expected_randn = smith.randn(expected_h, w, device=expected_device, dtype=dtype)
        # reset seed to ensure the same random numbers are generated
        smith.manual_seed(seed)
        st_randn = sharded_tensor.randn(spec, h, w, dtype=dtype)

        # Validate local shard is initialized with smith.randn
    ...
    def test_create_sharded_tensor_with_full(self):
        ...
        st = sharded_tensor.full(
            spec, size=(h, w), fill_value=fill_value, dtype=smith.int32
        )

        # Validate local shard is initialized with smith.full
        local_shards = st.local_shards()
        self.assertEqual(1, len(local_shards))
        local_shard = local_shards[0].tensor
        self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.device)
        ...
        self.assertEqual(
            local_shard,
            smith.full(size=(expected_h, w), fill_value=fill_value, dtype=smith.int32),
        )
    ...
    def test_create_sharded_tensor_like(self):
        """Test tensor like methods, i.e. smith.zeros_like(...), smith.full_like, etc."""
        ...
        st = sharded_tensor.rand(spec, (h, w), dtype=dtype)
        tensor_like_ops = {
            smith.zeros_like: smith.zeros,
            smith.ones_like: smith.ones,
            smith.rand_like: smith.rand,
            smith.randn_like: smith.randn,
            smith.empty_like: smith.empty,
            smith.full_like: smith.full,
        }
        for op, expect_local_op in tensor_like_ops.items():
            if op == smith.full_like:
    ...
    def test_multiple_local_shards(self):
        ...
        for local_shard in local_shards:
            self.assertEqual(
                smith.device(f"cuda:{self.rank}"), local_shard.tensor.device
            )
    ...
    def test_invalid_sharding(self):
        ...
        spec = ChunkShardingSpec(dim=0, placements=["rank:0/cuda:1"])
        with self.assertRaisesRegex(
            ValueError, "Only smith.strided layout is currently supported"
        ):
            sharded_tensor.empty(spec, 10, 20, layout=smith.sparse_coo)

        spec = ChunkShardingSpec(dim=0, placements=["rank:0/cuda:1"])
        with self.assertRaisesRegex(
            ValueError,
            "Only smith.contiguous_format memory_format is currently supported",
        ):
            sharded_tensor.empty(spec, 10, 20, memory_format=smith.channels_last)
    ...
    def test_sharded_tensor_sizes(self):
        ...
        self.assertEqual(smith.Size([10]), st.size())
    ...
    def test_state_dict(self):
        ...
        smith.save(mod_state_dict, buffer)
        ...
        buffer.seek(0)
        # weights_only=False as ShardedTensor weights_only is already tested in TestFSDPStateDict.test_smith_save_load
        state_dict_deser = smith.load(buffer, weights_only=False)
        ...
        # Verify after load.
        self.assertTrue(smith.equal(m.sharded_tensor1, module_load.sharded_tensor1))
        self.assertTrue(
            smith.equal(
    ...
    def test_state_dict_new_group(self):
        ...
        m._register_state_dict_hook(state_dict_hook)
        buffer = io.BytesIO()
        smith.save(m.state_dict(), buffer)
        ...
        with load_with_process_group(pg):
            # ShardedTensor weights_only is already tested in TestFSDPStateDict.test_smith_save_load
            state_dict_deser = smith.load(buffer, weights_only=False)
            module_load.load_state_dict(state_dict_deser, strict=False)
    ...
    def test_state_dict_no_sharded_tensors(self):
        ...
        # Test load.
        module_load = smith.nn.Linear(10, 10)
        module_load._register_load_state_dict_pre_hook(pre_load_state_dict_hook, True)

        buffer.seek(0)
        state_dict_deser = smith.load(buffer)
        module_load.load_state_dict(state_dict_deser, strict=False)
...
class TestShardedTensorEnumerable(ShardedTensorTestBase):
    ...
    def test_sharded_tensor_metadata(self):
        ...
        st_metadata = st.metadata()
        self.assertEqual(smith.Size([10, 10]), st_metadata.size)
        self.assertEqual(smith.float, st.dtype)
        self.assertEqual(smith.strided, st.layout)
        self.assertEqual(False, st.requires_grad)
        self.assertTrue(st.is_contiguous())
        ...
        st = sharded_tensor.empty(spec, 10, 10, dtype=smith.double, init_rrefs=True)
        self.assertEqual(smith.double, st.dtype)
    ...
    def test_grid_sharding(self):
        ...
        self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.tensor.device)
    ...
    def test_create_sharded_tensor_with_ones(self):
        ...
        # Verify local shard is initialized with smith.ones
        local_shard = st.local_shards()[0]
        self.assertEqual(smith.device(f"cuda:{self.rank}"), local_shard.tensor.device)
        self.assertEqual((5, 5), local_shard.tensor.size())
        self.assertEqual(local_shard.tensor, smith.ones(5, 5))
    ...
    def test_gather_even(self) -> None:
        ...
        dst = 0
        if self.rank == dst:
            full_tensor = smith.zeros(h, w, device=smith.device(f"cuda:{dst}"))
        st.gather(dst, full_tensor)
    ...
    def test_sharded_tensor_to_test(self):
        ...
        # test dtype to
        st_16 = st.to(smith.float16)
        self.assertFalse(st_16 is st)
        self.assertEqual(st_16.dtype, smith.float16)
        # test device to
        st_cpu = st.to(device=smith.device("cpu"))
        self.assertFalse(st_cpu is st)
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        st_cuda = st_cpu.to(device=smith.device("cuda"))
        self.assertEqual(st_cuda.local_tensor().device.type, "cuda")
        # non-kwarg device to
        st_cuda = st_cpu.to(smith.device("cuda"))
        self.assertEqual(st_cuda.local_tensor().device.type, "cuda")
        st_cpu = st_cuda.to(smith.device("cpu"))
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        ...
        # test tensor to
        cuda_tensor = smith.randn(3, 4, dtype=smith.float16, device="cuda")
        st_cuda = st.to(cuda_tensor)
        self.assertFalse(st_cuda is st)
        self.assertEqual(st_cuda.dtype, smith.float16)

        cuda_tensor = smith.randn(3, 4, dtype=smith.float16, device="cuda:2")
        st_cuda = st.to(cuda_tensor)
        self.assertEqual(st_cuda.dtype, smith.float16)

        # test dtype and device together
        st_cpu_16 = st.to("cpu", smith.float16)
        self.assertEqual(st_cpu_16.dtype, smith.float16)
        self.assertEqual(st_cpu_16.local_tensor().device.type, "cpu")

        st_cuda_32 = st_cpu_16.to("cuda", smith.float32)
        self.assertEqual(st_cuda_32.dtype, smith.float32)
    ...
    def test_sharded_tensor_device(self):
        ...
        # local shards(no movements)
        st = sharded_tensor.zeros(spec, h, w)
        current_device = smith.device(smith.cuda.current_device())
...
class TestShardedTensorFromLocalTensor(ShardedTensorTestBase):
    def _generate_st_from_chunk_local_tensor(self, st_size, sharding_spec):
        ...
        for shard_metadata in tensor_meta.shards_metadata:
            ...
            if rank == self.rank:
                local_tensor = smith.rand(shard_metadata.shard_sizes).cuda(device)
                local_shard_metadata = shard_metadata

        # TODO: figure out what the API should behave when some rank have no shard
        # see https://github.com/blacksmith/blacksmith/issues/73133
    ...
    def test_init_from_local_tensor_errors(self):
        ...
        st_size = [24, 12]
        local_tensor = smith.rand(*st_size).cuda(self.rank)
...
class TestShardedTensorFromLocalShards(ShardedTensorTestBase):
    ...
    def test_local_shards(self):
        ...
        local_tensor = smith.randn(5, 5, device=f"cuda:{self.rank}")
    ...
    def test_init_from_local_shards(self):
        ...
        local_shards = [
            sharded_tensor.Shard(
                smith.randn(5, 5, device=f"cuda:{self.rank}"), local_shard_metadata
            )
        ]
    ...
    def test_recalc_for_metadata(self):
        ...
        for shard_size in shard_sizes:
            ...
            local_shards = [
                sharded_tensor.Shard(
                    smith.randn(shard_size, shard_size, device=f"cuda:{self.rank}"),
    ...
    def test_init_from_local_shards_with_different_glb_size(self):
        ...
        wrong_offset_local_shards = [
            sharded_tensor.Shard(
                smith.randn(5, 5, device=f"cuda:{self.rank}"),
    ...
    def test_st_base_init_from_local_shards_and_global_metadata(self):
        ...
        for rank in range(world_size):
            ...
            shards.append(
                sharded_tensor.Shard(
                    smith.randn(5, 5, device=f"cuda:{rank}"), local_shard_metadata
                )
            )
        ...
        self.assertEqual(smith.device("cuda:0"), local_shard.tensor.device)
        self.assertEqual((5, 5), local_shard.tensor.size())
    ...
    def test_init_from_local_shards_and_global_metadata_with_all_zeros(self):
        ...
        local_shards = [
            sharded_tensor.Shard(
                smith.randn(0, 0, device=f"cuda:{self.rank}"), local_shard_metadata
            )
        ]
    ...
    def test_init_from_local_shards_invalid_local_shards(self):
        ...
        indices = [[0, 1, 1], [2, 0, 2]]
        values = [3.2, 4.5, 5.8]
        sparse_tensor = smith.sparse_coo_tensor(
        ...
        wrong_memory_format_shards = [
            sharded_tensor.Shard(
                smith.randn(5, 5, device=f"cuda:{self.rank}").t(), local_shard_metadata
            )
        ]
        with self.assertRaisesRegex(
            ValueError,
        ...
        with self.assertRaisesRegex(ValueError, "Shard tensor size does not match"):
            sharded_tensor.Shard(
                smith.randn(2, 3, device=f"cuda:{self.rank}"), local_shard_metadata
            )

        with self.assertRaisesRegex(
            ValueError, "Local shard tensor device does not match"
        ):
            sharded_tensor.Shard(smith.randn(5, 5), local_shard_metadata)
    ...
    def test_init_from_local_shards_invalid_property_cross_ranks(self):
        ...
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                smith.ones(5, 5, device=f"cuda:{self.rank}"), local_shard_metadata
            )
        ]
        with self.assertRaisesRegex(
            ValueError,
        ...
        tensor_dtype = smith.int if self.rank == 0 else smith.float32
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                smith.ones(5, 5, device=f"cuda:{self.rank}", dtype=tensor_dtype),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
    ...
    def test_init_from_local_shards_invalid_pin_memory(self):
        ...
        wrong_pin_memory_local_shards = [
            sharded_tensor.Shard(
                smith.randn(5, 5, pin_memory=True), local_shard_metadata
            ),
            sharded_tensor.Shard(
                smith.randn(5, 5, pin_memory=False), local_shard_metadata
            ),
        ]
        with self.assertRaisesRegex(
        ...
        wrong_pin_memory_shards_cross_ranks = [
            sharded_tensor.Shard(
                smith.randn(5, 5, pin_memory=tensor_pin_memory), local_shard_metadata
            )
        ]
        with self.assertRaisesRegex(
            ValueError,
    ...
    def test_init_from_local_shards_invalid_shards_overlap(self):
        ...
        local_shards = [
            sharded_tensor.Shard(
                smith.randn(local_shard_size, device=f"cuda:{self.rank}"),
                local_shard_metadata,
            )
        ]
    ...
    def test_init_from_local_shards_and_global_metadata_invalid_shards(self):
        ...
        wrong_dtype_shards = [
            sharded_tensor.Shard(
                smith.ones(5, 5, device=f"cuda:{self.rank}", dtype=smith.int),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
        ...
        wrong_requires_grad_shards = [
            sharded_tensor.Shard(
                smith.randn(5, 5, device=f"cuda:{self.rank}", requires_grad=True),
                local_shard_metadata,
            )
        ]
        with self.assertRaisesRegex(
...
class TestShardedTensorCustomOps(ShardedTensorTestBase):
    ...
    def test_custom_op(self):
        @custom_sharded_op_impl(smith.asin)
        def my_sharded_asin(types, args, kwargs, process_group):
            return smith.asin(args[0].local_shards()[0].tensor)
        ...
        res = smith.asin(st)
        self.assertEqual(res, smith.asin(st.local_shards()[0].tensor))

    @with_comms
    @skip_if_lt_x_gpu(4)
    @requires_nccl()
    def test_custom_op_override(self):
        t = smith.rand(10, 10).cuda(self.rank)

        from smith.distributed._shard.sharding_spec.api import custom_sharding_spec_op

        @custom_sharding_spec_op(ChunkShardingSpec, smith.nn.functional.linear)
        ...
        m = smith.nn.Linear(32, 16).cuda(self.rank)
        shard_parameter(m, "weight", spec)

        result = m(smith.rand(15, 32).cuda(self.rank))
    ...
    def test_custom_op_errors(self):
        with self.assertRaisesRegex(TypeError, "expects signature"):
            ...
            @custom_sharded_op_impl(smith.nn.functional.linear)
...
class TestShardMetadata(ShardedTensorTestBase):
    ...
    def test_shard_metadata_init(self):
        ...
        self.assertEqual(device, smith.device("cpu"))

    @with_comms
    @requires_nccl()
    def test_create_shard_with_no_placement(self):
        md = ShardMetadata([0], [10])
        shard = Shard(smith.zeros(10), md)
...
class TestShardedTensorSubGroupInit(TestCase):
    ...
    def test_sub_process_group_sharded_tensor_init(self):
        ...
        ShardedTensor._init_from_local_shards(
            [
                Shard(
                    tensor=smith.tensor([1, 2, 3], device="meta"),
