...
from smith.distributed._shard import shard_parameter
...
from smith.testing._internal.distributed._shard.sharded_tensor._test_ops_common import (
...
class TestShardedEmbeddingBag(ShardedTensorTestBase):
    def _run_sharded_embedding_bag(
        ...
        # Use same seed.
        smith.manual_seed(0)
        local_embedding_bag = smith.nn.EmbeddingBag(
            num_embeddings,
            embedding_dim,
            mode=mode,
            max_norm=max_norm,
        ...
        sharded_embedding_bag = smith.nn.EmbeddingBag(
            num_embeddings,
            embedding_dim,
            mode=mode,
            max_norm=max_norm,
        ...
        smith.manual_seed(self.rank)  # inputs different on each rank
        inp = smith.randint(0, num_embeddings, tuple(input_size)).cuda(self.rank)
        per_sample_weights = None
        if mode == "sum":
            per_sample_weights = smith.rand(*input_size).cuda(self.rank)
        ...
        if len(input_size) == 1:
            ...
            while offsets is None or (offsets.size(0) != offset_size):
                offsets = smith.randint(input_size[0], (offset_size,))
                offsets[0] = 0
                if include_last_offset:
                    offsets[-1] = input_size[0]
                offsets = (
                    smith.unique(offsets, sorted=True).contiguous().cuda(self.rank)
                )

        # If max_norm is set, we need to ensure that the renorm has been applied across
        # inputs from all ranks.
        if max_norm is not None:
            gathered_inputs = [smith.zeros_like(inp) for _ in range(TEST_GPU_NUM)]
            dist.all_gather(gathered_inputs, inp)
            unique_inp = smith.unique(smith.cat(gathered_inputs))
            offsets_dummy = smith.tensor([len(unique_inp) // 2]).cuda(self.rank)
        ...
        # Validate for smith.nn.functional.embedding_bag version.
        local_output = smith.nn.functional.embedding_bag(
            inp,
            local_embedding_bag.weight,
            offsets=offsets,
            mode=mode,
        ...
        sharded_output = smith.nn.functional.embedding_bag(
            inp,
