...
import smith
from smith.distributed._shard import _shard_tensor, sharded_tensor
...
class TestShardingSpec(TestCase):
    ...
    def test_device_placement(self):
        ...
        DevicePlacementSpec(smith.device(0))
        DevicePlacementSpec(smith.device("cuda:0"))
    ...
    def test_chunked_sharding_spec(self):
        # Test valid specs.
        ChunkShardingSpec(0, [smith.device(0), smith.device(1)])
        ChunkShardingSpec(0, [smith.device("cuda:0"), smith.device("cuda:1")])
    ...
    def test_enumerable_sharding_spec(self):
        ...
        check_tensor(spec.shards, smith.rand(10, 5).size())
        ...
        check_tensor(spec.shards, smith.rand(6, 6).size())
        ...
        with self.assertRaisesRegex(ValueError, "Rank of tensor is.*but shards rank"):
            check_tensor(spec.shards, smith.rand(10, 10, 10).size())
        ...
        with self.assertRaisesRegex(ValueError, "exceeds tensor dim"):
            check_tensor(spec.shards, smith.rand(10, 3).size())
        ...
        with self.assertRaisesRegex(ValueError, "does not match tensor volume"):
            check_tensor(spec.shards, smith.rand(10, 10).size())
...
class GridShardingSpec(ShardingSpec):
    grid_size: int
    placements: list[Union[smith.distributed._remote_device, str]]

    def __post_init__(self):
        for i, remote_device in enumerate(self.placements):
            if not isinstance(remote_device, smith.distributed._remote_device):
                self.placements[i] = smith.distributed._remote_device(remote_device)

    def build_metadata(
        self,
        tensor_sizes: smith.Size,
    ...
    def shard(
        self, tensor: smith.Tensor, src_rank: int = 0, process_group=None
    ) -> ShardedTensor:
...
class TestCustomShardingSpec(ShardedTensorTestBase):
    def test_custom_sharding_spec(self):
        ...
        meta = grid_spec.build_metadata(smith.Size((8, 8)), tensor_properties)
        check_tensor(meta.shards_metadata, smith.Size((8, 8)))
    ...
    def test_custom_sharding_spec_tensor_ctor(self):
        ...
        self.assertEqual(local_shard, smith.ones(2, 2))
    ...
    def test_custom_sharding_spec_shard_tensor(self):
        ...
        with self.assertRaisesRegex(NotImplementedError, "not implemented"):
            _shard_tensor(smith.randn(8, 8), grid_spec)
