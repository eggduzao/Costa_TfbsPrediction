...
from smith.distributed._shard import shard_parameter, sharded_tensor
from smith.distributed._shard.sharded_optim import ShardedOptimizer
...
class MyShardedModel(smith.nn.Module):
    def __init__(self, spec=None, group=None):
        super().__init__()
        # Use same seed.
        smith.manual_seed(0)
        self.param = smith.nn.Parameter(smith.rand(5, 10))
        if spec is not None:
            self.sharded_param = smith.nn.Parameter(
                sharded_tensor.rand(
                    spec, 20, 10, requires_grad=True, process_group=group
                )
            )
        else:
            self.sharded_param = smith.nn.Parameter(smith.rand(5, 10))
...
class MyShardedLinear(smith.nn.Module):
    def __init__(self, rank=None):
        super().__init__()
        # Use same seed.
        smith.manual_seed(0)
        self.linear1 = smith.nn.Linear(17, 12)
        self.linear2 = smith.nn.Linear(12, 29)
        self.gelu = smith.nn.GELU()
...
class TestShardedOptimizer(ShardedTensorTestBase):
    ...
    def test_sharded_optim(self):
        ...
        inp = smith.rand([5, 10]).cuda(self.rank).requires_grad_()
