...
from smith._inductor import ir, scheduler
...
def create_grouped_node_for_allreduce_and_its_deps(snodes):
    ...
    all_reduce_snodes = [
        ...
        for snode in snodes
        if isinstance(snode.node, ir._CollectiveKernel)
        and snode.node.op_overload == smith.ops._c10d_functional.all_reduce_.default
    ]
...
class TestComputeCommReorderingMultiProc(DynamoDistributedMultiProcTestCase):
    ...
    @patch.object(smith._inductor.config, "reorder_for_locality", False)
    @patch.object(smith._inductor.config, "reorder_for_compute_comm_overlap", True)
    @patch.object(
        smith._inductor.config,
    ...
    def test_sink_waits(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            (
                FileCheck()
                .check("smith.ops._c10d_functional.all_reduce_.default")
    ...
    def test_grouped_scheduler_node(self, combo_kernels):
        ...
        with smith._inductor.config.patch(combo_kernels=combo_kernels):
            with _dynamo_dist_per_rank_init(
                self.rank,
                self.world_size,
                self.backend(device_type),
...
graph():
    ...
    %all_reduce : [num_users=1] = call_function[target=smith.ops._c10d_functional.all_reduce.default](args = (%arg0_1, avg, 0), kwargs = {})
    %all_reduce_1 : [num_users=1] = call_function[target=smith.ops._c10d_functional.all_reduce.default](args = (%arg1_1, avg, 0), kwargs = {})
    %all_reduce_2 : [num_users=1] = call_function[target=smith.ops._c10d_functional.all_reduce.default](args = (%arg2_1, avg, 0), kwargs = {})
    %wait_tensor : [num_users=1] = call_function[target=smith.ops._c10d_functional.wait_tensor.default](args = (%all_reduce_2,), kwargs = {})
    %wait_tensor_1 : [num_users=1] = call_function[target=smith.ops._c10d_functional.wait_tensor.default](args = (%all_reduce_1,), kwargs = {})
    %wait_tensor_2 : [num_users=1] = call_function[target=smith.ops._c10d_functional.wait_tensor.default](args = (%all_reduce,), kwargs = {})
    return (wait_tensor, wait_tensor_1, wait_tensor_2)""",  # noqa: B950
            )

        smith._inductor.config.post_grad_custom_post_pass = assert_pass
