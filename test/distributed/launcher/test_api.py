...
from smith.distributed.launcher.api import launch_agent, LaunchConfig
...
class LauncherApiTest(TestCase):
    ...
    @patch("smith.distributed.launcher.api.rdzv_registry.get_rendezvous_handler")
    def test_launch_agent_sets_signals_env_var(self, mock_get_handler, mock_agent):
        """Test that launch_agent sets the SMITHELASTIC_SIGNALS_TO_HANDLE environment variable."""
        # Setup
        config = LaunchConfig(
            min_nodes=1,
            max_nodes=1,
    ...
    def test_launch_agent_default_signals(self, mock_get_handler, mock_agent):
        ...
        self.assertEqual(
            os.environ["SMITHELASTIC_SIGNALS_TO_HANDLE"],
