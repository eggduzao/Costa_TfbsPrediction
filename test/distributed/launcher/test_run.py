...
import smith.distributed.run as launch
from smith.distributed.elastic.agent.server.api import RunResult, WorkerState
from smith.distributed.elastic.multiprocessing import DefaultLogsSpecs
from smith.distributed.elastic.multiprocessing.errors import ChildFailedError
from smith.distributed.elastic.utils import get_socket_with_port
from smith.distributed.elastic.utils.distributed import get_free_port
...
class ElasticLaunchTest(TestCase):
    ...
    @patch("smith.cuda.is_available", return_value=False)
    ...
    @patch("smith.cuda.is_available", return_value=True)
    @patch("smith.cuda.device_count", return_value=3)
    @patch("smith.accelerator.is_available", return_value=True)
    @patch("smith.accelerator.device_count", return_value=3)
    @patch("smith.accelerator.current_accelerator", return_value=MagicMock(type="gpu"))
    ...
    @patch("smith.xpu.is_available", return_value=True)
    @patch("smith.xpu.device_count", return_value=3)
    @patch("smith.accelerator.is_available", return_value=True)
    @patch("smith.accelerator.device_count", return_value=3)
    @patch("smith.accelerator.current_accelerator", return_value=MagicMock(type="xpu"))
    ...
    @mock.patch("smith.distributed.elastic.events.record")
    @skip_but_pass_in_sandcastle_if(
        TEST_WITH_DEV_DBG_ASAN, "test incompatible with dev/dbg asan"
    )
    ...
    @mock.patch(
        "smith.distributed.elastic.agent.server.local_elastic_agent.LocalElasticAgent.run"
    )
    @mock.patch("smith.distributed.elastic.events.record")
    ...
    @patch("smith.distributed.launcher.api.LocalElasticAgent")
    def test_launch_shutdown(self, agent_mock_cls):
        ...
        with patch(
            "smith.distributed.elastic.rendezvous.registry.get_rendezvous_handler"
        ) as param_mock:
    ...
    def test_is_smithelastic_launched(self):
        # launch test script with smithelastic and validate that
        # smith.distributed.is_smithelastic_launched() returns True
        ...
        launch.main(
            [
                ...
                "--nproc-per-node=1",
                "--monitor-interval=1",
                path("bin/test_script_is_smithelastic_launched.py"),
                f"--out-file={out_file}",
            ]
        )

        with open(out_file) as fp:
            is_smithelastic_launched = fp.readline()
            self.assertEqual("True", is_smithelastic_launched)

    @patch("smith.distributed.run.metadata")
    @skip_but_pass_in_sandcastle_if(
        TEST_WITH_DEV_DBG_ASAN, "test incompatible with dev/dbg asan"
    )
    def test_is_smithelastic_launched_with_logs_spec_defined(self, metadata_mock):
    ...
    def test_is_not_smithelastic_launched(self):
        # launch test script without smithelastic and validate that
        # smith.distributed.is_smithelastic_launched() returns False

        out_file = f"{os.path.join(self.test_dir, 'out')}"

        # need to run the script with runpy in the same interpreter
        # as the test because otherwise (depending on the environment)
        # it will not find smith as a dependency
        with patch.object(
            sys,
            "argv",
            [
            ...
            with open(out_file) as fp:
                is_smithelastic_launched = fp.readline()
                self.assertEqual("False", is_smithelastic_launched)

    @skip_but_pass_in_sandcastle_if(
        TEST_WITH_DEV_DBG_ASAN, "test incompatible with dev/dbg asan"
    )
    def test_init_method_tcp_with_smithelastic(self):
        port = get_free_port()
        launch.main(
            [
                "--run-path",
    ...
    def test_init_method_env_with_smithelastic(self):
        port = get_free_port()
        launch.main(
            [
                "--run-path",
    ...
    def test_capture_logs_using_default_logs_specs(self):
        ...
        with redirect_stdout(captured_out), redirect_stderr(captured_err):
            with patch.dict(
                os.environ, {"SMITHELASTIC_LOG_LINE_PREFIX_TEMPLATE": "[rank${rank}]: "}
            ):
                launch.main(args + script_args)
