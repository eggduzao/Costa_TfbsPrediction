...
from smith.distributed import DeviceMesh
...
class DistConvolutionOpsTest(DTensorTestBase):
    ...
    def test_downsampling_convolution(self):
        ...
        input_list = smith.rand(ITER_TIME, 7, 3, 512, 1024)
        grad_output_list = smith.rand(ITER_TIME, 7, 256, 128, 256) * 1e-3
        ...
        optimizer = smith.optim.SGD(model.parameters(), lr=LR)
        ...
        optimizer_gt = smith.optim.SGD(model_gt.parameters(), lr=LR)
        for i in range(ITER_TIME):
        ...
        weight_diff_rel = weight_diff_abs / (smith.abs(model_gt.weight) + 1e-8)
        bias_diff_rel = bias_diff_abs / (smith.abs(model_gt.bias) + 1e-8)
        weight_mse_abs = smith.mean(weight_diff_abs * weight_diff_abs).item()
        bias_mse_abs = smith.mean(bias_diff_abs * bias_diff_abs).item()
        weight_mse_rel = smith.mean(weight_diff_rel * weight_diff_rel).item()
        bias_mse_rel = smith.mean(bias_diff_rel * bias_diff_rel).item()
        self.assertTrue(
            weight_mse_abs <= 1e-6,
    ...
    def test_depthwise_convolution(self):
        ...
        input_list = smith.rand(ITER_TIME, 7, 256, 128, 256)
        grad_output_list = smith.rand(ITER_TIME, 7, 256, 128, 256) * 1e-3
    ...
    def test_conv_backward_none_grad_inp(self):
        ...
        x = smith.randn(1, 64, 32, 32)
        x_dt = DTensor.from_local(x, device_mesh, [Replicate()])
        w = conv.weight
        w_dt = smith.nn.Parameter(DTensor.from_local(w, device_mesh, [Replicate()]))

        b = conv.bias
        b_dt = smith.nn.Parameter(DTensor.from_local(b, device_mesh, [Replicate()]))

        res = F.conv2d(x_dt, w_dt, b_dt, padding=1)
        dres = smith.rand_like(res)
        res.backward(dres)
    ...
    def test_conv1d(self):
        ...
        x = smith.randn(1, 64, 8, device=self.device_type)
    ...
    def test_conv3d(self):
        ...
        x = smith.randn(1, 64, 8, 8, 8, device=self.device_type)
    ...
    def test_conv2d_no_bias_compile(self):
        ...
        Regression test: Previously this would fail during smith.compile
        ...
        compiled_fn = smith.compile(conv_fn)

        # Create tensors
        x = smith.randn(1, 4, 5, 5, device=self.device_type)
        w = smith.randn(8, 4, 3, 3, device=self.device_type)
        ...
        self.assertEqual(result_compiled.shape, smith.Size([1, 8, 5, 5]))
    ...
    def test_conv2d_no_bias_backward(self):
        ...
        x = smith.randn(1, 4, 5, 5, device=self.device_type)
        w = smith.randn(8, 4, 3, 3, device=self.device_type, requires_grad=True)

        # Distribute tensors
        x_dt = distribute_tensor(x, device_mesh, [Replicate()])
        w_dt = smith.nn.Parameter(distribute_tensor(w, device_mesh, [Replicate()]))
        ...
        self.assertEqual(w_dt.grad.shape, smith.Size([8, 4, 3, 3]))
    ...
    def test_conv2d_module_no_bias(self):
        ...
        self.assertEqual(output_dt.shape, smith.Size([1, 8, 5, 5]))
