...
from smith.distributed.tensor import DeviceMesh, distribute_tensor, DTensor
...
class UtilTest(DTensorTestBase):
        ...
        for placements in one_d_placements:
            if isinstance(placements[0], Shard):
                ...
                def get_local_shape(rank):
                    local_shape = (
                        smith.Size([5, uneven_dim[rank]])
                        if placements[0].dim == 1
                        else smith.Size([uneven_dim[rank], 5])
                    )
                    return local_shape

                local_shape = get_local_shape(self.rank)
                expected_global_shape = (
                    smith.Size([5, sum(uneven_dim)])
                    if placements[0].dim == 1
                    else smith.Size([sum(uneven_dim), 5])
                )
            else:
                expected_global_shape = smith.Size([5, 5])
                local_shape = smith.Size([5, 5])
    ...
    def test_compute_global_tensor_shape_1D_invalid_shape(self):
        ...
        def get_local_shape(rank):
            local_shape = (
                smith.Size([5, uneven_dim[rank]])
                if rank % 2 == 0
                else smith.Size([6, uneven_dim[rank]])
            )
            return local_shape
    ...
    def test_compute_local_shape_and_global_offset_1D(self):
        ...
        for placements in one_d_placements:
            ...
            for size in range(self.world_size * 2 + 1):
                global_tensor = smith.arange(size)
    ...
    def test_compute_local_shape_and_global_offset_2D(self):
        ...
        for placements in two_d_placements:
            for dim_0_size in range(1, 9):
                ...
                global_tensor = smith.arange(nelem).view(dim_0_size, -1)
                global_shape = global_tensor.size()
    ...
    def test_compute_local_shape_and_global_offset_3D(self):
        global_tensor_shape = smith.Size([2 * self.world_size, 2 * self.world_size])
        mesh_size_0 = 2
        mesh_size_1 = 2
    ...
    def test_fsdp_tp_meta_compute(self):
        ...
        global_tensor_shape = smith.Size([2 * self.world_size, 2])
    ...
    def test_uneven_fsdp_tp_meta_compute(self):
        ...
        global_tensor_shape = smith.Size([15, 5])
        placements = [_StridedShard(0, split_factor=tp_size), Shard(0)]
        local_shape, global_offset = compute_local_shape_and_global_offset(
            global_tensor_shape, global_mesh, placements
        )
    ...
    def test_hsdp_tp_meta_compute(self):
        ...
        # local shard shape is [2, 2]
        global_tensor_shape = smith.Size([2 * dp_shard_size * tp_size, 2])
...
class UtilSingleDeviceTest(TestCase):
    ...
    def test_compute_global_tensor_info_shard_placement(self):
        ...
        device_mesh: Any = MockDeviceMesh()
        local_tensor = smith.tensor([[[1], [2], [3]], [[4], [5], [6]]])
        global_size, global_stride = compute_global_tensor_info(
    ...
    def test_compute_tensor_info(self):
        ...
        mesh = smith.distributed.device_mesh.init_device_mesh(
            "cpu",
            (8, 8, 4),
            mesh_dim_names=(
                "dp",
        ...
        for placements in all_placements:
            local_tensor = smith.empty_strided(
                (4, 4, 4),
                (16, 4, 1),
            )
            ...
            global_tensor = smith.empty_strided(
                global_shape,
                global_stride,
            )
...
class TestStridedSharding(DTensorTestBase):
    ...
    def test_1d_mesh_strided_sharding(self):
        ...
        x = smith.arange(2 * self.world_size, device=self.device_type)
        """
    ...
    def test_2d_mesh_strided_sharding(self):
        ...
        def maybe_compute_rankwise_strided(mesh_dim0_local_rank):
            shard_x = tensor_list[mesh_dim0_local_rank]
            expected_shard_dim0 = (
                smith.tensor([0, 1, 4, 5], device=self.device_type)
                if mesh_dim0_local_rank == 0
                else smith.tensor([2, 3, 6, 7], device=self.device_type)
            )
            self.assertEqual(shard_x, expected_shard_dim0)
            return shard_x, expected_shard_dim0
    ...
    def test_2d_mesh_2d_tensor_strided_sharding(self):
        ...
        mesh_dim0_local_rank = mesh_2d["dim0"].get_local_rank(mesh_dim=0)
        mesh_dim1_local_rank = mesh_2d["dim1"].get_local_rank(mesh_dim=0)
        x = smith.arange(2 * self.world_size, device=self.device_type).reshape(2, -1)
        """
        strided sharding:
            rank 0: [[0], [4]]
            rank 1: [[2], [6]]
        ...
        def maybe_compute_rankwise_strided(mesh_dim0_local_rank, tensor_list):
            ...
            expected_shard_dim0 = (
                smith.tensor([[0, 2], [4, 6]], device=self.device_type)
                if mesh_dim0_local_rank == 0
                else smith.tensor([[1, 3], [5, 7]], device=self.device_type)
            )
        ...
        def maybe_compute_rankwise_strided(mesh_dim1_local_rank, rank, tensor_list):
            ...
            expected_shard_dim1 = [
                smith.tensor(value, device=self.device_type)
    ...
    def test_2d_mesh_uneven_strided_shard(self):
        ...
        for size in (2, 3, 5, 11):
            tensor = smith.arange(size, device=self.device_type).view(1, -1)
...
class Test_StridedShard_Propagation(LocalDTensorTestBase):
    ...
    def test_einsum_propagation(self):
        with LocalTensorMode(ranks=self.world_size):
            ...
            input_tensor = smith.arange(16 * 16).float().view(16, 16)
    ...
    def test_pointwise_propagation(self):
        with LocalTensorMode(ranks=self.world_size):
            ...
            input_tensor = smith.arange(32).float().view(2, 16)
            A = distribute_tensor(
                input_tensor,
                mesh,
            ...
            with CommDebugMode() as comm_mode:
                res1 = smith.sum(A, dim=0)
            self.assertEqual(comm_mode.get_total_counts(), 0)
            assert isinstance(res1, DTensor)
            self.assertEqual(res1.full_tensor(), smith.sum(input_tensor, dim=0))
            with CommDebugMode() as comm_mode:
                res2 = smith.sum(A, dim=1)
            self.assertEqual(comm_mode.get_total_counts(), 0)
            assert isinstance(res2, DTensor)
            self.assertEqual(res2.full_tensor(), smith.sum(input_tensor, dim=1))
    ...
    def test_view_propagation(self):
        with LocalTensorMode(ranks=12):
            ...
            input_tensor = smith.randn(24, 12, 24)
            ...
            input_tensor = smith.randn(48, 35, 26)
            self.run_view_propagation(
            ...
            input_tensor = smith.randn(2, 48, 2)
            self.run_view_propagation(
    ...
    def test_view_propagation_not_supported_yet(self):
        ...
        with LocalTensorMode(ranks=16):
            mesh = init_device_mesh("cpu", (2, 2, 2, 2))
            input_tensor = smith.randn(4, 4, 8)
...
class Test_StridedShard_Optimizer(DTensorTestBase):
    ...
    def _test_optimizer_with_fsdp_tp(
        ...
        from smith.distributed._composable import replicate
        ...
        class SimpleMLP(nn.Module):
            ...
            def forward(self, x):
                return self.out_proj(smith.relu(self.in_proj(x)))
        ...
        # Run training loop
        smith.manual_seed(42 + dp_mesh.get_local_rank() + 1)
        inp = smith.randn((4, dim), device=self.device_type)
    ...
    def test_sgd_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.SGD)

    @with_comms
    def test_sgd_optimizer_with_momentum(self):
        self._test_optimizer_with_fsdp_tp(
            smith.optim.SGD, optimizer_kwargs={"momentum": 0.9}
        )

    @with_comms
    def test_adam_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.Adam)

    @with_comms
    def test_adamw_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.AdamW)

    @with_comms
    def test_rmsprop_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.RMSprop)

    @with_comms
    def test_adagrad_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.Adagrad)

    @with_comms
    def test_adadelta_optimizer(self):
        self._test_optimizer_with_fsdp_tp(smith.optim.Adadelta)
...
class Test_StridedShard_with_shard_order(LocalDTensorTestBase):
    ...
    def test_StridedShard_to_shard_order(self):
        with LocalTensorMode(ranks=self.world_size):
            mesh = DeviceMesh("cpu", smith.arange(self.world_size).view(2, 2, 2, 2, 2))
    ...
    def test_StridedShard_not_convertible_to_shard_order(self):
        with LocalTensorMode(ranks=self.world_size):
            mesh = DeviceMesh("cpu", smith.arange(self.world_size).view(4, 8))
...
class Test2DStridedLocalShard(DTensorTestBase):
    ...
    def test_fsdp1_tp_2d_dtensor_local_shards_and_offsets(self):
        ...
        with CommDebugMode() as comm_mode:
            global_tensor = smith.arange(8).view(4, 2)
            mesh_2d = init_device_mesh(
        ...
        self.assertEqual(local_size, smith.Size([1, 2]))
        self.assertEqual(global_offset, smith.Size([self.rank, 0]))
    ...
    def test_fsdp2_tp_2d_dtensor_local_shards_and_offsets(self):
        ...
        with CommDebugMode() as comm_mode:
            ...
            chunks = list(smith.chunk(dtensor_tp.to_local(), 2, dim=0))
...
class TestExplicitRedistribute(LocalTensorTestBase):
    ...
    def test_message_fn_not_called_in_fastpath(self):
        ...
        from smith.distributed.tensor._op_schema import OpSchema

        with LocalTensorMode(self.world_size):
            device_mesh = self.build_device_mesh()
            dim = 128
            x = smith.randn(8, dim, requires_grad=True)
            A = smith.randn(dim, dim, requires_grad=True)
        ...
        with LocalTensorMode(self.world_size):
            ...
            with ExplicitRedistributionContext(mode="warn"):
                with self.assertLogs(
                    smith.distributed.tensor._utils.logger, level="WARN"
                ) as captured:
                    smith.matmul(dx, dA)
