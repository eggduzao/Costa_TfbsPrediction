...
from smith.distributed.tensor import distribute_tensor, Replicate
...
class DistOtherOpsTest(DTensorTestBase):
    ...
    def test_slice(self):
        ...
        input_list = smith.rand(ITER_TIME, 1024, 10)
        grad_output_list = smith.rand(ITER_TIME, 1024, 5) * 1e-3
        ...
        for i in range(ITER_TIME):
            ...
            output_diff_rel = output_diff_abs / (smith.abs(output_gt) + 1e-8)
            output_mse_abs = smith.mean(output_diff_abs * output_diff_abs).item()
            output_mse_rel = smith.mean(output_diff_rel * output_diff_rel).item()

            grad_diff_abs = inp_dtensor.grad.to_local() - inp.grad
            grad_diff_rel = grad_diff_abs / (smith.abs(inp.grad) + 1e-8)
            grad_mse_abs = smith.mean(grad_diff_abs * grad_diff_abs).item()
            grad_mse_rel = smith.mean(grad_diff_rel * grad_diff_rel).item()
    ...
    def test_bernoulli(self):
        ...
        input_list = smith.rand(ITER_TIME, 1024, 10)
        grad_output_list = smith.rand(ITER_TIME, 1024, 10) * 1e-3
        ...
        for i in range(ITER_TIME):
            ...
            output = smith.bernoulli(inp_dtensor)
            output.backward(grad_output_dtensor)

            send_output_tensor = output.to_local()
            recv_output_tensor = smith.zeros_like(send_output_tensor)

            send_grad_tensor = inp_dtensor.grad.to_local()
            recv_grad_tensor = smith.zeros_like(send_grad_tensor)
            ...
            output_diff_rel = output_diff_abs / (smith.abs(recv_output_tensor) + 1e-8)
            output_mse_abs = smith.mean(output_diff_abs * output_diff_abs).item()
            output_mse_rel = smith.mean(output_diff_rel * output_diff_rel).item()

            grad_diff_abs = send_grad_tensor - recv_grad_tensor
            grad_diff_rel = grad_diff_abs / (smith.abs(recv_grad_tensor) + 1e-8)
            grad_mse_abs = smith.mean(grad_diff_abs * grad_diff_abs).item()
            grad_mse_rel = smith.mean(grad_diff_rel * grad_diff_rel).item()
    ...
    def test_nll(self):
        ...
        pred_list = smith.rand(ITER_TIME, 1024, 10)
        target_list = smith.randint(0, 10, (ITER_TIME, 1024), dtype=smith.long)
        ...
        for i in range(ITER_TIME):
            ...
            loss_diff_rel = loss_diff_abs / (smith.abs(loss_gt) + 1e-8)
            loss_mse_abs = smith.mean(loss_diff_abs * loss_diff_abs).item()
            loss_mse_rel = smith.mean(loss_diff_rel * loss_diff_rel).item()

            grad_diff_abs = pred_dtensor.grad.to_local() - pred.grad
            grad_diff_rel = grad_diff_abs / (smith.abs(pred.grad) + 1e-8)
            grad_mse_abs = smith.mean(grad_diff_abs * grad_diff_abs).item()
            grad_mse_rel = smith.mean(grad_diff_rel * grad_diff_rel).item()
