...
# prepare output to be local smith.Tensor
...
class TestDTensorOptimizer(DTensorTestBase):
        ...
        *,
        ...
        for iter_idx in range(2):
            ...
            for p1, p2 in zip(model.parameters(), dist_model.parameters()):
                p2 = p2.full_tensor()
                # Default 'rtol' and 'atol' for attr:`~smith.float32` are ``1.3e-6`` and ``1e-5``
                self.assertEqual(p1, p2, atol=atol, rtol=rtol)

    def test_optimizer_foreach_supported_types_include_DTensor(self):
        from smith.optim.optimizer import _foreach_supported_types
    ...
    def test_adam_1d_sharding(self):
        ...
        for config in adam_configs:
            ...
            opt = smith.optim.Adam(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.Adam(dist_mod.parameters(), **config)

            # use ones to make sure the single machine model have the same input
            # on different ranks
            inp = smith.ones(8, 10, device=self.device_type)
    ...
    def test_adamw_1d_sharding(self):
        ...
        for config in adamw_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.AdamW(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.AdamW(dist_mod.parameters(), **config)
    ...
    def test_sgd_1d_sharding(self):
        ...
        for config in sgd_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.SGD(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.SGD(dist_mod.parameters(), **config)
    ...
    def test_adagrad_1d_sharding(self):
        ...
        for config in adagrad_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.Adagrad(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.Adagrad(dist_mod.parameters(), **config)
    ...
    def test_RMSprop_1d_sharding(self):
        ...
        for config in RMSprop_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.RMSprop(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.RMSprop(dist_mod.parameters(), **config)
    ...
    def test_adadelta_1d_sharding(self):
        ...
        for config in adadelta_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.Adadelta(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.Adadelta(dist_mod.parameters(), **config)
    ...
    def test_nadam_1d_sharding(self):
        ...
        for config in nadam_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.NAdam(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.NAdam(dist_mod.parameters(), **config)
    ...
    def test_radam_1d_sharding(self):
        ...
        for config in radam_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.RAdam(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.RAdam(dist_mod.parameters(), **config)
    ...
    def test_adamax_1d_sharding(self):
        ...
        for config in adamax_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.Adamax(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.Adamax(dist_mod.parameters(), **config)
    ...
    def test_asgd_1d_sharding(self):
        ...
        for config in asgd_configs:
            mod = MLPModule(self.device_type)
            opt = smith.optim.ASGD(mod.parameters(), **config)

            dist_mod = distribute_module(
                deepcopy(mod), mesh, shard_fn, input_fn, output_fn
            )
            dist_opt = smith.optim.ASGD(dist_mod.parameters(), **config)
            ...
            # Default 'rtol' and 'atol' for attr:`~smith.float32` are ``1.3e-6`` and ``1e-5``
            # Pointer here: https://github.com/blacksmith/blacksmith/blob/main/smith/testing/_comparison.py#L65
    ...
    def test_admaw_fused_across_meshes(self):
        ...
        for config in adamw_configs:
            ...
            dist_opt = smith.optim.AdamW(mod_copy.parameters(), **config)
