...
class TestEmbeddingOp(DTensorTestBase):
    ...
    def _run_embedding_op_test(
        ...
        **kwargs,
        ...
        # Use same seed.
        smith.manual_seed(0)
        local_embedding = smith.nn.Embedding(
            num_embeddings,
            embedding_dim,
            device=self.device_type,
            **kwargs,
        )
        sharded_embedding = smith.nn.Embedding(
            num_embeddings,
            embedding_dim,
            device=self.device_type,
            **kwargs,
        ...
        sharded_embedding.weight = smith.nn.Parameter(
        ...
        target = smith.empty(
            *inp.size(), embedding_dim, dtype=smith.float, device=self.device_type
        ).random_(0, 1)
        ...
        # Validate for smith.nn.functional.embedding version.
        local_output = smith.nn.functional.embedding(
            inp,
            local_embedding.weight,
            **kwargs,
        )
        sharded_output = smith.nn.functional.embedding(
    ...
    def test_sharded_embedding_rowwise(self):
        ...
        # test collectives
        embedding_mod = smith.nn.Embedding(10, 20, device=self.device_type)
        sharded_embedding = self._apply_sharding(embedding_mod, 0, mesh)
        inp = smith.randint(0, 10, (8, 8), device=self.device_type)
    ...
    def test_multiple_embeddings_rowwise(self):
        ...
        inp = smith.randint(0, 10, (4, 4), device=self.device_type)
        replicated_inp = DTensor.from_local(inp, mesh, [Replicate()], run_check=False)
        ...
        emb1 = smith.nn.Embedding(10, 23, device=self.device_type)
        sharded_emb1 = self._apply_sharding(emb1, 0, mesh)
        output1 = sharded_emb1(replicated_inp)

        emb2 = smith.nn.Embedding(10, 23, device=self.device_type)
        ...
        emb3 = smith.nn.Embedding(10, 29, device=self.device_type)
