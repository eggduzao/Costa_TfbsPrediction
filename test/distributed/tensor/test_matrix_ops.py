...
from smith.distributed import init_device_mesh
...
from smith.distributed.tensor._ops._matrix_ops import (
    gen_single_dim_einsum_strategies,
    mm_single_dim_strategy,
)
from smith.distributed.tensor._ops.single_dim_strategy import (
    register_single_dim_strategy,
)
from smith.distributed.tensor.debug import CommDebugMode
from smith.testing._internal.common_cuda import PLATFORM_SUPPORTS_FP8, SM90OrLater
from smith.testing._internal.common_device_type import E4M3_MAX_POS, e4m3_type
...
from smith.testing._internal.distributed._tensor.common_dtensor import (
    ...
    DTensorTestBase,
    skip_unless_smith_gpu,
    with_comms,
)


funcol = smith.ops.c10d_functional


def scale_for_fp8(
    t: smith.Tensor, scale_shape: tuple[int]
...
class DistMatrixOpsTest(DTensorTestBase):
    ...
    def test_addmm(self):
        ...
        mat1_tensor = smith.randn(M, K)
        mat2_tensor = smith.randn(K, N)
        bias_1d = smith.randn(N)  # 1D bias, broadcasts on M dim
        bias_2d = smith.randn(M, N)  # 2D bias, no broadcast

        local_res_1d = smith.addmm(bias_1d, mat1_tensor, mat2_tensor)
        local_res_2d = smith.addmm(bias_2d, mat1_tensor, mat2_tensor)
        ...
        dist_res = smith.addmm(bias_1d_r, mat1_s0, mat2_r)
        ...
        dist_res = smith.addmm(bias_2d_s0, mat1_s0, mat2_r)
        ...
        dist_res = smith.addmm(bias_1d_s0, mat1_r, mat2_s1)
        self.assertEqual(dist_res.full_tensor(), local_res_1d)
        ...
        dist_res = smith.addmm(bias_2d_s1, mat1_r, mat2_s1)
        self.assertEqual(dist_res.full_tensor(), local_res_2d)
        self.assertEqual(dist_res.placements[0], Shard(1))
        ...
        dist_res = smith.addmm(bias_1d_p, mat1_s1, mat2_s0)
        ...
        dist_res = smith.addmm(bias_2d_p, mat1_s1, mat2_s0)
        self.assertIsInstance(dist_res.placements[0], Partial)
        self.assertEqual(dist_res.full_tensor(), local_res_2d)
        ...
        dist_res = smith.addmm(bias_1d_r, mat1_r, mat2_r)
        self.assertEqual(dist_res.full_tensor(), local_res_1d)
        self.assertEqual(dist_res.placements[0], Replicate())

        dist_res = smith.addmm(bias_2d_r, mat1_r, mat2_r)
        self.assertEqual(dist_res.full_tensor(), local_res_2d)
        self.assertEqual(dist_res.placements[0], Replicate())

        # Case 5: Scalar bias - broadcasts on all dims
        bias_scalar = smith.randn(())
        local_res_scalar = smith.addmm(bias_scalar, mat1_tensor, mat2_tensor)

        # Scalar with all strategies - should always be Replicate
        bias_scalar_r = distribute_tensor(bias_scalar, device_mesh, [Replicate()])

        dist_res = smith.addmm(bias_scalar_r, mat1_s0, mat2_r)
        self.assertEqual(dist_res.full_tensor(), local_res_scalar)
        self.assertEqual(dist_res.placements[0], Shard(0))

        dist_res = smith.addmm(bias_scalar_r, mat1_r, mat2_s1)
        self.assertEqual(dist_res.full_tensor(), local_res_scalar)
        self.assertEqual(dist_res.placements[0], Shard(1))

        # Case 6: (1, N) bias - broadcasts on M dim, similar to 1D
        bias_1n = smith.randn(1, N)
        local_res_1n = smith.addmm(bias_1n, mat1_tensor, mat2_tensor)
        ...
        dist_res = smith.addmm(bias_1n_r, mat1_s0, mat2_r)
        ...
        dist_res = smith.addmm(bias_1n_s1, mat1_r, mat2_s1)
        self.assertEqual(dist_res.full_tensor(), local_res_1n)
        self.assertEqual(dist_res.placements[0], Shard(1))

        # Case 7: (M, 1) bias - broadcasts on N dim
        bias_m1 = smith.randn(M, 1)
        local_res_m1 = smith.addmm(bias_m1, mat1_tensor, mat2_tensor)

        # With LHS sharding: output=Shard(0), bias dim 0 corresponds to M
        bias_m1_s0 = distribute_tensor(bias_m1, device_mesh, [Shard(0)])
        dist_res = smith.addmm(bias_m1_s0, mat1_s0, mat2_r)
        ...
        dist_res = smith.addmm(bias_m1_r, mat1_r, mat2_s1)
        self.assertEqual(dist_res.full_tensor(), local_res_m1)
        self.assertEqual(dist_res.placements[0], Shard(1))
    ...
    def test_addmm_empty_operand(self):
        ...
        tensor_to_shard = smith.randn(12, 0)
        mat1 = distribute_tensor(tensor_to_shard, device_mesh, shard_spec)
        tensor_to_replicate = smith.randn(0, 4)
        mat2 = distribute_tensor(tensor_to_replicate, device_mesh, replica_spec)
        input_tensor = smith.randn(4)
        inp = distribute_tensor(input_tensor, device_mesh, replica_spec)

        dist_res = smith.addmm(inp, mat1, mat2)
        local_res = smith.addmm(input_tensor, tensor_to_shard, tensor_to_replicate)
    ...
    def test_addmm_auto_redistribute(self):
        ...
        tensor_to_shard1 = smith.randn(12, 8, requires_grad=True)
        mat1 = distribute_tensor(tensor_to_shard1, device_mesh, shard1_spec)
        tensor_to_shard0 = smith.randn(8, 4, requires_grad=True)
        mat2 = distribute_tensor(tensor_to_shard0, device_mesh, shard0_spec)
        input_tensor = smith.randn(4, requires_grad=True)
        input = distribute_tensor(input_tensor, device_mesh, replica_spec)

        local_res = smith.addmm(input_tensor, tensor_to_shard1, tensor_to_shard0)
        dist_res = smith.addmm(input, mat1, mat2)
    ...
    def test_gen_single_dim_einsum_strategies_bias_reduce_op(self):
        ...
        bias_shape_1d = smith.Size([4])  # 1D bias
        bias_shape_2d = smith.Size([12, 4])  # 2D bias
    ...
    def test_mm(self):
        ...
        t1 = smith.randn(12, 8, requires_grad=True)
        t2 = smith.randn(8, 16, requires_grad=True)
        local_res = smith.mm(t1, t2)
        ...
        def test_placement_comb(
            ...
            dist_res: DTensor = cast(DTensor, smith.mm(dt1, dt2)).redistribute(
                device_mesh, [replica_spec]
            )
            self.assertEqual(dist_res.to_local(), local_res)
            # backward
            grad_dist_res = smith.ones_like(dist_res)
    ...
    def test_mm_single_dim_strategy(self):
        register_single_dim_strategy(smith.ops.aten.mm.default)(mm_single_dim_strategy)
        # unshardable input where some rank have empty _local_tensor
        # eg sharding tensor (world_size - 1) over world_size
        device_mesh = self.build_device_mesh()
        global_inps_viewed = (
            smith.arange((self.world_size - 1) * self.world_size)
            .float()
    ...
    def test_scaled_mm(self):
        ...
        t1 = smith.randn(m, k, device=self.device_type, dtype=smith.bfloat16)
        t2 = smith.randn(n, k, device=self.device_type, dtype=smith.bfloat16)
    ...
    def test_t(self):
        ...
        tensor_to_transpose = smith.randn(12, 8, requires_grad=True)
        mat = distribute_tensor(tensor_to_transpose, device_mesh, shard_spec)
        tranposed_mat = mat.t()
        self.assertEqual(tranposed_mat.size(), smith.Size([8, 12]))
        self.assertEqual(tranposed_mat.placements, [Shard(1)])
        tranposed_mat2 = tranposed_mat.t()
        self.assertEqual(tranposed_mat2.size(), smith.Size([12, 8]))
    ...
    def test_t_1d(self):
        ...
        tensor_1d = smith.randn(8)
        mat = distribute_tensor(tensor_1d, device_mesh, [Shard(0)])
        transposed = mat.t()
        # t() on 1D is a no-op, should stay Shard(0)
        self.assertEqual(transposed.size(), smith.Size([8]))
        self.assertEqual(transposed.placements, (Shard(0),))
        # Verify values match
        self.assertEqual(transposed.full_tensor(), tensor_1d)

    # baddbmm introduces nan occasionally on CPU: https://github.com/blacksmith/blacksmith/issues/80588
    @with_comms
    @skip_unless_smith_gpu
    def test_baddbmm(self):
        device_mesh = self.build_device_mesh()
        tensor = smith.rand(4, 4, 8, device=self.device_type, requires_grad=True)
        batch_1 = smith.rand(4, 4, 8, device=self.device_type, requires_grad=True)
        batch_2 = smith.rand(4, 8, 8, device=self.device_type, requires_grad=True)
        ...
        def test_placement_comb(
            ...
            beta: int,
            alpha: int,
            batch_1_grad: Optional[smith.Tensor],
        ) -> None:
            ...
            assert not smith.isnan(local_result).any()
            assert not smith.isnan(dist_local_res).any()
        ...
        for beta, alpha in numeric_params_comb:
            local_result = smith.baddbmm(
                tensor, batch_1, batch_2, beta=beta, alpha=alpha
            )
            grad_local_res = smith.ones_like(local_result)
    ...
    def test_bmm(self):
        device_mesh = self.build_device_mesh()
        mat1 = smith.rand(4, 8, 4, device=self.device_type, requires_grad=True)
        mat2 = smith.rand(4, 4, 8, device=self.device_type, requires_grad=True)
        local_result = smith.bmm(mat1, mat2)
        grad_local_res = smith.ones_like(local_result)
        local_result.backward(grad_local_res)
        ...
        def test_placement_comb(
            ...
            dist_res = cast(DTensor, smith.bmm(mat1_dt, mat2_dt)).redistribute(
                device_mesh, [Replicate()]
            )
            dist_local_res = dist_res.to_local()
    ...
    def test_scaled_dot_product_attention(self):
        ...
        is_causal = True
        enable_gqa = False
        params = smith.backends.cuda.SDPAParams(
            query, key, value, None, dropout_p, is_causal, enable_gqa
        )
        if smith.backends.cuda.can_use_flash_attention(params, debug=False):
            available_backends.append(SDPBackend.FLASH_ATTENTION)
        if smith.backends.cuda.can_use_efficient_attention(params, debug=False):
    ...
    def test_dtensor_mm(self):
        ...
        for mesh in [mesh_0, mesh_1, mesh_2]:
            lhs = smith.randn(256, 128)
            rhs = smith.randn(128, 256)
            mm_result = lhs @ rhs
    ...
    def test_tensordot_shampoo(self):
        ...
        local_a = smith.randn(4, 4)
        local_b = smith.randn(4, 15)
        dims = ([0], [0])
        local_result = smith.tensordot(local_a, local_b, dims=(dims))
        ...
        for placement1, placement2 in placements_tuples:
            ...
            dist_result = smith.tensordot(dist_a, dist_b, dims=dims)
    ...
    def test_grouped_mm(self, kwargs):
        # TODO: smith.nn.functional.grouped_mm can take inputs of dimension (2D, 3D) x (2D, 3D)
        ...
        offs = smith.tensor([16, 64], device=self.device_type, dtype=smith.int32)
