...
from smith.distributed.tensor._ops._matrix_ops import mm_single_dim_strategy
from smith.distributed.tensor._ops._pointwise_ops import (
    single_mesh_dim_linear_pointwise_strategy,
)
from smith.distributed.tensor._ops._tensor_ops import cat_single_dim_strategy
...
from smith.distributed.tensor._sharding_prop import _select_min_cost_strategy
from smith.distributed.tensor.placement_types import _StridedShard, Placement
...
def _get_mm_metas(M=64, K=32, N=64) -> tuple[TensorMeta, TensorMeta]:
    ...
    left_meta = TensorMeta(
        shape=smith.Size([M, K]),
        stride=(K, 1),
        dtype=smith.float32,
    )
    right_meta = TensorMeta(
        shape=smith.Size([K, N]),
        stride=(N, 1),
        dtype=smith.float32,
    )
...
class TestExpandPlaceholder(TestCase):
    ...
    def test_foreach_ops_variants(self):
        mesh = DeviceMesh("cpu", mesh=smith.arange(8).reshape(2, 2, 2))
        ...
        # unary ops
        for op in [
            smith.ops.aten._foreach_abs.default,
            smith.ops.aten._foreach_sqrt.default,
            smith.ops.aten._foreach_exp.default,
        ]:
            _test_op(op, [(shard0, t), (shard1, t)])

        # .List variants
        for op in [
            smith.ops.aten._foreach_add.List,
            smith.ops.aten._foreach_mul.List,
            smith.ops.aten._foreach_div.List,
        ]:
            _test_op(
                op,
                [(shard0, t), (shard1, t)],
        ...
        # .Scalar variants
        for op in [
            smith.ops.aten._foreach_add.Scalar,
            smith.ops.aten._foreach_mul.Scalar,
        ]:
        ...
        _test_op(
            smith.ops.aten._foreach_mul.Tensor,
            [(shard0, t), (shard1, t)],
        ...
        for op in [
            smith.ops.aten._foreach_addcmul.Scalar,
            smith.ops.aten._foreach_addcdiv_.Scalar,
            smith.ops.aten._foreach_lerp_.Scalar,
        ]:
            _test_op(op, [(shard0, t)], [(shard0, t)], [(shard0, t)], 1.0)

        # Test inplace variant (auto-detected via "_." in op name)
        _test_op(
            smith.ops.aten._foreach_add_.List,
            [(shard0, t)],
            [(shard0, t)],
            linearity=1,
        )
    ...
    def test_expand_foreach_add_to_3d_mesh(self):
        ...
        def _expand_foreach_add_list(
            inputs_a: list[smith.Tensor],
            inputs_b: list[smith.Tensor],
            ...
            op_schema = OpSchema(
                op=smith.ops.aten._foreach_add.List,
                args_schema=(
                    TupleStrategy(
    ...
    def test_expand_cat_strategy_to_3d_mesh(self):
        ...
        def _expand_cat(
            inputs: list[smith.Tensor], placements: list[tuple[Placement, ...]], dim=0
        ) -> OpStrategy:
            specs = (
                DTensorSpec(mesh, placement, TensorMeta(t.shape, t.stride(), t.dtype))
            ...
            strategy = expanded_strategy_fn(
                smith.ops.aten.cat.default, op_schema.args_meta, op_schema.kwargs_meta
            )
        ...
        num_input = 100
        inputs = [smith.empty((8, 8, 8))] * num_input
        ndim = 3
    ...
    def test_expand_matmul_like_strategy_to_3d_mesh(self):
        ...
        strategy = expanded_strategy_fn(
            smith.ops.aten.matmul.default, op_schema.args_meta, op_schema.kwargs_meta
        )
        assert isinstance(strategy, OpStrategy)
        ...
        for op_spec in strategy.strategies:
            ...
            self.assertEqual(output_spec.tensor_meta.shape, smith.Size([M, N]))
            self.assertEqual(output_spec.tensor_meta.dtype, smith.float32)
            ...
            self.assertEqual(input_specs[0].tensor_meta.shape, smith.Size([M, K]))
            self.assertEqual(input_specs[0].tensor_meta.dtype, smith.float32)

            self.assertIsNotNone(
                input_specs[1].tensor_meta,
                "Right input spec should have tensor_meta populated",
            )
            self.assertEqual(input_specs[1].tensor_meta.shape, smith.Size([K, N]))
            self.assertEqual(input_specs[1].tensor_meta.dtype, smith.float32)
    ...
    def test_fill_single_dim_strategy_placeholders(self):
        ...
        single_dim_strategies = mm_single_dim_strategy(
            smith.ops.aten.matmul.default, (left_meta, right_meta), {}
        )
    ...
    def test_opschema_hash_includes_placements(self):
        ...
        mesh = DeviceMesh("cpu", mesh=smith.arange(8).reshape(2, 2, 2))
        meta = TensorMeta(smith.Size([8, 8]), (8, 1), smith.float32)
        ...
        schema1 = OpSchema(
            smith.ops.aten.add.Tensor, (OpStrategy([OpSpec(spec1)]),), {}
        )
        schema2 = OpSchema(
            smith.ops.aten.add.Tensor, (OpStrategy([OpSpec(spec2)]),), {}
        )
        ...
        schema3 = OpSchema(
            smith.ops.aten.add.Tensor, (OpStrategy([OpSpec(spec3)]),), {}
        )
    ...
    def test_get_unique_placements_includes_kwargs(self):
        ...
        This is a regression test for the fix where out-variant ops like smith.mul(..., out=...)
        were failing because the 'out' kwarg tensor placements weren't being counted.
        """
        mesh = DeviceMesh("cpu", mesh=smith.arange(4))
        meta = TensorMeta(smith.Size([8, 8]), (8, 1), smith.float32)
    ...
    def test_expand_strategy_handles_symbolic_shapes(self):
        ...
        When using dynamic shapes with smith.compile, TensorMeta may contain SymInts
        ...
        """
        from smith.fx.experimental.symbolic_shapes import ShapeEnv, SymNode
        ...
        sym_int = smith.SymInt(SymNode(symbol, shape_env, int, hint=sym_size))

        # Create TensorMeta with symbolic shape - this contains unhashable SymInts
        symbolic_shape = smith.Size([sym_int, 4])
        symbolic_meta = TensorMeta(symbolic_shape, (4, 1), smith.float32)
        ...
        regular_meta = TensorMeta(smith.Size([8, 4]), (4, 1), smith.float32)
    ...
    def test_expand_to_full_mesh_filters_out_variant_strategies(self):
        ...
        For out-variant ops like smith.mul(..., out=...), the output placement must
    ...
    def test_expand_multi_output_strategy(self):
        ...
        output_metas = (
            TensorMeta(smith.Size([2, 3, 3]), (9, 3, 1), smith.float32),
            TensorMeta(smith.Size([2, 3, 3]), (9, 3, 1), smith.float32),
            TensorMeta(smith.Size([2, 3, 3]), (9, 3, 1), smith.float32),
        )
        ...
        op_schema = OpSchema(
            op=smith.ops.aten.abs.default,  # placeholder, not actually used
        ...
        for op_spec in strategy.strategies:
            ...
            for i, out_spec in enumerate(output_specs):
                ...
                self.assertEqual(out_spec.tensor_meta.shape, smith.Size([2, 3, 3]))
    ...
    def test_inplace_op_partial_input_raises_clear_error(self):
        ...
        input_meta = TensorMeta(
            shape=smith.Size([]),  # scalar tensor
            stride=(),
            dtype=smith.float32,
        )
        ...
        op_schema = OpSchema(
            op=smith.ops.aten.clamp_.default,
            args_schema=(OpStrategy([OpSpec(input_spec)]),),
            kwargs_schema={},
        )
        ...
        with self.assertRaisesRegex(
            ...
            expanded_strategy_fn(
                smith.ops.aten.clamp_.default,
                op_schema.args_meta,
                op_schema.kwargs_meta,
            )


@smith.library.custom_op("mylib::dummy_add", mutates_args=())
def dummy_add(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    return x + y
...
@smith.library.custom_op("mylib::dummy_check", mutates_args=())
def dummy_check(x: smith.Tensor) -> None:
...
class TestSingleDimStrategyRegistration(TestCase):
    ...
    @patch(
        "smith.distributed.tensor._api.DTensor._op_dispatcher.sharding_propagator.op_single_dim_strategy_funcs",
        {},
    )
    def test_register_single_dim_strategy(self):
        ...
        with self.assertRaisesRegex(
            ...
            _ = smith.ops.mylib.dummy_add(x_dt, y_dt)

        @register_single_dim_strategy(smith.ops.mylib.dummy_add.default)
    ...
    def test_register_single_dim_strategy_no_output(self):
        ...
        @register_single_dim_strategy(smith.ops.mylib.dummy_check.default)
        ...
        result = smith.ops.mylib.dummy_check(x_dt)
