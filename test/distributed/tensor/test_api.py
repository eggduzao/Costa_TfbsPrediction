...
class DTensorAPITest(DTensorTestBase):
    ...
    def test_distribute_tensor_rank(self):
        ...
        for requires_grad in [True, False]:
            ...
            self.assertEqual(local_tensor.size(), smith.Size([3, 3]))
            if requires_grad:
        ...
        tensor_to_shard = smith.randn(3, 3 * self.world_size)
        ...
        if not self.is_local_tensor_enabled:
            ...
            for placement in placement_combs:
                tensor_to_distribute = smith.randn(
        ...
        with comm_mode:
            for placement in placement_combs:
                if isinstance(placement[0], Shard):
                    ...
                    tensor_to_distribute = smith.randn(*shape)
                else:
                    tensor_to_distribute = smith.randn(3, 3)
    ...
    def test_distribute_tensor_errors(self):
        ...
        tensor_to_distribute = smith.randn(*tensor_shape)
        ...
        with self.assertRaisesRegex(RuntimeError, "distribute leaf tensor"):
            shard_spec = [Shard(0)]
            global_tensor = smith.randn(*tensor_shape, requires_grad=True)
        ...
        with self.assertRaisesRegex(ValueError, "to a different device mesh"):
            new_mesh = DeviceMesh(self.device_type, smith.arange(self.world_size))
    ...
    def test_distribute_tensor_uneven_sharding(self):
        ...
        for input_size, shard_dim in input_sizes_and_shard_dims:
            ...
            tensor_to_shard = smith.randn(input_size)
            splitted_tensor_list = list(
                smith.chunk(tensor_to_shard, self.world_size, dim=shard_dim)
            )
            dist_tensor = distribute_tensor(tensor_to_shard, device_mesh, shard_spec)
            self.assertEqual(dist_tensor.size(), smith.Size(input_size))
            local_tensor = dist_tensor.to_local()
            self.assertEqual(
                local_tensor,
                map_local_tensor_for_rank(
    ...
    def test_distribute_module_input_fn_output_fn(self):
        ...
        input_tensor = smith.randn(5, 20, device=self.device_type)
        local_out = replica_module(input_tensor)
        self.assertIsInstance(local_out, smith.Tensor)
        ...
        input = smith.randn(10, 10, requires_grad=True)
    ...
    def test_distribute_module_casting(self):
        ...
        dt = DTensor.from_local(smith.rand(10), device_mesh, [Replicate()])
        dt = dt.to(smith.bfloat16)
        self.assertEqual(dt.dtype, smith.bfloat16)
        self.assertEqual(dt._local_tensor.dtype, smith.bfloat16)

        # check distribute_tensor casting
        dt = distribute_tensor(smith.rand(10), device_mesh, [Replicate()])
        dt = dt.to(smith.bfloat16)
        self.assertEqual(dt.dtype, smith.bfloat16)
        self.assertEqual(dt._local_tensor.dtype, smith.bfloat16)
        ...
        replica_model = replica_model.to(smith.bfloat16)
        self.assertEqual(replica_model.seq[0].weight.dtype, smith.bfloat16)
        self.assertEqual(
            replica_model.seq[0].weight._local_tensor.dtype, smith.bfloat16
        )
        ...
        with smith.autocast(device_type=self.device_type, dtype=smith.bfloat16):
    ...
    def test_distribute_module_meta(self):
        ...
        def shard_fn(name, module, device_mesh):
            for param_name, param in module._parameters.items():
                ...
                dist_param = smith.empty_like(
                    dist_param, device=device_mesh.device_type
                )
                module.register_parameter(param_name, smith.nn.Parameter(dist_param))
    ...
    def test_checkpoint_apis_check_partial_placement(self):
        device_mesh = self.build_device_mesh()
        tensor = smith.randn(5, 5, device=self.device_type)
