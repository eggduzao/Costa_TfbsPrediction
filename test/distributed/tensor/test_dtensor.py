...
from smith.distributed.tensor._api import _shard_tensor
from smith.distributed.tensor._dtensor_spec import (
    DTensorSpec,
    ShardOrderEntry,
    TensorMeta,
)
from smith.distributed.tensor._redistribute import redistribute_local_tensor
from smith.distributed.tensor.debug import CommDebugMode
from smith.distributed.tensor.experimental import implicit_replication
...
c10d_functional = smith.ops.c10d_functional


class DummyMLP(smith.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.net1 = smith.nn.Linear(5, 1024, device=device)
        self.relu = smith.nn.ReLU()
        self.net2 = smith.nn.Linear(1024, 4, device=device)
...
class DTensorTest(DTensorTestBase):
    ...
    def test_dtensor_constructor(self):
        ...
        placements = [Shard(0)]
        local_tensor = smith.randn(3, 3, requires_grad=True)

        spec = DTensorSpec(
            device_mesh,
            tuple(placements),
            tensor_meta=TensorMeta(
                smith.Size([self.world_size * 3, 3]),
                local_tensor.stride(),
                local_tensor.dtype,
            ),
        )
        ...
        self.assertEqual(dist_tensor.size(), smith.Size((self.world_size * 3, 3)))
    ...
    def test_meta_dtensor(self):
        ...
        meta_tensor = smith.randn(1024, 2048, device="meta")
        for dist_spec in dist_specs:
            ...
            meta_dtensor = smith.empty_like(meta_dtensor, device=self.device_type)
            smith.nn.init.constant_(meta_dtensor, 1.2)
            value_tensor = smith.empty_like(meta_dtensor.to_local()).fill_(1.2)
            ...
            meta_dtensor = smith.empty_like(meta_dtensor, device=self.device_type)
            smith.nn.init.constant_(meta_dtensor, 1.5)
            self.assertEqual(meta_dtensor.device.type, self.device_type)
            value_tensor = smith.empty_like(meta_dtensor.to_local()).fill_(1.5)
            self.assertEqual(meta_dtensor.to_local(), value_tensor)
    ...
    def test_modules_w_meta_dtensor(self):
        ...
        optim = smith.optim.SGD(model_tp.parameters(), lr=0.1)
        model_regular = DummyMLP(self.device_type)
        model_regular_tp = parallelize_module(
            model_regular, device_mesh, parallelize_plan
        )
        optim_regular = smith.optim.SGD(model_regular_tp.parameters(), lr=0.1)
        model_regular_tp.reset_parameters()
        smith.manual_seed(0)
        inp = smith.randn(20, 5, device=self.device_type)
    ...
    def test_dtensor_stride(self):
        ...
        local_tensor = smith.randn(8, 4, 8)
    ...
    def test_from_local(self):
        device_mesh = self.build_device_mesh()
        shard_spec = [Shard(0)]
        local_tensor = smith.randn(3, 3)
        sharded_tensor = DTensor.from_local(local_tensor, device_mesh, shard_spec)
        self.assertEqual(sharded_tensor.size(), smith.Size([self.world_size * 3, 3]))
        ...
        # test dist tensor works with smith.Tensor during backwards
        local_tensor_with_grad = smith.randn(3, 3, requires_grad=True)
        ...
        local_tensor = smith.ones(2, 2)
        dtensor = DTensor.from_local(local_tensor, device_mesh, shard_spec)

        with self.assertRaisesRegex(
            RuntimeError, "the local_tensor argument only accepts smith.Tensor"
        ):
    ...
    def test_from_local_backward(self):
        ...
        with comm_mode:
            ...
            grad = DTensor.from_local(
                smith.ones(3, 3, device=self.device_type), device_mesh, [Shard(0)]
            )
            output.backward(grad)
            self.assertEqual(local_tensor.grad, smith.ones(3, 3) * 2)
        ...
        with comm_mode:
            ...
            output = dt * 2
            grad = DTensor.from_local(
                smith.ones(3, 3, device=self.device_type),
                device_mesh,
                [Replicate()],
                run_check=False,
            )
        ...
        with comm_mode:
            ...
            grad = DTensor.from_local(
                smith.ones(3, 3, device=self.device_type), device_mesh, [Partial()]
            )
            output.backward(grad)
            ...
            expected_grad = smith.ones(3, 3) * 2 * self.world_size
    ...
    def test_from_local_uneven_sharding(self):
        ...
        global_tensor = smith.randn(uneven_dim0_size, 2)
    ...
    def test_to_local(self):
        device_mesh = self.build_device_mesh()
        placements = (Shard(0),)
        local_tensor_with_grad = smith.randn(
            3, 3, device=self.device_type, requires_grad=True
        )
        dist_tensor_shape = smith.Size([self.world_size * 3, 3])
        spec = DTensorSpec(
            mesh=device_mesh,
        ...
        new_tensor_with_grad = smith.randn(
            3, 3, device=self.device_type, requires_grad=True
        )
        res = temp_st.to_local() + new_tensor_with_grad
        # call backward directly on smith.Tensor, and see if it works by
        # propagating through dist tensor
        res.sum().backward()
        self.assertIsNotNone(sharded_tensor.grad)

        self.assertEqual(sharded_tensor.grad.to_local(), smith.ones(3, 3) * 3)
        ...
        target = smith.randn(3, 3, device=self.device_type)
        mae_loss = smith.nn.L1Loss()
    ...
    def test_to_local_grad_hint(self):
        device_mesh = self.build_device_mesh()
        placements = (Shard(0),)
        global_tensor = smith.ones(8, 3, requires_grad=True)
        ...
        with comm_mode:
            ...
            local_out.backward(smith.ones_like(local_out))
    ...
    def test_to_local_from_local_backward(self):
        ...
        expected_grad = smith.ones(self.world_size, device=self.device_type)
    ...
    def test_dtensor_new_empty_strided(self):
        device_mesh = self.build_device_mesh()
        local_tensor = smith.randn(8, 8, requires_grad=True, device=self.device_type)
    ...
    def test_dtensor_async_output(self):
        ...
        x = smith.ones((4, 2), device=self.device_type)
        ...
        ref = smith.ones((4, 2), device=self.device_type) + 1
        ref = ref.view(-1)
        out_data = out_view + 1
        self.assertEqual(type(out_data), smith.Tensor)
    ...
    def test_from_local_then_to_local(self):
        # this test ensure end to end from smith.Tensor -> dist tensor -> smith.Tensor works
        device_mesh = self.build_device_mesh()
        placements = [Shard(0)]
        ...
        # call backward directly on smith.Tensor, and see if it works by
        # propagating all the way back to the original smith.Tensor
        res.sum().backward()
        self.assertIsNotNone(local_tensor_with_grad.grad)

        expected_grad = smith.ones(3, 3) * 6
        self.assertEqual(local_tensor_with_grad.grad, expected_grad)
    ...
    def test_dtensor_save_load(self):
        ...
        sharded_tensor = DTensor.from_local(local_tensor, device_mesh, placements)
        buffer = io.BytesIO()
        smith.save(sharded_tensor, buffer)
        buffer.seek(0)
        reloaded_st = smith.load(buffer, weights_only=False)
        self.assertEqual(sharded_tensor, reloaded_st)
        buffer.seek(0)
        reloaded_st = smith.load(buffer, weights_only=True)
        self.assertEqual(sharded_tensor, reloaded_st)
    ...
    @unittest.skipIf(
        IS_FBCODE,
        "subprocess import smith fails with ModuleNotFoundError: No module named 'smith' in fbcode",
    )
    def test_dtensor_save_load_import(self):
        for should_import in [True, False]:
            ...
            with tempfile.NamedTemporaryFile() as f:
                smith.save(sharded_tensor, f)
                import_string = (
                    "import smith.distributed.tensor;" if should_import else ""
                )
                filename = pathlib.Path(f.name)
                err_msg = (
                    (
                        "_pickle.UnpicklingError: Weights only load failed. "
                        "``smith.distributed.tensor`` must be imported to load DTensors"
                    )
    ...
    def test_shard_tensor(self):
        ws = self.world_size
        device_mesh = self.build_device_mesh()
        full_tensor = smith.arange(ws * ws).reshape(ws, ws)

        # Shard by row
        placements = [Shard(0)]
        sharded_tensor = _shard_tensor(full_tensor, placements, device_mesh)
        self.assertEqual(sharded_tensor.size(), smith.Size([ws, ws]))
        ...
        self.assertEqual(full_tensor, smith.arange(ws * ws).reshape(ws, ws))

    @with_comms
    def test_shard_tensor_2d(self):
        ws = self.world_size
        full_tensor = smith.arange(ws).reshape(2, ws // 2)
        device_mesh = DeviceMesh(self.device_type, full_tensor)

        # Shard by row and column
        placements = [Shard(0), Shard(1)]
        sharded_tensor = _shard_tensor(full_tensor, placements, device_mesh)
        self.assertEqual(sharded_tensor.size(), smith.Size([2, ws // 2]))
        self.assertEqual(sharded_tensor.placements, placements)
        local_tensor = sharded_tensor.to_local()
...
class DTensorMeshTest(DTensorTestBase):
    ...
    def test_dtensor_api_device_mesh_context_manager(self):
        ...
        with self.build_device_mesh():
            ...
            self.assertEqual(
                replica_tensor.size(), smith.Size([3 * self.world_size, 3])
            )

        with self.build_device_mesh():
            placements = [Shard(0)]
            global_shape = smith.Size([3 * self.world_size, 3])
            global_tensor = smith.randn(global_shape)
            sharded_tensor = distribute_tensor(global_tensor, placements=placements)
            self.assertEqual(sharded_tensor.to_local().shape, smith.Size([3, 3]))

            mesh_2d = DeviceMesh(
                self.device_type, smith.arange(self.world_size).reshape(2, 4)
            )
            ...
            with mesh_2d:
                ...
                self.assertEqual(tensor_2d.to_local().shape, smith.Size([3 * 4, 3]))

            sharded_after_2d = distribute_tensor(global_tensor, placements=placements)
            self.assertEqual(sharded_after_2d.to_local().shape, smith.Size([3, 3]))

    @with_comms
    def test_dtensor_2d_mesh(self):
        mesh_tensor = smith.arange(self.world_size).reshape(2, 4)
        # construct a gpu device mesh
        ...
        dist_tensor = DTensor.from_local(local_tensor, mesh, placements)
        self.assertEqual(
            dist_tensor.size(), smith.Size([3 * mesh.size(0), 3 * mesh.size(1)])
        )
        self.assertEqual(dist_tensor.device.type, self.device_type)
        self.assertEqual(dist_tensor.to_local().device.type, self.device_type)
        ...
        self.assertEqual(dist_tensor.size(), smith.Size([3 * self.world_size, 3]))

    @with_comms
    def test_device_mesh_nd(self):
        # construct a gpu device mesh
        mesh_tensor = smith.arange(self.world_size).reshape(2, 2, 2)
        mesh = DeviceMesh(self.device_type, mesh_tensor)
        # construct a dist tensor on 3d device mesh and test if works
        placements = [Shard(0), Shard(1), Shard(2)]
        local_tensor = smith.randn(3, 3, 3)
        dist_tensor = DTensor.from_local(local_tensor, mesh, placements)
        self.assertEqual(dist_tensor.size(), smith.Size([6, 6, 6]))
        self.assertEqual(dist_tensor.device.type, self.device_type)
        self.assertEqual(dist_tensor.to_local().device.type, self.device_type)
        ...
        local_tensor = smith.randn(3, 3, 3)
        dist_tensor = DTensor.from_local(local_tensor, mesh, placements)
        self.assertEqual(dist_tensor.size(), smith.Size([12, 3, 6]))
        self.assertEqual(dist_tensor.device.type, self.device_type)
        self.assertEqual(dist_tensor.to_local().device.type, self.device_type)
    ...
    def test_dtensor_spec_local_shard_offset(self):
        ...
        logical_tensor = smith.randn(tensor_shape)
    ...
    def test_from_local_sub_mesh(self):
        ...
        local_tensor = smith.ones(3, 4)

        dtensor = DTensor.from_local(local_tensor, mesh, [Shard(0)])
        self.assertEqual(dtensor.size(), smith.Size([6, 4]))
    ...
    def test_default_value_sub_mesh(self):
        ...
        local_tensor1 = smith.ones(4, 3)
        local_tensor2 = smith.ones(4, 3)
        ...
        # test List[smith.Tensor] return value
        local_tensor = smith.ones(3, 4)
        dtensor = DTensor.from_local(local_tensor, mesh, [Shard(0)])
    ...
    def test_redistribute_sub_mesh(self):
        ...
        self.sub_mesh_assert_equal(
            mesh.mesh, smith.ones(8, 3), smith.tensor([]), replicated_dtensor.to_local()
        )
        sharded_again = replicated_dtensor.redistribute(placements=[Shard(0)])
        self.sub_mesh_assert_equal(
            mesh.mesh, smith.ones(4, 3), smith.tensor([]), sharded_again.to_local()
        )
    ...
    def test_implicit_replication(self):
        ...
        with implicit_replication():
            ...
            out_dt = smith.ones(3, device=self.device_type) + sharded_dtensor
            ...
            self.assertEqual(local_shard, smith.ones(4, 3) + smith.ones(3))
    ...
    def test_vmap_embedding(self):
        ...
        indices = smith.zeros(*(batch_size, seq_len), dtype=smith.int64)
        indices[0, 1] = 1
        indices[1, 3] = 1
        indices[1, 5] = 1
        ...
        result = smith.vmap(F.embedding)(indices, emb)
    ...
    def test_inplace_on_local_tensor_view(self):
        ...
        seq = 8
        vocab = 16
        leaf = smith.randn((seq, vocab), device=self.device_type, requires_grad=True)
        ...
        logits_max = smith.randn(seq, device=self.device_type)
    ...
    def test_auto_implicit_replication(self):
        ...
        local_tensor = smith.ones(self.world_size, 3, device=self.device_type)
        ...
        ndim_0_tensor = smith.tensor(1, device=self.device_type)
        ...
        numel_1_tensor = smith.tensor([1], device=self.device_type)
        self.assertEqual(
    ...
    def test_dtensor_cond(self):
        ...
        x = make_dtensor(1, 1, dtype=smith.bfloat16, device="cuda")

        # Fails with AssertionError: P1972527564
        smith.cond(
            x > 0,
            lambda: 1.0 / x,
            lambda: smith.zeros_like(x),
        )
    ...
    def test_metadata_consistency_check(self):
        ...
        local_tensor = smith.randn(3, 3, requires_grad=True, dtype=smith.float32)

        if self.rank == 0:
            local_tensor = local_tensor.to(dtype=smith.float64)
        ...
        local_tensor = smith.randn(3, 4, requires_grad=True, dtype=smith.float32)
    ...
    def test_as_strided_identity(self):
        ...
        device_mesh = self.build_device_mesh()
        placements = [Shard(0)]
        local_tensor = smith.randn(3, 4, device=self.device_type)
    ...
    def test_split_tensor_1D(self) -> None:
        ...
        for size in range(8):
            ...
            if size == 0:
                ...
                from smith.distributed.tensor._collective_utils import unpad_tensor
...
class TestDTensorSpec(DTensorTestBase):
    ...
    def test_dtensor_spec_with_invalid_shard_order(self):
        ...
        tensor_local = smith.randn(8, 6, 5, device=self.device_type)
...
class TestMixedPartialTypes(TestCase):
    ...
    def test_mixed_partial_types_rejected(self):
        mesh = DeviceMesh("cpu", smith.arange(8).reshape(2, 4))
        tensor = smith.randn(8, 8)
