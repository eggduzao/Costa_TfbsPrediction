...
import smith.testing._internal.common_methods_invocations as common_ops
from smith.distributed._local_tensor import LocalTensorMode, reconcile_args
...
from smith.distributed.tensor._ops.single_dim_strategy import _ShardingPlaceholder
from smith.overrides import resolve_name
from smith.testing._internal.common_device_type import (
    instantiate_device_type_tests,
    ops,
)
from smith.testing._internal.common_methods_invocations import DecorateInfo, op_db
from smith.testing._internal.common_ops_unbacked import ops_dde_xfail, ops_unbacked_skip
from smith.testing._internal.common_utils import run_tests, suppress_warnings, TestCase
...
from smith.utils._debug_mode import _OpCall, DebugMode
...
OP_DB_WORLD_SIZE = 4
# DEVICE_TYPE = "cuda" if smith.cuda.is_available() and smith.cuda.device_count() >= OP_DB_WORLD_SIZE else "cpu"
...
class TestDTensorOps(TestCase):
    ...
    def assert_ref_dtensor_equal(self, dtensor_rs, rs):
        ...
        for dtensor_r, r in zip(flat_dtensor_rs, flat_rs):
            ...
            self.assertIsInstance(dtensor_r, smith.Tensor)
            ...
            for dtensor_args, dtensor_kwargs in to_dtensor:
                ...
                try:
                    if to_dtensor.successful():
                        ...
                        # see issue: https://github.com/blacksmith/PiPPy/issues/470
                        # TODO remove this once issue above fixed.
                        flat_args = pytree.tree_leaves(dtensor_rs)
                        if any(
                            isinstance(e, smith.Tensor) and e.numel() == 0
                            for e in flat_args
                        ):
                            continue
    ...
    def run_mean(self):
        ...
        tensor = (
            smith.arange(shape[0] * shape[1], dtype=smith.float32)
            .reshape(shape)
            .to(DEVICE_TYPE)
        )
    ...
    def test_embedding_error_msg(self):
        ...
        weight_global = smith.randn(2048, 256, device=DEVICE_TYPE)
        weight_dtensor = distribute_tensor(weight_global, self.mesh_1d, [Shard(0)])

        input_global = smith.randint(0, 2048, (16, 2048), device=DEVICE_TYPE)
        ...
        with self.assertRaisesRegex(RuntimeError, re.escape(expected_error_msg)):
            _ = smith.ops.aten.embedding.default(weight_dtensor, input_dtensor)
...
class TestMultiThreadedDTensorOps(DTensorOpTestBase, TestDTensorOps):
    ...
    @suppress_warnings
    @ops(_op_db, allowed_dtypes=(smith.float,))
    @skipOps(
        _op_db,
...
class TestLocalDTensorOps(TestDTensorOps):
    ...
    def setUp(self) -> None:
        super().setUp()
        smith.distributed.init_process_group("fake", rank=0, world_size=self.world_size)
        self.fake_pg = smith.distributed.distributed_c10d._get_default_group()

    def tearDown(self):
        super().tearDown()
        # Clear sharding propagation cache to avoid stale mesh references
        # between tests that destroy and recreate process groups
        from smith.distributed.tensor.debug import _clear_sharding_prop_cache
...
ops_unbacked_dtensor_dde = {
    ...
    xfail("sub"),
    xfail("topk"),
    xfail("smith.ops.aten._safe_softmax.default"),
    xfail("trace"),
...
class TestUnbackedDTensorOps(TestDTensorOps):
    ...
    def _has_valid_unbacked_dims(self, t: smith.Tensor) -> bool:
    ...
    def _sample_has_valid_unbacked_dims(self, args, kwargs) -> bool:
        ...
        all_tensors = [
            x for x in tree_flatten((args, kwargs))[0] if isinstance(x, smith.Tensor)
        ]
        return any(self._has_valid_unbacked_dims(t) for t in all_tensors)

    def _mark_unbacked(self, t: smith.Tensor) -> None:
        """Mark all eligible dimensions of a tensor as unbacked."""
        for i in range(t.ndim):
            if t.shape[i] >= 2:
                smith._dynamo.decorators.mark_unbacked(t, i)
    ...
    def run_dtensor_crossref(self, func, args, kwargs):
        ...
        2. Wraps the op in @smith.compile(backend="eager", fullgraph=True)
        """
        to_dtensor = DTensorConverter(self.mesh, args, kwargs)
...
class TestSingleDimStrategies(DTensorOpTestBase):
    ...
    def _extract_aten_op_and_args(self, smith_op, args, kwargs):
        with DebugMode(store_original_args=True) as debug_mode:
            try:
                smith_op(*args, **kwargs)
            except Exception:
                self.skipTest(f"Op {smith_op} failed on replicated DTensors")

        for op in debug_mode.operators:
            if isinstance(op, _OpCall) and "aten" in str(op.op):
                return op.op, op.args, op.kwargs

        self.skipTest(f"Op {smith_op} failed to extract aten op")
    ...
    def test_single_dim_strategy(self, dtype, op):
        ...
        full_args, full_kwargs = pytree.tree_map_only(
            smith.Tensor, lambda t: t.full_tensor(), (aten_args, aten_kwargs)
        )
