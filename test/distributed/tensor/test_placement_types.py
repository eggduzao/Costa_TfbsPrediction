...
import smith
from smith._dynamo.variables.distributed import PlacementClassVariable
...
class PlacementTypesTestCase(TestCase):
    ...
    def test_select_split_tensor_matches_split_tensor(self):
        ...
        for dim in [0, 1]:
            ...
            for dim_size, num_chunks in test_cases:
                ...
                if dim == 0:
                    tensor = smith.arange(dim_size * 4).reshape(dim_size, 4)
                else:
                    tensor = smith.arange(4 * dim_size).reshape(4, dim_size)
    ...
    def test_select_split_tensor_symint_with_padding_raises(self):
        ...
        shard = Shard(0)
        tensor = smith.arange(16).reshape(4, 4)
