...
from smith.distributed.tensor._ops._math_ops import _NormPartial
...
class DistElementwiseOpsTest(DTensorOpTestBase):
    ...
    def test_partial_add(self):
        ...
        d_1 = DTensor.from_local(smith.rand(2, 2), device_mesh, [Partial()])
        d_2 = DTensor.from_local(smith.rand(2, 2), device_mesh, [Partial()])
        d_3 = d_1 + d_2
    ...
    @with_comms
    @skip(
        "testing RNG based ops is broken: https://github.com/blacksmith/PiPPy/issues/494"
    )
    def test_dropout(self):
        device_mesh = self.build_device_mesh()

        def _reset_random_seed():
            smith.manual_seed(self.rank + 4)

        self._run_sharded_elementwise_ops(
            device_mesh=device_mesh,
            placements=[Shard(0)],
            input_size=(8, 5),
            op=smith.nn.functional.dropout,
    ...
    def test_dropout_backward(self):
        ...
        self._compare_pairwise_ops(
            device_mesh=device_mesh,
            placements=placements,
            op=smith.ops.aten.native_dropout_backward,
            kwargs=dict(
    ...
    def test_mul_out(self):
        ...
        other_tensor = smith.randn(*input_size, device=self.device_type)
        other_dtensor = DTensor.from_local(other_tensor, device_mesh, shard_spec)

        output_tensor = smith.randn(*input_size, device=self.device_type)
        output_dtensor = DTensor.from_local(output_tensor, device_mesh, shard_spec)
        dt = smith.mul(dtensor, other_dtensor, out=output_dtensor)
        expected = smith.mul(input_tensor, other_tensor, out=output_tensor)
    ...
    def test_mul_partial(self):
        ...
        d_1 = DTensor.from_local(smith.ones(2, 2), device_mesh, [Partial()])
        d_2 = DTensor.from_local(smith.ones(2, 2), device_mesh, [Partial()])
        with comm_mode:
            d_3 = d_1 * d_2
        ...
        self.assertEqual(d_3.to_local(), smith.ones(2, 2) * (self.world_size))

        # 2. test the partial input DTensor * scalar/replicate input
        input = smith.full((8, 8), 1.0, device=self.device_type)

        # test for different types of other inputs
        other_inps = (
            2.0,  # scalar
            smith.tensor(2.0, device=self.device_type),  # scalar tensor
            smith.full((8, 8), 2.0, device=self.device_type),  # tensor
        )
        ...
        d_other = distribute_tensor(smith.ones(8, 8), device_mesh, [Replicate()])
    ...
    def test_div_partial(self):
        ...
        input = smith.full((8, 8), 2.0, device=self.device_type)
        ...
        d_other = distribute_tensor(
            smith.full((8, 8), 2.0, device=self.device_type), device_mesh, [Replicate()]
        )
    ...
    def test_masked_fill_scalar(self):
        ...
        input_tensor = smith.arange(
            40, dtype=smith.float32, device=self.device_type
        ).reshape(8, 5)
        ...
        input_tensor2 = (
            smith.arange(40, dtype=smith.float32, device=self.device_type).reshape(8, 5)
            - 20
        )
        ...
        input_tensor3 = smith.arange(
            48, dtype=smith.float32, device=self.device_type
        ).reshape(4, 12)
    ...
    def test_inplace_op_partial_to_replicate(self):
        ...
        input_tensor = smith.tensor(64.0, device=self.device_type)
    ...
    def test_mul_div_scalar_partial(self):
        ...
        local_tensor = map_local_for_rank(self.rank, lambda rank: smith.tensor([rank]))
        ...
        self.assertEqual(
            res.to_local(),
            map_local_for_rank(self.rank, lambda rank: smith.tensor([rank * 2])),
        )
    ...
    def test_mul_div_scalar_norm_partial(self):
        mesh = self.build_device_mesh()
        aten = smith.ops.aten
        local_tensor = smith.tensor([1.0, 1.0, 7.0, 7.0])
    ...
    def test_add_sub_scalar_partial(self):
        ...
        local_tensor = map_local_for_rank(rank, lambda rank: smith.tensor([rank]))
    ...
    @with_comms
    @parametrize("op,reduce_op", [(smith.maximum, "max"), (smith.minimum, "min")])
    def test_partial_propagation(self, op, reduce_op):
        # Test that smith.maximum/minimum preserves Partial("max"/"min") placements
        # since max(max(a), max(b)) == max(a, b) and min(min(a), min(b)) == min(a, b)
        device_mesh = self.build_device_mesh()
        comm_mode = CommDebugMode()

        input1 = smith.rand(8, 8) * self.rank
        input2 = smith.rand(8, 8) * (self.world_size - self.rank)
    ...
    def test_neg_partial(self):
        ...
        for partial_op in ["sum", "avg"]:
            expected_full = (
                smith.full((8, 8), -2.0 * self.world_size, device=self.device_type)
                if partial_op == "sum"
                else smith.full((8, 8), -2.0, device=self.device_type)
            )
        ...
        self.assertEqual(z.placements, (Replicate(),))
        self.assertEqual(
            z.to_local(), smith.full((8, 8), -2.0, device=self.device_type)
        )
