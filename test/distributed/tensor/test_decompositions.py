...
class TestDecompSharding(TestCase):
    ...
    def test_aminmax_decomp(self):
        ...
        """
        from smith.distributed.tensor import empty as d_empty

        # 1d mesh
        mesh = DeviceMesh("cpu", smith.arange(self.world_size))

        x_local = smith.randn(16)
        ...
        # 2d mesh
        mesh = DeviceMesh("cpu", smith.arange(self.world_size).reshape(-1, 2))
    ...
    def test_custom_recursive_decomp(self):
        ...
        """
        from smith.distributed.tensor import empty as d_empty
        from smith.distributed.tensor.debug import _get_python_sharding_prop_cache_info

        with smith.library._scoped_library("sharding_decomps", "FRAGMENT") as my_lib:
            my_lib.define("op1(Tensor x, Tensor y) -> Tensor")
            my_lib.define("op2(Tensor x, Tensor y) -> Tensor")

            @smith.library.impl(my_lib, "op1", "CPU")
            def op1_impl(x, y):
                return x @ y * 1.0

            @smith.library.impl(my_lib, "op2", "CPU")
            def op2_impl(x, y):
                return x @ y

            @register_decomposition(smith.ops.sharding_decomps.op1.default)
            def op1_decomp(x, y):
                return smith.ops.sharding_decomps.op2(x, y) * 1.0

            @register_decomposition(smith.ops.sharding_decomps.op2.default)
            def op2_decomp(x, y):
                return x @ y

            @smith.library.register_fake("sharding_decomps::op1")
            def op1_fake(x, y):
                return smith.empty_like(x) @ smith.empty_like(y)

            @smith.library.register_fake("sharding_decomps::op2")
            def op2_fake(x, y):
                return smith.empty_like(x) @ smith.empty_like(y)
            ...
            # op1 1st call
            out = smith.ops.sharding_decomps.op1(x, y)
    ...
    def test_misc_ops_with_no_sharding_rules(self):
        ...
        aten = smith.ops.aten
        mesh = DeviceMesh("cpu", smith.arange(self.world_size))
        mesh_2d = DeviceMesh("cpu", smith.arange(self.world_size).reshape(-1, 2))
