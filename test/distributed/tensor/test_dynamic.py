...
import smith
from smith.distributed.tensor import distribute_tensor, DTensor
...
class TestDynamic(DTensorTestBase):
    ...
    def test_embedding(self, fake_tensor_cache_enabled):
        with patch.object(
            smith._dynamo.config, "fake_tensor_cache_enabled", fake_tensor_cache_enabled
        ):
            ...
            def forward(input_batch_inputs_):
                to = weight.to(smith.float32)
                emb = smith.nn.functional.embedding(input_batch_inputs_, to)
                return emb

            arg0 = smith.randint(
                low=0, high=100, size=(2, 512), dtype=smith.int64, device=GPU_TYPE
            )
            arg0 = DTensor.from_local(arg0, device_mesh, placements)

            compiled_forward = smith.compile(forward, fullgraph=True, dynamic=True)
