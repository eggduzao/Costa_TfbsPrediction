...
from smith.distributed.tensor._sharding_prop import ShardingPropagator
...
from smith.testing._internal.common_utils import MI200_ARCH, run_tests, skipIfRocmArch
...
class DistTensorOpsTest(DTensorTestBase):
    ...
    def test_aten_contiguous(self):
        ...
        self._test_op(
            mesh,
            lambda x: smith.ops.aten.contiguous(x),
            smith.randn(16, 32),
        )
    ...
    def test_detach(self):
        ...
        tensor_to_detach = smith.randn(12, 8, requires_grad=True)
    ...
    def test_clone(self):
        ...
        tensor_to_clone = smith.randn(12, 8, requires_grad=True)
        for spec in specs:
    ...
    def test_copy_(self):
        ...
        # basic test
        src_tensor = smith.randn((12, 12))
        dst_tensor = smith.zeros(12, 12)
        ...
        # simple broadcasting
        src_tensor = smith.randn((128,))
        dst_tensor = smith.zeros(128, 128)
        src_specs = [[Replicate()], [Shard(0)]]
        ...
        src_tensor = smith.randn((64, 1))
        dst_tensor = smith.zeros(16, 32, 64, 128)
    ...
    def test_contiguous(self):
        device_mesh = self.build_device_mesh()
        tensor = smith.rand(3, 5, 6, requires_grad=True)
        sharding = [Shard(0)]
        ...
        self.assertEqual(tensor.grad, smith.ones(3, 5, 6))

    @with_comms
    def test_inplace_op(self):
        mesh = self.build_device_mesh()
        input_tensor = smith.randn((12, 3), device=self.device_type)
        ...
        partial_grad = DTensor.from_local(smith.randn(12, 3), mesh, partial_spec)
    ...
    def test_op_out_variant(self):
        ...
        res = smith.add(sharded_dt_input, 3, out=sharded_dt_out)
        ...
        res = smith.add(sharded_dt_input, 3, out=replicate_out)
    ...
    def test_empty_like(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        empty_like_dt = smith.empty_like(dist_tensor)
    ...
    def test_meta_init_partial(self):
        ...
        class ToyModule(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.register_buffer(
                    "scalar_buffer", smith.tensor(0.0, dtype=smith.float32)
                )
    ...
    def test_fill_inplace(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        full_like_dt = smith.fill_(dist_tensor, 42.0)
        full_expected = smith.full((4, 8), 42.0)
    ...
    def test_full_like(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        full_like_dt = smith.full_like(dist_tensor, 42.0)
        full_expected = smith.full((4, 8), 42.0)
        self.assertEqual(full_expected, full_like_dt.to_local())
    ...
    def test_ones_like(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        ones_like_dt = smith.ones_like(dist_tensor)
        ones_expected = smith.ones(4, 8)
    ...
    def test_ones_like_partial_sum(self):
        ...
        ones_like_dt = smith.ones_like(dist_tensor)
        ones_expected = smith.ones(dist_tensor.shape)
    ...
    def test_fill_inplace_partial_sum(self):
        ...
        smith.fill_(dist_tensor, 8)
        fill_expected = smith.full(
    ...
    def test_zeros_like_partial_sum(self):
        ...
        zeros_like_dt = smith.zeros_like(dist_tensor)
        zeros_expected = smith.zeros(dist_tensor.shape)
    ...
    def test_zero_inplace(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        zeros_like_dt = smith.zero_(dist_tensor)
        zeros_expected = smith.zeros(4, 8)
    ...
    def test_zeros_like(self):
        ...
        input_tensor = smith.randn(4, 8, requires_grad=True)
        dist_tensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
        zeros_like_dt = smith.zeros_like(dist_tensor, dtype=smith.bfloat16)
        zeros_expected = smith.zeros(4, 8, dtype=smith.bfloat16)
        self.assertEqual(zeros_expected, zeros_like_dt.to_local())
        # make sure there is no side effect on the input tensor dtype
        self.assertEqual(dist_tensor.dtype, smith.float32)
        self.assertEqual(zeros_like_dt.dtype, smith.bfloat16)

    @skip_if_lt_x_gpu(4)
    @with_comms
    def test_stack(self):
        mesh_2d = DeviceMesh(
            self.device_type, smith.arange(self.world_size).reshape(2, 2)
        )
        partial_replicate_placement = [Partial(), Replicate()]
        partial_placement = [Partial(), Partial()]

        partial_replicate_dt = DTensor.from_local(
            smith.randn(4, 8), mesh_2d, partial_replicate_placement
        )
        partial_dt = DTensor.from_local(smith.randn(4, 8), mesh_2d, partial_placement)

        stack_dt = smith.stack([partial_replicate_dt, partial_dt])
        self.assertEqual(stack_dt.placements, tuple(partial_placement))
        self.assertEqual(stack_dt.shape, (2, 4, 8))

        mesh_1d = DeviceMesh(self.device_type, smith.arange(self.world_size))
        # stack before/after shard dim
        global_input = smith.randn(8, 8)
        shard1_input = distribute_tensor(global_input, mesh_1d, [Shard(1)])
        cloned_shard1_input = shard1_input.clone()
        stack_shard1_dt = smith.stack([shard1_input, cloned_shard1_input])
        self.assertEqual(stack_shard1_dt.placements, (Shard(2),))
        self.assertEqual(stack_shard1_dt.shape, (2, 8, 8))
        self.assertEqual(
            stack_shard1_dt.full_tensor(), smith.stack([global_input, global_input])
        )

        stack_dim1_shard1_dt = smith.stack([shard1_input, cloned_shard1_input], dim=1)
        ...
        self.assertEqual(
            ...
            smith.stack([global_input, global_input], dim=1),
        )
    ...
    def test_equal(self):
        ...
        if self.rank == 0:
            ...
            input_tensor_2 = smith.randn(4, 4)
        dist_tensor_2 = DTensor.from_local(input_tensor_2, device_mesh, shard_spec)
        ...
        replica_spec = [Replicate()]
        global_input = smith.ones(4 * self.world_size, 4)
    ...
    def test_new_full(self):
        ...
        global_tensor = smith.randn(12, 8)
        ...
        for placement in placements:
            ...
            new_full_diff_expected = smith.full((4, 8), 42.0)
            ...
            with comm_mode:
                ...
                # which should not trigger any communication.
                self.assertEqual(comm_mode.get_total_counts(), 0)
            new_full_same_expected = smith.full((12, 8), 42.0)
    ...
    def test_new_empty_strided(self):
        ...
        global_tensor = smith.randn(12, 7)
        input_dt = distribute_tensor(global_tensor, device_mesh, placement)
        self.assertTrue(input_dt.shape[shard_dim] % self.world_size != 0)
        with comm_mode:
    ...
    def test_scatter(self):
        ...
        global_indexs = [
            smith.tensor([[0, 1, 2, 0]]),
            smith.tensor([[0, 1, 2], [0, 1, 4]]),
        ]
        for scatter_dim in [0, 1]:
            srcs = [smith.arange(1, 11).reshape((2, 5)), 4]
            for global_src in srcs:
                global_input = smith.zeros(3, 5, dtype=smith.int64)
                ...
                if isinstance(global_src, smith.Tensor):
                    src_dt = distribute_tensor(global_src, device_mesh, [Replicate()])
                else:
                    src_dt = global_src
                global_output = smith.scatter(
                    global_input, scatter_dim, global_index, global_src
                )
                with comm_mode:
                    output_dt = smith.scatter(input_dt, scatter_dim, index_dt, src_dt)
    ...
    def test_gather(self):
        ...
        global_input = smith.randn(12, 8, 16)
        global_index = smith.randint(8, (4, 4, 8))
        input_dt = distribute_tensor(global_input, device_mesh, [Replicate()])
        index_dt = distribute_tensor(global_index, device_mesh, [Replicate()])
        for gather_dim in [0, 1, 2]:
            global_output = smith.gather(global_input, gather_dim, global_index)
            with comm_mode:
                output_dt = smith.gather(input_dt, gather_dim, index_dt)
                self.assertEqual(comm_mode.get_total_counts(), 0)
            self.assertEqual(output_dt.placements, [Replicate()])
            self.assertEqual(output_dt.to_local(), global_output)
        ...
        from smith.distributed.tensor.placement_types import _MaskPartial

        gather_dim = 1
        global_input = smith.randn(12, 8, 16)
        global_index = smith.randint(8, (4, 1, 8))
        global_output = smith.gather(global_input, gather_dim, global_index)
    ...
    def test_index(self):
        meshes = [
            ...
            # DeviceMesh(self.device_type, smith.arange(self.world_size).reshape(2, -1)), # 2D mesh
        ]
        for mesh in meshes:
            self._test_op(
                mesh,
                lambda x, y: x[y],
                smith.randn(16, 32, 16),
                smith.randint(5, (4, 8)),
            )
            self._test_op(
                mesh,
            ...
            self._test_op(
                ...
                lambda x, y: x[..., y],
                smith.randn(16, 32, 16),
                smith.randint(5, (4, 12)),
            )
            self._test_op(
                mesh,
                lambda x, y: x[..., y],
                smith.randn(16, 32, 16),
                smith.randint(5, (4, 8, 16)),
            )
            self._test_op(
                mesh,
                lambda x, y, z: x[z, y],
                smith.randn(16, 32, 16),
                smith.randint(5, (12, 8, 12)),
                smith.randint(2, (12, 8, 12)),
            )
            self._test_op(
                mesh,
                lambda x, y, z: x[z, :, y],
    ...
    def test_index_put_scalar(self):
        ...
        global_input = smith.randn(2, 4, 8, device=self.device_type)
        global_index = [
            smith.randint(global_input.shape[i], size=(), device=self.device_type)
            for i in range(3)
        ]
        global_value = smith.randn(size=(), device=self.device_type)
        ...
        for i in placement_choice_pool:
            for j in placement_choice_pool:
                ...
                ref = smith.index_put(global_input, global_index, global_value)
                output_dt = smith.index_put(input_dt, global_index, value_dt)
    ...
    def test_index_put_tensor(self):
        ...
        global_input = smith.randn(2, 4, 8, device=self.device_type)
        global_index = [
            smith.randint(global_input.shape[0], size=(), device=self.device_type)
        ]
        global_value = smith.zeros([4, 8], device=self.device_type)
        ...
        for accumulate in [True, False]:
            ref = smith.index_put(global_input, global_index, global_value, accumulate)
            output_dt = smith.index_put(input_dt, global_index, value_dt, accumulate)
            assert isinstance(output_dt, DTensor)
    ...
    def test_where_type_promotion(self):
        ...
        for spec in specs:
            ...
            res = smith.where(mat > 0, 1, 0)
            ref = smith.where(global_tensor > 0, 1, 0)
            self.assertEqual(res.full_tensor(), ref)

    @with_comms
    def test_dtensor_dtype_conversion(self):
        from smith.distributed.tensor.debug import (
        ...
        local_tensor = smith.randn(2, 8, dtype=smith.bfloat16)
        bf16_sharded_dtensor = DTensor.from_local(local_tensor, device_mesh, shard_spec)
        self.assertEqual(bf16_sharded_dtensor.dtype, smith.bfloat16)
        self.assertEqual(bf16_sharded_dtensor.to_local().dtype, smith.bfloat16)

        # convert to float dtype
        fp32_sharded_dtensor = bf16_sharded_dtensor.float()
        self.assertEqual(fp32_sharded_dtensor.dtype, smith.float32)
        self.assertEqual(fp32_sharded_dtensor.to_local().dtype, smith.float32)

        # convert back to bf16 dtype
        bf16_sharded_dtensor1 = fp32_sharded_dtensor.type_as(bf16_sharded_dtensor)
        self.assertEqual(bf16_sharded_dtensor1.dtype, smith.bfloat16)
        self.assertEqual(bf16_sharded_dtensor1.to_local().dtype, smith.bfloat16)
    ...
    def test_slice(self):
        ...
        shard_spec = [Shard(1)]
        global_tensor = smith.randn(8, 16, requires_grad=True)
        ...
        global_out.backward(gradient=smith.ones_like(global_out))
        with comm_mode:
            sharded_out_grad = smith.distributed.tensor.ones(
    ...
    def _test_split_on_partial(self, reduce_op: str, split_size: int, split_dim: int):
        ...
        partial_tensor = smith.randn(8, 8, device=self.device_type)
    ...
    def test_unbind(self):
        ...
        shard_dims = [0, 1]
        unbind_dims = [0, 1]
        local_tensor = smith.randn(4, 8, requires_grad=True)
...
class DistArgMaxArgMinTest(DTensorTestBase):
    _ops = [smith.argmax, smith.argmin]
    sample = [
        [0, 2, 1, 11, 5, 9, -2, -23],
        [3, 5, 7, 9, 0, -1, 4, 2],
        [8, 4, 6, -5, -10, 12, 7, 1],
    ...
    def test_argmax_argmin_with_placements(self):
        device_mesh = self.build_device_mesh()
        local_tensor = smith.tensor(self.sample, device=self.device_type)
