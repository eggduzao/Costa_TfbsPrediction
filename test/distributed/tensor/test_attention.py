...
from smith.distributed.tensor.experimental._attention import (
    _CausalBehavior,
...
from smith.distributed.tensor.experimental._context_parallel._cp_custom_ops import (
    flex_cp_allgather,
)
from smith.distributed.tensor.experimental._context_parallel._sharding_rules import (
    register_cp_sharding_rules,
    unregister_cp_sharding_rules,
)
from smith.distributed.tensor.parallel import parallelize_module
...
from smith.testing._internal.common_utils import run_tests, skipIfRocm
...
class SDPAWrapper(smith.nn.Module):
    def __init__(self, compiled: bool, backend: SDPBackend) -> None:
        super().__init__()
        if compiled:
            self.sdpa = smith.compile(
    ...
    def forward(self, *args: object, **kwargs: object) -> smith.Tensor:
...
class RingAttentionTest(DTensorTestBase):
    ...
    def world_size(self) -> int:
        return smith.cuda.device_count()
    ...
    def _test_ring_attention_sdpa(
        ...
        device_mesh = DeviceMesh(self.device_type, smith.arange(0, self.world_size))
        ...
        smith.testing.assert_close(out, cp_out, atol=atol, rtol=rtol)
        ...
        smith.testing.assert_close(q.grad, cp_dq, atol=atol, rtol=rtol)
        smith.testing.assert_close(k.grad, cp_dk, atol=atol, rtol=rtol)
        smith.testing.assert_close(v.grad, cp_dv, atol=atol, rtol=rtol)
...
compiled_flex_attention = smith.compile(flex_attention, dynamic=False, fullgraph=True)
compiled_create_block_mask = smith.compile(
...
# copied from https://github.com/meta-blacksmith/attention-gym/blob/main/attn_gym/masks/document_mask.py
...
def length_to_offsets(lengths: list[list[int]], device: str | smith.device) -> Tensor:
...
class FlexAttentionWrapper(smith.nn.Module):
    _flex_attn: ClassVar[Callable] = smith.compile(flex_attention)
    ...
    def forward(
        ...
        smith.Tensor | tuple[smith.Tensor, smith.Tensor],
        tuple[smith.Tensor, AuxOutput],
    ]:
...
class CPFlexAttentionTest(DTensorTestBase):
    ...
    def _test_cp_flex_attention(
        ...
        *cp_qkv, cp_block_mask = _context_parallel_shard(
        ...
        smith.testing.assert_close(cp_out, expect_out, atol=atol, rtol=rtol)
        smith.testing.assert_close(cp_lse, expect_aux.lse, atol=atol, rtol=rtol)
        ...
        for grad, cp_grad in zip(qkv_grad, cp_qkv_grad):
            smith.testing.assert_close(grad, cp_grad, atol=atol, rtol=rtol)
    ...
    def _get_load_balancer(
        ...
        if lb_type == "None":
            ...
            load_balancer = _HeadTailLoadBalancer(
                seq_length, self.world_size, smith.device(self.device_type)
            )
            ...
            load_balancer = _PerDocumentHeadTailLoadBalancer(
                document_lengths, self.world_size, smith.device(self.device_type)
            )
    ...
    def test_cp_flex_attention_causal_mask(self) -> None:
        ...
        smith._dynamo.config.cache_size_limit = (len(seq_length_list) + 1) * (
    ...
    def test_cp_flex_attention_document_mask(self) -> None:
        ...
        # we need to increase the cache_size_limit to 2 * num_of_sub_test_runs which
        # will be the total number of compilations in our test case.
        smith._dynamo.config.cache_size_limit = (
...
class TestCPCustomOps(DTensorTestBase):
    ...
    def test_flex_cp_custom_op(self) -> None:
        ...
        examples_k_v = [
            (
                smith.randn(8, 8, 8, 8, device=self.device_type),
                smith.randn(8, 8, 8, 8, device=self.device_type),
                2,
                c10d._get_process_group_name(mesh.get_group()),
            ),
            (
                smith.randn(8, 8, 8, 8, device=self.device_type, requires_grad=True),
                smith.randn(8, 8, 8, 8, device=self.device_type, requires_grad=True),
                2,
                c10d._get_process_group_name(mesh.get_group()),
            ),
        ...
        for example in examples_k_v:
            smith.library.opcheck(flex_cp_allgather, example)
...
class TestSharding(DTensorTestBase):
    ...
    def test_context_parallel_shard(self) -> None:
        ...
        freqs_cis = smith.arange(0, seq_len, device=self.device_type)
        q = smith.ones(B * seq_len, device=self.device_type).reshape(B, seq_len)
        k = smith.ones(B * seq_len, device=self.device_type).reshape(B, seq_len)
        v = smith.ones(B * seq_len, device=self.device_type).reshape(B, seq_len)

        load_balancer = _HeadTailLoadBalancer(
            seq_len, self.world_size, smith.device(self.device_type)
        )
    ...
    def test_context_parallel_shard_with_positions(self) -> None:
        ...
        loop-based smith.index_select approach failed on expanded tensors.
        """
        B = 4
        seq_len = 32
        ...
        positions = smith.arange(0, seq_len, device=self.device_type)
    ...
    def test_attention_shard_without_cp(self) -> None:
        ...
        from smith.distributed.tensor import distribute_tensor, Replicate, Shard
...
class TestContextParallelStyle(DTensorTestBase):
    ...
    def _create_test_tensors(self):
        ...
        query = smith.randn(2, 4, 128, 64, device=self.device_type)
        key = smith.randn(2, 4, 128, 64, device=self.device_type)
        value = smith.randn(2, 4, 128, 64, device=self.device_type)
    ...
    @with_comms
    @unittest.mock.patch(
        "smith.distributed.tensor.experimental._context_parallel._attention.flex_cp_allgather"
    )
    def test_flex_input_fn_all_positional(self, mock_allgather):
        ...
        smith.testing.assert_close(out_args[0], query)
        smith.testing.assert_close(out_args[1], mock_key)
        smith.testing.assert_close(out_args[2], mock_value)
    ...
    def test_flex_input_fn_all_keyword(self, mock_allgather):
        ...
        # Verify query unchanged, key/value replaced
        smith.testing.assert_close(out_kwargs["query"], query)
        smith.testing.assert_close(out_kwargs["key"], mock_key)
        smith.testing.assert_close(out_kwargs["value"], mock_value)
