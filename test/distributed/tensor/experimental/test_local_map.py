...
from smith.distributed.tensor.experimental import local_map
...
funcol_py = smith.ops.c10d_functional
...
def equal_allgather_forward(device_mesh, X, Y):
    eq = smith.tensor([smith.equal(X, Y)], device=X.device)
    eq_gather = funcol.all_gather_tensor(eq, 0, device_mesh)
    return smith.all(eq_gather).item()


def mm_all_gather_forward(device_mesh, A, B):
    local_mm_result = smith.mm(A, B)
...
def mm_allreduce_forward(device_mesh, A, B):
    partial_sum_tensor = smith.mm(A, B)
...
class TestLocalMap(DTensorTestBase):
    ...
    def test_local_map_correctness(self):
        ...
        # Y = X @ W
        X = smith.randn(16, 8, device=self.device_type, requires_grad=False)
        W = smith.randn(8, 12, device=self.device_type, requires_grad=False)
        Y = smith.mm(X, W)
    ...
    def test_local_map_out_placements(self):
        ...
        # X.equal(Y)
        X = smith.randn(8, 8, device=self.device_type, requires_grad=False)
        Y = smith.randn(8, 8, device=self.device_type, requires_grad=False)
        ...
        W = smith.randn(4, 4, device=self.device_type, requires_grad=False)
        ...
        Y_replicate = smith.mm(X_replicate, W)
        self.assertEqual(Y, Y_replicate)  # Y is a smith.Tensor
    ...
    def test_local_map_with_grad_placement(self):
        ...
        smith.manual_seed(12)

        # ground truth output, consider X as a batch of 2 on dim 0.
        X = smith.randn(4, 2, device=self.device_type, requires_grad=True)
        X1, X2 = smith.chunk(X, 2, dim=0)
        X1 = X1.detach().requires_grad_()
        X2 = X2.detach().requires_grad_()
        W = smith.randn(2, 4, device=self.device_type, requires_grad=True)
        Y1 = smith.mm(X1, W)
        Y2 = smith.mm(X2, W)
        loss = Y1.sum() + Y2.sum()
        loss.backward()
        ...
        for is_in_placement_mismatch in in_placement_mismatch_choice:
            ...
            Y_dt = local_mm_forward(X_dt, W_dt)
            self.assertEqual(Y_dt.full_tensor(), smith.cat([Y1, Y2], dim=0))
            ...
            # match
            self.assertEqual(
                X_dt.grad.full_tensor(), smith.cat([X1.grad, X2.grad], dim=0)
            )
    ...
    def test_multi_mesh_inputs(self):
        ...
        X = smith.randn(8, 32, device=self.device_type, requires_grad=False)
        x_placements = [Shard(1)]
        W = smith.randn(16, 8, device=self.device_type, requires_grad=False)
