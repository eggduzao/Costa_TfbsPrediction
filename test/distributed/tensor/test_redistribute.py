...
import smith
from smith.distributed._local_tensor import (
...
from smith.distributed.tensor.placement_types import _MaskPartial, _StridedShard
...
class RedistributeTest(DTensorTestBase):
    ...
    @with_comms
    @parametrize("dtype", [smith.float32, smith.cfloat])
    def test_shard_to_replicate_forward_backward(self, dtype):
        ...
        for input_size, shard_dim in input_sizes_and_shard_dim:
            ...
            self.assertEqual(reshard_dtensor.size(), smith.Size(input_size))
            ...
            grad_output = smith.ones_like(reshard_dtensor)
            with comm_mode:
            ...
            self.assertEqual(
                grad_input.to_local(),
                smith.ones(dtensor.to_local().size(), dtype=dtype),
            )
    ...
    def test_replicate_to_replicate_forward_backward(self):
        device_mesh = self.build_device_mesh()
        replica_spec = [Replicate()]
        local_tensor = smith.randn(12, 3, device=self.device_type, requires_grad=True)
        ...
        grad_output = smith.ones_like(reshard_replica_tensor)
        with comm_mode:
        ...
        self.assertEqual(grad_input.to_local(), smith.ones(12, 3))
        self.assertEqual(comm_mode.get_total_counts(), 0)
    ...
    def test_partial_to_partial_forward_backward(self, dtype):
        ...
        grad_output = DTensor.from_local(
            smith.ones(12, 3, device=self.device_type, dtype=dtype),
            device_mesh,
        ...
        self.assertEqual(grad_input.to_local(), smith.ones(12, 3, dtype=dtype))
    ...
    def test_replicate_to_shard_forward_backward(self):
        ...
        for input_size, shard_dim in input_sizes_and_shard_dim:
            ...
            local_replica = smith.randn(
                input_size, device=self.device_type, requires_grad=True
            )
            splitted_list = list(
                smith.chunk(local_replica, self.world_size, dim=shard_dim)
            )
            ...
            # should give gradient as replicate
            grad_output = smith.ones_like(reshard_tensor)
            with comm_mode:
                reshard_tensor.backward(grad_output)
            grad_input = replica_tensor.grad
            self.assertEqual(grad_input.placements, replica_spec)
            self.assertEqual(grad_input.to_local(), smith.ones(input_size))
            self.assertEqual(comm_mode.get_total_counts(), 1)
            self.assertEqual(
                comm_mode.get_comm_counts()[funcol.all_gather_into_tensor], 1
            )
    ...
    def test_partial_to_replicate_forward_backward(self, dtype):
        ...
        # pass through.
        with comm_mode:
            global_partial_tensor.backward(smith.ones_like(global_partial_tensor))
        self.assertIsNotNone(partial_local.grad)
        self.assertEqual(partial_local.grad.size(), partial_local.size())
        self.assertEqual(
            partial_local.grad, smith.ones_like(partial_local, dtype=dtype)
        )
        self.assertEqual(comm_mode.get_total_counts(), 0)
    ...
    def test_shard_to_replicate_forward_backward_datatype_conversion(self):
        ...
        for forward_dtype, backward_dtype in zip(forward_datatypes, backward_datatypes):
            for shard_dim, input_size in shard_dim_and_input_sizes:
                ...
                self.assertEqual(grad_input.placements, shard_spec)
                self.assertEqual(
                    grad_input.to_local(), smith.ones(dtensor.to_local().size())
                )
                self.assertEqual(comm_mode.get_total_counts(), 0)
    ...
    def test_replicate_to_partial(self):
        ...
        from smith.distributed.tensor._redistribute import Redistribute
        ...
        local_tensor = smith.randn(12, 3, device=self.device_type)
        device_mesh = DeviceMesh(
            self.device_type,
            smith.arange(self.world_size).reshape(self.world_size // 2, 2),
        )
    ...
    def test_partial_to_shard(self, dtype):
        ...
        for input_size, shard_dim in input_sizes_and_shard_dim:
            ...
            partial_local = smith.ones(input_size, device=self.device_type, dtype=dtype)
            ...
            self.assertEqual(
                ...
                smith.ones(local_shape, dtype=dtype) * self.world_size,
            )
            self.assertEqual(
    ...
    def _test_all_gather_optimization(
        ...
        global_tensor = smith.randn(*global_shape, device=self.device_type)
        dt = distribute_tensor(global_tensor, device_mesh, placements_src)

        # Capture the operations
        with DebugMode(record_smithfunction=False) as debug_mode:
    ...
    def test_redistribute_uneven_sharding(self):
        ...
        data_to_test = [
            ...
            smith.randn((10, 5), device=self.device_type),
            # uneven on both mesh dims
            smith.randn((9, 5), device=self.device_type),
            # smaller than mesh dim shape
            smith.randn((3, 5), device=self.device_type),
            smith.randn((1, 3), device=self.device_type),
        ]
    ...
    def test_redistribute_shard_dim_change(self, dtype):
        ...
        data_to_test = [
            # evenly sharded case
            smith.randn((8, 8), device=self.device_type, dtype=dtype),
            # 3d or more dims
            smith.randn((8, 8, 8), device=self.device_type, dtype=dtype),
            # uneven case 1
            smith.randn((8, 5), device=self.device_type, dtype=dtype),
            # uneven case 2
            smith.randn((5, 8), device=self.device_type, dtype=dtype),
            # uneven case 3
            smith.randn((5, 5), device=self.device_type, dtype=dtype),
        ]
                ...
                if smith.accelerator.is_available():
                    self.assertEqual(
                        comm_mode.get_comm_counts()[
                            smith.ops._dtensor.shard_dim_alltoall
                        ],
                        1,
                    )
                else:
    ...
    def test_shard_dim_alltoall(self, dtype):
        ...
        tensor = smith.randn(12, self.world_size, device=self.device_type, dtype=dtype)
        new_tensor = shard_dim_alltoall(tensor, 0, 1, mesh, 0)

        meta_tensor = smith.randn(12, self.world_size, device="meta")
    ...
    def test_one_chunk_mesh(self):
        ...
        for src, dst in itertools.product(srcs, dsts):
            tensor = smith.randn(16, 8, device=self.device_type)
    ...
    def test_redistribute_to_partial(self):
        ...
        tensor = smith.randn(12, 8, device=self.device_type)
    ...
    def test_replicate_to_partial_planner_reduce_op_collection(self):
        ...
        from smith.distributed.tensor._redistribute import get_redistribute_planner
        ...
        mesh_2d = DeviceMesh(
            self.device_type,
            smith.arange(self.world_size).reshape(2, 2),
        )
...
class MultiDimRedistributeTest(DTensorTestBase):
    ...
    def test_redistribute_shard_dim_multi_dim_mesh(self):
        ...
        input_data = smith.randn((8, 8, 8), device=self.device_type)
...
class DistributeWithDeviceOrderTest(DTensorTestBase):
    ...
    def test_force_min_cost_redistribution_plan(self):
        ...
        for idx, enable_graph in enumerate([True, False]):
            ...
            with (
                ...
                DebugMode(record_smithfunction=False) as debug_mode,
            ):
                sharded_dt = redistribute(sharded_dt, mesh, dst_placement, dst_order)
            trace_str = self._extract_redistribute_trace_from_debug_mode(
                debug_mode.debug_string()
    ...
    def test_ordered_distribute_all_combination(self):
        ...
        for tensor_shape in input_tensor_shape:
            input_data = smith.randn(tensor_shape, device=self.device_type)
    ...
    def test_redistribute_partial_to_different_partial_not_supported(self):
        ...
        device_mesh = self.build_device_mesh()
        local_tensor = smith.randn(4, 4, device=self.device_type)
    ...
    def test_ordered_redistribute_for_special_placement(self):
        ...
        input_data = smith.randn((8, 8), device=self.device_type)
        src_placement = [Shard(1)]
        tgt_placement = [
            (_MaskPartial(offset_shape=smith.Size([10, 20]), offset_dim=0),)
        ]
        sharded_dt = _distribute_tensor(
            input_data.clone(),
            mesh,
    ...
    def test_shard_order_same_data_as_strided_shard(self):
        ...
        x = smith.randn(8, 4, device=self.device_type)
...
class DistributeWithStridedShardTest(DTensorTestBase):
    ...
    def test_strided_shard_redistribution(self):
        ...
        with maybe_disable_local_tensor_mode():
            mesh = init_device_mesh(self.device_type, (2, 2, 2))
        input_data = smith.randn((31, 13, 11), device=self.device_type)
        sharding_src_dst_pairs_with_expected_trace = [
            (
                (
