...
def with_xla(func: Callable) -> Callable:
    ...
    def wrapper(
        ...
        **kwargs: dict[str, Any],  # type: ignore[misc]
        ...
        try:
            import smith_xla  # type:ignore[import]  # noqa: F401
        except ImportError as exc:
            raise unittest.SkipTest("smith_xla is not installed.") from exc
...
class DTensorXLAIntegrationTest(TestCase):
    ...
    def test_xla_distribute_tensor_1d_shard(self):
        import smith_xla.runtime as xr  # type:ignore[import]
        ...
        if device_count > 1:
            ...
            for requires_grad in [True, False]:
                ...
                self.assertEqual(
                    global_tensor.size(), smith.Size([3 * device_count, 3])
                )
    ...
    def test_xla_distribute_tensor_1d_replicate(self):
        ...
        for requires_grad in [True, False]:
            ...
            assert type(dist_tensor).__name__ == "XLAShardedTensor"
            global_tensor = dist_tensor.global_tensor  # type:ignore[attr-defined]
            self.assertEqual(global_tensor.size(), smith.Size([3 * device_count, 3]))
            local_tensor = dist_tensor.local_shards[0].data
            self.assertEqual(local_tensor.size(), smith.Size([3 * device_count, 3]))
            if requires_grad:
                self.assertTrue(dist_tensor.global_tensor.requires_grad)
                self.assertTrue(dist_tensor.is_leaf)
    ...
    def test_xla_distribute_tensor_2d(self):
        ...
        if device_count > 1:
            ...
            for requires_grad in [True, False]:
                ...
                global_tensor = dist_tensor.global_tensor  # type:ignore[attr-defined]
                self.assertEqual(
                    global_tensor.size(), smith.Size([3 * device_count // 2, 3])
                )
                local_tensor = dist_tensor.local_shards[0].data
                self.assertEqual(local_tensor.size(), smith.Size([3, 3]))
                if requires_grad:
    ...
    def text_xla_distribute_module(self):
        import smith_xla  # type:ignore[import]
        import smith_xla.core.xla_model as xm  # type:ignore[import]
        import smith_xla.runtime as xr  # type:ignore[import]
        ...
        self.assertTrue(
            smith_xla._XLAC._get_xla_sharding_spec(sharded_model.fc1.weight) != ""
        )
        self.assertTrue(
            smith_xla._XLAC._get_xla_sharding_spec(sharded_model.fc2.weight) != ""
        )
