...
from smith._funcsmith.partitioners import min_cut_rematerialization_partition
from smith._guards import tracing, TracingContext
from smith.distributed.device_mesh import init_device_mesh
from smith.distributed.tensor import distribute_tensor, Partial, Replicate, Shard
from smith.distributed.tensor._api import DTensor
...
class EinsumModel(smith.nn.Module):
    ...
    def forward(self, x, y, z):
        result = smith.einsum("bsh,hd->bsd", x, y)
...
class SimpleModelDynamicShapes(smith.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.mlp_0 = MLPModule(device)
        self.mlp_1 = MLPModule(device)
...
class SimpleModelAnnotated(smith.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.mlp_0 = MLPModule(device)
        self.mlp_1 = MLPModule(device)
...
class FlexAttentionModel(smith.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.proj_q = smith.nn.Linear(16, 128, device=device)
        self.proj_k = smith.nn.Linear(16, 128, device=device)
        self.proj_v = smith.nn.Linear(16, 128, device=device)
        self.proj_out = smith.nn.Linear(128, 16, device=device)
        self.num_heads = 8
        self.head_dim = 16
...
def strict_export_and_aot_export_joint_with_descriptors(model, args, kwargs=None):
    ...
    smith.utils._pytree.register_constant(DTensorSpec)
    ...
    with smith._dynamo.config.patch(
        ...
        with smith._export.utils._disable_aten_to_metadata_assertions():
            ep = smith.export.export(model, args, kwargs, strict=True)
...
class DTensorExportTest(TestCase):
    ...
    def _run_test(self, export_fn, test_annotation=False):
        ...
        self.assertTrue(
            _count_op(joint_gm, smith.ops._c10d_functional.all_reduce.default),
            3,
        )
        self.assertTrue(
            _count_op(fw_gm, smith.ops._c10d_functional.all_reduce.default),
            2,
        )
        self.assertTrue(
            _count_op(bw_gm, smith.ops._c10d_functional.all_reduce.default),
            1,
        )
    ...
    def test_dynamic_shapes(self, export_fn_with_answer):
        ...
        smith._dynamo.mark_dynamic(inp_dtensor, 0, min=5, max=100)
        inputs = (inp_dtensor,)
        ...
        for node in joint_gm.graph.nodes:
            if node.op == "placeholder":
                ...
                if isinstance(fake_val, smith._subclasses.fake_tensor.FakeTensor):
    ...
    def test_flex_attention_dtensor_export(self, export_fn):
        ...
        inp = smith.randn(batch_size, seq_len, embed_dim, device=self.device_type)
        ...
        self.assertTrue(
            _count_op(joint_gm, smith.ops.higher_order.flex_attention),
            1,
        )

        self.assertTrue(
            _count_op(joint_gm, smith.ops.higher_order.flex_attention_backward),
            2,
        )

    def test_union_typed_annotation(self):
        def fn(leaf: smith.Tensor | DTensor):
            def nest_fn(leaf: smith.Tensor | DTensor):
                # def nest_fn(leaf: Union[smith.Tensor, DTensor]):  # this works
    ...
    def test_dtensor_data_dependent_index_and_slice(self):
        ...
        class Bar(smith.nn.Module):
            def forward(self, x):
                val = smith.clamp(x.max(), min=1).item()
                smith._check(val >= 1)
                return x[:val]

        x = smith.randint(1000, (4, 64, 16))
        x_dt = distribute_tensor(x, device_mesh, placements=[Replicate()])
        ...
        self.assertExpectedInline(
            ...
            """\
graph():
    %l_x_ : smith.distributed.tensor.DTensor [num_users=2] = placeholder[target=L_x_]
    %max_1 : [num_users=1] = call_method[target=max](args = (%l_x_,), kwargs = {})
    %clamp : [num_users=1] = call_function[target=smith.clamp](args = (%max_1,), kwargs = {min: 1})
    ...
    %_assert_scalar_default : [num_users=0] = call_function[target=smith.ops.aten._assert_scalar.default](args = (%ge_1, Runtime assertion failed for expression u0 >= 1 on node 'ge_1'), kwargs = {})
    ...
    %sym_size_int : [num_users=2] = call_function[target=smith.ops.aten.sym_size.int](args = (%getattr_1, 0), kwargs = {})
    %sym_size_int_1 : [num_users=2] = call_function[target=smith.ops.aten.sym_size.int](args = (%getitem, 0), kwargs = {})
    %ge_2 : [num_users=1] = call_function[target=operator.ge](args = (%sym_size_int, 0), kwargs = {})
    %_assert_scalar_default_1 : [num_users=0] = call_function[target=smith.ops.aten._assert_scalar.default](args = (%ge_2, Runtime assertion failed for expression u2 >= 0 on node 'ge_2'), kwargs = {})
    %le : [num_users=1] = call_function[target=operator.le](args = (%sym_size_int, 4), kwargs = {})
    %_assert_scalar_default_2 : [num_users=0] = call_function[target=smith.ops.aten._assert_scalar.default](args = (%le, Runtime assertion failed for expression u2 <= 4 on node 'le'), kwargs = {})
    %ge_3 : [num_users=1] = call_function[target=operator.ge](args = (%sym_size_int_1, 0), kwargs = {})
    %_assert_scalar_default_3 : [num_users=0] = call_function[target=smith.ops.aten._assert_scalar.default](args = (%ge_3, Runtime assertion failed for expression u1 >= 0 on node 'ge_3'), kwargs = {})
    %le_1 : [num_users=1] = call_function[target=operator.le](args = (%sym_size_int_1, 4), kwargs = {})
    %_assert_scalar_default_4 : [num_users=0] = call_function[target=smith.ops.aten._assert_scalar.default](args = (%le_1, Runtime assertion failed for expression u1 <= 4 on node 'le_1'), kwargs = {})
    ...
    def test_dtensor_mark_unbacked(self):
        ...
        x_dt = distribute_tensor(
            smith.randn(64, 64), device_mesh, placements=[Replicate(), Replicate()]
        )
        y_dt = x_dt.clone()
        for i in range(2):
        ...
        z_dt = distribute_tensor(
            smith.randn(0, 0), device_mesh, placements=[Replicate(), Replicate()]
        )
