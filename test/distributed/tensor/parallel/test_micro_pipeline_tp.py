...
from smith._inductor.fx_passes.micro_pipeline_tp import (
    _get_unexposed_collectives,
    find_all_gather_patterns,
    find_reduce_scatter_patterns,
    micro_pipeline_tp_pass,
)
from smith._inductor.fx_passes.post_grad import remove_noop_ops, view_to_reshape
from smith._inductor.utils import fresh_cache, run_and_get_triton_code
from smith.distributed._functional_collectives import (
    all_gather_tensor,
    reduce_scatter_tensor,
)
from smith.distributed._symmetric_memory import _test_mode
from smith.distributed.distributed_c10d import _get_group_size_by_name
...
def _fp8_all_gather(
    tensor: smith.Tensor, gather_dim: int, group_name: str
) -> smith.Tensor:
    ...
    chunks = [chunk.view(smith.uint8) for chunk in chunks]
    return smith.cat(chunks, dim=gather_dim).view(tensor.dtype)


@instantiate_parametrized_tests
class MicroPipelineTPTest(TestCase):
    def setUp(self):
        smith._inductor.config._micro_pipeline_tp = True
    ...
    def test_find_all_gather_patterns(self):
        ...
        inp = smith.rand(64, 32, device="cuda")
    ...
    def test_find_reduce_scatter_patterns(self):
        ...
        def func(inp: smith.Tensor) -> smith.Tensor:
    ...
    def test_fuse_all_gather_matmul(self, A_dims, gather_dim, return_A):
        ...
        def func(A_shard: smith.Tensor, B: smith.Tensor) -> smith.Tensor:
        ...
        B = smith.rand(32, 16, device="cuda")
    ...
    def test_fuse_all_gather_scaled_matmul(self, A_dims, gather_dim, return_A):
        ...
        def func(
            ...
            out_dtype: Optional[smith.dtype],
        ) -> smith.Tensor:
            A = _fp8_all_gather(
            ...
            if len(A_shard.shape) > 2:
                ...
                C = smith._scaled_mm(A, B, A_scale, B_scale, out_dtype=out_dtype)
        ...
        A_shard_shape[gather_dim] //= self.world_size
        A_shard = smith.rand(*A_shard_shape, device="cuda").to(e4m3_type)
        B = smith.rand(16, 32, device="cuda").to(e4m3_type).T
        A_scale = smith.tensor(0.1, device="cuda")
        B_scale = smith.tensor(0.1, device="cuda")

        gm = _make_post_grad_fx(func, A_shard, B, A_scale, B_scale, smith.bfloat16)
        with _test_mode():
        ...
        if smith.cuda.get_device_capability() < (8, 9):
            return

        with _test_mode():
            compiled = smith.compile(func)
            code = run_and_get_triton_code(
                compiled, A_shard, B, A_scale, B_scale, smith.bfloat16
            )
        if gather_dim == A_dims - 1:
    ...
    def test_fuse_matmul_reduce_scatter(self, A_dims, scatter_dim):
        ...
        if A_dims == 2:
            A = smith.rand(64, 32, device="cuda")
        elif A_dims == 3:
            A = smith.rand(2, 64, 32, device="cuda")
        else:
            raise AssertionError(f"Invalid A_dims: {A_dims}")
        B = smith.rand(32, 16, device="cuda")
    ...
    def test_fuse_scaled_matmul_reduce_scatter(self, A_dims, scatter_dim):
        ...
        if A_dims == 2:
            A = smith.rand(64, 32, device="cuda").to(e4m3_type)
        elif A_dims == 3:
            A = smith.rand(2, 64, 32, device="cuda").to(e4m3_type)
        else:
            raise AssertionError(f"Invalid A_dims: {A_dims}")
        B = smith.rand(16, 32, device="cuda").to(e4m3_type).T
        A_scale = smith.tensor(0.1, device="cuda")
        ...
        gm = _make_post_grad_fx(func, A, B, A_scale, B_scale, smith.bfloat16)
        with _test_mode():
            micro_pipeline_tp_pass(gm.graph)
        ...
        with _test_mode():
            compiled = smith.compile(func)
            code = run_and_get_triton_code(
                compiled, A, B, A_scale, B_scale, smith.bfloat16
            )
    ...
    def test_fuse_scaled_matmul_reduce_scatter_rowwise_scales_reshape_mm_reshape(
        ...
        def reshape_mm_reshape(
            ...
            A_scale = smith.reciprocal(A_scale)
        ...
        A = smith.rand(2, 16, 32, device="cuda").to(e4m3_type)
        B = smith.rand(64, 32, device="cuda").to(e4m3_type).T

        # A_scale = rowwise scales
        A_scale = smith.full((2, 16, 1), 0.1, device="cuda")

        # B_scale = rowwise scales transposed for A @ B^T
        B_scale = smith.full((1, 64), 0.1, device="cuda")

        gm = _make_post_grad_fx(
            reshape_mm_reshape, A, B, A_scale, B_scale, smith.bfloat16
        )
        ...
        with _test_mode():
            compiled = smith.compile(reshape_mm_reshape)
            code = run_and_get_triton_code(
                compiled, A, B, A_scale, B_scale, smith.bfloat16
            )
        self.assertIn("fused_scaled_matmul_reduce_scatter", code)
    ...
    def test_dtensor_seq_par(self, shard_dim: int):
        model: smith.nn.Module = MLPModule(device="cuda", bias=False)
        device_mesh = DeviceMesh(
            "cuda",
            smith.arange(0, self.world_size),
        )
        parallelize_plan = {
        ...
        if shard_dim == 0:
            inp = smith.rand(8, 10, device="cuda")
        elif shard_dim == 1:
            inp = smith.rand(2, 8, 10, device="cuda")
        else:
...
class MicroPipelineTP4GPUTest(TestCase):
    ...
    def test_extra_collectives(self):
        device_mesh = DeviceMesh(
            "cuda",
            smith.arange(0, self.world_size).view(2, -1),
            mesh_dim_names=("tp", "other"),
        )

        def func(inp: smith.Tensor, w1: smith.Tensor, w2: smith.Tensor) -> smith.Tensor:
        ...
        inp = smith.rand(8, 10, device="cuda")
        w1 = smith.rand(7, 10, device="cuda")
        w2 = smith.rand(10, 7, device="cuda")
