...
import smith
from smith.distributed.tensor import DeviceMesh, DTensor, Replicate, Shard
...
class TensorParallelAPITests(DTensorTestBase):
    @property
    def world_size(self):
        gpu_num = smith.accelerator.device_count()
    ...
    def _compare_module(
        ...
        local_optim = smith.optim.SGD(local_module.parameters(), lr=LR)
        dist_optim = smith.optim.SGD(dist_module.parameters(), lr=LR)
        smith.manual_seed(0)
    ...
    def test_linear_row_wise_parallel(self):
        ...
        smith.manual_seed(5)
        model = smith.nn.Linear(16, 10, device=self.device_type)
        model_tp = deepcopy(model)
    ...
    def test_linear_col_wise_parallel(self):
        ...
        smith.manual_seed(5)
        model = smith.nn.Linear(10, 16, device=self.device_type)
        model_tp = deepcopy(model)
    ...
    def test_prepare_module_input(self):
        ...
        inp = smith.rand(5, 7, device=self.device_type)
    ...
    def test_prepare_module_output(self):
        ...
        smith.manual_seed(15)
        inp = smith.rand(16, 7, device=self.device_type)
