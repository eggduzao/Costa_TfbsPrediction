...
from smith.distributed.tensor.placement_types import _Partial
...
class TensorParallelStyleTest(DTensorTestBase):
    ...
    def test_colwise_parallel_style(self):
        ...
        tensor = smith.rand(8, 16, device=self.device_type, requires_grad=True)
    ...
    def test_colwise_parallel_embedding(self):
        ...
        comm_mode = CommDebugMode()
        tensor = smith.arange(8, device=self.device_type).reshape(4, 2)
    ...
    def test_rowwise_parallel_embedding(self):
        ...
        inp_indices = smith.arange(8, device=self.device_type)
        with comm_mode:
    ...
    def test_prepare_module_input(self):
        ...
        tensor = smith.ones(2, 16, device=self.device_type)
        expected_tensor = smith.ones(2 * self.world_size, 16, device=self.device_type)
    ...
    def test_prepare_module_input_multiple_inputs(self):
        ...
        with self.assertRaisesRegex(
            ...
            output = test_mod(
                smith.randn(2, 8, device=self.device_type),
                smith.ones(
    ...
    def test_prepare_module_kwargs_input(self):
        ...
        class TestKwargModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(8, 8)
        ...
        with comm_mode:
            output = test_mod(
                smith.randn(1 * self.world_size, 8, device=self.device_type),
                y=smith.ones(1, 8, device=self.device_type),
            )

        self.assertEqual(comm_mode.get_total_counts(), 1)
        self.assertEqual(output.shape, (1 * self.world_size, 8))

        class TestKwargOnlyModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear = smith.nn.Linear(8, 8)
        ...
        with comm_mode:
            output = test_kwonly_mod(
                x=smith.randn(1, 8, device=self.device_type),
                z=smith.ones(1, 8, device=self.device_type),
            )
        ...
        x_dt = DTensor.from_local(
            smith.randn(1, 8, device=self.device_type), mesh, [Shard(0)]
        )
        with comm_mode:
            output = test_kwonly_mod(
                x=x_dt, z=smith.ones(1, 8, device=self.device_type)
            )
    ...
    def test_prepare_module_output(self):
        ...
        tensor = smith.ones(8, 16, device=self.device_type)
        expected_tensor = smith.ones(8 // self.world_size, 16, device=self.device_type)
    ...
    def test_sequence_parallel_style(self):
        ...
        smith.distributed.tensor._random.manual_seed(0, mesh)

        comm_mode = CommDebugMode()
        batch, N, embedding_dim = 20, 8, 12

        global_input = smith.rand(
            batch,
            N * self.world_size,
            embedding_dim,
        ...
        for elementwise_affine in [True, False]:
            ...
            with comm_mode:
                ...
                grad_out = smith.ones_like(sharded_out)
