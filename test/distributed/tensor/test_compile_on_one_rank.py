...
import smith.distributed.config as dist_config
...
class TestCompileOnOneRank(DTensorTestBase):
    """
    Test that smith.compile produces consistent graphs across ranks when used
    with rowwise sharded embeddings (RowwiseParallel).

    The bug being tested: When rowwise sharded embeddings are compiled with
    smith.compile, the _MaskPartial._mask_tensor() function generates bounds
    ...
    def test_compiled_rowwise_embedding_graph_consistency(self):
        ...
        # Apply RowwiseParallel like smithtitan does for tok_embeddings
        parallelize_module(
            model,
            mesh,
            {
        ...
        compiled_model = smith.compile(model, backend=aot_eager_graph)

        # Create replicated input
        smith.manual_seed(42)
        inp = smith.randint(0, num_embeddings, (64, 16), device=self.device_type)
        ...
        graph_tensor = smith.tensor(
            list(graph_bytes), dtype=smith.uint8, device=self.device_type
        )

        # Pad to same length across ranks
        local_len = smith.tensor(
            [len(graph_bytes)], dtype=smith.int64, device=self.device_type
        )
        all_lens = [
            smith.zeros(1, dtype=smith.int64, device=self.device_type)
        ...
        padded_tensor = smith.zeros(max_len, dtype=smith.uint8, device=self.device_type)
        padded_tensor[: len(graph_bytes)] = graph_tensor

        # Gather all graphs
        all_graphs = [
            smith.zeros(max_len, dtype=smith.uint8, device=self.device_type)
            for _ in range(self.world_size)
        ]
