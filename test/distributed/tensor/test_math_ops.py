...
from smith.distributed.tensor._ops.utils import is_tensor_partial, normalize_dim
...
class DistMathOpsTest(DTensorTestBase):
    ...
    def test_softmax_with_bwd(self):
        ...
        for params in test_list:
            ...
            x = smith.rand(8, 12, 16, device=self.device_type, requires_grad=True)
    ...
    def test_nll_loss_and_cross_entropy(self):
        ...
        for input_ndim, input_size, target_size in test_setup:
            x = smith.rand(*input_size, device=self.device_type, requires_grad=True)
            target = smith.randint(channel_size, target_size, device=self.device_type)
    ...
    def test_shard_math_ops(self):
        ...
        mesh = DeviceMesh(
            self.device_type,
            smith.arange(self.world_size).reshape(*mesh_shape),
        )
        global_tensor = smith.ones(4, 4)
        ...
        # for op in [smith.add, smith.sub, smith.mul, smith.div]:
        for op in [smith.add, smith.sub, smith.mul, smith.div]:
    ...
    def test_layer_norm_fwd(self):
        ...
        # NLP example from blacksmith docs
        # https://blacksmith.org/docs/stable/generated/smith.nn.LayerNorm.html
        batch, sentence_length, embedding_dim = 20, 5, 10
        x = smith.rand(batch, sentence_length, embedding_dim, device=self.device_type)
        norm_shape_idx_list = list(range(x.ndim))
        shard_dims = [-1, 0, 1, 2]
        elementwise_affine_list = [False, True]

        # Test RMSNorm as well if CUDA
        norm_types = [smith.nn.LayerNorm]
        if self.device_type == "cuda" and hasattr(smith.nn, "RMSNorm"):
            norm_types.append(smith.nn.RMSNorm)
        ...
        # normalized shape is a smith.Size object
        for norm_type, shard_dim, norm_idx, elementwise_affine in test_config_list:
            ...
            from smith.distributed.tensor._dtensor_spec import TensorMeta
    ...
    def test_layer_norm_bwd_req_grad(self):
        ...
        for subtest_cfg in subtest_cfgs:
            try:
                ...
                class LnTpBlock(smith.nn.Module):
                    def __init__(self):
                        super().__init__()
                        self.preln_embeddings = smith.nn.Embedding(
                            vocab_size, embedding_dim
                        )
                        self.layer_norm = norm_type(
                            normalized_shape, elementwise_affine=elementwise_affine
                        )
                        self.postln_linear = smith.nn.Linear(
                ...
                x = smith.randint(vocab_size, (batch, seq_len), device=self.device_type)
    ...
    def test_topk(self):
        ...
        tensor = smith.randn(12, 8, 8, requires_grad=True)
    ...
    def test_shard0_svd(self):
        ...
        replicated_x = smith.randn((8, 8), device=self.device_type)
        sharded_x = distribute_tensor(replicated_x, device_mesh, (Shard(0),))
        with CommDebugMode() as comm_mode:
            U, S, V = smith.linalg.svd(sharded_x, full_matrices=False)
        ref_U, ref_S, ref_V = smith.linalg.svd(replicated_x, full_matrices=False)
    ...
    def test_vector_norm(self):
        ...
        grad = smith.randn(12, 8)

        sharded_grad = distribute_tensor(grad, device_mesh, [Shard(0)])

        # non-sharded op
        out = smith.ops.aten.linalg_vector_norm(grad, 2)

        # sharded op
        sharded_out = smith.ops.aten.linalg_vector_norm(sharded_grad, 2)
    ...
    def test_vector_norm_partial(self):
        ...
        local_grad = map_local_for_rank(
            self.rank, lambda rank: smith.tensor([rank, 1], dtype=smith.float32)
        )
        full_grad = smith.tensor([sum(all_ranks), self.world_size], dtype=smith.float32)

        partial_grad = DTensor.from_local(local_grad, device_mesh, [Partial()])

        # full result
        out = smith.ops.aten.linalg_vector_norm(full_grad, 2)

        # partial result
        partial_out = smith.ops.aten.linalg_vector_norm(partial_grad, 2)
    ...
    def test_foreach_norm(self):
        ...
        grad0 = smith.randn(12, 8)
        grad1 = smith.randn(8, 8)

        sharded_grad0 = distribute_tensor(grad0, device_mesh, [Shard(0)])
        sharded_grad1 = distribute_tensor(grad1, device_mesh, [Shard(0)])

        # non-sharded op
        out = smith.ops.aten._foreach_norm([grad0, grad1], 2)

        # sharded op
        sharded_out = smith.ops.aten._foreach_norm([sharded_grad0, sharded_grad1], 2)
    ...
    def test_foreach_norm_partial(self):
        ...
        local_grad1 = map_local_for_rank(
            self.rank, lambda rank: smith.tensor([rank + 1, 2], dtype=smith.float32)
        )

        grad0 = smith.tensor([sum(all_ranks), self.world_size], dtype=smith.float32)
        grad1 = smith.tensor(
            [sum(all_ranks) + self.world_size, 2 * self.world_size], dtype=smith.float32
        )
        ...
        # partial result
        partial_out = smith.ops.aten._foreach_norm([partial_grad0, partial_grad1], 2)
    ...
    def test_powsum_sharded(self):
        ...
        for ord in [1, 2, 3]:
            ...
            sharded_out = smith.ops.aten.linalg__powsum(sharded_grad, ord)
    ...
    def test_vector_norm_special_norms_placement(self):
        ...
        smith.manual_seed(42)
        grad = smith.randn(12, 8).abs() + 0.1  # Ensure positive for proper test
        sharded_grad = distribute_tensor(grad, device_mesh, [Shard(0)])

        # Test inf norm -> Partial("max")
        out_inf = smith.ops.aten.linalg_vector_norm(sharded_grad, float("inf"))
        ...
        out_neginf = smith.ops.aten.linalg_vector_norm(sharded_grad, float("-inf"))
        ...
        out_1 = smith.ops.aten.linalg_vector_norm(sharded_grad, 1)
        ...
        out_0 = smith.ops.aten.linalg_vector_norm(sharded_grad, 0)
    ...
    def test_foreach_powsum_sharded(self):
        ...
        # Test multiple ord values to validate output placements
        for ord in [1, 2, 3]:
            sharded_out = smith.ops.aten._foreach_powsum(
    ...
    def test_foreach_norm_different_mesh(self):
        ...
        out_tuple = smith.ops.aten._foreach_norm([replica_grad0, replica_grad1], 2)
    ...
    def test_foreach_add_different_mesh(self):
        ...
        inp00 = smith.ones(4, 8) * 2
        inp01 = smith.ones(8, 8) * 3
        inp10 = smith.ones(4, 8) * 4
        inp11 = smith.ones(8, 8) * 3
        ...
        out_tuple = smith.ops.aten._foreach_add(
    ...
    def test_foreach_compose(self):
        ...
        device_mesh = self.build_device_mesh()
        local_shards = tuple(smith.randn(4, 8) for _ in range(3))
        dt_inputs = tuple(
            distribute_tensor(shard, device_mesh, [Shard(0)]) for shard in local_shards
        )
        dt_abs = smith._foreach_abs(dt_inputs)
        dt_max = smith._foreach_max(dt_abs)

        abs = smith._foreach_abs(local_shards)
        expected_max = smith._foreach_max(abs)
    ...
    def test_linalg_eigh(self):
        ...
        distance = smith.dist(local_Q @ smith.diag(local_L) @ local_Q.mT, local_A)
        self.assertEqual(distance.item(), 0.0)

    @with_comms
    def test_upsampling(self):
        input = smith.arange(1, 5, dtype=smith.float32).view(1, 1, 2, 2)
        mesh = self.build_device_mesh()
        ...
        upsample_m = [
            smith.nn.UpsamplingBilinear2d(scale_factor=2),
            smith.nn.UpsamplingNearest2d(scale_factor=2),
            smith.nn.Upsample(scale_factor=2, mode="bicubic"),
        ]
        for m in upsample_m:
            result = m(input)
    ...
    def test_cumsum(self):
        mesh = self.build_device_mesh()
        comm_mode = CommDebugMode()
        inp = smith.rand(3, 5, device=self.device_type)
        ...
        for dim in cumsum_dims:
            output = smith.cumsum(inp, dim=dim)
            with comm_mode:
                output_dtensor = smith.cumsum(input_dtensor, dim=dim)
                if dim == shard_dim:
                    self.assertEqual(comm_mode.get_total_counts(), 1)
                    self.assertEqual(
                        comm_mode.get_comm_counts()[funcol.all_gather_into_tensor],
    ...
    def test_conj_complex_dtensor(self):
        ...
        freqs_cis = smith.randn(
            1, 1, dtype=smith.complex64, requires_grad=False, device=self.device_type
        )
    ...
    def test_rotary_embedding_complex_ops(self):
        ...
        def apply_rotary_emb(xq, freqs_cis):
            xq_ = smith.view_as_complex(xq)
            xq_out = smith.view_as_real(xq_ * freqs_cis)
            return xq_out

        xq = smith.randn(1, 1, 2, requires_grad=True, device=self.device_type)
        freqs_cis = smith.randn(
            1, 1, dtype=smith.complex64, requires_grad=False, device=self.device_type
        )
    ...
    def test_logsumexp(self):
        ...
        for dim in logsumexp_dims:
            output = smith.logsumexp(inp, dim=dim)
            with comm_mode:
                output_dtensor = smith.logsumexp(input_dtensor, dim=dim)
                if dim == shard_dim:
                    self.assertEqual(comm_mode.get_total_counts(), 1)
                    self.assertEqual(
                        comm_mode.get_comm_counts()[funcol.all_gather_into_tensor],
    ...
    def test_partial_reduction_ops(self):
        ...
        local_tensor = smith.rand(3, dtype=smith.float32, device=self.device_type)
        dt = DTensor.from_local(
            local_tensor, device_mesh=mesh, placements=[Partial("sum")]
        )
        out_without_redistribute = smith.norm(dt)

        dt = dt.redistribute(dt.device_mesh, placements=[Replicate()])
        out_with_redistribute = smith.norm(dt)
        ...
        out_without_redistribute = smith.max(dt)

        dt = dt.redistribute(dt.device_mesh, placements=[Replicate()])
        out_with_redistribute = smith.max(dt)
        ...
        out_without_redistribute = smith.min(dt)

        dt = dt.redistribute(dt.device_mesh, placements=[Replicate()])
        out_with_redistribute = smith.min(dt)
    ...
    @with_comms
    def test_std(self):
        mesh = DeviceMesh(self.device_type, smith.arange(4).reshape(2, 2))
        rank = self.rank
        comm_mode = CommDebugMode()

        global_tensor = map_local_for_rank(
            rank,
            lambda rank: smith.tensor(
        ...
        with comm_mode:
            res = dt.std(dim=1)
        expected_answer = smith.tensor([9.0, 11.0])
