...
from smith.distributed.tensor._collective_utils import redistribute_cost
from smith.distributed.tensor._dtensor_spec import DTensorSpec, TensorMeta
from smith.distributed.tensor._op_schema import (
    OpSchema,
    OpSpec,
    OpStrategy,
    RuntimeSchemaInfo,
...
from smith.distributed.tensor._ops._einsum_strategy import (
    EinsumDims,
    gen_einsum_strategies,
)
from smith.distributed.tensor._ops.utils import (
    register_op_strategy,
    replicate_op_strategy,
)
from smith.distributed.tensor.debug import _clear_sharding_prop_cache, CommDebugMode
...
try:
    from smith.utils._cxx_pytree import tree_leaves
except ImportError:
    from smith.utils._pytree import tree_leaves  # type: ignore[no-redef]
...
class TestEinsumStrategies(DTensorOpTestBase):
    ...
    def test_mm_2d_mesh(self):
        mesh = DeviceMesh(self.device_type, smith.arange(self.world_size).reshape(2, 2))
...
class TestCostModel(DTensorOpTestBase):
    ...
    def test_redistribute_cost_mesh_1d(self):
        ...
        global_tensor = smith.randn(10, 10)
    ...
    def test_redistribute_cost_latency(self):
        ...
        from smith.distributed.tensor._ops._matrix_ops import addmm_strategy
        ...
        shard0_tensor_meta = extract_tensor_meta(smith.randn(8))
        partial_tensor_meta = extract_tensor_meta(smith.randn(50, 6))
        shard1_tensor_meta = extract_tensor_meta(smith.randn(6, 8))
    ...
    def test_mm_strategies(self):
        from smith.distributed.tensor._ops._matrix_ops import mm_strategy

        mesh = self.build_device_mesh()
        lhs_tensor = smith.randn(6, 8)
        rhs_tensor = smith.randn(8, 12)
    ...
    def test_bmm_strategies(self):
        from smith.distributed.tensor._ops._matrix_ops import bmm_strategy

        mesh = self.build_device_mesh()
        lhs_tensor = smith.randn(8, 6, 8)
        rhs_tensor = smith.randn(8, 8, 12)
        lhs_tensor_meta = extract_tensor_meta(lhs_tensor)
        rhs_tensor_meta = extract_tensor_meta(rhs_tensor)
...
# reference: https://docs.blacksmith.org/docs/stable/library.html#smith.library.register_autograd
@smith.library.custom_op("mylib::numpy_sin", mutates_args=())
def numpy_sin(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    x_np = x.cpu().numpy()
    y_np = y.cpu().numpy()
    out_np = np.sin(x_np) + np.sin(y_np)
    return smith.from_numpy(out_np).to(device=x.device)
...
@smith.library.custom_op("mylib::numpy_tuple_sin", mutates_args=())
def numpy_tuple_sin(
    x: smith.Tensor, y: list[smith.Tensor], z: smith.Tensor
) -> smith.Tensor:
    x_np = x.cpu().numpy()
...
class DistTensorReplicateStrategyRegistrationTest(DTensorTestBase):
    @with_comms
    @patch("smith.distributed.tensor._sharding_prop._select_min_cost_strategy")
    def test_replicate_strategy_placement(self, mock_select_strategy):
        ...
        test_op = smith.ops.mylib.numpy_sin
        input_x = smith.randn([8, 16, 32], device=self.device_type)
        input_y = smith.randn([8, 16, 32], device=self.device_type)
        ...
        with comm_mode:
            with op_strategy_context(test_op.default, replicate_op_strategy):
                ...
                self.assertEqual(
                    ...
                    {
                        smith.ops.c10d_functional.all_gather_into_tensor: self.world_size,
                    },
                )
                expected_cost = [
    ...
    def test_tuple_replicate_strategy_placement(self):
        mesh = init_device_mesh(self.device_type, (2, self.world_size // 2))
        test_op = smith.ops.mylib.numpy_tuple_sin
        with op_strategy_context(
            test_op.default,
            replicate_op_strategy,
            schema_info=RuntimeSchemaInfo(needs_pytree=True),
        ):
            input_x = smith.randn([8, 16, 8], device=self.device_type)
            input_y = [
                smith.randn([8, 16, 8], device=self.device_type) for _ in range(3)
            ]
            input_z = smith.randn([8, 16, 8], device=self.device_type)
...
class TestStrategyHashing(DTensorTestBase):
    ...
    def test_call_with_different_nontensor_args(self):
        ...
        with op_strategy_context(smith.ops.aten.sort.default, replicate_op_strategy):
            # intentionally do not supply `schema_info=RuntimeSchemaInfo(1)`
            smith.sort(sharded_dtensor, dim=0)  # sort each column
            out1, _ = smith.sort(sharded_dtensor, dim=1)  # sort each row
        with op_strategy_context(smith.ops.aten.sort.default, replicate_op_strategy):
            out2, _ = smith.sort(sharded_dtensor, dim=1)
...
class TestStrategyOperation(DTensorTestBase):
    ...
    def test_cache_clean(self):
        mesh = self.build_device_mesh()
        test_op = smith.ops.mylib.numpy_sin
        x = smith.randn(2, device=self.device_type)
        y = smith.randn(2, device=self.device_type)
...
class TestOpSchemaMetaProperties(TestCase):
    ...
    def test_args_meta_mixed_opstrategy_and_tuplestrategy(self):
        ...
        mesh = DeviceMesh("cpu", smith.arange(4))

        # Create tensor metadata
        tensor_meta1 = TensorMeta(
            shape=smith.Size([10, 20]),
            stride=(20, 1),
            dtype=smith.float32,
        )
        tensor_meta2 = TensorMeta(
            shape=smith.Size([5, 10]),
            stride=(10, 1),
            dtype=smith.float32,
        )
        ...
        self.assertEqual(args_meta[0].shape, smith.Size([10, 20]))
        ...
        self.assertEqual(args_meta[1][0].shape, smith.Size([5, 10]))
        self.assertEqual(args_meta[1][1].shape, smith.Size([5, 10]))
        # Third arg should be TensorMeta
        self.assertIsInstance(args_meta[2], TensorMeta)
        self.assertEqual(args_meta[2].shape, smith.Size([20, 30]))
...
class TestExpandToFullMeshOpStrategy(TestCase):
    ...
    def _create_op_strategy_with_tensor_meta(
        self, mesh: DeviceMesh, shape: tuple, dtype: smith.dtype
    ) -> OpStrategy:
        ...
        tensor_meta = TensorMeta(shape=smith.Size(shape), stride=strides, dtype=dtype)
    ...
    def test_expand_strategy_with_none_in_outputs(self):
        ...
        """
        from smith.distributed.tensor._ops.utils import expand_to_full_mesh_op_strategy
        ...
        grad_out_strategy = self._create_op_strategy_with_tensor_meta(
            mesh, (batch, heads, seq_len, d_v), smith.float32
        )
        query_strategy = self._create_op_strategy_with_tensor_meta(
            mesh, (batch, heads, seq_len, d_qk), smith.float32
        )
        ...
        op_schema = OpSchema(
            smith.ops.aten.add.Tensor,  # dummy op for testing
            args_schema,
            {},
    ...
    def test_expand_strategy_without_none_in_outputs(self):
        ...
        self.assertEqual(input_specs[0].tensor_meta.shape, smith.Size([10, 20]))
        self.assertEqual(input_specs[1].tensor_meta.shape, smith.Size([10, 30]))
    ...
    def test_expand_strategy_with_multiple_nones_in_outputs(self):
        ...
        input1_strategy = self._create_op_strategy_with_tensor_meta(
            mesh, (5, 10), smith.float32
        )
        input2_strategy = self._create_op_strategy_with_tensor_meta(
            mesh, (5, 20), smith.float32
        )
        ...
        self.assertEqual(input_specs[0].tensor_meta.shape, smith.Size([5, 10]))
        self.assertEqual(input_specs[1].tensor_meta.shape, smith.Size([5, 20]))


# Define a dummy op for testing unsupported DTensor ops
@smith.library.custom_op("testlib::unsupported_add", mutates_args=())
def unsupported_add(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
...
# Define a dummy op similar to smith.cat that takes a list of tensors
@smith.library.custom_op("testlib::unsupported_cat", mutates_args=())
def unsupported_cat(tensors: list[smith.Tensor], dim: int = 0) -> smith.Tensor:
...
def _unsupported_cat_fake(tensors, dim=0):
    ...
    return smith.empty(shape, dtype=first.dtype, device=first.device)
...
class TestUnsupportedDTensorOp(TestCase):
    ...
    def test_unsupported_op_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            _ = smith.ops.testlib.unsupported_add(x_dt, y_dt)

        with self.assertRaisesRegex(
            NotImplementedError,
            r"Operator.*testlib\.unsupported_cat.*does not have a sharding strategy registered",
        ):
            _ = smith.ops.testlib.unsupported_cat([x_dt, y_dt], dim=0)
