...
from smith.distributed.tensor import DeviceMesh, DTensor, Replicate, Shard, zeros
...
class DTensorInitOpsTest(DTensorTestBase):
    ...
    def test_init_ops(self):
        ...
        self._run_init_op(smith.nn.init.constant_, 2.4)
...
class DTensorConstructorTest(DTensorTestBase):
    ...
    def test_empty(self):
        self._run_init_op(
            smith.empty,
            smith.distributed.tensor.empty,
    ...
    def test_zeros(self):
        self._run_init_op(
            smith.zeros,
            smith.distributed.tensor.zeros,
            self.assertEqual,
            requires_grad=True,
        )
    ...
    def test_zeros_full_mesh(self):
        ...
        self.assertEqual(dist_tensor.size(), smith.Size(size))
        local_tensor = dist_tensor.to_local()
        self.assertEqual(local_tensor.size(), smith.Size([8, 3]))

        local_tensor = smith.zeros(8, 3)
        ...
        def check_per_rank_tensors(rank, local_tensor):
            if rank <= 2:
                self.assertEqual(local_tensor.size(), smith.Size([8, 3]))
                self.assertEqual(smith.zeros(8, 3), local_tensor)
            else:
                self.assertEqual(local_tensor.size(), smith.Size([7, 3]))
                self.assertEqual(smith.zeros(7, 3), local_tensor)
        ...
        self.assertEqual(dist_tensor.size(), smith.Size(size))
        local_tensor = dist_tensor.to_local()
        self.assertEqual(local_tensor.size(), smith.Size([16, 4]))
        self.assertEqual(local_tensor, smith.zeros([16, 4]))
        ...
        self.assertEqual(dist_tensor.size(), smith.Size(size))
        local_tensor = dist_tensor.to_local()
        self.assertEqual(local_tensor.size(), smith.Size([16, 2]))
        self.assertEqual(local_tensor, smith.zeros([16, 2]))
        ...
        if self.rank == 0:
            ...
            self.assertEqual(local_tensor, smith.zeros([16, 1]))
        elif self.rank == 2:
            self.assertEqual(local_tensor, smith.zeros([15, 2]))
        elif self.rank == 3:
            self.assertEqual(local_tensor, smith.zeros([15, 1]))
    ...
    def test_zeros_submesh(self):
        ...
        if self.rank in sub_mesh_list:
            self.assertEqual(local_tensor.size(), smith.Size([16, 3]))
            self.assertEqual(local_tensor, smith.zeros([16, 3]))
        else:
            self.assertEqual(local_tensor.size(), smith.Size([0]))
            self.assertEqual(local_tensor, smith.zeros(0))
        ...
        if self.rank in sub_mesh_list:
            if self.rank != 3:
                self.assertEqual(local_tensor.size(), smith.Size([11, 3]))
                self.assertEqual(local_tensor, smith.zeros([11, 3]))
            else:
                self.assertEqual(local_tensor.size(), smith.Size([10, 3]))
                self.assertEqual(local_tensor, smith.zeros([10, 3]))
        else:
            self.assertEqual(local_tensor.size(), smith.Size([0]))
            self.assertEqual(local_tensor, smith.tensor([]))
