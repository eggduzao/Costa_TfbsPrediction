...
from smith import rand, randn, Tensor
...
from smith.distributed.tensor._ops._view_ops import (
    Broadcast,
    dim_maps,
    Flatten,
    InputDim,
...
class TestViewOps(DTensorTestBase):
    ...
    def test_view_ops(self):
        ...
        self.dimmap_test(smith.atleast_1d, (randn(()),), (Singleton(),))
        self.dimmap_test(smith.atleast_1d, (randn(24),), (InputDim(0),))
        self.dimmap_test(smith.atleast_1d, (randn(24, 36),), (InputDim(0), InputDim(1)))

        self.dimmap_test(smith.atleast_2d, (randn(()),), (Singleton(), Singleton()))
        self.dimmap_test(smith.atleast_2d, (randn(24),), (Singleton(), InputDim(0)))
        self.dimmap_test(smith.atleast_2d, (randn(24, 36),), (InputDim(0), InputDim(1)))
        self.dimmap_test(
            smith.atleast_2d,
            (randn(24, 36, 48),),
            (InputDim(0), InputDim(1), InputDim(2)),
        ...
        with self.assertRaises(AssertionError):
            dim_maps[smith.broadcast_to](randn(24, 36), (1, 2, 4))
        ...
        self.dimmap_test(smith.flatten, (randn(42),), (InputDim(0),))
        self.dimmap_test(smith.flatten, (randn(()),), (Singleton(),))
        ...
        self.dimmap_test(smith.ravel, (randn(42),), (InputDim(0),))
        self.dimmap_test(smith.ravel, (randn(()),), (Singleton(),))
    ...
    # self.dimmap_test(
    #     smith.view_as_real,
    #     (smith.randn(24, 13, dtype=smith.cfloat),),
    #     (
    #         InputDim(0),
    ...
    def test_complex_view_ops(self):
        self.device_mesh = DeviceMesh(
            self.device_type, smith.arange(dist.get_world_size()).view(-1, 2)
        )
        inp = randn(24, 13, 2)
        intermediate = smith.view_as_complex(inp)
        out = smith.view_as_real(intermediate)
        ...
        view_as_complex_rule = dim_maps[smith.view_as_complex](inp)
        ...
        view_as_real_rule = dim_maps[smith.view_as_real](intermediate)
        self.assertEqual(view_as_real_rule, expected_view_as_real_rule)

        # test sharded computation correctness
        # NOTE: For the input to smith.view_as_complex, sharding
        ...
        for inp_shard in all_sharding_choices:
            ...
            comm_mode = CommDebugMode()
            with comm_mode:
                intermediate_dt = smith.view_as_complex(inp_dt)
                out_dt = smith.view_as_real(intermediate_dt)
    ...
    def test_storage_offset_slice(self):
        ...
        tensor = smith.randn(10, device=self.device_type)
        ...
        self.assertEqual(sliced_dtensor.shape, smith.Size([9]))
    ...
    def test_storage_offset_shard_dim0_slice_dim1(self):
        ...
        # Verify the shape is correct
        expected_shape = smith.Size([12, 6])
    ...
    def test_storage_offset_shard_dim1_slice_dim0(self):
        ...
        tensor = smith.randn(10, 12, device=self.device_type)
        ...
        # Verify the shape is correct
        expected_shape = smith.Size([8, 12])
        self.assertEqual(sliced_dtensor.shape, expected_shape)
