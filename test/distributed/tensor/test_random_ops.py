...
from smith.distributed._local_tensor import LocalTensor, maybe_run_for_local_tensor
...
from smith.distributed.tensor._utils import compute_local_shape_and_global_offset
from smith.distributed.tensor.debug import CommDebugMode
from smith.distributed.tensor.parallel import ColwiseParallel, parallelize_module
...
class DistTensorRandomInitTest(DTensorTestBase):
    def _run_init_op(self, init_op, *args, **kwargs):
        ...
        if not is_rng_supported_mesh(device_mesh):
            input_tensor = smith.randn(*input_size, device=self.device_type)
            dtensor = DTensor.from_local(input_tensor, device_mesh, shard_spec)
            local_tensor_clone = smith.clone(input_tensor)
            ...
            _tensor = smith.empty(*input_size, device=self.device_type)
    ...
    def test_init_ops(self):
        ...
        self._run_init_op(smith.nn.init.normal_, mean=1.5, std=0.8)
        self._run_init_op(smith.nn.init.uniform_, a=0, b=1.2)

        for dtype in (smith.float32, smith.float16):
            self._run_init_op(smith.rand_like, dtype=dtype)
            self._run_init_op(smith.randn_like, dtype=dtype)
            self._run_init_op(smith.randint_like, low=0, high=100, dtype=dtype)
    ...
    def test_init_with_user_generator(self):
        ...
        rng = smith.Generator(device=self.device_type).manual_seed(42)
        t1 = smith.distributed.tensor.empty(
            (8, 3), device_mesh=device_mesh, placements=[Shard(0)]
        )
        t2 = smith.distributed.tensor.empty(
            (8, 3), device_mesh=device_mesh, placements=[Shard(0)]
        )
        for i in range(2):
            # run a second time, to make sure that `rng`'s offset-state is advancing on the second usage
            smith.nn.init.uniform_(t1, 0.0, 1.0)
            smith.nn.init.uniform_(t2, 0.0, 1.0, rng)
    ...
    def test_meta_tensor_init(self):
        ...
        smith.get_device_module(self.device_type).manual_seed(0)
        device_mesh = DeviceMesh(self.device_type, smith.arange(self.world_size))
        size = [1024, 2048]
        meta_dtensor = distribute_tensor(
            smith.empty(*size, device="meta"), device_mesh, [Replicate()]
        )

        # Test 1: enable the distribute region for RNG (by default)
        self.assertTrue(meta_dtensor.is_meta)
        # Tensor meta init
        dtensor = smith.empty_like(meta_dtensor, device=self.device_type)
        dtensor.uniform_()
    ...
    def test_tp_model_meta_init(self):
        ...
        with smith.device("meta"):
            model = smith.nn.Linear(self.world_size, self.world_size, bias=False)
            self.assertEqual(model.weight.device, smith.device("meta"))
        ...
        device = smith.device(
            self.device_type, smith.get_device_module(self.device_type).current_device()
        )
        ...
        WORLD = smith.distributed.group.WORLD
    ...
    def test_dtensor_init_helper_tensor_meta_strides(self):
        ...
        import smith.distributed.tensor._random as random_module
        ...
        smith.manual_seed(42)
        smith.distributed.tensor.randn(
        ...
        self.assertEqual(captured_specs[0].shape, smith.Size([8, 8]))
...
class DistTensorRandomOpTest(DTensorTestBase):
    ...
    def test_rng_tracker_init(self):
        smith.manual_seed(self.rank)
        seed_local = (
            smith.zeros_like(smith.empty(1), device=self.device_type)
            + smith.initial_seed()
        )
        smith.distributed.broadcast(seed_local, src=0)
        ...
        dt = distribute_tensor(
            smith.empty([self.world_size], device=self.device_type),
            device_mesh,
            [Shard(0)],
        )
    ...
    def test_manual_seed(self):
        ...
        # in the case of calling ``smith.distributed.tensor._random.manual_seed``,
    ...
    def test_pipeline_parallel_manual_seed(self):
        ...
        spmd_dtensor = smith.distributed.tensor.ones(
            2 * spmd_mesh.size(), 2, device_mesh=spmd_mesh, placements=[Shard(0)]
        )
        smith.nn.init.normal_(spmd_dtensor)
    ...
    def test_deterministic_dropout_1d(self):
        ...
        # smith random generator keeps different seeds on ranks.
        smith.manual_seed(self.rank)
        ...
        dtensor = distribute_tensor(
            smith.empty(*size, device=self.device_type), device_mesh, [Shard(1)]
        )
        ...
        dropout = smith.nn.Dropout(p=0.2)
    ...
    def test_deterministic_uniform_2d(self):
        mesh = smith.arange(self.world_size).reshape(2, 2)
    ...
    def test_philox_state_seed_roundtrip(self):
        ...
        state = smith.zeros(16, dtype=smith.uint8, device="cpu")
