...
from smith.distributed.tensor._dtensor_spec import ShardOrderEntry
...
from smith.utils._debug_mode import (
    _OpCall,
    _RedistributeCall,
    _TritonKernelCall,
    DebugMode,
...
from smith.utils._triton import has_triton_package
...
class TestDTensorDebugMode(TestCase):
    ...
    def test_debug_mode_mm(self):
        ...
        x = smith.randn(1, 8, requires_grad=False)
        y = smith.randn(1, 32, requires_grad=True)
        x_dtensor = DTensor.from_local(x, mesh, [Shard(0)], run_check=False)
        y_dtensor = DTensor.from_local(y, mesh, [Shard(0)], run_check=False)

        with DebugMode(record_smithfunction=True, record_ids=True) as debug_mode:
            smith.mm(x_dtensor, y_dtensor).sum()

        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
  smith.mm(dt$0: f32[8, 8]| S(0), dt$1: f32[8, 32]| S(0))  ->  dt$7: f32[8, 32]| S(0)
  ...
  <method 'sum' of 'smith._C.TensorBase' objects>(dt$7: f32[8, 32]| S(0))  ->  dt$9: f32[]| P(sum)
    aten::sum(dt$7: f32[8, 32]| S(0))
      aten::sum(t$6: f32[1, 32])  ->  t$8: f32[]""",
        ...
        self.assertEqual(next(iter(debug_mode.operators[1])), smith.ops.aten.mm.default)
        ...
        with (
            ...
            compiled_out = smith.compile(mm, backend="aot_eager")(x_dtensor, y_dtensor)

        # check numerical equivalence
        self.assertTrue(smith.equal(eager_out, compiled_out))
        sum_op = next(
            iter(
                op
        ...
        self.assertTrue(smith.equal(sum_op.record["output"], eager_out.to_local()))
        self.assertTrue(
        ...
        # some edge cases
        self.assertEqual(norm_(smith.tensor(smith.nan)), smith.nan)
        self.assertEqual(norm_(smith.tensor(smith.inf)), smith.inf)
        self.assertEqual(norm_(smith.complex(smith.ones(4), smith.zeros(4))), 4)
        self.assertEqual(hash_(smith.ones(4, dtype=smith.float8_e5m2)), 0)
        self.assertEqual(hash_(smith.ones(4, dtype=smith.int8)), 0)
        self.assertEqual(hash_(smith.ones(5, dtype=smith.int8)), 1)
    ...
    def test_debug_mode_backward(self):
        ...
        x = smith.randn(1, 8, requires_grad=True)
        y = smith.randn(8, 1, requires_grad=True)
        x_dtensor = DTensor.from_local(x, mesh, [Shard(0)], run_check=False)
        y_dtensor = DTensor.from_local(y, mesh, [Shard(1)], run_check=False)

        with DebugMode(
            record_smithfunction=True, record_stack_trace=True, record_output=False
        ) as debug_mode:
        ...
        self.assertIn(
            "<method 'add' of 'smith._C.TensorBase' objects>", debug_string
        ...
        self.assertIn(
            "<method 'sum' of 'smith._C.TensorBase' objects>", debug_string
        )  # Unique - no params needed
        ...
        self.assertIn(
            "smith._tensor.backward", debug_string
        )  # Unique - no params needed
        ...
        self.assertIn(
            "aten::_to_copy(t: f32[8, 1], dtype=smith.float32, layout=smith.strided, device=cpu)",
            debug_string,
        )
        self.assertIn(
            "aten::_to_copy(t: f32[1, 8], dtype=smith.float32, layout=smith.strided, device=cpu)",
            debug_string,
        )
    ...
    def test_stack_trace_in_compiled_region(self):
        class Foo(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.l1 = smith.nn.Linear(8, 4)
                self.l2 = smith.nn.Linear(4, 8)
        ...
        x = smith.randn(16, 8)
        model = smith.compile(Foo(), backend="aot_eager", fullgraph=True)
    ...
    def test_debug_mode_densor_redistribution_trace(self):
        mesh = DeviceMesh(self.device_type, smith.arange(self.world_size).view(4, 2))

        x = smith.randn(16, 8, requires_grad=True)
        y = smith.randn(8, 16, requires_grad=True)
        ...
        with DebugMode(record_smithfunction=False, record_output=False) as debug_mode:
            smith.mm(x_dtensor, y_dtensor).sum()
  ...
  aten::sum(dt: f32[128, 128]| S(0)[0]S(0)[1])
    ...
    def test_output_placements(self):
        ...
        x = smith.randn(8, 2, requires_grad=False)
        ...
        with DebugMode(record_output=True) as debug_mode:
            smith.topk(x_dtensor, k=4, dim=1)
  ...
  aten::topk(dt: f32[8, 16]| S(1), 4, 1)
    ...
    def test_debug_mode_einsum(self):
        ...
        with DebugMode(record_smithfunction=True, record_output=False) as debug_mode:
            smith.einsum("bld,dnh->blnh", a_dt, b_dt)

        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
  smith.functional.einsum(bld,dnh->blnh, dt: f32[16, 6, 8]| P(sum)R, dt: f32[8, 4, 4]| RP(sum))
    ...
    def test_real_tensor(self):
        ...
        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
  smith._C._nn.linear(t: f32[8, 8, 8], t: f32[8, 8], t: f32[8])
      aten::view(t: f32[8, 8, 8], [64, 8])
      aten::t(t: f32[8, 8])
      aten::addmm(t: f32[8], t: f32[64, 8], t: f32[8, 8])
      aten::view(t: f32[64, 8], [8, 8, 8])
  <method 'sum' of 'smith._C.TensorBase' objects>(t: f32[8, 8, 8])
    ...
    def test_fake_tensor(self):
        ...
        with DebugMode(
            record_smithfunction=True, record_faketensor=True, record_output=False
        ) as debug_mode:
            smith.matmul(y, x)
        ...
        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
  smith.matmul(ft: f32[8, 8, 8], ft: f32[8, 8])
    ...
    def test_tensor_attributes(self):
        ...
        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
  smith.matmul(t: f32[8, 8, 8]{a1=y}, t: f32[8, 8]{a1=x1, a2=x2})
    ...
    def test_nested_debug_mode(self, has_inner_mode, has_outer_mode):
        class DummySmithDispatchMode1(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                return func(*args, **kwargs)

        class DummySmithDispatchMode2(SmithDispatchMode):
            def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                return func(*args, **kwargs)
        ...
        inner_mode = (
            DummySmithDispatchMode1() if has_inner_mode else contextlib.nullcontext()
        )
        outer_mode = (
            DummySmithDispatchMode2() if has_outer_mode else contextlib.nullcontext()
        )

        with outer_mode:
            with DebugMode() as debug_mode:
                with inner_mode:
                    smith.mm(x_dtensor, y_dtensor)
    ...
    def test_debug_mode_higher_order_cond(self):
        ...
        with DebugMode(record_smithfunction=True) as debug_mode:
            # rewrite smith.conda as smith.ops.higher_order.cond to avoid compilation
            smith.ops.higher_order.cond(
                smith.tensor(True), lambda x: x + 1, lambda x: x - 1, (x,)
            )

        # Verify that cond operations are captured in debug mode
        self.assertIn("smith.ops.higher_order.cond", debug_mode.debug_string())
      ...
      [nn.Mod] Foo.l1
        ...
        for backend in ["aot_eager", "inductor"]:
            with DebugMode(run_compile_with_interpreter=True) as debug_mode:
                smith.compile(mod, backend=backend, fullgraph=True)(x)
  ...
  [aot_eager region (compile)] exit""",
    ...
    def test_nn_module_in_eager(self):
        class Foo(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.l1 = smith.nn.Linear(4, 4)
                self.l2 = smith.nn.Linear(4, 4)
    ...
    [nn.Mod] Bar
      ...
      [nn.Mod] Bar.xyz
        ...
        fn = smith.compile(mod, backend="eager", fullgraph=True)
        with DebugMode(record_nn_module=True) as debug_mode:
            fn(inp)
    ...
    def test_nn_module_in_compiled_regions(self):
        ...
        mod = Baz()
        mod.foo = smith.compile(mod.foo, backend="aot_eager", fullgraph=True)
        inp = smith.randn(4, 4)
        with DebugMode(
    ...
    aten::sum(t: f32[4, 4])  ->  t: f32[]""",
        ...
        mod = smith.compile(Baz(), backend="aot_eager", fullgraph=True)
        inp = smith.randn(4, 4)
        with DebugMode(
            record_nn_module=True, run_compile_with_interpreter=True
        ) as debug_mode:
  ...
  [aot_eager region (compile)] exit
    aten::sum(t: f32[4, 4])  ->  t: f32[]
    aten::ones_like(t: f32[], pin_memory=False, memory_format=smith.preserve_format)  ->  t: f32[]
    aten::expand(t: f32[], [4, 4])  ->  t: f32[4, 4]
    aten::clone(t: f32[4, 4], memory_format=smith.contiguous_format)  ->  t: f32[4, 4]
  [aot_eager region (compile)] enter
    [nn.Mod (compile)] L['self'].bar
      [nn.Mod (compile)] L['self'].bar.l3
        aten::t(t: f32[4, 4])  ->  t: f32[4, 4]
  ...
  [aot_eager region (compile)] exit
    ...
    def test_record_function(self):
        def fn(x, y):
            z = x @ y
            with smith.profiler.record_function("this_is_ignored"):
                z = z + 1
            return z.sum()

        x = smith.randn(8, 4, requires_grad=True)
        y = smith.randn(4, 2, requires_grad=True)
        with DebugMode(run_compile_with_interpreter=True) as debug_mode:
            with smith.profiler.record_function("FWD"):
                out = smith.compile(fn, backend="aot_eager")(x, y)
            out.backward()
  ...
  [aot_eager region (compile)] exit
    ...
    def test_triton_kernel_logs(self):
        ...
        from smith.testing._internal.triton_utils import add_kernel_autotuned
        ...
        x = smith.randn(128, device=GPU_TYPE)
        y = smith.randn(128, device=GPU_TYPE)

        with DebugMode() as debug_mode:
            smith.compile(call_triton)(x, y)
    ...
    def test_check_hash_mismatches(self):
        x = smith.randn(64, 64, device=GPU_TYPE)
        x_different = smith.randn(64, 64, device=GPU_TYPE)
    ...
    @unittest.skipIf(
        ...
        or smith.cuda.get_device_properties(0).total_memory < 2**26,
    ...
    def test_tensor_hash_redistribute(self):
        ...
        local_tensor = smith.ones(2**18, device=self.device_type)
        ...
        all_gather_logs = [
            ...
            and op.op == smith.ops._c10d_functional.all_gather_into_tensor.default
        ]
    ...
    def test_check_triton_hash_mismatches(self):
        ...
        a = smith.randn(128, device=GPU_TYPE)
        b = smith.randn(128, device=GPU_TYPE)
        c = smith.randn(128, device=GPU_TYPE)
    ...
    def test_check_structure_mismatches(self):
        x = smith.randn(32, 32, device=self.device_type)
    ...
    def test_pretty_print_dtensor_make_fx(self):
        ...
        self.assertTrue('"DTensor(f32[8, 32], S(0))" = smith.ops.aten.mm' in gm_str)
    ...
    def test_invoke_subgraph(self):
        ...
        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
    smith.ops.higher_order.invoke_subgraph(partitioned_fw_subgraph_0_0, t: f32[8, 8])  ->  ('t: f32[8, 8]', 't: f32[8, 8]')
    ...
    aten::clone(t: f32[8, 8], memory_format=smith.contiguous_format)  ->  t: f32[8, 8]
    smith.ops.higher_order.invoke_subgraph(partitioned_bw_subgraph_0_0, t: f32[8, 8], t: f32[8, 8])  ->  ('t: f32[8, 8]',)
    ...
    def test_nested_invoke_subgraph(self):
        ...
        self.assertExpectedInline(
            debug_mode.debug_string(),
            """\
    smith.ops.higher_order.invoke_subgraph(subgraph_1, t: f32[8, 8])  ->  ('t: f32[8, 8]',)
    [annotate] [enter InvokeSubgraph HOP] subgraph_1
      aten::add.Tensor(t: f32[8, 8], 1)  ->  t: f32[8, 8]
      smith.ops.higher_order.invoke_subgraph(subgraph_0, t: f32[8, 8])  ->  ('t: f32[8, 8]',)
...
class TestDebugModeUtils(TestCase):
    ...
    def test_hash_empty_tensor(self):
        ...
        out = smith.utils._debug_mode.hash_tensor_fn(t)
        self.assertTrue(isinstance(out, smith.Tensor))
        out = smith.utils._debug_mode.hash_tensor_fn(t, use_scalar=True)
...
class TestDTensorDebugModeNCCLBackend(MultiProcessTestCase):
    ...
    def test_allgather_base(self):
        ...
        tensor = smith.ones(10, 10, device=smith.device(self.device)) * (self.rank + 1)
        # Output size must be world_size * input_size
        output_tensor = smith.zeros(
            10 * self.world_size, 10, device=smith.device(self.device)
        )
        ...
        for i in range(self.world_size):
            expected_slice = smith.ones(10, 10, device=self.device) * (i + 1)
    ...
    def test_allgather_base_async_op(self):
        ...
        # https://github.com/blacksmith/blacksmith/pull/169295
        # self.assertEqual(debug_mode.operators[-1].log["hash"][0], hash_(output_tensor))
        self.assertEqual(
