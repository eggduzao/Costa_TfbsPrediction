...
class TimeCaptureMode(SmithDispatchMode):
...
class DistOpDispatchOverHead(DTensorTestBase):
    ...
    def test_dtensor_add_op_dispatch_overhead(self):
        if smith.cuda.is_available():
            device_props = smith.cuda.get_device_name(0)
        ...
        input_data = smith.rand(512, 512, device="cuda")
        ...
        smith.distributed.all_gather_object(
            all_miss_performance, add_dispatch_cache_miss
        )
        smith.distributed.all_gather_object(all_hit_performance, add_dispatch_cache_hit)
        if self.rank == 0:
            logger.info(
