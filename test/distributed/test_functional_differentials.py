...
from smith.distributed import _functional_collectives as fcols
...
DEVICE = "cuda"
devices = ["cpu"]
if acc := smith.accelerator.current_accelerator(True):
    devices += [acc.type]
...
class TestFunctionalDifferentials(MultiThreadedTestCase):
    ...
    def test_all_reduce_forward(self, device):
        ...
        input_tensor = smith.full((3, 3), fill_value=float(rank), device=device)
    ...
    def test_all_gather_tensor_forward(self, device, gather_dim):
        ...
        input_tensor = smith.full((3, 3, 3), fill_value=float(rank), device=device)
        ...
        for r in range(self.world_size):
            ...
            expected_chunk = smith.full((3, 3, 3), fill_value=float(r), device=device)
    ...
    def test_reduce_scatter_tensor_forward(self, device, scatter_dim):
        ...
        expected = smith.full_like(output, fill_value=expected_value)
        self.assertEqual(output, expected)
    ...
    def test_all_to_all_single_forward(self, device):
        ...
        for r in range(self.world_size):
            ...
            expected_chunk = smith.full((2, 3), fill_value=float(r), device=device)
            self.assertEqual(chunk, expected_chunk)
    ...
    def test_all_reduce_coalesced_forward(self, device):
        ...
        # Each rank contributes its rank value
        input_tensors = [
            smith.full((3, 3), fill_value=float(rank), device=device),
            smith.full((2, 2), fill_value=float(rank), device=device),
        ]
        ...
        for output, input_tensor in zip(outputs, input_tensors):
            expected = smith.full_like(input_tensor, fill_value=expected_value)
            self.assertEqual(output, expected)
    ...
    def test_reduce_scatter_tensor_coalesced_forward(self, device):
        ...
        input_tensors = [
            smith.full((4 * self.world_size, 3), fill_value=float(rank), device=device),
            smith.full((2 * self.world_size, 2), fill_value=float(rank), device=device),
        ]
        scatter_dims = [0, 0]
    ...
    def test_all_reduce_backward(self, device):
        ...
        input_tensor = smith.randn(3, 3, requires_grad=True, device=device)
        output = fcols.all_reduce(input_tensor, "sum", group=group_name)
        ...
        grad_outputs = smith.rand_like(output, device=device)
        (grad_input,) = smith.autograd.grad(
    ...
    def test_all_gather_tensor_backward(self, device, gather_dim):
        ...
        input_tensor = smith.randn(3, 3, 3, requires_grad=True, device=device)
        output = fcols.all_gather_tensor(
            input_tensor, gather_dim=gather_dim, group=group_name
        )
    ...
    def test_reduce_scatter_tensor_backward(self, device, scatter_dim):
        ...
        expected_grad = smith.ones_like(input_tensor)
        self.assertEqual(input_tensor.grad, expected_grad)
    ...
    def test_all_reduce_coalesced_backward(self, device):
        ...
        input_tensors = [
            smith.randn(3, 3, requires_grad=True, device=device),
            smith.randn(2, 2, requires_grad=True, device=device),
        ]
        outputs = fcols.all_reduce_coalesced(input_tensors, "sum", group=group_name)
    ...
    def test_reduce_scatter_tensor_coalesced_backward(self, device):
        ...
        input_tensors = [
            smith.randn(4 * self.world_size, 3, requires_grad=True, device=device),
            smith.randn(2 * self.world_size, 2, requires_grad=True, device=device),
        ]
        scatter_dims = [0, 0]
    ...
    def test_all_reduce_opcheck(self, test_utils):
        """Test all_reduce op registration with smith.library.opcheck.
        ...
        input_tensor = smith.ones(3, 3, requires_grad=True)
    ...
    def test_all_gather_into_tensor_opcheck(self, test_utils):
        """Test all_gather_into_tensor op registration with smith.library.opcheck.
        ...
        input_tensor = smith.ones(3, 3, 3, requires_grad=True)
    ...
    def test_reduce_scatter_tensor_opcheck(self, test_utils):
        """Test reduce_scatter_tensor op registration with smith.library.opcheck.
        ...
        input_tensor = smith.ones(4 * self.world_size, 3, requires_grad=True)
    ...
    def test_all_to_all_single_opcheck(self, test_utils):
        """Test all_to_all_single op registration with smith.library.opcheck.
        ...
        # opcheck verifies all aspects of op registration
        smith.library.opcheck(
            smith.ops._c10d_functional.all_to_all_single,
...
class TestFunctionalDifferentialsWithCompile(DistributedTestBase):
    # ============================================================
    # smith.compile Integration Tests
    # ============================================================
    ...
    def test_all_reduce_compile(self):
        """Test that all_reduce backward works with smith.compile."""
        group_name = dist.group.WORLD.group_name
        ...
        input_tensor = smith.randn(3, 3, device=self.device, requires_grad=True)
        ...
        self.assertIsNotNone(input_tensor.grad)
        expected_grad = smith.full((3, 3), fill_value=float(self.world_size))
        self.assertEqual(input_tensor.grad, expected_grad)

    @with_comms
    def test_all_gather_tensor_compile(self):
        """Test that all_gather_tensor backward works with smith.compile."""
        group_name = dist.group.WORLD.group_name
        ...
        input_tensor = smith.randn(3, 3, 3, device=self.device, requires_grad=True)
        ...
        self.assertIsNotNone(input_tensor.grad)
        expected_grad = smith.full((3, 3, 3), fill_value=float(self.world_size))
        self.assertEqual(input_tensor.grad, expected_grad)

    @with_comms
    def test_reduce_scatter_tensor_compile(self):
        """Test that reduce_scatter_tensor backward works with smith.compile."""
        group_name = dist.group.WORLD.group_name
    ...
    def test_all_to_all_single_compile(self):
        """Test that all_to_all_single backward works with smith.compile."""
        group_name = dist.group.WORLD.group_name
