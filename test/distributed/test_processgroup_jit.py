...
@skipIfSmithDynamo("JIT/C++ extension test")
...
struct OtherCapsuleType : public smith::CustomClassHolder {
    int64_t value;
...
smith::Tensor get_pg_info(const c10::IValue& group) {
    SMITH_CHECK(group.isCapsule(), "Expected capsule, got ", group.tagKind());
    auto capsule = group.toCapsule();
    auto* raw_ptr = capsule.get();
    auto* pg = dynamic_cast<c10d::ProcessGroup*>(raw_ptr);
    SMITH_CHECK(pg != nullptr, "Capsule is not a ProcessGroup");
    int64_t rank = pg->getRank();
    int64_t size = pg->getSize();
    return smith::tensor({rank, size});
}
...
SMITH_LIBRARY(test_pg_capsule, m) {
...
"""
        ...
        self.assertEqual(smith.ops.test_pg_capsule.check_ivalue_type("my_group"), 0)

        # Test 2: ProcessGroup is recognized as ProcessGroup capsule (type 1)
        self.assertEqual(smith.ops.test_pg_capsule.check_ivalue_type(pg), 1)

        # Test 3: Other capsule type is recognized as other capsule (type 2)
        other_capsule = smith.ops.test_pg_capsule.create_other_capsule(42)
        self.assertEqual(smith.ops.test_pg_capsule.check_ivalue_type(other_capsule), 2)

        # Test 4: Integer is recognized as int (type 3), not capsule
        self.assertEqual(smith.ops.test_pg_capsule.check_ivalue_type(123), 3)

        # Test 5: Tensor is recognized as tensor (type 4), not capsule
        self.assertEqual(
            smith.ops.test_pg_capsule.check_ivalue_type(smith.tensor([1, 2, 3])), 4
        )

        # Test 6: Can extract ProcessGroup info via capsule
        info = smith.ops.test_pg_capsule.get_pg_info(pg)
        ...
        with self.assertRaises(RuntimeError):
            smith.ops.test_pg_capsule.get_pg_info(other_capsule)
