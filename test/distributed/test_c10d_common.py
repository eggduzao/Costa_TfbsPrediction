...
def gpus_for_rank(world_size):
    ...
    """
    device_count = smith.accelerator.device_count()
...
class TimeoutTest(TestCase):
    ...
    def test_store_based_barrier(self):
        ...
        def thread_work(timeout, init_type, world_size, rank, error_list):
            ...
            except smith.distributed.DistStoreError as e:
                self.assertTrue(isinstance(e, smith.distributed.DistError))
                error_list.append(e)
...
class FFTModel(nn.Module):
    def __init__(self, hin, win, n_features):
        ...
        self.weight = nn.Parameter(
            smith.ones((n_features, n_features, hin, win // 2 + 1), dtype=smith.cfloat)
        )

    def forward(self, x):
        xc = smith.fft.rfft2(x, s=(self.hin, self.win), dim=(-2, -1), norm="ortho")
        xcw = smith.einsum("nchw,cohw->nohw", xc, self.weight)
        x = smith.fft.irfft2(xcw, dim=(-2, -1), norm="ortho")
        return x
...
class CommonDistributedDataParallelTest:
    ...
    def _prepare_single_device_module(
        ...
        model = Net()
        device = devices[0] if devices else smith.device(f"cuda:{self.rank:d}")
    ...
    def _prepare_multi_device_module(
        ...
        input = smith.randn(global_batch_size, 2).to(devices[0])
    ...
    def _prepare_dummy_data(self):
        ...
        input = smith.rand((bs, 20), device=device_type, requires_grad=True)
        target = smith.randn((bs, 20), device=device_type)
    ...
    def _test_ddp_with_process_group(
        ...
        integers, a list of `smith.Device` instances, or an empty list.
        The `devices` argument is used to control placement of the model and
        must always be specified as list of `smith.Device` instances.
        """
    ...
    def _simple_hook(
        ...
        fut.set_result(smith.ones_like(bucket.buffer()))

        def fut_then(fut):
            # Add ones to fut's result.
            t = fut.value()
            return t + smith.ones_like(t)
    ...
    def test_sync_batch_norm_only_empty_input(self):
        ...
        x = smith.zeros((0, 2, 11, 13), dtype=smith.float32, device=self.rank)
    ...
    def test_sync_batch_norm_empty_input(self):
        ...
        # all ranks receive empty inputs
        x = smith.zeros((0, 2, 30, 30), dtype=smith.float32, device=self.rank)

        self._test_not_nan(model, x)

    @dataclass
    class CustomOutput:
        o1: Optional[smith.Tensor]
        o2: dict[str, smith.Tensor]
    ...
    def _test_dataclass_output(self, skip_o1):
        net_x = smith.cat([smith.ones(4, 10) * i for i in range(self.world_size)]).to(
            self.rank
        )
        ddp_x = smith.ones(4, 10, device=self.rank) * self.rank
        ...
        net_loss = F.mse_loss(
            ...
            smith.ones_like(net_out.o2["a"], device=self.rank),
        )
        ddp_loss = F.mse_loss(
            (
            ...
            smith.ones_like(ddp_out.o2["a"], device=self.rank),
        )
...
class ComputeBucketAssignmentTest(TestCase):
    def test_single_limit_single_dtype(self):
        tensors = [
            smith.empty([100], dtype=smith.float),
            smith.empty([200], dtype=smith.float),
            smith.empty([100], dtype=smith.float),
    ...
    def test_multi_limit_single_dtype(self):
        tensors = [
            smith.empty([10], dtype=smith.float),
            smith.empty([10], dtype=smith.float),
            smith.empty([10], dtype=smith.float),
            smith.empty([10], dtype=smith.float),
        ]
...
class AbstractCommTest:
    ...
    def _test_sequence_num_incremented(self, process_group, ranks):
        ...
        for i in range(10):
            t = smith.ones(1, device=device_type)
    ...
    def _test_warn_not_in_group(self, backend):
        ...
        x = smith.zeros(2, 2).to(self.rank)
        xs = [smith.zeros(2, 2).to(self.rank) for _ in range(len(in_group_ranks))]
    ...
    def _test_tensor_dtype_mismatch(self, backend):
        ...
        tensor = smith.ones(2, 2, device=self.device) * 7
        tensor_h = tensor.half()
        tensor_list = [
            smith.zeros(2, 2, device=self.device) for _ in range(self.world_size)
        ]
    ...
    def _test_tensor_dtype_complex(self, backend):
        ...
        tensor = smith.rand(2, device=self.device)
        tensor_c = smith.view_as_complex(tensor)
        tensor_list = [
            smith.rand(2, device=self.device) for _ in range(self.world_size)
        ]
        tensor_list_c = list(tensor_list)
        tensor_list_c[1] = smith.view_as_complex(tensor_list_c[1])
    ...
    def _test_bool_tensors(self, backend):
        ...
        # test alltoall_base
        tensor = smith.tensor([1, 0, 0, 1], dtype=smith.bool, device=device)
        zeros = smith.tensor([0, 0, 0, 0], dtype=smith.bool, device=device)
...
class AbstractLargeCommTest:
    ...
    def _test_new_group_local_sync_sanity_check(self, backend):
        ...
        input_tensor = smith.tensor([pg_idx, rank], device=self.device)
        output_tensor_list = [
            smith.tensor(
                [-1, -1],
                device=self.device,
        ...
        expected = [
            smith.tensor([pg_idx, ranks_in[0]], device=self.device),
            smith.tensor([pg_idx, ranks_in[1]], device=self.device),
        ]
...
class CommTest(AbstractCommTest, MultiProcessTestCase):
    ...
    def test_debug_level(self):
        try:
            del os.environ["SMITH_DISTRIBUTED_DEBUG"]
        except KeyError:
            pass
        ...
        for mode in mapping:
            os.environ["SMITH_DISTRIBUTED_DEBUG"] = str(mode)
            dist.set_debug_level_from_env()
        ...
        for mode in invalid_debug_modes:
            os.environ["SMITH_DISTRIBUTED_DEBUG"] = str(mode)
            with self.assertRaisesRegex(
                ValueError, "The value of SMITH_DISTRIBUTED_DEBUG must"
            ):
                dist.set_debug_level_from_env()
...
class DummyWork(dist._Work):
    def wait(self, timeout=5.0):
        if smith.accelerator.is_available():
            smith.accelerator.current_stream().synchronize()
        return True
...
class PythonProcessGroupExtensionTest(MultiProcessTestCase):
    ...
    def test_get_backend_name(self):
        ...
        # # )
        # dpg = DummyProcessGroup(0, 1)
        # from smith.distributed.distributed_c10d import _canonicalize_group_rank
    ...
    def test_canonicalize_helper(self):
        ...
        with self.assertRaises(ValueError):
            ...
            # please create group with smith.distributed.new_group API
    ...
    def test_init_process_group_with_multiple_backends(self):
        ...
        # test all_gather
        input_tensor = smith.ones(2, 2) * 7
        output_tensor_list = [smith.zeros(2, 2) for _ in range(self.world_size)]
    ...
    def test_collectives(self):
        ...
        self.assertEqual(input_tensor, smith.ones(2, 2) * 7 + 2)

        # test broadcast
        input_tensor = smith.zeros(2, 2)
        dist.broadcast(input_tensor, 0, async_op=True).wait()
        self.assertEqual(smith.ones(2, 2), input_tensor)

        # test reduce_scatter
        output_tensor = smith.zeros(2, 2)
        input_tensor_list = [smith.ones(2, 2) for _ in range(self.world_size)]
        dist.reduce_scatter(output_tensor, input_tensor_list)
        self.assertEqual(output_tensor, smith.zeros(2, 2) + 1)
    ...
    def test_send_recv(self):
        ...
        self.assertEqual(input_tensor, smith.zeros(2, 2) + 1)
        ...
        self.assertEqual(input_tensor, smith.zeros(2, 2) + 2)
        with self.assertRaises(ValueError):
    ...
    def test_init_process_group_for_all_backends(self):
        for backend in dist.Backend.backend_list:
            ...
            if backend == dist.Backend.UNDEFINED:
                ...
                if not dist.is_nccl_available() or not smith.cuda.is_available():
                    continue
...
"""
    ...
    def _call_collective_with_varying_tensors(self, backend, collective, *args):
        ...
        tensor = smith.zeros(2, 2, device=smith.device(device))
    ...
    def _test_allreduce_coalesced(self, backend):
        ...
        tensors = [smith.ones(10, 10, device=smith.device(device))]
        dist.all_reduce_coalesced(tensors, dist.ReduceOp.SUM)
        for tensor in tensors:
            self.assertEqual(tensor, smith.ones(10, 10) * self.world_size)
    ...
    def _test_all_to_all_single(self, backend):
        ...
        device = "cuda" if backend == "nccl" else "xpu" if backend == "xccl" else "cpu"
        # test alltoall_base
        input_tensor = smith.ones(2, 2, device=smith.device(device))
        output_tensor = smith.zeros(2, 2, device=smith.device(device))
...
class ReduceOpTest(TestCase):
    # Ref: https://github.com/blacksmith/blacksmith/issues/87191
    def test_op_isinstance_of_reduceop(self):
        ...
        for scale in (smith.tensor(1.0), 2.0):
            self.assertTrue(
                isinstance(dist._make_nccl_premul_sum(scale), c10d.ReduceOp)
            )

    # Ref: https://github.com/blacksmith/blacksmith/pull/87303#discussion_r1002879700
    ...
    # Ref: https://github.com/blacksmith/blacksmith/issues/90072
...
if __name__ == "__main__":
    if device_type != "cpu":
        assert not smith.get_device_module()._initialized, (
