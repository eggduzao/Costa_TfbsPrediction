...
from smith._inductor.runtime.triton_compat import triton
...
from smith.testing._internal.inductor_utils import IS_H100, requires_triton
...
class NVSHMEMTritonTest(MultiProcContinuousTest):
    ...
    def test_triton_put(self) -> None:
        ...
        if rank == 1:
            ...
            smith.testing.assert_close(
                dst, smith.tensor(expected, device=self.device, dtype=dtype)
            )
    ...
    def test_triton_get(self, nbi: bool) -> None:
        ...
        if rank == 1:
            smith.testing.assert_close(
                out, val * smith.ones(numel, dtype=dtype, device=self.device)
            )
    ...
    def test_triton_get_ring(self) -> None:
        ...
        expected_value = peer
        smith.testing.assert_close(
            out, expected_value * smith.ones(numel, dtype=dtype, device=self.device)
        )
    ...
    def test_triton_put_signal_set(self) -> None:
        ...
        flag = out_hdl.get_signal_pad(rank, (1,), dtype=smith.int64).fill_(0)
        ...
        if rank == 1:
            ...
            smith.testing.assert_close(
                flag, smith.tensor([SIGNAL_VAL], dtype=smith.int64, device=self.device)
            )
    ...
    def test_triton_wait_until(self) -> None:
        ...
        flag = symm_mem.empty(1, dtype=smith.int32, device=self.device).fill_(
            FLAG_INITIAL_VALUE
        )
        symm_mem.rendezvous(flag, group=group_name)
        expected_flag = smith.tensor(
            [FLAG_FINAL_VALUE], dtype=smith.int32, device=self.device
        )
    ...
    def test_triton_signal_wait_until(self) -> None:
        ...
        if rank == 0:
            ...
            smith.testing.assert_close(
                out, val_to_put * smith.ones(numel, dtype=dtype, device=self.device)
            )
            smith.testing.assert_close(
                flag,
                smith.tensor(
    ...
    def test_triton_fence(self) -> None:
        ...
        flag = symm_mem.empty(1, dtype=smith.int32, device=self.device).fill_(0)
        symm_mem.rendezvous(flag, group=group_name)
        flag_update_val = smith.tensor(
            [flag_val], dtype=smith.int32, device=self.device
        )
        ...
        if rank == 0:
            ...
            smith.testing.assert_close(
                out1, val1 * smith.ones(numel, dtype=dtype, device=self.device)
            )
            smith.testing.assert_close(
                out2, val2 * smith.ones(numel, dtype=dtype, device=self.device)
            )
            smith.testing.assert_close(
                flag, smith.tensor([flag_val], dtype=smith.int32, device=self.device)
            )
    ...
    def test_triton_barrier(self) -> None:
        ...
        if rank == 0:
            smith.testing.assert_close(
                src, smith.tensor([42], device=self.device, dtype=dtype)
            )
        else:
            smith.testing.assert_close(
                dst, smith.tensor([43], device=self.device, dtype=dtype)
            )
    ...
    def test_triton_sync(self) -> None:
        ...
        smith.testing.assert_close(
            local_data, smith.tensor([expected_local], device=self.device, dtype=dtype)
        )
        ...
        smith.testing.assert_close(
            remote_data,
            smith.tensor([expected_remote], device=self.device, dtype=dtype),
        )
    ...
    def test_triton_alltoall(self) -> None:
        ...
        for i in range(world_size):
            ...
            smith.testing.assert_close(actual, smith.full_like(actual, expected))
    ...
    @parametrize(
        ...
        [
            ...
            smith.float16,
            smith.float32,
            # smith.float64,  # Tensor-likes are not close
            smith.bfloat16,
        ],
    )
    ...
    def test_triton_minmax_reduce(self, dtype) -> None:
        ...
        # Verify results
        smith.testing.assert_close(
            dst_min, smith.tensor(expected_min, device=self.device, dtype=dtype)
        )
        smith.testing.assert_close(
            dst_max, smith.tensor(expected_max, device=self.device, dtype=dtype)
        )
    ...
    def test_triton_prod_reduce(self, dtype) -> None:
        ...
        symm_mem.rendezvous(dst, group=group_name)
        # Calculate expected results
        vals = smith.empty(nreduce, world_size, dtype=dtype)
        vals[0, ::2] = 1
        vals[0, 1::2] = 2
        vals[1] = 1
