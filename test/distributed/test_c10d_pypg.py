...
from smith._C._distributed_c10d import _create_work_from_future
...
class MyWork(dist._Work):
    def __init__(self, result, pg):
        super().__init__()
        self.result_ = result
        self.future_ = smith.futures.Future()
...
class AbstractDDPSingleRank(test_c10d_common.CommonDistributedDataParallelTest):
    ...
    def test_ddp_with_pypg(self):
        ...
        self._test_ddp_with_process_group(pg, [smith.device("cpu")], device_ids=None)

    def test_ddp_with_pypg_with_grad_views(self):
        pg = self._get_process_group()

        self._test_ddp_with_process_group(
            pg, [smith.device("cpu")], device_ids=None, gradient_as_bucket_view=True
        )
