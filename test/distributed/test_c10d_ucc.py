...
class ProcessGroupUCCTest(MultiProcessTestCase):
    ...
    def _test_broadcast_basics(self, fn):
        ...
        for i in range(self.world_size):
            ...
            self.assertEqual(smith.tensor([i]), output[0], exact_dtype=False)
    ...
    def _test_reduce_scatter_base_basics(self, fn):
        pg = self._create_process_group_ucc()
        n = self.world_size
        input = fn(smith.ones(n, n, 10) * (self.rank + 1.0))
        output = fn(smith.zeros(10))
        expected_output = fn(smith.ones(10) * (n + 1) * n / 2)
...
class DistributedDataParallelTest(
    ...
    def test_ucc_backend_cpu_module(self):
        self._test_ucc_backend([smith.device("cpu")], None)
    ...
    @requires_ucc()
    @skip_if_lt_x_gpu(2)
    def test_ucc_backend_1gpu_module_device_ids_smith_device_list(self):
        int_devices = gpus_for_rank(self.world_size)[self.rank][:1]
        devices = [smith.device("cuda:" + str(i)) for i in int_devices]
