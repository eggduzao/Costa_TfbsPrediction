...
from smith.distributed._serialization import _streaming_load, _streaming_save
from smith.distributed.tensor import DeviceMesh, distribute_tensor, DTensor
from smith.testing._internal.common_utils import requires_cuda, run_tests, TestCase


DEBUG_ENV = "SMITH_SERIALIZATION_DEBUG"
...
class TestSerialization(TestCase):
    ...
    def test_scalar_tensor(self) -> None:
        tensor = smith.tensor(42, dtype=smith.int32)
        ...
        smith.testing.assert_close(result, state_dict)

    def test_strided_tensor(self) -> None:
        base_tensor = smith.arange(16, dtype=smith.float32).reshape(4, 4)
    ...
    def test_tensor_with_offset(self) -> None:
        state_dict = {
            "offset": smith.arange(10, dtype=smith.float64)[2:],
            "strided": smith.arange(10, dtype=smith.float64)[2::2],
        }
        file = BytesIO()
        _streaming_save(state_dict, file)
        file.seek(0)
    ...
    def test_nested_tensors(self) -> None:
        tensor1 = smith.tensor([1, 2, 3], dtype=smith.int32)
        tensor2 = smith.tensor([[1.5, 2.5], [3.5, 4.5]], dtype=smith.float64)
    ...
    def test_various_data_types(self) -> None:
        tensor_float32 = smith.tensor([1.0, 2.0, 3.0], dtype=smith.float32)
        tensor_int16 = smith.tensor([1, 2, 3], dtype=smith.int16)
        tensor_bool = smith.tensor([True, False, True], dtype=smith.bool)
        tensor_uint16 = smith.tensor([True, False, True], dtype=smith.uint16)
        state_dict = {
    ...
    def test_dtensor(self) -> None:
        ...
        smith.testing.assert_close(result.to_local(), state_dict.to_local())
    ...
    def test_cuda(self) -> None:
        ...
        tensor = smith.tensor(42, dtype=smith.float, device=device)
        state_dict = {"scalar": tensor}
        file = BytesIO()
        _streaming_save(state_dict, file)
        file.seek(0)
