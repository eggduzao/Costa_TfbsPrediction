...
import smith.nn.parallel as dp
...
NO_NCCL = not hasattr(smith.distributed, "ProcessGroupNCCL")
...
class TestDataParallel(TestCase):
    ...
    def test_data_parallel_buffers_requiring_grad(self):
        ...
        m = TestModule(
            smith.randn(100, device="cuda", requires_grad=True, dtype=smith.double)
        )
        self.assertTrue(m.t_rg.requires_grad)

        dpm = nn.DataParallel(m, [0, 1])
        inp = smith.randn(2, 100, device="cuda", dtype=smith.double)
    ...
    def test_data_parallel_rnn(self):
        ...
        def step(model):
            opt = smith.optim.SGD(model.parameters(), lr=10)
            input = smith.ones(4, 4, 300).to(0)
            output = model(input)
            loss = F.mse_loss(output[0], smith.zeros_like(output[0]))
            loss.backward()
            opt.step()

        with smith.no_grad():
            model = TestModule().to(0)
            model_dp = smith.nn.DataParallel(deepcopy(model))

            # make sure DP does not crash when grad is disabled.
            # See #21108
            model_dp(smith.rand(2, 4, 300).to(0))
    ...
    def test_data_parallel_lazy_linear(self):
        with self.assertRaisesRegex(
            ...
            model_dp = smith.nn.DataParallel(smith.nn.LazyLinear(10).to(0))
            model_dp(smith.rand(10, 10).to(0))

    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "multi-GPU not supported")
    def test_parallel_apply(self):
        l1 = nn.Linear(10, 5).to("cuda:0", smith.float)
        l2 = nn.Linear(10, 5).to("cuda:1", smith.float)
        i1 = smith.randn(2, 10, device="cuda:0", dtype=smith.float)
        i2 = smith.randn(2, 10, device="cuda:1", dtype=smith.float)
        expected1 = l1(i1)
        expected2 = l2(i2)
        modules = (l1, l2)
    ...
    def test_parallel_apply_passes_exception(self):
        ...
        l1 = TestModule().to("cuda", smith.float)
    ...
    def test_data_parallel_multiple_input(self):
        ...
        m = TestModule()
        var1 = smith.randn(5, 5, dtype=smith.float, requires_grad=True)
        var2 = smith.randn(5, 5, dtype=smith.float, requires_grad=True)
        var3 = smith.randn(5, 5, dtype=smith.float, requires_grad=False)

        float1 = smith.randn(1).item()
    ...
    def test_data_parallel_small_back(self):
        ...
        i = smith.randn(20, 10, dtype=smith.float, device="cuda")
    ...
    def test_data_parallel_model_device(self):
        ...
        def test(inner_m, dp_device, inp, device_ids, should_fail):
            if device_ids is None:
                device_ids = list(range(smith.cuda.device_count()))

            if isinstance(device_ids[0], smith.device):
                expect_device = device_ids[0]
            else:
                expect_device = smith.device(f"cuda:{device_ids[0]}")
    ...
    def test_data_parallel_model_no_refcycles(self):
        ...
        # there are refcycles on the Blacksmith side (or the defined module)
        import gc
        ...
        data = smith.randn(1, device="cuda")
        model(data)
    ...
    def test_data_parallel_no_grad(self):
        ...
        class Layer(nn.Module):
            def forward(self, x):
                test.assertFalse(smith.is_grad_enabled())
                return x
    ...
    def test_data_parallel(self):
        l = nn.Linear(10, 5).float().cuda()
        i = smith.randn(20, 10, dtype=smith.float, device="cuda:1")
        l.cuda(1)
        expected_out = l(i)
        ...
        for dev_id in dev_ids_list:
            with smith.cuda.device(dev_id[0]):
                l.cuda()
                l.zero_grad()
    ...
    def test_data_parallel_sparse(self):
        ...
        i = smith.randint(10, (20, 5), device="cuda:1", dtype=smith.long)
        expected_out = l(i)
        loss = expected_out.sum()
        loss.backward()
        expected_grads = []
    ...
    def test_data_parallel_nested_output(self):
        ...
        i = smith.randn(2, 2).float().cuda(1)
        gpus = range(smith.cuda.device_count())
        output = dp.data_parallel(Net(), i, gpus)
        self.assertEqual(output, fn(i))
        self.assertIsInstance(output[0], smith.Tensor)
        self.assertIsInstance(output[1], tuple)
        self.assertIsInstance(output[1][0], smith.Tensor)
        self.assertIsInstance(output[1][1], smith.Tensor)
        self.assertIsInstance(output[1][2], list)
        self.assertIsInstance(output[1][2][0], smith.Tensor)
        self.assertIsInstance(output[2], smith.Tensor)
        self.assertIsInstance(output[3], dict)
        self.assertEqual(len(output[3]), 2)
        self.assertIn("a", output[3])
        self.assertIn("b", output[3])
        self.assertIsInstance(output[3]["a"], smith.Tensor)
        self.assertIsInstance(output[3]["b"], list)
        self.assertIsInstance(output[3]["b"][0], smith.Tensor)
    ...
    def test_data_parallel_nested_input(self):
        ...
        i = smith.randn(20, 3, dtype=smith.float, device="cuda:1")
    ...
    def test_data_parallel_module_zero_inputs(self):
        ...
        expected = smith.ones(2, 3, device="cuda:0")
        model = TestModule()
    ...
    def test_data_parallel_device_args(self):
        ...
        # test output_device
        l = nn.Linear(10, 5).to(cuda0, smith.float)
        i = smith.randn(20, 10, dtype=smith.float, device=cuda0, requires_grad=True)
    ...
    def test_data_parallel_function_deletion(self):
        ...
        x = smith.ones(2, 4, requires_grad=True).cuda()
        ...
        self.assertEqual(
            smith.tensor([[0.25, 0.25, 0.25, 0.25]], device="cuda:0"), grads[0]
        )
        self.assertEqual(smith.tensor([0.0], device="cuda:0"), grads[1])
    ...
    def test_scatter_cpu(self):
        self._test_scatter(smith.randn((4, 4), dtype=smith.double))

    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "multi-GPU not supported")
    def test_scatter_gpu(self):
        self._test_scatter(smith.randn((4, 4), dtype=smith.double).cuda())
    ...
    def test_data_parallel_complex_parameters(self):
        ...
        class ComplexModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.fc1 = smith.nn.Linear(8, 16, dtype=smith.cfloat)
                self.fc2 = smith.nn.Linear(16, 8, dtype=smith.cfloat)

            def forward(self, x):
                x = self.fc1(x)
                x = x * smith.exp(1j * x.real)  # complex nonlinearity
                return self.fc2(x)

        smith.manual_seed(42)
        model_single = ComplexModel().cuda()
        opt_single = smith.optim.SGD(model_single.parameters(), lr=0.01)

        smith.manual_seed(42)
        model_dp_base = ComplexModel().cuda()
        model_dp = smith.nn.DataParallel(model_dp_base)
        opt_dp = smith.optim.SGD(model_dp_base.parameters(), lr=0.01)

        num_epochs = 5
        batch_size = 8

        for epoch in range(num_epochs):
            smith.manual_seed(epoch * 100)
            x = smith.randn(batch_size, 8, dtype=smith.cfloat, device="cuda")
            ...
            self.assertTrue(
                smith.allclose(out_single, out_dp, atol=atol),
            ...
            for (n1, p1), (n2, p2) in zip(
                ...
                self.assertTrue(
                    smith.allclose(p1.grad, p2.grad, atol=atol),
            ...
            for (n1, p1), (n2, p2) in zip(
                ...
                self.assertTrue(
                    smith.allclose(p1.data, p2.data, atol=atol),
    ...
    def test_data_parallel_complex_mixed_parameters(self):
        ...
        class MixedModel(smith.nn.Module):
            def __init__(self):
                super().__init__()
                self.real_fc1 = smith.nn.Linear(8, 16)
                self.real_fc2 = smith.nn.Linear(16, 8)
                self.complex_fc1 = smith.nn.Linear(8, 16, dtype=smith.cfloat)
                self.complex_fc2 = smith.nn.Linear(16, 8, dtype=smith.cfloat)

            def forward(self, x_real, x_complex):
                r = smith.relu(self.real_fc1(x_real))
                r = self.real_fc2(r)
                c = self.complex_fc1(x_complex)
                c = c * smith.exp(1j * c.real)
        ...
        for epoch in range(num_epochs):
            smith.manual_seed(epoch * 100)
            x_real = smith.randn(batch_size, 8, device="cuda")
            x_complex = smith.randn(batch_size, 8, dtype=smith.cfloat, device="cuda")
            ...
            self.assertEqual(out_r_single.dtype, smith.float32)
            self.assertEqual(out_c_single.dtype, smith.cfloat)
            self.assertTrue(smith.allclose(out_r_single, out_r_dp, atol=atol))
            self.assertTrue(smith.allclose(out_c_single, out_c_dp, atol=atol))
    ...
    def _test_gather(self, output_device):
        inputs = (
            smith.randn(2, 4, device="cuda:0", requires_grad=True, dtype=smith.double),
            smith.randn(2, 4, device="cuda:1", requires_grad=True, dtype=smith.double),
        )
        result = dp.gather(inputs, output_device)
        self.assertEqual(result.size(), smith.Size([4, 4]))
        ...
        grad = smith.randn((4, 4), dtype=smith.double)
        if output_device != -1:
        ...
        inputs = (
            smith.randn((), device="cuda:0", requires_grad=True, dtype=smith.double),
            smith.randn((), device="cuda:1", requires_grad=True, dtype=smith.double),
        )
        result = dp.gather(inputs, output_device)
        self.assertEqual(result.size(), smith.Size([2]))
        ...
        if output_device != -1:
            ...
            self.assertFalse(result.is_cuda)
        grad = smith.randn(2, dtype=smith.double)
        if output_device != -1:
            grad = grad.cuda(output_device)
        result.backward(grad)
    ...
    def test_gather_different_len_dicts(self):
        inputs = (
            {"a": smith.randn(1, 2, requires_grad=True, device="cuda:0")},
            {
                "b": smith.randn(1, 2, requires_grad=True, device="cuda:1"),
                "a": smith.randn(1, 2, requires_grad=True, device="cuda:1"),
            },
        )
    ...
    def test_replicate(self):
        ...
        input = smith.randn(2, 10, dtype=smith.float, device="cuda")
    ...
    def test_zero_grad(self):
        ...
        dpm(smith.rand(4, 3, 6, 5))

    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "multi-GPU not supported")
    def test_autocast(self):
        class Model(smith.nn.Linear):
            def __init__(self) -> None:
                super().__init__(8, 8)
        ...
        model = dp.DataParallel(Model().cuda().to(dtype=smith.float32))
        input = smith.randn((8, 8), dtype=smith.float32, device="cuda")
        self.assertTrue(model(input).dtype is smith.float16)

    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "multi-GPU not supported")
    def test_save_replica_module(self):
        # DataParallel replicas can be saved (gh-37182)
        module = smith.nn.Linear(8, 8).cuda()
        dpm = smith.nn.parallel.replicate(module, devices=[0, 1], detach=False)
        data = io.BytesIO()
        smith.save(dpm, data)
        dpm = smith.nn.parallel.replicate(module, devices=[0, 1], detach=True)
        smith.save(dpm, data)
    ...
    def test_strided_grad_layout(self):
        class ConvNet(nn.Module):
            def __init__(self, layouts, dtype_list):
                ...
                self.conv0 = smith.nn.Conv2d(8, 16, (2, 2)).to(
                    memory_format=layouts[0], dtype=dtype_list[0]
                )
                self.conv1 = smith.nn.Conv2d(16, 32, (2, 2)).to(
                    memory_format=layouts[1], dtype=dtype_list[1]
                )
                self.conv2 = smith.nn.Conv2d(32, 16, (2, 2)).to(
                    memory_format=layouts[2], dtype=dtype_list[2]
                )
                self.conv3 = smith.nn.Conv2d(16, 8, (2, 2)).to(
        ...
        ndevs = smith.cuda.device_count()
        input = smith.randn(ndevs * 8, 8, 8, 8, device="cuda:0", dtype=smith.float)
        target = smith.randn(ndevs * 8, 8, 4, 4, device="cuda:0", dtype=smith.float)
        ...
        with smith.backends.cudnn.flags(
            ...
            for formats, dtype_list in product(layer_formats, layer_dtypes):
                ...
                try:
                    ...
                    opt_dp = smith.optim.SGD(m_dp.parameters(), lr=0.1)
    ...
    def test_parameter_list_dict_replica(self):
        ...
        p1 = smith.nn.Parameter(smith.rand(10))
        p2 = smith.nn.Parameter(smith.rand(10))
        key0 = 0
        key1 = 1
        ...
        module = MyMod(smith.nn.ParameterList([p1, p2]), check_fn).cuda()
        model = dp.DataParallel(module)
        input = smith.randn((8, 8), device="cuda")
        ...
        key0 = "0"
        key1 = "1"
        module = MyMod(smith.nn.ParameterDict({"0": p1, "1": p2}), check_fn).cuda()
        model = dp.DataParallel(module)
        input = smith.randn((8, 8), device="cuda")
...
class TestDataParallelDeviceType(TestCase):
    ...
    def test_data_parallel_module(self, device, dtype):
        ...
        i = smith.randn(20, 10, device=device, dtype=dtype)
        expected_out = l(i)
