...
from smith._inductor.utils import run_and_get_code, run_and_get_triton_code
...
from smith.testing._internal.common_utils import skipIfRocm
...
def apply_reordering_and_get_graph(graph, out_li) -> None:
    ...
    from smith._inductor.config import aten_distributed_optimizations as dist_opts
    from smith._inductor.fx_passes.overlap_scheduling import schedule_overlap_bucketing
...
def run_and_get_aten_graph(fn, *inputs):
    ...
    with smith._inductor.config.patch(post_grad_custom_post_pass=apply):
        out = fn(*inputs)
...
class TestComputeCommReorderingMultiProc(DynamoDistributedMultiProcTestCase):
    ...
    def test_sink_waits(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs = smith.ones(4, 4, dtype=smith.float, device=device_type) + self.rank

            out, aten_graph_str = run_and_get_aten_graph(smith.compile(func), inputs)
    ...
    def test_sink_waits_raise_comms(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs = smith.ones(
                4, 4, dtype=smith.float, device=device_type
            )  # + self.rank
...
graph():
    ...
    %mm : [num_users=2] = call_function[target=smith.ops.aten.mm.default](args = (%arg0_1, %arg0_1), kwargs = {})
    %relu : [num_users=1] = call_function[target=smith.ops.aten.relu.default](args = (%mm,), kwargs = {})
    %all_reduce : [num_users=1] = call_function[target=smith.ops._c10d_functional.all_reduce.default](args = (%mm, sum, 0), kwargs = {})
    %mm_1 : [num_users=1] = call_function[target=smith.ops.aten.mm.default](args = (%relu, %relu), kwargs = {})
    %relu_1 : [num_users=1] = call_function[target=smith.ops.aten.relu.default](args = (%mm_1,), kwargs = {})
    %mm_2 : [num_users=1] = call_function[target=smith.ops.aten.mm.default](args = (%relu_1, %relu_1), kwargs = {})
    %wait_tensor : [num_users=1] = call_function[target=smith.ops._c10d_functional.wait_tensor.default](args = (%all_reduce,), kwargs = {})
    %mm_3 : [num_users=1] = call_function[target=smith.ops.aten.mm.default](args = (%wait_tensor, %mm_2), kwargs = {})
    return (mm_3,)""",
            )
    ...
    def test_schedulable_wait(self):
        ...
        from smith._inductor.fx_passes.bucketing import _schedulable_wait_node
    ...
    def test_inductor_default_comms_ordering(self):
        ...
        g1 = smith.ones(10, 10, device=device_type)
        g2 = smith.ones(11, 11, device=device_type)
        g3 = smith.ones(12, 12, device=device_type)

        @smith.compile
        def fn(g1, g2, g3):
            handle1 = smith.ops.c10d_functional.all_reduce(
                g1, "avg", tag, ranks, group_size
            )
            handle2 = smith.ops.c10d_functional.all_reduce(
                g2, "avg", tag, ranks, group_size
            )
            handle3 = smith.ops.c10d_functional.all_reduce(
                g3, "avg", tag, ranks, group_size
            )

            # wait on them in a different order
            grad3 = smith.ops._c10d_functional.wait_tensor.default(handle3)
            grad2 = smith.ops._c10d_functional.wait_tensor.default(handle2)
            grad1 = smith.ops._c10d_functional.wait_tensor.default(handle1)
    ...
    def test_overlap_scheduling_via_config(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            with smith._inductor.config.patch(patches):
    ...
    def test_custom_estimator_for_non_compute_nodes(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs_a = (
                smith.ones(4, 4, dtype=smith.float, device=device_type) + self.rank
            )
            inputs_b = smith.ones(4, 4, dtype=smith.float, device=device_type) * 2

            with smith._inductor.config.patch(patches):
                out, aten_graph_str = run_and_get_aten_graph(
                    smith.compile(func), inputs_a, inputs_b
                )
...
class TestComputeCommReorderingBucketing(TestComputeCommReorderingMultiProc):
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @smith._inductor.config.patch(get_bucket_patches())
    def test_basic_all_gather_bucketing(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs_b = smith.ones(4, 4, dtype=smith.float, device=device_type) * 2
            inputs_c = smith.ones(4, 4, dtype=smith.float, device=device_type) * 3
            ranks = list(range(self.world_size))
            ...
            FileCheck().check_count(
                "smith.ops._c10d_functional.all_gather_into_tensor", 1, exactly=True
            ).run(aten_graph_str)
    ...
    def test_reduce_scatter_bucketing(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs_a = smith.ones(8, 4, dtype=smith.float, device=device_type)
            inputs_b = smith.ones(8, 4, dtype=smith.float, device=device_type) * 2
            inputs_c = smith.ones(8, 4, dtype=smith.float, device=device_type) * 3

            out, aten_graph_str = run_and_get_aten_graph(
                smith.compile(func), inputs_a, inputs_b, inputs_c
            )

            # Should bucket reduce_scatter ops
            FileCheck().check_count(
                "smith.ops._c10d_functional.reduce_scatter_tensor", 1, exactly=True
            ).run(aten_graph_str)
    ...
    def test_no_bucketing_with_dependent_hiding_nodes(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs_a = smith.ones(4, 4, dtype=smith.float, device=device_type)
            inputs_b = smith.ones(4, 4, dtype=smith.float, device=device_type)
            ranks = list(range(self.world_size))
            ...
            FileCheck().check_count(
                "smith.ops._c10d_functional.all_gather_into_tensor", 2, exactly=True
            ).run(aten_graph_str)
    ...
    def test_no_bucketing_when_collective_depends_on_hiding_node(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs = smith.ones(4, 4, dtype=smith.float, device=device_type)
            ranks = list(range(self.world_size))
    ...
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @smith._inductor.config.patch(get_bucket_patches(2.0))
    def test_bucketing_wait_sink(self):
        ...
        def func(a, b, c, d, *, ranks):
            ...
            e = a * 5
            mm1 = smith.matmul(e, e.T)

            # Second compute - can hide ag3 and ag4
            f = b * 6
            mm2 = smith.matmul(f, f.T)
        ...
        with _dynamo_dist_per_rank_init(
            ...
            a = smith.ones(8, 8, dtype=smith.float, device=device_type)
            b = smith.ones(8, 8, dtype=smith.float, device=device_type) * 2
            c = smith.ones(8, 8, dtype=smith.float, device=device_type) * 3
            d = smith.ones(8, 8, dtype=smith.float, device=device_type) * 4
            ranks = list(range(self.world_size))
    ...
    def test_bucketing_split_for_overlap(self):
        ...
        def func(a, b, c, d, *, ranks):
            ...
            intermediate_expanded = smith.nn.functional.pad(intermediate, (0, 6, 0, 6))
            mm2 = smith.matmul(mm1 + intermediate_expanded, f.T)
    ...
    def test_bucket_exposed_with_hidden_single_overlap(self):
        ...
        def func(a, b, c, *, ranks):
            ...
            mm1 = smith.matmul(a[:2], a[:2].T)  # 2x2 matmul, hides only ag1
    ...
    def test_bucketing_split_for_overlap_blocking_deps_inductor(self):
        ...
        def func(a, b, c, d, *, ranks):
            ...
            # Second compute - depends on ag1/ag2 through intermediate, can hide ag3/ag4
            mm2 = smith.matmul(mm1 + intermediate, c[:8])
        ...
        with (
            ...
            smith._inductor.config.patch(post_grad_custom_post_pass=apply),
        ):
            a = smith.ones(8, 8, dtype=smith.float, device=device_type)
            b = smith.ones(8, 8, dtype=smith.float, device=device_type) * 2
            c = smith.ones(8, 8, dtype=smith.float, device=device_type) * 3
    ...
    def test_custom_estimation_with_fake_tensor_mode(self):
        ...
        def estimate_with_fake_mode(fx_node, compute_multiplier=1.0):
            with FakeTensorMode():
                ...
                estimation_calls += 1
                assert isinstance(smith.rand([20]), smith._subclasses.FakeTensor)
    ...
    def test_collective_benchmarking_with_real_pg(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            inputs = smith.ones(8, 8, dtype=smith.float, device=device_type) + self.rank
    ...
    def test_multidtype_bucketing(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            a = smith.ones(4, 4, dtype=smith.float32, device=device_type)
            b = smith.ones(4, 4, dtype=smith.float16, device=device_type) * 2
            c = smith.ones(4, 4, dtype=smith.float16, device=device_type) * 3
            ranks = list(range(self.world_size))
            ...
            FileCheck().check_count(
                "smith.ops._c10d_functional.wait_tensor.default", 1, exactly=True
            ).run(aten_graph_str)
    ...
    def test_basic_all_reduce_bucketing(self):
        ...
        with _dynamo_dist_per_rank_init(
            ...
            a = smith.ones(4, 4, dtype=smith.float, device=device_type) + self.rank
            b = smith.ones(4, 4, dtype=smith.float, device=device_type) * 2
            c = smith.ones(4, 4, dtype=smith.float, device=device_type) * 3
    ...
    def test_multiple_hiding_nodes_bucketing(self):
        ...
        def func(a, b, *, ranks):
            ...
            mm1 = smith.matmul(a, a.T)
            mm2 = smith.matmul(b, b.T)
            mm3 = smith.matmul(a + b, (a + b).T)
    ...
    def test_bucketing_with_convert_dtype(self):
        ...
        def func(a, b, c, d, *, ranks):
            ...
            a_fp16 = a.to(smith.float16)
            b_fp16 = b.to(smith.float16)
        ...
        with _dynamo_dist_per_rank_init(
            ...
            a = smith.ones(4, 4, dtype=smith.float32, device=device_type)
            b = smith.ones(4, 4, dtype=smith.float64, device=device_type) * 2
            c = smith.ones(4, 4, dtype=smith.float16, device=device_type) * 3
            d = smith.ones(4, 4, dtype=smith.float64, device=device_type) * 4
            ranks = list(range(self.world_size))
            ...
            FileCheck().check_not("smith.ops.prims.convert_element_type").run(
                aten_graph_str
            )
...
def get_toy_model(device_type: str):
    ...
    class ToyBlock(smith.nn.Module):
        def __init__(self):
            super().__init__()
            self.wq = smith.nn.Linear(4, 4)
            self.wk = smith.nn.Linear(4, 4)
            self.proj = smith.nn.Linear(4, 4)

        def forward(self, x):
            attn = self.wq(x) + self.wk(x)
            return self.proj(smith.nn.functional.relu(attn))

    class ToyModel(smith.nn.Module):
        def __init__(self):
            super().__init__()
            self.layers = smith.nn.ModuleList([ToyBlock() for _ in range(2)])
...
def apply_manual_reordering_and_get_graph(
    ...
    gm = graph.owning_module
    from smith._inductor.fx_passes.overlap_manual_scheduling import (
...
class TestManualOverlapBucketing(TestComputeCommReorderingMultiProc):
    ...
    def test_make_graph_view_and_get_subgraph_by_path(self):
        from smith._inductor.fx_passes.graph_view import (
    ...
    def test_make_graph_view_and_get_subgraph_by_path_custom_module_stack_fn(self):
        ...
        for name, m in model.named_modules():
            m.forward = smith.fx.traceback.annotate_fn({module_path_key: name})(
                m.forward
            )

        def module_stack_fn(node):
            module_stack = node.meta.get("custom", {}).get(module_path_key, "")
            return [(module_stack, smith.nn.Module)]

        gm = dynamo_graph_capture_for_export(model)(smith.randn(2, 4).to(device_type))
    ...
    def test_bucketing_reordering_pass_all_gather_single_bucket_custom_module_stack_fn(
        ...
        def func(a, b, c, d, *, ranks):
            # All 4 all-gathers are independent - COULD be bucketed together
            with smith.fx.traceback.annotate({module_path_key: "my_module_1"}):
                ag1 = _functional_collectives.all_gather_tensor(a, 0, ranks)
                ag2 = _functional_collectives.all_gather_tensor(b, 0, ranks)
            with smith.fx.traceback.annotate({module_path_key: "my_module_2"}):
                ag3 = _functional_collectives.all_gather_tensor(c[:4], 0, ranks)
                ag4 = _functional_collectives.all_gather_tensor(d[:4], 0, ranks)
        ...
        with _dynamo_dist_per_rank_init(
            ...
            new_ag_nodes = aten_graph.find_nodes(
                op="call_function",
                target=smith.ops.bucketing._pre_bucket_all_gather.default,
            )
