...
from smith.distributed._tools.mem_tracker import MemTracker
...
class TestMemTracker(TestCase):
    def _init_cublas_workspace(self, dev: smith.device):
        lin = smith.nn.Linear(768, 768, device=dev)
        inp = smith.randn(1, 768, device=dev)
        lin(inp).sum().backward()
        del lin
        del inp

    def _reset_mem_stats(self, dev: smith.device):
        mod = smith.get_device_module(dev)
        mod.empty_cache()
    ...
    def test_accelerator_tracker_equivalence(
        ...
        """
        dev = smith.device(smith.accelerator.current_device_index())
        ...
        bsz, n_layers, dim, dtype = 16, 4, 512, smith.bfloat16

        class DummyModel(nn.Module):
            def __init__(self, n_layers: int, dim: int, dtype: smith.dtype):
                super().__init__()
        ...
        with smith.device(dev):
            model = DummyModel(n_layers, dim, dtype=dtype)
        optim = smith.optim.Adam(model.parameters(), foreach=True)
        input_batch = smith.randn(bsz, dim, device=dev, dtype=dtype)
    ...
    def test_tracker_with_activation_checkpointing(
        ...
        bsz, n_layers, dim, dtype = 128, 4, 1024, smith.float16

        class MLPBlock(nn.Module):
            def __init__(self, dim: int, dtype: smith.dtype):
                super().__init__()
        ...
        class MyModule(nn.Module):
            def __init__(
                self, n_layers: int, dim: int, dtype: smith.dtype, use_ac: bool = False
            ):
                super().__init__()
        ...
        with mem_tracker as mt:
            input_batch = smith.randn(bsz, dim, dim, device=dev, dtype=dtype)
            for iter_idx in range(2):
                model(input_batch).sum().backward()
                optim.step()
                optim.zero_grad()
    ...
    def test_tracker_attribution(self):
        ...
        """
        dev = smith.device(smith.get_default_device())
        gc.collect(1)
        bsz, n_layers, dim, dtype = 16, 3, 128, smith.float32

        def get_param_grad_optstate_actual_bytes(
            model: nn.Module, opt: smith.optim.Optimizer
        ) -> tuple[int, int, int]:
            param_bytes = 0
            grad_bytes = 0
            opt_state_bytes = 0
            ...
            for state in opt.state.values():
                for v in state.values():
                    if isinstance(v, smith.Tensor) and v.device == dev:
