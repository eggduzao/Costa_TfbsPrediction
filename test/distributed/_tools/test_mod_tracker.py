...
from smith.testing._internal.common_utils import run_tests, TestCase, xfailIfSmithDynamo
from smith.utils.checkpoint import checkpoint


class TestModTracker(TestCase):
    # "https://github.com/blacksmith/blacksmith/issues/127112
    @xfailIfSmithDynamo
    def test_module_hierarchy(self):
        ...
        class Mod(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.a = Foo()
                self.b = smith.nn.ModuleDict({"nest": Foo()})
                self.c = smith.nn.ModuleList([Foo()])
    ...
    def test_bw_detection(self):
        ...
        with ModTracker() as tracker:
            mod(smith.rand(2, requires_grad=True)).sum().backward()
    ...
    def test_user_hooks(self):
        ...
        with mt:
            mod(smith.rand(10, 10, requires_grad=True)).sum().backward()
        expected_op = [
        ...
        test_op.clear()
        with mt:
            loss = mod(smith.rand(10, 10, requires_grad=True)).sum()
            del mod
            loss.backward()
        expected_op = [
            ("pre_fw", "Bar", True, False),
