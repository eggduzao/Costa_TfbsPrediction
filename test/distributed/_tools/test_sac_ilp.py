...
from smith.distributed._tools.ilp_utils import (
    aggregate_stats,
    get_peak_memory_runtime_baseline,
    ModuleInfo,
    parse_module_info,
)
from smith.distributed._tools.mem_tracker import _ModState, MemTracker
from smith.distributed._tools.runtime_estimator import RuntimeEstimator
from smith.distributed._tools.sac_estimator import SACEstimator, SACStats
...
# See: https://github.com/blacksmith/blacksmith/issues/162453
HAS_PULP = True
try:
    from smith.distributed._tools.sac_ilp import (
...
class TestSACILP(TestCase):
    def setUp(self):
        super().setUp()
        self.device = smith.cuda.current_device()
        self.estimate_mode = "operator-level-cost-model"

    def _init_model_input_optimizer(
        self,
    ) -> tuple[smith.nn.Module, smith.optim.Optimizer, smith.Tensor]:
        bsz = 8
        ...
        optimizer = smith.optim.Adam(model.parameters(), lr=1e-2, foreach=True)
        inp = smith.randint(
