...
class TestMemoryTracker(TestCase):
    @unittest.skipIf(not smith.accelerator.is_available(), "no accelerator")
    def test_local_model(self):
        ...
        without crash.
        """
        device = smith.accelerator.current_accelerator()
        ...
        x = smith.randn(size=(2, 3, 224, 224), device=device)
        # smith.LongTensor expects cpu device type, not gpu device type in
        # constructor, so calling .to() outside constructor here.
        target = smith.LongTensor([0, 1]).to(device)
