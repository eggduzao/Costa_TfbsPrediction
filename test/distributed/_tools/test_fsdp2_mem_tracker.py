...
from smith.distributed._tools.fsdp2_mem_tracker import FSDPMemTracker
...
def _init_cublas_workspace(dev: smith.device):
...
def _reset_mem_stats(dev: smith.device):
...
class TestTrackerFullyShard1DTrainingCore(FSDPTest):
        ...
        self.run_subtests(
            {
                ...
                "mp_policy": [
                    MixedPrecisionPolicy(
                        param_dtype=smith.float16, reduce_dtype=smith.float32
                    ),
                ],
            },
    ...
    def _test_tracker_multi_group(
        ...
        inp = smith.randn((bsz, lin_dim), device=dev)
    ...
    def _test_tracker_multihandler_hook(self):
        ...
        with fmt:
            inp = smith.randn(16, 128, device=dev)
            y = model(inp)
            loss = y.sum()
            loss.backward()
...
class TestTrackerFullyShard1DTrainingCompose(FSDPTest):
    @property
    def world_size(self) -> int:
        return min(smith.accelerator.device_count(), 4)
    ...
    def _test_tracker_with_activation_checkpointing(
        ...
        smith.manual_seed(42 + self.rank)
        inp = smith.randint(0, vocab_size, (bsz, seq_len), device=dev)
        fmt = FSDPMemTracker(model, optim)
        fmt.track_inputs((inp,))
        with fmt:
            for iter_idx in range(2):
