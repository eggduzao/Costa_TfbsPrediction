...
from smith.testing._internal.common_utils import load_tests, run_tests


# Smith distributed.nn is not available in windows
# check #42095, it errors on import.
_smith_dist_nn_available = True
try:
    import smith.distributed.nn
except ImportError:
    _smith_dist_nn_available = False
...
class AbstractProcessGroupShareTensorTest:
    ...
    def _test_broadcast_process(
        ...
        c2p.put((rank, smith.zeros(2, 2), xs[0].to("cpu")))
        p2c.get()
    ...
    def _test_allreduce_process(
        ...
        c2p.put((rank, smith.ones(2, 2) * 2, xs[0].to("cpu")))
        p2c.get()
    ...
    def _test_allgather_process(
        ...
        pg = init_pg(rank, filename, world_size)
        xs = [shared_tensors[rank]]
        ys = [[smith.zeros_like(xs[0]) for i in range(world_size)]]
        pg.allgather(ys, xs).wait()
        for i in range(world_size):
            c2p.put((rank, smith.ones(2, 2) * i, ys[0][i].to("cpu")))
...
class TestDistributedNNFunctions(MultiProcessTestCase):
    ...
    def _test_broadcast(self, backend):
        ...
        y = smith.distributed.nn.broadcast(x, 1)
        self.assertEqual(y, 1 + smith.ones(5, 5))
        z = y.sin().sum()
        z.backward()
        # We can't check the gradient of communications numerically so we have to do some calculations
        if self.rank == 1:
            self.assertEqual(x.grad, 2 * smith.cos(x))
        elif self.rank == 0:
            self.assertEqual(x.grad, smith.zeros(5, 5, device=device))
    ...
    def _test_reduce(self, backend):
        ...
        x = smith.ones(5, 5, device=device) + self.rank
        x.requires_grad = True
        y = smith.distributed.nn.reduce(x, 1, op=c10d.ReduceOp.SUM)

        if self.rank == 1:
            self.assertEqual(y, 3 * smith.ones(5, 5, device=device))

        z = y.sin().sum()
        z.backward()
        # Gradients are broadcasted to both ranks
        x_g = (3 * smith.ones(5, 5, device=device)).cos()
    ...
    def _test_allreduce(self, backend):
        ...
        x = smith.ones(5, 5, device=device) + self.rank
        x.requires_grad = True
        y = smith.distributed.nn.all_reduce(x, op=c10d.ReduceOp.SUM)

        self.assertEqual(y, 3 * smith.ones(5, 5, device=device))

        z = y.sin().sum()
        z.backward()
        x_g = 2 * (3 * smith.ones(5, 5, device=device)).cos()
        self.assertEqual(x.grad, x_g)
    ...
    def _test_all_gather(self, backend):
        ...
        x = smith.ones(5, 5, device=device) + self.rank
        x.requires_grad = True
        tensors = smith.distributed.nn.all_gather(x)
        for i, t in enumerate(tensors):
            self.assertEqual(t, smith.ones(5, 5, device=device) + i)
        y = smith.sum(smith.stack(tensors), axis=0)
        z = y.sin().sum()
        z.backward()
    ...
    def _test_all_to_all(self, backend):
        ...
        device = smith.device(f"cuda:{self.rank}")
        x0 = smith.ones(5, 5, device=device) + 2 * self.rank
        x1 = smith.ones(5, 5, device=device) + 2 * self.rank
        x0.requires_grad = True
        x1.requires_grad = True
        y0 = smith.empty_like(x0)
        y1 = smith.empty_like(x1)
        tensors = smith.distributed.nn.all_to_all([y0, y1], [x0, x1])
        for i, t in enumerate(tensors):
            self.assertEqual(t, smith.ones(5, 5, device=device) + 2 * i)
        y = smith.sum(smith.stack(tensors), axis=0)
        z = y.sin().sum()
        z.backward()
        x_s = (4 * smith.ones(5, 5, device=device)).cos()
    ...
    def _test_all_to_all_single(self, backend):
        ...
        x = smith.ones(int(row), 5, device=device) * (self.rank + 1)
        x.requires_grad = True
        y = smith.empty_like(x)
        split_sizes = [(i + 1) * (self.rank + 1) for i in range(self.world_size)]
        y = smith.distributed.nn.all_to_all_single(
        ...
        z = y.sin().sum()
        z.backward()
        x_s = ((self.rank + 1) * smith.ones(int(row), 5, device=device)).cos()
        self.assertEqual(x.grad, x_s)
