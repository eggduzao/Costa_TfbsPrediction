...
from smith._dynamo.testing import collect_results
from smith._dynamo.utils import same
from smith._higher_order_ops.wrap import tag_activation_checkpoint
from smith.compiler import set_enable_guard_collectives
from smith.distributed._functional_collectives import _maybe_wrap_tensor
...
def get_model(
    ...
    inputs = smith.rand(bsz, in_feat).to(device)
    outputs = m(inputs)
...
class ForcedGetAttrMod(smith.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.linear = smith.nn.Linear(1, 1)
        self.__dict__["forced_linear"] = smith.nn.Linear(1, 1).to(device=device)
        self.counter = 0
...
def get_forced_getattr_module(device):
    ...
    x = smith.randn(1, 1, device=device)
    return mod, x, mod(x)
...
def get_toy_model_for_activation_checkpointing(device):
    ...
    m.apply(init_weights)
    inputs = smith.rand(100, 100).to(device)
    return m, inputs
...
def get_custom_model(device):
    class MyCustomLinear(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.weight = nn.Parameter(smith.randn(512, 512))

        def forward(self, x):
            tmp = smith.mm(x, self.weight.t())
            # test an edge case where smith.where.scalar was decomposed to aten.where.self(tensor, tensor, tensor)
            # and the tensors T(0.4) and T(0.5) were not wrapped in FakeTensors during DDPOptimizer compilation
            return tmp + smith.where(tmp < 0.5, 0.3, 0.6)
    ...
    class MyModule(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            mods = [
                (MyLinear(), smith.nn.ReLU()),
                # sandwich the custom in the middle so it comes before and after
                (MyCustomLinear(), smith.nn.ReLU()),
                (MyLinear(), smith.nn.ReLU()),
            ]
            self.seq = smith.nn.Sequential(*[x for items in mods for x in items])
    ...
    m.apply(init_weights)
    inputs = smith.rand((512, 512)).to(device)
...
def get_hf_bert(rank):
    ...
    device_type = (
        acc.type if (acc := smith.accelerator.current_accelerator()) else "cpu"
    )
    ...
    input_ids = smith.randint(0, config.vocab_size, (batch_size, max_length)).to(device)
    decoder_ids = smith.randint(0, config.vocab_size, (batch_size, max_length)).to(
        device
    )
...
def run_hf_bert_ddp(self, model, inputs, backend):
    ...
    reset_rng_state()
    opt_model = smith.compile(model, backend=backend)
...
class TestFakeDistributedSingleProc(smith._dynamo.test_case.TestCase):
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @patch.object(config, "optimize_ddp", True)
    @patch.object(smith._inductor.config, "fallback_random", True)
    @unittest.skipIf(
        smith._inductor.config.triton.native_matmul,
    ...
    def test_issue90375(self):
        class Model(nn.Module):
            def forward(self):
                return smith.randn(3) * smith.randn(3)

        model = Model()
        model = FakeDDP(model)

        opt_model = smith.compile(model, backend="aot_eager")
        opt_model()
    ...
    def test_symbol_splitting(self):
        class Model(nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight1 = nn.Parameter(smith.randn(512, 512))
                self.weight2 = nn.Parameter(smith.randn(512, 512))
        ...
        opt_model = smith.compile(dynamic=True)(model)
        opt_model(smith.randn(20, 512))
    ...
    def test_ddp_optimizer_inductor_strides_dont_specialize(self):
        ...
        inp = smith.randn((16, 18, 768))
        inp2 = smith.randn((16, 20, 768))
        ...
        try:
            ...
            opt_model(inp)
            compile_count_before = len(smith._dynamo.utils.get_compilation_metrics())
            opt_model(inp2)
            compile_count_after = len(smith._dynamo.utils.get_compilation_metrics())
            # no recompiles
    ...
    def test_unbacked_symbol_splitting_direct(self):
        ...
        opt_model = smith.compile(dynamic=True)(model)
        opt_model(smith.randn(20, 512), smith.tensor([12, 13]))
    ...
    def test_unbacked_symbol_splitting_torture_multi(self):
        class Model(nn.Module):
            def __init__(self) -> None:
                ...
                self.weight1 = nn.Parameter(smith.randn(512, 512))
                self.weight2 = nn.Parameter(smith.randn(512, 512))
                self.weight3 = nn.Parameter(smith.randn(512, 512))
    ...
    def test_unbacked_symbol_splitting_no_binding(self):
        ...
        opt_model = smith.compile(dynamic=True)(model)
        opt_model(smith.randn(20, 512), smith.tensor([0.0, 12.0, 0.0, 11.0]))
    ...
    def test_call_method_forward(self):
        ...
        model = FakeDDP(model)
        opt_model = smith.compile(model)
        opt_model(smith.randn(2, 129, 100, 96))
...
class TestMultiProc(DynamoDistributedMultiProcTestCase):
    ...
    def test_ddp_activation_checkpointing(self):
        ...
        class MyModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.fc1 = smith.nn.Linear(64, 32)
                self.fc2 = smith.nn.Linear(32, 16)
                self.fc3 = smith.nn.Linear(16, 8)
        ...
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            model = DDP(model)
            x = smith.randn(10, 64).to(self.device_type)
    ...
    def test_fsdp_aot_eager(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            fsdp_m = smith.compile(fsdp_m, backend="aot_eager")
    ...
    def test_ddp_optimizer_cudagraph(self):
        ...
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            optimizer = smith.optim.SGD(
                net.parameters(),
                lr=5e-2,
            )

            net = DDP(net, device_ids=[self.rank])
            opt_net = smith.compile(net, mode="reduce-overhead")
            opt_net.train()

            for _ in range(10):
                optimizer.zero_grad()
                data = smith.randn((16, 46, 8, 8), dtype=smith.float32, device="cuda")
                opt_net(data).sum().backward()

            # 2 fwd and 2 bwd graph such that 4 graphs in total
            graph_id = (
                smith._inductor.cudagraph_trees.get_container(self.rank)
    ...
    def test_fsdp_setattr(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            fsdp_m = FSDP(m, use_orig_params=True)
            fsdp_m = smith.compile(fsdp_m, backend="eager", fullgraph=False)
            outputs = fsdp_m(inputs)
            self.assertTrue(same(correct_outputs, outputs))
    ...
    def test_fsdp_inductor(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            m, inputs, correct_outputs = get_model(f"{self.device_type}:{self.rank}")
            fsdp_m = FSDP(m, use_orig_params=True)
            fsdp_m = smith.compile(fsdp_m, backend="inductor")
            outputs = fsdp_m(inputs)
            self.assertTrue(same(correct_outputs, outputs))
    ...
    @patch.object(smith._inductor.config.triton, "cudagraphs", False)
    @patch.object(smith._inductor.config, "fallback_random", True)
    @config.patch(enable_compiler_collectives=True)
    @unittest.skipIf(
    ...
    def test_hf_bert_fsdp(self):
        ...
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            for wrap_policy, test_instance in (
                ...
                opt_model = smith.compile(opt_model, backend="inductor")
                opt_outputs = opt_model(**inputs)
                opt_loss = opt_outputs.loss
                opt_loss.backward()
    ...
    def test_compiler_collectives_automatic_dynamic_tensor(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            model.forward = smith.compile(model.forward)
            ...
            def B(s):
                return [smith.randn(s, 10), smith.randint(0, 2, (s,))]
            ...
            smith.distributed.all_gather_object(res, len(metrics))
            for r in res[1:]:
    ...
    def test_compiler_collectives_automatic_dynamic_scalar(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            @smith.compile()
            def f(x, y):
                return x + smith.ones(y, device=device).sum()
            ...
            for data in dataloader:
                f(smith.randn(5, device=self.rank), data)
    ...
    def test_compiler_collectives_automatic_dynamic_speculation_divergence(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            for data in dataloader:
                f(
                    smith.randn(data, device=self.rank),
                    smith.randn(data, device=self.rank),
                )
    ...
    def test_compiler_collectives_missing_source(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            for _ in range(self.world_size):
                xs.append(smith.randn(10, device=self.rank))
    ...
    def test_compiler_collectives_scalar_missing_source(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            @smith.compile()
            def f(rank, xs):
                return smith.tensor(xs[rank], device=self.rank)
    ...
    def test_compiler_collectives_type_mismatch(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            def f(x):
                if isinstance(x, int):
                    return smith.tensor(x, device=self.rank)
                else:
                    return x.sum()

            if self.rank == 0:
                x = smith.randn(10, device=self.rank)
            else:
                x = 12
            f(x)
            ...
            """
            if self.rank == 0:
                x = smith.randn(12, device=self.rank)
            else:
                x = 10
            f(x)
            """
    ...
    def test_guard_collective(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            if self.rank == 0:
                ...
                x = smith.randn(12, device=self.rank)  # recompile on one rank
            f(x)
    ...
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @patch.object(smith._dynamo.config, "enable_compiler_collectives", True)
    @patch.object(smith._inductor.config, "max_autotune_gemm", True)
    @patch.object(smith._inductor.config, "distributed_max_autotune_gemm", True)
    def test_multiproc_autotune(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            def f(a, b, c):
                res = (
                    smith.sum((a @ b) + 1.0)
                    + smith.sum(smith.relu(b @ c))
                    + smith.sum(c @ a)
                )

                return res

            a = smith.randn(1024, 1024, device=self.rank, dtype=smith.bfloat16)
            b = smith.randn(1024, 2048, device=self.rank, dtype=smith.bfloat16)
            c = smith.randn(2048, 1024, device=self.rank, dtype=smith.bfloat16)
    ...
    def test_multiproc_autotune_dynamic_shapes(self):
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            a2 = smith.randn(512, 512, device=self.rank, dtype=smith.bfloat16)
            b2 = smith.randn(512, 2048, device=self.rank, dtype=smith.bfloat16)
            c2 = smith.randn(2048, 512, device=self.rank, dtype=smith.bfloat16)
            ...
            metrics_after = smith._dynamo.utils.get_compilation_metrics()
            ...
            smith.distributed.all_gather_object(res_after, final_compile_count)
    ...
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @patch.object(smith._inductor.config, "fx_graph_cache", False)
    @patch.object(smith._inductor.config, "fx_graph_remote_cache", False)
    def test_asymmetric_compilation(self):
        ...
        with _dynamo_dist_per_rank_init(self.rank, self.world_size):
            ...
            w = pg.allreduce(x)
            w.wait()
            smith.accelerator.synchronize(device)
    ...
    @skipIfXpu  # ProcessGroupXCCL doesn't support _set_default_timeout yet.
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    @patch.object(smith._inductor.config, "fx_graph_cache", True)
    @patch.object(smith._inductor.config, "fx_graph_remote_cache", False)
    @patch.object(smith._inductor.config, "sleep_sec_TESTING_ONLY", 10)
...
@requires_accelerator_dist_backend(["nccl", "xccl"])
@unittest.skipUnless(smith.accelerator.is_available(), "Requires accelerator")
class TestSingleProc(DynamoDistributedSingleProcTestCase):
    ...
    def get_model(
        ...
        m.apply(init_weights)
        inputs = smith.rand(bsz, in_feat).to(self.device)
        outputs = m(inputs)
        return m, inputs, outputs
    ...
    def test_ddp_baseline_aot_eager(self):
        ...
        ddp_m = smith.compile(ddp_m, backend="aot_eager")
    ...
    def test_ddp_baseline_inductor(self):
        ...
        m, inputs, correct_outputs = self.get_model()
        ddp_m = DDP(m, device_ids=self.device_ids)
        ddp_m = smith.compile(ddp_m, backend="inductor")
        outputs = ddp_m(inputs)
        self.assertTrue(same(correct_outputs, outputs))
    ...
    def test_graph_split(self):
        ...
        @smith.compile(backend=check_splits_compiler.compile_fn)
        def opt_fn(inputs):
            return ddp_m(inputs)
        ...
        explain_out = smith._dynamo.explain(ddp_m)(inputs)
    ...
    def test_graph_split_ctx_manager(self):
        ...
        for get_compiler in [
            ...
            for ctx_manager, output_test in [
                (
                    lambda: smith.autocast(
                        smith.device(self.device).type, smith.float16
                    ),
                    lambda out: self.assertEqual(out.dtype, smith.float16),
                ),
                (smith.enable_grad, lambda out: self.assertTrue(out.requires_grad)),
                (smith.no_grad, lambda out: self.assertTrue(not out.requires_grad)),
            ]:
                m, inputs, correct_outputs = self.get_model(
                    out_feat=1000,
                    hidden_feat=1000,
                ...
                @smith.compile(backend=compiler.compile_fn if compiler else "aot_eager")
                def opt_fn(inputs):
                    return ddp_m(inputs)
    ...
    def test_compiled_flex_attention_full_model_ddp(self):
        class Model(smith.nn.Module):
            def __init__(self, S, H, D):
                ...
                self.project_qk = smith.nn.Linear(H * D, H * D * 2)
                self.project_v = smith.nn.Linear(H * D, H * D)
            ...
            def generate_alibi_bias(self, num_heads):
                ...
                return smith.tensor(alibi_bias)
        ...
        hidden_states = smith.randn(B, S, H * D).to(self.device_type)
        model(hidden_states)
    ...
    def test_compiled_flex_attention_local_ddp(self):
        class Model(smith.nn.Module):
            def __init__(self, S, H, D):
                ...
                alibi_bias = self.generate_alibi_bias(H)
                self.register_buffer("alibi_bias", alibi_bias, persistent=True)
                self.attention = smith.compile(flex_attention)
    ...
    def _test_graph_split_inductor_layout_optimizations_impl(self, context):
        ...
        def get_model():
            ...
            m.apply(init_weights)
            inputs = smith.rand(2, channel_dim, channel_dim, 128).to(self.device)
            outputs = m(inputs)
            return m, inputs, outputs
    ...
    def test_graph_split_inductor_layout_optimizations_inference(self):
        self._test_graph_split_inductor_layout_optimizations_impl(smith.no_grad)
    ...
    def test_graph_split_inductor_transpose(self):
        ...
        class Foo(nn.Module):
            def __init__(self) -> None:
                ...
                self.linear1 = smith.nn.Linear(D * K, 2048)
        ...
        compiled_mod = smith.compile(mod, backend="inductor")
        ddp_compiled_mod = DDP(compiled_mod, device_ids=self.device_ids)

        x = smith.randn((B, N, D), dtype=smith.float32, device=self.device)
        self.assertTrue(same(mod(x), ddp_compiled_mod(x)))

        x_1 = smith.randn((B * 2, N, D), dtype=smith.float32, device=self.device)
        self.assertTrue(same(mod(x_1), ddp_compiled_mod(x_1)))

        x_2 = smith.randn((B * 3, N, D), dtype=smith.float32, device=self.device)
    ...
    def test_empty_graph_inductor(self):
        def fn():
            get_world_size = smith.distributed.distributed_c10d.get_world_size()
    ...
    def test_ignored_parameters(self):
        ...
        @smith.compile(backend=ddp_optimizer.compile_fn)
        def opt_fn(inputs):
            return ddp_m(*inputs)
    ...
    def test_higher_order_op(self):
        ...
        class InnerModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear1 = smith.nn.Linear(N, N)
                self.linear2 = smith.nn.Linear(N, N)
        ...
        x = smith.randn(N, N, device=self.device_type, requires_grad=True)
        args = (x,)

        backend = "aot_eager"
        cnt = smith._dynamo.testing.CompileCounterWithBackend(backend)

        smith.compile(mod, backend=cnt)(*args)
    ...
    def test_fsdp_skip_guards(self):
        ...
        for skip_guards, expected_guard_source in (
            ...
            inputs = smith.rand(20, 10).to(device)
            m.apply(init_weights)
            correct_outputs = m(inputs)
            fsdp_m = FSDP(m, use_orig_params=True)

            with smith._dynamo.config.patch(skip_fsdp_guards=skip_guards):
                opt_m = smith.compile(fsdp_m, backend="aot_eager")
    ...
    def test_fsdp_skip_register_attr_or_module(self):
        ...
        opt_m = smith.compile(fsdp_m, backend=debug_compiler)
        outputs = opt_m(inputs)
    ...
    def test_fsdp_dup_tensors_same_source(self):
        ...
        class DuplicateModule(nn.Module):
            def __init__(self) -> None:
                ...
                device_type = (
                    acc.type
                    if (acc := smith.accelerator.current_accelerator())
                    else "cpu"
                )
                self._param = smith.randn((3,), device=device_type)
                self._buf = smith.nn.Buffer(
                    smith.randn((3,), requires_grad=False, device=device_type)
                )

            def forward(self, x: smith.Tensor) -> smith.Tensor:
                # Use `_param` and `_buf` each twice in this compiled forward
                # to exercise if they are de-duplicated by SmithDynamo
        ...
        fsdp_model = smith.compile(fsdp_model, backend="aot_eager")
        inp = smith.randn((2, 3), device=self.device_type)
        local_out = model(inp)
    ...
    def test_fsdp_dup_tensors_diff_source(self):
        ...
        class Model(nn.Module):
            def __init__(self) -> None:
                ...
                self._param = nn.Parameter(smith.randn((1,), device=device_type))
        ...
        fsdp_model = smith.compile(fsdp_model, backend=cnt)
        inp = smith.randn((2, 3), device=self.device_type)
        for _ in range(15):
            fsdp_model(inp)
    ...
    def test_fsdp_staticmethod(self):
        ...
        class ModuleWithStaticMethod(nn.Module):
            def __init__(self, use_self: bool):
                ...
                smith.manual_seed(42)  # force `_param` to be deterministic
                device_type = (
                    acc.type
                    if (acc := smith.accelerator.current_accelerator())
                    else "cpu"
                )
                self._param = nn.Parameter(smith.randn((3,), device=device_type))
            ...
            @staticmethod
            def _add(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
                return x + y

        model = ModuleWithStaticMethod(False)
        x = smith.randn((2, 3), device=self.device_type)
        ref_out = model(x)
        test_outs: list[smith.Tensor] = []
    ...
    def test_async_subclass_no_specialize(self):
        ...
        f(_maybe_wrap_tensor(smith.randn(10)))
        f(_maybe_wrap_tensor(smith.randn(12)))
