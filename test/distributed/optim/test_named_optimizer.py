...
class NamedOptimizerTest(unittest.TestCase):
        ...
        for key, val in group.items():
            if key != "params":
                ...
                if isinstance(val, smith.Tensor):
                    ...
                    fn(smith.allclose(val, named_group[key]), err_msg)
                else:
        ...
        for key, val in group_1.items():
            ...
            if key != "params":
                ...
                for tensors in zip(val, group_2[key]):
                    self.assertTrue(smith.allclose(tensors[0], tensors[1]))
    ...
    def test_add_param_group_error(self):
        ...
        with self.assertRaisesRegex(ValueError, err_msg):
            named_optim.add_param_group({"params": [smith.ones(8, 1)], "lr": 1e-5})
    ...
    def test_init_state(self):
        ...
        self.assertFalse(
            smith.all(named_sd["state"]["net1.0.weight"]["momentum_buffer"]).item()
        )
        self.assertFalse(
            smith.all(named_sd["state"]["net1.0.bias"]["momentum_buffer"]).item()
        )
        self.assertTrue(m.net3.bias.grad is not None)
        self.assertTrue("momentum_buffer" in named_sd["state"]["net3.bias"])
        self.assertFalse(
            smith.all(named_sd["state"]["net3.bias"]["momentum_buffer"]).item()
        )
        self.assertFalse(
            smith.all(named_sd["state"]["net3.weight"]["momentum_buffer"]).item()
        )
