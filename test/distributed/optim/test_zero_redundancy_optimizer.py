...
if not dist.is_available():
    ...
    sys.exit(0)
from smith.distributed.algorithms.ddp_comm_hooks.ddp_zero_hook import (
    hook_with_zero_step,
    hook_with_zero_step_interleaved,
)
from smith.distributed.algorithms.ddp_comm_hooks.default_hooks import allreduce_hook
from smith.distributed.algorithms.join import Join, Joinable, JoinHook
from smith.distributed.optim import ZeroRedundancyOptimizer
from smith.distributed.optim.zero_redundancy_optimizer import _broadcast_object
from smith.nn.parallel import DistributedDataParallel as DDP
from smith.optim import AdamW, SGD
...
device_type = (
    acc.type
    if (acc := smith.accelerator.current_accelerator(check_available=True))
    else "cpu"
)
...
def deterministic_algorithms(enabled=True):
    prev_state = smith.are_deterministic_algorithms_enabled()
    smith.use_deterministic_algorithms(enabled)
    try:
        yield
    finally:
        smith.use_deterministic_algorithms(prev_state)
...
class TestZeroRedundancyOptimizerSingleRank(TestZeroRedundancyOptimizer):
    def test_state_dict(self):
        ...
        x = smith.tensor([1.0], device=self.device, requires_grad=True)
        ...
        x.backward()
        o.step()
        self.assertEqual(x, smith.tensor([0.9], device=self.device))
        self.assertEqual(
            o.optim.state[x]["momentum_buffer"],
            smith.tensor([1.0], device=self.device),
        )
        ...
        # Check that the state dict has keys compliant with Blacksmith
        ...
        x.backward()
        o.step()
        self.assertEqual(x, smith.tensor([0.71], device=self.device))
        self.assertEqual(
            o.optim.state[x]["momentum_buffer"],
            smith.tensor([1.9], device=self.device),
        )
    ...
    def test_lr_scheduler(self):
        """Check that a normal Blacksmith ``lr_scheduler`` is usable with
        ZeroRedundancyOptimizer."""
        self.create_pg(self.device)
        NUM_ITERS = 5
        LR = 0.01
        x = smith.tensor([1.0], device=self.device, requires_grad=True)
        x2 = smith.tensor([1.0], device=self.device, requires_grad=True)
        o = ZeroRedundancyOptimizer([x], optimizer_class=SGD, lr=LR)
        o2 = smith.optim.SGD([x2], lr=LR)
        s = smith.optim.lr_scheduler.StepLR(o, 1)
        s2 = smith.optim.lr_scheduler.StepLR(o2, 1)
    ...
    def test_step_with_kwargs(self):
        ...
        class SGDWithStepKWArg(smith.optim.SGD):
    ...
    def test_step_with_extra_inner_key(self):
        ...
        class SGDWithNewKey(smith.optim.SGD):
    ...
    def test_step_without_closure(self):
        ...
        class SGDWithoutClosure(smith.optim.SGD):
            def step(self):
                return super().step()
    ...
    def test_zero_grad(self):
        ...
        y = m(x)
        y.backward(x)
        self.assertNotEqual(m.weight.grad, smith.zeros_like(m.weight))
        self.assertNotEqual(m.weight.grad, smith.zeros_like(m.weight))
        o.zero_grad()
    ...
    def test_constructor(self):
        ...
        ctor_inputs = [
            ...
            (smith.randn(1), TypeError),  # non-iterable: `smith.Tensor`
    ...
    def test_same_dense_param_type(self):
        ...
        inputs = [
            ...
            [smith.FloatTensor(1), smith.DoubleTensor(1)],
            [
                smith.FloatTensor(1),
                smith.FloatTensor(1),
                smith.sparse_coo_tensor(size=(2, 3)),
...
class TestZeroRedundancyOptimizerDistributed(TestZeroRedundancyOptimizer):
    @property
    def world_size(self):
        return min(4, max(2, smith.get_device_module(self.device).device_count()))

    @property
    def context(self):
        if requires_ddp_rank(self.device):
            return smith.get_device_module(self.device).device(self.rank)
        else:
            return nullcontext()

    def _check_same_model_params(
        self,
        model_a: smith.nn.Module,
        model_b: smith.nn.Module,
        message: str = "",
    ) -> None:
    ...
    def test_step(self):
        ...
        with self.context:
            x = smith.tensor([float(self.rank + 1)], device=self.device)
            m = smith.nn.Linear(1, 1)
            m.weight.data = smith.tensor([[1.0]])
            m.bias.data = smith.tensor([2.0])
            m = m.to(self.device)
    ...
    def test_step_with_closure(self):
        ...
        with self.context:
            for bucket_view in [False, True]:
                ...
                loss_fn = smith.nn.L1Loss()

                x = smith.tensor([float(x_val)], device=self.device)
                m = smith.nn.Linear(1, 1)
                m.weight.data = smith.tensor([[weight]])
                m.bias.data = smith.tensor([bias])
                m.to(self.device)
                ...
                self.assertEqual(loss, smith.tensor(error))
                self.assertEqual(m.weight, smith.tensor([[1.1]]))
                self.assertEqual(m.bias, smith.tensor([2.1]))
    ...
    def test_sharding(self):
        ...
        for size in sizes * self.world_size:
            params.append(smith.rand(size, 1))
    ...
    def test_add_param_group(self):
        ...
        def all_trainable():
            ...
            o.add_param_group({"params": [smith.rand(3, 1)]})
    ...
    def test_multiple_param_groups(self):
        ...
        WD, LR = 0.01, 0.01
        model1 = smith.nn.Sequential(
            smith.nn.Linear(INPUT_DIM, HIDDEN_DIM),
            smith.nn.Linear(HIDDEN_DIM, HIDDEN_DIM),
            smith.nn.Linear(HIDDEN_DIM, OUTPUT_DIM),
        )
        ...
        inputs = [
            smith.randn(BATCH_SIZE, INPUT_DIM).to(self.device) for _ in range(NUM_ITERS)
        ]
        ...
        for input in inputs:
            ...
            for layer1, layer2, layer3 in zip(model1, model2, model3):
                smith.testing.assert_close(layer1.weight, layer2.weight)
                smith.testing.assert_close(layer1.weight, layer3.weight)
                smith.testing.assert_close(layer1.bias, layer2.bias)
                smith.testing.assert_close(layer1.bias, layer3.bias)
    ...
    def test_collect_shards(self):
        ...
        REFERENCE_RANK = 0
        target = smith.rand((BATCH_SIZE, OUTPUT_DIM), device=self.device)
        inputs = smith.rand((BATCH_SIZE, INPUT_DIM), device=self.device)
    ...
    def test_nondefault_process_group(self):
        ...
        if smith.get_device_module(self.device).device_count() < self.world_size:
        ...
        loss_fn = smith.nn.L1Loss().to(device)

        def check(optimizer):
            for _ in range(EPOCHS):
                target = smith.rand((BATCH_SIZE, OUTPUT_DIM), device=device)
                inputs = smith.rand((BATCH_SIZE, INPUT_DIM), device=device)
    ...
    def test_local_optimizer_parity(
        ...
        if optimizer_class_str == "Adam":
            optimizer_class = smith.optim.Adam
        elif optimizer_class_str == "AdamW":
            optimizer_class = smith.optim.AdamW
        ...
        with self.context:
            ...
            model.test_buffer = smith.nn.Buffer(
                smith.ones((1), device=self.device) * self.rank,
            )
            ...
            def check_step():
                input_tensor = smith.rand((BATCH_SIZE, INPUT_DIM)).to(self.device)
    ...
    def _test_zero_join(self, device):
        ...
        if "cpu" not in device:
            smith.get_device_module(device).manual_seed(0)
        ...
        local_optim = smith.optim.Adam(ddp_model.parameters(), lr=LR)
        ...
        inputs = [smith.randn(20, 2).to(device) for _ in range(NUM_INPUTS + rank)]
        labels = smith.randn(20, 3).to(device)
    ...
    def _test_zero_model_parallel(self, parameters_as_bucket_view: bool, device: str):
        ...
        class ModelParallelModel(smith.nn.Module):
            def __init__(self, dev0, dev1):
                super().__init__()
                self.dev0 = dev0
                self.dev1 = dev1
                self.net0 = smith.nn.Linear(10, 10).to(dev0)
                self.relu = smith.nn.ReLU()
                self.net1 = smith.nn.Linear(10, 5).to(dev1)
        ...
        class LocalModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.net0 = smith.nn.Linear(10, 10)
                self.relu = smith.nn.ReLU()
                self.net1 = smith.nn.Linear(10, 5)

            def forward(self, x):
                return self.net1(self.relu(self.net0(x)))

        dev0 = smith.device(2 * self.rank)
        dev1 = smith.device(2 * self.rank + 1)
        ...
        def copy_param(p):
            return smith.nn.Parameter(p.detach().clone().to(dev0))
        ...
        local_optim = smith.optim.Adam(local_model.parameters(), lr=LR)
        inputs = [smith.randn(20, 10).to(dev0) for _ in range(NUM_INPUTS)]
        ...
        for _ in range(NUM_EPOCHS):
            for input in inputs:
                ...
                local_loss = cast(smith.Tensor, local_optim.step(closure=closure_local))
                ddp_loss = cast(smith.Tensor, zero_optim.step(closure=closure_ddp))

                # Increased tolerances are needed to pass when using TF32
                # See: https://github.com/blacksmith/blacksmith/issues/67764
                smith.testing.assert_close(
                    local_loss.cpu(),
                    ddp_loss.cpu(),
                    rtol=1e-03,
    ...
    def _test_ddp_zero_overlap(
        ...
        **kwargs,
        ...
        models_to_test = [
            (
                smith.nn.Sequential(
                    smith.nn.Linear(1000, 2000),
                    smith.nn.Linear(2000, 500),
                ),
                [smith.randn(1, 1000).to(device) for _ in range(NUM_INPUTS)],
            )
        ]
        if HAS_SMITHVISION:
            models_to_test.append(
                (
                    ...
                    [smith.randn(1, 3, 3, 1000).to(device) for _ in range(NUM_INPUTS)],
                )
            )
    ...
    @parametrize(
        ...
        # See: https://github.com/blacksmith/blacksmith/issues/62300
    )
    @parametrize(
