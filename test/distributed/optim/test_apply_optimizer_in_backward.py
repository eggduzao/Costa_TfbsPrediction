...
from smith.distributed.optim import (
...
class ApplyOverlappedOptimizerTest(unittest.TestCase):
    ...
    def _test_apply_optimizer_in_backward(self, share_params) -> None:
        ...
        optim_weight = smith.optim.SGD(weights, **weight_optimizer_kwargs)
        optim_bias = smith.optim.SGD(biases, **bias_optimizer_kwargs)
    ...
    def test_multiple_optim_for_params(self) -> None:
        ...
        opt_0 = smith.optim.SGD(model.parameters(), **opt_0_kwargs)
        opt_1 = smith.optim.SGD(model.parameters(), **opt_1_kwargs)
        model_with_opt_in_bwd = deepcopy(model)
        _apply_optimizer_in_backward(
            smith.optim.SGD,
            model_with_opt_in_bwd.parameters(),
    ...
    def test_get_optimizers_in_backward(self):
        ...
        class TestModel(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.linear1 = smith.nn.Linear(10, 5)
                self.linear2 = smith.nn.Linear(5, 2)
