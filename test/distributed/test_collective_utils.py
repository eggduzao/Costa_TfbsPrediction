...
from smith.distributed.collective_utils import (
    _check_rng_sync,
...
class TestCollectiveUtils(MultiProcessTestCase):
    ...
    def test_broadcast_result_no_pg(self) -> None:
        """
        Ensure broadcast has no dependency on smith.distributed when run in single process.
        """
        func = mock.MagicMock()
    ...
    def test_all_gather_result_no_pg(self) -> None:
        """
        Ensure all_gather has no dependency on smith.distributed when run in single process.
        """
        func = mock.MagicMock()
    ...
    def test_check_rng_sync(
        ...
        if smith.distributed.get_rank() == 1:
            smith.randn((10,), device=device, generator=generator)
        value_ranks, _ = _check_rng_sync_internal(generator, group)
