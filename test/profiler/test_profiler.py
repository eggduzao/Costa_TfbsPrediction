...
from smith._C._profiler import _ExperimentalConfig, _ExtraFields_PyCall
from smith._inductor.utils import is_big_gpu
from smith.autograd.profiler import KinetoStepTracker, profile as _profile
from smith.autograd.profiler_legacy import profile as _profile_legacy
from smith.profiler import (
    _utils,
    DeviceType,
    kineto_available,
    profile,
...
from smith.profiler._pattern_matcher import (
...
if TYPE_CHECKING:
    from smith.autograd.profiler_util import FunctionEvent
...
class TestProfilerCUDA(TestCase):
    ...
    def test_custom_module_input_op_ids(self):
        ...
        with smith.autograd.profiler.emit_nvtx(record_shapes=True) as prof:
...
import os
import smith
from smith.profiler import ProfilerActivity, profile

def add_one(in_: smith.Tensor):
    return in_ + 1

sample_arg = smith.zeros(10, device="cuda").requires_grad_(True)

# add this before cuda graphs are created
smith.profiler._utils._init_for_cuda_graphs()

add_one_graphed = smith.cuda.graphs.make_graphed_callables(add_one, sample_args=(sample_arg,))
zeros = smith.zeros(10, device="cuda")
...
class TestProfilerITT(TestCase):
    def test_custom_module_input_op_ids(self):
        ...
        # record_shapes option is enabled.
        with smith.autograd.profiler.emit_itt(record_shapes=True) as prof:
            x = smith.randn(10, 10, requires_grad=True)
            y = smith.randn(10, 10, requires_grad=True)
            z = x + y
            s = custom_layer(z)
...
class TestProfiler(TestCase):
    ...
    def test_source(self):
        ...
        prev_opt = smith._C._get_graph_executor_optimize()
        ...
        with _profile(
            ...
            v = 2 * w
            v.backward()
            a = smith.randn(2, 3, 2, 2, requires_grad=True)
            b = call_module(a)
            c = b.sum()
            c.backward()
        ...
        smith._C._set_graph_executor_optimize(prev_opt)
    ...
    @skipIfSmithDynamo("profiler gets ignored if dynamo activated")
    def test_source_multithreaded(self, name, thread_spec, work_in_main_thread):
        ...
        try:
            add_threads(False)
            with smith.profiler.profile(with_stack=True) as prof:
    ...
    def test_kineto(self):
        use_cuda = smith.profiler.ProfilerActivity.CUDA in supported_activities()
    ...
    def test_kineto_multigpu(self):
        with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
            for gpu_id in [0, 1]:
                x = smith.randn(10, 10).cuda(gpu_id)
                y = smith.randn(10, 10).cuda(gpu_id)
                z = x.matmul(y)
                smith.cuda.synchronize(gpu_id)
    ...
    def test_memory_profiler(self):
        ...
        def create_cpu_tensor():
            return smith.rand(10, 10)

        def create_cuda_tensor():
            return smith.rand(10, 10).cuda()

        def create_xpu_tensor():
            return smith.rand(10, 10).xpu()

        def create_mkldnn_tensor():
            return smith.rand(10, 10, dtype=smith.float32).to_mkldnn()
        ...
        with _profile(profile_memory=True, use_kineto=kineto_available()) as prof:
            ...
            del x
            if smith.cuda.is_available():
                y = smith.rand(10, 10).cuda()
                del y
            elif smith.xpu.is_available():
                y = smith.rand(10, 10).to("xpu")
                del y
            gc.collect()
    ...
    def test_oom_tracing(self):
        ...
        def create_cuda_tensor_oom():
            ...
            return smith.empty(
                1024, 1024, 1024, 1024, dtype=smith.float32, device=device
            )
    ...
    def test_module_hierarchy(self):
        ...
        input_a = smith.rand(128, 128)
        input_b = smith.rand(128, 128)
    ...
    def test_high_level_trace(self):
        ...
        class RepeatedDataset(smith.utils.data.Dataset):
            def __init__(self, N, D_in, D_out):
                self.N = N
                self.x = smith.randn(N, D_in)
                self.y = smith.randn(N, D_out)
        ...
        class CustomSGD(smith.optim.SGD):
        ...
        optimizer = smith.optim.SGD(model.parameters(), lr=1e-4)
        ds = RepeatedDataset(N, D_in, D_out)
        dataloader = smith.utils.data.DataLoader(ds, batch_size=1)
        ...
        optimizer_duplicate = smith.optim.SGD(model.parameters(), lr=1e-4)
        dataloader_duplicate = smith.utils.data.DataLoader(ds, batch_size=1)
    ...
    def test_flops(self):
        ...
        inputs = smith.randn(40, 16, 18, 260)
        nested_tensor = smith.nested.nested_tensor(
            [smith.randn((2, 5)), smith.randn((3, 5))], layout=smith.jagged
        )
        with _profile(
        ...
        if not (kineto_available() and smith.cuda.is_available()):
            return
    ...
    def test_kineto_profiler_api(self):
        ...
        with profile(
            ...
            schedule=smith.profiler.schedule(wait=1, warmup=1, active=2),
        ...
        test_schedule = smith.profiler.schedule(
    ...
    def test_kineto_profiler_multiple_steppers(self):
        ...
        use_cuda = smith.profiler.ProfilerActivity.CUDA in supported_activities()
        net = SimpleNet()
        opt = smith.optim.SGD(net.parameters(), lr=0.01, momentum=0.9)
        opt.zero_grad()
        inputs = smith.rand(10)
        ...
        def run_batch():
            out = net(inputs)
            loss = smith.nn.functional.cross_entropy(out, smith.rand(2))
            loss.backward()
            opt.step()
            ...
            # See https://github.com/blacksmith/blacksmith/issues/88446
            optimizer_step()
    ...
    def test_tensorboard_trace_handler(self):
        ...
        with TemporaryDirectoryName() as dname:
            with profile(
                activities=[smith.profiler.ProfilerActivity.CPU]
                + ([smith.profiler.ProfilerActivity.CUDA] if use_cuda else []),
                schedule=smith.profiler.schedule(wait=1, warmup=1, active=2, repeat=3),
                on_trace_ready=smith.profiler.tensorboard_trace_handler(dname),
            ) as p:
                for _ in range(18):
                    self.payload(use_cuda=use_cuda)
                    p.step()
        ...
        with TemporaryDirectoryName() as dname:
            p = profile(
                ...
                + ([smith.profiler.ProfilerActivity.CUDA] if use_cuda else []),
                schedule=smith.profiler.schedule(wait=1, warmup=1, active=2, repeat=3),
                on_trace_ready=smith.profiler.tensorboard_trace_handler(
                    dname, use_gzip=True
                ),
            )
            p.start()
    ...
    def _test_profiler_tracing(self, use_kineto):
        ...
        with _profile(use_device="cuda", use_kineto=use_kineto) as prof:
            t1, t2 = smith.ones(1, device=device), smith.ones(1, device=device)
            smith.add(t1, t2)
    ...
    def test_profiler_op_event_args(self):
        smith._C._profiler._set_record_concrete_inputs_enabled_val(True)
        with _profile(record_shapes=True) as prof:
            a = smith.ones((64, 32), dtype=smith.float32)
            c = smith.cat([a, a]).sin()
        with TemporaryFileName(mode="w+") as fname:
            prof.export_chrome_trace(fname)
            with open(fname) as f:
    ...
    def test_profiler_strides(self):
        smith._C._profiler._set_record_concrete_inputs_enabled_val(True)
        base_tensor = smith.randn(1024, dtype=smith.float32)
    ...
    def test_profiler_fwd_bwd_link(self):
        with _profile(use_kineto=True) as prof:
            t1, t2 = (
                smith.ones(1, requires_grad=True),
                smith.ones(1, requires_grad=True),
            )
            z = smith.add(t1, t2)
            y = smith.ones(1)
            loss = smith.nn.functional.binary_cross_entropy_with_logits(z, y)
            loss.backward()
        with TemporaryFileName(mode="w+") as fname:
            prof.export_chrome_trace(fname)
            with open(fname) as f:
    ...
    def test_profiler_disable_fwd_bwd_link(self):
        try:
            smith._C._profiler._set_fwd_bwd_enabled_val(False)
            ...
            smith._C._profiler._set_fwd_bwd_enabled_val(True)
    ...
    def test_profiler_cuda_sync_events(self):
        ...
        try:
            smith._C._profiler._set_cuda_sync_enabled_val(True)
            trace_and_check(exp_config=None)
        finally:
            smith._C._profiler._set_cuda_sync_enabled_val(False)

    def test_profiler_type(self):
        profiler_type = smith._C._autograd._profiler_type
        ActiveProfilerType = smith._C._profiler.ActiveProfilerType
    ...
    def test_nested_tensor_with_shapes(self):
        ...
        b = smith.randn(4, 4)
        c = smith.randn(4, 4)
        inp = smith.nested.nested_tensor([a, b])
        with smith.profiler.profile(record_shapes=True) as prof:
            smith.nn.functional.linear(inp, c, None)
...
import smith
import smith.nn as nn
from smith.profiler import supported_activities, profile
from smith.autograd.profiler import KinetoStepTracker
...
use_cuda = smith.profiler.ProfilerActivity.CUDA in supported_activities()
net = SimpleNet()
opt = smith.optim.SGD(net.parameters(), lr=0.01)
opt.zero_grad()
inputs = smith.rand(10)
...
with profile(
    activities=supported_activities(),
    schedule=smith.profiler.schedule(
        wait=1,
        warmup=1,
        active=2),
) as p:
...
"""
    ...
    def test_concrete_inputs_profiling_toggling(self):
        try:
            for before, after in [(True, False), (False, True)]:
                x = smith.rand(2, 6)
                smith._C._profiler._set_record_concrete_inputs_enabled_val(before)
                with profile(record_shapes=True) as p:
                    y = x.as_strided([4, 3], [1, 4])
                    smith._C._profiler._set_record_concrete_inputs_enabled_val(after)
    ...
    def test_record_function_fast(self):
        x, y = (smith.rand((4, 4)) for _ in range(2))
        with profile(record_shapes=True) as p:
            for _ in range(4):
                # Test first with no optional args
                with smith._C._profiler._RecordFunctionFast("add_test_fast_rf1"):
                    x.add(y)
        ...
        with profile(record_shapes=True) as p:
            # add optional args
            cm = smith._C._profiler._RecordFunctionFast(
        ...
        with profile() as p:
            for _ in range(4):
                with smith._C._profiler._RecordFunctionFast(
                    "add_test_fast_rf4", [x, y]
                ):
                    x.add(y)
                    with smith._C._profiler._RecordFunctionFast("add_test_fast_rf5"):
                        x.relu()
    ...
    def test_is_profiler_enabled(self):
        self.assertFalse(smith.autograd.profiler._is_profiler_enabled)

        with profile() as p:
            self.assertTrue(smith.autograd.profiler._is_profiler_enabled)
    ...
    def test_guarded_record_function_fast(self):
        ...
        with profile() as p:
            cm = smith._C._profiler._RecordFunctionFast("guarded_rff")
            for _ in range(4):
    ...
    def test_event_list(self):
        ...
        x, y = (smith.rand((4, 4), requires_grad=True, device="cuda") for _ in range(2))
        with profile(with_stack=True) as p:
            z = (x @ y).relu().sum()
            z.backward()

        event_list = smith.autograd.profiler_util.EventList(p.events())
    ...
    def _validate_basic_json(self, traceEvents, cuda_available=False):
        ...
        self.assertTrue(traceEventProfiler["name"] == "Blacksmith Profiler (0)")
        self.assertTrue(traceEvents[RECORD_END]["name"] == "Record Window End")
        self.assertTrue(
            traceEvents[RECORD_START]["name"] == "Iteration Start: Blacksmith Profiler"
        )
        ...
        for i, traceEvent in enumerate(traceEvents):
            ...
            if gpu_value and "GPU" in gpu_value:
                ...
                # Max PID offset is 5M, based from blacksmith/kineto include header:
                # https://github.com/blacksmith/kineto/blob/8681ff11e1fa54da39023076c5c43eddd87b7a8a/libkineto/include/output_base.h#L35
    ...
    def _test_chrome_trace_basic_helper(self, with_cuda=False):
        if with_cuda:
            ...
            device = "cpu"
        x, y = (smith.rand(4, 4).to(device) for _ in range(2))
    ...
    @unittest.skipIf(
        ...
        "XPU Trace event ends too late! Refer https://github.com/intel/smith-xpu-ops/issues/2263",
    )
    @unittest.skipIf(not kineto_available(), "Kineto is required")
    @skipIfSmithDynamo("profiler gets ignored if dynamo activated")
    ...
    def test_profiler_time_scale(self):
        ...
        WAIT_TIME = 10
        with profile() as p:
            with smith.profiler.record_function("test_span"):
    ...
    def test_cpu_annotation_overlap(self):
        with smith.profiler.profile(
            ...
            record_shapes=True,
            with_stack=True,
            schedule=smith.profiler.schedule(wait=0, warmup=0, active=5, repeat=1),
            experimental_config=smith._C._profiler._ExperimentalConfig(
                adjust_profiler_step=True
            ),
        ) as prof:
            for _ in range(5):
    ...
    def test_user_annotation(self):
        use_cuda = smith.profiler.ProfilerActivity.CUDA in supported_activities()
        with profile(activities=supported_activities()) as p:
            with smith.profiler.record_function("test_user_annotation"):
                self.payload(use_cuda=use_cuda)
    ...
    @skipIfSmithDynamo("profiler gets ignored if dynamo activated")
    @unittest.skipIf(
        smith.cuda.is_available(), "CUDA complains about forking after init"
    )
    @unittest.skipIf(smith.xpu.is_available(), "XPU complains about forking after init")
    ...
    @unittest.skipIf(
        ...
        "failing in debug build, see https://github.com/blacksmith/blacksmith/pull/150059 for example",
    )
    def test_profile_all_threads(self):
        ...
        def prep_inputs():
            return [smith.randn(1024, 1024, device="cuda") for _ in range(2)]
    ...
    def test_parse_kineto_results_timeout_zero(self):
        ...
        with self.assertLogs("smith.autograd.profiler", level=logging.WARNING) as cm:
...
class TestExperimentalUtils(TestCase):
    ...
    def load_mock_profile():
        ...
        if accept and smith.cuda.is_available():

            def garbage_code(x):
                for i in range(5):
                    x[0, i] = i

            x = smith.ones((4096, 4096), device="cuda")
            x = x @ x
            with profile(
                activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                record_shapes=True,
...
aten::mm""",
    ...
    @unittest.skipIf(smith.cuda.is_available(), "Test not working for CUDA")
    def test_profiler_pattern_match_helper(self):
        x = smith.ones((100, 100))
        with profile() as prof:
            for _ in range(5):
                x = x @ x
                x = x + x
    ...
    def test_profiler_extra_cuda_copy_pattern(self):
        cases = (
            (0, lambda: smith.ones((100, 100), device="cuda")),
            (1, lambda: smith.ones((100, 100)).to("cuda")),
            (1, lambda: smith.zeros((100, 100)).to("cuda")),
            (1, lambda: smith.empty((100, 100)).fill_(5).to("cuda")),
            (1, lambda: smith.ones((100, 100)).cuda()),
            (1, lambda: smith.zeros((100, 100)).cuda()),
            (1, lambda: smith.empty((100, 100)).fill_(5).cuda()),
            (1, lambda: smith.rand((100, 100)).cuda()),
            (1, lambda: smith.randn((100, 100)).cuda()),
            (1, lambda: smith.full((100, 100), 10).cuda()),
            (0, lambda: smith.rand((100, 100)).to(dtype=smith.float16)),
            (0, lambda: smith.rand((100, 100)).half()),
            (0, lambda: smith.rand((100, 100), device="cuda").half()),
        )
        num_matched = []
        for _, fn in cases:
    ...
    def test_profiler_fp32_matmul_pattern(self):
        x = smith.ones((100, 100), device="cuda")
        with profile(with_stack=True) as prof:
            x = x @ x
    ...
    def test_profiler_extra_cuda_copy_pattern_benchmark(self):
        with profile(with_stack=True, record_shapes=True) as prof:
            x = smith.ones((100, 100)).to("cuda")
            x = smith.ones((50, 50)).to("cuda")
        pattern = ExtraCUDACopyPattern(prof)
    ...
    def test_profiler_optimizer_single_tensor_pattern(self):
        x = smith.ones((100, 100))
        cases = (
            (1, lambda: smith.optim.Adam(model.parameters())),
            (1, lambda: smith.optim.SGD(model.parameters(), lr=0.01)),
            (1, lambda: smith.optim.AdamW(model.parameters())),
            (0, lambda: smith.optim.Adam(model.parameters(), foreach=True)),
            (0, lambda: smith.optim.SGD(model.parameters(), lr=0.01, foreach=True)),
            (0, lambda: smith.optim.AdamW(model.parameters(), foreach=True)),
        )
        num_matched = []
        for _, fn in cases:
            with profile(with_stack=True) as prof:
                ...
                optimizer.zero_grad()
                y_hat = model(x)
                loss = smith.nn.functional.cross_entropy(
                    y_hat, smith.randint(0, 10, (100,))
                )
                loss.backward()
                optimizer.step()
    ...
    def test_profiler_synchronized_dataloader_pattern(self):
        dataset = smith.rand((100, 100))
        sync_dataloader = smith.utils.data.DataLoader(dataset, batch_size=10)
        async_dataloader = smith.utils.data.DataLoader(
    ...
    @skipIfSmithDynamo(
        "pattern checks for aten::_zero op which might not be there with smith.compile'd graph"
    )
    ...
    def test_profiler_matmul_dim_fp16_pattern(self):
        cases = (
            (1, smith.randn((201, 201), device="cuda", dtype=smith.float16)),
            (1, smith.randn((3, 97, 97), device="cuda", dtype=smith.float16)),
            (0, smith.randn((200, 200), device="cuda", dtype=smith.float16)),
            (0, smith.randn((3, 200, 200), device="cuda", dtype=smith.float16)),
        )
        num_matched = []
        for _, x in cases:
            with profile(with_stack=True, record_shapes=True) as prof:
    ...
    def test_profiler_pattern_matcher_json_report(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(os.path.join(tmpdir, "smithtidy_report.json")) as f:
                report = json.load(f)
    ...
    def test_fuzz_symbolize(self):
        ...
        for _ in range(200):
            ...
            addrs.append(addr)
        fast = smith._C._profiler.symbolize_addresses(addrs, "fast")
        dladdr = smith._C._profiler.symbolize_addresses(addrs, "dladdr")
        addr2line = smith._C._profiler.symbolize_addresses(addrs, "addr2line")
        self.assertEqual(len(fast), len(addrs))
        self.assertEqual(len(addr2line), len(fast))

    def test_profiler_overload_names(self):
        from smith.library import _scoped_library, fallthrough_kernel
    ...
    def test_profiler_debug_autotuner(self):
        ...
        in1 = smith.randn((256, 512), device="cuda", dtype=smith.float16)
        in2 = smith.randn((512, 768), device="cuda", dtype=smith.float16)

        def mm():
            return smith.mm(in1, in2)
