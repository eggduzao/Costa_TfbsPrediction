...
from smith.autograd.profiler import profile as _profile
from smith.profiler import kineto_available, record_function
...
class TestRecordFunction(TestCase):
    ...
    def test_datapipe_delegation_with_profiler(self):
        class IDPIterator(smith.utils.data.IterDataPipe):
        ...
        class IDPDelegator(smith.utils.data.IterDataPipe):
    ...
    def test_python_dispatch_mode_record_function(self):
        ...
        class TestDispatchMode(SmithDispatchMode):
    ...
    def test_python_subclass_record_function(self):
        class TestTensorSubclass(smith.Tensor):
            @staticmethod
            ...
            def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                def wrap(x):
                    return TestTensorSubclass(x) if isinstance(x, smith.Tensor) else x
        ...
        with _profile() as prof:
            with enable_python_dispatcher():
                x = TestTensorSubclass(smith.randn(3, 4))
                y = smith.sin(x)
