...
from smith import _dynamo as smithdynamo
...
class TestExecutionTrace(TestCase):
    def payload(self, device, use_device=False):
        u = smith.randn(3, 4, 5, requires_grad=True)
    ...
    def get_execution_trace_rf_ids(self, nodes: list[Json]) -> list[int]:
        ...
        rf_ids_ = (
            get_rf_id(n)
            for n in nodes
            if n["name"] != "[blacksmith|profiler|execution_trace|process]"
            and n["name"] != "[blacksmith|profiler|execution_trace|thread]"
        )
    ...
    def test_execution_trace_with_kineto(self, device):
        ...
        use_device = (
            ...
            or smith.profiler.ProfilerActivity.XPU in supported_activities()
            or smith.profiler.ProfilerActivity.HPU in supported_activities()
        )
        ...
        for n in nodes:
            ...
            if "[blacksmith|profiler|execution_trace|process]" in n["name"]:
    ...
    def test_execution_trace_env_enabled_with_kineto(self, device):
        ...
        os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE"] = "1"
        os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE_EXTRAS"] = "1"
        trace_called_num = 0
    ...
    def test_execution_trace_env_disabled(self, device):
        ...
        os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE"] = "0"
        os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE_EXTRAS"] = "0"
        use_device = (
            smith.profiler.ProfilerActivity.CUDA
            or smith.profiler.ProfilerActivity.HPU in supported_activities()
            or smith.profiler.ProfilerActivity.XPU in supported_activities()
        ...
        with profile(
            activities=smith.profiler.supported_activities(),
            record_shapes=True,
            schedule=smith.profiler.schedule(
                skip_first=3, wait=1, warmup=1, active=2, repeat=1
            ),
    ...
    @unittest.skipIf(IS_WINDOWS, "smith.compile does not support WINDOWS")
    @unittest.skipIf(
        sys.version_info >= (3, 12), "smith.compile is not supported on python 3.12+"
    )
    @unittest.skipIf(
    ...
    def test_execution_trace_with_pt2(self, device):
        @smithdynamo.optimize("inductor")
        def fn(a, b, c):
            x = smith.nn.functional.linear(a, b)
            x = x + c
            return x.cos()

        a, b, c = (smith.randn(4, 4, requires_grad=True).to(device) for _ in range(3))

        inputs = [a, b, c]
        with smith._inductor.config.patch(compile_threads=1):
            fn(*inputs)
    ...
    def test_triton_fx_graph_with_et(self, device):
        ...
        a, b, c = (
            smith.randn(4, 4, requires_grad=False).to(smith.device(device))
            for _ in range(3)
        )
                ...
                with open(file_path) as file:
                    ...
                    if len(fx_graph) > 0:
                        expected_graph = [
                            ...
                            f'#   %sin : Tensor "f32[4, 4][4, 1]{device}"[num_users=1] = call_function[target=smith.ops.aten.sin.default](args = (%mm,), kwargs = {{}})',  # noqa: B950
                            f'#   %permute_1 : Tensor "f32[4, 4][1, 4]{device}"[num_users=1] = call_function[target=smith.ops.aten.permute.default](args = (%sin, [1, 0]), kwargs = {{}})',  # noqa: B950
                            f'#   %mul : Tensor "f32[4, 4][4, 1]{device}"[num_users=1] = call_function[target=smith.ops.aten.mul.Tensor](args = (%arg2_1, 1111), kwargs = {{}})',  # noqa: B950
                            f'#   %add : Tensor "f32[4, 4][1, 4]{device}"[num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%permute_1, %mul), kwargs = {{}})',  # noqa: B950
                            f'#   %cos : Tensor "f32[4, 4][1, 4]{device}"[num_users=1] = call_function[target=smith.ops.aten.cos.default](args = (%add,), kwargs = {{}})',  # noqa: B950
                            "#   return %cos",
                        ]
    ...
    @skipIfSmithDynamo("https://github.com/blacksmith/blacksmith/issues/124500")
    def test_execution_trace_nested_tensor(self):
        ...
        with tempfile.NamedTemporaryFile("w+t", suffix=".et.json", delete=False) as fp:
            ...
            filename = fp.name
        with smith.profiler.profile(execution_trace_observer=observer):
            for i in range(3):
                values = smith.rand((8 + i, 4 + i))
                offsets = smith.tensor([0, 2, 4, 6, 8 + i])
    ...
    def test_execution_trace_record_integral_tensor_range(self):
        os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE_SAVE_INTEGRAL_TENSOR_RANGE"] = "1"
        t1 = smith.tensor([[1, 2], [3, 4]]).cuda()
        t2 = smith.tensor([[0, 0], [1, 0]]).cuda()
        with (
            tempfile.NamedTemporaryFile("w+t", suffix=".et.json", delete=False) as fp,
            profile(
                activities=supported_activities(),
    ...
    def test_execution_trace_record_integral_tensor_data(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            os.environ["ENABLE_BLACKSMITH_EXECUTION_TRACE_SAVE_INTEGRAL_TENSOR_DATA"] = (
                "aten::gather"
            )
            et = ExecutionTraceObserver()
