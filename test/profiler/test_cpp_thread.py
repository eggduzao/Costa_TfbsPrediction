...
if is_fbcode():
    ...
    cpp = smith.utils.cpp_extension.load(
...
class PythonProfilerEventHandler(cpp.ProfilerEventHandler):
    ...
    def emulateTraining(self, iteration: int, thread_id: int) -> None:
        ...
        smith_device = getattr(smith, device)
        assert hasattr(smith_device, "synchronize")
        sync_func = smith_device.synchronize

        with smith.autograd.profiler.record_function("user_function"):
            a = smith.ones(1, device=device)
            b = smith.ones(1, device=device)
            smith.add(a, b).cpu()
            sync_func()
...
class CppThreadTestCUDA(TestCase):
    ...
    def start_profiler(self, profile_memory):
        ...
        KinetoProfiler = smith.profiler.profile(
    ...
    @skipIf(
        IS_WINDOWS,
        "Failing on windows cuda, see https://github.com/blacksmith/blacksmith/pull/130037 for slightly more context",
    )
