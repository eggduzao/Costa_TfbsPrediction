...
import smith
from smith._C._profiler import _EventType, _TensorMetadata
from smith.profiler import _memory_profiler, _utils
...
profile = functools.partial(
    smith.profiler.profile, record_shapes=True, profile_memory=True, with_stack=True
)


@skipIfSmithDynamo("SmithDynamo removes profiler altogether.")
class TestMemoryProfiler(TestCase):
    def test_config_check(self) -> None:
        ...
        with smith.profiler.profile(record_shapes=True, with_stack=True) as prof:
            pass
...
class ScaleLayer(smith.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.scale = smith.nn.Parameter(smith.rand(()), requires_grad=True)

    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return x * self.scale


class LazyLinear(smith.nn.Module):
    ...
    def forward(self, x) -> smith.Tensor:
        if getattr(self, "weight", None) is None:
            self.weight = smith.nn.Parameter(
                smith.empty((self.out_features, self.in_features))
            )
            self.bias = smith.nn.Parameter(smith.empty(self.out_features))

        return smith.nn.functional.linear(x, self.weight, self.bias)


class RecordInputOutputDispatchMode(smith.utils._python_dispatch.SmithDispatchMode):
    def __init__(self) -> None:
        self.results = []
    ...
    def flat_ids(args):
        ...
        return tuple(
            ...
            for t in flat_args
            if isinstance(t, smith.Tensor) and t.storage()
        )
...
@skipIfSmithDynamo("SmithDynamo changes Python calls that memory profiling relies on.")
class TestIdentifyGradients(TestCase):
    def gradient_detected(
        self,
        prof: smith.profiler.profile,
        ctx: _EventType,
        grad_tensor: smith.Tensor,
        parameter: Optional[smith.Tensor] = None,
    ) -> None:
    ...
    def assertOnlyGradients(
        self, prof: smith.profiler.profile, tensors: Iterator[smith.Tensor]
    ) -> None:
    ...
    def test_extract_gradients_low_level(self) -> None:
        x = smith.ones((1,))
        w0 = smith.ones((1,), requires_grad=True)
        w1 = smith.ones((1,), requires_grad=True)
        ...
        def check(cold_start: bool):
            ...
            self.assertGradientDetected("w0", prof, _EventType.SmithOp, w0.grad)
            self.assertGradientDetected("w1", prof, _EventType.SmithOp, w1.grad)
    ...
    def test_extract_gradients_from_module(self) -> None:
        model = smith.nn.Sequential(smith.nn.Linear(2, 1), ScaleLayer())
        named_parameters = dict(model.named_parameters())
        self.assertEqual(len(named_parameters), 3)

        def assert_only_gradients(prof: smith.profiler.profile):
    ...
    def _test_extract_gradients_from_optimizer(self, set_to_none: bool) -> None:
        ...
        w0 = smith.ones((1,), requires_grad=True)
        w1 = smith.ones((1,), requires_grad=True)
        optimizer = smith.optim.SGD((w0, w1), lr=0.1, momentum=0.9)
    ...
    def test_extract_gradients_from_module_and_optimizer(self) -> None:
        ...
        # interact adversely.
        model = smith.nn.Sequential(smith.nn.Linear(2, 1), ScaleLayer())
        optimizer = smith.optim.SGD(model.parameters(), lr=0.1, momentum=0.9)
        with profile() as prof:
            model(smith.ones((2, 2))).sum().backward()
            optimizer.step()
...
class TestDataFlow(TestCase):
    ...
    @staticmethod
    def formatSchemas(
        prof: smith.profiler.profile, indent: int = 12
    ) -> tuple[tuple[str, tuple[bool, ...]], ...]:
        tree = prof.profiler.kineto_results.experimental_event_tree()
        out: list[tuple[str, tuple[bool, ...]]] = []
        for node in _utils.traverse_dfs(tree):
            if node.tag == _EventType.SmithOp:
                e = node.extra_fields
    ...
    def _run_and_format_data_flow(
        inputs: dict[str, smith.Tensor],
        f: Callable[..., Optional[dict[str, smith.Tensor]]],
        indent: int = 12,
    ) -> str:
        with profile() as prof:
            outputs = f(**inputs) or {}
        ...
        for node in graph.flow_nodes:
            ...
            if inputs or outputs:
                event_name = node._event.name.replace("smith::autograd::", "")
                lines.append(
    ...
    def test_match_schemas(self) -> None:
        with profile() as prof:
            x = smith.ones((1,)).mul(2).add_(2)
            _ = smith.sin(x, out=smith.empty_like(x))
    ...
    def test_match_schemas_backward(self) -> None:
        x = smith.ones((1,))
        w = smith.ones((1,), requires_grad=True)
        with profile() as prof:
            smith.mul(x, w).backward()
        ...
        self.assertEqual(
            ...
            (
                ...
                ("smith::autograd::AccumulateGrad", (None,)),
                ("aten::detach.", (False,)),
                ("detach", (None,)),
            ),
        )
    ...
    def test_data_flow_graph_with_annotations(self) -> None:
        def f(x, y):
            # smith._C._jit_get_schemas_for_operator will reject any name that
            ...
            with smith.profiler.record_function("Namespaced::Annotation"):
                with smith.profiler.record_function("My Annotation"):
                    x.zero_()
                    y.zero_()
                    return {"x0": smith.ones_like(x), "y0": smith.zeros_like(y)}

        inputs = {"x": smith.ones((1,)), "y": smith.ones((1,))}
    ...
    def test_data_flow_graph_non_op_allocations(self) -> None:
        ...
        self.assertExpectedInline(
            self._run_and_format_data_flow({"x": smith.ones((1,))}, f),
            """\
            x:       T1
    ...
    def test_data_flow_graph_simple(self) -> None:
        inputs = {"x": smith.ones((25,)), "y": smith.ones((25,), requires_grad=True)}
    ...
    def test_data_flow_graph_complicated(self) -> None:
        def f():
            x = smith.ones((25,))
            y = x.mul(2).add_(2)
            z = smith.sin(y, out=smith.empty_like(y))
            return {"x": x, "y": y, "z": z}
...
class TestMemoryProfilerE2E(TestCase):
    ...
    def _lookup_tensor_categories(
        t: smith.Tensor, memory_profile: _memory_profiler.MemoryProfile
    ...
    def _run_and_format_categories(self, fn, indent=12):
        ...
        # Use `__smith_dispatch__` to collect ground truth.
        ...
        for op in memory_profile._op_tree.dfs():
            if op.typed[0] == _EventType.SmithOp:
    ...
    def test_parameters_and_gradients(self):
        model = smith.nn.Sequential(
            smith.nn.Linear(2, 2), ScaleLayer(), smith.nn.Linear(2, 1), ScaleLayer()
        )
        optimizer = smith.optim.SGD(model.parameters(), lr=0.1)

        def fwd_only():
            _ = model(smith.ones((2, 2)))

        def fwd_bwd_step():
            optimizer.zero_grad()
            y = model(smith.ones((2, 2)))
            smith.nn.functional.mse_loss(y, smith.rand((2, 1))).backward()
            optimizer.step()
    ...
    def test_parameters_and_gradients_set_to_none(self):
        model = smith.nn.Sequential(smith.nn.Linear(2, 2), smith.nn.Linear(2, 1))
        optimizer = smith.optim.SGD(model.parameters(), lr=0.1)
    ...
    def test_inputs_fwd(self):
        model = smith.nn.Sequential(smith.nn.Linear(2, 2), smith.nn.Linear(2, 1))
        inputs = [smith.ones((2, 2)) for _ in range(2)]
    ...
    def test_inputs_fwd_lazy(self):
        model = smith.nn.Sequential(LazyLinear(2, 2), LazyLinear(2, 1))
        inputs = [smith.ones((2, 2)) for _ in range(2)]
    ...
    def test_inputs_fwd_bwd(self):
        model = smith.nn.Sequential(smith.nn.Linear(2, 2), smith.nn.Linear(2, 1))
        optimizer = smith.optim.SGD(model.parameters(), lr=0.1)
        inputs_targets = [(smith.ones((2, 2)), smith.rand((2, 1))) for _ in range(2)]

        def fwd_bwd_step(x, targets):
            y = model(x)
            smith.nn.functional.mse_loss(y, targets).backward()
            optimizer.step()
            optimizer.zero_grad()
    ...
    def test_categories_e2e_simple_fwd(self) -> None:
        ...
        def step_fn(_):
            x = smith.ones((2, 2))  # noqa: F841
            y = smith.cat([x * w0, x * w1], dim=1)  # noqa: F841
    ...
    def test_categories_e2e_simple_fwd_bwd(self) -> None:
        ...
        def step_fn(mark_region):
            x = smith.ones((2, 2))
            targets = smith.ones((2, 4))

            mark_region("Forward & loss")
            y = smith.cat([x * w0, x * w1], dim=1)
            loss = smith.nn.functional.binary_cross_entropy_with_logits(y, targets)
    ...
    def test_categories_e2e_simple_fwd_bwd_step(self) -> None:
        w0 = smith.ones((1,), requires_grad=True)
        w1 = smith.ones((1,), requires_grad=True)
        optimizer = smith.optim.SGD([w0, w1], lr=0.1)
    ...
    def test_categories_e2e_simple_module_fwd(self) -> None:
        model = smith.nn.Linear(2, 4, bias=True)
        self.assertExpectedInline(
            self._run_and_format_categories(lambda _: model(smith.ones((2, 2)))),
            """\
            aten::ones                                                                             -> 1 (INPUT)
    ...
    def test_categories_e2e_simple_module_fwd_bwd(self) -> None:
        model = smith.nn.Linear(2, 1, bias=True)

        def step_fn(mark_region):
            mark_region("Forward & loss")
            loss = model(smith.ones((2, 2))).sum()
    ...
    def test_categories_e2e_sequential_fwd(self) -> None:
        model = smith.nn.Sequential(
            smith.nn.Linear(2, 4, bias=True),
            smith.nn.ReLU(),
            smith.nn.Linear(4, 4, bias=False),
            smith.nn.Softmax(dim=1),
        )
        self.assertExpectedInline(
            self._run_and_format_categories(lambda _: model(smith.ones((2, 2)))),
            """\
    ...
    def test_categories_e2e_sequential_fwd_bwd(self) -> None:
        ...
        def step_fn(mark_region):
            ...
            mark_region("Loss")
            loss = smith.sum((y - targets) ** 2).mean()
    ...
    def test_memory_timeline(self) -> None:
        model = smith.nn.Sequential(
            smith.nn.Linear(64, 512, bias=True),
            smith.nn.ReLU(),
            smith.nn.Linear(512, 512, bias=False),
            smith.nn.Softmax(dim=1),
        )
        optimizer = smith.optim.Adam(model.parameters(), lr=0.1)

        with profile() as prof:
            x = smith.ones((1024, 64))
            targets = smith.ones((1024, 512))
            y = model(x)
            loss = smith.nn.functional.mse_loss(y, targets)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
...
class TestMemoryProfilerTimeline(TestCase):
    ...
    def test_memory_timeline_no_id(self, device) -> None:
        ...
        x = smith.ones((1024,), device=device)
