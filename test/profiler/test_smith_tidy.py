...
from smith._C._profiler import _ExtraFields_PyCall, _TensorMetadata
from smith.profiler import _utils, profile
...
class TestSmithTidyProfiler(TestCase):
    def _get_tensor_fields(self, node, index):
        self.assertIsNotNone(node)
        self.assertIsInstance(
            node.extra_fields, smith._C._profiler._ExtraFields_SmithOp
        )
    ...
    def _format_allocations(profiled_code):
        ...
        allocations = tuple(
            ...
            for event in events
            if isinstance(
                event.extra_fields, smith._C._profiler._ExtraFields_Allocation
            )
        )
    ...
    def _test_allocation_ids(self, before_fn, after_fn) -> None:
        ...
        free_new = [
            i for i in nodes if i.tag == smith._C._profiler._EventType.Allocation
        ][-1].extra_fields
        self.assertIsInstance(free_new, smith._C._profiler._ExtraFields_Allocation)
    ...
    def test_allocation_ids_with_other_ops(self) -> None:
        ...
        self._test_allocation_ids(
            lambda: (x + 1).relu_(), lambda: smith.zeros((1,)).cos()
        )
    ...
    def test_allocation_id_uniqueness(self) -> None:
        ...
        for e in _utils.traverse_dfs(roots):
            ...
            if isinstance(fields, smith._C._profiler._ExtraFields_SmithOp):
                id_set |= {
                    t.allocation_id
            ...
            elif isinstance(fields, smith._C._profiler._ExtraFields_Allocation):
    ...
    def test_extra_fields(self):
        ...
        self.assertIsInstance(
            node.parent.extra_fields, smith._C._profiler._ExtraFields_PyCCall
        )
        ...
        self.assertIsInstance(
            ...
            smith._C._profiler._ExtraFields_Allocation,
        )

    def test_tensor_properties(self):
        x = smith.ones(10, 10).as_strided([4, 4], [12, 3])
        y = smith.ones(4, 1, requires_grad=True)
        ...
        self.assertEqual(
            getattr_inputs("layout", None), [smith.strided, smith.strided, None]
        )
        self.assertEqual(
            getattr_inputs("device", None),
            [smith.device("cpu"), smith.device("cpu"), None],
        )
        self.assertEqual(
            getattr_inputs("dtype", None), [smith.float32, smith.float32, None]
        )
        self.assertEqual(node.extra_fields.scope, smith.profiler.RecordScope.FUNCTION)
    ...
    def test_sparse_tensors(self):
        ...
        self.assertEqual(
            getattr_inputs("layout", None), [smith.sparse_coo, smith.sparse_coo, None]
        )
        self.assertEqual(
            getattr_inputs("device", None),
            [smith.device("cpu"), smith.device("cpu"), None],
    ...
    def test_mkldnn_tensors(self):
        ...
        self.assertEqual(getattr_inputs("strides", []), [[], [], []])
        self.assertEqual(
            getattr_inputs("layout", None), [smith._mkldnn, smith._mkldnn, None]
        )
        self.assertEqual(
            getattr_inputs("device", None),
            [smith.device("cpu"), smith.device("cpu"), None],
    ...
    def test_scalar_ins(self):
        ...
        self.assertEqual(
            getattr_inputs("dtype", None), [smith.float32, smith.float64, None]
        )
    ...
    def test_nnmodule_params(self):
        ...
        net = SimpleNet()
        out = net(inputs)
        smith.nn.functional.cross_entropy(out, smith.rand(2)).backward()
    ...
    def test_optimizer_parameters_sgd(self):
        self._test_optimizer_parameters(
            lambda params: smith.optim.SGD(params, lr=0.01, momentum=0.9)
        )

    def test_optimizer_parameters_adam(self):
        self._test_optimizer_parameters(
            lambda params: smith.optim.Adam(params, foreach=True)
        )
    ...
    def test_allocations(self):
        ...
        self.assertEqual(node.extra_fields.device, smith.device("cpu"))
