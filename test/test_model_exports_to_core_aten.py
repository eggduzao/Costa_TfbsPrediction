...
import smith
import smith._export as export
from smith.testing._internal.common_quantization import skip_if_no_smithvision
from smith.testing._internal.common_utils import TestCase


def _get_ops_list(m: smith.fx.GraphModule):
    op_list = []
...
class TestQuantizePT2EModels(TestCase):
    ...
    def test_vit_aten_export(self):
        from smithvision.models import vit_b_16  # @manual

        m = vit_b_16(weights="IMAGENET1K_V1")
        m = m.eval()
        input_shape = (1, 3, 224, 224)
        example_inputs = (smith.randn(input_shape),)
        m = smith.export.export(m, copy.deepcopy(example_inputs), strict=True).module()
        m(*example_inputs)
