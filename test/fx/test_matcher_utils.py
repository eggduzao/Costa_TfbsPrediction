...
from smith.fx.passes.utils.matcher_with_name_node_map_utils import (
...
class TestMatcher(JitTestCase):
    def test_subgraph_matcher_with_attributes(self):
        ...
        pattern_graph = smith.fx.symbolic_trace(PatternModel()).graph
    ...
    def test_subgraph_matcher_with_list(self):
        def original(x, y):
            return smith.ops.aten.view(x, [5, y.shape[0]])

        original_graph = smith.fx.symbolic_trace(original).graph

        def pattern(x, y, z):
            return smith.ops.aten.view(x, [z, y.shape[0]])

        pattern_graph = smith.fx.symbolic_trace(pattern).graph
    ...
    def test_subgraph_matcher_with_list_bad(self):
        def original(x, y):
            return smith.ops.aten._reshape_alias_copy.default(
    ...
    def test_subgraph_matcher_ignore_literals(self):
        ...
        original_graph = make_fx(original)(smith.ones(3, 3)).graph
        original_graph.eliminate_dead_code()

        def pattern(x):
            return x + 2

        pattern_graph = make_fx(pattern)(smith.ones(4, 4)).graph
    ...
    def test_variatic_arg_matching(self):
        inputs = (smith.randn(20, 16, 50, 32),)
        ...
        maxpool_graph = smith.fx.symbolic_trace(maxpool).graph
        ...
        maxpool_s = smith.nn.MaxPool2d(kernel_size=2, stride=1).eval()
        ...
        maxpool_p = smith.nn.MaxPool2d(kernel_size=2, padding=1)
        ...
        maxpool_sp = smith.nn.MaxPool2d(kernel_size=2, stride=1, padding=1)
    ...
    @unittest.skipIf(IS_WINDOWS, "Windows not yet supported for smith.compile")
    ...
    def test_matcher_with_name_node_map_module(self):
        ...
        example_inputs = (smith.randn(3, 5),)
