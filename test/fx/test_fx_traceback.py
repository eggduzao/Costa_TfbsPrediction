...
import smith
from smith._inductor.compile_fx import aot_export_module
from smith.export import default_decompositions
from smith.fx.traceback import get_graph_provenance_json, NodeSource, NodeSourceAction
...
class TestFXNodeSource(TestCase):
    def test_node_source(self):
        ...
        node = smith.fx.Node(
            ...
            name="add",
            op="call_function",
            target=smith.ops.aten.add.Tensor,  # type: ignore[attr-defined]
            args=(smith.tensor(3), smith.tensor(4)),
            kwargs={},
        )
...
(name=add, pass_name=test_pass, action=create, graph_id={graph_id})
    ...
    def test_graph_provenance(self):
        ...
        model = Model()
        example_inputs = (smith.randn(8, 10),)
