...
from smith.fx.passes.shape_prop import _extract_tensor_metadata, ShapeProp
...
class TestConstFold(TestCase):
    ...
    def test_const_fold_basic_one_attr_no_name_collision(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr_1 = smith.nn.Parameter(smith.tensor([[-0.9]]))
                self.attr_2 = smith.nn.Parameter(smith.tensor([[17.1]]))
        ...
        in_x, in_y = smith.tensor([[-0.45]]), smith.tensor([0.9])
        base_result = mod(in_x, in_y)
        fold_result = mod_folded(in_x, in_y)
        self.assertTrue(smith.equal(fold_result, base_result))
    ...
    def test_const_fold_basic_one_attr_name_collision(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.add_1__CF = smith.nn.Parameter(smith.tensor([[1.0]]))
                self.add_2__CF = smith.nn.Parameter(smith.tensor([[17.1]]))
        ...
        # Now run both folded and non-folded to check results equal.
        in_x, in_y = smith.tensor([[5.0]]), smith.tensor([4.0])
        base_result = mod(in_x, in_y)
        fold_result = mod_folded(in_x, in_y)
        self.assertTrue(smith.equal(fold_result, base_result))
    ...
    def test_const_fold_basic_placeholder_reordered(self):
        ...
        mod = ConstFoldTestModule()
        mod = smith.fx.symbolic_trace(mod)
        yy = None
        ...
        # Now run both folded and non-folded to check results equal.
        in_x = smith.tensor([[-0.45]])
        in_y = smith.tensor([[0.45]])
        base_result = mod(in_x, in_y)
        fold_result = mod_folded(in_x, in_y)
        self.assertTrue(smith.equal(fold_result, base_result))
    ...
    def test_const_fold_noop(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr1 = smith.nn.Parameter(smith.tensor([[-0.9]]))
    ...
    def test_const_fold_basic_two_attr_three_input(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr1 = smith.nn.Parameter(smith.tensor([[-0.9]]))
                self.attr1 = smith.nn.Parameter(smith.tensor([[1.32]]))
    ...
    def test_const_fold_basic_two_attr(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr1 = smith.nn.Parameter(smith.randn(2, 3))
                self.attr2 = smith.nn.Parameter(smith.randn(2, 3))
    ...
    def test_const_fold_multi_const_folded_attrs(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr1 = smith.nn.Parameter(smith.randn(4, 4))
                self.attr2 = smith.nn.Parameter(smith.randn(4, 4))
                self.lin = smith.nn.Linear(4, 4)

            def forward(self, x, y):
                a = self.attr1 + self.attr1.permute(1, 0)
                x = x - a
                amax = smith.sum(self.attr2, dim=1)
                y = y + amax
                return smith.sigmoid(self.lin(x + y))
        ...
        # Now run both folded and non-folded to check results equal.
        in_x, in_y = smith.randn(4, 4), smith.randn(4)
        fold_result = mod_folded(in_x, in_y)
        base_result = mod(in_x, in_y)
        self.assertTrue(smith.equal(fold_result, base_result))
    ...
    def test_const_fold_submod_hierarchy(self):
        ...
        class TracedThroughModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.internal_attr = smith.nn.Parameter(smith.randn(2, 3))
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.attr = smith.nn.Parameter(smith.randn(2, 3))
    ...
    def test_const_fold_has_inlined_call_module_node(self):
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr = smith.nn.Parameter(smith.randn(2, 3))
                self.mod = smith.nn.Identity()
                self.mod.relu = smith.nn.ReLU()
    ...
    def test_const_fold_module_attr(self):
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.const = smith.nn.Parameter(smith.randn(2, 3))
                self.mod = smith.nn.Identity()
                self.mod.attr = smith.nn.Parameter(smith.randn(2, 3))
    ...
    def test_dict_output(self):
        ...
        fold_result = gm_folded(in_x)
        base_result = mod(in_x)
        self.assertTrue(smith.equal(fold_result["result"], base_result["result"]))
    ...
    def test_two_outputs(self):
        ...
        fold_result = gm_folded(in_x)
        base_result = mod(in_x)
        self.assertTrue(smith.equal(fold_result[0], base_result[0]))
        self.assertTrue(smith.equal(fold_result[1], base_result[1]))
    ...
    def test_three_outputs(self):
        ...
        self.assertTrue(smith.equal(fold_result[0], base_result[0]))
        self.assertTrue(smith.equal(fold_result[1], base_result[1]))
        self.assertTrue(smith.equal(fold_result[2], base_result[2]))
    ...
    def test_check_skip_folding_quant_dequant_pattern(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.weight = smith.nn.Parameter(smith.randn(4, 4))
                self.bias = smith.nn.Parameter(smith.randn(4))
                self.relu = smith.nn.ReLU()

            def forward(self, x):
                quant_weight = smith.quantize_per_tensor(
                    self.weight, 0.5, 3, smith.quint8
                )
                dequant_weight = smith.dequantize(quant_weight)
                output = smith.nn.functional.linear(x, dequant_weight, self.bias)
                return self.relu(output)

        mod = ConstFoldTestModule()
        in_x = smith.randn(2, 4)
        gm = smith.fx.symbolic_trace(mod)

        def skip_folding_quant_dequant(node: smith.fx.Node):
            if node.target != smith.quantize_per_tensor:
                return False
            # If quantize_per_node -> dequantize, then skip folding.
            for user in node.users:
                if user.target == smith.dequantize:
                    return True
            return False
    ...
    def test_fold_module(self):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.lin_input = smith.nn.Parameter(smith.randn(4, 4))
                self.lin = smith.nn.Linear(4, 4)
        ...
        # Now run both folded and non-folded to check results equal.
        inp = smith.randn(4, 4)
        self.assertTrue(smith.equal(mod_folded(inp), mod(inp)))
    ...
    def _test_const_fold_tensor_meta(self, requires_grad):
        ...
        class ConstFoldTestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.attr_1 = smith.nn.Parameter(smith.tensor([[-0.9]]), requires_grad)
                self.attr_2 = smith.nn.Parameter(smith.tensor([[17.1]]), requires_grad)
    ...
    def test_const_fold_partial_graph(self):
        ...
        class TestModule(smith.nn.Module):
            ...
            def forward(self, x):
                probs = smith.empty_permuted(x.shape, [0, 1])
                mask = smith.bernoulli(probs, 1 - self.p)
                return x * mask / (1 - self.p)

        ep = smith.export.export(TestModule(0.4), (smith.randn(5, 10),))
