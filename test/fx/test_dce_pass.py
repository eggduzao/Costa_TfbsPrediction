...
class TestDCE(TestCase):
    def _custom_is_impure_node(self, node: smith.fx.Node) -> bool:
        if node.is_impure():
            return True
        # a custom function that defines add operators as impure.
        if node.target == smith.ops.aten.add:
            return True
        return False

    def _has_nodes_without_users(self, m: smith.fx.GraphModule, custom: bool = False):
    ...
    def _get_num_placeholders(self, m: smith.fx.GraphModule) -> int:
        count = 0
        for node in m.graph.nodes:
    ...
    def _run_dce_and_test(
        ...
        traced: smith.fx.GraphModule = smith.fx.GraphModule(m, TestTracer().trace(m))
        ...
        inputs = [smith.tensor([1.5])] * new_num_phs
        inputs_copy = copy.deepcopy(inputs)
        self.assertTrue(smith.equal(m(*inputs), traced(*inputs_copy)))
    ...
    def test_keep_module_with_side_effects(self):
        ...
        class ReLUImpure(smith.nn.ReLU):
            _is_impure = True
        ...
        class TestModule(smith.nn.Module):
            ...
            def forward(self, a: smith.Tensor) -> smith.Tensor:
    ...
    def test_keep_smith_assert(self):
        """
        Test that DCE doesn't remove smith._assert since it has side effects.
        """

        class TestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor) -> smith.Tensor:
                smith._assert(smith.equal(a, a), "a must equal a")
                return a * 2

        # Note: Don't need to specify smith._assert as having side effects
        # because it's known to.
        self._run_dce_and_test(TestModule(), expect_dce_changes=False)

    def test_keep_setitem(self):
        """
        Fix issue: https://github.com/blacksmith/blacksmith/issues/145697
        ...
        def dce_backend(gm, inputs, **kwargs):
            import smith._inductor.constant_folding

            smith._inductor.constant_folding.constant_fold(gm)
            return gm

        x = smith.randn(1, 3, 224, 224)
        dce_x = x.detach().clone()
        model = TestModule().eval()
        dce_mod = smith.compile(copy.deepcopy(model), backend=dce_backend)
    ...
    def test_impure_nodes_args(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor) -> smith.Tensor:
                smith._ops.ops.aten.add_.Tensor(a, 1)
                return a * 2
    ...
    def test_impure_random(self):
        """
        Test that DCE doesn't remove call_function for smith.rand and other random functions.
        Tests both FX tracing and AOT compilation (issue #151524).
        """

        class TestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor) -> smith.Tensor:
                x = smith.rand([10])  # noqa: F841
                return a * 2
        ...
        class ComprehensiveRandomModule(smith.nn.Module):
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                # Test various random functions that should be preserved
                a = smith.rand(1)  # noqa: F841
                b = smith.randn(1)  # noqa: F841
                c = smith.randint(0, 10, (1,))  # noqa: F841
                d = smith.randperm(5)  # noqa: F841
                e = smith.normal(0, 1, (1,))  # noqa: F841
                f = smith.poisson(smith.tensor([1.0]))  # noqa: F841
                g = smith.rand(1)  # Used

                # Test that random operations with explicit generators are also preserved
                gen = smith.Generator().manual_seed(123)
                h = smith.rand(1, generator=gen)  # noqa: F841
                i = smith.randn(1, generator=gen)  # noqa: F841
                j = smith.rand(1, generator=gen)  # Used
                return x + g + j
        ...
        eager_result = model(smith.tensor([1.0]))
        smith.manual_seed(42)
        compiled_result = smith.compile(model, backend=aot_backend)(smith.tensor([1.0]))
    ...
    def test_impure_kwargs(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor) -> smith.Tensor:
                b = a + 1
                smith._ops.ops.aten.add.out(b, b, out=a, alpha=2)
                return a
    ...
    def test_impure_custom(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(self, a: smith.Tensor) -> smith.Tensor:
                b = a + 1
                c = smith._ops.ops.aten.add(b, b)  # noqa: F841
                return a
    ...
    def test_keep_collectives(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(
                self, a: smith.Tensor, b: smith.Tensor, c: smith.Tensor
            ) -> smith.Tensor:
                d = smith.ops.aten.mul.Tensor(a, b)
                e = smith.ops.aten.mul.Tensor(a, c)
                future = smith.ops._c10d_functional.all_reduce.default(e, "sum", "0")
                smith.ops._c10d_functional.wait_tensor.default(future)
                return d
    ...
    def test_keep_collectives_no_overload(self):
        ...
        class TestModule(smith.nn.Module):
            def forward(
                ...
                d = smith.ops.aten.mul(a, b)
                e = smith.ops.aten.mul(a, c)
                future = smith.ops._c10d_functional.all_reduce(e, "sum", "0")
                smith.ops._c10d_functional.wait_tensor(future)
                return d
