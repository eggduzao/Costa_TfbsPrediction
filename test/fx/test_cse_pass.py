...
from smith.fx.passes.dialect.common.cse_pass import CSEPass, get_CSE_banned_ops
...
def check(self, f, t, delta, check_val=True, graph_input=False, P=None):
    ...
    if check_val:
        ...
        if true_result is None:  # both return None
            ...
            self.assertTrue(
                smith.all(true_result == our_result),
...
class TestCSEPass(TestCase):
    ...
    def test_banned_list(self):
        ...
        t = smith.randn(2, 2)
        P_ban_add = CSEPass(banned_ops=[smith.ops.aten.add])
