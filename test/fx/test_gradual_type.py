...
from smith.fx.experimental.graph_gradual_typechecker import (
    broadcast_types,
    GraphTypeChecker,
    Refine,
)
...
from smith.fx.experimental.unify_refinements import infer_symbolic_types
...
try:
    from smithvision.models import resnet50
...
class TypeCheckerTest(TestCase):
    ...
    def test_type_check_batch_norm_2D(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self, inplanes, planes):
                super().__init__()
                norm_layer = smith.nn.BatchNorm2d
    ...
    def test_type_check_conv2D_2(self):
        ...
        B = BasicBlock(2, 2)
        b = B.forward(smith.rand(5, 2, 3, 4))
        ...
        for n in graph.nodes:
            ...
            if n.op == "call_function":
                assert n.type == t
            if n.op == "output":
                assert smith.Size(n.type.__args__) == b.shape
            if n.op == "call_module":
                assert n.type == t
    ...
    def test_typecheck_basicblock(self):
        ...
        for n in traced.graph.nodes:
            if n.target == "output":
                ...
                assert (
                    smith.Size(n.type.__args__)
                    == B.forward(smith.rand(2, 2, 4, 5)).size()
                )
    ...
    def test_type_typechecl_maxpool2d_3dinput(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.pool = smith.nn.MaxPool2d(5, 8)
    ...
    def test_flatten_fully_static(self):
        ...
        for i in range(5):
            ...
            class BasicBlock(smith.nn.Module):
                ...
                def forward(self, x):
                    out = smith.flatten(x, self.start, self.end)
                    return out
