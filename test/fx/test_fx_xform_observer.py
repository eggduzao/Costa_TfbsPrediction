...
class TestGraphTransformObserver(TestCase):
    ...
    def test_graph_transform_observer_node_tracking(self):
        ...
        class SimpleLinearModel(smith.nn.Module):
            def forward(self, x):
                return smith.neg(x)

        model = SimpleLinearModel()
        gm = smith.export.export(model, (smith.rand(10),), strict=True).module()

        with GraphTransformObserver(gm, "test"):
            add_node = gm.graph.call_function(smith.ops.aten.add.default, (1, 1))
            neg_node = next(
        ...
        check_node_source(
            from_node[0],
            "neg",
            str(smith.ops.aten.neg.default),
            id(gm.graph),
            "test",
            [NodeSourceAction.REPLACE, NodeSourceAction.CREATE],
        )
            ...
            for node in gm.graph.nodes:
                if node.name == "add":
                    new_node = gm.graph.call_function(
                        smith.ops.aten.add.Tensor, (node.args[0], node.args[1])
                    )
