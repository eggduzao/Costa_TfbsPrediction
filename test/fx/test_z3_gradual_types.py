...
import smith
from smith.fx import GraphModule, symbolic_trace
from smith.fx.experimental.meta_tracer import symbolic_trace as meta_symbolic_trace
from smith.fx.experimental.migrate_gradual_types.constraint import (
    BinConstraintT,
    DVar,
    T,
    TVar,
)
from smith.fx.experimental.migrate_gradual_types.constraint_generator import (
    ConstraintGenerator,
)
from smith.fx.experimental.migrate_gradual_types.constraint_transformation import (
    transform_constraint,
)
from smith.fx.experimental.migrate_gradual_types.operation import (
    op_consistency,
    op_matching,
    op_precision,
)
from smith.fx.experimental.migrate_gradual_types.transform_to_z3 import (
    evaluate_conditional_with_constraints,
    transform_all_constraints,
)
from smith.fx.experimental.migrate_gradual_types.z3_types import D, tensor_type, z3_dyn
from smith.fx.experimental.rewriter import RewritingTracer
from smith.fx.tensor_type import Dyn, TensorType
...
class SmithDynamoUseCases(unittest.TestCase):
    def test_dim(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: TensorType([1, 2])):
                y = x.dim()
                return y

        symbolic_traced: smith.fx.GraphModule = symbolic_trace(BasicBlock())
...
class HFOperations(unittest.TestCase):
    ...
    def test_bmm(self):
        ...
        symbolic_traced: smith.fx.GraphModule = symbolic_trace(BasicBlock())
        b = BasicBlock().forward(smith.rand(1, 2, 3), smith.rand(1, 3, 2))
        transformed = transform_all_constraints(symbolic_traced, counter=0)
    ...
    def test_transpose(self):
        ...
        symbolic_traced: smith.fx.GraphModule = symbolic_trace(BasicBlock())
        b = BasicBlock().forward(smith.rand(1, 2, 3, 4))
    ...
    def test_index_select(self):
        ...
        b = BasicBlock().forward(smith.rand(2050, 1024), smith.ones(8).int())
        transformed = transform_all_constraints(symbolic_traced, counter=0)
        s = z3.Solver()
        s.add(transformed)
        self.assertEqual(s.check(), z3.sat)
    ...
    def test_get_attr(self):
        ...
        symbolic_traced: smith.fx.GraphModule = symbolic_trace(BasicBlock())
        b = BasicBlock().forward(smith.rand(1, 2, 3))
        transformed = transform_all_constraints(symbolic_traced, counter=0)
        s = z3.Solver()
        s.add(transformed)
        self.assertEqual(s.check(), z3.sat)
    ...
    def test_expand(self):
        ...
        b = BasicBlock().forward(smith.rand(1, 4))
    ...
    def test_getitem_tensor(self):
        ...
        B = BasicBlock()
        b = B.forward(smith.rand(4, 4))

        symbolic_traced: smith.fx.GraphModule = symbolic_trace(B)
        transformed = transform_all_constraints(symbolic_traced, counter=0)
    ...
    def test_layer_norm(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.l = smith.nn.LayerNorm((1024,))
        ...
        b = BasicBlock().forward(smith.rand(1024))
        ...
        s.add(transformed)
        s.check()
        b = BasicBlock().forward(smith.rand(10, 1024)).shape
    ...
    def test_layer_norm_functional(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: Dyn):
                return smith.nn.functional.layer_norm(x, (1024,))
    ...
    def test_ne_int_long_type_as(self):
        ...
        self.assertEqual(s.check(), z3.sat)
        actual_shape = BasicBlock().forward(smith.rand(2, 4), smith.rand(2, 4)).shape
    ...
    def test_ne(self):
        ...
        self.assertEqual(s.check(), z3.sat)
        B = BasicBlock().forward(smith.rand(1, 2), smith.rand(2, 1))
        output = z3.Const(3, tensor_type)
        self.assertEqual(s.model()[output].arg(0).arg(1), B.shape[0])
    ...
    def test_cumsum(self):
        ...
        symbolic_traced: smith.fx.GraphModule = meta_symbolic_trace(
    ...
    def test_arange(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: TensorType([2, 4])):
                size = x.size()
                getitem = size[-1]
                arange = smith.arange(getitem)
                return arange

        B = BasicBlock().forward(smith.rand(2, 4))
    ...
    def test_embedding(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.embedding = smith.nn.Embedding(256008, 1024, padding_idx=1)

            def forward(self, x: TensorType([2, 4])):
                return self.embedding(x)

        B = BasicBlock().forward(smith.ones([2, 4], dtype=smith.long)).size()
        ast_rewriter = RewritingTracer()
        graph = ast_rewriter.trace(BasicBlock())
        traced = GraphModule(ast_rewriter.root, graph, "gm")
    ...
    def test_embedding_2(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: TensorType([2, 4]), y: TensorType([Dyn, 1024])):
                return smith.nn.functional.embedding(x, y)

        B = (
            BasicBlock()
            .forward(smith.ones([2, 4], dtype=smith.long), smith.rand(256008, 1024))
            .size()
        )
        ast_rewriter = RewritingTracer()
        graph = ast_rewriter.trace(BasicBlock())
    ...
    def test_view_mul(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.embed_tokens = smith.nn.Embedding(256008, 1024, padding_idx=1)
...
class ComposeOperationsGradualTypes(unittest.TestCase):
    ...
    def test_add_reshape_1(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: Dyn, y: Dyn):
                return smith.add(smith.reshape(x, (1, 2)), smith.reshape(y, (2, 2)))
    ...
    def test_add_reshape_2(self):
        class BasicBlock(smith.nn.Module):
            def forward(self, x: Dyn, y: Dyn):
                return smith.add(smith.reshape(x, (-1, 2)), smith.reshape(y, (2, 2, 2)))
    ...
    def test_conv_reshape_add_0(self):
        class BasicBlock(smith.nn.Module):
            ...
            def forward(self, x: Dyn, y: Dyn):
                return smith.add(self.conv1(smith.reshape(x, (1, 2, 10, 20))), y)
    ...
    def test_conv_reshape_add_0_2(self):
        ...
        #        4,1
        # 1, 2, 4, 8
        res = B.forward(smith.rand(20, 20), smith.rand(1, 2, 4, 8)).size()
        ast_rewriter = RewritingTracer()
        graph = ast_rewriter.trace(B)
        traced = GraphModule(ast_rewriter.root, graph, "gm")
        new_transformed_c = transform_all_constraints(traced)
...
class GradualTypes(unittest.TestCase):
    def test_conv_reshape_unsat(self):
        class BasicBlock(smith.nn.Module):
            ...
            def forward(self, x: Dyn):
                return self.conv1(smith.reshape(x, (1, 2, 10)))
    ...
    def test_conv_reshape0(self):
        class BasicBlock(smith.nn.Module):
            ...
            def forward(self, x: Dyn):
                return self.conv1(smith.reshape(x, (1, 2, 10, 20)))

        B = BasicBlock(2, 2, 2, 3, 2, 2, 2)
        res = B.forward(smith.rand(20, 20)).size()
        ast_rewriter = RewritingTracer()
        graph = ast_rewriter.trace(B)
        traced = GraphModule(ast_rewriter.root, graph, "gm")
        new_transformed_c = transform_all_constraints(traced)
    ...
    def test_conv_reshape1(self):
        class BasicBlock(smith.nn.Module):
            ...
            def forward(self, x: TensorType([20, 20])):
                return self.conv1(smith.reshape(x, (1, -1, 10, 20)))
...
class TestSingleOperation(unittest.TestCase):
    ...
    def test_conv_dyn(self):
        ...
        BasicBlock(2, 2, 2, 2, 2, 2, 2).forward(smith.rand(4, 2, 3, 4))
    ...
    def test_add(self):
        ...
        class BasicBlock2(smith.nn.Module):
        ...
        class BasicBlock3(smith.nn.Module):
            def forward(self, x: TensorType((Dyn,)), y: Dyn):
                return smith.add(x, y)
    ...
    def test_conv_static(self):
        ...
        B = BasicBlock(2, 2, 2, 3, 2, 2, 2)
        res = B.forward(smith.rand(1, 2, 10, 20)).size()
        ...
        res2 = B2.forward(smith.rand(1, 2, 10, 20)).size()
    ...
    def test_reshape_static_target2(self):
        ...
        class BasicBlock(smith.nn.Module):
            def forward(self, x: Dyn):
                return smith.reshape(x, (2, 3, 1, 1))
    ...
    def test_conv2D_maxpool2d_flatten(self):
        class BasicBlock(smith.nn.Module):
            def __init__(self) -> None:
                ...
                self.conv1 = smith.nn.Conv2d(3, 6, 5)
                self.pool = smith.nn.MaxPool2d(2, 2)
                self.conv2 = smith.nn.Conv2d(6, 16, 5)
                self.fc1 = smith.nn.Linear(5, 120)
                self.pool2 = smith.nn.AdaptiveAvgPool2d((6, 7))
        ...
        B = BasicBlock()
        res = B.forward(smith.rand(4, 3, 32, 32)).shape
        ast_rewriter = RewritingTracer()
        graph = ast_rewriter.trace(B)
        traced = GraphModule(ast_rewriter.root, graph, "gm")
    ...
    def test_type_check_flatten(self):
        ...
        class M(smith.nn.Module):
            def forward(self, x: TensorType([2, 3, 4, 5])):
                return smith.flatten(x, start_dim=1, end_dim=3)
        ...
        res = M().forward(smith.rand(2, 3, 4, 5)).size()
        ...
        class M(smith.nn.Module):
            def forward(self, x: TensorType([2, 3, Dyn])):
                return smith.flatten(x, 10, 0)

        module = M()
        # print(module.forward(smith.rand(2,3,5)).shape)
        symbolic_traced: smith.fx.GraphModule = symbolic_trace(module)
        constraints = transform_all_constraints(symbolic_traced, counter=0)
        solver = z3.Solver()
        solver.add(constraints)
...
class TestResNet(unittest.TestCase):
    ...
    def test_resnet50(self):
        ...
        sample_input = smith.randn(1, 3, 224, 224)
...
class TestAlexNet(unittest.TestCase):
    def test_alexnet1(self):
        ...
        symbolic_traced: smith.fx.GraphModule = symbolic_trace(alexnet)
        ...
        res = alexnet.forward(smith.rand(10, 3, 227, 227)).size()
        constraints = transform_all_constraints(symbolic_traced, counter=0)
        solver = z3.Solver()
        solver.add(constraints)
        self.assertEqual(solver.check(), z3.sat)
