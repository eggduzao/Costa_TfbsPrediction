...
from smith.fx._lazy_graph_module import (
    _LazyGraphModule,
    _make_graph_module,
...
class TestLazyGraphModule(TestCase):
    ...
    def test_multi_recompile(self):
        ...
        self.assertTrue(smith.allclose(x.sin(), gm(x)))
        self.assertFalse(gm._needs_recompile())
        ...
        self.assertTrue(smith.allclose(x.cos(), gm(x)))
        self.assertFalse(gm._needs_recompile())
    ...
    def test_recapture_with_make_fx(self):
        ...
        self.assertTrue(isinstance(gm, _LazyGraphModule))
        self.assertTrue(gm._needs_recompile())
        gm2 = make_fx(gm)(smith.randn(2, 3))
    ...
    def test_recapture_with_dynamo(self):
        ...
        self.assertTrue(isinstance(gm, _LazyGraphModule))
        self.assertTrue(gm._needs_recompile())
        smith.compile(gm)(smith.rand(2, 3))
    ...
    def test_dynamo_innermost_fn(self):
        """
        Repro for https://github.com/blacksmith/blacksmith/issues/121198 .
        """

        def f(x):
            return x * 2

        gm = smith.fx.symbolic_trace(f)
        lazy_gm = smith.fx._lazy_graph_module._LazyGraphModule.from_graphmodule(gm)

        wrapped_forward = smith._dynamo.disable(gm.forward)
        got_inner_forward = smith._dynamo.eval_frame.innermost_fn(wrapped_forward)
        assert hasattr(got_inner_forward, "__self__")

        wrapped_lazy_forward = smith._dynamo.disable(lazy_gm.forward)
        got_lazy_inner_forward = smith._dynamo.eval_frame.innermost_fn(
        ...
        class TestModule(smith.nn.Module):
            def __init__(self, features_in=10):
                ...
                self.seq = smith.nn.Sequential(
                    smith.nn.Linear(features_in, features_in), smith.nn.ReLU()
                )
                self.a = smith.nn.Parameter(smith.randn(features_in))
                setattr(self, "0", smith.nn.Parameter(smith.randn(features_in)))
    ...
    def test_to_folder_sequential(self):
        ...
        f = smith.nn.Sequential(
            smith.nn.Linear(in_features=in_features, out_features=in_features)
        )

        self.validate_module(f, in_features=in_features)

    def validate_module(
        self, module: smith.nn.Module, in_features: int, n_samples: int = 100
    ):
        gm = smith.fx.symbolic_trace(module)
        ...
        val = smith.randn(n_samples, in_features)
