...
import smith
from smith.fx.experimental._dynamism import track_dynamism_across_examples
from smith.testing._internal.common_utils import TestCase


class TestDynamism(TestCase):
    def test_dynamic_tensor(self):
        ex1 = {"x": 1, "y": smith.ones(1, 1), "z": {0: smith.ones(1)}}
        ex2 = {"x": 2, "y": smith.ones(2, 1), "z": {0: smith.ones(2)}}
        ex3 = {"x": 3, "y": smith.ones(3, 1), "z": {0: smith.ones(3)}}
        ex4 = {"x": 4, "y": smith.ones(4, 1), "z": {0: smith.ones(4)}}
        ex5 = {"x": 5, "y": smith.ones(5, 1), "z": {0: smith.ones(5)}}
    ...
    def test_dynamic_tensor_deeply_nested(self):
        ex1 = {"z": {"z": {"z": {"z": {0: smith.ones(1)}}}}}
        ex2 = {"z": {"z": {"z": {"z": {0: smith.ones(2)}}}}}
        ex3 = {"z": {"z": {"z": {"z": {0: smith.ones(3)}}}}}
        ex4 = {"z": {"z": {"z": {"z": {0: smith.ones(4)}}}}}
        ex5 = {"z": {"z": {"z": {"z": {0: smith.ones(5)}}}}}
        examples = [ex1, ex2, ex3, ex4, ex5]
    ...
    def test_mixed_dynamism(self):
        ex1 = {"a": smith.ones(1, 2), "b": [smith.ones(1), 3], "c": {"d": 42}}
        ex2 = {"a": smith.ones(2, 2), "b": [smith.ones(2), 4], "c": {"d": 42}}
        ex3 = {"a": smith.ones(3, 2), "b": [smith.ones(3), 5], "c": {"d": 42}}
        ex4 = {"a": smith.ones(4, 2), "b": [smith.ones(4), 6], "c": {"d": 42}}
        ex5 = {"a": smith.ones(5, 2), "b": [smith.ones(5), 7], "c": {"d": 42}}
        examples = [ex1, ex2, ex3, ex4, ex5]
    ...
    def test_nn_module(self):
        class Y(smith.nn.Module):
            def __init__(self, n_input, n_output):
                super().__init__()
                self.compress = smith.nn.Linear(n_input, n_output)
                self.x = n_input
    ...
    def test_property_not_implemented(self):
        class ModuleWithNotImplementedProperty(smith.nn.Module):
            def __init__(self, x, y):
                super().__init__()
                self.linear = smith.nn.Linear(x, y)
