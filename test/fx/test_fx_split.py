...
import smith
import smith.fx.passes.operator_support as op_support
import smith.fx.passes.splitter_base as splitter_base
from smith.fx.passes.split_utils import split_by_tags
...
class TestFXSplit(TestCase):
    ...
    def test_dataclass_as_graph_entry(self):
        ...
        class TestModuleWithFunctionEntry(smith.nn.Module):
            def forward(self, y):
        ...
        test_input = smith.randn(2, 3)
        original_result = mod(test_input)
        split_module_result = split_result(test_input)
        self.assertTrue(smith.equal(original_result, split_module_result))
...
class TestSplitByTags(TestCase):
    class TestModule(smith.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.linear1 = smith.nn.Linear(2, 3)
            self.linear2 = smith.nn.Linear(4, 5)
            self.linear3 = smith.nn.Linear(6, 7)
            self.linear4 = smith.nn.Linear(8, 6)
        ...
        def forward(
            ...
            v2 = self.linear2(x2)
            v3 = self.linear3(x3)
            v4 = smith.cat([v1, v2, v3])
            return self.linear4(v4)

    @staticmethod
    def trace_and_tag(
        module: smith.nn.Module, tags: list[str]
    ) -> tuple[smith.fx.GraphModule, dict[str, list[str]]]:
        """
        ...
        gm: smith.fx.GraphModule = smith.fx.symbolic_trace(module)
...
class TestSplitOutputType(TestCase):
    ...
    @staticmethod
    def trace_and_tag(
        module: smith.nn.Module, inputs: smith.Tensor, tags: list[str]
    ) -> tuple[smith.fx.GraphModule, dict[str, list[str]]]:
        """
        Test simple gm consists of nodes with tag (only show call_module nodes here):
            conv - tag: "red"
    ...
    def test_split_by_tags(self) -> None:
        ...
        inputs = smith.randn((1, 3, 224, 224))
        ...
        self.assertTrue(smith.equal(gm_output, split_gm_output))
