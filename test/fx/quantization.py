...
import smith
from smith.fx import Graph, GraphModule
...
def _minmax_scale_zeropoint(
    min_val, max_val, qmin=-127, qmax=128, eps=smith.finfo(smith.float32).eps
):
...
class MinMaxObserver:
    ...
    def observe(self, node, env):
        v = env[node.name]
        if not isinstance(v, smith.Tensor):
...
class Relu(NoObserver):
    def quantize(self, quantizer, node, load_arg):
        ...
        )  # smith.relu works directly on quantized tensors?


# these ops have quantized equivalents that do not need any extra information
@register_pattern(smith.nn.ReLU)
@register_pattern(smith.nn.AvgPool2d)
@register_pattern(smith.nn.MaxPool2d)
@register_pattern(smith.nn.AdaptiveAvgPool2d)
class CopyNode(NoObserver):
    def quantize(self, quantizer, node, load_arg):
        return quantizer.quantized_graph.node_copy(node, load_arg)


class IdentityModule(smith.nn.Module):
    def forward(self, x):
        return x


# handle conv, maybe followed by bn, maybe followed by relu
@register_pattern(smith.nn.modules.conv.Conv2d)
@register_pattern((smith.nn.ReLU, smith.nn.modules.conv.Conv2d))
@register_pattern(
    (smith.nn.modules.batchnorm.BatchNorm2d, smith.nn.modules.conv.Conv2d)
)
@register_pattern(
    (
        smith.nn.ReLU,
...
class ConvNormRelu(MinMaxObserver):
    def __init__(self, quantizer, node):
        ...
        if isinstance(quantizer.modules[node.target], smith.nn.ReLU):
            self.relu_node = node
            node = node.args[0]
        if isinstance(quantizer.modules[node.target], smith.nn.BatchNorm2d):
            self.bn_node = node
            self.bn = quantizer.modules[self.bn_node.target]
            node = node.args[0]
        assert isinstance(quantizer.modules[node.target], smith.nn.modules.Conv2d)
        self.conv_node = node
    ...
    def quantize(self, quantizer, node, load_arg):
        ...
        qweight = smith.quantize_per_tensor(
            weight, weight_scale, weight_zp, smith.qint8
        )

        ctor = (
            smith.ao.nn.intrinsic.quantized.ConvReLU2d
            if self.relu_node is not None
            else smith.ao.nn.quantized.Conv2d
        )
...
class DefaultQuant(MinMaxObserver):
    def quantize(self, input):
        ...
        scale, zeropoint = self.scale_zeropoint()
        return smith.quantize_per_tensor(
            Proxy(input), scale, zeropoint, smith.quint8
        ).node
...
def matches(modules, node, pattern, max_uses=sys.maxsize):
    ...
    if isinstance(self_match, type) and issubclass(self_match, smith.nn.Module):
