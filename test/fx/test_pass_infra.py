...
from smith.fx.passes.infra.pass_manager import (
...
def replace_mul_with_div_pass(gm) -> PassResult:
    modified = False
    for node in gm.graph.nodes:
        if node.op == "call_function" and node.target == smith.mul:
            node.target = smith.div
            modified = True
    return PassResult(gm, modified)
...
def replace_sub_with_add_pass(gm) -> PassResult:
    for node in gm.graph.nodes:
        if node.op == "call_function" and node.target == smith.sub:
...
class TestPassManager(TestCase):
    def test_pass_manager(self):
        ...
        m = AddModule()
        traced_m = smith.fx.symbolic_trace(m)
        pm = PassManager(
            passes=[
        ...
        for node in modified_m.graph.nodes:
            if node.op == "call_function":
                self.assertEqual(node.target, smith.add)
