...
def make_test_cls(cls, xfail_prop="_expected_failure_graph_pickler"):
    return make_test_cls_with_patches(
        ...
        (
            ...
            "fx_compile_mode",
            smith._inductor.compile_fx.FxCompileMode.SERIALIZE,
        ),
...
class TestGraphPickler(TestCase):
    def setUp(self):
        ...
        self._stack.enter_context(
            patch(
                "smith._inductor.compile_fx.fx_compile_mode",
                smith._inductor.compile_fx.FxCompileMode.SERIALIZE,
            )
        )
    ...
    def test_simple(self):
        ...
        check_model(self, fn, (smith.tensor([False, True]), smith.tensor([True, True])))
...
class TestHigherOrderOperatorPickle(TestCase):
    ...
    def setUp(self):
        super().setUp()
        from smith.fx._graph_pickler import _OpPickleData, GraphPickler, Options
    ...
    def test_higher_order_operator_pickle_roundtrip(self):
        ...
        from smith.fx._graph_pickler import _HigherOrderOperatorPickleData
    ...
    def test_higher_order_operator_unpickle(self):
        ...
        from smith.fx._graph_pickler import (
...
class TestHopSchemaPickle(TestCase):
    ...
    def test_hop_schema_pickle_with_arguments(self):
        ...
        arg = smith._C.Argument("x", tensor_type, None, None, False, None)
        ret = smith._C.Argument("", tensor_type, None, None, False, None)
        schema = HopSchema(
...
class TestSerializedGraphModule(TestCase):
    ...
    def test_simple_graph_module_roundtrip(self):
        ...
        """
        from smith._funcsmith._aot_autograd.aot_autograd_result import (
        ...
        gm = smith.fx.symbolic_trace(SimpleModule())
        ...
        test_input = smith.tensor([1.0, 2.0, 3.0])
    ...
    def test_graph_module_with_multiple_ops(self):
        ...
        class MultiOpModule(smith.nn.Module):
            def forward(self, x, y):
                z = x + y
                w = z * 2
                return w - 1

        gm = smith.fx.symbolic_trace(MultiOpModule())
        serialized = SerializedGraphModule(gm)
        deserialized = serialized.deserialize()
        x = smith.tensor([1.0, 2.0])
        y = smith.tensor([3.0, 4.0])
...
class TestGraphModuleGetState(TestCase):
    ...
    def test_graph_module_getstate_is_called(self):
        ...
        from smith.fx._graph_pickler import _GraphModulePickleData, Options
...
class TestNodeMetadataKeyFilter(TestCase):
    ...
    def test_default_filter_excludes_known_unserializable_keys(self):
        ...
        """
        from smith.fx._graph_pickler import _node_metadata_key_filter_safe
    ...
    def test_node_pickle_data_uses_metadata_filter(self):
        ...
        """
        from smith.fx._graph_pickler import _NodePickleData, Options
        ...
        options = Options()
        node_mapping: dict[smith.fx.Node, _NodePickleData] = {}
        for node in gm.graph.nodes:
