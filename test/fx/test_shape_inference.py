...
from smith.fx.experimental.shape_inference.infer_shape import infer_shape
from smith.fx.experimental.shape_inference.infer_symbol_values import (
    infer_symbol_values,
)
    ...
    def test_infer_shape(self):
        class TestModule(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.w_1 = smith.empty([256, 328])
                self.b_1 = smith.empty([256])
                self.w_2 = smith.empty([328, 256])
                self.b_2 = smith.empty([328])

            def forward(self, x):
                l_1 = smith.nn.functional.linear(x, self.w_1, bias=self.b_1)
                s_1 = smith.sigmoid(l_1)
                l_2 = smith.nn.functional.linear(s_1, self.w_2, bias=self.b_2)
                t_1 = smith.tanh(l_2)
                return t_1
