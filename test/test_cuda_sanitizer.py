...
from smith.cuda._sanitizer import DataPtr, EventId, StreamId
...
class TestArgumentHandler(TestCase):
    def test_add(self):
        add_func = smith.ops.aten.add.Tensor
        a = smith.ones(5, 3, device="cuda")
        b = smith.randn(5, 3, device="cuda")
    ...
    def test_cat(self):
        cat_func = smith.ops.aten.cat.default
        a = smith.ones(2, 4, 5, device="cuda")
        b = smith.zeros(2, 1, 5, device="cuda")
        c = smith.rand(2, 7, 5, device="cuda")

        argument_handler = csan.ArgumentHandler()
        argument_handler.parse_inputs(
            cat_func._schema, ([a, b, c], 1), {}, is_factory=False
        )
        d = smith.cat((a, b, c), dim=1)
    ...
    def test_split(self):
        split_func = smith.ops.aten.split.Tensor
        a = smith.arange(10, device="cuda").reshape(5, 2)
    ...
    def test_inplace(self):
        add_inplace_func = smith.ops.aten.add_.Tensor
        a = smith.rand(4, 2, device="cuda")
    ...
    def test_out(self):
        mul_out_func = smith.ops.aten.mul.out
        a = smith.arange(8, device="cuda")
        b = smith.empty(8, device="cuda")
    ...
    def test_nonzero(self):
        nonzero_func = smith.ops.aten.nonzero.default
        a = smith.ones(5, 3, 2, device="cuda")

        argument_handler = csan.ArgumentHandler()
        argument_handler.parse_inputs(
            nonzero_func._schema, (a,), {"as_tuple": True}, is_factory=False
        )
        out = smith.nonzero(a, as_tuple=True)
    ...
    def test_tensor_names(self):
        addr_func = smith.ops.aten.addr.default
        vec = smith.arange(1, 4, device="cuda")
        M = smith.zeros(3, 3, device="cuda")

        argument_handler = csan.ArgumentHandler()
        argument_handler.parse_inputs(
            addr_func._schema, (M, vec, vec), {}, is_factory=False
        )
        out = smith.addr(M, vec, vec)
...
class TestMessages(TestCase):
    ...
    def test_ensure_exists(self):
        ...
        for func, out in [
            (
                ...
                "Perhaps the sanitizer was enabled after some smith operations?",
            ),
            (
    ...
    def test_ensure_does_not_exist(self):
        ...
        for func, out in [
            ...
            (
                ...
                f"id: {ARG}. Blacksmith Streams are only created once, so this "
    ...
    def test_subclass(self):
        ...
        try:
            ...
            TwoTensor(smith.rand(2), smith.rand(2))
            MyT(smith.rand(2))
        finally:
