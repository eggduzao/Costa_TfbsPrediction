...
createBoxplot <- function(dataFrame, yLabel, abLineList, myCexX, myWidth, outFileName){
  ...
  p = bwplot(value ~ variable, data = test_df2, scales=list(tck=0, x=list(rot=0,cex=myCexX), y=list(cex=1.0)), col = "black",
         main = "", xlab = "", ylab = list(yLabel,cex=1.0),
...
for(inName in inList){
  ...
  # Creating boxplot
  createBoxplot(bx,"FP-Exp Distribution (%)",c(0,20,40,60,80,100),1.0,4,outFileNameEps)

  # Performing T-test
  hypTest(bx, colnames(bx), outFileNameTest)
  
  aux=rbind(aux,medVec)
