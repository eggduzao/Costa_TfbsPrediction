...
outLoc = "/work/eg474423/eg474423_Projects/trunk/TfbsPrediction/Results/StatisticalTests/KmerBiasFull/bias/dict/"
...
for inFile in inList:
  ...
  for i in range(0,len(typeList)):
    ...
    outputName = outLoc+"_".join([typeList[i],lab,cell,k_nb])

    # Execution
    myL = labelList[i]+"_"+"_".join([lab,cell])
    clusterCommand = "bsub -J "+myL+" -o "+myL+"_out.txt -e "+myL+"_err.txt "
    clusterCommand += "-W 100:00 -M 24000 -S 100 -P izkf -R \"select[hpcwork]\" ./createKmerDict_pipeline.zsh "
    clusterCommand += k_nb+" "+allTagsFg+" "+bamFileName+" "+hsFileName+" "+fastaFileName+" "+csFileName+" "+outputName
