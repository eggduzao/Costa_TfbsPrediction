...
for(c in 1:length(cellList)){
  ...
  # Open data matrices
  dataO = read.table(paste(loc,"PearsonTop","3000",cell,'signal_original.txt',sep='/'), sep='\t', header=T)
  dataB = read.table(paste(loc,"PearsonTop","3000",cell,'signal_bias.txt',sep='/'), sep='\t', header=T)
  dataC = read.table(paste(loc,"PearsonTop","3000",cell,'signal_corrected.txt',sep='/'), sep='\t', header=T)
  pwmLoc = paste(loc,"PearsonTop","3000",cell,'PWM',sep='/')
  ...
  for(i in 0:((ncol(dataO)/2)-1)){
    ...
    out = paste(loc,'/LineGraphsTop/3000/',cell,'/',name,'.eps',sep='')
