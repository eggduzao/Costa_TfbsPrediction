...
cellList = c("DU_H1hesc", "DU_HeLaS3", "DU_HepG2", "DU_Huvec", "DU_K562", "DU_Mcf7", "DU_Saos2", "UW_LnCaP" )
...
for(c in 1:length(cellList)){
  ...
  # Open data matrices
  dataO = read.table(paste(loc,"PearsonNaked",cell,'signal_original.txt',sep='/'), sep='\t', header=T)
  dataB = read.table(paste(loc,"PearsonNaked",cell,'signal_bias.txt',sep='/'), sep='\t', header=T)
  dataC = read.table(paste(loc,"PearsonNaked",cell,'signal_corrected.txt',sep='/'), sep='\t', header=T)
  pwmLoc = paste(loc,"PearsonNaked",cell,'PWM',sep='/')
  ...
  for(i in 0:((ncol(dataO)/2)-1)){
    ...
    out = paste(loc,'/LineGraphsNaked/',cell,'/',name,'.eps',sep='')
