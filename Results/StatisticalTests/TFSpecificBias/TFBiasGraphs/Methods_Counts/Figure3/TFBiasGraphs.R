...
createGroupedCorrelation <- function(matrix1, matrix2, labelMatrix, outFileName, xLab, yLab, legendLab, xLim, yLim){
  ...
  # Legend
  legend(-0.15,1.93, legend=legendLab, lty=NA, col=colorList, xpd = TRUE, ncol=4, bty = "n", lwd=2.0, cex=1.63, text.width = 0.12,
         pch = 19)
...
for (j in 1:length(aucTypeList)){
  ...
  for (i in 1:length(inList)){
    ...
    for (j in 1:length(countNameList)){
      ...
      methodList = c("FOS","PWM","CUELLAR","PIQUE","NEPH","BOYLE","HMMD","HMMD_BC")
      legendList = c("FOS","PWM","Cuellar","Centipede","Neph","Boyle","HINT","HINT(BC)")
      methodIndexList = c(4,5,12,13,11,10,8,9)
  ...
  outLoc = paste(loc,"/TFBiasGraphs/Methods_Counts/Figure3/",sep="")
