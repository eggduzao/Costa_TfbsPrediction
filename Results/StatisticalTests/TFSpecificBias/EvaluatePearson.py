...
maxTfbs = sys.argv[9] # 0 for all peaks with ChIP.
...
for k in sortedKeys:
  ...
  for tfbs in tfbsDict[k]:
    ...
    for i in range(0,window):
      finalCorrectedF[i] += log(fCountA[i+(window/2)]+1) - log(fBiasN[i]+1)
      finalCorrectedR[i] += log(rCountA[i+(window/2)]+1) - log(rBiasN[i]+1)
