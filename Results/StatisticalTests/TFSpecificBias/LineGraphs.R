...
#invList <- c("P53", "P53_K562", "GR_withDEX")
invList <- c()
...
createLine2 <- function(vec1,vec2,vec3,vec4,vec5,vec6,title,outFileName){
  ...
  plot(xVec, newvec1, type="l", col=plot_colors2[1], xlab="Coordinates from Motif Center", 
       ylab="       Average DNase-seq Signal", lwd=2, xlim=c((-length(newvec1)/2),((length(newvec1)/2)-1)), 
       main="", lty=plot_lty2[1], axes = FALSE, ylim=c(-0.25,1.05))
...
for(c in 1:length(cellList)){
  ...
  for(i in 0:((ncol(dataO)/2)-1)){
    ...
    out = paste(loc,'/LineGraphs/',cell,'/',name,'.eps',sep='')
