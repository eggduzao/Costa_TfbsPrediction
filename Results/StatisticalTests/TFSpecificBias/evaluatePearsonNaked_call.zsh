...
do
  ...
  do
    ...
    outputLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/"$labLabel"_"$cell"/"
    pwmLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/"$labLabel"_"$cell"/PWM/"

    mkdir -p $outputLocation
    mkdir -p $pwmLocation

    # Execution
    bsub -J "EPN" -o "EPN_out.txt" -e "EPN_err.txt" -W 24:00 -M 12000 -S 100 -P izkf -R "select[hpcwork]" ./evaluatePearson_pipeline.zsh $fBiasFileName $rBiasFileName $bamFileName $fastaFileName $csFileName $tfbsLocation $outputLocation $pwmLocation "0"
...
outputLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/UW_denovo/"
pwmLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/UW_denovo/PWM/"
mkdir -p $outputLocation
mkdir -p $pwmLocation
...
outputLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/DU_Saos2/"
pwmLocation=$p"/TfbsPrediction/Results/StatisticalTests/TFSpecificBias/PearsonNaked/DU_Saos2/PWM/"
mkdir -p $outputLocation
mkdir -p $pwmLocation
