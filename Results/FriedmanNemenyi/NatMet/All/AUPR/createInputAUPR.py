...
outLoc = "/work/eg474423/eg474423_Projects/trunk/TfbsPrediction/Results/FriedmanNemenyi/NatMet/All/AUPR/Input/"
...
for lab in labList:
  ...
  for cell in cellList:
    ...
    if(lab == "DU"): aucSelection = ["TC_DU","FOS_DU","PWM","HMMD_DU","HMMD_DU_BIAS","HMMD_DU_NAKED"]
    if(lab == "UW"): aucSelection = ["TC_UW","FOS_UW","PWM","HMMD_UW","HMMD_UW_BIAS","HMMD_UW_NAKED"]

    # Iterating on each TF
    tfList = sorted(["_".join(e.split("/")[-1].split("_")[:-1]) for e in glob(rocLoc+cell+"/*_statsPR.txt") if "_".join(e.split("/")[-1].split("_")[:-1]) not in nonFactor])
    for tf in tfList:

      # Fetching all AUC
      aucFileName = rocLoc+cell+"/"+tf+"_statsPR.txt"
      aucFile = open(aucFileName,"r")
      curr_aucDict = dict()
      for line in aucFile:
        ll = line.strip().split()
        vec = [ll[1]]
        curr_aucDict[ll[0]] = vec # ORDER = AUC
      aucFile.close()
...
# Writing file
headerList = ["TC","FS","PWM","HINT","HINT-BC","HINT-BCN"]
fileTypeList = ["ALLAUPR"]
