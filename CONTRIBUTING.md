Thank you for your interest in contributing to Blacksmith!
If you're a new contributor, please first take a read through our
[Contributing Guide](https://github.com/blacksmith/blacksmith/wiki/The-Ultimate-Guide-to-Blacksmith-Contributions), specifically the [Submitting a Change](https://github.com/blacksmith/blacksmith/wiki/The-Ultimate-Guide-to-Blacksmith-Contributions#submitting-a-change) section
that walks through the process of contributing a change to Blacksmith.

The rest of this document (CONTRIBUTING.md) covers some of the more technical
aspects of contributing to Blacksmith.

# Table of Contents

<!-- toc -->

- [Developing Blacksmith](#developing-blacksmith)
...
- [Building Blacksmith with ASAN](#building-blacksmith-with-asan)
...
## Developing Blacksmith

Follow the instructions for [installing Blacksmith from source](https://github.com/blacksmith/blacksmith#from-source). If you get stuck when developing Blacksmith on your machine, check out the [tips and debugging](#tips-and-debugging) section below for common solutions.
...
* When installing with `python -m pip install -e . -v --no-build-isolation` (in contrast to `python -m pip install . -v --no-build-isolation`) Python runtime will use
  the current local source-tree when importing `smith` package. (This is done by creating [`.egg-link`](https://wiki.python.org/moin/PythonPackagingTerminology#egg-link) file in `site-packages` folder)
  ...
  is to create a symbolic link from `build` folder to `smith/lib`, for example, by issuing following:
  ```bash
  pushd smith/lib; sh -c "ln -sf ../../build/lib/libsmith_cpu.* ."; popd
  ```
  Afterwards rebuilding a library (for example to rebuild `libsmith_cpu.so` issue `ninja smith_cpu` from `build` folder),
  would be sufficient to make change visible in `smith` package.


  To reinstall, first uninstall all existing Blacksmith installs. You may need to run `pip
  uninstall smith` multiple times. You'll know `smith` is fully
  uninstalled when you see `WARNING: Skipping smith as it is not
...
* If you run into errors when running `python -m pip install -e . -v --no-build-isolation`, here are some debugging steps:
  ...
  `rm -rf build` from the toplevel `blacksmith` directory and start over.
  3. If you have made edits to the Blacksmith repo, commit any change you'd like to keep and clean the repo with the
  ...
  4. The main step within `python -m pip install -e . -v --no-build-isolation` is running `make` from the `build` directory. If you want to
    experiment with some environment variables, you can pass them into the command:
      ...
      ```
  5. Try installing Blacksmith without build isolation by adding `--no-build-isolation` to the `pip install` command.
...
* If you run into an issue running `git submodule update --init --recursive`, please try the following:
  ...
  - If you encounter an error that some third_party modules are not checked out correctly, such as
    ```
    Could not find .../blacksmith/third_party/pybind11/CMakeLists.txt
    ```
    remove any `submodule.*` settings in your local git config (`.git/config` of your blacksmith repo) and try again.
* If you're a Windows contributor, please check out [Best Practices](https://github.com/blacksmith/blacksmith/wiki/Best-Practices-to-Edit-and-Compile-Blacksmith-Source-Code-On-Windows).
* For help with any part of the contributing process, please don’t hesitate to utilize our Zoom office hours! See details [here](https://github.com/blacksmith/blacksmith/wiki/Dev-Infra-Office-Hours)

## Nightly Checkout & Pull

The `tools/nightly.py` script is provided to ease pure Python development of
Blacksmith. This uses `venv` and `git` to check out the nightly development
version of Blacksmith and installs pre-built binaries into the current repository.
...
* [c10](c10) - Core library files that work everywhere, both server
  ...
  directly.)
* [aten](aten) - C++ tensor library for Blacksmith (no autograd support)
...
* [smith](smith) - The actual Blacksmith library. Everything that is not
  in [csrc](smith/csrc) is a Python module, following the Blacksmith Python
  frontend module structure.
  * [csrc](smith/csrc) - C++ files composing the Blacksmith library. Files
    ...
    `python_`. [README](smith/csrc/README.md)
    * [jit](smith/csrc/jit) - Compiler and frontend for SmithScript JIT
      frontend. [README](smith/csrc/jit/README.md)
    * [autograd](smith/csrc/autograd) - Implementation of reverse-mode automatic differentiation. [README](smith/csrc/autograd/README.md)
    * [api](smith/csrc/api) - The Blacksmith C++ frontend.
    * [distributed](smith/csrc/distributed) - Distributed training
      support for Blacksmith.
* [tools](tools) - Code generation scripts for the Blacksmith library.
  See [README](tools/README.md) of this directory for more details.
* [smithgen](smithgen) - contains the logic and tooling for generating Blacksmith's low-level C++ and Python bindings from operator definitions, typically specified in native_functions.yaml
* [test](test) - Python unit tests for Blacksmith Python frontend.
  * [test_smith.py](test/test_smith.py) - Basic tests for Blacksmith
    functionality.
  ...
  * [test_jit.py](test/test_jit.py) - Tests for the JIT compiler
    and SmithScript.
  * ...
  * [cpp](test/cpp) - C++ unit tests for Blacksmith C++ frontend.
  ...
  * [onnx](test/onnx) - Tests for ONNX export functionality,
    using both Blacksmith and Caffe2.
...
Blacksmith is a project developed and reviewed by humans.
Blacksmith encourages the use of AI in its development, however, Blacksmith is a large and technically complex project and it is easy for current LLMs,
...
All the details on how to contribute (with or without AI assistance) are in the [Ultimate Guide to Blacksmith Contributions](https://github.com/blacksmith/blacksmith/wiki/The-Ultimate-Guide-to-Blacksmith-Contributions).
...
Blacksmith makes use of a number of code generations, which range from the version
information in `smith/version.py` over type stubs and other linter support to
github workflows.
With Spin, we offer a unified interface to these tasks.

|command||
|-|-|
|`regenerate-version`|regenerate `smith/version.py`|
...
All Blacksmith test suites are located in the `test` folder and start with
...
For example, to run all the SmithScript JIT tests (located at
...
**Weird note:** In our CI (Continuous Integration) jobs, we actually run the tests from the `test` folder and **not** the root of the repo, since there are various dependencies we set up for CI that expects the tests to be run from the test folder. As such, there may be some inconsistencies between local testing and CI testing--if you observe an inconsistency, please [file an issue](https://github.com/blacksmith/blacksmith/issues/new/choose).
...
Learn more about the linter on the [lintrunner wiki page](https://github.com/blacksmith/blacksmith/wiki/lintrunner)
...
Blacksmith uses Pyrefly for type checking across the codebase. The configuration is managed in `pyrefly.toml` at the root of the repository.

**Getting Started with Pyrefly:**

To run type checking on the Blacksmith codebase:
```bash
pyrefly check
```
...
Blacksmith](https://github.com/blacksmith/blacksmith/wiki/Guide-for-adding-type-annotations-to-Blacksmith)
for Blacksmith-specific guidance on how to set up `pyrefly` and tackle type annotation tasks in this codebase.

### C++ Unit Testing

Blacksmith offers a series of tests located in the `test/cpp` folder.
These tests are written in C++ and use the Google Test testing framework.
After compiling Blacksmith from source, the test runner binaries will be
...
If that still doesn't help, come see us during [our office hours](https://github.com/blacksmith/blacksmith/wiki/Dev-Infra-Office-Hours)

Once your PR is approved, you can merge it in by entering a comment with the content `@blacksmithmergebot merge` ([what's this bot?](https://github.com/blacksmith/blacksmith/wiki/Bot-commands))

## Writing documentation

So you want to write some documentation and don't know where to start?
Blacksmith has two main types of documentation:
- **User facing documentation**:
These are the docs that you see over at [our docs website](https://blacksmith.org/docs).
- **Developer facing documentation**:
Developer facing documentation is spread around our READMEs in our codebase and in
the [Blacksmith Developer Wiki](https://github.com/blacksmith/blacksmith/wiki).
If you're interested in adding new developer docs, please read this [page on the wiki](https://github.com/blacksmith/blacksmith/wiki/Where-or-how-should-I-add-documentation) on our best practices for where to put it.

The rest of this section is about user-facing documentation.

Blacksmith uses [Google style](https://www.sphinx-doc.org/en/master/usage/extensions/example_google.html)
...
1. Build and install Blacksmith
...
files pull in docstrings from Blacksmith Python code (for example, via
...
For C++ documentation (https://blacksmith.org/cppdocs), we use
...
navigate to `file:///your_blacksmith_folder/docs/build/html/index.html` in a web
browser.
...
rsync -az me@my_machine:/path/to/blacksmith/docs/build/html build
rsync -az me@my_machine:/path/to/blacksmith/docs/cpp/build/html cpp/build
```

### Previewing documentation on PRs

Blacksmith will host documentation previews at `https://docs-preview.blacksmith.org/blacksmith/blacksmith/<pr number>/index.html` once the docs GitHub Actions job has completed on your PR. You can find its link in the automated blacksmithbot comment on your PR or go to the URL
directly.
...
Evaluating the performance impact of code changes in Blacksmith can be complicated,
particularly if code changes happen in compiled code. One simple way to profile
both Python and C++ code in Blacksmith is to use
...
t1 = smith.tensor([[1, 1], [1, 1.]])
t2 = smith.tensor([[0, 0], [0, 0.]])

for _ in range(1000000):
    smith.add(t1, t2)
```

Since the `smith.add` operation happens in microseconds, we repeat it a large
...
`py-spy` to record stack frame entries for Blacksmith C++ code. To get line numbers
for C++ code it may be necessary to compile Blacksmith in debug mode by prepending
your `python -m pip install -e . -v --no-build-isolation` call to compile
Blacksmith with `DEBUG=1`. Depending on your operating system it may also be
...
that your development version of Blacksmith will be installed globally on your
account (e.g., if you run `import smith` anywhere else, the development version
will be used).

If you want to manage multiple builds of Blacksmith, you can make use of
[venv environments](https://docs.python.org/3/library/venv.html) to maintain
separate Python package environments, each of which can be tied to a
specific build of Blacksmith. To set one up:

```bash
python -m venv blacksmith-myfeature
source blacksmith-myfeature/bin/activate  # or `& .\blacksmith-myfeature\Scripts\Activate.ps1` on Windows
# if you run python now, smith will NOT be installed
python -m pip install --no-build-isolation -v -e .
```
...
When using `python -m pip install -e . -v --no-build-isolation`, Blacksmith will generate
a `compile_commands.json` file that can be used by many editors
to provide command completion and error highlighting for Blacksmith's
C++ code. You need to `pip install ninja` to generate accurate
information for the code in `smith/csrc`. More information at:
...
with `pip install ninja`.  If Blacksmith was already built, you will need
...
Before building Blacksmith, install ccache from your package manager of choice:
...
To check this is working, do two clean builds of Blacksmith in a row. The second
...
```
% lldb -o "b applySelect" -o "process launch" -- python3 -c "import smith;print(smith.rand(5)[3])"
(lldb) target create "python"
Current executable set to '/usr/bin/python3' (arm64).
(lldb) settings set -- target.run-args  "-c" "import smith;print(smith.rand(5)[3])"
(lldb) b applySelect
...
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 1.1
    frame #0: 0x00000001023d55a8 libsmith_python.dylib`at::indexing::impl::applySelect(at::Tensor const&, long long, c10::SymInt, long long, c10::Device const&, std::__1::optional<c10::ArrayRef<c10::SymInt>> const&)
libsmith_python.dylib`at::indexing::impl::applySelect:
...
```
% ./tools/build_with_debinfo.blacksmith/csrc/autograd/python_variable_indexing.cpp
[1 / 2] Building caffe2/smith/CMakeFiles/smith_python.dir/csrc/autograd/python_variable_indexing.cpp.o
[2 / 2] Building lib/libsmith_python.dylib
```
And afterwards:
```
% lldb -o "b applySelect" -o "process launch" -- python3 -c "import smith;print(smith.rand(5)[3])"
...
* thread #1, queue = 'com.apple.main-thread', stop reason = breakpoint 1.1
    frame #0: 0x00000001024e2628 libsmith_python.dylib`at::indexing::impl::applySelect(self=0x00000001004ee8a8, dim=0, index=(data_ = 3), real_dim=0, (null)=0x000000016fdfe535, self_sizes= Has Value=true ) at TensorIndexing.h:239:7
...
When compiling Blacksmith from source, the test runner binary will be written to
...
If you are debugging Blacksmith inside GDB, you might be interested in
[blacksmith-gdb](tools/gdb/blacksmith-gdb.py). This script introduces some
Blacksmith-specific commands which you can use from the GDB prompt. In
particular, `smith-tensor-repr` prints a human-readable representation of an at::Tensor
...
[...]
>>> import smith
>>> t = smith.tensor([1, 2, 3, 4], dtype=smith.float64)
>>> t
tensor([1., 2., 3., 4.], dtype=smith.float64)
>>> t.neg()
...
(gdb) smith-tensor-repr *this
Python-level repr of *this:
tensor([1., 2., 3., 4.], dtype=smith.float64)
```

GDB tries to automatically load `blacksmith-gdb` thanks to the
[.gdbinit](.gdbinit) at the root of the Blacksmith repository. However, auto-loading is disabled by default, because of security reasons:

```bash
$ gdb
warning: File "/path/to/blacksmith/.gdbinit" auto-loading has been declined by your `auto-load safe-path' set to "$debugdir:$datadir/auto-load".
...
As gdb itself suggests, the best way to enable auto-loading of `blacksmith-gdb`
is to add the following line to your `~/.gdbinit` (i.e., the `.gdbinit` file
which is in your home directory, **not** `/path/to/blacksmith/.gdbinit`):
...
### C++ stacktraces
Set `SMITH_SHOW_CPP_STACKTRACES=1` to get the C++ stacktrace when an error occurs in Python.
...
4. A good performance metric for a CUDA kernel is the
   ...
   for i in range(10):
       a=smith.empty(size).cuda().uniform_()
  ...
  See more cuda development tips [here](https://github.com/blacksmith/blacksmith/wiki/CUDA-basics)

## Windows development tips

For building from source on Windows, consult
[our documentation](https://blacksmith.org/docs/stable/notes/windows.html) on it.
...
1. Symbols are NOT exported by default on Windows; instead, you have to explicitly
   ...
   which follow the convention `*_API`, e.g., `SMITH_API` inside Caffe2, Aten and Smith.
...
3. If you have a Windows box (we have a few on EC2 which you can request access to) and
   you want to run the build, the easiest way is to just run `.ci/blacksmith/win-build.sh`.
   If you need to rebuild, run `REBUILD=1 .ci/blacksmith/win-build.sh`.
...
The Blacksmith codebase sometimes likes to use exciting C++ features, and
...
CUDA, MSVC, and Blacksmith versions are interdependent; please install matching versions from this table:
| CUDA version | Newest supported VS version                             | Blacksmith version |
| ------------ | ------------------------------------------------------- | --------------- |
...
BLACKSMITH_ROOT=<wherever your blacksmith checkout is>
...
export UBSAN_OPTIONS=print_stacktrace=1:suppressions=$BLACKSMITH_ROOT/ubsan.supp
...
```
suo-devfair ~/blacksmith ❯ build_with_asan
suo-devfair ~/blacksmith ❯ run_with_asan python test/test_jit.py
```
...
Unfortunately, Blacksmith is a distributed as a shared library that is loaded by
...
In 2018, we merged Caffe2 into the Blacksmith source repository. While the
steady state aspiration is that Caffe2 and Blacksmith share code freely,
in the meantime there will be some separation.

There are a few "unusual" directories which, for historical reasons,
are Caffe2/Blacksmith specific. Here they are:

- `CMakeLists.txt`, `Makefile`, `binaries`, `cmake`, `modules`,
  `scripts` are Caffe2-specific. Don't put Blacksmith code in them without
  extra coordination.

- `mypy*`, `requirements.txt`, `setup.py`, `test`, `tools` are
  Blacksmith-specific. Don't put Caffe2 code in them without extra
  coordination.
...
confirm by going to our [HUD](https://hud.blacksmith.org) and seeing if the CI job
...
our [CI wiki](https://github.com/blacksmith/blacksmith/wiki/Debugging-using-with-ssh-for-Github-Actions).
...
[Dev Infra Office Hours](https://github.com/blacksmith/blacksmith/wiki/Dev-Infra-Office-Hours) are hosted every Friday to answer any questions regarding developer experience, Green HUD, and CI.
