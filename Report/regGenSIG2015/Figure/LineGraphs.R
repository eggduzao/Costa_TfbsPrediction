...
ol = "/home/egg/Projects/TfbsPrediction/Report/regGenSIG2015/Figure/"
plot_colors <- c("#FF7777", "#006600")
plot_lty <- c(1,1)
mar.default <- c(5,5,2,0.3)
cellList = c("DU_K562")
...
createLine1 <- function(vec1,vec2,vec3,vec4,title,outFileName){
  ...
  xVec = (-20:20)
  plot(xVec, newvec1, type="l", col=plot_colors[1], xlab="Coordinates from Motif Center", ylab="      DNase I Signal Intensity", lwd=2,
  ...
  #legend(-0.5,1.20, c("Uncorrected","Corrected"), cex=1.4, col=plot_colors, lwd=5, lty=plot_lty, bty="n");

  text(x=-15.0,y=1.08,title,font=2,cex=2.0,bty="n")
...
for(c in 1:length(cellList)){
  ...
  for(cName in cNames){
    ...
    name = cName
    out = paste(ol,name,'_withLabel.eps',sep="")
    createLine1(vecO1,vecO2,vecC1,vecC2,name,out)

    out = paste(ol,name,'_woLabel.eps',sep="")
