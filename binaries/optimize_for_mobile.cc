...
C10_DEFINE_string(model, "", "The smith script model to optimize.");
C10_DEFINE_string(
    output,
    "",
...
int main(int argc, char** argv) {
  c10::SetUsageMessage(
    "\nRun optimization pass for blacksmith model. Example usage:\n"
    "./optimize_for_mobile"
    " --model=<model_file>"
  ...
  auto module = smith::jit::load(FLAGS_model);
  auto ops = smith::jit::export_opnames(module);
  ...
  smith::jit::Module optimized_module;
  if (FLAGS_backend == "" || FLAGS_backend == "cpu") {
    optimized_module = smith::jit::optimizeForMobile(module);
  } else if (FLAGS_backend == "vulkan") {
    optimized_module = smith::jit::vulkanOptimizeForMobile(
        module, std::set<MobileOptimizerType>(), preserved_methods);
  } else if (FLAGS_backend == "metal"){
    optimized_module = smith::jit::metalOptimizeForMobile(module, preserved_methods);
  }else{
    CAFFE_ENFORCE(false, "Unknown backend: " + FLAGS_backend);
  }
  auto new_ops = smith::jit::export_opnames(optimized_module);
  std::cout << "\npt_operator_library(" << std::endl;
