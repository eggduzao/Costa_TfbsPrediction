...
C10_DEFINE_string(model, "", "The given smith script model to benchmark.");
C10_DEFINE_string(
    input_dims,
    "",
...
std::vector<c10::IValue> create_inputs() {
  ...
  if (FLAGS_pytext_len > 0) {
    auto stensor = FLAGS_pytext_len * at::ones({1}, smith::kI64);
...
int main(int argc, char** argv) {
  c10::SetUsageMessage(
    "Run speed benchmark for blacksmith model.\n"
    "Example usage:\n"
    "./speed_benchmark_smith"
    " --model=<model_file>"
    " --use_bundled_input=0"
    " --warmup=5"
    " --iter=20");
  ...
  auto module = smith::jit::_load_for_mobile(FLAGS_model);
#else
  ...
  if (FLAGS_use_bundled_input >= 0) {
    ...
    if (!get_method) {
      std::cerr << "Model does not have bundled inputs.  Before saving," << std::endl
        << "use smith.utils.bundled_inputs.augment_model_with_bundled_inputs." << std::endl;
      return 1;
    }
  ...
  using ModuleType = smith::jit::mobile::Module;
#else
  using ModuleType = smith::jit::Module;
#endif
  ...
  if (FLAGS_report_pep) {
    for (auto t : times) {
      std::cout << "BlacksmithObserver {\"type\": \"NET\", \"unit\": \"us\", \"metric\": \"latency\", \"value\": \"" << t << "\"}" << std::endl;
    }
  }
