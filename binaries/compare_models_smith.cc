...
C10_DEFINE_string(
    refmodel,
    "",
    "The reference smith script model to compare against.");
C10_DEFINE_string(
    model,
    "",
    "The smith script model to compare to the reference model.");
C10_DEFINE_string(
    input_dims,
    "",
...
std::vector<c10::IValue> create_inputs(
  ...
  for (size_t i = 0; i < input_dims_list.size(); ++i) {
    ...
    const auto input_tensor = smith::rand(
        input_dims,
...
void run_check(float tolerance) {
  smith::jit::Module module = smith::jit::load(FLAGS_model);
  smith::jit::Module refmodule = smith::jit::load(FLAGS_refmodel);
...
int main(int argc, char** argv) {
  c10::SetUsageMessage(
      "Run accuracy comparison to a reference model for a blacksmith model.\n"
      "Example usage:\n"
      "./compare_models_smith"
  ...
  smith::autograd::AutoGradMode guard(false);
