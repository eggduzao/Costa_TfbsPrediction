...
import smithgen.api.dispatcher as dispatcher
import smithgen.api.meta as meta
import smithgen.api.native as native
import smithgen.api.structured as structured
import smithgen.dest as dest
from smithgen.api import cpp
from smithgen.api.translate import translate
from smithgen.api.types import (
    Binding,
    CppSignature,
    CppSignatureGroup,
    DispatcherSignature,
...
from smithgen.context import (
    method_with_native_function,
    native_function_manager,
    with_native_function,
    with_native_function_and_indices,
)
from smithgen.gen_aoti_c_shim import (
    gen_aoti_c_shim_files,
    gen_static_dispatch_backend_call_signature,
)
from smithgen.gen_functionalization_type import (
...
from smithgen.gen_vmap_plumbing import gen_all_vmap_plumbing
...
from smithgen.native_function_generation import (
...
from smithgen.yaml_utils import YamlDumper, YamlLoader
...
def generate_static_dispatch_fallback_call(
    ...
    if f.has_composite_explicit_autograd_kernel:
        ...
        return f"""SMITH_CHECK(false, "Static dispatch does not support {name} for\
...
class ComputeOperators:
    ...
    def __call__(self, f: NativeFunction) -> str:
        ...
        if self.target is Target.DECLARATION:
            ...
            #     This is helpful for blacksmith extenders who would like to decltype() an aten operator,
...
def compute_meta_function_declaration(g: NativeFunctionsGroup) -> str | None:
    ...
    with native_function_manager(g.out):
        ...
        if precomputed:
            ...
            # This typedef (actually a using statement) is needed so that SMITH_META_FUNC can reuse the return
            # type (which has a variable number of template parameters).
            meta_return_typedef = f"using meta_return_ty = precompute_out <{meta_return_template_params}>;"
            meta_return = "meta_return_ty"
            precomputed_decl = f"""
                {precompute_template_decl}
                struct SMITH_API precompute_out {{
                    {setter_methods_decl}
                    {precomputed_elements_decl};
            }};"""
        else:
        ...
        return f"""\
struct SMITH_API structured_{name} : public {parent_class} {{
    {precomputed_decl}
    {meta_return_typedef}
...
"""
            ...
            return f"""m.impl("aten::{f.func.name}", SMITH_FN({name}));"""
        else:
            assert_never(self.target)
...
def compute_returns_yaml(
    ...
    # schema itself.
    #
    # See also https://github.com/blacksmith/blacksmith/issues/43114
    ...
    for kernel_namespace in ns_definitions:
        ...
        for namespace in registrations[kernel_namespace]:
            if not registrations[kernel_namespace][namespace]:
                continue
            registration_body += f"""
SMITH_LIBRARY_IMPL({namespace}, {dispatch_key}, m) {{
    ...
    *,
    ...
    for namespace, funcs in ns_native_functions.items():
        ...
        if namespace == "aten":
            ...
            smith_library_macro = (
                "SMITH_LIBRARY_FRAGMENT"
                if namespace in FRAGMENT_NAMESPACES
                else "SMITH_LIBRARY"
            )
            schema_registrations += f"""
{smith_library_macro}({custom_namespace}, m) {{
...
def get_smithgen_root() -> Path:
    """
    If you're depending on smithgen out-of-tree, you can use the root to figure
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="smith/csrc/inductor/aoti_smith/generated",
    )
    parser.add_argument(
        "--rocm",
        action="store_true",
    ...
    parser.add_argument(
        "--update-aoti-c-shim",
        action="store_true",
        help="Update AOTInductor C shim after adding an entry to inductor_fallback_ops in smithgen/aoti/fallback_ops.py. "
    ...
    parser.add_argument(
        ...
        "   default is smith/csrc/inductor/aoti_smith/generated/extend"
        "WARNING: Do not use this unless you are sure what you are doing!!!",
    )
    ...
    from smithgen.model import dispatch_keys
    ...
    #
    #     C:/Jenkins/workspace/blacksmith-builds/blacksmith-win-ws2016-cuda9-cudnn7-py3-build/build/aten/src/ATen\core/TensorMethods.h
    #
