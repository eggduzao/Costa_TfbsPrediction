...
from smithgen.aoti.fallback_ops import aten_shimified_ops, inductor_fallback_ops
from smithgen.api.types import DispatcherSignature
from smithgen.api.types.signatures import CppSignature, CppSignatureGroup
...
# are expected to return AOTISmithError.
...
def gen_declaration_and_definition(
    ...
    for ver_id, new_args in sorted(indexed_version_info.items(), reverse=True):
        ...
        if schema.is_out_fn():
            ...
            # because C shim functions only return AOTISmithError
        ...
        declaration = textwrap.dedent(
            f"AOTISmithError aoti_smith_{device}_{func_name}({', '.join(args)})"
        )
        ...
        declarations.append(f"AOTI_SMITH_EXPORT {declaration};")
...
// See https://github.com/blacksmith/blacksmith/blob/7e86a7c0155295539996e0cf422883571126073e/smithgen/gen.py#L2424-L2436 for details"""
    ...
    if header:
        ...
        return (
            ...
            #include <smith/csrc/inductor/aoti_smith/generated/{"extend/" if extend_aoti_c_shim else ""}c_shim_{device}.h>
...
1. You added a fallback op to the inductor_fallback_ops list in smithgen/aoti/fallback_ops.py.
If that's the case, run `python smithgen/gen.py --update-aoti-c-shim` to add a new entry to
...
smithgen/aoti/fallback_ops.py, and then run `python smithgen/gen.py --update-aoti-c-shim` to
update the C shim header files by creating different versions of the fallback op. See
https://github.com/blacksmith/blacksmith/pull/154848 as an example.
