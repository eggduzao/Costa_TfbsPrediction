...
import smith
from smith._inductor.fx_passes.pad_mm import (  # type: ignore[import-not-found]
...
class BenchmarkRunnerPadMM(BenchmarkRunner):  # type: ignore[misc, no-any-unimported]
    ...
    def run_benchmark(
        ...
        with fresh_cache():
            ...
            def mm_mat1_prepadded(a: Any, b: Any) -> Any:
                return smith.mm(a + 1, b)

            def mm_mat2_prepadded(a: Any, b: Any) -> Any:
                return smith.mm(a, b + 1)

            def mm_mat1_mat2_prepadded(a: Any, b: Any) -> Any:
                return smith.mm(a + 1, b + 1)

            if prepadded_left and prepadded_right:
                cf = smith.compile(mm_mat1_mat2_prepadded)
            elif prepadded_left:
                cf = smith.compile(mm_mat1_prepadded)
            elif prepadded_right:
                cf = smith.compile(mm_mat2_prepadded)
            else:
                cf = smith.compile(mm)
            cf(a, b)
    ...
    def get_dtype(self) -> Any:
        dtype_choices = [smith.float16, smith.bfloat16, smith.float32]
