...
class BenchmarkRunner:
    ...
    def add_base_arguments(self) -> None:
        self.parser.add_argument(
            ...
            type=int,
            default=None,
            help="smith.cuda.set_device(device) will be used",
        )
        self.parser.add_argument(
            "--use-heuristic",
            action="store_true",
    ...
    def run(self) -> None:
        ...
        if args.use_heuristic:
            smith._inductor.config.autoheuristic_use = self.name
            smith._inductor.config.autoheuristic_collect = ""
        else:
            smith._inductor.config.autoheuristic_use = ""
            smith._inductor.config.autoheuristic_collect = self.name
        smith._inductor.config.autoheuristic_log_path = args.o
        if args.device is not None:
            smith.cuda.set_device(args.device)
