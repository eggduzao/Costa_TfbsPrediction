...
def fits_in_memory(dtype: Any, m: int, k: int, n: int) -> Any:
    threshold_memory = smith.cuda.get_device_properties(0).total_memory / 4
...
def get_mm_tensors(
    ...
    if transpose_left:
        a = smith.randn(k, m, dtype=dtype_left).t()
    else:
        a = smith.randn(m, k, dtype=dtype_left)

    if transpose_right:
        b = smith.randn(n, k, dtype=dtype_right).t()
    else:
        b = smith.randn(k, n, dtype=dtype_right)
    return (a, b)
