...
class BenchmarkRunnerMixedMM(BenchmarkRunner):  # type: ignore[misc, no-any-unimported]
    ...
    def run_benchmark(
        ...
        with fresh_cache():
            ...
            cf = smith.compile(mixed_mm, mode="max-autotune-no-cudagraphs")
            cf(a, b)
    ...
    def get_dtypes(self) -> Any:
        while True:
            dtype_floats = [smith.float16, smith.bfloat16]
            dtype_ints = [smith.int8, smith.uint8]
            mat1_dtype = random.choices(dtype_floats)[0]
            mat2_dtype = random.choices(dtype_ints)[0]
            if mat1_dtype == smith.bfloat16 and mat2_dtype == smith.uint8:
