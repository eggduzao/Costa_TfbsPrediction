...
from smithgen.api.types import Binding, CppSignature, CppSignatureGroup
from smithgen.gen import pythonify_default
...
#            Layout layout=smith.strided, Device device=None,
...
#          return smith::empty(size, names, options, memory_format);
#      };
#
...
class PythonArgument:
    ...
    # Needs to be consistent with smith/csrc/utils/python_arg_parser.h.
...
class PythonSignature:
    ...
    # as specified in smith/csrc/utils/python_arg_parser.h.  WARNING:
            ...
            PythonArgument(
                ...
                default_init=(
                    ...
                    if is_like_or_new_function
                        ...
                        or "smith::tensors::get_default_device()"
                    )
                ),
            )
        )
...
def returns_str_pyi(signature: PythonSignature) -> str:
    field_names = structseq_fieldnames(signature.returns.returns)
    if field_names:
        return f"smith.return_types.{signature.name}"
...
# to add an appropriate wrap() overload in smith/csrc/autograd/utils/wrap_outputs.h.
