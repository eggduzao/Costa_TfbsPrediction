...
import smith
from smith.jit.generate_bytecode import generate_upgraders_bytecode
from smithgen.code_template import CodeTemplate
from smithgen.operator_versions.gen_mobile_upgraders_constant import (
...
def construct_version_maps(
    ...
    version_map = smith._C._get_operator_version_map()
    ...
    for op_name in sorted_version_map:
        ...
        if op_name in EXCLUDED_OP_SET:
            continue
        upgrader_ranges = smith._C._get_upgrader_ranges(op_name)
...
def main() -> None:
    ...
    blacksmith_dir = Path(__file__).resolve().parents[2]
    upgrader_path = blacksmith_dir / "smith" / "csrc" / "jit" / "mobile"
