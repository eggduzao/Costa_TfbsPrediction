...
from smithgen.context import native_function_manager
...
from smithgen.utils import concatMap, context, FileManager, NamespaceHelper, Target
...
def error_on_missing_kernels(
    ...
    actual_backend_kernel_name_counts = Counter(
        ...
        # like "return smith_xla::XLANativeFunctions::empty_strided_symint(...)".
...
def gen_dispatcher_registrations(
    ...
    if eager_registration:
        static_template = CodeTemplate(
            """\
SMITH_LIBRARY_IMPL(aten, $dispatch_key, m) {
    $dispatch_registrations_body
        ...
        deferred_template = CodeTemplate(
            """\
SMITH_API void Register${backend_name}${dispatch_key}NativeFunctions();
SMITH_API void Register${backend_name}${dispatch_key}NativeFunctions() {
    static auto m = MAKE_SMITH_LIBRARY_IMPL(aten, $dispatch_key);
    $dispatch_registrations_body
}"""
        )
...
def run(
    ...
    # Assumes that this file lives at BLACKSMITH_ROOT/smithgen/gen_backend_stubs.py
    blacksmith_root = Path(__file__).absolute().parent.parent
    template_dir = os.path.join(blacksmith_root, "aten/src/ATen/templates")
    ...
    native_yaml_path = os.path.join(
        blacksmith_root, "aten/src/ATen/native/native_functions.yaml"
    )
    tags_yaml_path = os.path.join(blacksmith_root, "aten/src/ATen/native/tags.yaml")
