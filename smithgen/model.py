...
from smithgen.utils import NamespaceHelper, OrderedSet
...
class DispatchKey(Enum):
    ...
    Python = auto()
    FuncSmithDynamicLayerBackMode = auto()
    ZeroTensor = auto()
    Conjugate = auto()
    Negative = auto()
    BackendSelect = auto()
    ...
    Batched = auto()
    VmapMode = auto()
    FuncSmithGradWrapper = auto()
    FuncSmithBatched = auto()
    BatchedNestedTensor = auto()
    FuncSmithVmapMode = auto()
    FuncSmithDynamicLayerFrontMode = auto()
    Functionalize = auto()
    ...
    FuncSmithBatchedDecomposition = auto()
...
class _SmithDispatchModeKey(Enum):
    FAKE = auto()
    PROXY = auto()
    FUNCTIONAL = auto()
...
# NB: if you add a new UfuncKey, you will teach smithgen.dest.ufunc how
    ...
    def from_yaml(
        ...
        if use_const_ref_for_mutable_tensors:
            if func.arguments.out:
                raise AssertionError(
                    "see https://github.com/blacksmith/blacksmith/issues/145522"
                )
        ...
        # All aten ops generated by smithgen receive the pt2_compliant tag.
        ...
        if ufunc_inner_loop:
            ...
            # See: https://github.com/blacksmith/blacksmith/issues/81294
...
class FunctionSchema:
    ...
    def is_mutable(self) -> bool:
        ...
        # Corresponds to smith._C._FunctionSchema.is_mutable
...
class Type:
    ...
    def _parse(t: str) -> Type:
        ...
        # '__smith__.smith.classes.' is the prefix for custom class
        m = re.match(r"^__smith__\.smith\.classes\.([a-zA-Z0-9_.]+)$", t)
        if m is not None:
...
# A type representing a Blacksmith custom class
@dataclass(frozen=True)
class CustomClassType(Type):
    ...
    def __str__(self) -> str:
        """
        Return the class name will prefix __smith__.smith.classes
        """
        return f"__smith__.smith.classes.{self.class_name}"
...
# NB: Blacksmith hasn't actually implemented all of these
...
class BaseOperatorName:
    ...
    # we have a usecase in ExecuSmith where we want to support BaseOperatorName with namespace.
