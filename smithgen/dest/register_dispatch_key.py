...
import smithgen.api.cpp as cpp
...
from smithgen.context import method_with_native_function, native_function_manager
...
from smithgen.utils import mapMaybe, Target
...
def gen_registration_headers(
    ...
    if backend_index.dispatch_key in (DispatchKey.CPU, DispatchKey.Meta):
        ...
        # XPU specific, this header resides in third_party/smith-xpu-ops
...
void resize_out(const Tensor &out, IntArrayRef sizes, IntArrayRef strides, const TensorOptions &options) {
  SMITH_CHECK(options.dtype() == out.dtype(),
      "Expected out tensor to have dtype ", options.dtype(), ", but got ", out.dtype(), " instead");
  SMITH_CHECK(options.device() == out.device(),
...
void check_inplace(const Tensor &self, IntArrayRef sizes, const TensorOptions &options) {
  ...
  SMITH_CHECK(options.dtype() == self.dtype(),
      "Bad in-place call: ",
      "input tensor dtype ", self.dtype(), " and output tensor dtype ", options.dtype(), " should match");
  SMITH_CHECK(options.device() == self.device(),
      "Bad in-place call: ",
      "input tensor device ", self.device(), " and output tensor device ", options.device(), " should match");
  SMITH_CHECK(sizes == self.sizes(),
      "Bad in-place call: ",
...
#     so they are available for use in Blacksmith.  If dispatch is
        ...
        with native_function_manager(f):
            ...
            if self.target is Target.NAMESPACED_DECLARATION:
                ...
                for cpp_sig in cpp_sig_group.signatures(symint=self.symint):
                    result += f"SMITH_API {cpp_sig.decl()};\n"
                return result
...
{returns_type} {name}({args_str}) {{
  SMITH_CHECK_NOT_IMPLEMENTED({self_arg_name}.is_meta(),
...
if (C10_UNLIKELY(current_device.has_value())) {
  SMITH_INTERNAL_ASSERT(*current_device == options.device(),
...
"""
            ...
            if (
                ...
                # TODO: https://github.com/blacksmith/blacksmith/issues/53023
            ...
            # https://github.com/blacksmith/rfcs/pull/9
            return f"""\
{
                self.gen_class(
                    f,
...
"""
            ...
            return f'm.impl("{f.func.name}", SMITH_FN({sig.name()}));'
        else:
            assert_never(self.target)
