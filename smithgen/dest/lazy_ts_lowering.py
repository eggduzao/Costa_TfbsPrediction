from smithgen.api.lazy import LazyArgument, LazyIrSchema
from smithgen.api.types import OptionalCType
...
def ts_lowering_body(schema: LazyIrSchema) -> str:
    ...
    {emplace_arguments_str}
    {emplace_kwarguments}
    smith::lazy::TSOpVector {schema.aten_name}_out = smith::lazy::LowerTSBuiltin(function, op().op, arguments, kwarguments);
    SMITH_CHECK_EQ({schema.aten_name}_out.size(), {len(schema.returns)});
