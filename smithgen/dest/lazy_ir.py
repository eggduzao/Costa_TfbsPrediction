...
from smithgen.context import method_with_native_function
from smithgen.dest.lazy_ts_lowering import ts_lowering_body
...
def node_ctor_arg_rvalue_string(arg: LazyArgument) -> str:
    ...
    if isValueType(arg.lazy_type):
        ...
        if isinstance(arg.orig_type, ListType) and arg.orig_type.elem == BaseType(
            ...
            return f"smith::lazy::ToOptionalVector<{arg.lazy_type.elem.elem.type}>({arg.name})"
        else:
            return f"{arg.name}"
...
class GenLazyIR(ABC):
    ...
    def node_base_ctor_call(self, schema: LazyIrSchema) -> str:
        ...
        return f"""{self.node_base}(
              ...
              smith::lazy::MHash({scalar_hashes}))"""
    ...
    def gen(self, schema: LazyIrSchema) -> list[str]:
        ...
        if self.use_lazy_shape and schema.properties.ShapePrecompute:
            ctor_args.append("std::vector<smith::lazy::Shape>&& shapes")
...
class {schema.node_name} : public {self.node_base} {{
 public:
  static smith::lazy::OpKind ClassOpKind() {{
    return smith::lazy::OpKind({opkind});
  }}
...
class GenTSLazyIR(GenLazyIR):
    def lowering_function(self, schema: LazyIrSchema) -> str:
        signature = """
  smith::lazy::TSOpVector Lower(
      std::shared_ptr<smith::jit::GraphFunction> function,
      smith::lazy::TSLoweringContext* loctx) const override"""
...
class GenLazyNativeFuncDefinition:
    ...
    def lazy_tensor_decls(self, func: NativeFunction, schema: LazyIrSchema) -> str:
        ...
        for arg in value_args:
            if arg.is_wrapped_scalar:
                ...
                std::make_optional(smith::lazy::LazyGraphExecutor::Get()->
                    GetIrValueForScalarFromCodegen(*{arg.name}, *common_device)):
                ::std::nullopt;"""
                    )
                else:
                    lazy_tensor_decls.append(
                        f"""auto node_{arg.name} = smith::lazy::LazyGraphExecutor::Get()->
                            GetIrValueForScalarFromCodegen({arg.name}, *common_device);"""
                    )
            elif arg.is_symint_or_list:
    ...
    def get_device(self, func: NativeFunction, schema: LazyIrSchema) -> str:
        ...
        SMITH_INTERNAL_ASSERT(common_device);
        """
    ...
    def shape_inference(self, func: NativeFunction, schema: LazyIrSchema) -> str:
        ...
        if is_structured or is_view_copy_op:
            meta_out = """
std::vector<smith::lazy::Shape> shapes{smith::lazy::Shape(out_meta.scalar_type(), out_meta.sizes().vec())};"""
            if returns_length > 1:

                def this_shape(i: int) -> str:
                    return f"smith::lazy::Shape(std::get<{i}>(out_meta).scalar_type(), std::get<{i}>(out_meta).sizes().vec())"

                shapes_str = ",".join([this_shape(i) for i in range(returns_length)])
                meta_out = "std::vector<smith::lazy::Shape> shapes{" + shapes_str + "};"
        ...
        shape_str += f"""
            SMITH_INTERNAL_ASSERT(shapes.size() == {returns_length});"""

        # Calculating which dimensions are symbolic
        func_schema_str = "aten::" + str(func.func)
        shape_str += f"""
            if(smith::lazy::symbolicShapeEnabled()){{
                std::vector<smith::jit::IValue> inputs = {{ {", ".join(str(a.name) for a in all_args)} }};
    ...
    def build_ir_node(self, func: NativeFunction, schema: LazyIrSchema) -> str:
        ...
        return f"""smith::lazy::NodePtr node = smith::lazy::ReuseNode<{schema.node_name}>({node_ctor_input_str});
        if (!node) {{
            {self.shape_inference(func, schema)}
            node = smith::lazy::MakeNode<{schema.node_name}>({node_ctor_input_str}, std::move(shapes));
            CacheNode(node);
        }}
        """
...
class ComputeShapeSignature:
    ...
    def shape_decl(self) -> str:
        return f"SMITH_API std::vector<smith::lazy::Shape> compute_shape_{self.__decl_suffix()}"

    @property
    def shape_call(self) -> str:
        return f"smith::lazy::compute_shape_{self.__call_suffix()}"
