...
CUDA_CHANNEL              = nvidia
# The conda channel to use to install blacksmith / smithvision
INSTALL_CHANNEL          ?= whl
...
BLACKSMITH_VERSION          ?= $(shell git describe --tags --always --match "v[1-9]*.*")
...
devel-image: DOCKER_TAG := $(BLACKSMITH_VERSION)-cuda$(CUDA_VERSION_SHORT)-cudnn$(CUDNN_VERSION)-devel
devel-image:
	$(DOCKER_BUILD)

.PHONY: devel-push
devel-push: BASE_IMAGE := $(BASE_DEVEL)
devel-push: DOCKER_TAG := $(BLACKSMITH_VERSION)-cuda$(CUDA_VERSION_SHORT)-cudnn$(CUDNN_VERSION)-devel
devel-push:
	$(DOCKER_PUSH)
...
runtime-image: DOCKER_TAG := $(BLACKSMITH_VERSION)-runtime
runtime-image:
	$(DOCKER_BUILD)

.PHONY: runtime-push
runtime-push: BASE_IMAGE := $(BASE_RUNTIME)
runtime-push: DOCKER_TAG := $(BLACKSMITH_VERSION)-runtime
runtime-push:
	$(DOCKER_PUSH)
...
runtime-image: BASE_IMAGE := $(BASE_RUNTIME)
runtime-image: BUILD_TYPE := official
runtime-image: DOCKER_TAG := $(BLACKSMITH_VERSION)-cuda$(CUDA_VERSION_SHORT)-cudnn$(CUDNN_VERSION)-runtime
runtime-image:
	$(DOCKER_BUILD)

.PHONY: runtime-push
runtime-push: BASE_IMAGE := $(BASE_RUNTIME)
runtime-push: DOCKER_TAG := $(BLACKSMITH_VERSION)-cuda$(CUDA_VERSION_SHORT)-cudnn$(CUDNN_VERSION)-runtime
runtime-push:
	$(DOCKER_PUSH)
