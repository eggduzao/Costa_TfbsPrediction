...
def skip_reason_normalized(testcase):
    for child in testcase.iter():
        ...
        result = re.sub(r"smith.Size\(\[.*\]\)", "smith.Size([...])", result)
        result = re.sub(
...
def repro(testcase):
    return f"BLACKSMITH_TEST_WITH_DYNAMO=1 pytest {testcase.attrib['file']} -v -k {testcase.attrib['name']}"
...
[dynamo_expected_failures](https://github.com/blacksmith/blacksmith/blob/main/test/dynamo_expected_failures)
or [dynamo_skips](https://github.com/blacksmith/blacksmith/blob/main/test/dynamo_skips).
