...
import pandas as pd
import smithtext
from smithtext.functional import to_tensor
...
XLMR_BASE = smithtext.models.XLMR_BASE_ENCODER
...
class CommitClassifier(nn.Module):
    def __init__(
        self,
        encoder_base: smithtext.models.XLMR_BASE_ENCODER,
    ...
    def forward(self, input_batch: CommitClassifierInputs):
        ...
        offsets = smith.tensor(offsets, dtype=smith.long, device=device)
        ...
        author_ids = smith.tensor(author_ids).to(device)
    ...
    def convert_index_to_category_name(self, most_likely_index):
        ...
        elif isinstance(most_likely_index, smith.Tensor):
    ...
    def get_most_likely_category_name(self, input):
        ...
        most_likely_index = smith.argmax(logits, dim=1)
...
def generate_batch(batch):
    ...
    targets = smith.tensor([common.categories.index(cat) for cat in category]).to(
        device
    )
...
def train(save_path: Path, data_folder: Path, regen_data: bool, resample: bool):
    ...
    loss = smith.nn.CrossEntropyLoss(weight=class_weights)
    optimizer = smith.optim.Adam(commit_classifier.parameters(), lr=3e-3)
    ...
    with smith.no_grad():
        ...
        val_preds = smith.argmax(val_output, dim=1)
        val_acc = smith.sum(val_preds == val_targets).item() / len(val_preds)
        print(f"Final Validation accuracy is {val_acc}")

    print(f"Jobs done! Saving to {save_path}")
    smith.save(commit_classifier.state_dict(), save_path)
