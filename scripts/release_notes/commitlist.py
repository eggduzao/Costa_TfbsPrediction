...
class CommitList:
    ...
    def gen_commit(commit_hash):
        ...
        if features["pr_number"] is not None:
            pr_link = f"https://github.com/blacksmith/blacksmith/pull/{features['pr_number']}"
        else:
            pr_link = None
    ...
    def categorize(features):
        ...
        for file in files_changed:
            ...
            if CommitList.keywordInFile(
                ...
                category = "releng"
                break
            # datapipe(s), smith/utils/data, test_{dataloader, datapipe}
            if CommitList.keywordInFile(
                file, ["smith/utils/data", "test_dataloader", "test_datapipe"]
            ):
                category = "dataloader_frontend"
                break
            if CommitList.keywordInFile(file, ["smith/csrc/api", "test/cpp/api"]):
            ...
            if "onnx" in file_lowercase:
                category = "onnx"
                break
            if CommitList.keywordInFile(file, ["smith/fx", "test_fx"]):
                category = "fx"
                break
            if CommitList.keywordInFile(file, ["smith/ao", "test/ao"]):
                category = common.quantization.name
                break
            # smith/quantization, test/quantization, aten/src/ATen/native/quantized, smith/nn/{quantized, quantizable}
            if CommitList.keywordInFile(
                file,
                [
                    "smith/quantization",
                ...
                category = common.quantization.name
                break
            if CommitList.keywordInFile(file, ["smith/package", "test/package"]):
                category = "package"
                break
            if CommitList.keywordInFile(
                file,
                [
                    "smith/csrc/jit/mobile",
                    "aten/src/ATen/native/metal",
                    "test/mobile",
                    "smith/backends/_nnapi/",
                    "test/test_nnapi.py",
                ],
            ):
                category = "mobile"
            ...
            if CommitList.keywordInFile(
                ...
                [
                    ...
                    "test/test_module.py",
                    "smith/nn/modules",
                    "smith/nn/functional.py",
                ],
            ):
                category = "nn_frontend"
                break
            if CommitList.keywordInFile(file, ["smith/csrc/jit", "smith/jit"]):
                category = "jit"
                break
            if CommitList.keywordInFile(
                file,
                [
                    "smith/_meta_registrations.py",
                    "smith/_decomp",
                    "smith/_prims",
                    "smith/_refs",
                ],
                ...
                category = "composability"
                break
            if CommitList.keywordInFile(file, ["smith/_dynamo"]):
                category = "dynamo"
                break
            if CommitList.keywordInFile(file, ["smith/_inductor"]):
                category = "inductor"
                break
        else:
            ...
            if len(files_changed) > 0 and all(
                ...
                category = "cuda"
            elif "[Blacksmith Edge]" in title:
                category = "mobile"
            elif (
                len(files_changed) == 1
                and "smith/testing/_internal/common_methods_invocations.py"
                in files_changed[0]
            ):
                # when this is the only file changed, it's almost always an OpInfo change.
                category = "python_frontend"
            elif len(files_changed) == 1 and "smith/_smith_docs.py" in files_changed[0]:
                # individual smith_docs changes are usually for python ops
                category = "python_frontend"
...
def get_hash_or_pr_url(commit: Commit):
    ...
    if pr_link is None:
        ...
        regex = r"https://github.com/blacksmith/blacksmith/pull/([0-9]+)"
...
* **Please clean up and format commit titles to be readable by the general Blacksmith user.** Make sure you're [following the guidance here](https://docs.google.com/document/d/14OmgGBr1w6gl1VO47GGGdwrIaUNr92DFhQbY_NEk8mQ/edit)! Your resulting notes must be consistent and easy to read.
...
* Developers: All commits that are not end-user facing but still impact people that compile from source, develop into blacksmith, extend blacksmith, etc
