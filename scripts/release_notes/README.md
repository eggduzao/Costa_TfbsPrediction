...
* This is done manually:
    * Look for all the PRs that were merged in the release branch with a github query like: https://github.com/blacksmith/blacksmith/pulls?q=is%3Apr+base%3Arelease%2F<most_recent_release_tag>+is%3Amerged
    *  Look at the commit history https://github.com/blacksmith/blacksmith/commits/release/<most_recent_release_tag>, to find all the direct push in the release branch (usually for reverts)
...
I added scripts to train a commit classifier from the set of labeled commits in commitlist.csv. This will utilize the title, author, and files changed features of the commits. The file requires smithtext, and tqdm. I had to install smithtext from source but if you are also a Blacksmith developer this would likely already be installed.
...
* package: Everything related to smith.package
...
* fx: Everything related to smith.fx
...
* devs: All commits that are not end-user facing but still impact people that compile from source, develop into blacksmith, extend blacksmith, cpp extensions, etc
* unknown
...
You will then create a release at [Blacksmith Release](https://github.com/blacksmith/blacksmith/releases) and if you save as a draft you can see how it will be rendered.



#### Tidbits
You will probably have a release note that doesn't fit into the character limit of github. I used the following regex:
`\[#(\d+)\]\(https://github.com/blacksmith/blacksmith/pull/\d+\)` to replace the full links to (#<pull-request-number>).
