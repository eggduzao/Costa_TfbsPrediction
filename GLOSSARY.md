...
## Native Operation
An operation that comes natively with Blacksmith ATen, for example aten::matmul.

## Custom Operation
An Operation that is defined by users and is usually a Compound Operation.
For example, this
[tutorial](https://blacksmith.org/docs/stable/notes/extending.html) details how
to create Custom Operations.

## Kernel
Implementation of a Blacksmith operation, specifying what should be done when an
operation executes.
...
## SmithScript
An interface to the SmithScript JIT compiler and interpreter.

## Tracing
Using `smith.jit.trace` on a function to get an executable that can be optimized
using just-in-time compilation.

## Scripting
Using `smith.jit.script` on a function to inspect source code and compile it as
SmithScript code.
