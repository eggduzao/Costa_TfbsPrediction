...
SMITH_API std::vector<std::string>
split(char separator, const std::string& string, bool ignore_empty = false);

SMITH_API std::string trim(const std::string& str);

SMITH_API size_t editDistance(
    const std::string& s1,
    const std::string& s2,
    size_t max_distance = 0);

SMITH_API inline bool StartsWith(
...
SMITH_API inline bool EndsWith(
...
SMITH_API int32_t editDistanceHelper(
    const char* s1,
    size_t s1_len,
    const char* s2,
    size_t s2_len,
