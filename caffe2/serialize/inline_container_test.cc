...
TEST(BlacksmithStreamWriterAndReader, SaveAndLoad) {
  int64_t kFieldAlignment = 64L;

  std::ostringstream oss;
  // write records through writers
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
  ...
  BlacksmithStreamReader reader(&iss);
...
TEST(BlacksmithStreamWriterAndReader, LoadWithMultiThreads) {
  std::ostringstream oss;
  // write records through writers
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
  ...
  // read records through blacksmithStreamReader
  std::istringstream iss(the_file);
  BlacksmithStreamReader reader(&iss);
...
TEST(BlacksmithStreamWriterAndReader, GetNonexistentRecordThrows) {
  std::ostringstream oss;
  // write records through writers
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
...
TEST(BlacksmithStreamWriterAndReader, SkipDebugRecords) {
  std::ostringstream oss;
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
    return oss ? n : 0;
...
TEST(BlacksmithStreamWriterAndReader, ValidSerializationId) {
  std::ostringstream oss;
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
    return oss ? n : 0;
  ...
  // write a second time
  BlacksmithStreamWriter writer2([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
    return oss ? n : 0;
  });
...
TEST(BlacksmithStreamWriterAndReader, SkipDuplicateSerializationIdRecords) {
  std::ostringstream oss;
  BlacksmithStreamWriter writer([&](const void* b, size_t n) -> size_t {
    oss.write(static_cast<const char*>(b), n);
    return oss ? n : 0;
...
TEST(BlacksmithStreamWriterAndReader, LogAPIUsageMetadata) {
...
TEST(BlacksmithStreamWriterAndReader, SaveAndLoadWithAllocator) {
...
TEST(BlacksmithStreamWriterAndReader, LoadWithMultiThreadsWithAllocator) {
  auto defaultAllocator = at::GetCPUAllocator();
  TestAllocator overrideAllocator(defaultAllocator);
  TestAllocator baseAllocator(defaultAllocator);
  c10::SetCPUAllocator(&baseAllocator, 10 /* priority */);
...
TEST_P(ChunkRecordIteratorTest, ChunkRead) {
  ...
  BlacksmithStreamReader reader(fileName);
