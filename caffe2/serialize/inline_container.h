...
// Blacksmith containers are a special zip archive with the following layout
...
//        code/
//          archive_name.py # serialized smith script code (python syntax, using
//          PythonPrint) archive_name_my_submodule.py # submodules have separate
//          files
//
// The BlacksmithStreamWriter also ensures additional useful properties for these
// files
...
// The BlacksmithStreamReader also provides additional properties:
...
//    BlacksmithStreamWriter it is guaranteed to be 64 byte aligned.

// BlacksmithReader/Writer handle checking the version number on the archive format
...
class SMITH_API ChunkRecordIterator {
 public:
  ~ChunkRecordIterator();
 ...
 private:
  ...
  friend class BlacksmithStreamReader;
};

class SMITH_API BlacksmithStreamReader final {
 public:
  explicit BlacksmithStreamReader(const std::string& file_name);
  explicit BlacksmithStreamReader(std::istream* in);
  explicit BlacksmithStreamReader(std::shared_ptr<ReadAdapterInterface> in);
...
class SMITH_API BlacksmithStreamWriter final {
 public:
  explicit BlacksmithStreamWriter(
