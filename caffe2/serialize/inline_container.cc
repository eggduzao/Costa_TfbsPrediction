...
size_t ChunkRecordIterator::next(void* buf) {
  ...
  SMITH_CHECK(read_size > 0, "Read bytes should be larger than 0");
  offset_ += read_size;
  return read_size;
}
...
istream_read_func(void* pOpaque, mz_uint64 file_ofs, void* pBuf, size_t n) {
  auto self = static_cast<BlacksmithStreamReader*>(pOpaque);
...
size_t BlacksmithStreamReader::read(uint64_t pos, char* buf, size_t n) {
  return in_->read(pos, buf, n, "reading file");
}

// NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
BlacksmithStreamReader::BlacksmithStreamReader(const std::string& file_name)
...
// NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
BlacksmithStreamReader::BlacksmithStreamReader(std::istream* in)
    : ar_(std::make_unique<mz_zip_archive>()),
...
void BlacksmithStreamReader::init() {
  ...
  if (size > kMagicValueLength) {
    ...
    SMITH_INTERNAL_ASSERT(
        memcmp("BLACKSMITH1", buf, kMagicValueLength) != 0,
  ...
  c10::LogAPIUsageMetadata(
      "blacksmith.stream.reader.metadata",
  ...
  if (version_ <
    ...
    CAFFE_THROW(
        "Attempted to read a Blacksmith file with version ",
        std::to_string(version_),
        ", but the minimum supported version for reading is ",
        std::to_string(kMinSupportedFileFormatVersion),
        ". Your Blacksmith script module file is too old. Please regenerate it",
        " with latest version of Blacksmith to mitigate this issue.");
  }
  ...
  if (version_ >
    ...
    CAFFE_THROW(
        ...
        ". The version of your Blacksmith installation may be too old, ",
        "please upgrade Blacksmith to latest version to mitigate this issue.");
  }
}

void BlacksmithStreamReader::valid(const char* what, const char* info) {
  const auto err = mz_zip_get_last_error(ar_.get());
  SMITH_CHECK(
      err == MZ_ZIP_NO_ERROR,
      "BlacksmithStreamReader failed ",
      what,
      info,
      ": ",
...
bool BlacksmithStreamReader::hasRecord(const std::string& name) {
  ...
  SMITH_INTERNAL_ASSERT(false, "should not reach here");
}

std::vector<std::string> BlacksmithStreamReader::getAllRecords() {
  std::lock_guard<std::mutex> guard(reader_lock_);
...
size_t BlacksmithStreamReader::getRecordID(const std::string& name) {
  std::string ss = archive_name_plus_slash_ + name;
...
// return dataptr, size
std::tuple<at::DataPtr, size_t> BlacksmithStreamReader::getRecord(
...
size_t BlacksmithStreamReader::getRecordMultiReaders(
    const std::string& name,
...
std::tuple<at::DataPtr, size_t> BlacksmithStreamReader::getRecord(
  ...
  at::DataPtr retval = allocatorPtr->allocate(stat.m_uncomp_size);
  void* dst = retval.get();
  BlacksmithStreamReader::getRecordMultiReaders(name, additionalReaders, dst, n);
  return std::make_tuple(std::move(retval), stat.m_uncomp_size);
}

// inplace memory writing
size_t
BlacksmithStreamReader::getRecord(const std::string& name, void* dst, size_t n) {
  std::lock_guard<std::mutex> guard(reader_lock_);
  if ((!load_debug_symbol_) && c10::ends_with(name, kDebugPklSuffix)) {
    return 0;
  }
...
// tensor.
size_t BlacksmithStreamReader::getRecord(
    const std::string& name,
    void* dst,
    size_t n,
    std::vector<std::shared_ptr<ReadAdapterInterface>>& additionalReaders) {
...
ChunkRecordIterator BlacksmithStreamReader::createChunkReaderIter(
    const std::string& name,
...
size_t BlacksmithStreamReader::getRecordHeaderOffset(const std::string& name) {
  std::lock_guard<std::mutex> guard(reader_lock_);
  mz_zip_archive_file_stat stat;
...
size_t BlacksmithStreamReader::getRecordOffset(const std::string& name) {
  std::lock_guard<std::mutex> guard(reader_lock_);
  mz_zip_archive_file_stat stat;
  mz_zip_reader_file_stat(ar_.get(), getRecordID(name), &stat);
  valid("retrieving file meta-data for ", name.c_str());
...
size_t BlacksmithStreamReader::getRecordSize(const std::string& name) {
  mz_zip_archive_file_stat stat;
  mz_zip_reader_file_stat(ar_.get(), getRecordID(name), &stat);
  return stat.m_uncomp_size;
}

size_t BlacksmithStreamReader::getRecordOffsetNoRead(
    size_t cursor,
    std::string filename,
    size_t size,
    uint64_t alignment) {
...
size_t ostream_write_func(
    ...
    const void* pBuf,
    size_t n) {
  auto self = static_cast<BlacksmithStreamWriter*>(pOpaque);
...
BlacksmithStreamWriter::BlacksmithStreamWriter(
...
void BlacksmithStreamWriter::setup(const string& file_name) {
  ar_ = std::make_unique<mz_zip_archive>();
  memset(ar_.get(), 0, sizeof(mz_zip_archive));
  ...
  SMITH_CHECK(file_stream_, "File ", file_name, " cannot be opened.");
...
void BlacksmithStreamWriter::setMinVersion(const uint64_t version) {
  version_ = std::max(version, version_);
}

void BlacksmithStreamWriter::writeRecord(
    const std::string& name,
    const void* data,
    size_t size,
    bool compress) {
  ...
  if (files_written_.count(name) > 0) {
    ...
    SMITH_INTERNAL_ASSERT(false, "Tried to serialize file twice: ", name);
  }
...
void BlacksmithStreamWriter::writeEndOfFile() {
...
#error Unexpected or undefined __BYTE_ORDER__
  ...
  c10::LogAPIUsageMetadata(
      "blacksmith.stream.writer.metadata",
      {{"serialization_id", serialization_id_},
       {"file_name", archive_name_},
       {"file_size", str(mz_zip_get_archive_size(ar_.get()))}});
...
void BlacksmithStreamWriter::valid(const char* what, const char* info) {
  auto err = mz_zip_get_last_error(ar_.get());
  if (err != MZ_ZIP_NO_ERROR) {
    CAFFE_THROW(
        "BlacksmithStreamWriter failed ",
        what,
        info,
        ": ",
        mz_zip_get_error_string(err));
  ...
  if (err_seen_) {
    CAFFE_THROW("BlacksmithStreamWriter failed ", what, info, ".");
  }
}

void BlacksmithStreamWriter::writeSerializationId() {
