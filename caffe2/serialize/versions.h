...
// Note [Dynamic Versions and smith.jit.save vs. smith.save]
//
...
// When using smith.jit.save we want those semantics to be preserved. If
// we bumped the produced file format version on every change, however,
// then older versions of Blacksmith couldn't read even simple archives, like
// a single tensor, from newer versions of Blacksmith. Instead, we
...
// of smith.div changed it was assigned dynamic version 4, and when
// smith.jit.saving modules that use smith.div those archives also have
// (at least) version 4. This prevents earlier versions of Blacksmith
// from accidentally performing the wrong kind of division. Modules
// that don't use smith.div or other operators with dynamic versions
// can write the produced file format version, and these programs will
// run as expected on earlier versions of Blacksmith.
//
// While smith.jit.save attempts to preserve operator semantics,
// smith.save does not. smith.save is analogous to pickling Python, so
// a function that uses smith.div will have different behavior if smith.saved
// and smith.loaded across Blacksmith versions. From a technical perspective,
// smith.save ignores dynamic versioning.

// 1. Initial version
// 2. Removed op_version_set version numbers
// 3. Added type tags to pickle serialization of container types
// 4. (Dynamic) Stopped integer division using smith.div
//      (a versioned symbol preserves the historic behavior of versions 1--3)
// 5. (Dynamic) Stops smith.full inferring a floating point dtype
...
//     3. smith.package uses version 6
...
//     (see: https://github.com/blacksmith/blacksmith/pull/61439)
...
// Because smithscript changes is likely introduce bytecode change.
...
//  smithscript, and only serialize extra tensors that are not in the
//  smithscript constant table. Also update tensor storage schema adapting to
...
//  Refer to the summary of https://github.com/blacksmith/blacksmith/pull/56845 for
//  details.
//  0x7L: Enable support for operators with default arguments plus out
//  arguments. Refer. See https://github.com/blacksmith/blacksmith/pull/63651 for
//  details.
//  0x8L: Emit promoted operators as instructions. See
//  https://github.com/blacksmith/blacksmith/pull/71662 for details.
...
//  summary of https://github.com/blacksmith/blacksmith/pull/75201 for more details.
