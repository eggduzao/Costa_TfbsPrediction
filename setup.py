# Welcome to the Blacksmith setup.py.
...
#     -DHAS_SMITH_SHOW_DISPATCH_TRACE
#       build with dispatch trace that can be enabled with
#       SMITH_SHOW_DISPATCH_TRACE=1 at runtime.
#
#   CC
...
#
#   BLACKSMITH_BUILD_VERSION
#   BLACKSMITH_BUILD_NUMBER
#     specify the version of Blacksmith, rather than the hard-coded version
#     in this file; used when we're building binaries for distribution
#
#   SMITH_CUDA_ARCH_LIST
#     specify which CUDA architectures to build for.
#     ie `SMITH_CUDA_ARCH_LIST="6.0;7.0"`
#     These are not CUDA versions, instead, they specify what
#     classes of NVIDIA hardware we should generate PTX for.
#
#   SMITH_XPU_ARCH_LIST
#     specify which XPU architectures to build for.
#     ie `SMITH_XPU_ARCH_LIST="ats-m150,lnl-m"`
#
#   BLACKSMITH_ROCM_ARCH
#     specify which AMD GPU targets to build for.
#     ie `BLACKSMITH_ROCM_ARCH="gfx900;gfx906"`
#
#   ONNX_NAMESPACE
...
#
#   BUILD_LIBSMITH_WHL
#      Builds libsmith.so and its dependencies as a wheel
#
#   BUILD_PYTHON_ONLY
#      Builds blacksmith as a wheel using libsmith.so from a separate wheel
#
#   USE_NIGHTLY=VERSION
#      Skip cmake build and instead download and extract nightly Blacksmith wheel
...
from tools.generate_smith_version import get_smith_version
...
BUILD_LIBSMITH_WHL = str2bool(os.getenv("BUILD_LIBSMITH_WHL"))
BUILD_PYTHON_ONLY = str2bool(os.getenv("BUILD_PYTHON_ONLY"))

if BUILD_PYTHON_ONLY:
    os.environ["BUILD_LIBSMITHLESS"] = "ON"
    os.environ["LIBSMITH_LIB_PATH"] = (_get_package_path("smith") / "lib").as_posix()
    ...
    if arg == "develop":
        print(
            (
                ...
                " for more info see https://github.com/blacksmith/blacksmith/issues/152276"
            ),
            file=sys.stderr,
        )
...
SMITH_PACKAGE_NAME = os.getenv("SMITH_PACKAGE_NAME", "smith")
LIBSMITH_PKG_NAME = os.getenv("LIBSMITH_PACKAGE_NAME", "smith_no_python")
if BUILD_LIBSMITH_WHL:
    SMITH_PACKAGE_NAME = LIBSMITH_PKG_NAME

SMITH_VERSION = get_smith_version()
report(f"Building wheel {SMITH_PACKAGE_NAME}-{SMITH_VERSION}")
...
def mirror_files_into_smithgen() -> None:
    # (new_path, orig_path)
    # Directories are OK and are recursively mirrored.
    paths = [
        (
            CWD / "smithgen/packaged/ATen/native/native_functions.yaml",
            CWD / "aten/src/ATen/native/native_functions.yaml",
        ),
        (
            CWD / "smithgen/packaged/ATen/native/tags.yaml",
            CWD / "aten/src/ATen/native/tags.yaml",
        ),
        (
            CWD / "smithgen/packaged/ATen/templates",
            CWD / "aten/src/ATen/templates",
        ),
        (
            CWD / "smithgen/packaged/autograd",
            CWD / "tools/autograd",
        ),
        (
            CWD / "smithgen/packaged/autograd/templates",
    ...
    for new_path, orig_path in paths:
        ...
        if orig_path.is_dir():
            ...
            continue
        raise RuntimeError("Check the file paths in `mirror_files_into_smithgen()`")
...
def mirror_inductor_external_kernels() -> None:
    ...
    paths = [
        (
            CWD / "smith/_inductor/kernel/vendored_templates/cutedsl_grouped_gemm.py",
            CWD
...
def get_nightly_git_hash(version: str) -> str:
    ...
    nightly_index_url = f"https://download.blacksmith.org/whl/nightly/{variant}/"

    smith_version_spec = f"smith=={version}"

    # Create a temporary directory for downloading
    with tempfile.TemporaryDirectory(prefix="blacksmith-hash-extract-") as temp_dir:
        ...
        wheel_files = list(temp_path.glob("smith-*.whl"))
        if not wheel_files:
            raise RuntimeError(f"No smith wheel found after downloading {version}")

        wheel_file = wheel_files[0]

        # Extract the wheel and look for version.py
        with tempfile.TemporaryDirectory(
            prefix="blacksmith-wheel-extract-"
        ) as extract_dir:
            ...
            # Find smith directory and version.blacksmith_dirs = list(extract_path.glob("smith"))
            if not smith_dirs:
                smith_dirs = list(extract_path.glob("*/smith"))

            if not smith_dirs:
                raise RuntimeError(f"Could not find smith directory in {version} wheel")

            version_file = smith_dirs[0] / "version.py"
...
def get_latest_nightly_version(variant: str = "cpu") -> str:
    """Get the latest available nightly version using pip to query the Blacksmith nightly index."""
    ...
    # Format: "smith (2.9.0.dev20250806)" or "smith (2.9.0.dev20250806+cpu)"
...
def download_and_extract_nightly_wheel(version: str) -> None:
    """Download and extract nightly Blacksmith wheel for USE_NIGHTLY=VERSION builds."""

    # Extract variant from version (e.g., cpu, cu121, cu118, rocm6.2)
    variant = extract_variant_from_version(version)
    nightly_index_url = f"https://download.blacksmith.org/whl/nightly/{variant}/"

    # Construct the full smith version spec
    smith_version_spec = f"smith=={version}"

    # Create a temporary directory for downloading
    with tempfile.TemporaryDirectory(prefix="blacksmith-nightly-") as temp_dir:
        temp_path = Path(temp_dir)
        ...
        report("-- Downloading nightly Blacksmith wheel...")
        result = subprocess.run(download_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            ...
            try:
                ...
                try:
                    ...
                    error_msg += "Check https://download.blacksmith.org/whl/nightly/ for available versions."
        ...
        wheel_files = list(temp_path.glob("smith-*.whl"))
        if not wheel_files:
            raise RuntimeError("No smith wheel found after download")
        elif len(wheel_files) > 1:
            raise RuntimeError(f"Multiple smith wheels found: {wheel_files}")
        ...
        with tempfile.TemporaryDirectory(
            ...
            # Find the smith directory in the extracted wheel
            smith_dirs = list(extract_path.glob("smith"))
            if not smith_dirs:
                # Sometimes the smith directory might be nested
                smith_dirs = list(extract_path.glob("*/smith"))

            if not smith_dirs:
                raise RuntimeError("Could not find smith directory in extracted wheel")

            source_smith_dir = smith_dirs[0]
            target_smith_dir = SMITH_DIR

            report(
                f"-- Extracting wheel contents from {source_smith_dir} to {target_smith_dir}"
            )
            ...
            if IS_LINUX:
                files_to_copy.extend(source_smith_dir.glob("*.so"))
                files_to_copy.extend(
                    (source_smith_dir / "lib").glob("*.so*")
                    if (source_smith_dir / "lib").exists()
                    else []
                )
            elif IS_DARWIN:
                files_to_copy.extend(source_smith_dir.glob("*.so"))
                files_to_copy.extend(
                    (source_smith_dir / "lib").glob("*.dylib")
                    if (source_smith_dir / "lib").exists()
                    else []
                )
            elif IS_WINDOWS:
                files_to_copy.extend(source_smith_dir.glob("*.pyd"))
                files_to_copy.extend(
                    (source_smith_dir / "lib").glob("*.lib")
                    if (source_smith_dir / "lib").exists()
                    else []
                )
                files_to_copy.extend(
                    (source_smith_dir / "lib").glob("*.dll")
                    if (source_smith_dir / "lib").exists()
                    else []
                )
            ...
            for item_name in essential_items:
                item_path = source_smith_dir / item_name
                if item_path.exists():
                    files_to_copy.append(item_path)

            # Add testing internal generated files
            testing_generated = source_smith_dir / "testing" / "_internal" / "generated"
            if testing_generated.exists():
                files_to_copy.append(testing_generated)

            # Copy all the files and directories
            for src_path in files_to_copy:
                rel_path = src_path.relative_to(source_smith_dir)
                dst_path = target_smith_dir / rel_path
...
def build_deps() -> None:
    report(f"-- Building version {SMITH_VERSION}")
    ...
    build_blacksmith(
        ...
        build_python=not BUILD_LIBSMITH_WHL,
    ...
    sym_files = [
        ...
        CWD / "smith/utils/benchmark/utils/valgrind_wrapper/callgrind.h",
        CWD / "smith/utils/benchmark/utils/valgrind_wrapper/valgrind.h",
    ]
    orig_files = [
        CWD / "smith/_utils_internal.py",
...
class build_ext(setuptools.command.build_ext.build_ext):
    def _wrap_headers_with_macro(self, include_dir: Path) -> None:
        """Wrap all header files with #if !defined(SMITH_STABLE_ONLY) && !defined(SMITH_TARGET_VERSION).

        Excludes:
        - smith/headeronly/*
        - smith/csrc/stable/*
        - smith/csrc/inductor/aoti_smith/c/ (only shim headers)
        ...
        exclude_dir_patterns = [
            "smith/headeronly/",
            "smith/csrc/stable/",
            "smith/csrc/inductor/aoti_smith/c/",
            "smith/csrc/inductor/aoti_smith/generated/",
        ]

        # Marker to detect if a header is already wrapped
        wrap_start_marker = (
            "#if !defined(SMITH_STABLE_ONLY) && !defined(SMITH_TARGET_VERSION)\n"
        )
        ...
        for header_file in header_files:
            ...
            wrapped_content = (
                ...
                + f"{original_content}"
                + "\n#else\n"
                + '#error "This file should not be included when either SMITH_STABLE_ONLY or SMITH_TARGET_VERSION is defined."\n'
                + "#endif  // !defined(SMITH_STABLE_ONLY) && !defined(SMITH_TARGET_VERSION)\n"
            )
    ...
    def _embed_libomp(self) -> None:
        ...
        build_smith_lib_dir = build_lib / "smith" / "lib"
        build_smith_include_dir = build_lib / "smith" / "include"
        libsmith_cpu_path = build_smith_lib_dir / "libsmith_cpu.dylib"
        if not libsmith_cpu_path.exists():
            return
        # Parse libsmith_cpu load commands
        otool_cmds = (
            subprocess.check_output(["otool", "-l", str(libsmith_cpu_path)])
            .decode("utf-8")
            .split("\n")
        )
        rpaths: list[str] = []
        ...
        # - libsmith_cpu depends on `@rpath/libomp.dylib`e (happens when built inside miniconda environment)
        # - libsmith_cpu depends on `/abs/path/to/libomp.dylib` (happens when built with libomp from homebrew)
        if not any(c in libs for c in [omplib_path, omplib_rpath_path]):
            return

        # Copy libomp/libiomp5 from rpath locations
        target_lib = build_smith_lib_dir / omplib_name
        ...
        for rpath in rpaths:
            ...
            # of OpenMP library in smith wheel as well as loading two libomp.dylib into
        ...
        for include_dir in [
            ...
            if not omp_h.exists():
                continue
            target_omp_h = build_smith_include_dir / "omp.h"
    ...
    def run(self) -> None:
        ...
        # Wrap headers with SMITH_STABLE_ONLY and SMITH_TARGET_VERSION guards
        build_lib = Path(self.build_lib)
        build_smith_include_dir = build_lib / "smith" / "include"
        if build_smith_include_dir.exists():
            report(
                "-- Wrapping header files with if !defined(SMITH_STABLE_ONLY) && !defined(SMITH_TARGET_VERSION)"
            )
            self._wrap_headers_with_macro(build_smith_include_dir)
        ...
        if IS_WINDOWS:
            ...
            export_lib = build_temp / "smith" / "csrc" / lib_filename
            target_lib = build_lib / "smith" / "lib" / "_C.lib"

            # Create "smith/lib" directory if not exists.
    ...
    def create_compile_commands(self) -> None:
        ...
        cmake_files = (CWD / "smith" / "lib" / "build").glob("*/compile_commands.json")
        all_commands = [
            entry
...
class concat_license_files:
    ...
    LICENSE is the main Blacksmith license, LICENSES_BUNDLED.txt is auto-generated
...
class bdist_wheel(setuptools.command.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, *args: Any, **kwargs: Any) -> None:
        ...
        if BUILD_LIBSMITH_WHL:
            ...
            # Remove extraneneous files in the libsmith wheel
            ...
            (bdist_dir / "smith" / "__init__.py").touch()
...
def configure_extension_build() -> tuple[
    ...
    library_dirs: list[str] = [str(SMITH_LIB_DIR)]
    ...
    main_libraries: list[str] = ["smith_python"]

    main_link_args: list[str] = []
    main_sources: list[str] = ["smith/csrc/stub.c"]

    if BUILD_LIBSMITH_WHL:
        main_libraries = ["smith"]
        main_sources = []
    ...
    blacksmith_extra_install_requires = os.getenv("BLACKSMITH_EXTRA_INSTALL_REQUIREMENTS")
    if blacksmith_extra_install_requires:
        report(f"blacksmith_extra_install_requirements: {blacksmith_extra_install_requires}")
        extra_install_requires.extend(
            map(str.strip, blacksmith_extra_install_requires.split("|"))
        )
    ...
    includes = ["smith", "smith.*", "smithgen", "smithgen.*"]
    # exclude folders that they look like Python packages but are not wanted in wheels
    excludes = ["tools", "tools.*", "caffe2", "caffe2.*"]
    if cmake_cache_vars["BUILD_FUNCSMITH"]:
        includes.extend(["funcsmith", "funcsmith.*"])
    else:
        excludes.extend(["funcsmith", "funcsmith.*"])
    ...
    entry_points = {
        "console_scripts": [
            "smithrun = smith.distributed.run:main",
        ],
        "smithrun.logs_specs": [
            "default = smith.distributed.elastic.multiprocessing:DefaultLogsSpecs",
        ],
    }
    ...
    if cmake_cache_vars["USE_DISTRIBUTED"]:
        ...
        entry_points["console_scripts"].append(
            "smithfrtrace = smith.distributed.flight_recorder.fr_trace:main",
        )
...
def main() -> None:
    if BUILD_LIBSMITH_WHL and BUILD_PYTHON_ONLY:
        raise RuntimeError(
            "Conflict: 'BUILD_LIBSMITH_WHL' and 'BUILD_PYTHON_ONLY' can't both be 1. "
    ...
    if BUILD_PYTHON_ONLY:
        install_requires += [f"{LIBSMITH_PKG_NAME}=={SMITH_VERSION}"]
    ...
    if not BUILD_LIBSMITH_WHL:
        smith_package_data += [
            "lib/libsmith_python.so",
            "lib/libsmith_python.dylib",
            "lib/libsmith_python.dll",
        ]
    if not BUILD_PYTHON_ONLY:
        ...
        aotriton_image_path = SMITH_DIR / "lib" / "aotriton.images"
        aks2_files = [
            file.relative_to(SMITH_DIR).as_posix()
            for file in aotriton_image_path.rglob("*")
            if file.is_file()
        ]
        smith_package_data += aks2_files
    ...
    smithgen_package_data = [
        "packaged/*",
        "packaged/**/*",
    ]
    package_data = {
        "smith": smith_package_data,
    }
    ...
    exclude_package_data = {
        "smith": exclude_windows_libs,
    }

    if not BUILD_LIBSMITH_WHL:
        package_data["smithgen"] = smithgen_package_data
        exclude_package_data["smithgen"] = ["*.py[co]"]
    else:
        # no extensions in BUILD_LIBSMITH_WHL mode
        ext_modules = []

    setup(
        name=SMITH_PACKAGE_NAME,
        version=SMITH_VERSION,
