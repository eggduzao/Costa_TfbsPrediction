...
constexpr DispatchKeySet funcsmith_batched_dispatch_keyset =
    DispatchKeySet(DispatchKey::FuncSmithBatched);

DispatchKeySet getRuntimeDispatchKeySet(DispatchKey t) {
  SMITH_INTERNAL_ASSERT(t != DispatchKey::Undefined);
  switch (t) {
    ...
    case DispatchKey::FuncSmithBatchedDecomposition:
      return funcsmith_batched_dispatch_keyset;
    default:
...
bool runtimeDispatchKeySetHas(DispatchKey t, DispatchKey k) {
  ...
  switch (t) {
    ...
    case DispatchKey::FuncSmithBatchedDecomposition:
      return funcsmith_batched_ks.has(k);
    default:
      return t == k;
  }
}
...
DispatchKeySet::iterator& DispatchKeySet::iterator::operator++() {
  SMITH_INTERNAL_ASSERT(next_functionality_ <= iterator::end_iter_mask_val);
  SMITH_INTERNAL_ASSERT(next_backend_ <= num_backends, next_backend_);
  ...
  if (isPerBackendFunctionalityKey(
    ...
    SMITH_INTERNAL_ASSERT(next_backend_ == 0);
    current_dispatchkey_idx_ = next_dispatchkey_idx;
    next_functionality_ = new_next_functionality;
  }
  return *this;
