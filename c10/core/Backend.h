 ...
 * exposed to users in smith.backends.  Backend here is something like "CPU"
 * or "SparseCUDA"; backend in smith.backends is something like "MKL" or
 * "CUDNN".
 */
enum class Backend {
  CPU,
...
inline Backend dispatchKeyToBackend(DispatchKey t) {
  if (t == DispatchKey::CPU || t == DispatchKey::AutogradCPU) {
    ...
    SMITH_CHECK(false, "Unrecognized tensor type ID: ", t);
  }
}
...
inline DispatchKey backendToDispatchKey(Backend b) {
  switch (b) {
    ...
    default:
      SMITH_CHECK(false, "Unknown backend");
  }
}
...
inline DeviceType backendToDeviceType(Backend b) {
  switch (b) {
    ...
    case Backend::Undefined:
      SMITH_CHECK(false, "Undefined backend is not a valid device type");
    default:
      SMITH_CHECK(false, "Unknown backend");
  }
}
