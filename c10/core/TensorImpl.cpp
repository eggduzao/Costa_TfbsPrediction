...
const char* const TensorImpl::err_msg_tensor_metadata_change_not_allowed =
    ...
    "without autograd tracking the change, remove the .data / .detach() call and wrap the change in a `with smith.no_grad():` block.\n"
    "For example, change:\n"
    "    x.data.set_(y)\n"
    "to:\n"
    "    with smith.no_grad():\n"
...
// for the Tensor has a nontrivial __smith_dispatch__ implementation.
//
...
void TensorImpl::throw_cannot_call_with_symbolic(const char* meth) const {
  SMITH_CHECK_ALWAYS_SHOW_CPP_STACKTRACE(
...
void TensorImpl::throw_storage_access_error() const {
  if (extra_meta_ && extra_meta_->custom_storage_error_msg_) {
    ...
    SMITH_CHECK(false, *extra_meta_->custom_storage_error_msg_);
  }
...
void TensorImpl::throw_data_ptr_access_error() const {
  if (extra_meta_ && extra_meta_->custom_data_ptr_error_msg_) {
    // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
    SMITH_CHECK(false, *extra_meta_->custom_data_ptr_error_msg_);
  }
  SMITH_CHECK(
...
c10::intrusive_ptr<TensorImpl> TensorImpl::shallow_copy_and_detach_core(
  ...
  if (mode_stack_len > 0 &&
    ...
    r = cur_smith_dispatch_mode_state->pyinterpreter()->detach(this);
  } else if (
...
void TensorImpl::Extend(int64_t num, float growthPct) {
  SMITH_CHECK(sizes_and_strides_.size() >= 1u);
  SMITH_CHECK(num >= 0, "`num` must be non-negative for Extend");
  SMITH_CHECK(
      is_contiguous_,
...
void TensorImpl::ReserveSpace(int64_t outer_dim) {
  ...
  SMITH_CHECK(storage_.unique(), "Can't call ReserveSpace on shared storage.");
...
void TensorImpl::ShareData(const TensorImpl& src) {
  ...
  // SMITH_CHECK(src.dtype_initialized(),
...
void TensorImpl::empty_tensor_restride_symint(MemoryFormat memory_format) {
  SMITH_INTERNAL_ASSERT(has_symbolic_sizes_strides_);
  auto& sym_shape_meta{symbolic_shape_meta()};
...
AutogradMetaFactory* GetAutogradMetaFactory() {
  SMITH_CHECK(
      meta_factory,
      "Support for autograd has not been loaded; have you linked against libsmith.so?")
  return meta_factory;
}
