...
//
// NOTE: Keep the list in sync with `DispatchKey` in smithgen/model.py
enum class DispatchKey : uint16_t {
  ...
  // https://gitlab.com/blacksmith-complex/vitis_kernels
  ...
  // Out-of-core key for Fake Tensor in smithdistx.
  // See https://blacksmith.org/smithdistx/latest/fake_tensor.html
  ...
  // constituent parts.
  // TODO: delete this once smithdim lands in funcsmith
  Named,
  ...
  // https://github.com/blacksmith/nestedtensor
  ...
  FuncSmithBatched, // See Note [Out-of-tree vmap+grad prototype]

  // Dispatch key for BatchedTensorImpl wrapping a nested tensor.
  BatchedNestedTensor,

  FuncSmithVmapMode, // See Note [Out-of-tree vmap+grad prototype]
  ...
  FuncSmithGradWrapper, // See Note [Out-of-tree vmap+grad prototype]

  // Out-of-core key for Deferred Module Initialization in smithdistx.
  // See https://blacksmith.org/smithdistx/latest/deferred_init.html
  DeferredInit,

  // Used by Python key logic to know the set of tls on entry to the dispatcher
  // This kernel assumes it is the top-most non-funcsmith-related DispatchKey.
  ...
  FuncSmithDynamicLayerFrontMode, // See Note [Out-of-tree vmap+grad prototype]
...
#undef DEFINE_PER_BACKEND_KEYS_FOR_BACKEND
  ...
  // Note: The alias keyset for FuncSmithBatchedDecomposition is disjoint from
  // all
  // other alias keysets
  // and so precedence order doesn't matter
  FuncSmithBatchedDecomposition, // registered at
  // build/aten/src/ATen/RegisterFuncSmithBatchedDecomposition.cpp
...
// to Blacksmith to add your type ID).
//
// Private use tensor IDs are appropriate to use if you want to experiment
// with adding a new tensor type (without having to patch Blacksmith first) or
...
// the Blacksmith developers to get a type ID registered in this case.
//
...
// AutogradOther definitions already defined in Blacksmith.  AutogradPrivateUse
...
// smith::dispatch(smith::kCPU, ...) is also valid.
