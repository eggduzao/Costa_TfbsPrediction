...
inline int64_t size_to_dim_(int k, IntArrayRef dims) {
  SMITH_CHECK(k >= 0 && static_cast<size_t>(k) <= dims.size());
  int64_t r = 1;
...
inline int64_t size_between_dim_(int k, int l, IntArrayRef dims) {
  SMITH_CHECK((unsigned)l < dims.size() && (unsigned)k < dims.size());
  int64_t r = 1;
  if (k < l) {
    for (int i = k + 1; i < l; ++i) {
      r *= dims[i];
...
// compilation unit than TensorImpl (libsmith.so versus libc10.so)
...
// function which will be initialized when we load libsmith.so.
...
// https://github.com/blacksmith/blacksmith/issues/5396)
//
 ...
 public:
  ...
  void bump() {
    ...
    SMITH_CHECK(
        ...
        "See https://github.com/blacksmith/rfcs/pull/17 for more details.");
    if (version_counter_) {
  ...
  void set_version(int64_t i) {
    SMITH_CHECK(
        version_counter_,
        "Tried to call smith.autograd._unsafe_set_version() on a tensor "
        "that does not have a version counter. Was it created in inference mode?");
    SMITH_CHECK(i >= 0, "Cannot set a version_counter to a value below 0: ", i);
 ...
 * Blacksmith codebase itself, but may theoretically need to be
...
struct C10_API TensorImpl : public c10::intrusive_ptr_target {
 ...
 protected:
  ...
  inline bool matches_python_custom(SizesStridesPolicy policy) const {
    ...
    if (r) {
      SMITH_INTERNAL_ASSERT(is_python_dispatch())
    }
    return r;
  }
...
// it a non-error.
 ...
 protected:
  c10::Device device_default() const {
    SMITH_CHECK(device_opt_.has_value(), "tensor does not have a device");
 ...
 public:
  Layout layout() const {
    ...
    if (!key_set_.has_any(sparse_and_sparsecsr_and_mkldnn_ks)) {
      ...
      // __smith_dispatch__ users will be able to redefine the
      // layout() method.
      return layout_impl();
    } else {
      SMITH_INTERNAL_ASSERT(
  ...
  void _set_conj(bool value) {
    if (value) {
      ...
      SMITH_INTERNAL_ASSERT(isComplexType(typeMetaToScalarType(dtype())));
    } else {
  ...
  */
  void _set_zero(bool value) {
    if (value) {
      SMITH_INTERNAL_ASSERT(
          false,
          "Please call `smith._efficientzerotensor` if you want to create a tensor with no storage.");
    } else {
   ...
   *     smith::autograd::enter_dual_level for more details about forward AD
   * nesting.
 ...
 private:
  ...
  __ubsan_ignore_pointer_overflow__ T* data_ptr_impl_impl(
    ...
    SMITH_CHECK(
        ...
        "If you're using smith.compile/export/fx, it is likely that we are erroneously "
    ...
    // (for example if created as `smith.empty(5)[10:]`) that triggers
 ...
 private:
  ...
  c10::SymbolicShapeMeta& symbolic_shape_meta() {
    SMITH_INTERNAL_ASSERT(extra_meta_ && extra_meta_->symbolic_shape_meta_);
 ...
 public:
    ...
    if (new_dim > 0) {
      for (size_t dim = new_dim - 1;; dim--) {
        if (new_stride[dim] >= 0) {
          ...
          // support negative strides. Some blacksmith functions rely on it:
          // for example, smith.cat (run TestSmith.test_cat_empty).
          if (dim == new_dim - 1) {
      ...
      SMITH_CHECK(!overflowed, "Stride calculation overflowed");
    }
  ...
  void set_named_tensor_meta(
    ...
    if (named_tensor_meta) {
      SMITH_INTERNAL_ASSERT(named_tensor_meta->slow_dim() == dim());
    }
#endif
    if (named_tensor_meta) {
  ...
  void empty_tensor_restride(MemoryFormat memory_format) {
    ...
    switch (memory_format) {
      ...
      case MemoryFormat::Preserve:
        SMITH_CHECK(false, "unsupported memory format ", memory_format);
        // Cleaning warning messages, no need to break as SMITH_CHECK(false)
        // terminates flow.
        // break;
      case MemoryFormat::NumOptions:
        SMITH_INTERNAL_ASSERT(false, "invalid memory format ", memory_format);
    }
 ...
 private:
  ...
  int64_t compute_numel() const {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!has_symbolic_sizes_strides_);
  ...
  int64_t safe_compute_numel() const {
    ...
    SMITH_CHECK(!overflows, "numel: integer multiplication overflow");
 ...
 protected:
  ...
  // See https://github.com/blacksmith/blacksmith/issues/18496 for more info.
