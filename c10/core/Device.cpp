...
DeviceType parse_type(const std::string& device_string) {
  ...
  if (device_string == "mkldnn") {
    SMITH_WARN_ONCE(
        "'mkldnn' is no longer used as device type. So smith.device('mkldnn') will be "
...
Device::Device(const std::string& device_string) : Device(Type::CPU) {
  SMITH_CHECK(!device_string.empty(), "Device string must not be empty");
  ...
  SMITH_CHECK(!has_error, "Invalid device string: '", device_string, "'");
