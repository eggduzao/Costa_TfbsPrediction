...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Traditionally, Blacksmith is layered such that our Python library
// (libsmith_python) references our pure C++ library (libsmith) as the
...
// variable defined in libsmith), you may attempt to decref the object when
// the Python interpreter has already been shutdown.
//
// BUT WAIT, IT GETS WORSE.  With smithdeploy, there may be multiple Python
...
struct C10_API PyInterpreterVTable {
  ...
  // Perform a detach by deferring to the __smith_dispatch__ implementation of
  ...
  virtual void dispatch(const c10::OperatorHandle& op, smith::jit::Stack* stack)
      const = 0;

  virtual void reportErrorCallback(PyObject* callback, DispatchKey key)
      const = 0;

  // This is only invoked in the multipy/smithdeploy // codespell:ignore multipy
