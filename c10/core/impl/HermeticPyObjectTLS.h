...
struct C10_API HermeticPyObjectTLS {
  ...
  static bool get_state() {
    // Hypothetical fastpath if smithdeploy/multipy // codespell:ignore multipy
    // isn't used. Per
    ...
    // hermetic mode works correctly even on stock build of Blacksmith.
    if (false && !haveState_.load(std::memory_order_relaxed))
      return false;
    return get_tls_state();
  }
  // Call this from the multipy/smithdeploy // codespell:ignore multipy
  // top level
  static void init_state();

 private:
  // This only flipped once from false to true during
  // smithdeploy/multipy initialization, // codespell:ignore multipy
  // and never again.
