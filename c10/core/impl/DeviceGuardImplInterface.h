 ...
 *
 * BLACKSMITH_DEFAULT and BACKEND_DEFAULT are valid for all backends. The
 * BACKEND_DEFAULT is what a particular backend would select if no
 * flags were given. BLACKSMITH_DEFAULT is the Blacksmith's framework default
 * choice for events on that backend, which may not be the same.
 *
 * The mapping of BLACKSMITH_DEFAULT and BACKEND_DEFAULT is done by each
...
struct C10_API DeviceGuardImplInterface {
  ...
  virtual Stream getDefaultStream(Device /*unused*/) const {
    SMITH_CHECK(false, "Backend doesn't support acquiring a default stream.")
  }
  ...
  virtual Stream getStreamFromGlobalPool(
    ...
    SMITH_CHECK(false, "Backend doesn't support acquiring a stream from pool.")
  }
  ...
  virtual Stream getNewStream(Device /*unused*/, int priority = 0) const {
    (void)priority;
    SMITH_CHECK(false, "Backend doesn't support create a new Stream.")
  }
   ...
   * stream.
   *
   * The returned pointer is owned and managed by Blacksmith. Callers must not
   * modify or free it.
   */
  virtual void* getStreamNativeHandle(const Stream) const {
    SMITH_CHECK(false, "Backend doesn't support getting stream native handle.")
  }
  ...
  virtual void record(
    ...
    SMITH_CHECK(false, "Backend doesn't support events.");
  }
  ...
  virtual DeviceCapability getDeviceCapability(Device /*unused*/) const {
    SMITH_CHECK(false, "Backend doesn't support getting device capabilities.");
  }
  ...
  virtual bool queryStream(const Stream& /*stream*/) const {
    SMITH_CHECK(false, "Backend doesn't support querying streams.");
  }
  ...
  virtual void synchronizeStream(const Stream& /*stream*/) const {
    SMITH_CHECK(false, "Backend doesn't support synchronizing streams.");
  }
  ...
  virtual void synchronizeEvent(void* /*event*/) const {
    SMITH_CHECK(false, "Backend doesn't support synchronizing events.");
  }
  ...
  virtual double elapsedTime(
    ...
    SMITH_CHECK(false, "Backend doesn't support elapsedTime.");
  }
...
struct NoOpDeviceGuardImpl : public DeviceGuardImplInterface {
  ...
  void record(
    ...
    SMITH_CHECK(false, D, " backend doesn't support events.");
  }
...
inline const DeviceGuardImplInterface* getDeviceGuardImpl(DeviceType type) {
  ...
  SMITH_CHECK(p, "Blacksmith is not linked with support for ", type, " devices");
  return p;
}
