...
#define SMITH_SYM_CHECK(cond, ...) \
  SMITH_CHECK((cond).expect_true(__FILE__, __LINE__), __VA_ARGS__)
#define SMITH_SYM_INTERNAL_ASSERT(cond, ...) \
  SMITH_INTERNAL_ASSERT((cond).expect_true(__FILE__, __LINE__), __VA_ARGS__)
#define SMITH_MAYBE_SYM_CHECK(cond, ...)                                 \
  if constexpr (std::is_same_v<std::decay_t<decltype(cond)>, SymBool>) { \
    SMITH_CHECK((cond).expect_true(__FILE__, __LINE__), __VA_ARGS__)     \
  } else {                                                               \
    SMITH_CHECK((cond), __VA_ARGS__)                                     \
  }
...
#define SMITH_GUARD_SIZE_OBLIVIOUS(cond) \
  c10::guard_size_oblivious((cond), __FILE__, __LINE__)

#define SMITH_STATICALLY_KNOWN_TRUE(cond) \
  c10::statically_known_true((cond), __FILE__, __LINE__)

#define SMITH_GUARD_OR_FALSE(cond) \
  c10::guard_or_false((cond), __FILE__, __LINE__)

#define SMITH_GUARD_OR_TRUE(cond) c10::guard_or_true((cond), __FILE__, __LINE__)
