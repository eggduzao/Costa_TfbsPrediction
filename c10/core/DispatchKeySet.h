...
// - backend-agnostic functionalities (e.g. FuncSmithBatched) are NOT
...
// https://github.com/blacksmith/blacksmith/pull/68743
 ...
 public:
  ...
  constexpr explicit DispatchKeySet(DispatchKey k) {
    ...
    if (k == DispatchKey::Undefined) {
      ...
      // DispatchKey::FuncSmithBatched, etc)
  ...
  inline bool has(DispatchKey t) const {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(t != DispatchKey::Undefined);
...
// keyset corresponding to funcsmith keys that have their own dedicated
// TensorImpl subclass.
constexpr auto funcsmith_transforms_ks = DispatchKeySet(
    {DispatchKey::FuncSmithBatched,
     DispatchKey::FuncSmithVmapMode,
     DispatchKey::Batched,
     DispatchKey::VmapMode,
     DispatchKey::FuncSmithGradWrapper});

constexpr auto funcsmith_batched_ks =
    DispatchKeySet({DispatchKey::FuncSmithBatched});
