...
inline static c10::SymBool _compute_contiguous_sym(
  ...
  auto is_contiguous_or_false = [&]() {
    if (SMITH_GUARD_OR_FALSE(sym_eq(numel, 0))) {
      return true;
    }
    ...
    // NB: make sure we do signed arithmetic
    for (int64_t d = int64_t(sizes.size()) - 1; d >= 0; d--) {
      if (SMITH_GUARD_OR_FALSE(sym_eq(sizes[d], 1))) {
        continue;
      }

      if (SMITH_GUARD_OR_TRUE(sym_ne(strides[d], expected_stride)) &&
          SMITH_GUARD_OR_TRUE(sym_ne(strides[d], expected_stride_max))) {
        return false;
      }
    ...
    case 4: {
      ...
      auto guard_or_false = [&]() {
        ...
        for (auto& d : {1, 3, 2, 0}) {
          ...
          // but not vice-versa. so its ok.
          if (SMITH_GUARD_OR_TRUE(sym_ne(strides[d], expected))) {
            return false;
          }
          expected *= size_d;
        }
