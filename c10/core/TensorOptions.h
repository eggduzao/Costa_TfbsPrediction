...
///
///     smith.zeros(2, 3, dtype=smith.int32)
///
...
///
/// WARNING: In Blacksmith, there are `smith::` variants of factory functions,
/// e.g., smith::zeros for at::zeros.  These return Variables (while the
...
/// `smith.empty(sizes, X)` with `X` being a `smith.device`, `smith.dtype`, or a
/// `smith.layout`, we want TensorOptions to be implicitly convertible from
...
/// `smith.device` object (e.g., "cuda:1" can be passed to everywhere a
/// `smith.device("cuda:1")` is accepted). To support the syntax
...
#undef DO_CASE
    ...
    default:
      SMITH_CHECK(false, "Unsupported layout: ", layout_);
  }
}
