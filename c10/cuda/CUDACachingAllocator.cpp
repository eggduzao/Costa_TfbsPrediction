...
#if !defined(USE_ROCM) && defined(BLACKSMITH_C10_DRIVER_API_SUPPORTED)
...
SMITH_SDT_DEFINE_SEMAPHORE(malloc)
SMITH_SDT_DEFINE_SEMAPHORE(free)
...
struct Block {
  ...
  void splice(Block* before, Block* after) {
    if (before) {
      SMITH_INTERNAL_ASSERT(before->next == after);
      before->next = this;
    }
    prev = before;
    if (after) {
      SMITH_INTERNAL_ASSERT(after->prev == before);
      after->prev = this;
    }
    next = after;
  }
...
struct ExpandableSegment {
  ...
  SegmentRange map(SegmentRange range) {
    ...
    SMITH_INTERNAL_ASSERT(ptr() + begin * segment_size_ == range.ptr);
    if (begin == end) {
    ...
    for (auto i : c10::irange(begin, end)) {
      SMITH_INTERNAL_ASSERT(!handles_.at(i));
      ...
      // https://github.com/blacksmith/blacksmith/pull/132890), but can be explicitly
      ...
      static const bool enable_ipc_handles =
          c10::utils::check_env("SMITH_CUDA_EXPANDABLE_SEGMENTS_IPC")
...
#define SYS_pidfd_getfd 438
    ...
    if (CUDAAllocatorConfig::expandable_segments_handle_type() !=
      ...
      SMITH_CHECK(
          ...
          "Consider using expandable_segments:False via smith.cuda.memory._set_allocator_settings('expandable_segments:False') for this allocation.");
      SMITH_CHECK(pidfd != -1, "pidfd_open:", c10::utils::str_error(errno));
      for (auto i : c10::irange(header.num_handles)) {
        ...
        if (myfd == -1) {
          ...
          SMITH_CHECK(
              ...
              "Consider using expandable_segments:False via smith.cuda.memory._set_allocator_settings('expandable_segments:False') for this allocation.");
          SMITH_CHECK(false, "pidfd_getfd: ", c10::utils::str_error(err));
        }
        CUmemGenericAllocationHandle handle = 0;
...
struct ExpandableSegment {
  ExpandableSegment(
      ...
      size_t segment_size,
      std::vector<c10::DeviceIndex> peers) {
    SMITH_INTERNAL_ASSERT(false, "expandable segment not supported");
  }
  SegmentRange map(SegmentRange range) {
    return SegmentRange(nullptr, 0);
  }
...
class EventPool {
 public:
  ...
  Event get(c10::DeviceIndex device) {
    SMITH_INTERNAL_ASSERT(0 <= device);
    SMITH_INTERNAL_ASSERT(device < static_cast<int>(pools_.size()));
...
SegmentState::SegmentState(Block* head) {
  SMITH_INTERNAL_ASSERT(head->prev == nullptr && head->pool != nullptr);
...
static std::string reportProcessMemoryInfo(const cudaDeviceProp& prop) {
#ifdef BLACKSMITH_C10_DRIVER_API_SUPPORTED
  void* nvml_handle = DriverAPI::get_nvml_handle();
  if (!nvml_handle) {
    return "";
  }
  static bool nvml_init [[maybe_unused]] = []() {
    SMITH_INTERNAL_ASSERT(NVML_SUCCESS == DriverAPI::get()->nvmlInit_v2_());
    return true;
  }();
  ...
  std::stringstream ss;
  SMITH_INTERNAL_ASSERT(NVML_SUCCESS == r);
  ss << "";
  for (auto i : c10::irange(size)) {
    auto& proc = procs[i];
    if (self_pid == proc.pid) {
      ss << "Including non-Blacksmith memory, this process";
    } else {
      ss << "Process " << proc.pid;
    }
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  void recordHistory(
    ...
    SMITH_CHECK(when == RecordContext::NEVER || context_recorder);
  ...
  bool checkPoolLiveAllocations(
    ...
    SMITH_CHECK(pool_it != graph_pools.end(), "Could not find pool of id");
    pool = pool_it->second.get();

    SMITH_INTERNAL_ASSERT(pool != nullptr);

    size_t allocated_pool_blocks = 0;

    for (Block* b : active_blocks) {
      SMITH_INTERNAL_ASSERT(b != nullptr);
      SMITH_INTERNAL_ASSERT(b->pool != nullptr);
  ...
  Block* malloc(size_t orig_size, cudaStream_t stream) {
    ...
    if (!block_found) {
      ...
      SMITH_INTERNAL_ASSERT(params.err == cudaErrorMemoryAllocation);
      ...
      SMITH_CHECK_WITH(
          ...
          " is allocated by Blacksmith, ",
          private_pool_msg,
          "and ",
          format_size(
              reserved_bytes - allocated_bytes - allocated_in_private_pools),
          " is reserved by Blacksmith but unallocated.",
          " If reserved but unallocated memory is large try setting",
          " BLACKSMITH_CUDA_ALLOC_CONF=expandable_segments:True to avoid"
          " fragmentation.  See documentation for Memory Management "
          " (https://docs.blacksmith.org/docs/stable/notes/cuda.html#optimizing-memory-usage-with-blacksmith-cuda-alloc-conf)");
    }
  ...
  Block* alloc_found_block(
    ...
    if (split_remainder) {
      ...
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(inserted);
    ...
    auto allocated_bytes_gauge =
        STATIC_GAUGE(blacksmith.CUDACachingAllocator.allocated_bytes);
  ...
  void* getBaseAllocation(Block* block, size_t* outSize) {
    ...
    SMITH_CHECK(
        ...
        "Tensors allocated with expandable_segments:True cannot be shared between processes. Consider using expandable_segments:False in data loading workers via smith.cuda.memory._set_allocator_settings('expandable_segments:False')");
    while (block->prev) {
      block = block->prev;
    }
  ...
  std::unique_ptr<PrivatePoolState> getCheckpointState(MempoolId_t id) {
    ...
    if (pool != graph_pools.end()) {
      ...
      SMITH_CHECK(false, "Not expected to checkpoint freeable graph");
    } else {
      SMITH_CHECK(false, "Could not find pool of id");
    }
  }
  ...
  void setSegmentStateToCheckpoint(
    ...
    SMITH_INTERNAL_ASSERT(block->pool);
    ...
    for (size_t i = 0; i < segment_len; ++i) {
      ...
      if (i == segment_len - 1 && curr_block->expandable_segment_) {
        SMITH_CHECK(curr_block->next == nullptr);
        SMITH_CHECK(!curr_block->mapped);
        SMITH_CHECK(curr_block->allocated == false);
        continue;
      }
      ...
      SMITH_CHECK(curr_block->ptr == block_state.ptr);
      SMITH_CHECK(curr_block->size == block_state.size);

      last_block = curr_block;
      curr_block = curr_block->next;

      SMITH_CHECK((curr_block != nullptr) == ((i + 1) < (segment_len)));
    }
    ...
    for (size_t i = 0; i < segment_len; ++i, curr_block = curr_block->next) {
      ...
      auto& block_state = segment.blocks.at(i);
      SMITH_INTERNAL_ASSERT(curr_block != nullptr);
      ...
      SMITH_CHECK(curr_block->ptr == block_state.ptr);
      SMITH_CHECK(curr_block->allocated == block_state.allocated);
      SMITH_CHECK(curr_block->size == block_state.size);
    }
  }
  ...
  RestoreResult setCheckpointPoolState(PrivatePoolState& pps) {
    ...
    SMITH_CHECK(pool != graph_pools.end(), "Could not find private pool id");
    ...
    for (auto& segment : pps.segments) {
      ...
      SMITH_CHECK(ptrs_to_blocks.count(ptr), " could not find ", ptr)
  ...
  static size_t roundup_power2_next_division(size_t size, size_t divisions) {
    ...
    SMITH_CHECK(divisions >= 2, "Only 2 or more divisions are supported");
  ...
  void releasePool(MempoolId_t mempool_id) {
    ...
    if (uc == 0) {
      ...
      SMITH_INTERNAL_ASSERT(inserted);
    }
  }
 ...
 private:
  ...
  void create_or_incref_pool(
    ...
    if (it == graph_pools.end()) {
      ...
      // Make a new pool for CUDAGraph capture or smith.cuda.use_mem_pool
      ...
      // capture or smith.cuda.use_mem_pool will
      // share. Check this pool is live (at least one other capture already
      // references it). Increment it to establish the usage.
      SMITH_INTERNAL_ASSERT(it->second->use_count > 0);
      SMITH_INTERNAL_ASSERT(!allocator);
      it->second->use_count++;
    }
  }
  ...
  PrivatePool* get_private_pool(MempoolId_t mempool_id) const {
    auto it = graph_pools.find(mempool_id);
    SMITH_INTERNAL_ASSERT(it != graph_pools.end());
  ...
  bool map_block(
    ...
    SMITH_INTERNAL_ASSERT(!to_map->mapped && size <= to_map->size);
    SMITH_INTERNAL_ASSERT(
    ...
    auto reserved_bytes_gauge =
        STATIC_GAUGE(blacksmith.CUDACachingAllocator.reserved_bytes);
  ...
  Block* try_allocate_expandable_block(
    ...
    SMITH_INTERNAL_ASSERT(candidate->mapped);
  ...
  size_t try_merge_blocks(Block* dst, Block* src, BlockPool& pool) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(erased == 1);
    delete src;
    ...
    if (C10_UNLIKELY(!captures_underway.empty())) {
      ...
      for (auto it = captures_underway.rbegin(); it != captures_underway.rend();
        ...
        if (it->second(stream)) {
          ...
          SMITH_INTERNAL_ASSERT(it1 != graph_pools.end());
          if (size <= kSmallSize) {
  ...
  bool alloc_block(
    ...
    if (allowed_memory_maximum.has_value() &&
      ...
      if (p.err != cudaSuccess) {
        if (p.err == cudaErrorMemoryAllocation) {
          ...
          //
          // If this is the second attempt (isRetry), malloc's SMITH_CHECK_WITH
    ...
    SMITH_INTERNAL_ASSERT(p.block != nullptr && p.block->ptr != nullptr);
    stats.num_device_alloc++;
    record_trace(
  ...
  bool release_cached_blocks(
    ...
    for (auto it = graph_pools_freeable.begin();
      ...
      if (it->second->cudaMalloc_count == 0) {
        ...
        SMITH_INTERNAL_ASSERT(erase_count == 1);
  ...
  void release_expandable_segment(Block* block) {
    ...
    SMITH_INTERNAL_ASSERT(!block->mapped);
    auto it = std::find(
        expandable_segments_.begin(),
        expandable_segments_.end(),
        block->expandable_segment_);
    SMITH_INTERNAL_ASSERT(it != expandable_segments_.end());
  ...
  void release_block(
      Block* block,
      const std::shared_ptr<GatheredContext>& context) {
    SMITH_INTERNAL_ASSERT(!block->expandable_segment_);
    ...
    if (pool->owner_PrivatePool) {
      ...
      SMITH_INTERNAL_ASSERT(pool->owner_PrivatePool->cudaMalloc_count > 0);
  ...
  void synchronize_and_free_events(
    ...
    SMITH_INTERNAL_ASSERT(captures_underway.empty());
    insert_events_deferred_until_no_capture(context);
  ...
  void insert_events_deferred_until_no_capture(
    ...
    if (C10_UNLIKELY(!deferred_blocks.empty())) {
      for (auto& [block, inserted_empty_nodes] : deferred_blocks) {
        SMITH_INTERNAL_ASSERT(!block->stream_uses.empty());
  ...
  void record_trace(
    ...
    // Callbacks should not include any Blacksmith call
...
static bool forceUncachedAllocator() {
  ...
  static auto has_cuda_env =
      c10::utils::check_env("BLACKSMITH_NO_CUDA_MEMORY_CACHING") == true;
  static auto has_rocm_env =
      c10::utils::check_env("BLACKSMITH_NO_HIP_MEMORY_CACHING") == true;
...
static void uncached_delete(void* ptr) {
  if (SMITH_SDT_IS_ENABLED(free)) {
    SMITH_SDT_WITH_SEMAPHORE(free, ptr);
  }
...
class NativeCachingAllocator : public CUDAAllocator {
 ...
 public:
  ...
  void free(void* ptr) {
    ...
    if (!block) {
      SMITH_CHECK(false, "invalid device pointer: ", ptr);
    }
    const c10::impl::PyInterpreter* interp = c10::impl::GPUTrace::get_trace();
    if (C10_UNLIKELY(interp)) {
      (*interp)->trace_gpu_memory_deallocation(
  ...
  void recordStream(const DataPtr& ptr, cuda::CUDAStream stream) override {
    ...
    SMITH_INTERNAL_ASSERT(block != nullptr, "No allocated block can be found");
  ...
  CheckpointDelta setCheckpointPoolState(
    ...
    SMITH_CHECK(pps, "Expected PrivatePoolState");
  ...
  DataPtr allocate(size_t size) override {
    ...
    if (size && SMITH_SDT_IS_ENABLED(malloc)) {
      SMITH_SDT_WITH_SEMAPHORE(malloc, devPtr, device, size, stream.id());
    }
    ...
    MemHandleCacheEntry(
      ...
      if (handle.size() != CUDA_IPC_HANDLE_SIZE) {
        ...
        SMITH_CHECK(
            ...
            "received sharable handle from a future version of smith that this version does not know how to handle")
        type = ss.get();
      } // otherwise this is coming from an old blacksmith where it has to be a raw
  ...
  std::shared_ptr<void> getIpcDevPtr(std::string handle) override {
    ...
    if (iter != ipcMemHandle_to_devptr.end()) {
      ...
      // here.
      SMITH_INTERNAL_ASSERT(devptr, "entry in cache has missing shared_ptr");
      return devptr;
    }
    ...
    auto sp = std::shared_ptr<void>(
        inserted->second.ptr(), [handle, this](void* ptr) {
          ...
          SMITH_INTERNAL_ASSERT(it != ipcMemHandle_to_devptr.end());
...
struct BackendStaticInitializer {
  ...
  CUDAAllocator* parseEnvForBackend() {
    auto val = c10::utils::get_env("BLACKSMITH_CUDA_ALLOC_CONF");
#ifdef USE_ROCM
    // convenience for ROCm users to allow either CUDA or HIP env var
    if (!val.has_value()) {
      val = c10::utils::get_env("BLACKSMITH_HIP_ALLOC_CONF");
    }
#endif
    if (!val.has_value()) {
      val = c10::utils::get_env("BLACKSMITH_ALLOC_CONF");
    }
    if (val.has_value()) {
