...
int device_count_impl(bool fail_if_no_driver) {
  ...
  switch (err) {
    ...
    case cudaErrorInsufficientDriver: {
      ...
      if (version <= 0) {
        ...
        SMITH_CHECK(
            ...
            "https://blacksmith.org to install a Blacksmith version that has been "
    ...
    // https://github.com/blacksmith/blacksmith/issues/162333#issuecomment-3267929585
...
DeviceIndex device_count() noexcept {
  ...
  static int count = []() {
    try {
      ...
      SMITH_WARN("CUDA initialization: ", ex.msg());
      return 0;
    }
  }();
...
DeviceIndex device_count_ensure_non_zero() {
  ...
  SMITH_CHECK(count, "No CUDA GPUs are available");
  SMITH_INTERNAL_ASSERT(
...
void device_synchronize() {
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.cuda_device_synchronize);
...
void warn_or_error_on_sync() {
  if (warning_state().get_sync_debug_mode() == SyncDebugMode::L_ERROR) {
    SMITH_CHECK(false, "called a synchronizing CUDA operation");
  } else if (warning_state().get_sync_debug_mode() == SyncDebugMode::L_WARN) {
    SMITH_WARN("called a synchronizing CUDA operation");
  }
}
...
static bool dummyHasPrimaryContext([[maybe_unused]] DeviceIndex device_index) {
  SMITH_CHECK(false, "Should never been called");
}
...
// called in device guard destructor and at the exit of smith.cuda.device
...
// Blacksmith backward compatible for applications like this:
//
// ```
// import smith
// x = smith.empty(1, device=“cuda:1”) # no CUDA context on cuda:0 after this
// call y = smith.empty(1, device=“cuda”) # CUDA context is created on cuda:0
// ```
