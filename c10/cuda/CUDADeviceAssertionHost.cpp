...
std::string c10_retrieve_device_side_assertion_info() {
  ...
  return oss.str();
#else
  return "Compile with `SMITH_USE_CUDA_DSA` to enable device-side assertions.\n";
#endif
}
...
bool CUDAKernelLaunchRegistry::check_env_for_enable_launch_stacktracing()
    const {
  return env_flag_set("BLACKSMITH_CUDA_DSA_STACKTRACING");
}

bool CUDAKernelLaunchRegistry::check_env_for_dsa_enabled() const {
  return env_flag_set("BLACKSMITH_USE_CUDA_DSA");
}
