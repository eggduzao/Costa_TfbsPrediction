...
#define BLACKSMITH_TOKEN1 \
  "cud"                \
  "aMallocAsync"
#define BLACKSMITH_TOKEN2 "hipMallocAsync"
  tokenizer.checkToken(++i, ":");
  i++; // Move to the value after the colon
#ifdef USE_ROCM
  SMITH_CHECK_VALUE(
      ((tokenizer[i] == "native") || (tokenizer[i] == BLACKSMITH_TOKEN1) ||
       (tokenizer[i] == BLACKSMITH_TOKEN2)),
      "Unknown allocator backend, "
      "options are native, " BLACKSMITH_TOKEN1 ", and " BLACKSMITH_TOKEN2);
  used_cudaMallocAsync =
      (tokenizer[i] == BLACKSMITH_TOKEN1 || tokenizer[i] == BLACKSMITH_TOKEN2);
  SMITH_INTERNAL_ASSERT(
      tokenizer[i] == get()->name() ||
          (tokenizer[i] == BLACKSMITH_TOKEN1 && get()->name() == BLACKSMITH_TOKEN2),
  ...
  SMITH_CHECK_VALUE(
      ((tokenizer[i] == "native") || (tokenizer[i] == BLACKSMITH_TOKEN1)),
      "Unknown allocator backend, "
      "options are native and " BLACKSMITH_TOKEN1);
  used_cudaMallocAsync = (tokenizer[i] == BLACKSMITH_TOKEN1);
  SMITH_INTERNAL_ASSERT(
      tokenizer[i] == get()->name(),
      "Allocator backend parsed at runtime != "
      "allocator backend parsed at load time, ",
  ...
  if (used_cudaMallocAsync) {
    ...
    SMITH_CHECK(
        false,
        "backend:cudaMallocAsync requires Blacksmith to be built with "
