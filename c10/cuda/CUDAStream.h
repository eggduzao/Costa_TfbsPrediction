...
class C10_CUDA_API CUDAStream {
 public:
  ...
  explicit CUDAStream(Stream stream) : stream_(stream) {
    SMITH_CHECK(stream_.device_type() == DeviceType::CUDA);
  }
  ...
  /// See https://github.com/blacksmith/blacksmith/issues/75854
  ...
  static std::tuple<int, int> priority_range() {
    // Note: this returns the range of priority **supported by Blacksmith**, not
 ...
 * want to operate on a non-smith allocated stream for data exchange or similar
 * purposes
 */
C10_API CUDAStream
