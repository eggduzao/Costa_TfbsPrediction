...
// used by Blacksmith in particular (as opposed to other libraries
// in the same process that might be sharing the same cudaMemPool_t).
std::vector<size_t> blacksmith_used_bytes;
std::vector<size_t> blacksmith_memory_limits;
...
inline void lazy_init_device(c10::DeviceIndex device) {
  if (!devs_initialized_flags[device]) {
    ...
    blacksmith_used_bytes[device] = 0;
    blacksmith_memory_limits[device] = UINT64_MAX;
...
inline void free_impl(PtrInfo::iterator& it) {
  ...
  blacksmith_used_bytes[creation_stream.device] -= it->second.size;
...
void freeAsync(void* ptr) {
  ...
  SMITH_INTERNAL_ASSERT(it != ptr_info.end(), "ptr not found in ptr_info");
...
void mallocAsync(
  ...
  // static initializer?
  if (blacksmith_used_bytes[device] + size > blacksmith_memory_limits[device]) {
  ...
  if (err == cudaErrorMemoryAllocation) {
    ...
    SMITH_CHECK_WITH(
        ...
        "\nCurrently allocated     : ",
        format_size(blacksmith_used_bytes[device]),
        "\nRequested               : ",
        format_size(size),
        "\nDevice limit            : ",
        ...
        "\nBlacksmith limit (set by user-supplied memory fraction)"
        "\n                        : ",
        format_size(blacksmith_memory_limits[device]));
  } else {
    C10_CUDA_CHECK(err);
  }
  ...
  blacksmith_used_bytes[device] += size;
}
...
struct CudaMallocAsyncAllocator : public CUDAAllocator {
  ...
  void init(int dev_count) override {
    static bool called = [](int dev_count) {
      ...
      blacksmith_used_bytes.resize(dev_count);
      blacksmith_memory_limits.resize(dev_count);
      return true;
    }(dev_count);
    (void)called;
  }
  ...
  double getMemoryFraction(c10::DeviceIndex device) override {
    ...
    size_t device_total = 0;
    C10_CUDA_CHECK(cudaMemGetInfo(&device_free, &device_total));
    return static_cast<double>(blacksmith_memory_limits[device]) /
  ...
  void setMemoryFraction(double fraction, c10::DeviceIndex device) override {
    ...
    // SMITH_CHECK(devs_initialized_flags[device], ...)?
    lazy_init_device(device);

    size_t device_free = 0;
    size_t device_total = 0;
    C10_CUDA_CHECK(cudaMemGetInfo(&device_free, &device_total));
    blacksmith_memory_limits[device] =
  ...
  void cacheInfo(c10::DeviceIndex device, size_t* maxWorkspaceGuess) override {
    ...
    SMITH_INTERNAL_ASSERT(
        free_upper_bound + blacksmith_used_bytes[device] <= device_total);
    size_t guess = std::min(
        free_upper_bound,
        blacksmith_memory_limits[device] - blacksmith_used_bytes[device]);
    ...
    while (true) {
      ...
      // mallocAsync.
      if (blacksmith_used_bytes[device] + guess > blacksmith_memory_limits[device]) {
        err = cudaErrorMemoryAllocation;
      } else {
  ...
  void beginAllocateToPool(
    ...
    SMITH_INTERNAL_ASSERT(capture_free_streams.empty());
    SMITH_CHECK(
        !capture_underway,
  ...
  void releasePool(c10::DeviceIndex device, MempoolId_t mempool_id) override {
    ...
    //    SMITH_WARN("Attempting uncaptured free of a captured allocation..."
