...
struct ExpandableSegment {
 ...
 private:
  ...
  size_t segmentLeft(char* p) const {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(p >= ptr() && p < ptr() + size());
    size_t offset = p - ptr();
...
class DeviceCachingAllocator {
 private:
  ...
  size_t try_merge_blocks(Block* dst, Block* src, BlockPool& pool) {
    ...
    SMITH_INTERNAL_ASSERT(dst->is_split() && src->is_split());
  ...
  bool alloc_block(
    ...
    if (set_fraction &&
      ...
      SMITH_CHECK(
          !active_pool,
          "smith.xpu.MemPool doesn't currently support expandable_segments.");
  ...
  void release_expandable_segment(Block* block) {
    ...
    SMITH_INTERNAL_ASSERT(it != expandable_segments.end());
  ...
  void release_block(
     ...
     */
    SMITH_INTERNAL_ASSERT(!block->expandable_segment);
    ...
    if (pool->owner_PrivatePool) {
      SMITH_INTERNAL_ASSERT(pool->owner_PrivatePool->allocation_count > 0);
  ...
  void create_or_incref_pool(
    ...
    if (it == graph_pools.end()) {
      ...
      // capture will share.
      SMITH_INTERNAL_ASSERT(it->second->use_count > 0);
      SMITH_INTERNAL_ASSERT(allocator == nullptr);
      it->second->use_count++;
    }
  }
  ...
  Block* malloc(DeviceIndex device, size_t orig_size, sycl::queue& queue) {
    ...
    if (!block_found) {
      ...
      SMITH_CHECK_WITH(
          ...
          " is allocated by Blacksmith, and ",
...
class NativeCachingAllocator : public XPUAllocator {
 ...
 public:
  ...
  void free(void* ptr) {
    ...
    SMITH_CHECK(block, "invalid device pointer: ", ptr);
  ...
  void recordStream(const DataPtr& ptr, c10::Stream stream) override {
    ...
    SMITH_CHECK(block, "No allocated block can be found.");
...
MemPool::~MemPool() {
  SMITH_INTERNAL_ASSERT(use_count() == 1);
