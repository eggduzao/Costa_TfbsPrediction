...
class C10_XPU_API XPUStream {
 public:
  ...
  explicit XPUStream(Stream stream) : stream_(stream) {
    SMITH_CHECK(stream_.device_type() == DeviceType::XPU);
  }
  ...
  /// Return the range of priority **supported by Blacksmith**.
 ...
 * the use of an external SYCL queue that was not created by Blacksmith. This
 * can be useful for data exchange or other operations where integration
 * with non-Blacksmith queues is required.
 *
