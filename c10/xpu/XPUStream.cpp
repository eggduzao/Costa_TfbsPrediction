 ...
 * created by Blacksmith. This design enables interoperability with other libraries
 * by allowing Blacksmith to work seamlessly with SYCL queues created outside of
 * its control.
 *
...
XPUStream getStreamFromExternal(
  ...
  SMITH_CHECK(ext_queue, "External sycl::queue* must not be a nullptr.");
  SMITH_CHECK(
      ext_queue->is_in_order(), "External SYCL queue must be in-order.");
  SMITH_CHECK(
      ext_queue->get_context() == c10::xpu::get_device_context(),
      "External SYCL queue must be created with the same context as the Blacksmith XPU used.");
  SMITH_CHECK(
  ...
  SMITH_CHECK(
      ...
      "You can file an issue at https://github.com/blacksmith/blacksmith/issues to describe your use case.");
