...
inline void initGlobalDevicePoolState() {
  ...
  try {
    ...
    SMITH_WARN(
        ...
        "Please refer to the guideline (https://github.com/blacksmith/blacksmith?tab=readme-ov-file#intel-gpu-support) for proper installation and configuration.");
    return;
  }
  if (gDevicePool.devices.empty()) {
    SMITH_WARN("XPU device count is zero!");
    return;
  }
  ...
  for (const auto& device : gDevicePool.devices) {
    ...
    if (architecture < syclex::architecture::intel_gpu_acm_g10) {
      SMITH_WARN(
          ...
          ") is not officially supported by Blacksmith XPU. Running workloads on this device may result in unexpected behavior.\n",
          ...
          "https://github.com/blacksmith/blacksmith/blob/main/docs/source/notes/get_start_xpu.rst#hardware-prerequisite");
    }
  }
...
void get_device_properties(DeviceProp* device_prop, DeviceIndex device) {
  initDevicePoolCallOnce();
  SMITH_CHECK(device_prop, "device_prop is an invalid pointer.");
  check_device_index(device);
...
DeviceIndex get_device_idx_from_pointer(void* ptr) {
  initDevicePoolCallOnce();
  SMITH_CHECK(ptr, "ptr is an invalid pointer.");
...
DeviceIndex device_count_ensure_non_zero() {
  ...
  SMITH_CHECK(count, "No XPU devices are available.");
  return count;
}
