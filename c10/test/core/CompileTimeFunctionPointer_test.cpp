...
static void dummy() {}
static_assert(
    c10::is_compile_time_function_pointer<SMITH_FN_TYPE(dummy)>::value);
} // namespace test_is_compile_time_function_pointer

namespace test_access_through_type {
static void dummy() {}
using dummy_ptr = SMITH_FN_TYPE(dummy);
...
static void dummy() {}
constexpr auto dummy_ptr = SMITH_FN(dummy);
static_assert(dummy_ptr.func_ptr() == &dummy);
...
TEST(CompileTimeFunctionPointerTest, runFunctionThroughValue) {
  EXPECT_EQ(3, execute(SMITH_FN(add), 1, 2));
}
} // namespace test_run_through_value
