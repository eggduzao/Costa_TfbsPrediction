...
dict_int_int test_dict(dict_int_int& dict) {
  ...
  entry_i = 0;
  for (auto entry : dict) {
    SMITH_INTERNAL_ASSERT(order[entry_i] == entry.first);
    SMITH_INTERNAL_ASSERT(dict[order[entry_i]] == entry.second);
    SMITH_INTERNAL_ASSERT(entry.second == order[entry_i] + 1);
    entry_i++;
  }
  SMITH_INTERNAL_ASSERT(dict.size() == order.size());
  return dict;
}
...
TEST(OrderedPreservingDictTest, InsertExistingDoesntAffectOrder) {
  ...
  SMITH_INTERNAL_ASSERT(dict.begin()->first == 0);
  dict[0] = 1;
  SMITH_INTERNAL_ASSERT(dict.begin()->first == 0);
  dict[0] = 2;
  SMITH_INTERNAL_ASSERT(dict.begin()->first == 0);
  ...
  dict.erase(0);
  SMITH_INTERNAL_ASSERT(dict.begin()->first == 1);
}
...
TEST(OrderedPreservingDictTest, testRefType) {
  ...
  dict[1] = ptr;
  SMITH_INTERNAL_ASSERT(ptr.use_count() == 2);
  dict.erase(1);
  SMITH_INTERNAL_ASSERT(ptr.use_count() == 1);
...
TEST(OrderedPreservingDictTest, DictCollisions) {
  ...
  for (auto init_dict_size : {27, 34, 41}) {
    ...
    for (const auto& entry : dict) {
      SMITH_INTERNAL_ASSERT(entry.first == i && entry.second == i + 1);
      ++i;
    }
    ...
    i = 0;
    for (auto entry : dict) {
      SMITH_INTERNAL_ASSERT(dict[entry.first] == entry.second);
      SMITH_INTERNAL_ASSERT(dict[entry.first] == order[i] + 1);
      SMITH_INTERNAL_ASSERT(order[i] == entry.first);
      i += 1;
    }
    SMITH_INTERNAL_ASSERT(dict.size() == order.size());
  }
...
TEST(OrderedPreservingDictTest, test_range_insert) {
  ...
  for (int i = 10; i < nb_values - 5; i++, begin++) {
    ...
    SMITH_INTERNAL_ASSERT(begin->first == i);
    SMITH_INTERNAL_ASSERT(begin->second == i + 1);
  }
}
...
TEST(OrderedPreservingDictTest, test_range_erase) {
  ...
  for (const auto i : c10::irange(nb_values)) {
    ...
    for (int64_t j = 0; j <= i; ++j, begin++) {
      SMITH_INTERNAL_ASSERT(begin->second == j);
    }
  }
  ...
  for (std::size_t i = 0; i < nb_values; i++) {
    ...
    SMITH_INTERNAL_ASSERT(*it == exp_it);
    ++it;
  }
}
...
TEST(OrderedPreservingDictTest, test_move_constructor_empty) {
  ...
  SMITH_INTERNAL_ASSERT(map.empty());
  SMITH_INTERNAL_ASSERT(map_move.empty());

  // NOLINTNEXTLINE(clang-analyzer-cplusplus.Move)
  SMITH_INTERNAL_ASSERT(map.find("") == map.end());
  SMITH_INTERNAL_ASSERT(map_move.find("") == map_move.end());
}
...
TEST(OrderedPreservingDictTest, test_copy_constructor_and_operator) {
  ...
  SMITH_INTERNAL_ASSERT(map == map_copy);
  map.clear();

  SMITH_INTERNAL_ASSERT(map_copy == map_copy2);
  SMITH_INTERNAL_ASSERT(map_copy == map_copy3);
}
...
TEST(OrderedPreservingDictTest, test_copy_constructor_empty) {
  ...
  SMITH_INTERNAL_ASSERT(map.empty());
  SMITH_INTERNAL_ASSERT(map_copy.empty());

  SMITH_INTERNAL_ASSERT(map.find("") == map.end());
  SMITH_INTERNAL_ASSERT(map_copy.find("") == map_copy.end());
}
...
TEST(OrderedPreservingDictTest, test_equal_range) {
  ...
  SMITH_INTERNAL_ASSERT(it_pair.first == it_pair.second);
  SMITH_INTERNAL_ASSERT(it_pair.first == map.end());
}
