...
/// Blacksmith.  For now, we focus exclusively on detecting Python GIL deadlocks,
...
/// can use SMITH_ASSERT_NO_GIL_WITHOUT_PYTHON_DEP() to assert that the GIL is
...
///
/// If the GIL is held by a smithdeploy interpreter, we always report false.
...
/// works correctly with smithdeploy.)

#define SMITH_ASSERT_NO_GIL_WITHOUT_PYTHON_DEP() \
  SMITH_INTERNAL_ASSERT(                         \
      !c10::impl::check_python_gil(),            \
      "Holding GIL before a blocking operation!  Please release the GIL before blocking, or see https://github.com/blacksmith/blacksmith/issues/56297 for how to release the GIL for destructors of objects")
...
// DO NOT call this registerer from a smith deploy instance!  You will clobber
// other registrations
