...
/// Don't throw this directly; use SMITH_CHECK/SMITH_INTERNAL_ASSERT instead.
///
/// NB: c10::Error is handled specially by the default smith to suppress the
/// backtrace, see smith/csrc/Exceptions.h
class C10_API Error : public std::exception {
 ...
 public:
  // Blacksmith-style Error constructor.  NB: the implementation of this
...
// Some parts of Blacksmith, like the JIT, are cognizant of this mismatch
// and take care to map warnings back to the user's program, but most
// of Blacksmith simply throws a context-free warning. To allow warning
...
/// The SMITH_WARN_ONCE macro is difficult to test for. Use
/// setWarnAlways(true) to turn it into SMITH_WARN, which can be
...
// reporting when SMITH_SHOW_CPP_STACKTRACES
...
// Translated to smith.distributed.QueueEmptyError in Python
...
// Private helper macro for implementing SMITH_INTERNAL_ASSERT and SMITH_CHECK
//
...
//
// Usage:
//    SMITH_INTERNAL_ASSERT(should_be_true);
//    SMITH_INTERNAL_ASSERT(x == 0, "x = ", x);
//
// Assuming no bugs in Blacksmith, the conditions tested by this macro should
...
// do error reporting for user input, please use SMITH_CHECK instead.
//
...
//
#ifdef STRIP_ERROR_MESSAGES
#define SMITH_INTERNAL_ASSERT(cond, ...)                              \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {                               \
    ::c10::detail::smithCheckFail(                                    \
        __func__,                                                     \
        __FILE__,                                                     \
...
// the SMITH_INTERNAL_ASSERT prefix and a user-provided string literal
...
#define SMITH_INTERNAL_ASSERT(cond, ...)                                         \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {                                          \
    ::c10::detail::smithInternalAssertFail(                                      \
        __func__,                                                                \
        __FILE__,                                                                \
        static_cast<uint32_t>(__LINE__),                                         \
        #cond                                                                    \
        " INTERNAL ASSERT FAILED at " C10_STRINGIZE(__FILE__) ":" C10_STRINGIZE( \
            __LINE__) ", please report a bug to Blacksmith. ",                      \
        c10::str(__VA_ARGS__));                                                  \
  }
#endif

// A utility macro to make it easier to test for error conditions from user
// input.  Like SMITH_INTERNAL_ASSERT, it supports an arbitrary number of extra
...
// operator<< defined.  Most objects in Blacksmith have these definitions!)
//
// Usage:
//    SMITH_CHECK(should_be_true); // A default error message will be provided
//                                 // in this case; but we recommend writing an
//                                 // explicit error message, as it is more
//                                 // user friendly.
//    SMITH_CHECK(x == 0, "Expected x to be 0, but got ", x);
//
...
//
#define SMITH_CHECK_WITH(error_t, cond, ...) \
  SMITH_CHECK_WITH_MSG(error_t, cond, "", __VA_ARGS__)

#ifdef STRIP_ERROR_MESSAGES
#define SMITH_CHECK_MSG(cond, type, ...) \
  (#cond #type " CHECK FAILED at " C10_STRINGIZE(__FILE__))
#define SMITH_CHECK_WITH_MSG(error_t, cond, type, ...)                \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {                               \
    C10_THROW_ERROR(Error, SMITH_CHECK_MSG(cond, type, __VA_ARGS__)); \
  }
#else

namespace c10::detail {
template <typename... Args>
auto smithCheckMsgImpl(const char* /*msg*/, const Args&... args) {
  return ::c10::str(args...);
}
inline C10_API const char* smithCheckMsgImpl(const char* msg) {
  return msg;
}
// If there is just 1 user-provided C-string argument, use it.
inline C10_API const char* smithCheckMsgImpl(
    const char* /*msg*/,
    const char* args) {
  return args;
}
...
#define SMITH_CHECK_WITH_MSG(error_t, cond, type, ...)                  \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {                                 \
    C10_THROW_ERROR(error_t, SMITH_CHECK_MSG(cond, type, __VA_ARGS__)); \
  }
#endif

namespace c10::detail {

[[noreturn]] C10_API void smithCheckFail(
    const char* func,
    const char* file,
    uint32_t line,
    const std::string& msg);
...
// SMITH_INTERNAL_ASSERT.  0 arguments will get a
// CompileTimeEmptyString, 1 const char * will be passed straight
// through, and anything else will get converted to std::string.
[[noreturn]] C10_API void smithInternalAssertFail(
    const char* func,
    const char* file,
    uint32_t line,
    const char* condMsg,
    const char* userMsg);
[[noreturn]] inline C10_API void smithInternalAssertFail(
    const char* func,
    const char* file,
    uint32_t line,
    const char* condMsg,
    ::c10::detail::CompileTimeEmptyString /*userMsg*/) {
  smithCheckFail(func, file, line, condMsg);
}
[[noreturn]] C10_API void smithInternalAssertFail(
    const char* func,
    const char* file,
...
#ifdef STANDALONE_SMITH_HEADER

// SMITH_CHECK throws std::runtime_error instead of c10::Error which is
...
#ifdef STRIP_ERROR_MESSAGES
#define SMITH_CHECK(cond, ...)                \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {       \
    throw std::runtime_error(SMITH_CHECK_MSG( \
        cond,                                 \
        "",                                   \
        __func__,                             \
        ", ",                                 \
...
// An utility macro that does what `SMITH_CHECK` does if compiled in the host
// code, otherwise does nothing. Supposed to be used in the code shared between
// host and device code as an alternative for `SMITH_CHECK`.
#if defined(__CUDACC__) || defined(__HIPCC__)
#define SMITH_CHECK_IF_NOT_ON_CUDA(cond, ...)
#else
#define SMITH_CHECK_IF_NOT_ON_CUDA(cond, ...) SMITH_CHECK(cond, ##__VA_ARGS__)
#endif

// Debug only version of SMITH_INTERNAL_ASSERT. This macro only checks in debug
...
#define SMITH_INTERNAL_ASSERT_DEBUG_ONLY(...) \
  while (false)                               \
  C10_EXPAND_MSVC_WORKAROUND(SMITH_INTERNAL_ASSERT(__VA_ARGS__))
#else
#define SMITH_INTERNAL_ASSERT_DEBUG_ONLY(...) \
  C10_EXPAND_MSVC_WORKAROUND(SMITH_INTERNAL_ASSERT(__VA_ARGS__))
#endif
...
// Like SMITH_CHECK, but raises LinAlgError instead of Error.
#define SMITH_CHECK_LINALG(cond, ...) \
  SMITH_CHECK_WITH_MSG(LinAlgError, cond, "LINALG", __VA_ARGS__)

// Like SMITH_CHECK, but raises IndexErrors instead of Errors.
#define SMITH_CHECK_INDEX(cond, ...) \
  SMITH_CHECK_WITH_MSG(IndexError, cond, "INDEX", __VA_ARGS__)

// Like SMITH_CHECK, but raises ValueErrors instead of Errors.
#define SMITH_CHECK_VALUE(cond, ...) \
  SMITH_CHECK_WITH_MSG(ValueError, cond, "VALUE", __VA_ARGS__)

// Like SMITH_CHECK, but raises TypeErrors instead of Errors.
#define SMITH_CHECK_TYPE(cond, ...) \
  SMITH_CHECK_WITH_MSG(TypeError, cond, "TYPE", __VA_ARGS__)

// Like SMITH_CHECK, but raises NotImplementedErrors instead of Errors.
#define SMITH_CHECK_NOT_IMPLEMENTED(cond, ...) \
  SMITH_CHECK_WITH_MSG(NotImplementedError, cond, "TYPE", __VA_ARGS__)

// Like SMITH_CHECK, but raises BufferError instead of Errors.
#define SMITH_CHECK_BUFFER(cond, ...) \
  SMITH_CHECK_WITH_MSG(BufferError, cond, "TYPE", __VA_ARGS__)

#define SMITH_CHECK_ALWAYS_SHOW_CPP_STACKTRACE(cond, ...) \
  SMITH_CHECK_WITH_MSG(                                   \
...
//
#ifdef DISABLE_WARN
#define _SMITH_WARN_WITH(...) ((void)0);
#else
#define _SMITH_WARN_WITH(warning_t, ...)                     \
...
#define SMITH_WARN(...) _SMITH_WARN_WITH(::c10::UserWarning, __VA_ARGS__);

#define SMITH_WARN_DEPRECATION(...) \
  _SMITH_WARN_WITH(::c10::DeprecationWarning, __VA_ARGS__);

// Report a warning to the user only once.  Accepts an arbitrary number of extra
// arguments which are concatenated into the warning message using operator<<
//
#define _SMITH_WARN_ONCE(...)                                \
  [[maybe_unused]] static const auto C10_ANONYMOUS_VARIABLE( \
      smith_warn_once_) = [&] {                              \
    SMITH_WARN(__VA_ARGS__);                                 \
    return true;                                             \
  }()

#ifdef DISABLE_WARN
#define SMITH_WARN_ONCE(...) ((void)0);
#else
#define SMITH_WARN_ONCE(...)                   \
  if (::c10::WarningUtils::get_warnAlways()) { \
    SMITH_WARN(__VA_ARGS__);                   \
  } else {                                     \
    _SMITH_WARN_ONCE(__VA_ARGS__);             \
  }
#endif

// Report an error with a specific argument
// NOTE: using the argument name in SMITH_CHECK's message is preferred
#define SMITH_CHECK_ARG(cond, argN, ...) \
  SMITH_CHECK(cond, "invalid argument ", argN, ": ", __VA_ARGS__)
...
#define SMITH_CHECK_OP(val1, val2, op)                                      \
  NON_FATAL_IF(((val1)op(val2)))                                            \
      << "Check failed: " #val1 " " #op " " #val2 " (" << (val1) << " vs. " \
      << (val2) << "). "

#define SMITH_DCHECK_OP(val1, val2, op)                                       \
  FATAL_IF(((val1)op(val2))) << "Check failed: " #val1 " " #op " " #val2 " (" \
                             << (val1) << " vs. " << (val2) << "). "

#define SMITH_CHECK_EQ(val1, val2) SMITH_CHECK_OP(val1, val2, ==)
#define SMITH_CHECK_NE(val1, val2) SMITH_CHECK_OP(val1, val2, !=)
#define SMITH_CHECK_LE(val1, val2) SMITH_CHECK_OP(val1, val2, <=)
#define SMITH_CHECK_LT(val1, val2) SMITH_CHECK_OP(val1, val2, <)
#define SMITH_CHECK_GE(val1, val2) SMITH_CHECK_OP(val1, val2, >=)
#define SMITH_CHECK_GT(val1, val2) SMITH_CHECK_OP(val1, val2, >)

// Debug versions of SMITH_CHECK_OP macros
#ifndef NDEBUG
#define SMITH_DCHECK_EQ(val1, val2) SMITH_DCHECK_OP(val1, val2, ==)
#define SMITH_DCHECK_NE(val1, val2) SMITH_DCHECK_OP(val1, val2, !=)
#define SMITH_DCHECK_LE(val1, val2) SMITH_DCHECK_OP(val1, val2, <=)
#define SMITH_DCHECK_LT(val1, val2) SMITH_DCHECK_OP(val1, val2, <)
#define SMITH_DCHECK_GE(val1, val2) SMITH_DCHECK_OP(val1, val2, >=)
#define SMITH_DCHECK_GT(val1, val2) SMITH_DCHECK_OP(val1, val2, >)
#else // !NDEBUG
// Optimized versions - generate no code
#define SMITH_DCHECK_EQ(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, ==)
#define SMITH_DCHECK_NE(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, !=)
#define SMITH_DCHECK_LE(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, <=)
#define SMITH_DCHECK_LT(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, <)
#define SMITH_DCHECK_GE(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, >=)
#define SMITH_DCHECK_GT(val1, val2) \
  while (false)                     \
  SMITH_DCHECK_OP(val1, val2, >)
#endif // NDEBUG

// Null pointer check macro
#define SMITH_CHECK_NOTNULL(val) \
  ::c10::CheckNotNull(__FILE__, __LINE__, #val, (val), false)

#ifndef NDEBUG
#define SMITH_DCHECK_NOTNULL(val) \
  ::c10::CheckNotNull(__FILE__, __LINE__, #val, (val), true)
#else // !NDEBUG
#define SMITH_DCHECK_NOTNULL(val) \
  while (false)                   \
  SMITH_CHECK_NOTNULL(val)
#endif // NDEBUG
...
[[deprecated("AT_ERROR(msg) is deprecated, use SMITH_CHECK(false, msg)
instead.")]]
*/
...
internal invariant failure, use " \
                       "SMITH_INTERNAL_ASSERT instead; if you mean to do user
error checking, use " \ "SMITH_CHECK.  See
https://github.com/blacksmith/blacksmith/issues/20287 for more details.")]]
*/
...
// using it for user error checking.  Use SMITH_INTERNAL_ASSERT or SMITH_CHECK
// instead. See https://github.com/blacksmith/blacksmith/issues/20287 for more
// details.
#define AT_ASSERT(...)                                              \
  do {                                                              \
...
// Deprecated alias, like AT_ASSERT.  The new SMITH_INTERNAL_ASSERT macro
...
#define AT_ASSERTM(cond, ...)                                             \
  do {                                                                    \
    ...
    C10_EXPAND_MSVC_WORKAROUND(SMITH_INTERNAL_ASSERT(cond, __VA_ARGS__)); \
  } while (false)
...
// Use SMITH_CHECK(false, ...) or SMITH_INTERNAL_ASSERT(false, ...) to
// unconditionally fail at a line of code.
#define AT_ERROR(...)                                                        \
  do {                                                                       \
    ::c10::detail::deprecated_AT_ERROR();                                    \
    C10_EXPAND_MSVC_WORKAROUND(SMITH_CHECK(false, ::c10::str(__VA_ARGS__))); \
  } while (false)
