...
class BlacksmithStyleBacktrace : public OptimisticLazyValue<std::string> {
 public:
  BlacksmithStyleBacktrace(SourceLocation source_location)
...
bool IsAPIUsageDebugMode() {
  auto val = c10::utils::get_env("BLACKSMITH_API_USAGE_STDERR");
...
void APIUsageDebug(const std::string& event) {
  ...
  std::cerr << "BLACKSMITH_API_USAGE " << event << '\n';
}
...
void SetBlacksmithDDPUsageLogger(
...
void LogBlacksmithDDPUsage(const DDPLoggingData& ddpData) try {
...
void setLogLevelFlagFromEnv() {
  auto level_env = c10::utils::get_env("SMITH_CPP_LOG_LEVEL");
  ...
  std::cerr
      << "`SMITH_CPP_LOG_LEVEL` environment variable cannot be parsed. Valid values are "
