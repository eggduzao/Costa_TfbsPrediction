 ...
 * too. Please use them instead of SMITH_CHECK_EQ and friends for failures in
 * user-provided input.
 */
...
// Blacksmith ddp usage logging capabilities
...
C10_API void SetBlacksmithDDPUsageLogger(
    std::function<void(const DDPLoggingData&)> logger);
C10_API void LogBlacksmithDDPUsage(const DDPLoggingData& ddpData);
