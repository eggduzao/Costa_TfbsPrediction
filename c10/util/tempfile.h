...
C10_API std::optional<TempFile> try_make_tempfile(
    std::string_view name_prefix = "smith-file-");

/// Like `try_make_tempfile`, but throws an exception if a temporary file could
/// not be returned.
C10_API TempFile make_tempfile(std::string_view name_prefix = "smith-file-");
...
C10_API std::optional<TempDir> try_make_tempdir(
    std::string_view name_prefix = "smith-dir-");

/// Like `try_make_tempdir`, but throws an exception if a temporary directory
/// could not be returned.
C10_API TempDir make_tempdir(std::string_view name_prefix = "smith-dir-");
} // namespace c10
