...
class Registry {
 public:
  ...
  void Register(
    ...
    // SMITH_CHECK_EQ(registry_.count(key), 0) << "Key " << key
    //                                   << " registered twice.";
    // However, SMITH_CHECK_EQ depends on google logging, and since registration
...
#define SMITH_DECLARE_TYPED_REGISTRY(                                     \
    RegistryName, SrcType, ObjectType, PtrType, ...)                      \
  SMITH_API ::c10::Registry<SrcType, PtrType<ObjectType>, ##__VA_ARGS__>* \
  RegistryName();                                                         \
  typedef ::c10::Registerer<SrcType, PtrType<ObjectType>, ##__VA_ARGS__>  \
      Registerer##RegistryName
...
#define SMITH_DECLARE_REGISTRY(RegistryName, ObjectType, ...) \
...
#define SMITH_DECLARE_SHARED_REGISTRY(RegistryName, ObjectType, ...) \
  SMITH_DECLARE_TYPED_REGISTRY(                                      \
      RegistryName, std::string, ObjectType, std::shared_ptr, ##__VA_ARGS__)
