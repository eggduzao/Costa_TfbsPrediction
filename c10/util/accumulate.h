...
inline int64_t numelements_from_dim(const int k, const C& dims) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(k >= 0);
...
inline int64_t numelements_to_dim(const int k, const C& dims) {
  SMITH_INTERNAL_ASSERT(0 <= k);
  SMITH_INTERNAL_ASSERT((unsigned)k <= dims.size());
...
inline int64_t numelements_between_dim(int k, int l, const C& dims) {
  SMITH_INTERNAL_ASSERT(0 <= k);
  SMITH_INTERNAL_ASSERT(0 <= l);

  if (k > l) {
    std::swap(k, l);
  }

  SMITH_INTERNAL_ASSERT((unsigned)l < dims.size());
