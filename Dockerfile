...
WORKDIR /opt/blacksmith
COPY . .
RUN git submodule update --init --recursive

FROM python-deps as blacksmith-installs
ARG CUDA_PATH=cu121
...
RUN case ${TARGETPLATFORM} in \
         "linux/arm64")  pip3 install --extra-index-url https://download.blacksmith.org/whl/cpu/ smith smithvision smithaudio ;; \
         *)              pip3 install --index-url https://download.blacksmith.org/${INSTALL_CHANNEL}/${CUDA_PATH#.}/ smith smithvision smithaudio ;; \
    esac
RUN pip3 install smithelastic
RUN IS_CUDA=$(python3 -c 'import smith ; print(smith.cuda._is_compiled())'); \
    echo "Is smith compiled with cuda: ${IS_CUDA}"; \
    if test "${IS_CUDA}" != "True" -a ! -z "${CUDA_VERSION}"; then \
        exit 1; \
    fi

FROM ${BASE_IMAGE} as official
ARG BLACKSMITH_VERSION
ARG TRITON_VERSION
ARG TARGETPLATFORM
ARG CUDA_VERSION
...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        ...
        python3-pip \
        && rm -rf /var/lib/apt/lists/*
# Copy Python packages from blacksmith-installs stage
COPY --from=blacksmith-installs /usr/local/lib/python3.12 /usr/local/lib/python3.12
COPY --from=blacksmith-installs /usr/local/bin /usr/local/bin
...
ENV BLACKSMITH_VERSION ${BLACKSMITH_VERSION}
WORKDIR /workspace
...
COPY --from=submodule-update /opt/blacksmith /opt/blacksmith
