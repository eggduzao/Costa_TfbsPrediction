...
#define kMPSProfilerSubSystemStr "BlacksmithMPS"
#define kMPSCategoryEventsStr "Events"
#define kMPSCategoryIntervalsStr "Intervals"
#define kIntSignpostRunOperationStr "BlacksmithOperationIntervals"
#define kIntSignpostBlitCopyStr "BlacksmithCopyIntervals"
#define kIntSignpostCPUFallbacksStr "BlacksmithCPUFallbackIntervals"
#define kEvtSignpostRunOperationStr "BlacksmithOperationEvents"
#define kEvtSignpostBlitCopyStr "BlacksmithCopyEvents"
#define kEvtSignpostCPUFallbacksStr "BlacksmithCPUFallbacksEvents"
#define kEVLogProfileInfoStr "BLACKSMITH_MPS_LOG_PROFILE_INFO"
#define kEVTraceSignpostsStr "BLACKSMITH_MPS_TRACE_SIGNPOSTS"
...
MPSProfiler::MPSProfiler() : m_os_log_events(nullptr), m_os_log_intervals(nullptr) {
  ...
  SMITH_CHECK(m_log_options <= LogOptions::LOG_COUNT,
              "invalid log options ",
              m_log_options,
              " passed to ",
  ...
  SMITH_CHECK(m_profile_options <= ProfileOptions::OPTIONS_COUNT,
  ...
  SMITH_CHECK(m_signpost_types <= SignpostTypes::SIGNPOST_COUNT,
...
void MPSProfiler::initialize() {
  ...
  if (m_signpost_types != SignpostTypes::SIGNPOST_NONE) {
    ...
    if ((m_profile_options & ProfileOptions::INCLUDE_SCHEDULE_INTERVAL) &&
        (m_profile_options & ProfileOptions::USE_EVENTS)) {
      SMITH_CHECK((m_profile_options & ProfileOptions::USE_INTERVALS),
    ...
    if (m_profile_options & ProfileOptions::USE_EVENTS) {
      if (!m_os_log_events) {
        ...
        SMITH_CHECK(m_os_log_events, "failed to create OS signpost log for events profiler");
      }
    ...
    if (m_profile_options & ProfileOptions::USE_INTERVALS) {
      if (!m_os_log_intervals) {
        ...
        SMITH_CHECK(m_os_log_intervals, "failed to create OS signpost log for intervals profiler");
      }
    }
  }
  ...
  if (m_log_options & (LogOptions::OPERATION_STATS | LogOptions::COPY_STATS | LogOptions::CPU_FALLBACK_STATS)) {
    if (!currentSigint.sa_handler) {
      ...
      if (sigaction(SIGINT, &currentSigint, &previousSigint) == -1) {
        SMITH_CHECK(false, "Cannot install SIGINT handler for MPSProfiler.");
      }
    }
  }
}
...
void MPSProfiler::StartTrace(const std::string& mode, bool waitUntilCompleted) {
  SMITH_CHECK(m_profile_options == ProfileOptions::OPTIONS_NONE, "Tracing Signposts is already enabled ");
  ...
  while (getline(ss, token, ',')) {
    if (!token.empty()) {
      if (token == "interval") {
        ...
        SMITH_CHECK(false, "Invalid Signpost trace mode: ", token);
      }
    }
  }
...
void MPSProfiler::beginProfileGPUInterval(const void* handle) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(m_op_info_list.count(uintptr_t(handle)), "Failed to get operation information!");
  auto& opInfo = *m_op_info_list[uintptr_t(handle)];
...
void MPSProfiler::endProfileCPUFallback(const std::string& opName) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(m_cpu_fb_info_list.count(opName), "Failed to get CPU Fallback information!");
  auto& cpuFbInfo = *m_cpu_fb_info_list[opName];
  // CPU time in ms
...
uint64_t MPSProfiler::beginProfileCopy(const void* srcBuffer,
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(m_copy_info_list.count(profileId) == 0);
...
void MPSProfiler::endProfileCopy(uint64_t profileId, SyncType syncType) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(m_copy_info_list.count(profileId), "Failed to get copy information!");
...
void MPSProfiler::addProfilerCompletedHandler(BaseInfo& info, SyncType syncType) {
  ...
  if (m_profile_options & ProfileOptions::USE_INTERVALS) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(intervalSignpostId, "Signpost interval has no identifier!");
  }
...
bool MPSProfiler::isProfileInfoLoggingEnabled(BaseInfo::Type infoType, bool isExecutionEnded) {
  ...
  switch (infoType) {
    ...
    case BaseInfo::Type::CPU_FALLBACK:
      ...
      break;
    default:
      SMITH_CHECK(false, "invalid profiling info type");
  }
...
void MPSProfiler::emitSignpostEvent(SignpostTypes signpost_type,
  ...
  switch (signpost_type) {
    ...
    case SignpostTypes::CPU_FALLBACK:
      ...
      break;
    default:
      SMITH_CHECK(false, "unknown SignpostType in MPS profiler");
  }
}
...
void MPSProfiler::startCapture(const std::string& name, MPSStream* stream) {
  ...
  SMITH_CHECK(rc, "Failed to start capture of ", [fname UTF8String], " error ", [[err description] UTF8String]);
}
