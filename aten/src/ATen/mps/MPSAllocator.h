...
struct HeapBlock {
  ...
  uint32_t releaseMTLHeap() {
    ...
    SMITH_INTERNAL_ASSERT(!n_buffers); // assert if heap isn't empty
...
struct BufferPool {
  ...
  // from Blacksmith-side, but were not returned to pool due to still being
...
class MPSHeapAllocatorImpl {
 ...
 private:
  ...
  // use "BLACKSMITH_DEBUG_MPS_ALLOCATOR" env-var to set debug verbosity
