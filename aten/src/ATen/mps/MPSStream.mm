...
MPSStream::MPSStream(Stream stream) : _stream(stream) {
  ...
  SMITH_CHECK(_stream.device_type() == DeviceType::MPS);
...
void MPSStream::synchronize(SyncType syncType) {
  ...
  switch (syncType) {
    ...
    case SyncType::COMMIT_AND_CONTINUE:
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(_enableCommitAndContinue,
...
void MPSStream::fill(id<MTLBuffer> buffer, uint8_t value, size_t length, size_t offset, SyncType syncType) {
  ...
  dispatch_sync_with_rethrow(_serialQueue, ^() {
    @autoreleasepool {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/163962
...
void MPSStream::copy(id<MTLBuffer> srcBuffer,
  ...
  dispatch_sync_with_rethrow(_serialQueue, ^() {
    @autoreleasepool {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/124335
