...
struct CopyInfo : BaseInfo {
  ...
  static bool isStorageOnMPS(
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(buffer);
  ...
  static Kind getCopyKind(
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(isSrcOnMPS || isDstOnMPS);
...
class MPSProfiler {
 public:
  ...
  enum SignpostTypes : uint32_t {
    SIGNPOST_NONE = 0,
    // trace signposts for Blacksmith operation executions
    RUN_OPERATION = (1 << 16),
  ...
  enum LogOptions : uint32_t {
    ...
    // with the EV "BLACKSMITH_DEBUG_MPS_ALLOCATOR") to identify buffers that are
  ...
  // these are used to hook into Python bindings for smith.mps.profiler module.
