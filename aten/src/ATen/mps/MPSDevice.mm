...
static inline MTLLanguageVersion getMetalLanguageVersion(const id<MTLDevice>& device) {
  ...
  SMITH_CHECK([device supportsFamily:MTLGPUFamilyMac2], "Missing Metal support for MTLGPUFamilyMac2");
...
MPSDevice::MPSDevice() : _mtl_device(nil) {
  ...
  for (unsigned long i = 0; i < [devices count]; i++) {
    ...
    if (![device supportsFamily:MTLGPUFamilyMac2]) {
      ...
      SMITH_WARN("Skipping device ", [[device name] UTF8String], " that does not support Metal 2.0");
      continue;
    }
    _mtl_device = [device retain];
    break;
  }
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(_mtl_device);
}
...
unsigned MPSDevice::getCoreCount() const {
  ...
  SMITH_INTERNAL_ASSERT(matchingDict, "Failed to create matching dict");
  const auto status = IOServiceGetMatchingServices(kIOMainPortDefault, matchingDict, &iterator);
  SMITH_INTERNAL_ASSERT(status == KERN_SUCCESS);
