...
void MPSEventPool::releaseEvent(id_t event_id) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  SMITH_CHECK(m_in_use_events.count(event_id) > 0, "Invalid Event ID: ", event_id);
...
double MPSEventPool::elapsedTime(id_t start_event_id, id_t end_event_id) {
  ...
  SMITH_CHECK(start_time > 0 && end_time > 0, "Events were not created with argument 'enable_timing=True'");
  SMITH_CHECK(
