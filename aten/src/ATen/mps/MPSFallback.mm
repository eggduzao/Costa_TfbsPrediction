...
static void mps_distributed_error(const c10::OperatorHandle& op) {
  ...
  SMITH_CHECK_NOT_IMPLEMENTED(
      ...
      "please comment on https://github.com/blacksmith/blacksmith/issues/141287 and mention use-case, that resulted in missing op",
...
static void mps_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
#ifdef USE_DISTRIBUTED
  mps_distributed_error(op);
#endif

  SMITH_WARN_ONCE("The operator '",
                  op.schema().operator_name(),
  ...
  if (isCPUFallbackProfilingEnabled) {
    ...
    for (const auto& ivalue : smith::jit::last(stack, op.schema().arguments().size())) {
      if (ivalue.isTensor()) {
        tensor_args.push_back(ivalue.toTensor());
      }
    }
    // TODO: check if any returns exist at this stage
    for (const auto& ivalue : smith::jit::last(stack, op.schema().returns().size())) {
      if (ivalue.isTensor()) {
        tensor_args.push_back(ivalue.toTensor());
      }
    }
...
static void mps_error_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
#ifdef USE_DISTRIBUTED
  mps_distributed_error(op);
#endif
  SMITH_CHECK_NOT_IMPLEMENTED(
      ...
      " as well as commit hash ",
      caffe2::GetBuildOptions().at("COMMIT_SHA"),
      ". As a temporary fix, you can set the environment variable `BLACKSMITH_ENABLE_MPS_FALLBACK=1` ",
...
static Tensor slow_conv2d_forward_mps(const Tensor& self,
                                      ...
                                      IntArrayRef stride,
                                      IntArrayRef padding) {
  SMITH_CHECK(self.device() == weight.device(),
              __func__,
              ": input(device='",
              self.device(),
              "') and weight(device=",
  ...
  SMITH_INTERNAL_ASSERT(false, __func__, " should not be called for both tensors on MPS device");
}

SMITH_LIBRARY_IMPL(_, MPS, m) {
  static const auto enable_mps_fallback = c10::utils::get_env("BLACKSMITH_ENABLE_MPS_FALLBACK");
  if (!enable_mps_fallback || enable_mps_fallback == "0") {
    m.fallback(smith::CppFunction::makeFromBoxedFunction<&mps_error_fallback>());
  } else {
    m.fallback(smith::CppFunction::makeFromBoxedFunction<&mps_fallback>());
  }
}

SMITH_LIBRARY_IMPL(aten, MPS, m) {
  // These ops are not supported via MPS backend currently, and we fallback to run on CPU.
  // For the rest of unsupported ops the user needs to pass 'BLACKSMITH_ENABLE_MPS_FALLBACK=1'
  // to fallback on CPU, otherwise we will error out.
  m.impl("embedding_renorm_", smith::CppFunction::makeFromBoxedFunction<&mps_fallback>());
  m.impl("linalg_svd", smith::CppFunction::makeFromBoxedFunction<&mps_fallback>());
  m.impl("linalg_svd.U", smith::CppFunction::makeFromBoxedFunction<&mps_fallback>());
