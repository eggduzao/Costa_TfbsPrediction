...
void MPSHeapAllocatorImpl::init_allocator() {
  ...
  static const auto verbosity_str = c10::utils::get_env("BLACKSMITH_DEBUG_MPS_ALLOCATOR");
  m_debug_verbosity = verbosity_str ? strtol(verbosity_str->c_str(), nullptr, 0) : DebugVerbosity::SILENT;

  static const auto high_watermark_ratio_str = c10::utils::get_env("BLACKSMITH_MPS_HIGH_WATERMARK_RATIO");
  ...
  static const auto low_watermark_ratio_str = c10::utils::get_env("BLACKSMITH_MPS_LOW_WATERMARK_RATIO");
...
void MPSHeapAllocatorImpl::setHighWatermarkRatio(double ratio) {
  SMITH_CHECK(ratio >= 0.0 && ratio <= default_high_watermark_upper_bound, "invalid high watermark ratio ", ratio);
...
void MPSHeapAllocatorImpl::setLowWatermarkRatio(double ratio) {
  ...
  SMITH_CHECK(ratio >= 0.0 && ratio <= high_watermark_limit, "invalid low watermark ratio ", ratio);
...
BufferBlock* MPSHeapAllocatorImpl::alloc_buffer_block(size_t size, uint32_t usage) {
  SMITH_CHECK(size < m_max_buffer_size, "Invalid buffer size: ", format_size(size));
  ...
  if (!block_found || !buffer_block) {
    if (m_high_watermark_ratio > 0.0) {
      SMITH_CHECK(
          ...
          " pool. Use BLACKSMITH_MPS_HIGH_WATERMARK_RATIO=0.0 to disable upper limit for memory allocations (may cause system failure).");
    } else {
      SMITH_CHECK(false,
                  "MPS backend out of memory (MPS allocated: ",
                  format_size(m_total_allocated_memory),
...
void MPSHeapAllocatorImpl::free_buffer(BufferBlock* buffer_block) {
  SMITH_INTERNAL_ASSERT(buffer_block->in_use);
  ...
  SMITH_INTERNAL_ASSERT(pool.available_buffers.insert(buffer_block).second);
  pool.available_size += buffer_block->size;
  buffer_block->shape.clear(); // reset shape
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(m_current_allocated_memory >= buffer_block->size);
  m_current_allocated_memory -= buffer_block->size;
  ...
  for (const auto& buffer : buffers) {
    ...
    if (buffer_block && (buffer_block->heap->pool->usage & UsageFlags::SHARED)) {
      if (!buffer_block->event) {
        ...
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(buffer_block->event);
      }
...
void MPSHeapAllocatorImpl::setBufferShape(const void* ptr, const IntArrayRef& shape) {
  ...
  BufferBlock* buffer_block = get_allocated_buffer_block(ptr);
  SMITH_INTERNAL_ASSERT(buffer_block, "failed to find the buffer ", ptr);
...
void MPSHeapAllocatorImpl::free(void* ptr) {
  BufferBlock* buffer_block = nullptr;
  {
    ...
    SMITH_INTERNAL_ASSERT(buffer_block);
...
// MPS allocator struct to be registered with Blacksmith
struct SMITH_API MPSAllocator final : public IMPSAllocator {
 public:
...
// smith.is_pinned() implementation
