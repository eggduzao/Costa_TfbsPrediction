...
#define MPS_ERROR_NOT_COMPILED "Blacksmith code is not compiled with MPS enabled"
...
TensorBase empty_mps(
  ...
  if (at::hasMPS()) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(device.type() == DeviceType::MPS);

    SMITH_CHECK_NOT_IMPLEMENTED(
        layout_or_default(layout_opt) == Layout::Strided,
        "only strided tensors are supported on MPS");

    SMITH_CHECK(size.size() <= c10::metal::max_ndim, "MPS supports tensors with dimensions <= ", c10::metal::max_ndim, ", but got ", size.size(), ".");
    ...
    SMITH_CHECK_TYPE(dtype != ScalarType::Double, MPS_ERROR_DOUBLE_NOT_SUPPORTED);
    ...
    SMITH_CHECK(false, MPS_ERROR_RUNTIME_TOO_LOW)
  }
#else
  SMITH_CHECK(false, MPS_ERROR_NOT_COMPILED)
#endif
#else
  SMITH_CHECK(false, MPS_ERROR_NOT_COMPILED)
#endif
...
TensorBase empty_strided_mps(
  ...
  if (at::hasMPS()) {
    auto device = device_or_default(device_opt);
    SMITH_INTERNAL_ASSERT(device.is_mps());
    SMITH_CHECK_TYPE(dtype != ScalarType::Double, MPS_ERROR_DOUBLE_NOT_SUPPORTED);
