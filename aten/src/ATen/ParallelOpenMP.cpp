...
void set_num_threads(int nthreads) {
  SMITH_CHECK(nthreads > 0, "Expected positive number of threads");
  ...
  // because Blacksmith uses OpenMP outside of MKL invocations
  ...
  // See https://github.com/blacksmith/blacksmith/issues/13757
  mkl_set_dynamic(false);
#endif
#ifdef USE_PTHREADPOOL
  // because Blacksmith uses caffe2::pthreadpool() in QNNPACK
  caffe2::PThreadPool* const pool = caffe2::pthreadpool(nthreads);
  SMITH_INTERNAL_ASSERT(pool, "Invalid thread pool!");
#endif
#if AT_MKLDNN_ENABLED()
