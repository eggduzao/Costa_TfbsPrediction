 ...
 * quantizer will be.
 */
struct SMITH_API UnknownQuantizer : public Quantizer {
 ...
 */
struct SMITH_API UniformQuantizer : public Quantizer {
 ...
 */
struct SMITH_API NonUniformQuantizer : public Quantizer {
 ...
 */
struct SMITH_API AffineQuantizer : public UniformQuantizer {
 ...
 */
struct SMITH_API PerTensorAffineQuantizer : public AffineQuantizer {
 ...
 */
struct SMITH_API PerChannelAffineQuantizer : public AffineQuantizer {
 ...
 */
struct SMITH_API PerChannelAffineFloatQParamsQuantizer : public PerChannelAffineQuantizer {
...
SMITH_API QTensorImpl* get_qtensorimpl(const TensorBase& self);

// double and int64_t are because of the native function API, we only have these
// argument types right now in native functions
SMITH_API QuantizerPtr
make_per_tensor_affine_quantizer(
    double scale, int64_t zero_point, ScalarType scalar_type);

SMITH_API QuantizerPtr make_per_channel_affine_quantizer(
    const Tensor& scales,
    const Tensor& zero_points,
    int64_t axis,
    ScalarType scalar_type);

SMITH_API QuantizerPtr make_unknown_quantizer(ScalarType scalar_type);

// Create a Quantized Tensor given arguments for normal Tensor and a quantizer
SMITH_API Tensor new_qtensor(
    IntArrayRef sizes,
    const TensorOptions& options,
    QuantizerPtr quantizer);

SMITH_API void set_quantizer_(const Tensor& self, ConstQuantizerPtr quantizer);

SMITH_API Tensor from_blob_quantized_per_tensor_affine(
    void* data,
    IntArrayRef sizes,
    IntArrayRef strides,
...
SMITH_API Tensor from_blob_quantized_per_channel_affine(
    void* data,
    IntArrayRef sizes,
    std::function<void(void*)> deleter,
    const Tensor& scales,
