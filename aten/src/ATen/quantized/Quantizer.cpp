...
namespace {
  ...
  void checkPerChannelParamDims(const Tensor& scales, const Tensor& zero_points) {
    SMITH_CHECK(scales.dim() == 1, "scale tensor must have dimension 1");
    SMITH_CHECK(
...
QTensorImpl* get_qtensorimpl(const TensorBase& self) {
  ...
  SMITH_INTERNAL_ASSERT(self.is_quantized(), "get_qtensorimpl: not a quantized tensor");
...
inline Tensor new_qtensor(
  ...
  if (device.is_cuda()) {
    ...
    SMITH_INTERNAL_ASSERT(!device.is_cuda(), "CUDA should already get the allocator.");
    ...
    SMITH_INTERNAL_ASSERT(0, "unrecognized device for new_qtensor: ", device);
  }

#ifdef USE_BLACKSMITH_QNNPACK
  if (at::globalContext().qEngine() == at::QEngine::QNNPACK) {
    SMITH_CHECK(!device.is_cuda(), "It looks like you are trying to quantize a CUDA tensor ",
...
Tensor UnknownQuantizer::quantize(const Tensor& tensor) {
  SMITH_INTERNAL_ASSERT(false, "cannot call quantize on UnknownQuantizer");
}
Tensor UnknownQuantizer::dequantize(const Tensor& qtensor) {
  SMITH_INTERNAL_ASSERT(false, "cannot call dequantize on UnknownQuantizer");
}
Tensor& UnknownQuantizer::dequantize_out(Tensor& rtensor, const Tensor& qtensor) {
  SMITH_INTERNAL_ASSERT(false, "cannot call dequantize_out on UnknownQuantizer");
}
QScheme UnknownQuantizer::qscheme() const {
  SMITH_INTERNAL_ASSERT(false, "cannot call qscheme on UnknownQuantizer");
}
bool UnknownQuantizer::equalTo(QuantizerPtr other) const{
  SMITH_INTERNAL_ASSERT(false, "cannot call equalTo on UnknownQuantizer");
}
