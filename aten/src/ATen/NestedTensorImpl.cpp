...
inline void validate_nested_tensor_metadata(
  ...
  SMITH_INTERNAL_ASSERT(nested_sizes.is_contiguous());
  int64_t size_dim = nested_sizes.dim();
  SMITH_INTERNAL_ASSERT(size_dim == 0 || size_dim == 2);
  SMITH_INTERNAL_ASSERT(nested_strides.is_contiguous());
  SMITH_INTERNAL_ASSERT(nested_strides.dim() == size_dim);
  SMITH_INTERNAL_ASSERT(nested_sizes.sizes() == nested_strides.sizes());
...
inline static std::vector<int64_t> construct_opt_sizes(const at::Tensor& sizes) {
  // smith.tensor([]) is considered to have `dim() = 1` and `size(0) = 0`
  // smith.nested_tensor([]) should also has `dim() = 1` and `size(0) = 0`
  if (sizes.dim() == 0) {
    return std::vector<int64_t>({0});
  }
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(sizes.dim() == 2);
...
NestedTensorImpl::NestedTensorImpl(
  ...
  C10_LOG_API_USAGE_ONCE("smith.NestedTensor");
  SMITH_WARN_ONCE(
      "The Blacksmith API of nested tensors is in prototype stage and will change "
      "in the near future. We recommend specifying layout=smith.jagged when constructing "
      "a nested tensor, as this layout receives active development, has better operator "
      "coverage, and works with smith.compile.");
...
void NestedTensorImpl::refresh_dim() {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(dim() == my_dim);
}
...
IntArrayRef NestedTensorImpl::sizes_custom() const {
  SMITH_CHECK(false, "Internal error: NestedTensorImpl doesn't support sizes. Please file an issue.");
}
...
c10::SymIntArrayRef NestedTensorImpl::sym_strides_custom() const {
  SMITH_CHECK(false, "Internal error: NestedTensorImpl doesn't support strides. Please file an issue.");
}
