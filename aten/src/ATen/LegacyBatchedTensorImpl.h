...
SMITH_API Tensor makeBatched(Tensor tensor, BatchDims bdims);

// Adds a batch dim to `tensor`, returning a BatchedTensor
SMITH_API Tensor addBatchDim(Tensor tensor, int64_t level, int64_t dim);
...
SMITH_API bool inplaceIsVmapCompatible(const Tensor& self, const Tensor& other);
