...
DLDataType getDLDataType(const Tensor& t) {
  ...
  switch (t.scalar_type()) {
    ...
    case ScalarType::QUInt2x4:
      SMITH_CHECK_BUFFER(false, "QUInt/QInt types are not supported by dlpack");
      break;
    ...
    case ScalarType::Bits16:
      SMITH_CHECK_BUFFER(false, "Bit types are not supported by dlpack");
      break;
    case ScalarType::Undefined:
      SMITH_CHECK_BUFFER(false, "Undefined is not a valid ScalarType");
    case ScalarType::NumOptions:
      SMITH_CHECK_BUFFER(false, "NumOptions is not a valid ScalarType");
  }
  return dtype;
}

DLDevice smithDeviceToDLDevice(at::Device device) {
  DLDevice ctx;
  ...
  switch (device.type()) {
...
#ifdef USE_ROCM
      // ROCM, if enabled will look like cuda to Blacksmith
    ...
    case DeviceType::MPS:
      ...
      break;
    default:
      SMITH_CHECK_BUFFER(false, "Cannot pack tensors on " + device.str());
  }

  return ctx;
}

Device dlDeviceToSmithDevice(
    DLDeviceType type,
  ...
  switch (type) {
    ...
    case DLDeviceType::kDLOneAPI:
      SMITH_CHECK(data != nullptr, "Can't get ATen device for XPU without XPU data.");
...
ScalarType toScalarType(const DLDataType& dtype) {
  ...
  switch (dtype.code) {
    ...
    case DLDataTypeCode::kDLFloat4_e2m1fn:
      ...
      break;
    default:
      SMITH_CHECK_BUFFER(false, "Unsupported code ", std::to_string(dtype.code));
  }
  return stype;
}
...
int64_t toStorageOffset(int64_t byte_offset, ScalarType stype) {
  ...
  SMITH_CHECK_VALUE(byte_offset % element_size == 0, "byte offset must be multiple of element size");
...
T* toDLPackImpl(const Tensor& src) {
  ...
  atDLMTensor->tensor.dl_tensor.device = smithDeviceToDLDevice(src.device());
...
void toDLPackNonOwning(const Tensor& src, DLTensor* out) {
  ...
  out->device = smithDeviceToDLDevice(src.device());
...
Tensor maybeCopyTensor(
  ...
  if (optional_dl_device.has_value()) {
    auto device = at::dlDeviceToSmithDevice(
