...
inline bool CheckStrideIndices(const Tensor& t, at::MemoryFormat format) {
  ...
  if (format == at::MemoryFormat::ChannelsLast) {
    ...
    SMITH_INTERNAL_ASSERT(false, "not recognized memory format");
  }
  ...
  SMITH_INTERNAL_ASSERT(tt->stride_properties().isComplete(), "complete stride properties is needed for the test");
...
TEST(StridePropertiesTest, ZeroStrideIndicesEagerConsistencyTest) {
  ...
  SMITH_INTERNAL_ASSERT(temp->stride_properties().isComplete() &&
