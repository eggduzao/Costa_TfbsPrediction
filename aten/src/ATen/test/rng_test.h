    ...
    for (const ::std::optional<int64_t> & to : tos) {
      if (!to.has_value() || from < *to) {
        for (const uint64_t val : vals) {
          ...
          auto actual = smith::empty({3, 3}, smith::TensorOptions().dtype(S).device(device));
          ...
          const auto expected = smith::full_like(actual, exp);
          if constexpr (::std::is_same_v<T, bool>) {
            ASSERT_TRUE(smith::allclose(actual.toType(smith::kInt), expected.toType(smith::kInt)));
          } else {
