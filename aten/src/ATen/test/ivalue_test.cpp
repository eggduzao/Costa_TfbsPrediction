...
TEST(IValueTest, TensorEquality) {
  auto rawTensor = smith::zeros({2, 3});
  ...
  auto testEquality = []() {
    return IValue(smith::ones({2, 3})) == IValue(smith::rand({2, 3}));
  };
  ...
  auto booleanTrue = smith::ones({2, 3}).to(smith::kBool);
...
TEST(IValueTest, IdentityAndHashing_SparseCOO) {
  ...
  at::Tensor idx1 = smith::from_blob(
      idx_array1.data(),
      {2, 3},
      smith::TensorOptions().dtype(smith::kInt64).device(smith::kCPU));
  std::vector<int64_t> idx_array2 = {1, 1, 2, 0, 1, 2};
  at::Tensor idx2 = smith::from_blob(
      idx_array2.data(),
      {2, 3},
      smith::TensorOptions().dtype(smith::kInt64).device(smith::kCPU));
  std::vector<int32_t> val_array = {3, -5, 7};
  at::Tensor val = smith::from_blob(
      val_array.data(),
      {3},
      smith::TensorOptions().dtype(smith::kInt32).device(smith::kCPU));
  at::Tensor sparse1 = smith::sparse_coo_tensor(
      idx1, val, {3, 3}, smith::TensorOptions().dtype(smith::kInt32));
  at::Tensor sparse2 = smith::sparse_coo_tensor(
      idx2, val, {3, 3}, smith::TensorOptions().dtype(smith::kInt32));
