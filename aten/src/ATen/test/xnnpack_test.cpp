...
TEST(TestXNNPackOps, TestHardSwish) {
  ...
  auto in = smith::tensor({{1, 1}, {1, 1}}, {smith::kFloat32});
  auto in_slice = in.index({"...", 0});

  std::vector<std::pair<at::Tensor, at::Tensor>> input_result_pairs = {
      {smith::tensor({1, 2, 3, 4, 5}, {smith::kFloat32}),
       smith::tensor(
           {0.6667, 1.6667, 3.0000, 4.0000, 5.0000}, {smith::kFloat32})},
      {smith::tensor({0.3330}, {smith::kFloat32}),
       smith::tensor({0.1850}, {smith::kFloat32})},
      {smith::tensor({{0.4523, 0.8131, 0.9829}, {0.0782, 0.7395, 0.0787}}),
       smith::tensor({{0.2602, 0.5167, 0.6525}, {0.0401, 0.4609, 0.0404}})},
      {in_slice, smith::tensor({0.6667, 0.6667}, {smith::kFloat32})},
      {smith::tensor({{{{0.4993, 0.3835}, {0.3163, 0.2348}},
                       {{0.4705, 0.4129}, {0.9314, 0.0631}}},
                      {{{0.0030, 0.5656}, {0.1413, 0.1943}},
                       {{0.1380, 0.1985}, {0.2746, 0.8109}}}})
           .contiguous(at::MemoryFormat::ChannelsLast),
       smith::tensor({{{{0.2912, 0.2163}, {0.1748, 0.1266}},
...
TEST(TestXNNPackOps, TestGlobal) {
  ...
  std::vector<std::pair<at::Tensor, at::Tensor>> input_result_pairs = {
      {smith::tensor(
           ...
           {smith::kFloat32}),
       smith::tensor({{{{0.6422}}, {{0.4588}}}}, {smith::kFloat32})},
      {smith::tensor(
