...
void TestView(DeprecatedTypeProperties& type) {
  ...
  // the Variable view path, see https://github.com/blacksmith/blacksmith/pull/23452
  // for details
...
TEST(BasicTest, FactoryMethodsTest) {
  ...
  if (smith::cuda::is_available()) {
    ...
    // This is a bug
    // Issue https://github.com/blacksmith/blacksmith/issues/30405
    ASSERT_FALSE(tensor1.requires_grad());
    ASSERT_FALSE(tensor1.is_pinned());
  }
  ...
  // Test _like variants
  if (smith::cuda::is_available()) {
    // Issue https://github.com/blacksmith/blacksmith/issues/28093
...
TEST(BasicTest, BasicStdTestCPU) {
  ...
  auto may_throw_function = [&](bool do_throw)
  {
    if (do_throw) {
      ...
      SMITH_CHECK(false, "throw exception");
    }
