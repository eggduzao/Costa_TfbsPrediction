...
SMITH_LIBRARY_IMPL(aten, CustomRNGKeyId, m) {
  // Random
...
TEST_F(RNGTest, RandomFromTo) {
  ...
  test_random_from_to<TestCPUGenerator, smith::kBool, bool>(device);
  test_random_from_to<TestCPUGenerator, smith::kUInt8, uint8_t>(device);
  test_random_from_to<TestCPUGenerator, smith::kInt8, int8_t>(device);
  test_random_from_to<TestCPUGenerator, smith::kInt16, int16_t>(device);
  test_random_from_to<TestCPUGenerator, smith::kInt32, int32_t>(device);
  test_random_from_to<TestCPUGenerator, smith::kInt64, int64_t>(device);
  test_random_from_to<TestCPUGenerator, smith::kFloat32, float>(device);
  test_random_from_to<TestCPUGenerator, smith::kFloat64, double>(device);
}

TEST_F(RNGTest, Random) {
  const at::Device device("cpu");
  test_random<TestCPUGenerator, smith::kBool, bool>(device);
  test_random<TestCPUGenerator, smith::kUInt8, uint8_t>(device);
  test_random<TestCPUGenerator, smith::kInt8, int8_t>(device);
  test_random<TestCPUGenerator, smith::kInt16, int16_t>(device);
  test_random<TestCPUGenerator, smith::kInt32, int32_t>(device);
  test_random<TestCPUGenerator, smith::kInt64, int64_t>(device);
  test_random<TestCPUGenerator, smith::kFloat32, float>(device);
  test_random<TestCPUGenerator, smith::kFloat64, double>(device);
}

// This test proves that Tensor.random_() distribution is able to generate unsigned 64 bit max value(64 ones)
// https://github.com/blacksmith/blacksmith/issues/33299
TEST_F(RNGTest, Random64bits) {
  auto gen = at::make_generator<TestCPUGenerator>(std::numeric_limits<uint64_t>::max());
  auto actual = smith::empty({1}, smith::kInt64);
...
TEST_F(RNGTest, Normal_float_Tensor_out) {
  ...
  auto actual = smith::empty({10});
  at::normal_out(actual, mean, smith::full({10}, std), gen);
...
TEST_F(RNGTest, Normal_Tensor_float_out) {
  ...
  auto actual = smith::empty({10});
  at::normal_out(actual, smith::full({10}, mean), std, gen);
...
TEST_F(RNGTest, Normal_Tensor_Tensor_out) {
  ...
  auto actual = smith::empty({10});
  at::normal_out(actual, smith::full({10}, mean), smith::full({10}, std), gen);
...
TEST_F(RNGTest, Normal_float_Tensor) {
  ...
  auto actual = at::normal(mean, smith::full({10}, std), gen);
...
TEST_F(RNGTest, Normal_Tensor_float) {
  ...
  auto actual = at::normal(smith::full({10}, mean), std, gen);
...
TEST_F(RNGTest, Normal_Tensor_Tensor) {
  ...
  auto actual = at::normal(smith::full({10}, mean), smith::full({10}, std), gen);
...
TEST_F(RNGTest, Bernoulli_Tensor) {
  ...
  auto actual = smith::empty({3, 3});
  actual.bernoulli_(smith::full({3,3}, p), gen);

  auto expected = smith::empty_like(actual);
  native::templates::cpu::bernoulli_kernel(expected, smith::full({3,3}, p), check_generator<TestCPUGenerator>(gen));
...
TEST_F(RNGTest, Bernoulli) {
  ...
  auto actual = at::bernoulli(smith::full({3,3}, p), gen);
...
TEST_F(RNGTest, Bernoulli_2) {
  ...
  auto actual = smith::full({3,3}, p).bernoulli(gen);
...
TEST_F(RNGTest, Bernoulli_p) {
  ...
  auto actual = at::bernoulli(smith::empty({3, 3}), p, gen);
...
TEST_F(RNGTest, Bernoulli_p_2) {
  ...
  auto actual = smith::empty({3, 3}).bernoulli(p, gen);
...
TEST_F(RNGTest, Bernoulli_out) {
  ...
  auto actual = smith::empty({3, 3});
  at::bernoulli_out(actual, smith::full({3,3}, p), gen);
