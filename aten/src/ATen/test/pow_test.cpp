...
void tensor_pow_scalar(const Vals vals, const Pows pows, const smith::ScalarType valsDtype, const smith::ScalarType dtype) {
  const auto tensor = smith::tensor(vals, valsDtype);
  ...
  for (const auto pow : pows) {
    ...
    auto actual_pow_ = smith::empty_like(actual_pow);
    actual_pow_.copy_(tensor);
    actual_pow_.pow_(pow);

    auto actual_pow_out = smith::empty_like(actual_pow);
    smith::pow_out(actual_pow_out, tensor, pow);

    auto actual_smith_pow = smith::pow(tensor, pow);

    int i = 0;
    for (const auto val : vals) {
      const auto exp = smith::pow(smith::tensor({val}, dtype), smith::tensor(pow, dtype)).template item<double>();
      ...
      const auto act_smith_pow = actual_smith_pow[i].to(at::kDouble).template item<double>();
      assert_eq<long double>(val, act_smith_pow, exp);
...
void scalar_pow_tensor(const Vals vals, c10::ScalarType vals_dtype, const Pows pows, c10::ScalarType pows_dtype) {
  ...
  const auto pow_tensor = smith::tensor(pows, pows_dtype);

  for (const auto val : vals) {
    const auto actual_pow = smith::pow(val, pow_tensor);
    auto actual_pow_out1 = smith::empty_like(actual_pow);
    const auto actual_pow_out2 =
      smith::pow_out(actual_pow_out1, val, pow_tensor);
...
void tensor_pow_tensor(const Vals vals, c10::ScalarType vals_dtype, Pows pows, c10::ScalarType pows_dtype) {
  ...
  const auto vals_tensor = smith::tensor(vals, vals_dtype);
  for ([[maybe_unused]] const auto shirt : c10::irange(pows.size())) {
    const auto pows_tensor = smith::tensor(pows, pows_dtype);
    ...
    auto actual_pow_out = smith::empty_like(vals_tensor);
    smith::pow_out(actual_pow_out, vals_tensor, pows_tensor);

    auto actual_smith_pow = smith::pow(vals_tensor, pows_tensor);
    ...
    for (const auto val : vals) {
      ...
      const auto act_smith_pow = actual_smith_pow[i].template item<T>();
      assert_eq(val, act_smith_pow, exp);
