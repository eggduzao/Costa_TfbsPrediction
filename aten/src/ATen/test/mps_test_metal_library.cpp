...
TEST(MPSTestMetalLibrary, ArangeShader) {
  auto y = smith::arange(10.0, at::device(at::kMPS));
  auto x = smith::empty(10, at::device(at::kMPS));
...
TEST(MPSTestMetalLibrary, ArangeWithArgsShader) {
  ...
  const float step = .4;
  auto x = smith::empty(size, at::device(at::kMPS));
  auto y = smith::arange(start, start + size * step, step, at::device(at::kMPS));
...
TEST(MPSTestMetalLibrary, Arange2DShader) {
  const auto size = 16;
  auto x = smith::empty({size, size}, at::device(at::kMPS));
  DynamicMetalShaderLibrary lib(R"MTL(
...
TEST(MPSTestMetalLibrary, ArgumentBuffers) {
  ...
  for([[maybe_unused]] auto idx: c10::irange(nbuffers)) {
    ibuffers.push_back(smith::rand({size}, at::device(at::kMPS)));
    ibuffers_gpu_ptrs.push_back(get_tensor_gpu_address(ibuffers.back()));
  }
  auto output = smith::empty({size}, at::device(at::kMPS));
  DynamicMetalShaderLibrary lib(R"MTL(
  ...
  auto result = smith::zeros({size}, at::device(at::kMPS));
