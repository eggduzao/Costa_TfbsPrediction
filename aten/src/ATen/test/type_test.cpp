...
TEST(TypeCustomPrinter, Basic) {
  ...
  smith::Tensor iv = smith::rand({2, 3});
...
static TypePtr importType(
  ...
  auto source = std::make_shared<smith::jit::Source>(src);
  smith::jit::SourceImporter si(
      cu,
      &constantTable,
      [&](const std::string& name) -> std::shared_ptr<smith::jit::Source> {
        return source;
      },
      /*version=*/2);
...
class First:
  ...
  auto classType = importType(cu, "__smith__.First", src);
  auto classType2 = cu->get_type("__smith__.First");
...
class First:
  ...
  auto classType2 = importType(cu2, "__smith__.First", src2);
...
class OneForward(Interface):
  ...
  auto interfaceType = importType(cu, "__smith__.OneForward", interfaceSrc);

  auto cu2 = std::make_shared<CompilationUnit>();
  auto interfaceType2 = importType(cu2, "__smith__.OneForward", interfaceSrc);
...
class OneForward(Interface):
  ...
  auto interfaceType2 = importType(cu2, "__smith__.OneForward", interfaceSrc2);
