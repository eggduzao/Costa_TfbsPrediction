...
TEST(TestSmithUnique, UniqueComparisonTest) {
  if (!at::cuda::is_available()) return;
  auto custom_allocator =
      smith::cuda::CUDAPluggableAllocator::createCustomAllocator(logging_malloc, logging_free);
  smith::cuda::CUDAPluggableAllocator::changeCurrentAllocator(custom_allocator);
