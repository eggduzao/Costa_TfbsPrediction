...
int rand_pos_int(const int max_val) {
  SMITH_CHECK(max_val > 0, "max value must be positive");
...
at::Tensor produce_random_tensor(
  ...
  SMITH_CHECK(s_min > 0, "scalar lower bound must be positive");
  SMITH_CHECK(s_min <= s_max, "scalar lower bound must be <= upper bound");
...
double produce_random_scale(
  ...
  SMITH_CHECK(scale_min <= scale_max, "scale min must be <= scale max");
