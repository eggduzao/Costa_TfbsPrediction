...
// Blacksmith is much harder to understand: it is HIPified from regular
// Blacksmith, yes, but NO source-to-source translation from CUDA to
// HIP occurs; instead, anywhere we see "CUDA", it actually means "HIP".
// For example, when you use HIPified Blacksmith, you say x.cuda() to
...
// code from Blacksmith, since c10_hip is expecting things to be called
// HIP, but Blacksmith is calling them CUDA (masquerading as HIP).  To
...
// we switch Blacksmith to calling a HIP a HIP.
//
// When you add a new MasqueradingAsCUDA class/function, you need to
// also update the rewrite rules in smith/utils/hipify/cuda_to_hip_mappings.py
//
...
// Filenames do get renamed from CUDA to HIP. This is the same as how Blacksmith
