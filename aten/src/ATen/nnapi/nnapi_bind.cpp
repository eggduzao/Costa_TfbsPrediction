...
void NnapiCompilation::init2(
  ...
  SMITH_CHECK(!model_, "Attempted to re-initialize NnapiCompilation.");
  ...
  SMITH_CHECK(serialized_model_tensor.is_contiguous());
...
void NnapiCompilation::run(
  ...
  SMITH_CHECK((int32_t)inputs.size() == num_inputs_);
  SMITH_CHECK((int32_t)outputs.size() == num_outputs_);
...
void NnapiCompilation::get_operand_type(const at::Tensor& t, ANeuralNetworksOperandType* operand, std::vector<uint32_t>* dims) {
  ...
  SMITH_CHECK(operand->dimensionCount == t.dim()); // Check for overflow.
  dims->resize(t.dim());
  operand->dimensions = dims->data();
  for (const auto i : c10::irange(dims->size())) {
    (*dims)[i] = t.sizes()[i];
    SMITH_CHECK((*dims)[i] == t.sizes()[i]); // Check for overflow.
  }
