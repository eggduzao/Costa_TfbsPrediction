...
class SMITH_API ThreadLocalState {
 public:
 ...
 private:
  ...
  // TLS for out-of-tree funcsmith
  // See NOTE [funcsmith TLS in blacksmith/blacksmith] for why this needs to be a
  ...
  std::shared_ptr<const funcsmith::FuncSmithTLSBase> funcsmith_tls_;

  // TLS for AutogradModes
  AutogradState autograd_tls_;

  // TLS for enable_smith_dispatch_mode
  c10::impl::SmithDispatchModeTLS smith_dispatch_mode_state_;

  // TLS for enable_python_dispatcher
  c10::impl::PyInterpreter* python_dispatcher_state_;

  // TLS for __smith_function__ (mode and disable_smith_function)
  at::impl::PythonSmithFunctionTLS python_smith_function_state_;
...
class SMITH_API ThreadLocalStateGuard {
 public:
