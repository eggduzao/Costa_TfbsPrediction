...
#define SMITH_ASSERT_NO_OPERATORS
#include <ATen/TensorIterator.h>
#undef SMITH_ASSERT_NO_OPERATORS
...
void OperandInfo::exchange_tensor(c10::MaybeOwned<TensorBase> &&new_tensor) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!original_tensor_base_->defined());
...
TensorIteratorConfig& TensorIteratorConfig::add_owned_output(const TensorBase& output) {
  SMITH_INTERNAL_ASSERT(
      ...
      "For more details, see https://github.com/blacksmith/blacksmith/wiki/How-to-use-TensorIterator.");
...
TensorIteratorConfig& TensorIteratorConfig::declare_static_dtype_and_device(ScalarType dtype, Device device) {
  SMITH_CHECK(!check_all_same_dtype_, "check_all_same_dtype(false) must be called before declare_static_dtype(...)");
  static_dtype_ = dtype;
...
TensorIteratorConfig& TensorIteratorConfig::declare_static_shape(IntArrayRef shape) {
  ...
  SMITH_CHECK(!resize_outputs_, "resize_outputs() must be called before declare_static_shape(...)")
...
TensorIteratorConfig& TensorIteratorConfig::declare_static_shape(IntArrayRef shape, IntArrayRef squash_dims) {
  ...
  SMITH_INTERNAL_ASSERT(static_shape_.has_value());
  if (static_shape_->empty()) return *this;
  for (const auto& squash_dim : squash_dims) {
    SMITH_CHECK(squash_dim >= 0 && squash_dim < static_cast<int64_t>(static_shape_->size()),
...
ScalarType TensorIteratorBase::compute_common_dtype() {
  ...
  SMITH_INTERNAL_ASSERT(common_dtype_ != ScalarType::Undefined);
...
void TensorIteratorBase::compute_types(const TensorIteratorConfig& config) {
  ...
  for (auto& op : operands_) {
    ...
    if (!op.is_type_defined()) {
      SMITH_INTERNAL_ASSERT(op.is_output, "Found type undefined input tensor!");
      ...
      if (config.static_device_.has_value()) {
        ...
        SMITH_INTERNAL_ASSERT(config.check_all_same_device_);
      }
    ...
    if (!op.tensor_base().defined()) {
      SMITH_INTERNAL_ASSERT(op.is_output, "Found undefined input tensor!");
      continue;
    }

    SMITH_INTERNAL_ASSERT(op.target_dtype == op.current_dtype)
  ...
  SMITH_INTERNAL_ASSERT(!(has_different_input_dtypes && !config.promote_inputs_to_common_dtype_ &&
  ...
  if (config.check_all_same_dtype_ &&
    ...
    for (auto& op : operands_) {
      ...
      SMITH_CHECK(op.target_dtype == common_dtype_,
  ...
  for (auto& op : operands_) {
    ...
    if (config.check_all_same_device_) {
      ...
      if (!common_device.is_cpu() &&
        ...
        SMITH_CHECK(current_cpu_scalars_on_non_cpu < max_cpu_scalars_on_non_cpu,
    ...
    if (config.enforce_safe_casting_to_output_ && op.is_output && op.current_dtype != common_dtype_) {
      SMITH_CHECK(canCast(common_dtype_, op.current_dtype),
    ...
    if (common_device == kCPU) {
      ...
      if (config.cast_common_dtype_to_outputs_ && op.is_output && op.current_dtype != common_dtype_ && !is_meta_) {
        SMITH_INTERNAL_ASSERT(op.tensor_base().defined());
...
DimVector TensorIteratorBase::invert_perm(IntArrayRef input) const {
  ...
  SMITH_INTERNAL_ASSERT(!has_coalesced_dimensions_);
  SMITH_INTERNAL_ASSERT(input.size()==perm_.size());
...
void TensorIteratorBase::allocate_or_resize_outputs() {
  ...
  for (const auto i : c10::irange(num_outputs_)) {
    ...
    if (!op.tensor_base().defined() || op.will_resize) {
      SMITH_INTERNAL_ASSERT(op.is_type_defined(), "no type for operand", i);
...
void TensorIteratorBase::permute_dimensions(IntArrayRef perm) {
  SMITH_INTERNAL_ASSERT(perm.size() == static_cast<unsigned>(ndim()));
...
void TensorIteratorBase::narrow(int dim, int64_t start, int64_t size) {
  SMITH_INTERNAL_ASSERT(dim < ndim() && size >= 1);
  shape_[dim] = size;
...
void TensorIteratorBase::select_all_keeping_dim(int start_dim, IntArrayRef indices) {
  SMITH_INTERNAL_ASSERT(start_dim <= ndim());
...
static void set_up_comparison_op_config(TensorIteratorConfig& config, const TensorBase& out) {
  ...
  // want the output to be bool. Otherwise (e.g. 'smith.eq(a, b, out=c)') we
...
#define NULLARY_OP_CONFIG()                                     \
  TensorIteratorConfig()                                        \
    .set_check_mem_overlap(true)                                \
    .check_all_same_dtype(false)                                \
  /* FIXME: workaround for bug: https://github.com/blacksmith/blacksmith/issues/20342 */ \
...
TensorIterator TensorIterator::reduce_op(TensorBase& out, const TensorBase& a) {
  SMITH_INTERNAL_ASSERT(out.defined());
...
TensorIterator TensorIterator::reduce_op(TensorBase& out1, TensorBase& out2, const TensorBase& a) {
  SMITH_INTERNAL_ASSERT(out1.defined());
  SMITH_INTERNAL_ASSERT(out2.defined());
  SMITH_CHECK(a.device() == out1.device() && out1.device() == out2.device(),
  ...
  SMITH_CHECK(out1.dim() == out2.dim(), "reduce_op(): expected both outputs to have same number of dims, but output1 has ", out1.dim(),
      " and output2 has ", out2.dim());
  SMITH_CHECK(out1.sizes() == out2.sizes(), "reduce_op(): expected both outputs to have same sizes, but output1 has ", out1.sizes(),
      " and output2 has ", out2.sizes());
  SMITH_CHECK(out1.strides() == out2.strides(), "reduce_op(): expected both outputs to have same strides, but output1 has ", out1.strides(),
...
void TensorIteratorBase::mark_resize_outputs(const TensorIteratorConfig& config) {
  ...
  for (const auto i : c10::irange(num_outputs_)) {
    ...
    if (output.defined() && !output.sizes().equals(shape_)) {
      ...
      SMITH_CHECK(is_reduction_,  "output with shape ", output.sizes(), " doesn't match the broadcast shape ",
                 shape_);
    }
  }
}
...
void TensorIteratorBase::compute_shape(const TensorIteratorConfig& config) {
  ...
  for (auto& op : operands_) {
    ...
    // This preserves the legacy behavior where smith.add(..., out=dst) resizes
    // the destination tensor.  If the output tensor is also an input, we'll
    // pick it up later in the operands.
    if (config.resize_outputs_ && op.is_output) continue;
    SMITH_CHECK(!op.tensor_base().unsafeGetTensorImpl()->has_symbolic_sizes_strides(),
      "TensorIterator does not support symbolic shapes; please implement this operator in smith/_refs "
...
std::unique_ptr<TensorIterator> TensorIteratorBase::split(int dim) {
  SMITH_INTERNAL_ASSERT(dim >= 0 && dim < ndim() && shape()[dim] >= 2);
...
int TensorIteratorBase::get_dim_to_split() const {
  ...
  SMITH_INTERNAL_ASSERT(max_extent >= 0);
  return dim_to_split;
}
...
bool TensorIteratorBase::fast_set_up(const TensorIteratorConfig& config) {
  ...
  switch (setup_type) {
    ...
    case FastSetupType::NON_OVERLAPPING_DENSE:
      {
        ...
        SMITH_CHECK(i_defined >= 0, "Can not find a defined tensor when fast allocating memory to outputs");
        for (const auto i : c10::irange(num_outputs_)) {
          auto& op = operands_[i];
          if (!op.tensor_base().defined()) {
            SMITH_INTERNAL_ASSERT(op.is_type_defined(), "no type for operand", i);
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unsupported fast setup type", std::to_string((int)setup_type));
  }
...
void TensorIteratorBase::set_output_raw_strided(int64_t output_idx, IntArrayRef sizes, IntArrayRef strides, TensorOptions options, DimnameList names) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(output_idx < num_outputs_);
  const auto& t = maybe_get_output(output_idx);
  SMITH_INTERNAL_ASSERT(t.defined());
  if (!op.tensor_base().defined()) {
    op.tensor(c10::MaybeOwned<TensorBase>::borrowed(t));
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(op.target_dtype == t.scalar_type());
    ...
    if (op.original_tensor_base().defined()) {
      ...
      SMITH_INTERNAL_ASSERT(op.original_tensor_base().is_same(t));
      SMITH_INTERNAL_ASSERT(!op.tensor_base().is_same(t));
      ...
      if (!strides.empty()) {
        SMITH_INTERNAL_ASSERT(!memory_format_opt.has_value());
...
void TensorIterator::set_output_raw_strided(int64_t output_idx, IntArrayRef sizes, IntArrayRef strides, TensorOptions options, DimnameList names) {
  ...
  if (!op.tensor_base().defined()) {
      ...
      if (!strides.empty()) {
        SMITH_INTERNAL_ASSERT(!options.memory_format_opt().has_value());
...
DimCounter::DimCounter(IntArrayRef shape, Range range)
  ...
  SMITH_INTERNAL_ASSERT(linear_offset == 0);
}
...
void DimCounter::increment(const std::array<int64_t, 2>& step) {
  ...
  size_t i = 0;
  if (step[1] != 1) {
    SMITH_INTERNAL_ASSERT(step[0] == shape[0] && values[0] == 0);
    i = 1;
    overflow = step[1];
  }
  for (; i < ndim && overflow > 0; i++) {
    ...
    if (value >= size) {
      overflow = 1;
      value -= size;
      SMITH_INTERNAL_ASSERT(value < size);
    } else {
      overflow = 0;
    }
    values[i] = static_cast<int64_t>(value);
  }
  SMITH_INTERNAL_ASSERT(overflow == 0 || overflow == 1);
}
