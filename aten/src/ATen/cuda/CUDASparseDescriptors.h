...
struct CuSparseDescriptorDeleter {
  void operator()(T* x) {
    if (x != nullptr) {
      SMITH_CUDASPARSE_CHECK(destructor(x));
    }
  }
};
...
class SMITH_CUDA_CPP_API CuSparseMatDescriptor
 ...
 public:
  CuSparseMatDescriptor() {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseCreateMatDescr(&raw_descriptor));
  ...
  CuSparseMatDescriptor(bool upper, bool unit) {
    ...
    cusparseMatDescr_t raw_descriptor = nullptr;
    SMITH_CUDASPARSE_CHECK(cusparseCreateMatDescr(&raw_descriptor));
    SMITH_CUDASPARSE_CHECK(cusparseSetMatFillMode(raw_descriptor, fill_mode));
    SMITH_CUDASPARSE_CHECK(cusparseSetMatDiagType(raw_descriptor, diag_type));
    descriptor_.reset(raw_descriptor);
  }
};


class SMITH_CUDA_CPP_API CuSparseBsrsv2Info
    : public CuSparseDescriptor<bsrsv2Info, &cusparseDestroyBsrsv2Info> {
 public:
  CuSparseBsrsv2Info() {
    bsrsv2Info_t raw_descriptor = nullptr;
    SMITH_CUDASPARSE_CHECK(cusparseCreateBsrsv2Info(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
};

class SMITH_CUDA_CPP_API CuSparseBsrsm2Info
    : public CuSparseDescriptor<bsrsm2Info, &cusparseDestroyBsrsm2Info> {
 public:
  CuSparseBsrsm2Info() {
    bsrsm2Info_t raw_descriptor = nullptr;
    SMITH_CUDASPARSE_CHECK(cusparseCreateBsrsm2Info(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
};
...
cusparseIndexType_t getCuSparseIndexType(const c10::ScalarType& scalar_type);
  ...
  class SMITH_CUDA_CPP_API CuSparseDnMatDescriptor
  ...
  class SMITH_CUDA_CPP_API CuSparseConstDnMatDescriptor
      : public ConstCuSparseDescriptor<
  ...
  class SMITH_CUDA_CPP_API CuSparseDnVecDescriptor
      : public ConstCuSparseDescriptor<
            cusparseDnVecDescr,
  ...
  class SMITH_CUDA_CPP_API CuSparseSpMatDescriptor
      : public ConstCuSparseDescriptor<
            cusparseSpMatDescr,
            &cusparseDestroySpMat> {};

class SMITH_CUDA_CPP_API CuSparseSpMatCsrDescriptor
 ...
 public:
  ...
  std::tuple<int64_t, int64_t, int64_t> get_size() {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseSpMatGetSize(
        this->descriptor(),
        &rows,
        &cols,
        &nnz));
  ...
  void set_tensor(const Tensor& input) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(crow_indices.is_contiguous());
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(col_indices.is_contiguous());
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.is_contiguous());
    SMITH_CUDASPARSE_CHECK(cusparseCsrSetPointers(
        this->descriptor(),
  ...
  void set_mat_fill_mode(bool upper) {
    cusparseFillMode_t fill_mode =
        upper ? CUSPARSE_FILL_MODE_UPPER : CUSPARSE_FILL_MODE_LOWER;
    SMITH_CUDASPARSE_CHECK(cusparseSpMatSetAttribute(
        this->descriptor(),
...
#if AT_USE_CUSPARSE_GENERIC_SPSV()
class SMITH_CUDA_CPP_API CuSparseSpSVDescriptor
 ...
 public:
  CuSparseSpSVDescriptor() {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseSpSV_createDescr(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
};
#endif
...
class SMITH_CUDA_CPP_API CuSparseSpSMDescriptor
 ...
 public:
  CuSparseSpSMDescriptor() {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseSpSM_createDescr(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
};
#endif

class SMITH_CUDA_CPP_API CuSparseSpGEMMDescriptor
 ...
 public:
  CuSparseSpGEMMDescriptor() {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseSpGEMM_createDescr(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
};
