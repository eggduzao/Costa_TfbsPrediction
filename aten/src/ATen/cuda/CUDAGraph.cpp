 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * Q: Why do we need graph capture and launch bindings in Blacksmith?
...
void CUDAGraph::capture_begin(MempoolId_t pool/*=0*/, cudaStreamCaptureMode capture_mode) {
  ...
  SMITH_CHECK(stream != at::cuda::getDefaultCUDAStream(),
  ...
  SMITH_INTERNAL_ASSERT(status == cudaStreamCaptureStatus::cudaStreamCaptureStatusActive);
...
void CUDAGraph::capture_end() {
  ...
  SMITH_CHECK(stream.stream() == capture_stream_.stream(),
  ...
  SMITH_CHECK(graph_ != nullptr, "Invalid capture.");
  ...
  if (numCUDAGraphNodes == 0) {
      SMITH_WARN("The CUDA Graph is empty. This usually means that the graph was ",
...
void CUDAGraph::instantiate() {
  SMITH_CHECK(capture_ended_, "capture_end() must have been called before calling instantiate");

  if (has_graph_exec_) {
    SMITH_CHECK(keep_graph_, "instantiate() is intended to be called by the user only when keep_graph=true");
    AT_CUDA_CHECK(cudaGraphExecDestroy(graph_exec_));
  }
  // In typical graph usage some tensors (e.g. the tensors used for graph IO) are not freed
  // between replays.
  // If Blacksmith compiles and runs with a CUDA 11.4+ toolkit, there's a chance the allocator backend
  // is cudaMallocAsync.
...
void CUDAGraph::replay() {
  SMITH_CHECK(capture_ended_,
              "Called CUDAGraph::replay without a preceding successful capture.");

  if (!has_graph_exec_) {
    SMITH_INTERNAL_ASSERT(keep_graph_);
    instantiate();
  }
...
void CUDAGraph::debug_dump(const std::string& debug_path) {
  if (_cuda_graphs_debug || keep_graph_) {
    SMITH_WARN("DEBUG: calling debug_dump()");
    if (has_graph_) {
      SMITH_WARN("DEBUG: calling cudaGraphDebugDotPrint() with ", debug_path);
    ...
    SMITH_WARN("CUDA Graphs debug not enabled, set with [graph].enable_debug_mode()");
  }
}

cudaGraph_t CUDAGraph::raw_cuda_graph() {
  SMITH_CHECK(keep_graph_, "You cannot access the raw cudaGraph_t instance unless CUDAGraph was initialized with keep_graph=true");
  SMITH_CHECK(has_graph_, "You cannot access the raw cudaGraph_t instance until capture_end() has been called");
  return graph_;
}
