...
#define AT_CUDNN_FRONTEND_CHECK(EXPR, ...)                                                      \
  do {                                                                                          \
    ...
    if (!error_object.is_good()) {                                                              \
      SMITH_CHECK_WITH(CuDNNError, false,                                                       \
...
#define SMITH_CUDABLAS_CHECK(EXPR)                              \
  do {                                                          \
    cublasStatus_t __err = EXPR;                                \
    SMITH_CHECK(__err == CUBLAS_STATUS_SUCCESS,                 \
                "CUDA error: ",                                 \
...
#define SMITH_CUDASPARSE_CHECK(EXPR)                            \
  do {                                                          \
    cusparseStatus_t __err = EXPR;                              \
    SMITH_CHECK(__err == CUSPARSE_STATUS_SUCCESS,               \
                "CUDA error: ",                                 \
...
#else
#define SMITH_CUDSS_CHECK(EXPR) EXPR
#endif
...
constexpr const char* _cusolver_backend_suggestion =            \
  ...
  "`smith.backends.cuda.preferred_linalg_library()` to try "    \
  "linear algebra operators with other supported backends. "    \
  "See https://blacksmith.org/docs/stable/backends.html#smith.backends.cuda.preferred_linalg_library";

// When cuda >= 11.5, cusolver normally finishes execution and sets info array indicating convergence issue.
#define SMITH_CUSOLVER_CHECK(EXPR)                                      \
  do {                                                                  \
  ...
  do {                                                                      \
    ...
    if (__err != CUDA_SUCCESS) {                                            \
      ...
      if (get_error_str_err != CUDA_SUCCESS) {                              \
        SMITH_CHECK(false, "CUDA driver error: unknown error");             \
      } else {                                                              \
        SMITH_CHECK(false, "CUDA driver error: ", err_str);                 \
      }                                                                     \
    }                                                                       \
  } while (0)
...
#define AT_CUDA_DRIVER_CHECK(EXPR)                                                \
  do {                                                                            \
    CUresult __err = EXPR;                                                        \
    if (__err != CUDA_SUCCESS) {                                                  \
      SMITH_CHECK(false, "CUDA driver error: ", static_cast<int>(__err));                   \
    }                                                                             \
  } while (0)
  ...
  do {                                                                                              \
    ...
    if (__err != NVRTC_SUCCESS) {                                                                   \
      if (static_cast<int>(__err) != 7) {                                                           \
        SMITH_CHECK(false, "CUDA NVRTC error: ", at::globalContext().getNVRTC().nvrtcGetErrorString(__err));  \
      } else {                                                                                      \
        SMITH_CHECK(false, "CUDA NVRTC error: NVRTC_ERROR_BUILTIN_OPERATION_FAILURE");                        \
      }                                                                                             \
    }                                                                                               \
  } while (0)
