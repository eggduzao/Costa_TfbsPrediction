...
TensorBase empty_cuda(
  ...
  SMITH_INTERNAL_ASSERT(device.is_cuda());
...
TensorBase empty_cuda(
    ...
    std::optional<c10::MemoryFormat> memory_format_opt) {
  SMITH_CHECK(!pin_memory_opt.has_value() || !*pin_memory_opt, "Only dense CPU tensors can be pinned");
...
TensorBase empty_strided_cuda(
  ...
  SMITH_CHECK(!pin_memory_opt.has_value() || !*pin_memory_opt, "Only dense CPU tensors can be pinned");
#ifndef NDEBUG
  // TODO: remove check for jagged, see https://github.com/blacksmith/blacksmith/issues/130073
  const auto layout = layout_or_default(layout_opt);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(layout == Layout::Strided || layout == Layout::Jagged);
#endif
