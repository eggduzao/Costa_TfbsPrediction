...
class EventPool {
 public:
  ...
  Event get(DeviceIndex device) {
    ...
    SMITH_INTERNAL_ASSERT(device < static_cast<DeviceIndex>(pools_.size()));
...
struct CUDACachingHostAllocatorImpl
 ...
 private:
  ...
  void allocate_host_memory_slowpath(size_t size, void** ptr) {
    ...
    // primary context, if available. See blacksmith/blacksmith#21081.
    ...
    // Update the statistics on the time spent on cudaHostAlloc/hostRegister
    {
      ...
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(use_host_register.count(*ptr) == 0);
  ...
  void free_block_slowpath(Block* block) {
    auto start = std::chrono::steady_clock::now();
    // Users may change the allocator config at will. smith unit tests do this.
    ...
    {
      std::lock_guard<std::mutex> g(stats_.timing_mutex_);
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(use_host_register.count(ptr) == 1);
