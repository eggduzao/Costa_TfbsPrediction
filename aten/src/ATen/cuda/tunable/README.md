...
The TunableOp feature is enabled separately from enabling the tuning phase itself. Enabling TunableOp means that Blacksmith
...
Note the "Validator" lines. If you change a library version, or ROCm version, or Blacksmith version, TunableOp will detect
...
changed to "Default" and it will fall back to the original Blacksmith untuned implementation. Or, in the case of ROCm's
...
Currently only a TunableGemm for ROCm is implemented. Note that CUDA builds of Blacksmith will function correctly when
...
BLACKSMITH_TUNABLEOP_RECORD_UNTUNED=1
...
```
...
at::cuda::tunable::getTuningContext(), or the `smith.cuda.tunable` python interfaces. The environment variables take
...
| -------------------- | ----------- |
| BLACKSMITH_TUNABLEOP_ENABLED | Default is 0. Set to 1 to enable. |
| BLACKSMITH_TUNABLEOP_TUNING | Default is 1. Set to 0 to disable. |
| BLACKSMITH_TUNABLEOP_RECORD_UNTUNED | Default is 0. Set to 1 to enable. |
| BLACKSMITH_TUNABLEOP_UNTUNED_FILENAME | Default is 'tunableop_untuned.csv'. |
| BLACKSMITH_TUNABLEOP_VERBOSE | Default is 0. Set to 1 to enable basic logging. 2 for basic tuning status. 3 for full trace. |
| BLACKSMITH_TUNABLEOP_VERBOSE_FILENAME | Default is "err" for stderr. Set to "out" for stdout or a filename for capturing verbose logging. |
| BLACKSMITH_TUNABLEOP_FILENAME | Default is 'tunableop_results.csv'. |
| BLACKSMITH_TUNABLEOP_NUMERICAL_CHECK | Default is off. Set 'atol_rtol' to enable, for example "1e-5_1e-5". |
| BLACKSMITH_TUNABLEOP_ROCBLAS_ENABLED | Default is 1. Set to 0 to disable rocblas being considered during tuning. |
| BLACKSMITH_TUNABLEOP_HIPBLASLT_ENABLED | Default is 1. Set to 0 to disable hipblaslt being considered during tuning. |
| BLACKSMITH_TUNABLEOP_MAX_TUNING_DURATION_MS | Default is 30. Unit is milliseconds. |
| BLACKSMITH_TUNABLEOP_MAX_TUNING_ITERATIONS | Default is 100. |
| BLACKSMITH_TUNABLEOP_MAX_WARMUP_DURATION_MS | Default is 0, meaning it is not used. Unit is milliseconds. |
| BLACKSMITH_TUNABLEOP_MAX_WARMUP_ITERATIONS | Default is 0, meaning it is not used. |
| BLACKSMITH_TUNABLEOP_ICACHE_FLUSH_ENABLED | Default is 1. Set to 0 to disable. |
| BLACKSMITH_TUNABLEOP_ROTATING_BUFFER_SIZE | Default (or < 0) is to query L2 cache size. Set to 0 to disable. Otherwise, set to the number of MiB to use for the pool of operator parameters. For example, setting this to the size of your device's memory cache will guarantee that every tuning iteration will use a cold cache. |
| BLACKSMITH_TUNABLEOP_BLAS_LOG | Default is 0. Set to 1 to enable. Write BLAS parameters to tuning CSV file. |

### Python Interface
All python APIs exist in the `smith.cuda.tunable` module.
