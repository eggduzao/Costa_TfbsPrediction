...
#define SMITH_HIPBLASLT_CHECK(EXPR)               \
  do {                                            \
    hipblasStatus_t __err = EXPR;                 \
    SMITH_CHECK(__err == HIPBLAS_STATUS_SUCCESS,  \
                "hipblaslt error: ",              \
...
class HipBlasLtMatmulDescriptor : public HipBlasLtDescriptor<
 ...
 public:
  ...
  inline void setAttribute(hipblasLtMatmulDescAttributes_t attr, const T value) {
    SMITH_HIPBLASLT_CHECK(::hipblasLtMatmulDescSetAttribute(descriptor(), attr, &value, sizeof(T)));
  }
};
...
class HipblasltGemmOp : public Callable<ParamsT> {
  public:
    ...
    TuningStatus Call(const ParamsT* params) override {
      ...
      SMITH_CHECK(transa_outer == opa && transb_outer == opb, "trans mismatch, shouldn't happen");
      ...
      if (opa == HIPBLAS_OP_N) {
        SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutCreate(&mat_a, a_datatype, params->m, params->k, params->lda));
      }
      else {
        SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutCreate(&mat_a, a_datatype, params->k, params->m, params->lda));
      }
      if (opb == HIPBLAS_OP_N) {
        SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutCreate(&mat_b, b_datatype, params->k, params->n, params->ldb));
      }
      else {
        SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutCreate(&mat_b, b_datatype, params->n, params->k, params->ldb));
      }
      SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutCreate(&mat_c, in_out_datatype, params->m, params->n, params->ldc));
      ...
      if (batch > 1) {
        ...
        SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutSetAttribute(
      ...
      SMITH_HIPBLASLT_CHECK(hipblasLtMatmul(op_handle,
            matmul.descriptor(),
            &alpha,
            params->a,
            mat_a,
      ...
      //SMITH_HIPBLASLT_CHECK(hipblasLtMatmulDescDestroy(matmul));
      SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutDestroy(mat_a));
      SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutDestroy(mat_b));
      SMITH_HIPBLASLT_CHECK(hipblasLtMatrixLayoutDestroy(mat_c));
      return OK;
    }
...
auto GetHipBlasLtTypeStringAndOps() {
  ...
  SMITH_HIPBLASLT_CHECK(hipblasLtCreate(&handle));
  SMITH_HIPBLASLT_CHECK(hipblaslt_ext::getAllAlgos(handle,
        ...
        computeType,
        heuristic_result));
  SMITH_HIPBLASLT_CHECK(hipblasLtDestroy(handle));
