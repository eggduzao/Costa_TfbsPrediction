 ...
 * SMITH_CUBLASLT_UNIFIED_WORKSPACE can be used to opt-in to unifying the workspace pools.
 */
...
void createCublasLtHandle(cublasLtHandle_t *handle) {
  SMITH_CUDABLAS_CHECK(cublasLtCreate(handle));
}
...
size_t parseChosenWorkspaceSize() {
  ...
  if (val) {
    ...
    if (next == end) {
      SMITH_WARN("Could not parse CUBLAS_WORKSPACE_CONFIG, using default workspace size of ", default_size, " bytes.");
      return default_size;
    }
    while (next != end) {
      std::smatch match = *next;
      SMITH_CHECK(match.size() == 3, "Expected CUBLAS_WORKSPACE_SPACE_CONFIG match of size 3 (Format :SIZE:COUNT)");
...
#define SMITH_CUBLASLT_UNIFIED_WORKSPACE "SMITH_CUBLASLT_UNIFIED_WORKSPACE"

size_t getCUDABlasLtWorkspaceSize() {
  size_t pool_size = parseCUDABlasLtWorkspaceSize();
#ifndef USE_ROCM
  static auto unified_env_var = c10::utils::check_env(SMITH_CUBLASLT_UNIFIED_WORKSPACE);
#if !defined(FBCODE)
  ...
  if (unified) {
    ...
    if (cublasWorkspaceSize < pool_size) {
      SMITH_WARN_ONCE("Requested unified CUBLASLT workspace size of ", pool_size,
...
void setWorkspaceForHandle(cublasHandle_t handle, c10::cuda::CUDAStream stream) {
  ...
  // Fast path: check if workspace already exists
  {
    ...
    if (workspace_it != workspace.map.end()) {
      SMITH_CUDABLAS_CHECK(cublasSetWorkspace(
...
#ifndef USE_ROCM
  static bool unified = c10::utils::check_env(SMITH_CUBLASLT_UNIFIED_WORKSPACE) == true;
  if (unified) {
    ...
    std::shared_lock<std::shared_mutex> lock(workspace.mutex);
    auto workspace_it = workspace.map.find(key);
    SMITH_INTERNAL_ASSERT(workspace_it != workspace.map.end());
...
cublasHandle_t getCurrentCUDABlasHandle() {
  ...
  if (C10_UNLIKELY(!pctx)) {
    ...
    SMITH_WARN_ONCE("Attempting to run cuBLAS, but there was no current CUDA context! Attempting to set the primary context...");
  ...
  // See: https://github.com/blacksmith/blacksmith/pull/22405
  ...
  auto stream = c10::cuda::getCurrentCUDAStream();
  SMITH_CUDABLAS_CHECK(cublasSetStream(handle, stream));
  ...
  // original issue: https://github.com/blacksmith/blacksmith/pull/83461
  ...
  // Blacksmith's CUDA caching allocator. It's possible that CCA used up
  ...
  if (!NoTF32Guard::should_disable_tf32() &&
    ...
    SMITH_CUDABLAS_CHECK(cublasSetMathMode(handle, CUBLAS_TF32_TENSOR_OP_MATH));
  } else {
  ...
  SMITH_CUDABLAS_CHECK(hipblasSetAtomicsMode(handle, hipblas_mode));
#endif
  return handle;
}
