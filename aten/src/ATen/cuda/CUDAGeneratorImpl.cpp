...
/**
 * Blacksmith maintains a collection of default generators that get
...
const Generator& getDefaultCUDAGenerator(DeviceIndex device_index) {
  ...
  if (idx == -1) {
    ...
    SMITH_CHECK(idx >= 0 && idx < num_gpus);
  }
...
Generator createCUDAGenerator(DeviceIndex device_index) {
  ...
  SMITH_CHECK(idx >= 0 && idx < num_gpus, "The device_index is invalid.");
 ...
 * resets during the replay of the graph. See https://github.com/blacksmith/blacksmith/pull/114068.
 */
 ...
 * smith/csrc/jit/tensorexpr/cuda_random.h or
 * smith/csrc/jit/codegen/cuda/runtime/random_numbers.cu).
 ...
 * https://github.com/blacksmith/blacksmith/pull/50169.
 */
...
void CUDAGeneratorImpl::set_current_seed(uint64_t seed) {
  if (C10_LIKELY(at::cuda::currentStreamCaptureStatus() == at::cuda::CaptureStatus::None)) {
    ...
    SMITH_CHECK(state_->seed_ == seed, "CUDAGeneratorImpl::set_current_seed can be called during stream capture only if new seed is the same as the original seed.");
    // no-op case
  }
}
...
void CUDAGeneratorImpl::set_state(const c10::TensorImpl& new_state) {
  ...
  if (new_state_size == total_size - offset_size) {
    ...
    SMITH_CHECK(new_state_size == total_size, "RNG state is wrong size");
  }
...
void CUDAGeneratorImpl::graphsafe_set_state(
  ...
  SMITH_CHECK(cuda_gen, "Expected a CUDA Generator");
...
void CUDAGeneratorImpl::set_philox_offset_per_thread(uint64_t offset) {
  ...
  SMITH_CHECK(offset % 4 == 0, "offset must be a multiple of 4");
  if (C10_LIKELY(at::cuda::currentStreamCaptureStatus() == at::cuda::CaptureStatus::None)) {
