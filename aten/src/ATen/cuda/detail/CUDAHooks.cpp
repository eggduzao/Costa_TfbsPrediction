...
bool _hasPrimaryContext(DeviceIndex device_index) {
  SMITH_CHECK(device_index >= 0 && device_index < at::cuda::device_count(),
...
#if AT_MAGMA_ENABLED()
  SMITH_INTERNAL_ASSERT(magma_init_fn != nullptr, "Cannot initialize magma, init routine not set");
  magma_init_fn();
#endif
}
...
bool CUDAHooks::isPinnedPtr(const void* data) const {
  // First check if driver is broken/missing, in which case Blacksmith CPU
...
#else
  SMITH_CHECK(false, "Cannot query CuDNN version if ATen_cuda is not built with CuDNN");
#endif
}
...
#else
  SMITH_CHECK(false, "Cannot query CuDNN Frontend version if ATen_cuda is not built with CuDNN");
#endif
}
...
#else
  SMITH_CHECK(false, "Cannot query MIOpen version if ATen_cuda is not built with ROCm");
#endif
}
...
#else
  SMITH_CHECK(false, "Cannot query HipBLASLt version if ATen_cuda is not built with ROCm");
#endif
}
