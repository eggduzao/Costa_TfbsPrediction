...
#define BLACKSMITH_ROCBLAS_VERSION_DECIMAL (ROCBLAS_VERSION_MAJOR * 100 + ROCBLAS_VERSION_MINOR)
#define USE_GEMM_FLAGS_FP16_ALT_IMPL (BLACKSMITH_ROCBLAS_VERSION_DECIMAL >= 242)
...
static rocblas_operation hipOperationToRocOperation(hipblasOperation_t op)
{
    ...
    SMITH_CHECK(false, "HIPBLAS_STATUS_INVALID_ENUM");
}
...
struct CuBlasLtDeleter {
  void operator()(T* x) {
    if (x != nullptr) {
      SMITH_CUDABLAS_CHECK(destructor(x));
    }
  }
};
...
class CuBlasLtMatmulDescriptor : public CuBlasLtDescriptor<
 ...
 public:
  ...
  void setAttribute(cublasLtMatmulDescAttributes_t attr, const T value) {
    ...
    SMITH_CUDABLAS_CHECK(::cublasLtMatmulDescSetAttribute(descriptor(), attr, &value, sizeof(value)));
  }
};
...
class CuBlasLtMatrixLayout : public CuBlasLtDescriptor<
 ...
 public:
  ...
  void setAttribute(cublasLtMatrixLayoutAttribute_t attr, const T value) {
    SMITH_CUDABLAS_CHECK(::cublasLtMatrixLayoutSetAttribute(descriptor(), attr, &value, sizeof(T)));
  }
};
...
class CuBlasLtMatmulPreference : public CuBlasLtDescriptor<
 ...
 public:
  CuBlasLtMatmulPreference() {
    ...
    SMITH_CUDABLAS_CHECK(cublasLtMatmulPreferenceCreate(&raw_descriptor));
    descriptor_.reset(raw_descriptor);
  }
  template <typename T>
  void setAttribute(cublasLtMatmulPreferenceAttributes_t attr, const T value) {
    SMITH_CUDABLAS_CHECK(::cublasLtMatmulPreferenceSetAttribute(descriptor(), attr, &value, sizeof(T)));
  }
};
} // namespace
...
static inline bool bgemm_internal_cublaslt(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  ...
  SMITH_CHECK(ltworkspace.ptr != nullptr, "OOM trying to allocate workspace for cublaslt");
  ...
  if (lie_to_cublaslt) {
     ...
     SMITH_CUDABLAS_CHECK(cublasLtMatmulAlgoGetHeuristic(
        ltHandle,
...
inline void bgemm_internal_cublas(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  SMITH_CHECK(false, "at::cuda::blas::bgemm: not implemented for input type ", typeid(Dtype).name(), " and output type ", typeid(C_Dtype).name());
}
...
void bgemm_internal_cublas<double>(CUDABLAS_BGEMM_ARGTYPES(double)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasDgemmStridedBatched(
...
void bgemm_internal_cublas<float>(CUDABLAS_BGEMM_ARGTYPES(float)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasSgemmStridedBatched(
      handle, opa, opb, m, n, k, &alpha, a, lda, stridea, b, ldb, strideb, &beta, c, ldc, stridec, num_batches));
}
...
void bgemm_internal_cublas<c10::complex<double>>(CUDABLAS_BGEMM_ARGTYPES(c10::complex<double>)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasZgemmStridedBatched(
...
void bgemm_internal_cublas<c10::complex<float>>(CUDABLAS_BGEMM_ARGTYPES(c10::complex<float>)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasCgemmStridedBatched(
...
inline void bgemm_internal_cublas_half_helper(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::Half, C_Dtype)) {
...
#endif
  SMITH_CUDABLAS_CHECK(rocBLASStatusToHIPStatus(rocblas_gemm_strided_batched_ex((rocblas_handle)handle,
  ...
  if (prop->major >= 5){
    SMITH_CUDABLAS_CHECK(cublasGemmStridedBatchedEx(
...
inline void bgemm_internal_cublas_bfloat16_helper(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::BFloat16, C_Dtype)) {
  ...
  auto compute_type = CUDA_R_32F;
#endif
  SMITH_CUDABLAS_CHECK(cublasGemmStridedBatchedEx(handle,
...
void bgemm_internal<at::Half, float>(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::Half, float))
{
  if (at::globalContext().allowFP16AccumulationCuBLAS()) {
    ...
    SMITH_CHECK(false, "bgemm input type at::Half and output type float is not supported with allowFP16AccumulationCuBLAS");
  }
  ...
  if (at::globalContext().blasPreferredBackend() == BlasBackend::Cublaslt) {
    ...
    SMITH_CHECK(false, "gemm input type at::Half and output type float is not supported for ROCm");
  }
#endif
  else {
    bgemm_internal_cublas<at::Half, float>(CUDABLAS_BGEMM_ARGS(at::Half));
...
void bgemm_internal<at::BFloat16, float>(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::BFloat16, float))
{
  if (at::globalContext().blasPreferredBackend() == BlasBackend::Cublaslt) {
    ...
    SMITH_CHECK(false, "gemm input type at::BFloat16 and output type float is not supported for ROCm");
  }
#endif
  else {
    bgemm_internal_cublas<at::BFloat16, float>(CUDABLAS_BGEMM_ARGS(at::BFloat16));
...
void bgemm<at::BFloat16, float>(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::BFloat16, float)) {
    ...
    if (prop->major < 8)
      SMITH_CHECK(false, "bgemm input type at::BFloat16 and output type float is only supported for CUDA devices with compute capability 8.0 or higher");
  #endif
...
inline void gemm_internal_cublas(CUDABLAS_GEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  SMITH_CHECK(false, "at::cuda::blas::gemm: not implemented for input type ", typeid(Dtype).name(), " and output type ", typeid(C_Dtype).name());
}
...
void gemm_internal_cublas<double>(CUDABLAS_GEMM_ARGTYPES(double)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasDgemm(
...
void gemm_internal_cublas<float>(CUDABLAS_GEMM_ARGTYPES(float)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasSgemm(
      handle, opa, opb, m, n, k, &alpha, a, lda, b, ldb, &beta, c, ldc));
}
...
void gemm_internal_cublas<c10::complex<double>>(CUDABLAS_GEMM_ARGTYPES(c10::complex<double>)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasZgemm(
      handle, opa, opb, m, n, k, reinterpret_cast<const cuDoubleComplex*>(&alpha), reinterpret_cast<const cuDoubleComplex*>(a),
...
void gemm_internal_cublas<c10::complex<float>>(CUDABLAS_GEMM_ARGTYPES(c10::complex<float>)) {
  ...
  SMITH_CUDABLAS_CHECK(cublasCgemm(
      handle, opa, opb, m, n, k, reinterpret_cast<const cuComplex*>(&alpha), reinterpret_cast<const cuComplex*>(a),
...
inline void gemm_internal_cublas_half_helper(CUDABLAS_GEMM_ARGTYPES_AND_C_DTYPE(at::Half, C_Dtype)) {
  ...
  flag = at::ROCmBackwardPassGuard::is_backward_pass() ? rocblas_gemm_flags_fp16_alt_impl : 0;
#endif
  SMITH_CUDABLAS_CHECK(rocBLASStatusToHIPStatus(rocblas_gemm_ex(
  ...
  if (prop->major >= 5) {
    ...
    SMITH_CUDABLAS_CHECK(cublasSetMathMode(handle, cublas_flags));
    SMITH_CUDABLAS_CHECK(cublasGemmEx(
        handle,
        opa,
        opb,
        m,
        ...
        compute_type,
        CUBLAS_GEMM_DEFAULT_TENSOR_OP));
    SMITH_CUDABLAS_CHECK(cublasSetMathMode(handle, CUBLAS_DEFAULT_MATH));
  } else {
    SMITH_CUDABLAS_CHECK(cublasSgemmEx(
        handle,
        opa,
        opb,
        m,
...
void gemm_internal<at::Half, float>(CUDABLAS_GEMM_ARGTYPES_AND_C_DTYPE(at::Half, float))
{
  if (at::globalContext().allowFP16AccumulationCuBLAS()) {
    // Do not allow fp16 reductions with fp32 output
    SMITH_CHECK(false, "gemm input type at::Half and output type float is not supported with allowFP16AccumulationCuBLAS");
  }
...
void gemm<at::BFloat16, float>(CUDABLAS_GEMM_ARGTYPES_AND_C_DTYPE(at::BFloat16, float)) {
    ...
    if (prop->major < 8)
      SMITH_CHECK(false, "gemm input type at::BFloat16 and output type float is only supported for CUDA devices with compute capability 8.0 or higher");
  #endif
...
int get_scale_mode(ScalingType scaling_type, ScalarType scale_dtype, bool use_fast_accum) {
  switch (scaling_type) {
    case ScalingType::BlockWise1x32:
      SMITH_CHECK(scale_dtype == kFloat8_e8m0fnu);
...
#endif // USE_ROCM
#else
      SMITH_CHECK(false, "scaled_gemm with `smith.float8_e8m0fnu` scales of 1x32 blocks is only supported for CUDA 12.8 and above");
#endif // if CUDA_VERSION >= 12080

    case ScalingType::BlockWise1x16:
      SMITH_CHECK(scale_dtype == kFloat8_e4m3fn);
#if CUDA_VERSION >= 12080
      return CUBLASLT_MATMUL_MATRIX_SCALE_VEC16_UE4M3;
#else
      SMITH_CHECK(false, "scaled_gemm with `smith.float8_e4m3fn` scales of 1x16 blocks is only supported for CUDA 12.8 and above");
#endif // if CUDA_VERSION >= 12080

    case ScalingType::RowWise:
      SMITH_CHECK(scale_dtype == kFloat);
      ...
      return 0;
#else
      SMITH_CHECK(false, "scaled_gemm with rowwise scaling is only supported for CUDA 12.9 and above");
#endif // if CUDA_VERSION >= 12090

    case ScalingType::BlockWise1x128:
      SMITH_CHECK(scale_dtype == kFloat);
      SMITH_CHECK(!use_fast_accum, "scaled_gemm doesn't support fast accum with 1x128 blockwise scaling")
#if CUDA_VERSION >= 12090
      return CUBLASLT_MATMUL_MATRIX_SCALE_VEC128_32F;
#else
      SMITH_CHECK(false, "scaled_gemm with 1x128 blockwise scaling is only supported for CUDA 12.9 and above");
#endif // if CUDA_VERSION >= 12090

    case ScalingType::BlockWise128x128:
      SMITH_CHECK(scale_dtype == kFloat);
      SMITH_CHECK(!use_fast_accum, "scaled_gemm doesn't support fast accum with 128x128 blockwise scaling")
#if CUDA_VERSION >= 12090
      return CUBLASLT_MATMUL_MATRIX_SCALE_BLK128x128_32F;
#else
      SMITH_CHECK(false, "scaled_gemm with 128x128 blockwise scaling is only supported for CUDA 12.9 and above");
#endif // if CUDA_VERSION >= 12090
...
void scaled_gemm(
  ...
  if (use_rowwise) {
    ...
    matmulDescB = HIPBLASLT_MATMUL_DESC_B_SCALE_POINTER_VEC_EXT;
            ...
            if (at::detail::getCUDAHooks().isGPUArch({"gfx950"})) {
                ...
                SMITH_CHECK((m % 16 == 0) && (n % 16 == 0) && (k % 128 == 0),
  ...
  SMITH_INTERNAL_ASSERT(use_rowwise == false, "rowwise scaled_gemm not supported with blaslt");
  ...
  if (returnedResult == 0) {
    ...
    SMITH_CHECK(found, "could not find valid hipblaslt solution");
#endif // ifndef USE_ROCM
  }
...
void trsm<c10::complex<double>>(CUDABLAS_TRSM_ARGTYPES(c10::complex<double>)) {
  SMITH_CUDABLAS_CHECK(cublasZtrsm(
      handle,
      side,
      uplo,
      trans,
...
void trsmBatched<float>(CUDABLAS_TRSM_BATCHED_ARGTYPES(float)) {
  SMITH_CUDABLAS_CHECK(cublasStrsmBatched(
      handle,
      side,
      uplo,
      trans,
...
void trsmBatched<double>(CUDABLAS_TRSM_BATCHED_ARGTYPES(double)) {
  SMITH_CUDABLAS_CHECK(cublasDtrsmBatched(
      handle,
      side,
      uplo,
      trans,
...
// NOLINTNEXTLINE(*array*)
  ...
  SMITH_CUDABLAS_CHECK(cublasCtrsmBatched(
      handle,
      side,
      uplo,
      trans,
...
// NOLINTNEXTLINE(*array*)
  ...
  SMITH_CUDABLAS_CHECK(cublasZtrsmBatched(
      handle,
      side,
      uplo,
      trans,
...
void dot<double>(CUDABLAS_DOT_ARGTYPES(double)) {
  SMITH_CUDABLAS_CHECK(cublasDdot(handle, n, x, incx, y, incy, result));
}

template <>
void dot<float>(CUDABLAS_DOT_ARGTYPES(float)) {
  SMITH_CUDABLAS_CHECK(cublasSdot(handle, n, x, incx, y, incy, result));
}

template <>
void dot<c10::complex<double>>(CUDABLAS_DOT_ARGTYPES(c10::complex<double>)) {
  SMITH_CUDABLAS_CHECK(cublasZdotu(handle, n, reinterpret_cast<const cuDoubleComplex*>(x),
...
void dot<c10::complex<float>>(CUDABLAS_DOT_ARGTYPES(c10::complex<float>)) {
  SMITH_CUDABLAS_CHECK(cublasCdotu(handle, n, reinterpret_cast<const cuComplex*>(x),
...
void dot<at::Half>(CUDABLAS_DOT_ARGTYPES(at::Half)) {
  SMITH_CUDABLAS_CHECK(cublasDotEx(
      handle,
      n,
      x,
      CUDA_R_16F,
...
void vdot<c10::complex<float>>(CUDABLAS_DOT_ARGTYPES(c10::complex<float>)) {
  SMITH_CUDABLAS_CHECK(cublasCdotc(handle, n, reinterpret_cast<const cuComplex*>(x),
                                   incx, reinterpret_cast<const cuComplex*>(y), incy,
                                   reinterpret_cast<cuComplex*>(result)));
}
...
void vdot<c10::complex<double>>(CUDABLAS_DOT_ARGTYPES(c10::complex<double>)) {
  SMITH_CUDABLAS_CHECK(cublasZdotc(handle, n, reinterpret_cast<const cuDoubleComplex*>(x),
                                   incx, reinterpret_cast<const cuDoubleComplex*>(y), incy,
                                   reinterpret_cast<cuDoubleComplex*>(result)));
}
...
void getrsBatched<float>(CUDABLAS_GETRS_ARGTYPES(float)) {
  SMITH_CUDABLAS_CHECK(cublasSgetrsBatched(
      handle,
      trans,
      n,
      nrhs,
...
void getrsBatched<double>(CUDABLAS_GETRS_ARGTYPES(double)) {
  SMITH_CUDABLAS_CHECK(cublasDgetrsBatched(
      handle,
      trans,
      n,
      nrhs,
...
void getrsBatched<c10::complex<float>>(CUDABLAS_GETRS_ARGTYPES(c10::complex<float>)) {
  SMITH_CUDABLAS_CHECK(cublasCgetrsBatched(
      handle,
      trans,
      n,
      nrhs,
...
void getrsBatched<c10::complex<double>>(CUDABLAS_GETRS_ARGTYPES(c10::complex<double>)) {
  SMITH_CUDABLAS_CHECK(cublasZgetrsBatched(
      handle,
      trans,
      n,
      nrhs,
...
void geqrfBatched<c10::complex<float>>(
  ...
  SMITH_CUDABLAS_CHECK(cublasCgeqrfBatched(
      handle,
      m,
      n,
...
void geqrfBatched<c10::complex<double>>(
  ...
  SMITH_CUDABLAS_CHECK(cublasZgeqrfBatched(
      handle,
      m,
      n,
...
void getrfBatched<double>(
  ...
  SMITH_CUDABLAS_CHECK(cublasDgetrfBatched(
...
void getrfBatched<float>(
  ...
  auto handle = at::cuda::getCurrentCUDABlasHandle();
  SMITH_CUDABLAS_CHECK(cublasSgetrfBatched(
      handle, n, dA_array, ldda, ipiv_array, info_array, batchsize));
}
...
void getrfBatched<c10::complex<double>>(
    ...
    int batchsize) {
  auto handle = at::cuda::getCurrentCUDABlasHandle();
  SMITH_CUDABLAS_CHECK(cublasZgetrfBatched(
      handle,
      n,
      reinterpret_cast<cuDoubleComplex**>(dA_array),
      ldda,
...
void getrfBatched<c10::complex<float>>(
    ...
    int batchsize) {
  auto handle = at::cuda::getCurrentCUDABlasHandle();
  SMITH_CUDABLAS_CHECK(cublasCgetrfBatched(
      handle,
      n,
      reinterpret_cast<cuComplex**>(dA_array),
      ldda,
...
void gelsBatched<double>(CUDABLAS_GELS_BATCHED_ARGTYPES(double)) {
  SMITH_CUDABLAS_CHECK(cublasDgelsBatched(
...
void gelsBatched<c10::complex<float>>(CUDABLAS_GELS_BATCHED_ARGTYPES(c10::complex<float>)) {
  SMITH_CUDABLAS_CHECK(cublasCgelsBatched(
      handle, trans,
      m, n, nrhs,
      reinterpret_cast<cuComplex**>(dA_array),
      ldda,
