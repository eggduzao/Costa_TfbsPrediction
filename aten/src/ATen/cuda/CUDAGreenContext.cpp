...
#if defined(CUDA_VERSION) && CUDA_VERSION >= 12080 && !defined(USE_ROCM) && defined(BLACKSMITH_C10_DRIVER_API_SUPPORTED)
...
GreenContext::GreenContext(uint32_t device_id, uint32_t num_sms) {
  ...
  SMITH_CHECK(nb_groups == 1, "Failed to create single resource group");
  ...
  SMITH_CHECK(context_, "Green ctx conversion to regular ctx failed!");
#else
  SMITH_CHECK(false, "Green Context is only supported on CUDA 12.8+!");
#endif
  }
...
#else
  GreenContext::GreenContext(GreenContext&& other) noexcept {
    SMITH_CHECK(false, "Green Context move constructor is only supported on CUDA 12.8+!");
  }
#endif
