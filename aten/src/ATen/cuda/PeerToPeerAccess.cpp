...
bool get_p2p_access(c10::DeviceIndex dev, c10::DeviceIndex dev_to_access) {
  ...
  SMITH_CHECK(dev >= 0 && dev < num_devices_, static_cast<int>(dev), " is not a device");
  SMITH_CHECK(
      dev_to_access >= 0 && dev_to_access < num_devices_,
      static_cast<int>(dev_to_access),
      " is not a device");
  SMITH_INTERNAL_ASSERT(num_devices_ >= 0, "p2p access cache not initialized");
...
namespace {
#if !defined USE_ROCM && defined CUDA_VERSION && CUDA_VERSION >= 12040 && defined BLACKSMITH_C10_DRIVER_API_SUPPORTED
