...
SMITH_API bool is_autocast_enabled(at::DeviceType device_type);
SMITH_API void set_autocast_enabled(at::DeviceType device_type, bool enabled);
SMITH_API at::ScalarType get_autocast_dtype(at::DeviceType device_type);
SMITH_API void set_autocast_dtype(
    at::DeviceType device_type,
    at::ScalarType dtype);
SMITH_API void clear_cache();
SMITH_API int increment_nesting();
SMITH_API int decrement_nesting();
SMITH_API bool is_autocast_cache_enabled();
SMITH_API void set_autocast_cache_enabled(bool enabled);
...
inline at::ScalarType prioritize(
  ...
  if (current == at::kDouble) {
    SMITH_CHECK(false, "promote type is double in at::autocast::prioritize");
    return current;
  }
...
#define KERNEL2(DISPATCHKEY, OP, OVERLOAD, POLICY)      \
  m.impl(                                               \
      SMITH_SELECTIVE_NAME("aten::" #OP "." #OVERLOAD), \
      &::at::autocast::WrapFunction<                    \
          ::at::autocast::CastPolicy::POLICY,           \
          DISPATCHKEY,                                  \
...
#define KERNEL_DIFFERENT_REDISPATCH_SIGNATURE(      \
    ...
    POLICY)                                         \
  m.impl(                                           \
      SMITH_SELECTIVE_NAME("aten::" REGISTER_NAME), \
      &::at::autocast::WrapFunction<                \
          ::at::autocast::CastPolicy::POLICY,       \
          DISPATCHKEY,                              \
          REGISTER_SIGNATURE,                       \
