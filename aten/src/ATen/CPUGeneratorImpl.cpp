 ...
 * in smith.get_rng_state() and smith.set_rng_state().
 ...
 * as a helper for smith.get_rng_state() and smith.set_rng_state()
 * functions.
 */
...
void CPUGeneratorImpl::set_offset(uint64_t offset [[maybe_unused]]) {
  SMITH_CHECK(false, "CPU Generator does not use offset");
}
...
void CPUGeneratorImpl::set_state(const c10::TensorImpl& new_state) {
  ...
  SMITH_CHECK(new_state_size == size, "Expected a CPUGeneratorImplState of size ", size,
  ...
  SMITH_CHECK(engine.is_valid(), "Invalid mt19937 state");
  this->engine_ = engine;
