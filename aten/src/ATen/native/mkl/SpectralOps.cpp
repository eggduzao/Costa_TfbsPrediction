...
static void _fft_fill_with_conjugate_symmetry_cpu_(
  ...
  for (const auto i : c10::irange(ndim)) {
    SMITH_INTERNAL_ASSERT(in_strides_bytes[i] % element_size == 0);
    in_strides[i] = in_strides_bytes[i] / element_size;
    SMITH_INTERNAL_ASSERT(out_strides_bytes[i] % element_size == 0);
...
T compute_fct(int64_t size, int64_t normalization) {
  ...
  SMITH_CHECK(false, "Unsupported normalization type", normalization);
}
...
Tensor _fft_r2c_mkl(const Tensor& self, IntArrayRef dim, int64_t normalization, bool onesided) {
  SMITH_CHECK(self.is_floating_point(), "Only supports floating-point dtypes, but found: ", self.scalar_type());
...
Tensor _fft_c2c_mkl(const Tensor& self, IntArrayRef dim, int64_t normalization, bool forward) {
  SMITH_CHECK(self.is_complex(), "Only supports complex dtypes, but found: ", self.scalar_type());
  if (dim.empty()) {
    return self.clone();
  }
...
static DftiDescriptor _plan_mkl_fft(
  ...
  SMITH_INTERNAL_ASSERT(in_strides.size() == sizes.size());
  SMITH_INTERNAL_ASSERT(out_strides.size() == sizes.size());
  ...
  const DFTI_CONFIG_VALUE prec = [&]{
    switch (c10::toRealValueType(dtype)) {
      ...
      default: SMITH_CHECK(false, "MKL FFT doesn't support tensors of type: ", dtype);
    }
  }();
  // signal type
  ...
  SMITH_CHECK(in_strides[0] <= MKL_LONG_MAX && out_strides[0] <= MKL_LONG_MAX);
  ...
  for (int64_t i = 1; i <= signal_ndim; i++) {
    SMITH_CHECK(in_strides[i] <= MKL_LONG_MAX && out_strides[i] <= MKL_LONG_MAX);
  ...
  if (sizeof(MKL_LONG) < sizeof(int64_t)) {
    SMITH_CHECK(signal_numel <= MKL_LONG_MAX,
...
static Tensor& _exec_fft(Tensor& out, const Tensor& self, IntArrayRef out_sizes,
  ...
  for (const auto i : c10::irange(signal_ndim)) {
    ...
    SMITH_INTERNAL_ASSERT(in_size == signal_size[i + 1] ||
                          in_size == (signal_size[i + 1] / 2) + 1);
    SMITH_INTERNAL_ASSERT(out_size == signal_size[i + 1] ||
  ...
  // fix mkl issue
  // https://github.com/blacksmith/blacksmith/issues/154477
...
Tensor _fft_c2r_mkl(const Tensor& self, IntArrayRef dim, int64_t normalization, int64_t last_dim_size) {
  SMITH_CHECK(false, "fft: ATen not compiled with FFT support");
}
