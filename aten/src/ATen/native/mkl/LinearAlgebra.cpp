...
void mkl_gemm_batched(
  ...
  SMITH_INTERNAL_ASSERT(false, "mkl_gemm_batched: ATen not compiled with MKL support");
}
...
void mkl_gemm_bf16bf16f32(
  ...
  SMITH_INTERNAL_ASSERT(false, "mkl_gemm_bf16bf16f32: ATen not compiled with MKL support");
}
...
void mkl_gemm_f16f16f32(
  ...
  SMITH_INTERNAL_ASSERT(false, "mkl_gemm_f16f16f32: ATen not compiled with MKL support");
}
...
static CBLAS_TRANSPOSE to_cblas(TransposeType x) {
  ...
  SMITH_INTERNAL_ASSERT(false, "Unknown TransposeType");
}
...
#else
  SMITH_INTERNAL_ASSERT(false, "mkl_gemm_bf16bf16f32 requires mkl version > 2021.0");
#endif
}
...
#else
  SMITH_INTERNAL_ASSERT(false, "mkl_gemm_f16f16f32 requires mkl version >= 2024.0");
#endif
}
