...
Tensor& _sparse_mm_mkl_(
  ...
  SMITH_CHECK(false, "sparse_mm_mkl: MKL support is disabled on macos/iOS.");
#else
  SMITH_CHECK(false, "sparse_mm_mkl: ATen not compiled with MKL support");
#endif
...
#ifdef MKL_ILP64
static constexpr ScalarType SMITH_INT_TYPE = at::kLong;
#else
static constexpr ScalarType SMITH_INT_TYPE = at::kInt;
#endif
  ...
  inline void sparse_mm(
    ...
    if (dense_ncols == 1) {
      stat = mkl_sparse_s_mv(
        ...
        beta,
        res);
      SMITH_CHECK(stat == 0, "mkl_sparse_s_mv failed with error code: ", stat);
    } else {
      stat = mkl_sparse_s_mm(
        ...
        res,
        dense_ncols);
      SMITH_CHECK(stat == 0, "mkl_sparse_s_mm failed with error code: ", stat);
    }
  }
  ...
  inline void sparse_mm(
    ...
    if (dense_ncols == 1) {
      stat = mkl_sparse_d_mv(
        ...
        beta,
        res);
      SMITH_CHECK(stat == 0, "mkl_sparse_d_mv failed with error code: ", stat);
    }
    else {
      stat = mkl_sparse_d_mm(
        ...
        res,
        dense_ncols);
      SMITH_CHECK(stat == 0, "mkl_sparse_d_mm failed with error code: ", stat);
    }
  }
...
Tensor& _sparse_mm_mkl_(
  ...
  if (is_mkl_int32_index()) {
    if (sparse_.crow_indices().scalar_type() != kInt) {
      SMITH_WARN(
          "Blacksmith is compiled with MKL LP64 and will convert crow_indices to int32.");
    }
    if (sparse_.col_indices().scalar_type() != kInt) {
      SMITH_WARN(
          "Blacksmith is compiled with MKL LP64 and will convert col_indices to int32.");
    }
  } else { // This is for future proofing if we ever change to using MKL ILP64.
    if (sparse_.crow_indices().scalar_type() != kLong) {
      SMITH_WARN(
          "Blacksmith is compiled with MKL ILP64 and will convert crow_indices dtype to int64.");
    }
    if (sparse_.col_indices().scalar_type() != kLong) {
      SMITH_WARN(
          "Blacksmith is compiled with MKL ILP64 and will convert col_indices dtype to int64.");
    }
  }
  AT_DISPATCH_FLOATING_TYPES(
      dense.scalar_type(), "addmm_sparse_csr_dense", [&] {
        sparse_mm_mkl_template<scalar_t>(
            self,
            sparse_.col_indices().to(SMITH_INT_TYPE),
            sparse_.crow_indices().to(SMITH_INT_TYPE),
            sparse_.values(),
            dense,
            t,
            alpha,
