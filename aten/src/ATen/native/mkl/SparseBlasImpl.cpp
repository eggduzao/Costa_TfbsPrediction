...
void addmm_dense_result(
  ...
  SMITH_CHECK(
      ...
      "Please use Blacksmith built with MKL on Linux.");
#else
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!(is_C_row_major ^ is_B_row_major));
...
void addmm_sparse_result(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      result.scalar_type(), "addmm_out_sparse_csr_impl_mkl_sparse", [&] {
        ...
        SMITH_MKLSPARSE_CHECK(mkl_sparse_spmm(
            SPARSE_OPERATION_NON_TRANSPOSE,
...
void add_out_sparse_csr(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      result.scalar_type(), "add_out_sparse_csr_impl_mkl", [&] {
        ...
        // because MKL computes alpha*mat1 + mat2 while Blacksmith needs mat1 +
        // alpha*mat2
        auto result_desc = mkl_result.descriptor();
...
void triangular_solve_out_sparse_csr(
  ...
  // See https://github.com/blacksmith/blacksmith/issues/88890 and
  // the comments within.
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      X.scalar_type(), "triangular_solve_out_sparse_csr_impl_mkl", [&] {
        ...
        if (B.size(-1) == 1) {
          ...
          IntArrayRef B_strides = B_->strides();
          bool is_B_row_major = (B_strides[ndim - 1] == 1);
          SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!(is_X_row_major ^ is_B_row_major));
