...
#include <ATen/ops/_test_funcsmith_fallback_native.h>
#endif
...
bool is_nonzero(const Tensor& self) {
  auto n = self.numel();
  SMITH_CHECK(n != 0, "Boolean value of Tensor with no values is ambiguous");
  SMITH_CHECK(
  ...
  SMITH_INTERNAL_ASSERT(false, "Expected non-Tensor backend scalar");
}
...
// Aux function used to test funcsmith fallback warning
Tensor _test_funcsmith_fallback(const Tensor& self, const Tensor& other) {
  return self.clone();
}
