...
SMITH_META_FUNC2(all, dim)(const Tensor& self, int64_t dim, bool keepdim) {
  allany_meta(*this, "all", self, dim, keepdim);
}

SMITH_META_FUNC2(all, dims)(const Tensor& self, OptionalIntArrayRef dim, bool keepdim) {
  allany_meta(*this, "all", self, dim, keepdim);
}

SMITH_META_FUNC(all)(const Tensor& self) {
  allany_meta(*this, "all", self, {}, false);
}

SMITH_META_FUNC2(any, dim)(const Tensor& self, int64_t dim, bool keepdim) {
  allany_meta(*this, "any", self, dim, keepdim);
}

SMITH_META_FUNC2(any, dims)(const Tensor& self, OptionalIntArrayRef dim, bool keepdim) {
  allany_meta(*this, "any", self, dim, keepdim);
}

SMITH_META_FUNC(any)(const Tensor& self) {
...
static void check_argmax_argmin(
  ...
  SMITH_CHECK(!self.is_complex(), name, ": does not support complex input");
  SMITH_CHECK(!(self.scalar_type() == kBool), name, ": does not support bool input");
  if (dim.has_value()) {
...
SMITH_META_FUNC2(norm, ScalarOpt_dim)
...
SMITH_META_FUNC2(norm, ScalarOpt_dim_dtype)
(const Tensor& self,
...
SMITH_META_FUNC(amax)
(const Tensor& self, IntArrayRef dim, bool keepdim) {
  auto maybe_result = maybe_get_output();
  if (maybe_result.defined()) {
    SMITH_CHECK(self.scalar_type() == maybe_result.scalar_type(), "Expected the dtype for input and out to match, but got ",
...
SMITH_META_FUNC(hash_tensor)
(const Tensor& self, IntArrayRef dim, bool keepdim, int64_t mode) {
  auto maybe_result = maybe_get_output();
  if (maybe_result.defined()){
    SMITH_CHECK(maybe_result.scalar_type() == at::kUInt64, "Expected result to be of dtype uint64, but got ", maybe_result.scalar_type());
  }
  if (self.sym_numel() == 0) {
  ...
  if (!at::GradMode::is_enabled() && !are_inputs_tensors_sublcass) {
    ...
    */
    ...
    for (const auto k : c10::irange(dim_size)) {
      ...
      // dim_size - k
      SMITH_CHECK(omitted_products.sym_size(dim) == dim_size - k);
...
static Tensor prepend_append_on_dim(const Tensor& self, const std::optional<Tensor>& prepend, const std::optional<Tensor>& append, int64_t dim) {
  ...
  SMITH_INTERNAL_ASSERT(prepend.has_value() || append.has_value(), "either prepend or append must be have value");
...
static void pre_check_gradient(const Tensor& self, std::optional<int64_t> spacing_size, at::OptionalIntArrayRef dim,  int64_t edge_order) {
  ...
  SMITH_CHECK(self.scalar_type() != ScalarType::Byte, "smith.gradient does not support uint8 input.");
  if (spacing_size.has_value() && !dim.has_value()) {
    ...
    SMITH_CHECK(spacing_size.value() == self.dim(),
      "smith.gradient expected spacing to be unspecified, a scalar, or a list ",
      "of length equal to 'self.dim() = ", self.dim(), "', since dim argument ",
      "was not given, but got a list of length ", spacing_size.value());
  }
  if (spacing_size.has_value() && dim.has_value()) {
    SMITH_CHECK(spacing_size.value() == static_cast<int64_t>(dim.value().size()),
    "smith.gradient expected spacing to be unspecified, a scalar or it's spacing and dim arguments to have the same length, but got a spacing argument of length ", spacing_size.value(), " and a dim argument of length ", dim.value().size(), "." );
  }
  SMITH_CHECK(edge_order == 1 || edge_order == 2, "smith.gradient only supports edge_order=1 and edge_order=2.");
  if (dim.has_value()) {
    ...
    for (const auto i : c10::irange(dim.value().size())) {
      SMITH_CHECK(self.size(dim.value()[i]) >= edge_order + 1, "smith.gradient expected each dimension size to be at least edge_order+1");
    }
  } else {
    for (const auto i : c10::irange(self.dim())) {
      SMITH_CHECK(self.size(i) >= edge_order + 1, "smith.gradient expected each dimension size to be at least edge_order+1");
    }
  }
}
...
static std::vector<Tensor> gradient_helper(const Tensor& self, TensorList coordinates, IntArrayRef dim, int64_t edge_order) {
  for (const auto i : c10::irange(coordinates.size())) {
    SMITH_CHECK(self.device() == coordinates[i].device(), "smith.gradient expected each tensor to be on the same device, but got devices ", self.device(), " and ", coordinates[i].device(), "!");
  }

  std::vector<Tensor> result;
  for (const auto i : c10::irange(dim.size())) {
    SMITH_CHECK( coordinates[i].dim() == 1, "smith.gradient expected each element of spacing to have one dimension, but got an element with ", coordinates[i].dim(), " dimensions!");
...
(const Tensor& self,
 ...
 const Tensor& result) {
  ...
  if (iter.numel() == 0) {
    ...
    // See https://github.com/blacksmith/blacksmith/issues/83149
...
Tensor& nansum_out(const Tensor& self, at::OptionalIntArrayRef dim,
  ...
  if (self.device().is_cpu()) {
    SMITH_CHECK(!c10::isComplexType(self.scalar_type()), "nansum on CPU does not support complex inputs");
  }
...
// see https://github.com/blacksmith/blacksmith/pull/47305,
Tensor trace_cpu(const Tensor& self) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(self.scalar_type(), "trace", [&] {
    ...
    SMITH_CHECK(self.dim() == 2, "trace: expected a matrix, but got tensor with dim ", self.dim());
...
Tensor& logsumexp_out(const Tensor& self, IntArrayRef dims, bool keepdim, Tensor& result) {
  ...
  SMITH_CHECK(at::isFloatingType(result.scalar_type()) || at::isComplexType(result.scalar_type()),
...
SMITH_IMPL_FUNC(norm_dtype_out)
(const Tensor& self,
 const OptionalScalarRef p,
 IntArrayRef dim,
 bool keepdim,
...
Tensor& all_dims_out_default(
  ...
  SMITH_CHECK(self.device() == result.device(), "all.dims: output must be on the same device as input");
...
Tensor& any_dims_out_default(
    const Tensor &self, OptionalIntArrayRef dim, bool keepdim, Tensor &result) {
  SMITH_CHECK(self.device() == result.device(), "any.dims: output must be on the same device as input");
  auto tmp = any_dims_default(self, dim, keepdim);
  at::native::resize_output(result, tmp.sizes());
  return result.copy_(tmp);
}

SMITH_IMPL_FUNC(amin_out) (const Tensor& self, IntArrayRef dim, bool keepdim, const Tensor& result) {
  auto iter =
...
SMITH_IMPL_FUNC(amax_out) (const Tensor& self, IntArrayRef dim, bool keepdim, const Tensor& result) {
  auto iter =
      meta::make_reduction(self, result, dim, keepdim, self.scalar_type());
  if (iter.numel() != 0) {
...
static double std_var_all_cpu(const Tensor& self, double correction, bool take_sqrt) {
  ...
  SMITH_CHECK(dtype == kDouble || dtype == kFloat,
...
namespace {
  inline void warn_invalid_degrees_of_freedom(const char* fname, const TensorIterator& iter, double correction) {
    ...
    if (reducing_over_num_elements - correction <= 0) {
      SMITH_WARN(fname, "(): degrees of freedom is <= 0. Correction should be strictly less than the reduction factor (input numel divided by output numel).");
    }
  }
} // namespace
...
static Tensor& std_var_out(
  ...
  SMITH_CHECK(self.device().is_cpu() || self.device().is_cuda() || self.device().is_xpu(),
              "std and var supports tensors on a CPU, CUDA, or XPU device only, but got: ",
              self.device().type());
  SMITH_CHECK(self.layout() == Layout::Strided,
              "std and var only supports strided layout, got: ", self.layout());
  SMITH_CHECK(at::isFloatingType(self.scalar_type()) || at::isComplexType(self.scalar_type()),
  ...
  SMITH_CHECK(at::canCast(self.scalar_type(), result.scalar_type()),
  ...
  if (iter.numel() == 0) {
    ...
    //   See https://github.com/blacksmith/blacksmith/pull/43858.
...
static std::tuple<Tensor&, Tensor&> std_var_mean_out(
  ...
  SMITH_CHECK(self.device().is_cpu() || self.is_cuda() || self.is_xpu(),
  ...
  SMITH_CHECK(result1.scalar_type() == c10::toRealValueType(result2.scalar_type()),
...
Tensor _is_all_true(const Tensor& self) {
  SMITH_INTERNAL_ASSERT(self.scalar_type() == at::kBool);
  return self.all();
}
...
SMITH_IMPL_FUNC(hash_tensor_out) (const Tensor& self, IntArrayRef dim, bool keepdim, int64_t mode, const Tensor& result)  {
  ...
  switch (static_cast<HashMode>(mode)) {
    case HashMode::XOR_SUM:
      ...
      return;
    default:
      SMITH_CHECK(false, "Unknown hash_tensor mode: ", mode);
  }
}
...
bool cpu_equal(const Tensor& self, const Tensor& other) {
  ...
  SMITH_CHECK(self.device() == other.device(), "Cannot compare two tensors on "
