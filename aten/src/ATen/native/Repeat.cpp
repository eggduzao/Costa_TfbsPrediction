...
static void compute_cpu(
  ...
  at::parallel_for(0, size, 1, [&](int64_t i_begin, int64_t i_end) {
    for (const auto i : c10::irange(i_begin, i_end)) {
      ...
      SMITH_CHECK((size >= 0), "repeats can not be negative");
...
Tensor repeat_interleave_symint(
  ...
  if (repeats.dim() == 0 || (repeats.dim() == 1 && SMITH_GUARD_OR_FALSE(repeats.sym_size(0).sym_eq(1)))) {
    ...
    SMITH_CHECK(false, "repeats must be 0-dim or 1-dim tensor");
  }
...
Tensor repeat_interleave_symint(
  ...
  SMITH_CHECK(repeats >= 0, "Repeats must be non-negative");
  ...
  if (output_size) {
    ...
    SMITH_CHECK(*output_size == calculated_size, "repeat_interleave: Invalid output_size, expected ",
