...
SMITH_META_FUNC2(pow, Tensor_Tensor) (const Tensor& base, const Tensor& exp) {
  build_borrowing_binary_op(maybe_get_output(), base, exp);
}

SMITH_META_FUNC2(pow, Tensor_Scalar) (const Tensor& base, const Scalar& exp) {
  // Numpy compatibility check:
  SMITH_CHECK(!(isIntegralType(base.scalar_type(), true) &&
...
SMITH_META_FUNC2(pow, Scalar) (const Scalar& base, const Tensor& exp) {
...
SMITH_IMPL_FUNC(pow_Tensor_Tensor_out) (const Tensor& base, const Tensor& exp, const Tensor& out) {
  pow_tensor_tensor_stub(device_type(), *this);
}

SMITH_IMPL_FUNC(pow_Tensor_Scalar_out) (const Tensor& base, const Scalar& exp, const Tensor& out) {
...
SMITH_IMPL_FUNC(pow_Scalar_out) (const Scalar& base, const Tensor& exp, const Tensor& out) {
  if (base.equal(1.0)) {
    out.fill_(1);
  } else {
...
Tensor& float_power_out(const Tensor& base, const Tensor& exp, Tensor& result) {
  ...
  SMITH_CHECK(result.scalar_type() == dtype,
...
Tensor& float_power_(Tensor& base, const Tensor& exp) {
  auto dtype = (at::isComplexType(base.scalar_type()) || at::isComplexType(exp.scalar_type())) ? at::kComplexDouble : at::kDouble;
  SMITH_CHECK(base.scalar_type() == dtype,
