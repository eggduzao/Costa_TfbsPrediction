...
inline Tensor _view_as_real_physical(const Tensor& self) {
  SMITH_CHECK(self.is_complex(), "view_as_real is only supported for complex tensors");
...
Tensor view_as_real(const Tensor& self) {
  SMITH_CHECK(!self.is_conj(), "view_as_real doesn't work on unresolved conjugated tensors.  To resolve the conjugate tensor so you can view it as real, use self.resolve_conj(); however, be warned that the resulting tensor will NOT alias the original.");
...
inline SymDimVector computeStrideForViewAsComplex(
  ...
  SMITH_CHECK(dim > 0 && oldstride[dim - 1] == 1, "Tensor must have a last dimension with stride 1");

  SymDimVector res(dim - 1);
  for (const auto i : c10::irange(res.size())) {
    // Skip divisibility check for singleton dimensions
    if (oldsizes[i] != 1) {
      SMITH_CHECK(oldstride[i] % 2 == 0, "Tensor must have a stride divisible by 2 for all but last dimension");
    }
    res[i] = oldstride[i] / 2;
  }
  return res;
...
Tensor view_as_complex(const Tensor& self) {
  ...
  auto old_sizes = self.sym_sizes();
  SMITH_CHECK(!old_sizes.empty(), "Input tensor must have one or more dimensions");
  SMITH_CHECK(old_sizes[old_sizes.size()-1] == 2, "Tensor must have a last dimension of size 2");
  ...
  SMITH_CHECK(self.sym_storage_offset() % 2 == 0, "Tensor must have a storage_offset divisible by 2");
