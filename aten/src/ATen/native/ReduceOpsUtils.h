...
inline Tensor integer_upcast(const Tensor& self, std::optional<ScalarType> dtype) {
  ...
  SMITH_CHECK(!isBarebonesUnsignedType(scalarType), "integer upcasting for uint16, uint32 and uint64 is not currently implemented");
    ...
    ScalarType /*dtype*/)
{
  ...
  SMITH_CHECK(result.defined(), "Cannot create a new tensor inside a reduction op. You likely tried to call an operator with an out argument but the out argument was an undefined tensor.");
...
inline void zero_numel_check_dims(const Tensor& self, const int64_t dim, const char *fn_name) {
  if (self.ndimension() == 0) {
    SMITH_CHECK_INDEX(dim == 0 || dim == -1, fn_name,
      ": Expected reduction dim -1 or 0 for scalar but got ", dim);
  }
  else {
    SMITH_CHECK_INDEX(self.size(dim) != 0, fn_name,
...
inline std::vector<int64_t> get_zero_numel_tensor_size(
    ...
    const bool keepdim,
    const char* fn_name) {
  SMITH_INTERNAL_ASSERT(self.numel() == 0,  fn_name, ": Expected self.numel() == 0.");
...
inline void resize_reduction(
  ...
  if (self.layout() == kStrided) {
    ...
    SMITH_CHECK(false, "resize_reduction: support for output with ", self.layout(), " layout is not implemented yet");
  }
