...
Tensor refine_names(const Tensor& self, DimnameList names) {
  ...
  SMITH_CHECK(self_names.size() == names.size(),
  ...
  for (const auto idx : c10::irange(self_names.size())) {
    ...
    SMITH_CHECK(false,
        ...
        idx);
    SMITH_INTERNAL_ASSERT(false); // done handling errors
  }
...
// 2) smith.align_tensors([tensor, other])  (is_aligning_two_tensors=true)
