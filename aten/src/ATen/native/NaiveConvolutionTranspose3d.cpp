...
inline void slow_conv_transpose3d_shape_check(
  ...
  // the weight tensor
  if (weight.defined()) {
    /* TODO: SMITH_CHECK just have 2 args: condition and message */
    SMITH_CHECK(
    ...
    SMITH_CHECK(false, "weight tensor is expected to be non-nullable");
  }
...
void slow_conv_transpose3d_acc_grad_parameters_cpu(
  ...
  if (grad_weight.defined()) {
    SMITH_CHECK(grad_weight.is_contiguous(), "grad_weight needs to be contiguous");
  }
  if (grad_bias.defined()) {
    SMITH_CHECK(grad_bias.is_contiguous(), "grad_bias needs to be contiguous");
  }
