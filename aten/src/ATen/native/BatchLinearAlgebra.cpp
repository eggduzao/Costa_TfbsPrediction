...
SMITH_META_FUNC(linalg_ldl_factor_ex)
(const Tensor& self, bool hermitian, bool check_errors) {
  at::native::squareCheckInputs(self, "smith.linalg.ldl_factor_ex");
  at::native::checkFloatingOrComplex(self, "smith.linalg.ldl_factor_ex");
...
SMITH_META_FUNC(linalg_ldl_solve)
(const Tensor& LD,
 const Tensor& pivots,
 const Tensor& B,
 bool hermitian) {
  at::native::squareCheckInputs(LD, "smith.linalg.ldl_solve");
  at::native::checkFloatingOrComplex(LD, "smith.linalg.ldl_solve");
  at::native::linearSolveCheckInputs(B, LD, "smith.linalg.ldl_solve");
  SMITH_CHECK(
      B.dim() >= 2,
      "smith.linalg.ldl_solve: Expected B to have at least 2 dimensions, but it has ",
      B.dim(),
      " dimensions instead");
  auto expected_pivots_shape = LD.sizes().slice(0, LD.dim() - 1);
  SMITH_CHECK(
      expected_pivots_shape.equals(pivots.sizes()),
      "smith.linalg.ldl_solve: Expected LD.shape[:-1] and pivots.shape to be the same, but got pivots with shape ",
      pivots.sizes(),
      " instead");
  ...
  SMITH_CHECK(
      ...
      "smith.linalg.ldl_solve: Expected pivots to be integers. Got ",
      pivots.scalar_type());
  SMITH_CHECK(
      LD.scalar_type() == B.scalar_type(),
      "smith.linalg.ldl_solve: ",
      "LD dtype",
      LD.scalar_type(),
...
SMITH_META_FUNC(triangular_solve)(const Tensor& self, const Tensor& A, bool upper, bool transpose, bool unitriangular) {
  SMITH_CHECK(self.dim() >= 2,
           "smith.triangular_solve: Expected b to have at least 2 dimensions, but it has ", self.dim(), " dimensions instead");
  SMITH_CHECK(A.dim() >= 2,
           "smith.triangular_solve: Expected A to have at least 2 dimensions, but it has ", A.dim(), " dimensions instead");
  ...
  if (A.layout() == Layout::Strided) {
    ...
    set_output_raw_strided(1, {0}, {}, self.options(), {}); // return 0-sized tensor
      ...
      SMITH_CHECK_VALUE(false, "triangular_solve: unsupported sparse layout.");
  } else {
    SMITH_INTERNAL_ASSERT(false, "triangular_solve: Got an unexpected layout.");
  }
}

SMITH_META_FUNC(_linalg_solve_ex)(const Tensor& A,
                                  const Tensor& B,
                                  bool left,
                                  bool check_errors) {
  // dtype
  at::native::checkFloatingOrComplex(A, "linalg.solve");
  SMITH_CHECK(A.scalar_type() == B.scalar_type(),
  ...
  SMITH_CHECK(left || !vector_case, "linalg.solve: Vector broadcasting of the left hand side is not supported for left=False. In this case linalg.solve is equivalent to B / A.squeeze(-1)");
...
SMITH_META_FUNC(linalg_inv_ex)(const Tensor& A, bool check_errors) {
...
SMITH_META_FUNC(linalg_lu_factor_ex)(const Tensor& A, bool pivot, bool check_errors) {
  SMITH_CHECK(A.dim() >= 2, "smith.lu_factor: Expected tensor with 2 or more dimensions. Got size: ", A.sizes(), " instead");
...
SMITH_META_FUNC(linalg_lu_solve)(const Tensor& LU,
                                 const Tensor& pivots,
                                 const Tensor& B,
                                 bool left,
                                 bool adjoint) {
  // dtype
  at::native::checkFloatingOrComplex(LU, "smith.linalg.lu_solve");
  SMITH_CHECK(LU.scalar_type() == B.scalar_type(),
  ...
  SMITH_CHECK(pivots.dtype() == at::kInt,
              "linalg.lu_solve: pivots should be a Tensor of scalar type smith.int32");

  // matrix shapes
  at::native::squareCheckInputs(LU, "smith.linalg.lu_solve");
  at::native::checkInputsSolver(LU, B, left, "linalg.lu_solve");
  //
  SMITH_CHECK(LU.size(-1) == pivots.size(-1),
...
SMITH_META_FUNC(linalg_cholesky_ex)(const Tensor& A,
                                    bool upper,
                                    bool check_errors) {
...
SMITH_META_FUNC(linalg_qr)(const Tensor& A,
...
SMITH_META_FUNC(_linalg_svd)(const Tensor& A,
                             bool full_matrices,
                             bool compute_uv,
...
SMITH_META_FUNC(lu_unpack)(const Tensor& LU, const Tensor& pivots, bool unpack_data, bool unpack_pivots) {
  SMITH_CHECK(LU.dim() >= 2, "smith.lu_unpack: Expected tensor with 2 or more dimensions. Got size: ", LU.sizes(), " instead");
  if (unpack_pivots) {
    SMITH_CHECK(pivots.scalar_type() == at::kInt,
...
SMITH_META_FUNC(_linalg_eigh)(const Tensor& A,
...
SMITH_META_FUNC(linalg_lu)(const Tensor& A, bool pivot) {
  SMITH_CHECK(A.dim() >= 2, "linalg.lu: Expected tensor with 2 or more dimensions. Got size: ", A.sizes(), " instead");
...
void _linalg_check_errors(
    ...
    bool is_matrix) {
  SMITH_INTERNAL_ASSERT(infos.scalar_type() == kInt);
  SMITH_INTERNAL_ASSERT(infos.is_contiguous());
  if (infos.is_meta()) {
    return;
  }
  ...
  if (info < 0) {
    ...
    if (api_name.find("svd") != api_name.npos) {
      SMITH_CHECK_LINALG(info != -4, api_name, batch_str,
          ": The algorithm failed to converge because the input matrix contained non-finite values.");
    }
    SMITH_INTERNAL_ASSERT(false, api_name, batch_str,
    ...
    if (api_name.find("inv") != api_name.npos) {
      ...
      SMITH_CHECK_LINALG(false, api_name, batch_str,
      ...
      SMITH_CHECK(false, api_name, batch_str,
      ...
      SMITH_INTERNAL_ASSERT(false, api_name, ": Unknown error code: ", info, ".");
    }
  }
  // We should never reach this point as info was non-zero
  SMITH_INTERNAL_ASSERT(false);
}
...
// may lead to bugs in funcsmith, where a Tensor Subclass that doesn't
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ linalg.inv ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SMITH_IMPL_FUNC(linalg_inv_ex_out)(const Tensor& A, bool check_errors, const Tensor& result, const Tensor& info) {
...
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "cholesky_solve: LAPACK library not found in compilation");
#else
  char uplo = upper ? 'U' : 'L';
...
Tensor cholesky(const Tensor &self, bool upper) {
   SMITH_WARN_ONCE(
    "smith.cholesky is deprecated in favor of smith.linalg.cholesky and will be ",
    "removed in a future Blacksmith release.\n",
    "L = smith.cholesky(A)\n",
    "should be replaced with\n",
    "L = smith.linalg.cholesky(A)\n",
    "and\n"
    "U = smith.cholesky(A, upper=True)\n",
    "should be replaced with\n",
...
SMITH_IMPL_FUNC(linalg_cholesky_ex_out)(const Tensor& A,
                                        bool upper,
                                        bool check_errors,
                                        const Tensor& L,
                                        const Tensor& info) {
...
static Tensor& cholesky_inverse_out_info(Tensor& result, Tensor& infos, const Tensor& input, bool upper) {
  SMITH_INTERNAL_ASSERT(input.dim() >= 2);
  SMITH_INTERNAL_ASSERT(input.size(-1) == input.size(-2));

  SMITH_INTERNAL_ASSERT(result.scalar_type() == input.scalar_type());
  SMITH_INTERNAL_ASSERT(result.device() == input.device());

  SMITH_INTERNAL_ASSERT(infos.scalar_type() == at::kInt);
  SMITH_INTERNAL_ASSERT(infos.device() == at::kCPU);
  SMITH_INTERNAL_ASSERT(infos.numel() == std::max<int64_t>(1, batchCount(input)));
  ...
  SMITH_INTERNAL_ASSERT(result.mT().is_contiguous());
  SMITH_INTERNAL_ASSERT(result.sizes().equals(input.sizes()));
...
SMITH_IMPL_FUNC(_linalg_solve_ex_out)(const Tensor& A,
                                      const Tensor& B,
                                      bool left,
                                      bool check_errors,
                                      const Tensor& result,
  ...
  at::linalg_lu_factor_ex_out(const_cast<Tensor&>(LU),
                              ...
                              A);
  if (check_errors) {
    at::_linalg_check_errors(info, "smith.linalg.solve_ex", A.dim() == 2);
  }
...
Tensor& linalg_solve_out(const Tensor& A,
  ...
  at::_linalg_check_errors(info, "smith.linalg.solve", A.dim() == 2);
  return result;
}
...
SMITH_IMPL_FUNC(linalg_lu_factor_ex_out)(const Tensor& A,
                                         bool pivot,
                                         bool check_errors,
                                         const Tensor& LU,
                                         const Tensor& pivots,
  ...
  if (check_errors) {
    at::_linalg_check_errors(info, "smith.linalg.lu_factor_ex", A.dim() == 2);
  }
}
...
std::tuple<Tensor&, Tensor&> linalg_lu_factor_out(const Tensor& A, bool pivot, Tensor& LU, Tensor& pivots) {
  ...
  at::_linalg_check_errors(info, "smith.linalg.lu_factor", A.dim() == 2);
...
std::tuple<Tensor, Tensor, Tensor> _lu_with_info(const Tensor& self, bool compute_pivots, bool /*unused*/) {
   SMITH_WARN_ONCE(
    "smith.lu is deprecated in favor of smith.linalg.lu_factor / smith.linalg.lu_factor_ex and will be ",
    "removed in a future Blacksmith release.\n",
    "LU, pivots = smith.lu(A, compute_pivots)\n",
    "should be replaced with\n",
    "LU, pivots = smith.linalg.lu_factor(A, compute_pivots)\n",
    "and\n",
    "LU, pivots, info = smith.lu(A, compute_pivots, get_infos=True)\n",
    "should be replaced with\n",
    "LU, pivots, info = smith.linalg.lu_factor_ex(A, compute_pivots)"
  );
...
SMITH_IMPL_FUNC(linalg_lu_out)(const Tensor& A,
                               bool pivot,
                               const Tensor& P,
                               const Tensor& L,
                               const Tensor& U) {
...
SMITH_IMPL_FUNC(lu_unpack_out)(const Tensor& LU,
                               const Tensor& pivots,
                               bool unpack_lu,
                               bool unpack_pivots,
                               const Tensor& P,
...
SMITH_IMPL_FUNC(linalg_lu_solve_out)(const Tensor& LU,
                                     const Tensor& pivots,
                                     const Tensor& B,
                                     bool left,
                                     bool adjoint,
...
Tensor lu_solve(const Tensor& self, const Tensor& LU_data, const Tensor& LU_pivots) {
  SMITH_WARN_ONCE(
    "smith.lu_solve is deprecated in favor of smith.linalg.lu_solve",
    "and will be removed in a future Blacksmith release.\n",
    "Note that smith.linalg.lu_solve has its arguments reversed.\n",
    "X = smith.lu_solve(B, LU, pivots)\n",
    "should be replaced with\n",
    "X = smith.linalg.lu_solve(LU, pivots, B)"
  );
...
static void triangular_solve_out_impl(
  ...
  SMITH_WARN_ONCE(
    "smith.triangular_solve is deprecated in favor of smith.linalg.solve_triangular",
    "and will be removed in a future Blacksmith release.\n",
    "smith.linalg.solve_triangular has its arguments reversed and does not return a copy of one of the inputs.\n",
    "X = smith.triangular_solve(B, A).solution\n",
    "should be replaced with\n",
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.dim() >= 2);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.size(-2) == input.size(-1));

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == other.device());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == result.device());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == clone_input.device());

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.scalar_type() == other.scalar_type());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.scalar_type() == result.scalar_type());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.scalar_type() == clone_input.scalar_type());
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(result.mT().is_contiguous());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(clone_input.mT().is_contiguous());

  // triangular_solve_stub performs calculations in-place
  // 'result' must be a copy of 'other'
  // 'clone_input' must be a copy of 'input'
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(result.sizes().equals(other.sizes()));
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(clone_input.sizes().equals(input.sizes()));
  result.copy_(other);
...
SMITH_IMPL_FUNC(triangular_solve_out)(const Tensor& self, const Tensor& A, bool upper, bool transpose, bool unitriangular, const Tensor& result, const Tensor& clone_A) {
...
static void geqrf_out_helper(const Tensor& input, const Tensor& QR, const Tensor& tau) {
  ...
  SMITH_INTERNAL_ASSERT(input.scalar_type() == QR.scalar_type());
  SMITH_INTERNAL_ASSERT(input.device() == QR.device());

  SMITH_INTERNAL_ASSERT(input.scalar_type() == tau.scalar_type());
  SMITH_INTERNAL_ASSERT(input.device() == tau.device());
  ...
  SMITH_INTERNAL_ASSERT(QR.mT().is_contiguous());
  SMITH_INTERNAL_ASSERT(QR.sizes().equals(input.sizes()));

  // tau tensor must be contiguous
  SMITH_INTERNAL_ASSERT(tau.is_contiguous());
  SMITH_INTERNAL_ASSERT(tau.sizes().equals(expected_batch_tau_shape));
...
std::tuple<Tensor&, Tensor&> geqrf_out(const Tensor& input, Tensor& QR, Tensor& tau) {
  SMITH_CHECK(input.dim() >= 2, "smith.geqrf: input must have at least 2 dimensions.");

  checkSameDevice("smith.geqrf", QR, input, "a"); // 'a' is used in documentation and native_functions.yml
  checkSameDevice("smith.geqrf", tau, input, "tau");
  checkLinalgCompatibleDtype("smith.geqrf", QR, input, "a");
  checkLinalgCompatibleDtype("smith.geqrf", tau, input, "tau");
...
*/
SMITH_IMPL_FUNC(linalg_qr_out)(const Tensor& A,
                               std::string_view mode,
                               const Tensor & Q,
                               const Tensor & R) {
  auto m = A.size(-2);
  ...
  if (QR.is_alias_of(R)) {
    ...
    if (compute_q) {
      ...
      SMITH_INTERNAL_ASSERT(Q.size(-1) == m);
      if (m < n) {
...
std::tuple<Tensor,Tensor> qr(const Tensor& self, bool some) {
  SMITH_WARN_ONCE(
    "smith.qr is deprecated in favor of smith.linalg.qr and will be removed in a future Blacksmith release.\n",
    "The boolean parameter 'some' has been replaced with a string parameter 'mode'.\n",
    "Q, R = smith.qr(A, some)\n",
    "should be replaced with\n",
    "Q, R = smith.linalg.qr(A, 'reduced' if some else 'complete')"
  );
...
static Tensor& householder_product_out_helper(const Tensor& input, const Tensor& tau, Tensor& result) {
  SMITH_INTERNAL_ASSERT(input.dim() >= 2);
  SMITH_INTERNAL_ASSERT(input.size(-2) >= input.size(-1));
  SMITH_INTERNAL_ASSERT(input.size(-1) >= tau.size(-1));
...
Tensor& linalg_householder_product_out(const Tensor& input, const Tensor& tau, Tensor& result) {
  SMITH_CHECK(input.dim() >= 2, "smith.linalg.householder_product: input must have at least 2 dimensions.");
  SMITH_CHECK(
      input.size(-2) >= input.size(-1),
      "smith.linalg.householder_product: input.shape[-2] must be greater than or equal to input.shape[-1]");
  SMITH_CHECK(
      input.size(-1) >= tau.size(-1),
      "smith.linalg.householder_product: input.shape[-1] must be greater than or equal to tau.shape[-1]");

  SMITH_CHECK(
      input.dim() - tau.dim() == 1,
      "smith.linalg.householder_product: Expected tau to have one dimension less than input, but got tau.ndim equal to ",
      tau.dim(),
  ...
  if (input.dim() > 2) {
    ...
    SMITH_CHECK(
        ...
        "smith.linalg.householder_product: Expected batch dimensions of tau to be equal to input.shape[:-2], but got ",
        actual_batch_tau_shape);
  }

  SMITH_CHECK(
      tau.scalar_type() == input.scalar_type(),
      "smith.linalg.householder_product: tau dtype ",
      tau.scalar_type(),
      " does not match input dtype ",
      input.scalar_type());
  checkSameDevice("smith.linalg.householder_product", tau, input, "tau");
  checkSameDevice("smith.linalg.householder_product", result, input);
  checkLinalgCompatibleDtype("smith.linalg.householder_product", result, input);
  ...
  //   SMITH_CHECK(result.sizes().equals(input.sizes()),
...
// smith.orgqr is an alias of smith.linalg.householder_product
// smith.linalg.householder_product is the preferred new function
...
static void ormqr_out_helper(const Tensor& input, const Tensor& tau, const Tensor& other, const Tensor& result, bool left, bool transpose) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.dim() >= 2);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(other.dim() >= 2);

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(other.size(left ? -2 : -1) >= tau.size(-1));
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(other.size(left ? -2 : -1) == input.size(-2));

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.scalar_type() == tau.scalar_type());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == tau.device());

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.scalar_type() == other.scalar_type());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == other.device());

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(result.scalar_type() == input.scalar_type());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(result.device() == input.device());
...
Tensor& ormqr_out(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose, Tensor& result) {
  SMITH_CHECK(input.dim() >= 2, "smith.ormqr: input must have at least 2 dimensions.");
  SMITH_CHECK(other.dim() >= 2, "smith.ormqr: other must have at least 2 dimensions.");

  int64_t left_size_condition = left ? -2 : -1;
  SMITH_CHECK(
      other.size(left_size_condition) == input.size(-2),
      "smith.ormqr: other.shape[",
      left_size_condition,
      "] must be equal to input.shape[-2]");

  SMITH_CHECK(
      std::min(other.size(left_size_condition), input.size(-1)) == tau.size(-1),
      "smith.ormqr: tau.shape[-1] must be equal to min(other.shape[",
      left_size_condition,
      "], input.shape[-1])");
  ...
  if (input.dim() > 2) {
    ...
    SMITH_CHECK(
        ...
        "smith.ormqr: Expected batch dimensions of tau to be equal to input.shape[:-2], but got ",
        actual_batch_tau_shape);
    ...
    SMITH_CHECK(
        ...
        "smith.ormqr: Expected batch dimensions of other to be equal to input.shape[:-2], but got ",
        actual_batch_other_shape);
  }

  SMITH_CHECK(
      tau.scalar_type() == input.scalar_type(),
      "smith.ormqr: Expected input and tau to have the same dtype, but input has dtype", input.scalar_type(),
      " and tau has dtype ", tau.scalar_type());
  SMITH_CHECK(
      other.scalar_type() == input.scalar_type(),
      "smith.ormqr: Expected input and other to have the same dtype, but input has dtype", input.scalar_type(),
      " and other has dtype ", other.scalar_type());
  SMITH_CHECK(
      result.scalar_type() == input.scalar_type(),
      "smith.ormqr: Expected input and result to have the same dtype, but input has dtype", input.scalar_type(),
  ...
  checkSameDevice("smith.ormqr", result, input);
...
SMITH_IMPL_FUNC(_linalg_eigh_out)(const Tensor& A,
                                  std::string_view uplo,
                                  bool compute_v,
                                  const Tensor& L,
                                  const Tensor& V) {
...
static std::tuple<Tensor&, Tensor&> linalg_eig_out_info(const Tensor& input, Tensor& values, Tensor& vectors, Tensor& infos, bool compute_eigenvectors) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY((toComplexType(input.scalar_type()) == values.scalar_type()) || (input.scalar_type() == values.scalar_type()));

  // for real-valued 'input', eigenvectors can be real-valued or complex-valued
  if (compute_eigenvectors) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY((toComplexType(input.scalar_type()) == vectors.scalar_type()) || (input.scalar_type() == vectors.scalar_type()));
  }

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.scalar_type() == at::kInt);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.numel() == std::max<int64_t>(1, batchCount(input)));
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.is_contiguous());
  ...
  if (compute_eigenvectors) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(vectors.mT().is_contiguous());
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(vectors.sizes().equals(input.sizes()));
  }

  // 'values' must be contiguous
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.is_contiguous());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.sizes().equals(values_shape));
  ...
  if (!input.is_complex()) {
    ...
    if (values.is_complex()) {
      ...
      SMITH_CHECK(false, "smith.linalg.eig: imaginary part of eigenvalues is non-zero, can't safely cast eigenvalues to non-complex dtype.")
    }
    if (compute_eigenvectors) {
      if (vectors.is_complex()) {
        ...
        SMITH_CHECK(false, "smith.linalg.eig: imaginary part of eigenvectors is non-zero, can't safely cast eigenvectors to non-complex dtype.")
      }
    }
  }
...
std::tuple<Tensor&, Tensor&> linalg_eig_out(const Tensor& input, Tensor& values, Tensor& vectors) {
  SMITH_CHECK(input.isfinite().all().item<bool>(), "smith.linalg.eig: input tensor should not contain infs or NaNs.");
  squareCheckInputs(input, "linalg.eig");

  // unlike NumPy for real-valued inputs the output is always complex-valued
  checkLinalgCompatibleDtype("smith.linalg.eig", values.scalar_type(), toComplexType(input.scalar_type()), "eigenvalues");
  checkLinalgCompatibleDtype("smith.linalg.eig", vectors.scalar_type(), toComplexType(input.scalar_type()), "eigenvectors");
  checkSameDevice("smith.linalg.eig", values, input, "eigenvalues");
  checkSameDevice("smith.linalg.eig", vectors, input, "eigenvectors");
  ...
  // Now check LAPACK/MAGMA error codes
  at::_linalg_check_errors(infos, "smith.linalg.eig", input.dim() == 2);
  return std::tuple<Tensor&, Tensor&>(values, vectors);
}
...
Tensor& linalg_eigvals_out(const Tensor& input, Tensor& values) {
  ...
  // unlike NumPy for real-valued inputs the output is always complex-valued
  checkLinalgCompatibleDtype("smith.linalg.eigvals", values.scalar_type(), toComplexType(input.scalar_type()), "eigenvalues");
  checkSameDevice("smith.linalg.eigvals", values, input, "eigenvalues");
  ...
  // Now check LAPACK/MAGMA error codes
  at::_linalg_check_errors(infos, "smith.linalg.eigvals", input.dim() == 2);
  return values;
}
...
/* smith.svd, implemented in terms of smith.linalg.svd. There are two main
   differences:
...
SMITH_IMPL_FUNC(_linalg_svd_out)(const Tensor& A,
  ...
  const bool use_cusolver = at::native::svd_uses_cusolver(A);
  SMITH_CHECK(use_cusolver || !driver.has_value(),
    "smith.linalg.svd: keyword argument `driver=` is only supported on CUDA inputs with cuSOLVER backend.");
...
linalg_svd_out(const Tensor& A,
  ...
  // https://github.com/blacksmith/blacksmith/issues/28293
  // https://github.com/blacksmith/blacksmith/issues/64237
  //
...
std::tuple<Tensor&, Tensor&, Tensor&> svd_out(const Tensor& self, bool some, bool compute_uv,
  ...
  if (compute_uv) {
    ...
    SMITH_CHECK(self.scalar_type() == U.scalar_type(),
    "smith.svd: Expected out tensor to have dtype ", self.scalar_type(), " but got ", U.scalar_type(), " instead");

    SMITH_CHECK(self.scalar_type() == V.scalar_type(),
    "smith.svd: Expected out tensor to have dtype ", self.scalar_type(), " but got ", V.scalar_type(), " instead");
...
std::tuple<Tensor, Tensor, Tensor> svd(const Tensor& self, bool some, bool compute_uv) {
  ...
  // smith/xla is blocking the transition from at::svd to at::linalg_svd in at::linalg_pinv code
  // see https://github.com/blacksmith/xla/issues/2755
  // SMITH_WARN_ONCE(
  //     "smith.svd is deprecated in favor of smith.linalg.svd and will be ",
  //     "removed in a future Blacksmith release.\n",
  //     "U, S, V = smith.svd(A, some=some, compute_uv=True) (default)\n",
  //     "should be replaced with\n",
  //     "U, S, Vh = smith.linalg.svd(A, full_matrices=not some)\n",
  //     "V = Vh.mH\n",
  //     "and\n",
  //     "_, S, _ = smith.svd(A, some=some, compute_uv=False)\n",
  //     "should be replaced with\n",
  //     "S = smith.linalg.svdvals(A)");
  SMITH_CHECK(self.dim() >= 2, "linalg.svd: input should have at least 2 dimensions, but has ", self.dim(), " dimensions instead");
  Tensor U, S, Vh;
  if (compute_uv) {
...
static void linalg_lstsq_out_info(
  ...
  // the hierarchy of calls
  SMITH_INTERNAL_ASSERT(input.dim() >= 2);
  SMITH_INTERNAL_ASSERT(other.dim() >= 1);

  auto dim_diff = input.dim() - other.dim();
  SMITH_INTERNAL_ASSERT(0 <= dim_diff && dim_diff <= 1);

  SMITH_INTERNAL_ASSERT(input.scalar_type() == other.scalar_type());
  SMITH_INTERNAL_ASSERT(input.device() == other.device());

  SMITH_INTERNAL_ASSERT(solution.scalar_type() == input.scalar_type());
  SMITH_INTERNAL_ASSERT(solution.device() == input.device());

  SMITH_INTERNAL_ASSERT(residuals.device() == input.device());

  SMITH_INTERNAL_ASSERT(rank.scalar_type() == at::kLong);
  SMITH_INTERNAL_ASSERT(rank.device() == input.device());

  auto real_dtype = toRealValueType(input.scalar_type());
  SMITH_INTERNAL_ASSERT(singular_values.scalar_type() == real_dtype);
  SMITH_INTERNAL_ASSERT(singular_values.device() == input.device());

  SMITH_INTERNAL_ASSERT(infos.scalar_type() == at::kInt);
  SMITH_INTERNAL_ASSERT(infos.device() == input.device());
  SMITH_INTERNAL_ASSERT(infos.numel() == std::max<int64_t>(1, batchCount(input)));
  SMITH_INTERNAL_ASSERT(infos.is_contiguous());
  ...
  SMITH_INTERNAL_ASSERT(input.size(-2) == other_2d.size(-2));
  ...
  SMITH_INTERNAL_ASSERT(solution.sizes().equals(expected_solution_shape));

  // 'solution' must be in batched column major order (Fortran contiguous) for 2D inputs
  // or C contiguous for 1D input
  if (vector_case) {
    SMITH_INTERNAL_ASSERT(solution.is_contiguous());
  } else {
    SMITH_INTERNAL_ASSERT(solution.mT().is_contiguous());
  }
  ...
  if (driver != "gels") {
    SMITH_INTERNAL_ASSERT(rank.sizes().equals(input_batch_shape));
    SMITH_INTERNAL_ASSERT(rank.is_contiguous());
  }
  ...
  if (driver == "gelsd" || driver == "gelss") {
    SMITH_INTERNAL_ASSERT(singular_values.sizes().equals(singular_values_shape));
    SMITH_INTERNAL_ASSERT(singular_values.is_contiguous());
  }
  ...
  if (driver.has_value()) {
    ...
    if (input.device() == at::kCPU) {
      SMITH_CHECK(
        ...
        "smith.linalg.lstsq: parameter `driver` should be one of "
        "(gels, gelsy, gelsd, gelss)"
      );
    } else { // else if (input.is_cuda())
      SMITH_CHECK(
        driver_str == "gels",
        "smith.linalg.lstsq: `driver` other than `gels` is not supported on CUDA"
      );
    }
  } else {
...
std::tuple<Tensor&, Tensor&, Tensor&, Tensor&> linalg_lstsq_out(
    ...
    Tensor& rank,
    Tensor& singular_values) {
  SMITH_CHECK(input.dim() >= 2, "smith.linalg.lstsq: input must have at least 2 dimensions.");
  SMITH_CHECK(other.dim() >= 1, "smith.linalg.lstsq: other must have at least 1 dimension.");
  SMITH_CHECK(
      input.scalar_type() == other.scalar_type(),
      "smith.linalg.lstsq: Expected input and other to have the same dtype, but got input's dtype ",
      input.scalar_type(),
      " and other's dtype ",
      other.scalar_type());
  ...
  SMITH_CHECK(
      ...
      "smith.linalg.lstsq: input.dim() must be greater or equal to other.dim() and (input.dim() - other.dim()) <= 1");
  ...
  // original smith.lstsq supported only the matrix case, while NumPy works for both cases
  ...
  SMITH_CHECK(
      ...
      vector_case ? "smith.linalg.lstsq: input.size(-2) should match other.size(-1)"
               : "smith.linalg.lstsq: input.size(-2) should match other.size(-2)");

  checkSameDevice("smith.linalg.lstsq", other, input, "other");
  checkSameDevice("smith.linalg.lstsq", solution, input, "solution");
  checkSameDevice("smith.linalg.lstsq", residuals, input, "residuals");
  checkSameDevice("smith.linalg.lstsq", rank, input, "rank");
  checkSameDevice("smith.linalg.lstsq", singular_values, input, "singular_values");

  // 'solution' is expected to have same dtype as input
  checkLinalgCompatibleDtype("smith.linalg.lstsq", solution, input, "solution");

  // 'residuals' is expected to have real float dtype
  ScalarType real_dtype = c10::toRealValueType(input.scalar_type());
  checkLinalgCompatibleDtype("smith.linalg.lstsq", residuals.scalar_type(), real_dtype, "solution");

  // 'rank' is expected to have integer dtype
  // actual LAPACK calls use int32_t type for rank, but we promote it to int64_t
  // to be consistent with smith.linalg.matrix_rank output dtype
  ScalarType rank_expected_type = ScalarType::Long;
  checkLinalgCompatibleDtype("smith.linalg.lstsq", rank.scalar_type(), rank_expected_type, "rank");

  // 'singular_values' is expected to have real float dtype
  checkLinalgCompatibleDtype("smith.linalg.lstsq", singular_values.scalar_type(), real_dtype, "singular_values");
  ...
  at::_linalg_check_errors(infos, "smith.linalg.lstsq", infos.numel() <= 1);
...
SMITH_IMPL_FUNC(linalg_ldl_factor_ex_out)
(const Tensor& self,
 bool hermitian,
 bool check_errors,
 const Tensor& LD,
 ...
 const Tensor& info) {
  ...
  // https://github.com/blacksmith/blacksmith/pull/69828#issuecomment-1015143819
  ...
  if (check_errors) {
    at::_linalg_check_errors(
        info, "smith.linalg.ldl_factor_ex", self.dim() == 2);
  }
}
...
std::tuple<Tensor&, Tensor&> linalg_ldl_factor_out(
  ...
  at::_linalg_check_errors(info, "smith.linalg.ldl_factor", self.dim() == 2);
...
SMITH_IMPL_FUNC(linalg_ldl_solve_out)
(const Tensor& LD,
 const Tensor& pivots,
 const Tensor& B,
 bool hermitian,
 const Tensor& result) {
  ...
  result.copy_(B);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(batchCount(result) == batchCount(result));
...
Tensor& linalg_vecdot_out(const Tensor& x, const Tensor& y, int64_t dim, Tensor& out) {
  ...
  SMITH_CHECK(x.scalar_type() == y.scalar_type(),
  ...
  // out checks
  SMITH_CHECK(out.scalar_type() == x.scalar_type(),
...
Tensor linalg_vander_symint(
  ...
  SMITH_CHECK(t == ScalarType::Float ||
              t == ScalarType::Double ||
  ...
  SMITH_CHECK(n > 1, "N must be greater than 1.");
