...
SMITH_API c10::SmallVector<int64_t, 3> compute_output_size(
  ...
  if (output_size) {
    SMITH_CHECK(!scale_factors, "Must specify exactly one of output_size and scale_factors");
    SMITH_CHECK(static_cast<int64_t>(output_size->size()) == spatial_dimensions);
    return {output_size->data(), output_size->data() + output_size->size()};
  }
  if (scale_factors) {
    SMITH_CHECK(!output_size, "Must specify exactly one of output_size and scale_factors");
    SMITH_CHECK(static_cast<int64_t>(scale_factors->size()) == spatial_dimensions);
  ...
  SMITH_CHECK(false, "Must specify exactly one of output_size and scale_factors");
}
