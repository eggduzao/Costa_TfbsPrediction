...
{
  SMITH_CHECK(std::numeric_limits<dest_t>::min() <= v && v <= std::numeric_limits<dest_t>::max(),
...
inline T pooling_output_shape(
    ...
    SMITH_CHECK(stride != 0, "stride should not be zero");
    SMITH_CHECK(pad >= 0,
                "pad must be non-negative, but got pad: ", pad);
    SMITH_CHECK(pad <= ((kernelSize - 1) * dilation + 1) / 2,
  ...
  int64_t outputHeight, int64_t outputWidth, MemoryFormat memory_format)
{
  ...
  SMITH_CHECK(dW > 0 && dH > 0,
              ...
              "dH: ", dH, " dW: ", dW);
  SMITH_CHECK(dilationH > 0 && dilationW > 0,
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast){
    ...
    SMITH_CHECK((ndim == 4 && valid_dims && input.size(3) != 0),
  ...
  bool check_input_size=false)
{
  ...
  SMITH_CHECK(kT > 0 && kW > 0 && kH > 0,
              "kernel size should be greater than zero, but got ",
              "kT: ", kT, " kH: ", kH, " kW: ", kW);
  SMITH_CHECK(dT > 0 && dW > 0 && dH > 0,
              "stride should be greater than zero, but got ",
              "dT: ", dT, " dH: ", dH, " dW: ", dW);
  SMITH_CHECK(dilationT > 0 && dilationW > 0 && dilationH > 0,
              "dilation should be greater than zero, but got ",
              "dilationT: ", dilationT, " dilationH: ", dilationH, " dilationW: ", dilationW);

  SMITH_CHECK(ndim == 4 || ndim == 5,
