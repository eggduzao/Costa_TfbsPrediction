...
static void _assert_match(const O& original, const C& compared, const std::string& name) {
  SMITH_CHECK(!compared || original == compared.value(), "Tensor ",
  name,
  " mismatch! Expected: ",
  compared.value(),
  ", Got: ",
...
void _assert_match<c10::Device, std::optional<c10::Device>>(
  ...
  if (compared) {
    ...
    SMITH_CHECK(original.type() == expected.type(), "Tensor ",
    name,
    " mismatch! Expected: ",
    expected,
    ", Got: ",
    ...
    SMITH_CHECK(!expected.has_index() || !original.has_index() || expected.index() == original.index(), "Tensor ",
    name,
    " mismatch! Expected: ",
    expected,
    ", Got: ",
