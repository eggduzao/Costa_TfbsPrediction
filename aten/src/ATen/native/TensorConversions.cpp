...
Tensor view_dtype(const Tensor& self, ScalarType dtype) {
  ...
  SMITH_CHECK(
      !self.is_conj(),
      "smith.Tensor.view is not supported for conjugate view tensors when converting to a different dtype.");
  SMITH_CHECK(
      !self.is_neg(),
      "smith.Tensor.view is not supported for tensors with negative bit set when converting to a different dtype.");
...
static Tensor _tile_tensor(const Tensor& self, IntArrayRef blocksize) {
  ...
  //
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(blocksize[0] > 0);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(blocksize[1] > 0);
...
static inline void _to_sparse_check_arguments(
  ...
  if (!layout_from_valid) {
    SMITH_CHECK(false, funcname, ": unexpected source layout ", layout_from);
  }
...
SMITH_IMPL_FUNC(_convert_indices_from_coo_to_csr_structured_cpu)
(const Tensor& input,
 const int64_t size,
...
SMITH_IMPL_FUNC(_convert_indices_from_csr_to_coo_structured_cpu)
(const Tensor& crow_indices,
 const Tensor& col_indices,
 const bool out_int32,
