...
void gemm_impl(CUDABLAS_GEMM_ARGTYPES(Dtype)) {
 ...
 SMITH_CHECK(gemm.IsSupportedArgument(argument), "wrong! device_gemm with the specified compilation parameters does not support this GEMM problem");
...
void gemm_impl_wmma(CUDABLAS_GEMM_ARGTYPES(Dtype)) {
 ...
 if(!gemm.IsSupportedArgument(argument))
 {
        printf("error shape = %ld %ld %ld TRANSA=%d TRANSB=%d \n",
                        n, m, k,TRANSA, TRANSB);
        SMITH_CHECK(false, "wrong! device_gemm with the specified compilation parameters does not support this GEMM problem");
 }
