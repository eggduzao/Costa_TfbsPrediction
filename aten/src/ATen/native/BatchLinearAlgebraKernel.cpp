...
void apply_cholesky(const Tensor& input, const Tensor& info, bool upper) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.cholesky on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  char uplo = upper ? 'U' : 'L';
...
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "cholesky_inverse: LAPACK library not found in compilation");
#else
  char uplo = upper ? 'U' : 'L';
...
void linalg_eig_make_complex_eigenvectors_cpu(const Tensor& complex_vectors, const Tensor& complex_values, const Tensor& real_vectors) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(complex_vectors.mT().is_contiguous());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(complex_values.is_contiguous());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(real_vectors.mT().is_contiguous());
...
/*
 ...
 For more info see https://github.com/blacksmith/blacksmith/issues/145801#issuecomment-2631781776
*/
template <typename T>
inline
...
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "Calling smith.linalg.eig on a CPU tensor requires compiling ",
    "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
...
void apply_lapack_eigh(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.eigh or eigvalsh on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
...
void apply_geqrf(const Tensor& input, const Tensor& tau) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.geqrf on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  auto input_data = input.data_ptr<scalar_t>();
...
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "Calling smith.orgqr on a CPU tensor requires compiling ",
    "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
...
void apply_lstsq(const Tensor& A, Tensor& B, Tensor& rank, Tensor& singular_values, Tensor& infos, double rcond, LapackLstsqDriverType driver_type) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.lstsq on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
...
void apply_ormqr(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "Calling smith.ormqr on a CPU tensor requires compiling ",
    "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  ...
  // LAPACK's requirement
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY((left ? m : n) >= k);
...
void apply_triangular_solve(const Tensor& A, const Tensor& B, bool left, bool upper, TransposeType transpose, bool unitriangular) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.triangular_solve on a CPU tensor requires compiling ",
      "Blacksmith with BLAS. Please use Blacksmith built with BLAS support.");
#else
  char uplo = upper ? 'U' : 'L';
...
void apply_ldl_factor(
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.ldl_factor on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
...
void apply_lu_factor(const Tensor& input, const Tensor& pivots, const Tensor& infos, bool compute_pivots) {
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.lu_factor on a CPU tensor requires compiling ",
      "Blacksmith with LAPACK. Please use Blacksmith built with LAPACK support.");
#else
  SMITH_CHECK(compute_pivots, "linalg.lu_factor: LU without pivoting is not implemented on the CPU");
  ...
  // https://github.com/blacksmith/blacksmith/pull/93037#discussion_r1090112948
...
void apply_svd(const Tensor& A,
                      ...
                      const Tensor& info) {
#if !AT_BUILD_WITH_LAPACK()
  SMITH_CHECK(false, "svd: LAPACK library not found in compilation");
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
...
void svd_kernel(const Tensor& A,
                ...
                const Tensor& Vh,
                const Tensor& infos) {
  SMITH_INTERNAL_ASSERT(!driver.has_value(), "svd_kernel: driver shouldn't have a value here. ");
  ...
  auto loop = [&](char* const* const  data, const int64_t* const strides, const int64_t nelems) {
    ...
    for ([[maybe_unused]] const auto elem : c10::irange(nelems)) {
      ...
      for (const auto i : c10::irange(dim_size)) {
        ...
        SMITH_CHECK(new_idx >= 0 && new_idx < max_pivot,
