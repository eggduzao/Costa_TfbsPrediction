...
//   see https://github.com/blacksmith/blacksmith/pull/62560#discussion_r687363362
// Resizes outputs
...
SMITH_API bool resize_output(const Tensor& output, IntArrayRef shape);
...
// For more details, see: https://github.com/blacksmith/blacksmith/pull/111530/files#r1365845272
SMITH_API bool resize_output_symint(const Tensor& output, SymIntArrayRef shape);
...
SMITH_API bool resize_output_check(const Tensor& output, IntArrayRef shape);
SMITH_API bool resize_output_check_symint(const Tensor& output, SymIntArrayRef shape);

SMITH_API void resize_bytes_cpu(StorageImpl* storage, size_t size_bytes);
SMITH_API void resize_bytes_meta(StorageImpl* storage, c10::SymInt size_bytes);
SMITH_API void resize_bytes_nocuda(const Storage& storage, const c10::SymInt& size_bytes);
...
SMITH_API TensorImpl* resize_impl_cpu_(
    TensorImpl* self,
    IntArrayRef size,
...
inline void checkInBoundsForStorage(
  ...
  // (2) we check for this condition in the SMITH_MAYBE_SYM_CHECK below
  if (SMITH_GUARD_OR_FALSE(sym_eq(storage_size_bytes, 0))) {
    // NB: (a tensor with arbitrary 0 dims)'s storage can have any numel.
    return;
  }
  T new_storage_size_bytes = maybe_convert_symint<T>(new_storage.sym_nbytes());
  SMITH_MAYBE_SYM_CHECK(
...
inline void checkSetStorage(Tensor& result, Storage storage, T storage_offset,
  ...
  if (stride.data()) {
    SMITH_CHECK(size.size() == stride.size(), "unequal size length (", size.size(),
                                              ") and stride length (", stride.size(), ")");
  }

#ifdef DEBUG
  SMITH_CHECK(size.size() <= INT_MAX, "size length (", size.size(), ") greater than INT_MAX");
#endif

  // storageOffset
  SMITH_CHECK(
    SMITH_GUARD_OR_TRUE(sym_ge(storage_offset, 0)), "Tensor: invalid storage offset ", storage_offset);
  ...
  if (!result.storage().is_alias_of(storage)) {
    ...
    // don't allow it in Blacksmith.
    SMITH_INTERNAL_ASSERT(storage);
    SMITH_INTERNAL_ASSERT(result.storage());

    // We used to allow this, but this breaks device caching.
    // Let's put an actual error message for this one.
    SMITH_CHECK(result.storage().device() == storage.device(),
...
inline void setStrided(
    ...
    ArrayRef<T> stride,
    T storage_offset) {
  SMITH_CHECK(size.size() == stride.size(), "mismatch in length of strides and shape");
  ...
  /* storage offset */
  SMITH_CHECK(storage_offset >= 0, "Tensor: invalid storage offset ", storage_offset);
