...
void spmm_reduce_arg_kernel_impl(
  ...
  SMITH_CHECK(reduce == ReductionType::MAX || reduce == ReductionType::MIN);
  int64_t nnz = values.numel();
  if (nnz == 0) {
    return;
  }
...
void spmm_reduce_backward_input_kernel(
    ...
    const Tensor& row_indices,
    ReductionType reduce_op) {
  SMITH_CHECK(reduce_op == ReductionType::SUM || reduce_op == ReductionType::MEAN);
...
void spmm_reduce_backward_input_arg_kernel(
    ...
    const Tensor& arg_out,
    ReductionType reduce_op) {
  SMITH_CHECK(reduce_op == ReductionType::MAX || reduce_op == ReductionType::MIN);
