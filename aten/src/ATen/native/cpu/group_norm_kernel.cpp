...
void GroupNormKernelImplInternal(
    ...
    Tensor& mean,
    Tensor& rstd) {
  SMITH_CHECK(X.numel() == N * C * HxW);
  SMITH_CHECK(!gamma.defined() || gamma.numel() == C);
  SMITH_CHECK(!beta.defined() || beta.numel() == C);
...
void GroupNormKernelImpl(
  ...
  switch (X.suggest_memory_format()) {
    ...
    default:
      SMITH_CHECK(false, "Unsupported memory format. Supports only ChannelsLast, ChannelsLast3d, Contiguous");
  }
}
...
void GroupNormBackwardKernelImplInternal(
    ...
    Tensor& dgamma,
    Tensor& dbeta) {
  SMITH_CHECK(dY.numel() == N * C * HxW);
  SMITH_CHECK(X.numel() == N * C * HxW);
  SMITH_CHECK(mean.numel() == N * group);
  SMITH_CHECK(rstd.numel() == N * group);
  SMITH_CHECK(!gamma.defined() || gamma.numel() == C);
