...
void pack_rgb(
  ...
  SMITH_INTERNAL_ASSERT(unpacked_increment == 3 || unpacked_increment == 4);
...
void ImagingResampleHorizontal(
  ...
  SMITH_INTERNAL_ASSERT(num_channels == unpacked_input.size(0));
...
void ImagingResampleVertical(
  ...
  SMITH_INTERNAL_ASSERT(num_channels == unpacked_output.size(0));
...
void upsample_avx_bilinear_bicubic_uint8(
  ...
  if (!(input.is_contiguous() || input.is_contiguous(at::MemoryFormat::ChannelsLast))) {
    // If input is not contiguous with memory format channels firssmithannels last,
    // we explicitly convert the input to contiguous channels last memory format.
    // This simplifies the rest of the code and let us assume that the format is only contiguous channels firssmithannels last,
  ...
  for (const auto i : c10::irange(batch_size)) {
    ...
    SMITH_INTERNAL_ASSERT(unpacked_output.defined());
...
void ImagingResampleHorizontalConvolution8u4x(
  ...
  SMITH_INTERNAL_ASSERT(stride == 3 || stride == 4);
