...
void _amp_foreach_non_finite_check_and_unscale_cpu_kernel(
  ...
  SMITH_CHECK(inv_scale.is_cpu(), "inv_scale must be a CPU tensor.");
  SMITH_CHECK(found_inf.is_cpu(), "found_inf must be a CPU tensor.");
  ...
  for (const at::Tensor& t : scaled_grads) {
    SMITH_CHECK(t.is_cpu(), "one of scaled_grads was not a CPU tensor.");
    SMITH_CHECK(
...
at::Tensor& _amp_update_scale_cpu_kernel(
  ...
  SMITH_CHECK(growth_tracker.is_cpu(), "growth_tracker must be a CPU tensor.");
  SMITH_CHECK(current_scale.is_cpu(), "current_scale must be a CPU tensor.");
  SMITH_CHECK(found_inf.is_cpu(), "found_inf must be a CPU tensor.");
  SMITH_CHECK(
