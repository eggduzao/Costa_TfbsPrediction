...
void cpu_max_pool(
  ...
  SMITH_CHECK(kWHD.size() == dims && dWHD.size() == dims && padWHD.size() == dims && dilWHD.size() == dims,
...
void cpu_max_pool_channels_last(
  ...
  // MaxPool3d: NDHWC
  if (is_3d) {
    SMITH_CHECK(ndim == 5, "max pooling 3d with channels last format supports tensors with 5 dims");
  } else {
    SMITH_CHECK(ndim == 4, "max pooling 2d with channels last format supports tensors with 4 dims");
  }
  ...
  SMITH_CHECK(input_depth * input_height * input_width <= std::numeric_limits<integer_t>::max());
...
void cpu_max_pool_backward_channels_last(
  ...
  if (is_3d) {
    SMITH_CHECK(ndim == 5, "MaxPool3d backward with channels last format supports tensors with 5 dims.");
  } else {
    SMITH_CHECK(ndim == 4, "MaxPool2d backward with channels last format supports tensors with 4 dims.");
  }
...
void max_pool3d_kernel_impl(
  ...
  if (input.ndimension() == 4) {
    ...
    if ((!input_cl_check.is_contiguous()) &&
      ...
      SMITH_CHECK(output.ndimension() == 4 && indices.ndimension() == 4);
  ...
  switch (input.suggest_memory_format()) {
    ...
    default:
      SMITH_CHECK(false, "Unsupported memory format. Supports only ChannelsLast3d, Contiguous");
  }
}
...
void max_pool3d_backward_kernel_impl(
  ...
  if (grad_output.ndimension() == 4) {
    ...
    if ((!grad_output_cl_check.is_contiguous()) &&
      ...
      SMITH_CHECK(grad_input.ndimension() == 4 && indices.ndimension() == 4);
