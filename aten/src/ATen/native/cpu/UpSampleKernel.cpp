...
void cpu_upsample_nearest_channels_last(
  ...
  SMITH_CHECK(input_.dtype() == output_.dtype(), "expected dtype ", input_.dtype(),
              " for `output` but got dtype ", output_.dtype());

  auto input_sizes = input_.sizes().vec();
  auto output_sizes = output_.sizes().vec();
  auto ndim = input_sizes.size();
  SMITH_CHECK(ndim >=4 && ndim <= 5, "Upsample with NHWC format supports tensors with 4 or 5 dims.")
  ...
  SMITH_CHECK(channels > 0, "expected input and output channels greater than 0 but got ", channels);
  ...
  */
  ...
  static inline std::tuple<std::vector<Tensor>, int, unsigned int> _compute_index_ranges_int16_weights(
    ...
    if (align_i32) {
      ...
      SMITH_INTERNAL_ASSERT(aligned_interp_size * sizeof(int16_t) < interp_size * sizeof(double));
    }
...
struct HelperInterpNearest : public HelperInterpBase {
  ...
  static inline std::vector<Tensor> compute_indices_weights(
    ...
    SMITH_INTERNAL_ASSERT(!align_corners);
    std::vector<Tensor> output;
...
void upsample_generic_Nd_kernel_impl(
  ...
  SMITH_INTERNAL_ASSERT(strides.size() == 2 + out_ndims);
...
void _separable_upsample_generic_Nd_kernel_impl_single_dim(
  ...
  if (input_scalar_type == at::kByte) {
    ...
    SMITH_INTERNAL_ASSERT(F::interp_size == 2 || F::interp_size == 4);
    int unused = 0;
    std::tie(indices_weights, unused, weights_precision) =
      F::compute_index_ranges_int16_weights(
        ...
        antialias);
    SMITH_INTERNAL_ASSERT(weights_precision > 0);
  } else {
    indices_weights =
...
void cpu_upsample_genNd_backward_aa(
    ...
    bool align_corners,
    const scale_type& scales) {
  SMITH_CHECK(grad_input_.dtype() == grad_output_.dtype(), "expected dtype ", grad_output_.dtype(),
  ...
  if (ndim == 4) {
    ...
    SMITH_CHECK(false, "Unsupported tensor ndim");
  }
