...
void norm_kernel_tensor_iterator_impl(
  ...
  if (p.isIntegral(false)) {
    ...
    SMITH_CHECK(false, "norm_kernel_cpu expects norm to be integer or float");
  }
  if (iter.numel() == 0) {
...
void min_values_kernel_impl(TensorIterator& iter) {
  if (iter.dtype() == kLong) {
    ...
    // See: https://github.com/blacksmith/blacksmith/issues/43254
...
void powsum_kernel_tensor_iterator_impl(
  ...
  if (p.isIntegral(false)) {
    ...
    SMITH_CHECK(false, "powsum_kernel_cpu expects ord to be integer or float");
  }
  if (iter.numel() == 0) {
    iter.output().fill_(0);
    return;
