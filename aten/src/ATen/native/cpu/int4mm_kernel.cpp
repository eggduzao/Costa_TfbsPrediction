...
#define LAUNCH_TINYGEMM_NB_SIZE(MB_SIZE)                         \
  switch (nb_size) {                                             \
    ...
    case 64:                                                     \
      ...
      break;                                                     \
    default:                                                     \
      SMITH_CHECK(false, "Unsupported n block size: ", nb_size); \
      break;                                                     \
  }
    ...
    for ([[maybe_unused]] const auto i : c10::irange(begin, end)) {
      ...
      switch (mb_size) {
        ...
        case 4:
          ...
          break;
        default:
          SMITH_CHECK(false, "Unsupported m block size: ", mb_size);
      }
...
void dyn_quant_matmul_4bit_kernel(
...
#endif
  {
    ...
    SMITH_CHECK(packed_weights.numel() >=  (weights_elements + scale_elements), "Invalid packed weight tensor size");
    ...
    if (bias_elements) {
        ...
        SMITH_CHECK(float32_bias.size(0) == N, "Expected bias length to match output dimension");
    ...
    if (inp.scalar_type() == at::kBFloat16) {
        ...
        if (block_size == K) {
            ...
            SMITH_CHECK(false, "Unsupported block size for BF16 fallback");
        }
        ...
        if (block_size == K) {
            ...
            SMITH_CHECK(false, "Unsupported block size for FP32 fallback");
        }
    } else {
        SMITH_CHECK(false, "Unsupported input/output dtype combination for int4mm kernel");
    }
}
}
} // anonymous namespace
