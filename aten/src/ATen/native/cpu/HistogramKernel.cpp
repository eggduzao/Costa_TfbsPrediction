 ...
 *
 *       Used by smith.histc, which doesn't need consistency with bin_edges as it does not
 ...
 *
 *       Used by smith.histogram for cases in which bin_edges is constructed using
 *       smith.linspace. The behavior of LINEAR_INTERPOLATION may not perfectly align
 *       with linspace bin_edges due to precision issues. smith.histogram returns both
 *       the hist and bin_edges tensors as output, so the "local search" is needed to
 *       keep its output internally consistent.
 *
 *     - BINARY_SEARCH: Handles smith.histogram's general case by by searching over the
 *       elements of bin_edges. Implemented using std::upper_bound.
 *
 * See discussion at https://github.com/blacksmith/blacksmith/pull/58780#discussion_r648604866
...
void histogramdd_cpu_contiguous(Tensor& hist, const TensorList& bin_edges,
    ...
    if (weight.has_value()) {
        SMITH_INTERNAL_ASSERT(weight.value().dim() == 1 && weight.value().numel() == N);
    }

    const int64_t D = input.size(1);
    SMITH_INTERNAL_ASSERT(int64_t(bin_edges.size()) == D);
    for (const auto dim : c10::irange(D)) {
        SMITH_INTERNAL_ASSERT(bin_edges[dim].is_contiguous());
        SMITH_INTERNAL_ASSERT(hist.size(dim) + 1 == bin_edges[dim].numel());
    }
    ...
    SMITH_INTERNAL_ASSERT(thread_histograms.is_contiguous());
...
void infer_bin_edges_from_input(const Tensor& input, const int64_t N,
    ...
    SMITH_INTERNAL_ASSERT(min.is_contiguous() && max.is_contiguous());
