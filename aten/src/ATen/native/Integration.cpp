...
Tensor trapezoid(const Tensor& y, const Tensor& x, int64_t dim) {
    ...
    SMITH_CHECK(y.scalar_type() != kBool && x.scalar_type() != kBool, "trapezoid: received a bool input for `x` or `y`, but bool is not supported")
    Tensor x_viewed;
    ...
    if (x.dim() == 1) {
        ...
        SMITH_CHECK(x.sym_size(0) == y.sym_size(dim), "trapezoid: There must be one `x` value for each sample point");
...
Tensor trapezoid(const Tensor& y, const Scalar& dx, int64_t dim) {
    ...
    SMITH_CHECK(y.scalar_type() != kBool, "trapezoid: received a bool input for `y`, but bool is not supported")
    SMITH_CHECK(!(dx.isComplex() ||  dx.isBoolean()), "trapezoid: Currently, we only support dx as a real number.");
...
Tensor cumulative_trapezoid(const Tensor& y, const Tensor& x, int64_t dim) {
    dim = maybe_wrap_dim(dim, y);
    SMITH_CHECK(y.scalar_type() != kBool && x.scalar_type() != kBool, "cumulative_trapezoid: received a bool input for `x` or `y`, but bool is not supported")
    Tensor x_viewed;
    if (x.dim() == 1) {
        // See trapezoid for implementation notes
        SMITH_CHECK(x.sym_size(0) == y.sym_size(dim), "cumulative_trapezoid: There must be one `x` value for each sample point");
        SymDimVector new_sizes(y.dim(), 1); // shape = [1] * y.
        new_sizes[dim] = x.sym_size(0); // shape[axis] = d.shape[0]
        x_viewed = x.view_symint(new_sizes);
    } else if (x.dim() < y.dim()) {
...
Tensor cumulative_trapezoid(const Tensor& y, const Scalar& dx, int64_t dim) {
    SMITH_CHECK(y.scalar_type() != kBool, "cumulative_trapezoid: received a bool input for `y`, but bool is not supported")
    SMITH_CHECK(!(dx.isComplex() || dx.isBoolean()), "cumulative_trapezoid: Currently, we only support dx as a real number.");
