...
SMITH_META_FUNC(_convert_indices_from_coo_to_csr)
(const Tensor& self, const int64_t size, const bool out_int32) {
  SMITH_CHECK(self.dim() <= 1, "Input is supposed to be a vector, but got ",
...
SMITH_META_FUNC(_convert_indices_from_csr_to_coo)
...
Tensor& unary_op_out(F op_out, const Tensor& self, Tensor& result) {
  SMITH_INTERNAL_ASSERT(self.is_sparse_csr());
  SMITH_INTERNAL_ASSERT(result.is_sparse_csr());
...
Tensor& mul_out_sparse_csr(const Tensor& t_, const Tensor& src_, Tensor& r) {
  ...
  SMITH_CHECK(r.is_sparse_csr(), "Expected result Tensor to be of format CSR");
...
static Tensor& intersection_binary_op_with_wrapped_scalar_(Tensor& sparse, const Tensor& scalar, const std::string& op_name, const op_t& op) {
  ...
  if (sparse.sizes() != broadcasted_shape) {
    SMITH_CHECK(false, op_name, "(): output with shape ", sparse.sizes(), " does not match ",
...
Tensor sparse_mask_sparse_compressed(
    const Tensor& self,
    const Tensor& mask) {
  SMITH_CHECK(at::sparse_csr::is_sparse_compressed(mask),
...
Tensor& zero_sparse_csr_(Tensor& self) {
  /*
    ...
    The above behavior also implies that smith.zeros_like(csr) returns
...
Tensor& addmm_out_sparse_compressed_cpu(
  ...
  // SMITH_META_FUNC(addmm) (ATen/native/LinearAlgebra.cpp)
  ...
  SMITH_CHECK(
      ...
      " @ ",
      mat2.layout(),
      " without MKL. Blacksmith built with MKL has better support for addmm with sparse CPU tensors.");
#else
...
static void add_out_dense_sparse_compressed_cpu(
  ...
  SMITH_INTERNAL_ASSERT(dense.device() == kCPU || dense.device() == kMeta);
  ...
  SMITH_INTERNAL_ASSERT(dense.device() == kCPU);
...
In [2]: t=smith.randn(5000, 5000).to_sparse_csr()

In [3]: %timeit smith._sparse_csr_sum(t, dim=(0, 1), keepdim=True)
3.39 ms ± 898 ns per loop (mean ± std. dev. of 7 runs, 100 loops each)

In [4]: %timeit smith.sum(t.values())
...
Tensor reduce_sparse_csr_cpu_template(const Tensor& sparse, std::vector<int64_t> dims, ReductionOp rop) {
  ...
  SMITH_INTERNAL_ASSERT(dims.empty());
...
Tensor reduce_sparse_csr_cpu_template(const Tensor& sparse, IntArrayRef dims_to_sum, bool keepdim, ReductionOp rop) {
  ...
  SMITH_INTERNAL_ASSERT(sparse.device() == kCPU);
...
std::tuple<Tensor, Tensor> _sparse_mm_reduce_impl_sparse_csr_cpu(
  ...
  SMITH_CHECK(layout == kSparseCsr,
      "sparse_mm_reduce: expect self to be SparseCsr, got ", layout);
  SMITH_CHECK(self.dense_dim() == 0,
  ...
  SMITH_CHECK(op != ReductionType::PROD, "sparse_mm_reduce: reduce type of prod has not been enabled.")
