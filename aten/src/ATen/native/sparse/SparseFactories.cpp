...
Tensor spdiags(
  ...
  SMITH_CHECK(diagonals_2d.dim() == 2, "Diagonals must be vector or matrix");
  SMITH_CHECK(shape.size() == 2, "Output shape must be 2d");
  auto offsets_1d = offsets.dim() == 0 ? offsets.unsqueeze(0) : offsets;
  SMITH_CHECK(offsets_1d.dim() == 1, "Offsets must be scalar or vector");
  SMITH_CHECK(
