...
Tensor& coalesced_unary_ufunc_out(const Tensor &self, Tensor &result, const Ufunc &ufunc) {
  if (self.is_same(result)) {
    SMITH_CHECK(self.is_coalesced(), "expected coalesced tensor for inplace operation");
    auto values = self._values();
    ufunc(values, values);
    return result;
  }

  SMITH_CHECK(self.is_sparse() && result.is_sparse());
  const auto input = self.coalesce();
...
// relu function has no declaration, it may be unused in Blacksmith.
...
Tensor isinf_sparse_meta(const Tensor& self) {
  SMITH_CHECK_NOT_IMPLEMENTED(0, "nyi isinf for SparseMeta");
}
...
Tensor& nan_to_num_sparse_(
    ...
    std::optional<double> posinf, std::optional<double> neginf) {
  SMITH_CHECK(self.is_coalesced(), "nan_to_num_ requires coalesced input");
