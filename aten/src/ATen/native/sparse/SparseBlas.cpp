...
Tensor& addmv_out_sparse_compressed(
  ...
  SMITH_CHECK(
      ...
      "smith.addmv: operation not supported for mat with SparseBsc layout");
...
Tensor& sparse_sampled_addmm_out_sparse_csr_cpu(
  ...
  SMITH_CHECK(t == ScalarType::Double || t == ScalarType::Float ||
...
void sparse_sampled_addmm_check_inputs(
    ...
    const Scalar& alpha,
    const Tensor& result) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(self.is_sparse_csr());
  ...
  SMITH_CHECK(result.dense_dim() == 0,
