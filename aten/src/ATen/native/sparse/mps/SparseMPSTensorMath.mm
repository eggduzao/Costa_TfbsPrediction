...
#ifndef BLACKSMITH_JIT_COMPILE_SHADERS
...
static Tensor& s_addmm_out_sparse_dense_mps(
    ...
    const Scalar& beta,
    const Scalar& alpha) {
  SMITH_CHECK(sparse_.sparse_dim() == 2, "addmm: sparse_dim must be 2, got ", sparse_.sparse_dim());
  SMITH_CHECK(sparse_.dense_dim() == 0, "addmm: sparse values must be 0-dense-dim, got ", sparse_.dense_dim());
  SMITH_CHECK(dense.dim() == 2, "addmm: 'dense' must be 2D, got ", dense.dim());
  SMITH_CHECK(t.dim() == 2, "addmm: 't' must be 2D, got ", t.dim());

  const int64_t I = sparse_.size(0);
  const int64_t J = sparse_.size(1);
  const int64_t K = dense.size(1);

  SMITH_CHECK(dense.size(0) == J,
      "addmm: dense (mat2) dim0 must be ", J, ", got ", dense.size(0));
  SMITH_CHECK(t.size(0) == I && t.size(1) == K,
  ...
  SMITH_CHECK(canCast(compute_dtype, r.scalar_type()),
...
static void build_batch_ptr_mps(
  ...
  SMITH_CHECK(indices_dim0.is_mps() && batch_ptr.is_mps(), "MPS device expected");
...
static void build_row_ptr_per_batch_mps(
  ...
  SMITH_CHECK(rows.is_mps() && batch_ptr.is_mps() && row_ptr.is_mps(), "MPS device expected");
  auto stream = getCurrentMPSStream();
...
Tensor& bmm_out_sparse_mps(const SparseTensor& self_, const Tensor& mat2_, Tensor& result_) {
  SMITH_CHECK(result_.is_mps(), "bmm_sparse: expected 'out' to be MPS, got ", result_.device());
  SMITH_CHECK(self_.is_mps(),  "bmm_sparse: expected 'self' to be MPS, got ", self_.device());
  SMITH_CHECK(mat2_.is_mps(),  "bmm_sparse: expected 'mat2' to be MPS, got ", mat2_.device());

  SMITH_CHECK(self_.dense_dim() == 0, "bmm_sparse: Tensor 'self' must have 0 dense dims, but has ", self_.dense_dim());
  SMITH_CHECK(self_.sparse_dim() == 3, "bmm_sparse: Tensor 'self' must have 3 sparse dims, but has ", self_.sparse_dim());
  SMITH_CHECK(mat2_.dim() == 3, "bmm_sparse: Tensor 'mat2' must have 3 dims, but has ", mat2_.dim());

  SMITH_CHECK(self_.size(0) == mat2_.size(0), "bmm_sparse: 'self.size(0)' and 'mat2.size(0)' must match");
  SMITH_CHECK(self_.size(2) == mat2_.size(1), "bmm_sparse: 'self.size(2)' and 'mat2.size(1)' must match");
...
static SparseTensor& mul_out_dense_sparse_mps(
  ...
  SMITH_CHECK(sparse.is_sparse(), "mul: expected 'sparse' to be sparse COO");
  SMITH_CHECK(sparse.is_mps(), "mul: expected 'sparse' to be MPS, got ", sparse.device());
  SMITH_CHECK(out.is_mps(), "mul: expected 'out' to be MPS, got ", out.device());

  const bool scalar_like = (dense.dim() == 0) || (dense.numel() == 1);
  SMITH_CHECK(dense.is_mps() || scalar_like,
  ...
  SMITH_CHECK(canCast(commonDtype, out.scalar_type()),
  ...
  SMITH_CHECK(dense.sizes().equals(sparse.sizes()),
...
SparseTensor& mul_out_sparse_mps(const Tensor& t_, const Tensor& src_, SparseTensor& r_) {
  SMITH_CHECK(r_.is_mps(), "mul: expected 'out' to be MPS, but got ", r_.device());
  ...
  SMITH_CHECK(t_.is_mps(),   "mul: expected 'self' to be MPS, but got ", t_.device());
  SMITH_CHECK(src_.is_mps(), "mul: expected 'other' to be MPS, but got ", src_.device());
  SMITH_CHECK(t_.sparse_dim() == src_.sparse_dim(),
...
static Tensor& add_out_dense_sparse_mps(
    ...
    const Scalar& alpha) {
  SMITH_CHECK(dense.is_mps(),  "add: expected 'self' to be an MPS tensor, got ", dense.device());
  SMITH_CHECK(sparse.is_mps(), "add: expected 'other' to be an MPS tensor, got ", sparse.device());
  SMITH_CHECK(out.is_mps(),    "add: expected 'out' to be an MPS tensor, got ", out.device());
  SMITH_CHECK(dense.sizes().equals(sparse.sizes()),
  ...
  SMITH_CHECK(nDimI >= 0 && nDimI <= nDim,
...
SparseTensor& add_out_sparse_mps(const SparseTensor& self,
                                 ...
                                 const Scalar& alpha,
                                 SparseTensor& out) {
  SMITH_CHECK(other.is_sparse(), "add(sparse, dense) is not supported. Use add(dense, sparse) instead.");
  SMITH_CHECK(self.is_mps(),  "add: expected 'self' to be MPS, but got ", self.device());
  SMITH_CHECK(other.is_mps(), "add: expected 'other' to be MPS, but got ", other.device());
  SMITH_CHECK(out.is_mps(),   "add: expected 'out' to be MPS, but got ", out.device());
  if (!self.is_sparse()) {
  ...
  SMITH_CHECK(self.sizes().equals(other.sizes()),
  ...
  SMITH_CHECK(is_same_density(self, other),
...
static void sparse_mask_apply_out_mps_kernel(
  ...
  SMITH_CHECK(canCast(commonDtype, result.scalar_type()),
  ...
  SMITH_CHECK(sd > 0 || (src_nnz <= 1 && mask_nnz <= 1),
...
static void sparse_mask_projection_out_mps_kernel(
  ...
  SMITH_CHECK(lhs.is_sparse() && rhs.is_sparse(), "sparse_mask_projection: expected sparse COO");
  SMITH_CHECK(lhs.is_mps() && rhs.is_mps(), "sparse_mask_projection: expected MPS tensors");
  SMITH_CHECK(lhs.sparse_dim() == rhs.sparse_dim(), "sparse_dim mismatch");
...
Tensor _sparse_sum_backward_mps(const Tensor& grad_, const SparseTensor& input_, IntArrayRef dims_to_sum) {
  SMITH_CHECK(grad_.is_mps(), "_sparse_sum_backward_mps: expected 'grad_' to be MPS tensor, but got ", grad_.device());
  SMITH_CHECK(input_.is_mps(), "_sparse_sum_backward_mps: expected 'input_' to be MPS tensor, but got ", input_.device());
  ...
  if (sum_all_sparse_dim) {
    SMITH_CHECK(!grad_.is_sparse(), "_sparse_sum_backward_mps: expected grad Tensor to be dense since all sparse dims are summed");
  ...
  SMITH_CHECK(grad_.is_sparse(), "_sparse_sum_backward_mps: expected 'grad_' Tensor to be sparse when not all sparse dims are summed");
...
Tensor index_select_sparse_mps(const Tensor& self_, int64_t dim, const Tensor& index) {
  SMITH_CHECK(self_.is_sparse(), "index_select_sparse_mps: expected a sparse COO tensor");
  SMITH_CHECK(self_.is_mps(), "index_select_sparse_mps: expected 'self' to be on MPS, got ", self_.device());
  SMITH_CHECK(
  ...
  SMITH_CHECK_INDEX(ndim, "index_select() cannot be applied to a 0-dim tensor.");
  ...
  if (dim >= sd) {
    ...
    SMITH_CHECK(values_dim >= 1 && values_dim < (1 + dd),
  ...
  SMITH_CHECK(I >= 0, "index_select_sparse_mps: invalid size for selected dim");
  ...
  if (index_len > 0) {
    SMITH_CHECK(index.min().item<int64_t>() >= -I && index.max().item<int64_t>() < I,
