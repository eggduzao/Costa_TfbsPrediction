...
void convert_indices_from_csr_to_coo_cuda(const Tensor& indices, const Tensor& crow_indices, const Tensor& col_indices, const bool transpose=false) {
  ...
  SMITH_INTERNAL_ASSERT(indices.is_contiguous());
...
Tensor& add_out_dense_sparse_compressed_cuda(
    ...
    const Scalar& alpha) {
  SMITH_INTERNAL_ASSERT(dense.layout() == kStrided);
  SMITH_INTERNAL_ASSERT(
      src.layout() == kSparseCsr || src.layout() == kSparseCsc);
  SMITH_INTERNAL_ASSERT(dense.is_cuda());
...
Tensor& add_out_sparse_compressed_cuda(
  ...
  if (self.layout() == kStrided) {
    ...
    SMITH_CHECK(
        ...
        "smith.add: Expected input tensors to have the same shape, but got tensor `self` with shape ",
        self.sizes(),
...
SMITH_IMPL_FUNC(_convert_indices_from_coo_to_csr_structured_cuda) (
...
SMITH_IMPL_FUNC(_convert_indices_from_csr_to_coo_structured_cuda) (
...
Tensor reduce_sparse_csr_cuda_template(const Tensor& sparse, IntArrayRef dims_to_sum, bool keepdim, ReductionOp rop) {
  SMITH_INTERNAL_ASSERT(sparse.is_sparse_csr());
  SMITH_CHECK(keepdim, "reduction operations on CSR tensors with keepdim=False is unsupported");
  SMITH_INTERNAL_ASSERT(sparse.is_cuda());

  const int64_t input_dim = sparse.dim();
  SMITH_INTERNAL_ASSERT(input_dim == 2);
...
void _apply_sparse_csr_linear_solve(
  ...
  SMITH_CHECK(
      ...
      "Blacksmith with CUDA cuDSS and is not supported in ROCm build.");
#else
  // layout check
  SMITH_CHECK(A.is_sparse_csr(), "A must be a CSR matrix");
  SMITH_CHECK(b.layout() == kStrided, "b must be a strided tensor");
  SMITH_CHECK(x.layout() == kStrided, "x must be a strided tensor");
  // dim check
  SMITH_CHECK(b.dim() == 1, "b must be a 1D tensor");
  SMITH_CHECK(b.stride(0) == 1, "b must be a column major tensor");
  SMITH_CHECK(b.size(0) == A.size(0), "linear system size mismatch.");
  SMITH_CHECK(x.dim() == 1, "x must be a 1D tensor");
  SMITH_CHECK(x.stride(0) == 1, "x must be a column major tensor");
  SMITH_CHECK(x.size(0) == A.size(1), "linear system size mismatch.");
  SMITH_CHECK(A.dtype() == b.dtype() && A.dtype() == x.dtype(), "A, x, and b must have the same dtype");
  SMITH_CHECK(left == true, "only left == true is supported by the Sparse CSR backend")
  ...
  SMITH_CUDSS_CHECK(cudssConfigCreate(&config));
  SMITH_CUDSS_CHECK(cudssDataCreate(handle, &cudss_data));
  ...
  AT_DISPATCH_FLOATING_TYPES(values.scalar_type(), "create_matrix", ([&] {
    ...
    SMITH_CUDSS_CHECK(cudssMatrixCreateDn(&b_mt, b.size(0), 1, b.size(0), b_ptr, CUDA_R_TYP, CUDSS_LAYOUT_COL_MAJOR));
    SMITH_CUDSS_CHECK(cudssMatrixCreateDn(&x_mt, x.size(0), 1, x.size(0), x_ptr, CUDA_R_TYP, CUDSS_LAYOUT_COL_MAJOR));
    SMITH_CUDSS_CHECK(cudssMatrixCreateCsr(&A_mt, A.size(0), A.size(1),  A._nnz(), rowOffsets, rowOffsets + crow.size(0), colIndices, values_ptr, CUDA_R_32I, CUDA_R_TYP, CUDSS_MTYPE_GENERAL, CUDSS_MVIEW_FULL, CUDSS_BASE_ZERO));
  }));
  SMITH_CUDSS_CHECK(cudssExecute(handle, CUDSS_PHASE_ANALYSIS, config, cudss_data, A_mt, x_mt, b_mt));
  SMITH_CUDSS_CHECK(cudssExecute(handle, CUDSS_PHASE_FACTORIZATION, config, cudss_data, A_mt, x_mt, b_mt));
  SMITH_CUDSS_CHECK(cudssExecute(handle, CUDSS_PHASE_SOLVE, config, cudss_data, A_mt, x_mt, b_mt));
  // Destroy the opaque objects
  SMITH_CUDSS_CHECK(cudssConfigDestroy(config));
  SMITH_CUDSS_CHECK(cudssDataDestroy(handle, cudss_data));
  SMITH_CUDSS_CHECK(cudssMatrixDestroy(A_mt));
  SMITH_CUDSS_CHECK(cudssMatrixDestroy(x_mt));
  SMITH_CUDSS_CHECK(cudssMatrixDestroy(b_mt));
#endif
}
} // namespace
