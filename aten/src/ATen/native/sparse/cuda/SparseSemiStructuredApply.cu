...
std::tuple<Tensor, Tensor> _sparse_semi_structured_apply_typed(Tensor input, Tensor threads_masks)
{
  ...
  SMITH_CHECK(input.stride(1) == 1);
  SMITH_CHECK(input.stride(0) % 8 == 0);
  SMITH_CHECK(input.size(1) % 32 == 0, "Wrong alignment shape[1]");
...
{
#if defined(USE_ROCM) || defined(_MSC_VER)
  SMITH_CHECK(false, "_sparse_semi_structured_apply: not supported");
