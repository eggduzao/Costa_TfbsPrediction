...
void block_sparse_triangular_solve_vec(
    ...
    bool transpose,
    bool unitriangular) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(A.layout() == kSparseBsr);
  // values is expected to be a blocks of sparse matrix
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(A.values().dim() == 3);
  // blocks are expected to be square
  SMITH_INTERNAL_ASSERT(A.values().size(2) == A.values().size(1));
  // only block of size > 1 is supported in cuSPARSE
  SMITH_INTERNAL_ASSERT(A.values().size(-1) > 1);
...
void block_sparse_mv(
    ...
    const Scalar& alpha,
    const Tensor& result) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat.layout() == kSparseBsr);
  // values is expected to be a blocks of sparse matrix
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat.values().dim() == 3);
  // blocks are expected to be square
  SMITH_INTERNAL_ASSERT(mat.values().size(2) == mat.values().size(1));
  // only block of size > 1 is supported in cuSPARSE
  SMITH_INTERNAL_ASSERT(mat.values().size(-1) > 1);
  // blocks are expected to be in row- or column-major order
  SMITH_INTERNAL_ASSERT(
...
void block_sparse_mm(
    ...
    const Scalar& alpha,
    const Tensor& result) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat1.layout() == kSparseBsr);
  // values is expected to be a blocks of sparse matrix
  SMITH_INTERNAL_ASSERT(mat1.values().dim() == 3);
  // blocks are expected to be square
  SMITH_INTERNAL_ASSERT(mat1.values().size(2) == mat1.values().size(1));
  // only block of size > 1 is supported in cuSPARSE
  SMITH_INTERNAL_ASSERT(mat1.values().size(-1) > 1);
  // blocks are expected to be in row- or column-major order
  SMITH_INTERNAL_ASSERT(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(ndim == 2);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat1.dim() == 2);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat2.dim() == 2);
...
void spmm(
  ...
  IntArrayRef mat2_strides = mat2_->strides();
  auto ndim = result_->dim();
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(ndim == 2 || ndim == 3);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat1.dim() == 2 || mat1.dim() == 3);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat2.dim() == 2 || mat2.dim() == 3);
...
void spgemm(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&] {
        ...
        SMITH_CUDASPARSE_CHECK(cusparseSpGEMM_workEstimation(
            handle,
            opA,
            opB,
            &alpha_,
        ...
        SMITH_CUDASPARSE_CHECK(cusparseSpGEMM_compute(
            handle,
            opA,
            opB,
            &alpha_,
        ...
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(C_num_rows == m);
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(C_num_cols == n);
        ...
        SMITH_CUDASPARSE_CHECK(cusparseSpGEMM_copy(
            handle,
            opA,
            opB,
            &alpha_,
...
void addmm_out_sparse_csr(
  ...
  // Invalid combinations are omitted and will fall though to the SMITH check
...
void addmv_out_sparse_csr(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      [&] {
        ...
        size_t buffer_size;
        SMITH_CUDASPARSE_CHECK(cusparseSpMV_bufferSize(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
        ...
        SMITH_CUDASPARSE_CHECK(cusparseSpMV(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
...
void add_out_sparse_csr(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(A.sizes().equals(B.sizes()) && A.sizes().equals(C.sizes()));
...
// Let's extract manually the nnz from the C_crow_indices
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      C.scalar_type(), "add_out_sparse_csr_cuda_impl", [&] {
        ...
        auto handle = at::cuda::getCurrentCUDASparseHandle();
        SMITH_CUDASPARSE_CHECK(cusparseSetPointerMode(handle, CUSPARSE_POINTER_MODE_HOST));
  ...
  if (B.size(-1) == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        X.scalar_type(), "triangular_solve_out_sparse_csr_cuda_impl", [&] {
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSV_bufferSize(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSV_analysis(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSV_solve(
              handle,
              opA,
              &alpha,
              descA.descriptor(),
...
#define ROCM_EXTRA_ARG
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        X.scalar_type(), "triangular_solve_out_sparse_csr_cuda_impl", [&] {
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSM_bufferSize(
              handle,
              opA,
              opB,
              &alpha,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSM_analysis(
              handle,
              opA,
              opB,
              &alpha,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpSM_solve(
              handle,
              opA,
              opB,
              &alpha,
...
void sampled_addmm_out_sparse_csr(
    ...
    const Scalar& alpha,
    const at::sparse_csr::SparseCsrTensor& C) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(A.layout() == Layout::Strided);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(B.layout() == Layout::Strided);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(C.is_sparse_csr());

  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(batchCount(A) == batchCount(B));
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(batchCount(A) == batchCount(C));
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      [&] {
        ...
        for (const auto i : c10::irange(batchCount(A))) {
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSDDMM_bufferSize(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSDDMM_preprocess(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSDDMM(
              handle,
              opA,
              opB,
              &alpha_,
