...
#define CUTLASS_STATUS_CHECK(status)                                    \
  {                                                                     \
    SMITH_CHECK(status == cutlass::Status::kSuccess,                    \
...
void spgemm_cutlass(
    ...
    // Determine Blacksmith datatype for the metadata matrix.
    ...
    case 4:
        ...
        break;
    default:
        SMITH_CHECK(false, __func__, ": invalid size of meta tensor datatype "
                 "encountered");
    }
    SMITH_CHECK(tensor_e.dtype() == tensor_e_dtype,
...
void spgemm_cutlass_dispatch_layouts(
    ...
    SMITH_CHECK(false, __func__, "_dispatch_layouts: Combination of ",
...
#if defined(USE_ROCM) || defined(_MSC_VER)
    SMITH_CHECK(false, __func__, " : CUTLASS not supported");
    return Tensor{};
#else
...
// needed, so it would require changing Blacksmith CMake configuration;
// furthermore, including this file produces build errors in Blacksmith
// at the moment.
...
_to_sparse_semi_structured(const Tensor& dense) {
  ...
  if (dense.dtype() == at::kChar) {
    ...
    SMITH_CHECK(false, "_to_sparse_semi_structured: Invalid dense argument datatype ",
  ...
  if (dense_nrows % (meta_dtype == at::kShort ? 32 : 16) != 0) {
    SMITH_CHECK(false, "_to_sparse_semi_structured: Number of rows of dense matrix must "
             "be divisible by ", (meta_dtype == at::kShort ? 32 : 16),
             ", but it is ", dense_nrows);
  }
  if (dense_ncols % dense_elems_per_meta_elem != 0) {
    SMITH_CHECK(false, "_to_sparse_semi_structured: Number of columns of dense matrix "
    ...
    for (auto j = 0; j < meta_ncols; ++j) {
      ...
      for (auto k = 0; k < dense_elems_per_meta_elem / ksparse; ++k, mask_cpu_ptr += ksparse) {
        ...
        if (mask_elems == std::make_tuple(1, 1, 0, 0)) {
          ...
          SMITH_CHECK(false, "_to_sparse_semi_structured: dense argument does not match ",
