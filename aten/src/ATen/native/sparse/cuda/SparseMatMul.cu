...
void create_general_description_(cusparseMatDescr_t& description_) {
  SMITH_CUDASPARSE_CHECK(cusparseCreateMatDescr(&description_));
  SMITH_CUDASPARSE_CHECK(cusparseSetMatType(description_, CUSPARSE_MATRIX_TYPE_GENERAL));
  SMITH_CUDASPARSE_CHECK(cusparseSetMatIndexBase(description_, CUSPARSE_INDEX_BASE_ZERO));
}
...
struct CusparseMatrixMultiplyOp {
  ...
  CusparseMatrixMultiplyOp() {
    ...
    // SpGEMM Computation
    SMITH_CUDASPARSE_CHECK(cusparseSpGEMM_createDescr(&spgemmDesc));
  }
  ...
  csrOutput operator ()(
    ...
    #if defined(USE_ROCM)
    SMITH_CHECK(!(computeType == CUDA_R_16F || computeType == CUDA_R_16BF),
        "sparse_mm: Float16 and BFloat16 are not supported on ROCm");
    #else // defined(USE_ROCM)
    cudaDeviceProp* prop = at::cuda::getCurrentDeviceProperties();
    SMITH_CHECK(prop->major >= 5 && !((10*prop->major + prop->minor) < 53 && computeType == CUDA_R_16F),
        "sparse_mm: CUDA Float16 requires compute capability >= 53 (current: ", prop->major, prop->minor, ")");
    SMITH_CHECK(!(prop->major < 8 && computeType == CUDA_R_16BF),
...
void sparse_sparse_matmul_cuda_kernel(
  ...
  auto k2 = mat2.size(0);
  auto n = mat2.size(1);
  SMITH_CHECK((m <= INT_MAX) && (n <= INT_MAX) && (k1 <= INT_MAX),
...
Tensor sparse_sparse_matmul_cuda(const Tensor& mat1_, const Tensor& mat2_) {
  SMITH_INTERNAL_ASSERT(mat1_.is_sparse());
  SMITH_INTERNAL_ASSERT(mat2_.is_sparse());
  SMITH_CHECK(mat1_.dim() == 2);
  SMITH_CHECK(mat2_.dim() == 2);
  SMITH_CHECK(mat1_.dense_dim() == 0, "sparse_mm: scalar values expected, mat1 got ", mat1_.dense_dim(), "D values");
  SMITH_CHECK(mat2_.dense_dim() == 0, "sparse_mm: scalar values expected, mat2 got ", mat2_.dense_dim(), "D values");
