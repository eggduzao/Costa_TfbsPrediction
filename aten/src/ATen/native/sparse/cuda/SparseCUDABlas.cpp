...
void Xcoo2csr(const int *coorowind, int64_t nnz, int64_t m, int *csrrowptr) {
  SMITH_CHECK((m <= INT_MAX) && (nnz <= INT_MAX),
  ...
  SMITH_CUDASPARSE_CHECK(cusparseXcoo2csr(handle, coorowind, i_nnz, i_m, csrrowptr, CUSPARSE_INDEX_BASE_ZERO));
}
...
cusparseOperation_t convertTransToCusparseOperation(char trans) {
  if (trans == 't') return CUSPARSE_OPERATION_TRANSPOSE;
    ...
    SMITH_CHECK(false, "trans must be one of: t, n, c");
  }
}
  ...
  cudaDataType cusparse_value_type)
{
  ...
  SMITH_CHECK((m <= INT_MAX) && (n <= INT_MAX) && (k <= INT_MAX) && (nnz <= INT_MAX) && (ldb <= INT_MAX) && (ldc <= INT_MAX),
  ...
  SMITH_CUDASPARSE_CHECK(cusparseCreateCsr(
    &descA,                     /* output */
  ...
  SMITH_CUDASPARSE_CHECK(cusparseDestroySpMat(descA));
  SMITH_CUDASPARSE_CHECK(cusparseDestroyDnMat(descB));
  SMITH_CUDASPARSE_CHECK(cusparseDestroyDnMat(descC));
...
{
  SMITH_CHECK((m <= INT_MAX) && (n <= INT_MAX) && (nnz <= INT_MAX),
    "Xcsrsort_bufferSizeExt only supports m, n, nnz with the bound [val] <=",
    INT_MAX);
  int i_m = static_cast<int>(m);
  ...
  auto handle = at::cuda::getCurrentCUDASparseHandle();
  SMITH_CUDASPARSE_CHECK(cusparseXcsrsort_bufferSizeExt(handle, i_m, i_n, i_nnz, csrRowPtr, csrColInd, pBufferSizeInBytes));
}
...
void Xcsrsort(int64_t m, int64_t n, int64_t nnz, const int *csrRowPtr, int *csrColInd, int *P, void *pBuffer)
{
  ...
  SMITH_CUDASPARSE_CHECK(cusparseXcsrsort(handle, i_m, i_n, i_nnz, desc, csrRowPtr, csrColInd, P, pBuffer));
  SMITH_CUDASPARSE_CHECK(cusparseDestroyMatDescr(desc));
}
...
void Xcoosort_bufferSizeExt(int64_t m, int64_t n, int64_t nnz, const int *cooRows, const int *cooCols, size_t *pBufferSizeInBytes)
{
  ...
  auto handle = at::cuda::getCurrentCUDASparseHandle();
  SMITH_CUDASPARSE_CHECK(cusparseXcoosort_bufferSizeExt(handle, i_m, i_n, i_nnz, cooRows, cooCols, pBufferSizeInBytes));
}
...
void XcoosortByRow(int64_t m, int64_t n, int64_t nnz, int *cooRows, int *cooCols, int *P, void *pBuffer)
{
  ...
  auto handle = at::cuda::getCurrentCUDASparseHandle();
  SMITH_CUDASPARSE_CHECK(cusparseXcoosortByRow(handle, i_m, i_n, i_nnz, cooRows, cooCols, P, pBuffer));
}
