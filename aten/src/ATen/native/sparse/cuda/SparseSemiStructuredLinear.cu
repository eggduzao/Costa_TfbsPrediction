...
Tensor two_four_sgemm(
    ...
    case 4:
        ...
        break;
    default:
        SMITH_CHECK(false, "two_four_sgemm: invalid size of meta tensor datatype "
                 "encountered");
    }
    SMITH_CHECK(meta.dtype() == meta_dtype,
                "two_four_sgemm: Expected meta datatype ", meta_dtype,
                ", but got ", meta.dtype());

    // Determine Blacksmith datatype for the output matrix.
    ...
    if constexpr (std::is_same_v<ElementOutput, int8_t>) {
        ...
        SMITH_CHECK(false, "two_four_sgemm: invalid datatype for sparse GEMM output ",
                 "encountered");
    }
    if constexpr (use_bias) {
        SMITH_CHECK(tensor_c.dtype() == tensor_d_dtype,
...
Tensor two_four_sgemm_dispatch_layouts(
    ...
    SMITH_CHECK(false, "two_four_sgemm_dispatch_layouts: Combination of ",
...
Tensor two_four_sgemm_dispatch_layouts_bias_activation(
    ...
    SMITH_CHECK(false, "two_four_sgemm_dispatch_layouts: Activation \"", activation,
...
Tensor _sparse_semi_structured_linear(
    ...
    SMITH_WARN_ONCE("_sparse_semi_structured_linear is deprecated and will be "
                    "removed in a future Blacksmith release.  Please use "
                    "_sparse_semi_structured_mm/_sparse_semi_structured_addmm "
                    "instead.");
#if defined(USE_ROCM) || defined(_MSC_VER)
    SMITH_CHECK(false, "_sparse_semi_structured_linear: CUTLASS not supported");
    return Tensor{};
#else
    ...
    SMITH_CHECK(!out_dtype_opt.has_value() ||
    ...
    SMITH_CHECK(tensor_a.dtype() == at::kChar ||
    ...
    SMITH_CHECK(tensor_b.dtype() == tensor_a.dtype(),
