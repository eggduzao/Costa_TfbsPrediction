...
struct MetadataCuSparseLt {
  ...
  {
    SMITH_CHECK(rows % 128 == 0 && cols % 128 == 0, "Only supports rows/cols multiples of 128");
        ...
        std::string algorithm)
{
  ...
  SMITH_CHECK(input.dim() == 2, "Can only sparsify 2d tensors");
  SMITH_CHECK(
      input.stride(1) == 1,
  ...
  SMITH_CHECK(kernel_launched, "Unknown algorithm \"", algorithm, "\"");
...
{
#if defined(USE_ROCM) || defined(_MSC_VER)
  SMITH_CHECK(false, "_sparse_semi_structured_tile: not supported");
