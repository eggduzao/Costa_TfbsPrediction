...
static void initHipSparseLtSupport() {
    ...
    try {
        ...
        g_hipSparseLtSupported = false;
        SMITH_WARN("Exception occurred while checking hipSparseLt support. Assuming not supported.");
    }
}
...
at::Tensor _cslt_compress(const Tensor& sparse_input) {
  if (!handle_initialized) {
    SMITH_CUDASPARSE_CHECK(cusparseLtInit(&handle));
  ...
  #ifdef USE_ROCM
  SMITH_CHECK(isHipSparseLtSupported());
  #endif
...
#endif
    default:
      SMITH_CHECK(false, "Unsupported dtype for cuSPARSELt/hipSparseLt compressed matrix");
      break;
  }

  SMITH_CUDASPARSE_CHECK(cusparseLtStructuredDescriptorInit(
      &handle,
  ...
  SMITH_CUDASPARSE_CHECK(cusparseLtSpMMACompressedSize2(
      &handle,
      &sparse_input_descriptor,
      &compressed_size,
  ...
  SMITH_CUDASPARSE_CHECK(cusparseLtSpMMACompress2(
      &handle,
      &sparse_input_descriptor,
      true,
...
// cslt 0.6.2+: fp8 fp8 -> {fp8, fp16, bf16, fp32} support
  ...
  SMITH_INTERNAL_ASSERT(compressed_A.dim() == 2); // encoded M x S
  ...
  SMITH_CUDASPARSE_CHECK(cusparseLtDenseDescriptorInit(
      &handle,
  ...
  // initialize matmul
  SMITH_CUDASPARSE_CHECK(cusparseLtMatmulDescriptorInit(
      &handle,
      &matmul,
      CUSPARSE_OPERATION_NON_TRANSPOSE,
  ...
  if (bias_opt.has_value()) {
    ...
    SMITH_CUDASPARSE_CHECK(cusparseLtMatmulDescSetAttribute(
        &handle,
        &matmul,
  ...
  SMITH_CUDASPARSE_CHECK(cusparseLtMatmulAlgSelectionInit(
      &handle, &alg_sel, &matmul, CUSPARSELT_MATMUL_ALG_DEFAULT));

  // set matmul search params
  SMITH_CUDASPARSE_CHECK(cusparseLtMatmulAlgSetAttribute(
      &handle,
      &alg_sel,
  ...
  if (search_alg_id) {
    // run matmul search
    SMITH_CUDASPARSE_CHECK(cusparseLtMatmulSearch(
        &handle,
        &plan,
        alpha_ptr,
    ...
    SMITH_CUDASPARSE_CHECK(cusparseLtMatmulAlgGetAttribute(
        &handle,
        &alg_sel,
        CUSPARSELT_MATMUL_ALG_CONFIG_ID,
        &alg_id,
    ...
    // do normal matmul
    SMITH_CUDASPARSE_CHECK(cusparseLtMatmul(
        &handle,
        &plan,
        alpha_ptr,
        compressed_A.data_ptr(),
  ...
  SMITH_CUDASPARSE_CHECK(cusparseLtMatDescriptorDestroy(&res_descriptor));
  // destroy plan
  SMITH_CUDASPARSE_CHECK(cusparseLtMatmulPlanDestroy(&plan));
...
int64_t _cslt_sparse_mm_search(
    ...
    bool transpose_result) {
  SMITH_WARN_ONCE(
      "smith._cslt_sparse_mm_search is deprecated and will be removed in a future Blacksmith release. Please use smith._C._cusparselt.mm_search instead.");
  int alg_id_int = 0;
  int split_k = 1;
  int split_k_mode = -1;
  auto result = _cslt_sparse_mm_impl(
...
at::Tensor _cslt_compress(const Tensor& sparse_input) {
  SMITH_CHECK(false, "cuSPARSELt not supported on your machine.");
}
