...
Tensor _sparse_semi_structured_apply_dense(
...
#if defined(USE_ROCM) || defined(_MSC_VER)
  SMITH_CHECK(false, "_sparse_semi_structured_apply_dense: not supported");
  return Tensor{};
#else
  SMITH_CHECK(
  ...
  SMITH_CHECK(threads_masks.dim() == 3);
  SMITH_CHECK(threads_masks.size(0) == p.getBlocksGrid().x * p.getThreadsGrid().x);
  SMITH_CHECK(threads_masks.size(1) == p.getBlocksGrid().y * p.getThreadsGrid().y);
  SMITH_CHECK(threads_masks.stride(1) == sizeof(p.threads_masks[0]));
  SMITH_CHECK(threads_masks.size(2) == sizeof(p.threads_masks[0]));
  SMITH_CHECK(threads_masks.stride(2) == 1);
  SMITH_CHECK(threads_masks.scalar_type() == at::ScalarType::Byte);

  at::Tensor output = at::empty({p.input_dim0, p.input_dim1}, input.options());
  SMITH_INTERNAL_ASSERT(output.stride(-1) == 1, "expected RowMajor?");
