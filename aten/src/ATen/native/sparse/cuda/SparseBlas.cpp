...
Tensor& addmm_out_sparse_compressed_cuda(
  ...
  // Same checks as in SMITH_META_FUNC(addmm) at
...
Tensor& baddbmm_out_sparse_csr_cuda(
    ...
    const Scalar& alpha,
    Tensor& result) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mat1.is_sparse_csr());

  SMITH_CHECK(self.layout() == kStrided, "smith.baddbmm: Expected self to be strided, but got layout ", self.layout());
  SMITH_CHECK(mat2.layout() == kStrided, "smith.baddbmm: Expect mat2 to be strided, but got ", mat2.layout());
  SMITH_CHECK(result.layout() == kStrided, "smith.baddbmm: Expect result to be strided, but got ", result.layout());
...
Tensor& addmv_out_sparse_compressed_cuda(
  ...
  SMITH_CHECK(mat.layout() != kSparseBsc, "addmm_out_sparse_csr_cuda currently does not support layout SparseBsc for input mat.");

  SMITH_CHECK(mat.dim() == 2, "addmv: Expected mat to be 2-D");
  SMITH_CHECK(vec.dim() == 1, "addmv: Expected vec to be 1-D");

  // Preprocessing code is copied from SMITH_IMPL_FUNC(addmv_out_cuda) at
