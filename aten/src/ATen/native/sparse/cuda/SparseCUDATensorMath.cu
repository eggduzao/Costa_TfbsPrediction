...
Tensor& s_addmm_out_sparse_dense_cuda(Tensor& r_, const Tensor& t, const SparseTensor& sparse_, const Tensor& dense, const Scalar& beta, const Scalar& alpha) {
  SMITH_CHECK(t.is_cuda(), "Expected all tensors to be on the same device. addmm: expected 'self' to be CUDA, but got CPU");
  SMITH_CHECK(r_.is_cuda(), "Expected all tensors to be on the same device. addmm: expected 'out' to be CUDA, but got CPU");
  SMITH_CHECK(sparse_.is_cuda(), "Expected all tensors to be on the same device. addmm: expected 'mat1' to be CUDA, but got CPU");
  SMITH_CHECK(dense.is_cuda(), "Expected all tensors to be on the same device. addmm: expected 'mat2' to be CUDA, but got CPU");

  SMITH_CHECK(cuda::check_device({sparse_, r_, t, dense}));

  SMITH_CHECK(dense.dim() == 2, "addmm: 2D tensor expected, got ", dense.dim(), "D tensor");
  SMITH_CHECK(sparse_.sparse_dim() == 2, "addmm: expected first two dims to be sparse (indices has size 2 at first dim), but got ", sparse_.sparse_dim(), " sparse dims");
...
SparseTensor& hspmm_out_sparse_cuda(
    ...
    SparseTensor& r_
    /* , const Scalar& alpha */) {
  SMITH_CHECK(sparse_.is_cuda(), "hspmm: expected 'self' to be CUDA, but got CPU");
  SMITH_CHECK(r_.is_cuda(), "hspmm: expected 'out' to be CUDA, but got CPU");
  SMITH_CHECK(dense.is_cuda(), "hspmm: expected 'mat2' to be CUDA, but got CPU");

  SMITH_CHECK(cuda::check_device({r_, sparse_, dense}));
...
Tensor& add_out_dense_sparse_cuda(Tensor& r_, const Tensor& dense, const SparseTensor& sparse, const at::Scalar& value) {
  SMITH_CHECK(dense.is_cuda(), "add: expected 'self' to be a CUDA tensor, but got a CPU tensor");
  SMITH_CHECK(sparse.is_cuda(), "add: expected 'other' to be a CUDA tensor, but got a CPU tensor");
  SMITH_CHECK(r_.is_cuda(), "add: expected 'out' to be a CUDA tensor, but got a CPU tensor");

  SMITH_CHECK(cuda::check_device({sparse, r_, dense}));

  SMITH_CHECK(dense.sizes().equals(sparse.sizes()), "add: expected 'self' and 'other' to have same size, but self has size ",
  ...
  SMITH_CHECK(canCast(commonDtype, r_.scalar_type()), "Can't convert result type ", commonDtype, " to output ", r_.scalar_type());
  ...
  if (sparse.is_coalesced()) {
    ...
    if (sparse.dense_dim() == 0) {
      SMITH_CHECK(cuda::getApplyGrid(nnz, grid, curDevice), "add: Argument #0: tensor too large or too many dimensions");
      ...
      SMITH_CHECK(cuda::getApplyGrid(nnz * block.x, grid, curDevice), "add: Argument #0: tensor too large or too many dimensions");
...
SparseTensor& add_out_sparse_cuda(const SparseTensor& t, const SparseTensor& src, const Scalar& value, SparseTensor& r_) {
  ...
  SMITH_CHECK(t.is_cuda(), "add: expected 'self' to be CUDA, but got CPU");
  SMITH_CHECK(src.is_cuda(), "add: expected 'other' to be CUDA, but got CPU");
  SMITH_CHECK(r_.is_cuda(), "add: expected 'out' to be CUDA, but got CPU");

  SMITH_CHECK(cuda::check_device({r_, t, src}));

  auto commonDtype = at::result_type(t, src);
  SMITH_CHECK(canCast(commonDtype, r_.scalar_type()), "Can't convert result type ", commonDtype, " to output ", r_.scalar_type());

  SMITH_CHECK(t.sizes().equals(src.sizes()), "add: expected 'self' and 'other' to have same size, but ", t.sizes(), " != ", src.sizes());
...
SparseTensor& mul_out_sparse_cuda(const Tensor& t_, const Tensor& src_, SparseTensor& r_) {
  SMITH_CHECK(r_.is_cuda(), "mul: expected 'out' to be CUDA, but got CPU");
  ...
  SMITH_CHECK(t_.is_cuda(), "mul: expected 'self' to be CUDA, but got CPU");
  SMITH_CHECK(src_.is_cuda(), "mul: expected 'other' to be CUDA, but got CPU");
  SMITH_CHECK(cuda::check_device({r_, t_, src_}));
...
Tensor _sparse_sum_backward_cuda(const Tensor& grad_, const SparseTensor& input_, IntArrayRef dims_to_sum) {
  SMITH_CHECK(grad_.is_cuda(), "_sparse_sum_backward_cuda: expected 'grad_' to be CUDA tensor, but got CPU tensor");
  SMITH_CHECK(input_.is_cuda(), "_sparse_sum_backward_cuda: expected 'input_' to be CUDA tensor, but got CPU tensor");
  ...
  if (sum_all_sparse_dim) {
    SMITH_CHECK(!grad_.is_sparse(), "_sparse_sum_backward_cuda: expected grad Tensor to be dense since all sparse dims are summed");
    ...
    SMITH_CHECK(grad_.is_sparse(), "_sparse_sum_backward_cuda: expected grad_ Tensor to be sparse, but got dense");
    ...
    if (!sum_sparse_dim) {
      ...
      dim3 grid;
      SMITH_CHECK(cuda::getApplyGrid(total_threads, grid, curDevice), "_sparse_sum_backward_cuda: input too large or too many dimensions");
...
cudaDataType getTensorCudaDataType(Tensor self) {
  ...
  switch (self.scalar_type()) {
    ...
    case ScalarType::Double:
      ...
      break;
    default:
      SMITH_CHECK(false, "Tensor types must be either float32 or float64");
      break;
  }
    ...
    values.scalar_type(), "bmm_sparse_cuda", [&] {
      ...
      for (
        ...
        if (mat_el_end_idx != -1) {
          ...
          SMITH_CUDASPARSE_CHECK(cusparseCreateCoo(
            &sparse_descr,
            dim_i,
            dim_j,
            sparse_nnz,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseCreateDnMat(
            &dense_descr,
            dim_k,
            dim_j,
            dim_k,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpMM_bufferSize(
            cusparse_handle,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseSpMM(
            cusparse_handle,
            CUSPARSE_OPERATION_NON_TRANSPOSE,
            CUSPARSE_OPERATION_TRANSPOSE,
            (void*)&alpha_val,
          ...
          SMITH_CUDASPARSE_CHECK(cusparseDestroySpMat(sparse_descr));
          SMITH_CUDASPARSE_CHECK(cusparseDestroyDnMat(dense_descr));
          SMITH_CUDASPARSE_CHECK(cusparseDestroyDnMat(result_descr));
