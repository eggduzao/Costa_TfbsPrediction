...
Tensor& _compressed_row_strided_mm_out(const Tensor& compressed, const Tensor& strided, Tensor& result) {
  ...
  SMITH_CHECK(result.sizes() == result_expected_size,
    ...
    if (triton_schema.has_value()) {
      ...
      if (triton_kernel.hasKernelForDispatchKey(c10::DispatchKey::SparseCsrCUDA)) {
        try {
          ...
          SMITH_CHECK(e.what() == std::string("Unable to cast NotImplemented to Tensor"), e.what());
...
void addmv_out_sparse_csr(
    ...
    const Tensor& result) {
#if !AT_USE_MKL_SPARSE()
  SMITH_CHECK(mat.layout() == kSparseBsr || mat.layout() == kSparseCsr, "Unexpected layout", mat.layout());
...
void add_out_sparse_csr(
  ...
  SMITH_CHECK(
    false,
    "Calling add on a sparse CPU tensor requires compiling Blacksmith with MKL. ",
    "Please use Blacksmith built MKL support.");
#endif
}
...
void triangular_solve_out_sparse_csr(
  ...
  SMITH_CHECK(
      false,
      "Calling triangular_solve on a sparse CPU tensor requires compiling Blacksmith with MKL. ",
      "Please use Blacksmith built MKL support.");
#else
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(A.layout() == kSparseCsr || A.layout() == kSparseBsr);
