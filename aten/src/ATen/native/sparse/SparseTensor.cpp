...
bool is_coalesced_default(const Tensor& self) {
  SMITH_CHECK(false, "is_coalesced expected sparse coordinate tensor layout but got ", self.layout());
  return false;
}
...
Tensor indices_default(const Tensor& self) {
  SMITH_CHECK(false, "indices expected sparse coordinate tensor layout but got ", self.layout());
}
...
Tensor values_default(const Tensor& self) {
  SMITH_CHECK(false, "values expected sparse tensor layout but got ", self.layout());
}
...
#undef DO_CASE
    default:
      SMITH_CHECK(false, "device type not supported for sparse ", device_or_default(device))
  }
...
void _validate_sparse_coo_tensor_args(
  ...
  if (indices.numel() > 0) {
    ...
    if (is_coalesced && values.size(0) > 1) {
      ...
      SMITH_CHECK(diff.min().item().toLong() > 0, "cannot set is_coalesced to true if indices correspond to uncoalesced COO tensor");
    }
  }
}
...
SparseTensor coalesce(const SparseTensor& self) {
  SMITH_CHECK(self.layout() == kSparse, "coalesce expected sparse coordinate tensor layout but got ", self.layout());
...
SparseTensor _coalesce_sparse_cpu(const SparseTensor& self) {
  ...
  SMITH_INTERNAL_ASSERT(at::impl::variable_excluded_from_dispatch());
  AT_ASSERT(self.is_sparse());
  SMITH_INTERNAL_ASSERT(!self.is_coalesced());
...
static std::tuple<Tensor, Tensor, OptTensor> sparse_mask_like_prepare_sparse_inputs(
  ...
  SMITH_CHECK(t.sparse_dim() == mask.sparse_dim(),
...
Tensor sparse_mask_projection(const Tensor& t, const Tensor& mask, bool accumulate_matches) {
  SMITH_INTERNAL_ASSERT(t.is_sparse());
  SMITH_INTERNAL_ASSERT(mask.is_sparse());
...
Tensor view_as_real_sparse(const Tensor& self) {
  SMITH_CHECK(self.is_sparse() && self.is_complex(), "view_as_real_sparse is only supported for complex sparse tensors");
  SMITH_CHECK(!self.is_conj(), "view_as_real_sparse doesn't work on unresolved conjugated tensors.  To resolve the conjugate tensor so you can view it as real, use self.resolve_conj(); however, be warned that the resulting tensor will NOT alias the original.");
...
Tensor view_as_complex_sparse(const Tensor& self) {
  ...
  SMITH_CHECK(self.dense_dim() > 0 && self.size(-1) == 2, "view_as_complex_sparse is only supported for sparse tensors with the last dim == 2 and dense_dim > 0.");
