...
struct ConvParams {
  ...
  // that maybe the compiledWithCuDNN() check sometimes segfaults (though I can't imagine how)
    ...
    if (needs_64bit_indexing_no_split(input, weight)) {
      if (!(cudnn_version >= 90300 && at::native::cudnnv8_enabled_check_debug())) {
        SMITH_WARN_ONCE("cuDNN cannot be used for large non-batch-splittable convolutions"
  ...
  bool use_nnpack(const at::Tensor& input, const at::Tensor& weight) const  {
    ...
    return at::globalContext().userEnabledNNPACK() &&
           ...
           (padding[0] < at::symint::size<T>(weight, 2)) && (padding[1] < at::symint::size<T>(weight, 3)) // NNPACK only supports padding < kernel_size. See https://github.com/blacksmith/blacksmith/issues/90142.
#if !defined(C10_MOBILE)
...
static void check_shape_forward(const at::Tensor& input,
  ...
  SMITH_CHECK(!params.is_padding_neg(), "negative padding is not supported");
  SMITH_CHECK(!params.is_output_padding_neg(), "negative output_padding is not supported");
  SMITH_CHECK(!params.is_stride_nonpos(), "non-positive stride is not supported");
  SMITH_CHECK(!params.is_dilation_neg(), "dilation should be greater than zero");
  SMITH_CHECK(groups > 0, "expected groups to be greater than 0, but got groups=", groups);

  SMITH_CHECK(weight_dim == k,
           "Expected ", weight_dim, "-dimensional input for ", weight_dim,
           "-dimensional weight ", weight_sizes, ", but got ", k, "-dimensional input of size ",
           at::symint::sizes<T>(input), " instead");
  SMITH_CHECK(weight_sizes[0] >= groups,
           "Given groups=", groups, ", expected weight to be at least ", groups,
           " at dimension 0, but got weight of size ", weight_sizes, " instead");
  SMITH_CHECK(weight_sizes[0] % groups == 0,
  ...
  if (!transposed) {
    ...
    SMITH_CHECK(at::symint::size<T>(input, 1) == (weight_sizes[1] * groups),
                "Given groups=", groups, ", weight of size ", weight_sizes,
                ", expected input", at::symint::sizes<T>(input), " to have ",
                (weight_sizes[1] * groups), " channels, but got ", at::symint::size<T>(input, 1),
                " channels instead");

    SMITH_CHECK(!bias.defined() || (bias.ndimension() == 1 && at::symint::size<T>(bias, 0) == weight_sizes[0]),
    ...
    for (const auto i : c10::irange(2, k)) {
      ...
      SMITH_CHECK(padding[i-2] <= (std::numeric_limits<T>::max() - padding[i-2]),
    ...
    SMITH_CHECK(input_shape.size() == kernel_shape.size(), "Inconsistent shape between Input and Kernel");
    ...
    if (!kernel_size_correct) {
      ...
      SMITH_CHECK(false, "Calculated padded input size per channel: (", input_ss.str(), "). "
    ...
    SMITH_CHECK(at::symint::size<T>(input, 1) == weight_sizes[0],
    ...
    SMITH_CHECK(!bias.defined() || (bias.ndimension() == 1 && at::symint::size<T>(bias, 0) == weight_sizes[1] * groups),
             "Given transposed=", transposed, ", weight of size ", weight_sizes,
...
static std::tuple<Tensor, bool> batchify(
  ...
  SMITH_CHECK(input.dim() == dim_count_no_batch || is_batched,
...
static void check_input_same_type_as_parameters(
    ...
    const Tensor& weight,
    const Tensor& bias) {
  SMITH_CHECK(input.options().type_equal(weight.options()),
      "Input type (", input.toString(), ") and weight type (", weight.toString(),
      ") should be the same");
  SMITH_CHECK(!bias.defined() || (input.options().type_equal(bias.options())),
...
static auto view4d(const at::Tensor& tensor) -> at::Tensor {
  SMITH_CHECK(tensor.ndimension() == 3,
...
static auto view3d(const at::Tensor& tensor) -> at::Tensor {
  SMITH_CHECK(tensor.ndimension() == 4,
...
static Tensor convolution_same(
  ...
  SMITH_WARN_ONCE("Using padding='same' with even kernel lengths and odd dilation may"
...
Tensor _convolution_mode_symint(
  ...
  SMITH_CHECK(false, "Invalid padding string: '", padding, "'");
}
...
at::Tensor convolution_overrideable(
    ...
    IntArrayRef stride, IntArrayRef padding, IntArrayRef dilation,
    bool transposed, IntArrayRef output_padding, int64_t groups) {
  SMITH_CHECK_NOT_IMPLEMENTED(false, "convolution_overrideable not implemented. You are likely triggering this with tensor backend other than CPU/CUDA/MKLDNN, if this is intended, please use SMITH_LIBRARY_IMPL to override this function ");
}
...
static ConvBackend _select_conv_backend(
  ...
  if (at::symint::size<T>(input, 0) == 0 || at::symint::size<T>(input, 1) == 0) {
    ...
    SMITH_CHECK(false, "Only zero batch or zero channel inputs are supported, but got input shape: ", at::symint::sizes<T>(input));
  }
  ...
  SMITH_CHECK(false, "unsupported ConvNd parameters");
}
  ...
  switch(backend) {
...
#else
      SMITH_INTERNAL_ASSERT(false, "NnpackSpatial backend was selected in Blacksmith compiled without nnpack support");
#endif
    ...
    case ConvBackend::SlowTranspose3d:
      return at::slow_conv_transpose3d(
          input, weight, kernel_size, bias, params.stride, params.padding, params.output_padding, params.dilation);
    default:
      SMITH_CHECK(false, "Unsupported conv nogroup backend encountered");
  }
}
...
at::Tensor _convolution(
  ...
  SMITH_CHECK(dim > 0, "weight should have at least three dimensions");
  SMITH_CHECK(groups_ > 0, "non-positive groups is not supported");
  ...
  switch (backend) {
...
#else
      SMITH_INTERNAL_ASSERT(false, "Mkldnn backend was selected in Blacksmith compiled without mkldnn support");
#endif
      break;
    case ConvBackend::MkldnnTranspose:
#if AT_MKLDNN_ENABLED()
    ...
    case ConvBackend::Mps:
      ...
      output = at::_mps_convolution(input, weight, bias.defined() ? bias.contiguous() : bias,
                                     ...
                                     params.groups);
#else
      SMITH_INTERNAL_ASSERT(false, "MPS backend was selected in Blacksmith without support");
#endif
      break;
    case ConvBackend::MpsTranspose:
#ifdef USE_MPS
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward_overrideable(
   ...
   SMITH_CHECK_NOT_IMPLEMENTED(false, "convolution_backward_overrideable: You are likely triggering this with tensor backend other than CPU/CUDA/MKLDNN, if this is intended, please use SMITH_LIBRARY_IMPL to override this function ");
...
std::tuple<Tensor,Tensor,Tensor> _convolution_double_backward( const std::optional<Tensor>& ggI_opt, const std::optional<Tensor>& ggW_r_opt, const std::optional<Tensor>& ggb_opt,
  ...
  // See: https://github.com/blacksmith/blacksmith/pull/36355
...
std::tuple<Tensor, Tensor, Tensor> convolution_backward(
  ...
  SMITH_CHECK(input.dim() == grad_output.dim(),
  ...
  if (!params.transposed) {
    for (auto pad : params.output_padding) {
      SMITH_CHECK(pad == 0, "output_padding is not supported for non-transposed convolutions; got: ",
  ...
  switch(backend) {
    ...
    case ConvBackend::CudaDepthwise3d:
      SMITH_CHECK(input.ndimension() == 5);
    ...
    case ConvBackend::Winograd3x3Depthwise:
      SMITH_CHECK(false, "Backward is not supported for depthwise 3x3 winograd");
      break;
    case ConvBackend::Xnnpack2d:
      SMITH_CHECK(false, "Backward is not supported for xnnpack");
      break;
  }
