...
std::tuple<Tensor, Tensor, Tensor> miopen_batch_norm(
  ...
  SMITH_CHECK(false, "miopen_batch_norm: ATen not compiled with MIOpen support");
}
...
std::tuple<Tensor, Tensor, Tensor> miopen_batch_norm_backward(
    ...
    double epsilon) {
  SMITH_CHECK(false, "miopen_batch_norm_backward: ATen not compiled with MIOpen support");
}
    ...
    bool training, double exponential_average_factor, double epsilon)
{
  ...
  SMITH_CHECK(input->is_contiguous(input->suggest_memory_format()));
...
std::tuple<Tensor, Tensor, Tensor> miopen_batch_norm_backward(
  ...
  SMITH_CHECK(input->is_contiguous(input->suggest_memory_format()));
  SMITH_CHECK(grad_output->is_contiguous(input->suggest_memory_format()));
  checkDimRange(c, input, 2, 6 /* exclusive */);
