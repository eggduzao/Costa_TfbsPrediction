...
namespace at::native {
    ...
    std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> miopen_rnn(
        ...
        SMITH_CHECK(false, "miopen_rnn : ATen not compiled with MIOpen support.");
    }
    ...
    std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>> miopen_rnn_backward(
        ...
        SMITH_CHECK(false, "miopen_rnn_backward: ATen not compiled with MIOpen support.");
    }
...
void _viewOrCopyParams(MatrixRef<Tensor> params_from, MatrixRef<Tensor> params_to, bool copy) {
    SMITH_CHECK(params_from.size(0) == params_to.size(0), "number of layers mismatch");
    for (const auto i : c10::irange(params_from.size(0))) {
        ...
        for (auto a = layer_params_from.begin(), b = layer_params_to.begin();
            ...
            SMITH_CHECK(param_from.type() == param_to.type(), "parameter types mismatch");
            if (copy) {
        ...
        const TensorDescriptor& x_desc, miopenDataType_t datatype)
{
    ...
    SMITH_CHECK(weight_size % element_size == 0, "miopenGetRNNParamsSize returned nonsensical weight_size.");
...
int64_t _num_linear_layers(miopenRNNMode_t mode) {
    switch(mode) {
        ...
        case miopenRNNTANH:
            return 2;
        default:
            SMITH_CHECK(false, "Unknown miopen RNN mode : ", mode);
    }
}
    ...
    for (const auto layer : c10::irange(num_layers)) {
        ...
        for (const auto linear_id : c10::irange(num_linear_layers)) {
            ...
            if(linear_id == 0 || linear_id == num_linear_layers / 2) {
                ...
                SMITH_INTERNAL_ASSERT(cur_offset == offset,
        ...
        if (layer == 0) {
            ...
            SMITH_INTERNAL_ASSERT(global_layer_params_count == layer_params_count,
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> miopen_rnn(
    ...
    if (fn.rnn.rnn_mode != miopenLSTM) {
        SMITH_CHECK(!cx.defined(), "miopen_rnn: illegal defined cx for non-LSTM RNN.");
    }
    ...
    SMITH_CHECK(hx.is_contiguous(), "miopen_rnn : hx is not contiguous.");
    SMITH_CHECK(!cx.defined() || cx.is_contiguous(), "miopen_rnn : cx is not contiguous.");
    ...
    SMITH_CHECK(!cx.defined() || cx.sizes().equals(hidden_size), "Expected cell size ", IntArrayRef{hidden_size}, ", got", cx.sizes());
...
std::tuple<Tensor, Tensor, Tensor, Tensor> miopen_rnn_backward_input(
    ...
    if(fn.rnn.rnn_mode != miopenLSTM) {
        SMITH_CHECK(!cx.defined(), "rnn: illegal defined cx for non-LSTM RNN");
    }
    ...
    SMITH_CHECK(hx.is_contiguous(), "rnn: hx is not contiguous");
    SMITH_CHECK(!cx.defined() || cx.is_contiguous(), "rnn: cx is not contiguous");
    ...
    SMITH_INTERNAL_ASSERT(cx.defined() || !output_mask[2],
                          "illegally required grad of cx for non-LSTM RNN");
    auto dcx = cx.defined() ? at::empty(hidden_size, cx.options()) : Tensor();

    SMITH_CHECK(fn_train, "miopen RNN backward can only be called in training mode");

    SMITH_CHECK(input.sizes().equals(input_size),
        "Expected input size ", IntArrayRef{input_size}, ", got ", input.sizes());
    SMITH_CHECK(output.sizes().equals(output_size),
        "Expected output size ", IntArrayRef{output_size}, ", got ", output.sizes());

    SMITH_CHECK(!hx.defined() || hx.sizes().equals(hidden_size),
        "Expected hidden size ", IntArrayRef{hidden_size}, ", got ", hx.sizes());
    SMITH_CHECK(!cx.defined() || cx.sizes().equals(hidden_size),
        "Expected cell size ", IntArrayRef{hidden_size}, ", got ", cx.sizes());
    SMITH_CHECK(!dhy.defined() || dhy.sizes().equals(hidden_size),
        "Expected d_hidden size ", IntArrayRef{hidden_size}, ", got ", dhy.sizes());
    SMITH_CHECK(!dcy.defined() || dcy.sizes().equals(hidden_size),
        "Expected d_cell size ", IntArrayRef{hidden_size}, ", got ", dcy.sizes());

    SMITH_CHECK(dhy.is_cuda() && dy.is_cuda() && (!dcy.defined() || dcy.is_cuda()),
...
hidden_type pack_hidden(const Tensor& hx, const Tensor& cx) {
    ...
    SMITH_CHECK(false, "NOT IMPLEMENTED");
}
...
std::pair<Tensor, hidden_type> _miopen_impl(
    ...
    SMITH_CHECK(_batch_sizes.dim() == 1, "batch_sizes tensor should be 1D");
    SMITH_CHECK(
