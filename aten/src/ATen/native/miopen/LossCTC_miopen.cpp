...
std::tuple<Tensor, Tensor> miopen_ctc_loss(
    ...
    bool deterministic,
    bool zero_infinity) {
  SMITH_CHECK(false, "miopen_ctc_loss: ATen not compiled with MIOpen support");
}
...
std::tuple<Tensor, Tensor> miopen_ctc_loss(
  ...
  SMITH_CHECK(log_probs_t.numel() > 0, "log_probs tensor must not be empty");
  ...
  SMITH_CHECK(BLANK == 0, "blank must be label 0 for miopen_ctc_loss");
