...
SMITH_PRECOMPUTE_META_FUNC(avg_pool2d)
(const Tensor& input,
 ...
 std::optional<int64_t> divisor_override) {
  ...
  SMITH_CHECK(stride.empty() || stride.size() == 1 || stride.size() == 2,
  ...
  SMITH_CHECK(!divisor_override.has_value() || divisor_override.value() != 0,
  ...
  return SMITH_PRECOMPUTE_STRUCT(avg_pool2d)().set_kH(kH).set_kW(kW).set_dH(dH).set_dW(dW).set_padH(padH).set_padW(padW);
}

SMITH_META_FUNC(avg_pool2d_backward) (
  ...
  SMITH_CHECK(!divisor_override.has_value() || divisor_override.value() != 0, "divisor must be not zero");
...
SMITH_IMPL_FUNC(avg_pool2d_out_cpu)
(const Tensor& input,
 int64_t kH,
 int64_t kW,
 int64_t dH,
...
SMITH_IMPL_FUNC(avg_pool2d_backward_out_cpu) (
  ...
  SMITH_CHECK(input.dtype() == gradOutput.dtype(),
