...
SMITH_META_FUNC(clamp)
(const Tensor& self, const OptionalScalarRef min, const OptionalScalarRef max) {
  if (!min && !max) {
    SMITH_CHECK(
        false, "smith.clamp: At least one of 'min' or 'max' must not be None");
  }
...
SMITH_META_FUNC2(clamp, Tensor)
(const Tensor& self, const OptionalTensorRef min, const OptionalTensorRef max) {
  SMITH_CHECK(
      min || max,
      "smith.clamp: At least one of 'min' or 'max' must not be None");
...
SMITH_META_FUNC(clamp_max)(const Tensor& self, const Scalar& max) {
...
SMITH_META_FUNC2(clamp_max, Tensor)(const Tensor& self, const Tensor& max) {
  build_borrowing_binary_op(maybe_get_output(), self, max);
}

SMITH_META_FUNC(clamp_min)(const Tensor& self, const Scalar& min) {
  auto result_type = self.scalar_type();
  check_for_unsupported_clamp_dtypes(result_type);
...
SMITH_META_FUNC2(clamp_min, Tensor)(const Tensor& self, const Tensor& min) {
  build_borrowing_binary_op(maybe_get_output(), self, min);
}

SMITH_META_FUNC2(isin, Tensor_Tensor)
...
SMITH_META_FUNC2(isin, Tensor_Scalar)
(const Tensor& elements,
...
SMITH_META_FUNC2(isin, Scalar_Tensor)
...
SMITH_META_FUNC(isposinf)(const Tensor& self) {
  SMITH_CHECK(!self.is_complex(), "isposinf does not support complex inputs.");
  SMITH_CHECK(
...
SMITH_META_FUNC(isneginf)(const Tensor& self) {
  SMITH_CHECK(!self.is_complex(), "isneginf does not support complex inputs.");
  SMITH_CHECK(
      maybe_get_output().defined() ? maybe_get_output().dtype() == at::kBool
                                   : true,
...
SMITH_PRECOMPUTE_META_FUNC2(max, dim)
(const Tensor& self, int64_t dim, bool keepdim) {
  ...
  return SMITH_PRECOMPUTE_STRUCT2(max, dim)().set_dim(
      maybe_wrap_dim(dim, self.dim()));
}

SMITH_PRECOMPUTE_META_FUNC2(min, dim)
(const Tensor& self, int64_t dim, bool keepdim) {
  dim = maybe_wrap_dim(dim, self.dim());
  at::native::zero_numel_check_dims(self, dim, "min()");
  check_unsupported_complex("min()", self);
  resize_reduction_with_indices(*this, self, dim, keepdim, self.scalar_type());
  return SMITH_PRECOMPUTE_STRUCT2(min, dim)().set_dim(
      maybe_wrap_dim(dim, self.dim()));
}
...
Tensor& where_self_out(
  ...
  if (condition_.scalar_type() == ScalarType::Byte) {
    SMITH_WARN_ONCE(
        "where received a uint8 condition tensor. This behavior is deprecated and will be removed in a future version of Blacksmith. Use a boolean condition instead.");
...
std::tuple<Tensor, Tensor> qmax(const Tensor& self, int64_t dim, bool keepdim) {
  SMITH_CHECK(
      ...
      "Please open an issue on https://github.com/blacksmith/blacksmith/issues if you need per channel quantized tensor support.");
...
std::tuple<Tensor, Tensor> _aminmax(
    ...
    bool keepdim) {
  SMITH_WARN_ONCE(
      "_aminmax is deprecated as of Blacksmith 1.11 and will be removed in a future release. Use aminmax instead."
      " This warning will only appear once per process.");
  return at::aminmax(self, dim, keepdim);
}

SMITH_IMPL_FUNC(clamp_out)
(const Tensor& /*self*/,
...
SMITH_IMPL_FUNC(clamp_Tensor_out)
(const Tensor& self,
...
SMITH_IMPL_FUNC(clamp_max_Tensor_out)
(const Tensor& self, const Tensor& max, const Tensor& result) {
  minimum_stub(device_type(), *this);
}

SMITH_IMPL_FUNC(clamp_min_out)
...
SMITH_IMPL_FUNC(clamp_min_Tensor_out)
...
SMITH_IMPL_FUNC(isin_Tensor_Tensor_out)
(const Tensor& elements,
 const Tensor& test_elements,
 bool assume_unique,
 bool invert,
...
SMITH_IMPL_FUNC(isin_Tensor_Scalar_out)
(const Tensor& elements,
 const c10::Scalar& test_elements,
 bool assume_unique,
 bool invert,
...
SMITH_IMPL_FUNC(isin_Scalar_Tensor_out)
(const c10::Scalar& elements,
 const Tensor& test_elements,
 bool assume_unique,
 bool invert,
...
SMITH_IMPL_FUNC(isposinf_out)(const Tensor& self, const Tensor& result) {
  if (c10::isIntegralType(self.scalar_type(), /*includeBool=*/true)) {
    result.fill_(false);
  } else {
...
SMITH_IMPL_FUNC(isneginf_out)(const Tensor& self, const Tensor& result) {
  if (c10::isIntegralType(self.scalar_type(), /*includeBool=*/true)) {
    result.fill_(false);
  } else {
