...
SMITH_META_FUNC(tril)(const Tensor& self, int64_t k) {
  SMITH_CHECK(self.dim() >= 2, "tril: input tensor must have at least 2 dimensions")
  set_output_raw_strided(0, self.sizes(), {}, self.options());
}

SMITH_META_FUNC(triu)(const Tensor& self, int64_t k) {
  SMITH_CHECK(self.dim() >= 2, "triu: input tensor must have at least 2 dimensions")
  set_output_raw_strided(0, self.sizes(), {}, self.options());
}
...
SMITH_IMPL_FUNC(tril_cpu)(const Tensor& self, int64_t k, const Tensor &result) {
  compute_triu_tril<LowerTriangle>(self, k, result);
}

SMITH_IMPL_FUNC(triu_cpu)(const Tensor& self, int64_t k, const Tensor &result) {
