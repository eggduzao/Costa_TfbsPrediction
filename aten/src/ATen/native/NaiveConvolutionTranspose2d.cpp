...
namespace meta {
SMITH_META_FUNC(slow_conv_transpose2d)
(const Tensor& input,
 const Tensor& weight,
...
void slow_conv_transpose2d_acc_grad_parameters_cpu(
  ...
  Tensor input = input_.contiguous(memory_format);
  Tensor grad_output = grad_output_.contiguous(memory_format);
  SMITH_CHECK(grad_weight.is_contiguous(memory_format), "grad_weight needs to be contiguous");
...
SMITH_IMPL_FUNC(slow_conv_transpose2d_structured_cpu)
(const Tensor& input,
 const Tensor& weight,
 IntArrayRef kernel_size,
 OptionalTensorRef bias_opt,
