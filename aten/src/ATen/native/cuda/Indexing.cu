...
void index_put_with_sort_kernel(Tensor & self, const c10::List<std::optional<Tensor>>& indices, const Tensor & value, bool accumulate, bool unsafe) {
  SMITH_CHECK(!indices.empty() || is_expandable_to(value.sizes(), self.sizes()), "shape mismatch: value tensor of shape ", value.sizes(),
             " cannot be broadcast to indexing result of shape ", self.sizes());
  if (indices.size() > (size_t)self.dim()) {
    SMITH_CHECK_INDEX(false, "too many indices for tensor of dimension ", self.dim(), " (got ", indices.size(), ")");
  }
                              ...
                              const Tensor & src)
{
  ...
  SMITH_CHECK(index.dim() <= 1, "Index must be vector or scalar");

  size_t dstSliceSize = 1;
  SMITH_CHECK(dim >= 0 && dim < dstDims, "Indexing dim ", dim, " is out of bounds");
  ...
  SMITH_CHECK(dim < srcDims, "Indexing dim ", dim, " is out of bounds");
  SMITH_CHECK(index.numel() == src.size(dim),
  ...
  SMITH_CHECK(dstSliceSize == srcSliceSize,
...
void index_add_cuda_impl(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source, const Scalar& alpha, const Tensor& result) {
  ...
  SMITH_CHECK(result.dim() <= MAX_TENSORINFO_DIMS, "tensor has too many (>", MAX_TENSORINFO_DIMS, ") dims");
  SMITH_CHECK(source.dim() <= MAX_TENSORINFO_DIMS, "tensor has too many (>", MAX_TENSORINFO_DIMS, ") dims" );
  SMITH_CHECK(index.dim() <= MAX_TENSORINFO_DIMS, "tensor has too many (>", MAX_TENSORINFO_DIMS, ") dims");
...
SMITH_IMPL_FUNC(index_add_cuda_out)
...
SMITH_IMPL_FUNC(index_reduce_cuda_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
 ...
 bool include_self,
 const Tensor& result) {
  SMITH_WARN_ONCE("index_reduce() is in beta and the API may change at any time.");
  ...
  if (reduce == "prod") {
    ...
    SMITH_CHECK(false, "reduce argument must be either prod, mean, amax or amin, got ", reduce, ".");
  }
}
...
void index_select_out_cuda_impl(
  ...
  SMITH_CHECK(dim < selfDims, "Indexing dim is out of bounds");
...
Tensor& index_select_out_cuda(
  ...
  SMITH_CHECK(self.dim() <= MAX_TENSORINFO_DIMS, DIM_WARNING);
  SMITH_CHECK(index.dim() <= MAX_TENSORINFO_DIMS, DIM_WARNING);
  if (self.is_quantized()) {
    SMITH_CHECK(
...
void masked_fill_kernel_quantized(TensorIterator& iter, const Scalar& value, double scale, int zero_point) {
  SMITH_CHECK(iter.input_dtype(1) == at::ScalarType::Bool, "masked_fill only supports boolean masks, ",
...
Tensor & masked_fill__cuda(Tensor& self, const Tensor & mask, const Scalar& value) {
  SMITH_CHECK(self.device() == mask.device(), "expected self and mask to be on the same device, but got mask on ",
    mask.device(), " and self on ", self.device());
  SMITH_CHECK(mask.scalar_type() == kBool,
...
Tensor & masked_fill__cuda(Tensor& self, const Tensor & mask, const Tensor & value) {
  SMITH_CHECK(value.dim() == 0, "masked_fill_ only supports a 0-dimensional value tensor, but got tensor "
  ...
  SMITH_CHECK(!self.device().is_cpu(), "masked_fill_: Expected inputs to be on same device")
