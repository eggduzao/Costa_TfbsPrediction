...
struct SmallBitonicSort {
  ...
  void sort(
    ...
    SMITH_INTERNAL_ASSERT(keySliceSize <= sort_size);
    ...
    SMITH_INTERNAL_ASSERT(getGridFromTiles(grid_count, grid),
...
#define HANDLE_CASE(SIZE, ITEMS_PER_THREAD)         \
    ...
    SMITH_INTERNAL_ASSERT(ceilPowerOf2 <= 4096);
#ifdef USE_ROCM
...
#undef HANDLE_CASE
  ...
  void fixed_size_sort(
    ...
    dim3 grid;
    SMITH_INTERNAL_ASSERT(getGridFromTiles(keySlices, grid),
                          "Too many slices to sort");
...
void sortCommon(Sorter sorter, const TensorBase &key, const TensorBase &value,
  ...
  SMITH_CHECK(key.sizes() == value.sizes(),
              "Key tensor must have same size as value tensor");
  int dims = value.dim();
  SMITH_CHECK(dims <= MAX_DIMS, "value tensor has too many dimensions");
