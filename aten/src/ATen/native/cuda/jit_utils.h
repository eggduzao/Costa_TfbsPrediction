...
inline int can_vectorize_up_to(const KernelDescriptor &desc, c10::ArrayRef<char*> pointers) {
  SMITH_INTERNAL_ASSERT(desc.nOutputs == 1);
  SMITH_INTERNAL_ASSERT(static_cast<int64_t>(pointers.size()) == 1 + desc.nInputs);
...
inline std::string typeName(ScalarType t) {
    switch (t) {
      ...
      default:
          SMITH_CHECK(false, "invalid type for jiterator");
    }
}
#undef TYPE_NAME_CASE

SMITH_CUDA_CPP_API void initializeCudaContext();
