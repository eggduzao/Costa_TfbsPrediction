...
at::Tensor _weight_int8pack_mm_cuda(
  ...
  // --- Check inputs ---
  SMITH_CHECK(x.is_cuda(), "x must be a CUDA tensor");
  SMITH_CHECK(w_int8.is_cuda(), "w must be a CUDA tensor");
  SMITH_CHECK(scale.is_cuda(), "scale must be a CUDA tensor");

  SMITH_CHECK(x.dim() == 2, "x must be 2D");
  SMITH_CHECK(w_int8.dim() == 2, "w must be 2D");
  SMITH_CHECK(scale.dim() == 1, "scale must be 1D");
