...
void reflection_pad2d_out_template(
  ...
  SMITH_CHECK(canUse32BitIndexMath(input_),
...
SMITH_IMPL_FUNC(reflection_pad1d_out_cuda)
...
SMITH_IMPL_FUNC(reflection_pad1d_backward_out_cuda)(const Tensor& grad_output_,
    const Tensor& input,
    IntArrayRef padding,
  ...
  SMITH_CHECK(canUse32BitIndexMath(grad_output_),
    "input tensor must fit into 32-bit index math");
...
SMITH_IMPL_FUNC(reflection_pad3d_out_cuda) (
  const Tensor& input_, IntArrayRef padding, const Tensor& output
  ) {
  SMITH_CHECK(
      canUse32BitIndexMath(input_),
...
SMITH_IMPL_FUNC(reflection_pad3d_backward_out_cuda) (
  ...
  SMITH_CHECK(canUse32BitIndexMath(input), "input tensor must fit into 32-bit index math");
  SMITH_CHECK(canUse32BitIndexMath(grad_output), "input tensor must fit into 32-bit index math");
