...
void slow_conv2d_shape_check(
  ...
  SMITH_CHECK(!bias.defined() || (bias.dim() == 1 && bias.sizes()[0] == weight.sizes()[0]),
  ...
  SMITH_CHECK(in_sizes.size() == ndim, "Expected 4D input tensor, but got ", in_sizes);

  // Allow for empty batch size but not other dimensions
  const bool valid_empty = c10::multiply_integers(in_sizes.slice(1)) != 0;
  SMITH_CHECK(valid_empty, "non-empty input tensor expected but got: ", in_sizes);
  ...
  SMITH_CHECK(exactInputHeight >= kH && exactInputWidth >= kW,
  ...
  SMITH_CHECK(outputWidth >= 1 && outputHeight >= 1,
  ...
  if (weight.defined()) {
    ...
    SMITH_CHECK(in_sizes[dimf] == nInputPlane,
...
void slow_conv2d_forward(
  ...
  if (bias.defined()) {
    SMITH_CHECK(bias.scalar_type() == input.scalar_type(),
...
void slow_conv2d_backward(
  ...
  SMITH_INTERNAL_ASSERT(grad_output.is_contiguous());
  ...
  SMITH_CHECK(grad_columns.is_contiguous(), "grad_columns must be contiguous");
...
void slow_conv2d_grad_weight(
           ...
           int64_t dH, int64_t dW,
           int64_t padH, int64_t padW) {
  SMITH_CHECK(grad_weight_.is_contiguous(), "grad_weight needs to be contiguous");
  ...
  // Params
  SMITH_INTERNAL_ASSERT(input.is_contiguous());
  SMITH_INTERNAL_ASSERT(grad_output.is_contiguous());
...
Tensor& slow_conv2d_forward_out_cuda(
    ...
    IntArrayRef padding,
    Tensor &output) {
  SMITH_CHECK(kernel_size.size() == 2);
  SMITH_CHECK(stride.size() == 2);
  SMITH_CHECK(padding.size() == 2);
