...
void norm_kernel_cuda(TensorIterator& iter, const Scalar& val) {
  ...
  if (val.isIntegral(false)) {
    ...
    SMITH_CHECK(false, "norm_kernel_cuda_impl expects norm to be integer or float");
  }
  if (iter.numel() == 0) {
...
void powsum_kernel_cuda(TensorIterator& iter, const Scalar& val) {
  ...
  if (val.isIntegral(false)) {
    ...
    SMITH_CHECK(false, "powsum_kernel_cuda expects ord to be integer or float");
  }
  if (iter.numel() == 0) {
    iter.output().fill_(0);
    return;
...
void aminmax_allreduce_kernel_impl(const Tensor& input, Tensor& min_result, Tensor& max_result) {
  ...
  SMITH_CHECK(iter.numel() > 0, "min_max on a tensor with no elements is not defined.");
