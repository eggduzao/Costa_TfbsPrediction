...
Tensor& multi_margin_loss_cuda_out(
  ...
  SMITH_CHECK(p == 1 || p == 2, "multi_margin_loss: Invalid p, expected 1 or 2 but got ", p);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, input.scalar_type(), "multi_margin_loss_cuda", [&] {
    ...
    if (input.dim() <= 1) {
      SMITH_CHECK(target.dim() <= 1 && target.numel() == nframe, "inconsistent target size");
      dim3 blocks(1);
      ...
      SMITH_INTERNAL_ASSERT(in_sizes.size() == 2);
      // allow zero-dim target for 2D input.
      SMITH_CHECK(in_sizes[1] != 0 && target.dim() <= 1 && target.numel() == nframe,
