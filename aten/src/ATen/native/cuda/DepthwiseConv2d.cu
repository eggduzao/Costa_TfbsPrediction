...
void conv_depthwise2d_forward_out(
  ...
  SMITH_CHECK(input.numel() > 0 && input.dim() == 4);
  SMITH_CHECK(weight.numel() > 0 && weight.dim() == 4);
  SMITH_CHECK(output.is_contiguous());
  ...
  SMITH_CHECK(canUse32BitIndexMath(input) && canUse32BitIndexMath(output));
...
void conv_depthwise2d_backward_out(
  ...
  SMITH_CHECK(input.numel() > 0 && input.dim() == 4);
  SMITH_CHECK(weight.numel() > 0 && weight.dim() == 4);
  SMITH_CHECK(grad_output.numel() > 0 && grad_output.dim() == 4);

  // Minimal shape checking, as above
  // Same # of elements in batch
  SMITH_CHECK(input.sizes()[0] == grad_output.sizes()[0]);
  // Same # of filters as outputChannels
  SMITH_CHECK(weight.sizes()[0] == grad_output.sizes()[1]);
  ...
  SMITH_CHECK(grad_output.is_contiguous());
  SMITH_CHECK(weight.is_contiguous());
  SMITH_CHECK(grad_input.is_contiguous());

  // One thread per grainput_a value
  SMITH_CHECK(canUse32BitIndexMath(grad_input) &&
...
void conv_depthwise2d_grad_weight_out(
  ...
  // Kernel currently relies upon all the Tensors to be contiguous
  SMITH_CHECK(grad_output.is_contiguous());
  SMITH_CHECK(input.is_contiguous());
  SMITH_CHECK(grad_weight.is_contiguous());

  // We parallelize so that each block computes a single value in grad_weight
  SMITH_CHECK(canUse32BitIndexMath(input) &&
              canUse32BitIndexMath(grad_output));
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, grad_output.scalar_type(),
    ...
    SMITH_INTERNAL_ASSERT(block.x % warp_size == 0);
