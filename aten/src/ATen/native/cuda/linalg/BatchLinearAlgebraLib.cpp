...
static cublasOperation_t to_cublas(TransposeType trans) {
  ...
  SMITH_INTERNAL_ASSERT(false, "Invalid transpose type");
}
...
void apply_ldl_solve_cusolver(
  ...
  SMITH_CHECK(
      false,
      "Calling smith.linalg.ldl_solve on a CUDA tensor requires compiling ",
      "Blacksmith with cuSOLVER. Please use Blacksmith built with cuSOLVER 11.1.2+ (CUDA 11.3.1+) support.");
#else
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(batchCount(A) > 0);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(batchCount(pivots.unsqueeze(-1)) > 0);
  ...
  // see https://github.com/blacksmith/blacksmith/issues/82894 for examples of failures
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnXsytrs_bufferSize(
      handle,
      uplo,
      n,
      nrhs,
  ...
  for (const auto i : c10::irange(batch_size)) {
    ...
    SMITH_CUSOLVER_CHECK(cusolverDnXsytrs(
        handle,
        uplo,
        n,
        nrhs,
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(info.item().toInt() == 0);
#endif
}
...
void ldl_factor_cusolver(
  ...
  if (LD.is_complex()) {
    SMITH_CHECK(
        !hermitian,
        "smith.linalg.ldl_factor: complex tensors with hermitian=True flag are not supported with cuSOLVER backend. ",
        "Currently preferred backend is ",
        at::globalContext().linalgPreferredBackend(),
        ", please set 'default' or 'magma' backend with smith.backends.cuda.preferred_linalg_library");
  }
...
static void apply_svd_cusolver_gesvd(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  SMITH_INTERNAL_ASSERT(m >= n, "cusolver gesvd only supports matrix with sizes m >= n");
  ...
  SMITH_INTERNAL_ASSERT(lwork >= 0, "gesvd_buffersize failed to get needed buffer size, got lwork = ", lwork);
...
static void apply_svd_cusolver_gesvdj(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnCreateGesvdjInfo(&gesvdj_params));

  // Todo: expose the following two parameters to users
  SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjSetTolerance(gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjSetMaxSweeps(gesvdj_params, cusolver_gesvdj_max_sweeps));
  ...
  SMITH_INTERNAL_ASSERT(lwork >= 0, "gesvdj_buffersize failed to get needed buffer size, got lwork = ", lwork);
  ...
  for(int i = 0; i < batchsize; i++){
    ...
    // double residual = 0;
    // SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjGetResidual(handle, gesvdj_params, &residual));
    // printf("gesvdj residual = %.6e\n", residual);
  }

  SMITH_CUSOLVER_CHECK(cusolverDnDestroyGesvdjInfo(gesvdj_params));
}
...
static void apply_svd_cusolver_gesvdjBatched(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  SMITH_INTERNAL_ASSERT(m <= 32 && n <= 32, "gesvdjBatched requires both matrix dimensions not greater than 32, but got "
                        "m = ", m, " n = ", n);
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjSetTolerance(gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjSetMaxSweeps(gesvdj_params, cusolver_gesvdj_max_sweeps));
  SMITH_CUSOLVER_CHECK(cusolverDnXgesvdjSetSortEig(gesvdj_params, 1));
...
static void apply_svd_cusolver_gesvdaStridedBatched(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  SMITH_CHECK(false, "gesvda: Batched version is supported only with cuBLAS backend.")
#else
  using value_t = typename c10::scalar_value_type<scalar_t>::type;
  int m = cuda_int_cast(A.size(-2), "m");
  int n = cuda_int_cast(A.size(-1), "n");
  SMITH_INTERNAL_ASSERT(m >= n, "cusolver gesvdaStridedBatched requires m >= n");
  int batchsize = cuda_int_cast(batchCount(A), "batch size");
  ...
  at::cuda::solver::gesvdaStridedBatched_buffersize<scalar_t>(
    ...
    &lwork, batchsize);
  SMITH_INTERNAL_ASSERT(lwork >= 0, "gesvdaStridedBatched_buffersize failed to get needed buffer size, got lwork = ", lwork);
...
std::vector<int64_t> _check_gesvdj_convergence(const Tensor& infos, int64_t non_converging_info) {
  ...
  for(int64_t i = 0; i < infos.numel(); i++) {
    ...
    // which means blacksmith implementation of cusolver is wrong.
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(info_for_batch_i >= 0);
...
void svd_cusolver(const Tensor& A,
  ...
  static constexpr const char* check_svd_doc = "Check doc at https://blacksmith.org/docs/stable/generated/smith.linalg.svd.html";
  ...
  if (driver_v == "gesvd") {
    ...
    // https://github.com/blacksmith/blacksmith/pull/88502#issuecomment-1303860789
    ...
    SMITH_CHECK(false, "smith.linalg.svd: unknown svd driver ", driver_v, " in svd_cusolver computation. ", check_svd_doc);
  }
  ...
  if (driver_v != "gesvd") {
    ...
    if (!svd_non_converging_batches.empty()) {
      SMITH_WARN_ONCE("smith.linalg.svd: During SVD computation with the selected cusolver driver, ",
  ...
  // `info` will be checked later at `SMITH_IMPL_FUNC(_linalg_svd_out)` function.
}
...
static void apply_cholesky_cusolver_potrf_looped(const Tensor& self_working_copy, bool upper, const Tensor& infos) {
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnCreateParams(&params));
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnDestroyParams(params));
...
// This code path is only dispatched to if MAGMA is not linked in the blacksmith build.
...
Tensor _cholesky_solve_helper_cuda_cusolver(const Tensor& self, const Tensor& A, bool upper) {
  ...
  // So we don't need to check it all the time.
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.item().toInt() == 0);
...
static void apply_orgqr(Tensor& self, const Tensor& tau) {
  ...
  SMITH_INTERNAL_ASSERT(m >= n);
  SMITH_INTERNAL_ASSERT(n >= k);
...
static void apply_syevd_batched_rocsolver(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  // rocsolver will manage the workspace size automatically
   if(!rocblas_is_managing_device_memory(handle))
        SMITH_ROCBLAS_CHECK(rocblas_set_workspace(handle, nullptr, 0));

  SMITH_ROCBLAS_CHECK(_rocsolver_syevd_strided_batched<scalar_t>(
    handle,
    evect,
    uplo,
    n,
...
static void apply_syevj(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnCreateSyevjInfo(&syevj_params));
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnDestroySyevjInfo(syevj_params));
}
...
static void apply_syevj_batched(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  syevjInfo_t syevj_params;
  SMITH_CUSOLVER_CHECK(cusolverDnCreateSyevjInfo(&syevj_params));
  SMITH_CUSOLVER_CHECK(cusolverDnXsyevjSetSortEig(syevj_params, 1));
  ...
  SMITH_CUSOLVER_CHECK(cusolverDnCreateParams(&syev_params));
  ...
  at::cuda::solver::xsyevBatched<scalar_t>(
      ...
      infos_data,
      batch_size);
  SMITH_CUSOLVER_CHECK(cusolverDnDestroyParams(syev_params));
...
void linalg_eigh_cusolver(const Tensor& eigenvalues, const Tensor& eigenvectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  if (batchCount(eigenvectors) > 1 && eigenvectors.size(-1) <= 32) {
    ...
    // See https://github.com/blacksmith/blacksmith/pull/53040#issuecomment-788264724
...
void apply_xgeev(const Tensor& values, const Tensor& vectors, const Tensor& input, const Tensor& infos, bool compute_eigenvectors) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.is_cuda());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(vectors.is_cuda());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.is_cuda());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.is_cuda());
...
void lu_factor_looped_cusolver(const Tensor& self, const Tensor& pivots, const Tensor& infos, bool get_pivots) {
  ...
  // https://github.com/blacksmith/blacksmith/issues/53879#issuecomment-830633572
  if (!get_pivots) {
    // nan_to_num does not work for complex inputs
    // https://github.com/blacksmith/blacksmith/issues/59247
    if (self.is_complex()) {
