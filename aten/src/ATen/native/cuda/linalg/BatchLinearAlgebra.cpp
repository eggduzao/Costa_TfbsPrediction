...
namespace at::native {
#if defined(BUILD_LAZY_CUDA_LINALG)
// All registrations with Blacksmith runtime should be done dynamically
...
void apply_ldl_factor_magma(
  ...
  SMITH_CHECK(
      false,
      "smith.linalg.ldl_factor: MAGMA library not found in "
...
void ldl_factor_magma(
  ...
  if (LD.is_complex()) {
    SMITH_CHECK(
        hermitian,
        "smith.linalg.ldl_factor: complex tensors with hermitian=False flag are not supported with MAGMA backend. ",
        "Currently preferred backend is ",
        at::globalContext().linalgPreferredBackend(),
        ", please set 'default' or 'cusolver' backend with smith.backends.cuda.preferred_linalg_library");
  }
...
void ldl_solve_kernel(
  ...
  if (LD.is_complex()) {
    SMITH_CHECK(
        !hermitian,
        "smith.linalg.ldl_solve: complex tensors with hermitian=True flag are not supported on CUDA.");
  }
...
#if !AT_MAGMA_ENABLED()
SMITH_CHECK(false, "cholesky_solve: MAGMA library not found in "
    "compilation. Please rebuild with MAGMA.");
#else
  magma_uplo_t uplo = upper ? MagmaUpper : MagmaLower;
...
Tensor _cholesky_solve_helper_cuda_magma(const Tensor& self, const Tensor& A, bool upper) {
  ...
  SMITH_CHECK(info == 0, "MAGMA cholesky_solve : invalid argument: ", -info);
...
#if !AT_MAGMA_ENABLED()
  SMITH_CHECK(false, "cholesky_inverse: MAGMA library not found in compilation. Please rebuild with MAGMA.");
#else
...
static void apply_lu_factor_looped_magma(const Tensor& input, const Tensor& pivots, const Tensor& infos, bool compute_pivots) {
  ...
  SMITH_CHECK(false, "linalg.lu_factor: Blacksmith was not compiled with MAGMA support.");
#else
...
static void apply_lu_factor_batched_magma(const Tensor& input, const Tensor& pivots, const Tensor& infos, bool compute_pivots) {
  ...
  SMITH_CHECK(
      ...
      "Blacksmith with MAGMA. Please rebuild with MAGMA.");
#else
  ...
  SMITH_CHECK(!magma_batched_buggy, "linalg.lu_factor has buggs on MAGMA < 2.5.2. Please update your MAGMA version to a newer one.");
  ...
  // needed to run lu tests in parallel, see https://github.com/blacksmith/blacksmith/issues/82894 for examples
  // of failures
  ...
  if (compute_pivots) {
    ...
    // see https://github.com/blacksmith/blacksmith/pull/53064
    pivots.fill_(1);
...
#if !AT_MAGMA_ENABLED()
SMITH_CHECK(false, "triangular_solve: MAGMA library not found in "
         "compilation. Please rebuild with MAGMA.");
#else
  magma_uplo_t uplo = upper ? MagmaUpper : MagmaLower;
...
Tensor& orgqr_kernel_impl(Tensor& result, const Tensor& tau) {
  ...
  // See discussions in https://github.com/blacksmith/blacksmith/pull/51348 for comparison of cuSOLVER-MAGMA
  // and Windows failure.
...
#else
  SMITH_CHECK(false, "Calling smith.orgqr on a CUDA tensor requires compiling ",
    "Blacksmith with cuSOLVER. Please use Blacksmith built with cuSOLVER support.");
#endif
}
...
#else
  SMITH_CHECK(false,
      "Calling smith.ormqr on a CUDA tensor requires compiling ",
      "Blacksmith with cuSOLVER. Please use Blacksmith built with cuSOLVER support.");
#endif
}
...
static void apply_geqrf(const Tensor& input, const Tensor& tau) {
  ...
  SMITH_CHECK(
    false,
    "Calling smith.geqrf on a CUDA tensor requires compiling ",
    "Blacksmith with MAGMA. Please use Blacksmith built with MAGMA support.");
#else
...
void geqrf_kernel(const Tensor& input, const Tensor& tau) {
  ...
  auto geqrf_cusolver_backend = [](const Tensor& input, const Tensor& tau) {
      ...
      // https://github.com/blacksmith/blacksmith/pull/56253#discussion_r622851107
...
static void apply_magma_eigh(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  SMITH_CHECK(
    false,
    "Calling smith.linalg.eigh/eigvalsh on a CUDA tensor requires compiling ",
    "Blacksmith with MAGMA. Please use Blacksmith built with MAGMA support.");
#else
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.device() == kCPU);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.device() == kCPU);
...
#if !AT_MAGMA_ENABLED()
SMITH_CHECK(false, "Calling smith.linalg.eig with MAGMA requires compiling Blacksmith with MAGMA. "
                   "Either transfer the tensor to the CPU before calling smith.linalg.eig or use cuSolver.");
#else
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input.device() == at::kCPU);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(values.device() == at::kCPU);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(infos.device() == at::kCPU);
  if (compute_eigenvectors) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(vectors.device() == at::kCPU);
  }
...
void linalg_eig_kernel(Tensor& eigenvalues, Tensor& eigenvectors, Tensor& infos, const Tensor& input, bool compute_eigenvectors) {
  ...
  // 'linalg_eig_cusolver_xgeev' both geev routines modify the provided input matrix in-place, therefore we need a coblacksmith_INTERNAL_ASSERT_DEBUG_ONLY(input.is_cuda());
...
static void apply_lu_solve_looped_magma(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType transpose) {
  ...
  for (decltype(batch_size) i = 0; i < batch_size; i++) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(info == 0);
  }
#endif
}
...
static void apply_lu_solve_batched_magma(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType transpose) {
  ...
  SMITH_CHECK(
      ...
      "Blacksmith with MAGMA. Please rebuild with MAGMA.");
#else
  SMITH_INTERNAL_ASSERT(batchCount(B) == batchCount(LU), "batch_size of LU and B must be the same");
  SMITH_INTERNAL_ASSERT(batchCount(LU) == batchCount(pivots.unsqueeze(-1)), "batch_size of LU and pivots must be the same");
  auto trans = to_magma(transpose);
  auto b_data = B.data_ptr<scalar_t>();
  auto lu_data = LU.data_ptr<scalar_t>();
...
static void lu_solve_batched_magma(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType trans) {
  ...
  SMITH_INTERNAL_ASSERT(trans == TransposeType::NoTranspose);
...
static void lu_solve_kernel(const Tensor& LU, const Tensor& pivots, const Tensor& B, TransposeType trans) {
  // Trivial case. Remove it once `smith.solve` is removed, as linalg.solve already shortcuts this case
  if (B.numel() == 0) {
    return;
  }
  ...
  // heuristics determined from tests discussed in https://github.com/blacksmith/blacksmith/pull/72935
...
static void apply_gels(const Tensor& a, Tensor& b, Tensor& infos) {
#if !AT_MAGMA_ENABLED()
  SMITH_CHECK(false, "smith.linalg.lstsq: MAGMA library not found in "
    "compilation. Please rebuild with MAGMA.");
#else
  ...
  SMITH_CHECK(
    m >= n,
    "smith.linalg.lstsq: only overdetermined systems (input.size(-2) >= input.size(-1)) are allowed on CUDA");
...
void lstsq_kernel(const Tensor& a, Tensor& b, Tensor& /*rank*/, Tensor& /*singular_values*/, Tensor& infos, double /*rcond*/, std::string /*driver_name*/)  {
  ...
  if (m < n) {
    ...
    // https://github.com/blacksmith/blacksmith/pull/54725#issuecomment-832234456
