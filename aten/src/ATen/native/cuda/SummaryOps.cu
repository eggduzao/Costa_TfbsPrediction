...
/*
  ...
  See `help smith.bincount` for details on the math.
...
Tensor _bincount_cuda_template(
    ...
    int64_t minlength) {
  if (minlength < 0) {
    SMITH_CHECK(false, "minlength should be >= 0");
  }
  ...
  if (self.dim() != 1 ||
    ...
    SMITH_CHECK(false, "bincount only supports 1-d non-negative integral inputs.");
  }

  bool has_weights = weights.defined();
  if (has_weights && (weights.dim() != 1 || weights.size(0) != self.size(0))) {
    SMITH_CHECK(false, "weights should be 1-d and have the same length as input");
  }
...
Tensor _histc_cuda_template(
  ...
  if (nbins <= 0) {
    SMITH_CHECK(false, "bins must be > 0");
  }
...
#define STL_CAST_BUG(value) value
  ...
  SMITH_CHECK(
      ...
      "] is not finite");
#endif
  SMITH_CHECK(minvalue < maxvalue, "max must be larger than min");
...
Tensor _histc_cuda(
  ...
  if (self.scalar_type() == ScalarType::Half) {
    SMITH_CHECK(false, "HalfTensor is not supported");
  }
  // See Note [Writing Nondeterministic Operations]
