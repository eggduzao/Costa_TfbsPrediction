...
Tensor host_softmax(const Tensor & input_, const int64_t dim_, const bool half_to_float, const Tensor& output){
  if (half_to_float) {
    SMITH_CHECK(input_.scalar_type() == ScalarType::Half, "conversion is supported for Half type only");
  }
  auto input = input_.contiguous();
  static_assert(std::is_same_v<acc_type<at::Half, true>, float>, "accscalar_t for half should be float");
  if (input.dim() == 0) input = input.view(1);
  int64_t dim = maybe_wrap_dim(dim_, input.dim());
  SMITH_CHECK(dim >=0 && dim < input.dim(), "dim must be non-negative and less than input dimensions");
          ...
          if (dim_size <= 2048 && dim_size*sizeof(scalar_t) <= 8192) {
            ...
            if constexpr (use_fast_softmax) {
              ...
              if(potential_reg_cnt < 10){
                SMITH_INTERNAL_ASSERT(potential_reg_cnt > 0, "potential_reg_cnt for softmax with register should be greater than 0.");
...
void host_softmax_backward(const Tensor &grad_, const Tensor &output_, int64_t dim_, bool half_to_float, const Tensor &gI){
  ...
  SMITH_CHECK(dim >=0 && dim < grad.dim(), "dim must be non-negative and less than input dimensions");
...
SMITH_IMPL_FUNC(log_softmax_cuda_out) (
  const Tensor &input,
  const int64_t dim,
...
SMITH_IMPL_FUNC(log_softmax_backward_cuda_out) (
  const Tensor& grad,
  const Tensor& output,
  int64_t dim,
...
SMITH_IMPL_FUNC(softmax_cuda_out) (
  const Tensor &input,
  const int64_t dim,
  const bool half_to_float,
  const Tensor &output) {
...
SMITH_IMPL_FUNC(softmax_backward_cuda_out)
(const Tensor& grad,
 const Tensor& output,
 int64_t dim,
 ScalarType input_dtype,
...
Tensor masked_softmax_cuda(const Tensor& input_, const Tensor& mask_, const std::optional<int64_t> dim_, const std::optional<int64_t> mask_type_) {
  ...
  SMITH_CHECK(mask_.scalar_type() == ScalarType::Bool, "Mask should be a boolean tensor");

  SMITH_CHECK(mask_type_.has_value(), "Mask Type should be defined");
  int64_t mask_type = mask_type_.value();
  SMITH_CHECK((mask_type == 0) || (mask_type == 1) || (mask_type == 2), "Mask Type should be 0 (src_mask), 1 (src_key_padding_mask), or 2 (default_mask)");
  ...
  SMITH_CHECK(mask_.sizes() == input_.sizes() || is_BxT_mask || is_TxT_mask, "Mask shape should match input. mask: ", mask_.sizes(), " input: ", input_.sizes());
  ...
  if (is_BxT_mask) {
    ...
    SMITH_CHECK(input.size(1) % 2 == 0, "Only support when num_heads is even in transformer");
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::Half,
      ScalarType::BFloat16,
      input.scalar_type(),
...
Tensor masked_softmax_backward_cuda(
  ...
  SMITH_CHECK(dim >=0 && dim < grad.dim(), "dim must be non-negative and less than input dimensions");
  SMITH_CHECK(grad.sizes() == mask.sizes(), "Mask shape should match grad shape");
  SMITH_CHECK(mask.scalar_type() == ScalarType::Bool, "Mask should be a boolean tensor");
