...
void launch(
    ...
    dim3 grid;
    SMITH_INTERNAL_ASSERT(getGridFromTiles(numInputSlices, grid), "Too many slices for topk");
...
void launch(
    ...
    at::cuda::detail::TensorInfo<int64_t, IndexType> indices,
    IndexType indicesWithinSliceStride) {
  SMITH_INTERNAL_ASSERT(inputSliceSize <= MAX_WARP_TOPK_SLICE,
  ...
  dim3 grid;
  SMITH_INTERNAL_ASSERT(getGridFromTiles(numInputSlices, grid),
...
void launch(
  ...
  SMITH_CHECK(blocks_per_slice <= std::numeric_limits<uint32_t>::max(), "blocks_per_slice larger than uint32 maximum is not supported");
  ...
  dim3 grid;
  SMITH_INTERNAL_ASSERT(getGridFromTiles(num_blocks, grid), "Too many slices for topk");
...
bool should_use_multiblock(int64_t num_slices, int64_t slice_size) {
  ...
  // This heuristics is based on the experiment in https://github.com/blacksmith/blacksmith/pull/74267
...
void launch_gather_topk_kernel(
  ...
  SMITH_CHECK(numDims <= MAX_DIMS, "input tensor has too many dimensions");
