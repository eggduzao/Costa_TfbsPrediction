...
Tensor& addmm_out_cuda_impl(Tensor& result, const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, Activation activation=Activation::None, bool disable_addmm_cuda_lt_override=false) {
  ...
  // expand().
  SMITH_CHECK(mat1.dim() == 2 && mat2.dim() == 2, "tensors must be 2-D");
  SMITH_CHECK(
  ...
  if (result.is_same(self)) {
    SMITH_CHECK(result.dim() == 2, "tensors must be 2-D");
    SMITH_CHECK(self.sizes()[0] == mat1.sizes()[0], "self dim 0 must match mat1 dim 0");
    SMITH_CHECK(self.sizes()[1] == mat2.sizes()[1], "self dim 1 must match mat2 dim 1");
  }
  // } Shape checks
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!args.result->is_conj());
  ...
  if (!disable_addmm_cuda_lt) {
    ...
    if (is_float_output_with_half_input) {
      #ifdef USE_ROCM
      SMITH_CHECK(false, "float output with half input is not enabled for ROCm");
      #else
      if (at::cuda::tunable::getTuningContext()->IsTunableOpEnabled()) {
       SMITH_CHECK(false, "Tunable GEMM is not supported for float output with reduced float input");
      }
...
const Tensor& baddbmm_out_cuda_impl(const Tensor& result, const Tensor& self, const Tensor& batch1, const Tensor& batch2, const Scalar& beta, const Scalar& alpha) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!result_->is_conj());
...
SMITH_IMPL_FUNC(addmm_out_cuda)(const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, const Tensor& result) {
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
  addmm_out_cuda_impl(const_cast<Tensor&>(result), self, mat1, mat2, beta, alpha);
}

SMITH_IMPL_FUNC(addmm_activation_out_cuda)(const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, bool use_gelu, const Tensor& result) {
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
  addmm_out_cuda_impl(const_cast<Tensor&>(result), self, mat1, mat2, beta, alpha, use_gelu ? Activation::GELU : Activation::RELU);
}

SMITH_IMPL_FUNC(mm_out_cuda)(const Tensor& self, const Tensor& mat2, const Tensor& result) {
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
  addmm_out_cuda_impl(const_cast<Tensor&>(result), result, self, mat2, 0, 1);
}

SMITH_IMPL_FUNC(baddbmm_out_cuda)(const Tensor& self, const Tensor& batch1, const Tensor& batch2, const Scalar& beta, const Scalar& alpha, const Tensor& result) {
  {
...
SMITH_IMPL_FUNC(bmm_out_cuda)(const Tensor& batch1, const Tensor& batch2, const Tensor &result) {
  Scalar beta(0.0);
  Scalar alpha(1.0);
  {
    NoNamesGuard guard;
...
SMITH_IMPL_FUNC(addmv_out_cuda)(const Tensor &self, const Tensor &mat, const Tensor &vec, const Scalar& beta_, const Scalar& alpha_, const Tensor& result) {
...
Tensor& _int_mm_out_cuda(const Tensor& self, const Tensor& mat2, Tensor& result) {
  ...
  SMITH_CHECK(self.dim() == 2, "Expected self to be of dimension 2 but got ", self.dim());
  SMITH_CHECK(mat2.dim() == 2, "Expected mat2 to be of dimension 2 but got ", mat2.dim());
  SMITH_CHECK(self.size(0) > 16, "self.size(0) needs to be greater than 16, but got ", self.size(0));
  SMITH_CHECK(self.size(1) > 0 && self.size(1) % 8 == 0, "self.size(1) needs to be greater than 0 and a multiple of 8, but got ", self.size(1));
  SMITH_CHECK(self.size(1) == mat2.size(0), "self.size(1) needs to match mat2.size(0) but got ", self.size(1), " and ", mat2.size(0));
  SMITH_CHECK(mat2.size(1) > 0 && mat2.size(1) % 8 == 0, "mat2.size(1) needs to be greater than 0 and a multiple of 8, but got ", mat2.size(1));

  SMITH_CHECK(result.dtype() == at::kInt, "Expected result dtype to be of type kInt but got ", result.dtype());
  SMITH_CHECK(result.size(0) == self.size(0), "Expected result.size(0) to be ", self.size(0), " but got ", result.size(0));
  SMITH_CHECK(result.size(1) == mat2.size(1), "Expected result.size(1) to be ", mat2.size(1), " but got ", result.size(1));

  SMITH_CHECK(result.dim() == 2, "Expected result to be of dimension 2 but got ", result.dim());

  SMITH_CHECK(result.is_contiguous(), "Expected result to be contiguous.");
...
static void baddbmm_bmm_out_dtype_checks(const Tensor& batch1, const Tensor& batch2, const Scalar& beta, const Scalar& alpha, const at::ScalarType out_dtype, const std::optional<Tensor>& self_baddbmm = std::nullopt) {
  ...
  SMITH_CHECK(batch1.scalar_type() == batch2.scalar_type(), "batch1 and batch2 must have the same dtype");

  SMITH_CHECK(out_dtype == batch1.scalar_type() ||
  ...
  if (self_baddbmm.has_value()) {
    ...
    SMITH_CHECK(self.sizes() == output_size, "self must have the same shape as the output");
  }
}
...
Tensor _baddbmm_dtype_cuda(const Tensor& self, const Tensor& batch1, const Tensor& batch2, const at::ScalarType out_dtype, const Scalar& beta, const Scalar& alpha) {
  SMITH_CHECK(self.scalar_type() == out_dtype || self.scalar_type() == batch1.dtype(),
...
Tensor& _mm_dtype_out_cuda(const Tensor& self, const Tensor& mat2, const at::ScalarType out_dtype, Tensor &out) {
  SMITH_CHECK(self.dim() == 2,  "self must be a matrix, got ", self.dim(), "-D tensor");
  ...
  SMITH_CHECK(out_dtype == out.scalar_type(), "out_dtype must be the same as the dtype of the provided out tensor");
  SMITH_CHECK(self.scalar_type() == mat2.scalar_type(), "input dtypes must be the same");
  SMITH_CHECK(out_dtype == self.scalar_type() ||
...
// repeat dimensionality checks for direct calls to `out` overload
  ...
  SMITH_CHECK(out_dtype == mat1.scalar_type() ||
  (out_dtype == at::ScalarType::Float && (mat1.scalar_type() == at::ScalarType::Half || mat1.scalar_type() == at::ScalarType::BFloat16)),
  "out_dtype must be the same as input dtype or fp32 for fp16/bf16 inputs");

  SMITH_CHECK(out_dtype == out.scalar_type(), "out_dtype must be the same as the dtype of the provided out tensor");
  SMITH_CHECK(out_dtype == self.scalar_type() || self.scalar_type() == mat1.scalar_type(),
