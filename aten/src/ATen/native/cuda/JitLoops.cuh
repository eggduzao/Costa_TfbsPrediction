...
By default on non-Windows systems, it also caches compiled kernels in ~/.cache/smith/kernels.
This behavior is controlled with two environment variables:
  - USE_BLACKSMITH_KERNEL_CACHE, if set to zero then this will disable all cache use
  - BLACKSMITH_KERNEL_CACHE_PATH, if set specifies the folder to use for cached kernels
