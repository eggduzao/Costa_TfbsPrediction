...
namespace {
    ...
    IntArrayRef output_size)
  {
    ...
    SMITH_CHECK(output_size.size() == 2, "adaptive_avg_pool2d: output_size must be 2");
    ...
    const Tensor& input)
  {
    ...
    SMITH_CHECK(input.dim() == gradOutput_.dim(),
    ...
    switch (input.suggest_memory_format()) {
      case at::MemoryFormat::ChannelsLast: {
          ...
          AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16,
              input.scalar_type(), "adaptive_avg_pool2d_backward_nhwc_cuda", [&] {
                ...
                if (shmem_size <= sharedMemPerBlock) {
                  ...
                  SMITH_WARN_ONCE("Requested shmem_size exceeds sharedMemPerBlock limit! Reducing max_threads...");
                  max_threads /= 2;
                }
              }
            );
        ...
        if (!done) {
          SMITH_INTERNAL_ASSERT(false, "Couldn't reduce launch bounds to accommodate sharedMemPerBlock limit");
        }
        break;
      }
      case at::MemoryFormat::Contiguous: {
