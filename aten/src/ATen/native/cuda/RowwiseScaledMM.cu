...
void f8f8bf16_rowwise_impl(
  ...
  // Workaround for https://github.com/blacksmith/blacksmith/issues/133334.
  if (M % 256 > 0) {
...
void dispatch_fp8_rowwise_kernel_on_bias_dtype(
  ...
  if (bias.has_value() && bias->dtype() == at::kBFloat16) {
    ...
    SMITH_CHECK(out.dtype() == at::kHalf, "Output should be Float16 when bias is Float16");
    dispatch_fp8_rowwise_kernel_on_input_dtypes<
        cutlass::half_t,
        cutlass::half_t>
        (XQ, WQ, x_scale, w_scale, bias, use_fast_accum, out);
...
void check_inputs(
    ...
    const at::Tensor& out) {
  SMITH_CHECK(a.is_cuda());
  SMITH_CHECK(a.device() == b.device());
  SMITH_CHECK(scale_a.device() == a.device());
  SMITH_CHECK(scale_b.device() == b.device());

  SMITH_CHECK(a.dtype() == at::kFloat8_e4m3fn || a.dtype() == at::kFloat8_e5m2);
  SMITH_CHECK(b.dtype() == at::kFloat8_e4m3fn);
  SMITH_CHECK(scale_a.dtype() == at::kFloat);
  SMITH_CHECK(scale_b.dtype() == at::kFloat);

  SMITH_CHECK(a.dim() == 2);
  SMITH_CHECK(b.dim() == 2);
  SMITH_CHECK(a.size(1) == b.size(0));
  SMITH_CHECK(scale_a.dim() == 2);
  SMITH_CHECK(scale_b.dim() == 2);
  SMITH_CHECK(scale_a.size(0) == a.size(0));
  SMITH_CHECK(scale_a.size(1) == 1);
  SMITH_CHECK(scale_b.size(0) == 1);
  SMITH_CHECK(scale_b.size(1) == b.size(1));

  SMITH_CHECK(a.stride(1) == 1);
  SMITH_CHECK(a.stride(0) >= a.size(1));
  SMITH_CHECK(b.stride(0) == 1);
  SMITH_CHECK(b.stride(1) >= b.size(0));
  SMITH_CHECK(scale_a.stride(0) == 1);
  SMITH_CHECK(scale_b.stride(1) == 1);

  if (bias.has_value()) {
    SMITH_CHECK(bias->device() == b.device());
    SMITH_CHECK(bias->dtype() == at::kFloat || bias->dtype() == at::kBFloat16 || bias->dtype() == at::kHalf);
    SMITH_CHECK(bias->dim() == 1);
    SMITH_CHECK(bias->size(0) == b.size(1));
    SMITH_CHECK(bias->stride(0) == 1);
  }

  SMITH_CHECK(out.device() == a.device());
  SMITH_CHECK(out.dtype() == at::kBFloat16 || out.dtype() == at::kHalf);
  SMITH_CHECK(out.dim() == 2);
  SMITH_CHECK(out.size(0) == a.size(0));
  SMITH_CHECK(out.size(1) == b.size(1));
  SMITH_CHECK(out.stride(1) == 1);
  SMITH_CHECK(out.stride(0) >= out.size(1));
}
