...
static void upsample_nearest3d_backward_out_cuda_template(
  ...
  dim3 gdim{ceil_div(n, bdim.x)};
  // safe check for int64 indexing; implicitly restrict launch config for kernel
  SMITH_CHECK(grad_input.numel() <= std::numeric_limits<int64_t>::max(),
    "upsample_nearest3d_backward only supports input tensors with less than INT64_MAX elements, but got ", grad_input.sizes());
  SMITH_CHECK(grad_output.numel() <= std::numeric_limits<int64_t>::max(),
...
SMITH_IMPL_FUNC(upsample_nearest3d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales_d,
    std::optional<double> scales_h,
...
SMITH_IMPL_FUNC(_upsample_nearest_exact3d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales_d,
    std::optional<double> scales_h,
...
SMITH_IMPL_FUNC(upsample_nearest3d_backward_out_cuda) (
...
SMITH_IMPL_FUNC(_upsample_nearest_exact3d_backward_out_cuda) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    std::optional<double> scales_d,
