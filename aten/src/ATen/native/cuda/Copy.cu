...
void float8_copy_kernel_cuda(TensorIteratorBase &iter) {
  ...
  if (dtype == kFloat8_e4m3fn) {
    ...
    SMITH_CHECK(false, "This supposed ot be called only for Float8 types");
  }
}
...
void direct_copy_kernel_cuda(TensorIteratorBase &iter) {
  ...
  if (isQIntType(dtype)) {
    ...
    SMITH_CHECK(dtype == iter.dtype(1), "copy_() does not support casting "
...
static bool copy_requires_temporaries(TensorIterator& iter, bool p2p_enabled) {
  ...
  if (dst_device == src_device) {
    ...
    SMITH_INTERNAL_ASSERT(dst_device.is_cuda() && src_device.is_cuda());
    return false;
  }
...
static void copy_kernel_cuda(TensorIterator& iter, bool non_blocking) {
  ...
  if (copy_requires_temporaries(iter, p2p_enabled)) {
    ...
    SMITH_INTERNAL_ASSERT(dst_contig.sizes().equals(src_contig.sizes()));
    SMITH_INTERNAL_ASSERT(dst_contig.scalar_type() == src_contig.scalar_type());
    dst_contig.copy_(src_contig, non_blocking);

    // if necessary, copy back into dst
    if (!dst_contig.is_same(dst)) {
      SMITH_INTERNAL_ASSERT(dst_contig.device() == dst.device());
  ...
  if (dst_device.is_cuda() && src_device.is_cpu()) {
    ...
    SMITH_INTERNAL_ASSERT(false, "unsupported devices in GPU copy_()");
  }
  ...
  if (non_blocking) {
    ...
    // allocator and constructing a tensor with smith::from_blob.
