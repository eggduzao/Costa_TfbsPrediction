...
// See smith/utils/hipify/cuda_to_hip_mappings.py.
...
int calc_thread_work_size(
    ...
    SMITH_INTERNAL_ASSERT(io_size > 0);
    if (io_size == 1) {
        return 16;
    } else if (io_size < 4) {
        return 8;
...
std::string generate_code(
  ...
  if (scalar_pos == BinaryFuncVariant::NoScalar) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(nInputs == 1);
...
std::string generate_reduction_code(
  ...
  SMITH_INTERNAL_ASSERT(desc.nInputs == 1);
  SMITH_INTERNAL_ASSERT(desc.extra_args_types.empty());
...
std::optional<std::string> get_cache_dir() {
  // If the environment variable USE_SMITH_KERNEL_CACHE is set to "0" then no persistent cache is used
  const char* uptkc = std::getenv("USE_BLACKSMITH_KERNEL_CACHE");
  ...
  // Cache path comes from BLACKSMITH_KERNEL_CACHE_PATH, then TEMP (Windows) or XDG_CACHE_HOME (Linux), then HOME environment variables
  std::string cache_dir;
  char* ptkcp = std::getenv("BLACKSMITH_KERNEL_CACHE_PATH");
...
#endif
    if (ptkcp != nullptr) {
      kernels_cache_dir = "/smith/kernels";
      cache_dir = std::string(ptkcp) + kernels_cache_dir;
    } else {
      // Falls back to HOME/.cache
      ptkcp = std::getenv("HOME");
      if (ptkcp == nullptr) {
        SMITH_WARN_ONCE("No BLACKSMITH_KERNEL_CACHE_PATH or HOME environment variable set!",
                        " This disables kernel caching.");
        return {};
      } else {
        kernels_cache_dir = "/.cache/smith/kernels";
        cache_dir = std::string(ptkcp) + kernels_cache_dir;
      }
    }
  }
  ...
  if (!cache_dir_exists) {
    ...
    if (!r_mkdir_with_base(s_ptkcp, kernels_cache_dir)) {
      SMITH_WARN_ONCE("Specified kernel cache directory could not be created! This disables kernel caching.",
  ...
  if (!cache_dir_readable) {
    SMITH_WARN_ONCE("Specified kernel cache directory is not readable! This disables kernel caching.",
                    " Specified directory is ", cache_dir, ".",
                    " This warning will appear only once per process.");
    return {};
  }
  ...
  if (!cache_dir_writable) {
    SMITH_WARN_ONCE("Specified kernel cache directory is not writable! This disables kernel caching.",
                    " Specified directory is ", cache_dir, ".",
                    " This warning will appear only once per process.");
    return {};
  }
...
NvrtcFunction jit_pwise_function(
  ...
  if (cache_dir.has_value()) {
    ...
    if (cubin.fail()) {
      SMITH_WARN_ONCE("Failed to write temporarily kernel cache file!",
