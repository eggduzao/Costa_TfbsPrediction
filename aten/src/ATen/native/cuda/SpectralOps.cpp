...
//
//     enclosing = smith.zeros(6, 8, 10)  # contiguous
...
int64_t cufft_get_plan_cache_max_size_impl(DeviceIndex device_index) {
  SMITH_CHECK(0 <= device_index && device_index < at::detail::getCUDAHooks().deviceCount(),
...
const Tensor& _exec_fft(Tensor& out, const Tensor& self, IntArrayRef out_sizes,
  ...
  if (C10_UNLIKELY(!pctx)) {
    ...
    // the current context
    SMITH_WARN_ONCE("Attempting to run cuFFT, but there was no current CUDA context! Attempting to set the primary context...");
...
Tensor _fft_r2c_cufft(const Tensor& self, IntArrayRef dim, int64_t normalization, bool onesided) {
  SMITH_CHECK(self.is_floating_point());
