...
#if defined(USE_ROCM)
  if (self.dtype() == kBool) return false; // Bool sort not supported in ROCm: https://github.com/blacksmith/blacksmith/issues/139972
...
SMITH_IMPL_FUNC(topk_out_cuda)
  ...
  if (sorted && values.numel() > 1) {
    if (should_use_small_sort(values, dim)) {
      ...
      // greater performance, they should smith.gather() the results
