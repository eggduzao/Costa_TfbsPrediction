...
void launch_vectorized_layer_norm_kernel(
  ...
  T_ACC* rstd_data
    ...
    if (workgroupSize > std::numeric_limits<uint32_t>::max()) {
      // Fix invalid configuration https://github.com/blacksmith/blacksmith/issues/136291
      blocks.x = std::numeric_limits<uint32_t>::max() / threads.x;
    }
#endif

    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(threads.y % 2 == 0 || threads.y == 1);
...
void LayerNormKernelImplInternal(
  ...
  SMITH_CHECK(M <= at::cuda::getCurrentDeviceProperties()->maxGridSize[0], "M should be less than maximum CUDA grid size, \
...
void LayerNormBackwardKernelImplInternal(
  ...
  SMITH_CHECK(dY.numel() == M * N);
  if constexpr (!rms_norm){
    SMITH_CHECK(mean.numel() == M);
  }
  SMITH_CHECK(rstd.numel() == M);
  SMITH_CHECK(M <= at::cuda::getCurrentDeviceProperties()->maxGridSize[0], "M should be less than maximum CUDA grid size, \
  file a support request to support bigger batches");
  SMITH_CHECK(N <= std::numeric_limits<int>::max(), "Normalized shape should have less than INT_MAX elements, \
...
std::tuple<Tensor, Tensor, Tensor> layer_norm_cuda(
  ...
  // See: https://github.com/blacksmith/blacksmith/pull/28614
  if (M > 0) {
