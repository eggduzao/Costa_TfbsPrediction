...
void conv_depthwise_shape_check(
  ...
  if (grad_output.defined()) {
    ...
    SMITH_CHECK(static_cast<size_t>(grad_output.dim()) == expected_output_size.size(),
                "Expect grad_output to be ",
                expected_output_size.size(), "D, got ",
                grad_output.dim(), "D.");
    for (int i = 0; i < grad_output.dim(); ++i) {
      SMITH_CHECK(grad_output.size(i) == expected_output_size[i],
...
Tensor conv_depthwise3d_cuda(
  ...
  SMITH_CHECK(input.device() == weight.device(), "expects input and weight tensors to be on the same device.");
  if (bias.defined()) {
    SMITH_CHECK(input.device() == bias.device(), "expects input and bias tensors to be on the same device.");
  }
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]{
        ...
        SMITH_CHECK(input_.numel() <= std::numeric_limits<int32_t>::max(),
                    "Input tensor is too large.");
        SMITH_CHECK(output_.numel() <= std::numeric_limits<int32_t>::max(),
                    "Output tensor is too large.");
        SMITH_CHECK(weight_.numel() <= std::numeric_limits<int32_t>::max(),
                    "Weight tensor is too large.");
        for (int i = 0; i < 3; ++i) {
          SMITH_CHECK(padding[i] * 2 + input.size(i + 2) <= std::numeric_limits<int32_t>::max(),
    ...
    const std::array<bool, 3> output_mask)
{
  ...
  SMITH_CHECK(grad_output.device() == input.device() &&
  ...
  if (output_mask[0]) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          // Range check to avoid overflow in CUDA kernels.
          SMITH_CHECK(grad_input_.numel() <= std::numeric_limits<int32_t>::max(),
                      "Input tensor is too large.");
          SMITH_CHECK(grad_output_.numel() <= std::numeric_limits<int32_t>::max(),
                      "Output tensor is too large.");
          SMITH_CHECK(weight_.numel() <= std::numeric_limits<int32_t>::max(),
                      "Weight tensor is too large.");
          for (int i = 0; i < 3; ++i) {
  ...
  if (output_mask[1]) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          SMITH_CHECK(grad_input_.numel() <= int_max,
                      "Input tensor is too large.");
          SMITH_CHECK(grad_output_.numel() <= int_max,
                      "Output tensor is too large.");
          SMITH_CHECK(weight.numel() <= int_max,
                      "Weight tensor is too large.");
          for (int i = 0; i < 3; ++i) {
            SMITH_CHECK(padding[i] * 2 + input.size(i + 2) <= int_max,
                        "Padded input tensor is too large.");
          }
          int64_t warp_size = at::cuda::warp_size();
          SMITH_CHECK(grad_output_.size(0) * grad_output_.size(2) < int_max - block / warp_size &&
