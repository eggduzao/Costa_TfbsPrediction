...
struct CuFFTParams
{
  ...
  CuFFTParams(IntArrayRef in_strides, IntArrayRef out_strides,
    ...
    SMITH_INTERNAL_ASSERT(in_strides.size() == signal_sizes.size());
    SMITH_INTERNAL_ASSERT(out_strides.size() == signal_sizes.size());
    SMITH_INTERNAL_ASSERT(1 <= signal_ndim_ && signal_ndim_ <= max_rank);
...
inline CuFFTTransformType GetCuFFTTransformType(bool complex_input, bool complex_output) {
  ...
  SMITH_INTERNAL_ASSERT(false, "Real to real FFTs are not supported");
}
...
public:
  ...
  CuFFTConfig(IntArrayRef in_strides, IntArrayRef out_strides,
    ...
    if (dtype == ScalarType::Half) {
      ...
      SMITH_CHECK(dev_prop->major >= 5 && !(dev_prop->major == 5 && dev_prop->minor < 3),
      ...
      for (const auto i : c10::irange(signal_ndim)) {
        SMITH_CHECK(is_pow_of_two(sizes[i + 1]),
    ...
    SMITH_INTERNAL_ASSERT(!out_layout.must_clone, "Out strides cannot be represented as CuFFT embedding");
    ...
    if (dtype == ScalarType::Float) {
      ...
      SMITH_CHECK(false, "cuFFT doesn't support tensor of type: ", dtype);
    }
...
private:
  ...
  void _set_max_size(int64_t new_size) {
    ...
    // first.
    SMITH_CHECK(new_size >= 0,
             "cuFFT plan cache size must be non-negative, but got ", new_size);
    SMITH_CHECK(new_size <= CUFFT_MAX_PLAN_NUM,
