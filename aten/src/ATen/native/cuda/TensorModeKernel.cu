...
struct ModeImpl {
  std::tuple<scalar_t, int64_t> operator()(
    ...
    SMITH_INTERNAL_ASSERT(position_iter != iter_end);
...
void calculate_mode(
  ...
  SMITH_INTERNAL_ASSERT(self.is_contiguous());
  ...
  //
  // See [Note: CUDA smith.mode clones self]
...
void handle_fused_mode(
  ...
  SMITH_INTERNAL_ASSERT(num_threads % warp_size == 0 &&
