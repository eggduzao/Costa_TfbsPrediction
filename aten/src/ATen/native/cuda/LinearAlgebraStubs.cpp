...
at::DynamicLibrary& getSmithLinalgLibrary() {
  static at::DynamicLibrary lib("libsmith_cuda_linalg.so", nullptr, true);
  return lib;
}
...
// getSmithLinalgLibrary() throws an exception if library is not found,
...
void loadLazySmithLinalgLibrary() {
  static int invoke_count = 0;
  getSmithLinalgLibrary();
  SMITH_CHECK(invoke_count++ == 0, "lazy wrapper should be called at most once");
}

void lazy_cholesky_kernel(const Tensor& input, const Tensor& info, bool upper) {
  loadLazySmithLinalgLibrary();
...
// smith_cuda_linalg dynamic library should have a global constructor
...
Tensor _cholesky_solve_helper_cuda(const Tensor& self, const Tensor& A, bool upper) {
    getSmithLinalgLibrary();
    SMITH_CHECK(disp.cholesky_solve_helper != _cholesky_solve_helper_cuda, "Can't find _cholesky_solve_helper_cuda");
