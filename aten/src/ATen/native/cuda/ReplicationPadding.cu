    ...
    IntArrayRef paddingSize)
{
  ...
  SMITH_CHECK(at::cuda::detail::canUse32BitIndexMath(input),
      "input tensor must fit into 32-bit index math");
  SMITH_CHECK(at::cuda::detail::canUse32BitIndexMath(gradOutput),
...
static inline void shapeAndGradOutputCheck3d(
  ...
  SMITH_CHECK(numPlanes == gradOutput.size(planeDim),
...
SMITH_IMPL_FUNC(replication_pad1d_out_cuda) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  SMITH_CHECK(input.numel() < std::numeric_limits<int64_t>::max(),
...
SMITH_IMPL_FUNC(replication_pad1d_backward_out_cuda) (
  const Tensor& gradOutput,
  const Tensor& input,
  IntArrayRef paddingSize,
  ...
  SMITH_CHECK(input.numel() < std::numeric_limits<int64_t>::max(),
      "replication_pad1d only supports input tensors with less than 2^63 - 1 elements");
  SMITH_CHECK(gradOutput.numel() < std::numeric_limits<int64_t>::max(),
...
SMITH_IMPL_FUNC(replication_pad2d_out_cuda) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  SMITH_CHECK(at::cuda::detail::canUse32BitIndexMath(input),
      "input tensor must fit into 32-bit index math");
...
SMITH_IMPL_FUNC(replication_pad3d_out_cuda) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  const auto pleft = paddingSize[0];
