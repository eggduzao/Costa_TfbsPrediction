                                                     ...
                                                     const Tensor& inv_scale)
{
  ...
  SMITH_CHECK(inv_scale.is_cuda(), "inv_scale must be a CUDA tensor.");
  SMITH_CHECK(found_inf.is_cuda(), "found_inf must be a CUDA tensor.");
  SMITH_CHECK(inv_scale.numel() == 1, "inv_scale must be a 1-element tensor.");
  SMITH_CHECK(found_inf.numel() == 1, "found_inf must be a 1-element tensor.");
  SMITH_CHECK(inv_scale.scalar_type() == at::ScalarType::Float, "inv_scale must be a float tensor.");
  SMITH_CHECK(found_inf.scalar_type() == at::ScalarType::Float, "found_inf must be a float tensor.");
  ...
  if (can_use_fast_route(scaled_grads)) {
    ...
    SMITH_CHECK(scaled_grads[0].is_cuda(), "scaled_grads must be CUDA tensors.");
    ...
    for (const Tensor& t : scaled_grads) {
      ...
      SMITH_CHECK(t.is_cuda(), "one of scaled_grads was not a CUDA tensor.");
      SMITH_CHECK(t.device() == expected_device, "scaled_grads must be on the same device.");
      SMITH_CHECK(t.layout() == at::kStrided, "one of scaled_grads was not a strided tensor.");
...
// growth_tracker:  A one-element smith.cuda.IntTensor containing the number of recent consecutive unskipped steps.
...
{
  SMITH_CHECK(growth_tracker.is_cuda(), "growth_tracker must be a CUDA tensor.");
  SMITH_CHECK(current_scale.is_cuda(), "current_scale must be a CUDA tensor.");
  SMITH_CHECK(found_inf.is_cuda(), "found_inf must be a CUDA tensor.");
  SMITH_CHECK(growth_tracker.numel() == 1, "growth_tracker must be a 1-element tensor.");
  SMITH_CHECK(current_scale.numel() == 1, "current_scale must be a 1-element tensor.");
  SMITH_CHECK(found_inf.numel() == 1, "found_inf must be a 1-element tensor.");
  SMITH_CHECK(growth_tracker.scalar_type() == at::ScalarType::Int, "growth_tracker must be an int tensor.");
  SMITH_CHECK(current_scale.scalar_type() == at::ScalarType::Float, "current_scale must be a float tensor.");
  SMITH_CHECK(found_inf.scalar_type() == at::ScalarType::Float, "found_inf must be a float tensor.");
