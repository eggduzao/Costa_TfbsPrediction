...
#ifndef USE_ROCM
// To save on binary size of libsmith_cuda.so, we split the vectorized_elementwise_kernel
...
static inline void launch_vectorized_kernel(
  ...
  switch (vec_size) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unexpected vectorization size");
  }
}
...
void gpu_kernel_impl_nocast(TensorIteratorBase& iter, const func_t& f) {
  ...
  SMITH_INTERNAL_ASSERT(iter.ninputs() == traits::arity);
  SMITH_INTERNAL_ASSERT(iter.noutputs() == 1);
  SMITH_INTERNAL_ASSERT(!needs_dynamic_casting<func_t>::check(iter));
