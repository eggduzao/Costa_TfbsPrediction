...
static void upsample_nearest2d_out_cuda_template(
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast && channels >= 4 && \
    ...
    SMITH_CHECK(output.numel() < std::numeric_limits<int64_t>::max(),
...
static void upsample_nearest2d_backward_out_cuda_template(
  ...
  if (memory_format == at::MemoryFormat::ChannelsLast && channels >= 4 && \
    ...
    SMITH_CHECK(grad_input.numel() < std::numeric_limits<int>::max(),
      "upsample_nearest_nhwc only supports grad_input tensors with less than INT_MAX elements, but got ", grad_input.sizes());
    SMITH_CHECK(grad_output.numel() < std::numeric_limits<int>::max(),
    ...
    SMITH_CHECK(grad_input.numel() <= std::numeric_limits<int64_t>::max(), "upsample2d grad_input.numel() <= std::numeric_limits<int64_t>::max(), but got ", grad_input.sizes());
    SMITH_CHECK(grad_output.numel() <= std::numeric_limits<int64_t>::max(), "upsample2d grad_output.numel() <= std::numeric_limits<int64_t>::max(), but got ", grad_output.sizes());
...
SMITH_IMPL_FUNC(upsample_nearest2d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales_h,
    std::optional<double> scales_w,
...
SMITH_IMPL_FUNC(_upsample_nearest_exact2d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales_h,
    std::optional<double> scales_w,
...
SMITH_IMPL_FUNC(upsample_nearest2d_backward_out_cuda) (
...
SMITH_IMPL_FUNC(_upsample_nearest_exact2d_backward_out_cuda) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    std::optional<double> scales_h,
