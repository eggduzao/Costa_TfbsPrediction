...
// TODO: Add compiler warning? Add Blacksmith config flag?
#else
#include <cuda_fp16.h>
...
mixed_dtypes_linear_cutlass(
  ...
  SMITH_CHECK(length_k % 64 == 0 && length_n % 64 == 0,
...
mixed_dtypes_linear_dispatch_bias_activation(
    ...
    if (bias.numel() == 0) {
      ...
      SMITH_CHECK(false, "mixed_dtypes_linear_dispatch_bias_activation: Activation \"",
...
_mixed_dtypes_linear(const Tensor& input, const Tensor& weight,
...
#if defined(USE_ROCM) || defined(_MSC_VER)
  SMITH_CHECK(false, "_mixed_dtypes_linear: not compiled for this platform");
  return Tensor{};
#else
  ...
  SMITH_CHECK(input.dtype() == at::kHalf ||
  ...
  SMITH_CHECK(weight.dtype() == at::kByte,
              ...
              " is not supported");
  SMITH_CHECK(scale.dtype() == input.dtype(),
              "_mixed_dtypes_linear: Expected scale datatype ", input.dtype(),
              " but got", scale.dtype());
  if (bias.numel() != 0) {
    SMITH_CHECK(bias.dtype() == input.dtype(),
  ...
  SMITH_CHECK(weight.size(1) == scale.size(0)  ||
              ...
              weight.size(1));
  if (bias.numel() != 0) {
      SMITH_CHECK(bias.size(0) == scale.size(0),
