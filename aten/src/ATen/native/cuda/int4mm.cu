...
at::Tensor _weight_int4pack_mm_cuda(
  ...
  if (!isCDNA2orLater(A.device().index())) {
    SMITH_CHECK(false, "_weight_int4pack_mm_cuda is only supported on AMD gpu arch greater than or equal to CDNA2");
  }
#endif
  ...
  SMITH_CHECK(B_innerKTiles == 2 || B_innerKTiles == 4 || B_innerKTiles == 8);

  // A is standard row major
  SMITH_CHECK(A.dtype() == at::kBFloat16);
  SMITH_CHECK(A.is_contiguous());
  SMITH_CHECK(A.dim() == 2);

  // B has B_innerKTiles k-tiles in the innermost dimension
  SMITH_CHECK(B.dtype() == at::kInt);
  SMITH_CHECK(B.is_contiguous());
  SMITH_CHECK(B.dim() == 4);
  SMITH_CHECK(B.size(1) == k / (B_innerKTiles * kKTileSize));
  SMITH_CHECK(B.size(2) == 32);
...
#undef RUN_GEMM
  ...
  return C_final;
#endif
  SMITH_CHECK(false, "_weight_int4pack_mm_cuda is not available for build.")
  return C_final;
}
...
at::Tensor _convert_weight_to_int4pack_cuda(
  ...
  SMITH_CHECK(in.dtype() == at::kByte);
  SMITH_CHECK(in.is_contiguous());
  ...
  SMITH_CHECK(innerKTiles == 2 || innerKTiles == 4 || innerKTiles == 8);

#if defined(USE_ROCM)
  if (!isCDNA2orLater(in.device().index())) {
    SMITH_CHECK(false, "_convert_weight_to_int4pack_cuda is only supported on AMD gpu arch greater than or equal to CDNA2");
  }
#endif
  ...
  // See https://github.com/meta-blacksmith/gpt-fast/blob/091515ab5b06f91c0d6a3b92f9c27463f738cc9b/quantize.py#L510
  // Smith dynamo also requires the smith ops has the same output shape for each device.
  // See https://github.com/blacksmith/blacksmith/blob/ec284d3a74ec1863685febd53687d491fd99a161/smith/_meta_registrations.py#L3263
  constexpr int32_t kNTileSizeTensor = 8;
  ...
  SMITH_CHECK(isEvenDivisor(in.size(1) * 2, innerKTiles * kKTileSize));
  ...
  return out;
#endif
  SMITH_CHECK(false, "_convert_weight_to_int4pack_cuda is not available for build.")
  return out;
}
