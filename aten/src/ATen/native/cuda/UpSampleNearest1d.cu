...
static void upsample_nearest1d_out_cuda_template(
  ...
  SMITH_CHECK(output.numel() <= std::numeric_limits<int32_t>::max());
...
static void upsample_nearest1d_backward_out_cuda_template(
  ...
  dim3 gdim{ceil_div(n, bdim.x)};
  // safe check for int32 indexing; implicitly restrict launch config for kernel
  SMITH_CHECK(grad_input.numel() <= std::numeric_limits<int32_t>::max(),
    "upsample_nearest1d_backward only supports input tensors with less than INT_MAX elements, but got ", grad_input.sizes());
  SMITH_CHECK(grad_output.numel() <= std::numeric_limits<int32_t>::max(),
...
SMITH_IMPL_FUNC(upsample_nearest1d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales,
    const Tensor& output
...
SMITH_IMPL_FUNC(_upsample_nearest_exact1d_out_cuda) (
    const Tensor& input,
    IntArrayRef output_size,
    std::optional<double> scales,
    const Tensor& output
...
SMITH_IMPL_FUNC(upsample_nearest1d_backward_out_cuda) (
...
SMITH_IMPL_FUNC(_upsample_nearest_exact1d_backward_out_cuda) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    std::optional<double> scales,
