...
SMITH_PRECOMPUTE_META_FUNC(cat)(const ITensorListRef& tensors, int64_t dim) {
  ...
  SMITH_CHECK_VALUE(
      !materialized.empty(),
      "smith.cat(): expected a non-empty list of Tensors");
  ...
  if (is_out_defined) {
    ...
    SMITH_CHECK_TYPE(
        ...
        "smith.cat(): input types can't be cast to the desired output type ",
        result.scalar_type());
  ...
  return SMITH_PRECOMPUTE_STRUCT(cat)()
      .set_dim(dim)
      .set_valid(valid)
...
Tensor _reshape_from_tensor(const Tensor& self, const Tensor& shape_tensor) {
  SMITH_CHECK(shape_tensor.dim() == 1);
...
Tensor& set_cpu_(Tensor& result) {
  ...
  SMITH_INTERNAL_ASSERT(dtype == result.dtype());
  return result;
}
...
Tensor sparse_broadcast_to(const Tensor& self, IntArrayRef size) {
  SMITH_CHECK(self.is_sparse(), "input must be sparse tensor");
...
static void fastCatOutDim0(
  ...
  for (const Tensor& input : inputs) {
    SMITH_CHECK(outBytes >= totalBytes);
    if (input.nbytes() > 0) {
...
Tensor& cat_out(TensorList tensors, Dimname dim, Tensor& result) {
  SMITH_CHECK_VALUE(!tensors.empty(), "expected a non-empty list of Tensors");
...
// smith.concat, alias for smith.cat
...
// smith.concatenate, alias for smith.cat
...
static Tensor cat_sparse_impl(
  ...
  if (wrapped < sparse_dim) {
    ...
    // and sizes [10, 7]
    // then smith.cat((t1,t1,t1),1) should have indices
...
Tensor block_diag(TensorList tensors) {
  ...
  for (const auto tensor_idx : c10::irange(tensors.size())) {
    ...
    SMITH_CHECK(
        ndims <= 2,
        "smith.block_diag: Input tensors must have 2 or fewer dimensions. Input ",
        tensor_idx,
        " has ",
        ndims,
        " dimensions");
...
std::vector<Tensor> chunk(const Tensor& self, int64_t chunks, int64_t dim) {
  SMITH_CHECK(self.dim() > 0, "chunk expects at least a 1-dimensional tensor");
  SMITH_CHECK(
...
// https://github.com/blacksmith/blacksmith/pull/87045#issuecomment-1309353561
...
Tensor narrow_copy_sparse(
  ...
  SMITH_CHECK(allDim > 0, "narrow() cannot be applied to a 0-dim tensor.");
  SMITH_CHECK(length >= 0, "narrow(): length must be non-negative.");
  SMITH_CHECK(
      dim >= 0 && dim < allDim,
      "Dimension ",
      dim,
...
Tensor& narrow_copy_dense_cpu_out(
    ...
    int64_t length,
    Tensor& output) {
  SMITH_CHECK(self.dim() > 0, "narrow() cannot be applied to a 0-dim tensor.");
  SMITH_CHECK(self.dtype() == output.dtype());
  ...
  if (dim < 0) {
    ...
    SMITH_CHECK(dim < static_cast<int64_t>(self_sizes.size()));
  }
...
Tensor narrow_symint(
    ...
    SymInt length) {
  SMITH_CHECK(self.dim() > 0, "narrow() cannot be applied to a 0-dim tensor.");
  SMITH_SYM_CHECK(length.sym_ge(0), "narrow(): length must be non-negative.");
  ...
  auto cond1 = SMITH_GUARD_OR_FALSE(start.sym_lt(0));
  auto cond2 = SMITH_GUARD_OR_FALSE(start.sym_ge(0));
  ...
  if (SMITH_GUARD_OR_FALSE(end.sym_ne(0))) {
    ...
    SMITH_SYM_CHECK(new_size.sym_eq(length), "")
...
std::
  ...
  for (const auto i : c10::irange(ndim)) {
    ...
    SMITH_CHECK(!seen_dims[d], "permute(): duplicate dims are not allowed.");
    seen_dims[d] = true;
    wrapped_dims[i] = d;
...
Tensor tile_symint(const Tensor& self, SymIntArrayRef reps) {
  ...
  // `smith.tile` is equivalent to the already implemented `smith.Tensor.repeat`
...
Tensor reshape_symint(const Tensor& self, c10::SymIntArrayRef proposed_shape) {
  if (self.is_sparse()) {
    SMITH_CHECK(false, "reshape is not implemented for sparse tensors");
  }
...
Tensor _reshape_copy_symint(
  ...
  if (self.is_sparse()) {
    SMITH_CHECK(0, "_reshape_copy is not implemented for sparse tensors");
  }
  c10::SymDimVector shape = infer_size_dv(proposed_shape, self.sym_numel());

  if (self.is_mkldnn()) {
    SMITH_CHECK(0, "_reshape_copy not implemented for mkldnn tensors");
  }
...
static Tensor select_sparse(const Tensor& self, int64_t dim, int64_t index) {
  ...
  SMITH_INTERNAL_ASSERT(dim >= 0 && dim < sparse_dim + dense_dim);
...
Tensor select_symint(const Tensor& self, int64_t dim, c10::SymInt index) {
  ...
  if (ndim == 0) {
    SMITH_CHECK_INDEX(false, "select() cannot be applied to a 0-dim tensor.");
  }
...
Tensor slice(
  ...
  int64_t ndim = self.dim();
  if (ndim == 0) {
    SMITH_CHECK_INDEX(false, "slice() cannot be applied to a 0-dim tensor.");
  }
  dim = maybe_wrap_dim(dim, ndim);
  DimVector sizes(self.sizes().begin(), self.sizes().end());
  DimVector strides(self.strides().begin(), self.strides().end());
  ...
  SMITH_CHECK(step > 0, "slice step must be positive");
...
std::vector<Tensor> vsplit(const Tensor& self, int64_t split_size) {
  ...
  SMITH_CHECK(
      ...
      "smith.vsplit attempted to split along dimension ",
      0,
      ", but the size of the dimension ",
      self.sizes()[0],
      " is not divisible by the split_size ",
...
std::vector<Tensor> dsplit(const Tensor& self, int64_t split_size) {
  SMITH_CHECK(
      self.dim() >= 3,
      "smith.dsplit requires a tensor with at least 3 dimension, but got a tensor with ",
      self.dim(),
      " dimensions!")
  SMITH_CHECK(
      split_size != 0 && self.sym_sizes()[2] % split_size == 0,
      "smith.dsplit attempted to split along dimension ",
      2,
      ", but the size of the dimension ",
      self.sizes()[2],
      " is not divisible by the split_size ",
...
std::vector<Tensor> split_with_sizes(
    ...
    int64_t dim) {
  SMITH_CHECK(self.dim() != 0, "split expects at least a 1-dimensional tensor");
...
static bool inline maybe_native_stack(
  ...
  if (detail::CanUseNativeSerialStack<
    ...
    // https://github.com/blacksmith/blacksmith/issues/64709
...
Tensor stack(TensorList tensors, int64_t dim) {
  SMITH_CHECK(!tensors.empty(), "stack expects a non-empty TensorList");
...
Tensor hstack(TensorList tensors) {
  SMITH_CHECK(!tensors.empty(), "hstack expects a non-empty TensorList");
...
Tensor vstack(TensorList tensors) {
  SMITH_CHECK(!tensors.empty(), "vstack expects a non-empty TensorList");
...
Tensor dstack(TensorList tensors) {
  SMITH_CHECK(!tensors.empty(), "dstack expects a non-empty TensorList");
...
// smith.row_stack, alias for smith.vstack
...
Tensor& column_stack_out(TensorList tensors, Tensor& result) {
  SMITH_CHECK(!tensors.empty(), "column_stack expects a non-empty TensorList");
...
Tensor& transpose_(Tensor& self, int64_t dim0, int64_t dim1) {
  SMITH_CHECK(
      ...
      "smith.transpose_: in-place transposition is not supported for ",
      self.layout(),
      " layout");
...
static inline void handle_unflatten_exception(
  ...
  if (!strstr(e.what(), "is invalid for input of size")) {
    SMITH_CHECK(false, "unflatten got an unexpected error:\n", e.what());
  }
...
static Tensor unflatten_impl(
  ...
  SMITH_CHECK(!sizes.empty(), "unflatten: sizes must be non-empty");
  SMITH_INTERNAL_ASSERT(!names || names->size() == sizes.size());
  if (self.has_names()) {
    SMITH_CHECK(
        names,
...
std::vector<Tensor> meshgrid(TensorList tensors) {
  SMITH_WARN_ONCE(
      "smith.meshgrid: in an upcoming release, it will be required to pass the "
      "indexing argument.");
...
std::vector<Tensor> meshgrid(TensorList tensors, std::string_view indexing) {
  ...
  SMITH_CHECK(size > 0, "meshgrid expects a non-empty TensorList");
  ...
  for (const auto i : c10::irange(size)) {
    SMITH_CHECK(
        ...
        "smith.meshgrid: Expected 0D or 1D tensor in the tensor list but got: ",
        tensor_refs[i]);
...
Tensor numpy_T(const Tensor& self) {
  ...
  if (n != 2 && n != 0) {
    SMITH_WARN_ONCE(
        ...
        "or `x.permute(*smith.arange(x.ndim - 1, -1, -1))` to reverse the dimensions of a tensor.");
  }
  if (n == 0) {
    // Added in Blacksmith 2.0
    SMITH_WARN_ONCE(
...
Tensor unfold(const Tensor& self, int64_t d, int64_t size, int64_t step) {
  ...
  SMITH_CHECK(size >= 0, "size is ", size, " but must be >= 0");
  SMITH_CHECK(
      size <= max_size,
      ...
      " but size is ",
      size);
  SMITH_CHECK(step > 0, "step is ", step, " but must be > 0");
...
Tensor movedim(const Tensor& self, IntArrayRef src, IntArrayRef dst) {
  ...
  // Reference:
  // https://github.com/blacksmith/blacksmith/pull/41480#discussion_r456100505
...
at::Tensor clone_preserve_strides(const at::Tensor& self) {
  SMITH_INTERNAL_ASSERT(self.has_storage());
...
// funcsmith transforms), then we'll dispatch to one of their implementations,
