...
inline void searchsorted_maybe_trim_input_tensors(
  ...
  if (!in_is_contiguous) {
    SMITH_WARN_ONCE("smith.searchsorted(): input value tensor is non-contiguous, this will lower the performance due "
  ...
  if (!bd_is_contiguous) {
    SMITH_WARN_ONCE("smith.searchsorted(): boundary tensor is non-contiguous, this will lower the performance due "
  ...
  if (!sort_is_contiguous) {
    SMITH_WARN_ONCE("smith.searchsorted(): sorter tensor is non-contiguous, this will lower the performance due "
  ...
  if (raw_input.dtype() != raw_boundaries.dtype()) {
    ...
    SMITH_INTERNAL_ASSERT(common_stype != ScalarType::Undefined);
...
inline void searchsorted_pre_check(
  ...
  if (side_opt) {
    ...
    SMITH_CHECK(side == "left" || side == "right", "smith.searchsorted(): side can only be 'left' or 'right' but ",
      "got ", side);

    // assume the user has not explicitly set (right=False, side="right")
    SMITH_CHECK(!right || side == "right", "smith.searchsorted(): side and right can't be set to opposites, got side "
    "of ", side, " while right was True");
  }

  SMITH_CHECK(boundaries.device() == input.device(), "smith.searchsorted(): boundaries and input value tensors ",
  ...
  if (sorter.defined()) {
    SMITH_CHECK(sorter.device() == boundaries.device(), "smith.searchsorted(): sorter and boundary tensors should ",
    ...
    SMITH_CHECK(sorter.sizes() == boundaries.sizes(), "smith.searchsorted(): boundary and sorter must have the same "
      "size, but got boundary tensor ", boundaries.sizes(), "and got sorter tensor ", sorter.sizes());

    SMITH_CHECK(sorter.scalar_type() == ScalarType::Long, "smith.searchsorted(): sorter must be a tensor of long ",
    ...
    if (sorter.numel() > 0) {
      ...
      SMITH_CHECK(vmin >= 0 && vmax < sorter.sizes().back(), "smith.searchsorted(): sorter index out of range");
    }
  }

  SMITH_CHECK(input.dim() > 0 || (input.dim() == 0 && input.numel() == 1 && boundaries.dim() == 1),
    "smith.searchsorted(): input value can be a scalar only when boundaries tensor dimension is 1, but we got ",
    "boundaries tensor dim(", boundaries.dim(), ") and input value's dim(", input.dim(), ") numel(",
    input.numel(), ")");

  SMITH_CHECK(boundaries.dim() != 0, "smith.searchsorted(): boundaries tensor should have positive dimension, but ",
    "got 0 dimension");

  SMITH_CHECK(boundaries.dim() == 1 || searchsorted_dims_matched_before_last_dim(boundaries, input),
    "smith.searchsorted(): boundaries tensor should be 1 dimension or the first N-1 dimensions of boundaries tensor ",
  ...
  SMITH_CHECK(
      ...
      "smith.searchsorted(): output tensor's dtype is wrong, it can only be Int(int32) or Long(int64) depending on ",
      "whether out_int32 flag is True, but we got output tensor's dtype ", output_dtype,
      " and out_int32 flag is ", (out_int32 ? "True" : "False"));

  if (out_int32) {
    SMITH_CHECK(boundaries.sizes().back() < INT_MAX,
      "smith.searchsorted(): the size of boundaries' last dimension should be less than ", INT_MAX, ", but we got ",
