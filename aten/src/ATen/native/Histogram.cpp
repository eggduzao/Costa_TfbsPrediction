 ...
 *
 * See the docstr for smith.histogramdd in smith/functional.py for further explanation.
 *
 * - smith.histogramdd(input, bins, range=None, weight=None, density=False)
...
void histogramdd_check_inputs(const Tensor& input, const TensorList& bins, const std::optional<Tensor>& weight) {
    SMITH_CHECK(input.dim() >= 2, "smith.histogramdd: input tensor should have at least 2 dimensions, but got ",
                input.dim());

    const int64_t N = input.size(-1);

    SMITH_CHECK(static_cast<int64_t>(bins.size()) == N, "smith.histogramdd: expected ", N, " sequences of bin edges for a ", N,
    ...
    for (const auto dim : c10::irange(N)) {
        ...
        SMITH_CHECK(input_dtype == bins_dtype, "smith.histogramdd: input tensor and bins tensors should",
        ...
        SMITH_CHECK(dim_bins_dim == 1, "smith.histogramdd: bins tensor should have one dimension,",
                " but got ", dim_bins_dim, " dimensions in the bins tensor for dimension ", dim);

        const int64_t numel = dim_bins.numel();
        SMITH_CHECK(numel > 0, "smith.histogramdd: bins tensor should have at least 1 element,",
                " but got ", numel, " elements in the bins tensor for dimension ", dim);
    }

    if (weight.has_value()) {
        SMITH_CHECK(input.dtype() == weight.value().dtype(), "smith.histogramdd: if weight tensor is provided,"
        ...
        SMITH_CHECK(input_sizes == weight_sizes, "smith.histogramdd: if weight tensor is provided it should have"
...
void histogramdd_prepare_out(const Tensor& input, const std::vector<int64_t>& bin_ct,
    ...
    SMITH_INTERNAL_ASSERT((int64_t)bin_ct.size() == N);
    SMITH_INTERNAL_ASSERT((int64_t)bin_edges.size() == N);

    SMITH_CHECK(input.dtype() == hist.dtype(), "smith.histogram: input tensor and hist tensor should",
            " have the same dtype, but got input ", input.dtype(), " and hist ", hist.dtype());

    for (const auto dim : c10::irange(N)) {
        SMITH_CHECK(input.dtype() == bin_edges[dim].dtype(), "smith.histogram: input tensor and bin_edges tensor should",
                " have the same dtype, but got input ", input.dtype(), " and bin_edges ", bin_edges[dim].dtype(),
                " for dimension ", dim);

        SMITH_CHECK(bin_ct[dim] > 0,
                "smith.histogram(): bins must be > 0, but got ", bin_ct[dim], " for dimension ", dim);
...
select_outer_bin_edges(const Tensor& input, std::optional<c10::ArrayRef<double>> range) {
    SMITH_INTERNAL_ASSERT(input.dim() == 2, "expected input to have shape (M, N)");
    const int64_t N = input.size(-1);
    ...
    if (range.has_value()) {
        // range is specified
        SMITH_CHECK((int64_t)range.value().size() == 2 * N, "smith.histogramdd: for a ", N, "-dimensional histogram",
    ...
    for (const auto dim : c10::irange(N)) {
        ...
        SMITH_CHECK(std::isfinite(leftmost_edge) && std::isfinite(rightmost_edge),
                "smith.histogramdd: dimension ", dim, "'s range [",
                leftmost_edge, ", ", rightmost_edge, "] is not finite");

        SMITH_CHECK(leftmost_edge <= rightmost_edge, "smith.histogramdd: min should not exceed max, but got",
...
std::pair<double, double> histc_select_outer_bin_edges(const Tensor& input,
    ...
    SMITH_CHECK(!(std::isinf(leftmost_edge) || std::isinf(rightmost_edge) ||
            std::isnan(leftmost_edge) || std::isnan(rightmost_edge)),
            "smith.histc: range of [", leftmost_edge, ", ", rightmost_edge, "] is not finite");

    SMITH_CHECK(leftmost_edge < rightmost_edge, "smith.histc: max must be larger than min");
...
static std::vector<Tensor> allocate_bin_edges_tensors(const Tensor& self) {
    SMITH_CHECK(self.dim() >= 2, "smith.histogramdd: input tensor should have at least 2 dimensions");
...
/* Narrowed interface for the legacy smith.histc function.
 */
