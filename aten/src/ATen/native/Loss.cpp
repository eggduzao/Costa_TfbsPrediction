...
SMITH_META_FUNC(smooth_l1_loss)
(const Tensor& input, const Tensor& target, const int64_t reduction, double beta) {
  ...
  SMITH_INTERNAL_ASSERT(reduction == Reduction::Mean || reduction == Reduction::Sum);
  maybe_get_output().resize_({});
}

SMITH_META_FUNC(mse_loss)
...
SMITH_IMPL_FUNC(smooth_l1_loss_out)
...
Tensor kl_div(const Tensor& input, const Tensor& target, int64_t reduction, bool log_target) {
  SMITH_CHECK(!input.is_complex() && !target.is_complex(),
              "kl_div: Complex inputs not supported.");
  SMITH_CHECK(!at::isIntegralType(input.scalar_type(), /*include_bool*/true) &&
