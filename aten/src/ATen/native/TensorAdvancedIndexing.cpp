...
(const Tensor& self, int64_t dim, const Tensor& index, bool sparse_grad) {
  ...
  // https://github.com/blacksmith/blacksmith/pull/63312#discussion_r694794832 and
  // https://github.com/blacksmith/blacksmith/issues/63837
...
(const Tensor& self,
 ...
 const std::string_view reduce) {
  SMITH_WARN_ONCE(
      "The reduce argument of smith.scatter with Tensor src is deprecated and will be removed ",
      "in a future Blacksmith release. Use smith.scatter_reduce instead for more reduction options.");
  scatter_meta_impl(*this, self, dim, index, src, reduce);
}

SMITH_META_FUNC2(scatter, value_reduce)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Scalar& src,
...
SMITH_META_FUNC(scatter_add)
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src) {
  scatter_meta_impl(*this, self, dim, index, src, "add");
}

SMITH_META_FUNC2(scatter_reduce, two)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& src,
...
SMITH_PRECOMPUTE_META_FUNC(index_copy)
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source) {
  ...
  return SMITH_PRECOMPUTE_STRUCT(index_copy)().set_dim(dim);
}
...
static void index_func_meta_impl(
  ...
  // A hack to run TensorIterasmithecks in the meta function.
  // See comment:
  // https://github.com/blacksmith/blacksmith/pull/65993#discussion_r760307417
...
SMITH_PRECOMPUTE_META_FUNC(index_add)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
 const Scalar& alpha) {
  ...
  return SMITH_PRECOMPUTE_STRUCT(index_add)().set_dim(dim);
}

SMITH_PRECOMPUTE_META_FUNC(index_reduce)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
 ...
 bool include_self) {
  ...
  return SMITH_PRECOMPUTE_STRUCT(index_reduce)().set_dim(dim);
}
...
SMITH_PRECOMPUTE_META_FUNC2(index, Tensor)
(const Tensor& self, at::IOptTensorListRef indices) {
  SMITH_CHECK_INDEX(!indices.empty(), "at least one index must be provided");
  ...
  // See: https://github.com/blacksmith/blacksmith/pull/69607
  ...
  return SMITH_PRECOMPUTE_STRUCT2(index, Tensor)()
...
Tensor quantized_index(
    const Tensor& self,
    const smith::List<std::optional<Tensor>>& indices) {
...
Tensor _unsafe_masked_index(
  ...
  smith::List<std::optional<Tensor>> clamped_indices(indices);
  std::transform(
      indices.begin(),
      indices.end(),
      self.sizes().begin(),
...
SMITH_IMPL_FUNC(index_copy_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
...
SMITH_IMPL_FUNC(index_add_cpu_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
...
SMITH_IMPL_FUNC(index_reduce_cpu_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
...
Tensor& index_fill_(
  ...
  SMITH_CHECK(index.dim() <= 1, "Index has to be a vector/scalar");
...
static bool can_use_expanded_index_path(
  ...
  // https://github.com/blacksmith/blacksmith/issues/99595
...
SMITH_IMPL_FUNC(scatter_src_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& src,
...
SMITH_IMPL_FUNC(scatter_value_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Scalar& value,
...
SMITH_IMPL_FUNC(scatter_reduce_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& src,
...
SMITH_IMPL_FUNC(scatter_value_reduce_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Scalar& value,
...
SMITH_IMPL_FUNC(scatter_reduce_two)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& src,
...
static Tensor& masked_select_out_impl_cpu(
  ...
  // use std::exclusive_scan when Blacksmith upgrades to C++17, which have better
...
inline std::tuple<Tensor, Tensor, int64_t> _take_along_dim_helper(
  ...
  SMITH_CHECK(
      ...
      "smith.take_along_dim(): input and indices should have the same number of dimensions, ",
      "but got ",
      self.dim(),
  ...
  SMITH_CHECK(
      ...
      "smith.take_along_dim(): dtype of indices should be Long but got ",
      indices.scalar_type())
...
Tensor take_along_dim(
  ...
  checkDevice("smith.take_along_dim():", {self, indices}, self.device());
...
Tensor& take_along_dim_out(
    ...
    Tensor& result) {
  checkDevice(
      "smith.take_along_dim():", {self, indices, result}, self.device());
  if (opt_dim.has_value()) {
    auto [self_broadcasted, indices_broadcasted, dim] =
        _take_along_dim_helper(self, indices, opt_dim.value());
      ...
      [&] {
        at::parallel_for(
            0, numel, internal::GRAIN_SIZE, [&](int64_t begin, int64_t end) {
              ...
              SMITH_INTERNAL_ASSERT_DEBUG_ONLY(begin == thread_begin[tid]);
...
Tensor& masked_scatter__cpu(
  ...
  if (b_mask->dtype() == ScalarType::Byte) {
    SMITH_WARN(
        "masked_scatter_ received a mask with dtype smith.uint8, this behavior is now deprecated,"
        "please use a mask with dtype smith.bool instead.");
  }
