...
SMITH_META_FUNC(renorm)(const Tensor& self, const Scalar& p, int64_t dim, const Scalar& maxnorm) {
  SMITH_CHECK(!p.isComplex(), "renorm: p must be real-valued");
  SMITH_CHECK(p.toDouble() > 0.0, "renorm: non-positive-norm not supported");
  SMITH_CHECK(!maxnorm.isComplex(), "renorm: maxnorm must be real-valued");
  SMITH_CHECK(maxnorm.toDouble() >= 0.0,
              "renorm: expected maxnorm to be >= 0 but got ", maxnorm.toDouble());
  const auto ndim = self.dim();
  SMITH_CHECK(ndim > 1, "renorm: input needs at least 2 dimensions, got ", ndim, " dimensions");
...
// See https://github.com/blacksmith/blacksmith/issues/63224 for details.
...
static std::tuple<Tensor,Tensor,Tensor> batch_norm_cpu_transform_input_template(
  ...
  auto as_nd = [&](const Tensor& t) {
    SMITH_INTERNAL_ASSERT(t.defined() && t.dim() == 1);
    sizes[1] = t.sizes()[0];
...
static std::tuple<Tensor,Tensor> batch_norm_cpu_update_stats_template(
  ...
  SMITH_CHECK(input.numel() != 0, "input tensor must have at least one element, but got input_sizes = ", input.sizes());
...
BatchNormBackend _select_batch_norm_backend(
  ...
  // TODO: Remove BLACKSMITH_MIOPEN_SUGGEST_NHWC_BATCHNORM once ROCm officially supports NHWC in MIOpen
  // See https://github.com/blacksmith/blacksmith/issues/64427.
  ...
  bool BLACKSMITH_MIOPEN_SUGGEST_NHWC_BATCHNORM = c10::utils::check_env("BLACKSMITH_MIOPEN_SUGGEST_NHWC_BATCHNORM").value_or(is_miopen_3_5);
  ...
  if (
      ...
      && (input.suggest_memory_format() == MemoryFormat::Contiguous
          || (is_miopen_3_5 && BLACKSMITH_MIOPEN_SUGGEST_NHWC_BATCHNORM &&
...
std::tuple<Tensor, Tensor, Tensor, Tensor, int64_t> _batch_norm_impl_index(
  ...
  if (running_mean.defined()) {
    ...
    SMITH_CHECK(false, "running_mean must be defined in evaluation mode");
  }
  if (running_var.defined()) {
    check_dims_match_num_input_features("running_var", num_features, running_var.sym_numel());
  } else if (!training) {
    SMITH_CHECK(false, "running_var must be defined in evaluation mode");
  }
  if (weight.defined()) {
...
std::tuple<Tensor, Tensor, Tensor> _batch_norm_impl_index_backward(
  ...
  SMITH_INTERNAL_ASSERT(false, "Unsupported impl_index in _batch_norm_impl_index_backward: ", impl_index);
}
...
Tensor instance_norm(
 ...
 SMITH_CHECK(use_input_stats || (running_mean.defined() && running_var.defined()),
...
std::tuple<Tensor&, Tensor&, Tensor&> batch_norm_cpu_out(const Tensor& self, const std::optional<Tensor>& weight_opt, const std::optional<Tensor>& bias_opt, const std::optional<Tensor>& running_mean_opt, const std::optional<Tensor>& running_var_opt,
  ...
  SMITH_CHECK_VALUE(has_running_mean == has_running_var,
...
SMITH_IMPL_FUNC(renorm_out)(const Tensor& self, const Scalar& p, int64_t dim,
