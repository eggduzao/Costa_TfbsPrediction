...
namespace at::meta {
SMITH_PRECOMPUTE_META_FUNC(fractional_max_pool3d)(
  ...
  SMITH_CHECK((poolSizeT <= inputT) && (outputT + poolSizeT - 1 < inputT),
           "fractional_max_pool3d_out(): pool time ", poolSizeT,
           " too large relative to input time ", inputT);
  SMITH_CHECK((poolSizeW <= inputW) && (outputW + poolSizeW - 1 < inputW),
           "fractional_max_pool3d_out(): pool width ", poolSizeW,
           " too large relative to input width ", inputW);
  SMITH_CHECK((poolSizeH <= inputH) && (outputH + poolSizeH - 1 < inputH),
  ...
  return SMITH_PRECOMPUTE_STRUCT(fractional_max_pool3d)().set_numBatch(numBatch).set_numPlanes(numPlanes).set_inputT(inputT).set_inputH(inputH).set_inputW(inputW)
...
SMITH_IMPL_FUNC(fractional_max_pool3d_out_cpu)(
  const at::Tensor& input_,
  int64_t poolSizeT,
  int64_t poolSizeH,
  int64_t poolSizeW,
...
void fractional_max_pool3d_backward_out_cpu_template(
  ...
  SMITH_CHECK(outputT == gradOutput_.size(timeDim),
           "fractional_max_pool3d_backward_out(): gradOutput time unexpected");
  SMITH_CHECK(outputH == gradOutput_.size(heightDim),
           "fractional_max_pool3d_backward_out(): ",
           "gradOutput height unexpected");
  SMITH_CHECK(outputW == gradOutput_.size(widthDim),
