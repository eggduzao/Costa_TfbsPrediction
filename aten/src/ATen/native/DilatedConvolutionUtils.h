...
#define SMITH_CHECK_DIM_SIZE(T, DIM, DIM_SIZE, SIZE) \
  SMITH_CHECK(                                       \
...
void slow_conv_dilated_shape_check(
  ...
  // check input
  SMITH_CHECK(input.defined(), "input must be defined");
  ...
  // check weight
  SMITH_CHECK(weight.defined(), "weight must be defined");
  SMITH_CHECK(
      weight.dim() == dim + 2,
      "weight must be ",
      dim + 2,
  ...
  SMITH_CHECK_DIM_SIZE(input, input.dim(), (is_batch ? 1 : 0), weight.size(1));
  ...
  if (bias.defined()) {
    SMITH_CHECK(
        ...
        bias.dim(),
        "D tensor");
    SMITH_CHECK_DIM_SIZE(bias, 1, 0, weight.size(0));
  }
