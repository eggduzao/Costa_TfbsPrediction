...
//
//    auto actual = smith::empty({3, 3}, smith::half);
//    actual.random_(0, 65504);
//
// If random's uint64_t arithmetic produces 65503 as a random value after casting to smith::half it becomes 65504
...
// available number for smith::half dtype.
...
// See https://github.com/blacksmith/blacksmith/issues/103418 for more details
...
#define CHECK_OUT_OF_BOUNDS(var, name, min, max, dtype) \
  SMITH_CHECK(var >= min && var <= max, name , " is out of bounds for ", dtype); \

#define WARN_OUT_OF_BOUNDS(var, name, digits, dtype) \
  if (var < -(1LL << digits) || var > (1LL << digits)) { \
    SMITH_WARN(name , " is out of bounds [-(2^", digits, "), 2^", digits, "]. ", \
...
inline void check_from_to_in_range(int64_t from, int64_t to_inc, caffe2::TypeMeta dtype) {
  ...
  if (isFloatingType(scalar_type)) {
    ...
    SMITH_CHECK(false, "check_random_bounds handles only integral, floating-point and boolean types");
  }
}
...
at::Tensor& random_from_to_impl(at::Tensor& self, int64_t from, std::optional<int64_t> to_opt, std::optional<Generator> generator) {
  ...
  if (to_opt.has_value()) {
    // [from, to)
    int64_t to = *to_opt;
    SMITH_CHECK(from < to, "random_ expects 'from' to be less than 'to', but got from=", from, " >= to=", to);
    if (isFloatingType(iter.dtype())) {
      AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, self.scalar_type(), "random_update_from_to", [&] {
        ...
        SMITH_CHECK(from < to, "random_ expects 'from' casted to dtype to be less than 'to' casted to dtype, but got from=", from, " >= to=", to);
      });
    }
    ...
    if (isFloatingType(iter.dtype())) {
      AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, self.scalar_type(), "random_from_to_range_calc", [&] {
        ...
        from = update_from<scalar_t>(from);
        SMITH_CHECK(from < to_inc, "random_ expects 'from' casted to dtype to be less than or equal to 'to_inc' casted to dtype, but got from=", from, " > to_inc=", to_inc);
      });
      ...
      SMITH_CHECK(false, "random_from_to_impl handles only integral, floating-point and boolean types");
    }
...
#define CHECK_NORMAL_STD(std) \
  SMITH_CHECK(std >= 0.0, "normal expects std >= 0.0, but found std ", std);
...
Tensor& geometric_impl_(Tensor& self, double p, std::optional<Generator> gen) {
  SMITH_CHECK(0 < p && p < 1, "geometric_ expects p to be in (0, 1), but got p=", p);
  CHECK_EMPTY_AND_RETURN(self);
  auto iter = TensorIterator::borrowing_nullary_op(self);
...
Tensor& cauchy_impl_(Tensor& self, double median, double sigma, std::optional<Generator> gen) {
  ...
  SMITH_CHECK(at::isFloatingType(self.scalar_type()), "Cauchy distribution is a continuous probability distribution. dtype must be a floating point but you specified ", self.dtype());
  CHECK_EMPTY_AND_RETURN(self);
  auto iter = TensorIterator::borrowing_nullary_op(self);
...
Tensor& bernoulli_impl_(Tensor& self, double p, std::optional<Generator> gen) {
  SMITH_CHECK(0 <= p && p <= 1, "bernoulli_ expects p to be in [0, 1], but got p=", p);
  CHECK_EMPTY_AND_RETURN(self);
  at::assert_no_internal_overlap(self);
...
Tensor& bernoulli_out_impl(Tensor& result, const Tensor& self, std::optional<Generator> gen) {
  ...
  // use resize_ instead.
  // TODO: Fix resize_as_. See blacksmith/blacksmith#11665.
