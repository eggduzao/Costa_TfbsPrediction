...
void cpu_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack, bool error_on_views,
                  c10::DispatchKey cpu_dispatch_key) {
  SMITH_CHECK(c10::BackendComponent::CPUBit == c10::toBackendComponent(cpu_dispatch_key),
    ...
    if (alias_info != nullptr && alias_info->isWrite()) {
      ...
      if (returns[idx].isTensor() && returns[idx].toTensor().defined()) {
        ...
        for (const auto i : c10::irange(tensor_args_indices.size())) {
          ...
          SMITH_INTERNAL_ASSERT_DEBUG_ONLY(alias_info != nullptr);
