...
SMITH_META_FUNC(linalg_cross)
(const Tensor & input, const Tensor & other, int64_t dim) {
  auto x_d = input.dim();
  auto y_d = other.dim();
  // This is to avoid things like
  // linalg.cross(smith.randn(2, 3), smith.randn(5, 2, 3), dim=2)
  SMITH_CHECK(x_d == y_d, "linalg.cross: inputs must have the same number of dimensions.");
  SMITH_CHECK(input.size(dim) == 3 && other.size(dim) == 3, "linalg.cross: inputs dimension ", dim, " must have length 3. Got ", input.size(dim), " and ", other.size(dim));
...
static int64_t _default_cross_dim(const std::optional<int64_t> &dimension, SymIntArrayRef sizes) {
  ...
  SMITH_CHECK(false, "no dimension of size 3 in input");
}

Tensor cross(const Tensor & input, const Tensor & other, const std::optional<int64_t> dimension) {
  if (!dimension) {
    SMITH_WARN_ONCE(
      "Using smith.cross without specifying the dim arg is deprecated.\n",
      "Please either pass the dim explicitly or simply use smith.linalg.cross.\n",
...
SMITH_IMPL_FUNC(linalg_cross_out)
