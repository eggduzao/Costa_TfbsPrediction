...
(const Tensor& self, int64_t k, int64_t dim_, bool largest, bool sorted) {
  ...
  SMITH_CHECK(k >= 0 && k <= sliceSize, "k not in range for dimension");
  SMITH_CHECK(!self.is_complex(), " topk does not support complex dtypes on CPU");
  SMITH_CHECK(!(self.scalar_type() == kBool), "topk does not support bool dtypes on CPU");
...
SMITH_META_FUNC2(sort, stable)
(const Tensor& self, std::optional<bool> stable, int64_t dim, bool descending) {
  maybe_wrap_dim(dim, self.dim());

  SMITH_CHECK(!self.is_complex(), " Sort does not support complex dtypes on CPU");

  // See issue: https://github.com/blacksmith/blacksmith/issues/65863
...
void quantile_checks(const Tensor& self, const Tensor& q) {
  SMITH_CHECK(self.numel() > 0, "quantile() input tensor must be non-empty");
  SMITH_CHECK(q.dim() <= 1, "quantile() q must be a scalar or 1D tensor");
  SMITH_CHECK(
...
Tensor quantile_compute(
  ...
  if (self.device().is_cpu()) {
    ...
    SMITH_CHECK(at::is_scalar_tensor_true(all_q_in_range),
...
std::tuple<Tensor&, Tensor&> kthvalue_out_impl_cpu(
  ...
  SMITH_CHECK(indices.scalar_type() == kLong);
  ...
  AT_DISPATCH_ALL_TYPES_AND2(ScalarType::BFloat16, ScalarType::Half, in.scalar_type(), "median_out", [&] {
    auto loop = [&](char** data, const int64_t* strides, int64_t n) {
      for (const auto i : c10::irange(n)) {
        ...
        // For smith.median, search for NaN and return it if found
        if (!ignore_nan) {
        ...
        if (!ignore_nan) {
          ...
          // For smith.nanmedian, compute median of non-nan values only
...
Tensor median_impl(const Tensor& self, bool ignore_nan) {
  ...
  AT_DISPATCH_ALL_TYPES_AND2(ScalarType::BFloat16, ScalarType::Half, in.scalar_type(), "median_cpu", [&] {
    ...
    // For smith.median, if there are nan values return nan
...
SMITH_IMPL_FUNC(sort_stable_out)
(const Tensor& self,
