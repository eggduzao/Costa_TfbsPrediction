...
namespace {
  bool is_cudnnv8_heuristic_mode_b() {
    static const bool is_cudnnv8_heuristic_mode_b = c10::utils::check_env("SMITH_CUDNN_USE_HEURISTIC_MODE_B") == true;
    return is_cudnnv8_heuristic_mode_b;
  }
}

inline bool cudnnv8_enabled_check_debug() {
  static bool cudnnv8_flag = c10::utils::check_env("SMITH_CUDNN_V8_API_DISABLED") != true;
  static bool cudnnv8_debug = c10::utils::check_env("SMITH_CUDNN_V8_API_DEBUG") == true;
  static uint8_t cudnnv8_debugcount = 0;
  if (cudnnv8_debug == 1 && cudnnv8_debugcount < 10) {
    SMITH_WARN("SMITH_CUDNN_V8_DEBUG ON, V8 ON: ", cudnnv8_flag, " SMITH_CUDNN_USE_HEURISTIC_MODE B: ", is_cudnnv8_heuristic_mode_b());
    cudnnv8_debugcount++;
  }
...
SMITH_API ConvBackend select_conv_backend(
...
SMITH_API at::MemoryFormat _determine_backend_memory_format(const Tensor& input,
    const Tensor& weight,
...
{
  SMITH_CHECK(args.size() <= expected_size,
    ...
    IntArrayRef padding, IntArrayRef stride, IntArrayRef dilation, int64_t groups)
{
  ...
  for (auto s : stride) {
      SMITH_CHECK(s > 0, "Stride must be greater than 0 but got ", s);
  }
  for (auto d : dilation) {
      SMITH_CHECK(d > 0, "Dilation must be greater than 0 but got ", d);
  }
  for (auto p : padding) {
      SMITH_CHECK(p >= 0, "Padding must be non-negative but got ", p);
  }
...
SMITH_API void _cudnn_set_conv_benchmark_empty_cache(bool enable);
SMITH_API bool _cudnn_get_conv_benchmark_empty_cache();
...
inline at::MemoryFormat miopen_conv_suggest_memory_format(const at::Tensor& input, const at::Tensor& weight) {
  ...
  bool suggest_nhwc = c10::utils::check_env("BLACKSMITH_MIOPEN_SUGGEST_NHWC").value_or(false);
