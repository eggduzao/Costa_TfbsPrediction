...
SMITH_API Tensor flatten_indices(
    const Tensor& indices,
...
SMITH_API Tensor flatten_indices_by_dims(
    const Tensor& indices,
    const IntArrayRef& sizes,
    const IntArrayRef& dims_to_flatten);

// Find the CSR representation for a row `indices` from the COO format
SMITH_API Tensor coo_to_csr(const int64_t* indices, int64_t dim, int64_t nnz);

SMITH_API Tensor zeros_like_with_indices(const Tensor& t);
...
// smith.ones(shape).nonzero().transpose(-2, -1) but much faster.
SMITH_API Tensor full_coo_indices(IntArrayRef sizes, TensorOptions options);
