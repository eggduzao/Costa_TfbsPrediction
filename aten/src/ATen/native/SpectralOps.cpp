...
ScalarType promote_type_fft(ScalarType type, bool require_complex, Device device) {
  ...
  if (maybe_support_half) {
    SMITH_CHECK(type == kHalf || type == kFloat || type == kDouble, "Unsupported dtype ", type);
  } else {
    SMITH_CHECK(type == kFloat || type == kDouble, "Unsupported dtype ", type);
  }
  ...
  default: SMITH_INTERNAL_ASSERT(false, "Unhandled dtype");
  }
}
...
fft_norm_mode norm_from_string(std::optional<std::string_view> norm, bool forward) {
  ...
  SMITH_CHECK(false, "Invalid normalization mode: \"", *norm, "\"")
}

// Fixes the shape of x such that x.size(dims[i]) == sizes[i],
// either by zero-padding, or by slicing x starting from 0.
Tensor resize_fft_input(Tensor x, IntArrayRef dims, SymIntArrayRef sizes) {
  SMITH_INTERNAL_ASSERT(dims.size() == sizes.size());
...
Tensor fft_r2c_maybe_out(
  ...
  if (out.defined()) {
    SMITH_CHECK(out.is_complex(), fname,
...
Tensor fft_c2r_maybe_out(
  ...
  if (out.defined()) {
    SMITH_CHECK(out.is_floating_point(), fname,
...
Tensor fft_c2r(std::string_view function_name,
               ...
               bool forward) {
  SMITH_CHECK(!out.defined() || out.is_floating_point(), function_name,
              " expects a floating point output tensor, but got ", out.scalar_type());
  ...
  SMITH_CHECK(n >= 1, "Invalid number of data points (", n, ") specified");
  if (n_opt) {
...
Tensor fft_r2c(std::string_view function_name,
  ...
  SMITH_CHECK(!input.is_complex(), function_name,
              " expects a real input tensor, but got ", input.scalar_type());
  SMITH_CHECK(!out.defined() || out.is_complex(), function_name,
              " expects a complex output tensor, but got ", out.scalar_type());
...
ShapeAndDims canonicalize_fft_shape_and_dim_args(
  ...
  if (dim) {
    ...
    SMITH_CHECK(duplicate == copy.end(), "FFT dims must be unique");
  }
  ...
  if (shape) {
    ...
    SMITH_CHECK(static_cast<int64_t>(shape->size()) <= input_dim,
...
Tensor fftn_c2c(
  ...
  SMITH_CHECK(input.is_complex(), function_name, " expects a complex input tensor, but got", input.scalar_type());
...
// smith.fft.fft, analogous to NumPy's numpy.fft.fft
...
static Tensor fft_rfftn_impl(Tensor out, const Tensor& self,
  ...
  SMITH_CHECK(!self.is_complex(), "rfftn expects a real-valued input tensor, but got ", self.scalar_type());
  auto desc = canonicalize_fft_shape_and_dim_args(self, s, dim);
  SMITH_CHECK(!desc.shape.empty(), "rfftn must transform at least one axis");
...
static ShapeAndDims canonicalize_fft_c2r_shape_and_dim_args(
  ...
  SMITH_CHECK(!desc.shape.empty(), fname, " must transform at least one axis");
  ...
  SMITH_CHECK(last_dim_size >= 1, "Invalid number of data points (", last_dim_size, ") specified");
...
static Tensor fft_ihfftn_impl(
  ...
  auto desc = canonicalize_fft_shape_and_dim_args(self, s, dim);
  SMITH_CHECK(!desc.shape.empty(), "ihfftn must transform at least one axis");
...
Tensor& fft_fftfreq_out(int64_t n, double d, Tensor& out) {
  ...
  SMITH_CHECK(at::isFloatingType(dtype) || at::isComplexType(dtype),
...
Tensor stft(const Tensor& self, const int64_t n_fft, const std::optional<int64_t> hop_lengthOpt,
  ...
  Tensor out;
  if (complex_fft) {
    SMITH_CHECK(!onesided, "Cannot have onesided output if window or input is complex");
...
Tensor istft(const Tensor& self, const int64_t n_fft, const std::optional<int64_t> hop_lengthOpt,
  ...
  if (win_length != n_fft) {
    ...
    SMITH_INTERNAL_ASSERT(window_tmp.size(0) == n_fft);
  }
  ...
  if (return_complex) {
    ...
    SMITH_CHECK(!window.defined() || !window.is_complex(),
  ...
  SMITH_INTERNAL_ASSERT(input.size(2) == n_fft);
  ...
  SMITH_INTERNAL_ASSERT(expected_output_signal_len == y.size(1));
  SMITH_INTERNAL_ASSERT(expected_output_signal_len == window_envelop.size(1));
...
void _fft_fill_with_conjugate_symmetry_(const Tensor& input, IntArrayRef dim_) {
  ...
  SMITH_INTERNAL_ASSERT(mirror_dims.size() == dim.size() - 1);
