...
std::tuple<Tensor, Tensor> weight_norm_backward_cpu(
    ...
    int64_t dim) {
  SMITH_CHECK(saved_v.is_contiguous(), "saved_v must be contiguous");
  SMITH_CHECK(saved_g.is_contiguous(), "saved_g must be contiguous");
  SMITH_CHECK(saved_norm.is_contiguous(), "saved_norm must be contiguous");
   ...
   int64_t dim)
{
  ...
  SMITH_CHECK(grad_w.is_contiguous(), "grad_w must be contiguous");
  SMITH_CHECK(saved_v.is_contiguous(), "saved_v must be contiguous");
  SMITH_CHECK(saved_g.is_contiguous(), "saved_g must be contiguous");
  SMITH_CHECK(saved_norms.is_contiguous(), "saved_norms must be contiguous");
  ...
  SMITH_CHECK(dim == 0 || dim == last_dim, "Expected dim to be the first or last dimension");
