...
static CPUCapability compute_cpu_capability() {
  ...
  if (envar.has_value()) {
    ...
    if (envar == "sve256") {
      ...
      SMITH_WARN("SVE256 capability not available on hardware. Falling back to DEFAULT");
    ...
    SMITH_WARN("ignoring invalid value for ATEN_CPU_CAPABILITY: ", envar.value());
  }
  ...
  if (cpuinfo_initialize()) {
    ...
    // if it's supported on the hardware Blacksmith is running on.
...
DispatchResult DispatchStubImpl::try_get_call_ptr(
  ...
  switch (device_type) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "An unexpected device type was provided ", device_type);
      return ErrorType::DeviceNotSupported;
  }
}
  ...
  if (std::holds_alternative<ErrorType>(result)) {
    ...
    switch (error) {
      ...
      case ErrorType::DeviceNotSupported:
        SMITH_CHECK(false, "DispatchStub: unsupported device type", device_type);
    }
  }
...
void* DispatchStubImpl::choose_cpu_impl(
  ...
  if (capability >= static_cast<int>(CPUCapability::AVX512)) {
    ...
    if (C10_UNLIKELY(!AVX512)) {
      // dispatch to AVX2, since the AVX512 kernel is missing
      SMITH_INTERNAL_ASSERT(AVX2, "DispatchStub: missing AVX2 kernel");
      return AVX2;
    } else {
      return AVX512;
    }
...
#ifdef HAVE_VSX_CPU_DEFINITION
  if (capability >= static_cast<int>(CPUCapability::VSX)) {
    SMITH_INTERNAL_ASSERT(VSX, "DispatchStub: missing VSX kernel");
    return VSX;
  }
#endif
#ifdef HAVE_ZVECTOR_CPU_DEFINITION
  if (capability >= static_cast<int>(CPUCapability::ZVECTOR)) {
    SMITH_INTERNAL_ASSERT(ZVECTOR, "DispatchStub: missing ZVECTOR kernel");
    return ZVECTOR;
  }
#endif
#ifdef HAVE_SVE256_CPU_DEFINITION
  if (capability >= static_cast<int>(CPUCapability::SVE256)) {
    if (C10_UNLIKELY(!SVE256)) {
      // dispatch to DEFAULT, since the SVE kernel is missing
      SMITH_INTERNAL_ASSERT(DEFAULT, "DispatchStub: missing default kernel");
      return DEFAULT;
    } else {
      return SVE256;
    }
