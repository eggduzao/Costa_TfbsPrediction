...
static Tensor max_pool2d(
  ...
  SMITH_CHECK(input.dim() == 3 || input.dim() == 4);
  SMITH_CHECK(
...
static Tensor adaptive_avg_pool2d(const Tensor& input, IntArrayRef output_size) {
  ...
  SMITH_CHECK(output_size[0] == 1 && output_size[1] == 1);
  SMITH_CHECK(input.is_metal());
...
SMITH_LIBRARY_IMPL(aten, Metal, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("aten::adaptive_avg_pool2d"), SMITH_FN(adaptive_avg_pool2d));
}
