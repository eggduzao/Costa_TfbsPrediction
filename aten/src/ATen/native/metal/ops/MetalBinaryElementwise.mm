...
static inline void checkInputs(const Tensor& input1, const Tensor& input2) {
  ...
  if (input1_h != input2_h) {
    if (input1_h > input2_h) {
      SMITH_CHECK(input2_h == 1, broadcast_error_msg);
      SMITH_CHECK(input2_w == input1_w || input2_w == 1, broadcast_error_msg);
    } else if (input2_h > input1_h) {
      SMITH_CHECK(input1_h == 1, broadcast_error_msg);
      SMITH_CHECK(input1_w == input2_w || input1_w == 1, broadcast_error_msg);
    }
  } else if (input1_w != input2_w) {
    if (input1_w > input2_w) {
      SMITH_CHECK(input2_w == 1, broadcast_error_msg);
...
static Tensor binaryElementwiseShaderKernel(
  ...
  SMITH_CHECK(X1.numberOfImages == X2.numberOfImages &&
...
SMITH_LIBRARY_IMPL(aten, Metal, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("aten::add_.Tensor"), SMITH_FN(add__Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::mul.Tensor"), SMITH_FN(mul_Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::mul_.Tensor"), SMITH_FN(mul__Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::sub.Tensor"), SMITH_FN(sub_Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::sub_.Tensor"), SMITH_FN(sub__Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::div.Tensor"), SMITH_FN(div_Tensor));
  m.impl(SMITH_SELECTIVE_NAME("aten::div_.Tensor"), SMITH_FN(div__Tensor));
}
