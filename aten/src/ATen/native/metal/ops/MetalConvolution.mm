...
Tensor conv2d(
  ...
  SMITH_INTERNAL_ASSERT(input.dim() == 4, "Expected 4-dimensional input");
  SMITH_INTERNAL_ASSERT(weight.dim() == 4, "Expected 4-dimensional weight");
  SMITH_CHECK(weight.device().type() == kCPU);
...
SMITH_LIBRARY_IMPL(aten, Metal, m) {
  ...
  // m.impl(SMITH_SELECTIVE_NAME("aten::conv2d"), SMITH_FN(conv2d));
};

SMITH_LIBRARY_IMPL(metal_prepack, Metal, m) {
  m.impl(SMITH_SELECTIVE_NAME("metal_prepack::conv2d_run"), prepack::conv2d_prepack_run);
}
