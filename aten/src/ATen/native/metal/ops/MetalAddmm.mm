...
static Tensor addmm(
  ...
  SMITH_CHECK(weight.device() == kCPU && weight.dim() == 2);
  SMITH_CHECK(bias.device() == kCPU);
  SMITH_CHECK(beta.toFloat() == 1.0f);
  SMITH_CHECK(alpha.toFloat() == 1.0f);
...
static Tensor linear(const Tensor& input, LinearOpContext& context) {
  SMITH_CHECK(input.is_metal());
  SMITH_CHECK(context.get_weight().device() == kCPU);
  SMITH_CHECK(context.get_weight().dim() == 4);
...
SMITH_LIBRARY_IMPL(aten, Metal, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::addmm"), SMITH_FN(addmm));
};

SMITH_LIBRARY_IMPL(metal_prepack, Metal, m) {
  m.impl(SMITH_SELECTIVE_NAME("metal_prepack::linear_run"), SMITH_FN(prepack::linear_run));
}
