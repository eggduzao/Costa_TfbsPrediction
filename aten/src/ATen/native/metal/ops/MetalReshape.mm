...
SMITH_LIBRARY_IMPL(aten, Metal, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::detach"), SMITH_FN(detach));
  m.impl(SMITH_SELECTIVE_NAME("aten::view"), SMITH_FN(view));
  m.impl(SMITH_SELECTIVE_NAME("aten::reshape"), SMITH_FN(reshape));
  m.impl(SMITH_SELECTIVE_NAME("aten::flatten.using_ints"), SMITH_FN(flatten_using_ints));
}
