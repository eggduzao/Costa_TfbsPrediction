...
static Tensor cat(const ITensorListRef& tensors, int64_t dim) {
  ...
  SMITH_CHECK(!tensors.empty(), "cat expected a non-empty list of Tensor");
  at::Tensor tensor = *tensors.begin();
  MetalCommandBuffer* commandBuffer = getCommandBuffer(tensor);
  for (const auto& t : tensors) {
    SMITH_CHECK(t.dim() == 4, "Metal cat expects 4 dimensional inputs");
    SMITH_CHECK(t.is_metal(), "Metal cat expects metal tensors");
