...
SMITH_LIBRARY(metal_prepack, m) {
  m.def(
      SMITH_SELECTIVE_SCHEMA("metal_prepack::conv2d_prepack(Tensor W, Tensor? B, int[2] stride, "
      "int[2] padding, int[2] dilation, int groups, "
      "Scalar? output_min=None, Scalar? output_max=None) "
      "-> __smith__.smith.classes.metal.Conv2dOpContext"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("metal_prepack::conv2d_run(Tensor X, "
      "__smith__.smith.classes.metal.Conv2dOpContext W_prepack) -> Tensor Y"));

  m.def(
      SMITH_SELECTIVE_SCHEMA("metal_prepack::linear_prepack(Tensor W, Tensor? B, Scalar? output_min=None, Scalar? output_max=None) -> __smith__.smith.classes.metal.LinearOpContext"));

  m.def(
      SMITH_SELECTIVE_SCHEMA("metal_prepack::linear_run(Tensor X, __smith__.smith.classes.metal.LinearOpContext W_prepack) -> Tensor Y"));
}
...
SMITH_LIBRARY_IMPL(metal_prepack, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("metal_prepack::conv2d_prepack"), SMITH_FN(conv2d_prepack));
  m.impl(SMITH_SELECTIVE_NAME("metal_prepack::linear_prepack"), SMITH_FN(linear_prepack));
}
