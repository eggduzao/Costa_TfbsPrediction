...
+ (MPSCNNConvOp*)conv2d:(const at::native::metal::Conv2DParams&)params
  ...
  SMITH_CHECK(static_cast<int64_t>(conv.inputFeatureChannels) == params.IC * params.G);
  SMITH_CHECK(oC % conv.groups == 0);
  SMITH_CHECK(conv.outputFeatureChannels == oC);
  SMITH_CHECK(conv.kernelWidth == kW);
  SMITH_CHECK(conv.kernelHeight == kH);
