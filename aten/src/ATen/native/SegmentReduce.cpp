...
Tensor _segment_reduce_lengths_cpu_kernel(
  ...
  SMITH_CHECK(data.is_contiguous(), "Expected data to be contiguous.");
  SMITH_CHECK(lengths.is_contiguous(), "Expected lengths to be contiguous.");
...
Tensor _segment_reduce_offsets_cpu_kernel(
  ...
  // data and lengths should be contiguous from the call to .contiguous in segment_reduce_kernel
  SMITH_CHECK(data.is_contiguous(), "Expected data to be contiguous.");
  SMITH_CHECK(offsets.is_contiguous(), "Expected offsets to be contiguous.");
  // reduction axis should always be the last dimension of lengths
  axis = offsets.dim() - 1;
...
Tensor segment_reduce_kernel(
  ...
  if (offsets_has_value) {
    ...
    SMITH_CHECK(data.get_device() == offsets_value.get_device());
    SMITH_CHECK(data.dim() >= offsets_value.dim());
    SMITH_CHECK(axis == offsets_value.dim() - 1,
    ...
    SMITH_CHECK(data.get_device() == lengths_value.get_device());
    SMITH_CHECK(data.dim() >= lengths_value.dim());
    SMITH_CHECK(axis == lengths_value.dim() - 1,
    ...
    if (!unsafe) {
      ...
      SMITH_CHECK((min_length >= 0), "lengths contains negative value!");
      SMITH_CHECK(all(lengths_value.sum({-1}) == data.size(axis)).item<bool>(),
