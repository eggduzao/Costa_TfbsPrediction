...
Tensor constant_pad_nd(const Tensor& self, IntArrayRef pad, const Scalar& value) {
    SMITH_CHECK(pad.size() % 2 == 0, "Length of pad must be even but instead it equals ",
             pad.size());
    ...
    SMITH_CHECK(l_inp >= (int64_t)l_pad, "Length of pad should be no more than twice the number of "
    ...
    for (const auto i : c10::irange(l_pad)) {
        ...
        SMITH_CHECK(new_dim >= 0, "The input size ", input_sizes[l_diff + i], ", plus negative padding ",
    ...
    if (self.is_quantized()) {
        ...
        SMITH_CHECK(qscheme == kPerTensorAffine || qscheme == kPerTensorSymmetric,
...
Tensor _pad_circular_symint(const Tensor &self, c10::SymIntArrayRef padding) {
  ...
  SMITH_CHECK(ndim_nonpadded == 1 || ndim_nonpadded == 2,
...
static std::string_view padding_mode_string(padding_mode m) {
  ...
  SMITH_CHECK(false, "Invalid padding mode (", static_cast<int64_t>(m), ")");
}
...
Tensor _pad_enum_symint(const Tensor &self, c10::SymIntArrayRef pad, int64_t mode_int, std::optional<double> value) {
  ...
  SMITH_CHECK(pad.size() % 2 == 0, "Padding length must be divisible by 2");
  SMITH_CHECK(static_cast<int64_t>(pad.size()) <= input_dim * 2,
  ...
  SMITH_CHECK(!value.has_value() || *value == 0,
