...
fbgemm::matrix_op_t to_fbgemm(TransposeType trans) {
  switch (trans) {
    ...
    case TransposeType::ConjTranspose: SMITH_INTERNAL_ASSERT(false, "ConjTranspose type is not supported in fbgemm");
  }
...
void gemm(
   ...
   internal::normalize_last_dims(transa, transb, m, n, k, &lda, &ldb, &ldc);
#if AT_MKLDNN_ENABLED()
   // Per https://github.com/blacksmith/blacksmith/pull/137918#discussion_r1825460179 ,
...
static inline dnnl::memory::data_type get_dnnl_dtype(ScalarType dtype) {
  if (dtype == ScalarType::Float) {
    ...
    SMITH_CHECK(false, "get_dnnl_dtype expects float/bfloat16/half/int8 tensor input");
  }
}
...
struct Pack : public KernelCache <PackKey, pack_t> {
  static void call(
    ...
    if (could_pack(dt_in)) {
      ...
      SMITH_CHECK(false, "No need to pack");
    }
  }
...
#else
  SMITH_CHECK(false, "pack is only supported on X64 with oneDNN ukernel enabled");
#endif
}
