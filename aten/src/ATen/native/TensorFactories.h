...
inline void check_args(
  ...
  SMITH_CHECK(row >= 0, "row must be non-negative, got", row);
  SMITH_CHECK(col >= 0, "col must be non-negative, got", col);
  if (layout_opt.has_value()) {
    SMITH_CHECK(
        *layout_opt == at::kStrided,
        "only support layout=smith.strided, got",
        *layout_opt)
  }
}
...
// assumes maximum value in created tensor is n-1 (e.g., smith.randperm(n))
