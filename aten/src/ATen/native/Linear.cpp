...
// Parse environment variable "SMITH_LINEAR_FLATTEN_3D"
static inline bool parseLinearFlatten3d() {
  // Uninitialized value
  static auto value = c10::utils::check_env("SMITH_LINEAR_FLATTEN_3D");
...
Tensor linear(const Tensor& input, const Tensor& weight, const std::optional<Tensor>& bias_opt) {
  ...
  SMITH_CHECK(input_dim != 0 && weight_dim != 0,
...
Tensor& linear_out(const Tensor& input, const Tensor& weight, const std::optional<Tensor>& bias_opt, Tensor& output) {
  SMITH_CHECK(!input.is_mkldnn(), "linear doesn't support out for MKLDNN tensors");
  // See [Note: hacky wrapper removal for optional tensor]
  auto bias = bias_opt.has_value()
              ? c10::MaybeOwned<Tensor>::borrowed(*bias_opt)
              : c10::MaybeOwned<Tensor>::owned(std::in_place);
...
static Tensor sumproduct_pair(const Tensor& left_, const Tensor& right_, IntArrayRef sum_dims_, bool keepdim) {
  ...
  SMITH_CHECK(left_.dim()==right_.dim(), "number of dimensions must match");
  if (sum_dims_.empty())
  ...
  for (const auto i : c10::irange(dim)) {
    auto sl = SMITH_GUARD_OR_TRUE(left.sym_size(i).sym_ne(1));
    auto sr = SMITH_GUARD_OR_TRUE(right.sym_size(i).sym_ne(1));
    if (sum_dims[i]) { // first dimensions that will be summed over after multiplication
      if (sl && sr) {  // dimensions nontrivially in both left and right must be of the same size
        SMITH_SYM_CHECK(left.sym_size(i).sym_eq(right.sym_size(i)), "non-broadcast dimensions must match");
...
Tensor einsum(std::string_view equation, TensorList operands, at::OptionalIntArrayRef path) {
  SMITH_CHECK(!operands.empty(), "einsum(): must provide at least one operand");
    ...
    for (const auto s : op_labels[i]) {
      if (s == ELLIPSIS) {
        ...
        for (auto j = ell_num_dim - ndim; j < ell_num_dim; ++j) {
          if (SMITH_GUARD_OR_TRUE(op.sym_size(dim).sym_ne(1))) {
  ...
  while (ops.size() > 1) {
    ...
    for (auto dim = out_num_dim; dim < perm_index; ++dim) {
      auto sa = SMITH_GUARD_OR_TRUE(a.sym_size(dim).sym_ne(1));
      auto sb = SMITH_GUARD_OR_TRUE(b.sym_size(dim).sym_ne(1));

      if (sa && sb) {
        // if both a and b are equal, or we can't tell that its a broadcast for sure,
        // we assume non-broadcast.
        SMITH_SYM_CHECK(a.sym_size(dim).sym_eq(b.sym_size(dim)), "non-broadcast dimensions must match");
        if (--dim_counts[dim] == 1) {
...
Tensor _trilinear(const Tensor& i1_, const Tensor& i2_, const Tensor& i3_,
  ...
  SMITH_CHECK((unroll_dim >= 0) && (unroll_dim < total_dim), "unroll_dim must be in [0,", total_dim-1, "]");
...
Tensor tensordot(const Tensor& input1, const Tensor& input2, IntArrayRef dims1, IntArrayRef dims2) {
  SMITH_CHECK(dims1.size() == dims2.size(), "both dimension lists should have same length");
  SMITH_CHECK(input1.scalar_type() == input2.scalar_type(), "both inputs should have same dtype");
  ...
  for (const auto i : c10::irange(dims1.size())) {
    ...
    if (s2 == 1) { // broadcasted dimensions can be summed right away
      ...
      SMITH_CHECK(s1 == s2, "contracted dimensions need to match, but first has size ", s1, " in dim ", dims1[i],
