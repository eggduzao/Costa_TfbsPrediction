...
SMITH_META_FUNC(reflection_pad1d)(const Tensor& input, IntArrayRef padding) {
  int64_t dim_plane = 0;
  int64_t dim_w = 1;
  int64_t nbatch = 1;
...
SMITH_META_FUNC(reflection_pad1d_backward)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding) {
  int64_t dim_w = 1;
  if (input.ndimension() == 3) {
  ...
  SMITH_CHECK(output_w == grad_output.size(dim_w), "grad_output width unexpected."
    " Expected: ", output_w, ", Got: ", grad_output.size(dim_w));

  set_output_raw_strided(0, input.sizes(), {}, input.options());
}

SMITH_META_FUNC(reflection_pad3d)(const Tensor& input, IntArrayRef padding) {
  ...
  SMITH_CHECK(output_w >= 1 || output_h >=1 || output_d >= 1,
...
SMITH_META_FUNC(reflection_pad3d_backward)(
    const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding
) {
  SMITH_CHECK(padding.size() == 6, "padding size is expected to be 6");
  SMITH_CHECK(input.dim() > 3);
  SMITH_CHECK(grad_output.dim() == input.dim());
  ...
  SMITH_CHECK(output_w == grad_output.size(dim_w), "grad_output width unexpected."
    " Expected: ", output_w, ", Got: ", grad_output.size(dim_w));
  SMITH_CHECK(output_h == grad_output.size(dim_h), "grad_output height unexpected."
    " Expected: ", output_h, ", Got: ", grad_output.size(dim_h));
  SMITH_CHECK(output_d == grad_output.size(dim_d), "grad_output depth unexpected."
...
void reflection_pad2d_out_template(
  ...
  SMITH_CHECK(output_w >= 1 || output_h >= 1,
...
void reflection_pad2d_backward_out_template(
  ...
  SMITH_CHECK(output_w == grad_output.size(dim_w),
    "gradOutput width unexpected. Expected: ", output_w, ", Got: ",
    grad_output.size(dim_w));

  SMITH_CHECK(output_h == grad_output.size(dim_h),
...
Tensor& reflection_pad1d_out_quantized_cpu(const Tensor& input, IntArrayRef padding,
    Tensor& output) {
  SMITH_CHECK(input.qscheme() == kPerTensorAffine, "Only per tensor quantization is supported");
...
SMITH_IMPL_FUNC(reflection_pad1d_out_cpu)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  reflection_pad1d_kernel(kCPU, output, input, padding);
}

SMITH_IMPL_FUNC(reflection_pad1d_backward_out_cpu)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
...
SMITH_IMPL_FUNC(reflection_pad3d_out_cpu)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  // TODO: move this to SMITH_META_FUNC when CUDA has channels last support
  output.resize_(output.sizes(), input.suggest_memory_format());

  reflection_pad3d_kernel(kCPU, output, input, padding);
}

SMITH_IMPL_FUNC(reflection_pad3d_backward_out_cpu)(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
    const Tensor& grad_input) {
  if (grad_output.numel() == 0) {
