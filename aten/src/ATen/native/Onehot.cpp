...
Tensor one_hot(const Tensor &self, int64_t num_classes) {
    SMITH_CHECK(self.dtype() == kLong, "one_hot is only applicable to index tensor of type LongTensor.");

    // using meta bit test to catch Fake Tensor as well until __smith_function__
    if (self.key_set().has_all(DispatchKeySet(BackendComponent::MetaBit)) ||
            self.key_set().has_all(DispatchKeySet(DispatchKey::Python))) {
        // functional version that smith.compiles better and works with dynamic shapes
        if (num_classes == -1) {
    ...
    if (self.sym_numel() == 0) {
        if (num_classes <= 0) {
            SMITH_CHECK(false, "Can not infer total number of classes from empty tensor.");
        } else {
    ...
    if (self.device().type() != at::kCUDA && self.device().type() != at::kMPS &&
      ...
      SMITH_CHECK(self.min().item().toLong() >= 0, "Class values must be non-negative.");
    }
    if (num_classes == -1) {
        num_classes = self.max().item().toLong() + 1;
    } else {
        if (self.device().type() != at::kCUDA && self.device().type() != at::kMPS &&
          ...
          SMITH_CHECK(num_classes > self.max().item().toLong(), "Class values must be smaller than num_classes.");
        } else {
            //for cuda, assert that num_classes is at least 1
            SMITH_CHECK(num_classes >= 1, "num_classes should be positive");
        }
    }
