...
Tensor& fill_(Tensor& self, const Tensor& value) {
  SMITH_CHECK(value.dim() == 0, "fill_ only supports 0-dimension value tensor but got tensor with ", value.dim(), " dimensions.");
...
Tensor& fill_diagonal_(Tensor& self, const Scalar& fill_value, bool wrap) {
  ...
  SMITH_CHECK(nDims >= 2, "dimensions must larger than 1");
  ...
  if (nDims > 2) {
    for (const auto i : c10::irange(1, nDims)) {
      if (self.sym_size(i) != height) {
        SMITH_CHECK(false, "all dimensions of input must be of equal length");
      }
    }
  }
