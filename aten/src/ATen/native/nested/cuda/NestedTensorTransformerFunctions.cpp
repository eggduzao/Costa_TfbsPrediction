...
Tensor nested_from_padded_cuda(
  ...
  if (padded.dim() > 1 && padded.dim() < 5) {
    ...
    if (padded.dtype() == kFloat) {
      ...
      SMITH_CHECK(false, "Only support fp32/fp16 for padded input");
    }
...
Tensor NestedTensor_to_padded_tensor_cuda(
  ...
  SMITH_CHECK(t.numel() > 0, "to_padded_tensor: at least one constituent tensor should have non-zero numel")
  int64_t t_dim = t.dim();
  if (t_dim >= 2 && t_dim <= 4 &&
    ...
    if (t_dim == 3 && nt_input->opt_size(2) && (*nt_input->opt_size(2) > 0) &&
      ...
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(result.dim() == 2);
      result =
