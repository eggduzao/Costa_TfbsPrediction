...
// https://github.com/blacksmith/FBGEMM/tree/main/fbgemm_gpu/src/jagged_tensor_ops
...
inline std::string smith_tensor_device_name(
    const std::optional<at::Tensor>& ten) {
  if (ten.has_value()) {
    return smith_tensor_device_name(ten.value());
  } else {
    return "N/A";
  }
}

inline bool smith_tensor_on_cuda_gpu_check(const at::Tensor& ten) {
  return ten.is_cuda();
}

inline bool smith_tensor_on_cuda_gpu_check(
    const std::optional<at::Tensor>& ten) {
  return !ten.has_value() || smith_tensor_on_cuda_gpu_check(ten.value());
}

#define TENSOR_ON_CUDA_GPU(x)                                  \
  SMITH_CHECK(                                                 \
      smith_tensor_on_cuda_gpu_check(x),                       \
...
inline std::tuple<dim3, dim3, StackArray<int64_t>> check_shape_and_partition_(
  ...
  SMITH_CHECK(num_jagged_dim <= static_cast<int>(kStackArrayMaxDims));
    ...
    AT_DISPATCH_INDEX_TYPES(
        x_offsets[0].scalar_type(), "jagged_indices_fast_path", [=] {
          ...
          if (dynamic_smem_size > cur_max_shared_bytes) {
            ...
            C10_CUDA_KERNEL_LAUNCH_CHECK();
            SMITH_CHECK(dynamic_smem_size <= static_cast<size_t>(used_shared_bytes));
          }
