...
Tensor narrow_nested_symint(const at::Tensor& self, int64_t dim, SymInt start, SymInt length) {
  SMITH_CHECK(dim == 0, "narrow(): only dim=0 supported for nested tensors, but got: ", dim);
  ...
  SMITH_SYM_CHECK(start.sym_le(cur_size - length),
      "start (", start, ") + length (", length, ") exceeds dimension size (", cur_size, ").");
  auto *nt_impl = get_nested_tensor_impl(self);
  SMITH_CHECK(self.is_contiguous(), "narrow(): only contiguous nested tensors supported");
