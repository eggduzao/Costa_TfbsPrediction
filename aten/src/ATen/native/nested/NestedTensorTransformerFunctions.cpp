...
inline void check_nested_tensor_matrix_constraints(
  ...
  SMITH_INTERNAL_ASSERT(nt_input != nullptr);
  SMITH_CHECK(
...
Tensor NestedTensor_times_Tensor_plus_Tensor_addmm(
  ...
  SMITH_INTERNAL_ASSERT(nt_mat1 != nullptr);
  SMITH_INTERNAL_ASSERT(!mat2.is_nested());
  SMITH_INTERNAL_ASSERT(!self.is_nested());
  SMITH_INTERNAL_ASSERT(nested_tensor_impl_is_contiguous(nt_mat1));
  SMITH_INTERNAL_ASSERT(mat1.dim() == 3 && mat2.dim() == 2);
...
Tensor NestedTensor_add_NestedTensor_in_place(
    const Tensor& self,
    const Tensor& other) {
  SMITH_INTERNAL_ASSERT(self.is_nested() && other.is_nested());
  ...
  SMITH_CHECK(at::equal(self_sizes, other_sizes));
  SMITH_INTERNAL_ASSERT(
...
Tensor NestedTensor_softmax_dropout(const Tensor& self, const Tensor& query) {
  ...
  SMITH_INTERNAL_ASSERT(query_nt != nullptr);
  SMITH_INTERNAL_ASSERT(nested_tensor_impl_is_contiguous(query_nt));
  ...
  SMITH_INTERNAL_ASSERT(output.is_contiguous());
...
Tensor NestedTensor_to_mask(const Tensor& nt, std::optional<int64_t> mask_dim, std::optional<int64_t> mask_dim_length) {
  ...
  SMITH_CHECK(nested_tensor_impl_is_contiguous(nt_impl), "to_mask only works on contiguous NestedTensors.");
  SMITH_CHECK(
  ...
  // blacksmith/nestedtensor's version.
