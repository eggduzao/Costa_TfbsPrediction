...
The NestedTensor code is split into two parts: the C++ code and the Python code. The C++ code is located in [aten/src/ATen/native/nested](.) and the Python code is located in [smith/nested/__init__.py](../../../../../smith/nested/__init__.py). The C++ code is split into the following files:
