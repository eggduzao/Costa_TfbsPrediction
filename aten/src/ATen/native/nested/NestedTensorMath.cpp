...
int64_t num_bytes(IntArrayRef sizes) {
  // 0-dim Tensors have smith.Size of .size() 0, but carry 1 memory.
  // Empty 1-dim Tensors (smith.tensor([])) have smith.Size of .size() 1,
  // but carry 0 memory.
  int64_t result = 1;
  int64_t stride = 1;
...
Tensor NestedTensor_nested_tensor_from_mask(const Tensor& t, const Tensor& mask, bool mask_check) {
    SMITH_CHECK(mask.scalar_type() == at::ScalarType::Bool, "Expected mask to be of ScalarType Bool, but got ", mask.scalar_type(), " instead.");
    SMITH_CHECK(mask.dim() == 2, "Padding mask should be 2D");
    SMITH_CHECK(t.dim() == 3, "Input should be a 3D tensor, N * L * D");
    auto N = t.size(0), L = t.size(1), D = t.size(2);
    auto NN = mask.size(0), LL = mask.size(1);
    SMITH_CHECK(N == NN && L == LL, "Mask size should match input size");
    ...
    if (mask_check)
      SMITH_CHECK(sizes.equal(nums), "Mask must be left-aligned without gaps");
...
std::tuple<Tensor, Tensor, Tensor> nested_layer_norm(
    ...
    double eps) {
  SMITH_CHECK_VALUE(weight_opt && bias_opt, "NestedTensor layer_norm requires weight and bias");
  const auto& weight = *weight_opt;
  const auto& bias = *bias_opt;
  SMITH_CHECK(!weight.is_nested(), "NestedTensor weight not supported for layer_norm");
  SMITH_CHECK(!bias.is_nested(), "NestedTensor bias not supported for layer_norm");
  auto* nt_input = get_nested_tensor_impl(input);
  SMITH_CHECK(nested_tensor_impl_is_contiguous(nt_input));
...
Tensor nested_from_padded_generic(
  ...
  SMITH_CHECK(dim - 1 == target_size.size(), "dim: ", dim, "target_size: ", target_size.size());
...
Tensor NestedTensor_to_padded_tensor_generic(
  ...
  if (sizes.numel() == 0 || sizes.dim() == 0) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(nt.get_buffer().numel() == 0);
  ...
  // blacksmith/nestedtensor's masking.cpp.
...
Tensor NestedTensor_embedding(
  ...
  SMITH_CHECK(indices.dim() < 3);
  SMITH_CHECK(indices.dim() > 0, "NestedTensor embedding doesn't support empty indices.")
  SMITH_CHECK(weight.dim() == 2);
  SMITH_CHECK(nested_tensor_impl_is_contiguous(nt_indices));
  SMITH_CHECK(weight.is_contiguous());
...
// Very rudimentary sum_dim for prototyping with smith_scatter.segment_reduce.
Tensor NestedTensor_sum_dim_CPU(
  ...
  SMITH_CHECK(keepdim, "NestedTensor always requires keepdim=True for now.");
  // acc_dtype is not supported for now
  SMITH_CHECK(!dtype, "NestedTensor does not support dtype argument for now.");
...
Tensor select_nested(const Tensor& self, int64_t dim, int64_t index) {
  ...
  SMITH_CHECK_INDEX(ntensors > 0, "You can only select when the NT is not empty.");
...
Tensor transpose_nested(const Tensor& self, int64_t dim0, int64_t dim1) {
  ...
  SMITH_CHECK(positive_dim0 > 0 && positive_dim1 > 0, "Nested tensor dimension 0 cannot be transposed");
...
Tensor squeeze_dim_nested(const Tensor& self, IntArrayRef dims) {
  ...
  // if tensor.size(dim) != 1 smith.squeeze will return the result, we do the same here
  ...
  SMITH_CHECK(ndim > static_cast<int64_t>(1 + dims.size()),
  ...
  for (int64_t itensor = 0; itensor < ntensors; itensor++) {
    ...
    if (ndims_underlying < ndims_underlying_reshaped) {
      ...
      for (int64_t idim = 0; idim < ndims_underlying; idim++) {
        ...
        SMITH_CHECK(size_reshaped >= -1, "invalid shape dimension ", size_reshaped);
        if (size_reshaped == -1) {
          SMITH_CHECK(infer_index_old == -1, "only one dimension can be inferred");
      ...
      for (int64_t idim = ndims_underlying; idim < ndims_underlying_reshaped; idim++) {
        ...
        if (size_reshaped >= 0) {
          ...
          SMITH_CHECK(infer_index <= -1, "only one dimension can be inferred");
          infer_index = idim;
        }
        else {
          SMITH_CHECK(false, "invalid shape dimension ", size_reshaped);
        }
      }
      // See Note [Special size rule for nested tensor]
      SMITH_CHECK(infer_index == -1, "nested tensor does not infer shape");
      SMITH_CHECK(
...
Tensor values_nested(const Tensor& self) {
  SMITH_INTERNAL_ASSERT(self.is_nested(), "Can only create a buffer from Nested Tensor");
...
Tensor _nested_view_from_buffer(
  ...
  SMITH_INTERNAL_ASSERT(buffer.dim() == 1, "The input buffer must be flat");
  SMITH_INTERNAL_ASSERT(nested_sizes.dim() == 2, "Expected the nested size tensor to be two dimensional.");
  SMITH_INTERNAL_ASSERT(nested_strides.dim() == 2, "Expected the nested stride tensor to be two dimensional.");
  SMITH_INTERNAL_ASSERT(nested_sizes.size(0) == nested_strides.size(0), "Expected the first dimension of nested size and nested stride tensor to be equal.");
  SMITH_INTERNAL_ASSERT(nested_strides.size(0) == storage_offsets.size(0), "Expected the first dimension of nested stride tensor to equal the length of offsets.");
...
static Tensor cat_nested_as_jagged(
  ...
  for (auto i : c10::irange(tensors.size())) {
    ...
    for (auto d : c10::irange(first_item_dim - 2)) {
      SMITH_CHECK(nt_impl->opt_size(d + 2).has_value(),
