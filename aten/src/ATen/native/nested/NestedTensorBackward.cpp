...
std::tuple<Tensor, Tensor, Tensor> nested_linear_backward(
  ...
  SMITH_INTERNAL_ASSERT(nt_grad_output != nullptr);
  SMITH_INTERNAL_ASSERT(nt_input != nullptr);
  SMITH_INTERNAL_ASSERT(nested_tensor_impl_is_contiguous(nt_grad_output));
...
Tensor nested_softmax_backward(
  ...
  SMITH_INTERNAL_ASSERT(grad.is_nested(), "Should be nested grad")
  SMITH_INTERNAL_ASSERT(output.is_nested(), "Should be nested output")
  ...
  SMITH_INTERNAL_ASSERT(output_sizemat.equal(grad_sizemat));
  Tensor grad_output =
...
std::tuple<Tensor, Tensor, Tensor> layer_norm_backward_nested(
  ...
  SMITH_CHECK_VALUE(weight_opt.has_value() && bias_opt.has_value(), "NestedTensor layer_norm requires weight and bias");
