...
static Tensor NestedTensor_elementwise_Tensor(
  ...
  if (self.is_nested() && !other.is_nested() && self.is_cuda() && other.is_cuda()) {
    ...
    if (is_broadcastable_3d) {
      ...
      if (op_name == "add") {
        ...
        SMITH_CHECK(false, "Unsupported nested dense elementwise op: ", op_name, ".");
      }
      return result;
    }
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(self_impl);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(other_impl);
  return wrap_buffer(
...
Tensor eq_tensor_nested(const Tensor& self, const Tensor& other) {
  SMITH_CHECK(!other.is_nested(), "eq does not support nested tensor as other value.");
  return NestedTensor_elementwise_Tensor(
