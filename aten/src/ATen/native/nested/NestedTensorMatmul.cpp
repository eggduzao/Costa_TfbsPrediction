...
Tensor bmm_nested(const Tensor& self, const Tensor& mat2) {
  SMITH_CHECK(self.dim() == 3, "batch1 must be a 3D tensor");
  SMITH_CHECK(mat2.dim() == 3, "batch2 must be a 3D tensor");
  ...
  SMITH_CHECK(ntensors == ntensors2,
  ...
  for (int64_t i = 0; i < ntensors; i++) {
    ...
    SMITH_CHECK(self_size1 == mat2_size0,
