...
std::vector<int64_t> NestedTensor_get_max_size_from_size_tensor(
  ...
  SMITH_INTERNAL_ASSERT(sizes_size_1 > 0);
...
std::vector<Tensor> chunk_nested_tensor(const Tensor& self, int64_t chunks, int64_t dim) {
  ...
  if (ndim == 0) {
    SMITH_CHECK_INDEX(false, "chunk() cannot be applied to a 0-dim tensor.");
  }
  dim = maybe_wrap_dim(dim, ndim);
  SMITH_CHECK(self.dim() - 1 == dim,
           "Chunk for nested tensors is currently only supported for the last dimension.");
  SMITH_CHECK(chunks > 0,"chunk expects `chunks` to be greater than 0, got: ", chunks);
  SMITH_CHECK(self.is_contiguous(), "chunk expects `self` to be contiguous.");
  auto self_impl = get_nested_tensor_impl(self);
  const int64_t last_dim_size = get_consistent_last_dim_of_nested_tensor(*self_impl);
    SMITH_CHECK(last_dim_size % chunks == 0,
...
std::vector<Tensor> split_with_sizes_nested(
  ...
  SMITH_CHECK(total_size == self_size,
