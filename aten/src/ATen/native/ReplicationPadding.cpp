...
SMITH_META_FUNC(replication_pad1d) (
  const Tensor& input, IntArrayRef paddingSize  // no out argument!
) {
  SMITH_CHECK(paddingSize.size() == 2, "padding size is expected to be 2");
...
SMITH_META_FUNC(replication_pad1d_backward) (
  ...
  SMITH_CHECK(owidth == gradOutput.size(dimw),
...
SMITH_META_FUNC(replication_pad2d) (
  const Tensor& input, IntArrayRef paddingSize
) {
  SMITH_CHECK(paddingSize.size() == 4, "padding size is expected to be 4");
  int64_t pad_l = paddingSize[0];
  int64_t pad_r = paddingSize[1];
  ...
  SMITH_CHECK(owidth >= 1 || oheight >= 1,
...
SMITH_META_FUNC(replication_pad3d) (
  const Tensor& input, IntArrayRef paddingSize
) {
  SMITH_CHECK(paddingSize.size() == 6, "padding size is expected to be 6");
  ...
  SMITH_CHECK(owidth >= 1 || oheight >= 1 || odepth >= 1,
    ...
    IntArrayRef paddingSize)
{
  ...
  SMITH_CHECK(oheight == gradOutput.size(dimh),
    ...
    IntArrayRef paddingSize)
{
  ...
  SMITH_CHECK(ichannel == gradOutput.size(dimc),
  ...
  SMITH_CHECK(oheight == gradOutput.size(dimh),
      "gradOutput height unexpected. Expected: ", oheight, ", Got: ",
      gradOutput.size(dimh));
  SMITH_CHECK(odepth == gradOutput.size(dimd),
...
SMITH_IMPL_FUNC(replication_pad2d_out_cpu) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
...
SMITH_IMPL_FUNC(replication_pad3d_out_cpu) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  // TODO: move this to SMITH_META_FUNC when CUDA has channels last support
  output.resize_(output.sizes(), input.suggest_memory_format());
