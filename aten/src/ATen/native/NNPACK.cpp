...
at::Tensor _nnpack_spatial_convolution(
  ...
  SMITH_CHECK(false, "nnpack_spatial_convolution: ATen not compiled with NNPACK support");
}
...
struct Workspace {
  ...
  void allocate() {
    ...
    if (res != 0) {
      SMITH_CHECK(false, "posix_memalign failed:", c10::utils::str_error(errno), " (", errno, ")");
    }
    return;
  }
...
Tensor _nnpack_spatial_convolution(
  ...
  if (input.device().type() != kCPU || input.scalar_type() != kFloat ||
    ...
    SMITH_CHECK(false, "Mismatched Tensor types in NNPack convolutionOutput");
  }
