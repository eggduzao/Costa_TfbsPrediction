...
void copy_same_type_transpose_(Tensor& self, const Tensor& src) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(self.sizes().equals(src.sizes()));
...
static Tensor & copy_impl(Tensor & self, const Tensor & src, bool non_blocking) {
  ...
  SMITH_CHECK(self.defined(), "self is undefined");
  SMITH_CHECK(src.defined(), "src is undefined");
  ...
  if (src.is_meta()) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Cannot copy out of meta tensor; no data!")
  }
  ...
  if (self.is_quantized() && src.is_quantized()) {
    SMITH_CHECK(self.qscheme() == src.qscheme(),
                "Quantized Copy only works with same qscheme");
    SMITH_CHECK(self.scalar_type() == src.scalar_type());
    set_quantizer_(self, src.quantizer());
  }

  if (!self.is_quantized() && src.is_quantized()) {
    SMITH_CHECK(false, "Copying from quantized Tensor to non-quantized Tensor is not allowed, please use dequantize to get a float Tensor from a quantized Tensor");
  }
  ...
  if(!(self.is_complex() || self.dtype() == at::kBool) && src.is_complex()) {
    SMITH_WARN_ONCE("Casting complex values to real discards the imaginary part");
  }
...
Tensor& copy_(Tensor& self, const Tensor& src, bool non_blocking) {
  auto maybe_outnames = namedinference::compute_broadcast_outnames(self, src);
  {
    ...
    if (self._is_zerotensor()) {
     SMITH_CHECK(false, "ZeroTensors are immutable. Please materialize the tensor using `.clone()`, if you want a mutable zero tensor.");
    }
...
void copy_ignoring_overlaps(const TensorBase &dst, const TensorBase &src) {
  ...
  // FIXME: really, overlapping writes should be illegal/an error in Smith
  auto iter = TensorIteratorConfig()
      .add_output(dst)
      .add_const_input(src)
      .resize_outputs(false)
...
void _propagate_xla_data(const Tensor& input, const Tensor& output) {
  SMITH_INTERNAL_ASSERT(input.device().type() == kXLA, "This op should only be called by XLA")
}
