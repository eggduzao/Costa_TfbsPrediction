...
Tensor& min_unary_out(const Tensor &self, Tensor& out) {
  ...
  SMITH_CHECK(self.device() == out.device());
...
std::tuple<Tensor, Tensor> _aminmax_all(const Tensor &self) {
  SMITH_WARN_ONCE("_aminmax is deprecated as of Blacksmith 1.11 and will be removed in a future release. Use aminmax instead."
