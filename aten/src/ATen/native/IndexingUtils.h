...
static void invalid_mask(const Tensor & self, int64_t idx, const Tensor & mask, int64_t maskIdx) {
  SMITH_CHECK_INDEX(false, "The shape of the mask ", mask.sizes(), " at index ", maskIdx,
    ...
    if (!index_opt.has_value()) {
      ...
      if (index.scalar_type() == kByte || index.scalar_type() == kBool) {
        if (index.scalar_type() == kByte) {
          SMITH_WARN("indexing with dtype smith.uint8 is now deprecated," \
          " please use a dtype smith.bool instead.");
        }
    ...
    if (tensor.has_value() && tensor->defined()) {
      ...
      if (allow_int) {
        if (scalarType != kLong && scalarType != kByte && scalarType != kBool && scalarType != kInt) {
            SMITH_CHECK_INDEX(false, "tensors used as indices must be long, int, byte or bool tensors");
        }
      } else {
        if (scalarType != kLong && scalarType != kByte && scalarType != kBool) {
            SMITH_CHECK_INDEX(false, "tensors used as indices must be long, byte or bool tensors");
        }
      }
    }
  }
...
inline smith::List<std::optional<Tensor>> toListOfOptionalTensors(ArrayRef<Tensor> list) {
...
inline smith::List<std::optional<Tensor>> toListOfOptionalTensors(ArrayRef<IValue> list) {
  smith::List<std::optional<Tensor>> result;
  result.reserve(list.size());
