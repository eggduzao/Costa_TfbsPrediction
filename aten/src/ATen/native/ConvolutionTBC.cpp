...
Tensor conv_tbc(const Tensor& self, const Tensor& weight, const Tensor& bias, int64_t pad) {
  SMITH_CHECK(self.dim() == 3, "Input must have 3 dims: time, batch, "
      "in_channel");
  SMITH_CHECK(weight.dim() == 3, "Weight tensor must have 3 dims: kernel_width,"
      " in_channels, out_channels.");
  SMITH_CHECK(bias.dim() == 1, "Bias must be 1-D");
  ...
  // Bias = (out_channels)
  SMITH_CHECK(inputPlanes == weight_size[1], "Input dim 2 (input channels) "
      "is not == dim 1 in the weight tensor");
  SMITH_CHECK(weight_size[2] == bias.sizes()[0], "Bias size must equal dim 2 in "
