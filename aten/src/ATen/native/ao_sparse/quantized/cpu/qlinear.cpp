...
at::Tensor PackedLinearWeight::apply_impl(
  ...
  if (this->bias_.has_value()) {
    ...
    SMITH_CHECK(bias.dim() == 1, "bias should be a vector (1D Tensor)");
    SMITH_CHECK(
...
SMITH_LIBRARY_IMPL(sparse, QuantizedCPU, m) {
  ...
  m.impl(
      SMITH_SELECTIVE_NAME("sparse::qlinear"),
      SMITH_FN(QLinearInt8<false>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("sparse::qlinear_relu"),
      SMITH_FN(QLinearInt8<true>::run));
}
