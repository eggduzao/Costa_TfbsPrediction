...
PackedLinearWeightQnnp::PackedLinearWeightQnnp(
  ...
  SMITH_CHECK(out_features_block_size > 0, "Row block size must be > 0.");
  SMITH_CHECK(in_features_block_size > 0, "Row block size must be > 0.");
...
SMITH_LIBRARY_IMPL(sparse, QuantizedCPU, m) {
  register_linear_params();
  m.impl(
      SMITH_SELECTIVE_NAME("sparse::qlinear_prepack"),
      SMITH_FN(QLinearPackWeightInt8::run));
}
}  // namespace
}  // namespace ao::sparse
