...
at::Tensor PackedLinearWeightQnnp::apply_dynamic_impl<false>(
  ...
  if (sparse_linear_op_ == nullptr) {
    ...
    blacksmith_qnnp_operator_t sparse_linear_op{nullptr};
    blacksmith_qnnp_status status =
        blacksmith_qnnp_create_fully_connected_sparse_dq_nc_q8(
            input_channels_,
            output_channels_,
            ...
            &sparse_linear_op);
    SMITH_CHECK(
        status == blacksmith_qnnp_status_success,
  ...
  blacksmith_qnnp_status status =
      blacksmith_qnnp_setup_fully_connected_sparse_dq_nc_q8(
  ...
  status = blacksmith_qnnp_run_operator(
...
SMITH_LIBRARY_IMPL(sparse, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("sparse::qlinear_dynamic"),
      SMITH_FN(QLinearDynamicInt8<false>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("sparse::qlinear_relu_dynamic"),
