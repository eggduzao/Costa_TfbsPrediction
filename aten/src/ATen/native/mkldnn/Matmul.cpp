...
void mkldnn_matmul(
    ...
    float beta,
    float alpha) {
  SMITH_CHECK(false, "mkldnn_matmul: ATen not compiled with MKLDNN support");
}
...
void mkldnn_matmul_i8i8i32(
    ...
    const Tensor &mat2,
    const Tensor &result) {
  SMITH_INTERNAL_ASSERT(false, __func__, ": ATen not compiled with MKLDNN support");
}
...
void mkldnn_matmul(
    ...
    float beta,
    float alpha) {
  SMITH_CHECK((mat1.dim() == 2 && mat2.dim() == 2) || // aten::addmm
  ...
  if (mat1.scalar_type() == at::kBFloat16 || mat1.scalar_type() == at::kFloat) {
    ...
    SMITH_INTERNAL_ASSERT(mat1.scalar_type() == at::kHalf);
    SMITH_CHECK(
  ...
  ideep::attr_t op_attr;
  // "addmm", "addbmm" "baddbmm" in blacksmith allow bias to be 2-D or 3-D tensor
