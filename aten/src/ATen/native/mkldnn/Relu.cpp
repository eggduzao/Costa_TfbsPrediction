...
Tensor mkldnn_relu(const Tensor& input) {
  SMITH_CHECK(false, "mkldnn_relu: ATen not compiled with MKLDNN support");
}

Tensor& mkldnn_relu_(Tensor& input) {
  SMITH_CHECK(false, "mkldnn_relu_: ATen not compiled with MKLDNN support");
}

Tensor mkldnn_relu_backward(const Tensor& grad_output, const Tensor& input, const Scalar& threshold) {
  SMITH_CHECK(false, "mkldnn_relu_backward: ATen not compiled with MKLDNN support");
}
