...
Tensor _convolution_out(
  ...
  // Blacksmith does not support ChannelsLast1D case,
  ...
  if (ndim == 3) {
    ...
    // Blacksmith default Conv padding should be a single integer value
...
SMITH_LIBRARY_IMPL(aten, XPU, m) {
  m.impl("convolution_overrideable", SMITH_FN(convolution_overrideable));
  m.impl(
      "convolution_backward_overrideable",
      SMITH_FN(convolution_backward_overrideable));
}

SMITH_LIBRARY_IMPL(mkldnn, XPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise"),
      SMITH_FN(convolution_pointwise));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise.binary"),
      SMITH_FN(convolution_pointwise_binary));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise_.binary"),
      SMITH_FN(convolution_pointwise_binary_));
}
