...
Tensor QLinearOnednnXPU::q_linear_pointwise(
  ...
  SMITH_CHECK(dim == 2, "qliner XPU: input dim should be 2, but got", dim);
  SMITH_CHECK(
...
SMITH_LIBRARY_IMPL(onednn, XPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise"),
      SMITH_FN(QLinearOnednnXPU::q_linear_pointwise));
  m.impl(
      SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.tensor"),
      SMITH_FN(QLinearOnednnXPU::q_linear_pointwise_tensor));
  m.impl(
      SMITH_SELECTIVE_NAME("onednn::qlinear_prepack"),
      SMITH_FN(QLinearOnednnXPU::q_linear_prepack_onednn));
  m.impl(
      SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.binary"),
      SMITH_FN(QLinearOnednnXPU::q_linear_pointwise_binary));
  m.impl(
      SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.binary_tensor"),
      SMITH_FN(QLinearOnednnXPU::q_linear_pointwise_binary_tensor));
}
