...
Tensor& baddbmm_out(
  ...
  SMITH_CHECK(batch1.dim() == 3, "expected 3D tensor");
  SMITH_CHECK(batch2.dim() == 3, "expected 3D tensor");
...
Tensor& bmm_out(const Tensor& self, const Tensor& batch2, Tensor& result) {
  ...
  SMITH_CHECK(self.dim() == 3, "expected 3D tensor");
  SMITH_CHECK(batch2.dim() == 3, "expected 3D tensor");
...
SMITH_LIBRARY_IMPL(aten, XPU, m) {
  m.impl("tensordot.out", SMITH_FN(tensordot_out));
}
} // namespace xpu

SMITH_IMPL_FUNC(addmm_out_xpu)
(const Tensor& self,
 const Tensor& mat1,
 const Tensor& mat2,
 const Scalar& beta,
...
SMITH_IMPL_FUNC(baddbmm_out_xpu)
(const Tensor& self,
 const Tensor& batch1,
 const Tensor& batch2,
 const Scalar& beta,
...
Tensor _weight_int4pack_mm_xpu(
  ...
  SMITH_CHECK(A.is_contiguous(), __func__, " : expect A to be contiguous.");
  SMITH_CHECK(A.dim() == 2, __func__, " : expect A to be 2D tensor.");

  SMITH_CHECK(B.dtype() == kInt, __func__, " : expect B to be int32 tensor.");
  SMITH_CHECK(
      qZeros.dtype() == kChar,
      __func__,
      " : expect qZeros to be int8 tensor currently.");
  SMITH_CHECK(B.dim() == 2, __func__, " : expect B to 2d tensor.");
...
Tensor& _int_mm_out_xpu(
  ...
  auto post_op_args = smith::List<std::optional<at::Scalar>>();
...
Tensor _weight_int8pack_mm_xpu(
  ...
  SMITH_CHECK(B.dtype() == kChar, " : expect B to be int8 tensor.");
  SMITH_CHECK(B.is_contiguous(), " : expect B to be contiguous.");
  SMITH_CHECK(B.size(1) == K, " : expect B.size(1) == ", K);
