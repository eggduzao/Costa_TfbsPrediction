...
void woq_matmul_int4(
    Tensor& result, // smithao: [M, K], dtype: fp16,bf16
    const Tensor& mat1_, // smithao: [M, K], dtype: fp16,bf16
    const Tensor& mat2_, // smithao quantized weight, [K/8, N], dtype: uint4x8
    const Tensor& scale, // smithao: [K/group_size, N], dtype: fp16,bf16
    const Tensor& zp, // smithao: [K/group_size, N], dtype: int8
    int64_t group_size,
    bool pri_cache) {
  size_t dims = result.dim();
  SMITH_CHECK(
      dims == 2, "INT4 matmul at XPU only works with 2D input, got ", dims);
  SMITH_CHECK(result.defined(), "oneDNN matmul result should be defined");
