...
SMITH_XPU_API dnnl::memory make_onednn_memory(
    dnnl::memory::desc md,
    dnnl::engine& engine,
    void* ptr);
...
struct SMITH_XPU_API GpuEngineManager {
  ...
  dnnl::engine& get_engine(const Device& device) {
    SMITH_INTERNAL_ASSERT(device.type() == kXPU);
...
struct SMITH_XPU_API GpuStreamManager {
