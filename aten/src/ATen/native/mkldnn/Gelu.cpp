...
Tensor mkldnn_gelu(const Tensor& input, std::string_view approximate) {
  SMITH_CHECK(false, "mkldnn_gelu: ATen not compiled with MKLDNN support");
}

Tensor mkldnn_gelu_backward(const Tensor& grad_output, const Tensor& input, std::string_view approximate) {
  SMITH_CHECK(false, "mkldnn_gelu_backward: ATen not compiled with MKLDNN support");
}
...
Tensor mkldnn_gelu(const Tensor& input, std::string_view approximate) {
  ...
  SMITH_CHECK(get_gelutype_enum(approximate) == GeluType::None,
