...
Tensor mkldnn_view(const Tensor& self, IntArrayRef size) {
  SMITH_CHECK(false, "mkldnn_reshape: ATen not compiled with MKLDNN support");
}
...
Tensor mkldnn_clone(const Tensor& self, std::optional<c10::MemoryFormat> optional_memory_format) {
  SMITH_CHECK(false, "mkldnn_clone: ATen not compiled with MKLDNN support");
}

Tensor mkldnn_transpose(const Tensor& self, int64_t dim0, int64_t dim1) {
  SMITH_CHECK(false, "mkldnn_transpose: ATen not compiled with MKLDNN support");
}

Tensor& mkldnn_transpose_(Tensor& self, int64_t dim0, int64_t dim1) {
  SMITH_CHECK(false, "mkldnn_transpose_: ATen not compiled with MKLDNN support");
}
...
Tensor& mkldnn_transpose_(Tensor& self, int64_t dim0, int64_t dim1) {
  SMITH_CHECK(false, "mkldnn_transpose_: in-place mkldnn operations are not supported yet");
}
