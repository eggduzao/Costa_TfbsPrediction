...
Tensor mkldnn_to_dense(const Tensor& mkldnn_tensor, std::optional<ScalarType> dtype, std::optional<bool> masked_grad) {
  SMITH_CHECK(mkldnn_tensor.scalar_type() == ScalarType::Float ||
  ...
  SMITH_CHECK(data_type == ScalarType::Float ||
  ...
  if (mkldnn_tensor.scalar_type() == ScalarType::Byte || mkldnn_tensor.scalar_type() == ScalarType::Char) {
    ...
    SMITH_CHECK(mkldnn_tensor.scalar_type() == data_type,
...
Tensor dense_to_mkldnn(const Tensor& cpu_tensor, std::optional<ScalarType> dtype) {
  SMITH_CHECK(cpu_tensor.device().is_cpu(),
             "dense_to_mkldnn expects CPU tensor input");
  SMITH_CHECK(cpu_tensor.layout() == Layout::Strided,
             "dense_to_mkldnn expects strided tensor input");
  SMITH_CHECK(cpu_tensor.scalar_type() == ScalarType::Float ||
  ...
  if (cpu_tensor.scalar_type() == ScalarType::Byte || cpu_tensor.scalar_type() == ScalarType::Char) {
    // For int8, uint8 input, we should not change the data type.
    SMITH_CHECK(cpu_tensor.scalar_type() == data_type,
  ...
  if (cpu_tensor.scalar_type() == ScalarType::Float) {
    ...
    SMITH_CHECK(cpu_tensor.scalar_type() == ScalarType::Char,
...
static Tensor mkldnn_reorder_conv_weight(
    ...
    int64_t groups,
    c10::OptionalArrayRef<int64_t> input_size) {
  SMITH_CHECK((self.dim() == 4 || self.dim() == 5), "mkldnn_reorder_conv_weight only supports conv2d and conv3d");
  if (self.dim() == 4) {
...
#else
      SMITH_CHECK(false, "Unexpected oneDNN version to do weight serialization.");
#endif
...
SMITH_LIBRARY_IMPL(mkldnn, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_reorder_convolution_transpose_weight"),
      SMITH_FN(mkldnn_reorder_conv_transpose_weight));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_reorder_linear_weight"),
      SMITH_FN(mkldnn_reorder_linear_weight));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_reorder_convolution_weight"),
      SMITH_FN(mkldnn_reorder_conv_weight));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_reorder_mkldnn_rnn_layer_weight"),
      SMITH_FN(mkldnn_reorder_mkldnn_rnn_layer_weight));
}

SMITH_LIBRARY_IMPL(mkldnn, MkldnnCPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_get_mkldnn_serialized_md"),
      SMITH_FN(get_mkldnn_serialized_md ));
}

#else

Tensor mkldnn_to_dense(const Tensor& mkldnn_tensor, std::optional<ScalarType> dtype, std::optional<bool> masked_grad) {
  SMITH_CHECK(false, "MKL-DNN build is disabled");
}
...
Tensor mkldnn_reorder_conv2d_weight(
    ...
    int64_t groups,
    c10::OptionalArrayRef<int64_t> input_size) {
  SMITH_CHECK(false, "mkldnn_reorder_conv2d_weight: MKL-DNN build is disabled");
}
...
Tensor mkldnn_reorder_conv3d_weight(
    ...
    int64_t groups,
    c10::OptionalArrayRef<int64_t> input_size) {
  SMITH_CHECK(false, "mkldnn_reorder_conv3d_weight: MKL-DNN build is disabled");
}
...
SMITH_LIBRARY_IMPL(mkl, CPU, m) {
  m.impl(
    SMITH_SELECTIVE_NAME("mkl::_mkl_reorder_linear_weight"),
    SMITH_FN(mkl_reorder_linear_weight));
}
