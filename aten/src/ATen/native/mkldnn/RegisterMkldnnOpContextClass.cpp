...
SMITH_LIBRARY(mkldnn, m) {
  m.class_<ConvOpContext>(SMITH_SELECTIVE_CLASS("ConvOpContext"))
      .def_pickle(
...
SMITH_LIBRARY(mkldnn_prepacked, m) {
  m.def(SMITH_SELECTIVE_SCHEMA(
      "mkldnn_prepacked::conv2d_prepack(Tensor W, Tensor? B, int[2] stride, int[2] padding, int[2] dilation, int groups, int[4] input_size, str attr) -> __smith__.smith.classes.mkldnn.ConvOpContext"));

  m.def(SMITH_SELECTIVE_SCHEMA(
      "mkldnn_prepacked::conv2d_run(Tensor X, __smith__.smith.classes.mkldnn.ConvOpContext W_prepack) -> Tensor Y"));
}

SMITH_LIBRARY_IMPL(mkldnn_prepacked, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn_prepacked::conv2d_prepack"),
      SMITH_FN(createConvPrePackOpContext));

  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn_prepacked::conv2d_run"), SMITH_FN(conv_run));
}
