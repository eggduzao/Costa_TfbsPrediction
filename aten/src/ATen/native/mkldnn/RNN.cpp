...
const Tensor& input,
    ...
    bool batch_first,
    bool train) {
      SMITH_CHECK(false, "mkldnn_rnn_layer: ATen not compiled with MKLDNN support");
  }
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> mkldnn_rnn_layer_backward(
    ...
    const at::Tensor& workspace) {
      SMITH_CHECK(false, "mkldnn_rnn_layer_backward: ATen not compiled with MKLDNN support");
    }
...
// MKLDNN GRU gate order is different from Blacksmith's which requires gates shuffle
// (let rt,zt,nt be reset, update, new gates respectively)
//
//   MKLDNN GRU weight_ih/weight_hh gates order: (zt, rt, nt)
//   Blacksmith GRU weight_ih/weight_hh gates order: (rt, zt, nt)
//
// MKLDNN GRU bias has 4 gates instead of 3
//  (Blacksmith GRU bias)     (MKLDNN GRU bias)
//
...
static std::tuple<Tensor, Tensor, Tensor> mkldnn_rnn(
  ...
  SMITH_CHECK(batch_sizes.empty(), "mkldnn_rnn doesn't support packed input");
  if (static_cast<ideep::rnn_kind>(mode) != ideep::rnn_kind::LSTM) {
    SMITH_CHECK(!cx_.defined(), "mkldnn_rnn: illegal defined cx for non-LSTM RNN");
  }
  ...
  for (const auto layer: c10::irange(num_layers)) {
    for (const auto direction: c10::irange(num_directions)) {
      ...
      SMITH_CHECK(layer_weights.size() == 2 || layer_weights.size() == 4);
