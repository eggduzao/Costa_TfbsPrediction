...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm(
    ...
    double momentum,
    double eps) {
  SMITH_CHECK(false, "mkldnn_batch_norm: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm_backward(
  ...
  SMITH_CHECK(false, "mkldnn_batch_norm_backward: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_layer_norm_last_index_weight_bias_f32(
  ...
  SMITH_CHECK(false, "mkldnn_layer_norm_last_index_weight_bias_f32: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _mkldnn_batch_norm_legit(
    ...
    double momentum,
    double eps) {
  SMITH_CHECK(false, "_mkldnn_batch_norm_legit: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _mkldnn_batch_norm_legit_no_stats(
    ...
    double momentum,
    double eps) {
  SMITH_CHECK(false, "_mkldnn_batch_norm_legit_no_stats: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor, Tensor> _batch_norm_with_update_mkldnn(
  ...
  SMITH_CHECK(false, "_batch_norm_with_update_mkldnn: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> _new_batch_norm_backward_mkldnn(
  ...
  SMITH_CHECK(false, "_new_batch_norm_backward_mkldnn: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_layer_norm_last_index_weight_bias_f32(
  ...
  SMITH_INTERNAL_ASSERT(normalized_shape.size() == 1, "only accept shapes with the last dimension");
  SMITH_INTERNAL_ASSERT(input.scalar_type() == at::kFloat);
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm(
  ...
  SMITH_CHECK(weight.defined() && bias.defined(),
  ...
  if (train) {
    ...
    SMITH_CHECK(input.dim() == 4 || input.dim() == 5,
    ...
    if (use_running_stat) {
      ...
      SMITH_CHECK(false, "mkldnn_batch_norm: mkldnn inference is not keep running estimates.");
    }
    return std::make_tuple(
        new_with_itensor_mkldnn(std::move(y), optTypeMetaToScalarType(input.options().dtype_opt()),
                                input.options().device_opt()),
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_batch_norm_backward(const Tensor& grad_output,
  ...
  SMITH_CHECK(train, "mkldnn_batch_norm_backward: currently mkldnn only support train model");
