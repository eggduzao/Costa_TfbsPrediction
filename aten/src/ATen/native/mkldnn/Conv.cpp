...
Tensor mkldnn_convolution(
  ...
  SMITH_CHECK(false, "mkldnn_convolution_forward: ATen not compiled with MKLDNN support");
}
...
#undef MKLDNN_CONV_ARG_CHECK
  SMITH_CHECK(!is_padding_neg, "negative padding is not supported");
  SMITH_CHECK(!is_stride_nonpos, "non-positive stride is not supported");
  SMITH_CHECK(!is_dilation_nonpos, "non-positive dilation is not supported");
  ...
  SMITH_CHECK(weight_sizes[0] % groups == 0,
              ...
              "] instead");
  SMITH_CHECK(input.size(1) == (weight_sizes[1] * groups),
              "Given groups=", groups, ", weight of size ", weight_sizes,
              ", expected input", input.sizes(), " to have ",
              (weight_sizes[1] * groups), " channels, but got ", input.size(1),
              " channels instead");
  SMITH_CHECK(!bias.defined() || (bias.ndimension() == 1 && bias.size(0) == weight_sizes[0]),
...
//
// In case memory format passed from Blacksmith (aka. user layout)
...
//      model = smith.utils.mkldnn.to_mkldnn(model)
//
...
static Tensor _mkldnn_convolution(
    ...
    smith::List<std::optional<at::Scalar>> scalars =
        smith::List<std::optional<at::Scalar>>(),
...
std::vector<int64_t> _original_deconv_weight_size(
    const Tensor& weight_t,
    int64_t groups) {
  SMITH_CHECK(weight_t.is_mkldnn() || weight_t.is_meta(), "expects weight_t to be mkldnn or meta tensor");
...
Tensor _mkldnn_convolution_transpose(
  ...
  if (attr != "none") {
    auto it = fusion_unary_attr_map().find(attr);
    SMITH_CHECK(it != fusion_unary_attr_map().end(), "Fusion behavior undefined.");
    op_attr = it->second(scalars, algorithm);
  }
  ...
  if (!weight.is_mkldnn()) {
    ...
    // while Blacksmith has IOHW or IODHW, `._transpose()` switches strides (no memory copy).
    w.transpose_(0, 1);
  }
...
Tensor mkldnn_convolution_backward_input(
  ...
  if (mkldnn_conv_enabled_fpmath_mode_bf16() &&
      weight.scalar_type() == at::kFloat) {
    SMITH_WARN_ONCE(
        "Unexpected oneDNN version to support fpmath_mode_bf16, please update oneDNN version to align with blacksmith main branch");
      }
  if (mkldnn_conv_enabled_fpmath_mode_tf32() &&
      weight.scalar_type() == at::kFloat) {
    SMITH_WARN_ONCE(
        "Unexpected oneDNN version to support fpmath_mode_tf32, please update oneDNN version to align with blacksmith main branch");
      }
#endif
...
SMITH_LIBRARY_IMPL(mkldnn, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise"),
      SMITH_FN(mkldnn_convolution_pointwise));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise.binary"),
      SMITH_FN(mkldnn_convolution_pointwise_binary));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise_.binary"),
      SMITH_FN(mkldnn_convolution_pointwise_binary_));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_transpose_pointwise"),
      SMITH_FN(mkldnn_convolution_transpose_pointwise));
}

SMITH_LIBRARY_IMPL(mkldnn, MkldnnCPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_pointwise"),
      SMITH_FN(mkldnn_convolution_pointwise));
  m.impl(
...
SMITH_LIBRARY_IMPL(mkldnn, Meta, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_convolution_transpose_pointwise"),
      SMITH_FN(mkldnn_convolution_transpose_pointwise_meta));
}
}  // namespace at::native
