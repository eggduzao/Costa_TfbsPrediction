 ...
 */
template <typename T>
struct SMITH_API IntrusivePtrTargetWrapper : c10::intrusive_ptr_target {
private:
  T target_;
...
dnnl::memory::data_type get_mkldnn_dtype(ScalarType type) {
  switch (type) {
    ...
    default:
      SMITH_CHECK(false, "get_mkldnn_dtype: unsupported data type");
  }
}
...
#else
  SMITH_CHECK(false, "Unexpected oneDNN version to do weight deserialization.");
#endif
...
ideep::tensor& itensor_from_mkldnn(const MKLDNNTensor& mkldnn_tensor) {
  SMITH_CHECK(mkldnn_tensor.is_mkldnn(),
...
ideep::tensor itensor_view_from_dense(const Tensor& tensor, bool from_const_data_ptr) {
  ...
  if (tensor.scalar_type() == ScalarType::Float) {
    ...
    SMITH_CHECK(false, "itensor_view_from_dense expects float/bfloat16/half/int8/fp8 tensor input", tensor.scalar_type());
  }
}
...
SMITH_LIBRARY_IMPL(mkldnn, MkldnnCPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::data_ptr"),
      SMITH_FN(data_ptr_from_mkldnn));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_nbytes"),
      SMITH_FN(nbytes_from_mkldnn));
}
