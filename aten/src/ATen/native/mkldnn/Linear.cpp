...
Tensor mkldnn_linear(
  ...
  SMITH_CHECK(false, "mkldnn_linear: ATen not compiled with MKLDNN support");
}
Tensor mkldnn_linear_backward_input(
    IntArrayRef input_size, const Tensor& grad_output, const Tensor& weight) {
  SMITH_CHECK(false, "mkldnn_linear_backward_input: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor> mkldnn_linear_backward_weights(
  ...
  SMITH_CHECK(false, "mkldnn_linear_backward_weights: ATen not compiled with MKLDNN support");
}
...
std::tuple<Tensor, Tensor, Tensor> mkldnn_linear_backward(
  ...
  SMITH_CHECK(false, "mkldnn_linear_backward: ATen not compiled with MKLDNN support");
}
...
mkldnn_scaled_mm(const Tensor& mat1, const Tensor& mat2,
          ...
          bool use_fast_accum,
          Tensor& out) {
  SMITH_INTERNAL_ASSERT(false, "mkldnn_scaled_mm: ATen not compiled with MKLDNN support");
}
...
Tensor mkldnn_linear(
  ...
  if (self.scalar_type() == ScalarType::BFloat16) {
    ...
    SMITH_CHECK(mkldnn_fp16_device_check(),
...
Tensor mkldnn_linear_backward_input(
  ...
  SMITH_CHECK(grad_output.is_mkldnn(),
      "mkldnn_linear_backward: grad_output needs to be mkldnn layout");
  SMITH_CHECK(weight_t.device().is_cpu() && weight_t.scalar_type() == kFloat,
...
std::tuple<Tensor, Tensor> mkldnn_linear_backward_weights(
    const Tensor& grad_output, const Tensor& input, const Tensor& weight, bool bias_defined) {
  SMITH_CHECK(grad_output.is_mkldnn() && input.is_mkldnn(),
      "mkldnn_linear_backward: grad_output and input needs to be mkldnn layout");
  SMITH_CHECK(weight.device().is_cpu() && weight.scalar_type() == kFloat,
...
SMITH_LIBRARY_IMPL(mkl, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("mkl::_mkl_linear"), SMITH_FN(mkl_linear));
}

SMITH_LIBRARY_IMPL(mkl, MkldnnCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("mkl::_mkl_linear"), SMITH_FN(mkl_linear));
}
...
SMITH_LIBRARY_IMPL(mkldnn, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_linear_pointwise"),
      SMITH_FN(mkldnn_linear_pointwise));
  m.impl(
      SMITH_SELECTIVE_NAME("mkldnn::_linear_pointwise.binary"),
      SMITH_FN(mkldnn_linear_pointwise_binary));
}
...
mkldnn_scaled_mm(const Tensor& mat1, const Tensor& mat2,
  ...
  SMITH_INTERNAL_ASSERT((scale_a.numel() == 1 && scale_b.numel() == 1), "Now _scaled_mm only supports per-tensor scaling for CPU backend.");
  SMITH_CHECK(
      !scale_result ||
  ...
  SMITH_CHECK(isFloat8Type(mat1.scalar_type()), "Expected mat1 to be Float8 matrix got ", mat1.scalar_type());
  SMITH_CHECK(isFloat8Type(mat2.scalar_type()), "Expected mat2 to be Float8 matrix got ", mat2.scalar_type());
