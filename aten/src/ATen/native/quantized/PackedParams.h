...
struct LinearPackedParamsBase : public smith::jit::CustomClassHolder {
  ...
  virtual at::Tensor& apply_out(
      ...
      at::Tensor& output) {
    SMITH_CHECK(false, "apply_out is not implemented for this packed parameter type");
    return output;
  }
  ...
  virtual at::Tensor& apply_relu_out(
      ...
      int64_t /*output_zero_point*/,
      at::Tensor& output) {
    SMITH_CHECK(false, "apply_relu_out is not implemented for this packed parameter type");
    return output;
  }
  ...
  virtual at::Tensor apply_with_input_q_dq_qweight_dq_output_fp32(
    ...
    SMITH_CHECK(false, "apply_with_input_q_dq_qweight_dq_output_fp32 is not implemented for this packed parameter type");
    return {};
  }
  ...
  virtual at::Tensor apply_with_input_q_dq_qweight_dq_relu_output_fp32(
      ...
      double input_scale,
      int64_t input_zero_point) {
        SMITH_CHECK(false, "apply_with_input_q_dq_qweight_dq_relu_output_fp32 is not implemented for this packed parameter type");
    return {};
  }
  ...
  virtual at::Tensor& apply_dynamic_out(
      ...
      at::Tensor& output,
      bool /* reduce_range */) {
        SMITH_CHECK(false, "apply_dynamic_out is not implemented for this packed parameter type");
    return output;
  }
  virtual at::Tensor& apply_dynamic_relu_out(
      ...
      at::Tensor& output,
      bool /* reduce_range */) {
    SMITH_CHECK(false, "apply_dynamic_relu_out is not implemented for this packed parameter type");
    return output;
  }
  ...
  virtual void set_bias(std::optional<at::Tensor> /*bias*/) {
        SMITH_CHECK(false, "set_bias is not implemented for this packed parameter type");
  }
};

template <int kSpatialDim = 2>
struct ConvPackedParamsBase : public smith::jit::CustomClassHolder {
  virtual at::Tensor apply(
      const at::Tensor& input,
      double output_scale,
      int64_t output_zero_point) = 0;
  ...
  virtual smith::List<int64_t> stride() const = 0;
  virtual smith::List<int64_t> padding() const = 0;
  virtual smith::List<int64_t> output_padding() const = 0;
  virtual smith::List<int64_t> dilation() const = 0;
