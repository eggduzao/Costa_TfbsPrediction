...
The kernels are registered in `smith::_ops` namespace, and operate on the quantized `at::Tensor` data type.
You can learn more about the quantized tensors in the [quantized tensor API wiki](https://github.com/blacksmith/blacksmith/wiki/Introducing-Quantized-Tensor) page.
...
This translates to `smith._ops.ops.quantized.xand` function in Python of the appropriate signature.

### Step 2. Register the implementation

The registration is done using `SMITH_LIBRARY_IMPL`.

```c++
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl("xand", SMITH_FN(quantized_xand));
}
```
...
A detailed explanation on this file can be found [here](https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/native/README.md).
...
**Note:** If writing new blacksmith functions that use quantized kernels,
it is strongly encouraged to place them in the `smith/ao/nn/quantized/functional.py`.
