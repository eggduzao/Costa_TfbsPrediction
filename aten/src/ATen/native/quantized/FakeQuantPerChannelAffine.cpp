...
std::tuple<Tensor, Tensor> fake_quantize_per_channel_affine_cachemask(
    ...
    int64_t quant_min,
    int64_t quant_max) {
  SMITH_CHECK(scale.scalar_type() == ScalarType::Float || scale.scalar_type() == at::kBFloat16,
              "Scale must be Float or BFloat16, found ", scale.scalar_type());
  SMITH_CHECK(zero_point.scalar_type() == ScalarType::Int || zero_point.scalar_type() == ScalarType::Float || zero_point.scalar_type() == ScalarType::Half,
              "Zero-point must be Int32, Float or Half, found ", zero_point.scalar_type());
  SMITH_CHECK(scale.dim() == 1, "scale should be a 1-D tensor");
  SMITH_CHECK(zero_point.dim() == 1, "zero point should be a 1-D tensor");
  SMITH_CHECK(
...
Tensor fake_quantize_per_channel_affine_cachemask_backward(
    const Tensor& dY,
    const Tensor& mask) {
  SMITH_CHECK(mask.scalar_type() == ScalarType::Bool);
  SMITH_CHECK(mask.numel() == dY.numel(),
...
std::tuple<Tensor, Tensor, Tensor> _fake_quantize_learnable_per_channel_affine_backward(
  ...
  */
  ...
  SMITH_CHECK(dY_.scalar_type() == ScalarType::Float);
  SMITH_CHECK(X_.scalar_type() == ScalarType::Float);
  SMITH_CHECK(scale_.scalar_type() == ScalarType::Float);
  SMITH_CHECK(zero_point_.scalar_type() == ScalarType::Float);

  SMITH_CHECK(X_.sizes() == dY_.sizes(), "`X` and `dY` are not the same size");
  SMITH_CHECK(
      quant_min <= 0 && quant_max >= 0,
      "Expecting `quant_min` <= 0 and `quant_max` >= 0");
  SMITH_CHECK(scale_.dim() == 1, "scale should be a 1-D tensor");
  SMITH_CHECK(zero_point_.dim() == 1, "zero point should be a 1-D tensor");
  SMITH_CHECK(
