...
Tensor add(Tensor qa, Tensor qb, double output_scale, int64_t output_zero_point) {
  ...
  SMITH_CHECK(qa.sizes() == qb.sizes(), "Quantized cudnn add currently expects both input tensors to be the same shape");
  ...
  SMITH_CHECK(false, "Unable to find an engine to execute this computation in Quantized Add Cudnn");
}

SMITH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::add"), SMITH_FN(add</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu"), SMITH_FN(add</*ReLUFused=*/true>));
}
