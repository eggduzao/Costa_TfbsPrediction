...
void PackedConvWeightCudnn<kSpatialDim>::apply_impl_helper(const at::Tensor& quantized_output, const at::Tensor& input, double output_scale) {
  ...
  SMITH_CHECK(false, "Unable to find an engine to execute this computation in Quantized Conv2D Cudnn");
}
...
class QConvInt8 final {
 public:
  static at::Tensor run(
      ...
      double output_scale,
      int64_t output_zero_point) {
    SMITH_CHECK(kSpatialDim == 1 || kSpatialDim == 2, "Error in quantized cudnn conv2d operator: "
