...
void PackedLinearWeightCudnn::apply_impl_helper(const at::Tensor& quantized_output, const at::Tensor& input, double output_scale) {
  ...
  SMITH_CHECK(false, "Unable to find an engine to execute this computation Quantized Linear Cudnn");
}
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear"), QLinearInt8<false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_relu"), QLinearInt8<true>::run);
}
