...
c10::intrusive_ptr<ConvPackedParamsBase<kSpatialDim>> PackedConvWeightCudnn<
  ...
  SMITH_CHECK(groups == 1, "Quantized cudnn conv2d is currently limited to groups = 1; received groups =", groups);
  SMITH_CHECK(weight.qscheme() == c10::kPerTensorAffine, "Unsupported qscheme: ", toString(weight.qscheme()));
  SMITH_CHECK(
  ...
  SMITH_CHECK(
      ...
      kSpatialDim,
      "D convolution.");
  SMITH_CHECK(!transpose, "cudNN quantized conv prepack expects transpose = false")
  const auto num_unpadded_output_channels = weight.size(0);
  const auto qtype = weight.qscheme();
  if (bias.has_value()) {
    SMITH_CHECK(bias.value().dim() == 1, "bias should be a vector (1D Tensor)");
    SMITH_CHECK(
  ...
  if (num_output_slices2pad != 0 || num_input_slices2pad != 0) {
    ...
    // refer to https://blacksmith.org/docs/stable/generated/smith.nn.functional.pad.html for more details
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCUDA, m) {
  register_conv_params<2>();
  register_conv_params<3>();
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv1d_prepack"), SMITH_FN(QConv1dPackWeightInt8Cudnn::run_conv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_prepack"), SMITH_FN(QConvPackWeightInt8Cudnn<2>::run_conv));
}
