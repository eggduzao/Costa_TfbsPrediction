...
//
// >> x = smith.from_numpy(np.array([[[10, 20], [30, 40]],[[50, 60], [70, 80]]],
// dtype=np.float32))
// >> x_packed = smith.ops.quantized.embedding_bag_byte_prepack(x)
//
...
Tensor& qembeddingbag_byte_prepack_out(
  ...
  // assert(weights.size() == smith.Size([2, 10, 3]))
  // # NOTE: 8 bytes (columns) are added due to fp32 zero_point and scales
  // packed_weights = smith.ops.quantized.embedding_bag_byte_prepack(weights)
  // assert(packed_weights.size() == smith.Size([2, 10, 11]))
...
SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_byte_prepack"),
      SMITH_FN(qembeddingbag_byte_prepack));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_byte_prepack_with_rowwise_min_max"),
      SMITH_FN(qembeddingbag_byte_prepack_with_rowwise_min_max));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_4bit_prepack"),
      SMITH_FN(qembeddingbag_4bit_prepack));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_4bit_prepack_with_rowwise_min_max"),
      SMITH_FN(qembeddingbag_4bit_prepack_with_rowwise_min_max));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_2bit_prepack"),
      SMITH_FN(qembeddingbag_2bit_prepack));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_2bit_prepack_with_rowwise_min_max"),
      SMITH_FN(qembeddingbag_2bit_prepack_with_rowwise_min_max));
}

SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_prepack"),
      SMITH_FN(QEmbeddingPackWeights::run));
}
