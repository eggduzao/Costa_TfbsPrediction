...
static inline at::Tensor pack_weight_to_fp16_onednn_tensor(
    at::Tensor& weight,
    std::optional<smith::List<int64_t>>& input_shape) {
  SMITH_CHECK(weight.scalar_type() == at::kHalf || weight.scalar_type() == at::kFloat, "Weight should be of type float or float16");
...
at::Tensor wrapped_quantized_linear(
  ...
  // bypass the issue from smith.export
#ifdef USE_FBGEMM
...
#else // USE_FBGEMM
  SMITH_CHECK(
      false, "This Blacksmith installation was not built with FBGEMM operators");
#endif // USE_FBGEMM
}
 ...
 public:
  static Tensor run(
    ...
    SMITH_CHECK(false,
        ...
        "Please re-export your model using the newer definitions in smith.jit.quantized");
  }
};
...
class QLinearPackWeightInt8Onednn final {
 public:
  static at::Tensor run(
    ...
    // NOLINTNEXTLINE(performance-unnecessary-value-param)
    [[maybe_unused]] std::optional<smith::List<int64_t>> input_shape) {
#if AT_MKLDNN_ENABLED()
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_prepack"), SMITH_FN(QLinearPackWeightInt8::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_prepack_legacy"), SMITH_FN(QLinearPackWeightInt8Legacy::run));
}

SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  register_linear_params();
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_prepack_fp16"), SMITH_FN(QLinearPackWeightFp16::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_prepack_fp16_legacy"), SMITH_FN(QLinearPackWeightFp16Legacy::run));
}

SMITH_LIBRARY_IMPL(_quantized, QuantizedCPU, m) {
  register_linear_params();
  m.impl(SMITH_SELECTIVE_NAME("_quantized::linear_prepack"), SMITH_FN(QLinearPackWeightInt8::run));
}

SMITH_LIBRARY_IMPL(_quantized, CPU, m) {
  register_linear_params();
  m.impl(SMITH_SELECTIVE_NAME("_quantized::linear_prepack_fp16"), SMITH_FN(QLinearPackWeightFp16::run));
  m.impl(SMITH_SELECTIVE_NAME("_quantized::linear_prepack_fp16_legacy"), SMITH_FN(QLinearPackWeightFp16Legacy::run));
  m.impl(SMITH_SELECTIVE_NAME("_quantized::wrapped_quantized_linear"), SMITH_FN(wrapped_quantized_linear));
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::_wrapped_linear_prepack"),
      _wrapped_linear_prepack);
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::_wrapped_quantized_linear_prepacked"),
      _wrapped_quantized_linear_prepacked);
}

SMITH_LIBRARY_IMPL(_quantized, Meta, m) {
  m.impl(SMITH_SELECTIVE_NAME("_quantized::wrapped_quantized_linear"), SMITH_FN(wrapped_quantized_linear_meta));
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::_wrapped_linear_prepack"),
...
SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::qlinear_prepack"), SMITH_FN(QLinearPackWeightInt8Onednn::run));
}

SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::linear_prepack_fp16"), SMITH_FN(QLinearPackWeightFp16Onednn::run));
}
