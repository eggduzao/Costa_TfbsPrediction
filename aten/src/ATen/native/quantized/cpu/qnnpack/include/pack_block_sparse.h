  ...
  for (uint32_t i = 0; i < num_row_blocks; ++i) {
    for (uint32_t j = 0; j < num_col_blocks; ++j) {
      ...
      for (uint32_t ib = 0; ib < row_block_size; ++ib) {
        ...
        if BLACKSMITH_QNNP_UNLIKELY(row_index >= N) {
          break;
        }
        for (uint32_t jb = 0; jb < col_block_size; ++jb) {
          uint32_t col_index = j * col_block_size + jb;
          if BLACKSMITH_QNNP_UNLIKELY(col_index >= K) {
            goto block_scanned;
          }
...
struct IndicesDtypeEnumTrait<uint32_t> {
  const static blacksmith_qnnp_sparse_matrix_indices_dtype dtype =
      blacksmith_qnnp_sparse_matrix_indices_dtype_uint32_t;
};

template <>
struct IndicesDtypeEnumTrait<uint16_t> {
  const static blacksmith_qnnp_sparse_matrix_indices_dtype dtype =
      blacksmith_qnnp_sparse_matrix_indices_dtype_uint16_t;
};

template <>
struct IndicesDtypeEnumTrait<uint8_t> {
  const static blacksmith_qnnp_sparse_matrix_indices_dtype dtype =
      blacksmith_qnnp_sparse_matrix_indices_dtype_uint8_t;
};
...
void TypedBCSRMatrix<INDICES_DTYPE>::print() const {
  ...
  std::cout
      << "indices dtype: uint"
      << static_cast<
             std::underlying_type_t<blacksmith_qnnp_sparse_matrix_indices_dtype>>(
             indices_dtype)
      << "_t" << std::endl;
