...
static void hardsigmoid_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_status status = blacksmith_qnnp_initialize();
  if (status != blacksmith_qnnp_status_success) {
    state.SkipWithError("failed to initialize QNNPACK");
  }

  blacksmith_qnnp_operator_t hardsigmoidOperator = nullptr;
  status = blacksmith_qnnp_create_hardsigmoid_nc_q8(
      channels,
      ...
      0 /* flags */,
      &hardsigmoidOperator);
  if (status != blacksmith_qnnp_status_success || hardsigmoidOperator == nullptr) {
    state.SkipWithError("failed to create Hardsigmoid operator");
  }

  status = blacksmith_qnnp_setup_hardsigmoid_nc_q8(
      hardsigmoidOperator,
      batchSize,
      input.data(),
  ...
  for (auto _ : state) {
    status =
        blacksmith_qnnp_run_operator(hardsigmoidOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
  ...
  status = blacksmith_qnnp_delete_operator(hardsigmoidOperator);
  if (status != blacksmith_qnnp_status_success) {
