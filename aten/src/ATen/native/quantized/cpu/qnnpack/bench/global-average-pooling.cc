...
static void global_average_pooling_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t globalPoolingOperator = nullptr;
  status = blacksmith_qnnp_create_global_average_pooling_nwc_q8(
      channels,
  ...
  status = blacksmith_qnnp_setup_global_average_pooling_nwc_q8(
      globalPoolingOperator,
      batchSize,
  ...
  for (auto _ : state) {
    blacksmith_qnnp_run_operator(globalPoolingOperator, nullptr /* thread pool */);
  }

  status = blacksmith_qnnp_delete_operator(globalPoolingOperator);
  if (status != blacksmith_qnnp_status_success) {
