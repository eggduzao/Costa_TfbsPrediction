...
static void convolution_q8(benchmark::State& state, const char* net, bool per_channel=false) {
  ...
  blacksmith_qnnp_operator_t convolutionObject = nullptr;
  ...
  status = blacksmith_qnnp_create_convolution2d_nhwc_q8(
      paddingHeight,
      paddingWidth,
      kernelHeight,
      kernelWidth,
  ...
  status = blacksmith_qnnp_setup_convolution2d_nhwc_q8(
      convolutionObject,
      batchSize,
      inputHeight,
      inputWidth,
  ...
  for (auto _ : state) {
    blacksmith_qnnp_run_operator(convolutionObject, nullptr /* thread pool */);
  }

  status = blacksmith_qnnp_delete_operator(convolutionObject);
  if (status != blacksmith_qnnp_status_success) {
