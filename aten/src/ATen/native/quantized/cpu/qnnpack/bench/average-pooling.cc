...
static void average_pooling_q8(benchmark::State& state, const char* net) {
  ...
  blacksmith_qnnp_operator_t poolingOperator = nullptr;
  status = blacksmith_qnnp_create_average_pooling2d_nhwc_q8(
      paddingSize,
      paddingSize,
      poolingSize,
      poolingSize,
  ...
  status = blacksmith_qnnp_setup_average_pooling2d_nhwc_q8(
      poolingOperator,
      batchSize,
      inputHeight,
      inputWidth,
  ...
  for (auto _ : state) {
    status =
        blacksmith_qnnp_run_operator(poolingOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
      state.SkipWithError("failed to run Average Pooling operator");
    }
  }

  status = blacksmith_qnnp_delete_operator(poolingOperator);
  if (status != blacksmith_qnnp_status_success) {
