...
#if BLACKSMITH_QNNPACK_BENCHMARK_GEMMLOWP
...
class Q8GEMM_XZP : public Q8GEMM {
 public:
  inline Q8GEMM_XZP(uint32_t mr, uint32_t nr, uint32_t np, uint32_t kr)
   ...
   void SetUp(const benchmark::State&) override {
    ...
    requantizationParams_ =
        blacksmith_qnnp_compute_requantization_params(0.75f, 127, 1, 254);
  }
  ...
  inline const blacksmith_qnnp_q31_requantization_params* requantizationParams()
      const {
 ...
 protected:
  ...
  blacksmith_qnnp_q31_requantization_params requantizationParams_;
};
...
static void q8gemm_compute_row_sum(
  ...
  for (size_t block_start = 0; block_start < m; block_start += block_size) {
    blacksmith_q8sumrows_ukernel_4x__neon(
