...
static void hardswish_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t hardswishOperator = nullptr;
  status = blacksmith_qnnp_create_hardswish_nc_q8(
      channels,
      ...
      0 /* flags */,
      &hardswishOperator);
  if (status != blacksmith_qnnp_status_success || hardswishOperator == nullptr) {
    state.SkipWithError("failed to create Hardswish operator");
  }

  status = blacksmith_qnnp_setup_hardswish_nc_q8(
      hardswishOperator,
      batchSize,
      input.data(),
  ...
  for (auto _ : state) {
    status =
        blacksmith_qnnp_run_operator(hardswishOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
  ...
  status = blacksmith_qnnp_delete_operator(hardswishOperator);
  if (status != blacksmith_qnnp_status_success) {
