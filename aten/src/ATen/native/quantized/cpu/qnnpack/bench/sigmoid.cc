...
static void sigmoid_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t sigmoidOperator = nullptr;
  status = blacksmith_qnnp_create_sigmoid_nc_q8(
      channels,
      ...
      0 /* flags */,
      &sigmoidOperator);
  if (status != blacksmith_qnnp_status_success || sigmoidOperator == nullptr) {
    state.SkipWithError("failed to create Sigmoid operator");
  }

  status = blacksmith_qnnp_setup_sigmoid_nc_q8(
      sigmoidOperator,
      batchSize,
      input.data(),
  ...
  for (auto _ : state) {
    status =
        blacksmith_qnnp_run_operator(sigmoidOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
  ...
  status = blacksmith_qnnp_delete_operator(sigmoidOperator);
  if (status != blacksmith_qnnp_status_success) {
