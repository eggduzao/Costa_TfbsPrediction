...
static void tanh_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t tanhOperator = nullptr;
  status = blacksmith_qnnp_create_tanh_nc_q8(
      channels,
      ...
      0 /* flags */,
      &tanhOperator);
  if (status != blacksmith_qnnp_status_success || tanhOperator == nullptr) {
    state.SkipWithError("failed to create TanH operator");
  }

  status = blacksmith_qnnp_setup_tanh_nc_q8(
      tanhOperator,
      batchSize,
      input.data(),
  ...
  for (auto _ : state) {
    status =
        blacksmith_qnnp_run_operator(tanhOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
  ...
  status = blacksmith_qnnp_delete_operator(tanhOperator);
  if (status != blacksmith_qnnp_status_success) {
