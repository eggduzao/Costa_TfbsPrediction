...
static void add_nc_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t addOperator = nullptr;
  status = blacksmith_qnnp_create_add_nc_q8(
      channels,
      127 /* a:zero point */,
      1.0f /* a:scale */,
      127 /* b:zero point */,
      ...
      0 /* flags */,
      &addOperator);
  if (status != blacksmith_qnnp_status_success || addOperator == nullptr) {
    state.SkipWithError("failed to create Q8 Add operator");
  }

  status = blacksmith_qnnp_setup_add_nc_q8(
      addOperator,
      batchSize,
      a.data(),
      channels /* a:stride */,
  ...
  for (auto _ : state) {
    status = blacksmith_qnnp_run_operator(addOperator, nullptr /* thread pool */);
    if (status != blacksmith_qnnp_status_success) {
  ...
  status = blacksmith_qnnp_delete_operator(addOperator);
  if (status != blacksmith_qnnp_status_success) {
