...
static void softargmax_q8(benchmark::State& state) {
  ...
  blacksmith_qnnp_operator_t softArgMaxOperator = nullptr;
  status = blacksmith_qnnp_create_softargmax_nc_q8(
      channels,
      1.0f /* input scale */,
      0 /* output zero point */,
      ...
      0 /* flags */,
      &softArgMaxOperator);
  if (status != blacksmith_qnnp_status_success || softArgMaxOperator == nullptr) {
    state.SkipWithError("failed to create SoftArgMax operator");
  }

  status = blacksmith_qnnp_setup_softargmax_nc_q8(
      softArgMaxOperator,
      batchSize,
      input.data(),
  ...
  status = blacksmith_qnnp_delete_operator(softArgMaxOperator);
  if (status != blacksmith_qnnp_status_success) {
