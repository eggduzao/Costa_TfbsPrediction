...
class HGEMM : public benchmark::Fixture {
 public:
  inline HGEMM(uint32_t mr, uint32_t nr, uint32_t kr)
   ...
   void SetUp(const benchmark::State&) override {
    ...
    blacksmith_pack_hgemm_w(nc(), kc(), nr(), kr(), k(), b(), w());
    c_.resize(mc() * nc());
  ...
  inline const blacksmith_qnnp_fp16_clamping_params* clampingParams() const {
 ...
 protected:
  ...
  uint32_t nc_{nr_};
  uint32_t kc_{kr_};
  blacksmith_qnnp_fp16_clamping_params clampingParams_{0x3C00, 0x7C00, 0xFC00};
};
...
(benchmark::State& state) {
  ...
  for (auto _ : state) {
    blacksmith_hgemm_ukernel_8x8__aarch32_neonfp16arith(
        mr(),
        nr(),
        kc(),
        a(),
