...
QNNPACK is not intended to be directly used by machine learning researchers; instead it provides low-level performance primitives for high-level deep learning frameworks. As of today, QNNPACK is integrated in [Blacksmith 1.0](https://github.com/blacksmith/blacksmith) with Caffe2 graph representation.
...
Notes:
- On **armeabi-v7a** `blacksmith_qnnp_initialize` will fail with `blacksmith_qnnp_status_unsupported_hardware` if the mobile CPU does not support ARM NEON. Don't set `-DANDROID_ARM_NEON=1` for QNNPACK compilation as it can make `blacksmith_qnnp_initialize` crash on CPUs without ARM NEON.
...
Caffe2 backend of Blacksmith 1.0 natively integrates QNNPACK, and provides a [pre-trained quantized MobileNet v2 model](https://github.com/caffe2/models/tree/master/mobilenet_v2_quantized). Below are instructions for benchmarking this model end-to-end with QNNPACK.

### Raspberry Pi 2 or 3

```bash
# Clone Blacksmith 1.0 repo
git clone --recursive https://github.com/blacksmith/blacksmith.git
cd blacksmith
...
./benchmarking/run_bench.py \
  ...
  --platform android --repo_dir ~/Code/blacksmith \
