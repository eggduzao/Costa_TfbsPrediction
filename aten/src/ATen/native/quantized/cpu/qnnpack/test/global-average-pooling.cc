...
TEST(GLOBAL_AVERAGE_POOLING_OP, unit_batch_many_channels_small_width) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = blacksmith_qnnp_params.q8gavgpool.nr;
       channels <= 3 * blacksmith_qnnp_params.q8gavgpool.nr;
       channels++) {
    for (size_t width = 1; width <= blacksmith_qnnp_params.q8gavgpool.mr;
         width++) {
      GlobalAveragePoolingOperatorTester()
          .batchSize(1)
          .width(width)
  ...
  for (size_t channels = blacksmith_qnnp_params.q8gavgpool.nr;
    ...
    for (size_t width = 1; width <= blacksmith_qnnp_params.q8gavgpool.mr;
      ...
      GlobalAveragePoolingOperatorTester()
          ...
          .width(width)
          .channels(channels)
          .inputStride(5 * blacksmith_qnnp_params.q8gavgpool.nr)
          .testQ8();
    }
  }
}
...
TEST(GLOBAL_AVERAGE_POOLING_OP, unit_batch_many_channels_large_width) {
  ...
  for (size_t channels = blacksmith_qnnp_params.q8gavgpool.nr;
       channels <= 3 * blacksmith_qnnp_params.q8gavgpool.nr;
       channels++) {
    for (size_t width = blacksmith_qnnp_params.q8gavgpool.mr;
         width <= 4 * blacksmith_qnnp_params.q8gavgpool.mr;
         width++) {
      GlobalAveragePoolingOperatorTester()
          .batchSize(1)
          .width(width)
...
TEST(GLOBAL_AVERAGE_POOLING_OP, unit_batch_few_channels) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = 1; channels < blacksmith_qnnp_params.q8gavgpool.nr;
       channels++) {
    for (size_t width = 1; width <= 2 * blacksmith_qnnp_params.q8gavgpool.nr;
         width++) {
      GlobalAveragePoolingOperatorTester()
          .batchSize(1)
          .width(width)
  ...
  for (size_t channels = blacksmith_qnnp_params.q8gavgpool.nr;
    ...
    for (size_t width = 1; width <= blacksmith_qnnp_params.q8gavgpool.mr;
      ...
      GlobalAveragePoolingOperatorTester()
          ...
          .width(width)
          .channels(channels)
          .outputStride(5 * blacksmith_qnnp_params.q8gavgpool.nr)
          .testQ8();
    }
  }
}
