...
TEST(PRECISE__SCALAR_UNSIGNED32, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__scalar_unsigned32);
  }
}
...
TEST(PRECISE__SCALAR_UNSIGNED64, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__scalar_unsigned64);
  }
}
...
TEST(PRECISE__SCALAR_SIGNED64, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__scalar_signed64);
  }
}
...
TEST(FP32__SCALAR_LRINTF, random_cases) {
  RequantizationTester().iterations(1000).testRandomCasesApproximate(
      blacksmith_qnnp_requantize_fp32__scalar_lrintf);
}
...
TEST(FP32__SCALAR_MAGIC, random_cases) {
  RequantizationTester().iterations(1000).testRandomCasesApproximate(
      blacksmith_qnnp_requantize_fp32__scalar_magic);
}
...
TEST(Q31__SCALAR, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_q31__scalar);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_q31__scalar);
    }
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingAway(blacksmith_qnnp_requantize_q31__scalar);
    }
  }
}

TEST(Q31__SCALAR, special_cases) {
  RequantizationTester().testSpecialCases(blacksmith_qnnp_requantize_q31__scalar);
}
...
TEST(Q31__SCALAR, random_match_gemmlowp) {
  RequantizationTester().iterations(100).testRandomCasesAgainstReference(
      blacksmith_qnnp_requantize_q31__scalar,
      blacksmith_qnnp_requantize_gemmlowp__scalar);
}
...
TEST(PRECISE__PSIMD, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__psimd);
  }
}
...
TEST(FP32__PSIMD, random_cases) {
  RequantizationTester().iterations(1000).testRandomCasesApproximate(
      blacksmith_qnnp_requantize_fp32__psimd);
}
...
TEST(PRECISE__SSE2, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__sse2);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_precise__sse2);
    }
  }
}
...
TEST(PRECISE__SSSE3, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__ssse3);
  }
}
...
TEST(PRECISE__SSE4, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__sse4);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_precise__sse4);
    }
  }
}
...
TEST(FP32__SSE2, random_cases) {
  RequantizationTester().iterations(1000).testRandomCasesApproximate(
      blacksmith_qnnp_requantize_fp32__sse2);
}
...
TEST(Q31__SSE2, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_q31__sse2);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_q31__sse2);
    }
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingAway(blacksmith_qnnp_requantize_q31__sse2);
    }
  }
}

TEST(Q31__SSE2, special_cases) {
  RequantizationTester().testSpecialCases(blacksmith_qnnp_requantize_q31__sse2);
}
...
TEST(Q31__SSE2, random_match_gemmlowp) {
  RequantizationTester().iterations(100).testRandomCasesAgainstReference(
      blacksmith_qnnp_requantize_q31__sse2,
      blacksmith_qnnp_requantize_gemmlowp__sse2);
}
...
TEST(Q31__SSSE3, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_q31__ssse3);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_q31__ssse3);
    }
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingAway(blacksmith_qnnp_requantize_q31__ssse3);
    }
  }
}

TEST(Q31__SSSE3, special_cases) {
  RequantizationTester().testSpecialCases(blacksmith_qnnp_requantize_q31__ssse3);
}
...
TEST(Q31__SSSE3, random_match_gemmlowp) {
  RequantizationTester().iterations(100).testRandomCasesAgainstReference(
      blacksmith_qnnp_requantize_q31__ssse3,
      blacksmith_qnnp_requantize_gemmlowp__ssse3);
}
...
TEST(Q31__SSE4, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_q31__sse4);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_q31__sse4);
    }
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingAway(blacksmith_qnnp_requantize_q31__sse4);
    }
  }
}

TEST(Q31__SSE4, special_cases) {
  RequantizationTester().testSpecialCases(blacksmith_qnnp_requantize_q31__sse4);
}
...
TEST(Q31__SSE4, random_match_gemmlowp) {
  RequantizationTester().iterations(100).testRandomCasesAgainstReference(
      blacksmith_qnnp_requantize_q31__sse4,
      blacksmith_qnnp_requantize_gemmlowp__sse4);
}
...
TEST(PRECISE__NEON, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_precise__neon);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_precise__neon);
    }
  }
}
...
TEST(FP32__NEON, random_cases) {
  RequantizationTester().iterations(1000).testRandomCasesApproximate(
      blacksmith_qnnp_requantize_fp32__neon);
}
...
TEST(Q31__NEON, exact_divide_by_po2) {
  for (uint32_t s = 1; s < 32; s++) {
    RequantizationTester().s(s).testExactDivideByPO2(
        blacksmith_qnnp_requantize_q31__neon);
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingUp(blacksmith_qnnp_requantize_q31__neon);
    }
  }
}
  ...
  for (int32_t zeroPoint = 0; zeroPoint < 256; zeroPoint++) {
    for (uint32_t s = 1; s < 32; s++) {
      RequantizationTester()
          .zeroPoint(zeroPoint)
          .s(s)
          .testDivideByPO2WithRoundingAway(blacksmith_qnnp_requantize_q31__neon);
    }
  }
}

TEST(Q31__NEON, special_cases) {
  RequantizationTester().testSpecialCases(blacksmith_qnnp_requantize_q31__neon);
}
...
TEST(Q31__NEON, random_match_gemmlowp) {
  RequantizationTester().iterations(100).testRandomCasesAgainstReference(
      blacksmith_qnnp_requantize_q31__neon,
      blacksmith_qnnp_requantize_gemmlowp__neon);
}
