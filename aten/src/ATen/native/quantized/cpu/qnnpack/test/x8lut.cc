...
TEST(X8LUT__SCALAR, n_eq_1) {
  LUTMicrokernelTester().n(1).test(blacksmith_x8lut_ukernel__scalar);
}

TEST(X8LUT__SCALAR, small_n) {
  for (size_t n = 2; n <= 16; n++) {
    LUTMicrokernelTester().n(n).test(blacksmith_x8lut_ukernel__scalar);
  }
}
...
TEST(X8LUT__SCALAR, n_eq_1_inplace) {
  LUTMicrokernelTester().n(1).inplace(true).test(blacksmith_x8lut_ukernel__scalar);
}

TEST(X8LUT__SCALAR, small_n_inplace) {
  for (size_t n = 2; n <= 16; n++) {
    LUTMicrokernelTester().n(n).inplace(true).test(blacksmith_x8lut_ukernel__scalar);
  }
}
