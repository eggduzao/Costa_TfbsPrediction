...
class AvgPoolMicrokernelTester {
 public:
  ...
  void test(blacksmith_q8avgpool_up_ukernel_function q8avgpool) const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < n(); i++) {
        for (size_t k = 0; k < kc(); k++) {
          ...
          yAcc[i * kc() + k] = acc;
          yRef[i * kc() + k] =
              blacksmith_qnnp_avgpool_quantize(acc, scalarQuantizationParams);
          yFP[i * kc() + k] =
  ...
  void test(blacksmith_q8avgpool_mp_ukernel_function q8avgpool) const {
    std::random_device randomDevice;
    auto rng = std::mt19937(randomDevice());
    auto u8rng = std::bind(std::uniform_int_distribution<uint8_t>(), rng);
