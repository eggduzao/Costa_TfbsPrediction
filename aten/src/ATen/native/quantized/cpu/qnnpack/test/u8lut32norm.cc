...
TEST(U8LUT32NORM__SCALAR, n_eq_1) {
  LUTNormMicrokernelTester().n(1).test(blacksmith_u8lut32norm_ukernel__scalar);
}

TEST(U8LUT32NORM__SCALAR, small_n) {
  for (size_t n = 2; n <= 16; n++) {
    LUTNormMicrokernelTester().n(n).test(blacksmith_u8lut32norm_ukernel__scalar);
  }
}
...
TEST(U8LUT32NORM__SCALAR, n_eq_1_inplace) {
  LUTNormMicrokernelTester().n(1).inplace(true).test(
      blacksmith_u8lut32norm_ukernel__scalar);
}
