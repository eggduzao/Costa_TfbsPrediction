...
class GemmMicrokernelTester {
 public:
  ...
  void test(blacksmith_q8gemm_ukernel_function qgemm) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const union blacksmith_qnnp_conv_quantization_params quantizationParams =
          blacksmith_qnnp_compute_conv_quantization_params(
              ...
              qmin(),
              qmax());
      const union blacksmith_qnnp_fp32_requantization_params
          scalarRequantizationParams =
              blacksmith_qnnp_compute_scalar_fp32_requantization_params(
      ...
      for (size_t mIndex = 0; mIndex < m(); mIndex++) {
        for (size_t nIndex = 0; nIndex < n(); nIndex++) {
          ...
          cRef[mIndex * n() + nIndex] = blacksmith_qnnp_fp32_requantize_magic(
              acc[mIndex * n() + nIndex], scalarRequantizationParams, nIndex);
#else
          cRef[mIndex * n() + nIndex] = blacksmith_qnnp_fp32_requantize(
              acc[mIndex * n() + nIndex], scalarRequantizationParams, nIndex);
#endif
        }
      }
  ...
  void test(blacksmith_q8gemm_dq_ukernel_function qgemm) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const struct blacksmith_qnnp_conv_dynamic_quantization_params quantizationParams{
        aZeroPoint(),
        kernel_zero_points.data(),
  ...
  void test(blacksmith_q8conv_ukernel_function qconv) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
  ...
  static void q8gemm_compute_row_sum(
      ...
      int32_t* row_sum,
      blacksmith_q8sum_rows_ukernel_function q8sum_rows) {
  ...
  void test(blacksmith_q8gemm_xzp_ukernel_function qgemm) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const union blacksmith_qnnp_q31_requantization_params requantizationParams =
      ...
      const union blacksmith_qnnp_q31_requantization_params
          scalarRequantizationParams =
              blacksmith_qnnp_compute_scalar_requantization_params(
                  requantizationScale, cZeroPoint, qmin(), qmax());
      ...
      for (size_t mIndex = 0; mIndex < m(); mIndex++) {
        for (size_t nIndex = 0; nIndex < n(); nIndex++) {
          cRef[mIndex * n() + nIndex] = blacksmith_qnnp_q31_requantize(
  ...
  void test(blacksmith_hgemm_ukernel_function hgemm) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ASSERT_GE(aStride(), k());
    ...
    struct blacksmith_qnnp_fp16_clamping_params clampingParams;
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      std::fill(packedW.begin(), packedW.end(), 0);
      blacksmith_pack_hgemm_w(n(), k(), np(), kr(), b.data(), bias.data(), packedW.data());
  ...
  void test(blacksmith_sgemm_ukernel_function sgemm) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ASSERT_GE(aStride(), k());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      blacksmith_pack_sgemm_w(n(), k(), np(), kr(), b.data(), bias.data(), packedW.data());
      ...
      const float cMax =
          accMax - (accMax - accMin) / 255.0f * float(255 - qmax());
      struct blacksmith_qnnp_fp32_clamping_params clampingParams = {
          .max = cMax,
          .min = cMin,
      };
  ...
  void test(blacksmith_sconv_ukernel_function sconv) const {
    ASSERT_LE(m(), mr());
    ASSERT_LE(n(), nr());
    ASSERT_GE(k(), kr());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const struct blacksmith_qnnp_fp32_clamping_params clampingParams {
        cRefMax, cRefMin
      };
