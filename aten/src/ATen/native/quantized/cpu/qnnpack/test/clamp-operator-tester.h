 ...
 public:
  ...
  void testU8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      blacksmith_qnnp_operator_t clampOp = nullptr;

      ASSERT_EQ(
          blacksmith_qnnp_status_success,
          blacksmith_qnnp_create_clamp_nc_u8(
              channels(), qmin(), qmax(), 0, &clampOp));
      ASSERT_NE(nullptr, clampOp);

      ASSERT_EQ(
          blacksmith_qnnp_status_success,
          blacksmith_qnnp_setup_clamp_nc_u8(
              clampOp,
              batchSize(),
              input.data(),
              inputStride(),
      ...
      ASSERT_EQ(
          blacksmith_qnnp_status_success,
          blacksmith_qnnp_run_operator(clampOp, nullptr /* thread pool */));

      ASSERT_EQ(
          blacksmith_qnnp_status_success, blacksmith_qnnp_delete_operator(clampOp));
      clampOp = nullptr;
