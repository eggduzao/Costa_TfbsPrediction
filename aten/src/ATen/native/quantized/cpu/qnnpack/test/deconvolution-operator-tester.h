 ...
 public:
  ...
  void testQ8(const qnnpack::testing::Mode mode = qnnpack::testing::Mode::Static) const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      blacksmith_qnnp_operator_t deconvolution = nullptr;

      ASSERT_EQ(
          blacksmith_qnnp_status_success,
          blacksmith_qnnp_create_deconvolution2d_nhwc_q8(
              paddingHeight(),
              paddingWidth(),
              adjustmentHeight(),
              adjustmentWidth(),
      ...
      switch (mode) {
        case qnnpack::testing::Mode::Static: {
          ASSERT_EQ(
              blacksmith_qnnp_status_success,
              blacksmith_qnnp_setup_deconvolution2d_nhwc_q8(
                  deconvolution,
                  batchSize(),
                  inputHeight(),
                  inputWidth(),
          ...
          ASSERT_EQ(
              blacksmith_qnnp_status_success,
              blacksmith_qnnp_run_operator(deconvolution, nullptr /* thread pool */));
