...
TEST(AVERAGE_POOLING_OP, unit_batch_many_channels_small_1xM_pool) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = blacksmith_qnnp_params.q8avgpool.kr;
       channels <= 3 * blacksmith_qnnp_params.q8avgpool.kr;
       channels++) {
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.q8avgpool.mr;
         poolSize++) {
      AveragePoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(AVERAGE_POOLING_OP, unit_batch_many_channels_small_1xM_pool_with_padding) {
  ...
  for (size_t channels = blacksmith_qnnp_params.q8avgpool.kr;
       channels <= 3 * blacksmith_qnnp_params.q8avgpool.kr;
       channels += 3) {
    for (size_t poolSize = 3; poolSize <= blacksmith_qnnp_params.q8avgpool.mr;
         poolSize++) {
  ...
  for (size_t channels = blacksmith_qnnp_params.q8avgpool.kr;
    ...
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.q8avgpool.mr;
      ...
      AveragePoolingOperatorTester()
          ...
          .poolingWidth(1)
          .channels(channels)
          .inputPixelStride(5 * blacksmith_qnnp_params.q8avgpool.kr)
          .testQ8();
      AveragePoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
  ...
  for (size_t channels = blacksmith_qnnp_params.q8avgpool.kr;
    ...
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.q8avgpool.mr;
      ...
      AveragePoolingOperatorTester()
          ...
          .poolingWidth(1)
          .channels(channels)
          .outputPixelStride(5 * blacksmith_qnnp_params.q8avgpool.kr)
          .testQ8();
      AveragePoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(AVERAGE_POOLING_OP, unit_batch_many_channels_large_1xM_pool) {
  ...
  for (size_t channels = blacksmith_qnnp_params.q8avgpool.kr;
       channels <= 3 * blacksmith_qnnp_params.q8avgpool.kr;
       channels++) {
    for (size_t poolSize = blacksmith_qnnp_params.q8avgpool.mr + 1; poolSize <=
         blacksmith_qnnp_params.q8avgpool.mr + blacksmith_qnnp_params.q8avgpool.qr;
         poolSize++) {
      AveragePoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(AVERAGE_POOLING_OP, unit_batch_few_channels_1xM_pool) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = 1; channels < blacksmith_qnnp_params.q8avgpool.kr;
       channels++) {
    for (size_t poolSize = 2; poolSize <= 2 * blacksmith_qnnp_params.q8avgpool.kr;
         poolSize++) {
      AveragePoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
