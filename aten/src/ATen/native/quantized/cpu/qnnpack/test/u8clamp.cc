...
TEST(U8CLAMP__NEON, n_eq_8) {
  ...
  ClampMicrokernelTester().n(8).test(blacksmith_u8clamp_ukernel__neon);
}

TEST(U8CLAMP__NEON, n_div_8) {
  TEST_REQUIRES_ARM_NEON;
  for (size_t n = 8; n < 512; n += 8) {
    ClampMicrokernelTester().n(n).test(blacksmith_u8clamp_ukernel__neon);
  }
}
...
TEST(U8CLAMP__NEON, inplace) {
  ...
  for (size_t n = 1; n < 128; n += 5) {
    ClampMicrokernelTester().iterations(1).n(n).inplace(true).test(
        blacksmith_u8clamp_ukernel__neon);
  }
}
...
TEST(U8CLAMP__SSE2, n_eq_8) {
  ...
  ClampMicrokernelTester().n(8).test(blacksmith_u8clamp_ukernel__sse2);
}

TEST(U8CLAMP__SSE2, n_div_8) {
  TEST_REQUIRES_X86_SSE2;
  for (size_t n = 8; n < 512; n += 8) {
    ClampMicrokernelTester().n(n).test(blacksmith_u8clamp_ukernel__sse2);
  }
}
...
TEST(U8CLAMP__SSE2, inplace) {
  ...
  for (size_t n = 1; n < 128; n += 5) {
    ClampMicrokernelTester().iterations(1).n(n).inplace(true).test(
        blacksmith_u8clamp_ukernel__sse2);
  }
}
