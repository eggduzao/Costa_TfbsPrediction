...
class DWConvMicrokernelTester {
 public:
  ...
  void test(
      blacksmith_q8dwconv2d_up_ukernel_function q8dwconv,
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t x = 0; x < width(); x++) {
        for (size_t c = 0; c < channels(); c++) {
          ...
          const uint8_t referenceOutput = blacksmith_qnnp_fp32_requantize_magic(
              accumulators[x * channels() + c], scalarRequantizationParams, c);
#else
          const uint8_t referenceOutput = blacksmith_qnnp_fp32_requantize(
              accumulators[x * channels() + c], scalarRequantizationParams, c);
#endif
  ...
  void test(
      blacksmith_q8dwconv2d_mp_ukernel_function q8dwconv,
      bool per_channel = false) const {
