...
TEST(MAX_POOLING_OP, zero_batch) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
...
TEST(MAX_POOLING_OP, unit_batch_many_channels_small_1xM_pool) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
       channels <= 3 * blacksmith_qnnp_params.u8maxpool.kr;
       channels++) {
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.u8maxpool.mr;
         poolSize++) {
      MaxPoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(MAX_POOLING_OP, unit_batch_many_channels_small_1xM_pool_with_padding) {
  ...
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
       channels <= 3 * blacksmith_qnnp_params.u8maxpool.kr;
       channels += 3) {
    for (size_t poolSize = 3; poolSize <= blacksmith_qnnp_params.u8maxpool.mr;
         poolSize++) {
  ...
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
    ...
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.u8maxpool.mr;
      ...
      MaxPoolingOperatorTester()
          ...
          .poolingWidth(1)
          .channels(channels)
          .inputPixelStride(5 * blacksmith_qnnp_params.u8maxpool.kr)
          .testU8();
      MaxPoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
  ...
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
    ...
    for (size_t poolSize = 2; poolSize <= blacksmith_qnnp_params.u8maxpool.mr;
      ...
      MaxPoolingOperatorTester()
          ...
          .poolingWidth(1)
          .channels(channels)
          .outputPixelStride(5 * blacksmith_qnnp_params.u8maxpool.kr)
          .testU8();
      MaxPoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(MAX_POOLING_OP, unit_batch_many_channels_large_1xM_pool) {
  ...
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
       channels <= 3 * blacksmith_qnnp_params.u8maxpool.kr;
       channels++) {
    for (size_t poolSize = blacksmith_qnnp_params.u8maxpool.mr; poolSize <=
         blacksmith_qnnp_params.u8maxpool.mr + blacksmith_qnnp_params.u8maxpool.qr;
         poolSize++) {
      MaxPoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(MAX_POOLING_OP, unit_batch_few_channels_1xM_pool) {
  ASSERT_EQ(blacksmith_qnnp_status_success, blacksmith_qnnp_initialize());
  for (size_t channels = 1; channels < blacksmith_qnnp_params.u8maxpool.kr;
       channels++) {
    for (size_t poolSize = 2; poolSize <= 2 * blacksmith_qnnp_params.u8maxpool.kr;
         poolSize++) {
      MaxPoolingOperatorTester()
          .batchSize(1)
          .inputHeight(2)
...
TEST(MAX_POOLING_OP, small_batch_many_channels_large_pool) {
  ...
  for (size_t channels = blacksmith_qnnp_params.u8maxpool.kr;
       channels <= 3 * blacksmith_qnnp_params.u8maxpool.kr;
       channels++) {
    for (size_t poolSize = blacksmith_qnnp_params.u8maxpool.mr + 1; poolSize <=
         blacksmith_qnnp_params.u8maxpool.mr + blacksmith_qnnp_params.u8maxpool.qr;
         poolSize++) {
      MaxPoolingOperatorTester()
          .batchSize(3)
