...
TEST(U8RMAX__NEON, n_lt_16) {
  ...
  for (size_t n = 1; n < 16; n++) {
    RMaxMicrokernelTester().n(n).test(blacksmith_u8rmax_ukernel__neon);
  }
}

TEST(U8RMAX__NEON, n_eq_16) {
  TEST_REQUIRES_ARM_NEON;
  RMaxMicrokernelTester().n(16).test(blacksmith_u8rmax_ukernel__neon);
}
...
TEST(U8RMAX__SSE2, n_lt_16) {
  ...
  for (size_t n = 1; n < 16; n++) {
    RMaxMicrokernelTester().n(n).test(blacksmith_u8rmax_ukernel__sse2);
  }
}

TEST(U8RMAX__SSE2, n_eq_16) {
  TEST_REQUIRES_X86_SSE2;
  RMaxMicrokernelTester().n(16).test(blacksmith_u8rmax_ukernel__sse2);
}
