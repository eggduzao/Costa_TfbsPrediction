...
class GAvgPoolMicrokernelTester {
 public:
  ...
  void test(blacksmith_q8gavgpool_up_ukernel_function q8gavgpool) const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const union blacksmith_qnnp_avgpool_quantization_params quantizationParams =
      ...
      for (size_t j = 0; j < n(); j++) {
        ...
        yAcc[j] = acc;
        yRef[j] = blacksmith_qnnp_avgpool_quantize(acc, scalarQuantizationParams);
  ...
  void test(blacksmith_q8gavgpool_mp_ukernel_function q8gavgpool) const {
    std::random_device randomDevice;
    auto rng = std::mt19937(randomDevice());
    auto u8rng = std::bind(std::uniform_int_distribution<uint8_t>(), rng);
