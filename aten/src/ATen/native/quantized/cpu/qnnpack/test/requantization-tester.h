...
class RequantizationTester {
 public:
   ...
   * overflow.
   */
  void testExactDivideByPO2(blacksmith_requantization_function requantize) const {
   ...
   * overflow.
   */
  void testDivideByPO2WithRoundingUp(blacksmith_requantization_function requantize) {
    ASSERT_GE(zeroPoint(), 0);
    ASSERT_LE(zeroPoint(), 255);
   ...
   * overflow.
   */
  void testDivideByPO2WithRoundingDown(blacksmith_requantization_function requantize) {
    ASSERT_GE(zeroPoint(), 0);
    ASSERT_LE(zeroPoint(), 255);
  ...
  void testDivideByPO2WithRoundingAway(blacksmith_requantization_function requantize) {
    ASSERT_GE(zeroPoint(), 0);
    ASSERT_LE(zeroPoint(), 255);
  ...
  void testSpecialCases(blacksmith_requantization_function requantize) {
    std::vector<int32_t> inputs(256);
    std::vector<uint8_t> outputs(inputs.size());
  ...
  void testRandomCasesPrecise(blacksmith_requantization_function requantize) {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < inputs.size(); i++) {
        const uint8_t referenceOutput = blacksmith_scalar_requantize_precise(
            inputs[i],
            scale,
            zeroPoint,
            std::numeric_limits<uint8_t>::min(),
  ...
  void testRandomCasesApproximate(blacksmith_requantization_function requantize) {
    std::random_device randomDevice;
    std::mt19937 mtRng(randomDevice());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      auto rng = std::bind(std::uniform_int_distribution<uint8_t>(), mtRng);
  ...
  void testRandomCasesAgainstReference(
      blacksmith_requantization_function requantize,
      blacksmith_requantization_function requantizeReference) {
    std::random_device randomDevice;
    std::mt19937 mtRng(randomDevice());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      auto rng = std::bind(std::uniform_int_distribution<uint8_t>(), mtRng);
