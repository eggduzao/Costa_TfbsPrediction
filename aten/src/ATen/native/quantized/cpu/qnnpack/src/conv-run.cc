...
struct QnnpackDeleter {
  void operator()(blacksmith_qnnp_operator_t op) {
    blacksmith_qnnp_delete_operator(op);
  }
};

enum blacksmith_qnnp_status qnnpackConv(
    const blacksmith_qnnp_operator_t convolution,
    void* packed_weights,
  ...
  union blacksmith_qnnp_q31_requantization_params requantization_params {};
  union blacksmith_qnnp_conv_quantization_params conv_quantization_params {};
  if (convolution->ukernel_type == blacksmith_qnnp_ukernel_type_xzp_gemm) {
    requantization_params = blacksmith_qnnp_compute_requantization_params(
    ...
    conv_quantization_params = blacksmith_qnnp_compute_conv_quantization_params(
        input_zero_point,
        kernel_zero_points,
  ...
  if (convolution->input != input || convolution->batch_size != batch_size ||
    ...
    blacksmith_qnnp_status status = blacksmith_qnnp_setup_convolution_ndhwc_q8(
        convolution,
        batch_size,
        input_depth,
        input_height,
        ...
        output_pixel_stride,
        threadpool);
    if (status != blacksmith_qnnp_status_success) {
  ...
  switch (convolution->ukernel_type) {
    ...
    default: {
      blacksmith_qnnp_log_error("Invalid kernel type. QNNPACK convolution run failed.");
      BLACKSMITH_QNNP_UNREACHABLE;
    }
  }
  return blacksmith_qnnp_status_success;
