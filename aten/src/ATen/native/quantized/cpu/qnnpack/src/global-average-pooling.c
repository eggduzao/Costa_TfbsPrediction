...
enum blacksmith_qnnp_status blacksmith_qnnp_create_global_average_pooling_nwc_q8(
    ...
    uint8_t output_max,
    uint32_t flags,
    blacksmith_qnnp_operator_t* global_average_pooling_out) {
  blacksmith_qnnp_operator_t global_average_pooling_op = NULL;
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_create_global_average_pooling_nwc_q8 failed because QNNPACK is not properly initialized");
    goto error;
  }
  ...
  global_average_pooling_op = calloc(1, sizeof(struct blacksmith_qnnp_operator));
  ...
  global_average_pooling_op->format = blacksmith_qnnp_format_quint8;

  *global_average_pooling_out = global_average_pooling_op;
  return blacksmith_qnnp_status_success;

error:
  blacksmith_qnnp_delete_operator(global_average_pooling_op);
  return status;
}

enum blacksmith_qnnp_status blacksmith_qnnp_setup_global_average_pooling_nwc_q8(
    blacksmith_qnnp_operator_t global_average_pooling_op,
    size_t batch_size,
    size_t width,
    const uint8_t* input,
    size_t input_stride,
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_setup_global_average_pooling_nwc_q8 failed because QNNPACK is not properly initialized");
