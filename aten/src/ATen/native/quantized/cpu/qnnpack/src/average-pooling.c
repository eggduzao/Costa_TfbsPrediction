...
enum blacksmith_qnnp_status blacksmith_qnnp_create_average_pooling2d_nhwc_q8(
    ...
    uint8_t output_max,
    uint32_t flags,
    blacksmith_qnnp_operator_t* average_pooling_out) {
  blacksmith_qnnp_operator_t average_pooling = NULL;
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_create_average_pooling2d_nhwc_q8 failed because QNNPACK is not properly initialized");
    goto error;
  }
  ...
  average_pooling = calloc(1, sizeof(struct blacksmith_qnnp_operator));
  ...
  if (channels >= blacksmith_qnnp_params.q8avgpool.kr) {
    if (nrows <= mr) {
      nrows = mr;
    } else {
  ...
  average_pooling->ukernel_type = blacksmith_qnnp_ukernel_type_average_pooling;
  average_pooling->format = blacksmith_qnnp_format_quint8;

  *average_pooling_out = average_pooling;
  return blacksmith_qnnp_status_success;

error:
  blacksmith_qnnp_delete_operator(average_pooling);
  return status;
}
...
enum blacksmith_qnnp_status blacksmith_qnnp_setup_average_pooling2d_nhwc_q8(
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_setup_average_pooling2d_nhwc_q8 failed because QNNPACK is not properly initialized");
  ...
  blacksmith_qnnp_indirection_set_step_dimensions(average_pooling);
  ...
  blacksmith_qnnp_indirection_init_dwconv(average_pooling, valid_batch_size);
