...
PrePackConvWeights::PrePackConvWeights(
    ...
    const int32_t* bias) {
  enum blacksmith_qnnp_ukernel_type ukernel_type = convolution->ukernel_type;
  ...
  if (convolution->transpose &&
    ...
    blacksmith_qnnp_log_error("Wrong micro-kernel for deconvolution");
  ...
  switch (ukernel_type) {
    ...
    case blacksmith_qnnp_ukernel_type_xzp_gemm: {
      ...
      for (uint32_t group = 0; group < groups; group++) {
        blacksmith_pack_swizzle_q8gemm_brq(
    ...
    case blacksmith_qnnp_ukernel_type_conv: {
      ...
      switch (ukernel_type) {
        case blacksmith_qnnp_ukernel_type_gemm:
          ...
          break;
        case blacksmith_qnnp_ukernel_type_conv:  // The transpose can only be here
          for (uint32_t group = 0; group < groups; group++) {
