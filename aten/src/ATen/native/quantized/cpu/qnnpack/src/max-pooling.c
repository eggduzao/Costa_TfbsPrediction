...
enum blacksmith_qnnp_status blacksmith_qnnp_create_max_pooling2d_nhwc_u8(
    ...
    uint8_t output_max,
    uint32_t flags,
    blacksmith_qnnp_operator_t* max_pooling_out) {
  blacksmith_qnnp_operator_t max_pooling = NULL;
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_create_max_pooling2d_nhwc_u8 failed because QNNPACK is not properly initialized");
    goto error;
  }
  ...
  max_pooling = calloc(1, sizeof(struct blacksmith_qnnp_operator));
  if (max_pooling == NULL) {
    blacksmith_qnnp_log_error(
  ...
  max_pooling->ukernel_type = blacksmith_qnnp_ukernel_type_max_pooling;
  max_pooling->format = blacksmith_qnnp_format_quint8;

  *max_pooling_out = max_pooling;
  return blacksmith_qnnp_status_success;

error:
  blacksmith_qnnp_delete_operator(max_pooling);
  return status;
}
...
enum blacksmith_qnnp_status blacksmith_qnnp_setup_max_pooling2d_nhwc_u8(
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_setup_max_pooling2d_nhwc_u8 failed because QNNPACK is not properly initialized");
  ...
  blacksmith_qnnp_indirection_set_step_dimensions(max_pooling);
  ...
  blacksmith_qnnp_indirection_init_maxpool2d(max_pooling, valid_batch_size);
