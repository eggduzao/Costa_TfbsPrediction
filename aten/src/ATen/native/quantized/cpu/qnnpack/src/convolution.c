 ...
 */
static enum blacksmith_qnnp_status blacksmith_qnnp_create_convolution_ndhwc_q8(
    ...
    bool per_channel,
    blacksmith_qnnp_operator_t* convolution_out,
    bool is_2d /* true: 2d, false: 3d */) {
  blacksmith_qnnp_operator_t convolution = NULL;
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_create_convolution2d_nhwc_q8 failed because QNNPACK is not properly initialized");
    goto error;
  }
  ...
  convolution = calloc(1, sizeof(struct blacksmith_qnnp_operator));
  if (convolution == NULL) {
    blacksmith_qnnp_log_error(
  ...
  enum blacksmith_qnnp_ukernel_type ukernel_type = blacksmith_qnnp_ukernel_type_none;
  ...
  if (has_depthwise_dimensions && has_depthwise_grouping) {
    ukernel_type = blacksmith_qnnp_ukernel_type_dwconv;
  } else if (
      kernel_size == 1 && subsampling_height == 1 && subsampling_width == 1 &&
      !any_padding) {
    ukernel_type =
        group_input_channels >= blacksmith_qnnp_params.q8conv_xzp.kthreshold
        ? blacksmith_qnnp_ukernel_type_xzp_gemm
        : blacksmith_qnnp_ukernel_type_gemm;
  } else {
    ukernel_type = blacksmith_qnnp_ukernel_type_conv;
  }
  ...
  switch (ukernel_type) {
    ...
    case blacksmith_qnnp_ukernel_type_dwconv: {
      const uint32_t cr = blacksmith_qnnp_params.q8dw9.cr;
      ...
      switch (kernel_size) {
        ...
        case 25:
          /* change this later */
          blacksmith_pack_q8dw_2d_w_dilation(
              kernel_height,
              kernel_width,
              groups,
              cr,
          ...
          break;
        case 27:
          blacksmith_pack_q8dw_3d_w_dilation(
              kernel_depth,
              kernel_height,
              kernel_width,
              groups,
    ...
    case blacksmith_qnnp_ukernel_type_xzp_gemm: {
      ...
      const uint32_t sr = blacksmith_qnnp_params.q8conv_xzp.kc;
      ...
      for (uint32_t group = 0; group < groups; group++) {
        blacksmith_pack_swizzle_q8gemm_b(
  ...
  if (ukernel_type == blacksmith_qnnp_ukernel_type_xzp_gemm) {
    convolution->requantization_params =
        blacksmith_qnnp_compute_requantization_params(
  ...
  convolution->format = blacksmith_qnnp_format_quint8;
...
error:
  blacksmith_qnnp_delete_operator(convolution);
  return status;
}
...
enum blacksmith_qnnp_status blacksmith_qnnp_create_convolution2d_nhwc_q8(
    ...
    bool per_channel,
    blacksmith_qnnp_operator_t* convolution_out) {
  return blacksmith_qnnp_create_convolution_ndhwc_q8(
      0,
      input_padding_height,
      input_padding_width,
      1,
...
enum blacksmith_qnnp_status blacksmith_qnnp_setup_convolution2d_nhwc_q8(
  ...
  return blacksmith_qnnp_setup_convolution_ndhwc_q8(
      convolution,
      batch_size,
      1,
      input_height,
...
enum blacksmith_qnnp_status blacksmith_qnnp_setup_convolution_ndhwc_q8(
  ...
  if (!blacksmith_qnnp_params.initialized) {
    blacksmith_qnnp_log_error(
        "blacksmith_qnnp_setup_convolution_ndhwc_q8 failed because QNNPACK is not properly initialized");
  ...
  switch (convolution->ukernel_type) {
    ...
    case blacksmith_qnnp_ukernel_type_conv: {
      ...
      const size_t output_tile_size = blacksmith_qnnp_params.q8conv.mr;
      ...
      blacksmith_qnnp_indirection_init_conv3d(
          convolution, output_tile_size, tiled_output_size);
      return blacksmith_qnnp_status_success;
    }
    case blacksmith_qnnp_ukernel_type_dwconv: {
      blacksmith_qnnp_indirection_set_step_dimensions(convolution);
      ...
      blacksmith_qnnp_indirection_init_dwconv(convolution, 0);
      return blacksmith_qnnp_status_success;
    }
    default:
      BLACKSMITH_QNNP_UNREACHABLE;
