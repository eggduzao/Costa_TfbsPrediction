...
inline TensorQuantizationParams ChooseQuantizationParams(
  ...
  SMITH_CHECK(scale > 0, "quantization scale should be > 0");
...
[[maybe_unused]] static smith::List<int64_t> MakeArgForConv1d(
    const smith::List<int64_t>& arg,
    int64_t base_value) {
  SMITH_CHECK(!arg.empty(), "Argument must have elements.");
  smith::List<int64_t> result({arg.get(0), base_value});
  if (arg.size() == 1) {
    result[1] = arg.get(0);
  } else {
    result[1] = arg.get(1);
