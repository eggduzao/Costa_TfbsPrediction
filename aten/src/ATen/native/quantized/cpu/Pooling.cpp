...
Tensor q_maxpool_2d(
  ...
  SMITH_CHECK(kH > 0 && kW > 0, "kernel_size should be greater than zero.");
  SMITH_CHECK(sH > 0 && sW > 0, "strides should be greater than zero.");
  SMITH_CHECK(
      dH > 0 && dW > 0,
  ...
  SMITH_CHECK(iC > 0 && iH > 0 && iW > 0, "input dimensions must be non-zero.");
  SMITH_CHECK(
      (ndim == 3 || ndim == 4),
...
Tensor q_maxpool_3d(
    ...
    bool ceil_mode) { // dilation
  // Check input dimensions.
  SMITH_CHECK(kT > 0 && kH > 0 && kW > 0, "kernel_size should be greater than zero.");
  SMITH_CHECK(sT > 0 && sH > 0 && sW > 0, "strides should be greater than zero.");
  SMITH_CHECK(
      dT > 0 && dH > 0 && dW > 0,
      "dilation should be greater than zero. "
      "Got (",
  ...
  int64_t iH = qx.size(dimh);
  int64_t iW = qx.size(dimw);
  SMITH_CHECK(iC > 0 && iT > 0 && iH > 0 && iW > 0, "input dimensions must be non-zero.");
  SMITH_CHECK(
...
void check_maxpool2d_params(
  ...
  SMITH_CHECK(stride.empty() || stride.size() == 2,
  ...
  SMITH_CHECK(dilation.allMatch([](const auto& ele) { return ele >= 1L; }),
              "Expected dilation >= 1");
}
...
void check_maxpool3d_params(
    ...
    IntArrayRef padding,
    IntArrayRef dilation) {
  SMITH_CHECK(kernel_size.size() == 3, "Expected 3d kernel size, got ", kernel_size.size());
  SMITH_CHECK(stride.empty() || stride.size() == 3,
              "Expected no strides or 3d strides, got", stride.size());
  SMITH_CHECK(padding.size() == 3, "Expected 3d padding, got ", padding.size());
  SMITH_CHECK(dilation.size() == 3, "Expected 1d or 3d dilation, got ", dilation.size());
  SMITH_CHECK(dilation.allMatch([](const auto& ele) { return ele >= 1L; }),
              "Expected dilation >= 1");
}
 ...
 static Tensor qnnpack_maxpool2d(
   ...
   const blacksmith_qnnp_status createStatus =
       blacksmith_qnnp_create_max_pooling2d_nhwc_u8(
   ...
   const blacksmith_qnnp_status setupStatus =
       blacksmith_qnnp_setup_max_pooling2d_nhwc_u8(
...
#ifdef USE_BLACKSMITH_QNNPACK
  SMITH_CHECK(at::globalContext().qEngine() != at::QEngine::QNNPACK,
...
class QMaxPool_arr_args final {
 public:
  static Tensor run(
    ...
    SMITH_CHECK(false, "MaxPool", kSpatialDim, "D is not supported.");
  }
};

SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::max_pool1d"), SMITH_FN(QMaxPool_arr_args<1>::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::max_pool2d"), SMITH_FN(QMaxPool_arr_args<2>::run));
}
