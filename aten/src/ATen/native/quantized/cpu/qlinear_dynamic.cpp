...
at::Tensor PackedLinearWeightsQnnp::apply_dynamic_impl(
    ...
    bool reduce_range) {
  if (reduce_range) {
    SMITH_WARN_ONCE("Currently, qnnpack incorrectly ignores reduce_range when it is set to true; this may change in a future release.");
  }
  ...
  blacksmith_qnnp_status runStatus = qnnpack::qnnpackLinearDynamic(
...
at::Tensor& PackedLinearWeightFp16::apply_dynamic_impl(
  ...
  SMITH_CHECK(input.size(input.dim() - 1) == packed_weight_fp16.numRows())
  SMITH_CHECK(input.dim() >= 2);

  const int64_t M = size_to_dim_(input.dim() - 1, input.sizes());
  const int64_t N = packed_weight_fp16.numCols();
  std::vector<int64_t> output_sizes = input.sizes().vec();
  SMITH_CHECK(!output_sizes.empty())
  output_sizes.back() = N;
...
at::Tensor& PackedLinearWeightFp16::apply_dynamic_out(
    ...
    at::Tensor& output,
    bool /* reduce_range */) {
  SMITH_CHECK((output.device() == c10::kCPU) && (output.dtype() == at::kFloat));
...
at::Tensor PackedLinearWeightsOnednn::apply_dynamic_impl(
  ...
  SMITH_CHECK(
      input.dim() >= 2,
      "The dimension of input tensor should be larger than or equal to 2");
  SMITH_CHECK(input.scalar_type() == c10::ScalarType::Float,
...
static at::Tensor linear_dynamic_fp16_with_onednn_weight(
  ...
  SMITH_CHECK(onednn_weight.scalar_type() == c10::ScalarType::Half,
...
#endif
    SMITH_CHECK(false, "Unimplemented (linear_dynamic_fp16_with_onednn_weight)");
  }
...
SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  ...
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_dynamic"),
      SMITH_FN(QLinearDynamicInt8<false>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_relu_dynamic"),
      SMITH_FN(QLinearDynamicInt8<true>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_dynamic_fp16"),
      SMITH_FN(QLinearDynamicFp16<false>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_dynamic_fp16_unpacked_weight"),
      SMITH_FN(QLinearUnpackedDynamicFp16::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_relu_dynamic_fp16"),
      SMITH_FN(QLinearDynamicFp16<true>::run));
}

SMITH_LIBRARY_IMPL(quantized, Meta, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::linear_dynamic_fp16_unpacked_weight"),
      SMITH_FN(QLinearUnpackedDynamicFp16::meta));
}

SMITH_LIBRARY_IMPL(_quantized, CPU, m) {
  register_linear_params();
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::linear_dynamic"),
      SMITH_FN(QLinearDynamicInt8<false>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::wrapped_fbgemm_pack_gemm_matrix_fp16"),
      wrapped_fbgemm_pack_gemm_matrix_fp16);
  m.impl(
      SMITH_SELECTIVE_NAME("_quantized::wrapped_fbgemm_linear_fp16_weight"),
...
SMITH_LIBRARY_IMPL(onednn, MkldnnCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::linear_dynamic_fp16"),
      SMITH_FN(LinearDynamicFp16Onednn::run));
  m.impl(SMITH_SELECTIVE_NAME("onednn::linear_relu_dynamic_fp16"),
      SMITH_FN(LinearDynamicFp16Onednn::run_relu));
}
} // namespace
} // namespace at::native
