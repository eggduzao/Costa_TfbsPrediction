...
static Tensor qnnpack_sigmoid(
  ...
  SMITH_CHECK(input.ndimension() > 0, "qnnpack_sigmoid(): Got empty input tensor");
  ...
  blacksmith_qnnp_operator_t sigmoid_op{nullptr};
  const blacksmith_qnnp_status createStatus = blacksmith_qnnp_create_sigmoid_nc_q8(
    num_elems /* channels */,
  ...
  const blacksmith_qnnp_status setupStatus = blacksmith_qnnp_setup_sigmoid_nc_q8(
    sigmoid_op,
  ...
  const blacksmith_qnnp_status runStatus =
    blacksmith_qnnp_run_operator(sigmoid_op, threadpool);
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::sigmoid"), SMITH_FN(QSigmoid::run));
}
} // namespace
