...
// For Blacksmith Mobile, once the model is scripted and serialized we don't need
...
// with input scale which is available at runtime in blacksmith. During runtime if
...
struct PackedConvWeightsQnnp : public ConvPackedParamsBase<kSpatialDim> {
  PackedConvWeightsQnnp(
    ...
    blacksmith_qnnp_ukernel_type ukernel_type;
    if (transpose_) {
      ukernel_type = blacksmith_qnnp_ukernel_type_conv;
    } else {
      ukernel_type = blacksmith_qnnp_ukernel_type_none;
    ...
    if (is_per_channel && ukernel_type == blacksmith_qnnp_ukernel_type_xzp_gemm) {
      SMITH_INTERNAL_ASSERT(
          false, "Per channel quantized weights are not supported for XZP kernels");
    }

    blacksmith_qnnp_operator_t convolution{nullptr};
    // Initially all the params are set to zero.
    convolution = static_cast<blacksmith_qnnp_operator_t>(
        calloc(1, sizeof(struct blacksmith_qnnp_operator)));
    if (convolution == nullptr) {
      SMITH_INTERNAL_ASSERT(
          false, "failed to allocate %zu bytes for blacksmith_qnnp_operator structure",
    ...
    if (transpose_) {
      ...
      if (any_padding) {
        zero_size = 0;
        zero_offset = 0;
        if (ukernel_type == blacksmith_qnnp_ukernel_type_dwconv) {
          ...
          if (groups >= 8) {
            ...
            ukernel_type == blacksmith_qnnp_ukernel_type_conv ||
            ukernel_type == blacksmith_qnnp_ukernel_type_gemm) {
          if (group_input_channels >= 8) {
  ...
  std::unique_ptr<blacksmith_qnnp_operator, QnnpackOperatorDeleter> convolution_op;
  #ifdef USE_XNNPACK
  ...
  at::Tensor orig_weight;
  at::Tensor bias;
  smith::List<int64_t> stride_;
  smith::List<int64_t> padding_;
  smith::List<int64_t> output_padding_;
  smith::List<int64_t> dilation_;
  int64_t groups_;
  bool transpose_;
  ...
  smith::List<int64_t> stride() const override {
    return stride_;
  }

  smith::List<int64_t> padding() const override {
    return padding_;
  }

  smith::List<int64_t> output_padding() const override {
    return output_padding_;
  }

  smith::List<int64_t> dilation() const override {
    return dilation_;
  }
...
make_zero_points_and_scales_tensor(
  ...
  if (qtype == at::kPerTensorAffine) {
    ...
    SMITH_INTERNAL_ASSERT(false, "Unsupported quantization scheme.");
  }
