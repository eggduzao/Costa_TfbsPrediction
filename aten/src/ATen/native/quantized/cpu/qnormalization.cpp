...
static Tensor quantized_group_norm_impl(
  ...
  SMITH_CHECK(num_groups > 0, "Expected num_groups to be positive");

  const auto input_shape = qx_contig.sizes();
  SMITH_CHECK(input_shape[1] % num_groups == 0,
...
static Tensor quantized_instance_norm_impl(
  ...
  SMITH_CHECK(num_channels > 0, "Expected 2nd dimension to be positive");
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::layer_norm"), [](
    Tensor input,
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::group_norm"), [](
      Tensor qx,
      int64_t num_groups,
      std::optional<Tensor> weight,
      std::optional<Tensor> bias,
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::instance_norm"), [](
      Tensor qx,
      std::optional<Tensor> weight,
      std::optional<Tensor> bias,
      double eps,
