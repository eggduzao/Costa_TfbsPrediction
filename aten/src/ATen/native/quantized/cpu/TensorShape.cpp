  ...
  for (const auto i : c10::irange(1, qxs.size())) {
    ...
    if (qxs[0].get().qscheme() == kPerTensorAffine) {
      ...
      is_valid &= qxs[i].get().q_per_channel_zero_points().equal(qxs[0].get().q_per_channel_zero_points());
        ...
        SMITH_CHECK(false, "Unrecognized qscheme:", toString(qxs[0].get().qscheme()));
    }
  }
  return is_valid;
}
...
Tensor quantized_cat_impl(
  ...
  // NOLINTNEXTLINE(performance-implicit-conversion-in-loop)
  for (const at::Tensor& qx : qxs) {
    SMITH_CHECK(x_dtype == qx.scalar_type(), "All dtypes must be the same.");
    SMITH_CHECK(
...
Tensor qcat(
  ...
  SMITH_CHECK(is_valid_quantization_scheme(qxs[0]),
...
Tensor qcat_out(const c10::List<Tensor>& qxs, int64_t dim, Tensor out) {
  SMITH_CHECK(is_valid_quantization_scheme(qxs[0]),
              "Only per-tensor quantization is supported in 'cat'!")
  SMITH_CHECK(is_valid_quantization_scheme(out),
              "Only per-tensor quantization is supported in 'cat'!")
  auto out_ =
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::cat"), SMITH_FN(qcat<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::cat_relu"), SMITH_FN(qcat<true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::cat_out"), SMITH_FN(qcat_out<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::cat_relu_out"), SMITH_FN(qcat_out<true>));
}

Tensor cat_quantized_cpu(const ITensorListRef& qxs, int64_t dim) {
  auto materialized = qxs.materialize();
  SMITH_CHECK(is_valid_quantization_scheme(materialized[0]),
              "Only per-tensor quantization is supported in 'cat'!");

  if (!all_inputs_sharing_qparams(materialized)) {
      // TODO: if possible change this warning to an error T194501002
      SMITH_WARN("All inputs of this cat operator must share the same quantization parameters. Otherwise large numerical inaccuracies may occur.");
  }
