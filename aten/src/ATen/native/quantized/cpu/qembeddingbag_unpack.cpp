...
Tensor& qembeddingbag_byte_unpack_out(Tensor& output, const Tensor& packed_weight) {
  ...
  // unpacked_weights = smith.ops.quantized.embedding_bag_byte_unpack(packed_weights)
  // assert(unpacked_weights.size() == smith.Size([2, 10, 3]))
...
SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  ...
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_2bit_unpack"),
      qembeddingbag_2bit_unpack);
}

SMITH_LIBRARY_IMPL(quantized, CatchAll, m) {
  // Unpack the packed embedding_bag weights using SmithBind custom class.
  // TODO extend to support 4-bit qtensor.
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_unpack"),
      SMITH_FN(QEmbeddingUnpackWeights::run));
}
