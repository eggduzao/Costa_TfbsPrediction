...
static at::SmallVector<int64_t, kSpatialDim + 2> MakeDeConvOutputShape(
    ...
    const smith::List<int64_t>& stride,
    const smith::List<int64_t>& input_padding,
    const smith::List<int64_t>& output_padding,
    const smith::List<int64_t>& dilation) {
  ...
  for (const auto idx : c10::irange(kSpatialDim)) {
    output_shape[idx + 2] = compute_deconv_shape(input_shape[idx],
                                                 ...
                                                 output_padding[idx],
                                                 dilation[idx]);
    SMITH_CHECK(output_shape[idx + 2] > 0,
    ...
    SMITH_CHECK(output_shape[idx + 2] < kReasonableMaxDim,
  ...
  if (bias.has_value()) {
    *bias_ptr = bias.value();
    SMITH_CHECK(
        ...
        "[QConv3D] The 'bias' tensor must have 'smith.float' dtype");
    *bias_ptr = bias_ptr->contiguous();
    SMITH_CHECK(bias_ptr->dim() == 1, "bias should be a vector (1D Tensor)");
    const int M = w->outputChannels();
    SMITH_CHECK(bias_ptr->size(0) == M, "bias should have ", M, " elements.");
...
void PackedConvWeight<kSpatialDim>::GetQuantizationParams(
  ...
  if (q_scheme == c10::kPerTensorAffine) {
    ...
    *output_multiplier_float = {act_times_w_scale->front() / out_scale};
    ...
    SMITH_CHECK(false, "[QConv", kSpatialDim, "D] Unknown quantization scheme");
  }
}
...
at::Tensor PackedConvWeight<kSpatialDim>::apply_impl(
  ...
  // See https://github.com/blacksmith/blacksmith/issues/23403
  const std::string func_name = transpose() ? "quantized::conv_transpose"
                                            : "quantized::conv";
  SMITH_CHECK(
      fbgemm::fbgemmSupportedCPU(), "Your CPU does not support FBGEMM.");
  SMITH_CHECK(act.scalar_type() == c10::kQUInt8,
                func_name,
...
at::Tensor PackedConvWeightsQnnp<kSpatialDim>::apply_impl(
  ...
  const std::string func_name = transpose() ? "quantized::conv_transpose"
                                            : "quantized::conv";
  SMITH_CHECK(!(kReluFused && transpose()),
              kSpatialDim == 2,
  ...
  if (!input_scale.has_value() || input_scale.value() != act_input_scale) {
    SMITH_CHECK(M == (transpose() ? groups() : 1) * orig_weight.size(out_ch_idx),
        "Output channel size of weight and bias must match.");
    SMITH_CHECK(C == (transpose() ? 1 : groups()) * orig_weight.size(1 - out_ch_idx),
  ...
  SMITH_INTERNAL_ASSERT(pack_w != nullptr, "Packed Weights are NULL");
  at::SmallVector<int64_t, kSpatialDim + 2> output_shape;
  const auto input_shape = MakeInputShape<kSpatialDim>(D, H, W);
  if (transpose()) {
    output_shape = MakeDeConvOutputShape<kSpatialDim>(
  ...
  blacksmith_qnnp_status run_status{};
  if (transpose()) {
  ...
  SMITH_INTERNAL_ASSERT(
      run_status == blacksmith_qnnp_status_success,
...
at::Tensor PackedConvWeightsOnednn<kSpatialDim>::apply_add(
    ...
    double output_scale,
    int64_t output_zero_point) {
  SMITH_CHECK(kSpatialDim == 2, " Currently, only conv2d with add is supported.");
...
at::Tensor PackedConvWeightsOnednn<kSpatialDim>::apply_add_relu(
    ...
    double output_scale,
    int64_t output_zero_point) {
  SMITH_CHECK(kSpatialDim == 2, " Currently, only conv2d add relu is supported.");
...
at::Tensor PackedConvWeightsOnednn<kSpatialDim>::apply_impl(
  ...
  ConvDimChecks<kSpatialDim>(
      act.ndimension(), stride().size(), padding().size(),
      output_padding().size(), dilation().size(), func_name, transpose());
  SMITH_CHECK(act.scalar_type() == c10::ScalarType::QUInt8,
  ...
  if (transpose()) {
    ...
    SMITH_CHECK(C == groups() * weights.get_dim(1), // weight: [o, i, ...]
  ...
  if (has_accum) {
    ...
    SMITH_CHECK(accum_contig.dtype() == output.dtype(), "The output tensor should have same dtype as the accum tensor.");
  ...
  // Scales of ONEDNN and Blacksmith are reciprocal
...
static at::Tensor _fp8_convolution_onednn_ref(
    ...
    smith::List<int64_t> stride,
    smith::List<int64_t> padding,
    smith::List<int64_t> dilation,
    int64_t groups,
    double output_scale,
    ...
    smith::List<std::optional<at::Scalar>> unary_scalars,
  ...
  SMITH_CHECK(kSpatialDim != 1, "Expect 2D or 3D convolution, but got 1D convolution.");
  ...
  if (!binary_attr.has_value() || binary_attr == "none") {
    ...
    SMITH_CHECK(accum.has_value(), "onednn qconv: the extra input is missing for post op sum");
    auto x1 = accum.value();
    SMITH_CHECK(x1.sizes() == y_f32.sizes());
...
static at::Tensor _quantized_convolution_onednn(
  ...
  // https://github.com/blacksmith/blacksmith/blob/b99d605a3070de35677cc43f0196c2f2e807b822/smith/ao/quantization/fx/_decomposed.py#L62-L63
  ...
  if (fp32_output || bfloat16_output) {
    ...
    SMITH_CHECK(output_scale == 1.0,  " (ONEDNN): fp32 or bf16 output, output_scale must be 1.0.");
    SMITH_CHECK(output_zero_point == 0,  " (ONEDNN): fp32 or bf16 output, output_zero_point must be 0");
  }
  ...
  if (has_accum_postop_sum) {
    SMITH_CHECK(accum.has_value(), "For post op sum, accum tensor should not be empty.");
    SMITH_CHECK(
    ...
    if (fp32_output || bfloat16_output) {
      SMITH_CHECK(accum_scale == 1.0,  " (ONEDNN): fp32 or bf16 output, accum_scale must be 1.0.");
      SMITH_CHECK(accum_zero_point == 0,  " (ONEDNN): fp32 or bf16 output, accum_zero_point must be 0");
      SMITH_CHECK((accum.value().scalar_type() == c10::kFloat) || (accum.value().scalar_type() == c10::kBFloat16), "The accum tensor should be KFloat or KBFloat.");
    }
  }
  ...
  if (is_fp8) {
#endif
    SMITH_CHECK(act_dtype == c10::ScalarType::Float8_e4m3fn,
      func_name, ": expect input tensor to have fp8 data type, but got ", act_dtype);
    SMITH_CHECK(act_zero_point == 0,
  ...
  // https://github.com/blacksmith/blacksmith/blob/d2c24eca8a60c56b31ca967a44d5cc4522802aa6/smith/ao/quantization/observer.py#L323
  // 2. If the weight scale got from the quantized tensor, like did in the UT. It's with dtype of double.
  // https://github.com/blacksmith/blacksmith/blob/d2fa3f608b5e4f582a8aaf752f10efe4ca72a7d0/aten/src/ATen/quantized/Quantizer.cpp#L69
  SMITH_CHECK(
  ...
  //   2.1 Input of weights_scales is Blacksmith Dense tensor, we convert it to vector<float>
  ...
  // We should be able to directly convert weights_scales from Blacksmith Dense Tensor to IDeep Tensor which can share same data ptr.
  ...
  if (weight_scales.ndimension() == 0) {
    ...
    weights_scales[0] = 1.0 / weight_scales.item().toDouble(); // Scales of ONEDNN and Blacksmith are reciprocal
  } else {
...
#else
  SMITH_CHECK(false, "Unexpected IDeep version to do qconv calculation.");
#endif
  ...
  if (with_bias) {
    ...
    SMITH_CHECK(bias_val_float.dim() == 1, "bias should be a vector (1D Tensor)");
    SMITH_CHECK(
...
namespace at::native {
  ...
  at::Tensor QConvoneDNN::run_pointwise(
      ...
      smith::List<std::optional<at::Scalar>> scalars,
...
#else
    SMITH_CHECK(false, "Unimplemented as onednn is not available.")
#endif
  }
  ...
  at::Tensor QConvoneDNN::run_pointwise_tensor(
      ...
      std::optional<std::string_view> algorithm) {
#if AT_MKLDNN_ENABLED()
    SMITH_CHECK(act_scale.numel() == 1 && act_zero_point.numel() == 1,
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv1d"),          QConv1dInt8<false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv1d_relu"),     QConv1dInt8<true>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d.new"),      QConvInt8<2, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_relu.new"), QConvInt8<2, true>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_add"),      QConvAddInt8<2, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_add_relu"), QConvAddInt8<2, true>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv3d.new"),      QConvInt8<3, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv3d_relu.new"), QConvInt8<3, true>::run);
  // for backward compatibility
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d"), QConvInt8ForBC<2, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_relu"), QConvInt8ForBC<2, true>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv3d"), QConvInt8ForBC<3, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv3d_relu"), QConvInt8ForBC<3, true>::run);

  // transpose
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_transpose1d"),  QConv1dInt8<false>::run);
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_transpose2d"),  QConvInt8<2, false>::run);
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv_transpose3d"),
      QConvInt8<3, false>::run);
}

SMITH_LIBRARY_IMPL(_quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv2d"),      QConvInt8<2, false>::run);
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv2d_relu"), QConvInt8<2, true>::run);

  // transpose
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv_transpose1d"),  QConv1dInt8<false>::run);
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv_transpose2d"),  QConvInt8<2, false>::run);
}

SMITH_LIBRARY_IMPL(onednn, MkldnnCPU, m) {
  // Conv1D/2D/3D with unary postop
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv1d_pointwise"), at::native::QConvoneDNN::run_pointwise);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise"), at::native::QConvoneDNN::run_pointwise);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise.tensor"), at::native::QConvoneDNN::run_pointwise_tensor);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv3d_pointwise"), at::native::QConvoneDNN::run_pointwise);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv_pointwise"), at::native::QConvoneDNN::run_pointwise);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv_pointwise.tensor"), at::native::QConvoneDNN::run_pointwise_tensor);

  // Conv2D with binary postop
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise.binary"), at::native::QConvoneDNN::run_pointwise_binary);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise.binary_tensor"), at::native::QConvoneDNN::run_pointwise_binary_tensor);
}

SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv_pointwise"), at::native::QConvoneDNN::run_pointwise);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv_pointwise.tensor"), at::native::QConvoneDNN::run_pointwise_tensor);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise.binary"), at::native::QConvoneDNN::run_pointwise_binary);
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv2d_pointwise.binary_tensor"), at::native::QConvoneDNN::run_pointwise_binary_tensor);
