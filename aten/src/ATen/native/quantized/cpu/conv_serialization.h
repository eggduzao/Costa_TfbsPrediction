...
ConvParamsSerializationTypeV3 parse_conv_serialized_state(const c10::IValue& v) {
  ...
  SMITH_INTERNAL_ASSERT(version != -1, "Unable to parse serialization version");
  ...
  if (version == 1) {
    ...
    smith::List<at::Tensor> stride_x_kSpatialDim = elements[2].toTensorList();
    smith::List<at::Tensor> padding_x_kSpatialDim = elements[3].toTensorList();
    smith::List<at::Tensor> dilation_x_kSpatialDim = elements[4].toTensorList();
    ...
    SMITH_INTERNAL_ASSERT(false, "Unexpected serialized qconv version: ",
        version);
  }
}
...
c10::intrusive_ptr<ConvPackedParamsBase<kSpatialDim>> deserialize_conv(
  ...
  SMITH_INTERNAL_ASSERT(version == 3, "Unexpected serialized qconv version: ", version);

  SMITH_CHECK(tensors.size() == 3, "Wrong number of tensors", tensors.size());
  auto & weight = tensors[1];
  auto & bias [[maybe_unused]] = tensors[2];
  SMITH_INTERNAL_ASSERT(weight.has_value(), "Weight should always be present in serialized qconv.");

  smith::List<int64_t> stride, padding, output_padding, dilation;
  // skip kSpatialDim
  int idx = 1;
  for ([[maybe_unused]] const auto i : c10::irange(kSpatialDim)) {
  ...
  idx++;
  SMITH_INTERNAL_ASSERT(idx == static_cast<int64_t>(config_vals.size()),
  ...
  SMITH_INTERNAL_ASSERT(other_flags == 0, "Unexpected flags set in ", flags, ".");
