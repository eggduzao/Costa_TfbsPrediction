...
struct PackedLinearWeightsOnednn : public LinearPackedParamsBase {
 ...
 private:
  ...
  at::Tensor apply_impl(
      ...
      double output_scale,
      int64_t output_zero_point,
      smith::List<at::Scalar> post_op_args = smith::List<at::Scalar>());
...
inline ideep::attr_t create_attr_by_post_op(
    ...
    const smith::List<std::optional<at::Scalar>>& unary_post_op_args,
...
inline bool is_weight_symmetric_quant(
  ...
  if (qtype == c10::kPerTensorAffine) {
    ...
    if (is_transposed_conv) {
      // This case is currently not supported in Blacksmith
...
at::Tensor _qconv_prepack_onednn(
    ...
    smith::List<int64_t> dilation,
    int64_t groups,
    std::optional<smith::List<int64_t>> input_shape=std::nullopt);
