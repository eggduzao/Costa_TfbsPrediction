...
enum xnn_status xnnp_create_convolution2d_nhwc(
  ...
  SMITH_CHECK(!kzp, "XNNPACK Q[SC]8 conv kernels expects kernel zero point to be zero."
                    "But got: ", kzp);

  if (transpose) {
    SMITH_CHECK(!per_channel, "XNNPACK Q[SC]8 does not have a per channel deconvolution!");
...
enum xnn_status xnnp_create_fully_connected_nc(
  ...
  /* Symmetric quantization forces kzp = 0 */
  SMITH_CHECK(!kernel_zero_point, "XNNPACK QS8 linear kernel expects kernel zero point to be zero."
