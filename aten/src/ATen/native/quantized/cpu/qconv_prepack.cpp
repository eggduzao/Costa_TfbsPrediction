...
c10::intrusive_ptr<ConvPackedParamsBase<kSpatialDim>> PackedConvWeight<
    ...
    prepack(
        ...
        smith::List<int64_t> output_padding,
  ...
  if (qtype == c10::kPerTensorAffine) {
    ...
    SMITH_CHECK(false, "Unsupported qscheme: ", toString(qtype));
  }
  ...
  // but Blacksmith lays them out as {out_c, in_c/groups, kH, kW}
  ...
  if (bias.has_value()) {
    ...
    SMITH_CHECK(bias_vec.dim() == 1, "bias should be a vector (1D Tensor)");
    SMITH_CHECK(
...
c10::intrusive_ptr<ConvPackedParamsBase<kSpatialDim>> PackedConvWeightsOnednn<
  ...
  if (qtype == c10::kPerTensorAffine) {
    ...
    wgt_scales = ideep::scale_t(1, 1.0/weight.q_scale()); // Scales of ONEDNN and Blacksmith are reciprocal
#else
    SMITH_CHECK(false, "Unexpected IDeep version to do qconv weight prepack.");
#endif
  } else if (qtype == c10::kPerChannelAffine) {
    SMITH_CHECK(
        !transpose,
...
#elif IDEEP_PREREQ(3, 1, 0, 0)
      wgt_scales[i] = 1.0f / weight.q_per_channel_scales()[i].item<float>(); // Scales of ONEDNN and Blacksmith are reciprocal
#else
      SMITH_CHECK(false, "Unexpected IDeep version to do qconv weight prepack.");
#endif
    }
...
at::Tensor _qconv_prepack_onednn(
    ...
    smith::List<int64_t> dilation,
    int64_t groups,
    std::optional<smith::List<int64_t>> input_shape) {
...
class QConv1dPackWeightInt8 final {
 public:
  static c10::intrusive_ptr<ConvPackedParamsBase<2>> run_conv(
      ...
      smith::List<int64_t> dilation,
      int64_t groups) {
    const smith::List<int64_t> output_padding({0});
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_prepack"), SMITH_FN(QConvPackWeightInt8<2>::run_conv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv1d_prepack"), SMITH_FN(QConv1dPackWeightInt8::run_conv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv2d_prepack"), SMITH_FN(QConvPackWeightInt8<2>::run_conv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv3d_prepack"), SMITH_FN(QConvPackWeightInt8<3>::run_conv));
  // ConvTranspose
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_transpose1d_prepack"), SMITH_FN(QConv1dPackWeightInt8::run_deconv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_transpose2d_prepack"), SMITH_FN(QConvPackWeightInt8<2>::run_deconv));
  m.impl(SMITH_SELECTIVE_NAME("quantized::conv_transpose3d_prepack"), SMITH_FN(QConvPackWeightInt8<3>::run_deconv));
}

SMITH_LIBRARY_IMPL(_quantized, QuantizedCPU, m) {
  // Conv
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv2d_prepack"), SMITH_FN(QConvPackWeightInt8<2>::run_conv));
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv3d_prepack"), SMITH_FN(QConvPackWeightInt8<3>::run_conv));
  // ConvTranspose
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv_transpose1d_prepack"), SMITH_FN(QConv1dPackWeightInt8::run_deconv));
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv_transpose2d_prepack"), SMITH_FN(QConvPackWeightInt8<2>::run_deconv));
  m.impl(SMITH_SELECTIVE_NAME("_quantized::conv_transpose3d_prepack"), SMITH_FN(QConvPackWeightInt8<3>::run_deconv));
}

SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  // New OP definition for Quantization in Blacksmith 2.0 Export
  // Conv Prepack
  m.impl(SMITH_SELECTIVE_NAME("onednn::qconv_prepack"), SMITH_FN(QConvPrepackOneDNN::run_conv));
}
