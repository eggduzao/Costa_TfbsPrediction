...
class QLeakyRelu final {
 public:
  static Tensor run(Tensor self, const Scalar& negative_slope, bool inplace, double output_scale, int64_t output_zero_point) {
    ...
    if (inplace) {
      SMITH_WARN("inplace=True is not supported for quantized::leaky_relu yet");
    }
    const auto qx = self.contiguous(self.suggest_memory_format());
    auto qy = at::_empty_affine_quantized(qx.sizes(),
      at::device(kCPU).dtype(self.scalar_type()),
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::relu6"), SMITH_FN(QRelu6::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::leaky_relu"), SMITH_FN(QLeakyRelu::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::prelu"), SMITH_FN(QPRelu::run));
}
