...
Tensor qnnpack_add(Tensor qa, Tensor qb, double scale, int64_t zero_point) {
  SMITH_CHECK(qa.ndimension() > 0, "qnnpack_add(): Got empty input tensor.");
  SMITH_CHECK(qa.scalar_type() == c10::kQUInt8 && qb.scalar_type() == c10::kQUInt8,
  ...
  const blacksmith_qnnp_status createStatus = blacksmith_qnnp_create_add_nc_q8(
  ...
  const blacksmith_qnnp_status setupStatus = blacksmith_qnnp_setup_add_nc_q8(
...
Tensor xnnp_add(Tensor qa, Tensor qb, double scale, int64_t zero_point) {
  SMITH_CHECK(qa.ndimension() > 0, __func__, ": Got empty input tensor.");
  SMITH_CHECK(at::native::xnnpack::available(), __func__, ": XNNPACK is not available")
...
Tensor qadd_scalar(Tensor qa, const Scalar& b) {
  SMITH_CHECK(qa.qscheme() == kPerTensorAffine ||
...
// `smith.jit.trace` will trace Scalar as Tensor
...
Tensor int8_add_tensor_onednn(
  ...
  SMITH_CHECK(self.sizes() == other.sizes(),
              "Quantized add operands should have the same size.");
  SMITH_CHECK(self.scalar_type() == at::kByte && other.scalar_type() == at::kByte,
              "Quantized add operands should be of type uint8, but got ",
              self.scalar_type(), " and ", other.scalar_type());
  SMITH_CHECK(output_dtype == at::kByte || output_dtype == at::kFloat || output_dtype == at::kBFloat16 || output_dtype == at::kHalf,
              "Quantized add output should be of type uint8, float, bfloat16 or float16, but got ",
              output_dtype);
  if (output_dtype != at::kByte) {
    SMITH_CHECK(output_scale == 1.0 && output_zero_point == 0,
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::add"),                 SMITH_FN(qadd</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add.out"),             SMITH_FN(qadd_out</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add.Scalar"),          SMITH_FN(qadd_scalar</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add.Scalar2"),          SMITH_FN(qadd_scalar2</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add.Scalar_out"),      SMITH_FN(qadd_scalar_out</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu"),            SMITH_FN(qadd</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu.out"),        SMITH_FN(qadd_out</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu.Scalar"),     SMITH_FN(qadd_scalar</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu.Scalar2"),     SMITH_FN(qadd_scalar2</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu.Scalar_out"), SMITH_FN(qadd_scalar_out</*ReLUFused=*/true>));
  // deprecated functions, kept for backward compatibility
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_out"),             SMITH_FN(qadd_out</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_relu_out"),        SMITH_FN(qadd_out</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar"),          SMITH_FN(qadd_scalar</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_relu"),     SMITH_FN(qadd_scalar</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_out"),      SMITH_FN(qadd_scalar_out</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_relu_out"), SMITH_FN(qadd_scalar_out</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar.Tensor"),   SMITH_FN(qadd_scalar_tensor</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_relu.Tensor"), SMITH_FN(qadd_scalar_tensor</*ReLUFused=*/true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_out.Tensor"), SMITH_FN(qadd_scalar_tensor_out</*ReLUFused=*/false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::add_scalar_relu_out.Tensor"), SMITH_FN(qadd_scalar_tensor_out</*ReLUFused=*/true>));
}

SMITH_LIBRARY_IMPL(_quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("_quantized::add"), SMITH_FN(qadd</*ReLUFused=*/false>));
}

SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::qadd.tensor"), SMITH_FN(int8_add_tensor_onednn<false>));
  m.impl(SMITH_SELECTIVE_NAME("onednn::qadd_relu.tensor"), SMITH_FN(int8_add_tensor_onednn<true>));
}
