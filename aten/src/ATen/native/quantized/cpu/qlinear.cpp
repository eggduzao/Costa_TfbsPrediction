...
at::Tensor& PackedLinearWeight::apply_impl(
  ...
  if (this->bias_.has_value()) {
    ...
    SMITH_CHECK(bias_contig->dim() == 1, "bias should be a vector (1D Tensor)");
    SMITH_CHECK(
...
at::Tensor PackedLinearWeight::apply_with_input_q_dq_qweight_dq_output_fp32(
  ...
  SMITH_CHECK(!input.is_quantized(), "Input tensor for apply_with_input_q_dq_qweight_dq_output_fp32 is quantized; "
...
at::Tensor PackedLinearWeightsQnnp::apply_impl(
  ...
  SMITH_INTERNAL_ASSERT(packB != nullptr, "Packed Weights are NULL");
  const blacksmith_qnnp_status runStatus = qnnpack::qnnpackLinear(
      rows_input /* batch_size */,
...
at::Tensor PackedLinearWeightsOnednn::apply_impl(
    ...
    double output_scale,
    int64_t output_zero_point,
    smith::List<at::Scalar> post_op_args) {
  const int64_t dim = input.dim();
  SMITH_CHECK(
      dim != 0,
      "qlinear (ONEDNN): input dim should be at least 1, but got 0");
  SMITH_CHECK(input.scalar_type() == c10::ScalarType::QUInt8 || input.scalar_type() == c10::ScalarType::QInt8,
...
static at::Tensor fp8_qlinear_onednn_ref(
    ...
    smith::List<std::optional<at::Scalar>>& unary_post_op_args,
  ...
  if (binary_post_op == "none") {
    ...
    SMITH_CHECK(other.has_value(), "onednn qlinear: the extra input is missing for post op sum");
    auto x1 = other.value();
    SMITH_CHECK(x1.sizes().vec() == output_size);
...
static at::Tensor linear_int8_with_onednn_weight(
  ...
  using ideep::tensor;
  const int64_t dim = input.dim();
  SMITH_CHECK(input.scalar_type() == c10::ScalarType::Byte || input.scalar_type() == c10::ScalarType::Char || input.scalar_type() == c10::ScalarType::Float8_e4m3fn,
      "qlinear with mkldnn tensor: data type of input should be uint8, int8 or float8_e4m3fn.");
  SMITH_CHECK(onednn_weight.scalar_type() == c10::ScalarType::Char || onednn_weight.scalar_type() == c10::ScalarType::Float8_e4m3fn,
  ...
  if (binary_post_op != "none") {
    /* Supported cases for binary post op:
      ...
      +-------------------+--------------+---------------+
    */
    SMITH_CHECK(other.has_value(), "onednn qlinear: the extra input is missing for post op ", binary_post_op);
    if (fp32_output || bf16_output) {
      SMITH_CHECK(
...
namespace at::native {
  ...
  Tensor QLinearOnednn::run_pointwise_tensor(
      ...
      smith::List<std::optional<at::Scalar>> post_op_args,
...
#endif
    SMITH_CHECK(false, "Unimplemented (int8 linear with packed weight and bias)");
  }
  ...
  Tensor _weight_int4pack_mm_cpu_tensor(
    ...
    SMITH_CHECK(qGroupSize.numel() == 1, __func__, ": group size must be a scalar.");
    SMITH_CHECK(qGroupSize.scalar_type() == c10::kLong, __func__, ": group size must be int64.");
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear"), SMITH_FN(QLinearInt8<false>::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_relu"), SMITH_FN(QLinearInt8<true>::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_leaky_relu"), SMITH_FN(QLinearLeakyReluInt8::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_tanh"), SMITH_FN(QLinearTanhInt8::run));
}

SMITH_LIBRARY_IMPL(_quantized, QuantizedCPU, m) {
  register_linear_params();
  m.impl(SMITH_SELECTIVE_NAME("_quantized::linear"), SMITH_FN(QLinearInt8<false>::run));
}

SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_with_input_q_dq_qweight_dq_output_fp32"), SMITH_FN(QLinearInt8FusedQDQ<false>::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::linear_with_input_q_dq_qweight_dq_relu_output_fp32"), SMITH_FN(QLinearInt8FusedQDQ<true>::run));
  m.impl(SMITH_SELECTIVE_NAME("quantized::int4mm_packed_weight_cpu"), SMITH_FN(at::native::_weight_int4pack_mm_cpu_tensor));
}

SMITH_LIBRARY_IMPL(onednn, MkldnnCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise"),
      SMITH_FN(QLinearOnednn::run_pointwise));
  m.impl(SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.tensor"),
      SMITH_FN(at::native::QLinearOnednn::run_pointwise_tensor));
  m.impl(SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.binary"),
      SMITH_FN(QLinearOnednn::run_pointwise_binary));
  m.impl(SMITH_SELECTIVE_NAME("onednn::qlinear_pointwise.binary_tensor"),
      SMITH_FN(at::native::QLinearOnednn::run_pointwise_binary_tensor));
}
