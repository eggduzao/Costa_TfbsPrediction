...
at::Tensor PackedConvWeightsQnnp<kSpatialDim>::apply_dynamic(
    ...
    bool reduce_range) {
  if (reduce_range) {
    SMITH_WARN("Currently, qnnpack incorrectly ignores reduce_range when it is set to true; this may change in a future release.");
  }
...
SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv1d_dynamic"),
      SMITH_FN(QConv1dDynamicInt8::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv2d_dynamic"),
      SMITH_FN(QConvDynamicInt8<2>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv3d_dynamic"),
      SMITH_FN(QConvDynamicInt8<3>::run));

  // transpose
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv_transpose1d_dynamic"),
      SMITH_FN(QConv1dDynamicInt8::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv_transpose2d_dynamic"),
      SMITH_FN(QConvDynamicInt8<2>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::conv_transpose3d_dynamic"),
      SMITH_FN(QConvDynamicInt8<3>::run));
}
