...
Tensor qnnpack_clamp(Tensor input, const Scalar& min, const Scalar& max) {
  ...
  SMITH_CHECK(input.ndimension() > 0, "qnnpack_clamp(): Got empty input tensor");
  ...
  blacksmith_qnnp_operator_t clamp_op{nullptr};
  const blacksmith_qnnp_status createStatus = blacksmith_qnnp_create_clamp_nc_u8(
    num_elems, // channels
    min_q,
    max_q,
    0, // flags
  ...
  const blacksmith_qnnp_status setupStatus = blacksmith_qnnp_setup_clamp_nc_u8(
    clamp_op,
  ...
  const blacksmith_qnnp_status runStatus =
    blacksmith_qnnp_run_operator(clamp_op, threadpool);
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::clamp"), SMITH_FN(clamp_quantized_cpu));
}
