  ...
  for (const auto m : c10::irange(output_size)) {
    ...
    for (int i = 0; i < lengths_data[m]; ++i, ++current) {
      ...
      if (!pruned) {
        ...
        SMITH_CHECK((idx >= 0 && idx < N), "Invalid indices data");
      } else {
...
at::Tensor& embedding_bag_byte_impl(
    ...
    bool include_last_offset,
    bool is_embedding_op) {
  SMITH_CHECK(weight.scalar_type() == at::kByte);
  SMITH_CHECK(weight.dim() == 2);
  SMITH_CHECK(offsets.dim() == 1);
  auto offsets_data = offsets.data_ptr<OffsetType>();
...
SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  // Function that works on SmithBind packed weights.
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_byte"),
      SMITH_FN(QEmbeddingBag<8>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_4bit"),
      SMITH_FN(QEmbeddingBag<4>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_byte"),
      SMITH_FN(QEmbedding<8>::run));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_4bit"),
      SMITH_FN(QEmbedding<4>::run));
  ...
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_2bit_rowwise_offsets"),
