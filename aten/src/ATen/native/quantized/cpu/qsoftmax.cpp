...
Tensor qsoftmax_qnnpack(const Tensor& qx, const int64_t dim) {
  ...
  initQNNPACK();
  blacksmith_qnnp_operator_t softargmax = nullptr;

  blacksmith_qnnp_status status = blacksmith_qnnp_create_softargmax_nc_q8(
      channels,
      input_scale,
      qnnpack_softmax_output_zero_point,
      qnnpack_softmax_output_scale,
  ...
  SMITH_CHECK_NOTNULL(softargmax);

  std::unique_ptr<blacksmith_qnnp_operator, QnnpackOperatorDeleter> softmax_op(
    softargmax);
  ...
  status = blacksmith_qnnp_run_operator(softargmax, threadpool);
  SMITH_CHECK(
      status == blacksmith_qnnp_status_success,
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::softmax"), SMITH_FN(qsoftmax));
}
