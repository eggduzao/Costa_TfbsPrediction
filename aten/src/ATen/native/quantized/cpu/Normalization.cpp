...
Tensor q_batch_norm1d_impl(
  ...
  SMITH_CHECK(mb_weight.has_value(), "Weight must be provided");
  SMITH_CHECK(mb_bias.has_value(), "Bias must be provided");
  ...
  int64_t ndim = qx.dim();
  SMITH_CHECK(ndim == 2 || ndim == 3, "Expecting the input tensor of rank 2 or 3.");
  const int64_t N = qx.size(0);
  const int64_t C = qx.size(1);
  const int64_t H = ndim == 3 ? qx.size(2) : 1;

  SMITH_CHECK(weight.numel() == C, "Expect weight size to match C");
  SMITH_CHECK(bias.numel() == C, "Expect weight size to match C");

  const float* weight_data = weight.template const_data_ptr<float>();
  const float* bias_data = bias.template const_data_ptr<float>();

  SMITH_CHECK(mean.numel() == C, "Mean size must match channel dimension");
  SMITH_CHECK(var.numel() == C, "Variance size must match channel dimension");
...
Tensor q_batch_norm2d_impl(
  ...
  int64_t ndim = qx.dim();
  SMITH_CHECK(ndim == 4, "Expecting the input tensor of rank 4.");
  const int64_t N = qx.size(0);
  const int64_t C = qx.size(1);
...
Tensor q_batch_norm3d_impl(
  ...
  int64_t ndim = qx.dim();
  SMITH_CHECK(ndim == 5, "Expecting the input tensor of rank 5.");
  const int64_t N = qx.size(0);
  const int64_t C = qx.size(1);
...
Tensor q_batch_norm_impl(
  ...
  if (dim == 2 || dim == 3) {
    ...
    SMITH_CHECK(false, "quantized::batch_norm only support 2d, 3d, 4d or 5d inputs.");
  }
  return qy;
}
...
Tensor int8_batch_norm2d_cpu_impl(
  ...
  int64_t ndim = qx.dim();
  SMITH_CHECK(ndim == 4, "Int8 batch_norm2d: Expecting the input tensor of rank 4.");
  const int64_t N = qx.size(0);
  const int64_t C = qx.size(1);
  const int64_t H = qx.size(2);
  const int64_t W = qx.size(3);
...
SMITH_LIBRARY_IMPL(quantized, QuantizedCPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm"),        SMITH_FN(q_batch_norm_impl<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm_relu"),   SMITH_FN(q_batch_norm_impl<true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm1d"),      SMITH_FN(q_batch_norm1d_impl<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm1d_relu"), SMITH_FN(q_batch_norm1d_impl<true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm2d"),      SMITH_FN(q_batch_norm2d_impl<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm2d_relu"), SMITH_FN(q_batch_norm2d_impl<true>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm3d"),      SMITH_FN(q_batch_norm3d_impl<false>));
  m.impl(SMITH_SELECTIVE_NAME("quantized::batch_norm3d_relu"), SMITH_FN(q_batch_norm3d_impl<true>));
}

SMITH_LIBRARY_IMPL(onednn, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("onednn::qbatch_norm2d"), SMITH_FN(int8_batch_norm2d_cpu_impl));
}
