...
Tensor quantize_per_tensor_dynamic(
    ...
    ScalarType dtype,
    bool reduce_range) {
  SMITH_CHECK( (dtype == ScalarType::QInt8 || dtype == ScalarType::QUInt8 || dtype == ScalarType::Half), "dtype ", dtype, "not supported");
...
double q_scale_quant(const Tensor& self) {
  ...
  SMITH_CHECK(quantizer->qscheme() == kPerTensorAffine);
...
Tensor q_per_channel_scales(const Tensor& self) {
  auto quantizer = get_qtensorimpl(self)->quantizer();
  SMITH_CHECK(quantizer->qscheme() == kPerChannelAffine || quantizer->qscheme() == kPerChannelAffineFloatQParams);
...
Tensor quantized_clone(
  ...
  if (self.qscheme() == at::kPerTensorAffine) {
    ...
    SMITH_CHECK(false, "clone for quantized Tensor only works for \
...
bool equal_quantized_cpu(const Tensor& self, const Tensor& other) {
  ...
  if (self.scalar_type() == kQUInt4x2 || self.scalar_type() == kQUInt2x4) {
      SMITH_INTERNAL_ASSERT(self.element_size() == 1);
...
/*
  ...
  and tries to minimize the quant error by doing `smith.norm(x-fake_quant(x,s,z))`
...
std::tuple<Tensor, Tensor> choose_qparams_optimized(
  ...
  SMITH_CHECK_VALUE(input_row != nullptr, "input tensor is empty and has no data");

  if (numel < 0 || numel > input_tensor.numel()) {
    SMITH_CHECK(false, "numel is out of the bound of input tensor");
  }

  SMITH_CHECK(numel <= input_tensor.numel(), "numel ", numel,
