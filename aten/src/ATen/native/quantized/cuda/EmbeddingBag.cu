...
at::Tensor& embedding_bag_byte_impl(
  ...
  SMITH_CHECK(weight.is_cuda());
  SMITH_CHECK(indices.is_cuda());
  SMITH_CHECK(offsets.is_cuda());
  SMITH_CHECK(indices.device() == weight.device())
  SMITH_CHECK(offsets.device() == weight.device());
  if (per_sample_weights_.has_value()) {
    SMITH_CHECK(per_sample_weights_.value().device() == weight.device());
  }
  if (compressed_indices_mapping.has_value()) {
    SMITH_CHECK(compressed_indices_mapping.value().device() == weight.device());
  }

  SMITH_CHECK(weight.dtype() == at::kByte);
  ...
  SMITH_CHECK(D % 4 == 0);
  if(per_sample_weights_.has_value()) {
      SMITH_CHECK(per_sample_weights_.value().scalar_type() == at::kFloat,
...
Tensor embedding_bag_byte_rowwise_offsets(
  ...
  // default case given the SMITH_CHECK above
...
SMITH_LIBRARY_IMPL(quantized, CUDA, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_byte_unpack"),
      SMITH_FN(qembeddingbag_byte_unpack));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_byte_rowwise_offsets"),
      SMITH_FN(embedding_bag_byte_rowwise_offsets));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_4bit_unpack"),
      SMITH_FN(qembeddingbag_4bit_unpack));
  m.impl(
      SMITH_SELECTIVE_NAME("quantized::embedding_bag_4bit_rowwise_offsets"),
      SMITH_FN(embedding_bag_4bit_rowwise_offsets));
}
