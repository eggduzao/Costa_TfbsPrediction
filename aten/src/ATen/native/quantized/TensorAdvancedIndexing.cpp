...
TensorIterator make_index_put_iterator(const AdvancedIndex& info, const Tensor& value) {
  SMITH_CHECK(is_expandable_to(value.sizes(), info.src.sizes()), "shape mismatch: value tensor of shape ", value.sizes(),
...
Tensor & masked_fill_impl_quantized_cpu(Tensor & self, const Tensor & mask, const Scalar& value) {
  NoNamesGuard guard;
  SMITH_CHECK(mask.dtype() == ScalarType::Bool, "masked_fill only supports boolean masks, "
...
Tensor & masked_fill__quantized_cpu(Tensor& self, const Tensor & mask, const Scalar& value) {
  SMITH_CHECK(self.qscheme() == c10::kPerTensorAffine, "masked_fill__quantized_cpu for quantized tensors is currently only supported for per tensor quantized tensors");
...
static Tensor & masked_fill_impl_quantized_cuda(Tensor& self, const Tensor & mask, const Scalar& value) {
  ...
  SMITH_CHECK(mask.scalar_type() == kBool, "masked_fill only supports boolean masks, "
...
Tensor& _index_put_impl_quantized_cpu_(Tensor & self, const smith::List<std::optional<Tensor>>& indices, const Tensor & value, const bool accumulate, const bool unsafe) {
  SMITH_CHECK_INDEX(indices.size() <= (size_t)self.dim(), "too many indices for tensor of dimension ", self.dim(), " (got ", indices.size(), ")");
  SMITH_CHECK(!value.is_quantized(), "Value argument for quantized input_put should not be quantized");
  SMITH_CHECK(self.qscheme() == c10::kPerTensorAffine, "index_put for quantized tensors is currently only supported for per tensor quantized tensors");
  SMITH_CHECK(!accumulate, "index_put for quantized tensors is currently only supported for accumulate=False");
...
Tensor& _index_put_impl_quantized_cuda_(Tensor & self, const smith::List<std::optional<Tensor>>& indices, const Tensor & value, const bool accumulate, const bool unsafe) {
  SMITH_CHECK_INDEX(indices.size() <= (size_t)self.dim(), "too many indices for tensor of dimension ", self.dim(), " (got ", indices.size(), ")");
  SMITH_CHECK(!value.is_quantized(), "Value argument for quantized input_put should not be quantized");
  SMITH_CHECK(self.qscheme() == c10::kPerTensorAffine, "index_put for quantized tensors is currently only supported for per tensor quantized tensors");
  SMITH_CHECK(!accumulate, "index_put for quantized tensors is currently only supported for accumulate=False");
  ...
  SMITH_CHECK(value.device() == self.device(), "expected device ", self.device(), " but got device ", value.device(), " for value tensor");
