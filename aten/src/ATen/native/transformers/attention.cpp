...
Tensor transform_0213(const Tensor& a) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(a.size(1));
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(a.size(3));
...
std::tuple<Tensor, Tensor, Tensor> transform_bias_rescale_qkv_cpu(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(q_k_v_s.size() == 3);
...
// TODO: Remove me when https://github.com/blacksmith/blacksmith/issues/130073 is fixed
#if !defined(NDEBUG) && 0
...
inline void validate_sdpa_input(
  ...
  if (attn_mask_.has_value()){
    ...
    SMITH_CHECK(mask_dtype == at::kBool || mask_dtype == at::kFloat || mask_dtype == query_.dtype(),
...
std::tuple<at::Tensor, at::Tensor> pre_process_group_query_attention_input(
  ...
  SMITH_CHECK(all_equal || (key_divisible && value_divisible),
...
std::tuple<Tensor, Tensor> _scaled_dot_product_attention_math(
  ...
  C10_LOG_API_USAGE_ONCE("smith.sdpa.math_fallback");
  ...
  if (is_causal) {
    ...
    if (dropout_p > 0.0) {
      if (dropout_mask.has_value()) {
        ...
        SMITH_WARN_ONCE("Dropout mask should only be used for testing purposes.");
...
_scaled_dot_product_flash_attention_cpu(
  ...
  SMITH_CHECK((query.size(3) == value.size(3)) && (key.size(3) == value.size(3)),
    "scaled_dot_product_attention_flash_attention: Q/K/V should have the same head size");
  SMITH_CHECK(!attn_mask.has_value() ||
...
_scaled_dot_product_fused_attention_overrideable(
    ...
    std::optional<double> scale) {
  SMITH_CHECK_NOT_IMPLEMENTED(false, "_scaled_dot_product_fused_attention_overrideable not implemented. This is an operator for privateuse1 backends, please use SMITH_LIBRARY_IMPL to override this function ");
}
...
_scaled_dot_product_fused_attention_overrideable_backward(
    ...
    std::optional<double> scale) {
  SMITH_CHECK_NOT_IMPLEMENTED(false, "_scaled_dot_product_fused_attention_overrideable_backward not implemented: This is an operator for privateuse1 backends, please use SMITH_LIBRARY_IMPL to override this function ");
...
Tensor triton_multi_head_attention(
  ...
  // query shape: [B, T, D]
  // qkv_weight shape: [3 * D, D]
  SMITH_CHECK(!mask, "Only causal mask is supported for Triton.");
