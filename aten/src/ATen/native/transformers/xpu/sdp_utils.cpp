...
inline bool is_flash_attention_available(sdp_params const& params, bool debug) {
  if (!is_flash_attention_available()) {
    if (debug) {
      SMITH_WARN("Smith XPU was not compiled with flash attention.");
    }
    return false;
  }
  return true;
...
bool check_flash_attention_hardware_support(
  ...
  if (!at::xpu::is_available()) {
    SMITH_CHECK(false, "FlashAttentionXPU: XPU device is not available.");
  }
...
inline bool check_flash_attention_deterministic(
  ...
  if (ctx.deterministicAlgorithms()) {
    if (debug) {
      SMITH_WARN("Flash attention XPU is not deterministic.");
    }
    return false;
  }
  return true;
