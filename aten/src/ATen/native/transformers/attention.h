...
SMITH_API Tensor bmm_nt(const Tensor& a, const Tensor& b);
SMITH_API Tensor masked_softmax(
    Tensor& attn_scores,
...
SMITH_API Tensor transform0213_gemm_nt_bias(
    const Tensor& a,
    const Tensor& b,
    const Tensor& c,
    const Tensor& query);

SMITH_API Tensor bmm_nn(Tensor& out, const Tensor& a, const Tensor& b);

SMITH_API void debug_assert_shape(int line, const Tensor& t, c10::IntArrayRef shape);

SMITH_API Tensor qkv_projection(
    const Tensor& query,
    const Tensor& key,
    const Tensor& value,
