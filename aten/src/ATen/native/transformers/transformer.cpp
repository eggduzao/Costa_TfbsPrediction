...
Tensor ffn(
    ...
    bool use_gelu,
    bool add_norm) {
  SMITH_CHECK(add_norm == false, "TODO add_norm to be supported in FFN");
  SMITH_CHECK(input.dim() == 3, "batched input size should be 3");
  SMITH_CHECK(w1.dim() == 2, "2d weights expected");
  SMITH_CHECK(w2.dim() == 2, "2d weights expected");
