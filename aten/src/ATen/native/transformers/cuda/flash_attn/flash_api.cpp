...
#define CHECK_DEVICE(x) SMITH_CHECK(x.is_cuda(), #x " must be on CUDA")
#define CHECK_SHAPE(x, ...) SMITH_CHECK(x.sizes() == at::IntArrayRef({__VA_ARGS__}), #x " must have shape (" #__VA_ARGS__ ")")
#define CHECK_CONTIGUOUS(x) SMITH_CHECK(x.is_contiguous(), #x " must be contiguous")
...
void set_params_fprop(Flash_fwd_params &params,
    ...
    // Set the different scale values.
        ...
        SMITH_CHECK(softcap <= 0.0, "This flash attention build does not support softcap.");
    #endif
    if (softcap > 0.0) {
    ...
    SMITH_CHECK(p_dropout < 1.f);
    #ifdef FLASHATTENTION_DISABLE_DROPOUT
        SMITH_CHECK(p_dropout == 0.0f, "This flash attention build does not support dropout.");
    #endif
    ...
    params.window_size_right = window_size_right;
        ...
        SMITH_CHECK(params.is_causal || (window_size_left < 0 && window_size_right < 0),
    ...
    params.is_seqlens_k_cumulative = true;
        ...
        SMITH_CHECK(d == d_rounded, "This flash attention build does not support headdim not being a multiple of 32.");
    #endif
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        ...
        SMITH_CHECK(params.num_splits <= 128, "num_splits > 128 not supported");
    }
...
void set_params_alibi(Flash_fwd_params &params, std::optional<at::Tensor> &alibi_slopes_, int batch_size, int num_heads){
    ...
    SMITH_CHECK(!alibi_slopes_.has_value(), "This flash attention build does not support alibi.");
    params.alibi_slopes_ptr = nullptr;
#else
    if (alibi_slopes_.has_value()) {
        auto alibi_slopes = alibi_slopes_.value();
        SMITH_CHECK(alibi_slopes.dtype() == at::kFloat, "ALiBi slopes must have dtype fp32");
        CHECK_DEVICE(alibi_slopes);
        SMITH_CHECK(alibi_slopes.stride(-1) == 1, "ALiBi slopes tensor must have contiguous last dimension");
        SMITH_CHECK(alibi_slopes.sizes() == at::IntArrayRef({num_heads}) || alibi_slopes.sizes() == at::IntArrayRef({batch_size, num_heads}));
...
mha_fwd(const at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    SMITH_CHECK(is_sm80_or_newer, "FlashAttention only supports Ampere GPUs or newer.");

    auto q_dtype = q.dtype();
    SMITH_CHECK(q_dtype == at::kHalf || q_dtype == at::kBFloat16,
                "FlashAttention only support fp16 and bf16 data type");
    if (q_dtype == at::kBFloat16) {
        SMITH_CHECK(is_sm80_or_newer, "bfloat16 is only supported on Ampere GPUs or newer");
    }
    SMITH_CHECK(k.dtype() == q_dtype, "query and key must have the same dtype");
    SMITH_CHECK(v.dtype() == q_dtype, "query and value must have the same dtype");

    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v);

    SMITH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(k.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(v.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    ...
    SMITH_CHECK(batch_size > 0, "batch size must be positive");
    SMITH_CHECK(head_size_og % 8 == 0, "head_size must be a multiple of 8, this is ensured by padding!");
    SMITH_CHECK(head_size_og <= 256, "FlashAttention forward only supports head dimension at most 256");
    SMITH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");

    if (softcap > 0.f) { SMITH_CHECK(p_dropout == 0.f, "Softcapping does not support dropout for now"); }
    ...
    if (out_.has_value()) {
        out = out_.value();
        SMITH_CHECK(out.dtype() == q_dtype, "Output must have the same dtype as inputs");
        CHECK_DEVICE(out);
        SMITH_CHECK(out.stride(-1) == 1, "Output tensor must have contiguous last dimension");
    ...
    if (return_softmax) {
        SMITH_CHECK(p_dropout > 0.0f, "return_softmax is only supported when p_dropout > 0.0");
...
mha_varlen_fwd(const at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    SMITH_CHECK(k.dtype() == q_dtype, "query and key must have the same dtype");
    SMITH_CHECK(v.dtype() == q_dtype, "query and value must have the same dtype");
    SMITH_CHECK(cu_seqlens_q.dtype() == at::kInt, "cu_seqlens_q must have dtype int32");
    SMITH_CHECK(cu_seqlens_k.dtype() == at::kInt, "cu_seqlens_k must have dtype int32");
    ...
    if (paged_KV) {
        ...
        SMITH_CHECK(block_table.dtype() == at::kInt, "block_table must have dtype smith.int32");
        SMITH_CHECK(block_table.stride(-1) == 1, "block_table must have contiguous last dimension");
    }
    ...
    SMITH_CHECK(!paged_KV || page_block_size % 256 == 0, "Paged KV cache block size must be divisible by 256");
    ...
    if (seqused_k.has_value()){
        ...
        SMITH_CHECK(seqused_k_.dtype() == at::kInt, "seqused_k must have dtype int32");
        SMITH_CHECK(seqused_k_.is_cuda(), "seqused_k must be on CUDA device");
        SMITH_CHECK(seqused_k_.is_contiguous(), "seqused_k must be contiguous");
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
        ...
        SMITH_CHECK(false, "This flash attention build does not support backward.");
    #endif
    if (is_causal) { window_size_right = 0; }
    auto dprops = at::cuda::getCurrentDeviceProperties();
    bool is_sm80_or_newer = (dprops->major * 10) >= 80;
    ...
    SMITH_CHECK(k.dtype() == q_dtype, "query and key must have the same dtype");
    SMITH_CHECK(v.dtype() == q_dtype, "query and value must have the same dtype");
    SMITH_CHECK(out.dtype() == q_dtype, "query and out must have the same dtype");
    SMITH_CHECK(dout.dtype() == q_dtype, "query and dout must have the same dtype");
    ...
    SMITH_CHECK(k.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(v.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(out.stride(-1) == 1, "out tensor must have contiguous last dimension");
    ...
    SMITH_CHECK(dout.stride(-1) == 1, "dout tensor must have contiguous last dimension");

    SMITH_CHECK(batch_size > 0, "batch size must be positive");
    SMITH_CHECK(head_size % 8 == 0, "head_size should be a multiple of 8");
    SMITH_CHECK(head_size_og % 8 == 0, "head_size_og should be a multiple of 8, this is ensured by padding!");
    SMITH_CHECK(head_size <= 256, "FlashAttention backward only supports head dimension at most 256");
    if (head_size > 192 && (head_size <= 224 || is_dropout)) {
        SMITH_CHECK(is_sm80_or_newer, "FlashAttention backward for head dim 256 with dropout, or head dim 224 with/without dropout requires A100/A800 or H100/H800");
    }
    SMITH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
    ...
    SMITH_CHECK(head_size == round_multiple(head_size_og, 8), "head_size must be head_size_og rounded to a multiple of 8");
    ...
    if (dq_.has_value()) {
        dq = dq_.value();
        SMITH_CHECK(dq.dtype() == q_dtype, "dq must have the same dtype as q");
        CHECK_DEVICE(dq);
        SMITH_CHECK(dq.stride(-1) == 1, "dq must have contiguous last dimension");
    ...
    if (dk_.has_value()) {
        dk = dk_.value();
        SMITH_CHECK(dk.dtype() == q_dtype, "dk must have the same dtype as q");
        CHECK_DEVICE(dk);
        SMITH_CHECK(dk.stride(-1) == 1, "dk must have contiguous last dimension");
    ...
    if (dv_.has_value()) {
        dv = dv_.value();
        SMITH_CHECK(dv.dtype() == q_dtype, "dv must have the same dtype as q");
        CHECK_DEVICE(dv);
        SMITH_CHECK(dv.stride(-1) == 1, "dv must have contiguous last dimension");
    ...
    if (loop) {
        ...
        // dk_accum = smith::empty({batch_size, num_heads_k, seqlen_k_rounded, head_size_rounded}, opts.dtype(at::kFloat));
        // dv_accum = smith::empty({batch_size, num_heads_k, seqlen_k_rounded, head_size_rounded}, opts.dtype(at::kFloat));
    }
...
mha_fwd_kvcache(at::Tensor &q,                 // batch_size x seqlen_q x num_heads x head_size
    ...
    SMITH_CHECK(kcache.dtype() == q_dtype, "query and key must have the same dtype");
    SMITH_CHECK(vcache.dtype() == q_dtype, "query and value must have the same dtype");

    CHECK_DEVICE(q); CHECK_DEVICE(kcache); CHECK_DEVICE(vcache);

    SMITH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(kcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    SMITH_CHECK(vcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");

    at::Tensor block_table;
    const bool paged_KV = block_table_.has_value();
    if (paged_KV) {
        SMITH_CHECK(!cache_batch_idx_.has_value(), "Paged KVcache does not support cache_batch_idx");
        block_table = block_table_.value();
        CHECK_DEVICE(block_table);
        SMITH_CHECK(block_table.dtype() == at::kInt, "block_table must have dtype smith.int32");
        SMITH_CHECK(block_table.stride(-1) == 1, "block_table must have contiguous last dimension");
    ...
    if (k_.has_value()) {
        SMITH_CHECK(v_.has_value(), "If key is supplied, value must also be passed in");
        SMITH_CHECK(seqlens_k_.has_value(), "If key is supplied, seqlens_k must also be passed in");
        SMITH_CHECK(seqlen_q <= seqlen_k, "If key is supplied, it must have seqlen <= the seqlen of the KV cache");
        k = k_.value();
        v = v_.value();
        SMITH_CHECK(k.dtype() == q_dtype, "Key must have the same dtype as query");
        SMITH_CHECK(v.dtype() == q_dtype, "Value must have the same dtype as query");
        CHECK_DEVICE(k); CHECK_DEVICE(v);
        SMITH_CHECK(k.stride(-1) == 1, "Key tensor must have contiguous last dimension");
        SMITH_CHECK(v.stride(-1) == 1, "Value tensor must have contiguous last dimension");
    ...
    if (seqlens_k_.has_value()) {
        ...
        SMITH_CHECK(seqlens_k.dtype() == at::kInt, "seqlens_k must have dtype int32");
    ...
    if (rotary_cos_.has_value()) {
        SMITH_CHECK(k_.has_value(), "If rotary cos/sin are provided, new key / value to be appended to KV cache must also be provided");
        auto rotary_cos = rotary_cos_.value();
        CHECK_DEVICE(rotary_cos);
        params.rotary_dim = rotary_cos.size(1) * 2;
        SMITH_CHECK(params.rotary_dim <= head_size, "rotary_dim must be <= headdim");
        SMITH_CHECK(params.rotary_dim % 16 == 0, "Only rotary dimensions divisible by 16 are currently supported");
        const int seqlen_ro = rotary_cos.size(0);
        SMITH_CHECK(seqlen_ro >= seqlen_k, "cos/sin seqlen must be at least the seqlen of KV cache");
        CHECK_SHAPE(rotary_cos, seqlen_ro, params.rotary_dim / 2);
        CHECK_CONTIGUOUS(rotary_cos);
        SMITH_CHECK(rotary_cos.scalar_type() == q_dtype, "rotary_cos must have the same dtype as query");

        SMITH_CHECK(rotary_sin_.has_value(), "If rotary cos is provided, rotary sin must also be provided");
        auto rotary_sin = rotary_sin_.value();
        CHECK_DEVICE(rotary_sin);
        CHECK_SHAPE(rotary_sin, seqlen_ro, params.rotary_dim / 2);
        CHECK_CONTIGUOUS(rotary_sin);
        SMITH_CHECK(rotary_sin.scalar_type() == q_dtype, "rotary_cos must have the same dtype as query");
    ...
    if (cache_batch_idx_.has_value()) {
        ...
        SMITH_CHECK(cache_batch_idx.scalar_type() == at::kInt, "cache_batch_idx must have dtype int32");
