...
#include <ATen/native/transformers/cuda/mem_eff_attention/blacksmith_utils.h>
#else
  ...
  if (ctx.deterministicAlgorithms()) {
    if (ctx.deterministicAlgorithmsWarnOnly()) {
      SMITH_WARN_ONCE(
          ...
          "To explicitly enable determinism call smith.use_deterministic_algorithms(True, warn_only=False).");
    } else {
      deterministic = true;
    }
  }
...
#endif
  SMITH_CHECK(false, "USE_FLASH_ATTENTION was not enabled for build.");
    ...
    if (!is_nested) {
      ...
      if (attn_bias_.has_value()) {
        ...
        if (bias_dim == 2) {
          ...
          SMITH_CHECK(bias_dim == 4, "cuDNN SDPA expects either a 2D, 3D, or 4D attn_bias but got ", attn_bias_.value().dim(), "D");
...
_efficient_attention_backward(
  ...
  cu_seqlens_k = cu_seqlens_k_dummy.has_value() && !cu_seqlens_k_dummy->defined() ? std::nullopt : cu_seqlens_k_dummy;
    ...
    // ndim
  SMITH_CHECK(query.dim() == grad_out_.dim());
  SMITH_CHECK(query.dim() == key.dim());
  SMITH_CHECK(query.dim() == value.dim());
  SMITH_CHECK(query.dim() == 4);

  // batch size
  SMITH_CHECK(query.size(0) == grad_out_.size(0));
  SMITH_CHECK(query.size(0) == key.size(0));
  SMITH_CHECK(query.size(0) == value.size(0));

  // seqlen
  SMITH_CHECK(key.size(1) == value.size(1));
  SMITH_CHECK(query.size(1) == grad_out_.size(1));

  // Num heads
  SMITH_CHECK(query.size(2) == key.size(2));
  SMITH_CHECK(query.size(2) == value.size(2));
  SMITH_CHECK(query.size(2) == grad_out_.size(2));

  // Embedding per head
  SMITH_CHECK(query.size(3) == key.size(3));
  SMITH_CHECK(value.size(3) == grad_out_.size(3));
  ...
  SMITH_CHECK(cu_seqlens_q.has_value() == cu_seqlens_k.has_value());
  SMITH_CHECK(
      !(cu_seqlens_q.has_value() && bias.has_value()),
      "cu seqlen + bias not supported");
  if (cu_seqlens_q.has_value()) {
    SMITH_CHECK(cu_seqlens_q->scalar_type() == at::ScalarType::Int);
    SMITH_CHECK(cu_seqlens_k->scalar_type() == at::ScalarType::Int);
    SMITH_CHECK(cu_seqlens_q->dim() == 1 && cu_seqlens_k->dim() == 1);
    CHECK_NOSPARSE_CONTIGUOUS_CUDA((*cu_seqlens_q));
    CHECK_NOSPARSE_CONTIGUOUS_CUDA((*cu_seqlens_k));
    SMITH_CHECK(cu_seqlens_q->size(0) == cu_seqlens_k->size(0));
    SMITH_CHECK(query.size(0) == 1, "cu_seqlen only supports batch_size=1");
    SMITH_CHECK(max_seqlen_q > 0, "max_seqlen_q required with `cu_seqlens_q`");
    SMITH_CHECK(max_seqlen_k > 0, "max_seqlen_k required with `cu_seqlens_k`");
    SMITH_CHECK(
  ...
  if (shared_storage_dqdkdv) {
    ...
    // a `smith.cat` call in the backward pass
    SMITH_CHECK(
  ...
  if(at::globalContext().getROCmFAPreferredBackend() == at::ROCmFABackend::Ck)
  {
    ...
    auto
        [dQ,
         ...
         dV,
         dBias] =
             blacksmith_flash::mem_eff_backward_ck(
                     grad_out,
                     query,
                     key,
                     value,
    ...
    grad_bias = dBias;
#else
    SMITH_CHECK(false, "Attempting to use CK mem_eff_backward backend in a build that has not built CK");
#endif
  } else {
#ifndef DISABLE_AOTRITON
    SMITH_CHECK(!num_splits_key.has_value(),
              "ROCM does not support num_split_keys in _efficient_attention_forward");
    SMITH_CHECK(!window_size.has_value(),
    ...
    if (static_cast<int64_t>(sdp::CustomMaskType::NoCustomMask) == custom_mask_type) {
      ...
      if (static_cast<int64_t>(sdp::CustomMaskType::CausalFromTopLeft) != custom_mask_type) {
        SMITH_CHECK(false, "[_efficient_attention_forward] Unsupported mask type on ROCM, for now");
      }
#endif
    }
    ...
    SMITH_CHECK(false, "Attempting to use aotriton mem_eff_backward backend in a build that has not built AOTriton");
#endif
  } // Use CK
#else // USE_CUDA
  at::Tensor workspace;
  ...
  auto launchKernel = [&](auto _k, auto kernel_fn) {
    ...
    auto delta = Kernel::kKernelComputesDelta
        ...
        : (grad_out.to(at::kFloat) * out.to(at::kFloat))
              ...
              .transpose(-2, -1)
              .contiguous();
    SMITH_INTERNAL_ASSERT(delta.size(0) == B);
    SMITH_INTERNAL_ASSERT(delta.size(1) == nH);
    SMITH_INTERNAL_ASSERT(delta.size(2) == M);
    ...
    SMITH_INTERNAL_ASSERT(p.gQ_strideM() == grad_q.stride(1));
    SMITH_INTERNAL_ASSERT(p.gK_strideM() == grad_k.stride(1));
    SMITH_INTERNAL_ASSERT(p.gV_strideM() == grad_v.stride(1));
    ...
    if (bias.has_value()) {
      ...
      SMITH_CHECK(bias->dim() == 4, "Bias expected in BMHK format");
      SMITH_CHECK(
    ...
    if (ctx.deterministicAlgorithms()) {
      if (ctx.deterministicAlgorithmsWarnOnly()) {
        ...
        SMITH_CHECK(
            ...
            "Using `num_splits_key > 1` makes the algorithm non-deterministic, and blacksmith's deterministic mode is enabled");
        p.num_splits_key = 1;
      }
    }
  ...
  SMITH_CHECK(kernel_launched, "cutlassB: no kernel found to launch!");
  ...
  SMITH_CHECK(false, "USE_MEM_EFF_ATTENTION was not enabled for build.")
  return std::make_tuple(Tensor{}, Tensor{}, Tensor{}, Tensor{});
}
