...
#define DISPATCH_TYPES(tensor, func)                                           \
  {                                                                            \
    if (query.scalar_type() == at::ScalarType::Float) {                        \
      ...
      SMITH_CHECK(false, "Only fp32, half & bf16 supported at the moment"); \
    }                                                                          \
  }
...
#define CHECK_NOSPARSE_CONTIGUOUS_CUDA(TENSOR)                            \
  SMITH_CHECK(TENSOR.is_cuda(), #TENSOR " must be a CUDA tensor");     \
  SMITH_CHECK(!TENSOR.is_sparse(), #TENSOR " must be a dense tensor"); \
