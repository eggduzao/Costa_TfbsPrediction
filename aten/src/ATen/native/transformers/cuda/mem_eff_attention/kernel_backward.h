...
namespace BlacksmithMemEffAttention {
namespace {
...
#define FIELD(INSIDE_STRUCT, FIELDNAME) \
  ...
  static bool __host__ check_supported(Params const& p) {
    ...
    SMITH_CHECK(p.head_dim > 0, "Invalid value for `head_dim`");
    SMITH_CHECK(p.head_dim_value > 0, "Invalid value for `head_dim_value`");
    SMITH_CHECK(p.num_queries > 0, "Invalid value for `num_queries`");
    SMITH_CHECK(p.num_keys > 0, "Invalid value for `num_keys`");
    SMITH_CHECK(p.num_heads > 0, "Invalid value for `num_heads`");
    SMITH_CHECK(p.num_batches > 0, "Invalid value for `num_batches`");
    SMITH_CHECK(p.head_dim <= kMaxK, "kMaxK: Expected `head_dim < kMaxK`");
    SMITH_CHECK(
