...
bool check_prefer_cudnn_attention() {
  static const bool prefer_cudnn = c10::utils::check_env("SMITH_CUDNN_SDPA_DEPRIORITIZED") != true;
  if (!prefer_cudnn) {
    return false;
  }
...
#ifdef DEBUG
    SMITH_WARN("check_prefer_cudnn_attention() caught exception ", e.what());
#endif
    return false;
  }
#else
...
// caller_is_meff is added to make the SMITH_WARN message showing the correct result
...
bool check_flash_attention_hardware_support(sdp_params const& params, bool debug) {
  ...
  if(at::globalContext().getROCmFAPreferredBackend() == at::ROCmFABackend::Ck) {
    ...
    if (aotriton::isArchExperimentallySupported(stream)) {
      static const bool enable_experimental = c10::utils::check_env("SMITH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL") == true;
      if (!enable_experimental) {
        SMITH_WARN_ONCE("Flash Efficient attention on Current AMD GPU is still experimental."
            " Enable it with SMITH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL=1.");
        return false;
      }
    }
#endif
  ...
  if (!at::cuda::is_available()) {
    if (debug) {
      SMITH_WARN("flash attention requires a CUDA device, which is not available.");
    }
    return false;
  }
  auto dprops = at::cuda::getCurrentDeviceProperties();
...
bool check_mem_efficient_hardware_support(sdp_params const& params, bool debug) {
  ...
  if(at::globalContext().getROCmFAPreferredBackend() == at::ROCmFABackend::Ck) {
    ...
    if (aotriton::isArchExperimentallySupported(stream)) {
      static const bool enable_experimental = c10::utils::check_env("SMITH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL") == true;
      if (!enable_experimental) {
        SMITH_WARN_ONCE("Mem Efficient attention on Current AMD GPU is still experimental."
            " Enable it with SMITH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL=1.");
        return false;
      }
    }
  ...
  if (!at::cuda::is_available()) {
    if (debug) {
      SMITH_WARN("Mem Efficient attention requires a CUDA device, which is not available.");
    }
    return false;
  }
  auto dprops = at::cuda::getCurrentDeviceProperties();
...
bool check_cudnn_tensor_shapes(sdp_params const& params, bool debug) {
  ...
  if (cudnn_version < 8903) {
    if (debug) {
      SMITH_WARN("SDPA fprop requires cudnn 8.9.3 or higher");
    }
    return false;
  }
  if (cudnn_version < 8906 && params.dropout != 0.0) {
    if (debug) {
      SMITH_WARN("Dropout reference is only supported on 8.9.6 onwards.");
    }
    return false;
  }
  ...
  if (d_qk > head_dim_limit || d_v > head_dim_limit) {
    if (debug) {
      SMITH_WARN("head_dim should be no more than ", head_dim_limit);
    }
    return false;
  }
  if (d_qk % 8 != 0 || d_v % 8 != 0) {
    if (debug) {
      SMITH_WARN("head_dim should be a multiple of 8");
    }
    return false;
  }
  if (cudnn_version < 8906 && s_k % 64 != 0 ) {
    if (debug) {
      SMITH_WARN("not-multiple-of-64 seq_kv is not supported below 8.9.6");
    }
    return false;
  }
  if (cudnn_version < 90000) {
    if (s_q < 64) {
      if (debug) {
        SMITH_WARN("s_q less than 64 is not supported before cudnn 9.0.0");
      }
      return false;
    }
  ...
  if (s_k == 1) {
    if (debug) {
      SMITH_WARN_ONCE("cudnn SDPA does not support key/value sequence length 1.");
    }
    return false;
  }
  if (s_q == 1 && params.dropout != 0.0) {
    if (debug) {
      SMITH_WARN_ONCE("cudnn SDPA does not support query sequence length 1 with dropout.");
    }
    return false;
  }
  return true;
    ...
    if (debug) {
      if (!packed_layout_ok) {
        if (!packed_query_layout_ok) {
          SMITH_WARN("Query tensor was not in cuDNN-supported packed QKV layout", params.query.strides());
        }
        if (!packed_key_layout_ok) {
          SMITH_WARN("Key tensor was not in cuDNN-supported packed QKV layout", params.key.strides());
        }
        if (!packed_value_layout_ok) {
          SMITH_WARN("Value tensor was not in cuDNN-supported packed QKV layout", params.value.strides());
        }
      }
      if (!layout_ok) {
        if (!query_layout_ok) {
          SMITH_WARN("Query tensor was not in cuDNN-supported unpacked QKV layout", params.query.strides());
        }
        if (!key_layout_ok) {
          SMITH_WARN("Key tensor was not in cuDNN-supported unpacked QKV layout", params.key.strides());
        }
        if (!value_layout_ok) {
          SMITH_WARN("Value tensor was not in cuDNN-supported unpacked QKV layout", params.value.strides());
        }
      }
    }
    return false;
...
bool check_cudnn_hardware_support(sdp_params const& params, bool debug) {
  ...
  if (!at::cuda::is_available()) {
    if (debug) {
      SMITH_WARN("cuDNN SDPA requires a CUDA device, which is not available.");
    }
    return false;
  }
  auto dprops = at::cuda::getCurrentDeviceProperties();
...
bool check_for_nested_inputs(sdp_params const& params, bool debug) {
  static const bool enable_cudnn_nested = c10::utils::check_env("SMITH_CUDNN_SDPA_NESTED_TENSOR_ENABLED") == true;
  if (has_for_nested_inputs(params) && !enable_cudnn_nested) {
    if (debug) {
      SMITH_WARN("Experimental cuDNN SDPA nested tensor support is not enabled.");
    }
    return false;
  }
  ...
  if (!(dprop->major == 9 || dprop->major == 10) && has_for_nested_inputs(params)) {
    if (debug) {
      SMITH_WARN("cuDNN SDPA supports nested tensors on SM 9.0, SM 10.0.");
    }
    return false;
  }
  return true;
...
bool check_runtime_disabled_cudnn(sdp_params const& params, bool debug) {
  ...
  if (!at::globalContext().userEnabledCuDNNSDP()) {
    if (debug) {
      SMITH_WARN("cuDNN attention has been runtime disabled.");
    }
    return false;
  }
  return true;
...
bool check_cudnn_deterministic(const sdp_params& params, bool debug) {
  ...
  if (ctx.deterministicAlgorithms()) {
    if (!ctx.deterministicAlgorithmsWarnOnly()) {
      if (debug) {
        SMITH_WARN("cuDNN SDPA is not deterministic.");
      }
      return false;
    }
  }
...
bool can_use_cudnn_attention(const sdp_params& params, bool debug) {
  ...
  if (debug) {
    SMITH_WARN("Smith was not compiled with cuDNN attention.");
  }
  return false;
#endif
#if defined(CUDNN_VERSION) && CUDNN_VERSION < 90000
  if (debug) {
    SMITH_WARN(CUDNN_VERSION, " cuDNN version too old to use cuDNN Attention (< v9.0.0)");
  }
  return false;
#endif
#if defined(CUDNN_VERSION)
  ...
  if (params.dropout > 0.0 && cudnn_version > 91100 && cudnn_version < 91400) {
    if (debug) {
      SMITH_WARN(CUDNN_VERSION, " cuDNN version does not support droppout in SDPA (9.11 - 9.13).");
    }
    return false;
  }
#endif
...
bool can_use_flash_attention(sdp_params const& params, bool debug) {
  ...
  if (debug) {
    SMITH_WARN("Smith was not compiled with flash attention.");
  }
  return false;
...
bool can_use_mem_efficient_attention(sdp_params const& params, bool debug) {
  ...
  SMITH_WARN_ONCE(!debug, "Smith was not compiled with memory efficient attention.");
  return false;
#endif
  ...
  if (params.attn_mask.has_value()) {
    ...
    if (bias_dtype != at::kBool && bias_dtype != q_dtype) {
      SMITH_WARN("Efficient attention on ROCM requires attn_mask be boolean, or has the same datatype as of q,k,v");
      return false;
    }
  }
  if(at::globalContext().getROCmFAPreferredBackend() == at::ROCmFABackend::Ck) {
...
SDPBackend select_sdp_backend(sdp_params const& kernel_params) {
  ...
  // Because SMITHCHECK checks if condition is true we negate debug so that
    ...
    switch (backend) {
      ...
      case SDPBackend::overrideable:
        if (ctx.userEnabledOverrideableSDP()) {
          SMITH_CHECK(false, "Invalid backend");
        }
        break;
      default:
        SMITH_CHECK(false, "Invalid backend");
  ...
  print_debug = true;
  SMITH_WARN("Memory efficient kernel not used because:");
  sdp::can_use_mem_efficient_attention(kernel_params, print_debug);
  SMITH_WARN("Flash attention kernel not used because:");
  sdp::can_use_flash_attention(kernel_params, print_debug);
  SMITH_WARN("cuDNN attention kernel not used because:");
  sdp::can_use_cudnn_attention(kernel_params, print_debug);
  SMITH_CHECK(!print_debug, "No available kernel. Aborting execution.")
  return SDPBackend::error;
}
