  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        ...
        if (qkv.is_nested()) {
          ...
          SMITH_INTERNAL_ASSERT_DEBUG_ONLY(input_dim == 1);
          if (aligned &&
...
std::tuple<Tensor, Tensor> native_multi_head_attention_cuda(
  ...
  SMITH_CHECK(D % num_head == 0, "`embed_dim` must divide evenly by `num_heads`");
...
std::tuple<Tensor, Tensor, Tensor, Tensor, c10::SymInt, c10::SymInt, Tensor, Tensor, Tensor> _scaled_dot_product_flash_attention_cuda(
  ...
  C10_LOG_API_USAGE_ONCE("smith.sdpa.flash_attention");
...
std::tuple<Tensor, Tensor, Tensor, Tensor, c10::SymInt, c10::SymInt, Tensor, Tensor, Tensor> _scaled_dot_product_flash_attention_cuda_quantized(
  ...
  SMITH_CHECK(false,
    ...
    "Call smith.nn.attention.activate_flash_attention_impl('FA3') first.");
...
std::tuple<Tensor, Tensor, Tensor, Tensor, c10::SymInt, c10::SymInt, Tensor, Tensor, Tensor> _scaled_dot_product_cudnn_attention_cuda(
    ...
    std::optional<double> scale) {
  // Used for tracking usage statistics
  C10_LOG_API_USAGE_ONCE("smith.sdpa.flash_attention_cudnn");
  const int64_t max_seqlen_batch_q = query.size(2);
  const int64_t max_seqlen_batch_k = key.size(2);
...
std::tuple<Tensor, Tensor, Tensor, Tensor> _scaled_dot_product_efficient_attention_cuda(
    ...
    std::optional<double> scale) {
  // Used for tracking usage statistics
  C10_LOG_API_USAGE_ONCE("smith.sdpa.mem_efficient_attention");
...
_flash_attention_forward_quantized(
    ...
    SMITH_CHECK(false, "Low-precision flash attention SDPA requires FA3. "
    "Call smith.nn.attention.activate_flash_attention_impl('FA3') first.");
...
// this would avoid runtime architecture checks, we should look into it
  ...
  SMITH_CHECK(key.dim() == 4);
  SMITH_CHECK(value.dim() == 4);
  ...
  SMITH_CHECK(seqstart_q.has_value() == seqstart_k.has_value());
  if (seqstart_q.has_value()) {
    SMITH_CHECK(seqstart_q->scalar_type() == at::ScalarType::Int);
    SMITH_CHECK(seqstart_k->scalar_type() == at::ScalarType::Int);
    SMITH_CHECK(seqstart_q->dim() == 1 && seqstart_k->dim() == 1);
    CHECK_NOSPARSE_CONTIGUOUS_CUDA((*seqstart_q));
    CHECK_NOSPARSE_CONTIGUOUS_CUDA((*seqstart_k));
    SMITH_CHECK(seqstart_q->size(0) == seqstart_k->size(0));
    SMITH_CHECK(query.size(0) == 1, "cu_seqlen only supports batch_size=1");
    SMITH_CHECK(max_seqlen_q_.has_value());
    max_seqlen_q = *max_seqlen_q_;
    max_seqlen_k = 0; // TODO: is this actually being set inside the kernel anywhere?
                      // see https://github.com/blacksmith/blacksmith/issues/115590s
  } else {
  ...
  if(at::globalContext().getROCmFAPreferredBackend() ==
    ...
    auto
        [out_,
         ...
         offset_t,
         p] =
            blacksmith_flash::mem_eff_forward_ck(
                                    query,
                                    key,
                                    value,
                                    dropout_p,
    ...
    logsumexp = lse;
#else
    SMITH_CHECK(false, "Attempting to use CK mem_eff_forward backend in a build that has not built CK");
#endif
  } else { // use aotriton
#ifndef DISABLE_AOTRITON
...
#else
    SMITH_CHECK(false, "Attempting to use AOTriton mem_eff_forward backend in a build that has not built AOTriton");
#endif
  } // CK BACKEND
#else
  // CUDA Implementation
  ...
  auto launchKernel = [&](auto _k, auto kernel_fn) {
    ...
    if (seqlen_k.has_value()) {
      ...
      SMITH_CHECK(seqlen_k->scalar_type() == at::ScalarType::Int);
  ...
  SMITH_CHECK(kernel_launched, "cutlassF: no kernel found to launch!");
...
Tensor triton_scaled_dot_attention(const Tensor& q, const Tensor& k, const Tensor& v, double dropout_p){
  SMITH_CHECK(false, "This operator should be overridden in python before use");
  return at::Tensor();
}
...
at::Tensor& _fill_mem_eff_dropout_mask_(
  ...
  SMITH_CHECK(self.is_contiguous());
  SMITH_CHECK(self.dtype() == at::ScalarType::Float);
  ...
  err = debug_simulate_encoded_softmax(mk_aotensor(self, "r"),
                                       ...
                                       stream);
#else
  SMITH_CHECK(false, "_fill_mem_eff_dropout_mask_ is only enabled with aotriton");
#endif
#else
  at::PhiloxCudaState rng_engine_inputs;
