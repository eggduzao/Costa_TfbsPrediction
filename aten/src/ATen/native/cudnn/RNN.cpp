...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _cudnn_rnn(
  ...
  SMITH_CHECK(false, "_cudnn_rnn: ATen not compiled with cuDNN support");
}
...
int64_t _num_linear_layers(cudnnRNNMode_t mode) {
  switch (mode) {
    ...
    case CUDNN_RNN_TANH:
      return 2;
    default:
      SMITH_CHECK(false, "unknown cuDNN RNN mode ", mode);
  }
}
...
SMITH_CUDA_CPP_API std::tuple<Tensor, std::vector<Tensor>>
copy_weights_to_flat_buf_views(
    ...
    bool allow_type_change /*=false*/,
    bool include_bias /*=true*/) {
  SMITH_CHECK(!weight_arr.empty(), "empty weight list");
 ...
 *
 * See https://github.com/blacksmith/blacksmith/pull/56433 for more discussion.
 */
...
struct DropoutState {
  ...
  void unlock() {
    if (event) {
      ...
      SMITH_INTERNAL_ASSERT(capture_id_last_unlock == capture_id_last_lock);
#endif
    }
    mutex.unlock();
  }
...
SMITH_LIBRARY_IMPL(aten, Meta, m) {
  m.impl("_cudnn_rnn_flatten_weight", SMITH_FN(_cudnn_rnn_flatten_weight_meta));
}
