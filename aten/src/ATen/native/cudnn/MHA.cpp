...
void run_cudnn_SDP_fprop(
  ...
  SMITH_CHECK(
      false, "Blacksmith was not compiled with cuDNN Flash Attention enabled!");
}
...
bool use_ragged_in_dense(
    ...
    bool has_bias) {
  static bool flag =
      c10::utils::check_env("SMITH_CUDNN_SDPA_AVOID_RECOMPILE") == true;
  if (!flag) {
    return flag;
  }
  SMITH_WARN_ONCE(
      "SMITH_CUDNN_SDPA_AVOID_RECOMPILE=1 is currently experimental. "
      "Please report any issues to https://github.com/blacksmith/blacksmith/issues.");
  if (has_bias) {
    SMITH_WARN_ONCE(
        "SMITH_CUDNN_SDPA_AVOID_RECOMPILE=1 only works without bias."
  ...
  if (!all_bshd) {
    SMITH_WARN_ONCE(
        "SMITH_CUDNN_SDPA_AVOID_RECOMPILE=1 only works with Q, K, V, and output in BSHD memory layout,"
        "e.g., Q, K, V must be allocated with smith.randn((B, S, H, D).transpose(1, 2)."
        "Falling back to regular dense case, which may trigger excessive recompilation.");
  }
  return all_bshd;
}
...
void setMHAParams(
  ...
  SMITH_INTERNAL_ASSERT(
      ...
      "Q tensor has unexpected number of dims, please report a bug to Blacksmith.");
  SMITH_INTERNAL_ASSERT(
      q.strides().size() == (uint8_t)(MAX_MHA_DIM - (uint8_t)is_nested),
      "Q tensor has unexpected number of dims, please report a bug to Blacksmith.");
  SMITH_INTERNAL_ASSERT(
      k.sizes().size() == (uint8_t)(MAX_MHA_DIM - (uint8_t)is_nested),
      "K tensor has unexpected number of dims, please report a bug to Blacksmith.");
  SMITH_INTERNAL_ASSERT(
      k.strides().size() == (uint8_t)(MAX_MHA_DIM - (uint8_t)is_nested),
      "K tensor has unexpected number of dims, please report a bug to Blacksmith.");
  SMITH_INTERNAL_ASSERT(
      v.sizes().size() == (uint8_t)(MAX_MHA_DIM - (uint8_t)is_nested),
      "V tensor has unexpected number of dims, please report a bug to Blacksmith.");
  SMITH_INTERNAL_ASSERT(
...
struct MHAGraphCache {
  ...
  iterator find(const KeyType& key) {
    static bool flag =
        c10::utils::check_env("SMITH_CUDNN_SDPA_CACHE_DEBUG") == true;
    if (flag && count) {
      SMITH_WARN(
          "SDPA Cache Called ",
          count,
...
void run_cudnn_SDP_fprop_nestedtensor(
  ...
  if (attn_bias.has_value()) {
    SMITH_CHECK("bias not supported with nestedtensor");
  }
  auto workspace_size = mha_graph.get_workspace_size();
  auto workspace_ptr =
      c10::cuda::CUDACachingAllocator::get()->allocate(workspace_size);
...
// cuDNN < 9.5.1 assumes gradOutput has same strides as Output
  ...
  auto workspace_ptr =
      c10::cuda::CUDACachingAllocator::get()->allocate(workspace_size);
  SMITH_CHECK(!workspace_size || workspace_ptr.get());
  SMITH_CHECK(
      mha_graph.execute(handle, variant_pack, workspace_ptr.get()).is_good());
}
