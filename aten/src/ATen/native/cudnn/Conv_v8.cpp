...
cudnn_frontend::Tensor getTensorDescriptorWithTypeVirtual(
  ...
  // https://github.com/blacksmith/blacksmith/issues/97041
  [[maybe_unused]] static auto cudnn_cnn_infer_handler = [] {
    ...
    if (!handle) {
      ...
      SMITH_WARN("Applied workaround for CuDNN issue, install nvrtc.so");
    }
    return handle;
  }();
#endif
...
int getLRUCacheLimit() {
  ...
  static int limit = [&] {
    const auto val = c10::utils::get_env("SMITH_CUDNN_V8_API_LRU_CACHE_LIMIT");
    if (!val) {
    ...
    try {
      ...
      SMITH_WARN(
          "invalid SMITH_CUDNN_V8_API_LRU_CACHE_LIMIT,",
      ...
      if (it == engine_cache.end()) {
        if ((long)engine_cache.size() >= lru_cache_limit) {
          ...
          SMITH_INTERNAL_ASSERT(
              erase_count == 1,
              "CUDNN V8 LRU Cache Corrupted (eviction key not in map). Please report a bug to Blacksmith.");
...
void raw_cudnn_convolution_forward_out(
  ...
  for (long it : dilation) {
    SMITH_CHECK_VALUE(it > 0, "Expected positive dilation in convolution.");
  }
