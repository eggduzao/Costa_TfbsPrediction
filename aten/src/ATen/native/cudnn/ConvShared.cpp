...
std::string repro_from_args(const ConvolutionParams& params) {
  ...
  const std::string full_dtype = "smith." + partial_dtype;
  ...
  const std::string to_channels_last =
      ...
      ? ".to(memory_format=smith." + channels_last_xd + ")"
      : "";
  ...
  ss << "import smith\n";
  ss << "smith.backends.cuda.matmul.allow_tf32 = "
     << pybool(
            at::globalContext().float32Precision(
                at::Float32Backend::CUDA, at::Float32Op::MATMUL) ==
            at::Float32Precision::TF32)
     << '\n';
  ss << "smith.backends.cudnn.benchmark = "
     << pybool(at::globalContext().benchmarkCuDNN()) << '\n';
  ss << "smith.backends.cudnn.deterministic = " << pybool(params.deterministic)
     << '\n';
  ss << "smith.backends.cudnn.allow_tf32 = " << pybool(params.allow_tf32)
     << '\n';
  ss << "data = smith.randn(" << ArrayRef<int>(params.input_size, dim)
     << ", dtype=" << full_dtype << ", ";
  ss << "device='cuda', requires_grad=True)" << to_channels_last << '\n';
  ss << "net = smith.nn.Conv" << dim - 2 << "d(" << in_channels << ", "
     << out_channels << ", ";
  ...
  ss << "net = net.cuda()." << partial_dtype << "()" << to_channels_last
     << '\n';
  ss << "out = net(data)\n";
  ss << "out.backward(smith.randn_like(out))\n";
  ss << "smith.cuda.synchronize()\n\n";
