...
SMITH_META_FUNC(lerp_Tensor)(
    const Tensor& self, const Tensor& end, const Tensor& weight) {
  SMITH_CHECK(self.dtype() == end.dtype(), "expected dtype ", self.dtype(),
              " for `end` but got dtype ", end.dtype());
  bool promote_weight = weight.dim() == 0;
  if (!promote_weight) {
    SMITH_CHECK(self.dtype() == weight.dtype(), "expected dtype ", self.dtype(),
