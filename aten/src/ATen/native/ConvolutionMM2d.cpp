...
inline void slow_conv2d_shape_check(
  ...
  if (weight.defined()) {
    ...
    SMITH_CHECK(weight_optional, "weight tensor is undefined");
  }
  ...
  SMITH_CHECK(ndim == 4, "Expected 4D input tensor, but got: ", input.sizes());
  for (const auto dim : c10::irange(2, ndim)) {
    SMITH_CHECK(input.size(dim) != 0,
  ...
  if (grad_output.defined()) {
    if (weight.defined()) {
      ...
      SMITH_CHECK(bias.numel() > 0, "non-empty bias tensor expected");
...
void slow_conv2d_backward_out_cpu_template(
  ...
  grad_input.zero_();
  SMITH_CHECK(grad_input.is_contiguous(memory_format), "slow_conv2d: grad_input must be contiguous");
...
void slow_conv2d_backward_weight_out_cpu_template(
  ...
  SMITH_CHECK(grad_weight.is_contiguous(memory_format), "slow_conv2d: grad_weight must be contiguous");
...
Tensor& slow_conv2d_forward_out_cpu(
  ...
  SMITH_CHECK(kernel_size.size() == 2, "2D kernel_size expected");
  SMITH_CHECK(stride.size() == 2, "2D stride expected");
  SMITH_CHECK(padding.size() == 2, "2D padding expected");
  ...
  SMITH_CHECK(output.is_contiguous(memory_format), "slow_conv2d output tensor must be contiguous");
