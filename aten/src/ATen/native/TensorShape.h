...
SMITH_API at::Tensor clone_preserve_strides(const at::Tensor& self);
...
inline int64_t preprocess_chunk_cat_inputs(
    ...
    int64_t dim,
    int64_t num_chunks) {
  SMITH_CHECK(num_chunks >= 1, "_chunk_cat expects positive num_chunks");
  SMITH_CHECK(
      !tensors.empty(), "_chunk_cat expects a non-empty input tensor list");
  auto expected_dtype = tensors[0].dtype();
  auto expected_device = tensors[0].device();
  for (const auto i : c10::irange(tensors.size())) {
    SMITH_CHECK(tensors[i].numel() > 0, "_chunk_cat expects non-empty tensor");
    SMITH_CHECK(
