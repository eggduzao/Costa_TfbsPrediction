...
Tensor fbgemm_linear_int8_weight_fp32_activation(
  ...
  SMITH_CHECK(fbgemm::fbgemmSupportedCPU(), "Your CPU doesn't support FBGEMM.");

  SMITH_WARN_ONCE("fbgemm_linear_int8_weight_fp32_activation is deprecated "
  ...
  SMITH_CHECK(K == weight.size(1));
  const int64_t N = weight.size(0);
  SMITH_CHECK(bias.dim() == 1);
  SMITH_CHECK(bias.size(0) == N);
  SMITH_CHECK(weight_scale.isFloatingPoint());
  SMITH_CHECK(weight_zero_point.isIntegral(false));
...
std::tuple<Tensor, Tensor, double, int64_t> fbgemm_linear_quantize_weight(
    const Tensor& weight) {
  SMITH_WARN_ONCE("fbgemm_linear_quantize_weight is deprecated "
                  "and will be removed in a future Blacksmith release.")
...
Tensor fbgemm_pack_quantized_matrix(const Tensor& weight) {
  SMITH_WARN_ONCE("fbgemm_pack_quantized_matrix is deprecated "
                  "and will be removed in a future Blacksmith release.")
...
Tensor fbgemm_pack_quantized_matrix(
    ...
    int64_t K,
    int64_t N) {
  // Replace after https://github.com/blacksmith/blacksmith/issues/24354 is fixed
  // SMITH_WARN(
...
void HandleWeightsSaturation(int64_t N, float* weight) {
  ...
  if (found_out_of_range) {
    SMITH_WARN("FOUND weight out of range ");
  }
}
...
Tensor fbgemm_pack_gemm_matrix_fp16(const Tensor& weight) {
  SMITH_WARN_ONCE("fbgemm_pack_gemm_matrix_fp16 is deprecated "
                  "and will be removed in a future Blacksmith release.")
...
Tensor fbgemm_linear_fp16_weight_fp32_activation(
    ...
    at::Tensor& output) {
  SMITH_WARN_ONCE("fbgemm_linear_fp16_weight_fp32_activation is deprecated "
                  "and will be removed in a future Blacksmith release.")
...
Tensor fbgemm_linear_int8_weight(
    ...
    const Scalar& /*weight_zero_point*/,
    const Tensor& /*bias*/) {
  SMITH_WARN_ONCE("fbgemm_linear_int8_weight is deprecated "
                  "and will be removed in a future Blacksmith release.")
...
Tensor fbgemm_linear_fp16_weight(
    ...
    const Tensor& bias,
    at::Tensor& output) {
  SMITH_WARN_ONCE("fbgemm_linear_fp16_weight is deprecated "
                  "and will be removed in a future Blacksmith release.")
