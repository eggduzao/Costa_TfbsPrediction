...
namespace at::meta {
SMITH_META_FUNC(nll_loss_forward)
(const Tensor& self,
 const Tensor& target,
...
SMITH_META_FUNC(nll_loss_backward)
...
void nll_loss_backward_out_frame(
  ...
  if (input.dim() == 1) {
    ...
    if (t != ignore_index) {
      SMITH_CHECK_INDEX(t >= 0 && t < n_classes, "Target ", t, " is out of bounds.");
...
SMITH_IMPL_FUNC(nll_loss_forward_out_cpu)
(const Tensor& self,
 const Tensor& target,
 const OptionalTensorRef weight_opt,
 int64_t reduction,
...
SMITH_IMPL_FUNC(nll_loss_backward_out_cpu)
(const Tensor& grad_output,
 const Tensor& self,
 const Tensor& target,
 OptionalTensorRef weight_opt,
...
static Tensor cross_entropy_loss_prob_target(
  ...
  if (label_smoothing > 0.0) {
    SMITH_CHECK(label_smoothing <= 1.0, "label_smoothing must be between 0.0 and 1.0. Got: ", label_smoothing);
  ...
  if (weight.defined()) {
    ...
    switch (reduction) {
      ...
      default:
        SMITH_CHECK(false, "Invalid reduction type encountered in cross_entropy: ", reduction);
    }
  } else {
    switch (reduction) {
      case Reduction::Mean:
...
Tensor cross_entropy_loss_symint(
  ...
  if (self.sym_sizes() == target.sym_sizes()) {
    ...
    SMITH_CHECK(at::isFloatingType(target.scalar_type()),
        "Expected floating point type for target with class probabilities, got ", target.scalar_type());
    SMITH_CHECK(ignore_index < 0, "ignore_index is not supported for floating point target");
