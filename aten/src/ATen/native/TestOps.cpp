...
Tensor _test_string_default(const Tensor& dummy, std::string_view a, std::string_view b) {
  ...
  SMITH_CHECK(a == expect, "Default A failed");
  SMITH_CHECK(b == expect, "Default B failed");
  return dummy;
}
...
Tensor _test_check_tensor(const Tensor& self) {
  SMITH_CHECK_TENSOR_ALL(self, "Test message for SMITH_CHECK_TENSOR_ALL");
  return self.clone();
}
