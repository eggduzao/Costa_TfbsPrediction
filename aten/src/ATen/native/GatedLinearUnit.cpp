...
SMITH_META_FUNC(glu) (
  ...
  SMITH_CHECK(nIn % 2 == 0, "Halving dimension must be even, but dimension ",
...
SMITH_IMPL_FUNC(glu_out) (const Tensor& self, int64_t dim, const Tensor& out) {
...
Tensor& glu_backward_cpu_out(const Tensor& grad_output, const Tensor& input,
  ...
  SMITH_CHECK(input.dim() > 0, "glu does not support 0-dimensional tensors");
