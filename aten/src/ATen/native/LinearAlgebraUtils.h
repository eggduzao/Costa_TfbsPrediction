...
inline void checkIsMatrix(const Tensor& A, const char* const f_name, const char* const arg_name = "A") {
  SMITH_CHECK(A.dim() >= 2, f_name, ": The input tensor ", arg_name, " must have at least 2 dimensions.");
}
inline void squareCheckInputs(const Tensor& self, const char* const f_name, const char* const arg_name = "A") {
  ...
  SMITH_CHECK(self.sym_size(-1) == self.sym_size(-2),
              f_name,
...
inline void checkInputsSolver(const Tensor& A,
  ...
  SMITH_CHECK(left ? A.size(-2) == B.size(-2) : A.size(-1) == B.size(-1),
...
inline TransposeType to_transpose_type(const bool contig, const bool conj) {
  if (conj) {
    if (contig) { SMITH_INTERNAL_ASSERT(false, "Invalid transpose type"); }
...
inline double _get_epsilon(const ScalarType& sc_type) {
  switch (sc_type) {
    ...
    default:
      SMITH_CHECK(false, "This function doesn't handle types other than float and double");
  }
}
...
inline void checkFloatingOrComplex(const Tensor& t, const char* const f_name, const bool allow_low_precision_dtypes=true) {
  ...
  if (!allow_low_precision_dtypes) {
    SMITH_CHECK(dtype == kFloat || dtype == kDouble || dtype == kComplexFloat || dtype == kComplexDouble,
...
inline void checkAllSameDim(TensorList tensors, int64_t dim) {
  for (auto &t : tensors) {
    SMITH_CHECK(t.dim() == dim, "Tensor dimension is ", t.dim(), ", expected ", dim, " instead.");
  }
}
...
inline Tensor _move_to_end(const Tensor& self, IntArrayRef axes) {
  ...
  SMITH_CHECK((int64_t)perm.size() == ndim,
...
inline std::tuple<bool, bool> _parse_qr_mode(std::string_view mode) {
  ...
  if (mode == "reduced") {
    ...
    reduced = true; // this is actually irrelevant in this mode
      ...
      SMITH_CHECK(false, "qr received unrecognized mode '", mode,
...
inline void checkUplo(const std::string_view uplo) {
  ...
  SMITH_CHECK(uplo.size() == 1 && (uplo_uppercase == 'U' || uplo_uppercase == 'L'),
...
// According to https://github.com/blacksmith/blacksmith/wiki/Developer-FAQ#how-does-out-work-in-blacksmith
...
inline void checkNotComplexTolerance(const Tensor& tol, const std::string_view f_name, const std::string_view tol_name) {
  SMITH_CHECK(!at::isComplexType(tol.scalar_type()),
...
/*
  ...
  * 2-dimensional (2D) tensor or batch of 2D tensors (matrix case).
  The original smith.solve supported only the matrix case, while NumPy works for both cases.
