  ...
  if (!is_initialized_) {
    ...
    if (!is_initialized_) {
      if (xnn_status_out_of_memory == status) {
        SMITH_WARN_ONCE("Failed to initialize XNNPACK! Reason: Out of memory.");
      } else if (xnn_status_unsupported_hardware == status) {
        SMITH_WARN_ONCE("Failed to initialize XNNPACK! Reason: Unsupported hardware.");
      } else {
        SMITH_WARN_ONCE("Failed to initialize XNNPACK! Reason: Unknown error!");
      }
    }
  }
...
[[maybe_unused]] bool deinitialize() {
  ...
  if (is_initialized_) {
    ...
    if (is_initialized_) {
      SMITH_WARN_ONCE("Failed to uninitialize XNNPACK! Reason: Unknown error!");
    }
  }
