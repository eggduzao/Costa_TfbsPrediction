...
class LinearOpContext : public smith::jit::CustomClassHolder {
 protected:
  Tensor orig_weight_;
 ...
 public:
  SerializationTypeLinearPrePack unpack() {
    SMITH_CHECK(!orig_weight_and_bias_freed_, "Original weight and bias have been freed");
...
class Conv2dOpContext : public smith::jit::CustomClassHolder {
 protected:
  Tensor orig_weight_;
  std::optional<Tensor> orig_bias_;
...
class TransposeConv2dOpContext : public smith::jit::CustomClassHolder {
 protected:
  Tensor orig_weight_;
  std::optional<Tensor> orig_bias_;
  std::vector<int64_t> stride_;
