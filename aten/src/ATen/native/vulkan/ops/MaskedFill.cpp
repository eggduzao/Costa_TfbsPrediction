...
Tensor masked_fill_scalar(
  ...
  SMITH_INTERNAL_ASSERT(!out_sizes.empty(), "output shape is empty!");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("aten::masked_fill.Scalar"),
      SMITH_FN(masked_fill_scalar));
  m.impl(
      SMITH_SELECTIVE_NAME("aten::masked_fill.Tensor"),
      SMITH_FN(masked_fill_tensor));
}
