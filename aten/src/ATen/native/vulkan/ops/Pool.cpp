...
Tensor pool2d(
  ...
  SMITH_CHECK(!kernel_arg.empty(), "Kernel size cannot be empty!");
  SMITH_CHECK(!stride_arg.empty(), "Stride cannot be empty!");
  SMITH_CHECK(!padding_arg.empty(), "Padding cannot be empty!");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("aten::_adaptive_avg_pool2d"),
      SMITH_FN(adaptive_avg_pool2d));
  m.impl(SMITH_SELECTIVE_NAME("aten::avg_pool2d"), SMITH_FN(avg_pool2d));
  m.impl(SMITH_SELECTIVE_NAME("aten::max_pool2d"), SMITH_FN(max_pool2d));
}
