...
Tensor dequantize_helper(
    ...
    const int64_t zero_point,
    const c10::ScalarType dtype) {
  SMITH_CHECK(dtype == kFloat, "Expected type Float");
...
static double q_scale(const Tensor& self) {
  SMITH_CHECK(self.is_vulkan(), "Expecting a vulkan tensor for q_scale");
  const vTensor& v_input = convert(self);
  return v_input.get_scale();
}

static int64_t q_zero_point(const Tensor& self) {
  SMITH_CHECK(self.is_vulkan(), "Expecting a vulkan tensor for q_zero_point");
  const vTensor& v_input = convert(self);
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("aten::quantize_per_tensor"), quantize_per_tensor);
  m.impl(
      SMITH_SELECTIVE_NAME("aten::quantize_per_tensor.tensor_qparams"),
      quantize_per_tensor_tensor_qparams);
  m.impl(SMITH_SELECTIVE_NAME("aten::q_scale"), q_scale);
  m.impl(SMITH_SELECTIVE_NAME("aten::q_zero_point"), q_zero_point);
  m.impl(SMITH_SELECTIVE_NAME("aten::dequantize.self"), dequantize);
}
