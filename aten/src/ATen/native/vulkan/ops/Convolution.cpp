...
std::array<int64_t, 2> pack_params(const std::vector<int64_t>& vector) {
  SMITH_INTERNAL_ASSERT(2u == vector.size(), "Invalid usage!");
...
static vTensor pack_weights_using_width_packing(const Tensor& weight_arg) {
  ...
  SMITH_CHECK(weight.is_vulkan(), "Weight must be on Vulkan device!");
...
static Tensor run_conv1d_context_impl(
  ...
  SMITH_CHECK(input.dim() == 3, "input must be a 3-dim tensor");
  SMITH_CHECK(weight.dim() == 3, "weight must be a 3-dim tensor");
  SMITH_CHECK(
...
static Tensor run_conv2d_context_impl(
  ...
  SMITH_CHECK(input_arg.is_vulkan(), "Input tensor must be Vulkan!");
...
Conv2dOpContext::State Conv2dOpContext::unpack() const {
  ...
  SMITH_CHECK(!unpacked_.empty(), "unpacked_ does not have any elements!");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("aten::conv1d"), SMITH_FN(convolution1d));
}
