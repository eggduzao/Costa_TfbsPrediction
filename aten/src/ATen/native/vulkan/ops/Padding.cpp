...
Tensor pad2d(
  ...
  SMITH_CHECK(input_dim >= 2, "Input tensor must have dim >= 2!");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(
      SMITH_SELECTIVE_NAME("aten::reflection_pad2d"),
      SMITH_FN(reflection_pad2d));
  m.impl(
      SMITH_SELECTIVE_NAME("aten::replication_pad2d"),
      SMITH_FN(replication_pad2d));
}
