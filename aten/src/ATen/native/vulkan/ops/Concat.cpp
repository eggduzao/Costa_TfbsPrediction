...
Tensor cat_width(const MaterializedITensorListRef& tensors, vTensor& v_output) {
  // SMITH_CHECK(false, "Vulkan cat not implemented for width dimension!");
  api::Context* const context = api::context();
...
Tensor cat(const at::ITensorListRef& tensors, const int64_t in_dim) {
  SMITH_CHECK(!tensors.empty(), "Vulkan cat expects at least one tensor");
  auto materialized = tensors.materialize();
  SMITH_INTERNAL_ASSERT(!materialized.empty(), "Accessing empty array");
  ...
  SMITH_INTERNAL_ASSERT(!result_size.empty(), "Accessing empty array");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::cat"), SMITH_FN(cat));
}
