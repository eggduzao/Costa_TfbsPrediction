...
static Tensor& normal_(
  ...
  SMITH_CHECK(std >= 0, "Vulkan: Standard deviation (std) can be negative.");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::uniform_"), SMITH_FN(uniform_));
  m.impl(SMITH_SELECTIVE_NAME("aten::rand_like"), SMITH_FN(rand_like));
  m.impl(SMITH_SELECTIVE_NAME("aten::normal_"), SMITH_FN(normal_));
  m.impl(SMITH_SELECTIVE_NAME("aten::randn_like"), SMITH_FN(randn_like));
}
