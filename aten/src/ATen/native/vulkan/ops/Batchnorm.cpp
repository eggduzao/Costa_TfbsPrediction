...
Tensor batch_norm(
  ...
  SMITH_CHECK(!training, "Only evaluation mode is supported!");
  SMITH_CHECK(input_arg.dim() == 4, "Input must have dim == 4!");
  SMITH_CHECK(
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::batch_norm"), SMITH_FN(batch_norm));
}
...
BatchNormPackedContext::BatchNormPackedContext(
  ...
  // Weight
  SMITH_CHECK(weight_opt, "Weight must be provided!");
  SMITH_CHECK(weight_opt->dim() == 1, "Weight must have ndim == 1!");
  ...
  // Bias
  SMITH_CHECK(bias_opt, "Bias must be provided!");
  SMITH_CHECK(bias_opt->dim() == 1, "Bias must have ndim == 1!");
  SMITH_CHECK(
  ...
  // Running Mean
  SMITH_CHECK(running_mean_opt, "Running mean must be provided!");
  SMITH_CHECK(running_mean_opt->dim() == 1, "Running mean must have ndim == 1");
  SMITH_CHECK(
  ...
  // Running var
  SMITH_CHECK(running_var_opt, "Running var must be provided!");
  SMITH_CHECK(running_var_opt->dim() == 1, "Running var must have ndim == 1");
  SMITH_CHECK(
