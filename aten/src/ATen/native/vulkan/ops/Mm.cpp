...
vTensor pack_inputs_using_width_packing(const Tensor& input_arg) {
  ...
  SMITH_CHECK(input.is_vulkan(), "Input must be on Vulkan device!");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  ...
  m.impl(SMITH_SELECTIVE_NAME("aten::mm"), SMITH_FN(mm));
  m.impl(SMITH_SELECTIVE_NAME("aten::bmm"), SMITH_FN(bmm));
  m.impl(SMITH_SELECTIVE_NAME("aten::baddbmm"), SMITH_FN(baddbmm));
}
