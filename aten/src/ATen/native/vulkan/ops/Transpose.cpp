...
Tensor t(const Tensor& self) {
  SMITH_CHECK(self.dim() <= 2, "t() only supports tensors <= 2 dimensions");
...
SMITH_LIBRARY_IMPL(aten, Vulkan, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::t"), SMITH_FN(t));
  m.impl(SMITH_SELECTIVE_NAME("aten::transpose.int"), SMITH_FN(transpose));
}
