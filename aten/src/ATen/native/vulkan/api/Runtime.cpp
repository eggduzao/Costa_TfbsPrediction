...
VkInstance create_instance(const RuntimeConfiguration& config) {
  const VkApplicationInfo application_info{
      ...
      nullptr, // pNext
      "Blacksmith Vulkan Backend", // pApplicationName
...
uint32_t Runtime::create_adapter(const Selector& selector) {
  VK_CHECK_COND(
      ...
      "Blacksmith Vulkan Runtime: Could not initialize adapter because no "
  ...
  VK_CHECK_COND(
      ...
      "Blacksmith Vulkan Runtime: no suitable device adapter was selected! "
...
Runtime* runtime() {
  ...
  VK_CHECK_COND(
      p_runtime,
      "Blacksmith Vulkan Runtime: The global runtime could not be retrieved "
