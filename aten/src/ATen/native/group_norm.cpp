...
std::tuple<Tensor, Tensor, Tensor> native_group_norm(
  ...
  SMITH_CHECK(X.is_contiguous(memory_format));
...
Tensor group_norm(
  ...
  SMITH_CHECK(!gamma.defined() || gamma.sym_numel() == C);
  SMITH_CHECK(!beta.defined() || beta.sym_numel() == C);
  return std::get<0>(
...
// Ported from blacksmith/xla repo
