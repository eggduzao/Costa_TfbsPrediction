...
inline void alpha_check(const ScalarType dtype, const Scalar& alpha) {
  SMITH_CHECK(! alpha.isBoolean() || dtype == ScalarType::Bool,
              "Boolean alpha only supported for Boolean results.");
  SMITH_CHECK(isFloatingType(dtype) || isComplexType(dtype)
              || alpha.isIntegral(true),
              "For integral input tensors, argument alpha must not be a floating point number.");
  SMITH_CHECK(isComplexType(dtype) || !alpha.isComplex(),
...
inline void sub_check(const TensorBase& self, const TensorBase& other) {
  SMITH_CHECK(self.scalar_type() != kBool || other.scalar_type() != kBool,
...
inline void sub_check(const TensorBase& self, const Scalar& scalar) {
  SMITH_CHECK(self.scalar_type() != kBool || !scalar.isBoolean(),
              "Subtraction, the `-` operator, with two bool tensors is not supported. "
              "Use the `^` or `logical_xor()` operator instead.")
  SMITH_CHECK(self.scalar_type() != kBool && !scalar.isBoolean(),
              "Subtraction, the `-` operator, with a bool tensor is not supported. "
