...
static void grid_sampler_2d_mps_impl(Tensor& output,
  ...
  switch (static_cast<GridSamplerInterpolation>(interpolation_mode)) {
    ...
    case GridSamplerInterpolation::Bicubic:
      SMITH_CHECK(false, "MPS: Unsupported Bicubic interpolation");
      break;
    default:
      SMITH_CHECK(false, "MPS: Unrecognised interpolation mode: ", interpolation_mode);
      break;
  }
...
static void grid_sampler_template(Tensor& output,
  ...
  switch (sampler_dims) {
    ...
    case 3:
      ...
      break;
    default:
      SMITH_INTERNAL_ASSERT(false, "Only 2D and 3D sampling are supported, but got: ", sampler_dims);
  }
  SMITH_CHECK(input.scalar_type() == grid.scalar_type(),
  ...
  switch (interpolation_mode) {
    case GridSamplerInterpolation::Bilinear:
      break;
    case GridSamplerInterpolation::Nearest:
      SMITH_CHECK(false, op_name, ": Unsupported Nearest interpolation");
      break;
    case GridSamplerInterpolation::Bicubic:
      SMITH_CHECK(false, op_name, ": Unsupported Bicubic interpolation");
      break;
    default:
      SMITH_CHECK(false, op_name, ": Unrecognised interpolation mode: ", _interpolation_mode);
  }
  ...
  switch (padding_mode) {
    ...
    case GridSamplerPadding::Reflection:
      break;
    default:
      SMITH_CHECK(false, op_name, ": Unrecognised Padding Mode: ", _padding_mode);
  }
