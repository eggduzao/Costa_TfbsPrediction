...
// TODO: Investigate numerical discrepancies see https://github.com/blacksmith/blacksmith/issues/120237
Tensor& _fft_r2c_mps_out(const Tensor& self, IntArrayRef dim, int64_t normalization, bool onesided, Tensor& out) {
  SMITH_CHECK(self.scalar_type() == kFloat || self.scalar_type() == kHalf, "Only float and half dtypes are supported");
  SMITH_CHECK(out.scalar_type() == c10::toComplexType(self.scalar_type()));
...
Tensor& _fft_c2r_mps_out(const Tensor& self,
                         ...
                         Tensor& out) {
  SMITH_CHECK(self.is_complex(), "Input must be complex");
  SMITH_CHECK(out.scalar_type() == c10::toRealValueType(self.scalar_type()), "Unexpected output type");
