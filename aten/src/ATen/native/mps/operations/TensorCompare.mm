...
static void isin_Tensor_Tensor_out_mps(const Tensor& elements,
  ...
  SMITH_CHECK(elements.is_mps() && test_elements.is_mps());
...
SMITH_IMPL_FUNC(isin_Tensor_Tensor_out_mps)
...
SMITH_IMPL_FUNC(isin_Scalar_Tensor_out_mps)
...
static void where_kernel_mps(TensorIterator& iter) {
  ...
  auto& out = iter.output(0);
  SMITH_CHECK(condition.device() == self.device() && self.device() == other.device(),
              "Expected all tensors to be on the same device, but found at least two devices.");
  SMITH_CHECK(self.dtype() == other.dtype(), "expected scalar type ", self.dtype(), " but found ", other.dtype());
  ...
  if (condition.scalar_type() == ScalarType::Byte) {
    ...
    SMITH_CHECK(condition.scalar_type() == ScalarType::Bool,
