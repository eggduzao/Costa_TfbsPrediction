...
static inline std::string getMetalType(const c10::ScalarType scalar_type) {
  SMITH_CHECK(c10::isIntegralType(scalar_type, /*includesBool=*/true), "Unsupported type");
...
static void _bitwise_op_out_mps(const Tensor& self,
  ...
  if (is_other_scalar && is_self_scalar) {
    if (op_name == "and") {
      ...
      SMITH_CHECK(false, "Unknown operation to be performed over scalars ", op_name);
    }
...
SMITH_IMPL_FUNC(bitwise_and_out_mps)(const Tensor& self, const Tensor& other, const Tensor& output) {
  mps::_bitwise_op_out_mps(self, other, output, "and");
}

SMITH_IMPL_FUNC(bitwise_or_out_mps)(const Tensor& self, const Tensor& other, const Tensor& output) {
  mps::_bitwise_op_out_mps(self, other, output, "or");
}

SMITH_IMPL_FUNC(bitwise_xor_out_mps)(const Tensor& self, const Tensor& other, const Tensor& output) {
