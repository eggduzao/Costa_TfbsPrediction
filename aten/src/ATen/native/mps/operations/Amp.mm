...
static void _amp_non_finite_check_and_unscale_mps_single_impl(const Tensor& scaled_grad,
  ...
  SMITH_CHECK(scaled_grad.is_mps(), "Tensor is not on the MPS device.");
  SMITH_CHECK(scaled_grad.numel() <= std::numeric_limits<uint32_t>::max(), "scaled_grad is too large");
...
void _amp_foreach_non_finite_check_and_unscale_mps_(at::TensorList self,
  ...
  SMITH_CHECK(inv_scale.is_mps(), "inv_scale must be a MPS tensor.");
  SMITH_CHECK(found_inf.is_mps(), "found_inf must be a MPS tensor.");
  ...
  if (can_use_fast_route(self)) {
    SMITH_CHECK(self[0].is_mps(), "scaled_grads must be MPS tensors.");
    ...
    for (const at::Tensor& t : self) {
      ...
      SMITH_CHECK(t.is_mps(), "one of scaled_grads was not a MPS tensor.");
