...
SMITH_IMPL_FUNC(signbit_out_mps)(const Tensor& self, const Tensor& output) {
...
SMITH_IMPL_FUNC(sign_out_mps)(const Tensor& self, const Tensor& output) {
...
#define CREATE_MPS_STRUCTURED_UNARY_SMITH_IMPL_FUNC(func_out, func_stub)                                         \
  SMITH_IMPL_FUNC(func_out)(const Tensor& self, const Tensor& output) {                                          \
...
CREATE_MPS_STRUCTURED_UNARY_SMITH_IMPL_FUNC(asinh_out_mps, asinh)
CREATE_MPS_STRUCTURED_UNARY_SMITH_IMPL_FUNC(acosh_out_mps, acosh)
CREATE_MPS_STRUCTURED_UNARY_SMITH_IMPL_FUNC(atanh_out_mps, atanh)
...
SMITH_IMPL_FUNC(frac_out_mps)(const Tensor& self, const Tensor& output) {
  SMITH_CHECK(isFloatingType(self.scalar_type()), "frac_out_mps is only implemented for floating types");
...
SMITH_IMPL_FUNC(logit_backward_out_mps)
...
static void cumulative_op_impl(const Tensor& self,
  ...
  SMITH_CHECK(wrapped_dim >= 0 && wrapped_dim < std::max(1LL, self.ndimension()),
              ...
              dim,
              ")");
  SMITH_CHECK(!self.is_complex(), "cumulative ops are not yet supported for complex");
...
SMITH_IMPL_FUNC(cumsum_out_mps)
...
SMITH_IMPL_FUNC(cumprod_out_mps)
(const Tensor& self, int64_t dim, std::optional<ScalarType> dtype, const Tensor& result) {
  return cumulative_op_impl(self, dim, dtype, result, MPSCumulativeOpType::CUMPROD, "cumprod_out_mps");
}

SMITH_IMPL_FUNC(sgn_out_mps)(const Tensor& self, const Tensor& output) {
  if (!self.is_complex()) {
...
Tensor& conj_physical_out_mps(const Tensor& self, Tensor& result) {
  ...
  SMITH_CHECK(self.dtype() != at::kComplexDouble);
