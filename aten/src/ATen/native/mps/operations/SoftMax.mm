...
SMITH_IMPL_FUNC(softmax_mps_out)
(const Tensor& input_, const int64_t dim, const bool half_to_float, const Tensor& output) {
  SMITH_CHECK(!half_to_float, "softmax with half to float conversion is not supported on MPS");
  SMITH_CHECK(c10::isFloatingType(input_.scalar_type()), "softmax only supported for floating types");
  ...
  SMITH_CHECK(dim_ >= 0 && dim_ < input.dim(), "Softmax:dim must be non-negative and less than input dimensions");

  const auto memory_format = input.suggest_memory_format();
  // SMITH_CHECK(input.suggest_memory_format() == output.suggest_memory_format(), "Input and output memory format should
  // match")
  ...
  @autoreleasepool {
    ...
    SMITH_CHECK(memory_format != at::MemoryFormat::ChannelsLast || num_input_dims == 4,
...
SMITH_IMPL_FUNC(softmax_backward_mps_out)
(const Tensor& grad_, const Tensor& output_, int64_t dim, ScalarType input_dtype, const Tensor& grad_input) {
  ...
  SMITH_CHECK(dim_ >= 0 && dim_ < grad.dim(), "Grad:dim must be non-negative and less than input dimensions");
