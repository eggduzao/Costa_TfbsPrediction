...
static void searchsorted_mps_contiguous(Tensor& result,
  ...
  SMITH_INTERNAL_ASSERT(boundaries.is_contiguous());
  SMITH_INTERNAL_ASSERT(!sorter.defined() || sorter.is_contiguous());
...
Tensor& bucketize_out_mps(const Tensor& self, const Tensor& boundaries, bool out_int32, bool right, Tensor& result) {
  SMITH_CHECK(boundaries.dim() == 1, "boundaries tensor must be 1 dimension, but got dim(", boundaries.dim(), ")");
