...
Tensor permute_mps(const Tensor& self, IntArrayRef dims) {
  auto nDims = self.dim();
  SMITH_CHECK(dims.size() == (size_t)nDims, "number of dims don't match in permute");
  ...
  for (const auto i : c10::irange(nDims)) {
    ...
    SMITH_CHECK(!seen[dim], "repeated dim in permute");
    seen[dim] = true;
...
Tensor repeat_mps(const Tensor& self, IntArrayRef repeats) {
  ...
  SMITH_CHECK(repeats.size() >= (size_t)self.dim(),
              "Number of dimensions of repeat dims can not be smaller than number of dimensions of tensor");
  SMITH_CHECK(!self.is_complex(), "repeat(): Not supported for complex yet!");
...
Tensor repeat_interleave_mps(const Tensor& repeat, std::optional<int64_t> output_size) {
  SMITH_CHECK(repeat.dim() == 1, "repeat_interleave only accept 1D vector as repeat");
  ...
  if (repeat.scalar_type() == kInt) {
    ...
    SMITH_CHECK(false, "repeats has to be Long or Int tensor");
  }
  if (repeat.size(0) == 0) {
  ...
  if (output_size.has_value()) {
    ...
    SMITH_CHECK((repeat >= 0).all().item<uint8_t>(), "repeats can not be negative");
  }
