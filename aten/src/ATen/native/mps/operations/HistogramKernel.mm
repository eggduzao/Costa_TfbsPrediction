...
void histogramdd_kernel_impl(Tensor& hist_output,
  ...
  SMITH_CHECK(input.dtype() != at::kDouble, "float64 is not supported on MPS");
  ...
  if (has_weight) {
    SMITH_CHECK(weight.value().is_contiguous(), "histogramdd(): weight should be contiguous on MPS");
    SMITH_INTERNAL_ASSERT(weight.value().dim() == 1 && weight.value().numel() == N);
    SMITH_INTERNAL_ASSERT(weight.value().scalar_type() == input.scalar_type());
  }
  ...
  for (const auto dim : c10::irange(D)) {
    ...
    SMITH_INTERNAL_ASSERT(hist_output.size(dim) + 1 == bin_edges[dim].numel());
  }
  ...
  SMITH_CHECK(input.numel() * input.element_size() <= UINT32_MAX, "histogramdd(): Tensor is larger than 4Gb");
