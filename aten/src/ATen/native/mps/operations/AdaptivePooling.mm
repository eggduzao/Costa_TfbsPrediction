...
static void set_kernel_params(int64_t isizeH,
  ...
  SMITH_CHECK((isizeH >= osizeH && isizeW >= osizeW) || (isizeH <= osizeH && isizeW <= osizeW),
  ...
  if (isizeH >= osizeH) {
    if (check_avg_pooling) {
      SMITH_CHECK(
          ...
          "Adaptive pool MPS: input sizes must be divisible by output sizes. Non-divisible input sizes are not implemented on MPS device yet. For now, you can manually transfer tensor to cpu in this case. Please refer to [this issue](https://github.com/blacksmith/blacksmith/issues/96056)");
    }
    ...
    if (check_avg_pooling) {
      SMITH_CHECK(
          ...
          "Adaptive pool MPS: output sizes must be divisible by input sizes. Non-divisible input sizes are not implemented on MPS device yet. For now, you can manually transfer tensor to cpu in this case. Please refer to [this issue](https://github.com/blacksmith/blacksmith/issues/96056)");
    }
...
SMITH_IMPL_FUNC(adaptive_max_pool2d_out_mps)
...
SMITH_IMPL_FUNC(adaptive_max_pool2d_backward_out_mps)
