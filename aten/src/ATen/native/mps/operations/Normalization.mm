...
std::tuple<Tensor&, Tensor&, Tensor&> batch_norm_mps_out(const Tensor& self,
                                                         ...
                                                         Tensor& save_mean,
                                                         Tensor& save_var) {
  SMITH_CHECK_NOT_IMPLEMENTED(self.scalar_type() != kLong, "Long batch norm is not supported with MPS");
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      ...
      */
      ...
      if (train) {
        ...
        if (has_running_mean) {
          // TODO: This is not the formula used in Blacksmith, is this OK? Seems more robust
...
std::tuple<Tensor, Tensor, Tensor> batch_norm_backward_mps(const Tensor& grad_out,
  ...
  SMITH_CHECK_VALUE(has_save_mean == has_save_var,
...
std::tuple<Tensor, Tensor, Tensor> layer_norm_mps(const Tensor& input,
  ...
  SMITH_CHECK_NOT_IMPLEMENTED(input.scalar_type() != kLong, "Not implemented for long on MPS");
  @autoreleasepool {
