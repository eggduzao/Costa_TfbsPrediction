...
static Tensor& pad_out_template(Tensor& output,
  ...
  SMITH_CHECK(ndims >= (int64_t)padding_dim,
  ...
  if (!is_backward_pass && mode != MPSGraphPaddingModeConstant && ndims > padding_dim) {
    ...
    SMITH_CHECK((ndims == 1 + padding_dim && valid_dims) ||
  ...
  if (!is_backward_pass) {
    SMITH_CHECK(output_w >= 1 || output_h >= padding_dim - 1,
                "input (H: ",
                input_h,
                ", W: ",
                input_w,
    ...
    if (mode == MPSGraphPaddingModeConstant) {
      ...
      if (mode == MPSGraphPaddingModeReflect) {
        ...
        if (padding_dim > 2) {
          SMITH_CHECK(pad_front < input_d && pad_back < input_d,
    ...
    SMITH_CHECK(output_w == grad_output_.size(dim_w),
                "gradOutput width unexpected. Expected: ",
                output_w,
                ", Got: ",
                grad_output_.size(dim_w));
    if (padding_dim > 1) {
      SMITH_CHECK(output_h == grad_output_.size(dim_h),
      ...
      if (!is_backward_pass) {
        ...
        if (needsSlice) {
          for (auto i : c10::irange(ndims)) {
            ...
            SMITH_INTERNAL_ASSERT(start == 0 || start < input_size);
          }
          newCachedGraph->gradInputTensor_ =
...
SMITH_IMPL_FUNC(reflection_pad1d_out_mps)
...
SMITH_IMPL_FUNC(reflection_pad1d_backward_out_mps)
...
SMITH_IMPL_FUNC(replication_pad1d_out_mps)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  mps::pad_out_template(const_cast<Tensor&>(output),
                        input,
                        padding,
...
SMITH_IMPL_FUNC(replication_pad1d_backward_out_mps)
(const Tensor& grad_output, const Tensor& input, IntArrayRef padding, const Tensor& grad_input) {
  grad_input.resize_as_(input).zero_();
  mps::pad_out_template(const_cast<Tensor&>(grad_input),
                        input,
...
SMITH_IMPL_FUNC(replication_pad2d_out_mps)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  mps::pad_out_template(const_cast<Tensor&>(output),
                        input,
                        padding,
...
SMITH_IMPL_FUNC(reflection_pad3d_out_mps)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  mps::pad_out_template(const_cast<Tensor&>(output),
                        input,
                        padding,
...
SMITH_IMPL_FUNC(reflection_pad3d_backward_out_mps)
(const Tensor& grad_output, const Tensor& input, IntArrayRef padding, const Tensor& grad_input) {
  grad_input.resize_as_(input).zero_();
  mps::pad_out_template(const_cast<Tensor&>(grad_input),
                        input,
...
SMITH_IMPL_FUNC(replication_pad3d_out_mps)
(const Tensor& input, IntArrayRef padding, const Tensor& output) {
  mps::pad_out_template(const_cast<Tensor&>(output),
                        input,
                        padding,
...
Tensor constant_pad_nd_mps(const Tensor& self, IntArrayRef pad, const Scalar& value) {
  ...
  if (pad.size() > 6) {
    SMITH_WARN_ONCE("MPS: The constant padding of more than 3 dimensions is not currently supported natively. ",
