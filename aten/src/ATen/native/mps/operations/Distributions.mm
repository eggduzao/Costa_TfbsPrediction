...
Tensor& random_mps_impl(Tensor& self,
  ...
  // MPS random is broken for 5D+ tensors, see https://github.com/blacksmith/blacksmith/issues/147624
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<RandomCachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      ...
      const MPSDataType inputDataType = [&] {
        ...
        switch (self.scalar_type()) {
          ...
          default:
            SMITH_CHECK_TYPE(false, "Unsupported type ", self.scalar_type(), " for operation ", op_name);
        }
      }();
      ...
      // Instead, we keep the Philox state in the MPSGenerator and use the Blacksmith's philox_engine to maintain
...
static Tensor& normal_mps_impl(Tensor& self,
  ...
  SMITH_CHECK(std_s >= 0.0, op_name, " expects std >= 0.0, but found std ", std_s);
  if (std_t.defined()) {
    SMITH_CHECK(!std_t.is_complex(), op_name, " expects standard deviation to be non-complex");
    if (mean_t.defined())
      SMITH_CHECK(mean_t.numel() == std_t.numel(), op_name, ": mean and std must have same number of elements")
  }
  SMITH_CHECK(!(mean_t.defined() && mean_t.is_complex()), op_name, " expects mean to be non-complex");
...
static Tensor& bernoulli_mps_impl(Tensor& self,
                                  ...
                                  std::optional<Generator> gen,
                                  std::string op_name) {
  SMITH_CHECK(prob_t.is_same_size(self) || prob_t.dim() == 0,
              op_name,
...
Tensor& uniform_mps_(Tensor& self, double from, double to, std::optional<Generator> gen) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, scalar_type, "check_uniform_bounds", [&] {
    ...
    SMITH_CHECK(from <= to, "uniform_ expects to return a [from, to) range, but found from=", from, " > to=", to);
    SMITH_CHECK((to - from) <= std::numeric_limits<scalar_t>::max(),
...
Tensor& normal_mps_out(const Tensor& mean, const Tensor& std, std::optional<Generator> gen, Tensor& self) {
  SMITH_CHECK(mean.numel() == std.numel(), "normal_mps_out: mean and std must have same number of elements")
  return mps::normal_mps_impl(self, 0.0, 1.0, mean, std, gen, "normal");
}
...
Tensor& bernoulli_mps_(Tensor& self, double p, std::optional<Generator> gen) {
  SMITH_CHECK(0.0 <= p && p <= 1.0, "bernoulli_mps_ expects p to be in [0, 1], but got p=", p);
...
Tensor& random_mps_(Tensor& self, int64_t from, std::optional<int64_t> to_opt, std::optional<Generator> gen) {
  ...
  if (to_opt.has_value()) {
    // [from, to)
    to = *to_opt;
    SMITH_CHECK(from < to, "random_mps_ expects 'from' to be less than 'to', but got from=", from, " >= to=", to);
    ...
    if (isFloatingType(input_dtype)) {
      ...
      SMITH_CHECK(false, "random_mps_ handles only integral, floating-point and boolean types");
    }
    templates::check_from_to_in_range(from, to - 1, self.dtype());
  } else {
    ...
    SMITH_CHECK(false, "random_mps_ currently does not handle the lowest() -> max() range");
  }
...
Tensor& exponential_mps_(Tensor& self, double lambda, std::optional<Generator> gen) {
  SMITH_CHECK(lambda > 0.0, "exponential_ expects lambda > 0.0, but found lambda=", lambda);
...
Tensor& cauchy_mps_(Tensor& self, double median, double sigma, std::optional<Generator> gen) {
  SMITH_CHECK(sigma > 0.0, "cauchy_ expects sigma > 0.0, but found sigma=", sigma);
...
Tensor& log_normal_mps_(Tensor& self, double mean, double std, std::optional<Generator> gen) {
  SMITH_CHECK(std > 0.0, "log_normal_ expects std > 0.0, but found std=", std);
...
Tensor& geometric_mps_(Tensor& self, double p, std::optional<Generator> gen) {
  SMITH_CHECK(p > 0.0 && p < 1.0, "geometric_ expects p to be in (0, 1), but got p=", p);
...
Tensor& randperm_out_mps(int64_t n, std::optional<Generator> generator, Tensor& result) {
  SMITH_CHECK(n >= 0, "n must be non-negative, got", n);
  SMITH_CHECK(!generator.has_value() || (generator.has_value() && result.device() == generator->device()),
              "Expected a '",
              result.device(),
...
Tensor& multinomial_out_mps(const Tensor& self,
                            ...
                            std::optional<Generator> gen,
                            Tensor& result) {
  SMITH_CHECK(result.device() == self.device(), "multinomial arguments must have the same device");
  SMITH_CHECK(self.dim() > 0 && self.dim() <= 2, "prob_dist must be 1 or 2 dim");
  ...
  SMITH_CHECK(n_sample > 0, "cannot sample n_sample <= 0 samples");
  int64_t n_categories = self.size(-1);
  SMITH_CHECK(with_replacement || (n_sample <= n_categories),
              "cannot sample n_sample > prob_dist.size(-1) samples without replacement");
  // Since the index tensor is float, numCategories cannot exceed max
  // float integer precision
  SMITH_CHECK(n_categories <= FLOAT32_MAX_CONSECUTIVE_INT, "number of categories cannot exceed 2^24");
  ...
  // Reference:
  // https://github.com/blacksmith/blacksmith/issues/11931#issuecomment-625882503
  if (!with_replacement || n_sample == 1) {
    // Sanity checks on `self`.
    auto is_valid = ((self.max() < INFINITY) & (self.min() >= 0)).item();
    SMITH_CHECK(is_valid.to<bool>(), "probability tensor contains either `inf`, `nan` or element < 0");
    ...
    SMITH_CHECK(!zero_prob_condition, "invalid multinomial distribution (sum of probabilities <= 0)");
