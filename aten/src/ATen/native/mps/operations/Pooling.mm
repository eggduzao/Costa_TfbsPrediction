...
static void pool2d_template(const Tensor& input,
  ...
  // be incompatible with the Blacksmith's global NCHW layout.
  const auto memory_format = has_indices ? MemoryFormat::Contiguous : suggested_memory_format;

  SMITH_CHECK(kernel_size.size() == 1 || kernel_size.size() == 2,
              op_name,
              ": kernel_size must either be a single int, or a tuple of two ints")
  SMITH_CHECK(stride.size() == 0 || stride.size() == 1 || stride.size() == 2,
              op_name,
              ": stride must either be omitted, a single int, or a tuple of two ints")
  SMITH_CHECK(padding.size() == 1 || padding.size() == 2,
              op_name,
              ": padding must either be a single int, or a tuple of two ints");
  SMITH_CHECK(dilation.size() == 1 || dilation.size() == 2,
              op_name,
              ": dilation must be either a single int, or a tuple of two ints");

  if (suggested_memory_format == at::MemoryFormat::ChannelsLast) {
    SMITH_CHECK(ndims == 4, "non-empty 4D (batch mode) tensor expected for input with channels_last layout");
  } else if (suggested_memory_format == at::MemoryFormat::Contiguous) {
    SMITH_CHECK((ndims == 3 || ndims == 4), "non-empty 3D or 4D (batch mode) tensor expected for input");
  } else {
    SMITH_CHECK(false, "Unsupported memory format. Supports only ChannelsLast, Contiguous");
  }
...
static PoolSizes process_pool_sizes(const Tensor& input,
                                    ...
                                    const std::string& op_name) {
  SMITH_INTERNAL_ASSERT(pooling_dims == 1 || pooling_dims == 2 || pooling_dims == 3);

  const int32_t dims = input.dim();

  SMITH_CHECK(dims == pooling_dims + 1 || dims == pooling_dims + 2,
              op_name,
              ": non-empty ",
              pooling_dims + 1,
              "D or ",
  ...
  SMITH_CHECK(kernel_size.size() == 1 || kernel_size.size() == pooling_dims,
              op_name,
              ": kernel_size must either be a single int, or a tuple of ",
              pooling_dims,
              " ints");

  SMITH_CHECK(stride.empty() || stride.size() == 1 || stride.size() == pooling_dims,
              op_name,
              ": stride must either be omitted, a single int, or a tuple of ",
              pooling_dims,
              " ints");

  SMITH_CHECK(padding.size() == 1 || padding.size() == pooling_dims,
              op_name,
  ...
  if (dilation_opt.has_value()) {
    ...
    SMITH_CHECK(dilation.size() == 1 || dilation.size() == pooling_dims,
                op_name,
  ...
  for (const auto dim : c10::irange(pooling_dims)) {
    SMITH_CHECK(padding_expanded[dim] >= 0, op_name, ": pad must be non-negative");
    SMITH_CHECK(padding_expanded[dim] * 2 <= kernel_size_expanded[dim],
                op_name,
  ...
  if (pooling_dims == 2) {
    ...
    if (memory_format == at::MemoryFormat::ChannelsLast) {
      ...
      SMITH_CHECK((dims == 4 && valid_dims && input.size(3) != 0),
      ...
      SMITH_CHECK((dims == 3 && input.size(0) != 0 && valid_dims) || (dims == 4 && valid_dims && input.size(3) != 0),
  ...
  for (const auto dim : c10::irange(static_cast<int>(leading_dims == 2), dims)) {
    SMITH_CHECK(input.size(dim) > 0, op_name, ": Expected input's non-batch dimensions to have positive length");
  }
...
static void max_pool_with_indices_out_mps_template(const Tensor& output,
  ...
  SMITH_INTERNAL_ASSERT(dilation_opt.has_value());
  auto dilation = dilation_opt.value();
  const Tensor& indices = *(at::borrow_from_optional_tensor(indices_opt));
  ...
  SMITH_INTERNAL_ASSERT(numThreads == output.numel());
...
static void max_unpool_out_mps_template(const Tensor& input,
                                        ...
                                        const int32_t pooling_dims,
                                        const std::string& op_name) {
  SMITH_CHECK(output_size_.size() == static_cast<size_t>(pooling_dims),
              op_name,
  ...
  SMITH_CHECK(input.sizes() == indices.sizes(),
...
static void avg_pool2d_template(const Tensor& input,
                                ...
                                const std::optional<int64_t> divisor_override,
                                const std::string& op_name) {
  SMITH_CHECK_NOT_IMPLEMENTED(!c10::isComplexType(input.scalar_type()), "Not implemented for complex");
  const Tensor& grad_output = *(at::borrow_from_optional_tensor(grad_output_opt));
  const bool is_backward_pass = grad_output.defined();
  ...
  if (use_divisor && input.scalar_type() == ScalarType::Long) {
    SMITH_WARN_ONCE("MPS: passing divisor to Average Pooling op with int64 input is ",
...
SMITH_IMPL_FUNC(max_pool2d_with_indices_out_mps)
(const Tensor& input,
 IntArrayRef kernel_size,
 IntArrayRef stride,
 IntArrayRef padding,
 ...
 const Tensor& output,
 const Tensor& indices) {
  SMITH_CHECK_NOT_IMPLEMENTED(!c10::isComplexType(input.scalar_type()),
...
SMITH_IMPL_FUNC(max_pool2d_with_indices_backward_out_mps)
(const Tensor& grad_output,
 const Tensor& input,
 IntArrayRef kernel_size,
 IntArrayRef stride,
...
SMITH_IMPL_FUNC(avg_pool2d_out_mps)
(const Tensor& input,
 int64_t kH,
 int64_t kW,
 int64_t dH,
...
SMITH_IMPL_FUNC(avg_pool2d_backward_out_mps)
...
SMITH_IMPL_FUNC(avg_pool3d_out_mps)
(const Tensor& input,
 IntArrayRef kernel_size,
 IntArrayRef stride,
 IntArrayRef padding,
...
SMITH_IMPL_FUNC(avg_pool3d_backward_out_mps)(const Tensor& grad_output,
                                             const Tensor& input,
                                             IntArrayRef kernel_size,
                                             IntArrayRef stride,
                                             IntArrayRef padding,
