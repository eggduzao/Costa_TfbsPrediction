...
static void set_axes(NSMutableArray<NSNumber*>*& axes,
  ...
  if (num_reduce_dims == 0) {
    ...
    SMITH_INTERNAL_ASSERT(opt_dim.has_value());
...
static void reduction_out_mps(const Tensor& input_t,
  ...
  if (opt_dim.has_value()) {
    ...
    for (const auto dim_val : dim) {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/136132#issuecomment-2354482608
  ...
  if (output_t.numel() == 0 || input_t.numel() == 0) {
    switch (reduction_type) {
      ...
      case MPSReductionType::MIN:
        SMITH_CHECK(opt_dim.has_value(), "Expected reduction dim to be specified for input.numel() == 0");
        break;
      default:
        SMITH_INTERNAL_ASSERT(false, "Unexpected reduction type ", reduction_type);
        break;
    }
    return;
  }
...
static void impl_func_norm_mps(const Tensor& input_tensor,
  ...
  SMITH_CHECK(!input_tensor.is_complex(), "norm ops are not supported for complex yet");

  IntArrayRef input_shape = cdist ? input_broadcasted_shape.value() : input_t.sizes();

  for (const auto dim_val : dim) {
    auto wrap_dim = maybe_wrap_dim(dim_val, input_shape.size());
    SMITH_CHECK(wrap_dim < static_cast<decltype(wrap_dim)>(input_shape.size()),
...
static Tensor std_var_common_impl_mps(const Tensor& input_t,
  ...
  SMITH_CHECK_NOT_IMPLEMENTED(input_t.scalar_type() != kLong, "Not implemented for MPS");
  using CachedGraph = MPSUnaryCachedGraph;
  ...
  if (use_dim) {
    ...
    for (const auto dim : dim_value) {
      ...
      SMITH_CHECK(wrap_dim < (num_input_dims ? num_input_dims : 1), errMessage.c_str())
    }
  }
...
static void argmax_argmin_out_mps(const Tensor& input_t,
  ...
  if (dim.has_value()) {
    ...
    SMITH_CHECK_INDEX(input_t.numel() != 0,
...
Tensor& nansum_out_mps(const Tensor& self,
                       ...
                       std::optional<ScalarType> opt_dtype,
                       Tensor& result) {
  SMITH_CHECK(!c10::isComplexType(self.scalar_type()), "nansum on MPS does not support complex inputs");
...
SMITH_IMPL_FUNC(amax_out_mps)(const Tensor& input_t, IntArrayRef dim, bool keepdim, const Tensor& output_t) {
  SMITH_CHECK(!c10::isComplexType(input_t.scalar_type()), "amax is not defined for complex types");
  reduction_out_mps(input_t, dim, keepdim, std::nullopt, output_t, MPSReductionType::AMAX, "amax_out_mps");
}

SMITH_IMPL_FUNC(amin_out_mps)(const Tensor& input_t, IntArrayRef dim, bool keepdim, const Tensor& output_t) {
  SMITH_CHECK(!c10::isComplexType(input_t.scalar_type()), "amin is not defined for complex types");
  reduction_out_mps(input_t, dim, keepdim, std::nullopt, output_t, MPSReductionType::AMIN, "amin_out_mps");
}

SMITH_IMPL_FUNC(aminmax_out_mps)
(const Tensor& input_t, std::optional<int64_t> dim_opt, bool keepdim, const Tensor& min_t, const Tensor& max_t) {
  SMITH_CHECK(!c10::isComplexType(input_t.scalar_type()), "aminmax is not defined for complex types");
...
SMITH_IMPL_FUNC(norm_dtype_out_mps)
(const Tensor& self,
 const OptionalScalarRef opt_p,
 IntArrayRef dim,
 bool keepdim,
...
SMITH_IMPL_FUNC(linalg_vector_norm_out_mps)
(const Tensor& self,
...
Tensor _cdist_forward_mps(const Tensor& x1, const Tensor& x2, const double p, std::optional<int64_t> compute_mode) {
  SMITH_CHECK(x1.dim() >= 2, "cdist only supports at least 2D tensors, X1 got: ", x1.dim(), "D");
  SMITH_CHECK(x2.dim() >= 2, "cdist only supports at least 2D tensors, X2 got: ", x2.dim(), "D");
  SMITH_CHECK(x1.size(-1) == x2.size(-1),
  ...
  SMITH_CHECK(p >= 0, "cdist only supports non-negative p values");
  SMITH_CHECK(device1 == device2, "X1 and X2 must have the same device type. X1: ", device1, " X2: ", device2);
  SMITH_CHECK(x1.is_mps() && (x1.get_device() == x2.get_device()),
              "device of X1 (",
              x1.get_device(),
  ...
  SMITH_CHECK(mode >= 0 && mode <= 2, "possible modes: 0, 1, 2, but was: ", mode);
...
static void all_any_common_impl_mps(const Tensor& input_t,
  ...
  @autoreleasepool {
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/95538
      MPSGraphTensor* outputTensor = nil;
...
SMITH_IMPL_FUNC(any_all_out_mps)(const Tensor& input_t, const Tensor& output_t) {
  using CachedGraph = MPSUnaryCachedGraph;
  if (input_t.numel() == 0) {
    output_t.zero_();
    return;
...
SMITH_IMPL_FUNC(all_all_out_mps)(const Tensor& input_t, const Tensor& output_t) {
  using CachedGraph = MPSUnaryCachedGraph;
  if (output_t.numel() == 0 || input_t.numel() == 0) {
...
SMITH_IMPL_FUNC(argmax_out_mps)
...
SMITH_IMPL_FUNC(argmin_out_mps)
(const Tensor& input_t, std::optional<int64_t> dim, bool keepdim, const Tensor& output_t) {
...
SMITH_API ::std::tuple<at::Tensor&, at::Tensor&> median_out_mps(const at::Tensor& input_t,
                                                                int64_t dim,
                                                                bool keepdim,
                                                                at::Tensor& values,
                                                                at::Tensor& indices) {
