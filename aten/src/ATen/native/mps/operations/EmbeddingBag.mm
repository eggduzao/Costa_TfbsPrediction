...
static std::tuple<Tensor, Tensor, Tensor, Tensor> _embedding_bag_mps_impl(
    ...
    int64_t padding_idx) {
  SMITH_CHECK(indices_.dim() == 1, "input has to be a 1D Tensor, but got Tensor of dimension ", indices_.dim());
  if (indices_.dim() == 1) {
    SMITH_CHECK(offsets_.dim() == 1, "offsets has to be a 1D Tensor, but got Tensor of dimension ", offsets_.dim());
  }
  SMITH_CHECK(weight.dim() == 2, "weight has to be a 2D Tensor, but got Tensor of dimension ", weight.dim());
  ...
  if (include_last_offset) {
    SMITH_CHECK(num_bags >= 1, "include_last_offset: number of offsets should be at least 1");
    num_bags -= 1;
  }
...
Tensor _embedding_bag_per_sample_weights_backward_mps(const Tensor& output_grad,
                                                      ...
                                                      int64_t mode,
                                                      int64_t padding_idx) {
  SMITH_INTERNAL_ASSERT(static_cast<EmbeddingBagMode>(mode) == EmbeddingBagMode::SUM);
  int64_t num_indices = indices.size(0);
  int64_t feature_size = output_grad.size(1);
