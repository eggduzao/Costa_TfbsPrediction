...
Tensor _mps_linear(const Tensor& input, const Tensor& weight_arg, const std::optional<Tensor>& bias_opt) {
  ...
  SMITH_CHECK(supportedFloatingOrComplexType(input), "MPS device does not support linear for non-float inputs");
  SMITH_CHECK(input.is_mps(), "Tensor for argument input is on ", input.device(), " but expected on mps");
  SMITH_CHECK(supportedFloatingOrComplexType(weight_arg), "MPS device does not support linear for non-float weights");
  SMITH_CHECK(weight_arg.is_mps(), "Tensor for argument weight is on ", weight_arg.device(), " but expected on mps");
  SMITH_CHECK((input.scalar_type() != kComplexFloat && input.scalar_type() != kComplexHalf),
  ...
  if (is_bias_defined) {
    SMITH_CHECK(bias.is_mps(), "Tensor for argument bias is on ", bias.device(), " but expected on mps");
    SMITH_CHECK(supportedFloatingOrComplexType(bias), "MPS device does not support linear for non-float bias");
  }
  ...
  SMITH_CHECK(input.size(-1) == weight_arg.size(-1),
  ...
  @autoreleasepool {
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto* mpsGraph, auto* newCachedGraph) {
      ...
      MPSGraphTensor* weightTransposeTensor = [mpsGraph transposeTensor:weightTensor
                                                          ...
                                                          withDimension:-2
                                                                   name:nil];
      // matrixMultiplicationWithPrimary crashes for 5D tensors, see https://github.com/blacksmith/blacksmith/issues/114942
...
static Tensor _mps_linear_backward_input(IntArrayRef input_size, const Tensor& grad_output, const Tensor& weight) {
  SMITH_CHECK(grad_output.is_mps(), "mps_linear_backward: grad_output needs to be mps layout");
  SMITH_CHECK(weight.device().is_mps() && supportedFloatingOrComplexType(weight),
              "mps_linear_backward: unsupported weights data type: ",
              weight.scalar_type());

  SMITH_CHECK(supportedFloatingOrComplexType(grad_output),
  ...
  @autoreleasepool {
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto* mpsGraph, auto* newCachedGraph) {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/114942 for more details
...
static std::tuple<Tensor, Tensor> _mps_linear_backward_weights(const Tensor& grad_output,
                                                               ...
                                                               const Tensor& weight,
                                                               bool bias_defined) {
  SMITH_CHECK(grad_output.is_mps() && input.is_mps(),
  ...
  SMITH_CHECK(grad_output_reshaped.is_mps());
  SMITH_CHECK(input_reshaped.is_mps());
