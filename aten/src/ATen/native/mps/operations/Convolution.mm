...
static void fill_conv_desc(MPSGraphConvolution2DOpDescriptor* descriptor_,
  ...
  // Blacksmith always uses OIHW memory layout for weights
...
static Tensor _mps_convolution_impl(const Tensor& input_t,
  ...
  SMITH_CHECK(isFloatingType(input_t.scalar_type()), "Convolution is supported only for Floating types");
  ...
  if (!is_macos_13_or_newer(MacOSVersion::MACOS_VER_15_1_PLUS)) {
    ...
    for (auto elem : output_t.sizes()) {
      SMITH_CHECK_NOT_IMPLEMENTED(elem <= (1 << 16), "Output channels > 65536 not supported at the MPS device. ");
    }
  }
...
static Tensor mps_convolution_backward_input(IntArrayRef input_size,
  ...
  SMITH_CHECK(isFloatingType(grad_output_t.scalar_type()), "Convolution is supported only for Floating types");
...
Tensor _mps_convolution_transpose(const Tensor& input_t,
  ...
  SMITH_CHECK(!is_unsupported_3d_dtype, "ConvTranspose 3D with BF16 or FP16 types is not supported on MPS");
