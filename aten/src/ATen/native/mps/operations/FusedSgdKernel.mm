...
static void _fused_sgd_with_momentum_kernel_mps_(TensorList params,
  ...
  SMITH_CHECK_GT(momentum, 0);
  SMITH_CHECK(native::check_fast_path_restrictions({params, grads, momentum_buffer_list}));
...
static void _fused_sgd_with_momentum_kernel_mps_(TensorList params,
  ...
  SMITH_CHECK(lr_tensor.device() == params[0].device(), "lr must be on the same GPU device as the params");
...
void _fused_sgd_kernel_mps_(TensorList params,
  ...
  SMITH_CHECK_EQ(momentum, 0);
  SMITH_CHECK(native::check_fast_path_restrictions({params, grads}));
  if (is_first_step) {
    SMITH_WARN_ONCE("`is_first_step` argument has no effect when `momentum_buffer_list` is empty");
  }
