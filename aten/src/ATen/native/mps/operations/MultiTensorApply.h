...
static void multi_tensor_apply_for_fused_optimizer(const std::string& kernel_name,
  ...
  SMITH_CHECK(tensor_lists.size() == depth, "Number of tensor lists has to match the depth");
  for (const auto& d : c10::irange(depth)) {
    const auto scalar_type = tensor_lists[d][0].scalar_type();
    SMITH_CHECK(scalar_type == kFloat || scalar_type == kHalf || scalar_type == kBFloat16,
...
void multi_tensor_apply(const std::string& kernel_name,
  ...
  SMITH_CHECK(tensor_lists.size() == depth, "Number of tensor lists must match depth.");
