...
static at::Tensor& copy_from_mps_(at::Tensor& dst_, const at::Tensor& src_, bool non_blocking) {
  ...
  @autoreleasepool {
    ...
    SMITH_INTERNAL_ASSERT(destOffset % 4 == 0, "Unaligned blit request");
    ...
    if (needsBlit) {
      ...
      SMITH_INTERNAL_ASSERT(sourceBuffer && dst_tensor_nbytes > 0);
      uint64_t profile_id =
...
static void copy_to_mps_stride_contig(at::Tensor& dst, const at::Tensor& src, bool non_blocking) {
  ...
  SMITH_INTERNAL_ASSERT(src.dtype() == dst.dtype() && src.strides() == dst.strides() && is_dense_in_storage(src));
...
static at::Tensor& copy_to_mps_(at::Tensor& dst_, const at::Tensor& src_, bool non_blocking) {
  ...
  if (!is_dense_in_storage(src)) {
    src = src.clone();
    SMITH_INTERNAL_ASSERT(is_dense_in_storage(src));
  }
  Tensor dst = dst_;
...
at::Tensor& mps_copy_(at::Tensor& dst, const at::Tensor& src, bool non_blocking) {
  SMITH_CHECK(dst.defined(), "dst is undefined");
  ...
  SMITH_INTERNAL_ASSERT(src.device().type() == DeviceType::MPS, "mps_copy_ is implemented only for *->MPS; MPS->*");
  return dst;
}
} // namespace mps
