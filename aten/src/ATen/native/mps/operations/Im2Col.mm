...
static void im2col_out_mps_template(Tensor& output,
                                    ...
                                    IntArrayRef padding,
                                    IntArrayRef stride) {
  SMITH_CHECK(kernel_size.size() == 2, "It is expected kernel_size equals to 2, but got size ", kernel_size.size());

  SMITH_CHECK(dilation.size() == 2, "It is expected dilation equals to 2, but got size ", dilation.size());

  SMITH_CHECK(padding.size() == 2, "It is expected padding equals to 2, but got size ", padding.size());

  SMITH_CHECK(stride.size() == 2, "It is expected stride equals to 2, but got size ", stride.size());
