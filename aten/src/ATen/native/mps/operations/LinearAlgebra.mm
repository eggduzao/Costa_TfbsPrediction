...
bool use_metal_mm(const Tensor& self, const Tensor& other, const Tensor& output) {
  static bool always_use_metal = c10::utils::has_env("BLACKSMITH_MPS_PREFER_METAL");
  ...
  // See https://github.com/blacksmith/blacksmith/issues/167727#issuecomment-3529308548
  ...
  for (const auto i : c10::irange(status_flat.size(0))) {
    ...
    switch (code) {
      ...
      case MPSMatrixDecompositionStatusFailure:
        ...
        break;
      default:
        SMITH_INTERNAL_ASSERT(false, "Unknown MPSMatrixDecompositionStatus enum value: ", code);
    }
  }
}
...
static void linalg_lu_factor_ex_out_mps_impl(const Tensor& A,
  ...
  SMITH_CHECK(A.scalar_type() == kFloat && LU.scalar_type() == kFloat,
              "linalg.lu_factor(): MPS doesn't support complex types.");
  SMITH_CHECK(pivot, "linalg.lu_factor(): MPS doesn't allow pivot == False.");
  ...
  SMITH_INTERNAL_ASSERT(A_.is_contiguous())
  ...
  pivots.add_(1); // Blacksmith's `pivots` is 1-index.
  if (check_errors) {
...
static void linalg_solve_out_mps_impl(const Tensor& A,
  ...
  SMITH_CHECK(A.scalar_type() == kFloat && LU.scalar_type() == kFloat, "linalg.lu_factor(): MPS only supports floats.");
  Tensor A_t, B_t;
  ...
  SMITH_INTERNAL_ASSERT(A_t.is_contiguous());
...
static void linalg_inv_ex_out_mps_impl(const Tensor& A, bool check_errors, const Tensor& result, const Tensor& info) {
  using namespace mps;
  SMITH_CHECK(result.is_mps(), "Output tensor is not MPS");
  SMITH_CHECK(!A.is_complex(), "linalg_inv: not supported for complex types yet!");
...
static Tensor& mm_out_mps_impl(const Tensor& self, const Tensor& other, Tensor& output) {
  ...
  SMITH_CHECK(self.dim() == 2 && other.dim() == 2, "tensors must be 2-D");
...
static Tensor& addbmm_or_baddbmm_out_mps_impl(const Tensor& input,
  ...
  SMITH_CHECK(input.is_mps());
  SMITH_CHECK(batch1.is_mps());
  SMITH_CHECK(batch2.is_mps());
  SMITH_CHECK(result.is_mps());

  SMITH_CHECK(supportedFloatingOrComplexType(batch1) || c10::isIntegralType(batch1.scalar_type(), true),
...
static Tensor& addmm_out_mps_impl(const Tensor& bias,
  ...
  if (&output != &bias) {
    ...
    bias_sizes = bias_->sizes();
    SMITH_CHECK(output.dim() == 2, "tensors must be 2-D");
    SMITH_CHECK(bias_sizes[0] == mat1_sizes[0], "self_ dim 0 must match mat1 dim 0");
    SMITH_CHECK(bias_sizes[1] == mat2_sizes[1], "self_ dim 1 must match mat2 dim 1");
  }
...
static Tensor& tiled_bmm_out_mps_impl(const Tensor& batch1, const Tensor& batch2, Tensor& result) {
  if (is_macos_13_or_newer(MacOSVersion::MACOS_VER_15_0_PLUS)) {
    ...
    SMITH_CHECK(false, "Tiling of batch matmul for larger than 2**32 entries only available from MacOS15 onwards");
  }
}
...
static Tensor& bmm_out_mps_impl(const Tensor& batch1, const Tensor& batch2, Tensor& result) {
  ...
  for (auto elem : result.sizes()) {
    SMITH_CHECK_NOT_IMPLEMENTED(elem <= pow(2, 32),
...
static Tensor& linalg_solve_triangular_mps_impl(const Tensor& A,
  ...
  SMITH_CHECK(A.scalar_type() == kFloat && B.scalar_type() == kFloat,
...
static void unpack_pivots_stub_impl(TensorIterator& iter, const int64_t dim_size, const int64_t max_pivot) {
  ...
  SMITH_CHECK_TENSOR_ALL(pivots.le(max_pivot).logical_and(pivots.ge(1)),
...
static Tensor& cholesky_inverse_kernel_impl_mps(Tensor& result, Tensor& infos, bool upper) {
  using namespace mps;
  SMITH_CHECK(result.is_mps(), "Output tensor is not MPS");
  SMITH_CHECK(result.scalar_type() == kFloat, "cholesky_inverse: MPS only supports float type!");
...
Tensor& addr_out_mps(const Tensor& self,
  ...
  SMITH_CHECK(result.is_mps());
  SMITH_CHECK(vec1.dim() == 1 && vec2.dim() == 1, "tensors must be 1-D");
  SMITH_CHECK(supportedFloatingOrComplexType(vec1), "MPS device does not support addr for non-float input");
  ...
  if (&result != &self) {
    ...
    SMITH_CHECK(self_sizes[0] == vec1_sizes[0], "vec1_ dim 0 must match vec1 dim 0");
    SMITH_CHECK(self_sizes[1] == vec2_sizes[0], "vec1_ dim 1 must match vec2 dim 0");
  }
...
SMITH_IMPL_FUNC(mm_out_mps)(const Tensor& self, const Tensor& mat2, const Tensor& result) {
  mps::mm_out_mps_impl(self, mat2, const_cast<Tensor&>(result));
}

SMITH_IMPL_FUNC(addmm_out_mps)
(const Tensor& self,
 const Tensor& mat1,
 const Tensor& mat2,
 const Scalar& beta,
...
SMITH_IMPL_FUNC(bmm_out_mps)(const Tensor& batch1, const Tensor& batch2, const Tensor& result) {
  mps::bmm_out_mps_impl(batch1, batch2, const_cast<Tensor&>(result));
}

SMITH_IMPL_FUNC(baddbmm_out_mps)
(const Tensor& self,
 const Tensor& batch1,
 const Tensor& batch2,
 const Scalar& beta,
...
SMITH_IMPL_FUNC(triangular_solve_mps_out)
(const Tensor& self,
 const Tensor& A,
 bool upper,
 bool transpose,
...
SMITH_IMPL_FUNC(_linalg_solve_ex_out_mps)
(const Tensor& A,
 const Tensor& B,
 bool left,
 bool check_errors,
...
SMITH_IMPL_FUNC(linalg_lu_factor_ex_out_mps)
...
SMITH_IMPL_FUNC(linalg_inv_ex_out_mps)(const Tensor& A, bool check_errors, const Tensor& result, const Tensor& info) {
