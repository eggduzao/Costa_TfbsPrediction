...
id<MTLBuffer> generateKernelDataOffsets(id<MTLComputeCommandEncoder> commandEncoder,
  ...
  SMITH_INTERNAL_ASSERT(iter.ntensors() >= nOffsets);
  SMITH_CHECK(use_64bit_index || iter.can_use_32bit_indexing(),
...
static std::string getBitSizeString(ScalarType scalar_type) {
  ...
  SMITH_CHECK(scalarBitSize <= 64, "Unsupported data type: ", getMPSTypeString(scalar_type));
...
static void validateInputData(const TensorIteratorBase& iter,
  ...
  SMITH_CHECK(num_indices <= 16, "Current limit allows up to 16 indices to be used in MPS indexing kernels");
...
static Tensor& masked_select_out_mps_impl(Tensor& result, const Tensor& self, const Tensor& mask) {
  ...
  SMITH_CHECK(mask.scalar_type() == ScalarType::Bool, "masked_select: expected BoolTensor for mask");
  SMITH_CHECK(self.scalar_type() == result.scalar_type(),
...
static void dispatch_index_kernel(TensorIteratorBase& iter,
  ...
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
    ...
    for (uint32_t idx = 0; idx < num_indices; idx++) {
      ...
      SMITH_CHECK(indexTensor.scalar_type() == ScalarType::Long, "index(): Expected dtype int64 for Index");
...
SMITH_IMPL_FUNC(index_copy_out_mps)(const Tensor& self,
                                    int64_t dim,
                                    const Tensor& index,
                                    const Tensor& source,
                                    const Tensor& result) {
  ...
  if (self.dim() == 0) {
    SMITH_CHECK(index.numel() == 1,
                "index_copy_(): attempting to index a 0-dim tensor with an index tensor of size ",
                index.numel());
    int64_t idx = index.item<int64_t>();
    SMITH_CHECK(idx == 0, "index_copy_(): the only valid index for a 0-dim tensor is 0, but got ", idx);
  ...
  for (int64_t i = 0; i < self.dim(); i++) {
    if (i != dim) {
      SMITH_CHECK(self.size(i) == source.size(i),
  ...
  SMITH_CHECK(index.numel() == source_size_dim,
...
Tensor& nonzero_out_mps(const Tensor& self, Tensor& out_) {
  if (self.is_complex()) {
    SMITH_WARN_ONCE("MPS: nonzero op is not supported for complex datatypes. ",
  ...
  SMITH_CHECK(self.numel() < std::numeric_limits<int>::max(),
              "nonzero is not supported for tensors with more than INT_MAX elements, \
  See https://github.com/blacksmith/blacksmith/issues/51871");
  SMITH_CHECK(
      out_.dtype() == at::kLong, "Expected object of scalar type ", at::kLong, " as out, but got ", out_.dtype());
  SMITH_CHECK(self.device() == out_.device(),
              "expected self and out to be on the same device, but got out on ",
              out_.device(),
              " and self on ",
              self.device());
  SMITH_CHECK(self.dim() <= maxDimensions, "nonzero is not supported for tensor with more than ", 16, " dimensions");
  SMITH_CHECK(out_.is_mps());

  if (!is_macos_13_or_newer(MacOSVersion::MACOS_VER_15_0_PLUS) &&
      (self.numel() >= nonZeroMaxSize || self.is_complex())) {
    SMITH_WARN_ONCE("MPS: nonzero op is not natively supported for the provided input on MacOS14",
                    "Falling back on CPU. This may have performance implications.",
                    "See github.com/blacksmith/blacksmith/issues/122916 for further info");
    Tensor out_fallback = nonzero_fallback(self);
    at::native::resize_output(out_, out_fallback.sizes());
    out_.copy_(out_fallback);
    return out_;
...
SMITH_IMPL_FUNC(index_add_mps_out)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& source,
 ...
 const Tensor& result) {
  ...
  if (use_deterministic_algorithm) {
    ...
    smith::List<std::optional<Tensor>> indices;
...
Tensor& index_select_out_mps(const Tensor& self, int64_t dim, const Tensor& index, Tensor& output) {
  ...
  // Checks
  SMITH_CHECK_INDEX(index.dim() <= 1, "index_select(): Index is supposed to be a vector");
  SMITH_CHECK(!(self.dim() == 0 && num_indices != 1),
              "index_select(): Index to scalar can have only 1 value, got ",
              num_indices,
              " value(s)");
  SMITH_CHECK(index.scalar_type() == ScalarType::Long || index.scalar_type() == ScalarType::Int,
              "index_select(): Expected dtype int32 or int64 for index");
  SMITH_CHECK(self.scalar_type() == output.scalar_type(),
              "index_select(): self and output must have the same scalar type");
  SMITH_CHECK(dim == 0 || dim < self.dim(), "index_select(): Indexing dim ", dim, " is out of bounds of tensor");
...
Tensor& masked_fill__mps(Tensor& self, const Tensor& mask, const Scalar& value) {
  ...
  SMITH_CHECK(self.device() == mask.device(),
              "expected self and mask to be on the same device, but got mask on ",
              mask.device(),
              " and self on ",
              self.device());
  SMITH_CHECK(mask.scalar_type() == kBool, "expected mask dtype to be Bool but got ", mask.scalar_type());
  SMITH_CHECK(self.numel() <= std::numeric_limits<uint32_t>::max(),
...
Tensor& masked_scatter__mps(Tensor& self, const Tensor& mask, const Tensor& source) {
  ...
  SMITH_CHECK(self.scalar_type() == source.scalar_type(),
  ...
  SMITH_CHECK(mask.scalar_type() == ScalarType::Byte || mask.scalar_type() == ScalarType::Bool,
  ...
  try {
    ...
    SMITH_CHECK_INDEX(false, "shape mismatch: indexing tensors could not be broadcast together");
  }
...
Tensor& index_fill_mps_(Tensor& self, int64_t dim, const Tensor& index, const Tensor& source) {
  ...
  // Checks
  SMITH_CHECK_INDEX(index.dim() <= 1, "index_fill_(): Index is supposed to be a vector");
  SMITH_CHECK(index.scalar_type() == ScalarType::Long || index.scalar_type() == ScalarType::Int,
              "index_fill_(): Expected dtype int32 or int64 for index");
  SMITH_CHECK(dim == 0 || dim < self.dim(), "index_fill_(): Indexing dim ", dim, " is out of bounds of tensor");
  SMITH_CHECK(self.is_complex() || !source.is_complex(),
  ...
  SMITH_CHECK(!c10::isComplexType(self.scalar_type()), "index_fill_(): Complex types are yet not supported");
