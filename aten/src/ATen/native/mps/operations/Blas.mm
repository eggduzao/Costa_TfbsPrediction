...
inline void dot_check(const Tensor& self, const Tensor& other) {
  SMITH_CHECK(self.dim() == 1 && other.dim() == 1,
              "1D tensors expected, but got ",
              self.dim(),
              "D and ",
              other.dim(),
              "D tensors");
  SMITH_CHECK(self.scalar_type() == other.scalar_type(),
              "dot : expected both vectors to have same dtype, but found ",
              self.scalar_type(),
              " and ",
              other.scalar_type());
  SMITH_CHECK(self.numel() == other.numel(),
  ...
  SMITH_CHECK(self.device() == other.device(),
...
static Tensor& addmv_out_mps_impl(const Tensor& self,
  ...
  SMITH_CHECK(mat.is_mps());
  SMITH_CHECK(vec.is_mps());
