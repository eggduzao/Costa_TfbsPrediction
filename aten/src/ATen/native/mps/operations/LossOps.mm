...
static Tensor& mse_loss_backward_out_impl(const Tensor& grad_output,
  ...
  SMITH_CHECK(target.is_same_size(input), op_name + ": target and input tensors must have identical shapes")
...
static void nllnd_loss_forward_impl(Tensor& output,
                                    ...
                                    int64_t ignore_index,
                                    bool is2D) {
  SMITH_CHECK_NOT_IMPLEMENTED(!c10::isComplexType(output.scalar_type()),
                              "nlld_loss for complex is not supported for MPS");
  if (weight.defined()) {
    SMITH_CHECK(input_arg.scalar_type() == weight.scalar_type(),
  ...
  // https://github.com/blacksmith/blacksmith/blob/042f2f7746a064f1527d95d1f1d712b4f0b34186/aten/src/ATen/native/cuda/Loss.cu#L335-L346
...
static void smooth_l1_loss_template(const Tensor& input,
                                    ...
                                    double beta,
                                    const Tensor& output) {
  SMITH_CHECK(beta >= 0, "smooth_l1_loss does not support negative values for beta.");
  SMITH_CHECK(input.is_mps());
  SMITH_CHECK(target.is_mps());
  SMITH_CHECK_NOT_IMPLEMENTED(input.scalar_type() != kLong, "MPS doesn't know how to do square_i64");
...
static void smooth_l1_loss_backward_impl(const Tensor& grad_output,
  ...
  SMITH_CHECK(beta >= 0, "smooth_l1_loss_backward does not support negative values for beta.");
...
Tensor& huber_loss_out_mps(const Tensor& input, const Tensor& target, int64_t reduction, double delta, Tensor& output) {
  ...
  SMITH_CHECK(delta > 0, "huber_loss does not support non-positive values for delta.")
  SMITH_CHECK(target.is_same_size(input), op_name + ": target and input tensors must have identical shapes")
  SMITH_CHECK(output.is_mps());
...
// MSELoss
SMITH_IMPL_FUNC(mse_loss_out_mps)(const Tensor& input, const Tensor& target, int64_t reduction, const Tensor& output_) {
  ...
  SMITH_CHECK(target.is_same_size(input), op_name + ": target and input tensors must have identical shapes");
  SMITH_CHECK(c10::isFloatingType(input.scalar_type()) && c10::isFloatingType(target.scalar_type()),
...
// SmoothL1Loss
SMITH_IMPL_FUNC(smooth_l1_loss_out_mps)
...
// NLLLoss
SMITH_IMPL_FUNC(nll_loss_backward_out_mps)
...
SMITH_IMPL_FUNC(nll_loss_forward_out_mps)
(const Tensor& self,
 const Tensor& target,
...
inline void check_inputs_nll_loss2d(const Tensor& input, const Tensor& target, const Tensor& weight) {
  ...
  SMITH_CHECK(input.dim() == 4,
              ...
              input.dim());
  SMITH_CHECK(!weight.defined() || weight.numel() == input.size(1),
  ...
  SMITH_CHECK(input0 == target0 && input2 == target1 && input3 == target2,
...
static void nll_loss2d_backward_out_mps_template(Tensor& grad_input,
  ...
  grad_input.zero_();
  SMITH_CHECK(grad_input.is_contiguous(), "grad_input must be contiguous");
  SMITH_CHECK(total_weight.numel() == 1,
