...
SMITH_IMPL_FUNC(gather_out_mps)
(const Tensor& self_arg, int64_t dim, const Tensor& index, bool sparse_grad, const Tensor& output) {
  ...
  SMITH_CHECK(!sparse_grad, "sparse_grad not supported in MPS yet")
  SMITH_CHECK(self.scalar_type() == output.scalar_type(), "gather(): self and output must have the same scalar type");
  SMITH_CHECK(dim >= 0 && dim < self.dim(), "gather(): Indexing dim ", dim, " is out of bounds of tensor");
  SMITH_CHECK(!self.is_complex(), "gather(): Yet not supported for complex");
  ...
  @autoreleasepool {
    ...
    SMITH_CHECK(num_input_dims == num_index_dims, "Input and index must have same rank")

    // Determine if we need to slice into the input tensor
    bool needSlice = false;

    for (const auto i : c10::irange(num_input_dims)) {
      SMITH_CHECK(i == dim || [index_shape[i] intValue] <= [input_shape[i] intValue],
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      ...
      // Blacksmith issue #135240: MPS reshape underneath goes haywire in
...
static void scatter_mps_general(const Tensor& self_arg,
  ...
  SMITH_CHECK(self.scalar_type() == output.scalar_type() && output.scalar_type() == src.scalar_type(),
              "scatter(): self, src and output must have the same scalar type");
  SMITH_CHECK(dim >= 0 && dim < self.dim(), "scatter(): Indexing dim ", dim, " is out of bounds of tensor");
  SMITH_CHECK(!self.is_complex(), "scatter(): Yet not supported for complex");
  ...
  @autoreleasepool {
    ...
    SMITH_CHECK(num_input_dims == num_index_dims && num_index_dims == num_src_dims,
    ...
    for (const auto i : c10::irange(num_input_dims)) {
      SMITH_CHECK(i == dim || [index_shape[i] intValue] <= [input_shape[i] intValue],
                  "Index dim must not exceed input dim except at gathering axis")
      SMITH_CHECK([index_shape[i] intValue] <= [src_shape[i] intValue],
                  "Index dim must not exceed input dim except at gathering axis")
    ...
    SMITH_CHECK(reduce != "mean", "Scatter reduce mean mode not yet supported in MPS")
...
SMITH_IMPL_FUNC(scatter_src_out_mps)
...
SMITH_IMPL_FUNC(scatter_value_out_mps)
...
SMITH_IMPL_FUNC(scatter_reduce_out_mps)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Tensor& src,
...
SMITH_IMPL_FUNC(scatter_value_reduce_out_mps)
(const Tensor& self,
 int64_t dim,
 const Tensor& index,
 const Scalar& value,
...
SMITH_IMPL_FUNC(scatter_add_mps_out)
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src, const Tensor& output) {
...
static void scatter_reduce_two_mps_kernel(const Tensor& self,
  ...
  switch (reduce) {
    ...
    case ReductionType::MIN:
      SMITH_CHECK(self.scalar_type() != kInt || is_macOS_15_or_newer, "smith.int32 is only supported on MacOS15+");
      SMITH_CHECK(self.scalar_type() != kLong, "not supported for smith.int64");
  ...
  SMITH_CHECK(false, "Unsupported reduction type: ", static_cast<int>(reduce));
}
