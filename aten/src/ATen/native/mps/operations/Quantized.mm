...
Tensor _convert_weight_to_int4pack_mps(const Tensor& in, int64_t innerKTiles) {
  SMITH_CHECK(in.dim() == 2, __func__, " : expect weight to be 2D tensor.");
  SMITH_CHECK(in.dtype() == at::kByte, __func__, " : expect weight to be kByte.");
...
Tensor _weight_int4pack_mm_mps(const Tensor& A, const Tensor& B, int64_t qGroupSize, const Tensor& qScaleAndZeros) {
  ...
  SMITH_CHECK(B.is_contiguous(), __func__, " : expect B to be contiguous.");
  SMITH_CHECK(B.dim() == 4, __func__, " : expect B to 4d tensor.");

  SMITH_CHECK(qGroupSize == 32 || qGroupSize == 64 || qGroupSize == 128 || qGroupSize == 256,
              __func__,
              ": expect qGroupSize to be 32, 64, 128 or 256, got ",
              qGroupSize);

  SMITH_CHECK(qScaleAndZeros.dim() == 3 && qScaleAndZeros.size(1) == N && qScaleAndZeros.size(2) == 2,
              __func__,
...
Tensor _weight_int8pack_mm_mps(const Tensor& A, const Tensor& B, const Tensor& scales) {
  ...
  SMITH_CHECK(B.dtype() == kChar, __func__, " : expect B to be int8 tensor.");
  SMITH_CHECK(B.is_contiguous(), __func__, " : expect B to be contiguous.");
  SMITH_CHECK(B.size(1) == K, __func__, " : expect B.size(1) == ", K);

  SMITH_CHECK(scales.dim() == 1 && scales.size(0) == N, __func__, " : expect scales to be 1d tensor with size ", N);

  auto C = at::empty({M, N}, A.options());
  SMITH_CHECK(N % 32 == 0 && K % 32 == 0);
#if 1
  MPSStream* mpsStream = getCurrentMPSStream();
  std::array<uint32_t, 4> sizes = {static_cast<uint32_t>(M), static_cast<uint32_t>(K), static_cast<uint32_t>(N), 0};
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
