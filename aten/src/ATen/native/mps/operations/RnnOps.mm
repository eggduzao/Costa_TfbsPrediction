...
/**
 * Accepts tensors in Blacksmith API format and returns tensors in MPS API format
...
getMPSTensorsFromBlacksmithTensors(MPSGraph* mpsGraph,
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> _lstm_mps(const Tensor& input,
  ...
  if (has_projections) {
    SMITH_CHECK(false, "LSTM with projections is not currently supported with MPS.");
  }
  ...
  @autoreleasepool {
    ...
    auto cachedGraph = LookUpOrCreateCachedGraph<CachedGraph>(key, [&](auto mpsGraph, auto newCachedGraph) {
      ...
      for (int i = 0; i < num_layers; i++) {
        auto tensorsData = getMPSTensorsFromBlacksmithTensors(mpsGraph,
                                                           stateTensor,
                                                           cellStateTensor,
                                                           recurrentKernelWeightsList,
                                                           kernelWeightsList,
  ...
  @autoreleasepool {
    ...
    for (const auto i : c10::irange(total_layers)) {
      ...
      if (has_biases) {
        ...
        // In Blacksmith LSTM API there are two biases. The second bias is included for CuDNN compatibility.
