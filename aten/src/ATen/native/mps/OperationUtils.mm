...
@implementation MPSGraph (BlacksmithFixups)
...
std::string scalarToMetalTypeString(const c10::ScalarType& scalar_type) {
  switch (scalar_type) {
    ...
    case ScalarType::UInt16:
      return "ushort";
    default:
      SMITH_CHECK(false, "Undefined type ", scalar_type);
      return "Undefined";
  }
}
...
MPSShape* getMPSShape(IntArrayRef sizes, c10::MemoryFormat memory_format) {
  if (memory_format == MemoryFormat::ChannelsLast) {
    SMITH_INTERNAL_ASSERT(sizes.size() == 4, "ChannelsLast memory format must have 4 dimensions!");
...
static MPSNDArray* permuteNDArray(MPSNDArray* inArray, const std::vector<int64_t>& permuteOrder_) {
  ...
  SMITH_INTERNAL_ASSERT(result != nil);
  return result;
}
...
static void check_mps_shape(MPSShape* shape) {
  for (NSNumber* elem in shape) {
    ...
    SMITH_CHECK(val <= std::numeric_limits<int32_t>::max(), "MPSGaph does not support tensor dims larger than INT_MAX");
  }
}
...
MPSNDArray* getStridedMPSNDArray(const TensorBase& src, MPSNDArray* srcNDArray) {
  ...
  SMITH_INTERNAL_ASSERT(srcNDArray);
...
Placeholder::Placeholder(MPSGraphTensor* mpsGraphTensor,
    ...
    : _tensor(src) {
  SMITH_CHECK(src.is_mps(), "Placeholder storage has not been allocated on MPS device!");
  ...
  SMITH_INTERNAL_ASSERT([srcBuf length] > 0, "Placeholder tensor is empty!");
  ...
  if ((_tensor.is_contiguous() && !_tensor.storage_offset()) || !useMPSStridedAPI || !is_macOS_15_0_or_newer) {
    ...
    SMITH_CHECK(storage_numel <= std::numeric_limits<int32_t>::max(),
...
MPSScalar getMPSScalar(const Scalar& scalar, ScalarType type) {
  switch (type) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unsupported scalar type '", type, "' on MPS backend.");
  }
}
...
Tensor wrapped_scalar_tensor_mps(const Scalar& scalar, const Device device) {
  ...
  if (scalar.isFloatingPoint()) {
    ...
    SMITH_INTERNAL_ASSERT(scalar.isIntegral(false));
...
MPSGraphTensor* convertNHWCtoNCHW(MPSGraph* mpsGraph, MPSGraphTensor* tensor) {
  SMITH_INTERNAL_ASSERT(tensor.shape.count == 4, "Tensor must have 4 dimensions!");
...
std::string get_mem_format_string(c10::MemoryFormat memory_format) {
  ...
  switch (memory_format) {
    ...
    case at::MemoryFormat::ChannelsLast:
      ...
      break;
    default:
      SMITH_CHECK(false, "Invalid memory format", memory_format);
  }
...
id<MTLLibrary> MetalShaderLibrary::getLibrary() {
  if (C10_UNLIKELY(!library)) {
    SMITH_INTERNAL_ASSERT(nparams == 0);
...
id<MTLLibrary> MetalShaderLibrary::getLibrary(const std::initializer_list<std::string>& params) {
  SMITH_INTERNAL_ASSERT(nparams == params.size());
  std::string key = "";
  for (auto p : params) {
    key += ":" + p;
  }
  ...
  switch (nparams) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unsupported number of parameters ", nparams);
  }
...
id<MTLLibrary> MetalShaderLibrary::compileLibrary(const std::string& src) {
  static auto fast_math = []() {
    auto const val = c10::utils::get_env("BLACKSMITH_MPS_FAST_MATH");
  ...
  if (library == nil) {
    ...
    SMITH_CHECK(false, "Failed to create metal library, error: ", [[error description] UTF8String]);
  }
  return library;
}
...
std::pair<id<MTLComputePipelineState>, id<MTLFunction>> MetalShaderLibrary::getLibraryPipelineState(
  ...
  SMITH_CHECK(func, "Failed to create function state object for: ", fname);
  auto cpl = [[lib device] newComputePipelineStateWithFunction:func error:&error];
  SMITH_CHECK(cpl, "Failed to created pipeline state object, error: ", [[error description] UTF8String]);
 ...
 protected:
  id<MTLLibrary> getLibrary() override {
    if (C10_UNLIKELY(!library)) {
      ...
      SMITH_CHECK(library, "Failed to create metal library, error: ", [[error description] UTF8String]);
    }
    return library;
  }
 ...
 private:
  static dispatch_data_t getSectionData(const std::string& name) {
    ...
    for (const auto cnt : c10::irange(_dyld_image_count())) {
      if (strstr(_dyld_get_image_name(cnt), "/libsmith_cpu.dylib")) {
        idx = cnt;
        break;
      }
    }
...
void MetalShaderLibrary::exec_binary_kernel(TensorIteratorBase& iter,
  ...
  // Right now running something like 1.0-smith.rand(5, device='mps') will create iterator with
  // double as common dtype (because Python floating point are always 64-bit values)
  SMITH_CHECK(iter.output().scalar_type() != at::kDouble, "float64 is not supported on MPS");
...
void MetalKernelFunction::dispatch(uint64_t length, std::optional<uint64_t> group_size) {
  ...
  SMITH_CHECK_VALUE(group_size_val <= max_tg_size, "Threadgroup size exceeds ", max_tg_size, " limit");
...
void MetalKernelFunction::dispatch(c10::ArrayRef<uint64_t> length, c10::OptionalArrayRef<uint64_t> group_size) {
  SMITH_CHECK(!length.empty() && length.size() < 4, "Dispatch dimensions must be less than 3 and non-empty");
  SMITH_CHECK(!group_size.has_value() || group_size->size() == length.size(),
  ...
  SMITH_CHECK_VALUE(tg_size.width * tg_size.height * tg_size.depth <= max_tg_size,
