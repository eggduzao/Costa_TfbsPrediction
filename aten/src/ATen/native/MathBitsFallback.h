...
struct MathOpFallback {
  ...
  void fallback_impl(const c10::OperatorHandle& op, DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
    /*
    ...
    */
    ...
    for (const auto i : c10::irange(num_arguments)) {
      ...
      if (ivalue.isTensor()) {
        ...
        if (mut_arg) {
          SMITH_CHECK(mutable_inputs_with_their_clones.empty(), op_name, " fallback does not support operators with more than one mutable tensors with ",
        ...
        for(const auto j : c10::irange(tensors.size())) {
          ...
          SMITH_CHECK(!mut_arg, " fallback doesn't currently support mutable TensorLists with ",
    ...
    SMITH_INTERNAL_ASSERT(mutable_inputs_with_their_clones.size() <= 1);
    ...
    for (std::pair<Tensor, Tensor> mut_tensors: mutable_inputs_with_their_clones) {
      ...
      SMITH_INTERNAL_ASSERT(cloned_mutable_input.is_same(returned_output));
