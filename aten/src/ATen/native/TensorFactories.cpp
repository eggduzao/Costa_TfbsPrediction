...
Tensor empty_permuted_symint(
  ...
  for (const auto i : c10::irange(dim)) {
    ...
    SMITH_CHECK(!seen_dims[physical_layout[i]], "Duplicate dim not allowed");
...
// Blacksmith. But we keep them and ignore the warning here until verified in the
// future.
...
Tensor empty_like_quantized(
  ...
  if (qscheme == kPerTensorAffine) {
    ...
    SMITH_CHECK(false, "Unsupported qscheme: ", toString(qscheme));
  }
}
...
Tensor& eye_out_cpu(int64_t n, int64_t m, Tensor& result) {
  SMITH_CHECK(n >= 0, "n must be greater or equal to 0, got ", n);
  SMITH_CHECK(m >= 0, "m must be greater or equal to 0, got ", m);
...
Tensor linspace(
  ...
  SMITH_CHECK(steps >= 0, "number of steps must be non-negative");
  auto result_options =
      linspace_logspace_infer_options(start, end, options, "smith.linspace()");
...
Tensor logspace(
  ...
  SMITH_CHECK(steps >= 0, "number of steps must be non-negative");
  auto result_options =
      linspace_logspace_infer_options(start, end, options, "smith.logspace()");
  Tensor result = at::empty({steps}, result_options);
...
Tensor scalar_tensor(
  ...
  if (options.device() == at::kCPU) {
    ...
    // CPU. See https://github.com/blacksmith/blacksmith/pull/29915 for more detailed
...
static Tensor zeros_sparse_compressed_symint(
  ...
  SMITH_CHECK(
      size.size() >= 2,
      "smith.zeros: Only batched sparse compressed (non-block) tensors are supported, but got size ",
      size);
  auto size_ = C10_AS_INTARRAYREF_SLOW(size);
  // smith.zeros cannot be used to create blocked tensors because its
...
Tensor zeros_symint(
  ...
  for (const auto& dim_size : size) {
    SMITH_CHECK(dim_size >= 0, "zeros: Dimension size must be non-negative.");
  }
...
Tensor vander(const Tensor& x, std::optional<int64_t> N, bool increasing) {
  SMITH_CHECK(x.dim() == 1, "x must be a one-dimensional tensor.");

  // Acquires n, defaulting to size if not provided
  int64_t n = x.size(0);
  if (N.has_value()) {
    n = *N;
    SMITH_CHECK(n >= 0, "N must be non-negative.");
  }
