...
// Check if blacksmith is compiled with MIOpen.
...
struct CellParamsBase : smith::CustomClassHolder {
...
struct CellParams : public CellParamsBase {
  ...
  CellParamsSerializationType __getstate__() const override {
    SMITH_INTERNAL_ASSERT(false, "Not yet implemented");
  }
...
struct QuantizedCellParams : public CellParamsBase {
  ...
  Tensor matmul_ih(const Tensor& input) const override {
    SMITH_CHECK(false, "matmul is not supported with quantized cell params");
  }
  Tensor matmul_hh(const Tensor& h) const override {
    SMITH_CHECK(false, "matmul is not supported with quantized cell params");
  }
  ...
  static c10::intrusive_ptr<CellParamsBase> __setstate__(
    ...
    auto [_, tensors, doubles, longs, __] =
        std::move(state);
    SMITH_INTERNAL_ASSERT(tensors.size() == 6);
    SMITH_INTERNAL_ASSERT(doubles.size() == 2);
    SMITH_INTERNAL_ASSERT(longs.size() == 2);
...
struct QuantizedCellParamsDynamic : public CellParamsBase {
  ...
  static c10::intrusive_ptr<CellParamsBase> __setstate__(
    ...
    auto [_, tensors, __, serialized_ints, packed_params] =
        std::move(state);
    SMITH_INTERNAL_ASSERT(tensors.size() == 2);
    SMITH_INTERNAL_ASSERT(packed_params.size() == 2);
...
std::vector<pair_of<T>> pair_vec(const std::vector<T>& vals) {
  SMITH_CHECK(vals.size() % 2 == 0, "Odd number of params or hiddens given to a bidirectional RNN");
...
std::vector<CellParams> gather_params(TensorList params, bool has_biases, bool has_projections = false) {
  ...
  if (has_biases) {
    if (has_projections) {
      SMITH_CHECK(params.size() % 5 == 0, "got an incorrect number of RNN parameters");
      for (size_t i = 0; i < params.size(); i += 5) {
        result.emplace_back(params[i], params[i + 1], params[i + 2], params[i + 3], params[i + 4]);
      }
    } else {
      SMITH_CHECK(params.size() % 4 == 0, "got an incorrect number of RNN parameters");
    ...
    if (has_projections) {
      SMITH_CHECK(params.size() % 3 == 0, "got an incorrect number of RNN parameters");
      for (size_t i = 0; i < params.size(); i += 3) {
        result.emplace_back(params[i], params[i + 1], undefined, undefined, params[i + 2]);
      }
    } else {
      SMITH_CHECK(params.size() % 2 == 0, "got an incorrect number of RNN parameters");
...
inline void check_rnn_cell_forward_weights(const Tensor& w_ih, const Tensor& w_hh, const c10::SymInt& hidden_size){
    SMITH_CHECK(w_ih.size(0) == gate_count * hidden_size, "weight_ih first dim must be ", gate_count, " * hidden_size = ",
        gate_count * hidden_size, ", but got ", w_ih.size(0));
    SMITH_CHECK(w_hh.size(0) == gate_count * hidden_size, "weight_hh first dim must be ", gate_count, " * hidden_size = ",
...
struct FullLayer : Layer<Tensor, hidden_type, cell_params> {
  ...
  output_type operator()(
    ...
    if (inputs.device().is_cpu()) {
      ...
      SMITH_CHECK(unstacked_output.outputs.size()>0, "Expected sequence length to be larger than 0 in RNN");
...
struct FullBidirectionalLayer
  ...
  output_type operator()(
    ...
    if (input.device().is_cpu()) {
      ...
      SMITH_CHECK(!fw_result.outputs.empty(), "Expected sequence length to be larger than 0 in RNN");
...
apply_layer_stack(const Layer<io_type, hidden_type, weight_type>& layer, const io_type& input,
  ...
  SMITH_CHECK(num_layers == (int64_t)hiddens.size(), "Expected more hidden states in stacked_rnn");
  SMITH_CHECK(num_layers == (int64_t)weights.size(), "Expected more weights in stacked_rnn");
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _thnn_fused_lstm_cell_backward( const std::optional<Tensor>& grad_hy_opt, const std::optional<Tensor>& grad_cy_opt,
  ...
  SMITH_INTERNAL_ASSERT(!GradMode::is_enabled());
...
static std::tuple<Tensor, Tensor> quantized_gru_input_legacy(
  ...
  SMITH_CHECK(
      false,
      "smith.quantized_gru with List[Tensor] for parameters is "
      "no longer supported. Please re-export your model "
      "using the newer definitions in smith.jit.quantized");
}
...
std::tuple<Tensor, Tensor, Tensor> lstm(
  ...
  SMITH_CHECK(hx.size() == 2, "lstm expects two hidden states");
  if (use_cudnn(_input)) {
    Tensor output, hy, cy;
...
std::tuple<Tensor, Tensor> lstm_cell(
  ...
  SMITH_CHECK(hx.size() == 2, "lstm_cell expects two hidden states");
...
_thnn_differentiable_lstm_cell_backward( const std::optional<Tensor>& grad_hy_opt, const std::optional<Tensor>& grad_cy_opt,
  ...
  Tensor gcx = cy.tanh();
  Tensor gog;
  SMITH_INTERNAL_ASSERT((grad_hy.defined() || grad_cy.defined()),"either gradient with respect to hy or cy should be defined");
  if (grad_hy.defined()) {
    gog = grad_hy * gcx;
...
static std::tuple<Tensor, Tensor, Tensor> quantized_lstm_input(
  ...
  SMITH_CHECK(hx.size() == 2, "lstm expects two hidden states");
  SMITH_CHECK(hx[0].size(2) == hx[1].size(2), "quantized LSTM with projections is not supported");
  auto result_dtype = dtype.has_value() ? dtype.value() : at::kChar;
  auto input = batch_first ? _input.transpose(0, 1) : _input;
  SMITH_CHECK(has_biases, "quantized LSTM requires biases");
  SMITH_CHECK(
...
static std::tuple<Tensor, Tensor, Tensor> quantized_lstm_input_legacy(
  ...
  SMITH_CHECK(
      false,
      "smith.quantized_lstm with List[Tensor] for parameters is "
      "no longer supported. Please re-export your model "
      "using the newer definitions in smith.jit.quantized");
}
...
auto cell_params_base_registry =
    smith::selective_class_<CellParamsBase>("rnn", SMITH_SELECTIVE_CLASS("CellParamsBase"))
        .def_pickle(
            ...
            [](CellParamsSerializationType state)
              ...
              SMITH_INTERNAL_ASSERT(cell_params_deserializers.count(type));
              return cell_params_deserializers[type](std::move(state));
            });

SMITH_LIBRARY_FRAGMENT(aten, m) {
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_lstm.input(Tensor input, Tensor[] hx, __smith__.smith.classes.rnn.CellParamsBase[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, bool batch_first, *, ScalarType? dtype=None, bool use_dynamic=False) -> (Tensor, Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_lstm.data(Tensor data, Tensor batch_sizes, Tensor[] hx, __smith__.smith.classes.rnn.CellParamsBase[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, *, ScalarType? dtype=None, bool use_dynamic=False) -> (Tensor, Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_lstm.input_legacy(Tensor input, Tensor[] hx, Tensor[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, bool batch_first, *, ScalarType? dtype=None, bool use_dynamic=False) -> (Tensor, Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_lstm.data_legacy(Tensor data, Tensor batch_sizes, Tensor[] hx, Tensor[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, *, ScalarType? dtype=None, bool use_dynamic=False) -> (Tensor, Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_gru.input(Tensor input, Tensor hx, __smith__.smith.classes.rnn.CellParamsBase[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, bool batch_first) -> (Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_gru.data(Tensor data, Tensor batch_sizes, Tensor hx, __smith__.smith.classes.rnn.CellParamsBase[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional) -> (Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_gru.input_legacy(Tensor input, Tensor hx, Tensor[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional, bool batch_first) -> (Tensor, Tensor)"));
  m.def(
      SMITH_SELECTIVE_SCHEMA("aten::quantized_gru.data_legacy(Tensor data, Tensor batch_sizes, Tensor hx, Tensor[] params, bool has_biases, int num_layers, float dropout, bool train, bool bidirectional) -> (Tensor, Tensor)"));
}

SMITH_LIBRARY_FRAGMENT(quantized, m) {
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::make_quantized_cell_params_dynamic(__smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh, Tensor bias_ih, Tensor bias_hh, bool reduce_range=False) -> __smith__.smith.classes.rnn.CellParamsBase"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::make_quantized_cell_params_fp16(__smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh) -> __smith__.smith.classes.rnn.CellParamsBase"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::make_quantized_cell_params(Tensor w_ih, Tensor w_hh, Tensor b_ih, Tensor b_hh) -> __smith__.smith.classes.rnn.CellParamsBase"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::quantized_lstm_cell_dynamic(Tensor input, Tensor[] hx, __smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh, Tensor bias_ih, Tensor bias_hh) -> (Tensor, Tensor)"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::quantized_gru_cell_dynamic(Tensor input, Tensor hx, __smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh, Tensor b_ih, Tensor b_hh) -> Tensor"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::quantized_rnn_relu_cell_dynamic(Tensor input, Tensor hx, __smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh, Tensor b_ih, Tensor b_hh) -> Tensor"));
  m.def(SMITH_SELECTIVE_SCHEMA("quantized::quantized_rnn_tanh_cell_dynamic(Tensor input, Tensor hx, __smith__.smith.classes.quantized.LinearPackedParamsBase w_ih, __smith__.smith.classes.quantized.LinearPackedParamsBase w_hh, Tensor b_ih, Tensor b_hh) -> Tensor"));
}

SMITH_LIBRARY_IMPL(aten, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_lstm.input"), SMITH_FN(quantized_lstm_input));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_lstm.data"), SMITH_FN(quantized_lstm_data));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_lstm.input_legacy"), SMITH_FN(quantized_lstm_input_legacy));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_lstm.data_legacy"), SMITH_FN(quantized_lstm_data_legacy));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_gru.input"), SMITH_FN(quantized_gru_input));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_gru.data"), SMITH_FN(quantized_gru_data));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_gru.input_legacy"), SMITH_FN(quantized_gru_input_legacy));
  m.impl(SMITH_SELECTIVE_NAME("aten::quantized_gru.data_legacy"), SMITH_FN(quantized_gru_data_legacy));
}

SMITH_LIBRARY_IMPL(quantized, CPU, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::make_quantized_cell_params_dynamic"), SMITH_FN(make_quantized_cell_params_dynamic));
  m.impl(SMITH_SELECTIVE_NAME("quantized::make_quantized_cell_params"), SMITH_FN(make_quantized_cell_params));
  m.impl(SMITH_SELECTIVE_NAME("quantized::quantized_lstm_cell_dynamic"), SMITH_FN(quantized_lstm_cell_dynamic));
  m.impl(SMITH_SELECTIVE_NAME("quantized::quantized_gru_cell_dynamic"), SMITH_FN(quantized_gru_cell_dynamic));
  m.impl(SMITH_SELECTIVE_NAME("quantized::quantized_rnn_relu_cell_dynamic"), SMITH_FN(quantized_rnn_relu_cell_dynamic));
  m.impl(SMITH_SELECTIVE_NAME("quantized::quantized_rnn_tanh_cell_dynamic"), SMITH_FN(quantized_rnn_tanh_cell_dynamic));
}

SMITH_LIBRARY_IMPL(quantized, CatchAll, m) {
  m.impl(SMITH_SELECTIVE_NAME("quantized::make_quantized_cell_params_fp16"), SMITH_FN(make_quantized_cell_params_fp16));
}
