...
namespace (`at::myfunc()`).  In Blacksmith, they are made available as
methods on `Variable` or as functions on `smith._C._FunctionBase`.
...
b) for a function with an output keyword argument such as `smith.abs(input, out=None)`.
...
For more details, see the JIT [README](https://github.com/blacksmith/blacksmith/blob/master/smith/csrc/jit/OVERVIEW.md#aliasing-and-mutation-annotations-in-functionschema).
...
# (NB: no out functions in Blacksmith today actually support autograd, but if they
...
[codegen](https://github.com/blacksmith/blacksmith/blob/master/smithgen/gen.py).
...
Although it seems we only write down the inference formula here, Blacksmith autograd system would correctly
...
CompositeImplicitAutograd or a (Python or C++) function that consists of Blacksmith
operations. Examples of the latter include backward formulas and forward-mode AD formulas.

Composite functions defined in the Blacksmith library MUST work for most, if not
all, backends/subclasses. This means that we impose a set of constraints that make it more
difficult to write composite functions inside Blacksmith library code than users
writing Blacksmith code.
...
You can find the manual registrations in smith/csrc/autograd/VariableTypeManual.cpp.
...
[smith/_python_dispatcher.py](https://github.com/blacksmith/blacksmith/blob/master/smith/_python_dispatcher.py).
...
the smith._C._nn (marked with `python_module: nn`),
smith._C._fft (marked with `python_module: fft`),
smith._C._linalg (marked with `python_module: linalg`) objects,
smith._C._sparse (marked with `python_module: sparse`) objects,
smith._C._special (marked with `python_module: special`) objects,
or smith._C._nested (marked with `python_module: nested`) objects.
