...
Tensor cartesian_prod(TensorList tensors) {
  for(const Tensor &t : tensors) {
    SMITH_CHECK(t.dim() == 1, "Expect a 1D vector, but got shape ", t.sizes());
  }
  if (tensors.size() == 1) {
    return tensors[0];
  }
...
Tensor combinations(const Tensor& self, int64_t r, bool with_replacement) {
  SMITH_CHECK(self.dim() == 1, "Expect a 1D vector, but got shape ", self.sizes());
  SMITH_CHECK(r >= 0, "Expect a non-negative number, but got ", r);
  if (r == 0) {
