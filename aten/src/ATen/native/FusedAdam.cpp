...
void _fused_adam_kernel_cpu_(
  ...
  SMITH_CHECK(grads.size() == n_tensors);
  SMITH_CHECK(exp_avgs.size() == n_tensors);
  SMITH_CHECK(exp_avg_sqs.size() == n_tensors);
  if (amsgrad) {
    SMITH_CHECK(max_exp_avg_sqs.size() == n_tensors);
  } else {
    SMITH_CHECK(max_exp_avg_sqs.empty());
  }
  SMITH_CHECK(state_steps.size() == n_tensors);
