...
class SMITH_API OpaqueOptionalTensorRef {
...
struct SMITH_API OperandInfo {
  ...
  // 1) Functional call(like smith.add(self, other)): output tensor is
  //    undefined, and blacksmith creates a new tensor by using common shape
  //    and computed stride in TensorIterator;
  // 2) Inplace call(like smith.add_(self, other)): output tensor is same
  //    with input tensor, and can't to modify tensor's size and stride;
  // 3) Op call with output(like smith.add(self, other, out = output)):
...
struct SMITH_API TensorIteratorBase : public impl::MetaBase {
 ...
 public:
  ...
  // Helper functions for advanced stride manipulations (e.g. smith.flip)
...
#define SMITH_DISALLOW_TEMPORARIES_IMPL(methodname, maybestatic)            \
...
#define SMITH_DISALLOW_TEMPORARIES(methodname) \
  SMITH_DISALLOW_TEMPORARIES_IMPL(methodname, )
  ...
  void build_borrowing_binary_float_op(
      ...
      const TensorBase& a,
      const TensorBase& b);
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_binary_float_op)
  ...
  void build_borrowing_binary_op(
      ...
      const TensorBase& a,
      const TensorBase& b);
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_binary_op)
  void build_unary_float_op(const TensorBase& out, const TensorBase& a);
  void build_borrowing_unary_float_op(
      const TensorBase& out,
      const TensorBase& a);
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_unary_float_op)
  ...
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_unary_op)
  void build_borrowing_unary_force_boolean_op(
      const TensorBase& out,
      const TensorBase& a);
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_unary_force_boolean_op)
  ...
  void build_borrowing_comparison_op(
      ...
      const TensorBase& a,
      const TensorBase& b);
  SMITH_DISALLOW_TEMPORARIES(build_borrowing_comparison_op)
...
#undef SMITH_DISALLOW_TEMPORARIES
 protected:
...
struct SMITH_API TensorIterator final : public TensorIteratorBase {
...
#define SMITH_DISALLOW_TEMPORARIES(methodname) \
  SMITH_DISALLOW_TEMPORARIES_IMPL(methodname, static)
  ...
  static TensorIterator borrowing_binary_op(
      ...
      const TensorBase& a,
      const TensorBase& b);
  SMITH_DISALLOW_TEMPORARIES(borrowing_binary_op)
  ...
  static TensorIterator reduce_op(
      ...
      const TensorBase& a);
#undef SMITH_DISALLOW_TEMPORARIES
#undef SMITH_DISALLOW_TEMPORARIES_IMPL
...
class SMITH_API TensorIteratorConfig final {
 public:
...
struct SMITH_API SplitUntil32Bit {
  // NOLINTNEXTLINE(cppcoreguidelines-special-member-functions)
  struct SMITH_API iterator {
    iterator() = default;
