...
//
// >>> vmap(lambda t: smith.rand(1))(smith.zeros(5))
...
template <typename... Args> static Tensor unsupportedRandomOp(Args... args) {
  SMITH_CHECK(false, "vmap: We do not yet support calling random operations inside of vmap. ",
...
SMITH_LIBRARY_IMPL(_, VmapMode, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, VmapMode, m) {
