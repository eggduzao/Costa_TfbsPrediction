...
SMITH_API const char* typeKindToString(TypeKind kind);
...
#define SMITH_DECLARE_SINGLETON(Type) \
  struct Type;                                                          \
  namespace detail { \
  template <> struct IsSingletonType<Type> : public std::integral_constant<bool, true> {}; \
  }

SMITH_DECLARE_SINGLETON(AnyType)
SMITH_DECLARE_SINGLETON(AnyEnumType)
SMITH_DECLARE_SINGLETON(NumberType)
SMITH_DECLARE_SINGLETON(FloatType)
SMITH_DECLARE_SINGLETON(ComplexType)
SMITH_DECLARE_SINGLETON(IntType)
SMITH_DECLARE_SINGLETON(BoolType)
SMITH_DECLARE_SINGLETON(StringType)
SMITH_DECLARE_SINGLETON(StorageType)
SMITH_DECLARE_SINGLETON(NoneType)
SMITH_DECLARE_SINGLETON(GeneratorType)
SMITH_DECLARE_SINGLETON(QuantizerType)
SMITH_DECLARE_SINGLETON(QSchemeType)
SMITH_DECLARE_SINGLETON(DeviceObjType)
SMITH_DECLARE_SINGLETON(StreamObjType)
SMITH_DECLARE_SINGLETON(CapsuleType)
SMITH_DECLARE_SINGLETON(PyObjectType)
SMITH_DECLARE_SINGLETON(ScalarTypeType)
SMITH_DECLARE_SINGLETON(LayoutType)
SMITH_DECLARE_SINGLETON(MemoryFormatType)
SMITH_DECLARE_SINGLETON(AnyListType)
SMITH_DECLARE_SINGLETON(AnyTupleType)
SMITH_DECLARE_SINGLETON(AnyClassType)
...
struct SMITH_API Type {
  friend SMITH_API bool operator==(const Type& lhs, const Type& rhs);
  private:
  TypeKind kind_;
 ...
 public:
  ...
  class SingletonOrSharedTypePtr {
   public:
    ...
    /* implicit */ SingletonOrSharedTypePtr(T* p)
        : repr_(makeSingletonSharedPtr(p)) {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(dynamic_cast<typename detail::as_shared_type<U>::type>(p) == nullptr);
    }
  ...
  typename detail::CastReturnType<T>::type cast() {
    if (T::Kind == kind()) {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(this == T::get().get());
...
struct SMITH_API SharedType : public Type, public std::enable_shared_from_this<SharedType> {
  using Type::Type;
};
...
inline TypePtr Type::withContained(std::vector<TypePtr> contained_types) {
  ...
  SMITH_INTERNAL_ASSERT(!current_contained.empty() && current_contained.size() == contained_types.size());
...
struct SMITH_API NamedType : public SharedType {
