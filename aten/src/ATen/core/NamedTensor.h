...
// non-wildcard
struct SMITH_API NamedTensorMeta final : public c10::NamedTensorMetaInterface {
  ...
  void set_names(HAS_NON_WILDCARD /*unused*/, DimnameList new_names) {
    SMITH_INTERNAL_ASSERT(new_names.size() == names_.size());
...
struct SMITH_API NoNamesGuard {
  ...
  void reset() {
    SMITH_INTERNAL_ASSERT(initialized);
...
SMITH_API const TensorBase& internal_set_names_inplace(const TensorBase& tensor, std::optional<DimnameList> names);
SMITH_API const TensorBase& internal_set_names_inplace(const TensorBase& tensor, std::vector<Dimname>&& names, bool validate_names);
...
SMITH_API void internal_set_names_inplace(TensorImpl* impl, std::optional<DimnameList> names, bool validate_names);
SMITH_API void internal_set_names_inplace(TensorImpl* impl, std::vector<Dimname>&& names, bool validate_names);
...
SMITH_API bool has_names(const TensorImpl* impl);
...
SMITH_API DimnameList get_names(const TensorImpl* impl);
...
SMITH_API std::optional<DimnameList> get_opt_names(const TensorImpl* impl);
