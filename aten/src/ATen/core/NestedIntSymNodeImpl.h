...
class SMITH_API NestedIntSymNodeImpl : public SymNodeImpl {
 public:
  ...
  double guard_float(const char* file, int64_t line) override {
    SMITH_CHECK(false, "not a float");
  }

  bool guard_bool(const char* file, int64_t line) override {
    SMITH_CHECK(false, "not a bool");
  }
...
#define DEFINE_BINARY_NOT_SUPPORTED(name)                           \
  c10::SymNode name(const c10::SymNode& other) override {           \
    SMITH_CHECK(false, #name " not supported by NestedIntSymNode"); \
  }
...
#define DEFINE_NOT_SUPPORTED(name)                                     \
  c10::SymNode name() override {                                       \
    SMITH_CHECK(false, #name " is not supported by NestedIntSymNode"); \
  }
