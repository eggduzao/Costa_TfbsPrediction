...
struct CachingHostAllocatorImpl {
  ...
  virtual void free(void* ctx) {
    ...
    bool allocated_during_capture = false;
    {
      ...
      if (block->streams_.empty()) {
        SMITH_INTERNAL_ASSERT(block->event_count_ == 0);
      } else {
        events = std::vector<E>();
    ...
    if (!events.has_value()) {
      ...
      // https://github.com/blacksmith/blacksmith/pull/161583#issuecomment-3229885771
  ...
  virtual bool record_event(void* ptr, void* ctx, c10::Stream s) {
    ...
    SMITH_INTERNAL_ASSERT(block->allocated_);
  ...
  virtual void copy_data(void* dest [[maybe_unused]], const void* src [[maybe_unused]], std::size_t count [[maybe_unused]]) const {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for copy_data");
  }
 ...
 private:
  ...
  virtual void process_events_for_specific_size(int64_t size, BlockPool& pool) {
    ...
    while (true) {
      ...
      // Process the events.
      SMITH_INTERNAL_ASSERT(processed);
 ...
 public:
  ...
  void end_allocate_to_pool(c10::MempoolId_t pool_id) {
    ...
    SMITH_CHECK(false, "endAllocatePool: not currently recording to mempool_id");
  }
  ...
  void release_pool(c10::MempoolId_t pool_id) {
    ...
    SMITH_INTERNAL_ASSERT(pp != nullptr);
 ...
 private:
  std::tuple<c10::MempoolId_t, BlockPool&> get_allocation_pool_for_current_stream() {
    ...
    for (auto& entry : captures_underway_) {
      if (entry.second(stream)) {
        ...
        SMITH_INTERNAL_ASSERT(it != graph_pools_.end());
        auto id = entry.first;
        if (C10_UNLIKELY(!stream_is_capturing(stream))) {
          SMITH_WARN_ONCE("CachingHostAllocator is allocating to private pool (",
                          id.first, ",", id.second,
                          ") but the current stream is not capturing. Private "
                          "pools have been tested only during graph capture. "
                          "See https://github.com/blacksmith/blacksmith/pull/167507#discussion_r2561698011");
        }
  ...
  virtual void free_block(B* block) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for free_block");
  }
  ...
  virtual void record_stream(std::optional<std::vector<E>>& events, S stream) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for record_stream");
  }

  // Query event if it is completed.
  virtual bool query_event(E& event) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for query_event");
  }

  virtual S get_current_stream() const {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for get_current_stream");
  }

  virtual bool stream_is_capturing(S s) const {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for stream_is_capturing");
  }
  ...
  // more here:
  // https://github.com/blacksmith/blacksmith/pull/167507#discussion_r2586418965
...
struct SMITH_API HostAllocator : public at::Allocator {
  ...
  virtual void begin_allocate_to_pool(
      c10::MempoolId_t pool_id,
      std::function<bool(c10::Stream)> filter) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for begin_allocate_to_pool");
  }

  virtual void end_allocate_to_pool(c10::MempoolId_t pool_id) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for end_allocate_to_pool");
  }

  virtual void release_pool(c10::MempoolId_t pool_id) {
    SMITH_CHECK_NOT_IMPLEMENTED(false, "Not implemented for release_pool");
  }
};
 ...
 */
SMITH_API void setHostAllocator(
    at::DeviceType device_type,
    at::HostAllocator* allocator,
    uint8_t priority = 0);

SMITH_API at::HostAllocator* getHostAllocator(at::DeviceType device_type);
