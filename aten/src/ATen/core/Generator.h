 ...
 * lazily created. A user can use the smith.Generator() api to create their own generator.
 */
...
struct SMITH_API Generator {
  Generator() = default;

  explicit Generator(c10::intrusive_ptr<c10::GeneratorImpl> gen_impl)
   : impl_(std::move(gen_impl)) {
    SMITH_CHECK(impl_.get(), "GeneratorImpl with nullptr is not supported");
  }
...
inline T * check_generator(std::optional<Generator> gen) {
  SMITH_CHECK(gen.has_value(), "Expected Generator but received nullopt");
  SMITH_CHECK(gen->defined(), "Generator with undefined implementation is not allowed");
  SMITH_CHECK(T::device_type() == gen->device().type(), "Expected a '", T::device_type(), "' device type for generator but found '", gen->device().type(), "'");
  return gen->get<T>();
}
 ...
 *
 * - The new state tensor must be a smith.ByteTensor
...
inline void check_rng_state(const c10::TensorImpl& new_state) {
  SMITH_CHECK_TYPE(
    ...
    "RNG state must be a smith.ByteTensor"
  );

  SMITH_CHECK(new_state.is_contiguous(), "RNG state must be contiguous");
}
