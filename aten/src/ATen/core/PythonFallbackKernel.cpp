...
// we call into user smith_dispatch or returning back to python after this call.

thread_local std::optional<c10::impl::LocalDispatchKeySet> tls_on_entry;

c10::impl::LocalDispatchKeySet safe_get_tls_on_entry() {
  SMITH_CHECK(tls_on_entry.has_value(), "Accessing smith dispatch state outside of '__smith_dispatch__' "
              "is not allowed.");
...
public:
  ...
  ~StashTLSOnEntryGuard() {
    SMITH_INTERNAL_ASSERT(!tls_on_entry.has_value());
    tls_on_entry = saved_;
  }
...
void pythonFallback(const c10::OperatorHandle& op, c10::DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
  SMITH_INTERNAL_ASSERT(tls_on_entry.has_value());
  ...
  if (mode_stack_len > 0) {
    RECORD_FUNCTION("PythonDispatchMode", smith::jit::last(*stack, num_arguments));
    const auto& cur_smith_dispatch_mode_state = c10::impl::SmithDispatchModeTLS::get_stack_at(mode_stack_len - 1);
    cur_smith_dispatch_mode_state->pyinterpreter()->dispatch(op, stack);
    return;
  }

  RECORD_FUNCTION("PythonSubclass", smith::jit::last(*stack, num_arguments));
  ...
  if (interpreter) {
    if (tensors_with_python_key_present) {
      ...
      // See https://github.com/blacksmith/blacksmith/issues/136565
  ...
  SMITH_INTERNAL_ASSERT(0, "Hit Python dispatch key but no arguments had PyInterpreter (no tensor args?)");
}

void pythonDispatcherFallback(const c10::OperatorHandle& op, c10::DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
  auto* state = c10::impl::PythonDispatcherTLS::get_state();
  SMITH_INTERNAL_ASSERT(state, "Hit PythonDispatcher dispatch key but PythonDispatcherTLS was not set");
  (*state)->python_dispatcher(op, dispatch_keys.remove(c10::DispatchKey::PythonDispatcher), stack);
}

void pythonTLSSnapshotFallback(const c10::OperatorHandle &op, c10::DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
...
// The main way this key is used is that we can register a mode to it from python (e.g. SmithProxyDispatchMode, for pre_dispatch tracing)
...
// Alternatively, we could have hardcoded this kernel (in C++) to directly call in SmithProxyDispatchMode.
// Doing that in C++ is a pain though, so it's done in python using the PythonDispatcher for convenience.
void preDispatchFallback(const c10::OperatorHandle& op, c10::DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
...
SMITH_LIBRARY_IMPL(_, Python, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&pythonFallback>());
}

SMITH_LIBRARY_IMPL(_, PythonDispatcher, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&pythonDispatcherFallback>());
}

SMITH_LIBRARY_IMPL(_, PythonTLSSnapshot, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&pythonTLSSnapshotFallback>());
}

SMITH_LIBRARY_IMPL(_, PreDispatch, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&preDispatchFallback>());
}
