...
void standardizeVectorForUnion(std::vector<TypePtr>* to_flatten) {
  SMITH_INTERNAL_ASSERT(to_flatten, "`standardizeVectorForUnion` was ",
                        "passed a `nullptr`");
...
UnionType::UnionType(std::vector<TypePtr> reference, TypeKind kind) : SharedType(kind) {
  SMITH_INTERNAL_ASSERT(!reference.empty(), "Cannot create an empty Union");
  ...
  if (types_.size() == 1) {
    ...
    msg << "} has the single type " << types_[0]->repr_str()
         ...
         << "type";
    SMITH_INTERNAL_ASSERT(false, msg.str());
  }
