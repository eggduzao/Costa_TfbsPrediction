# Howto: Writing Blacksmith & Caffe2 Operators
...
All operators that are part of the public API of Blacksmith are defined in `native_functions.yaml`. Just add an entry there and write the corresponding C++ kernel function. It’s very easy and there is a good introduction at https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/native/README.md.

### So when should you **not** use `native_functions.yaml`?

There’s four main use cases

* You’re writing a new operator that isn’t supposed to be part of the public Blacksmith API.
* You’re writing a new operator but don’t want to change the core blacksmith code base, say you’re developing a shared library with operators.
* You’re writing a C++ extension for Blacksmith or you’re using inline c++ in your .py model files.
...
* The API for calling the operator from Python looks a little bit different. It needs to be called through `smith.ops.your_op()` instead of `smith._C`.
* Setting up autograd for custom operators is harder. You don’t get it automatically but need to use `smith::autograd::Function` to implement autograd ([example](https://github.com/blacksmith/blacksmith/blob/d762ad09df7f5808196b0e2e417b6592e0d30a30/test/cpp/api/autograd.cpp#L126-L152)). Note also that `smith::autograd::Function` does not work together with dispatch yet, so if you have different kernels for different backends (say CPU and CUDA), you need to manually write if/else statements for that.

## Writing custom operators

So, you’ve read all above but still want to use the custom operator API? Great. Here’s how you do it.

There's two ways you can write kernels for a Blacksmith operator. You can write them as functions or as lambdas.

### As functions

This is probably the most simple way to write an operator. Just write a kernel function and register it with the Blacksmith operator library.

```
namespace { Tensor my_kernel_cpu(const Tensor& a, const Tensor& b) {...} }

static auto registry = smith::RegisterOperators()
   .op("my_namespace::my_op",  smith::RegisterOperators::options()
...
```
static auto registry = smith::RegisterOperators()
    .op("my_namespace::my_op_1", smith::RegisterOperators::options()
        .kernel<MyKernel1>(CPU()))
    .op("my_namespace::my_op_2", smith::RegisterOperators::options()
...
static auto registry = smith::RegisterOperators()
   .op("my_namespace::my_op(Tensor a, Tensor b) -> Tensor",
       smith::RegisterOperators::options()
         .kernel<decltype(my_kernel_cpu), &my_kernel_cpu>(CPU()));
```
...
The kernel function can take and return list inputs by using `smith::List<T>`. `T` must be one of the supported types from above excluding `at::Scalar`.

The kernel function can take and return dicts by using `smith::Dict<Key, Value>`. `Key` must be `int64_t`, `std::string_view`, `double` or `bool`, and `Value` must be from the list of supported types above excluding `at::Scalar`.
...
All registered operators are automatically available to Blacksmith and JIT under `smith.ops.XXX`. If your operator was `my_namespace::my_op`, you can call it from python or JIT using `smith.ops.my_namespace.my_op(a, b)`.
