...
/**
 * This header implements functionality to build Blacksmith with only a certain
 * set of operators (+ dependencies) included.
 *
 * - Build with -DSMITH_OPERATOR_WHITELIST="aten::add;aten::sub" and only these
...
#if !defined(SMITH_BUILD_FEATURE_ALLOWLIST)
  (void)name;
  return true;
#else
  return allowlist_contains(
    C10_STRINGIZE(SMITH_BUILD_FEATURE_ALLOWLIST),
    name);
#endif
...
constexpr bool op_allowlist_check(std::string_view op_name [[maybe_unused]]) {
  ...
  assert(op_name.find('(') == std::string_view::npos);
#if !defined(SMITH_OPERATOR_WHITELIST)
  // If the SMITH_OPERATOR_WHITELIST parameter is not defined,
  // all ops are to be registered
  return true;
#else
  return allowlist_contains(
    C10_STRINGIZE(SMITH_OPERATOR_WHITELIST),
...
#if defined(SMITH_FORCE_SCHEMA_REGISTRATION)
  return true;
#else
...
#if !defined(SMITH_CUSTOM_CLASS_ALLOWLIST)
  // If the SMITH_CUSTOM_CLASS_ALLOWLIST parameter is not defined,
  // all custom classes are to be registered
  return true;
#else
  return allowlist_contains(
    C10_STRINGIZE(SMITH_CUSTOM_CLASS_ALLOWLIST),
    custom_class_name);
#endif
}

// schema_allowlist_check() implicitly depends on a macro, SMITH_OPERATOR_WHITELIST.
