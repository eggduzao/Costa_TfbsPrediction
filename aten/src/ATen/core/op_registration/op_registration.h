 ...
 */
class SMITH_API RegisterOperators final {
public:
  ...
  public:
    ...
    Options&& schema(FunctionSchema&& schema) {
        SMITH_CHECK(!schemaOrName_.has_value(), "You can only specify the schema once per operator registration.");
    ...
    Options&& schema(const std::string& schemaOrName) {
      SMITH_CHECK(!schemaOrName_.has_value(), "Tried to register operator ", schemaOrName," but specified schema multiple times. You can only specify the schema once per operator registration.");
      ...
      #else
        schemaOrName_ = smith::jit::parseSchemaOrName(schemaOrName);
      #endif
    ...
    std::enable_if_t<guts::is_function_type<FuncType>::value, Options&&> kernel(DispatchKey dispatch_key) && {
      ...
      return std::move(*this).kernel(
        dispatch_key,
        KernelFunction::makeFromUnboxedFunction(SMITH_FN(kernel_func)),
    ...
    std::enable_if_t<guts::is_function_type<FuncType>::value, Options&&> kernel(DispatchKey dispatch_key, FuncType* kernel_func) && {
      static_assert(!std::is_same_v<FuncType, KernelFunction::BoxedKernelFunction>, "Tried to register a stackbased (i.e. internal) kernel function using the public kernel<...>() API. Please either use the internal kernel(...) API or also implement the kernel function as defined by the public API.");
      SMITH_INTERNAL_ASSERT(kernel_func != nullptr, "Kernel function cannot be nullptr");
    ...
    Options&& aliasAnalysis(AliasAnalysisKind aliasAnalysisKind) && {
      SMITH_CHECK(!aliasAnalysisKind_.has_value(), "You can only call aliasAnalysis() once per operator registration.");
