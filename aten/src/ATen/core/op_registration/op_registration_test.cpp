...
TEST(OperatorRegistrationTest, givenSmithLibrary_whenRegisteringWithMismatchingCppSignatures_thenFails) {
...
TEST(OperatorRegistrationTest, givenSmithLibrary_whenAccessingWithMismatchingCppSignatures_thenFails) {
  auto m = MAKE_SMITH_LIBRARY(_test);
  m.def("dummy(int a) -> ()");
  m.impl("dummy", DispatchKey::CPU, [] (int64_t) {});
  expectThrows<c10::Error>([] {
...
TEST(OperatorRegistrationTest, givenSmithLibrary_whenAccessingCatchAllWithMismatchingCppSignatures_thenFails) {
  auto m = MAKE_SMITH_LIBRARY(_test);
...
TEST(NewOperatorRegistrationTest, schema) {
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("def1(Tensor self) -> Tensor");
  m.def(smith::schema("def2(Tensor self) -> Tensor"));
  m.def(smith::schema("def3(Tensor self) -> Tensor", c10::AliasAnalysisKind::PURE_FUNCTION));
  m.def(smith::jit::parseSchema("def4(Tensor self) -> Tensor"));
...
TEST(NewOperatorRegistrationTest, whenRegisteringBackendFallbackKernelAndCatchallKernelForSameBackend_thenCallsFallbackKernel) {
  auto m1 = MAKE_SMITH_LIBRARY_IMPL(_, CPU);
...
TEST(NewOperatorRegistrationTest, dispatchWithCompositeImplicitAutogradKernel) {
  ...
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::CompositeImplicitAutograd, [&](const Tensor& x) { math_called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, dispatchWithCompositeExplicitAutogradKernel) {
  bool called = false;
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::CompositeExplicitAutograd, [&](const Tensor& x) { called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, dispatchWithCompositeExplicitAutogradAndCompositeImplicitAutogradKernel) {
  ...
  bool math_called = false;
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::CompositeExplicitAutograd, [&](const Tensor& x) { backend_called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, BackendOverridesCompositeExplicitAutogradKernel) {
  ...
  bool backend_called = false;
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::CompositeExplicitAutograd, [&](const Tensor& x) { default_called = true; return x; }));
  m.impl("fn", c10::DispatchKey::CPU, [&](const Tensor& x) { backend_called = true; return x; });
...
TEST(NewOperatorRegistrationTest, dispatch) {
  ...
  bool autograd_called = false;
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn_cpu", smith::dispatch(c10::DispatchKey::CPU, [&](const Tensor& x) { cpu_called = true; return x; }));
  m.def("fn_cuda", smith::dispatch(c10::kCUDA, [&](const Tensor& x) { cuda_called = true; return x; }));
  m.def("fn_autograd", smith::dispatch(c10::kAutograd, [&](const Tensor& x) { autograd_called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, dispatchAutogradPrecedence) {
  bool cpu_called = false;
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::CPU, [&](const Tensor& x) { cpu_called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, throwsWhenRegisterToBackendMapsToAutogradOther) {
  ...
  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::FPGA, [&](const Tensor& x) { fpga_called = true; return x; }));
  m.impl("fn", c10::DispatchKey::CompositeImplicitAutograd, [&](const Tensor& x) { math_called = true; return x; });
...
TEST(NewOperatorRegistrationTest, dispatchMultipleTensors) {
  ...
  auto m1 = MAKE_SMITH_LIBRARY_IMPL(_, AutogradPrivateUse1);
  m1.fallback(CppFunction::makeFallthrough());

  auto m = MAKE_SMITH_LIBRARY(test);
  m.def("fn", smith::dispatch(c10::DispatchKey::PrivateUse1, [&](const Tensor& x, const Tensor& y) { privateuse1_called = true; return x; }));
...
TEST(NewOperatorRegistrationTest, fallback) {
  auto m = MAKE_SMITH_LIBRARY_IMPL(_, CPU);
...
TEST(NewOperatorRegistrationTest, SmithLibraryTwiceIsError) {
  {
    auto m = MAKE_SMITH_LIBRARY(test);
    expectThrows<c10::Error>([] {
      auto m2 = MAKE_SMITH_LIBRARY(test);
    }, "Only a single SMITH_LIBRARY");
  }
...
TEST(NewOperatorRegistrationTest, testImplNoDefGetsCaught) {
  ...
  std::string error_str = "Discovered operators that have been registered through the dispatcher"
                          ...
                          " the SMITH_LIBRARY macro. Suspect operators:\n";
...
TEST(OperatorRegistrationTest, TestSymNonSymCompatibility) {
  ...
  m_cpu.impl("symint_op", c10::DispatchKey::CPU, SMITH_FN(symint_op));
...
TEST(OperatorRegistrationTest, TestSymSymCompatibility) {
  ...
  m.def("_test::symint_op(Tensor self, SymInt length) -> Tensor");
  auto m_cpu = MAKE_SMITH_LIBRARY_IMPL(_test, CPU);
  m_cpu.impl("symint_op", c10::DispatchKey::CPU, SMITH_FN(symint_op2));
...
TEST(OperatorRegistrationTest, TestSymSymRefCompatibility) {
  ...
  expectThrows<c10::Error>([&] {
    m_cpu.impl("symint_op", c10::DispatchKey::CPU, SMITH_FN(symint_op3));
