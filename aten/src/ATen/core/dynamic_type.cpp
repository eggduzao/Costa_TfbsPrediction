...
DynamicType::Arguments::Arguments(
    ...
    : Arguments(args) {
  SMITH_INTERNAL_ASSERT(names.size() == args.size());
...
DynamicType::DynamicType(const Type& other) : SharedType(DynamicType::Kind) {
  ...
  SMITH_INTERNAL_ASSERT(kind != Kind);
...
#undef CASE_TYPE
    default:
      SMITH_INTERNAL_ASSERT(false, "Unsupported dynamic type: ", other.str());
  }
...
TypePtr DynamicType::containedType(size_t i) const {
  SMITH_INTERNAL_ASSERT(tag_ != Tag::Class);
...
#undef CASE_TYPE
    default:
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(false);
...
SMITH_API TupleTypePtr ivalue::TupleTypeFactory<TupleType>::fallback(
