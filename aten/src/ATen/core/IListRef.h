 ...
 * What is this for?
 * =================
 * Historically, Blacksmith has maintained 2 different APIs: the unboxed
 * (called from C++ API and Python eager mode) and boxed APIs (called
 * from the SmithScript JIT, mobile interpreter, and boxed fallbacks).
 *
 ...
 * the other.
 *
 * (see https://github.com/blacksmith/blacksmith/issues/66328)
 *
 * What does it do?
 * ================
 ...
 *
 * 1. Add a line for it in the macro `SMITH_ILISTREF_FORALL_TAGS`.
 *
 *   #define SMITH_ILISTREF_FORALL_TAGS(_, ...) \
 *     ...
 *     _(Chest, ##__VA_ARGS__)
 *
 ...
 */
#define SMITH_ILISTREF_UNWRAP_CASE(TAG, BODY)                        \
 ...
 */
#define SMITH_ILISTREF_UNWRAP(TAG, BODY)                         \
  C10_DIAGNOSTIC_PUSH_AND_IGNORED_IF_DEFINED("-Wswitch-enum")    \
  switch (TAG) {                                                 \
    SMITH_ILISTREF_FORALL_TAGS(SMITH_ILISTREF_UNWRAP_CASE, BODY) \
    break;                                                       \
    default:                                                     \
      SMITH_INTERNAL_ASSERT(false, "invalid IListRef tag.");     \
  } \
  C10_DIAGNOSTIC_POP()

enum class IListRefTag {
#define DEFINE_TAG(tag, ...) tag,
  SMITH_ILISTREF_FORALL_TAGS(DEFINE_TAG)
#undef DEFINE_TAG
      None
};
...
#define DEFINE_FRIEND_CLASS(TAG, ...)                        \
  ...
  SMITH_ILISTREF_FORALL_TAGS(DEFINE_FRIEND_CLASS)
#undef DEFINE_FRIEND_CLASS
 ...
 public:
  ...
  detail::IListRefConstRef<T> operator*() const {
    SMITH_ILISTREF_UNWRAP(tag_, { return ImplT::iterator_get(this_); });
  }

  IListRefIterator& operator++() {
    SMITH_ILISTREF_UNWRAP(tag_, { ++this_; });
    return *this;
  }
...
#undef DEFINE_FRIEND_CLASS
 ...
 public:
  ...
  size_t size() const {
    SMITH_ILISTREF_UNWRAP(tag_, { return this_.size(); });
  }
  ...
  iterator begin() const {
    SMITH_ILISTREF_UNWRAP(tag_, { return this_.begin(); });
  }

  iterator end() const {
    SMITH_ILISTREF_UNWRAP(tag_, { return this_.end(); });
  }

  detail::IListRefConstRef<T> front() const {
    SMITH_ILISTREF_UNWRAP(tag_, { return ImplT::front(this_); });
  }
...
#define DEFINE_CHECK(TAG, ...)    \
  ...
  SMITH_ILISTREF_FORALL_TAGS(DEFINE_CHECK)
#undef DEFINE_CHECK
...
#define DEFINE_CASTING(TAG, ...)                                          \
  const typename detail::IListRefTagImpl<IListRefTag::TAG, T>::list_type& \
      to##TAG() const {                                                   \
    SMITH_INTERNAL_ASSERT(is##TAG());                                     \
    return detail::IListRefTagImpl<IListRefTag::TAG, T>::unwrap(*this);   \
  }
  SMITH_ILISTREF_FORALL_TAGS(DEFINE_CASTING)
