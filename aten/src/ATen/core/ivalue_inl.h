...
inline c10::intrusive_ptr<ivalue::PyObjectHolder> IValue::
    toPyObjectHolder() && {
  SMITH_INTERNAL_ASSERT(isPyObject(), "Expected PyObject but got ", tagKind());
...
inline c10::intrusive_ptr<ivalue::EnumHolder> IValue::toEnumHolder() && {
  SMITH_INTERNAL_ASSERT(isEnum(), "Expected Enum but got ", tagKind());
...
inline c10::complex<double> IValue::toComplexDouble() const {
  SMITH_INTERNAL_ASSERT(isComplexDouble(), "Expected ComplexDouble but got ", tagKind());
...
inline c10::intrusive_ptr<smith::CustomClassHolder> IValue::toCapsule() && {
  SMITH_INTERNAL_ASSERT(isCapsule());
  return moveToIntrusivePtr<smith::CustomClassHolder>();
}
inline c10::intrusive_ptr<smith::CustomClassHolder> IValue::toCapsule() const& {
  SMITH_INTERNAL_ASSERT(isCapsule());
  return toIntrusivePtr<smith::CustomClassHolder>();
}
...
// string
struct SMITH_API ConstantString final : c10::intrusive_ptr_target {
 private:
 ...
 public:
  ...
  SMITH_API friend std::ostream& operator<<(
      std::ostream& out,
...
struct SMITH_API TupleElements {
 private:
  size_t inlineSize_;
 ...
 public:
  ...
  [[nodiscard]] IValue& at(size_t idx) {
    if (inlineSize_) {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(inlineSize_ <= 3);
      SMITH_CHECK(idx < inlineSize_, "TupleElements: invalid index Index = ", idx, "; Length = ", inlineSize_);
      return elementsInline_[idx];
    } else {
  ...
  [[nodiscard]] const IValue& at(size_t idx) const {
    if (inlineSize_) {
      ...
      SMITH_CHECK(idx < elementsVector_.size(), "TupleElements: invalid index Index = ", idx, "; Length = ", elementsVector_.size());
      return elementsVector_.at(idx);
    }
  }
...
template <>
struct SMITH_API TupleTypeFactory<TupleType> {
...
struct C10_EXPORT ivalue::Future final : c10::intrusive_ptr_target {
 ...
 public:
  ...
  struct SMITH_API FutureError final : public std::exception {
  ...
  void markCompleted(
    ...
    // See https://github.com/blacksmith/blacksmith/issues/58239.
  ...
  std::string tryRetrieveErrorMessage() const {
    SMITH_CHECK(hasError(), "No error present on the future.");
    std::unique_lock<std::mutex> lock(mutex_);
 ...
 private:
  ...
  void setErrorInternal(
    ...
    SMITH_INTERNAL_ASSERT(!completed(), "Future is already marked completed");
    completed_ = true;
    eptr_ = std::move(eptr);
...
struct C10_EXPORT ivalue::Await final : c10::intrusive_ptr_target {
 ...
 public:
  ...
  IValue wait() {
    if (!completed_) {
      SMITH_CHECK(fn_, "Incompleted Await: fn can't be None");
      value_ = fn_();
      completed_ = true;
      args_ = {};
    }
  ...
  IValue value() {
    SMITH_CHECK(completed_, "Await must be completed");
    return value_;
  }
...
struct C10_EXPORT ivalue::Object final : c10::intrusive_ptr_target {
 public:
  ...
  const IValue& getSlot(size_t slot) const {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(slot < slots_.size());
   ...
   *
   * Note: Unlike in Python, SmithScript must make a distinction between
  ...
  std::shared_ptr<smith::jit::CompilationUnit> compilation_unit() {
    if (type_.holds_strong_ref()) {
      return type_.cu_.getStrongRefOrThrow();
    } else {
      auto weak_ptr = type_.cu_.getWeakRefOrThrow();
      return std::shared_ptr<smith::jit::CompilationUnit>(weak_ptr);
    }
  }
...
// because the py::object and refcounting logic should happen in libsmith_python
...
struct ivalue::EnumHolder : c10::intrusive_ptr_target {
 public:
  ...
  SMITH_API const std::string& qualifiedClassName() const;
...
inline ivalue::Object& IValue::toObjectRef() const {
  AT_ASSERT(isObject(), "Expected Object but got ", tagKind());
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(payload.u.as_intrusive_ptr != c10::UndefinedTensorImpl::singleton(), "Attempted to create null reference");
...
// and deprecated. Please use smith::List<T> instead.")
...
c10::intrusive_ptr<T> IValue::toCustomClass() && {
  static_assert(
      std::is_base_of_v<smith::CustomClassHolder, T> == true,
      "toCustomClass requires that template parameter T must inherit "
      "from smith::CustomClassHolder");
  auto obj = toObject();
  SMITH_CHECK(
      obj->slots().size() == 1,
...
std::tuple<Args...> generic_to(const IValue& ivalue, _fake_type<std::tuple<Args...>> /*unused*/) {
  ...
  SMITH_CHECK(vals.size() == sizeof...(Args));
...
template <
    typename T,
    std::enable_if_t<std::is_base_of_v<smith::CustomClassHolder, T>, int>>
...
inline bool IValue::isCustomClass() const {
  return smith::isCustomClass(*this);
}
...
struct IValue::TagType<c10::Type> {
  static SMITH_API c10::TypePtr get(const IValue& /*v*/);
};
