...
TensorTypePtr TensorType::create(
  ...
  if(stride_concrete_sizes.has_value()){
    ...
    SMITH_INTERNAL_ASSERT(size_concrete_sizes.has_value() && size_concrete_sizes->size() == stride_concrete_sizes->size());
    auto sprops =
    ...
    SMITH_INTERNAL_ASSERT(sizes_opt.has_value() && sizes.size());
...
TensorTypePtr TensorType::createContiguous(
  ...
  SMITH_INTERNAL_ASSERT(strides.size() == sizes.size());
  return create(
      scalar_type,
      device,
      VaryingShape<int64_t>(sizes),
