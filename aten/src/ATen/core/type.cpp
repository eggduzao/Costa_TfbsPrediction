...
TypeVerbosity type_verbosity() {
  static const auto c_verbosity = c10::utils::get_env("BLACKSMITH_JIT_TYPE_VERBOSITY");
...
MatchTypeReturn matchTypeVariables(
  ...
  SMITH_CHECK(false, "Unhandled free variable container: ", formal->repr_str());
}

// change return types like List[List[t]] into List[List[int]]
SMITH_API TypePtr tryEvalTypeVariables(const TypePtr& type, std::unordered_map<std::string, TypePtr>& type_env) {
...
TupleTypePtr TupleType::createWithSpec(const std::optional<c10::QualifiedName>& qualName,
    ...
    const std::vector<TypePtr>& field_types,
    std::vector<IValue>& field_defaults) {
  SMITH_INTERNAL_ASSERT(field_names.size() == field_types.size());
  ...
  for (size_t i = 0; i < field_names.size(); ++i) {
    if (i < min_default_idx) {
      ...
      SMITH_CHECK(field_defaults[j].tagKind() != "Tensor", "Tensors are "
...
TupleType::TupleType(
    ...
    : NamedType(TypeKind::TupleType, std::move(name)),
      ...
      has_free_variables_(std::any_of(elements_.begin(), elements_.end(), [](const TypePtr& v) {
        SMITH_CHECK(v, "Can not create tuple with None type");
...
FunctionType::FunctionType(smith::jit::Function* function)
