...
struct QualifiedName {
  ...
  explicit QualifiedName(std::vector<std::string> atoms) : atoms_(std::move(atoms)) {
    for (const auto& atom : atoms_) {
      SMITH_CHECK(!atom.empty(), "Atom cannot be empty");
      SMITH_CHECK(
  ...
  explicit QualifiedName(const QualifiedName& prefix, std::string name) {
    SMITH_INTERNAL_ASSERT(!name.empty());
    SMITH_INTERNAL_ASSERT(name.find(delimiter_) == std::string::npos);
