...
/**
 * DynamicType is designed as a low dependency type system for SmithScript. The
...
#undef DYNAMIC_TYPE_ITEM
  ...
  static SMITH_API DynamicTypePtr create(Type& ty);
  ...
  SMITH_API TypeKind dynamicKind() const;
...
#define DYNAMIC_TYPE_TAG_VALUE(NAME, _, IS_BASE_TYPE)      \
  template <>                                              \
  struct SMITH_API DynamicTypeTrait<NAME##Type> {          \
