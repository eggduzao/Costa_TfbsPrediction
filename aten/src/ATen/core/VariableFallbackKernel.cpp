...
//      described in https://github.com/blacksmith/blacksmith/issues/29548
...
#define AUTOGRAD_FALLBACK smith::CppFunction::makeFallthrough()
#else
...
void autograd_fallback(
  ...
  // Blacksmith has separate builds, some of which don't include autograd.
...
#define AUTOGRAD_FALLBACK smith::CppFunction::makeFromBoxedFunction<&autograd_fallback>()
#endif

SMITH_LIBRARY_IMPL(_, AutogradOther, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradCPU, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradXPU, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradCUDA, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradMTIA, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradMAIA, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}
...
SMITH_LIBRARY_IMPL(_, AutogradLazy, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradMPS, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradMeta, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

// see Note [ADInplaceOrView key]
SMITH_LIBRARY_IMPL(_, ADInplaceOrView, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(_, AutogradHPU, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}

SMITH_LIBRARY_IMPL(_, AutogradPrivateUse1, m) {
  m.fallback(AUTOGRAD_FALLBACK);
}
