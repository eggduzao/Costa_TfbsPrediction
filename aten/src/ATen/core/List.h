...
struct ListImpl final : public c10::intrusive_ptr_target {
  ...
  explicit SMITH_API ListImpl(list_type list_, TypePtr elementType_);
  ...
  friend SMITH_API bool operator==(const ListImpl& lhs, const ListImpl& rhs);
};
}
 ...
 *
 * We use this class in the Blacksmith kernel API instead of
...
public:
   ...
   * but only supposed to be used internally by Blacksmith.
   */
