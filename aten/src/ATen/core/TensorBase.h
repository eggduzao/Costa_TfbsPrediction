...
// See https://github.com/blacksmith/blacksmith/issues/161660
...
// solution to ensure that non-stable libsmith APIs are not used in the extension.
// The long term solution is to have a smith_stable target that excludes headers
// that are not in smith/stable or smith/headeronly.
// See test/cpp_extensions/smith_stable_test_extension/setup.py for an example
// of how this is used.
#ifdef SMITH_STABLE_ONLY
#error \
    "TensorBase.h should not be included when SMITH_STABLE_ONLY compile flag is passed"
#endif
...
SMITH_API const TensorBase& get_tensor_base(const Tensor& t);
...
//
// Tensor, being the central data structure in Blacksmith, gets used and
...
// incremental build times for all Blacksmith developers. TensorBase
...
// Usage tips
// ----------
// - You can `#define SMITH_ASSERT_NO_OPERATORS` at the top of a .cpp
...
class SMITH_API TensorBase {
 ...
 public:
  ...
  explicit TensorBase(
    ...
    SMITH_CHECK(impl_.get(), "TensorImpl with nullptr is not supported");
  }
 ...
 public:
  ...
  // Same as itemsize().  This is the Blacksmith naming.
  ...
  // https://github.com/blacksmith/blacksmith/blob/4dca9bde0552afc67b5b74f4a0696fe6055709c4/smith/storage.py#L196-L212
  ...
  TensorAccessor<T,N> accessor() const& {
    ...
    SMITH_CHECK(dim() == N, "TensorAccessor expected ", N, " dims but tensor has ", dim());
    T* ptr = nullptr;
  ...
  /// Example:
  /// @code
  /// auto a = smith::rand(10, smith::requires_grad());
  /// std::cout << a.is_leaf() << std::endl; // prints `true`
  ///
  /// auto b = smith::rand(10, smith::requires_grad()).to(smith::kCUDA);
  /// std::cout << b.is_leaf() << std::endl; // prints `false`
  /// // b was created by the operation that cast a cpu Tensor into a cuda Tensor
  ///
  /// auto c = smith::rand(10, smith::requires_grad()) + 2;
  /// std::cout << c.is_leaf() << std::endl; // prints `false`
  /// // c was created by the addition operation
  ///
  /// auto d = smith::rand(10).cuda();
  /// std::cout << d.is_leaf() << std::endl; // prints `true`
  /// // d does not require gradients and so has no operation creating it (that is tracked by the autograd engine)
  ///
  /// auto e = smith::rand(10).cuda().requires_grad_();
  /// std::cout << e.is_leaf() << std::endl; // prints `true`
  /// // e requires gradients and has no operations creating it
  ///
  /// auto f = smith::rand(10, smith::device(smith::kCUDA).requires_grad(true));
  ...
  // users who should use the API provided in smith/csrc/autograd.h
  ...
  const std::shared_ptr<smith::autograd::Node>& grad_fn() const;
  ...
  /// Example:
  /// @code
  /// auto v = smith::tensor({0., 0., 0.}, smith::requires_grad());
  /// auto h = v.register_hook([](smith::Tensor grad){ return grad * 2; }); // double the gradient
