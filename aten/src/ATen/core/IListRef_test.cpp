...
TEST(ITensorListRefTest, UnboxedIndirect_Equal) {
  ...
  // Vector construcsmitheck_elements_same(vec, vec, /* use_count= */ 1);
  // InitializerList construcsmitheck_elements_same({vec[0], vec[1], vec[2]}, std::vector<at::Tensor>{vec[0], vec[1], vec[2]}, /* use_count= */ 4);
}
