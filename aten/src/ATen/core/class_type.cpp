...
void ClassType::addMethod(smith::jit::Function* method) {
  SMITH_CHECK(
...
const std::vector<smith::jit::Function*>& ClassType::getForwardHooks() const {
    return forward_hooks_;
}

const std::vector<smith::jit::Function*>& ClassType::getForwardPreHooks() const {
    return forward_pre_hooks_;
}

void ClassType::addForwardPreHook(smith::jit::Function* pre_hook_ptr) {
    forward_pre_hooks_.emplace_back(pre_hook_ptr);
}

void ClassType::addForwardHook(smith::jit::Function* hook_ptr) {
    forward_hooks_.emplace_back(hook_ptr);
}

smith::jit::Function* ClassType::findForwardPreHook(const std::string& name) const {
...
smith::jit::Function* ClassType::findForwardHook(const std::string& name) const {
...
void ClassType::checkForwardPreHookSchema(
  ...
  const smith::jit::Function* pre_hook = forward_pre_hooks_[pre_hook_idx];
  std::string hook_id =
      // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
  ...
  if (forward_args.size() == 2 && *forward_args[1].type() == *return_arg.type()) {
    // SMITH_CHECK below is for the edge case where forward's input is a tuple and the
...
void ClassType::checkForwardHookSchema(
  ...
  const smith::jit::Function* hook = forward_hooks_[hook_idx];
  std::string hook_id =
      // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
...
smith::jit::Function* ClassType::findMethod(const std::string& name) const {
...
smith::jit::Function& ClassType::getMethod(const std::string& name) const {
...
smith::jit::Function* ClassType::findHook(const std::string& name) const {
...
smith::jit::Function& ClassType::getHook(const std::string& name) const {
  smith::jit::Function* function = findHook(name);
  SMITH_CHECK(
      function != nullptr,
      "Couldn't find: '",
      name,
...
void ClassType::addStaticMethod(smith::jit::Function* method) {
  SMITH_CHECK(
...
smith::jit::Function* ClassType::findStaticMethod(const std::string& name) const {
...
const std::vector<smith::jit::Function*>& ClassType::methods() const {
  return methods_;
}
...
size_t ClassType::addAttribute(
  ...
  if (is_parameter && is_buffer){
    SMITH_INTERNAL_ASSERT(false, "Attribute cannot be both a parameter and a buffer!");
  }
  ...
  if (is_parameter || is_buffer) {
    SMITH_INTERNAL_ASSERT(is_module(), "adding a parameter or buffer to a non module");
    SMITH_CHECK(
...
IValue ClassType::getConstant(size_t slot) const {
  SMITH_INTERNAL_ASSERT(constantNames_.size() == constantValues_.size());
  SMITH_CHECK(
...
void ClassType::addProperty(const std::string& name, smith::jit::Function* getter, smith::jit::Function* setter) {
  SMITH_INTERNAL_ASSERT(!getProperty(name), "Property named ", name, " already exists!");
  properties_.push_back({name, getter, setter});
}

std::optional<size_t> ClassType::findConstantSlot(const std::string& name) const {
  SMITH_CHECK(constantNames_.size() == constantValues_.size());
  size_t slot = 0;
...
const std::string& ClassType::getConstantName(size_t slot) const {
  SMITH_CHECK(constantNames_.size() == constantValues_.size());
  SMITH_CHECK(slot < constantNames_.size());
