...
void check_names_valid_for(size_t tensor_dim, DimnameList names) {
  ...
  SMITH_CHECK(tensor_dim == names.size(),
...
void internal_set_names_inplace(TensorImpl* impl, std::optional<DimnameList> names, bool validate_names) {
  SMITH_CHECK(impl->layout() == Layout::Strided,
      "NYI: named tensors only support strided layout");
  SMITH_CHECK(impl->device().is_cpu() || impl->device().is_cuda() || impl->device().is_xpu() || impl->device().is_privateuseone(),
