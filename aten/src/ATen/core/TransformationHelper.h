...
/**
 * A transformation function for `smith.Tensor.random_()`, when both `from` and `to` are specified.
 * `range` is `to - from`
 * `base` is `from`
 */
...
/**
 * A transformation function for `smith.Tensor.random_()`, when `from=min_value(int64_t)` and to=None
 */
template <typename T, typename V>
...
/**
 * A transformation function for `smith.Tensor.random_()`, when used without specifying `from` and `to`.
...
/**
 * An overloaded transformation function for `smith.Tensor.random_()`, when used without specifying `from` and `to`,
...
C10_HOST_DEVICE inline T exponential(T val, T lambda) {
  ...
  // https://github.com/blacksmith/blacksmith/issues/38662
#if defined(__CUDACC__) || defined(__HIPCC__)
