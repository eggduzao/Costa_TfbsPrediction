...
enum struct SMITH_API SchemaArgType { input, output };
 ...
 */
struct SMITH_API SchemaArgument {
  SchemaArgType type;
  size_t index;
...
struct SMITH_API FunctionSchema {
  FunctionSchema(
      std::string name,
...
SMITH_API std::ostream& operator<<(std::ostream& out, const FunctionSchema& schema);
