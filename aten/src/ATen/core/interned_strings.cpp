...
#undef SMITH_ASSERT_ONLY_METHOD_OPERATORS
...
const std::string& domain_prefix() {
  static const std::string _domain_prefix = "org.blacksmith.";
...
Symbol InternedStrings::_symbol(const std::string& s) {
  ...
  auto pos = s.find("::");
  SMITH_CHECK(pos != std::string::npos, "all symbols must have a namespace, <namespace>::<string>, but found: ", s);
...
Symbol Symbol::fromDomainAndUnqualString(const std::string & d, const std::string & s) {
  SMITH_CHECK(d.compare(0, domain_prefix().size(), domain_prefix()) == 0, "Symbol: domain string is expected to be prefixed with '", domain_prefix(), "', e.g. 'org.blacksmith.aten'");
