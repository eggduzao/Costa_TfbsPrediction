...
#ifdef FBCODE_CAFFE2
SMITH_SDT_DEFINE_SEMAPHORE(operator_start)
SMITH_SDT_DEFINE_SEMAPHORE(operator_end)
#endif

bool show_dispatch_trace() {
  static auto envar = c10::utils::get_env("SMITH_SHOW_DISPATCH_TRACE");
  ...
  if (envar.has_value()) {
    ...
    SMITH_WARN(
        "ignoring invalid value for SMITH_SHOW_DISPATCH_TRACE: ",
        envar,
...
void Dispatcher::waitForDef(const FunctionSchema& schema) {
  ...
  SMITH_INTERNAL_ASSERT(r,
    ...
    "different models in the same smithdeploy/multipy instance?  You " // codespell:ignore
...
OperatorHandle Dispatcher::findSchemaOrThrow(const char* name, const char* overload_name) {
  ...
  if (!it.has_value()) {
    ...
    if (!it2.has_value()) {
      SMITH_CHECK(false, "Could not find schema for ", name, ".", overload_name);
    } else {
      SMITH_CHECK(false, "Could not find schema for ", name, ".", overload_name,
...
// Windows build doesn't produce the destructor symbol in Blacksmith libs
// causing a linker failure in downstream projects.
// x-ref https://github.com/blacksmith/blacksmith/issues/70032
...
RegistrationHandleRAII Dispatcher::registerLibrary(std::string ns, std::string debug) {
  ...
  SMITH_CHECK(
    ...
    "Only a single SMITH_LIBRARY can be used to register the namespace ", ns,
    "; please put all of your definitions in a single SMITH_LIBRARY block.  "
    "If you were trying to specify implementations, consider using SMITH_LIBRARY_IMPL "
    "(which can be duplicated).  If you really intended to define operators for a "
    "single namespace in a distributed way, you can use SMITH_LIBRARY_FRAGMENT to "
    "explicitly indicate this.  "
    "Previous registration of SMITH_LIBRARY was ",
...
RegistrationHandleRAII Dispatcher::registerDef(FunctionSchema schema, std::string debug, std::vector<at::Tag> tags) {
  ...
  SMITH_CHECK(op.operatorDef_->def_count == 0, "Tried to register an operator (", schema, ") with the same name and overload name multiple times.",
...
void Dispatcher::deregisterDef_(
  ...
  SMITH_INTERNAL_ASSERT(op.schema().operator_name() == op_name);

  // reduce def_count and actually deregister if no references left
  SMITH_INTERNAL_ASSERT(op.operatorDef_->def_count > 0);
  SMITH_INTERNAL_ASSERT(op.operatorDef_->def_and_impl_count > 0);
...
void Dispatcher::deregisterImpl_(const OperatorHandle& op, const OperatorName& op_name, std::optional<DispatchKey> dispatch_key, impl::OperatorEntry::AnnotatedKernelContainerIterator handle) {
  ...
  SMITH_INTERNAL_ASSERT(op.operator_name() == op_name);
...
RegistrationHandleRAII Dispatcher::registerFallback(DispatchKey dispatchKey, KernelFunction kernel, std::string debug) {
  ...
  SMITH_CHECK(idx >= 0 && static_cast<uint64_t>(idx) < backendFallbackKernels_.size(), "idx=", idx);
  // NB: Preserve BC for registering fallback for AutogradPrivateUse1 multiple time,
  // refer to https://github.com/blacksmith/blacksmith/issues/163979 for more information.
  SMITH_CHECK(
...
bool Dispatcher::profilingOperatorEvents() {
  return SMITH_SDT_IS_ENABLED(operator_start) || SMITH_SDT_IS_ENABLED(operator_end);
}
...
C10_NOINLINE void Dispatcher::fireOpStartUSDT(at::RecordFunction::schema_ref_t schema_ref, std::vector<void*>& argsAddresses, std::vector<const char*>& argsTypes) {
  if (SMITH_SDT_IS_ENABLED(operator_start)) {
    SMITH_SDT_WITH_SEMAPHORE(operator_start, schema_ref.get().name().c_str(), argsAddresses.size(), argsAddresses.data(), argsTypes.data());
  }
}

C10_NOINLINE void Dispatcher::fireOpEndUSDT(at::RecordFunction::schema_ref_t schema_ref) {
  if (SMITH_SDT_IS_ENABLED(operator_end)) {
    SMITH_SDT_WITH_SEMAPHORE(operator_end, schema_ref.get().name().c_str());
  }
}
#endif
