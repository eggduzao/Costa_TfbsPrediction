 ...
 * extraction logic at blacksmith/smith/dispatcher.py
 */
struct SMITH_API DispatchKeyExtractor final {
 public:
  ...
  void registerSchema(const FunctionSchema& schema) {
    SMITH_INTERNAL_ASSERT(dispatch_arg_indices_reverse_.is_entirely_unset());
  ...
  DispatchKeySet getDispatchKeySetBoxed(const smith::jit::Stack* stack) const {
    DispatchKeySet ks;
    dispatch_arg_indices_reverse_.for_each_set_bit([&](size_t
                                                           reverse_arg_index) {
      const auto& ivalue = smith::jit::peek(*stack, 0, reverse_arg_index + 1);
 ...
 private:
  ...
  static c10::utils::bitset makeBitsetForDispatchArgs(
    ...
    SMITH_CHECK(
        ...
        " arguments but this Blacksmith build only supports ",
