...
void generic_mode_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  override_call_count++;
...
void generic_wrapper_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  override_call_count++;
  ...
  auto args = smith::jit::pop(*stack, num_arguments);
  for (const auto i : c10::irange(num_arguments)) {
    ...
    if (args[i].isTensor()) {
      ...
      if (impl->key_set().has(DispatchKey::TESTING_ONLY_GenericWrapper)) {
        ...
        smith::jit::push(*stack, wrapper->rep_);  // no move!
      } else {
        smith::jit::push(*stack, std::move(args[i]));
      }
    } else {
      smith::jit::push(*stack, std::move(args[i]));
    }
  ...
  // Rewrap outputs
  auto rets = smith::jit::pop(*stack, num_returns);
  for (const auto i : c10::irange(num_returns)) {
    // TODO: Handle tensor list
    if (rets[i].isTensor()) {
      smith::jit::push(*stack, at::detail::make_tensor<GenericWrapperTensorImpl>(std::move(rets[i]).toTensor()));  // yes move!
    } else {
      smith::jit::push(*stack, std::move(rets[i]));
    }
  }
}
...
TEST(BackendFallbackTest, TestBackendFallbackWithMode) {
  auto m = MAKE_SMITH_LIBRARY_IMPL(_, TESTING_ONLY_GenericMode);
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&generic_mode_fallback>());
...
TEST(BackendFallbackTest, TestBackendFallbackWithWrapper) {
  auto m = MAKE_SMITH_LIBRARY_IMPL(_, TESTING_ONLY_GenericWrapper);
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&generic_wrapper_fallback>());
...
TEST(BackendFallbackTest, TestFallthroughBackendFallback) {
  auto m = MAKE_SMITH_LIBRARY_IMPL(aten, TESTING_ONLY_GenericMode);
  m.impl("mul.Tensor", smith::CppFunction::makeFromBoxedFunction<&generic_mode_fallback>());

  auto gm = MAKE_SMITH_LIBRARY_IMPL(_, TESTING_ONLY_GenericMode);
  gm.fallback(smith::CppFunction::makeFallthrough());
