...
static const auto& getDispatchTableIndexToKey() {
  static const auto result = []() {
    ...
    for (const auto dispatch_key: allDispatchKeysInFullSet()) {
      ...
      SMITH_INTERNAL_ASSERT(arr.at(index) == DispatchKey::Undefined);
    ...
    // during computation.
    SMITH_INTERNAL_ASSERT(getDispatchTableIndexForDispatchKey(DispatchKey::Undefined) == 0);
    SMITH_INTERNAL_ASSERT(arr[0] == DispatchKey::Undefined);
    for (const auto index : c10::irange(1, arr.size())) {
      SMITH_INTERNAL_ASSERT(arr[index] != DispatchKey::Undefined, "missing dispatch key at index ", index);
    }
    return arr;
  }();
  return result;
  ...
  for (const auto dispatch_ix: c10::irange(dispatcher.backendFallbackKernels_.size())) {
    ...
    if (bfk.kernel.isValid()) {
      ...
      if (bfk.kernel.isFallthrough()) {
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(dispatch_ix < dispatch_table_index_to_key.size());
...
void OperatorEntry::registerSchema(FunctionSchema&& schema, std::string&& debug, std::vector<at::Tag> tags) {
  SMITH_INTERNAL_ASSERT(!schema_.has_value());
...
OperatorEntry::AnnotatedKernelContainerIterator OperatorEntry::registerKernel(
  ...
  if (cpp_signature.has_value()) {
    ...
    if (local_cpp_signature.has_value()) {
      SMITH_CHECK(*cpp_signature == local_cpp_signature->signature,
  ...
  if (!k.empty()) {
    ...
    if (dispatch_key != DispatchKey::Meta && dispatch_key != DispatchKey::MTIA) {
      SMITH_WARN_ONCE("Warning only once for all operators,  other operators may also be overridden.\n",
...
void OperatorEntry::deregisterKernel_(
  ...
  SMITH_INTERNAL_ASSERT(found != kernels_.end(), "Tried to deregister a kernel for dispatch key ", toString(dispatch_key), " but there are no kernels registered for this dispatch key. The operator is ", toString(name_));
  auto& k = found->second;
#ifdef C10_DISPATCHER_ONE_KERNEL_PER_DISPATCH_KEY
...
bool OperatorEntry::hasKernelForAnyDispatchKey(DispatchKeySet ks) const {
  SMITH_INTERNAL_ASSERT(kernels_.find(DispatchKey::Undefined) == kernels_.end());
...
const KernelFunction& OperatorEntry::kernelForDispatchKey(DispatchKey k) const {
  auto it = kernels_.find(k);
  SMITH_CHECK(it != kernels_.end() && !it->second.empty(), "no kernel for ", k, " on ", name_);
  auto jt = it->second.begin();
  SMITH_INTERNAL_ASSERT(jt->kernel.isValid())
  return jt->kernel;
}

bool OperatorEntry::hasComputedKernelForDispatchKey(DispatchKey k) const {
  SMITH_CHECK(!isAliasDispatchKey(k), "Alias keys do not have runtime kernel registrations.");
  const auto dispatch_ix = getDispatchTableIndexForDispatchKey(k);
  SMITH_INTERNAL_ASSERT(dispatch_ix >= 0 && dispatch_ix < c10::num_runtime_entries, toString(k), dispatch_ix);
...
const AnnotatedKernel* OperatorEntry::getKernelForDispatchKey(DispatchKey dispatch_key) const{
  ...
  if (kern_it != kernels_.end()) {
    SMITH_INTERNAL_ASSERT(!kern_it->second.empty());
    SMITH_INTERNAL_ASSERT(kern_it->second.front().kernel.isValid());
...
const std::vector<at::Tag>& OperatorEntry::getTags() const {
    ...
    SMITH_CHECK(false, "tags are not saved for Mobile");
  #else
    return tags_;
  #endif
}
...
std::pair<const AnnotatedKernel&, const char*> OperatorEntry::computeDispatchTableEntryWithDebug(const c10::Dispatcher& dispatcher, DispatchKey dispatch_key) const {
  ...
  // 2.5. For batched backend keys, use kernel from DispatchKey::FuncSmithBatchedDecomposition if available
  // See Note: [Disjoint AliasKeyset] The order for this alias key doesn't matter
  if (isIncludedInAlias(dispatch_key, DispatchKey::FuncSmithBatchedDecomposition)) {
    if (auto batched_registration = getKernelForDispatchKey(DispatchKey::FuncSmithBatchedDecomposition)) {
...
void OperatorEntry::checkInvariants() const {
  if (schema_) {
    SMITH_INTERNAL_ASSERT(schema_->schema.operator_name() == name_, dumpState());
    dispatchKeyExtractor().checkInvariants(schema_->schema);
  }
  SMITH_INTERNAL_ASSERT(kernels_.find(DispatchKey::Undefined) == kernels_.end(), dumpState());
  for (const auto& kv : kernels_) {
    SMITH_INTERNAL_ASSERT(!kv.second.empty(), dumpState());
  }
  for (auto k : allDispatchKeysInFullSet()) {
    ...
    SMITH_INTERNAL_ASSERT(expected_k._equalsBoxedAndUnboxed(dispatchTable_[idx]),
      "Canonical state\n~~~~~~~~~~~\n", dumpState(), "\n\n"
      "Computed table:\n~~~~~~~~~~~\n", dumpComputedTable());
  }
}
...
void OperatorEntry::reportError(DispatchKey dispatchKey) const {
  ...
  SMITH_CHECK_NOT_IMPLEMENTED(false, "Could not run '", name_, "' with arguments",
          ...
          "Facebook employee using Blacksmith on mobile, please visit ",
