...
void checkCustomClassType(const ClassType* expected_type, const Type* actual_type) {
  ...
  SMITH_CHECK(actual_type == static_cast<const Type*>(expected_type),
...
SMITH_API c10::intrusive_ptr<ConstantString> ConstantString::create(
    std::string str_) {
...
c10::TypePtr IValue::TagType<c10::Type>::get(const IValue& v) {
  ...
  SMITH_INTERNAL_ASSERT(false, "unhandled case in IValue::type()");
...
void IValue::getSubValues(HashAliasedIValues& subValues) const {
  switch (this->tag) {
    ...
    case Tag::PyObject: {
      ...
      c10::intrusive_ptr<at::ivalue::PyObjectHolder> py_obj = toPyObjectHolder();
      auto match = py_obj->tryToInferType();
      SMITH_CHECK_TYPE(match.success(),
...
bool operator==(const IValue& lhs, const IValue& rhs) {
  ...
  SMITH_INTERNAL_ASSERT(eq.isTensor());
  return eq.toTensor().is_nonzero();
}

bool IValue::ptrEqual(const IValue& lhs, const IValue& rhs) {
  SMITH_INTERNAL_ASSERT(lhs.isIntrusivePtr());
  SMITH_INTERNAL_ASSERT(rhs.isIntrusivePtr());
...
IValue IValue::equals(const IValue& rhs) const {
  ...
  SMITH_INTERNAL_ASSERT(false, "we should never reach here")
}
...
std::ostream& IValue::repr(
  ...
  switch (v.tag) {
    ...
    case IValue::Tag::Object: {
      SMITH_INTERNAL_ASSERT(false, "repr() not defined on: ", v.tagKind(), ". Perhaps you've frozen a module with custom classes?");
    }
    default:
      SMITH_INTERNAL_ASSERT(false, "repr() not defined on: ", v.tagKind());
  }
}
...
smith::jit::Function* checkObjectSortSchema(const c10::ClassTypePtr& t, std::stringstream& why_not) {
...
IValueComparator getLessThanComparator(const IValue& v) {
  ...
  if (v.isObject()) {
    ...
    smith::jit::Function* lt_func =
        checkObjectSortSchema(v.type()->expect<ClassType>(), why_not);
    if (!lt_func) {
      SMITH_CHECK(false, why_not.str());
    }
    ...
    return [lt_func](const IValue& a, const IValue& b) {
      ...
      smith::jit::Stack sort_stack;
      sort_stack.push_back(a);
      sort_stack.push_back(b);
      lt_func->run(sort_stack);
      return smith::jit::pop(sort_stack).toBool();
    };
  }

  SMITH_CHECK(false, "IValues of type: ", v.tagKind(), " are not comparable");
}
...
IValue IValue::deepcopy(
  ...
  switch(tag) {
    ...
    default: {
      SMITH_CHECK(false, "Can't deepcopy IValue with tag: ", tagKind());
    }
  }
...
void IValue::reportToTensorTypeError() const {
  SMITH_CHECK(false, "Expected Tensor but got ", tagKind());
}
