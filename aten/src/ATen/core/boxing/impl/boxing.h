...
using can_box = std::disjunction<
    ...
    // but smith::jit::push knows how to handle them
...
//
template <class... Args>
smith::jit::Stack boxArgs(Args... args) {
...
// smith::jit::push pushes 4 values for a TensorOptions; this needs to
// be kept in sync.
template <>
...
struct BoxedKernelWrapper<
  ...
  static Result call(
      ...
      Args... args) {
    smith::jit::Stack stack = boxArgs<Args...>(std::forward<Args>(args)...);
...
struct BoxedKernelWrapper<
  ...
  static at::Tensor& call(
      ...
      at::Tensor& outArg,
      OtherArgs... otherArgs) {
    smith::jit::Stack stack = boxArgs<at::Tensor&, OtherArgs...>(
...
struct BoxedKernelWrapper<
  ...
  static const at::Tensor& call(
      ...
      OtherArgs... otherArgs) {
    smith::jit::Stack stack = boxArgs(outArg, otherArgs...);
    boxed_kernel_func.callBoxed(opHandle, dispatchKeySet, &stack);
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(
        stack.size() == 1,
        "Boxed kernel was expected to return a single value on the stack, ",
...
struct BoxedKernelWrapper<
  ...
  static at::Tensor& call(
      ...
      FirstArg firstArg,
      RestArgs... restArgs) {
    smith::jit::Stack stack = boxArgs<FirstArg, RestArgs...>(
