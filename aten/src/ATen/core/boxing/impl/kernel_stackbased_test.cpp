...
void incrementKernel(const OperatorHandle&, Stack* stack) {
  int input = smith::jit::pop(*stack).toInt();
  smith::jit::pop(*stack); // pop the dummy tensor
  smith::jit::push(*stack, input + 1);
}
...
TEST(OperatorRegistrationTestStackBasedKernel, givenKernel_whenRegistrationRunsOutOfScope_thenCannotBeCalledAnymore) {
  {
    ...
    m_cpu.impl("my_op", DispatchKey::CPU, smith::CppFunction::makeFromBoxedFunction<incrementKernel>());
    {
      auto m_cuda = MAKE_SMITH_LIBRARY_IMPL(_test, CUDA);
      m_cuda.impl("my_op", DispatchKey::CUDA, smith::CppFunction::makeFromBoxedFunction<decrementKernel>());
...
TEST(OperatorRegistrationTestStackBasedKernel, callKernelsWithDispatchKeySetConvention_redispatchesToLowerPriorityKernels) {
  ...
  auto m_cpu = MAKE_SMITH_LIBRARY_IMPL(_, CPU);
  m_cpu.fallback(smith::CppFunction::makeFromBoxedFunction<&incrementKernel>());
  auto m_testing = MAKE_SMITH_LIBRARY_IMPL(_, TESTING_ONLY_GenericWrapper);
  m_testing.fallback(smith::CppFunction::makeFromBoxedFunction<&redispatchingKernel_with_DispatchKeySet>());
