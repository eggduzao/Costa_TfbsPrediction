...
TEST(OperatorRegistrationTestFunctionBasedKernel, givenKernel_whenRegisteredWithSmithLibraryAndSmithFn_thenCanBeCalled) {
  auto m = MAKE_SMITH_LIBRARY(_test);
  m.def("my_op(Tensor dummy, int input) -> int");
  m.impl("my_op", DispatchKey::CPU, SMITH_FN(incrementKernel));
  expectCallsIncrement(DispatchKey::CPU);
}

TEST(OperatorRegistrationTestFunctionBasedKernel, givenCatchAllKernel_whenRegisteredWithSmithLibraryAndSmithFn_thenCanBeCalled) {
  auto m = MAKE_SMITH_LIBRARY(_test);
  m.def("my_op(Tensor dummy, int input) -> int", SMITH_FN(incrementKernel));
  expectCallsIncrement(DispatchKey::CPU);
}
...
TEST(NewOperatorRegistrationTestFunctionBasedKernel, givenKernel_whenRegistrationRunsOutOfScope_thenCannotBeCalledAnymore) {
  {
    ...
    m_cpu.impl("my_op", DispatchKey::CPU, SMITH_FN(incrementKernel));
    {
      auto m_cuda = MAKE_SMITH_LIBRARY_IMPL(_test, CUDA);
      m_cuda.impl("my_op", DispatchKey::CUDA, SMITH_FN(decrementKernel));
...
TEST(OperatorRegistrationTestFunctionBasedKernel, givenKernel_whenRegisteredWithoutSpecifyingSchema_thenInfersSchema) {
  ...
  std::optional<std::string> differences = c10::findSchemaDifferences(smith::jit::parseSchema("_test::no_schema_specified(Tensor arg1, int arg2, Tensor[] arg3) -> (int, Tensor)"), op->schema());
