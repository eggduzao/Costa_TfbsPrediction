...
namespace smith {
class SMITH_API CustomClassHolder : public c10::intrusive_ptr_target {};
namespace jit {
using ::smith::CustomClassHolder;
struct Function;
...
SMITH_API bool _fastEqualsForContainer(const IValue& lhs, const IValue& rhs);

SMITH_API smith::jit::Function* checkObjectSortSchema(
...
SMITH_API IValueComparator getLessThanComparator(const IValue& v);
SMITH_API IValueComparator getGreaterThanComparator(const IValue& v);
...
// c10::intrusive_ptr<smith::CustomClassHolder> This wrapper is here to serve as
...
struct Capsule {
  c10::intrusive_ptr<smith::CustomClassHolder> obj_ptr;
  explicit Capsule(c10::intrusive_ptr<smith::CustomClassHolder> ptr)
...
/// supported by the SmithScript interpreter. IValues contain their
...
///
/// IValues are used as inputs to and outputs from the SmithScript interpreter.
...
///   smith::Tensor my_tensor = my_ivalue.toTensor();
/// \endrst
struct SMITH_API IValue final {
   ...
   *
   * Why does this return an IValue instead of a bool? Because in Blacksmith,
   ...
   */
  SMITH_API friend bool operator==(const IValue& lhs, const IValue& rhs);
  SMITH_API friend bool operator!=(const IValue& lhs, const IValue& rhs);
 ...
 public:
  ...
  at::TensorImpl* unsafeToTensorImpl() const {
    SMITH_INTERNAL_ASSERT(isTensor());
  ...
  static inline IValue make_capsule(
      intrusive_ptr<smith::CustomClassHolder> blob);
  bool isCapsule() const {
    return Tag::Capsule == tag;
  }
  c10::intrusive_ptr<smith::CustomClassHolder> toCapsule() &&;
  c10::intrusive_ptr<smith::CustomClassHolder> toCapsule() const&;

  // Custom C++ classes
  template <
      typename T,
      std::enable_if_t<std::is_base_of_v<smith::CustomClassHolder, T>, int> = 0>
  ...
  double toDouble() const {
    if (isDouble()) {
      ...
      SMITH_INTERNAL_ASSERT(0, "expected double");
    }
  }
...
#error Unexpected or undefined __BYTE_ORDER__
  ...
  int64_t toInt() const {
    if (isInt()) {
      ...
      SMITH_INTERNAL_ASSERT(0, "expected int");
    }
  }
  ...
  uint64_t toUInt() const {
    if (isUnsigned()) {
      ...
      SMITH_INTERNAL_ASSERT(0, "expected unsigned int");
    }
  }
  ...
  IValue(bool b) : tag(Tag::Bool) {
    ...
    // See https://github.com/blacksmith/blacksmith/issues/37117
    payload.u.as_int = b;
#else
    payload.u.as_bool = b;
#endif
  ...
  bool toBool() const {
    if (isBool()) {
      ...
      SMITH_INTERNAL_ASSERT(0, "expected bool");
    }
  }
  ...
  smith::jit::Module toModule() const;
  bool isModule() const;
  ...
  at::Scalar toScalar() const {
    if (isDouble())
      ...
      return toUInt();
    SMITH_CHECK(false, "IValue is not a Scalar");
  }
...
#undef DEFINE_CASE
  ...
  // SmithScript expression that can be used to recreate an IValue with the same
  ...
  // concise/pretty form
  SMITH_API friend std::ostream& operator<<(std::ostream& out, const IValue& v);
...
#define DEFINE_TAG(x) x,
    SMITH_FORALL_TAGS(DEFINE_TAG)
#undef DEFINE_TAG
  };

#define COUNT_TAG(x) 1 +
  static constexpr auto kNumTags = SMITH_FORALL_TAGS(COUNT_TAG) 0;
#undef COUNT_TAG
...
#define TRUTH_TABLE_ENTRY(tag) \
  (uint32_t(isIntrusivePtrConstexpr(Tag::tag)) << uint32_t(Tag::tag)) |
        SMITH_FORALL_TAGS(TRUTH_TABLE_ENTRY)
...
struct SMITH_API WeakIValue final {
  WeakIValue() = default;
...
struct SMITH_API StrongTypePtr {
  StrongTypePtr(std::shared_ptr<smith::jit::CompilationUnit> cu, TypePtr type);

  std::shared_ptr<smith::jit::CompilationUnit> cu_;
  TypePtr type_;
};
...
// Constant Object)
struct SMITH_API WeakTypePtr {
  WeakTypePtr(std::weak_ptr<smith::jit::CompilationUnit> cu, TypePtr type);

  std::weak_ptr<smith::jit::CompilationUnit> cu_;
  TypePtr type_;
};
...
struct WeakOrStrongCompilationUnit {
  explicit WeakOrStrongCompilationUnit(
      std::shared_ptr<smith::jit::CompilationUnit> shared_cu)
      : strong_ptr_(std::move(shared_cu)), weak_ptr_(std::nullopt) {}

  explicit WeakOrStrongCompilationUnit(
      std::weak_ptr<smith::jit::CompilationUnit> weak_cu)
      : strong_ptr_(std::nullopt), weak_ptr_(std::move(weak_cu)) {}

  std::shared_ptr<smith::jit::CompilationUnit> getStrongRefOrThrow() const {
    SMITH_INTERNAL_ASSERT(strong_ptr_.has_value());
    return *strong_ptr_;
  }

  std::weak_ptr<smith::jit::CompilationUnit> getWeakRefOrThrow() const {
    SMITH_INTERNAL_ASSERT(weak_ptr_.has_value());
    return *weak_ptr_;
  }
  ...
  std::optional<std::shared_ptr<smith::jit::CompilationUnit>> strong_ptr_;
  std::optional<std::weak_ptr<smith::jit::CompilationUnit>> weak_ptr_;
};
...
struct SMITH_API WeakOrStrongTypePtr {
