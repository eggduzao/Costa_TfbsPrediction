...
// smith/csrc/autograd/variable.h from aten/src/ATen/core/Tensor.cpp and
...
// have a split between aten and smith/csrc.  So we cannot simply just
...
// incorporating all of the extra code from libsmith would push them
// over (admarket/adreview/service:adreviewservice, see also
// https://github.com/blacksmith/blacksmith/pull/29299)  So if you want to do that,
...
// implementations (which live in smith/csrc/autograd/variable.cpp, FYI).
//
...
// see this bug https://github.com/blacksmith/blacksmith/issues/30102
...
struct SMITH_API VariableHooksInterface {
  ...
  virtual const std::shared_ptr<smith::autograd::Node>& grad_fn(
...
SMITH_API void SetVariableHooks(VariableHooksInterface* hooks);
SMITH_API VariableHooksInterface* GetVariableHooks();
SMITH_API bool HasVariableHooks();

struct SMITH_API VariableHooksRegisterer {
