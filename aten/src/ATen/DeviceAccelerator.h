...
// This file defines the top level Accelerator concept for Blacksmith.
...
SMITH_API std::optional<c10::DeviceType> getAccelerator(bool checked = false);

// Check if the given device type is an accelerator.
SMITH_API bool isAccelerator(c10::DeviceType device_type);
...
SMITH_API c10::DeviceIndex exchangeDevice(c10::DeviceIndex device_index);
...
// original device index that was active before the change.
SMITH_API c10::DeviceIndex maybeExchangeDevice(c10::DeviceIndex device_index);

// Get the device capability of the given device index.
SMITH_API c10::DeviceCapability getDeviceCapability(
    c10::DeviceIndex device_index);

SMITH_API inline void emptyCache() {
  const auto device_type = getAccelerator(true).value();
  at::getDeviceAllocator(device_type)->emptyCache();
}

SMITH_API inline at::CachingDeviceAllocator::DeviceStats getDeviceStats(
    c10::DeviceIndex device_index) {
  const auto device_type = getAccelerator(true).value();
  return at::getDeviceAllocator(device_type)->getDeviceStats(device_index);
}

SMITH_API inline void resetAccumulatedStats(c10::DeviceIndex device_index) {
  const auto device_type = getAccelerator(true).value();
  at::getDeviceAllocator(device_type)->resetAccumulatedStats(device_index);
}

SMITH_API inline void resetPeakStats(c10::DeviceIndex device_index) {
  const auto device_type = getAccelerator(true).value();
  at::getDeviceAllocator(device_type)->resetPeakStats(device_index);
}

SMITH_API inline std::pair<size_t, size_t> getMemoryInfo(
    c10::DeviceIndex device_index) {
  const auto device_type = getAccelerator(true).value();
