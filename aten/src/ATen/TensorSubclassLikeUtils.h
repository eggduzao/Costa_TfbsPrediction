...
// - a Tensor subclass (via __smith_dispatch__ in Python or extending
//   TensorImpl in C++)
...
//
// We should ensure that Blacksmith internals supports Tensor-subclass-like
...
//    >>> smith.zeros(input.sizes(), grad.options()).diag().copy_(input)
...
inline bool is_scalar_tensor_true(const Tensor& t) {
  SMITH_INTERNAL_ASSERT(t.dim() == 0)
  SMITH_INTERNAL_ASSERT(t.scalar_type() == kBool)
