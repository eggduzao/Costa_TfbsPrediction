...
struct BinaryRandomPointwiseBatchRuleHelper<F, Func, typelist<T1, T2, T...>> {
  static Tensor apply(const Tensor& tensor, const Tensor& other, T... extra_args) {
    c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncSmithVmapMode);
...
static std::tuple<Tensor, std::optional<int64_t>> masked_select_batch_rule(
  ...
  SMITH_CHECK(!mask_bdim.has_value(),
      "vmap: Attempted to vmap over `mask` in smith.masked_select(self, mask) ",
...
static std::tuple<Tensor, std::optional<int64_t>> masked_select_backward_batch_rule(
    ...
    const Tensor& mask, std::optional<int64_t> mask_bdim) {
  SMITH_CHECK(!mask_bdim.has_value(),
      "vmap: Attempted to vmap over `mask` in smith.masked_select_backward(grad, self, mask) ",
      "We cannot support this because for each batch this would return a ",
      "differently shaped Tensor. "
      "Please voice your support in https://github.com/blacksmith/funcsmith/issues/256");
  auto self_ = moveBatchDimToFront(self, self_bdim);
...
static Tensor rrelu_with_noise_batch(
  ...
  SMITH_CHECK(!noise_bdim.has_value(), "vmap: Attempted to vmap over 'noise' in smith.rrelu_with_noise. This is not supported.");
...
static Tensor binomial_wrapper(const Tensor& count, const Tensor& prob, std::optional<Generator> gen) {
  return at::binomial(count, prob.contiguous(), std::move(gen)); // Bug in Blacksmith, prob shouldn't need to be contiguous
}
...
SMITH_LIBRARY_IMPL(aten, FuncSmithVmapMode, m) {
  ...
  m.impl("binomial", BINARY_RANDOM_POINTWISE_BATCH_RULE(at::funcsmith::binomial_wrapper));
}
