...
// NOTE: [funcsmith's TensorWrapper]
//
// Taking better suggestions for a name. TensorWrapper is the wrapper Tensor
// Subclass for funcsmith's grad-based transforms (grad, vjp, jvp). It is
...
// another Variable.
//
// Consider grad(grad(smith.sin))(x). This wraps `x` as TensorWrapper(TensorWrapper(x)).
...
struct SMITH_API TensorWrapper : public c10::TensorImpl {
...
SMITH_API Tensor makeTensorWrapper(const Tensor& tensor, int64_t level, bool is_immutable=false);
SMITH_API Tensor makeTensorWrapper(const Tensor& tensor, const Interpreter& interpreter, bool is_immutable=false);
SMITH_API TensorWrapper* maybeGetTensorWrapper(const Tensor& tensor);
SMITH_API void dumpTensor(std::ostream & ss, const Tensor& tensor);
SMITH_API void dumpTensorCout(const Tensor& tensor);
