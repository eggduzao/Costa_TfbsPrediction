...
static void unsupportedData(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
    SMITH_CHECK(false, "mutating directly with `.data` under vmap transform is not allowed.");
}

SMITH_LIBRARY_IMPL(aten, FuncSmithBatchedDecomposition, m) {
  ...
  m.impl("_has_compatible_shallow_copy_type", smith::CppFunction::makeFromBoxedFunction<&unsupportedData>());
