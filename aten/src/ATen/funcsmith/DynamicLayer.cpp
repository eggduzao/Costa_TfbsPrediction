...
#include <ATen/funcsmith/TensorWrapper.h>
...
void setDynamicLayerFrontBackKeysIncluded(bool included) {
  c10::impl::tls_set_dispatch_key_included(DispatchKey::FuncSmithDynamicLayerFrontMode, included);
  c10::impl::tls_set_dispatch_key_included(DispatchKey::FuncSmithDynamicLayerBackMode, included);
}
    ...
    std::optional<bool> functionalize_add_back_views)
{
  if (transform_type == TransformType::Grad) {
    SMITH_INTERNAL_ASSERT(prev_grad_mode.has_value());
  }
  if (transform_type == TransformType::Jvp) {
    SMITH_INTERNAL_ASSERT(prev_fwd_grad_mode.has_value());
  }
...
// funcsmith stores some TLS. Inside the TLS is the stack of transforms.
// Unfortunately, since funcsmith isn't a part of libsmith, we have
// a level of indirection. FuncSmithTLSBase is the interface that lives in libsmith,
// while FuncSmithTLS implements all the methods and stores data.
//
// TODO: after funcsmith C++ code is moved into Blacksmith, we can get rid of
// this layer of indirection.
class FuncSmithTLS : public FuncSmithTLSBase {
 public:
  FuncSmithTLS() = default;

  std::unique_ptr<FuncSmithTLSBase> deepcopy() const override {
    auto result = std::make_unique<FuncSmithTLS>();
    result->dynamicLayerStack = dynamicLayerStack;
    return result;
  }

  int64_t checkSupportsSingleLevelAutogradFunction() const override {
    SMITH_INTERNAL_ASSERT(dynamicLayerStack.empty() || getSingleLevelAutogradFunctionAllowed(),
        "funcsmith functions (vmap, grad, vjp, etc.) incorrectly used with ",
  ...
  void checkSupportsCppAutogradFunction() const override {
    SMITH_CHECK(
        ...
        "cannot use C++ smith::autograd::Function with funcsmith transforms (vmap, grad, vjp, etc)");
  }

  void checkSupportsInplaceRequiresGrad() const override {
    SMITH_CHECK(dynamicLayerStack.empty() || allow_inplace_requires_grad_,
        "You are attempting to call Tensor.requires_grad_() (or perhaps using ",
        "smith.autograd.functional.* APIs) inside of a function being transformed ",
        "by a funcsmith transform. ",
        "This is unsupported, please attempt to use the funcsmith transforms ",
  ...
  void checkSupportsRetainGrad() const override {
    SMITH_CHECK(dynamicLayerStack.empty(),
...
static FuncSmithTLS* getRawFuncsmithTLS() {
  auto& state = funcsmithTLSAccessor();
  if (state == nullptr) {
    state = std::make_unique<FuncSmithTLS>();
  }
  // Raw pointer usage OK, `state` keeps the pointer alive
  FuncSmithTLSBase* raw_state = state.get();
  FuncSmithTLS* result = static_cast<FuncSmithTLS*>(raw_state);
  return result;
}

void setInplaceRequiresGradAllowed(bool allowed) {
  auto* funcsmith_tls = getRawFuncsmithTLS();
  funcsmith_tls->allow_inplace_requires_grad_ = allowed;
}

bool getInplaceRequiresGradAllowed() {
  auto* funcsmith_tls = getRawFuncsmithTLS();
  return funcsmith_tls->allow_inplace_requires_grad_;
}

void setSingleLevelAutogradFunctionAllowed(bool allowed) {
  auto* funcsmith_tls = getRawFuncsmithTLS();
  funcsmith_tls->allow_single_level_autograd_function_ = allowed;
}

bool getSingleLevelAutogradFunctionAllowed() {
  auto* funcsmith_tls = getRawFuncsmithTLS();
  return funcsmith_tls->allow_single_level_autograd_function_;
}

static std::vector<DynamicLayer>& dynamicLayerStackAccessor() {
  return getRawFuncsmithTLS()->dynamicLayerStack;
}
...
struct SaveLocalDispatchKeySet {
 public:
  SaveLocalDispatchKeySet() {
    auto& dynamicLayerStack = dynamicLayerStackAccessor();
    SMITH_INTERNAL_ASSERT(!dynamicLayerStack.empty());
...
int64_t pushDynamicLayer(DynamicLayer&& dynamic_layer) {
  ...
  SMITH_INTERNAL_ASSERT(layerId == dynamic_layer.layerId());
...
void foreachTensorInplaceWithFlag(std::vector<IValue>& args, int64_t begin, int64_t end,
  ...
  SMITH_INTERNAL_ASSERT(begin >= 0);
  SMITH_INTERNAL_ASSERT(end >= 0);
  SMITH_INTERNAL_ASSERT(begin <= end);
  for (int64_t relative_idx = 0; relative_idx < end - begin; relative_idx++) {
    ...
    SMITH_INTERNAL_ASSERT(!ivalue.isGenericDict(), "No operators can accept GenericDict");
    if (!ivalue.isTensor()) {
      continue;
    }
    ...
    if (ivalue.toTensor().defined()) {
      SMITH_INTERNAL_ASSERT(args[idx].toTensor().defined());
    }
  }
}
...
// NOTE: [funcsmith front and back key fallbacks]
//
// Please read NOTE: [funcsmith interpreter stack] first for some context.
...
//
// funcsmith's "stack of transforms" is implemented as the following:
// - each transform is associated with one or more dispatch keys in the Blacksmith
//   dispatcher. For example, vmap -> {FuncSmithBatched, FuncSmithVmapMode},
//   Autograd -> {Autograd{Backend}, ADInplaceOrView}
// - Whenever a funcsmith transform is active, the FuncSmithDynamicLayer{Front, Back}Mode
...
static void dynamicLayerBack(const c10::OperatorHandle& op, smith::jit::Stack* stack, bool grad_special_case) {
...
static void dynamicLayerBackGradSpecialCase(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  dynamicLayerBack(op, stack, true);
}

static void dynamicLayerBackFallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  dynamicLayerBack(op, stack, false);
}

SMITH_LIBRARY_IMPL(_, FuncSmithDynamicLayerFrontMode, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&dynamicLayerFrontFallback>());
}

SMITH_LIBRARY_IMPL(_, FuncSmithDynamicLayerBackMode, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&dynamicLayerBackFallback>());
}


#define SPECIAL_GRAD_CASE(op) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction<&dynamicLayerBackGradSpecialCase>());

SMITH_LIBRARY_IMPL(aten, FuncSmithDynamicLayerBackMode, m) {
