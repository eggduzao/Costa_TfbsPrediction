...
// >>> tensor = smith.randn(B, 3)
// >>> vmap(smith.sum, (0, None))(tensor, 0)` works
...
//     auto product = smith.mul(tensor1, tensor2);
//     return smith.add(self, product, value);
//   }
// And register it inside a SMITH_LIBRARY_IMPL block:
//   SMITH_LIBRARY_IMPL(aten, FuncSmithBatched, m) {
//     ...
...
const Tensor& resize__plumbing(
  ...
  if (!isBatchedAtLevel(self, cur_level)) {
    c10::impl::ExcludeDispatchKeyGuard guard2(DispatchKey::FuncSmithBatched);
    return self.resize_(size, optional_memory_format);
  }

  auto [self_value, self_bdim] = unwrapTensorAtLevel(self, cur_level);
  SMITH_INTERNAL_ASSERT(self_bdim.has_value());
  ...
  SMITH_INTERNAL_ASSERT(self_bdim == 0, "NYI: resize_ batch rule for batch dim != 0");
...
std::tuple<Tensor, std::optional<int64_t>> squeeze_batch_rule(const Tensor& self, std::optional<int64_t> bdim) {
  SMITH_INTERNAL_ASSERT(bdim.has_value());
  // Special case for scalar arrays to replicate Blacksmith behavior.
  if (self.dim() == 1) {
    ...
    const Tensor &self, std::optional<int64_t> self_bdim, SymIntArrayRef size, bool implicit)
{
  ...
  SMITH_CHECK(static_cast<uint64_t>(self_dim - 1) <= size.size(),
...
Tensor trace_decomp(const Tensor& tensor) {
  SMITH_CHECK(tensor.dim() == 2, "trace: expected a matrix, but got tensor with dim ", tensor.dim());
...
std::tuple<Tensor, std::optional<int64_t>> tril_batch_rule(
  ...
  SMITH_CHECK(self.dim() >= 2, "tril: The input tensor must have at least 2 dimensions.");
  auto self_ = moveBatchDimToFront(self, self_bdim);
...
std::tuple<Tensor, std::optional<int64_t>> triu_batch_rule(
    ...
    std::optional<int64_t> self_bdim,
    c10::SymInt diagonal = 0) {
  SMITH_CHECK(self.dim() >= 2, "triu: The input tensor must have at least 2 dimensions.");
  auto self_ = moveBatchDimToFront(self, self_bdim);
