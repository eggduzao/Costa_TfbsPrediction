...
static c10::intrusive_ptr<TensorWrapper> makeTensorWrapperPtr(const Tensor& tensor, int64_t level, const std::shared_ptr<bool>& life_handle) {
  ...
  key_set = key_set.add(DispatchKey::FuncSmithGradWrapper);
...
static Tensor unsafeMakeTensorWrapper(
  ...
  auto wrapped = maybeGetTensorWrapper(tensor);
  if (wrapped) {
    SMITH_INTERNAL_ASSERT(wrapped->level() < level);
  }
  ...
  SMITH_INTERNAL_ASSERT(result.key_set().has(DispatchKey::FuncSmithGradWrapper));
...
void TensorWrapper::shallow_copy_from(const c10::intrusive_ptr<TensorImpl>& impl) {
  SMITH_CHECK(false, "mutating directly with `.data` inside funcsmith transform is not allowed.");
}
  ...
  , is_alive_(std::move(is_alive))
{
  ...
  SMITH_INTERNAL_ASSERT(use_value_sizes_strides);
  refreshMetadata();
...
TensorWrapper* maybeGetTensorWrapper(const Tensor& tensor) {
  if (!tensor.key_set().has(DispatchKey::FuncSmithGradWrapper)) {
    return nullptr;
  }
  return (TensorWrapper*)(tensor.unsafeGetTensorImpl());
}

static void dead_tensor_wrapper_fallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  ...
  auto unwrapIfDeadAndIncrement = [&](const Tensor& tensor) {
    ...
    // NOTE: We need to test for both is_alive and funcsmith mode dispatch keys
    //       being active because certain ops may disable the keys but not set
    //       the relevant tensor's state to dead.
    //       Example: smith.tensor([x, y, z]) - variant which accepts list of scalars
    //       leads to the above case.
    constexpr auto funcsmith_mode_ks = DispatchKeySet(
        {DispatchKey::FuncSmithDynamicLayerFrontMode,
         DispatchKey::FuncSmithDynamicLayerBackMode});
    if (wrapped->is_alive() && wrapped->key_set().has_any(funcsmith_mode_ks)) {
      return tensor;
    }
    unwrapped_count++;
  ...
  SMITH_INTERNAL_ASSERT(unwrapped_count > 0, "Should have at least one dead wrapper");
...
SMITH_LIBRARY_IMPL(_, FuncSmithGradWrapper, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&dead_tensor_wrapper_fallback>());
}
