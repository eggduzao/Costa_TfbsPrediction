...
SMITH_API Tensor makeBatched(Tensor tensor, std::optional<int64_t> bdim, int64_t level);
...
SMITH_API std::tuple<Tensor, std::optional<int64_t>> unwrapTensorAtLevel(const Tensor& tensor, int64_t level);

// Creates a vector of BatchedTensor
SMITH_API std::vector<Tensor> makeBatchedVector(std::vector<Tensor> tensors, std::optional<int64_t> bdim, int64_t level);

// Returns True if ANY tensor in tensors is batched at level
SMITH_API bool isBatchedAtLevel(ITensorListRef tensors, int64_t level);
SMITH_API bool isBatchedAtLevel(const c10::List<std::optional<Tensor>>& maybe_tensors, int64_t level);
SMITH_API bool isBatchedAtLevel(const Tensor& tensor, int64_t level);
SMITH_API bool isBatchedAtLevel(const std::optional<Tensor>& maybe_tensor, int64_t level);

// Convenience helper. Returns true if any tensor is batched at level
SMITH_API bool areAnyBatchedAtLevel(ArrayRef<std::optional<Tensor>> maybe_tensors, int64_t level);
