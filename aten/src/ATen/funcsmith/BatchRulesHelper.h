...
#include <ATen/funcsmith/BatchingMetaprogramming.h>
#include <ATen/funcsmith/LegacyVmapTransforms.h>
...
SMITH_API Tensor reshape_dim_into(int64_t src, int64_t dst, const Tensor& x);
SMITH_API Tensor reshape_dim_outof(int64_t src, int64_t size1, const Tensor& x);

SMITH_API Tensor reshape_dim_outof_symint(int64_t src, const c10::SymInt& size1, const Tensor& x);
...
void boxed_tensor_inputs_batch_rule(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  const auto& schema = op.schema();
  const auto num_returns = schema.returns().size();
  const auto num_arguments = schema.arguments().size();

  c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncSmithBatched);
  auto maybe_layer = maybeCurrentDynamicLayer();
  vmap_check_escaped(maybe_layer, "boxed_tensor_inputs_batch_rule");

  int64_t cur_level = maybe_layer->layerId();

  auto orig_arguments = smith::jit::last(*stack, num_arguments);
  if (std::none_of(orig_arguments.begin(), orig_arguments.end(), ivalueParticipatesInCurrentLevel)) {
    op.callBoxed(stack);
    return;
  }

  auto arguments = smith::jit::pop(*stack, num_arguments);
  ...
  size_t tensor_idx = 0;
  SMITH_INTERNAL_ASSERT(!tensor_pos.empty());
  for (const auto arg_idx : c10::irange(0, num_arguments)) {
    if (tensor_idx >= tensor_pos.size() || (int64_t)arg_idx != tensor_pos[tensor_idx]) {
      smith::jit::push(stack, arguments[arg_idx]);
    } else {
      SMITH_INTERNAL_ASSERT(tensor_idx < tensor_inputs.size());
      smith::jit::push(stack, tensor_inputs[tensor_idx].first);
      tensor_idx++;
    }
  }

  op.callBoxed(stack);
  const auto returns = smith::jit::pop(*stack, num_returns);
  for (const auto& ret : returns) {
    if (ret.isTensor()) {
      smith::jit::push(stack, makeBatched(ret.toTensor(), 0, cur_level));
    } else {
      SMITH_INTERNAL_ASSERT(false, "This boxed batching rule does not currently support ops that return non-tensor values");
    }
  }
}
...
#define POINTWISE_BOXED(op) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction<boxed_tensor_inputs_batch_rule<decltype(&handle_pointwise_ops), &handle_pointwise_ops>>());

#define POINTWISE_BOXED2(op, overload) \
  m.impl(#op "." #overload, smith::CppFunction::makeFromBoxedFunction<boxed_tensor_inputs_batch_rule<decltype(&handle_pointwise_ops), &handle_pointwise_ops>>());
...
#define VARIADIC_BDIMS_BOXED(op) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction<boxed_tensor_inputs_batch_rule<decltype(&handle_variadic_bdims), &handle_variadic_bdims>>());

using UnpackedBatchedTensor = std::tuple<Tensor, std::optional<int64_t>>;

inline void find_and_unpack_tensors(
    const smith::jit::Stack* stack,
    int64_t num_args,
    int64_t cur_level,
  ...
  for (const auto idx : c10::irange(0, num_args)) {
    ...
    if (tensor_bdim.has_value()) {
      ...
      SMITH_INTERNAL_ASSERT(candidate_batch_size == computed_batch_size);
    }

    tensors->push_back(std::move(unpacked));
    tensors_pos->push_back(idx);
  }
  SMITH_INTERNAL_ASSERT(computed_batch_size > -1);
  *batch_size = computed_batch_size;
}

inline void boxed_existing_bdim_all_batch_rule(
    const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  const auto& schema = op.schema();
  const auto num_returns = schema.returns().size();
  ...
  const auto arguments = smith::jit::last(stack, num_arguments);
  ...
  for (const auto idx : c10::irange(args_begin, args_begin + num_returns)) {
    ...
    SMITH_INTERNAL_ASSERT(ret.isTensor(),
...
#define EXISTING_BDIM_ALL_BOXED(op) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction<boxed_existing_bdim_all_batch_rule>());
...
inline void boxed_all_tensors_have_optional_bdim(
  ...
  for (const auto tensor_idx : c10::irange(0, tensor_inputs.size())) {
    ...
    if (*is_no_batch_dim_case) {
      SMITH_INTERNAL_ASSERT(logical_rank == feature_rank);
    ...
    SMITH_INTERNAL_ASSERT(logical_rank == feature_rank + 1);
...
#define ALL_TENSORS_HAVE_OPTIONAL_BDIM_BOXED(feature_rank, op) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction<boxed_all_tensors_have_optional_bdim<feature_rank>>());

#define ALL_TENSORS_HAVE_OPTIONAL_BDIM_BOXED_CONTIG1(feature_rank, op, contig_tensor_index) \
  m.impl(#op, \
         smith::CppFunction::makeFromBoxedFunction<\
...
inline VmapDimVector range(int64_t start, int64_t stop) {
  SMITH_INTERNAL_ASSERT(stop >= start);
  VmapDimVector dims;
