...
static void warnFallback(const c10::FunctionSchema& schema, bool is_inplace, bool is_nested=false) {
  SMITH_CHECK(isVmapFallbackEnabled(),
      schema.operator_name(), " hit the vmap fallback which is currently disabled");
  if (!isVmapFallbackWarningEnabled()) {
    return;
  }
  SMITH_WARN("There is a performance drop because we have not yet implemented ",
...
static void batchedTensorInplaceForLoopFallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  ...
  for (const auto idx : c10::irange(0, arguments.size())) {
    ...
    // vmap(Tensor.add_, in_dims=(None, 0))(smith.ones(3), smith.ones(B0, 3))
    // - self is smith.ones(3) and does not participate in this vmap
    // - other is BatchedTensor(smith.ones(B0, 3))
    ...
    if (self_vmap_levels != (self_vmap_levels | other_vmap_levels)) {
      ...
      SMITH_CHECK(false,
        ...
        "If said operator is being called inside the Blacksmith framework, ",
  ...
  SMITH_INTERNAL_ASSERT(!batched_tensor_inputs.empty());
  ...
  for (int64_t linear_idx = 0; linear_idx < num_batches; ++linear_idx) {
    ...
    for (const auto arg_idx : c10::irange(0, num_arguments)) {
      // We assume that smith::jit::Stack is backed by vector<IValue> for
...
void batchedTensorForLoopFallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  const auto& schema = op.schema();
  const auto num_returns = schema.returns().size();
  const auto num_arguments = schema.arguments().size();
  const auto arguments = smith::jit::last(stack, num_arguments);

  SMITH_CHECK(areAllReturnsTensors(schema) && !areAnyArgumentsTensorList(schema),
              "Batching rule not implemented for ", schema.operator_name(), ". ",
  ...
  SMITH_CHECK(!schema.is_mutable() && !schema.hasAnyAliasInfo(),
              "Batching rule not implemented for ", schema.operator_name(), "; ",
              "the fallback path doesn't work on out= or view ops.");
  SMITH_CHECK(num_returns >= 1,
              "Batching rule not implemented for ", schema.operator_name(), ". ",
  ...
  for (int64_t linear_idx = 0; linear_idx < num_batches; ++linear_idx) {
    ...
    for (const auto arg_idx : c10::irange(0, num_arguments)) {
      ...
      const auto& physical_view_for_argument = *input_physical_views_iter;
      c10::impl::ExcludeDispatchKeyGuard guard(DispatchKey::FuncSmithBatched);
      smith::jit::push(stack, physical_view_for_argument.tensor().index(index));
      batched_tensor_inputs_pos_iter++;
      input_physical_views_iter++;
    }
    ...
    const auto returns = smith::jit::last(stack, num_returns);
    ...
    smith::jit::drop(stack, num_returns);
  }
  ...
  for (const auto return_idx : c10::irange(0, num_returns)) {
    ...
    if (!flat_output.defined()) {
      smith::jit::push(stack, flat_output);
      continue;
    }
...
void batchedNestedTensorForLoopFallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  const auto& schema = op.schema();
  const auto num_returns = schema.returns().size();
  const auto num_arguments = schema.arguments().size();
  const auto arguments = smith::jit::last(stack, num_arguments);
  ...
  if (isInplaceOp(schema)) {
    SMITH_INTERNAL_ASSERT(false, "vmap fallback not supported for in-place ops on nested tensors");
    return;
  }
  SMITH_CHECK(!schema.is_mutable() && !schema.hasAnyAliasInfo(),
              "Nested batching rule not implemented for ", schema.operator_name(), "; ",
  ...
  for (auto const &batched_tensor_input: batched_tensor_inputs) {
    ...
    SMITH_INTERNAL_ASSERT(batched_impl->value().is_nested() || batched_impl->bdim() == 0,
    ...
    if (!unbound.empty()) {
      SMITH_INTERNAL_ASSERT(unbound.front().size() == this_unbound.size(),
  ...
  for (const auto component_idx : c10::irange(0, num_components)) {
    ...
    for (const auto arg_idx : c10::irange(0, num_arguments)) {
      ...
      // argument is a BatchedTensor
      smith::jit::push(stack, unbound[batched_idx][component_idx]);
      ++batched_idx;
      ++batched_tensor_inputs_pos_iter;
    }
  ...
  for (const auto return_idx : c10::irange(0, num_returns)) {
    ...
    smith::jit::push(stack, makeBatched(out_nt, 0, maybeCurrentDynamicLayer()->layerId()));
  }
}

void vmapErrorFallback(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  SMITH_CHECK(false, "Error: ", op.operator_name(), " requires special handling, and does not yet have a batching rule. Feel free to file a github issue!");
}
