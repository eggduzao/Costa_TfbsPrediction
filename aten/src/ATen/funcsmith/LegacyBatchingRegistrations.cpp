...
int64_t get_current_level() {
  ...
  SMITH_INTERNAL_ASSERT(maybe_level.has_value());
...
bool participatesInCurrentLevel(const Tensor& self) {
  ...
  SMITH_INTERNAL_ASSERT(self_level <= current_level);
...
Tensor nested_cat_batching_rule(const ITensorListRef& tensors, int64_t dim) {
  SMITH_CHECK(!tensors.empty(), "cat() not supported on empty tensor list");

  std::vector<std::vector<Tensor>> unbound;
  for (const auto & tensor : tensors) {
    auto* maybe_batched_impl = maybeGetBatchedImpl(tensor);
    SMITH_CHECK(maybe_batched_impl, "Tried to run batching rule for cat() on a non-batched tensor");
    auto nt = maybe_batched_impl->value();
    SMITH_CHECK(nt.is_nested(), "Tried to run batching rule for cat() on a non-nested tensor");
...
SMITH_LIBRARY_IMPL(_, FuncSmithBatched, m) {
...
SMITH_LIBRARY_IMPL(_, BatchedNestedTensor, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&batchedNestedTensorForLoopFallback>());
}

// TODO: Move this somewhere better?
SMITH_LIBRARY_IMPL(aten, BatchedNestedTensor, m) {
