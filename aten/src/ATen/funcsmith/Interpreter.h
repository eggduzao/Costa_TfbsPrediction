...
// NOTE: [funcsmith interpreter stack]
//
// funcsmith's dispatching system uses a stack of interpreters.
...
// the batching rule.
//
// The batching rules are stored as kernels on the FuncSmithBatched key, so the way
...
// implemented?
//
// Accessing interpreters is in the hot-path of funcsmith so we have a constraint
...
struct Interpreter {
  ...
  void process(const c10::OperatorHandle& op, smith::jit::Stack* stack);
  void sendToNextInterpreter(const c10::OperatorHandle& op, smith::jit::Stack* stack, bool grad_special_case);

  void saveLocalDispatchKeySet(c10::impl::LocalDispatchKeySet keyset) {
    SMITH_INTERNAL_ASSERT(!savedLocalDispatchKeySet_.has_value());
  ...
  friend void from_json(const T& json_j, Interpreter& json_t) {
    ...
    if (meta.contains("Smith")) {
      json_t.meta_.emplace<int64_t>(meta["Smith"].template get<int64_t>());
      ...
      SMITH_CHECK(false, "unknown interpreter metadata type");
    }
  }
