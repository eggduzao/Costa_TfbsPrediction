...
static std::optional<int64_t> compute_stat_bdim(
  ...
  // It's possible that this is a bug on the Blacksmith side.
...
batch_norm_batch_rule(
  ...
  SMITH_CHECK(!training || (!input_bdim || ((!running_mean.defined() || running_mean_bdim) && (!running_var.defined() || running_var_bdim))),
      ...
      "If you are using a prebuilt module and do not need eval mode, please see the funcsmith website for resources on ",
...
std::tuple<at::Tensor, std::optional<int64_t>> batch_norm_backward_no_weight_bias_batch_rule(
  ...
  if (!grad_out_bdim.has_value() && !input_bdim.has_value() && !running_mean_bdim.has_value() && !running_var_bdim.has_value()) {
    ...
    SMITH_INTERNAL_ASSERT(!mean_bdim);
    SMITH_INTERNAL_ASSERT(!rstd_bdim);
  ...
  input_ = reshape_dim_into(0, /*channels dim*/1, input_);
  SMITH_INTERNAL_ASSERT(mean_.dim() == 2);
  SMITH_INTERNAL_ASSERT(rstd_.dim() == 2);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> batch_norm_backward_plumbing(
  ...
  SMITH_INTERNAL_ASSERT(save_mean_opt.has_value());
  SMITH_INTERNAL_ASSERT(save_rstd_opt.has_value());
  const Tensor& save_mean = *save_mean_opt;
  const Tensor& save_rstd = *save_rstd_opt;
  SMITH_INTERNAL_ASSERT(save_mean.defined());
  SMITH_INTERNAL_ASSERT(save_rstd.defined());
  ...
  SMITH_INTERNAL_ASSERT(grad_out_value.dim() > 1);  // batch_norm can't operate on 1D tensors so the output will be at least 2D
  if (output_mask[2]) {
...
static std::tuple<Tensor,Tensor,Tensor> native_group_norm_backward_plumbing(
  ...
  SMITH_INTERNAL_ASSERT(grad_out.dim() > 1);  // group_norm can't operate on 1D tensors so the output will be at least 2D
  if (output_mask[2]) {
    grad_bias = grad_out.transpose(0, 1).sum(range(1, grad_out.dim()));
  }
...
static C10_ALWAYS_INLINE void check_same_shape(
  ...
  SMITH_CHECK(has_same_shape(tensor, tensor_bdim, normalized_shape),
...
static std::tuple<at::Tensor,at::Tensor,at::Tensor> cudnn_batch_norm_backward_wrapper(
    ...
    double eps,
    std::array<bool,3> output_mask) {
    SMITH_INTERNAL_ASSERT(!training);
...
static std::tuple<at::Tensor,at::Tensor,at::Tensor> miopen_batch_norm_backward_wrapper(
    ...
    double eps,
    std::array<bool,3> output_mask) {
    SMITH_INTERNAL_ASSERT(!training); // this should be ensured by batch_norm_impl
...
SMITH_LIBRARY_IMPL(aten, FuncSmithBatched, m) {
  ...
  m.impl("cudnn_batch_norm_backward", CUDNN_BATCH_NORM_BACKWARD_BATCH_RULE(at::funcsmith::cudnn_batch_norm_backward_wrapper));
  m.impl("miopen_batch_norm_backward", MIOPEN_BATCH_NORM_BACKWARD_BATCH_RULE(at::funcsmith::miopen_batch_norm_backward_wrapper));
