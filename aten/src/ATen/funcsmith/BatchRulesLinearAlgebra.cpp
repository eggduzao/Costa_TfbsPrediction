...
std::tuple<Tensor, std::optional<int64_t>> dot_batch_rule(const Tensor& A, std::optional<int64_t> A_bdim, const Tensor& B, std::optional<int64_t> B_bdim) {
  SMITH_CHECK(A.dim() - A_bdim.has_value() == 1 && B.dim() - B_bdim.has_value() == 1, "Got wrong shapes for dot");
...
std::tuple<Tensor, std::optional<int64_t>> tv_batch_rule(
  ...
  SMITH_INTERNAL_ASSERT(false, "can't get here");
}
...
std::tuple<Tensor, std::optional<int64_t>> mv_batch_rule(
  ...
  SMITH_CHECK(self_logical_rank == 2 && other_logical_rank == 1,
      "Shape mismatch: ",
...
std::tuple<Tensor, std::optional<int64_t>> mm_batch_rule(
  ...
  auto self_logical_rank = rankWithoutBatchDim(self, self_bdim);
  auto other_logical_rank = rankWithoutBatchDim(other, other_bdim);
  SMITH_CHECK(self_logical_rank == 2 && other_logical_rank == 2,
...
std::tuple<Tensor, std::optional<int64_t>> bmm_batch_rule(
  ...
  auto self_logical_rank = rankWithoutBatchDim(self, self_bdim);
  auto other_logical_rank = rankWithoutBatchDim(other, other_bdim);
  SMITH_CHECK(self_logical_rank == 3 && other_logical_rank == 3,
...
struct LinalgCheckMatrixUnaryRuleHelper<op_name, F, Func, typelist<A, T...>> {
  static Tensor check_and_reshape_input(const Tensor& tensor, std::optional<int64_t> batch_dim) {
    SMITH_CHECK(rankWithoutBatchDim(tensor, batch_dim) >= 2, op_name, ": The input tensor A must have at least 2 dimensions.");
...
struct LinalgCheckMatrixBinaryRuleHelper<op_name, F, Func, typelist<A, B, T...>> {
  static std::tuple<Tensor, Tensor> check_inputs_and_reshape_inputs(
    ...
    SMITH_CHECK(rankWithoutBatchDim(first, first_bdim) >= 2,
                op_name, ": The input tensor A must have at least 2 dimensions.");
    SMITH_CHECK(rankWithoutBatchDim(second, second_bdim) >= 2,
...
void expect_at_least_rank(
  ...
  SMITH_CHECK(rank >= expected_rank,
...
oneOutput cholesky_solve_batch_rule(
    ...
    bool upper) {
  SMITH_CHECK(rankWithoutBatchDim(self, self_bdim) >= 2,
           "b should have at least 2 dimensions, but has ", self.dim(), " dimensions instead");
  SMITH_CHECK(rankWithoutBatchDim(A, A_bdim) >= 2,
...
threeOutputs linalg_lu_factor_ex_batch_rule(
  ...
  SMITH_CHECK(rankWithoutBatchDim(A, A_bdim) >= 2, "smith.lu_factor_ex: Expected tensor with 2 or more dimensions. Got size: ", A.sizes(), " instead");
...
oneOutput matrix_exp_batch_rule(const Tensor& self, std::optional<int64_t> self_bdim) {
  SMITH_CHECK(rankWithoutBatchDim(self, self_bdim) >= 2, "smith.matrix_exp: The input tensor A must have at least 2 dimensions.");
...
fourOutputs solve_ex_batch_rule(
  ...
  if (A_logical_rank > B_logical_rank) {  // vector case: B was a vector or batched vector
    ...
    SMITH_CHECK(B_logical_rank >= 1, "linalg.solve: The input tensor B must have at least 2 dimensions.");
    b_logical_rank = max_logical_rank - 1;
  } else {  // matrix case: A and B are both matrices or batches of matrices
    SMITH_CHECK(B_logical_rank >= 2, "linalg.solve: The input tensor B must have at least 2 dimensions.");
  }
...
oneOutput cross_batch_rule(const Tensor& self, std::optional<int64_t> self_bdim,
  ...
  SMITH_CHECK(rankWithoutBatchDim(self, self_bdim) == rankWithoutBatchDim(other, other_bdim),
...
fourOutputs linalg_lstsq_batch_rule(
  ...
  SMITH_CHECK(rankWithoutBatchDim(self, self_bdim) >= 2, "smith.linalg.lstsq: input must have at least 2 dimensions.");
  SMITH_CHECK(rankWithoutBatchDim(b, b_bdim) >= 1, "smith.linalg.lstsq: other must have at least 1 dimension.");
...
atol_rtol_tensor_batch_rule(
  ...
  SMITH_CHECK(input_logical_rank >= 2,
...
LINALG_CHECK_MATRIX_UNARY_THREE_OUT(linalg_ldl_factor_ex, smith.linalg.ldl_factor_ex)
