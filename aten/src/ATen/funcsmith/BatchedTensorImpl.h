...
struct SMITH_API BatchedTensorImpl : public c10::TensorImpl {
  ...
  // Used in smithdim. smithdim uses non-lexical BatchedTensor; the way it
...
inline bool isBatchedTensor(const Tensor& tensor) {
  return tensor.unsafeGetTensorImpl()->key_set().has(DispatchKey::FuncSmithBatched) ||
...
SMITH_API Tensor makeBatched(Tensor tensor, int64_t dim, int64_t level);

// Adds a batch dim to `tensor`, returning a BatchedTensor
SMITH_API Tensor addBatchDim(Tensor tensor, int64_t dim, int64_t level);
