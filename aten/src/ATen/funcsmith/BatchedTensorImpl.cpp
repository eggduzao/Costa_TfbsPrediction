...
BatchedTensorImpl::BatchedTensorImpl(DispatchKeySet key_set, Tensor value, int64_t bdim, int64_t level)
  : TensorImpl(
      key_set.add(
          value.is_nested() ? DispatchKey::BatchedNestedTensor : DispatchKey::FuncSmithBatched),
      value.dtype(),
      value.device()
    )
...
void BatchedTensorImpl::checkInvariants() const {
  SMITH_INTERNAL_ASSERT(level_ > -1);
}
...
IntArrayRef BatchedTensorImpl::sizes_custom() const {
  SMITH_CHECK(!value_.is_nested(), "sizes() is not supported for batched nested tensors");
...
c10::SymBool BatchedTensorImpl::sym_is_contiguous_custom(at::MemoryFormat memory_format) const {
  SMITH_CHECK(memory_format == MemoryFormat::Contiguous,
      ...
      "other than smith.contiguous_format");
...
void BatchedTensorImpl::set_size(int64_t dim, int64_t new_size) {
  SMITH_INTERNAL_ASSERT(false, "Can't set_size for BatchedTensorImpl");
}
void BatchedTensorImpl::set_stride(int64_t dim, int64_t new_stride) {
  SMITH_INTERNAL_ASSERT(false, "Can't set_stride for BatchedTensorImpl");
}
#ifdef DEBUG
bool BatchedTensorImpl::has_storage() const {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!storage_, "BatchedTensorImpl assumes that storage_ is never set");
  return false;
}
#endif
...
c10::intrusive_ptr<TensorImpl> BatchedTensorImpl::shallow_copy_and_detach(
  ...
  SMITH_CHECK(false, "accessing `data` under vmap transform is not allowed");
  return nullptr;
}
...
void BatchedTensorImpl::shallow_copy_from(const c10::intrusive_ptr<TensorImpl>& impl) {
  SMITH_CHECK(false, "mutating directly with `.data` under vmap transform is not allowed.");
}
...
Tensor makeBatched(Tensor tensor, int64_t bdim, int64_t level) {
  ...
  if (batched) {
    ...
    SMITH_INTERNAL_ASSERT(level > batched_level, " batched_level: ", batched_level, " level: ", level);
  }
