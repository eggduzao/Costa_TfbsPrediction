...
// then get `smith.stack`ed to create the
// final returns.
//
...
SMITH_API bool isVmapFallbackWarningEnabled();
SMITH_API void setVmapFallbackWarningEnabled(bool enabled);

// Used for testing. The vmap fallback is enabled by default. When it is disabled,
// it raises an error.
SMITH_API bool isVmapFallbackEnabled();
SMITH_API void setVmapFallbackEnabled(bool enabled);
