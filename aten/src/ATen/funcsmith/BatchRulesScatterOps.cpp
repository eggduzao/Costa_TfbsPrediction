...
int64_t get_max_index_logical_dim(
  ...
  SMITH_INTERNAL_ASSERT(indices.size() == indices_bdims.size());
  SMITH_INTERNAL_ASSERT(!indices.empty());
...
std::tuple<Tensor, std::optional<int64_t>> index_batch_rule(
  ...
  SMITH_INTERNAL_ASSERT(self_batched || indices_batched, "Requires at least one batched to get here");
  ...
  SMITH_INTERNAL_ASSERT(self_batched && indices_batched);
  if (!advanced_indices_are_adjacent) {
    // self: Tensor[B, 5, 6, 7, 8]
    // indices: [:, Tensor[B, 2, 2], :, Tensor[2, 2]]
...
namespace {
  ...
  // https://github.com/blacksmith/blacksmith/blob/fb0e27d38a8fdab4e1c14d6378c9e41cb30fd6a3
  ...
  VmapSymDimVector get_indexed_shape(Tensor self, const smith::List<std::optional<at::Tensor>> &orig)
  {
...
Tensor index_select_decomp(const Tensor &self, int64_t dim, const Tensor &index)
{
  ...
  // Eg. t = smith.tensor(1)
  //     idx = smith.tensor([0])
  //     smith.index_select(t, 0, idx) # 0-D
  //     smith.gather(t, 0, idx) # 1-D
...
std::tuple<Tensor, std::optional<int64_t>> index_add_batch_rule_impl(
  ...
  // right now. We really want generalized index_add kernel in Blacksmith
...
SMITH_LIBRARY_IMPL(aten, FuncSmithBatched, m) {
  ...
  m.impl("as_strided_scatter", smith::CppFunction::makeFromBoxedFunction<&vmapErrorFallback>());
}
