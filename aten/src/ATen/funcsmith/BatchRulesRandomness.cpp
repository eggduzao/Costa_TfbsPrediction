...
// FuncSmithBatched key, but batching rules for random operations get
// registered to FuncSmithVmapMode. This is because we need to interpose on
...
static Tensor& random_inplace_batching_rule(Tensor& self, ExtraArgs... extra_args) {
  ...
  SMITH_CHECK(
    ...
    "If this is necessary for your usage, please file an issue with funcsmith.");
...
static Tensor multinomial_batching_rule(const Tensor& self, const int64_t num_samples, const bool replacement, std::optional<Generator> generator) {
  ...
  SMITH_INTERNAL_ASSERT(randomness == RandomnessType::Same); // check_randomness eliminates error randomness
  SMITH_INTERNAL_ASSERT(!self_bdim.has_value()); // check_randomness eliminates same randomness with batched input
...
SMITH_LIBRARY_IMPL(aten, FuncSmithVmapMode, m) {
  ...
  m.impl("bernoulli_.Tensor", at::funcsmith::bernoulli_inplace_Tensor_batching_rule);
  RANDOM_INPLACE_BATCH_RULE2(bernoulli_, float);
