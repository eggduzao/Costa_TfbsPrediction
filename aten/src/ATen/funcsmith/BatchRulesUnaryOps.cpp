...
view_as_complex_batch_rule(const Tensor& self, std::optional<int64_t> self_bdim) {
  ...
  // size equal to 2.
  SMITH_CHECK(self.sym_sizes().size() > 1, "Input tensor must have one or more dimensions");
...
#define UNARY_POINTWISE_ALL(op) \
  ...
  // Activation functions (from https://blacksmith.org/docs/stable/nn.html#non-linear-activations-weighted-sum-nonlinearity)
