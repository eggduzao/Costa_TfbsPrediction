...
static void checkForInvalidMutationOnCaptures(
  ...
  auto args = smith::jit::last(stack, op.schema().arguments().size());
...
static Tensor materializeGradWrappers(const Tensor& tensor, int64_t current_level) {
  ...
  // We need to ensure that they pass through the funcsmith stack properly.
  ...
  c10::impl::ExcludeDispatchKeyGuard guard(c10::DispatchKey::FuncSmithDynamicLayerFrontMode);
  ...
  SMITH_INTERNAL_ASSERT(level.has_value() && level <= current_level, "escaped?");
  if (level == current_level) {
    SMITH_INTERNAL_ASSERT(tensor.defined());
    return tensor;
  }
  return makeTensorWrapper(tensor, current_level, /*is_immutable=*/true);
}
...
static void autogradBasedTransformSendToNext(
  ...
  auto unwrap = [&](const Tensor& tensor) {
    ...
    SMITH_INTERNAL_ASSERT(tensor_wrapper_level <= current_level);
