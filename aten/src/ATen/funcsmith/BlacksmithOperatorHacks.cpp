...
// NOTE: [funcsmith's Blacksmith Operator Hacks]
//
// This file contains hacks for composite Blacksmith operators that are problematic.
...
// e.g., upstream the changes to Blacksmith.
//
...
Tensor trace_backward_decomp(const Tensor& grad, IntArrayRef sizes) {
  SMITH_CHECK(sizes.size() == 2, "expected matrix input");
...
// dropout hack
// TODO: make the following changes in blacksmith/blacksmith
...
Tensor make_feature_noise(const Tensor& input) {
  ...
  SMITH_CHECK(input.dim() >= 2, "Feature dropout requires at least 2 dimensions in the input");
...
Ctype<inplace> _dropout_impl(T& input, double p, bool train) {
  SMITH_CHECK(p >= 0 && p <= 1, "dropout probability has to be between 0 and 1, but got ", p);
...
SMITH_LIBRARY_IMPL(aten, FuncSmithDynamicLayerFrontMode, m) {
