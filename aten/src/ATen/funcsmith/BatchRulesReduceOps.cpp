...
// For most Blacksmith ops, this is equal to 1.
//
// NOTE: [keepdim cases]
...
>
static void boxed_reduction_batch_rule(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  ...
  if (std::none_of(orig_arguments.begin(), orig_arguments.end(), ivalueParticipatesInCurrentLevel)) {
    c10::impl::ExcludeDispatchKeyGuard guard_2(DispatchKey::FuncSmithBatched);
    op.callBoxed(stack);
    return;
  }
  ...
  SMITH_INTERNAL_ASSERT(arguments[0].isTensor());
  ...
  if (arguments[dim_arg_pos].isIntList()) {
    ...
    if (param_type->kind() == IntType::Kind) {
      ...
      SMITH_INTERNAL_ASSERT(false, "Unexpected dtype found at dims");
    }
  } else{
    SMITH_INTERNAL_ASSERT(false, "Unexpected dtype found at dims");
  }
  ...
  if (is_scalar_case) {
    ...
    // Reduction operations in Blacksmith have an edge case where they allow
    // dim=0 and dim=-1 if the tensor has shape [].
    //
    // This can come up if we do something like
    // vmap(lambda x: x.sum(0))(smith.tensor([10.])),
    //
    ...
    switch (keepdim_case) {
      ...
      case KEEPDIM_CASE_VARIABLE:
        SMITH_INTERNAL_ASSERT(maybe_keepdim_arg_pos >= 0);
  ...
  auto returns = smith::jit::pop(*stack, num_returns);
  for (auto& ret : returns) {
    if (ret.isTensor()) {
      ...
      if (is_scalar_case && maybe_keepdim.value()) {
        ...
        SMITH_INTERNAL_ASSERT(res.size(-1) == 1);
        res = res.squeeze(-1);
      }
      smith::jit::push(stack, makeBatched(std::move(res), 0, cur_level));
    } else {
...
static std::tuple<Tensor, std::optional<int64_t>> _softmax_backward_batch_rule(
  ...
  // have changed in Blacksmith (and output of softmax is probably always contiguous)
...
static Tensor bucketize_decomp_Tensor(
  ...
  SMITH_CHECK(boundaries.dim() == 1, "bucketize: boundaries tensor must be 1 dimension, but got dim(", boundaries.dim(), ")");
...
#define REDUCTION_BOXED_ARGS(op, dim_pos, keepdim_case, maybe_keepdim_pos) \
  m.impl(#op, smith::CppFunction::makeFromBoxedFunction< \
