...
static std::tuple<Tensor, std::optional<int64_t>> embedding_batch_rule(
  ...
  SMITH_INTERNAL_ASSERT(weight_bdim && indices_bdim);
  // B*, BED -> B*, (BE)D -> B*D
...
// fakeTensor/dynamic shapes and compiled funcsmith transforms.
...
static Tensor one_hot_decomposition_hack(const Tensor &self, int64_t num_classes) {
    SMITH_CHECK(self.dtype() == kLong, "one_hot is only applicable to index tensor.");

    // disallow implicit inference under vmap; this would be data-dependent
    // and is intentionally guarded by Dynamo in smith/_dynamo/variables/smith.py.
    SMITH_CHECK(num_classes > 0, "When vmap-ing smith.nn.functional.one_hot, please "
...
struct UpsampleBackwardBatchRuleHelper<F, Func, typelist<A, B, C, T...>> {
  static std::tuple<Tensor, std::optional<int64_t>> apply(
    ...
    SMITH_INTERNAL_ASSERT(!input_size.empty());
