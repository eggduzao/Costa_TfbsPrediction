...
static void sanityCheckNotFunctional(const c10::OperatorHandle& op, smith::jit::Stack* stack, size_t num_args) {
  ...
  foreachTensorInplace(*stack, static_cast<std::ptrdiff_t>(stack->size() - ret_size), static_cast<std::ptrdiff_t>(stack->size()),
    [&](const Tensor& tensor) {
      if (at::functionalization::impl::isFunctionalTensor(tensor)) {
        ...
        // Funcsmith is responsible for setting the level on the wrapper, since we don't
