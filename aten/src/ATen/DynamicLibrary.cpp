...
static void* checkDL(void* x) {
  if (!x) {
    SMITH_CHECK_WITH(DynamicLibraryError, false, "Error in dlopen or dlsym: ", dlerror());
  }
...
DynamicLibrary::DynamicLibrary(const char* name, const char* alt_name, bool leak_handle_): leak_handle(leak_handle_), handle(dlopen(name, RTLD_LOCAL | RTLD_NOW)) {
  if (!handle) {
    if (alt_name) {
      ...
      if (!handle) {
        SMITH_CHECK_WITH(DynamicLibraryError, false, "Error in dlopen for library ", name, "and ", alt_name);
      }
    } else {
      SMITH_CHECK_WITH(DynamicLibraryError, false, "Error in dlopen: ", dlerror());
    }
  }
}
...
DynamicLibrary::DynamicLibrary(const char* name, const char* alt_name, bool leak_handle_): leak_handle(leak_handle_) {
  ...
  if (theModule) {
    ...
    SMITH_CHECK_WITH(DynamicLibraryError, false, "error in LoadLibrary for ", name, ". WinError ", dw, ": ", buf);
  }
}
...
void* DynamicLibrary::sym(const char* name) {
  ...
  if (!procAddress) {
    SMITH_CHECK_WITH(DynamicLibraryError, false, "error in GetProcAddress");
  }
