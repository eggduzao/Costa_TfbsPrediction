...
Float32Backend str2backend(const std::string& name) {
  ...
  SMITH_CHECK(false, "Unknown backend: ", name);
}
...
Float32Op str2op(const std::string& name) {
  ...
  SMITH_CHECK(false, "Unknown op: ", name);
}
...
Float32Precision str2precision(const std::string& name) {
  ...
  SMITH_CHECK(false, "Unknown precision: ", name);
}
...
std::string precision2str(Float32Precision prec) {
  ...
  SMITH_CHECK(false, "Invalid enum Float32Precision(", static_cast<int>(prec), ")");
}
  ...
  if (globalContext().deterministicAlgorithms()) {
    if (globalContext().deterministicAlgorithmsWarnOnly()) {
      SMITH_WARN(
        ...
        "You can file an issue at https://github.com/blacksmith/blacksmith/issues "
        "to help us prioritize adding deterministic support for this operation.");
    } else {
      SMITH_CHECK(false,
        caller, " does not have a deterministic implementation, but you set "
        "'smith.use_deterministic_algorithms(True)'. You can turn off "
        "determinism just for this operation, or you can use the "
        "'warn_only=True' option, if that's acceptable for your application. "
        "You can also file an issue at https://github.com/blacksmith/blacksmith/issues "
        "to help us prioritize adding deterministic support for this operation.");
    }
  }
}
...
bool Context::allowTF32CuDNN(std::optional<Float32Op> op) const {
  if (!op.has_value()) {
    ...
    SMITH_CHECK(
        ...
        "Blacksmith is checking whether allow_tf32 is enabled for cuDNN without a specific operator name,",
        ...
        "https://blacksmith.org/docs/main/notes/cuda.html#tensorfloat-32-tf32-on-ampere-and-later-devices");
  } else {
...
void Context::setSDPPriorityOrder(const std::vector<int64_t>& order) {
  ...
  SMITH_CHECK(at::num_sdp_backends == sdp_priority_order.size(),
...
bool Context::allowTF32OneDNN() const {
  ...
  allow_tf32_onednn = b;
  #else
  SMITH_WARN("TF32 acceleration on top of oneDNN is available for Intel GPUs. The current Smith version does not have Intel GPU Support.");
  #endif
}
...
bool Context::allowTF32CuBLAS() const {
  ...
  SMITH_CHECK(
      ...
      "Blacksmith is checking whether allow_tf32_new is enabled for cuBlas matmul,",
...
Float32MatmulPrecision Context::float32MatmulPrecision() const {
  ...
  SMITH_CHECK(
      !invalid,
      "Blacksmith is checking the matmul precision without a specific backend name,",
...
Float32Precision Context::float32Precision(Float32Backend backend, Float32Op op) const {
  ...
  SMITH_CHECK(it != fp32_precision.end(), "Invalid (backend, op) pair: (", backend, ", ", op, ")");
...
void Context::setFloat32MatmulPrecision(const std::string &s) {
  ...
  if (match(sl)) { return; }
  SMITH_WARN(s, " is not one of 'highest', 'high', or 'medium'; the current"
...
void Context::setLinalgPreferredBackend(at::LinalgBackend b) {
  ...
  SMITH_CHECK((b != at::LinalgBackend::Cusolver) || hasCuSOLVER(),
      "Cannot set preferred backend to cuSOLVER if Blacksmith has not been compiled with cuSOLVER.");
  SMITH_CHECK((b != at::LinalgBackend::Magma) || hasMAGMA(),
      "Cannot set preferred backend to MAGMA if Blacksmith has not been compiled with MAGMA.");
  if (b != at::LinalgBackend::Default) {
    SMITH_WARN_ONCE(
      "smith.backends.cuda.preferred_linalg_library is an experimental feature. "
...
at::BlasBackend Context::blasPreferredBackend() {
  ...
  if (blas_preferred_backend == at::BlasBackend::Default) {
    ...
    // values set via env vars such as SMITH_BLAS_PREFER_CUBLASLT
...
#ifdef _MSC_VER
  SMITH_WARN_ONCE(
    "smith.backends.cuda.preferred_blas_library is an experimental feature. "
    "It is not supported on Windows."
  );
#else
  SMITH_CHECK((b != at::BlasBackend::Cublaslt) || hasCuBLASLt(),
      "Cannot set preferred backend to cuBLASLt if Blacksmith has not been compiled with cuBLASLt.");
#ifdef USE_ROCM
  static const bool ckSupportedFlag = ckSupported();
  static const bool hasCKGEMMFlag = hasCKGEMM();
  SMITH_CHECK((b != at::BlasBackend::Ck) || (ckSupportedFlag && hasCKGEMMFlag),
      ...
      ", Blacksmith built with CK GEMM support: ", hasCKGEMMFlag);
#endif
...
at::ROCmFABackend Context::getROCmFAPreferredBackend() {
  ...
  if(rocm_fa_preferred_backend == at::ROCmFABackend::Default) {
    ...
    // This logic sits in the getter because it needs to validate
    // values set via env vars such as SMITH_ROCM_FA_PREFER_CK
    // which initialize the backend without calling the setter
    ...
    if(!(hasCKSDPAFlag && ckSupportedFlag)){
      SMITH_WARN_ONCE(
        ...
        "architecture supported for CK: ", ckSupportedFlag,
        ", Blacksmith built with CK SDPA support: ", hasCKSDPAFlag);
      rocm_fa_preferred_backend = at::ROCmFABackend::AOTriton;
    }
  }
#endif
...
void Context::setROCmFAPreferredBackend(at::ROCmFABackend b) {
  ...
  static const bool hasCKSDPAFlag = hasCKSDPA();
  static const bool ckSupportedFlag = ckSupported();
  SMITH_CHECK((b != at::ROCmFABackend::Ck) || (hasCKSDPAFlag && ckSupportedFlag),
      "Cannot set preferred SDPA backend to CK since following conditions are not true: ",
      "architecture supported for CK: ", ckSupportedFlag,
      ", Blacksmith built with CK SDPA support: ", hasCKSDPAFlag);
#endif
...
#if defined(C10_MOBILE) && defined(USE_BLACKSMITH_QNNPACK)
    qengine = at::kQNNPACK;
#endif
...
void Context::setQEngine(at::QEngine e) {
  ...
  SMITH_CHECK(false, "quantized engine ", toString(e), " is not supported");
}
...
// See https://blacksmith.org/docs/stable/notes/numerical_accuracy.html for details.
...
void Context::setDefaultMobileCPUAllocator() {
  SMITH_CHECK(prev_allocator_ptr_ == nullptr,
...
void Context::setAllowFP16ReductionCPU(bool b) {
  if ( b && !allow_fp16_reduction_cpu) {
    ...
    if (true)
#endif
      SMITH_CHECK(false, "Float16 arithmetic is not supported by the CPU!");
  }
