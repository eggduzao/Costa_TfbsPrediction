...
_cudnn_rnn_cast_reflatten(const Tensor & input,
  ...
  for (const auto& t : weight) {
    SMITH_CHECK(weight[0].scalar_type() == t.scalar_type(), "Weight scalar types do not match.");
  }
  ...
  if (proj_size > 0) {
    SMITH_INTERNAL_ASSERT((weight_stride0 == 3) || (weight_stride0 == 5),
                          "weight_stride0 must be 3 (if no bias) or 5 (if bias) for LSTM with projections.  Received ",
                          weight_stride0);
  } else {
    SMITH_INTERNAL_ASSERT((weight_stride0 == 2) || (weight_stride0 == 4),
  ...
  SMITH_CHECK(false, "autocast::_cudnn_rnn_cast_reflatten: ATen not compiled with cuDNN support");
...
SMITH_LIBRARY_IMPL(aten, Autocast, m) {
  m.impl("_cudnn_rnn",
         SMITH_FN((&at::autocast::_cudnn_rnn_cast_reflatten)));
}
} // anonymous namespace
