...
// e.g. smith.sum(scalar_tensor, 0) Otherwise, dim should be in the range
...
inline int64_t legacy_cat_wrap_dim_symint(
  ...
  for (auto& sizes : tensor_sizes) {
    if (sizes.size() == 1) {
      if (SMITH_GUARD_OR_FALSE(sizes[0].sym_eq(0))) {
        continue;
      }
    }
    return maybe_wrap_dim(dim, static_cast<int64_t>(sizes.size()));
...
inline int64_t legacy_cat_wrap_dim(
  ...
  for (const Tensor& tensor : tensors) {
    if (tensor.dim() == 1) {
      if (SMITH_GUARD_OR_FALSE(tensor.sym_sizes()[0].sym_eq(0))) {
        continue;
      }
    }
    return maybe_wrap_dim(dim, tensor.dim());
