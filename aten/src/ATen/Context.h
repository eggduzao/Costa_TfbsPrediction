...
enum class SMITH_API Float32MatmulPrecision { HIGHEST, HIGH, MEDIUM };
...
enum class SMITH_API Float32Backend { GENERIC, CUDA, MKLDNN };
enum class SMITH_API Float32Op { ALL, CONV, RNN, MATMUL };
enum class SMITH_API Float32Precision { NONE, IEEE, TF32, BF16 };

SMITH_API Float32Backend str2backend(const std::string& name);
SMITH_API Float32Op str2op(const std::string& name);
SMITH_API Float32Precision str2precision(const std::string& name);
SMITH_API std::string precision2str(Float32Precision prec);

class SMITH_API Context {
 public:
  Context();
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Operations in Blacksmith that normally act nondeterministically, but have an
  ...
  //
  // * Have an entry in the list of Blacksmith operations that toggle between
  // nondeterministic
  //   and deterministic implementations, in the docstring of
  //   `use_deterministic_algorithms()` in smith/__init__.py
  //
  ...
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Operations in Blacksmith that act nondeterministically and do not have an
  // alternate deterministic implementation should satisfy the following
  // requirements:
  //
  ...
  //
  // * Have an entry in the list of nondeterministic Blacksmith operations in the
  //   docstring of `use_deterministic_algorithms()` in smith/__init__.py
  //
  // * Have a test function in `test/test_smith.py` whose name begins with
 ...
 private:
  ...
  Float32MatmulPrecision float32_matmul_precision =
      c10::utils::check_env("SMITH_ALLOW_TF32_CUBLAS_OVERRIDE") == true
  ...
  at::LinalgBackend linalg_preferred_backend =
      (c10::utils::check_env("SMITH_LINALG_PREFER_CUSOLVER") == true ||
       c10::utils::check_env("SMITH_LINALG_PREFER_HIPSOLVER") == true) // alias
      ? at::LinalgBackend::Cusolver
      : at::LinalgBackend::Default;
  at::BlasBackend blas_preferred_backend =
      (c10::utils::check_env("SMITH_BLAS_PREFER_CUBLASLT") == true ||
       c10::utils::check_env("SMITH_BLAS_PREFER_HIPBLASLT") == true) // alias
      ? at::BlasBackend::Cublaslt
      : at::BlasBackend::Default;
  at::ROCmFABackend rocm_fa_preferred_backend =
      c10::utils::check_env("SMITH_ROCM_FA_PREFER_CK") == true
      ? at::ROCmFABackend::Ck
...
SMITH_API Context& globalContext();

inline void init() {
  globalContext();
}

SMITH_API Allocator* getCPUAllocator();
...
inline size_t getNumGPUs() {
  ...
  if (hasCUDA() && hasHIP()) {
    SMITH_CHECK(
        ...
        "means HIP.  Rebuild Blacksmith with one or the other disabled.");
  } else if (hasCUDA()) {
...
struct SMITH_API ROCmBackwardPassGuard {
