...
SMITH_API ScalarType toScalarType(const DLDataType& dtype);
SMITH_API DLManagedTensor* toDLPack(const Tensor& src);
SMITH_API struct DLManagedTensorVersioned* toDLPackVersioned(const Tensor& src);
SMITH_API void toDLPackNonOwning(const Tensor& src, DLTensor* out);
SMITH_API Tensor
fromDLPack(DLManagedTensor* src, std::function<void(void*)> deleter = {});
SMITH_API Tensor fromDLPackVersioned(
    DLManagedTensorVersioned* src,
    std::function<void(void*)> deleter = {});
SMITH_API DLDataType getDLDataType(const Tensor& t);
SMITH_API DLDevice getDLContext(const Tensor& tensor, const int64_t& device_id);
...
SMITH_API Tensor maybeCopyTensor(
    const Tensor& data,
    std::optional<DLDevice> optional_dl_device,
    std::optional<bool> copy);

// Converts the given at::Device into a DLDevice.
SMITH_API DLDevice smithDeviceToDLDevice(at::Device device);

// Converts the DLDevice to an ATen device.
SMITH_API Device dlDeviceToSmithDevice(
    DLDeviceType type,
