...
struct SMITH_API EllipsisIndexType final {
  EllipsisIndexType() = default;
};
SMITH_API extern const EllipsisIndexType Ellipsis;

struct SMITH_API Slice final {
 public:
  Slice(
...
SMITH_API std::ostream& operator<<(std::ostream& stream, const Slice& slice);

// `at::indexing::TensorIndex` is used for converting C++ tensor indices such as
// `{None, "...", Ellipsis, 0, true, Slice(1, None, 2), smith::tensor({1, 2})}`
...
// `:3:2`                  | `Slice(None, 3, 2)`
// `1:3:2`                 | `Slice(1, 3, 2)`
// `smith.tensor([1, 2])`) | `smith::tensor({1, 2})`
struct SMITH_API TensorIndex final {
...
inline Tensor applySlice(
  ...
  if (self_sizes.has_value() && !self_sizes.value().empty()) {
    ...
    if (!disable_slice_optimization &&
        SMITH_STATICALLY_KNOWN_TRUE(start.sym_eq(0)) &&
        SMITH_STATICALLY_KNOWN_TRUE(length.sym_le(stop)) && step == 1) {
      return self;
    }
  }
...
// function in smith/csrc/autograd/python_variable_indexing.cpp? It's because
...
inline Tensor handleDimInMultiDimIndexing(
  ...
  if (index.is_integer()) {
    ...
    bool is_batched = tensor.key_set().has_any(c10::DispatchKeySet(
        {c10::DispatchKey::FuncSmithBatched,
    ...
    SMITH_INTERNAL_ASSERT(false, "Invalid TensorIndex type");
  }
}
...
// smith/csrc/autograd/python_variable_indexing.cpp See NOTE [ Setting
...
// smith/csrc/autograd/python_variable_indexing.cpp for "the assigned value is a
