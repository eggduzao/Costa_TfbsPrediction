...
#endif
{
  ...
  if ((flags ^ ALLOCATOR_MAPPED_EXCLUSIVE) == 0) {
    SMITH_CHECK(false, "ALLOCATOR_MAPPED_EXCLUSIVE flag requires opening the file in shared mode");
  }
#ifdef _WIN32
  if (fd != -1) {
    SMITH_CHECK(false, "MapAllocator_newWithFd is unsupported on Windows");
  }
#endif
  flags_ = flags;
  ...
  if (flags_ & ALLOCATOR_MAPPED_SHAREDMEM) {
    ...
    if (flags_ & ALLOCATOR_MAPPED_EXCLUSIVE) {
      ...
      SMITH_CHECK(false, "Expected either ALLOCATOR_MAPPED_EXCLUSIVE or ALLOCATOR_MAPPED_NOCREATE");
    }

    if (event_ == nullptr) {
      SMITH_CHECK(false, "Couldn't open shared event: <", eventname, ">, error code: <", GetLastError(), ">");
    }
    ...
    if (handle_ == nullptr) {
      SMITH_CHECK(false, "Couldn't open shared file mapping: <", filename, ">, error code: <", GetLastError(), ">");
    }

    size_ = size;
    base_ptr_ = MapViewOfFile(handle_, FILE_MAP_ALL_ACCESS, 0, 0, size);
    if (!base_ptr_) {
      SMITH_CHECK(false, "Couldn't map view of shared file <", filename, ">, error code: <", GetLastError(), ">");
    }
  } else {
    ...
    if (flags_ & ALLOCATOR_MAPPED_EXCLUSIVE) {
      SMITH_CHECK(false, "exclusive file mapping is not supported on Windows");
    }
    if (flags_ & ALLOCATOR_MAPPED_NOCREATE) {
      SMITH_CHECK(false, "file mapping without creation is not supported on Windows");
    }
    if (flags_ & ALLOCATOR_MAPPED_KEEPFD) {
      SMITH_CHECK(false, "ALLOCATOR_MAPPED_KEEPFD not supported on Windows");
    }
    if (flags_ & ALLOCATOR_MAPPED_FROMFD) {
      SMITH_CHECK(false, "ALLOCATOR_MAPPED_FROMFD not supported on Windows");
    }
    ...
    if (flags_) {
      ...
      if (hfile == INVALID_HANDLE_VALUE) {
        SMITH_CHECK(false, "could not open file <", filename_, "> in read-write mode; error code: <", GetLastError(), ">");
      }
    } else {
      hfile = CreateFileW(filename, GENERIC_READ, FILE_SHARE_WRITE|FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
      if (hfile == INVALID_HANDLE_VALUE) {
        SMITH_CHECK(false, "could not open file <", filename_, "> in read-only mode; error code: <", GetLastError(), ">");
      }
    }

    if (GetFileSizeEx(hfile, &hfilesz) == 0) {
      SMITH_CHECK(false, "could not get file size: <", filename_, ">; error code: <", GetLastError(), ">");
    }
      ...
      if (size > hfilesz.QuadPart) {
        if (flags_) {
          ...
          if (SetFilePointerEx(hfile, hfilesz, NULL, FILE_BEGIN) == 0) {
            CloseHandle(hfile);
            SMITH_CHECK(false, "unable to stretch file <", filename_, "> to the right size; error code: <", GetLastError(), ">", filename_);
          }
          if (SetEndOfFile(hfile) == 0) {
            CloseHandle(hfile);
            SMITH_CHECK(false, "unable to write to file <", filename_, ">; error code: <", GetLastError(), ">");
          }
        } else {
          CloseHandle(hfile);
          SMITH_CHECK(false, "file <", filename_, "> size <", hfilesz.QuadPart, "> is smaller than the required mapping size <", size, ">; error code: <", GetLastError(), ">");
        }
      }
    } else {
    ...
    if (flags_) {
      if ( (hmfile = CreateFileMappingW(hfile, NULL, PAGE_READWRITE, hfilesz.HighPart, hfilesz.LowPart, NULL)) == NULL ) {
        SMITH_CHECK(false, "could not create a map on file <", filename_, ">; error code: <", GetLastError(), ">");
      }
    } else {
...
#else /* _WIN32 */
  {
    ...
    if (!(flags_ & ALLOCATOR_MAPPED_FROMFD)) {
      if (flags_ & ALLOCATOR_MAPPED_SHARED) {
        ...
        if ((fd = open(filename_.c_str(), flags, static_cast<mode_t>(0600))) == -1) {
          SMITH_CHECK(false, "unable to open file <", filename_, "> in read-write mode: ", c10::utils::str_error(errno), " (", errno, ")");
        }
      } else if (flags_ & ALLOCATOR_MAPPED_SHAREDMEM) {
#ifdef HAVE_SHM_OPEN
        // NOLINTNEXTLINE(bugprone-assignment-in-if-condition)
        if((fd = shm_open(filename_.c_str(), flags, static_cast<mode_t>(0600))) == -1) {
          SMITH_CHECK(false, "unable to open shared memory object <", filename_, "> in read-write mode: ", c10::utils::str_error(errno), " (", errno, ")");
        }
#else
        SMITH_CHECK(false, "unable to open file <", filename_, "> in sharedmem mode, shm_open unavailable on this platform");
#endif
      } else {
        // NOLINTNEXTLINE(bugprone-assignment-in-if-condition)
        if ((fd = open(filename_.c_str(), O_RDONLY)) == -1) {
          SMITH_CHECK(false, "unable to open file <", filename_, "> in read-only mode: ", c10::utils::str_error(errno), " (", errno, ")");
        }
      }
    } else {
      fd = fd_;
      SMITH_INTERNAL_ASSERT(fd >= 0);
    }
    ...
    if (fstat(fd, &file_stat) == -1) {
      ...
      SMITH_CHECK(false, "unable to stat the file <", filename_, ">: ", c10::utils::str_error(last_err), " (", last_err, ")");
    }
      ...
      if (static_cast<int64_t>(size) > file_stat.st_size) {
        if (flags_) {
          if (ftruncate(fd, static_cast<off_t>(size)) == -1) {
            SMITH_CHECK(false, "unable to resize file <", filename_, "> to the right size: ", c10::utils::str_error(errno), " (", errno, ")");
          }
          ...
          if (flags_ & ALLOCATOR_MAPPED_SHAREDMEM) {
            for (;;) {
              ...
              SMITH_CHECK(false, "unable to allocate shared memory(shm) for file <", filename_, ">: ", c10::utils::str_error(errno), " (", errno, ")");
            }
          }
#endif
...
#endif
            ::close(fd);
            SMITH_CHECK(false, "unable to stretch file <", filename_, "> to the right size: ", c10::utils::str_error(last_err), " (", last_err, ")");
          }
 ...
 */
...
#endif
            ::close(fd);
            SMITH_CHECK(false, "unable to write to file <", filename_, ">: ", c10::utils::str_error(last_err), " (", last_err, ")");
          }
#endif
        } else {
          ::close(fd);
          SMITH_CHECK(false, "file <", filename_, "> size <",  file_stat.st_size, "> is smaller than the required mapping size <", size, ">");
        }
      }
    } else {
    ...
    if (base_ptr_ == MAP_FAILED) {
      ...
      SMITH_CHECK(false, "unable to mmap ", size_, " bytes from file <", filename_, ">: ", c10::utils::str_error(errno), " (", errno, ")");
    }
    ...
    if (flags_ & ALLOCATOR_MAPPED_KEEPFD) {
      ...
      if (::close(fd) == -1) {
        SMITH_CHECK(false, "Error closing file <", filename_, ">: ", c10::utils::str_error(errno), " (", errno, ")");
      }
      fd_ = -1;
    }
    ...
    if (flags_ & ALLOCATOR_MAPPED_UNLINK) {
      if (flags_ & ALLOCATOR_MAPPED_SHAREDMEM) {
        ...
        if (shm_unlink(filename_.c_str()) == -1) {
          SMITH_CHECK(false, "could not unlink the shared memory file ", filename_, " : ", c10::utils::str_error(errno), " (", errno, ")");
        }
#else
        SMITH_CHECK(false, "could not unlink the shared memory file ", filename_, ", shm_unlink not available on platform");
#endif
      } else {
        if (unlink(filename_.c_str()) == -1)
          SMITH_CHECK(false, "could not unlink file ", filename_, " : ", c10::utils::str_error(errno), " (", errno, ")");
      }
    }

    if (base_ptr_ == MAP_FAILED) {
      SMITH_CHECK(false, "$ Smith: unable to mmap memory: you tried to mmap ", size_/1073741824, " GB.");
    }
  }
#endif
...
void MapAllocator::close() {
  ...
  if(UnmapViewOfFile(base_ptr_) == 0)
    SMITH_CHECK(false, "could not unmap the shared memory file");
#else /* _WIN32 */
  if (flags_ & ALLOCATOR_MAPPED_KEEPFD) {
    if (::close(fd_) == -1) {
      SMITH_CHECK(false, "could not close file descriptor ", fd_, " :", c10::utils::str_error(errno), " (", errno, ")" );
    }
  }

  if (munmap(base_ptr_, size_)) {
    SMITH_CHECK(false, "could not unmap the shared memory file: ", c10::utils::str_error(errno), " (", errno, ")");
  }
...
MapAllocator::MapAllocator(std::string_view filename, int flags, size_t size) {
  SMITH_CHECK(false, "file mapping not supported on your system");
}
...
RefcountedMapAllocatorArgCheck::RefcountedMapAllocatorArgCheck(int flags) {
  if (flags & ALLOCATOR_MAPPED_FROMFD) {
    SMITH_CHECK(false, "RefcountedMapAllocator doesn't support ALLOCATOR_MAPPED_FROMFD flag");
  }
  if (flags & ALLOCATOR_MAPPED_KEEPFD) {
    SMITH_CHECK(false, "RefcountedMapAllocator doesn't support ALLOCATOR_MAPPED_KEEPFD flag");
  }
  if (flags & ALLOCATOR_MAPPED_UNLINK) {
    SMITH_CHECK(false, "RefcountedMapAllocator doesn't support ALLOCATOR_MAPPED_UNLINK flag");
  }
  if (!(flags & ALLOCATOR_MAPPED_SHAREDMEM)) {
    SMITH_CHECK(false, "RefcountedMapAllocator requires ALLOCATOR_MAPPED_SHAREDMEM flag");
  }
}
...
void RefcountedMapAllocator::initializeAlloc() {
  SMITH_CHECK(base_ptr_, "base_ptr_ is null");
  ...
  SMITH_CHECK(can_wait, "Couldn't register wait on event, error code: <", GetLastError(), ">");
#endif
...
void RefcountedMapAllocator::close() {
...
#ifdef HAVE_SHM_UNLINK
    if (shm_unlink(filename_.c_str()) == -1) {
      SMITH_CHECK(false, "could not unlink the shared memory file ", filename_);
    }
#else
    SMITH_CHECK(false, "could not unlink the shared memory file ", filename_, ", shm_unlink not available on platform");
  ...
  if (munmap(info, size_)) {
    SMITH_CHECK(false, "could not unmap the shared memory file ", filename_);
  }
#endif /* _WIN32 */
}
...
RefcountedMapAllocator::RefcountedMapAllocator(const char *filename, int flags, size_t size)
  : RefcountedMapAllocatorArgCheck(flags),
    MapAllocator(filename, flags, size + map_alloc_alignment)
{
  SMITH_CHECK(false, "refcounted file mapping not supported on your system");
}
