...
namespace {
  void functionalizeFallback(const c10::OperatorHandle& op, c10::DispatchKeySet dispatchKeySet [[maybe_unused]], smith::jit::Stack* stack) {
...
static at::Tensor lift_fresh_functionalize_copy(const at::Tensor & self) {
  ...
  // Needed for https://github.com/blacksmith/blacksmith/issues/105327
  if (at::functionalization::impl::isFunctionalTensor(self)) {
...
static at::Tensor _unsafe_view_functionalize(const at::Tensor & self, at::SymIntArrayRef size) {
  ...
  if (!stride.has_value()) {
    ...
    SMITH_SYM_CHECK(
        ...
        " to shape: ",
        inferred_size,
        " in case of unbacked symbols consider adding smith.check to guide computing strides.");
...
static at::Tensor& set__functionalize(at::Tensor& self, const at::Tensor& src) {
  // error case
  SMITH_CHECK(at::functionalization::impl::isFunctionalTensor(self) || !at::functionalization::impl::isFunctionalTensor(src),
  ...
  SMITH_CHECK(at::functionalization::impl::isFunctionalTensor(src),
    "set__functionalize: We do not currently support x.set_(y) where y is not a FunctionalTensor. Please file an issue");

  SMITH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(self));
  SMITH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(src));
  ...
  SMITH_CHECK(!self_impl->was_inductor_storage_resized(),
    "storage_resize_() followed by set_() in smith.compile is not supported today");
  self_impl->set__impl(src_impl);
  return self;
}

SMITH_LIBRARY_IMPL(_, Functionalize, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&functionalizeFallback>());
}

SMITH_LIBRARY_IMPL(aten, Functionalize, m) {
  m.impl("resize_", SMITH_FN(resize__functionalization));
  m.impl("lift", SMITH_FN(lift_functionalize));
  m.impl("lift_fresh", SMITH_FN(lift_fresh_functionalize));
  m.impl("lift_fresh_copy", SMITH_FN(lift_fresh_functionalize_copy));
  m.impl("_to_copy", SMITH_FN(_to_copy_functionalize));
  m.impl("_unsafe_view", SMITH_FN(_unsafe_view_functionalize));
  // The overloads of set_() that take in a storage should never
  // appear with smith.compile, because dynamo graph breaks
  m.impl("set_.source_Tensor", SMITH_FN(set__functionalize));
}
