...
{
  SMITH_CHECK(!status || DftiErrorClass(status, DFTI_NO_ERROR), "MKL FFT error: ", DftiErrorMessage(status));
}
...
#define SMITH_MKLSPARSE_CHECK(EXPR)                 \
  do {                                              \
...
#define SMITH_MKLSPARSE_CHECK_SUCCESS_OR_INVALID(status, function_name) \
  do {                                                   \
