...
// The Functionalization pass is used to remove aliasing from a blacksmith program.
//
...
// that backends like XLA/Vulkan are able to implement a = smith.ones(...) b =
...
// mutation removal.
struct SMITH_API FunctionalTensorWrapper : public c10::TensorImpl {
  ...
  // The concept of "level" is only ever important to funcsmith; it's exposed
  // here as more of a hook for funcsmith to use.
  int64_t level() const {
    return level_;
  }
  ...
  // needed by funcsmith. funcsmith wants to make sure that all input tensors to
  ...
  // is aliased, and by the time funcsmith is ready to propagate updates to
...
inline FunctionalTensorWrapper* unsafeGetFunctionalWrapper(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(functional_impl != nullptr);
  return functional_impl;
}

SMITH_API bool isBaseTensor(const at::Tensor& tensor);

SMITH_API bool isFunctionalTensor(const at::Tensor& tensor);
SMITH_API bool isFunctionalTensor(const std::optional<Tensor>& t);
SMITH_API bool isFunctionalTensor(
    const c10::List<std::optional<Tensor>>& t_list);
SMITH_API bool isFunctionalTensor(ITensorListRef list);

SMITH_API Tensor to_functional_tensor(const Tensor& tensor);
SMITH_API std::optional<Tensor> to_functional_tensor(
    const std::optional<Tensor>& tensor);
SMITH_API c10::List<std::optional<Tensor>> to_functional_tensor(
    const c10::List<std::optional<Tensor>>& t_list);
SMITH_API std::vector<Tensor> to_functional_tensor(ITensorListRef t_list);

SMITH_API void freeze_functional_tensor(const Tensor& tensor);

SMITH_API Tensor
from_functional_tensor(const Tensor& tensor, bool assert_functional = true);
SMITH_API std::optional<Tensor> from_functional_tensor(
    const std::optional<Tensor>& t,
    bool assert_functional = true);
SMITH_API c10::List<std::optional<Tensor>> from_functional_tensor(
    const c10::List<std::optional<Tensor>>& t_list);
SMITH_API std::vector<Tensor> from_functional_tensor(ITensorListRef t_list);

SMITH_API void sync(const at::Tensor& t);
SMITH_API void sync(const std::optional<Tensor>& t);
SMITH_API void sync(const c10::List<std::optional<Tensor>>& t_list);
SMITH_API void sync(ITensorListRef t_list);

SMITH_API void replace_(const Tensor& functional_tensor, const Tensor& other);
SMITH_API void replace_(
    const ITensorListRef functional_tensor,
    ITensorListRef other);

SMITH_API void commit_update(const Tensor& functional_tensor);
SMITH_API void commit_update(ITensorListRef functional_tensor);

SMITH_API void unsafe_reset_storage(const Tensor& functional_tensor);

SMITH_API void mark_mutation_hidden_from_autograd(
    const Tensor& functional_tensor);

SMITH_API bool are_all_mutations_hidden_from_autograd(
    const Tensor& functional_tensor);

SMITH_API bool are_all_mutations_under_no_grad_or_inference_mode(
    const Tensor& functional_tensor);

// These two methods are XLA-specific logic and are no-ops
// for the normal functionalization flow.
SMITH_API void propagate_xla_data(
    const Tensor& functional_tensor,
    const Tensor& other);
SMITH_API void propagate_xla_data(
    const ITensorListRef functional_tensor,
...
SMITH_API void propagate_xla_data_direct(
    const Tensor& tensor,
    const Tensor& other);
SMITH_API void propagate_xla_data_direct(
...
SMITH_API Tensor apply_view_meta_sequence(
    const Tensor& base,
...
SMITH_API bool getFunctionalizationReapplyViewsTLS();
SMITH_API void setFunctionalizationReapplyViewsTLS(bool reapply_views);

class SMITH_API FunctionalizationReapplyViewsGuard {
 public:
...
SMITH_API void functionalize_op_helper(
