...
SMITH_API std::vector<int64_t> infer_size(IntArrayRef a, IntArrayRef b);
SMITH_API std::vector<SymInt> infer_size_symint(
    SymIntArrayRef a,
    SymIntArrayRef b);
SMITH_API DimVector infer_size_dimvector(IntArrayRef a, IntArrayRef b);
SMITH_API SymDimVector
...
SMITH_API std::tuple<std::vector<int64_t>, std::vector<int64_t>>
inferExpandGeometry(
    IntArrayRef tensor_sizes,
    IntArrayRef tensor_strides,
    IntArrayRef sizes);

SMITH_API InferExpandGeometryResult<DimVector> inferExpandGeometry_dimvector(
    IntArrayRef tensor_sizes,
    IntArrayRef tensor_strides,
    IntArrayRef sizes);

SMITH_API std::vector<int64_t> infer_dense_strides(
    IntArrayRef tensor_sizes,
    IntArrayRef tensor_strides);
...
inline void check_defined(
  ...
  for (auto& t : tensors) {
    if (!t.get().defined()) {
      SMITH_CHECK(false, api_name, "(...) called with an undefined Tensor");
    }
  }
}
...
inline Tensor _sum_to(
  ...
  for (int64_t i = leading_dims; i < static_cast<int64_t>(sizes.size()); ++i) {
    if (SMITH_GUARD_OR_FALSE(sym_eq(shape[i - leading_dims], 1)) &&
        SMITH_GUARD_OR_TRUE(sym_ne(sizes[i], 1))) {
      reduce_dims.push_back(i);
    } else {
