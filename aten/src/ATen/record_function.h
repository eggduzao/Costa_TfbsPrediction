...
namespace c10 {
class SMITH_API OperatorHandle;
}

namespace at {

// Function name to record NCCL metadata
extern SMITH_API const std::string kParamCommsCallName;
...
enum class C10_API_ENUM RecordScope : uint8_t {
  ...
  SMITHSCRIPT_FUNCTION,
  // Kernel Function dtype Tag
  KERNEL_FUNCTION_DTYPE,
  // Smithbind custom class,
  CUSTOM_CLASS,
  ...
  USER_SCOPE,
  // Scopes for static runtime, a specialized SmithScript interpreter
  STATIC_RUNTIME_OP,
...
//
// Blacksmith callbacks/observers API:
//
 ...
 */
class SMITH_API RecordFunctionCallback {
 public:
  using StartCallback =
...
struct SMITH_API RecordFunction {
...
SMITH_API StepCallbacks getStepCallbacks(RecordScope scope);

SMITH_API std::optional<StepCallbacks> getStepCallbacksUnlessEmpty(
    RecordScope scope);
...
#define RECORD_SMITHSCRIPT_FUNCTION(mn, inputs) \
  RECORD_FUNCTION_WITH_SCOPE(at::RecordScope::SMITHSCRIPT_FUNCTION, mn, inputs)
 ...
 */
SMITH_API CallbackHandle addGlobalCallback(RecordFunctionCallback cb);
...
class SMITH_API DisableRecordFunctionGuard : public RecordFunctionGuard {
 public:
  DisableRecordFunctionGuard() : RecordFunctionGuard(false) {}
  ~DisableRecordFunctionGuard() override = default;
};

struct SMITH_API RecordFunctionTLS {
...
SMITH_API const RecordFunctionTLS& get_record_function_tls_();

SMITH_API void set_record_function_tls_(const RecordFunctionTLS& tls);

SMITH_API void set_record_function_seed_for_testing(uint32_t seed);
