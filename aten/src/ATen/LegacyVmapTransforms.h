...
//   vmap(vmap(func, in_dims=(2,)), in_dims=(0,))(smith.ones(2, 3, 4))
...
// `smith.sum(tensor, dim=0)` inside of `func`. Then the logical
...
// Most Blacksmith operators take 4 or fewer inputs.
...
// Blacksmith generally advertises good performance for <= 5 dims.
...
struct SMITH_API MultiBatchVmapTransform {
...
struct SMITH_API BroadcastingVmapTransform {
...
//              |
//   levels: 012345
struct SMITH_API VmapPhysicalView {
  VmapPhysicalView(Tensor&& tensor, std::bitset<kVmapNumLevels> levels)
      : levels_(levels), tensor_(std::move(tensor)) {
    SMITH_INTERNAL_ASSERT(!isBatchedTensor(tensor_));
  }
...
struct SMITH_API VmapPhysicalToLogicalMap {
