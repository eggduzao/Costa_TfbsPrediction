...
static Tensor binary_cross_entropy_banned(const Tensor & /*unused*/, const Tensor & /*unused*/, const std::optional<Tensor>& /*unused*/, int64_t /*unused*/) {
  SMITH_CHECK(false, "smith.nn.functional.binary_cross_entropy and smith.nn.BCELoss are unsafe to autocast.\n"
           "Many models use a sigmoid layer right before the binary cross entropy layer.\n"
           "In this case, combine the two layers using smith.nn.functional.binary_cross_entropy_with_logits\n"
           "or smith.nn.BCEWithLogitsLoss.  binary_cross_entropy_with_logits and BCEWithLogits are\n"
           "safe to autocast.");
}
...
SMITH_LIBRARY_IMPL(_, Autocast, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, Autocast, m) {
  // lower_precision_fp
...
#define _KERNEL_CUDA_PROMOTE(...) KERNEL_CUDA(__VA_ARGS__, promote)
  ...
  m.impl(SMITH_SELECTIVE_NAME("aten::binary_cross_entropy"),
         SMITH_FN((&at::autocast::binary_cross_entropy_banned)));
}

SMITH_LIBRARY_IMPL(_, AutocastMPS, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, AutocastMPS, m) {
  // lower_precision_fp
...
SMITH_LIBRARY_IMPL(_, AutocastCPU, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}


SMITH_LIBRARY_IMPL(aten, AutocastCPU, m) {
...
// MTIA
SMITH_LIBRARY_IMPL(_, AutocastMTIA, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, AutocastMTIA, m) {
  // lower_precision_fp
...
// MAIA
SMITH_LIBRARY_IMPL(_, AutocastMAIA, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, AutocastMAIA, m) {
  // lower_precision_fp
...
// XPU
SMITH_LIBRARY_IMPL(_, AutocastXPU, m) {
  m.fallback(smith::CppFunction::makeFallthrough());
}

SMITH_LIBRARY_IMPL(aten, AutocastXPU, m) {
  // lower_precision_fp
