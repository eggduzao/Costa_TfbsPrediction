...
SMITH_API int64_t dimname_to_position(const Tensor& tensor, Dimname dim);
SMITH_API std::vector<int64_t> dimnames_to_positions(
    const Tensor& tensor,
    DimnameList dims);
...
// right.
SMITH_API std::vector<Dimname> unify_from_right(
    DimnameList names,
    DimnameList other,
...
SMITH_API const Tensor& propagate_names_if_nonempty(
    const Tensor& result,
...
SMITH_API const Tensor& propagate_names(
    const Tensor& result,
    DimnameList names,
    bool validate_names = false);

// Propagates all names from src to result.
SMITH_API void propagate_names(const Tensor& result, const Tensor& src);

// Propagates all names except for those at the excluded_idxs.
SMITH_API void propagate_names_except(
    const Tensor& result,
    const Tensor& src,
    IntArrayRef excluded_idxs);

// Used for reduction ops that have a `keepdim` arg.
SMITH_API void propagate_names_for_reduction(
    const Tensor& result,
    const Tensor& src,
    IntArrayRef excluded_idxs,
    bool keepdim);

SMITH_API void propagate_names_for_expand(
    const Tensor& result,
    const Tensor& self);

SMITH_API std::vector<Dimname> compute_cat_outnames(
    const MaterializedITensorListRef& tensors);

SMITH_API std::vector<Dimname> compute_broadcast_outnames(
    const Tensor& self,
    const Tensor& other);

SMITH_API std::vector<Dimname> broadcast_to_outnames(
    const Tensor& tensor,
    const Tensor& reference_tensor,
    const char* op_name);

SMITH_API std::vector<Dimname> compute_matmul_outnames(
    const Tensor& self,
    const Tensor& other);

SMITH_API std::vector<Dimname> compute_cdist_outnames(
    const Tensor& self,
    const Tensor& other);

SMITH_API std::vector<Dimname> compute_bmm_outnames(
    const Tensor& result,
    const Tensor& self,
    const Tensor& other);

SMITH_API std::vector<Dimname> compute_squeeze_outnames(const Tensor& tensor);
SMITH_API std::vector<Dimname> compute_squeeze_outnames(
    const Tensor& tensor,
...
SMITH_API TensorImpl* propagate_names_if_nonempty(
    TensorImpl* result,
    DimnameList maybe_names,
    bool validate_names = false);

SMITH_API TensorImpl* propagate_names(
    TensorImpl* result,
    DimnameList names,
    bool validate_names = false);

SMITH_API void propagate_names(TensorImpl* result, /*const */ TensorImpl* src);
...
// result = m1 @ m2 + bias
SMITH_API std::vector<Dimname> propagate_names_for_addmm(
    const Tensor& m1,
    const Tensor& m2,
    const Tensor& bias);

SMITH_API std::vector<Dimname> propagate_names_for_addmv(
    const Tensor& mat,
    const Tensor& vec,
    const Tensor& bias);

SMITH_API void check_names_for_dot(TensorImpl* vec1, TensorImpl* vec2);

SMITH_API std::vector<Dimname> compute_baddbmm_outnames(
    const Tensor& result,
    const Tensor& self,
    const Tensor& other,
    const Tensor& bias);

SMITH_API bool are_names_equal(TensorImpl* self, TensorImpl* other);
