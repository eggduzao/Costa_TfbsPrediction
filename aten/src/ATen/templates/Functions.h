...
#ifdef SMITH_ASSERT_NO_OPERATORS
...
#if defined(AT_PER_OPERATOR_HEADERS) && defined(SMITH_ASSERT_ONLY_METHOD_OPERATORS)
#error This change adds a dependency on all blacksmith operators, meaning the     \
  file will need to be re-compiled every time an operator is changed or added. \
  Consider including a specific operator from <ATen/ops/{my_operator}.h> and   \
  see NOTE [SMITH_ASSERT_ONLY_METHOD_OPERATORS].
#endif

// NOTE: [SMITH_ASSERT_ONLY_METHOD_OPERATORS]
//
// In ATen, certain generated headers files include the definitions of
// every single operator in Blacksmith. Unfortunately this means every
// time an operator signature is updated or changed in
// native_functions.yaml, you (and every other Blacksmith developer) need
...
// build times for all Blacksmith developers. These headers are split
