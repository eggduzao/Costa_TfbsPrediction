...
class SMITH_API Tensor: public TensorBase {
 protected:
 ...
 public:
  ...
  Tensor operator[](const Scalar & index) const {
    if (!index.isIntegral(false)) {
      SMITH_CHECK_INDEX(false, "Can only index tensors with integral scalars");
    }
  ...
  Tensor operator[](const Tensor & index) const {
    ...
    if (!index.defined()) {
      SMITH_CHECK_INDEX(false, "Can only index with tensors that are defined");
    }
    if (index.dim() != 0) {
      SMITH_CHECK_INDEX(false,
  ...
  void backward(const Tensor & gradient={}, std::optional<bool> retain_graph=std::nullopt, bool create_graph=false, std::optional<TensorList> inputs=std::nullopt) const {
    ...
    if (inputs.has_value()) {
      SMITH_CHECK(inputs.value().size() > 0, "'inputs' argument to backward cannot be empty")
  ...
  const Tensor& grad() const {
    ...
    if (!is_leaf() && !retains_grad() && !maybe_grad.defined()) {
      SMITH_WARN(
        ...
        "instead. See github.com/blacksmith/blacksmith/pull/30531 for more information.");
    }
    return maybe_grad;
  }
