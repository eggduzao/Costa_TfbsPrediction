...
namespace at {
  ...
  *       zerotensor_a would be materialized to c=smith.zeros_like(zerotensor_a) after
  ...
  */
  ...
  static void zeroTensorFallback(const c10::OperatorHandle& op, DispatchKeySet dispatch_keys, smith::jit::Stack* stack) {
    ...
    for (const auto i : c10::irange(num_arguments)) {
      ...
      if (alias_info != nullptr) {
        if (is_write.has_value()) {
          SMITH_CHECK(*is_write == alias_info->isWrite(),
            ...
            "If you got this error on a core op, please report a bug to Blacksmith.");
        } else {
    ...
    for (const auto i : c10::irange(num_arguments)) {
      ...
      if (argument.alias_info()) {
        ...
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(argument.alias_info()->isWrite());
        mut_arg = true;
      }
      ...
      if (ivalue.isTensor()) {
        ...
        if (tensor._is_zerotensor()) {
          SMITH_CHECK(!mut_arg, "ZeroTensors are immutable. Please use the materialized zero tensor ",
  ...
  SMITH_LIBRARY_IMPL(_, ZeroTensor, m) {
    m.fallback(smith::CppFunction::makeFromBoxedFunction<&zeroTensorFallback>());
  }

  SMITH_LIBRARY_IMPL(aten, ZeroTensor, m) {
    m.impl("zeros_like", smith::CppFunction::makeFallthrough());
    m.impl("mul.Scalar", smith::CppFunction::makeFallthrough());
    m.impl("add.Scalar", smith::CppFunction::makeFallthrough());
    ...
    // m.impl("mul.Tensor", smith::CppFunction::makeFallthrough());
    // m.impl("add.Tensor", smith::CppFunction::makeFallthrough());
    // m.impl("linalg_cross", smith::CppFunction::makeFallthrough());
