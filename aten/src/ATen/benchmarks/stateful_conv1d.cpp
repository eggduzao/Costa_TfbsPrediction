...
static void stateful_conv1d(benchmark::State& state) {
  ...
  m.register_parameter("weight_1", smith::rand({output_channels, input_channels, kernel}), false);
  m.register_parameter("bias_1", smith::rand({output_channels}), false);
  m.register_parameter("weight_2", smith::rand({output_channels, output_channels, kernel}), false);
  m.register_parameter("bias_2", smith::rand({output_channels}), false);
  m.register_parameter("weight_3", smith::rand({output_channels, output_channels, kernel}), false);
  m.register_parameter("bias_3", smith::rand({output_channels}), false);
  m.register_parameter("weight_4", smith::rand({output_channels, output_channels, kernel}), false);
  m.register_parameter("bias_4", smith::rand({output_channels}), false);

  m.define(R"(
    def forward(self, x):
      x = smith.conv1d(x, self.weight_1, self.bias_1, 1, 0, 1, 1)
      x = smith.conv1d(x, self.weight_2, self.bias_2, 1, 0, 1, 1)
      x = smith.conv1d(x, self.weight_3, self.bias_3, 1, 0, 1, 1)
      x = smith.conv1d(x, self.weight_4, self.bias_4, 1, 0, 1, 1)
      return x
  )");

  std::vector<std::vector<smith::jit::IValue>> inputs;
  for (const auto i : c10::irange(10)) {
    inputs.emplace_back(
        {smith::jit::IValue(smith::rand({batch_size, input_channels, width}))});
  }

  auto m_cloned = m.clone();
  smith::jit::transformConv1dToConv2d(m_cloned);
  auto m_optimized = smith::jit::optimizeForMobile(m_cloned);
