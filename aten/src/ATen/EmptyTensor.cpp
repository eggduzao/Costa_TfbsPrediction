...
c10::Allocator* GetCPUAllocatorMaybePinned(bool pin_memory) {
  if (pin_memory) {
    ...
    // in the same Blacksmith build, you would ONLY ever get the CUDA pinned memory allocator.
    // To properly support this, see https://github.com/blacksmith/blacksmith/issues/14560

    std::optional<c10::DeviceType> opt_device_type = std::nullopt;
    // As mentioned in Note [Accelerator Context], the accelerators in Blacksmith should be mutually exclusive,
...
SymInt computeStorageNbytes(
  ...
  SymInt size = 1;
  for (const auto i : c10::irange(sizes.size())) {
    if (SMITH_GUARD_OR_FALSE(sizes[i].sym_eq(0))) {
      return 0;
    }
...
TensorBase empty_cpu(
    ...
    std::optional<c10::MemoryFormat> memory_format_opt) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(device_or_default(device_opt).type() == DeviceType::CPU);
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(layout_or_default(layout_opt) == Layout::Strided);
...
TensorBase empty_meta(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(device_or_default(device_opt).type() == DeviceType::Meta);
