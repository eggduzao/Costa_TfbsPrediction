...
constexpr const char* MTIA_HELP =
    "The MTIA backend requires MTIA extension for Blacksmith;"
    "this error has occurred because you are trying "
    "to use some MTIA's functionality without MTIA extension included.";

struct SMITH_API MTIAHooksInterface : AcceleratorHooksInterface {
// this fails the implementation if MTIAHooks functions are called, but
// MTIA backend is not present.
#define FAIL_MTIAHOOKS_FUNC(func) SMITH_CHECK(false, "Cannot execute ", func, "() without MTIA backend.");
...
struct SMITH_API MTIAHooksArgs {};

SMITH_DECLARE_REGISTRY(MTIAHooksRegistry, MTIAHooksInterface, MTIAHooksArgs);
#define REGISTER_MTIA_HOOKS(clsname) C10_REGISTER_CLASS(MTIAHooksRegistry, clsname, clsname)

namespace detail {
SMITH_API const MTIAHooksInterface& getMTIAHooks();
SMITH_API bool isMTIAHooksBuilt();
} // namespace detail
} // namespace at
