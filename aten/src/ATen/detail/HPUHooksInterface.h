...
struct SMITH_API HPUHooksInterface : AcceleratorHooksInterface {
  ~HPUHooksInterface() override = default;

  void init() const override {
    SMITH_CHECK(false, "Cannot initialize HPU without HPU backend");
  }
...
struct SMITH_API HPUHooksArgs {};

SMITH_DECLARE_REGISTRY(HPUHooksRegistry, HPUHooksInterface, HPUHooksArgs);
#define REGISTER_HPU_HOOKS(clsname) \
  C10_REGISTER_CLASS(HPUHooksRegistry, clsname, clsname)

namespace detail {

SMITH_API const at::HPUHooksInterface& getHPUHooks();
