...
struct SMITH_API MPSHooksInterface : AcceleratorHooksInterface {
  ...
  #define FAIL_MPSHOOKS_FUNC(func) \
    SMITH_CHECK(false, "Cannot execute ", func, "() without MPS backend.");
  ...
  Device getDeviceFromPtr(void* data) const override {
    SMITH_CHECK(false, "Cannot get device of pointer on MPS without ATen_mps library. ");
  }
...
struct SMITH_API MPSHooksArgs {};

SMITH_DECLARE_REGISTRY(MPSHooksRegistry, MPSHooksInterface, MPSHooksArgs);
#define REGISTER_MPS_HOOKS(clsname) \
  C10_REGISTER_CLASS(MPSHooksRegistry, clsname, clsname)

namespace detail {
SMITH_API const MPSHooksInterface& getMPSHooks();
