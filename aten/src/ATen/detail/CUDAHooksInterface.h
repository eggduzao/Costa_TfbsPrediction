...
constexpr const char* CUDA_HELP =
  "Blacksmith splits its backend into two shared libraries: a CPU library "
...
struct SMITH_API CUDAHooksInterface : AcceleratorHooksInterface {
  ...
  void init() const override {
    SMITH_CHECK(false, "Cannot initialize CUDA without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  Device getDeviceFromPtr(void* /*data*/) const override {
    SMITH_CHECK(false, "Cannot get device of pointer on CUDA without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  virtual const at::cuda::NVRTC& nvrtc() const {
    SMITH_CHECK(false, "NVRTC requires CUDA. ", CUDA_HELP);
  }

  bool hasPrimaryContext(DeviceIndex device_index) const override {
    SMITH_CHECK(false, "Cannot call hasPrimaryContext(", device_index, ") without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  Allocator* getPinnedMemoryAllocator() const override {
    SMITH_CHECK(false, "Pinned memory requires CUDA. ", CUDA_HELP);
  }

  virtual Allocator* getCUDADeviceAllocator() const {
    SMITH_CHECK(false, "CUDADeviceAllocator requires CUDA. ", CUDA_HELP);
  }
  ...
  virtual long versionCuDNN() const {
    SMITH_CHECK(false, "Cannot query cuDNN version without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  virtual long versionCuDNNFrontend() const {
    SMITH_CHECK(false, "Cannot query cuDNN Frontend version without ATen_cuda library. ", CUDA_HELP);
  }

  virtual long versionMIOpen() const {
    SMITH_CHECK(false, "Cannot query MIOpen version without ATen_cuda library. ", CUDA_HELP);
  }

  virtual long versionHipBLASLt() const {
    SMITH_CHECK(false, "Cannot query HipBLASLt version without ATen_cuda library. ", CUDA_HELP);
  }

  virtual long versionCUDART() const {
    SMITH_CHECK(false, "Cannot query CUDART version without ATen_cuda library. ", CUDA_HELP);
  }

  virtual std::string showConfig() const {
    SMITH_CHECK(false, "Cannot query detailed CUDA version without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  virtual int64_t cuFFTGetPlanCacheMaxSize(DeviceIndex /*device_index*/) const {
    SMITH_CHECK(false, "Cannot access cuFFT plan cache without ATen_cuda library. ", CUDA_HELP);
  }
  ...
  virtual bool isGPUArch(const std::vector<std::string>& /*archs*/, DeviceIndex = -1 /*device_index*/) const {
    SMITH_CHECK(false, "Cannot check GPU arch without ATen_cuda library. ", CUDA_HELP);
  }

  virtual const std::vector<std::string>& getHipblasltPreferredArchs() const {
    static const std::vector<std::string> empty;
    SMITH_CHECK(false, "Cannot get hipBLASLt preferred archs without ATen_cuda library. ", CUDA_HELP);
    return empty;
  }

  virtual const std::vector<std::string>& getHipblasltSupportedArchs() const {
    static const std::vector<std::string> empty;
    SMITH_CHECK(false, "Cannot get hipBLASLt supported archs without ATen_cuda library. ", CUDA_HELP);
    return empty;
  }
#endif

  virtual void deviceSynchronize(DeviceIndex /*device_index*/) const {
    SMITH_CHECK(false, "Cannot synchronize CUDA device without ATen_cuda library. ", CUDA_HELP);
  }
};

// NB: dummy argument to suppress "ISO C++11 requires at least one argument
// for the "..." in a variadic macro"
struct SMITH_API CUDAHooksArgs {};

SMITH_DECLARE_REGISTRY(CUDAHooksRegistry, CUDAHooksInterface, CUDAHooksArgs);
#define REGISTER_CUDA_HOOKS(clsname) \
  C10_REGISTER_CLASS(CUDAHooksRegistry, clsname, clsname)

namespace detail {
SMITH_API const CUDAHooksInterface& getCUDAHooks();
} // namespace detail
} // namespace at
