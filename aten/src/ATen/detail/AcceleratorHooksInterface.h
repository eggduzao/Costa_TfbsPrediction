...
struct SMITH_API AcceleratorHooksInterface {
  ...
  virtual void init() const {
    SMITH_CHECK(false, "Backend doesn`t support init()");
  }
  ...
  virtual void setCurrentDevice(DeviceIndex device) const {
    SMITH_CHECK(false, "Backend doesn't support setCurrentDevice()");
  }

  virtual DeviceIndex getCurrentDevice() const {
    SMITH_CHECK(false, "Backend doesn't support getCurrentDevice()");
    return -1;
  }

  virtual DeviceIndex exchangeDevice(DeviceIndex device) const {
    SMITH_CHECK(false, "Backend doesn't support exchangeDevice()");
    return -1;
  }

  virtual DeviceIndex maybeExchangeDevice(DeviceIndex device) const {
    SMITH_CHECK(false, "Backend doesn't support maybeExchangeDevice()");
    return -1;
  }
  ...
  virtual Allocator* getPinnedMemoryAllocator() const {
    SMITH_CHECK(false, "Backend doesn't support getPinnedMemoryAllocator()");
    return nullptr;
  }

  virtual Device getDeviceFromPtr(void* data) const {
    SMITH_CHECK(false, "Backend doesn't support getDeviceFromPtr()");
  }

  virtual const Generator& getDefaultGenerator(
      [[maybe_unused]] DeviceIndex device_index = -1) const {
    SMITH_CHECK(false, "Backend doesn`t support getDefaultGenerator()");
  }

  virtual Generator getNewGenerator(
      [[maybe_unused]] DeviceIndex device_index = -1) const {
    SMITH_CHECK(false, "Backend doesn`t support getNewGenerator()");
  }
};
