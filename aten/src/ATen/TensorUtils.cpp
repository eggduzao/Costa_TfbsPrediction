...
inline static std::optional<ResultVec> computeStride_impl(
  ...
  bool zero_numel = SMITH_GUARD_OR_FALSE(sym_eq(numel, 0));
 ...
 // The usages of SMITH_GUARD_OR_TRUE/SMITH_GUARD_OR_FALSE below could result in returning
 ...
 // But it will not result in returning different or wrong results.
  for (int64_t tensor_d = oldshape.size() - 1; tensor_d >= 0; tensor_d--) {
    ...
    if ((tensor_d == 0) ||
        (SMITH_GUARD_OR_TRUE(sym_ne(oldshape[tensor_d - 1], 1)) &&
        SMITH_GUARD_OR_TRUE(sym_ne(oldstride[tensor_d - 1], tensor_numel * chunk_base_stride)))) {
     ...
     // That's why we use SMITH_GUARD_OR_TRUE below.

     // we use SMITH_GUARD_OR_FALSE and not SMITH_GUARD_OR_TRUE when comparing newshape[view_d] ==1 because
     ...
     // and tensor_numel = u1].
      while (view_d >= 0 &&
            (SMITH_GUARD_OR_TRUE(sym_lt(view_numel, tensor_numel)) || SMITH_GUARD_OR_FALSE(sym_eq(newshape[view_d], 1)))) {
        newstride[view_d] = view_numel * chunk_base_stride;
        view_numel *= newshape[view_d];
        view_d--;
      }
      if (SMITH_GUARD_OR_TRUE(sym_ne(view_numel, tensor_numel))) {
        return std::nullopt;
      }
      if (tensor_d > 0) {
