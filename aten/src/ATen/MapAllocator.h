...
SMITH_API std::string NewProcessWideShmHandle();

class SMITH_API MapAllocator {
 public:
  ...
  int fd() const {
#ifdef _WIN32
    SMITH_CHECK(false, "MapAllocator::fd() is unsupported on Windows");
#else
    return fd_;
#endif
  }
...
struct SMITH_API RefcountedMapAllocatorArgCheck {
  RefcountedMapAllocatorArgCheck(int flags);
};

class SMITH_API RefcountedMapAllocator : private RefcountedMapAllocatorArgCheck,
                                         public MapAllocator {
 public:
