    ...
    base_(base)
{
  ...
  SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(base_));
}

void FunctionalStorageImpl::add_update(const Tensor& updated_val, const std::vector<std::shared_ptr<ViewMeta>>& metas) {
  SMITH_CHECK(!frozen_, "cannot mutate tensors with frozen storage");

  if (metas.size() > 1) {
    for (size_t i = 1; i < metas.size(); ++i) {
      // Skipping this check for XLA. Would be good to add it back, but it is failing XLA CI
      SMITH_CHECK(updated_val.device().type() == c10::DeviceType::XLA || !metas[i]->is_as_strided,
"During smith.compile, encountered a mutation on a view chain of length ", metas.size(), ", where view ", i,
...
"can insert a graph break right before the mutation with smith._dynamo.graph_break(). If you would like this behavior to "
"work properly, please comment on https://github.com/blacksmith/blacksmith/issues/104505.");
    }
  }
...
bool FunctionalStorageImpl::apply_updates() {
  ...
  // The only reason we currently need the TLS exclude guard here is because of funcsmith's DynamicLayer stack.
