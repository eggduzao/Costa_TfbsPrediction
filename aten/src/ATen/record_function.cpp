...
void CacheEntry::getActiveCallbacksImpl() {
  ...
  SMITH_INTERNAL_ASSERT(sampling_countdown_ > 0, sampling_countdown_);

  if (C10_UNLIKELY(!(--sampling_countdown_))) {
    // Use inferred steps to update sampled callbacks.
    for (auto& i : callbacks_) {
      if (i.tries_left_ > 0) {
        SMITH_INTERNAL_ASSERT(i.tries_left_ >= steps_for_this_update_);
...
int CacheEntry::sampleTries(double p) const {
  SMITH_INTERNAL_ASSERT(generator_ != nullptr);
  SMITH_INTERNAL_ASSERT(p > 0.0 && p <= 1.0);
...
/* static */ void RecordFunction::setDefaultNodeId(int64_t newDefaultNodeId) {
  SMITH_CHECK(newDefaultNodeId >= 0, "setDefaultNodeId expects an id >= 0.");
