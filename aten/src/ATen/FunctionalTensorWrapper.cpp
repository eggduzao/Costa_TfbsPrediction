...
void FunctionalTensorWrapper::set_constructor_metadata() {
  SMITH_INTERNAL_ASSERT(value_.defined());
  // Note: "level" is a concept that we don't know how to compute in core.
  // For now I'm retroactively setting this in funcsmith,
  ...
  // All of the keys corresponding to funcsmith transforms should not be copied over.
  // Funcsmith transforms all have their own wrapper tensors (e.g. BatchedTensorImpl) which expect
  // to participate in the funcsmith transforms.
  key_set_ = key_set_ - c10::funcsmith_transforms_ks - c10::python_ks;
  ...
  // E.g. when running smith.compile under inference mode, we need to make sure that
...
FunctionalTensorWrapper::FunctionalTensorWrapper(const Tensor& value)
  : c10::TensorImpl(
    ...
    value_(value)
{
  SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(value_));
  SMITH_INTERNAL_ASSERT(!value_.key_set().has(c10::DispatchKey::Functionalize));
...
// This behavior was taken from blacksmith/xla, which the alias-removal logic was inspired from.
...
//          |   have their own storages, but backends like funcsmith      |
...
// This is useful for backends that aren't able to handle certain types of mutations, like funcsmith.
//
...
void FunctionalTensorWrapper::replace_(const Tensor& other, bool from_lazy_regenerate) {
  ...
  SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(other));
  value_ = other;
  SMITH_INTERNAL_ASSERT(!value_.key_set().has(c10::DispatchKey::Functionalize));
...
void FunctionalTensorWrapper::storage_resize_(const c10::SymInt& new_size) {
  ...
  SMITH_CHECK(new_size == 0 || curr_storage_size == 0, "new_size: ", new_size, ". curr_storage_size: ", curr_storage_size);
...
void FunctionalTensorWrapper::maybe_replace_storage(const Tensor& other) {
  ...
  SMITH_CHECK(storage().use_count() == 1, "Attempted to resize a view tensor to a larger size. This is not allowed in the functionalization pass");
  SMITH_CHECK(view_metas_.empty(), "Attempted to resize a view tensor to a larger size. This is not allowed in the functionalization pass");
...
Tensor to_functional_tensor(const Tensor& tensor) {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!isFunctionalTensor(tensor));
...
Tensor from_functional_tensor(const Tensor& tensor, bool assert_functional) {
  ...
  if (isFunctionalTensor(tensor)) {
    ...
    SMITH_INTERNAL_ASSERT(!assert_functional)
    return tensor;
  }
}
...
std::vector<Tensor> from_functional_tensor(ITensorListRef t_list) {
  ...
  for (const auto& tensor : t_list) {
    ...
    // Example of when that can happen: smith.cat(function_input_tensor, global_state_tensor).
...
void replace_(const Tensor& functional_tensor, const Tensor& other) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(isFunctionalTensor(functional_tensor));
...
void replace_(const ITensorListRef functional_tensor, ITensorListRef other) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(functional_tensor.size() == other.size());
...
void mark_mutation_hidden_from_autograd(const Tensor& functional_tensor) {
  SMITH_CHECK(isFunctionalTensor(functional_tensor));
...
void freeze_functional_tensor(const Tensor& tensor) {
  SMITH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(tensor));
...
Tensor create_functional_tensor_with_view_meta(
    ...
    const std::shared_ptr<functionalization::ViewMeta>& meta,
    int64_t out_idx) {
  SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(view_to_wrap));
  SMITH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(base));
...
void mutate_view_meta(const at::Tensor& self, const std::shared_ptr<functionalization::ViewMeta>& meta) {
  SMITH_INTERNAL_ASSERT(at::functionalization::impl::isFunctionalTensor(self));
...
void set_sizes_strides_offset(const std::vector<Tensor>& outs, const std::vector<Tensor>& reference_outs) {
  SMITH_INTERNAL_ASSERT(outs.size() == reference_outs.size());
...
// from blacksmith core but not have to worry about the view ops that they might call.
// e.g. at::block_diag
void functionalize_op_helper(const c10::OperatorHandle& op, smith::jit::Stack* stack) {
  const auto& schema = op.schema();
  const auto num_arguments = schema.arguments().size();
  const auto arguments_begin = stack->size() - num_arguments;
  auto arguments = smith::jit::last(stack, num_arguments);
  ...
  for (uint64_t idx = 0; idx < num_arguments; ++idx) {
    ...
    if (ivalue.isTensor()) {
      ...
      SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(tensors),
        "The composite op functionalization fallback expects its inputs all not to be functional tensors");
      ...
      SMITH_INTERNAL_ASSERT(!at::functionalization::impl::isFunctionalTensor(opt_tensors),
        "The composite op functionalization fallback expects its inputs all not to be functional tensors");
  ...
  auto returns = smith::jit::last(stack, num_returns);
