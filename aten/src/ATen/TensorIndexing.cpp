...
// This mirrors `THPVariable_setitem` in smith/csrc/autograd/python_variable_indexing.cpp
...
Tensor Tensor::index(ArrayRef<at::indexing::TensorIndex> indices) const {
  SMITH_CHECK(!indices.empty(), "Passing an empty index list to Tensor::index() is not valid syntax");
...
Tensor & Tensor::index_put_(ArrayRef<at::indexing::TensorIndex> indices, Tensor const & rhs) {
  SMITH_CHECK(!indices.empty(), "Passing an empty index list to Tensor::index_put_() is not valid syntax");
  OptionalDeviceGuard device_guard(device_of(*this));
