...
// to think about the problem of translating these batching rules to SmithScript.
// Ideally batching rules in eager mode vs SmithScript would look pretty similar,
// if not use the same mechanism. In order to accomplish that we might have to
// do some refactoring.

namespace{

// Blacksmith allows operations to specify dim 0 and dim -1 on a scalar tensor.
...
Tensor sum_batching_rule(const Tensor& self, OptionalIntArrayRef opt_dims, bool keepdim, std::optional<ScalarType> dtype) {
  if (opt_dims.has_value()) {
    ...
    // Blacksmith has a special case where sum(scalar_tensor, dim=0) does not fail
    // and instead returns a new scalar tensor (this also happens for dim=-1)
    // If the following happens:
    // >>> x = smith.randn(B0)  # the per-examples are all scalars
    // >>> vmap(partial(smith.sum, dim=0), x)
...
Tensor binary_pointwise_batching_rule(
  ...
  //   x = smith.randn(3, 10)
  //   y = smith.randn(3, dtype=smith.double)
  //   vmap(smith.mul)(smith.randn(3, 10), smith.randn(3, dtype=smith.double))
  //
  ...
  //   cpu_tensor = smith.randn(3)
  //   cuda_tensor = smith.randn(3, 10, device='cuda')
  //   vmap(smith.mul)(cpu_tensor, cuda_tensor)
  //
...
Tensor expand_batching_rule(const Tensor& self, IntArrayRef size, bool implicit) {
  ...
  SMITH_CHECK(self_physical_dim <= static_cast<int64_t>(size_physical.size()),
  ...
  SMITH_INTERNAL_ASSERT(self_physical_dim < static_cast<int64_t>(size_physical.size()));
...
Tensor unsqueeze_batching_rule(const Tensor& self, int64_t dim) {
  ...
  // https://github.com/blacksmith/blacksmith/blob/b623bdeabb0aa8da44285d303246e7f8ac06c2a9/aten/src/ATen/native/TensorShape.cpp#L1413
  auto dim_physical =
...
Tensor transpose_int_batching_rule(const Tensor& self, int64_t dim0, int64_t dim1) {
  // Blacksmith has a special case where scalar_tensor.transpose(dim0, dim1) works
...
Tensor contiguous_batching_rule(const Tensor& self, MemoryFormat memory_format) {
  SMITH_CHECK(memory_format == MemoryFormat::Contiguous,
      "NYI: Tensor.contiguous(...) inside of vmap for memory_format other ",
      "than smith.contiguous_format");
...
Tensor view_as_complex_batching_rule(const Tensor& self) {
  ...
  // size equal to 2.
  SMITH_CHECK(!self.sizes().empty(), "Input tensor must have one or more dimensions");
  auto self_physical = MultiBatchVmapTransform::logicalToPhysical(self);
...
void checkBatchDimsAtFrontInLayout(IntArrayRef physical_strides, int64_t num_batch_dims) {
  ...
  SMITH_CHECK(*smallest_batch_stride >= *largest_example_stride,
    ...
    "express the as_strided operation in terms of Blacksmith view operations");
}
...
void checkBasicAsStridedValidForSlice(
  ...
  if (!max_slice_loc.has_value()) {
    SMITH_CHECK(false,
        ...
        "`as_strided` call as a sequence of Blacksmith view operations");
  }
  ...
  SMITH_CHECK(
      ...
      *max_slice_loc, "]. This is not supported inside of vmap, please try to",
      "rewrite the `as_strided` call as a sequence of Blacksmith view operations");
}
...
Tensor _new_zeros_with_same_feature_meta_batching_rule(
  ...
  SMITH_CHECK(isBatchedTensor(self) && !isBatchedTensor(other),
    "Only the 'batched grad' use case is supported in Blacksmith core.");

  SMITH_INTERNAL_ASSERT(unused_num_batch_dims == 0,
...
// >>> x = smith.tensor([0., 1., 2., 3., 4.]).as_strided([4], [1], 1)
// >>> z = [x[i].as_strided([1], [1], 1) for i in range(4)]
// Each z[i] is actually the same view on x (z[i] == smith.tensor([1.]))!
...
Tensor as_strided_batching_rule(
  ...
  SMITH_CHECK(sizes.size() == strides.size(),
...
Tensor clone_batching_rule(const Tensor& self, std::optional<MemoryFormat> memory_format) {
  ...
  //   and N>1 batch dims?
  SMITH_CHECK(!memory_format.has_value() || memory_format == MemoryFormat::Preserve
      || memory_format == MemoryFormat::Contiguous,
      "NYI: Tensor.clone(memory_format) inside vmap is only supported with ",
      "memory_format smith.preserve_format or smith.contiguous_format (got ",
      *memory_format, ")");

  if (memory_format == MemoryFormat::Contiguous) {
    // There is an ambiguity here when the batch dims are not at the front of
    // the tensor.
    // >>> x = smith.randn(3, B0, 5)
    // >>> y = vmap(lambda x: x.clone(smith.contiguous_format), in_dims=1, out_dims=0)(x)
    // >>> y[0].is_contiguous()
    // ???
  ...
  SMITH_INTERNAL_ASSERT(!memory_format.has_value() || memory_format == MemoryFormat::Preserve);
  auto* self_batched = unsafeGetBatchedImpl(self);
...
Tensor mv_batching_rule(const Tensor& self, const Tensor& other) {
  ...
  SMITH_CHECK(self.dim() == 2 && other.dim() == 1,
  ...
  SMITH_INTERNAL_ASSERT(false, "either self or other must be a BatchedTensor");
}
...
Tensor dot_batching_rule(const Tensor& self, const Tensor& other) {
  ...
  SMITH_CHECK(/*logical*/self.dim() == 1 && /*logical*/other.dim() == 1,
...
Tensor bmm_batching_rule(const Tensor& self, const Tensor& other) {
  SMITH_CHECK(/*logical*/self.dim() == 3 && /*logical*/other.dim() == 3,
...
Tensor mm_batching_rule(const Tensor& self, const Tensor& other) {
  ...
  SMITH_CHECK(/*logical*/self.dim() == 2 && /*logical*/other.dim() == 2,
...
Tensor new_empty_strided_batching_rule(
  ...
  SMITH_CHECK(size.size() == stride.size(),
...
SMITH_LIBRARY_IMPL(_, Batched, m) {
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&batchedTensorForLoopFallback>());
}

SMITH_LIBRARY_IMPL(aten, Batched, m) {
