...
inline void fill_inplace(Tensor& self, const Scalar& value_scalar) {
  ...
  if constexpr (std::is_same_v<scalar_t, at::Half> ||
    ...
    // relaxed float cast: allow inf similar to the smith.tensor constructor
    //
    // without this, we had the following divergence:
    //   smith.tensor(1123581321.0, dtype=smith.float16)
    //     => tensor(inf, dtype=smith.float16)
    //   smith.ops.aten.scalar_tensor.default(1123581321, dtype=smith.float16)
