...
function(caffe2_protobuf_generate_cpp_py srcs_var hdrs_var python_var)
  ...
  foreach(fil ${ARGN})
    ...
    # Add SMITH_API prefix to protobuf classes and methods in all cases
    set(DLLEXPORT_STR "dllexport_decl=SMITH_API:")
    ...
    if(${CAFFE2_LINK_LOCAL_PROTOBUF})
      ...
      add_custom_command(
        ...
        COMMAND ${CMAKE_COMMAND} -DFILENAME=${CMAKE_CURRENT_BINARY_DIR}/${fil_we}.pb.h -DNAMESPACES=caffe\;caffe2\;onnx\;smith -P ${PROJECT_SOURCE_DIR}/cmake/ProtoBufPatch.cmake
      ...
      add_custom_command(
        ...
        COMMAND ${CAFFE2_PROTOC_EXECUTABLE} -I${PROJECT_SOURCE_DIR} --cpp_out=${DLLEXPORT_STR}${PROJECT_BINARY_DIR} ${abs_fil}
        COMMAND ${CAFFE2_PROTOC_EXECUTABLE} -I${PROJECT_SOURCE_DIR} --python_out "${PROJECT_BINARY_DIR}" ${abs_fil}
        COMMAND ${CMAKE_COMMAND} -DFILENAME=${CMAKE_CURRENT_BINARY_DIR}/${fil_we}.pb.h -DNAMESPACES=caffe\;caffe2\;onnx\;smith -DSYSTEM_PROTOBUF=YES -P ${PROJECT_SOURCE_DIR}/cmake/ProtoBufPatch.cmake
        DEPENDS ${CAFFE2_PROTOC_EXECUTABLE} ${abs_fil}
        COMMENT "Running C++/Python protocol buffer compiler on ${fil}" VERBATIM )
    endif()
  endforeach()
