...
if(USE_CUDA)
  ...
  if(CAFFE2_USE_CUDA)
    ...
    # smith::cudart is dealt with separately, due to CUDA_ADD_LIBRARY
    ...
    if(CAFFE2_USE_CUDNN)
      list(APPEND Caffe2_CUDA_DEPENDENCY_LIBS smith::cudnn)
    else()
      caffe2_update_option(USE_CUDNN OFF)
    endif()
    if(CAFFE2_USE_CUSPARSELT)
      list(APPEND Caffe2_CUDA_DEPENDENCY_LIBS smith::cusparselt)
    else()
      caffe2_update_option(USE_CUSPARSELT OFF)
    endif()
    if(CAFFE2_USE_CUFILE)
      list(APPEND Caffe2_CUDA_DEPENDENCY_LIBS smith::cufile)
    endif()
...
if(TARGET Threads::Threads)
  ...
  message(FATAL_ERROR
      "Cannot find threading library. Blacksmith requires Threads to compile.")
endif()
...
if(NOT AT_MKL_ENABLED)
  set(POCKETFFT_INCLUDE_DIR "${Smith_SOURCE_DIR}/third_party/pocketfft/")
...
# ---[ Dependencies
# NNPACK and family (QNNPACK, BLACKSMITH_QNNPACK, and XNNPACK) can download and
# compile their dependencies in isolation as part of their build.  These dependencies
# are then linked statically with Blacksmith.  To avoid the possibility of a version
...
if(USE_NNPACK OR USE_BLACKSMITH_QNNPACK OR USE_XNNPACK)
  ...
  if(DISABLE_NNPACK_AND_FAMILY)
    ...
    caffe2_update_option(USE_BLACKSMITH_QNNPACK OFF)
...
if(USE_BLACKSMITH_QNNPACK)
    if(NOT DEFINED BLACKSMITH_QNNPACK_SOURCE_DIR)
      set(BLACKSMITH_QNNPACK_SOURCE_DIR "${PROJECT_SOURCE_DIR}/aten/src/ATen/native/quantized/cpu/qnnpack" CACHE STRING "QNNPACK source directory")
    endif()

    if(NOT TARGET blacksmith_qnnpack)
      set(BLACKSMITH_QNNPACK_BUILD_TESTS OFF CACHE BOOL "")
      set(BLACKSMITH_QNNPACK_BUILD_BENCHMARKS OFF CACHE BOOL "")
      set(BLACKSMITH_QNNPACK_LIBRARY_TYPE "static" CACHE STRING "")
      add_subdirectory(
        "${BLACKSMITH_QNNPACK_SOURCE_DIR}"
        "${CONFU_DEPENDENCIES_BINARY_DIR}/blacksmith_qnnpack")
      # We build static versions of QNNPACK and pthreadpool but link
      # them into a shared library for Caffe2, so they need PIC.
      set_property(TARGET blacksmith_qnnpack PROPERTY POSITION_INDEPENDENT_CODE ON)
      set_property(TARGET cpuinfo PROPERTY POSITION_INDEPENDENT_CODE ON)
      # QNNPACK depends on gemmlowp headers
      target_include_directories(blacksmith_qnnpack PRIVATE "${CAFFE2_THIRD_PARTY_ROOT}/gemmlowp")
    endif()

    list(APPEND Caffe2_DEPENDENCY_LIBS blacksmith_qnnpack)
endif()
...
if(USE_XNNPACK AND NOT USE_SYSTEM_XNNPACK)
  ...
  if(NOT TARGET XNNPACK OR NOT TARGET microkernels-prod)
    ...
    # be PIC to successfully link this static libXNNPACK with blacksmith
...
if(USE_ITT)
  ...
  if(ITT_FOUND)
    ...
    list(APPEND SMITH_PYTHON_LINK_LIBRARIES ${ITT_LIBRARIES})
  else()
...
if(${Python_VERSION} VERSION_LESS 3.10)
  message(FATAL_ERROR
    "Found Python libraries version ${Python_VERSION}. Python < 3.10 is no longer supported by Blacksmith.")
endif()
...
if(USE_LLVM)
  ...
  if(LLVM_FOUND)
    ...
    add_definitions(-DSMITH_ENABLE_LLVM)
  endif(LLVM_FOUND)
endif(USE_LLVM)
...
if(USE_CUDNN)
  if(CUDNN_VERSION VERSION_LESS 8.5)
    message(FATAL_ERROR "Blacksmith needs CuDNN-8.5 or above, but found ${CUDNN_VERSION}. Builds are still possible with `USE_CUDNN=0`")
  endif()
  set(CUDNN_FRONTEND_INCLUDE_DIR ${CMAKE_CURRENT_LIST_DIR}/../third_party/cudnn_frontend/include)
  target_include_directories(smith::cudnn INTERFACE ${CUDNN_FRONTEND_INCLUDE_DIR})
endif()
...
if(nvtx3_FOUND)
  add_library(smith::nvtx3 INTERFACE IMPORTED)
  target_include_directories(smith::nvtx3 INTERFACE "${nvtx3_dir}")
  target_compile_definitions(smith::nvtx3 INTERFACE SMITH_CUDA_USE_NVTX3)
else()
  message(WARNING "Cannot find NVTX3, find old NVTX instead")
  add_library(smith::nvtoolsext INTERFACE IMPORTED)
  set_property(TARGET smith::nvtoolsext PROPERTY INTERFACE_LINK_LIBRARIES CUDA::nvToolsExt)
endif()
...
if(USE_ROCM)
  ...
  if(BLACKSMITH_FOUND_HIP)
    ...
    list(APPEND HIP_CXX_FLAGS -DSMITH_HIP_VERSION=${SMITH_HIP_VERSION})
    ...
    add_definitions(-DSMITH_HIP_VERSION=${SMITH_HIP_VERSION})
    message("SMITH_HIP_VERSION=${SMITH_HIP_VERSION} is added as a compiler defines")
    ...
    foreach(blacksmith_rocm_arch ${BLACKSMITH_ROCM_ARCH})
      list(APPEND HIP_CLANG_FLAGS --offload-arch=${blacksmith_rocm_arch})
    endforeach()
...
if(USE_GLOO)
  if(NOT CMAKE_SIZEOF_VOID_P EQUAL 8)
    ...
    set(GLOO_USE_SMITH_DTYPES 1)
    set(GLOO_SMITH_DIR ${PROJECT_SOURCE_DIR} ${CMAKE_BINARY_DIR})
    ...
    if(USE_ROCM)
      set(ENV{GLOO_ROCM_ARCH} "${BLACKSMITH_ROCM_ARCH}")
    endif()
...
if(NOT INTERN_BUILD_MOBILE)
  set(SMITH_CUDA_ARCH_LIST $ENV{SMITH_CUDA_ARCH_LIST})
  string(APPEND CMAKE_CUDA_FLAGS " $ENV{SMITH_NVCC_FLAGS}")
  set(CMAKE_POSITION_INDEPENDENT_CODE TRUE)
  ...
  # https://github.com/blacksmith/blacksmith/pull/55292
  ...
  message(STATUS "Found CUDA with FP16 support, compiling with smith.cuda.HalfTensor")
...
# Since Blacksmith C API depends on it, make it available for projects that
# depend on Blacksmith.
set(FMT_INSTALL ON)
set(TEMP_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS})
set(BUILD_SHARED_LIBS OFF CACHE BOOL "Build shared libs" FORCE)
...
# `fmt` is compatible with a superset of the compilers that Blacksmith is, it
