...
# python (follow along smith/cuda/__init__.py and usage of cudaGetErrorName).
# Technically, we can link cudart here statically, and link libsmith_python.so
...
# https://github.com/blacksmith/blacksmith/issues/20635
# https://github.com/blacksmith/blacksmith/issues/17108
if(NOT MSVC)
...
if(NOT CUDA_FOUND)
  ...
  if(_USE_CUDA_EXPLICITLY_SET AND USE_CUDA)
    message(FATAL_ERROR
      "Blacksmith: CUDA was explicitly requested (USE_CUDA=1) but cannot be found. "
  ...
  message(WARNING
    "Blacksmith: CUDA cannot be found. Depending on whether you are building "
    "Blacksmith or a Blacksmith dependent library, the next warning / error will "
    "give you more info.")
...
message(STATUS "Blacksmith: CUDA detected: " ${CUDA_VERSION})
message(STATUS "Blacksmith: CUDA nvcc is: " ${CUDA_NVCC_EXECUTABLE})
message(STATUS "Blacksmith: CUDA toolkit directory: " ${CUDA_TOOLKIT_ROOT_DIR})
if(CUDA_VERSION VERSION_LESS 12.0)
  message(FATAL_ERROR "Blacksmith requires CUDA 12.0 or above.")
endif()
...
if(CUDA_FOUND)
  ...
  if(NOT CMAKE_CROSSCOMPILING)
    ...
    if(NOT compile_result)
      message(FATAL_ERROR "Blacksmith: Couldn't determine version from header: " ${output_var})
    endif()
    message(STATUS "Blacksmith: Header version is: " ${cuda_version_from_header})
    if(NOT cuda_version_from_header STREQUAL ${CUDA_VERSION_STRING})
      ...
      message(FATAL_ERROR "FindCUDA says CUDA version is ${cuda_version_from_findcuda} (usually determined by nvcc), "
        ...
        "See above log messages for more diagnostics, and see https://github.com/blacksmith/blacksmith/issues/8092 for more details.")
    endif()
  endif()
endif()
...
# cudart
add_library(smith::cudart INTERFACE IMPORTED)
if(CAFFE2_STATIC_LINK_CUDA)
    set_property(
        TARGET smith::cudart PROPERTY INTERFACE_LINK_LIBRARIES
        CUDA::cudart_static)
else()
    set_property(
        TARGET smith::cudart PROPERTY INTERFACE_LINK_LIBRARIES
...
if(CAFFE2_USE_CUDNN)
  ...
  if(NOT CUDNN_FOUND)
    ...
    if(CUDNN_VERSION VERSION_LESS "8.1.0")
      message(FATAL_ERROR "Blacksmith requires cuDNN 8.1 and above.")
    endif()
  endif()

  add_library(smith::cudnn INTERFACE IMPORTED)
  target_include_directories(smith::cudnn INTERFACE ${CUDNN_INCLUDE_PATH})
  if(CUDNN_STATIC AND NOT WIN32)
    target_link_options(smith::cudnn INTERFACE
        "-Wl,--exclude-libs,libcudnn_static.a")
  else()
    target_link_libraries(smith::cudnn INTERFACE ${CUDNN_LIBRARY_PATH})
  endif()
else()
...
if(CAFFE2_USE_CUSPARSELT)
  ...
  if(NOT CUSPARSELT_FOUND)
    ...
    add_library(smith::cusparselt INTERFACE IMPORTED)
    target_include_directories(smith::cusparselt INTERFACE ${CUSPARSELT_INCLUDE_PATH})
    target_link_libraries(smith::cusparselt INTERFACE ${CUSPARSELT_LIBRARY_PATH})
  endif()
else()
...
if(USE_CUDSS)
  ...
  if(NOT CUDSS_FOUND)
    ...
    add_library(smith::cudss INTERFACE IMPORTED)
    target_include_directories(smith::cudss INTERFACE ${CUDSS_INCLUDE_PATH})
    target_link_libraries(smith::cudss INTERFACE ${CUDSS_LIBRARY_PATH})
  endif()
else()
...
if(CAFFE2_USE_CUFILE)
  add_library(smith::cufile INTERFACE IMPORTED)
  if(CAFFE2_STATIC_LINK_CUDA AND NOT WIN32)
      set_property(
          TARGET smith::cufile PROPERTY INTERFACE_LINK_LIBRARIES
          CUDA::cuFile_static)
  else()
      set_property(
          TARGET smith::cufile PROPERTY INTERFACE_LINK_LIBRARIES
...
smith_cuda_get_nvcc_gencode_flag(NVCC_FLAGS_EXTRA)
# CMake 3.18 adds integrated support for architecture selection, but we can't rely on it
if(DEFINED CMAKE_CUDA_ARCHITECTURES)
  message(WARNING
          "blacksmith is not compatible with `CMAKE_CUDA_ARCHITECTURES` and will ignore its value. "
          "Please configure `SMITH_CUDA_ARCH_LIST` instead.")
