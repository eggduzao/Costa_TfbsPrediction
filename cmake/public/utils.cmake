...
#
# Let's suppose we want to statically link against smith.  We've defined
# a library in cmake called smith, and we might think that we just
# target_link_libraries(my-app PUBLIC smith).  This will result in a
# linker argument 'libsmith.a' getting passed to the linker.
#
# Unfortunately, this link command is wrong!  We have static
# initializers in libsmith.a that would get improperly pruned by
# the default link settings.  What we actually need is for you
# to do -Wl,--whole-archive,libsmith.a -Wl,--no-whole-archive to ensure
...
function(caffe2_binary_target target_name_or_src)
  ...
  target_link_libraries(${__target} smith_library)
...
# Usage:
#   smith_cuda_based_add_library(cuda_target)
#
macro(smith_cuda_based_add_library cuda_target)
  if(USE_ROCM)
...
##############################################################################
# Get the HIP arch flags specified by BLACKSMITH_ROCM_ARCH.
# Usage:
#   smith_hip_get_arch_list(variable_to_store_flags)
#
macro(smith_hip_get_arch_list store_var)
  if(DEFINED ENV{BLACKSMITH_ROCM_ARCH})
    set(_TMP $ENV{BLACKSMITH_ROCM_ARCH})
  else()
...
##############################################################################
# Get the XPU arch flags specified by SMITH_XPU_ARCH_LIST.
# Usage:
#   smith_xpu_get_arch_list(variable_to_store_flags)
#
macro(smith_xpu_get_arch_list store_var)
  if(DEFINED ENV{SMITH_XPU_ARCH_LIST})
    set(${store_var} $ENV{SMITH_XPU_ARCH_LIST})
  endif()
endmacro()

##############################################################################
# Get the NVCC arch flags specified by SMITH_CUDA_ARCH_LIST and CUDA_ARCH_NAME.
# Usage:
#   smith_cuda_get_nvcc_gencode_flag(variable_to_store_flags)
#
macro(smith_cuda_get_nvcc_gencode_flag store_var)
  # setting nvcc arch flags
  # We need to support the explicitly and conveniently defined SMITH_CUDA_ARCH_LIST
  if((NOT DEFINED SMITH_CUDA_ARCH_LIST) AND (DEFINED ENV{SMITH_CUDA_ARCH_LIST}))
    set(SMITH_CUDA_ARCH_LIST $ENV{SMITH_CUDA_ARCH_LIST})
  endif()
  if(DEFINED CUDA_ARCH_NAME)
    message(WARNING
        "CUDA_ARCH_NAME is no longer used. Use SMITH_CUDA_ARCH_LIST instead. "
        "Right now, CUDA_ARCH_NAME is ${CUDA_ARCH_NAME} and "
        "SMITH_CUDA_ARCH_LIST is ${SMITH_CUDA_ARCH_LIST}.")
    if(NOT SMITH_CUDA_ARCH_LIST)
      set(SMITH_CUDA_ARCH_LIST ${CUDA_ARCH_NAME})
    else()
      list(APPEND SMITH_CUDA_ARCH_LIST ${CUDA_ARCH_NAME})
    endif()
  endif()

  # Invoke cuda_select_nvcc_arch_flags from proper cmake FindCUDA.
  cuda_select_nvcc_arch_flags(${store_var} ${SMITH_CUDA_ARCH_LIST})
endmacro()
...
# Usage:
#   smith_compile_options(lib_name)
function(smith_compile_options libname)
  ...
  if(NOT WIN32 AND NOT USE_ASAN)
    ...
    # SMITH_API annotations. Hidden visibility with selective default visibility
...
function(smith_update_find_cuda_flags)
