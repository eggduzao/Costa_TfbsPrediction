...
- Assume the reader has familiarity with Blacksmith. They may not be the expert
...
Use `smith._dynamo.config.patch` for temporarily changing config. It can be used as a decorator on test methods or as a context manager:
...
with smith._dynamo.config.patch(force_compile_during_fx_trace=True):
    # test code here
    pass

# Bad - manual save/restore
orig = smith._dynamo.config.force_compile_during_fx_trace
try:
    smith._dynamo.config.force_compile_during_fx_trace = True
    # test code here
finally:
    smith._dynamo.config.force_compile_during_fx_trace = orig
```
