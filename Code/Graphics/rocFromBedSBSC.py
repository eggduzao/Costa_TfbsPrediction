...
params.append("    2. labelList = List of labels to each of the bed files in the list.")
params.append("    3. bedList = List of files containing bed coordinates separated by comma.")
...
import operator
import numpy as np
from simps_trapz import simp
import matplotlib
matplotlib.use('Agg')
import pylab
from pyl import get_rate_dict
...
fileNL = [e.split("/")[-1].split(".")[0] for e in bedList]
...
for k in range(0,len(bedList)):
    ...
    # Segmentation base
    rateDict = get_rate_dict(fileNL)
    myName = bedFileName.split("/")[-1].split(".")[0]
    if(rateDict[myName][0]): sortedFileName = simp(mpbsName, cellType, labelList[k], bedFileName, sortedFileName, rateDict)
