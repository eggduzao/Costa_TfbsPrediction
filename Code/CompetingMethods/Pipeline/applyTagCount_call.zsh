...
do
  ...
  # Cell Parameters
  if [[ $lab == "DU" ]]; then
    cellList=( "H1hesc" "H7hesc" "HeLaS3" "HepG2" "Huvec" "K562" "Mcf7" "Saos2" )
  elif [[ $lab == "UW" ]]; then
    ...
    do
      ...
      do
        ...
        if [[ $type == "regular" ]]; then
          ...
          outLoc="/hpcwork/izkf/projects/TfbsPrediction/Results/Footprints/Results/"$cell"/fdr_4/TagCount_"$lab"/"
          ...
          outLoc="/hpcwork/izkf/projects/TfbsPrediction/Results/Footprints/Results/"$cell"/fdr_4/TagCount_"$lab"_BIAS/"
          outputFileName=$outLoc$tfbs".bed"
          ...
          outLoc="/hpcwork/izkf/projects/TfbsPrediction/Results/Footprints/Results/"$cell"/fdr_4/TagCount_"$lab"_NAKED2/"
          outputFileName=$outLoc$tfbs".bed"
        fi
        mkdir -p $outLoc

        # Execution
        bsub -J "ATC2" -o "ATC2_out.txt" -e "ATC2_err.txt" -W 48:00 -M 12000 -S 100 -R "select[hpcwork]" ./applyTagCount_pipeline.zsh $mpbsFileName $signalFileName $outputFileName
# -P izkf
      done
    done
  done
