...
for cell in cellList:
  ...
  for tfFile in tfList:
    ...
    outputLocation = "/hpcwork/izkf/projects/TfbsPrediction/Results/Footprints/Predictions/"+cell+"/TC_batch/"
    outputFileName = outputLocation+tfName+".bed"
    os.system("mkdir -p "+outputLocation)

    # Execution
    myL = "_".join([cell,tfName,"TCB"])
    clusterCommand = "bsub "
    clusterCommand += "-J "+myL+" -o "+myL+"_out.txt -e "+myL+"_err.txt "
    clusterCommand += "-W 100:00 -M 12000 -S 100 -P izkf -R \"select[hpcwork]\" ./applyTagCount_pipeline.zsh "
