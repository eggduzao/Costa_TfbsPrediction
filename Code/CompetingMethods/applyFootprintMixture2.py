...
# Parameters
mvLoc = "/home/eg474423/Software/FootprintMixture/"
...
# Creating new mpbsfile
newMpbsFileName = outLoc+"newmpbs.bed"
mpbsFile = open(mpbsFileName,"r")
newMpbsFile = open(newMpbsFileName,"w")
for line in mpbsFile:
  ll = line.strip().split("\t")
  newMpbsFile.write(" ".join(ll[:3])+"\n")
mpbsFile.close()
newMpbsFile.close()
...
rFile.write("b <- read.table('"+newMpbsFileName+"')\n")
rFile.write("p <- read.table('"+peakFileName+"')\n")
rFile.write("m <- MultMMixture_Full(TF_Bed=b,Cuts=c,peakbed=p,Plot=F,PadLen=25,Collapse=T,k=2,ReturnPar=T,Fixed=F,Background='Seq',FastaName='"+seqFileName+"')\n")
