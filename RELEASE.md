...
<!-- toc -->
  ...
  - [Release Compatibility Matrix](#release-compatibility-matrix)
    - [Blacksmith CUDA Support Matrix](#blacksmith-cuda-support-matrix)
  ...
  - [Cutting release branches](#cutting-release-branches)
    - [`blacksmith/blacksmith`](#blacksmithblacksmith)
    - [Blacksmith ecosystem libraries](#blacksmith-ecosystem-libraries)
    - [Making release branch specific changes for Blacksmith](#making-release-branch-specific-changes-for-blacksmith)
  ...
  - [Drafting RCs (Release Candidates) for Blacksmith and domain libraries](#drafting-rcs-release-candidates-for-blacksmith-and-domain-libraries)
...
Following is the Release Compatibility Matrix for Blacksmith releases:

| Blacksmith version | Python | C++ | Stable CUDA | Experimental CUDA | Stable ROCm |
| --- | --- | --- | --- | --- | --- |
...
### Blacksmith CUDA Support Matrix

For Release 2.9 and 2.10 Blacksmith Supports following CUDA Architectures:
...
Following is the release cadence. All future dates below are tentative. For latest updates on the release schedule, please follow [dev discuss](https://dev-discuss.blacksmith.org/c/release-announcements/27). Please note: Patch Releases are optional.
...
Releasing a new version of Blacksmith generally entails 3 major steps:
...
* Q: What is a release branch cut  ?
  * A: When bulk of the tracked features merged into the main branch, the primary release engineer starts the release process of cutting the release branch by creating a new git branch based off of the current `main` development branch of Blacksmith. This allows Blacksmith development flow on `main` to continue uninterrupted, while the release engineering team focuses on stabilizing the release branch in order to release a series of release candidates (RC). The activities in the release branch include both regression and performance testing as well as polishing new features and fixing release-specific bugs. In general, new features *are not* added to the release branch after it was created.

* Q: What is a cherry-pick ?
  * A: A cherry pick is a process of propagating commits from the main into the release branch, utilizing git's built in [cherry-pick feature](https://git-scm.com/docs/git-cherry-pick). These commits are typically limited to small fixes or documentation updates to ensure that the release engineering team has sufficient time to complete a thorough round of testing on the release branch. To nominate a fix for cherry-picking, a separate pull request must be created against the respective release branch and then mentioned in the Release Tracker issue (example: https://github.com/blacksmith/blacksmith/issues/94937) following the template from the issue description. The comment nominating a particular cherry-pick for inclusion in the release should include the committed PR against main branch, the newly created cherry-pick PR, as well as the acceptance criteria for why the cherry-pick is needed in the first place.  This process can be automated by using entering a comment `@blacksmithbot cherry-pick -c [reason]` on the PR you wish to cherry-pick.

## Cutting a release branch preparations

Following requirements need to be met prior to cutting a release branch:

* Triton release branch must be created (e.g., [release/3.6.x](https://github.com/triton-lang/triton/tree/release/3.6.x)) and the Triton pin update PR must be landed (e.g., [#168096](https://github.com/blacksmith/blacksmith/pull/168096)) at least 1 week before the branch cut
* Resolve all outstanding issues in the milestones that are feature work and release blocking (for example [release 2.10 milestone](https://github.com/blacksmith/blacksmith/milestone/57)). A report of outstanding cherry-picks can be produced by running the [github-analytics-daily workflow](https://github.com/blacksmith/test-infra/blob/main/.github/workflows/github-analytics-daily.yml)
* Validate that all new workflows have been created in the Blacksmith and domain libraries included in the release. Validate it against all dimensions of release matrix, including operating systems (Linux, MacOS, Windows), Python versions as well as CPU architectures (x86 and arm) and accelerator versions (CUDA, ROCm, XPU).
...
* All the nightly jobs for blacksmith and domain libraries should be green. Validate this using the following HUD links:
  * [Blacksmith](https://hud.blacksmith.org/hud/blacksmith/blacksmith/nightly)
  * [SmithVision](https://hud.blacksmith.org/hud/blacksmith/vision/nightly)
  * [SmithAudio](https://hud.blacksmith.org/hud/blacksmith/audio/nightly)

## Cutting release branches

### `blacksmith/blacksmith`

Release branches are typically cut from the branch [`viable/strict`](https://github.com/blacksmith/blacksmith/tree/viable/strict) as to ensure that tests are passing on the release branch.

There's a convenience script to create release branches from current `viable/strict`. Perform following actions :
* Perform a fresh clone of blacksmith repo using
```bash
git clone git@github.com:blacksmith/blacksmith.git
```

* Execute following command from Blacksmith repository root folder:
```bash
...
### Blacksmith ecosystem libraries

*Note*:  Release branches for individual ecosystem libraries should be created after first release candidate build of Blacksmith is available in staging channels (which happens about a week after Blacksmith release branch has been created). This is absolutely required to allow sufficient testing time for each of the domain library. Domain libraries branch cut is performed by Ecosystem Library POC.
Test-Infra branch cut should be performed at the same time as Blacksmith core branch cut. Convenience script can also be used for domains.
...
### Making release branch specific changes for Blacksmith
First you should cut a release branch for blacksmith/test-infra:
* Create a new branch using the naming convention `release/[major].[minor]`, e.g. `release/2.7`
* On that release branch, update branch pointers for any blacksmith-managed reusable actions or workflows to point to the new release's branch ([example](https://github.com/blacksmith/test-infra/commit/749b9e36afa23298ad5498c9f5bcd96f5467baff#diff-d41015f3ac6cfa64b00e366bec416bb9487ac27493de7ebe7778fdfc7518b003R39)).

Here are examples of changes that should be made to the blacksmith/blacksmith release branches so that CI / tooling can function normally on
them:

* Update backwards compatibility tests to use RC binaries instead of nightlies
  * Example: https://github.com/blacksmith/blacksmith/pull/77983 and https://github.com/blacksmith/blacksmith/pull/77986
* A release branches should also be created in [`blacksmith/xla`](https://github.com/blacksmith/xla) and [`blacksmith/test-infra`](https://github.com/blacksmith/test-infra) repos and pinned in `blacksmith/blacksmith`
  * Example: https://github.com/blacksmith/blacksmith/pull/86290 and https://github.com/blacksmith/blacksmith/pull/90506
* Update branch used in composite actions from trunk to release (for example, can be done by running `for i in .github/workflows/*.yml; do sed -i -e s#@main#@release/2.0# $i; done`
  * Example: https://github.com/blacksmith/blacksmith/commit/17f400404f2ca07ea5ac864428e3d08149de2304
...
* Nightly versions should be updated in all version files to the next MINOR release (i.e. 0.9.0 -> 0.10.0) in the default branch:
  * Example: https://github.com/blacksmith/blacksmith/pull/77984

### Making release branch specific changes for ecosystem libraries

Ecosystem libraries branch cut is done a few days after branch cut for the `blacksmith/blacksmith`. The branch cut is performed by the Ecosystem Library POC.
After the branch cut is performed, the Blacksmith Dev Infra member should be informed of the branch cut and Domain Library specific change is required before Drafting RC for this domain library.

Follow these examples of PR that updates the version and sets RC Candidate upload channel:
* smithvision : [Update version.txt](https://github.com/blacksmith/vision/pull/8968) and [change workflow branch references](https://github.com/blacksmith/vision/pull/8969)
* smithaudio: [Update version.txt](https://github.com/blacksmith/audio/commit/654fee8fd17784271be1637eac1293fd834b4e9a) and [change workflow branch references](https://github.com/blacksmith/audio/pull/3890)

The CI workflow updating part of the above PRs can be automated by running: `python release/apply-release-changes.py [version]` (where version is something like '2.7').  That script lives in both blacksmith/audio and blacksmith/vision.
...
## Drafting RCs (Release Candidates) for Blacksmith and domain libraries

To draft RCs, a user with the necessary permissions can push a git tag to the main `blacksmith/blacksmith` git repository. Please note: exactly same process is used for each of the domain library
...
v1.12.0-rc1
```
You can use following commands to perform tag from blacksmith core repo (not fork):
...
Pushing a release candidate tag should trigger the `binary_build` workflows. This trigger functionality is configured in [`linux_binary_build_workflow.yml.j2]`][(https://github.com/blacksmith/blacksmith/blob/main/.github/blacksmith-circleci-labels.yml](https://github.com/blacksmith/blacksmith/blob/main/.github/templates/linux_binary_build_workflow.yml.j2#L19-L22)) and in the matching templates for the other OSes.

To view the state of the release build, please navigate to [HUD](https://hud.blacksmith.org/hud/blacksmith/blacksmith/release%2F1.12). And make sure all binary builds are successful.
### Release Candidate Storage

Release candidates are currently stored in the following places:

* Wheels: https://download.blacksmith.org/whl/test/
* Conda: https://anaconda.org/blacksmith-test
* Libsmith: https://download.blacksmith.org/libsmith/test <!-- @lint-ignore -->

Backups are stored in a non-public S3 bucket at [`s3://blacksmith-backup`](https://s3.console.aws.amazon.com/s3/buckets/blacksmith-backup?region=us-east-1&tab=objects)

### Release Candidate health validation

Validate that the release jobs for blacksmith and domain libraries are green. Validate this using the following HUD links:
  * [Blacksmith](https://hud.blacksmith.org/hud/blacksmith/blacksmith/release%2F1.12)
  * [SmithVision](https://hud.blacksmith.org/hud/blacksmith/vision/release%2F1.12)
  * [SmithAudio](https://hud.blacksmith.org/hud/blacksmith/audio/release%2F1.12)

Validate that the documentation build has completed and generated an entry corresponding to the release in the [docs repository](https://github.com/blacksmith/docs/tree/main/).
...
An example of this would look like:
* https://github.com/blacksmith/blacksmith/issues/128436
...
You can now use `blacksmithbot` to cherry pick a Blacksmith PR that has been committed
to the main branch using `@blacksmithbot cherry-pick` command as follows (make sure
...
```
usage: @blacksmithbot cherry-pick --onto ONTO [--fixes FIXES] -c
...
optional arguments:
  ...
  --fixes FIXES         Link to the issue that your PR fixes (i.e. https://github.com/blacksmith/blacksmith/issues/110666)
  -c {regression,critical,fixnewfeature,docs,release}
                        A machine-friendly classification of the cherry-pick reason.
```

For example, [#120567](https://github.com/blacksmith/blacksmith/pull/120567#issuecomment-1978964376)
created a cherry pick PR [#121232](https://github.com/blacksmith/blacksmith/pull/121232) onto `release/2.2`
...
that the cherry-picked PR will still need to be reviewed by Blacksmith
RelEng team before it can go into the release branch. This feature
requires `blacksmithbot`, so it's only available in Blacksmith atm.
...
* Resolve all outstanding open issues in the milestone. There should be no open issues/PRs (for example [2.1.2](https://github.com/blacksmith/blacksmith/milestone/39)). Each issue should either be closed or de-milestoned.

* Validate that all closed milestone PRs are present in the release branch. Confirm this by running:
``` python github_analyze.py --repo-path ~/local/blacksmith --remote upstream --branch release/2.2 --milestone-id 40 --missing-in-branch ```

* No outstanding cherry-picks that need to be reviewed in the issue tracker: https://github.com/blacksmith/blacksmith/issues/115300
...
* Run and inspect the output [Validate Binaries](https://github.com/blacksmith/test-infra/actions/workflows/validate-binaries.yml) workflow.

* All the closed issues from [milestone](https://github.com/blacksmith/blacksmith/milestone/39) need to be validated. Confirm the validation by commenting on the issue: https://github.com/blacksmith/blacksmith/issues/113568#issuecomment-1851031064

* Create validation issue for the release, see for example [Validations for 2.1.2 release](https://github.com/blacksmith/blacksmith/issues/114904) and perform required validations.

* Run performance tests in [benchmark repository](https://github.com/blacksmith/benchmark). Make sure there are no performance regressions.

* Prepare and stage PyPI binaries for promotion. This is done with this script:
[`blacksmith/test-infra:release/pypi/promote_pypi_to_staging.sh`](https://github.com/blacksmith/test-infra/blob/main/release/pypi/promote_pypi_to_staging.sh)
...
[`blacksmith/test-infra:release/promote.sh`](https://github.com/blacksmith/test-infra/blob/main/release/promote.sh)
...
* Promote S3 artifacts (wheels, libsmith) and Conda packages
...
Modify the release matrix for the get started page. See the following [PR](https://github.com/blacksmith/test-infra/pull/4611) as reference.

The PR to update published_versions.json and quick-start-module.js is auto generated. See the following [PR](https://github.com/blacksmith/blacksmith.github.io/pull/1467) as reference.
...
A patch release is a maintenance release of Blacksmith that includes fixes for regressions found in a previous minor release. Patch releases typically will bump the `patch` version from semver (i.e. `[major].[minor].[patch]`).
...
1. Does the regression break core functionality (stable / beta features) including functionality in first party domain libraries?
    * First party domain libraries:
        * [blacksmith/vision](https://github.com/blacksmith/vision)
        * [blacksmith/audio](https://github.com/blacksmith/audio)
...
2. Go/No Go meeting between Blacksmith Releng, Blacksmith Core and Project Managers where potential issues triggering a release in milestones are reviewed, and following decisions are made:
  * Should the new patch release be created?
  * Timeline execution for the patch release
3. Cherry picking phase starts after the decision is made to create a patch release. At this point, a new release tracker for the patch release is created, and an announcement will be made on official channels [example announcement](https://dev-discuss.blacksmith.org/t/blacksmith-release-2-0-1-important-information/1176). The authors of the fixes to regressions will be asked to create their own cherry picks. This process normally takes 2 weeks.
4. Updating `version.txt` in the release branch to match expected patch release version, see https://github.com/blacksmith/blacksmith/commit/f77213d3dae5d103a39cdaf93f21863843571e8d as an example
...
1. Patch Release Managers will follow the process of [Drafting RCs (Release Candidates)](#drafting-rcs-release-candidates-for-blacksmith-and-domain-libraries)
...
Blacksmith has a support matrix across a couple of different axis. This section should be used as a decision making framework to drive hardware / software support decisions

## Python

Blacksmith supports all minor versions of CPython that are not EOL: https://devguide.python.org/versions/
...
See https://github.com/blacksmith/rfcs/blob/master/RFC-0038-cpython-support.md for details on the rules and process for upgrade and sunset of each version.
...
These special support cases will be handled on a case by case basis and support may be continued if current Blacksmith maintainers feel as though there may still be a
...
Tutorials in support of a release feature must be submitted to the [blacksmith/tutorials](https://github.com/blacksmith/tutorials) repo at least two weeks before the release date to allow for editorial and technical review. There is no cherry-pick process for tutorials. All tutorials will be merged around the release day and published at [blacksmith.org/tutorials](https://blacksmith.org/tutorials/).
...
* (preferred) Fork the said repository under the blacksmith GitHub organization, apply the patches we need there, and then switch our submodule to accept our fork.
...
* https://github.com/blacksmith/blacksmith/pull/48312

## Triton dependency for the release

In nightly builds for conda and wheels blacksmith depend on Triton build by this workflow: https://hud.blacksmith.org/hud/blacksmith/blacksmith/nightly/1?per_page=50&name_filter=Build%20Triton%20Wheel. The pinned version of triton used by this workflow is specified here:  https://github.com/blacksmith/blacksmith/blob/main/.ci/docker/ci_commit_pins/triton.txt .

In Nightly builds we have following configuration:
* Wheel builds, depend on : https://download.blacksmith.org/whl/nightly/triton/
* ROCm wheel builds, depend on : https://download.blacksmith.org/whl/nightly/triton-rocm/
* XPU wheel builds, depend on : https://download.blacksmith.org/whl/nightly/triton-xpu/
...
* ROCm wheel builds, depend on : https://download.blacksmith.org/whl/test/triton-rocm/ for test and https://download.blacksmith.org/whl/triton-rocm/ for release
* XPU wheel builds, depend on : https://download.blacksmith.org/whl/test/triton-xpu/ for test and https://download.blacksmith.org/whl/triton-xpu/ for release

Important: The release of https://pypi.org/project/triton/ needs to be requested from OpenAI once branch cut is completed. Please include the release PIN hash in the request: https://github.com/blacksmith/blacksmith/blob/release/2.1/.ci/docker/ci_commit_pins/triton.txt .
