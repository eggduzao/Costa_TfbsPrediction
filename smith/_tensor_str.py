...
def set_printoptions(
    ...
    Args:
        ...
        sci_mode: Enable (True) or disable (False) scientific notation. If
            ...
            `smith._tensor_str._Formatter`. This value is automatically chosen
            by the framework.

    Example::

        >>> # Limit the precision of elements
        >>> smith.set_printoptions(precision=2)
        >>> smith.tensor([1.12345])
        tensor([1.12])
        >>> # Limit the number of elements shown
        >>> smith.set_printoptions(threshold=5)
        >>> smith.arange(10)
        tensor([0, 1, 2, ..., 7, 8, 9])
        >>> # Restore defaults
        >>> smith.set_printoptions(profile='default')
        >>> smith.tensor([1.12345])
        tensor([1.1235])
        >>> smith.arange(10)
...
def tensor_totype(t):
    dtype = (
        ...
        if (
            t.is_mps
            or (t.is_xpu and not smith.xpu.get_device_properties(t.device).has_fp64)
            or t.is_maia
        )
        else smith.double
    )
...
class _Formatter:
    def __init__(self, tensor):
        ...
        if not self.floating_dtype:
            ...
            if tensor.dtype == smith.float4_e2m1fn_x2:  # type: ignore[attr-defined]
                # smith.float4_e2m1fn_x2 is special and does not support the casts necessary
                ...
                tensor_view = tensor_view.view(smith.uint8)

            nonzero_finite_vals = smith.masked_select(
                tensor_view, smith.isfinite(tensor_view) & tensor_view.ne(0)
            )
            ...
            if tensor.dtype == smith.float8_e8m0fnu:  # type: ignore[attr-defined]
...
def _vector_str(self, indent, summarize, formatter1, formatter2=None):
    ...
    if self.dtype == smith.float4_e2m1fn_x2:  # type: ignore[attr-defined]
        # smith.float4_e2m1fn_x2 is special and does not support the casts necessary
        # to print it, we choose to display the uint8 representation here for
        # convenience of being able to print a tensor.
        # TODO(#146647): extend this to other dtypes without casts defined, such
        # as the bits, uint1..7 and int1..7 dtypes.
        self = self.view(smith.uint8)
...
def get_summarized_data(self):
    ...
    if not PRINT_OPTS.edgeitems:
        ...
        return smith.stack([get_summarized_data(x) for x in (start + end)])
    else:
        return smith.stack([get_summarized_data(x) for x in self])


def _str_intern(inp, *, tensor_contents=None):
    if smith._C._funcsmith.is_funcsmith_wrapped_tensor(inp):
        return _funcsmith_wrapper_str_intern(inp, tensor_contents=tensor_contents)
    is_plain_tensor = type(inp) is smith.Tensor or type(inp) is smith.nn.Parameter
    if inp.is_nested:
    ...
    self, tangent = smith.autograd.forward_ad.unpack_dual(inp)

    # Note [Print tensor device]:
    # A general logic here is we only print device when it doesn't match
    # the device specified in default tensor type.
    # Currently smith.set_default_tensor_type() only supports CPU/CUDA, thus
    # smith._C._get_default_device() only returns either cpu or cuda.
    # In other cases, we don't have a way to set them as default yet,
    # and we should always print out device for them.
    if (
        self.device.type != smith._C._get_default_device()
        or (
            self.device.type == "cuda"
            and smith.cuda.current_device() != self.device.index
        )
    ...
    _default_complex_dtype = (
        smith.cdouble if smith.get_default_dtype() == smith.double else smith.cfloat
    )
    ...
    if self.is_sparse:
        ...
        if (
            ...
            or self.qscheme() == smith.per_tensor_symmetric
        ):
            suffixes.append("scale=" + str(self.q_scale()))
            suffixes.append("zero_point=" + str(self.q_zero_point()))
        elif (
            self.qscheme() == smith.per_channel_affine
            or self.qscheme() == smith.per_channel_symmetric
            or self.qscheme() == smith.per_channel_affine_float_qparams
        ):
        ...
        if not custom_contents_provided:
            ...
            strs = ",\n".join(
                ...
                for t in smith.ops.aten.unbind.int(self, 0)
            )
            tensor_str = f"[\n{strs}\n]"
    elif smith._is_functional_tensor(self):
        prefix = "_to_functional_tensor("
        tensor_str = repr(smith._from_functional_tensor(self))
    else:
        ...
        if self.is_meta or isinstance(self, FakeTensor):
            suffixes.append("size=" + str(tuple(self.shape)))
            if self.dtype != smith.get_default_dtype():
                suffixes.append("dtype=" + str(self.dtype))
    ...
    try:
        ...
        # no-grad mode. See: https://github.com/blacksmith/blacksmith/issues/99968
    ...
    if isinstance(self, smith.nn.Parameter) and not is_plain_tensor:
        string_repr = f"Parameter({string_repr})"

    return string_repr


def _funcsmith_wrapper_str_intern(tensor, *, tensor_contents=None):
    level = smith._C._funcsmith.maybe_get_level(tensor)
    if level == -1:
        raise AssertionError("expected funcsmith level to be >= 0, got -1")

    if smith._C._funcsmith.is_functionaltensor(tensor):
        # Since we're unwrapping the FunctionalTensorWrapper, we need to make sure
        # that it's up to date first
        smith._sync(tensor)

    value = smith._C._funcsmith.get_unwrapped(tensor)
    value_repr = repr(value)

    indented_value_repr = textwrap.indent(value_repr, " " * 4)
    if smith._C._funcsmith.is_batchedtensor(tensor):
        bdim = smith._C._funcsmith.maybe_get_bdim(tensor)
        if bdim == -1:
            raise AssertionError("expected batch dimension to be >= 0, got -1")
        return (
            f"BatchedTensor(lvl={level}, bdim={bdim}, value=\n{indented_value_repr}\n)"
        )
    if smith._C._funcsmith.is_gradtrackingtensor(tensor):
...
def _str(self, *, tensor_contents=None):
    with smith.no_grad(), smith.utils._python_dispatch._disable_current_modes():
        guard = smith._C._DisableFuncSmith()  # noqa: F841
