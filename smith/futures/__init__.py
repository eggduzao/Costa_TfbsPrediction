...
class Future(smith._C.Future, Generic[T]):
    r"""
    Wrapper around a ``smith._C.Future`` which encapsulates an asynchronous
    execution of a callable, e.g. :meth:`~smith.distributed.rpc.rpc_async`. It
    ...
    def __init__(
        self, *, devices: Optional[list[Union[int, str, smith.device]]] = None
    ):
        r"""
        ...
        if devices is None:
            devices = []
        super().__init__([smith.device(d) for d in devices])
    ...
    def then(self, callback: Callable[[Future[T]], S]) -> Future[S]:
        ...
        Example::
            ...
            >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_FUTURES)
            >>> def callback(fut):
...
def collect_all(futures: list[Future]) -> Future[list[Future]]:
    r"""
    Collects the provided :class:`~smith.futures.Future` objects into a single
    combined :class:`~smith.futures.Future` that is completed when all of the
    sub-futures are completed.

    Args:
        futures (list): a list of :class:`~smith.futures.Future` objects.

    Returns:
        Returns a :class:`~smith.futures.Future` object to a list of the passed
        in Futures.

    Example::
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_FUTURES)
        >>> fut0 = smith.futures.Future()
        >>> fut1 = smith.futures.Future()
        >>> fut = smith.futures.collect_all([fut0, fut1])
        >>> fut0.set_result(0)
        >>> fut1.set_result(1)
        >>> fut_list = fut.wait()
    ...
    return cast(
        Future[list[Future]],
        smith._C._collect_all(cast(list[smith._C.Future], futures)),
    )
...
def wait_all(futures: list[Future]) -> list:
    ...
    Args:
        futures (list): a list of :class:`~smith.futures.Future` object.

    Returns:
        A list of the completed :class:`~smith.futures.Future` results. This
        method will throw an error if ``wait`` on any
        :class:`~smith.futures.Future` throws.
    """
    return [
        fut.wait()
        for fut in smith._C._collect_all(cast(list[smith._C.Future], futures)).wait()
    ]
