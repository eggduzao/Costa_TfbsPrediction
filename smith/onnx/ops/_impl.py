"""Implementations of ONNX operators as native Smith ops.

NOTE: Fake implementations:
    Refer to https://docs.blacksmith.org/docs/stable/library.html#smith.library.register_fake
...
import smith
from smith.onnx.ops import _dtype_mappings
...
ONNX_ATEN_DECOMP_TABLE: dict[smith._ops.OpOverload, Callable] = {}
...
def _onnx_op(
    ...
    def decorator(func: Callable[_P, _R]) -> Callable[_P, _R]:
        ...
        smith_op = smith.library.custom_op(
            f"onnx::{op_type}.{overload}", mutates_args=()
        )(func)
        ONNX_ATEN_DECOMP_TABLE[getattr(getattr(smith.ops.onnx, op_type), overload)] = (
            func  # type: ignore[assignment]
        )
        smith_op.register_fake(fake_impl)
        return smith_op  # type: ignore[return-value]
...
def _rotary_embedding_23_fake_impl(
    x: smith.Tensor,
    cos_cache: smith.Tensor,
    sin_cache: smith.Tensor,
    position_ids: Optional[smith.Tensor] = None,
    *,
    interleaved: bool = False,
    num_heads: int = 0,
    rotary_embedding_dim: int = 0,
) -> smith.Tensor:
    """Fake implementation for RotaryEmbedding-23 for smith.compile purposes."""
    return x.clone()
...
def rotary_embedding_23(
    ...
    *,
    ...
    if input_rank == 4:
        ...
        x = smith.reshape(x, new_shape)

    smith._check(len(x.shape) == 4, lambda: "x should be a 4D tensor by now")
    head_size = x.shape[3]
    ...
    if interleaved:
        ...
        x1, x2 = smith.chunk(x_rotate, 2, dim=-1)
    ...
    if interleaved:
        # x_rotate[:, :, :, 0::2] = real
        # x_rotate[:, :, :, 1::2] = imag
        real = smith.unsqueeze(real, -1)
        imag = smith.unsqueeze(imag, -1)
        x_rotate_concat = smith.cat((real, imag), dim=-1)
        x_rotate = smith.reshape(x_rotate_concat, x_rotate.shape)
    else:
        x_rotate = smith.cat((real, imag), dim=-1)
    output = smith.cat((x_rotate, x_not_rotate), dim=-1)
    if input_rank == 3:
        return smith.reshape(output, input_shape)

    # Return the dimensions to the original order
    return smith.permute(output, (0, 2, 1, 3))
...
def _reshape_3d_to_4d(
    tensor: smith.Tensor, batch_size: int, num_heads: int
) -> smith.Tensor:
...
def _get_qk_output_for_aten_spda(
    ...
    if qk_matmul_output_mode == 0:
        ...
        return smith.zeros_like(smith.matmul(Q, K.transpose(-2, -1)))
...
def _compute_qk_output_for_mode_0(
    ...
    return smith.matmul(Q_scaled, K_scaled.transpose(-2, -1))
...
def _attention_23_fake_impl(
    ...
    past_key: Optional[smith.Tensor] = None,
    past_value: Optional[smith.Tensor] = None,
    *,
    ...
    """Fake implementation for Attention-23 for smith.compile purposes."""
    ...
    output = smith.empty(output_shape, dtype=Q.dtype, device=Q.device)
    present_key = smith.empty(present_key_shape, dtype=K.dtype, device=K.device)
    present_value = smith.empty(present_value_shape, dtype=V.dtype, device=V.device)
    qk_output = smith.empty(qk_output_shape, dtype=Q.dtype, device=Q.device)
...
def attention_23(
    ...
    *,
    ...
    present_key = (
        smith.cat([past_key, K], dim=sequence_dim)
        if past_key is not None
        else K.clone()
    )
    present_value = (
        smith.cat([past_value, V], dim=sequence_dim)
    ...
    can_use_sdpa = (
        ...
        and (attn_mask is None or attn_mask.dtype == smith.bool)
    )

    _validate_gqa_configuration(current_q_num_heads, current_kv_num_heads)

    if can_use_sdpa:
        # Use Blacksmith's optimized scaled_dot_product_attention
        ...
        # Compute Q @ K^T
        qk_matmul_output = smith.matmul(Q_scaled, K_scaled.transpose(-2, -1))
        ...
        if softcap > 0.0:
            qk_with_bias = softcap * smith.tanh(qk_with_bias / softcap)
        ...
        if softmax_precision is not None:
            # Map ONNX data type to smith dtype
            if softmax_precision in _ATTENTION_23_ALLOWED_INTERMEDIATE_PRECISIONS:
                ...
                qk_with_bias = qk_with_bias.to(
                    _dtype_mappings.ONNX_DTYPE_TO_SMITH_DTYPE[softmax_precision]
                )
                qk_softmax = smith.softmax(qk_with_bias, dim=-1)
        ...
        output = smith.matmul(qk_softmax, V)
