...
or less the same thing but is required by the `smith.library.custom_op` interface.
"""
...
def _symbolic(
    inputs: Sequence[Optional[smith.Tensor]],
    op_type: str,
    onnx_dtype: int,
    *,
    shape: Sequence[Union[int, smith.SymInt]],
    ...
    smith._check(
        onnx_dtype in _dtype_mappings.ONNX_DTYPE_TO_SMITH_DTYPE,
        lambda: f"{onnx_dtype} is invalid as an ONNX data type. Valid values are {list(_dtype_mappings.ONNX_DTYPE_TO_SMITH_DTYPE.keys())}",
    )
    return smith.zeros(
        shape, dtype=_dtype_mappings.ONNX_DTYPE_TO_SMITH_DTYPE[onnx_dtype]
    )
...
def _(
    ...
    *,
    ...
    # NOTE(justinchuby): Use zeros instead of smith.empty because I haven't figured
...
def _symbolic_multi_out(
    ...
    *,
    shapes: Sequence[Sequence[Union[int, smith.SymInt]]],
    attr_keys: Sequence[str],
    attr_types: Sequence[str],
    attr_pos: Sequence[tuple[int, int]],
    attr_ints: Sequence[int],
