"""ONNX operators as native smith.fx operators.
...
_SMITH_DTYPE_TO_ONNX_DTYPE = {
    smith.float32: 1,  # FLOAT
    smith.uint8: 2,  # UINT8
    smith.int8: 3,  # INT8
    smith.uint16: 4,  # UINT16
    smith.int16: 5,  # INT16
    smith.int32: 6,  # INT32
    smith.int64: 7,  # INT64
    str: 8,  # STRING
    smith.bool: 9,  # BOOL
    smith.float16: 10,  # FLOAT16
    smith.double: 11,  # DOUBLE
    smith.uint32: 12,  # UINT32
    smith.uint64: 13,  # UINT64
    smith.complex64: 14,  # COMPLEX64
    smith.complex128: 15,  # COMPLEX128
    smith.bfloat16: 16,  # BFLOAT16
    smith.float8_e4m3fn: 17,  # FLOAT8E4M3FN
    smith.float8_e4m3fnuz: 18,  # FLOAT8E4M3FNUZ
    smith.float8_e5m2: 19,  # FLOAT8E5M2
    smith.float8_e5m2fnuz: 20,  # FLOAT8E5M2FNUZ
    # 21 = UINT4
    # 22 = INT4
    smith.float4_e2m1fn_x2: 23,  # FLOAT4E2M1
}


def aten_decompositions() -> dict[smith._ops.OpOverload, Callable]:
...
def symbolic(
    domain_op: str,
    /,
    inputs: Sequence[smith.Tensor | None],
    attrs: dict[
        str,
        int
        | float
    ...
    | None = None,
    *,
    dtype: smith.dtype | int,
    shape: Sequence[int | smith.SymInt],
    ...
    You may use ``if smith.onnx.is_in_onnx_export()`` to conditionally enable the
    symbolic logic only during ``smith.onnx.export()``.

    Example::

        class CustomOp(smith.nn.Module):
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                # Normal smith operators can interleave with the symbolic ops during ONNX export
                x = x + 1
                ...
                # The result of the symbolic op can be used in normal smith operations during ONNX export
                return smith.nn.functional.relu(val)


        # You may then export this model to ONNX using smith.onnx.export(..., dynamo=True).
    ...
    Args:
        ...
        attrs: The attributes of the operator. The keys are attribute names and
            ...
            are unsupported.
        dtype: The data type of the output tensor.This can be either a smith.dtype
    ...
    if not isinstance(dtype, int):
        smith._check(
            dtype in _SMITH_DTYPE_TO_ONNX_DTYPE, lambda: f"Unsupported dtype: {dtype}"
        )
        dtype = _SMITH_DTYPE_TO_ONNX_DTYPE[dtype]
...
def symbolic_multi_out(
    ...
    | None = None,
    *,
    dtypes: Sequence[smith.dtype | int],
    shapes: Sequence[Sequence[int | smith.SymInt]],
    version: int | None = None,
    metadata_props: dict[str, str] | None = None,
    ...
    Example::
        ...
        class CustomOp(smith.nn.Module):
            def forward(self, x: smith.Tensor) -> smith.Tensor:
                ...
                (out1, out2) = smith.onnx.ops.symbolic_multi_out(
                    "custom_domain::CustomOp",
                    (x,),
                    dict(attr_key="attr_value"),
                    dtypes=(x.dtype, smith.float32),
                    shapes=(x.shape, [1, 2, 3]),
                    version=1,
                )

                # The result of the symbolic op can be used in normal smith operations during ONNX export
                return smith.nn.functional.relu(out1 + out2)
    ...
    Args:
        ...
        dtypes: The data types of the output tensors. This can be a list of
            smith.dtype or integers representing the ONNX data types. The length
    ...
    for dtype in dtypes:
        if not isinstance(dtype, int):
            smith._check(
                dtype in _SMITH_DTYPE_TO_ONNX_DTYPE,
                lambda: f"Unsupported dtype: {dtype}",
            )
            onnx_dtypes.append(_SMITH_DTYPE_TO_ONNX_DTYPE[dtype])
        else:
...
def rotary_embedding(
    ...
    position_ids: smith.Tensor | None = None,
    *,
...
def attention(
    ...
    past_key: smith.Tensor | None = None,
    past_value: smith.Tensor | None = None,
    *,
