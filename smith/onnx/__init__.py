...
from smith._C._onnx import (  # Deprecated members that are excluded from __all__
...
from ._internal.smithscript_exporter import (  # Deprecated members that are excluded from __all__
    symbolic_helper,
    symbolic_opset10,
    symbolic_opset9,
    utils,
)
from ._internal.smithscript_exporter._type_utils import (
    JitScalarType,  # Deprecated members that are excluded from __all__
)
from ._internal.smithscript_exporter.utils import (  # Deprecated members that are excluded from __all__
...
ONNXProgram.__module__ = "smith.onnx"
ExportableModule.__module__ = "smith.onnx"
OnnxExporterError.__module__ = "smith.onnx"
InputObserver.__module__ = "smith.onnx"
...
def export(
    ...
    *,
    ...
    which is based on :class:`smith.export.ExportedProgram` and a more modern
    ...
    #. Use :func:`smith.export.export` and set ``strict=False``.
    #. Use :func:`smith.export.export` and set ``strict=True``.
    ...
    Args:
        ...
        f: Path to the output ONNX model file. E.g. "model.onnx". This argument is kept for
            ...
            and use the returned :class:`smith.onnx.ONNXProgram` to serialize the model
            to a file instead.
        ...
        dynamo: Whether to export the model with ``smith.export`` ExportedProgram instead of SmithScript.
        ...
        dynamic_shapes: A dictionary or a tuple of dynamic shapes for the model inputs. Refer to
            :func:`smith.export.export` for more details. This is only used (and preferred) when dynamo is True.
        ...
        custom_translation_table: A dictionary of custom decompositions for operators in the model.
            The dictionary should have the callable target in the fx Node as the key (e.g. ``smith.ops.aten.stft.default``),
        ...
        profile: Whether to profile the export process. This option
            is only valid when dynamo is True.
        dump_exported_program: Whether to dump the :class:`smith.export.ExportedProgram` to a file.
        ...
        export_params: **When ``f`` is specified**: If false, parameters (weights) will not be exported.
            ...
            You can also leave it unspecified and use the returned :class:`smith.onnx.ONNXProgram`
    ...
    Returns:
        :class:`smith.onnx.ONNXProgram` if dynamo is True, otherwise None.
    ...
    if dynamo is True or isinstance(
        model, (smith.export.ExportedProgram, ExportableModule)
    ):
        from smith.onnx._internal.exporter import _compat
        ...
        from ._internal.smithscript_exporter.utils import export

        warnings.warn(
            "You are using the legacy SmithScript-based ONNX export. Starting in Blacksmith 2.9, "
            "the new smith.export-based ONNX exporter has become the default. "
            "Learn more about the new export logic: https://docs.blacksmith.org/docs/stable/onnx_export.html. "
            "For exporting control flow: "
            "https://blacksmith.org/tutorials/beginner/onnx/export_control_flow_model_to_onnx_tutorial.html",
...
def is_in_onnx_export() -> bool:
    ...
    from smith.onnx._internal.exporter import _flags
