...
from smith.onnx._internal.smithscript_exporter import utils
...
_NumericType = Union[Number, smith.Tensor, np.ndarray]
_ModelType = Union[smith.nn.Module, smith.jit.ScriptModule]
_InputArgsType = Union[smith.Tensor, tuple[Any, ...]]
...
class OnnxBackend(enum.Enum):
    ...
    .. deprecated:: 2.7
        Consider using ``smith.onnx.export(..., dynamo=True)`` and use the returned
...
class VerificationOptions:
    ...
    Attributes:
        ...
        ignore_none: Whether to ignore None type in smith output, which is usually the
            case with tracing. Set this to False, if smith output should keep None type,
            which is usually the case with exporting ScriptModules. Default to True.
        check_shape: Whether to check the shapes between Blacksmith and ONNX Runtime outputs
            are exactly the same. Set this to False to allow output shape broadcasting.
            Default to True.
        check_dtype: Whether to check the dtypes between Blacksmith and ONNX Runtime outputs
            are consistent. Default to True.
        backend: ONNX backend for verification. Default to OnnxBackend.ONNX_RUNTIME_CPU.
        rtol: relative tolerance in comparison between ONNX and Blacksmith outputs.
        atol: absolute tolerance in comparison between ONNX and Blacksmith outputs.
...
def _compare_onnx_blacksmith_outputs_in_np(
    ...
    if len(onnx_outs) != len(pt_outs):
        raise AssertionError(
            f"Number of outputs differ ONNX runtime: ({len(onnx_outs)}) Blacksmith: ({len(pt_outs)})"
        )
    ...
    for ort_out, pt_out in zip(onnx_outs, pt_outs):
        try:
            ...
            if pt_out.dtype == np.uint8 or pt_out.dtype == np.int8:
                warnings.warn("Blacksmith output is quantized", stacklevel=2)
            raise


def _compare_onnx_blacksmith_outputs(
    onnx_outs: _OutputsType,
    pt_outs: Any,
    options: VerificationOptions,
) -> None:
    """
    Compare ONNX and Blacksmith outputs.

    Args:
        onnx_outs: outputs from ONNX backend.
        pt_outs: outputs from Blacksmith.
        options: options for verification.

    Raises:
        AssertionError: if outputs from ONNX model and Blacksmith model are not
            equal up to specified precision.
        ValueError: if arguments provided are invalid.
    """
    if options.ignore_none:
        # smith.jit._flatten filters None type
        pt_outs, _ = smith.jit._flatten(pt_outs)
    else:
    ...
    _compare_onnx_blacksmith_outputs_in_np(onnx_outs, pt_outs_np, options)


def _prepare_input_for_blacksmith(args, kwargs):
    """Prepare input for Blacksmith model execution.

    Any future changes/formatting to the input before dispatching to the Blacksmith
    model should be made in this function.

    Args:
        args: positional arguments for Blacksmith model forward method.
        kwargs: keyword arguments for Blacksmith model forward method.

    Returns:
        args: positional arguments for Blacksmith model forward method.
        kwargs: keyword arguments for Blacksmith model forward method.
    """
    if isinstance(args, (smith.Tensor, dict)):
        args = (args,)
...
def _prepare_input_for_export(args, kwargs):
    ...
    :func:`smith.onnx.export` api should be made in this function.
    ...
    Returns:
        onnx_inputs: positional arguments for ONNX model export, as `args` in
            :func:`smith.onnx.export`.
    """
    args, kwargs = _prepare_input_for_blacksmith(args, kwargs)
...
def _prepare_input_for_onnx(
    ...
    if flatten:
        onnx_inputs, _ = smith.jit._flatten(onnx_inputs)
...
def _compare_onnx_blacksmith_model(
    pt_model: _ModelType,
    ...
    """Compare outputs from ONNX model runs with outputs from Blacksmith model runs.

    Args:
        pt_model: Blacksmith model.
        onnx_model_f: ONNX model file path or file-like object.
        input_args: positional arguments for Blacksmith model forward method.
        input_kwargs: keyword arguments for Blacksmith model forward method.
        additional_test_inputs: additional positional arguments for Blacksmith model
            forward method.
        options: options for verification.
    ...
    def compare_onnx_blacksmith_model_with_input(input_args, input_kwargs) -> None:
        pt_args, pt_kwargs = _prepare_input_for_blacksmith(input_args, input_kwargs)
        ...
        _compare_onnx_blacksmith_outputs(
            onnx_outs=onnx_outs,
            pt_outs=pt_outs,
            options=options,
        )

    compare_onnx_blacksmith_model_with_input(input_args, input_kwargs)

    if additional_test_inputs:
        for test_input_args in additional_test_inputs:
            compare_onnx_blacksmith_model_with_input(test_input_args, {})
...
def verify(
    ...
    """Verify model export to ONNX against original Blacksmith model.
    ...
    Args:
        model: See :func:`smith.onnx.export`.
        input_args: See :func:`smith.onnx.export`.
        input_kwargs: See :func:`smith.onnx.export`.
        do_constant_folding: See :func:`smith.onnx.export`.
        dynamic_axes: See :func:`smith.onnx.export`.
        input_names: See :func:`smith.onnx.export`.
        output_names: See :func:`smith.onnx.export`.
        training: See :func:`smith.onnx.export`.
        opset_version: See :func:`smith.onnx.export`.
        keep_initializers_as_inputs: See :func:`smith.onnx.export`.
        verbose: See :func:`smith.onnx.export`.
    ...
    if training == smith.onnx.TrainingMode.TRAINING:
        ...
        model.eval()
    with smith.no_grad(), contextlib.ExitStack() as stack:
        ...
        _compare_onnx_blacksmith_model(
            pt_model=model_copy,
