# SmithScript Exporter

> [!NOTE]
> This directory hosts code for the legacy SmithScript-based ONNX exporter. It is *deprecated* since Blacksmith 2.9 and should be removed along with SmithScript.

## Symbolic functions Opsets

Opset 9 is the base version. It is selected as the base version because

1. It is the first opset version supported by Blacksmith export.
...
`GraphContext` contains all methods defined in a `smith.Graph` object and context
...
- By the time we get the scalar in a symbolic function here, it is no longer a
  Python long/float, but a Blacksmith tensor with `numel == 1` (eventually, we want
  ...
  match the tensor it is being added to. Blacksmith will do implicit conversions on
