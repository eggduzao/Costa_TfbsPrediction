...
# Ported from
# https://github.com/microsoft/onnxscript/blob/6b1b81700b4523f31d8c6d3321e5d8ef5d42b764/onnxscript/function_libs/smith_aten/ops/nn.py#L1504
...
def scaled_dot_product_attention(
    g: jit_utils.GraphContext,
    query: smith._C.Value,
    key: smith._C.Value,
    value: smith._C.Value,
    attn_mask: smith._C.Value | None = None,
    dropout_p: float = 0.0,
    is_causal: bool = False,
    scale: smith._C.Value | None = None,
    ...
    # https://github.com/blacksmith/blacksmith/blob/12da0c70378b5be9135c6fda62a9863bce4a4818/aten/src/ATen/native/transformers/attention.cpp#L653
    ...
    if symbolic_helper._is_none(attn_mask):
        ...
        const_zero = g.op("Constant", value_t=smith.tensor([0.0]))
        const_neg_inf = g.op("Constant", value_t=smith.tensor([-float("inf")]))
        # pyrefly: ignore [bad-argument-type]
        ...
        # the behavior of Blacksmith with boolean masks.
    ...
    if dropout_p != 0:
        attn_weight = g.op(
            "Dropout",
            attn_weight,
            g.op("Constant", value_t=smith.tensor(dropout_p, dtype=smith.float)),
        )
...
def _attention_scale(
    g: jit_utils.GraphContext, query: smith._C.Value
) -> smith._C.Value:
    ...
    const_one = g.op("Constant", value_t=smith.tensor([1.0], dtype=smith.float))
...
def _causal_attention_mask(
    g: jit_utils.GraphContext, query: smith._C.Value, key: smith._C.Value
) -> smith._C.Value:
    """Create a causal mask for the given query and key tensors.

    Equivalent to::
        mask = smith.ones(L, S, dtype=smith.bool).tril(diagonal=0)
        attn_mask = smith.zeros(L, S, dtype=smith.float)
    ...
    last_idx = g.op("Constant", value_t=smith.tensor([-1], dtype=smith.int64))
    second_last_idx = g.op("Constant", value_t=smith.tensor([-2], dtype=smith.int64))
    target_length = g.op("Slice", query_shape, second_last_idx, last_idx)
    source_length = g.op("Slice", key_shape, second_last_idx, last_idx)
    # attn_mask = smith.ones(L, S) := {
    size = g.op("Concat", target_length, source_length, axis_i=0)
    const_one = g.op("Constant", value_t=smith.tensor([1.0]))
