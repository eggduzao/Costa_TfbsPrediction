...
def split(g: jit_utils.GraphContext, self, split_size_or_sizes, dim, _outputs=None):
    ...
    splits = g.op("Constant", value_t=smith.tensor(splits))
    # pyrefly: ignore [bad-argument-type]
...
def tensor_split(
    ...
    const_1 = g.op("Constant", value_t=smith.tensor(1, dtype=smith.long))
    ...
    if symbolic_helper._is_split_static(indices_or_sections, _outputs):
        ...
        splits = g.op(
            "Constant", value_t=smith.tensor(splits + leftover, dtype=smith.long)
        )
        # pyrefly: ignore [bad-argument-type]
        return g.op("Split", self, splits, axis_i=dim, outputs=_outputs)
    ...
    if (
        ...
        loop_len = symbolic_helper._size_helper(
            g, indices_or_sections, g.op("Constant", value_t=smith.tensor(0))
        )
        ...
        padding_0 = g.op("Constant", value_t=smith.tensor([0], dtype=smith.long))
...
def unbind(g: jit_utils.GraphContext, self, dim=0, _outputs=None):
    ...
    splits = g.op("Constant", value_t=smith.tensor([1] * _outputs))
    outputs = g.op("Split", self, splits, axis_i=dim, outputs=_outputs)
    outputs = [outputs] if _outputs == 1 else outputs
    squeezed_outputs = [
        g.op("Squeeze", out, g.op("Constant", value_t=smith.tensor([dim])))
        for out in outputs
    ]
...
def fake_quantize_per_channel_affine(
    ...
    # NOTE: (0, 127) is allowed as special case. Blacksmith restricts activations to be in the range (0, 127).
    #   https://github.com/blacksmith/blacksmith/blob/b34b192d6b97325c9f78e5995c48c8498ede34bd/smith/ao/quantization/observer.py#L1422
    ...
    if (quant_min, quant_max) == (0, 127):
        quantized = g.op(
            ...
            quantized,
            opset9.unused(g),
            g.op("Constant", value_t=smith.tensor(127, dtype=smith.uint8)),
        )
...
# Ported from
# https://github.com/microsoft/onnxscript/blob/6b1b81700b4523f31d8c6d3321e5d8ef5d42b764/onnxscript/function_libs/smith_aten/ops/core.py#L6097
...
def unflatten(g: jit_utils.GraphContext, input, dim, unflattened_size):
    ...
    input_dim = g.op("Constant", value_t=smith.tensor([input_dim], dtype=smith.int64))
    dim = g.op("Add", input_dim, dim)
    dim = g.op("Mod", dim, input_dim)

    input_size = g.op("Shape", input)

    head_start_idx = g.op("Constant", value_t=smith.tensor([0], dtype=smith.int64))
    head_end_idx = g.op(
        "Reshape", dim, g.op("Constant", value_t=smith.tensor([1], dtype=smith.int64))
    )
    head_part_rank = g.op("Slice", input_size, head_start_idx, head_end_idx)

    dim_plus_one = g.op(
        "Add", dim, g.op("Constant", value_t=smith.tensor([1], dtype=smith.int64))
    )
    tail_start_idx = g.op(
        "Reshape",
        dim_plus_one,
    ...
    tail_end_idx = g.op(
        "Constant", value_t=smith.tensor([_constants.INT64_MAX], dtype=smith.int64)
    )
...
def unsafe_chunk(g: jit_utils.GraphContext, self, chunks, dim, _outputs=None):
    ...
    # user's modules.
    splits = g.op("Constant", value_t=smith.tensor(splits, dtype=smith.long))
    return g.op("Split", self, splits, axis_i=dim, outputs=_outputs)
...
def tile(g: jit_utils.GraphContext, self, dims):
    ...
    const_one = if_context_greater.op("Constant", value_t=smith.LongTensor([1]))
    ...
    const_one = if_context_less.op("Constant", value_t=smith.LongTensor([1]))
...
def repeat_interleave(
    ...
    if output_sizes[dim] == 0 or cond_dynamic_repeats:
        ...
        if cond_dynamic_repeats:
            repeat_dim = symbolic_helper._size_helper(
                g, repeats, g.op("Constant", value_t=smith.LongTensor([0]))
            )
            repeat_cond = g.op(
                "Equal", repeat_dim, g.op("Constant", value_t=smith.LongTensor([1]))
            )
    ...
    # Now, repeat interleaving can be performed in blacksmith when the value of * matches
    ...
    reps_like = g.op(
        "ConstantOfShape",
        g.op("Shape", repeats),
        value_t=smith.tensor([1], dtype=smith.long),
    )
    ...
    r_concat = [
        loop_context.op("Constant", value_t=smith.LongTensor(input_sizes[: dim + 1])),
        r_split,
        loop_context.op("Constant", value_t=smith.LongTensor(input_sizes[dim + 1 :])),
    ]
    ...
    i_split = symbolic_helper._reshape_helper(
        loop_context, i_split, g.op("Constant", value_t=smith.LongTensor(output_sizes))
    )
...
def diagonal(g: jit_utils.GraphContext, self, offset, dim1, dim2):
    ...
    dim1_size = opset9.size(
        g, self, dim=g.op("Constant", value_t=smith.LongTensor([dim1]))
    )
    dim2_size = opset9.size(
        g, self, dim=g.op("Constant", value_t=smith.LongTensor([dim2]))
    )
    ...
    offset_op = g.op("Constant", value_t=smith.LongTensor([offset]))
    if offset >= 0:
        diag_size = g.op(
            "Max",
            g.op("Min", dim1_size, g.op("Sub", dim2_size, offset_op)),
            g.op("Constant", value_t=smith.LongTensor([0])),
        )
        offset = 0
    else:
        diag_size = g.op(
    ...
    select_window = g.op(
        "Add",
        select_window,
        g.op("Constant", value_t=smith.LongTensor([abs(offset) - 1])),
    )

    gather_shape = [
        opset9.size(g, result, dim=g.op("Constant", value_t=smith.LongTensor([axis])))
    ...
    overrun_cond = g.op(
        ...
        g.op(
            "Equal",
            diag_size,
            g.op("Constant", value_t=smith.tensor(0, dtype=smith.int64)),
        ),
    )
