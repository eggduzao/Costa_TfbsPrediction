...
def _aten_max_pool_with_indices_onnx(
    ...
    ends = g.op("Constant", value_t=smith.tensor(n_dims_one))
    starts = g.op("Constant", value_t=smith.tensor(n_dims_zero))
    axes = g.op("Constant", value_t=smith.tensor(n_dims_axes))
    ...
    if self_rank == unbatched_rank:
        pool_result = g.op(
            "Squeeze", pool_result, value_t=smith.tensor([0], dtype=smith.int64)
        )
        indices = g.op("Squeeze", indices, value_t=smith.tensor([0], dtype=smith.int64))
...
def _slice(
    ...
    axes: list | smith.Tensor | smith._C.Value,
    starts: list | smith.Tensor | smith._C.Value,
    ends: list | smith.Tensor | smith._C.Value,
    steps: list | smith.Tensor | smith._C.Value | None = None,
):
    def is_none_value(value):
        if value is None:
            return True
        return (
            isinstance(value, smith._C.Value)
    ...
    def get_const_value(list_or_value):
        if isinstance(list_or_value, (list, smith.Tensor)):
...
def embedding_bag(
    ...
    if offsets_dim_0 is not None:
        if include_last_offset:
            ...
            offsets_extended = [
                offsets,
                g.op("Constant", value_t=smith.tensor([sys.maxsize])),
            ]
        ...
        for i in range(offset_len):
            start_ = symbolic_helper._unsqueeze_helper(
                g,
                opset9.select(g, offsets_extended, smith.tensor(0), smith.tensor(i)),
                [0],
            )
            end_ = symbolic_helper._unsqueeze_helper(
                g,
                opset9.select(
                    g, offsets_extended, smith.tensor(0), smith.tensor(i + 1)
                ),
                [0],
            )
            axes_ = g.op("Constant", value_t=smith.tensor([0]))
...
def fake_quantize_per_tensor_affine(
    ...
    # NOTE: (0, 127) is a special case. Blacksmith restricts activations to be in the range (0, 127).
...
def nan_to_num(g: jit_utils.GraphContext, input, nan, posinf, neginf):
    ...
    nan_result = g.op(
        "Where",
        nan_cond,
        g.op("Constant", value_t=smith.tensor([nan], dtype=input_dtype)),
        input,
    )

    # For None values of posinf, neginf we use the greatest/lowest finite
    # value representable by input's dtype.
    finfo = smith.finfo(input_dtype)
    if posinf is None:
        posinf = finfo.max
    posinf_cond = opset9.logical_and(
        g,
        isinf(g, nan_result),
        opset9.gt(g, nan_result, g.op("Constant", value_t=smith.LongTensor([0]))),
    )
    nan_posinf_result = g.op(
        "Where",
        posinf_cond,
        g.op("Constant", value_t=smith.tensor([posinf], dtype=input_dtype)),
        nan_result,
    )
    ...
    neginf_cond = opset9.logical_and(
        ...
        opset9.lt(
            g, nan_posinf_result, g.op("Constant", value_t=smith.LongTensor([0]))
        ),
    )
    return g.op(
        "Where",
        neginf_cond,
        g.op("Constant", value_t=smith.tensor([neginf], dtype=input_dtype)),
        nan_posinf_result,
    )


# Quantized symbolics ---------------------------------------------------------
# https://github.com/blacksmith/blacksmith/wiki/Blacksmith-ONNX-exporter#quantized-model-export
