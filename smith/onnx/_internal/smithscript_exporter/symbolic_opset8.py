    ...
    def symbolic_fn(g, input, output_size, *args):
        ...
        if symbolic_helper._is_value(output_size):
            return symbolic_helper._unimplemented(
                name, "smith._C.Value (output_size) indexing"
            )
        if scales is None:
            scales = [
                1.0
...
def _cast_to_type(g: jit_utils.GraphContext, input, to_type):
    if to_type is None:
        return input
    return g.op("Cast", input, to_i=symbolic_helper.cast_blacksmith_to_onnx[to_type])
...
def full(
    ...
    if symbolic_helper._is_value(const_value):
        ...
        return opset9.add(g, tmp, value, g.op("Constant", value_t=smith.tensor(1)))
    else:
...
def repeat(g: jit_utils.GraphContext, self, repeats):
    ...
    if self.isCompleteTensor():
        ...
        if diff_dims > 0:
            self = opset9.view(
                g, self, g.op("Constant", value_t=smith.tensor([1] * diff_dims + sizes))
            )
