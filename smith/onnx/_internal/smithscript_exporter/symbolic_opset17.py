...
def layer_norm(
    ...
    if symbolic_helper._is_none(weight):
        weight_value = smith.ones(normalized_shape, dtype=dtype)
        weight = g.op("Constant", value_t=weight_value)
    if symbolic_helper._is_none(bias):
        bias_value = smith.zeros(normalized_shape, dtype=dtype)
...
def stft(
    ...
    """Associates `smith.stft` with the `STFT` ONNX operator.
    Note that smith.stft calls _VF.stft, without centering or padding options.
    Hence, this function does not contain these two arguments.
    See smith.stft source code for more info.
    ...
    Returns:
        op: Operator for smith.stft associated with STFT (ONNX)
    """
    # Checks
    if return_complex:
    ...
    frame_step_const = g.op(
        "Constant", value_t=smith.tensor(frame_step_value, dtype=smith.int64)
    )
    frame_length_const = g.op(
        "Constant", value_t=smith.tensor(n_fft, dtype=smith.int64)
    )
    ...
    if n_win is not None:
        ...
        if n_win < n_fft:
            ...
            left_win = g.op("Constant", value_t=smith.zeros(left))
            right_win = g.op("Constant", value_t=smith.zeros(right))
            # pyrefly: ignore [bad-argument-type]
    ...
    if symbolic_helper._is_none(window):
        if win_length:
            ...
            smith_window = smith.hstack(
                (smith.zeros(left), smith.ones(win_length), smith.zeros(right))
            )
        else:
            # Rectangle window
            smith_window = smith.ones(n_fft)
        if smith_window.shape[0] != n_fft:
            raise AssertionError(
                f"smith_window.shape[0]={smith_window.shape[0]} != n_fft={n_fft}"
            )
        window = g.op("Constant", value_t=smith_window)
    window = g.op(
        "Cast",
        # pyrefly: ignore [bad-argument-type]
        window,
    ...
    # Transpose to mimic smith.stft's behavior
    ...
    # Normalize, if needed
    if normalized:
        sqrt_nfft = smith.sqrt(smith.tensor(n_fft, dtype=signal.type().dtype()))
