...
__all__ = [
    ...
    "cast_blacksmith_to_onnx",
    "check_training_mode",
    "dequantize_helper",
    "is_complex_value",
    "parse_args",
    "blacksmith_name_to_type",
    "quantize_helper",
    "quantized_args",
    "requantize_bias_helper",
    "scalar_name_to_blacksmith",
    "scalar_type_to_onnx",
    "scalar_type_to_blacksmith_type",
]
...
from smith.onnx._internal.smithscript_exporter import _type_utils, jit_utils, utils
...
def _maybe_get_const(
    value: _C.Value | smith.Tensor | Number | Sequence | None,
...
def _maybe_get_scalar(value):
    ...
    if isinstance(value_t, smith.Tensor) and value_t.shape == ():
        return value_t
    return value
...
def _unpack_quantized_tensor(tuple_value: _C.Value) -> tuple[_C.Value, ...]:
    ...
    if not _is_tuple_construct(tuple_value):
        raise errors.SymbolicValueError(
            ...
            f"`{tuple_node.kind()}`. Please create an issue on {_constants.BLACKSMITH_GITHUB_ISSUES_URL}",
            tuple_value,
        )
...
def parse_args(
    *arg_descriptors: _ValueDescriptor,
    ...
    """A decorator which converts args from smith._C.Value to built-in types.
    ...
    Args:
        arg_descriptors: list of str, where each element is
            ...
            "v": no conversion, keep smith._C.Value.
            "i": int
            "is": list of int
            "f": float
            "fs": list of float
    ...
    def decorator(
        ...
        def wrapper(g: _U, *args: _P.args, **kwargs: _P.kwargs) -> _T:
            ...
            FILE_BUG_MSG = (
                ...
                "https://github.com/blacksmith/blacksmith/issues/new?template=bug-report.yml to report this bug."
            )
    ...
    def decorator(fn):
        ...
        def wrapper(g, *args, **kwargs):
            ...
            if scale is not None:
                _scale = g.op("Constant", value_t=smith.tensor(scale))
            else:
                _scale = None
            if zero_point is not None:
                _zero_point = g.op("Constant", value_t=smith.tensor(zero_point))
            else:
                _zero_point = None
...
def _scalar(x: Any) -> Number | None:
    ...
    if isinstance(x, smith.Tensor) and x.shape == ():
        return x.item()
    return None
...
def _onnx_unsupported(op_name: str, value: _C.Value | None = None) -> NoReturn:
    message = (
        ...
        f"on Blacksmith GitHub: {_constants.BLACKSMITH_GITHUB_ISSUES_URL}"
    )
...
def _type_promote_from_values(*args) -> _type_utils.JitScalarType:
    ...
    for t in jit_types:
        new_dtype = smith.promote_types(new_dtype, t.dtype())  # type: ignore[union-attr]
...
def _select_helper(g: jit_utils.GraphContext, self, dim, index, apply_reshape=True):
    ...
    if not _is_value(index_const):
        ...
        index = g.op("Constant", value_t=smith.LongTensor([index_const]))
    elif index_dim is not None and apply_reshape:
        if index_dim == 0:
            # Index is a scalar. Reshape it to a size 1 tensor.
            index = _reshape_helper(
                g, index, g.op("Constant", value_t=smith.LongTensor([1]))
            )
...
def _slice_helper(
    ...
    if g.opset <= 9:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import (
            _slice as _slice9,
        )

        return _slice9(g, input, axes, starts, ends)
    else:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset10 import (
            _slice as _slice10,
        )
...
def _generate_wrapped_number(g: jit_utils.GraphContext, scalar):
    """Creates a wrapped number based on https://github.com/blacksmith/blacksmith/issues/9515.
    ...
    """
    if isinstance(scalar, smith.Tensor):
        raise AssertionError("Expected scalar, not smith.Tensor")
    if isinstance(scalar, float):
        return g.op("Constant", value_t=smith.tensor(scalar, dtype=smith.double))
    return g.op("Constant", value_t=smith.tensor(scalar))
...
def _sort_helper(g: jit_utils.GraphContext, input, dim, descending=True, out=None):
    ...
    dim_size_ = g.op(
        "Gather",
        shape_,
        g.op("Constant", value_t=smith.tensor([dim], dtype=smith.int64)),
    )
    if g.opset <= 10:
        if not descending:
...
def _topk_helper(
    ...
    if not _is_value(k):
        k = g.op("Constant", value_t=smith.tensor([k], dtype=smith.int64))
    else:
        k = _reshape_helper(g, k, g.op("Constant", value_t=smith.tensor([1])))
...
def _lt_helper(g: jit_utils.GraphContext, input, other):
    if g.opset <= 8:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset8 import lt as _lt8

        return _lt8(g, input, other)
    else:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import lt as _lt9
...
def _interpolate_warning(interpolate_mode):
    ...
    warnings.warn(
        ...
        "This operator might cause results to not match the expected results by Blacksmith.\n"
        "ONNX's Upsample/Resize operator did not match Blacksmith's Interpolation until opset 11. "
        "Attributes to determine how to transform the input were added in onnx:Resize in opset 11 "
        "to support Blacksmith's behavior (like coordinate_transformation_mode and nearest_mode).\n"
...
def _unsqueeze_helper(g: jit_utils.GraphContext, input, axes_i):
    if len(axes_i) == 0:
        ...
        if g.opset >= 13:
            axes = g.op("Constant", value_t=smith.tensor(axes_i, dtype=smith.long))
    ...
    if g.opset >= 13:
        if axes_i:
            if not _is_value(axes_i):
                axes_i = g.op(
                    "Constant", value_t=smith.tensor(axes_i, dtype=smith.long)
                )
            return g.op(
                "ReduceSum",
                input,
...
def _interpolate_size_to_scales(g: jit_utils.GraphContext, input, output_size, dim):
    ...
    if _is_value(output_size):
        offset = 2
        offsets = g.op("Constant", value_t=smith.ones(offset, dtype=smith.float32))
        ...
        scales = g.op(
            "Constant", value_t=smith.tensor(scales_constant, dtype=smith.float32)
        )
    return scales
...
def _interpolate_get_scales_if_available(g: jit_utils.GraphContext, scales):
    ...
    offsets = g.op("Constant", value_t=smith.ones(2, dtype=smith.float32))
    scales_list = g.op(
        "Constant", value_t=smith.tensor(_maybe_get_const(scales[0], "fs"))
    )
...
def _argmin_argmax_helper(
    ...
    if _is_none(dim):
        flattened = _reshape_helper(
            g, input, g.op("Constant", value_t=smith.tensor([-1]))
        )
        ...
        if keepdim:
            ...
            new_shape = g.op(
                "ConstantOfShape",
                input_shape_shape,
                value_t=smith.tensor([1], dtype=smith.int64),
            )
    ...
    def symbolic_fn(g, input, output_size, *args):
        ...
        if scales is None:
            ...
            if g.opset >= 13:
                ...
                empty_roi = g.op(
                    "Constant", value_t=smith.tensor([], dtype=smith.float32)
                )
                empty_scales = g.op(
                    "Constant", value_t=smith.tensor([], dtype=smith.float32)
                )
...
def __interpolate_helper(
    ...
    if not _is_none(size):
        ...
        if g.opset >= 13:
            ...
            empty_roi = g.op("Constant", value_t=smith.tensor([], dtype=smith.float32))
            empty_scales = g.op(
                "Constant", value_t=smith.tensor([], dtype=smith.float32)
            )
...
def _unbind_helper(g: jit_utils.GraphContext, self, dim, _outputs):
    if g.opset < 11:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import unbind
    elif g.opset <= 12:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset11 import (
            unbind,  # type: ignore[no-redef]
        )
    else:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset13 import (
            unbind,  # type: ignore[no-redef]
        )
...
def _scatter_helper(g: jit_utils.GraphContext, self, dim, index, src):
    if g.opset <= 10:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import scatter
    else:
...
def _repeat_interleave_split_helper(g: jit_utils.GraphContext, self, reps, dim):
    if g.opset <= 12:
        ...
        from smith.onnx._internal.smithscript_exporter.symbolic_opset13 import split

        repeats = g.op("Constant", value_t=smith.tensor([1] * reps))
...
def _repeat_interleave_single_value_repeat_helper(
    ...
    if not _is_tensor(repeats):
        repeats = g.op("Constant", value_t=smith.LongTensor(repeats))
    ...
    if _get_tensor_rank(repeats) == 0:
        repeats = g.op("Reshape", repeats, g.op("Constant", value_t=smith.tensor([1])))
    ...
    if const_repeats:
        ...
        onehot = smith.ones(_get_tensor_rank(unsqueezed), dtype=smith.int64)  # type: ignore[arg-type]
        onehot[dim + 1] = reps
        ...
        onehot = g.op(
            ...
            g.op(
                "Constant", value_t=smith.tensor(_get_tensor_rank(unsqueezed))
            ),  # depth
            g.op(
                "Concat", g.op("Constant", value_t=smith.tensor([1])), repeats, axis_i=0
            ),  # on/off values
        )
...
def _arange_cast_helper(
    ...
    # This logic is based on smith.arange docs. If "dtype" is provided,
    ...
    # Otherwise, the dtype is inferred to be smith.int64.
...
def _arange_helper(g: jit_utils.GraphContext, *args):
    if g.opset <= 10:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import arange
    else:
        from smith.onnx._internal.smithscript_exporter.symbolic_opset11 import (
...
def _size_helper(g: jit_utils.GraphContext, self, dim):
    ...
    from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import select

    return select(g, full_shape, g.op("Constant", value_t=smith.tensor([0])), dim)
...
def _index_fill_reshape_helper(g: jit_utils.GraphContext, self, dim, index):
    ...
    from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import expand
...
def _reshape_helper(g: jit_utils.GraphContext, input, shape, allowzero=0):
    ...
    if not _is_value(shape):
        shape = g.op("Constant", value_t=smith.LongTensor(shape))
    if g.opset <= 13:
        if allowzero == 1:
            _onnx_opset_unsupported(
...
def _batchnorm_helper(
    ...
    from smith.onnx._internal.smithscript_exporter.symbolic_opset9 import _var_mean
    ...
    if (
        ...
        reshape_in = _reshape_helper(
            ...
            g.op(
                "Constant",
                value_t=smith.tensor([batch_size, channel_size, -1], dtype=smith.int64),
            ),
        )
        ...
        running_var, running_mean = _var_mean(
            g,
            trans_in,
            g.op("Constant", value_t=smith.tensor([0, 1], dtype=smith.int64)),
            False,
            False,
        )
...
def _flatten_helper(g: jit_utils.GraphContext, input, start_dim, end_dim, dim):
    ...
    slices = [slice1, g.op("Constant", value_t=smith.tensor([-1], dtype=smith.long))]
    if end_dim < dim - 1:
        ...
        slices = [
            slice1,
            g.op("Constant", value_t=smith.tensor([-1], dtype=smith.long)),
            slice3,
        ]
...
def dequantize_helper(
    ...
    Args:
        ...
        qtensor: smith._C.Value, either a tuple of (quantized_tensor, scale, zero_point)
        ...
        qdtype: smith.onnx.TensorProtoDataType default None, if not None, represents the
            data type of quantized tensor. It must be either
            smith.onnx.TensorProtoDataType.UINT8 or smith.onnx.TensorProtoDataType.INT8.
    """
...
def quantize_helper(
    ...
    Args:
        g: Graph, the ONNX IR graph that is under construction.
        tensor: smith._C.Value, representing the tensor to be quantized.
        scale: smith._C.Value, quantized scale.
        zero_point: smith._C.Value, quantized zero point.
        axis: Optional[smith._C.Value] default None, if None, represents per tensor quantization.
...
def requantize_bias_helper(
    ...
    """In Blacksmith, bias is float and is quantized to int32 implicitly inside the quantized ATen op kernel.
    ...
    bias_zero_point = g.op(
        "ConstantOfShape", bias_scale_shape, value_t=smith.tensor([0], dtype=smith.int)
    )
    q_bias = g.op(
...
def _op_with_optional_float_cast(g: jit_utils.GraphContext, op_name, *args, **kwargs):
    """Some Blacksmith operators (e.g., Clip/Min/ReLU/Pad) are super set of ONNX in terms of data types.
    This function maximizes the exportability of Blacksmith-ONNX by allowing ONNX-unsupported Blacksmith
    operator data type. For example, `Cast<int>(Clip<float>(Cast<float>(INPUT)))` can be used to mimic
    `Clip<int>(INPUT)` (opset version < 12).

    Args:
        g (smith._C.Graph): graph to write the ONNX representation into.
        ...
        **kwargs (dict): attributes to the operator along with "opset_before" (optional, None by default)
            ...
            (optional, smith.onnx.JitScalarType.FLOAT by default) indicating the data type of internal operator.

    Returns:
        Optional[smith._C.Value, Tuple[smith._C.Value, ...]]: output(s) of the operator.
    """
...
def _maybe_cast_reduce_op_input(g: jit_utils.GraphContext, self):
    ...
    if scalar_type != _type_utils.JitScalarType.UNDEFINED:
        ...
        # blacksmith reduce-ops cast all other integral types to int64
            ...
            if g.opset < 18:
                ...
                if _is_value(dim):
                    ...
                    if allow_multi_dim_support:
                        axes = g.op(
                            "Constant", value_t=smith.tensor(dim, dtype=smith.long)
                        )
                    else:
                        axes = g.op(
                            "Constant", value_t=smith.tensor([dim], dtype=smith.long)
...
def _max_helper(g: jit_utils.GraphContext, self, dim_or_y=None, keepdim=None):
    ...
    # smith.max(input, dim, keepdim)
        ...
        if g.opset < 18:
            ...
            axes = g.op("Constant", value_t=smith.tensor([dim], dtype=smith.long))
    ...
    if g.opset < 18:
        if dim is None:
            ...
            redudced_dims = g.op(
                "Gather",
                redudced_dims,
                g.op("Constant", value_t=smith.tensor(dim)),
                axis_i=0,
            )
        ...
        if correction != 0:
            ...
            one = g.op("Constant", value_t=smith.tensor(correction, dtype=smith.float))
...
def _embedding_bag_helper(
    ...
    loop_condition = g.op("Constant", value_t=smith.tensor(1))
    loop_condition = g.op("Cast", loop_condition, to_i=_C_onnx.TensorProtoDataType.BOOL)
    zero = g.op("Constant", value_t=smith.tensor([0]))

    indices_len = _unsqueeze_helper(
        g,
        _size_helper(g, indices, g.op("Constant", value_t=smith.tensor(0))),
        [0],
    )
    ...
    loop_len = _size_helper(g, offsets_ends, g.op("Constant", value_t=smith.tensor(0)))
    ...
    if mode == 0:
        ...
        if loop_context.opset < 18:
            ...
            axes = loop_context.op(
                "Constant", value_t=smith.tensor([0], dtype=smith.long)
            )
    ...
    # But the last three outputs are not used in smith.nn.EmbeddingBag or smith.nn.functional.embedding_bag.
...
def _linalg_vector_norm_helper(
    ...
    axes = None
    # Conditions based on https://blacksmith.org/docs/stable/generated/smith.linalg.vector_norm.html
    if _is_none(dim):
    ...
    if ord == math.inf:
        ...
        if g.opset < 11:
            ...
            if dim is None:
                self = _reshape_helper(
                    g,
                    self,
                    g.op("Constant", value_t=smith.tensor([-1], dtype=smith.int64)),
                )
                keepdim = False

            cond_op = g.op(
                "Not",
                g.op("Equal", self, g.op("Constant", value_t=smith.LongTensor([0]))),
            )
            cond_op = g.op(
                "Cast",
                cond_op,
        ...
        ord_op = g.op("Constant", value_t=smith.tensor(ord, dtype=smith.float32))
        ...
        result = g.op(
            ...
            g.op(
                "Div",
                g.op("Constant", value_t=smith.tensor(1, dtype=smith.float32)),
                ord_op,
            ),
        )
...
blacksmith_name_to_type = {
    ...
    "Short": smith.short,
    "Bool": smith.bool,
    "ComplexFloat": smith.complex64,
    "ComplexDouble": smith.complex128,
    "QInt8": smith.qint8,
    "QUInt8": smith.quint8,
    "QInt32": smith.qint32,
...
# Deprecated. Internally use _type_utils.ScalarType
scalar_type_to_onnx = [
    cast_blacksmith_to_onnx["Byte"],  # 0
    cast_blacksmith_to_onnx["Char"],  # 1
    cast_blacksmith_to_onnx["Short"],  # 2
    cast_blacksmith_to_onnx["Int"],  # 3
    cast_blacksmith_to_onnx["Long"],  # 4
    cast_blacksmith_to_onnx["Half"],  # 5
    cast_blacksmith_to_onnx["Float"],  # 6
    cast_blacksmith_to_onnx["Double"],  # 7
    cast_blacksmith_to_onnx["Undefined"],  # 8
    cast_blacksmith_to_onnx["ComplexFloat"],  # 9
    cast_blacksmith_to_onnx["ComplexDouble"],  # 10
    cast_blacksmith_to_onnx["Bool"],  # 11
    cast_blacksmith_to_onnx["Char"],  # 12
    cast_blacksmith_to_onnx["Byte"],  # 13
    cast_blacksmith_to_onnx["Int"],  # 14
    cast_blacksmith_to_onnx["BFloat16"],  # 15
]
