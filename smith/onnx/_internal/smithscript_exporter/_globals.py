...
Do not use this module outside of `smith.onnx` and its tests.
...
# This module should only depend on _constants and nothing else in smith.onnx to keep
...
class _InternalGlobals:
    ...
    def training_mode(self, training_mode: _C_onnx.TrainingMode) -> None:
        if not isinstance(training_mode, _C_onnx.TrainingMode):
            raise TypeError(
                "training_mode must be of type 'smith.onnx.TrainingMode'. This is "
                "likely a bug in smith.onnx."
            )
