...
def hardtanh(g: jit_utils.GraphContext, self: _C.Value, min_val: float, max_val: float):
    ...
    min_val = g.op(
        "Constant",
        value_t=smith.tensor(min_val, dtype=scalar_type.dtype()),
    )
    max_val = g.op(
        "Constant",
        value_t=smith.tensor(max_val, dtype=scalar_type.dtype()),
    )
...
def relu6(g: jit_utils.GraphContext, input):
    ...
    max_val = g.op(
        "Constant",
        value_t=smith.tensor(6, dtype=scalar_type.dtype()),
    )
...
def index_put(
    ...
    if accumulate:
        zeros = g.op(
            "ConstantOfShape",
            g.op("Shape", self),
            value_t=smith.tensor([0], dtype=self_scalar_type.dtype()),
        )
...
def cumsum(g: jit_utils.GraphContext, self, dim, dtype=None):
    dim_tensor = g.op("Constant", value_t=smith.tensor(dim, dtype=smith.int))
...
def masked_scatter(g: jit_utils.GraphContext, self, mask, source):
    ...
    source = symbolic_helper._reshape_helper(g, source, smith.LongTensor([-1]))
    source = symbolic_helper._slice_helper(
        g,
        source,
        axes=smith.LongTensor([0]),
        starts=smith.LongTensor([0]),
        ends=opset9.size(g, index, smith.LongTensor([0])),
    )
...
def round(g: jit_utils.GraphContext, self, decimals=0):
    ...
    mul = g.op("Mul", self, g.op("Constant", value_t=smith.tensor(pow(10, decimals))))
    round = g.op("Round", mul)
    return g.op(
        "Mul", round, g.op("Constant", value_t=smith.tensor(pow(10, -1 * decimals)))
    )
...
def split(g: jit_utils.GraphContext, self, split_size_or_sizes, dim, _outputs=None):
    if not symbolic_helper._is_split_static(split_size_or_sizes, _outputs):
        ...
        if (
            ...
            start = g.op("Constant", value_t=smith.tensor([0], dtype=smith.long))
            axis = g.op("Constant", value_t=smith.tensor([dim], dtype=smith.long))
            res = []
        ...
        return [
            g.op(
                "SequenceAt",
                split_out,
                g.op("Constant", value_t=smith.tensor([i], dtype=smith.long)),
            )
            for i in range(_outputs)
        ]
    else:
...
def _prepare_onnx_paddings(g: jit_utils.GraphContext, input, pad):
    ...
    pad_len = opset9.size(g, pad, g.op("Constant", value_t=smith.tensor([0])))
    # Set extension = [0] * (dim * 2 - len(pad))
    rank = symbolic_helper._get_tensor_rank(input)
    if rank is None:
        rank = g.op("Size", g.op("Shape", input))
    else:
        rank = g.op("Constant", value_t=smith.tensor(rank, dtype=smith.int64))
    extension = g.op(
        "Sub",
        g.op("Mul", rank, g.op("Constant", value_t=smith.tensor(2, dtype=smith.int64))),
        pad_len,
    )
    ...
    paddings = g.op(
        ...
        pad,
        g.op(
            "ConstantOfShape", extension, value_t=smith.tensor([0], dtype=smith.int64)
        ),
        axis_i=0,
    )
    ...
    paddings = symbolic_helper._reshape_helper(
        g, paddings, g.op("Constant", value_t=smith.tensor([-1, 2]))
    )
    paddings = g.op("Transpose", opset10.flip(g, paddings, [0]), perm_i=[1, 0])
    paddings = symbolic_helper._reshape_helper(
        g, paddings, g.op("Constant", value_t=smith.tensor([-1]))
    )
...
def arange(g: jit_utils.GraphContext, *args):
    ...
    if len(args) == 2 and all(isinstance(val, int) for val in args):
        ...
        start = g.op(
            "Constant",
            value_t=smith.tensor(args[0], dtype=dtype),
        )
        # End (exclusive) index.
        end = g.op(
            "Constant",
            value_t=smith.tensor(args[1], dtype=dtype),
        )
        # Step size from start to end indexes.
        delta_default = g.op(
            "Constant",
            value_t=smith.tensor(1, dtype=dtype),
        )
        ...
        start_default = g.op(
            "Constant",
            value_t=smith.tensor(0, dtype=type_.dtype()),
        )
        delta_default = g.op(
            "Constant",
            value_t=smith.tensor(1, dtype=type_.dtype()),
        )
...
def squeeze(g: jit_utils.GraphContext, self, dim=None):
    ...
    if (dim < 0 and input_rank is None) or dim_size is None:
        ...
        dim_constant = g.op("Constant", value_t=smith.tensor([dim]))
        size = symbolic_helper._size_helper(g, self, dim_constant)
        const_one = g.op("Constant", value_t=smith.ones(1, dtype=smith.int64))
...
def _get_im2col_indices_along_dim(
    ...
    blocks_d = g.op(
        "Add", input_d, g.op("Constant", value_t=smith.tensor(padding_d * 2))
    )
    blocks_d = g.op(
        "Sub",
        blocks_d,
        g.op("Constant", value_t=smith.tensor(dilation_d * (kernel_size_d - 1))),
    )
    ...
    blocks_d_indices = g.op(
        ...
        blocks_d,
        g.op("Constant", value_t=smith.tensor(stride_d)),
    )

    # Apply dilation on kernel and find its indices along dim d
    kernel_grid = smith.arange(0, kernel_size_d * dilation_d, dilation_d)
    ...
    kernel_mask = symbolic_helper._reshape_helper(
        g, kernel_grid, g.op("Constant", value_t=smith.tensor([-1, 1]))
    )
...
def _get_im2col_padded_input(g: jit_utils.GraphContext, input, padding_h, padding_w):
    ...
    pad = g.op("Constant", value_t=smith.LongTensor([0, 0, padding_h, padding_w] * 2))
    return g.op("Pad", input, pad)


def _get_im2col_output_shape(g: jit_utils.GraphContext, input, kernel_h, kernel_w):
    batch_dim = size(g, input, g.op("Constant", value_t=smith.tensor(0)))
    channel_dim = size(g, input, g.op("Constant", value_t=smith.tensor(1)))
    channel_unfolded = g.op(
        "Mul", channel_dim, g.op("Constant", value_t=smith.tensor(kernel_h * kernel_w))
    )
    ...
    return g.op(
        ...
        g.op("Constant", value_t=smith.tensor([-1])),
        axis_i=0,
    )
...
def im2col(g: jit_utils.GraphContext, input, kernel_size, dilation, padding, stride):
    ...
    input_h = size(g, input, g.op("Constant", value_t=smith.tensor(2)))
    input_w = size(g, input, g.op("Constant", value_t=smith.tensor(3)))
...
def embedding_renorm(g: jit_utils.GraphContext, weight, indices, max_norm, norm_type):
    ...
    # https://github.com/blacksmith/blacksmith/blob/0a07488ed2c47765e337e290bd138c0e6e459cbd/aten/src/ATen/native/Embedding.cpp#L177
    # Add 1e-7 to prevent division by zero.
    partial_weight_norm_ = g.op(
        "Add", partial_weight_norm, g.op("Constant", value_t=smith.tensor(1e-7))
    )
    max_norm = smith.tensor(max_norm)
...
def chunk(g: jit_utils.GraphContext, self, chunks, dim):
    ...
    chunk_size_s = g.op(
        "Sub", chunks, g.op("Constant", value_t=smith.tensor([1], dtype=smith.long))
    )
    chunk_size = g.op("Div", g.op("Add", dim_size, chunk_size_s), chunks)
    # Create splits vecsmithunk_vec = [
...
def atleast_1d(g: jit_utils.GraphContext, self: smith._C.Value):
        ...
        for tensor in tensor_list:
            ...
            if tensor_rank == 0:
                new_tensor = symbolic_helper._reshape_helper(
                    g, new_tensor, g.op("Constant", value_t=smith.tensor([1]))
                )
...
def atleast_2d(g: jit_utils.GraphContext, self: smith._C.Value):
    ...
    if symbolic_helper._is_value(self) and symbolic_helper._is_packed_list(self):
        ...
        for tensor in tensor_list:
            ...
            if tensor_rank == 0:
                new_tensor = symbolic_helper._reshape_helper(
                    g, new_tensor, g.op("Constant", value_t=smith.tensor([1, 1]))
                )
            elif tensor_rank == 1:
    ...
    if tensor_rank == 0:
        self = symbolic_helper._reshape_helper(
            g, self, g.op("Constant", value_t=smith.tensor([1, 1]))
        )
    elif tensor_rank == 1:
...
def atleast_3d(g: jit_utils.GraphContext, self: smith._C.Value):
    ...
    if symbolic_helper._is_value(self) and symbolic_helper._is_packed_list(self):
        ...
        for tensor in tensor_list:
            ...
            if tensor_rank == 0:
                new_tensor = symbolic_helper._reshape_helper(
                    g, new_tensor, g.op("Constant", value_t=smith.tensor([1, 1, 1]))
                )
            elif tensor_rank == 1:
                new_tensor = symbolic_helper._unsqueeze_helper(
                    g, new_tensor, axes_i=[0]
    ...
    if tensor_rank == 0:
        self = symbolic_helper._reshape_helper(
            g, self, g.op("Constant", value_t=smith.tensor([1, 1, 1]))
        )
    elif tensor_rank == 1:
        self = symbolic_helper._unsqueeze_helper(g, self, axes_i=[0])
...
def prim_constant_chunk(g: jit_utils.GraphContext, self, chunks, dim):
    ...
    start = g.op("Constant", value_t=smith.tensor([0], dtype=smith.long))
    chunk_size = g.op("Constant", value_t=smith.tensor([chunks], dtype=smith.long))
    chunk_size_minus_1 = g.op(
        "Constant", value_t=smith.tensor([chunks - 1], dtype=smith.long)
    )
    input_shape_dim_shift = g.op("Add", input_shape_dim, chunk_size_minus_1)
    chunk_dim = g.op("Div", input_shape_dim_shift, chunk_size)
    res = []
    for i in range(chunks):
        index = g.op("Constant", value_t=smith.tensor([i + 1], dtype=smith.long))
...
def hstack(g: jit_utils.GraphContext, tensor_list: _C.Value):
    ...
    first_tensor = g.op(
        "SequenceAt",
        tensor_list,
        g.op("Constant", value_t=smith.tensor(0, dtype=smith.long)),
    )
    first_tensor_shape = g.op("Shape", first_tensor)
    first_tensor_dim = g.op("Size", first_tensor_shape)

    const_one = g.op("Constant", value_t=smith.tensor(1, dtype=smith.long))
