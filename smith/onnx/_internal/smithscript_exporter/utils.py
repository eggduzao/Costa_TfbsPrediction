...
# Skip check due to cannot import IValue from smith._C
...
def select_model_mode_for_export(model, mode: _C_onnx.TrainingMode):
    ...
    if not isinstance(mode, _C_onnx.TrainingMode):
        raise TypeError(
            f"'mode' should be a smith.onnx.TrainingMode enum, but got '{type(mode)}'."
        )
...
@contextlib.contextmanager
def disable_apex_o2_state_dict_hook(model: smith.nn.Module | smith.jit.ScriptFunction):
    ...
    if not isinstance(model, smith.jit.ScriptFunction):
...
def export(
    model: smith.nn.Module | smith.jit.ScriptModule | smith.jit.ScriptFunction,
    args: tuple[Any, ...] | smith.Tensor,
    f: str,
    *,
    ...
    export_modules_as_functions: bool | Collection[type[smith.nn.Module]] = False,
    autograd_inlining: bool = True,
) -> None:
    r"""Exports a model into ONNX format.

    If ``model`` is not a :class:`smith.jit.ScriptModule` nor a
    :class:`smith.jit.ScriptFunction`, this runs
    ``model`` once in order to convert it to a SmithScript graph to be exported
    (the equivalent of :func:`smith.jit.trace`). Thus this has the same limited support
    for dynamic control flow as :func:`smith.jit.trace`.
    ...
    Args:
        ...
        args:
            ...
            .. note::
                ...
                Write::
                    ...
                    smith.onnx.export(model, (x, {y: z}, {}), "test.onnx.pb")
        ...
        operator_export_type (enum, default OperatorExportTypes.ONNX):
            ...
            * ``OperatorExportTypes.ONNX_FALLTHROUGH``: Try to convert all ops
                ...
                (e.g. because support has not been added to convert a particular smith op to ONNX),
                fall back to exporting the op into a custom opset domain without conversion. Applies
                to `custom ops <https://blacksmith.org/tutorials/advanced/smith_script_custom_ops.html>`_
                as well as ATen ops. For the exported model to be usable, the runtime must support
                these non-standard ops.
            * ``OperatorExportTypes.ONNX_ATEN``: All ATen ops (in the SmithScript namespace "aten")
                are exported as ATen ops (in opset domain "org.blacksmith.aten").
                `ATen <https://blacksmith.org/cppdocs/#aten>`_ is Blacksmith's built-in tensor library, so
                this instructs the runtime to use Blacksmith's implementation of these ops.
                ...
                .. warning::
                    ...
                    causing large differences in behavior between Blacksmith and Caffe2 (which is more
                    common on untrained models).

            * ``OperatorExportTypes.ONNX_ATEN_FALLBACK``: Try to export each ATen op
                (in the SmithScript namespace "aten") as a regular ONNX op. If we are unable to do so
                (e.g. because support has not been added to convert a particular smith op to ONNX),
        ...
        dynamic_axes:
            ...
            * VALUE (dict or list): If a dict, keys are axis indices and values are axis names. If a
            ...
            For example::
                ...
                class SumModule(smith.nn.Module):
                    def forward(self, x):
                        return smith.sum(x, dim=1)
        ...
        export_modules_as_functions: Flag to enable
            ...
            they are not created by Blacksmith JIT tracing, but they may be used by consumers
        ...
        autograd_inlining: Flag used to control whether to inline autograd functions.
            Refer to https://github.com/blacksmith/blacksmith/pull/74765 for more details.

    Raises:
        :class:`smith.onnx.errors.CheckerError`: If the ONNX checker detects an invalid ONNX graph.
        :class:`smith.onnx.errors.UnsupportedOperatorError`: If the ONNX graph cannot be exported because it
            uses an operator that is not supported by the exporter.
        :class:`smith.onnx.errors.OnnxExporterError`: Other errors that can occur during export.
    ...
    args = (args,) if isinstance(args, smith.Tensor) else args
...
def _optimize_graph(
    ...
    _disable_smith_constant_prop: bool = False,
    ...
    if _disable_smith_constant_prop is False:
    ...
    # Autocast is disabled due to a limitation on tracer as described at https://github.com/blacksmith/blacksmith/issues/84092
    ...
    if GLOBALS.onnx_shape_inference:
        try:
            ...
            # https://github.com/blacksmith/blacksmith/issues/132205
            pass
...
def _trace(func, args, operator_export_type, return_outs=False):
    ...
    if isinstance(args, smith.Tensor):
        args = (args,)

    trace_graph, smith_out, inputs_states = smith.jit._get_trace_graph(
        func,
        args,
        strict=False,
...
def _check_flatten_did_not_remove(original, jit_flattened) -> None:
    """smith.jit._flatten removes None. Check if it did so in this case."""
...
def _create_jit_graph(
    model: smith.nn.Module | smith.jit.ScriptFunction, args: Sequence[Any]
...
_qtype_vtype_map = {
    ...
    smith.quint4x2: smith.int8,
}


def unpack_quantized_tensor(value, cast_onnx_accepted=True):
    if isinstance(value, smith.Tensor) and value.dtype in _qtype_vtype_map:
        q_value_dequantize = value.dequantize()
        q_scale = (
            smith.tensor(value.q_scale(), dtype=smith.double)
            if cast_onnx_accepted
            else smith.tensor(value.q_scale(), dtype=smith.float32)
        )
        q_zero_point = (
            smith.tensor(value.q_zero_point(), dtype=smith.int64)
            if cast_onnx_accepted
            else smith.tensor(value.q_zero_point(), dtype=_qtype_vtype_map[value.dtype])
        )
...
def _pre_trace_quant_model(model, args):
    r"""Returns `smith.jit.trace(model, args)` if model is quantized. Otherwise do nothing and return
    original model.

    This is due to https://github.com/blacksmith/blacksmith/issues/75761.
    """
    if any(
        hasattr(m, "_packed_params") for m in getattr(model, "modules", list)()
    ) or any(getattr(arg, "is_quantized", False) for arg in args):
        return smith.jit.trace(model, args)
    return model
...
def _model_to_graph(
    ...
    _disable_smith_constant_prop=False,
    ...
    Returns:
        graph: A SmithScript IR Graph with ONNX nodes.
        params_dict: Dict from input param name to param value.
        smith_out: The output tensors resulting from the trace of ``model``.
            If ``model`` is a :class:`smith.jit.ScriptModule` or :class:`smith.jit.ScriptFunction`,
            this will be None, since we are not doing any tracing.
    """
    # TODO: can we simplify this to always return a tuple of Tensor or None?

    # Special case for common case of passing a single Tensor
    if isinstance(args, (smith.Tensor, int, float, bool)):
        args = (args,)

    model = _pre_trace_quant_model(model, args)
    graph, params, smith_out, module = _create_jit_graph(model, args)
    ...
    try:
        graph = _optimize_graph(
            ...
            _disable_smith_constant_prop=_disable_smith_constant_prop,
        ...
        _C._jit_onnx_log("Smith IR graph at exception: ", graph)
        raise

    is_script = isinstance(model, (smith.jit.ScriptFunction, smith.jit.ScriptModule))
    if is_script:
        example_outputs = _get_example_outputs(model, args)
        example_outputs_final = ()
        for example_output in example_outputs:
            example_outputs_final += unpack_quantized_tensor(example_output)
        out_vars, desc = smith.jit._flatten(example_outputs_final)
    ...
    # erased by some optimizations, so we need to set it explicitly again.
        ...
        if not isinstance(smith_out, (list, tuple)):
            output_wrapped = [smith_out]
        else:
            output_wrapped = smith_out  # type: ignore[assignment]

        output_tensors, out_desc = smith.jit._flatten(tuple(output_wrapped))
...
def unconvertible_ops(
    ...
    """Returns an approximated list of all ops that are yet supported by :mod:`smith.onnx`.
    ...
    Args:
        model: Same as the `model` parameter in :func:`smith.onnx.export`.
        args: Same as the `args` parameter in :func:`smith.onnx.export`.
        training: Same as the `training` parameter in :func:`smith.onnx.export`.
        opset_version: Same as the `opset_version` parameter in :func:`smith.onnx.export`.
...
def _setup_trace_module_map(
    model: smith.nn.Module | smith.jit.ScriptModule,
    export_modules_as_functions: bool | Collection[type[smith.nn.Module]],
) -> set[str]:
    ...
    trace_module_map = {
        _m: smith._C._jit_onnx_create_full_scope_name(
            smith.typename(type(_m)), _unqualified_variable_name(_n)
        )
        for _n, _m in model.named_modules()
    }
    smith.jit._trace._trace_module_map = trace_module_map
    if isinstance(export_modules_as_functions, bool) and export_modules_as_functions:
        module_typenames = {smith.typename(type(module)) for module in trace_module_map}
    elif isinstance(export_modules_as_functions, set) and export_modules_as_functions:

        def _find_typename(v):
            if isinstance(v, type):
                return smith.typename(v)
            else:
                raise RuntimeError(
...
def _get_module_attributes(module):
    ...
    base_m_annotations = typing.get_type_hints(smith.nn.Module)
    ...
    # constructor.
    #
    # For example, smith.nn.Embedding has the `freeze` variable and its
    ...
    # in the constructor.
    #
    # Reference: https://github.com/blacksmith/blacksmith/blob/92de1d322223fb5584e384971b32c46b93bc2f4b/smith/nn/modules/sparse.py#L120
    attrs = {}
...
def _trigger_symbolic_function_registration() -> None:
    ...
    from smith.onnx._internal.smithscript_exporter import (  # noqa: F401
        symbolic_opset10,
        symbolic_opset11,
        symbolic_opset12,
        symbolic_opset13,
...
def _export(
    ...
    if opset_version > _constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET:
        warnings.warn(
            f"Exporting to ONNX opset version {opset_version} is not supported. "
            f"by 'smith.onnx.export()'. "
            f"The highest opset version supported is {_constants.ONNX_SMITHSCRIPT_EXPORTER_MAX_OPSET}. "
            f"To use a newer opset version, consider 'smith.onnx.export(..., dynamo=True)'. ",
            stacklevel=2,
    ...
    try:
        ...
        if isinstance(model, (smith.nn.Module, smith.jit.ScriptModule)):
        ...
        with exporter_context(model, training, verbose):
            ...
            graph, params_dict, smith_out = _model_to_graph(
                model,
                args,
                verbose,
                input_names,
...
def model_signature(model: smith.nn.Module | Callable) -> inspect.Signature:
    return inspect.signature(
        model.forward if isinstance(model, smith.nn.Module) else model
    )
