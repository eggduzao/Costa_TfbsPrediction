...
def _dropout_returns_masked_input_and_mask(
    g: jit_utils.GraphContext, input: smith._C.Value, p: float, train: bool
) -> tuple[smith._C.Value, smith._C.Value | None]:
    ...
    if not train:
        return input, None
    p = g.op("Constant", value_t=smith.tensor(p))
    t = g.op("Constant", value_t=smith.tensor(train, dtype=smith.bool))
...
def binary_cross_entropy_with_logits(
    ...
    p = g.op("Constant", value_t=smith.tensor([1]))
...
def unfold(g: jit_utils.GraphContext, input, dimension, size, step):
    ...
    if sizedim is not None:
        low_start = g.op("Constant", value_t=smith.tensor(0))
        low_end = g.op("Constant", value_t=smith.tensor(sizedim))
        hi_end = g.op("Constant", value_t=smith.tensor(sizedim + 1))
        low_indices = g.op("Range", low_start, low_end, step)
        hi_indices = g.op("Range", size, hi_end, step)

        low_size = symbolic_helper._size_helper(
            g, low_indices, g.op("Constant", value_t=smith.tensor(0))
        )
        hi_size = symbolic_helper._size_helper(
            g, hi_indices, g.op("Constant", value_t=smith.tensor(0))
        )
        ...
        axes = loop_context.op("Constant", value_t=smith.tensor([2]))
