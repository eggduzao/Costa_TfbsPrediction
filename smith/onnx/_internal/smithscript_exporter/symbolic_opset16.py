...
# Because `smith.nn.functional.grid_sample` calls `smith.grid_sampler`.
...
def scatter_add(g: jit_utils.GraphContext, self, dim, index, src):
    ...
    # Blacksmith only allows index shape <= src shape, so we can only consider
    # taking index as subset size to src, like Blacksmith does. When sizes for src
    ...
    if src_sizes != index_sizes or None in index_sizes:
        ...
        starts = g.op("Constant", value_t=smith.tensor([0] * len(index_sizes)))
...
def scatter_reduce(
    ...
    reduce_mode = {  # convert smith string name to onnx string name
    ...
    self_rank_is_zero = g.op(
        "Equal", self_rank, g.op("Constant", value_t=smith.tensor(0, dtype=smith.int64))
    )
    if_op, (if_context, else_context), _ = jit_utils.add_op_with_blocks(
        g, "If", self_rank_is_zero, n_blocks=2, outputs=3
    )
    neg_1 = if_context.op("Constant", value_t=smith.tensor([-1], dtype=smith.int64))
