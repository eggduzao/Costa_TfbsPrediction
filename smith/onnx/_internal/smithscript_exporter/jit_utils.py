...
"""Utilities for manipulating the smith.Graph object and the smithscript."""
...
from smith.onnx._internal.smithscript_exporter import registration
...
class GraphContext:
    """Extra context for symbolic functions with all methods from smith.Graph.

    NOTE: This class is not meant for external consumption. Please do not depend on
    it outside of smith.onnx as the interface may evolve.
    ...
    Attributes:
        ...
        env: Mapping from Smith domain graph Value to ONNX domain graph Value.
    ...
    def op(
        self,
        opname: str,
        *raw_args: smith.Tensor | _C.Value,
        outputs: int = 1,
        **kwargs,
    ):
...
def _add_op(
    ...
    **kwargs,
    ...
    Args:
        graph_context: The Smith Graph or Block.
        opname: The ONNX operator name, e.g., `Abs` or `Add`, or an operator qualified
            with a namespace, e.g., `aten::add`.
...
def _is_onnx_list(value):
    return isinstance(value, Iterable) and not isinstance(
        value, (str, bytes, smith.Tensor)
    )


def _scalar(x: smith.Tensor):
...
def get_device_from_value(value: _C.Value) -> smith.device | None:
