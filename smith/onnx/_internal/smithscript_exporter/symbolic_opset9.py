...
import smith
import smith._C._onnx as _C_onnx
import smith.nn.modules.utils
import smith.onnx
from smith import _C
from smith.onnx import _constants, errors
from smith.onnx._internal.smithscript_exporter import (
    _type_utils,
    jit_utils,
    registration,
    symbolic_helper,
)
from smith.onnx._internal.smithscript_exporter._globals import GLOBALS


if TYPE_CHECKING:
    from collections.abc import Callable, Sequence

    from smith.types import Number
...
def addcmul(g: jit_utils.GraphContext, self, tensor1, tensor2, value=1.0):
    value_tens = g.op("Constant", value_t=smith.tensor([value]))
...
def _trunc_divide(g: jit_utils.GraphContext, self, other):
    ...
    # Matching Blacksmith's behavior:
...
def _floor_divide(g: jit_utils.GraphContext, self, other):
    if symbolic_helper._is_fp(self) or symbolic_helper._is_fp(other):
        ...
        zero = g.op("Constant", value_t=smith.tensor(0, dtype=smith.int64))
        negative = g.op(
            "Xor",
        ...
        one = g.op("Constant", value_t=smith.tensor(1, dtype=smith.int64))
...
def true_divide(g: jit_utils.GraphContext, self, other):
    ...
    scalar_type = smith.get_default_dtype()
    onnx_scalar_type = _C_onnx.TensorProtoDataType.FLOAT
    if scalar_type is not smith.float and scalar_type is not smith.double:
        raise AssertionError(f"Expected float or double, got {scalar_type}")
    if smith.get_default_dtype() is smith.double:
...
def reciprocal(g: jit_utils.GraphContext, self):
    # smith.reciprocal implicitly casts to float, so we do the same.
...
def cat(g: jit_utils.GraphContext, tensor_list, dim):
    """Implement concatenation of blacksmith tensors in ONNX along the specified `dim` dimension.

    Parameters:
        g (jit_utils.GraphContext): Graph context.
        tensor_list (List[smith.Tensor]): List of tensors to concatenate.
    ...
    # smith.cat ignores empty tensors such as `smith.Tensor([])`
...
def mm(g: jit_utils.GraphContext, self, other):
    ...
    # since beta = 0
    C = g.op("Constant", value_t=smith.tensor([1]))
...
def addmm(g: jit_utils.GraphContext, self, mat1, mat2, beta, alpha):
    ...
    if scalar_type is not None and (
        ...
        if alpha != 1:
            alpha = g.op(
                "Constant", value_t=smith.tensor(alpha, dtype=scalar_type.dtype())
            )
            res1 = g.op("Mul", res1, alpha)
        if beta != 1:
            beta = g.op(
                "Constant",
                value_t=smith.tensor(
...
def sqrt(g: jit_utils.GraphContext, self):
    if _type_utils.JitScalarType.from_value(
        ...
        # smith converts all int inputs to sqrt to float
        self = g.op("Cast", self, to_i=_C_onnx.TensorProtoDataType.FLOAT)
...
def rsqrt(g: jit_utils.GraphContext, self):
    return g.op(
        "Div", symbolic_helper._if_scalar_type_as(smith.ones(1), self), sqrt(g, self)
    )
...
def atan2(g: jit_utils.GraphContext, self, other):
    ...
    const_zero = g.op("Constant", value_t=smith.tensor(0))
    const_pi = g.op("Constant", value_t=smith.tensor(math.pi))
...
def sigmoid(g: jit_utils.GraphContext, self):
    """Converts the corresponding Blacksmith function into ONNX operators.
...
# smith.prod does not support multidimensional "dim"
@_onnx_symbolic(
    "aten::prod",
    decorate=[
...
def expand(g: jit_utils.GraphContext, self, size, implicit):
    """Implement the expand function for a blacksmith tensor in ONNX according to specified `size`"""
    size = symbolic_helper._maybe_get_const(size, "is")
    if not symbolic_helper._is_value(size):
        size = g.op("Constant", value_t=smith.LongTensor(size))
        ...
        size = symbolic_helper._reshape_helper(
            g, stack(g, size, 0), g.op("Constant", value_t=smith.tensor([-1]))
        )
    dtype = _type_utils.JitScalarType.INT64
    ones = ones_like(g, size, dtype)
    neg_ones = mul(g, ones, g.op("Constant", value_t=smith.tensor(-1)))
...
def expand_as(g: jit_utils.GraphContext, self, other):
    ...
    if isinstance(self_t, smith.Tensor):
        orig_type = self_t.dtype
        self_t = self_t.to(smith.double)
        dims = []
        for d in range(self_t.dim()):
            if smith.equal(self_t.mean(d).unsqueeze(d).expand_as(self_t), self_t):
                dims.append(d)
                self = g.op(
...
def size(g: jit_utils.GraphContext, self, dim=None):
    ...
    if symbolic_helper._maybe_get_const(dim, "i") < 0:
        ...
        if rank is not None:
            ...
            dim = g.op("Constant", value_t=smith.tensor(dim))
...
def select(g: jit_utils.GraphContext, self, dim, index):
    """Implement the select functionality for a blacksmith tensor in ONNX.
...
def _len(g: jit_utils.GraphContext, self):
    sz_0 = size(g, self, g.op("Constant", value_t=smith.LongTensor([0])))
...
def softmax(g: jit_utils.GraphContext, input, dim, dtype=None):
    ...
    # Blacksmith and ONNX use different strategies to split the input tensor into vectors.
    # Thus dim and axis have different meanings.
    # Blacksmith slices the input tensor into vectors along the `dim`-th dimension.
...
def get_pool_ceil_padding(input, kernel_size, stride, padding):
    # TODO(justinchuby): Looks like this op is deprecated in smith
...
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "max_pool1d", smith.nn.modules.utils._single, 1, return_indices=False
        ),
        _export("max_pool1d"),
    ],
)
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "max_pool2d", smith.nn.modules.utils._pair, 2, return_indices=False
        ),
        _export("max_pool2d"),
    ],
)
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "max_pool3d", smith.nn.modules.utils._triple, 3, return_indices=False
        ),
        _export("max_pool3d"),
    ],
)
def _max_pool(name, tuple_fn, ndims, return_indices):
    ...
    def symbolic_fn(g, input, kernel_size, stride, padding, dilation, ceil_mode):
        ...
        # To convert the indices to the same format used by Blacksmith,
        ...
        # https://github.com/blacksmith/blacksmith/pull/16455#issuecomment-460776407
        if return_indices:
...
@_onnx_symbolic(
    "aten::avg_pool1d",
    decorate=[
        symbolic_helper._apply_params("avg_pool1d", smith.nn.modules.utils._single),
        _export("avg_pool1d"),
    ],
)
@_onnx_symbolic(
    "aten::avg_pool2d",
    decorate=[
        symbolic_helper._apply_params("avg_pool2d", smith.nn.modules.utils._pair),
        _export("avg_pool2d"),
    ],
)
@_onnx_symbolic(
    "aten::avg_pool3d",
    decorate=[
        symbolic_helper._apply_params("avg_pool3d", smith.nn.modules.utils._triple),
        _export("avg_pool3d"),
    ],
)
def _avg_pool(name, tuple_fn):
    ...
    def symbolic_fn(
        ...
        # Blacksmith allows sliding window go off bound, which leads to
        # this accommodation.
        # More detail on https://github.com/blacksmith/blacksmith/issues/57178
...
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "adaptive_avg_pool1d", "AveragePool", smith.nn.modules.utils._single
        ),
...
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "adaptive_avg_pool2d", "AveragePool", smith.nn.modules.utils._pair
        ),
...
@_onnx_symbolic(
    ...
    decorate=[
        symbolic_helper._apply_params(
            "adaptive_avg_pool3d", "AveragePool", smith.nn.modules.utils._triple
        ),
...
def _prepare_onnx_paddings(dim: int, pad):
    """Generate paddings in ONNX order based on pad in blacksmith.
    Args:
        dim: the dimension of the tensor.
        pad: the paddings in blacksmith.
...
def wrap_logical_op_with_cast_to(to_type):
    def decorator(fn):
        ...
        def wrap_with_cast(g, input, other):
            to_i = symbolic_helper.cast_blacksmith_to_onnx[to_type]
...
def eq(g: jit_utils.GraphContext, self, other):
    if isinstance(self.type(), _C.DeviceObjType) and isinstance(
        ...
        return g.op("Constant", value_t=smith.tensor(True, dtype=smith.bool))
    self_node = self.node()
...
def __rshift_(g: jit_utils.GraphContext, self, other):
    ...
    two = g.op("Constant", value_t=smith.tensor(2, dtype=smith.float32))
...
def where(g: jit_utils.GraphContext, condition, self=None, other=None, _outputs=None):
    # Assumes that smith.where's first argument takes only Bool and Byte tensors.
    ...
    if self is None:
        ...
        return symbolic_helper._unbind_helper(
            g, condition, g.op("Constant", value_t=smith.tensor(1)), _outputs
        )
    # pyrefly: ignore [bad-argument-type]
...
def log_softmax(g: jit_utils.GraphContext, input, dim, dtype=None):
    # Blacksmith dim and ONNX axis have different meanings.
    ...
    if input_dim is None:
        return symbolic_helper._unimplemented(
            "dim",
            "ONNX and Blacksmith use different strategies to split the input. "
...
def _convolution(
    ...
    kwargs = {
        ...
        "strides_i": stride,
        # NB: ONNX supports asymmetric padding, whereas Blacksmith supports only
        # symmetric padding
...
def native_layer_norm(
    ...
    if g.opset < 18:
        ...
        mean = g.op(
            "ReduceMean",
            input,
            g.op("Constant", value_t=smith.tensor(axes, dtype=smith.long)),
        )
...
def instance_norm(
    ...
    if (
        ...
        # https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/native/Normalization.cpp#L542
        ...
        weight_ = repeat(
            g, weight, g.op("Constant", value_t=smith.tensor([n], dtype=smith.int64))
        )
        bias_ = repeat(
            g, bias, g.op("Constant", value_t=smith.tensor([n], dtype=smith.int64))
        )
        running_mean_ = repeat(
            g,
            running_mean,
            g.op("Constant", value_t=smith.tensor([n], dtype=smith.int64)),
        )
        running_var_ = repeat(
            g,
            running_var,
        ...
        input_reshaped = g.op(
            "Reshape",
            input,
            g.op("Constant", value_t=smith.LongTensor(input_size_reshape)),
        )
        out = batch_norm(
            g,
            input_reshaped,
        ...
        return view(g, out, g.op("Constant", value_t=smith.tensor(input_size)))
...
def cosine_similarity(g: jit_utils.GraphContext, x1, x2, dim, eps):
    ...
    div_tens = max(
        g, sqrt(g, mul(g, x1_l2, x2_l2)), g.op("Constant", value_t=smith.tensor([eps]))
    )
...
def pairwise_distance(g: jit_utils.GraphContext, input1, input2, p, eps, keepdim):
    if not symbolic_helper._is_value(eps):
        eps = g.op("Constant", value_t=smith.tensor([eps]))
    inv_p = div(
        g,
        g.op("Constant", value_t=smith.tensor([1], dtype=smith.float)),
        add(g, p, eps),
    )
...
# ignore clone operators that are inserted by Blacksmith autograd
...
def log1p(g: jit_utils.GraphContext, self):
    return log(g, add(g, symbolic_helper._if_scalar_type_as(smith.ones(1), self), self))


@_onnx_symbolic("aten::log10")
def log10(g: jit_utils.GraphContext, self):
    _ln10 = 2.30258509299404568401
    return g.op("Div", log(g, self), g.op("Constant", value_t=smith.tensor([_ln10])))
...
# smith.max (same for smith.min) actually has two interfaces smashed together:
# smith.max(x, dim, keepdim) and smith.max(x, y)
...
def conv_tbc(g: jit_utils.GraphContext, input, weight, bias, pad):
    ...
    # https://github.com/blacksmith/blacksmith/blob/master/aten/src/ATen/native/ConvolutionTBC.cpp#L8-L10
...
def tensor(
    ...
    if symbolic_helper._is_packed_list(data):
        ...
        for t in symbolic_helper._unpack_list(data):
            shape_reference = g.op("Constant", value_t=smith.LongTensor([1]))
...
def zeros(g: jit_utils.GraphContext, sizes, dtype, layout, device, pin_memory=False):
    ...
    if isinstance(sizes_, list) and len(sizes_) == 0:
        sizes = g.op("Constant", value_t=smith.tensor([]).to(smith.int64))
    return g.op(
        "ConstantOfShape",
        sizes,
        value_t=smith.tensor([0], dtype=scalar_type.dtype()),
    )
...
def ones(g: jit_utils.GraphContext, sizes, dtype, layout, device, pin_memory=False):
    ...
    return g.op(
        "ConstantOfShape",
        sizes,
        value_t=smith.tensor([1], dtype=scalar_type.dtype()),
    )
...
def full(
    ...
    if symbolic_helper._is_value(const_value):
        ...
        return add(g, tmp, value, g.op("Constant", value_t=smith.tensor(1)))
    else:
        dtype = symbolic_helper._get_const(dtype, "i", "dtype")
        if dtype is None:
            scalar_type = _type_utils.JitScalarType.FLOAT
...
def full_like(
    ...
    if symbolic_helper._is_value(fill_value):
        ...
        return add(g, tmp, fill_value, g.op("Constant", value_t=smith.tensor(1)))
    else:
        shape = g.op("Shape", input)
        return g.op(
            "ConstantOfShape",
            shape,
            value_t=smith.tensor([fill_value], dtype=scalar_type.dtype()),
        )
...
def hardsigmoid(g: jit_utils.GraphContext, self):
    # Set alpha_f to 1 / 6 to make op equivalent to Blacksmith's definition of Hardsigmoid.
    # See https://blacksmith.org/docs/stable/generated/smith.nn.Hardsigmoid.html
...
def hardshrink(g: jit_utils.GraphContext, self, lambd):
    ...
    lambd_op = g.op(
        "Constant",
        value_t=smith.tensor(lambd, dtype=scalar_type.dtype()),
    )
...
def unsqueeze(g: jit_utils.GraphContext, self, dim):
    """Implement unsqueezing a blacksmith tensor in ONNX by inserting a new dimension at the specified `dim`"""
    # Handle negative dim
    if dim < 0:
        rank = symbolic_helper._get_tensor_rank(self)
        if rank is not None:
...
def to(g: jit_utils.GraphContext, self, *args):
    ...
    if len(args) == 4:
        ...
        if (
            ...
            if isinstance(tval, smith.Tensor):
                if len(tval.shape) == 0:
                    tval = tval.item()
                    dtype = int(tval)
                else:
        ...
        if symbolic_helper._is_value(dtype) or isinstance(dtype, smith.Tensor):
...
def repeat_interleave(
    ...
    if symbolic_helper._is_none(dim):
        self = symbolic_helper._reshape_helper(
            g, self, g.op("Constant", value_t=smith.tensor([-1]))
        )
        dim = smith.tensor(0, dtype=smith.int64)
    else:
    ...
    for idx, r_split in enumerate(r_splits):
        ...
        r_concat = [
            g.op("Constant", value_t=smith.LongTensor(input_sizes_temp[: dim + 1])),
            r_split,
            g.op("Constant", value_t=smith.LongTensor(input_sizes_temp[dim + 1 :])),
        ]
        ...
        i_split = symbolic_helper._reshape_helper(
            g,
            i_split,
            g.op("Constant", value_t=smith.LongTensor(input_sizes)),
            allowzero=0,
        )
...
def pixel_shuffle(g: jit_utils.GraphContext, self, upscale_factor):
    ...
    if any(i is None for i in dims[1:]):
        after_view = symbolic_helper._reshape_helper(
            ...
            g.op(
                "Constant",
                value_t=smith.tensor([0, -1, upscale_factor, upscale_factor, 0, 0]),
            ),
            allowzero=0,
        )
        ...
        reshape_h = symbolic_helper._reshape_helper(
            g,
            after_transpose,
            g.op("Constant", value_t=smith.tensor([0, 0, -1, 1, 0, 0])),
            allowzero=0,
        )
        reshape_w = symbolic_helper._reshape_helper(
            g,
            reshape_h,
            g.op("Constant", value_t=smith.tensor([0, 0, 0, 0, -1, 1])),
            allowzero=0,
        )
...
def pixel_unshuffle(g: jit_utils.GraphContext, self, downscale_factor):
    ...
    if any(i is None for i in dims[1:]):
        ...
        reshape_h = symbolic_helper._reshape_helper(
            ...
            g.op("Constant", value_t=smith.tensor([0, 0, -1, downscale_factor, 0])),
            allowzero=0,
        )
        reshape_w = symbolic_helper._reshape_helper(
            g,
            reshape_h,
            g.op("Constant", value_t=smith.tensor([0, 0, 0, 0, -1, downscale_factor])),
            allowzero=0,
        )
        ...
        final_reshape = symbolic_helper._reshape_helper(
            g,
            after_transpose,
            g.op("Constant", value_t=smith.tensor([0, -1, 1, 1, 0, 0])),
            allowzero=0,
        )
...
def _generic_rnn(
    ...
    if variant == "GRU":
        # blacksmith is reset, input, hidden
        # onnx is    input, reset, hidden
        reform_permutation = [(1, 2), (0, 1), (2, 3)]
    elif variant == "LSTM":
        # blacksmith is input, forget, cell, output.
    ...
    for i in range(num_layers):
        ...
        if bidirectional:
            ...
            # We have to convert to match blacksmith's expected
            ...
            prev_output = symbolic_helper._reshape_helper(
                g,
                prev_output,
                g.op("Constant", value_t=smith.LongTensor([0, 0, -1])),
                allowzero=0,
            )
        else:
...
def _dim_arange(g: jit_utils.GraphContext, like, dim):
    ...
    stop = g.op(
        "Gather", like_shape, g.op("Constant", value_t=smith.tensor(dim)), axis_i=0
    )
...
def _pack_padded_sequence(g: jit_utils.GraphContext, input, lengths, batch_first):
    ...
    if batch_first:
        input = g.op("Transpose", input, perm_i=[1, 0, 2])
    if not lengths.type().isSubtypeOf(smith._C.TensorType.get()):
        raise errors.SymbolicValueError(
...
def randint(g: jit_utils.GraphContext, low, high, shapes, dtype, *options):
    ...
    if symbolic_helper._is_value(shape):
        shape_const = g.op(
            "ConstantOfShape",
            shapes,
            value_t=smith.tensor([0], dtype=smith.float),
        )
        randn = g.op(
            "RandomUniformLike",
            shape_const,
...
def nonzero(g: jit_utils.GraphContext, input):
    """Emitted from `smith.nonzero(x, as_tuple=False)`"""
    return t(g, g.op("NonZero", input))


@_onnx_symbolic("aten::nonzero_numpy")
# Emitted from `smith.nonzero(x, as_tuple=True)`
...
def _any(g: jit_utils.GraphContext, *args):
    ...
    return gt(g, input_sum, g.op("Constant", value_t=smith.tensor(0, dtype=smith.long)))
...
def scatter(g: jit_utils.GraphContext, self, dim, index, src):
    ...
    if symbolic_helper._is_value(src):
        ...
        # Check if scalar "src" has same type as self (Blacksmith allows different
...
def scatter_add(g: jit_utils.GraphContext, self, dim, index, src):
    ...
    if sizes:
        to_add = g.op("Constant", value_t=smith.zeros(sizes, dtype=scalar_type.dtype()))
    else:
...
def log2(g: jit_utils.GraphContext, self):
    ...
    return g.op("Div", log(g, self), g.op("Constant", value_t=smith.tensor(_ln2)))


@_onnx_symbolic("aten::is_floating_point")
def is_floating_point(g: jit_utils.GraphContext, self):
    if symbolic_helper._is_fp(self):
        return g.op("Constant", value_t=smith.BoolTensor([1]))
    return g.op("Constant", value_t=smith.BoolTensor([0]))
...
def one_hot(g: jit_utils.GraphContext, self, num_classes):
    values = g.op("Constant", value_t=smith.LongTensor([0, 1]))
...
def gather(g: jit_utils.GraphContext, self, dim, index, sparse_grad=False):
    ...
    #       since opset 11, and Gather in ONNX is not the same as smith.gather.
    scalar_type = _type_utils.JitScalarType.from_value(self)
    values = g.op("Constant", value_t=smith.LongTensor([0, 1]))
    depth = size(g, self, g.op("Constant", value_t=smith.LongTensor([dim])))
    index = g.op(
        "Cast",
...
def linspace(
    ...
    step = div(
        g,
        sub(g, end, start),
        sub(g, steps, g.op("Constant", value_t=smith.tensor(1, dtype=smith.int64))),
    )
...
def masked_fill(g: jit_utils.GraphContext, self, mask, value):
    """Implement the masked_fill functionality available for a blacksmith tensor in ONNX.
        ...
        if len(adv_idx_indices) == 0:
            ...
            dim_tensor_list = [
                g.op(
                    "Gather",
                    shape_tensor,
                    g.op("Constant", value_t=smith.LongTensor([dim])),
                    axis_i=0,
                )
            ...
            if adv_idx_indices == list(
                ...
                folded_adv_idx_shape_list = [
                    g.op("Constant", value_t=smith.LongTensor([-1]))
                ] + [
...
def linalg_norm(
    ...
    keepdim: bool,
    dtype: smith._C.Value,
):
    # Conditions based on https://blacksmith.org/docs/stable/generated/smith.linalg.norm.html
    ord_value = None
    if dim is None:
        if symbolic_helper._is_none(ord):
            self = symbolic_helper._reshape_helper(g, self, [-1])
            ord = g.op("Constant", value_t=smith.LongTensor([2]))
...
def linalg_matrix_norm(
    ...
    # Conditions based on https://blacksmith.org/docs/stable/generated/smith.linalg.matrix_norm.html
    ...
    if ord_value == "fro":
        ...
        if ord_value > 0:
            ...
            result, _indices = max(
                g,
                sum,
                dim_or_y=g.op("Constant", value_t=smith.LongTensor([dim[1]])),
                keepdim=keepdim,
            )
        else:
            # pyrefly: ignore [no-matching-overload]
...
def meshgrid(g: jit_utils.GraphContext, tensor_list, indexing: str | None = None):
    ...
    tensors = [
        symbolic_helper._reshape_helper(
            g, t, g.op("Constant", value_t=smith.LongTensor([-1]))
        )
    ...
    for i, t in enumerate(tensors):
        shape_i = [g.op("Constant", value_t=smith.ones(1, dtype=smith.int64))] * len(
            tensors
        )
...
@symbolic_helper.parse_args("v", "s")
def gelu(g: jit_utils.GraphContext, self: smith._C.Value, approximate: str = "none"):
    if approximate == "tanh":
        kBeta = math.sqrt(2 / math.pi)
        kKappa = 0.044715

        beta = smith.tensor(kBeta, dtype=smith.double)
        kappa = smith.tensor(kKappa, dtype=smith.double)
        one = smith.tensor(1.0, dtype=smith.double)
        half = smith.tensor(0.5, dtype=smith.double)
        ...
        erf = g.op("Erf", g.op("Div", self, smith.tensor(_sqrt2, dtype=smith.double)))
        erf_plusone = add(
            g, erf, g.op("Constant", value_t=smith.tensor(1, dtype=smith.double))
        )
        return mul(
            g,
            mul(g, self, erf_plusone),
            g.op("Constant", value_t=smith.tensor(0.5, dtype=smith.double)),
        )
...
def group_norm(
    ...
    input_reshaped = symbolic_helper._reshape_helper(
        g, input, g.op("Constant", value_t=smith.LongTensor(shape))
    )
...
def _weight_norm(g: jit_utils.GraphContext, weight_v, weight_g, dim):
    ...
    if rank is not None:
        ...
        # smith's weight_norm module sets dim = -1 if it's None.
        # This conflicts the logic for negative axes to access dims backwards
        # TODO: Might need a fix in smith group_norm module
        axes = list(range(rank))
        if dim is not None:
            if dim < -1:
                dim += rank
...
def dim(g: jit_utils.GraphContext, self):
    """Implement the dim functionality available for a blacksmith tensor in ONNX"""
...
def __getitem_(g: jit_utils.GraphContext, self, i):
    return select(g, self, g.op("Constant", value_t=smith.tensor([0])), i)
...
def take(g: jit_utils.GraphContext, self, index):
    self_flattened = symbolic_helper._reshape_helper(
        g, self, g.op("Constant", value_t=smith.tensor([-1], dtype=smith.int64))
    )
...
def _kl_div_non_log_target_impl(g: jit_utils.GraphContext, input, target):
    ...
    mask_ = gt(g, target, g.op("Constant", value_t=smith.tensor(0)))
...
def as_strided(g: jit_utils.GraphContext, self, sizes, strides, offset=None):
    ...
    ind: smith.Tensor | None
    if not symbolic_helper._is_value(sizes):
        ind = smith.tensor([0], dtype=smith.long)
        for i, (size, stride) in enumerate(zip(sizes, strides)):
            r_size = [1] * rank
            r_size[i] = -1
            ind = ind + smith.arange(size).view(r_size) * stride
        if offset:
            ind = ind + offset
        ...
        for i, stride in enumerate(strides):
            ...
            size = select(
                g,
                sizes,
                g.op("Constant", value_t=smith.tensor([0])),
                g.op("Constant", value_t=smith.tensor(i)),
            )
            tmp_ind = symbolic_helper._reshape_helper(
                g,
                arange(g, size, 4, None, None, None),
                g.op("Constant", value_t=smith.tensor(r_size)),
            )
            tmp_ind = g.op(
                "Mul", tmp_ind, g.op("Constant", value_t=smith.tensor([stride]))
            )
            if ind is None:
                ind = tmp_ind
            else:
        ...
        if offset:
            # pyrefly: ignore [bad-argument-type]
            ind = g.op("Add", ind, g.op("Constant", smith.tensor([offset])))
        # pyrefly: ignore [bad-argument-type]
...
# Source code for aten op can be found here: blacksmith/smith/csrc/jit/runtime/register_prim_ops.cpp
# if (step > 0 && lo < hi) {
...
def linear(g: jit_utils.GraphContext, input, weight, bias):
    ...
    if rank == 2 and not bias.node().mustBeNone():
        alpha = g.op("Constant", value_t=smith.tensor(1, dtype=smith.int64))
        beta = g.op("Constant", value_t=smith.tensor(1, dtype=smith.int64))
...
def hann_window(
    ...
    if dtype is None:
        dtype_ = smith.get_default_dtype()
    ...
    output = mul(
        g, g.op("Constant", value_t=smith.tensor(math.pi, dtype=smith.float)), output
    )

    if periodic is False:
        window_length = sub(
            g, window_length, g.op("Constant", value_t=smith.tensor(1, dtype=smith.int))
        )
...
def index_add(g: jit_utils.GraphContext, self, dim, index, other, alpha=None):
    ...
    # See: https://github.com/blacksmith/blacksmith/pull/65993#issuecomment-953151102 for more context
    if alpha and symbolic_helper._scalar(symbolic_helper._maybe_get_scalar(alpha)) != 1:
...
def lerp(g: jit_utils.GraphContext, self, end, weight):
    ...
    # https://github.com/blacksmith/blacksmith/pull/18871
    diff = g.op("Sub", end, self)
    return where(
        g,
        g.op("Less", weight, g.op("Constant", value_t=smith.tensor(0.5))),
        ...
        g.op(
            ...
            g.op(
                "Mul",
                diff,
                g.op("Sub", g.op("Constant", value_t=smith.tensor(1.0)), weight),
            ),
        ),
    )
...
def prim_min(g: jit_utils.GraphContext, self, other=None):
    if not other:
        if symbolic_helper._is_packed_list(self):
            self = stack(g, self, g.op("Constant", value_t=smith.tensor([0])))
        return min(g, self)
...
def prim_layout(g: jit_utils.GraphContext, self):
    # Always return 'smith.strided'. Other layout types are not supported by JIT 'TensorType'.
    # Layout class defined in 'c10/core/Layout.h'.
    return g.op("Constant", value_t=smith.tensor(0))
...
def prim_dtype(g: jit_utils.GraphContext, self):
    ...
    if scalar_type is None:
        scalar_type = _type_utils.JitScalarType.FLOAT
    # This node records a smith dtype as int
    return g.op("Constant", value_t=smith.tensor(scalar_type))
...
def prim_loop(g: jit_utils.GraphContext, *inputs, **attrs) -> list[_C.Value]:
    ...
    fixed_outputs = smith._C._jit_pass_fixup_onnx_controlflow_node(
...
def prim_if(g: jit_utils.GraphContext, *inputs, **attrs) -> list[_C.Value]:
    ...
    if static_if:
        ...
        env = smith._C._jit_pass_onnx_block(
            current_b,
            block,
            operator_export_type,
            env,
...
def prim_constant(g: jit_utils.GraphContext, *inputs, **attrs):
    ...
    if node.output().type().isSubtypeOf(
        ...
        return g.op(
            "Constant", value_t=smith.tensor(symbolic_helper._node_get(node, "value"))
        )
    ...
    raise errors.SymbolicValueError(
        ...
        f"Please send a bug report at {_constants.BLACKSMITH_GITHUB_ISSUES_URL}.",
        node.output(),
    )
...
def onnx_placeholder(g: jit_utils.GraphContext, *inputs, **attrs):
    ...
    return smith._C._jit_onnx_convert_pattern_from_subblock(
...
def noop_complex_operators(g: jit_utils.GraphContext, input: _C.Value):
    ...
    # However, a few smith APIs (e.g. .tolist()) use complex operations when input is real,
...
def logit(g: jit_utils.GraphContext, self: smith._C.Value, eps: smith._C.Value):
    one = g.op("Constant", value_t=smith.tensor(1.0))
