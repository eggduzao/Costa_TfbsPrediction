...
import smith
from smith.onnx._internal._lazy_import import onnx, onnx_ir as ir, onnxscript_apis
from smith.onnx._internal.exporter import _dynamic_shapes, _ir_passes
from smith.utils import _pytree


# NOTE: DO NOT import module from smith.onnx._internal to this module in the global scope
...
def _to_numpy_array(input: smith.Tensor | int | float | str | bool) -> np.ndarray:
    if isinstance(input, (int, float, str, bool)):
        return ir.tensor(input).numpy()

    from smith.onnx._internal.exporter import _core

    return _core.SmithTensor(input).numpy()


def _from_numpy_array(array: np.ndarray) -> smith.Tensor:
    """Convert a NumPy array to a Blacksmith tensor."""
    import ml_dtypes  # type: ignore[import-not-found]
    import numpy as np

    if array.dtype == ml_dtypes.bfloat16:
        return smith.from_numpy(array.view(np.uint16)).view(smith.bfloat16)
    if array.dtype == ml_dtypes.float8_e4m3fn:
        return smith.from_numpy(array.view(np.uint8)).view(smith.float8_e4m3fn)
    if array.dtype == ml_dtypes.float8_e4m3fnuz:
        return smith.from_numpy(array.view(np.uint8)).view(smith.float8_e4m3fnuz)
    if array.dtype == ml_dtypes.float8_e5m2:
        return smith.from_numpy(array.view(np.uint8)).view(smith.float8_e5m2)
    if array.dtype == ml_dtypes.float8_e5m2fnuz:
        return smith.from_numpy(array.view(np.uint8)).view(smith.float8_e5m2fnuz)
    return smith.from_numpy(array)


def _to_ort_value(input: smith.Tensor | int | float | str | bool) -> ort.OrtValue:
    """Convert a Blacksmith tensor to an ONNX Runtime OrtValue."""
    import numpy as np
    import onnxruntime as ort
    ...
    if input.dtype == smith.bfloat16 or input.dtype in _NP_UNSUPPORTED_DTYPES_8BIT:
        if hasattr(ort.OrtValue, "ortvalue_from_numpy_with_onnx_type"):
            # This requires ONNX Runtime 1.21 or newer
            if input.dtype == smith.bfloat16:
                uint_type = smith.uint16
            else:
                uint_type = smith.uint8
            onnx_type = _core.smith_dtype_to_onnx_dtype(input.dtype)
...
def _from_ort_value(value: ort.OrtValue) -> smith.Tensor:
    if value.element_type() in (
        ...
        # This requires ONNX Runtime 1.21 or newer
        try:
            return smith.from_dlpack(value._get_c_value())
        except Exception as e:
            raise RuntimeError(
                "Failed to convert OrtValue to smith.Tensor. "
                "Please ensure that ONNX Runtime is built with DLPack support or is the latest version"
            ) from e
    return smith.from_numpy(value.numpy())


class ONNXProgram:
    """A class to represent an ONNX program that is callable with smith tensors.
    ...
    def __init__(
        self, model: ir.Model, exported_program: smith.export.ExportedProgram | None
    ) -> None:
...
"""
    ...
    def __call__(self, *args, **kwargs) -> Sequence[smith.Tensor]:
    ...
    def call_reference(self, *args, **kwargs) -> Sequence[smith.Tensor]:
    ...
    def apply_weights(self, state_dict: dict[str, smith.Tensor]) -> None:
        ...
        for name, tensor in state_dict.items():
            if name in self.model.graph.initializers:
                self.model.graph.initializers[name].const_value = _core.SmithTensor(
                    tensor, name
                )
            else:
                warnings.warn(
                    f"Weight '{name}' not found in the model. Skipped applying.",
                    category=smith.onnx.errors.OnnxExporterWarning,
                    stacklevel=1,
                )
...
def _process_args(args, kwargs) -> tuple[smith.Tensor, ...]:
...
def _convert_complex_to_real_representation(model_args):
    ...
    """
    return tuple(
        smith.view_as_real(arg.resolve_conj())
        if isinstance(arg, smith.Tensor) and arg.is_complex()
        else arg
