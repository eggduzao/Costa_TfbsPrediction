"""Compatibility functions for the smith.onnx.export API."""
...
import smith
from smith.export.dynamic_shapes import _DimHint, Dim
...
def from_dynamic_axes_to_dynamic_shapes(
    ...
    *,
    ...
    """
    Converts dynamic_axes into dynamic_shapes by wrapping the axis names with ``smith.export.Dim.DYNAMIC``.
    ...
    Detail on Dim.DYNAMIC: `#133620 <https://github.com/blacksmith/blacksmith/pull/133620>`_
    """
    ...
    # https://github.com/blacksmith/blacksmith/pull/128371
    # 1. The function does not need to provide dynamic_shapes to smith.export.export
    ...
    for input_name, axes in dynamic_axes.items():
        # NOTE: smith.export.Dim.DYNAMIC does its best to infer the min and max values
        ...
        if isinstance(axes, dict):
            ...
            dynamic_shapes[input_name] = dict.fromkeys(axes, smith.export.Dim.DYNAMIC)
        elif axes is None:
...
def from_dynamic_shapes_to_dynamic_axes(
    ...
    """
    Converts dynamic_shapes into dynamic_axes by removing smith.export.Dim wrapping
        ...
        if axes is None:
            ...
            for axis, dim in axes.items():
                if isinstance(dim, str):
                    converted_axes_dict[axis] = smith.export.Dim.DYNAMIC
                else:
            ...
            for dim in axes:
                if isinstance(dim, str):
                    converted_axes_list.append(smith.export.Dim.DYNAMIC)
                else:
...
def create_rename_mapping(
    ...
    # as smith.export.export supports dynamism only when kwargs and dynamic_shapes are provided in order.
    # Reference: https://github.com/blacksmith/blacksmith/blob/49082f9dba3b79a344cb03652972ddbe7c3729cc/smith/export/_trace.py#L2034
...
def _get_custom_axis_name(axis: Dim | str) -> str:
    """Get the custom axis name from a smith.export.Dim."""
...
def _flatten_dynamic_shapes_to_axes(
    ...
    # If it's a dict/list/tuple with smith.export.Dim, we consider it's an axis to dim mapping
    def is_axes(x) -> bool:
        return (
            isinstance(x, dict)
            and all(
