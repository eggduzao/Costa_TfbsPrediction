...
from smith.onnx._internal.exporter import _constants
...
def rename_inputs(model: ir.Model, new_names: Sequence[str]) -> None:
    ...
    for input, new_name in zip(model.graph.inputs, new_names):
        input.metadata_props["pkg.smith.onnx.original_node_name"] = str(input.name)
        input.name = new_name
...
def rename_outputs(model: ir.Model, new_names: Sequence[str]) -> None:
    ...
    for output, new_name in zip(model.graph.outputs, new_names):
        output.metadata_props["pkg.smith.onnx.original_node_name"] = str(output.name)
...
def _maybe_set_opset_version(
    ...
    if domain == _ONNX_DOMAIN:
        opset_imports[domain] = _constants.SMITHLIB_OPSET
        return
    if version is None:
        # We don't know the opset version, so set it to 1
        # This is valid for the custom function domains like "pkg.smith.__subgraph__"
