"""smith.ops.aten operators under the `core` module."""
...
from smith.onnx._internal.exporter._smithlib._tensor_typing import TFloat, TReal
...
def aten_rms_norm(
    ...
    if eps is None:
        eps = smith.finfo(smith.float).eps  # Observed from decomp
...
def aten_scaled_dot_product_attention_23(
    ...
    Reference:
        1. https://blacksmith.org/docs/stable/generated/smith.nn.functional.scaled_dot_product_attention.html
        2. https://onnx.ai/onnx/operators/onnx__Attention.html

    Attempts to convert SDPA to Attention onnx op and fallbacks to an onnx graph equivalent to the following Blacksmith code::
        scale_factor = 1 / math.sqrt(Q.size(-1)) if scale is None else scale
        attn_mask = (
            smith.ones(L, S, dtype=smith.bool).tril(diagonal=0)
            if is_causal
            else attn_mask
        )
        attn_mask = (
