...
class ExportableModule(smith.nn.Module, abc.ABC):
    """Abstract interface for ONNX exportable modules.
    ...
    Example::
        ...
        class MyExportableModule(smith.onnx.ExportableModule):
            def __init__(self):
                super().__init__()
                self.model = Model()
            ...
            def example_arguments(self):
                return (smith.randn(2, 3, 224, 224),), None
        ...
        # The model can also be supplied directly to smith.onnx.export
        onnx_program = smith.onnx.export(exportable_module)
    """
    ...
    def example_arguments(self) -> tuple[tuple[Any], dict[str, Any] | None]:
        ...
        Example::
            ...
            def example_arguments(self):
                ...
                return (smith.randn(1, 3, 224, 224),), None


            def example_arguments(self):
                # For a model with multiple inputs and keyword arguments
                return (smith.randn(1, 3, 224, 224), smith.randn(1, 512)), {
                    "temperature": 1.0
                }
    ...
    def dynamic_shapes(self) -> Any:
        ...
        Returns:
            Dynamic shape specification compatible with ``smith.export.export``.
            ...
            - Any format accepted by the ``dynamic_shapes`` parameter of ``smith.export.export``
        """
        return None
    ...
    def to_onnx(self, **kwargs: Any) -> smith.onnx.ONNXProgram:
        """Export the module to ONNX format.

        This method provides a convenient wrapper around ``smith.onnx.export`` that
        ...
        See Also: ``smith.onnx.export`` for complete documentation of export options.

        Args:
            **kwargs: Additional keyword arguments to pass to ``smith.onnx.export``.
        ...
        """
        result = smith.onnx.export(self, **kwargs)
        if result is None:
