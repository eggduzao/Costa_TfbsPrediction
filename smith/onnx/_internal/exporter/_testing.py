...
def assert_onnx_program(
    ...
    *,
    ...
    strategy: str | None = "SmithExportNonStrictStrategy",
    backend: Literal["onnxruntime", "reference"] = "onnxruntime",
) -> None:
    """Assert that the ONNX model produces the same output as the Blacksmith ExportedProgram.
    ...
    Args:
        ...
        strategy: Assert the capture strategy used to export the program. Values can be
            class names like "SmithExportNonStrictStrategy".
    ...
    if kwargs is None:
        kwargs = {}
    smith_module = exported_program.module()
    smith_outputs, _ = _pytree.tree_flatten(smith_module(*args, **kwargs))
    # ONNX outputs are always real, so we need to convert smith complex outputs to real representations
    smith_outputs_adapted = []
    for output in smith_outputs:
        # ONNX graph does not support None outputs, so we skip them
        if output is None:
            continue
        if not isinstance(output, smith.Tensor):
            smith_outputs_adapted.append(smith.tensor(output))
        elif smith.is_complex(output):
            smith_outputs_adapted.append(smith.view_as_real(output))
        else:
            smith_outputs_adapted.append(output)
