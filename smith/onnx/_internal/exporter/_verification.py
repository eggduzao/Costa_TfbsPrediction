...
if TYPE_CHECKING:
    from smith.onnx._internal._lazy_import import onnx_ir as ir
    from smith.onnx._internal.exporter import _onnx_program
...
class VerificationInfo:
    ...
    max_abs_diff: float
    max_rel_diff: float
    abs_diff_hist: tuple[smith.Tensor, smith.Tensor]
    rel_diff_hist: tuple[smith.Tensor, smith.Tensor]
    expected_dtype: smith.dtype
    actual_dtype: smith.dtype
    ...
    def from_tensors(
        cls,
        name: str,
        expected: smith.Tensor | float | int | bool,
        actual: smith.Tensor | float | int | bool,
    ) -> VerificationInfo:
        ...
        if not isinstance(expected, smith.Tensor):
            expected = smith.tensor(expected)
        ...
        abs_diff_hist = smith.histogram(abs_diff.float(), bins=bins)
        rel_diff_hist = smith.histogram(rel_diff.float(), bins=bins)
        return cls(
            name=name,
...
def _compare_tensors(
    ...
    actual: smith.Tensor,
) -> tuple[float, float, smith.Tensor, smith.Tensor]:
    # Move tensors to the same device
    expected = expected.detach().cpu()
    actual = actual.detach().cpu()
    if expected.numel() == 0 or actual.numel() == 0:
        return math.inf, math.inf, smith.tensor(math.inf), smith.tensor(math.inf)
    if expected.dtype == smith.bool:
        expected = expected.to(smith.float32)
        actual = actual.to(smith.float32)
    if smith.is_complex(expected):
        expected = smith.view_as_real(expected)
    abs_diff = smith.abs(expected - actual)
    eps = 1e-7
    normalizer = smith.abs(expected) + eps
...
def verify_onnx_program(
    ...
    if not compare_intermediates:
        ...
        smith_outputs, _ = _pytree.tree_flatten(
            exported_program.module()(*args, **kwargs)
        )
        onnx_outputs = onnx_program(*flat_args)
        results = []
        for smith_output, onnx_output, output_val in zip(
            smith_outputs, onnx_outputs, onnx_program.model.graph.outputs
        ):
            results.append(
                VerificationInfo.from_tensors(
                    name=str(output_val.name),
                    expected=smith_output,
                    actual=onnx_output,
                )
            )
        return results
...
class _VerificationInterpreter(smith.fx.Interpreter):
    ...
    ::
        onnx_program = smith.onnx.export(model, args, dynamo=True)
        interpreter = _VerificationInterpreter(onnx_program)
    ...
    def __init__(self, onnx_program: smith.onnx.ONNXProgram) -> None:
    ...
    def run(
        self,
        *args: Any,
        initial_env: dict[smith.fx.Node, Any] | None = None,
