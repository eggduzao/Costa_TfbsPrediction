"""Compatibility analyzer for Blacksmith models."""
...
import smith
import smith._export.serde.schema
from smith.export import graph_signature
from smith.onnx._internal.exporter import _dispatching, _registration
...
class ModelInfo:
    ...
    parameter_count: defaultdict[smith.dtype, int] = dataclasses.field(
        default_factory=lambda: defaultdict(int)
    )
    buffer_count: defaultdict[smith.dtype, int] = dataclasses.field(
        default_factory=lambda: defaultdict(int)
    )
    fx_node_count: int = 0
    ...
    dispatch_failures: list[tuple[smith.fx.Node, str]] = dataclasses.field(
        default_factory=list
    )
    inputs: dict[str, smith._export.serde.schema.TensorMeta] = dataclasses.field(
        default_factory=dict
    )
    outputs: dict[str, smith._export.serde.schema.TensorMeta] = dataclasses.field(
        default_factory=dict
    )
...
) -> tuple[defaultdict[smith.dtype, int], defaultdict[smith.dtype, int]]:
    """Count the size of the parameters in the exported program."""

    parameter_count: defaultdict[smith.dtype, int] = defaultdict(int)
    buffer_count: defaultdict[smith.dtype, int] = defaultdict(int)
...
def _format_model_info(model_info: ModelInfo) -> str:
    ...
    lines = [
        textwrap.dedent(
            f"""\
            Blacksmith ONNX Conversion Analysis
...
def _get_io_specs(exported_program: smith.export.ExportedProgram) -> tuple[dict, dict]:
    """Get the input and output specs of the exported program."""

    nodes: dict[str, smith.fx.Node] = {
    ...
    inputs: dict[str, smith._export.serde.schema.TensorMeta | str] = {}
    outputs: dict[str, smith._export.serde.schema.TensorMeta | str] = {}
...
def _log_spec_into_io_specs(
    ...
    nodes: dict[str, smith.fx.Node],
    inputs_or_outputs: dict[str, smith._export.serde.schema.TensorMeta | str],
) -> dict[str, smith._export.serde.schema.TensorMeta | str]:
...
def compare_ops(
    program_a: smith.export.ExportedProgram, program_b: smith.export.ExportedProgram
