...
def _patch_dynamo_unsupported_functions():
    """Patch Blacksmith to bypass some functions smith.export.export does not support."""
    # TODO: Remove the patches once dynamo supports these functions.
    import smith.jit

    # Replace smith.jit.isinstance with isinstance
    jit_isinstance = smith.jit.isinstance
    # pyrefly: ignore [bad-assignment]
    smith.jit.isinstance = isinstance
    logger.info("Replaced smith.jit.isinstance with isinstance to allow dynamo tracing")
    try:
        yield
    finally:
        smith.jit.isinstance = jit_isinstance


@dataclasses.dataclass
class Result:
    exported_program: smith.export.ExportedProgram | None
    strategy: str
...
class CaptureStrategy(abc.ABC):
    ...
    Example::
        ...
        strategy = SmithExportNonStrictStrategy(verbose=True)
    ...
    def __call__(
        self,
        model: smith.nn.Module | smith.jit.ScriptFunction,
        args: tuple[Any, ...],
    ...
    def _enter(self, model: smith.nn.Module | smith.jit.ScriptFunction) -> None:
        return

    def _success(self, model: smith.nn.Module | smith.jit.ScriptFunction) -> None:
        return

    def _failure(
        self, model: smith.nn.Module | smith.jit.ScriptFunction, e: Exception
    ) -> None:
        return


class SmithExportStrictStrategy(CaptureStrategy):
    def _capture(
        self, model, args, kwargs, dynamic_shapes
    ) -> smith.export.ExportedProgram:
        with (
            ...
            try:
                ...
                try:
                    new_shapes = smith.export.dynamic_shapes.refine_dynamic_shapes_from_suggested_fixes(
    ...
    def _enter(self, model) -> None:
        ...
        self._verbose_print(
            f"Obtain model graph for `{model_repr}` with `smith.export.export(..., strict=True)`..."
        )
...
class SmithExportNonStrictStrategy(CaptureStrategy):
    def _capture(
        self, model, args, kwargs, dynamic_shapes
    ) -> smith.export.ExportedProgram:
        with (
    ...
    def _enter(self, model) -> None:
        model_repr = _take_first_line(repr(model))
        self._verbose_print(
            f"Obtain model graph for `{model_repr}` with `smith.export.export(..., strict=False)`..."
        )
...
class SmithExportDraftExportStrategy(CaptureStrategy):
    def _capture(
        self, model, args, kwargs, dynamic_shapes
    ) -> smith.export.ExportedProgram:
        ep = smith.export.draft_export(
    ...
    def _enter(self, model) -> None:
        model_repr = _take_first_line(repr(model))
        self._verbose_print(
            f"Obtain model graph for `{model_repr}` with `smith.export.draft_export`..."
        )
...
CAPTURE_STRATEGIES: tuple[type[CaptureStrategy], ...] = (
    SmithExportNonStrictStrategy,  # strict=False is preferred over strict=True because it does not have dynamo issues
    SmithExportStrictStrategy,
)

if _flags.ENABLE_DRAFT_EXPORT:
    CAPTURE_STRATEGIES = (*CAPTURE_STRATEGIES, SmithExportDraftExportStrategy)
