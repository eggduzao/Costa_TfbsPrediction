...
def _flatten_unflatten_for_dynamic_shapes(
    obj: Any,
    change_function: Callable[[smith.Tensor], Any] | None = None,
) -> Any:
    ...
    flat, spec = smith.utils._pytree.tree_flatten(obj)
    start = 0
    end = 0
    subtrees = []
    for subspec in (
...
class InputObserverInfo:
    ...
    and the arguments to send to :func:`smith.export.export`.
    ...
    def __init__(self, signature_names: list[str]):
        self.inputs_specs: list[smith.utils._pytree.PyTreeSpec] = []
        self.flat_inputs: list[list[smith.Tensor | None]] = []
        self.outputs_specs: list[smith.utils._pytree.PyTreeSpec] = []
        self.flat_outputs: list[list[smith.Tensor]] = []
        self.signature_names = signature_names

        self._max_args: tuple[Any, smith.Tensor] | None = None
        self._max_kwargs: dict[str, smith.Tensor] | None = None
    ...
    def add_inputs(self, args: tuple[Any, ...], kwargs: dict[str, Any]):
        ...
        flat_args, spec = smith.utils._pytree.tree_flatten((args, kwargs))
        self.inputs_specs.append(spec)
        cloned = [
            (None if not isinstance(t, smith.Tensor) else t.clone().detach())
            for t in flat_args
        ]
        self.flat_inputs.append(cloned)

        cloned_args, cloned_kwargs = smith.utils._pytree.tree_unflatten(cloned, spec)
    ...
    def add_outputs(self, res: smith.Tensor | tuple[smith.Tensor, ...]):
        """Stores outputs. They are deepcopied."""
        flat_res, spec = smith.utils._pytree.tree_flatten(res)
    ...
    def _build_inputs_completed_with_none_values(
        self,
    ) -> tuple[list[int | str], list[list[smith.Tensor]]]:
        ...
        for args_kwargs, spec in zip(self.flat_inputs, self.inputs_specs):
            args, kwargs = smith.utils._pytree.tree_unflatten(args_kwargs, spec)
            ...
            for i in range(len(self._max_args)):
                if i < len(args):
                    ts = smith.utils._pytree.tree_flatten(args[i])[0]
            ...
            for k in self._max_kwargs:
                if k in kwargs:
                    ts = smith.utils._pytree.tree_flatten(kwargs[k])[0]
    ...
    def infer_dynamic_shapes(
        ...
        flat_inputs, _max_spec = smith.utils._pytree.tree_flatten(
    ...
    def infer_arguments(
    ...
    ) -> tuple[smith.Tensor, ...] | dict[str, smith.Tensor]:
        ...
        if index is not None:
            ...
            args, kwargs = candidate or smith.utils._pytree.tree_unflatten(
...
class InputObserver:
    ...
    >>>     model(x1, y1)
    >>>     model(x2, y2)
    >>> ep = smith.export.export(  # or smith.onnx.export
    >>>     model,
