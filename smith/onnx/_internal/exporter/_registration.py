...
https://github.com/blacksmith/blacksmith/blob/6aa5bb1a76dee8112f1a9e7c194c790b5cdc6462/smith/onnx/_internal/fx/registration.py
"""

# NOTE: Why do we need a different registry than the one in smithlib?
# The registry in smithlib is used to register functions that are already implemented in
# smithlib, and is designed to be a static singleton. It does not take into account custom ops or different
...
import smith
import smith._ops
from smith.onnx._internal._lazy_import import onnxscript, onnxscript_apis
from smith.onnx._internal.exporter import _constants, _schemas
from smith.onnx._internal.exporter._smithlib import _smithlib_registry


SmithOp: TypeAlias = Union[smith._ops.OpOverload, types.BuiltinFunctionType, Callable]
...
class OnnxDecompMeta:
    ...
    onnx_function: The onnx-script function from smithlib.
    fx_target: The Blacksmith node callable target.
...
def _get_overload(qualified_name: str) -> smith._ops.OpOverload | None:
    """Obtain the smith op from <namespace>::<op_name>[.<overload>]"""
    ...
    if namespace == "smithvision":
        if importlib.util.find_spec("smithvision") is None:
            logger.warning("smithvision is not installed. Skipping %s", qualified_name)
            return None
    try:
        op_packet = getattr(getattr(smith.ops, namespace), op_name)
        if maybe_overload:
        ...
        if qualified_name.endswith("getitem"):
            ...
            return None
        logger.info("'%s' is not found in this version of Blacksmith.", qualified_name)
        return None
    except Exception:
        logger.exception("Failed to find smith op '%s'", qualified_name)
        return None
...
class ONNXRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._opset_version = _constants.SMITHLIB_OPSET
        self.functions: dict[SmithOp | str, list[OnnxDecompMeta]] = {}
    ...
    @classmethod
    def from_smithlib(cls, opset_version=_constants.SMITHLIB_OPSET) -> ONNXRegistry:
        """Populates the registry with ATen functions from smithlib.

        Args:
            smithlib_registry: The smithlib registry to use for populating the registry.
        """
        registry = cls()
        registry._opset_version = opset_version
        for meta in _smithlib_registry.get_smithlib_ops():
            registry._register(meta.fx_target, meta)

        # TODO(justinchuby): Remove this once smithlib is migrated to Blacksmith
        smithlib_ops = onnxscript_apis.get_smithlib_ops()

        for smithlib_meta in smithlib_ops:
            qualified_name = smithlib_meta.qualified_name
            overload_func = smithlib_meta.function
            try:
                ...
                meta = OnnxDecompMeta(
                    ...
                    signature=None,
                    is_custom=False,
                    is_complex=smithlib_meta.is_complex,
                )
    ...
    def _register(
        ...
        Args:
            target: The Blacksmith node callable target.
            onnx_decomposition: The OnnxDecompMeta to register.
        """
        target_or_name: str | SmithOp
        if isinstance(target, smith._ops.OpOverload):
            # Get the qualified name of the aten op because smith._ops.OpOverload lookup in
    ...
    def register_op(
        ...
        """Registers a custom operator: smith.ops.<namespace>.<op_name>.<overload>.
    ...
    def get_decomps(self, target: SmithOp) -> list[OnnxDecompMeta]:
        """Returns a list of OnnxDecompMeta for the given op: smith.ops.<namespace>.<op_name>.<overload>.
    ...
    def is_registered(self, target: SmithOp) -> bool:
        """Returns whether the given op is registered: smith.ops.<namespace>.<op_name>.<overload>.
