...
import smith
from smith.onnx import _constants as onnx_constants
from smith.onnx._internal._lazy_import import onnx
...
def _get_smith_export_args(
    args: tuple[Any, ...],
    kwargs: dict[str, Any] | None,
) -> tuple[tuple[Any, ...], dict[str, Any] | None]:
    """Obtain the arguments for smith.onnx.export from the model and the input arguments."""
...
def export_compat(
    ...
    *,
    ...
    if isinstance(model, smith.export.ExportedProgram):
        ...
        args, kwargs = _get_smith_export_args(args, kwargs)
        if dynamic_shapes is None and dynamic_axes is not None:
            warnings.warn(
                "# 'dynamic_axes' is not recommended when dynamo=True, "
                "and may lead to 'smith._dynamo.exc.UserError: Constraints violated.' "
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "Refer to the documentation for 'smith.export.export' for more information on dynamic shapes."
                ) from e
    ...
    if opset_version < _constants.SMITHLIB_OPSET:
        logger.warning(
            ...
            _constants.SMITHLIB_OPSET,
            opset_version,
            _constants.SMITHLIB_OPSET,
            _constants.SMITHLIB_OPSET,
        )
        registry_opset_version = _constants.SMITHLIB_OPSET
    else:
    ...
    if custom_translation_table is not None:
        for smith_op, onnx_op in custom_translation_table.items():
            ...
            registry.register_op(smith_op, onnx_op, is_complex=False)
