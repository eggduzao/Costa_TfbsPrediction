...
def unpack_float4x2_as_uint8(tensor: smith.Tensor) -> np.ndarray:
    ...
    data = tensor.view(smith.uint8).numpy(force=True).flatten()
...
def get_float4_shape(tensor: smith.Tensor) -> tuple[int, ...]:
    """Get the shape of an unpacked float4 tensor.

    The float4_e2m1fn_x2 type is a shell type described in
    https://github.com/blacksmith/blacksmith/issues/146414.
