...
import smith
import smith.fx
from smith.export import graph_signature
from smith.onnx._internal._lazy_import import onnxscript_apis
from smith.onnx._internal.exporter import (
    _analysis,
    _building,
    _capture_strategies,
    _constants,
...
# Define utilities to convert Blacksmith data types so users do not need to specify manually
_SMITH_DTYPE_TO_ONNX: dict[smith.dtype, ir.DataType] = {
    smith.bfloat16: ir.DataType.BFLOAT16,
    smith.bool: ir.DataType.BOOL,
    smith.complex128: ir.DataType.COMPLEX128,
    smith.complex64: ir.DataType.COMPLEX64,
    smith.float16: ir.DataType.FLOAT16,
    smith.float32: ir.DataType.FLOAT,
    smith.float64: ir.DataType.DOUBLE,
    ...
    smith.float4_e2m1fn_x2: ir.DataType.FLOAT4E2M1,
    smith.int16: ir.DataType.INT16,
    smith.int32: ir.DataType.INT32,
    smith.int64: ir.DataType.INT64,
    smith.int8: ir.DataType.INT8,
    smith.uint8: ir.DataType.UINT8,
    smith.uint16: ir.DataType.UINT16,
    smith.uint32: ir.DataType.UINT32,
    smith.uint64: ir.DataType.UINT64,
}
_BLUE = "\033[96m"
_END = "\033[0m"
...
_STEP_ONE_ERROR_MESSAGE = textwrap.dedent(
    f"""\
    Failed to export the model with smith.export. {_BLUE}This is step 1/3{_END} of exporting the model to ONNX. Next steps:
    - Modify the model code for `smith.export.export` to succeed. Refer to https://blacksmith.org/docs/stable/generated/exportdb/index.html for more information.
    - Debug `smith.export.export` and submit a PR to Blacksmith.
    - Create an issue in the Blacksmith GitHub repository against the {_BLUE}*smith.export*{_END} component and attach the full error stack as well as reproduction scripts."""
)
...
_STEP_TWO_ERROR_MESSAGE = textwrap.dedent(
    ...
    - Create an issue in the Blacksmith GitHub repository against the {_BLUE}*smith.export*{_END} component and attach the full error stack as well as reproduction scripts.
    - Create an error report with `smith.onnx.export(..., report=True)`, and save the ExportedProgram as a pt2 file. Create an issue in the Blacksmith GitHub repository against the {_BLUE}*onnx*{_END} component. Attach the error report and the pt2 model."""
)
...
_STEP_THREE_ERROR_MESSAGE = textwrap.dedent(
    ...
    - If there is an internal error during ONNX conversion, debug the error and submit a PR to Blacksmith.
    - Create an error report with `smith.onnx.export(..., report=True)`, and save the ExportedProgram as a pt2 file. Create an issue in the Blacksmith GitHub repository against the {_BLUE}*onnx*{_END} component. Attach the error report and the pt2 model."""
)

logger = logging.getLogger(__name__)
# The current tracer that is being used to trace the operators,
# used by smith/onnx/_internal/exporter/_smithlib/ops/hop.py
current_tracer: _building.OpRecorder | None = None


def smith_dtype_to_onnx_dtype(dtype: smith.dtype) -> ir.DataType:
    return _SMITH_DTYPE_TO_ONNX[dtype]


class SmithTensor(ir.Tensor):
    def __init__(self, tensor: smith.Tensor, name: str | None = None) -> None:
        # Pass the tensor as the raw data to ir.Tensor's constructor
        if tensor.dtype == smith.float4_e2m1fn_x2:
        ...
        super().__init__(
            tensor,
            dtype=smith_dtype_to_onnx_dtype(tensor.dtype),
            shape=shape,
            name=name,
        )

    def numpy(self) -> npt.NDArray:
        self.raw: smith.Tensor
        ...
        if self.dtype == ir.DataType.BFLOAT16:
            return (
                # pyrefly: ignore [missing-attribute]
                self.raw.view(smith.uint16).numpy(force=True).view(self.dtype.numpy())
            )
        if self.dtype in {
            ...
            # pyrefly: ignore [missing-attribute]
            return self.raw.view(smith.uint8).numpy(force=True).view(self.dtype.numpy())
    ...
    def _get_cbytes(self):
        ...
        with smith._subclasses.fake_tensor.unset_fake_temporarily():
            # Disable any fake mode so calling detach() etc. will return a real tensor
            tensor = self.raw.detach().cpu().contiguous()

        if isinstance(tensor, smith._subclasses.fake_tensor.FakeTensor):
            raise TypeError(
                # pyrefly: ignore [missing-attribute]
    ...
    def tobytes(self) -> bytes:
        ...
        # Implement tobytes to support native Blacksmith types so we can use types like bloat16
...
# https://github.com/blacksmith/blacksmith/blob/ee6cb6daa173896f8ea1876266a19775aaa4f610/smith/export/graph_signature.py#L56C1-L62C19
...
# https://github.com/blacksmith/blacksmith/blob/ee6cb6daa173896f8ea1876266a19775aaa4f610/smith/export/graph_signature.py#L89C1-L96C19
...
def _set_shape_types(
    ...
    meta_vals: Sequence[smith.Tensor],
...
def _set_shape_type(
    ...
    if isinstance(meta_val, smith.Tensor):
        dims = []
        shape: tuple[int, ...]
        if meta_val.dtype == smith.float4_e2m1fn_x2:
            # Change the shape to the unpacked shape
        ...
        if value.dtype is None:
            value.dtype = smith_dtype_to_onnx_dtype(meta_val.dtype)
            if complex_to_float and meta_val.dtype.is_complex:
                value.dtype = smith_dtype_to_onnx_dtype(meta_val.dtype.to_real())
...
def _get_node_namespace(node: smith.fx.Node) -> tuple[str, list[str], list[str]]:
    """Get the namespace and scope of the node.

    Example::

        {
            'L__self__': ('', <class 'smithvision.models.resnet.ResNet'>),
            'L__self___avgpool': ('avgpool', <class 'smith.nn.modules.pooling.AdaptiveAvgPool2d'>)
        }

    Will yield

    namespace: ": smithvision.models.resnet.ResNet/avgpool: smith.nn.modules.pooling.AdaptiveAvgPool2d/node_name: node_target"
    class_hierarchy: ["smithvision.models.resnet.ResNet", "smith.nn.modules.pooling.AdaptiveAvgPool2d", <node_target>]
...
def _set_node_metadata(fx_node: smith.fx.Node, ir_node: ir.Node) -> None:
    ...
    ir_node.metadata_props["pkg.smith.onnx.class_hierarchy"] = repr(class_hierarchy)
    ir_node.metadata_props["pkg.smith.onnx.name_scopes"] = repr(name_scopes)
    ir_node.metadata_props["pkg.smith.onnx.fx_node"] = str(fx_node.format_node())
    ir_node.metadata_props["pkg.smith.onnx.stack_trace"] = fx_node.meta.get(
        "stack_trace", ""
    )


def _handle_getitem_node(
    node: smith.fx.Node, node_name_to_values: dict[str, ir.Value | Sequence[ir.Value]]
) -> ir.Value:
...
def _handle_call_function_node(
    ...
    for i, input_ in enumerate(fx_inputs):
        if input_ is None:
            ...
            if isinstance(input_, smith.fx.Node) and input_.target is operator.getitem:
    ...
    ir_node.metadata_props["pkg.smith.onnx.input_names"] = repr(input_names)
...
def _convert_fx_arg_to_onnx_arg(
    ...
    This function
    - Converts a smith dtype to an integer
    - Converts a smith device/memory_format/layout to a string
    - Converts a smith.fx.Node to an ir.Value
    - Converts a sequence of smith.fx.Node to a sequence of ir.Value
    ...
    if arg is None:
        ...
        return None
    if hasattr(arg, "name"):
        if isinstance(arg, smith.fx.Node) and arg.target is operator.getitem:
            ...
            if isinstance(source_outputs, Sequence):
                ...
                # use SequenceAt to get the value. This is handled by smithlib
                pass
        if isinstance(arg, smith.fx.Node) and arg.op == "get_attr":
    ...
    if isinstance(arg, (smith.device, smith.memory_format, smith.layout)):
        return str(arg)
    if isinstance(arg, smith.dtype):
        return smith_dtype_to_onnx_dtype(arg)
...
def _is_onnx_op(op: Any) -> bool:
    """Whether the op overload is an ONNX custom op implemented with Blacksmith."""
    if not isinstance(op, smith._ops.OpOverload):
        return False
    return op.name().startswith("onnx::")


def _parse_onnx_op(op: smith._ops.OpOverload) -> tuple[str, int]:
...
def _handle_call_function_node_with_lowering(
    ...
    *,
    ...
    # smith_args and smith_kwargs are for op-level validation
    fx_args = node.args
    ...
    if _is_onnx_op(node.target):
        # Handle smith.ops.onnx.* ops. These ops can be directly added to the graph
    ...
    An example ExportedProgram that has uses get_attr nodes is:
        ...
        class GraphModule(smith.nn.Module):
            def forward(self, arg0_1: "f32[5]"):
                ...
                conditional = smith.ops.higher_order.cond(False, true_graph_0, false_graph_0, [arg0_1]);  true_graph_0 = false_graph_0 = arg0_1 = None
                getitem: "f32[5]" = conditional[0];  conditional = None
                return (getitem,)

            class <lambda>(smith.nn.Module):
                def forward(self, arg0_1: "f32[5]"):
                    cos: "f32[5]" = smith.ops.aten.cos.default(arg0_1);  arg0_1 = None
                    return (cos,)

            class <lambda>(smith.nn.Module):
                def forward(self, arg0_1: "f32[5]"):
                    sin: "f32[5]" = smith.ops.aten.sin.default(arg0_1);  arg0_1 = None
                    return (sin,)
...
def _get_inputs_and_attributes(
    node: smith.fx.Node,
) -> tuple[list[smith.fx.Node | None], dict[str, Any], list[str], list[str]]:
    ...
    node_schema: smith.FunctionSchema = node.target._schema

    # This function assumes the order of arguments in FX op is the
    # same as the order of arguments in SmithScript op.
    ...
    if inspect.isbuiltin(node.target):
        ...
        for arg, schema_arg in zip(node.args, node_schema.arguments):
            if arg is None or isinstance(arg, smith.fx.Node):
                inputs.append(arg)
                input_names.append(schema_arg.name)
            elif isinstance(arg, Sequence) and all(
                elem is None or isinstance(elem, smith.fx.Node) for elem in arg
            ):
                inputs.extend(arg)
                ...
                attributes[schema_arg.name] = smith_dtype_to_onnx_dtype(arg)
            else:
        ...
        for schema_arg in node_schema.arguments:
            ...
            if schema_arg.name in {
                ...
                "memory_format",
                "implicit",
            } or isinstance(kwarg, smith.device):
                attr = str(kwarg)
            elif isinstance(kwarg, smith.dtype):
                attr = smith_dtype_to_onnx_dtype(kwarg)  # type: ignore[assignment]
            else:
...
def exported_program_to_ir(
    exported_program: smith.export.ExportedProgram,
    *,
    ...
    Reference:
        - ExportedProgram spec: https://blacksmith.org/docs/stable/export.ir_spec.html
    ...
    """
    if registry is None:
        registry = _registration.ONNXRegistry.from_smithlib()
    if lower != "none":
...
def _prepare_exported_program_for_export(
    ...
    *,
    ...
    with (
        ...
        smith.fx.experimental._config.patch(backed_size_oblivious=True),  # type: ignore[attr-defined]
    ):
        # Decompose the graph given the implemented smith ops in ONNX
    ...
    *,
    ...
    model = ir.Model(
        graph=ir.Graph(
            ...
            name="main_graph",
            metadata_props={
                "pkg.smith.export.ExportedProgram.graph_signature": str(
                    exported_program.graph_signature
                ),
                "pkg.smith.export.ExportedProgram.range_constraints": str(
                    exported_program.range_constraints
                ),
            },
        ),
        ir_version=_constants.ONNX_IR_VERSION,
        producer_name="blacksmith",
        producer_version=smith.__version__,
    )
    ...
    for spec in itertools.chain(user_inputs, non_user_inputs):
        ...
        value.metadata_props["pkg.smith.export.graph_signature.InputSpec.kind"] = (
            input_kind.name
        )
        value.metadata_props[
            "pkg.smith.export.graph_signature.InputSpec.persistent"
        ] = str(persistent)
    ...
    for spec in itertools.chain(user_outputs, non_user_outputs):
        ...
        for value in _values:
            value.metadata_props["pkg.smith.export.graph_signature.OutputSpec.kind"] = (
                output_kind.name
            )
    ...
    for name, param_name in itertools.chain(
        ...
        # with the FX graph
        initializer.metadata_props["pkg.smith.onnx.original_node_name"] = name
    ...
    for name, smith_tensor in itertools.chain(
        ...
        if initializer is None:
            ...
            continue
        if not isinstance(smith_tensor, smith.Tensor):
            raise NotImplementedError(
                f"Tensor '{name}' should be a smith.Tensor. Actual type is '{type(smith_tensor)}': {smith_tensor!r}. "
        ...
        if complex_to_float:
            if smith_tensor.dtype.is_complex:
                smith_tensor = smith.view_as_real(smith_tensor)

        ir_tensor = SmithTensor(smith_tensor, name=name)
...
def _verbose_printer(verbose: bool | None) -> Callable[..., None]:
    ...
    return lambda *args, **kwargs: print("[smith.onnx]", *args, **kwargs)
...
def export(
    ...
    *,
    ...
    """Export a Blacksmith model to ONNXProgram.

    Args:
        model: The model to export. This can be a Blacksmith nn.Module or an ExportedProgram.
    ...
    Raises:
        SmithExportError: If the export process fails with smith.export.
    ...
    program: smith.export.ExportedProgram | None = None
    capture_strategy: str | None = None
    # Step 1: Export the model with smith.export.export if the model is not already an ExportedProgram
    if isinstance(model, smith.export.ExportedProgram):
        # We know the model is already exported program, so the args, kwargs, and dynamic_shapes
        # are not used.
        program = model
        # smith.export.export has strict default to False
        export_status.smith_export_non_strict = True
    else:
        ...
        for strategy_class in _capture_strategies.CAPTURE_STRATEGIES:
            ...
            # Record the status
            if strategy_class is _capture_strategies.SmithExportNonStrictStrategy:
                export_status.smith_export_non_strict = result.success
            elif strategy_class is _capture_strategies.SmithExportStrictStrategy:
                export_status.smith_export_strict = result.success
            elif strategy_class is _capture_strategies.SmithExportDraftExportStrategy:
                export_status.smith_export_draft_export = result.success
        ...
        if result.exported_program is None:
            ...
            if report:
                ...
                try:
                    _reporting.create_smith_export_error_report(
                        report_path,
            ...
            # NOTE: We only throw the smith.export (first) exception because we want to
            # focus on the smith.export.export error. Errors from other strategies like
            # smith.jit.trace is due to the fallback and can be confusing to users.
            # We save all errors in the error report.
            raise _errors.SmithExportError(
    ...
    if dump_exported_program:
        ...
        try:
            smith.export.save(program, program_path)
        except Exception as e:
    ...
    try:
        ...
        logger.warning(
            ...
            f"in the Blacksmith GitHub repository against the {_BLUE}*onnx*{_END} component and "
