...
For example, if we have smith.ops.aten.add(Tensor, 1.0), we need to promote 1.0
to the same type as Tensor. The same thing needs to work for
smith.ops.aten.add(1.0, Tensor) as well, which means we need a mechanism to`
"""
...
import smith
from smith.onnx._internal.exporter import _errors, _schemas, _tensors
...
# Logic for adapting inputs from general Python or Blacksmith inputs to ONNX ir.Value
    ...
    for param in signature.params:
        if isinstance(param, _schemas.Parameter):
            ...
            if isinstance(attribute, int) and param.type == ir.AttributeType.FLOAT:
                # Convert the attribute to float if needed. This happens in Blacksmith
...
def _resolve_parameter_dtypes(
    ...
    for name, arg in named_inputs.items():
        ...
        if isinstance(arg, (int, float, bool, str, Sequence, smith.Tensor)):
...
class OpRecorder(evaluator.Evaluator):
    ...
    def eval_function(  # type: ignore[override]
        ...
        try:
            ...
            # TODO(after smithlib migration): Remove traceable function handling
