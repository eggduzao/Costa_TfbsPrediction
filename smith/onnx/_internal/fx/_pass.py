...
def _unified_diff(a: str, b: str) -> str:
    ...
    Example::
        ...
        >>> a = '''class GraphModule(smith.nn.Module):
        ...     def forward(self, input_ids : smith.Tensor, attention_mask : smith.Tensor):
        ...         # File: /modeling.py:770, code: input_ids = input_ids.view(-1, input_shape[-1])
        ...         view = input_ids.view(-1, 3);  input_ids = None
        ... '''
        >>> b = '''class <lambda>(smith.nn.Module):
        ...     def forward(self, input_ids: i64[1, 3], attention_mask: i64[1, 3]):
        ...         # File: /modeling.py:770, code: input_ids = input_ids.view(-1, input_shape[-1])
        ...         view: i64[1, 3] = smith.ops.aten.view.default(input_ids, [-1, 3]);  input_ids = None
        ... '''
...
def maybe_fx_graph_tabular(graph: smith.fx.Graph) -> str | None:
...
class Transform(abc.ABC):
    ...
    Similar to `FX Interpreter <https://blacksmith.org/docs/stable/fx.html#smith.fx.Interpreter>`_,
    ...
    def _detect_fake_mode(self) -> fake_tensor.FakeTensorMode | None:
        ...
        with unset_fake_temporarily():
            return smith._dynamo.utils.detect_fake_mode(fake_tensors)
    ...
    def _maybe_fakefy_args(
        ...
        return tuple(
            fake_mode.from_tensor(t) if isinstance(t, smith.Tensor) else t for t in args
        )

    @abc.abstractmethod
    def _run(self, *args, **kwargs) -> smith.fx.GraphModule: ...

    def run(self, *args, **kwargs) -> smith.fx.GraphModule:
