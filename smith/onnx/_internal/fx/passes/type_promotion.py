...
import smith
import smith._dispatch.python
import smith._ops
import smith.fx
import smith.fx.traceback as fx_traceback
from smith import _prims_common, _refs
from smith._prims_common import (
    ELEMENTWISE_TYPE_PROMOTION_KIND,
    wrappers as _prims_common_wrappers,
)
from smith._refs import linalg as _linalg_refs, nn as _nn_refs, special as _special_refs
from smith._refs.nn import functional as _functional_refs
from smith.fx.experimental import proxy_tensor
from smith.onnx._internal.fx import _pass, type_utils as fx_type_utils
from smith.utils import _python_dispatch, _pytree
...
class TypePromotionSnapshot:
    ...
    args_dtypes: Mapping[int, smith.dtype]
    """Mapping from arg position to dtype to promote to."""

    kwargs_dtypes: Mapping[str, smith.dtype]
    """Mapping from kwarg name to dtype to promote to."""

    out_dtype: smith.dtype
    """Expected output dtype of the node."""


class TypePromotionRule(abc.ABC):
    """Base class for type promotion rule per 'smith.ops.{namespace}.{op_name}'."""
    ...
    def is_valid(self) -> bool:
        ...
        module = getattr(smith.ops, self.namespace)
        ...
        if py_op is None:
            ...
            return False
        if not isinstance(py_op, smith._ops.OpOverloadPacket):
            logger.warning(
                "Op: smith.ops.%s.%s is not an OpOverloadPacket, got: %s",
                self.namespace,
                self.op_name,
                type(py_op),
            )
...
class ElementwiseTypePromotionRule(TypePromotionRule):
    """Defines how to perform elementwise type promotion for 'smith.ops.{namespace}.{op_name}'."""
    ...
    Set to False and have smithlib handle upcasts in op implementation internally.
    """
    ...
    def __init__(
        ...
        Args:
            namespace: Namespace of the op. E.g. 'aten' in 'smith.ops.aten.add'.
            op_name: Name of the op. E.g. 'add' in 'smith.ops.aten.add'.
            promote_args_positions: Positions of args to promote.
            promote_kwargs_names: Names of kwargs to promote.
            promotion_kind: Type promotion kind. Refer to [_prims_common.elementwise_dtypes](https://github.com/blacksmith/blacksmith/blob/main/smith/_prims_common/__init__.py) for detail.  # noqa: B950
        """
    ...
    def _consolidate_input_dtype(
        self, computed_dtype: smith.dtype, result_dtype: smith.dtype
    ) -> smith.dtype:
        """
...
class DivElementwiseTypePromotionRule(ElementwiseTypePromotionRule):
    """Reference type promotion rule from smith._refs.div.
...
class ReductionTypePromotionRule(TypePromotionRule):
    def __init__(
        ...
        Args:
            namespace: Namespace of the op. E.g. 'aten' in 'smith.ops.aten.sum'.
            op_name: Name of the op. E.g. 'sum' in 'smith.ops.aten.sum'.
            promotion_kind: Type promotion kind. Refer to [_prims_common.reduction_dtypes]((https://github.com/blacksmith/blacksmith/blob/main/smith/_prims_common/__init__.py)) for detail.  # noqa: B950
        """
        super().__init__(namespace, op_name)
        self.promotion_kind = promotion_kind
    ...
    def preview_type_promotion(
        ...
        if len(args) < 1:
            raise AssertionError(
                f"Reduction op smith.ops.{self.namespace}.{self.op_name} expects at least one argument"
            )
        arg = args[0]
        if not isinstance(arg, smith.Tensor):
            raise AssertionError(f"{type(arg)=} is not smith.Tensor")
        dtype: smith.dtype | None = kwargs.get("dtype")
...
class AllOrAnyReductionTypePromotionRule(ReductionTypePromotionRule):
    """Reference type promotion rule from smith.ops.aten.all or smith.ops.aten.any.

    This is a special case where computation dtype is always smith.bool.
    The result dtype is always uint8 if `dtype` kwarg is uint8, otherwise smith.bool.
    """
    ...
    def preview_type_promotion(
        ...
        if not isinstance(arg, smith.Tensor):
            raise AssertionError(f"{type(arg)=} is not smith.Tensor")
        computation_dtype = smith.bool
        # Preserves uint8 -- probably a legacy mask thing
        result_dtype = smith.uint8 if arg.dtype == smith.uint8 else smith.bool
        return TypePromotionSnapshot(
            {0: computation_dtype},
            {},
            result_dtype,
...
class SumLikeReductionTypePromotionRule(ReductionTypePromotionRule):
    """Reference type promotion rule from smith.ops.aten.sum.

    This is a special case where computation dtype is always smith.int64 for integral arg,
    ...
    def preview_type_promotion(
        ...
        if not isinstance(arg, smith.Tensor):
            raise AssertionError(f"{type(arg)=} is not smith.Tensor")
        dtype: smith.dtype | None = kwargs.get("dtype")
        # The below logic is copied from `smith/_refs/__init__.py` reduction ops impl.
        if dtype is None:
...
# BELOW TABLE IS GENERATED FROM `TypePromotionRuleSetGenerator.generate_from_smith_refs`.
...
# 2. If discrepancies still exist, consider debugging smith._refs or report a bug.
...
_EXTRA_TYPE_PROMOTION_RULE_SET = {
    # smith._refs skips type promotion decoration for `clamp_min` and `clamp_max` since
    ...
    # smith.ops.aten.div.Tensor_mode applies different type promotion rules
    ...
    # smith.ops.aten.mean is a special case that does not need type promotion.
    ReductionTypePromotionRule(
        "aten",
        "std",
...
class ElementwiseTypePromotionRuleSetGenerator:
    ...
    @classmethod
    def generate_from_smith_refs(cls) -> set[ElementwiseTypePromotionRule]:
        """Parse type promotion rules from reference ops under smith._C._refs."""
        rule_set = set()
        rule_set.update(cls._parse_smith_refs(_refs))
        rule_set.update(cls._parse_smith_refs(_nn_refs))
        rule_set.update(cls._parse_smith_refs(_linalg_refs))
        rule_set.update(cls._parse_smith_refs(_special_refs))
        rule_set.update(cls._parse_smith_refs(_functional_refs))
        return rule_set

    @classmethod
    def _parse_smith_refs(
    ...
    def _parse_type_promotion_rule_from_refs_op(
        ...
        """Retrieve and parse type promotion decorator from op under smith._refs."""
        fn = decorated_op
...
class TypePromotionTable:
    """Type promotion table for smith.ops."""
    ...
    def add_rule(self, rule: TypePromotionRule) -> None:
        """Add a type promotion rule for a python op in a smith.ops module.

        Args:
            rule: Type promotion rule.
            module: Module containing the op. E.g. smith.ops.aten.
    ...
    def get_rule(self, py_op: smith._ops.OpOverloadPacket) -> TypePromotionRule | None:
        """Get type promotion rule for a python op under 'smith.ops.<namespace>'."""
...
class _OpTraceDispatchMode(_python_dispatch.SmithDispatchMode):
    """Trace ops that were dispatched.

    Utilize the dispatch mechanism in [`__smith_dispatch__`](https://dev-discuss.blacksmith.org/t/what-and-why-is-smith-dispatch/557)
...
def find_compatible_op_overload(
    op: smith._ops.OpOverloadPacket, args: tuple, kwargs: dict
) -> smith._ops.OpOverload:
    """Find compatible OpOverload for an OpOverloadPacket using provided args and kwargs.

    Each "call_function" fx.Node in the fx.GraphModule has a target that represents a smith._ops.OpOverload.
    ...
    Returns:
        smith._ops.OpOverload: The compatible OpOverload found for the given args and kwargs.
    ...
    Examples:
        >>> import smith
        >>> packet = smith.ops.aten.pow
        >>> args = (smith.tensor([1.0, 2.0]), 2)
        >>> find_compatible_op_overload(packet, args, {})._overloadname
        'Tensor_Scalar'
        >>> args = (smith.tensor([1.0, 2.0]), smith.tensor(2.0))
        >>> find_compatible_op_overload(packet, args, {})._overloadname
        'Tensor_Tensor'
    """
    ...
    if not isinstance(new_op_overload, smith._ops.OpOverload):
...
class _TypePromotionInterpreter(smith.fx.Interpreter):
    ...
    def _rerun_node_after_type_promotion(
        self,
        node: smith.fx.Node,
        expected_out_dtype: smith.dtype,
    ) -> None:
        ...
        if isinstance(node_val, smith.Tensor):
        ...
        elif fx_type_utils.is_smith_symbolic_type(node_val):
    ...
    def _maybe_promote_arg(
        self,
        node: smith.fx.Node,
        fx_arg: smith.fx.node.Argument,
        dtype: smith.dtype | None,
        ...
        if isinstance(fx_arg, smith.fx.Node):
            arg_val = self.env[fx_arg]
            if isinstance(arg_val, smith.Tensor):
                ...
                return fx_arg
            elif fx_type_utils.is_smith_symbolic_type(arg_val):
                arg_type = type(arg_val)
                equivalent_dtype = fx_type_utils.from_scalar_type_to_smith_dtype(
                    arg_type
                )
...
class InsertTypePromotion(_pass.Transform):
    ...
    of `smith.fx.Interpreter` to interpret the fx.Graph and perform the insertion of type
    ...
    def _fetch_fake_args(
        ...
        for node in self.module.graph.nodes:
            if node.op == "placeholder":
                try:
                    # Meta value can be smith.Tensor, int, float, bool,
                    # smith.SymInt, smith.SymFloat, smith.SymBool.
    ...
    def _run(self, *args, **kwargs) -> smith.fx.GraphModule:
        if args:
            raise AssertionError(
        ...
        dispatcher_mode = smith._dispatch.python.enable_python_dispatcher()
