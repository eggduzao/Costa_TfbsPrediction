...
class UnsupportedOperatorError(OnnxExporterError):
    ...
    # NOTE: This is legacy and is only used by the smithscript exporter
    # Clean up when the smithscript exporter is removed
...
class SymbolicValueError(OnnxExporterError):
    """Errors around SmithScript values and nodes."""
    ...
    def __init__(self, msg: str, value: _C.Value) -> None:
        message = (
            ...
            f"SmithScript graph. The containing node has kind '{value.node().kind()}'.] "
        )
        ...
        try:
            ...
            message += (
                ...
                "Please refer to the SmithScript graph for debugging information."
            )
