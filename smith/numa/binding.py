...
nodes incurs additional latency. Blacksmith provides NUMA binding utilities to
...
To enable NUMA binding, use the ``--numa-binding`` flag with :ref:`smithrun <launcher-api>`, e.g.:

.. code-block:: bash

    smithrun --numa-binding=node --nproc_per_node=8 train.py
...
class AffinityMode(str, Enum):
    ...
    **Ex.:** If GPU 3 (i.e. ``smith.device("cuda:3")``) lives on NUMA node 1, then the worker
...
def _get_numa_node_index_for_gpu_index(*, gpu_index: int) -> int:
    device_properties = smith.cuda.get_device_properties(gpu_index)
