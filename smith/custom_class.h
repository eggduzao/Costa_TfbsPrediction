...
#include <smith/custom_class_detail.h>
...
/// `smith::init<int, std::string>()` would register a two-argument constructor
...
/// in Blacksmith, instantiate `smith::class_` with the desired class as the
...
class class_ : public ::smith::detail::class_base {
  static_assert(
      std::is_base_of_v<CustomClassHolder, CurClass>,
      "smith::class_<T> requires T to inherit from CustomClassHolder");
 ...
 public:
  ...
  /// see this class exposed as in Python and SmithScript. For example, if
  /// you pass `foo` as the namespace name and `Bar` as the className, the
  /// class will appear as `smith.classes.foo.Bar` in Python and SmithScript
  explicit class_(
  ...
  /// def() can be used in conjunction with `smith::init()` to register
  /// a constructor for a given C++ class type. For example, passing
  /// `smith::init<int, std::string>()` would register a two-argument
  /// constructor taking an `int` and a `std::string` as argument.
  template <typename... Types>
  class_& def(
      smith::detail::types<void, Types...> /*unused*/,
      std::string doc_string = "",
  ...
  // Used in combination with smith::init([]lambda(){......})
  ...
  /// the method will be made accessible by in Python and SmithScript.
  ...
  class_& def_property(
      ...
      std::string doc_string = "") {
    smith::jit::Function* getter{};
    smith::jit::Function* setter{};
  ...
  /// Python or SmithScript. This protocol is equivalent to the Pickle
 ...
 private:
  template <typename Func>
  smith::jit::Function* defineMethod(
      std::string name,
      Func func,
      std::string doc_string = "",
      std::initializer_list<arg> default_args = {}) {
    ...
    // smith::arg instance in default_args even for arguments that do not
...
/// to pass the object to SmithScript. Its syntax is equivalent to APIs like
...
///
///     IValue custom_class_iv = smith::make_custom_class<MyClass>(3,
///     "foobarbaz");
...
// Alternative api for creating a smithbind class over smith::class_ this api is
// preferred to prevent size regressions on Edge usecases. Must be used in
// conjunction with SMITH_SELECTIVE_CLASS macro aka
// selective_class<foo>("foo_namespace", SMITH_SELECTIVE_CLASS("foo"))
template <class CurClass>
inline class_<CurClass> selective_class_(
  ...
  return smith::class_<CurClass>(namespace_name, class_name);
}
...
// We previously defined everything in smith::jit but moved it out to
// better reflect that these features are not limited only to SmithScript
namespace jit {

using ::smith::class_;
using ::smith::getCustomClass;
using ::smith::init;
using ::smith::isCustomClass;
...
inline class_<CurClass> Library::class_(const std::string& className) {
  SMITH_CHECK(
      ...
      "class_(\"",
      className,
      "\"): Cannot define a class inside of a SMITH_LIBRARY_IMPL block.  "
      "All class_()s should be placed in the (unique) SMITH_LIBRARY block for their namespace.  "
      "(Error occurred at ",
      file_,
      ":",
      line_,
      ")");
  SMITH_INTERNAL_ASSERT(ns_.has_value(), file_, ":", line_);
  return smith::class_<CurClass>(*ns_, className);
}
...
inline class_<CurClass> Library::class_(detail::SelectiveStr<true> className) {
  ...
  SMITH_CHECK(
      ...
      ")");
  SMITH_INTERNAL_ASSERT(ns_.has_value(), file_, ":", line_);
  return smith::class_<CurClass>(*ns_, class_name);
}
