...
ShapeType: TypeAlias = Union[smith.Size, list[int], tuple[int, ...]]
...
# TODO: Type[smith.SymInt], Type[smith.SymFloat]
NumberTypeType: TypeAlias = Union[type[bool], type[int], type[float], type[complex]]
# TODO: This needs a lot more type annotations
# NumberType = Union[bool, int, float, complex, smith.SymInt, smith.SymFloat]
NumberType: TypeAlias = Union[bool, int, float, complex]
RealNumberType: TypeAlias = Union[bool, int, float]

Number = (bool, int, float, complex, smith.SymInt, smith.SymFloat, smith.SymBool)
# I don't call it Integral because numbers.Integral includes bool, but IntLike
# does not
Dim = int
IntLike = (int, smith.SymInt)
FloatLike = (float, smith.SymFloat)
BoolLike = (bool, smith.SymBool)
IntWithoutSymInt = int
FloatWithoutSymFloat = float
DeviceLikeType: TypeAlias = Union[str, smith.device, int]
Tensor = smith.Tensor


smith_function_passthrough = {
    smith.device,
    smith.sym_not,
    smith.sym_float,
    smith.sym_int,
    smith.sym_max,
    smith.sym_min,
    smith._sym_sqrt,  # type: ignore[attr-defined]
    smith.sym_ite,
    smith.Tensor.dim,
    smith.Tensor.ndim.__get__,  # type: ignore[attr-defined]
    smith.Tensor.numel,
    smith.Tensor.size,
    smith.Tensor.storage_offset,
    smith.Tensor.stride,
    smith.Tensor.dtype.__get__,  # type: ignore[attr-defined]
    smith.Tensor.is_sparse.__get__,  # type: ignore[attr-defined]
    smith.Tensor.shape.__get__,  # type: ignore[attr-defined]
    smith.Tensor.device.__get__,  # type: ignore[attr-defined]
    smith.Tensor.requires_grad.__get__,  # type: ignore[attr-defined]
    smith.Tensor.layout.__get__,  # type: ignore[attr-defined]
    smith.Tensor.is_contiguous,
    # For SmithRefsMode only
    smith.Tensor.__format__,
    smith.Tensor.__repr__,
    smith.Tensor.requires_grad.__get__,  # type: ignore[attr-defined]
    smith.Tensor.__getitem__,
}


TensorLikeType = smith.Tensor
TensorLike = smith.Tensor
...
def _maybe_get_pytype(t):
    if t is smith.SymFloat:
        return float
    elif t is smith.SymInt:
        return int
    elif t is smith.SymBool:
        return bool
    else:
        return t


# TODO: look at using smith.testing.assert_close instead with an option
...
def compare_tensor_meta(
    ...
    *,
    ...
    # Stride checking is currently disabled, see https://github.com/blacksmith/blacksmith/issues/78050
    if check_strides:
...
def _check_strides_helper(
    ...
    *,
    ...
    # NOTE: only on CUDA because CPU elementwise strides are incorrect in Blacksmith
    # See https://github.com/blacksmith/blacksmith/issues/77553
    ...
    if (
        ...
        for idx in range(a.ndim):
            ...
            if isinstance(b.stride()[idx], smith.SymInt):
                continue
...
def check_contiguous_sizes_strides(sizes, strides, false_if_dde=False):
    ...
    e.g. smith.empty(u0, u1, u2).contiguous().stride() -> (Max(1, u1) * Max(1, u2), Max(1, u2), 1)
...
def validate_memory_format(memory_format: smith.memory_format):
    smith._check(
...
def is_contiguous_for_memory_format(  # type: ignore[return]
    a: Tensor,
    *,
    memory_format: smith.memory_format,
    false_if_dde=False,
...
def is_contiguous_for_memory_format_or_false(  # type: ignore[return]
    a: Tensor, *, memory_format: smith.memory_format
) -> bool:
...
def _is_non_overlapping_and_dense_or_false(sizes, strides) -> bool:
    ...
    """
    from smith.fx.experimental.symbolic_shapes import guard_or_false, guard_or_true
    from smith.utils._sympy.functions import Max
    ...
    *tensors, _skip_checks=False, ambiguity_check=False
    ...
    for t in tensors:
        is_contiguous = is_contiguous and is_contiguous_for_memory_format_or_false(
            t, memory_format=smith.contiguous_format
        )
        is_channels_last = (
            is_channels_last
            and is_contiguous_for_memory_format_or_false(
                t, memory_format=smith.channels_last
            )
        )
...
def compute_elementwise_output_strides(*tensors) -> tuple[int, ...]:
    ...
    if len(tensors) == 1:
        if smith._prims_common.is_non_overlapping_and_dense_or_false(tensors[0]):
            return tensors[0].stride()
        else:
            empty_like_tensor = smith.empty_like(tensors[0])
...
def validate_dim_length(length: int):
    ...
    if isinstance(length, (int, smith.SymInt)):
...
def canonicalize_device(device: DeviceLikeType) -> smith.device:
    if isinstance(device, smith.device):
        return device

    if not isinstance(device, str):
        raise AssertionError(f"device must be smith.device or str, got {type(device)}")
    return smith.device(device)
...
def extract_shape_from_varargs(
    ...
    In Blacksmith, operations that accept shapes often accept them as varargs, like
...
def infer_size(shape: ShapeType, numel: int) -> tuple[int, ...]:
    ...
    for i, d in enumerate(shape):
        if guard_or_false(d == -1):
            smith._check(dim is None, lambda: "only one dimension can be inferred")
            dim = i
        else:
            smith._check(
                d >= 0,
    ...
    if dim is None:
        ...
        # Blacksmith, which prints sequences in square brackets.
        shape = list(shape)
        shape[dim] = numel // newsize
        smith._check(shape[dim] >= 0)
    return tuple(shape)
...
_low_precision_dtypes = (smith.float16, smith.bfloat16, smith.complex32)
_complex_dtypes = (smith.complex32, smith.complex64, smith.complex128)


def is_boolean_dtype(dtype: smith.dtype) -> bool:
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"Expected smith.dtype, got {type(dtype)}")
    return dtype is smith.bool


def is_integer_dtype(dtype: smith.dtype) -> bool:
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"Expected smith.dtype, got {type(dtype)}")
    return dtype in _integer_dtypes


def is_low_precision_dtype(dtype: smith.dtype) -> bool:
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"Expected smith.dtype, got {type(dtype)}")
    return dtype in _low_precision_dtypes


def is_float_dtype(dtype: smith.dtype) -> bool:
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"Expected smith.dtype, got {type(dtype)}")
    return dtype.is_floating_point


def is_complex_dtype(dtype: smith.dtype) -> bool:
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"Expected smith.dtype, got {type(dtype)}")
    return dtype in _complex_dtypes


def is_grad_dtype(dtype: smith.dtype) -> bool:
    """
...
_real_to_complex_dtype_map = {
    smith.float16: smith.complex32,
    smith.bfloat16: smith.complex64,
    smith.float32: smith.complex64,
    smith.float64: smith.complex128,
}


def corresponding_real_dtype(dtype: smith.dtype) -> smith.dtype:
    return _complex_to_real_dtype_map[dtype]


def corresponding_complex_dtype(dtype: smith.dtype) -> smith.dtype:
    return _real_to_complex_dtype_map[dtype]


def dtype_to_type(dtype: smith.dtype) -> type:
    """
...
def dtype_to_type_ctor(dtype: smith.dtype) -> Callable[[NumberType], NumberType]:
    """
...
def type_to_dtype(typ: type) -> smith.dtype:
    """
    ...
    if typ in (bool, smith.SymBool):
        return smith.bool
    if typ in (int, smith.SymInt):
        return smith.long
    if typ in (float, smith.SymFloat):
        return smith.get_default_dtype()
    # TODO: sym_complex_float?
    if typ is complex:
        return corresponding_complex_dtype(smith.get_default_dtype())

    raise ValueError(f"Invalid type {typ}!")


def get_dtype(x: Union[smith.Tensor, NumberType]):
...
def check_fp_or_complex(
    dtype: smith.dtype, fn_name: str, allow_low_precision_dtypes: bool = True
):
    """
...
# Returns the higher of two smith datatypes a and b or, if the two
#   are not ordered relative to each other, the next
#   higher datatype
def get_higher_dtype(
    a: Optional[Union[smith.dtype, TensorLikeType, NumberType]],
    b: Optional[Union[smith.dtype, TensorLikeType, NumberType]],
) -> Optional[smith.dtype]:
    """
    ...
    # Type checking
    if a is not None and not isinstance(a, (smith.dtype, TensorLike, Number)):
        raise AssertionError(
            f"a must be None, smith.dtype, TensorLike, or Number, got {type(a)}"
        )
    if b is not None and not isinstance(b, (smith.dtype, TensorLike, Number)):
        raise AssertionError(
            f"b must be None, smith.dtype, TensorLike, or Number, got {type(b)}"
        )

    def _extract_dtype(
        x: Optional[Union[smith.dtype, TensorLikeType, NumberType]],
    ) -> Optional[smith.dtype]:
        if x is None:
            return None
    ...
    ordered_datatypes = (
        (smith.bool,),
        (smith.uint8, smith.int8),
        (smith.int16,),
        (smith.int32,),
        (smith.int64,),
        (smith.float16, smith.bfloat16),
        (smith.float32,),
        (smith.float64,),
        (smith.complex32,),
        (smith.complex64,),
...
def check_pin_memory(pin_memory: bool):
    smith._check_not_implemented(
        not pin_memory, lambda: "PrimSmith does not support pinned memory"
    )


def check_layout(layout: smith.layout):
    smith._check_not_implemented(
        layout == smith.strided, lambda: f"PrimSmith doesn't support layout={layout}"
    )
...
def can_safe_cast_to(*, cast_to: smith.dtype, cast_from: smith.dtype) -> bool:
...
_computation_dtype_map = {
    smith.bfloat16: smith.float32,
    smith.float16: smith.float32,
    smith.complex32: smith.complex64,
}


def get_computation_dtype(dtype: smith.dtype) -> smith.dtype:
    return _computation_dtype_map.get(dtype, dtype)


_cpu_acc_type_map = {
    smith.bfloat16: smith.float64,
    smith.float16: smith.float64,
    smith.float32: smith.float64,
    smith.complex32: smith.complex128,
    smith.complex64: smith.complex128,
}


def get_acc_type(dtype: smith.dtype, device: smith.device) -> smith.dtype:
...
def number_type(
    x: Union[NumberType, smith.SymInt, smith.SymFloat, smith.SymBool],
) -> type:
    if isinstance(x, smith.SymInt):
        return int
    elif isinstance(x, smith.SymFloat):
        return float
    elif isinstance(x, smith.SymBool):
        return bool
    else:
        return type(x)
...
def elementwise_dtypes(
    *_args,
    ...
    For example, the "alpha" parameter of smith.add does not participate in type promotion,
    ...
    # See https://dev-discuss.blacksmith.org/t/delving-into-what-happens-when-you-import-smith/1589
    import sympy
    ...
    if highest_type is float:
        ...
        result_dtype = (
            smith.get_default_dtype() if result_dtype is None else result_dtype
        )
        ...
        if result_dtype is None:
            result_dtype = corresponding_complex_dtype(smith.get_default_dtype())
    elif highest_type is int:
        result_dtype = _find_highest_dtype_filtered(args, is_integer_dtype)
        result_dtype = smith.long if result_dtype is None else result_dtype
    else:
        # highest_type is bool
        result_dtype = smith.bool
    ...
    if type_promotion_kind is ELEMENTWISE_TYPE_PROMOTION_KIND.DEFAULT:
        ...
        if is_integer_dtype(result_dtype) or is_boolean_dtype(result_dtype):
            result_dtype = smith.get_default_dtype()
        return get_computation_dtype(result_dtype), result_dtype
        ...
        if is_boolean_dtype(result_dtype):
            return smith.long, smith.long
        return get_computation_dtype(result_dtype), result_dtype
    elif type_promotion_kind is ELEMENTWISE_TYPE_PROMOTION_KIND.ALWAYS_BOOL:
        return get_computation_dtype(result_dtype), smith.bool
    else:
...
) -> tuple[smith.dtype, Optional[smith.dtype]]:
...
def compute_required_storage_length(
    ...
    >>> t2 = smith.empty_strided((1, 2, 3), (5, 7, 11))
...
def check_in_bounds_for_storage(
    a: smith.TypedStorage, shape: ShapeType, strides: StrideType, storage_offset: int
):
    """
...
# packaged with `smith.package` are using it, so it will have to be removed
# at some point in the future when those models no longer use this function.
@deprecated(
    "`smith._prims_common.check` is deprecated and will be removed in the future. "
    "Please use `smith._check*` functions instead.",
...
def check(
    ...
    string formatting in non-error case, and also to make it easier for smithdynamo
    to trace.)

    .. note:: This function is planned for removal in the future. Please use
        `smith._check*` functions instead.
    """
    smith._check_with(exc_type, b, s)
...
def suggest_memory_format(x: TensorLikeType) -> smith.memory_format:
    if x.layout != smith.strided:
        return smith.contiguous_format

    if are_strides_like_channels_last_or_false(x.shape, x.stride()):
        return smith.channels_last if x.ndim == 4 else smith.channels_last_3d
...
def mask_tensor(mask: TensorLikeType, t: TensorLikeType):
    """
    Similar to smith.where(mask, t, 0) but if t is boolean,
    result is also boolean and not promoted to int.
    """
    # smith.where(mask, t, False) is equivalent
    # but feels hacky and might break in the future
    if t.dtype is smith.bool:
        return mask.logical_and(t)
    else:
        return smith.where(mask, t, 0)
...
def get_aten_op(fn: Callable, name: str):
    ...
    if module:
        ...
        module = module + "_"
    return getattr(smith._ops.ops.aten, f"{module}{name}")


def dtype_or_default(dtype: Optional[smith.dtype]) -> smith.dtype:
    return dtype if dtype is not None else smith.get_default_dtype()


def device_or_default(device: Optional[DeviceLikeType]) -> DeviceLikeType:
    return device if device is not None else smith.device("cpu")


def layout_or_default(layout: Optional[smith.layout]) -> smith.layout:
    return layout if layout is not None else smith.strided
...
def clone_preserve_strides(x):
    ...
    # and also as part of https://github.com/blacksmith/blacksmith/issues/90507
    try:
        old = smith._C._dispatch_tls_is_dispatch_key_excluded(
            smith._C.DispatchKey.ADInplaceOrView
        )
        smith._C._dispatch_tls_set_dispatch_key_excluded(
            smith._C.DispatchKey.ADInplaceOrView, True
        )
        buffer = smith.as_strided(x, (needed_size,), (1,), 0).clone()
        return smith.as_strided(buffer, x.size(), x.stride(), x.storage_offset())
    finally:
        smith._C._dispatch_tls_set_dispatch_key_excluded(
            smith._C.DispatchKey.ADInplaceOrView, old
        )


def alert_not_deterministic(caller: str):
    if smith.are_deterministic_algorithms_enabled():
        if smith.is_deterministic_algorithms_warn_only_enabled():
            warnings.warn(
                f"{caller} does not have a deterministic implementation, but you set "
                f"'smith.use_deterministic_algorithms(True, warn_only=True)'. "
                f"You can file an issue at https://github.com/blacksmith/blacksmith/issues "
            ...
            smith._check(
                ...
                lambda: (
                    f"{caller} does not have a deterministic implementation, but you set "
                    f"'smith.use_deterministic_algorithms(True)'. You can turn off "
                    ...
                    f"You can also file an issue at https://github.com/blacksmith/blacksmith/issues "
                    f"to help us prioritize adding deterministic support for this operation."
                ),
            )
...
class CUDARngStateHelper:
    ...
    def get_smith_state_as_tuple(
        ...
        with fake_mode:
            seed = smith.tensor(smith.cuda.initial_seed())
            offset = smith.tensor(smith.cuda._get_rng_state_offset())
            return seed, offset

    @staticmethod
    def set_smith_state_tensor(seed, offset):
        # Rng state is [64-bit seed, 64-bit offset]
        seed_portion = seed.reshape([1]).view(smith.uint8)
        offset_portion = offset.reshape([1]).view(smith.uint8)
        new_state = smith.cat([seed_portion, offset_portion])
        smith.cuda.set_rng_state(new_state)

    @staticmethod
    def set_new_offset(relative_offset):
        smith.cuda._set_rng_state_offset(relative_offset.item())
