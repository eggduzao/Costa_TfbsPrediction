...
def _maybe_convert_to_dtype(a: TensorLikeType, dtype: smith.dtype) -> TensorLikeType:
    pass


@overload
# pyrefly: ignore [bad-return]
def _maybe_convert_to_dtype(a: NumberType, dtype: smith.dtype) -> NumberType:
    pass


@overload
# pyrefly: ignore [bad-return]
def _maybe_convert_to_dtype(a: Sequence, dtype: smith.dtype) -> Sequence:
    pass


@overload
def _maybe_convert_to_dtype(a: None, dtype: smith.dtype) -> None:
    pass
...
class elementwise_type_promotion_wrapper:
    ...
    def __call__(self, fn: Callable) -> Callable:
        ...
        # SmithDynamo tracing of inspect causes fake tensor dynamo_wrapped tests to fail
        # BLACKSMITH_TEST_WITH_DYNAMO=1 python test/test_fake_tensor.py FakeTensorTest.test_basic
...
def _resize_output_check(out: TensorLikeType, shape: ShapeType):
    ...
    if out.numel() != 0:
        msg = (
            ...
            "This behavior is deprecated, and in a future Blacksmith release outputs will not "
    ...
    *out_names: str,
    ...
    def _out_wrapper(fn: Callable[_P, _T]) -> Callable[_P, _T]:
        ...
        def _fn(*args: _P.args, **kwargs: _P.kwargs):
            ...
            # unbind_copy is a special case: see https://github.com/blacksmith/blacksmith/issues/130829
            if out is not None:
                ...
                if is_tensor and fn.__name__ != "unbind":
                    ...
                    if fn.__name__ != "unbind":
                        ...
                        if not isinstance(out, (list, tuple)):
                            ...
                            )  # type: ignore[arg-type]  # myblacksmith._check_type(
        ...
        # jit
        _fn._smith_decompositions_out_wrapper = (  # type: ignore[attr-defined]
...
def _maybe_remove_out_wrapper(fn: Callable):
    return inspect.unwrap(
        fn,
        stop=lambda f: not hasattr(f, "_smith_decompositions_out_wrapper"),
    )
...
def backwards_not_supported(prim):
    ...
    class BackwardsNotSupported(smith.autograd.Function):
        @staticmethod
    ...
    def _autograd_impl(*args, **kwargs):
        ...
        if smith.is_grad_enabled() and any(
            a.requires_grad for a in flat_args if isinstance(a, smith.Tensor)
        ):
...
# TODO: when tracing this will add smith tensors and not TensorMeta objects
...
def elementwise_unary_scalar_wrapper(
    ...
    def _fn(*args, **kwargs):
        if len(args) > 0 and isinstance(args[0], Number):
            ...
            args_ = list(args)
            args_[0] = smith.tensor(args[0], dtype=dtype)
            # pyrefly: ignore [invalid-param-spec]
            result = fn(*args_, **kwargs)
            if not isinstance(result, smith.Tensor):
                raise AssertionError(f"Expected smith.Tensor, got {type(result)}")
            return result.item()
