...
from smith._custom_op.impl import (
...
from smith.library import get_ctx
...
def custom_op(qualname, func_or_schema=None):
    ...
    Arguments:
        qualname (str): Should be a string that looks like
            "namespace::operator_name". Operators in Blacksmith need a namespace to
            avoid name collisions; a given operator may only be created once.
            If you are writing a Python library, we recommend the namespace to
            be the name of your top-level module.
        func_or_schema (Union[Callable, str]): Each Blacksmith operator needs a
            schema that tells Blacksmith the types of the inputs/outputs.
    ...
    Example::
        ...
        >>> @smith._custom_ops.custom_op("mylibrary::numpy_sin")
        >>> def numpy_sin(x: Tensor) -> Tensor:
        >>>     raise NotImplementedError
        >>>
        >>> # The custom op is now accessible via the smith.ops module:
        >>> smith.ops.mylibrary.numpy_sin
        >>>
        >>> # Step 2: Register an implementation for various Blacksmith subsystems
        >>>
        >>> # Register an implementation for CPU tensors
        >>> @smith._custom_ops.impl("mylibrary::numpy_sin", device_types="cpu")
        >>> def numpy_sin_impl_cpu(x):
        >>>     return smith.from_numpy(np.sin(x.numpy()))
        >>>
        >>> # Register an implementation for CUDA tensors
        >>> @smith._custom_ops.impl("mylibrary::numpy_sin", device_types="cuda")
        >>> def numpy_sin_impl_cuda(x):
        >>>     return smith.from_numpy(np.sin(x.cpu().numpy())).to(x.device)
        >>>
        >>> x = smith.randn(3)
        >>> smith.ops.mylibrary.numpy_sin(x)  # calls numpy_sin_impl_cpu
        >>>
        >>> x_cuda = x.cuda()
        >>> smith.ops.mylibrary.numpy_sin(x)  # calls numpy_sin_impl_cuda
...
def impl(qualname, *, device_types=("cpu", "cuda"), func=None):
    ...
    Example::
        ...
        >>> # (a function that returns NotImplementedError), from which
        >>> # we will infer the types of the inputs and outputs.
        >>> @smith._custom_ops.custom_op("mylibrary::numpy_cos")
        >>> def numpy_cos(x: Tensor) -> Tensor:
        >>>     raise NotImplementedError
        >>>
        >>> # The custom op is now accessible via the smith.ops module:
        >>> smith.ops.mylibrary.numpy_cos
        >>>
        >>> # Step 2: Register an implementation for various Blacksmith subsystems
        >>>
        >>> # Register an implementation for CPU tensors
        >>> @smith._custom_ops.impl("mylibrary::numpy_cos", device_types="cpu")
        >>> def numpy_cos_impl_cpu(x):
        >>>     return smith.from_numpy(np.cos(x.numpy()))
        >>>
        >>> # Register an implementation for CUDA tensors
        >>> @smith._custom_ops.impl("mylibrary::numpy_cos", device_types="cuda")
        >>> def numpy_cos_impl_cuda(x):
        >>>     return smith.from_numpy(np.cos(x.cpu().numpy())).to(x.device)
        >>>
        >>> x = smith.randn(3)
        >>> smith.ops.mylibrary.numpy_cos(x)  # calls numpy_cos_impl_cpu
        >>>
        >>> x_cuda = x.cuda()
        >>> smith.ops.mylibrary.numpy_cos(x)  # calls numpy_cos_impl_cuda

    """

    def inner(func):
        custom_op = _find_custom_op(qualname, also_check_smith_library=True)
...
def impl_abstract(qualname, *, func=None):
    ...
    The abstract implementation must consist of only Blacksmith operations
    ...
    Examples::
        ...
        >>> @smith._custom_ops.custom_op("mylibrary::custom_linear")
        >>> def custom_linear(x: Tensor, weight: Tensor, bias: Tensor) -> Tensor:
        >>>     raise NotImplementedError
        >>>
        >>> @smith._custom_ops.impl_abstract("mylibrary::custom_linear")
        ...
        >>> @smith._custom_ops.custom_op('mylibrary::custom_nonzero')
        >>> def custom_nonzero(x: Tensor) -> Tensor:
        >>>     ...
        >>>
        >>> @smith._custom_ops.impl_abstract("mylibrary::custom_nonzero")
        >>> def custom_nonzero_abstract(x):
        >>>     # Number of nonzero-elements is data-dependent.
        >>>     # Since we cannot peek at the data in an abstract impl,
        >>>     # we use the ctx object to construct a new symint that
        >>>     # represents the data-dependent size.
        >>>     ctx = smith._custom_ops.get_ctx()
        >>>     nnz = ctx.create_unbacked_symint()
        >>>     shape = [x.dim(), nnz]
        >>>     result = x.new_empty(shape, dtype=smith.long)
        >>>     return result
        >>>
        >>> @smith._custom_ops.impl("mylibrary::custom_nonzero")
        >>> def custom_nonzero_impl(x):
        >>>     x_np = to_numpy(x)
        >>>     res = np.stack(np.nonzero(x_np), axis=1)
        >>>     # unbacked symbolic ints in Blacksmith must be >= 2, so we
    ...
    return smith.library.register_fake(qualname, func, _stacklevel=2)
...
def impl_backward(qualname, output_differentiability=None, *, func=None):
    ...
    During runtime, when you call the operator in a forwards pass, Blacksmith
