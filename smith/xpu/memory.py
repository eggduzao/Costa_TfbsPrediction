...
if not _is_compiled():
    ...
    smith._C.__dict__["_xpu_XPUAllocator"] = _dummy_type("_xpu_XPUAllocator")
    smith._C.__dict__["_XPUMemPool"] = _dummy_type("_XPUMemPool")
    smith._C.__dict__["_xpu_beginAllocateCurrentThreadToPool"] = _dummy_type(
        "_xpu_beginAllocateCurrentThreadToPool"
    )
    smith._C.__dict__["_xpu_endAllocateToPool"] = _dummy_type("_xpu_endAllocateToPool")
    smith._C.__dict__["_xpu_releasePool"] = _dummy_type("_xpu_releasePool")
...
def empty_cache() -> None:
    ...
    .. note::
        :func:`~smith.xpu.empty_cache` doesn't increase the amount of XPU
...
def reset_peak_memory_stats(device: Device = None) -> None:
    ...
    See :func:`~smith.xpu.memory_stats` for details. Peak stats correspond to the
    `"peak"` key in each individual stat dict.

    Args:
        device (smith.device or int or str, optional): selected device. Returns
            statistic for the current device, given by :func:`~smith.xpu.current_device`,
            if :attr:`device` is ``None`` (default).
    """
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_resetPeakMemoryStats(device)
...
def reset_accumulated_memory_stats(device: Device = None) -> None:
    ...
    See :func:`~smith.xpu.memory_stats` for details. Accumulated stats correspond to
    ...
    """
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_resetAccumulatedMemoryStats(device)


def memory_stats_as_nested_dict(device: Device = None) -> dict[str, Any]:
    r"""Return the result of :func:`~smith.xpu.memory_stats` as a nested dictionary."""
    if not is_initialized():
        return {}
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_memoryStats(device)
...
def memory_stats(device: Device = None) -> dict[str, Any]:
    ...
    Args:
        device (smith.device or int or str, optional): selected device. Returns
            statistics for the current device, given by :func:`~smith.xpu.current_device`,
            if :attr:`device` is ``None`` (default).
    """
    result = []
...
def max_memory_allocated(device: Device = None) -> int:
    ...
    this program. :func:`~smith.xpu.reset_peak_memory_stats` can be used to
...
def max_memory_reserved(device: Device = None) -> int:
    ...
    program. :func:`~smith.xpu.reset_peak_memory_stats` can be used to reset
...
def mem_get_info(device: Device = None) -> tuple[int, int]:
    ...
    _lazy_init()
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_getMemoryInfo(device)
...
def get_per_process_memory_fraction(device: Device = None) -> float:
    ...
    Args:
        device (smith.device or int or str, optional): selected device. It uses the current device,
            given by :func:`~smith.xpu.current_device`, if :attr:`device` is ``None`` (default).
    ...
    _lazy_init()
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_getMemoryFraction(device)
...
def set_per_process_memory_fraction(fraction: float, device: Device = None) -> None:
    ...
    smith._C._xpu_setMemoryFraction(fraction, device)
...
def memory_snapshot(
    ...
    if not is_initialized():
        return []
    # pyrefly: ignore [missing-attribute]
    return smith._C._xpu_memorySnapshot(mempool_id)["segments"]
...
def _snapshot(device: Device = None, augment_with_fx_traces: bool = False):
    ...
    .. code-block:: python
        ...
        class Segment(TypedDict):
            ...
            #
            # Calling :func:`~smith.xpu.memory.empty_cache` releases segments that are entirely inactive.
            address: int
        ...
        class TraceEntry(TypedDict):
            # Trace entries are recorded only when :func:`~smith.xpu.memory._record_memory_history` is enabled.
            action: Literal[
    ...
    """
    # pyrefly: ignore [missing-attribute]
    s = smith._C._xpu_memorySnapshot(None)
...
def _record_memory_history(
    ...
    tell what allocated any piece of memory in :func:`~smith.xpu.memory._snapshot()`.

    In addition to keeping stack traces with each current allocation and free,
    this will also enable recording of a history of all alloc/free events.

    Use :func:`~smith.xpu.memory._snapshot()` to retrieve this information,
    ...
    """
    # pyrefly: ignore [missing-attribute]
    smith._C._xpu_recordMemoryHistory(
        enabled,
        context,
        stacks,
        max_entries,
...
class _XPUAllocator:
    ...
    # pyrefly: ignore [missing-attribute]
    def __init__(self, allocator: smith._C._xpu_XPUAllocator):
...
class XPUPluggableAllocator(_XPUAllocator):
    ...
    available for use via :func:`~smith.xpu.memory.change_current_allocator`.
    ...
    def __init__(self, path_to_lib_file: str, alloc_fn_name: str, free_fn_name: str):
        ...
        # pyrefly: ignore [missing-attribute]
        self._allocator = smith._C._xpu_customAllocator(alloc_fn_addr, free_fn_addr)
...
def change_current_allocator(allocator: _XPUAllocator) -> None:
    ...
    Arguments:
        allocator (smith.xpu.memory._XPUAllocator): allocator to be set as the active one.
    """
    # pyrefly: ignore [missing-attribute]
    smith._C._xpu_changeCurrentAllocator(allocator.allocator())
...
def _get_current_allocator() -> _XPUAllocator:
    ...
    """
    # pyrefly: ignore [missing-attribute]
    return _XPUAllocator(smith._C._xpu_getAllocator())


class MemPool(smith._C._XPUMemPool):
    ...
    Args:
        allocator(smith._C._xpu_XPUAllocator, optional): a
            smith._C._xpu_XPUAllocator object that can be used to
    ...
    def __init__(
        self,
        allocator: smith._C._xpu_XPUAllocator | None = None,
    ...
    @property
    def allocator(self) -> smith._C._xpu_XPUAllocator | None:
    ...
    def snapshot(self):
        ...
        """
        snapshot = smith.xpu.memory_snapshot(self.id)
        return snapshot
...
def use_mem_pool(pool: MemPool, device: "Device" = None):
    ...
    Args:
        pool(smith.xpu.MemPool): a :class:`MemPool` object to be made active so that
            allocations route to this pool.
        device (smith.device or int, optional): selected device. Uses :class:`MemPool on
            the current device, given by :func:`~smith.xpu.current_device`,
            if :attr:`device` is ``None`` (default).
    ...
    """
    device_index = (
        smith.xpu.current_device() if device is None else _get_device_index(device)
    )
    smith._C._xpu_beginAllocateCurrentThreadToPool(device_index, pool.id)
    try:
        yield
    finally:
        smith._C._xpu_endAllocateToPool(device_index, pool.id)
        smith._C._xpu_releasePool(device_index, pool.id)
