...
if not hasattr(smith._C, "_XpuStreamBase"):
    # Define dummy base classes
    smith._C.__dict__["_XpuStreamBase"] = _dummy_type("_XpuStreamBase")
    smith._C.__dict__["_XpuEventBase"] = _dummy_type("_XpuEventBase")


class Stream(smith._C._XpuStreamBase):
    ...
    def wait_event(self, event) -> None:
        ...
        Args:
            event (smith.xpu.Event): an event to wait for.
        """
        event.wait(self)
    ...
    def record_event(self, event=None):
        ...
        Args:
            event (smith.xpu.Event, optional): event to record. If not given, a new one
                will be allocated.
    ...
    def __repr__(self) -> str:
        return f"smith.xpu.Stream(device={self.device} sycl_queue={self.sycl_queue:#x})"


class Event(smith._C._XpuEventBase):
    ...
    def record(self, stream=None) -> None:
        ...
        Uses ``smith.xpu.current_stream()`` if no stream is specified. The
    ...
    def wait(self, stream=None) -> None:
        ...
        Use ``smith.xpu.current_stream()`` if no stream is specified.
        """
        if stream is None:
            stream = smith.xpu.current_stream()
        super().wait(stream)
    ...
    def __repr__(self) -> str:
        if self.sycl_event:
            return f"smith.xpu.Event(sycl_event={self.sycl_event:#x})"
        else:
            return "smith.xpu.Event(uninitialized)"
