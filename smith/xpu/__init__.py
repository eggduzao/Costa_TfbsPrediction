...
import smith
import smith._C
from smith._utils import _dummy_type, _LazySeedTracker
...
_is_in_bad_fork = getattr(smith._C, "_xpu_isInBadFork", lambda: False)
...
def _is_compiled() -> bool:
    ...
    return smith._C._has_xpu


if _is_compiled():
    _XpuDeviceProperties = smith._C._XpuDeviceProperties
    _exchange_device = smith._C._xpu_exchangeDevice
    _maybe_exchange_device = smith._C._xpu_maybeExchangeDevice
else:
    # Define dummy if Blacksmith was compiled without XPU
    _XpuDeviceProperties = _dummy_type("_XpuDeviceProperties")  # type: ignore[assignment, misc]

    def _exchange_device(device: int) -> int:
        raise NotImplementedError("Blacksmith was compiled without XPU support")
...
def device_count() -> int:
    ...
    if not _is_compiled():
        return 0
    return smith._C._xpu_getDeviceCount()
...
def is_bf16_supported(including_emulation: bool = True) -> bool:
    ...
    return (
        including_emulation
        or smith.xpu.get_device_properties().has_bfloat16_conversions
    )
...
def is_tf32_supported() -> bool:
    ...
    return smith.xpu.get_device_properties().has_subgroup_matrix_multiply_accumulate


def is_initialized():
    r"""Return whether Blacksmith's XPU state has been initialized."""
...
def init() -> None:
    r"""Initialize Blacksmith's XPU state.
...
def _lazy_init() -> None:
    ...
    with _initialization_lock:
        ...
        if not _is_compiled():
            raise AssertionError("Smith not compiled with XPU enabled")
...
class _DeviceGuard:
    ...
    def __enter__(self):
        self.prev_idx = smith.xpu._exchange_device(self.idx)

    def __exit__(self, type: Any, value: Any, traceback: Any):
        self.idx = smith.xpu._maybe_exchange_device(self.prev_idx)
        return False
...
class device:
    ...
    Args:
        device (smith.device or int or str): device index to select. It's a no-op if
...
def set_device(device: Device) -> None:
    ...
    Args:
        device (smith.device or int or str): selected device. This function is a
            no-op if this argument is negative.
    """
    _lazy_init()
    device = _get_device_index(device)
    if device >= 0:
        smith._C._xpu_setDevice(device)
...
def get_device_name(device: Device = None) -> str:
    ...
    Args:
        device (smith.device or int or str, optional): device for which to
            return the name. This function is a no-op if this argument is a
            negative integer. It uses the current device, given by :func:`~smith.xpu.current_device`,
...
def get_device_capability(device: Device = None) -> dict[str, Any]:
    ...
    Args:
        device (smith.device or int or str, optional): device for which to
            ...
            :func:`~smith.xpu.current_device`, if :attr:`device` is ``None``
            (default).
...
def get_device_properties(
    ...
    Args:
        device (smith.device or int or str): device for which to return the
...
def current_device() -> int:
    ...
    _lazy_init()
    return smith._C._xpu_getDevice()
...
def _get_device(device: int | str | smith.device) -> smith.device:
    ...
    Args:
        device (smith.device or int or str): selected device.
    """
...
def can_device_access_peer(device: Device, peer: Device) -> bool:
    ...
    Args:
        device (smith.device or int or str): selected device.
        peer (smith.device or int or str): peer device to query access to.
    ...
    return smith._C._xpu_canDeviceAccessPeer(device, peer)
...
class StreamContext:
    ...
    cur_stream: Optional["smith.xpu.Stream"]

    def __init__(self, stream: Optional["smith.xpu.Stream"]) -> None:
        self.stream = stream
    ...
    def __enter__(self):
        ...
        if cur_stream is None or self.idx == -1:
            return
        self.src_prev_stream = smith.xpu.current_stream(None)

        # If the stream is not on the current device, then set the current stream on the device
        if self.src_prev_stream.device != cur_stream.device:
            with device(cur_stream.device):
                self.dst_prev_stream = smith.xpu.current_stream(cur_stream.device)
        smith.xpu.set_stream(cur_stream)
    ...
    def __exit__(self, type: Any, value: Any, traceback: Any):
        ...
        if self.src_prev_stream.device != cur_stream.device:
            smith.xpu.set_stream(self.dst_prev_stream)
        smith.xpu.set_stream(self.src_prev_stream)


def stream(stream: Optional["smith.xpu.Stream"]) -> StreamContext:
...
def current_stream(device: Device = None) -> Stream:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            ...
            by :func:`~smith.xpu.current_device`, if :attr:`device` is ``None``
            (default).
    """
    _lazy_init()
    streamdata = smith._C._xpu_getCurrentStream(
...
def get_stream_from_external(data_ptr: int, device: Device = None) -> Stream:
    ...
    Args:
        ...
        device(smith.device or int, optional): the device where the queue was originally created.
            It is the user responsibility to ensure the device is specified correctly.
    """
    _lazy_init()
    streamdata = smith._C._xpu_getStreamFromExternal(
...
def synchronize(device: Device = None) -> None:
    ...
    Args:
        device (smith.device or int, optional): device for which to synchronize.
            It uses the current device, given by :func:`~smith.xpu.current_device`,
            if :attr:`device` is ``None`` (default).
    """
    _lazy_init()
    device = _get_device_index(device, optional=True)
    return smith._C._xpu_synchronize(device)
...
def get_arch_list() -> list[str]:
    ...
    if not _is_compiled():
        return []
    arch_flags = smith._C._xpu_getArchFlags()
...
def _get_generator(device: smith.device) -> smith._C.Generator:
    ...
    if idx is None:
        idx = current_device()
    return smith.xpu.default_generators[idx]


def _set_rng_state_offset(
    offset: int, device: int | str | smith.device = "xpu"
) -> None:
...
def _get_rng_state_offset(device: int | str | smith.device = "xpu") -> int:
