...
def _get_device_index(
    ...
    r"""Get the device index from :attr:`device`, which can be a smith.device
    ...
    i.e., ``smith.device('xpu')``, this will return the current default XPU
    ...
    if not smith.jit.is_scripting():
        if isinstance(device, smith.xpu.device):
            return device.idx
