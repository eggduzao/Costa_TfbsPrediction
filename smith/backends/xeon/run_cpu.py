...
"""
This is a script for launching Blacksmith inference on Intel(R) Xeon(R) Scalable Processors with optimal configurations.
...
::
    ...
    |            process          |        thread        | core  |
    +=============================+======================+=======+
    | smith.backends.xeon.run_cpu | instance 0: thread 0 |   0   |
    |                             |             thread 1 |   1   |
    |                             +----------------------+-------+
...
::
   ...
   python -m smith.backends.xeon.run_cpu --throughput-mode script.py args

2. Run single-instance inference on a single CPU node.

::

   python -m smith.backends.xeon.run_cpu --node-id 1 script.py args
...
::
   ...
   python -m smith.backends.xeon.run_cpu -- python_script args

   eg: on an Intel(R) Xeon(R) Scalable Processor with 14 instance, 4 cores per instance

::

   python -m smith.backends.xeon.run_cpu --ninstances 14 --ncores-per-instance 4 python_script args
...
::
   ...
   python -m smith.backends.xeon.run_cpu --ninstances 2 --rank 0 python_script args

   eg: run 1st instance on an Intel(R) Xeon(R) Scalable Processor with 2 instance (i.e., numactl -C 28-55)

::

   python -m smith.backends.xeon.run_cpu --ninstances 2 --rank 1 python_script args
...
::
   ...
   python -m smith.backends.xeon.run_cpu --core-list "0, 1, 2, 3" --ninstances 2 --ncores-per-instance 2
...
::
    ...
    python -m smith.backends.xeon.run_cpu --help
...
{expanduser('~')}/.local/lib/ so the LD_PRELOAD environment variable will not be set."
    ...
    def set_memory_allocator(
        ...
        By default, PTMalloc will be used for Blacksmith, but TCMalloc and JeMalloc can get better
...
Value applied: %s. Value ignored: %s",
    ...
    def set_multi_thread_and_allocator(
        ...
        By default, GNU openMP and PTMalloc are used in Blacksmith. but Intel openMP and TCMalloc/JeMalloc are better alternatives
...
--use-logical-core. They won't take effect even they are set explicitly."
        ...
        launch_tee = {}
        # check whether is launched from smithrun with --nproc-per-node <num workers>
...
https://github.com/intel/intel-extension-for-blacksmith/blob/master/docs/tutorials/performance_tuning/launch_script.md",
    )
    group.add_argument(
        "--latency-mode",
        "--latency_mode",
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="This is a script for launching Blacksmith inference on Intel(R) Xeon(R) Scalable "
        ...
        "\n   >>> python -m smith.backends.xeon.run_cpu python_script args \n"
        "\n2. multi-instance \n"
        "\n   >>> python -m smith.backends.xeon.run_cpu --ninstances xxx "
