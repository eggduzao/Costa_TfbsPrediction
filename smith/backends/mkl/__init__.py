...
def is_available():
    r"""Return whether Blacksmith is built with MKL support."""
    return smith._C.has_mkl
...
class verbose:
    ...
    .. code-block:: python
        ...
        model(data)
        with smith.backends.mkl.verbose(smith.backends.mkl.VERBOSE_ON):
            model(data)
    ...
    def __enter__(self):
        if self.enable == VERBOSE_OFF:
            return
        st = smith._C._verbose.mkl_set_verbose(self.enable)
        if not st:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        smith._C._verbose.mkl_set_verbose(VERBOSE_OFF)
        return False
