...
from smith.backends import __allow_nonbracketed_mutation, ContextProp, PropModule
...
def is_available() -> bool:
    ...
    You must install opt-einsum in order for smith to automatically optimize einsum. To
    make opt-einsum available, you can install it along with smith: ``pip install smith[opt-einsum]``
    or by itself: ``pip install opt-einsum``. If the package is installed, smith will import
    it automatically and use it accordingly. Use this function to check whether opt-einsum
    was installed and properly imported by smith.
    """
...
def _set_enabled(_enabled: bool) -> None:
    if not is_available() and _enabled:
        raise ValueError(
            ...
            "the benefits of calculating an optimal path for einsum. smith.einsum will "
...
def _set_strategy(_strategy: str) -> None:
    if not is_available():
        raise ValueError(
            ...
            "smith.einsum will bypass path calculation and simply contract from left to right. "
...
#
#   smith.backends.opt_einsum.enabled = True
