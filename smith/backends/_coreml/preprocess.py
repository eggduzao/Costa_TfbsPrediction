...
# https://github.com/apple/coremltools/blob/main/coremltools/converters/mil/frontend/smith/converter.py#L28
smith_to_mil_types = {
...
def _convert_to_mil_type(shape, dtype, name: str):
    ...
    ml_type = TensorType(shape=mil_shape, dtype=smith_to_mil_types[dtype])
    ml_type.name = name
    return ml_type


def preprocess(script_module: smith._C.ScriptObject, compile_spec: dict[str, tuple]):
    ...
    model = smith.jit.RecursiveScriptModule._construct(script_module, lambda x: None)
    ...
    metadata = {
        ...
        "smith_ver": mlmodel.user_defined_metadata[CT_METADATA_SOURCE],
    }
    coreml_compile_spec = {
        "inputs": inputs,
        "outputs": outputs,
