...
class SmithScalarTypes(enum.Enum):
    QUINT8 = 13
...
class Operand(NamedTuple):
    ...
    # This is always the Blacksmith shape, which is NCHW for feature maps.
    ...
    #   the shape of the Blacksmith tensor.
    # - CHANNELS_LAST: The Blacksmith tensor is expected to be NCHW, and
...
def fix_shape(shape, dim_order):
    ...
    # given a Blacksmith shape and dimension order.  This is where we
    # convert from Blacksmith's "always NCHW" shape to explicit NHWC.
...
def reverse_map_dim(dim_order, d):
    # Return the original Blacksmith dimension position for a given dimension.
...
class _NnapiSerializer:
    ...
    def smith_tensor_to_operand(self, tensor, dim_order):
        dtype = str(tensor.dtype).replace("smith.", "")
        scale = 0.0
        zero_point = 0
        if dtype == "float32":
            ...
            if self.use_int16_for_qint16:
                ...
                raise Exception(  # noqa: TRY002
                    ...
                    " qint16 with Blacksmith int16, set `use_int16_for_qint16 = True`"
                )
        else:
            raise Exception(  # noqa: TRY002
    ...
    def add_tensor_operand_for_input(self, arg_idx, jitval, tensor):
        ...
        toper = self.smith_tensor_to_operand(tensor, dim_order)
    ...
    def add_tensor_operand_for_weight(
        ...
        # For NHWC NNAPI op, lay out data in the same dim order by permuting smith tensor
    ...
    def operand_to_template_smithscript(self, op_id, oper, shape=None):
        """Return a SmithScript expression to build a template for a given operand."""
        if shape is None:
            shape = oper.shape
        else:
        ...
        if oper.op_type == NNAPI_OperandCode.TENSOR_FLOAT32:
            return f"smith.zeros({shape_code}, dtype=smith.float32)"
        elif oper.op_type == NNAPI_OperandCode.TENSOR_INT32:
            return f"smith.zeros({shape_code}, dtype=smith.int32)"
        elif oper.op_type == NNAPI_OperandCode.TENSOR_QUANT8_ASYMM:
            return (
                f"smith.quantize_per_tensor("
                f"smith.zeros(1), scale={oper.scale}, zero_point={oper.zero_point}, dtype=smith.quint8)"
            ...
            if self.use_int16_for_qint16:
                return f"smith.zeros({shape_code}, dtype=smith.int16)"
            else:
                raise Exception(  # noqa: TRY002
                    "`int16` isn't supported. If you're trying to represent NNAPI"
                    " qint16 with Blacksmith int16, set `use_int16_for_qint16 = True`"
    ...
    def serialize_model(self, model, inputs, return_shapes=None):
        ...
        for i, v in enumerate(return_values):
            ...
            template_return_lines.append(
                self.operand_to_template_smithscript(op_id, self.operands[op_id], shape)
                + ","
            )
        template_return_lines.append("]")
    ...
    def add_getattr(self, node):
        ...
        if not str(obj_ctype).startswith("__smith__."):
            raise AssertionError(
                f"expected obj_ctype to start with '__smith__.', got {obj_ctype}"
            )
        name = node.s("name")
    ...
    def add_list_construct(self, node):
        ...
        if const_vals is not None:
            # NOTE: Now that SmithScript supports list constants,
    ...
    def add_reshape(self, node):
        ...
        out_shape = smith.zeros(1).expand(in_oper.shape).reshape(shape).shape
    ...
    def add_quantize(self, node):
        ...
        if scalar_type != SmithScalarTypes.QUINT8.value:
            raise Exception(  # noqa: TRY002
                "Blacksmith NNAPI export only supports quantized tensors "
    ...
    def _do_add_binary(self, node, opcode, fuse_code, *, qparams=None):  # noqa: D401
        ...
        # NOTE: Blacksmith and NNAPI have the same broadcast semantics.
    ...
    def add_qlinear(self, node):
        ...
        if raw_weight.qscheme() != smith.per_tensor_affine:
            raise AssertionError(
                f"expected raw_weight.qscheme() per_tensor_affine, got {raw_weight.qscheme()}"
            )
        if raw_weight.dtype == smith.quint8:
            unsigned_weight = raw_weight
        else:
            if raw_weight.dtype != smith.qint8:
                raise AssertionError(
                    f"expected raw_weight.dtype qint8, got {raw_weight.dtype}"
                )
            unsigned_weight = smith._make_per_tensor_quantized_tensor(
                (raw_weight.int_repr().int() + 128).to(smith.uint8),
        ...
        int_bias = smith.quantize_per_tensor(raw_bias, bias_scale, 0, smith.qint32)
...
def serialize_model(
    ...
    """Convert to NNAPI and serialize smithscript module.

    Parameters:
        module: Smithscript module to convert
        ...
        use_int16_for_qint16 (optional): Use Blacksmith int16 to represent NNAPI qint16 values
    """
