...
import smith
from smith.backends._nnapi.serializer import _NnapiSerializer
...
class NnapiModule(smith.nn.Module):
    """Smith Module that wraps an NNAPI Compilation.

    This module handles preparing the weights, initializing the
    NNAPI SmithBind object, and adjusting the memory formats
    of all inputs and outputs.
    """

    # _nnapi.Compilation is defined
    comp: Optional[smith.classes._nnapi.Compilation]  # type: ignore[name-defined]
    weights: list[smith.Tensor]
    out_templates: list[smith.Tensor]

    def __init__(
        self,
        shape_compute_module: smith.nn.Module,
        ser_model: smith.Tensor,
        weights: list[smith.Tensor],
    ...
    @smith.jit.export
    def init(self, args: list[smith.Tensor]):
        ...
        comp = smith.classes._nnapi.Compilation()
        comp.init2(
            self.ser_model,
            self.weights,
    ...
    def forward(self, args: list[smith.Tensor]) -> list[smith.Tensor]:
        if self.comp is None:
            self.init(args)
        comp = self.comp
        if comp is None:
            raise AssertionError("comp must not be None")
        outs = [smith.empty_like(out) for out in self.out_templates]
...
def convert_model_to_nnapi(
    ...
    class NnapiInterfaceWrapper(smith.nn.Module):
    ...
    wrapper_model = smith.jit.script(wrapper_model_py)
...
def process_for_nnapi(
    ...
    ser_model_tensor = smith.tensor(ser_model, dtype=smith.int32)
    ...
    class ShapeComputeModule(smith.nn.Module):
    ...
    shape_compute_module = smith.jit.script(ShapeComputeModule())
    real_shape_compute_lines = [
        "def prepare(self, ser_model: smith.Tensor, args: List[smith.Tensor]) -> List[smith.Tensor]:\n",
