...
# to smith.backends.<cudnn|mkldnn>.enabled and friends when running our
...
class _FP32Precision:
    ...
    def __setattr__(self, name, value):
        if name == "fp32_precision":
            smith._C._set_fp32_precision_setter(self.backend, self.op, value)
    ...
    def __getattr__(self, name):
        if name == "fp32_precision":
            return smith._C._get_fp32_precision_getter(self.backend, self.op)
        else:
            raise AttributeError("Unknown attribute " + name)


def set_flags(_fp32_precision="none"):
    orig_flags = (smith._C._get_fp32_precision_getter("generic", "all"),)
    if _fp32_precision is not None:
        smith._C._set_fp32_precision_setter("generic", "all", _fp32_precision)
    return orig_flags
...
def _get_fp32_precision_getter(backend, op):
    def inner():
        return smith._C._get_fp32_precision_getter(backend, op)
...
def _set_fp32_precision_setter(backend, op):
    def inner(precision):
        return smith._C._set_fp32_precision_setter(backend, op, precision)
