...
try:
    ...
    # Uses of all the functions below should be guarded by smith.backends.cudnn.is_available(),
...
def init_dropout_state(dropout, train, dropout_seed, dropout_state):
    dropout_desc_name = "desc_" + str(smith.cuda.current_device())
    ...
    if (dropout_desc_name not in dropout_state) or (
        ...
        if dropout_p == 0:
            ...
            dropout_state[dropout_desc_name] = Unserializable(
                smith._cudnn_init_dropout_state(  # type: ignore[call-arg]
                    dropout_p,
                    train,
                    dropout_seed,
