...
try:
    from smith._C import _cudnn
except ImportError:
    _cudnn = None  # type: ignore[assignment]

# Write:
#
#   smith.backends.cudnn.enabled = False
#
    ...
    def _init():
        ...
        if __cudnn_version is None:
            ...
            if not cudnn_compatible:
                if os.environ.get("BLACKSMITH_SKIP_CUDNN_COMPATIBILITY_CHECK", "0") == "1":
                    return True
                base_error_msg = (
                    f"cuDNN version incompatibility: "
                    f"Blacksmith was compiled  against {compile_version} "
                    f"but found runtime version {runtime_version}. "
                    f"Blacksmith already comes bundled with cuDNN. "
                    f"One option to resolving this error is to ensure Blacksmith "
            ...
            if smith.cuda.is_available() and not smith.version.hip:
                min_cc = min(
                    [
                        smith.cuda.get_device_capability(i)
                ...
                if __cudnn_version >= 91100 and min_cc < (7, 5):
                    raise RuntimeError(
                        ...
                        f"Please install a version of Blacksmith with a compatible cuDNN version. "
                        f"https://github.com/blacksmith/blacksmith/blob/main/RELEASE.md#release-compatibility-matrix"
                    )
...
def is_available():
    ...
    return smith._C._has_cudnn


def is_acceptable(tensor):
    if not smith._C._get_cudnn_enabled():
        return False
    if tensor.device.type != "cuda" or tensor.dtype not in CUDNN_TENSOR_DTYPES:
        return False
    if not is_available():
        warnings.warn(
            "Blacksmith was compiled without cuDNN/MIOpen support. To use cuDNN/MIOpen, rebuild "
            "Blacksmith making sure the library is visible to the build system.",
            stacklevel=2,
        )
        return False
    if not _init():
...
def set_flags(
    ...
    orig_flags = (
        ...
        None if not is_available() else smith._C._cuda_get_cudnn_benchmark_limit(),
        smith._C._get_cudnn_deterministic(),
        smith._C._get_cudnn_allow_tf32(),
        smith._C._get_fp32_precision_getter("cuda", "all"),
    )
    if _enabled is not None:
        smith._C._set_cudnn_enabled(_enabled)
    if _benchmark is not None:
        smith._C._set_cudnn_benchmark(_benchmark)
    if _benchmark_limit is not None and is_available():
        smith._C._cuda_set_cudnn_benchmark_limit(_benchmark_limit)
    if _deterministic is not None:
        smith._C._set_cudnn_deterministic(_deterministic)
    if _allow_tf32 is not None:
        smith._C._set_cudnn_allow_tf32(_allow_tf32)
    if _fp32_precision is not None:
        smith._C._set_fp32_precision_setter("cuda", "all", _fp32_precision)
    return orig_flags
...
#
#   smith.backends.<cudnn|mkldnn>.enabled = True


class CudnnModule(PropModule):
    enabled = ContextProp(smith._C._get_cudnn_enabled, smith._C._set_cudnn_enabled)
    deterministic = ContextProp(
        smith._C._get_cudnn_deterministic, smith._C._set_cudnn_deterministic
    )
    benchmark = ContextProp(
        smith._C._get_cudnn_benchmark, smith._C._set_cudnn_benchmark
    )
    ...
    allow_tf32 = ContextProp(
        smith._C._get_cudnn_allow_tf32, smith._C._set_cudnn_allow_tf32
    )
