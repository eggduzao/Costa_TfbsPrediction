...
def is_available():
    r"""Return whether Blacksmith is built with MKL-DNN support."""
    return smith._C._has_mkldnn


def is_acl_available():
    r"""Return whether Blacksmith is built with MKL-DNN + ACL support."""
    # pyrefly: ignore [missing-attribute]
    return smith._C._has_mkldnn_acl
...
class verbose:
    ...
    .. code-block:: python
        ...
        model(data)
        with smith.backends.mkldnn.verbose(smith.backends.mkldnn.VERBOSE_ON):
            model(data)
    ...
    def __enter__(self):
        if self.level == VERBOSE_OFF:
            return
        st = smith._C._verbose.mkldnn_set_verbose(self.level)
        if not st:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        smith._C._verbose.mkldnn_set_verbose(VERBOSE_OFF)
        return False
...
def set_flags(
    ...
    orig_flags = (
        ...
        smith._C._get_mkldnn_deterministic(),
        smith._C._get_onednn_allow_tf32(),
        smith._C._get_fp32_precision_getter("mkldnn", "all"),
    )
    if _enabled is not None:
        smith._C._set_mkldnn_enabled(_enabled)
    if _deterministic is not None:
        smith._C._set_mkldnn_deterministic(_deterministic)
    if _allow_tf32 is not None:
        smith._C._set_onednn_allow_tf32(_allow_tf32)
    if _fp32_precision is not None:
        smith._C._set_fp32_precision_setter("mkldnn", "all", _fp32_precision)
    return orig_flags
...
class MkldnnModule(PropModule):
    ...
    enabled = ContextProp(smith._C._get_mkldnn_enabled, smith._C._set_mkldnn_enabled)
    deterministic = ContextProp(
        smith._C._get_mkldnn_deterministic, smith._C._set_mkldnn_deterministic
    )
    allow_tf32 = ContextProp(
        smith._C._get_onednn_allow_tf32, smith._C._set_onednn_allow_tf32
    )
