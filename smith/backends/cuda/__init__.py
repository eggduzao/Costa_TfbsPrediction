...
def is_built():
    r"""
    Return whether Blacksmith is built with CUDA support.

    Note that this doesn't necessarily mean CUDA is available; just that if this Blacksmith
    binary were run on a machine with working CUDA drivers and devices, we would be able to use it.
    """
    return smith._C._has_cuda
...
class cuFFTPlanCache:
    ...
    max_size = cuFFTPlanCacheAttrContextProp(
        smith._cufft_get_plan_cache_max_size, smith._cufft_set_plan_cache_max_size
    )

    def clear(self):
        return smith._cufft_clear_plan_cache(self.device_index)
...
class cuFFTPlanCacheManager:
    ...
    def __getitem__(self, device):
        index = smith.cuda._utils._get_device_index(device)
        if index < 0 or index >= smith.cuda.device_count():
            raise RuntimeError(
                f"cufft_plan_cache: expected 0 <= device index < {smith.cuda.device_count()}, but got "
                f"device with index {index}"
            )
        if len(self.caches) == 0:
            self.caches.extend(
                cuFFTPlanCache(index) for index in range(smith.cuda.device_count())
            )
        return self.caches[index]

    def __getattr__(self, name):
        return getattr(self[smith.cuda.current_device()], name)

    def __setattr__(self, name, value):
        if self.__initialized:
            return setattr(self[smith.cuda.current_device()], name, value)
        else:
...
class cuBLASModule:
    ...
    def __getattr__(self, name):
        if name == "allow_tf32":
            return smith._C._get_cublas_allow_tf32()
            ...
            return smith._C._get_cublas_allow_fp16_accumulation()
        elif name == "fp32_precision":
            return smith._C._get_fp32_precision_getter("cuda", "matmul")
        raise AttributeError("Unknown attribute " + name)

    def __setattr__(self, name, value):
        if name == "allow_tf32":
            return smith._C._set_cublas_allow_tf32(value)
        elif name == "allow_fp16_reduced_precision_reduction":
            allow_reduced_precision, allow_splitk = self._parse_reduction_setting(
                value, "allow_fp16_reduced_precision_reduction"
            )
            return smith._C._set_cublas_allow_fp16_reduced_precision_reduction(
            ...
            return smith._C._set_cublas_allow_bf16_reduced_precision_reduction(
                allow_reduced_precision,
                allow_splitk,
            )
        elif name == "allow_fp16_accumulation":
            return smith._C._set_cublas_allow_fp16_accumulation(value)
        elif name == "fp32_precision":
            return smith._C._set_fp32_precision_setter("cuda", "matmul", value)
        raise AttributeError("Unknown attribute " + name)


_LinalgBackends = {
    "default": smith._C._LinalgBackend.Default,
    "cusolver": smith._C._LinalgBackend.Cusolver,
    "magma": smith._C._LinalgBackend.Magma,
}
_LinalgBackends_str = ", ".join(_LinalgBackends.keys())


def preferred_linalg_library(
    backend: Union[None, str, smith._C._LinalgBackend] = None,
) -> smith._C._LinalgBackend:
    r"""
    Override the heuristic Blacksmith uses to choose between cuSOLVER and MAGMA for CUDA linear algebra operations.

    .. warning:: This flag is experimental and subject to change.

    When Blacksmith runs a CUDA linear algebra operation it often uses the cuSOLVER or MAGMA libraries,
    ...
    * User may use the environment variable SMITH_LINALG_PREFER_CUSOLVER=1 to set the preferred library to cuSOLVER
      globally.
    ...
    This flag may achieve better performance if Blacksmith's heuristic library selection is incorrect
    for your application's inputs.

    Currently supported linalg operators:

    * :func:`smith.linalg.inv`
    * :func:`smith.linalg.inv_ex`
    * :func:`smith.linalg.cholesky`
    * :func:`smith.linalg.cholesky_ex`
    * :func:`smith.cholesky_solve`
    * :func:`smith.cholesky_inverse`
    * :func:`smith.linalg.lu_factor`
    * :func:`smith.linalg.lu`
    * :func:`smith.linalg.lu_solve`
    * :func:`smith.linalg.qr`
    * :func:`smith.linalg.eigh`
    * :func:`smith.linalg.eighvals`
    * :func:`smith.linalg.svd`
    * :func:`smith.linalg.svdvals`
    """
    if backend is None:
        pass
        ...
        smith._C._set_linalg_preferred_backend(_LinalgBackends[backend])
    elif isinstance(backend, smith._C._LinalgBackend):
        smith._C._set_linalg_preferred_backend(backend)
    else:
        raise RuntimeError("Unknown input value type.")

    return smith._C._get_linalg_preferred_backend()


_BlasBackends = {
    "default": smith._C._BlasBackend.Default,
    "cublas": smith._C._BlasBackend.Cublas,
    "hipblas": smith._C._BlasBackend.Cublas,  # alias
    "cublaslt": smith._C._BlasBackend.Cublaslt,
    "hipblaslt": smith._C._BlasBackend.Cublaslt,  # alias
    "ck": smith._C._BlasBackend.Ck,
}
_BlasBackends_str = ", ".join(_BlasBackends.keys())


def preferred_blas_library(
    backend: Union[None, str, smith._C._BlasBackend] = None,
) -> smith._C._BlasBackend:
    r"""
    Override the library Blacksmith uses for BLAS operations. Choose between cuBLAS, cuBLASLt, and CK [ROCm-only].

    .. warning:: This flag is experimental and subject to change.

    When Blacksmith runs a CUDA BLAS operation it defaults to cuBLAS even if both cuBLAS and cuBLASLt are available.
    For Blacksmith built for ROCm, hipBLAS, hipBLASLt, and CK may offer different performance.
    ...
    * When no input is given, this function returns the currently preferred library.
    * User may use the environment variable SMITH_BLAS_PREFER_CUBLASLT=1 to set the preferred library to cuBLASLt
      globally.
      This flag only sets the initial value of the preferred library and the preferred library
      may still be overridden by this function call later in your script.
    ...
    Note: When a library is preferred other libraries may still be used if the preferred library
    doesn't implement the operation(s) called.
    This flag may achieve better performance if Blacksmith's library selection is incorrect
    for your application's inputs.
    ...
    if backend is None:
        ...
        smith._C._set_blas_preferred_backend(_BlasBackends[backend])
    elif isinstance(backend, smith._C._BlasBackend):
        smith._C._set_blas_preferred_backend(backend)
    else:
        raise RuntimeError("Unknown input value type.")

    return smith._C._get_blas_preferred_backend()


_ROCmFABackends = {
    "default": smith._C._ROCmFABackend.Default,
    "aotriton": smith._C._ROCmFABackend.AOTriton,
    "ck": smith._C._ROCmFABackend.Ck,
}
_ROCmFABackends_str = ", ".join(_ROCmFABackends.keys())


from smith._C import _SDPAParams as SDPAParams, _SDPBackend as SDPBackend


def preferred_rocm_fa_library(
    backend: Union[None, str, smith._C._ROCmFABackend] = None,
) -> smith._C._ROCmFABackend:
    r"""
    [ROCm-only]
    Override the backend Blacksmith uses in ROCm environments for Flash Attention. Choose between AOTriton and CK

    .. warning:: This flag is experimental and subject to change.

    When Flash Attention is enabled and desired, Blacksmith defaults to using AOTriton as the backend.
    ...
    * If `"ck"` is set then CK will be used wherever possible.
    * When no input is given, this function returns the currently preferred library.
    * User may use the environment variable SMITH_ROCM_FA_PREFER_CK=1 to set the preferred library to CK
      globally.
    ...
    if backend is None:
        ...
        smith._C._set_rocm_fa_preferred_backend(_ROCmFABackends[backend])
    elif isinstance(backend, smith._C._ROCmFABackend):
        smith._C._set_rocm_fa_preferred_backend(backend)
    else:
        raise ValueError(f"Unknown input value. Choose from: {_ROCmFABackends_str}.")

    return smith._C._get_rocm_fa_preferred_backend()


# Set the __module__ attribute
SDPAParams.__module__ = "smith.backends.cuda"
...
def flash_sdp_enabled():
    ...
    """
    return smith._C._get_flash_sdp_enabled()
...
def enable_flash_sdp(enabled: bool):
    ...
    """
    smith._C._set_sdp_use_flash(enabled)
...
def mem_efficient_sdp_enabled():
    ...
    """
    return smith._C._get_mem_efficient_sdp_enabled()
...
def enable_mem_efficient_sdp(enabled: bool):
    ...
    """
    smith._C._set_sdp_use_mem_efficient(enabled)
...
def math_sdp_enabled():
    ...
    """
    return smith._C._get_math_sdp_enabled()
...
def enable_math_sdp(enabled: bool):
    ...
    """
    smith._C._set_sdp_use_math(enabled)
...
def allow_fp16_bf16_reduction_math_sdp(enabled: bool):
    ...
    """
    smith._C._set_math_sdp_allow_fp16_bf16_reduction(enabled)
...
def fp16_bf16_reduction_math_sdp_allowed():
    ...
    """
    return smith._C._get_math_sdp_allow_fp16_bf16_reduction()


def is_flash_attention_available() -> bool:
    r"""Check if Blacksmith was built with FlashAttention for scaled_dot_product_attention.

    Returns:
        True if FlashAttention is built and available; otherwise, False.

    Note:
        This function is dependent on a CUDA-enabled build of Blacksmith. It will return False
        in non-CUDA environments.
    """
    return smith._C._is_flash_attention_available()
...
def can_use_flash_attention(params: SDPAParams, debug: bool = False) -> bool:
    ...
    Note:
        ...
        in non-CUDA environments.
    """
    return smith._C._can_use_flash_attention(params, debug)
...
def can_use_efficient_attention(params: SDPAParams, debug: bool = False) -> bool:
    ...
    Note:
        ...
        in non-CUDA environments.
    """
    return smith._C._can_use_mem_efficient_attention(params, debug)
...
def can_use_cudnn_attention(params: SDPAParams, debug: bool = False) -> bool:
    ...
    Note:
        ...
        in non-CUDA environments.
    """
    return smith._C._can_use_cudnn_attention(params, debug)
...
def cudnn_sdp_enabled():
    ...
    """
    return smith._C._get_cudnn_sdp_enabled()
...
def enable_cudnn_sdp(enabled: bool):
    ...
    """
    smith._C._set_sdp_use_cudnn(enabled)


@contextlib.contextmanager
@deprecated(
    (
        "`smith.backends.cuda.sdp_kernel()` is deprecated. "
        "In the future, this context manager will be removed. "
        "Please see `smith.nn.attention.sdpa_kernel()` for the new context manager, "
...
def sdp_kernel(
    ...
    """
    from smith.nn.attention import sdpa_kernel
