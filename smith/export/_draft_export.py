...
import smith
import smith._logging._internal
import smith.utils._pytree as pytree
from smith._dynamo.exc import UserError, UserErrorType
from smith._export.passes.insert_custom_op_guards import (
    get_op_profiles,
    insert_custom_op_guards,
    OpProfile,
)
from smith._utils_internal import log_draft_export_usage
...
class FailureReport:
    ...
    smith.ops.{op} is missing a fake kernel implementation.
...
"""  # noqa: B950
    ...
    following. You can also use smith.export.Dim.AUTO instead to specify your
...
"""
    ...
    Please add `smith._check(...)` to the original code to assert this data-dependent assumption.
...
"""  # noqa: B950
    ...
    smith.ops.{op} has a fake kernel implementation, but it has incorrect behavior, based on the real kernel.
...
You can now change back to smith.export.export()
##############################################################################################
{END_COLOR}"""
...
class CaptureStructuredTrace(smith._logging._internal.LazyTraceHandler):
    def __init__(self) -> None:
        ...
        self.logger = logging.getLogger("smith.__trace")
        self.prev_get_dtrace = False

        if root_dir := os.environ.get(smith._logging._internal.DTRACE_ENV_VAR):
        ...
        self.setFormatter(smith._logging._internal.SmithLogsFormatter(trace=True))
    ...
    def __enter__(self) -> "CaptureStructuredTrace":
        ...
        possible_lazy_trace_handlers = [
            ...
            if isinstance(handler, smith._logging._internal.LazyTraceHandler)
        ]
        for handler in possible_lazy_trace_handlers:
            self.logger.removeHandler(handler)

        self.logger.addHandler(self)
        self.prev_get_dtrace = smith._logging._internal.GET_DTRACE_STRUCTURED
        # pyrefly: ignore [bad-assignment]
        smith._logging._internal.GET_DTRACE_STRUCTURED = True
        return self
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:  # type: ignore[no-untyped-def]
        ...
        # pyrefly: ignore [bad-assignment]
        smith._logging._internal.GET_DTRACE_STRUCTURED = self.prev_get_dtrace
        self.prev_get_dtrace = False
...
def draft_export(
    ...
    *,
    ...
    with (
        ...
        smith._logging.dtrace_structured("exported_program", payload_fn=lambda: str(ep))
