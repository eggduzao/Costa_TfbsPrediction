...
def _end_ptr(tensor: smith.Tensor) -> int:
    if tensor.nelement():
...
class Weights(dict):
    ...
    def __init__(self, weight_dict: dict[str, tuple[smith.Tensor, TensorProperties]]):
        super().__init__(weight_dict)

    def get_weight(self, name: str) -> tuple[smith.Tensor, TensorProperties]:
        return self[name]
...
def group_weights(all_weights: dict[str, Weights]) -> list[OrderedSet[tuple[str, str]]]:
    ...
    weights_dict: dict[tuple[int, smith.dtype], OrderedSet[tuple[str, str]]] = (
