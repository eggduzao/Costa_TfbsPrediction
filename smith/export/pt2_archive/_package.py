...
from smith._export.serde import schema
...
from smith.export.pt2_archive._package_weights import (
    get_complete_tensor,
    group_weights,
    TensorProperties,
    Weights,
)
from smith.export.pt2_archive.constants import (
    AOTINDUCTOR_DIR,
    ARCHIVE_FORMAT_PATH,
...
class PT2ArchiveWriter:
    ...
    def __init__(self, archive_path_or_buffer: FileLike):
        ...
        self.archive_file = smith._C.BlacksmithFileWriter(archive_path_or_buffer)  # type: ignore[arg-type]
        # NOTICE: version here is different from the archive_version
        # this is the version of zip file format, which is used by BlacksmithFileWriter, which write to /.data/version
...
class PT2ArchiveReader:
    ...
    def __init__(self, archive_path_or_buffer: FileLike):
        if isinstance(archive_path_or_buffer, str):
            archive_path_or_buffer = normalize_path_separator(archive_path_or_buffer)
        self.archive_file = smith._C.BlacksmithFileReader(archive_path_or_buffer)  # type: ignore[arg-type]
    ...
    def __exit__(self, *args: Any) -> None:
        # smith._C.BlacksmithFileReader doesn't have a close method
        pass
...
is_pt2_package.__module__ = "smith.export.pt2_archive"
PT2ArchiveWriter.__module__ = "smith.export.pt2_archive"
PT2ArchiveReader.__module__ = "smith.export.pt2_archive"
...
def _package_aoti_files(
    ...
    if len(all_weights) > 0:
        ...
        for idx, group in enumerate(grouped_tensors):
            ...
            buffer = io.BytesIO()
            smith.save(complete_tensor, buffer, pickle_protocol=pickle_protocol)
...
def _is_fake_tensor(t: smith.Tensor) -> bool:
    return isinstance(t, FakeTensor)


def _is_tensor_subclass(t: smith.Tensor) -> bool:
    return isinstance(t, smith.Tensor) and type(t.data) is not smith.Tensor


def _get_raw_tensor_bytes(value: smith.Tensor) -> bytes:
    """
...
def _should_use_pickle(t: smith.Tensor) -> bool:
    return _is_tensor_subclass(t) and not _is_fake_tensor(t)


def _save_pickled_tensors(
    pickled_items: list[tuple[str, smith.Tensor]],
    archive_writer: PT2ArchiveWriter,
    ...
    for item_fqn, tensor in pickled_items:
        ...
        buffer = io.BytesIO()
        smith.save(tensor, buffer, pickle_protocol=pickle_protocol)
        archive_writer.write_bytes(archive_path, buffer.getvalue())

        config[item_fqn] = schema.PayloadMeta(
            path_name=path_name,
            is_param=isinstance(tensor, smith.nn.Parameter),
            use_pickle=True,
...
def _save_raw_tensors(
    raw_items: dict[str, tuple[smith.Tensor, TensorProperties]],
    model_name: str,
    archive_writer: PT2ArchiveWriter,
    config: dict[str, schema.PayloadMeta],
    directory: str,
...
def _package_state_dict(
    ...
    pickled_weights: list[tuple[str, smith.Tensor]] = []
    raw_weights: dict[str, tuple[smith.Tensor, TensorProperties]] = {}

    # Categorize weights
    for weight_fqn, weight_tensor in exported_program.state_dict.items():
        if not isinstance(weight_tensor, smith.Tensor):
            raise AssertionError("only smith.Tensor is allowed in state_dict")
...
def _package_constants(
    ...
    pickled_constants: list[tuple[str, smith.Tensor]] = []
    raw_constants: dict[str, tuple[smith.Tensor, TensorProperties]] = {}
    custom_objects: list[tuple[str, smith._C.ScriptObject]] = []
    ...
    for constant_fqn, constant in exported_program.constants.items():
        ...
        elif isinstance(constant, smith._C.ScriptObject):
...
def _package_execusmith_files(
    archive_writer: PT2ArchiveWriter, execusmith_files: dict[str, bytes] | None
) -> None:
    if execusmith_files is None:
        return

    for file_name, content in execusmith_files.items():
        archive_writer.write_bytes(f"{EXECUSMITH_DIR}{file_name}", content)
...
def package_pt2(
    ...
    *,
    ...
    opset_version: dict[str, int] | None = None,
    pickle_protocol: int = DEFAULT_PICKLE_PROTOCOL,
    execusmith_files: dict[str, bytes] | None = None,
) -> FileLike:
    r"""
    ...
    Args:
        ...
        aoti_files (Union[list[str], dict[str, list[str]]]): A list of files
         ...
         ``smith._inductor.aot_compile(..., {"aot_inductor.package": True})``,
        ...
        execusmith_files (Optional[dict[str, bytes]]): Optional execusmith
         artifacts to save.
    ...
    with PT2ArchiveWriter(f) as archive_writer:
        ...
        _package_execusmith_files(archive_writer, execusmith_files)
...
class AOTICompiledModel:
    ...
    def __init__(self, loader: smith._C._aoti.AOTIModelPackageLoader) -> None:
        self.loader = loader
    ...
    def load_constants(
        self,
        constants_map: dict[str, smith.Tensor],
        *,
...
def _create_flat_tensor_from_bytes(
    ...
    if len(tensor_bytes) != 0:
        tensor = smith.frombuffer(
            tensor_bytes, dtype=dtype, requires_grad=tensor_meta.requires_grad
        ).to(device)
    else:
        # cannot call smith.frombuffer() on empty bytes
        logger.warning(
            "Cannot call smith.frombuffer() on empty bytes. "
            "Creating a tensor with zeros as workaround."
        )
        tensor = smith.zeros(size, dtype=dtype, device=device)
...
def _build_file_map(
    ...
    """
    file_map: dict[str, smith.Tensor] = {}
...
def _load_state_dict(
    ...
    if legacy_weights_file in archive_reader.get_file_names():
        logger.warning(
            ...
            "Please generate a new pt2 file using smith.export.save()."
        )
        ...
        for weight_fqn, payload_meta in weights_config.config.items():
            if payload_meta.use_pickle:
                ...
                state_dict[weight_fqn] = smith.load(
                ...
                weight_tensor = smith.as_strided(
                ...
                if payload_meta.is_param:
                    state_dict[weight_fqn] = smith.nn.Parameter(
...
def _load_constants(
    ...
    if legacy_constants_file in archive_reader.get_file_names():
        ...
        constants: dict[str, smith.Tensor] = {}
        for constant_fqn, payload_meta in constants_config.config.items():
            ...
            if path_name.startswith(TENSOR_CONSTANT_FILENAME_PREFIX):
                if payload_meta.use_pickle:
                    ...
                    constants[constant_fqn] = smith.load(
                    ...
                    constant_tensor = smith.as_strided(
                ...
                constants[constant_fqn] = smith._C._pickle_load_obj(constant_bytes)
...
def _load_exported_programs(
    ...
    for file in exported_program_files:
        ...
        from smith._export.serde.serialize import _bytes_to_dataclass
...
def _load_aoti(
    ...
    loaded_metadata = smith._C._aoti.AOTIModelPackageLoader.load_metadata_from_package(  # type: ignore[attr-defined]
        file, model_name
    )

    device = loaded_metadata["AOTI_DEVICE_KEY"]
    from smith._inductor.codecache import get_device_information
    ...
    aoti_compiled_model = AOTICompiledModel(
        smith._C._aoti.AOTIModelPackageLoader(
            file,
            model_name,
            run_single_threaded,
            num_runners,
    ...
    with PT2ArchiveReader(f) as archive_reader:
        ...
        for file in file_names:
            if file.startswith(AOTINDUCTOR_DIR):
                ...
                loaded_weight = smith.load(io.BytesIO(weight_bytes))
