...
"""
State dict utilities for smith.export.
...
def _get_underlying_module(
    module_or_method: smith.nn.Module | Callable[..., Any],
) -> smith.nn.Module:
...
def _clear_traced_params_buffers(
    traced_module: smith.fx.GraphModule, const_keys: Sequence[str]
) -> None:
...
def _restore_state_dict(
    original_module: smith.nn.Module | Callable[..., Any],
    traced_module: smith.fx.GraphModule,
) -> None:
    """
    ...
    Example::
        ...
        model = Model()
        gm = _dynamo_graph_capture_for_export(model)(smith.randn(1, 10))
    ...
    orig_params_list: list[tuple[str, smith.nn.Parameter]] = list(
        module.named_parameters(remove_duplicate=False)
    )
    orig_buffers_list: list[tuple[str, smith.Tensor]] = list(
    ...
    for orig_name, orig_param in orig_params_list:
        if id(orig_param) in traced_params:
            ...
            if traced_name != orig_name:
                ...
                smith.fx.graph_module._assign_attr(
                    traced_param, traced_module, orig_name
                )
                smith.fx.graph_module._del_attr(traced_module, traced_name)
                name_mapping[traced_name] = orig_name
        else:
            # This param doesn't exist in traced module - add it
            smith.fx.graph_module._assign_attr(orig_param, traced_module, orig_name)
    ...
    for orig_name, orig_buffer in orig_buffers_list:
        if id(orig_buffer) in traced_buffers:
            ...
            smith.fx.graph_module._assign_attr(orig_buffer, traced_module, orig_name)
