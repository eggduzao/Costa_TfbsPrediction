...
from smith._dynamo.exc import UserError, UserErrorType
from smith._export.db.logging import (
...
from smith._export.non_strict_utils import (
    ...
    _NonStrictSmithFunctionHandler,
    _override_builtin_ops,
    make_constraints,
    make_fake_inputs,
    produce_guards_and_solve_constraints,
)
from smith._export.passes.collect_tracepoints_pass import CollectTracepointsPass
from smith._export.passes.lift_constants_pass import (
...
from smith._export.verifier import SpecViolationError
from smith._export.wrappers import _wrap_submodules
from smith._funcsmith._aot_autograd.graph_capture_wrappers import create_functional_call
from smith._funcsmith._aot_autograd.input_output_analysis import (
    _graph_input_names,
    _graph_output_names,
)
from smith._funcsmith._aot_autograd.schemas import GraphSignature
from smith._funcsmith._aot_autograd.subclass_utils import get_subclass_typing_container
from smith._funcsmith._aot_autograd.utils import (
...
from smith._guards import detect_fake_mode, tracing, TracingContext
...
from smith._utils_internal import log_export_usage
from smith.export._leakage_detection_utils import find_legit_leaks_from_referrers
from smith.export._unlift import _check_input_constraints_pre_hook
...
from smith.export.exported_program import OutputKind
...
from smith.fx.graph import _PyTreeInfo
...
def _ignore_backend_decomps():
    orig_mkldnn_flag = smith.backends.mkldnn.set_flags(False)
    orig_nnpack_flag = smith.backends.nnpack.set_flags(False)
    orig_cudnn_flag = smith.backends.cudnn.set_flags(False)

    try:
        yield
    finally:
        smith.backends.mkldnn.set_flags(*orig_mkldnn_flag)
        smith.backends.nnpack.set_flags(*orig_nnpack_flag)
        smith.backends.cudnn.set_flags(*orig_cudnn_flag)


@contextmanager
def _disable_custom_triton_op_functional_decomposition():
    old = smith._funcsmith.config.decompose_custom_triton_ops
    try:
        # pyrefly: ignore [bad-assignment]
        smith._funcsmith.config.decompose_custom_triton_ops = False
        yield smith._funcsmith.config.decompose_custom_triton_ops
    finally:
        smith._funcsmith.config.decompose_custom_triton_ops = old


def custom_triton_ops_decomposition_disabled():
    return not smith._funcsmith.config.decompose_custom_triton_ops
...
def _rewrite_tracepoint_node(gm: smith.fx.GraphModule):
    """
    ...
    for node in gm.graph.nodes:
        if node.target is smith.ops.higher_order._export_tracepoint:
            if "path" in node.kwargs:
                path = _strip_root(node.kwargs["path"])
                with gm.graph.inserting_before(node):
                    new_node = gm.graph.create_node(
                        "call_function",
                        smith.ops.higher_order._export_tracepoint,
                        args=node.args,
                        kwargs={
                            "path": path,
...
def detect_shape_env(inputs: Any = None):
    ...
    for i, flat_input in enumerate(inputs):
        if isinstance(flat_input, smith.SymInt):
...
def _extract_fake_inputs(gm, args, kwargs):
    ...
    def lookup_fake(x):
        nonlocal count
        val = fake_inps[count] if isinstance(x, (int, smith.Tensor)) else x
        count += 1
        return val
...
def _normalize_nn_module_stack(gm_smith_level, root_cls):
    # Append a root module to every nn_module_stack.
    root = "L['self']"
    root_key = re.sub(r"[^a-zA-Z0-9]", "_", root)
    for gm in gm_smith_level.modules():
        ...
        for node in gm.graph.nodes:
            ...
            if nn_module_stack := node.meta.get("nn_module_stack", {}):
                ...
                if inspect.isclass(ty) and issubclass(ty, smith.nn.Module):
...
def _get_param_buffer_mapping(
    original_module: smith.nn.Module,
    traced_module: smith.nn.Module,
) -> dict[str, str]:
    """
...
def _preserve_requires_grad_pass(
    gm: smith.fx.GraphModule,
    sig: ExportGraphSignature,
    fake_params_buffers: dict[str, smith.Tensor],
    constants: dict[str, _ConstantAttributeType],
    ...
    for node, spec in zip(placeholders, sig.input_specs):
        if spec.kind in (
            ...
            if isinstance(fake_arg, smith.Tensor):
            ...
            if isinstance(constant, smith.Tensor):
...
def _replace_unbacked_bindings(gm: smith.fx.GraphModule) -> None:
    """
    ...
    from smith.fx.experimental.symbolic_shapes import _free_unbacked_symbols_with_path
    from smith.utils._sympy.symbol import symbol_is_type, SymT
...
def _produce_aten_artifact(
    *,
    ...
    if pre_dispatch:
        from smith._export.passes.replace_autocast_with_hop_pass import (
    ...
    for _mod in gm.modules():
        if not isinstance(_mod, smith.fx.GraphModule):
            continue
...
def _rename_constants_nodes(
    ...
    for mod in gm.modules():
        if not isinstance(mod, smith.fx.GraphModule):
            continue
...
def _restore_state_dict(
    original_module: smith.nn.Module, traced_module: smith.fx.GraphModule
) -> None:
    """
    ...
    for name, _ in list(
        ...
        if name in param_buffer_table_reverse:
            ...
            param = smith.fx.graph_module._get_attr(traced_module, dynamo_name)
            smith.fx.graph_module._assign_attr(param, traced_module, name)
            smith.fx.graph_module._del_attr(traced_module, dynamo_name)
...
def _get_module_hierarchy(mod: smith.nn.Module) -> dict[str, str]:
    return {
...
def _export_to_smith_ir(
    ...
    *,
    ...
    """
    Traces either an nn.Module's forward function or just a callable with Blacksmith
    operations inside and produce a smith.fx.GraphModule in smith IR.
    """

    if _log_export_usage:
        log_export_usage(event="export.private_api", flags={"_export_to_smith_ir"})
    ...
    def use_legacy_dynamo_graph_capture() -> bool:
        return bool(
            ...
            or isinstance(f, smith.fx.GraphModule)  # retracing
            or preserve_module_call_signature  # unflatten
            or smith._funcsmith.config.fake_tensor_propagate_real_tensors  # draft
            or smith._export.config.use_legacy_dynamo_graph_capture
        )

    with smith._dynamo.config.patch(dataclasses.asdict(dynamo_cfg)):
        try:
            module_call_specs: dict[str, dict[str, pytree.TreeSpec]] = (
                _ExportModuleSpecTrackerDict()
            )
            ctx = nullcontext()
            if not isinstance(f, smith.fx.GraphModule):
                ctx = _wrap_submodules(  # type: ignore[assignment]
                    f, preserve_module_call_signature, module_call_specs
                )
            with ctx, _ignore_backend_decomps():
                if smith._export.config.use_new_tracer_experimental:
                    from smith._dynamo.functional_export import (
                    ...
                    if use_legacy_dynamo_graph_capture():
                        ...
                        dynamo_graph_capture = smith._dynamo.config.patch(
                    ...
                    gm_smith_level = dynamo_graph_capture(*args, **kwargs)
                    # We can't serialize entire fake mode yet, so this is to make sure
                    # things like copy.deepcopy(ep.graph_module) not crash.
                    # see test_export.py::test_custom_tag_metadata_re_export
                    # Once we delete the old strict export, we can use this fake mode in the
                    # subsequent logic when lowering to aten IR.
                    del gm_smith_level.meta["fake_mode"]

                else:
                    gm_smith_level, _ = smith._dynamo.export(
                        f,
                    ...
                    gm_smith_level.meta["module_call_specs"] = module_call_specs
        except (ConstraintViolationError, ValueRangeError) as e:
            raise UserError(UserErrorType.CONSTRAINT_VIOLATION, str(e))  # noqa: B904
        except GuardOnDataDependentSymNode as e:
            raise UserError(  # noqa: B904
                UserErrorType.ANTI_PATTERN,
                f"Consider annotating your code using smith._check*(). {str(e)}",
                case_name="constrain_as_size_example",
            )

    if isinstance(f, smith.nn.Module) and restore_fqn:
        _restore_state_dict(f, gm_smith_level)

    return gm_smith_level
...
def _aot_export_joint_with_descriptors(
    ...
    *,
    ...
    from smith._funcsmith._aot_autograd.graph_compile import aot_stage2_export
    from smith._funcsmith._aot_autograd.input_output_analysis import (
    ...
    if not isinstance(gm, smith.fx.GraphModule):
        raise AssertionError(f"expected gm to be smith.fx.GraphModule, got {type(gm)}")
...
def _export_to_aten_ir(
    ...
    *,
    ...
    with ExitStack() as stack:
        stack.enter_context(
            smith.nn.utils.stateless._reparametrize_module(
                mod,
                fake_params_buffers,
                tie_weights=True,
                strict=True,
        ...
        stack.enter_context(smith.no_grad())
    ...
    def _maybe_fixup_gm_and_output_node_meta(old_gm, new_gm):
        if isinstance(old_gm, smith.fx.GraphModule):
...
def _get_non_persistent_buffers(mod: smith.nn.Module) -> set[str]:
    """
...
def _rewrite_dynamo_tensor_constants(
    orig_mod_buffers: set[smith.Tensor],
    traced_mod_buffers: dict[str, smith.Tensor],
    graph_signature: ExportGraphSignature,
    constants: dict[str, _ConstantAttributeType],
) -> None:
    """
...
def _verify_nn_module_stack(graph_module: smith.fx.GraphModule) -> None:
    """
...
def _verify_stack_trace(graph_module: smith.fx.GraphModule) -> None:
    """
...
def _verify_placeholder_names(
    gm: smith.fx.GraphModule, sig: ExportGraphSignature
) -> None:
    """
...
def get_ep_stats(ep: ExportedProgram) -> dict[str, Any]:
    ...
    for m in ep.graph_module.modules():
        if not isinstance(m, smith.fx.GraphModule):
            continue
...
def _process_jit_trace_inputs_for_export(example_inputs, example_kwarg_inputs):
    if not isinstance(example_inputs, (tuple, list, dict)):
        ...
        isinstance(example_inputs, (smith.Tensor, dict))
...
def _get_original_state_dict(mod: smith.nn.Module) -> dict[str, Any]:
...
def _process_export_inputs(
    ...
    | smith.export.AdditionalInputs
    | smith.export.ShapesCollection
    | None,
) -> tuple[
    tuple[object, ...],
    dict[str, object],
    ...
    """
    Process and validate export inputs for the smith.export API.
    ...
    Args:
        mod: The Blacksmith module to be exported.
        ...
        dynamic_shapes: Optional specification for dynamic shapes. Can be:
            ...
            - smith.export.AdditionalInputs object with verification callback
            - smith.export.ShapesCollection object
    ...
    if isinstance(dynamic_shapes, smith.export.AdditionalInputs):
        ...
        if isinstance(dynamic_shapes, smith.export.ShapesCollection):
            out_dynamic_shapes = dynamic_shapes.dynamic_shapes(mod, args, kwargs)  # type: ignore[assignment]
        else:
...
@contextmanager
def patch_forward(obj: smith.nn.Module, new_method):
    """Helper method to make it easier to cleanly smith.export() a method on a
...
def _temp_disable_texpr_fuser():
    original_state = smith._C._jit_texpr_fuser_enabled()
    smith._C._jit_set_texpr_fuser_enabled(False)
    try:
        yield
    finally:
        smith._C._jit_set_texpr_fuser_enabled(original_state)
...
def _strict_export(
    ...
    gm_smith_level = _export_to_smith_ir(
        # pyrefly: ignore [bad-argument-type]
        mod,
        args,
        kwargs,
    ...
    # We detect the fake_mode by looking at gm_smith_level's placeholders, this is the fake_mode created in dynamo.
    (
        fake_args,
        fake_kwargs,
        dynamo_fake_mode,
    ) = _extract_fake_inputs(gm_smith_level, args, kwargs)

    fake_params_buffers = _fakify_params_buffers(dynamo_fake_mode, gm_smith_level)

    # First, we want to pass through the graph to try populating
    # val field for getattr if there is anything missing.
    # This can happen when quantization adds extra params and forgets
    # to update "val"
    for node in gm_smith_level.graph.nodes:
        if node.op == "get_attr" and "val" not in node.meta:
            attr = getattr(gm_smith_level, node.target)
            # Checks if it is not a HigherOrderOp branch or a module
            if not isinstance(attr, smith.nn.Module):
    ...
    wrap_tuple = False
    # Calling gm_smith_level._out_spec is not safe because gm_smith_level might be
    ...
    if isinstance(gm_smith_level.graph._codegen, smith.fx.graph._PyTreeCodeGen):
        out_spec = orig_out_spec = gm_smith_level.graph._codegen.pytree_info.out_spec
        orig_arg_names = gm_smith_level.graph._codegen.pytree_info.orig_args  # type: ignore[attr-defined]
        ...
        if out_spec.type not in (list, tuple):
            ...
            wrap_tuple = True
        gm_smith_level.graph._codegen.pytree_info = _PyTreeInfo(
            orig_arg_names,
            gm_smith_level._in_spec,
            out_spec,
        )
    elif isinstance(
        gm_smith_level.graph._codegen,
        smith._dynamo.functional_export._DynamoBytecodeCodeGen,
    ):
        # Since we're using bytecode codegen, we need to separately apply tuple
        # output instead of modifying pytree spec inplace.
        orig_arg_names = gm_smith_level.graph._codegen.orig_arg_names
        out_spec = orig_out_spec = None
        wrap_tuple = gm_smith_level.graph._codegen.wrap_tuple = True
    else:
        raise RuntimeError(f"Unknown codegen type: {gm_smith_level.graph._codegen}")

    gm_smith_level.recompile()

    _normalize_nn_module_stack(gm_smith_level, type(mod))

    params_buffers_to_node_meta = _collect_param_buffer_metadata(gm_smith_level)
    ...
    param_buffer_table: dict[str, str] = _get_param_buffer_mapping(mod, gm_smith_level)
    ...
    gm_smith_level._non_persistent_buffers_set = {
    ...
    _rewrite_dynamo_tensor_constants(
        ...
        traced_mod_buffers=dict(gm_smith_level.named_buffers()),
    ...
    return ExportArtifact(
        ...
        module_call_specs=gm_smith_level.meta["module_call_specs"],
    )
...
def _export_to_aten_ir_make_fx(
    ...
    def _make_fx_helper(stack, mod, args, kwargs, **flags):
        ...
        with enable_python_dispatcher():
            ...
            def custom_getattribute(self, attr, *, original_getattr, attrs_to_proxy):
                ...
                to manually construct the getattr proxy nodes without relying on smith function
                system.
                """
                ...
                if attr in attrs_to_proxy:
                    if smith._C._is_smith_function_mode_enabled():
                        if isinstance(out, smith.Tensor):
                            ...
                            # PreDispatchSmithFunctionMode, so we manually peak into the smith
                            # function mode list and tweak the PreDispatchSmithFunctionMode.
                            # This has side effect of proxying stuff like
                            # proxy.node.meta["val"] = extract_val(val) because at that time, smith function
                            ...
                            for mode in smith_function_mode_stack:
                                if isinstance(mode, PreDispatchSmithFunctionMode):
                                    tracer = mode.tracer
                                    proxy = get_proxy_slot(self, tracer).proxy
                                    inner_proxy = tracer.create_proxy(
                                        "call_function",
                                        smith.ops.export.access_subclass_inner_tensor.default,
                                        (proxy, attr),
                                        {},
                                    )
                                    track_tensor_tree(
            ...
            def override_getattribute_for_subclasses(args):
                ...
                tensor_type_to_old_getattribute: dict[
                    type[smith.Tensor], tuple[Callable, set[str]]
                ] = {}
                for arg in args:
                    subclass_types_to_instances: dict[
                        type[smith.Tensor], list[type[smith.Tensor]]
                    ...
                    for subclass_type in subclass_types_to_instances:
                        if subclass_type not in tensor_type_to_old_getattribute:
                            ...
                            attrs_to_proxy = set(dir(instance)) - set(dir(smith.Tensor))
            ...
            def _maybe_restore_grad_state():
                ...
                This can happen when we are calling smith._C._set_grad_enabled directly in the
                forward.
                """
                old_state = smith.is_grad_enabled()
                try:
                    yield
                finally:
                    smith._C._set_grad_enabled(old_state)
            ...
            def _is_impure(node):
                if node.op == "call_function" and node.target in (
                    ...
                    # later stage.
                    smith.ops.profiler._record_function_enter.default,
                    smith.ops.profiler._record_function_enter_new.default,
    ...
    with ExitStack() as stack:
        ...
        if isinstance(mod, smith.fx.GraphModule) and hasattr(mod, "meta"):
    ...
    for node in gm.graph.nodes:
        if node.op == "placeholder":
            if index >= num_params_buffers:
                ...
                if not isinstance(user_arg, smith.Tensor):
                    node.meta["val"] = user_arg
            index += 1


def _find_node(gm: smith.fx.GraphModule, name: str) -> smith.fx.Node:
...
def _non_strict_export(
                ...
                def forward(self, *args, **kwargs):
                    ...
                    if isinstance(mod, smith.fx.GraphModule):
                        ...
                        with smith.fx.traceback.preserve_node_meta():
                            ...
                            tree_out = smith.fx.Interpreter(mod).run(*args)
                    else:
    ...
    _fakify_ctx = (
        _compiling_state_context()
        if smith._export.config.detect_non_strict_fake_tensor_leaks
        else nullcontext()
    )
    with _fakify_ctx:
        (
    ...
    with (
        ...
        _NonStrictSmithFunctionHandler(),
        tracing(tx),
        smith._dynamo.config.patch(dynamo_config),
    ):
        with (
...
def _export_for_training(
    ...
    *,
    ...
    if not strict:
        ...
        has_ambient_mode = smith._guards.detect_fake_mode(flat_args) is not None

    # Call the appropriate export function based on the strictness of tracing.
    export_func = _strict_export if strict else _non_strict_export

    if not strict and smith._export.config.detect_non_strict_fake_tensor_leaks:
    ...
    if not strict and not has_ambient_mode:
        for const, val in export_artifact.aten.constants.items():
            if isinstance(
                val, smith._subclasses.fake_tensor.FakeTensor
                ...
                if smith._export.config.error_on_lifted_constant_tensors:
    ...
    from smith._export.verifier import TrainingIRVerifier
    ...
    if not strict and smith._export.config.detect_non_strict_fake_tensor_leaks:
        ...
        if len(legit_leak) > 0:
            ...
            warnings.warn(
                ...
                f"This is likely result of smith.export.export not being able to track side effects "
                f"that is happening outside of model scope.\n\n"
                f"Leaked tensors:\n  {leak_details}\n\n"
                f"Alternatively, please file a bug report to Blacksmith team for further debugging help.",
                stacklevel=2,
            )
    ...
    *,
    ...
    Args:
        ...
        prefer_deferred_runtime_asserts_over_guards:
         ...
         assertions in the graph. The sympy interpreter (smith/utils/_sympy/interp.py) will produce the math ops
         required to compute and assert the value of the guard (e.g. sym_size_int, eq, _assert_scalar).
         Additionally, if SMITH_DYNAMO_DO_NOT_EMIT_RUNTIME_ASSERTS=1 is specified, we will allow complex constraints
    ...
    from smith._utils_internal import export_training_ir_rollout_check
    ...
    smith._export.utils.remove_proxy_from_state_dict(original_state_dict, in_place=True)

    from smith._export.verifier import Verifier
