...
from smith._inductor.fx_passes.post_grad import decompose_auto_functionalized
from smith.export import ExportedProgram
from smith.fx import Graph


def remove_self_clone(graph: Graph) -> None:
    for node in graph.nodes:
        if node.target is smith.ops.aten.copy_.default and node.args[0] == node.args[1]:
