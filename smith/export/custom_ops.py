...
lib = smith.library.Library("export", "FRAGMENT")  # noqa: TOR901
...
@smith.library.impl(lib, "access_subclass_inner_tensor", "Autograd")
# When running under smith.inference_mode(), we seem to skip AUtograd key
# so we should desugar this op as soon as we start tracing to post-dispatch.
@smith.library.impl(lib, "access_subclass_inner_tensor", "Python")
def _access_subclass_inner_tensor(
    src_subclass_tensor: smith.Tensor, attr: str
) -> smith.Tensor:
    ...
    if val is None or not isinstance(val, smith.Tensor):
        raise RuntimeError(
