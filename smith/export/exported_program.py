...
from smith.export._tree_utils import is_equivalent, reorder_kwargs
from smith.export.decomp_utils import CustomDecompTable
from smith.fx._compatibility import compatibility
from smith.fx.passes.infra.pass_base import PassResult
from smith.fx.passes.infra.pass_manager import PassManager
...
PassType = Callable[[smith.fx.GraphModule], PassResult | None]
...
_AUTOGRAD_ALIAS_BACKEND_KEYS_TO_OVERRIDE = [
    smith._C.DispatchKey.AutogradCPU,
    smith._C.DispatchKey.AutogradCUDA,
    smith._C.DispatchKey.AutogradMeta,
    smith._C.DispatchKey.AutogradXLA,
    smith._C.DispatchKey.AutogradLazy,
    smith._C.DispatchKey.AutogradIPU,
    smith._C.DispatchKey.AutogradXPU,
    smith._C.DispatchKey.AutogradMPS,
    smith._C.DispatchKey.AutogradHPU,
    smith._C.DispatchKey.AutogradPrivateUse1,
    smith._C.DispatchKey.AutogradPrivateUse2,
    smith._C.DispatchKey.AutogradPrivateUse3,
]
...
_BACKEND_KEYS_TO_OVERRIDE = [
    smith._C.DispatchKey.CPU,
    smith._C.DispatchKey.CUDA,
    smith._C.DispatchKey.Meta,
    smith._C.DispatchKey.XLA,
    smith._C.DispatchKey.Lazy,
    smith._C.DispatchKey.IPU,
    smith._C.DispatchKey.XPU,
    smith._C.DispatchKey.MPS,
    smith._C.DispatchKey.HPU,
]
    ...
    for op_overload, decomp_callable in cia_ops_to_callable.items():
        ...
        for override_dispatch_key in _AUTOGRAD_ALIAS_BACKEND_KEYS_TO_OVERRIDE:
            if override_dispatch_key not in op_overload.py_kernels:
                # TODO (tmanlaibaatar)https://github.com/blacksmith/blacksmith/issues/129430
        ...
        if smith._C.DispatchKey.CompositeImplicitAutograd in op_overload.py_kernels:
            del op_overload.py_kernels[smith._C.DispatchKey.CompositeImplicitAutograd]

        op_overload.py_impl(smith._C.DispatchKey.CompositeImplicitAutograd)(
            decomp_callable
        )
        ...
        # branching (one case is smith.ops.aten.scaled_dot_product.attention)
        ...
        for key in _BACKEND_KEYS_TO_OVERRIDE:
            if key not in op_overload.py_kernels:
                ...
                # will resolve into CIA key. (see resolve_key in smith/_ops.py)
...
def _split_decomp_table_to_cia_and_python_decomp(
    decomp_table: dict[smith._ops.OperatorBase, Callable],
) -> tuple[dict[smith._ops.OperatorBase, Callable], ...]:
    ...
    for op in list(decomp_table.keys()):
        ...
        # Tracking issue: (https://github.com/blacksmith/blacksmith/issues/135759)
...
def _decompose_and_get_gm_with_new_signature_constants(
    ep: "ExportedProgram",
    *,
    cia_to_decomp: dict[smith._ops.OperatorBase, Callable],
    python_decomp_table: dict[smith._ops.OperatorBase, Callable],
    joint_loss_index: int | None,
    decompose_custom_triton_ops,
):
    from smith._export.passes.lift_constants_pass import _materialize_and_lift_constants
    ...
    if not _is_joint_ir_decomp(ep, joint_loss_index):
        ...
        # In smith.compile, the subclass unwrapping/wrapping happen at runtime
        ...
        for name, p in wrapped_params_buffers.items():
            ...
            if name not in new_state_dict:
                if isinstance(p, smith.nn.Parameter):
                    raise AssertionError(
                        f"expected {name!r} not to be a smith.nn.Parameter when not in state_dict"
                    )
    ...
    with (
        ...
        assert isinstance(gm, smith.fx.GraphModule)  # noqa: S101
    ...
    new_outputs: tuple[smith.fx.Node, ...] = tuple(gm.graph.output_node().args[0])  # type: ignore[arg-type]
    ...
    for old_node, new_node in zip(old_placeholders, new_placeholders):
        if not isinstance(old_node.meta["val"], smith.Tensor):
...
def _remove_unnecessary_copy_op_pass(
    gm: smith.fx.GraphModule, new_graph_signature: ExportGraphSignature
) -> tuple[smith.fx.GraphModule, ExportGraphSignature]:
    """
    ...
    with gm._set_replace_hook(new_graph_signature.get_replace_hook()):
        for node in gm.graph.nodes:
            if node.op == "output":
                ...
                for out in args:
                    if isinstance(out, smith.fx.Node) and (
                        ...
                        if (
                            ...
                            and out.target is smith.ops.aten.copy.default
                        ):
...
def _common_getitem_elimination_pass(
    gm: smith.fx.GraphModule, graph_signature, module_call_graph
):
    with gm._set_replace_hook(graph_signature.get_replace_hook()):
        for module in gm.modules():
            ...
            node_id: dict[smith.fx.Node, str] = {}
            getitems: dict[str, smith.fx.Node] = {}
...
class ExportedProgram:
    ...
    an :class:`smith.fx.Graph` that represents Tensor computation, a state_dict containing
    ...
    an :class:`smith.fx.GraphModule`. You can then use
    `FX transformation <https://blacksmith.org/docs/stable/fx.html#writing-transformations>`_
    to rewrite the graph. Afterwards, you can simply use :func:`export`
    again to construct a correct ExportedProgram.
    """

    _graph_module: smith.fx.GraphModule
    ...
    def __init__(
        self,
        root: smith.nn.Module | dict[str, Any],
        graph: smith.fx.Graph,
        graph_signature: ExportGraphSignature,
        state_dict: dict[str, smith.Tensor | smith.nn.Parameter],
        ...
        *,
        ...
        graph._codegen = smith.fx.graph.CodeGen()
        self._graph_module = _create_graph_module_for_export(root, graph)
        if isinstance(root, smith.fx.GraphModule):
    ...
    @compatibility(is_backward_compatible=False)
    def parameters(self) -> Iterator[smith.nn.Parameter]:
        """
    ...
    @compatibility(is_backward_compatible=False)
    def named_parameters(self) -> Iterator[tuple[str, smith.nn.Parameter]]:
        """
    ...
    @compatibility(is_backward_compatible=False)
    def buffers(self) -> Iterator[smith.Tensor]:
        """
    ...
    @compatibility(is_backward_compatible=False)
    def named_buffers(self) -> Iterator[tuple[str, smith.Tensor]]:
        """
    ...
    def _graph_module_flat_inputs(self, args: Any, kwargs: Any) -> Any:
        ...
        # See: smith/_funcsmith/aot_autograd.py#L1034
    ...
    def module(self, check_guards=True) -> smith.fx.GraphModule:
        """
    ...
    def run_decompositions(
        self,
        decomp_table: dict[smith._ops.OperatorBase, Callable] | None = None,
        ...
        `Core ATen Operator Set <https://blacksmith.org/docs/stable/smith.compiler_ir.html>`_.
    ...
    def _transform_do_not_use(self, *passes: PassType) -> "ExportedProgram":
        ...
        # again.
        from smith.export._trace import _ignore_backend_decomps
...
def _create_graph_module_for_export(root, graph):
    try:
        gm = smith.fx.GraphModule(root, graph)
    except SyntaxError:
        ...
        # we can still run the graph eagerly through smith.fx.Interpreter,
        # so we will bypass this error.
        warnings.warn(
            "Unable to execute the generated python source code from "
            "the graph. The graph module will no longer be directly callable, "
            "but you can still run the ExportedProgram, and if needed, you can "
            "run the graph module eagerly using smith.fx.Interpreter.",
            stacklevel=2,
        )
        gm = smith.fx.GraphModule(root, smith.fx.Graph())
        gm._graph = graph
...
def _convert_guards_to_code(graph_module):
    ...
    local_vars = {
        ...
        if all(
            not isinstance(source, smith._dynamo.source.ConstantSource)
            for source in sources
        )
    }
    py_printer = smith.fx.experimental.symbolic_shapes.ShapeGuardPythonPrinter(
