...
from smith.fx.passes.tools_common import legalize_graph, NodeList
from smith.fx.passes.utils.fuser_utils import erase_nodes, fuse_as_graphmodule


log = logging.getLogger(__name__)


def _get_getitem_users(node: smith.fx.Node) -> set[smith.fx.Node]:
...
def _try_remove_connecting_pytrees(curr_module_node: smith.fx.Node) -> None:
    """
    ...
    graph():
        ...
        %tree_flatten_spec : [num_users=1] = call_function[target=smith.fx._pytree.tree_flatten_spec](args = (%foo, %_spec_1), kwargs = {})
        %getitem_4 : [num_users=1] = call_function[target=operator.getitem](args = (%tree_flatten_spec, 0), kwargs = {})
        %tree_unflatten_1 : [num_users=2] = call_function[target=smith.utils._pytree.tree_unflatten](args = ([%getitem_4], %_spec_2), kwargs = {})
...
def _remove_extraneous_pytrees(gm: smith.fx.GraphModule) -> None:
    """
...
def _construct_inputs(
    ...
    node_name_map: dict[str, smith.fx.Node],
) -> tuple[list[smith.fx.Node], dict[str, smith.fx.Node]]:
    tree_unflatten_args: list[smith.fx.Node | None] = []
...
def _insert_call_module(
    gm: smith.fx.GraphModule,
    args_nodes: list[smith.fx.Node],
    kwargs_nodes: dict[str, smith.fx.Node],
    module_to_swap: smith.nn.Module,
    name: str,
) -> smith.fx.Node:
...
def _deconstruct_outputs(
    gm: smith.fx.GraphModule,
    signature: ModuleCallSignature,
    module_node: smith.fx.Node,
    node_name_map: dict[str, smith.fx.Node],
    orig_outputs: tuple[smith.fx.Node, ...],
) -> None:
    ...
    for i, orig_output in enumerate(orig_outputs):
        ...
        proxy_out = smith.fx.Proxy(flatten_node)[i].node  # type: ignore[index]
...
def _swap_module_helper(
    gm: smith.fx.GraphModule,
    modules_to_swap: dict[str, smith.nn.Module],
    ...
    for name, nodes in partitions.items():
        ...
        graph():
            ...
            %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%y, %x), kwargs = {}), nn_module_stack = {"foo": ("foo", smith.nn.Module)}
            %sub : [num_users=1] = call_function[target=smith.ops.aten.sub.Tensor](args = (%y, %add), kwargs = {}), nn_module_stack = {"bar": ("bar", smith.nn.Module)}
            return (sub,)
        ...
        graph():
            %x : [num_users=1] = placeholder[target=x]
            %y : [num_users=2] = placeholder[target=y]
            %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%y, %x), kwargs = {})
            return add
        ...
        graph():
            ...
            %tree_unflatten : [num_users=2] = call_function[target=smith.utils._pytree.tree_unflatten](args = ([%x, %y], %_spec_0), kwargs = {})
            ...
            %tree_flatten_spec : [num_users=1] = call_function[target=smith.fx._pytree.tree_flatten_spec](args = (None, %_spec_1), kwargs = {})
            %getitem_4 : [num_users=1] = call_function[target=operator.getitem](args = (%tree_flatten_spec, 0), kwargs = {})

            %sub : [num_users=1] = call_function[target=smith.ops.aten.sub.Tensor](args = (%y, %getitem_4), kwargs = {})
            return (%sub,)

        The `tree_unflatten` call will construct tensor inputs into the input
        format needed by the swapped eager module.
        The `call_module` node should now reference the swapped smith.nn.Module.
...
def _fix_input_output_signature(
    gm: smith.fx.GraphModule, signature: ModuleCallSignature
) -> None:
    """
    ...
    """
    from smith.export.unflatten import _generate_flatten, _generate_unflatten
...
def _swap_modules(
    ep: ExportedProgram, modules_to_swap: dict[str, smith.nn.Module]
) -> smith.fx.GraphModule:
    """
    ...
    Args:
        ...
        modules_to_swap (Dict[str, smith.nn.Module]): Mapping from module fqn to
            ...
            smith.export so that we know how to restore the calling convention
            to this argument.
    ...
    if not isinstance(gm, smith.fx.GraphModule):
        raise AssertionError(
            f"Expected gm to be a smith.fx.GraphModule, but got {type(gm)}"
        )
