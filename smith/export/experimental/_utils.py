...
def _get_main_cpp_file(
    ...
    ib.newline()
    for model_name in model_names:
        ib.writeline(f"using smith::aot_inductor::AOTInductorModel{model_name};")

    ib.writelines(
        [
            "using smith::aot_inductor::ConstantHandle;",
            "using smith::aot_inductor::ConstantMap;",
            "",
    ...
    with ib.indent():
        ...
        with ib.indent():
            ...
            if example_inputs_map is not None:
                ...
                for i, model_name in enumerate(model_names):
                    ...
                    with ib.indent():
                        ...
                        ib.writeline(
                            "smith::IValue ivalue = smith::pickle_load(buffer);"
                        )
                        ib.writeline(
                ...
                for i in range(len(model_names)):
                    ib.writelines(
                        [
                            ...
                            f"    smith::aot_inductor::unsafe_alloc_new_handles_from_tensors(input_tensors{i + 1});",
                        ]
                    )
            ...
            if example_inputs_map is not None:
                ...
                for i in range(len(model_names)):
                    ib.writeline(
                        f"smith::aot_inductor::DeviceStreamType stream{i + 1} = nullptr;"
                    )
                    ib.writeline(
                ...
                for i in range(len(model_names)):
                    ib.writelines(
                        [
                            ...
                            f"    smith::aot_inductor::alloc_tensors_by_stealing_from_handles(&output_handle{i + 1}, 1);",
                        ]
                    )
                ...
                for i in range(len(model_names)):
                    ...
                    ib.writeline(
                        f"""smith::save(output_tensor{i + 1}, "output_tensor{i + 1}.pt");"""
                    )
...
def _get_make_file(package_name: str, model_names: list[str], device_type: str) -> str:
    ...
    from smith._inductor.config import test_configs

    if test_configs.use_libsmith:
        ib.writeline("find_package(Smith REQUIRED)")
    ...
    ib.writeline(f"target_link_libraries(main PRIVATE smith {model_libs})")
