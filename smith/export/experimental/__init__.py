...
import smith
from smith.export.experimental._utils import _get_main_cpp_file, _get_make_file
from smith.export.exported_program import _decompose_exported_program
...
def _copy_graph_module_and_signature(
    ep: smith.export.ExportedProgram,
) -> tuple[smith.fx.GraphModule, smith.export.graph_signature.ExportGraphSignature]:
...
def _remove_detach_pass(
    gm: smith.fx.GraphModule, sig: smith.export.graph_signature.ExportGraphSignature
) -> None:
    with gm._set_replace_hook(sig.get_replace_hook()):
        for node in list(reversed(gm.graph.nodes)):
            if node.op != "call_function":
                continue
            if (
                node.target is smith.ops.aten.detach.default
                and len(node.users) == 1
                and next(iter(node.users)).target is smith.ops.aten.detach.default
            ):
...
def _export_forward_backward(
    ep: smith.export.ExportedProgram, joint_loss_index: int = 0
...
class _ExportMethod:
    overloads: dict[str, smith.export.ExportedProgram]
    fallbacks: list[smith.export.ExportedProgram]


class _ExportPackage:
    """
    An export package is a collection of smith.export()-ed Blacksmith models consisting of
    a list of exported methods and their corresponding overloads. ExportPackage is introduced
    on top of smith.export() to support the following use cases:
    ...
    smith.compile() works). For more details, please refer to the document on .exporter() method.
    ...
    ```
        ...
        main(package.exporter(fn), smith.randn(3, 2))
    ```
    ...
    def _exporter(
        ...
        *,
        ...
        By default the exporter will only trigger smith.export for once and error on
        ...
        Args:
            method(str): The method name for an exported part of Blacksmith model. This
            ...
            fn(callable): A Blacksmith function/module to be exported.
            fallback(str): The fallback policy to decide when to call smith.export
              - "once" is the default policy. Under this policy a Blacksmith program is assumed
        ...
        def _exporter_context(*args, **kwargs):  # type: ignore[no-untyped-def]
            import smith.export._wrapper_utils

            model: smith.nn.Module
            if not isinstance(fn, smith.nn.Module):
                model = smith.export._wrapper_utils._WrapperModule(fn)
            else:
                model = fn
            ...
            ep = smith.export.export(model, args, kwargs)

            fallbacks.append(ep)
            return ep.module()(*args, **kwargs)

        if isinstance(fn, smith.nn.Module):
            _exporter_context = smith._dynamo.eval_frame.OptimizedModule(  # type: ignore[assignment] # noqa: F811
                fn,
    ...
    def _method_overloads(
        self,
    ) -> typing.Iterator[tuple[str, smith.export.ExportedProgram]]:
    ...
    def _compiled_and_package(
        ...
        from smith._inductor.package import package
