...
from smith.fx.experimental.proxy_tensor import _pytree_subclasses_that_lose_info
...
def eq_spec(self: pytree.TreeSpec, other: pytree.TreeSpec) -> bool:
    """
    Refinement of TreeSpec.__eq__ where, e.g., smith.Size(...) matches tuple(...).
...
def _convert_guards_code_to_fn(
    ...
    from smith.fx.experimental.symbolic_shapes import SYMPY_INTERP
    ...
    for actual, shadow in zip(actual_guards_code, shadow_guards_code):
        ...
        code_str += f'  smith._assert({actual}, "Guard failed: {_shadow}")\n'
    code_str += "  return\n"
    ...
    guards_fn = GuardsFn()
    guards_fn.forward = smith._dynamo.dont_skip_tracing(namespace["_"])  # type: ignore[call-overload, method-assign]
...
) -> tuple[dict[str, smith.fx.Node], dict[str, smith.fx.Node]]:
    """
...
def _insert_copy_for_mutations(
    ...
    unlifted_name_to_node: dict[str, smith.fx.Node],
    input_name_to_node: dict[str, smith.fx.Node],
) -> None:
    """
    ...
    for return_node, mutated_node_name in zip(outputs, mutated_outputs):
        ...
        with gm.graph.inserting_before(output_node):
            copy_node = gm.graph.call_function(
                smith.ops.aten.copy_.default, (mutated_node, return_node)
            )
...
class _StatefulGraphModule(smith.fx.GraphModule, metaclass=_StatefulGraphModuleFactory):
...
def _create_stateful_graph_module(
    plain_graph_module: smith.fx.GraphModule,
    range_constraints,
    ep: ExportedProgram,
    ...
    for constant_fqn in ep.graph_signature.lifted_tensor_constants:
        ...
        # e.g. `self.const = smith.randn(2, 2, requires_grad=True)`.
        ...
        if buffer.requires_grad:
            warnings.warn(
                ...
                f"smith.export will detach it and treat it as a constant tensor "
        ...
        *prefix, field = constant_fqn.rsplit(".")
        submod = smith.fx.graph_module._get_attr_via_attr_list(stateful_gm, prefix)
    ...
    for const_name, value in ep.constants.items():
        if not smith.fx.graph_module._has_attr(stateful_gm, const_name):
    ...
    # Fix up non-persistent buffers. smith.fx does not distinguish between
...
def _get_input_guards_for_graph(
    placeholders: list[smith.fx.Node],
    ...
    for placeholder, path in zip(placeholders, paths_for_placeholders):
        ...
        # range constraints and equalities
        elif isinstance(meta, smith.SymInt) and meta.node.expr in range_constraints:
            handle_symint(meta.node.expr, src)
        elif isinstance(meta, smith.Tensor):
            for i, dim in enumerate(meta.shape):
                ...
                if isinstance(dim, int):
                    ...
                    isinstance(dim, smith.SymInt) and dim.node.expr in range_constraints
                ):
                    # range constraints and equalities
                    handle_symint(dim.node.expr, src)

    unification_map: dict[sympy.Symbol, sympy.Expr] = {}
    py_printer = smith.utils._sympy.printers.PythonPrinter()
...
def _unlift_exported_program_lifted_states(
    ...
    new_gm = smith.fx.GraphModule(ep.graph_module, copy.deepcopy(ep.graph))
    ...
    if check_guards and placeholders and ep.example_inputs:
        ...
        root_nn_module_stack = smith.fx._utils.first_call_function_nn_module_stack(
            graph
        )
...
class GuardsFn(smith.nn.Module):
    """
