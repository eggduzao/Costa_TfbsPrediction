...
def move_to_device_pass(
    ep: ExportedProgram, location: smith.device | str | dict[str, str]
) -> ExportedProgram:
    """
    ...
    Args:
        ...
        location (Union[smith.device, str, Dict[str, str]]): The device to move the exported program to.
    ...
    def _get_new_device(
        curr_device: smith.device,
        location: smith.device | str | dict[str, str],
    ) -> str:
    ...
    for k, v in ep.state_dict.items():
        if isinstance(v, smith.nn.Parameter):
            ep._state_dict[k] = smith.nn.Parameter(
        ...
        if isinstance(m, smith.fx.GraphModule):
            for node in m.graph.nodes:
                ...
                if (
                    ...
                    and node.target is smith.ops.aten.to.device
                ):
                    args = list(node.args)
