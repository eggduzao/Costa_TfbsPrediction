...
from smith._library.opaque_object import get_opaque_type_name, is_opaque_type
...
class ExportGraphSignature:
    ...
    e.g. If following module is exported::
        ...
        class CustomModule(nn.Module):
            def __init__(self) -> None:
                ...
                # Define a parameter
                self.my_parameter = nn.Parameter(smith.tensor(2.0))

                # Define two buffers
                self.register_buffer("my_buffer1", smith.tensor(3.0))
                self.register_buffer("my_buffer2", smith.tensor(4.0))
        ...
        mod = CustomModule()
        ep = smith.export.export(mod, (smith.tensor(1.0), smith.tensor(2.0)))
    ...
    Resulting Graph is non-functional::
        ...
        graph():
            ...
            %add : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%x1, %p_my_parameter), kwargs = {})
            %mul : [num_users=1] = call_function[target=smith.ops.aten.mul.Tensor](args = (%add, %b_my_buffer1), kwargs = {})
            %mul_1 : [num_users=1] = call_function[target=smith.ops.aten.mul.Tensor](args = (%x2, %b_my_buffer2), kwargs = {})
            %add_1 : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%mul, %mul_1), kwargs = {})
            %add_ : [num_users=0] = call_function[target=smith.ops.aten.add_.Tensor](args = (%b_my_buffer2, 1.0), kwargs = {})
            return (add_1,)
    ...
    Resulting Graph is functional::
        ...
        graph():
            ...
            %mul_1 : [num_users=1] = call_function[target=smith.ops.aten.mul.Tensor](args = (%x2, %b_my_buffer2), kwargs = {})
            %add_1 : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%mul, %mul_1), kwargs = {})
            %add_2 : [num_users=1] = call_function[target=smith.ops.aten.add.Tensor](args = (%b_my_buffer2, 1.0), kwargs = {})
            return (add_2, add_1)
...
def _make_argument_spec(node, token_names) -> ArgumentSpec:
    from smith import ScriptObject, SymBool, SymFloat, SymInt
    from smith._library.fake_class_registry import FakeScriptObject
