...
from smith.export._tree_utils import reorder_kwargs
from smith.export.exported_program import (
    ConstantArgument,
...
from smith.fx._symbolic_trace import is_fx_symbolic_tracing
from smith.fx.graph_module import _get_attr, _get_attr_via_attr_list, _print_readable
from smith.utils._pytree import GetAttrKey, SequenceKey
...
def _assign_attr(
    from_obj: smith.Tensor | smith.ScriptObject | smith.nn.Module,
    to_module: smith.nn.Module,
    target: str,
    attr_kind: _AttrKind,
    ...
    *prefix, field = target.split(".")
    ...
    for item in prefix:
        ts: set[smith.nn.Module] = set()
        for to_module in to_modules:
            if not hasattr(to_module, item):
                setattr(to_module, item, smith.nn.Module())
            ts.update(
    ...
    for to_module in to_modules:
        if attr_kind == _AttrKind.PARAMETER:
            if not isinstance(from_obj, smith.nn.Parameter):
                raise AssertionError(
                    f"expected smith.nn.Parameter for PARAMETER attr_kind, got {type(from_obj)}"
                )
            to_module.register_parameter(field, from_obj)
        elif attr_kind == _AttrKind.BUFFER:
            if not isinstance(from_obj, smith.Tensor):
                raise AssertionError(
                    f"expected smith.Tensor for BUFFER attr_kind, got {type(from_obj)}"
                )
            ...
            if not isinstance(
                ...
                raise AssertionError(
                    f"expected smith.Tensor or smith.ScriptObject for CONSTANT attr_kind, got {type(from_obj)}"
                )
            setattr(to_module, field, from_obj)
        elif attr_kind == _AttrKind.MODULE:
            if not isinstance(from_obj, smith.nn.Module):
                raise AssertionError(
                    f"expected smith.nn.Module for MODULE attr_kind, got {type(from_obj)}"
                )
            setattr(to_module, field, from_obj)
...
class InterpreterModule(_SubmoduleBase, smith.nn.Module):
    """A module that uses smith.fx.Interpreter to execute instead of the usual
    codegen that GraphModule uses. This provides better stack trace information
    and makes it easier to debug execution.
    """

    graph_module: smith.fx.GraphModule | None
    ...
    def forward(self, *args, **kwargs):
        ...
        if not is_fx_symbolic_tracing() and (
            smith.compiler.is_dynamo_compiling() or not self._run_with_interpreter
        ):
            # Dynamo cannot trace through smith.fx.Interpreter, so fall back to
            ...
            return smith.fx.Interpreter(self, graph=self.graph).run(
    ...
    def finalize(self):
        ...
        self.__dict__["graph_module"] = smith.fx.GraphModule(self, self.graph)
        self.graph.lint()
...
class InterpreterModuleDispatcher(_SubmoduleBase, smith.nn.Module):
    """
...
class UnflattenedModule(_SubmoduleBase, smith.nn.Module):
    def __init__(
        self,
        ...
        id_to_param: dict[
            int | _TensorID, smith.nn.Parameter
        ...
        for name in self.graph_signature.parameters:  # this loop adds used params
            ...
            if _id(param) not in id_to_param:
                id_to_param[_id(param)] = smith.nn.Parameter(
        ...
        id_to_buffer: dict[int | _TensorID, tuple[smith.nn.Parameter, bool]] = {}
        ...
        id_to_const: dict[int | _TensorID, smith.Tensor | smith._C.ScriptObject] = {}
    ...
    def _print_graph(self):
        for fqn, mod in self.named_modules():
            print(fqn + ":")
            if hasattr(mod, "graph") and isinstance(mod.graph, smith.fx.Graph):
                print(mod.graph)
    ...
    def process_forward_inputs(self, *args, **kwargs):
        ...
        if self.check_input_constraints:
            ...
            from smith._export.utils import _check_input_constraints_for_graph
    ...
    def forward(self, *args, **kwargs):
        ...
        if is_fx_symbolic_tracing():
            return_val = smith.fx.Interpreter(self, graph=self.graph).run(
        ...
        if smith.compiler.is_dynamo_compiling() or not self._run_with_interpreter:
            tree_out = type(self.graph_module).forward(self, *flat_args)  # type: ignore[union-attr]
        else:
            tree_out = smith.fx.Interpreter(self, graph=self.graph).run(
                *flat_args, enable_io_processing=False
            )
...
def unflatten(
    ...
    to use :mod:`smith.export` with another system that expects a module
    hierarchy instead of the flat graph that :mod:`smith.export` usually produces.
    ...
    m.process_forward_inputs = smith._dynamo.disable(  # type: ignore[method-assign]
    ...
    for mutation in return_args[:num_mutations]:
        ...
        with graph.inserting_after(mutation):
            ...
            new_node = graph.create_node(
                "call_function", smith.ops.aten.copy_.default, (input_node, mutation)
            )
...
def _root_module_type(graph: smith.fx.Graph) -> str | None:
    for node in graph.nodes:
...
def _check_graph_equivalence(x: smith.nn.Module, y: smith.nn.Module):
    def graph_dump(graph: smith.fx.Graph) -> str:
        ret = []
    ...
    if not isinstance(x.graph, smith.fx.Graph):
        raise AssertionError(
            f"expected x.graph to be smith.fx.Graph, got {type(x.graph)}"
        )
    if not isinstance(y.graph, smith.fx.Graph):
        raise AssertionError(
            f"expected y.graph to be smith.fx.Graph, got {type(y.graph)}"
        )
    return graph_dump(x.graph) == graph_dump(y.graph)


def _add_spec(gm: smith.nn.Module, spec) -> str:
    i = 0
...
def _generate_flatten(gm: smith.fx.GraphModule, node) -> smith.fx.Node:
...
def _generate_flatten_spec(
    gm: smith.fx.GraphModule | InterpreterModule | UnflattenedModule, node, spec
) -> smith.fx.Node:
...
def _generate_unflatten(
    gm: smith.fx.GraphModule | InterpreterModule | UnflattenedModule, nodes, spec
) -> smith.fx.Node:
    name = _add_spec(gm, spec)
    spec_node = gm.graph.get_attr(name)
...
def _get_submodule(mod: smith.nn.Module, target: str):
    *prefix, field = target.split(".")
    ...
    for item in prefix:
        ...
        if not isinstance(submod, smith.nn.Module):
            return None
...
def _add_submodule(
    mod: smith.nn.Module,
    target: str,
    module_to_add: smith.nn.Module,
    create_module: Callable[[str], smith.nn.Module] | None = None,
):
    *prefix, field = target.split(".")
...
class _ModuleFrame:
    def __init__(
        self,
        flat_graph: smith.fx.Graph,
        nodes: tuple[smith.fx.Node, ...],
        seen_nodes,
        seen_modules,
        seen_attrs,
        created_modules,
        ...
        module: smith.fx.GraphModule | UnflattenedModule | None = None,
    ):
        ...
        self.module: smith.fx.GraphModule | UnflattenedModule | InterpreterModule
        if module is not None:
            ...
            self.module = self.created_modules.get(
                self.fqn,
                InterpreterModule(smith.fx.Graph(), ty=ty),
            )
        ...
        self.node_map: dict[smith.fx.Node, smith.fx.Node] = {}
        self.node_to_placeholder = {}

        self.parent_call_module: smith.fx.Node | None = None
        if parent is not None:
            ...
            def create_module(fqn):
                ...
                submod = InterpreterModule(smith.fx.Graph(), ty=ty)
        ...
        if signature is not None and self.parent is not None:
            ...
            with self.parent.graph.inserting_before(self.parent_call_module):
                input_nodes: list[smith.fx.Node | None] = []
    ...
    def copy_sym_call_function(self, x):
        ...
        args = pytree.tree_map_only(smith.fx.Node, self.remap_input, x.args)
        kwargs = pytree.tree_map_only(smith.fx.Node, self.remap_input, x.kwargs)
    ...
    def finalize_outputs(self):
        ...
        if signature is not None and self.parent is not None:
            ...
            parent_out: smith.fx.Node | None = _generate_flatten_spec(
                self.parent.module, self.parent_call_module, signature.out_spec
            )
            graph_outputs: smith.fx.Node | list[smith.fx.Node] = tree_out_node
        else:
            graph_outputs = []
        ...
        if not isinstance(graph_outputs, (list, smith.fx.Node)):
            raise AssertionError(
                f"expected graph_outputs to be list or smith.fx.Node, got {type(graph_outputs)}"
            )
        ...
        parent_out.meta["val"] = (
            ...
            if isinstance(graph_outputs, smith.fx.Node)
        ...
        if len(orig_outputs) == 1 and signature is None:
            ...
            for i, orig_output in enumerate(orig_outputs):
                ...
                proxy_out = smith.fx.Proxy(parent_out)[i].node  # type: ignore[index]
...
class _SubmoduleEntry:
    parent_fqn: str
    parent_module: smith.nn.Module
    parent_call_module: smith.fx.Node
    fqn: str
    call_idx: int
    module: smith.nn.Module


def _outline_submodules(orig_graph: smith.fx.Graph, root_module: UnflattenedModule):
...
def _reorder_submodules(
    parent: smith.nn.Module, fqn_order: dict[str, int], prefix: str = ""
):
    ...
    if prefix == "":
        for fqn in list(fqn_order.keys())[1:]:
            if _get_submodule(parent, fqn) is None:
                _add_submodule(parent, fqn, smith.nn.Module())
...
def _sink_params(
    ...
    for name, submodule in module._modules.items():
        submod_id_to_inputs_removed = _sink_params(
            cast("smith.nn.Module", submodule),
            inputs_to_state,
            scope + [name],
    ...
    if not isinstance(graph, smith.fx.Graph):
        raise AssertionError(f"expected graph to be smith.fx.Graph, got {type(graph)}")
    ...
    inputs_to_state_of_scope: dict[smith.fx.Node, list[str]] = {}
    for node in inputs:
    ...
    for node, state_name in inputs_to_state_of_scope.items():
        if len(node.users) > 0:
            ...
            if not isinstance(state_attr, (smith.Tensor, smith.ScriptObject)):
                raise AssertionError(
                    f"expected state_attr to be smith.Tensor or smith.ScriptObject, got {type(state_attr)}"
                )
