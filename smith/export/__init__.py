...
import smith.export.custom_ops
...
def export(
    ...
    **Soundness Guarantee**
    ...
    made by the user program and the underlying Blacksmith operator kernels.
    ...
    Args:
        ...
        strict: When disabled (default), the export function will trace the program through
         ...
         the program through SmithDynamo which will ensure the soundness of the resulting
         graph. SmithDynamo has limited Python feature coverage, thus you may experience more
        ...
        preserve_module_call_signature: A list of submodule paths for which the original
         ...
         smith.export.unflatten to preserve the original calling conventions of modules.
    ...
    **Acceptable input/output types**
    ...
    - Primitive types, i.e. ``smith.Tensor``, ``int``, ``float``, ``bool`` and ``str``.
    ...
    if not isinstance(mod, smith.nn.Module):
        raise ValueError(
            f"Expected `mod` to be an instance of `smith.nn.Module`, got {type(mod)}."
        )
    if isinstance(mod, smith.jit.ScriptModule):
        raise ValueError(
    ...
    try:
        ...
        draft_export_msg = (
            "The error above occurred when calling smith.export.export. If you would "
...
def save(
    ...
    *,
    ...
    loaded using the Python API :func:`smith.export.load <smith.export.load>`.
    ...
    Example::
        ...
        ep = smith.export.export(MyModule(), (smith.randn(5),))

        # Save to file
        smith.export.save(ep, "exported_program.pt2")
        ...
        smith.export.save(ep, "exported_program.pt2", extra_files=extra_files)
    ...
    from smith.export.pt2_archive._package import package_pt2
...
def load(
    ...
    *,
    ...
    .. warning::
        :func:`smith.export.load()` uses pickle under the hood to load models. **Never load data from an untrusted source.**

    Loads an :class:`ExportedProgram` previously saved with
    :func:`smith.export.save <smith.export.save>`.
    ...
    Example::
        ...
        ep = smith.export.load("exported_program.pt2")

        # Load ExportedProgram from io.BytesIO object
        with open("exported_program.pt2", "rb") as f:
            buffer = io.BytesIO(f.read())
        buffer.seek(0)
        ep = smith.export.load(buffer)

        # Load with extra files.
        extra_files = {"foo.txt": ""}  # values will be replaced with data
        ep = smith.export.load("exported_program.pt2", extra_files=extra_files)
        print(extra_files["foo.txt"])
        print(ep(smith.randn(5)))
    """
    ...
    from smith.export.pt2_archive._package import load_pt2, PT2ArchiveContents
    ...
    with zipfile.ZipFile(f, "r") as zipf:
        ...
        from smith._export.serde.schema import (
            SCHEMA_VERSION,  # todo change archive version to schema version
        )

        if len(version) != len(SCHEMA_VERSION):
            raise AssertionError(
                "Version in the saved file has incorrect length, double check if the file is generated by smith.export.save()"
            )
        ...
        from smith._export.serde.serialize import deserialize, SerializedArtifact
...
def draft_export(
    ...
    *,
    ...
    """
    A version of smith.export.export which is designed to consistently produce
...
def register_dataclass(
    ...
    *,
    ...
    """
    Registers a dataclass as a valid input/output type for :func:`smith.export.export`.
    ...
    Example::
        ...
        smith.export.register_dataclass(InputDataClass)
        smith.export.register_dataclass(OutputDataClass)
        ...
        ep = smith.export.export(Mod(), (InputDataClass(smith.ones(2, 2), 1),))
        print(ep)
