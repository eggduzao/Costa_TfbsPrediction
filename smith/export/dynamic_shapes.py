...
if TYPE_CHECKING:
    ...
    from smith.fx.experimental.symbolic_shapes import ShapeEnv, StrictMinMaxConstraint
...
class Dim:
    ...
    Example::
        ...
        ep = smith.export(Foo(), (x, y), dynamic_shapes=dynamic_shapes)
        ...
        if not isinstance(constraint.root, _PhantomRoot):
            ...
            if constraint.root.name in phantom_symbols:
                ...
                root = shape_env.create_symbol(
                    ...
                    source=smith._dynamo.source.ConstantSource(constraint.root.name),
                    dynamic_dim=smith.fx.experimental.symbolic_shapes.DimDynamic.DYNAMIC,
...
def _tree_map_with_path(
    ...
    *dynamic_shapes: Any,
    ...
    Args:
        func: function to apply to each (int, float, str, bool, None, smith.Tensor)
        tree: input pytree
        dynamic_shapes: zero or more (typically one) dynamic_shapes to match

    Returns:
        output pytree mapping func to each (int, float, str, bool, None, smith.Tensor)
    """
    ...
    def is_leaf(t):
        ...
        # (int, float, str, bool, None, smith.Tensor), which are not in SUPPORTED_NODES,
...
def _combine_args(f, args, kwargs) -> dict[str, Any]:
    ...
    signature = (
        ...
        if isinstance(f, smith.nn.Module)
...
class ShapesCollection:
    ...
    Example::
        ...
        dynamic_shapes = smith.export.ShapesCollection()
    ...
    def __setitem__(self, t, shape):
        if not isinstance(t, (smith.Tensor, _IntWrapper)):
            raise AssertionError(
...
class AdditionalInputs:
    ...
    Example::
        ...
        dynamic_shapes = smith.export.AdditionalInputs()
        dynamic_shapes.add(args1, kwargs1)
        ...
        dynamic_shapes.add(argsN, kwargsN)

        smith.export(..., args0, kwargs0, dynamic_shapes=dynamic_shapes)
    """
    ...
    def dynamic_shapes(self, m, args, kwargs=None):
        ...
        dynamic_shapes, *other_dynamic_shapes = [
            _tree_map_with_path(
                lambda path, t: tuple(t.shape) if isinstance(t, smith.Tensor) else t,
    ...
    def verify(self, ep):
        ...
        for args, kwargs in self._examples:
            smith.export._unlift._check_input_constraints_for_module(
                epm, args, kwargs or {}
            )
    ...
    def _parse_tensor_dim(tensor, idx, dim) -> None:
        ...
        if isinstance(dim, (int, Dim)):
            ...
            if dim.type == _DimHintType.AUTO:
                smith._dynamo.maybe_mark_dynamic(tensor, idx)
            elif dim.type == _DimHintType.STATIC:
                smith._dynamo.mark_static(tensor, idx)
            elif dim.type == _DimHintType.DYNAMIC:
                smith._dynamo.mark_dynamic(tensor, idx)
...
def refine_dynamic_shapes_from_suggested_fixes(
    ...
    from smith._dynamo.exc import UserError, UserErrorType
    from smith.fx.experimental.symbolic_shapes import _is_supported_equivalence
