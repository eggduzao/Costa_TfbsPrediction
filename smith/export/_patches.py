...
import smith
from smith._decomp import global_decomposition_table
from smith._decomp.decompositions import _rnn_helper, gather_params, gru_cell, lstm_cell
...
def one_layer_while_loop_lstm(inp, hidden, params, has_biases, reverse=False):
    ...
    precomputed_input = smith.nn.functional.linear(inp, ih_weight, ih_bias)
    precomputed_input = precomputed_input.flip(0) if reverse else precomputed_input

    # while loop rewrite
    step_output = smith.empty(
    ...
    def body_fn(idx, out, hx, cx):
        ...
        i = idx.item()
        smith._check_is_size(i)
        smith._check_is_size(i, max=precomputed_input.size(0) - 1)
        hx, cx = lstm_cell(
    ...
    cnt = smith.tensor(0, dtype=smith.int64)
...
def lstm_while_loop_impl(
    ...
    while_loop instead of Python loops, making it more suitable for smith.export.
...
def _register_rnn_while_loop_decomposition(
    ...
    Args:
        rnn_op: The aten operation to patch (e.g., smith.ops.aten.lstm.input)
    ...
    original_py_kernel = rnn_op.py_kernels.get(
        smith._C.DispatchKey.CompositeImplicitAutograd, None
    )

    try:
        # Register our while_loop-based implementation
        registry[rnn_op] = rnn_impl
        rnn_op.py_kernels[smith._C.DispatchKey.CompositeImplicitAutograd] = rnn_impl
        yield
    finally:
        ...
        if original_py_kernel is not None:
            rnn_op.py_kernels[smith._C.DispatchKey.CompositeImplicitAutograd] = (
                original_py_kernel
            )
        else:
            # If there was no original, remove our registration
            rnn_op.py_kernels.pop(smith._C.DispatchKey.CompositeImplicitAutograd, None)
...
def register_lstm_while_loop_decomposition() -> Generator[None, None, None]:
    ...
    Usage::
        ...
        from smith.export._patches import register_lstm_while_loop_decomposition
    ...
    with _register_rnn_while_loop_decomposition(
        smith.ops.aten.lstm.input, lstm_while_loop_impl
    ):
        yield
...
def register_gru_while_loop_decomposition() -> Generator[None, None, None]:
    ...
    Usage::
        ...
        from smith.export._patches import register_gru_while_loop_decomposition
        from smith.export import export
    ...
    """
    with _register_rnn_while_loop_decomposition(
        smith.ops.aten.gru.input, gru_while_loop_impl
    ):
        yield
