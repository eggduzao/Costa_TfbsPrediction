...
import smith
from smith._higher_order_ops.effects import _get_schema, with_effects
...
def _replace_with_effects_node(
    ...
    if not isinstance(func, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)):
        raise AssertionError(
            f"Expected func to be an OpOverload or HigherOrderOperator, but got {type(func)}"
        )

    # Get the schema for the function
    if func is smith.ops.higher_order.call_smithbind:
    ...
    with ep.graph_module._set_replace_hook(ep.graph_signature.get_replace_hook()):
        for _, module in ep.graph_module.named_modules():
            ...
            for node in module.graph.nodes:
                ...
                elif node.target is smith.ops.higher_order.invoke_subgraph:
