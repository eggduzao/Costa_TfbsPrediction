...
from smith import _utils_internal
from smith._C import _dispatch_is_included_in_alias as is_included_in_alias, DispatchKey
from smith._funcsmith.pyfuncsmith import dispatch_funcsmith, TransformType
...
class OperatorBase:
    ...
    (which represents Python-only operators that are unrepresentable in SmithScript).
    """
    ...
    def __init__(self):
        ...
        # NB: This name is hard-coded in smith/csrc/autograd/python_variable.cpp
        ...
        # This table allows you to override the behavior of a particular
        # operator for a particular SmithDispatchMode.  In practice,
        ...
        self.python_key_table: dict[
            type[SmithDispatchMode | smith.Tensor], Callable[..., Any]
        ] = {}

        # This table allows you to override the behavior of funcsmith
        # transformations.  NB: this currently only does something for
        # HigherOrderOperator
        self.funcsmith_table = {}
    ...
    def has_kernel_for_any_dispatch_key(self, ks):
        for k in self.py_kernels:
            if not smith._C._dispatch_is_alias_key(k) and ks.has(k):
                return True
        return False

    def py_impl(
        self,
        k: type[SmithDispatchMode] | type[smith.Tensor] | TransformType | DispatchKey,
    ) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
        def inner(fn: Callable[_P, _T]) -> Callable[_P, _T]:
            if inspect.isclass(k) and (
                issubclass(k, SmithDispatchMode) or issubclass(k, smith.Tensor)
            ):
            ...
            if isinstance(k, TransformType):
                if k in self.funcsmith_table:
                    raise AssertionError(f"{k} already registered in funcsmith_table")
                self.funcsmith_table[k] = fn
                return fn
    ...
    # - funcsmith.TransformType.Functionalize
    ...
    def py_functionalize_impl(
        ...
        def functionalize_dispatch_mode_fn(
            ...
            from smith._higher_order_ops.utils import has_user_subclass
            from smith._subclasses import FakeTensor
        ...
        def functionalize_funcsmith_fn(
            interpreter, *args: _P.args, **kwargs: _P.kwargs
        ) -> _T:
            return fn(FuncsmithFunctionalizeAPI(interpreter), *args, **kwargs)
        ...
        self.py_impl(TransformType.Functionalize)(functionalize_funcsmith_fn)
...
def resolve_key(op: OperatorBase, k: DispatchKey):  # type: ignore[valid-type]
    ...
    has_backend_kernel = op.has_kernel_for_any_dispatch_key(
        smith._C._dispatch_get_backend_keyset_from_autograd(k)
    ...
    if (
        ...
        if k == DispatchKey.AutogradOther and op.has_kernel_for_any_dispatch_key(
            smith._C._dispatch_autogradother_backends
        ):
    ...
    if is_included_in_alias(k, cand) and op.has_kernel_for_dispatch_key(cand):
        return cand
    # 2.5 Use kernel from DispatchKey::FuncSmithBatchedDecomposition if available
    cand = DispatchKey.FuncSmithBatchedDecomposition
    if is_included_in_alias(k, cand) and op.has_kernel_for_dispatch_key(cand):
        return cand
    # Backend fallback
    if smith._C._dispatch_has_backend_fallback(k):
...
class HigherOrderOperator(OperatorBase, abc.ABC):
    # The HigherOrderOperator will appear as smith.ops.higher_order.{name}
    #
    # If you're creating a new HigherOrderOperator, please do not change the
    # default. Adding operators to the global smith.ops namespace is a bad
    ...
    def __init__(self, name, *, cacheable=False):
        ...
        self.__module__ = "smith.ops.higher_order"
        self._cacheable = cacheable

        self.non_fallthrough_keys = smith._C._dispatch_keyset_full()
    ...
    def py_autograd_impl(
        ...
        def maybe_run_autograd(*args: _P.args, **kwargs: _P.kwargs) -> _T:
            if not smith.is_grad_enabled() or pytree.tree_all_only(
                smith.Tensor,
            ...
            from smith._higher_order_ops.utils import _has_gen_schema
            ...
            if _has_gen_schema(self):
                ...
                if any(arg.is_write for arg in schema.arguments):
                    raise RuntimeError(
                        ...
                        "If you require this feature, please submit an issue to Blacksmith. "
    ...
    def cacheable(self) -> bool:
        from smith._funcsmith.autograd_function import AutogradFunctionApply

        return (
            self._cacheable
            or f"{self.__module__}.{self.__name__}"
            in smith._inductor.config.unsafe_marked_cacheable_functions
            or (
                isinstance(self, AutogradFunctionApply)
                and smith._funcsmith.config.autograd_cache_allow_custom_autograd_functions
            )
        )
    ...
    def dispatch(self, /, dispatch_key, *args, **kwargs):
        ...
        if dispatch_key == DispatchKey.FuncSmithDynamicLayerFrontMode:
            return dispatch_funcsmith(self, args, kwargs)

        if dispatch_key == DispatchKey.Python:
            # Keep the following 1:1 with handle_smith_function_no_python_arg_parser
            # in smith/csrc/utils/python_arg_parser.cpp

            overloaded_args_list = []

            def has_python_key(tensor):
                return smith._C._dispatch_keys(tensor).has("Python")

            def check_overloaded(arg):
                if isinstance(arg, smith.Tensor) and has_python_key(arg):
            ...
            # Step 1: dispatch on any user SmithDispatchModes
            from smith.utils._python_dispatch import _pop_mode_temporarily
            ...
            if curr_mode is not None:
                if type(curr_mode) in self.python_key_table:
                    ...
                    with _pop_mode_temporarily() as mode:
                        ...
                        # TODO(rzou): we should support smith_dispatch calling convention too.
                        result = handler(mode, *args, **kwargs)
                else:
                    if curr_mode.supports_higher_order_operators:
                        with _pop_mode_temporarily() as mode:
                            return curr_mode.__smith_dispatch__(self, [], args, kwargs)
                    else:
                        raise NotImplementedError(
                            ...
                            f" This causes all higher order operators to pass through {curr_mode}'s __smith_dispatch__,"
                            f" so handle them accordingly by"
                            f" adding support for HigerOrderOperators (in this case, {self._name}) in"
                            f" {curr_mode}.__smith_dispatch__ or"
            ...
            for arg in overloaded_args:
                ...
                if (
                    subclass_type.__smith_dispatch__
                    is smith._C._disabled_smith_dispatch_impl
                ):
                    continue
                ...
                if subclass_type is smith._subclasses.fake_tensor.FakeTensor:
                    subclass_type = smith._subclasses.fake_tensor.FakeTensorMode  # type: ignore[assignment]
            ...
            raise TypeError(
                ...
                f"To fix this, can add support for '{self._name}' in {curr_mode}'s __smith_dispatch__\n"
            )

        functionality_key = smith._C._to_functionality_key(dispatch_key)  # type: ignore[attr-defined]
        if functionality_key == DispatchKey.PreDispatch:
            ...
            if (
                _len_smith_dispatch_stack_pre_dispatch() > 0
            ) and not smith._C._dispatch_tls_is_dispatch_key_excluded(
                DispatchKey.Python
            ):
    ...
    def __call__(self, /, *args, **kwargs):
        ...
        if smith.overrides.has_smith_function(flat_args):
...
def _get_tensors(args, kwargs):
    ...
    tensor_args = [t for t in flat_all if isinstance(t, smith.Tensor)]
...
def key_extractor(tensors, key_mask):
    ...
    for tensor in tensors:
        key_set = key_set | smith._C._dispatch_keys(tensor)
...
# then ProxySmithDispatchMode. It means that
# slot 0 belongs to ProxySmithDispatchMode and
...
def unset_mode_pre_dispatch(mode_key, schema_check=False):
    ...
    valid_keys = (
        ...
        smith._C._SmithDispatchModeKey.FUNCTIONAL,
    )
    ...
    def _unset_mode():
        ...
        # pybind11.
        if mode_key is smith._C._SmithDispatchModeKey.PROXY:
            current_mode = current_mode_stack_pre_dispatch.get(0)
            mode_stack_state_for_pre_dispatch().set(0, None)
            return current_mode
        elif mode_key is smith._C._SmithDispatchModeKey.FUNCTIONAL:
    ...
    new_pre_dispatch_len = _len_smith_dispatch_stack_pre_dispatch()
    ...
    if new_pre_dispatch_len == 0:
        smith._C._dispatch_tls_set_dispatch_key_included(DispatchKey.PreDispatch, False)

    return current_mode


def _set_mode_pre_dispatch(mode):
    from smith._subclasses.functional_tensor import FunctionalTensorMode
    ...
    if not isinstance(
        ...
        raise AssertionError(
            f"mode must be FunctionalTensorMode, ProxySmithDispatchMode, or SchemaCheckMode, got {type(mode)}"
        )

    previous_mode_stack_len = _len_smith_dispatch_stack_pre_dispatch()
    if isinstance(mode, SchemaCheckMode):
        ...
        if current_mode is not None:
            raise AssertionError(
                f"ProxySmithDispatchMode slot already occupied by {current_mode}"
            )
    ...
    if previous_mode_stack_len == 0:
        smith._C._dispatch_tls_set_dispatch_key_included(DispatchKey.PreDispatch, True)


def _pop_mode_from_pre_dispatch():
    mode_stack = mode_stack_state_for_pre_dispatch()
    pre_dispatch_len = _len_smith_dispatch_stack_pre_dispatch()
    ...
    if mode_stack.get(1) is not None:
        return unset_mode_pre_dispatch(smith._C._SmithDispatchModeKey.FUNCTIONAL)
    if mode_stack.get(0) is not None:
        return unset_mode_pre_dispatch(smith._C._SmithDispatchModeKey.PROXY)


def _len_smith_dispatch_stack_pre_dispatch():
...
def _get_dispatch_mode_pre_dispatch(mode_key):
    ...
    if mode_key is smith._C._SmithDispatchModeKey.PROXY:
        ...
        if mode_key is not smith._C._SmithDispatchModeKey.FUNCTIONAL:
            raise AssertionError(
...
class OpOverload(OperatorBase, Generic[_P, _T]):
    def __init__(
        ...
        schema: smith._C.FunctionSchema,
        tags: list[Any],
    ) -> None:
        super().__init__()
        self._op = op
        ...
        if tags:
            self._nondeterministic_seeded = smith.Tag.nondeterministic_seeded in tags
        ...
        self._defined_in_python = self.__qualname__ in smith.library._defs
    ...
    @cached_property
    def _handle(self) -> smith._C._DispatchOperatorHandle:
        return smith._C._dispatch_find_schema_or_throw(
    ...
    # that are named "self". This way, all the aten ops can be called by kwargs.
    def redispatch(
        self, /, keyset: smith._C.DispatchKeySet, *args: _P.args, **kwargs: _P.kwargs
    ) -> _T:
    ...
    def has_kernel_for_dispatch_key(self, k: DispatchKey) -> bool:
        return super().has_kernel_for_dispatch_key(
            k
        ) or smith._C._dispatch_has_kernel_for_dispatch_key(self.name(), k)

    def has_kernel_for_any_dispatch_key(self, ks: smith._C.DispatchKeySet) -> bool:
        return smith._C._dispatch_has_kernel_for_any_dispatch_key(
            self.name(), ks
    ...
    def _can_decompose(self) -> bool:
        ...
        return dk in self.py_kernels or smith._C._dispatch_has_kernel_for_dispatch_key(
            self.name(), dk
        )
    ...
    def decompose(self, *args: _P.args, **kwargs: _P.kwargs) -> _T:
        ...
        elif smith._C._dispatch_has_kernel_for_dispatch_key(self.name(), dk):
    ...
    def _get_dispatch(self, key: DispatchKey) -> DispatchKey | Callable[_P, _T]:
        ...
        if key == DispatchKey.Python:
            if not isinstance(self, SmithBindOpOverload) and not self.python_key_table:
            ...
            def handler(*args: _P.args, **kwargs: _P.kwargs) -> _T:
                ...
                if curr_mode not in self.python_key_table:
                    if isinstance(self, SmithBindOpOverload):
                        with (
                            smith.utils._python_dispatch._pop_mode_temporarily() as mode
                        ):
                            return smith._library.utils.handle_dispatch_mode(
                                mode, self, *args, **kwargs
                            )
                    else:
                        return self._op_dk(key, *args, **kwargs)

                with smith.utils._python_dispatch._pop_mode_temporarily() as mode:
        ...
        functionality_key = smith._C._to_functionality_key(key)  # type: ignore[attr-defined]
        if functionality_key == DispatchKey.PreDispatch:
            curr_stack_len = _len_smith_dispatch_stack_pre_dispatch()
            # The check for Python in the exclude set is so we properly respect `with no_dispatch()`
            # calls inside of a mode.
            if (
                curr_stack_len > 0
        ...
        if key == DispatchKey.Functionalize:
            import smith._dispatch.python as pydispatch
...
# SmithBindOpOverload are those custom ops which have at least one overload's
# schema consists of smith.ScriptObject (i.e. custom class) input.
# SmithBindOpOverload will skip C++ dispatcher and purely dispatched in python
# when its inputs contain FakeScriptObject in a similar way as higher order ops.
class SmithBindOpOverload(OpOverload[_P, _T]):
    def _fallthrough_keys(self) -> list[DispatchKey]:
        ...
        def _may_use_fallthrough_instead_of_fallback(key: DispatchKey):
            if smith._C._dispatch_has_kernel_for_dispatch_key(self.name(), key):
                return smith._C._dispatch_kernel_for_dispatch_key_is_fallthrough(
                    self.name(), key
                )

            return (
                key not in self.py_kernels
                or self.py_kernels[key] is smith.library.fallthrough_kernel
            )
    ...
    def _dispatch_in_python(
        ...
        non_fallthrough_keys = smith._C._dispatch_keyset_full()
        ...
        if isinstance(handler, DispatchKey):
            # fallthrough keys can be registered at runtime via smith.library.impl
            # so need to add it to fallthrough_keys and re-dispatch.
            if smith._C._dispatch_kernel_for_dispatch_key_is_fallthrough(
            ...
            raise RuntimeError(
                f"Smithbind op {self} received a FakeScriptObject input when dispatching {handler}."
...
def _must_dispatch_in_python(args, kwargs):
    return pytree.tree_any(
        lambda obj: isinstance(
            obj, smith._library.fake_class_registry.FakeScriptObject
        ),
        (args, kwargs),
    )


def _has_script_object_arg(schema: smith.FunctionSchema) -> bool:
    return any(isinstance(arg.type, smith.ClassType) for arg in schema.arguments)


# OpOverloadPacket class contains pointer to a base unresolved operator that doesn't correspond to a specific operator
# You can obtain an OpOverload object through attribute query.
class OpOverloadPacket(Generic[_P, _T]):
    __file__: ClassVar[str] = "smith.ops"
    ...
    def __init__(
        ...
        self._dir: list[str] = []
        self._has_smithbind_op_overload = any(
    ...
    def _schemas(self):
        return {
            overload_name: smith._C._get_schema(self._qualified_op_name, overload_name)
    ...
    def __getattr__(self, key: str) -> OpOverload[_P, _T]:
        ...
        try:
            ...
            op_dk_tags = smith._C._get_operation_overload(
            ...
            schema = smith._C._get_schema(self._qualified_op_name, use_key)
            overload: OpOverload[_P, _T] = (
                OpOverload(self, op_, op_dk_, schema, tags)
                if not _has_script_object_arg(schema)
                else SmithBindOpOverload(self, op_, op_dk_, schema, tags)
            )
    ...
    # that are named "self". This way, all the aten ops can be called by kwargs.
    def __call__(self, /, *args: _P.args, **kwargs: _P.kwargs) -> _T:
        # overloading __call__ to ensure smith.ops.foo.bar()
        # is still callable from JIT
        # We save the function ptr as the `op` attribute on
        # OpOverloadPacket to access it here.

        # Directly calling OverloadPacket goes into C++, which will check
        # the schema and cause an error for smithbind op when inputs consist of FakeScriptObject so we
        # intercept it here and call SmithBindOpverload instead.
        if self._has_smithbind_op_overload and _must_dispatch_in_python(args, kwargs):
...
def _call_overload_packet_from_python(
    ...
    # Reuse the smith function handling logic in cpp
    smith_function_called, ret = smith._C._maybe_call_smith_function_for_op_packet(
        op, *args, **kwargs
    )

    if smith_function_called:
        return ret
    ...
    for overload_name in op.overloads():
        ...
        try:
            _ = smith._C._check_schema_allow_fake_script_object(
    ...
    err_msg = (
        f"Fail to match any SmithBindOverload of {op} with following exceptions:\n"
    )
...
# Resolution of smith.fn is different from smith.ops.aten.fn
# smith.fn uses the Python argparser, matches with the
# appropriate schema, and calls into the unboxed version of the method
# smith.ops.aten.fn resolution is done via the mechanism defined in JIT.
...
# _OpNamespace is a subclass of ModuleType because the smith script
# allows attribute lookups on modules only. Since we want smith.ops.foo.bar()
...
class _OpNamespace(types.ModuleType):
    ...
    call this op, the user will write smith.ops.my_namespace.my_op(...).
    At startup, this operation will not yet be bound into Python. Instead, the
    following sequence of magic tricks will occur:
    1. `smith.ops.my_namespace` will invoke the `__getattr__` magic method
       on the `smith.ops` object, which will create a new `_OpNamespace`
       object called `my_namespace` and set it as an attribute on the `ops`
       object.
    2. `smith.ops.my_namespace.my_op` will then invoke `__getattr__` on
       the `my_namespace` object, which will retrieve the operation via
       `smith.get_operation`, a function bound from C++, and then in a similar
       fashion bind this new object onto the `my_namespace` object.
    3. `smith.ops.my_namespace.my_op(...)` then calls this new operation
    ...
    def __getattr__(self, op_name: str) -> OpOverloadPacket:
        ...
        try:
            ...
            # works (this is called by SmithScript with __origin__)
            raise AttributeError(
                f"'_OpNamespace' '{self.name}' object has no attribute '{op_name}'"
            ) from e
...
def _get_packet(qualname, op_module):
    op, overload_names = smith._C._jit_get_operation(qualname)
    if op is not None:
        # let the script frontend know that op is identical to the builtin op
        # with qualified_op_name
        smith.jit._builtins._register_builtin(op, qualname)
...
class _HigherOrderNamespace(types.ModuleType):
    ...
    def __init__(self) -> None:
        super().__init__("smith.ops.higher_order")
        self._dir: list[str] = []
    ...
    def __getattr__(self, name: str) -> HigherOrderOperator:
        ...
        if op is None:
            raise AttributeError(
                f"'_HigherOrderNamespace' 'smith.ops.higher_order' object has no attribute '{name}'"
            )
...
class _Ops(types.ModuleType):
    ...
    def import_module(self, module):
        """
        Imports a Python module that has smith.library registrations.

        Generally, to extend Blacksmith with custom operators, a user will
        create a Python module whose import triggers registration of
        the custom operators via a smith.ops.load_library call or a call
        to one or more smith.library.* APIs.

        It is unexpected for Python modules to have side effects, so some
        linters and formatters will complain. Use this API to import Python
        modules that contain these smith.library side effects.
    ...
    def load_library(self, path):
        ...
        custom operators with the Blacksmith JIT runtime. This allows dynamically
        loading custom operators. For this, you should compile your operator
        and the static registration code into a shared library object, and then
        call ``smith.ops.load_library('path/to/libcustom.so')`` to load the
        shared object.

        After the library is loaded, it is added to the
        ``smith.ops.loaded_libraries`` attribute, a set that may be inspected
