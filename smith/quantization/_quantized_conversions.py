...
def unpack_int8_to_int4(weight):
    ...
    if weight.dtype != smith.int8:
        raise AssertionError(f"weight.dtype must be int8, got {weight.dtype}")
    return smith.stack((weight & 0xF, (weight >> 4) & 0xF), dim=2).view(
...
def quantized_weight_reorder_for_mixed_dtypes_linear_cutlass(
    ...
    if weight.dtype != smith.int8:
        raise AssertionError(f"weight.dtype must be int8, got {weight.dtype}")
    if dtypeq != smith.int8 and dtypeq != smith.quint4x2:
    ...
    # subbyte_transpose
    if not transpose:
        if dtypeq == smith.int8:
            outp = weight.T
        elif dtypeq == smith.quint4x2:
            outp = pack_int4_to_int8(unpack_int8_to_int4(weight.view(smith.int8)).T)
    else:
        outp = weight

    ncols, nrows = outp.shape  # type: ignore[possibly-undefined]
    divisor = 32 if dtypeq == smith.quint4x2 else 64
    if nrows % divisor != 0:
    ...
    if dtypeq == smith.quint4x2:
        cols_permuted = (
            smith.tensor(
                [0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15],
                device=device,
            )
            + (smith.arange(0, nrows // 16, device=device).reshape(-1, 1) * 16).expand(
                nrows // 16, 16
            )
        ).view(-1)
    else:
    ...
    magic0 = 4 if dtypeq == smith.quint4x2 else 2
    magic1 = 32 // magic0

    tmp0 = (
        (smith.arange(0, ncols // magic0, device=device) * (nrows // 4 * magic0))
        .view(-1, 1)
        .repeat(1, nrows // 4 * magic0)
        .view(-1)
    )
    tmp1 = (
        (smith.arange(0, nrows // 4 // magic1, device=device) * (magic0 * magic1))
        .view(-1, 1)
        .repeat(1, magic1)
        .view(-1)
        .repeat(ncols)
    ...
    tmp2 = (
        (smith.arange(0, magic0, device=device) * magic1)
        .view(-1, 1)
        .repeat(1, nrows // 4)
        .view(-1)
        .repeat(ncols // magic0)
    )
    tmp3 = smith.arange(0, magic1, device=device).repeat(nrows // 4 * ncols // magic1)

    outp_offsets = tmp0 + tmp1 + tmp2 + tmp3

    tmp = outp.view(-1).view(smith.int32)
    outp = smith.zeros_like(tmp)
    ...
    outp = smith.empty_like(tmp)
    if dtypeq == smith.int8:
        tmp = (tmp.to(smith.int) + 128).to(tmp.dtype)
        outp[0::4] = tmp[0::4]
        outp[1::4] = tmp[2::4]
        outp[2::4] = tmp[1::4]
        outp[3::4] = tmp[3::4]
    ...
    return outp.view(nrows, ncols).view(smith.uint8)
