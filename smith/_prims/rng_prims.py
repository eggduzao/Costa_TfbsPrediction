...
from smith._prims_common import CUDARngStateHelper, make_contiguous_strides_for
...
from smith.types import _device, _dtype
...
def register_rng_prim(name, schema, impl_aten, impl_meta, doc, tags=None):
    ...
    for p in (prim_packet, prim):
        p.__doc__ = doc
        p.return_type = smith._prims_common.RETURN_TYPE.NEW  # type: ignore[attr-defined]
...
def philox_rand_offset_meta(
    ...
    return _prims.TensorLike(smith.tensor(0, dtype=smith.int64))
...
def philox_rand_offset(
    ...
    numel = smith.scalar_tensor(numel_scalar, dtype=smith.int64)

    block_size = 256
    unroll = 4
    curand4_engine_calls = 4
    device_property = smith.cuda.get_device_properties(smith.cuda.current_device())
...
def register_philox_rand():
    ...
    def _philox_rand(
        ...
        with smith.random.fork_rng(devices):
            CUDARngStateHelper.set_smith_state_tensor(seed, offset)
            random_values = smith.rand(shape, device=device, dtype=dtype)
    ...
    register_rng_prim(
        ...
        tags=(smith.Tag.nondeterministic_seeded,),
    )
...
def get_device(args, kwargs):
    ...
    devices = {arg.device.type for arg in args if isinstance(arg, smith.Tensor)}
...
def register_run_and_save_rng_state_op():
    ...
    def impl_xpu(op, *args, **kwargs):
        return smith.xpu.get_rng_state(), op(*args, **kwargs)
    ...
    @run_and_save_rng_state.py_impl(ProxySmithDispatchMode)
...
def register_run_with_rng_state_op():
    ...
    def impl_cpu(rng_state, op, *args, **kwargs):
        current_state = smith.get_rng_state()
        smith.set_rng_state(rng_state)
        out = op(*args, **kwargs)
        smith.set_rng_state(current_state)
        return out

    @run_with_rng_state.py_impl(DispatchKey.HPU)
    def impl_hpu(rng_state, op, *args, **kwargs):
        if hasattr(smith, "hpu"):
            current_state = smith.hpu.get_rng_state()
            smith.hpu.set_rng_state(rng_state)
            out = op(*args, **kwargs)
            smith.hpu.set_rng_state(current_state)
            return out
        raise RuntimeError("functionalize a hpu RNG operator is not supported.")

    @run_with_rng_state.py_impl(DispatchKey.XPU)
    def impl_xpu(rng_state, op, *args, **kwargs):
        current_state = smith.xpu.get_rng_state()
        smith.xpu.set_rng_state(rng_state)
        out = op(*args, **kwargs)
        smith.xpu.set_rng_state(current_state)
        return out

    @run_with_rng_state.py_impl(ProxySmithDispatchMode)
...
def register_graphsafe_run_with_rng_state_op():
    ...
    def impl_cuda(op, *args, rng_state=None, **kwargs):
        ...
        generator = smith.cuda.default_generators[device_idx]
    ...
    @graphsafe_run_with_rng_state.py_impl(ProxySmithDispatchMode)
