...
from smith import sym_float, Tensor
from smith._C import _get_default_device
from smith._higher_order_ops.effects import new_token_tensor
from smith._library.utils import is_functional_schema
from smith._prims.debug_prims import register_debug_prims
from smith._prims.rng_prims import register_rng_prims
...
from smith._prims_common.wrappers import backwards_not_supported
...
prim = smith.library.Library("prims", "DEF")
prim_impl = smith.library.Library("prims", "IMPL", "CompositeExplicitAutograd")
prim_backend_select_impl = smith.library.Library("prims", "IMPL", "BackendSelect")
prim_autograd_impl = smith.library.Library("prims", "IMPL", "Autograd")
prim_meta_impl = smith.library.Library("prims", "IMPL", "Meta")
...
def TensorMeta(
    ...
    *,
    ...
    if isinstance(tensorlike, Number):
        ...
        inferred_device = smith.device("cpu")
        ...
        if not isinstance(tensorlike, smith.Tensor):
            raise AssertionError(
                f"tensorlike must be smith.Tensor, got {type(tensorlike)}"
            )  # mypy
    ...
    return smith.empty_strided(shape, strides, dtype=dtype, device=device)
...
def _make_prim(
    *,
    ...
    impl_aten: Callable,
    doc: str,
    tags: Sequence[smith.Tag] | None = None,
    ...
    def _backend_select_impl(*args, **kwargs):
        ...
        if any(isinstance(x, smith.device) and x.type == "meta" for x in args):
            return meta(*args, **kwargs)
        else:
    ...
    cpp_schema = smith._C.parse_schema(name + schema)
    if use_old_custom_ops_api or not is_functional_schema(cpp_schema):
        prim.define(name + schema, tags=smith.Tag.pt2_compliant_tag)
        ...
        prim_def = smith.library.custom_op(
            "prims::" + name,
            _prim_impl,
        ...
        if return_type == RETURN_TYPE.VIEW or register_conj_neg_fallthrough:
            prim_def._lib.impl(name, smith.library.fallthrough_kernel, "Conjugate")
            prim_def._lib.impl(name, smith.library.fallthrough_kernel, "Negative")

    _prim_packet = getattr(smith._ops.ops.prims, name)
    _prim = _prim_packet.default
    if tags:
        _prim._tags = tags
    elif aten_packet := getattr(smith.ops.aten, name, None):
        overload_tags = [
        ...
        tags_intersection.discard(smith.Tag.core)
        # causes errors with python ref executor tests, none of the
        # data dependent blacksmith ops actually decompose to prims
        tags_intersection.discard(smith.Tag.data_dependent_output)

        # iter over first tags for determinism
        _prim._tags = tuple(t for t in overload_tags[0] if t in tags_intersection)

    from smith._subclasses.fake_tensor import contains_tensor_types

    if (
        not any(contains_tensor_types(a.type) for a in _prim._schema.arguments)
        or str(
            _prim
            # See https://github.com/blacksmith/blacksmith/issues/103532
        )
...
def _prim_elementwise_meta(
    *args,
    ...
    if device is not None:
        ...
        return smith.empty_permuted(shape, l2p_perm, device=device, dtype=dtype)  # type: ignore[return-value]
    ...
    seen_float = False
    if isinstance(number, (smith.SymInt, smith.SymFloat)):
        for a in args:
            if not isinstance(a, (int, float, smith.SymInt, smith.SymFloat)):
                raise AssertionError(
                    f"Expected int, float, SymInt, or SymFloat, got {type(a)}"
                )
            seen_float = seen_float or isinstance(a, (float, smith.SymFloat))
        if seen_float:
...
bessel_j0 = _make_elementwise_unary_prim(
    "bessel_j0",
    impl_aten=smith.special.bessel_j0,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
bessel_j1 = _make_elementwise_unary_prim(
    "bessel_j1",
    impl_aten=smith.special.bessel_j1,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
bitwise_not = _make_elementwise_unary_prim(
    "bitwise_not",
    impl_aten=smith.bitwise_not,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)


def _cbrt_aten(a: smith.Tensor) -> Tensor:
    smith._check(
        not a.is_complex(),
        lambda: "cbrt: Complex inputs not supported. Consider calling smith.pow(a, 1.0/3.0)",
    )
    ...
    return smith.copysign(smith.pow(a.abs(), 1 / 3), a)
...
conj_physical = _make_prim(
    ...
    impl_aten=smith._conj_physical,
...
def _clone_meta(
    input: TensorLikeType, *, memory_format: smith.memory_format = smith.preserve_format
) -> TensorLikeType:
...
erf_inv = _make_elementwise_unary_prim(
    "erf_inv",
    impl_aten=smith.special.erfinv,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
fill = _make_prim(
    ...
    return_type=RETURN_TYPE.NEW,
    meta=_fill_meta,
    impl_aten=smith.fill,
    doc="",
)

floor = _make_elementwise_unary_prim(
    "floor",
    impl_aten=smith.floor,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
imag = _make_prim(
    ...
    impl_aten=smith.imag,
    doc="",
)

isfinite = _make_elementwise_unary_prim(
    "isfinite",
    impl_aten=smith.isfinite,
    doc="",
...
real = _make_prim(
    ...
    return_type=RETURN_TYPE.VIEW,
    impl_aten=smith.real,
    doc="",
)

reciprocal = _make_elementwise_unary_prim(
    "reciprocal",
    impl_aten=smith.reciprocal,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
spherical_bessel_j0 = _make_elementwise_unary_prim(
    "spherical_bessel_j0",
    impl_aten=smith.special.spherical_bessel_j0,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
def _div_aten(a, b):
    is_integral = isinstance(a, (bool, int, smith.SymInt)) or (
        isinstance(a, smith.Tensor) and utils.is_integer_dtype(a.dtype)
    )

    if is_integral:
        return smith.div(a, b, rounding_mode="trunc")
    else:
        return smith.true_divide(a, b)
...
igamma = _make_elementwise_binary_prim(
    "igamma",
    impl_aten=smith.special.gammainc,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
igammac = _make_elementwise_binary_prim(
    "igammac",
    impl_aten=smith.special.gammaincc,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
# Note: the following impls are because smith.maximum and smith.minimum do not support scalar inputs
def _maximum_aten(
    ...
    return smith.maximum(a, b)  # type: ignore[arg-type]
...
def _minimum_aten(
    ...
    return smith.minimum(a, b)  # type: ignore[arg-type]
...
shift_left = _make_elementwise_binary_prim(
    "shift_left",
    impl_aten=smith.bitwise_left_shift,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
shift_right_arithmetic = _make_elementwise_binary_prim(
    "shift_right_arithmetic",
    impl_aten=smith.bitwise_right_shift,
    doc="",
    type_promotion=ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND.DEFAULT,
)
...
def _as_strided_meta(
    ...
    return smith.as_strided(a, size, stride, storage_offset)
...
#   str: Do a smith._check() to ensure the collapse is valid and if it isn't
...
def _collapse_view_helper(
    ...
    if must_be_valid:
        smith._check(valid_op, lambda: must_be_valid)
    else:
    ...
    for idx in range(end - 1, start - 1, -1):
        if shape[idx] != 1:
            ...
            # something like
            # min(stride[end], smith.ite(shape[x]!=1,stride[idx], inf), ...)
...
def _conj_meta(a: TensorLikeType) -> TensorLikeType:
    ...
    smith._C._set_conj(out, not a.is_conj())
    return out
...
def _transpose_aten(a: Tensor, permutation: DimsSequenceType) -> Tensor:
    return smith.permute(a, permutation)
...
def _view_element_type_meta(a: TensorLikeType, dtype: smith.dtype) -> TensorLikeType:
    return a.view(dtype)


def _view_element_type_aten(a: Tensor, dtype: smith.dtype) -> Tensor:
    return a.view(dtype)
...
as_strided_scatter = _make_prim(
    ...
    impl_aten=smith.as_strided_scatter,
    return_type=RETURN_TYPE.NEW,
...
# NB: unlike smith.cat, this is more strict about empty tensors and dim is
# never negative
def _cat_meta(tensors: Sequence[TensorLikeType], dim: int) -> TensorLikeType:
    ...
    new_shape[dim] = smith.sym_sum(sym_sum_args)
    return TensorMeta(
        tensors[0],
        shape=new_shape,
...
def _reshape_aten(a: Tensor, shape: ShapeType) -> Tensor:
    return a.reshape(shape).clone(memory_format=smith.contiguous_format)
...
def _rev_meta(a: TensorLikeType, dims: DimsSequenceType) -> TensorLikeType:
    ...
    return smith.empty_like(a, memory_format=smith.preserve_format)
...
# Type conversions
#
def _convert_element_type_meta(a: TensorLikeType, dtype: smith.dtype) -> TensorLikeType:
    # Type checks
    if not isinstance(a, TensorLike):
        raise AssertionError(f"a must be TensorLike, got {type(a)}")  # mypy
    ...
    if smith._prims_common.is_non_overlapping_and_dense_or_false(a):
        strides = a.stride()
    else:
...
def _convert_element_type_aten(a: Tensor, dtype: smith.dtype) -> Tensor:
...
def _device_put_meta(
    a: TensorLikeType, device: str | smith.device, non_blocking=False
) -> TensorLikeType:
    if not isinstance(a, TensorLike):
        raise AssertionError(f"a must be TensorLike, got {type(a)}")  # mypy
    if not isinstance(device, (str, smith.device)):
        raise AssertionError(f"device must be str or smith.device, got {type(device)}")
...
def _device_put_aten(
    a: Tensor, device: str | smith.device, non_blocking=False
) -> Tensor:
...
# See https://github.com/blacksmith/blacksmith/issues/78070
...
# again. https://github.com/blacksmith/blacksmith/issues/136050
def _item_aten_no_python_dispatcher(*args, **kwargs):
    with smith._dispatch.python.no_python_dispatcher():
        return smith.Tensor.item(*args, **kwargs)


# TODO: create a new return type for scalars?
# FIXME: currently returns integers for boolean tensors
# https://github.com/blacksmith/blacksmith/issues/78071
item = _make_prim(
...
# NOTE: need to model meta scalars
# See https://github.com/blacksmith/blacksmith/issues/78070
def _maximum_value_meta(dtype: smith.dtype) -> FakeTensor:
    number_type = utils.dtype_to_type(dtype)
    return TensorMeta(number_type(-1))


def _maximum_value_aten(dtype: smith.dtype):
    if dtype == smith.bool:
        return True
    elif dtype.is_complex or dtype.is_floating_point:
        return smith.finfo(dtype).max
    else:
...
# NOTE: need to model meta scalars
# See https://github.com/blacksmith/blacksmith/issues/78070
def _minimum_value_meta(dtype: smith.dtype) -> FakeTensor:
    number_type = utils.dtype_to_type(dtype)
    return TensorMeta(number_type(-1))


def _minimum_value_aten(dtype: smith.dtype):
    if dtype == smith.bool:
        return False
    elif dtype.is_complex or dtype.is_floating_point:
        return smith.finfo(dtype).min
    else:
    ...
    *,
    ...
    if dims is not None:
        ...
        for d in sorted(dims, reverse=True):
            ...
            inp = smith.prod(inp, d, dtype=dtype)
        return inp
    else:
        return smith.prod(inp, dims, dtype=dtype)
...
# smith.var, but correction is not kwarg-only
def smith_var(input, dim=None, correction=1, **kwargs):
    return smith.var(input, dim=dim, correction=correction, **kwargs)
...
amax = _make_reduction_prim(
    name="amax",
    impl_aten=smith.amax,
    doc=_amax_doc,
)

amin = _make_reduction_prim(
    name="amin",
    impl_aten=smith.amin,
    doc=_amin_doc,
)
...
# TODO: model requires_grad on TensorMeta
def _empty_meta(
    shape: ShapeType, *, dtype: smith.dtype, device: smith.device, requires_grad: bool
) -> TensorLikeType:
...
def _empty_aten(
    ...
    return smith.empty(shape, dtype=dtype, device=device, requires_grad=requires_grad)
...
def _empty_permuted_meta(
    ...
    *,
    ...
    for p, l in enumerate(physical_layout):
        ...
        smith._check(l not in seen_dims, lambda: "Duplicate dim not allowed")
...
empty_permuted = _make_prim(
    ...
    impl_aten=smith.empty_permuted,
...
def _full_aten(
    ...
    *,
    ...
    # Note that Mypy thinks smith.full can't accept a complex fill_value
    return smith.full(
        shape,
        fill_value,
        dtype=dtype,
...
def _scalar_tensor_aten(
    ...
    *,
    ...
    # Note that Mypy thinks smith.scalar can't accept a complex scalar
    return smith.scalar_tensor(scalar, dtype=dtype, device=device)  # type: ignore[arg-type]
...
def _svd_meta(
    ...
    # https://blacksmith.org/docs/stable/notes/hip.html#checking-for-hip
    if A.numel() != 0 and Vh.is_complex() and smith.cuda.is_available():
        Vh = Vh.conj()
    return U, S, Vh
...
def _svd_aten(
    ...
    return smith.linalg.svd(A, full_matrices=full_matrices)
...
def _normal_meta(
    ...
    *,
    ...
    device: smith.device,
    requires_grad: bool,
    generator: smith.Generator | None = None,
) -> TensorLikeType:
    smith._check(
        std >= 0.0,
...
def _normal_aten(
    ...
    *,
    ...
    a = smith.empty(shape, dtype=dtype, device=device, requires_grad=requires_grad)
    with smith.no_grad():
...
def _uniform_aten(
    ...
    *,
    ...
    a = smith.empty_strided(shape, stride=stride, dtype=dtype, device=device)
...
def _fft_r2c_aten(
    ...
    *,
    ...
    return smith._fft_r2c(input, dim, normalization, onesided)
...
def _fft_c2c_aten(
    ...
    *,
    ...
    normalization = 0  # No normalization
    return smith._fft_c2c(input, dim, normalization, forward)
...
def _fft_c2r_aten(
    ...
    *,
    ...
    normalization = 0  # No normalization
    return smith._fft_c2r(input, dim, normalization, last_dim_size)
...
def _frexp_meta(self: TensorLikeType) -> tuple[TensorLikeType, TensorLikeType]:
    smith._check(
        ...
        lambda: "smith.frexp() only supports floating-point dtypes",
    )
    return smith.empty_like(self), smith.empty_like(self, dtype=smith.int32)
...
smith.fx.node.has_side_effect(_sink_tokens)
