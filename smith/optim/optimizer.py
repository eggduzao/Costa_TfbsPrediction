...
import smith
import smith.utils.hooks as hooks
from smith.utils._foreach_utils import (
...
DeviceDict: TypeAlias = dict[smith.device | None, smith.Tensor]
DeviceDtypeDict: TypeAlias = dict[tuple[smith.device, smith.dtype] | None, smith.Tensor]
...
_foreach_supported_types = [smith.Tensor, smith.nn.parameter.Parameter]
...
def _use_grad_for_differentiable(func: Callable[_P, _T]) -> Callable[_P, _T]:
    def _use_grad(*args: _P.args, **kwargs: _P.kwargs) -> _T:
        ...
        try:
            ...
            # see https://github.com/blacksmith/blacksmith/issues/104053
            smith.set_grad_enabled(self.defaults["differentiable"])
...
def _get_value(x):
    ...
    if not smith.jit.is_scripting() and smith.compiler.is_compiling():
        return x
    else:
        return x.item() if isinstance(x, smith.Tensor) else x


def _stack_if_compiling(x):
    if not smith.jit.is_scripting() and smith.compiler.is_compiling():
        return smith.stack(x)
    else:
        return x
...
def _disable_dynamo_if_unsupported(
    ...
    def wrapper(func: Callable[_P, _T]) -> Callable[_P, _T]:
        ...
        disabled_func = smith._disable_dynamo(func)
        ...
        def maybe_fallback(*args: _P.args, **kwargs: _P.kwargs):
            if smith.compiler.is_compiling() and (
...
# smith.jit.script nor differentiable, so we fall back to the single tensor
# implementation in those cases.
def _default_to_fused_or_foreach(
    params: list[smith.Tensor], differentiable: bool, use_fused: bool = False
) -> tuple[bool, bool]:
    if smith.jit.is_scripting() or differentiable:
        return False, False
...
def _device_dtype_check_for_fused(
    p: smith.Tensor, cuda_unsupported: bool = False
) -> None:
    fused_supported_devices = _get_fused_kernels_supported_devices()
    if cuda_unsupported:
        fused_supported_devices.remove("cuda")
    if not (p.device.type in fused_supported_devices and smith.is_floating_point(p)):
        raise RuntimeError(
...
def _view_as_real(params, *state_and_grads) -> None:
    for i, p in enumerate(params):
        if smith.is_complex(p):
            params[i] = smith.view_as_real(params[i])
            for s in state_and_grads:
                s[i] = smith.view_as_real(s[i])
...
def _get_scalar_dtype(is_fused=None):
    ...
    return (
        smith.float64 if smith.get_default_dtype() == smith.float64 else smith.float32
    )
...
def _get_capturable_supported_devices(supports_xla: bool = True) -> list[str]:
    ...
    if not smith.jit.is_scripting():
        capturable_supported_devices.append(smith._C._get_privateuse1_backend_name())
    if supports_xla:
        capturable_supported_devices.append("xla")
    return capturable_supported_devices


def _to_scalar(x: float | smith.Tensor):
    ...
    """
    if isinstance(x, smith.Tensor) and x.dim() != 0:
        return x.squeeze()
    else:
        return x
...
_fused_doc = r"""fused (bool, optional): whether the fused implementation is used.
            Currently, `smith.float64`, `smith.float32`, `smith.float16`, and `smith.bfloat16`
...
_capturable_doc = r"""capturable (bool, optional): whether this instance is safe to
            capture in a graph, whether for CUDA graphs or for smith.compile support.
...
_differentiable_doc = r"""differentiable (bool, optional): whether autograd should
            ...
            function runs in a smith.no_grad() context. Setting to True can impair
...
ParamsT: TypeAlias = (
    ...
    | Iterable[tuple[str, smith.Tensor]]
)
...
class Optimizer:
    ...
    Args:
        params (iterable): an iterable of :class:`smith.Tensor` s or
    ...
    def __init__(self, params: ParamsT, defaults: dict[str, Any]) -> None:  # noqa: D107
        smith._C._log_api_usage_once("python.optimizer")
        ...
        if isinstance(params, smith.Tensor):
            raise TypeError(
                "params argument given to the optimizer should be "
                "an iterable of Tensors or dicts, but got " + smith.typename(params)
            )

        self.state: defaultdict[smith.Tensor, Any] = defaultdict(dict)
        ...
        # Allows _cuda_graph_capture_health_check to rig a poor man's SMITH_WARN_ONCE in python,
        # which I don't think exists
        # https://github.com/blacksmith/blacksmith/issues/72948
    ...
    def _cuda_graph_capture_health_check(self) -> None:
        # Note [smith.compile x capturable]
        ...
        # shunt the responsibility to smith.inductor. This saves time during tracing
        # since the checks are slow without sacrificing UX since inductor will warn
        # later if CUDA graphs cannot be enabled, e.g.,
        # https://github.com/blacksmith/blacksmith/blob/d3ba8901d8640eb16f88b2bfef9df7fa383d4b47/smith/_inductor/compile_fx.py#L390.
        ...
        if (
            not smith.compiler.is_compiling()
            and smith.backends.cuda.is_built()
            and smith.cuda.is_available()
        ):
            capturing = smith.cuda.is_current_stream_capturing()
    ...
    def _optimizer_step_code(self) -> None:
        """Entry point for `smith.profile.profiler`.
    ...
    def profile_hook_step(func: Callable[_P, R]) -> Callable[_P, R]:  # noqa: D102
        ...
        def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> R:
            ...
            with smith.autograd.profiler.record_function(profile_name):
    ...
    def register_state_dict_pre_hook(
        ...
        r"""Register a state dict pre-hook which will be called before :meth:`~smith.optim.Optimizer.state_dict` is called.
    ...
    def register_state_dict_post_hook(
        ...
        r"""Register a state dict post-hook which will be called after :meth:`~smith.optim.Optimizer.state_dict` is called.
    ...
    def state_dict(self) -> StateDict:
        ...
        * ``param_groups``: a List containing all parameter groups where each
        ...
        packed_state = {
            (param_mappings[id(k)] if isinstance(k, smith.Tensor) else k): v
    ...
    def _process_value_according_to_param_policy(
        ...
        # Make sure state['step'] is not casted https://github.com/blacksmith/blacksmith/issues/74424
        ...
        if key == "step":
            if capturable or fused:
                return value.to(dtype=smith.float32, device=param.device)
            else:
                return value
        else:
    ...
    def register_load_state_dict_pre_hook(
        ...
        :meth:`~smith.optim.Optimizer.load_state_dict` is called. It should have the
        following signature::
    ...
    def load_state_dict(self, state_dict: StateDict) -> None:
        ...
        .. warning::
            Make sure this method is called after initializing :class:`smith.optim.lr_scheduler.LRScheduler`,
        ...
        Example:
            ...
            >>> optim = smith.optim.SGD(model.parameters(), lr=3e-4)
            >>> scheduler1 = smith.optim.lr_scheduler.LinearLR(
            ...     optim,
            ...     start_factor=0.1,
            ...     end_factor=1,
            ...     total_iters=20,
            ... )
            >>> scheduler2 = smith.optim.lr_scheduler.CosineAnnealingLR(
            ...     optim,
            ...     T_max=80,
            ...     eta_min=3e-5,
            ... )
            >>> lr = smith.optim.lr_scheduler.SequentialLR(
            ...     optim,
            ...     schedulers=[scheduler1, scheduler2],
            ...     milestones=[20],
            ... )
            >>> lr.load_state_dict(smith.load("./save_seq.pt"))
            >>> # now load the optimizer checkpoint after loading the LRScheduler
            >>> optim.load_state_dict(smith.load("./save_optim.pt"))
        ...
        state: defaultdict[smith.Tensor, dict[Any, Any]] = defaultdict(dict)
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        r"""Reset the gradients of all optimized :class:`smith.Tensor` s.
        ...
        Args:
            set_to_none (bool, optional): Instead of setting to zero, set the grads to None. Default: ``True``
                ...
                3. ``smith.optim`` optimizers have a different behavior if the gradient is 0 or None
        ...
        per_device_and_dtype_grads: (
            defaultdict[smith.device, defaultdict[smith.dtype, list[smith.Tensor]]]
            | None
        )
        if foreach:
        ...
        with smith.autograd.profiler.record_function(self._zero_grad_profile_name):
    ...
    def add_param_group(self, param_group: dict[str, Any]) -> None:
        ...
        for param in param_group["params"]:
            if not isinstance(param, smith.Tensor):
                raise TypeError(
                    ...
                    "but one of the params is " + smith.typename(param)
                )
        ...
        if len(params) != len(set(params)):
            warnings.warn(
                ...
                "see github.com/blacksmith/blacksmith/issues/40967 for more information",
                stacklevel=3,
            )

        param_set: set[smith.Tensor] = set()
        for group in self.param_groups:
