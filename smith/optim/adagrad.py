...
class Adagrad(Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                state["sum"] = smith.full_like(
                    p, init_value, memory_format=smith.preserve_format
                )
    ...
    def __setstate__(self, state):
        ...
        step_is_tensor = (len(state_values) != 0) and smith.is_tensor(
            state_values[0]["step"]
        )
...
def adagrad(
    ...
    # setting these as kwargs for now as functional API is compiled by smith/distributed/optim
    ...
    *,
    ...
    See :class:`~smith.optim.Adagrad` for details.
    """
...
def _make_sparse(grad, grad_indices, values):
    size = grad.size()
    return smith.sparse_coo_tensor(grad_indices, values, size)
    ...
    *,
    ...
    for param, grad, state_sum, step_t in zip(
        ...
        if grad.is_sparse:
            ...
            is_complex = smith.is_complex(param)
            if is_complex:
                grad = smith.view_as_real(grad)
                state_sum = smith.view_as_real(state_sum)
            ...
            if is_complex:
                param = smith.view_as_complex(param)
                state_sum = smith.view_as_complex(state_sum)
...
def _multi_tensor_adagrad(
    ...
    *,
    ...
    for (
        ...
        smith._foreach_addcmul_(device_state_sums, device_grads, device_grads, value=1)

        std = smith._foreach_sqrt(device_state_sums)
        smith._foreach_add_(std, eps)

        if weight_decay != 0 or maximize:
            # Again, reuse the intermediate memory (device_grads) already allocated
            smith._foreach_mul_(device_grads, minus_clr)
            numerator = device_grads
        else:
            numerator = smith._foreach_mul(device_grads, minus_clr)  # type: ignore[assignment]

        smith._foreach_addcdiv_(device_params, numerator, std)
