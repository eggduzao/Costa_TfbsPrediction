...
from smith.optim.lr_scheduler import _format_param, LRScheduler
...
from smith.utils._foreach_utils import _group_tensors_by_device_and_dtype
...
def get_ema_multi_avg_fn(decay=0.999):
    ...
    def ema_update(
        ...
        if smith.is_floating_point(ema_param_list[0]) or smith.is_complex(
            ema_param_list[0]
        ):
            smith._foreach_lerp_(ema_param_list, current_param_list, 1 - decay)
        else:
...
def get_swa_multi_avg_fn():
    ...
    def swa_update(
        ...
        # foreach lerp only handles float and complex
        if smith.is_floating_point(averaged_param_list[0]) or smith.is_complex(
            ...
            diffs = smith._foreach_sub(current_param_list, averaged_param_list)
...
class AveragedModel(Module):
    ...
    Args:
        model (smith.nn.Module): model to use with SWA/EMA
        device (smith.device, optional): if provided, the averaged model will be
    ...
    Example:
        ...
        >>> scheduler = smith.optim.lr_scheduler.CosineAnnealingLR(optimizer,
        >>>                                     T_max=300)
        >>> swa_start = 160
    ...
    Example:
        ...
        >>>             smith.optim.swa_utils.get_ema_multi_avg_fn(0.9), use_buffers=True)

    .. note::
        When using SWA/EMA with models containing Batch Normalization you may
        need to update the activation statistics for Batch Normalization.
        This can be done either by using the :meth:`smith.optim.swa_utils.update_bn`
    ...
    def __init__(
        ...
        self.register_buffer(
            "n_averaged", smith.tensor(0, dtype=smith.long, device=device)
        )
        self.avg_fn = avg_fn
...
def update_bn(
    ...
    Args:
        loader (smith.utils.data.DataLoader): dataset loader to compute the
            activation statistics on. Each data batch should be either a
            tensor, or a list/tuple whose first element is a tensor
            containing data.
        model (smith.nn.Module): model for which we seek to update BatchNorm
            statistics.
        device (smith.device, optional): If set, data will be transferred to
            :attr:`device` before being passed into :attr:`model`.

    Example:
        >>> # xdoctest: +SKIP("Undefined variables")
        >>> loader, model = ...
        >>> smith.optim.swa_utils.update_bn(loader, model)
...
class SWALR(LRScheduler):
    ...
    Averaging (SWA) method (see `smith.optim.swa_utils.AveragedModel`).

    Args:
        optimizer (smith.optim.Optimizer): wrapped optimizer
    ...
    Example:
        ...
        >>> scheduler = smith.optim.lr_scheduler.MultiplicativeLR(optimizer,
        >>>        lr_lambda=lr_lambda)
