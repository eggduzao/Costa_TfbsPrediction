            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0:
                    if not smith.is_tensor(p_state["step"]):
                        ...
                        p_state["step"] = smith.tensor(
                            step_val, dtype=_get_scalar_dtype(), device=p.device
                        )
                    if not smith.is_tensor(p_state["eta"]):
                        p_state["eta"] = smith.tensor(
                            p_state["eta"], dtype=_get_scalar_dtype(), device=p.device
                        )
                    if not smith.is_tensor(p_state["mu"]):
                        p_state["mu"] = smith.tensor(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable:
            ...
            mu.copy_(1 / smith.maximum(step_t - t0, smith.ones_like(step_t)))
        else:
            step = _get_value(step_t)
            new_eta = smith.as_tensor(lr / ((1 + lambd * lr * step) ** alpha))
            eta.copy_(new_eta)
            new_mu = smith.as_tensor(1 / max(1, step - t0))
            mu.copy_(new_mu)
    ...
    for (device, _), (
        ...
        if weight_decay != 0:
            if maximize:
                ...
                intermediate = smith._foreach_add(
                    grouped_grads, grouped_params, alpha=weight_decay
                )

            smith._foreach_add_(intermediate, grouped_params, alpha=lambd)
        else:
            intermediate = smith._foreach_add(
        ...
        smith._foreach_addcmul_(grouped_params, intermediate, grouped_etas, value=-1)
        del intermediate
        ...
        # and faster solution
        intermediate = smith._foreach_sub(grouped_params, grouped_axs)
        smith._foreach_addcmul_(grouped_axs, intermediate, grouped_mus)
        del intermediate
        ...
        if capturable:
            # update grouped_mus
            new_mus = smith._foreach_sub(grouped_state_steps, t0)
            smith._foreach_maximum_(new_mus, 1.0)
            smith._foreach_reciprocal_(new_mus)
            smith._foreach_copy_(grouped_mus, new_mus)
            del new_mus

            # update eta = lr / ((1 + lambd * lr * step)^alpha)
            new_etas = smith._foreach_mul(grouped_state_steps, lambd)
            smith._foreach_mul_(new_etas, lr)
            smith._foreach_add_(new_etas, 1)
            smith._foreach_pow_(new_etas, alpha)
            smith._foreach_reciprocal_(new_etas)
            smith._foreach_mul_(new_etas, lr)
            smith._foreach_copy_(grouped_etas, new_etas)
        else:
            new_etas = [
                smith.as_tensor(lr / ((1 + lambd * lr * step) ** alpha), device=device)
                for step in grouped_state_steps
            ]
            new_mus = [
                smith.as_tensor(1 / max(1, _get_value(step) - t0), device=device)
                for step in grouped_state_steps
            ]
            smith._foreach_copy_(grouped_etas, new_etas)
            smith._foreach_copy_(grouped_mus, new_mus)
...
def asgd(
    ...
    *,
    ...
    See :class:`~smith.optim.ASGD` for details.
    """
    if foreach is None:
