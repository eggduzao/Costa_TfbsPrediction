        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0 and not smith.is_tensor(p_state["step"]):
                    ...
                    p_state["step"] = smith.tensor(step_val, dtype=_get_scalar_dtype())
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                state["step"] = smith.tensor(0.0, dtype=_get_scalar_dtype())
                ...
                if p.grad.dim() > 1:
                    ...
                    state["variance"] = smith.zeros_like(
                        p.grad, memory_format=smith.preserve_format
                    )
...
def _single_tensor_adafactor(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if eps1 is None:
            eps1 = smith.finfo(param.dtype).eps
        ...
        if grad.dim() > 1:
            ...
            row_mean = (
                smith.norm(grad, dim=-1, keepdim=True).square_().div_(grad.size(-1))
            )
            row_var.lerp_(row_mean, one_minus_beta2_t)
            # same as (g * g).mean(dim=-2) w/o materializing an intermediate size g
            col_mean = (
                smith.norm(grad, dim=-2, keepdim=True).square_().div_(grad.size(-2))
            )
...
def _group_tensors_by_device_dtype_and_is_multidim(
    ...
    tuple[smith.device | None, smith.dtype | None, bool],
    ...
    *,
    ...
    for (_, dtype, is_multidim), (
        ...
        if not smith.compiler.is_compiling() and device_state_steps[0].is_cpu:
            ...
            smith._foreach_add_(device_state_steps, 1.0)
        ...
        if is_multidim:
            ...
            # same as (g * g).mean(dim=-1) w/o materializing an intermediate size g
            row_means = [
                smith.norm(grad, dim=-1, keepdim=True) for grad in device_grads
            ]
            smith._foreach_mul_(row_means, row_means)
            smith._foreach_div_(row_means, [grad.size(-1) for grad in device_grads])
            smith._foreach_lerp_(device_row_vars, row_means, one_minus_beta2_ts)
            del row_means

            # same as (g * g).mean(dim=-2) w/o materializing an intermediate size g
            col_means = [
                smith.norm(grad, dim=-2, keepdim=True) for grad in device_grads
            ]
            smith._foreach_mul_(col_means, col_means)
            smith._foreach_div_(col_means, [grad.size(-2) for grad in device_grads])
            smith._foreach_lerp_(device_col_vars, col_means, one_minus_beta2_ts)
            del col_means
            ...
            smith._foreach_clamp_min_(row_var_means, eps1)
            smith._foreach_div_(var_estimates, row_var_means)
            del row_var_means
        else:
            ...
            grads_squared = smith._foreach_mul(device_grads, device_grads)
            smith._foreach_lerp_(device_variances, grads_squared, one_minus_beta2_ts)
            del grads_squared
        ...
        # square the eps1 as we sqrt after to keep eps1's magnitude
        smith._foreach_clamp_min_(var_estimates, eps1 * eps1)
        smith._foreach_rsqrt_(var_estimates)
        smith._foreach_mul_(var_estimates, device_grads)
        ...
        smith._foreach_mul_(updates, alphas)
        smith._foreach_add_(device_params, updates)
...
def adafactor(
    ...
    *,
    ...
    See :class:`~smith.optim.Adafactor` for details.
    """
