...
def sgd(
    ...
    *,
    ...
    See :class:`~smith.optim.SGD` for details.
    """
    ...
    if foreach is None and fused is None:
        # why must we be explicit about an if statement for smith.jit.is_scripting here?
    ...
    elif fused and not smith.jit.is_scripting():
        func = _fused_sgd
    else:
    ...
    for (
        ...
        if momentum != 0:
            ...
            if all_states_with_momentum_buffer:
                smith._foreach_mul_(bufs, momentum)
                smith._foreach_add_(bufs, device_grads, alpha=1 - dampening)
            else:
                bufs = []
            ...
            if nesterov:
                smith._foreach_add_(device_grads, bufs, alpha=momentum)
            else:
                device_grads = bufs

        if not device_has_sparse_grad:
            # handle internal item() call if lr is a tensor
            if isinstance(lr, smith.Tensor) and smith.compiler.is_compiling():
                grads_x_lr = smith._foreach_mul(device_grads, -lr)
                smith._foreach_add_(device_params, grads_x_lr)
            else:
                smith._foreach_add_(device_params, device_grads, alpha=-lr)
        else:
    ...
    *,
    ...
    if is_first_step:
        for i, g in enumerate(grads):
            momentum_buffer_list[i] = smith.empty_like(g)
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, momentum_buffer_list],  # type: ignore[list-item]
        with_indices=False,
    )
