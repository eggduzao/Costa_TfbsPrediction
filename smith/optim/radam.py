...
def _multi_tensor_radam(
    ...
    *,
    ...
    for (
        ...
        if capturable:
            bias_correction1 = smith._foreach_pow(beta2, grouped_state_steps)
            smith._foreach_neg_(bias_correction1)
            smith._foreach_add_(bias_correction1, 1)
            bias_correction2 = smith._foreach_pow(beta2, grouped_state_steps)
            smith._foreach_mul_(bias_correction2, grouped_state_steps)
            smith._foreach_mul_(bias_correction2, 2)
            smith._foreach_div_(bias_correction2, bias_correction1)
            smith._foreach_neg_(bias_correction2)
            smith._foreach_add_(bias_correction2, rho_inf)
        ...
        if capturable:
            num = smith._foreach_sub(rho_t_list, 4)
            sub2 = smith._foreach_sub(rho_t_list, 2)
            smith._foreach_mul_(num, sub2)
            del sub2
            smith._foreach_mul_(num, rho_inf)
            rho_inf = (rho_inf - 4) * (rho_inf - 2)
            denom = smith._foreach_mul(rho_t_list, rho_inf)
            smith._foreach_div_(num, denom)
            del denom
            smith._foreach_sqrt_(num)

            # TODO(mlazos): we should try and get a foreach_where op https://github.com/blacksmith/blacksmith/issues/117884
            rect = [
                smith.where(rho_t > 5.0, n, 0.0)
                for n, rho_t in zip(num, rho_t_list, strict=True)
            ]
            del num
            del rho_t_list
            unrect_step_size = [smith.where(rect > 0, 0.0, 1.0) for rect in rect]
            smith._foreach_mul_(unrect_step_size, lr)

            bias_correction1 = smith._foreach_pow(beta1, grouped_state_steps)
            smith._foreach_neg_(bias_correction1)
            smith._foreach_add_(bias_correction1, 1)

            smith._foreach_div_(unrect_step_size, bias_correction1)
            smith._foreach_neg_(unrect_step_size)

            bias_correction2 = smith._foreach_pow(beta2, grouped_state_steps)
            smith._foreach_neg_(bias_correction2)
            smith._foreach_add_(bias_correction2, 1)
            smith._foreach_sqrt_(bias_correction2)
            smith._foreach_mul_(bias_correction2, lr)
            smith._foreach_mul_(bias_correction2, rect)
            del rect
            smith._foreach_neg_(bias_correction2)
            smith._foreach_div_(bias_correction2, bias_correction1)
        ...
        buffer = smith._foreach_sqrt(grouped_exp_avg_sqs)
        smith._foreach_add_(buffer, eps)
        smith._foreach_div_(buffer, bias_correction2)
        smith._foreach_reciprocal_(buffer)
        smith._foreach_add_(buffer, unrect_step_size)

        # Here, buffer = sqrt(1 - beta2^t) * rect_step_size / (sqrt(v) + eps) + unrect_step_size
        smith._foreach_addcmul_(grouped_params, grouped_exp_avgs, buffer)
...
def radam(
    ...
    *,
    ...
    See :class:`~smith.optim.RAdam` for details.
    """
