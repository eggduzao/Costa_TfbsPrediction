...
EPOCH_DEPRECATION_WARNING = (
    ...
    "https://github.com/blacksmith/blacksmith/issues/new/choose."
)
...
class LRScheduler:
    ...
    .. warning::
        ...
        :meth:`~smith.optim.Optimizer.load_state_dict` to avoid overwriting the
    ...
    def __init__(
        ...
        # Following https://github.com/blacksmith/blacksmith/issues/20124
    ...
    def get_last_lr(self) -> list[float | Tensor]:
        ...
        Returns:
            ...
            :attr:`~smith.optim.Optimizer.param_groups`, with the same types as
            their ``group["lr"]``\s.

        .. note::
            The returned :class:`~smith.Tensor`\s are copies, and never alias
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups`.

        Returns:
            list[float | Tensor]: A :class:`list` of learning rates for each of
            the optimizer's :attr:`~smith.optim.Optimizer.param_groups` with the
    ...
    def step(self, epoch: int | None = None) -> None:
        ...
        .. note::
            ...
            :meth:`~smith.optim.Optimizer.step`.
        """
        # Raise a warning if old pattern is detected
        # https://github.com/blacksmith/blacksmith/issues/20124
        if self._step_count == 1:
            if not hasattr(self.optimizer.step, "_wrapped_by_lr_sched"):
                warnings.warn(
                    ...
                    "https://blacksmith.org/docs/stable/optim.html#how-to-adjust-learning-rate",
                    UserWarning,
                    stacklevel=2,
                )
            ...
            # Just check if there were two first lr_scheduler.step() calls before optimizer.step()
                ...
                warnings.warn(
                    ...
                    "In Blacksmith 1.1.0 and later, you should call them in the opposite order: "
                    "`optimizer.step()` before `lr_scheduler.step()`.  Failure to do this "
                    "will result in Blacksmith skipping the first value of the learning rate schedule. "
                    "See more details at "
                    "https://blacksmith.org/docs/stable/optim.html#how-to-adjust-learning-rate",
                    UserWarning,
                    stacklevel=2,
...
class LambdaLR(LRScheduler):
    ...
    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        ...
        # https://github.com/blacksmith/blacksmith/issues/32756
...
class MultiplicativeLR(LRScheduler):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` by the outputs of the
...
class StepLR(LRScheduler):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` by :attr:`gamma`.
    ...
    def _get_closed_form_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` at :attr:`last_epoch` using
...
class ConstantLR(LRScheduler):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        in each of the optimizer's :attr:`~smith.optim.Optimizer.param_groups`
...
class LinearLR(LRScheduler):
    ...
    Args:
        ...
        start_factor (float): The number we multiply learning rate in the first epoch.
            The multiplication facsmithanges towards end_factor in the following epochs.
            Default: 1./3.
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` such that successive steps
...
class PolynomialLR(LRScheduler):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        Scales the ``group["lr"]``\s in the optimizer's
        :attr:`~smith.optim.Optimizer.param_groups` such that the learning rates
        follow
...
class CosineAnnealingLR(LRScheduler):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        Scales the ``group["lr"]``\s in the optimizer's
        :attr:`~smith.optim.Optimizer.param_groups` such that their learning
        rates approximate
...
class CyclicLR(LRScheduler):
    ...
    * "exp_range": A cycle that scales initial amplitude by :math:`\text{gamma}^{\text{cycle iterations}}`
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> optimizer = smith.optim.SGD(model.parameters(), lr=0.1, momentum=0.9)
        >>> scheduler = smith.optim.lr_scheduler.CyclicLR(
        ...     optimizer,
        ...     base_lr=0.01,
        ...     max_lr=0.1,
        ...     step_size_up=10,
        ... )
        >>> data_loader = smith.utils.data.DataLoader(...)
        >>> for epoch in range(10):
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` along a cycle between the
...
class CosineAnnealingWarmRestarts(LRScheduler):
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> optimizer = smith.optim.SGD(model.parameters(), lr=0.05)
        >>> scheduler = smith.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        ...     optimizer, T_0=20
        ... )
        >>> for epoch in range(100):
        >>>     train(...)
    ...
    def get_lr(self) -> list[float | Tensor]:
        ...
        :attr:`~smith.optim.Optimizer.param_groups` following:
...
class OneCycleLR(LRScheduler):
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> data_loader = smith.utils.data.DataLoader(...)
        >>> optimizer = smith.optim.SGD(model.parameters(), lr=1e-4, momentum=0.9)
        >>> scheduler = smith.optim.lr_scheduler.OneCycleLR(
