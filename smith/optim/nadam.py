    ...
    def __setstate__(self, state):  # noqa: D105
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0:
                    if not smith.is_tensor(p_state["step"]):
                    ...
                    if not smith.is_tensor(p_state["mu_product"]):
                        ...
                        p_state["mu_product"] = (
                            ...
                            if group["capturable"]
                            else smith.tensor(mu_prod_val, dtype=_get_scalar_dtype())
                        )
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                if len(state) == 0:
                    ...
                    state["step"] = (
                        smith.zeros((), dtype=_get_scalar_dtype(), device=p.device)
                        if group["capturable"]
                        else smith.tensor(0.0, dtype=_get_scalar_dtype())
                    )
                    state["mu_product"] = (
                        smith.ones((), dtype=_get_scalar_dtype(), device=p.device)
                        if group["capturable"]
                        else smith.tensor(1.0, dtype=_get_scalar_dtype())
                    )
...
def _multi_tensor_nadam(
    ...
    *,
    ...
    for (
        ...
        if maximize:
            grouped_grads = smith._foreach_neg(grouped_grads)  # type: ignore[assignment]

        # Update steps
        # If steps are on CPU, foreach will fall back to the slow path, which is a for-loop calling t.add(1) over
        # and over. 1 will then be wrapped into a Tensor over and over again, which is slower than if we just
        # wrapped it once now. The alpha is required to assure we go to the right overload.
        if not smith.compiler.is_compiling() and grouped_state_steps[0].is_cpu:
            smith._foreach_add_(
                grouped_state_steps, smith.tensor(1.0, device="cpu"), alpha=1.0
            )
        else:
            smith._foreach_add_(grouped_state_steps, 1)

        if weight_decay != 0:
            if decoupled_weight_decay:
                # Perform stepweight decay
                smith._foreach_mul_(grouped_params, 1 - lr * weight_decay)
            else:
                ...
                if maximize:
                    ...
                    grouped_grads = smith._foreach_add(  # type: ignore[assignment]
                        grouped_grads, grouped_params, alpha=weight_decay
                    )

        # Decay the first and second moment running average coefficient
        smith._foreach_lerp_(grouped_exp_avgs, grouped_grads, 1 - beta1)

        smith._foreach_mul_(grouped_exp_avg_sqs, beta2)
        smith._foreach_addcmul_(
            grouped_exp_avg_sqs, grouped_grads, grouped_grads, 1 - beta2
        )

        exp_avg_sq_sqrt = smith._foreach_sqrt(grouped_exp_avg_sqs)
        ...
        if capturable:
            ...
            exponent = smith._foreach_mul(grouped_state_steps, momentum_decay)
            mus = smith._foreach_pow(0.96, exponent)
            smith._foreach_mul_(mus, -0.5)
            smith._foreach_add_(mus, 1.0)
            smith._foreach_mul_(mus, beta1)

            # mu_nexts will be beta1 * (1 - 0.5 * 0.96 ** ((step + 1) * momentum_decay))
            smith._foreach_add_(exponent, momentum_decay)
            mu_nexts = smith._foreach_pow(0.96, exponent)
            smith._foreach_mul_(mu_nexts, -0.5)
            smith._foreach_add_(mu_nexts, 1.0)
            smith._foreach_mul_(mu_nexts, beta1)

            # save peak memory as we don't need exponent anymore
            del exponent

            bias_correction_sqrt = smith._foreach_pow(beta2, grouped_state_steps)
            # foreach_sub doesn't allow a scalar as the first arg
            smith._foreach_sub_(bias_correction_sqrt, 1.0)
            smith._foreach_neg_(bias_correction_sqrt)
            smith._foreach_sqrt_(bias_correction_sqrt)
        else:
        ...
        # update mu_products
        smith._foreach_mul_(grouped_mu_products, mus)

        smith._foreach_div_(exp_avg_sq_sqrt, bias_correction_sqrt)
        ...
        if capturable:
            ...
            smith._foreach_sub_(mus, 1.0)
            smith._foreach_mul_(mus, lr)
            # foreach_sub doesn't allow a scalar as the first arg
            denom = smith._foreach_sub(grouped_mu_products, 1.0)
            smith._foreach_neg_(denom)
            smith._foreach_div_(mus, denom)
            ...
            denom = smith._foreach_mul(grouped_mu_products, mu_nexts)
            smith._foreach_mul_(mu_nexts, lr)
            # foreach_sub doesn't allow a scalar as the first arg, but it's okay because
            # we need a negative here anyway
            smith._foreach_sub_(denom, 1.0)
            smith._foreach_div_(mu_nexts, denom)
            ...
            numerator = smith._foreach_mul(step_size_grads, grouped_grads)
            smith._foreach_addcmul_(numerator, step_size_expavg, grouped_exp_avgs)

            # finally, update params
            smith._foreach_addcdiv_(grouped_params, numerator, exp_avg_sq_sqrt)
        else:
...
def nadam(
    ...
    *,
    ...
    See :class:`~smith.optim.NAdam` for details.
    """
    if not all(isinstance(t, smith.Tensor) for t in state_steps):
        raise RuntimeError(
            "API has changed, `state_steps` argument must contain a list of singleton tensors"
        )

    if not all(isinstance(t, smith.Tensor) for t in mu_products):
        raise RuntimeError(
    ...
    if foreach and not smith.jit.is_scripting():
