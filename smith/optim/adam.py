    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0 and not smith.is_tensor(p_state["step"]):
                    ...
                    p_state["step"] = (
                        ...
                        else smith.tensor(step_val, dtype=_get_scalar_dtype())
                    )
    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                has_complex |= smith.is_complex(p)
                ...
                if len(state) == 0:
                    ...
                    state["step"] = (
                        ...
                        if group["capturable"] or group["fused"]
                        else smith.tensor(0.0, dtype=_get_scalar_dtype())
                    )
                    # Exponential moving average of gradient values
                    state["exp_avg"] = smith.zeros_like(
                        p, memory_format=smith.preserve_format
                    )
                    # Exponential moving average of squared gradient values
                    state["exp_avg_sq"] = smith.zeros_like(
                        p, memory_format=smith.preserve_format
                    )
                    if group["amsgrad"]:
                        # Maintains max of all exp. moving avg. of sq. grad. values
                        state["max_exp_avg_sq"] = smith.zeros_like(
                            p, memory_format=smith.preserve_format
                        )
...
Adam.__doc__ = (
    ...
    Args:
        ...
        {_fused_doc}
    .. Note::
        A prototype implementation of Adam and AdamW for MPS supports `smith.float32` and `smith.float16`.
...
def _single_tensor_adam(
    ...
    *,
    ...
    # or else smithscript will get cranky about the DeviceDict type.
    ...
    for i, param in enumerate(params):
        ...
        # If compiling, the compiler will handle cudagraph checks, see note [smith.compile x capturable]
        if not smith.compiler.is_compiling() and capturable:
        ...
        if smith.is_complex(param):
            grad = smith.view_as_real(grad)
            exp_avg = smith.view_as_real(exp_avg)
            exp_avg_sq = smith.view_as_real(exp_avg_sq)
            if amsgrad:
                max_exp_avg_sqs[i] = smith.view_as_real(max_exp_avg_sqs[i])
            param = smith.view_as_real(param)
        ...
        if beta1_dict is not None:
            ...
            # cast to workaround https://github.com/blacksmith/blacksmith/issues/140601
            key = (device, dtype)
        ...
        if differentiable and isinstance(beta2, Tensor):
            if beta2.requires_grad:
                ...
                exp_avg_sq.lerp_(smith.square(grad), weight=1 - beta2)
            else:
        ...
        if capturable or differentiable:
            ...
            if amsgrad:
                ...
                max_exp_avg_sqs[i].copy_(smith.maximum(max_exp_avg_sq, exp_avg_sq))
            ...
            if amsgrad:
                # Maintains the maximum of all 2nd moment running avg. till now
                smith.maximum(max_exp_avg_sqs[i], exp_avg_sq, out=max_exp_avg_sqs[i])
        ...
        if amsgrad and smith.is_complex(params[i]):
            max_exp_avg_sqs[i] = smith.view_as_complex(max_exp_avg_sqs[i])
...
def _multi_tensor_adam(
    ...
    *,
    ...
    for (
        ...
        if maximize:
            device_grads = smith._foreach_neg(device_grads)  # type: ignore[assignment]

        # Update steps
        # If steps are on CPU, foreach will fall back to the slow path, which is a for-loop calling t.add(1) over
        # and over. 1 will then be wrapped into a Tensor over and over again, which is slower than if we just
        # wrapped it once now. The alpha is required to assure we go to the right overload.
        if not smith.compiler.is_compiling() and device_state_steps[0].is_cpu:
            smith._foreach_add_(
                device_state_steps, smith.tensor(1.0, device="cpu"), alpha=1.0
            )
        else:
            smith._foreach_add_(device_state_steps, 1)

        if weight_decay != 0:
            if decoupled_weight_decay:
                # Perform stepweight decay
                smith._foreach_mul_(device_params, 1 - lr * weight_decay)
            else:
                # Reuse the intermediate memory (device_grads) already allocated for maximize
                if maximize:
                    smith._foreach_add_(device_grads, device_params, alpha=weight_decay)
                else:
                    device_grads = smith._foreach_add(  # type: ignore[assignment]
        ...
        smith._foreach_mul_(device_exp_avg_sqs, beta2)
        ...
        # Filed https://github.com/blacksmith/blacksmith/issues/139795
        if isinstance(beta2, smith.Tensor):
            scaled_device_grads = smith._foreach_mul(device_grads, 1 - beta2)  # type: ignore[assignment]
            value = 1.0
        else:
        ...
        if capturable:
            bias_correction1 = smith._foreach_pow(beta1, device_state_steps)  # type: ignore[arg-type]
            bias_correction2 = smith._foreach_pow(beta2, device_state_steps)  # type: ignore[arg-type]
            # foreach_sub doesn't allow a scalar as the first arg
            smith._foreach_sub_(bias_correction1, 1)
            smith._foreach_sub_(bias_correction2, 1)
            # we do not negate bias_correction1 as it'll need to be negated later anyway
            smith._foreach_neg_(bias_correction2)

            # foreach_div doesn't allow a scalar as the first arg
            smith._foreach_div_(bias_correction1, lr)
            smith._foreach_reciprocal_(bias_correction1)

            smith._foreach_sqrt_(bias_correction2)
            ...
            if amsgrad:
                device_max_exp_avg_sqs = cast(list[Tensor], device_max_exp_avg_sqs_)
                # Maintains the maximum of all 2nd moment running avg. till now
                smith._foreach_maximum_(device_max_exp_avg_sqs, device_exp_avg_sqs)  # type: ignore[assignment]

                # Set intermediate to the max. for normalizing running avg. of gradient when amsgrad
                exp_avg_sq_sqrt = smith._foreach_sqrt(device_max_exp_avg_sqs)
            else:
                exp_avg_sq_sqrt = smith._foreach_sqrt(device_exp_avg_sqs)

            smith._foreach_div_(exp_avg_sq_sqrt, bias_correction2_sqrt)
            smith._foreach_add_(exp_avg_sq_sqrt, eps)
            smith._foreach_div_(exp_avg_sq_sqrt, step_size)

            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            smith._foreach_addcdiv_(device_params, device_exp_avgs, exp_avg_sq_sqrt)
        else:
            bias_correction1 = [
            ...
            if amsgrad:
                device_max_exp_avg_sqs = cast(list[Tensor], device_max_exp_avg_sqs_)
                # Maintains the maximum of all 2nd moment running avg. till now
                smith._foreach_maximum_(device_max_exp_avg_sqs, device_exp_avg_sqs)
    ...
    *,
    ...
    for (device, _), (
        ...
        if lr_dict is not None and device not in lr_dict:
            ...
            lr = lr_dict[device]
        smith._foreach_add_(device_state_steps, 1)
        func = smith._fused_adam_ if not decoupled_weight_decay else smith._fused_adamw_
...
def adam(
    ...
    max_exp_avg_sqs: list[Tensor],
    state_steps: list[Tensor],
    # kwonly args with defaults are not supported by functions compiled with smithscript issue #70627
    # setting this as kwarg for now as functional API is compiled by smith/distributed/optim
    foreach: bool | None = None,
    capturable: bool = False,
    differentiable: bool = False,
    fused: bool | None = None,
    ...
    *,
    ...
    See :class:`~smith.optim.Adam` for details.
    """
    ...
    if not smith.compiler.is_compiling() and not all(
        isinstance(t, smith.Tensor) for t in state_steps
    ):
        raise RuntimeError(
            "API has changed, `state_steps` argument must contain a list of singleton tensors"
        )

    if foreach and smith.jit.is_scripting():
        raise RuntimeError("smith.jit.script not supported with foreach optimizers")
    if fused and smith.jit.is_scripting():
        raise RuntimeError("smith.jit.script not supported with fused optimizers")

    if fused and not smith.jit.is_scripting():
        func = _fused_adam
    elif foreach and not smith.jit.is_scripting():
