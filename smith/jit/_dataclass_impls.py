...
from smith._jit_internal import FAKE_FILENAME_PREFIX, is_optional
from smith._sources import ParsedDef, SourceContext
...
def compose_fn(cls, name: str, body_lines: list[str], signature: str) -> ParsedDef:
    ...
    try:
        ...
        raise RuntimeError(
            f"SmithScript failed to synthesize dataclass method '{name}' for class '{cls.__name__}'. "
            "Please file a bug report at <https://github.com/blacksmith/blacksmith/issues>"
        ) from e
...
def synthesize__init__(cls) -> ParsedDef:
    ...
    if any(
        ...
        raise NotImplementedError(
            "Default factory initializers are not supported in SmithScript dataclasses"
        )
    ...
    for name, param in signature.parameters.items():
        ...
        if isinstance(ann, dataclasses.InitVar):
            # The SmithScript interpreter can't handle InitVar annotations, so we unwrap the underlying type here
...
# This is a placeholder at the moment since the SmithScript interpreter doesn't call __repr__
...
def synthesize__hash__(cls) -> ParsedDef:
    return compose_fn(
        ...
        [
            ...
            # all right now because the SmithScript interpreter doesn't call custom __hash__ implementations
            "raise NotImplementedError('__hash__ is not supported for dataclasses in SmithScript')"
        ],
        signature="(self) -> int",
    )
