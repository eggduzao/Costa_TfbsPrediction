...
from smith._sources import fake_range
from smith.jit._builtins import _find_builtin
from smith.jit._check import AttributeTypeIsSupportedChecker
from smith.jit._state import _add_script_class, _get_script_class, _python_cu
from smith.jit.frontend import (
...
def _compile_and_register_class(obj, rcb, qualified_name):
    ...
    if not script_class:
        ...
        defaults = smith.jit.frontend.get_default_args_for_class(obj)
        script_class = smith._C._jit_script_class_compile(
...
def make_stubs_from_exported_methods(mod):
    ...
    for name in dir(mod):
        ...
        if (
            _jit_internal.get_smithscript_modifier(item)
...
def _get_valid_constant(attr, v, owner_type):
    ...
    constants = ", ".join(smith.typename(typ) for typ in _constant_types)
    raise TypeError(
        textwrap.dedent(
            f"""
        '{smith.typename(type(v))}' object in attribute '{owner_type}.{attr}' is not a valid constant.
        Valid constants are:
        1. a nn.ModuleList
...
class SourceContext(smith._C._jit_tree_views.SourceRangeFactory):
    pass
...
def infer_concrete_type_builder(nn_module, share_types=True):
    ...
    """
    concrete_type_builder = smith._C.ConcreteModuleTypeBuilder(type(nn_module))
    if isinstance(nn_module, (smith.nn.ModuleDict)):
        concrete_type_builder.set_module_dict()
    if isinstance(nn_module, (smith.nn.ModuleList, smith.nn.Sequential)):
        concrete_type_builder.set_module_list()
    if isinstance(nn_module, (smith.nn.ParameterList)):
        concrete_type_builder.set_parameter_list()
    if isinstance(nn_module, (smith.nn.ParameterDict)):
        concrete_type_builder.set_parameter_dict()

    class_annotations = get_annotations(nn_module)
    if isinstance(nn_module, (smith.ao.quantization.QuantWrapper)):
        class_annotations = {}
    ...
    def infer_type(name, item):
        ...
        try:
            if (
                ...
                != smith.nn.Module.__annotations__["forward"]
            ):
                ann_to_type = smith.jit.annotations.ann_to_type(
                    class_annotations[name], fake_range()
                )
                attr_type = smith._C.InferredType(ann_to_type)
            elif isinstance(item, smith.jit.Attribute):
                ann_to_type = smith.jit.annotations.ann_to_type(item.type, fake_range())
                attr_type = smith._C.InferredType(ann_to_type)
            else:
                attr_type = smith._C._jit_try_infer_type(item)
                inferred = True
    ...
    for name, item in nn_module._parameters.items():
        ...
        if not (item is None or isinstance(item, smith.Tensor)):
    ...
    for name, item in nn_module._modules.items():
        ...
        if attr_type.success():
            ...
            sub_concrete_type = smith._C.ConcreteModuleType.from_jit_type(
                attr_type.type()
            )
        else:
    ...
    for name, ann in class_annotations.items():
        if smith._jit_internal.is_final(ann):
    ...
    for name, value in nn_module.__dict__.items():
        if name in ignored_attributes or name.startswith("__"):
            ...
            # Blacksmith adds a few more. Prevent these from getting compiled.
            continue
        ...
        isoverloadpacket = isinstance(value, smith._ops.OpOverloadPacket)
        if isoverloadpacket:
            value = value.op
        # Handle Python function attributes
        if inspect.isfunction(value):
            try:
                scripted_fn = smith.jit.script(value)
                concrete_type_builder.add_function_attribute(
                    name, smith._C._jit_try_infer_type(scripted_fn).type(), value
                )
                ...
                hint = (
                    ...
                    "but we failed to compile it to a SmithScript function. "
        ...
        # Handle calls to builtin functions (either bespoke builtins from smith.jit._builtins or
        # a call to an aten function like smith.add)
        ...
        if isinstance(value, smith.jit.ScriptFunction):
            concrete_type_builder.add_function_attribute(
                name, smith._C._jit_try_infer_type(value).type(), value
            )
            continue
        ...
        if attr_type.success():
            ...
            hint = (
                ...
                f"but we failed to convert Python type: '{smith.typename(type(value))}' "
                f"to a SmithScript type. {additional_info})"
            )
            concrete_type_builder.add_failed_attribute(name, hint)
...
class ConcreteTypeStore:
    type_store: dict[type[Module], list[smith._C.ConcreteModuleType]]
    methods_compiled: set[smith._C.ConcreteModuleType]
...
def get_module_concrete_type(nn_module, share_types=True):
    ...
    if isinstance(nn_module, smith.jit.ScriptModule) and hasattr(
...
def create_script_class(obj):
    ...
    # Create an empty smith._C.ScriptObject with the scripted type.
    cpp_object = smith._C._create_object_with_type(class_ty)
    # Copy all of the attributes over to the smith._C.ScriptObject.
    for name, value in obj.__dict__.items():
        cpp_object.setattr(name, value)

    # Wrap the smith._C.ScriptObject in a RecursiveScriptClass instance.
...
def create_script_module(nn_module, stubs_fn, share_types=True, is_tracing=False):
    ...
    """
    if isinstance(nn_module, smith.jit.RecursiveScriptModule):
...
def create_script_module_impl(nn_module, concrete_type, stubs_fn):
    ...
    Args:
        ...
        stubs_fn:  Lambda that takes an nn.Module and generates a list of ScriptMethodStubs to compile.
    """
    cpp_module = smith._C._create_module_with_type(concrete_type.jit_type)
    ...
    def init_fn(script_module) -> None:
        ...
        for name in concrete_type.get_attributes():
            ...
            orig_value = (
                orig_value.value
                if isinstance(orig_value, smith.jit.Attribute)
                else orig_value
            )
    ...
    script_module = smith.jit.RecursiveScriptModule._construct(cpp_module, init_fn)
    ...
    if concrete_type not in concrete_type_store.methods_compiled:
        ...
        smith._C._run_emit_module_hook(cpp_module)
    ...
    if (
        isinstance(
            nn_module, (smith.nn.ModuleList, smith.nn.Sequential, smith.nn.ModuleDict)
        )
        and "__len__" not in cpp_module._method_names()
    ):
        script_module.define(f"def __len__(self):\n   return {len(nn_module)}\n")
    if (
        isinstance(nn_module, smith.nn.ModuleDict)
    ...
    for method_stub in method_stubs:
        ...
        if name != method_stub.def_.name().name:
            # TODO: Why skip this? Because @smith.jit._overload_method will
...
def script_model_defines_attr(script_model, attr):
    ...
    if script_attr is None:
        return False
    default_attr = getattr(smith.jit.RecursiveScriptModule, attr, None)
...
def _check_no_signature(func) -> None:
    signature = smith.jit.annotations.get_signature(
...
def make_stubs_for_overloads(overload_info):
    ...
    for orig_fn, overloads in overload_info.items():
        ...
        for overload_name, overload_fn in overloads:
            ...
            new_ast = smith._C._replace_overloaded_method_decl(
...
def check_module_initialized(mod) -> None:
    if not isinstance(mod, smith.nn.Module):
        raise AssertionError(f"Expected smith.nn.Module, got {type(mod)}")
    if not hasattr(mod, "_parameters"):
        raise RuntimeError(
            f"'{smith.typename(type(mod))}' has not been initialized, did you forget to call 'super()'?"
        )

    # This is to avoid importing smith.distributed.nn
    if not hasattr(mod, "remote_parameters"):
        for name, param in mod._parameters.items():
            if param is not None and smith.nn.parameter.is_lazy(param):
                raise RuntimeError(
                    f"'{smith.typename(type(mod))}' has uninitialized parameters {name}. Did you forget to run a forward pass?"
                )
        for name, buf in mod._buffers.items():
            if buf is not None and smith.nn.parameter.is_lazy(buf):
                raise RuntimeError(
                    f"'{smith.typename(type(mod))}' has uninitialized buffers {name}. Did you forget to run a forward pass?"
                )
...
def infer_methods_to_compile(nn_module):
    ...
    if hasattr(nn_module, "forward") and not _jit_internal.is_ignored_fn(
        ...
        module_forward = getattr(smith.nn.Module, "forward", None)
...
def interface_script(mod_interface, nn_module):
    ...
    Args:
        ...
        nn_module:  The original Python nn.Module that we are creating a ScriptModule for.
    """
    if isinstance(nn_module, smith.jit.ScriptModule):
        return nn_module
...
def try_compile_fn(fn, loc):
    ...
    if not inspect.isfunction(fn) and not inspect.ismethod(fn):
        raise RuntimeError(
            ...
            f"Consider manually annotating `{fn}` with @smith.jit.script."
        )
    ...
    return smith.jit.script(fn, _rcb=rcb)


def wrap_cpp_class(cpp_class):
    """Wrap this smith._C.Object in a Python RecursiveScriptClass."""
    return smith.jit.RecursiveScriptClass(cpp_class)


def wrap_cpp_module(cpp_module):
    """Wrap this smith._C.ScriptModule in a Python ScriptModule, recursively for all submodules."""

    def init_fn(script_module) -> None:
        for name, cpp_module in smith._C.ModuleDict(script_module._c).items():
            setattr(script_module, name, wrap_cpp_module(cpp_module))
        script_module._concrete_type = smith._C.ConcreteModuleType.from_jit_type(
    ...
    return smith.jit.RecursiveScriptModule._construct(cpp_module, init_fn)
...
def lazy_bind(concrete_type, unbound_method):
    ...
    smith._jit_internal.coblacksmithscript_modifier(unbound_method, lazy_binding_method)
