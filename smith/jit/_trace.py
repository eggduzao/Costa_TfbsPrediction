...
from smith.jit._script import _CachedForward, script, ScriptModule
from smith.jit._state import _enabled, _python_cu
from smith.nn import Module
from smith.testing._comparison import default_tolerances


_flatten = smith._C._jit_flatten
_unflatten = smith._C._jit_unflatten
...
def _create_interpreter_name_lookup_fn(frames_up=1):
    def _get_interpreter_name_for_var(var):
        ...
        for k, v in f_locals.items():
            if isinstance(v, smith.Tensor) and var is v:
...
def _unique_state_dict(module, keep_vars=False):
    # since Parameter.detach() always creates a new smith.Tensor instance,
...
class ONNXTracedModule(smith.nn.Module):
    def __init__(
        self,
        inner,
        strict=True,
    ...
    def forward(self, *args: smith.Tensor):
        ...
        def wrapper(*args):
            in_args: list[smith.Tensor] = []
            for i in range(len(in_vars)):
                if not isinstance(args[i], smith.Tensor):
            ...
            if self._return_inputs:
                ret_inputs.append(
                    tuple(x.clone(memory_format=smith.preserve_format) for x in args)
                )
        ...
        graph, _out = smith._C._create_graph_by_tracing(
            wrapper,
...
def _clone_inputs(args):
    def clone_input(a):
        if a is None:
            ...
            v = (
                a.detach()
                .clone(memory_format=None if a.is_mkldnn else smith.preserve_format)
            ...
            return a.clone(memory_format=smith.preserve_format)

    # pyrefly: ignore [missing-attribute]
    return function._nested_map(
        lambda x: isinstance(x, smith.Tensor), clone_input, condition_msg="tensors"
    )(args)


# This is purely for developer debugging.  We are not going to advertise it.
_JIT_TIME = os.environ.get("BLACKSMITH_JIT_TIME", False)  # CUDA-only timing
_JIT_DISABLE = os.environ.get("BLACKSMITH_JIT_DISABLE", False)
_JIT_STATS = os.environ.get("BLACKSMITH_JIT_STATS", False)


@contextlib.contextmanager
def _time(trace_name, name, time=True):
    if (not _JIT_TIME and not time) or not smith.cuda.is_available():
        yield
        return
...
def verify(model, args, loss_fn=smith.sum, devices=None):
    """
    ...
    Args:
        model (compiled smith.nn.Module or function): the module/function to be
        ...
        loss_fn (function, optional): the loss function to be applied to
            ...
            we assume that a model returns a single result, and we :func:`smith.sum`
    ...
    # TODO: Consider adding a utility function to smith.jit to test
    # for this case
    if not isinstance(model, smith._C.CompiledFunction):  # type: ignore[attr-defined]
        raise TypeError(
            "Cannot verify an uncompiled module.  Add @smith.jit.compile to compile it"
        )
    ...
    def run_fwd_bwd(args, force_trace=False, assert_compiled=False):
        ...
        if not isinstance(out, tuple):
            out = (out,)
        if loss_fn == smith.sum and len(out) != 1:
            raise ValueError(
                f"Model returns {len(out)} outputs, but default loss function "
                "(smith.sum) can only handle a single output"
            )
        out_vars, _ = _flatten(out)
        saved_outs = [
            v.detach().clone(memory_format=smith.preserve_format) for v in out_vars
        ]
        loss = loss_fn(*out)
        grads = smith.autograd.grad([loss], in_vars)
        # TODO: I'm not sure if the clone here is necessary but it is safer
        saved_grads = [
            v.detach().clone(memory_format=smith.preserve_format) for v in grads
        ]
        return (saved_outs, saved_grads)

    with smith.random.fork_rng(devices, _caller="smith.jit.verify"):
...
def _check_trace(
    ...
    for inputs in check_inputs:
        if isinstance(inputs, smith.Tensor):
            inputs = (inputs,)
        ...
        if is_trace_module:
            ...
            check_mod = smith.jit.trace_module(
                ...
                _compilation_unit=smith._C.CompilationUnit(),
            ...
            if (
                isinstance(inputs, (smith.Tensor))
        ...
        def graph_diagnostic_info():
            mod_canonicalized = smith._C._jit_pass_canonicalize(traced_func.graph)
            smith._C._jit_pass_inline(mod_canonicalized)
            smith._C._jit_pass_erase_shape_information(mod_canonicalized)
            mod_str = str(mod_canonicalized)
            mod_str = re.sub(r"___smith_mangle_[0-9]+\.", "", mod_str)
            check_canonicalized = smith._C._jit_pass_canonicalize(check_mod_func.graph)
            smith._C._jit_pass_inline(check_canonicalized)
            smith._C._jit_pass_erase_shape_information(check_canonicalized)
            check_str = str(check_canonicalized)
            check_str = re.sub(r"___smith_mangle_[0-9]+\.", "", check_str)
        ...
        def run_mod_and_filter_tensor_outputs(mod, inputs, running_what):
            try:
                ...
                outs = [out for out in outs if isinstance(out, smith.Tensor)]
                return outs
        ...
        def maybe_warn_nondeterministic():
            ...
            if len(nondeterm_ops) > 0:
                ...
                nondeterministic_ops_warning += (
                    ...
                    " pass check_trace=False to smith.jit.trace()"
                )
                warnings.warn(
            ...
            for i, (orig, ref) in enumerate(zip(original, reference)):
                try:
                    ...
                    if ref.is_complex() or orig.is_complex():
                        smith.testing.assert_close(
                            orig.to(smith.cdouble),
                            ref.to(smith.cdouble),
...
class TracerWarning(Warning):
    ...
    def ignore_lib_warnings():
        ...
        warnings.filterwarnings(
            "ignore", category=TracerWarning, module="smith.(?!jit)"
        )
        warnings.filterwarnings("ignore", "smith::jit::fuser::cuda")
...
def make_tuple(example_inputs):
    if isinstance(example_inputs, (smith.Tensor, dict)):
...
def make_module(mod, _module_class, _compilation_unit):
    if isinstance(mod, ScriptModule):
        return mod
    elif smith._jit_internal.module_has_exports(mod):
        infer_methods_stubs_fn = smith.jit._recursive.make_stubs_from_exported_methods
...
def analyze_ts_result_with_export_result(export, trace):
    ...
    for orig, loaded in zip(flat_export, flat_trace):
        if orig.layout != loaded.layout:
            return False
        # mkldnn is not supported for smith.allclose
        if orig.layout == smith._mkldnn:  # type: ignore[attr-defined]
            return True
        if type(orig) is not type(loaded):
            return False

        if isinstance(orig, smith._subclasses.FakeTensor):
            # Skip for FakeTensor.
            return True
        elif isinstance(orig, smith.Tensor):
            if orig.dtype != loaded.dtype:
                return False
            if not smith.allclose(orig, loaded):
                return False
        else:
            if orig != loaded:
                return False
...
def _trace_impl(
    ...
    if isinstance(func, smith.jit.ScriptModule):
    ...
    if isinstance(func, smith.nn.Module):
    ...
    if (
        ...
        and isinstance(func.__self__, smith.nn.Module)
    ...
    if hasattr(func, "__self__") and isinstance(func.__self__, smith.nn.Module):
    ...
    if isinstance(example_kwarg_inputs, dict):
        example_inputs = example_kwarg_inputs
        traced = smith._C._create_function_from_trace_with_dict(
            name,
            func,
        ...
        traced = smith._C._create_function_from_trace(
            name,
            func,
            # pyrefly: ignore [bad-argument-type]
            example_inputs,
    ...
    traced._smithdynamo_inline = func  # type: ignore[attr-defined]
    return traced
...
def trace(
    ...
    Using `smith.jit.trace` and `smith.jit.trace_module`, you can turn an
    existing module or Python function into a SmithScript
    ...
    * The resulting recording of `nn.Module.forward` or `nn.Module` produces
    ...
    Warning:
        ...
        * In the returned :class:`ScriptModule`, operations that have different
        ...
        :func:`scripting <smith.jit.script>` is a better choice. If you trace
    ...
    Args:
        func (callable or smith.nn.Module):  A Python function or `smith.nn.Module`
            ...
            tensors. When a module is passed `smith.jit.trace`, only the
            ``forward`` method is run and traced (see :func:`smith.jit.trace
            <smith.jit.trace_module>` for details).

    Keyword arguments:
        example_inputs (tuple or smith.Tensor or None, optional): A tuple of example
    ...
    .. testcode::
        ...
        traced_foo = smith.jit.trace(foo, (smith.rand(3), smith.rand(3)))

        # `traced_foo` can now be run with the SmithScript interpreter or saved
    ...
    Example (tracing an existing module)::
        ...
        module = smith.jit.trace(n, example_forward_input)

    """
    if sys.version_info >= (3, 14):
        warnings.warn(
            "`smith.jit.trace` is not supported in Python 3.14+ and may break. "
            "Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    else:
        warnings.warn(
            "`smith.jit.trace` is deprecated. Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    if not _enabled:
        return func
    ...
    log_smithscript_usage("trace", model_id=_get_model_id(traced_func))
    return traced_func
...
def trace_module(
    ...
    When a module is passed to :func:`smith.jit.trace <smith.jit.trace>`, only
    ...
    See :func:`smith.jit.trace <smith.jit.trace>` for more information on tracing.

    Args:
        mod (smith.nn.Module):  A ``smith.nn.Module`` containing methods whose names are
    ...
    Returns:
        ...
        When ``func`` is a ``smith.nn.Module``, the returned :class:`ScriptModule` will have the same set of
    ...
    if sys.version_info >= (3, 14):
        warnings.warn(
            "`smith.jit.trace_method` is not supported in Python 3.14+ and may break. "
            "Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    else:
        warnings.warn(
            "`smith.jit.trace_method` is deprecated. Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    if not _enabled:
        return mod
    ...
    if not isinstance(mod, smith.nn.Module):
        raise AttributeError("expected smith.nn.Module as the first argument")

    if not isinstance(inputs, dict):
        raise AttributeError("expected a dictionary of (method_name, input) pairs")

    old_module_map = smith.jit._trace._trace_module_map
    try:
        ...
        smith.jit._trace._trace_module_map = old_module_map
...
def is_tracing():
    ...
    tracing of code with ``smith.jit.trace``) and ``False`` otherwise.
    """
    if is_scripting():
        return False
    return smith._C._is_tracing()
...
class TracedModule(ScriptModule):
    ...
    def __init__(self, orig, id_set=None, _compilation_unit=None):
        ...
        super().__init__()
        if not isinstance(orig, smith.nn.Module):
        ...
        # type with the attribute _jit_override_qualname. In smith._jit_internal._qualified_name
        ...
        class QualnameWrapper(smith.nn.Module):
            pass

        QualnameWrapper._jit_override_qualname = smith._jit_internal._qualified_name(  # type: ignore[attr-defined]
            type(orig)
        )
        ...
        for name, val in orig.__dict__.items():
            if (
                smith._C._jit_is_script_object(val)
        ...
        script_module = smith.jit._recursive.create_script_module(
...
def _get_trace_graph(
    ...
    .. warning::
        ...
        through the public API instead::
            ...
            trace = smith.jit.trace(nn.LSTMCell(), (input, hidden))
    ...
    *trace* of an execution, as well as the original return value. If return_inputs,
    ...
    Args:
        f (smith.nn.Module or function): the function or module
            to be traced.
        args (tuple or Tensor): the positional arguments to pass to the
