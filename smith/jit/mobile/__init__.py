...
import smith
from smith.jit._serialization import validate_map_location


def _load_for_lite_interpreter(f, map_location=None):
    r"""
    Load a :class:`LiteScriptModule` saved with :func:`smith.jit._save_for_lite_interpreter`.

    Args:
        f: a file-like object (has to implement read, readline, tell, and seek),
            or a string containing a file name
        map_location: a string or smith.device used to dynamically remap
    ...
    .. testcode::
        ...
        smith.jit._load_for_lite_interpreter('lite_script_module.pt')
        ...
        smith.jit.mobile._load_for_lite_interpreter(buffer)
    """
    ...
    if isinstance(f, (str, os.PathLike)):
        cpp_module = smith._C._load_for_lite_interpreter(os.fspath(f), map_location)
    else:
        cpp_module = smith._C._load_for_lite_interpreter_from_buffer(
            f.read(),
            map_location,
        )
...
def _export_operator_list(module: LiteScriptModule):
    ...
    return smith._C._export_operator_list(module._c)
...
def _get_model_bytecode_version(f_input) -> int:
    ...
    .. testcode::
        ...
        from smith.jit.mobile import _get_model_bytecode_version
    ...
    if isinstance(f_input, (str, os.PathLike)):
        return smith._C._get_model_bytecode_version(os.fspath(f_input))
    else:
        return smith._C._get_model_bytecode_version_from_buffer(f_input.read())
...
def _get_mobile_model_contained_types(f_input) -> int:
    ...
    .. testcode::
        ...
        from smith.jit.mobile import _get_mobile_model_contained_types
    ...
    if isinstance(f_input, (str, os.PathLike)):
        return smith._C._get_mobile_model_contained_types(os.fspath(f_input))
    else:
        return smith._C._get_mobile_model_contained_types_from_buffer(f_input.read())
...
def _backport_for_mobile(f_input, f_output, to_version):
    ...
    if (isinstance(f_input, (str, os.PathLike))) and (
        ...
        return smith._C._backport_for_mobile(
            os.fspath(f_input),
            os.fspath(f_output),
            to_version,
        )
    else:
        return smith._C._backport_for_mobile_from_buffer(
            f_input.read(),
            str(f_output),
            to_version,
        )
...
def _backport_for_mobile_to_buffer(f_input, to_version):
    ...
    if isinstance(f_input, (str, os.PathLike)):
        return smith._C._backport_for_mobile_to_buffer(os.fspath(f_input), to_version)
    else:
        return smith._C._backport_for_mobile_from_buffer_to_buffer(
            f_input.read(),
            to_version,
        )
...
def _get_model_ops_and_info(f_input):
    ...
    .. testcode::
        ...
        from smith.jit.mobile import _get_model_ops_and_info
    ...
    if isinstance(f_input, (str, os.PathLike)):
        return smith._C._get_model_ops_and_info(os.fspath(f_input))
    else:
        return smith._C._get_model_ops_and_info(f_input.read())
