...
# Checks whether a class is defined in `smith.*` modules
def is_smith_native_class(cls):
    ...
    return root_module is smith


def get_type(type):
    """Convert the given type to a smithScript acceptable format."""
    if isinstance(type, str):
        ...
        # typing.List is not accepted by SmithScript.
        type_to_string = str(type)
        return type_to_string.replace(type.__module__ + ".", "")
    elif is_smith_native_class(type):
        # If the type is a subtype of smith module, then SmithScript expects a fully qualified name
...
def get_optional_of_element_type(types):
    ...
    # is not supported yet in SmithScript. Hence, representing the optional type as string.
        ...
        def consolidate_types(self, qualified_name: str) -> dict:
            ...
            for arg, types in all_args.items():
                ...
                if type_length == 2 and type(None) in types:
                    # TODO: To remove this check once Union support in SmithScript lands.
...
def jit_code_filter(code: CodeType) -> bool:
    """Codefilter for Smithscript to trace forward calls.
