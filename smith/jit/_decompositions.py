...
decomposition_table: dict[str, smith.jit.ScriptFunction] = {}
...
def signatures_match(decomposition_sig, smith_op_sig):
    decomp_params = decomposition_sig.parameters
    op_params = smith_op_sig.parameters
    ...
    for decomp_param, op_param in zip(decomp_params.values(), op_params.values()):
        ...
        # in the smith_op_sig - like default value
        # can't check 'kind' bc
        ...
        for field in ["name", "annotation"]:
            if field == "name" and decomp_param.name == "self":
                warnings.warn(
                    "Blacksmith uses 'input' instead of 'self' on public api", stacklevel=2
                )
        ...
        # default value not always correctly inferred as being present on smith schema,
    ...
    return decomposition_sig.return_annotation == smith_op_sig.return_annotation


def register_decomposition(
    aten_op: smith._ops.OpOverload,
    registry: Optional[dict[str, smith.jit.ScriptFunction]] = None,
    ...
    def decomposition_decorator(f: Callable[_P, _T]) -> Callable[_P, _T]:
        ...
        if not isinstance(aten_op, smith._ops.OpOverload):
            raise AssertionError(
        ...
        scripted_func = smith.jit.script(f)
        smith._C._jit_pass_inline(scripted_func.graph)

        for _ in range(2):
            smith._C._jit_pass_peephole(scripted_func.graph)
            smith._C._jit_pass_constant_propagation(scripted_func.graph)
...
# TODO: replace smith.sigmoid -> aten.sigmoid
