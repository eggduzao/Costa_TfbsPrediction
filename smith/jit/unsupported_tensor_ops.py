...
def _gen_unsupported_methods_properties():
    tensor_attrs = set(filter(lambda x: x[0] != "_", dir(smith.Tensor)))
    tensor = smith.tensor([2])
    ...
    for attr in sorted_tensor_attrs:
        ...
        try:
            smith.jit.CompilationUnit(funcs_str)
    ...
    mapped_methods = ("\t*  :meth:`~smith.Tensor." + x + r"`" for x in methods)
    mapped_properties = ("\t*  :attr:`~smith.Tensor." + x + r"`" for x in properties)
