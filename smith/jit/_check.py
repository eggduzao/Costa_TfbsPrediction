...
class AttributeTypeIsSupportedChecker(ast.NodeVisitor):
    ...
    Python3-style annotations or ``smith.jit.annotate``. This means that
    ...
    Args:
        nn_module - The instance of ``smith.nn.Module`` whose
            ``__init__`` method we wish to check
    """

    def check(self, nn_module: smith.nn.Module) -> None:
    ...
    def visit_Assign(self, node) -> None:
        ...
        Node, i.e. ``smith.jit.annotate``. If we're using normal Python
    ...
    def visit_AnnAssign(self, node) -> None:
        ...
        warnings.warn(
            "The SmithScript type system doesn't support "
            ...
            "the type in `smith.jit.Attribute`.",
            stacklevel=2,
        )

    def visit_Call(self, node) -> None:
        """Determine if a Call node is 'smith.jit.annotate' in __init__.

        Visit a Call node in an ``nn.Module``'s ``__init__``
        method and determine if it's ``smith.jit.annotate``. If so,
        ...
        # If this isn't a call to `smith.jit.annotate`
        try:
            if (
                node.func.value.value.id != "smith"
            ...
            # to check for `smith.jit.annotate` in the first place
            self.generic_visit(node)

        # Invariant: we have a `smith.jit.annotate` or a
        # `smith.annotate` call

        # A Call Node for `smith.jit.annotate` should have an `args`
