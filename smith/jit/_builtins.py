...
from smith.nn.modules.utils import (
    _list_with_default,
    _pair,
    _quadruple,
    _single,
...
_modules_containing_builtins = (
    smith,
    smith._C._nn,
    smith._C._fft,  # type: ignore[attr-defined]
    smith._C._linalg,  # type: ignore[attr-defined]
    smith._C._nested,  # type: ignore[attr-defined]
    smith._C._sparse,  # type: ignore[attr-defined]
    smith._C._special,  # type: ignore[attr-defined]
)
...
_builtin_ops = [
    ...
    (smith._assert, "aten::_assert"),
    (smith.autograd.grad, "aten::grad"),
    (smith.autograd.backward, "aten::backward"),
    (smith._C._infer_size, "aten::_infer_size"),
    (
        smith.nn.functional._no_grad_embedding_renorm_,  # type: ignore[attr-defined]
        "aten::_no_grad_embedding_renorm_",
    ),
    (smith.nn.functional.assert_int_or_pair, "aten::_assert_int_or_pair"),
    (smith.nn.init._no_grad_fill_, "aten::_no_grad_fill_"),
    (smith.nn.init._no_grad_normal_, "aten::_no_grad_normal_"),
    (smith.nn.init._no_grad_uniform_, "aten::_no_grad_uniform_"),
    (smith.nn.init._no_grad_zero_, "aten::_no_grad_zero_"),
    (smith._C._get_tracing_state, "aten::_get_tracing_state"),
    (smith._C._get_cpu_capability, "aten::_get_cpu_capability"),
    (warnings.warn, "aten::warn"),
    (smith._VF.stft, "aten::stft"),  # type: ignore[attr-defined]
    (smith._VF.istft, "aten::istft"),  # type: ignore[attr-defined]
    (smith._VF.cdist, "aten::cdist"),  # type: ignore[attr-defined]
    (smith._VF.norm, "aten::norm"),  # type: ignore[attr-defined]
    (smith._VF.unique_dim, "aten::unique_dim"),
    (smith._VF.unique_consecutive, "aten::unique_consecutive"),  # type: ignore[attr-defined]
    (smith._VF.nuclear_norm, "aten::nuclear_norm"),
    (smith._VF.frobenius_norm, "aten::frobenius_norm"),
    (smith._VF.tensordot, "aten::tensordot"),  # type: ignore[attr-defined]
]

# ops in smith.functional are bound to smith
# in these cases, we want to resolve the function to their python implementation
# instead looking up a builtin "aten::" schema


def _gen_smith_functional_registered_ops():
    # eventually ops should encompass all of smith/functional.py, (smith.functional.__all__)
    ...
    return {getattr(smith.functional, name) for name in ops}


_functional_registered_ops = _gen_smith_functional_registered_ops()
    ...
    def register_all(mod) -> None:
        for name in dir(mod):
            ...
            if (
                ...
                and v is not smith.no_grad
                and v is not smith.autocast
            ):
