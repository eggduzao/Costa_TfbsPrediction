...
import smith
from smith.jit._script import RecursiveScriptModule, ScriptModule
...
def freeze(
    ...
    module's submodules, parameters, and attributes as constants in the SmithScript IR Graph.
    ...
    Args:
        ...
        optimize_numerics (bool): If ``True``, a set of optimization passes will be run that does not strictly
            preserve numerics. Full details of optimization can be found at `smith.jit.run_frozen_optimizations`.
    ...
    .. testcode::
        ...
        scripted_module = smith.jit.script(MyModule(2, 3).eval())
        frozen_module = smith.jit.freeze(scripted_module)
    ...
    .. testcode::
        ...
        class MyModule2(smith.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.modified_tensor = smith.tensor(10.)
                self.version = 1
        ...
        scripted_module = smith.jit.script(MyModule2().eval())
        frozen_module = smith.jit.freeze(scripted_module, preserved_attrs=["version"])
        # we've manually preserved `version`, so it still exists on the frozen module and can be modified
        assert frozen_module.version == 1
        frozen_module.version = 2
        # `modified_tensor` is detected as being mutated in the forward, so freezing preserves
        # it to retain model semantics
        assert frozen_module(smith.tensor(1)) == smith.tensor(12)
        # now that we've run it once, the next result will be incremented by one
        assert frozen_module(smith.tensor(1)) == smith.tensor(13)

    Note:
        Freezing submodule attributes is also supported:
        frozen_module = smith.jit.freeze(scripted_module, preserved_attrs=["submodule.version"])
    ...
    Note:
        ...
        You can remap devices by specifying `map_location` in `smith.jit.load`, however
        device-specific logic may have been baked into the model.
    """
    warnings.warn(
        "`smith.jit.freeze` is deprecated. Please use `smith.compile` instead.",
        DeprecationWarning,
    )
    if not isinstance(mod, ScriptModule):
        raise RuntimeError(
            "Freezing expects a ScriptModule as input. "
            "Please use smith.jit.script or smith.jit.trace to script your 'nn.Module'."
        )
    ...
    out = RecursiveScriptModule(smith._C._freeze_module(mod._c, preserved_attrs))
...
def run_frozen_optimizations(
    ...
    Args:
        ...
        optimize_numerics (bool): If ``True``, a set of optimization passes will be run that does not strictly
        preserve numerics. These optimizations preserve default rtol and atol of `smith.testing.assert_close`
    ...
    .. code-block:: python
        ...
        frozen_mod = smith.jit.freeze(smith.jit.script(mod.eval()), optimize=False)
        # inspect frozen mod
    ...
    if mod._c._has_method("forward"):
        smith._C._jit_pass_optimize_frozen_graph(mod.graph, optimize_numerics)
...
def optimize_for_inference(
    ...
    will invoke `smith.jit.freeze` automatically.
    ...
    Example (optimizing a module with Conv->Batchnorm)::
        ...
        bn = smith.nn.BatchNorm2d(out_channels, eps=0.001)
        mod = smith.nn.Sequential(conv, bn)
        frozen_mod = smith.jit.optimize_for_inference(smith.jit.script(mod.eval()))
        assert "batch_norm" not in str(frozen_mod.graph)
        # if built with MKLDNN, convolution will be run with MKLDNN weights
        assert "MKLDNN" in frozen_mod.graph
    """
    warnings.warn(
        "`smith.jit.optimize_for_inference` is deprecated. Please use `smith.compile` instead.",
        DeprecationWarning,
    )
    if not isinstance(mod, ScriptModule):
        raise RuntimeError(
    ...
    smith._C._jit_pass_optimize_for_inference(mod._c, other_methods)
