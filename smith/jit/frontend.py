...
import smith
import smith.jit.annotations
from smith import _jit_internal
from smith._C._jit_tree_views import (
    Apply,
    Assert,
    Assign,
    Attribute,
...
from smith._jit_internal import (  # noqa: F401
    _is_drop_fn,
    FunctionModifiers,
    is_static_fn,
    should_drop,
)
from smith._sources import (
    get_source_lines_and_file,
    make_source_context,
    parse_def,
    ParsedDef as _ParsedDef,
)
from smith.jit._dataclass_impls import DATACLASS_MAGIC_METHODS
from smith.jit._monkeytype_config import get_qualified_name, monkeytype_trace
...
class FrontendError(Exception):
    def __init__(self, source_range, msg) -> None:
        ...
        self.error_report = smith._C.ErrorReport(self.source_range)
...
def get_jit_class_def(cls, self_name):
    ...
    Returns:
        smith._C._jit_tree_views.ClassDef: A representation of the class,
    ...
    sourcelines, file_lineno, filename = get_source_lines_and_file(
        cls, smith._C.ErrorReport.call_stack()
    )
    ...
    # workaround, we synthesize SmithScript-friendly implementations ourselves.
...
def get_jit_def(fn, def_name, self_name=None, is_classmethod=False):
    ...
    type_line = smith.jit.annotations.get_type_line(parsed_def.source)
    ...
    type_trace_db = smith.jit._script._get_type_trace_db()
    pdt_arg_types = None
...
def is_smith_jit_ignore_context_manager(stmt) -> bool:
    # checks if the statement is smith.jit.ignore context manager
    if isinstance(stmt.items[0].context_expr, ast.Call):
        ...
        if isinstance(function, ast.Attribute):
            ...
            if attr_name == "_IgnoreContextManager" and isinstance(
                ...
                # there should be at most two nested attributes (e.g smith.jit._IgnoreContextManager)
                if attr_value.attr == "jit" and isinstance(attr_value.value, ast.Name):
                    if attr_value.value.id == "smith":
                        return True
    return False
...
def build_def(ctx, py_def, type_line, def_name, self_name=None, pdt_arg_types=None):
    ...
    if type_line is not None:
        type_comment_decl = smith._C.parse_type_comment(type_line)
        decl = smith._C.merge_type_from_type_comment(
...
"""
    ...
    # <out_1>, <out_2>, ... = smith.jit.frontend.<func>(<in_1>, <in_2>)
    assign_str_lhs = build_args(outputs)
    # this function will be registered in smith.jit.frontend module by default
    assign_str_rhs = (
        f"smith.jit.frontend.{ignore_function_name}(" + build_args(inputs) + ")"
    )
...
class StmtBuilder(Builder):
    ...
    def build_With(ctx, stmt):
        ...
        if is_smith_jit_ignore_context_manager(stmt):
...
class ExprBuilder(Builder):
    ...
    def build_BinOp(ctx, expr):
        ...
        if op == ast.Div and not ctx.uses_true_division:
            ...
            raise FrontendError(
                err_range,
                "Division of ints in SmithScript uses Python 3 true "
