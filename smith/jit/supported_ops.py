...
# > help(smith.jit.supported_ops) will also give a nice listed of the
...
def _get_tensor_ops():
    def is_tensor_method(schema) -> bool:
        ...
        if self.name != "self":
            return False
        if not self.type.isSubtypeOf(smith._C.TensorType.get()):
            return False
        return True

    methods = []
    # discover methods
    for elem in dir(smith.Tensor):
        if not _hidden(elem):
            schemas = smith._C._jit_get_schemas_for_operator("aten::" + elem)
...
def _get_nn_functional_ops():
    ...
    # Iterate over smith.nn.functional
    mod = smith.nn.functional
    name = mod.__name__
    for elem in dir(smith.nn.functional):
        ...
        if "smith.nn.functional" not in attr_module.__name__:
            # Ignore functions from outside smith.nn.functional
            continue

        try:
            # compile fn, get schema
            scripted = smith.jit.script(attr)
    ...
    for mod in smith.jit._builtins._modules_containing_builtins:
        name = mod.__name__
        for elem in dir(mod):
            builtin = _find_builtin(getattr(mod, elem))
            if builtin is not None:
                schemas = smith._C._jit_get_schemas_for_operator(builtin)
                for schema in schemas:
                    # remove _tan but not __and__
                    if not _hidden(elem):
                        functions.append(_emit_schema(name, elem, schema))
    return "Supported Blacksmith Functions", functions


def _get_builtins_helper():
    builtins = []
    for fn, _builtin_name in smith.jit._builtins._builtin_ops:
...
def _get_smithscript_builtins():
    ...
    return "SmithScript Builtin Functions", functions
...
def _get_global_builtins():
    # Taken from the 'globals' map in smith/csrc/jit/frontend/ir_emitter.cpp
    supported_builtins = [
        "print",
        "tuple",
        "float",
    ...
    for fn in supported_builtins:
        ...
        schemas = smith._C._jit_get_schemas_for_operator(op_name)
        for s in schemas:
...
The following functions will use the corresponding magic method on SmithScript classes
