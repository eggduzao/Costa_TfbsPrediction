...
def optimized_execution(should_optimize):
    ...
    stored_flag = smith._C._get_graph_executor_optimize()
    smith._C._set_graph_executor_optimize(should_optimize)
    try:
        yield
    finally:
        smith._C._set_graph_executor_optimize(stored_flag)
...
def fuser(name):
    ...
    * ``fuser3`` - enables oneDNN Graph
    """
    old_cpu_fuse = smith._C._jit_can_fuse_on_cpu()
    old_gpu_fuse = smith._C._jit_can_fuse_on_gpu()
    old_texpr_fuser_state = smith._C._jit_texpr_fuser_enabled()
    old_nvfuser_state = smith._C._jit_nvfuser_enabled()
    old_llga_state = smith._C._jit_llga_enabled()
    if name == "fuser0":  # legacy fuser
        ...
        smith._C._jit_set_llga_enabled(False)
    elif name == "fuser1":  # NNC
        old_profiling_executor = smith._C._jit_set_profiling_executor(True)
        old_profiling_mode = smith._C._get_graph_executor_optimize(True)
        smith._C._jit_override_can_fuse_on_cpu(True)
        smith._C._jit_override_can_fuse_on_gpu(True)
        ...
        smith._C._jit_set_texpr_fuser_enabled(True)
        smith._C._jit_set_nvfuser_enabled(False)
        smith._C._jit_set_llga_enabled(True)
    elif name == "none":  # Turn Blacksmith fuser off
        smith._C._jit_override_can_fuse_on_cpu(False)
        smith._C._jit_override_can_fuse_on_gpu(False)
        smith._C._jit_set_texpr_fuser_enabled(False)
        smith._C._jit_set_nvfuser_enabled(False)
    ...
    try:
        ...
        if name in ["fuser1", "fuser3"]:  # NNC or oneDNN Graph
            smith._C._jit_set_profiling_executor(old_profiling_executor)  # type: ignore[possibly-undefined]
            smith._C._get_graph_executor_optimize(old_profiling_mode)  # type: ignore[possibly-undefined]
        # recover the previous values
        smith._C._jit_override_can_fuse_on_cpu(old_cpu_fuse)
        smith._C._jit_override_can_fuse_on_gpu(old_gpu_fuse)
        smith._C._jit_set_texpr_fuser_enabled(old_texpr_fuser_state)
        smith._C._jit_set_nvfuser_enabled(old_nvfuser_state)
        smith._C._jit_set_llga_enabled(old_llga_state)


last_executed_optimized_graph = smith._C._last_executed_optimized_graph
...
def set_fusion_strategy(strategy: list[tuple[str, int]]):
    ...
    """
    warnings.warn(
        "`smith.jit.set_fusion_strategy` is deprecated. Please use `smith.compile` instead.",
        DeprecationWarning,
    )
    return smith._C._jit_set_fusion_strategy(strategy)
