...
import smith
from smith import TensorType
from smith._C import Graph
...
def apply_input_props_using_example(graph: Graph, example_input: list[Any]) -> None:
    ...
    in_0 = graph_inputs[0]
    if isinstance(in_0.type(), smith._C.ClassType) and in_0.debugName() == "self":
    ...
    for i, (graph_i, example_i) in enumerate(zip(graph_inputs, example_input)):
        ...
        if isinstance(example_i, smith.Tensor) != isinstance(
        ...
        if isinstance(example_i, smith.Tensor):
