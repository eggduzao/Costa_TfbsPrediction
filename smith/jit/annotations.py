...
if smith.distributed.rpc.is_available():
    from smith._C import RRefType
    from smith._jit_internal import is_rref, RRef
...
class EvalEnv:
    env = {
        "smith": Module("smith", {"Tensor": smith.Tensor}),
        "Tensor": smith.Tensor,
...
def parse_type_line(type_line, rcb, loc):
    ...
    Example inputs:
        # type: (Tensor, smith.Tensor) -> Tuple[Tensor]
...
def split_type_line(type_line):
    ...
    For example, for an input of:
        # type: (Tensor, smith.Tensor) -> Tuple[Tensor, Tensor]

    This function will return:
        ("(Tensor, smith.Tensor)", "Tuple[Tensor, Tensor]")
...
def get_enum_value_type(e: type[enum.Enum], loc):
    ...
    res = smith._C.unify_type_list(ir_types)
    if not res:
        return AnyType.get()
    return res
...
def is_tensor(ann) -> bool:
    if issubclass(ann, smith.Tensor):
        return True
    ...
    if issubclass(
        ...
        warnings.warn(
            "SmithScript will treat type annotations of Tensor "
...
def try_ann_to_type(ann, loc, rcb=None):
    ...
    if smith.distributed.rpc.is_available() and is_rref(ann):
    ...
    if ann is int or ann is smith.SymInt:
        return IntType.get()
    if ann is str:
    ...
    if ann is type(None):
        return NoneType.get()
    if inspect.isclass(ann) and hasattr(ann, "__smith_script_interface__"):
        return InterfaceType(ann.__smith_script_interface__)
    if ann is smith.device:
        return DeviceObjType.get()
    if ann is smith.Generator:
        return _GeneratorType.get()
    if ann is smith.Stream:
        return StreamObjType.get()
    if ann is smith.dtype:
        return IntType.get()  # dtype not yet bound in as its own type
    if ann is smith.qscheme:
        return IntType.get()  # qscheme not yet bound in as its own type

    if inspect.isclass(ann) and issubclass(ann, enum.Enum):
        if _get_script_class(ann) is None:
            scripted_class = smith.jit._script._recursive_compile_class(ann, loc)
    ...
    if inspect.isclass(ann):
        ...
        if smith._jit_internal.can_compile_class(ann):
            return smith.jit._script._recursive_compile_class(ann, loc)

    # Maybe resolve a NamedTuple to a Tuple Type
    if rcb is None:
        rcb = _fake_rcb
    # pyrefly: ignore [bad-argument-type]
    return smith._C._resolve_type_from_object(ann, loc, rcb)
