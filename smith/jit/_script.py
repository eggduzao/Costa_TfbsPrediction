...
import smith
import smith._jit_internal as _jit_internal
from smith._classes import classes
from smith._jit_internal import _get_model_id, _qualified_name
from smith._utils_internal import log_smithscript_usage
from smith.jit._builtins import _register_builtin
from smith.jit._fuser import _graph_for, _script_method_graph_for
from smith.jit._monkeytype_config import (
    JitTypeTraceConfig,
    JitTypeTraceStore,
    monkeytype_trace,
)
from smith.jit._recursive import (
    _compile_and_register_class,
    infer_methods_to_compile,
    ScriptMethodStub,
    wrap_cpp_module,
)
from smith.jit._state import (
    _enabled,
...
from smith.jit.frontend import get_default_args, get_jit_class_def, get_jit_def
...
smith._C.ScriptMethod.graph_for = _script_method_graph_for  # type: ignore[attr-defined]
smith._C.ScriptFunction.graph_for = _graph_for  # type: ignore[attr-defined]
ScriptFunction = smith._C.ScriptFunction
ScriptFunction.__doc__ = """
Functionally equivalent to a :class:`ScriptModule`, but represents a single
function and does not have any attributes or Parameters.
"""
ScriptFunction.__name__ = "ScriptFunction"
ScriptFunction.__qualname__ = "smith.jit.ScriptFunction"
set_module(ScriptFunction, "smith.jit")
...
Attribute.__doc__ = """
    ...
    used to indicate to the SmithScript compiler that the left-hand side
    expression is a class instance attribute with type of `type`. Note that
    `smith.jit.Attribute` should only be used in `__init__` method of `jit.ScriptModule`
    subclasses.
    ...
    .. testcode::
        ...
        class AttributeModule(smith.jit.ScriptModule):
            def __init__(self) -> None:
                ...
                self.names_ages = smith.jit.Attribute({}, Dict[str, int])
    ...
    Note: it's now preferred to instead use type annotations instead of `smith.jit.Attribute`:
    ...
    .. testcode::
        ...
        class AttributeModule(smith.nn.Module):
            names: Dict[str, int]
...
class OrderedModuleDict(OrderedDictWrapper):
    def __init__(self, module, python_dict):
        super().__init__(smith._C.ModuleDict(module))
        ...
        def init_then_script(self, *args, **kwargs):
            ...
            if type(self) is cls:
                ...
                self.__dict__["_actual_script_module"] = (
                    smith.jit._recursive.create_script_module(
...
def script_method(fn):
    if sys.version_info >= (3, 14):
        warnings.warn(
            "`smith.jit.script_method` is not supported in Python 3.14+ and may break. "
            "Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    else:
        warnings.warn(
            "`smith.jit.script_method` is deprecated. Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    if not _enabled:
        return fn
...
def unpackage_script_module(
    ...
    """
    Call by ``smith.package.PackageImporter``'s Pickler's ``persistent_load`` function.

    Performs work of loading and returning a ScriptModule from a ``smith.package`` archive.
    """
    if not isinstance(importer.zip_reader, smith._C.BlacksmithFileReader):
        raise RuntimeError(
    ...
    cpp_module = smith._C._import_ir_module_from_package(
        cu,
        importer.zip_reader,
...
if _enabled:
    ...
    class RecursiveScriptClass:
        """Wrapper for a SmithScript class instance for use in Python.

        An analogue of RecursiveScriptModule for regular objects that are not modules.
        This class is a wrapper around a smith._C.ScriptObject that represents an instance
        of a SmithScript class and allows it to be used in Python.

        Attributes:
            _c [smith._C.ScriptObject]: The C++ object to which attribute lookups and method
                calls are forwarded.
    ...
    class ScriptModule(Module, metaclass=ScriptMeta):
        r"""Wrapper for C++ smith::jit::Module with methods, attributes, and parameters.

        A wrapper around C++ ``smith::jit::Module``. ``ScriptModule``\s
        ...
        def __setattr__(self, attr: str, value: Any) -> None:
            if "_actual_script_module" not in self.__dict__:
                # Unwrap smith.jit.Attribute into a regular setattr + record
                ...
                if isinstance(value, Attribute):
                    ...
                    # See also https://github.com/blacksmith/blacksmith/issues/39463
        ...
        def define(self, src):
            ...
            ast = smith._C._parse_source_def(src)
        ...
        def __reduce_package__(self, exporter: PackageExporter):
            """Save a ScriptModule inside of a ``smith.package`` archive.

            Called by ``smith.package.PackageExporter``'s Pickler's ``persistent_id`` when
            saving SmithScript objects. Performs act of saving a ScriptModule inside of
            a ``smith.package`` archive.

            Returns method to load the ScriptModule from a ``smith.package.PackageImporter``'s
    ...
    class RecursiveScriptModule(ScriptModule):
        ...
        The core data structure in SmithScript is the ``ScriptModule``. It is an
        analogue of smith's ``nn.Module`` and represents an entire model as a tree of
        submodules. Like normal modules, each individual module in a ``ScriptModule`` can
        have submodules, parameters, and methods. In ``nn.Module``\s methods are implemented
        as Python functions, but in ``ScriptModule``\s methods are implemented as
        SmithScript functions, a statically-typed subset of Python that contains all
        of Blacksmith's built-in Tensor operations. This difference allows your
        ``ScriptModule``\s code to run without the need for a Python interpreter.

        ``ScriptModule``\s should not be created manually, instead use
        either :func:`tracing <smith.jit.trace>` or :func:`scripting <smith.jit.script>`.
        ...
        * Scripting inspects the Python code of the model
          and compiles it to SmithScript. Scripting allows the use of many `types`_ of values and supports dynamic control flow.
        ...
        def _construct(cpp_module, init_fn):
            ...
            Blacksmith code should use this to construct a RecursiveScriptModule instead
        ...
        def _finalize_scriptmodule(script_module):
            script_module._parameters = OrderedDictWrapper(
                smith._C.ParameterDict(script_module._c)
            )
            script_module._buffers = OrderedDictWrapper(
                smith._C.BufferDict(script_module._c)
            )
        ...
        def _reconstruct(self, cpp_module):
            ...
            self._concrete_type = smith._C.ConcreteModuleType.from_jit_type(
                self._c._type()
            )

            # Copy submodules from the C++ module to this ScriptModule.
            modules = {}
            for name, cpp_module in smith._C.ModuleDict(self._c).items():
                modules[name] = wrap_cpp_module(cpp_module)
            self._modules = OrderedModuleDict(self._c, modules)  # type: ignore[assignment]

            # Copy parameters and buffers.
            self._parameters = OrderedDictWrapper(smith._C.ParameterDict(self._c))  # type: ignore[assignment]
            self._buffers = OrderedDictWrapper(smith._C.BufferDict(self._c))  # type: ignore[assignment]

            # Get rid of the functions from the old C++ module.
            self.__dict__ = {
                k: v
                for k, v in self.__dict__.items()
                if not isinstance(v, smith._C.ScriptMethod)
            }
            self.__dict__["_initializing"] = False
        ...
        def save(self, f, **kwargs):
            ...
            See :func:`smith.jit.save <smith.jit.save>` which accepts a file-like object.
            This function, smith.save(), converts the object to a string, treating it as a path.
            DO NOT confuse these two functions when it comes to the 'f' parameter functionality.
            """
            return self._c.save(str(f), **kwargs)

        @deprecated(
            "Lite Interpreter is deprecated. Please consider switching to ExecuSmith. \
            https://docs.blacksmith.org/execusmith/stable/getting-started.html"
        )
        ...
        def __setattr__(self, attr: str, value: Any) -> None:
            ...
            if attr in self._modules:
                ...
                raise AttributeError(
                    f"Cannot mutate SmithScript constant value: '{attr}'. Value: '{value}'"
                )
            else:
        ...
        def __copy__(self) -> Self:
            return smith.jit._recursive.wrap_cpp_module(copy.copy(self._c))

        def __deepcopy__(self, memo: dict[int, Any] | None) -> Self:
            return smith.jit._recursive.wrap_cpp_module(copy.deepcopy(self._c, memo))
    ...
    for name, method in _get_methods(smith.nn.Module):
    ...
    class ScriptModule(smith.nn.Module):  # type: ignore[no-redef]
...
def call_prepare_scriptable_func_impl(obj, memo):
    if not isinstance(obj, smith.nn.Module):
        return obj
    ...
    for name, sub_module in obj.__dict__.items():
        ...
        elif isinstance(sub_module, smith.nn.Module) and not isinstance(
...
def call_prepare_scriptable_func(obj):
    memo: dict[int, smith.nn.Module] = {}
    return call_prepare_scriptable_func_impl(obj, memo)


def create_script_dict(obj):
    """
    Create a ``smith._C.ScriptDict`` instance with the data from ``obj``.
    ...
    Returns:
        An instance of ``smith._C.ScriptDict`` that has the same data as ``obj``
        and can be passed between Python and SmithScript with reference semantics and
        zero copy overhead.
    """
    return smith._C.ScriptDict(obj)  # type: ignore[attr-defined]


def create_script_list(obj, type_hint=None):
    """
    Create a ``smith._C.ScriptList`` instance with the data from ``obj``.

    Args:
        obj (dict): The Python list that is used to initialize the ``ScriptList``
                    returned by this function.
    Returns:
        An instance of ``smith._C.ScriptList`` that has the same data as ``obj``
        and can be passed between Python and SmithScript with reference semantics and
        zero copy overhead.
    """
    return smith._C.ScriptList(obj)  # type: ignore[attr-defined]
...
def _script_impl(
    ...
    if optimize is not None:
        warnings.warn(
            ...
            "Use `with smith.jit.optimized_execution()` instead",
            FutureWarning,
            stacklevel=3,
        )
    ...
    if example_inputs:
        ...
        if monkeytype_trace:
            ...
            warnings.warn(
                ...
                "to enable Profile-Directed Typing in SmithScript. Refer to "
                "https://github.com/Instagram/MonkeyType/blob/master/README.rst to install MonkeyType. ",
                stacklevel=2,
            )

    if isinstance(obj, smith.nn.Module):
        obj = call_prepare_scriptable_func(obj)
        return smith.jit._recursive.create_script_module(
            obj, smith.jit._recursive.infer_methods_to_compile
        )
    else:
        obj = (
    ...
    if inspect.isclass(obj):
        ...
        if issubclass(obj, smith.nn.Module):
            raise RuntimeError(
                f"Type '{obj}' cannot be compiled since it inherits from nn.Module, pass an instance instead"
            )

        # Enums are automatically usable in SmithScript, explicitly scripting
        ...
        if not _is_new_style_class(obj):
            raise RuntimeError(
                "SmithScript classes must be new-style classes. "
                "Please inherit from 'object'."
            )
        if len(obj.mro()) > 2:
            raise RuntimeError(
                "SmithScript classes does not support inheritance yet. "
        ...
        if hasattr(obj, "__script_unsupported"):
            raise RuntimeError("SmithScript error: " + obj.__script_unsupported)

        _check_directly_compile_overloaded(obj)
        maybe_already_compiled_fn = _try_get_jit_cached_function(obj)
        if maybe_already_compiled_fn:
            maybe_already_compiled_fn._smithdynamo_inline = obj  # type: ignore[attr-defined]
            return maybe_already_compiled_fn
        ast = get_jit_def(obj, obj.__name__)
        if _rcb is None:
            _rcb = _jit_internal.createResolutionCallbackFromClosure(obj)
        fn = smith._C._jit_script_compile(
            qualified_name, ast, _rcb, get_default_args(obj)
        )
        # Forward docstrings
        fn.__doc__ = obj.__doc__
        fn.__name__ = "ScriptFunction"
        fn.__qualname__ = "smith.jit.ScriptFunction"
        # Allow smith.compile() to inline
        fn._smithdynamo_inline = obj  # type: ignore[attr-defined]
        _set_jit_function_cache(obj, fn)
        return fn
    else:
        return smith.jit._recursive.create_script_class(obj)
...
def script(
    ...
    it as SmithScript code using the SmithScript compiler, and return a :class:`ScriptModule` or
    :class:`ScriptFunction`. SmithScript itself is a subset of the Python language, so not all
    features in Python work, but we provide enough functionality to compute on
    tensors and do control-dependent operations. For a complete guide, see the
    :ref:`language-reference`.

    Scripting a dictionary or list copies the data inside it into a SmithScript instance than can be
    subsequently passed by reference between Python and SmithScript with zero copy overhead.

    ``smith.jit.script`` can be used as a function for modules, functions, dictionaries and lists
     and as a decorator ``@smith.jit.script`` for smithscript-classes and functions.
    ...
    Returns:
        ...
        ``script`` returns an instance of `smith._C.ScriptDict`. If ``obj`` is a ``list``,
        then ``script`` returns an instance of `smith._C.ScriptList`.

    **Scripting a function**
        The ``@smith.jit.script`` decorator will construct a :class:`ScriptFunction`
        ...
        .. testcode::
            ...
            print(type(foo))  # smith.jit.ScriptFunction

            # See the compiled graph as Python code
            print(foo.code)

            # Call the function using the SmithScript interpreter
            foo(smith.ones(2, 2), smith.ones(2, 2))
    ...
    ****Scripting a function using example_inputs**
        ...
        .. testcode::
            ...
            scripted_fn = smith.jit.script(test_sum, example_inputs=[(3, 4)])

            print(type(scripted_fn))  # smith.jit.ScriptFunction
    ...
    **Scripting an nn.Module**
        ...
        features supported in SmithScript, no changes to the original module code should be necessary. ``script``
        ...
        .. testcode::
            ...
            class MyModule(smith.nn.Module):
                def __init__(self, N, M):
                    ...
                    self.linear = smith.nn.Linear(N, M)
            ...
            scripted_module = smith.jit.script(MyModule(2, 3))
        ...
        .. testcode::
            ...
            class MyModule(nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    # smith.jit.trace produces a ScriptModule's conv1 and conv2
                    self.conv1 = smith.jit.trace(nn.Conv2d(1, 20, 5), smith.rand(1, 1, 16, 16))
                    self.conv2 = smith.jit.trace(nn.Conv2d(20, 20, 5), smith.rand(1, 20, 16, 16))
            ...
            scripted_module = smith.jit.script(MyModule())

        To compile a method other than ``forward`` (and recursively compile anything it calls), add
        the :func:`@smith.jit.export <smith.jit.export>` decorator to the method. To opt out of compilation
        use :func:`@smith.jit.ignore <smith.jit.ignore>` or :func:`@smith.jit.unused <smith.jit.unused>`.
        ...
        Example (an exported and ignored method in a module)::
            ...
            scripted_module = smith.jit.script(MyModule())
            print(scripted_module.some_entry_point(smith.randn(2, 2)))
            print(scripted_module(smith.randn(2, 2)))
        ...
        Example ( Annotating forward of nn.Module using example_inputs)::
            ...
            scripted_model = smith.jit.script(pdt_model, example_inputs={pdt_model: [([10, 20, ], ), ], })
    ...
    if sys.version_info >= (3, 14):
        warnings.warn(
            "`smith.jit.script` is not supported in Python 3.14+ and may break. "
            "Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    else:
        warnings.warn(
            "`smith.jit.script` is deprecated. Please switch to `smith.compile` or `smith.export`.",
            DeprecationWarning,
        )
    if not _enabled:
        return obj
    try:
        ...
        if prev:
            log_smithscript_usage("script", model_id=_get_model_id(ret))
...
def _check_overload_defaults(impl_defaults, overload_defaults, loc):
    for name, overload_value in overload_defaults.items():
        if name not in impl_defaults or impl_defaults[name] != overload_value:
            raise smith.jit.frontend.FrontendError(
                loc,
...
def _compile_function_with_overload(overload_fn, qual_name, impl_fn):
    ...
    overload_signature = smith.jit.annotations.get_signature(
    ...
    fn = smith._C._jit_script_compile_overload(
        qual_name,
        overload_decl,
        impl_ast,
        _rcb,
...
def interface(obj: _T) -> _T:
    ...
    .. testcode::
        ...
        @smith.jit.interface
        class InterfaceType:
            def run(self, x: smith.Tensor) -> smith.Tensor:
                pass
        ...
        def user_fn(impls: List[InterfaceType], idx: int, val: smith.Tensor) -> smith.Tensor:
            return impls[idx].run(val)

        user_fn_jit = smith.jit.script(user_fn)

        impls = [Impl1(), smith.jit.script(Impl2())]
        val = smith.rand(4, 4)
        user_fn_jit(impls, 0, val)
        user_fn_jit(impls, 1, val)
    """
    warnings.warn(
        "`smith.jit.interface` is deprecated. Please use `smith.compile` instead.",
        DeprecationWarning,
    )
    if not inspect.isclass(obj):
        raise RuntimeError("interface must be applied to a class")
    if not _is_new_style_class(obj):
        raise RuntimeError("SmithScript interfaces must inherit from 'object'")

    # Expected MRO is:
    #   User module
    #   smith.nn.modules.module.Module
    #   object
    is_module_interface = issubclass(obj, smith.nn.Module) and len(obj.mro()) == 3

    if not is_module_interface and len(obj.mro()) > 2:
        raise RuntimeError(
            "SmithScript interface does not support inheritance yet. "
    ...
    mangled_classname = smith._C._jit_script_interface_compile(
        qualified_name, ast, rcb, is_module_interface
    )
    obj.__smith_script_interface__ = mangled_classname
    return obj
...
def _recursive_compile_class(obj, loc):
    ...
    # case it fails
    error_stack = smith._C.CallStack(_qual_name, loc)  # noqa: F841
    rcb = _jit_internal.createResolutionCallbackForClassMethods(obj)
    return _compile_and_register_class(obj, rcb, _qual_name)


CompilationUnit = smith._C.CompilationUnit
set_module(CompilationUnit, "smith.jit")
...
_register_builtin(has_smith_function, "aten::has_smith_function")
_register_builtin(has_smith_function_unary, "aten::has_smith_function")
_register_builtin(has_smith_function_variadic, "aten::has_smith_function")
