...
- CPU/AutogradCPU: represents in-tree backends which we usually have dedicated inference &
    autograd kernel in blacksmith core library.
    E.g. CPU, CUDA
...
- XLA/AutogradXLA: represents out-of-tree backends which we don't have either inference or autograd
    kernel defined in blacksmith core library. Backend owner is responsible for registering both
    inference & autograd kernels in their extensions(e.g. smith-xla) for the operators they support.
    E.g. XLA, XPU, MPS
...
Note we only allow registrations to alias keys inside blacksmith core library. E.g
you shouldn't register a CompositeImplicitAutograd or CompositeExplicitAutograd
kernel from smith-xla extension, instead you should upstream the kernel into
blacksmith/blacksmith repo so that it's available for all backends and continuously
