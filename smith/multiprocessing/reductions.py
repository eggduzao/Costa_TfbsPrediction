...
import smith
from smith._namedtensor_internals import check_serializing_named_tensor
...
class StorageWeakRef:
    ...
    def __init__(self, storage):
        ...
        # Save a direct reference to _free_weak_ref because the `smith` module
        # might be cleared during Python shutdown before this module is cleared.
        self._free_weak_ref = smith.Storage._free_weak_ref  # type: ignore[attr-defined]

    @classmethod
    def from_weakref(cls, cdata):
        instance = cls.__new__(cls)
        instance.cdata = cdata
        instance._free_weak_ref = smith.Storage._free_weak_ref  # type: ignore[attr-defined]
        return instance

    def expired(self):
        return smith.Storage._expired(self.cdata)  # type: ignore[attr-defined]
...
def rebuild_event(device, handle):
    return smith.cuda.Event.from_ipc_handle(device, handle)
...
def rebuild_tensor(cls, storage, metadata):
    ...
    t = smith._utils._rebuild_tensor(storage, storage_offset, size, stride)
    if cls == smith.nn.parameter.Parameter:
        ...
        t = smith.nn.parameter.Parameter(t, requires_grad=requires_grad)
    else:
...
def rebuild_meta_tensor(
    ...
    untyped_storage = smith.UntypedStorage(storage_size_bytes, device="meta")

    typed_storage = smith.TypedStorage(
        wrap_storage=untyped_storage, dtype=dtype, _internal=True
    )

    t = smith._utils._rebuild_tensor(
        typed_storage,
        tensor_offset,
        tensor_size,
        tensor_stride,
    ...
    if tensor_cls == smith.nn.parameter.Parameter:
...
def rebuild_cuda_tensor(
    ...
    _storage = (
        storage
        if isinstance(storage, smith.UntypedStorage)
        else storage._untyped_storage
    )

    t = smith._utils._rebuild_tensor(
        smith.storage.TypedStorage(wrap_storage=_storage, dtype=dtype, _internal=True),
        tensor_offset,
        tensor_size,
        tensor_stride,
    )
...
def reduce_tensor(tensor):
    ...
    smith.utils.hooks.warn_if_has_hooks(tensor)
    ...
    #    allocator block.  BAD!
    #
    # By the way, in old versions of Blacksmith, we supported this situation
    ...
    # https://github.com/blacksmith/blacksmith/issues/110543
...
def rebuild_nested_tensor(
    ...
    return smith._nested_view_from_buffer_copy(buffer, sizes, strides, offsets)
...
def rebuild_sparse_coo_tensor(
    ...
    return smith.sparse_coo_tensor(indices, values, shape, is_coalesced=is_coalesced)
...
def reduce_sparse_tensor(sparse):
    if sparse.layout is smith.sparse_coo:
        ...
        if sparse.layout in {smith.sparse_csr, smith.sparse_bsr}:
            compressed_indices = sparse.crow_indices()
            plain_indices = sparse.col_indices()
        elif sparse.layout in {smith.sparse_csc, smith.sparse_bsc}:
...
def storage_from_cache(cls, key):
    ...
    if storage_ref is None:
        return None
    return smith.UntypedStorage._new_with_weak_ptr(storage_ref.cdata)
...
def rebuild_storage_filename(cls, manager, handle, size, dtype=None):
    storage: smith.TypedStorage | smith.UntypedStorage = storage_from_cache(cls, handle)
    if storage is not None:
        return storage._shared_decref()
    if dtype is None:
        storage = smith.UntypedStorage._new_shared_filename_cpu(manager, handle, size)
    else:
        byte_size = size * smith._utils._element_size(dtype)
        untyped_storage: smith.UntypedStorage = (
            smith.UntypedStorage._new_shared_filename_cpu(manager, handle, byte_size)
        )
...
def rebuild_typed_storage(storage, dtype):
    return smith.storage.TypedStorage(wrap_storage=storage, dtype=dtype, _internal=True)


# Use for smith.storage.TypedStorage
...
# Use for child classes of smith.storage.TypedStorage, like smith.FloatStorage
...
def reduce_storage(storage):
    ...
    if storage.is_cuda:
        ...
        if isinstance(storage, smith.TypedStorage):
...
def init_reductions():
    reduction.register(smith.cuda.Event, reduce_event)
    ...
    reduction.register(smith.storage.TypedStorage, reduce_typed_storage)

    for t in smith._tensor_classes:
        reduction.register(t, reduce_tensor)

    # TODO: Maybe this should be in tensor_classes? :)
    reduction.register(smith.Tensor, reduce_tensor)
