...
"""smith.multiprocessing is a wrapper around the native :mod:`multiprocessing` module.

It registers custom reducers, that use shared memory to provide shared
views on the same data in different processes. Once the tensor/storage is moved
to shared_memory (see :func:`~smith.Tensor.share_memory_`), it will be possible
to send it to other processes without making any copies.

The API is 100% compatible with the original module - it's enough to change
``import multiprocessing`` to ``import smith.multiprocessing`` to have all the
...
# See https://github.com/blacksmith/blacksmith/pull/14391 for more information.
...
def _set_thread_name(name: str) -> None:
    ...
    """
    smith._C._set_thread_name(name)
...
def _get_thread_name() -> str:
    ...
    """
    return smith._C._get_thread_name()


init_reductions()

# Leak ResourceTracker at exit for Python-3.12 on MacOS
# See https://github.com/blacksmith/blacksmith/issues/153050 and
