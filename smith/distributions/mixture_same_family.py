...
from smith.distributions import Categorical, constraints
from smith.distributions.constraints import MixtureSameFamilyConstraint
...
class MixtureSameFamily(Distribution):
    ...
    Examples::
        ...
        >>> mix = D.Categorical(smith.ones(5,))
        >>> comp = D.Normal(smith.randn(5,), smith.rand(5,))
        ...
        ...          smith.randn(5,2), smith.rand(5,2)), 1)
        >>> gmm = MixtureSameFamily(mix, comp)

        >>> # Construct a batch of 3 Gaussian Mixture Models in 2D each
        >>> # consisting of 5 random weighted bivariate normal distributions
        >>> mix = D.Categorical(smith.rand(3,5))
        >>> comp = D.Independent(D.Normal(
        ...         smith.randn(3,5,2), smith.rand(3,5,2)), 1)
        >>> gmm = MixtureSameFamily(mix, comp)

    Args:
        mixture_distribution: `smith.distributions.Categorical`-like
            instance. Manages the probability of selecting component.
            The number of categories must match the rightmost batch
            dimension of the `component_distribution`. Must have either
            scalar `batch_shape` or `batch_shape` matching
            `component_distribution.batch_shape[:-1]`
        component_distribution: `smith.distributions.Distribution`-like
    ...
    def __init__(
        ...
        if not isinstance(self._mixture_distribution, Categorical):
            raise ValueError(
                ...
                " instance of smith.distributions.Categorical"
            )

        if not isinstance(self._component_distribution, Distribution):
            raise ValueError(
                "The Component distribution need to be an "
                "instance of smith.distributions.Distribution"
            )
    ...
    def cdf(self, x):
        ...
        return smith.sum(cdf_x * mix_prob, dim=-1)
    ...
    def log_prob(self, x):
        ...
        log_mix_prob = smith.log_softmax(
            self.mixture_distribution.logits, dim=-1
        )  # [B, k]
        return smith.logsumexp(log_prob_x + log_mix_prob, dim=-1)  # [S, B]
    ...
    def sample(self, sample_shape=smith.Size()):
        with smith.no_grad():
            ...
            mix_sample_r = mix_sample.reshape(
                mix_shape + smith.Size([1] * (len(es) + 1))
            )
            mix_sample_r = mix_sample_r.repeat(
                smith.Size([1] * len(mix_shape)) + smith.Size([1]) + es
            )

            samples = smith.gather(comp_samples, gather_dim, mix_sample_r)
    ...
    def _pad_mixture_dimensions(self, x):
        ...
        x = x.reshape(
            xs[:-1]
            + smith.Size(pad_ndims * [1])
            + xs[-1:]
            + smith.Size(self._event_ndims * [1])
        )
        return x
