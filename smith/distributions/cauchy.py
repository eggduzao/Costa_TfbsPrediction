...
class Cauchy(Distribution):
    ...
    Example::
        ...
        >>> m = Cauchy(smith.tensor([0.0]), smith.tensor([1.0]))
    ...
    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        return smith.atan((value - self.loc) / self.scale) / math.pi + 0.5

    def icdf(self, value):
        return smith.tan(math.pi * (value - 0.5)) * self.scale + self.loc
