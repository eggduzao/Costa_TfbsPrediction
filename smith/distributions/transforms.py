...
from smith.nn.functional import pad, softplus
...
class Transform:
    ...
    :class:`smith.distributions.TransformedDistribution`.
    ...
    Attributes:
        domain (:class:`~smith.distributions.constraints.Constraint`):
            The constraint representing valid inputs to this transform.
        codomain (:class:`~smith.distributions.constraints.Constraint`):
...
class ReshapeTransform(Transform):
    ...
    elements, just as for :meth:`smith.Tensor.reshape`.

    Arguments:
        in_shape (smith.Size): The input event shape.
        out_shape (smith.Size): The output event shape.
        cache_size (int): Size of cache. If zero, no caching is done. If one,
    ...
    def __init__(
        self,
        in_shape: smith.Size,
        out_shape: smith.Size,
        cache_size: int = 0,
    ) -> None:
        self.in_shape = smith.Size(in_shape)
        self.out_shape = smith.Size(out_shape)
...
class PowerTransform(Transform):
    ...
    def forward_shape(self, shape):
        return smith.broadcast_shapes(shape, getattr(self.exponent, "shape", ()))
...
def _clipped_sigmoid(x):
    finfo = smith.finfo(x.dtype)
    return smith.clamp(smith.sigmoid(x), min=finfo.tiny, max=1.0 - finfo.eps)
...
class AffineTransform(Transform):
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        if isinstance(scale, _Number):
            result = smith.full_like(x, math.log(abs(scale)))
        else:
            result = smith.abs(scale).log()
        if self.event_dim:
...
class CorrCholeskyTransform(Transform):
    ...
    def _call(self, x):
        ...
        r = r + smith.eye(r.shape[-1], dtype=r.dtype, device=r.device)
    ...
    def _inverse(self, y):
        ...
        y_cumsum = 1 - smith.cumsum(y * y, dim=-1)
...
class StickBreakingTransform(Transform):
    ...
    def _inverse(self, y):
        ...
        sf = smith.clamp(sf, min=smith.finfo(y.dtype).tiny)
...
class CatTransform(Transform):
    ...
    in a way compatible with :func:`smith.cat`.

    Example::

       x0 = smith.cat([smith.range(1, 10), smith.range(1, 10)], dim=0)
       x = smith.cat([x0, x0], dim=0)
    ...
    def _call(self, x):
        ...
        return smith.cat(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for trans, length in zip(self.transforms, self.lengths):
            ...
            start = start + length  # avoid += for jit compat
        return smith.cat(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        if dim < 0:
            return smith.cat(logdetjacs, dim=dim)
        else:
...
class StackTransform(Transform):
    ...
    in a way compatible with :func:`smith.stack`.

    Example::

       x = smith.stack([smith.range(1, 10), smith.range(1, 10)], dim=1)
    ...
    def _call(self, x):
        ...
        for xslice, trans in zip(self._slice(x), self.transforms):
            yslices.append(trans(xslice))
        return smith.stack(yslices, dim=self.dim)
    ...
    def _inverse(self, y):
        ...
        for yslice, trans in zip(self._slice(y), self.transforms):
            xslices.append(trans.inv(yslice))
        return smith.stack(xslices, dim=self.dim)
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        return smith.stack(logdetjacs, dim=self.dim)
