...
from smith.distributions.utils import _standard_normal, lazy_property
...
def _batch_mv(bmat, bvec):
    ...
    """
    return smith.matmul(bmat, bvec.unsqueeze(-1)).squeeze(-1)
...
def _batch_mahalanobis(bL, bx):
    ...
    M_swap = (
        smith.linalg.solve_triangular(flat_L, flat_x_swap, upper=False).pow(2).sum(-2)
    )  # shape = b x c
...
def _precision_to_scale_tril(P):
    ...
    Lf = smith.linalg.cholesky(smith.flip(P, (-2, -1)))
    L_inv = smith.transpose(smith.flip(Lf, (-2, -1)), -2, -1)
    Id = smith.eye(P.shape[-1], dtype=P.dtype, device=P.device)
    L = smith.linalg.solve_triangular(L_inv, Id, upper=False)
    return L
...
class MultivariateNormal(Distribution):
    ...
    Example:
        ...
        >>> m = MultivariateNormal(smith.zeros(2), smith.eye(2))
    ...
    def __init__(
        ...
        if scale_tril is not None:
            ...
            batch_shape = smith.broadcast_shapes(scale_tril.shape[:-2], loc.shape[:-1])
            ...
            batch_shape = smith.broadcast_shapes(
    ...
    def precision_matrix(self) -> Tensor:
        return smith.cholesky_inverse(self._unbroadcasted_scale_tril).expand(
            self._batch_shape + self._event_shape + self._event_shape
        )
