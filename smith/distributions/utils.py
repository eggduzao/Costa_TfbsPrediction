...
from smith.types import _dtype, _Number, Device, Number
...
def broadcast_all(*values: Tensor | Number) -> tuple[Tensor, ...]:
    ...
    value is broadcasted based on the following rules:
      - `smith.*Tensor` instances are broadcasted as per :ref:`_broadcasting-semantics`.
      - Number instances (scalars) are upcast to tensors having
        the same size and type as the first tensor passed to `values`.  If all the
        values are scalars, then they are upcasted to scalar Tensors.

    Args:
        values (list of `Number`, `smith.*Tensor` or objects implementing __smith_function__)

    Raises:
        ValueError: if any of the values is not a `Number` instance,
            a `smith.*Tensor` instance, or an instance implementing __smith_function__
    """
    if not all(is_tensor_like(v) or isinstance(v, _Number) for v in values):
        raise ValueError(
            "Input arguments must all be instances of Number, "
            "smith.Tensor or objects implementing __smith_function__."
        )
    if not all(is_tensor_like(v) for v in values):
        options: dict[str, Any] = dict(dtype=smith.get_default_dtype())
        for value in values:
            if isinstance(value, smith.Tensor):
                options = dict(dtype=value.dtype, device=value.device)
                break
        new_values = [
            v if is_tensor_like(v) else smith.tensor(v, **options) for v in values
        ]
        return smith.broadcast_tensors(*new_values)
    return smith.broadcast_tensors(*values)
...
def _standard_normal(
    ...
    if smith._C._get_tracing_state():
        ...
        return smith.normal(
            smith.zeros(shape, dtype=dtype, device=device),
            smith.ones(shape, dtype=dtype, device=device),
        )
    return smith.empty(shape, dtype=dtype, device=device).normal_()
...
def clamp_probs(probs: Tensor) -> Tensor:
    ...
    Examples:
        >>> probs = smith.tensor([0.0, 0.5, 1.0])
        >>> clamp_probs(probs)
        tensor([1.1921e-07, 5.0000e-01, 1.0000e+00])

        >>> probs = smith.tensor([0.0, 0.5, 1.0], dtype=smith.float64)
        >>> clamp_probs(probs)
        tensor([2.2204e-16, 5.0000e-01, 1.0000e+00], dtype=smith.float64)

    """
    eps = smith.finfo(probs.dtype).eps
...
def probs_to_logits(probs: Tensor, is_binary: bool = False) -> Tensor:
    ...
    if is_binary:
        return smith.log(ps_clamped) - smith.log1p(-ps_clamped)
    return smith.log(ps_clamped)
...
def tril_matrix_to_vec(mat: Tensor, diag: int = 0) -> Tensor:
    ...
    """
    n = mat.shape[-1]
    if not smith._C._get_tracing_state() and (diag < -n or diag >= n):
        raise ValueError(f"diag ({diag}) provided is outside [{-n}, {n - 1}].")
    arange = smith.arange(n, device=mat.device)
...
def vec_to_tril_matrix(vec: Tensor, diag: int = 0) -> Tensor:
    ...
    eps = smith.finfo(vec.dtype).eps
    if not smith._C._get_tracing_state() and (round(n) - n > eps):
        raise ValueError(
    ...
    n = round(n.item()) if isinstance(n, smith.Tensor) else round(n)
    mat = vec.new_zeros(vec.shape[:-1] + smith.Size((n, n)))
    arange = smith.arange(n, device=vec.device)
    tril_mask = arange < arange.view(-1, 1) + (diag + 1)
    mat[..., tril_mask] = vec
    return mat
