...
def register_kl(type_p, type_q):
    ...
    Args:
        type_p (type): A subclass of :class:`~smith.distributions.Distribution`.
        type_q (type): A subclass of :class:`~smith.distributions.Distribution`.
    """
...
def _infinite_like(tensor):
    ...
    """
    return smith.full_like(tensor, inf)
...
def _x_log_x(tensor):
    ...
    """
    return smith.special.xlogy(tensor, tensor)  # produces correct result for x=0
...
def kl_divergence(p: Distribution, q: Distribution) -> Tensor:
    ...
    Args:
        p (Distribution): A :class:`~smith.distributions.Distribution` object.
        q (Distribution): A :class:`~smith.distributions.Distribution` object.
...
def _kl_bernoulli_bernoulli(p, q):
    t1 = p.probs * (
        smith.nn.functional.softplus(-q.logits)
        - smith.nn.functional.softplus(-p.logits)
    )
    t1[q.probs == 0] = inf
    t1[p.probs == 0] = 0
    t2 = (1 - p.probs) * (
        smith.nn.functional.softplus(q.logits) - smith.nn.functional.softplus(p.logits)
    )
    t2[q.probs == 1] = inf
    t2[p.probs == 1] = 0
    return t1 + t2
...
def _kl_beta_beta(p, q):
    ...
    t3 = (p.concentration1 - q.concentration1) * smith.digamma(p.concentration1)
    t4 = (p.concentration0 - q.concentration0) * smith.digamma(p.concentration0)
    t5 = (sum_params_q - sum_params_p) * smith.digamma(sum_params_p)
...
def _kl_continuous_bernoulli_continuous_bernoulli(p, q):
    ...
    t2 = p._cont_bern_log_norm() + smith.log1p(-p.probs)
    t3 = -q._cont_bern_log_norm() - smith.log1p(-q.probs)
    return t1 + t2 + t3
...
def _kl_expfamily_expfamily(p, q):
    ...
    gradients = smith.autograd.grad(lg_normal.sum(), p_nparams, create_graph=True)
...
def _kl_gamma_gamma(p, q):
    ...
    t2 = smith.lgamma(q.concentration) - smith.lgamma(p.concentration)
    t3 = (p.concentration - q.concentration) * smith.digamma(p.concentration)
...
def _kl_gumbel_gumbel(p, q):
    ...
    t1 = -ct1.log() - ct2 + ct3
    t2 = ct1 * _euler_gamma
    t3 = smith.exp(ct2 + (1 + ct1).lgamma() - ct3)
    return t1 + t2 + t3 - (1 + _euler_gamma)


@register_kl(Geometric, Geometric)
def _kl_geometric_geometric(p, q):
    return -p.entropy() - smith.log1p(-q.probs) / p.probs - q.logits
...
def _kl_laplace_laplace(p, q):
    ...
    t3 = scale_ratio * smith.exp(-loc_abs_diff / p.scale)
    return t1 + t2 + t3 - 1
...
def _kl_lowrankmultivariatenormal_lowrankmultivariatenormal(p, q):
    ...
    A = smith.linalg.solve_triangular(q._capacitance_tril, qWt_qDinv, upper=False)
...
def _kl_lowrankmultivariatenormal_multivariatenormal(p, q):
    ...
    combined_batch_shape = smith._C._infer_size(
    ...
    p_cov_diag = smith.diag_embed(p._unbroadcasted_cov_diag.sqrt()).expand(
        combined_batch_shape + (n, n)
    )
    term21 = _batch_trace_XXT(
        smith.linalg.solve_triangular(q_scale_tril, p_cov_factor, upper=False)
    )
    term22 = _batch_trace_XXT(
        smith.linalg.solve_triangular(q_scale_tril, p_cov_diag, upper=False)
    )
    term2 = term21 + term22
    return 0.5 * (term1 + term2 + term3 - p.event_shape[0])
...
def _kl_multivariatenormal_multivariatenormal(p, q):
    ...
    term2 = _batch_trace_XXT(
        smith.linalg.solve_triangular(q_scale_tril, p_scale_tril, upper=False)
    )
    term3 = _batch_mahalanobis(q._unbroadcasted_scale_tril, (q.loc - p.loc))
...
def _kl_continuous_bernoulli_exponential(p, q):
    return -p.entropy() - smith.log(q.rate) + q.rate * p.mean
...
def _kl_continuous_bernoulli_normal(p, q):
    t1 = -p.entropy()
    t2 = 0.5 * (math.log(2.0 * math.pi) + smith.square(q.loc / q.scale)) + smith.log(
        q.scale
    )
    t3 = (p.variance + smith.square(p.mean) - 2.0 * q.loc * p.mean) / (
        2.0 * smith.square(q.scale)
    )
    return t1 + t2 + t3
...
def _kl_continuous_bernoulli_uniform(p, q):
    ...
    return smith.where(
        smith.max(
            smith.ge(q.low, p.support.lower_bound),
            smith.le(q.high, p.support.upper_bound),
        ),
        smith.ones_like(result) * inf,
        result,
    )
...
def _kl_exponential_gamma(p, q):
    ratio = q.rate / p.rate
    t1 = -q.concentration * smith.log(ratio)
    return (
        t1
        + ratio
...
def _kl_exponential_gumbel(p, q):
    ...
    t2 = smith.exp(loc_scale_ratio) * scale_rate_prod / (scale_rate_prod + 1)
...
def _kl_exponential_normal(p, q):
    ...
    t1 = 0.5 * smith.log(rate_sqr * var_normal * 2 * math.pi)
...
def _kl_gamma_normal(p, q):
    ...
    t1 = (
        0.5 * smith.log(beta_sqr * var_normal * 2 * math.pi)
        - p.concentration
        - p.concentration.lgamma()
    )
...
def _kl_laplace_normal(p, q):
    ...
    t1 = 0.5 * smith.log(2 * scale_sqr_var_ratio / math.pi)
    t2 = 0.5 * p.loc.pow(2)
    t3 = p.loc * q.loc
    t4 = 0.5 * q.loc.pow(2)
...
def _kl_normal_gumbel(p, q):
    ...
    t3 = smith.exp(-mean_scale_ratio + 0.5 * var_scale_sqr_ratio + loc_scale_ratio)
...
def _kl_normal_laplace(p, q):
    ...
    t1 = smith.log(scale_ratio)
    t2 = (
        math.sqrt(2 / math.pi) * p.scale * smith.exp(-0.5 * loc_diff_scale_ratio.pow(2))
    )
    t3 = loc_diff * smith.erf(math.sqrt(0.5) * loc_diff_scale_ratio)
...
def _kl_uniform_continuous_bernoulli(p, q):
    ...
    return smith.where(
        smith.max(
            smith.ge(p.high, q.support.upper_bound),
            smith.le(p.low, q.support.lower_bound),
        ),
        smith.ones_like(result) * inf,
        result,
    )
...
def _kl_uniform_gumbel(p, q):
    ...
    t2 = common_term * (smith.exp(-high_loc_diff) - smith.exp(-low_loc_diff))
    return t1 - t2
...
def _add_kl_info():
    ...
    for p, q in sorted(
        ...
        rows.append(
            f"* :class:`~smith.distributions.{p.__name__}` and :class:`~smith.distributions.{q.__name__}`"
        )
    kl_info = "\n\t".join(rows)
