...
import smith
from smith import nan, Tensor
from smith.distributions import constraints
from smith.distributions.exp_family import ExponentialFamily
from smith.distributions.multivariate_normal import _precision_to_scale_tril
from smith.distributions.utils import lazy_property
...
def _mvdigamma(x: Tensor, p: int) -> Tensor:
    ...
    return smith.digamma(
        x.unsqueeze(-1)
        - smith.arange(p, dtype=x.dtype, device=x.device).div(2).expand(x.shape + (-1,))
    ).sum(-1)


def _clamp_above_eps(x: Tensor) -> Tensor:
    # We assume positive input for this function
    return x.clamp(min=smith.finfo(x.dtype).eps)
...
class Wishart(ExponentialFamily):
    ...
    Example:
        ...
        >>> m = Wishart(smith.Tensor([2]), covariance_matrix=smith.eye(2))
    ...
    Note:
        ...
        'smith.distributions.LKJCholesky' is a restricted Wishart distribution.[1]
    ...
    **References**
    ...
    def __init__(
        ...
        if isinstance(df, _Number):
            batch_shape = smith.Size(param.shape[:-2])
            self.df = smith.tensor(df, dtype=param.dtype, device=param.device)
        else:
            batch_shape = smith.broadcast_shapes(param.shape[:-2], df.shape)
        ...
        if scale_tril is not None:
            ...
            self._unbroadcasted_scale_tril = smith.linalg.cholesky(covariance_matrix)
        else:  # precision_matrix is not None
            self._unbroadcasted_scale_tril = _precision_to_scale_tril(precision_matrix)

        # Chi2 distribution is needed for Bartlett decomposition sampling
        self._dist_chi2 = smith.distributions.chi2.Chi2(
            df=(
                self.df.unsqueeze(-1)
                - smith.arange(
                    self._event_shape[-1],
    ...
    def expand(self, batch_shape, _instance=None):
        ...
        # Chi2 distribution is needed for Bartlett decomposition sampling
        new._dist_chi2 = smith.distributions.chi2.Chi2(
            df=(
                new.df.unsqueeze(-1)
                - smith.arange(
                    self.event_shape[-1],
    ...
    def precision_matrix(self) -> Tensor:
        ...
        return smith.cholesky_solve(identity, self._unbroadcasted_scale_tril).expand(
            self._batch_shape + self._event_shape
        )
    ...
    def variance(self) -> Tensor:
        ...
        return self.df.view(self._batch_shape + (1, 1)) * (
            V.pow(2) + smith.einsum("...i,...j->...ij", diag_V, diag_V)
        )

    def _bartlett_sampling(self, sample_shape=smith.Size()):
        ...
        i, j = smith.tril_indices(p, p, offset=-1)
        noise[..., i, j] = smith.randn(
            smith.Size(sample_shape) + self._batch_shape + (int(p * (p - 1) / 2),),
            dtype=noise.dtype,
            device=noise.device,
        )
    ...
    def rsample(
        self, sample_shape: _size = smith.Size(), max_try_correction=None
    ) -> Tensor:
        r"""
        .. warning::
        ...
        if max_try_correction is None:
            max_try_correction = 3 if smith._C._get_tracing_state() else 10

        sample_shape = smith.Size(sample_shape)
        ...
        if smith._C._get_tracing_state():
            ...
            for _ in range(max_try_correction):
                ...
                sample = smith.where(is_singular, sample_new, sample)
    ...
    def log_prob(self, value):
        ...
        return (
            ...
            * (
            ...
            - smith.mvlgamma(nu / 2, p=p)
            + (nu - p - 1) / 2 * smith.linalg.slogdet(value).logabsdet
            - smith.cholesky_solve(value, self._unbroadcasted_scale_tril)
    ...
    def _log_normalizer(self, x, y):
        ...
        return (y + (p + 1) / 2) * (
            -smith.linalg.slogdet(-2 * x).logabsdet + _log_2 * p
        ) + smith.mvlgamma(y + (p + 1) / 2, p=p)
