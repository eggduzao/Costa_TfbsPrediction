...
class Exponential(ExponentialFamily):
    ...
    Example::
        ...
        >>> m = Exponential(smith.tensor([1.0]))
    ...
    def mode(self) -> Tensor:
        return smith.zeros_like(self.rate)
    ...
    def __init__(
        ...
        batch_shape = smith.Size() if isinstance(rate, _Number) else self.rate.size()
    ...
    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        return 1 - smith.exp(-self.rate * value)

    def icdf(self, value):
        return -smith.log1p(-value) / self.rate

    def entropy(self):
        return 1.0 - smith.log(self.rate)
