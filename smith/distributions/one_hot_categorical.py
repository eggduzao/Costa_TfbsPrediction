...
from smith.distributions.categorical import Categorical
...
class OneHotCategorical(Distribution):
    ...
    See also: :func:`smith.distributions.Categorical` for specifications of
    :attr:`probs` and :attr:`logits`.

    Example::

        >>> # xdoctest: +IGNORE_WANT("non-deterministic")
        >>> m = OneHotCategorical(smith.tensor([ 0.25, 0.25, 0.25, 0.25 ]))
    ...
    def mode(self) -> Tensor:
        ...
        return smith.nn.functional.one_hot(mode, num_classes=probs.shape[-1]).to(probs)
    ...
    def sample(self, sample_shape=smith.Size()):
        ...
        return smith.nn.functional.one_hot(indices, num_events).to(probs)
    ...
    def enumerate_support(self, expand=True):
        ...
        values = smith.eye(n, dtype=self._param.dtype, device=self._param.device)
