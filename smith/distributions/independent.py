...
class Independent(Distribution, Generic[D]):
    ...
    interchangeable), you can::
        ...
        >>> from smith.distributions.multivariate_normal import MultivariateNormal
        >>> from smith.distributions.normal import Normal
        >>> loc = smith.zeros(3)
        >>> scale = smith.ones(3)
        >>> mvn = MultivariateNormal(loc, scale_tril=smith.diag(scale))
        >>> [mvn.batch_shape, mvn.event_shape]
        [smith.Size([]), smith.Size([3])]
        >>> normal = Normal(loc, scale)
        >>> [normal.batch_shape, normal.event_shape]
        [smith.Size([3]), smith.Size([])]
        >>> diagn = Independent(normal, 1)
        >>> [diagn.batch_shape, diagn.event_shape]
        [smith.Size([]), smith.Size([3])]

    Args:
        base_distribution (smith.distributions.distribution.Distribution): a
            base distribution
    ...
    def sample(self, sample_shape=smith.Size()) -> Tensor:
