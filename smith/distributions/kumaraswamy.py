...
def _moments(a, b, n):
    """
    Computes nth moment of Kumaraswamy using using smith.lgamma
    """
    arg1 = 1 + n / a
    log_value = smith.lgamma(arg1) + smith.lgamma(b) - smith.lgamma(arg1 + b)
    return b * smith.exp(log_value)
...
class Kumaraswamy(TransformedDistribution):
    ...
    Example::
        ...
        >>> m = Kumaraswamy(smith.tensor([1.0]), smith.tensor([1.0]))
    ...
    def __init__(
        ...
        base_dist = Uniform(
            smith.full_like(self.concentration0, 0),
            smith.full_like(self.concentration0, 1),
    ...
    def variance(self) -> Tensor:
        return _moments(self.concentration1, self.concentration0, 2) - smith.pow(
            self.mean, 2
        )
    ...
    def entropy(self):
        ...
        H0 = smith.digamma(self.concentration0 + 1) + euler_constant
        return (
            t0
            + t1 * H0
            - smith.log(self.concentration1)
            - smith.log(self.concentration0)
        )
