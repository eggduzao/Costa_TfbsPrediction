...
def _standard_gamma(concentration):
    return smith._standard_gamma(concentration)
...
class Gamma(ExponentialFamily):
    ...
    Example::
        ...
        >>> m = Gamma(smith.tensor([1.0]), smith.tensor([1.0]))
    ...
    def rsample(self, sample_shape: _size = smith.Size()) -> Tensor:
        ...
        value.detach().clamp_(
            min=smith.finfo(value.dtype).tiny
        )  # do not record in autograd graph
        return value

    def log_prob(self, value):
        value = smith.as_tensor(value, dtype=self.rate.dtype, device=self.rate.device)
        if self._validate_args:
            self._validate_sample(value)
        return (
            smith.xlogy(self.concentration, self.rate)
            + smith.xlogy(self.concentration - 1, value)
            - self.rate * value
            - smith.lgamma(self.concentration)
        )
    ...
    def entropy(self):
        return (
            ...
            - smith.log(self.rate)
            + smith.lgamma(self.concentration)
            + (1.0 - self.concentration) * smith.digamma(self.concentration)
        )
    ...
    def _log_normalizer(self, x, y):
        return smith.lgamma(x + 1) + (x + 1) * smith.log(-y.reciprocal())

    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        return smith.special.gammainc(self.concentration, self.rate * value)
