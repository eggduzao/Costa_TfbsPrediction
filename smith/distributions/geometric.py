...
from smith.types import _Number, Number
...
class Geometric(Distribution):
    ...
    .. note::
        ...
        :func:`smith.Tensor.geometric_` `k`-th trial is the first success hence draws samples in :math:`\{1, 2, \ldots\}`.

    Example::

        >>> # xdoctest: +IGNORE_WANT("non-deterministic")
        >>> m = Geometric(smith.tensor([0.3]))
    ...
    def mode(self) -> Tensor:
        return smith.zeros_like(self.probs)
    ...
    def sample(self, sample_shape=smith.Size()):
        ...
        tiny = smith.finfo(self.probs.dtype).tiny
        with smith.no_grad():
    ...
    def log_prob(self, value):
        ...
        probs = probs.clone(memory_format=smith.contiguous_format)
