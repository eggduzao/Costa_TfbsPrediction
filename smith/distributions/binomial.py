...
class Binomial(Distribution):
    ...
    Example::
        ...
        >>> m = Binomial(100, smith.tensor([0 , .2, .8, 1]))
        >>> x = m.sample()
        tensor([   0.,   22.,   71.,  100.])

        >>> m = Binomial(smith.tensor([[5.], [10.]]), smith.tensor([0.5, 0.8]))
        >>> x = m.sample()
        tensor([[ 4.,  5.],
                [ 7.,  6.]])
    ...
    def log_prob(self, value):
        ...
        log_factorial_n = smith.lgamma(self.total_count + 1)
        log_factorial_k = smith.lgamma(value + 1)
        log_factorial_nmk = smith.lgamma(self.total_count - value + 1)
        ...
        normalize_term = (
            ...
            + self.total_count * smith.log1p(smith.exp(-smith.abs(self.logits)))
            - log_factorial_n
        )
        return (
    ...
    def entropy(self):
        ...
        return -(smith.exp(log_prob) * log_prob).sum(0)
