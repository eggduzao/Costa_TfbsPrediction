...
def _Dirichlet_backward(x, concentration, grad_output):
    ...
    grad = smith._dirichlet_grad(x, concentration, total)
...
class _Dirichlet(Function):
    ...
    def forward(ctx, concentration):
        x = smith._sample_dirichlet(concentration)
...
class Dirichlet(ExponentialFamily):
    ...
    Example::
        ...
        >>> m = Dirichlet(smith.tensor([0.5, 0.5]))
    ...
    def log_prob(self, value):
        ...
        return (
            smith.xlogy(self.concentration - 1.0, value).sum(-1)
            + smith.lgamma(self.concentration.sum(-1))
            - smith.lgamma(self.concentration).sum(-1)
        )
    ...
    def mode(self) -> Tensor:
        ...
        mode[mask] = smith.nn.functional.one_hot(
    ...
    def entropy(self):
        ...
        return (
            smith.lgamma(self.concentration).sum(-1)
            - smith.lgamma(a0)
            - (k - a0) * smith.digamma(a0)
            - ((self.concentration - 1.0) * smith.digamma(self.concentration)).sum(-1)
        )
    ...
    def _log_normalizer(self, x):
        return x.lgamma().sum(-1) - smith.lgamma(x.sum(-1))
