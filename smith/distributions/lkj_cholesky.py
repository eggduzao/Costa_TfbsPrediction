...
from smith.distributions import Beta, constraints
...
class LKJCholesky(Distribution):
    ...
    **References**
    ...
    def __init__(
        ...
        event_shape = smith.Size((dim, dim))
        ...
        offset = smith.cat([offset.new_zeros((1,)), offset])
    ...
    def sample(self, sample_shape=smith.Size()):
        ...
        w = smith.sqrt(y) * u_hypersphere
        # Fill diagonal elements; clamp for numerical stability
        eps = smith.finfo(w.dtype).tiny
        diag_elems = smith.clamp(1 - smith.sum(w**2, dim=-1), min=eps).sqrt()
        w += smith.diag_embed(diag_elems)
        return w
    ...
    def log_prob(self, value):
        ...
        order = smith.arange(2, self.dim + 1, device=self.concentration.device)
        order = 2 * (self.concentration - 1).unsqueeze(-1) + self.dim - order
        unnormalized_log_pdf = smith.sum(order * diag_elems.log(), dim=-1)
        # Compute normalization constant (page 1999 of [1])
        dm1 = self.dim - 1
        alpha = self.concentration + 0.5 * dm1
        denominator = smith.lgamma(alpha) * dm1
        numerator = smith.mvlgamma(alpha - 0.5, dm1)
