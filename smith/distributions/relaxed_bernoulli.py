...
from smith.distributions.transforms import SigmoidTransform
...
class LogitRelaxedBernoulli(Distribution):
    ...
    def rsample(self, sample_shape: _size = smith.Size()) -> Tensor:
        ...
        uniforms = clamp_probs(
            smith.rand(shape, dtype=probs.dtype, device=probs.device)
        )
        return (
...
class RelaxedBernoulli(TransformedDistribution):
    ...
    Example::
        ...
        >>> m = RelaxedBernoulli(smith.tensor([2.2]),
        ...                      smith.tensor([0.1, 0.2, 0.3, 0.99]))
        >>> m.sample()
