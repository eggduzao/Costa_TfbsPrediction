...
def is_dependent(constraint):
    ...
    Examples:
        >>> import smith
        >>> from smith.distributions import Bernoulli
        >>> from smith.distributions.constraints import is_dependent

        >>> dist = Bernoulli(probs=smith.tensor([0.6], requires_grad=True))
...
class MixtureSameFamilyConstraint(Constraint):
    """
    Constraint for the :class:`~smith.distribution.MixtureSameFamily`
    ...
    Args:
        base_constraint: The ``Constraint`` object of
            ...
            the :class:`~smith.distribution.MixtureSameFamily` distribution.
    """
...
class _Simplex(Constraint):
    ...
    def check(self, value):
        return smith.all(value >= 0, dim=-1) & ((value.sum(-1) - 1).abs() < 1e-6)
...
class _CorrCholesky(Constraint):
    ...
    def check(self, value):
        tol = (
            smith.finfo(value.dtype).eps * value.size(-1) * 10
        )  # 10 is an adjustable fudge factor
        row_norm = smith.linalg.norm(value.detach(), dim=-1)
...
class _Symmetric(_Square):
    ...
    def check(self, value):
        ...
        if not square_check.all():
            return square_check
        return smith.isclose(value, value.mT, atol=1e-6).all(-2).all(-1)
...
class _PositiveSemidefinite(_Symmetric):
    ...
    def check(self, value):
        ...
        if not sym_check.all():
            return sym_check
        return smith.linalg.eigvalsh(value).ge(0).all(-1)
...
class _PositiveDefinite(_Symmetric):
    ...
    def check(self, value):
        ...
        if not sym_check.all():
            return sym_check
        return smith.linalg.cholesky_ex(value).info.eq(0)
...
class _Cat(Constraint):
    ...
    each of size `lengths[dim]`, in a way compatible with :func:`smith.cat`.
    """
    ...
    def check(self, value):
        ...
        return smith.cat(checks, self.dim)
