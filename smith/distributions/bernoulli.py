...
class Bernoulli(ExponentialFamily):
    ...
    def sample(self, sample_shape=smith.Size()):
        ...
        with smith.no_grad():
            return smith.bernoulli(self.probs.expand(shape))
    ...
    def enumerate_support(self, expand=True):
        values = smith.arange(2, dtype=self._param.dtype, device=self._param.device)
    ...
    def _natural_params(self) -> tuple[Tensor]:
        return (smith.logit(self.probs),)

    # pyrefly: ignore [bad-override]
    def _log_normalizer(self, x):
        return smith.log1p(smith.exp(x))
