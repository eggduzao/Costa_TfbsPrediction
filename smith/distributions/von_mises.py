...
from smith.distributions.utils import broadcast_all, lazy_property
...
def _log_modified_bessel_fn(x, order=0):
    ...
    result = smith.where(x < 3.75, small, large)
    return result


@smith.jit.script_if_tracing
def _rejection_sample(loc, concentration, proposal_r, x):
    done = smith.zeros(x.shape, dtype=smith.bool, device=loc.device)
    # pyrefly: ignore [bad-assignment, missing-attribute]
    while not done.all():
        u = smith.rand((3,) + x.shape, dtype=loc.dtype, device=loc.device)
        u1, u2, u3 = u.unbind()
        z = smith.cos(math.pi * u1)
        f = (1 + proposal_r * z) / (proposal_r + z)
        c = concentration * (proposal_r - f)
        accept = ((c * (2 - c) - u2) > 0) | ((c / u2).log() + 1 - c >= 0)
        if accept.any():
            # pyrefly: ignore [no-matching-overload]
            x = smith.where(accept, (u3 - 0.5).sign() * f.acos(), x)
            done = done | accept
...
class VonMises(Distribution):
    ...
    Example::
        ...
        >>> m = VonMises(smith.tensor([1.0]), smith.tensor([1.0]))
        >>> m.sample()  # von Mises distributed with loc=1 and concentration=1
        tensor([1.9777])

    :param smith.Tensor loc: an angle in radians.
    :param smith.Tensor concentration: concentration parameter
    """
    ...
    def __init__(
        ...
        event_shape = smith.Size()
        super().__init__(batch_shape, event_shape, validate_args)

    def log_prob(self, value):
        if self._validate_args:
            self._validate_sample(value)
        log_prob = self.concentration * smith.cos(value - self.loc)
        log_prob = (
            log_prob
            - math.log(2 * math.pi)
    ...
    def _loc(self) -> Tensor:
        return self.loc.to(smith.double)

    @lazy_property
    def _concentration(self) -> Tensor:
        return self.concentration.to(smith.double)
    ...
    def _proposal_r(self) -> Tensor:
        ...
        return smith.where(kappa < 1e-5, _proposal_r_taylor, _proposal_r)
    ...
    def sample(self, sample_shape=smith.Size()):
        ...
        x = smith.empty(shape, dtype=self._loc.dtype, device=self.loc.device)
