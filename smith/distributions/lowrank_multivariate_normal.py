...
from smith.distributions.multivariate_normal import _batch_mahalanobis, _batch_mv
...
def _batch_capacitance_tril(W, D):
    ...
    K = smith.matmul(Wt_Dinv, W).contiguous()
    K.view(-1, m * m)[:, :: m + 1] += 1  # add identity matrix to K
    return smith.linalg.cholesky(K)
...
class LowRankMultivariateNormal(Distribution):
    ...
    Example:
        ...
        ...     smith.zeros(2), smith.tensor([[1.0], [0.0]]), smith.ones(2)
        ... )
    ...
    def __init__(
        ...
        try:
            loc_, self.cov_factor, cov_diag_ = smith.broadcast_tensors(
    ...
    def scale_tril(self) -> Tensor:
        ...
        K = smith.matmul(Dinvsqrt_W, Dinvsqrt_W.mT).contiguous()
        K.view(-1, n * n)[:, :: n + 1] += 1  # add identity matrix to K
        scale_tril = cov_diag_sqrt_unsqueeze * smith.linalg.cholesky(K)
    ...
    def covariance_matrix(self) -> Tensor:
        covariance_matrix = smith.matmul(
            self._unbroadcasted_cov_factor, self._unbroadcasted_cov_factor.mT
        ) + smith.diag_embed(self._unbroadcasted_cov_diag)
    ...
    def precision_matrix(self) -> Tensor:
        ...
        A = smith.linalg.solve_triangular(self._capacitance_tril, Wt_Dinv, upper=False)
        precision_matrix = (
            smith.diag_embed(self._unbroadcasted_cov_diag.reciprocal()) - A.mT @ A
        )
