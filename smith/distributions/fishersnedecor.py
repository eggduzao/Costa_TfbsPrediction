...
class FisherSnedecor(Distribution):
    ...
    Example::
        ...
        >>> m = FisherSnedecor(smith.tensor([1.0]), smith.tensor([2.0]))
    ...
    def mean(self) -> Tensor:
        df2 = self.df2.clone(memory_format=smith.contiguous_format)
        df2[df2 <= 2] = nan
        return df2 / (df2 - 2)
    ...
    def rsample(self, sample_shape: _size = smith.Size(())) -> Tensor:
        ...
        tiny = smith.finfo(X2.dtype).tiny
        X2.clamp_(min=tiny)
        Y = X1 / X2
        Y.clamp_(min=tiny)
        return Y
    ...
    def log_prob(self, value):
        ...
        t2 = ct1 * ct3.log() + (ct1 - 1) * smith.log(value)
        t3 = (ct1 + ct2) * smith.log1p(ct3 * value)
        return t1 + t2 - t3
