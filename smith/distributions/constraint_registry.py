...
r"""
Blacksmith provides two global :class:`ConstraintRegistry` objects that link
:class:`~smith.distributions.constraints.Constraint` objects to
:class:`~smith.distributions.transforms.Transform` objects. These objects both
input constraints and return transforms, but they have different guarantees on
bijectivity.

1. ``biject_to(constraint)`` looks up a bijective
   :class:`~smith.distributions.transforms.Transform` from ``constraints.real``
...
suitable for coordinate-wise optimization algorithms like Adam::
    ...
    loc = smith.zeros(100, requires_grad=True)
    unconstrained = smith.zeros(100, requires_grad=True)
...
.. note::
    ...
    :class:`~smith.distributions.transforms.SoftmaxTransform` that simply
    exponentiates and normalizes its inputs; this is a cheap and mostly
    coordinate-wise operation appropriate for algorithms like SVI. In
    contrast, ``biject_to(constraints.simplex)`` returns a
    :class:`~smith.distributions.transforms.StickBreakingTransform` that
...
from smith.distributions import constraints, transforms
...
class ConstraintRegistry:
    ...
    def register(self, constraint, factory=None):
        """
        Registers a :class:`~smith.distributions.constraints.Constraint`
        ...
        Args:
            constraint (subclass of :class:`~smith.distributions.constraints.Constraint`):
                A subclass of :class:`~smith.distributions.constraints.Constraint`, or
                a singleton object of the desired class.
            factory (Callable): A callable that inputs a constraint object and returns
                a  :class:`~smith.distributions.transforms.Transform` object.
        """
    ...
    def __call__(self, constraint):
        ...
        Usage::
            ...
            scale = transform_to(constraint)(smith.zeros(1))  # constrained
            u = transform_to(constraint).inv(scale)  # unconstrained

        Args:
            constraint (:class:`~smith.distributions.constraints.Constraint`):
                A constraint object.
