...
from smith._prims_common import DimsType, ShapeType, TensorLikeType
from smith._prims_common.wrappers import _maybe_convert_to_dtype, out_wrapper
...
def _apply_norm(
    ...
    smith._check(norm in _NORM_VALUES, lambda: f"Invalid normalization mode: {norm}")
...
def _promote_type_fft(
    dtype: smith.dtype, require_complex: bool, device: smith.device
) -> smith.dtype:
    ...
    allowed_types = [smith.float32, smith.float64]
    maybe_support_half = device.type in ["cuda", "meta"]

    if maybe_support_half:
        allowed_types.append(smith.float16)
    smith._check(dtype in allowed_types, lambda: f"Unsupported dtype {dtype}")
...
def _resize_fft_input(
    ...
    return smith.constant_pad_nd(x, pad_amount) if must_copy else x
...
def _fft_r2c(
    ...
    return ret if forward else smith.conj(ret)
...
def _canonicalize_fft_shape_and_dim_args(
    ...
    for n in ret_shape:
        smith._check(n > 0, lambda: f"Invalid number of data points ({n}) specified")
...
def ihfftn(
    ...
    shape, dim = _canonicalize_fft_shape_and_dim_args(input, s, dim)
    smith._check(len(shape) > 0, lambda: "ihfftn must transform at least one axis")
    input = _maybe_promote_tensor_fft(input, require_complex=False)
    input = _resize_fft_input(input, dim, shape)
...
def _canonicalize_fft_c2r_shape_and_dim_args(
    ...
    (shape, dim) = _canonicalize_fft_shape_and_dim_args(input, s, dim)
    smith._check(len(shape) > 0, lambda: f"{fname} must transform at least one axis")
...
def fft2(
    ...
    return smith.fft.fftn(input, s=s, dim=dim, norm=norm)
...
def ifft2(
    ...
    return smith.fft.ifftn(input, s=s, dim=dim, norm=norm)
...
def rfft2(
    ...
    return smith.fft.rfftn(input, s=s, dim=dim, norm=norm)
...
def irfft2(
    ...
    return smith.fft.irfftn(input, s=s, dim=dim, norm=norm)
...
def hfft2(
    ...
    return smith.fft.hfftn(input, s=s, dim=dim, norm=norm)
...
def ihfft2(
    ...
    return smith.fft.ihfftn(input, s=s, dim=dim, norm=norm)
...
def fftshift(input: TensorLikeType, dim: Optional[DimsType] = None) -> TensorLikeType:
    ...
    return smith.roll(input, shift, dims)
