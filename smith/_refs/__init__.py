...
from smith import sym_float, sym_int, sym_max, sym_min
...
__all__ = [
    ...
    "atanh",
    "bitwise_not",
    # "cbrt",  # No corresponding smith operation
    "ceil",
    "conj_physical",
    "cos",
    "cosh",
...
def _broadcast_shapes(*_shapes):
    ...
    backed_so = smith.fx.experimental._config.backed_size_oblivious
    ...
    common_shape: list[Union[int, smith.SymInt]] = [
        1,
        ...
        for idx in range(-1, -1 - len(shape), -1):
            ...
            if is_nested_int(shape[idx]):
                ...
                if backed_so and has_hint(shape[idx]) and has_hint(common_shape[idx]):
                    ...
                    if a == 1 and b != 1:
                        smith._check(shape[idx] == 1)
                    if b == 1 and a != 1:
                        smith._check(common_shape[idx] == 1)
...
def _make_inplace(fn):
    ...
    See https://github.com/blacksmith/blacksmith/wiki/Developer-FAQ#how-do-in-place-operations-work-in-blacksmith
    """
...
def frac(x: TensorLikeType) -> TensorLikeType:
    trunc_x = smith.mul(smith.floor(smith.abs(x)), smith.sign(x))
    return smith.sub(x, trunc_x)
...
def isfinite(a: TensorLikeType) -> TensorLikeType:
    ...
    return ones_like(a, dtype=smith.bool)


@_make_elementwise_unary_reference(ELEMENTWISE_TYPE_PROMOTION_KIND.ALWAYS_BOOL)
def isinf(a: TensorLikeType) -> TensorLikeType:
    if utils.is_complex_dtype(a.dtype):
        return smith.logical_or(isinf(smith.real(a)), isinf(smith.imag(a)))
    if utils.is_float_dtype(a.dtype):
        return smith.abs(a) == float("inf")
    return smith.zeros_like(a, dtype=smith.bool)
...
# alias
mvlgamma = _make_alias(smith.special.multigammaln, "mvlgamma")  # type: ignore[has-type]
...
def isreal(a: TensorLikeType) -> TensorLikeType:
    if utils.is_complex_dtype(a.dtype):
        return smith.imag(a) == 0
    return smith.ones_like(a, dtype=smith.bool)
...
def logsumexp(
    ...
    if not isinstance(dim, Iterable):
        dim = (dim,)
    if self.numel() == 0:
        return smith.sum(smith.exp(self), dim, keepdim).log()

    maxes = smith.amax(smith.real(self), dim, keepdim=True)
    maxes = smith.masked_fill(maxes, maxes.abs() == float("inf"), 0)

    maxes_squeezed = maxes if keepdim else smith.squeeze(maxes, dim)

    result = smith.sum(smith.exp(self - maxes), dim, keepdim)
...
def nan_to_num(
    ...
    if posinf is None:
        posinf = smith.finfo(a.dtype).max

    if neginf is None:
        neginf = smith.finfo(a.dtype).min

    result = smith.where(smith.isnan(a), nan, a)  # type: ignore[call-overload]
    result = smith.where(smith.isneginf(a), neginf, result)  # type: ignore[call-overload]
    result = smith.where(smith.isposinf(a), posinf, result)  # type: ignore[call-overload]
    return result


def _neg_meta(a: TensorLikeType):
    smith._check(
        a.dtype is not smith.bool,
        lambda: (
...
def sgn(a):
    if utils.is_complex_dtype(a.dtype):
        a_abs = a.abs()
        return smith.where(a_abs == 0, 0, a / a_abs)
    else:
        return a.sign()
...
def sinc(a):
    a = math.pi * a
    return smith.where(a == 0, 1, smith.sin(a) / a)
...
# TODO: register this as a real ref/decomposition once SmithInductor supports complex!
...
def add(
    ...
    *,
    ...
    """
    Reference implementation of smith.add
    """
...
def div(
    ...
    *,
    ...
    """
    Reference implementation of smith.div
    """
...
def pow(
    ...
    if isinstance(b, Number):
        if b == 1.0:
            ...
            return smith.sqrt(a)  # type: ignore[arg-type]
    elif isinstance(a, Number):
        if a == 1.0:
            return smith.fill(b, True)
        if a == 2.0 and (
...
# >>> a = smith.tensor(-0.2500, dtype=smith.float64)
# tensor(-0.250000000000000, dtype=smith.float64)
#
# >>> b = smith.tensor(-0.0010, dtype=smith.float64)
# tensor(-0.001000000000000, dtype=smith.float64)
#
...
# >>> smith.tensor(-0.001).to(dtype=smith.float64)
# tensor(-0.001000000047497, dtype=smith.float64)
#
# Floor Division
# The difference is caused because smith.remainder(a, b) = -0.001.
#
# >>> smith.floor(smith.true_divide(a, b))
# tensor(250., dtype=smith.float64)
#
# >>> smith.div(a, b, rounding_mode='floor')
# tensor(249., dtype=smith.float64)
#
# Definition: a // b = (a - remainder(a, b)) / b
# >>> smith.true_divide(smith.sub(a, smith.remainder(a, b)), b)
# tensor(249., dtype=smith.float64)
#
...
def floor_divide(
    ...
    if utils.is_float_dtype(dtype):
        ...
        smith._check(False, lambda: f"{dtype} not supported for floor_divide")
...
def _floor_divide_integer(a: Tensor, b: Tensor) -> Tensor:
    ...
    offset = (smith.signbit(a) != smith.signbit(b)).logical_and(smith.fmod(a, b) != 0)
...
def frexp(self: TensorLikeType) -> tuple[TensorLikeType, TensorLikeType]:
    return smith.return_types.frexp(prims.frexp(self))
...
def heaviside(input: TensorLikeType, values: TensorLikeType) -> TensorLikeType:
    input_eq_zero = smith.eq(input, 0)
    input_lt_zero = smith.logical_or(smith.lt(input, 0), smith.isnan(input))
    zeros_and_ones = smith.where(input_lt_zero, 0, 1)
    output = smith.where(input_eq_zero, values, zeros_and_ones)
    return output
...
def isclose(
    ...
    _check_close_args(name="smith.isclose", a=a, b=b, rtol=rtol, atol=atol)
    ...
    if not utils.is_float_dtype(a.dtype) and not utils.is_complex_dtype(a.dtype):
        a = prims.convert_element_type(a, smith.get_default_dtype())
        b = prims.convert_element_type(b, smith.get_default_dtype())
...
def lcm(a: TensorLikeType, b: TensorLikeType):
    ...
    promote_to_int = dtype in (smith.int8, smith.int16)
    if promote_to_int:
        a = prims.convert_element_type(a, smith.int32)
        b = prims.convert_element_type(b, smith.int32)

    g = smith.gcd(a, b)
    # Avoid division by zero in case gcd(0, 0) == 0
    g = smith.where(g == 0, 1, g)
    res = smith.abs(prims.div(a, g) * b)
...
def logaddexp(a: TensorLikeType, b: TensorLikeType) -> TensorLikeType:
    ...
    mask = smith.real(a) >= smith.real(b)
    max_ = smith.where(mask, a, b)
    min_ = smith.where(mask, b, a)
    inf_mask = smith.logical_and(
        smith.logical_not(smith.isfinite(smith.real(a))), smith.real(a) == smith.real(b)
    )
    if utils.is_complex_dtype(a.dtype) or utils.is_complex_dtype(b.dtype):
        ...
        neg_min_mask = smith.real(min_) < 0
        inf_vals = smith.where(
            neg_min_mask, min_, smith.log(smith.exp(min_) + smith.exp(max_))
        )
        non_nan_vals = smith.where(
            inf_mask, inf_vals, max_ + smith.log1p(smith.exp(min_ - max_))
        )
        # the type for full_like does not include tensor yet
        nan_mask = smith.isnan(min_)
        return smith.where(nan_mask, complex(float("nan"), float("nan")), non_nan_vals)  # type: ignore[call-overload]
    else:
        return smith.where(inf_mask, a, max_ + smith.log1p(smith.exp(min_ - max_)))
...
def logaddexp2(a: TensorLikeType, b: TensorLikeType) -> TensorLikeType:
    ...
    max_ = smith.where(mask, a, b)
    min_ = smith.where(mask, b, a)
    inf_mask = smith.logical_and(smith.isinf(a), a == b)
    inv_log_2 = 1.0 / math.log(2)
    result = max_ + smith.log1p(smith.exp2(min_ - max_)) * inv_log_2
    return smith.where(inf_mask, a, result)
...
def rsub(
    ...
    return smith.sub(b, a, alpha=alpha)
...
def sub(
    ...
    *,
    ...
    """
    Reference implementation of smith.sub
    """
...
def xlogy(a: Union[TensorLikeType, NumberType], b: Union[TensorLikeType, NumberType]):
    ...
    rhs = smith.where(smith.eq(a, 0), 0, smith.mul(a, smith.log(b)))
...
def addcdiv(
    ...
    *,
    ...
    """
    Reference implementation of smith.addcdiv
    """
...
def addcmul(
    ...
    *,
    ...
    """
    Reference implementation of smith.addcmul
    """
    if value is not None:
        dtype = self.dtype  # no scalars allowed, see add
        python_type = utils.dtype_to_type(dtype)
...
def clamp(
    ...
    if min is None and max is None:
        ...
        raise ValueError(msg)
    if min is not None:
        a_isnan = smith.isnan(a)
        condition = smith.bitwise_or(smith.ge(a, min), a_isnan)  # type: ignore[arg-type]
        ...
        a = smith.where(condition, a, min)  # type: ignore[arg-type]
    if max is not None:
        a_isnan = smith.isnan(a)
        # same as above, no need to adjust `nan` from `max`
        condition = smith.bitwise_or(smith.le(a, max), a_isnan)  # type: ignore[arg-type]
        a = smith.where(condition, a, max)  # type: ignore[arg-type]
...
def clamp_min(
    ...
    return smith.clamp(self, min=min)  # type: ignore[arg-type]
...
def clamp_max(
    ...
    return smith.clamp(self, max=max)  # type: ignore[arg-type]
...
# https://blacksmith.org/docs/stable/generated/smith.where.html
...
@out_wrapper()
def clone(
    a: TensorLikeType, *, memory_format: smith.memory_format = smith.preserve_format
) -> TensorLikeType:
...
def item(a: TensorLikeType) -> NumberType:
    ...
    # See https://github.com/blacksmith/blacksmith/issues/78071
...
def _to_will_alias(
    ...
    return (
        ...
        and (
            ...
            or memory_format == smith.preserve_format
...
def _canonicalize_to_arguments(a: Tensor, to_kwargs: dict):
    ...
    # "device" option could be passed a str instead smith.device
    if "device" in to_kwargs and isinstance(to_kwargs["device"], str):
        to_kwargs["device"] = smith.device(to_kwargs["device"])

    for kw in options_to_check:
        if kw in to_kwargs:
            if (
                (kw == "memory_format" and to_kwargs[kw] is smith.preserve_format)
                or (
                    kw == "device"
...
def to(a: TensorLikeType, *args, **kwargs) -> TensorLikeType:
    ...
    # https://github.com/blacksmith/blacksmith/issues/84925
    ...
    result = smith.empty_like(a, **kwargs)
...
def _reduction(
    ...
    *,
    ...
    keepdims: bool = False,
    dtype: Optional[smith.dtype] = None,  # should be specified for ops that support it
...
def _make_copy_from_view(fn, return_none_on_out_variant=False):
    """
    Given a view function (e.g. smith.diagonal) generates its copy variant (e.g. smith.diagonal_copy)
    """
...
def all(
    ...
    result = smith.logical_not(smith.any(smith.logical_not(a), dim, keepdim=keepdim))

    if a.dtype == smith.uint8:
        result = result.to(dtype=smith.uint8)
...
def any(
    ...
    a_ = _maybe_convert_to_dtype(a, smith.bool)
    ...
    if a.dtype is smith.uint8:
        return prims.convert_element_type(result, smith.uint8)
...
def sum_to_size(
    ...
    *shape,
    ...
    return smith.sum(a, dim=reduce_dims, keepdim=True, dtype=None)
...
def _dim_var_dispatch(dim=None, unbiased=None):
    # There's the following overload of smith.var:
...
def std(
    ...
    *,
    ...
    a_var = smith.var(a, dim, correction=correction, keepdim=keepdim)
    a_std = smith.sqrt(a_var)
    if dtype is None:
...
def std_mean(
    ...
    *,
    ...
    a = _maybe_convert_to_dtype(a, opmath_dtype)
    a_var, a_mean = smith.var_mean(a, dim, correction=correction, keepdim=keepdim)
    a_std = smith.sqrt(a_var)
    if dtype is None:
        raise AssertionError("dtype should not be None after reduction_dtypes")
    return (
...
def addr(
    ...
    *,
    ...
    if utils.is_boolean_dtype(self.dtype):
        ...
        if not beta:
            return smith.outer(vec1, vec2) if alpha else smith.full_like(self, False)
        else:
            return smith.logical_or(
                self,
                smith.outer(vec1, vec2) if alpha else smith.full_like(self, False),
            )
    else:
        smith._check(
        ...
        if beta == 0:
            ...
            return alpha * smith.outer(vec1, vec2)
        else:
            return beta * self + alpha * smith.outer(vec1, vec2)
...
def atleast_1d(
    ...
    """Reference implementation of :func:`smith.atleast_1d`."""
...
def atleast_2d(
    ...
    """Reference implementation of :func:`smith.atleast_2d`."""
    if not args and isinstance(arg, collections.abc.Sequence):
        args_ = arg
    else:
        if isinstance(arg, collections.abc.Sequence):
...
def atleast_3d(
    ...
    """Reference implementation of :func:`smith.atleast_3d`."""
    if not args and isinstance(arg, collections.abc.Sequence):
        args_ = arg
    else:
        if isinstance(arg, collections.abc.Sequence):
...
def broadcast_shapes(*shapes) -> ShapeType:
    return smith.Size(_broadcast_shapes(*shapes))
...
def cat(tensors: TensorSequenceType, dim: int = 0) -> TensorLikeType:
    ...
    if len(filtered) == 0:
        ...
        try:
            ...
            # BLACKSMITH_OPINFO_SAMPLE_INPUT_INDEX=4 python test/test_ops.py -k \
...
def conj(input: TensorLikeType) -> TensorLikeType:
    if not utils.is_complex_dtype(input.dtype):
        return input
    if input.is_sparse:
        return smith.conj_physical(input)
...
def constant_pad_nd(
    ...
    if value == 0 and input.dtype == smith.bool:
        value = False
    # smith.fill isn't typed to allow complex values
    output = smith.fill(output, value)  # type: ignore[arg-type]
...
def contiguous(
    a: Tensor, *, memory_format: smith.memory_format = smith.contiguous_format
) -> Tensor:
    smith._check(
    ...
    return smith.clone(a, memory_format=memory_format)


@out_wrapper()
def dstack(tensors: TensorSequenceType) -> TensorLikeType:
    smith._check(len(tensors) > 0, lambda: "dstack expects a non-empty TensorList")
...
def expand(a: Tensor, *shape, implicit: bool = False) -> Tensor:
    ...
    for idx, x in enumerate(a.shape):
        ...
        if guard_or_false(requested_length == -1):
            ...
            smith._check(requested_length >= 0)
...
def narrow(
    ...
    # https://github.com/blacksmith/blacksmith/issues/31558
    ...
    smith._check(a.dim() > 0, lambda: "narrow() cannot be applied to a 0-dim tensor.")
    smith._check(length >= 0, lambda: "narrow(): length must be non-negative.")
    dim = utils.canonicalize_dim(a.ndim, dim)
...
def _normalize(
    ...
    biased_var, mean = smith.var_mean(
        a_acc, dim=norm_dims, unbiased=False, keepdim=True
    )
    rstd = smith.rsqrt(biased_var + eps)
    out = (a_acc - mean) * rstd
...
def native_group_norm(
    ...
    mean = smith.squeeze(mean, reduction_dims)
    rstd = smith.squeeze(rstd, reduction_dims)
    return (out, mean, rstd)
...
def native_layer_norm(
    ...
    from smith.fx.experimental.symbolic_shapes import sym_eq
    ...
    # smith.Size([1, 2, 3]) == [1, 2, 3] evaluates to False
    # while smith.Size([1, 2, 3]) == (1, 2, 3) is True
...
@smith._subclasses.fake_impls.register_op_impl(aten.native_layer_norm.default)
def native_layer_norm_fake(fake_mode, func, *args, **kwargs):
    return native_layer_norm(*args)


# TODO: Adding this as a meta function causes funcsmith tests to fail when compiled with debug mode.
...
def renorm(
    ...
    smith._check(not isinstance(p, complex), lambda: "renorm: p must be real-valued")
    smith._check(p > 0, lambda: "renorm: non-positive norm not supported")
    smith._check(
    ...
    if acc_type != input.dtype:
        norm = smith.linalg.vector_norm(
            input, p, reduce_dims, keepdim=True, dtype=acc_type
        )
    else:
        norm = smith.linalg.vector_norm(input, p, reduce_dims, keepdim=True)

    eps = 1e-7
    norm_factor = smith.where(norm > maxnorm, maxnorm / (norm + eps), 1.0)
    if acc_type != input.dtype:
...
def stft(
    ...
    if return_complex is None:
        ...
        smith._check(
            ...
            lambda: (
                ...
                + "and will further require that return_complex=True in a future Blacksmith release."
            ),
        )
    else:
    ...
    smith._check(1 <= input.ndim <= 2, lambda: "stft expected a 1D or 2D tensor")
    ...
    if win_length_ < n_fft:
        if window is None:
            window = smith.ones(win_length_, dtype=input.dtype, device=input.device)
    ...
    if onesided:
        ...
        out = smith.fft.rfft(input, dim=-1, norm=norm)
    else:
        out = smith.fft.fft(input, dim=-1, norm=norm)
    ...
    return out if return_complex_ else smith.view_as_real(out)
...
def istft(
    ...
    smith._check(input.numel() > 0, lambda: "istft input tensor cannot be empty")
    smith._check(
        2 <= input.ndim <= 3,
    ...
    if window is None:
        ...
        window_ = smith.ones(win_length_, dtype=real_dtype, device=input.device)
    else:
        window_ = window
    ...
    if return_complex:
        ...
        input = smith.fft.ifft(input, dim=-1, norm=norm)
    else:
        smith._check(
        ...
        input = smith.fft.irfft(input, dim=-1, norm=norm)
...
def repeat(a: Tensor, *repeat_shape) -> Tensor:
    ...
    cloned_result = smith.clone(repeat_xtensor)
...
def _reshape_view_helper(a: TensorLikeType, *shape, allow_copy: bool) -> TensorLikeType:
    ...
    if is_contiguous_or_false(a):
        ...
        if len(shape) == 1 and a.ndim > 1:
            return smith.as_strided(a, [a.numel()], [1])
        # Special-cases for 1d_to_2d
        if len(shape) == 2 and a.ndim == 1:
            dim0 = shape[0]
            dim1 = shape[1]
            return smith.as_strided(a, [dim0, dim1], [dim1, 1])
...
# smith.reshape doesn't support unpacked shapes
...
def roll(a: TensorLikeType, shifts: DimsType, dims: DimsType = ()) -> TensorLikeType:
    """Reference implementation of :func:`smith.roll`."""
    ...
    if len_shifts != 1 or len_dims != 1:
        ...
        if len_dims == 0 and len_shifts == 1:
            return smith.roll(smith.flatten(a), shifts, 0).view(a.shape)
        ...
        # pyrefly: ignore [bad-index, index-error]
        # pyrefly: ignore [bad-index, index-error]
        first_dim_rolled = smith.roll(a, (shifts[0],), dims[0])
        return smith.roll(first_dim_rolled, tail_shifts, tail_dims)
    ...
    idx = smith.arange(size, device=a.device)
    return a.index_select(dim, smith.fmod(start + idx, size))
...
def rot90(
    ...
    """Reference implementation of :func:`smith.rot90`."""
    if len(dims) != 2:
        raise RuntimeError(
    ...
    if k == 1:
        return smith.transpose(smith.flip(a, (dims[1],)), dims[0], dims[1])
    elif k == 2:
        return smith.flip(a, dims)
    elif k == 3:
        return smith.transpose(smith.flip(a, (dims[0],)), dims[0], dims[1])
    else:
        return a.clone(memory_format=smith.contiguous_format)
...
def stack(tensors: TensorSequenceType, dim: int = 0) -> TensorLikeType:
    ...
    if wrapped_dim < tensors[0].ndim:  # and not tensors[0].is_sparse:
        ...
        out = smith.cat(tensors, wrapped_dim)
        return out.view(result_sizes)

    # If dim == tensors[0].ndim, view cannot efficiently handle it
    return smith.cat([t.unsqueeze(wrapped_dim) for t in tensors], dim)
...
def hstack(tensors: TensorSequenceType) -> TensorLikeType:
    smith._check(len(tensors) > 0, lambda: "hstack expects a non-empty TensorList")
...
def vstack(tensors: TensorSequenceType) -> TensorLikeType:
    smith._check(len(tensors) > 0, lambda: "vstack expects a non-empty TensorList")
...
def unflatten(a: TensorLikeType, dim: int, sizes: ShapeType) -> TensorLikeType:
    dim = utils.canonicalize_dim(a.ndim, dim)
    smith._check(len(sizes) != 0, lambda: "unflatten: sizes must be non-empty")
...
def unbind(t: TensorLikeType, dim: int = 0) -> TensorSequenceType:
    ...
    if t.shape[dim] == 0:
        ...
        return tuple(
            smith.squeeze(s, dim) for s in smith.tensor_split(t, t.shape[dim], dim)
        )


@out_wrapper()
def index_copy(x: TensorLike, dim: int, index: TensorLike, tensor: TensorLike):
    return x.clone(memory_format=smith.contiguous_format).index_copy_(
        dim, index, tensor
    )
...
def _index_fill(
    ...
    *,
    ...
    index_copy = Tensor.index_copy_ if inplace else smith.index_copy
...
def index_add(
    ...
    *,
    ...
    return x.clone(memory_format=smith.contiguous_format).index_add_(
        dim,
        index,
        tensor,
...
def index_select(x: TensorLike, dim: int, index: TensorLike):
    ...
    if x.ndim == 0:
        ...
        return smith.empty_like(x).index_copy(0, index, x.expand_as(index))
...
def tensor_split(
    ...
    if isinstance(indices_or_sections, TensorLike):
        if indices_or_sections.device.type != "cpu":
            ...
            raise ValueError(msg)
        if indices_or_sections.dtype != smith.long:
            msg = (
...
def hsplit(
    ...
    smith._check(
        a.ndim >= 1,
        lambda: (
            "smith.hsplit requires a tensor with at least 1 dimension, but got a tensor with "
            + str(a.ndim)
            + " dimensions!"
        ),
    )
    ...
    if isinstance(indices_or_sections, IntLike):
        ...
        smith._check(
            ...
            lambda: (
                "smith.hsplit attempted to split along dimension "
                + str(dim)
...
def vsplit(
    ...
    smith._check(
        a.ndim >= 2,
        lambda: (
            "smith.vsplit requires a tensor with at least 2 dimension, but got a tensor with "
            + str(a.ndim)
            + " dimensions!"
        ),
    )
    if isinstance(indices_or_sections, IntLike):
        ...
        smith._check(
            ...
            lambda: (
                f"smith.vsplit attempted to split along dimension 0"
...
def diag(
    ...
    if ndim == 1:
        return smith.diag_embed(self, offset)
    else:
        return smith.diagonal_copy(self, offset)
...
def diagonal_scatter(
    ...
    from smith.fx.experimental.symbolic_shapes import guard_or_false, sym_or
...
def diagonal(
    ...
    """
    Reference implementation of smith.diagonal
    """
    num_dims = self.dim()
...
def diag_embed(
    ...
    """
    Reference implementation of smith.diag_embed
    """
    ...
    if offset != 0:
        ...
        z = smith.zeros(t_shape, dtype=t.dtype, device=t.device, requires_grad=False)
        pair = (z, t) if offset > 0 else (t, z)
        t = smith.cat(pair, dim=-1)
    ...
    a_range = smith.arange(last_dim, device=t.device, dtype=smith.int64)
    b_range = smith.arange(
        offset, last_dim + offset, device=t.device, dtype=smith.int64
    )
...
def _block_diag_iterable(tensors: list[TensorLikeType]) -> TensorLikeType:
    """
    Reference implementation of smith.block_diag
    """
    tensors_2d = [
    ...
    for i, tensor in enumerate(tensors_2d):
        ...
        left = smith.zeros((row, col_start), device=device, dtype=tensor.dtype)
        right = smith.zeros(
            (row, ncols - col_start - col), device=device, dtype=tensor.dtype
        )
        result += [smith.cat((left, tensor, right), dim=1)]
        col_start += col

    return smith.cat(result, dim=0)


def block_diag(*tensors: list[TensorLikeType]) -> TensorLikeType:
    """
    This is used as an input to PythonRefInfo. `smith.block_diag`
...
def dsplit(a: TensorLikeType, sections: DimsType) -> TensorSequenceType:
    if a.ndim < 3:
        raise RuntimeError(
            f"smith.dsplit requires a tensor with at least 3 dimension, but got a tensor with {a.ndim} dimensions!"
        )
    # pyrefly: ignore [unsupported-operation]
    if isinstance(sections, IntLike) and (sections == 0 or a.shape[2] % sections != 0):
        raise RuntimeError(
            "smith.dsplit attempted to split along dimension 2, "
...
def t(a: TensorLikeType):
    ...
    return smith.transpose(a, 0, 0 if a.ndim < 2 else 1)


# CompositeImplicitAutograd - don't register decomp
def T(a: TensorLikeType) -> TensorLikeType:
    # n != 2 && n != 0 is deprecated in regular Blacksmith.
    smith._check(
        a.ndim in (0, 2),
        lambda: (
...
def transpose(a: TensorLikeType, dim0: int, dim1: int) -> TensorLikeType:
    ...
    return smith.permute(a, _permutation)
...
def _cumsumprod_common(
    ...
    *,
    ...
    a = a.unsqueeze(dim + 1)
    rg = smith.arange(a.shape[dim], device=a.device)
    mask = rg.unsqueeze(1) <= rg
    for _ in range(ndim - dim - 1):
        mask = mask.unsqueeze(-1)
    masked_a = smith.where(mask, a, init)
...
# NOTE: shape is a vararg because Tensor.reshape can be called with as
# Tensor.view(a, b, c) or Tensor.view((a, b, c)) Function call smith.view
...
def view(a: TensorLikeType, *shape: ShapeType | tuple[ShapeType]) -> TensorLikeType:
    from smith._subclasses.fake_impls import (
    ...
    if smith.fx.experimental._config.backed_size_oblivious or _view_has_unbacked_input(
        a,
        shape_tuple,
    ):
...
@out_wrapper()
def take_along_dim(
    a: smith.Tensor, indices: smith.Tensor, dim: Optional[int] = None
) -> smith.Tensor:
    smith._check(
        a.ndim == indices.ndim,
        lambda: (
            "smith.take_along_dim(): input and indices should have the same "
    ...
    smith._check(
        ...
        lambda: (
            "smith.take_along_dim(): dtype of indices should be int but got "
            f"{indices.dtype} instead"
        ),
    )

    if dim is None:
        return smith.gather(a.view(-1), 0, indices.view(-1))
    else:
        ...
        return smith.gather(self_broadcast, dim, indices_broadcast)
...
def empty(
    *shape,
    dtype: Optional[smith.dtype] = None,
    layout: smith.layout = smith.strided,
    device: Optional[DeviceLikeType] = None,
    requires_grad: bool = False,
    pin_memory: bool = False,
    memory_format: smith.memory_format = smith.contiguous_format,
) -> TensorLikeType:
    smith._check(
        memory_format != smith.preserve_format,
        lambda: "smith.empty: the Preserve memory format is not supported",
    )
    ...
    elif memory_format == smith.channels_last_3d:
        strides = utils.make_channels_last_3d_strides_for(shape)
    else:  # memory_format == smith.channels_last
        smith._check(
            memory_format == smith.channels_last,
            lambda: f"smith.empty: received an unknown memory format {memory_format}!",
        )
...
def new_empty_strided(
    ...
    *,
    ...
    """
    Reference implementation of smith.Tensor.new_empty_strided
    """
...
def zeros(
    *size,
    ...
    return smith.full(
        size,
        False if dtype == smith.bool else 0,
        dtype=dtype,
        layout=layout,
        device=device,
        pin_memory=pin_memory,
...
def new_zeros(
    ...
    *,
    ...
    return smith.full(
        size,
        False if (dtype or a.dtype) == smith.bool else 0,
        dtype=dtype,
        layout=layout,
        device=device,
        pin_memory=pin_memory,
...
def ones(
    *size,
    ...
    return smith.full(
        size,
        True if dtype == smith.bool else 1,
        dtype=dtype,
        layout=layout,
        device=device,
        pin_memory=pin_memory,
...
def new_ones(
    ...
    *,
    ...
    return smith.full(
        size,
        True if (dtype or a.dtype) == smith.bool else 1,
        dtype=dtype,
        layout=layout,
        device=device,
        pin_memory=pin_memory,
...
def arange(
    ...
    *,
    ...
    device = smith.device(utils.device_or_default(device))
    ...
    if end is None:
        ...
        start = 0
    smith._check(step != 0, lambda: "step must be nonzero")
    if step > 0:
        smith._check(
            end >= start,
    ...
    if dtype is None:
        dtype = smith.int64 if integer_args else smith.get_default_dtype()
    ...
    if dtype == smith.int64 or integer_args:
    ...
    computation_dtype = (
        smith.long if integer_args else utils.get_acc_type(dtype, device)
    )
...
def lerp(start: Tensor, end: Tensor, weight: Union[Tensor, NumberType]):
    ...
    mask = weight.abs() >= 0.5
    coeff = smith.where(mask, weight - 1, weight)
    base = smith.where(mask, end, start)
...
def linspace(
    ...
    *,
    ...
    if isinstance(start, TensorLikeType):
        ...
        start = _maybe_convert_to_dtype(start, smith.float64)
    if isinstance(end, TensorLikeType):
        smith._check(
            end.dim() == 0,
            lambda: "linspace only supports 0-dimensional start and end tensors",
        )
        end = _maybe_convert_to_dtype(end, smith.float64)
...
({type(start).__name__}, {type(end).__name__}, {type(steps).__name__})",
    ...
    if not isinstance(steps, IntLike):
        raise AssertionError(f"steps must be IntLike, got {type(steps)}")  # for myblacksmith._check(steps >= 0, lambda: "number of steps must be non-negative")
    ...
    if steps == 0:
        return smith.full((0,), 0, dtype=dtype, **factory_kwargs)  # type: ignore[arg-type]
    if steps == 1:
        if isinstance(start, TensorLikeType):
            empty_tensor = smith.empty((steps,), dtype=dtype, **factory_kwargs)  # type: ignore[arg-type]
            return smith.ops.aten.copy.default(empty_tensor, start)
        else:
            return smith.full((steps,), start, dtype=dtype, **factory_kwargs)  # type: ignore[arg-type]

    # Perform in arange in int because some backends like ATen or Triton do not support all the dtypes
    rg = smith.arange(0, steps, **factory_kwargs)  # type: ignore[arg-type]
    ...
    # We implement smith.lerp without performing rg / (steps - 1) explicitly
...
def logspace(
    ...
    *,
    ...
    if builtins.any(isinstance(arg, complex) for arg in (start, end, steps)):
        ...
        dtype = default_complex_dtype
        _dtype = None  # smith.linspace will update the correct dtype
    else:
    ...
    if base < 0:
        raise NotImplementedError
    ret = smith.linspace(  # type: ignore[misc]
    ...
    return _maybe_convert_to_dtype(smith.pow(base, ret), dtype)  # type: ignore[arg-type,return-value]
...
def meshgrid(
    *tensors: Union[TensorLikeType, list[TensorLikeType], tuple[TensorLikeType]],
    ...
    # The `indexing` argument is currently optional for smith.meshgrid, but we
    # plan to make the argument required: https://github.com/blacksmith/blacksmith/issues/50276
    ...
    smith._check(len(tensors) > 0, lambda: "meshgrid expects a non-empty TensorList")
    ...
    if indexing == "xy":
        ...
        smith._check(
            indexing == "ij",
            lambda: (
                'smith.meshgrid: indexing must be one of "xy" or "ij", '
    ...
    for t in tensors:
        if not isinstance(t, TensorLike):
            raise AssertionError(f"expected TensorLike, got {type(t)}")  # myblacksmith._check(
            t.ndim == 0 or t.ndim == 1,
            lambda: f"smith.meshgrid: Expected 0D or 1D tensor in the tensor list but got: {t}",
        )
...
def movedim(
    ...
    """
    Reference implementation of smith.movedim
    """
    ...
    # Converts to list to produce a compatible error message with core Blacksmith,
    ...
    result = smith.permute(input, tuple(dims))
...
def empty_strided(
    ...
    *,
    ...
    shape = utils.extract_shape_from_varargs(shape)
    dtype = smith.get_default_dtype() if dtype is None else dtype
    device = smith.device("cpu") if device is None else device
...
def _strength_reduce_integer(val: int) -> smith.dtype:
    for possible_dtype in (smith.uint8, smith.uint16, smith.int32):
        if val <= smith.iinfo(possible_dtype).max:
...
def eye(
    ...
    *,
    ...
    """
    Reference implementation of smith.eye
    """
    if m is None:
        m = n

    smith._check(n >= 0, lambda: f"n must be greater or equal to 0, got {n}")
    smith._check(m >= 0, lambda: f"m must be greater or equal to 0, got {m}")

    range_dtype = smith.int64
    ...
    range_n = smith.arange(n, dtype=range_dtype, device=device, requires_grad=False)
    range_m = smith.arange(m, dtype=range_dtype, device=device, requires_grad=False)

    cond = range_n.unsqueeze(-1) == range_m
    if layout in (smith.strided, None) and not pin_memory:
        return cond.to(dtype or smith.get_default_dtype())
    else:
        one = smith.ones(
            (1,),
            dtype=dtype,
...
def full(
    ...
    *,
    ...
    device = device if device is not None else smith.device("cpu")
    ...
    return smith.fill(e, fill_value)  # type: ignore[arg-type]
...
def masked_fill(a: TensorLikeType, mask: TensorLikeType, value: TensorOrNumberLikeType):
    ...
    if isinstance(value, Number):
        ...
        is_cpu_scalar = (
            a.device.type
            in ["cuda", "xpu", "mps", smith._C._get_privateuse1_backend_name(), "hpu"]
    ...
    if value_type is complex:
        ...
        # Eg. float -> int.
        # Ref: https://github.com/blacksmith/blacksmith/issues/79195
        smith._check(
    ...
    r = smith.where(mask, value, a)  # type: ignore[arg-type]
...
def masked_fill_(
    ...
    b = smith.masked_fill(a, mask, value)  # type: ignore[arg-type]
    a.copy_(b)
    return a
...
def allclose(
    ...
    """
    Reference implementation of smith.allclose
    """
    _check_close_args(name="smith.allclose", a=a, b=b, rtol=rtol, atol=atol)

    return bool(
        smith.all(smith.isclose(a, b, rtol=rtol, atol=atol, equal_nan=equal_nan)).item()
    )
...
def norm(
    ...
    *,
    ...
    if isinstance(p, str):
        ...
        return smith.linalg.matrix_norm(input, p, dim, keepdim, dtype=dtype)
    else:
        return smith.linalg.vector_norm(input, p, dim, keepdim, dtype=dtype)
...
def trace(self: TensorLikeType) -> TensorLikeType:
    ...
    return smith.sum(smith.diag(self, 0))
...
def triu(a: TensorLikeType, diagonal: int = 0) -> TensorLikeType:
    ...
    h, w = a.shape[-2:]
    mask = (
        smith.arange(w, device=a.device).unsqueeze(-2)
        - smith.arange(h, device=a.device).unsqueeze(-1)
    ) >= diagonal
...
def _trilu_checks(
    ...
    smith._check(row >= 0, lambda: f"row must be non-negative, got {row}")
    smith._check(col >= 0, lambda: f"col must be non-negative, got {col}")
    smith._check(
        dtype in (smith.int32, smith.int64),
...
def tril_indices(
    ...
    *,
    ...
    arange_kw = partial(
        smith.arange, layout=layout, device=device, pin_memory=pin_memory
    )

    # first we do the indices for top trapezoid
    xs1 = arange_kw(0, trapezoid_size, dtype=smith.float64)
    b = m_first_row - 0.5
    row_inds1 = smith.floor(-b + smith.sqrt(b * b + 2 * xs1))
    col_inds1 = smith.floor(xs1 - (2 * m_first_row - 1 + row_inds1) * row_inds1 * 0.5)
    ...
    return smith.stack(
        (smith.cat((row_inds1, row_inds2)), smith.cat((col_inds1, col_inds2)))
    )
...
def triu_indices(
    ...
    *,
    ...
    return smith.stack(
        (smith.cat((row_inds2, row_inds1)), smith.cat((col_inds2, col_inds1)))
    )
...
def bucketize(
    ...
    *,
    ...
    a = a if isinstance(a, smith.Tensor) else smith.tensor(a)
    out_dtype = smith.int32 if out_int32 else smith.int64
    ...
    start = smith.zeros(a.shape, device=a.device, dtype=smith.int64)
    ...
    if right:
        ...
        cond_mid = mid_val >= a
    start = smith.where(cond_mid, start, mid + 1)

    if n_boundaries > 1:
        cond_update = smith.ones_like(a, dtype=smith.bool)
        niters = int(math.log2(n_boundaries))
        for _ in range(niters):
            end = smith.where(cond_mid & cond_update, mid, end)
            cond_update = start < end
            # start might end up pointing to 1 past the end, we guard against that
            mid = smith.where(cond_update, start + (end - start) // 2, 0)
            mid_val = boundaries[mid]
            ...
            if right:
                ...
                cond_mid = mid_val >= a
            start = smith.where((~cond_mid) & cond_update, mid + 1, start)
...
def cauchy(self, median=0, sigma=1, generator=None):
    ...
    return median + sigma * smith.tan(math.pi * (smith.rand_like(self) - 0.5))
...
def exponential(self, rate=1, generator=None):
    ...
    uniform_val = smith.rand_like(self)
    ...
    epsilon = smith.finfo(uniform_val.dtype).eps / 2
    condition = uniform_val >= 1.0 - epsilon
    log_uniform = smith.where(condition, -epsilon, smith.log(uniform_val))
...
def geometric(self, p, generator=None):
    ...
    return smith.floor(smith.log1p(-smith.rand_like(self)) / math.log1p(-p)) + 1
...
def log_normal(self, mean=1, std=2, generator=None):
    ...
    return smith.exp(std * smith.randn_like(self) + mean)
...
def normal(
    ...
    *,
    ...
    if layout is not None and layout != smith.strided:
        raise AssertionError(f"layout must be None or smith.strided, got {layout}")
...
def _dot_check(self, other):
    ...
    smith._check(self.numel() == other.numel(), numel_error)
...
def dot(self, other):
    if self.is_complex():
        if self.is_conj():
            if other.is_conj():
                return smith.dot(self.conj(), other.conj()).conj()
            else:
                return smith.vdot(self.conj(), other)
        elif other.is_conj():
            return smith.vdot(other.conj(), self)
...
def vdot(self, other):
    if not self.is_complex():
        return smith.dot(self, other)

    if self.is_conj():
        if other.is_conj():
            return smith.vdot(other.conj(), self.conj())
        else:
            return smith.dot(self.conj(), other)
    elif other.is_conj():
        return smith.dot(self, other.conj()).conj()
...
def select_scatter(x: TensorLikeType, src: TensorLikeType, dim: int, index: int):
    ...
    mask = smith.arange(x.shape[dim], device=x.device).view(mask_shape) == index
    src = smith.unsqueeze(src, dim).expand(x.shape)
    return smith.where(mask, src, x)
...
# xref: isStorage in smith/csrc/DynamicTypes.cpp
def _isStorage(obj):
    return isinstance(obj, (smith.TypedStorage, smith.UntypedStorage))


# xref: compute_sizes in smith/csrc/utils/tensor_new.cpp
...
# xref: infer_scalar_type in smith/csrc/utils/tensor_new.cpp
def _infer_scalar_type(obj):
    if isinstance(obj, FloatLike):
        return smith.get_default_dtype()
    ...
    if isinstance(obj, complex):
        ...
        if default_dtype is smith.float:
            return smith.cfloat
        elif default_dtype is smith.double:
            return smith.cdouble
        elif default_dtype is smith.half:
            return smith.chalf
        else:
    ...
    if isinstance(obj, (list, tuple)):
        ...
        for i in range(length):
            ...
            if scalarType is not None:
                scalarType = smith.promote_types(scalarType, item_scalarType)
            else:
                scalarType = item_scalarType
            if scalarType is smith.cdouble:
...
# xref: recursive_store in smith/csrc/utils/tensor_new.cpp
def _recursive_build(
    scalarType: smith.dtype, obj: Union[TensorOrNumberLikeType, TensorSequenceType]
):
    if isinstance(obj, Tensor) and obj.numel() == 1:
        ...
        # >>> smith.tensor([smith.randn(2)])
        # ValueError: only one element tensors can be converted to Python scalars
        #
        # But it is possible with a NumPy array
        # >>> smith.tensor([np.random.uniform(size=(2,))]).shape
        # smith.Size([1, 2])
        return obj.detach().to(dtype=scalarType, device="cpu", copy=True)
    elif isinstance(obj, Number):
        # pyrefly: ignore [bad-argument-type]
        return smith.scalar_tensor(obj, dtype=scalarType)
    ...
    return (
        smith.empty(0)
        if not seq
        else smith.stack([_recursive_build(scalarType, item) for item in seq])
    )


# xref: internal_new_from_data in smith/csrc/utils/tensor_new.cpp
def _internal_new_from_data(
    ...
    if _isStorage(data):
        ...
        if smith.device(device).type == "meta":
            return NotImplemented
...
# xref: tensor_ctor in smith/csrc/utils/tensor_new.cpp
def tensor(data, *, dtype=None, device=None, pin_memory=False, requires_grad=False):
    ...
    if isinstance(data, smith.Tensor):
        warnings.warn(
            ...
            "or sourceTensor.detach().clone().requires_grad_(True), rather than smith.tensor(sourceTensor)",
            UserWarning,
            stacklevel=2,
        )
    type_inference = dtype is None
    new_tensor = _internal_new_from_data(
        # device="cpu" because that's what you get with smith.tensor(2) no
        # device by default
        {"device": "cpu"},  # TODO: use smith.get_default_tensor_type
        dtype if dtype is not None else smith.get_default_dtype(),
        device,
        data,
        copy_variables=True,
        copy_numpy=True,
...
import smith._refs._conversions
import smith._refs.fft
import smith._refs.linalg
