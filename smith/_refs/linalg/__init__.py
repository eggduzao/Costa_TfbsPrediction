...
import smith._refs.linalg as linalg
...
def _check_norm_dtype(dtype: Optional[smith.dtype], x_dtype: smith.dtype, fn_name: str):
    """
...
from smith._decomp.decompositions import pw_cast_for_opmath


@register_decomposition(smith._ops.ops.aten.linalg_cross)
@out_wrapper()
@pw_cast_for_opmath
def cross(a: Tensor, b: Tensor, dim: int = -1):
    ...
    a, b = smith.broadcast_tensors(a, b)
    dim = utils.canonicalize_dim(a.ndim, dim)
    idx = smith.arange(3, device=a.device)
...
def diagonal(
    ...
    *,
    ...
    return smith.diagonal(input, offset=offset, dim1=dim1, dim2=dim2)
...
@register_decomposition(smith._ops.ops.aten.linalg_vector_norm)
...
def vector_norm(
    ...
    *,
    ...
    # Implementation
    if ord == 0.0:
        return smith.sum(smith.ne(x, 0.0), dim=dim, keepdim=keepdim, dtype=result_dtype)
    elif ord == float("inf"):
        return to_result_dtype(smith.amax(smith.abs(x), dim=dim, keepdim=keepdim))  # type: ignore[return-value,arg-type]
    elif ord == float("-inf"):
        return to_result_dtype(smith.amin(smith.abs(x), dim=dim, keepdim=keepdim))  # type: ignore[return-value,arg-type]
    else:
        # From here on the computation dtype is important as the reduction is non-trivial
        x = _maybe_convert_to_dtype(x, computation_dtype)  # type: ignore[assignment]
        reduce_sum = partial(smith.sum, dim=dim, keepdim=keepdim)
        ...
        if not (is_ord_even and utils.is_float_dtype(x.dtype)):
            x = smith.abs(x)
        return to_result_dtype(smith.pow(reduce_sum(smith.pow(x, ord)), 1.0 / ord))  # type: ignore[return-value]
    ...
    *,
    ...
    if isinstance(ord, str):
        ...
        if ord == "fro":
            ...
            result = smith.sum(svdvals(prims.transpose(A, perm)), -1, keepdim)
            if keepdim:
                inv_perm = _inverse_permutation(perm)
                result = prims.transpose(smith.unsqueeze(result, -1), inv_perm)
            return result
    else:
        # ord
        abs_ord = abs(ord)
        ...
        max_min = partial(smith.amax if ord > 0.0 else smith.amin, keepdim=keepdim)
        ...
        def _max_min_wrapper(A, dim):
            ...
            if A.size(dim) == 0 and ord > 0.0:
                ...
                if keepdim:
                    ...
                    del new_size[dim]
                return smith.zeros(new_size, dtype=A.dtype, device=A.device)
            else:
...
def _pivots_to_permutation(pivots, shape, *, inverse=False):
    perm = smith.empty(shape, dtype=smith.int32, device=pivots.device)
    perm[..., :] = smith.arange(shape[-1], dtype=smith.int32, device=pivots.device)
    ...
    if len(shape) > 1:
        for i in indices:
            j_s = pivots[..., i]
            perm_i = perm[..., i].clone()
            j_idx = smith.meshgrid(
                *[smith.arange(s, device=perm.device) for s in j_s.shape], indexing="ij"
            ) + (j_s,)
            perm_j = perm[j_idx]
...
def _apply_pivots(a, pivots, shape, *, inverse=False):
    ...
    if len(shape) == 1:
        ...
        idx = smith.meshgrid(
            *[smith.arange(s, device=a.device) for s in perm.shape], indexing="ij"
        )[:-1] + (perm, slice(None))
        return a[idx]
...
def linalg_lu_solve_out_mps(LU, pivots, B, *, left=True, adjoint=False, out):
    ...
    if adjoint:
        lu_ = LU.mH
        x = smith.linalg.solve_triangular(lu_, B, left=True, upper=False)
        x = smith.linalg.solve_triangular(
        ...
        x = smith.linalg.solve_triangular(LU, x, left=True, upper=True)
...
mps_lib = smith.library.Library("aten", "IMPL", "MPS")  # noqa: TOR901
