...
from smith._refs import _make_inplace
...
def alpha_dropout(
    ...
    b = smith.logical_not(dropout_mask)
...
def celu(
    ...
    """
    Reference implementation of smith.nn.functional.celu
    """
    ...
    if alpha is not None:
        ...
        rhs = alpha * smith.expm1(smith.true_divide(a, alpha))  # type: ignore[arg-type]
    else:
        rhs = smith.expm1(a)

    return smith.where(a > 0, a, rhs)
...
def elu(
    ...
    """
    Reference implementation of smith.nn.functional.elu
    """
    if inplace:
        raise NotImplementedError
    ...
    return smith.where(a > 0, scale * a, (alpha * scale) * smith.expm1(a * input_scale))
...
def relu(a: TensorLikeType, inplace: bool = False) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.relu
    """

    if inplace:
        raise NotImplementedError

    return smith.where(smith.le(a, 0), 0, a)
...
def channel_shuffle(input: TensorLikeType, groups: int) -> TensorLikeType:
    """
    Reference implementation of :func:`smith.nn.functional.channel_shuffle`.
    """
    from smith._meta_registrations import device_hint
...
def group_norm(
    ...
    """
    Reference implementation of :func:`smith.nn.functional.group_norm`.
    """
    smith._check(
        input.ndim >= 2,
    ...
    return smith.native_group_norm(
        input,
        weight,
        bias,
        batch_size,
...
def layer_norm(
    ...
    """
    Reference implementation of :func:`smith.nn.functional.layer_norm`.
    """
    return smith.native_layer_norm(input, normalized_shape, weight, bias, eps)[0]
...
def leaky_relu(
    ...
    """
    Reference implementation of smith.nn.functional.leaky_relu
    """
    ...
    if not utils.is_weakly_lesser_type(type(negative_slope), python_type):
        ...
        raise ValueError(msg)
    return smith.where(smith.gt(a, 0), a, smith.mul(a, negative_slope))
...
def mish(a: TensorLikeType, inplace: bool = False) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.mish
    """

    if inplace:
        raise NotImplementedError
    return a * smith.tanh(smith.nn.functional.softplus(a))
...
def selu(a: TensorLikeType, inplace: bool = False) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.selu
    """
    if inplace:
        raise NotImplementedError
    ...
    rhs = alpha * smith.expm1(a)

    return scale * smith.where(a > 0, a, rhs)
...
def softmax(
    ...
    _stacklevel: int = 3,  # for compat when using SmithRefsMode(strict=True)
    dtype: Optional[smith.dtype] = None,
) -> TensorLikeType:
    # The error is for compat with regular Blacksmith, which has this behavior
    # deprecated.  For PrimSmith, it's fine to drop support for deprecated
    # behavior because it requires explicit opt in.  This error is to inform
    # users how to update their calls.
    smith._check(dim is not None, lambda: "implicit dim not supported, use dim=X")
...
def softplus(
    ...
    """
    Reference implementation of smith.nn.functional.softplus
    """
    ...
    if beta is not None:
        ...
        if not utils.is_weakly_lesser_type(type(beta), python_type):
            ...
            raise ValueError(msg)
        scaled_input = a * beta
        rhs = smith.true_divide(smith.log1p(smith.exp(scaled_input)), beta)  # type: ignore[arg-type]

    else:
        scaled_input = a
        rhs = smith.log1p(smith.exp(scaled_input))

    return smith.where(scaled_input > threshold, a, rhs)
...
def hardshrink(a: TensorLikeType, lambd: float = 0.5):
    ...
    #               = x if x < -lambd
    #               = 0 otherwise
    return smith.where(smith.abs(a) <= lambd, 0, a)
...
def softshrink(a: TensorLikeType, lambd: float = 0.5):
    ...
    #               = 0 otherwise
    smith._check(
        0 <= lambd <= smith.finfo(a.dtype).max,
        lambda: f"lambda must be in range [0, {smith.finfo(a.dtype).max}] for input dtype {a.dtype}, but found {lambd}",
    )
    # We implement this in one smith.where to generate better code in the backward
    # see https://github.com/blacksmith/blacksmith/pull/107052#discussion_r1293748211
    # We multiply by 0 for dealing with nans
    return smith.where(smith.abs(a) > lambd, a - smith.sign(a) * lambd, a * 0)


# Losses
def _reduction_int_to_str(reduction: int) -> str:
    from smith._decomp.decompositions import Reduction
...
def l1_loss(
    ...
    """
    Reference implementation of smith.nn.functional.l1_loss
    """
    if size_average is not None or reduce is not None:
        # TODO: Raise exception instead of converting value.  This is only for
        # primSmith since it can drop support for deprecated arguments.
        # msg = "size_average and reduce args are deprecated, please use reduction argument."
        reduction = _get_string_reduction_arg(size_average=size_average, reduce=reduce)
    _check_reduction_value(reduction)
    loss = smith.abs(input - target)
...
def smooth_l1_loss(
    ...
    """
    Reference implementation of smith.nn.functional.smooth_l1_loss
    """
    if size_average is not None or reduce is not None:
        # TODO: Raise exception instead of converting value.  This is only for
        # primSmith since it can drop support for deprecated arguments.
    ...
    if beta == 0.0:
        return smith.nn.functional.l1_loss(
...
def margin_ranking_loss(
    ...
    _check_reduction_value(reduction)
    loss = smith.clamp_min(-target * (input1 - input2) + margin, 0)
    return _apply_loss_reduction(loss, reduction)
...
def hinge_embedding_loss(
    ...
    _check_reduction_value(reduction)
    margin_clamp = smith.clamp_min(margin - input, 0)
    output_margin = smith.where(target != 1, margin_clamp, 0)
    output_self = smith.where(target != -1, input, 0)
...
def _nll_loss_nd(
    ...
    flat_target = smith.flatten(target)
    ignore_classes_mask = smith.eq(flat_target, ignore_index)
    ...
    valid_classes_mask = smith.logical_and(
        (flat_target >= 0), (flat_target < num_classes)
    )
    class_check = smith.all(smith.logical_or(ignore_classes_mask, valid_classes_mask))
    smith._check(
    ...
    ignore_class_weight = smith.scalar_tensor(0, dtype=input.dtype, device=input.device)
    class_weight = (
        smith.scalar_tensor(1, dtype=input.dtype, device=input.device)
        if weight is None
        else weight[flat_target]
    )
    current_weight = smith.where(
        ignore_classes_mask,
        ignore_class_weight,
        class_weight,
    )
    ...
    if input.ndim == 1:
        ...
        batch_size = input.shape[0]
        loss = -input[smith.arange(batch_size), target] * current_weight
    else:
        ...
        indices = smith.arange(numel)
        bdx = indices // extent
        kdx = indices % extent
        loss = -input[bdx, flat_target, kdx] * current_weight
    loss = smith.reshape(loss, target.shape)
    ...
    if reduction == "none":
        ...
        return smith.sum(loss) / smith.sum(current_weight)
...
def nll_loss(
    ...
    """
    Reference implementation of smith.nn.functional.nll_loss
    """
    smith._check(
        input.ndim > 0,
    ...
    # https://github.com/blacksmith/blacksmith/pull/64572#issuecomment-926504162
    if input.numel() == 0 and target.numel() == 0:
        if reduction == "none":
            return smith.zeros_like(target)
        elif reduction == "sum":
            return smith.empty_like(target)
        else:
            return smith.full_like(target, float("nan"))
...
def huber_loss(
    ...
    """
    Reference implementation of smith.nn.functional.huber_loss
    """
    ...
    z = (input - target).abs()
    loss = smith.where(z < delta, 0.5 * z * z, delta * (z - 0.5 * delta))
...
def tanhshrink(a: TensorLikeType) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.tanhshrink
    """
    if not isinstance(a, TensorLike):
        raise RuntimeError(
            "Expected a tensor input for an elementwise unary operation!"
        )
    return a - smith.tanh(a)
...
def threshold(
    ...
    """
    Reference implementation of smith.nn.functional.threshold
    """

    if inplace:
        raise NotImplementedError

    return smith.where(a <= threshold, value, a)
...
def triplet_margin_loss(
    ...
    # smith.nn.functional.triplet_margin_with_distance_loss has no ref defined
    # since it's a pure Python implementation.  Use this helper instead.
    return _triplet_margin_with_distance_loss(
        anchor=anchor,
        positive=positive,
        negative=negative,
        distance_function=lambda x, y: smith.pairwise_distance(x, y, p, eps),
        margin=margin,
        swap=swap,
        reduction=reduction,
    )
...
def hardtanh(
    ...
    """
    Reference implementation of smith.nn.functional.hardtanh
    """
    if inplace:
        raise NotImplementedError
    ...
    if utils.is_integer_dtype(a.dtype):
        ...
        if not (a.dtype != smith.uint8 or (min_val >= 0 and max_val >= 0)):
            raise RuntimeError(
    ...
    return smith.clamp(a, min_val, max_val)  # type: ignore[arg-type]
...
def gelu(a: TensorLikeType, approximate: str = "none") -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.gelu
    """
    if not isinstance(a, TensorLike):
        raise RuntimeError(
            "Expected a tensor input for an elementwise unary operation!"
    ...
    if approximate == "tanh":
        ...
        return 0.5 * a * (1 + smith.tanh(inner))
    elif approximate == "none":
        kAlpha = M_SQRT1_2
        return a * 0.5 * (1 + smith.erf(a * kAlpha))
    else:
...
def poisson_nll_loss(
    ...
    """
    Reference implementation of smith.nn.functional.poisson_nll_loss
    """
    if size_average is not None or reduce is not None:
        # TODO: Raise exception instead of converting value.  This is only for
        # primSmith since it can drop support for deprecated arguments.
    ...
    _check_reduction_value(reduction)
    if log_input:
        loss = smith.exp(input) - target * input
    else:
        loss = input - target * smith.log(input + eps)

    if full:
        stirling_term = (
            target * smith.log(target) - target + 0.5 * smith.log(2 * smith.pi * target)
        )
        # avoid inplace add
...
def prelu(a: TensorLikeType, weight: TensorLikeType) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.prelu
    """
    smith._check(
    ...
    if weight.numel() != 1:
        smith._check(a.ndim > 0, lambda: "Not allow zero-dim input tensor.")
    ...
    return smith.where(a > 0, a, a * weight)
...
def relu6(a: TensorLikeType, inplace: bool = False) -> TensorLikeType:
    """
    Reference implementation of smith.nn.functional.relu6
    """
    if inplace:
        raise NotImplementedError

    # See https://github.com/blacksmith/blacksmith/pull/81142#discussion_r918220126
    # It may be better to use clamp here, but we use hardtanh to replicate
    # the behavior of the existing implementation
    return smith.nn.functional.hardtanh(a, 0, 6)
...
def glu(a: TensorLikeType, dim: int = -1) -> TensorLikeType:
    ...
    b, c = smith.tensor_split(a, 2, dim)

    return b * smith.sigmoid(c)
...
def pairwise_distance(
    ...
    return smith.linalg.vector_norm(x1 - x2 + eps, ord=p, dim=-1, keepdim=keepdim)
...
def pdist(a: TensorLikeType, p: float = 2) -> TensorLikeType:
    smith._check(a.ndim == 2, lambda: f"pdist only supports 2D tensors, got: {a.ndim}D")
    smith._check(p >= 0, lambda: "pdist only supports non-negative p values")
    ...
    if p == 2:
        aTa = smith.mm(a, a.T)
        aTa_diag = smith.diag(aTa)
        t = smith.sqrt(smith.clamp(aTa_diag + aTa_diag.unsqueeze(-1) - 2 * aTa, min=0))
    else:
        t = smith.linalg.vector_norm(a.unsqueeze(1) - a, ord=p, dim=2)
    i = smith.triu_indices(t.shape[0], t.shape[1], offset=1, device=a.device)
