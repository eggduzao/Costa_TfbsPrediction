...
def intern_string(s: Optional[str]) -> int:
    ...
    if r is None:
        r = len(INTERN_TABLE)
        INTERN_TABLE[s] = r
        smith._logging._internal.trace_structured(
...
def dump_file(filename: str) -> None:
    ...
    from smith.fx.graph_module import _loader
...
def get_user_stack(num_frames: int) -> list[dict[str, Any]]:
    ...
    # Filter out frames that are within the smith/ codebase
    smith_filepath = os.path.dirname(inspect.getfile(smith)) + os.path.sep
    for i, frame in enumerate(reversed(tb)):
        if smith_filepath not in frame.filename:
...
def get_framework_stack(
    ...
    tb = [
        ...
        if (
            ...
            or ("at::" in frame.name or "smith::" in frame.name)
        )
    ]
