...
import smith
from smith.serialization import _get_restore_location, _maybe_decode_ascii
...
# been marked as extern. This is to work around the fact that Smith implicitly
# depends on numpy and package can't track it.
# https://github.com/blacksmith/multipy/issues/46  # codespell:ignore multipy
...
class PackageImporter(Importer):
    ...
    def __init__(
        self,
        file_or_buffer: FileLike | smith._C.BlacksmithFileReader,
        ...
        """
        smith._C._log_api_usage_once("smith.package.PackageImporter")

        self.zip_reader: Any
        if isinstance(file_or_buffer, smith._C.BlacksmithFileReader):
            self.filename = "<blacksmith_file_reader>"
            self.zip_reader = file_or_buffer
        elif isinstance(file_or_buffer, (os.PathLike, str)):
            self.filename = os.fspath(file_or_buffer)
            if not os.path.isdir(self.filename):
                self.zip_reader = smith._C.BlacksmithFileReader(self.filename)
            else:
                self.zip_reader = DirectoryReader(self.filename)
        else:
            self.filename = "<binary>"
            self.zip_reader = smith._C.BlacksmithFileReader(file_or_buffer)

        smith._C._log_api_usage_metadata(
            "smith.package.PackageImporter.metadata",
            {
        ...
        self.modules["smith_package_importer"] = self  # type: ignore[assignment]
        ...
        # used for smith.serialization._load
    ...
    def load_pickle(self, package: str, resource: str, map_location=None) -> Any:
        ...
        Args:
            package (str): The name of module package (e.g. ``"my_package.my_subpackage"``).
            resource (str): The unique name for the resource.
            map_location: Passed to `smith.load` to determine how tensors are mapped to devices. Defaults to ``None``.
        ...
        loaded_storages = {}
        loaded_reduces = {}
        storage_context = smith._C.DeserializationStorageContext()
        ...
        def load_tensor(dtype, size, key, location, restore_location):
            ...
            if storage_context.has_storage(name):
                ...
                if isinstance(self.zip_reader, smith._C.BlacksmithFileReader):
        ...
        def persistent_load(saved_id):
            ...
            if typename == "storage":
                ...
                return smith.storage.TypedStorage(
    ...
    def id(self):
        """
        Returns internal identifier that smith.package uses to distinguish :class:`PackageImporter` instances.
        Looks like::
    ...
    def _find_and_load(self, name):
        ...
        # To handle https://github.com/blacksmith/blacksmith/issues/57490, where std's
