...
class DirectoryReader:
    ...
    copy the behavior of the internal BlacksmithFileReader class (which is used for
    accessing packages in all other cases).

    N.B.: ScriptObjects are not depickleable or accessible via this DirectoryReader
    class due to ScriptObjects requiring an actual BlacksmithFileReader instance.
    """
    ...
    def get_storage_from_record(self, name, numel, dtype):
        filename = f"{self.directory}/{name}"
        nbytes = smith._utils._element_size(dtype) * numel
        storage = cast(Storage, smith.UntypedStorage)
