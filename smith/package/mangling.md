# Import mangling in `smith.package`
...
To avoid accidental name collisions with modules in `sys.modules`. Consider the following:
    ...
    i = smith.PackageImporter('my_resnet_18.pt')
    loaded_resnet18 = i.load_pickle('model', 'model.pkl')

    print(type(local_resnet18).__module__)  # 'smithvision.models.resnet18'
    print(type(loaded_resnet18).__module__)  # ALSO 'smithvision.models.resnet18'
...
- `inspect`: used in SmithScript to retrieve source code to compile
- …probably more that we don't know about.

In these cases, we may silently pick up the wrong module for `loaded_resnet18`
and e.g. SmithScript the wrong source code for our model.
...
becomes
    ...
    <smith_package_0>.smithvision.models.resnet18

The mangle parent is made unique to a given `PackageImporter` instance by
bumping a process-global `mangle_index`, i.e. `<smith__package{mangle_index}>`.
...
An imported module's `__file__` attribute is mangled in the same way, so:
    ...
    smithvision/modules/resnet18.py

becomes

    <smith_package_0>.smithvision/modules/resnet18.py
