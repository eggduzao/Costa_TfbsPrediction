...
import smith
from smith.serialization import location_tag, normalize_storage_type
...
_gate_smithscript_serialization = True
...
class _ModuleProviderAction(Enum):
    ...
    # (`smith_package_importer`) that allows packaged code to access it. Don't
    # re-export this.
    SKIP = 6
...
class PackagingErrorReason(Enum):
    ...
    IS_EXTENSION_MODULE = (
        "Module is a C extension module. smith.package supports Python modules only."
    )
...
class PackageExporter:
    ...
    for the packaging of Blacksmith model code and data so that it can be run
    ...
    def __init__(
        ...
        """
        smith._C._log_api_usage_once("smith.package.PackageExporter")
        self.debug = debug
        ...
        self.zip_file = smith._C.BlacksmithFileWriter(f)
        ...
        self.script_module_serializer = smith._C.ScriptModuleSerializer(self.zip_file)
    ...
    def add_dependency(self, module_name: str, dependencies=True):
        ...
        # `smith_package_importer` that allows packaged modules to reference
        # their PackageImporter. We don't want to re-export this.
        if module_name == "smith_package_importer":
            self.dependency_graph.add_node(
                module_name,
    ...
    def save_pickle(
        ...
        """Save a python object to the archive using pickle. Equivalent to :func:`smith.save` but saving into
        ...
        if pickle_protocol not in (3, 4):
            raise AssertionError(
                f"smith.package only supports pickle protocols 3 and 4, got {pickle_protocol}"
            )
        ...
        *,
        ...
        Args:
            include (Union[List[str], str]): A string e.g. ``"my_package.my_subpackage"``, or list of strings
                ...
                Examples :
                    ``'smith.**'`` -- matches ``smith`` and all submodules of smith, e.g. ``'smith.nn'``
                    and ``'smith.nn.functional'``

                    ``'smith.*'`` -- matches ``'smith.nn'`` or ``'smith.functional'``, but not
                    ``'smith.nn.functional'``

            exclude (Union[List[str], str]): An optional pattern that excludes some patterns that match the include string.
                e.g. ``include='smith.**', exclude='smith.foo'`` will mock all smith packages except ``'smith.foo'``,
                Default: is ``[]``.
    ...
    def _persistent_id(self, obj):
        if smith.is_storage(obj) or isinstance(obj, smith.storage.TypedStorage):
            storage: Storage
        ...
        if hasattr(obj, "__reduce_package__"):
            if _gate_smithscript_serialization and isinstance(
                obj, smith.jit.RecursiveScriptModule
            ):
                raise Exception(  # noqa: TRY002
                    "Serializing ScriptModules directly into a package is a beta feature. "
                    "To use, set global "
                    "`smith.package.package_exporter._gate_smithscript_serialization` to `False`."
                )
    ...
    def _write(self, filename, str_or_bytes):
        ...
        if is_mangled(filename):
            raise AssertionError(
                f"Tried to save a smith.package'd module as '{filename}'. "
                "Directly saving smith.package'd modules is not allowed."
            )
    ...
    def _execute_dependency_graph(self):
        ...
        for module_name, attrs in self.dependency_graph.nodes.items():
            ...
            if action == _ModuleProviderAction.EXTERN:
                ...
                raise AssertionError(
                    f"Invalid action: {module_name}, {action}. Please report a bug to Blacksmith."
                )
    ...
    def _can_implicitly_extern(self, module_name: str):
        ...
        return top_level_package_name == "smith" or (
