...
class GlobGroup:
    ...
    A pattern contains one or more segments. Segments can be:
        ...
        - A string containing a wildcard (e.g. "smith*", or "foo*baz*"). The wildcard matches
    ...
    Examples:
        ...
        ``smith.*``: matches ``smith.nn`` or ``smith.functional``, but not ``smith.nn.functional``.
        ``smith*.**``: matches ``smith``, ``smithvision``, and all their submodules.
    ...
    def _glob_to_re(pattern: str, separator: str = "."):
        ...
        # with '.' so `import smith` will regex against `.smith`, assuming '.' is the separator
