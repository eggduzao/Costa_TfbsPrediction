...
class Importer(ABC):
    ...
    smith.package introduces module importers other than the default one.
...
class _SysImporter(Importer):
    ...
    def whichmodule(self, obj: Any, name: str) -> str:
        ...
        # which fails for mangled package names like '<smith_package_0>'.
...
class OrderedImporter(Importer):
    ...
    def _is_smithpackage_dummy(self, module):
        """Returns true iff this module is an empty PackageNode in a smith.package.
        ...
        See: https://github.com/blacksmith/blacksmith/pull/71520#issuecomment-1029603769
        """
        if not getattr(module, "__smith_package__", False):
            return False
    ...
    def import_module(self, module_name: str) -> ModuleType:
        ...
        for importer in self._importers:
            ...
            try:
                ...
                if self._is_smithpackage_dummy(module):
                    continue
                return module
