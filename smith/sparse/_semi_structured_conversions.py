...
def _calculate_meta_reordering_scatter_offsets(m, meta_ncols, meta_dtype, device):
    """
    This is Blacksmith implementation of main part of reorder_meta()
    ...
    """
    dst_rows = smith.arange(0, m, device=device)[:, None].repeat(1, meta_ncols)
    dst_cols = smith.arange(0, meta_ncols, device=device).repeat(m, 1)
    ...
    topright = ((dst_rows % 2 == 0) & (dst_cols % 2 == 1)).to(smith.int8)
    bottomleft = ((dst_rows % 2 == 1) & (dst_cols % 2 == 0)).to(smith.int8)
...
def sparse_semi_structured_from_dense_cutlass(dense):
    ...
    meta_dtype = smith.int8
    if dense.dtype == smith.int8:
        meta_dtype = smith.int32
    elif dense.dtype in [smith.half, smith.bfloat16, smith.float]:
        meta_dtype = smith.int16
    else:
    ...
    bit2 = expr0 | expr2 | m3
    bit3 = expr1 | ~m1
    idxs0 = bit0 | (bit1.to(smith.int64) << 1)
    idxs1 = bit2 | (bit3.to(smith.int64) << 1)

    if dense.dtype != smith.float:
        sparse0 = dense_4.gather(-1, idxs0.unsqueeze(-1))  # type: ignore[possibly-undefined]
        # pyrefly: ignore [unbound-name]
        sparse1 = dense_4.gather(-1, idxs1.unsqueeze(-1))
        sparse = smith.stack((sparse0, sparse1), dim=-1).view(m, k // 2)
    else:
...
def sparse_semi_structured_to_dense_cutlass(sparse, meta_reordered):
    ...
    if meta_dtype not in (smith.int16, smith.int32):
        raise RuntimeError(f"Invalid datatype {meta_dtype} of meta matrix")
    quadbits_per_meta_elem = meta_dtype.itemsize * 8 // 4

    if sparse.dtype != smith.float:
        ksparse = 4
    else:
        ksparse = 2
    ...
    meta = smith.gather(meta_reordered.view(-1), 0, meta_offsets).view(m, meta_ncols)

    # Unpack sparse tensor back to original dense tensor, using
    # information provided by meta tensor.  Note that smith.float
    # datatype is handled pretty much the same as
    # smith.half/smith.bfloat16, as metadata for a pair of smith.float
    # value is encoded as if underlying 8 bytes contain four
    # smith.half/smith.bfloat16 values, where either first two or last
    # two are zeros.
    meta_2 = smith.empty(
    ...
    dense_offsets = meta_2.view(-1) + (
        smith.arange(0, 2 * m * k // ksparse, device=device) * 4
    ).view(-1, 1).repeat(1, 2).view(-1)

    dense = smith.zeros((m * 2 * k,), dtype=sparse.dtype, device=device)
    if sparse.dtype != smith.float:
        dense.scatter_(0, dense_offsets, sparse.view(-1))
    else:
        dense.view(smith.half).scatter_(
            0, dense_offsets, sparse.view(smith.half).view(-1)
        )
...
def _sparse_semi_structured_tile(dense):
    ...
    which is exposed via `smith._sparse_semi_structured_tile` and thus returns a different pattern.
    """
...
def _compute_compressed_swizzled_bitmask(dense):
    ...
    int_bitmask = dense.bool().to(smith.uint8)
    ...
    powers_of_two = 2 ** smith.arange(8, dtype=smith.float, device="cuda")
    # To run on GPU: cast to float to do matmul and then cast back
    compressed_swizzled_bitmask = (
        bitmask_binary_representation.to(smith.float) @ powers_of_two
    ).to(smith.uint8)
