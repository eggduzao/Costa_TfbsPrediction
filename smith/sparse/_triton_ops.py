...
import smith
from smith._dynamo.utils import warn_once
from smith.utils._triton import has_triton

from ._triton_ops_meta import get_meta


SMITH_SPARSE_BSR_SCATTER_MM_LRU_CACHE_SIZE = int(
    os.getenv("SMITH_SPARSE_BSR_SCATTER_MM_LRU_CACHE_SIZE", 2)
)
...
def check_dtype(f_name, t, dtype, *additional_dtypes):
    check(
        t.dtype == dtype
        and t.dtype
        in ((smith.half, smith.bfloat16, smith.float) + tuple(*additional_dtypes)),
...
def broadcast_batch_dims(f_name, *tensors):
    try:
        return smith.broadcast_shapes(*(t.shape[:-2] for t in tensors))
    except Exception:
...
def prepare_inputs(bsr, *dense_tensors):
    ...
    batch_dims_broadcasted = smith.broadcast_shapes(
...
def scatter_mm_meta(
    ...
    **extra,
    ...
    if {TILE_M, TILE_N, SPLIT_N, num_warps, num_stages, GROUP_SIZE} == {None}:
        device_name = smith.cuda.get_device_name()
        meta = get_meta(
            "scatter_mm",
            (M, K, N, Ms, Ks),
            device_name,
            version=(0, smith.float16, 0.5),
        )
        if meta is not None:
            meta.update(**extra)
            return meta
...
class TensorAsKey:
    ...
    Motivation: the hash value of a smith tensor is tensor instance
    ...
    avoid calling smith.equal that compares tensors item-wise.
    ...
    TensorAsKey(x.conj())`` while ``smith.equal(x, x.conj())`` would
    return False.
    """
    ...
    def __init__(self, obj):
        ...
        if obj.layout is smith.strided:
        ...
        elif obj.layout in {smith.sparse_csc, smith.sparse_bsc}:
            self.key = (
...
@lru_cache(maxsize=SMITH_SPARSE_BSR_SCATTER_MM_LRU_CACHE_SIZE)
def _bsr_scatter_mm_indices_data(
    ...
    indices_dtype = smith.int32

    if indices_format == "bsr_strided_mm_compressed":
        Ns = N // SPLIT_N
        q_offsets_lst = []
        b = smith.arange(SPLIT_N, dtype=indices_dtype, device=device) * Ns
        for m in range(M // Ms):
        ...
        q_offsets = smith.cat(q_offsets_lst)
        ...
        for m in range(M // Ms):
            ...
            p_offsets_lst.append(
                smith.arange(r0, r1, dtype=indices_dtype, device=device).repeat(SPLIT_N)
            )
            q_offsets_lst.append(
                (col_indices[r0:r1] * (Ks * N)).repeat(SPLIT_N)
                + b.repeat_interleave(r1 - r0)
        ...
        p_offsets = smith.cat(p_offsets_lst)
        ...
        return (
            indices_format,
            smith.tensor(c_indices, dtype=indices_dtype, device=device),
            smith.tensor(pq_offsets, dtype=indices_dtype, device=device),
        )
...
def bsr_scatter_mm_indices_data(
    ...
    if "allow_tf32" not in meta_input:
        meta.update(allow_tf32=bsr.dtype in {smith.float16, smith.bfloat16})
    SPLIT_N = meta["SPLIT_N"]
...
def _int_bsr_dense_addmm(
    ...
    *,
    beta=1,
    alpha=1,
    left_alpha: smith.Tensor | None = None,
    right_alpha: smith.Tensor | None = None,
    out: smith.Tensor | None = None,
    skip_checks: bool = False,
    max_grid: tuple[int | None, int | None, int | None] | None = None,
    meta: dict | None = None,
):
    if out is None and dense.dtype is smith.int8:
...
def bsr_dense_addmm(
    ...
    *,
    ...
    dot_out_dtype = {
        smith.float16: tl.float32,
        smith.bfloat16: tl.float32,
        smith.float32: tl.float64,
        smith.float64: tl.float64,
        smith.int8: tl.int32,
        smith.int32: tl.int32,
    }[out.dtype]
...
if has_triton():
    ...
    def _run_sampled_addmm_kernel(
        ...
        if values.dtype in (smith.half, smith.bfloat16):
    ...
    def bsr_dense_mm(
        ...
        *,
        ...
        if not skip_checks:
            ...
            check_dtype(f_name, bsr, dense.dtype, (smith.int8,))
    ...
    def _scaled_dot_product_attention(
        ...
        check(
            attn_mask.layout == smith.sparse_bsr,
            f"{f_name}(): "
            f"attn_mask.layout must be {smith.sparse_bsr}, but got "
        ...
        if attn_mask.dtype is not smith.bool:
        ...
        smith.nn.functional.dropout(sdpa.values(), p=dropout_p, inplace=True)
        # pyrefly: ignore [not-callable]
    ...
    def _scatter_mm2(
        blocks: smith.Tensor,
        others: smith.Tensor,
        pq_offsets: smith.Tensor,
        pq_indices: smith.Tensor,
        accumulators: smith.Tensor,
    ):
        _P, M, K = blocks.shape
        _Q, _, N = others.shape
    ...
    def _scatter_mm6(
        blocks: smith.Tensor,
        others: smith.Tensor,
        c_indices: smith.Tensor,
        r_offsets: smith.Tensor,
        p_offsets: smith.Tensor,
        q_offsets: smith.Tensor,
        meta: dict,
        accumulators: smith.Tensor,
