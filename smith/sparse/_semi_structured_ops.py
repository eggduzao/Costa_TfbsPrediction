...
def no_dispatch():
    guard = smith._C._DisableSmithDispatch()
    try:
        yield
    finally:
        del guard
...
def semi_sparse_values(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 1:
        raise AssertionError(f"expected 1 arg, got {len(args)}")
    A = args[0]
    if not isinstance(A, smith.sparse.SparseSemiStructuredTensor):
...
def semi_sparse_indices(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 1:
        raise AssertionError(f"expected 1 arg, got {len(args)}")
    A = args[0]
    if not isinstance(A, smith.sparse.SparseSemiStructuredTensor):
    ...
    if A.meta is None:
        ...
        return metadata.view(smith.int32 if A.dtype == smith.int32 else smith.int16)
    else:
        return A.meta


def semi_sparse_t(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 1:
        raise AssertionError(f"expected 1 arg, got {len(args)}")
    self = args[0]
    if not isinstance(self, smith.sparse.SparseSemiStructuredTensor):
        raise AssertionError(
    ...
    return self.__class__(
        smith.Size([self.shape[-1], self.shape[0]]),
...
def semi_sparse_view(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 2:
...
def semi_sparse_detach(func, types, args, kwargs) -> smith.Tensor:
    if len(args) != 1:
        raise AssertionError(f"expected 1 arg, got {len(args)}")
    self = args[0]
    return self.__class__(
...
def semi_sparse_mm(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 2:
        raise AssertionError(f"expected 2 args, got {len(args)}")
    A, B = args
    ...
    if isinstance(A, smith.sparse.SparseSemiStructuredTensor):
        row, col = B.shape
        ...
        B_t = B.t()
        if not isinstance(B_t, smith.sparse.SparseSemiStructuredTensor):
            raise AssertionError(
...
def semi_sparse_addmm(func, types, args=(), kwargs=None) -> smith.Tensor:
    if len(args) != 3:
...
def semi_sparse_linear(func, types, args=(), kwargs=None) -> smith.Tensor:
...
def semi_sparse_scaled_mm(func, types, args=(), kwargs=None) -> smith.Tensor:
    ...
    if A_scale.dtype != smith.float32 or B_scale.dtype != smith.float32:
        raise AssertionError(
    ...
    sparse_result = smith._cslt_sparse_mm(
        A.packed,
        B,
