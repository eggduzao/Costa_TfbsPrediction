...
smith/sparse/_triton_ops.py, see bsr_dense_addmm_meta function, for
instance.
...
the blacksmith development tree::

  cd /path/to/blacksmith
  python -m pip install --no-build-isolation -v -e .
  python smith/sparse/_triton_ops_meta.py
...
modified when the script is run. Create a blacksmith PR with the
...
addmm-based operations such as smith.addmm, smith.mm,
smith.nn.functional.linear will benefit from using the computed
...
def get_meta(op, key, device_name=None, version=(0, smith.float16, 0.5), exact=False):
...
def dump():
    ...
    def sort_key(key):
        ...
        version = tuple(
            (str(item) if isinstance(item, smith.dtype) else item) for item in version
        )
...
def create_blocked_tensor(B, M, N, blocksize, sparsity, dtype, device):
    ...
    A = smith.bernoulli(
        smith.full(shape, 1 - sparsity, dtype=smith.float32, device=device)
    ).to(dtype)
...
def optimize_scatter_mm(
    m, k, n, bm, bk, dtype=smith.float16, device="cuda", sparsity=0.5, force=False
):
    import triton
    ...
    def step_meta_parameter(name, value, direction, meta, m=m, n=n, k=k, bm=bm, bk=bk):
        ...
        if name == "SPLIT_N" and n % next_value != 0:
            return value
        # Hard-skip parameter combinations that break CUDA state for blacksmith:
        if (dtype, name, next_value, m, n, k, bm, bk) in {
            (smith.float32, "num_warps", 32, 256, 256, 256, 16, 16),
            (smith.float32, "num_warps", 16, 256, 256, 256, 32, 32),
            (smith.float32, "num_warps", 16, 256, 256, 256, 64, 64),
            (smith.float32, "num_warps", 16, 256, 256, 256, 128, 128),
            (smith.float32, "num_warps", 16, 512, 512, 256, 128, 128),
...
def tune_bsr_dense_addmm(
    ...
    *,
    ...
    if opname == "_int_bsr_dense_addmm":
        from smith.sparse._triton_ops import _int_bsr_dense_addmm as bsr_dense_addmm
    else:
...
def main(op="scatter_mm", force=False, dtype=smith.float16, verbose=True):
    import itertools
            ...
            for sparsity1 in sparsity_lst:
                ...
                for sparsity in sparsity_lst:
                    ...
                    def bench(meta, bsr=bsr, dense=dense):
                        ...
                        if op == "scatter_mm":
                            from smith.sparse._triton_ops import (
                                bsr_scatter_mm,
...
_operation_device_version_data: dict[Any, dict] = {
    ...
    ("_int_bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.int8, 0.5)): {
    ...
    ("_int_bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.int8, 0.56)): {
    ...
    ("_int_bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.int8, 1.0)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.bfloat16, 0.5)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.bfloat16, 0.56)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.float16, 0.5)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.float16, 0.56)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.float32, 0.5)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.float32, 0.56)): {
    ...
    ("bsr_dense_addmm", "NVIDIA A100-SXM4-80GB", (0, smith.int8, 0.5)): {
        (1280, 5120, 65792, 32, 32, False, True, True): (1, 1028, 1, 8),
        (1280, 5120, 65792, 32, 32, True, False, True): (1, 514, 3, 2),
    ...
    ("scatter_mm", "NVIDIA A100-SXM4-80GB", (0, smith.bfloat16, 0.5)): {
    ...
    ("scatter_mm", "NVIDIA A100-SXM4-80GB", (0, smith.float16, 0.5)): {
    ...
    ("scatter_mm", "NVIDIA A100-SXM4-80GB", (0, smith.float32, 0.5)): {
...
if __name__ == "__main__":
    for dtype in [smith.int8]:
        for op in ["_int_bsr_dense_addmm"]:
            main(op=op, force=False, dtype=dtype)
    for dtype in [smith.float16, smith.bfloat16, smith.float32, smith.int8]:
