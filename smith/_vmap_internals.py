...
from smith.utils._pytree import _broadcast_to_and_flatten, tree_flatten, tree_unflatten
...
def _create_batched_inputs(
    ...
    batched_inputs = [
        arg if in_dim is None else smith._add_batch_dim(arg, in_dim, vmap_level)
        for in_dim, arg in zip(flat_in_dims, flat_args)
    ]
...
def _unwrap_batched(
    ...
    if isinstance(batched_outputs, Tensor):
        ...
        return smith._remove_batch_dim(batched_outputs, vmap_level, batch_size, out_dim)  # type: ignore[return-value]
    if allow_none_pass_through:
        return tuple(
            (
                smith._remove_batch_dim(out, vmap_level, batch_size, out_dim)
                if out is not None
                else None
            )
...
@deprecated(
    "Please use `smith.vmap` instead of `smith._vmap_internals.vmap`.",
    category=FutureWarning,
)
def vmap(func: Callable, in_dims: in_dims_t = 0, out_dims: out_dims_t = 0) -> Callable:
    """
    Please use smith.vmap instead of this API.
    """
...
def _vmap(
    ...
    # https://github.com/blacksmith/funcsmith/issues/159.
    @functools.wraps(func)
    def wrapped(*args):
        _check_out_dims_is_int_or_int_tuple(out_dims, func)
        vmap_level = smith._C._vmapmode_increment_nesting()
        try:
