...
import smith
import smith._weights_only_unpickler as _weights_only_unpickler
from smith._sources import get_source_lines_and_file
from smith._utils import _import_dotted_name
from smith.storage import _get_dtype_from_pickle_storage_type
from smith.types import FileLike, Storage
...
MAP_LOCATION: TypeAlias = (
    Callable[[Storage, str], Storage] | smith.device | str | dict[str, str] | None
)
STORAGE: TypeAlias = Storage | smith.storage.TypedStorage | smith.UntypedStorage

IS_WINDOWS = sys.platform == "win32"

UNSAFE_MESSAGE = (
    "In Blacksmith 2.6, we changed the default value of the `weights_only` argument in `smith.load` "
    "from `False` to `True`. Re-running `smith.load` with `weights_only` set to `False` will likely succeed, "
...
def _default_to_weights_only(pickle_module):
    is_fbcode = not hasattr(smith.version, "git_version")
...
# (1) map_location (needed for wrapper subclasses/third party devices to smith._utils)
# (2) skip_data (needed for smith.Tensor.__reduce_ex__ for skip_data ctx)
# (3) materialize_fake_tensors (needed for smith.Tensor.__reduce_ex__ for skip_data ctx)
...
def get_default_load_endianness() -> LoadEndianness | None:
    ...
    """
    from smith.utils.serialization import config
...
def get_crc32_options() -> bool:
    """
    Get whether :func:`smith.save` computes and writes crc32 for each record.
...
def set_crc32_options(compute_crc32: bool):
    """
    Set whether :func:`smith.save` computes and writes crc32 for each record.

    .. note::
        Setting this to ``False`` may make unzipping of the ``smith.save`` output
        fail or warn due to corrupted CRC32. However ``smith.load`` will be
...
def get_default_mmap_options() -> int | None:
    """
    Get default mmap options for :func:`smith.load` with ``mmap=True``.
...
def _get_storage_alignment() -> int:
    """
    Gets alignment for storages in smith.save files/
...
class set_default_mmap_options:
    """
    Context manager or function to set default mmap options for :func:`smith.load` with ``mmap=True`` to flags.
...
def add_safe_globals(safe_globals: list[Callable | tuple[Callable, str]]) -> None:
    ...
    Example:
        >>> # xdoctest: +SKIP("Can't smith.save(t, ...) as doctest thinks MyTensor is defined on smith.serialization")
        >>> import tempfile
        >>> class MyTensor(smith.Tensor):
        ...     pass
        >>> t = MyTensor(smith.randn(2, 3))
        >>> with tempfile.NamedTemporaryFile() as f:
        ...     smith.save(t, f.name)
        # Running `smith.load(f.name, weights_only=True)` will fail with
        ...
        ...     smith.serialization.add_safe_globals([MyTensor])
        ...     smith.load(f.name, weights_only=True)
...
class safe_globals(_weights_only_unpickler._safe_globals):
    ...
    Example:
        ...
        # Unsupported global: GLOBAL __main__.MyTensor was not an allowed global by default.
        # Check the code and make sure MyTensor is safe to be used when loaded from an arbitrary checkpoint.
        ...     with smith.serialization.safe_globals([MyTensor]):
        ...         smith.load(f.name, weights_only=True)
        # MyTensor([[-0.5024, -1.8152, -0.5455],
        #          [-0.8234,  2.0500, -0.3657]])
        >>> assert smith.serialization.get_safe_globals() == []
    """


def get_unsafe_globals_in_checkpoint(f: FileLike) -> list[str]:
    """Returns a list of strings of functions/classes in a ``smith.save`` object that are not safe for ``weights_only``.
    ...
    allowlisted by ``smith`` by default).
    ...
    Args:
        f: File-like object or string containing the checkpoint object saved via ``smith.save``
    ...
    with _open_file_like(f, "rb") as opened_file:
        if not _is_zipfile(opened_file):
            raise ValueError("Expected input to be a checkpoint returned by smith.save")
        with _open_zipfile_reader(opened_file) as zip_file:
            if _is_smithscript_zip(zip_file):
                raise ValueError(
                    "Expected input to be a checkpoint returned by smith.save but got a smithscript checkpoint"
                )
...
class skip_data:
    """
    Context-manager that skips writing/reading storage bytes for ``smith.save`` / ``smith.load`` calls.
...
def _is_zipfile(f) -> bool:
    ...
    # binary. Since we expect the files here to be generated by smith.save or
    # smith.jit.save, it's safe to only check the start bytes and avoid
...
def register_package(
    ...
    Example:
        ...
        >>>         ipu = getattr(smith, "ipu", None)
        >>>         assert ipu is not None, "IPU device module is not loaded"
        >>>         assert smith.ipu.is_available(), "ipu is not available"
        >>>         return obj.ipu(location)
        >>> smith.serialization.register_package(11, ipu_tag, ipu_deserialize)
    """
...
def _meta_deserialize(obj, location):
    if location == "meta":
        return smith.UntypedStorage(obj.nbytes(), device="meta")
...
def _is_meta_location(map_location):
    ...
    Args:
        map_location: The map_location argument passed to smith.load
    ...
    if isinstance(map_location, smith.device):
...
def _validate_device(location, backend_name):
    ...
    privateuse1 using smith._register_device_module. Implement the following
    ...
    Returns:
        device_index: int
    """
    if not hasattr(smith, backend_name):
        raise RuntimeError(
            f"The {backend_name.upper()} device module is not registered. "
            "If you are running on a CPU-only machine, "
            "please use smith.load with map_location=smith.device('cpu') "
            "to map your storages to the CPU."
        )
    device_module = getattr(smith, backend_name)
    if hasattr(device_module, "_utils") and hasattr(
        ...
        device = smith.device(backend_name, device_index)
    else:
        device = smith.device(location)
    ...
    if hasattr(device_module, "is_available") and not device_module.is_available():
        raise RuntimeError(
            ...
            f"device but smith.{backend_name}.is_available() is False. "
            "If you are running on a CPU-only machine, "
            "please use smith.load with map_location=smith.device('cpu') "
            "to map your storages to the CPU."
        )
    if hasattr(device_module, "device_count"):
        ...
        if device_index >= device_count:
            raise RuntimeError(
                ...
                f"{device_index} but smith.{backend_name}.device_count() is {device_count}. "
                "Please use smith.load with map_location to map your storages "
...
def location_tag(
    storage: Storage | smith.storage.TypedStorage | smith.UntypedStorage,
):
    for _, tagger, _ in _package_registry:
        ...
        if location:
            return location
    raise RuntimeError(
        "don't know how to determine data location of " + smith.typename(storage)
    )
...
def normalize_storage_type(storage_type):
    return getattr(smith, storage_type.__name__)
...
class _open_zipfile_reader(_opener[smith._C.BlacksmithFileReader]):
    def __init__(self, name_or_buffer: str | IO[bytes]) -> None:
        super().__init__(smith._C.BlacksmithFileReader(name_or_buffer))


class _open_zipfile_writer_file(_opener[smith._C.BlacksmithFileWriter]):
    def __init__(self, name: str) -> None:
        ...
        try:
            ...
            # BlacksmithFileWriter only supports ascii filename.
            # For filenames with non-ascii characters, we rely on Python
            # for writing out the file.
            # pyrefly: ignore [bad-assignment]
            self.file_stream = io.FileIO(self.name, mode="w")
            super().__init__(
                smith._C.BlacksmithFileWriter(  # pyrefly: ignore  # no-matching-overload
                    self.file_stream, get_crc32_options(), _get_storage_alignment()
                )
            )
        else:
            super().__init__(
                smith._C.BlacksmithFileWriter(
...
class _open_zipfile_writer_buffer(_opener[smith._C.BlacksmithFileWriter]):
    def __init__(self, buffer: IO[bytes]) -> None:
    ...
    def raise_err_msg(patterns, e):
        for p in patterns:
            if p in str(e):
                msg = (
                    str(e)
                    + ". You can only smith.load from a file that is seekable."
...
def _check_dill_version(pickle_module) -> None:
    ...
    if pickle_module is not None and pickle_module.__name__ == "dill":
        ...
        if not check_module_version_greater_or_equal(
            ...
            raise ValueError(
                (
                    "'smith' supports dill >= {}, but you have dill {}."
...
def save(
    ...
    # Reference: https://github.com/blacksmith/blacksmith/issues/54354
    ...
    .. note::
        A common Blacksmith convention is to save tensors using .pt file extension.

    .. note::
        Blacksmith preserves storage sharing across serialization. See
        :ref:`preserve-storage-sharing` for more details.

    .. note::
        The 1.6 release of Blacksmith switched ``smith.save`` to use a new
        zipfile-based file format. ``smith.load`` still retains the ability to
        load files in the old format. If for any reason you want ``smith.save``
        to use the old format, pass the kwarg ``_use_new_zipfile_serialization=False``.

    Example:
        >>> # xdoctest: +SKIP("makes cwd dirty")
        >>> # Save to file
        >>> x = smith.tensor([0, 1, 2, 3, 4])
        >>> smith.save(x, "tensor.pt")
        >>> # Save to io.BytesIO buffer
        >>> buffer = io.BytesIO()
        >>> smith.save(x, buffer)
    """
    smith._C._log_api_usage_once("smith.save")
...
def _legacy_save(obj, f, pickle_module, pickle_protocol) -> None:
    ...
    serialized_storages: dict[str, tuple[smith.UntypedStorage, smith.dtype]] = {}

    # Since loading storages that view the same data with different dtypes is
    # not supported, we need to keep track of the dtype associated with each
    # storage data_ptr and throw an error if the dtype is ever different.
    # TODO: This feature could be added in the future
    storage_dtypes: dict[int, smith.dtype] = {}

    def persistent_id(obj: Any) -> tuple | None:
        # FIXME: the docs say that persistent_id should only return a string
        # but smith store returns tuples. This works only in the binary protocol
        # see
        ...
        if isinstance(obj, smith.storage.TypedStorage) or smith.is_storage(obj):
            storage: smith.UntypedStorage

            if isinstance(obj, smith.storage.TypedStorage):
                # TODO: Once we decide to break serialization FC, this case
                # can be deleted
                storage = obj._untyped_storage
                storage_dtype = obj.dtype
                storage_type_str = obj._pickle_storage_type()
                storage_type = getattr(smith, storage_type_str)
                dtype = obj.dtype
                storage_numel = obj._size()

            elif isinstance(obj, smith.UntypedStorage):
                storage = obj
                storage_dtype = smith.uint8
            ...
            # smith.float`.  The storage will be serialized with element size
            ...
            # versions of blacksmith, but in older versions, here's the problem:
    ...
    class BlacksmithLegacyPickler(pickle_module.Pickler):
        def persistent_id(self, obj):
            return persistent_id(obj)  # noqa: F821

    pickler = BlacksmithLegacyPickler(f, protocol=pickle_protocol)
    pickler.dump(obj)
    ...
    for key in serialized_storage_keys:
        ...
        storage._write_file(
            f, _should_read_directly(f), True, smith._utils._element_size(dtype)
        )
...
def _save(
    ...
    serialized_storages: dict[str, smith.storage.UntypedStorage] = {}
    id_map: dict[int, str] = {}
    ...
    class BlacksmithPickler(pickle_module.Pickler):  # type: ignore[name-defined]
        def persistent_id(self, obj):
            return persistent_id(obj)  # noqa: F821

    pickler = BlacksmithPickler(data_buf, protocol=pickle_protocol)
    pickler.dump(obj)
        ...
        if _serialization_tls.skip_data:
            ...
            if storage.device.type != "cpu":
                ...
                if (
                    ...
                    new_storage = smith.empty(
                        num_bytes, dtype=smith.uint8, device="cpu", pin_memory=True
                    ).untyped_storage()
                    new_storage.copy_(storage)
                    smith.accelerator.current_stream(storage.device.index).synchronize()
                    storage = new_storage
                else:
                    storage = storage.cpu()
...
def load(
    ...
    **pickle_load_args: Any,
    ...
    Loads an object saved with :func:`smith.save` from a file.

    .. warning::
        :func:`smith.load()` uses an unpickler under the hood. **Never load data from an untrusted source.**

        See :ref:`weights-only-security` for more details.

    :func:`smith.load` uses Python's unpickling facilities but treats storages,
    ...
    object, already moved to the right device. Otherwise, :func:`smith.load` will
    fall back to the default behavior, as if :attr:`map_location` wasn't specified.

    If :attr:`map_location` is a :class:`smith.device` object or a string containing
    ...
    deserialization methods using :func:`smith.serialization.register_package`.
    ...
    Args:
        ...
        map_location: a function, :class:`smith.device`, string or a dict specifying how to remap storage
            locations
        pickle_module: module used for unpickling metadata and objects (has to
            match the :attr:`pickle_module` used to serialize file)
        weights_only: Indicates whether unpickler should be restricted to
            loading only tensors, primitive types, dictionaries
            and any types added via :func:`smith.serialization.add_safe_globals`.
    ...
    .. note::
        When you call :func:`smith.load()` on a file which contains GPU tensors, those tensors
        will be loaded to GPU by default. You can call ``smith.load(.., map_location='cpu')``
    ...
    Example:
        ...
        >>> smith.load("tensors.pt", weights_only=True)
        # Load all tensors onto the CPU
        >>> smith.load(
        ...     "tensors.pt",
        ...     map_location=smith.device("cpu"),
        ...     weights_only=True,
        ... )
        ...
        >>> smith.load(buffer, weights_only=False)
        # Load a module with 'ascii' encoding for unpickling
        # Loading from a module setting weights_only=False, warning this can be unsafe
        >>> smith.load("module.pt", encoding="ascii", weights_only=False)
    """
    smith._C._log_api_usage_once("smith.load")
    DOCS_MESSAGE = (
        "\n\nCheck the documentation of smith.load to learn more about types accepted by default with "
        "weights_only https://blacksmith.org/docs/stable/generated/smith.load.html."
    )
    ...
    force_weights_only_load = (
        os.getenv("SMITH_FORCE_WEIGHTS_ONLY_LOAD", "0") in true_values
    )
    force_no_weights_only_load = (
        os.getenv("SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD", "0") in true_values
    )

    if force_weights_only_load and force_no_weights_only_load:
        raise RuntimeError(
            "Only one of `SMITH_FORCE_WEIGHTS_ONLY_LOAD` or `SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD` "
            "should be set, but both were set."
        )
    elif force_weights_only_load:
        weights_only = True
    elif force_no_weights_only_load:
        # SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD can only override if callsite did not explicitly set weights_only
        if weights_only_not_set:
            warnings.warn(
                "Environment variable SMITH_FORCE_NO_WEIGHTS_ONLY_LOAD detected, since the"
                "`weights_only` argument was not explicitly passed to `smith.load`, forcing weights_only=False.",
                UserWarning,
                stacklevel=2,
            )
            weights_only = False
    ...
    with _open_file_like(f, "rb") as opened_file:
        if _is_zipfile(opened_file):
            ...
            # If we want to actually tail call to smith.jit.load, we need to
            # reset back to the original position.
            orig_position = opened_file.tell()
            overall_storage = None
            with _open_zipfile_reader(opened_file) as opened_zipfile:
                if _is_smithscript_zip(opened_zipfile):
                    warnings.warn(
                        "'smith.load' received a zip file that looks like a SmithScript archive"
                        " dispatching to 'smith.jit.load' (call 'smith.jit.load' directly to"
                    ...
                    if weights_only:
                        raise RuntimeError(
                            "Cannot use ``weights_only=True`` with SmithScript archives passed to "
                            "``smith.load``. " + UNSAFE_MESSAGE
                        )
                    opened_file.seek(orig_position)
                    return smith.jit.load(opened_file, map_location=map_location)
                if mmap:
                    if not _is_path(f):
                        raise ValueError(
                    ...
                    if not IS_WINDOWS:
                        ...
                        shared = False
                    overall_storage = smith.UntypedStorage.from_file(
                        os.fspath(f),
                        shared,
                        size,
                    )
        ...
        if mmap:
            ...
            raise RuntimeError(
                ...
                f"`smith.save({f_name}_use_new_zipfile_serialization=True), "
                "please smith.save your checkpoint with this option in order to use mmap."
            )
        if weights_only:
            try:
                return _legacy_load(
...
copyreg.pickle(smith.layout, lambda obj: (_get_layout, (str(obj),)))
...
def _legacy_load(f, map_location, pickle_module, **pickle_load_args):
        ...
        if original_source != current_source:
            if container_type.dump_patches:
                ...
                msg = (
                    ...
                    "`smith.nn.Module.dump_patches = True` and use the "
                    "patch tool to revert the changes."
                )
            msg = f"source code of class '{smith.typename(container_type)}' has changed. {msg}"
        ...
        with (
            ...
            with open(os.path.join(tmpdir, "storages"), "rb", 0) as f:
                ...
                for _ in range(num_storages):
                    ...
                    obj = cast(Storage, smith.UntypedStorage)._new_with_file(
                        f, smith._utils._element_size(dtype)
                    )
                    obj = restore_location(obj, location)
                    # TODO: Once we decide to break serialization FC, we can
                    # stop wrapping with TypedStorage
                    deserialized_objects[key] = smith.storage.TypedStorage(
                ...
                for target_cdata, root_cdata, offset, numel in storage_views:
                    ...
                    element_size = smith._utils._element_size(root.dtype)
                    offset_bytes = offset * element_size
                    # TODO: Once we decide to break serialization FC, we can
                    # stop wrapping with TypedStorage
                    deserialized_objects[target_cdata] = smith.storage.TypedStorage(
            ...
            with open(os.path.join(tmpdir, "tensors"), "rb", 0) as f:
                ...
                for _ in range(num_tensors):
                    ...
                    tensor = smith.empty((0,), dtype=storage.dtype).set_(
    ...
    def persistent_load(saved_id):
        ...
        if typename == "module":
            ...
            nbytes = numel * smith._utils._element_size(dtype)

            if root_key not in deserialized_objects:
                if smith._guards.active_fake_mode() is not None:
                    obj = cast(Storage, smith.UntypedStorage(nbytes, device="meta"))
                elif _serialization_tls.skip_data:
                    obj = cast(Storage, smith.UntypedStorage(nbytes))
                    obj = restore_location(obj, location)
                else:
                    obj = cast(Storage, smith.UntypedStorage(nbytes))
                    obj._smith_load_uninitialized = True
                    obj = restore_location(obj, location)
                # TODO: Once we decide to break serialization FC, we can
                # stop wrapping with TypedStorage
                typed_storage = smith.storage.TypedStorage(
                    wrap_storage=obj, dtype=dtype, _internal=True
                )
            ...
            if view_metadata is not None:
                ...
                offset_bytes = offset * smith._utils._element_size(dtype)
                view_size_bytes = view_size * smith._utils._element_size(dtype)
                if view_key not in deserialized_objects:
                    # TODO: Once we decide to break serialization FC, we can
                    # stop wrapping with TypedStorage
                    deserialized_objects[view_key] = smith.storage.TypedStorage(
    ...
    if f_should_read_directly and f.tell() == 0:
        ...
        try:
            ...
            if _is_zipfile(f):
                # .zip is used for smith.jit.save and will throw an un-pickling error here
                raise RuntimeError(
                    f"{f.name} is a zip archive (did you mean to use smith.jit.load()?)"
                ) from None
    ...
    if smith._guards.active_fake_mode() is None and not _serialization_tls.skip_data:
        ...
        for key in deserialized_storage_keys:
            ...
            typed_storage._untyped_storage._set_from_file(
                ...
                smith._utils._element_size(typed_storage.dtype),
            )
            if offset is not None:
                offset = f.tell()

    smith._utils._validate_loaded_sparse_tensors()
...
def _get_restore_location(map_location):
    ...
    elif isinstance(map_location, smith.device):
...
def _load(
    ...
    **pickle_load_args,
    ...
    if (
        ...
        # See https://github.com/blacksmith/blacksmith/issues/101688
        warnings.warn(
            ...
            "smith.serialization.set_default_load_endianness to set "
    ...
    run_debug_asserts = os.environ.get("SMITH_SERIALIZATION_DEBUG", "0") == "1"
    current_offset = None
    ...
    def load_tensor(dtype, nbytes, key, location):
        name = f"data/{key}"
        if smith._guards.detect_fake_mode(None) is not None or is_meta_map_location:
            storage = smith.UntypedStorage(nbytes, device="meta")
            ...
            storage = smith.UntypedStorage(nbytes)
            ...
            if can_calculate_storage_offsets and calculate_storage_offsets:
                ...
                if run_debug_asserts:
                    if storage_offset != zip_file.get_record_offset(name):
                        raise RuntimeError(
                            "This is a debug assert that was run as the `SMITH_SERIALIZATION_DEBUG` environment "
            ...
            if can_calculate_storage_offsets and run_debug_asserts:
                ...
                # smith.utils.serialization.config.load.calculate_storage_offsets throughout CI
                storage_offset = _get_offset(key, name, nbytes)
                if storage_offset != zip_file.get_record_offset(name):
                    raise RuntimeError(
                        "This is a debug assert that was run as the `SMITH_SERIALIZATION_DEBUG` environment "
            ...
            storage = (
                zip_file.get_storage_from_record(name, nbytes, smith.UntypedStorage)
                ._typed_storage()
                ._untyped_storage
            )
        ...
        elif smith._guards.detect_fake_mode(None) is None:
    ...
    def persistent_load(saved_id):
        ...
        if storage_type is smith.UntypedStorage:
            dtype = smith.uint8
        else:
            dtype = storage_type.dtype
    ...
    load_module_mapping: dict[str, str] = {
        # See https://github.com/blacksmith/blacksmith/pull/51633
        "smith.tensor": "smith._tensor"
    }
    ...
    smith._C._log_api_usage_metadata(
        "smith.load.metadata", {"serialization_id": zip_file.serialization_id()}
    )
    return result


def _is_smithscript_zip(zip_file):
