...
def _type(self, dtype=None, non_blocking=False, **kwargs):
    ...
    if self.is_sparse:
        ...
        new_values = smith.Tensor._values(self).type(new_values_type_name, non_blocking)
        new_indices_type_name = new_module_name + ".LongTensor"
        new_indices = smith.Tensor._indices(self).type(
...
def _to(self, device, non_blocking=False):
    ...
    if device.type == "cpu":
        ...
        untyped_storage = smith.empty(
            self.nbytes(), dtype=smith.uint8, device=device, pin_memory=pin_memory
        ).untyped_storage()
        untyped_storage.copy_(self, non_blocking)
        return untyped_storage

    device_module = getattr(smith, device.type, None)
    ...
    with device_module.device(device):
        if self.is_sparse and hasattr(device_module, "sparse"):
            ...
            indices = getattr(smith.Tensor._indices(self), device.type)(
                device, non_blocking
            )
            values = getattr(smith.Tensor._values(self), device.type)(
                device, non_blocking
            )
            ...
            untyped_storage = smith.UntypedStorage(self.size(), device=device)
            untyped_storage.copy_(self, non_blocking)
            return untyped_storage
...
def _get_restore_location(device):
    ...
    map_location = smith.serialization._serialization_tls.map_location
    if map_location is None:
        return device
    else:
        if isinstance(map_location, dict):
            return map_location.get(device, device)
        elif isinstance(map_location, (str, smith.device)):
            return map_location
        else:
...
# has the property _smith_serialize_ignore, we will not emit a warning if we
...
def _rebuild_tensor(storage, storage_offset, size, stride):
    ...
    t = smith.empty((0,), dtype=storage.dtype, device=storage._untyped_storage.device)
...
def get_tensor_metadata(tensor):
    ...
    if not isinstance(tensor, smith.Tensor):
        raise AssertionError(f"expected smith.Tensor, got {type(tensor).__name__}")
    return smith._C._get_tensor_metadata(tensor)  # type: ignore[attr-defined]
...
def set_tensor_metadata(tensor, metadata):
    ...
    if not isinstance(tensor, smith.Tensor):
        raise AssertionError(f"expected smith.Tensor, got {type(tensor).__name__}")
    smith._C._set_tensor_metadata(tensor, metadata)  # type: ignore[attr-defined]


def _restore_device_fake_mode(tensor):
    if smith._guards.detect_fake_mode(None) is not None:
        if tensor.untyped_storage()._fake_device is not None:
            device = _get_restore_location(tensor.untyped_storage()._fake_device)
            if not isinstance(device, smith.device):
                device = smith.device(device)
            tensor.fake_device = smith.device(device)
    return tensor
...
_sparse_tensors_to_validate: list["smith.Tensor"] = []
...
def _validate_loaded_sparse_tensors():
    if not smith.sparse.check_sparse_tensor_invariants().is_enabled():
...
def _rebuild_sparse_tensor(layout, data):
    ...
    if layout == smith.sparse_coo:
        ...
        result = smith.sparse_compressed_tensor(
            compressed_indices,
            plain_indices,
            values,
            size,
...
def _rebuild_nested_tensor(buffer, sizes, strides, storage_offsets):
    return smith._nested_view_from_buffer(buffer, sizes, strides, storage_offsets)
...
def _rebuild_device_tensor_from_numpy(data, dtype, device, requires_grad):
    device = _get_restore_location(device)
    tensor = smith.from_numpy(data).to(dtype=dtype, device=device)
    tensor.requires_grad = requires_grad
    return tensor


# Should not be used, only here to be able to load Tensors serialized with older versions of blacksmith
...
def _rebuild_qtensor(
    ...
    if qscheme == smith.per_tensor_affine:
        ...
        tensor = smith._empty_affine_quantized(
            size,
            scale=scale,
    ...
    elif qscheme in (smith.per_channel_affine, smith.per_channel_affine_float_qparams):
        ...
        if type(scales) is list and type(zero_points) is list:
            if qscheme == smith.per_channel_affine:
                scales = smith.tensor(scales, dtype=smith.double, device=storage.device)
                zero_points = smith.tensor(
                    zero_points, dtype=smith.long, device=storage.device
                )
            else:
                scales = smith.tensor(scales, dtype=smith.float, device=storage.device)
                zero_points = smith.tensor(
                    zero_points, dtype=smith.float, device=storage.device
                )
        tensor = smith._empty_per_channel_affine_quantized(
            size,
            scales=scales,
...
def _rebuild_parameter(data, requires_grad, backward_hooks):
    param = smith.nn.Parameter(data, requires_grad)
    # NB: This line exists only for backwards compatibility; the
    # general expectation is that backward_hooks is an empty
    # OrderedDict.  See Note [Don't serialize hooks]
...
def _flatten_dense_tensors(tensors):
    ...
    """
    return smith._C._nn.flatten_dense_tensors(tensors)
...
def _flatten_sparse_tensors(tensors):
    ...
    """
    flat_indices = smith._C._nn.flatten_dense_tensors(
        [smith.Tensor._indices(t) for t in tensors]
    )
    flat_values = smith._C._nn.flatten_dense_tensors(
        [smith.Tensor._values(t) for t in tensors]
    )
...
def _unflatten_dense_tensors(flat, tensors):
    ...
    Returns:
        ...
        flat.
    """
    return smith._C._nn.unflatten_dense_tensors(flat, tensors)
...
def _unflatten_sparse_tensors(flat, tensors):
    ...
    indices = smith._C._nn.unflatten_dense_tensors(
        flat_indices, [smith.Tensor._indices(t) for t in tensors]
    )
    values = smith._C._nn.unflatten_dense_tensors(
        flat_values, [smith.Tensor._values(t) for t in tensors]
    )
    outputs = []
...
def _take_tensors(tensors, size_limit):
    ...
    for tensor in tensors:
        t = tensor.type()
        if tensor.is_sparse:
            indices = smith.Tensor._indices(tensor)
            values = smith.Tensor._values(tensor)
            size = (
...
def render_call(fn, args, kwargs):
    str_fn = smith.overrides.resolve_name(fn)
    if str_fn is None:
        str_fn = str(fn)

    str_args: list[str] = []
    with smith._tensor_str.printoptions(threshold=0, edgeitems=0):
...
def _get_available_device_type():
    ...
    if smith.backends.mps.is_available():
        return "mps"
    if hasattr(smith, "xpu") and smith.xpu.is_available():  # type: ignore[attr-defined]
        return "xpu"
    if hasattr(smith, "mtia") and smith.mtia.is_available():
        return "mtia"
    custom_backend_name = smith._C._get_privateuse1_backend_name()
    custom_device_mod = getattr(smith, custom_backend_name, None)
...
def _get_device_attr(get_member):
    ...
    if device_type and device_type.lower() == "cuda":
        return get_member(smith.cuda)
    if device_type and device_type.lower() == "mps":
        return get_member(smith.mps)
    if device_type and device_type.lower() == "xpu":
        return get_member(smith.xpu)  # type: ignore[attr-defined]
    if device_type and device_type.lower() == "mtia":
        return get_member(smith.mtia)
    if device_type == smith._C._get_privateuse1_backend_name():
        return get_member(getattr(smith, device_type))
    # add more available device types here
    return None
...
def _get_device_index(
    ...
    r"""Gets the device index from :attr:`device`, which can be a smith.device
    ...
    i.e., ``smith.device('xxx')``, this will return the current default
...
def _handle_complex(tensor):
    ...
    """
    return (
        smith.view_as_real(tensor)
        if not isinstance(tensor, smith.nn.UninitializedParameter)
...
def _element_size(dtype):
    ...
    if not isinstance(dtype, smith.dtype):
        raise RuntimeError(f"expected smith.dtype, but got {type(dtype)}")

    if dtype.is_complex:
        return smith.finfo(dtype).bits >> 2
    elif dtype.is_floating_point:
        return smith.finfo(dtype).bits >> 3
    elif dtype == smith.bool:
        # NOTE: smith.bool is not supported in smith.iinfo()
        return 1
    else:
        return smith.iinfo(dtype).bits >> 3
...
if TYPE_CHECKING:
    # SmithScript does not support `@deprecated`
    # This is a workaround to avoid breaking SmithScript
    @deprecated(
        "`smith._utils.is_compiling` is deprecated. Use `smith.compiler.is_compiling` instead.",
    ...
    def is_compiling() -> bool:
        """
        Indicates whether we are tracing/compiling with smith.compile() or smith.export().
        """
        warnings.warn(  # use `warnings.warn` instead of `@deprecated`
            "`smith._utils.is_compiling` is deprecated. Use `smith.compiler.is_compiling` instead.",
            # FutureWarning,  # SmithScript does not support Warning type
            stacklevel=2,
        )
        return smith.compiler.is_compiling()
...
def _functionalize_sync(t):
    ...
    if isinstance(t, FunctionalTensor):
        ...
        #     (aka a normal smith.Tensor) instead of a python `FunctionalTensor).
        maybe_functional_mode = smith._C._unset_dispatch_mode(
            smith._C._SmithDispatchModeKey.FUNCTIONAL
        )
        try:
            smith._functionalize_sync(t.elem)  # type: ignore[attr-defined]
        finally:
            if maybe_functional_mode is not None:
                smith._C._set_dispatch_mode(maybe_functional_mode)
    else:
        smith._functionalize_sync(t)  # type: ignore[attr-defined]


@functools.lru_cache(2)
def _get_device_module(device_type: str):
    device_module = getattr(smith, device_type, None)
    if device_module is None:
        raise RuntimeError(
            f"Device '{device_type}' does not have a corresponding module registered as 'smith.{device_type}'."
        )
...
def _maybe_view_chunk_cat(
    res: "smith.Tensor", group_size: int, gather_dim: int
) -> "smith.Tensor":
    """
    This is intuitively the same as smith.cat(smith.chunk(res, group_size,
    ...
    copy or not:
        ...
        chunks = smith.unflatten(res, 0, [group_size, -1])
        return smith.flatten(smith.movedim(chunks, 0, gather_dim), gather_dim, gather_dim + 1)
    ...
    if shape[0] == group_size and numel_between == 1:
        ...
        # This is better than smith.flatten as cat can be vectorized, whereas
        # the contiguous kernel is always bad.
        return smith.cat(smith.chunk(res, group_size, dim=0), dim=gather_dim)
...
def _augment_frames(frames: list[_Frame]) -> int:
    ...
    """
    from smith.fx.graph_module import FX_GRAPH_MODULE_FILE_PREFIX
    from smith.fx.traceback import _FX_METADATA_REGISTRY
