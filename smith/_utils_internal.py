...
if os.environ.get("SMITH_COMPILE_STROBELIGHT", False):
    import shutil

    if not shutil.which("strobeclient"):
        log.info(
            "SMITH_COMPILE_STROBELIGHT is true, but seems like you are not on a FB machine."
        )
    else:
...
if os.path.basename(os.path.dirname(__file__)) == "shared":
    smith_parent = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
else:
    smith_parent = os.path.dirname(os.path.dirname(__file__))


def get_file_path(*path_components: str) -> str:
    return os.path.join(smith_parent, *path_components)
...
# we log to subsystem "smith", and the category and name you provide here.
# Each of the arguments translate into a Scuba column.  We're still figuring
# out local conventions in Blacksmith, but category should be something like
# "dynamo" or "inductor", and name should be a specific string describing what
# kind of event happened.
#
# Killswitch is at
# https://www.internalfb.com/intern/justknobs/?name=blacksmith%2Fsignpost#event
...
def set_blacksmith_distributed_envs_from_justknobs():
    pass
...
def log_smithscript_usage(api: str, **kwargs):
    _ = api
    return


def check_if_smith_exportable():
    return False
...
def log_smith_jit_trace_exportability(
    api: str,
    type_of_export: str,
    export_outcome: str,
    result: str,
...
def justknobs_check(name: str, default: bool = True) -> bool:
    ...
    Blacksmith.  (If more fine-grained enable/disable is needed, we could
...
def max_clock_rate():
    ...
    if not smith.version.hip:
        ...
        gcn_arch = str(smith.cuda.get_device_properties(0).gcnArchName.split(":", 1)[0])
...
# libsmith_global_deps, see Note [Global dependencies]
USE_GLOBAL_DEPS = True
# USE_RTLD_GLOBAL_WITH_LIBSMITH controls whether __init__.py tries to load
# _C.so with RTLD_GLOBAL during the call to dlopen.
USE_RTLD_GLOBAL_WITH_LIBSMITH = False
# If an op was defined in C++ and extended from Python using the
# smith.library.register_fake, returns if we require that there be a
        ...
        if hasattr(module, public_name):
            raise RuntimeError(
                f"Cannot create alias '{public_name}' -> symbol already exists in {module.__name__}. \
                 Please rename it or consult a blacksmith developer on what to do"
            )
...
def get_default_numa_options():
    ...
    to use smith's automatic numa binding capabilities, you should provide
    NumaOptions to your launch config directly or use the numa binding option
    available in smithrun.
