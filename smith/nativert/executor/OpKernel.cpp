...
c10::OperatorHandle getOperatorForTarget(
    ...
    const Node* node) {
  // target could come as either "smith.ops.aten.add.default" or
  // "aten.add.default"
  std::vector<std::string_view> atoms = c10::split(target, '.');

  size_t numAtoms = atoms.size();
  if (numAtoms < 3) {
    SMITH_CHECK(false, "Invalid target: ", target);
  }
...
void fillDynamicInputs(
  ...
  for (auto [idx, value] : arguments.getDynamicArgs()) {
    SMITH_CHECK(
        ...
        " for stack size ",
        stack.size());
    SMITH_CHECK(stack.at(idx).isNone(), "Encountered None at index ", idx);
