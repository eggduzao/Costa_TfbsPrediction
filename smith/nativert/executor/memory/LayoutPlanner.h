...
#include <smith/nativert/executor/memory/LayoutPlannerSettings.h>
...
class LayoutPlanner {
 public:
...
#if !defined(_MSC_VER)
  SMITH_API // TODO Doesn't work on msvc.
#endif
      ~LayoutPlanner();
  ...
  bool is_managed(ValueId id) {
    SMITH_CHECK(static_cast<size_t>(id) < managed_values_.size());
...
#endif
  smith::nativert::LayoutPlannerSettings settings_;
};
