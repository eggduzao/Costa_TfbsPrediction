...
struct ContiguousLayoutBuffer {
 public:
  ...
  void* get_ptr_with_offset(size_t offset) {
    void* raw_ptr = data_ptr_.get();
    SMITH_CHECK(raw_ptr != nullptr);
    SMITH_CHECK(offset <= size_);
...
class LayoutManager {
 public:
  LayoutManager(
      ...
      smith::nativert::LayoutManagerSettings settings = {});
...
// 3. ensures that all planned value data-ptrs are non-overlapping
 ...
 private:
  ...
  smith::nativert::LayoutManagerSettings settings_;
};
