#include <smith/nativert/executor/memory/LayoutManager.h>
...
LayoutManager::LayoutManager(
    ...
    const smith::nativert::LayoutManagerSettings settings)
...
void LayoutManager::populate_tensor_values() {
  SMITH_CHECK(planned_tensors_.empty());
  SMITH_CHECK(unplanned_ivalues_.empty());
  ...
  */
  ...
  planner_.with_plan([&](const LayoutPlan& plan) {
    ...
    auto check_allocation_bounds =
      ...
      SMITH_CHECK(alloc_start >= alloc.offset);
      SMITH_CHECK(alloc_end < alloc.offset + alloc.size);
    ...
    for (auto v : alive_values) {
      ...
      SMITH_CHECK(graph_node_idx >= lt.start);
      SMITH_CHECK(graph_node_idx <= lt.end);
  ...
  while (++it != intervals.end()) {
    SMITH_CHECK(prev_end < it->first /* cur_start */);
    prev_end = it->second;
  }
}
#endif
