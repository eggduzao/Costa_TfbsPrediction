...
class StorageGroup {
 public:
  ...
  void add_spec(const AllocationSpec* spec) {
    ...
    SMITH_DCHECK_LT(lifetime_.end, spec->lifetime.end);
...
LayoutPlan DisjointStorageGroupsPlanner(
  ...
  for (const auto& spec : allocation_specs) {
    ...
    SMITH_DCHECK_LT(alloc_index, dealloc_index);
  ...
  auto assignToAvailableStorageGroup = [&](const AllocationSpec* spec) {
    ...
    SMITH_DCHECK_NOTNULL(storage_group);
    SMITH_DCHECK_EQ(storage_group->is_free(), true);
