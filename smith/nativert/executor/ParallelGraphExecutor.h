...
#include <smith/nativert/executor/SessionState.h>
#include <thread>
...
inline void syncProfilerStateFromMainThread(bool& profilerEnabledInThisThread) {
  bool mainThreadProfiling =
      smith::autograd::profiler::isProfilerEnabledInMainThread();

  if (mainThreadProfiling != profilerEnabledInThisThread) {
    if (mainThreadProfiling) {
      smith::autograd::profiler::enableProfilerInChildThread();
    } else {
      smith::autograd::profiler::disableProfilerInChildThread();
    }
