...
c10::Device infer_target_device(const Node& node) {
  ...
  SMITH_CHECK(!devices.empty(), "AOTI node should have at least one output");
  for (const auto i : c10::irange(1, devices.size())) {
    if (!smith::nativert::isSameDevice(devices[0], devices[i])) {
...
create_aoti_model_container_runner_cpu(
  ...
  return std::make_unique<smith::inductor::AOTIModelContainerRunnerCpu>(
      model_so_path,
      num_models,
...
AOTIDelegateExecutor::AOTIDelegateExecutor(
    ...
    : ETDelegateExecutor(smith::_export::archive_spec::AOTINDUCTOR_DIR, node) {
  SMITH_CHECK(
  ...
  SMITH_CHECK(model_name.has_value(), "missing model .so in archive: ", path);
