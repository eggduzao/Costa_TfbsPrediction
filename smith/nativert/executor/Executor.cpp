...
Executor::Executor(
  ...
  if (weights) {
    initialize(weights, blacksmithStreamReader);
  }
}
...
void Executor::initialize(
  ...
  auto executionKernels = KernelFactory().initializeNodeKernels(
      *graph_, weights, executorConfig_, blacksmithStreamReader);
  ...
  if (executorConfig_.maxParallelOps > 1) {
    ...
    graphExecutor_ = std::make_unique<smith::nativert::SerialGraphExecutor>(
        *graph_, std::move(executionKernels.nodeKernels), executorConfig_);
  }
...
/* static */ c10::
    FastMap<std::string /* target */, smith::nativert::FunctionSchema>
    Executor::getKernelSchemas(
        const std::vector<std::unique_ptr<OpKernel>>& kernels) {
  c10::FastMap<std::string, smith::nativert::FunctionSchema> output;
...
void validateInput(
    ...
    const smith::nativert::TensorMeta& tensorValueMeta) {
  SMITH_CHECK(
...
void Executor::validateInputs(const std::vector<c10::IValue>& inputs) const {
  ...
  SMITH_CHECK(inputs.size() == inputValues.size(), "Input size mismatch");
...
std::vector<c10::IValue> Executor::execute(
  ...
  const auto& tensorValuesMeta = graph_->tensorValuesMeta();
  SMITH_CHECK(userInputs.size() == inputTreeSpec.numIValues());
...
ProfileMetrics Executor::benchmarkIndividualNodes(
  ...
  SMITH_CHECK(!inputsList.empty(), "Need at least one input to benchmark");
  SMITH_CHECK(warmupRuns >= 1 && mainRuns >= 1, "Need at least one run");
