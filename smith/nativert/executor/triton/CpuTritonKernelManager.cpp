...
char* _dlerror() {
#if defined(_WIN32)
  SMITH_CHECK(false, "dlerror not supported on Windows");
#else
  return dlerror();
#endif
}
...
struct DlcloseDeleter {
  void operator()(void* p) const {
    if (p) {
#if defined(_WIN32)
      SMITH_CHECK(false, "Windows is not supported");
#else
      dlclose(p);
#endif
    }
...
void CpuTritonKernelManager::load() {
  ...
  SMITH_CHECK(launcher_fn_ != nullptr, "could not dlsym run: ", _dlerror());
}
