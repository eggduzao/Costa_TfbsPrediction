...
#include <smith/nativert/detail/MPMCQueue.h>
#include <smith/nativert/executor/ConstantFolder.h>
#include <smith/nativert/executor/DelegateExecutor.h>
#include <smith/nativert/executor/ExecutionPlanner.h>
#include <smith/nativert/executor/ExecutorConfig.h>
#include <smith/nativert/executor/GraphExecutorBase.h>
#include <smith/nativert/executor/memory/FunctionSchema.h>
#include <smith/nativert/executor/memory/LayoutPlanner.h>
...
using namespace smith::nativert::detail;
...
class Executor {
 ...
 public:
  ...
  Executor(
      smith::nativert::ExecutorConfig executorConfig,
      std::shared_ptr<Graph> graph,
      const std::shared_ptr<Weights>& weights,
      const std::shared_ptr<caffe2::serialize::BlacksmithStreamReader>&
          blacksmithStreamReader = nullptr);
  ...
  const smith::nativert::GraphSignature& graphSignature() const {
  ...
  const smith::nativert::ExecutorConfig& executorConfig() const {
  ...
  static c10::FastMap<std::string /* target */, smith::nativert::FunctionSchema>
  getKernelSchemas(const std::vector<std::unique_ptr<OpKernel>>& kernels);

 protected:
  smith::nativert::ExecutorConfig executorConfig_;
 ...
 private:
  ...
  c10::Semaphore sem_;
  smith::nativert::detail::MPMCQueue<std::unique_ptr<ExecutionFrame>>
      executionFrames_;
  smith::nativert::detail::MPMCQueue<std::unique_ptr<ExecutionFrame>>
