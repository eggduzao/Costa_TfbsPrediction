...
#include <smith/csrc/export/pt2_archive_constants.h>
  ...
  if (constants.has_value()) {
    for (const auto& [name, value] : constants.value()) {
      if (value.isTensor()) {
        ...
        SMITH_CHECK(false, "Unknown constant type: ", value.tagKind());
      }
    }
  }
}
...
Weights::Weights(
    const Graph* graph,
    std::shared_ptr<caffe2::serialize::BlacksmithStreamReader> blacksmithStreamReader,
    ...
    std::shared_ptr<std::unordered_map<
        std::string,
        std::shared_ptr<smith::nativert::TensorMeta>>> maybeNewWeightsMeta)
    : graph_(graph),
      weightsMeta_(graph->weightsMeta()),
      version_(globalVersion_++),
  ...
  auto loadAndInsert = [&](const std::string& tensorName,
                           ...
                           std::shared_ptr<std::unordered_map<
                               std::string,
                               std::shared_ptr<smith::nativert::TensorMeta>>>
                               maybeNewWeightsMeta) {
    ...
    SMITH_CHECK(
        blacksmithStreamReader->hasRecord(tensorPath), tensorPath, " not found");

    auto [tensorData, tensorDataSize] =
        blacksmithStreamReader->getRecord(tensorPath);
    ...
    if (maybeNewWeightsMeta) {
      if (stateDictPaths.find(tensorName) == stateDictPaths.end()) {
        SMITH_CHECK(false, "Tensor name not found in state dict paths");
      }
  ...
  auto loadConstants = [&](const auto& constants) {
    for (const auto& constantName : constants) {
      ...
      if (c10::starts_with(
              fileName,
              smith::_export::archive_spec::TENSOR_CONSTANT_FILENAME_PREFIX)) {
        // tensor constants
        isUsed = !weightValues[weightIndex]->users().empty();
        if (!isUsed) {
        ...
        SMITH_CHECK(false, "Unknown constant path: ", fileName);
      }
    }
  };
  ...
  for (const auto& customObjName : graph->signature().customObjs()) {
    ...
    if (!c10::starts_with(
            fileName,
            smith::_export::archive_spec::CUSTOM_OBJ_FILENAME_PREFIX)) {
      SMITH_CHECK(false, "Unknown constant path: ", fileName);
    }
    ...
    SMITH_CHECK(
        blacksmithStreamReader->hasRecord(customObjPath),
        customObjPath,
        " not found");

    const auto& [customObjData, customObjDataSize] =
        blacksmithStreamReader->getRecord(customObjPath);
    ...
    c10::IValue customObj = smith::jit::pickle_load_obj(customObjBytes);
    SMITH_CHECK(
        customObj.isCustomClass(), "Custom object is not a custom class");
    SMITH_CHECK(!customObj.isNone(), "Custom object is None");
...
at::Tensor Weights::at(const std::string& name) const {
  ...
  SMITH_CHECK(false, name, " not found in Weights ", toString());
}
...
c10::IValue Weights::getCustomObj(const std::string& name) const {
  ...
  SMITH_CHECK(false, "Custom objects ", name, " not found in Weights");
}
...
void Weights::loadStateDict(
  ...
  auto validateAndInsert = [&](const std::string& name) {
    ...
    SMITH_CHECK(tensor.sizes() == it->second.sizes());
    SMITH_CHECK(tensor.dtype() == it->second.dtype());
...
void Weights::validateAllWeightsLoaded() {
  auto checkNames = [&](const auto& names) {
    for (const auto& name : names) {
      ...
      SMITH_CHECK(it != allValues_.end(), "Missing weight: ", name);
      SMITH_CHECK(it->second.defined(), "Weight not defined: ", name);
