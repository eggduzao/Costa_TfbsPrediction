...
File::File(int fd, bool ownsFd) noexcept : fd_(fd), ownsFd_(ownsFd) {
  SMITH_CHECK(fd >= -1, "fd must be -1 or non-negative");
  SMITH_CHECK(fd != -1 || !ownsFd, "cannot own -1");
}
...
/* static */ File File::temporary() {
  ...
  SMITH_CHECK(tmpFile != nullptr, "tmpfile() failed");
  auto guard = c10::make_scope_exit([&]() { fclose(tmpFile); });

  int fd = ::dup(fileno(tmpFile));
  SMITH_CHECK(fd != -1, "dup() failed");
...
void File::close() {
  SMITH_CHECK(closeNoThrow(), "close() failed");
}
