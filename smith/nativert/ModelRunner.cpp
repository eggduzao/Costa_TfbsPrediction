...
using smith::nativert::jsonToGraph;
using smith::nativert::detail::itreeSpecLoads;

ModelRunner::ModelRunner(
    const std::string& packagePath,
    const std::string& modelName) {
  register_kernel_handlers();
  auto blacksmithStreamReader =
  ...
  std::string modelFilePath = fmt::format(
      smith::_export::archive_spec::MODELS_FILENAME_FORMAT, modelName);
  LOG(INFO) << "Loading model from: " << modelFilePath;

  SMITH_CHECK(
      blacksmithStreamReader->hasRecord(modelFilePath),
      modelFilePath,
      " not found in package");
  const auto& [modelData, modelSize] =
      blacksmithStreamReader->getRecord(modelFilePath);
  ...
  exportedProgram_ = nlohmann::json::parse(modelSerialized)
                         .template get<smith::_export::ExportedProgram>();

  SMITH_CHECK(exportedProgram_.get_graph_module()
  ...
  tensorPaths_ = getPayloadConfig(
      ...
      smith::_export::archive_spec::WEIGHTS_CONFIG_FILENAME_FORMAT,
      modelName);

  constantPaths_ = getPayloadConfig(
      blacksmithStreamReader,
      smith::_export::archive_spec::CONSTANTS_CONFIG_FILENAME_FORMAT,
      modelName);
  ...
  smith::nativert::Placement placement;

  graph_->applyDevicePlacement(placement);
  selectScalarOverload(graph_.get());

  auto weights = loadWeightsDefault(*graph_, blacksmithStreamReader);
  ...
  executor_ = std::make_unique<Executor>(
      config, graph_, std::move(weights), blacksmithStreamReader);
}
...
std::unordered_map<std::string, std::string> ModelRunner::getPayloadConfig(
  ...
  SMITH_CHECK(
      blacksmithStreamReader->hasRecord(configPath),
      configPath,
      " not found in package");

  const auto& [configData, configSize] =
      blacksmithStreamReader->getRecord(configPath);
  ...
  auto configJson = nlohmann::json::parse(configSerialized)
                        .template get<smith::_export::PayloadConfig>();
...
std::shared_ptr<Weights> ModelRunner::loadWeightsDefault(
  ...
  return std::make_shared<Weights>(
      ...
      reader,
      tensorPaths_,
      smith::_export::archive_spec::WEIGHTS_DIR,
      constantPaths_,
      smith::_export::archive_spec::CONSTANTS_DIR);
}

c10::IValue ModelRunner::run(
    const std::vector<c10::IValue>& args,
    const std::unordered_map<std::string, c10::IValue>& kwargs) {
  SMITH_CHECK(executor_, "ModelRunner not initialized");
