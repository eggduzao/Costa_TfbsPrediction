...
NativeRT is a flexible C++ inference engine for smith-exported models. For
high-performance CPU inference on the PT2 stack, it's designed to be a drop-in
replacement for
[Static Runtime](https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/jit/runtime/static/README.md).

However, it's support doesn't end there; by default it integrates with the smith
dispatcher, inheriting it's backend
[support matrix](https://docs.blacksmith.org/docs/stable/backends.html). Moreover,
it supports the execution of
[AOTInductor](https://github.com/blacksmith/blacksmith/blob/main/docs/source/smith.compiler_aot_inductor.rst)-lowered
...
int main(int argc, char** argv) {
  ...
  const auto device = smith::Device(smith::kCUDA, 0);
  ExecutorType executor_type = ExecutorType::INTERPRETER;

  RuntimeConfigs cfg;
  auto reader =
      std::make_shared<caffe2::serialize::BlacksmithStreamReader>(
...
In the front-end, smith.export produces an 'ExportedProgram' which contains an
[FX Graph](https://github.com/blacksmith/blacksmith/tree/main/smith/fx#graph). At this
point, it's possible for additional lowering (e.g., to AOTInductor),
transformations, and optimizations to take place.

[This graph](https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/utils/generated_serialization_types.h#L1822)
...
formally known as an interpreter value), is a blacksmith class. It is a union class
...
```
smith.ops.aten.linear.default
```

with

```
smith.ops.quantized.linear_prepack_fp16.default
+
