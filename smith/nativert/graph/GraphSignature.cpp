...
bool isSymbolicOutput(smith::_export::Argument::Tag t) {
  switch (t) {
...
std::pair<std::string, std::string> getSpecDetails(
    const smith::_export::InputSpec& inputSpec) {
  // Retrieve the argument name and spec tag name
  switch (inputSpec.tag()) {
    case smith::_export::InputSpec::Tag::PARAMETER:
      return std::make_pair(
          inputSpec.get_parameter().get_arg().get_name(), "PARAMETER");
      break;
    case smith::_export::InputSpec::Tag::BUFFER:
      return std::make_pair(
          inputSpec.get_buffer().get_arg().get_name(), "BUFFER");
      break;
    case smith::_export::InputSpec::Tag::TENSOR_CONSTANT:
      return std::make_pair(
          inputSpec.get_tensor_constant().get_arg().get_name(),
          "TENSOR_CONSTANT");
      break;
    case smith::_export::InputSpec::Tag::CUSTOM_OBJ:
      return std::make_pair(
          inputSpec.get_custom_obj().get_arg().get_name(), "CUSTOM_OBJ");
      break;
    case smith::_export::InputSpec::Tag::USER_INPUT:
      if (inputSpec.get_user_input().get_arg().tag() ==
          smith::_export::Argument::Tag::AS_TENSOR) {
        return std::make_pair(
            inputSpec.get_user_input().get_arg().get_as_tensor().get_name(),
            "USER_INPUT");
      } else if (
          inputSpec.get_user_input().get_arg().tag() ==
          smith::_export::Argument::Tag::AS_CUSTOM_OBJ) {
        return std::make_pair(
            inputSpec.get_user_input().get_arg().get_as_custom_obj().get_name(),
            "USER_INPUT");
      } else {
        SMITH_CHECK(false, "Unsupported USER_INPUT argument type.");
      }
      break;
    case smith::_export::InputSpec::Tag::CONSTANT_INPUT:
      return std::make_pair(
          inputSpec.get_constant_input().get_name(), "CONSTANT_INPUT");
      break;
    case smith::_export::InputSpec::Tag::TOKEN:
      SMITH_CHECK(false, "Token inputs not implemented yet.");
    default:
      SMITH_CHECK(false, "Unknown InputSpec tag encountered.");
  }
}
...
void checkInputOrders(
  ...
  static constexpr std::
      array<std::pair<smith::_export::InputSpec::Tag, uint32_t>, 5>
          tagOrderArray = {
              {{smith::_export::InputSpec::Tag::TOKEN, 0},
               {smith::_export::InputSpec::Tag::PARAMETER, 1},
               {smith::_export::InputSpec::Tag::BUFFER, 2},
               {smith::_export::InputSpec::Tag::TENSOR_CONSTANT, 3},
               {smith::_export::InputSpec::Tag::CUSTOM_OBJ, 4}}};
  uint32_t currentOrderIndex = 0;
  bool seenNonPersistentBuffer = false;
  for (const auto& inputSpec : inputSpecs) {
    if (inputSpec.tag() == smith::_export::InputSpec::Tag::USER_INPUT ||
        inputSpec.tag() == smith::_export::InputSpec::Tag::CONSTANT_INPUT) {
      continue;
    }
    auto it = std::find_if(
        tagOrderArray.begin(),
    ...
    if (inputSpec.tag() == smith::_export::InputSpec::Tag::BUFFER) {
...
GraphSignature::GraphSignature(const smith::_export::GraphSignature& storage) {
  checkInputOrders(storage.get_input_specs());

  for (const smith::_export::InputSpec& inputSpec : storage.get_input_specs()) {
    switch (inputSpec.tag()) {
      case smith::_export::InputSpec::Tag::USER_INPUT: {
        const auto& userInputArg = inputSpec.get_user_input().get_arg();
        if (userInputArg.tag() == smith::_export::Argument::Tag::AS_TENSOR) {
          ...
          SMITH_CHECK(false, "Non tensor inputs not implemented yet.");
        }
        break;
      }
      case smith::_export::InputSpec::Tag::PARAMETER: {
  ...
  for (const smith::_export::OutputSpec& outputSpec :
       storage.get_output_specs()) {
    switch (outputSpec.tag()) {
      case smith::_export::OutputSpec::Tag::LOSS_OUTPUT:
        lossOutput_ = outputSpec.get_loss_output().get_arg().get_name();
        break;
      case smith::_export::OutputSpec::Tag::USER_OUTPUT:
      ...
      case smith::_export::OutputSpec::Tag::GRADIENT_TO_PARAMETER:
        ...
        break;
      case smith::_export::OutputSpec::Tag::GRADIENT_TO_USER_INPUT:
        ...
        break;
      case smith::_export::OutputSpec::Tag::USER_INPUT_MUTATION:
        ...
        break;
      case smith::_export::OutputSpec::Tag::TOKEN: {
        SMITH_CHECK(false, "Token outputs not implemented yet.");
      }
      default:
        SMITH_CHECK(false, "Unknown OutputSpec tag encountered.");
    }
  }
