...
Node* Graph::insertBefore(Node* toInsert, Node* insertionPoint) {
  SMITH_CHECK(insertionPoint != inputNode_, "can't insert before prim.Input");
  SMITH_CHECK(
...
Node* Graph::insertAfter(Node* toInsert, Node* insertionPoint) {
  SMITH_CHECK(insertionPoint != outputNode_, "can't insert after prim.Output");
  SMITH_CHECK(
      !toInsert->is_linked(), "expected node to be unlinked: ", *toInsert);
  SMITH_CHECK(
      insertionPoint->is_linked(),
        ...
        if constexpr (is_same_v<T, Type::Kind>) {
          switch (arg) {
            ...
            case Type::Kind::CustomObj:
              ...
              break;
            default:
              SMITH_CHECK(false, "Unhandled type");
          }
...
const NamedArgument* Node::tryGetInput(std::string_view name) const {
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(inputs_.size() < 1000);
...
const Attribute* Node::tryGetAttribute(std::string_view name) const {
  ...
  // second datastructure for lookups.
  // Drop a debug check here, just to make sure :)
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(attributes_.size() < 1000);
...
Node* Graph::createListPack(std::vector<Value*> inputs, const Type& inputType) {
  ...
  for (auto& input : inputs) {
    SMITH_CHECK(input->type() == inputType);
  }
...
void Graph::lint() const {
  ...
  for (const auto& [name, value] : values_) {
    ...
    if (value->type().kind() != Type::Kind::SymInt &&
      ...
      SMITH_CHECK(value->isFolded() || value->producer() != nullptr);
    }
  }
  for (const auto& node : nodes()) {
    SMITH_CHECK(node.owningGraph() == this);
  }
  ...
  for (const auto& [_, value] : values_) {
    ...
    SMITH_CHECK(producedByListPack || consumedByListUnpack);
  }
  ...
  for (auto& outputName : signature_.userOutputs()) {
    if (outputName.has_value()) {
      ...
      if (constantIndex < constantOutputs_.size()) {
        ...
        SMITH_CHECK(false, "No more constant outputs available");
      }
    }
  }
}
...
Node* Graph::nodeAfter(Node* n) {
  SMITH_CHECK(n->owningGraph() == this);
  if (n == outputNode_) {
    return nullptr;
  }
...
void Graph::removeNode(Node* n) {
  SMITH_CHECK(n->owningGraph() == this, "Node does not belong to this graph!");
  ...
  SMITH_CHECK(n->is_linked(), "Node is not linked to the graph!");
  n->unlink();
  ...
  SMITH_CHECK(it != nodesOwner_.end(), "Node not found in nodesOwner_!");
  nodesOwner_.erase(it);
}

void Graph::removeValue(Value* value) {
  // TODO: assuming not removing from constantSymIntValues_
  SMITH_CHECK(value->users().empty(), "Cannot erase a value with users.");
...
std::vector<Value*> Graph::insertGraph(
  ...
  SMITH_CHECK(subgraph.inputs().size() == inputs.size(), "Input size mismatch");
  ...
  for (const auto& n : subgraph.nodes()) {
    ...
    for (auto& inp : inputs) {
      ...
      SMITH_CHECK(it != valueMap.end(), "Missing input value in subgraph");
...
Value* Node::addOutput(const Type& type) {
  SMITH_CHECK(type == Type::Kind::None);
...
std::vector<const Value*> Value::getListElements() const {
  ...
  if (auto p = producer(); p && p->target() == "prim.ListPack") {
    ...
    SMITH_CHECK(users().size() == 1);
    const auto listUnpack = users()[0];
    SMITH_CHECK(listUnpack->target() == "prim.ListUnpack");
...
c10::Device convertDevice(std::string_view symbol) {
  ...
  SMITH_CHECK(indexValue.has_value(), "Invalid device index format");
...
Constant convertAtomicConstant(std::string_view symbol) {
  if (c10::starts_with(symbol, "\"")) {
    ...
    smith::_export::MemoryFormat value = smith::_export::MemoryFormat::Unknown;
    symbol.remove_prefix(kMemoryFormatPrefix.length());
    smith::_export::parseEnum(symbol, value);
    return convertJsonMemoryFormat(value);
  } else if (c10::starts_with(symbol, kLayoutPrefix)) {
    smith::_export::Layout value = smith::_export::Layout::Unknown;
    ...
    smith::_export::ScalarType value = smith::_export::ScalarType::UNKNOWN;
  ...
  SMITH_CHECK(false, "unhandled symbol: ", symbol);
}
  ...
  while (true) {
    ...
    if (std::holds_alternative<None>(type)) {
      ...
      if (auto intPtr = std::get_if<int64_t>(&val)) {
        ...
        SMITH_CHECK(false, "constant lists only support int, float, bool");
      }
    } else {
      SMITH_CHECK(
...
class Parser {
 ...
 private:
  ...
  smith::_export::GraphSignature signature_;
};
...
void Parser::parseGraphInputs() {
  ...
  for (const auto& input : inputs) {
    ...
    smith::_export::TensorArgument inputTensorArg;
    inputTensorArg.set_name(std::string{input});
    smith::_export::Argument inputArg;
    inputArg.set_as_tensor(std::move(inputTensorArg));
    smith::_export::UserInputSpec userInput;
...
void Parser::parseReturn() {
  ...
  for (const auto ret : returns) {
    ...
    smith::_export::TensorArgument retTensorArg;
    retTensorArg.set_name(std::string{ret->name()});
    smith::_export::Argument retArg;
    retArg.set_as_tensor(std::move(retTensorArg));
    smith::_export::UserOutputSpec userOutput;
...
std::pair<std::string_view, Type> Parser::parseOutput() {
  consumeWhitespace();
  SMITH_CHECK(cur() == '%', fmt::format("expected % but got {}", cur()));
