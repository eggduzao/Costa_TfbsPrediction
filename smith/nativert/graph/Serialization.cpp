...
std::unique_ptr<Graph> jsonToSubgraph(
    const smith::_export::Graph& jsonGraph,
    const smith::_export::GraphSignature* signature,
    bool loadNodeMetadata);

Value* symbolicToValue(
    const smith::_export::Argument& arg,
    Graph& graph,
    Node* insertBefore) {
  switch (arg.tag()) {
    case smith::_export::Argument::Tag::AS_TENSOR:
      return graph.getValue(arg.get_as_tensor().get_name());
    case smith::_export::Argument::Tag::AS_TENSORS: {
    ...
    case smith::_export::Argument::Tag::AS_OPTIONAL_TENSORS: {
      // Need to insert a list pack node
      std::vector<Value*> listValue;
      for (const auto& listEl : arg.get_as_optional_tensors()) {
        switch (listEl.tag()) {
          case smith::_export::OptionalTensorArgument::Tag::AS_TENSOR: {
            listValue.push_back(
                graph.getValue(listEl.get_as_tensor().get_name()));
            break;
          }
          case smith::_export::OptionalTensorArgument::Tag::AS_NONE: {
            listValue.push_back(
          ...
          default:
            SMITH_CHECK(
                ...
                fmt::format(
                    ...
                    smith::_export::printEnum(listEl.tag())));
        }
      }
      auto listPack = graph.createOptionalListPack(std::move(listValue));
      return graph.insertBefore(listPack, insertBefore)->outputs()[0];
    }
    case smith::_export::Argument::Tag::AS_SYM_INT: {
      return graph.getValue(arg.get_as_sym_int().get_as_name());
    }
    case smith::_export::Argument::Tag::AS_SYM_INTS: {
      // Need to insert a list pack node
      std::vector<Value*> listValue;
      for (const auto& listEl : arg.get_as_sym_ints()) {
        switch (listEl.tag()) {
          case smith::_export::SymIntArgument::Tag::AS_NAME: {
            listValue.push_back(graph.getValue(listEl.get_as_name()));
            break;
          }
          case smith::_export::SymIntArgument::Tag::AS_INT: {
    ...
    case smith::_export::Argument::Tag::AS_CUSTOM_OBJ: {
      return graph.getValue(arg.get_as_custom_obj().get_name());
    }
    case smith::_export::Argument::Tag::AS_SYM_BOOL: {
      return graph.getValue(arg.get_as_sym_bool().get_as_name());
    }
    case smith::_export::Argument::Tag::AS_SYM_FLOAT: {
      return graph.getValue(arg.get_as_sym_float().get_as_name());
    }
    case smith::_export::Argument::Tag::AS_STRING_TO_ARGUMENT: {
      SMITH_CHECK(
          false,
    ...
    default:
      SMITH_CHECK(
          ...
          fmt::format(
              ...
              smith::_export::printEnum(arg.tag())));
  }
}

std::pair<
    std::vector<smith::_export::InputSpec>,
    std::vector<smith::_export::Argument>>
enforceInputOrder(
    const std::vector<smith::_export::InputSpec>& inputSpecs,
    const std::vector<smith::_export::Argument>& graphInputs) {
  ...
  std::vector<smith::_export::InputSpec> reorderedInputSpecs;
  std::vector<smith::_export::Argument> reorderedGraphInputs;
  std::vector<smith::_export::InputSpec::Tag> desiredOrder = {
      smith::_export::InputSpec::Tag::TOKEN,
      smith::_export::InputSpec::Tag::PARAMETER,
      smith::_export::InputSpec::Tag::BUFFER,
      smith::_export::InputSpec::Tag::TENSOR_CONSTANT,
      smith::_export::InputSpec::Tag::CUSTOM_OBJ};
  ...
  for (const auto& tag : desiredOrder) {
    if (tag == smith::_export::InputSpec::Tag::BUFFER) {
  ...
  for (size_t i = 0; i < inputSpecs.size(); ++i) {
    ...
    if (tag == smith::_export::InputSpec::Tag::USER_INPUT ||
        tag == smith::_export::InputSpec::Tag::CONSTANT_INPUT) {
      reorderedInputSpecs.push_back(inputSpecs[i]);
      reorderedGraphInputs.push_back(graphInputs[i]);
    }
  }
...
std::unique_ptr<Graph> jsonToSubgraph(
        ...
        default:
          SMITH_CHECK(
              ...
              fmt::format(
                  ...
                  smith::_export::printEnum(input.tag())));
      }
    } else {
      switch (input.tag()) {
        case smith::_export::Argument::Tag::AS_INT:
        case smith::_export::Argument::Tag::AS_FLOAT:
        case smith::_export::Argument::Tag::AS_STRING:
        case smith::_export::Argument::Tag::AS_BOOL:
        case smith::_export::Argument::Tag::AS_NONE: {
  ...
  for (const auto& jsonNode : jsonGraph.get_nodes()) {
    ...
    for (const auto& input : jsonNode.get_inputs()) {
      ...
      } else if (arg.tag() == smith::_export::Argument::Tag::AS_NONE) {
        node->addInput(NamedArgument{
            input.get_name(),
    ...
    for (const auto& output : jsonNode.get_outputs()) {
      switch (output.tag()) {
        ...
        case smith::_export::Argument::Tag::AS_SYM_BOOLS: {
          SMITH_CHECK(
              false,
        ...
        case smith::_export::Argument::Tag::AS_SYM_FLOATS: {
          SMITH_CHECK(
              false,
        ...
        default:
          SMITH_CHECK(
              ...
              fmt::format(
                  ...
                  smith::_export::printEnum(output.tag())));
      }
    }
  }
    ...
    if (isSymbolic(output)) {
      switch (output.tag()) {
        ...
        case smith::_export::Argument::Tag::AS_SYM_INT: {
          ...
          SMITH_CHECK(
              asSymInt.tag() == smith::_export::SymIntArgument::Tag::AS_NAME);
  ...
  if (!signature) {
    ...
    smith::_export::GraphSignature sig;
    std::vector<smith::_export::InputSpec> inputSpecs;
    for (const auto& input : graph->inputs()) {
      smith::_export::Argument arg;
      if (input->type().kind() == Type::Kind::Tensor) {
        smith::_export::TensorArgument targ;
      ...
      smith::_export::UserInputSpec userInputSpec;
      userInputSpec.set_arg(std::move(arg));
      smith::_export::InputSpec inputSpec;
    ...
    std::vector<smith::_export::OutputSpec> outputSpecs;
    for (const auto& output : graph->outputs()) {
      ...
      smith::_export::UserOutputSpec userOutputSpec;
      userOutputSpec.set_arg(std::move(arg));
      smith::_export::OutputSpec outputSpec;
  ...
  std::unordered_map<std::string, smith::_export::TensorMeta> weightsTensorMeta;
...
bool isSymbolic(const smith::_export::Argument& arg) {
  switch (arg.tag()) {
    ...
    case smith::_export::Argument::Tag::AS_SYM_FLOATS:
    case smith::_export::Argument::Tag::AS_CUSTOM_OBJ:
    case smith::_export::Argument::Tag::AS_OPTIONAL_TENSOR:
      return true;
    default:
      return false;
  }
...
Constant constantToValue(
  ...
  switch (jsonArg.tag()) {
    ...
    case smith::_export::Argument::Tag::AS_INTS: {
    ...
    case smith::_export::Argument::Tag::AS_FLOATS: {
    ...
    case smith::_export::Argument::Tag::AS_STRINGS: {
    ...
    case smith::_export::Argument::Tag::AS_GRAPH: {
    ...
    case smith::_export::Argument::Tag::AS_TENSORS:
    case smith::_export::Argument::Tag::AS_OPTIONAL_TENSORS:
      SMITH_CHECK(false, "Tensor values are symbolic, not constant.");
    case smith::_export::Argument::Tag::AS_SYM_INT:
    case smith::_export::Argument::Tag::AS_SYM_INTS:
    case smith::_export::Argument::Tag::AS_SYM_BOOL:
    case smith::_export::Argument::Tag::AS_SYM_BOOLS:
      SMITH_CHECK(false, "Symint/Symbool Values are symbolic, not constant.");
    case smith::_export::Argument::Tag::AS_CUSTOM_OBJ:
      SMITH_CHECK(false, "custom obj is symbolic, not constant");
    case smith::_export::Argument::Tag::AS_OPERATOR:
      return jsonArg.get_as_operator();
    case smith::_export::Argument::Tag::AS_SYM_FLOAT: {
      SMITH_CHECK(false, "SymFloat is not yet implemented");
    }
    case smith::_export::Argument::Tag::AS_SYM_FLOATS: {
      SMITH_CHECK(false, "SymFloats is not yet implemented");
    }
    case smith::_export::Argument::Tag::AS_OPTIONAL_TENSOR:
      SMITH_CHECK(false, "Optional tensor is symbolic, not constant");
    case smith::_export::Argument::Tag::AS_COMPLEX:
      SMITH_CHECK(false, "Complex values are not yet supported as constants");
    case smith::_export::Argument::Tag::AS_INT_LISTS: {
    ...
    case smith::_export::Argument::Tag::AS_STRING_TO_ARGUMENT:
      return None();
    default:
      SMITH_CHECK(false, "Got unknown json argument");
  }
}

std::unique_ptr<Graph> jsonToGraph(
    const smith::_export::GraphModule& jsonGraphModule,
    bool loadNodeMetadata) {
