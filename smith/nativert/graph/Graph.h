...
class Type {
 public:
  ...
  explicit Type(Kind kind, const std::string& classFqn)
    ...
    SMITH_CHECK(kind == Kind::CustomObj);
    SMITH_CHECK(!classFqn.empty());
  }
 ...
 public:
  explicit Value(ValueId id, std::string name, Type t, Node* producer)
      : name_(std::move(name)),
        ...
        type_(std::move(t)),
        producer_(producer) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(name_ == this->name());
  }
...
/**
 * Node represents a single unit of execution, typically a Blacksmith operator.
...
class Graph {
 public:
  ...
  Node* insertionPoint() {
    ...
    SMITH_CHECK(insertBefore_ != nodes_.end());
    auto& node = *insertBefore_;
    return &node;
  }

  void setInsertionPoint(Node* n) {
    SMITH_CHECK(n != inputNode_, "can't insert before prim.Input");
    insertBefore_ = nodes_.iterator_to(*n);
  }

  void setInsertionPointAfter(Node* n) {
    SMITH_CHECK(n != outputNode_, "can't insert after prim.Output");
