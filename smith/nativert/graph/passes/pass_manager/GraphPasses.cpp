#include <smith/nativert/graph/passes/pass_manager/GraphPasses.h>

#include <smith/nativert/graph/passes/SubgraphRewriter.h>
#include <smith/nativert/graph/passes/pass_manager/GraphPassRegistry.h>
...
void register_base_passes() {
  ...
  GraphPassRegistry::add_pass(
      "LinearDynamicFp16UnpackedWeight", [](Graph* graph) {
        std::string p = R"(
    graph(%i, %w, %b):
    %out_0 = smith.ops.aten.linear.default(input=%i, weight=%w, bias=%b)
    return (%out_0))";

        std::string p_1 = R"(
    graph(%i, %w, %b):
    %out_0 = smith.ops.quantized.linear_dynamic_fp16_unpacked_weight.default(X=%i, weight=%w, bias=%b)
    return (%out_0))";

        std::string p_new = R"(
    graph(%i, %w, %b):
    %pw = smith.ops.quantized.linear_prepack_fp16.default(W=%w, B=%b)
    %out_0 = smith.ops.quantized.linear_dynamic_fp16.default(X=%i, W_prepack=%pw)
    return (%out_0))";
  ...
  GraphPassRegistry::add_pass(
    ...
    graph(%i, %w, %b):
    %out_0 = smith.ops.aten.linear.default(input=%i, weight=%w, bias=%b)
    %out_1 = smith.ops.aten.relu.default(self=%out_0)
    return (%out_1))";
    ...
    graph(%i, %w, %b):
    %pw = smith.ops.quantized.linear_prepack_fp16.default(W=%w, B=%b)
    %out_0 = smith.ops.quantized.linear_relu_dynamic_fp16.default(X=%i, W_prepack=%pw)
    return (%out_0))";
  ...
  GraphPassRegistry::add_pass("RemoveDetach", [](Graph* graph) {
    ...
    for (auto& node : graph->nodes()) {
      if (node.target() == "smith.ops.aten.detach.default") {
