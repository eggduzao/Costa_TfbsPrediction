...
bool SubgraphMatcher::tryMatchNodeInputs(
  ...
  SMITH_CHECK(target_node->numInputs() <= pattern_node->numInputs());
  SMITH_CHECK(pattern_node->attributes().size() <= target_node->numInputs());
...
void SubgraphRewriter::rewriteMatch(
  ...
  SMITH_CHECK(insertionPoint, "No insertion point found");
  ...
  SMITH_CHECK(outputs.size() == newOutputs.size());
