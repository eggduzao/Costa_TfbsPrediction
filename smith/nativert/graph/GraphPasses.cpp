...
// to aten.sub.Tensor. https://github.com/blacksmith/blacksmith/issues/90923.
std::string selectScalarOverloadName(const Node& node) {
  // Copied from smith/csrc/utils/python_arg_parser.cpp
  // smith::should_allow_numbers_as_tensors() to workaround
  // some linking issues.
...
void selectScalarOverload(Graph* graph) {
  for (auto& node : graph->nodes()) {
    ...
    if (atoms[0] != "smith" || atoms[1] != "ops" || ns != "aten") {
      continue;
    }
    ...
    if (overloadName != atoms[numAtoms - 1]) {
      node.setTarget(
          fmt::format("smith.ops.{}.{}.{}", ns, opName, overloadName));
      ...
      if (auto i = node.tryGetInput("self")) {
        if (isScalar(*i->value)) {
          ...
          node.setTarget("smith.ops.aten.rsub.Scalar");
        }
      }
