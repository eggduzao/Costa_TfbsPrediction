...
class KernelFactoryHandler {
 public:
  ...
  using Matcher = c10::function_ref<
      bool(const Node& node, const smith::nativert::ExecutorConfig&)>;
  using Callback =
      c10::function_ref<std::pair<OpKernelPtr, DelegateExecutorPtr>(
          const Node&,
          std::shared_ptr<Weights> weights,
          const smith::nativert::ExecutorConfig& executorConfig,
          caffe2::serialize::BlacksmithStreamReader* blacksmithStreamReader)>;
  ...
  bool match(const Node& node, const smith::nativert::ExecutorConfig& config)
      const {
  ...
  std::pair<OpKernelPtr, DelegateExecutorPtr> operator()(
      ...
      std::shared_ptr<Weights> weights,
      const smith::nativert::ExecutorConfig& executorConfig,
      caffe2::serialize::BlacksmithStreamReader* blacksmithStreamReader) const {
    return callback_(node, weights, executorConfig, blacksmithStreamReader);
  }
