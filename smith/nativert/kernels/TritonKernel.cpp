...
// in this case, we want to use the symbol from smith_cpu.dll
...
TritonKernel::TritonKernel(
  ...
  SMITH_CHECK(reader != nullptr, "reader is null");
  ...
  SMITH_CHECK(!kernel_name.empty(), "kernel name not found");
  SMITH_CHECK(!symbol_name.empty(), "symbol_name not found");
  SMITH_CHECK(!output_indices_.empty(), "output_indices attribute not found");
