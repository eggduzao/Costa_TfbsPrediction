...
HigherOrderKernel::HigherOrderKernel(
  ...
  static constexpr std::string_view prefix = "smith.ops.higher_order.";
  SMITH_CHECK(c10::starts_with(node->target(), prefix));
  auto opName = node->target().substr(prefix.size());
  if (opName == "cond") {
    opType_ = OpType::COND;
    // Checking smith.cond schema is as expected:
    // smith.cond(Tensor predicate, Graph graph1, Graph graph2, Tensor[] args)
    // -> Tensor[]
    SMITH_CHECK(node_->attributes().size() == 2);
    SMITH_CHECK(node_->inputs().size() == 2);
  } else if (opName == "while_loop") {
    opType_ = OpType::WHILE_LOOP;
    // Checking smith.while_loop schema is as expected:
    // smith.while_loop(Graph cond, Graph body, Tensor[] args, Tensor[]
    // additional) -> Tensor[]
    SMITH_CHECK(node_->attributes().size() == 2);
    SMITH_CHECK(node_->inputs().size() == 2);
  } else if (opName == "run_const_graph") {
    opType_ = OpType::RUN_CONST_GRAPH;
    // Checking smith.run_const_graph schema is as expected:
    // smith.run_const_graph(Graph graph, Tensor[] args) -> Tensor[]
    SMITH_CHECK(!node_->attributes().empty());
    SMITH_CHECK(node_->inputs().size() == 1);
  } else {
    SMITH_CHECK(false, "Unknown higher order op: ", opName);
  }
}
  ...
  switch (opType_) {
    case OpType::COND: {
      ...
      if (cond.isTensor()) {
        ...
        SMITH_CHECK(false, "Unsupported type for cond predicate");
      }
    ...
    case OpType::WHILE_LOOP: {
      ...
      while (true) {
        ...
        SMITH_CHECK(out.size() == numCarriedVals);
    ...
    default:
      SMITH_CHECK(false, "Unknown higher order op");
  }
}
