...
std::unordered_map<std::string, c10::IValue> getSymInputs(
  ...
  for (const auto& input : node.inputs()) {
    ...
    if (val.isInt() || val.isDouble() || val.isBool()) {
      ...
      SMITH_CHECK(false, "unsupported type for symbolic input");
    }
  }
...
void computeScalarBinaryOp(
  ...
  if (target == "_operator.add") {
    ...
    SMITH_CHECK(false, "unsupported operator for scalar binary op: ", target);
  }
...
void SymIntOpKernel::computeInternal(ExecutionFrame& executionFrame) const {
  ...
  if (target == "_operator.floordiv") {
    ...
    out = a % b;
  } else if (target == "smith.sym_max") {
    out = std::max(a, b);
  } else if (target == "smith.sym_min") {
    out = std::min(a, b);
  } else {
    SMITH_CHECK(false, "unsupported operator for SymInt: ", node_->target())
  }
...
void SymBoolOpKernel::computeInternal(ExecutionFrame& executionFrame) const {
  ...
  if (target == "smith.sym_not") {
    ...
    SMITH_CHECK(false, "unsupported operator for SymBool: ", node_->target())
  }
...
void SymFloatOpKernel::computeInternal(ExecutionFrame& executionFrame) const {
  ...
  if (target == "math.trunc") {
    ...
    int64_t out = trunc(x);
    executionFrame.setIValue(node_->outputs()[0]->id(), out);
  } else if (target == "smith._sym_sqrt") {
    ...
    SMITH_CHECK(false, "unsupported operator for SymFloat: ", node_->target());
  }
}
} // namespace smith::nativert
