...
REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.slice.Tensor", aten_slice_Tensor, {
...
REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.sym_size.int", aten_sym_size_int, {
  const auto& self = KernelInput(0).toTensor();
  const auto dim = KernelInput(1).toInt();
  auto& out = KernelOutput(0);
  SMITH_CHECK(dim >= 0 && dim < self.dim(), "Invalid dimension");
  out = self.sym_size(dim);
})

REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.reshape.default", aten_reshape, {
  const auto& self = KernelInput(0).toTensor();
  const auto& shape = KernelInput(1).toIntVector();
  KernelOutput(0) = at::native::reshape(self, shape);
})

REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.view.default", aten_view, {
  const auto& self = KernelInput(0).toTensor();
  const auto& size = KernelInput(1).toIntVector();
  KernelOutput(0) = at::native::view(self, size);
})

REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.permute.default", aten_permute, {
  const auto& self = KernelInput(0).toTensor();
  const auto& dims = KernelInput(1).toDimVector();
  KernelOutput(0) = at::native::permute(self, dims);
})

REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.select.int", aten_select, {
  const auto& self = KernelInput(0).toTensor();
  const auto dim = KernelInput(1).toInt();
...
REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.split.Tensor", aten_split_Tensor, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_NATIVE_CPU_KERNEL(
    "smith.ops.aten.tensor_split.sections",
...
REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.item.default", aten_item, {
  const auto& self = KernelInput(0).toTensor();
  KernelOutput(0) = at::native::item(self);
})

REGISTER_NATIVE_CPU_KERNEL("smith.ops.aten.narrow.default", aten_narrow, {
  const auto& self = KernelInput(0).toTensor();
  const auto dim = KernelInput(1).toInt();
  int64_t start = 0;
