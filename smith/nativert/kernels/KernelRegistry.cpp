...
static at::Tensor& mul_out(
  ...
  SMITH_CHECK(at::native::result_type(self, other) == t_output);
...
REGISTER_CPU_KERNEL("smith.ops.aten.remainder.Tensor", aten_remainder_Tensor, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.remainder.Scalar", aten_remainder_Scalar, {
  const auto& self = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.matmul.default", aten_matmul, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.bmm.default", aten_bmm, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t = KernelInput(1).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.abs.default", aten_abs, {
  const auto& in0_t = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.mul.Tensor", aten_mul, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t = KernelInput(1).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.mul.Scalar", aten_mul_Scalar, {
  const auto& in0_t = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.nan_to_num.default", aten_nan_to_num, {
  const auto& in0_t = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.leaky_relu.default", aten_leaky_relu, {
  const auto& in0_t = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.relu.default", aten_relu, {
  const auto& in0_t = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
    KernelOutput(0) = create_empty_from(in0_t);
  }
...
REGISTER_CPU_KERNEL("smith.ops.aten.clone.default", aten_clone, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.index.Tensor", aten_index, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto in1_l =
...
REGISTER_CPU_KERNEL("smith.ops.aten.index_select.default", aten_index_select, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.pow.Tensor_Tensor",
...
REGISTER_CPU_KERNEL("smith.ops.aten.pow.Scalar", aten_pow_Scalar, {
  if (KernelOutput(0).isNone()) {
    const auto& in1_t = KernelInput(1).toTensor();
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.pow.Tensor_Scalar",
...
REGISTER_CPU_KERNEL("smith.ops.aten.sum.default", aten_sum_default, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.sum.dim_IntList", aten_sum_dim_IntList, {
  // if (n->inputs().size() != 2 && n->inputs().size() != 4) {
  //   return nullptr;
  // }
  const at::Tensor& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.mean.dim", aten_mean_dim, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.mean.default", aten_mean_default, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.max.other", aten_max_other, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.max.default", aten_max_default, {
  const auto& self = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.sign.Tensor", aten_sign_Tensor, {
  const auto& in0_t = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.log.default", aten_log, {
  const auto& in0_t = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.sub.Tensor", aten_sub_Tensor, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t = KernelInput(1).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.sub.Scalar", aten_sub, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t =
...
REGISTER_CPU_KERNEL("smith.ops.aten.argmin.default", aten_argmin, {
  const auto& in0_t = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.softmax.int", aten_softmax_int, {
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.norm.ScalarOpt_dtype",
...
REGISTER_CPU_KERNEL("smith.ops.aten.full.default", aten_full, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.ones.default", aten_ones, {
  const auto size = KernelInput(0).toDimVector();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.ones_like.default", aten_ones_like, {
  const auto& self = KernelInput(0).toTensor();
  const auto dtype = KernelInput(1).toOptional<c10::ScalarType>();
  const auto layout = KernelInput(2).toOptional<c10::Layout>();
  const auto device = KernelInput(3).toOptional<c10::Device>();
...
REGISTER_CPU_KERNEL("smith.ops.aten.zeros.default", aten_zeros, {
  const auto size = KernelInput(0).toDimVector();
  const auto dtype = KernelInput(1).toOptional<c10::ScalarType>();
  const auto layout = KernelInput(2).toOptional<c10::Layout>();
  if (!hasTensorWithOptions(KernelOutput(0), dtype, layout)) {
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.linalg_norm.default",
...
REGISTER_CPU_KERNEL("smith.ops.aten.linalg_norm.ord_str", aten_linalg_norm, {
  const auto& self = KernelInput(0).toTensor();
  const auto dim = KernelInput(2).toDimVector();
  const auto keepdim = KernelInput(3).toBool();
  const auto dtype = KernelInput(4).toOptional<c10::ScalarType>();
...
REGISTER_CPU_KERNEL("smith.ops.aten.cat.default", aten_cat, {
  const auto inputs = KernelInput(0).toTensorVector();
  SMITH_CHECK(!inputs.empty(), "concat expects non-empty tensor list");
  const auto dim = KernelInput(1).toInt();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.cumsum.default", aten_cumsum, {
  const auto& self = KernelInput(0).toTensor();
  const auto dim = KernelInput(1).toInt();
  const auto dtype = KernelInput(2).toOptional<c10::ScalarType>();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.nonzero.default", aten_nonzero, {
  const auto& self = KernelInput(0).toTensor();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.addmm.default", aten_addmm, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t = KernelInput(1).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.narrow_copy.default", aten_narrow_copy, {
...
REGISTER_CPU_KERNEL("smith.ops.aten.repeat.default", aten_repeat, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.max.dim", aten_max_dim, {
  const auto& self = KernelInput(0).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.layer_norm.default", aten_layer_norm, {
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.norm.ScalarOpt_dim_dtype",
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.norm.ScalarOpt_dim",
...
REGISTER_CPU_KERNEL("smith.ops.aten.full_like.default", aten_full_like, {
  const auto in1_s = KernelInput(1).toScalar();
  const auto& in0_t = KernelInput(0).toTensor();
  const auto dtype = KernelInput(2).toOptional<c10::ScalarType>();
  const auto layout = KernelInput(3).toOptional<c10::Layout>();
...
REGISTER_CPU_KERNEL("smith.ops.aten.linear.default", aten_linear, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_t = KernelInput(1).toTensor();
...
REGISTER_CPU_KERNEL("smith.ops.aten.where.self", aten_where, {
...
REGISTER_CPU_KERNEL("smith.ops.fb.scale_gradient.default", fb_scale_gradient, {
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.embedding_bag_byte_rowwise_offsets.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.embedding_bag_4bit_rowwise_offsets.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.linear_dynamic_fp16.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops._quantized.wrapped_fbgemm_linear_fp16_weight.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.linear_relu_dynamic_fp16.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.linear.default",
...
REGISTER_CPU_KERNEL("smith.ops.aten.logit.default", aten_logit, {
  const auto& in0_t = KernelInput(0).toTensor();
  const auto& in1_d = KernelInput(1).toOptional<double>();
  if (KernelOutput(0).isNone()) {
    KernelOutput(0) = create_empty_from(in0_t);
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.embedding_bag_byte_unpack.default",
...
REGISTER_CPU_KERNEL(
    "smith.ops.quantized.embedding_bag_byte_prepack.default",
...
REGISTER_CPU_KERNEL("smith.ops.aten.stack.default", aten_stack, {
  const auto& inputs = KernelInput(0).toTensorVector();
  const auto dim = KernelInput(1).toInt();
  if (KernelOutput(0).isNone()) {
...
REGISTER_CPU_KERNEL("smith.ops.aten.fmod.Scalar", aten_fmod_scalar, {
  const auto& self = KernelInput(0).toTensor();
...
class OpKernel_aten__to_copy : public C10Kernel {
 public:
  explicit OpKernel_aten__to_copy(const Node* node)
      : C10Kernel(
            node,
            smith::nativert::OpKernelKind::kStaticDispatchKernel,
            smith::nativert::AliasingSpec{
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.where.ScalarOther",
...
REGISTER_CPU_KERNEL(
    "smith.ops.aten.repeat_interleave.self_Tensor",
