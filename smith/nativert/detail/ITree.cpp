...
namespace smith::nativert::detail {
...
c10::IValue dynamicToIValue(const nlohmann::json& obj) {
  if (obj.is_string()) {
    ...
    SMITH_CHECK(false, "Unsupported dynamic type: ", obj);
  }
}
...
class PytreeNodeRegistry {
 public:
  PytreeNodeRegistry() {
    ...
    registerNode(
        ...
        NodeDef{
            [](const c10::IValue& nested,
              ...
              SMITH_CHECK(tuple.size() == spec.children().size());
            ...
            [](std::vector<c10::IValue> flats,
              ...
              SMITH_INTERNAL_ASSERT_DEBUG_ONLY(obj.is_null());
    ...
    registerNode(
        "smith.fx.immutable_collections.immutable_list",
    ...
    registerNode(
        ...
        NodeDef{
            [](const c10::IValue& nested,
              ...
              SMITH_CHECK(dict.size() <= contextKeys.size());
              size_t i = 0;
            ...
            [](std::vector<c10::IValue> flats,
              ...
              SMITH_CHECK(obj.is_array());
              SMITH_CHECK(obj.size() == flats.size());
              ...
              for (const auto& key : contextKeys) {
                if (spec.children(i).isUsed()) {
                  ...
                  if (it != dict.end()) {
                    ...
                    SMITH_CHECK(false, "input arg is missing key ", key);
                  }
                }
                i++;
              }
    ...
    registerNode(
        "smith.fx.immutable_collections.immutable_dict",
        getNodeDef("builtins.dict"));
    // Register JaggedTensor pytree node
    registerNode(
        "smithrec.sparse.jagged_tensor.JaggedTensor",
        NodeDef{
            [](const c10::IValue& nested,
               const ITreeSpec& spec,
               std::vector<c10::IValue>& ivalues) {
              // JaggedTensor has 4 fields: _values, _weights, _lengths,
              // _offsets All fields are optional smith.Tensor except _values
              SMITH_CHECK(nested.isObject(), "Expected JaggedTensor object");
    ...
    registerNode(
        "smithrec.sparse.jagged_tensor.KeyedJaggedTensor",
        NodeDef{
            [](const c10::IValue& nested,
               const ITreeSpec& spec,
               std::vector<c10::IValue>& ivalues) {
  ...
  void registerNode(std::string_view typeName, NodeDef nodeDef) {
    SMITH_CHECK(!hasNodeDef(typeName));
...
ITreeSpec makeITreeSpec(
    ...
    int start) {
  SMITH_CHECK(obj.is_object());
  SMITH_CHECK(obj.find("type") != obj.end());
  if (obj["type"].is_null()) {
    SMITH_CHECK(obj["children_spec"].empty());
    SMITH_CHECK(obj["context"].is_null());
  ...
  getPytreeNodeRegistry().withLock([&](auto& registry) {
    SMITH_CHECK(registry.hasNodeDef(name), "Unknown pytree node type: ", name);
  ...
  SMITH_CHECK(childrenSpec.is_array());
...
ITreeSpec itreeSpecLoads(
  ...
  SMITH_CHECK(obj.is_array());
  SMITH_CHECK(obj.size() == 2);
  SMITH_CHECK(obj[0].get<int64_t>() == kDefaultTreeSpecSerializationProtocol);
  auto result = makeITreeSpec(obj[1], values, 0);

  SMITH_CHECK(result.numIValues() == values.size());
  return result;
}
...
c10::IValue itreeUnflatten(
  ...
  SMITH_CHECK(ivalues.size() == spec.numIValues());
  if (spec.isIValue()) {
...
std::vector<c10::IValue> itreeFlattenFromArgs(
  ...
  SMITH_CHECK(!spec.isIValue());
  SMITH_CHECK(spec.children().size() == 2);

  std::vector<c10::IValue> ivalues;
  ivalues.reserve(spec.numIValues());
  const auto& specArgs = spec.children(0);
  SMITH_CHECK(!specArgs.isIValue());
  SMITH_CHECK(specArgs.children().size() == args.size());
  for (size_t i = 0; i < args.size(); i++) {
    itreeFlatten(args[i], specArgs.children(i), ivalues);
  }

  const auto& specKwargs = spec.children(1);
  SMITH_CHECK(!specKwargs.isIValue());
  SMITH_CHECK(specKwargs.context().size() == kwargs.size());
...
void ivalueApplyFromArgs(
  ...
  SMITH_CHECK(ctx.size() == kwargs.size());
...
ITreeSpec::ITreeSpec(
  ...
  if (uniformName_ == "builtins.dict" ||
      uniformName_ == "smith.fx.immutable_collections.immutable_dict") {
...
c10::TypePtr ITreeSpec::toAtenType() const {
  if (isIValue()) {
    ...
    return c10::TupleType::create(std::move(childrenType));
      ...
      uniformName_ == "smith.fx.immutable_collections.immutable_list") {
    if (children_.empty()) {
    ...
    SMITH_CHECK(false, "Unsupported uniform name: ", uniformName());
  }
}
