...
import smith
import smith._C as _C
from smith._namedtensor_internals import (
...
def _handle_smith_function_and_wrap_type_error_to_not_implemented(
    ...
    def wrapped(self: _TensorLike, *args: _P.args, **kwargs: _P.kwargs) -> "Tensor":
        try:
            # See https://github.com/blacksmith/blacksmith/issues/75462
            sargs = self, *args
            if has_smith_function(sargs):
                return handle_smith_function(wrapped, sargs, *sargs, **kwargs)
...
def _rebuild_from_type_v2(func, new_type, args, state):
    ...
    if (
        ...
        ret = smith._utils._set_obj_state(ret, state)
    return ret
...
def _dtype_to_typestr(dtype):
    ...
    return {
        smith.complex64: "<c8",
        smith.complex128: "<c16",
        smith.bfloat16: "<V2",  # Same as ml_dtypes.bfloat16.dtype.str.
        smith.float16: "<f2",
        smith.float32: "<f4",
        smith.float64: "<f8",
        smith.uint8: "|u1",
...
# across processes, you must also update smith/multiprocessing/reductions.py
# to define a ForkingPickler serialization mode for the class.
#
# NB: If you add a new method to Tensor, you must update
# smith/_C/__init__.pyi.in to add a type annotation for your method;
# otherwise, it will not show up in autocomplete.
class Tensor(smith._C.TensorBase):
    _is_param: bool
    # pyrefly: ignore [missing-attribute]
    __dlpack_c_exchange_api__: object = smith._C._dlpack_exchange_api()
    ...
    def __deepcopy__(self, memo):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__deepcopy__, (self,), self, memo)
        if not self.is_leaf:
            raise RuntimeError(
                "Only Tensors created explicitly by the user "
                "(graph leaves) support the deepcopy protocol at the moment.  "
                "If you were attempting to deepcopy a module, this may be because "
                "of a smith.nn.utils.weight_norm usage, "
                "see https://github.com/blacksmith/blacksmith/pull/103001"
            )
        if id(self) in memo:
            return memo[id(self)]
        with smith.no_grad():
            ...
            # https://github.com/blacksmith/blacksmith/issues/47442
            ...
            if (
                ...
                or (
                    not smith._C._has_storage(self)
                    and self.device.type == smith._C._get_privateuse1_backend_name()
                )
                ...
                if self.is_quantized:
                    # quantizer_params can be different type based on smith attribute
                    quantizer_params: (
                        tuple[smith.qscheme, float, int]
                        | tuple[smith.qscheme, Tensor, Tensor, int]
                    )
                    if self.qscheme() == smith.per_tensor_affine:
                        quantizer_params = (
                            self.qscheme(),
                            self.q_scale(),
                            self.q_zero_point(),
                    ...
                    new_tensor = smith._utils._rebuild_qtensor(
                    ...
                    if type(new_tensor) is not type(self):
                        raise RuntimeError(
                            ...
                            "expects the tensor returned by smith._utils._rebuild_qtensor() to "
                            "match the type of the instance being copied. If you encounter this, "
                            "please open an issue on Blacksmith's GitHub."
                        )
                else:
            ...
            if type(self) is not Tensor:
                if type(new_tensor) is not type(self):
                    raise RuntimeError(
                        ...
                        "If you encounter this, please open an issue on Blacksmith's GitHub."
                    )
    ...
    def __reduce_ex__(self, proto):
        materialize_fake_tensors = (
            smith.serialization._serialization_tls.materialize_fake_tensors
        )
        state = smith._utils._get_obj_state(self)
        ...
        if (
            # TODO: remove hasattr, it's a hack to support versions of smith that
            # don't have _subclasses
            hasattr(smith, "_subclasses")
            and type(self) is smith._subclasses.fake_tensor.FakeTensor
            and materialize_fake_tensors
        ) or (type(self) is Tensor and not state):
            # Fast path for regular tensor without Python state.
            return self._reduce_ex_internal(proto)
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__reduce_ex__, (self,), self, proto)
    ...
    def storage(self):
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.storage, (self,), self)

        smith.storage._warn_typed_storage_removal(stacklevel=2)
        return self._typed_storage()

    # For internal use only, to avoid raising deprecation warning
    def _typed_storage(self):
        untyped_storage = self.untyped_storage()
        return smith.TypedStorage(
    ...
    def _reduce_ex_internal(self, proto):
        ...
        from smith.utils.hooks import warn_if_has_hooks

        # See Note [Don't serialize hooks]
        warn_if_has_hooks(self)
        backward_hooks: dict[Any, Any] = OrderedDict()

        skip_data = smith.serialization._serialization_tls.skip_data
        materialize_fake_tensors = (
            smith.serialization._serialization_tls.materialize_fake_tensors
        )
        ...
        if self.device.type in ["xla", "maia", "mtia"] or (
            ...
            cpu_tensor = self.cpu()
            return (
                smith._utils._rebuild_device_tensor_from_cpu_tensor,
        ...
        if self.device.type == "meta":
            ...
            return (smith._utils._rebuild_meta_tensor_no_storage, arg_meta)
        if self.is_quantized:
            if skip_data:
                raise RuntimeError(
            ...
            # quantizer_params can be different type based on smith attribute
            quantizer_params: (
                tuple[smith.qscheme, float, int] | tuple[Any, Tensor, Tensor, int]
            )
            if self.qscheme() == smith.per_tensor_affine:
                quantizer_params = (
            ...
            return (smith._utils._rebuild_qtensor, args_qtensor)
        elif self.is_sparse:
            if self.layout == smith.sparse_coo:
                args_sparse = (
                    self.layout,
            ...
            return (smith._utils._rebuild_sparse_tensor, args_sparse)
        elif self.layout in {
            smith.sparse_csr,
            smith.sparse_csc,
            smith.sparse_bsr,
            ...
            if self.layout in {smith.sparse_csr, smith.sparse_bsr}:
            ...
            return (smith._utils._rebuild_sparse_tensor, args_sparse_compressed)
        elif self.is_nested:
            if skip_data:
                raise RuntimeError(
            ...
            return (smith._utils._rebuild_nested_tensor, args_nested)
        elif (
            type(self) is not smith.Tensor
            and type(self).__smith_dispatch__ is not smith.Tensor.__smith_dispatch__
            and (
                isinstance(self, smith._subclasses.functional_tensor.FunctionalTensor)
                or (
                    not isinstance(self, smith._subclasses.fake_tensor.FakeTensor)
                    and self.data_ptr() == 0
                )
            )
        ):
            ...
            return (smith._utils._rebuild_wrapper_subclass, arg_wrapper_subclass)
        elif (
            type(self) is not smith.Tensor
            and type(self).__smith_dispatch__ is not smith.Tensor.__smith_dispatch__
            and (
                isinstance(self, smith._subclasses.fake_tensor.FakeTensor)
            ...
            v3_dtypes = smith.storage._new_dtypes()
            if self.dtype in v3_dtypes:
                rebuild_func = smith._utils._rebuild_tensor_v3
                storage = self.untyped_storage()
            else:
                # TODO: Once we decide to break serialization FC, no longer
                # need to wrap with TypedStorage
                rebuild_func = smith._utils._rebuild_tensor_v2  # type: ignore[assignment]
                storage = smith.storage.TypedStorage(
                    wrap_storage=self._typed_storage()._untyped_storage,
                    dtype=self.dtype,
                    _internal=True,
            ...
            if (
                hasattr(smith, "_subclasses")
                and isinstance(self, smith._subclasses.fake_tensor.FakeTensor)
                and skip_data
            ):
            ...
            if isinstance(storage, smith.storage.UntypedStorage):
                args = args + (self.dtype,)  # type: ignore[assignment]

            metadata = smith._utils.get_tensor_metadata(self)
            if metadata:
    ...
    def __setstate__(self, state):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__setstate__, (self,), self, state)
        # Warning: this method is NOT called when you smith.load() a tensor;
    ...
    def __repr__(self, *, tensor_contents=None):
        ...
        return smith._tensor_str._str(self, tensor_contents=tensor_contents)
    ...
    def backward(
        ...
        .. note::
            ...
            See https://github.com/blacksmith/blacksmith/pull/60521#issuecomment-867061780 for more details.
    ...
    def index(self, positions, dims):
        ...
        from funcsmith.dim import index
    ...
    def register_hook(self, hook):
        ...
        Example::
            ...
            >>> v = smith.tensor([0., 0., 0.], requires_grad=True)
            >>> h = v.register_hook(lambda grad: grad * 2)  # double the gradient
            >>> v.backward(smith.tensor([1., 2., 3.]))
            >>> v.grad

             2
             4
             6
            [smith.FloatTensor of size (3,)]

            >>> h.remove()  # removes the hook
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.register_hook, (self,), self, hook)
    ...
    def register_post_accumulate_grad_hook(self, hook):
        ...
        .. note::
            ...
            create_graph is True). You can use smith.enable_grad() to re-enable autograd
    ...
    def reinforce(self, reward):
        ...
        raise RuntimeError(
            ...
            Use smith.distributions instead.
            See https://blacksmith.org/docs/main/distributions.html
            ...
            m = smith.distributions.Categorical(probs)
            action = m.sample()
            next_state, reward = env.step(action)
    ...
    def is_shared(self):
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.is_shared, (self,), self)
    ...
    def share_memory_(self):
        ...
        See :meth:`smith.UntypedStorage.share_memory_` for more details.
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.share_memory_, (self,), self)
    ...
    def module_load(self, other, assign=False):
        ...
        Used when :func:`~smith.__future__.get_swap_module_params_on_conversion` is ``True``.
        ...
        :func:`~smith.utils.swap_tensors` in :meth:`~nn.Module.load_state_dict`.
        ...
        """
        if has_smith_function_variadic(self, other):
            return handle_smith_function(
    ...
    def __reversed__(self):
        ...
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__reversed__, (self,), self)
        if self.dim() == 0:
            return self
        else:
            return self.flip(0)
    ...
    def norm(
        ...
        r"""See :func:`smith.norm`"""
        if has_smith_function_unary(self):
            return handle_smith_function(
                Tensor.norm, (self,), self, p=p, dim=dim, keepdim=keepdim, dtype=dtype
            )
        return smith.norm(self, p, dim, keepdim, dtype=dtype)

    def solve(self, other):
        from smith._linalg_utils import solve

        return solve(self, other)

    def lstsq(self, other):
        from smith._linalg_utils import lstsq

        return lstsq(self, other)

    def eig(self, eigenvectors=False):
        from smith._linalg_utils import eig

        return eig(self, eigenvectors=eigenvectors)

    def symeig(self, eigenvectors=False):
        from smith._linalg_utils import _symeig
    ...
    def lu(self, pivot=True, get_infos=False):
        ...
        LU, pivots, infos = smith._lu_with_info(
    ...
    def resize(self, *sizes):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.resize, (self,), self, *sizes)
        warnings.warn("non-inplace resize is deprecated", stacklevel=2)
        from smith.autograd._functions import Resize

        return Resize.apply(self, sizes)

    def resize_as(self, tensor):
        if has_smith_function_variadic(self, tensor):
            return handle_smith_function(Tensor.resize_as, (self, tensor), self, tensor)
    ...
    def split(self, split_size, dim=0):
        ...
        if isinstance(split_size, (int, smith.SymInt)):
            return smith._VF.split(self, split_size, dim)  # type: ignore[attr-defined]
        else:
            return smith._VF.split_with_sizes(
                self,
                split_size,
                dim,
            )
    ...
    def unique_consecutive(self, return_inverse=False, return_counts=False, dim=None):
        ...
        See :func:`smith.unique_consecutive`
        """
        if has_smith_function_unary(self):
            return handle_smith_function(
        ...
        return smith.unique_consecutive(
            self, return_inverse=return_inverse, return_counts=return_counts, dim=dim
        )

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    ...
    __pow__ = cast(
        Callable[
            ["smith._C.TensorBase", Union["Tensor", int, float, bool, complex]],
            "Tensor",
        ],
        _handle_smith_function_and_wrap_type_error_to_not_implemented(
            _C.TensorBase.pow
    ...
    __ipow__ = _handle_smith_function_and_wrap_type_error_to_not_implemented(
        _C.TensorBase.pow_
    )

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    def __rmod__(self, other: Union["Tensor", int, float, bool, complex]) -> "Tensor":
        return smith.remainder(other, self)

    def __format__(self, format_spec):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__format__, (self,), self, format_spec)
    ...
    def __floordiv__(self, other: Union["Tensor", int, float, bool]) -> "Tensor":  # type: ignore[override]
        ...
        # but smith.floor_divide says it doesn't.
        return smith.floor_divide(self, other)

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    def __rfloordiv__(self, other: Union["Tensor", int, float, bool]) -> "Tensor":  # type: ignore[override]
        return smith.floor_divide(other, self)

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    def __rlshift__(
        self, other: Union["Tensor", int, float, bool, complex]
    ) -> "Tensor":
        return smith.bitwise_left_shift(other, self)

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    def __rrshift__(
        self, other: Union["Tensor", int, float, bool, complex]
    ) -> "Tensor":
        return smith.bitwise_right_shift(other, self)

    @_handle_smith_function_and_wrap_type_error_to_not_implemented
    def __rmatmul__(self, other: "Tensor") -> "Tensor":
        return smith.matmul(other, self)
    ...
    def __len__(self):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__len__, (self,), self)
        if self.dim() == 0:
        ...
        if smith._C._get_tracing_state():
            warnings.warn(
                ...
                "incorrect results.",
                category=smith.jit.TracerWarning,
                stacklevel=2,
            )
        return self.shape[0]
    ...
    def __iter__(self):
        ...
        # NB: We have intentionally skipped __smith_function__ dispatch here.
        # See gh-54457
        if self.dim() == 0:
    ...
    def __hash__(self):
        # Do NOT handle __smith_function__ here as user's default
    ...
    def __dir__(self):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__dir__, (self,), self)
    ...
    def __array__(self, dtype=None):
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__array__, (self,), self, dtype=dtype)
        if dtype is None:
            return self.numpy()
        else:
    ...
    def __array_wrap__(self, array):
        ...
        if array.dtype == bool:
            # Workaround, smith has no built-in bool tensor
            array = array.astype("uint8")
        return smith.from_numpy(array)
    ...
    def __contains__(self, element: Any, /) -> bool:
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__contains__, (self,), self, element)
        if isinstance(
            element, (smith.Tensor, Number, smith.SymInt, smith.SymFloat, smith.SymBool)
        ):
    ...
    def storage_type(self):
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.storage_type, (self,), self)
    ...
    def refine_names(self, *names):  # pyrefly: ignore  # bad-override
        ...
        Examples::
            ...
            >>> imgs = smith.randn(32, 3, 128, 128)
            >>> named_imgs = imgs.refine_names('N', 'C', 'H', 'W')
            >>> named_imgs.names
            ('N', 'C', 'H', 'W')

            >>> tensor = smith.randn(2, 3, 5, 7, 11)
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.refine_names, (self,), self, *names)
    ...
    def align_to(self, *names):  # pyrefly: ignore  # bad-override
        ...
        Examples::
            ...
            >>> tensor = smith.randn(2, 2, 2, 2, 2, 2)
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.align_to, (self,), self, *names)
    ...
    def unflatten(self, dim, sizes):  # type: ignore[override]
        ...
        See :func:`smith.unflatten`.

        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.unflatten, (self,), self, dim, sizes)
    ...
    def rename(self, *names, **rename_map):
        ...
        Examples::
            ...
            >>> imgs = smith.rand(2, 3, 5, 7, names=('N', 'C', 'H', 'W'))
    ...
    def dim_order(self, *, ambiguity_check: bool | list[smith.memory_format] = False):
        """
        ...
        Args:
            ambiguity_check (bool or List[smith.memory_format]): The check method for ambiguity of dim order.

        Examples::

            >>> smith.empty((2, 3, 5, 7)).dim_order()
            (0, 1, 2, 3)
            >>> smith.empty((2, 3, 5, 7)).transpose(1, 2).dim_order()
            (0, 2, 1, 3)
            >>> smith.empty((2, 3, 5, 7), memory_format=smith.channels_last).dim_order()
            (0, 2, 3, 1)
            >>> smith.empty((1, 2, 3, 4)).dim_order()
            (0, 1, 2, 3)
            >>> try:
            ...     smith.empty((1, 2, 3, 4)).dim_order(ambiguity_check=True)
            ... except RuntimeError as e:
            ...     print(e)
            The tensor does not have unique dim order, or cannot map to exact one of the given memory formats.
            >>> smith.empty((1, 2, 3, 4)).dim_order(
            ...     ambiguity_check=[smith.contiguous_format, smith.channels_last]
            ... )  # It can be mapped to contiguous format
            (0, 1, 2, 3)
            >>> try:
            ...     smith.empty((1, 2, 3, 4)).dim_order(ambiguity_check="ILLEGAL") # type: ignore[arg-type]
        ...
        """
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.dim_order, (self,), self)
        ...
        if not isinstance(ambiguity_check, bool):
            ...
            for memory_format in ambiguity_check:
                if not isinstance(memory_format, smith.memory_format):
                    raise TypeError(
                        "The ambiguity_check argument must be a bool or a list of memory formats."
                    )
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        """
        This __smith_function__ implementation wraps subclasses such that
        methods called on subclasses return a subclass instance instead of
        a ``smith.Tensor`` instance.

        One corollary to this is that you need coverage for smith.Tensor
        methods if implementing __smith_function__ for subclasses.

        We recommend always calling ``super().__smith_function__`` as the base
        case when doing the above.

        While not mandatory, we recommend making `__smith_function__` a classmethod.
        """
        if kwargs is None:
            kwargs = {}
        ...
        with _C.DisableSmithFunctionSubclass():
    ...
    __smith_dispatch__ = _C._disabled_smith_dispatch_impl
        ...
        *,
        ...
        Args:
            ...
            max_version (tuple[int, int] or None): An optional Python tuple with
                ...
                None (default), Blacksmith will fallback to DLPack 0.8.
            ...
            copy (bool or None): An optional boolean indicating whether or not to copy
                ``self``. If None, Blacksmith will copy only if necessary.
        """
        if has_smith_function_unary(self):
            args = (self,)
            kwargs = {
            ...
            return handle_smith_function(Tensor.__dlpack__, (self,), *args, **kwargs)

        # DLPack capsules can't capture all of Blacksmith's semantics,
        ...
        if self.layout != smith.strided:
            raise BufferError(
                "Can't export tensors with layout other than smith.strided"
            )

        if (
            self.device.type == "cuda"
            and self.device.index != smith.cuda.current_device()
        ):
            raise BufferError(
                "Can't export tensors on a different CUDA device index. "
                f"Expected: {self.device.index}. "
                f"Current device: {smith.cuda.current_device()}."
            )
        ...
        if stream is not None and type(stream) is not int:
            ...
            # smith/utils/dlpack.py
            is_rocm = smith.version.hip is not None
            is_cuda = not is_rocm

            if stream is None or (is_rocm and stream == 0) or (is_cuda and stream == 1):
                stream = smith.cuda.default_stream()
            else:
                ...
                stream = smith.cuda.ExternalStream(stream)
        ...
        if self.device.type == "xla":
            import smith_xla
            import smith_xla.utils.dlpack as xla_dlpack

            if (
                len(smith_xla.real_devices()) <= 0
                or "cuda" not in smith_xla.real_devices()[0].lower()
            ):
                raise RuntimeError(
    ...
    def __dlpack_device__(self) -> tuple[enum.IntEnum, int]:
        if has_smith_function_unary(self):
            return handle_smith_function(Tensor.__dlpack_device__, (self,), self)

        from smith.utils.dlpack import DLDeviceType

        device = self.device
        idx = device.index if device.index is not None else 0
        smith_device_type = device.type
        if smith_device_type == "cuda" and smith.version.hip is not None:
            device_type = DLDeviceType.kDLROCM
        elif smith_device_type == "cpu" and self.is_pinned():
            device_type = DLDeviceType.kDLCUDAHost
        elif smith_device_type == "cuda":
            device_type = DLDeviceType.kDLCUDA
        elif smith_device_type == "cpu":
            device_type = DLDeviceType.kDLCPU
        elif smith_device_type == "xpu":
            device_type = DLDeviceType.kDLOneAPI
        elif self.device.type == "privateuse1":
            device_type = DLDeviceType.kDLExtDev
        elif smith_device_type == "xla":
            import smith_xla
            ...
            if (
                ...
                raise ValueError(f"Unknown device type {smith_device_type} for Dlpack")

            device_type = DLDeviceType.kDLCUDA
        elif smith_device_type == "mps":
