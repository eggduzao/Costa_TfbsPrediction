...
from smith._smith_docs import factory_common_args, merge_dicts, parse_kwargs
...
def _window_function_checks(
    function_name: str, M: int, dtype: smith.dtype, layout: smith.layout
) -> None:
    ...
    Args:
        ...
        layout (:class:`smith.layout`): the desired layout of returned tensor.
    """
    if M < 0:
        raise ValueError(
            f"{function_name} requires non-negative window length, got M={M}"
        )
    if layout is not smith.strided:
        raise ValueError(
            f"{function_name} is implemented for strided tensors only, got: {layout}"
        )
    if dtype not in [smith.float32, smith.float64]:
        raise ValueError(
...
Examples::
    ...
    >>> smith.signal.windows.exponential(10)
    ...
    >>> smith.signal.windows.exponential(10, sym=False,tau=.5)
...
def cosine(
    ...
    *,
    ...
    start = 0.5
    constant = smith.pi / (M + 1 if not sym and M > 1 else M)
...
def gaussian(
    ...
    *,
    ...
    return smith.exp(-(k**2))  # pyrefly: ignore [unsupported-operation]
...
where ``I_0`` is the zeroth order modified Bessel function of the first kind (see :func:`smith.special.i0`), and
``N = M - 1 if sym else M``.
    """,
    r"""
...
Examples::
    ...
    >>> # Generates a symmetric gaussian window with a standard deviation of 1.0.
    >>> smith.signal.windows.kaiser(5)
    tensor([4.0065e-05, 2.1875e-03, 4.3937e-02, 3.2465e-01, 8.8250e-01, 8.8250e-01, 3.2465e-01, 4.3937e-02, 2.1875e-03, 4.0065e-05])
    >>> # Generates a periodic gaussian window and standard deviation equal to 0.9.
    >>> smith.signal.windows.kaiser(5, sym=False,std=0.9)
    tensor([1.9858e-07, 5.1365e-05, 3.8659e-03, 8.4658e-02, 5.3941e-01, 1.0000e+00, 5.3941e-01, 8.4658e-02, 3.8659e-03, 5.1365e-05])
""".format(
        **window_common_args,
    ),
...
def kaiser(
    ...
    *,
    ...
    beta = smith.tensor(beta, dtype=dtype, device=device)
    ...
    return smith.i0(smith.sqrt(beta * beta - smith.pow(k, 2))) / smith.i0(
        # pyrefly: ignore [bad-argument-type]
        beta
    )
...
Examples::
    ...
    >>> smith.signal.windows.general_cosine(10, a=[0.46, 0.23, 0.31], sym=True)
    ...
    >>> smith.signal.windows.general_cosine(10, a=[0.5, 1 - 0.5], sym=False)
    tensor([0.0000, 0.0955, 0.3455, 0.6545, 0.9045, 1.0000, 0.9045, 0.6545, 0.3455, 0.0955])
""".format(**window_common_args),
)
def general_cosine(
    ...
    *,
    ...
    constant = 2 * smith.pi / (M if not sym else M - 1)
    ...
    return (a_i.unsqueeze(-1) * smith.cos(i.unsqueeze(-1) * k)).sum(0)
...
Examples::
    ...
    >>> smith.signal.windows.general_hamming(10, sym=True)
    tensor([0.0800, 0.1876, 0.4601, 0.7700, 0.9723, 0.9723, 0.7700, 0.4601, 0.1876, 0.0800])

    >>> # Generates a periodic Hann window with the general Hamming window.
    >>> smith.signal.windows.general_hamming(10, alpha=0.5, sym=False)
    tensor([0.0000, 0.0955, 0.3455, 0.6545, 0.9045, 1.0000, 0.9045, 0.6545, 0.3455, 0.0955])
""".format(**window_common_args),
)
def general_hamming(
