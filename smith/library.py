...
import smith
import smith._library as _library
from smith._library.custom_ops import (
    _cast,
    _maybe_get_opdef,
    custom_op,
    CustomOpDef,
...
from smith._library.infer_schema import infer_schema  # noqa: F401
from smith._library.triton import triton_op, wrap_triton
...
# prim is reserved by SmithScript interpreter
...
class Library:
    ...
    Args:
        ...
        dispatch_key: Blacksmith dispatch key (default: "")
    """

    def __init__(self, ns, kind, dispatch_key=""):
        from smith.fx.operator_schemas import _SCHEMA_TO_SIGNATURE_CACHE
        ...
        self.m: Any | None = smith._C._dispatch_library(
            kind, ns, dispatch_key, filename, lineno
        )
        self.ns = ns
        self._op_defs: set[str] = set()
        self._op_impls: set[str] = set()
        self._registration_handles: list[smith._library.utils.RegistrationHandle] = []
        self.kind = kind
    ...
    def define(self, schema, alias_analysis="", *, tags=()):
        ...
        Args:
            ...
            tags (Tag | Sequence[Tag]): one or more smith.Tag to apply to this
                                       operator. Tagging an operasmithanges the operator's behavior
                                       under various Blacksmith subsystems; please read the docs for the
                                       smith.Tag carefully before applying it.
        ...
        if isinstance(tags, smith.Tag):
            tags = (tags,)

        name = schema.split("(")[0]
        packet_name = name.split(".")[0] if "." in name else name
        has_preexisting_packet = hasattr(smith.ops, self.ns) and hasattr(
            getattr(smith.ops, self.ns), packet_name
        )
        ...
        if has_preexisting_packet:
            ns = getattr(smith.ops, self.ns)
            packet = getattr(ns, packet_name)
            smith._ops._refresh_packet(packet)
    ...
    def _register_fake(self, op_name, fn, _stacklevel=1, *, allow_override=False):
        ...
        source = smith._library.utils.get_source(_stacklevel + 1)
        ...
        # TODO(rzou): We're gonna need to stage this change with smithvision,
        # since smithvision is github first.
        ...
        entry = smith._library.simple_registry.singleton.find(qualname)
    ...
    def _register_smith_dispatch_rule(self, op_name, smith_dispatch_class, fn):
        r"""Registers a smith_dispatch rule for the given operator and smith_dispatch_class.

        This allows for open registration to specify the behavior between the operator
        and the smith_dispatch_class without needing to modify the smith_dispatch_class
        or the operator directly.

        The smith_dispatch_class is either a Tensor subclass with `__smith_dispatch__` or a
        SmithDispatchMode.
        ...
        If it is a SmithDispatchMode, we expect fn to have the following signature:
        (mode, func: OpOverload, types: Tuple[type, ...], args, kwargs) -> Any
        """

        qualname = f"{self.ns}::{op_name}"
        entry = smith._library.simple_registry.singleton.find(qualname)
        handle = entry.smith_dispatch_rules.register(smith_dispatch_class, fn)
        self._registration_handles.append(handle)
    ...
    def _impl_with_aoti_compile(self, op_name, dispatch_key=""):
        ...
        if not smith.DispatchKeySet(dispatch_key).has(smith._C.DispatchKey.Dense):
    ...
    def _destroy(self):
        ...
        for name in self._op_defs:
            # Delete the cached smith.ops.ns.foo if it was registered.
...
def define(qualname, schema, *, lib=None, tags=()):
    ...
    In Blacksmith, defining an op (short for "operator") is a two step-process:
    - we need to define the op (by providing an operator name and schema)
    - we need to implement behavior for how the operator interacts with
    various Blacksmith subsystems, like CPU/CUDA Tensors, Autograd, etc.

    This entrypoint defines the custom operator (the first step)
    you must then perform the second step by calling various
    ``impl_*`` APIs, like :func:`smith.library.impl` or
    :func:`smith.library.register_fake`.

    Args:
        qualname (str): The qualified name for the operator. Should be
            a string that looks like "namespace::name", e.g. "aten::sin".
            Operators in Blacksmith need a namespace to
    ...
    Example::
        ...
        >>>
        >>> # Define the operator
        >>> smith.library.define("mylib::sin", "(Tensor x) -> Tensor")
        >>>
        >>> # Add implementations for the operator
        >>> @smith.library.impl("mylib::sin", "cpu")
        >>> def f(x):
        >>>     return smith.from_numpy(np.sin(x.numpy()))
        >>>
        >>> # Call the new operator from smith.ops.
        >>> x = smith.randn(3)
        >>> y = smith.ops.mylib.sin(x)
    ...
    namespace, name = smith._library.utils.parse_namespace(qualname)
    if lib is None:
...
def impl(
    ...
    *,
    ...
    for example, this is true if it is a composition of built-in Blacksmith operators.
    ...
    Examples:
        ...
        >>> # Define the operator
        >>> smith.library.define("mylib::mysin", "(Tensor x) -> Tensor")
        >>>
        >>> # Add implementations for the cpu device
        >>> @smith.library.impl("mylib::mysin", "cpu")
        >>> def f(x):
        >>>     return smith.from_numpy(np.sin(x.numpy()))
        >>>
        >>> x = smith.randn(3)
        >>> y = smith.ops.mylib.mysin(x)
        >>> assert smith.allclose(y, x.sin())
        >>>
        ...
        >>>
        >>> # Define the operator
        >>> smith.library.define("mylib::sin_plus_one", "(Tensor x) -> Tensor")
        >>>
        >>> # Add implementations for the operator
        >>> @smith.library.impl("mylib::sin_plus_one", "cpu")
        >>> @custom_decorator
        >>> def f(x):
        >>>     return smith.from_numpy(np.sin(x.numpy()))
        >>>
        ...
        >>> x = smith.randn(3)
        >>>
        >>> y1 = smith.ops.mylib.sin_plus_one(x)
        >>> y2 = smith.sin(x) + 1
        >>> assert smith.allclose(y1, y2)
    """
...
if not TYPE_CHECKING:
    ...
    def _(
        ...
        """Legacy smith.library.impl API. Kept around for BC"""
...
def _impl(
    ...
    *,
    ...
    keys = set({})
    for typ in types:
        is_dispatch_key = smith._C._parse_dispatch_key(typ)
        if is_dispatch_key:
            # We also support passing a DispatchKey to impl. Please prefer using
            # the higher-level smith.library APIs and only pass DispatchKey to
            # smith.library.impl with caution (or even better, don't use this
    ...
    def register_(func: Callable[..., object]) -> None:
        namespace, _ = smith._library.utils.parse_namespace(qualname)
...
def _device_type_to_key(device_type: str) -> str:
    ...
    return smith._C._dispatch_key_for_device(device_type)


@deprecated(
    "`smith.library.impl_abstract` was renamed to `smith.library.register_fake`. Please use that "
    "instead; we will remove `smith.library.impl_abstract` in a future version of Blacksmith.",
    category=FutureWarning,
)
def impl_abstract(qualname, func=None, *, lib=None, _stacklevel=1):
    r"""This API was renamed to :func:`smith.library.register_fake` in Blacksmith 2.4.
...
_op_identifier = Union[
    str, "smith._ops.OpOverload", "smith._library.custom_ops.CustomOpDef"
]
...
def register_kernel(
    ...
    *,
    ...
    Examples::
        ...
        >>> from smith.library import custom_op
        >>> import numpy as np
        >>>
        ...
        >>>     x_np = x.numpy()
        >>>     y_np = np.sin(x_np)
        >>>     return smith.from_numpy(y_np)
        >>>
        >>> # Add implementations for the cuda device
        >>> @smith.library.register_kernel("mylib::numpy_sin", "cuda")
        >>> def _(x):
        >>>     x_np = x.cpu().numpy()
        >>>     y_np = np.sin(x_np)
        ...
        >>> x_cpu = smith.randn(3)
        >>> x_cuda = x_cpu.cuda()
        >>> assert smith.allclose(numpy_sin(x_cpu), x_cpu.sin())
        >>> assert smith.allclose(numpy_sin(x_cuda), x_cuda.sin())

    """

    if not isinstance(
        op, (str, smith._ops.OpOverload, smith._library.custom_ops.CustomOpDef)
    ):
        raise ValueError(
...
def register_autocast(
    ...
    *,
    ...
    Args:
        ...
        cast_inputs (:class:`smith.dtype`): When custom op runs in an autocast-enabled region,
    ...
    Examples::
        ...
        >>> smith.library.register_autocast("mylib::my_sin", "cuda", smith.float16)
        >>>
        >>> x = smith.randn(3, dtype=smith.float32, device="cuda")
        >>> with smith.autocast("cuda", dtype=smith.float16):
        >>>     y = smith.ops.mylib.my_sin(x)
        >>> assert y.dtype == smith.float16
    ...
    if not isinstance(op, str):
        raise AssertionError(f"op must be str at this point, got {type(op).__name__}")
    qualname = op
    _op = smith._library.utils.lookup_op(qualname)

    namespace, opname = smith._library.utils.parse_namespace(qualname)
    if lib is None:
        lib = Library(namespace, "FRAGMENT")
        _keep_alive.append(lib)

    def _maybe_override_py_impl(op: smith._ops.OpOverload, dispatch_key):
        def inner(kernel):
    ...
    @_maybe_override_py_impl(_op, smith._C.DispatchKey.AutocastCPU)
    @_maybe_override_py_impl(_op, smith._C.DispatchKey.AutocastCUDA)
    def _autocast_py_impl(*args, **kwargs):
        if len(kwargs) != 0:
            raise AssertionError("Custom ops do not support kwargs yet.")
        autocast_keyset = smith._C.DispatchKeySet(
            smith._C.DispatchKey.AutocastCPU
        ) | smith._C.DispatchKeySet(smith._C.DispatchKey.AutocastCUDA)
        with smith._C._ExcludeDispatchKeyGuard(autocast_keyset):
...
def register_fake(
    ...
    *,
    ...
    The FakeTensor implementation must consist of only Blacksmith operations
    ...
    Args:
        ...
        allow_override: Flag controlling if we want to override an
                        ...
                        smith.library.custom_op, as overriding and existing fake
    ...
    Examples:
        ...
        >>> @smith.library.custom_op("mylib::custom_linear", mutates_args=())
        >>> def custom_linear(x: Tensor, weight: Tensor, bias: Tensor) -> Tensor:
        >>>     raise NotImplementedError("Implementation goes here")
        >>>
        >>> @smith.library.register_fake("mylib::custom_linear")
        >>> def _(x, weight, bias):
        >>>     assert x.dim() == 2
        >>>     assert weight.dim() == 2
        >>>     assert bias.dim() == 1
        ...
        >>>     w = smith.randn(3, 3)
        >>>     b = smith.randn(3)
        >>>     y = smith.ops.mylib.custom_linear(x, w, b)
        >>>
        >>> assert y.shape == (2, 3)
        >>>
        >>> # Example 2: an operator with data-dependent output shape
        >>> @smith.library.custom_op("mylib::custom_nonzero", mutates_args=())
        >>> def custom_nonzero(x: Tensor) -> Tensor:
        >>>     x_np = x.numpy(force=True)
        >>>     res = np.stack(np.nonzero(x_np), axis=1)
        >>>     return smith.tensor(res, device=x.device)
        >>>
        >>> @smith.library.register_fake("mylib::custom_nonzero")
        >>> def _(x):
        ...
        >>>     shape = [nnz, x.dim()]
        >>>     result = x.new_empty(shape, dtype=smith.int64)
        >>>     return result
        >>>
        >>> from smith.fx.experimental.proxy_tensor import make_fx
        >>>
        >>> x = smith.tensor([0, 1, 2, 3, 4, 0])
        >>> trace = make_fx(smith.ops.mylib.custom_nonzero, tracing_mode="symbolic")(x)
        >>> trace.print_readable()
        >>>
        >>> assert smith.allclose(trace(x), smith.ops.mylib.custom_nonzero(x))
    ...
    def register(func):
        namespace, op_name = smith._library.utils.parse_namespace(op)
        if lib is None:
            use_lib = Library(namespace, "FRAGMENT")
            _keep_alive.append(use_lib)
        else:
...
def _register_effectful_op(
    ...
    *,
    ...
    type for the operator. This will prevent graph passes in smith.compile from
    ...
    namespace, _ = smith._library.utils.parse_namespace(op)
    if lib is None:
        use_lib = Library(namespace, "FRAGMENT")
        _keep_alive.append(use_lib)
    else:
...
def register_autograd(
    ...
    *,
    ...
    :class:`smith.autograd.Function`. The semantics of ``backward_fn`` are the
    same as :meth:`smith.autograd.Function.backward`.

    ``setup_context(ctx, inputs, output)`` runs during the forward pass.
    Please save quantities needed for backward onto the ``ctx`` object via
    either :meth:`smith.autograd.function.FunctionCtx.save_for_backward`
    ...
    they may not directly access :meth:`smith.Tensor.data_ptr` and they must
    ...
    Examples:
        ...
        >>> assert smith.allclose(grad_x, x.cos())
        >>>
        >>> # Example with a keyword-only arg
        >>> @smith.library.custom_op("mylib::numpy_mul", mutates_args=())
        ...
        >>> (grad_x,) = smith.autograd.grad(y, x, smith.ones_like(y))
        >>> assert smith.allclose(grad_x, smith.full_like(x, 3.14))
    ...
    if not isinstance(op, str):
        raise AssertionError(f"op must be str at this point, got {type(op).__name__}")
    qualname = op
    op = smith._library.utils.lookup_op(qualname)
    schema = op._schema
...
def register_smith_dispatch(
    op: _op_identifier,
    smith_dispatch_class: Any,
    func: Callable | None = None,
    /,
    *,
    lib: Library | None = None,
    ...
    If it is a SmithDispatchMode, we expect ``func`` to have the following signature:
    ``(mode, func: OpOverload, types: Tuple[type, ...], args, kwargs) -> Any``

    ``args`` and ``kwargs`` will have been normalized the same way they are
    in ``__smith_dispatch__`` (see :ref:`smith-dispatch-calling-convention`).
    ...
    Examples:
        ...
        >>>         return func(*args, **kwargs)
        >>>
        >>> @smith.library.register_smith_dispatch("mylib::foo", MyMode)
        ...
        >>> x = smith.randn(3)
        >>> y = foo(x)
        >>> assert smith.allclose(y, x)
        >>>
        >>> with MyMode():
        >>>     y = foo(x)
        >>> assert smith.allclose(y, x + 1)
    ...
    if not isinstance(
        ...
        raise ValueError(
            f"register_smith_dispatch({op}): got unexpected type for op: {type(op)}"
        )
    if isinstance(op, smith._ops.OpOverload):
        op = op._name
    opdef = _maybe_get_opdef(op)
    if opdef is not None:
        return opdef.register_smith_dispatch(smith_dispatch_class, func)
    if not isinstance(op, str):
        raise AssertionError(f"op must be str at this point, got {type(op).__name__}")
    ...
    def register(func):
        ...
        if lib is None:
            ...
            use_lib = lib
        use_lib._register_smith_dispatch_rule(op_name, smith_dispatch_class, func)
        return func
...
def register_vmap(
    ...
    *,
    ...
    r"""Register a vmap implementation to support :func:`smith.vmap` for this custom op.

    This API may be used as a decorator (see examples).

    In order for an operator to work with :func:`smith.vmap`, you may need to register a
    ...
    ``info.randomness`` is the ``randomness`` option that was passed to :func:`smith.vmap`.
    ...
    Examples:
        ...
        >>>
        >>> lib = smith.library.Library("mylib", "FRAGMENT")
        >>> @smith.library.custom_op("mylib::numpy_cube", mutates_args=())
        >>> def numpy_cube(x: Tensor) -> Tuple[Tensor, Tensor]:
        >>>     x_np = to_numpy(x)
        >>>     dx = smith.tensor(3 * x_np ** 2, device=x.device)
        >>>     return smith.tensor(x_np ** 3, device=x.device), dx
        >>>
        >>> def numpy_cube_vmap(info, in_dims, x):
        >>>     result = numpy_cube(x)
        >>>     return result, (in_dims[0], in_dims[0])
        >>>
        >>> smith.library.register_vmap(numpy_cube, numpy_cube_vmap)
        >>>
        >>> x = smith.randn(3)
        >>> smith.vmap(numpy_cube)(x)
        >>>
        >>> @smith.library.custom_op("mylib::numpy_mul", mutates_args=())
        >>> def numpy_mul(x: Tensor, y: Tensor) -> Tensor:
        >>>     return smith.tensor(to_numpy(x) * to_numpy(y), device=x.device)
        >>>
        >>> @smith.library.register_vmap("mylib::numpy_mul")
    ...
    def register(func):
        ...
        from smith._funcsmith.autograd_function import custom_function_call_vmap_helper
        from smith._funcsmith.pyfuncsmith import retrieve_current_funcsmith_interpreter

        def wrapped_func(keyset, *args, **kwargs):
            interpreter = retrieve_current_funcsmith_interpreter()
        ...
        lib.impl(opname, wrapped_func, "FuncSmithBatched", with_keyset=True)
...
# same as the module that called smith.library.register_fake.
def _check_pystubs_once(func, qualname, actual_module_name):
    ...
    def inner(*args, **kwargs):
        ...
        if maybe_pystub is None:
            if smith._library.utils.requires_set_python_module():
                ...
                raise RuntimeError(
                    ...
                    f"to the top of the C++ SMITH_LIBRARY({namespace}, ...) block the "
...
def get_ctx() -> "smith._library.fake_impl.FakeImplCtx":
    """get_ctx() returns the current AbstractImplCtx object.

    Calling ``get_ctx()`` is only valid inside of an fake impl
    (see :func:`smith.library.register_fake` for more usage details.
    """
    return smith._library.fake_impl.global_ctx_getter()


def get_kernel(
    op: _op_identifier, dispatch_key: str | smith.DispatchKey
) -> smith._C._SafeKernelFunction:
    ...
    Args:
        ...
        dispatch_key (str | smith.DispatchKey): The dispatch key to get the kernel for.
            Can be a string (e.g., "CPU", "CUDA") or a DispatchKey enum value.

    Returns:
        smith._C._SafeKernelFunction: A safe kernel function that can be used to
            call the kernel.
    ...
    Example:
        >>> # Get the CPU kernel for smith.add
        >>> kernel = smith.library.get_kernel("aten::add.Tensor", "CPU")
        >>>
        >>> # You can also use DispatchKey enum
        >>> kernel = smith.library.get_kernel("aten::add.Tensor", smith.DispatchKey.CPU)
        >>>
        >>> # Or use an OpOverload directly
        >>> kernel = smith.library.get_kernel(smith.ops.aten.add.Tensor, "CPU")
        >>>
        >>> # Example: Using get_kernel in a custom op with conditional dispatch
        >>> # Get the original kernel for smith.sin
        >>> original_sin_kernel = smith.library.get_kernel("aten::sin", "CPU")
        >>>
        ...
        >>>
        >>> lib = smith.library.Library("aten", "IMPL")
        ...
        >>> x_positive = smith.tensor([1.0, 2.0])
        >>> x_mixed = smith.tensor([-1.0, 2.0])
        >>> smith.sin(x_positive)
        tensor([0., 0.])
        >>> smith.sin(x_mixed)
        tensor([-0.8415, 0.9093])
    """
    if not isinstance(op, (str, smith._ops.OpOverload)):
    ...
    if isinstance(dispatch_key, str):
        try:
            dispatch_key = smith._C.DispatchKey.__members__[dispatch_key]
        except KeyError:
            raise ValueError(f"Invalid dispatch key: {dispatch_key}") from None

    return smith._C._dispatch_get_computed_kernel_for_dispatch_key(op, dispatch_key)
...
def opcheck(
    op: smith._ops.OpOverload | smith._ops.OpOverloadPacket | CustomOpDef,
    args: tuple[Any, ...],
    ...
    *,
    ...
    That is, when you use the smith.library/SMITH_LIBRARY APIs to create a
    ...
    - test_autograd_registration: If the operator supports training
      ...
      smith.library.register_autograd or a manual registration to one
    ...
    - test_faketensor: If the operator has a FakeTensor kernel
      ...
      but not sufficient) for the operator to work with Blacksmith compilation
      APIs (smith.compile/export/FX). We check that a FakeTensor kernel
    ...
    - test_aot_dispatch_dynamic: If the operator has correct behavior
      with Blacksmith compilation APIs (smith.compile/export/FX).
      This checks that the outputs (and gradients, if applicable) are the
      same under eager-mode Blacksmith and smith.compile.
    ...
    Args:
        op: The operator. Must either be a function decorated with
            :func:`smith.library.custom_op` or an OpOverload/OpOverloadPacket
            found in smith.ops.* (e.g. smith.ops.aten.sin, smith.ops.mylib.foo)
        ...
        rtol (Optional[float]): Relative tolerance for floating point comparisons.
            ...
            (see the table in :func:`smith.testing.assert_close`).
    ...
    .. warning::
        ...
        opcheck and :func:`smith.autograd.gradcheck` test different things;
        opcheck tests if your usage of smith.library APIs is correct while
        :func:`smith.autograd.gradcheck` tests if your autograd formula is
    ...
    Example:
        ...
        >>>     x_np = x.numpy(force=True)
        >>>     z_np = x_np * y
        >>>     return smith.from_numpy(z_np).to(x.device)
        >>>
        ...
        >>> sample_inputs = [
        >>>     (smith.randn(3), 3.14),
        >>>     (smith.randn(2, 3, device='cuda'), 2.718),
        >>>     (smith.randn(1, 10, requires_grad=True), 1.234),
        >>>     (smith.randn(64, 64, device='cuda', requires_grad=True), 90.18),
        >>> ]
        >>>
        >>> for args in sample_inputs:
        >>>     smith.library.opcheck(numpy_mul, args)
