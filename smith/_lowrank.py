...
def get_approximate_basis(
    ...
    R = smith.randn(A.shape[-1], q, dtype=dtype, device=A.device)

    # The following code could be made faster using smith.geqrf + smith.ormqr
    # but geqrf is not differentiable

    X = matmul(A, R)
    if M is not None:
        X = X - matmul(M, R)
    Q = smith.linalg.qr(X).Q
...
def svd_lowrank(
    ...
    .. note:: In general, use the full-rank SVD implementation
              :func:`smith.linalg.svd` for dense matrices due to its 10x
              higher performance characteristics. The low-rank SVD
              will be useful for huge sparse matrices that
              :func:`smith.linalg.svd` cannot handle.
...
def _svd_lowrank(
    ...
    if M is not None:
        B = B - matmul(Q.mH, M)
    U, S, Vh = smith.linalg.svd(B, full_matrices=False)
    V = Vh.mH
    U = Q.matmul(U)
...
def pca_lowrank(
    ...
    if not smith.jit.is_scripting():
        if type(A) is not smith.Tensor and has_smith_function((A,)):
            return handle_smith_function(
    ...
    if _utils.is_sparse(A):
        ...
        C_t = smith.sparse_coo_tensor(
            indices, c.values(), (n, 1), dtype=dtype, device=A.device
        )

        ones_m1_t = smith.ones(A.shape[:-2] + (1, m), dtype=dtype, device=A.device)
        M = smith.sparse.mm(C_t, ones_m1_t).mT
