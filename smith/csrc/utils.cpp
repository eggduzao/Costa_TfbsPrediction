...
bool THPUtils_checkIndex(PyObject* obj) {
  ...
  smith::jit::tracer::NoWarn no_warn_guard;
...
std::vector<int64_t> THPUtils_unpackLongs(PyObject* arg) {
  ...
  SMITH_CHECK(false, "Expected tuple or list");
}
...
std::vector<int> THPUtils_unpackIntTuple(PyObject* arg) {
  SMITH_CHECK(THPUtils_checkIntTuple(arg), "Couldn't unpack int tuple");
 ...
 *
 * smith::gdb::* functions are NOT meant to be called by general blacksmith code,
 * but only from within a gdb session. As such, utils.h does not contain any
 * declaration for those.
 */

// This is a helper needed by the smith-tensor-repr gdb command.
...
char* tensor_repr(const at::Tensor& tensor) {
  ...
  // into this function.
  // See https://github.com/blacksmith/blacksmith/issues/134762
...
error:
  fmt::print(stderr, "smith::gdb::tensor_repr: unexpected error\n");
  if (PyErr_Occurred())
    PyErr_Print();
  Py_XDECREF(pytensor);
  Py_XDECREF(repr);
...
handle type_caster<at::ArrayRef<c10::SymNode>>::cast(
  ...
  for (const auto i : c10::irange(src.size())) {
    ...
    auto* py_node = dynamic_cast<smith::impl::PythonSymNodeImpl*>(src[i].get());
    if (py_node) {
