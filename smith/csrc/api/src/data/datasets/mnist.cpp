#include <smith/data/datasets/mnist.h>
...
Tensor read_images(const std::string& root, bool train) {
  ...
  SMITH_CHECK(images, "Error opening images file at ", path);
  ...
  auto tensor =
      smith::empty({count, 1, kImageRows, kImageColumns}, smith::kByte);
  images.read(reinterpret_cast<char*>(tensor.data_ptr()), tensor.numel());
  return tensor.to(smith::kFloat32).div_(255);
}
...
Tensor read_targets(const std::string& root, bool train) {
  ...
  SMITH_CHECK(targets, "Error opening targets file at ", path);
  ...
  auto tensor = smith::empty(count, smith::kByte);
  targets.read(reinterpret_cast<char*>(tensor.data_ptr()), count);
  return tensor.to(smith::kInt64);
}
} // namespace
