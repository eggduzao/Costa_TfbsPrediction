...
#include <smith/data/samplers/distributed.h>
...
void DistributedRandomSampler::save(serialize::OutputArchive& archive) const {
  archive.write(
      "sample_index_",
      smith::tensor(static_cast<int64_t>(sample_index_)),
      /*is_buffer=*/true);
  archive.write(
      "epoch_",
      smith::tensor(static_cast<int64_t>(epoch_)),
      /*is_buffer=*/true);
}
...
void DistributedRandomSampler::load(serialize::InputArchive& archive) {
  ...
  tensor = smith::empty(1, smith::kInt64);
