...
void LBFGSOptions::serialize(smith::serialize::OutputArchive& archive) const {
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(lr);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(max_iter);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(max_eval);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(tolerance_grad);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(tolerance_change);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(history_size);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(line_search_fn);
}

void LBFGSOptions::serialize(smith::serialize::InputArchive& archive) {
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(double, lr);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(int64_t, max_iter);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_OPTIONAL(int64_t, max_eval);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(double, tolerance_grad);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(double, tolerance_change);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(int64_t, history_size);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_OPTIONAL(std::string, line_search_fn);
}
...
static bool if_container_equal(T lhs, T rhs) {
  ...
  for (const auto i : c10::irange(lhs.size())) {
    if (!smith::equal(lhs.at(i), rhs.at(i)))
      return false;
  }
  return true;
}
...
bool operator==(const LBFGSParamState& lhs, const LBFGSParamState& rhs) {
  ...
  return (lhs.func_evals() == rhs.func_evals()) &&
      ...
      smith::equal_if_defined(lhs.d(), rhs.d()) &&
      smith::equal_if_defined(lhs.H_diag(), rhs.H_diag()) &&
      smith::equal_if_defined(lhs.prev_flat_grad(), rhs.prev_flat_grad()) &&
...
void LBFGSParamState::serialize(
  ...
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(func_evals);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(n_iter);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(t);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(prev_loss);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(d);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(H_diag);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG(prev_flat_grad);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG_DEQUE(old_dirs);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG_DEQUE(old_stps);
  _SMITH_OPTIM_SERIALIZE_SMITH_ARG_DEQUE(ro);
  // Python version only serializes state vars if explicitly defined
  if (al().has_value()) {
    _SMITH_OPTIM_SERIALIZE_SMITH_ARG(al);
  }
}

void LBFGSParamState::serialize(smith::serialize::InputArchive& archive) {
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(int64_t, func_evals);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(int64_t, n_iter);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(double, t);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(double, prev_loss);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(Tensor, d);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(Tensor, H_diag);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(Tensor, prev_flat_grad);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_DEQUE(std::deque<Tensor>, old_dirs);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_DEQUE(std::deque<Tensor>, old_stps);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_DEQUE(std::deque<Tensor>, ro);
  _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_OPTIONAL(std::vector<Tensor>, al);
}
...
std::tuple<double, Tensor> LBFGS::_directional_evaluate(
  ...
  double loss = 0;
  {
    smith::AutoGradMode enable_grad(true);
...
static double _cubic_interpolate(
  ...
  // ported from https://github.com/smith/optim/blob/master/polyinterp.lua
...
Tensor LBFGS::step(LossClosure closure) {
  NoGradGuard no_grad;
  SMITH_CHECK(closure != nullptr, "LBFGS requires a closure function");
  SMITH_INTERNAL_ASSERT(param_groups_.size() == 1);
  auto val = [](const Tensor& t) { return t.item<double>(); };
  ...
  while (n_iter < max_iter) {
    ...
    if (line_search_fn.has_value()) {
      ...
      if (n_iter != max_iter) {
        ...
        opt_cond = val(smith::max(flat_grad.abs())) <= tolerance_grad;
        ls_func_evals = 1;
      }
    }
    // update func eval
...
void LBFGS::load(serialize::InputArchive& archive) {
  ...
  if (archive.try_read("blacksmith_version", blacksmith_version)) {
    ...
    smith::optim::serialize(archive, "old_dirs", old_dirs);
    smith::optim::serialize(archive, "old_stps", old_stps);
