#include <smith/serialize/input-archive.h>
...
void InputArchive::load_from(
    ...
    std::optional<smith::Device> device /*= std::nullopt*/) {
  module_ = smith::jit::load(filename, device);
}

void InputArchive::load_from(
    std::istream& stream,
    std::optional<smith::Device> device /*= std::nullopt*/) {
  module_ = smith::jit::load(stream, device);
}
...
void InputArchive::load_from(
  ...
  module_ = smith::jit::load(std::make_unique<OurAdapter>(data, size), device);
}
...
std::vector<std::string> InputArchive::keys() {
  ...
  for (const smith::jit::NameValue& s :
