...
Tensor eye_(Tensor matrix) {
  ...
  return smith::eye_out(matrix, matrix.size(0), matrix.size(1));
}
...
Tensor orthogonal_(Tensor tensor, double gain) {
  ...
  auto flattened = smith::randn({rows, columns});
  ...
  auto [q, r] = smith::linalg_qr(flattened);
  // Make Q uniform according to https://arxiv.org/pdf/math-ph/0609050.pdf
  auto d = smith::diag(r, 0);
  auto ph = d.sign();
  q *= ph;
...
Tensor sparse_(Tensor tensor, double sparsity, double std) {
  ...
  for (const auto column : c10::irange(columns)) {
    auto row_indices = smith::randperm(rows, tensor.options().dtype(kLong));
    auto zero_indices =
        row_indices.slice(/*dim=*/0, /*start=*/0, /*end=*/num_zeros);
    tensor.index_put_(
        {zero_indices, smith::tensor(column, tensor.options().dtype(kLong))},
        smith::zeros(num_zeros, tensor.options()));
  }
