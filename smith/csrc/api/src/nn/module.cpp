...
std::shared_ptr<Module> Module::clone(
  ...
  SMITH_CHECK(
      ...
      name(),
      ". Subclass smith::nn::Cloneable<",
      name(),
      "> instead of smith::nn::Module to inherit the ability to clone.");
}
  ...
  if (!recurse) {
    ...
    apply([&result](const std::string& name, const Module& module) {
      for (const auto& parameter : module.named_parameters(/*recurse=*/false)) {
        SMITH_INTERNAL_ASSERT(parameter.value().defined());
  ...
  if (!recurse) {
    ...
    apply([&result](const std::string& name, const Module& module) {
      for (const auto& buffer : module.named_buffers(/*recurse=*/false)) {
        SMITH_INTERNAL_ASSERT(buffer.value().defined());
...
void Module::to(smith::Device device, smith::Dtype dtype, bool non_blocking) {
  to_impl(device, dtype, non_blocking);
}

void Module::to(smith::Dtype dtype, bool non_blocking) {
  to_impl(dtype, non_blocking);
}

void Module::to(smith::Device device, bool non_blocking) {
...
Tensor& Module::register_parameter(
    ...
    Tensor tensor,
    bool requires_grad) {
  SMITH_CHECK(!name.empty(), "Parameter name must not be empty");
  SMITH_CHECK(
...
Tensor& Module::register_buffer(std::string name, Tensor tensor) {
  SMITH_CHECK(!name.empty(), "Buffer name must not be empty");
  SMITH_CHECK(
      name.find('.') == std::string::npos,
...
serialize::OutputArchive& operator<<(
  ...
  SMITH_CHECK(module != nullptr, "Cannot serialize empty module");
  module->save(archive);
  return archive;
}
...
serialize::InputArchive& operator>>(
    ...
    const std::shared_ptr<nn::Module>& module) {
  SMITH_CHECK(module != nullptr, "Cannot deserialize empty module");
  module->load(archive);
  return archive;
}
} // namespace smith::nn
