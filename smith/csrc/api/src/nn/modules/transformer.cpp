...
#include <smith/nn/modules/transformer.h>
#include <smith/nn/modules/transformercoder.h>
#include <smith/nn/modules/transformerlayer.h>
...
Tensor TransformerEncoderLayerImpl::forward(
  ...
  if (std::holds_alternative<enumtype::kGELU>(options.activation())) {
    ...
    SMITH_CHECK(false, "activation should be kGELU, kReLU, or a callable");
  }
...
void TransformerImpl::reset_parameters() {
  ...
  for (auto& param : parameters) {
    if (param.dim() > 1) {
      smith::nn::init::xavier_uniform_(param);
    }
  }
}
...
Tensor TransformerImpl::generate_square_subsequent_mask(int64_t sz) {
  ...
  if (std::numeric_limits<float>::is_iec559) {
    return smith::triu(
        smith::full({sz, sz}, -std::numeric_limits<float>::infinity()), 1);
  }
  ...
  // platform
    ...
    return smith::triu(
        smith::full({sz, sz}, std::numeric_limits<float>::lowest()), 1);
  }
}
