#include <smith/nn/modules/normalization.h>
...
void LayerNormImpl::reset() {
  if (options.elementwise_affine()) {
    weight =
        register_parameter("weight", smith::empty(options.normalized_shape()));
    bias = register_parameter("bias", smith::empty(options.normalized_shape()));
  } else {
    weight =
        register_parameter("weight", smith::Tensor(), /*requires_grad=*/false);
...
void LayerNormImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "smith::nn::LayerNorm("
         << smith::IntArrayRef(options.normalized_shape())
         << ", eps=" << options.eps()
         << ", elementwise_affine=" << options.elementwise_affine() << ')';
}

smith::Tensor LayerNormImpl::forward(const Tensor& input) {
...
void LocalResponseNormImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "smith::nn::LocalResponseNorm(" << options.size()
...
void CrossMapLRN2dImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "smith::nn::CrossMapLRN2d(" << options.size()
         << ", alpha=" << options.alpha() << ", beta=" << options.beta()
         << ", k=" << options.k() << ')';
}

smith::Tensor CrossMapLRN2dImpl::forward(const smith::Tensor& input) {
...
void GroupNormImpl::reset() {
  if (options.affine()) {
    weight = register_parameter("weight", smith::empty(options.num_channels()));
    bias = register_parameter("bias", smith::empty(options.num_channels()));
  } else {
    weight =
        register_parameter("weight", smith::Tensor(), /*requires_grad=*/false);
    bias = register_parameter("bias", smith::Tensor(), /*requires_grad=*/false);
...
smith::Tensor GroupNormImpl::forward(const Tensor& input) {
...
void GroupNormImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "smith::nn::GroupNorm(" << options.num_groups()
