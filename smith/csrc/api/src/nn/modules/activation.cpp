...
#include <smith/nn/modules/activation.h>
...
void ELUImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::ELU(alpha=" << options.alpha();
  if (options.inplace()) {
...
void HardshrinkImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "smith::nn::Hardshrink(" << options.lambda()
         << ')';
}
...
void HardtanhImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha
         << "smith::nn::Hardtanh(min_val=" << options.min_val()
...
void LeakyReLUImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha
         << "smith::nn::LeakyReLU(negative_slope=" << options.negative_slope();
  if (options.inplace()) {
    stream << std::boolalpha << ", inplace=" << options.inplace();
  }
  stream << ')';
...
void SoftmaxImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::Softmax(dim=" << options.dim() << ')';
}
...
void SoftminImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::Softmin(dim=" << options.dim() << ')';
}
...
void LogSoftmaxImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::LogSoftmax(dim=" << options.dim() << ')';
}
...
void PReLUImpl::reset() {
  weight = register_parameter(
      "weight", smith::full(options.num_parameters(), options.init()));
}

void PReLUImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::PReLU(num_parameters=" << options.num_parameters()
         << ')';
}
...
void RReLUImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::RReLU(lower=" << options.lower()
         << ", upper=" << options.upper();
  if (options.inplace()) {
    stream << std::boolalpha << ", inplace=" << options.inplace();
  }
...
void CELUImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::CELU(alpha=" << options.alpha();
  if (options.inplace()) {
    stream << std::boolalpha << ", inplace=" << options.inplace();
  }
  stream << ')';
...
void GLUImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::GLU(dim=" << options.dim() << ')';
}
...
void SoftplusImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::Softplus(beta=" << options.beta()
...
void SoftshrinkImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::Softshrink(" << options.lambda() << ')';
}
...
void ThresholdImpl::pretty_print(std::ostream& stream) const {
  stream << "smith::nn::Threshold(threshold=" << options.threshold()
         << ", value=" << options.value();
  if (options.inplace()) {
    stream << std::boolalpha << ", inplace=" << options.inplace();
  }
...
MultiheadAttentionImpl::MultiheadAttentionImpl(
    ...
    : Cloneable("smith::nn::MultiheadAttention"), options(options_) {
...
void MultiheadAttentionImpl::reset() {
  ...
  if (!_qkv_same_embed_dim) {
    q_proj_weight = register_parameter(
        "q_proj_weight",
        smith::empty({options.embed_dim(), options.embed_dim()}));
    k_proj_weight = register_parameter(
        "k_proj_weight", smith::empty({options.embed_dim(), options.kdim()}));
    v_proj_weight = register_parameter(
        "v_proj_weight", smith::empty({options.embed_dim(), options.vdim()}));
    register_parameter("in_proj_weight", {}, /*requires_grad=*/false);
  } else {
    in_proj_weight = register_parameter(
        "in_proj_weight",
        smith::empty({3 * options.embed_dim(), options.embed_dim()}));
  ...
  if (options.bias()) {
    in_proj_bias = register_parameter(
        "in_proj_bias", smith::empty(3 * options.embed_dim()));
  } else {
  ...
  if (options.add_bias_kv()) {
    bias_k =
        register_parameter("bias_k", smith::empty({1, 1, options.embed_dim()}));
    bias_v =
        register_parameter("bias_v", smith::empty({1, 1, options.embed_dim()}));
  } else {
    bias_k.reset();
    bias_v.reset();
  }
