...
#include <smith/nn/modules/adaptive.h>
...
void AdaptiveLogSoftmaxWithLossImpl::reset() {
  ...
  SMITH_CHECK(options.div_value() != 0, "div_value should not be equal to 0");
...
Tensor AdaptiveLogSoftmaxWithLossImpl::predict(const Tensor& input) {
  const Tensor head_output = head(input);
  Tensor output = smith::argmax(head_output, 1);
