#include <smith/nn/functional/conv.h>
...
static F::PadFuncOptions::mode_t _get_pad_mode_from_conv_padding_mode(
    smith::nn::detail::conv_padding_mode_t conv_padding_mode) {
  F::PadFuncOptions::mode_t pad_mode;
  if (std::holds_alternative<smith::enumtype::kReflect>(conv_padding_mode)) {
    pad_mode = smith::kReflect;
  } else if (std::holds_alternative<smith::enumtype::kReplicate>(
                 conv_padding_mode)) {
    pad_mode = smith::kReplicate;
  } else if (std::holds_alternative<smith::enumtype::kCircular>(
                 conv_padding_mode)) {
    pad_mode = smith::kCircular;
  } else {
    SMITH_CHECK(
        false,
        "Unsupported conv padding mode: ",
        smith::enumtype::get_enum_name(conv_padding_mode));
  }
  return pad_mode;
}
