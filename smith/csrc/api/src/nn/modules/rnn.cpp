#include <smith/nn/modules/rnn.h>
...
using namespace smith::nn::utils::rnn;
...
static CuDNNMode get_cudnn_mode_for_rnn(
  ...
  if (std::holds_alternative<enumtype::kRNN_RELU>(mode)) {
    ...
    SMITH_CHECK(false, "Unknown mode: ", smith::enumtype::get_enum_name(mode));
  }
}
...
void RNNImplBase<Derived>::reset() {
  ...
  if (std::holds_alternative<enumtype::kLSTM>(options_base.mode())) {
    ...
    SMITH_CHECK(
        ...
        "Unrecognized RNN mode: " +
            smith::enumtype::get_enum_name(options_base.mode()));
  }
  ...
  for (const auto layer : c10::irange(options_base.num_layers())) {
    for (const auto direction : c10::irange(num_directions)) {
      ...
      auto w_ih = smith::empty({gate_size, layer_input_size});
      auto w_hh = smith::empty({gate_size, real_hidden_size});
      auto b_ih = smith::empty({gate_size});
      // Second bias vector included for CuDNN compatibility. Only one
      // bias vector is needed in standard definition.
      auto b_hh = smith::empty({gate_size});
...
void RNNImplBase<Derived>::flatten_parameters() {
  ...
  {
    smith::DeviceGuard device_guard(first_fw.device());
...
template <typename Derived>
void RNNImplBase<Derived>::to(smith::Dtype dtype, bool non_blocking) {
...
template <typename Derived>
void RNNImplBase<Derived>::to(smith::Device device, bool non_blocking) {
...
compute_rnn_options_base_mode(RNNOptions::nonlinearity_t nonlinearity) {
  if (std::holds_alternative<enumtype::kTanh>(nonlinearity)) {
    return smith::kRNN_TANH;
  } else if (std::holds_alternative<enumtype::kReLU>(nonlinearity)) {
    return smith::kRNN_RELU;
  } else {
    SMITH_CHECK(
        false,
        "Unknown nonlinearity ",
        smith::enumtype::get_enum_name(nonlinearity));
  }
}
  ...
  if (!hx.defined()) {
    ...
    hx = smith::zeros(
        {options_base.num_layers() * num_directions,
         max_batch_size,
         options_base.hidden_size()},
        smith::dtype(input.dtype()).device(input.device()));
  } else {
...
std::tuple<Tensor, Tensor> RNNImpl::forward(const Tensor& input, Tensor hx) {
  auto batch_sizes = smith::Tensor();
  auto max_batch_size =
      options_base.batch_first() ? input.size(0) : input.size(1);
  auto sorted_indices = smith::Tensor();
  auto unsorted_indices = smith::Tensor();
...
std::tuple<Tensor, std::tuple<Tensor, Tensor>> LSTMImpl::forward_helper(
  ...
  if (!hx_opt.has_value()) {
    ...
    auto h_zeros = smith::zeros(
        {options.num_layers() * num_directions,
         max_batch_size,
         real_hidden_size},
        smith::dtype(input.dtype()).device(input.device()));
    auto c_zeros = smith::zeros(
        {options.num_layers() * num_directions,
         max_batch_size,
         options.hidden_size()},
        smith::dtype(input.dtype()).device(input.device()));
...
void RNNCellImplBase<Derived>::reset() {
  ...
  if (options_base.bias()) {
    bias_ih = this->register_parameter(
        "bias_ih",
        smith::empty({options_base.num_chunks() * options_base.hidden_size()}));
...
Tensor GRUCellImpl::forward(const Tensor& input, const Tensor& hx) {
  ...
  ret = smith::gru_cell(r_input, r_hx, weight_ih, weight_hh, bias_ih, bias_hh);
