...
#include <smith/data/transforms/base.h>
...
struct Normalize : public TensorTransform<Target> {
  ...
  Normalize(ArrayRef<double> mean, ArrayRef<double> stddev)
      : mean(smith::tensor(mean, smith::kFloat32)
                 .unsqueeze(/*dim=*/1)
                 .unsqueeze(/*dim=*/2)),
        stddev(smith::tensor(stddev, smith::kFloat32)
                   .unsqueeze(/*dim=*/1)
                   .unsqueeze(/*dim=*/2)) {}

  smith::Tensor operator()(Tensor input) override {
    return input.sub(mean).div(stddev);
  }

  smith::Tensor mean, stddev;
};
} // namespace smith::data::transforms
