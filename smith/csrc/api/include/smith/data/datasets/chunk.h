...
#include <smith/arg.h>
#include <smith/data/datasets/stateful.h>
  ...
  void add_chunk_data(UnwrappedBatchType data) {
    ...
    auto fill_batch = [&](size_t example_count, UnwrappedBatchType& batch) {
      ...
      for (size_t i : indices) {
        SMITH_CHECK(i < data_size, "Index out of range");
...
struct ChunkDatasetOptions {
  ...
  SMITH_ARG(size_t, preloader_count);

  /// The size of each batch.
  SMITH_ARG(size_t, batch_size);

  /// The capacity of the queue for batch caching.
  SMITH_ARG(size_t, cache_size) = 2048;
  ...
  SMITH_ARG(size_t, cross_chunk_shuffle_count) = 1;
};
