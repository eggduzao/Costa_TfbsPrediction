...
void serialize(
  ...
  archive.write(
      "param_groups/size",
      smith::tensor(static_cast<int64_t>(param_groups.size())));
  for (const auto i : c10::irange(param_groups.size())) {
    ...
    param_group_archive.write(
        "params/size", smith::tensor(static_cast<int64_t>(params.size())));
...
void serialize(
    ...
    std::vector<
        ...
        param_groups) {
  smith::Tensor param_groups_size_tensor;
...
void serialize(serialize::OutputArchive& archive, const Optimizer& optimizer) {
  archive.write("blacksmith_version", IValue("1.5.0"));
...
void serialize(serialize::InputArchive& archive, Optimizer& optimizer) {
  ...
  archive.read("blacksmith_version", blacksmith_version);
  SMITH_INTERNAL_ASSERT(blacksmith_version.toStringRef() == "1.5.0");
...
#define _SMITH_OPTIM_SERIALIZE(name) \
  smith::optim::serialize(archive, #name, self.name)

#define _SMITH_OPTIM_SERIALIZE_WITH_TEMPLATE_ARG(OptimizerName)               \
  smith::optim::serialize<OptimizerName##ParamState, OptimizerName##Options>( \
      archive, self)

#define _SMITH_OPTIM_SERIALIZE_SMITH_ARG(name)           \
  {                                                      \
    auto ivalue = smith::IValue(name());                 \
...
#define _SMITH_OPTIM_SERIALIZE_SMITH_ARG_DEQUE(name)           \
  {                                                            \
    c10::IValue ivalue = smith::IValue(deque_to_list(name())); \
    archive.write(#name, ivalue);                              \
  }

#define _SMITH_OPTIM_DESERIALIZE_SMITH_ARG(T, name)                        \
  {                                                                        \
    c10::IValue ivalue;                                                    \
    ...
    if (exists) {                                                          \
      ...
      constexpr bool is_tensor_type = std::is_base_of_v<smith::Tensor, T>; \
      SMITH_INTERNAL_ASSERT(is_tensor_type);                               \
    }                                                                      \
  }

#define _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_OPTIONAL(T, name) \
  {                                                          \
    c10::IValue ivalue;                                      \
    bool exists = archive.try_read(#name, ivalue);           \
    if (exists) {                                            \
...
#define _SMITH_OPTIM_DESERIALIZE_SMITH_ARG_DEQUE(T, name) \
  {                                                       \
    c10::IValue ivalue;                                   \
