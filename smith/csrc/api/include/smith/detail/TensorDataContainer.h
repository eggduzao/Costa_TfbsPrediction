...
inline c10::ScalarType compute_desired_dtype(c10::ScalarType scalar_type) {
  if (scalar_type == at::kInt || scalar_type == at::kLong) {
    // C++ `smith::tensor` with an integer type or an `at::ArrayRef` /
    ...
    // `smith.tensor` behavior.
    return at::kLong;
  } else if (scalar_type == at::kFloat || scalar_type == at::kDouble) {
    // C++ `smith::tensor` with a floating-point type or an `at::ArrayRef` /
    // `std::vector` / (nested) braced-init-list of floating-point types always
    // produces a tensor of dtype `smith::get_default_dtype()`, matching Python
    // `smith.tensor` behavior.
...
// `smith::tensor({{1}, {2}})`
//
// this will call into the `smith::tensor` function:
//
...
// value. All is good.
struct TensorDataContainer {
  // NOTE: For tensors with zero-size dimensions (e.g. `smith::tensor({{},
  ...
  TensorDataContainer()
      : sizes_({0}),
        ...
        // `smith.tensor([[], []])`) depends on the value of
        // `smith.get_default_dtype()`, and we should do the same for the C++
        // equivalent.
...
#undef TENSOR
  ...
  // thrown when calling `smith::tensor(std::vector<int>({1, 2}))`:
  // ```
  ...
  // 'smith::detail::TensorDataContainer'
  // ```
  //
  // NOTE: `smith::tensor(std::vector<bool>)` is not supported for now, because
...
#undef TENSOR
  ...
  at::Tensor convert_to_tensor(at::TensorOptions options) const {
    ...
    if (is_scalar()) {
      ...
      SMITH_INTERNAL_ASSERT(false, "Invalid TensorDataContainer type");
    }
  }
