...
// NOTE [ Exposing declarations in `at::` to `smith::` ]
//
// The following line `using namespace at;` is responsible for exposing all
// declarations in `at::` namespace to `smith::` namespace.
//
...
// ```
//
// This means that if both `at::` and `smith::` namespaces have a function with
// the same signature (e.g. both `at::func()` and `smith::func()` exist), after
// `namespace smith { using namespace at; }`, when we call `smith::func()`, the
// `func()` function defined in `smith::` namespace will always be called, and
