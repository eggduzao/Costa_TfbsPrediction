...
/// ```
struct SMITH_API FlattenOptions {
  /// first dim to flatten
  SMITH_ARG(int64_t, start_dim) = 1;
  /// last dim to flatten
  SMITH_ARG(int64_t, end_dim) = -1;
};
...
/// ```
struct SMITH_API UnflattenOptions {
  ...
  SMITH_ARG(std::string, dimname);
  /// new shape of unflattened dim
  SMITH_ARG(std::vector<int64_t>, sizes);
  /// new shape of unflattened dim with names, for use with named tensors
  SMITH_ARG(namedshape_t, namedshape);
};
