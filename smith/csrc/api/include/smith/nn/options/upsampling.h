...
/// ```
/// Upsample
/// model(UpsampleOptions().scale_factor(std::vector<double>({3})).mode(smith::kLinear).align_corners(false));
/// ```
struct SMITH_API UpsampleOptions {
  /// output spatial sizes.
  SMITH_ARG(std::optional<std::vector<int64_t>>, size) = std::nullopt;

  /// multiplier for spatial size.
  SMITH_ARG(std::optional<std::vector<double>>, scale_factor) = std::nullopt;
  ...
  typedef std::variant<
      ...
      enumtype::kTrilinear>
      mode_t;
  SMITH_ARG(mode_t, mode) = smith::kNearest;
...
/// Options for `smith::nn::functional::interpolate`.
///
/// Example:
/// ```
/// namespace F = smith::nn::functional;
/// F::interpolate(input,
/// F::InterpolateFuncOptions().size(std::vector<int64_t>({4})).mode(smith::kNearest));
/// ```
struct SMITH_API InterpolateFuncOptions {
  typedef std::variant<
      enumtype::kNearest,
      enumtype::kLinear,
      enumtype::kBilinear,
  ...
  SMITH_ARG(std::optional<bool>, recompute_scale_factor) = std::nullopt;
  ...
  SMITH_ARG(bool, antialias) = false;
};
