...
struct ConvNdOptions {
  ...
  /// numbers.
  /// This parameter __can__ be changed after construction.
  SMITH_ARG(ExpandingArray<D>, stride) = 1;

  /// The padding to add to the input volumes.
  /// For a `D`-dim convolution, must be a single number or a list of `D`
  /// numbers.
  /// This parameter __can__ be changed after construction.
  SMITH_ARG(padding_t, padding) = 0;
 ...
 public:
  ...
  /// deconvolutions).
  /// Changing this parameter after construction __has no effect__.
  SMITH_ARG(bool, transposed) = false;

  /// For transpose convolutions, the padding to add to output volumes.
  /// For a `D`-dim convolution, must be a single number or a list of `D`
  /// numbers.
  /// This parameter __can__ be changed after construction.
  SMITH_ARG(ExpandingArray<D>, output_padding) = 0;

  /// The number of convolution groups.
  /// This parameter __can__ be changed after construction.
  SMITH_ARG(int64_t, groups) = 1;

  /// Whether to add a bias after individual applications of the kernel.
  /// Changing this parameter after construction __has no effect__.
  SMITH_ARG(bool, bias) = true;

  /// Accepted values `smith::kZeros`, `smith::kReflect`, `smith::kReplicate` or
  /// `smith::kCircular`. Default: `smith::kZeros`
  SMITH_ARG(conv_padding_mode_t, padding_mode) = smith::kZeros;
};
...
struct ConvFuncOptions {
  using padding_t = smith::nn::detail::conv_padding_t<D>;

  /// optional bias of shape `(out_channels)`. Default: ``None``
  SMITH_ARG(smith::Tensor, bias);
...
/// `ConvFuncOptions` specialized for `smith::nn::functional::conv1d`.
///
/// Example:
/// ```
...
/// `ConvFuncOptions` specialized for `smith::nn::functional::conv2d`.
///
/// Example:
/// ```
/// namespace F = smith::nn::functional;
...
/// `ConvFuncOptions` specialized for `smith::nn::functional::conv3d`.
///
/// Example:
/// ```
/// namespace F = smith::nn::functional;
