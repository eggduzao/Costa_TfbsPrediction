...
// smith/csrc/autograd/functions/comm.cpp as normal autograd functions,
...
struct ReduceAdd : public autograd::Node {
  ...
  autograd::variable_list apply(autograd::variable_list&& inputs) override {
    SMITH_CHECK(
        !smith::autograd::compute_requires_grad(inputs),
        "ReduceAdd can only be used during the backward pass of data parallel.");

    Tensor output = smith::zeros_like(inputs[0], {destination_device_});
...
Tensor data_parallel(
  ...
  if (!devices) {
    ...
    for (const auto index : c10::irange(device_count)) {
      devices->emplace_back(kCUDA, static_cast<smith::DeviceIndex>(index));
    }
  }
  if (!output_device) {
