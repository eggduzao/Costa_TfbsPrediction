...
inline std::vector<int64_t> _interp_output_size(
  ...
  if (size == std::nullopt && scale_factor == std::nullopt) {
    SMITH_CHECK(false, "either size or scale_factor should be defined");
  }
  if (size != std::nullopt && scale_factor != std::nullopt) {
    SMITH_CHECK(false, "only one of size or scale_factor should be defined");
  }
  if (scale_factor != std::nullopt) {
    if (static_cast<int64_t>(scale_factor.value().size()) != dim) {
      SMITH_CHECK(
          ...
          smith::ArrayRef<double>(*scale_factor));
    }
  }
  if (size != std::nullopt) {
    return *size;
  ...
  SMITH_INTERNAL_ASSERT(scale_factor != std::nullopt);
  ...
  if (recompute_scale_factor == std::nullopt) {
    ...
    if (is_float_scale_factor) {
      SMITH_WARN(
          "The default behavior for interpolate/upsample with float scale_facsmithanged "
...
inline Tensor interpolate(
  ...
  if (input.dim() == 3 && std::get_if<enumtype::kNearest>(&mode)) {
    return smith::upsample_nearest1d(
        input,
    ...
    return smith::upsample_nearest3d(
        input,
    ...
    return smith::upsample_linear1d(
        input,
        _interp_output_size(1, std::move(closed_over_args)),
        *align_corners,
        scale_factor_list.at(0));
  } else if (input.dim() == 3 && std::get_if<enumtype::kBilinear>(&mode)) {
    SMITH_CHECK(false, "Got 3D input, but bilinear mode needs 4D input");
  } else if (input.dim() == 3 && std::get_if<enumtype::kTrilinear>(&mode)) {
    SMITH_CHECK(false, "Got 3D input, but trilinear mode needs 5D input");
  } else if (input.dim() == 4 && std::get_if<enumtype::kLinear>(&mode)) {
    SMITH_CHECK(false, "Got 4D input, but linear mode needs 3D input");
  } else if (input.dim() == 4 && std::get_if<enumtype::kBilinear>(&mode)) {
    SMITH_CHECK(
        align_corners != std::nullopt, "align_corners should be specified.");
    if (antialias) {
      return smith::_upsample_bilinear2d_aa(
          input,
          _interp_output_size(2, std::move(closed_over_args)),
          *align_corners,
          scale_factor_list.at(0),
    ...
    return smith::upsample_bilinear2d(
        input,
        _interp_output_size(2, std::move(closed_over_args)),
        *align_corners,
        scale_factor_list.at(0),
    ...
    SMITH_CHECK(false, "Got 4D input, but trilinear mode needs 5D input");
  } else if (input.dim() == 5 && std::get_if<enumtype::kLinear>(&mode)) {
    SMITH_CHECK(false, "Got 5D input, but linear mode needs 3D input");
  } else if (input.dim() == 5 && std::get_if<enumtype::kBilinear>(&mode)) {
    SMITH_CHECK(false, "Got 5D input, but bilinear mode needs 4D input");
  } else if (input.dim() == 5 && std::get_if<enumtype::kTrilinear>(&mode)) {
    SMITH_CHECK(
        align_corners != std::nullopt, "align_corners should be specified.");
    return smith::upsample_trilinear3d(
        input,
        _interp_output_size(3, std::move(closed_over_args)),
        *align_corners,
        scale_factor_list.at(0),
    ...
    SMITH_CHECK(
        align_corners != std::nullopt, "align_corners should be specified.");
    if (antialias) {
      return smith::_upsample_bicubic2d_aa(
          input,
          _interp_output_size(2, std::move(closed_over_args)),
          *align_corners,
          scale_factor_list.at(0),
    ...
    return smith::upsample_bicubic2d(
        input,
        _interp_output_size(2, std::move(closed_over_args)),
        *align_corners,
        scale_factor_list.at(0),
...
/// See
/// https://blacksmith.org/docs/main/nn.functional.html#smith.nn.functional.interpolate
/// about the exact behavior of this functional.
///
/// See the documentation for `smith::nn::functional::InterpolateFuncOptions`
...
/// F::interpolate(input,
/// F::InterpolateFuncOptions().size({4}).mode(smith::kNearest));
/// ```
inline Tensor interpolate(
    const Tensor& input,
