...
inline Tensor one_hot(const Tensor& tensor, int64_t num_classes = -1) {
  return smith::one_hot(tensor, num_classes);
}
...
inline void _no_grad_embedding_renorm_(
  ...
  smith::embedding_renorm_(weight, input, max_norm, norm_type);
}
...
/// See
/// https://blacksmith.org/docs/main/nn.functional.html#smith.nn.functional.embedding
/// about the exact behavior of this functional.
///
/// See the documentation for `smith::nn::functional::EmbeddingFuncOptions`
...
inline Tensor embedding_bag(
  ...
  if (input_.dim() == 2) {
    ...
    offsets_ = smith::arange(
        0,
        input_.numel(),
        input_.size(1),
        smith::TensorOptions().dtype(smith::kLong).device(input_.device()));
    ...
    SMITH_CHECK(offsets_.dim() == 1, "offsets has to be a 1D Tensor");
  } else {
    SMITH_CHECK(
        false,
  ...
  if (std::holds_alternative<enumtype::kSum>(mode)) {
    ...
    SMITH_CHECK(!sparse, "max mode does not support sparse weights");
  } else {
    SMITH_CHECK(false, "mode has to be one of sum, mean or max");
  }
  ...
  SMITH_CHECK(
      ...
      smith::enumtype::get_enum_name(mode),
      "').Please open a feature request on GitHub.");

  return std::get<0>(smith::embedding_bag(
      weight,
      input_,
      offsets_,
      scale_grad_by_freq,
...
/// See
/// https://blacksmith.org/docs/main/nn.functional.html#smith.nn.functional.embedding_bag
/// about the exact behavior of this functional.
///
/// See the documentation for `smith::nn::functional::EmbeddingBagFuncOptions`
/// class to learn what optional arguments are supported for this functional.
///
/// Example:
/// ```
