...
#include <smith/csrc/api/include/smith/nn/pimpl-inl.h>
} // namespace detail
...
class ModuleHolder : smith::detail::ModuleHolderIndicator {
 protected:
 ...
 public:
  ...
  template <
      ...
      typename = std::enable_if_t<
          !(smith::detail::is_module_holder_of<Head, ContainedType>::value &&
            (sizeof...(Tail) == 0))>>
  ...
  const std::shared_ptr<Contained>& ptr() const {
    SMITH_CHECK(!is_empty(), "Accessing empty ModuleHolder");
    return impl_;
  }
  ...
  auto operator()(Args&&... args)
      -> smith::detail::return_type_of_forward_t<Contained, Args...> {
...
// using declarations.
#ifdef __CUDACC__
#define SMITH_UNUSED_EXCEPT_CUDA
#else
#define SMITH_UNUSED_EXCEPT_CUDA [[maybe_unused]]
#endif
...
/// method of `ImplType`.
#define SMITH_MODULE_IMPL(Name, ImplType)                              \
  class Name : public smith::nn::ModuleHolder<ImplType> { /* NOLINT */ \
   public:                                                             \
    using smith::nn::ModuleHolder<ImplType>::ModuleHolder;             \
    using Impl SMITH_UNUSED_EXCEPT_CUDA = ImplType;                    \
  }

/// Like `SMITH_MODULE_IMPL`, but defaults the `ImplType` name to `<Name>Impl`.
#define SMITH_MODULE(Name) SMITH_MODULE_IMPL(Name, Name##Impl)
