...
inline Tensor invert_permutation(const Tensor& permutation) {
  ...
  Tensor output =
      smith::empty_like(permutation, smith::MemoryFormat::Contiguous);
  output.scatter_(
      0,
      permutation,
      smith::arange(0, permutation.numel(), permutation.device()));
  return output;
}
...
///     `sorted_indices` and `unsorted_indices` must be ``smith::kInt64``
///     tensors on the same device as `data`.
///
///     However, `batch_sizes` should always be a CPU ``smith::kInt64`` tensor.
///
///     This invariant is maintained throughout `PackedSequence` class,
///     and all functions that construct a `PackedSequence` in libsmith
...
class PackedSequence {
 public:
  ...
  PackedSequence to(TensorOptions options) const {
    ...
    //
    // If the ``data_`` Tensor already has the correct `smith::Dtype`
    // and `smith::Device`, then ``self`` is returned.
...
inline PackedSequence pack_padded_sequence(
  ...
  if (enforce_sorted) {
    ...
    std::tie(lengths, sorted_indices) =
        smith::sort(lengths, /*dim=*/-1, /*descending=*/true);
  ...
  auto [data, batch_sizes] =
      smith::_pack_padded_sequence(input, lengths, batch_first);
  return PackedSequence(
...
inline std::tuple<Tensor, Tensor> pad_packed_sequence(
  ...
  auto [padded_output, lengths] = smith::_pad_packed_sequence(
      sequence.data(),
...
///
/// Arguments:
///     sequences (smith::ArrayRef<Tensor>): list of variable length sequences.
...
///
/// Arguments:
///     sequences (smith::ArrayRef<Tensor>): A list of sequences of decreasing
...
inline PackedSequence pack_sequence(
    ArrayRef<Tensor> sequences,
    bool enforce_sorted = true) {
  Tensor lengths = smith::empty({(int64_t)sequences.size()}, kInt64);
