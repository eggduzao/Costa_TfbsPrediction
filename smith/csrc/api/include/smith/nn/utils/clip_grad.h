...
// See
// https://blacksmith.org/docs/stable/nn.html?highlight=clip_grad_norm#smith.nn.utils.clip_grad_norm_
...
inline double clip_grad_norm_(
  ...
  if (norm_type == std::numeric_limits<double>::infinity()) {
    ...
    total_norm_tensor =
        (norms.size() == 1) ? norms[0] : smith::max(smith::stack(norms));
  } else if (norm_type == 0) {
    total_norm_tensor =
        smith::full({}, static_cast<double>(params_with_grad.size()));
  } else {
    std::vector<Tensor> norms;
    norms.reserve(params_with_grad.size());
    ...
    total_norm_tensor =
        (norms.size() == 1) ? norms[0] : smith::stack(norms).norm(norm_type);
  }
  ...
  auto clip_coef_clamped =
      smith::clamp(clip_coef, std::nullopt /* min */, 1.0 /* max */);
...
// See https://blacksmith.org/docs/stable/nn.html#clip-grad-value
// for more details about this module.
