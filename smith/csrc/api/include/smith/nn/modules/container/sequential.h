...
#include <smith/nn/modules/container/named_any.h>
...
/// .. code-block:: cpp
///
///   smith::nn::Sequential seq(
///     smith::nn::Linear(3, 4),
///     smith::nn::BatchNorm1d(4),
///     smith::nn::Dropout(0.5)
///   );
///
///   auto output = seq->forward(smith::ones(3));
///
/// \endrst
///
...
/// `.to(smith::kCUDA)` on a `Sequential` will move each module in the list to
...
class SequentialImpl : public Cloneable<SequentialImpl> {
 public:
  ...
  explicit SequentialImpl(
      smith::OrderedDict<std::string, AnyModule>&& ordered_dict) {
  ...
  /// .. code-block:: cpp
  ///
  ///   smith::Tensor tensor = sequential1->forward(inputs);
  ...
  ReturnType forward(InputTypes&&... inputs) {
    SMITH_CHECK(!is_empty(), "Cannot call forward() on an empty Sequential");
  ...
  template <typename M, typename = smith::detail::enable_if_module_t<M>>
  ...
  T& at(size_t index) {
    static_assert(
        smith::detail::is_module<T>::value,
        "Can only call Sequential::at with an nn::Module type");
    SMITH_CHECK(index < size(), "Index out of range");
...
class Sequential : public smith::nn::ModuleHolder<SequentialImpl> {
 public:
  using smith::nn::ModuleHolder<SequentialImpl>::ModuleHolder;
