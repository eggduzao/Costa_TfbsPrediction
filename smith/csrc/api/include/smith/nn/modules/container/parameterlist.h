...
class ParameterListImpl : public Cloneable<ParameterListImpl> {
 public:
  using Iterator =
      std::vector<OrderedDict<std::string, smith::Tensor>::Item>::iterator;
  using ConstIterator = std::vector<
      OrderedDict<std::string, smith::Tensor>::Item>::const_iterator;
  ...
  void append(smith::Tensor&& param) {
  ...
  /// push the a given parameter at the end of the list
  void append(const smith::Tensor& param) {
    bool requires_grad = param.requires_grad();
    register_parameter(
  ...
  void append(const OrderedDict<std::string, smith::Tensor>::Item& pair) {
    register_parameter(
  ...
  at::Tensor& at(size_t idx) {
    SMITH_CHECK(idx < size(), "Index out of range");
