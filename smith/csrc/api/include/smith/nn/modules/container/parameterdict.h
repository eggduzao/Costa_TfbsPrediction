...
class ParameterDictImpl : public Cloneable<ParameterDictImpl> {
 public:
  ...
  explicit ParameterDictImpl(
      const smith::OrderedDict<std::string, smith::Tensor>& params) {
    parameters_ = params;
  }
  ...
  Tensor pop(const std::string& key) {
    smith::Tensor v = parameters_[key];
  ...
  ::std::vector<smith::Tensor> values() const {
