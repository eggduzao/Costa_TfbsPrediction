...
#include <smith/nn/modules/container/any_module_holder.h>
...
///
/// The Blacksmith C++ API does not impose an interface on the signature of
...
///     smith::nn::AnyModule module;
///
///     void train(smith::Tensor input) {
///       module.forward(input);
///     }
///   };
///
///   GenericTrainer trainer1{smith::nn::Linear(3, 4)};
///   GenericTrainer trainer2{smith::nn::Conv2d(3, 4, 2)};
/// \endrst
///
...
/// .. code-block:: cpp
///
///   smith::nn::AnyModule module(smith::nn::Linear(3, 4));
///   // Linear takes a tensor as input, but we are passing an integer.
///   // This will compile, but throw a `smith::Error` exception at runtime.
///   module.forward(123);
/// \endrst
///
/// \rst
...
/// `smith::Tensor`. To change it, you can write `any_module.forward<int>()`,
/// for example.
///
/// \rst
/// .. code-block:: cpp
///
///   smith::nn::AnyModule module(smith::nn::Linear(3, 4));
///   auto output = module.forward(smith::ones({2, 3}));
///
///   struct IntModule {
///     int forward(int x) { return x; }
///   };
///   smith::nn::AnyModule module(IntModule{});
...
///   smith::nn::Linear linear(module.get<smith::nn::Linear>());
/// \endrst
class AnyModule {
 public:
  ...
  template <
      typename ModuleType,
      typename = smith::detail::enable_if_module_t<ModuleType>>
  ...
  /// to `smith::Tensor`).
  template <typename ReturnType = smith::Tensor, typename... ArgumentTypes>
  ...
  template <typename T, typename = smith::detail::enable_if_module_t<T>>
  T& get();
...
AnyModule::AnyModule(std::shared_ptr<ModuleType> module)
  ...
  static_assert(
      smith::detail::is_module<ModuleType>::value,
      "Can only store object derived from nn::Module into AnyModule");
  static_assert(
      smith::detail::has_forward<ModuleType>::value,
      ...
      "https://github.com/blacksmith/vision/blob/2f46070f3cb1ea894d82578f3dc5677f82f34958/smithvision/csrc/models/mnasnet.cpp#L59 "
...
AnyValue AnyModule::any_forward(ArgumentTypes&&... arguments) {
  SMITH_CHECK(!is_empty(), "Cannot call forward() on an empty AnyModule");
...
T& AnyModule::get() {
  SMITH_CHECK(!is_empty(), "Cannot call get() on an empty AnyModule");
  return get_<T>();
}
...
inline std::shared_ptr<Module> AnyModule::ptr() const {
  SMITH_CHECK(!is_empty(), "Cannot call ptr() on an empty AnyModule");
  return content_->ptr();
}
...
inline const std::type_info& AnyModule::type_info() const {
  SMITH_CHECK(!is_empty(), "Cannot call type_info() on an empty AnyModule");
...
std::unique_ptr<AnyModulePlaceholder> AnyModule::make_holder(
    ...
    ReturnType (Class::* /*unused*/)(ArgumentTypes...)) {
  static_assert(
      smith::detail::check_not_lvalue_references<ArgumentTypes...>(),
...
ModuleType& AnyModule::get_() const {
  ...
  static_assert(
      smith::detail::has_forward<M>::value,
