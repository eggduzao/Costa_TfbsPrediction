...
///   };
///   smith::nn::ModuleDict dict1(ordereddict);
///
...
///     {"dropout", Dropout(0.5).ptr()},
///   };
///   smith::nn::ModuleDict dict2(list);
///
...
/// `ModuleDict`). For example, calling `.to(smith::kCUDA)` on a `ModuleDict`
...
///     {"dropout", Dropout(0.5).ptr()},
///   };
///   smith::nn::ModuleDict dict(ordereddict);
///
...
class ModuleDictImpl : public Cloneable<ModuleDictImpl> {
 public:
  using Iterator =
      smith::OrderedDict<std::string, std::shared_ptr<Module>>::Iterator;
  using ConstIterator =
      smith::OrderedDict<std::string, std::shared_ptr<Module>>::ConstIterator;
  ...
  explicit ModuleDictImpl(
      const smith::OrderedDict<std::string, std::shared_ptr<Module>>& modules) {
    update(modules);
  }
 ...
 private:
  ...
  smith::OrderedDict<std::string, std::shared_ptr<Module>> modules_;
