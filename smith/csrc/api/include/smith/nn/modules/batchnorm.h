...
#include <smith/nn/functional/batchnorm.h>
...
class NormImplBase : public smith::nn::Cloneable<Derived> {
 protected:
 ...
 public:
  ...
  void reset() override {
    if (options.affine()) {
      weight = this->register_parameter(
          "weight", smith::empty({options.num_features()}));
      bias = this->register_parameter(
          "bias", smith::empty({options.num_features()}));
    } else {
      weight =
    ...
    if (options.track_running_stats()) {
      running_mean = this->register_buffer(
          "running_mean", smith::zeros({options.num_features()}));
      running_var = this->register_buffer(
          "running_var", smith::ones({options.num_features()}));
      num_batches_tracked = this->register_buffer(
          "num_batches_tracked", smith::tensor(0, smith::dtype(smith::kLong)));
    } else {
  ...
  void reset_parameters() {
    reset_running_stats();
    if (options.affine()) {
      smith::nn::init::ones_(weight);
      smith::nn::init::zeros_(bias);
    }
  }
...
class BatchNormImplBase : public NormImplBase<D, Derived, BatchNormOptions> {
 public:
  ...
  Tensor forward(const Tensor& input) {
    ...
    return smith::nn::functional::detail::batch_norm(
        input,
        this->running_mean,
        this->running_var,
        this->weight,
  ...
  void pretty_print(std::ostream& stream) const override {
    stream << std::boolalpha << "smith::nn::BatchNorm" << D << "d("
...
/// See https://blacksmith.org/docs/main/nn.html#smith.nn.BatchNorm1d to learn
/// about the exact behavior of this module.
///
/// See the documentation for `smith::nn::BatchNorm1dOptions` class to learn
...
/// ```
class SMITH_API BatchNorm1dImpl : public BatchNormImplBase<1, BatchNorm1dImpl> {
 protected:
...
/// `smith::nn::BatchNorm1dOptions`. See the documentation for `ModuleHolder` to
/// learn about Blacksmith's module storage semantics.
SMITH_MODULE(BatchNorm1d);

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ BatchNorm2d
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

/// Applies the BatchNorm2d function.
/// See https://blacksmith.org/docs/main/nn.html#smith.nn.BatchNorm2d to learn
/// about the exact behavior of this module.
///
/// See the documentation for `smith::nn::BatchNorm2dOptions` class to learn
/// what constructor arguments are supported for this module.
///
/// Example:
/// ```
...
/// ```
class SMITH_API BatchNorm2dImpl : public BatchNormImplBase<2, BatchNorm2dImpl> {
 protected:
  void _check_input_dim(const Tensor& input) override;
...
/// `smith::nn::BatchNorm2dOptions`. See the documentation for `ModuleHolder` to
/// learn about Blacksmith's module storage semantics.
SMITH_MODULE(BatchNorm2d);

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ BatchNorm3d
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

/// Applies the BatchNorm3d function.
/// See https://blacksmith.org/docs/main/nn.html#smith.nn.BatchNorm3d to learn
/// about the exact behavior of this module.
///
/// See the documentation for `smith::nn::BatchNorm3dOptions` class to learn
/// what constructor arguments are supported for this module.
///
/// Example:
/// ```
...
/// ```
class SMITH_API BatchNorm3dImpl : public BatchNormImplBase<3, BatchNorm3dImpl> {
 protected:
  void _check_input_dim(const Tensor& input) override;
...
/// `smith::nn::BatchNorm3dOptions`. See the documentation for `ModuleHolder` to
/// learn about Blacksmith's module storage semantics.
SMITH_MODULE(BatchNorm3d);
