...
#include <smith/nn/modules/container/any_value.h>
...
/// The base class for all modules in Blacksmith.
///
/// \rst
/// .. note::
...
///   :py:class:`blacksmith:smith.nn.Module` for further clarification on certain
///   methods or behavior.
/// \endrst
///
...
/// `module->to(smith::kCUDA)` to move all parameters to GPU memory. Lastly,
...
class SMITH_API Module : public std::enable_shared_from_this<Module> {
 public:
  ...
  /// asynchronously with respect to the host. Otherwise, the argument has no
  /// effect.
  virtual void to(smith::Dtype dtype, bool non_blocking = false);
  ...
  /// asynchronously with respect to the host. Otherwise, the argument has no
  /// effect.
  virtual void to(smith::Device device, bool non_blocking = false);
  ...
  template <
      typename ModuleType,
      typename = smith::detail::disable_if_module_holder_t<ModuleType>>
  ...
  ///
  ///   MyModule::MyModule() {
  ///     weight_ = register_parameter("weight", smith::randn({A, B}));
  ///   }
  /// \endrst
  ...
  ///
  ///   MyModule::MyModule() {
  ///     mean_ = register_buffer("mean", smith::empty({num_features_}));
  ///   }
  /// \endrst
  ...
  ///
  ///   MyModule::MyModule() {
  ///     submodule_ = register_module("linear", smith::nn::Linear(3, 4));
  ///   }
  /// \endrst
  template <typename ModuleType>
 ...
 protected:
  ...
  virtual unsigned int _forward_num_required_args() {
    SMITH_CHECK(
        false,
        "smith::nn::Module subclass that has default arguments in `forward` method ",
...
SMITH_API serialize::OutputArchive& operator<<(
    serialize::OutputArchive& archive,
    const std::shared_ptr<nn::Module>& module);

/// Deserializes a `Module` from an `InputArchive`.
SMITH_API serialize::InputArchive& operator>>(
...
std::shared_ptr<ModuleType> Module::register_module(
    std::string name,
    std::shared_ptr<ModuleType> module) {
  SMITH_CHECK(!name.empty(), "Submodule name must not be empty");
  SMITH_CHECK(
