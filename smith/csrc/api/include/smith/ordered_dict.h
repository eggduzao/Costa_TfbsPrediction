...
typename OrderedDict<Key, Value>::Item& OrderedDict<Key, Value>::front() {
  SMITH_CHECK(!items_.empty(), "Called front() on an empty OrderedDict");
  return items_.front();
}
...
typename OrderedDict<Key, Value>::Item& OrderedDict<Key, Value>::back() {
  SMITH_CHECK(!items_.empty(), "Called back() on an empty OrderedDict");
  return items_.back();
}
...
typename OrderedDict<Key, Value>::Item& OrderedDict<Key, Value>::operator[](
    size_t index) {
  SMITH_CHECK(index < items_.size(), "Index ", index, " is out of bounds");
  return items_[index];
}
...
Value& OrderedDict<Key, Value>::operator[](const Key& key) {
  ...
  SMITH_CHECK(false, key_description_, " '", key, "' is not defined");
}
...
void OrderedDict<Key, Value>::erase(const Key& key) {
  ...
  SMITH_CHECK(it != index_.end(), "Key '", key, "' doesn't exist");
...
template <typename K, typename V>
bool operator==(
    const smith::OrderedDict<K, V>& a,
    const smith::OrderedDict<K, V>& b) {
  using Item = typename smith::OrderedDict<K, V>::Item;
