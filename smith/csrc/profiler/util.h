...
// TODO: replace with blacksmith/rfcs#43 when it is ready.
#define SOFT_ASSERT(cond, ...)                         \
  [&]() -> bool {                                      \
    if (C10_UNLIKELY(!(cond))) {                       \
      smith::profiler::impl::logSoftAssert(            \
          __func__,                                    \
          __FILE__,                                    \
          static_cast<uint32_t>(__LINE__),             \
          #cond,                                       \
          ::c10::str(__VA_ARGS__));                    \
      if (smith::profiler::impl::softAssertRaises()) { \
        SMITH_INTERNAL_ASSERT(cond, __VA_ARGS__);      \
      } else {                                         \
        SMITH_WARN_ONCE(__VA_ARGS__);                  \
      }                                                \
      return false;                                    \
    }                                                  \
    return true;                                       \
...
SMITH_API bool softAssertRaises();
SMITH_API void setSoftAssertRaises(std::optional<bool> value);
SMITH_API void logSoftAssert(
    const char* func,
    const char* file,
    uint32_t line,
    const char* cond,
...
struct SMITH_API FileLineFunc {
  std::string filename;
  size_t line;
  std::string funcname;
};

struct SMITH_API SaveNcclMetaConfig {
  bool truncate;
...
SMITH_API std::vector<FileLineFunc> prepareCallstack(
    const std::vector<jit::StackEntry>& cs);
SMITH_API std::vector<std::string> callstackStr(
    const std::vector<FileLineFunc>& cs);
SMITH_API std::string stacksToStr(
    const std::vector<std::string>& stacks,
    const char* delim);
SMITH_API std::vector<std::vector<int64_t>> inputSizes(
    const at::RecordFunction& fn,
    const bool flatten_list_enabled = false);
SMITH_API std::string variantShapesToStr(const std::vector<shape>& shapes);
SMITH_API std::string shapesToStr(
    const std::vector<std::vector<int64_t>>& shapes);
SMITH_API std::string strListToStr(const std::vector<std::string>& types);
SMITH_API std::string inputOpIdsToStr(
    const std::list<std::pair<at::RecordFunctionHandle, int>>& input_op_ids);
SMITH_API std::string ivalueToStr(const c10::IValue& val, bool isString);
SMITH_API std::string ivalueListToStr(const std::vector<c10::IValue>& list);
SMITH_API std::vector<std::string> inputTypes(const at::RecordFunction& fn);

std::unordered_map<std::string, c10::IValue> SMITH_API
saveExtraArgs(const at::RecordFunction& fn);
std::unordered_map<std::string, std::string> SMITH_API saveNcclMeta(
    const at::RecordFunction& fn,
...
uint64_t SMITH_API computeFlops(
...
template <typename T>
class SMITH_API GlobalStateManager {
 public:
