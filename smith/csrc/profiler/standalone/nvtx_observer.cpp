...
struct NVTXThreadLocalState : ProfilerStateBase {
  explicit NVTXThreadLocalState(const ProfilerConfig& config)
    ...
    SMITH_CHECK(!config.profile_memory);
    SMITH_CHECK(!config.with_stack);
    SMITH_CHECK(!config.with_flops);
    SMITH_CHECK(!config.with_modules);
  }
...
static std::list<std::pair<at::RecordFunctionHandle, int>> flattenOpIdList(
  ...
  SMITH_INTERNAL_ASSERT(state_ptr, "Expected profiler state set");
...
static std::unique_ptr<at::ObserverContext> enterNVTX(
  ...
  if (NVTXThreadLocalState::getTLS() != nullptr) {
    ...
    smith::profiler::impl::cudaStubs()->rangePush(
        smith::profiler::impl::getNvtxStr(
            fn.name(),
            fn.seqNr(),
            report_input_shapes ? smith::profiler::impl::inputSizes(fn, true)
