...
// https://github.com/blacksmith/blacksmith/issues/124674
#ifdef USE_DISTRIBUTED
...
//******************************************************************************
// JSON output utility functions. To be merged with Blacksmith profiler.
//******************************************************************************
template <typename T>
...
// safe containers.
struct SMITH_API ExecutionTraceObserver { // NOLINT
  using ID = size_t;
...
static std::ofstream openOutputFile(const std::string& name) {
  ...
  if (!stream) {
    ...
    VLOG(1) << "Blacksmith Execution Trace: writing to " << name;
  }
  return stream;
}
...
static void finalizeExecutionTraceOutput(ExecutionTraceObserver& ob) {
  writeJsonNode(
      ob.out,
      "[blacksmith|profiler|execution_trace|process]",
      kRootId,
      0, // rf_id
  ...
  ob.out.close();
  VLOG(1) << "Blacksmith Execution Trace: written to file " << ob.fileName;
}
  ...
  try {
    {
      ...
      if (ob.opStack[tid].empty()) {
        ...
        writeJsonNode(
            ob.out,
            "[blacksmith|profiler|execution_trace|thread]",
            thread_node_id,
            0, // rf_id
            kRootId,
            0, // fw_parent
...
static void onFunctionExit(const RecordFunction& fn, ObserverContext* ctx_ptr) {
  ...
  if (ob->getState() == RunState::enabled) {
    ...
    // SMITH_INTERNAL_ASSERT(fc_ptr != nullptr);
    if (fc_ptr == nullptr) {
...
bool addExecutionTraceObserver(const std::string& output_file_path) {
  ...
  if (ObserverManager::get() == nullptr) {
    ...
    auto env_variable = c10::utils::get_env(
        "ENABLE_BLACKSMITH_EXECUTION_TRACE_SAVE_INTEGRAL_TENSOR_RANGE");
    ...
    env_variable = c10::utils::get_env(
        "ENABLE_BLACKSMITH_EXECUTION_TRACE_SAVE_INTEGRAL_TENSOR_DATA");
    if (env_variable.has_value()) {
    ...
    VLOG(1) << "Blacksmith Execution Trace: added observer, output="
            << output_file_path;
...
void removeExecutionTraceObserver() {
  ...
  if (ob != nullptr) {
    ...
    if (ob->cbHandle != INVALID_CALLBACK_HANDLE) {
      ...
      VLOG(1) << "Blacksmith Execution Trace: removed observer";
    } else {
