...
#include <smith/csrc/profiler/unwind/communicate.h>
#include <smith/csrc/profiler/unwind/eh_frame_hdr.h>
#include <smith/csrc/profiler/unwind/fast_symbolizer.h>
#include <smith/csrc/profiler/unwind/fde.h>
#include <smith/csrc/profiler/unwind/unwinder.h>
...
static const char* process_name() {
  ...
  if (*name == '\0') {
    ...
    SMITH_INTERNAL_ASSERT(len != -1, "can't get path to exe")
    name[len] = '\0';
  }
  return name;
}
...
struct UnwindCache {
  ...
  const Unwinder& unwinderFor(
    ...
    try {
      ...
      SMITH_WARN("Unsupported unwinding pattern: ", err.what());
    }
  ...
  const LibraryInfo& libraryFor(uint64_t addr) {
    ...
    if (!r) {
      for ([[maybe_unused]] const auto& l : libraries_with_no_unwind_) {
        SMITH_WARN("Did not find a PT_GNU_EH_FRAME segment for ", l);
      }
...
struct Symbolizer {
  Symbolizer() {
    auto envar = c10::utils::get_env("SMITH_ADDR2LINE_BINARY");
    if (envar.has_value()) {
      // currently we take user's input as is without checking
      addr2line_binary_ = std::move(envar.value());
      SMITH_WARN("Use custom addr2line binary: ", addr2line_binary_);
    } else {
...
extern "C" C10_USED void unwind_c(
    ...
    int64_t rsp,
    int64_t rbp) {
  std::shared_lock lock(smith::unwind::cache_mutex_);
  smith::unwind::UnwindState state{};
  // NOLINTNEXTLINE(performance-no-int-to-ptr)
  ...
  state.rsp = rsp + 8;
  state.rbp = rbp;
  smith::unwind::unwind_cache.checkRefresh(lock);
  while (true) { // unwind for _start sets rip as being undefined
    // NOLINTNEXTLINE(performance-no-int-to-ptr)
    result->push_back((void*)state.rip);
    const smith::unwind::Unwinder& uw =
        smith::unwind::unwind_cache.unwinderFor(state.rip, lock);
    if (uw.terminator()) {
      if (uw.isUnknown()) {
