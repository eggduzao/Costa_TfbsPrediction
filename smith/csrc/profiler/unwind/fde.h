...
#include <smith/csrc/profiler/unwind/action.h>
...
struct FDE {
  FDE(void* data, const char* library_name, uint64_t load_bias)
    ...
    SMITH_INTERNAL_ASSERT(zero == 0, "expected 0 for CIE");
    ...
    SMITH_INTERNAL_ASSERT(ra_register_ == 16, "unexpected number of registers");
