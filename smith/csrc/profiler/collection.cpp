...
#include <smith/csrc/profiler/orchestration/vulkan.h>
...
#include <smith/csrc/profiler/data_flow.h>
      ...
      switch (*tag_it) {
        ...
        case Tag::TensorListBegin: {
          ...
          while (*(++tag_it) != Tag::TERMINATOR) {
            ...
            SMITH_INTERNAL_ASSERT(*tag_it == Tag::Tensor, (int)(*tag_it));
...
ThreadLocalSubqueue::SmithOpStorage::EventBlock<T, ChunkSize>::EventBlock() {
...
std::pair<KinetoObserverContext::Event*, uint64_t> ThreadLocalSubqueue::
    SmithOpStorage::OpList::emplace_back(Args&&... args) {
...
uint64_t ThreadLocalSubqueue::SmithOpStorage::OpList::correlationID(
    const OpList::Iterator& e) {
  return e.address().first->correlation_id(&*e);
}

template <typename T, size_t ChunkSize>
uint64_t ThreadLocalSubqueue::SmithOpStorage::EventBlock<T, ChunkSize>::
...
std::unique_ptr<KinetoObserverContext> ThreadLocalSubqueue::begin_op(
  ...
  auto overload_name = config_.experimental_config.capture_overload_names
      ? fn.overload_name()
      : "";
  auto [event, corr_id] = smith_ops_.op_events_.emplace_back(
      smith::profiler::impl::SmithOpBasicFields{
          fn.seqNr(),
          fn.forwardThreadId(),
          fn.scope(),
          fn.isAsync(),
  ...
  if (config_.report_input_shapes) {
    smith_ops_.inputs_outputs_.push(fn.inputs());
    smith_ops_.kwinputs_.emplace_back(fn.kwinputs());
  }
  if (!config_.experimental_config.disable_external_correlation) {
    if (fn.scope() == at::RecordScope::USER_SCOPE) {
      smith::profiler::impl::kineto::pushUserCorrelationId(corr_id);
    } else {
      smith::profiler::impl::kineto::pushCorrelationId(corr_id);
    }
  }
  ...
  if (config_.with_stack && fn.scope() != at::RecordScope::BACKWARD_FUNCTION) {
    auto cs = smith::profiler::impl::prepareCallstack(jit::currentCallstack());
    smith_ops_.jit_stack_.emplace_back(callstackStr(cs));
  }
  if (config_.with_modules &&
      fn.scope() != at::RecordScope::BACKWARD_FUNCTION) {
    smith_ops_.jit_modules_.emplace_back(jit::currentModuleHierarchy());
  }
#endif
  if (config_.with_flops) {
    smith_ops_.extra_args_.emplace_back(
        smith::profiler::impl::saveExtraArgs(fn));
  }
  ...
  if (fn.isNcclMeta()) {
    ...
    out->event_->extra_nccl_meta_ = smith_ops_.extra_meta_.emplace_back(
  ...
  if (config_.state == ProfilerState::KINETO_GPU_FALLBACK) {
    try {
      out->fallback_ = smith_ops_.device_fallback_.emplace_back();
...
void ThreadLocalSubqueue::SmithOpStorage::materialize(
  ...
  const std::string accumulate_grad = "smith::autograd::AccumulateGrad";
  ...
  for (auto event = op_events_.begin(); event != op_events_.end(); ++event) {
    ExtraFields<EventType::SmithOp> e{
        std::move(event->basic_fields_),
        ThreadLocalSubqueue::SmithOpStorage::OpList::correlationID(event),
...
static void materialize_vulkan(
  ...
  for (const auto& i : raw_events) {
    const auto name_and_duration_ns =
        smith::profiler::impl::vulkan::getShaderNameAndDurationNs(i.second);
...
int64_t smithOpEndNS(
    const ExtraFields<EventType::SmithOp>& e,
    const bool finished,
...
std::string Result::overload_name() const {
  return visit(c10::overloaded(
      ATTRIBUTE(SmithOp, std::string(e.overload_name_)),
      [](const auto& e) -> std::string { return ""; }));
}

libkineto::ActivityType Result::kinetoType() const {
  return visit(c10::overloaded(
      ATTRIBUTE(SmithOp, scopeToType(e.scope_)),
...
uint64_t Result::correlationID() const {
  return visit(c10::overloaded(
      ATTRIBUTE(SmithOp, e.correlation_id_),
...
int64_t Result::endTimeNS() const {
  auto end_time_ns = visit(c10::overloaded(
      ATTRIBUTE(SmithOp, smithOpEndNS(e, finished_, parent_)),
...
uint64_t Result::endTID() const {
  return visit(c10::overloaded(
      ATTRIBUTE(SmithOp, e.end_tid_),
      [&](const auto&) -> uint64_t { return start_tid_; }));
}

c10::DeviceType Result::deviceType() const {
  using smith::autograd::profiler::deviceTypeFromActivity;
  return visit(c10::overloaded(
...
ThreadLocalSubqueue::ThreadLocalSubqueue(
  ...
  smith::profiler::impl::kineto::recordThreadInfo();
  if (!config_.experimental_config.performance_events.empty()) {
    perf_profiler_ =
        std::make_unique<smith::profiler::impl::linux_perf::PerfProfiler>();
...
void mark_finished(std::shared_ptr<Result>& r) {
  SMITH_INTERNAL_ASSERT(!r->finished_, r->name());
  r->finished_ = true;
  SMITH_INTERNAL_ASSERT(r->endTimeNS() >= r->start_time_ns_, r->name());
}
...
void generateForwardBackwardLink(
    ...
    std::unordered_map<uint64_t, libkineto::GenericTraceActivity*>&
        tidSeq2activity) {
  const ExtraFields<EventType::SmithOp>& extra_fields =
      std::get<ExtraFields<EventType::SmithOp>>(profiler_result.extra_fields_);
...
void generateForwardBackwardLinks(
    std::unique_ptr<smith::profiler::impl::kineto::trace_t>& cpu_trace,
    const std::vector<std::shared_ptr<Result>>& results) {
#ifndef USE_KINETO
}
#else // USE_KINETO
  SMITH_INTERNAL_ASSERT(cpu_trace->activities.size() == results.size());
  ...
  std::vector<result_activity_t> smith_events;
  ...
  for (const auto idx : c10::irange(cpu_trace->activities.size())) {
    ...
    profiler_result->visit_if_base<ExtraFields<EventType::SmithOp>>(
        [&](const auto& e) {
          if (e.sequence_number_ >= 0) {
            smith_events.emplace_back(profiler_result.get(), activity.get());
          }
        });
  }
  ...
  std::sort(
      ...
      [](const result_activity_t& left, const result_activity_t& right) {
        auto left_end_time =
            std::get<ExtraFields<EventType::SmithOp>>(left.first->extra_fields_)
                .end_time_ns_;
        auto right_end_time = std::get<ExtraFields<EventType::SmithOp>>(
                                  right.first->extra_fields_)
                                  .end_time_ns_;
        return left_end_time < right_end_time;
      });

  for (auto& [profiler_result, activity] : smith_events) {
...
void passEventsToKineto(
  ...
  using namespace smith::profiler::impl::kineto;
  TraceWrapper cpu_trace(
      static_cast<int64_t>(start_time_ns), "Blacksmith Profiler");

  // Generate Kineto events for each event recorded by the Blacksmith profiler.
  for (const auto i : c10::irange(results.size())) {
    ...
    SMITH_INTERNAL_ASSERT(activity || !kKinetoAvailable);
    if (activity) {
...
// dependency information between kineto ops. Consider a call to `smith.add`.
// Three events will be collected:
//   `aten::add`          (SmithOp, collected by profiler)
...
class TransferEvents {
  ...
  using activity_t = smith::profiler::impl::kineto::activity_t;
 ...
 public:
  TransferEvents(
    ...
    SMITH_INTERNAL_ASSERT(trace_activities_ptr != nullptr);
 ...
 private:
  ...
  std::shared_ptr<Result> resultFromActivity(const itrace_t* activity) {
    ...
    smith::profiler::impl::kineto::DeviceAndResource device_and_resource{
  ...
  std::shared_ptr<Result> toResult(const itrace_t* activity) {
    ...
    if (e == nullptr &&
      ...
      SMITH_WARN_ONCE(
          "Detected an event which was likely passed to kineto by the Blacksmith "
      ...
      if (e) {
        if (config_.experimental_config.expose_kineto_event_metadata) {
          e->visit(c10::overloaded(
              [&](ExtraFields<EventType::SmithOp>& i) {
        ...
        if (linked_activity) {
          e->visit(c10::overloaded(
              ...
              [](auto&) { SMITH_INTERNAL_ASSERT(false); }));
        }
      }
    }
  }
  ...
  void setKinetoTID(
    ...
    r->visit(c10::overloaded(
        [&]([[maybe_unused]] ExtraFields<EventType::Kineto>& i) {
          SMITH_INTERNAL_ASSERT(r->start_tid_ == noTID);
  ...
  void setParents() {
    ...
    for (auto& e : results_.get()) {
      SMITH_INTERNAL_ASSERT(e != nullptr);
      e->visit(c10::overloaded(
...
#else // USE_ROCM
              SMITH_INTERNAL_ASSERT(inserted.second);
#endif // USE_ROCM
            }
            SMITH_INTERNAL_ASSERT(e->parent_.expired());
...
trace_ptr_t addKinetoEvents(
  ...
  SMITH_INTERNAL_ASSERT(trace || !kKinetoAvailable);
...
void build_tree(std::vector<std::shared_ptr<Result>>& sorted_events) {
  ...
  using op_fields = ExtraFields<EventType::SmithOp>;
  ...
  auto push_event = [&stacks, &end_events_](std::shared_ptr<Result>& event) {
    ...
    SMITH_INTERNAL_ASSERT(event->parent_.expired());
    for (const auto& child : event->children_) {
      SMITH_INTERNAL_ASSERT(child->finished_);
    }
    SMITH_INTERNAL_ASSERT(!event->finished_);
  ...
  auto pop_event = [&stacks](std::shared_ptr<Result> event) {
    ...
    while (frame.get() != event.get()) {
      SMITH_INTERNAL_ASSERT(frame != nullptr);
      mark_finished(frame);
      SMITH_INTERNAL_ASSERT(!frame->parent_.expired());
...
int64_t adjust_durations_dfs(std::shared_ptr<Result>& r) {
  if (SOFT_ASSERT(r != nullptr)) {
    ...
    if (children_total_duration > original_duration) {
      r->visit(c10::overloaded(
          [&r, &children_total_duration](ExtraFields<EventType::SmithOp>& i) {
...
int64_t adjust_timestamps_dfs(
  ...
  if (SOFT_ASSERT(r != nullptr)) {
    if (r->start_time_ns_ != new_start_time) {
      ...
      r->visit(c10::overloaded(
          [&r, &new_start_time](ExtraFields<EventType::SmithOp>& i) {
            i.end_time_ns_ =
...
RecordQueue::getRecords(
  ...
  if (python_tracer_) {
    std::vector<std::shared_ptr<smith::profiler::impl::Result>> ev;
    try {
      ...
      // exception.
      smith::profiler::impl::kineto::stopTrace();
      throw;
    }
