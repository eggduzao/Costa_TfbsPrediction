...
#include <smith/csrc/profiler/python/combined_traceback.h>
#include <smith/csrc/profiler/standalone/execution_trace_observer.h>
#include <smith/csrc/utils/pybind.h>

struct THPCapturedTraceback {
  PyObject_HEAD
  std::shared_ptr<smith::CapturedTraceback> data;
};
...
static void THPCapturedTraceback_dealloc(PyObject* self_) {
  ...
  self->data.~shared_ptr<smith::CapturedTraceback>();
  // promptly trigger delayed frees since we have GIL
  smith::freeDeadCapturedTracebackFrames();
  PyObject_GC_Del(self);
}

static PyTypeObject THPCapturedTracebackType = {
    PyVarObject_HEAD_INIT(nullptr, 0)
    "smith._C._profiler.CapturedTraceback", /* tp_name */
...
template <>
struct type_caster<std::shared_ptr<smith::CapturedTraceback>> {
 public:
  PYBIND11_TYPE_CASTER(
      std::shared_ptr<smith::CapturedTraceback>,
      _("smith._C._profiler.CapturedTraceback"));
  ...
  static handle cast(
      std::shared_ptr<smith::CapturedTraceback> src,
      return_value_policy /* policy */,
      handle /* parent */) {
    auto* r = PyObject_GC_New(THPCapturedTraceback, &THPCapturedTracebackType);
    new (&r->data) std::shared_ptr<smith::CapturedTraceback>(std::move(src));
 ...
 * The smith.profiler.record_function context manager is slow (~14us on
 ...
 * record_function calls work with smithscript.
 *
 * This implementation doesn't go through the dispatcher and so it won't work
 * with any feature relying on the dispatcher (e.g. smithscript or
 * smith.compile)
 *
 ...
 *      rf = smith._C._record_function_fast_enter(name)
 *      try:
 *        yield
 *      finally:
 *        smith._C._record_function_fast_exit(rf)
...
int RecordFunctionFast_init(
  ...
  if (keyword_values) {
    SMITH_CHECK(PyDict_Check(keyword_values), "keyword_values must be dict");
...
PyObject* RecordFunctionFast_enter(PyObject* selfGeneric, PyObject* unused) {
  HANDLE_TH_ERRORS
  if (smith::profiler::impl::ProfilerStateBase::get() != nullptr) {
    auto self = (RecordFunctionFast*)selfGeneric;
    ...
    bool profiler_need_input = smith::autograd::profiler::profilerEnabled() &&
        smith::autograd::profiler::getProfilerConfig().report_input_shapes;
    ...
    if (self->input_values != nullptr && profiler_need_input) {
      ...
      for (int i = 0; i < PySequence_Fast_GET_SIZE(input_fast.get()); i++) {
        ...
        auto match = smith::jit::tryToInferType(item);
        if (match.success()) {
          args.push_back(smith::jit::toIValue(item, match.type()));
        }
      }
    }
    ...
    if (self->keyword_values != nullptr && profiler_need_input) {
      ...
      while (PyDict_Next(self->keyword_values, &pos, &key, &value)) {
        ...
        if (THPUtils_checkString(value)) {
          ...
          auto match = smith::jit::tryToInferPrimitiveType(value);
          if (match.success()) {
            ivalue = smith::jit::toIValue(value, match.type());
            ...
            SMITH_WARN("Unable to infer type of value for keyword: ", key_str);
...
void initPythonBindings(PyObject* module) {
  ...
  py::enum_<at::RecordScope>(m, "RecordScope")
      ...
      .value("SMITHSCRIPT_FUNCTION", at::RecordScope::SMITHSCRIPT_FUNCTION)
  ...
  py::class_<ExperimentalConfig>(m, "_ExperimentalConfig")
      .def(
          ...
          "    expose_kineto_event_metadata (bool) : whether to expose KinetoEvent metadata in the Blacksmith Profiler\n"
      ...
      .def(py::pickle(
          ...
          [](const py::tuple& t) { // __setstate__
            SMITH_CHECK(t.size() >= 12, "Expected at least 12 values in state");
  ...
  py::enum_<EventType>(m, "_EventType")
      .value("SmithOp", EventType::SmithOp)
  ...
  py::class_<TensorMetadata>(m, "_TensorMetadata")
      ...
      .def_property_readonly(
          ...
          [](const TensorMetadata& metadata) {
            PyObject* layout_obj =
                smith::autograd::utils::wrap(metadata.layout_);
      ...
      .def_property_readonly(
          ...
          [](const TensorMetadata& metadata) {
            return py::reinterpret_borrow<py::object>(
                smith::autograd::utils::wrap(metadata.dtype_));
          })
  ...
  using smith_op_t = ExtraFields<EventType::SmithOp>;
  py::class_<smith_op_t>(m, "_ExtraFields_SmithOp")
      .def_readonly("name", &smith_op_t::name_)
      .def_property_readonly(
          "inputs",
          [](const smith_op_t& op) {
            py::list out;
            for (const auto& input : op.inputs_) {
              std::visit(
                  c10::overloaded(
                      [&](const c10::IValue& v) {
                        out.append(smith::jit::toPyObject(v));
                      },
      ...
      .def_readonly("scope", &smith_op_t::scope_)
      .def_readonly("sequence_number", &smith_op_t::sequence_number_)
      .def_readonly("allow_tf32_cublas", &smith_op_t::allow_tf32_cublas_);
  ...
  // Blacksmith profiler execution trace internal interface.
  m.def(
      "_add_execution_trace_observer",
      &smith::profiler::impl::addExecutionTraceObserver,
      py::arg("output_file_name"));
  m.def(
      "_remove_execution_trace_observer",
      &smith::profiler::impl::removeExecutionTraceObserver);
  m.def(
      "_enable_execution_trace_observer",
      &smith::profiler::impl::enableExecutionTraceObserver);
  m.def(
      "_disable_execution_trace_observer",
      &smith::profiler::impl::disableExecutionTraceObserver);
  m.def(
      "_set_record_concrete_inputs_enabled_val",
      &smith::profiler::impl::set_record_concrete_inputs_enabled_val);
  m.def(
      "_set_fwd_bwd_enabled_val",
      &smith::profiler::impl::set_fwd_bwd_enabled_val);
  m.def(
      "_set_cuda_sync_enabled_val",
      &smith::profiler::impl::set_cuda_sync_enabled_val);

  SMITH_CHECK(PyType_Ready(&THPCapturedTracebackType) >= 0);
  PyModule_AddObject(
  ...
  m.def(
      ...
      [](const std::vector<uint64_t>& frames, const std::string& mode_s) {
        ...
        smith::unwind::Mode mode = smith::unwind::Mode::addr2line;
        if (mode_s == "fast") {
          mode = smith::unwind::Mode::fast;
        } else if (mode_s == "addr2line") {
          mode = smith::unwind::Mode::addr2line;
        } else if (mode_s == "dladdr") {
          mode = smith::unwind::Mode::dladdr;
        } else {
          SMITH_CHECK(false, "unexpected mode ", mode_s);
        }
  ...
  RecordFunctionFast_Type.tp_name = "smith._C._profiler.RecordFunctionFast",
