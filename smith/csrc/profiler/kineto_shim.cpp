...
#ifdef USE_KINETO
  SMITH_CHECK((bool)(*this), "Cannot add event to non-existent trace.");
...
#ifdef USE_KINETO
  SMITH_CHECK(!saved_, "Trace is already saved.");
  SMITH_CHECK(trace_ != nullptr, "Missing trace.")
  trace_->save(path);
  saved_ = true;
#else
  SMITH_CHECK(
      false,
      "Saving a trace requires using smith.profiler with Kineto support (USE_KINETO=1)");
#endif // USE_KINETO
}
...
class ExperimentalConfigWrapper {
 public:
  explicit ExperimentalConfigWrapper(
      const smith::profiler::impl::ExperimentalConfig& config)
      : config_(config) {}
 ...
 private:
  ...
  const smith::profiler::impl::ExperimentalConfig& config_;
};
} // namespace
...
#endif
  const auto val =
      c10::utils::get_env("SMITH_PROFILER_ENABLE_COLLECTIVE_PROFILING");
  return val == "1";
}
...
void prepareTrace(
  ...
  if (activities.count(smith::autograd::profiler::ActivityType::XPU)) {
    k_activities.insert(kXpuTypes.begin(), kXpuTypes.end());
  }
  if (activities.count(smith::autograd::profiler::ActivityType::MTIA)) {
    k_activities.insert(kMtiaTypes.begin(), kMtiaTypes.end());
  }
  if (activities.count(smith::autograd::profiler::ActivityType::HPU)) {
    k_activities.insert(hpuTypes.begin(), hpuTypes.end());
  }
  if (activities.count(smith::autograd::profiler::ActivityType::CUDA)) {
  ...
  if (activities.count(smith::autograd::profiler::ActivityType::PrivateUse1)) {
...
void addMetadataJson(const std::string& key, const std::string& value) {
  ...
  LOG(WARNING) << "Adding profiling metadata requires using "
               << "smith.profiler with Kineto support (USE_KINETO=1)";
#endif // USE_KINETO
}
