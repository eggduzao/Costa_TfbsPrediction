...
bool ProfilerConfig::disabled() const {
  return state == smith::profiler::impl::ProfilerState::Disabled;
}

bool ProfilerConfig::global() const {
  return state == smith::profiler::impl::ProfilerState::KINETO_ONDEMAND;
}
...
/*static*/ void ProfilerStateBase::push(
  ...
  SMITH_INTERNAL_ASSERT(state != nullptr);
...
/*static*/ std::shared_ptr<ProfilerStateBase> ProfilerStateBase::pop(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(!out || out->config().global() == global);
  return out;
}
...
SMITH_API ActiveProfilerType profilerType() {
  auto* state_ptr = ProfilerStateBase::get(/*global=*/false);
  return state_ptr == nullptr ? ActiveProfilerType::NONE
                              : state_ptr->profilerType();
}

smith::profiler::impl::ProfilerConfig getProfilerConfig() {
  auto* state_ptr = ProfilerStateBase::get(/*global=*/false);
  SMITH_CHECK(
      state_ptr,
