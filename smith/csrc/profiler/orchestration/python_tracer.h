...
/*
Libsmith does not depend on Python (e.g. cannot #include <Python.h>); however
when we call the profiler from libsmith_python we need the profiler to be able
...
exposes itself by calling `registerTracer` from `smith/csrc/autograd/init.cpp`.
This pattern of registration for faux python dependencies in libsmith is common
in the Blacksmith codebase.
*/
struct SMITH_API PythonTracerBase {
...
SMITH_API void registerTracer(MakeFn make_tracer);

/**
 * Memory Tracer Implementation
 */
struct SMITH_API PythonMemoryTracerBase {
...
SMITH_API void registerMemoryTracer(MakeMemoryFn make_memory_tracer);
