...
#include <smith/csrc/profiler/perf-inl.h>
#include <smith/csrc/profiler/perf.h>

namespace smith::profiler::impl::linux_perf {
...
void PerfEvent::Init() {
  SMITH_CHECK(!name_.empty(), "Invalid profiler event name");

  auto const it = EventTable.find(name_);
  if (it == EventTable.end()) {
    SMITH_CHECK(false, "Unsupported profiler event name: ", name_);
  }
