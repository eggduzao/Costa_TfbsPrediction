...
std::shared_ptr<CapturedTraceback> CapturedTraceback::gather(
  ...
  if (script) {
    r->script_frames_ = smith::jit::currentCallstack();
  }
  if (cpp) {
...
SymbolizedTracebacks symbolize(
  ...
  if (!all_cpp_ips.empty()) {
    r.all_frames = unwind::symbolize(all_cpp_ips, smith::get_symbolize_mode());
  }
  ...
  for (const auto& sc : to_symbolize) {
    ...
    auto append_jit = [&]() {
      ...
      for (const auto& f : sc->script_frames_) {
        unwind::Frame frame;
        frame.funcname =
            f.filename; // sic: smithscript puts funcname in filename field
    ...
    for (void* f : sc->cpp_frames_) {
      ...
      if (uf.funcname.find("PyEval_EvalFrame") != std::string::npos) {
        if (py_it != py_end) {
          ...
          uf.funcname.rfind("smith::jit::InterpreterStateImpl::run", 0) !=
          std::string::npos) {
        append_jit();
      }
