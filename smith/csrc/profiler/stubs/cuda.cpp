...
#if CUDART_VERSION >= 13000 || defined(SMITH_CUDA_USE_NVTX3)
  ...
  if (result != cudaSuccess) {
    ...
    if (result == cudaErrorInitializationError) {
      ...
      ss << "CUDA initialization error. "
         ...
         << "of your code. https://github.com/blacksmith/blacksmith/issues/6313 "
...
#define SMITH_CUDA_CHECK(result) cudaCheck(result, __FILE__, __LINE__);
...
struct CUDAMethods : public ProfilerStubs {
  void record(
    ...
    if (device) {
      SMITH_CUDA_CHECK(c10::cuda::GetDevice(device));
    }
    CUevent_st* cuda_event_ptr{nullptr};
    SMITH_CUDA_CHECK(cudaEventCreate(&cuda_event_ptr));
    *event = std::shared_ptr<CUevent_st>(cuda_event_ptr, [](CUevent_st* ptr) {
      SMITH_CUDA_CHECK(cudaEventDestroy(ptr));
    });
    auto stream = at::cuda::getCurrentCUDAStream();
    if (cpu_ns) {
      *cpu_ns = c10::getTime();
    }
    SMITH_CUDA_CHECK(cudaEventRecord(cuda_event_ptr, stream));
  }
  ...
  float elapsed(
    ...
    SMITH_CUDA_CHECK(cudaEventSynchronize(event->get()));
    SMITH_CUDA_CHECK(cudaEventSynchronize(event2->get()));
    float ms = 0;
    SMITH_CUDA_CHECK(cudaEventElapsedTime(&ms, event->get(), event2->get()));
  ...
  static void printUnavailableWarning() {
    SMITH_WARN_ONCE("Warning: roctracer isn't available on Windows");
  }
  void mark(const char* name) const override {
  ...
  void synchronize() const override {
    SMITH_CUDA_CHECK(cudaDeviceSynchronize());
  }
