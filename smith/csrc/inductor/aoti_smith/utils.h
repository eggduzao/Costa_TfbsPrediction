...
#define AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE(...)    \
  try {                                                    \
    __VA_ARGS__                                            \
  } catch (const std::exception& e) {                      \
    LOG(ERROR) << "Exception in aoti_smith: " << e.what(); \
    return AOTI_SMITH_FAILURE;                             \
  } catch (...) {                                          \
    LOG(ERROR) << "Exception in aoti_smith: UNKNOWN";      \
    return AOTI_SMITH_FAILURE;                             \
  }                                                        \
