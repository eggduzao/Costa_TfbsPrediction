...
void OSSProxyExecutor::prefill_stack_with_static_arguments(
    ...
    const std::string& smithbind_obj_name) {
  auto& stack = op_kernel->stack_;
  auto& dynamic_args = op_kernel->dynamic_args_;
  auto& smithbind_args = op_kernel->smithbind_args_;

  SMITH_CHECK(serialized_arg.size() == 1);
  ...
  switch (schema_arg_type->kind()) {
    case c10::TypeKind::ClassType: {
      ...
      SMITH_CHECK(
          has_key(custom_objs_, smithbind_obj_name),
      ...
      LOG(INFO) << "Prefilling stack with smithbind argument "
                << smithbind_obj_name;
      smithbind_args.emplace_back(index, smithbind_obj_name);
      break;
    }
    ...
    case c10::TypeKind::ListType: {
      if (schema_arg_type->isSubtypeOf(at::ListType::ofTensors())) {
        SMITH_CHECK(
            ...
            " but got ",
            serialized_arg_type);
        SMITH_CHECK(serialized_arg_type == "as_tensors");
    ...
    case c10::TypeKind::OptionalType: {
      ...
      if (serialized_arg_type == "as_none") {
        ...
        prefill_stack_with_static_arguments(
            index, inner_type, serialized_arg, op_kernel, smithbind_obj_name);
      }
      break;
    }
...
void OSSProxyExecutor::get_input_info_from_serialized(
  ...
  SMITH_CHECK(op_kernel.stack_.empty());
...
void OSSProxyExecutor::get_output_info_from_serialized(
  ...
  for (const auto& serialized_output : serialized_node["outputs"]) {
    SMITH_CHECK(serialized_output.size() == 1);
    ...
    switch (schema_return_type->kind()) {
      ...
      case c10::TypeKind::OptionalType: {
        ...
        if (inner_type->kind() == c10::TypeKind::TensorType) {
          SMITH_CHECK(serialized_output_type == "as_optional_tensor");
...
std::unique_ptr<OSSCallSmithBindKernel> OSSProxyExecutor::
    get_call_smith_bind_kernel(const nlohmann::json& serialized_node) {
  // const std::string& target = serialized_node["target"].get<std::string>();
  SMITH_CHECK(
      serialized_node["inputs"].size() > 1,
      "Expects higher_order.call_smithbind to only have at least 2 attributes, object and methodName");

  const auto first_input = serialized_node["inputs"][0]["arg"]["as_custom_obj"];
  const std::string smithbind_obj_name = first_input["name"].get<std::string>();
  ...
  auto customClassType_ = smith::jit::getCustomClass(class_fqn);
  ...
  std::unique_ptr<OSSCallSmithBindKernel> op_kernel =
      std::make_unique<OSSCallSmithBindKernel>("call_smithbind", method);
...
OSSProxyExecutor::OSSProxyExecutor(
  ...
  if (custom_objs.has_value()) {
    ...
    if (!custom_objs_json_file.is_open()) {
      ...
      // Load custom objects from binary smithbind file
      for (auto& [customObjName, file_name] : custom_objs_json.items()) {
        ...
        c10::IValue custom_obj = smith::jit::pickle_load_obj(customObjBytes);
  ...
  SMITH_CHECK(json_file.is_open(), "Unable to open file ", json_path);
  ...
  for (auto const& serialized_extern_node : json_obj["nodes"]) {
    ...
    if (pos == std::string::npos) {
      ...
      SMITH_CHECK(pos2 == std::string::npos);

      opName = target.substr(0, pos);
      overloadName = target.substr(pos + 1, target.length() - pos);
    }

    if (target == "call_smithbind") {
      // Special handling for CallSmithBind HOP
      std::unique_ptr<OSSCallSmithBindKernel> op_kernel =
          get_call_smith_bind_kernel(serialized_node);
...
void OSSProxyExecutor::call_function(
  ...
  for (auto& dynamic_arg : dynamic_args) {
    ...
    switch (dynamic_arg_type) {
      ...
      default:
        SMITH_CHECK(false, "Unsupported dynamic arg type: ", dynamic_arg_type);
    }
  }

  for (auto& smithbind_arg : smithbind_args) {
    int arg_index = smithbind_arg.arg_index;
    stack[arg_index] = custom_objs_[smithbind_arg.arg_name];
  }
  ...
  SMITH_CHECK(op_kernel->outputs_.size() == stack.size());
  SMITH_CHECK(stack.size() == schema_returns.size());
      ...
      if (op_kernel->outputs_[index].arg_type == DynamicArgType::TensorType) {
        ...
        if (stack_tensor.has_value()) {
          *tensor = stack_tensor.value();
          ...
          SMITH_CHECK(false, "Expected tensor, got None");
        }
      } else {
        index++;
      }
