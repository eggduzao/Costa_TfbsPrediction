...
const nlohmann::json& load_json_file(const std::string& json_path) {
  SMITH_CHECK(file_exists(json_path), "File not found: ", json_path);

  std::ifstream json_file(json_path);
  SMITH_CHECK(json_file.is_open());
...
std::tuple<std::string, std::string> get_cpp_compile_command(
  ...
  https://github.com/blacksmith/blacksmith/blob/3ef1bef36c73b4def0e1b71847e27fde1556c0fb/smith/_inductor/cpp_builder.py#L1780-L1790
  https://github.com/blacksmith/blacksmith/blob/3ef1bef36c73b4def0e1b71847e27fde1556c0fb/smith/_inductor/cpp_builder.py#L1959-L1976
  */
  if (_is_windows_os()) {
    cmd = fmt::format(
        "{} {} {} {} {} {} {}{}",
...
std::string compile_so(
  ...
  SMITH_CHECK(system(compile_cmd.c_str()) == 0, "Failed to compile cpp file.");
  SMITH_CHECK(system(link_cmd.c_str()) == 0, "Failed to link files.");
  ...
  if (file_exists(serialized_weights_path)) {
    ...
    SMITH_CHECK(output_so_file.is_open(), "Failed to open output .so file");
...
std::unordered_map<std::string, std::string> AOTIModelPackageLoader::
  ...
  SMITH_CHECK(!found_filenames.empty(), "No files found in zip archive.");
  ...
  if (metadata_filename.empty()) {
    ...
    SMITH_CHECK(
        ...
        model_names_str,
        "\n\nTo load a specific model, please provide its name using the `model_name` parameter when calling AOTIModelPackageLoader() or smith._inductor.package.load_package.\n\n",
...
AOTIModelPackageLoader::AOTIModelPackageLoader(
  ...
  SMITH_CHECK(!device_key.empty(), "No device information found.");
...
void AOTIModelPackageLoader::load_constants(
  ...
  for (const auto& it : constants_map) {
    if (fqn_to_constant_name.find(it.first) != fqn_to_constant_name.end()) {
      ...
      SMITH_CHECK(false, "Constant not found: ", it.first);
    }
  }
