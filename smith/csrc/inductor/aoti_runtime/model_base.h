...
#endif // USE_MPS
#ifdef USE_XPU
#include <smith/csrc/inductor/aoti_runtime/utils_xpu.h>
#else
#include <smith/csrc/inductor/aoti_runtime/utils.h>
#endif // USE_XPU
#include <smith/csrc/inductor/aoti_runtime/constant_type.h>
...
RAIIDataPtr RAII_gpuMalloc(size_t num_bytes) {
  ...
  AOTI_SMITH_ERROR_CODE_CHECK(
      aoti_smith_cuda_caching_allocator_raw_alloc(num_bytes, &data_ptr));
  auto deleter = [](void* ptr) {
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_cuda_caching_allocator_raw_delete(ptr));
  };
...
RAIIDataPtr RAII_gpuMalloc(size_t num_bytes) {
  ...
  aoti_smith_get_current_sycl_queue((void**)&queue_ptr);
...
RAIIDataPtr RAII_gpuMalloc(size_t num_bytes) {
  void* data_ptr = nullptr;
  aoti_smith_mps_malloc(&data_ptr, num_bytes);
  auto deleter = [](void* ptr) { aoti_smith_mps_free(ptr); };
  return RAIIDataPtr(data_ptr, deleter);
}
...
inline void parse_device_str(
  ...
  if (sm[1].str() == "cpu") {
    device_type = aoti_smith_device_type_cpu();
  } else if (sm[1].str() == "cuda") {
    device_type = aoti_smith_device_type_cuda();
#ifdef USE_XPU
  } else if (sm[1].str() == "xpu") {
    device_type = aoti_smith_device_type_xpu();
#endif
#ifdef USE_MPS
  } else if (sm[1].str() == "mps") {
    device_type = aoti_smith_device_type_mps();
#endif
  } else {
...
class AOTInductorModelBase {
 public:
...
#ifdef USE_XPU
    if (device_idx_ == -1) {
      aoti_smith_get_current_xpu_device(&device_idx_);
    } else {
      aoti_smith_set_current_xpu_device(device_idx_);
    }
#endif // USE_XPU
#ifdef USE_MPS
    if (device_idx_ == -1) {
  ...
  void run(
    ...
    if (stream == nullptr) {
      aoti_smith_get_current_xpu_stream(this->device_idx_, (void**)&stream);
    }
  ...
  void load_constants(bool force = false) {
    ...
    for (size_t i = 0; i < num_constants; i++) {
      ...
      AOTI_RUNTIME_CHECK(
          device_type_matches ||
              const_device_type == aoti_smith_device_type_cpu(),
      ...
      auto offset = this->constant_offset(i) +
          (constants_internal_offset[i] / aoti_smith_dtype_element_size(dtype));
#else
      auto offset = this->constant_offset(i);
#endif
      ...
      AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_create_tensor_from_blob_v2(
          internal_ptr,
          ndim,
          size,
          stride,
