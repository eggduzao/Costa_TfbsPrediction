...
#define AOTI_SMITH_ERROR_CODE_CHECK(call)                                  \
  if ((call) != AOTI_SMITH_SUCCESS) {                                      \
...
inline void delete_record_function_object(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_record_function_end(
      reinterpret_cast<AtenRecordFunctionHandle>(ptr)));
}

inline void delete_tensor_object(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(
      aoti_smith_delete_tensor_object(reinterpret_cast<AtenTensorHandle>(ptr)));
}

inline void delete_c10_value_object(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_delete_c10_value_object(
...
class RAIIAtenRecordFunctionHandle {
 public:
  ...
  // Return a raw AtenRecordFunctionHandle to be used by aoti_smith functions
...
// RAIIAtenTensorHandle steals the tensor objects created by the libsmith C ABI
class RAIIAtenTensorHandle {
 public:
  ...
  // Return a raw AtenTensorHandle to be used by aoti_smith functions
  // Note: this function does NOT transfer the ownership of the handle
  ...
  int64_t size(int64_t d) {
    int64_t size = 0;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_size(handle_.get(), d, &size));
    return size;
  }
  ...
  int64_t stride(int64_t d) {
    int64_t stride = 0;
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_stride(handle_.get(), d, &stride));
    return stride;
  }
  ...
  int64_t storage_offset() {
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_storage_offset(handle_.get(), &storage_offset));
  ...
  void* data_ptr() const {
    void* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_data_ptr(handle_.get(), &result));
    return result;
  }

  int64_t* sizes() const {
    int64_t* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_sizes(handle_.get(), &result));
    return result;
  }

  int64_t* strides() const {
    int64_t* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_strides(handle_.get(), &result));
    return result;
  }
...
// RAIIC10IValueHandle steals the IValue objects created by the libsmith C ABI
class RAIIC10IValueHandle {
 public:
  ...
  // Return a raw C10IValueHandle to be used by aoti_smith functions
  // Note: this function does NOT transfer the ownership of the handle
...
class MaybeOwningAtenTensorHandle {
 public:
  ...
  MaybeOwningAtenTensorHandle(AtenTensorHandle handle, bool user_managed) {
    if (user_managed) {
      aoti_smith_new_tensor_handle(handle, &handle_);
    } else {
  ...
  int64_t size(int64_t d) {
    int64_t size = 0;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_size(handle_, d, &size));
    return size;
  }

  int64_t stride(int64_t d) {
    int64_t stride = 0;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_stride(handle_, d, &stride));
    return stride;
  }
  ...
  int64_t storage_offset() {
    int64_t storage_offset = 0;
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_storage_offset(handle_, &storage_offset));
    return storage_offset;
  }

  void* data_ptr() const {
    void* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_data_ptr(handle_, &result));
    return result;
  }

  int64_t* sizes() const {
    int64_t* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_sizes(handle_, &result));
    return result;
  }

  int64_t* strides() const {
    int64_t* result = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_strides(handle_, &result));
    return result;
  }
...
inline AtenTensorHandle reinterpret_tensor_wrapper(
  ...
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith__reinterpret_tensor(
...
inline void* get_data_ptr_wrapper(AtenTensorHandle tensor) {
  void* result = nullptr;
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_data_ptr(tensor, &result));
  return result;
}
...
class ConstantHandle {
 public:
  ...
  explicit ConstantHandle(AtenTensorHandle handle) : handle_(handle) {
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_data_ptr(handle_, &data_));
  }
...
#define CACHE_SMITH_DTYPE(typename) \
  static auto cached_smith_dtype_##typename = aoti_smith_dtype_##typename()

#define CACHE_SMITH_DEVICE(device)                \
  static auto cached_smith_device_type_##device = \
      aoti_smith_device_type_##device()

#define CACHE_SMITH_LAYOUT(layout) \
  static auto cached_smith_layout_##layout = aoti_smith_layout_##layout()

#define CACHE_SMITH_MEMORY_FORMAT(format)           \
  static auto cached_smith_memory_format_##format = \
      aoti_smith_memory_format_##format()
