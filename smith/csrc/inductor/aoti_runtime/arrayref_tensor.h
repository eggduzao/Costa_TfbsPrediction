...
#include <smith/csrc/inductor/aoti_runtime/mini_array_ref.h>
...
class ArrayRefTensor {
 public:
  ...
  AtenTensorHandle expensiveCopyToTensor() const {
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_empty_strided(
        sizes_.size(),
        sizes_.data(),
        strides_.data(),
        aoti_smith_dtype<std::remove_const_t<T>>(),
        device_type_,
        device_idx_,
        &result));
    void* dataPtr = nullptr;
    AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_data_ptr(result, &dataPtr));
