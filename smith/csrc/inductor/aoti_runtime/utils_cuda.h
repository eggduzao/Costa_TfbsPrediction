...
inline void delete_cuda_guard(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(
      aoti_smith_delete_cuda_guard(reinterpret_cast<CUDAGuardHandle>(ptr)));
}

inline void delete_cuda_stream_guard(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_delete_cuda_stream_guard(
...
class AOTICudaGuard {
 public:
  AOTICudaGuard(int32_t device_index) : guard_(nullptr, delete_cuda_guard) {
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_create_cuda_guard(device_index, &ptr));
    guard_.reset(ptr);
  }

  void set_index(int32_t device_index) {
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_cuda_guard_set_index(guard_.get(), device_index));
  }
...
class AOTICudaStreamGuard {
 public:
  AOTICudaStreamGuard(cudaStream_t stream, int32_t device_index)
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_create_cuda_stream_guard(stream, device_index, &ptr));
    guard_.reset(ptr);
  }
