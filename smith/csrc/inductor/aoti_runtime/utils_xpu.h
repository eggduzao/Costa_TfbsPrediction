...
inline void delete_xpu_guard(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(
      aoti_smith_delete_xpu_guard(reinterpret_cast<XPUGuardHandle>(ptr)));
}

inline void delete_xpu_stream_guard(void* ptr) {
  AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_delete_xpu_stream_guard(
...
class AOTIXpuGuard {
 public:
  AOTIXpuGuard(int32_t device_index) : guard_(nullptr, delete_xpu_guard) {
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_create_xpu_guard(device_index, &ptr));
    guard_.reset(ptr);
  }

  void set_index(int32_t device_index) {
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_xpu_guard_set_index(guard_.get(), device_index));
  }
...
class AOTIXpuStreamGuard {
 public:
  AOTIXpuStreamGuard(void* stream, int32_t device_index)
    ...
    AOTI_SMITH_ERROR_CODE_CHECK(
        aoti_smith_create_xpu_stream_guard(stream, device_index, &ptr));
    guard_.reset(ptr);
  }
