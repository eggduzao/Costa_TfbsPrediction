...
#include <smith/csrc/inductor/aoti_runtime/model.h>
 ...
 public:
  ...
  void update_constant_buffer(
    ...
    for (const auto& kv : constants_map) {
      ...
      aoti_smith_get_device_type(kv.second, &tensor_device_type);
    ...
    for (size_t idx = 0; idx < num_constants; idx++) {
      ...
      int64_t* stride;
      int64_t offset;
      aoti_smith_get_data_ptr(tensor, &user_constant_ptr);
      aoti_smith_get_storage_size(tensor, &constant_size);
      AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_strides(tensor, &stride));
      AOTI_SMITH_ERROR_CODE_CHECK(
          aoti_smith_get_storage_offset(tensor, &offset));
      ...
      offset = constants_internal_offset_[idx] /
          aoti_smith_dtype_element_size(dtype);
#elif USE_CUDA
      ...
      AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_create_tensor_from_blob(
          internal_constants_ptr,
