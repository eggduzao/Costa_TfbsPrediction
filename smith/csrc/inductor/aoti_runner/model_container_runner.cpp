...
AOTIModelContainerRunner::AOTIModelContainerRunner(
  ...
  SMITH_CHECK(model_so_, "Failed to load model: ", model_so_path);
...
#undef TRY_LOAD_SYMBOL
  ...
  if (c10::filesystem::exists(json_filename)) {
    proxy_executor_ = std::make_unique<smith::aot_inductor::OSSProxyExecutor>(
...
std::vector<at::Tensor> AOTIModelContainerRunner::run(
    ...
    void* stream_handle) {
  std::vector<AtenTensorHandle> input_handles =
      smith::aot_inductor::unsafe_alloc_new_handles_from_tensors(inputs);
...
void AOTIModelContainerRunner::update_constant_buffer_from_blob(
  ...
  SMITH_CHECK(fd >= 0, "Failed to open external weights file: " + weights_path);

  uint8_t* ptr = static_cast<uint8_t*>(
      mmap(NULL, weights_size, PROT_READ, MAP_PRIVATE, fd, 0));

  close(fd);
  SMITH_CHECK(ptr != MAP_FAILED, "mmap() failed");
#endif
  AOTI_RUNTIME_ERROR_CODE_CHECK(
