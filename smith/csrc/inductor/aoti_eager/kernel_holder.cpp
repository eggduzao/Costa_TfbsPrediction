...
#endif
#ifdef USE_XPU
#include <smith/csrc/inductor/aoti_runner/model_container_runner_xpu.h>
#endif
...
void AOTIPythonKernelHolder::cache_hit(
  ...
  smith::jit::drop(*stack, op.schema().arguments().size());
...
void AOTIPythonKernelHolder::init_aoti_kernel_cache() {
  ...
  py::handle load_aoti_eager_cache_function =
      py::module::import("smith._inductor.aoti_eager")
          .attr("load_aoti_eager_cache");
  SMITH_INTERNAL_ASSERT(
      load_aoti_eager_cache_function.ptr() != nullptr,
      "Failed to import - smith._inductor.aoti_eager.load_aoti_eager_cache");
  ...
  auto build_tensor_metadata = [](const py::dict& metadata) -> TensorMetadata {
    ...
    SMITH_INTERNAL_ASSERT(THPDtype_Check(data_type_obj.ptr()));
    auto data_type =
  ...
  SMITH_INTERNAL_ASSERT(py::isinstance<py::list>(result));
  auto kernel_info_list = result.cast<py::list>();
  for (auto kernel_info : kernel_info_list) {
    SMITH_INTERNAL_ASSERT(py::isinstance<py::dict>(kernel_info));
    ...
    for (auto item_metadata : inputs_metadata) {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(py::isinstance<py::dict>(item_metadata));
      auto metadata = item_metadata.cast<py::dict>();
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(metadata.contains("arg_order"));
      ...
      if (is_tensor_list) {
        ...
        SMITH_INTERNAL_ASSERT(THPDtype_Check(dtype_obj.ptr()));
        auto dtype_value =
        ...
        SMITH_INTERNAL_ASSERT(THPDtype_Check(dtype_value_obj.ptr()));
        auto dtype_value =
        ...
        SMITH_INTERNAL_ASSERT(THPLayout_Check(layout_value_obj.ptr()));
        auto layout_value =
...
void AOTIPythonKernelHolder::cache_miss(
  ...
  // output type.
  for (auto& output : outputs) {
    smith::jit::push(*stack, std::move(output));
  }
}
...
std::string AOTIPythonKernelHolder::produce_aoti_kernel_lib(
    ...
    const c10::DispatchKeySet& keyset,
    const smith::jit::Stack* stack) {
  auto arguments = smith::jit::last(*stack, op.schema().arguments().size());
  ...
  py::handle op_py_func = op.getPythonOp(pyinterpreter_, [&]() -> PyObject* {
    py::handle smith_api_function = py::module::import("smith")
                                        .attr("ops")
                                        .attr(ns_str.c_str())
  ...
  SMITH_INTERNAL_ASSERT(
      ...
      "Failed to import - smith._inductor.aoti_eager.aoti_compile_with_persistent_cache");
  ...
  auto result = py::reinterpret_steal<py::object>(PyObject_CallFunctionObjArgs(
      ...
      nullptr));
  SMITH_INTERNAL_ASSERT(result.ptr() != nullptr && result.ptr() != Py_None);
