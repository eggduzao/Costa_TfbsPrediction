...
#include <smith/csrc/inductor/aoti_eager/kernel_meta_info.h>
#include <iostream>
#include <utility>
...
void TensorMetadata::build_guard(const smith::dynamo::LocalState& local_state) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(
      !is_symbolic_, "Not support symbolic shape now");
  ...
  tensor_check_ = smith::dynamo::TensorCheck(
      local_state,
      nullptr,
      dispatch_key_set_,
      dtype_,
...
bool TensorMetadata::operator==(const TensorMetadata& other) const {
  ...
  if (tensor_check_.has_value()) {
    ...
    smith::dynamo::LocalState local_state;
...
bool ParameterMetadata::equal_to(const c10::Scalar& scalar) const {
  SMITH_INTERNAL_ASSERT(scalar.isFloatingPoint() || scalar.isIntegral(true));
  if (tag_ != SCALAR) {
    return false;
  }
