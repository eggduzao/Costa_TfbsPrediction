...
inline static PyObject* immutable_list_cls() {
  ...
  if (!immutable_list_cls) {
    immutable_list_cls =
        import_from("smith.fx.immutable_collections", "immutable_list");
  }
...
inline static PyObject* immutable_dict_cls() {
  ...
  if (!immutable_dict_cls) {
    immutable_dict_cls =
        import_from("smith.fx.immutable_collections", "immutable_dict");
  }
...
inline static bool is_node(PyObject* obj) {
  ...
  if (!node_cls) {
    node_cls = import_from("smith.fx.node", "Node");
  }
...
struct NodeBase {
  ...
  inline void set_prev(NodeBase* value) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(value);
