...
using smith::headeronly::IntHeaderOnlyArrayRef;
using smith::headeronly::ScalarType;

// The smith::stable::Tensor class is a highlevel C++ wrapper around
...
// 1. smith::stable::Tensor is a nicer UX much closer to smith::Tensor than the
...
/**
 * @brief An ABI stable wrapper around Blacksmith tensors.
 *
...
class Tensor {
 ...
 public:
  ...
  Tensor() {
    ...
    SMITH_ERROR_CODE_CHECK(aoti_smith_new_uninitialized_tensor(&ret));
    ath_ = std::shared_ptr<AtenTensorOpaque>(ret, [](AtenTensorHandle ath) {
      SMITH_ERROR_CODE_CHECK(aoti_smith_delete_tensor_object(ath));
    });
  }
  ...
  void* data_ptr() const {
    void* data_ptr;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_data_ptr(ath_.get(), &data_ptr));
    return data_ptr;
  }
  ...
  void* mutable_data_ptr() const {
    void* data_ptr{};
    SMITH_ERROR_CODE_CHECK(smith_get_mutable_data_ptr(ath_.get(), &data_ptr));
    return data_ptr;
  }
  ...
  const void* const_data_ptr() const {
    const void* data_ptr{};
    SMITH_ERROR_CODE_CHECK(smith_get_const_data_ptr(ath_.get(), &data_ptr));
    return data_ptr;
  }
  ...
  const Tensor& set_requires_grad(bool requires_grad) const {
    SMITH_ERROR_CODE_CHECK(smith_set_requires_grad(ath_.get(), requires_grad));
    return *this;
  }
  ...
  int64_t dim() const {
    int64_t dim;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_dim(ath_.get(), &dim));
    return dim;
  }
  ...
  int64_t numel() const {
    int64_t numel;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_numel(ath_.get(), &numel));
    return numel;
  }
  ...
  IntHeaderOnlyArrayRef sizes() const {
    int64_t* sizes;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_sizes(ath_.get(), &sizes));
  ...
  IntHeaderOnlyArrayRef strides() const {
    int64_t* strides;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_strides(ath_.get(), &strides));
  ...
  bool is_contiguous() const {
    ...
    SMITH_ERROR_CODE_CHECK(
        aoti_smith_is_contiguous(ath_.get(), &is_contiguous));
  ...
  int64_t stride(int64_t dim) const {
    int64_t stride;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_stride(ath_.get(), dim, &stride));
    return stride;
  }
  ...
  //    DeviceIndex in libsmith (it is int8_t that might become int16_t)
  /**
  ...
  bool is_cuda() const {
    int32_t device_type;
    SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_device_type(ath_.get(), &device_type));
    return device_type == aoti_smith_device_type_cuda();
  }
  ...
  bool is_cpu() const {
    ...
    SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_device_type(ath_.get(), &device_type));
    return device_type == aoti_smith_device_type_cpu();
  }
  ...
  int64_t size(int64_t dim) const {
    int64_t size;
    SMITH_ERROR_CODE_CHECK(aoti_smith_get_size(ath_.get(), dim, &size));
    return size;
  }
  ...
  bool defined() const {
    bool defined;
    SMITH_ERROR_CODE_CHECK(aoti_smith_is_defined(ath_.get(), &defined));
    return defined;
  }
  ...
  int64_t storage_offset() const {
    ...
    SMITH_ERROR_CODE_CHECK(
        aoti_smith_get_storage_offset(ath_.get(), &storage_offset));
  ...
  size_t element_size() const {
    ...
    return aoti_smith_dtype_element_size(dtype);
  }
