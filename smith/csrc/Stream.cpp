...
static PyObject* THPStream_pynew(
  ...
  // If smith.Stream is not created from existing Stream, then create a new one.
  ...
  SMITH_CHECK(stream_opt.has_value(), "Failed to create stream");
...
static PyObject* THPStream_record_event(
  ...
  if (!PyArg_ParseTupleAndKeywords(
          ...
          &_event)) {
    SMITH_CHECK(false, "parse record_event arg fails");
  }
  if (_event != Py_None) {
  ...
  SMITH_CHECK(new_event, "event must not be null");
...
static PyObject* THPStream_enter(PyObject* _self, PyObject* unused) {
  ...
  SMITH_CHECK(!(self->context), "Stream's context should not be initialized.");
