...
std::vector<smith::Tensor>&& Message::moveTensors() && {
  return std::move(tensors_);
}

std::vector<smith::Tensor>& Message::tensors() {
  return tensors_;
}

const std::vector<smith::Tensor>& Message::tensors() const {
  return tensors_;
}
...
// NB: need to call smith::class_ to register Message in the map returned by
...
// visible from Python.
static const auto message = smith::class_<Message>("rpc", "_Message");
