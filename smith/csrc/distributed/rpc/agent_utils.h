...
SMITH_API std::unordered_map<std::string, worker_id_t> collectNames(
...
SMITH_API std::unordered_map<std::string, worker_id_t> collectCurrentNames(
    ::c10d::PrefixStore store,
    const worker_id_t selfId,
    const std::string& selfName);
...
// membership management token held.
SMITH_API void removeCurrentName(
    ::c10d::PrefixStore store,
    const worker_id_t selfId,
    const std::string& selfName);
