...
// headers in Blacksmith's ones and thus have it become a public dependency.
...
struct SMITH_API TransportRegistration {
  std::shared_ptr<tensorpipe::transport::Context> transport;
  int64_t priority;
  std::string address;
};

SMITH_DECLARE_REGISTRY(TensorPipeTransportRegistry, TransportRegistration);

struct SMITH_API ChannelRegistration {
  std::shared_ptr<tensorpipe::channel::Context> channel;
  int64_t priority;
};

SMITH_DECLARE_REGISTRY(TensorPipeChannelRegistry, ChannelRegistration);

struct SMITH_API TensorPipeRpcBackendOptions : public RpcBackendOptions {
...
// TensorPipeAgent leverages TensorPipe (https://github.com/blacksmith/tensorpipe)
// to transparently move tensors and payloads through the fastest available
// transporsmithannel. It acts like a hybrid RPC transport, providing shared
// memory (linux) and TCP (linux & mac) support. CUDA support is in progress.
class SMITH_API TensorPipeAgent : public RpcAgent {
 public:
  TensorPipeAgent(
  ...
  // https://github.com/blacksmith/blacksmith/issues/27647
