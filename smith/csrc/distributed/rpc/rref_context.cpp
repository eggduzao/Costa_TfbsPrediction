...
void confirmPendingUser(
  ...
  if (!jitFuture.hasError()) {
    ...
    SMITH_INTERNAL_ASSERT(rr.forkId() == expectedForkId);
  } else {
...
c10::intrusive_ptr<UserRRef> RRefContext::createUserRRef(
    worker_id_t ownerId,
    const TypePtr& type) {
  SMITH_CHECK(ownerId != getWorkerId(), "Cannot create UserRRef on owner.");
...
c10::intrusive_ptr<UserRRef> RRefContext::createUserRRef(
    ...
    const ForkId& forkId,
    const TypePtr& type) {
  SMITH_CHECK(ownerId != getWorkerId(), "RRef owner cannot create user RRef.");
...
c10::intrusive_ptr<RRef> RRefContext::delForkOfOwner(
  ...
  // There were previously multiple SMITH_CHECKs in this function that checked
