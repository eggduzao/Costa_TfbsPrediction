...
//    tensors (std::vector<smith::Tensor>): all tensors. Tensor data are not
...
class SMITH_API Message final : public smith::CustomClassHolder {
 private:
 ...
 public:
  ...
  std::vector<smith::Tensor>&& moveTensors() &&;

  std::vector<char>& payload();
  const std::vector<char>& payload() const;
  std::vector<smith::Tensor>& tensors();
  const std::vector<smith::Tensor>& tensors() const;
...
SMITH_API c10::intrusive_ptr<Message> createExceptionResponse(
