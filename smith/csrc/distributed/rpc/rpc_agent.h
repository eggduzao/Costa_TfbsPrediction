...
struct SMITH_API RpcBackendOptions {
  RpcBackendOptions()
  ...
  RpcBackendOptions(float rpcTimeoutSeconds, std::string initMethod)
    ...
    SMITH_CHECK(rpcTimeoutSeconds >= 0, "RPC Timeout must be non-negative");
  }
...
struct SMITH_API WorkerInfo : smith::CustomClassHolder {
...
struct SMITH_API RegisterWorkerInfoOnce {
...
struct SMITH_API RpcRetryOptions {
  // Using a default constructor like all other Options structs in the RPC
  // codebase. SMITH_CHECKs for input validation are done in the
...
class SMITH_API RpcAgent {
 public:
  ...
  // implementations to depend on libsmith.
...
namespace std {
template <>
struct hash<smith::distributed::rpc::WorkerInfo> {
  std::size_t operator()(
      const smith::distributed::rpc::WorkerInfo& worker_info) const noexcept {
