...
#include <smith/csrc/distributed/autograd/rpc_messages/cleanup_autograd_context_req.h>
#include <smith/csrc/distributed/autograd/rpc_messages/cleanup_autograd_context_resp.h>
#include <smith/csrc/distributed/autograd/rpc_messages/propagate_gradients_req.h>
#include <smith/csrc/distributed/autograd/rpc_messages/propagate_gradients_resp.h>
#include <smith/csrc/distributed/autograd/rpc_messages/rpc_with_autograd.h>
#include <smith/csrc/distributed/autograd/rpc_messages/rpc_with_profiling_req.h>
#include <smith/csrc/distributed/autograd/rpc_messages/rpc_with_profiling_resp.h>
#include <smith/csrc/distributed/autograd/rpc_messages/rref_backward_req.h>
...
#include <smith/csrc/distributed/rpc/python_call.h>
#include <smith/csrc/distributed/rpc/python_remote_call.h>
#include <smith/csrc/distributed/rpc/python_resp.h>
#include <smith/csrc/distributed/rpc/rref_proto.h>
...
RPCErrorType getRPCErrorType(const JitFuture& jitFuture) {
  ...
  if (pos != std::string::npos) {
    ...
    auto errStartIdx =
        pos + smith::distributed::rpc::kRPCErrorPrefix.size() + 1;
...
std::string makeRPCError(
  ...
  return fmt::format(
      "{}:{}:{}",
      smith::distributed::rpc::kRPCErrorPrefix,
...
parseWireSections(const void* data, size_t data_size) {
  ...
  if (!ok) {
    SMITH_CHECK(false, "failed parse");
  }
  ...
  if (ptr != endp) {
    SMITH_CHECK(false, "failed bounds");
  }
  return out;
}
...
std::string wireSerialize(
  ...
  if (!tensors.empty()) {
    smith::jit::Pickler pickler([&](const void* buf, size_t sz) -> size_t {
...
std::pair<std::vector<char>, std::vector<at::Tensor>> wireDeserialize(
  ...
  if (metaIt != sections.end()) {
    ...
    auto sectionReadFunc = [&](const std::string& ename) -> at::DataPtr {
      ...
      if (it == sections.end()) {
        SMITH_CHECK(false, "Couldn't find entity " + ename);
      }
...
std::vector<at::IValue> readWrappedPayload(
  ...
  SMITH_INTERNAL_ASSERT(payload.size() >= sizeof(int64_t));
  size_t indexToRead = payload.size() - sizeof(int64_t);
  int64_t additionalPayloadSize = 0;
  smith::utils::THP_decodeBuffer(
      &additionalPayloadSize,
      ...
      if (e.hasCuda()) {
        if (e.kind() == EventKind::PopRange) {
          ...
          if (it != startEvents.end()) {
            ...
            SMITH_WARN("Found a pop event without a corresponding push event");
            e.setCudaUs(0);
          }
        } else {
          e.setCudaUs(0);
