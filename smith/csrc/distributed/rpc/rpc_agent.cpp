...
RegisterWorkerInfoOnce::RegisterWorkerInfoOnce() {
  ...
  // happens in libsmith_python we wrap the class registration in a helper to
  // make sure that if there's multiple copies of Python such as used in
  // smith::deploy we only ever register it once.
  static auto workerInfo = smith::class_<WorkerInfo>("dist_rpc", "WorkerInfo")
                               .def(smith::init<std::string, int64_t>());
}
...
c10::intrusive_ptr<JitFuture> RpcAgent::sendWithRetries(
  ...
  SMITH_CHECK(retryOptions.maxRetries >= 0, "maxRetries cannot be negative.");
  SMITH_CHECK(
...
std::shared_ptr<TypeResolver> RpcAgent::getTypeResolver() {
  SMITH_INTERNAL_ASSERT(typeResolver_, "Type resolver is not set!");
