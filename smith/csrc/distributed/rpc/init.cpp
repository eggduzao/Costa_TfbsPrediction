...
#include <smith/csrc/distributed/rpc/profiler/server_process_global_profiler.h>
#include <smith/csrc/distributed/rpc/py_rref.h>
#include <smith/csrc/distributed/rpc/python_functions.h>
#include <smith/csrc/distributed/rpc/python_rpc_handler.h>
#include <smith/csrc/distributed/rpc/request_callback_impl.h>
...
#include <smith/csrc/distributed/rpc/tensorpipe_agent.h>
...
PyObject* rpc_init(PyObject* _unused, PyObject* noargs) {
  HANDLE_TH_ERRORS
  auto rpc_module =
      THPObjectPtr(PyImport_ImportModule("smith.distributed.rpc"));
  if (!rpc_module) {
    throw python_error();
  }
  ...
  auto m =
      smith_C_m.def_submodule("_distributed_rpc", "distributed rpc bindings");
  ...
  auto rpcBackendOptions =
      shared_ptr_class_<RpcBackendOptions>(
          ...
          R"(An abstract structure encapsulating the options passed into the RPC
            ...
            :meth:`~smith.distributed.rpc.init_rpc` in order to initialize RPC
  ...
  auto workerInfo =
      shared_ptr_class_<WorkerInfo>(
          ...
          R"(A structure that encapsulates information of a worker in the system.
            ...
            through :meth:`~smith.distributed.rpc.get_worker_info` and the
            result can be passed in to functions such as
            :meth:`~smith.distributed.rpc.rpc_sync`, :meth:`~smith.distributed.rpc.rpc_async`,
            :meth:`~smith.distributed.rpc.remote` to avoid copying a string on
            every invocation.)")
          .def(
          ...
          .def(py::pickle(
              ...
              /* __setstate__ */
              [](const py::tuple& t) {
                SMITH_CHECK(t.size() == 2, "Invalid WorkerInfo state.");
  ...
  auto pyRRef =
      shared_ptr_class_<PyRRef>(module, "PyRRef", R"(
          ...
          .. warning::
              ...
              pickle, smith :meth:`~smith.save` / :meth:`~smith.load`,
              JIT :meth:`~smith.jit.save` / :meth:`~smith.jit.load`, etc.) will
              lead to errors.

          Args:
              value (object): The value to be wrapped by this RRef.
              type_hint (Type, optional): Python type that should be passed to
                  ``SmithScript`` compiler as type hint for ``value``.
          ...
          Example::
              ...
              >>> rref = rpc.remote("worker1", smith.add, args=(smith.ones(2), 3))
          ...
          .def(
              ...
              R"(
                  ...
                  Example::
                      ...
                      >>> rref = rpc.remote("worker1", smith.add, args=(smith.zeros(2, 2), 1))
                      >>> rref.rpc_sync().size()  # returns smith.Size([2, 2])
          ...
          .def(
              ...
              R"(
                  ...
                  Example::
                      >>> from smith.distributed import rpc
                      >>> rref = rpc.remote("worker1", smith.add, args=(smith.zeros(2, 2), 1))
                      >>> rref.rpc_async().size().wait()  # returns smith.Size([2, 2])
              ...
              R"(
                  ...
                  Args:
                      timeout (float, optional): Timeout for ``rref.remote()``. If
                          the creation of this :class:`~smith.distributed.rpc.RRef`
                          ...
                          :class:`~smith.distributed.rpc.RRef`.

                  Example::
                      >>> from smith.distributed import rpc
                      >>> rref = rpc.remote("worker1", smith.add, args=(smith.zeros(2, 2), 1))
                      >>> rref.remote().size().to_here()  # returns smith.Size([2, 2])
          ...
          .def(
              ...
              R"(
                  ...
                  Args:
                    rref (smith.distributed.rpc.RRef): The RRef to get type of.
          ...
          .def(
              ...
              R"(
                  ...
                  :meth:`~smith.distributed.autograd.get_gradients` should be
  ...
  // Base class: smith.distributed.rpc.RpcBackendOptions.
  py::class_<TensorPipeRpcBackendOptions>(
      ...
      .def_readwrite(
          ...
          R"(
              ...
              :class:`~smith.distributed.rpc.TensorPipeAgent` to execute
              requests.
          )")
      .def_readwrite(
          "device_maps",
  ...
  module.def(
      ...
      [](const std::string& dstWorkerName,
         ...
         const float rpcTimeoutSeconds,
         const bool isAsyncExecution) {
        return std::make_shared<jit::PythonFutureWrapper>(pyRpcSmithscript(
            dstWorkerName,
            qualifiedNameStr,
            argsTuple,
            kwargsDict,
  ...
  module.def(
      ...
      R"(
          ...
          :meth:`~smith.distributed.rpc.rpc_async`.
  ...
  module.def(
      ...
      &enableJitRRefPickle,
      R"(
        Allows ``smith.jit.save`` to save a ``smith.jit.ScriptModule`` with
