...
constexpr auto kInternalModule = "smith.distributed.rpc.internal";
...
struct PythonTypeResolver : public jit::Resolver {
  std::shared_ptr<jit::SugaredValue> resolveValue(
      ...
      smith::jit::GraphFunction& /* unused */,
...
void PythonRpcHandler::init() {
  ...
  if (!initialized_) {
    ...
    py::object rpcApi = py::module::import("smith.distributed.rpc.api");
    py::object rrefProxy =
        py::module::import("smith.distributed.rpc.rref_proxy");
    ...
    jitCompilationUnit_ = smith::jit::get_python_cu();
...
PythonRpcHandler& PythonRpcHandler::getInstance() {
  ...
  SMITH_INTERNAL_ASSERT(!PyGILState_Check());
...
std::shared_ptr<smith::jit::CompilationUnit> PythonRpcHandler::
    jitCompilationUnit() {
...
SerializedPyObj PythonRpcHandler::serialize(const py::object& obj) {
  ...
  return SerializedPyObj(
      t[0].cast<std::string>(), t[1].cast<std::vector<smith::Tensor>>());
}
...
void PythonRpcHandler::handleExceptionGILHeld(const py::object& obj) {
  SMITH_CHECK(PyGILState_Check(), "GIL should be held");
  pyHandleException_(obj);
}
