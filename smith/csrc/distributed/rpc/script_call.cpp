...
const std::string ScriptCall::BUILTIN_OP_NAMESPACE_("smith.ops.aten.");
...
void ScriptCall::toIValues(std::vector<at::IValue>& ivalues) const {
  ...
  if (op_.has_value()) {
    ...
    SMITH_CHECK(
        !hasOp(),
        "It is SmithScript function call, operator should not be set.");
    ivalues.emplace_back(qualifiedName().qualifiedName());
  } else {
    SMITH_INTERNAL_ASSERT(
        false,
        "Either builtin operator or SmithScript function name should be set.");
  }
}
...
std::unique_ptr<ScriptCall> ScriptCall::fromIValues(
  ...
  // builtin operator and SmithScript function
  // If the qualifiedName is not a builtin operator name, then treat it
  // as SmithScript function name
...
std::shared_ptr<Operator> ScriptCall::matchOperator(
  ...
  auto schema = smith::jit::parseSchema(str_schema);
  auto symbol = at::Symbol::fromQualString(schema.name());

  for (auto op : smith::jit::getAllOperatorsFor(symbol)) {
    if (toString(op->schema()) == str_schema) {
      return op;
    }
  }

  SMITH_CHECK(false, "Cannot find matching operator for schema ", str_schema);
}
