...
IValue toPyIValue(const Message& message) {
  ...
  switch (msgType) {
    case MessageType::SCRIPT_RET: {
      ...
      return jit::toIValue(
          smith::jit::createPyObjectForStack(std::move(stack)),
          PyObjectType::get());
    }
    ...
    default: {
      SMITH_CHECK(false, "Unrecognized response message type ", msgType);
    }
  }
}
...
std::shared_ptr<Operator> matchBuiltinOp(
  ...
  if (symbol.is_aten()) {
    ...
    // small (i.e. it is 10 for smith.add) so a worst-case linear search should
    // not incur significant extra overhead.
    auto ops = smith::jit::getAllOperatorsFor(symbol);
    std::vector<std::shared_ptr<smith::jit::Operator>> c10OpsForSymbol;
    ...
    std::pair<std::shared_ptr<smith::jit::Operator>, smith::jit::Stack>
        opWithStack;
    try {
      opWithStack = smith::jit::getOpWithStack(c10OpsForSymbol, args, kwargs);
    } catch (const std::runtime_error&) {
      opWithStack = smith::jit::getOpWithStack(ops, args, kwargs);
    }
...
c10::intrusive_ptr<JitFuture> sendPythonRemoteCall(
  ...
  return smith::distributed::autograd::sendMessageWithAutograd(
      *agent,
      dst,
...
c10::intrusive_ptr<JitFuture> pyRpcSmithscript(
  ...
  Stack stack;
  {
    ...
    py::gil_scoped_acquire acquire;
    stack = smith::jit::createStackForSchema(
        functionSchema,
  ...
  DCHECK(!PyGILState_Check());
  c10::intrusive_ptr<c10::ivalue::Future> fut = rpcSmithscript(
      dstWorkerName,
      qualifiedName,
      functionSchema,
      std::move(stack),
...
PyRRef pyRemoteSmithscript(
  ...
  DCHECK(!PyGILState_Check());
  auto rrefPtr = remoteSmithscript(
      dstWorkerName,
      qualifiedName,
      functionSchema,
      stack,
