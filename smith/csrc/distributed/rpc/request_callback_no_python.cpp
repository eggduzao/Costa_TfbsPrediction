#include <smith/csrc/distributed/rpc/request_callback_no_python.h>
...
c10::intrusive_ptr<JitFuture> RequestCallbackNoPython::retrieveOwnerRRef(
  ...
  SMITH_INTERNAL_ASSERT(type->kind() == at::RRefType::Kind);
...
void RequestCallbackNoPython::handleRRefDelete(
  ...
  SMITH_CHECK(!rref->isPyObj(), "RRefs with python objects not supported!");
}
...
c10::intrusive_ptr<JitFuture> RequestCallbackNoPython::
  ...
  auto responseFuture = wrappedRpcResponseFuture->then(
      [fromWorkerId, ctxId = autogradContext->contextId()](
        ...
        // NB: The execution of SmithScript functions can also run on a
        // different thread, which is addressed by
        // https://github.com/blacksmith/blacksmith/pull/36395
        ...
        // https://github.com/blacksmith/blacksmith/issues/38510
...
c10::intrusive_ptr<JitFuture> RequestCallbackNoPython::
      ...
      true /* cleanup TLS state */, false /* consolidate events */);
  {
    ...
    auto wrappedRpcResponseFuture = processRpc(
        ...
        wrappedMsgType,
        {}); // TODO: https://github.com/blacksmith/blacksmith/issues/55757
...
c10::intrusive_ptr<JitFuture> RequestCallbackNoPython::runJitOperator(
  ...
  SMITH_INTERNAL_ASSERT(
      stack.size() == 1,
      "Return value of a builtin operator or a SmithScript function should be "
