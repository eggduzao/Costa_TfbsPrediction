...
PyRRef::PyRRef(c10::intrusive_ptr<RRef> rref)
  ...
  SMITH_CHECK(rref_, "PyRRef must not wrap nullptr");
  C10_LOG_API_USAGE_ONCE("smith.distributed.rref");
}
...
c10::intrusive_ptr<JitFuture> PyRRef::getProfilingFuture() const {
  SMITH_INTERNAL_ASSERT(profilingFuture_, "Profiling future has not been set!");
...
py::object PyRRef::toHere(const float timeoutSeconds) const {
  C10_LOG_API_USAGE_ONCE("smith.distributed.rref.to_here");
  if (rref_->isOwner()) {
    return localValue();
  } else {
    ...
    if (rref_->isPyObj()) {
      ...
      // acquiring GIL as smith::jit::toPyObject creates new py::object
      // without grabbing the GIL.
      pybind11::gil_scoped_acquire ag;
      return smith::jit::toPyObject(std::move(value));
    }
  }
}
...
py::object PyRRef::localValue() const {
  ...
  {
    // acquiring GIL as smith::jit::toPyObject creates new py::object without
    // grabbing the GIL.
    pybind11::gil_scoped_acquire ag;
    res = smith::jit::toPyObject(std::move(value));
...
py::object PyRRef::createRRefProxy(
  ...
  switch (type) {
    ...
    default: {
      SMITH_INTERNAL_ASSERT(false, "Unrecognized RRefProxy type ", type);
    }
  }
}
...
void PyRRef::backwardOwnerRRef(
  ...
  if (value.isPyObject()) {
    ...
    py::object obj = smith::jit::toPyObject(value);
    try {
      value = smith::jit::toIValue(obj, c10::TensorType::get());
    } catch (py::cast_error&) {
      SMITH_CHECK(false, "RRef should contain a tensor for .backward()");
    }
  }

  SMITH_CHECK(value.isTensor(), "RRef should contain a tensor for .backward()");
  auto root = value.toTensor();

  if (autogradContextId == -1) {
    smith::autograd::backward({root});
  } else {
