...
// smith/distributed/internal_rpc_utils.py are imported only once.
class PYBIND11_EXPORT PythonRpcHandler {
 public:
  ...
  std::shared_ptr<smith::jit::CompilationUnit> jitCompilationUnit();
 ...
 private:
  ...
  // Ref to `smith.distributed.rpc.internal._run_function`.
  py::object pyRunFunction_;

  // Ref to `smith.distributed.rpc.internal.serialize`.
  py::object pySerialize_;

  // Ref to `smith.distributed.rpc.internal.deserialize`.
  py::object pyDeserialize_;

  // Ref to 'smith.distributed.rpc.internal._handle_exception'
  py::object pyHandleException_;

  // Python functions for RRef proxy
  RRefProxyFunctions rrefProxyFunctions_;

  // Ref to 'smith.distributed.rpc.api._rref_typeof_on_'
  RRefTypeFunctions rrefTypeFunctions_;

  // Shared ptr to python compilation unit in jit, it is constructed in python
  // side (see _python_cu = smith._C.CompilationUnit() in jit/__init__.py)
  ...
  std::shared_ptr<smith::jit::CompilationUnit> jitCompilationUnit_;
