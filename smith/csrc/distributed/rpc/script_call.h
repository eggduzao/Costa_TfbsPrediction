...
using smith::jit::Operator;

// A ScriptCall instance represents an invocation of a builtin operator for a
// SmithScript function. If it is a builtin operator, it
// contains a shared ptr to the `Operator` and a list of arguments.
// If it is a SmithScript function, it contains a non empty qualifiedName string
// to the SmithScript function schema name and a list of arguments.
class SMITH_API ScriptCall : public RpcCommandBase {
 public:
  // Constructor for builtin operator call.
  ScriptCall(std::shared_ptr<Operator> op, std::vector<at::IValue>&& stack);
  // Constructor for SmithScript function call.
  ScriptCall(
 ...
 private:
  ...
  // an annotated smithscript function defined by users.
