...
struct SMITH_API _SupplementBase : smith::CustomClassHolder {
...
struct SMITH_API ReduceOp : smith::CustomClassHolder {
  ...
  ReduceOp(RedOpType op) : op_(op) {
    SMITH_INTERNAL_ASSERT(
        op_ != PREMUL_SUM,
        "Use `smith.distributed._make_nccl_premul_sum` to create an instance of ReduceOp with PREMUL_SUM");
  }
  ...
  bool operator==(const std::uint8_t other) {
    SMITH_INTERNAL_ASSERT(other < 9, "Invalid other op value");
    return other == op_;
  }
...
SMITH_API bool isComplexViewAsRealAllowed(const ReduceOp& reduceOp);
