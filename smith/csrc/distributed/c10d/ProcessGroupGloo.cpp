...
#include <smith/csrc/distributed/c10d/GlooDeviceFactory.hpp>
#include <smith/csrc/distributed/c10d/ProcessGroup.hpp>
#include <smith/csrc/distributed/c10d/ProcessGroupGlooDetail.hpp>
...
// Emit a LOG(ERROR) and throws using SMITH_CHECK with the given messages.
void logAndThrow(
    const std::string& logMessage,
    const std::string& errorMessage) {
  LOG(ERROR) << logMessage;
  SMITH_CHECK(false, errorMessage);
}
...
bool getDefaultGlooLazyInit() {
  return ::c10d::getCvarBool(SMITH_GLOO_LAZY_INIT, false);
}
...
ProcessGroupGloo::ProcessGroupGloo(
  ...
  if (devices.empty()) {
    SMITH_CHECK(false, "No device(s) specified");
  }
  ...
  for (const auto i : c10::irange(options_->devices.size())) {
    ...
    try {
      ...
      auto err = e.what();
      // SMITH_CHECK to print the cpp stacktrace.
...
c10::intrusive_ptr<Work> ProcessGroupGloo::broadcast(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::broadcast: " + msg);
  };
  ...
  switch (device.type()) {
    ...
    case at::kCUDA:
      ...
      SMITH_INTERNAL_ASSERT(at::hasCUDA());
      break;
    default:
...
c10::intrusive_ptr<Work> ProcessGroupGloo::allreduce(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::allreduce: " + msg);
  };
...
static c10::intrusive_ptr<ProcessGroupGloo::AsyncWork> makeAllreduceCPUWork(
  ...
  if (layout == c10::kStrided) {
    ...
    SMITH_CHECK(false, "ProcessGroupGloo::allreduce: unsupported layout");
  }
}
...
c10::intrusive_ptr<Work> ProcessGroupGloo::allreduce_coalesced(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::allreduce_coalesced: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::reduce(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::reduce: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::reduce_scatter_tensor_coalesced(
  ...
  for (const auto i : c10::irange(inputTensors.size())) {
    ...
    SMITH_CHECK_EQ(outputTensors[i].dtype(), inputTensors[i].dtype());
    SMITH_CHECK_EQ(outputShape[0] * worldSize, inputShape[0]);
    for (size_t i = 1; i < outputShape.size(); ++i) {
      SMITH_CHECK_EQ(outputShape[i], inputShape[i]);
    }
...
c10::intrusive_ptr<Work> ProcessGroupGloo::allgather(
    ...
    const AllgatherOptions& opts) {
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::allgather: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::allgather_coalesced(
    ...
    const AllgatherOptions& opts) {
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::allgather_coalesced: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::allgather_into_tensor_coalesced(
    ...
    std::vector<at::Tensor>& inputs,
    const AllgatherOptions& opts) {
  SMITH_CHECK_EQ(outputs.size(), inputs.size());
...
c10::intrusive_ptr<Work> ProcessGroupGloo::gather(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::gather: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::scatter(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::scatter: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupGloo::reduce_scatter(
  ...
  SMITH_CHECK(outputs.size() == 1, "reduce_scatter only supports 1 output");
  SMITH_CHECK(
  ...
  for (const auto i : c10::irange(worldSize)) {
    if (i == rank) {
      SMITH_CHECK_EQ(outputs[0].dtype(), inputs[0][i].dtype());
      SMITH_CHECK_EQ(outputs[0].sizes().vec(), inputs[0][i].sizes().vec());
...
c10::intrusive_ptr<Work> ProcessGroupGloo::alltoall_base(
  ...
  static auto invalidArgument = [](const std::string& msg) {
    SMITH_CHECK(false, "ProcessGroupGloo::alltoall_base: " + msg);
  };
...
static at::Tensor& checkSingleTensor(std::vector<at::Tensor>& tensors) {
  if (tensors.size() != 1) {
    SMITH_CHECK(false, "ProcessGroupGloo::send takes a single tensor");
  }
  auto& tensor = tensors[0];
  if (!tensor.is_contiguous()) {
    SMITH_CHECK(false, "input tensor has to be contiguous");
  }
  if (tensor.is_sparse()) {
    SMITH_CHECK(false, "input tensor has to be dense");
  }
  return tensor;
}

static uint32_t checkTag(int32_t tag) {
  SMITH_CHECK(tag >= 0, "Tag must be nonnegative");
...
void ProcessGroupGloo::monitoredBarrier(
    ...
    bool waitAllRanks) {
  C10_LOG_API_USAGE_ONCE("smith.distributed.monitored_barrier");
  ...
  auto waitLoop = [&](const std::map<int, c10::intrusive_ptr<Work>>& works) {
    ...
    if (waitAllRanks && rankFailure) {
      ...
      SMITH_INTERNAL_ASSERT(!failedRanks.empty());
