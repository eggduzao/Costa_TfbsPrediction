...
void Logger::log_if_graph_static(bool is_static) {
  static bool log_graph_static_flag [[maybe_unused]] = [this, is_static]() {
    ...
    at::LogBlacksmithDDPUsage(*ddp_logging_data_);
    return true;
  }();
}
...
void Logger::set_env_variables() {
  ...
  ddp_logging_data_->strs_map["smith_distributed_debug"] =
      getCvarString({"SMITH_DISTRIBUTED_DEBUG"}, "N/A");
  ...
  if (reducer_->process_group_->getBackendName() == "nccl") {
    ...
    ddp_logging_data_->strs_map["nccl_blocking_wait"] =
        getCvarString(SMITH_NCCL_BLOCKING_WAIT, "N/A");
    ddp_logging_data_->strs_map["nccl_async_error_handling"] =
        getCvarString(SMITH_NCCL_ASYNC_ERROR_HANDLING, "N/A");
...
void Logger::calculate_avg_time(
  ...
  SMITH_CHECK(num_iterations_stats_recorded_ > 0);
...
void Logger::set_runtime_stats_and_log() {
  ...
  SMITH_INTERNAL_ASSERT(reducer_->timer_);
  calculate_avg_time(
  ...
  // Log runtime stats to stderr if SMITH_DISTRIBUTED_DEBUG=DETAIL is enabled.
