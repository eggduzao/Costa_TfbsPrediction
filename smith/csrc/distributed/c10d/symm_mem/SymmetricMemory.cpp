 ...
 public:
  ...
  void set_backend(const std::string& name) {
    ...
    if (existing != map_.end()) {
      ...
      SMITH_CHECK(!in_use_, "Backend can not be changed after use.");
    }
...
void set_group_info(
  ...
  SMITH_CHECK(group_info_map.find(group_name) == group_info_map.end());
...
SMITH_LIBRARY_FRAGMENT(symm_mem, m) {
  m.def(
...
SMITH_LIBRARY_IMPL(symm_mem, Meta, m) {
