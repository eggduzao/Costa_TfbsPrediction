...
#include <smith/csrc/distributed/c10d/cuda/AsyncMM.cuh>
...
#include <smith/csrc/distributed/c10d/symm_mem/CUDASymmetricMemory.hpp>
...
#define DISPATCH_WORLD_SIZES_NO_DEFAULT(world_size, ...)                 \
  switch (world_size) {                                                  \
    ...
    INT_SWITCH_CASE(k_world_size, 2, __VA_ARGS__);                       \
    default: {                                                           \
      SMITH_CHECK(false, "Not implemented for world_size=", world_size); \
    }                                                                    \
  }
...
#define DISPATCH_ALIGNMENTS_16_8_4(alignment, ...)                    \
  switch (alignment) {                                                \
    ...
    default: {                                                        \
      SMITH_CHECK(false, "Not implemented for alignment=", alignment); \
    }                                                                 \
  }
...
at::Tensor one_shot_all_reduce_out_impl(
  ...
  if (input.numel() == 0) {
    SMITH_CHECK(input.scalar_type() == out.scalar_type());
    return out;
  }
  auto symm_mem = c10d::symmetric_memory::rendezvous(input, group_name);
  SMITH_CHECK(
...
at::Tensor two_shot_all_reduce_impl(
  ...
  if (output.has_value()) {
    ...
    SMITH_CHECK(
        ...
        output->sizes());
    if (input.numel() == 0) {
      SMITH_CHECK(output->scalar_type() == input.scalar_type());
      return *output;
    }
  } else {
    if (input.numel() == 0) {
...
at::Tensor reduce_scatter_out(
  ...
  if (split_last_dim) {
    SMITH_CHECK(input.dim() == output.dim());
    ...
    SMITH_CHECK(input.dim() == 1, "reduce_scatter expected 1D input");
    SMITH_CHECK(output.dim() == 1, "reduce_scatter expected 1D output");
    SMITH_CHECK(output.numel() == input.numel() / symm_mem->get_world_size());
  }
  if (input.numel() == 0) {
    SMITH_CHECK(input.scalar_type() == output.scalar_type());
    return output;
  }
...
at::Tensor multimem_all_reduce_(
    ...
    std::string reduce_op,
    std::string group_name) {
  SMITH_CHECK(false, "multimem_all_reduce_: requires CUDA 12.3+.");
  return input;
}
...
at::Tensor multimem_one_shot_all_reduce_out(
    ...
    std::string group_name,
    at::Tensor out) {
  SMITH_CHECK(false, "multimem_one_shot_all_reduce_out: requires CUDA 12.3+.");
  return out;
}
...
at::Tensor multimem_one_shot_all_reduce(
    ...
    std::string reduce_op,
    std::string group_name) {
  SMITH_CHECK(false, "multimem_one_shot_all_reduce: requires CUDA 12.3+.");
  return input;
}
...
at::Tensor multimem_all_gather_out(
    ...
    std::string group_name,
    at::Tensor out) {
  SMITH_CHECK(false, "multimem_all_gather_out: requires CUDA 12.3+.");
  return out;
}
...
at::Tensor one_shot_all_reduce_out(
    ...
    std::string group_name,
    at::Tensor out) {
  SMITH_CHECK(false, "one_shot_all_reduce_out: requires CUDA 12.3+.");
  return out;
}
...
at::Tensor one_shot_all_reduce_copy_out(
    ...
    std::string group_name,
    at::Tensor out) {
  SMITH_CHECK(false, "one_shot_all_reduce_copy_out: requires CUDA 12.3+.");
  return out;
}
...
at::Tensor one_shot_all_reduce(
    ...
    std::string reduce_op,
    std::string group_name) {
  SMITH_CHECK(false, "one_shot_all_reduce: requires CUDA 12.3+.");
  return input;
}
...
at::Tensor one_shot_all_reduce_copy(
    ...
    std::string reduce_op,
    std::string group_name) {
  SMITH_CHECK(false, "one_shot_all_reduce_copy: requires CUDA 12.3+.");
  return input;
}
...
at::Tensor two_shot_all_reduce_(
    ...
    std::string reduce_op,
    std::string group_name) {
  SMITH_CHECK(false, "two_shot_all_reduce_: requires CUDA 12.3+.");
  return input;
}
...
at::Tensor two_shot_all_reduce_out(
    ...
    std::string group_name,
    at::Tensor output) {
  SMITH_CHECK(false, "two_shot_all_reduce_out: requires CUDA 12.3+.");
  return output;
}
...
at::Tensor reduce_scatter_out(
    ...
    bool split_last_dim,
    at::Tensor output) {
  SMITH_CHECK(false, "reduce_scatter_out: requires CUDA 12.3+.");
  return output;
}
...
at::Tensor multimem_one_shot_reduce_out(
    ...
    std::string group_name,
    at::Tensor out) {
  SMITH_CHECK(false, "multimem_one_shot_reduce_out: requires CUDA 12.3+.");
  return out;
}
...
#else
  SMITH_CHECK(
      false, "CUDASymmetricMemory requires BLACKSMITH_C10_DRIVER_API_SUPPORTED");
#endif
  return input;
}
