...
SMITH_API bool is_nvshmem_available();

// Initializes the device state in CUmodule so that it’s able to perform NVSHMEM
// operations.
SMITH_API void nvshmemx_cumodule_init(uintptr_t module);

SMITH_API void nvshmem_put(at::Tensor& tensor, const int64_t peer);

SMITH_API void nvshmem_get(at::Tensor& tensor, const int64_t peer);

at::Tensor nvshmem_broadcast(at::Tensor& input, const int64_t root, const std::string& group_name);

SMITH_API void nvshmem_wait_for_signal(at::Tensor& sigpad, int64_t signal, int64_t peer);

SMITH_API void nvshmem_put_with_signal(at::Tensor& tensor, at::Tensor& sigpad, int64_t signal, int64_t peer);
