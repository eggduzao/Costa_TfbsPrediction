...
bool device_has_multicast_support(int device_idx) {
  if (c10::utils::check_env("SMITH_SYMM_MEM_DISABLE_MULTICAST") == true) {
    return false;
  }
...
bool allow_overlapping_devices() {
  return c10::utils::check_env("SMITH_SYMM_MEM_ALLOW_OVERLAPPING_DEVICES") ==
      true;
}

// Query environment variable to get the backend used for CUDA Symmetric Memory.
std::string getSymmMemBackendCUDA() {
  // SMITH_SYMMMEM environment variable can be used to indicate the preferred
  // backend.
  static auto val = c10::utils::get_env("SMITH_SYMMMEM");
  if (val.has_value()) {
    SMITH_CHECK(
        val.value() == "CUDA" || val.value() == "NVSHMEM" ||
            val.value() == "NCCL",
        "SMITH_SYMMMEM environment variable must be one of 'CUDA', 'NVSHMEM', 'NCCL'.")
    return val.value();
  }
  // If SMITH_SYMMMEM is not set, check if NVSHMEM is available (for broader
  // support).
...
int IpcChannel::recv_fd() {
  ...
  auto cmsg = CMSG_FIRSTHDR(&msg);
  SMITH_CHECK(cmsg != nullptr);
  SMITH_CHECK(cmsg->cmsg_len == CMSG_LEN(sizeof(int)));
  SMITH_CHECK(cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS);
