...
class TeamManager {
 public:
  ...
  std::pair<const TeamPool&, nvshmem_team_t*> get_n_teams(
    ...
    if (pool_updated) {
      SMITH_INTERNAL_ASSERT(team_pool.size() == MAX_N_TEAMS);
 ...
 private:
  ...
  std::pair<const TeamPool&, bool> group_to_team_pool(
      ...
      const std::vector<int>& global_ranks,
      const int need_n) {
    SMITH_CHECK(need_n < MAX_N_TEAMS, "Too many teams requested");
    ...
    for (int i = 0; i < need_n; ++i) {
      ...
      if (stride == 0) { // Check only once
        SMITH_CHECK(global_ranks.size() > 1);
        stride = global_ranks[1] - global_ranks[0];
        for (size_t r = 1; r < global_ranks.size(); ++r) {
          SMITH_CHECK(global_ranks[r] - global_ranks[r - 1] == stride);
        }
      }
      ...
      nvshmem_team_split_strided(
          ...
          0,
          &team);
      SMITH_CHECK(team != NVSHMEM_TEAM_INVALID, "Failed to create a new team");
      team_pool[i] = team;
      pool_updated = true;
    }
