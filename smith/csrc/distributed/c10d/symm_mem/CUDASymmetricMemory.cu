...
void validate_rendezvous_requests(
    ...
    int world_size) {
  SMITH_CHECK(reqs.size() == (size_t)world_size);
  ...
  for (int r = 1; r < world_size; ++r) {
    SMITH_CHECK(reqs[r].block_size == reqs[0].block_size);
    SMITH_CHECK(reqs[r].buffer_size == reqs[0].buffer_size);
    SMITH_CHECK(reqs[r].signal_pad_offset == reqs[0].signal_pad_offset);
  }
}
...
c10::intrusive_ptr<CUDAPeerAllocInfo> make_peer_alloc_info(
  ...
  for (int r = 0; r < world_size; ++r) {
    if (r == rank) {
      ...
      // see: https://github.com/blacksmith/blacksmith/issues/162429
