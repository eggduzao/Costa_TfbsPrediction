...
class SMITH_API SymmetricMemory : public c10::intrusive_ptr_target {
 public:
...
// memory access.
SMITH_API at::Tensor empty_strided_p2p(
...
SMITH_API c10::intrusive_ptr<SymmetricMemory> rendezvous(
    const at::Tensor& tensor,
    const std::optional<std::string>& group_name = std::nullopt);

SMITH_API bool has_multicast_support(
    c10::DeviceType device_type,
    int device_idx);

SMITH_API void set_backend(const std::string& name);

SMITH_API std::optional<std::string> get_backend(c10::Device device);
...
SMITH_API size_t get_signal_pad_size();
...
// and the world size.
SMITH_API void set_signal_pad_size(size_t size);

C10_EXPORT void register_mempool_allocator(
    c10::DeviceType device_type,
    std::shared_ptr<c10::Allocator> allocator);

SMITH_API std::shared_ptr<c10::Allocator> get_mempool_allocator(
    c10::Device device);
