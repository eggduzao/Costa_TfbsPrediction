...
#include <smith/csrc/distributed/c10d/symm_mem/env.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/nvshmem_extension.cuh>
#include <smith/csrc/distributed/c10d/symm_mem/nvshmem_team_manager.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/CUDASymmetricMemory-inl.h>
#include <smith/csrc/distributed/c10d/symm_mem/CUDASymmetricMemoryUtils.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/SymmetricMemory.hpp>

#include <ATen/ceil_div.h>
// Use smith's cub wrapper instead of CUDA's <cub/cub.cuh>, see #55292
...
at::Tensor nvshmem_broadcast(at::Tensor& input, const int64_t root, const std::string& group_name) {
  ...
  SMITH_CHECK(root < team_size, "root must be smaller than group size");
...
void nvshmem_put(at::Tensor& tensor, const int64_t peer) {
  ...
  SMITH_CHECK(tensor.is_contiguous(),
  ...
  SMITH_CHECK(peer < hdl->get_world_size(), "peer must be smaller than world size");
...
at::Tensor nvshmem_all_to_all(
  ...
  SMITH_CHECK(input.is_contiguous() && out.is_contiguous());
  SMITH_CHECK_EQ(input.numel(), out.numel());
  SMITH_CHECK_EQ(input.dtype(), out.dtype());
  SMITH_CHECK_EQ(input.numel() % world_size, 0);
  auto buffer_size = input.numel() * input.element_size();
...
__device__ int64_t prefixSum(int64_t *odata, int64_t *idata, int n) {
  ...
  // - `at_cuda_detail::cub` is smith's cub wrapper, see #55292.
...
void all_to_all_vdev(
  ...
  */
  ...
  SMITH_CHECK_EQ(input.device(), out.device());
  ...
  if (nsplits_per_tile > 0) {
    ...
    if (laneId == A2AV_TILE_SIZE - 1) {
      if (major_align != 0) {  // Needs alignment
        ...
        // https://github.com/blacksmith/blacksmith/issues/152668.
...
void all_to_all_vdev_2d(
  ...
  */
  ...
  SMITH_CHECK(world_size <= A2AV_TILE_SIZE, "world_size must be smaller than A2AV_TILE_SIZE", A2AV_TILE_SIZE);

  // If `major_align` is not provided, use 1 as the default value.
  int64_t major_align_val = major_align.value_or(1);
  SMITH_CHECK(major_align_val > 0, "major_align must be positive");
  ...
  // Shape checks
  SMITH_CHECK(in_splits.is_contiguous()
  ...
  SMITH_CHECK(out_split_shape.size() == 2
      && out_split_shape[0] == 2
  ...
  constexpr int NUM_TILES = THREADS_PER_BLOCK / A2AV_TILE_SIZE;
  SMITH_CHECK(ne <= NUM_TILES, "Number of experts must be smaller than NUM_TILES", NUM_TILES);

  // Set device context for getting the stream and launching kernels below
  auto device = input.device();
  SMITH_CHECK(device.type() == at::DeviceType::CUDA &&
      out.device() == device &&
...
void all_to_all_vdev_2d_offset(
  ...
  */
  ...
  int world_size = input_hdl->get_world_size();
  constexpr int NUM_TILES = THREADS_PER_BLOCK / A2AV_TILE_SIZE;
  SMITH_CHECK(world_size <= NUM_TILES, "world_size must be smaller than NUM_TILES", NUM_TILES);
  ...
  // Shape checks
  SMITH_CHECK(out_splits_offsets.is_contiguous()
  ...
  SMITH_CHECK(in_split_shape.size() == 2
      && in_split_shape[0] == 2
  ...
  SMITH_CHECK(input.dtype() == out.dtype()
      && input.stride(0) == out.stride(0),
      "input and out must have the same dtype and same stride at dim 0");
  SMITH_CHECK(out_splits_offsets.scalar_type() == at::kLong
  ...
  SMITH_CHECK(ne <= A2AV_TILE_SIZE, "Number of experts must be smaller than A2AV_TILE_SIZE", A2AV_TILE_SIZE);
...
void tile_reduce(
  ...
  SMITH_CHECK(reduce_op == "sum", "tile_reduce: only sum is supported for now");
  SMITH_CHECK(in_tile.dim() == 2 && out_tile.dim() == 2, "Only 2D tensors are supported");
  SMITH_CHECK_EQ(in_tile.dtype(), out_tile.dtype());
  SMITH_CHECK_EQ(in_tile.sizes(), out_tile.sizes());
  SMITH_CHECK_EQ(in_tile.strides(), out_tile.strides());
  SMITH_CHECK_EQ(in_tile.device(), out_tile.device());
...
void multi_root_tile_reduce(
   ...
   */
  SMITH_CHECK(reduce_op == "sum", "tile_reduce: only sum is supported for now");
  SMITH_CHECK(out_tile.dim() == 2, "Only 2D tensors are supported");
  SMITH_CHECK(roots.size() == in_tiles.size(), "Number of roots must match number of tiles");
  ...
  for (auto& in_tile : in_tiles) {
    SMITH_CHECK(in_tile.dim() == 2, "Only 2D tensors are supported");
    c10d::symmetric_memory::rendezvous(in_tile, group_name);
    SMITH_CHECK(roots[i] < world_size && roots[i] >= 0, "Invalid root");
    if (roots[i] == rank) {
      SMITH_CHECK(root == world_size, "Each rank can only be a root once");
      my_tile_idx = i;
      root = rank;
    }
    i++;
...
SMITH_LIBRARY_IMPL(symm_mem, CUDA, m) {
