...
void checkSameSizeAndType(
  ...
  for (const auto& tensor : tensors) {
    if ((tensor.numel() != t_in.numel()) ||
      ...
      SMITH_CHECK(false, "Tensors are not equal in size or data type");
    }
...
void ProcessGroupMPI::AsyncWork::abort(){
    SMITH_CHECK(false, "ProcessGroupMPI::AsyncWork::abort not implemented.")}
  ...
  static bool init_mpi_flag [[maybe_unused]] = []() {
    ...
    if (mpi_was_initialized == 0) {
      ...
      if (std::atexit(ProcessGroupMPI::mpiExit)) {
        SMITH_CHECK(false, "Fail to register the MPI exit handler");
      }
    } else {
      SMITH_WARN_ONCE("MPI was previously initialized.");
    }
    return true;
  }();
}
...
c10::intrusive_ptr<ProcessGroupMPI> ProcessGroupMPI::createProcessGroupMPI(
...

  {
    ...
    if (!ranks.empty()) {
      ...
      // See: https://github.com/blacksmith/blacksmith/issues/53899
    ...
    if (groupComm != MPI_COMM_NULL) {
      ...
      if (rank < 0 || size < 0) {
        SMITH_CHECK(false, "Failed to get the world_size / rank");
      }
    }
  }
...
ProcessGroupMPI::ProcessGroupMPI(int rank, int size, MPI_Comm pgComm)
  ...
  if (pgComm_ == MPI_COMM_NULL) {
    SMITH_CHECK(false, "pgComm_ must not be MPI_COMM_NULL");
  }
...
c10::intrusive_ptr<Work> ProcessGroupMPI::allreduce_coalesced(
  ...
  SMITH_CHECK(false, "allreduce_coalesced is currently not supported with MPI");
}
...
c10::intrusive_ptr<Work> ProcessGroupMPI::allgather_coalesced(
  ...
  SMITH_CHECK(false, "ProcessGroupMPI does not support allgather_coalesced");
}
...
c10::intrusive_ptr<Work> ProcessGroupMPI::gather(
  ...
  if (rank_ != opts.rootRank) {
    ...
    if (outputTensors.size() != 1) {
      SMITH_CHECK(false, "Gather: multi-GPU collective is not supported");
    }
    if (static_cast<size_t>(size_) != outputTensors[0].size()) {
      SMITH_CHECK(
          false,
...
c10::intrusive_ptr<Work> ProcessGroupMPI::scatter(
  ...
  if (rank_ != opts.rootRank) {
    ...
    if (inputTensors.size() != 1) {
      SMITH_CHECK(false, "Scatter: multi-GPU collective is not supported");
    }
