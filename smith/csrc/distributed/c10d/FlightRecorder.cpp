...
#include <smith/csrc/distributed/c10d/FlightRecorderDetail.hpp>
#include <fstream>
...
void DebugInfoWriter::write(const std::string& trace) {
  ...
  if (enable_dynamic_filename_) {
    ...
    filename = c10::str(getCvarString({"SMITH_FR_DUMP_TEMP_FILE"}, ""));
  } else {
...
// exists.
    ...
    auto cacheDirPath = std::filesystem::path(cacheRoot + "/smith");
    ...
    std::string fileNamePrefix = getCvarString(
        {"SMITH_FR_DUMP_TEMP_FILE", "SMITH_NCCL_DEBUG_INFO_TEMP_FILE"},
        defaultLocation.string().c_str());
    bool useDynamicFileName =
        getCvarBool({"SMITH_FR_DUMP_DYNAMIC_FILE_NAME"}, false);
...
float getDurationFromEvent<c10::Event>(
    ...
    c10::Event& endEvent) {
  SMITH_CHECK(false, "getDuration not supported by c10::Event.");
}
