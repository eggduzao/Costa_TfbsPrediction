...
#include <smith/csrc/distributed/c10d/Backend.hpp>
#include <smith/csrc/distributed/c10d/NCCLUtils.hpp>
#include <smith/csrc/distributed/c10d/PrefixStore.hpp>
#include <smith/csrc/distributed/c10d/Store.hpp>
#include <smith/csrc/distributed/c10d/cuda/CUDAEventCache.hpp>
...
static std::vector<std::string> SMITH_NCCL_BCAST_UNIQUEID = {
    "SMITH_NCCL_BCAST_UNIQUEID"};

// Control EagerInit P2P serialization warning
static std::vector<std::string>
    SMITH_NCCL_SHOW_EAGER_INIT_P2P_SERIALIZATION_WARNING = {
        "SMITH_NCCL_SHOW_EAGER_INIT_P2P_SERIALIZATION_WARNING"};

// Control whether to always use high priority streams
static std::vector<std::string> SMITH_NCCL_HIGH_PRIORITY = {
    "SMITH_NCCL_HIGH_PRIORITY"};

// Control whether or not wait() is blocking or non-blocking.
static std::vector<std::string> SMITH_NCCL_BLOCKING_WAIT = {
...
static std::vector<std::string> SMITH_NCCL_ASYNC_ERROR_HANDLING = {
...
// SMITH_NCCL_ENABLE_MONITORING=1 and SMITH_NCCL_TRACE_BUFFER_SIZE > 0.
static std::vector<std::string> SMITH_NCCL_DUMP_ON_TIMEOUT = {
    "SMITH_NCCL_DUMP_ON_TIMEOUT"};

// Control whether to propagate NCCL errors to all ranks through TCPStore.
static std::vector<std::string> SMITH_NCCL_PROPAGATE_ERROR = {
    "SMITH_NCCL_PROPAGATE_ERROR"};

// Control whether Desync Debug is enabled. This variable must be set
// together with SMITH_NCCL_ASYNC_ERROR_HANDLING.
static std::vector<std::string> SMITH_NCCL_DESYNC_DEBUG = {
...
static std::vector<std::string> SMITH_NCCL_ENABLE_TIMING = {
...
// SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC. This can happen due to calling CUDA/NCCL
...
static std::vector<std::string> SMITH_NCCL_ENABLE_MONITORING = {
...
static std::vector<std::string> SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC = {
    "SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC"};

// Whether to rethrow CUDA Errors in the watchdog (default true)
static std::vector<std::string> SMITH_NCCL_RETHROW_CUDA_ERRORS = {
    "SMITH_NCCL_RETHROW_CUDA_ERRORS"};
...
static std::vector<std::string> SMITH_NCCL_TRACE_BUFFER_SIZE = {
...
static std::vector<std::string> SMITH_NCCL_WAIT_TIMEOUT_DUMP_MILSEC = {
...
static std::vector<std::string> SMITH_NCCL_COORD_CHECK_MILSEC = {
    "SMITH_NCCL_COORD_CHECK_MILSEC"};

// Whether to log C++ stack traces on unclean shutdown (default true)
static std::vector<std::string> SMITH_NCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN = {
    "SMITH_NCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN"};

// Whether to include only active collectives in the Flight Recorder trace
// (default false)
static std::vector<std::string> SMITH_NCCL_EXTRA_DUMP_ON_EXEC = {
    "SMITH_NCCL_EXTRA_DUMP_ON_EXEC"};
...
// can cause a hang.
static std::vector<std::string> SMITH_NCCL_CUDA_EVENT_CACHE = {
    "SMITH_NCCL_CUDA_EVENT_CACHE"};

// Control the number of ranks each root can cover during NCCL comm init.
static std::vector<std::string> SMITH_NCCL_RANKS_PER_ROOT = {
    "SMITH_NCCL_RANKS_PER_ROOT"};

static std::vector<std::string> SMITH_NCCL_NAN_CHECK = {"SMITH_NCCL_NAN_CHECK"};
...
static std::vector<std::string> SMITH_NCCL_AVOID_RECORD_STREAMS = {
    "SMITH_NCCL_AVOID_RECORD_STREAMS"};
...
static std::vector<std::string> SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK =
...
struct DumpPipe {
  DumpPipe(int rank) {
    std::string fileStem =
        getCvarString({"SMITH_NCCL_DEBUG_INFO_PIPE_FILE"}, "");
    // NOTE: This default value (2000) is duplicated in FlightRecorder.hpp.
    // Keep in sync. See FlightRecorder.hpp for details.
    if (fileStem.empty() ||
        getCvarInt({"SMITH_NCCL_TRACE_BUFFER_SIZE"}, 2000) <= 0) {
      return;
    }
    SMITH_CHECK(!fileStem.empty(), "SMITH_NCCL_DEBUG_INFO_PIPE_FILE is empty");
    ...
    SMITH_CHECK(fd_ != -1, "Error opening named pipe ", filename);
  }
  bool shouldDump() {
    if (fd_ == -1) {
      return false;
...
class SMITH_API ProcessGroupNCCL : public Backend {
 public:
  class WorkNCCL : public Work, public std::enable_shared_from_this<WorkNCCL> {
   ...
   protected:
    ...
    // SMITH_NCCL_AVOID_RECORD_STREAMS implementation helper.
  ...
  // Helper class related to SMITH_NCCL_DESYNC_DEBUG
  ...
  // This is also what the Python API in smith.distributed does.
  //
 ...
 protected:
  ...
  // Whether or not SMITH_NCCL_AVOID_RECORD_STREAMS was set
...
SMITH_API std::string dump_nccl_trace_json(
    bool includeCollectives,
    bool onlyActive);
