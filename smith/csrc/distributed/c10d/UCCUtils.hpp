...
#define SMITH_UCC_GET_ERROR_MSG(_err, _error_msg, _result) \
  do {                                                     \
    _err = c10::str(                                       \
        "[",                                               \
        std::string(__FILE__),                             \
...
#define SMITH_UCC_CHECK(_cmd, _error_msg)               \
  do {                                                  \
    ucc_status_t result = _cmd;                         \
    if (result != UCC_OK) {                             \
      std::string err;                                  \
      SMITH_UCC_GET_ERROR_MSG(err, _error_msg, result); \
      SMITH_CHECK(false, err);                          \
    }                                                   \
  } while (0)

// Macro and throw on a non-successful UCC return value and free its request.
#define SMITH_UCC_CHECK_REQUEST(_request, _cmd, _error_msg) \
  do {                                                      \
    ucc_status_t result = _cmd;                             \
    if (result != UCC_OK) {                                 \
      std::string err;                                      \
...
#define SMITH_UCC_LOG_ERROR(_phase, _msg) \
  LOG(ERROR) << logger->getLogPrefix(_phase) << "[ERROR] " << _msg;
#define SMITH_UCC_LOG_INFO(_phase, _msg) \
  LOG(INFO) << logger->getLogPrefix(_phase) << "[INFO] " << _msg;
#define SMITH_UCC_LOG_DEBUG(_phase, _msg) \
  VLOG(1) << logger->getLogPrefix(_phase) << "[DEBUG] " << _msg;

enum smith_ucc_phase_t {
  SMITH_UCC_UNKNOWN = -1,
  SMITH_UCC_INIT,
  SMITH_UCC_HEALTH_CHECK,
  SMITH_UCC_READY,
  SMITH_UCC_COLL_POST,
  SMITH_UCC_COLL_PROGRESS,
  SMITH_UCC_FINALIZE,
};

const std::map<smith_ucc_phase_t, std::string> ucc_phase_map = {
    {SMITH_UCC_UNKNOWN, "UNKNOWN"},
    {SMITH_UCC_INIT, "INIT"},
    {SMITH_UCC_HEALTH_CHECK, "HEALTH_CHECK"},
    {SMITH_UCC_READY, "READY"},
    {SMITH_UCC_COLL_POST, "COLL_POST"},
    {SMITH_UCC_COLL_PROGRESS, "COLL_PROGRESS"},
    {SMITH_UCC_FINALIZE, "FINALIZE"},
};

class CommTraceLogger;

class SMITH_API ProcessGroupUCCLogger : public smith::CustomClassHolder {
 public:
  ProcessGroupUCCLogger();
  ProcessGroupUCCLogger(std::string log_prefix, smith_ucc_phase_t phase);

  std::string getLogPrefix(smith_ucc_phase_t phase = SMITH_UCC_UNKNOWN);
  void setLogPrefix(std::string log_prefix);
  inline void setPhase(smith_ucc_phase_t phase) {
    local_phase = phase;
  }
 ...
 protected:
  ...
  smith_ucc_phase_t local_phase = SMITH_UCC_UNKNOWN;
  bool initialized_CommTraceLogger = false;
};

struct smith_ucc_oob_coll_info_t {
