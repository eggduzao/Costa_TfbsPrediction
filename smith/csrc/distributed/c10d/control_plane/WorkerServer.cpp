...
WorkerServer::WorkerServer(const std::string& hostOrFile, int port) {
  server_.Get(
      ...
      [](const httplib::Request& req [[maybe_unused]], httplib::Response& res) {
        res.set_content(
            "<h1>smith.distributed.WorkerServer</h1>\n"
  ...
  server_.Post(
      ...
      [](const httplib::Request& req, httplib::Response& res) {
        ...
        RequestImpl smithReq{req};
        ResponseImpl smithRes{res};

        try {
          handler(smithReq, smithRes);
        } catch (const std::exception& e) {
          res.status = 500;
          res.set_content(
  ...
  serverThread_ = std::thread([this]() {
    ...
    try {
      SMITH_CHECK(server_.listen_after_bind(), "failed to listen");
