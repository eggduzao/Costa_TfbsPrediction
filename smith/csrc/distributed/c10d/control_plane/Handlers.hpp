...
SMITH_API void registerHandler(const std::string& name, HandlerFunc f);

// Fetches a handler by name.
SMITH_API HandlerFunc getHandler(const std::string& name);

SMITH_API std::vector<std::string> getHandlerNames();

// Registers a handler statically.
// See registerHandler for more details.
class SMITH_API RegisterHandler {
 public:
