 ...
 public:
  BroadcastWork(
      ...
      : bucket_tensors_(std::move(bucket_tensors)),
        flat_tensor_({smith::utils::flatten_dense_tensors(bucket_tensors_)}) {
  ...
  void finish() {
    ...
    auto output_tensors = smith::utils::unflatten_dense_tensors(
        flat_tensor_.front(), bucket_tensors_);
    SMITH_INTERNAL_ASSERT(output_tensors.size() == bucket_tensors_.size());
    for (const auto i : c10::irange(output_tensors.size())) {
      ...
      // https://github.com/blacksmith/blacksmith/issues/87280
