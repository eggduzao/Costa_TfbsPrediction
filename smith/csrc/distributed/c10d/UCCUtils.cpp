...
ucc_status_t oob_allgather(
    ...
    void* coll_info,
    void** req) {
  auto* info = reinterpret_cast<smith_ucc_oob_coll_info_t*>(coll_info);
  SMITH_CHECK(info != nullptr);
...
ucc_status_t oob_allgather_test(void* req) {
  auto* info = reinterpret_cast<smith_ucc_oob_coll_info_t*>(req);
  SMITH_CHECK(info != nullptr);
...
CommUCC::CommUCC(
  ...
  SMITH_UCC_CHECK(
      ucc_lib_config_read("SMITH", nullptr, &lib_config),
  ...
  if (st != UCC_OK) {
    ...
    SMITH_CHECK(false, ucc_status_string(st));
  }
...
CommUCC::~CommUCC() {
  ...
  if (lib != nullptr) {
    SMITH_UCC_CHECK(ucc_finalize(lib), "failed to finalize UCC library");
  }
  context = nullptr;
  lib = nullptr;
}

std::string ProcessGroupUCCLogger::getLogPrefix(smith_ucc_phase_t phase) {
  // caller can override the phase stored locally
  smith_ucc_phase_t phase_ =
      (local_phase != phase && phase != SMITH_UCC_UNKNOWN) ? phase
                                                           : local_phase;
