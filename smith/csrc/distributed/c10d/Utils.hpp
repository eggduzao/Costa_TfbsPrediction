...
SMITH_API size_t getTensorsNumel(const std::vector<at::Tensor>& tensors);

// Retrieve tensor shapes from a given tensor.
SMITH_API std::vector<at::Tensor> getTensorShapes(
...
inline std::string getCvarString(
  ...
  if (env.empty()) {
    SMITH_CHECK(false, "No environment variables passed");
    return ret;
  }
...
inline int getCvarInt(const std::vector<std::string>& env, int def) {
  ...
  for (ssize_t i = static_cast<ssize_t>(env.size()) - 1; i >= 0; i--) {
    ...
    try {
      ...
      SMITH_CHECK(false, "Invalid value for environment variable: " + env[i]);
    }
  }
...
// Copied from smith/csrc/utils/tensor_flatten.h.
...
void SMITH_API getGlobalRankStartAndStride(
