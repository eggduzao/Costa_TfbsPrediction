...
#include <smith/csrc/distributed/c10d/RankLocal.hpp>
...
std::string opTypeToString(OpType opType) {
  switch (opType) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unknown op type!");
  }
  return "UNKNOWN";
}
...
const std::string& ProcessGroup::getGroupName() const {
  SMITH_CHECK(!deviceTypeToBackend_.empty(), "ProcessGroup name not set");
  ...
  ~WorkRegistry() {
    ...
    if (registry_size > 0) {
      SMITH_WARN(
          ...
          "2. c10d_functional.wait_tensor() is invoked on all output tensors of async_op=True smith.distributed collective "
