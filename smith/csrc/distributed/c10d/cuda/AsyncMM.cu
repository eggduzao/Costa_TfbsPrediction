...
#include <smith/csrc/distributed/c10d/cuda/cutlass/gemm/kernel/persistent_async_input_scheduler.cuh>
...
at::Tensor async_input_mm_impl(
  ...
  SMITH_CHECK(a.dim() == 2 && b.dim() == 2 && out.dim() == 2);
  SMITH_CHECK(a.is_contiguous() && out.is_contiguous());
  ...
  SMITH_CHECK_EQ(a.scalar_type(), at::kBFloat16);
  SMITH_CHECK_EQ(b.scalar_type(), at::kBFloat16);
  SMITH_CHECK_EQ(out.scalar_type(), at::kBFloat16);

  int M = static_cast<int>(a.sizes()[0]);
  int N = static_cast<int>(b.sizes()[1]);
  int K = static_cast<int>(a.sizes()[1]);
  SMITH_CHECK_EQ(b.sizes()[0], K);
  SMITH_CHECK_EQ(out.sizes()[0], M);
  SMITH_CHECK_EQ(out.sizes()[1], N);
  ...
  SMITH_CHECK(chunk_size_M % tile_size_M == 0);
  ...
  SMITH_CHECK(gemm.can_implement(arguments) == cutlass::Status::kSuccess);
  SMITH_CHECK(
