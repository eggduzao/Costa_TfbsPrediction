...
std::string getExceptionMsgFromExceptionPtr(
  ...
  SMITH_CHECK(exceptionPtr != nullptr);
  try {
...
inline void errorIfCapturingNonCapturableNCCL(c10::cuda::CaptureStatus status) {
  ...
  static const uint64_t cur_version = smith::cuda::nccl::version();
...
// When SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK is set, all tensors (no
...
bool shouldAllCommunicatorsRegisterAllTensors() {
  ...
  static const bool flag = [] {
    const bool flag =
        getCvarBool(SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK, false);
    if (flag &&
        c10::cuda::CUDACachingAllocator::CUDAAllocatorConfig::
            expandable_segments()) {
      LOG(INFO)
          << "disables SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK because it is not compatible with CUDA allocator expandable segments mode.";
      return false;
    }
    return flag;
  }();
...
// - If SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK=1 then *ALL* segments
...
static std::future<bool> launchAsyncGilCheck() {
  ...
  SMITH_CHECK(get_gil_checker(), "Can't check GIL with null GIL checker");
...
bool ProcessGroupNCCL::WorkNCCL::checkTimeout(
  ...
  STATIC_SCOPED_WAIT_COUNTER(
      blacksmith.wait_counter.ProcessGroupNCCL__checkTimeout);
...
std::string ProcessGroupNCCL::WorkNCCL::getTraceback() const {
  ...
  if (entry.has_value()) {
    ...
    // Note: `getTraceback` call below invokes `smith::symbolize`, which may
...
void ProcessGroupNCCL::WorkNCCL::printTraceback() const {
  ...
  if (!tracebackStr.empty()) {
    ...
    LOG(ERROR)
        ...
        << "You can enable it by setting SMITH_NCCL_TRACE_BUFFER_SIZE to a non-zero value.";
  }
}
...
constexpr const char* MULTI_DEVICE_ERROR_MSG =
    ...
    "https://blacksmith.org/docs/stable/distributed.html#multi-gpu-collective-functions. "
...
ProcessGroupNCCL::ProcessGroupNCCL(
  ...
  blockingWait_ = getCvarBool(SMITH_NCCL_BLOCKING_WAIT, false);
  asyncErrorHandling_ = static_cast<ErrorHandlingMode>(
      getCvarInt(SMITH_NCCL_ASYNC_ERROR_HANDLING, 3 /*SkipCleanUp*/));
  enableNanCheck_ = getCvarBool(SMITH_NCCL_NAN_CHECK, false);
  cudaEventCacheEnabled_.store(getCvarBool(SMITH_NCCL_CUDA_EVENT_CACHE, true));
  // NOTE: This default value (2000) is duplicated in FlightRecorder.hpp.
  // Keep in sync. See FlightRecorder.hpp for details.
  traceBufferSize_ = getCvarInt(SMITH_NCCL_TRACE_BUFFER_SIZE, 2000);
  ...
  auto desyncDebug = getCvarBool(SMITH_NCCL_DESYNC_DEBUG, false) ||
      (dist_debug_level_ >= DebugLevel::Detail);
#ifdef ENABLE_NCCL_ERROR_CHECKING
  enableTiming_.store(
      getCvarBool(SMITH_NCCL_ENABLE_TIMING, false) || desyncDebug);
#endif // ENABLE_NCCL_ERROR_CHECKING
  if (getCvarBool(SMITH_NCCL_AVOID_RECORD_STREAMS, false)) {
    SMITH_WARN_ONCE(
        "SMITH_NCCL_AVOID_RECORD_STREAMS is the default now, this environment variable is thus deprecated.");
  }
  showSerializationWarning_ =
      getCvarBool(SMITH_NCCL_SHOW_EAGER_INIT_P2P_SERIALIZATION_WARNING, true);

  if (blockingWait_) {
    LOG(INFO)
        << logPrefix()
        << "SMITH_NCCL_BLOCKING_WAIT is enabled, NO watchdog thread is created.";
  } else {
    if (desyncDebug && asyncErrorHandling_ == NoHandling) {
      LOG(INFO)
          << logPrefix()
          << "SMITH_NCCL_DESYNC_DEBUG and SMITH_NCCL_ASYNC_ERROR_HANDLING "
          << "must both be enabled. "
          << "Enabling SMITH_NCCL_ASYNC_ERROR_HANDLING.";
  ...
  if (at::globalContext().deterministicAlgorithms()) {
    ...
    if (!nccl_algo.has_value()) {
      LOG(INFO)
          << "smith deterministic mode is enabled, "
  ...
  std::string smith_distributed_debug =
      getCvarString({"SMITH_DISTRIBUTED_DEBUG"}, OFF.c_str());
  LOG(INFO) << logPrefix()
  ...
  if (local_id_ == 0) {
    LOG(INFO) << logPrefix() << "ProcessGroupNCCL environments: "
              ...
              << ", SMITH_NCCL_ASYNC_ERROR_HANDLING: " << asyncErrorHandling_
              << ", SMITH_NCCL_ENABLE_TIMING: " << enableTiming_.load()
              << ", SMITH_NCCL_BLOCKING_WAIT: " << blockingWait_
              << ", SMITH_DISTRIBUTED_DEBUG: " << smith_distributed_debug
#ifdef NCCL_HAS_COMM_REGISTER
              << ", SMITH_NCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK: "
              << shouldAllCommunicatorsRegisterAllTensors()
#endif // NCCL_HAS_COMM_REGISTER
              << ", SMITH_NCCL_TRACE_BUFFER_SIZE: " << traceBufferSize_
              << ", SMITH_NCCL_NAN_CHECK: " << enableNanCheck_
              << ", SMITH_NCCL_CUDA_EVENT_CACHE: " << cudaEventCacheEnabled_;
  }
...
bool ProcessGroupNCCL::useNonblocking() {
  ...
  auto nbEnv = c10::utils::check_env("SMITH_NCCL_USE_COMM_NONBLOCKING");
  ...
  // Note: this automatic selection is disabled in smith 2.7.1 to work around a
  // hang in NCCL 2.26 in non-blocking mode. We can revisit if NCCL fixes the
  // bug. See https://github.com/blacksmith/blacksmith/issues/153960
...
void ProcessGroupNCCL::registerOnCompletionHook(
  ...
  SMITH_CHECK_WITH(
      ...
      "events which require setting SMITH_NCCL_ENABLE_TIMING environment variable. "
...
c10::intrusive_ptr<Backend> ProcessGroupNCCL::split(
  ...
  SMITH_CHECK(ncclOpts != nullptr, "opts not a ProcessGroupNCCL::Options.");
...
bool ProcessGroupNCCL::waitForFutureOrTimeout(
  ...
  SMITH_CHECK(fut.valid(), "Expected a valid future");
...
void ProcessGroupNCCL::dumpExtraDebuggingInfo() {
  ...
  bool dumpExtraOnExec_ = getCvarBool(SMITH_NCCL_EXTRA_DUMP_ON_EXEC, false);
  if (dumpExtraOnExec_) {
    ...
    if (succeeded) {
      ...
      bool onlyActive = getCvarBool(SMITH_INCLUDE_ONLY_ACTIVE, false);
...
void ProcessGroupNCCL::abort() {
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.ProcessGroupNCCL__abort);
  ...
  if (!terminateProcessGroup_.load()) {
    ...
    if (rank_ % localDeviceCount_ == 0) {
      SMITH_WARN_ONCE(
          ...
          "https://blacksmith.org/docs/stable/distributed.html#shutdown");
    }
    ...
    // TODO: Note 1 is not true in case of a C++ program using libsmith, which
...
bool ProcessGroupNCCL::dumpDebuggingInfo(
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.ProcessGroupNCCL__dumpDebuggingInfo);
...
ProcessGroupNCCL::HeartbeatMonitor::HeartbeatMonitor(ProcessGroupNCCL* pg) {
  ...
  heartbeatTimeoutInSec_ =
      getCvarInt(SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC, 60 * 8 /*8 Mins*/);
  waitTimeoutDumpInMilSec_ =
      getCvarInt(SMITH_NCCL_WAIT_TIMEOUT_DUMP_MILSEC, 15 * 1000 /*15 Sec*/);
  coordCheckIntervalMilSec_ = getCvarInt(SMITH_NCCL_COORD_CHECK_MILSEC, 1000);
  // TODO, we should either deprecate SMITH_NCCL_DUMP_ON_TIMEOUT
  // or change its name to reflect that dump happens on exception including
  // both timeout and other errors.
  dumpOnTimeoutOrEx_ = getCvarBool(SMITH_NCCL_DUMP_ON_TIMEOUT, true);
  // logging C++ stack isn't safe. Gate it with an ENV.
  logCppStackOnUncleanShutdown_ =
      getCvarBool(SMITH_NCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN, true);
  watchdogHeartbeatMonitorEnabled_ =
      getCvarBool(SMITH_NCCL_ENABLE_MONITORING, true);

  // print out ENV settings for the heartbeat monitor thread.
  if (pg_->getUid() == 0) {
    LOG(INFO)
        << pg_->logPrefix() << "HeartbeatMonitor environments: "
        << "SMITH_NCCL_ENABLE_MONITORING (Whether to kill program when no watchdog heartbeat detected): "
        << watchdogHeartbeatMonitorEnabled_
        << ", SMITH_NCCL_DUMP_ON_TIMEOUT: " << dumpOnTimeoutOrEx_
        << ", SMITH_NCCL_WAIT_TIMEOUT_DUMP_MILSEC: " << waitTimeoutDumpInMilSec_
        << ", SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC: " << heartbeatTimeoutInSec_
        << ", SMITH_NCCL_COORD_CHECK_MILSEC: " << coordCheckIntervalMilSec_
        << ", SMITH_NCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN: "
        << logCppStackOnUncleanShutdown_;
  }
}
...
void ProcessGroupNCCL::HeartbeatMonitor::runLoop() {
  ...
  STATIC_SCOPED_WAIT_COUNTER(
      blacksmith.ProcessGroupNCCL__HeartbeatMonitor__runLoop);
    ...
    if (computeDeltaMS(lastTimeHeartBeatCheck, currentTime) >=
      ...
      if (heartbeat != heartBeatCounter) {
        ...
        errorMsg = c10::str(
            ...
            "you can either increase the timeout (SMITH_NCCL_HEARTBEAT_TIMEOUT_SEC) to a larger value "
            "or disable the heartbeat monitor (SMITH_NCCL_ENABLE_MONITORING=0)."
            "If either of aforementioned helps, feel free to file an issue to Blacksmith about the short timeout "
  ...
  //    SMITH_NCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN=0).

  // Dump the nccl trace (flight recorder).
  if (checkDumpSignal && shouldDump_.load()) {
    // Store debug info to storage if no other thread does it. (By default to
    // local disk)
    bool dumpStackTrace = getCvarBool(SMITH_INCLUDE_STACK_TRACE, true);
    bool onlyActive = getCvarBool(SMITH_INCLUDE_ONLY_ACTIVE, false);
    ::c10d::C10dLoggingData debugLog;
...
ProcessGroupNCCL::Watchdog::Watchdog(ProcessGroupNCCL* pg) {
  pg_ = pg;
  heartbeat_ = 1ULL;
  rethrowCUDAErrors_ = getCvarBool(SMITH_NCCL_RETHROW_CUDA_ERRORS, true);
  propagatePgError_ = getCvarBool(SMITH_NCCL_PROPAGATE_ERROR, false);
  desyncDebug_ = getCvarBool(SMITH_NCCL_DESYNC_DEBUG, false) ||
  ...
  if (pg_->getUid() == 0) {
    LOG(INFO) << pg_->logPrefix() << "PGNCCL Watchdog environments: "
              << "SMITH_NCCL_RETHROW_CUDA_ERRORS: " << rethrowCUDAErrors_
              << ", SMITH_NCCL_PROPAGATE_ERROR: " << propagatePgError_
              << ", SMITH_NCCL_DESYNC_DEBUG: " << desyncDebug_;
  }
...
void ProcessGroupNCCL::Watchdog::run() {
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.ProcessGroupNCCL__Watchdog__run);
...
// in which case we don't want to spam the logs.
    ...
    for (auto it = pg_->workMetaList_.begin(); it != pg_->workMetaList_.end();
      ...
      // Report desync state in case of timeout (if SMITH_NCCL_DESYNC_DEBUG is
...
void ProcessGroupNCCL::DesyncDebugger::run() {
  ...
  try {
    ...
    enabled_ = false;
    LOG(ERROR) << logPrefix
               << " Failed to retrieve SMITH_NCCL_DESYNC_DEBUG report. "
    ...
    LOG(ERROR)
        << logPrefix
        << " Failed to rerieve SMITH_NCCL_DESYNC_DEBUG report with unknown error."
...
std::exception_ptr ProcessGroupNCCL::checkForNCCLErrorsInternal(
  ...
  // When nonblocking mode is enabled by SMITH_NCCL_USE_COMM_NONBLOCKING,
...
std::shared_ptr<NCCLComm> ProcessGroupNCCL::initNCCLComm(
  ...
  // according to
  // https://github.com/blacksmith/blacksmith/pull/136789#discussion_r1779171615.
  auto ranksPerRoot = getCvarInt(SMITH_NCCL_RANKS_PER_ROOT, 128);
  ...
  if (useScalableInit) {
    ...
    if (!ncclComm) {
      if (getCvarBool(SMITH_NCCL_BCAST_UNIQUEID, true) && !isSendRecvSelf) {
  ...
  bool force_high = getCvarBool(SMITH_NCCL_HIGH_PRIORITY, false);
...
float ProcessGroupNCCL::WorkNCCL::getDuration() const {
  SMITH_CHECK(timingEnabled_, "getDuration only works if timing was enabled");
  SMITH_CHECK(
      ncclStartEvent_,
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::collective(
  ...
  if (coalescing_state_ & CoalActive) {
    ...
    if (coalescedComm_ == nullptr) {
      ...
      SMITH_CHECK(coalescedComm_ == ncclComm, MULTI_DEVICE_ERROR_MSG);
    }
...
// runtime.
  ...
  {
    smith::cuda::nccl::AutoNcclGroup nccl_group_guard(comm, useNonblocking());
  ...
  if (this->eagerInit_) {
    ...
    if (!batchP2P && showSerializationWarning_) {
      SMITH_WARN_ONCE(c10::str(
          ...
          "SMITH_NCCL_SHOW_EAGER_INIT_P2P_SERIALIZATION_WARNING to false."));
    }
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::allreduce_sparse(
  ...
  SMITH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
  ...
  at::Tensor outputTensor =
      smith::zeros(tensor.sizes(), tensor.options().layout(smith::kStrided));
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::allgather(
  ...
  SMITH_CHECK(inputTensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::reduce_scatter(
  ...
  SMITH_CHECK(outputTensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
  ...
  if (outputSplitSizes.empty() && inputSplitSizes.empty()) {
    ...
    return collective(
        ...
        [&](at::Tensor& input,
            ...
            ncclComm_t comm,
            at::cuda::CUDAStream& stream) {
          smith::cuda::nccl::all2all_single_equal_split(
    ...
    return collective(
        ...
        [&](at::Tensor& input,
          ...
          // See [Sync Streams].
          smith::cuda::nccl::all2all_single_unequal_split(
              input.data_ptr(),
              send_lengths.data(),
              send_offsets.data(),
              output.data_ptr(),
...
c10::intrusive_ptr<Work> ProcessGroupNCCL::alltoall(
  ...
  return collective(
      ...
      [&](at::Tensor& /* unused */,
          ...
          ncclComm_t comm,
          at::cuda::CUDAStream& stream) {
        smith::cuda::nccl::all2all(outputTensors, inputTensors, comm, stream);
        return ncclSuccess;
      },
      [&](at::cuda::CUDAStream&,
          c10::intrusive_ptr<ProcessGroupNCCL::WorkNCCL>& work) {},
...
static void* _ncclMemAlloc(size_t size, int device, void* stream) {
  ...
  at::cuda::OptionalCUDAGuard gpuGuard(device);
  void* ptr = nullptr;
  SMITH_CHECK(ncclMemAlloc(&ptr, size) == ncclSuccess, "ncclMemAlloc failed");
  return ptr;
...
static void _ncclMemFree(void* ptr, size_t size, int device, void* stream) {
  ...
  at::cuda::OptionalCUDAGuard gpuGuard(device);
  SMITH_CHECK(ncclMemFree(ptr) == ncclSuccess, "ncclMemFree failed");
#endif // NCCL_HAS_MEM_ALLOC
}
...
std::shared_ptr<c10::Allocator> ProcessGroupNCCL::getMemAllocator() {
  ...
  static std::shared_ptr<c10::cuda::CUDACachingAllocator::CUDAAllocator>
      ncclMemAllocator =
          smith::cuda::CUDAPluggableAllocator::createCustomAllocator(
...
at::Tensor ProcessGroupNCCL::allocateTensor(
  ...
  // Some checks
  SMITH_CHECK_VALUE(options.has_device(), "Tensor options must include device");
...
c10::intrusive_ptr<Backend> ProcessGroupNCCL::shrink(
  ...
  auto runtime_version = smith::cuda::nccl::version();
  SMITH_CHECK(
...
c10::intrusive_ptr<Backend> ProcessGroupNCCL::shrink(
  ...
  SMITH_CHECK(
      false,
      "ProcessGroupNCCL::shrink requires NCCL version 2.27.0 or later, "
      "but Blacksmith was built with an older version or without NCCL shrink support.");
}
