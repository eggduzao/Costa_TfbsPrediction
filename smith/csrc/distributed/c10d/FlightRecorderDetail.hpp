...
// Note: `getTraceback` invokes `smith::symbolize`, which may need to acquire
...
std::string FlightRecorder<EventType>::Entry::getTraceback() {
  smith::CapturedTraceback* traceback = traceback_.get();
  smith::SymbolizedTracebacks s_tbs = smith::symbolize({traceback});
  ...
  /* Resulted format is like:
    #0 all_reduce from blacksmith/smith/distributed/distributed_c10d.py:2696
    #1 wrapper from blacksmith/smith/distributed/c10d_logger.py:83
...
typename FlightRecorder<EventType>::TraceIdentifier FlightRecorder<EventType>::
  ...
  auto traceback =
      smith::CapturedTraceback::gather(true, true, capture_cpp_stack_);
...
size_t FlightRecorder<EventType>::getIdxFromId(size_t id, size_t reset_epoch)
  ...
  SMITH_CHECK(it != reset_epoch_start_idx_.end());
...
const c10::List<c10::IValue> FlightRecorder<EventType>::getCollectiveTrace(
  ...
  std::vector<smith::CapturedTraceback*> tracebacks;
  smith::SymbolizedTracebacks stracebacks;
  std::vector<c10::IValue> all_frames;
  if (includeStacktraces) {
    for (auto& e : result) {
      tracebacks.push_back(e.traceback_.get());
    }
    stracebacks = smith::symbolize(tracebacks);
...
std::string FlightRecorder<EventType>::dump(
    ...
    bool includeStackTraces,
    bool onlyActive) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.FlightRecorder__dump);
