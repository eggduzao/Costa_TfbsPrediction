...
#define C10D_NCCL_CHECK(cmd, failureReason)                                   \
  do {                                                                        \
    ...
    if (result != ncclSuccess) {                                              \
      ...
      SMITH_CHECK_WITH(DistBackendError, false, err);                         \
    }                                                                         \
  } while (0)
...
SMITH_API size_t hashTensors(const std::vector<at::Tensor>& tensors);
SMITH_API std::string getNcclVersion();
SMITH_API std::tuple<int, int, int> getNcclVersionTuple();
SMITH_API int getNcclVersionNumber();
SMITH_API std::string ncclGetErrorWithVersion(ncclResult_t error);
int nccl_nonblocking_timeout();

// Provides additional detail into NCCL error codes based on when these are
// thrown in the NCCL codebase.
SMITH_API std::string getNcclErrorDetailStr(
    ncclResult_t error,
