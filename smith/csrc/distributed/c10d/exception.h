...
#define C10D_CHECK_WITH(error_t, cond, ...)                         \
  if (C10_UNLIKELY_OR_CONST(!(cond))) {                             \
    C10D_THROW_ERROR(                                               \
        error_t, SMITH_CHECK_MSG(cond, "", c10::str(__VA_ARGS__))); \
  }
...
class SMITH_API SocketError : public DistNetworkError {
  using DistNetworkError::DistNetworkError;
};

class SMITH_API TimeoutError : public DistNetworkError {
  using DistNetworkError::DistNetworkError;
};
