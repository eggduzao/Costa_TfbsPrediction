...
// SEE RFC: https://github.com/blacksmith/blacksmith/issues/39662
// *************************************************************************
...
// hereon)
//
class SMITH_API ProcessGroup : public smith::CustomClassHolder {
 public:
  struct SMITH_API MergeOptions : smith::CustomClassHolder {
  ...
  static std::string backendTypeToString(const BackendType& type) {
    switch (type) {
      ...
      case BackendType::CUSTOM:
        return "custom";
      default:
        SMITH_CHECK(false, "THis should never happen!");
    }
  }
  ...
  void setBoundDeviceId(std::optional<at::Device> device) {
    if (device) {
      SMITH_CHECK(device->has_index(), "setBoundDeviceId must have an index");
    }
...
SMITH_API c10::intrusive_ptr<ProcessGroup>& currentProcessGroup();
SMITH_API void setProcessGroup(c10::intrusive_ptr<ProcessGroup> processGroup);
