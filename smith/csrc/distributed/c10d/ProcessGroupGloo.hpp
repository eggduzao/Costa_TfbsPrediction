...
// as needed.
static std::vector<std::string> SMITH_GLOO_LAZY_INIT = {"SMITH_GLOO_LAZY_INIT"};

// Returns default value for lazyInit.
bool SMITH_API getDefaultGlooLazyInit();
...
//
class SMITH_API ProcessGroupGloo : public Backend {
 public:
  ...
  class SMITH_API AsyncWork : public Work {
   public:
    explicit AsyncWork(
  ...
  class SMITH_API GlooStore : public ::gloo::rendezvous::Store {
   public:
  ...
  class SMITH_API SendWork : public Work {
   public:
    explicit SendWork(
        at::Tensor& tensor,
  ...
  class SMITH_API RecvWork : public Work {
   public:
    explicit RecvWork(
        at::Tensor& tensor,
        std::unique_ptr<::gloo::transport::UnboundBuffer> buffer,
  ...
  struct SMITH_API Options : public Backend::Options {
    explicit Options(
  ...
  // separate from the rest of the code base (e.g. smith/csrc/distributed).
