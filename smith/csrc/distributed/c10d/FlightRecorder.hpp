...
//    auto-registered).
class SMITH_API DebugInfoWriter {
 public:
...
struct FlightRecorder {
  ...
  FlightRecorder() {
    ...
    max_entries_ = getCvarInt(
        {"SMITH_FR_BUFFER_SIZE", "SMITH_NCCL_TRACE_BUFFER_SIZE"}, 2000);
    capture_cpp_stack_ = getCvarBool(
        {"SMITH_FR_CPP_STACK", "SMITH_NCCL_TRACE_CPP_STACK"}, false);
  ...
  struct Entry {
    ...
    std::shared_ptr<smith::CapturedTraceback> traceback_;
    ...
    // Note: `getTraceback` invokes `smith::symbolize`, which may need to
    ...
    // like std::async.
    SMITH_API std::string getTraceback();
  };
  ...
  SMITH_API void record_pg_ranks(
      const std::tuple<std::string, std::string>& pg_name,
  ...
  SMITH_API std::optional<Entry> getEntry(
      std::optional<size_t> id,
      std::optional<size_t> reset_epoch);

  SMITH_API std::optional<Entry> getEntry(std::optional<size_t> id);
...
static std::vector<std::string> SMITH_INCLUDE_STACK_TRACE = {
    "SMITH_INCLUDE_STACK_TRACE"};

// Whether to include only active collectives in the Flight Recorder trace
// (default false)
static std::vector<std::string> SMITH_INCLUDE_ONLY_ACTIVE = {
    "SMITH_INCLUDE_ONLY_ACTIVE"};

// Dumps the fr traces and additional information about the Process
// Group.
SMITH_API std::string dump_fr_trace(
    bool includeCollectives,
    bool includeStackTraces,
    bool onlyActive);
...
SMITH_API std::string dump_fr_trace_json(
    bool includeCollectives,
    bool onlyActive);
} // namespace c10d
