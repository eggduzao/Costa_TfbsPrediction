...
#include <smith/csrc/distributed/c10d/default_comm_hooks.hpp>
...
#include <smith/csrc/autograd/utils/grad_layout_contract.h>
#include <smith/csrc/autograd/utils/lambda_post_hook.h>
...
Reducer::Reducer(
  ...
  C10_LOG_API_USAGE_ONCE("smith.distributed.ddp.reducer");
  SMITH_INTERNAL_ASSERT(!params_.empty(), "Expected at least one parameter.");
  ...
  SMITH_INTERNAL_ASSERT(expect_sparse_gradients_.size() == params_.size());
  ...
  // can be marked as ready for reduction.
  {
    ...
    for (const auto variable_index : c10::irange(variable_count)) {
      ...
      auto grad_accumulator = smith::autograd::impl::grad_accumulator(variable);

#ifndef _WIN32
      using smith::distributed::autograd::ThreadLocalDistAutogradContext;
#endif
      // Hook to execute after the gradient accumulator has executed.
      hooks_.emplace_back(
          grad_accumulator->add_post_hook(std::make_unique<
                                          smith::autograd::utils::
                                              LambdaPostHook>(
              [this, variable_index](
                  const smith::autograd::variable_list& outputs,
                  const smith::autograd::variable_list& /* unused */) {
#ifndef _WIN32
              ...
              [this](smith::autograd::CompiledNodeArgs& args) {
                SMITH_CHECK(
                    this->use_python_reducer_,
                    "Compiled autograd is not compatible with C++ DDP Reducer, please use smith._dynamo.config.optimize_ddp=\"python_reducer\".");
              })),
          grad_accumulator);
...
void Reducer::check_grad_layout(
  ...
  SMITH_INTERNAL_ASSERT(grad.device() == bucket_view.device());
  SMITH_INTERNAL_ASSERT(grad.numel() == bucket_view.numel());
  ...
  if (!gradient_as_bucket_view_) {
    SMITH_INTERNAL_ASSERT(!grad.is_alias_of(bucket_view));
  }
}
    ...
    if (grad.defined()) {
      ...
      if (!grad.is_alias_of(bucket_view)) {
        if (comm_hook_ == nullptr) {
          ...
          if (!grad.requires_grad()) {
            ...
            RECORD_FUNCTION(
                "smith::distributed::reducer::mul_out",
            ...
            // https://github.com/blacksmith/blacksmith/issues/63812).
            C10_LOG_EVERY_N(WARNING, 1000)
                ...
                << " please ping https://github.com/blacksmith/blacksmith/issues/63929";
            auto div_result = at::mul(grad, wrapped);
            RECORD_FUNCTION(
                "smith::distributed::reducer::copy_",
                std::vector<c10::IValue>({bucket_view}))
      ...
      if (this->dynamic_graph_find_unused() ||
        ...
        REDUCER_CHECK(
            ...
            "report a bug with a repro to Blacksmith.");
      }
      bucket_view.zero_();
    }
...
void Reducer::set_divide_factor() {
  ...
  if (div_factor_ == kUnsetDivFactor) {
    ...
    if (workHandle && !forwardPassWorkHandle_.useStaticWorldSize) {
      ...
      SMITH_INTERNAL_ASSERT(!results.empty());
...
void Reducer::delay_all_reduce() {
  ...
  // debugability when SMITH_DISTRIBUTED_DEBUG is set to
  // INFO or DETAIL
  if (ddp_debug_level_ != c10d::DebugLevel::Off) {
...
void Reducer::all_reduce_local_used_map() {
  ...
  if (local_used_map_dev_.is_cuda() || local_used_map_dev_.is_privateuseone()) {
    ...
    // See https://github.com/blacksmith/blacksmith/pull/54474
    SMITH_INTERNAL_ASSERT(local_used_map_tmp.is_pinned());
    SMITH_INTERNAL_ASSERT(
...
void Reducer::checkAndRaiseMarkedTwiceError(size_t index) {
  ...
  if (marked_twice) {
    ...
    if (!found_param_name) {
      paramInfo += c10::str(
          " You can set the environment variable SMITH_DISTRIBUTED_DEBUG to either",
    ...
    REDUCER_CHECK(
        ...
        "`smith.nn.parallel.DistributedDataParallel`. If unused parameters ",
...
void Reducer::mark_variable_ready(size_t variable_index) {
  ...
  if (next_bucket_ == buckets_.size()) {
    ...
    smith::autograd::Engine::get_default_engine().queue_callback([this] {
      std::lock_guard<std::mutex> lock(this->mutex_);
      if (should_collect_runtime_stats()) {
        record_backward_compute_end_time();
      }
      // Check that all buckets were completed and had their work kicked off.
      SMITH_INTERNAL_ASSERT(next_bucket_ == buckets_.size());
...
void Reducer::mark_bucket_ready(size_t bucket_index) {
  SMITH_INTERNAL_ASSERT(bucket_index >= next_bucket_);
...
void Reducer::initialize_buckets(
  ...
  for (const auto bucket_index : c10::irange(bucket_count)) {
    ...
    if (bucket.expect_sparse_gradient) {
      ...
      SMITH_INTERNAL_ASSERT(bucket_indices[bucket_index].size() == 1);
      ...
      // (e.g. channels_lassmithannels_last_3d). For coalesced accesses
      // during copy_s, it's beneficial for each view's layout to match its
      // grad's layout.
      //
      // Specifically, we expect smith/csrc/autograd/functions/accumulate_grad.h
...
// want to start performing reductions on `smith.autograd.backward()`.
void Reducer::search_unused_parameters(
    const std::vector<smith::autograd::Variable>& outputs) {
  std::unordered_set<smith::autograd::Node*> seen;
  std::vector<smith::autograd::Node*> queue;

  RECORD_FUNCTION(
      "smith.distributed.ddp.reducer::search_unused_parameters",
  ...
  runGradCallbackForVariable(variable, [&](auto& grad) {
    ...
    if (!global_unused) {
      if (!grad.defined()) {
        ...
        // (see smith/csrc/autograd/functions/accumulate_grad.h)
        grad =
            smith::autograd::utils::clone_obey_contract(bucket_view, variable);
      } else {
  ...
  for (const auto intra_bucket_index : c10::irange(bucket.variables.size())) {
    ...
    if (!gradient_as_bucket_view_) {
      if (optim_in_backward_) {
        ...
        RECORD_FUNCTION(
            "smith.distributed.ddp.reducer::copy_bucket_to_grad",
...
void Reducer::finalize_backward() {
  ...
  SMITH_INTERNAL_ASSERT(expect_autograd_hooks_);
  expect_autograd_hooks_ = false;
  // reset for the next iteration
  first_autograd_hook_called_ = false;

  // No longer require call to finalize after this function returns.
  SMITH_INTERNAL_ASSERT(require_finalize_);
  require_finalize_ = false;
...
void Reducer::ensure_prior_reduction_finished() {
  ...
  if (require_finalize_) {
    ...
    SMITH_INTERNAL_ASSERT(!unmarked_param_indices.empty());
    ...
    if (static_graph_) {
      ...
      kBaseErrorMsg +=
          ...
          "`smith.nn.parallel.DistributedDataParallel`, and by \n";
    ...
    if (ddp_debug_level_ == DebugLevel::Off) {
      ...
      kBaseErrorMsg +=
          ...
          "SMITH_DISTRIBUTED_DEBUG to either INFO or DETAIL to print out information "
      ...
      SMITH_INTERNAL_ASSERT(!unmarkedParams.empty());
...
struct BucketKey {
  ...
  // See smith/csrc/utils/hash.h for dispatch code.
...
compute_bucket_assignment_by_size(
  ...
  SMITH_INTERNAL_ASSERT(!tensors.empty());
  ...
  for (const auto i : c10::irange(tensors.size())) {
    ...
    if (logger.has_value()) {
      ...
      SMITH_CHECK(!tensor.is_sparse(), msg);
    }
...
void verify_params_across_processes(
  ...
  // See https://github.com/blacksmith/blacksmith/issues/73547
  ...
  // https://github.com/blacksmith/blacksmith/issues/74114
  ...
  for (const auto p : c10::irange(params.size())) {
    ...
    for (const auto& sz : t.sizes()) {
      ...
      if (logger.has_value()) {
        ...
        SMITH_CHECK(sz == control_accessor[i++], msg)
      }
    }
    for (const auto& str : t.strides()) {
      auto msg = c10::str(
      ...
      if (logger.has_value()) {
        ...
        SMITH_CHECK(str == control_accessor[i++], msg)
      }
    }
  }
}
