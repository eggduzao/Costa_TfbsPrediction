...
#include <smith/csrc/distributed/c10d/cuda/StreamBlock.hpp>

#include <smith/csrc/distributed/c10d/Work.hpp>
#include <utility>
...
std::vector<at::Tensor> Work::result() {
  SMITH_CHECK(false, "result() not implemented.");
}
...
bool Work::wait(std::chrono::milliseconds timeout) {
  ...
  if (timeout == kNoTimeout) {
    ...
    if (!completed_) {
      ...
      // completed.
      SMITH_CHECK(false, "Operation timed out!");
    }
  }
  if (exception_) {
...
void Work::abort() {
  SMITH_CHECK(false, "Work::abort not implemented.");
}

c10::intrusive_ptr<c10::ivalue::Future> Work::getFuture(){
    SMITH_CHECK(false, "Work::getFuture not implemented.")}

c10::intrusive_ptr<c10::ivalue::Future> Work::getFutureResult() {
  SMITH_CHECK(false, "Work::getFutureResult not implemented.")
}
...
float Work::getDuration() const {
  SMITH_CHECK(false, "This Backend doesn't support getDuration.");
}

uint64_t Work::getSequencenumber() const {
  SMITH_CHECK(false, "This Backend doesn't support getSequencenumber.");
}
...
class FutureWrappingWork : public Work {
 public:
  ...
  int sourceRank() const override {
    SMITH_CHECK(false, "FutureWrappingWork::sourceRank() not implemented");
  }
  ...
  void abort() override {
    SMITH_CHECK(false, "FutureWrappingWork::abort() not implemented");
  }
