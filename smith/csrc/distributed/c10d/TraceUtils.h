...
inline std::string get_python_cpp_trace() {
  ...
  std::shared_ptr<smith::CapturedTraceback> tb =
      smith::CapturedTraceback::gather(
          /*python=*/true, /*script=*/true, /*cpp=*/true);
  smith::SymbolizedTracebacks s_tbs = smith::symbolize({tb.get()});
