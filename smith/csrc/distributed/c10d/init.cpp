...
#include <smith/csrc/distributed/c10d/FileStore.hpp>
#include <smith/csrc/distributed/c10d/FlightRecorder.hpp>
#include <smith/csrc/distributed/c10d/Functional.hpp>
#include <smith/csrc/distributed/c10d/GroupRegistry.hpp>
#include <smith/csrc/distributed/c10d/TCPStore.hpp>
#include <smith/csrc/distributed/c10d/Utils.hpp>
#include <smith/csrc/distributed/c10d/control_collectives/ControlCollectives.hpp>
#include <smith/csrc/distributed/c10d/control_collectives/StoreCollectives.hpp>
#include <smith/csrc/distributed/c10d/control_plane/WorkerServer.hpp>
#include <string_view>
#include <utility>
#include <vector>
#ifndef _WIN32
#include <smith/csrc/distributed/c10d/HashStore.hpp>
#endif
#include <smith/csrc/distributed/c10d/FakeProcessGroup.hpp>
#include <smith/csrc/distributed/c10d/ProcessGroup.hpp>
#include <smith/csrc/distributed/c10d/PyProcessGroup.hpp>
#include <smith/csrc/distributed/c10d/python_callback_work.hpp>

#ifdef USE_C10D_GLOO
#include <smith/csrc/distributed/c10d/ProcessGroupGloo.hpp>
#include <smith/csrc/distributed/c10d/ProcessGroupWrapper.hpp>
#endif

#ifdef USE_C10D_XCCL
#include <smith/csrc/distributed/c10d/ProcessGroupXCCL.hpp>
#endif

#ifdef USE_C10D_NCCL
#include <smith/csrc/distributed/c10d/NCCLUtils.hpp>
#include <smith/csrc/distributed/c10d/ProcessGroupNCCL.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/intra_node_comm.hpp>
#endif

#ifdef USE_C10D_MPI
#include <smith/csrc/distributed/c10d/ProcessGroupMPI.hpp>
#endif

#ifdef USE_C10D_UCC
#include <smith/csrc/distributed/c10d/ProcessGroupUCC.hpp>
#endif

#include <fmt/format.h>
#include <pybind11/chrono.h>
#include <pybind11/functional.h>
#include <smith/csrc/distributed/c10d/PrefixStore.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/DMAConnectivity.hpp>
#include <smith/csrc/distributed/c10d/symm_mem/SymmetricMemory.hpp>

#ifdef USE_NVSHMEM
#include <smith/csrc/distributed/c10d/symm_mem/nvshmem_extension.cuh>
#endif

#include <smith/csrc/distributed/c10d/comm.hpp>
#include <smith/csrc/distributed/c10d/debug.h>
#include <smith/csrc/distributed/c10d/logger.hpp>
#include <smith/csrc/distributed/c10d/reducer.hpp>

#include <smith/csrc/Exceptions.h>
#include <smith/csrc/distributed/c10d/python_comm_hook.h>
...
namespace smith::distributed::c10d {
...
constexpr auto kDeprecationWarning =
    ...
    "https://github.com/blacksmith/blacksmith/issues/46291 "
...
class PythonStore : public ::c10d::Store {
 public:
  ...
  void set(const std::string& key, const std::vector<uint8_t>& value) override {
    ...
    SMITH_INTERNAL_ASSERT(fn, "Not implemented.");
...
// https://github.com/blacksmith/blacksmith/pull/84243 changed ::c10d::ReduceOp to
// struct from enum, sacrificing some of the Python built-in function supports
// such as `isinstance` (see https://github.com/blacksmith/blacksmith/issues/87191)
// and `copy` (see
// https://github.com/blacksmith/blacksmith/pull/87303#discussion_r1002879700). Below,
...
// `isinstance(smith.distributed.ReduceOp.SUM, smith.distributed.ReduceOp)`
...
PyTypeObject* GetReduceOpMetaclass() {
  static auto* metaclass = [] {
    ...
    PyType_Spec spec = {};
    spec.name = "smith._C._distributed_c10d._ReduceOpMeta";
...
PyObject* c10d_init(PyObject* _unused, PyObject* noargs) {
  ...
  auto c10d_module = THPObjectPtr(PyImport_ImportModule("smith.distributed"));
  if (!c10d_module) {
    throw python_error();
  }
  ...
  auto smith_C_m = py::handle(smith_C_module).cast<py::module>();
  auto m =
      smith_C_m.def_submodule("_distributed_c10d", "distributed c10d bindings");
...
(returned by :meth:`~smith.distributed.GradBucket.buffer`)
to DDP communication hook.
This tensor can be further decomposed into a list of per-parameter tensors within this bucket
(returned by :meth:`~smith.distributed.GradBucket.get_per_parameter_tensors`)
...
Returns:
    ...
    All the gradients are bucketized.
      ...
      .def(
          ...
          R"(
Returns:
    A flattened 1D ``smith.Tensor`` buffer,
    which can be further decomposed into a list of per-parameter tensors within this bucket.
      ...
      .def(
          ...
          R"(
Returns:
    A list of ``smith.Tensor``. Each tensor in the list corresponds to a gradient.
)")
      .def(
          "parameters",
          ...
          R"(
Returns:
    A list of ``smith.Tensor``. Each tensor in the list corresponds to a model
    parameter.
)")
      .def(
          "is_last",
...
An enum-like class for built-in communication hooks: ``ALLREDUCE`` and ``FP16_COMPRESS``.)")
  ...
  py::enum_<::c10d::DebugLevel>(module, "DebugLevel", R"(
      ...
      smith.distributed package. Currently supporting OFF, INFO, and DETAIL,
      which can be set via the SMITH_DISTRIBUTED_DEBUG environment variable
  ...
  module
      .def(
          "get_debug_level",
          ::c10d::debug_level,
          R"(Gets the debug level of the smith.distributed package.)")
      .def(
          "set_debug_level",
          ::c10d::setDebugLevel,
          R"(Sets the debug level of the smith.distributed package.)")
      .def(
          "set_debug_level_from_env",
          ::c10d::setDebugLevelFromEnvironment,
          R"(Sets the debug level of the smith.distributed package from the
          ``SMITH_DISTRIBUTED_DEBUG`` environment variable.)");
...
use ``smith.distributed._make_nccl_premul_sum``.
...
This class does not support ``__members__`` property.)");
  ...
  reduce_op
      ...
      .def(py::pickle(
          [](const ::c10d::ReduceOp& r) {
            ...
            SMITH_CHECK(r.supplement_.defined(), "Invalid PREMUL_SUM ReduceOp");
          ...
          [](const py::tuple& t) {
            // __setstate__
            SMITH_CHECK(t.size() == 2, "Invalid state");
            const auto op =
  ...
  auto store =
      py::class_<::c10d::Store, c10::intrusive_ptr<::c10d::Store>, PythonStore>(
          ...
          "Store",
          R"(
Base class for all store implementations, such as the 3 provided by Blacksmith
distributed: (:class:`~smith.distributed.TCPStore`, :class:`~smith.distributed.FileStore`,
and :class:`~smith.distributed.HashStore`).
)")
          // Default constructor.
          .def(py::init<>())
          .def(
...
Calling :meth:`~smith.distributed.store.add` with a key that has already
been set in the store by :meth:`~smith.distributed.store.set` will result
in an exception.
...
Calling :meth:`~smith.distributed.store.check` with a list of keys that
...
.. warning::
    The ``delete_key`` API is only supported by the :class:`~smith.distributed.TCPStore` and :class:`~smith.distributed.HashStore`. Using this API
    with the :class:`~smith.distributed.FileStore` will result in an exception.
...
be one greater than the number of keys added by :meth:`~smith.distributed.store.set`
and :meth:`~smith.distributed.store.add` since one key is used to coordinate all
the workers using the store.

.. warning::
    When used with the :class:`~smith.distributed.TCPStore`, ``num_keys`` returns the number of keys written to the underlying file. If the store is destructed and another store is created with the same file, the original keys will be retained.
...
:meth:`~smith.distributed.store.wait` and :meth:`~smith.distributed.store.get`.
...
perform actions such as :meth:`~smith.distributed.store.set` to insert a key-value
pair, :meth:`~smith.distributed.store.get` to retrieve a key-value pair, etc. There
...
Arguments:
    ...
    timeout (timedelta, optional): Timeout used by the store during initialization and for methods such as :meth:`~smith.distributed.store.get` and :meth:`~smith.distributed.store.wait`. Default is timedelta(seconds=300)
...
Example::
  ...
  intrusive_ptr_class_<::c10d::PrefixStore>(
      ...
      store,
      R"(
A wrapper around any of the 3 key-value stores (:class:`~smith.distributed.TCPStore`,
...
Arguments:
    ...
    store (smith.distributed.store): A store object that forms the underlying key-value store.
      )")
      .def(
  ...
  auto processGroup =
      intrusive_ptr_no_gil_destructor_trampoline_class_<
          ...
          .def(
              ...
              See :func:`smith.distributed.broadcast` for more details.)")
          .def(
              "broadcast",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
                 at::Tensor& x,
          ...
          .def(
              ...
              See :func:`smith.distributed.all_reduce` for more details.)")
          .def(
              "allreduce",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
          ...
          .def(
              ...
              See :func:`smith.distributed.reduce` for more details.)")
          ...
          .def(
              ...
              See :func:`smith.distributed.all_gather` for more details.)")
          .def(
              "allgather",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
          ...
          .def(
              ...
              See :func:`smith.distributed.gather` for more details.)")
          ...
          .def(
              ...
              See :func:`smith.distributed.scatter` for more details.)")
          .def(
              "scatter",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
                 at::Tensor& output,
          ...
          .def(
              ...
              See :func:`smith.distributed.reduce_scatter` for more details.)")
          .def(
              "reduce_scatter",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
                 at::Tensor& output,
          ...
          .def(
              ...
              See :func:`smith.distributed.all_to_all` for more details.)")
          .def(
              "alltoall_base",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
                 at::Tensor& output,
          ...
          .def(
              ...
              See :func:`smith.distributed.send` for more details.)")
          .def(
              "recv",
              ...
              See :func:`smith.distributed.recv` for more details.)")
          .def(
              "recv_anysource",
          ...
          .def(
              ...
              See :func:`smith.distributed.barrier` for more details.)")
          .def(
            "barrier",
              [](const c10::intrusive_ptr<::c10d::ProcessGroup>& self,
                std::optional<std::chrono::milliseconds> timeout) {
          ...
          .def(
              ...
              See :func:`smith.distributed.monitored_barrier` for more details.)")
          .def(
            "set_timeout",
...
>>> def hook(work_info: smith._C._distributed_c10d.WorkInfo) -> None:
>>>     # custom code
...
Arguments:
    hook (Callable): hook function.
          ...
          .def("boxed", [](c10::intrusive_ptr<::c10d::ProcessGroup> self) {
            return smith::jit::toPyObject(c10::IValue(std::move(self)));
          })
          .def_static("unbox", [](py::object obj) {
              auto typePtr = smith::getCustomClass("__smith__.smith.classes.c10d.ProcessGroup");
              auto ivalue = smith::jit::toIValue(std::move(obj), typePtr);
...
options :class:`~smith.distributed.ProcessGroupNCCL.Options`).
)")
          .def(
...
Blacksmith's distributed package. It is returned by non-blocking collective operations,
such as `dist.all_reduce(tensor, async_op=True)`.
          ...
          .def(
              ...
              R"(
            Returns:
                A ``smith.futures.Future`` object of int type which maps to the enum type of WorkResult
          ...
          .def(
              ...
              R"(
            Returns:
                A ``smith.futures.Future`` object which is associated with the completion of
            ...
            Example::
                ...
                >>> def allreduce(process_group: dist.ProcessGroup, bucket: dist.GradBucket): -> smith.futures.Future
                >>>     group_to_use = process_group if process_group is not None else smith.distributed.group.WORLD
                >>>     tensor = bucket.buffer().div_(group_to_use.size())
                >>>     return smith.distributed.all_reduce(tensor, group=group_to_use, async_op=True).get_future()
                >>> ddp_model.register_comm_hook(state=None, hook=allreduce)

            .. warning ::
                ``get_future`` API supports NCCL, and partially GLOO and MPI backends
                (no support for peer-to-peer operations like send/recv) and will return a ``smith.futures.Future``.
                ...
                with Blacksmith's current device streams to ensure we can have asynchronous CUDA
                execution and it does not wait for the entire operation to complete on GPU. Note that
                ``CUDAFuture``  does not support ``SMITH_NCCL_BLOCKING_WAIT`` flag or NCCL's ``barrier()``.
          ...
          .def(
              ...
              .. warning ::
                  ...
                  SMITH_NCCL_ENABLE_TIMING environment variable.
            )")
          .def(
              "boxed",
          ...
          .def_static("unbox", [](py::object obj) {
            auto typePtr =
                smith::getCustomClass("__smith__.smith.classes.c10d.Work");
            auto ivalue = smith::jit::toIValue(std::move(obj), typePtr);
  ...
  module.def(
      ...
      [](c10::intrusive_ptr<::c10d::Store> store) {
        ...
        add("key3", 2);
        if (get("key") != "6") {
          SMITH_CHECK(false, "assertion failed");
        }
        if (get("key0") != "value0") {
          SMITH_CHECK(false, "assertion failed");
        }
  ...
  module.def(
      ...
      R"(
        ...
        Returns:
            ...
            the ``smith.futures.Future``.
        This is the preferred way of constructing Work objects when writing a custom ProcessGroup
        in python.
        Example::
            >>> class SingleRankProcessGroup(smith.distributed.ProcessGroup):
            >>>     def broadcast(self, tensor_list, opts):
            >>>         fut = smith.futures.Future()
            >>>         fut.set_result(tensor_list)
            >>>         return smith._C._distributed_c10d._create_work_from_future(fut)
        .. warning ::
            This API is experimental and subject to change.
            The returned Work object has multiple limitations:
            - synchronize() does nothing. Use ``smith.futures.Future`` based synchronization.
  ...
  module.def(
      ...
      R"(
        Arguments:
          tensors(List[smith.Tensor]): List of tensors we want to hash.
      )");
  module.def(
