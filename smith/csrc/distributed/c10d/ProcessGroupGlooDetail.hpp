...
#define GENERATE_ALL_TYPES(type, func, ...)      \
  switch (type) {                                \
    ...
    case ::at::ScalarType::Long:                 \
      ...
      break;                                     \
    default:                                     \
      SMITH_CHECK(false, "Invalid scalar type"); \
  }
...
// on the tensors.
SMITH_API void initializeStreamsEvents(
...
ReduceFunc toFunction(const ReduceOp& r) {
  switch (r) {
    ...
    case ReduceOp::BAND:
      SMITH_CHECK(false, "Cannot use ReduceOp.BAND with non-integral dtype");
      break;
    case ReduceOp::BOR:
      SMITH_CHECK(false, "Cannot use ReduceOp.BOR with non-integral dtype");
      break;
    case ReduceOp::BXOR:
      SMITH_CHECK(false, "Cannot use ReduceOp.BXOR with non-integral dtype");
      break;
    case ReduceOp::PREMUL_SUM:
      SMITH_CHECK(false, "Cannot use ReduceOp.PREMUL_SUM with Gloo");
      break;
  ...
  SMITH_CHECK(false, "Unhandled ReduceOp");
}
...
class AsyncSparseAllreduceWork : public ProcessGroupGloo::AsyncWork {
 public:
  ...
  at::Tensor allreduce(std::vector<at::Tensor>& tensors) {
    ...
    // but to conveniently do this c10d must depend on smith not ATen
    ...
    // Sanity check dimensionality across ranks.
    {
      ...
      for (const auto i : c10::irange(context_->size)) {
        ...
        SMITH_CHECK(actual == expected, "Sparse dimensions do not match");
      }
    }
