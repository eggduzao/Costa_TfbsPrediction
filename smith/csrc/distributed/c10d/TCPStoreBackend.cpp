...
#ifdef _WIN32
#include <smith/csrc/distributed/c10d/WinSockUtils.hpp>
#else
#include <smith/csrc/distributed/c10d/UnixSockUtils.hpp>
#endif

#include <smith/csrc/distributed/c10d/socket.h>
  ...
  if (controlPipeFd_[1] != -1) {
    ...
    while (true) {
      ...
      if (written_bytes < 0) {
        ...
        SMITH_CHECK(false, "Failed to write the control pipe:", errno);
      }
      break;
    }
    if (written_bytes == 0) {
      SMITH_CHECK(false, "Failed to write the control pipe");
    }
...
void TCPStoreMasterDaemon::query(int socket) {
  ...
  if (isMiscellaneousSocket(socket)) {
    ...
    SMITH_CHECK(false, "Unexpected query type");
  }
}
