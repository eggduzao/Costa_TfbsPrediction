...
#include <smith/csrc/distributed/c10d/UCCTracing.hpp>
#include <smith/csrc/distributed/c10d/UCCUtils.hpp>
#include <list>
#include <memory>
...
ucc_datatype_t to_ucc_dType(at::Tensor _tensor) {
  ...
  try {
    ...
    SMITH_CHECK(false, "Not supported data type for UCC");
  }
}
...
ucc_reduction_op_t to_ucc_reduceOp(
  ...
  if (_dt == at::kBool) {
    if (_op == ReduceOp::SUM) {
      ...
      SMITH_CHECK(false, "Cannot use ReduceOp.AVG with boolean inputs");
    }
  }
  ...
  try {
    ...
    SMITH_CHECK(false, "Not supported ReduceOp for UCC");
  }
}

struct smith_ucc_config_t {
  c10::once_flag flag;
...
std::unordered_map<std::string, std::string> smith_ucc_envs_map = {
    // SMITH_UCC_BLOCKING_WAIT allowed syntax:
    // - SMITH_UCC_BLOCKING_WAIT=none --> blocking wait completely disabled
    // - SMITH_UCC_BLOCKING_WAIT=all --> blocking wait completely enabled
    // - SMITH_UCC_BLOCKING_WAIT=allreduce,send,recv --> blocking wait enabled
    ...
    {"SMITH_UCC_BLOCKING_WAIT", "none"},

    {"SMITH_UCC_USE_FUTURE", "1"},
    {"SMITH_UCC_PROFILING_ENABLE", "0"},
    {"SMITH_UCC_SHARED_COMM", "1"},
    {"SMITH_UCC_USE_ALLGATHERV", "0"},
    {"SMITH_UCC_ENABLE_HEALTH_CHECK", "0"},
    {"SMITH_UCC_ENABLE_COMMS_LOGGER", "0"},
};
...
void read_config() {
  ...
  smith_ucc_config.blocking_wait.fill(false);
  smith_ucc_config.use_future = true;
  smith_ucc_config.shared_comm = false;
  smith_ucc_config.use_allgatherv = false;
  smith_ucc_config.enable_health_check = false;
  smith_ucc_config.enable_comms_logger = false;

  // read all smith_ucc env. variables and update the map
  for (auto& [env_name, value] : smith_ucc_envs_map) {
  ...
  auto blocking_wait_str = smith_ucc_envs_map.at("SMITH_UCC_BLOCKING_WAIT");
  for (auto op : parse_blocking_wait(blocking_wait_str)) {
    smith_ucc_config.blocking_wait[(std::uint8_t)op] = true;
  }
  // barrier is always blocking
  smith_ucc_config.blocking_wait[(std::uint8_t)OpType::BARRIER] = true;

  smith_ucc_config.use_future =
      std::stoi(smith_ucc_envs_map.at("SMITH_UCC_USE_FUTURE"));
  smith_ucc_config.shared_comm =
      std::stoi(smith_ucc_envs_map.at("SMITH_UCC_SHARED_COMM"));
  smith_ucc_config.use_allgatherv =
      std::stoi(smith_ucc_envs_map.at("SMITH_UCC_USE_ALLGATHERV"));
  smith_ucc_config.enable_health_check =
      std::stoi(smith_ucc_envs_map.at("SMITH_UCC_ENABLE_HEALTH_CHECK"));
  smith_ucc_config.enable_comms_logger =
      std::stoi(smith_ucc_envs_map.at("SMITH_UCC_ENABLE_COMMS_LOGGER"));
}
...
bool ProcessGroupUCC::WorkUCC::wait(std::chrono::milliseconds /* unused */) {
  if (smith_ucc_config.enable_comms_logger && logger_) {
    logger_->trace_generator->recordComms("wait", (uintptr_t)this, rank_);
  }
#ifdef USE_CUDA
  if (fence && !smith_ucc_config.blocking_wait[(int)opType_]) {
    // block user stream
...
Comm::Comm(
    ...
    std::shared_ptr<smith_ucc_oob_coll_info_t> oob_,
    c10::Device dev,
    ...
    : logger(logger_),
      ...
      ucc_comm(oob, logger),
      finalize_phase(
          is_health_check ? SMITH_UCC_HEALTH_CHECK : SMITH_UCC_FINALIZE),
      cuda_device_index(SMITH_UCC_DEVICE_NOT_SET) {
  if (dev.is_cuda()) {
...
std::shared_ptr<Comm> Comm::get_comm(
    uint32_t& id,
    c10::Device dev,
    std::shared_ptr<smith_ucc_oob_coll_info_t> oob,
  ...
  if (smith_ucc_config.shared_comm) {
    ...
    if (!shared_comm) {
      ...
      if (dev.is_cuda() && !is_health_check) {
        if ((shared_comm->cuda_device_index != SMITH_UCC_DEVICE_NOT_SET) &&
            (shared_comm->cuda_device_index != dev.index())) {
          SMITH_UCC_LOG_ERROR(
              is_health_check ? SMITH_UCC_HEALTH_CHECK : SMITH_UCC_INIT,
...
void Comm::ucc_create_team(
  ...
  SMITH_UCC_CHECK(st, "failed to create UCC team");
}
...
void Comm::enqueue_cuda_collective(
  ...
  SMITH_CHECK(st == UCC_OK && post_ev->ev_type == UCC_EVENT_COLLECTIVE_POST);
...
void Comm::progress_loop() {
  ...
  while (!stop_progress_loop) {
    ...
    lock.unlock();
#ifdef USE_CUDA
    if ((!device_set) && (cuda_device_index != SMITH_UCC_DEVICE_NOT_SET)) {
    ...
    try {
      ...
      if (work->request_->status < 0) {
        ...
        SMITH_UCC_LOG_ERROR(SMITH_UCC_COLL_PROGRESS, err_log);
      }
    } catch (...) {
...
ProcessGroupUCC::ProcessGroupUCC(
  ...
  c10::call_once(smith_ucc_config.flag, read_config);
  oob = std::make_shared<smith_ucc_oob_coll_info_t>();
  oob->rank = rank;
  oob->size = size;
  oob->store = store;
  comm = nullptr;
  ...
  SMITH_UCC_LOG_INFO(
      ...
      c10::str(
          ...
          " secs"));
  std::string envs = "";
  for (auto& smith_ucc_env : smith_ucc_envs_map) {
    envs += ("\n\t" + smith_ucc_env.first + "=" + smith_ucc_env.second);
  }
  SMITH_UCC_LOG_INFO(
      SMITH_UCC_INIT,
      c10::str(
  ...
  if (smith_ucc_config.enable_health_check) {
  ...
  if (smith_ucc_config.enable_comms_logger) {
...
ProcessGroupUCC::~ProcessGroupUCC() {
  ...
  if (comm) {
    logger->setPhase(SMITH_UCC_FINALIZE);
    comm->ucc_destroy_team(team);
    SMITH_UCC_LOG_INFO(
        SMITH_UCC_FINALIZE, "Successfully destroyed UCC library");
    try {
      if (cuda_ee) {
...
c10::Device getCUDADeviceForRank(int rank) {
  SMITH_CHECK(rank >= 0, "Invalid rank ", rank);
...
void ProcessGroupUCC::runHealthCheck() {
  ...
  auto t = std::thread([&healthCheckData, this]() {
    ...
    for (auto device : devices) {
      ...
      try {
        auto oob = std::make_shared<smith_ucc_oob_coll_info_t>();
...
c10::intrusive_ptr<Work> ProcessGroupUCC::collective_post(
  ...
  switch (dev.type()) {
    case c10::DeviceType::CPU: {
      if (smith_ucc_config.use_future) {
    ...
    default: {
      SMITH_UCC_LOG_ERROR(
          SMITH_UCC_COLL_POST, c10::str("unsupported device type ", dev.str()));
      throw std::invalid_argument(ucc_status_string(UCC_ERR_NOT_SUPPORTED));
    }
  }
}
...
c10::intrusive_ptr<Work> ProcessGroupUCC::allgather(
  ...
  if (tensor.device().is_cpu() || smith_ucc_config.use_allgatherv) {
...
c10::intrusive_ptr<Work> ProcessGroupUCC::barrier(const BarrierOptions& opts) {
  ...
  } else if (comm && comm->cuda_device_index != SMITH_UCC_DEVICE_NOT_SET) {
...
c10::intrusive_ptr<Work> ProcessGroupUCC::gather(
  ...
  if (getRank() == opts.rootRank) {
    ...
    if (!outputTensors.empty()) {
      SMITH_UCC_LOG_ERROR(
          SMITH_UCC_COLL_POST, "requires empty output on non-root");
    }
    outputs = {};
...
void ProcessGroupUCC::initComm(c10::Device dev) {
  if (!comm) {
    ...
    SMITH_UCC_LOG_INFO(SMITH_UCC_INIT, "Successfully initialized UCX library");
    comm->ucc_create_team(team, oob);
    SMITH_UCC_LOG_INFO(SMITH_UCC_INIT, "Successfully initialized UCC library");
    logger->setPhase(SMITH_UCC_READY);
  } else {
    if (dev.is_cuda()) {
      if ((comm->cuda_device_index != SMITH_UCC_DEVICE_NOT_SET) &&
