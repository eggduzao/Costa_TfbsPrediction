...
#include <smith/csrc/distributed/c10d/Backoff.hpp>
#include <smith/csrc/distributed/c10d/TCPStore.hpp>
#include <smith/csrc/distributed/c10d/TCPStoreBackend.hpp>
#include <smith/csrc/distributed/c10d/Utils.hpp>
#include <smith/csrc/distributed/c10d/logging.h>
...
TCPStore::TCPStore(std::string host, const TCPStoreOptions& opts)
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__init);

  if (opts.useLibUV) {
    SMITH_CHECK_WITH(
        DistStoreError,
        ::c10d::detail::is_libuv_tcpstore_backend_available(),
        "use_libuv was requested but Blacksmith was built without libuv support, run with USE_LIBUV=0 to disable it.");
  }
  ...
  if (opts.isServer) {
    try {
      ...
      bool useAgentStore = getCvarBool({"SMITHELASTIC_USE_AGENT_STORE"}, false);
      int masterPort = getCvarInt({"MASTER_PORT"}, 0);
      if (useAgentStore && masterPort == opts.port) {
        C10D_ERROR(
            "The server socket on {} has failed to bind. "
            "SMITHELASTIC_USE_AGENT_STORE is enabled so ignoring the error.",
            opts.port);
      } else {
        throw;
      }
...
void TCPStore::waitForWorkers() {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__waitForWorkers);
...
void TCPStore::set(const std::string& key, const std::vector<uint8_t>& data) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__set);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  detail::SendBuffer buffer(*client_, detail::QueryType::SET);
  buffer.appendString(keyPrefix_ + key);
  buffer.appendBytes(data);
...
std::vector<uint8_t> TCPStore::compareSet(
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__compareSet);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
std::vector<uint8_t> TCPStore::get(const std::string& key) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__get);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
int64_t TCPStore::add(const std::string& key, int64_t value) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__add);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  return incrementValueBy(keyPrefix_ + key, value);
}

bool TCPStore::deleteKey(const std::string& key) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__delete);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
bool TCPStore::check(const std::vector<std::string>& keys) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__check);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
void TCPStore::wait(
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__wait);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
void TCPStore::append(
    const std::string& key,
    const std::vector<uint8_t>& data) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__append);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
...
std::vector<std::vector<uint8_t>> TCPStore::multiGet(
    const std::vector<std::string>& keys) {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__multiGet);
  const std::lock_guard<std::mutex> lock(activeOpLock_);
  std::vector<std::string> prefixedKeys;
  prefixedKeys.reserve(keys.size());
  for (const std::string& key : keys) {
...
void TCPStore::multiSet(
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__multiSet);
  SMITH_CHECK_WITH(
      DistStoreError,
...
void TCPStore::queuePush(
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__queuePush);
...
std::vector<uint8_t> TCPStore::queuePop(const std::string& key, bool block) {
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__queuePop);
  ...
  SMITH_CHECK_WITH(DistQueueEmptyError, keys > 0, "queue is empty");
...
int64_t TCPStore::queueLen(const std::string& key) {
  ...
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__queueLen);
...
std::vector<std::string> TCPStore::listKeys() {
  STATIC_SCOPED_WAIT_COUNTER(blacksmith.wait_counter.TCPStore__list);
