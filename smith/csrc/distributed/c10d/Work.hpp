...
SMITH_API std::string opTypeToString(OpType opType);

// Whether or not an OP is an p2p op (SEND, RECV, RECVANYSOURCE)
SMITH_API bool isP2POp(OpType opType, bool batchP2P = false);
...
class SMITH_API Work : public smith::CustomClassHolder {
 public:
  Work(
      int rank = -1,
