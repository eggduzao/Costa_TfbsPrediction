...
#include <smith/csrc/distributed/c10d/reducer_timer.hpp>
#ifndef _WIN32
#include <smith/csrc/distributed/autograd/context/context.h>
#endif
...
class SMITH_API Reducer {
 ...
 protected:
  ...
  std::vector<std::shared_ptr<smith::autograd::Node>>
      grad_accumulators_; // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
  // NOLINTNEXTLINE(cppcoreguidelines-non-private-member-variables-in-classes)
  std::unordered_map<smith::autograd::Node*, size_t> gradAccToVariableMap_;
  std::vector<std::pair<uintptr_t, std::shared_ptr<smith::autograd::Node>>>
  ...
  static_assert(
      std::is_same_v<
          GradCallback,
          smith::distributed::autograd::DistAutogradContext::GradCallback>);
#endif
  ...
  struct Bucket {
    ...
    // smith/csrc/autograd/functions/accumulate_grad.h).
...
#ifndef _WIN32
  struct RpcContext {
    using ContextPtr = smith::distributed::autograd::ContextPtr;
...
// bucket is i.
SMITH_API std::tuple<std::vector<std::vector<size_t>>, std::vector<size_t>>
...
SMITH_API void verify_params_across_processes(
