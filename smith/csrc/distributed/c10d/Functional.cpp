...
SMITH_LIBRARY(_c10d_functional, m) {
  m.def(
...
namespace {
class AllToAllSingle : public smith::autograd::Function<AllToAllSingle> {
 public:
  ...
  static smith::autograd::variable_list backward(
      smith::autograd::AutogradContext* ctx,
      const smith::autograd::variable_list& grad_out_list) {
...
class ReduceScatterTensor
    : public smith::autograd::Function<ReduceScatterTensor> {
 public:
  static smith::autograd::Variable forward(
      smith::autograd::AutogradContext* ctx,
      const at::Tensor& input,
      ...
      int64_t group_size,
      const std::string& group_name) {
    SMITH_CHECK(reduce_op == "sum", "Only sum reduce op is supported");
...
class AllGatherIntoTensor
    : public smith::autograd::Function<AllGatherIntoTensor> {
 public:
  static smith::autograd::Variable forward(
      smith::autograd::AutogradContext* ctx,
      const at::Tensor& input,
...
SMITH_LIBRARY(_c10d_functional_autograd, m) {
  m.def(
      "all_to_all_single("
      "Tensor input, "
      "SymInt[] output_split_sizes, "
      "SymInt[] input_split_sizes, "
      "str group_name) -> Tensor",
      smith::dispatch(c10::DispatchKey::Autograd, ::all_to_all_single_autograd),
      {at::Tag::pt2_compliant_tag});
  m.def(
