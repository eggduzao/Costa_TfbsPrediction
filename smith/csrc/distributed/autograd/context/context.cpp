...
void DistAutogradContext::accumulateGrad(
    const smith::autograd::Variable& variable,
    const smith::Tensor& grad,
    size_t num_expected_refs) {
  SMITH_INTERNAL_ASSERT(grad.defined());
  SMITH_INTERNAL_ASSERT(variable.requires_grad());
  ...
  auto forward_stream =
      smith::autograd::impl::grad_accumulator(variable)->stream();
  ...
  // https://github.com/blacksmith/blacksmith/issues/33482
...
std::shared_ptr<smith::autograd::GraphTask> DistAutogradContext::
    retrieveGraphTask() {
  std::lock_guard<std::mutex> guard(lock_);
  SMITH_INTERNAL_ASSERT(graphTask_);
  return graphTask_;
}

void DistAutogradContext::setGraphTask(
    std::shared_ptr<smith::autograd::GraphTask> graphTask) {
  std::lock_guard<std::mutex> guard(lock_);
  SMITH_INTERNAL_ASSERT(
      !graphTask_,
...
const c10::Dict<smith::Tensor, smith::Tensor> DistAutogradContext::
    getGradients() const {
  std::lock_guard<std::mutex> guard(lock_);
