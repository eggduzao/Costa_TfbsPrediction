...
class SMITH_API DistAutogradContext {
 public:
  using GradCallback = std::function<bool(smith::Tensor&)>;
  ...
  const c10::Dict<smith::Tensor, smith::Tensor> getGradients() const;
 ...
 private:
  ...
  std::shared_ptr<smith::autograd::GraphTask> retrieveGraphTask();

  // Set the appropriate graph task for the backward pass. Can be called only
  // once.
  void setGraphTask(std::shared_ptr<smith::autograd::GraphTask> graphTask);
  ...
  c10::Dict<smith::Tensor, smith::Tensor> accumulatedGrads_;
  ...
  std::shared_ptr<smith::autograd::GraphTask> graphTask_;
...
class SMITH_API ThreadLocalDistAutogradContext {
 public:
