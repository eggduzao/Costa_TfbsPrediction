...
PyObject* dist_autograd_init(PyObject* _unused, PyObject* noargs) {
  auto autograd_module =
      THPObjectPtr(PyImport_ImportModule("smith.distributed.autograd"));
  if (!autograd_module) {
    throw python_error();
  }
                ...
                for (const auto& map_entry : recvFunctions) {
                  funcs.emplace(
                      ...
                      py::reinterpret_steal<py::object>(
                          smith::autograd::functionToPyObject(
                              map_entry.second)));
                }
                return funcs;
              },
...
:class:`smith.distributed.autograd.context` on each of the nodes. The autograd
context to be used is looked up given the ``context_id`` that is passed in when
:meth:`smith.distributed.autograd.backward` is called. If there is no valid
...
Example::
  ...
  module.def(
      ...
      [](int64_t contextId) -> py::dict {
        ...
        pybind11::gil_scoped_acquire ag;
        return smith::jit::toPyObject(ival);
      },
      R"(
