...
c10::intrusive_ptr<Message> RpcWithAutograd::toMessageImpl() && {
  ...
  SMITH_INTERNAL_ASSERT(!payload.empty());
  ...
  SMITH_INTERNAL_ASSERT(tensorTable.empty());
...
std::unique_ptr<RpcWithAutograd> RpcWithAutograd::fromMessage(
  ...
  SMITH_INTERNAL_ASSERT(tupleElements.size() == 5);
  MessageType wrappedMessageType =
...
std::vector<smith::Tensor>& RpcWithAutograd::tensors() {
  return tensors_;
}
