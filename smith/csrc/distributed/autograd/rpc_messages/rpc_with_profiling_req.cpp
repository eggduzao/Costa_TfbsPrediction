...
RpcWithProfilingReq::RpcWithProfilingReq(
    ...
    : messageType_(messageType),
      ...
      profilerConfig_(std::move(profilerConfig)),
      profilingKeyId_(profilingKeyId) {
  SMITH_INTERNAL_ASSERT(wrappedRpc_ != nullptr, "wrappedRpc can't be null");
}
...
smith::autograd::profiler::ProfilerConfig RpcWithProfilingReq::
...
std::unique_ptr<RpcWithProfilingReq> RpcWithProfilingReq::fromMessage(
  ...
  smith::autograd::profiler::ProfilerConfig cfg =
      smith::autograd::profiler::ProfilerConfig::fromIValue(tupleElements[1]);
