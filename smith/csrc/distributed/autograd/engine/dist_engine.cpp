...
#include <smith/csrc/autograd/input_buffer.h>
#include <smith/csrc/distributed/autograd/context/container.h>
#include <smith/csrc/distributed/autograd/engine/dist_engine.h>

namespace smith::distributed::autograd {

using smith::autograd::AccumulateGrad;
using smith::autograd::edge_list;
using smith::autograd::Engine;
using smith::autograd::GraphRoot;
using smith::autograd::GraphTask;
using smith::autograd::GraphTaskGuard;
using smith::autograd::InputBuffer;
using smith::autograd::Node;
using smith::autograd::NodeTask;
using smith::autograd::ReadyQueue;
using smith::autograd::validate_outputs;
using smith::autograd::variable_list;
...
void DistEngine::globalCpuThread(
  ...
  while (true) {
    ...
    if (task.isShutdownTask_) {
      ...
      C10_LOG_API_USAGE_ONCE("smith.autograd.thread_shutdown");
      break;
    }
...
DistEngine::~DistEngine() {
  ...
  SMITH_ASSERT_NO_GIL_WITHOUT_PYTHON_DEP();
...
void DistEngine::validateRootsAndRetrieveEdges(
    ...
    edge_list& rootEdges,
    variable_list& grads) {
  SMITH_CHECK(!roots.empty(), "No tensors provided for gradient computation.");
  SMITH_INTERNAL_ASSERT(rootEdges.empty());
  SMITH_INTERNAL_ASSERT(grads.empty());

  // Verify roots are all scalar and require gradients.
  for (const auto& root : roots) {
    SMITH_CHECK(root.requires_grad(), "requires_grad not set on root");
    SMITH_CHECK(
        root.numel() == 1,
        root.name(),
    ...
    rootEdges.push_back(smith::autograd::impl::gradient_edge(root));
...
void DistEngine::computeDependencies(
    ...
    bool retainGraph) {
  SMITH_INTERNAL_ASSERT(graphRoot, "graphRoot is null!");
  ...
  graphTask->owner_ = smith::autograd::CPU_DEVICE;
...
void DistEngine::execute_graph_task_until_ready_queue_empty(
  ...
  smith::autograd::set_device(smith::autograd::CPU_DEVICE);
...
c10::intrusive_ptr<c10::ivalue::Future> DistEngine::
  ...
  futureGrads->addCallback([autogradContext, outputEdges, accumulateGradFuture](
    ...
    try {
      ...
      SMITH_INTERNAL_ASSERT(grads.size() == outputEdges.size());
...
void DistEngine::cleanupBackwardPass(const ContextPtr& autogradContext) {
  ...
  SMITH_INTERNAL_ASSERT(futureGrads.use_count() == 1);
