...
class SMITH_API DistEngine {
 public:
  ...
  void execute(
      int64_t context_id,
      const smith::autograd::variable_list& roots,
      bool retainGraph);
 ...
 private:
  ...
  void validateRootsAndRetrieveEdges(
      const smith::autograd::variable_list& roots,
      smith::autograd::edge_list& rootEdges,
      smith::autograd::variable_list& grads);
  ...
  void computeDependencies(
      ...
      const smith::autograd::edge_list& rootEdges,
      const smith::autograd::variable_list& grads,
      const std::shared_ptr<smith::autograd::Node>& graphRoot,
      smith::autograd::edge_list& outputEdges,
      bool retainGraph);
  ...
  void execute_graph_task_until_ready_queue_empty(
      smith::autograd::NodeTask&& node_task,
  ...
  c10::intrusive_ptr<c10::ivalue::Future> runEngineAndAccumulateGradients(
      const ContextPtr& autogradContext,
      const std::shared_ptr<smith::autograd::Node>& graphRoot,
      const smith::autograd::edge_list& outputEdges,
      bool incrementOutStandingTasks = true);
  ...
  void globalCpuThread(
      const std::shared_ptr<smith::autograd::ReadyQueue>& ready_queue);
  ...
  smith::autograd::Engine& engine_;

  // Ready queue used by the CPU thread in distributed engine.
  // See Note [GPU to CPU continuations]
  std::shared_ptr<smith::autograd::ReadyQueue> global_cpu_ready_queue_;
