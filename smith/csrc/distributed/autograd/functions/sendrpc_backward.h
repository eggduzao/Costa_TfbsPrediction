...
struct SMITH_API SendRpcBackward : public smith::autograd::Node {
 public:
  smith::autograd::variable_list apply(
      smith::autograd::variable_list&& inputs) override;
  ...
  // computation.
  void setGrads(const smith::autograd::variable_list& grads);

  // Retrieve the grads for the function.
  const smith::autograd::variable_list& getGrads() const;

 private:
  smith::autograd::variable_list grads_;
};
