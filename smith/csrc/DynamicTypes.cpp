...
static PyTypeObject* loadTypedStorageTypeObject() {
  PyObject* storage_module = PyImport_ImportModule("smith.storage");
  SMITH_INTERNAL_ASSERT(storage_module && PyModule_Check(storage_module));

  PyObject* typed_storage_obj =
      PyObject_GetAttrString(storage_module, "TypedStorage");
  SMITH_INTERNAL_ASSERT(typed_storage_obj && PyType_Check(typed_storage_obj));
...
std::tuple<at::Storage, at::ScalarType, bool> createStorageGetType(
  ...
  if (is_typed_storage) {
    ...
    SMITH_INTERNAL_ASSERT(dtype_obj);
    SMITH_INTERNAL_ASSERT(THPDtype_Check(dtype_obj));
    ...
    SMITH_INTERNAL_ASSERT(untyped_storage_obj);
