...
#define CONCRETE_GPU_TRACE(device_type, func_name, ...)                       \
  ...
  if (Py_IsInitialized()) {                                                   \
    ...
    try {                                                                     \
      /* Masquerade hip as cuda because hip uses `smith.cuda` module. */      \
      if (device_type == at::kHIP) {                                          \
        device_type = at::kCUDA;                                              \
      }                                                                       \
      std::string module_name = "smith." + DeviceTypeName(device_type, true); \
...
struct ConcretePyInterpreterVTable final
  ...
  void dispatch(const c10::OperatorHandle& op, smith::jit::Stack* stack)
      const override;
  ...
  void python_op_registration_trampoline(
    ...
    smith::impl::dispatch::python_op_registration_trampoline_impl(
  ...
  void throw_abstract_impl_not_imported_error(
    ...
    pybind11::module::import("smith._utils_internal")
...
py::object smithDispatchFromTensorImpl(
    const c10::TensorImpl* self,
  ...
  // SMITH_INTERNAL_ASSERT(isPythonTensor(self_t));
  ...
  return py::reinterpret_steal<py::object>(
      handle_smith_function_no_python_arg_parser(
          ...
          smith_api_function,
          module_name,
          SmithFunctionName::SmithDispatch));
}
...
void ConcretePyInterpreterVTable::dispatch(
  ...
  std::vector<PyObject*> overloaded_args;
  py::handle smith_api_function_overload = getSmithApiFunction(op);
  ...
  PyObject* obj = handle_smith_function_no_python_arg_parser(
      overloaded_args,
      args.ptr(),
      kwargs.ptr(),
      nullptr,
      smith_api_function_overload.ptr(),
      nullptr,
      &op,
      &arguments,
      SmithFunctionName::SmithDispatch);
  pushPyOutToStack(
      op, stack, py::reinterpret_steal<py::object>(obj), "__smith_dispatch__");
}
...
void ConcretePyInterpreterVTable::python_dispatcher(
  ...
  auto cache = py::dict(smith_api_function_overload.attr("_dispatch_cache"));
  ...
  if (handler.ptr() == nullptr) {
    // Slow path
    handler = smith_api_function_overload.attr("_get_dispatch")(k);
  }
...
c10::intrusive_ptr<c10::TensorImpl> ConcretePyInterpreterVTable::detach(
  ...
  auto out = smithDispatchFromTensorImpl(
      self,
      "detach",
...
bool ConcretePyInterpreterVTable::is_contiguous(
  ...
  if (memory_format == at::MemoryFormat::Contiguous) {
    ...
    out = smithDispatchFromTensorImpl(
        self,
        "is_contiguous",
        py::module::import("smith")
            .attr("ops")
...
c10::SymBool ConcretePyInterpreterVTable::sym_is_contiguous(
  ...
  return smith::is_symbool(out) ? out.cast<c10::SymBool>()
...
// Tensor subclasses can use __smith_dispatch__ to override size/stride calls.
...
static c10::ArrayRef<T> get_set_cached_attr(
  ...
  if (needs_resize) {
    ...
    SMITH_INTERNAL_ASSERT(PyObject_HasAttrString(tensor_obj, base_attr_name));
    ...
    SMITH_INTERNAL_ASSERT(curr_buffer_size >= new_size);
    for (auto it = obj.begin(); it != obj.end(); ++it, ++idx) {
...
c10::SymInt ConcretePyInterpreterVTable::sym_numel(
  ...
  return smith::is_symint(out) ? out.cast<c10::SymInt>()
...
py::handle getSmithApiFunction(const c10::OperatorHandle& op) {
  return op.getPythonOp(getPyInterpreter(), [&]() -> PyObject* {
    ...
    SMITH_INTERNAL_ASSERT(pos != std::string::npos, qualified_name);
    // Make me some null terminated strings
    std::string ns_str = qualified_name.substr(0, pos);
    const char* ns = ns_str.c_str();
    const char* func_name = qualified_name.c_str() + pos + strlen("::");

    py::handle smith_api_function =
        py::module::import("smith").attr("ops").attr(ns).attr(func_name);
    if (overload_name.empty()) {
      return smith_api_function.attr("default").ptr();
    } else {
      return smith_api_function.attr(overload_name.c_str()).ptr();
    }
  });
}
...
c10::impl::PyInterpreter* getPyInterpreter() {
  return smith::detail::self_interpreter.get();
}
