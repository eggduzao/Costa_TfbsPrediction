...
#include <smith/csrc/xpu/Module.h>
#include <smith/csrc/xpu/XPUPluggableAllocator.h>
#include <smith/csrc/xpu/memory_snapshot.h>
...
static PyObject* THXPModule_isInBadFork_wrap(PyObject* self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  return PyBool_FromLong(smith::utils::is_device_in_bad_fork(at::kXPU));
  END_HANDLE_TH_ERRORS
}

static PyObject* THXPModule_setDevice_wrap(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to set_device");
...
static PyObject* THXPModule_exchangeDevice_wrap(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to exchange_device");
...
static PyObject* THXPModule_getDeviceCount_wrap(
  ...
  smith::utils::register_fork_handler_for_device_init(at::kXPU);
...
static PyObject* THXPModule_xpuSynchronize(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to synchronize");
  auto device_index = THPUtils_unpackDeviceIndex(arg);
  {
...
static PyObject* THXPModule_memoryStats(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to memory_stats");
...
static void registerXpuDeviceProperties(PyObject* module) {
  // Add _xpuDeviceProperties class to smith._C
...
static void registerXpuPluggableAllocator(PyObject* module) {
  ...
  m.def("_xpu_getAllocator", []() {
    return py::cast(smith::xpu::XPUPluggableAllocator::getCurrentAllocator());
  });
  m.def(
      "_xpu_changeCurrentAllocator",
      [](std::shared_ptr<c10::xpu::XPUCachingAllocator::XPUAllocator>
             allocator) {
        smith::xpu::XPUPluggableAllocator::changeCurrentAllocator(allocator);
      });
  m.def("_xpu_customAllocator", [](uint64_t malloc_ptr, uint64_t free_ptr) {
    ...
    return smith::xpu::XPUPluggableAllocator::createCustomAllocator(
        malloc_fn, free_fn);
  });
}

static void bindGetDeviceProperties(PyObject* module) {
  // Add method to smith.xpu
  auto m = py::handle(module).cast<py::module>();
  m.def(
...
static void initXpuMethodBindings(PyObject* module) {
  ...
  m.def("_xpu_memorySnapshot", [](std::optional<c10::MempoolId_t> mempool_id) {
    ...
    py::str last_allocator_settings_s = "BLACKSMITH_ALLOC_CONF";
  ...
  m.def("_xpu_recordMemoryHistory", &smith::xpu::_record_memory_history);
  m.def(
...
static PyObject* THXPModule_initExtension(PyObject* self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  SMITH_INTERNAL_ASSERT(!smith::utils::is_device_in_bad_fork(at::kXPU));
  smith::utils::register_fork_handler_for_device_init(at::kXPU);
  at::globalContext().lazyInitDevice(c10::DeviceType::XPU);

  auto m = THPObjectPtr(PyImport_ImportModule("smith.xpu"));
  if (!m)
    throw python_error();
