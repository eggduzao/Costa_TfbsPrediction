...
struct _MTIAGraph {
  // MTIA use accelerator hooks to connect blacksmith and outside.
...
void initModule(PyObject* module) {
  ...
  m.def("_mtia_init", []() {
    SMITH_INTERNAL_ASSERT(!smith::utils::is_device_in_bad_fork(at::kMTIA));
    smith::utils::register_fork_handler_for_device_init(at::kMTIA);
    at::globalContext().lazyInitDevice(c10::DeviceType::MTIA);

    // Initialize default generators for each MTIA device
    auto mtia_module = py::module_::import("smith.mtia");
  ...
  m.def("_mtia_isInBadFork", []() {
    return smith::utils::is_device_in_bad_fork(at::kMTIA);
  });
