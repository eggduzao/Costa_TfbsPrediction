...
#include <smith/csrc/monitor/counters.h>
#include <smith/csrc/monitor/events.h>
#include <smith/csrc/monitor/python_init.h>

namespace pybind11::detail {
template <>
struct type_caster<smith::monitor::data_value_t> {
 public:
  PYBIND11_TYPE_CASTER(smith::monitor::data_value_t, _("data_value_t"));
  ...
  // C++ -> Python
  static handle cast(
      smith::monitor::data_value_t src,
    ...
    SMITH_CHECK(false, "unknown data_value_t type");
  }
};
} // namespace pybind11::detail
...
void initMonitorBindings(PyObject* module) {
  ...
  py::class_<Stat<double>>(
      ...
      R"DOC(
        ...
        via the event handlers as a ``smith.monitor.Stat`` event.
