...
struct TensorGroup {
  ...
  size_t type_id() {
    ...
    return ::smith::utils::type_id(tensors[0]);
  }
...
SMITH_API std::vector<TensorGroup> take_tensors(
    at::TensorList tensors,
    size_t size_limit,
    bool fine_grained = false);

SMITH_API void reorder_tensors_like(
    std::vector<at::Tensor>& tensors,
    at::TensorList order);

SMITH_API std::pair<at::Tensor, at::Tensor> flatten_sparse_tensors(
    at::TensorList tensors);

SMITH_API std::vector<at::Tensor> unflatten_sparse_tensors(
