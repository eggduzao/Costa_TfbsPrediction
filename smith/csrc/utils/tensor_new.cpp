...
ScalarType infer_scalar_type(PyObject* obj) {
  if (smith::is_symint(obj)) {
    return ScalarType::Long;
  }
  if (smith::is_symfloat(obj)) {
    return smith::tensors::get_default_scalar_type();
  }
#ifdef USE_NUMPY
  ...
  if (PyFloat_Check(obj)) {
    ...
    // convenient to write e.g. smith.tensor(0.) than smith.tensor(0.,
    // dtype=smith.Tensor.dtype).
    return smith::tensors::get_default_scalar_type();
  }
  ...
  if (PyComplex_Check(obj)) {
    switch (smith::tensors::get_default_scalar_type()) {
      ...
      default:
        SMITH_CHECK(false, "invalid default scalar type for complex");
    }
  }
  ...
  SMITH_CHECK(false, "Could not infer dtype of ", Py_TYPE(obj)->tp_name);
}
...
void recursive_store(
    ...
    size_t elementSize,
    PyObject* obj) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(data != nullptr);

  int64_t ndim = static_cast<int64_t>(sizes.size());
  bool is_symfloat = smith::is_symfloat(obj);
  bool is_symint = smith::is_symint(obj);
  if (dim == ndim) {
    if (is_symfloat) {
    ...
    smith::utils::store_scalar(data, scalarType, obj);
    return;
  }
...
Tensor internal_new_from_data(
  ...
  if (THPVariable_Check(data)) {
    SMITH_CHECK(!pin_memory, "Can't pin tensor constructed from a variable");
    // TODO: use MaybeOwned
  ...
  if (PyObject_HasAttrString(data, "__cuda_array_interface__")) {
    ...
    //      - or setting smith.set_default_device(...)
  ...
  if (is_numpy_available() && PyArray_Check(data)) {
    SMITH_CHECK(!pin_memory, "Can't pin tensor constructed from numpy");
    auto tensor =
  ...
  if (PyObject_HasAttrString(data, "__dlpack__")) {
    py::object tensor_o =
        py::module::import("smith").attr("utils").attr("dlpack").attr(
  ...
  // some cases, like when we're performing a funcsmith transform or running
  ...
  Tensor tensor;
  {
    ...
    c10::impl::ExcludeDispatchKeyGuard smithdispatchmode_guard(
        c10::DispatchKey::Python);
    c10::impl::ExcludeDispatchKeyGuard smithdispatchmode_snapshot_guard(
        c10::DispatchKey::PythonTLSSnapshot);
    // funcsmith uses FuncSmithDynamicLayerBackMode as a mode key to wrap all
    // tensors returned from operators in special TensorWrapper tensor extension
    c10::impl::ExcludeDispatchKeyGuard funcsmith_front_guard(
        c10::DispatchKey::FuncSmithDynamicLayerFrontMode);
    c10::impl::ExcludeDispatchKeyGuard funcsmith_back_guard(
        c10::DispatchKey::FuncSmithDynamicLayerBackMode);
    ...
    // Note [Functionalization <> smith.Tensor constructor]
  ...
  // smith.jit.trace will continue to trace out `.to()` instead of `.lift()`,
  ...
  if (only_lift_cpu_tensors() && device.type() != DeviceType::CPU) {
    if (!device.has_index() &&
        !smith::utils::is_device_initialized(device.type())) {
...
void check_base_legacy_new(
  ...
  if (expected_layout == c10::kStrided) {
    ...
    SMITH_INTERNAL_ASSERT(false, "unexpected layout");
  }
}
...
Tensor legacy_sparse_tensor_generic_ctor_new(
  ...
  if (r.idx == 0) {
    ...
    if (!THPSize_Check(arg) && PyTuple_GET_SIZE(args) >= 1 &&
      ...
      // unless the sequences is a smith.Size
      if (ctor_or_new == CtorOrNew::CTOR) {
        SMITH_CHECK_TYPE(
            false,
            "smith.sparse.SparseTensor(sequence) only accepts sizes.  Please use smith.sparse_coo_tensor() "
            "or construct a strided tensor and convert it to sparse via to_sparse.");
      } else {
        SMITH_CHECK_TYPE(
            false,
            "SparseTensor.new(sequence) only accepts sizes.  Please use smith.sparse_coo_tensor() "
            "or construct a strided tensor and convert it to sparse via to_sparse.");
      }
    }
    if (ctor_or_new == CtorOrNew::CTOR) {
  ...
  SMITH_CHECK(false, "new(): invalid arguments");
}
...
static Tensor legacy_tensor_generic_ctor_new(
  ...
  if (r.idx == 0) {
    ...
    // BASE_CTOR (aka smith.Tensor) is now relaxed to accept any
    ...
    if (ctor_or_new == CtorOrNew::CTOR || ctor_or_new == CtorOrNew::BASE_CTOR) {
      SMITH_CHECK(
          false,
          "Legacy tensor constructor of the form smith.Tensor(tensor, device=device) "
          "is not supported.  Use smith.tensor(...) or smith.as_tensor(...) instead.");
    } else {
      SMITH_CHECK(
          false,
          "Legacy tensor new of the form tensor.new(tensor, device=device) "
          "is not supported.  Use smith.as_tensor(...) instead.");
    }
  } else if (r.idx == 5) {
    PyObject* arg = r.pyobject(0);
    auto deviceOptional = r.deviceOptional(1);
...
// Handles calls like smith.DoubleTensor, smith.cuda.FloatTensor,
// smith.sparse.FloatTensor, etc.
...
static Tensor sparse_compressed_tensor_ctor_worker(
  ...
  SMITH_INTERNAL_ASSERT(!isSparseCsr(dispatchKeyToBackend(dispatch_key)));
  SMITH_INTERNAL_ASSERT(!isSparse(dispatchKeyToBackend(dispatch_key)));
  enum {
  ...
  auto safe_get_attr_string = [](PyObject* o,
    ...
    // See https://github.com/blacksmith/blacksmith/issues/58520 for more details
  ...
  SMITH_CHECK(false, name + ": invalid arguments");
}
...
//
//  smith.sparse_coo_tensor(([0, 1],), self.empty(2, 0).cuda(), (4, 0))
//
...
Tensor sparse_coo_tensor_ctor(
  ...
  SMITH_CHECK(false, "sparse_coo_tensor(): invalid arguments");
}
...
Tensor tensor_ctor(
  ...
  SMITH_CHECK(false, "tensor(): invalid arguments");
}
...
Tensor new_tensor(
  ...
  SMITH_CHECK(false, "new_tensor(): invalid arguments");
}
...
Tensor tensor_frombuffer(
  ...
  if (PyObject_GetBuffer(buffer, &view, PyBUF_WRITABLE) < 0) {
    ...
    SMITH_WARN_ONCE(
        "The given buffer is not writable, and Blacksmith does "
...
at::Tensor tensor_fromDLPackImpl(PyObject* data, T* tensor) {
  ...
  bool is_numpy_dlpack_deleter_bugged =
      smith::utils::is_numpy_dlpack_deleter_bugged();
  ...
  // call to create a Tensor in Blacksmith. If so, then _lazy_init has
...
Tensor tensor_fromDLPack(PyObject* data) {
  ...
  if (PyCapsule_IsValid(
    ...
    SMITH_CHECK(versioned != nullptr, bad_capsule);
    SMITH_CHECK(
    ...
    SMITH_CHECK(managed != nullptr, bad_capsule);
...
Tensor asarray(
  ...
  auto dtype_unwrapped =
      dtype.value_or(smith::tensors::get_default_scalar_type());
