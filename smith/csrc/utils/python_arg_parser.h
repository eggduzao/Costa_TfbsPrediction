...
// the types relevant to Blacksmith and distinguishes between overloaded function
// signatures.
//
// Example:
//
...
// are smith/csrc/autograd/generated/python_*.cpp
//
...
//
//    - Zero-dim tensors (e.g., smith.tensor(2)) bind to both
...
#include <smith/csrc/dynamo/eval_frame.h>
#include <smith/csrc/jit/frontend/tracer.h>
#include <smith/csrc/python_dimname.h>
...
#include <smith/csrc/utils/six.h>
...
#ifdef USE_NUMPY
  if (smith::utils::is_numpy_scalar(obj)) {
    return true;
  }
#endif
  return PyFloat_Check(obj) || PyLong_Check(obj) || PyComplex_Check(obj) ||
      smith::is_symint(py::handle(obj)) || smith::is_dynint(py::handle(obj)) ||
      smith::is_symfloat(py::handle(obj)) || smith::is_symbool(py::handle(obj));
}

namespace smith {

SMITH_PYTHON_API bool should_allow_numbers_as_tensors(const std::string& name);
...
struct PYBIND11_EXPORT PythonArgParser {
  ...
  // meant only for `smith` functions.
  template <int N>
...
struct SMITH_PYTHON_API PythonArgs {
  PythonArgs(
      bool traceable,
  ...
  inline bool has_smith_function();
  ...
  inline smith::List<std::optional<at::Tensor>> list_of_optional_tensors(int i);
  template <int N>
 ...
 private:
  // Non-inline functions' symbols are exposed to smith_python DLL
  // via SMITH_PYTHON_API tag at struct level.
...
struct FunctionParameter {
  ...
  SMITH_PYTHON_API std::string type_name() const;
...
inline bool PythonArgs::has_smith_function() {
  return !overloaded_args.empty() || at::impl::smith_function_mode_enabled();
}
...
inline smith::List<std::optional<at::Tensor>> PythonArgs::
    list_of_optional_tensors(int i) {
  if (!args[i])
    return smith::List<std::optional<at::Tensor>>();
  auto tuple = six::isTuple(args[i]);
  THPObjectPtr arg = six::maybeAsTuple(args[i]);
  // NOLINTNEXTLINE(bugprone-branch-clone)
  auto size = tuple ? PyTuple_GET_SIZE(arg.get()) : PyList_GET_SIZE(arg.get());
  smith::List<std::optional<at::Tensor>> res;
  res.reserve(size);
  for (const auto idx : c10::irange(size)) {
    PyObject* obj = tuple ? PyTuple_GET_ITEM(arg.get(), idx)
                          : PyList_GET_ITEM(arg.get(), idx);
...
inline std::vector<c10::SymInt> PythonArgs::symintlist(int i) {
  ...
  if (size1 > 0 && smith::is_symint(py::handle(args[i]))) {
  ...
  if (!tuple) {
    SMITH_INTERNAL_ASSERT(PyList_Check(arg), "expected tuple or list");
  }
  // NOLINTNEXTLINE(bugprone-branch-clone)
  ...
  for (const auto idx : c10::irange(size2)) {
    ...
    // Elements of smith.Size are tensors during tracing, and we need to
    ...
    if (traceable && jit::tracer::isTracing() && THPVariable_Check(obj)) {
      ...
      if (THPUtils_checkLongExact(obj)) {
        ...
        SMITH_CHECK(scalar.isIntegral(/*include bool*/ false));
...
inline std::vector<int64_t> PythonArgs::intlistWithDefault(
  ...
  if (size1 > 0 && smith::is_symint(py::handle(arg))) {
    return std::vector<int64_t>(
        size1,
        py::handle(arg).cast<c10::SymInt>().guard_int(__FILE__, __LINE__));
  }
  if (size1 > 0 && smith::is_dynint(py::handle(arg))) {
  ...
  for (const auto idx : c10::irange(size2)) {
    ...
    if (traceable && jit::tracer::isTracing() && THPVariable_Check(obj)) {
      ...
      } else if (smith::is_symint(py::handle(obj))) {
        res[idx] = py::cast<c10::SymInt>(py::handle(obj))
                       .guard_int(__FILE__, __LINE__);
      } else if (smith::is_dynint(py::handle(obj))) {
...
inline at::Device deviceFromLong(int64_t device_index) {
  SMITH_CHECK(device_index >= 0, "Device index must not be negative");
  return at::Device(
      // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
...
inline at::Device PythonArgs::device(int i) {
  if (!args[i]) {
    return smith::tensors::get_default_device();
  }
...
inline at::Dimname PythonArgs::dimname(int i) {
  SMITH_INTERNAL_ASSERT(args[i] != nullptr);
...
inline std::vector<at::Dimname> PythonArgs::dimnamelist(int i) {
  SMITH_INTERNAL_ASSERT(args[i]);
  PyObject* arg = args[i];
  auto size = signature.params[i].size;
  SMITH_INTERNAL_ASSERT(size == 0 || size == 1);
...
inline at::MemoryFormat PythonArgs::memoryformat(int i) {
  ...
  SMITH_CHECK(
      ...
      "memory_format arg must be an instance of the smith.memory_format");
...
inline at::QScheme PythonArgs::toQScheme(int i) {
  ...
  SMITH_CHECK(
      ...
      "qscheme arg must be an instance of the smith.qscheme");
...
inline int64_t PythonArgs::toInt64(int i) {
  ...
  if (smith::is_symint(py::handle(args[i]))) {
    return py::cast<c10::SymInt>(py::handle(args[i]))
        .guard_int(__FILE__, __LINE__);
  }
  if (smith::is_dynint(py::handle(args[i]))) {
...
inline double PythonArgs::toDouble(int i) {
  ...
  if (smith::is_symfloat(py::handle(args[i]))) {
...
inline bool PythonArgs::toBool(int i) {
  ...
  if (smith::is_symbool(py::handle(args[i]))) {
    return py::cast<c10::SymBool>(py::handle(args[i]))
...
/*
 *
 * Handle __smith_function__ overrides if we know that there are overloaded
 * arguments.  All objects stored in r.overloaded_args must have a
 * __smith_function__ implementation and the arguments must be ordered in order
 ...
 *
 * If the result of calling __smith_function__ is NotImplemented, the
 * next implementation in the precedence order is called. If all
 * arguments return NotImplemented from their __smith_function__
 * implementation, a TypeError is raised in Python.
 *
 * Assumes overloaded_args has at least one entry. All entries must have
 * a __smith_function__ attribute that resolves to a callable that
 * accepts a smith API function, a tuple of arguments, and a dict of
 * keyword arguments for the smith API function.
 *
 * It is sufficient to call PythonArgs::has_smith_function before
 ...
 * __smith_function__.
 *
 * See smith._overrides.handle_smith_function for the equivalent
 * code in the pure-python implementation.
 *
 * 'r' is a parsed PythonArgs instance, returned from
 * PythonArgParser::parse.
 *
 * 'args' is a reference to the python tuple of arguments to the smith
 * API function.
 *
 * 'kwargs' is a reference to the python dict of keyword arguments to
 * the smith API function.
 *
 * 'smith_api' is a reference to a python smith API namespace.
 *
 * 'smith_api_function' is the reference to the original smith method, usually,
 * we can use smith_api and func_name to get smith_api_function. In some cases,
 * e.g., smith custom op, we create the function in C++, if we still use
 * smith_api and func_name to fetch original api, a cyclic call will happen.
 *
 * 'overloaded_args' is the args which have overloaded __smith_function__.
 *
 * 'func_name' is the named of the original smith method.
 *
 * TODO: we could use different names for the following 'handle_smith_function'
...
auto handle_smith_function(
    ...
    PyObject* smith_api = THPVariableClass,
    const std::string& module_name = "smith.Tensor") -> PyObject*;

// Used for functions created in C++, e.g., C++ custom op, which doesn't use
// PythonArgParser to get overloaded_args.
enum class SmithFunctionName { SmithFunction, SmithDispatch };

auto SMITH_PYTHON_API handle_smith_function_no_python_arg_parser(
    ...
    const char* module_name,
    SmithFunctionName smith_function_name = SmithFunctionName::SmithFunction)
    -> PyObject*;
 ...
 * type. If the type defines __smith_function__, it also returns true.
 * Otherwise returns false. If the class is not smith.Tensor, and it defines
 * __smith_function__, we append obj to overloaded_args.
 *
 ...
 * __smith_dispatch__, where we operate on classes that have a
 * __smith_dispatch__ classmethod.
 *
 * 'overloaded_args': the vector to append the overloaded type
 * 'obj': the input class that has a __smith_dispatch__ classmethod.
 */
