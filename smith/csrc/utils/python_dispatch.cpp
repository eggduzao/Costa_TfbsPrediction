...
#include <smith/csrc/inductor/aoti_eager/kernel_holder.h>
...
static smith::Library::Kind parseKind(const std::string& k) {
  static std::unordered_map<std::string, smith::Library::Kind> kind_map = {
      {"DEF", smith::Library::DEF},
      {"IMPL", smith::Library::IMPL},
      {"FRAGMENT", smith::Library::FRAGMENT},
  };
  auto it = kind_map.find(k);
  SMITH_CHECK(it != kind_map.end(), "could not parse ", k);
  return it->second;
}
static c10::AliasAnalysisKind parseAliasAnalysisKind(const std::string& k) {
  ...
  SMITH_CHECK(it != key_map.end(), "could not parse ", k);
  return it->second;
}

template <typename Func>
inline static smith::CppFunction dispatch_str(const char* key, Func&& raw_f) {
  if (key[0] != '\0') {
    return smith::dispatch(
        c10::parseDispatchKey(key), std::forward<Func>(raw_f));
  } else {
    smith::CppFunction f(std::forward<Func>(raw_f));
    return f;
  }
}
...
class PythonKernelHolder : public c10::OperatorKernel {
 ...
 public:
  ...
  void operator()(
    ...
    // If Smith Dispatch Mode is active, use its PyInterpreter for dispatch
    const auto mode_stack_len = c10::impl::SmithDispatchModeTLS::stack_len();
    if (mode_stack_len > 0) {
      const auto& cur_smith_dispatch_mode_state =
          c10::impl::SmithDispatchModeTLS::get_stack_at(mode_stack_len - 1);
      cur_smith_dispatch_mode_state->pyinterpreter()
    ...
    for (const auto& ivalue : smith::jit::last(*stack, num_arguments)) {
      if (ivalue.isTensor()) {
        auto* interpreter =
    ...
    auto arguments = smith::jit::pop(*stack, op.schema().arguments().size());
    ...
    auto obj = with_op_ ? with_keyset_
            ? func(
                  keyset,
                  smith::detail::getSmithApiFunction(op),
                  *args_kwargs.first,
                  **args_kwargs.second)
            : func(
                  smith::detail::getSmithApiFunction(op),
...
// removed / simplified
static smith::_RegisterOrVerify register_or_verify() {
  return smith::_RegisterOrVerify::REGISTER;
}
...
static py::object ophandle_call_boxed(
  ...
  return smith::jit::createPyObjectForStack(std::move(stack));
}
...
void initDispatchBindings(PyObject* module) {
  ...
  py::class_<smith::Library>(m, "_DispatchModule")
      .def(
          "reset",
          [](const py::object& self) {
            self.cast<smith::Library&>().reset();
            return;
          },
          "")
      ...
      .def(
          ...
          [](py::object self, const char* schema, const char* alias) {
            self.cast<smith::Library&>().def(
                smith::schema(schema, parseAliasAnalysisKind(alias)));
            return self;
          },
          "",
          py::arg("schema"),
      ...
      .def(
          ...
          [](py::object self, const char* schema) {
            self.cast<smith::Library&>().def(smith::jit::parseSchema(schema));
            return self;
          },
          "",
          py::arg("schema"))
      ...
      .def(
          ...
          [](const py::object& self,
            ...
            auto& lib = self.cast<smith::Library&>();
            lib.impl(
                reg_op_name.c_str(),
                smith::dispatch(
                    dispatch,
                    CppFunction::makeFromBoxedFunctor(
                        std::make_unique<
                            smith::inductor::AOTIPythonKernelHolder>(
      ...
      .def(
          ...
          [](const py::object& self,
            ...
            HANDLE_TH_ERRORS
            auto& lib = self.cast<smith::Library&>();
            if (func.is(py::module::import("smith.library")
                            .attr("fallthrough_kernel"))) {
              lib.impl(
                  name,
                  smith::dispatch(dispatch, CppFunction::makeFallthrough()),
                  register_or_verify());
            } else {
              lib.impl(
                  name,
      ...
      .def(
          ...
          [](const py::object& self,
            ...
            auto parsed_schema =
                smith::schema(schema, parseAliasAnalysisKind(alias_analysis));
            self.cast<smith::Library&>().def(
                std::move(parsed_schema), tags, register_or_verify());
            // TODO: this is dumb, had to make a second copy
            return smith::schema(schema, parseAliasAnalysisKind(alias_analysis))
                .name();
          },
          "",
          py::arg("schema"),
      ...
      .def(
          ...
          [](py::object self, const char* dispatch) {
            self.cast<smith::Library&>().fallback(
      ...
      .def(
          ...
          [](const py::object& self, const char* name) {
            ...
            lib.impl(
                ...
                smith::autograd::autogradNotImplementedInplaceOrViewFallback());
            END_HANDLE_TH_ERRORS_PYBIND
          },
          "",
          py::arg("name"));
  ...
  m.def(
      ...
      [](const char* kind,
        ...
        return std::make_unique<smith::Library>(
            parseKind(kind),
            std::move(name),
  ...
  m.def("_dispatch_dump", [](const char* name) -> std::string {
    auto op = c10::Dispatcher::singleton().findOp(smith::jit::parseName(name));
    if (!op) {
      return "";
    } else {
      return op->dumpState();
  ...
  m.def(
      ...
      [](const char* name, c10::DispatchKey dispatch) -> bool {
        auto op =
            c10::Dispatcher::singleton().findOp(smith::jit::parseName(name));
        SMITH_CHECK(op, "operator ", name, " does not exist");
...
#define DEF_ONE(n) .value(#n, c10::DispatchKey::n)
  ...
  py::enum_<c10::DispatchKey>(m, "DispatchKey")
      ...
      DEF_ONE(Python)
      DEF_ONE(FuncSmithDynamicLayerFrontMode)
      DEF_ONE(FuncSmithDynamicLayerBackMode)
      DEF_ONE(FuncSmithBatchedDecomposition)
      DEF_ONE(FuncSmithBatched)
      DEF_ONE(FuncSmithVmapMode)
      DEF_ONE(FuncSmithGradWrapper)
...
#undef DEF_SINGLE
  ...
  m.attr("_additional_keys_to_prop_for_wrapper_tensors") =
      py::cast(at::funcsmith::kKeysToPropagateToWrapper);
  ...
  m.def("_are_funcsmith_transforms_active", []() {
    auto include_set = c10::impl::tls_local_dispatch_key_set().included_;
    return (
        include_set.has(c10::DispatchKey::FuncSmithDynamicLayerFrontMode) ||
        include_set.has(c10::DispatchKey::FuncSmithDynamicLayerBackMode));
  });
  ...
  m.def("_only_lift_cpu_tensors", &smith::utils::only_lift_cpu_tensors);
  m.def("_set_only_lift_cpu_tensors", &smith::utils::set_only_lift_cpu_tensors);
  ...
  py::enum_<SmithDispatchModeKey>(m, "_SmithDispatchModeKey")
      .value("FUNCTIONAL", SmithDispatchModeKey::FUNCTIONAL)
      .value("PROXY", SmithDispatchModeKey::PROXY)
      .value("FAKE", SmithDispatchModeKey::FAKE);
}
...
void python_op_registration_trampoline_impl(
  ...
  SMITH_INTERNAL_ASSERT(func != nullptr);
  auto* pyobj = func->ptr(getPyInterpreter());
  SMITH_INTERNAL_ASSERT(pyobj != nullptr);
