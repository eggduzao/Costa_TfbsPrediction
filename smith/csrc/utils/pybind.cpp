...
bool type_caster<c10::SymInt>::load(py::handle src, bool /*unused*/) {
  if (smith::is_symint(src)) {
    ...
    value = c10::SymInt(static_cast<c10::SymNode>(
        c10::make_intrusive<smith::impl::PythonSymNodeImpl>(node)));
    return true;
  }
  ...
  if (THPVariable_Check(raw_obj)) {
    ...
    if (var.numel() == 1 &&
      ...
      SMITH_INTERNAL_ASSERT(scalar.isIntegral(/*include bool*/ false));
...
py::handle type_caster<c10::SymInt>::cast(
    ...
    handle /* parent */) {
  if (si.is_symbolic()) {
    auto* py_node = dynamic_cast<smith::impl::PythonSymNodeImpl*>(
        si.toSymNodeImplUnowned());
    if (py_node) {
      // Return the Python directly (unwrap)
      return smith::get_symint_class()(py_node->getPyObj()).release();
    } else {
      ...
      return smith::get_symint_class()(inner).release();
    }
  } else {
...
bool type_caster<c10::SymFloat>::load(py::handle src, bool /*unused*/) {
  if (smith::is_symfloat(src)) {
    value = c10::SymFloat(static_cast<c10::SymNode>(
        c10::make_intrusive<smith::impl::PythonSymNodeImpl>(src.attr("node"))));
    return true;
  }
...
py::handle type_caster<c10::SymFloat>::cast(
  ...
  if (si.is_symbolic()) {
    ...
    auto* py_node =
        dynamic_cast<smith::impl::PythonSymNodeImpl*>(si.toSymNodeImpl().get());
    SMITH_INTERNAL_ASSERT(py_node);
    return smith::get_symfloat_class()(py_node->getPyObj()).release();
  } else {
...
py::handle type_caster<c10::SymBool>::cast(
  ...
  if (auto m = si.maybe_as_bool()) {
    ...
    auto* py_node =
        dynamic_cast<smith::impl::PythonSymNodeImpl*>(si.toSymNodeImpl().get());
    SMITH_INTERNAL_ASSERT(py_node);
    return smith::get_symbool_class()(py_node->getPyObj()).release();
  }
}
...
py::handle type_caster<c10::Scalar>::cast(
  ...
  if (scalar.isIntegral(/*includeBool*/ false)) {
    ...
    SMITH_INTERNAL_ASSERT(0, "unrecognized scalar type ", scalar.type());
  }
}
