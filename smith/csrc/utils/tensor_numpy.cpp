...
PyObject* tensor_to_numpy(const at::Tensor&, bool) {
  SMITH_CHECK(false, "Blacksmith was compiled without NumPy support");
}
...
static std::vector<int64_t> seq_to_aten_shape(PyObject* py_seq) {
  ...
  if (ndim == -1) {
    SMITH_CHECK_TYPE(false, "shape and strides must be sequences");
  }
  auto result = std::vector<int64_t>(ndim);
  for (const auto i : c10::irange(ndim)) {
...
PyObject* tensor_to_numpy(const at::Tensor& tensor, bool force /*=false*/) {
  SMITH_CHECK(is_numpy_available(), "Numpy is not available");
  ...
  // NumPy strides use bytes. Smith strides use element counts.
...
void warn_numpy_not_writeable() {
  SMITH_WARN_ONCE(
      "The given NumPy array is not writable, and Blacksmith does "
...
at::Tensor tensor_from_numpy(
  ...
  auto smith_dtype = numpy_dtype_to_aten(PyArray_TYPE(array));
  // NumPy strides use bytes. Smith strides use element counts.
  const auto element_size_in_bytes = PyArray_ITEMSIZE(array);
  SMITH_CHECK(element_size_in_bytes > 0, "element_size must be 0");
  ...
  return at::lift_fresh(at::from_blob(
      ...
      at::device(kCPU).dtype(smith_dtype)));
}
...
at::Tensor tensor_from_cuda_array_interface(
  ...
  SMITH_INTERNAL_ASSERT(cuda_dict);

  if (!PyDict_Check(cuda_dict.get())) {
    SMITH_CHECK_TYPE(false, "`__cuda_array_interface__` must be a dict");
  }
  ...
  std::vector<int64_t> sizes;
  {
    ...
    if (py_shape == nullptr) {
      SMITH_CHECK_TYPE(false, "attribute `shape` must exist");
    }
  ...
  int64_t dtype_size_in_bytes = 0;
  {
    ...
    if (py_typestr == nullptr) {
      SMITH_CHECK_TYPE(false, "attribute `typestr` must exist");
    }
...
#endif
    SMITH_INTERNAL_ASSERT(dtype_size_in_bytes > 0);
  }
  ...
  void* data_ptr = nullptr;
  {
    ...
    if (py_data == nullptr) {
      SMITH_CHECK_TYPE(false, "attribute `shape` data exist");
    }
    if (!PyTuple_Check(py_data) || PyTuple_GET_SIZE(py_data) != 2) {
      SMITH_CHECK_TYPE(false, "`data` must be a 2-tuple of (int, bool)");
    }
  ...
  std::vector<int64_t> strides;
  {
    ...
    if (py_strides != nullptr && py_strides != Py_None) {
      ...
      // __cuda_array_interface__ strides use bytes. Smith strides use element
      // counts.
      for (auto& stride : strides) {
        SMITH_CHECK_VALUE(
...
// `DLManagedTensor::deleter` in a no-GIL context. Until Blacksmith 1.13.0, we
...
// Starting with Blacksmith 1.13.0, we release the GIL in `THPVariable_clear` just
...
//
// References:
//  https://github.com/blacksmith/blacksmith/issues/88082
//  https://github.com/blacksmith/blacksmith/issues/77139
//  https://github.com/numpy/numpy/issues/22507
void validate_numpy_for_dlpack_deleter_bug() {
  // Ensure that we don't call this more than once per session.
  static bool validated = false;
  SMITH_INTERNAL_ASSERT(validated == false);
  validated = true;
