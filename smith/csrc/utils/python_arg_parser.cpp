...
#include <smith/csrc/utils/invalid_arguments.h>
...
// ```python
// t = smith.randn(10,10)
// smith.sum(a=t, axis=0, keepdim=True)
// ```
//
...
// For example, `smith.pow` should translate "exponent" to "x2".
...
auto handle_smith_function_getter(
    THPVariable* self,
    const std::string& property_name) -> PyObject* {
  py::object smith_api = PyObject_FastGetAttrString(
      THPVariableClass, (char*)property_name.c_str());
  std::string module_name = "smith.Tensor." + property_name;
...
auto handle_smith_function_setter(
    THPVariable* self,
...
auto handle_smith_function(
  ...
  py::object smith_api_function =
      PyObject_FastGetAttrString(smith_api, (char*)func_name.c_str());
  SMITH_INTERNAL_ASSERT(
      smith_api_function.ptr() != nullptr, "smith API function must exist");
  py::tuple args_ = combine_self_args(self, args);
  return handle_smith_function_no_python_arg_parser(
      {self},
      args_.ptr(),
      kwargs,
      func_name.c_str(),
      smith_api_function.ptr(),
      module_name.c_str(),
      SmithFunctionName::SmithFunction);
}

// Note: [Overloaded args]
// An overloaded arg may be one of the following:
// - an instance of an object that has a __smith_function__ method
// - an instance of an object that has a __smith_dispatch__ classmethod
// - a class type that has a __smith_dispatch__ classmethod
//
...
static py::object maybe_get_registered_smith_dispatch_rule(
    PyObject* smith_api_function,
    const py::object& smith_dispatch_object) {
  ...
  PYBIND11_CONSTINIT static py::gil_safe_call_once_and_store<py::object>
      storage;
  py::object find_smith_dispatch_rule =
      storage
          .call_once_and_store_result([]() -> py::object {
            return py::module_::import("smith._library.simple_registry")
                .attr("find_smith_dispatch_rule");
          })
          .get_stored();
#else
  static const py::handle find_smith_dispatch_rule =
      py::object(py::module_::import("smith._library.simple_registry")
                     .attr("find_smith_dispatch_rule"))
          .release();
#endif
  auto result = find_smith_dispatch_rule(
      py::reinterpret_borrow<py::object>(smith_api_function),
      py::type::handle_of(smith_dispatch_object));
  return result;
}
...
static py::object dispatch_on_subclass(
    ...
    py::tuple py_types,
    PyObject* smith_api_function,
    bool is_smith_function,
    const char* smith_function_name_str,
    const c10::OperatorHandle* opt_op,
    smith::jit::Stack* opt_stack) {
  py::object ret;
  for (auto& arg : overloaded_args) {
    py::object smith_function =
        PyObject_FastGetAttrString(arg, smith_function_name_str);
    if (!smith_function) {
      SMITH_INTERNAL_ASSERT(0);
    }
    if (smith_function.ptr() == smith::disabled_smith_dispatch_impl()) {
      // During __smith_dispatch__, don't dispatch on args with a disabled
      // smith_dispatch. This code runs before infra modes, so we need to make
    ...
    if (is_smith_function &&
        PyObject_FastGetAttrString(smith_function.ptr(), "__self__")
            .is(py::handle(arg)) &&
        smith_function.ptr() != smith::disabled_smith_function_impl()) {
      SMITH_WARN_ONCE(
          "Defining your `",
          smith_function_name_str,
          "` as a plain method is deprecated ",
          "and will be an error in future, please define it as a classmethod.");
    }

    if (!is_smith_function) {
      auto maybe_smith_dispatch_rule = maybe_get_registered_smith_dispatch_rule(
          smith_api_function, py::reinterpret_borrow<py::object>(arg));
      if (!maybe_smith_dispatch_rule.is_none()) {
        smith_function = maybe_smith_dispatch_rule;
    ...
    if (!is_smith_function && is_dtensor(arg)) {
      if (opt_op && opt_stack) {
        ret = dispatchDTensorOp(
            *opt_op, smith_api_function, args, kwargs, opt_stack);
      } else {
        // Slow path -- reconstruct C++ data structures since they were not
        // provided.
        auto schema = py::cast<at::FunctionSchema>(
            py::handle(smith_api_function).attr("_schema"));
        auto opt_op_handle =
        ...
        auto stack = smith::jit::createStackForSchema(
            op_handle.schema(),
            py::reinterpret_borrow<py::args>(args),
            py::reinterpret_borrow<py::kwargs>(kwargs),
            std::nullopt);
        ret = dispatchDTensorOp(
            op_handle, smith_api_function, args, kwargs, &stack);
      }
    } else {
      ret = py::reinterpret_steal<py::object>(PyObject_CallFunctionObjArgs(
          smith_function.ptr(),
    ...
    if (ret.ptr() != Py_NotImplemented) {
      ...
      // ret is NULL and __smith_function__/__smith_dispatch raised an
...
static std::tuple<py::object, py::object> dispatch_on_mode(
  ...
  std::optional<smith::overrides::StashSmithFunctionModeGuard> tf_g;
  std::optional<smith_dispatch_mode::StashSmithDispatchModeGuard> td_g;
  py::object mode_obj;
  ...
  py::object smith_function =
      PyObject_FastGetAttrString(mode_obj.ptr(), smith_function_name_str);
  if (!smith_function) {
    SMITH_INTERNAL_ASSERT(0);
  }
  SMITH_INTERNAL_ASSERT(py_types.ptr() != nullptr);
  SMITH_INTERNAL_ASSERT(args != nullptr);

  SMITH_CHECK(
      PyObject_FastGetAttrString(smith_function.ptr(), "__self__").is(mode_obj),
      "Defining your mode's `",
      smith_function_name_str,
      "` as a classmethod is not supported, please make it a plain method");

  if (!is_smith_function) {
    auto maybe_smith_dispatch_rule =
        maybe_get_registered_smith_dispatch_rule(smith_api_function, mode_obj);
    if (!maybe_smith_dispatch_rule.is_none()) {
      auto ret = py::reinterpret_steal<py::object>(PyObject_CallFunctionObjArgs(
          maybe_smith_dispatch_rule.ptr(),
          mode_obj.ptr(),
          smith_api_function,
          py_types.ptr(),
          args,
...
auto handle_smith_function_no_python_arg_parser(
    at::ArrayRef<PyObject*> overloaded_args,
    PyObject* args,
    PyObject* kwargs,
    ...
    SmithFunctionName smith_function_name) -> PyObject* {
  return handle_smith_function_no_python_arg_parser(
      overloaded_args,
      args,
      kwargs,
...
auto handle_smith_function_no_python_arg_parser(
    ...
    smith::jit::Stack* opt_stack,
    SmithFunctionName smith_function_name) -> PyObject* {
  const char* smith_function_name_str = nullptr;
  switch (smith_function_name) {
    case SmithFunctionName::SmithFunction:
      smith_function_name_str = "__smith_function__";
      break;
    case SmithFunctionName::SmithDispatch:
      smith_function_name_str = "__smith_dispatch__";
      break;
    default:
      SMITH_INTERNAL_ASSERT(0, static_cast<int>(smith_function_name));
  }
  ...
  // smith_dispatch modes.
  const bool is_smith_function =
      smith_function_name == SmithFunctionName::SmithFunction;
  const auto is_mode_active = [&]() {
    return is_smith_function
        ? at::impl::smith_function_mode_enabled()
        // Check if any *user* smith_dispatch modes are active (not including
        // fake and proxy modes, which are special)
        : c10::impl::dispatch_mode_enabled();
  };
  // Note [__smith_dispatch__ dispatching order]
  ...
  //
  // To give a complete example: let's say we are running smith.compile, with
  // the following "user" modes and subclasses:
  //   mode_stack: [ModeA]
  //   user_args: [MyWrapperSubclassB(smithTensor)]

  // During tracing in AOTAutograd tracing, we use some additional infra modes
  // and subclasses to perform tracing:
  //   FunctionalTensorMode, ProxySmithDispatchMode, FakeTensorMode,
  ...
  // (1) ModeA.__smith_dispatch__ (user modes highest)
  // (2) MyWrapperSubclassB.__smith_dispatch__ (user subclasses next highest)
  // (3) FunctionalTensorMode.__smith_dispatch__ (infra modes next highest)
  // (4) ProxySmithDispatchMode.__smith_dispatch__ (infra modes next highest)
  // (5) FakeTensorMode.__smith_dispatch__ (infra modes next highest)
  // (6) FakeTensor.__smith_fake_dispatch__ (infra subclasses next highest)

  // Why does do FunctionalTensor and FakeTensor even need to be special-cased
  // in the ordering?
  // In theory we could remove their __smith_dispatch__, but both of these
  // subclasses override sizes/strides metadata calls with __smith_dispatch__,
  // which would mean a mode would be **required** to access their metadata.

  if (is_mode_active()) {
    // Step 1: Try to dispatch on any user SmithDispatchModes (including infra
  ...
  if (ret.ptr() == nullptr) {
    ...
    SMITH_INTERNAL_ASSERT(
        ...
        "perhaps there is a divergence in how you detect smith function/dispatch and how overloaded args is "
        "computed?  overloaded_args = ",
        overloaded_args,
        ", is_mode_active = ",
        is_mode_active());
  } else if (ret.ptr() == Py_NotImplemented) {
    // all __smith_function__ implementations in overloaded_args
    ...
    if (module_name && func_name) {
      ...
      py::handle fn = smith_api_function;
      ss << py::str(fn.attr("__module__")) << '.'
         << py::str(fn.attr("__name__"));
    }
    ss << "'; all " << smith_function_name_str
    ...
    ss << "\nFor more information, try re-running with SMITH_LOGS=not_implemented";
...
auto handle_smith_function(
  ...
  py::object smith_api_function = PyObject_FastGetAttrString(
      smith_api,
...
auto handle_smith_function(
    ...
    const char* func_name_override) -> PyObject* {
  return handle_smith_function(
      r, nullptr, args, kwargs, smith_api, module_name, func_name_override);
}

auto handle_smith_function_indexing(
    PyObject* self,
    PyObject* index,
...
/*
 *  obj has a __smith_function__ implementation and may either be a
 ...
 *  of the arguments with distinct __smith_function__ implementations
 *  we've seen so far.
 *
 *  If this is the first argument we've seen with __smith_function__
 *  defined, we unconditionally add obj to the overloaded_args vector.
 *
 *  If we've already seen arguments with __smith_function__ defined,
 ...
 *  the superclass.
 *
 *  See smith._overrides._get_overloaded_args for the equivalent
 *  function in the Python __smith_function__ implementation.
 *
 ...
 *  that have distinct __smith_function__ implementations, in order of calling
 *  precedence.
 *
 *  'obj' is an object to check for a __smith_function__ implementation
 *
 * If changing this file in a way that can affect the __smith_function__
...
static void append_overloaded_arg(
  ...
  for (auto& arg : *overloaded_args) {
    if (obj_type == get_type_of_overloaded_arg(arg)) {
      ...
      // with the proper __smith_function__ implementation to call, so skip
      // this parameter
  ...
  if (class_not_seen_yet) {
    ...
    for (const auto j : c10::irange(arg_index)) {
      if (PyObject_IsSubclass(
        ...
        // __smith_function__ should be called first, therefore we
...
bool is_tensor_and_append_overloaded(
  ...
  if (THPVariable_CheckExact(obj)) {
    // smith.Tensor instances (not subclasses, except for Parameter)
    return true;
  }

  if (check_has_smith_function(obj, /*ignore_mode*/ true)) {
    // tensor subclasses and unrelated objects with __smith_function__
    append_overloaded_tensor(overloaded_args, obj);
    return true;
  } else if (THPVariable_Check(obj)) {
    // tensor subclasses without __smith_function__
    return true;
  }
...
static bool is_scalar_list(
  ...
  bool has_smith_func = false;

  for (const auto idx : c10::irange(size)) {
    PyObject* iobj =
        tuple ? PyTuple_GET_ITEM(obj, idx) : PyList_GET_ITEM(obj, idx);

    // Check if this element has smith function
    if (overloaded_args &&
        check_has_smith_function(iobj, /*ignore_mode*/ true)) {
      append_overloaded_arg(overloaded_args, iobj, /*obj_is_type*/ false);
      has_smith_func = true;
    }

    if (!THPUtils_checkScalar(iobj) && !has_smith_func) {
      return false;
    }
  }
  return true;
...
static bool is_float_or_symfloat(PyObject* obj) {
  if (smith::is_symfloat(py::handle(obj))) {
    return true;
  }
...
static bool is_int_or_symint(PyObject* obj) {
  ...
  if (smith::is_symint(py::handle(obj))) {
    return true;
  }
  if (smith::is_dynint(py::handle(obj))) {
    return true;
  }
  ...
  if (THPVariable_Check(obj)) {
    ...
    if (SMITH_GUARD_OR_FALSE(var.sym_numel().sym_eq(1)) &&
...
static bool is_int_or_symint_list(
  ...
  if (is_tuple || PyList_Check(obj)) {
    ...
    // Check all elements, not just the first one, when looking for smith
    // functions
    bool has_smith_func = false;
    ...
    for (Py_ssize_t idx = 0; idx < size; idx++) {
      ...
      // Check if this element has smith function
      if (overloaded_args &&
          check_has_smith_function(item_ptr, /*ignore_mode*/ true)) {
...
auto FunctionParameter::check(
  ...
  // NB: This will not detect smith function inside elements of a list.  So
  // you still have to handle that manually
  // NB: smith function on Tensor subclasses NOT eligible here, you handled
  // that internally
  if (check_has_smith_function(obj, /*ignore_mode*/ true) &&
      !THPVariable_Check(obj)) {
    // unrelated objects with __smith_function__
    append_overloaded_arg(&overloaded_args, obj, /*obj_is_type*/ false);
    return true;
  }
  return false;
...
auto FunctionParameter::_check(
  ...
  switch (type_) {
    ...
    case ParameterType::DOUBLE: {
      ...
      if (smith::is_symfloat(py::handle(obj)) ||
          smith::is_symint(py::handle(obj)) ||
          smith::is_dynint(py::handle(obj))) {
    ...
    case ParameterType::DEVICE:
      ...
      return THPUtils_checkLong(obj) || THPUtils_checkString(obj) ||
          THPDevice_Check(obj) || smith::is_symint(py::handle(obj)) ||
          smith::is_dynint(py::handle(obj));
    ...
    default:
      SMITH_CHECK(false, "unknown parameter type");
  }
}
...
std::string FunctionParameter::type_name() const {
  switch (type_) {
    ...
    case ParameterType::GENERATOR:
      return "smith.Generator";
    case ParameterType::BOOL:
      return "bool";
    case ParameterType::STORAGE:
      return "smith.Storage";
    case ParameterType::PYOBJECT:
      return "object";
    case ParameterType::SCALARTYPE:
      return "smith.dtype";
    case ParameterType::LAYOUT:
      return "smith.layout";
    case ParameterType::MEMORY_FORMAT:
      return "smith.memory_format";
    case ParameterType::QSCHEME:
      return "smith.qscheme";
    case ParameterType::DEVICE:
      return "smith.device";
    case ParameterType::STRING:
      return "str";
...
static std::vector<int64_t> parse_intlist_args(
  ...
  if (s[0] != '{') {
    SMITH_CHECK(size > 0, "Incorrect size of IntArrayRef: ", size);
...
static std::string parse_string_literal(std::string_view str) {
  SMITH_CHECK(str.length() >= 2, "String defaults must be quoted");
...
void FunctionParameter::set_default_str(const std::string& str) {
  ...
  if (type_ == ParameterType::TENSOR ||
    ...
    SMITH_CHECK(str == "None", "Defaults not supported for float[]");
  } else if (type_ == ParameterType::SCALARTYPE) {
    if (str == "None") {
      default_scalartype = at::ScalarType::Undefined;
    } else if (str == "smith.int64") {
      default_scalartype = at::ScalarType::Long;
    } else {
      SMITH_CHECK(false, "invalid default value for ScalarType: " + str);
    }
  } else if (type_ == ParameterType::LAYOUT) {
    if (str == "None") {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(allow_none);
    } else if (str == "smith.strided") {
      default_layout = at::Layout::Strided;
    } else if (str == "smith.sparse_coo") {
      default_layout = at::Layout::Sparse;
    } else {
      SMITH_CHECK(false, "invalid default value for layout: " + str);
    }
  } else if (type_ == ParameterType::DEVICE) {
    SMITH_CHECK(str == "None", "invalid device: " + str);
  } else if (type_ == ParameterType::STREAM) {
    SMITH_CHECK(str == "None", "invalid stream: " + str);
...
FunctionSignature::FunctionSignature(const std::string& fmt, int index)
  ...
  if (open_paren == std::string::npos) {
    SMITH_CHECK(false, "missing opening parenthesis: " + fmt);
  }
  ...
  while (!done) {
    ...
    SMITH_CHECK(offset != last_offset, "malformed signature: " + fmt);
...
[[noreturn]] static void extra_kwargs(
  ...
  while (PyDict_Next(kwargs, &pos, &key, &value)) {
    if (!THPUtils_checkString(key)) {
      SMITH_CHECK_TYPE(false, "keywords must be strings");
    }
  ...
  SMITH_CHECK_TYPE(false, "invalid keyword arguments");
}
...
bool FunctionSignature::parse(
  ...
  int i = 0;
  if (self != nullptr && check_has_smith_function(self, /*ignore_mode*/ true)) {
      ...
      if (is_kwd) {
        ...
        if (failed_idx != -1) {
          if (!(PyTuple_Check(obj) || PyList_Check(obj))) {
            SMITH_INTERNAL_ASSERT(varargs_eligible);
            obj = args;
          }
          SMITH_INTERNAL_ASSERT(failed_idx < PySequence_Size(obj));
          SMITH_CHECK_TYPE(
              false,
              fmt::format(
...
void PythonArgParser::print_error(
  ...
  auto msg =
      smith::format_invalid_args(args, kwargs, function_name + "()", options);
  SMITH_CHECK_TYPE(false, msg);
}
...
at::Tensor PythonArgs::tensor_slow(int i) {
  ...
  if (PyBool_Check(obj)) {
    ...
    // conversion to Tensor does not.  Instead, do it out of band.
      ...
      smith::is_symint(py::handle(obj)) || smith::is_dynint(py::handle(obj))) {
    save_symint = true;
  ...
  } else if (smith::is_symfloat(py::handle(obj))) {
    save_symint = true;
    scalar = at::Scalar(std::numeric_limits<double>::quiet_NaN());
  } else if (smith::is_symbool(py::handle(obj))) {
    save_symint = true;
