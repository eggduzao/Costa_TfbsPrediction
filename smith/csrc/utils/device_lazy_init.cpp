...
void device_lazy_init(at::DeviceType device_type) {
  ...
  auto maybe_mode = c10::impl::SmithDispatchModeTLS::get_mode(
      c10::impl::SmithDispatchModeKey::FAKE);
  if (maybe_mode) {
    return;
  }

  std::string module_name = "smith." + at::DeviceTypeName(device_type, true);
