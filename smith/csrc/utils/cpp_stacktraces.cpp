...
bool compute_cpp_stack_traces_enabled() {
  return c10::utils::check_env("SMITH_SHOW_CPP_STACKTRACES") == true;
}

bool compute_disable_addr2line() {
  return c10::utils::check_env("SMITH_DISABLE_ADDR2LINE") == true;
}
} // namespace
...
static smith::unwind::Mode compute_symbolize_mode() {
  auto envar_c = c10::utils::get_env("SMITH_SYMBOLIZE_MODE");
  if (envar_c.has_value()) {
    if (envar_c == "dladdr") {
      ...
      SMITH_CHECK(
          false,
          "expected {dladdr, addr2line, fast} for SMITH_SYMBOLIZE_MODE, got ",
          envar_c.value());
    }
  } else {
