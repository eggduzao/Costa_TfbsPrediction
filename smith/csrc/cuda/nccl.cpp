...
#include <smith/csrc/cuda/device_set.h>
...
ncclComm_t* to_nccl_comm(smith::cuda::nccl::ncclComm_t* var) {
...
ncclUniqueId* to_nccl_unique_id(smith::cuda::nccl::ncclUniqueId* var) {
  return reinterpret_cast<ncclUniqueId*>(var);
}

ncclResult_t to_nccl_result(smith::cuda::nccl::ncclResult var) {
  switch (var) {
    case smith::cuda::nccl::ncclResult::Success:
      return ncclResult_t::ncclSuccess;
    case smith::cuda::nccl::ncclResult::UnhandledCudaError:
      return ncclResult_t::ncclUnhandledCudaError;
    case smith::cuda::nccl::ncclResult::SystemError:
      return ncclResult_t::ncclSystemError;
    case smith::cuda::nccl::ncclResult::InternalError:
      return ncclResult_t::ncclInternalError;
    case smith::cuda::nccl::ncclResult::InvalidArgument:
      return ncclResult_t::ncclInvalidArgument;
    case smith::cuda::nccl::ncclResult::InvalidUsage:
      return ncclResult_t::ncclInvalidUsage;
#ifdef NCCL_HAS_REMOTE_ERROR
    case smith::cuda::nccl::ncclResult::RemoteError:
      return ncclResult_t::ncclRemoteError;
#endif
#ifdef NCCL_HAS_COMM_NONBLOCKING
    case smith::cuda::nccl::ncclResult::InProgress:
      return ncclResult_t::ncclInProgress;
#endif
    case smith::cuda::nccl::ncclResult::NumResults:
      return ncclResult_t::ncclNumResults;
    default:
      SMITH_CHECK(false, "Unconvertible NCCL type");
  }
}

smith::cuda::nccl::ncclResult from_nccl_result(ncclResult_t var) {
  switch (var) {
    case ncclSuccess:
      return smith::cuda::nccl::ncclResult::Success;
    case ncclUnhandledCudaError:
      return smith::cuda::nccl::ncclResult::UnhandledCudaError;
    case ncclSystemError:
      return smith::cuda::nccl::ncclResult::SystemError;
    case ncclInternalError:
      return smith::cuda::nccl::ncclResult::InternalError;
    case ncclInvalidArgument:
      return smith::cuda::nccl::ncclResult::InvalidArgument;
    case ncclInvalidUsage:
      return smith::cuda::nccl::ncclResult::InvalidUsage;
#ifdef NCCL_HAS_REMOTE_ERROR
    case ncclRemoteError:
      return smith::cuda::nccl::ncclResult::RemoteError;
#endif
#ifdef NCCL_HAS_COMM_NONBLOCKING
    case ncclInProgress:
      return smith::cuda::nccl::ncclResult::InProgress;
#endif
    case ncclNumResults:
      return smith::cuda::nccl::ncclResult::NumResults;
    default:
      SMITH_CHECK(false, "Unconvertible NCCL type");
  }
}
...
#endif
    default:
      SMITH_CHECK(false, "Unconvertible NCCL type ", type);
  }
}
...
bool nccl_use_nonblocking() {
  static bool nccl_use_nonblocking_ =
      c10::utils::check_env("SMITH_NCCL_USE_COMM_NONBLOCKING") == true;
  if (nccl_use_nonblocking_) {
    SMITH_WARN("Using experimental non-blocking NCCL communicator.");
  }
...
static int nccl_nonblocking_timeout() {
  ...
  if (timeout == -2) {
    const auto val = c10::utils::get_env("SMITH_NCCL_NONBLOCKING_TIMEOUT");
...
void check_inputs(
  ...
  SMITH_CHECK(len != 0, "input sequence can't be empty");
...
#ifdef USE_NCCL
  using namespace smith::cuda::nccl::detail;
  NCCL_CHECK(ncclGetUniqueId(to_nccl_unique_id(&id)));
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
}
...
#endif
#else
  SMITH_CHECK(false, "all2all is only supported for NCCL lib version >= 2.7.0");
#endif
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
...
#endif
#else
  SMITH_CHECK(false, "Send is only supported for NCCL lib version >= 2.7.0");
#endif
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
...
#endif
#else
  SMITH_CHECK(false, "Recv is only supported for NCCL lib version >= 2.7.0");
#endif
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
...
#else
  SMITH_CHECK(false, "gather is only supported for NCCL lib version >= 2.7.0");
#endif
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
...
#endif
#else
  SMITH_CHECK(false, "scatter is only supported for NCCL lib version >= 2.7.0");
#endif
#else
  SMITH_CHECK(false, "Blacksmith built without NCCL support");
#endif
