#include <smith/csrc/cuda/comm.h>

#include <smith/csrc/cuda/device_set.h>
#include <smith/csrc/utils/tensor_flatten.h>
...
std::vector<Tensor> broadcast(const Tensor& tensor, IntArrayRef devices) {
  ...
  SMITH_INTERNAL_ASSERT(it == diff_device_dst_tensors.end());
  return dst_tensors;
}
...
tensor_list2d broadcast_coalesced(
  ...
  SMITH_CHECK(
      ...
      devices[0]);
#ifdef USE_NCCL
  buffer_size = std::min(smith::cuda::nccl::get_max_count(), buffer_size);
#endif
  ...
  for (auto& chunk : smith::utils::take_tensors(tensors, buffer_size)) {
    auto type_id = chunk.type_id();
    type_checker.show(type_id);
    std::vector<at::Tensor> results;
    if (chunk.options().is_sparse()) {
      auto flat_tuple = smith::utils::flatten_sparse_tensors(chunk.tensors);
      ...
      for (size_t i = 1, num_devices = devices.size(); i < num_devices; ++i) {
        ...
        for (const auto& var : smith::utils::unflatten_sparse_tensors(
      ...
      auto results = broadcast(
          smith::utils::flatten_dense_tensors(chunk.tensors), devices);
      for (size_t i = 1, num_devices = devices.size(); i < num_devices; ++i) {
        device_guard.set_index(static_cast<DeviceIndex>(devices[i]));
        auto& device_outputs = outputs[i];
        for (auto& var :
             smith::utils::unflatten_dense_tensors(results[i], chunk.tensors)) {
          // See NOTE [ Version Counter in comm.*_coalesced ]
          device_outputs.emplace_back(make_variable(var.tensor_data(), false));
        }
      }
  ...
  if (!type_checker.unique) {
    for (auto& o : outputs)
      smith::utils::reorder_tensors_like(o, tensors);
  }
  return outputs;
}
...
std::vector<at::Tensor> scatter(
    ...
    const std::optional<std::vector<std::optional<at::cuda::CUDAStream>>>&
        streams) {
  SMITH_CHECK(!devices.empty(), "Expected at least one device to scatter to");
...
at::Tensor& gather_out(
    ...
    at::Tensor& out_tensor,
    int64_t dim) {
  SMITH_CHECK(!tensors.empty(), "Expected at least one tensor to gather from");
  int64_t total_size = 0;
