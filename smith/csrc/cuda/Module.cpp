...
#ifdef USE_NCCL
#include <smith/csrc/cuda/python_nccl.h>
#endif
#include <c10/util/irange.h>

#include <smith/csrc/CudaIPCTypes.h>
#include <smith/csrc/Generator.h>
#include <smith/csrc/cuda/CUDAPluggableAllocator.h>
#include <smith/csrc/cuda/GdsFile.h>
#include <smith/csrc/cuda/THCP.h>
#include <smith/csrc/cuda/memory_snapshot.h>
#include <smith/csrc/cuda/python_comm.h>
...
PyObject* THCPModule_setDevice_wrap(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to setDevice");
...
PyObject* THCPModule_exchangeDevice(PyObject* self, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to exchangeDevice");
...
PyObject* THCPModule_getDeviceCount_wrap(PyObject* self, PyObject* noargs) {
  ...
  // fully initialize.
  smith::utils::register_fork_handler_for_device_init(at::kCUDA);
...
static PyObject* THCPModule_isInBadFork(PyObject* self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  return PyBool_FromLong(smith::utils::is_device_in_bad_fork(at::kCUDA));
  END_HANDLE_TH_ERRORS
}
...
// Entrypoint for the callable created by smith.cuda.jiterator
...
PyObject* THCPModule_cudaSleep(PyObject* _unused, PyObject* cycles) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(
      THPUtils_checkLong(cycles), "smith.cuda._sleep(): expected 'int'");
...
PyObject* THCPModule_memoryStats(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "invalid argument to memory_allocated");
...
PyObject* THCPModule_memorySnapshot(PyObject* _unused, PyObject* arg) {
  ...
  if (arg && arg != Py_None) {
    SMITH_CHECK(PyTuple_Check(arg), "Expected tuple or None");
    ...
    if (size == 2) {
      ...
      SMITH_CHECK(false, "Expected tuple of size 2 or 3");
    }
  }
  ...
  const auto segmentInfoToDict = [&](const SegmentInfo& segmentInfo) {
    ...
    // represent the stream rather than a smith.cuda.stream object
  ...
  py::str last_allocator_settings_s = "BLACKSMITH_CUDA_ALLOC_CONF";
...
static void registerCudaDeviceProperties(PyObject* module) {
  // Add _cudaDeviceProperties class to smith._C
  ...
  m.def(
      ...
      static_cast<void (*)(
          ...
          smith::cuda::_record_memory_history));
...
void removeStorageDeleterFns(
  ...
  for (c10::StorageImpl* stale_storage : stale_live_storages) {
    ...
    SMITH_CHECK(allocated_pointer != definitely_stale_pointers.end());
...
void addStorageDeleterFns(
  ...
  for (auto& data_ptr : delta.dataptrs_allocd) {
    ...
    if (storage_pair != storages.end()) {
      ...
      SMITH_CHECK(ctx == nullptr, " Not expecting deleter function");
...
static void registerCudaPluggableAllocator(PyObject* module) {
  ...
  m.def("_cuda_getAllocator", []() {
    return py::cast(smith::cuda::CUDAPluggableAllocator::getCurrentAllocator());
  });
  ...
  m.def(
      ...
      [](const std::shared_ptr<c10::cuda::CUDACachingAllocator::CUDAAllocator>&
             allocator) {
        smith::cuda::CUDAPluggableAllocator::changeCurrentAllocator(allocator);
      });
  py::class_<
      smith::cuda::CUDAPluggableAllocator::CUDAPluggableAllocator,
      c10::cuda::CUDACachingAllocator::CUDAAllocator,
      std::shared_ptr<
          smith::cuda::CUDAPluggableAllocator::CUDAPluggableAllocator>>(
      ...
      .def(
          "set_init_fn",
          [](smith::cuda::CUDAPluggableAllocator::CUDAPluggableAllocator& self,
             uint64_t func_ptr) {
  ...
  m.def("_cuda_customAllocator", [](uint64_t malloc_ptr, uint64_t free_ptr) {
    ...
    return smith::cuda::CUDAPluggableAllocator::createCustomAllocator(
        malloc_fn, free_fn);
  });
  ...
  m.def("_free_And_Remove_DeleterFn", [](size_t storage_impl_ptr) {
    ...
    SMITH_CHECK(succeeded, "Expected standard deleter");
  ...
  m.def(
      ...
      [](c10::DeviceIndex device, at::cuda::MempoolId_t mempool_id) {
        ...
        SMITH_CHECK(stream, "Expected stream capture to be under way");
...
#if C10_ASAN_ENABLED
  SMITH_WARN(
      "smith.cuda: your blacksmith binary has address sanitizer (asan) built in, "
      "asan is currently not compatible with smith.cuda module, "
      "you might get unexpected behavior (eg. out of memory, crash, etc.), "
      "please rebuild blacksmith without asan if you need to use this module");
#endif
  HANDLE_TH_ERRORS
  SMITH_INTERNAL_ASSERT(!smith::utils::is_device_in_bad_fork(at::kCUDA));
  smith::utils::register_fork_handler_for_device_init(at::kCUDA);
  at::globalContext().lazyInitDevice(c10::DeviceType::CUDA);

  auto m = THPObjectPtr(PyImport_ImportModule("smith.cuda"));
  if (!m)
    throw python_error();
