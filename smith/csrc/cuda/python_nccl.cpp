...
using namespace smith::cuda::nccl;
using namespace smith::cuda::nccl::detail;

static constexpr const char* COMM_CAPSULE_NAME = "smith.cuda.nccl.Communicator";
...
PyObject* THCPModule_nccl_reduce(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(
    ...
    THPUtils_invalidArguments(
        ...
        " int op, sequence[smith.cuda.Stream or None])");
    return nullptr;
  }
  ...
  {
    pybind11::gil_scoped_release no_gil;
    smith::cuda::nccl::reduce(inputs, output, root, op, streams, user_comms);
  }
...
PyObject* THCPModule_nccl_all_reduce(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(
    ...
    THPUtils_invalidArguments(
        ...
        " sequence[smith.cuda.Stream] streams,"
        " sequence[smith.cuda.nccl.Communicator] comms)");
    return nullptr;
  }
...
PyObject* THCPModule_nccl_broadcast(PyObject* self, PyObject* args) {
  ...
  std::vector<at::Tensor> inputs = extract_tensors(_inputs);
  SMITH_CHECK(root >= 0 && (size_t)root < inputs.size(), "invalid root");
  auto streams = unpack_streams(_streams, inputs.size());
  auto user_comms = unpack_comms(_comms, inputs.size());

  {
    pybind11::gil_scoped_release no_gil;
    smith::cuda::nccl::broadcast(inputs, streams, user_comms);
  }
