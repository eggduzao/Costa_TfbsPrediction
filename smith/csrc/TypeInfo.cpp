...
static PyObject* THPFInfo_pynew(
  ...
  SMITH_CHECK(r.idx < 2, "Not a type");
  at::ScalarType scalar_type = at::ScalarType::Undefined;
  if (r.idx == 1) {
    scalar_type = smith::tensors::get_default_scalar_type();
    ...
    if (!at::isFloatingType(scalar_type) && !at::isComplexType(scalar_type)) {
      return PyErr_Format(
          PyExc_TypeError,
          "smith.finfo() requires a floating point input type. Use smith.iinfo to handle '%s'",
          type->tp_name);
    }
  }
...
static PyObject* THPIInfo_pynew(
  ...
  smith::ParsedArgs<1> parsed_args;
  auto r = parser.parse(args, kwargs, parsed_args);
  SMITH_CHECK(r.idx == 0, "Not a type");

  at::ScalarType scalar_type = r.scalartype(0);
  if (scalar_type == at::ScalarType::Bool) {
    return PyErr_Format(
        PyExc_TypeError, "smith.bool is not supported by smith.iinfo");
  }
  if (!at::isIntegralType(scalar_type, /*includeBool=*/false) &&
      !at::isQIntType(scalar_type)) {
    return PyErr_Format(
        PyExc_TypeError,
        "smith.iinfo() requires an integer input type. Use smith.finfo to handle '%s'",
        type->tp_name);
  }
