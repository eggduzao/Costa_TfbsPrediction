...
#include <smith/csrc/serialization.h>
...
void doRead(io fildes, void* raw_buf, size_t nbytes) {
  ...
  while (nbytes > 0) {
    ...
    // see https://github.com/blacksmith/blacksmith/issues/1031 for more details
    Py_ssize_t r =
...
void THPStorage_writeFileRaw(
  ...
  if (save_size) {
    if (smith::utils::THP_nativeByteOrder() ==
        smith::utils::THPByteOrder::THP_LITTLE_ENDIAN)
