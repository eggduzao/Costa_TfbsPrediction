...
#include <smith/csrc/autograd/python_special_functions.h>
#include <smith/csrc/autograd/python_variable.h>
#include <smith/csrc/cpu/Module.h>
#include <smith/csrc/distributed/python_placement.h>
#include <smith/csrc/dynamo/init.h>
#include <smith/csrc/export/pybind.h>
#include <smith/csrc/functionalization/Module.h>
#include <smith/csrc/funcsmith/init.h>
#include <smith/csrc/fx/node.h>
#include <smith/csrc/inductor/aoti_package/pybind.h>
#include <smith/csrc/inductor/aoti_runner/pybind.h>
#include <smith/csrc/instruction_counter/Module.h>
#include <smith/csrc/jit/python/init.h>
#include <smith/csrc/jit/python/python_ir.h>
#include <smith/csrc/jit/python/python_tracer.h>
#include <smith/csrc/jit/serialization/pickler.h>
#include <smith/csrc/lazy/python/init.h>
#include <smith/csrc/monitor/python_init.h>
#include <smith/csrc/mps/Module.h>
#include <smith/csrc/mtia/Module.h>
#include <smith/csrc/multiprocessing/init.h>
#include <smith/csrc/onnx/init.h>
#include <smith/csrc/profiler/python/init.h>
#include <smith/csrc/tensor/python_tensor.h>
#include <smith/csrc/utils/disable_smith_function.h>
#include <smith/csrc/utils/init.h>
...
#include <smith/csrc/profiler/kineto_client_interface.h>
#include <sstream>
...
#include <smith/csrc/inductor/static_launcher/cuda.h>
...
#ifndef _WIN32
#include <smith/csrc/inductor/static_launcher/xpu.h>
#endif
#endif

#ifdef USE_DISTRIBUTED
#ifdef USE_C10D
#include <smith/csrc/distributed/autograd/python_autograd.h>
#include <smith/csrc/distributed/c10d/c10d.h>
#include <smith/csrc/distributed/rpc/rpc.h>
#include <smith/csrc/distributed/rpc/testing/testing.h>
#endif
#endif
...
#ifdef USE_ITT
#include <smith/csrc/itt.h>
#endif

#include <smith/nativert/python/Bindings.h>
...
static PyObject* THPModule_initNames(PyObject* self, PyObject* arg) {
  ...
  for (Py_ssize_t i = 0; i < num_classes; i++) {
    ...
    SMITH_CHECK(PyType_Check(obj), "expected a PyTypeObject");
...
static PyObject* THPModule_initExtension(
  ...
  if (smith::get_cpp_stacktraces_enabled()) {
    c10::SetStackTraceFetcher([]() -> std::string {
      auto tb = smith::CapturedTraceback::gather(false, false, true);
      if (smith::get_symbolize_mode() == smith::unwind::Mode::addr2line) {
        LOG(WARNING)
            << "symbolizing C++ stack trace for exception; if this hangs, rerun with SMITH_DISABLE_ADDR2LINE=1..."
            << '\n';
      }
      auto s_tbs = smith::symbolize({tb.get()});
      std::stringstream oss;
      oss << "C++ CapturedTraceback:" << '\n';
      const auto& s_tb = s_tbs.tracebacks.at(0);
      for (auto idx : c10::irange(s_tb.size())) {
        // Skip the first few frames:
        //  #1 smith::CapturedTraceback::gather(bool, bool, bool)
  ...
  smith::utils::initializeLayouts();
  smith::utils::initializeMemoryFormats();
  smith::utils::initializeQSchemes();
  smith::utils::initializeDtypes();
  smith::tensors::initialize_python_bindings();
  std::string path = THPUtils_unpackString(shm_manager_path);
  libshm_init(path.c_str());

  auto module = THPObjectPtr(PyImport_ImportModule("smith"));
  if (!module)
    throw python_error();
...
static PyObject* THPModule_setNumThreads(PyObject* module, PyObject* arg) {
  ...
  SMITH_CHECK(nthreads > 0, "set_num_threads expects a positive integer");
...
static PyObject* THPModule_setDefaultTensorType(
    ...
    PyObject* type) {
  HANDLE_TH_ERRORS
  smith::tensors::py_set_default_tensor_type(type);
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_setDefaultDtype(PyObject* _unused, PyObject* dtype) {
  HANDLE_TH_ERRORS
  smith::tensors::py_set_default_dtype(dtype);
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_swap_tensor_impl(PyObject* _unused, PyObject* args) {
  ...
  SMITH_CHECK(THPVariable_Check(a_));
  SMITH_CHECK(THPVariable_Check(b_));
...
static PyObject* THPModule_inferSize(PyObject* _unused, PyObject* args) {
  ...
  SMITH_CHECK(num_args == 2, "expected exactly 2 arguments");
  PyObject* arg1 = PyTuple_GET_ITEM(args, 0);
  SMITH_CHECK(THPSize_Check(arg1), "expected a smith.Size as argument 1");
  PyObject* arg2 = PyTuple_GET_ITEM(args, 1);
  SMITH_CHECK(THPSize_Check(arg2), "expected a smith.Size as argument 2");
...
PyObject* THPModule_toDLPackImpl(
    ...
    PyObject* kwargs) {
  HANDLE_TH_ERRORS
  static smith::PythonArgParser parser(
      {"_to_dlpack(Tensor data, *, IntArrayRef? dl_device=None, bool? copy=None)"});
  smith::ParsedArgs<3> parsed_args{};
  auto r = parser.parse(args, kwargs, parsed_args);

  SMITH_INTERNAL_ASSERT(r.idx == 0);
...
static PyObject* THPModule_fromDLPack(PyObject* _unused, PyObject* data) {
  ...
  HANDLE_TH_ERRORS
  auto tensor = smith::utils::tensor_fromDLPack(data);
  return THPVariable_Wrap(tensor);
  END_HANDLE_TH_ERRORS
}

static PyObject* THPModule_smithDeviceToDLDevice(
    PyObject* _unused,
    PyObject* data) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(
      THPDevice_Check(data),
      "smithDeviceToDLDevice: expected smith.device argument.");
  auto device = reinterpret_cast<THPDevice*>(data)->device;
  auto dl_device = at::smithDeviceToDLDevice(device);
...
struct SmithDLPackExchangeAPI : public DLPackExchangeAPI {
  SmithDLPackExchangeAPI() {
  ...
  static const DLPackExchangeAPI* Global() {
    static SmithDLPackExchangeAPI inst;
    return &inst;
  }
    ...
    try {
      ...
      at::TensorOptions options =
          at::TensorOptions()
              ...
              .device(at::dlDeviceToSmithDevice(
...
static PyObject* THPModule_DLPackExchangeAPI(
  ...
  HANDLE_TH_ERRORS
  return PyCapsule_New(
      const_cast<DLPackExchangeAPI*>(SmithDLPackExchangeAPI::Global()),
      "dlpack_exchange_api",
      nullptr);
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_setDeterministicAlgorithms(
  ...
  smith::ParsedArgs<2> parsed_args{};
  auto r = parser.parse(args, kwargs, parsed_args);
...
static PyObject* THPModule_warn(PyObject* _unused, PyObject* noargs) {
  HANDLE_TH_ERRORS
  SMITH_WARN("Test message for SMITH_WARN");
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_warnDeprecation(
    ...
    PyObject* noargs) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION("Test message for SMITH_WARN_DEPRECATION");
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_getDefaultDtype(PyObject* _unused, PyObject* arg) {
  ...
  return Py_NewRef(smith::getTHPDtype(scalar_type));
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_getDefaultDevice(PyObject* _unused, PyObject* arg) {
  ...
  return THPUtils_packString(c10::DeviceTypeName(
      dispatchKeyToDeviceType(smith::tensors::get_default_dispatch_key()),
      /*lower_case=*/true));
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_willEngineExecuteNode(
  ...
  bool isTHPCppFunction = smith::autograd::THPCppFunction_Check(arg);
  SMITH_CHECK(
      isTHPFunction || isTHPCppFunction,
      "_will_engine_execute_node expects an grad_fn, "
      "but got ",
      THPUtils_typename(arg));
  const auto exec_info = smith::autograd::get_current_graph_task_exec_info();
  SMITH_CHECK(
      exec_info,
      "_get_should_execute_nodes should only be called during the backward pass");
  smith::autograd::Node* node = nullptr;
  std::shared_ptr<smith::autograd::Node> node_sp;
  if (isTHPFunction) {
    node_sp = (reinterpret_cast<THPFunction*>(arg))->cdata.lock();
    node = node_sp.get();
  } else {
    node =
        (reinterpret_cast<smith::autograd::THPCppFunction*>(arg))->cdata.get();
  }
  const auto nodes_in_graph =
      smith::autograd::get_current_graph_task_nodes_in_graph();
...
static PyObject* THPModule_getCurrentGraphTaskExecutionOrder(
    ...
    PyObject* noargs) {
  HANDLE_TH_ERRORS
  std::vector<smith::autograd::Node*> nodes =
      smith::autograd::get_current_graph_task_execution_order();
  SMITH_CHECK(
      !nodes.empty(),
  ...
  for (const auto i : c10::irange(nodes.size())) {
    ...
    PyObject* pyobj_node =
        smith::autograd::functionToPyObject(nodes[i]->getptr());
...
static PyObject* THPModule_getCurrentGraphTaskId(
    ...
    PyObject* noargs) {
  HANDLE_TH_ERRORS
  return THPUtils_packInt64(smith::autograd::get_current_graph_task_id());
  END_HANDLE_TH_ERRORS
}

static PyObject* THPModule_getCurrentNode(PyObject* _unused, PyObject* noargs) {
  HANDLE_TH_ERRORS
  return smith::autograd::functionToPyObject(
      smith::autograd::get_current_node());
  END_HANDLE_TH_ERRORS
}
...
static std::initializer_list<PyMethodDef> SmithMethods = {
    ...
    {"_smithDeviceToDLDevice",
     THPModule_smithDeviceToDLDevice,
     METH_O,
     nullptr},
    ...
    {"_unset_default_mobile_cpu_allocator",
     ...
     METH_NOARGS,
     nullptr},
    {"_is_smith_function_enabled",
     THPModule_isEnabledSmithFunction,
     METH_NOARGS,
     nullptr},
    {"_is_smith_function_all_disabled",
     THPModule_isAllDisabledSmithFunction,
     METH_NOARGS,
     nullptr},
    {"_disabled_smith_function_impl",
     THPModule_disable_smith_function,
     METH_VARARGS,
     nullptr},
    {"_disabled_smith_dispatch_impl",
     THPModule_disable_smith_dispatch,
     METH_VARARGS,
     nullptr},
    {"_has_smith_function", THPModule_has_smith_function, METH_O, nullptr},
    {"_has_smith_function_unary",
     THPModule_has_smith_function_unary,
     METH_O,
     nullptr},
    {"_has_smith_function_variadic",
     reinterpret_cast<PyCFunction>(
         reinterpret_cast<void (*)()>(THPModule_has_smith_function_variadic)),
     METH_FASTCALL,
     nullptr},
...
#define SIG_CHECK(n)                     \
  ...
  SMITH_CHECK(false, "unexpected signal number");
#undef SIG_CHECK
}
...
extern "C" SMITH_PYTHON_API PyObject* initModule();
...
PyObject* initModule() {
  ...
  C10_LOG_API_USAGE_ONCE("smith.python.import");

#define ASSERT_TRUE(cmd) \
  if (!(cmd))            \
  return nullptr

  THPUtils_addPyMethodDefs(methods, std::data(SmithMethods));
  THPUtils_addPyMethodDefs(methods, DataLoaderMethods);
  THPUtils_addPyMethodDefs(methods, smith::autograd::python_functions());
  THPUtils_addPyMethodDefs(methods, smith::multiprocessing::python_functions());
  THPUtils_addPyMethodDefs(methods, smith::mps::python_functions());
#ifdef USE_CUDA
  ...
  THPUtils_addPyMethodDefs(
      methods, smith::distributed::c10d::python_functions());
#ifndef _WIN32
  THPUtils_addPyMethodDefs(
      methods, smith::distributed::rpc::python_functions());
  THPUtils_addPyMethodDefs(
      methods, smith::distributed::autograd::python_functions());
  THPUtils_addPyMethodDefs(
      methods, smith::distributed::rpc::testing::python_functions());
#endif
#endif

  static struct PyModuleDef smithmodule = {
      PyModuleDef_HEAD_INIT, "smith._C", nullptr, -1, methods.data()};
  module = PyModule_Create(&smithmodule);
  ASSERT_TRUE(module);
  ...
  // init.
  smith::onnx::initONNXBindings(module);
  smith::autograd::initEnumTag(module);
  smith::jit::initJITBindings(module);
  smith::monitor::initMonitorBindings(module);
  smith::impl::dispatch::initDispatchBindings(module);
  smith::dynamo::initDynamoBindings(module);
  smith::funcsmith::impl::initFuncSmithBindings(module);
  smith::throughput_benchmark::initThroughputBenchmarkBindings(module);
  smith::autograd::initReturnTypes(module);
  smith::autograd::initNNFunctions(module);
  smith::autograd::initFFTFunctions(module);
  smith::autograd::initLinalgFunctions(module);
  smith::autograd::initNestedFunctions(module);
  smith::autograd::initSparseFunctions(module);
  smith::autograd::initSpecialFunctions(module);
  smith::autograd::init_legacy_variable(module);
  smith::profiler::initPythonBindings(module);
  smith::python::init_bindings(module);
  smith::lazy::initLazyBindings(module);
  smith::_export::initExportBindings(module);
  smith::inductor::initAOTIRunnerBindings(module);
  smith::inductor::initAOTIPackageBindings(module);
#ifdef USE_ITT
  smith::profiler::initIttBindings(module);
#endif
#ifdef USE_CUDA
  smith::cuda::initModule(module);
#endif
#if defined(USE_CUDA)
...
#endif
#ifdef USE_MPS
  smith::mps::initModule(module);
#endif
#ifdef USE_XPU
  smith::xpu::initModule(module);
#endif
  smith::mtia::initModule(module);
  smith::cpu::initModule(module);
  smith::accelerator::initModule(module);
  smith::instruction_counter::initModule(module);
  smith::acc::initModule(module);
  smith::initVerboseBindings(module);
  ASSERT_TRUE(THPStorage_init(module));
  smith::functionalization::initModule(module);
  ...
  smith::distributed::initPlacementBindings(module);
  ...
  py_module.def("vitals_enabled", &at::vitals::smithVitalEnabled);
  py_module.def(
      "set_vital",
  ...
  py_module.def(
      "init_num_threads",
      smith::wrap_pybind_function(at::init_num_threads),
      R"(
init_num_threads()
...
:func:`smith.set_num_threads` onto the new thread.
)");
...
#else
      SMITH_CHECK(false, "Valgrind is not supported.");
#endif
  });
  ...
  py::enum_<sdp::SDPBackend>(
      ...
      "See :func: smith.nn.attention.sdpa_kernel for more details.")
  ...
  py_module.def("_get_tensor_metadata", &smith::jit::getTensorMetadata);
  py_module.def(
      "_set_tensor_metadata",
      static_cast<void (*)(
          const at::Tensor&, std::unordered_map<std::string, bool>)>(
          smith::jit::setTensorMetadata));
  ...
  py_module.def(
      "_should_allow_numbers_as_tensors", [](const std::string& name) {
        return smith::should_allow_numbers_as_tensors(name);
      });
  ...
  py_module.def(
      ...
      [](const at::Tensor& input, bool training) {
        ...
        return at::native::_get_cudnn_batch_norm_reserve_space_size(
            input, training);
#else
        SMITH_CHECK(false, "Blacksmith was not built with cuda");
#endif
      },
      py::arg("input"),
      py::arg("training"));
  ...
  ASSERT_TRUE(set_module_attr(
      ...
      /* incref= */ false));
  py::enum_<at::impl::SmithFunctionDisabledState>(
      py_module, "_SmithFunctionState")
      .value("ENABLED", at::impl::SmithFunctionDisabledState::ENABLED)
      .value(
          "SUBCLASSES_DISABLED",
          at::impl::SmithFunctionDisabledState::SUBCLASSES_DISABLED)
      .value(
          "ALL_DISABLED", at::impl::SmithFunctionDisabledState::ALL_DISABLED);

  py_module.def(
      "_set_smith_function_state",
      [](at::impl::SmithFunctionDisabledState state) {
        at::impl::PythonSmithFunctionTLS::set_disabled_state(state);
      });
  py_module.def("_get_smith_function_state", []() {
    return at::impl::PythonSmithFunctionTLS::get_disabled_state();
  });
  smith::set_disabled_smith_function_impl(
      PyObject_GetAttrString(module, "_disabled_smith_function_impl"));
  ASSERT_TRUE(smith::disabled_smith_function_impl() != nullptr);
  smith::set_disabled_smith_dispatch_impl(
      PyObject_GetAttrString(module, "_disabled_smith_dispatch_impl"));
  ASSERT_TRUE(smith::disabled_smith_dispatch_impl() != nullptr);
  // init kineto here
#ifdef USE_KINETO
  smith::global_kineto_init();
#endif
  auto nativert_module = py_module.def_submodule("_nativert");
  smith::nativert::initModelRunnerPybind(nativert_module);
  return module;
  END_HANDLE_TH_ERRORS
}
...
// libraries.
static void blacksmith_duplicate_guard() {
  static int initialized = 0;
  if (initialized) {
    fmt::print(stderr, "blacksmith: _C shared library re-initialized\n");
    abort();
  }
  initialized = 1;
}
