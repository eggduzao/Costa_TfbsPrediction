...
#include <smith/csrc/shim_conversion_utils.h>
#include <smith/csrc/stable/c/shim.h>

AOTISmithError smith_new_list_reserve_size(size_t size, StableListHandle* ret) {
...
AOTISmithError smith_list_push_back(
    StableListHandle list_handle,
    StableIValue element) {
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    std::vector<StableIValue>* list = list_handle_to_list_pointer(list_handle);
...
static StableIValue from_ivalue(
  ...
  switch (type->kind()) {
    case c10::TypeKind::TensorType: {
      AtenTensorHandle ath = smith::aot_inductor::new_tensor_handle(
          std::move(const_cast<at::Tensor&>(ivalue.toTensor())));
      return smith::stable::detail::_from(ath, extension_build_version);
    }
    case c10::TypeKind::IntType: {
      return smith::stable::detail::_from(
    ...
    case c10::TypeKind::OptionalType: {
      ...
      //
      // return
      // smith::stable::detail::from<std::optional<inner_type::t>>(ivalue.toInnerTypeT()));
      //
      ...
      // be kept in sync with smith::stable::detail::from<std::optional<T>>
      // function in smith/csrc/stable/stableivalue_conversions.h
      if (ivalue.isNone()) {
        return smith::stable::detail::_from(
      ...
      return smith::stable::detail::_from(sivp, extension_build_version);
    }
...
static c10::IValue to_ivalue(
  ...
  switch (type->kind()) {
    case c10::TypeKind::TensorType: {
      auto ret_raiiath = smith::aot_inductor::RAIIAtenTensorHandle(
          smith::stable::detail::_to<AtenTensorHandle>(
              stable_ivalue, extension_build_version));
      return (c10::IValue(*smith::aot_inductor::tensor_handle_to_tensor_pointer(
          ret_raiiath.get())));
    }
    case c10::TypeKind::IntType: {
      return c10::IValue(smith::stable::detail::_to<int64_t>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::FloatType: {
      return c10::IValue(smith::stable::detail::_to<double>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::BoolType: {
      return c10::IValue(smith::stable::detail::_to<bool>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::ScalarTypeType: {
      return c10::IValue(smith::stable::detail::_to<c10::ScalarType>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::DeviceObjType: {
    ...
    case c10::TypeKind::LayoutType: {
      return c10::IValue(smith::stable::detail::_to<c10::Layout>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::MemoryFormatType: {
      return c10::IValue(smith::stable::detail::_to<c10::MemoryFormat>(
          stable_ivalue, extension_build_version));
    }
    case c10::TypeKind::OptionalType: {
      auto inner_type = type->castRaw<at::OptionalType>()->getElementType();
      ...
      //
      // return
      // c10::IValue(smith::stable::detail::to<std::optional<inner_type::t>>(stable_ivalue));
      //
      // BUT we do NOT have that type inner_type::t readily available, so we
      // will manually unwrap and recursively call. This implementation MUST
      // be kept in sync with the smith::stable::detail::_to<T> function in
      // smith/csrc/stable/library.h
      if (stable_ivalue ==
          smith::stable::detail::_from(std::nullopt, extension_build_version)) {
        return c10::IValue();
      }
      auto sivp = smith::stable::detail::_to<StableIValue*>(
          stable_ivalue, extension_build_version);
    ...
    case c10::TypeKind::ListType: {
      auto inner_type = type->castRaw<c10::ListType>()->getElementType();
      auto list_handle = smith::stable::detail::_to<StableListHandle>(
          stable_ivalue, extension_build_version);
    ...
    case c10::TypeKind::StringType: {
      return c10::IValue(smith::stable::detail::_to<std::string>(
          stable_ivalue, extension_build_version));
    }
    default: {
      SMITH_CHECK(
  ...
  void operator()(
    ...
    for (const auto idx : c10::irange(num_arguments)) {
      ...
      ministack[ministack_idx] = from_ivalue(
          arg_type, smith::jit::pop(stack), extension_build_version_);
    }
...
AOTI_SMITH_EXPORT AOTISmithError aoti_smith_library_impl(
    ...
    void (*fn)(StableIValue*, uint64_t, uint64_t)) {
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    reinterpret_cast<smith::Library*>(self)->impl(
        name,
        smith::CppFunction::makeFromBoxedFunctor(
            std::make_unique<StableIValueBoxedKernel>(fn, SMITH_ABI_VERSION)));
  });
}
...
AOTISmithError aoti_smith_call_dispatcher(
    const char* opName,
  ...
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    for (const auto idx : c10::irange(num_arguments)) {
      ...
      smith::jit::push(
          ivalue_stack, to_ivalue(arg_type, stable_ivalue, SMITH_ABI_VERSION));
    }
    ...
    for (const auto idx : c10::irange(num_returns)) {
      ...
      stack[stack_idx] = from_ivalue(
          ret_type, smith::jit::pop(ivalue_stack), SMITH_ABI_VERSION);
    }
  });
}
...
namespace {
using SchemaAdapterFn = std::function<smith::jit::Stack(
...
[[maybe_unused]] AOTISmithError register_schema_adapter(
    const char* op_name,
...
static AOTISmithError _register_adapters() {
  // ** Schema adapters should be registered here**
  // Refer to https://github.com/blacksmith/blacksmith/pull/165284/ for an example.
  //
...
AOTI_SMITH_EXPORT AOTISmithError smith_call_dispatcher(
  ...
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    if (auto adapter = registry.get_adapter(opName, extension_build_version)) {
      ...
      // No adapter needed - implementation matches aoti_smith_call_dispatcher
      ivalue_stack.reserve(std::max(num_arguments, num_returns));
      for (const auto idx : c10::irange(num_arguments)) {
        auto stable_ivalue = stack[idx];
        auto arg_type = schema.arguments()[idx].type();
    ...
    for (const auto idx : c10::irange(num_returns)) {
      ...
      const c10::TypePtr& ret_type = schema.returns()[idx].type();
      stack[stack_idx] = from_ivalue(
          ret_type, smith::jit::pop(ivalue_stack), extension_build_version);
    }
  });
}
...
AOTI_SMITH_EXPORT AOTISmithError
smith_get_thread_idx(uint32_t* out_thread_idx) {
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE(
...
AOTI_SMITH_EXPORT AOTISmithError
smith_get_const_data_ptr(AtenTensorHandle tensor, const void** ret_data_ptr) {
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    at::Tensor* t =
        smith::aot_inductor::tensor_handle_to_tensor_pointer(tensor);
    *ret_data_ptr = t->const_data_ptr();
  });
}

AOTI_SMITH_EXPORT AOTISmithError
smith_get_mutable_data_ptr(AtenTensorHandle tensor, void** ret_data_ptr) {
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    at::Tensor* t =
        smith::aot_inductor::tensor_handle_to_tensor_pointer(tensor);
...
AOTI_SMITH_EXPORT AOTISmithError smith_from_blob(
  ...
  AOTI_SMITH_CONVERT_EXCEPTION_TO_ERROR_CODE({
    ...
    *ret_new_tensor = smith::aot_inductor::new_tensor_handle(std::move(tensor));
  });
}
