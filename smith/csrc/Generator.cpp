...
static PyObject* THPGenerator_pynew(
    ...
    PyObject* kwargs) {
  HANDLE_TH_ERRORS
  static smith::PythonArgParser parser({"Generator(Device device=None)"});
  smith::ParsedArgs<1> parsed_args;
...
static PyObject* THPGenerator_setState(PyObject* _self, PyObject* _new_state) {
  ...
  if (!THPVariable_Check(_new_state)) {
    SMITH_CHECK_TYPE(
        false,
        fmt::format(
            "expected a smith.ByteTensor, but got {}",
...
static PyObject* THPGenerator_reduce(PyObject* _self, PyObject* noargs) {
  ...
  py::object smith_generator = smith_module.attr("Generator");
  PyTuple_SET_ITEM(ret.get(), 0, smith_generator.release().ptr());
...
static void set_pyobj(const Generator& self, PyObject* pyobj) {
  SMITH_CHECK(self.defined(), "cannot call set_pyobj() on undefined generator");
  self.set_pyobj(pyobj);
}

static PyObject* pyobj(const Generator& self) {
  SMITH_CHECK(self.defined(), "cannot call pyobj() on undefined generator");
  return self.pyobj();
}
