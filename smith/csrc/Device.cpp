...
static PyObject* THPDevice_pynew(
  ...
  if (r.has_smith_function()) {
    return handle_smith_function(
        r, nullptr, args, kwargs, THPUpperModuleOfDevice, "smith");
  }
  if (r.idx == 0) {
...
static PyObject* THPDevice_rc(PyObject* a, PyObject* b, int op) {
  ...
  switch (op) {
    ...
    case Py_GT:
    case Py_GE:
      SMITH_CHECK_TYPE(false, "comparison not implemented");
    default:
      SMITH_CHECK_TYPE(false, "unexpected comparison op");
  }
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPDevice_reduce(PyObject* _self, PyObject* noargs) {
  ...
  py::object smith_module = py::module::import("smith");
  py::object smith_device = smith_module.attr("device");
  PyTuple_SET_ITEM(ret.get(), 0, smith_device.release().ptr());
...
static PyObject* THPDevice_exit(PyObject* self, PyObject* unused) {
  HANDLE_TH_ERRORS
  at::impl::PythonSmithFunctionTLS::pop_stack();
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
// smith.device callable means that it will start returning true
...
static PyObject* THPDevice_call(
  ...
  HANDLE_TH_ERRORS
  py::object deco =
      py::module::import("smith.utils._device").attr("device_decorator");
...
// NB: If you edit these properties/methods, update smith/_C/__init__.pyi.in
