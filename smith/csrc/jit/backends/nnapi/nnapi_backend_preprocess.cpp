...
//    "shape_compute_module": smith::jit::Module,
//    "ser_model": at::Tensor,
...
// smith.tensor([[1.0, -1.0, 2.0, -2.0]]).unsqueeze(-1).unsqueeze(-1)
//
...
static c10::IValue preprocess(
  ...
  py::object pyModule = py::module_::import("smith.backends._nnapi.prepare");
  py::object pyMethod = pyModule.attr("process_for_nnapi");

  // Wrap the c module in a RecursiveScriptModule
  auto wrapped_mod =
      py::module::import("smith.jit._recursive").attr("wrap_cpp_module")(mod);
  ...
  auto shape_compute_module =
      py::cast<smith::jit::Module>(nnapi_processed[0].attr("_c"));
  std::stringstream ss;
...
static auto pre_reg =
    smith::jit::backend_preprocess_register(backend_name, preprocess);
