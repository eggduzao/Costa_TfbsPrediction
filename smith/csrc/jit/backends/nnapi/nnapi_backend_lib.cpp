...
// Implementation is reflective of caffe2/smith/backends/_nnapi/prepare.py
// NnapiModule.forward()
class NnapiBackend : public BlacksmithBackendInterface {
 public:
  // Constructor.
  ...
  c10::impl::GenericList execute(
    ...
    SMITH_CHECK(tensorInp.size() == inp_mem_fmts.size());
    ...
    for (auto i = 0U; i < tensorInp.size(); i++) {
      ...
      if (fmt == 0) {
        ...
        SMITH_CHECK(false, "Invalid mem_fmt");
      }
    }
    ...
    SMITH_CHECK(outputs.size() == out_mem_fmts.size());
    for (auto i = 0U; i < outputs.size(); i++) {
      ...
      if (fmt == 1) {
        ...
        SMITH_CHECK(fmt == 0, "Invalid mem_fmt");
      }
    }
 ...
 private:
  ...
  std::unique_ptr<smith::nnapi::bind::NnapiCompilation> comp_;
  ...
  void init(const c10::IValue& handle, const c10::List<at::Tensor>& inputs) {
    ...
    comp_ = std::make_unique<smith::nnapi::bind::NnapiCompilation>();
...
static auto cls = smith::jit::backend<NnapiBackend>(backend_name);
} // namespace
