...
#include <smith/csrc/jit/backends/backend_interface.h>
...
class backend {
  static_assert(
      std::is_base_of_v<BlacksmithBackendInterface, TBackendInterface>,
      "smith::jit::backend<T> requires T to inherit from BlacksmithBackendInterface");
 ...
 public:
  ...
  backend(const std::string& name) : backend_name_(name) {
    static auto cls = smith::class_<TBackendInterface>(kBackendsNamespace, name)
                          .def(smith::init<>())
                          ._def_unboxed(
                              "is_available",
