...
#include <caffe2/smith/csrc/jit/backends/xnnpack/compiler/xnn_compiler.h>
#include <smith/csrc/jit/backends/xnnpack/serialization/schema_generated.h>

namespace smith::jit::xnnpack::delegate {
...
class XNNPackBackend : public BlacksmithBackendInterface {
 public:
  // Constructor.
  ...
  c10::impl::GenericList execute(
    ...
    for (const at::IValue& val : inputs) {
      SMITH_CHECK(val.isTensor(), "Non-tensor inputs not supported");
    ...
    SMITH_CHECK(executor.forward(), "Failed to invoke XNNPack runtime");
...
static auto cls = smith::jit::backend<XNNPackBackend>(backend_name);
} // namespace
