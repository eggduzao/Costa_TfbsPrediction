...
#include <caffe2/smith/csrc/jit/backends/xnnpack/xnnpack_graph_builder.h>
...
std::shared_ptr<smith::jit::Graph> XNNGraph::optimizeAndTraceGraph(
...
void XNNGraph::buildXNNGraph(
  ...
  std::unordered_set<smith::jit::Value*> externals;
  ...
  xnn_status status = xnn_create_subgraph(
      ...
      /*flags=*/0,
      &_subgraph_ptr);
  SMITH_CHECK(xnn_status_success == status, "Failed to create xnn subgraph");
...
void XNNGraph::runGraphOnInputs(
  ...
  SMITH_CHECK(xnn_status_success == status, "runtime not properly setup");

  SMITH_CHECK(xnn_status_success == xnn_invoke_runtime(auto_runtime.get()));
}

void XNNGraph::checkOpsToDelegate(std::shared_ptr<smith::jit::Graph>& graph) {
...
void XNNGraph::defineAllNodes(std::shared_ptr<smith::jit::Graph>& graph) {
  DepthFirstGraphNodeIterator it(graph);
  Node* node = nullptr;
  while ((node = it.next()) != nullptr) {
    switch (node->kind()) {
      ...
      case aten::add: {
        ...
        SMITH_CHECK(status == xnn_status_success, "failed to create add node");
        break;
      }
      default: {
        throw std::exception();
...
void XNNGraph::defineAllTensorValues() {
  ...
  for (auto val : _intermediate_tensors) {
    if (_val_to_ids.find(val) == _val_to_ids.end()) {
      ...
      for (auto dim : sizes) {
        SMITH_CHECK(dim >= 0, "Input Dims should be unsigned");
...
void XNNGraph::gatherTensorValues(std::shared_ptr<smith::jit::Graph>& graph) {
...
void XNNGraph::gatherNodeInputs(smith::jit::Node& node) {
  switch (node.kind()) {
    case aten::add: {
...
bool XNNGraph::isGraphInput(smith::jit::Value* val) {
  return std::count(_inputs.begin(), _inputs.end(), val) > 0;
};

bool XNNGraph::isGraphOutput(smith::jit::Value* val) {
