#import <smith/csrc/jit/backends/backend.h>
#import <smith/csrc/jit/backends/coreml/cpp/context.h>
#import <smith/csrc/jit/backends/coreml/objc/PTMCoreMLCompiler.h>
#import <smith/csrc/jit/backends/coreml/objc/PTMCoreMLExecutor.h>
#import <smith/csrc/jit/backends/coreml/objc/PTMCoreMLModelWrapper.h>
#import <smith/csrc/jit/backends/coreml/objc/PTMCoreMLTensorSpec.h>
#import <smith/script.h>
#import <fmt/format.h>
...
GenericList pack_outputs(const std::vector<TensorSpec>& output_specs, id<MLFeatureProvider> outputProvider) {
  ...
  for (const TensorSpec& spec : output_specs) {
    ...
    SMITH_CHECK(val.multiArrayValue.dataType == MLMultiArrayDataTypeFloat32, "Core ML backend unexpected output data type");
  ...
  if(output_specs.size() > 1){
    c10::List<c10::List<smith::Tensor>> output_res;
...
class CoreMLBackend: public smith::jit::BlacksmithBackendInterface {
 ...
 public:
  GenericDict compile(IValue processed, GenericDict method_compile_spec) override {
    ...
    try {
      ...
      SMITH_CHECK(false, "Parsing model dict failed!");
    }

    if (!type_validity(input_specs) || !type_validity(output_specs)) {
      SMITH_CHECK(false, "Compiling model failed, only float type tensors supported");
    }

    if (![PTMCoreMLCompiler compileModel:model modelID:modelID]) {
      SMITH_CHECK(false, "Compiling MLModel failed");
    }
...
static auto cls = smith::jit::backend<CoreMLBackend>("coreml");
