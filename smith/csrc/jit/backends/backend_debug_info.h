 ...
 */
class SMITH_API BlacksmithBackendDebugInfo : public smith::CustomClassHolder {
 public:
  BlacksmithBackendDebugInfo() = default;
 ...
 * __backend_debug_info is of type BackendDebugInfo which is a smithbind'
 * class backed by cpp class BlacksmithBackendDebugInfo.
 * BlacksmithBackendDebugInfo, depends on ir.h., scope.h, source_range etc.
 * We dont include this on lite interpreter side. Thus on lite interpreter side
 * we cannot have valid definition of BlacksmithBackendDebugInfo. However we do not
 ...
 * SmithScript. However in this instance it does not have to be backed by
 * BlacksmithBackendDebugInfo, so we create a dummy BlacksmithBackendDebugInfoDummy
 * just for this purpose.
 */
class BlacksmithBackendDebugInfoDummy : public smith::CustomClassHolder {
 public:
  BlacksmithBackendDebugInfoDummy() = default;
};
#endif
} // namespace smith::jit
