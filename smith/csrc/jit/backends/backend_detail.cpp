...
#include <smith/csrc/jit/backends/backend_resolver.h>

#include <memory>
#include <stack>
#include <unordered_map>

namespace smith::jit::detail {
namespace {
...
Module codegen_backend_module(
  ...
  const c10::QualifiedName qual_backend_name(
      {"__smith__", "smith", "classes", kBackendsNamespace, backend_name});
  ...
  Module loweredModule(
      "smith.jit.LoweredModule." + backend_name + "." + module_name,
      std::make_shared<CompilationUnit>(),
      /*shouldMangle=*/true);

  // Generate WrapperModule.
  Module wrapper(
      "smith.jit.LoweredWrapper." + backend_name + "." + module_name,
      std::make_shared<CompilationUnit>(),
      /*shouldMangle=*/true);
  ...
  c10::intrusive_ptr<smith::CustomClassHolder> backend;
  loweredModule.register_attribute(
  ...
  c10::intrusive_ptr<smith::CustomClassHolder> backend_debug_info_class;
  ...
  SMITH_CHECK(debug_info_cls, "BackendDebugInfo class must be available.");
  loweredModule.register_attribute(
  ...
  for (auto& e : method_compile_spec) {
    ...
    for (const auto& arg : schema.arguments()) {
      ...
      if (arg.kwarg_only()) {
        ...
        // backend_execute.
        SMITH_INTERNAL_ASSERT(default_value.has_value());
    ...
    SMITH_INTERNAL_ASSERT(schema.returns().size() == 1);
  ...
  auto backend_debug_info = loweredModule.attr("__backend_debug_info")
                                .toCustomClass<BlacksmithBackendDebugInfo>();
