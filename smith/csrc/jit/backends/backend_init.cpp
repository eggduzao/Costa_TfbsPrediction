...
static void toBackendSelectiveImpl(
  ...
  for (const auto& module_to_lower : modules_to_lower) {
    ...
    auto lowered_submodule =
        py::cast<Module>(to_backend(py::module::import("smith.jit._recursive")
...
void initJitBackendBindings(PyObject* module) {
  ...
  //
  //  static auto cls = smith::jit::backend<ExampleBackend>("example_backend");
  //
  // this function must be called like
  //
  //  smith._C._jit_to_backend("example_backend", module, spec)
  ...
  m.def(
      ...
      [=](const std::string& backend_name,
        ...
        return py::module::import("smith.jit._recursive")
