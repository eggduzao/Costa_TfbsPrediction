...
// Content of the file is written using BlacksmithStreamWriter, for details please
...
class ScriptModuleDeserializer final {
 public:
  ScriptModuleDeserializer(
      ...
      std::shared_ptr<BlacksmithStreamReader> reader)
 ...
 private:
  ...
  std::shared_ptr<BlacksmithStreamReader> reader_;
...
Module ScriptModuleDeserializer::deserialize(
  ...
  C10_LOG_API_USAGE_ONCE("smith.jit.load");
  device_ = device;
  // Load extra files.
  ...
  if (reader_->hasRecord("model.json") && code_prefix_ == "code/") {
    SMITH_CHECK(false, "Legacy model format is not supported on mobile.");
  }
  ...
  if (restore_shapes && reader_->hasRecord("traced_inputs.pkl")) {
    ...
    if (restore_shapes) {
      SMITH_WARN("Cannot restore shapes as no traced inputs were stored");
    }
  }
  c10::LogAPIUsageMetadata(
      "smith.script.load.metadata",
...
Module import_ir_module(
  ...
  if (getFileFormat(in) != FileFormat::FlatbufferFileFormat) {
    auto reader = std::make_unique<BlacksmithStreamReader>(&in);
...
Module import_ir_module(
  ...
  if (getFileFormat(filename) != FileFormat::FlatbufferFileFormat) {
    auto reader = std::make_unique<BlacksmithStreamReader>(filename);
    reader->setShouldLoadDebugSymbol(load_debug_files);
    ScriptModuleDeserializer deserializer(std::move(cu), std::move(reader));
    return deserializer.deserialize(device, extra_files, restore_shapes);
  }
...
Module import_ir_module(
    ...
    ExtraFilesMap& extra_files,
    bool load_debug_files) {
  auto reader = std::make_shared<BlacksmithStreamReader>(std::move(rai));
  reader->setShouldLoadDebugSymbol(load_debug_files);
  ScriptModuleDeserializer deserializer(std::move(cu), std::move(reader));
  return deserializer.deserialize(device, extra_files);
}
...
Module _load_jit_module_from_bytes(
    ...
    ExtraFilesMap& extra_files,
    bool restore_shapes) {
  SMITH_CHECK(size >= kFileFormatHeaderSize, "Unrecognized data format");
  ...
  switch (format) {
    ...
    default:
      SMITH_CHECK(false, "Unrecognized data format");
  }
}
