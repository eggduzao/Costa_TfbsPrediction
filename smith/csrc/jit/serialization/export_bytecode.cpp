...
#include <smith/csrc/jit/api/method.h>
        ...
        if (input_type->kind() == TypeKind::ListType ||
          ...
          for (const TypePtr& element_type : input_type->containedTypes()) {
            SMITH_CHECK(
                ...
                "Returning a list or dictionary with blacksmith class type ",
                "is not supported in mobile module "
                "(List[Foo] or Dict[int, Foo] for class Foo(smith.nn.Module)). "
                "Workaround: instead of using blacksmith class as their element type, ",
