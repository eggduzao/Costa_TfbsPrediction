...
struct SMITH_API ClassNamespaceValue : public SugaredValue {
  /**
...
SourceImporterImpl::SourceImporterImpl(
    ...
    : cu_(std::move(cu)),
      ...
      version_(version) {
  env_ = {
      {"smith", std::make_shared<BuiltinModule>("aten", version)},
...
void SourceImporterImpl::parseSourceIfNeeded(const std::string& qualifier) {
  // qualifier may be blank, for instance checking if __smith__ is a class.
...
std::optional<Assign> SourceImporterImpl::
  ...
  static std::unordered_map<std::string, AttrTypeReplacementDescr> replacements{
      {"__smith__.smith.ao.nn.quantized.modules.linear.LinearPackedParams",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}},
      {"__smith__.smith.ao.nn.quantized.modules.linear.Linear",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}},
      {"__smith__.smith.ao.nn.quantized.dynamic.modules.linear.Linear",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}},
      {"__smith__.smith.ao.nn.quantized.modules.conv.Conv2d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv2dPackedParamsBase"}},
      {"__smith__.smith.nn.intrinsic.quantized.modules.conv_relu.ConvReLU2d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv2dPackedParamsBase"}},
      {"__smith__.smith.ao.nn.quantized.modules.conv.Conv3d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv3dPackedParamsBase"}},
      {"__smith__.smith.nn.intrinsic.quantized.modules.conv_relu.ConvReLU3d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv3dPackedParamsBase"}},
      // BC Stuff
      {"__smith__.smith.nn.quantized.modules.linear.LinearPackedParams",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}},
      {"__smith__.smith.nn.quantized.modules.linear.Linear",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}},
      {"__smith__.smith.nn.quantized.modules.conv.Conv2d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv2dPackedParamsBase"}},
      {"__smith__.smith.nn.quantized.modules.conv.Conv3d",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.Conv3dPackedParamsBase"}},
      {"__smith__.smith.nn.quantized.dynamic.modules.linear.Linear",
       {"_packed_params",
        "Tensor",
        "__smith__.smith.classes.quantized.LinearPackedParamsBase"}}};
  // @lint-ignore-every CLANGTIDY facebook-hte-StdRegexIsAwful
  static std::regex mangle_re("\\.___smith_mangle_\\d+");
...
void SourceImporterImpl::importClass(
    ...
    const ClassDef& class_def,
    bool is_module) {
  // BC for SmithBind classes
  //
  // Previously we would serialize SmithBind classes as actual
  // classes with methods that delegate to things in the
  // smith.ops.* namespace. We've switched away from this and
  // now just rely on those classes being present in the binary
  // and emit code for them based on the ClassType in memory.
  //
  // TODO: remove this once we no longer have old SmithBind code
  // in production models
  {
    static QualifiedName smith_classes_qualname("__smith__.smith.classes");
    if (smith_classes_qualname.isPrefixOf(qualified_classname)) {
      return;
    }
  }
      ...
      case TK_ASSIGN: {
        ...
        switch (assign.lhs().kind()) {
          ...
          case TK_SUBSCRIPT: {
            ...
            SMITH_INTERNAL_ASSERT(lhs.subscript_exprs().size() == 1);
            attributes.push_back(assign);
          } break;
          default: {
            SMITH_INTERNAL_ASSERT(
  ...
  for (const auto& assign : attributes) {
    ...
    switch (assign.lhs().kind()) {
      case TK_VAR: {
        const auto name = Var(assign.lhs()).name().name();
        SMITH_INTERNAL_ASSERT(name != "__parameters__");
...
void SourceImporterImpl::importNamedTuple(
  ...
  for (const auto& statement : named_tuple_def.body()) {
    ...
    const auto assign = Assign(statement);
    SMITH_INTERNAL_ASSERT(assign.type().present());
    ...
    if (assign.rhs().present()) {
      ...
      SMITH_INTERNAL_ASSERT(parsed.size() == 1);
...
void SourceImporterImpl::parsePossibleVersionNumber(Lexer& L) {
  ...
  // BlacksmithStreamReader. We used to check if op_version_set was _newer_ for
