...
#include <smith/csrc/autograd/symbolic.h>
...
#include <smith/csrc/jit/serialization/import_export_constants.h>
#include <smith/csrc/jit/serialization/import_export_functions.h>
#include <smith/csrc/jit/serialization/import_export_helpers.h>
...
void writeArchiveAndTensors(
    ...
    caffe2::serialize::BlacksmithStreamWriter& out) {
...
void validateBlock(
    Block* b,
    onnx_smith::OperatorExportTypes operator_export_type) {
  ...
  throw std::runtime_error(                        \
    ...
    if (node->kind() == prim::PythonOp) {
      if (operator_export_type !=
          onnx_smith::OperatorExportTypes::ONNX_FALLTHROUGH) {
      ...
      bool is_aten_enabled = operator_export_type ==
              onnx_smith::OperatorExportTypes::ONNX_ATEN_FALLBACK ||
          operator_export_type == onnx_smith::OperatorExportTypes::ONNX_ATEN ||
...
class GraphEncoder {
 ...
 private:
  ...
  onnx_smith::OperatorExportTypes operator_export_type_;
  bool strip_doc_;
...
static onnx::TensorProto_DataType ATenTypeToOnnxType(at::ScalarType at_type) {
  switch (at_type) {
    ...
    case at::kFloat8_e4m3fn:
      return onnx_smith::TensorProto_DataType_FLOAT8E4M3FN;
    case at::kFloat8_e5m2:
      return onnx_smith::TensorProto_DataType_FLOAT8E5M2;
    case at::kFloat8_e4m3fnuz:
      return onnx_smith::TensorProto_DataType_FLOAT8E4M3FNUZ;
    case at::kFloat8_e5m2fnuz:
      return onnx_smith::TensorProto_DataType_FLOAT8E5M2FNUZ;
    default:
      SMITH_CHECK(
          false,
          "ScalarType ",
...
GraphEncoder::GraphEncoder(
  ...
  model_proto_->set_producer_name("blacksmith");
  SMITH_CHECK(
  ...
  model_proto_->set_producer_version(SMITH_VERSION);
...
void GraphEncoder::EncodeBlock(
    ...
    bool use_external_data_format,
    const std::string& onnx_file_path) {
  SMITH_INTERNAL_ASSERT(graph_proto != nullptr);
...
void GraphEncoder::AddInitializersIntoGraphProto(
    ...
    bool use_external_data_format,
    const std::string& onnx_file_path) {
  SMITH_INTERNAL_ASSERT(block->inputs().size() >= initializers.size());
  for (auto input : block->inputs()) {
...
void GraphEncoder::EncodeNode(
  ...
  if (operator_export_type_ == onnx_smith::OperatorExportTypes::ONNX) {
  ...
  const auto node_name_attribute_symbol =
      Symbol::attr(::smith::onnx::kOnnxNodeNameAttribute);
  if (add_node_names) {
  ...
  if (node->kind() == ::c10::onnx::Loop) {
    SMITH_INTERNAL_ASSERT(node->blocks().size() == 1);
  ...
  if (node->kind() == ::c10::onnx::If) {
    SMITH_INTERNAL_ASSERT(node->blocks().size() == 2);
...
void GraphEncoder::AddAttribute(
  ...
  SMITH_INTERNAL_ASSERT(name.is_attr());
...
void GraphEncoder::AddAttribute(
    onnx::FunctionProto* func_proto,
    const std::string& name) {
  SMITH_INTERNAL_ASSERT(nullptr != func_proto);
...
void GraphEncoder::EncodeTensor(
  ...
  if (defer_weight_export_ && external_ref) {
    ...
    SMITH_INTERNAL_ASSERT(external_ref.value() == tensor_proto->name());
    SMITH_INTERNAL_ASSERT(
    ...
    SMITH_INTERNAL_ASSERT(t.is_contiguous());
    ...
    if (use_external_data_format &&
      ...
      SMITH_INTERNAL_ASSERT(!onnx_file_path.empty());
      SMITH_INTERNAL_ASSERT(tensor_proto->has_name());
...
void check_onnx_proto(const std::string& proto_string) {
  ...
  try {
    ...
    SMITH_WARN(
        ...
        "please report an issue at https://github.com/blacksmith/blacksmith/issues. "
