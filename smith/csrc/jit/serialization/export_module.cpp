...
#include <smith/csrc/jit/backends/backend_debug_handler.h>
...
#include <smith/csrc/jit/mobile/method.h>
...
#include <smith/csrc/jit/serialization/callstack_debug_info_serialization.h>
...
#include <smith/csrc/jit/serialization/type_name_uniquer.h>
 ...
 *  "__smith__.base_models.sparse_nn.blacksmith_preproc_types.PreprocOutputType[
 *     NamedTuple, [
...
std::pair<IValue, IValue> getFunctionTuple(
  ...
  static const std::string smith_prefix("__smith__");
  static const std::string class_prefix("__smith__.smith.classes");
  ...
  for (const TypePtr& ty : mobile_code.types_) {
    ...
    if (t->kind() == TypeKind::DictType) {
      ...
      // "Dict[str,__smith__.dper3.core.blacksmith_schema_utils.IdScoreListFeatureTuple[NamedTuple,
      // [[lengths, Tensor],[values,
      // __smith__.dper3.core.blacksmith_schema_utils.IdScoreTuple[NamedTuple,
      ...
      continue;
    } else if (type_str.find(smith_prefix) == 0) {
      SMITH_CHECK(
          type_str.find(class_prefix) == 0,
          "__smith__ types other than custom c++ classes (__smith__.smith.classes)"
          "are not supported in lite interpreter. ",
          "Workaround: instead of using arbitrary class type (class Foo()), ",
          "define a blacksmith class (class Foo(smith.nn.Module)). The problematic type is: ",
          type_str);
    }
...
void getBackendDebugInfoMap(
  ...
  if (isLoweredModule(m)) {
    auto backend_debug_info =
        m.attr("__backend_debug_info").toCustomClass<BlacksmithBackendDebugInfo>();
...
SMITH_API void enableMobileInterfaceCallExport() {
...
void ScriptModuleSerializer::serialize(
  ...
  C10_LOG_API_USAGE_ONCE("smith.jit.save");
...
void ScriptModuleSerializer::writeArchive(
  ...
  SMITH_INTERNAL_ASSERT(tensor_names.size() == data_pickle.tensorData().size());
...
std::optional<std::string> type_printer(
    const c10::Type& type,
    smith::jit::TypeNameUniquer& type_name_uniquer) {
...
void ExportModule(
  ...
  if (!use_flatbuffer) {
    ...
    caffe2::serialize::BlacksmithStreamWriter writer(filename);
...
void ExportModule(
  ...
  if (use_flatbuffer) {
    ...
    caffe2::serialize::BlacksmithStreamWriter writer(writer_func);
    ScriptModuleSerializer serializer(writer);
    serializer.serialize(
        module, extra_files, bytecode_format, save_mobile_debug_info);
  }
