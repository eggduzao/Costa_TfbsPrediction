...
// Used in smith.package and SmithScript serialization to coordinate
// sharing of storages between models. Also used to create deterministic
// naming for storages.
class SMITH_API SerializationStorageContext {
 public:
...
// Used in smith.package and SmithScript deserialization to coordinate
// sharing of storages between models.
class SMITH_API DeserializationStorageContext {
 public:
  ...
  void addStorage(std::string name, c10::Storage storage) {
    SMITH_INTERNAL_ASSERT(!hasStorage(name));
