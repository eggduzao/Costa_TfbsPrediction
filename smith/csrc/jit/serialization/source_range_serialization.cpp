...
#include <smith/csrc/jit/serialization/source_range_serialization_impl.h>
...
// "Compact file is smaller but cannot be loaded by old smith binaries."
...
SourceRange SourceRangeDeserializer::deserialize(const c10::IValue& iv) {
  ...
  SMITH_INTERNAL_ASSERT(tup_elems.size() == 3);
...
std::vector<char> SourceRangePickler::pickle(
  ...
  SMITH_CHECK(table.empty(), "Expected 0 tensors to be written");
  return result;
}
...
SMITH_API void setShouldUseFormatWithStringTable(
