...
c10::StrongTypePtr customClassResolver(const c10::QualifiedName& qn) {
  ...
  if (c10::QualifiedName("__smith__").isPrefixOf(qn)) {
    type = smith::getCustomClass(qn.qualifiedName());
  } else {
    // This is a regular type, fall back to the default type parser
    smith::jit::ScriptTypeParser parser;
...
// This has to live here instead of the C++ API to mirror smith.save since the
...
std::vector<char> pickle_save(const at::IValue& ivalue) {
  ...
  caffe2::serialize::BlacksmithStreamWriter writer(
  ...
  SMITH_CHECK(
      ...
      "(see https://github.com/blacksmith/blacksmith/pull/30108)");
#endif
}
...
#ifndef C10_MOBILE
  caffe2::serialize::BlacksmithStreamReader reader(
      std::make_unique<smith::jit::StringViewReader>(data));
  return smith::jit::readArchiveAndTensors(
      "data",
      /*pickle_prefix=*/"",
      /*tensor_prefix=*/"",
      /*type_resolver=*/customClassResolver,
      /*obj_loader=*/smith::jit::ObjLoaderFunc,
      /*device=*/std::nullopt,
      reader);
#else
  SMITH_CHECK(
