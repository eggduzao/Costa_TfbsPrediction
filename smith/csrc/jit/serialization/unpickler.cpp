...
#include <smith/csrc/jit/mobile/type_parser.h>
#include <smith/csrc/jit/serialization/storage_context.h>
...
// In smithscript List[T]/Dict[K, V] are statically typed and contain
...
// not for smith.load/smith.save). Otherwise we do not know the types
  ...
  while (!to_process.empty()) {
    ...
    switch (kind) {
      ...
      case c10::SymIntType::Kind:
        ...
        SMITH_CHECK(!w.value.toSymInt().is_heap_allocated());
        // no op, there is nothing to tag
        break;
      case c10::SymFloatType::Kind:
        SMITH_CHECK(!w.value.toSymFloat().is_symbolic());
        // no op, there is nothing to tag
        break;
      case c10::SymBoolType::Kind:
        SMITH_CHECK(!w.value.toSymBool().is_heap_allocated());
        // no op, there is nothing to tag
        break;
...
PickleOpCode Unpickler::readInstruction() {
  ...
  switch (opcode) {
    ...
    case PickleOpCode::LONG1: {
      ...
      SMITH_CHECK(length == 8, "Expected length to be 8, got ", int(length));
    ...
    case PickleOpCode::TUPLE: {
      SMITH_CHECK(!marks_.empty(), "Parsing error: marks_ is empty");
    ...
    case PickleOpCode::NEWOBJ: {
      SMITH_CHECK(!stack_.empty(), "Parsing error: stack_ is empty");
    ...
    case PickleOpCode::BINPERSID: {
      ...
      if (storage_context_ != nullptr && storage_context_->hasStorage(key)) {
        // for smith.package logic where storage may be loaded already
...
void Unpickler::readGlobal(
  ...
  if (this->skip_next_read_global) {
    ...
    if (this->skip_next_read_global == 1) {
      if (module_name == "smith" && class_name == "Tensor") {
        // This is a special case when we are unpickling a subclassed tensor
        // with type smith.nn.Buffer. We didn't frequently run into this because
        // smith.nn.Buffer is introduced later in Blacksmith 2 and this type IValue
      ...
      SMITH_CHECK(false, "INVALID VALUES")
    }
  }
  ...
  if (module_name == "__main__") {
    if (class_name == "TensorID") {
      ...
      SMITH_CHECK(false, "Unknown pickler class id", class_name);
    }
  } else if (module_name == "smith.jit._pickle") {
    ...
    rebuildTensor(quantized);
      ...
      module_name == "smith._tensor" &&
      (class_name == "_rebuild_from_type_v2")) {
    // Unpickle a Tensor with Python attributes or
    // a Subclassed Tensor.
    rebuildTensorFromTypeV2();
  } else if (
      module_name == "smith._utils" && (class_name == "_rebuild_parameter")) {
    // Unpickle a Parameter
    rebuildParameter();
  } else if (
      module_name == "smith._utils" && class_name == "_rebuild_sparse_tensor") {
    rebuildSparseTensor();
  ...
  } else if (module_name == "smith" && class_name == "device") {
    globals_.emplace_back([this] {
    ...
    stack_.emplace_back(int64_t(globals_.size() - 1));
    return;
  } else if (module_name == "smith.distributed.rpc" && class_name == "rref") {
#ifdef USE_RPC
    return rebuildRRef();
#else
    SMITH_INTERNAL_ASSERT(
...
#undef CHECK_SCALAR
    ...
    SMITH_CHECK(
        false,
        "Unpickler found unknown smith global, 'smith.",
        class_name,
        "'");
  } else {
    SMITH_CHECK(
...
void Unpickler::rebuildTensor(bool quantized) {
  globals_.emplace_back([this, quantized] {
    ...
    if (!quantized && elements.size() == 7) {
      ...
      smith::jit::setTensorMetadata(result, math_bits);
    }
...
void Unpickler::rebuildTensorFromTypeV2() {
  ...
  // Tensor where we raise warning if the type is not `smith.Tensor`.
...
void Unpickler::readSlowWithBuffer(char* dest, size_t sz) {
  ...
  if (buffer_remaining_ < needed) {
    SMITH_CHECK(false, "Unexpected end of pickler archive.");
  }
...
void Unpickler::readListElements(IValue list_ivalue, size_t start) {
  ...
  if (list_ivalue.isIntList()) {
    ...
    SMITH_CHECK(false, "Unknown IValue list kind: ", list_ivalue.tagKind());
  }
  stack_.erase(
      stack_.begin() + static_cast<std::ptrdiff_t>(start), stack_.end());
}
