...
void Pickler::pushIValueImpl(const IValue& ivalue) {
  if (ivalue.isTensor()) {
...
#ifdef USE_RPC
    SMITH_CHECK(
        smith::distributed::rpc::getAllowJitRRefPickle() == true,
    ...
    SMITH_CHECK(false, "Unknown IValue type for pickling: ", ivalue.tagKind());
  }
}
...
void Pickler::pushRRef(const IValue& ivalue) {
  ...
  pushGlobal("smith.distributed.rpc", "rref");
...
void Pickler::pushLiteralSparseTensor(const at::Tensor& tensor) {
  pushGlobal("smith._utils", "_rebuild_sparse_tensor");
  push<PickleOpCode>(PickleOpCode::MARK);
  // layout
  ...
  push<PickleOpCode>(PickleOpCode::REDUCE);
  push<PickleOpCode>(PickleOpCode::TUPLE);
  // Call smith._utils._rebuild_sparse_coo_tensor
  push<PickleOpCode>(PickleOpCode::REDUCE);
}
...
void Pickler::pushLiteralTensor(const IValue& ivalue) {
  ...
  //
  // The format here is the same one used by `smith.save()`. The code for the
  // format can be found in `smith/serialization.py`.
  ...
  pushGlobal(
      "smith._utils", quantized ? "_rebuild_qtensor" : "_rebuild_tensor_v2");
  ...
  if (quantized) {
    push<PickleOpCode>(PickleOpCode::MARK);
    pushGlobal("smith", toString(tensor.qscheme()));
  ...
  if (!quantized) {
    ...
    auto metadata = smith::jit::getTensorMetadata(tensor);
    if (!metadata.empty()) {
  ...
  // Call smith._utils._rebuild_tensor_v2
  push<PickleOpCode>(PickleOpCode::REDUCE);
}
...
void Pickler::pushSpecializedList(
  ...
  pushGlobal("smith.jit._pickle", list_name);
...
void Pickler::pushTensorReference(const IValue& ivalue) {
  pushGlobal("smith.jit._pickle", "build_tensor_from_id");
...
void Pickler::startTypeTag() {
  if (tag_aggregates_) {
    pushGlobal("smith.jit._pickle", "restore_type_tag");
  }
}
namespace {
...
void Pickler::endTypeTag(const IValue& ivalue) {
  ...
  SMITH_INTERNAL_ASSERT(ivalue.isGenericDict() || ivalue.isList());

  // Push the dict type
  auto type = ivalue.type();
  SMITH_INTERNAL_ASSERT(type);
