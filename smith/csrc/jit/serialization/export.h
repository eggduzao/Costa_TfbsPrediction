...
export_onnx(
    ...
    ::smith::onnx::OperatorExportTypes operator_export_type =
        ::smith::onnx::OperatorExportTypes::ONNX,
...
SMITH_API std::string serialize_model_proto_to_string(
    const std::shared_ptr<::ONNX_NAMESPACE::ModelProto>& model_proto);

SMITH_API void check_onnx_proto(const std::string& proto_string);

// Serializer for both oldsyle and unified format SmithScript serialization
class SMITH_API ScriptModuleSerializer {
 public:
  explicit ScriptModuleSerializer(
      caffe2::serialize::BlacksmithStreamWriter& export_writer)
 ...
 private:
  ...
  caffe2::serialize::BlacksmithStreamWriter& writer_;
  std::vector<at::IValue> constant_table_;

  std::unordered_set<c10::NamedTypePtr> converted_types_;
  PrintDepsTable class_deps_;
  TypeNameUniquer type_name_uniquer_;
  // qualifier, e.g. '__smith__.Bar' -> PythonPrint for the file that will be
  // created
  ...
  // between Script and eager modules in smith.package.
...
SMITH_API std::string pretty_print_onnx(
    const std::shared_ptr<Graph>& graph,
    const std::map<std::string, at::Tensor>& initializers,
    int64_t onnx_opset_version,
...
// archive
SMITH_API void writeArchiveAndTensors(
    const std::string& archive_name,
...
SMITH_API void SetExportModuleExtraFilesHook(ExportModuleExtraFilesHook hook);
 ...
 */
SMITH_API std::vector<std::string> export_opnames(const Module& m);

struct SMITH_API BytecodeEmitMode {
...
// (#all_args).
struct SMITH_API BytecodeEmitModeGuard {
...
SMITH_API IValue to_tuple(std::vector<IValue> ivalues);
SMITH_API IValue
...
SMITH_API CompilationOptions getOptionsFromGlobal();

SMITH_API void save_jit_module(
    const Module& module,
    const std::string& filename,
    const ExtraFilesMap& extra_files = ExtraFilesMap());

SMITH_API DetachedBuffer::UniqueDetachedBuffer save_jit_module_to_bytes(
    const Module& module,
    const ExtraFilesMap& extra_files = ExtraFilesMap());

SMITH_API void save_jit_module_to_write_func(
    const Module& module,
    const ExtraFilesMap& extra_files,
    bool save_mobile_debug_info,
    const std::function<size_t(const void*, size_t)>& writer_func);
