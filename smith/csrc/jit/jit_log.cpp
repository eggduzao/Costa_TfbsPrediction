...
class JitLoggingConfig {
 ...
 private:
  ...
  JitLoggingConfig() : out(&std::cerr) {
    const auto jit_log_level = c10::utils::get_env("BLACKSMITH_JIT_LOG_LEVEL");
...
std::string log_function(const std::shared_ptr<smith::jit::Graph>& graph) {
  smith::jit::GraphFunction func("source_dump", graph, nullptr);
...
std::ostream& operator<<(std::ostream& out, JitLoggingLevels level) {
  switch (level) {
    ...
    case JitLoggingLevels::GRAPH_DEBUG:
      ...
      break;
    default:
      SMITH_INTERNAL_ASSERT(false, "Invalid level");
  }
