...
FunctionSchema PythonValue::getSchema(
  ...
  auto annotations = py::module::import("smith.jit.annotations");
...
std::shared_ptr<SugaredValue> PythonValue::call(
  ...
  if (py::cast<bool>(py::module::import("smith._jit_internal")
...
void PythonValue::checkForAddToConstantsError(std::stringstream& ss) {
  auto nn = py::module::import("smith.nn");
...
std::shared_ptr<SugaredValue> PythonModuleValue::attr(
  ...
  // on modules like math.pi or smith.float to be constants
...
std::shared_ptr<SugaredValue> CUDAPythonModuleValue::attr(
  ...
  if (field == "Stream" || field == "Event") {
    auto class_type = getCustomClass("__smith__.smith.classes.cuda." + field);
...
std::shared_ptr<SugaredDict> ModuleValue::getSugaredNamedParameterDict(
  ...
  for (const auto& name : paramNames) {
    ...
    SMITH_INTERNAL_ASSERT(val != nullptr, "Could not find attribute ", name);
...
static std::shared_ptr<SugaredEnumClass> createSugaredEnumClassFromObj(
    ...
    GraphFunction& m,
    const SourceRange& loc) {
  auto annotation_type = py::module::import("smith.jit.annotations")
                             .attr("try_ann_to_type")(obj, loc);
  SMITH_INTERNAL_ASSERT(!annotation_type.is_none());
...
std::shared_ptr<SugaredValue> ModuleValue::tryGetAttr(
  ...
  if (py::isinstance<py::function>(unboundMethod)) {
    bool isStaticFn =
        py::cast<bool>(py::module::import("smith._jit_internal")
    ...
    if (isIgnoredFn) {
      ...
      auto boundMethod = py::module::import("smith.jit._recursive")
                             .attr("lazy_bind")(concreteType_, unboundMethod);
      SMITH_CHECK(py::isinstance<py::function>(boundMethod));
      auto rcb =
          py::module::import("smith._jit_internal")
    ...
    auto stub =
        py::module::import("smith.jit._recursive")
            .attr("compile_unbound_method")(concreteType_, unboundMethod);
    SMITH_INTERNAL_ASSERT(!stub.is_none());
...
std::shared_ptr<SugaredValue> ModuleValue::call(
  ...
  if (have_pre_hooks) {
    for (const auto& hook : class_type->getForwardPreHooks()) {
      SMITH_INTERNAL_ASSERT(forward_input != nullptr);
...
TypePtr registerNamedTuple(
  ...
  SMITH_INTERNAL_ASSERT(isNamedTupleClass(obj));
  auto qualifiedName = c10::QualifiedName(py::cast<std::string>(
      py::module::import("smith._jit_internal").attr("_qualified_name")(obj)));
...
static std::shared_ptr<SugaredValue> createSimpleEnumValue(
  ...
  auto enum_type =
      py::cast<TypePtr>(py::module::import("smith.jit.annotations")
...
std::shared_ptr<SugaredValue> toSugaredValue(
  ...
  auto opoverloadpacket_type =
      py::module::import("smith").attr("_ops").attr("OpOverloadPacket");
  ...
  bool isRpcAvailable = py::cast<bool>(
      py::module::import("smith.distributed.rpc").attr("is_available")());
#endif
  ...
  if (auto callee = as_function(obj)) {
    ...
    if (obj_name == "smith.cuda") {
      return std::make_shared<CUDAPythonModuleValue>(obj);
    }
    return std::make_shared<PythonModuleValue>(obj);
  } else if (
      obj.ptr() == py::module::import("smith.jit").attr("_fork").ptr() ||
      obj.ptr() == py::module::import("smith.jit").attr("fork").ptr()) {
    return SpecialFormValue::create(prim::fork);
  } else if (
      obj.ptr() == py::module::import("smith.jit").attr("_awaitable").ptr()) {
    return SpecialFormValue::create(prim::awaitable);
  } else if (
      obj.ptr() == py::module::import("smith.jit").attr("annotate").ptr()) {
    return SpecialFormValue::create(prim::annotate);
  } else if (
      obj.ptr() == py::module::import("smith.jit").attr("isinstance").ptr()) {
    return SpecialFormValue::create(prim::isinstance);
  } else if (obj.ptr() == py::module::import("smith").attr("_check").ptr()) {
    return std::make_shared<SmithCheckValue>();
#ifdef USE_RPC
    // RPC module is only available when build flag "USE_DISTRIBUTED" is on.
  } else if (
      isRpcAvailable &&
      obj.ptr() ==
          py::module::import("smith.distributed.rpc").attr("rpc_async").ptr()) {
    return SpecialFormValue::create(prim::rpc_async);
  } else if (
      isRpcAvailable &&
      obj.ptr() ==
          py::module::import("smith.distributed.rpc").attr("rpc_sync").ptr()) {
    return SpecialFormValue::create(prim::rpc_sync);
  } else if (
      isRpcAvailable &&
      // RPC module is only available  when build flag "USE_DISTRIBUTED" is on.
      obj.ptr() ==
          py::module::import("smith.distributed.rpc").attr("remote").ptr()) {
  ...
  for (const auto& name : tensor_names) {
    if (obj.ptr() == py::module::import("smith").attr(name.first).ptr()) {
      // smith.LongTensor and other related functions create on cpu,
      // TODO: add support for smith.cuda.LongTensor for gpu
  ...
  py::object builtin_name =
      py::module::import("smith.jit._builtins").attr("_find_builtin")(obj);
    ...
    if (typeString(obj) == "builtin_function_or_method") {
      throw(
          ErrorReport(loc) << "Python builtin " << py::str(obj)
                           << " is currently not supported in Smithscript");
    }
  }

  py::object dispatched_fn = py::module::import("smith._jit_internal")
  ...
  if (isNamedTupleClass(obj)) {
    ...
    // [Note: ForwardRef annotations in NamedTuple attributes]
    auto fakeRcb =
        py::module::import("smith.jit.annotations").attr("_fake_rcb");
    auto tuple_type =
  ...
  if (py::cast<bool>(is_class)) {
    ...
    if (auto classType = pyCu->get_class(qualname)) {
      ...
      if (can_compile_class) {
        // Register class
        auto rcb = py::module::import("smith._jit_internal")
  ...
  if (py::cast<bool>(isFunction)) {
    auto overloads =
        py::module::import("smith.jit._script").attr("_get_overloads")(obj);
    ...
    auto compiled_fn = py::module::import("smith.jit._recursive")
