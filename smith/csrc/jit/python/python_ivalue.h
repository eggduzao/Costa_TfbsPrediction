...
struct C10_EXPORT ConcretePyObjectHolder final : PyObjectHolder {
 public:
  ...
  InferredType tryToInferType() override {
    ...
    return smith::jit::tryToInferType(py_obj_);
  }

  IValue toIValue(const TypePtr& type, std::optional<int32_t> N = std::nullopt)
      override {
    pybind11::gil_scoped_acquire ag;
    return smith::jit::toIValue(py_obj_, type, N);
  }
      ...
      auto& extractorFn =
          storage
              .call_once_and_store_result([]() -> py::object {
                return py::module_::import("smith._jit_internal")
                    .attr("_extract_tensors");
              })
              .get_stored();
#else
      static py::object& extractorFn = *new py::object(
          py::module::import("smith._jit_internal").attr("_extract_tensors"));
#endif
