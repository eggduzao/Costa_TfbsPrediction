...
#include <smith/csrc/jit/api/object.h>
...
#include <smith/csrc/jit/mobile/quantization.h>
...
#include <smith/csrc/jit/python/module_python.h>
#include <smith/csrc/jit/python/python_ivalue.h>
#include <smith/csrc/jit/python/python_sugared_value.h>
...
#include <smith/csrc/jit/frontend/parser.h>
#include <smith/csrc/jit/frontend/tracer.h>
#include <smith/csrc/jit/ir/constants.h>
#include <smith/csrc/jit/ir/graph_utils.h>
...
#include <smith/csrc/jit/python/python_dict.h>
#include <smith/csrc/jit/python/python_list.h>
#include <smith/csrc/jit/python/python_tracer.h>
#include <smith/csrc/jit/runtime/graph_executor.h>
#include <smith/csrc/jit/runtime/instruction.h>
#include <smith/csrc/jit/runtime/interpreter.h>
#include <smith/csrc/jit/runtime/logging.h>
#include <smith/csrc/jit/serialization/import_source.h>
#include <smith/csrc/jit/serialization/pickle.h>
#include <smith/csrc/jit/serialization/python_print.h>
#include <smith/csrc/jit/testing/hooks_for_testing.h>

#include <smith/csrc/api/include/smith/ordered_dict.h>
...
#include <smith/csrc/jit/mobile/train/export_data.h>
#include <cstddef>
#include <memory>
#include <sstream>
#include <string>
...
struct PythonResolver : public Resolver {
  ...
  TypePtr resolveTypeFromObject(const py::object& obj, const SourceRange& loc) {
    ...
    auto qualifiedName = c10::QualifiedName(
        py::cast<std::string>(py::module::import("smith._jit_internal")
  ...
  TypePtr resolveType(const std::string& name, const SourceRange& loc)
    ...
    auto annotation_type =
        py::module::import("smith.jit.annotations")
...
static StrongFunctionPtr script_compile_overloaded_function(
  ...
  auto defined_functions = cu->define(
      ...
      nullptr,
      true);
  SMITH_INTERNAL_ASSERT(defined_functions.size() == 1);
...
static IValue pyIValueDeepcopy(const IValue& ivalue, const py::dict& memo) {
  if (!memo.contains(py::str("__smith_script_memo_table"))) {
    memo["__smith_script_memo_table"] =
        DeepCopyMemoTable{std::make_shared<IValue::HashIdentityIValueMap>()};
  }
  auto& ivalue_memo =
      *py::cast<DeepCopyMemoTable>(memo["__smith_script_memo_table"]).map;
...
static void pyCompilationUnitDefine(
  ...
  if (rcb && *rcb) {
    ...
    py::object py_default_rcb =
        py::module::import("smith._jit_internal")
...
static std::shared_ptr<char> copyStr(const std::string& bytes) {
  ...
  SMITH_INTERNAL_ASSERT(p, "Could not allocate memory for flatbuffer");
...
void initJitScriptBindings(PyObject* module) {
      ...
      py::class_<Object>(m, "ScriptObject")
                ...
                try {
                  if (self.has_property(name)) {
                    ...
                    if (!prop.setter_func.has_value()) {
                      SMITH_CHECK(false, "can't set attribute");
                    }
                    // wrap the Method into callable PyObject
                ...
                if (auto getstate_method = self.find_method("__getstate__")) {
                  ...
                  if (auto qualname = self.type()->name()) {
                    ...
                    auto self = Object(c10::ivalue::Object::create(
                        c10::StrongTypePtr(
                            std::shared_ptr<smith::jit::CompilationUnit>(),
                            class_type),
                        1));
          ...
          .def(py::pickle(
              [](const Object& self)
                ...
                if (auto getstate_method = self.find_method("__getstate__")) {
                  auto object_state = toPyObject((*getstate_method)(Stack{}));
                  SMITH_INTERNAL_ASSERT(self.type()->name());
  ...
  special_magic_methods.emplace(
      ...
      [](const Object& self,
        ...
        if (!method) {
          ...
          return py::str("<smith.ScriptObject object at " + ss.str() + ">");
        }
        return invokeScriptMethodFromPython(*method, args, kwargs);
      });
  ...
  // Used by smith.package to save ScriptModule objects in unified format.
  py::class_<ScriptModuleSerializer>(m, "ScriptModuleSerializer")
      .def(py::init<caffe2::serialize::BlacksmithStreamWriter&>())
  ...
  // Used by smith.package to coordinate sharing of storages between eager
  // and ScriptModules.
  py::class_<
  ...
  // smith.jit.ScriptModule is a subclass of this C++ object.
  ...
  py::class_<StrongFunctionPtr>(m, "ScriptFunction", py::dynamic_attr())
      .def(
          ...
          [](py::args args, const py::kwargs& kwargs) {
            ...
            if (py::module::import("smith")
                    .attr("compiler")
                    .attr("is_exporting")()
                    .cast<bool>()) {
              SMITH_INTERNAL_ASSERT(
                  py::hasattr(args[0], py::str("_smithdynamo_inline")),
                  "During PT2 exporting, we encountered SmithScripted function",
                  strongPtr.function_->name(),
                  "When tracing through it, we cannot find its _smithdynamo_inline attribute, ",
                  "which stores non scripted kcallable. ",
                  "Please file an issue to Blacksmith if you see this error.");

              // remove the function itself with args[1:]
              py::slice slice0(1, args.size(), 1);
              return args[0].attr("_smithdynamo_inline")(
                  *args[slice0], **kwargs);
            } else {
      ...
      .def(
          ...
          [](const StrongFunctionPtr& self,
             const std::string& filename,
             const ExtraFilesMap& _extra_files = ExtraFilesMap()) {
            Module module("__smith__.PlaceholderModule");
            // [issue 27343]
            // Modules have 'training' attributes by default, but due to
            // https://github.com/blacksmith/blacksmith/issues/27343, functions end
      ...
      .def(
          ...
          [](StrongFunctionPtr& self, bool ignore) {
            ...
            SMITH_INTERNAL_ASSERT(fn->isGraphFunction());
      ...
      .def(
          ...
          [](py::args args, const py::kwargs& kwargs) {
            ...
            if (py::module::import("smith")
                ...
                //    def forward(self, x):
                //        return x.cos() + x.sin()
                //  traced_module = smith.jit.trace(M(), example_inputs=inps)
  ...
  m.def(
      ...
      [](const std::string& qualname,
         ...
         const ResolutionCallback& rcb,
         const FunctionDefaults& defaults) {
        C10_LOG_API_USAGE_ONCE("smith.script.compile");
        const auto name = c10::QualifiedName(qualname);
        SMITH_INTERNAL_ASSERT(name.name() == def.name().name());
  ...
  m.def(
      ...
      [](const std::string& qualifiedName,
         ...
         const ResolutionCallback& rcb) {
        C10_LOG_API_USAGE_ONCE("smith.script.class");
        if (classDef.superclass().present()) {
          throw(
              ErrorReport(classDef.range())
              << "Smithscript does not support class inheritance.");
        }
        auto cu = get_python_cu();
  ...
  py::class_<smith::jit::ErrorReport::CallStack>(
  ...
  m.def(
      ...
      [](std::shared_ptr<CompilationUnit> cu,
         ...
         std::shared_ptr<smith::jit::DeserializationStorageContext>
             storage_context,
         py::object map_location,
  ...
  m.def("_export_operator_list", [](smith::jit::mobile::Module& sm) {
    return debugMakeSet(smith::jit::mobile::_export_operator_list(sm));
  });
  m.def(
  ...
  m.def(
      ...
      &smith::jit::enableMobileInterfaceCallExport);
  ...
  m.def("_export_opnames", [](Module& sm) {
    return debugMakeList(smith::jit::export_opnames(sm));
  });
  ...
  py::class_<ConcreteModuleType, std::shared_ptr<ConcreteModuleType>>(
      ...
      .def(
          ...
          [](std::shared_ptr<ConcreteModuleType> concreteType,
            ...
            SMITH_INTERNAL_ASSERT(methodDefs.size() == methodRcbs.size());
            SMITH_INTERNAL_ASSERT(properties.size() == propertyRcbs.size());
      ...
      .def(
          ...
          [](std::shared_ptr<ConcreteModuleType> concreteType,
            ...
            SMITH_INTERNAL_ASSERT(hookDefs.size() == hookRcbs.size());
            SMITH_INTERNAL_ASSERT(preHookDefs.size() == preHookRcbs.size());
  ...
  m.def("_load_mobile_module_from_file", [](const std::string& filename) {
    return smith::jit::load_mobile_module_from_file(filename);
  });
  m.def("_load_mobile_module_from_bytes", [](const std::string& bytes) {
    auto bytes_copy = copyStr(bytes);
    return smith::jit::parse_and_initialize_mobile_module(
        bytes_copy, bytes.size());
  });
  m.def("_load_jit_module_from_file", [](const std::string& filename) {
    ExtraFilesMap extra_files = ExtraFilesMap();
    return smith::jit::load_jit_module_from_file(filename, extra_files);
  });
  m.def("_load_jit_module_from_bytes", [](const std::string& bytes) {
    auto bytes_copy = copyStr(bytes);
    ExtraFilesMap extra_files = ExtraFilesMap();
    return smith::jit::parse_and_initialize_jit_module(
        bytes_copy, bytes.size(), extra_files);
  });
  m.def(
      "_save_mobile_module",
      [](const smith::jit::mobile::Module& module,
         const std::string& filename,
         const ExtraFilesMap& _extra_files = ExtraFilesMap()) {
        return smith::jit::save_mobile_module(module, filename, _extra_files);
      });
  m.def(
      "_save_jit_module",
      [](const smith::jit::Module& module,
         const std::string& filename,
         const ExtraFilesMap& _extra_files = ExtraFilesMap()) {
        return smith::jit::save_jit_module(module, filename, _extra_files);
      });
  m.def(
      ...
      [](const smith::jit::mobile::Module& module,
        ...
        auto detached_buffer =
            smith::jit::save_mobile_module_to_bytes(module, _extra_files);
        return py::bytes(
  ...
  m.def(
      ...
      [](const smith::jit::Module& module,
         const ExtraFilesMap& _extra_files = ExtraFilesMap()) {
        auto detached_buffer =
            smith::jit::save_jit_module_to_bytes(module, _extra_files);
        return py::bytes(
            reinterpret_cast<char*>(detached_buffer->data()),
            detached_buffer->size());
      });
  m.def("_get_module_info_from_flatbuffer", [](std::string flatbuffer_content) {
    ...
    mobile::ModuleInfo minfo =
        smith::jit::get_module_info_from_flatbuffer(&flatbuffer_content[0]);
  ...
  m.def("_pickle_save", [](const IValue& v) {
    auto bytes = smith::jit::pickle_save(v);
  ...
  m.def("_pickle_load_obj", [](const py::bytes& bytes) {
    ...
    return smith::jit::pickle_load_obj(buffer);
  });
