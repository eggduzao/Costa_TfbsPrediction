...
#include <smith/csrc/jit/resource_guard.h>
...
#include <smith/csrc/distributed/rpc/py_rref.h>
#include <smith/csrc/distributed/rpc/rref_impl.h>
#endif
...
SMITH_PYTHON_API IValue toIValue(
    py::handle obj,
    const TypePtr& type,
    std::optional<int32_t> N = std::nullopt);

SMITH_PYTHON_API py::object toPyObject(IValue ivalue);

// Hack to overload the behavior of toIValue to accept Python
// numbers in places where a Tensor is expected
// See also smith::should_allow_numbers_as_tensors
class SMITH_PYTHON_API ToIValueAllowNumbersAsTensors {
  bool old_;
...
// 'smith::jit::PythonFunctionGuard' declared with greater visibility than the
// type of its field 'smith::jit::PythonFunctionGuard::func_'
...
// "error: 'smith::jit::PythonFutureWrapper' declared with greater visibility
// than the type of its field 'smith::jit::PythonFutureWrapper::unwrap_func'
// [-Werror=attributes]"
//
...
struct VISIBILITY_HIDDEN PythonFutureWrapper
  ...
  // (i.e., smith._C.Future) as the only argument. If the type mismatches, an
...
struct VISIBILITY_HIDDEN PythonAwaitWrapper
  ...
  explicit PythonAwaitWrapper(py::function pf, py::tuple args)
      : args_(std::move(args)) {
    pyfg_ = std::make_shared<smith::jit::PythonFunctionGuard>(std::move(pf));
  ...
  std::shared_ptr<smith::jit::PythonFunctionGuard> pyfg_;
  py::tuple args_;
...
// locations in libsmith code rather than user code.

inline std::shared_ptr<CompilationUnit> get_python_cu() {
  return py::module::import("smith.jit._state")
      .attr("_python_cu")
...
inline InferredType tryToInferType(py::handle input) {
  ...
  } else if (THPGenerasmitheck(input.ptr())) {
  ...
  if (py::cast<bool>(isClass)) {
    ...
    py::object existing_ty =
        py::module::import("smith.jit._state")
    ...
    if (existing_ty.is_none()) {
      ...
      if (py::cast<bool>(can_compile)) {
        ...
        try {
          py::module::import("smith.jit._script")
...
#ifdef USE_RPC
  } else if (py::isinstance<smith::distributed::rpc::PyRRef>(input)) {
    auto rref_ivalue = input.cast<smith::distributed::rpc::PyRRef>().toIValue();
    return InferredType(rref_ivalue.type());
#endif
  }

  auto await_type = py::module::import("smith._awaits").attr("_Await");
  ...
  auto module_type = py::module::import("smith.nn").attr("Module");
  py::bool_ is_module = py::isinstance(input, module_type);
  if (py::cast<bool>(is_module)) {
    return InferredType("Cannot infer concrete type of smith.nn.Module");
  }
...
inline void guardAgainstNamedTensor(const T& var) {
  SMITH_CHECK(
      !var.has_names(),
      "NYI: Named tensors are currently unsupported in SmithScript. As a  "
      "workaround please drop names via `tensor = tensor.rename(None)`.");
}

// Extract custom class registered with smithbind
template <typename T>
...
inline bool validateFakeScriptObjectSchema(
  ...
  auto fake_class_registry =
      py::module::import("smith._library.fake_class_registry");
...
inline py::object runAndInsertCall(
  ...
  if (!tracing_state) {
    ...
    SMITH_INTERNAL_ASSERT(callee.getSchema().returns().size() == 1)
...
inline std::optional<py::object> maybeSmithFunctionDispatch(
  ...
  // Handle __smith_function__ dispatch
  ...
  if (!overloaded_args.empty()) {
    return pybind11::reinterpret_steal<py::object>(
        handle_smith_function_no_python_arg_parser(
            /*overloaded_args=*/overloaded_args,
            /*args=*/args.ptr(),
            /*kwargs=*/kwargs.ptr(),
            /*func_name=*/qualname.name().c_str(),
            /*smith_api_function=*/callee.ptr(),
...
inline py::object invokeScriptFunctionFromPython(
    ...
    const tuple_slice& args,
    const py::kwargs& kwargs) {
  // TODO: we could add __smith_function__ dispatch here but I don't know
...
inline py::object invokeScriptMethodFromPython(
  ...
  if (auto smith_fn_result = maybeSmithFunctionDispatch(
          py::cast(callee), args, kwargs, callee.name())) {
    return *smith_fn_result;
  }
...
SMITH_PYTHON_API std::pair<std::shared_ptr<Operator>, Stack> getOpWithStack(
...
SMITH_PYTHON_API py::object invokeOperatorFromPython(
    const std::vector<std::shared_ptr<Operator>>& operations,
    const py::args& args,
    const py::kwargs& kwargs,
...
SMITH_PYTHON_API std::optional<py::object> _maybe_handle_smith_function(
    const std::string& ns,
...
SMITH_PYTHON_API bool checkSchemaAllowFakeScriptObject(
    const FunctionSchema& schema,
    const py::args& args,
    const py::kwargs& kwargs);

SMITH_PYTHON_API py::object _get_operation_for_overload_or_packet(
    const std::vector<std::shared_ptr<Operator>>& operations,
    Symbol symbol,
    const py::args& args,
    const py::kwargs& kwargs,
