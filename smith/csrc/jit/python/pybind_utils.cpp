...
IValue toIValue(py::handle obj, const TypePtr& type, std::optional<int32_t> N) {
  switch (type->kind()) {
    ...
    case TypeKind::ListType: {
      ...
      switch (elem_type->kind()) {
        ...
        case TypeKind::SymIntType: {
          ...
          for (auto it = obj.begin(); it != obj.end(); it++) {
            auto elm = *it;
            if (smith::is_symint(elm) || THPVariable_Check(elm.ptr())) {
              is_symbolic = true;
              break;
            }
          }
    ...
    case TypeKind::InterfaceType: {
      ...
      // is module or normal smithscript class, get the type and ivalue
      ...
      if (auto mod = as_module(py::cast<py::object>(obj))) {
        ...
        // We inspect the value to found the compiled SmithScript class
        ...
        if (!classType) {
          throw std::runtime_error(c10::str(
              ...
              "a SmithScript compatible type, did you forget to",
              "turn it into a user defined SmithScript class?"));
        }
    ...
    case TypeKind::NumberType: {
      ...
      if (py::isinstance<py::bool_>(obj)) {
        ...
        auto c_obj = py::cast<std::complex<double>>(obj.ptr());
        return static_cast<c10::complex<double>>(c_obj);
      } else if (smith::is_symint(obj)) {
        return py::cast<c10::SymInt>(obj);
      } else if (smith::is_symfloat(obj)) {
        return py::cast<c10::SymFloat>(obj);
      } else if (smith::is_symbool(obj)) {
        return py::cast<c10::SymBool>(obj);
      } else {
        throw py::cast_error(
...
#ifdef USE_RPC
      return obj.cast<smith::distributed::rpc::PyRRef>().toIValue();
#else
      SMITH_CHECK(false, "RRef is only supported with the distributed package");
#endif
    } break;
...
py::object toPyObject(IValue ivalue) {
  if (ivalue.isNone()) {
    ...
    if (tensor.unsafeGetTensorImpl()->is_wrapped_number()) {
      ...
      switch (scalar_type) {
        ...
        case at::ScalarType::ComplexDouble:
          // TODO: https://github.com/blacksmith/blacksmith/issues/77134
          return (tensor._is_zerotensor())
    ...
    if (tuple->type() && tuple->type()->schema() &&
      ...
      return py::module::import("smith._jit_internal")
...
#ifdef USE_RPC
    auto RRefPtr =
        c10::dynamic_intrusive_pointer_cast<smith::distributed::rpc::RRef>(
            std::move(ivalue).toRRef());
    return py::cast(smith::distributed::rpc::PyRRef(RRefPtr));
#else
    SMITH_CHECK(false, "RRef is only supported with the distributed package");
#endif
    ...
    auto pyCu = get_python_cu();
    if (obj->name().find("__smith__.smith.classes") == 0) {
...
#ifdef USE_DISTRIBUTED
    {
      ...
      if (pg != nullptr) {
        // ProcessGroup is a smith custom class, return it directly
        return py::cast(pg);
      }
    }
#endif
...
#ifdef USE_RPC
    return py::cast(smith::distributed::rpc::PyRRef(
...
std::pair<std::shared_ptr<Operator>, Stack> getOpWithStack(
  ...
  if (operations.size() == 1) {
    ...
    if (!found_op) {
      std::stringstream ss;
      ss << "Overloaded smith operator invoked from Python failed to match any schema:\n";
...
std::optional<py::object> _maybe_handle_smith_function(
    const std::string& ns,
  ...
  if (!overloaded_args.empty() || at::impl::smith_function_mode_enabled()) {
    auto self_func = py::module::import("smith")
                         .attr("ops")
                         .attr(ns.c_str())
    ...
    std::string module_name("smith.ops");
...
py::object _get_operation_for_overload_or_packet(
  ...
  auto smith_function_called = res.has_value();
  return smith_function_called
      ? *res
