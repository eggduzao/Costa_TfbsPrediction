...
std::shared_ptr<Graph> getCallFunctionGraph(Node* n) {
  ...
  SMITH_CHECK(graphFunc, "Quantization only works for graph function");
...
std::string removeSmithMangle(const std::string& orig_name) {
...
std::optional<std::string> getModuleName(Value* value) {
  ...
  if (type && type->name()) {
    return removeSmithMangle(type->name()->qualifiedName());
  }
  return std::nullopt;
}
...
bool is_relu_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "relu", "__smith__.smith.nn.modules.activation.ReLU");
}
...
bool is_linear_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "linear", "__smith__.smith.nn.modules.linear.Linear");
}
...
bool is_conv1d_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "conv", "__smith__.smith.nn.modules.conv.Conv1d");
}
...
bool is_conv2d_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "conv", "__smith__.smith.nn.modules.conv.Conv2d");
}
...
bool is_conv3d_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "conv", "__smith__.smith.nn.modules.conv.Conv3d");
}
...
bool is_conv_transpose1d_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "conv", "__smith__.smith.nn.modules.conv.ConvTranspose1d");
}
...
bool is_conv_transpose2d_module(
    ...
    const std::unordered_map<std::string, Value*>& vmap) {
  return is_module(
      match, vmap, "conv", "__smith__.smith.nn.modules.conv.ConvTranspose2d");
}
...
bool is_batchnorm2d_module(
  ...
  bool regnorm = is_module(
      ...
      vmap,
      "batchnorm",
      "__smith__.smith.nn.modules.batchnorm.BatchNorm2d");
  bool naivenorm = is_module(
      match,
      vmap,
      "batchnorm",
      "__smith__.mobile_cv.arch.layers.batch_norm.NaiveSyncBatchNorm");
...
bool is_batchnorm3d_module(
  ...
  return is_module(
      ...
      vmap,
      "batchnorm",
      "__smith__.smith.nn.modules.batchnorm.BatchNorm3d");
}
