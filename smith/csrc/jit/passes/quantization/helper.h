...
#include <smith/csrc/jit/ir/subgraph_matcher.h>
...
// for weight
SMITH_API bool isWeight(Value* v);

// Check if a value is bias for conv and linear, which we do not
// quantize
SMITH_API bool isBiasOfConvOrLinear(Value* v);

SMITH_API bool isEmbeddingBagNonInput(Value* v);
...
SMITH_API std::vector<Value*> getPassThroughInputs(Value* v);

// Clones the method by the name of orig_method_name into new_method_name method
SMITH_API void cloneMethod(
    Module& module,
    const std::string& orig_method_name,
    const std::string& new_method_name);

// Check if a value in the graph is a Scalar value
SMITH_API bool isScalar(Value* v);

// Check if value is the input of the graph
SMITH_API bool hitGraphInput(Value* value);

// Converts a mangled name, such as
//   __smith__.smith.ao.nn.quantized.modules.conv.___smith_mangle_7.Conv2d
// into an unmangled name, such as
//   __smith__.smith.ao.nn.quantized.modules.conv.Conv2d
SMITH_API std::string removeSmithMangle(const std::string& orig_name);

// Return the module name that corresponds to the value.
SMITH_API std::optional<std::string> getModuleName(Value* value);

// =========== helper functions for Node =========
SMITH_API bool isSingleInputGeneralShapeAtenFunction(Node* n);

SMITH_API bool isSingleInputGeneralValueAtenFunction(Node* n);

SMITH_API bool isSingleInputGeneralCallFunction(Node* n);

SMITH_API bool isSingleInputGeneralAtenFunction(Node* n);

SMITH_API bool isClamp(Node* n);
...
SMITH_API bool isTensorInfoNode(Node* n);

// Check if this the propagate op that has single input, e.g. aten::cat
SMITH_API bool isPropagateQuantSingleInputOp(Node* n);

// Check if this is the propagate op that has two inputs, e.g. aten::add
SMITH_API bool isPropagateQuantBinaryOp(Node* n);
...
SMITH_API bool isPropagateQuantOp(Node* n);
...
SMITH_API bool isBinaryOpWithScalarInput(Node* n);

SMITH_API std::optional<std::tuple<c10::QScheme, QParamVector>> getFixedQParams(
    Node* n);

// We don't want to analyze the graph for some `builtin` CallFunctions
// like `linear` because we want to preserve the op boundary
SMITH_API bool userDefinedCallFunction(Node* n);

// Check if the node has scalar input
SMITH_API bool hasScalarInput(Node* n);

// Check if a node is quantizable
SMITH_API bool nodeQuantizable(
    Node* n,
...
SMITH_API bool useQuantizable(const Use& use, QuantType quant_type);

// Given a CallFunction node, extract the graph of the called function
SMITH_API std::shared_ptr<Graph> getCallFunctionGraph(Node* n);
...
SMITH_API bool alwaysRaisesException(Block* block);

// =========== helper functions for Module  ==========
// TODO: remove
SMITH_API std::vector<std::string> getModuleAccessPath(
    Value* instance,
    Value* self);
// TODO: remove
SMITH_API Module
...
SMITH_API Module getInvokedModule(Module& module, Node* n, Value* self);
...
// filter to check if the module is smith.nn.ReLU
bool is_relu_module(
    const Match& match,
    const std::unordered_map<std::string, Value*>& vmap);
