...
#include <smith/csrc/jit/passes/prepack_folding.h>
...
void insertPrePackedBatchNormOp(std::shared_ptr<Graph>& graph) {
  ...
  std::string prepacked_ops_pattern = R"(
    graph(%input, %weight, %bias, %mean, %var, %training, %momentum, %eps, %cudnn_enable):
        %op_context : __smith__.smith.classes.vulkan.BatchNormPackedContext = vulkan_prepack::create_batchnorm_context(
...
void insertPrePackedLayernormOp(std::shared_ptr<Graph>& graph) {
  ...
  std::string prepacked_ops_pattern = R"(
    graph(%input, %normalized_shape, %weight, %bias, %eps, %cudnn_enable):
        %op_context : __smith__.smith.classes.vulkan.LayernormPackedContext = vulkan_prepack::create_layernorm_context(
            %weight, %bias, %eps)
...
void rewriteQuantizedOps(std::shared_ptr<Graph>& graph) {
  ...
  smith::jit::SubgraphRewriter quantized_add_rewriter;
  ...
  smith::jit::SubgraphRewriter quantized_mul_rewriter;
  ...
  std::string vk_quantized_conv2d_pattern = R"(
    graph(%a_quant, %packed_params, %r_scale, %r_zero_point):
      %output_min_max : None = prim::Constant()
      %vk_packed_params : __smith__.smith.classes.vulkan.Conv2dPackedContext = vulkan_quantized_prepack::convert_qconv2d_context(
  ...
  smith::jit::SubgraphRewriter quantized_conv2d_rewriter;
  ...
  std::string vk_quantized_conv_transpose2d_pattern = R"(
    graph(%a_quant, %packed_params, %r_scale, %r_zero_point):
      %output_min_max : None = prim::Constant()
      %vk_packed_params : __smith__.smith.classes.vulkan.Conv2dPackedContext = vulkan_quantized_prepack::convert_qtconv2d_context(
        %packed_params, %output_min_max, %output_min_max)
      %res = vulkan_prepack::run_qconv2d_context(
        %a_quant, %r_scale, %r_zero_point, %vk_packed_params)
      return (%res) )";

  smith::jit::SubgraphRewriter quantized_conv_transpose2d_rewriter;
  ...
  smith::jit::SubgraphRewriter quantized_conv2d_relu_rewriter;
  ...
  std::string vk_quantized_linear_pattern = R"(
    graph(%a_quant, %packed_params, %r_scale, %r_zero_point):
      %vk_packed_params : __smith__.smith.classes.vulkan.LinearPackedContext = vulkan_quantized_prepack::convert_linear_context(
        %packed_params)
      %res = vulkan_prepack::run_qlinear_context(
        %a_quant, %r_scale, %r_zero_point, %vk_packed_params)
      return (%res) )";

  smith::jit::SubgraphRewriter quantized_linear_rewriter;
...
void fuseHardtanhWithPackedOps(std::shared_ptr<Graph>& graph) {
  ...
  std::string conv2d_prepack_run_hardtanh_fused = R"(
    graph(%input, %weight, %bias, %stride:int[], %padding:int[],
        ...
        %packed_weight_bias : __smith__.smith.classes.vulkan.Conv2dPackedContext = vulkan_prepack::create_conv2d_context(
            %weight, %bias, %stride, %padding, %dilation, %groups,
  ...
  rewriter.runOnGraph(graph, smith::jit::graph_rewrite_helper::isClampFusable);
}
