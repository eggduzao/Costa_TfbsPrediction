...
struct ValueMapper {
  ...
  void copyAliasing(Node* merged_node, AliasDb& db) {
    ...
    for (Value* v : new_outputs) {
      ...
      SMITH_INTERNAL_ASSERT(maybe_last_use);
      ...
      SMITH_INTERNAL_ASSERT(i != last_uses_.size());
...
std::vector<std::set<Value*, topo_cmp_value>> buildAliasedSets(
  ...
  SMITH_INTERNAL_ASSERT(outputs.size() > 1);
...
static void collectNestedUses(
  ...
  if (input_node->kind() == prim::If) {
    ...
    SMITH_INTERNAL_ASSERT(false, input_node, " kind not handled yet");
  }
