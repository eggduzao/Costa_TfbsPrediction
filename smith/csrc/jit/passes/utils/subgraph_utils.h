...
namespace smith::jit::SubgraphUtils {
...
SMITH_API Node* createSingletonSubgraph(Node* n, Symbol subgraphKind);
...
SMITH_API Node* createSingletonSubgraphAndUpdateAliasing(
    Node* to_merge,
    Symbol subgraphKind,
    AliasDb& db);
...
SMITH_API void mergeNodeIntoSubgraph(
    Node* toMerge,
    Node* subgraphNode,
...
// outputs
SMITH_API void mergeNodeIntoSubgraphAndUpdateAliasing(
    Node* to_merge,
    Node* subgraphNode,
    AliasDb& db);

SMITH_API std::vector<Node*> unmergeAliasedOutputs(
    Node* subgraphNode,
    AliasDb& db);

// Move nodes from a subgraph node to the outer graph.
// `subgraphNode` is destroyed.
SMITH_API void unmergeSubgraph(Node* subgraphNode);
...
SMITH_API void unmergeNode(Node* node_to_unmerge, Node* subgraphNode);

SMITH_API bool unmergeOutputsAlisingInputs(Node* subgraphNode);

SMITH_API bool unmergeAliasedOutputs(Node* subgraphNode);

// Convenience function
std::shared_ptr<Graph> getSubgraph(Node* n);

SMITH_API std::string generateNameForGraph(
