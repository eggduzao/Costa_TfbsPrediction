...
// Utility functions for Blacksmith to ONNX conversion.
...
SMITH_API void buildParamsMapFromValueToParamsMap(
    const ValueToParamPairMap& valsToParamsMap,
    ParamMap& paramsDict);
SMITH_API ValueToParamPairMap
buildValueToParamsMap(Block* b, const ParamMap& paramsDict);
SMITH_API void eraseUnusedValuesFromMap(ValueToParamPairMap& valsToParamsMap);
SMITH_API void eraseUnusedBlockInputs(Block* b);

SMITH_API Node* addNodeToBlock(
    Block* block,
    Symbol kind,
    ArrayRef<Value*> inputs);

SMITH_API Value* addInputToBlock(Block* block);

SMITH_API std::optional<at::ScalarType> ONNXTypeToATenType(int32_t onnx_type);

// Use int return type as no sable way exists to forward declare protobuf enum
SMITH_API int ATenTypeToOnnxType(at::ScalarType at_type);

SMITH_API void ONNXLintGraph(const std::shared_ptr<Graph>& graph);
