...
Node* InsertCastForCond(
  ...
  // NOTE: The cast is required because operators like Blacksmith Greater/Less
  //       return tensor in type smith.uint8. However the type for condition
...
bool IsErasableSequence(const Node* loop_node, size_t i) {
  SMITH_INTERNAL_ASSERT(loop_node->blocks().size() == 1);
...
Node* ONNXOptionalNode(const OptionalTypePtr& opt_type, Graph* g) {
  SMITH_INTERNAL_ASSERT(opt_type);
...
static std::vector<Value*> FixupONNXLoopNode(Node* node, int opset_version) {
  ...
  SMITH_INTERNAL_ASSERT(output_size == new_outputs.size());
  return new_outputs;
}
...
//   test/onnx/test_blacksmith_onnx_onnxruntime.py:614:20
//     block0():
//       %y.5 : Tensor = aten::add(%y.1, %3, %6) #
//       test/onnx/test_blacksmith_onnx_onnxruntime.py:615:28
//       -> (%2, %7, %y.5)
//     block1():
//       -> (%1, %y.1, %7)
//   ...
...
static void ONNXMergeIfBlockOutputShapes(Node* node) {
  SMITH_INTERNAL_ASSERT(node->kind() == ::c10::onnx::If);
