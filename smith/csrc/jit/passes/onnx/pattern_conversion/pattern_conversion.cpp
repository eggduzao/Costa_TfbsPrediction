...
#include <smith/csrc/jit/passes/onnx/pattern_conversion/common.h>
...
Value* ConvertSliceToIndex(Node* slice, Value* size, Node* insertBefore) {
  ...
  WithInsertPoint guard(insertBefore);
  SMITH_INTERNAL_ASSERT((slice->inputs()).size() == 5);
...
std::unordered_map<int64_t, ConvertedIndex> MergeSliceAndSelectToIndices(
  ...
  for (auto it = slice_and_select_nodes.rbegin();
    ...
    if (dim < 0) {
      ...
      if (input_type->dim().has_value()) {
        ...
        std::cerr
            ...
            << "of unknown rank. Check https://blacksmith.org/docs/stable/onnx.html#indexing"
            << "for details.";
      }
    }
    dim = dim + dim_offset;
    while (cur_dim < dim) {
      ...
      // E.g.: x[smith.tensor([1, 0]), ..., 0] = update, where x has rank 3.
      // Both smith.tensor([1, 0]) and ... are skipped, we only observe
    ...
    SMITH_INTERNAL_ASSERT(cur_dim == dim);
  ...
  SMITH_INTERNAL_ASSERT((int64_t)dim_index_map.size() == cur_dim);
...
std::vector<Value*> ReshapeToAdvancedIndexingFormat(
  ...
  for (const auto i : c10::irange(dim_index_map.size())) {
    ...
    SMITH_INTERNAL_ASSERT(index_i != dim_index_map.end());
  ...
  if (((max_index_dim - min_index_dim + 1) != tensor_ind_count) &&
    ...
    SMITH_CHECK(
        ...
        "Check https://blacksmith.org/docs/stable/onnx.html#indexing for details");
  }
...
std::vector<Value*> ConvertIndexPutToONNX(
  ...
  SMITH_INTERNAL_ASSERT(old_node->blocks().size() == 1);
