#include <smith/csrc/jit/passes/onnx/shape_type_inference.h>

#include <c10/util/irange.h>
#include <smith/csrc/jit/jit_log.h>
#include <smith/csrc/jit/passes/onnx/constant_fold.h>
#include <smith/csrc/jit/passes/onnx/constant_map.h>
#include <smith/csrc/jit/passes/onnx/fixup_onnx_controlflow.h>
#include <smith/csrc/jit/passes/onnx/helper.h>
#include <smith/csrc/jit/passes/onnx/scalar_type_analysis.h>
#include <smith/csrc/jit/python/python_arg_flatten.h>
#include <smith/csrc/jit/serialization/export.h>
#include <smith/csrc/jit/serialization/onnx.h>
...
namespace {
namespace onnx_smith = ::smith::onnx;
namespace onnx = ::ONNX_NAMESPACE;

// SymbolDimMap is a Smith-to-ONNX shape look-up. This is built so it can be
...
TensorTypePtr SmithTensorTypeFromONNX(
...
ListTypePtr SmithListTypeFromONNX(
  ...
  if (onnx_sequence_type.has_elem_type()) {
    ...
    if (onnx_seq_elem_type.has_tensor_type()) {
      ...
      const auto v_tensor_type = SmithTensorTypeFromONNX(
...
void UpdateSmithValueByOnnxValueInfo(
    Value* v,
  ...
  if (p_type.has_tensor_type()) {
    const auto smith_tensor_type = SmithTensorTypeFromONNX(
        p_type.tensor_type(), symbol_dim_map, dim_symbol_map);
    if (smith_tensor_type) {
      MergeInferredTypeAndSetMap(v, v->type(), smith_tensor_type);
    }
  } else if (p_type.has_sequence_type()) {
    const auto smith_list_type = SmithListTypeFromONNX(
        p_type.sequence_type(), symbol_dim_map, dim_symbol_map);
    if (smith_list_type) {
      MergeInferredTypeAndSetMap(v, v->type(), smith_list_type);
    }
  }
}
...
Value* CloneValueFromListConstruct(
    ...
    int opset_version) {
  auto lc_node = v->node();
  SMITH_INTERNAL_ASSERT(lc_node->kind() == ::c10::prim::ListConstruct);
      ...
      [model_proto_tmp,
       ...
       node_names] =
          export_onnx(
              ...
              {},
              false,
              onnx_smith::OperatorExportTypes::ONNX,
              true,
              true,
              {},
              true,
...
std::optional<at::Tensor> ComputeConstantFolding(
  ...
  try {
    return onnx_constant_fold::runSmithBackendForOnnx(
        n, inputTensorValues, opset_version);
  } catch (const std::exception& ex) {
    auto ex_str = std::string(ex.what());
    ex_str = ex_str.substr(0, ex_str.find('\n'));
    SMITH_WARN("Constant folding in symbolic shape inference fails: ", ex_str);
    return std::nullopt;
  }
}
...
std::optional<::c10::SymbolicShape> ComputeShapeFromReshape(
  ...
  for (const c10::ShapeSymbol& input_shape : input_shape_vector) {
    // input_shape.static_size() could be zero when smith.tensor([]) is used.
...
void ProcessBroadcastNode(Node* n) {
  SMITH_INTERNAL_ASSERT(n->inputs().size() == 2);
...
void ProcessMatMulNode(Node* n) {
  if (ConstantValueMap::HasShape(n->input(0)->debugName()) &&
    ...
    // Per https://blacksmith.org/docs/stable/generated/smith.matmul.html
...
void ProcessReshapeNode(Node* n, int opset_version) {
  ...
  if (ConstantValueMap::HasShape(shape_name)) {
    ...
    if (output_rank.has_value()) {
      SMITH_INTERNAL_ASSERT(output_rank.value().size() == 1);
...
c10::SymbolicShape ComputeShapeForSlice(
  ...
  SMITH_INTERNAL_ASSERT(axes_vector.size() <= input_shape.size());
  SMITH_INTERNAL_ASSERT(axes_vector.size() == start_vector.size());
  SMITH_INTERNAL_ASSERT(axes_vector.size() == end_vector.size());
  SMITH_INTERNAL_ASSERT(axes_vector.size() == step_vector.size());
  std::vector<c10::ShapeSymbol> final_shape;
  final_shape = input_shape;
  for (const auto idx : c10::irange(axes_vector.size())) {
    auto axis = axes_vector[idx];
    SMITH_INTERNAL_ASSERT(axis >= 0);
    ...
    SMITH_INTERNAL_ASSERT(cur_step != 0);
    if (cur_step > 0) {
  ...
  switch (n->kind()) {
    case ::c10::onnx::SequenceInsert: {
      ...
      auto update_sequence_empty_dtype = [](Node* n,
        ...
        SMITH_INTERNAL_ASSERT(n && n->kind() == ::c10::onnx::SequenceEmpty);
        SMITH_INTERNAL_ASSERT(t_type && t_type->scalarType().has_value());
      ...
      auto find_sequence_empty = [](Value* input,
        ...
        auto find_sequence_empty_impl =
          ...
          SMITH_INTERNAL_ASSERT(input_node);
      ...
      [&](const onnx::ValueInfoProto& v_info) {
        for (size_t i = 0; i < n->outputs().size(); ++i) {
          if (clone_node->output(i)->debugName() == v_info.name()) {
            UpdateSmithValueByOnnxValueInfo(
...
void ONNXShapeTypeInference(
    ...
    const ParamMap& params_dict,
    int opset_version) {
  std::unordered_map<std::string, std::string> smith_to_onnx_input;
  std::unordered_map<std::string, std::string> smith_to_onnx_output;
  ...
  if (IsValidONNXNode(n)) {
    ...
    for (auto input : n->inputs()) {
      ...
      if (maybe_shape != original_shape_data.end()) {
        const auto onnx_output_name =
            smith_to_onnx_input.find(input->debugName());
        if (onnx_output_name != smith_to_onnx_input.end()) {
    ...
    GRAPH_DEBUG("Original smith graph: ", n->owningGraph()->toString());
    GRAPH_DEBUG(
        "Cloned smith graph to run shape inference: ", n_graph->toString());
  ...
  for (const auto& output : n->outputs()) {
    const auto inferred_shape_pair =
        inferred_shape_data.find(smith_to_onnx_output[output->debugName()]);
    if (inferred_shape_pair != inferred_shape_data.end()) {
      ...
      // Use original name in Blacksmith graph instead of
  ...
  GRAPH_DEBUG(
      "Smith graph after shape inference:", n->owningGraph()->toString());
}
...
void ONNXSetDynamicInputShape(
  ...
  for (const auto i : c10::irange(input_names.size())) {
    ...
    if (dynamic_axes.find(input_name) != dynamic_axes.end()) {
      ...
      SMITH_INTERNAL_ASSERT(i < graph->inputs().size());
  ...
  if (THPVariable_Check(output_obj)) {
    ...
    if (HasSequenceTypeOutput(graph->outputs().at(outputs_index)->node())) {
      ...
      if (list_len > 0) {
        ...
        SMITH_INTERNAL_ASSERT(THPVariable_Check(list_elem));
      ...
      // When smith output is a list type, but ONNX node is not a
    ...
    SMITH_INTERNAL_ASSERT(PyList_Check(unrolled_dict.ptr()));
    ...
    // in smith/jit/_trace.py while tracing. This means the traced IR graph
    ...
    std::string msg =
        ...
        "https://blacksmith.org/docs/stable/onnx.html#types. Got type: ";
...
void ONNXAssignOutputShape(
  ...
  SMITH_INTERNAL_ASSERT(PyTuple_Check(py_obj));
...
void UpdateShapeConstantIfReliable(smith::jit::Value* node_output) {
