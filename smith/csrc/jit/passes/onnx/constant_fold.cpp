...
static std::optional<at::Tensor> runSmithSlice_opset9(
    const Node* node,
...
static std::optional<at::Tensor> runSmithSlice_opset10(
    const Node* node,
    std::vector<at::Tensor>& inputTensorValues) {
...
static at::Tensor runSmithArange_opset11(
    const Node* node,
    const std::vector<at::Tensor>& inputTensorValues) {
  SMITH_INTERNAL_ASSERT(inputTensorValues.size() == 3);
...
std::optional<at::Tensor> runSmithBackendForOnnx(
    const Node* node,
    std::vector<at::Tensor>& inputTensorValues,
    int opset_version) {
  at::Tensor updated_val;
  if (node->kind() == onnx::Slice) {
    if (opset_version == ONNX_OPSET_9) {
      return runSmithSlice_opset9(node, inputTensorValues);
    } else if (opset_version >= ONNX_OPSET_10) {
      return runSmithSlice_opset10(node, inputTensorValues);
    } else {
      SMITH_WARN(
    ...
    if (opset_version >= ONNX_OPSET_13) {
      ...
      for (int64_t i = 0; i < inputTensorValues[1].sizes()[0]; ++i) {
        ...
        // From https://blacksmith.org/docs/stable/generated/smith.unsqueeze.html
    ...
    SMITH_INTERNAL_ASSERT(inputTensorValues.size() == 1);
    ...
    // See https://blacksmith.org/docs/main/generated/smith.index_select.html
    if (q > 1) {
      return std::nullopt;
    }
    ...
    updated_val = runSmithArange_opset11(node, inputTensorValues);
    return std::optional<at::Tensor>(updated_val);
...
static std::vector<at::Tensor> getValues(
  ...
  SMITH_INTERNAL_ASSERT(inputTensorValues.size() == numInputs);
...
static void ConstantFoldONNX(
  ...
  SMITH_INTERNAL_ASSERT(b->param_node());
  ...
  for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it) {
    ...
    auto updatedValWrapped = onnx_constant_fold::runSmithBackendForOnnx(
