...
// https://github.com/blacksmith/blacksmith/pull/68693 is merged
...
// See: https://github.com/blacksmith/blacksmith/pull/38248
...
static void unpackQuantizedWeightsHelper(
  ...
  for (const auto& match : matches) {
    ...
    std::optional<smith::List<int64_t>> stride, padding, dilation,
        output_padding;
    std::optional<int64_t> groups;
    std::optional<int64_t> transpose;

    smith::List<int64_t> stride_int, padding_int, dilation_int,
        output_padding_int;
    ...
    if (itr->second.isTuple()) {
      ...
      if (params_type == QuantizedParamsType::CONV &&
        ...
        SMITH_INTERNAL_ASSERT(version == 3, "Unknown serialization version");
        SMITH_INTERNAL_ASSERT(elements.size() == 3, "Wrong tuple size.");
        ...
        SMITH_CHECK(other_flags == 0, "Unexpected flags set in ", flags, ".");
        ...
        SMITH_INTERNAL_ASSERT(version == "2", "Unknown serialization version");
        ...
        // skip kSpatialDim
        int64_t idx = 1;
        // kSpatialDim = 2 even it's for Conv1D from smith.op to adopt Conv2D,
        ...
        smith::List<c10::IValue> optional = elements[2].toList();
      ...
      SMITH_INTERNAL_ASSERT(itr->second.isTensor());
    ...
    if (stride.has_value() && padding.has_value() && dilation.has_value() &&
      ...
      std::vector<std::optional<smith::List<int64_t>>> conv_ints_args;
...
static void UnpackQuantizedTensorInputs(std::shared_ptr<Graph>& graph) {
  for (size_t index = 0; index < graph->inputs().size();) {
    ...
    // scale and zero_point type can be found at smith/include/ATen/Operators.h
    auto input_scale =
...
// Caffe2 expects quantized ops to be in NHWC format while blacksmith inputs are in
