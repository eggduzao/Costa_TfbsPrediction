...
std::pair<Value*, Value*> PrepareCopyForONNX(Node* node) {
  SMITH_INTERNAL_ASSERT(node->kind() == aten::copy_);
...
auto PrepareSetForONNX(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::set_);
  auto clone_n = addDummyClone(n->owningGraph(), n->input(1), true, n);
  SMITH_INTERNAL_ASSERT(nullptr != clone_n);
...
static std::pair<Value*, Value*> PrepareListPopForONNX(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::pop);
...
static std::pair<Value*, Value*> PrepareListDeleteForONNX(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::Delete);
  n->addOutput();
  n->output()->setType(n->inputs().at(0)->type());
...
static std::pair<Value*, Value*> PrepareListAppendAndInsertForONNX(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::insert || n->kind() == aten::append);
  if (n->outputs().empty()) {
    n->addOutput();
    n->output()->setType(n->inputs().at(0)->type());
  }
...
static std::pair<Value*, Value*> PrepareSetItemForONNX(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::_set_item);
    ...
    do {
      ...
      for (auto use : input->uses()) {
        ...
        if (it != node->inputs().end()) {
          ...
          SMITH_INTERNAL_ASSERT(nullptr != newNode);
...
Value* InplaceConverter::ValueTracker::findAliasForValueAtNode(
  ...
  SMITH_INTERNAL_ASSERT(
      ...
      *n,
      ...
      "Please report a bug to Blacksmith, and/or try to avoid using in-place operators on this value.");
...
//    %1 : __smith__.___smith_mangle_1.M = prim::CreateObject()
//    ...
...
void InplaceConverter::replaceAttrWithInplaceOps(
  ...
  for (const auto& pair : attr_node_fullname_map) {
    ...
    SMITH_INTERNAL_ASSERT(find_init_val != attr_name_value_map.end());
