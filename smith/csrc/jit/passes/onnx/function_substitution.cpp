...
std::string TidyClassNameFromSmithScript(
  ...
  for (const auto& atom : class_name->atoms()) {
    bool is_internal_smith_atom = (atom == "__smith__");
    bool is_mangle_atom = (atom.find("__smith_mangle") != std::string::npos);
    if (!is_internal_smith_atom && !is_mangle_atom) {
      if (!out.empty()) {
        out += ".";
      }
      out += atom;
  ...
  while (parent_module_value) {
    ...
    if (parent_module_type &&
        parent_module_type->name() ==
            "__smith__.smith.nn.modules.container.ModuleList") {
...
ScopePtr ForwardCallScope(Graph& graph, Node* call_node) {
  SMITH_INTERNAL_ASSERT(call_node->kind() == prim::CallMethod);
  const std::string& method_name = call_node->s(attr::name);
  if (method_name == "forward") {
    const auto type = call_node->input(0)->type()->expect<c10::NamedType>();
    const std::string class_name = TidyClassNameFromSmithScript(type->name());
  ...
  for (auto it = block->nodes().begin(), end = block->nodes().end();
    ...
    switch (cur->kind()) {
      case prim::CallFunction: {
        SMITH_INTERNAL_ASSERT(cur->input(0)->node()->kind() == prim::Constant);
        auto function_constant = cur->input(0)->node();
        auto fun_type =
            function_constant->output()->type()->expect<FunctionType>();

        if ((fun_type->function()->qualname().qualifiedName().find(
                 "smith.nn.functional") != std::string::npos) &&
...
ScopePtr ONNXGraphTopLevelScope(Graph& graph) {
  ...
  if (auto top_module_type = graph.inputs().at(0)->type()->cast<ClassType>()) {
    auto scope_name = ::smith::jit::onnx::ONNXScopeName::createFullScopeName(
        TidyClassNameFromSmithScript(top_module_type->name()),
