...
// because they are not created by Blacksmith JIT tracing, but they may be used by
...
FunctionExtractor::FunctionContext::FunctionContext(
  ...
  SMITH_INTERNAL_ASSERT(!scopes.empty());
  ...
  for (const auto& scope : scopes) {
    ...
    SMITH_INTERNAL_ASSERT(scope_ctxs.find(scope) != scope_ctxs.end());
...
std::shared_ptr<Graph> FunctionExtractor::ConstructFuncGraph(
  ...
  for (auto* n : nlist) {
    auto clone_n = g->createClone(n, [&](Value* v) {
      SMITH_INTERNAL_ASSERT(env.find(v) != env.end());
      return env[v];
    });
...
void FunctionExtractor::ConvertScopeToFunction(
  ...
  SMITH_INTERNAL_ASSERT(pos != std::string::npos);
