...
// Function attribute map information is also returned, as Smith IR cannot
...
// attributes, that are unable to be traced through Smith IR.
...
//         super().__init__()
//         self.lns = smith.nn.ModuleList([smith.nn.LayerNorm(3, eps = i) for i in range(2)])
//         self.celu1 = smith.nn.CELU(1.0)
//         self.celu2 = smith.nn.CELU(2.0)

//     def forward(self, x: smith.Tensor, y: smith.Tensor, z: smith.Tensor) -> smith.Tensor:
//         res1 = self.celu1(x)
//         res2 = self.celu2(y)
//         for ln in self.lns:
//             z = ln(z)
...
SMITH_API NodeAttrNameMap ONNXFunctionExtraction(
    std::shared_ptr<Graph>& graph,
    const std::unordered_set<std::string>& module_names,
    const std::vector<std::string>& param_names);

SMITH_API void ONNXClearScopeRecords();

SMITH_API void ONNXTrackScopeAttributes(
    std::shared_ptr<Graph>& graph,
