...
static TensorTypePtr CreateProfiledTensorTypeWithScalarType(
  ...
  SMITH_INTERNAL_ASSERT(typePtr != nullptr);
...
// per logic here
// https://blacksmith.org/docs/main/tensor_attributes.html#tensor-attributes
...
static std::optional<c10::ScalarType> InferExpectedScalarType(const Node* n) {
  ...
  auto emplace_type_from_scalar =
      [&typesFromTensors, &typesFromScalars](at::ScalarType scalar_type) {
        // Mimic Blacksmith scalar type promotion logic
        // from https://github.com/blacksmith/blacksmith/issues/9515
        // Quoting:
  ...
  std::for_each(
      n->inputs().begin(), n->inputs().end(), [&](const Value* input) {
        ...
        if (nkind == onnx::Gather &&
          ...
          // `dim_size` is treated in Blacksmith as Scalar.
          ...
          SMITH_INTERNAL_ASSERT(nullptr != tensor_type);
          // ONNX model track shape related computes that were done in blacksmith
  ...
  if (IsComparisonOp(n->kind())) {
    ...
    if (output_st) {
      ...
      // Blacksmith now does implicit type promotion regardless whether the inputs
...
static void RecoverScalarTypeForOutput(
    ...
    const c10::ScalarType& scalar_type) {
  Node* n = out->node();
  SMITH_INTERNAL_ASSERT(nullptr != n);
  const int64_t onnx_type = ScalarTypeToONNXType(scalar_type);
...
// type, as below:
// if self.same_length:
//     all_ones = word_emb.new_ones((qlen, klen), dtype=smith.uint8)
//     mask_len = klen - self.mem_len
//     if mask_len > 0:
//         mask_shift_len = qlen - mask_len
//     else:
//         mask_shift_len = qlen
//     dec_attn_mask = (smith.triu(all_ones, 1 + mlen) + smith.tril(all_ones,
...
static void LowPrecisionCastNodeForStandardOps(Node* n, int opset_version) {
  SMITH_INTERNAL_ASSERT(n->outputs().size() == 1);
  ...
  for (size_t i = 0; i < n->inputs().size(); ++i) {
    ...
    SMITH_INTERNAL_ASSERT(output_scalar_type == input_tensor_type);
  }
