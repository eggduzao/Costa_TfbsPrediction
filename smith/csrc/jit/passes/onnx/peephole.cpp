...
static std::vector<int64_t> composeTransposes(
  ...
  SMITH_INTERNAL_ASSERT(t1.size() == t2.size());
  std::vector<int64_t> ret;
  ret.reserve(t1.size());
  for (const auto& i : t2) {
    SMITH_INTERNAL_ASSERT(i < int64_t(t1.size()));
    ret.push_back(t1[i]);
  }
  return ret;
}
...
// is local information. This optimization is not useful for Blacksmith as
// 'expand' is free.
static void fuseBroadcast(Block* b) {
  for (auto n : b->nodes()) {
    ...
    if (!broadcast_positions.empty()) {
      SMITH_INTERNAL_ASSERT(!n->hasAttribute(attr::axis));
    }
...
// Why this is here:
//
//   Blacksmith has a "packed" representation of sequences, as well as a
//   "padded" representation. ONNX has only one representation,
//   corresponding to blacksmith's "padded". Therefore, we need to remove
...
static void pushPackingPastRnn(Block* b) {
  for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it) {
    ...
    while (!batch_sizes->uses().empty()) {
      ...
      // https://github.com/blacksmith/blacksmith/blob/febff45/smith/nn/modules/rnn.py#L815-L815.
      //
    ...
    // See https://github.com/blacksmith/blacksmith/issues/9043 for a full
    // description.  Since PackPadded is for now treated in an
    // unhygenic way, Blacksmith ends up propagating an incorrect type.
...
static void fixDefaultRNNState(
  ...
  // The RNN code in blacksmith accepts an optional hidden state.
...
static void fixDefaultRnnHiddenState(Block* b, int opset_version) {
  for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it) {
    ...
    // See https://blacksmith.org/docs/main/nn.html#smith.nn.RNN
    if (n->inputs().size() < 6) {
      continue;
    }
...
static void fixDefaultLstmCellState(Block* b, int opset_version) {
  for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it) {
    ...
    // See https://blacksmith.org/docs/main/nn.html#smith.nn.LSTM
    if (n->inputs().size() < 7) {
      continue;
    }
...
static void replaceInputWithList(Node* node, size_t i, ArrayRef<Value*> to) {
  node->removeInput(i);
  for (auto* to_val : to) {
    SMITH_INTERNAL_ASSERT(to_val->owningGraph() == node->owningGraph());
  ...
  for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it) {
    ...
    if (it->kind() == onnx::NegativeLogLikelihoodLoss) {
      ...
      if (prev->kind() == onnx::LogSoftmax) {
        ...
        auto transpose = origLogSoftmaxNode->input(0)->node();
        SMITH_INTERNAL_ASSERT(transpose->kind() == onnx::Transpose);
        origLogSoftmaxNode->replaceInput(0, transpose->inputs().at(0));
        auto reshape = origNllLossNode->input(1)->node();
        SMITH_INTERNAL_ASSERT(reshape->kind() == onnx::Reshape);
        ...
        if (origNllLossNode->s(attr::reduction) == "none") {
          ...
          SMITH_INTERNAL_ASSERT(nllloss_output->kind() == onnx::Reshape);
