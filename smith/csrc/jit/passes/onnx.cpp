...
static void checkONNXCompatibility(const c10::FunctionSchema& schema) {
  ...
  for (const auto& arg : args) {
    ...
    if (type->kind() == TypeKind::OptionalType) {
      ...
      SMITH_INTERNAL_ASSERT(type->kind() != TypeKind::OptionalType);
    }
  ...
  for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end;
    ...
    if (it->kind().is_caffe2()) {
      ...
      for (const auto& arg : args) {
        ...
        SMITH_INTERNAL_ASSERT(origin_inputs_index < origin_inputs.size());
        ...
        if (type->isSubtypeOf(*TensorType::get())) {
          ...
          SMITH_INTERNAL_ASSERT(constant_node->kind() == prim::Constant);
          ...
          if (elem_type->isSubtypeOf(*TensorType::get())) {
            SMITH_INTERNAL_ASSERT(list_node->kind(), prim::ListConstruct);
...
py::dict BlockToONNX(
    ...
    py::set& values_in_env,
    bool is_sub_block) {
  smith::autograd::SymbolicContext ctx{};
  ctx.block = new_block;
...
static bool ConstantFoldCondition(smith::jit::Value* output) {
...
void NodeToONNX(
  ...
  py::object onnx_utils =
      py::module::import("smith.onnx._internal.smithscript_exporter.utils");
  ...
  py::object onnx_registration = py::module::import(
      "smith.onnx._internal.smithscript_exporter.registration");
  ...
  auto envFn = [&env](Value* n) -> Value* {
    auto py_n = py::cast(n);
    SMITH_CHECK(env.contains(py_n), "Dangling node reference");
    auto py_value = env[py_n];
    SMITH_CHECK(!py_value.is_none(), "Unused node was subsequently used");
    Value* value = py_value.cast<Value*>();
    return value;
  };
  ...
  auto callPySymbolicMethod = [&](ConcretePythonOp* op) {
    ...
    if (!py::hasattr(pyobj, "symbolic") && !is_registered_op) {
      ...
      // 1. The smith.autograd.Function class of this node object has `symbolic`
      // method defined.
      // 2. Custom export symbolic is registered for prim::PythonOp.
      if ((operator_export_type == ::smith::onnx::OperatorExportTypes::ONNX ||
           operator_export_type ==
               ::smith::onnx::OperatorExportTypes::ONNX_ATEN_FALLBACK) &&
    ...
    for (auto arg_type : op->cconv) {
      ...
      if (arg_type == 'c') {
        ...
        SMITH_CHECK(node_it != inputs.end(), "expected too many inputs");
    ...
    if (py::hasattr(pyobj, "symbolic")) {
      ...
      SMITH_INTERNAL_ASSERT(is_registered_op);
