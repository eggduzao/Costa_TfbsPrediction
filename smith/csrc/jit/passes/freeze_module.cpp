...
class AttributePropagator {
 public:
  AttributePropagator(
    ...
    for (auto name : preservedAttrs) {
      SMITH_CHECK(checkName(name), "Unknown name: " + name);
    }
  }
 ...
 private:
  ...
  // corresponding value. Based on initial test on resnet50 and other smith
    ...
    while (!blocks.empty()) {
      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          SMITH_INTERNAL_ASSERT(attrModule.hasattr(name));
          auto attr = attrModule.attr(name);
          ...
          SMITH_INTERNAL_ASSERT(path.has_value());
  ...
  void removeExtraWaitCalls(Block* b) {
    ...
    for (auto it = nodes.begin(); it != nodes.end(); it++) {
      ...
      SMITH_INTERNAL_ASSERT(node->inputs().size() == 1);
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          for (auto& mptr : modules) {
            ...
            if (module.type() == n->inputs()[0]->type()) {
              SMITH_INTERNAL_ASSERT(module.hasattr(name));
              auto module = Module(mptr);
...
void freeze_module_inplace(
    ...
    bool freezeInterfaces,
    bool preserveParameters) {
  SMITH_CHECK(module != nullptr, "module cannot be nullptr");
  checkModuleDoesNotReturnSelf(*module);
  AttributePropagator attrPropagator(
