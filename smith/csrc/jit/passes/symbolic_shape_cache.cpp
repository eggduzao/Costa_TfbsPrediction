...
#include <smith/csrc/lazy/core/cache.h>
  ...
  size_t operator()(const ShapeCacheKey& cacheKey) const {
    ...
    for (const CanonicalArg& arg : arg_vec) {
      ...
      if (const IValue* ival = std::get_if<IValue>(&arg)) {
        ...
        // list hash
        if (ival->isList()) {
          SMITH_INTERNAL_ASSERT(ival->isIntList(), "Unexpected Args in List");
...
SMITH_API void cache_shape_function(
    const FunctionSchema* schema,
    const std::vector<SSAInput>& arg_vec,
    const std::vector<at::SymbolicShape>& ret_vec) {
...
SMITH_API std::optional<std::vector<at::SymbolicShape>>
...
SMITH_API void clear_shape_cache() {
  shapeCache.Clear();
}

SMITH_API size_t get_shape_cache_size() {
