...
bool isAutocastNode(Value* value) {
  ...
  return class_name.has_value() &&
      (*class_name == "__smith__.smith.cuda.amp.autocast_mode.autocast" ||
       *class_name == "__smith__.smith.cpu.amp.autocast_mode.autocast" ||
       *class_name == "__smith__.smith.amp.autocast_mode.autocast");
}
...
// %5 : __smith__.smith.cuda.amp.autocast_mode.autocast = prim::CreateObject()
...
std::optional<AutocastScope> parseAutocast(
  ...
  if (value->node()->kind() == prim::CreateObject) {
    ...
    SMITH_CHECK(enabled.has_value(), "Autocast missing _enabled attribute");
    SMITH_CHECK(!device.empty(), "Autocast missing device attribute");
    ...
    //
    SMITH_CHECK(false, "Unsupported autocast syntax");
  }
...
// Users can call smith.is_autocast_enabled() or is_autocast_cpu_enabled() to
...
//
// Casting policy below mostly follows blacksmith/aten/src/ATen/autocast.cpp, with
    ...
    switch (node->kind()) {
      ...
      case prim::Enter:
        if (auto autocast_scope =
          ...
          if (node->hasUses()) {
            // TODO: better error message
            SMITH_CHECK(false, "`with autocast() as ...` is not supported");
          }
          SMITH_INTERNAL_ASSERT(
              !incompatible_amp.has_value() || !incompatible_amp.value(),
      ...
      case prim::Exit:
        if (isAutocastNode(node->input(0))) {
          SMITH_INTERNAL_ASSERT(!autocast_stack.empty());
          SMITH_INTERNAL_ASSERT(autocast_stack.top().instance == node->input());
          SMITH_INTERNAL_ASSERT(
              !incompatible_amp.has_value() || !incompatible_amp.value(),
              "Unsupported case by AMP & JIT");
          incompatible_amp = false;
      ...
      case aten::binary_cross_entropy:
        if (current_state()) {
          SMITH_CHECK(false, "Unsafe to autocast");
        }
    }
