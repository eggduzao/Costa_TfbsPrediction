...
struct BailOutGraphBuilderForNode {
  ...
  Value* addNewInputForValue(Value* old_value) {
    ...
    if (node->kind() == prim::Constant) {
      SMITH_INTERNAL_ASSERT(!shouldBeCapturedInByBailOut(node));
  ...
  Value* getInputForValue(Value* v) {
    SMITH_INTERNAL_ASSERT(this->old_to_new_.count(v));
    return this->old_to_new_[v];
  }
  ...
  void buildBailOutBlockFrom(Node* n) {
    ...
    if (outer_node) {
      if (outer_node->kind() == prim::Loop) {
        ...
        SMITH_CHECK(false, "Unexpected outer node");
      }
    }
  }
  ...
  void mapValues(
    ...
    SMITH_INTERNAL_ASSERT(block_outputs.size() == carried_deps.size());
...
// see `bailout_graph.h`
SMITH_API std::shared_ptr<Graph> BuildBailOutGraphFrom(
