...
#include <smith/csrc/jit/passes/utils/optimization_utils.h>
...
bool FoldFrozenConvBatchnorm(Block* b) {
  ...
  for (Node* n : b->nodes()) {
    ...
    if (n->kind() == aten::batch_norm &&
      ...
      // implementation taken from smith/nn/utils/fusion.py
      Tensor conv_b;
...
Tensor resizeConstantScalarOrTensorToShape(
  ...
  if (ret_tensor.numel() == 1) {
    ...
    SMITH_INTERNAL_ASSERT(ret_tensor.numel() == c10::multiply_integers(shape));
...
bool FoldFrozenConvAddOrSub(Block* b) {
  ...
  for (Node* n : b->nodes()) {
    ...
    if (supportedAddOrSub(n) && supportedConvNode(n->inputs().at(0)->node())) {
      ...
      SMITH_INTERNAL_ASSERT(stack_out && stack_out->size() == 1);
