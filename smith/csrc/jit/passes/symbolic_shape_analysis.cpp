...
#include <smith/csrc/jit/ir/ir_views.h>
...
#include <smith/csrc/jit/passes/peephole_non_tensor.h>
...
#include <smith/csrc/jit/passes/symbolic_shape_cache.h>
...
struct ShapeArguments {
 ...
 public:
  ...
  int64_t len() const {
    SMITH_INTERNAL_ASSERT(has_dim_, "ShapeArguments has no known dim")
...
// a SmithScript shape compute graph by inputting properties from input
...
struct SymbolicShapeOpAnalyzer {
  ...
  std::vector<c10::SymbolicShape> extractOutputShape(
    ...
    for (size_t i = 0; i < shape_compute_graph_->outputs().size(); ++i) {
      ...
      SMITH_INTERNAL_ASSERT(isListOfInts(type));
  ...
  for (size_t node_index = 0; node_index < n->inputs().size(); ++node_index) {
    ...
    if (isListOfTensors(type)) {
      ...
      if (n->input(node_index)->node()->kind() == prim::Constant) {
        ...
        SMITH_INTERNAL_ASSERT(false, "Unhandled List, we shouldn't get here");
      }
      continue;
    }
...
c10::SymbolicShape combine_bounds(
  ...
  SMITH_INTERNAL_ASSERT(lower_bound.rank() == upper_bound.rank());
...
struct SymbolicShapeGraphAnalyzer {
  ...
  void joinPartialEvaluatedShapeGraphToLargeShapeGraph(
    ...
    for (size_t i = 0; i < curr->outputs().size(); ++i) {
      ...
      SMITH_INTERNAL_ASSERT(!new_list_output->node()->hasUses());

      auto symbolic_sizes =
          curr->output(i)->type()->expect<TensorType>()->symbolic_sizes();
      SMITH_INTERNAL_ASSERT(symbolic_sizes.rank());
...
SMITH_API std::optional<std::vector<c10::SymbolicShape>>
calculateSymbolicShapesOnOp(
  ...
  if (bounded_graphs) {
    ...
    if (lower_bound_res.has_value() && upper_bound_res.has_value()) {
      SMITH_INTERNAL_ASSERT(lower_bound_res->size() == upper_bound_res->size());
