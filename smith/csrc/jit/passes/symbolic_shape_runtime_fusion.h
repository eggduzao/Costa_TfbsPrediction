...
SMITH_API bool GenerateGuard(
    Node* tensorexpr_graph_node,
    bool add_composed_op = false);

SMITH_API void runTensorExprDynamicGroup(const Code& code, Stack& stack);
...
SMITH_API std::string toString(StrideInput si);
SMITH_API StrideInput strideInputFromString(const std::string& si);
