...
SMITH_API bool UnrollLoops(std::shared_ptr<Graph>& graph);

// Only unrolls constant loops. Will unroll them regardless of loop block size
SMITH_API bool UnrollConstantLoops(std::shared_ptr<Graph>& graph);

SMITH_API Node* PeelLoop(Node* n, size_t times);

// return true if graph is modified
SMITH_API bool PeelProfilingLoops(const std::shared_ptr<Graph>& graph);

struct SMITH_API LoopsPeeler {
