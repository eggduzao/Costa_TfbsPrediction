...
struct SMITH_API MutationRemover {
  MutationRemover(
...
// return true if graph is modified
SMITH_API bool RemoveListMutation(const std::shared_ptr<Graph>& graph);
...
// return true if graph is modified
SMITH_API bool RemoveTensorMutation(
    const std::shared_ptr<Graph>& graph,
    std::optional<std::function<bool(Node*)>> mutation_filter = std::nullopt);

// Replaces in-place aten activation ops with their functional equivalence
SMITH_API bool InplaceToFunctionalActivation(
    const std::shared_ptr<Graph>& graph);
