...
struct GraphFuser {
  ...
  bool isFusableDevice(Value* v, bool strict_fuser_check) {
    ...
    if ((*device).is_cpu()) {
      ...
      SMITH_CHECK_NOT_IMPLEMENTED(false, "Unknown device for graph fuser");
    }
  }
      ...
      if (inputs_map.count(input) == 0) {
        if (input->type()->isSubtypeOf(*TensorType::get())) {
          ...
          Node* in_const = subgraph.createClone(
              input->node(),
              [](Value*) -> Value* { SMITH_CHECK(false, "unexpected input"); });
  ...
  at::ArrayRef<Value*> broadcast_tensors(value_list inputs) {
    ...
    SMITH_INTERNAL_ASSERT(unpack_node->outputs().size() == inputs.size());
