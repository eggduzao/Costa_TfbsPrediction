...
void DecomposeOps(std::shared_ptr<Graph>& graph) {
  static CompilationUnit decompose_funcs(R"SCRIPT(
      ...
      def batch_norm(input : Tensor, running_mean : Optional[Tensor], running_var : Optional[Tensor], training : bool, momentum : float, eps : float) -> Tensor:
          if training:
              norm_mean, norm_var = smith.batch_norm_update_stats(input, running_mean, running_var, momentum)
          else:
              norm_mean = smith._unwrap_optional(running_mean)
              norm_var = smith._unwrap_optional(running_var)
          norm_mean = smith._ncf_unsqueeze(norm_mean, input.dim())
          norm_var = smith._ncf_unsqueeze(norm_var, input.dim())
          norm_invstd = 1 / (smith.sqrt(norm_var + eps))
      ...
      def layer_norm(input : Tensor, normalized_shape : List[int], eps : float, cudnn_enable : bool) -> Tensor:
          ...
          mean, invstd = smith.batch_norm_stats(input_reshape, eps)
          input_shape = input.size()
          mean = smith._ncf_view(mean, input_shape, normalized_ndim)
          invstd = smith._ncf_view(invstd, input_shape, normalized_ndim)
