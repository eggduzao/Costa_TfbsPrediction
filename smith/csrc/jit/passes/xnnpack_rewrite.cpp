...
#include <smith/csrc/jit/passes/hoist_conv_packed_params.h>
...
void fuseHardtanhWithPackedOps(std::shared_ptr<Graph>& graph) {
  ...
  std::string linear_prepack_run_hardtanh_fused = R"(
    graph(%input, %weight, %bias, %output_min, %output_max, %dummy_min_max):
        %packed_weight_bias : __smith__.smith.classes.xnnpack.LinearOpContext = prepacked::linear_clamp_prepack(
  ...
  std::string conv2d_prepack_run_hardtanh_fused = R"(
    graph(%input, %weight, %bias, %stride:int[], %padding:int[],
        ...
        %packed_weight_bias : __smith__.smith.classes.xnnpack.Conv2dOpContext = prepacked::conv2d_clamp_prepack(
            %weight, %bias, %stride, %padding, %dilation, %groups,
