...
TryGeneralizeInputDimensionsToSymbolicShapes(
  ...
  for (Value* v : tensorexpr_graph->inputs()) {
    ...
    auto new_sizes = c10::fmap(shape_vec, [&](const at::ShapeSymbol& shape) {
      ...
      SMITH_INTERNAL_ASSERT(value >= 0, "Expected complete tensor");
      if (value == 1) {
        return value;
...
static void inlineFallbackGraphAndAddSRCopyOutOp(std::shared_ptr<Graph> graph) {
  ...
  SMITH_INTERNAL_ASSERT(n != nullptr, "Expected to find fallback graph");
  ...
  SMITH_INTERNAL_ASSERT(!false_block_outputs.empty());

  for (auto out : false_block_outputs) {
    SMITH_INTERNAL_ASSERT(out->type()->cast<TensorType>());
  }
...
void insertDynamicShapesGuard(
  ...
  SMITH_INTERNAL_ASSERT(inputs_to_check.size());
...
static RegisterOperators reg_guard({
    Operator(
        ...
        [](const Node* node) -> Operation {
          ...
          SMITH_INTERNAL_ASSERT(!types.empty());
          ...
          SMITH_INTERNAL_ASSERT(maybe_device);
          ...
          for (const auto& type : types) {
            ...
            SMITH_INTERNAL_ASSERT(ss.rank());
            expected_dims.push_back(*ss.rank());
            SMITH_INTERNAL_ASSERT(tt->scalarType());
            expected_scalar_types.push_back(*tt->scalarType());
            SMITH_INTERNAL_ASSERT(tt->device() && *tt->device() == device);
