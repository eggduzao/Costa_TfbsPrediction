...
void addBiasForConvIfNone(Module& module, const std::string& pattern_name) {
  ...
  const std::string demangled_typename = removeSmithMangle(real_typename);
  bool is_floating_point_conv =
      ((demangled_typename == "__smith__.smith.nn.modules.conv.Conv1d") ||
       (demangled_typename == "__smith__.smith.nn.modules.conv.Conv2d") ||
       (demangled_typename == "__smith__.smith.nn.modules.conv.Conv3d"));
  ...
  while (!worklist.empty()) {
    ...
    for (auto& method : current.get_methods()) {
      ...
      for (const auto& conv_bn : conv_bn_paths_.at(g)) {
        ...
        ConvBNParameters params;
        SMITH_INTERNAL_ASSERT(tryExtractingConvBNParameters(
            conv_submodule, bn_submodule, params));
