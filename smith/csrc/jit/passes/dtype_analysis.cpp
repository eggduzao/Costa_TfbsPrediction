...
#include <smith/csrc/jit/passes/utils/op_registry.h>
#include <optional>
...
std::unique_ptr<Stack> MTensorArgumentCreator(Node* n) {
  ...
  for (Value* inp : n->inputs()) {
    ...
    if (inp->type() == FloatType::get()) {
      ...
      // incorrect anyways.
      SMITH_CHECK(false, "Unsupported input type for Tensor argument");
    }
  }
  return stack;
}
...
bool setDtype(
  ...
  auto tensor_type = value->type()->cast<TensorType>();
  SMITH_INTERNAL_ASSERT(tensor_type, "Expecting a tensor type");
  if (!tensor_type->scalarType().has_value()) {
...
bool setIfAllDtypeMatch(Node* n) {
  ...
  SMITH_INTERNAL_ASSERT(!n->inputs().empty());
...
struct DtypePropagationPass {
 ...
 private:
  ...
  bool processNode(Node* n) {
    ...
    switch (n->kind()) {
      ...
      case prim::CallFunction:
        SMITH_INTERNAL_ASSERT(false, "Loop/Call not handled now");
      default:
        break;
    }
  ...
  bool mergeTensorProperties(
    ...
    SMITH_INTERNAL_ASSERT(list1.empty(), "Not implemented yet");
    return false;
  }
