#include <smith/csrc/jit/passes/concat_opt.h>
...
void removeCatNodeFromGraph(Node* n) {
  SMITH_INTERNAL_ASSERT(n->kind() == aten::cat);
...
class ConcatExpander {
 ...
 private:
  ...
  void expandCat(Node* node) {
    ...
    SMITH_INTERNAL_ASSERT(tensortype);
    ...
    for (auto cat_inp : cat_inp_list->inputs()) {
      ...
      SMITH_INTERNAL_ASSERT(cat_inp_tensor_type);
      SMITH_INTERNAL_ASSERT(cat_inp_tensor_type->dim());
...
size_t determineUsageIdx(Value* value, Node* user) {
  const auto idx =
      ...
      user->inputs().begin();
  using c10::ssize;
  SMITH_CHECK(idx != ssize(user->inputs()));
  return idx;
}

std::vector<Value*> getConcatInputs(Node* concat) {
  SMITH_CHECK(concat->kind() == aten::cat);
  auto* list = concat->input(0);
  auto* list_construct = list->node();
  SMITH_CHECK(list_construct->kind() == prim::ListConstruct);
