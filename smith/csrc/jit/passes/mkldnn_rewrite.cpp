...
static void insertPrePackedConvOpForNode(Node* n) {
  ...
  prepack_node->output()->setType(
      getCustomClass("__smith__.smith.classes.mkldnn.ConvOpContext"));
...
static void FuseReluWithPackedOps(std::shared_ptr<Graph>& graph) {
  ...
  auto conv_op_fused_rstring = at::jit::CodeTemplate(R"(
    graph(%input, %weight, %bias, %stride:int[], %padding:int[],
        ...
        %packed_weight_bias : __smith__.smith.classes.mkldnn.ConvOpContext = mkldnn_prepacked::conv2d_prepack(
            %weight, %bias, %stride, %padding, %dilation, %groups,
            %input_size, %attr)
...
static void PrePackingOpsFolder(Block* b) {
  ...
  for (Node* n : b->nodes()) {
    ...
    if (is_foldable_op(n)) {
      auto optional_outputs = smith::jit::runNodeIfInputsAreConstant(n);
      if (optional_outputs) {
        auto outputs = optional_outputs.value();
        SMITH_CHECK(outputs.size() == 1, "Prepack ops have single output");
