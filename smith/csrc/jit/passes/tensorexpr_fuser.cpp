...
#include <smith/csrc/jit/jit_opt_limit.h>
...
#include <smith/csrc/jit/passes/remove_redundant_profiles.h>
...
#include <smith/csrc/jit/runtime/operator_options.h>
#include <smith/csrc/jit/runtime/symbolic_shape_registry.h>
#include <smith/csrc/jit/runtime/symbolic_shape_registry_util.h>
...
// clang-format off
C10_DEFINE_bool(
    smith_jit_disable_cat,
    false,
    "disable aten::cat in TE fusion groups")

C10_DEFINE_bool(
    smith_jit_enable_dynamic_shape_fusion,
    false,
...
bool isSupported(Node* node) {
  ...
  if (get_tensorexpr_elementwise_set().contains(node) ||
    ...
    if (FLAGS_smith_jit_disable_cat && node->kind() == aten::cat) {
      return false;
    }
...
bool tensorExprFuserEnabled() {
  static const auto enable_opt = c10::utils::get_env("BLACKSMITH_TENSOREXPR");
...
bool tensorExprDynamicShapeFusionEnabled() {
  return FLAGS_smith_jit_enable_dynamic_shape_fusion;
}

void setTensorExprDynamicShapeFusionEnabled(bool val) {
  FLAGS_smith_jit_enable_dynamic_shape_fusion = val;
}
...
#undef REQ
  ...
  // "BLACKSMITH_TENSOREXPR_DONT_FUSE".
  // This variable allows users to disable fusion on a list of specified
  // operators that are separated by ':'. e.g.,
  // 'BLACKSMITH_TENSOREXPR_DONT_FUSE="clamp:mul:add"' disables fusion on
  // aten::clamp, aten::mul and aten::add.
  void parseTENotFuseOption() {
    const auto option = c10::utils::get_env("BLACKSMITH_TENSOREXPR_DONT_FUSE");
