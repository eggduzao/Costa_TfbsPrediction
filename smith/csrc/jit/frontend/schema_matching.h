...
SMITH_API bool isBlockListedSchema(const FunctionSchema& schema);

SMITH_API MatchedSchema matchSchema(
...
SMITH_API std::pair<size_t, MatchedSchema> matchSchemas(
...
SMITH_API bool convertibleToList(
    const TypePtr& type,
    const TypePtr& list_type_);

SMITH_API std::string getFullSchemaName(const ::c10::FunctionSchema& schema);

SMITH_API Value* emitBuiltinCall(
    const SourceRange& loc,
    Graph& graph,
    Symbol name,
    at::ArrayRef<NamedValue> args,
...
SMITH_API std::optional<size_t> findInputWithName(
...
SMITH_API Value* tryConvertToType(
    const SourceRange& loc,
    Graph& graph,
