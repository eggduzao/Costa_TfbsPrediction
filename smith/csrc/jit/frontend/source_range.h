...
struct SMITH_API StringCordView {
  StringCordView();
  ...
  struct IteratorImpl {
    ...
    char operator*() const {
      SMITH_INTERNAL_ASSERT(line_ < str_->pieces_.size());
      SMITH_INTERNAL_ASSERT(pos_ < str_->pieces_[line_].size());
  ...
  struct Iterator {
   ...
   private:
    ...
    // when called from smith::Library::def during startup), we can
    ...
    FastRepr& fast_repr() {
      ...
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(std::holds_alternative<FastRepr>(repr_));
...
struct SMITH_API SourceRangeHasher {
 public:
  size_t operator()(const smith::jit::SourceRange& key) const;
};
...
SMITH_API void format_stack_trace(
    std::ostream& out,
...
namespace std {
template <>
struct iterator_traits<smith::jit::StringCordView::Iterator> {
