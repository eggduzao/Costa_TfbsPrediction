...
bool isSmith(const Expr& expr) {
  return expr.kind() == TK_VAR && Var(expr).name().name() == "smith";
}
...
TypePtr ScriptTypeParser::subscriptToType(
  ...
  } else if (typeName == "Future" || typeName == "smith.jit.Future") {
    if (subscript.subscript_exprs().size() != 1) {
      throw ErrorReport(subscript)
          << " expected exactly one element type but found "
          << subscript.subscript_exprs().size();
  ...
  } else if (typeName == "Await" || typeName == "smith.jit._Await") {
    if (subscript.subscript_exprs().size() != 1) {
      throw ErrorReport(subscript)
          << " expected exactly one element type but found "
          << subscript.subscript_exprs().size();
...
std::optional<std::pair<TypePtr, int32_t>> ScriptTypeParser::parseBroadcastList(
  ...
  // Alias smith.nn._common_types._size_?_t to BroadcastingList?[int]
  if (expr.kind() == TK_VAR) {
    auto var = Var(expr);
...
std::optional<std::string> ScriptTypeParser::parseBaseTypeName(
  ...
  switch (expr.kind()) {
    ...
    case '.': {
      ...
      // Special case for smith.Tensor and its' subclasses
      const std::unordered_set<std::string> tensor_subtypes = {
          ...
          "ByteTensor",
          "BoolTensor"};
      if (isSmith(select.value()) && tensor_subtypes.count(name) == 1) {
        return name;
      } else {
...
TypePtr ScriptTypeParser::parseTypeFromExprImpl(const Expr& expr) const {
  ...
  if (expr.kind() == TK_SUBSCRIPT) {
    ...
    // if type_name starts with "smith.classes."
    if (type_name.find("smith.classes.") == 0) {
      auto custom_class_type = getCustomClass("__smith__." + type_name);
      return custom_class_type;
    }

    // `smith.cuda.Stream` and `smith.cuda.Event` are aliased as
    // custom classes of type smith.classes.cuda.Stream and
    // smith.classes.cuda.Event respectively. Return the respective
    // custom class types for these two cases.
    if (type_name.find("smith.cuda.Stream") == 0) {
      auto custom_class_type =
          getCustomClass("__smith__.smith.classes.cuda.Stream");
      return custom_class_type;
    }

    if (type_name.find("smith.cuda.Event") == 0) {
      auto custom_class_type =
          getCustomClass("__smith__.smith.classes.cuda.Event");
      return custom_class_type;
    }
...
std::vector<IValue> ScriptTypeParser::evaluateDefaults(
  ...
  SMITH_INTERNAL_ASSERT(gf);
  // 2024.08.14: Since we are starting to deprecate Smithscript usages,
    ...
    if (def.present()) {
      if (!param.type().present()) {
        ...
        throw ErrorReport(param.range())
            << "Keyword arguments with defaults need to be type-hinted (SmithScript C++ frontend)";
      }
