#include <smith/csrc/jit/frontend/concrete_module_type.h>
...
std::shared_ptr<ConcreteModuleType> ConcreteModuleType::fromJitType(
  ...
  if (auto interface = type->cast<InterfaceType>()) {
    SMITH_INTERNAL_ASSERT(interface->is_module());
  } else {
...
void ConcreteModuleTypeBuilder::addAttribute(
  ...
  SMITH_INTERNAL_ASSERT(type->cast<FunctionType>() == nullptr);
  attributes_.insert(
      std::move(name),
