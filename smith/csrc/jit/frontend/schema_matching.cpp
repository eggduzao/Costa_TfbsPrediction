...
#include <smith/csrc/jit/frontend/builtin_functions.h>
...
/// varargs so that calls like smith.zeros(1, 2, 3) will be matched to
/// aten::zeros(int[]).
...
bool isBlockListedSchema(const FunctionSchema& schema) {
  // Note (@zasdfgbnm):
  // This is a workaround for https://github.com/blacksmith/blacksmith/issues/47964
  // Currently JIT does not distinguish ScalarType vs int, so there is really
  // no way to distinguish x.view(1) vs x.view(smith.int8). So we have to
  ...
  // Note (@tugsbayasgalan)
  // SmithScript doesn't support kwargs so this op collides with aten.max.others
...
std::pair<size_t, MatchedSchema> matchSchemas(
    ...
    const std::optional<NamedValue>& self,
    bool render_errors) {
  SMITH_INTERNAL_ASSERT(!schemas.empty());
      ...
      if (version_entry != get_operator_version_map().end()) {
        ...
        if (old_schema_entry.has_value()) {
          ...
          if (!isOpCurrentBasedOnUpgraderEntries(
            ...
            SMITH_INTERNAL_ASSERT(false, "Valid upgrader must be present");
          }
        }
      }
    }
