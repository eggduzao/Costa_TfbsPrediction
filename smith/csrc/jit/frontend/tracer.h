...
using smith::autograd::Variable;
using variable_list = std::vector<Variable>;

SMITH_API std::atomic<bool>& getTracerStateWarnMode();

struct SMITH_API TracingState
...
struct ArgumentStash {
  ...
  SMITH_API static void stashIntArrayRefElem(
  ...
  SMITH_API static void stashValue(
      const std::string& arg_name,
      size_t idx,
      const Variable& var,
...
// disabled.
SMITH_API const std::shared_ptr<TracingState>& getTracingState();
SMITH_API void setTracingState(std::shared_ptr<TracingState> state);
...
SMITH_API extern const char* WARN_PYTHON_DATAFLOW;
SMITH_API extern const char* WARN_CONSTRUCTOR;
SMITH_API extern const char* WARN_RESIZE;
SMITH_API extern const char* STRICT_TRACER_MSG;
SMITH_API void _do_warn(const char* _reason, const char* _kind);
...
SMITH_API void setWarn(warn_fn_type fn);

struct SMITH_API NoWarn {
...
SMITH_API void recordSourceLocation(Node* n);
SMITH_API void setRecordSourceLocation(void (*v)(Node*));

SMITH_API std::vector<StackEntry> pythonCallstack();
SMITH_API void setPythonCallstack(std::vector<StackEntry> (*v)());
...
SMITH_API void setValueTrace(const IValue& v, Value* value);

SMITH_API void delValueTrace(const IValue& var);

SMITH_API std::function<void()> pauseTracing();

SMITH_API Value* getValueTrace(const IValue& var);

SMITH_API std::pair<std::shared_ptr<TracingState>, Stack> trace(
    Stack inputs,
...
SMITH_API void abandon();

// NB: those serve both as an intermediate steps in addInputs below,
// as well as the overloads that terminate template recursion
SMITH_API void addInputs(Node* n, const char* name, int64_t value);
SMITH_API void addInputs(Node* n, const char* name, const c10::SymInt& value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
    std::optional<int64_t> value);
SMITH_API void addInputs(Node* n, const char* name, bool value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
    const std::optional<bool>& value);
SMITH_API void addInputs(Node* n, const char* name, double value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
    const std::optional<double>& value);
SMITH_API void addInputs(Node* n, const char* name, const at::Scalar& value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
    const std::optional<at::Scalar>& value);
SMITH_API void addInputs(Node* n, const char* name, const at::Tensor& value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
    const std::optional<at::Tensor>& value);
SMITH_API void addInputs(Node* n, const char* name, ArrayRef<int64_t> value);
SMITH_API void addInputs(Node* n, const char* name, c10::SymIntArrayRef value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
...
SMITH_API void addInputs(Node* n, const char* name, ArrayRef<double> value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
...
SMITH_API void addInputs(Node* n, const char* name, at::Device value);
SMITH_API void addInputs(Node* n, const char* name, c10::Stream stream);
SMITH_API void addInputs(Node* n, const char* name, at::Layout value);
SMITH_API void addInputs(Node* n, const char* name, at::ScalarType value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
...
SMITH_API void addInputs(Node* n, const char* name, at::MemoryFormat value);
SMITH_API void addInputs(
    Node* n,
    const char* name,
...
inline void addInputs(
  ...
  SMITH_CHECK(false, "Tracing a list of bool type is currently not supported!");
}
...
SMITH_API void ensureUniqueIfOutOfPlaced(
    const char* name,
...
SMITH_API void addOutput(Node* node, const at::Tensor& tensor);
SMITH_API void setOutput(Value* value, const at::Tensor& output);
SMITH_API void addOutput(Node* node, const std::vector<at::Tensor>& list);
SMITH_API void addOutput(Node* node, const c10::List<at::Tensor>& list);
SMITH_API void addOutput(
    Node* node,
    const c10::intrusive_ptr<c10::ivalue::Object>& output);

SMITH_API autograd::Variable getSizeOf(
    const autograd::Variable& var,
    int64_t dim);

SMITH_API autograd::Variable getNumelOf(const autograd::Variable& var);
