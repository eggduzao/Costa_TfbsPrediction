...
// When the schema or behavior of a symbol changes, serialized Smithscript
// programs using that symbol are likely to break. To prevent those breaks,
// the symbol's historic behavior can be implemented as a Smithscript builtin
// and when an older Smithscript program is loaded the program's uses of the
