...
Value* IterableTree::len(const SourceRange& loc, GraphFunction& m) {
  ...
  SMITH_INTERNAL_ASSERT(!unroll_length_);
  Graph& g = *m.graph();
...
std::shared_ptr<SugaredValue> MagicMethod::call(
  ...
  SMITH_INTERNAL_ASSERT(base_value_);
...
std::shared_ptr<SugaredValue> SmithCheckValue::call(
    const SourceRange& loc,
    GraphFunction& m,
    at::ArrayRef<NamedValue> args,
    at::ArrayRef<NamedValue> kwargs,
  ...
  if (args.size() + kwargs.size() < 1 || args.size() + kwargs.size() > 2) {
    throw(
        ErrorReport(loc) << "smith._check() expects 1 or 2 arguments, got "
  ...
  for (const auto& kwarg : kwargs) {
    if (kwarg.name() == "cond") {
      if (found_cond_kwarg) {
        throw(
            ErrorReport(loc)
            << "smith._check() got multiple values for argument 'cond'");
      }
      ...
      if (found_message_kwarg) {
        throw(
            ErrorReport(loc)
            << "smith._check() got multiple values for argument 'message'");
      }
      message_arg = const_cast<NamedValue*>(&kwarg);
      found_message_kwarg = true;
    } else {
      throw(
          ErrorReport(loc) << "smith._check() got unexpected keyword argument '"
                           << kwarg.name() << "'");
    }
  }
  ...
  if (!cond_arg) {
    throw(
        ErrorReport(loc) << "smith._check() missing required argument 'cond'");
  }
