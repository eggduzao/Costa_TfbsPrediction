...
#include <smith/csrc/jit/frontend/parse_string_literal.h>
  ...
  if (it == type_map.end()) {
    ...
    throw(
        ...
        << "unknown type specifier. Common valid schema types include "
           ...
           "https://github.com/blacksmith/blacksmith/blob/main/aten/src/ATen/native/README.md#func ");
  }
  return it->second;
}
...
SchemaTypeParser::parseFakeAndRealType() {
  ...
  if (L.cur().kind == '(') {
    ...
    alias_info = parseAliasAnnotation();
  } else if (L.cur().kind == TK_IDENT && L.cur().text_view() == "__smith__") {
    L.next();
    L.expect('.');
    auto smith_tok = L.expect(TK_IDENT);
    if (smith_tok.text() != "smith") {
      throw(
          ErrorReport(smith_tok.range)
          << "Expected classes namespace but got " << smith_tok.text());
    }
    L.expect('.');
    ...
    fake_value = real_value = getCustomClass(
        std::string("__smith__.smith.classes.") + ns_tok.text() + "." +
        class_tok.text());
    if (!fake_value) {
      throw(
