...
#include <smith/csrc/jit/frontend/canonicalize_modified_loop.h>
#include <smith/csrc/jit/frontend/convert_to_ssa.h>
#include <smith/csrc/jit/frontend/lexer.h>
#include <smith/csrc/jit/frontend/parser.h>
#include <smith/csrc/jit/frontend/schema_matching.h>
#include <smith/csrc/jit/frontend/script_type_parser.h>
#include <smith/csrc/jit/ir/ir.h>
#include <smith/csrc/jit/passes/annotate_warns.h>
...
#include <smith/csrc/jit/passes/inline_forked_closures.h>
#include <smith/csrc/jit/passes/inliner.h>
#include <smith/csrc/jit/passes/lift_closures.h>
...
bool reportSourceLocation(size_t file_size) {
  ...
  const auto enable_env =
      c10::utils::get_env("BLACKSMITH_JIT_ENABLE_LARGE_SOURCE_LOCATION");
  bool flag = true;
      ...
      if (!as_simple_value->type()->isSubtypeOfExt(*parent_type, &why_not)) {
        ...
        if (simple_parent->type()->kind() == TypeKind::ListType &&
          ...
          error << "\nEmpty lists default to List[Tensor]. Add a variable "
                   ...
                   "of another type (smith.jit.annotate(List[T, []]) where T "
  ...
  SugaredValuePtr getSugaredVar(
    ...
    if (!retval) {
      static std::unordered_map<std::string, SugaredValuePtr> globals = {
          ...
          // todo(zach): remove when we can correctly export smith.full via ONNX
...
struct to_ir {
 ...
 private:
  ...
  static TypePtr getTypeForSetStateArg(const Def& def, const Self* self) {
    SMITH_CHECK(self, "Expected __setstate__ to have a `self` argument");
    ...
    if (!getstate) {
      throw(
          ...
          << "Did you forget to use `@smith.jit.export`?");
    }
  ...
  static bool shouldDeriveSetStateType(
    ...
    // well-formed
    SMITH_INTERNAL_ASSERT(def.name().name() == "__setstate__");
  ...
  std::vector<Argument> emitFormalArguments(
    ...
    for (; it != end; ++it) {
      ...
      if (shouldDeriveType) {
        SMITH_INTERNAL_ASSERT(schema.arguments().size() == 1);
  ...
  Argument emitOutput(
    ...
    SMITH_INTERNAL_ASSERT(ret_type);
        ...
        if (subscript_exprs[0].kind() == TK_SLICE_EXPR) {
          throw(
              ...
              << "del statements only support deletion at a single index, "
                 ...
                 " (see https://github.com/blacksmith/blacksmith/issues/31430)");
        }
    ...
    switch (expr.kind()) {
      ...
      default: {
        ...
        if (kind == aten::is_scripting) {
          static_if = true;
        } else if (kind == aten::has_smith_function) {
          static_if = false;
        }
          ...
          if (reportSourceLocation(loc.source()->size())) {
            ...
            environment_stack->setVariableTypeError(v, [=]() -> std::string {
              ...
              ss << v << " is not defined in the false branch. "
                 ...
                 << "To get it back, please set BLACKSMITH_JIT_ENABLE_LARGE_SOURCE_LOCATION=1 "
                 << "as env var";
              return ss.str();
            });
          }
        ...
        for (unsigned i = 1; i < 4; ++i) {
          if (exitSchema.arguments().at(i).type() != AnyType::get()) {
            throw(
                ErrorReport(e.range())
                << "argument " << i
                << " of __exit__ must have Any type; SmithScript does not currently support passing exception type, value, or traceback to the __exit__ function.");
          }
        }
      }
  ...
  // class A(Module):
  //  def __init__():
  //    self.register_buffer("running_var", smith.zeros(1))
  //
  //  def forward():
  //    self.num_batches += 1
  ...
  void emitAssignment(const Assign& stmt) {
    ...
    SMITH_INTERNAL_ASSERT(stmt.lhs_list().size() > 1 && !stmt.type().present());
    // a = b = expr()
  ...
  void emitSingleAssignment(const Assign& stmt) {
    ...
    switch (stmt.lhs().kind()) {
      case TK_VAR: {
        ...
        // changed linear_prepack to return a SmithBind class and not a
        ...
        if (method.qualname() ==
            "__smith__.smith.nn.quantized.dynamic.modules.rnn.PackedParameter.__setstate__") {
          if (auto sv =
  ...
  NodeKind getNodeKind(int kind, size_t ninputs) {
    switch (kind) {
      ...
      default:
        SMITH_CHECK(false, "unknown kind ", kind);
    }
  }
    ...
    switch (form) {
      ...
      case prim::LegacyTypedConstructor: {
        ...
        auto get_base_error_msg = [&]() {
          ...
          base_error_msg
              << "Legacy Tensor Constructor only supports two schemas in SmithScript: \n";
          base_error_msg
              << "'new(IntArrayRef size, *, Device? device=None)',\n";
        ...
        if (args.empty()) {
          // empty inputs == smith.tensor([], dtype=....)
          auto inp_list =
      ...
      default:
        SMITH_INTERNAL_ASSERT(false, "unknown special form: ", form);
    }
  }
  ...
  NodeKind reverseComparision(NodeKind kind) {
    ...
    SMITH_CHECK(false, "reverseComparision: unsupported NodeKind. File a bug");
  }
  ...
  // e.g. a : List[int] = []
  // or a = smith.jit.annotate(List[int], [])
  ...
  std::shared_ptr<SugaredValue> emitForkExpr(
    ...
    // into the fork input. If a closure doesn't already exist, we create one.
    {
      ...
      if (ClosureValue* sv = dynamic_cast<ClosureValue*>(forked.get())) {
        ...
        SMITH_INTERNAL_ASSERT(closure_block->outputs().size() == 1);
  ...
  std::shared_ptr<SugaredValue> emitRpcExpr(const Apply& apply, Symbol rpc_op) {
    ...
    SMITH_INTERNAL_ASSERT(returns.size() == 1);
    ...
    if (rpc_op == prim::rpc_async) {
      ...
      throw(
          ErrorReport(apply)
          << rpc_op.toDisplayString() << " is not supported in SmithScript!'");
    }
  ...
  Value* emitDictLiteral(DictLiteral dl, const TypePtr& type_hint) {
    ...
    SMITH_INTERNAL_ASSERT(!all_candidates.empty() || refined_type_hint);
  ...
  std::pair<Value*, std::vector<Value*>> emitIntAndSliceIndexing(
    ...
    auto handle_indexing = [&](const Expr& subscript_expr,
      ...
      // @smith.jit.script
      // def f(x):
      //   return x[smith.tensor([0, 1, 5])]  # or
      //   return x[smith.tensor([0, 1]), smith.tensor([0, 1])]  # or
      //   return x[smith.tensor([[0, 1], [0, 1]]),
      //            smith.tensor([[0, 1], [0, 1]])]  # or
      //   ls = [0, 1]
      //   return x[smith.tensor(ls)]
      if (index->type()->kind() == c10::TypeKind::ListType) {
        // Always create index tensor as LongTensor.
        // This is to match Blacksmith eager frontend behavior which accepts
  ...
  Value* emitTupleSlice(
    ...
    if (step) {
      ...
      SMITH_CHECK(val->isInt(), "Step size should always be an integer");
...
struct CompilationUnit::PropertyPair
  ...
  PropertyPair(
    ...
    SMITH_INTERNAL_ASSERT(getter, "Property pair must have defined getter")
...
std::unique_ptr<Function> CompilationUnit::define(
  ...
  SMITH_INTERNAL_ASSERT(resolver);
  ...
  auto creator = [def, _resolver, self](GraphFunction& method) {
    ...
    if (self) {
      ...
      SMITH_INTERNAL_ASSERT(atoms.size() >= 2);
...
std::vector<Function*> CompilationUnit::define(
  ...
  SMITH_INTERNAL_ASSERT(definitions.size() == defResolvers.size());
  SMITH_INTERNAL_ASSERT(properties.size() == propResolvers.size());
...
void CompilationUnit::define_hooks(
    ...
    const Self* self,
    bool shouldMangle) {
  SMITH_INTERNAL_ASSERT(hookDefs.size() == hookResolvers.size());
  SMITH_INTERNAL_ASSERT(preHookDefs.size() == preHookResolvers.size());
  std::vector<Function*> functions;
  std::unordered_map<std::string, Function*> function_table;
