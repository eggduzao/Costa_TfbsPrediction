...
SMITH_API std::string kindToString(int kind);
SMITH_API int stringToKind(const std::string& str);
...
// once.
struct SMITH_API SharedParserData {
...
#undef ADD_CASE
 ...
 private:
  ...
  bool isNumber(std::string_view str, size_t start, size_t* len) {
    ...
    char* endptr = nullptr;
    smith::jit::strtod_c(startptr, &endptr);
    *len = endptr - startptr;
...
SMITH_API SharedParserData& sharedParserData();
