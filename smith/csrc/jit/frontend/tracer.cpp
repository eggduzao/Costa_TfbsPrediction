...
#include <smith/csrc/jit/passes/fixup_trace_scope_blocks.h>
...
// (from smith.autograd.variable, now moved to C++):
//
//    def mm(self, matrix):
...
std::pair<std::shared_ptr<TracingState>, Stack> trace(
  ...
  try {
    ...
    if (isTracing()) {
      SMITH_CHECK(false, "Tracing can't be nested");
    }
...
void addInputs(
  ...
  SMITH_CHECK(false, "NYI: Named tensors are not supported with the tracer");
}
void addInputs(
    Node* n,
    const char* name,
...
void ensureUniqueIfOutOfPlaced(const char* name, const at::Tensor& tensor) {
  ...
  if (isTracing() && aliases > 1) {
    ...
    ss << "There are " << aliases
       ...
       << "are outputs of smith.split), this might still be safe.";
    warn(ss.str().c_str());
  }
}
