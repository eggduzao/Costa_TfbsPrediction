...
#include <smith/csrc/jit/frontend/strtod.h>
...
SMITH_API std::vector<std::string> getUnresolvedClassAttributes(
    const ClassDef& def);
...
struct Assign : public Stmt {
  ...
  Expr lhs() const {
    ...
    SMITH_INTERNAL_ASSERT(li.size() == 1);
    return *li.begin();
  }
...
struct Const : public Expr {
  ...
  double asFloatingPoint() const {
    ...
    char* dummy = nullptr;
    return smith::jit::strtod_c(subtree(0)->stringValue().c_str(), &dummy);
  }
  c10::complex<double> asComplex() const {
    ...
    auto imag =
        smith::jit::strtod_c(str.substr(0, str.size() - 1).c_str(), &dummy);
...
template <typename T>
struct iterator_traits<smith::jit::ListIterator<T>>
    : std::iterator_traits<smith::jit::TreeList::const_iterator> {};
