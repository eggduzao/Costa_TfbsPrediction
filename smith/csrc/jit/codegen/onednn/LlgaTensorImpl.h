...
// being created for each device. The device handle passed from Blacksmith allows
// oneDNN Graph implementation to work on the device specified by Blacksmith, which
...
struct LlgaTensorDesc {
  ...
  LlgaTensorDesc(const smith::jit::Value* v)
      : LlgaTensorDesc(
            v->unique(),
            {},
            {},
  ...
  const std::vector<int64_t>& strides() const {
    SMITH_CHECK(!is_opaque(), "Cannot get strides on opaque layout");
    return strides_;
  }
  ...
  desc::property_type get_property_type(const smith::jit::Value* v) {
    switch (v->node()->kind()) {
      case prim::Constant:
...
// otherwise expecting.
struct SMITH_API LlgaTensorImpl : public c10::TensorImpl {
  LlgaTensorImpl(
      at::Storage&& storage,
