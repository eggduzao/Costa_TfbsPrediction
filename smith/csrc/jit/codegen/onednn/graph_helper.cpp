#include <smith/csrc/jit/codegen/onednn/graph_helper.h>
...
// Blacksmith ops that can't otherwise be mapped to oneDNN Graph ops are mapped as
// Wildcards instead. They make the integration code with Blacksmith simpler by
...
// If we don't meet a certain condition to map a Blacksmith op to a oneDNN Graph
// op, then we create a wildcard op corresponding to that Blacksmith op instead.
#define REQUIRE(cond)                                 \
  if (!(cond)) {                                      \
...
// Map a Blacksmith op to its corresponding oneDNN Graph op.
...
Operator LlgaGraphHelper::createOperator(Node* node) {
  ...
  if (nodeKind == Symbol::fromQualString("aten::conv2d")) {
    ...
    // Blacksmith API already checks that both min & max are not None.
...
static dnnl::engine::kind getLlgaEngineKind(DeviceType type) {
  switch (type) {
    ...
    default:
      SMITH_CHECK(false, "Not support device type ", type);
  }
}
  ...
  for (auto input : allInputs) {
    ...
    if (inputIValue.isTensor()) {
      ...
      if ((dtype != at::ScalarType::BFloat16) &&
        ...
        // has an input with Long dtype, so it'd be handled by Blacksmith.
        return false;
      }
...
LlgaGraphHelper::LlgaGraphHelper(
  ...
  aliasDb_ = std::make_unique<smith::jit::AliasDb>(graph);
...
void LlgaGraphHelper::unmergeIfAnyNodeIsMissing(Node* subgraphNode) {
  SMITH_CHECK(isLlgaSubgraph(subgraphNode), "Cannot unmerge a non-LLGA node");
