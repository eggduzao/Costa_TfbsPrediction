...
static void handleBinaryOpInputs(Node* node) {
  ...
  if (node->input(0)->type()->isSubtypeOf(TensorType::get())) {
    ...
    if (node->input(1)->type()->isSubtypeOf(FloatType::get()) ||
      ...
      // dtype to that of the first input. Doing so helps us satisfy Blacksmith's
      ...
      // clang-format off
      //   >>> (1. + smith.rand([2]).half()).dtype
      //       smith.float16
      //   >>> (smith.tensor(1.).unsqueeze(0) + (smith.rand([2]).half())).dtype
      //       smith.float32
      // clang-format on
      ...
      // same dtype. We'll follow Blacksmith's type-promotion rules here.
