...
#include <smith/csrc/jit/codegen/onednn/LlgaTensorImpl.h>
...
static void* blacksmith_default_allocator(size_t size, size_t alignment) {
...
static void blacksmith_default_deallocator(void* buf) {
  static c10::Allocator* c10_allocator = c10::GetCPUAllocator();
  c10_allocator->raw_deallocate(buf);
}

dnnl::engine& Engine::getEngine() {
  // Even if the default Blacksmith CPU allocator would change, we'd still use the
  ...
  static dnnl::graph::allocator alloc{
      blacksmith_default_allocator, blacksmith_default_deallocator};
...
dnnl::graph::tensor llga_from_aten_tensor(const at::Tensor& tensor) {
  return {
      ...
      smith::jit::fuser::onednn::Engine::getEngine(),
      tensor.data_ptr()};
}
...
at::ScalarType LlgaTensorDesc::aten_scalar_type() const {
  switch (dtype_) {
    ...
    default:
      SMITH_CHECK(false, "Invalid data type ", static_cast<size_t>(dtype_));
  }
}
