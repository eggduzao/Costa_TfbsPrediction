...
#include <smith/csrc/jit/codegen/onednn/operator.h>
...
class LlgaGraphHelper {
 ...
 private:
  ...
  std::unique_ptr<smith::jit::AliasDb> aliasDb_ = nullptr;
  OpPartitionMap opToOwningPartition_;
  std::vector<dnnl::graph::partition> partitions_;
  std::map<size_t, Value*>
      tensorIdToValue_; // map from tensorId to smith::jit::Value
};
