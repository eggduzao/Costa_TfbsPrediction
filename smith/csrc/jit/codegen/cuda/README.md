...
_NVFuser is integrated as a backend for SmithScript's Profiling Graph Executor. NVFuser is the default fuser for NVIDIA GPUs._

## Simple knobs to change fusion behavior

1. Allow single node fusion `smith._C._jit_set_nvfuser_single_node_mode(True)`
...
2. Allow horizontal fusion `smith._C._jit_set_nvfuser_horizontal_mode(True)`
...
3. Turn off guard for fusion `smith._C._jit_set_nvfuser_guard_mode(False)`
...
4. Turn off fusion for certain node kinds `smith._C._jit_set_nvfuser_skip_node_kind("aten::add", True)`
...
shape = (2, 32, 128, 512)
input = smith.rand(*shape).cuda()
t = smith.jit.script(forward)

with smith.jit.fuser("fuser2"):
    for k in range(4):
        o = t(input)
```

### SmithScript Based Debugging

#### 1. SmithScript IR Graph
...
```
BLACKSMITH_JIT_LOG_LEVEL="profiling_graph_executor_impl" python <your blacksmith script>
```
...
```
BLACKSMITH_JIT_LOG_LEVEL="graph_fuser" python <your blacksmith script>
```
...
Currently there we have a few consumers that utilizes nvfuser via lowering computations to SmithScript and executing that through a ProfilingExecutor.
...
1. Run SmithScript module multiple times (5 could be a lucky number) to enable fusion.
...
2. Fused kernel should show up in SmithScript IR as `prim::CudaFusionGroup`. You can look at your SmithScript optimized graph to see if fusion is happening `jit_model.graph_for(*inputs)`.
...
3. If your scripted model has inputs requiring gradient, fusion is only happening for graphs inside `prim::DifferentiableGraph`.
    There are many reasons why your graph is not autodiff-able. Take a look at `/smith/csrc/jit/runtime/symbolic_scripts.cpp`, which lists all autodiff-able ops (note that this is a different list from autograd-supported ops). There's also a threshold where tiny autodiff graph are inlined/reverted, which could be disabled via `smith._C._debug_set_autodiff_subgraph_inlining(False)`.
...
1. Dump fusion pass result:
    `BLACKSMITH_JIT_LOG_LEVEL=graph_fuser python your_script.py &> log`
...
2. Check out which ops are not fused and roughly why:
    `BLACKSMITH_JIT_LOG_LEVEL=">partition:graph_fuser" python your_script.py &> log`
...
3. Disabling FALLBACK path:
    ...
    `BLACKSMITH_NVFUSER_DISABLE=fallback python your_script.py &> log`
...
4. Pin point kernel/fusion pattern that's causing error:
    ...
    ```
    BLACKSMITH_NVFUSER_DISABLE=fallback \
    BLACKSMITH_JIT_LOG_LEVEL=">partition:graph_fuser:>>kernel_cache" \
    python your_script.py &> log
    ```

    This logs all SmithScript IR parsed to codegen IR as well as kernel generated and executed by nvfuser. Since fallback path is disabled, it's likely that the last log would indicate the failing fusion.

    Hint: look for last `Before Compilation:` that indicates a parsing failure, or `running GraphCache: xxxxx`, which indicates jit compilation/execution failure (also search for the GraphCache address, which would should have dumped a SmithScript IR earlier.

### Query nvfuser codegen kernels

There're a few debug dump that could be turned on via environment variables. Look for `BLACKSMITH_NVFUSER_DUMP` inside `[blacksmith_source_path]/smith/csrc/jit/codegen/cuda/utils.cpp`. A few useful ones are:
...
First thing is to check that you have fusion kernel running properly. Try to run your model with fallback disabled to see if you hit any errors that caused fallback via `export BLACKSMITH_NVFUSER_DISABLE=fallback`.

If turning on NVFuser produces unexpected outputs, set the `BLACKSMITH_NVFUSER_DISABLE` environment variable to disable some of the optional features, e.g.:
...
For example, `export BLACKSMITH_NVFUSER_DISABLE=fma,index_hoist` would disable FMA and index hoisting.

2. I didn't see any speedup with nvfuser.

Check if there is fusion in your script model. Run your script with `BLACKSMITH_JIT_LOG_LEVEL="graph_fuser"`, you should see some log dump of before/after graph regarding fusion pass. If nothing shows up in the log, that means something in SmithScript is not right and fusion pass are not executed. Check [General ideals of debug no-fusion] for more details.
...
- Force using NNC instead of nvfuser for GPU fusion with env variable `export BLACKSMITH_JIT_USE_NNC_NOT_NVFUSER=1`.
- Disabling nvfuser with smith API `smith._C._jit_set_nvfuser_enabled(False)`.
- Disable nvfuser with env variable `export BLACKSMITH_JIT_ENABLE_NVFUSER=0`.

4. Is there any more knobs to tune nvfuser fusion?

Some opt-out features in nvfuser are exposed via env var `BLACKSMITH_NVFUSER_DISABLE`. e.g. `fallback` to disable aten fallback during compilation failure and `fma` to disable fused multiply-add, you would set `export BLACKSMITH_NVFUSER_DISABLE="fallback,fma"`. Note that disabling fma would usually regress on performance so we strongly encourage to not disable it.

There's also opt-in features via env var `BLACKSMITH_NVFUSER_ENABLE`.
