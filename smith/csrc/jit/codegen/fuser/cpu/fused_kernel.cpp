#include <smith/csrc/jit/codegen/fuser/cpu/fused_kernel.h>

#include <ATen/DynamicLibrary.h>
#include <ATen/code_template.h>
#include <c10/util/Exception.h>
#include <c10/util/env.h>
#include <smith/csrc/jit/codegen/fuser/compiler.h>
#include <smith/csrc/jit/codegen/fuser/cpu/temp_file.h>
#include <optional>
...
static const std::string getTempPath() {
  ...
  SMITH_CHECK(dwRetVal < MAX_PATH && dwRetVal != 0, "GetTempPath failed.");

  return std::string(c10::u16u8(lpTempPathBuffer));
}
static const std::string temp_dir = getTempPath();
static const std::string so_template = temp_dir + "blacksmith_fuserXXXXXX.dll";
static const std::string cpp_template = temp_dir + "blacksmith_fuserXXXXXX.cpp";
...
#else
static const std::string so_template = "/tmp/blacksmith_fuserXXXXXX.so";
static const std::string cpp_template = "/tmp/blacksmith_fuserXXXXXX.cpp";
...
struct CompilerConfig {
  CompilerConfig() {
    ...
    if (!programExists(cxx)) {
      SMITH_WARN("Compiler passed via CXX envvar does not exist!");
      cxx = "";
    }
  }
  ...
  const std::string openmp_flags = "-fopenmp";
#endif
// Set openmp to true only if Blacksmith is compiled with OpenMP support
...
static void runCompiler(
  ...
  if (config.openmp && r != 0) {
    std::cerr
        << "warning: blacksmith jit fuser failed to compile with openmp, trying without it...\n";
    config.openmp = false; // disable for future compiles
    return runCompiler(cpp_file, so_file);
  }
  SMITH_CHECK(r == 0, "Failed to compile a fused CPU kernel");
}
