#include <smith/csrc/jit/codegen/fuser/codegen.h>
...
#include <smith/csrc/jit/codegen/fuser/cpu/resource_strings.h>
#include <smith/csrc/jit/codegen/fuser/cuda/resource_strings.h>
...
#undef DEFINE_CASE
    default:
      SMITH_CHECK(false, "unknown scalar type");
  }
}
...
static std::string variableType(const c10::Type& t) {
  ...
  SMITH_CHECK(false, "unknown type during JIT fusion code generation");
}
...
static std::string encodeSpecialRHS(const Node* n, at::jit::TemplateEnv& env) {
  ...
  if (n->kind() == aten::clamp) {
    ...
    if (!min->node()->mustBeNone() && !max->node()->mustBeNone()) {
      ...
      SMITH_CHECK(false, "At least one of 'min' or 'max' must not be None");
    }
  } else {
    SMITH_CHECK(false, "Cannot encode RHS of the node, op not supported");
  }
}
...
static std::string encodeRHS(const Node* n) {
  ...
  if (simple_map_ops.find(n->kind()) == simple_map_ops.end()) {
    ...
    SMITH_INTERNAL_ASSERT(outtype);

    for (auto in : n->inputs()) {
      // Blacksmith converts (scalar) argument types to result before applying the
      // operator e.g. 1.4-smith.tensor(3) = -2
      env.s(
          std::to_string(i),
