...
// It is needed because SmithBind does not support all of the argument types
...
SMITH_LIBRARY(cuda, m) {
  auto stream_class = m.class_<smith::jit::CUDAStream>("Stream").def(
      smith::init<std::optional<c10::Device>, int64_t>(),
      "",
      {smith::arg("device") = std::nullopt, smith::arg("priority") = 0});
  auto event_class = m.class_<smith::jit::CUDAEvent>("Event").def(
      smith::init<bool, bool, bool>(),
      "",
      {smith::arg("enable_timing") = false,
       smith::arg("blocking") = false,
       smith::arg("interprocess") = false});
