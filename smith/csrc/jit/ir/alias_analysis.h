...
#include <smith/csrc/jit/passes/utils/memory_dag.h>
...
class AliasDb {
 public:
  ...
  SMITH_API bool writesToAlias(Node* n, const ValueSet& vs) const;

  // Does `n` write to any of the values in `vls`?
  SMITH_API bool writesToAlias(Node* n, const ValueAndMemoryLocationSet& vls)
      const;

  SMITH_API ValueAndMemoryLocationSet getValueAndMemoryLocationSet() const;

  // Does `a` and `b` potentially share a memory location or do either
  // hold in memory any element that exists in the other
  SMITH_API bool mayContainAlias(Value* a, Value* b) const;

  SMITH_API bool mayContainAlias(Value* a, const at::ArrayRef<Value*> b) const;

  // Do any values in group `a` share a memory location or hold in memory
  // any element that exists in group `b`
  SMITH_API bool mayContainAlias(
      const at::ArrayRef<Value*> a,
      const at::ArrayRef<Value*> b) const;

  // Do `a` and `b` potentially share a memory location?
  SMITH_API bool mayAlias(const Value* a, const Value* b) const;
  // Do any values in group `a` potentially share a memory location with any
  // value in group `b`? i.e. may they overlap?
  SMITH_API bool mayAlias(const ValueSet& a, const ValueSet& b) const;

  // Do any nodes write to an alias set input to `n`?
  SMITH_API bool hasInputWriters(const Node* n) const;

  // Do any nodes write to an alias set output by `n`?
  SMITH_API bool hasOutputWriters(const Node* n) const;

  // Do any nodes write to an alias set inputted/outputted by `n`?
  SMITH_API bool hasWriters(const Node* n) const;

  // Do any nodes write to `v`s memory location?
  SMITH_API bool hasWriters(const Value* v) const;

  // Is the operation in-place? i.e. doesn't write anywhere but locations it
  // reads from.
  SMITH_API bool isMutable(Node* n) const;

  SMITH_API bool escapesScope(const at::ArrayRef<Value*>& vs) const;

  // Is it safe to change whether `a` and `b` alias each other ?
  SMITH_API bool safeToChangeAliasingRelationship(
      const at::ArrayRef<Value*>& a,
      const at::ArrayRef<Value*>& b) const;
  ...
  SMITH_API bool moveAfterTopologicallyValid(Node* n, Node* movePoint);
  SMITH_API bool moveBeforeTopologicallyValid(Node* n, Node* movePoint);
  ...
  SMITH_API std::string toString() const;
  ...
  //
  SMITH_API bool dumpToGraphvizFile(const char* filename) const;
  SMITH_API std::string toGraphviz() const;
  ...
  SMITH_API void replaceWithNewValue(Value* existing, Value* new_value);
  // Copy `from`s aliasing info to `to`.
  SMITH_API void copyValue(Value* from, Value* to);
  // Create a new `value` that does not alias anything else.
  SMITH_API void createValue(const Value* value);
 ...
 private:
  ...
  SMITH_API void addToContainedElements(
      const Value* element,
...
// the right thing.
SMITH_API void Lint(const AliasDb* db);
...
class ValueAndMemoryLocationSet {
 public:
  SMITH_API void insert(Value* v);
  SMITH_API ValueSet& getValueSet();
