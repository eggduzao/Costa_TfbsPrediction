...
Value* insertConstant(
  ...
  SMITH_CHECK(false, "Unsupported value kind: ", val.tagKind());
}
...
std::optional<Value*> tryInsertConstant(
  ...
  if (val.isTensor()) {
    ...
    SMITH_INTERNAL_ASSERT(!ref.requires_grad());
...
std::optional<IValue> toIValue(const Value* v) {
  ...
  if (type->isSubtypeOf(*TensorType::get())) {
    ...
    SMITH_INTERNAL_ASSERT(list.isList());
    return list;
  } else if (
      type->cast<DictType>() &&
      node->kindOf(attr::value) == AttributeKind::ival) {
    const auto& dict = node->ival(attr::value);
    SMITH_INTERNAL_ASSERT(dict.isGenericDict());
    return dict;
  } else if (
      type->cast<TupleType>() &&
      node->kindOf(attr::value) == AttributeKind::ival) {
    const auto& tup = node->ival(attr::value);
    SMITH_INTERNAL_ASSERT(tup.isTuple());
    return tup;
    ...
    SMITH_CHECK(false, "constant literal not supported for: ", type->str());
  }
}
