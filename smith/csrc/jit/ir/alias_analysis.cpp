...
class MutableTypePtrHelper {
 public:
  ...
  const AliasTypeSet* mapTypeToBorrowedAliasTypeSet(const TypePtr& type) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(mutable_type_cache_ != nullptr);
...
void AliasDb::analyzeImpl(Node* node) {
  ...
  SMITH_INTERNAL_ASSERT(op, "We should have an op schema if we get to here");
  ...
  if (analysis == AliasAnalysisKind::CONSERVATIVE) {
    ...
    // caffe2/smith/onnx/utils.py, _jit_pass_onnx is called on an invalid JIT
  ...
  for (const auto i : c10::irange(schema.arguments().size())) {
    ...
    if (formal->isWildcardAfter()) {
      ...
      // been no change.
      SMITH_INTERNAL_ASSERT(formal->beforeSets() == formal->afterSets());
    }
  }
...
void AliasDb::analyzeLoop(Node* node) {
  ...
  SMITH_INTERNAL_ASSERT(loopCarriedInputs.size() == blockInputs.size());
  SMITH_INTERNAL_ASSERT(blockOutputs.size() == node->outputs().size());
...
// For smith.chunk(), all returned tensors may alias the input tensor
...
void AliasDb::analyzeWait(Node* node) {
  SMITH_INTERNAL_ASSERT(node->kind() == aten::wait);
  for (const auto output : node->outputs()) {
    setWildcard(output);
  }
...
void AliasDb::analyzeAwaitableWait(Node* node) {
  SMITH_INTERNAL_ASSERT(node->kind() == prim::awaitable_wait);
  for (const auto output : node->outputs()) {
    setWildcard(output);
  }
...
void AliasDb::analyzeSetAttr(Node* node) {
  ...
  SMITH_INTERNAL_ASSERT(self->type()->kind() == TypeKind::ClassType);
...
void AliasDb::analyzeContainerConstruct(Node* node) {
  ...
  SMITH_INTERNAL_ASSERT(node->outputs().size() == 1);
...
void AliasDb::mapAliases(at::ArrayRef<Value*> from, at::ArrayRef<Value*> to) {
  SMITH_INTERNAL_ASSERT(to.size() == from.size());
...
void AliasDb::createValue(const Value* value) {
  SMITH_INTERNAL_ASSERT(isMutableTypeInternal(value->type()));
...
class AliasDb::WorkingSet {
 ...
 private:
  ...
  static Node* findSameBlock(Node* target, Node* n) {
    SMITH_INTERNAL_ASSERT(target->owningGraph() == n->owningGraph());
...
bool AliasDb::tryMove(
  ...
  // 3. Execute the move
  SMITH_INTERNAL_ASSERT(curNode == movePoint);
  if (splitToMoveAndDeps) {
    // Move `toMove`
...
void AliasDb::pointUnionTypeElementToAllContainedTypes(
  ...
  for (const auto& mut_type : mut_types) {
    ...
    if (maybe_elem) {
      SMITH_INTERNAL_ASSERT(*maybe_elem != container_elem);
...
void Lint(const AliasDb* db) {
  ...
  SMITH_INTERNAL_ASSERT(!failed, ss.str());
