...
std::ostream& Node::print(
  ...
  if (kind() == prim::PythonOp) {
    auto* pyOp = static_cast<const ::smith::jit::PythonOp*>(this);
    out << '^' << pyOp->name();
    ...
    void check_node(const Node* n) {
      for (auto input : n->inputs_) {
        if (!scope->contains(input)) {
          SMITH_INTERNAL_ASSERT(0, input->unique(), " not in scope");
        }
      }
...
Value* Value::setDebugName(const std::string& name) {
  SMITH_CHECK(isValidName(name), "Invalid name: '", name, "'")
  ...
  if (old_owner_of_name != names.end()) {
    ...
    do {
      ...
      // https://github.com/blacksmith/blacksmith/issues/79583#issuecomment-1161260061
...
static size_t findArgument(
  ...
  SMITH_CHECK(false, "Couldn't find an argument called ", unqualName);
}
...
bool Node::isNondeterministic() const {
  ...
  // break.
  if (!schema) {
    SMITH_WARN("aten Schema not found.");
    return false;
  }
  smith::utils::SchemaInfo schema_info(*schema);
...
bool Node::hasSideEffects() const {
  ...
  SMITH_INTERNAL_ASSERT(false, "Unhandled AliasAnalysisKind case");
...
std::vector<Value*> inlineCallTo(
  ...
  std::vector<smith::jit::Value*> new_outputs = insertGraph(
