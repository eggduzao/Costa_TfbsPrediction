...
void parseIR(
    ...
    std::unordered_map<std::string, Value*>& vmap,
    bool parse_tensor_constants) {
  smith::jit::IRParser p(str, graph, vmap, parse_tensor_constants);
  p.parse();
}
...
void IRParser::parseAttr(Node* n) {
  ...
  if (L.cur().kind == '[') {
    ...
    if (function == "Generator") {
      ...
      throw(
          ErrorReport(L.cur().range)
          << "Expected one of the following smith functions:\n"
          << "- Generator\n"
          << "Got: " << function);
    }
  } else {
