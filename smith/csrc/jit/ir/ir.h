...
#include <smith/csrc/jit/ir/graph_node_list.h>
#include <smith/csrc/jit/ir/named_value.h>
...
namespace smith::jit {
namespace utils {
SMITH_API std::string getNodesModuleHierarchy(const Node& n);
} // namespace utils
class AliasDb;
...
SMITH_API std::ostream& operator<<(std::ostream& out, const Graph& g);
SMITH_API std::ostream& operator<<(std::ostream& out, const Node& n);
...
struct Value {
 ...
 public:
  ...
  SMITH_API void inferTypeFrom(const at::Tensor& output);
  SMITH_API void inferTypeFrom(
  ...
  SMITH_API bool mustBeNone() const;
  SMITH_API bool mustNotBeNone() const;
  size_t unique() const {
    return unique_;
  }
  ...
  SMITH_API Value* setDebugName(const std::string& name);
  ...
  SMITH_API std::string debugNameBase() const;
  Node* node() {
    return node_;
  }
  size_t offset() const {
  ...
  SMITH_API void replaceFirstUseWith(Value* newValue);
  ...
  //          %4 = g(%6)
  //          %5 = h(%6, %6)
  SMITH_API void replaceAllUsesWith(Value* newValue);
  ...
  SMITH_API void replaceAllUsesAfterNodeWith(const Node* node, Value* newValue);
  ...
  SMITH_API void replaceAllUsesDominatedByNodeWith(
      const Node* node,
      Value* newValue);

  SMITH_API Value* copyMetadata(Value* from);

  SMITH_API std::shared_ptr<Wrap<Value>> wrap() {
    if (!wrap_) {
...
struct Block {
  ...
  SMITH_API Block(Graph* graph_, Node* node_);
  ...
  SMITH_API void cloneFrom(Block* src, std::function<Value*(Value*)> value_map);
  SMITH_API void remapTypes(const std::function<TypePtr(TypePtr)>& type_map);

  SMITH_API std::shared_ptr<Wrap<Block>> wrap() {
    if (!wrap_) {
...
struct Graph : std::enable_shared_from_this<Graph> {
 ...
 public:
  ...
  SMITH_API void push_scope(const std::string& scope_name);
  SMITH_API void pop_scope();
  ...
  SMITH_API Node* create(NodeKind kind, size_t num_outputs = 1);
  SMITH_API Node* create(
      NodeKind kind,
      ArrayRef<Value*> inputs,
      size_t num_outputs = 1);

  SMITH_API Node* createNone();
  SMITH_API Node* createAutogradZero();
  SMITH_API Node* createUninitialized(TypePtr typ);
  SMITH_API Node* createWithSubgraph(Symbol kind);
  SMITH_API Node* createDifferentiableSubgraph();
  SMITH_API Node* createTuple(
      at::ArrayRef<Value*> values,
      TupleTypePtr optional_named_tuple = nullptr);
  SMITH_API Node* createTupleUnpack(Value* v);
  SMITH_API Node* createTupleIndex(
      Value* tup,
      Value* idx,
      const TypePtr& output_type);
  SMITH_API Node* createTupleSlice(
      Value* tup,
      int64_t beg,
      int64_t step_size,
      int64_t num_values);
  SMITH_API Node* createEnumName(Value* e);
  SMITH_API Node* createEnumValue(Value* e);
  SMITH_API Node* createList(
      const TypePtr& contained_type,
      at::ArrayRef<Value*> values);
  SMITH_API Node* createListUnpack(Value* v, size_t size);
  SMITH_API Node* createDict(
      const TypePtr& key_type,
      const TypePtr& value_type,
      at::ArrayRef<Value*> keys,
      at::ArrayRef<Value*> values);
  SMITH_API Node* createNumToTensor(Value* value);
  SMITH_API Node* createObject(const ClassTypePtr& type);
  SMITH_API Node* createSetAttr(
      Value* obj,
      const std::string& field,
      Value* newValue);
  SMITH_API Node* createGetAttr(Value* obj, const std::string& field);
  Value* insertGetAttr(Value* obj, const std::string& field) {
    return insertNode(createGetAttr(obj, field))->output();
  }
  SMITH_API Node* createStore(const std::string& name, Value* v);
  SMITH_API Node* createLoad(const std::string& name, const TypePtr& type);
  SMITH_API Node* createIsInstance(Value* v, at::ArrayRef<TypePtr> types);

  SMITH_API Value* insertUncheckedCast(Value* v, TypePtr type);

  // Insert a ToList operator with argument \p v and output type \p type.
  // \returns the output of the operation.
  SMITH_API Value* insertToList(Value* v, TypePtr type);

  SMITH_API Value* insertFunctionCall(
      Function* callee,
      const MatchedSchema& matched);
  SMITH_API Value* insertMethodCall(
  ...
  SMITH_API void lint() const;
  // for use in debugger
  SMITH_API void dump() const;

  SMITH_API ~Graph();

  SMITH_API std::string toString(bool print_source_locations = true) const;

  SMITH_API std::ostream& print(
      std::ostream& out,
      bool print_source_locations = true) const;

  friend SMITH_API std::ostream& operator<<(std::ostream& out, const Graph& g);

  SMITH_API std::shared_ptr<Graph> copy();
  SMITH_API std::unique_ptr<Graph> copyUnique();
  SMITH_API void remapTypes(const std::function<TypePtr(TypePtr)>& type_map);

 private:
  friend SMITH_API void Lint(const AliasDb* db);
  SMITH_API void freeNode(Node* n);
  SMITH_API void freeValue(Value* v);
  SMITH_API void freeBlock(Block* b);
...
struct SMITH_API ProfileIValueOp : public Node {
  static const Symbol Kind;
  ProfileIValueOp(
      Graph* graph,
...
// which is not included in libsmith.so. We still include some bits and pieces
...
struct SMITH_API PythonOp : public Node {
  using Node::Node;
...
SMITH_API void LintGraph(const std::shared_ptr<Graph>& graph);

SMITH_API at::ArrayRef<Value*> createTupleUnpack(Value* v);
 ...
 */
SMITH_API std::vector<Value*> insertGraph(
    Graph& g,
    Graph& callee,
    ArrayRef<Value*> inputs);
SMITH_API std::vector<Value*> insertGraph(
 ...
 * graph are the same.
 */
SMITH_API std::vector<Value*> inlineCallTo(
    Node* to_replace,
 ...
 */
SMITH_API std::vector<Value*> unpackOutputs(const std::vector<Value*>& outputs);

SMITH_API std::vector<Node*> findAllNodes(Graph& g, Symbol kind, bool recurse);
SMITH_API std::vector<Node*> findAllNodes(Block& b, Symbol kind, bool recurse);
SMITH_API std::vector<Node*> findAllNodes(
    at::ArrayRef<Block*> a,
    Symbol kind,
    bool recurse);

struct SMITH_API OperatorSet {
