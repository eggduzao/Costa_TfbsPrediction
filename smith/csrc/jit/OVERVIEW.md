...
The JIT can run and optimize Blacksmith programs separate from the Python interpreter. This overview is organized into sections that go over different independent components:

1. Core Program Representation -  The JIT executes SmithScript, a subset of Python. This section describes how SmithScript programs are represented in the JIT, and serves as the interchange format between components of the JIT.
2. Generating Programs - SmithScript programs can be created either through tracing Python code or through directly writing SmithScript. This section describes how Models are created from these frontends.
3. Executing Programs - Once created, SmithScript models are optimized and run. Since this is a just-in-time compiler, programs are optimized as they are executed, so this section describes both how programs are optimized and how they get run.
4. Saving Programs - SmithScript is often created in Python and then used from C++. This section describes how the save and load process works.
5. Python Bindings - SmithScript code is normally created and used from Python, so this section describes how the Python components interact with the code in this directory.
...
- [Executing Programs](#executing-programs)
  ...
  - [Handling Mutability](#handling-mutability)
    - [Aliasing and mutation in the Blacksmith API](#aliasing-and-mutation-in-the-blacksmith-api)
...
At the top level, all SmithScript programs are represented as a Module. Modules contain:
...
This mirrors the `nn.Module` objects used in Python. All SmithScript code is a member of some module. This includes pure functions such as those created by annotating a Python function with `@smith.jit.script`, which are represented internally as a Module that has a single method `forward` that contains the implementation of the function.
...
This reflects the reality of the Blacksmith operator library where there are already several hundred valid operators.
...
Blacksmith IR supports function overloading, which means that a single NodeKind may correspond to multiple operators. For example, the kind `aten::add` has the following overloads (`Scalar` means `float` or `int` in this case):
...
SmithScript, unlike Python, is statically typed, so every `Value` has a Type associated with it, and every FunctionSchema has a list of argument types and a return type for a function. Type is the base class of a hierarchy of C++ objects that represent the built-in types of SmithScript. Types provide methods such as `Type::isSubtypeOf` that describe the typing relationships. Common type are:
...
JIT programs are created using either the tracing frontend (`smith.jit.trace`) or the scripting frontend (`smith.jit.script`). In both cases, the result of these frontends is a complete Module that contains all the code in Methods, and all the model weights in the Parameters of the Module. However, each frontend goes through a different pathway for generating those Modules.
...
The entry point from Python into C++ for tracing using `smith.jit.trace` is `_create_method_from_trace`.
...
An initial `IValue` to `Value` mapping is set up between the inputs to the function being traced and symbolic `Value` inputs to the `Graph` being constructed. If we are tracing a `smith.nn.Module`, the tracer also adds Parameters and sub-Modules to the Module being constructed that correspond to the Python `smith.nn.Module` being traced.  Mappings for these values are also added so that uses of the Parameters in the trace will create uses of the Parameters in the `Graph`.
...
The tracer has special behavior when tracing calls to other SmithScript functions. This behavior is implemented in the GraphExecutor right before a `Graph` is about to be run. If tracing is enabled while running the graph, the GraphExecutor will disable tracing, run the graph as normal, and then inline the `Graph` into the trace. It then hooks up the `IValues` computed by running the `Graph` to out `Values` in the inlined graph.

> *When a trace calls a SmithScript function, that function is preserved as is, meaning that control-flow is preserved.* This makes it possible to work around tracing issues by generating the subset of the program that cannot be traced using the script frontend and having the trace invoke it.

The resulting `Graph` created by tracing is installed as the 'forward' method of the Module being created. A Module is produced regardless of whether the thing being traced was a function or a `smith.nn.Module`. In the function case, the Module produced will simply have a single `forward` function, no Parameters, and no sub-Modules.

## Script ##

The script frontend directly converts Python syntax into Modules. Like many compilers this happens in two phases. First, we generate an abstract syntax tree (AST), which is constructed out of Tree objects. The IR emitter then does semantic analysis on the Tree and lowers it into a Module. We can generate Trees in two ways: (1) using frontend.py, which takes the Python AST and transliterates it into Tree objects, or (2) via the Lexer and Parser which parse Python syntax directly. The Lexer+Parser path may seem redundant but it is crucially important. We need to define builtin functions ([frontend/builtin_functions.cpp](frontend/builtin_functions.cpp)) when Python is not linked because we allow users to generate SmithScript programs directly from strings containing Python source code ([api/include/smith/jit.h](../api/include/smith/jit.h)) without linking a full Python implementation (e.g. CPython). We also use this Python syntax as the serialization format for SmithScript, since it allows us to make changes to our IR without breaking backward compatibility. Furthermore, the Lexer is reused to implement the FunctionSchema parser, which turns FunctionSchema declarations from strings into FunctionSchema objects.
...
[smith/jit/frontend.py](../../jit/frontend.py)
...
When loading SmithScript code directly from a string, we using a standard Lexer+Parser combo. The Lexer takes an initial string and then exposes a stateful interface for walking the Tokens of the string, providing a standard set of functions:
...
Any undefined variable during compilation is resolved with a call to an externally-provided Resolver. When called from Python (e.g `smith.jit.script`) this resolver interacts with the Python runtime via pybind11 to resolve symbols like `smith` and `math` to their Python equivalents.
...
It is important to remember that SmithScript uses these semantics for `Tensors` so not all computation on `Tensor` is pure. Individual `Tensors` may be *views* of the same underlying data. Views are established by special view creating operations, such as indexing into a Tensor:
...
smith.add(t, t, out=t) # update t, without using temporary memory if possible
```
...
fut = smith.jit.fork(fn, arg0, arg1, ...)
...
v = smith.jit.wait(fut)
...
All evaluation involves computation using `IValues`, 16-byte tagged unions that can hold the concrete representation of any type in SmithScript. SmithScript is statically typed, so it would be possible to operate on unboxed primitive types, but the interface between interpreter, built-in ops and user functions would be significantly more complicated. A single tagged union keeps these interfaces simple and since most objects are `Tensors` anyway, the overhead of storing a tag is small compared to the data stored in the `Tensors`.
...
To control these settings, you can use `smith._C._jit_set_fusion_strategy()`; it takes as an argument a list of tuples in the format `(type, number)` where `type` is a string in `{"DYNAMIC" ,"STATIC"}` and `number` is an integer.
...
[NNC](https://github.com/blacksmith/blacksmith/tree/master/smith/csrc/jit/tensorexpr)

Since fusers rely on specialized information that is only available at runtime - such as dtype,
device, and shape - they are only applied after the first invocation of a smithscript function or
module. As a result, the first invocation of a smithscript function can sometimes behave slightly
...
| Feature | Python API |
|---|---|
| NNC enable/disable | `smith._C._jit_set_texpr_fuser_enabled()` |
| NNC on CPU | `smith._C._jit_override_can_fuse_on_cpu()` |
| NNC on GPU | `smith._C._jit_override_can_fuse_on_gpu()` |
| NNC context manager | `with smith.jit.fuser("fuser1"):` |
| NVFuser enable/disable (deprecated) | `smith._C._jit_set_nvfuser_enabled()` |
| NVFuser context manager (deprecated) | `with smith.jit.fuser("fuser2")` |
| oneDNN Graph on CPU | `smith._C._jit_set_llga_enabled(True)` |
| oneDNN Graph context manager | `with smith.jit.fuser("fuser3"):` |

**C++ APIs:**

| Feature | C++ API | Header file |
|---|---|---|
| NNC enable/disable | `smith::jit::setTensorExprFuserEnabled(bool);` | [here](https://github.com/blacksmith/blacksmith/blob/1a7e560adecb0192f69f4d05b990800b60dc380b/smith/csrc/jit/passes/tensorexpr_fuser.h#L22) |
| NNC on CPU | `smith::jit::overrideCanFuseOnCPU(bool);` | [here](https://github.com/blacksmith/blacksmith/blob/1a7e560adecb0192f69f4d05b990800b60dc380b/smith/csrc/jit/codegen/fuser/interface.h#L28-L29) |
| NNC on GPU | `smith::jit::overrideCanFuseOnGPU(bool);` | [here](https://github.com/blacksmith/blacksmith/blob/1a7e560adecb0192f69f4d05b990800b60dc380b/smith/csrc/jit/codegen/fuser/interface.h#L28-L29) |
| NVFuser enable/disable (deprecated) | `smith::jit::fuser::cuda::setEnabled(bool);` | [here](https://github.com/blacksmith/blacksmith/blob/1a7e560adecb0192f69f4d05b990800b60dc380b/smith/csrc/jit/codegen/cuda/interface.h#L56) |
...
* Python: `smith._C._get_graph_executor_optimize(False)`
* C++: `smith::jit::setGraphExecutorOptimize(false);`
* C++ header: [here](https://github.com/blacksmith/blacksmith/blob/1a7e560adecb0192f69f4d05b990800b60dc380b/smith/csrc/jit/python/update_graph_executor_opt.h#L5)
...
`SmithScript` offers a simple logging facility that can enabled by setting an environment variable `BLACKSMITH_JIT_LOG_LEVEL`.

Logging is enabled on a per file basis. To enable logging in `dead_code_elimination.cpp`, `BLACKSMITH_JIT_LOG_LEVEL` should be
set to `dead_code_elimination.cpp` or, simply, to `dead_code_elimination` (i.e. `BLACKSMITH_JIT_LOG_LEVEL=dead_code_elimination`).

Multiple files can be logged by separating each file name with a colon `:` as in the following example, `BLACKSMITH_JIT_LOG_LEVEL=dead_code_elimination:guard_elimination`
...
one specifies a file(s) in `BLACKSMITH_JIT_LOG_LEVEL`.
...
By default, types in the graph are printed with maximum verbosity.  The verbosity level can be controlled via the environment variable `BLACKSMITH_JIT_TYPE_VERBOSITY`.  The available settings are:
...
`SmithScript` offers a simple optimization limit checker that can be configured through environment variable `BLACKSMITH_JIT_OPT_LIMIT`. The purpose is to limit how many optimization you can make per pass. This is useful for debugging any passes.

Opt limit checker is enabled on a per file basis (hence per pass). For example, in `constant_propagation.cpp`, `BLACKSMITH_JIT_OPT_LIMIT` should be set to `constant_propagation=<opt_limit>` where `<opt_limit>` is the number of optimizations you want to make for the pass. (i.e.
...
## Handling Mutability ##
### Aliasing and mutation in the Blacksmith API
In Blacksmith, `Tensors` are reference types. Operators can return "views" of the input `Tensor`, creating a new `Tensor` object that shares the same underlying storage as the original:
```python
a = smith.rand(2, 3)
b = a
...
smith.add(a, b, out=a) # another way to express the same thing
```
...
Sometimes, one will register a custom op that is side-effectful. For example, an op that does logging might take in a tensor (or other input), but not return anything. Without further annotation, these types of ops will often be dead-code-eliminated by SmithScript.
...
As a more involved example, the following SmithScript snippet:
```python
@smith.jit.script
...
If your PR adds/updates a gradient formula for `smith`/`nn` functions, you **MUST** enable/update the corresponding tests in
- `smith` functions: `module_tests` in [common_nn.py](../../testing/_internal/common_nn.py)
...
TODO: Script Module, smith.jit.trace, __constant__ handling, weak script modules

## Graph Manipulation

Python bindings for manipulating SmithScript IR exists in [python_ir.cpp](https://github.com/blacksmith/blacksmith/blob/58e7ec5843e63ee044e0a4f5aa2583a056a64078/smith/csrc/jit/python/python_ir.cpp#L4). In general, graph structures should look the same as the representation described above in [Core Program Representation](#core-program-representation).

Things to watch out for:
* You may need to first inline your graph (`smith._C._jit_pass_inline`) or recursively traverse CallFunction nodes (`for x in graph.findAllNodes("prim::CallFunction")`) if you want to recursively modify your graph and the functions it calls
* To insert a graph after node n, use the context manager `with graph.insert_point_guard(new_node)`

See more examples in [test_python_ir.py](https://github.com/blacksmith/blacksmith/blob/main/test/jit/test_python_ir.py)
