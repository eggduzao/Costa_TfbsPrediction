#include <smith/csrc/jit/tensorexpr/eval.h>

#include <smith/csrc/jit/jit_log.h>
#include <smith/csrc/jit/tensorexpr/external_functions_core.h>
...
static inline std::enable_if_t<std::is_integral_v<T>, T> div_value(
    T lhs,
    T rhs) {
  SMITH_CHECK(rhs != 0, "Division by zero");
  return lhs / rhs;
}
...
class SimpleIREvaluatorImpl : public IRVisitor {
 public:
  ...
  SMITH_API void visit(const AddPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const SubPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const MulPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const DivPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const ModPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const MaxPtr& v) override {
    visit_binary_op(v, v->propagate_nans());
  }
  SMITH_API void visit(const MinPtr& v) override {
    visit_binary_op(v, v->propagate_nans());
  }

  SMITH_API void visit(const AndPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const OrPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const XorPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const LshiftPtr& v) override {
    visit_binary_op(v);
  }
  SMITH_API void visit(const RshiftPtr& v) override {
    visit_binary_op(v);
  }
...
#define IMM_VISIT(Type, Name)                            \
  SMITH_API void visit(const Name##ImmPtr& v) override { \
    value_ = InterpValue(v->value());                    \
  }
  AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, IMM_VISIT)
#undef IMM_VISIT

  SMITH_API void visit(const BlockPtr& v) override {
  ...
  SMITH_API void visit(const VarPtr& v) override {
...
#undef DST_TYPE_CASE_QUANT
  ...
  SMITH_API void visit(const CastPtr& v) override {
...
#undef DST_TYPE_CASE
  ...
  SMITH_API void visit(const BitCastPtr& v) override {
    ExprPtr src_value = v->src_value();
    src_value->accept(this);
    Dtype dst_dtype = v->dtype();
    Dtype src_dtype = src_value->dtype();
...
#undef SRC_TYPE_CASE
  ...
  SMITH_API void visit(const ForPtr& v) override {
  ...
  SMITH_API void visit(const RampPtr& v) override {
    v->base()->accept(this);
  ...
  SMITH_API void visit(const BroadcastPtr& v) override {
    v->value()->accept(this);
...
#undef TYPE_CASE
  ...
  SMITH_API void visit(const IfThenElsePtr& v) override {
...
#undef TYPE_CASE
  ...
  SMITH_API void visit(const LoadPtr& v) override {
...
#undef TYPE_CASE
  ...
  SMITH_API void visit(const StorePtr& v) override {
    auto iter = buffer_mapping_.find(v->buf());
    if (iter == buffer_mapping_.end()) {
...
#undef TYPE_CASE
  ...
  SMITH_API void visit(const IntrinsicsPtr& v) override {
