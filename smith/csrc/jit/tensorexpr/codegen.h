...
class SMITH_API ExtCallMemoryReuse : public IRMutator {
...
#define ARG_PTR_DEFINE(Type, Name)                  \
  Type* Name##Ptr() const {                         \
    SMITH_INTERNAL_ASSERT(data_ == (void*)buffer_); \
    return (Type*)data_;                            \
  }
...
class RegisterCodeGenList {
 public:
  SMITH_API static RegisterCodeGenList& GetInstance();
  ...
  SMITH_API StmtFactoryMethod FindStmtFactoryMethod(const std::string& name);
 ...
 private:
  ...
  SMITH_API void AddStmtFactoryMethod(
...
SMITH_API std::unique_ptr<CodeGen> CreateCodeGen(
    const std::string& name,
    StmtPtr stmt,
    const std::vector<CodeGen::BufferArg>& params,
    at::Device device = at::kCPU,
...
class SMITH_API GenericIntrinsicsExpander : public IRMutator {
 protected:
