...
#include <smith/csrc/jit/tensorexpr/mem_dependency_checker.h>
...
struct SMITH_API TensorAccessBoundsInfo {
...
// have been run.
SMITH_API BoundsInfo getInferredBounds(
...
SMITH_API void printBoundsInfo(const BoundsInfo& v);

SMITH_API std::vector<ExprPtr> getBoundExtents(
...
SMITH_API HazardKind getPotentialHazards(
    analysis::MemDependencyChecker& analyzer,
    const StmtPtr& A,
    const StmtPtr& B);
...
SMITH_API bool hasConflictingOverlap(
    analysis::MemDependencyChecker& analyzer,
    const StmtPtr& A,
    const StmtPtr& B);
// Same as above, between accesses in stores S1 and S2.
SMITH_API bool isOverlapping(
    analysis::MemDependencyChecker& analyzer,
    const StorePtr& S1,
    const StorePtr& S2);
