...
// dimension.
OverlapKind SMITH_API boundOverlap(const Bound& A, const Bound& B);
...
CmpEvalResult SMITH_API compareBound(
    const Bound& a,
    const Bound& b,
...
bool SMITH_API indexBoundsEquals(const IndexBounds& A, const IndexBounds& B);
...
Bound SMITH_API flattenBounds(const IndexBounds& a);

// Determines the kind of overlap in X dimensions.
OverlapKind SMITH_API overlaps(const IndexBounds& a, const IndexBounds& b);
...
std::vector<Bound> SMITH_API subtractBound(const Bound& a, const Bound& b);

// Returns the bound slices created by subtracting the IndexBounds B from A.
std::vector<IndexBounds> SMITH_API subtractIndicesBounds(
    const IndexBounds& A,
    const IndexBounds& B,
    OverlapKind overlap);
std::vector<IndexBounds> SMITH_API
