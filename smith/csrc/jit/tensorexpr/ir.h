...
#include <smith/csrc/jit/tensorexpr/exceptions.h>
#include <smith/csrc/jit/tensorexpr/expr.h>
#include <smith/csrc/jit/tensorexpr/fwd_decls.h>
#include <smith/csrc/jit/tensorexpr/stmt.h>
...
class SMITH_API Cast : public ExprNode<Cast> {
 public:
...
class SMITH_API BitCast : public ExprNode<BitCast> {
 public:
  ExprPtr src_value() const {
    return src_value_;
  }
  ...
  BitCast(Dtype dtype, ExprPtr src_value)
    ...
    SMITH_CHECK(src_value_->dtype().byte_size() == dtype.byte_size());
  }
...
#define IMM_DECLARE(Type, Name)                               \
  class SMITH_API Name##Imm : public ExprNode<Name##Imm> {    \
   public:                                                    \
    Name##Imm(Type value)                                     \
...
SMITH_API bool immediateIsNegative(const ExprPtr& e);

SMITH_API bool immediateIsPositive(const ExprPtr& e);

SMITH_API bool immediateIsZero(const ExprPtr& e);

// Represents a ramp vector node:
//     [base, base + 1 * stride, ... , base + (lanes - 1) * stride]
class SMITH_API Ramp : public ExprNode<Ramp> {
 public:
  ExprPtr base() const {
    return base_;
  }
...
class SMITH_API Load : public ExprNode<Load> {
 public:
  ...
  ExprPtr flat_index() const {
    SMITH_CHECK(indices_.size() == 1, "Indices haven't been flattened.");
    return indices_[0];
  }
  BufPtr buf() const {
    return buf_;
...
class SMITH_API Broadcast : public ExprNode<Broadcast> {
 public:
  ExprPtr value() const {
    return value_;
  }
...
class SMITH_API IfThenElse : public ExprNode<IfThenElse> {
 public:
...
class SMITH_API CompareSelect : public ExprNode<CompareSelect> {
 public:
...
class SMITH_API Intrinsics : public ExprNode<Intrinsics> {
 public:
...
SMITH_API std::vector<ExprPtr> ExprHandleVectorToExprVector(
    const std::vector<ExprHandle>& /*v*/);
SMITH_API std::vector<ExprHandle> ExprVectorToExprHandleVector(
    const std::vector<ExprPtr>& /*v*/);
SMITH_API std::vector<VarPtr> VarHandleVectorToVarVector(
    const std::vector<VarHandle>& /*v*/);
SMITH_API std::vector<VarHandle> VarVectorToVarHandleVector(
    const std::vector<VarPtr>& /*v*/);
SMITH_API ExprPtr flatten_index(
