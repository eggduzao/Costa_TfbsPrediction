...
#ifdef USE_CUDA
#include <smith/csrc/jit/tensorexpr/cuda_codegen.h>
#endif
...
#include <smith/csrc/jit/tensorexpr/reduction.h>
...
template <>
struct pybind11::detail::type_caster<smith::jit::tensorexpr::ArgValue>
    : public type_caster_base<smith::jit::tensorexpr::ArgValue> {};
...
static Dtype parsePythonDtype(py::handle obj) {
  if (THPDtype_Check(obj.ptr())) {
    ...
    throw std::runtime_error("expected a smith.dtype instance");
  }
}
...
#else
          throw std::runtime_error("Blacksmith not compiled with LLVM support!");
#endif
        } else if (name == "cuda") {
#ifdef USE_CUDA
          cg = new CudaCodeGen(stmt, args);
#else
          throw std::runtime_error("Blacksmith not compiled with CUDA support!");
#endif
