...
struct SMITH_API SimplifierHashType {
...
namespace std {
template <>
struct hash<smith::jit::tensorexpr::SimplifierHashType> {
  size_t operator()(
      const smith::jit::tensorexpr::SimplifierHashType& k) const noexcept {
    return k._h;
  }
};
...
class SMITH_API HashProvider : public IRVisitor {
 public:
  template <class T>
