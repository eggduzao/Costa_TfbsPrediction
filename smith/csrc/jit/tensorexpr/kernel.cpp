...
#include <smith/csrc/jit/passes/graph_rewrite_helper.h>
#include <smith/csrc/jit/passes/mkldnn_rewrite.h>
#include <smith/csrc/jit/passes/symbolic_shape_runtime_fusion.h>
#include <smith/csrc/jit/tensorexpr/analysis.h>
#include <smith/csrc/jit/tensorexpr/expr.h>
#include <smith/csrc/jit/tensorexpr/graph_opt.h>
#include <smith/csrc/jit/tensorexpr/ir_printer.h>
#include <smith/csrc/jit/tensorexpr/ir_simplifier.h>
#include <smith/csrc/jit/tensorexpr/loopnest.h>
#include <smith/csrc/jit/tensorexpr/loopnest_randomization.h>
#include <smith/csrc/jit/tensorexpr/operators/operators.h>
...
bool fallbackAllowed() {
  static const auto enable_opt =
      c10::utils::get_env("BLACKSMITH_TENSOREXPR_FALLBACK");
...
static int64_t randomTransformsRequested() {
  const auto enable_opt =
      c10::utils::get_env("BLACKSMITH_TENSOREXPR_RANDOM_TRANSFORM_SEED");
  if (!enable_opt.has_value()) {
    return 0;
  }
...
static bool dontUseLLVMFlag() {
  static const auto enable_opt =
      c10::utils::get_env("BLACKSMITH_TENSOREXPR_DONT_USE_LLVM");
  if (!enable_opt) {
    return false;
  }
  return enable_opt == "1";
...
std::optional<at::Device> pickDeviceType(
    const at::ArrayRef<smith::jit::Value*>& inputs) {
...
// nullopt.
static std::optional<TensorInfo> getTensorInfoJit(smith::jit::Value* v) {
...
bool isContiguous(const smith::jit::Value* v, at::MemoryFormat memory_format) {
...
static size_t get_conv_groups_index(const smith::jit::Node* node) {
  switch (node->kind()) {
    case aten::conv2d:
      return 6;
...
// - 3x3 kernel
bool conv2dIsSupportedJit(const smith::jit::Node* node) {
...
bool mkldnnPrepackedConvIsSupportedJit(const smith::jit::Node* node) {
...
bool matmulIsSupported(const smith::jit::Node* node) {
...
ExprHandle TensorExprKernel::constant(const smith::jit::Value* v) {
...
ArgValue TensorExprKernel::toArg(const smith::jit::Value* v) const {
...
std::vector<ExprHandle> TensorExprKernel::sizesFromSymbolicShape(
  ...
  SMITH_INTERNAL_ASSERT(maybe_rank);
...
static std::optional<ScalarType> findDtypeForValue(const smith::jit::Value* v) {
  if (v->type()->kind() == TypeKind::TensorType) {
    auto tt = v->type()->cast<TensorType>();
    if (tt->scalarType()) {
...
Tensor TensorExprKernel::computeValue(const smith::jit::Value* v) {
...
StmtPtr TensorExprKernel::transformLoops(BackendType backendType, StmtPtr st) {
  smith::jit::tensorexpr::LoopNest l(std::move(st), bufOutputs_);
...
void TensorExprKernel::genInputDebugNames() {
  ...
  std::unordered_map<const smith::jit::Value*, std::string> value_to_name;
  for (const smith::jit::Value* input : graph_->inputs()) {
...
std::vector<smith::jit::StrideInput>& TensorExprKernel::getSymbolicStrideDesc(
    const smith::jit::Value* value) {
  SMITH_INTERNAL_ASSERT(symbolic_strides_.count(value));
  return symbolic_strides_[value];
}

std::vector<ExprHandle> TensorExprKernel::getInputStrides(
    const smith::jit::Value* input,
  ...
  for (const auto i : c10::irange(rank)) {
    if (stride_input[i] == smith::jit::StrideInput::S_ONE) {
      inputTensorStrides[i] = LongImm::make(1);
      stride_set[i] = true;
      generated_strides++;
    } else if (stride_input[i] == smith::jit::StrideInput::S_AS_ARG) {
  ...
  while (generated_strides != rank) {
    for (int i = static_cast<int>(rank) - 1; i >= 0; i--) {
      if (stride_input[i] == smith::jit::StrideInput::S_CONT &&
          stride_set[i + 1]) {
    ...
    for (int i = 0; i < static_cast<int>(rank); i++) {
      if (stride_input[i] == smith::jit::StrideInput::S_TRAN_CONT &&
          stride_set[i - 1]) {
        inputTensorStrides[i] =
            inputTensorStrides[i - 1] * inputTensorDims[i - 1];
        stride_set[i] = true;
...
Tensor TensorExprKernel::bindInput(const smith::jit::Value* input) {
  ...
  auto is_symbolic_cont = [](std::vector<smith::jit::StrideInput> desc,
                             const MemoryLayoutPolicy& mem_layout_policy) {
    if (desc.size() == 1) {
      auto mem_layout = (mem_layout_policy == MemoryLayoutPolicy::kContiguous)
          ? smith::jit::StrideInput::TENSOR_CONT
          : smith::jit::StrideInput::TENSOR_CONT_CHANNELS_LAST;
...
Tensor TensorExprKernel::convertSymbolicOutputToCorrectStrides(
  ...
  SMITH_INTERNAL_ASSERT(buf != nullptr);
  SMITH_INTERNAL_ASSERT(tt != nullptr);
  SMITH_INTERNAL_ASSERT(tt->symbolic_sizes().rank() != std::nullopt);

  auto stride_desc = getSymbolicStrideDesc(v);
  SMITH_INTERNAL_ASSERT(stride_desc.size() == 1);
  auto memory_format = (stride_desc[0] == smith::jit::StrideInput::TENSOR_CONT)
      ? at::MemoryFormat::Contiguous
      : at::MemoryFormat::ChannelsLast;
  ...
  SMITH_INTERNAL_ASSERT(
      stride_desc[0] == smith::jit::StrideInput::TENSOR_CONT_CHANNELS_LAST);
...
void TensorExprKernel::bindConstant(const smith::jit::Value* v) {
  auto val = toIValue(v).value();
  if (smith::isCustomClass(val)) {
  ...
  if (has_symbolic_shapes_) {
    ...
    for (size_t i = 0; i < symbolic_shape_inputs_start_pos; ++i) {
      ...
      for (size_t j = 0; j < symbolic_stride.size(); ++j) {
        if (symbolic_stride[j] == smith::jit::StrideInput::S_AS_ARG) {
          VarHandle v("v" + input_name_map_[input], kLong);
...
void TensorExprKernel::deduceMemoryLayoutPolicy() {
  ...
  auto _prefer_symbolic_mem =
      [](const smith::jit::Value* val,
         const std::vector<smith::jit::StrideInput>& stride_desc_vec) {
        SMITH_INTERNAL_ASSERT(!stride_desc_vec.empty());
  ...
  auto _prefer_static_mem = [](const smith::jit::Value* val) {
    // No shape info is present in the graph
    SMITH_INTERNAL_ASSERT(
  ...
  std::vector<smith::jit::Value*> graph_io_tensors;
  std::copy_if(
      graph_->inputs().begin(),
      graph_->inputs().end(),
...
void TensorExprKernel::optimizeOwningGraph() {
  ...
  SMITH_INTERNAL_ASSERT(graph_outputs.size() == _orignal_graph_outputs.size());
...
void TensorExprKernel::compile() {
  ...
  for (auto i : c10::irange(graph_->outputs().size())) {
    ...
    if (has_symbolic_shapes_) {
      ...
      SMITH_INTERNAL_ASSERT(symbolic_strides_.count(output));
      auto stride_desc_vec = symbolic_strides_[output];
      SMITH_INTERNAL_ASSERT(stride_desc_vec.size() == 1);
...
TensorExprKernel::TensorExprKernel(
    ...
    std::unordered_map<
        const smith::jit::Value*,
        std::vector<smith::jit::StrideInput>> symbolic_strides)
    : graph_(subgraph),
      code_(subgraph, ""),
...
void TensorExprKernel::getStaticOutputSizesAndStrides(
  ...
  SMITH_INTERNAL_ASSERT(has_symbolic_shapes_);
  ...
  SMITH_INTERNAL_ASSERT(sizes);
  SMITH_INTERNAL_ASSERT(strides);
  ...
  for (size_t i = 0, e = bufOutputs_.size(); i < e; ++i) {
    ...
    for (auto t : tensorOutputSymbolicSizes_[i]) {
      if (t.AsNode<LongImm>()) {
        ...
        SMITH_INTERNAL_ASSERT(input_pos < inputs.size());
        SMITH_INTERNAL_ASSERT(inputs[input_pos].isInt());
        static_sizes[i].emplace_back(inputs[input_pos].toInt());
      }
    }

    if (tensorOutputStrideDesc_[i] == smith::jit::StrideInput::TENSOR_CONT) {
