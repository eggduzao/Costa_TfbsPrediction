...
#include <smith/csrc/jit/tensorexpr/llvm_jit.h>
...
#include <smith/csrc/jit/tensorexpr/half_support.h>
...
C10_DEFINE_bool(
    smith_jit_llvm_use_fast_intrinsics,
    false,
...
class LLVMCodeGenCallee {
 public:
  LLVMCodeGenCallee(
      std::unique_ptr<llvm::orc::BlacksmithLLVMJIT> jit,
      void* kernelAddress)
      : jit_(std::move(jit)), kernelAddress_(kernelAddress) {}

  llvm::orc::BlacksmithLLVMJIT* getJIT() {
    return jit_.get();
  }
 ...
 private:
  std::unique_ptr<llvm::orc::BlacksmithLLVMJIT> jit_;
  void* kernelAddress_;
};
...
#undef LLVM_TYPE_DECLARE
 ...
 public:
  ...
  std::unique_ptr<llvm::orc::BlacksmithLLVMJIT> releaseJIT();
...
std::unique_ptr<llvm::orc::BlacksmithLLVMJIT> LLVMCodeGenImpl::releaseJIT() {
...
LLVMCodeGenImpl::LLVMCodeGenImpl(
...

  {
    ...
    jit_ = std::make_unique<llvm::orc::BlacksmithLLVMJIT>(triple, cpu, attrs);
  }

  module_ = std::make_unique<llvm::Module>("blacksmith", getContext());
  ...
  fn_ = llvm::Function::Create(
      fntype, llvm::Function::PrivateLinkage, "blacksmith", module_.get());
...
void LLVMCodeGenImpl::emitKernel(
  ...
  if (FLAGS_smith_jit_llvm_use_fast_intrinsics) {
...
void LLVMCodeGenImpl::visit(const BitCastPtr& v) {
  ...
  SMITH_CHECK(llvm::CastInst::isBitCastable(
...
void LLVMCodeGenImpl::replaceVarMapping(
    const std::vector<VarPtr>& vars,
    const std::vector<llvm::Value*>& vals) {
  SMITH_CHECK(vars.size() == vals.size());
...
#undef SIMD_BINARY_MATH_CASE
    ...
    // abs is only intrinsic defined for integer inputs in blacksmith eager
    v->params().front()->accept(this);
