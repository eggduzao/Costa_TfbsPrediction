...
#include <smith/csrc/jit/tensorexpr/intrinsic_symbols.h>
...
class SMITH_API BlacksmithLLVMJITImpl {
 private:
...
BlacksmithLLVMJIT::BlacksmithLLVMJIT(
    std::optional<std::string> triple,
    std::optional<std::string> cpu,
    std::optional<std::string> attrs)
    : impl_(std::make_unique<BlacksmithLLVMJITImpl>(triple, cpu, attrs)) {}

BlacksmithLLVMJIT::~BlacksmithLLVMJIT() = default;

void BlacksmithLLVMJIT::addModule(
    std::unique_ptr<Module> M,
    std::unique_ptr<LLVMContext> C) {
  impl_->addModule(std::move(M), std::move(C));
}

JITSymbol BlacksmithLLVMJIT::findSymbol(const std::string Name) {
  return impl_->findSymbol(std::move(Name));
}

bool BlacksmithLLVMJIT::hasSymbol(const std::string& Name) {
  return impl_->hasSymbol(Name);
}

TargetMachine& BlacksmithLLVMJIT::getTargetMachine() {
  return impl_->getTargetMachine();
}

const DataLayout& BlacksmithLLVMJIT::getDataLayout() {
...
void dumpCFG(const llvm::cfg::Update<llvm::BasicBlock*>& update) {
  ...
  // link a debug-mode blacksmith with an opt-mode llvm, the symbol is undefined.
  update.dump();
}
#endif
