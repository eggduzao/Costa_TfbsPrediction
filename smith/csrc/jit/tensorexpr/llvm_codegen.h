...
class SMITH_API LLVMCodeGen : public CodeGen {
 public:
  ...
  SMITH_API void call(const std::vector<CallArg>& args) override;
  SMITH_API void call_raw(const std::vector<void*>& args) override;
  SMITH_API void call_with_numel(void** args, int64_t numel) override;
...
struct SMITH_API LLVMCodeGenBuilder {
...
SMITH_API std::optional<std::string>& LLVMTargetTriple();
SMITH_API std::optional<std::string>& LLVMTargetCPU();
SMITH_API std::optional<std::string>& LLVMTargetAttrs();
SMITH_API bool& LLVMAOTWorkflow();
