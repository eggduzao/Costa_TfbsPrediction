...
SMITH_API void annotateInputShapes(
    const std::shared_ptr<Graph>& graph,
    const std::vector<std::optional<at::Tensor>>& example_inputs);
SMITH_API std::shared_ptr<Graph> removeUnusedSelfArgument(
    const std::shared_ptr<Graph>& graph);
SMITH_API std::shared_ptr<Graph> removeGraphOutput(
    const std::shared_ptr<Graph>& graph,
    size_t idx);
SMITH_API std::shared_ptr<Graph> replaceListOutputWithTuple(
    const std::shared_ptr<Graph>& graph);
...
SMITH_API std::shared_ptr<Graph> trimGraph(
    const std::shared_ptr<Graph>& graph,
    int64_t iters);
...
SMITH_API std::vector<int64_t> makeShapesSymbolic(
...
SMITH_API bool isGraphCompilable(const std::shared_ptr<Graph>& graph);
...
SMITH_API void fixupMissingShapeInfo(const std::shared_ptr<Graph>& graph);
