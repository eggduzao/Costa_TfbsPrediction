...
T assertSuccess(llvm::Expected<T> valOrErr, const char* msg = nullptr) {
  SMITH_INTERNAL_ASSERT(valOrErr, formatError(valOrErr.takeError(), msg));
  return std::move(*valOrErr);
}

inline void assertSuccess(llvm::Error err, const char* msg = nullptr) {
  SMITH_INTERNAL_ASSERT(!err, formatError(std::move(err), msg));
}
...
class BlacksmithLLVMJITImpl;

class SMITH_API BlacksmithLLVMJIT {
 public:
  BlacksmithLLVMJIT(
 ...
 private:
  ...
  std::unique_ptr<BlacksmithLLVMJITImpl> impl_;
};
