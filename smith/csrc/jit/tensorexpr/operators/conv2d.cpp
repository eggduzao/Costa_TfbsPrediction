...
#include <smith/csrc/jit/tensorexpr/operators/conv2d.h>
...
void assert_dims_constant(const BufHandle& buf) {
  for (auto const& dim : buf.node()->dims()) {
    SMITH_INTERNAL_ASSERT(dim->isConstant());
  }
}
...
Tensor conv2d_depthwise_static(
    ...
    int pad,
    int groups) {
  SMITH_INTERNAL_ASSERT(input.ndim() == 4);
  SMITH_INTERNAL_ASSERT(weight.ndim() == 4);
  ...
  SMITH_INTERNAL_ASSERT(C == K && K == groups && CperG == 1);
  SMITH_INTERNAL_ASSERT(R == S);
