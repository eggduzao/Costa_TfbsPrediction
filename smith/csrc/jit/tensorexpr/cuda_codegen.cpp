...
#include <smith/csrc/jit/codegen/fuser/cuda/fused_kernel.h>
...
#include <smith/csrc/jit/tensorexpr/cuda_random.h>
...
#include <smith/csrc/jit/tensorexpr/registerizer.h>
...
StmtPtr GPUMetaVarRewriter::mutate(const BlockPtr& v) {
  ...
  for (auto& segment : innerSegments) {
    ...
    if (!segment.mask()) {
      SMITH_INTERNAL_ASSERT(segment.stmts().size() == 1, buildErrorMessage());
