...
void nnc_aten_conv2d(
  ...
  if (args_num > 0) {
    ...
    // also present
    SMITH_INTERNAL_ASSERT(args_num == 7 && bufs_num == 4);
...
void nnc_aten_quantized_add(
    ...
    int64_t /*unused*/,
    int64_t* extra_args) {
  // SMITH_INTERNAL_ASSERT(tensors.size() == 3);
...
void nnc_aten_conv1d(
  ...
  if (args_num > 0) {
    // Check that if the extra arguments are provided, then the bias tensor is
    // also present
    SMITH_INTERNAL_ASSERT(args_num == 4 && bufs_num == 4);
    const at::Tensor& b = tensors[3];
...
void nnc_aten_conv1d_out(
  ...
  if (args_num > 0) {
    // Check that if the extra arguments are provided, then the bias tensor is
    // also present
    SMITH_INTERNAL_ASSERT(args_num == 4 && bufs_in_num == 3);
    const at::Tensor& b = tensors[3];
