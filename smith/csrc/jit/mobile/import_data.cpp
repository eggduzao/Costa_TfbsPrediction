...
#include <smith/csrc/jit/mobile/import_export_common.h>
...
class IValueUnpickler final {
 public:
  explicit IValueUnpickler(std::unique_ptr<BlacksmithStreamReader> reader);
...
IValueUnpickler::IValueUnpickler(std::unique_ptr<BlacksmithStreamReader> reader)
...
c10::IValue IValueUnpickler::readArchive(
  ...
  auto type_resolver = [&](const c10::QualifiedName& qn) {
    TypePtr type;
    // HACK: first we check whether the name starts with `__smith__` to tell if
