...
static constexpr std::string_view kSmithPrefix = "__smith__";
static constexpr std::string_view kJitPrefix = "smith.jit";
...
class FlatbufferLoader final {
 public:
  ...
  IValue& getIValue(uint32_t pos) {
    SMITH_CHECK(pos < all_ivalues_.size());
  ...
  ClassTypePtr getType(uint32_t pos) {
    SMITH_CHECK(pos < all_types_.size());
...
TypePtr resolveType(
  ...
  if (c10::starts_with(type_str, kCustomClassPrefix)) {
    ...
    SMITH_CHECK(
      ...
      c10::starts_with(type_str, kSmithPrefix) ||
...
void parseExtraFilesFromVector(
    const flatbuffers::Vector<flatbuffers::Offset<
        smith::jit::mobile::serialization::ExtraFile>>* files,
...
std::vector<T> parseListNative(const U* list) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(list != nullptr);
...
ClassTypePtr FlatbufferLoader::getOrCreateClassTypeForObject(
  ...
  if (cls == nullptr) {
    ...
    if (c10::starts_with(qn_str, kSmithPrefix) ||
    ...
    SMITH_CHECK(object->type_index() < all_ivalues_.size());
...
IValue parseObject(
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(object != nullptr);
  ...
  switch (obj_type->type()) {
    ...
    case mobile::serialization::TypeType::CUSTOM_CLASS: {
      auto custom_class_type =
          smith::jit::getCustomClass(cls->name()->qualifiedName());
      IValue input = loader.getIValue(object->state());
      auto obj = c10::ivalue::Object::create(
...
c10::Storage FlatbufferLoader::getStorage(uint32_t index) {
  SMITH_CHECK(index < storage_loaded_.size());
  SMITH_CHECK(index < storages_.size());
