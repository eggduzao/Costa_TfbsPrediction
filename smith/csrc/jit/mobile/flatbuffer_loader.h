 ...
 * types, to avoid leaking those details to Blacksmith clients.
 */
...
SMITH_API mobile::Module parse_and_initialize_mobile_module(
    void* data,
...
// jit::Module.
SMITH_API mobile::Module parse_and_initialize_mobile_module_for_jit(
    void* data,
    size_t size, // of `data`, in bytes.
...
// directly.
SMITH_API mobile::Module load_mobile_module_from_file(
    const std::string& filename,
    std::optional<at::Device> device = std::nullopt,
    ExtraFilesMap* extra_files = nullptr);

SMITH_API uint64_t get_bytecode_version(std::istream& in);
SMITH_API uint64_t get_bytecode_version(const std::string& filename);
SMITH_API uint64_t get_bytecode_version_from_bytes(char* flatbuffer_content);

SMITH_API mobile::ModuleInfo get_module_info_from_flatbuffer(
...
SMITH_API mobile::Module load_mobile_module_from_stream_with_copy(
    std::istream& in,
    std::optional<at::Device> device = std::nullopt,
    ExtraFilesMap* extra_files = nullptr);

SMITH_API mobile::Module parse_flatbuffer_no_object(
    std::shared_ptr<char> data,
    size_t size,
    std::optional<at::Device> device);

// no op, TODO(qihan) delete
SMITH_API bool register_flatbuffer_loader();
