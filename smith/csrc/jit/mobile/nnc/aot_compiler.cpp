#include <smith/csrc/jit/mobile/nnc/aot_compiler.h>

#include <ATen/Functions.h>
#include <ATen/NativeFunctions.h>
#include <smith/csrc/jit/backends/backend.h>
#include <smith/csrc/jit/backends/backend_detail.h>
...
namespace smith::jit::mobile::nnc {
...
static std::vector<mobile::nnc::InputSpec> toInputSpecs(
  ...
  for (const auto i : c10::irange(num_inputs)) {
    ...
    SMITH_CHECK(t->kind() == TypeKind::TensorType, "Unsupported input type");
...
static std::unique_ptr<Function> compileMethod(
  ...
  for (int64_t idx = n_inputs; idx < n_inputs + n_outputs; idx++) {
    ...
    if (isQIntType(output.dtype_)) {
      ...
      output.qscale_ =
          to<DoubleImm>(smith::jit::tensorexpr::IRSimplifier::simplify(
                            ba.buf()->qscale()))
              ->value();
      output.qzero_ =
          to<LongImm>(
              smith::jit::tensorexpr::IRSimplifier::simplify(ba.buf()->qzero()))
              ->value();
    }
...
static std::pair<std::unique_ptr<Function>, const std::string> aotCompile(
  ...
  std::shared_ptr<tensorexpr::TensorExprKernel> kernel;
  std::vector<smith::jit::StrideInput> stride_desc = {
...
preprocessGraphPasses(
  ...
  smith::jit::RemoveTensorMutation(graph);
  smith::jit::EliminateDeadCode(graph->block());
  graph = smith::jit::tensorexpr::removeUnusedSelfArgument(graph);

  smith::jit::tensorexpr::annotateInputShapes(graph, example_inputs);
  smith::jit::OptimizeFrozenGraph(graph, true);
  smith::jit::PropagateShapesOnGraph(graph);
  smith::jit::PeepholeOptimize(graph, false);
  smith::jit::ConstantPropagation(graph);
  smith::jit::PropagateShapesOnGraph(graph);
  smith::jit::PeepholeOptimize(graph, false);
  smith::jit::ConstantPropagation(graph);
  ...
  auto sym_val =
      smith::jit::tensorexpr::makeShapesSymbolic(graph, dynamic_sizes);
...
static c10::IValue preprocess(
  ...
  smith::jit::mobile::nnc::CompilationUnit cu;
  for (const auto& kv : compile_spec) {
    ...
    auto compiled = smith::jit::mobile::nnc::aotCompile(
        method_name,
        processed_graph,
        sizes,
        types,
...
// static auto reg = smith::jit::backend_preprocess_register("nnc", preprocess);
