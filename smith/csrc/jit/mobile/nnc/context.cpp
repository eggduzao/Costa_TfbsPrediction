#include <smith/csrc/jit/mobile/nnc/context.h>
...
#include <smith/csrc/jit/mobile/nnc/registry.h>
...
void Function::init_execution_state() const {
  ...
  for (const auto& param : parameters_) {
    ...
    } else if (smith::isCustomClass(ivalue)) {
      arguments.emplace_back(ivalue.toObjectRef().getSlot(0).toCapsule().get());
    } else {
      SMITH_CHECK(false, "Invalid parameter: ", ivalue);
    }
  }
...
c10::impl::GenericList Function::run(
  ...
  for (const auto i : c10::irange(inputs.size())) {
    ...
    SMITH_CHECK(spec.validate(input_tensor), "Invalid input at pos: ", i);
