...
#include <smith/csrc/jit/frontend/parser_constants.h>
#include <smith/custom_class.h>
#include <string_view>

using smith::jit::valid_single_char_tokens;

namespace c10 {

namespace {

// Smithbind custom class always starts with the follow prefix, so use it as
// an identifier for smithbind custom class type
static constexpr const char* kTypeSmithbindCustomClass =
...
const std::unordered_set<std::string>& TypeParser::getCustomType() {
  static std::unordered_set<std::string> customeTypes{
      kTypeSmithbindCustomClass, kTypeNamedTuple};
  return customeTypes;
}
...
TypePtr TypeParser::parse() {
  ...
  } else if (token == "__smith__") {
    expectChar('.');
    if (cur() == "smith") {
      // smith bind class starts with __smith__.smith.classes
      return parseSmithbindClassType();
    } else {
      // other class starts with __smith__ following by custom names
...
TypePtr TypeParser::parseCustomType() {
  ...
  std::string qualified_name = "__smith__.";
...
TypePtr TypeParser::parseSmithbindClassType() {
  ...
  std::string customClassName = "__smith__.smith.classes.";
  ...
  return smith::getCustomClass(customClassName);
}
...
SMITH_API at::TypePtr parseType(const std::string& pythonStr) {
  at::TypeParser parser(pythonStr);
  return parser.parse();
}

SMITH_API std::vector<at::TypePtr> parseType(
