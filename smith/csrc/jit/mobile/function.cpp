...
#include <smith/csrc/jit/mobile/prim_ops_registery.h>
...
bool Function::initialize_operators(bool should_check_operators) {
  ...
  if (should_check_operators) {
    SMITH_CHECK(
        ...
        "]. Please check if the operator library is included in the build. If built with selected ops, check if these ops are in the list. If you are a Meta employee, please see fburl.com/missing_ops for a fix. Or post it in https://discuss.blacksmith.org/c/mobile/");
  }
...
smith::jit::Function& Function::setSchema(c10::FunctionSchema schema) {
  schema_ = std::move(schema);
  return *this;
}
...
std::optional<std::function<void(Stack&)>> makeOperatorFunction(
    ...
    if (jit_op) {
      ...
      if (op.has_value()) {
        ...
        if (op->hasSchema()) {
          ...
          SMITH_CHECK(false, "arguments are missing for operator ", opname);
        }
      } else {
        return std::nullopt;
      }
  ...
  if (!promoted_op) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(pArgs);
