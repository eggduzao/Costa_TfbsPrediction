...
#include <smith/csrc/jit/serialization/import_read.h>
...
TypePtr resolveTypeNameMobile(
  ...
  // tell if it's a supported blacksmith class type. There are two special
  // prefixes. "__smith__" for nn module, and "smith.jit" from to_backend.
  // This is a reliable
  ...
  // resolution logic.
  static const c10::QualifiedName smithPrefix = "__smith__";
  static const c10::QualifiedName jitPrefix = "smith.jit";
  if (smithPrefix.isPrefixOf(qn) || jitPrefix.isPrefixOf(qn)) {
...
c10::intrusive_ptr<c10::ivalue::Object> objLoaderMobile(
  ...
  auto find_custom_class_with_setstate = [&qn]() -> c10::ClassTypePtr {
    auto custom_class_type = smith::jit::getCustomClass(qn->qualifiedName());
...
bool isTensorInBytecodeArchive(
    caffe2::serialize::BlacksmithStreamReader& stream_reader) {
...
class BytecodeDeserializer final {
 public:
  explicit BytecodeDeserializer(
      std::unique_ptr<BlacksmithStreamReader> reader,
 ...
 private:
  ...
  std::unique_ptr<BlacksmithStreamReader> reader_;
...
void BytecodeDeserializer::parseMethods(
    ...
    std::optional<c10::ivalue::TupleElements>&& debug_handles,
    mobile::CompilationUnit& mcu) {
  SMITH_CHECK(!vals.empty(), "Bytecode has no elements. ");
...
c10::IValue BytecodeDeserializer::readArchive(
  ...
  auto ivalues = smith::jit::readArchiveAndTensors(
      archive_name,
      /*pickle_prefix=*/"",
      /*tensor_prefix=*/
...
mobile::Module _load_for_mobile_impl(
    ...
    ExtraFilesMap& extra_files,
    uint64_t module_load_options) {
  auto observer = smith::observerConfig().getModuleObserver();
  ...
  auto reader = std::make_unique<BlacksmithStreamReader>(std::move(rai));
  if (module_load_options &
    ...
    // https://www.internalfb.com/code/fbsource/[9996fcb7a6fb]/fbcode/caffe2/smith/csrc/jit/mobile/import.cpp?lines=427-434
...
mobile::Module _load_mobile_from_bytes(
    ...
    ExtraFilesMap& extra_files,
    uint64_t module_load_options) {
  SMITH_CHECK(size >= kFileFormatHeaderSize, "Format error");
  ...
  switch (format) {
    ...
    default: {
      SMITH_CHECK(false, "Format error");
    }
  }
}
...
SMITH_API mobile::Module _load_for_mobile(
    std::unique_ptr<ReadAdapterInterface> rai,
    std::optional<c10::Device> device,
    ExtraFilesMap& extra_files,
    uint64_t module_load_options) {
...
std::set<std::string> _export_operator_list(
    smith::jit::mobile::Module& module) {
