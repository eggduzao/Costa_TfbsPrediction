...
void selective_copy(
  ...
  for (const auto& record : records) {
    ...
    // `bytecode`. Archive `version` will be written when BlacksmithStreamWriter is
    ...
    // data.pkl
    // code/__smith__/___smith_mangle_5.py
    // code/__smith__/___smith_mangle_5.py.debug_pkl
...
std::stringstream update_bytecode_version(
    ...
    const int64_t to_version) {
  BlacksmithStreamReader reader_bytecode(&input_model);
  ...
  BlacksmithStreamWriter writer_bytecode(writer_func);
...
/*
 ...
 input_model_stream, modify the file, and use BlacksmithWriter to write it to
 output_model_stream. See backport_v5_to_v4.

 2) [Both format and content change] ]Use smith.jit.load() to load the stream,
...
(smith._utils._rebuild_tensor_v2(pers.obj(('storage', smith.DoubleStorage,
'0', 'cpu', 8),),
   0,
   (2, 4),
   (4, 1),
 ...
 1)),
 So, if the program defines: smith.add(x, h, out=x)
...
std::stringstream backport_v5_to_v4(std::stringstream& input_model_stream) {
  ...
  BlacksmithStreamReader reader(&input_model_stream);
  ...
  auto writeArchiveV4 = [](BlacksmithStreamWriter& writer,
                           const std::string& archive_name,
...
std::stringstream backport_v6_to_v5(std::stringstream& input_model_stream) {
  ...
  auto reader = std::make_shared<BlacksmithStreamReader>(rai);
  ...
  Module smith_script = smith::jit::load(rai, std::nullopt, extra_files);
...
For example schema is: smith.add(x, h, alpha=1.0, out=x) So, if the program
defines: smith.add(x, h, out=x) Previously, in v6, we serialized the bytecode to
...
std::stringstream backport_v9_to_v8(std::stringstream& input_model_stream) {
  ExtraFilesMap extra_files;
  Module smith_script =
      smith::jit::load(input_model_stream, std::nullopt, extra_files);
  std::stringstream intermediate_model_stream;
...
// Args:
// * BlacksmithStreamReader has access to the input model from N bytecode version.
// * BlacksmithStreamWriter has access to the output model backported to the
...
bool BackportManager::backport(
    std::istream& oss,
    BlacksmithStreamWriter& final_writer,
  ...
  if (output_model_stream.str().empty()) {
    SMITH_WARN("No output model from backport.");
    return false;
  }
  BlacksmithStreamReader last_model_reader(&output_model_stream);
  selective_copy(
      last_model_reader,
      final_writer,
