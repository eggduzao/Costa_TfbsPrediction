...
SMITH_API uint64_t _get_model_bytecode_version(std::istream& in);

SMITH_API uint64_t _get_model_bytecode_version(const std::string& filename);

SMITH_API uint64_t _get_model_bytecode_version(
...
// Throws if not passed in a well formed model
SMITH_API uint64_t _get_model_operator_version(std::istream& in);

SMITH_API uint64_t _get_model_operator_version(const std::string& filename);

SMITH_API uint64_t _get_model_operator_version(
...
SMITH_API std::unordered_map<std::string, OperatorInfo> _get_model_ops_and_info(
    std::istream& in);
...
// Throws if not passed in a well formed model
SMITH_API std::unordered_set<std::string> _get_mobile_model_contained_types(
    std::istream& in);
...
struct ModelCompatibilityInfo {
  ...
  // Factory Methods
  static SMITH_API ModelCompatibilityInfo get(std::istream& in);
  static SMITH_API ModelCompatibilityInfo get(const std::string& filename);
  static SMITH_API ModelCompatibilityInfo
...
SMITH_API ModelCompatCheckResult is_compatible(
