...
#include <smith/csrc/jit/api/compilation_unit.h> // removed after using simple type_resolver/obj_loader
...
#include <smith/csrc/jit/mobile/import.h> // removed after using simple type_resolver/obj_loader
...
c10::IValue readArchive(
    ...
    BlacksmithStreamReader& stream_reader) {
...
std::vector<IValue> get_bytecode_ivalues(BlacksmithStreamReader& reader) {
...
static uint64_t _get_model_bytecode_version_zip(
  ...
  BlacksmithStreamReader reader(std::move(rai));
...
uint64_t _get_model_bytecode_version_from_bytes(char* data, size_t size) {
  SMITH_CHECK(data != nullptr, "Pointer to bytes is null.");
...
uint64_t _get_model_bytecode_version(
  ...
  SMITH_CHECK(false, "Failed to get bytecode version.");
}

/********************** Operator Version **********************/

static uint64_t _get_model_operator_version(
    BlacksmithStreamReader& reader); // Forward Declare
...
uint64_t _get_model_operator_version(BlacksmithStreamReader& reader) {
...
std::unordered_map<std::string, OperatorInfo> _get_model_ops_and_info(
    std::shared_ptr<ReadAdapterInterface> rai) {
  if (!check_zip_file(rai)) {
    SMITH_WARN("Failed to open zip file for model ops.");
...
std::unordered_map<std::string, OperatorInfo> _get_model_ops_and_info(
  ...
  if (bytecode_ivalues.empty()) {
    SMITH_WARN("Failed to get model ops and info.");
    return result;
  }
