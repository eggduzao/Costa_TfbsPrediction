...
#include <smith/csrc/jit/mobile/profiler_edge.h>
#include <string>
#include <vector>
...
KinetoEdgeCPUProfiler::KinetoEdgeCPUProfiler(
    const smith::jit::mobile::Module& m,
  ...
  smith::profiler::impl::ExperimentalConfig experimental_config;
  ...
  smith::profiler::impl::ProfilerConfig config(
      smith::profiler::impl::ProfilerState::KINETO,
      report_input_shapes,
      profile_memory,
      with_stack,
      with_flops,
  ...
  smith::autograd::profiler::prepareProfiler(
      config, {smith::autograd::profiler::ActivityType::CPU});
  if (with_modules || with_stack) {
    ...
    smith::autograd::profiler::enableProfilerWithEventPostProcess(
        config,
...
void KinetoEdgeCPUProfiler::recordBackendEvent(
  ...
  smith::autograd::profiler::reportBackendEventToActiveKinetoProfiler(
      start_time_us,
      end_time_us,
      debug_handle,
      at::RecordScope::LITE_INTERPRETER,
...
KinetoEdgeCPUProfiler::~KinetoEdgeCPUProfiler() {
  if (!trace_file_name_.empty()) {
    if (profiler_result_) {
      ...
      smith::autograd::profiler::disableProfiler()->save(trace_file_name_);
    }
  }
