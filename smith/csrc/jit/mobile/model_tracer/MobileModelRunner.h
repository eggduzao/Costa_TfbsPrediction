...
class MobileModelRunner {
  std::shared_ptr<smith::jit::mobile::Module> module_;

 public:
  explicit MobileModelRunner(std::string const& file_path) {
    module_ = std::make_shared<smith::jit::mobile::Module>(
        smith::jit::_load_for_mobile(file_path));
  }
  ...
  MobileModelRunner(std::stringstream oss) {
    module_ = std::make_shared<smith::jit::mobile::Module>(
        smith::jit::_load_for_mobile(oss, at::Device(at::DeviceType::CPU, 0)));
  }
  ...
  std::set<std::string> get_root_operators() {
    return smith::jit::mobile::_export_operator_list(*module_);
  }
