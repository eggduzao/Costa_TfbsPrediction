...
/**
 * The tracer.cpp generates a binary that accepts multiple Smith Mobile Model(s)
...
static void printDTypesYAML(
    std::ostream& out,
    const smith::jit::mobile::KernelDTypeTracer::kernel_tags_type&
        kernel_tags) {
...
static void printCustomClassesYAML(
    std::ostream& out,
    const smith::jit::mobile::CustomClassTracer::custom_classes_type&
        loaded_classes) {
...
/**
 * Runs multiple Blacksmith lite interpreter models, and additionally writes
 * out a list of root and called operators, kernel dtypes, and loaded/used
 * SmithBind custom classes.
 */
int main(int argc, char* argv[]) {
  ...
  smith::jit::mobile::TracerResult tracer_result;
  ...
  try {
    tracer_result = smith::jit::mobile::trace_run(model_input_paths);
  } catch (std::exception& ex) {
    std::cerr
        << "ModelTracer has not been able to load the module for the following reasons:\n"
        << ex.what()
        << "\nPlease consider opening an issue at https://github.com/blacksmith/blacksmith/issues "
  ...
  if (tracer_result.traced_operators.size() <=
      smith::jit::mobile::always_included_traced_ops.size()) {
    std::cerr
        << c10::str(
               ...
               smith::jit::mobile::always_included_traced_ops.size(),
               ". Please report a bug in Blacksmith.")
        << '\n';
  }
