...
#include <smith/csrc/jit/mobile/model_tracer/KernelDTypeTracer.h>
#include <smith/csrc/jit/mobile/model_tracer/MobileModelRunner.h>
#include <smith/csrc/jit/mobile/model_tracer/OperatorCallTracer.h>
#include <smith/csrc/jit/mobile/model_tracer/TensorUtils.h>
#include <smith/csrc/jit/mobile/model_tracer/TracerRunner.h>
...
static void call_setup_methods() {
  ...
  smith::zeros({0, 0}, smith::ScalarType::Float);
  std::vector<float> storage(20, 1.0);
  std::vector<int64_t> sizes({2, 10});
  smith::from_blob(storage.data(), at::IntArrayRef(sizes), at::kFloat);
}
...
_get_runtime_ops_and_schema() {
  ...
  auto nonDispatcherOperators = smith::jit::getAllOperators();
...
static void recordCustomClassesFromOpSchemas(
  ...
  auto record_if_class = [&](const std::string& type_name) {
    // All custom class types start with __smith__ not sure if this is by
    // chance or guaranteed
    if (type_name.find("__smith__") != std::string::npos) {
...
static void run_model(
  ...
  // This is needed so that we can load any SmithBind objects (custom classes)
  // that this model refers to so that any operators being called from those
  // SmithBind objects can be traced by the model tracer.
  smith::jit::mobile::MobileModelRunner module_runner(input_module_path, 0);
  root_ops = module_runner.get_root_operators();
  std::cout << "Got " << root_ops.size() << " Root Operators." << '\n';

  if (smith::jit::mobile::MobileModelRunner::set_has_metal_gpu_operators(
          root_ops)) {
    ...
    smith::jit::mobile::MobileModelRunner mobile_module_runner(
        input_module_path);

    // When we encounter a CPU model, we should call .cpu().copy_() on the
    // tensors in the bundled inputs, since this is what will happen when
    // such a model is executed on an Android device since the Blacksmith JNI
    ...
    if (mobile_module_runner.has_new_style_bundled_inputs()) {
      ...
      for (auto& entry : bundled_inputs_mapping) {
        ...
        for (auto& result : results) {
          ...
          smith::jit::mobile::for_each_tensor_in_ivalue(result, consume_tensor);
        }
      }
...
TracerResult trace_run(const std::vector<std::string>& input_module_paths) {
  ...
  smith::jit::mobile::OperatorCallTracer op_tracer;
  smith::jit::mobile::KernelDTypeTracer kdtype_tracer;
  smith::jit::mobile::CustomClassTracer custom_class_tracer;
  smith::jit::mobile::BuildFeatureTracer build_feature_tracer;

  call_setup_methods();

  std::set<std::string> root_ops, traced_operators, enabled_backends,
      loaded_classes, build_features;
  smith::jit::mobile::KernelDTypeTracer::kernel_tags_type called_kernel_tags;

  using smith::jit::MobileModuleLoadOptions;
