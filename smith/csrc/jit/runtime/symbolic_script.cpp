#include <smith/csrc/jit/runtime/symbolic_script.h>
...
const std::vector<std::string> functions = {
    R"(
        ...
        def mean_0(self, *, dtype: Optional[int]):
            ...
            return smith.mean(self, dtype=dtype), backward
        ...
        def mean_1(self,
            ...
            return smith.mean(self, dim, keepdim, dtype=dtype), backward

        def logsumexp(self,
                      dim: List[int],
                      keepdim: bool):
            result = smith.logsumexp(self, dim, keepdim)
            self_dim = self.dim()
            def backward(grad_output):
        ...
        def AD_var_backward_0(grad, self, correction: number):
            # FIXME: smithscript: div(float, float)
        ...
        def std_0(self,
                  unbiased: bool=True):
            std_out = smith.std(self, unbiased)
            def backward(grad_output):
        ...
        def std_1(self,
                  ...
                  unbiased: bool,
                  keepdim: bool):
            std_out = smith.std(self, dim, unbiased, keepdim)
            def backward(grad_output):
                correction = AD_bool_to_int(unbiased)
        ...
        def std_2(self,
                  ...
                  *,
                  correction: Optional[number],
                  keepdim: bool):
            std_out = smith.std(self, dim, correction=correction, keepdim=keepdim)
            def backward(grad_output):
                grad_self = AD_var_backward_2(grad_output / (std_out * 2), self, dim, correction, keepdim)
                return grad_self, None, None, None
        ...
        def var_0(self,
            ...
            return smith.var(self, unbiased), backward
        ...
        def var_1(self,
            ...
            return smith.var(self, dim, unbiased, keepdim), backward
        ...
        def var_2(self,
            ...
            return smith.var(self, dim, correction=correction, keepdim=keepdim), backward
        ...
        def AD_index_select_backward(grad,
            ...
            # FIXME: smithscript: smith.zeros(sizes, grad.options())
            return smith.zeros(sizes).to(grad).scatter_(dim, indices, grad)
        ...
        #          largest: bool = True,
        #          sorted: bool = True):
        #     result0, result1 = smith.topk(self, k, dim, largest, sorted)
        #     self_size = self.size()
        #     def backward(grad_output):
        ...
        #              dim: int,
        #              keepdim: bool):
        #     result0, result1 = smith.kthvalue(self, k, dim, keepdim)
        #     self_size = self.size()
        #     def backward(grad_output):
        ...
        def mm(self, mat2):
            ...
            return smith.mm(self, mat2), backward
        ...
        def permute(self,
            ...
            return smith.permute(self, dims), backward
        ...
        def AD_select_backward(grad,
                               ...
                               index: int):
            # FIXME: smithscript: smith.zeros(sizes, grad.options())
            grad_input = smith.zeros(input_sizes).to(grad)
            grad_input.select(dim, index).copy_(grad)
            return grad_input

        # TODO: fix smith.zeros(sizes, grad.options()) before enabling select, topk, kthvalue
        # def select(self,
        #            dim: int,
        #            index: int):
        #     self_size = self.size()
        ...
        #     return smith.select(self, dim, index), backward
        ...
        #     return smith.slice(self, dim, start, end, step), backward
        ...
        def squeeze_0(self):
            ...
            return smith.squeeze(self), backward
        ...
        def squeeze_1(self,
            ...
            return smith.squeeze(self, dim), backward
        ...
        def bmm(self, mat2):
            def backward(grad_output):
                ...
                return grad_self, grad_mat2
            return smith.bmm(self, mat2), backward
    )",
    R"(
        ...
        def AD_matmul_bw_size(mat1, mat2,
            ...
            if dim1 == 0 or dim2 == 0:
                ...
                if dim2 == 1:
                    ...
                    target_dim2 = -2
                out = smith.matmul(mat1.unsqueeze(dim1), mat2.unsqueeze(target_dim2))
            elif dim_out == dim1 - dim2:
                out = smith.matmul(mat1, mat2.unsqueeze(dim2)).squeeze(-1)
            elif dim_out == dim2 - dim1:
                out = smith.matmul(mat1.unsqueeze(-2), mat2).squeeze(-2)
            else:
                out = smith.matmul(mat1, mat2)
            return out
        ...
        def matmul(self, other):
            ...
            return smith.matmul(self, other), backward

        def linear(input : Tensor,
                   weight : Tensor,
                   bias : Optional[Tensor]):
            result = smith.linear(input, weight, bias)
            ...
            def backward(grad_output):
                ...
                grad_input = smith.matmul(grad_output, weight)
                grad_weight = smith.matmul(grad_output.reshape(-1, weight_size[0]).t(), input.reshape(-1, weight_size[1]))
    ...
    R"(
        def addcmul(self,
                    ...
                    *,
                    value: number):
            result = smith.addcmul(self, tensor1, tensor2, value=value)
            self_size = smith._size_if_not_equal(self.size(), result.size())
            tensor1_size = smith._size_if_not_equal(tensor1.size(), result.size())
            tensor2_size = smith._size_if_not_equal(tensor2.size(), result.size())
            def backward(grad_output):
        ...
        def _autocast_to_full_precision(self, cuda_enabled : bool, cpu_enabled : bool):
            ...
            return smith._autocast_to_full_precision(self, cuda_enabled, cpu_enabled), backward
        ...
        def _autocast_to_reduced_precision(self,
            ...
            return smith._autocast_to_reduced_precision(self, cuda_enabled, cpu_enabled, cuda_dtype, cpu_dtype), backward
        ...
        def _dim_arange(like,
            ...
            return smith._dim_arange(like, dim), backward
        ...
        def dot(self, tensor):
            ...
            return smith.dot(self, tensor), backward

        def erf(self):
            def backward(grad_output):
                # Precomputed constant C = 2.0 / math.sqrt(math.pi)
                C = 1.1283791670955126
                return C * smith.exp(- self * self) * grad_output

            return smith.erf(self), backward

        def expand(self,
                   size: List[int],
                   *,
                   implicit: bool=False):
            result = smith.expand(self, size, implicit=implicit)
            self_size = smith._size_if_not_equal(self.size(), result.size())
        ...
        def expand_as(self, other):
            result = smith.expand_as(self, other)
            self_size = smith._size_if_not_equal(self.size(), result.size())
        ...
        def full_like(self,
            ...
            return smith.full_like(self, fill_value, memory_format=1), backward

        def lerp_0(self,
                   end,
                   weight: number):
            result = smith.lerp(self, end, weight)
            self_size = smith._size_if_not_equal(self.size(), result.size())
            end_size = smith._size_if_not_equal(end.size(), result.size())
        ...
        def lerp_1(self,
            ...
            self_size = smith._size_if_not_equal(self.size(), result.size())
            end_size = smith._size_if_not_equal(end.size(), result.size())
            weight_size = smith._size_if_not_equal(weight.size(), result.size())
        ...
        def reshape(self,
            ...
            return smith.reshape(self, shape), backward

        def split(self,
                  split_size: int,
                  dim: int):
            def backward(grad_outputs: List[Tensor]):
                grad_self = smith.cat(grad_outputs, dim)
                return grad_self, None, None

            return smith.split(self, split_size, dim), backward
        ...
        def split_with_sizes(self,
            ...
            return smith.split_with_sizes(self, split_sizes, dim), backward

        def stack(tensors: List[Tensor],
                  dim: int=0):
            def backward(grad_output):
                grad_tensors = smith.unbind(grad_output, dim)
                return grad_tensors, None

            return smith.stack(tensors, dim), backward

        def unbind(self,
                   dim: int):
            def backward(grad_outputs: List[Tensor]):
                grad_self = smith.stack(grad_outputs, dim)
                return grad_self, None

            return smith.unbind(self, dim), backward
        ...
        def cat(tensors: List[Tensor],
            ...
            def backward(grad_output):
                grad_tensors = smith.split_with_sizes(grad_output, split_sizes, dim)
                return grad_tensors, None

            return smith.cat(tensors, dim), backward

        def index(self,
                  indices: List[Tensor]):
            def backward(grad_output):
                grad_self = smith.zeros_like(self, memory_format=1).index_put_(indices, grad_output, True)
                return grad_self, None

            return smith.index(self, indices), backward
        ...
        def meshgrid(tensors: List[Tensor]):
            ...
            return smith.meshgrid(tensors), backward

        def mv(self, vec):
            def backward(grad_output):
                return grad_output.ger(vec), self.t().mv(grad_output)

            return smith.mv(self, vec), backward

        def nonzero(self):
            def backward(grad_output):
                return None

            return smith.nonzero(self), backward

        def ones_like(self):
            def backward(grad_output):
                return None

            return smith.ones_like(self, memory_format=1), backward

        def pow_0(self,
                  exponent: number):
            def backward(grad_output):
                if float(exponent) == 0.0:
                    grad_self = smith.zeros_like(self, memory_format=1)
                else:
                    grad_self = grad_output * exponent * smith.pow(self, float(exponent) - 1)
                return grad_self, None

            return smith.pow(self, exponent), backward

        def pow_1(self, exponent):
            result = smith.pow(self, exponent)
            self_size = smith._size_if_not_equal(self.size(), result.size())
            exponent_size = smith._size_if_not_equal(exponent.size(), result.size())

            def backward(grad_output):
                grad_self = smith.where(exponent == 0.0, smith.zeros_like(self, memory_format=1), grad_output * exponent * smith.pow(self, exponent - 1))._grad_sum_to_size(self_size)
                grad_exponent = (grad_output * smith.pow(self, exponent) * smith.log(self))._grad_sum_to_size(exponent_size)
        ...
        def pow_2(self: number,
                  exponent):
            def backward(grad_output):
                grad_exponent = grad_output * smith.pow(self, exponent) * smith.log(float(self))
        ...
        def rsub_0(self,
                   other,
                   alpha: number):
            result = smith.rsub(self, other, alpha=alpha)
            self_size = smith._size_if_not_equal(self.size(), result.size())
            other_size = smith._size_if_not_equal(other.size(), result.size())
            def backward(grad_output):
        ...
        def rsub_1(self,
            ...
            return smith.rsub(self, other, alpha), backward

        def sqrt(self):
            result = smith.sqrt(self)
            def backward(grad_output):
        ...
        def t(self):
            def backward(grad_output):
                return smith.t(grad_output)

            return smith.t(self), backward
        ...
        def transpose(self,
                      ...
                      dim1: int):
            def backward(grad_output):
                return smith.transpose(grad_output, dim0, dim1), None, None

            return smith.transpose(self, dim0, dim1), backward
        ...
        def view(self,
            ...
            return smith.view(self, size), backward
    )",
    R"(
        def AD_sizes_if_not_equal_multi_0(t1, t2, res):
            return smith._size_if_not_equal(t1.size(), res.size()), smith._size_if_not_equal(t2.size(), res.size())
        ...
        def div_2(self, other, *, rounding_mode: Optional[str]):
            result = smith.div(self, other, rounding_mode=rounding_mode)
            self_size, other_size = AD_sizes_if_not_equal_multi_0(self, other, result)
            def backward(grad_output):
                if rounding_mode is None:
                    ...
                    grad_self = smith.zeros_like(self)
                    grad_other = smith.zeros_like(other)
        ...
        def min(self, other):
            ...
            return smith.min(self, other), backward

        def sigmoid(self):
            result = smith.sigmoid(self)
            def backward(grad_output):
        ...
        def leaky_relu(self, negative_slope: number):
            result = smith.leaky_relu(self, negative_slope)
            def backward(grad_output):
                return grad_output * smith.where(self > 0, 1.0, negative_slope).type_as(result), None
            return result, backward

        def gelu(self : Tensor, *, approximate : str):
            result = smith.gelu(self, approximate=approximate)
            def backward(grad_output):
                return smith.gelu_backward(grad_output, self, approximate=approximate), None
            return result, backward

        def silu(self):
            result = smith.silu(self)
            def backward(grad_output):
                input_sigmoid = smith.sigmoid(self)
                return grad_output * (input_sigmoid * (1 + self * (1 - input_sigmoid)))
            return result, backward

        def hardswish(self):
            result = smith.hardswish(self)
            def backward(grad_output):
                m = (self >= 3.).type_as(result)
                m = smith.where((self > -3.) & (self < 3.),  self / 3. + .5, m)
                return grad_output * m
            return result, backward

        def hardsigmoid(self):
            result = smith.hardsigmoid(self)
            def backward(grad_output):
                m = (self > -3.) & (self < 3.)
                lhs = grad_output * (1.0 / 6.0)
                return smith.where(m, lhs, m.type_as(self))
            return result, backward
        ...
        def erfc(self):
            ...
            return smith.erfc(self), backward

        def exp(self):
            result = smith.exp(self)
            def backward(grad_output):
        ...
        def neg(self):
            ...
            return smith.neg(self), backward

        def where(condition, self, other):
            result = smith.where(condition, self, other)
            self_size, other_size = AD_sizes_if_not_equal_multi_0(self, other, result)
            def backward(grad_output):
        ...
        def type_as(self, other):
            ...
            return smith.type_as(self, other), backward

        def unsqueeze(self, dim: int):
            def backward(grad_output):
                return grad_output.squeeze(dim), None

            return smith.unsqueeze(self, dim), backward

        def abs(self):
            def backward(grad_output):
                return grad_output * self.sign()

            return smith.abs(self), backward

        def acos(self):
            def backward(grad_output):
                return grad_output * -((-self * self + 1).rsqrt())

            return smith.acos(self), backward

        def asin(self):
            def backward(grad_output):
                return grad_output * (-self * self + 1).rsqrt()

            return smith.asin(self), backward

        def atan(self):
            def backward(grad_output):
                return grad_output / (self * self + 1)

            return smith.atan(self), backward

        def ceil(self):
            def backward(grad_output):
                return smith.zeros_like(grad_output, memory_format=1)

            return smith.ceil(self), backward

        def cos(self):
            def backward(grad_output):
                return grad_output * -self.sin()

            return smith.cos(self), backward

        def cosh(self):
            def backward(grad_output):
                return grad_output * self.sinh()

            return smith.cosh(self), backward

        def expm1(self):
            result = smith.expm1(self)
            def backward(grad_output):
        ...
        def floor(self):
            ...
            return smith.floor(self), backward

        def frac(self):
            def backward(grad_output):
                return grad_output

            return smith.frac(self), backward

        def log(self):
            def backward(grad_output):
                return grad_output.div(self)

            return smith.log(self), backward

        def log10(self):
            def backward(grad_output):
                return grad_output / (self * 2.3025850929940456)

            return smith.log10(self), backward

        def log1p(self):
            def backward(grad_output):
                return grad_output / (self + 1)

            return smith.log1p(self), backward

        def log2(self):
            def backward(grad_output):
                return grad_output / (self * 0.6931471805599453)

            return smith.log2(self), backward
        ...
        #    return smith.rand_like(self, memory_format=memory_format), backward

        def reciprocal(self):
            result = smith.reciprocal(self)
            def backward(grad_output):
        ...
        def round(self):
            ...
            return smith.round(self), backward

        def rsqrt(self):
            result = smith.rsqrt(self)
            def backward(grad_output):
        ...
        def sin(self):
            ...
            return smith.sin(self), backward

        def sinh(self):
            def backward(grad_output):
                return grad_output * self.cosh()

            return smith.sinh(self), backward

        def tan(self):
            result = smith.tan(self)
            def backward(grad_output):
        ...
        def trunc(self):
            ...
            return smith.trunc(self), backward

        def _grad_sum_to_size(self,
                              size: Optional[List[int]]):
            result = smith._grad_sum_to_size(self, size)
            self_size = smith._size_if_not_equal(self.size(), result.size())
    ...
    R"(
        def batch_norm(input : Tensor,
            ...
            output, save1, save2, reserve, impl_idx = smith._batch_norm_impl_index(
            ...
            def backward(grad_output):
                dinput, dweight, dbias = smith._batch_norm_impl_index_backward(
        ...
        def layer_norm(input : Tensor,
            ...
            output, mean, rstd = smith.native_layer_norm(input, normalized_shape, weight, bias, eps)

            def backward(grad_output):
                output_mask = [True, weight is not None, bias is not None]
                grad_input, grad_weight, grad_bias = smith.native_layer_norm_backward(grad_output, input, normalized_shape, mean, rstd, weight, bias, output_mask)
        ...
        def dropout(input,
            ...
            res,mask = smith.native_dropout(input, p, train)

            def backward(grad_output):
                grad_input = smith.native_dropout_backward(grad_output, mask, scale)
        ...
        def embedding(weight,
            ...
            def backward(grad_output):
                grad_weight = smith.embedding_backward(grad_output, indices, weight_size_0, padding_idx, scale_grad_by_freq, sparse)
                return grad_weight, None, None, None, None

            return smith.embedding(weight, indices, padding_idx, scale_grad_by_freq, sparse), backward

        def log_softmax(self, dim: int, dtype: Optional[int]):
            result = smith.log_softmax(self, dim, dtype)
            def backward(grad_output):
                grad_self = smith._log_softmax_backward_data(grad_output, result, dim, self.dtype)
                return grad_self, None, None

            return result, backward

        def nll_loss(self, target, weight: Optional[Tensor], reduction: int, ignore_index: int):
            result, total_weight = smith.nll_loss_forward(self, target, weight, reduction, ignore_index)
            def backward(grad):
                return smith.nll_loss_backward(grad, self, target, weight, reduction, ignore_index, total_weight), None, None, None, None
            return result, backward

        def softmax(self, dim: int, dtype: Optional[int]):
            result = smith.softmax(self, dim, dtype)
            def backward(grad_output):
                grad_self = smith._softmax_backward_data(grad_output, result, dim, self.dtype)
                return grad_self, None, None
    ...
    R"(
        def AD_adaptive_avg_pool3d_backward(grad,
            ...
            if output_size[0] == 1 and output_size[1] == 1 and output_size[2] == 1:
                ...
                grad_self = smith._adaptive_avg_pool3d_backward(grad, self)
        ...
        def AD_adaptive_avg_pool2d_backward(grad,
            ...
            if output_size[0] == 1 and output_size[1] == 1:
                ...
                grad_self = smith._adaptive_avg_pool2d_backward(grad, self)
        ...
        def adaptive_avg_pool1d(self,
            ...
            return smith.adaptive_avg_pool1d(self, output_size), backward
        ...
        def adaptive_avg_pool2d(self,
            ...
            def backward(grad_output):
                ...
                return grad_self, None
            return smith.adaptive_avg_pool2d(self, output_size), backward
        ...
        def adaptive_avg_pool3d(self,
            ...
            return smith.adaptive_avg_pool3d(self, output_size), backward
        ...
        def avg_pool2d(self,
            ...
            def backward(grad_output):
                grad_self = smith.avg_pool2d_backward(grad_output, self, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override)
                return grad_self, None, None, None, None, None, None

            return smith.avg_pool2d(self, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override), backward
        ...
        def max_pool2d(self,
                       ...
                       dilation: List[int],
                       ceil_mode: bool):
            output, indices = smith.max_pool2d_with_indices(self, kernel_size, stride, padding, dilation, ceil_mode)
            def backward(grad_output):
                grad_self = smith.max_pool2d_with_indices_backward(grad_output, self, kernel_size, stride, padding, dilation, ceil_mode, indices)
    ...
    R"(
        ...
        def add_0(self,
                  ...
                  *,
                  alpha: number):
            result = smith.add(self, other, alpha=alpha)
        ...
        def add_1(self,
            ...
            return smith.add(self, other, alpha=alpha), backward

        def sub_0(self,
                  other,
                  *,
                  alpha: number):
            result = smith.sub(self, other, alpha=alpha)
            self_size, other_size = AD_sizes_if_not_equal_multi_1(self, other, result)
            def backward(grad_output):
                grad_other = (-grad_output * alpha)._grad_sum_to_size(other_size)
                grad_self = (grad_output)._grad_sum_to_size(self_size)
        ...
        def sub_1(self,
            ...
            def backward(grad_output):
                return grad_output, None, None
            return smith.sub(self, other, alpha=alpha), backward
        ...
        def threshold(self,
            ...
            return smith.threshold(self, threshold, value), backward

        def softplus(self,
                      beta: number,
                      threshold: number):
            result = smith.softplus(self, beta, threshold)
            def backward(grad_output):
                z = smith.exp(result * beta)
                return smith.where((result * beta) > threshold, grad_output, grad_output * (z - 1.) / z), None, None
            return result, backward
        ...
        def fmod(self,
            ...
            def backward(grad_output):
                return grad_output, None
            return smith.fmod(self, other), backward

        def remainder(self,
                      other: number):
            def backward(grad_output):
                return grad_output, None
            return smith.remainder(self, other), backward
        ...
        def addmm(self,
                  ...
                  *,
                  beta: number,
                  alpha: number):
            result = smith.addmm(self, mat1, mat2, beta=beta, alpha=alpha)
            self_size = smith._size_if_not_equal(self.size(), result.size())
            def backward(grad_output):
        ...
        def lt(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.lt(self, other), backward

        def le(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.le(self, other), backward

        def gt(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.gt(self, other), backward

        def ge(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.ge(self, other), backward

        def eq(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.eq(self, other), backward

        def ne(self, other: number):
            def backward(grad_output):
                return None, None
            return smith.ne(self, other), backward

        def hardshrink(self, lambd: number):
          def backward(grad_output):
            mask = ((self > lambd) | (self < -lambd))
            return grad_output * mask, None
          return smith.hardshrink(self, lambd=lambd), backward

        def hardtanh(self, min_val: number, max_val: number):
          def backward(grad_output):
            mask = ((self >= min_val) * (self <= max_val))
            return grad_output * mask, None, None
          return smith.hardtanh(self, min_val=min_val, max_val=max_val), backward
        ...
        def clamp_1(self,
          ...
          def backward(grad_output):
            if min is not None and max is not None:
                ...
                return grad_output, None, None
          return smith.clamp(self, min=min, max=max), backward
...
static Argument originalReturnType(const TupleTypePtr& tup) {
  SMITH_CHECK(tup->elements().size() > 1);
...
// In smithscript AD formulas, we define {func_0, func_1, ...} as
