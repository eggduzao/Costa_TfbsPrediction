...
#include <smith/csrc/jit/mobile/register_ops_common_utils.h>
...
SMITH_API void checkDoubleInRange(double a);
...
void listRemove(Stack& stack) {
  ...
  if (pos != list.end()) {
    ...
    SMITH_CHECK(false, "list.remove(x): x not in list");
  }
}
...
void listIndex(Stack& stack) {
  ...
  if (pos != list.end()) {
    ...
    SMITH_CHECK(false, "'", elem, "' is not in list");
  }
}
...
struct OperatorGeneratorArgs {
  ...
  explicit constexpr OperatorGeneratorArgs(
      smith::detail::SelectiveStr<true> schema_str,
      void (*op)(Stack&),
  ...
  explicit constexpr OperatorGeneratorArgs(
      smith::detail::SelectiveStr<false> /*unused*/,
      Args... /*unused*/)
      : schema_str(nullptr),
        isOperationCreator(false),
        operation(nullptr),
...
#define DEFINE_GENERIC_BINARY_OP(                                             \
  ...
  OperatorGeneratorArgs(                                                      \
      SMITH_SELECTIVE_SCHEMA(#aten_op                                         \
...
#define DEFINE_GENERIC_OP(aten_op, int_op, float_op, int_result, float_result) \
  OperatorGeneratorArgs(                                                       \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".int(int a, int b) -> " #int_result),   \
      [](Stack& stack) {                                                       \
        int64_t a, b;                                                          \
        pop(stack, a, b);                                                      \
        push(stack, int_op);                                                   \
...
#define DEFINE_INT_OP(aten_op, op)                                  \
  OperatorGeneratorArgs(                                            \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".int(int a, int b) -> int"), \
      [](Stack& stack) {                                            \
        int64_t a, b;                                               \
        pop(stack, a, b);                                           \
...
#define DEFINE_STR_CMP_OP(aten_op, op)                               \
  OperatorGeneratorArgs(                                             \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".str(str a, str b) -> bool"), \
      [](Stack& stack) {                                             \
...
#define DEFINE_SCALAR_BINARY_OP_AVOID_COLLISION_GENERIC(          \
  ...
  OperatorGeneratorArgs(                                          \
      SMITH_SELECTIVE_SCHEMA(#aten_op string_val                  \
...
#define DEFINE_UNARY_INT_OP(aten_op, op, result)                  \
  OperatorGeneratorArgs(                                          \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".int(int a) -> " #result), \
      [](Stack& stack) {                                          \
        int64_t a;                                                \
        pop(stack, a);                                            \
        push(stack, op);                                          \
...
#define DEFINE_UNARY_FLOAT_OP(aten_op, op, result)                    \
  OperatorGeneratorArgs(                                              \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".float(float a) -> " #result), \
      [](Stack& stack) {                                              \
        double a;                                                     \
        pop(stack, a);                                                \
        push(stack, op);                                              \
...
#define DEFINE_UNARY_OP(aten_op, op, int_result, float_result)            \
  DEFINE_UNARY_INT_OP(aten_op, op, int_result),                           \
      ...
      OperatorGeneratorArgs(                                              \
          SMITH_SELECTIVE_SCHEMA(#aten_op ".Scalar(Scalar a) -> Scalar"), \
          [](Stack& stack) {                                              \
            IValue x;                                                     \
            pop(stack, x);                                                \
            if (x.isDouble()) {                                           \
...
#define DEFINE_BOOL_OP(aten_op, op)                                     \
  OperatorGeneratorArgs(                                                \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".bool(bool a, bool b) -> bool"), \
      [](Stack& stack) {                                                \
        bool a, b;                                                      \
        pop(stack, a, b);                                               \
        push(stack, op);                                                \
...
#define DEFINE_STRING_OP(op_name, string_op, result)                    \
  OperatorGeneratorArgs(                                                \
      SMITH_SELECTIVE_SCHEMA(#op_name ".str(str a, str b) ->" #result), \
      [](Stack& stack) {                                                \
        auto b = pop(stack).toStringRef();                              \
        auto a = pop(stack).toStringRef();                              \
...
#define DEFINE_UNARY_COMPLEX_OP(aten_op, op, result)                      \
  OperatorGeneratorArgs(                                                  \
      SMITH_SELECTIVE_SCHEMA(#aten_op ".complex(complex a) -> " #result), \
      [](Stack& stack) {                                                  \
...
#define DEFINE_SCALAR_BINARY_OP_WITH_COMPLEX_WITHOUT_INT_COMPLEX_PAIR(     \
  ...
  OperatorGeneratorArgs(                                                   \
      SMITH_SELECTIVE_SCHEMA(#aten_op "(Scalar a, Scalar b) -> " #result), \
      [](Stack& stack) {                                                   \
        IValue x, y;                                                       \
        pop(stack, x, y);                                                  \
        if (x.isComplexDouble()) {                                         \
...
SMITH_API at::Generator make_generator_for_device(
    c10::Device device,
