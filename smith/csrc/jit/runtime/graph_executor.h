...
#include <smith/csrc/jit/runtime/variable_tensor_list.h>

SMITH_DECLARE_bool(smith_jit_enable_new_executor);

SMITH_DECLARE_bool(smith_jit_execution_plan_reuse_code_graph);
...
struct ExecutionPlan {
  ...
  ExecutionPlan(std::shared_ptr<Graph> graph, std::string function_name)
      : code(graph, std::move(function_name)),
        graph(
            FLAGS_smith_jit_execution_plan_reuse_code_graph
                ? code.graph()
                : std::move(graph)) {}
...
struct SMITH_API EnableProfilingGuard {
...
struct SMITH_API GraphExecutor {
...
SMITH_API Node* replaceBlockWithFallbackGraph(
    Block* b,
...
SMITH_API void runRequiredPasses(const std::shared_ptr<Graph>& g);

SMITH_API void debugSetFusionGroupInlining(bool state);
SMITH_API bool getFusionGroupInlining();

SMITH_API void debugSetAutodiffSubgraphInlining(bool state);
SMITH_API std::shared_ptr<Graph> lastExecutedOptimizedGraph();

SMITH_API std::atomic<bool>& getProfilingMode();
SMITH_API std::atomic<bool>& getExecutorMode();
SMITH_API std::atomic<size_t>& getNumProfiledRuns();
SMITH_API size_t getBailoutDepth();
SMITH_API bool IsNewExecutorEnabled();

struct SMITH_API GraphOptimizerEnabledGuard {
