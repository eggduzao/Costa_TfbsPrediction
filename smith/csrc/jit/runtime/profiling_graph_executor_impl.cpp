...
#include <smith/csrc/jit/passes/add_if_then_else.h>
#include <smith/csrc/jit/passes/batch_mm.h>
#include <smith/csrc/jit/passes/canonicalize_graph_fuser_ops.h>
#include <smith/csrc/jit/passes/check_strict_fusion.h>
#include <smith/csrc/jit/passes/clear_profiling.h>
#include <smith/csrc/jit/passes/clear_undefinedness.h>
...
#include <smith/csrc/jit/passes/inplace_check.h>
...
#include <smith/csrc/jit/passes/update_differentiable_graph_requires_grad.h>
...
// clang-format off
C10_DEFINE_bool(
    smith_jit_enable_new_executor,
    true,
    "If this flag is set to false SmithScript will be using the legacy/original executor")

C10_DEFINE_bool(
    smith_jit_disable_warning_prints,
    false,
    "Disables warning.warn prints in SmithScript graph")

C10_DEFINE_bool(
    smith_jit_static_then_dynamic,
    false,
    "fuse on two static compilations then 10 dynamic")

C10_DEFINE_bool(
    smith_jit_always_dynamic,
    false,
    "fuse on 12 dynamic compilations")

C10_DEFINE_bool(
    smith_jit_release_profiling_graph_after_optimization,
    false,
...
C10_DEFINE_int32(
    smith_jit_release_profiling_graph_delay_in_seconds,
    60,
    "How long to wait before releasing the profiling graph after optimizaiton is done. Only used if smith_jit_release_profiling_graph_after_optimization is set to true.")

constexpr size_t kDefaultNumProfiledRuns = 1;
constexpr size_t kDefaultBailoutDepth = 20;

C10_DEFINE_int64(
    smith_jit_num_profiled_runs,
    kDefaultNumProfiledRuns,
    "Number of profiling runs")
C10_DEFINE_int64(
    smith_jit_bailout_depth,
...
std::atomic<size_t>& getNumProfiledRuns() {
  ...
  static const size_t init = []() {
    return num_profiled_runs = FLAGS_smith_jit_num_profiled_runs;
  }();
...
FusionBehavior ProfilingGraphExecutorImpl::getCurrentBehavior(
  ...
  SMITH_WARN("Strategy changed mid-invocation, NYI");
...
void ProfilingGraphExecutorImpl::runProfilingOptimizations(
  ...
  if (needsGradientInProfilingMode(copy->block())) {
    ...
    for (Node* dnode : diff_nodes) {
      ...
      // graph is essentially in a smith.no_grad context.
...
const ExecutionPlan& ProfilingGraphExecutorImpl::getOptimizedPlanFor(
  ...
  // right.
  if (FLAGS_smith_jit_release_profiling_graph_after_optimization &&
...
const ExecutionPlan& ProfilingGraphExecutorImpl::getPlanFor(
  ...
  // IMPORTANT: This is a hot path of calling a smithscript function. Try not to
  ...
  if (optimized_plan_) {
    if (FLAGS_smith_jit_release_profiling_graph_after_optimization &&
      ...
      if ((now - time_optimized_plan_created_) >
          FLAGS_smith_jit_release_profiling_graph_delay_in_seconds) {
        clearTheGraphCompilationIntermediateGraphs();
      }
    }
    return *optimized_plan_;
...
GraphExecutorState ProfilingGraphExecutorImpl::getDebugState() {
  ...
  SMITH_INTERNAL_ASSERT(optimized_plan_);
...
void ProfilingGraphExecutorImpl::replaceFallbackGraphWithFallbackFunction(
  ...
  for (auto it = b->nodes().begin(); it != b->nodes().end();) {
    if (it->kind() == prim::FallbackGraph) {
      ...
      SMITH_INTERNAL_ASSERT(*remaining_bailout_depth_ > 0);
      GRAPH_DEBUG(
