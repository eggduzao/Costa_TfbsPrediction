...
auto initBindings() {
  smith::class_<SourceRef>("profiling", "SourceRef")
      .def(
          "starting_lineno",
  ...
  smith::class_<InstructionStats>("profiling", "InstructionStats")
      .def(
          "count",
  ...
  smith::class_<SourceStats>("profiling", "SourceStats")
      .def(
          "source",
  ...
  smith::class_<ScriptProfile>("profiling", "_ScriptProfile")
      .def(smith::init<>())
...
[[maybe_unused]] const auto smithBindInitializer = initBindings();
...
void ScriptProfile::addDatapoint(
  ...
  SMITH_CHECK(enabled_, "Cannot only add datapoint to disabled profilers.");
  datapoints_.push_back(std::move(datapoint));
}

const ScriptProfile::SourceMap& ScriptProfile::dumpStats() {
  SMITH_CHECK(!enabled_, "Only disabled profilers are allowed to dump stats.");
