...
static void DecomposeOp(Node* n) {
  ...
  SMITH_INTERNAL_ASSERT(outputs.size() == n->outputs().size());
...
struct JitDecomp final : smith::autograd::impl::JitDecompInterface {
...
static smith::autograd::impl::JitDecompRegisterer registerJitDecomp(&jitDecomp);

void JitDecomp::run_jit_decomposition(
    const c10::OperatorHandle& op,
    smith::jit::Stack* stack) const {
  ::smith::jit::run_jit_decomposition(op, stack);
}

bool JitDecomp::has_jit_decomposition(const FunctionSchema& schema) const {
  return ::smith::jit::has_jit_decomposition(schema);
}
...
void run_jit_decomposition(
  ...
  auto* trace_exec = smith::jit::GetDecompositionExecutor(schema);
...
Function* GetDecompositionExecutor(const FunctionSchema& schema) {
  ...
  SMITH_INTERNAL_ASSERT(maybe_func);
  return *maybe_func;
}
