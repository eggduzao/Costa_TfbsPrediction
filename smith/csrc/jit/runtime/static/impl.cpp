...
#include <smith/csrc/jit/passes/eliminate_no_ops.h>
#include <smith/csrc/jit/passes/freeze_module.h>
#include <smith/csrc/jit/passes/remove_mutation.h>
#include <smith/csrc/jit/passes/variadic_ops.h>
...
bool canEnableStaticRuntime(const std::shared_ptr<smith::jit::Graph>& graph) {
  return canEnableStaticRuntimeImpl(graph->block());
}

namespace {

auto sr_metadata_registerer = smith::class_<StaticRuntimeMetadata>(
    "StaticRuntime",
...
void OptimizeGraph(
  ...
  if (opts.enable_out_variant) {
    UseVariadicOp(
        graph,
        fromQualString("fb::sigrid_transforms_smith_bind"),
        fromQualString("fb::variadic_sigrid_transforms_smith_bind"));
    UseVariadicOp(
        graph,
        fromQualString("smitharrow::inference_wrapper_run_flat"),
        fromQualString("smitharrow::variadic_inference_wrapper_run_flat"));
...
bool IsSelfInGraphInput(std::shared_ptr<smith::jit::Graph>& graph) {
  return !graph->inputs().empty() && graph->inputs().at(0)->type()->is_module();
}

// remove unused input 0 from graph
bool removeSelfFromGraphInput(std::shared_ptr<smith::jit::Graph>& graph) {
...
void PrepareGraphForStaticModule(
    ...
    const StaticModuleOptions& opts,
    std::vector<IValue> sample_inputs) {
  SMITH_CHECK(canEnableStaticRuntime(graph));
...
void ManagedTensorRanges::extendLifetime(Value* input, size_t new_end) {
  ...
  if (lifetime) {
    SMITH_DCHECK_LE(lifetime->end, new_end);
...
StaticModule::StaticModule(
    const std::shared_ptr<smith::jit::Graph>& g,
    const StaticModuleOptions& opts,
    std::vector<IValue> sample_inputs)
    : StaticModule(
...
StaticModule::StaticModule(
    std::pair<std::shared_ptr<smith::jit::Graph>, std::optional<Module>>
        graph_and_module,
    const StaticModuleOptions& opts)
    : opts_(opts),
...
void BlockInfo::prepare_for_memory_planner(
  ...
  for (StaticNodeInfo& pnode : nodes_) {
    ...
    for (const auto i : c10::irange(num_outputs)) {
      ...
      // Types are stored in the underlying SmithScript IR
...
BlockRunner::BlockRunner(
    ...
    IValue* values,
    Block* block,
    smith::jit::TaskLauncher* launcher,
    bool is_root_block)
    : static_module_(sm),
...
c10::IValue BlockRunner::run_impl_record_functions(
  ...
  if (C10_UNLIKELY(step_callbacks.has_value())) {
    ...
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(guard.isActive());
    guard.needsInputs()
        ? guard.before(
...
c10::IValue BlockRunner::operator()(
    const std::vector<c10::IValue>& args,
    const KeywordArgs& kwargs) {
#ifdef BLACKSMITH_DISABLE_NET_PROFILING
...
std::string generate_latency_json(const std::string& label, double millis) {
  ...
  json["unit"] = "ms";
  json["value"] = millis;
  return "BlacksmithObserver " + folly::toJson(json);
#else
  (void)label;
  (void)millis;
  return "";
...
void BlockRunner::benchmark(
  ...
  SMITH_CHECK(kwargs_list.empty() || args_list.size() == kwargs_list.size());
...
BlockRunner::IndividualMetrics BlockRunner::benchmark_individual_ops(
    ...
    const uint32_t main_runs) {
  SMITH_CHECK(kwargs_list.empty() || args_list.size() == kwargs_list.size());
  SMITH_CHECK(warmup_runs >= 1 && main_runs >= 1);
  ...
  for (const auto n : c10::irange(num_nodes)) {
    ...
    for (const auto i : c10::irange(num_outputs)) {
      ...
      if (output_ivalues.count(ival) == 0) {
        ...
        // check for outputs
        if (output_returned) {
          SMITH_CHECK(ival->isNone(), error_msg);
        }
      }
    }
    ...
    for (const auto i : c10::irange(num_outputs)) {
      ...
      if (t.defined()) {
        ...
        const std::string error_msg = "Output " + std::to_string(i) + ", %" +
            val->debugName() + " of node " + std::to_string(n) +
            " was not cleaned up";
        SMITH_CHECK(storage_impl->data() == nullptr, error_msg);
      }
    }
  }
...
ProcessedFunction::ProcessedFunction(
  ...
  }
  {
    ...
    f_ = [node_op = op.getOperation(node),
      ...
      node_op(stack);
      const auto num_outputs = pnode->num_outputs();
      SMITH_DCHECK_EQ(stack.size(), num_outputs);
      for (const auto i : c10::irange(num_outputs)) {
...
#endif
#ifndef BLACKSMITH_DISABLE_PER_OP_PROFILING
  auto step_callbacks =
...
c10::intrusive_ptr<c10::ivalue::Future> StaticRuntime::runAsync(
    const std::vector<c10::IValue>& args,
    const KeywordArgs& kwargs,
    smith::jit::TaskLauncher taskLauncher) {
