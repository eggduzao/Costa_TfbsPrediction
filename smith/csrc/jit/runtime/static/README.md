...
Static Runtime is an optimized CPU inference runtime for Blacksmith models.
It can be used as a drop-in replacement for the SmithScript JIT interpreter
...
2. Blacksmith overhead (tensor creation, etc) accounts for
...
When loading a model, ops are selected for each `smith::jit::Node` in the graph as follows:
...
`StaticModule`s are constructed from `smith::jit::Module`s and can be used to construct `StaticRuntime`
...
* `prim::fork` operations have `smith::jit::TaskLauncher` (`std::function<void(std::function<void()>)>`) responsible for forked graph execution.
