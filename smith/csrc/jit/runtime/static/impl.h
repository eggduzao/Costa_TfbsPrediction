...
SMITH_API bool canEnableStaticRuntime(
    const std::shared_ptr<smith::jit::Graph>& graph);

SMITH_API std::string dumpValueSet(
...
class SMITH_API ManagedTensorRanges {
 public:
...
struct SMITH_API StaticModuleOptions {
...
*/
class SMITH_API StaticRuntimeMetadata : public smith::CustomClassHolder {
 public:
...
#ifdef FBCODE_CAFFE2
struct SMITH_API SROperatorObserver {
...
class SMITH_API ProcessedFunction {
 public:
  ProcessedFunction(
      Node* node,
...
class SMITH_API StaticModule {
 ...
 private:
  ...
  std::optional<smith::jit::Module> module_;
...
class SMITH_API BlockRunner {
 public:
  ...
  IValue& Input(uint32_t i) {
    SMITH_DCHECK_LT(i, block_info_.num_inputs());
...
class SMITH_API StaticNodeInfo {
 public:
  StaticNodeInfo(
      Node* n,
...
*/
class SMITH_API ProcessedNodeMetadata {
 public:
  ...
  void set_launcher(smith::jit::TaskLauncher* launcher) {
    launcher_ = launcher;
  }
...
class SMITH_API ProcessedNode {
 public:
  ...
  void set_metadata(smith::jit::TaskLauncher* launcher) {
    if (metadata_ == nullptr) {
      metadata_ = std::make_unique<ProcessedNodeMetadata>();
    }
...
class SMITH_API StaticRuntime {
 public:
  ...
  c10::intrusive_ptr<c10::ivalue::Future> runAsync(
      const std::vector<c10::IValue>& args,
      const KeywordArgs& kwargs = KeywordArgs(),
      smith::jit::TaskLauncher taskLauncher = at::launch);
 ...
 private:
  ...
  smith::jit::TaskLauncher async_task_launcher_;
  IValueArray values_;
};
