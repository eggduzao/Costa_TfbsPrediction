...
bool forwardHasOp(
    const smith::jit::script::Module& module,
    const char* op_name) {
  using Method = ::smith::jit::Method;
...
void FuseInferenceOpsForSparseNN(std::shared_ptr<smith::jit::Graph>& graph) {
#ifdef FBCODE_CAFFE2
...
SMITH_LIBRARY_FRAGMENT(static_runtime, m) {
  m.def(smith::schema(
...
void FuseSignLog1P(std::shared_ptr<smith::jit::Graph>& graph) {
  std::string pattern = R"IR(
    graph(%input):
...
void ReplaceWithMaybeCopy(
  ...
  for (auto n = graph_it.next(); n != nullptr; n = graph_it.next()) {
    ...
    SMITH_DCHECK_EQ(new_node->outputs().size(), 2);
...
void ReplaceWithCopy(
  ...
  static const std::vector<std::pair<c10::FunctionSchema, c10::Symbol>>
      supported_schema = {
          {{smith::schema("aten::dequantize.self(Tensor self) -> Tensor"),
...
void EliminateTrivialEquallySplit(std::shared_ptr<smith::jit::Graph>& graph) {
...
bool shouldNotFuseListUnpackSpecialCase(const Node* node) {
  const static std::array<c10::Symbol, 3> sigrid_transforms_symbols{
      c10::Symbol::fromQualString("fb::variadic_sigrid_transforms_smith_bind"),
      c10::Symbol::fromQualString("fb::sigrid_transforms_smith_bind"),
...
void FuseListUnpack(std::shared_ptr<smith::jit::Graph>& graph) {
  const c10::FastMap<c10::Symbol, c10::Symbol> unfused_to_fused = {
      OP_PAIR(
          "smitharrow::inference_wrapper_run_flat",
          "static_runtime::fused_inference_wrapper_run_flat"),
      OP_PAIR(
          "smitharrow::variadic_inference_wrapper_run_flat",
      ...
      OP_PAIR(
          "fb::sigrid_transforms_smith_bind",
          "static_runtime::fused_sigrid_transforms_smith_bind"),
      OP_PAIR(
          "fb::variadic_sigrid_transforms_smith_bind",
          "static_runtime::fused_variadic_sigrid_transforms_smith_bind"),
      OP_PAIR(
