...
#include <smith/csrc/jit/runtime/static/processed_node_wrapper.h>
...
#define SMITH_OPS_UNSUPPORTED_TYPE(_, type) \
  case k##type:                             \
    return true;
    switch (t) {
      AT_FORALL_QINT_TYPES(SMITH_OPS_UNSUPPORTED_TYPE)
      AT_FORALL_COMPLEX_TYPES(SMITH_OPS_UNSUPPORTED_TYPE)
      default:
        return false;
    }
#undef SMITH_OPS_UNSUPPORTED_TYPE
  };
...
bool sr_schema_check_kind(smith::jit::Node* node, c10::Symbol node_kind) {
  auto is_match = node->kind() == node_kind;
  if (!is_match) {
    smith::jit::LogAndDumpSchema(node);
  }
  return is_match;
}
...
REGISTER_OPERATOR_FUNCTOR(aten::abs, aten_abs, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema("aten::abs(Tensor self) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      SMITH_CHECK(clamp_min_opt.has_value() && clamp_max_opt.has_value());
      ...
      return [te = createClampNanToNum(),
        ...
        // just turn it into `inf` in that case. The Blacksmith core nan_to_num
...
REGISTER_OPERATOR_FUNCTOR(aten::bmm, aten_bmm, [](Node* n) -> SROperator {
  if (!n->matches(
          smith::schema("aten::bmm(Tensor self, Tensor mat2) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  return [](ProcessedNode* p_node) {
...
void varStackFastOut(
  ...
  SMITH_CHECK(num_inputs > 0, "stack expects a non-empty list of tensors");
...
void varStackOut(ProcessedNode& pnode, int64_t dim) {
  ...
  SMITH_CHECK(num_inputs > 1, "stack expects a non-empty list of tensors");
...
static SROperator aten_stack(Node* n) {
  ...
  return [](ProcessedNode* p_node) {
    ...
    SMITH_CHECK(!inputs.empty(), "stack expects non-empty tensor list");
...
REGISTER_OPERATOR_FUNCTOR(aten::relu, aten_relu, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema("aten::relu(Tensor self) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  auto te = createRelu();
...
REGISTER_OPERATOR_FUNCTOR(aten::tanh, aten_tanh, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema("aten::tanh(Tensor self) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  auto te = createTanh();
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    aten_sigmoid,
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema("aten::sigmoid(Tensor self) -> Tensor"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
...
ToArgs extract_to_args(ProcessedNode* p_node) {
  ...
  if (!has_constant_non_tensor_dtype_and_flags && p_node->Input(1).isTensor()) {
    ...
    SMITH_DCHECK_EQ(other.device().type(), c10::DeviceType::CPU);
  } else {
    const auto& self = p_node->Input(0).toTensor();
    ...
    SMITH_DCHECK_EQ(self.device().type(), c10::DeviceType::CPU);
  ...
  if (has_memory_format) {
    SMITH_DCHECK_EQ(p_node->num_inputs(), 5);
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      // Keep SMITH_CHECK here because there is no alternative for fallback
      if (!sr_schema_check(
              n,
      ...
      SMITH_CHECK(n->inputs().size() == 4 || n->inputs().size() == 5);
      ...
      if (has_constant_non_tensor_dtype_and_flags) {
        const auto copyArg =
            smith::jit::constant_as<bool>(n->inputs()[3]->node()->output());
...
REGISTER_OPERATOR_FUNCTOR(aten::max, aten_max, [](Node* n) -> SROperator {
  ...
  if (n->matches(smith::schema("aten::max(Tensor self) -> Tensor"))) {
    return [](ProcessedNode* p_node) {
      const auto& self = p_node->Input(0).toTensor();
      if (p_node->Output(0).isNone()) {
        p_node->Output(0) = create_empty_from(self);
...
REGISTER_OPERATOR_FUNCTOR(aten::sign, aten_sign, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema("aten::sign.Tensor(Tensor input) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  return [](ProcessedNode* p_node) {
...
REGISTER_OPERATOR_FUNCTOR(aten::log, aten_log, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema("aten::log.Tensor(Tensor input) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  return [](ProcessedNode* p_node) {
...
REGISTER_OPERATOR_FUNCTOR(aten::matmul, aten_matmul, [](Node* n) -> SROperator {
  if (!n->matches(
          smith::schema("aten::matmul(Tensor self, Tensor other) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  return [](ProcessedNode* p_node) {
...
REGISTER_OPERATOR_FUNCTOR(quantized::linear, quantized_linear, [](Node* n) -> SROperator {
  if (!n->matches(smith::schema(
          "quantized::linear(Tensor X, __smith__.smith.classes.quantized.LinearPackedParamsBase W_prepack, float Y_scale_i, int Y_zero_point_i) -> Tensor Y"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema(
              "fb::quantized_linear(Tensor X, __smith__.smith.classes.quantized.LinearPackedParamsBase w_prepack, Tensor Y_scale_i, Tensor Y_zero_point_i) -> Tensor"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      const auto w = toIValue(n->inputs()[1]);
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema(
              "quantized::linear_dynamic_fp16(Tensor X, __smith__.smith.classes."
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema(
              "quantized::linear_relu_dynamic_fp16(Tensor X, __smith__.smith.classes."
              "quantized.LinearPackedParamsBase W_prepack) -> Tensor Y"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
...
REGISTER_OPERATOR_FUNCTOR(aten::cat, aten_cat, [](Node* n) -> SROperator {
  if (!n->matches(
          smith::schema("aten::cat(Tensor[] tensors, int dim=0) -> Tensor"))) {
    LogAndDumpSchema(n);
    return nullptr;
  }
  return [](ProcessedNode* p_node) {
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    aten_nonzero,
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema("aten::nonzero(Tensor self) -> Tensor"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* p_node) {
...
REGISTER_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (n->matches(
              smith::schema("prim::NumToTensor.Scalar(Scalar s) -> Tensor")) ||
          n->matches(
              smith::schema("prim::NumToTensor.bool(bool a) -> Tensor"))) {
