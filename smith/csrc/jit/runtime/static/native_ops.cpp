...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      SMITH_DCHECK_EQ(num_inputs % 2, 0);
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        for (size_t i = 1; i < num_inputs; ++i) {
          ...
          SMITH_CHECK(value != dict.end(), "Key not in dict: ", key);
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_list,
    [](Node* n) -> SROperator {
      if (n->matches(smith::schema("aten::list(str t) -> str[]"))) {
        return [](ProcessedNode* p_node) {
      ...
      if (n->matches(smith::schema("aten::list.t(t[] l) -> t[]"))) {
        return [](ProcessedNode* p_node) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(
              smith::schema("aten::detach(Tensor(a) self) -> Tensor(a)"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* p_node) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(
              smith::schema("prim::isinstance(Any to_check) -> bool"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* p_node) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (n->matches(
              smith::schema("aten::size(Tensor self, int dim) -> int"))) {
        return [](ProcessedNode* p_node) {
          const auto& input = p_node->Input(0).toTensor();
      ...
      if (n->matches(smith::schema("aten::size(Tensor self) -> int[]"))) {
        return [](ProcessedNode* p_node) {
          const auto& input = p_node->Input(0).toTensor();
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        SMITH_DCHECK_NE(&assignFrom, &p_node->Output(0));
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      if (!n->matches(
              smith::schema("aten::mul.left_t(t[] l, int n) -> (t[])"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* pnode) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_sub,
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema("aten::sub.int(int a, int b) -> (int)"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* pnode) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_add,
    [](Node* n) -> SROperator {
      if (n->matches(smith::schema("aten::add.t(t[] a, t[] b) -> (t[])"))) {
        return [](ProcessedNode* pnode) {
      ...
      if (n->matches(smith::schema("aten::add.int(int a, int b) -> (int)"))) {
        return [](ProcessedNode* pnode) {
          const auto a = pnode->Input(0).toInt();
          const auto b = pnode->Input(1).toInt();
          pnode->Output(0) = a + b;
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_Int,
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema("aten::Int(Tensor a) -> int"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* pnode) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* node) -> SROperator {
      ...
      SMITH_DCHECK_EQ(node->blocks().size(), 2);
      ...
      switch (block_run_plan) {
        case BlockRunPlan::kRunBothBlocks:
          return [](ProcessedNode* p_node) {
            ...
            SMITH_DCHECK_EQ(block_runners.size(), 2);
            ...
            SMITH_DCHECK_EQ(elems.size(), p_node->num_outputs());
...
class ForkedSubgraphSRLauncher {
 ...
 private:
  ...
  smith::jit::TaskLauncher launcher_;
};
...
c10::intrusive_ptr<Future> createFutureTypeFromGraphOutput(
    const std::shared_ptr<smith::jit::Graph>& graph) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        SMITH_INTERNAL_ASSERT(p_node->Input(0).isFuture());
        auto future = p_node->Input(0).toFuture();

        // blocking call: waiting for the future to be completed
        future->waitAndThrow();

        SMITH_INTERNAL_ASSERT(future->completed());
        SMITH_INTERNAL_ASSERT(!future->hasError());
        SMITH_INTERNAL_ASSERT(future->hasValue());
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    [](Node* n) -> SROperator {
      ...
      return [](ProcessedNode* p_node) {
        ...
        DCHECK(metadata);
        auto& block_runners = metadata->block_runners();
        SMITH_DCHECK_EQ(block_runners.size(), 1);
        ...
        SMITH_DCHECK_EQ(args.size(), num_outputs + 1);
        for (const auto i : c10::irange(num_outputs)) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_Bool,
    [](Node* n) -> SROperator {
      if (n->matches(smith::schema("aten::Bool.Tensor(Tensor a) -> bool"))) {
        return [](ProcessedNode* pnode) {
          const auto& input = pnode->Input(0).toTensor();
          pnode->Output(0) = at::native::is_nonzero(input);
        };
      }
      if (n->matches(smith::schema("aten::Bool.int(int a) -> bool"))) {
        return [](ProcessedNode* pnode) {
          const auto input = pnode->Input(0).toInt();
          pnode->Output(0) = static_cast<bool>(input);
        };
      }
      if (n->matches(smith::schema("aten::Bool.float(float a) -> bool"))) {
        return [](ProcessedNode* pnode) {
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_len,
    [](Node* n) -> SROperator {
      if (n->matches(smith::schema("aten::len.t(t[] a) -> int")) ||
          n->matches(smith::schema("aten::len.any(Any[] a) -> int"))) {
        return [](ProcessedNode* pnode) {
      ...
      if (n->matches(smith::schema("aten::len.Tensor(Tensor t) -> int"))) {
        return [](ProcessedNode* pnode) {
      ...
      if (n->matches(smith::schema("aten::len.str(str s) -> int"))) {
        return [](ProcessedNode* pnode) {
      ...
      if (n->matches(
              smith::schema("aten::len.Dict_str(Dict(str, t) self) -> int")) ||
          n->matches(
              smith::schema("aten::len.Dict_int(Dict(int, t) self) -> int")) ||
          n->matches(smith::schema(
...
REGISTER_NATIVE_OPERATOR_FUNCTOR(
    ...
    aten_IntImplicit,
    [](Node* n) -> SROperator {
      if (!n->matches(smith::schema("aten::IntImplicit(Tensor a) -> int"))) {
        LogAndDumpSchema(n);
        return nullptr;
      }
      return [](ProcessedNode* pnode) {
