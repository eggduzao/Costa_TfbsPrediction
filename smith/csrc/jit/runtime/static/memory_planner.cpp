...
std::vector<StorageGroup> assignStorageToManagedTensors(
  ...
  auto assignToAvailableStorageGroup = [&](const Value* value) {
    ...
    SMITH_DCHECK_LT(storage_group, managed_tensor_groups.size());
    storage_group_mapping.emplace(value, storage_group);
    auto* tensor_ptr = tensor_value_to_tensor.at(value);
...
void ManagedStorages::allocate(size_t capacity) {
  SMITH_CHECK(!is_allocated(), "Must deallocate before allocating again");
  // `size_` should already be 0 if not allocated, so double check it here
  SMITH_INTERNAL_ASSERT(size_ == 0);
  capacity_ = capacity;
...
void ManagedStorages::append(at::StorageImpl& storageImpl) {
  SMITH_INTERNAL_ASSERT(size_ < capacity_);
...
void MemoryPlanner::allocateOutputTensors() {
  ...
  for (const auto& ms : managed_output_tensors_) {
    ...
    SMITH_DCHECK_LE(offset + tensor_size, output_buffer_bytes_);
    ...
    // to avoid `memcpy`.
    // `smith::distributed::detail::WireDumpOp::dumpTensorData is a concrete
    // example of doing this (See `smith::distributed::detail::hasDeleter`).
  ...
  SMITH_DCHECK_EQ(offset, output_buffer_bytes_);
}
...
void StandardMemoryPlanner::allocateManagedTensors() {
  ...
  for (const size_t storages_idx : c10::irange(storages_.size())) {
    ...
    SMITH_DCHECK_LE(offset + tensor_size, managed_bytes_);
    void* src = static_cast<void*>(start + offset);

#ifndef NDEBUG
    SMITH_DCHECK_EQ(tensor_size, managed_tensors_[group_idx].maxTensorSize());
    for (auto* tensor : managed_tensors_[group_idx].group()) {
      SMITH_DCHECK_EQ(storageImpl, tensor->storage().unsafeGetStorageImpl());
    }
#endif
    SMITH_DCHECK_NE(managed_tensors_[group_idx].numManagedTensors(), 0);
  ...
  SMITH_DCHECK_EQ(offset, managed_bytes_);
}
...
void StandardMemoryPlanner::deallocateManagedTensors() {
  ...
  SMITH_DCHECK_EQ(storages_.size(), managed_tensors_.size());
