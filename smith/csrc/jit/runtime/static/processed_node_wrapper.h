  ...
  class ProcessedNodeWrapperBaseIter {
   public:
    ...
    ProcessedNodeWrapperBaseIter& operator++() {
      SMITH_DCHECK_NE(idx_, container_->size());
      ++idx_;
      return *this;
    }
    ...
    reference operator*() const {
      SMITH_CHECK(container_ != nullptr);
    ...
    friend bool operator==(
      ...
      SMITH_DCHECK_EQ(lhs.container_, rhs.container_);
...
// variadic variant in static runtime. To use the Blacksmith library implementation
...
class ProcessedNodeInputWrapper
 ...
 public:
  ...
  explicit ProcessedNodeInputWrapper(
    ...
    SMITH_CHECK(front_elements_ignored_ <= pnode_.num_inputs());
    SMITH_CHECK(
