...
SMITH_API void FuseInferenceOpsForSparseNN(
    std::shared_ptr<smith::jit::Graph>& graph);

SMITH_API void EliminateTrivialEquallySplit(
    std::shared_ptr<smith::jit::Graph>& graph);

SMITH_API void FuseListUnpack(std::shared_ptr<smith::jit::Graph>& graph);
...
SMITH_API void ReplaceWithCopy(
    std::shared_ptr<smith::jit::Graph>& graph,
    bool outputs_are_immutable = true);

SMITH_API void ReplacePermuteWithCopy(
    std::shared_ptr<smith::jit::Graph>& graph,
    bool outputs_are_immutable = true);

SMITH_API void ReplaceWithMaybeCopy(
    std::shared_ptr<smith::jit::Graph>& graph,
    bool outputs_are_immutable = true);

SMITH_API void RemoveImmutableInputDictLookups(
    std::shared_ptr<smith::jit::Graph>& graph);

SMITH_API bool graphHasOp(std::shared_ptr<Graph>& graph, const char* op_name);

SMITH_API bool forwardHasOp(const Module& module, const char* op_name);

SMITH_API void FuseSignLog1P(std::shared_ptr<Graph>& graph);

SMITH_API void UseVariadicTupleUnpack(const std::shared_ptr<Graph>& graph);
...
SMITH_API void CreateOwnedRefsForSpecialValues(Graph& graph);
...
SMITH_API void ForceNonEmptyOutputs(Graph& graph);

SMITH_API void UseVariadicGroupedAccessor(const std::shared_ptr<Graph>& graph);

SMITH_API void EliminateExtraPermuteOps(std::shared_ptr<Graph>& graph);

SMITH_API void EliminateNoOpSlice(std::shared_ptr<Graph>& graph);

SMITH_API void UseSplitAndSqueeze(std::shared_ptr<Graph>& graph);
...
SMITH_API void RemoveUnnecessaryOutputs(std::shared_ptr<Graph>& graph);

SMITH_API void RemoveUnnecessaryEmbeddingBagOutputs(
    std::shared_ptr<Graph>& graph);

SMITH_API void FuseClampNaNToNum(std::shared_ptr<Graph>& graph);

SMITH_API void UseInPlaceGetRealInputsFromOptionalInputsV2(
    std::shared_ptr<Graph>& graph);

SMITH_API void PrepackWeights(std::shared_ptr<Graph>& graph);
