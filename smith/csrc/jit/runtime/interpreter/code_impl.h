...
#include <smith/csrc/jit/runtime/interpreter/preprocess_graph.h>

SMITH_DECLARE_bool(smith_jit_enable_expanded_stacks);
SMITH_DECLARE_bool(smith_jit_expanded_stacks_mangled);
...
struct CodeImpl {
  ...
  NodeSourceInfo getSourceInfoFromSourceRange(const SourceRange& range) {
    ...
    SourceRange r = range;
    if (!FLAGS_smith_jit_expanded_stacks_mangled && range.source()) {
  ...
  void insertInstruction(OpCode op, int64_t X = 0, uint64_t N = 0) {
    ...
    if (FLAGS_smith_jit_enable_expanded_stacks &&
  ...
  void emitBailOut(Node* node) {
    ...
    SMITH_INTERNAL_ASSERT(node->kind() == prim::BailOut);
    auto bailout_index = node->i(attr::index);
    SMITH_INTERNAL_ASSERT(bailout_index >= 0);
  ...
  void insertBailoutBlocks() {
    for (const BailoutBlock& block : bailout_blocks_) {
      SMITH_INTERNAL_ASSERT(instructions_[block.jf_instruction_index].op == JF)
  ...
  void emitWarn(Node* node) {
    if (FLAGS_smith_jit_disable_warning_prints) {
      return;
    }
