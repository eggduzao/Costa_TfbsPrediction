  ...
  switch (key) {
    ...
    case 'c':
      SMITH_CHECK(
          ...
          "%",
          key,
          " requires an insmithar for formatting, but got ",
          ival.tagKind());
      if (ival.isInt()) {
    ...
    default:
      SMITH_CHECK(
          ...
          "The specifier %",
          key,
          " is not supported in SmithScript format strings");
  }
}
...
void format(Stack& stack, size_t num_inputs) {
  ...
  //   SMITH_WARN("Format options are not supported.");
  // }
  ...
  for (size_t begin = 0, used_args = 0; true; ++used_args) {
    ...
    if (used_args >= args.size()) {
      SMITH_CHECK(false, "Too few arguments for format string: ", format);
    }
    ss << args[used_args];
    begin = loc + 2;
  }
...
void percentFormat(Stack& stack, size_t num_inputs) {
  ...
  while (true) {
    ...
    SMITH_CHECK(used_args < args_size, "Too few arguments for format string");
  ...
  SMITH_CHECK(used_args == args_size, "Too many arguments for format string");
  drop(stack, num_inputs);
  push(stack, ss.str());
}
...
void tupleConstruct(Stack& stack, size_t num_inputs) {
  if (num_inputs > stack.size()) {
    SMITH_CHECK(false, "Invalid number of inputs: ", num_inputs);
  }
  switch (num_inputs) {
    case 0:
