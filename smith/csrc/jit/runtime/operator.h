...
SMITH_API std::string canonicalSchemaString(const FunctionSchema& schema);

SMITH_API const std::vector<std::shared_ptr<Operator>> getAllOperators();
SMITH_API const std::vector<std::shared_ptr<Operator>>& getAllOperatorsFor(
    Symbol name);
// Returns operators in the order which OpOverloadPacket resolves them.
SMITH_API std::vector<std::shared_ptr<Operator>> getAllSortedOperatorsFor(
    Symbol name);

// given a operator with an overload name, find the specific operator related to
// it, may return nullptr if no operator exists.
SMITH_API std::shared_ptr<Operator> findOperatorFor(
    const c10::OperatorName& full_name);

SMITH_API std::vector<Symbol> findSimilarOperators(Symbol input_op);

SMITH_API void registerOperator(Operator&& op);
SMITH_API void deregisterOperator(const FunctionSchema& schema);

// XXX: this function is meant to be used with string literals only!
SMITH_API std::shared_ptr<Operator> getOperatorForLiteral(
...
SMITH_API void ensure_c10_registerer_defined();

// Used to assert that unschematized operators have an analysis method written
SMITH_API bool aliasAnalysisHasSpecialCaseFor(c10::Symbol sym);
...
template <typename Func>
std::optional<Operator> OperatorGenerator(
    smith::detail::SelectiveStr<false> schema_str,
    Func&& op,
    AliasAnalysisKind alias_analysis) {
  return std::nullopt;
}
