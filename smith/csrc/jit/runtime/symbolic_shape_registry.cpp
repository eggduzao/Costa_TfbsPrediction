...
const std::string _xnnpack_shape_compute_functions =
    ...
    assert isinstance(conv2dOpContext, __smith__.smith.classes.xnnpack.Conv2dOpContext)
...
def prepacked_linear_clamp_run(input: List[int], linearOpContext: Any):
    assert isinstance(linearOpContext, __smith__.smith.classes.xnnpack.LinearOpContext)
...
#ifdef USE_XNNPACK
      {"prepacked::conv2d_clamp_run(Tensor X, __smith__.smith.classes.xnnpack.Conv2dOpContext W_prepack) -> Tensor Y", "prepacked_conv2d_clamp_run"},
      {"prepacked::linear_clamp_run(Tensor X, __smith__.smith.classes.xnnpack.LinearOpContext W_prepack) -> Tensor Y", "prepacked_linear_clamp_run"},
#endif
  };
  // clang-format on
...
void transformShapeFunction(
  ...
  // functions defined in SmithScript or User-Registered Operators
...
SMITH_API std::optional<BoundedShapeGraphs> boundedGraphsForSchema(
    const FunctionSchema& schema) {
  std::lock_guard<std::mutex> guard(lock);
