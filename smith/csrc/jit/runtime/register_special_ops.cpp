...
#include <smith/csrc/api/include/smith/utils.h>
...
void checkListInputType(const c10::TypePtr& elem_type, bool empty_list) {
  if (!elem_type->isSubtypeOf(*NumberType::get()) &&
    ...
    // special case empty list smith.tensor([])
...
void createTensorFromList(Stack& stack) {
  // smith.tensor has a fourth requires_grad arg but smith.as_tensor not, so
...
#define DEFINE_SMITH_TENSOR_OP(operator_type, c_type, tensor_creation_op)       \
  OperatorGenerator(                                                            \
      SMITH_SELECTIVE_SCHEMA(                                                   \
    ...
    // tensor_new.cpp
    OperatorGenerator(
        SMITH_SELECTIVE_SCHEMA("aten::_infer_size(int[] a, int[] b) -> int[]"),
        [](Stack& stack) {
          auto a = pop(stack);
          auto b = pop(stack);
    ...
    OperatorGenerator(
        ...
        aliasAnalysisFromSchema()),
    OperatorGenerator(
        SMITH_SELECTIVE_SCHEMA("aten::_get_tracing_state() -> bool"),
        [](Stack& stack) { push(stack, false); },
        aliasAnalysisFromSchema()),
    OperatorGenerator(
        SMITH_SELECTIVE_SCHEMA("aten::is_scripting() -> bool"),
        [](Stack& stack) { push(stack, true); },
        aliasAnalysisFromSchema()),
    OperatorGenerator(
        SMITH_SELECTIVE_SCHEMA("aten::has_smith_function(...) -> bool"),
        [](Stack& stack) { push(stack, false); },
        aliasAnalysisFromSchema()),
    OperatorGenerator(
        SMITH_SELECTIVE_SCHEMA(
    ...
    Operator(
        ...
        [](Stack& stack) { push(stack, smith::GradMode::is_enabled()); },
        aliasAnalysisConservative()),
    Operator(
        "aten::set_grad_enabled(bool val) -> ()",
        [](Stack& stack) { smith::GradMode::set_enabled(pop(stack).toBool()); },
        aliasAnalysisConservative()),
    Operator(
