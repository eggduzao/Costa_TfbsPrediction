...
// NB: Write gradient using smithscript
...
class GradientHelper {
 public:
  ...
  std::vector<Value*> gradient(ArrayRef<Value*> grad_values) {
    ...
    // If AD is defined using smithscript, use it instead of symbolic
    auto script_grads = build_script_grad(node, grad_values);
    if (script_grads)
      return *script_grads;

    // Definition not found in smithscript, look up in the buildSymbolicGradient
    // TODO: migrate all to using smithscript
...
Gradient differentiate(std::shared_ptr<Graph>& graph) {
  ...
  // Clean up old nodes which has been replaced by forward graphs in smithscript
