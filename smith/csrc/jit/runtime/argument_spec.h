...
struct ArgumentSpec {
  ...
  void addTensor(const IValue& input, bool with_grad) {
    ...
    // https://github.com/zdevito/blacksmith/commit/21e7200a0a0fc456bea2f10e95b1781f83933d10
...
// input tensors.
struct SMITH_API ArgumentSpecCreator {
...
struct hash<c10::TensorType> {
  size_t operator()(const c10::TensorType& ptt) const {
    return c10::get_hash<
        ...
        c10::VaryingShape<int64_t>,
        std::optional<bool>>(
        smith::jit::convertOptional(ptt.scalarType()),
        ptt.sizes(),
        ptt.strides(),
        ptt.requiresGrad());
  }
...
template <>
struct hash<smith::jit::ArgumentSpec> {
  size_t operator()(const smith::jit::ArgumentSpec& spec) const noexcept {
    return spec.hashCode();
  }
};
template <>
struct hash<smith::jit::CompleteArgumentSpec> {
  size_t operator()(
      const smith::jit::CompleteArgumentSpec& spec) const noexcept {
    return spec.hashCode();
  }
};
} // namespace std
