...
//   `Dimension` across **one** profiling execution or run of a SmithScript
//   function.
...
SMITH_API void RegisterProfilingNode(
...
struct ShapeSymbolTable {
  ...
  void assign(c10::ShapeSymbol s, Dimension v) {
    SMITH_INTERNAL_ASSERT(!s.is_static());
    data_[s] = v;
  }
...
struct ProfilingRecord {
  ...
  SMITH_API static std::unique_ptr<ProfilingRecord> instrumentGraph(
      const std::shared_ptr<Graph>& graph);
  SMITH_API static void removeProfilingNodes(Block* b);
  SMITH_API static void removeProfileCounter(Block* b);
  ...
  SMITH_API ProfileIValueOp* createProfileIValueNode(Value* in_val);
  SMITH_API ProfileIValueOp* createProfileIValueNode(ArrayRef<Value*> inputs);
