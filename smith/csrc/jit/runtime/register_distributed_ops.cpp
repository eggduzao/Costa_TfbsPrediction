...
namespace dist_autograd = smith::distributed::autograd;
namespace dist_rpc = smith::distributed::rpc;
...
void prepare_and_call_rpc_op(
  ...
  IValue noTimeout(smith::distributed::rpc::kUnsetRpcTimeout);
  ...
  SMITH_INTERNAL_ASSERT(qualifiedNameIValue.isString());
  SMITH_INTERNAL_ASSERT(argsTupleIValue.isTuple());
  SMITH_INTERNAL_ASSERT(kwargsDictIValue.isGenericDict());
  SMITH_INTERNAL_ASSERT(timeoutIValue.isDouble());
  ...
  if (rpc_op == "rpc_async") {
    // Send RPC request.
    auto futureIValuePtr = dist_rpc::rpcSmithscript(
        dstWorkerNameStr,
        qualifiedName,
        functionSchema,
        userCallableStack,
    ...
    auto rrefPtr = dist_rpc::remoteSmithscript(
        dstWorkerNameStr,
        qualifiedName,
        functionSchema,
        userCallableStack,
    ...
    throw std::runtime_error(
        c10::str(rpc_op, "() is not supported in SmithScript!'"));
  }
}
...
RegisterOperators reg_rpc_ops(
    {Operator(
         fmt::format(
             ...
             smith::distributed::rpc::kDefaultRpcTimeoutSeconds),
         [](Stack& stack) {
     ...
     Operator(
         ...
         aliasAnalysisFromSchema()),
     Operator(
         "aten::owner(RRef(t) self) -> __smith__.smith.classes.dist_rpc.WorkerInfo",
         [](Stack& stack) {
           auto rref = pop(stack).toRRef();
           push(
               stack,
               smith::make_custom_class<distributed::rpc::WorkerInfo>(
     ...
     Operator(
         ...
         [](Stack& stack) {
           ...
           smith::autograd::variable_list roots(
...
SMITH_LIBRARY_IMPL(aten, CatchAll, m) {
