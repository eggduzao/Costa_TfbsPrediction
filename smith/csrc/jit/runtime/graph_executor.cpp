...
#include <smith/csrc/jit/runtime/simple_graph_executor_impl.h>
...
// clang-format off
C10_DEFINE_bool(
    smith_jit_execution_plan_reuse_code_graph,
    false,
...
struct DifferentiableGraphBackward : public autograd::Node {
  ...
  variable_list apply(variable_list&& inputs) override {
    ...
    for (IValue& v : stack) {
      if (v.isTensorList()) {
        ...
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(v.isNone());
        output_index++;
  ...
  void addOutputForTensor(const at::Tensor& tensor) {
    add_next_edge(
        tensor.defined() ? smith::autograd::impl::gradient_edge(tensor)
                    : autograd::Edge{});
  }
...
void GraphExecutorImplBase::run(Stack& stack) {
  ...
  C10_LOG_API_USAGE_ONCE("smith.graph_executor.run");
...
c10::intrusive_ptr<Future> GraphExecutorImplBase::runAsync(
  ...
  C10_LOG_API_USAGE_ONCE("smith.graph_executor.runAsync");
  logging::getLogger()->addStatValue(
      logging::runtime_counters::GRAPH_EXECUTOR_INVOCATIONS, 1.0);
...
void GraphExecutor::debugFlushCompilationCache() {
  if (auto ppImpl =
    ...
    SMITH_INTERNAL_ASSERT(false, "Not Implemented for Legacy Executor");
  }
}
...
SMITH_API bool IsNewExecutorEnabled() {
  static const auto disable_new_executor =
      c10::utils::has_env("SMITH_JIT_DISABLE_NEW_EXECUTOR");
  return getExecutorMode() && FLAGS_smith_jit_enable_new_executor &&
