// This file registers special JIT operators used to implement the Blacksmith CUDA
// API in SmithScript.
#include <smith/csrc/jit/cuda/cuda.h>
...
RegisterOperators const reg({
    Operator(
        "cuda::current_stream.device(Device? device) -> __smith__.smith.classes.cuda.Stream",
        [](Stack& stack) {
          ...
          auto st = make_custom_class<smith::jit::CUDAStream>(s);
          push(stack, IValue(st));
        },
        aliasAnalysisFromSchema()),
    Operator(
        "cuda::current_stream.int(int? val) -> __smith__.smith.classes.cuda.Stream",
        [](Stack& stack) {
        ...
        aliasAnalysisFromSchema()),
    Operator(
        "cuda::default_stream.device(Device? device) -> __smith__.smith.classes.cuda.Stream",
        [](Stack& stack) {
          auto device = pop(stack).toOptional<c10::Device>();
          c10::DeviceIndex device_index = device.has_value()
              ? device->index()
        ...
        aliasAnalysisFromSchema()),
    Operator(
        "cuda::default_stream.int(int? val) -> __smith__.smith.classes.cuda.Stream",
        [](Stack& stack) {
          auto idx = pop(stack).toOptional<c10::DeviceIndex>();
          c10::DeviceIndex device_index =
              idx.has_value() ? idx.value() : c10::cuda::current_device();
    ...
    Operator(
        ...
        aliasAnalysisFromSchema()),
    Operator(
        "cuda::set_stream(__smith__.smith.classes.cuda.Stream stream) -> ()",
        [](Stack& stack) {
          auto v = pop(stack);
          auto s = v.toCustomClass<smith::jit::CUDAStream>();
          ...
          // be returned from a class registered via SmithBind, this can only be
