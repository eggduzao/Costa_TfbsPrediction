...
#include <smith/csrc/jit/runtime/exception_message.h>
#include <smith/csrc/jit/runtime/graph_executor.h>
#include <smith/csrc/jit/runtime/instruction.h>
#include <smith/csrc/jit/runtime/interpreter/code_impl.h>
...
#include <smith/csrc/jit/runtime/script_profile.h>
...
using smith::distributed::autograd::DistAutogradContainer;
#endif
...
struct InterpreterStateImpl : c10::intrusive_ptr_target {
 ...
 private:
  using Frame = smith::jit::interpreter::Frame;
  struct WarnedNodes {
   public:
...
#define INST_NEXT      \
  ...
  bool runTemplate(Stack& stack) {
    ...
    // stack
    if (stack_start_ == -1) {
      SMITH_INTERNAL_ASSERT(stack.size() >= frames.back().function->n_inputs);
...
#undef DISPATCH_TABLE_ENTRY
    ...
    try {
      while (true) {
        ...
        switch (inst.op) {
          case INST(ENTER): {
            ...
            SMITH_INTERNAL_ASSERT(obj.isObject());
          ...
          case INST(STOREN): {
            [[maybe_unused]] auto _ = instGuard();
            SMITH_INTERNAL_ASSERT(stack.size() >= inst.N);
          ...
          case INST(TYPECHECK): {
            ...
            SMITH_INTERNAL_ASSERT(stack.size() >= num_inputs && num_inputs > 0);
          ...
          case INST(TAIL_CALL): {
            ...
            SMITH_INTERNAL_ASSERT(stack.size() >= num_inputs);
          ...
          case INST(DTYPE): {
            [[maybe_unused]] auto _ = instGuard();
            SMITH_INTERNAL_ASSERT(!stack.empty());
            dtype(stack);
          }
            INST_NEXT;
          case INST(DIM): {
          ...
          case INST(WARN): {
            ...
            if (range->filename()) {
              ...
              if (need_warn) {
                SMITH_WARN(stack.back().toStringRef());
              }
              stack.pop_back();
            }
          }
      ...
      if (FLAGS_smith_jit_enable_rethrow_caught_exception) {
        if (future_) {
      ...
      // Janky af.  See https://github.com/blacksmith/blacksmith/issues/54612
...
#undef JIT_USE_COMPUTED_GOTO
  ...
  void handleError(
    ...
    ss << "The following operation failed in the SmithScript interpreter.\n";
    formatStackTrace(ss);
    ss << class_name << ": " << msg << '\n';
    if (future_) {
  ...
  static void checkAndStartRecordFunction(Frame& frame, Stack& stack) {
    if (!frame.record_function) {
      auto step_callbacks = at::getStepCallbacksUnlessEmpty(
          at::RecordScope::SMITHSCRIPT_FUNCTION);
      if (C10_UNLIKELY(step_callbacks.has_value())) {
        auto rec_fn =
            std::make_unique<at::RecordFunction>(std::move(*step_callbacks));
        SMITH_INTERNAL_ASSERT_DEBUG_ONLY(rec_fn->isActive());
 ...
 public:
  ...
  void run(Stack& stack) {
    ...
    SMITH_INTERNAL_ASSERT(!frames.empty());
