...
namespace smith::jit::logging {

class LoggerBase {
 public:
  SMITH_API virtual void addStatValue(
      const std::string& stat_name,
      int64_t val) = 0;
  virtual ~LoggerBase() = default;
};

SMITH_API LoggerBase* getLogger();
SMITH_API LoggerBase* setLogger(LoggerBase* logger);
...
class SMITH_API LockingLogger : public LoggerBase {
 public:
...
SMITH_API JITTimePoint timePoint();
SMITH_API void recordDurationSince(
    const std::string& name,
    const JITTimePoint& tp);

namespace runtime_counters {
constexpr const char* GRAPH_EXECUTORS_CONSTRUCTED =
    "blacksmith_runtime.graph_executors_constructed";
constexpr const char* GRAPH_EXECUTOR_INVOCATIONS =
    "blacksmith_runtime.graph_executor_invocations";
constexpr const char* EXECUTION_PLAN_CACHE_HIT =
    "blacksmith_runtime.execution_plan_cache_hit";
constexpr const char* EXECUTION_PLAN_CACHE_MISS =
    "blacksmith_runtime.execution_plan_cache_miss";
