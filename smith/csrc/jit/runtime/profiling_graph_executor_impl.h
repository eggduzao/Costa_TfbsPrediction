...
SMITH_DECLARE_bool(smith_jit_static_then_dynamic);

SMITH_DECLARE_bool(smith_jit_always_dynamic);

C10_DECLARE_bool(smith_jit_release_profiling_graph_after_optimization);
C10_DECLARE_int32(smith_jit_release_profiling_graph_delay_in_seconds);
C10_DECLARE_int64(smith_jit_num_profiled_runs);
C10_DECLARE_int64(smith_jit_bailout_depth);

namespace smith::jit {

SMITH_API void runNooptPassPipeline(std::shared_ptr<Graph>& graph);

struct SMITH_API ProfilingGraphExecutorImpl : public GraphExecutorImplBase {
