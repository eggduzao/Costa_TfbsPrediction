...
SMITH_DECLARE_bool(smith_jit_disable_warning_prints);
SMITH_DECLARE_bool(smith_jit_enable_rethrow_caught_exception);

namespace at {
class Tensor;
SMITH_API void launch(std::function<void()> func);
} // namespace at
namespace c10 {
struct IValue;
struct OperatorName;
...
bool SMITH_API in_smithscript_runtime();

struct SMITH_API Code {
  Code() = default;
...
struct SMITH_API MobileCode : Code {
  explicit MobileCode(
      const std::shared_ptr<Graph>& graph,
      std::string function_name,
...
struct InterpreterState {
  SMITH_API InterpreterState(
      const Code& code,
      TaskLauncher taskLauncher = at::launch);
  SMITH_API void run(Stack& stack);
  SMITH_API c10::intrusive_ptr<Future> runAsync(Stack& stack);
...
SMITH_API at::TensorTypePtr tensorTypeInCurrentExecutionContext(
    const at::Tensor& t);

// current (TLS) SmithScript interpreter callstack
SMITH_API std::vector<StackEntry> currentCallstack();
SMITH_API std::vector<std::string> currentModuleHierarchy();
