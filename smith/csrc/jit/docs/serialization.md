# SmithScript serialization

This document explains the SmithScript serialization format, and the anatomy
of a call to `smith::jit::save()` or `smith::jit::load()`.
...
for developers familiar with Python but not familiar with SmithScript.
...
m = MyModule()
m.save("foo.pt")
m_loaded = smith.load("foo.pt")
m_loaded.save("foo2.pt")
m_loaded2 = smith.load("foo2.pt")
```
...
special properties (see [the `smith.nn`
documentation](https://blacksmith.org/docs/stable/nn.html#parameters) for exact
...
Classes at the root of the hierarchy are given the qualified name `__smith__`
as a prefix, just so that they can go in `__smith__.py`. (Why not `__main__`?
...
* **self describing** - a separate definition file is not needed to understand
 the pickled data
* **eager mode save** - `smith.save()` already produces a `pickle` archive, so
...
Blacksmith functions defined in [smith/jit/_pickle.py](../../../jit/_pickle.py)
...
- We'd like to maintain compatibility with regular `Blacksmith`'s serialization
  format

## `constants.pkl`: Constants in code

The `pickle` format enforces a separation between data and code, which the
SmithScript serialization process represents by having `code/` and
`data.pkl + tensors/`.

However, SmithScript inlines constants (i.e. `prim::Constant` nodes) directly
...
A `CompilationUnit` is created whenever `smith::jit::load()` is invoked, to
...
SmithScript class, or a `ScriptModule`. Owns other its attribute types
...
`__smith__.MyModule.forward`). Owns the `Object`, which transitively owns the
`CompilationUnit`.
...
The rules for mangling are simple. Say we have a qualified name `__smith__.foo.Bar`:

```
__smith__.foo.Bar                    # first time, unchanged
__smith__.foo.__smith_mangle_0.Bar   # second time, when we request a mangle
__smith__.foo.__smith_mangle_1.Bar   # and so on
```
