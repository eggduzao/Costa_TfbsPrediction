...
struct SMITH_API Object {
  ...
  ObjectPtr _ivalue() const {
    SMITH_INTERNAL_ASSERT(_ivalue_);
    return _ivalue_;
  }
  ...
  void setattr(const std::string& name, c10::IValue v) {
    if (_ivalue()->type()->hasConstant(name)) {
      ...
      SMITH_CHECK(false, "Module has no attribute '", name, "'");
    }
  }
  ...
  const Property get_property(const std::string& name) const {
    ...
    SMITH_CHECK(false, "Property '", name, "' is not defined.");
  }
