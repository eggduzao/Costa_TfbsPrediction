...
namespace detail {
struct SMITH_API ModulePolicy;
struct SMITH_API ParameterPolicy;
struct SMITH_API AttributePolicy;
struct SMITH_API BufferPolicy;
template <typename P>
struct NamedPolicy;
} // namespace detail
...
struct SMITH_API Module : public Object {
  ...
  // A set of functions to maintain input shapes through smith.jit.save and
  // smith.jit.load. It only works on tensors and lists/dicts of tensors
...
// C++ equivalent api of `smith.jit.freeze`. See documentation there for
// details.
SMITH_API Module freeze(
    const Module& module,
    const std::optional<std::vector<std::string>>& preserved_attrs =
        std::nullopt,
    bool optimize_numerics = true);

// C++ equivalent api of `smith.jit.optimize_for_inference`. See documentation
// there for details.
SMITH_API Module optimize_for_inference(
    Module& module,
...
*/
// clang-format on
SMITH_API FusionStrategy getFusionStrategy();
// returns previous strategy
SMITH_API FusionStrategy setFusionStrategy(FusionStrategy& fusion_strategy);

namespace detail {

struct SMITH_API SlotCursor {
  Module module_;
...
SMITH_API bool& getInlineEverythingMode();

namespace script {
// We once had a `script::` namespace that was deleted. This is for backcompat
// of the public API; new code should not use this type alias.
using Module = ::smith::jit::Module;
using ExtraFilesMap = ::smith::jit::ExtraFilesMap;
} // namespace script
