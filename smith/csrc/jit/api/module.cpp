...
static ObjectPtr create_module_object(
  ...
  // If the name is unqualified, prepend a `__smith__`, similar to what Python
  // does with `__main__` for top-level code.
  if (class_name.prefix().empty()) {
    class_name = c10::QualifiedName("__smith__", class_name.name());
  }
...
void Method::run(Stack& stack) {
  ...
  RECORD_SMITHSCRIPT_FUNCTION(name(), stack);
  function_->run(stack);
}
...
void Method::setArgumentNames(
  ...
  SMITH_INTERNAL_ASSERT(function_);
...
Module Module::clone_impl(
  ...
  if (!type_already_cloned) {
    ...
    if (auto setstate_method = r.find_method("__setstate__")) {
      ...
      SMITH_INTERNAL_ASSERT(getstate_method, "expect __getstate__");
...
void Module::train(bool on) {
  for (Module m : modules()) {
    if (auto slot = m._ivalue()->type()->findAttributeSlot("training")) {
      ...
      /* SMITH_INTERNAL_ASSERT(false, "'training' attribute not found"); */
    }
  }
}
...
std::string Module::dump_to_str(
  ...
  for (const Method& method : get_methods()) {
    ...
    if (print_method_bodies) {
      methods_ss << smith::jit::jit_log_prefix(
  ...
  ss << "  parameters {" << '\n';
  ss << smith::jit::jit_log_prefix("    ", parameters_ss.str());
  ss << "  }" << '\n';
  ss << "  attributes {" << '\n';
  ss << smith::jit::jit_log_prefix("    ", attributes_ss.str());
  ss << "  }" << '\n';
  ss << "  methods {" << '\n';
  ss << smith::jit::jit_log_prefix("  ", methods_ss.str());
  ss << "  }" << '\n';
  ss << "  submodules {" << '\n';
...
smith::jit::Module IValue::toModule() const {
  return smith::jit::Module(toObject());
}
