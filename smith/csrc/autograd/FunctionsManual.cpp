...
void copy_range(variable_list& out, IndexRange range, const Tensor& t) {
  SMITH_CHECK(range.second <= out.size());
  SMITH_CHECK(
...
Tensor norm_jvp(
  ...
  SMITH_INTERNAL_ASSERT(!self_t._is_zerotensor());
...
Tensor masked_fill_backward(const Tensor& grad, const Tensor& mask) {
  // masked_select does not work well with funcsmith, as its shape is
  // data-dependent
...
std::vector<Tensor> cat_tensors_backward(
  ...
  for (const auto i : c10::irange(sizes.size())) {
    ...
    if (shape.size() == 1) {
      if (SMITH_GUARD_OR_FALSE(shape[0].sym_eq(0))) {
...
// Blacksmith ops and the gradient we got from as_strided.
//
...
// layout-aware.
//
// However, in Blacksmith, most (probably all) other ops (forward and backward) are
// layout-agnostic. E.g.,
//
//   t = smith.randn(1)
//   x = t.expand(2)
...
//   y.backward()
//
// Layout-agnostic autograd (as it is currently in Blacksmith) will give you
//
//   gy = 1
//   gx = [ 1, 1]  # SumBackward:    smith.ones_like(x)
...
std::tuple<Tensor, Tensor, Tensor> linalg_svd_jvp(
  ...
  SMITH_INTERNAL_ASSERT(U_.dim() >= 2 && Vh_.dim() >= 2);
...
Tensor svd_backward(
  ...
  // Checks compute_uv=true
  SMITH_INTERNAL_ASSERT(U.dim() >= 2 && Vh.dim() >= 2);
...
Tensor linalg_eig_backward(
  ...
  // This check just can be triggered in the backwards of smith.symeig
  SMITH_CHECK(
      symeig_eigenvectors,
      "linalg_eig_backward: smith.symeig(A, eigenvectors=False) is not differentiable. ",
      "Use smith.linalg.eigvalsh(A) instead.");
...
static Tensor masked_fmap(
  ...
  SMITH_INTERNAL_ASSERT(t.sym_numel() != 0);
...
Tensor log1p_backward(const Tensor& grad, const Tensor& self) {
  ...
  if (self.layout() == c10::kSparse || self.layout() == c10::kSparseCsr ||
    ...
    // strided layout, that would breaks funcsmith tests.
...
// ..., tau_k, the smith.linalg.householder_product computes the first n columns
...
Tensor i1_backward(
  ...
  return AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "i1_backward", [&]() {
    ...
    // Look at https://github.com/blacksmith/blacksmith/issues/52248
...
Tensor linalg_solve_jvp(
  ...
  SMITH_INTERNAL_ASSERT(left || !vector_case);
...
static Tensor _affine_jvp(
  ...
  SMITH_INTERNAL_ASSERT(input_p.has_value() == weight_p.defined());
  if (weight_p.defined()) {
