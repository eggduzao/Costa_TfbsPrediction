...
using namespace at;
using namespace smith::autograd::generated;
...
Tensor _fw_primal(c10::DispatchKeySet ks, const Tensor& self, int64_t level) {
  ...
  if (isFwGradDefined(self)) {
    ...
    SMITH_CHECK(level == 0, "Invalid level given to _fw_primal");
...
Tensor _make_dual(
  ...
  SMITH_CHECK(level == 0, "Invalid level given to _make_dual");
...
const Tensor& resize_(
  ...
  if (self.requires_grad()) {
    SMITH_CHECK(false, "cannot resize variables that require grad");
  }
  {
    at::AutoDispatchBelowAutograd mode;
  ...
  if (self._fw_grad(/* level */ 0).defined()) {
    SMITH_CHECK(false, "cannot resize variables that has a forward grad");
  }
...
Tensor& detach_(c10::DispatchKeySet ks, Tensor& self) {
  ...
  if (self.is_view()) {
    ...
    SMITH_CHECK(
        ...
        "Optimizer.zero_grad() function in smith/optim/optimizer.py "
        "as the solution.");
  }
...
// https://github.com/blacksmith/blacksmith/tree/master/aten/src/ATen/native#choosing-the-right-dispatch-keyword
// for more details.
// Invariant:
...
// `MANUAL_AUTOGRAD` in tools/autograd/gen_variable_type.blacksmith_LIBRARY_IMPL(aten, Autograd, m) {
  m.impl(
      "resize_",
      smith::dispatch(DispatchKey::Autograd, SMITH_FN(VariableType::resize_)));
  m.impl(
      "resize_as_",
      smith::dispatch(
          DispatchKey::Autograd, SMITH_FN(VariableType::resize_as_)));
  m.impl(
      "detach",
      smith::dispatch(DispatchKey::Autograd, SMITH_FN(VariableType::detach)));
  m.impl(
      "detach_",
      smith::dispatch(DispatchKey::Autograd, SMITH_FN(VariableType::detach_)));
  m.impl(
      "copy_",
      smith::dispatch(DispatchKey::Autograd, SMITH_FN(VariableType::copy_)));
  m.impl(
      "_fw_primal",
      smith::dispatch(
          DispatchKey::Autograd, SMITH_FN(VariableType::_fw_primal)));
  m.impl(
      "_make_dual",
      smith::dispatch(
          DispatchKey::Autograd, SMITH_FN(VariableType::_make_dual)));
}
...
static Tensor& copy_(
  ...
  smith::autograd::increment_version(self);
  return self;
}
...
static Tensor detach(c10::DispatchKeySet ks, const Tensor& self) {
  ...
  return ::smith::autograd::make_variable_non_differentiable_view(
      self,
      out,
...
static Tensor _fw_primal(
  ...
  std::unique_ptr<smith::autograd::ViewFunc> func(nullptr);
...
SMITH_LIBRARY_IMPL(aten, ADInplaceOrView, m) {
  m.impl(
      "copy_",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::copy_)));
  m.impl(
      "detach",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::detach)));
  m.impl(
      "resize_",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::resize_)));
  m.impl(
      "resize_as_",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::resize_as_)));
  m.impl(
      "_fw_primal",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::_fw_primal)));
  m.impl(
      "_make_dual",
      smith::dispatch(
          DispatchKey::ADInplaceOrView, SMITH_FN(ADInplaceOrView::_make_dual)));
}
} // namespace
} // namespace ADInplaceOrView
} // namespace smith
