...
void PyFunctionTensorPostAccGradHooks::apply_with_saved(
    Variable& tensor,
    smith::dynamo::autograd::SwapSavedVariables& saved) {
...
static variable_list unwrap_variables(PyObject* py_variables) {
  ...
  for (const auto i : c10::irange(results.size())) {
    ...
    if (item == Py_None) {
      ...
      SMITH_CHECK(false, "expected variable but got ", Py_TYPE(item)->tp_name);
    }
  }
  return results;
}
...
static void check_single_result(
  ...
  smith::autograd::check_variable_result(original, result, hook_name(hook));
}
