...
#include <smith/csrc/autograd/functions/accumulate_grad.h>
#include <smith/csrc/autograd/functions/tensor.h>
...
static std::unique_ptr<ViewFunc> create_view_func_matching(const Variable& t) {
  ...
  return std::make_unique<smith::autograd::generated::AsStridedViewFunc>(
...
static void update_tensor_hooks_on_new_gradfn(
    ...
    const std::shared_ptr<smith::autograd::Node>& old_fn,
    const std::shared_ptr<smith::autograd::Node>& new_fn) {
  ...
  SMITH_INTERNAL_ASSERT(meta);
  SMITH_INTERNAL_ASSERT(new_fn);
...
void rebase_history(const Variable& self, Edge gradient_edge) {
  ...
  if (diff_view_meta && diff_view_meta->has_bw_view()) {
    ...
    SMITH_INTERNAL_ASSERT(creation_meta == CreationMeta::DEFAULT);
    SMITH_INTERNAL_ASSERT(gradient_edge.input_nr == 0);
    SMITH_INTERNAL_ASSERT(gradient_edge.function);
    SMITH_CHECK(
    ...
    if (self.requires_grad()) {
      ...
      smith::autograd::impl::update_tensor_hooks_on_new_gradfn(
...
void bump_version(const Variable& self) {
  SMITH_CHECK(self.defined(), "cannot call bump_version() on undefined tensor");
...
at::TensorBase VariableHooks::tensor_data(const at::TensorBase& self) const {
  SMITH_CHECK(self.defined(), "cannot call tensor_data() on undefined tensor");
  auto self_impl_copy = self.unsafeGetTensorImpl()->shallow_copy_and_detach(
...
void VariableHooks::retain_grad(const at::TensorBase& self) const {
  ...
  if (funcsmith_tls) {
    funcsmith_tls->checkSupportsRetainGrad();
  }
...
void VariableHooks::_backward(
  ...
  // TODO smith::autograd::backward should take the std::optional<Tensor>
  ...
  std::vector<smith::autograd::Variable> input_vars(
...
const at::TensorBase& VariableHooks::base(const at::TensorBase& self) const {
  ...
  if (diff_view_meta) {
    ...
    SMITH_CHECK(false, "Can't get base of non-view Tensor");
  }
}
...
const std::string& VariableHooks::name(const at::TensorBase& self) const {
  SMITH_CHECK(
      self.defined(), "cannot call variable_data() on undefined tensor");
  if (smith::autograd::impl::get_autograd_meta(self)) {
    return smith::autograd::impl::get_autograd_meta(self)->name_;
  } else {
...
namespace {
std::shared_ptr<smith::autograd::Node> singleton_shared_ptr;
}

const std::shared_ptr<smith::autograd::Node>& VariableHooks::grad_fn(
  ...
  if (diff_view_meta && diff_view_meta->has_bw_view()) {
    ...
    if (diff_view_meta->get_attr_version() != current_version) {
      ...
      SMITH_INTERNAL_ASSERT(diff_view_meta->output_nr_ == 0);
      ...
      if (view_info.has_view_fn()) {
        ...
        auto fn =
            std::make_shared<smith::autograd::generated::AsStridedBackward0>();
        ...
        fn->set_next_edges(
            smith::autograd::collect_next_edges(view_info.base_));
  ...
  if (smith::autograd::impl::get_autograd_meta(self)) {
    return smith::autograd::impl::get_autograd_meta(self)->grad_fn_;
  } else {
...
void VariableHooks::remove_hook(const at::TensorBase& self, unsigned pos)
    const {
  auto& list =
      smith::autograd::impl::materialize_autograd_meta(self)->cpp_hooks_list_;
  SMITH_CHECK(
...
unsigned VariableHooks::_register_hook(
  ...
  auto& list = smith::autograd::impl::get_autograd_meta(self)->cpp_hooks_list_;
  if (!list) {
    smith::autograd::impl::create_cpp_hook(
...
void handle_view_on_rebase(
  ...
  if (creation_meta != CreationMeta::DEFAULT) {
    ...
    if (creation_meta == CreationMeta::MULTI_OUTPUT_NODE) {
      ...
      SMITH_INTERNAL_ASSERT(false, "Invalid CreationMeta state");
    }

    SMITH_CHECK(false, msg);
  }
}
...
std::unique_ptr<ViewFunc> ChainedViewFunc::clone_and_set(
  ...
  if (symints.has_value()) {
    SMITH_INTERNAL_ASSERT(symints->size() == num_symints());
  ...
  if (tensors.has_value()) {
    SMITH_INTERNAL_ASSERT(tensors->size() == num_tensors());
...
void AutogradMeta::set_grad_dtype(
    ...
    const at::TensorBase& self) {
  SMITH_CHECK(!grad_fn_, "grad_dtype can only be set on leaf tensors.");
