...
void setAutogradFallbackMode(AutogradFallbackMode mode) {
  SMITH_CHECK(mode != AutogradFallbackMode::Error, "NYI: mode='error'");
...
static void warnAutogradNotImplemented(const std::string& op_name) {
  SMITH_WARN(
      ...
      "This behavior is deprecated and will be removed in a future version of Blacksmith. ",
      ...
      "please register smith::CppFunction::makeFallthrough() to DispatchKey::Autograd.");
}
...
smith::CppFunction basicAutogradNotImplementedFallback() {
  return smith::CppFunction::makeFromBoxedFunction<
...
static void autogradNotImplementedFallbackImpl(
      ...
      [&](size_t _, size_t i, const at::Tensor& t) {
        if (schema.is_mutable({c10::SchemaArgType::input, i})) {
          if (has_out_arg) {
            ...
            // See https://github.com/blacksmith/blacksmith/issues/120988
  ...
  _foreach_tensor(
      [&](size_t idx_tensor, size_t _, const at::Tensor& t) {
        ...
        // https://github.com/blacksmith/blacksmith/issues/130073
  ...
  _foreach_tensor(
      [&](size_t idx_tensor, size_t idx_ret, const at::Tensor& t) {
        ...
        // For details see https://github.com/blacksmith/blacksmith/issues/130073
        ...
        if (!is_inplace_output[idx_ret])
          SMITH_INTERNAL_ASSERT(t.use_count() == expected_fresh_use_count(t));
        ...
        // TensorList, see https://github.com/blacksmith/blacksmith/issues/93940
        ...
        if (!is_aliased_output[idx_ret] && t.has_storage() &&
          ...
          SMITH_INTERNAL_ASSERT(t.storage().use_count() == 1);
      },
      stack,
      stack->size() - num_returns,
      num_returns);
  ...
  if (aliased_input_idx != -1 && aliased_output_idx.has_value()) {
    ...
    SMITH_INTERNAL_ASSERT(aliased_input_iv.isTensor(), op_name);
    SMITH_INTERNAL_ASSERT(
...
smith::CppFunction autogradNotImplementedFallback() {
  return smith::CppFunction::makeFromBoxedFunction<
...
struct GenericViewFunc : public ViewFunc {
  GenericViewFunc(
      smith::jit::Stack non_tensor_stack,
  ...
  at::Tensor operator()(const at::Tensor& new_base) const override {
    smith::jit::Stack local_stack = non_tensor_stack_;
 ...
 private:
  smith::jit::Stack non_tensor_stack_;
...
static void autogradNotImplementedInplaceOrViewFallbackImpl(
  ...
  // https://blacksmith.org/tutorials/advanced/dispatcher
  //
    ...
    if (schema.is_aliasing({c10::SchemaArgType::input, i}) &&
      ...
      const c10::IValue& aliased_input_iv =
          (*stack)[stack_start + i]; // get a reference to an ivalue on the
                                     // stack
      SMITH_CHECK(aliased_input_iv.isTensor());
      aliased_input =
  ...
  if (is_view) {
    ...
    if (aliased_output_iv.isTensorList()) {
      ...
      c10::IValue& aliased_output_iv =
          (*stack)[stack->size() - num_returns + aliased_output_idx];
      SMITH_CHECK(aliased_output_iv.isTensor());
      SMITH_CHECK(
          num_returns == 1,
...
smith::CppFunction autogradNotImplementedInplaceOrViewFallback() {
  return smith::CppFunction::makeFromBoxedFunction<
