...
namespace smith::autograd::generated::details {

extern const char* kCudnnDoubleBackwardMsg;

// A simple way to imperatively compute index ranges for slots
// that have been flattened
struct SMITH_API IndexRangeGenerator {
...
SMITH_API Tensor toNonOptFwGrad(const std::optional<Tensor>& t);
SMITH_API Tensor toNonOptPrimal(const std::optional<Tensor>& t);
SMITH_API Tensor toNonOptTensor(const std::optional<Tensor>& t);
...
SMITH_API bool any_variable_defined(const variable_list& variables);
SMITH_API void update_wrapped_number(Tensor& input, Tensor& output);
SMITH_API void copy_range(
    variable_list& out,
    IndexRange range,
    const at::Tensor& t);
SMITH_API void copy_range(
...
SMITH_API at::Tensor copysign_tensor_self_backward(
    const Tensor& grad,
    const Tensor& self,
    const Tensor& result);
SMITH_API at::Tensor not_implemented(const char* name, const char* reason = "");
SMITH_API std::vector<Tensor> not_implemented_list(
    const char* name,
    const char* reason = "");
...
Tensor chunk_backward_nested(
    const std::vector<smith::autograd::Variable>& grads,
    const Tensor& self,
    int64_t chunks,
    int64_t dim);
