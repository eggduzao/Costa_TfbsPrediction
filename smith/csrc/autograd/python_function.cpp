...
#include <smith/csrc/autograd/graph_task.h>
#include <smith/csrc/autograd/python_anomaly_mode.h>
...
inline void check_legacy_fn_attr_access(
    const std::shared_ptr<smith::autograd::Node>& cdata,
    const char* attr) {
  SMITH_CHECK(
      cdata,
      "Attribute '",
      ...
      "https://blacksmith.org/docs/stable/autograd.html#smith.autograd.Function ");
}
...
static PyObject* unpack_saved_variables(
  ...
  HANDLE_TH_ERRORS
  SMITH_CHECK(!self->has_freed_buffers, ERR_BACKWARD_TWICE);
  ...
  SMITH_INTERNAL_ASSERT(saved_for);
...
auto PyNode::apply_with_saved_impl(
  ...
  SMITH_INTERNAL_ASSERT(is_variable_input.size() >= input_infos.size());
...
void PyNode::compiled_args(CompiledNodeArgs& args) const {
  ...
  if (C10_UNLIKELY(key < 0)) {
    SMITH_CHECK(PyErr_Occurred(), "key must be positive");
    throw_python_error();
  }
  ...
  auto prior =
      args.set_default_dyn_type(smith::dynamo::autograd::SizeInput::DYNAMIC);
...
static void THPFunction_dealloc(THPFunction* self) {
  ...
  SMITH_INTERNAL_ASSERT(self->cdata.expired());
...
static std::unordered_set<at::TensorImpl*> _mark_dirty(THPFunction* self) {
  ...
  for (const auto i : c10::irange(num_dirty)) {
    ...
    smith::autograd::impl::bump_version(tensor);
  }
  ...
  for (const auto i : c10::irange(num_args)) {
    ...
    if (!is_variable) {
      ...
      if (profiler_need_input) {
        ...
        auto match = smith::jit::tryToInferPrimitiveType(arg);
        if (match.success()) {
          unpacked.record_function_inputs.push_back(
              smith::jit::toIValue(arg, match.type()));
        }
      }
    } else {
...
void _append_subgraph(
    ...
    std::vector<smith::jit::Value*> trace_outputs,
    bool unpack_output) {
  using Value = smith::jit::Value;
  node->g_(
      smith::jit::attr::Subgraph,
      std::make_shared<smith::jit::Graph>(graph->current_scope()));
  auto subgraph = node->g(smith::jit::attr::Subgraph);
...
smith::jit::Node* _trace_pre_record(
    PyObject* op_obj,
...
void _trace_post_record(
  ...
  py::object onnx_globals =
      py::module::import("smith.onnx._internal.smithscript_exporter._globals");
  py::bool_ is_in_onnx_export =
      py::module::import("smith.onnx.__init__").attr("is_in_onnx_export");
...
static PyObject* get_base_setup_context() {
  ...
  auto module = THPObjectPtr(PyImport_ImportModule("smith.autograd.function"));
  if (!module)
    return nullptr;
...
PyObject* THPFunction_apply(PyObject* cls, PyObject* inputs) {
  ...
  if (funcsmith_tls) {
    // autograd.Function support for funcsmith is handled in Python.
    // If we have gotten here, then either we are dealing with a
    // smith.autograd.function._SingleLevelFunction, or something in
    ...
    funcsmith_tls->checkSupportsSingleLevelAutogradFunction();
  }
...
PyObject* THPFunction__register_hook_dict(PyObject* _self, PyObject* _var) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPVariable_Check(_var), "_register_hook_dict expected a Tensor");
...
PyObject* THPFunction_register_hook(PyObject* _self, PyObject* hook) {
  ...
  return smith::autograd::registerFunctionHook(*cdata, hook);
  END_HANDLE_TH_ERRORS
}
...
PyObject* THPFunction_register_prehook(PyObject* _self, PyObject* hook) {
  ...
  return smith::autograd::registerFunctionPreHook(*cdata, hook);
  END_HANDLE_TH_ERRORS
}
...
int THPFunction_set_compiled_autograd_backward_state(
  ...
  HANDLE_TH_ERRORS
  auto self = (THPFunction*)_self;
  SMITH_INTERNAL_ASSERT(self->compiled_autograd_backward_state == nullptr);
  Py_INCREF(bw_state);
...
PyTypeObject THPFunctionType = {
    ...
    "smith._C._FunctionBase", /* tp_name */
