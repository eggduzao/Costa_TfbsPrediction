...
variable_list Scatter::apply(variable_list&& inputs) {
  ...
  auto tensors =
      smith::cuda::scatter(input, device_indices, chunk_sizes_, dim_, streams_);
...
variable_list Gather::apply(variable_list&& inputs) {
  ...
  // smith::cuda::gather does not return a view or change things inplace
  // so no need for extra logic here
  at::Tensor variable;
  {
    at::AutoDispatchBelowAutograd mode;
    // This is special logic for smith::cuda::gather!
    const auto destination_index =
        destination_device_.is_cpu() ? -1 : destination_device_.index();
    variable = smith::cuda::gather(tensors, dim_, destination_index);
  }
  if (grad_fn) {
