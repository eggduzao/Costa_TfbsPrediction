...
variable_list AccumulateGrad::apply_with_saved(
  ...
  SMITH_INTERNAL_ASSERT(!variable.grad_fn() && grads.size() == 1);
  ...
  // smith::autograd::AccumulateGrad For Compiled Autograd, we just want the op
  // name without the namespace
  std::string name = "AccumulateGrad";

  // proxy a call to smith.ops.inductor.accumulate_grad_.default
