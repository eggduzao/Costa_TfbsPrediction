...
#include <smith/csrc/autograd/functions/pybind.h>
...
struct DelayedErrorCtor {
  DelayedError* operator()(PyObject* args) {
    ...
    SMITH_CHECK(THPUtils_checkString(arg1), "argument 'msg' must be a string");
...
struct NoCtor {
  Node* operator()(PyObject* args) {
    SMITH_CHECK(false, "Cannot construct");
  }
};
...
void THPAutograd_initFunctions() {
  THPObjectPtr module(PyModule_New("smith._C._functions"));
  if (!module)
    throw python_error();
  ...
  addClass<smith::distributed::autograd::SendRpcBackward, NoCtor>(
  ...
  auto c_module = THPObjectPtr(PyImport_ImportModule("smith._C"));
  if (!c_module)
    throw python_error();
