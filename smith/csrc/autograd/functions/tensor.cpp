...
using smith::dynamo::autograd::IValuePacker;
...
variable_list CopyBackwards::apply_with_saved(
  ...
  static bool flag [[maybe_unused]] = [&]() {
    ...
    const auto& interface = smith::dynamo::autograd::getPyCompilerInterface();
...
void CopySlices::update_exec_info() {
  ...
  if (exec_info && !exec_info->empty()) {
    ...
    SMITH_INTERNAL_ASSERT(fn_edge.is_valid() == this_edge.is_valid());
    if (fn_edge.is_valid()) {
  ...
  // read-only!)
  SMITH_INTERNAL_ASSERT(num_outputs() == fn->num_outputs());
...
inline variable_list CopySlices::apply_impl(
  ...
  SMITH_CHECK(fn, ERR_BACKWARD_TWICE);
...
void CopySlices::compiled_args(CompiledNodeArgs& args) const {
  SMITH_CHECK(!view_fn, "view_fn not supported by compiled autograd")
  SMITH_INTERNAL_ASSERT((bool)fn);
  args.collect(base);
  args.collect(view);
  args.collect(fn);
...
variable_list CopySlices::apply_with_saved(
  ...
  if (grads[0].defined()) {
    ...
    // (this only happens by default with smith-xla).
    SMITH_INTERNAL_ASSERT(!view_fn);
    const auto& interface = smith::dynamo::autograd::getPyCompilerInterface();
    variable_list stuff = interface->call_copy_slices_prologue(
        saved.get_py_compiler(), grads, base, view);
    SMITH_INTERNAL_ASSERT(stuff.size() == 3);
