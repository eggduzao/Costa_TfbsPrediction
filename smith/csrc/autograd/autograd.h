...
/// (`smith::Tensor()` is an acceptable value for all tensors that don't need
/// gradient tensors).
///
...
/// gradients
///     w.r.t. each element of corresponding tensors. `smith::Tensor()` values
///     can be specified for scalar Tensors or ones that don't require grad. If
///     a `smith::Tensor()` value would be acceptable for all grad_tensors, then
...
//      https://github.com/blacksmith/blacksmith/pull/60521#issuecomment-867061780 for
//      more details.
SMITH_API void backward(
...
/// then the gradient can be ``smith::Tensor()``).
///
...
///     Usually gradients w.r.t. each output. `smith::Tensor()` values can be
///     specified for scalar Tensors or ones that don't require grad. If a
///     `smith::Tensor()` value would be acceptable for all grad_tensors, then
...
SMITH_API uint64_t enter_dual_level();
...
SMITH_API void exit_dual_level(uint64_t level);
