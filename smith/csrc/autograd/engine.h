...
#include <smith/csrc/autograd/utils/warnings.h>
...
SMITH_API void validate_outputs(
...
SMITH_API void validate_outputs(
    ...
    variable_list& grads,
    const std::function<std::string(const std::string&)>& format_error);
SMITH_API std::vector<std::optional<InputMetadata>> collect_input_metadata(
    const edge_list& edges);
...
SMITH_API void set_default_engine_stub(EngineStub stub);
