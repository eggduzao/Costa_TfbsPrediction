...
#include <smith/csrc/autograd/cpp_hook.h>
#include <smith/csrc/autograd/edge.h>
#include <smith/csrc/autograd/forward_grad.h>
...
// The following are all internal APIs and should not be shown in libsmith docs.
...
/// smith/autograd/__init__.py file and
...
SMITH_API AutogradMeta* get_autograd_meta(const at::TensorBase& /*self*/);

// WARNING: This will return a nullptr if the Tensor is not a view.
SMITH_API DifferentiableViewMeta* get_view_autograd_meta(
...
SMITH_API AutogradMeta* materialize_autograd_meta(
    const at::TensorBase& /*self*/);
...
SMITH_API void set_grad_accumulator(
    const Variable& /*self*/,
...
SMITH_API std::shared_ptr<Node> try_get_grad_accumulator(
    const Variable& /*self*/);
SMITH_API std::shared_ptr<Node> try_get_grad_accumulator(
    const at::TensorBase& /*self*/);
...
SMITH_API std::shared_ptr<Node> grad_accumulator(const Variable& /*self*/);
...
SMITH_API Edge gradient_edge(const Variable& /*self*/);
...
/// `Variable`.
SMITH_API void set_gradient_edge(const Variable& /*self*/, Edge edge);
...
/// Variable.
SMITH_API void rebase_history(const Variable& /*self*/, Edge gradient_edge);

/// Gets the raw gradient function pointer, whatever it currently is.
SMITH_API Node* grad_fn_unsafe(const Variable& /*self*/);

/// Increments the version count of this `Variable`.
SMITH_API void bump_version(const Variable& /*self*/);
SMITH_API void set_version_counter(
    const Variable& /*self*/,
    const c10::VariableVersion& version_counter);

/// Retrieves this `Variable`s version counter.
SMITH_API const c10::VariableVersion& version_counter(const Variable& /*self*/);

SMITH_API void set_name(const Variable& /*self*/, const std::string& name);

SMITH_API void add_hook(
    const at::TensorBase& /*self*/,
    std::unique_ptr<FunctionPreHook> hook);
SMITH_API std::vector<std::unique_ptr<FunctionPreHook>>& hooks(
    const Variable& /*self*/);
SMITH_API void clear_hooks(const at::TensorBase& /*self*/);

SMITH_API void set_post_acc_grad_hooks(
    const at::TensorBase& /*self*/,
    std::unique_ptr<PostAccumulateGradHook> dict);
SMITH_API std::unique_ptr<PostAccumulateGradHook>& post_acc_grad_hooks(
    const Variable& /*self*/);

SMITH_API void create_cpp_hook(
    const at::TensorBase& /*self*/,
...
struct SMITH_API AutogradMeta : public c10::AutogradMetaInterface {
  std::string name_;
  ...
  AutogradMeta(
    ...
    if (requires_grad) {
      SMITH_INTERNAL_ASSERT(self_impl);
...
struct SMITH_API ViewInfo {
  ...
  ViewInfo(
    ...
    SMITH_CHECK(base_.defined(), "base is undefined");
  }
};
...
/// Variable.
///
/// In Blacksmith, we have two types of views: differentiable views, and
...
///     smith.autograd.grad(base.sum(), var)  <- should return an all ones
///     tensor
///
...
///     view = base[1]
///     base.copy_(var)
///     smith.autograd.grad(view.sum(), var)  <- should return a tensor with
...
///     base.requires_grad_()
///     view.copy_(var)
///     smith.autograd.grad(base.sum(), var)  <- what should it return?
///
...
SMITH_API void handle_view_on_rebase(
    DifferentiableViewMeta* diff_view_meta,
    bool indirect = false);

struct SMITH_API DifferentiableViewMeta : public AutogradMeta {
 private:
...
inline Variable make_variable_differentiable_view(
  ...
  if (data.defined()) {
    SMITH_CHECK(
        ...
        "using a __smith_dispatch__ mode, the most common cause for this "
        "problem is that you used smith.overrides.enable_reentrant_dispatch() "
        "improperly; tensors created within the extent of reentrant dispatch "
        "MUST NOT be directly returned from __smith_dispatch__; instead, they "
        ...
        "If this doesn't seem applicable, please file a bug to Blacksmith.");
...
struct VariableHooks final : at::impl::VariableHooksInterface {
  ...
  at::TensorBase variable_data(
      const at::TensorBase& /*self*/ /*unused*/) const override;
  const std::shared_ptr<smith::autograd::Node>& grad_fn(
      const at::TensorBase& /*self*/ /*unused*/) const override;
  ...
  void basic_autograd_not_implemented_fallback(
      ...
      smith::jit::Stack* stack) const override;
...
SMITH_API bool has_same_meta(const Variable& base, const Variable& other);
