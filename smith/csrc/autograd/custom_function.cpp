...
static void _process_forward_mode_AD(
  ...
  smith::autograd::variable_list forward_grads;
  {
...
static at::Tensor _view_as_self_with_no_grad(
  ...
  // view_as from python so that smith function logic can still trigger.
  ...
  auto set_history = [&](Variable& var,
    ...
    if (!is_differentiable) {
      ...
      if (!is_input) {
        ...
        SMITH_CHECK(!var.is_view(), mark_dirty_error_msg);
        SMITH_WARN(mark_dirty_error_msg);
      }
      ...
      SMITH_CHECK(
          ...
          " that does the other operations). You can ask on the forum https://discuss.blacksmith.org/ if"
      ...
      SMITH_CHECK(!is_saved_and_setup_context, error_msg_input_returned_as_is)
      var = _view_as_self_with_no_grad(var, view_as_self_fn);
...
variable_list AutogradContext::get_saved_variables() const {
  SMITH_CHECK(!has_freed_buffers_, ERR_BACKWARD_TWICE);
