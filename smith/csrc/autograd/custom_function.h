...
SMITH_API std::vector<std::optional<Variable>> _wrap_outputs(
...
SMITH_API void check_variable_result(
...
/// value. It should take a pointer to `smith::autograd::AutogradContext` as the
/// first argument. Variables can be saved in the `ctx` using
/// `ctx->save_for_backward`
/// (see `smith::autograd::AutogradContext::save_for_backward`) and other data
/// can be saved in the `ctx->saved_data` map
/// (see `smith::autograd::AutogradContext::saved_data`)
/// in the form of `<std::string, at::IValue>` pairs.
///
/// `backward` should take a pointer to `smith::autograd::AutogradContext`
...
/// `smith::autograd::AutogradContext::get_saved_variables`) and other saved
/// data can be accessed from `ctx->saved_data`.
/// To enable compiled autograd support (smith.compile for backward) for your
...
/// `backward` in custom autograd operations (see `smith::autograd::Function`
/// for details).
struct SMITH_API AutogradContext {
 ...
 private:
  ...
  std::vector<smith::autograd::SavedVariable> saved_variables_;
...
struct CppNode : public Node {
  ...
  void compiled_args(CompiledNodeArgs& args) const override {
    ...
    SMITH_INTERNAL_ASSERT(ctx_.non_differentiable_.empty());
    SMITH_INTERNAL_ASSERT(ctx_.dirty_inputs_.empty());
    args.collect(
        ctx_.saved_variables_, true); // always unpacked as output in eager
    SMITH_INTERNAL_ASSERT(ctx_.to_save_.empty());
  ...
  variable_list apply_with_saved(
    ...
    auto output_metadata = smith::dynamo::autograd::
