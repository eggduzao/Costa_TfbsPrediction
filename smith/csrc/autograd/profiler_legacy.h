...
struct SMITH_API LegacyEvent {
  ...
  LegacyEvent(
    ...
    SMITH_INTERNAL_ASSERT(cpu_ns_ > 0);
    if (cuda_recorded) {
      SMITH_INTERNAL_ASSERT(device_ >= 0);
      SMITH_INTERNAL_ASSERT(cuda_us_ >= 0);
    }
  }
  ...
  std::string kindStr() const {
    switch (kind_) {
      ...
      case EventKind::MemoryAlloc:
        return "memory_alloc";
      default:
        SMITH_CHECK(false, "unknown event kind");
    }
  }
 ...
 private:
  ...
  smith::profiler::impl::ProfilerVoidEventStub cuda_event = nullptr;
  int64_t node_id_ = 0;
...
struct SMITH_API ProfilerDisableOptions {
...
SMITH_API void addEventList(std::vector<LegacyEvent>&& profiledEvents);
// Writes profiled events to a stream.
SMITH_API void writeProfilerEventsToStream(
    std::ostream& out,
...
struct SMITH_API RecordProfile {
...
//   Code to profile
// }
struct SMITH_API TLSLegacyProfilerGuard {
  explicit TLSLegacyProfilerGuard(
      const smith::profiler::impl::ProfilerConfig& cfg,
