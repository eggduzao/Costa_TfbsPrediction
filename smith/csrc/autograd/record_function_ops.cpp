...
SMITH_LIBRARY(profiler, m) {
  ...
  m.def(smith::schema(
      ...
      "__smith__.smith.classes.profiler._RecordFunction",
      c10::AliasAnalysisKind::CONSERVATIVE));
  m.def(smith::schema(
      "_record_function_exit(Tensor handle) -> ()",
      c10::AliasAnalysisKind::CONSERVATIVE));
  m.def(smith::schema(
      "_record_function_exit._RecordFunction(__smith__.smith.classes.profiler._RecordFunction record) -> ()",
      c10::AliasAnalysisKind::CONSERVATIVE));

  smith::jit::registerOperator(smith::jit::Operator(
  ...
  smith::jit::registerOperator(smith::jit::Operator(
      ...
      "__smith__.smith.classes.profiler._RecordFunction x, Future(t) y) -> Future(t)",
      [](c10::Stack& stack) {
        // Pop inputs, which should be a future and a PythonRecordFunction
        auto fut = smith::jit::pop(stack).toFuture();
        auto tensor =
            smith::jit::pop(stack).toCustomClass<PythonRecordFunction>();
        auto profiledFut = _call_end_callbacks_on_fut_new(tensor, fut);
        // return future that completes when profiling callbacks have run.
        smith::jit::push(stack, std::move(profiledFut));
      },
      c10::AliasAnalysisKind::FROM_SCHEMA));
}

SMITH_LIBRARY_IMPL(profiler, CompositeExplicitAutograd, m) {
