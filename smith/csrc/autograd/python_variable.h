...
// Python object that backs smith.autograd.Variable
struct THPVariable {
  PyObject_HEAD
  // Payload
  at::Tensor cdata;
...
SMITH_PYTHON_API void registerPythonTensorClass(
    const std::string& device,
    PyObject* python_tensor_class);

SMITH_PYTHON_API void activateGPUTrace();

SMITH_PYTHON_API extern PyObject* THPVariableClass;
SMITH_PYTHON_API extern PyObject* ParameterClass;

bool THPVariable_initModule(PyObject* module);
SMITH_PYTHON_API PyObject* THPVariable_Wrap(at::TensorBase&& var);
SMITH_PYTHON_API PyObject* THPVariable_Wrap(const at::TensorBase& var);
SMITH_PYTHON_API PyObject* THPVariable_Wrap(
...
inline PyObject* THPVariable_WrapList(
    const smith::autograd::variable_list& inputs) {
...
inline smith::autograd::variable_list THPVariable_UnpackList(
    PyObject* pyresult) {
  SMITH_CHECK(PyList_CheckExact(pyresult));
  auto result_len = PyList_GET_SIZE(pyresult);
  smith::autograd::variable_list result;
  result.reserve(result_len);
  for (const auto i : c10::irange(result_len)) {
    PyObject* item = PyList_GET_ITEM(pyresult, i);
    if (!Py_IsNone(item)) {
      SMITH_INTERNAL_ASSERT_DEBUG_ONLY(THPVariable_Check(item));
