...
namespace smith::autograd::python {
...
Engine& PythonEngine::get_python_engine() {
  ...
  if (_reinitialize_engine) {
    ...
    engine.~PythonEngine();
    new (&engine) smith::autograd::python::PythonEngine();
...
variable_list PythonEngine::execute(
  ...
  SMITH_CHECK(
      ...
      ". If you are not using the C++ API, please report a bug to the blacksmith team.")
  try {
...
static Edge parseGradientEdge(PyObject* obj, int64_t index) {
  ...
  std::shared_ptr<smith::autograd::Node> grad_fn_sp;
...
// Implementation of smith._C._EngineBase.run_backward
static PyObject* THPEngine_run_backward(
  ...
  SMITH_CHECK(
      ...
      "backward() called inside smith.vmap. This is not supported, "
      "please call backward() outside smith.vmap or instead use "
      "smith.autograd.grad inside smith.vmap");
  ...
  for (const auto i : c10::irange(num_tensors)) {
    ...
    if (THPVariable_Check(_tensor)) {
      ...
      SMITH_CHECK(
          ...
          "smith.autograd.grad(outputs, inputs, grad_outputs) called inside ",
          "smith.vmap. We do not support the case where any outputs are ",
          "vmapped tensors (output ",
          i,
          " is being vmapped over). Please "
          "call autograd.grad() outside smith.vmap or file a bug report "
          "with your use case.");
      gradient_edge = smith::autograd::impl::gradient_edge(mb_output.value());
    ...
    for (const auto i : c10::irange(num_inputs)) {
      ...
      if (THPVariable_Check(input)) {
        ...
        SMITH_CHECK(
            ...
            "smith.autograd.grad(outputs, inputs, grad_outputs) called inside ",
            "smith.vmap. We do not support the case where any inputs are ",
        ...
        if (!grad_fn) {
          grad_fn = smith::autograd::impl::try_get_grad_accumulator(tensor);
        }
        if (accumulate_grad) {
          tensor.retain_grad();
        }
