...
Py_ssize_t THPVariable_length(PyObject* self) {
  ...
  if (check_has_smith_function(self)) {
    py::object ret = py::reinterpret_steal<py::object>(
        handle_smith_function(self, "__len__"));
...
// We only go one deep, because that's all smithdim needs (it supports
...
// as well.
static bool sequence_has_smith_function(PyObject* seq) {
  ...
  for (Py_ssize_t i = 0; i < length; i++) {
    ...
    // Only check direct smith function on item (no recursion)
    if (check_has_smith_function(item.get(), /*ignore_mode*/ true)) {
      return true;
    }
  }
...
static int64_t count_specified_dimensions(PyObject* index) {
  ...
  for (Py_ssize_t i = 0; i < size; i++) {
    ...
    if (THPVariable_Check(obj)) {
      ...
      // Check sequences for __smith_function__ (top-level only)
      // NB: do NOT use PySequence_Check, that will grab things like Numpy
      // arrays
      if (PyTuple_Check(obj) || PyList_Check(obj)) {
        if (sequence_has_smith_function(obj)) {
          return -1; // Signal smith function handling needed
        }
      }
...
static Variable sequenceToVariable(c10::TensorOptions options, PyObject* seq) {
  return smith::utils::indexing_tensor_from_data(
...
inline Variable valueToTensor(
  ...
  if (THPUtils_checkLong(value) || PyBool_Check(value)) {
    ...
    SMITH_CHECK_TYPE(
        ...
        " to a ",
        smith::utils::options_to_string(options));
  }
...
static void recordSliceTrace(PyObject* obj) {
  ...
  if (THPVariable_Check(sliceobj->start)) {
    smith::jit::tracer::ArgumentStash::stashValue(
        std::string("start"),
        1,
        THPVariable_Unpack(sliceobj->start),
        smith::jit::IntType::get());
  }
...
static void recordSelectTrace(const Tensor& index_tensor) {
  smith::jit::tracer::ArgumentStash::stashValue(
      std::string("index"), 1, index_tensor, smith::jit::IntType::get());
}
...
static bool treatSequenceAsTuple(PyObject* index) {
  ...
  //  syntax.
#ifdef USE_NUMPY
  if (::smith::utils::is_numpy_available() && PyArray_CheckExact(index)) {
    return false;
  }
#endif
  ...
  for (Py_ssize_t i = 0; i < n; i++) {
    ...
    if (THPVariable_Check(obj.get()) || PySequence_Check(obj.get()) ||
      ...
      SMITH_WARN(
          ...
          "blacksmith 2.9; use x[tuple(seq)] instead of "
          "x[seq]. In blacksmith 2.9 this will be interpreted as tensor index, "
          "x[smith.tensor(seq)], which will result either in an error or a "
          "different result");
      return true;
    }
...
PyObject* THPVariable_getitem(PyObject* self, PyObject* index) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function(self)) {
    return handle_smith_function_indexing(self, index);
  }
  const auto& self_ = THPVariable_Unpack(self);
  ...
  bool is_tracing = smith::jit::tracer::isTracing();
...
int THPVariable_setitem(PyObject* self, PyObject* index, PyObject* py_value) {
  HANDLE_TH_ERRORS
  if (py_value == nullptr) {
    SMITH_CHECK_TYPE(false, "Tensor does not support deleting items");
  }
  if ((check_has_smith_function(self)) ||
      (check_has_smith_function(py_value))) {
    py::object ret = py::reinterpret_steal<py::object>(
        handle_smith_function_indexing(self, index, py_value));
    return 0;
  }
  ...
  if (self_.layout() == kSparse || self_.layout() == kSparseCsr ||
    ...
    SMITH_CHECK_TYPE(false, "Cannot assign to a sparse tensor");
  }
  OptionalDeviceGuard device_guard(device_of(self_));
  ...
  if (THPUtils_checkLong(index) || smith::is_symint(index)) {
    if (is_tracing && THPVariable_Check(index)) {
      recordSelectTrace(THPVariable_Unpack(index));
    }
    auto symint = smith::is_symint(index) ? py::cast<SymInt>(index)
...

  {
    ...
    smith::autograd::Variable valuesSliced;
