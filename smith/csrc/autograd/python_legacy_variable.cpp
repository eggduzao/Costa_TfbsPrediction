...
static PyObject* THPVariable_pynew(
  ...
  if (is_volatile) {
    auto r = PyErr_WarnEx(
        PyExc_UserWarning,
        "volatile was removed and now has no effect. Use `with smith.no_grad():` "
        "instead.",
        1);
    if (r != 0)
      throw python_error();
  ...
  if (!data || data == Py_None) {
    ...
    auto dispatch_key = smith::tensors::get_default_dispatch_key();
...
static PyTypeObject THPLegacyVariableType = {
    ...
    "smith._C._LegacyVariableBase", /* tp_name */
    0, /* tp_basicsize */
    0, /* tp_itemsize */
    nullptr, /* tp_dealloc */
