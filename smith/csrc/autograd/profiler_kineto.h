...
using experimental_event_t = std::shared_ptr<smith::profiler::impl::Result>;
using extra_meta_t = std::unordered_map<std::string, std::string>;

struct SMITH_API KinetoEvent {
  KinetoEvent(
  ...
  void getPerfEventCounters(smith::profiler::perf_counters_t& /*in*/) const;
  extra_meta_t extraMeta() const;
  std::string metadataJson() const;

 private:
  smith::profiler::impl::ProfilerVoidEventStub fallbackStart() const;
  smith::profiler::impl::ProfilerVoidEventStub fallbackEnd() const;

  std::shared_ptr<const smith::profiler::impl::Result> result_;
...
struct SMITH_API ProfilerResult {
  ProfilerResult();
  ProfilerResult(
      uint64_t start_time,
 ...
 private:
  ...
  std::unique_ptr<smith::profiler::impl::kineto::ActivityTraceWrapper> trace_;
  std::vector<experimental_event_t> event_tree_;
};

/*
 * This API is used by backends to record latency of events that
 * happened in the backend but were not visible to blacksmith runtime.
 ...
 */
SMITH_API void reportBackendEventToActiveKinetoProfiler(
 ...
 * KinetoEdgeCPUProfiler, in smith/csrc/jit/mobile/profiler_edge.cpp, enables
...
SMITH_API void enableProfilerWithEventPostProcess(
    const smith::profiler::impl::ProfilerConfig& config,
    const std::set<smith::profiler::impl::ActivityType>& activities,
    post_process_t&& cb,
    const std::unordered_set<at::RecordScope>& scopes = {});

SMITH_API std::unique_ptr<ProfilerResult> disableProfiler();

SMITH_API void prepareProfiler(
    const smith::profiler::impl::ProfilerConfig& config,
    const std::set<smith::profiler::impl::ActivityType>& activities);

SMITH_API void toggleCollectionDynamic(
    const bool enable,
    const std::set<smith::profiler::impl::ActivityType>& activities);

SMITH_API void startMemoryProfile();
SMITH_API void stopMemoryProfile();
SMITH_API void exportMemoryProfile(const std::string& path);
 ...
 *    }
 */
SMITH_API bool isProfilerEnabledInMainThread();
SMITH_API void enableProfilerInChildThread();
SMITH_API void disableProfilerInChildThread();
...
// Experimental.
SMITH_API void _reportVulkanEventToProfiler(vulkan_id_t id);
