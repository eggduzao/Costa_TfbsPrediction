...
// require gradients: a = smith.rand(2) b = smith.rand(2, requires_grad=True)
// a.copy_(b)
...
inline void throw_error_if_base_and_tensor_are_same(
  ...
  SMITH_CHECK(
      ...
      "As a user, you could have made a mistake implementing __smith_dispatch__ or a Python "
      "operator decomposition or meta registration; if that's not the case, please "
      "report a bug to Blacksmith or the backend you are using.");
}
...
inline at::Tensor as_view(
  ...
  auto diff_view_meta = smith::autograd::impl::get_view_autograd_meta(base);
