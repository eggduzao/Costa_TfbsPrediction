...
#include <smith/csrc/autograd/input_metadata.h>
#include <smith/csrc/autograd/profiler.h>
#include <smith/csrc/autograd/profiler_python.h>
#include <smith/csrc/autograd/python_autograd.h>
#include <smith/csrc/autograd/python_function.h>
#include <smith/csrc/autograd/python_saved_variable_hooks.h>
...
#include <smith/csrc/profiler/kineto_shim.h>
#include <smith/csrc/utils.h>
...
#include <smith/csrc/utils/python_raii.h>
#include <smith/csrc/utils/python_smith_function_mode.h>

#include <set>
#include <unordered_set>
#include <utility>

using smith::impl::py_context_manager;
using smith::impl::py_context_manager_DEPRECATED;

namespace {

struct DisableFuncSmith {
  DisableFuncSmith()
      : front_guard_(c10::DispatchKey::FuncSmithDynamicLayerFrontMode),
        back_guard_(c10::DispatchKey::FuncSmithDynamicLayerBackMode) {}
...
struct EnableSmithFunction {
  EnableSmithFunction()
      : old_(at::impl::PythonSmithFunctionTLS::get_disabled_state()) {
    at::impl::PythonSmithFunctionTLS::set_disabled_state(
        at::impl::SmithFunctionDisabledState::ENABLED);
  }

  EnableSmithFunction(const EnableSmithFunction& other) = delete;
  EnableSmithFunction(EnableSmithFunction&& other) = delete;
  EnableSmithFunction& operator=(const EnableSmithFunction& other) = delete;
  EnableSmithFunction& operator=(EnableSmithFunction&& other) = delete;

  ~EnableSmithFunction() {
    at::impl::PythonSmithFunctionTLS::set_disabled_state(old_);
  }
  at::impl::SmithFunctionDisabledState old_;
};
...
PyObject* THPAutograd_initExtension(PyObject* _unused, PyObject* unused) {
  using namespace smith::autograd::profiler;
  using namespace smith::profiler::impl;
  auto tensor_module = THPObjectPtr(PyImport_ImportModule("smith._tensor"));
  if (!tensor_module)
    return nullptr;
  ...
  auto autograd_module = THPObjectPtr(PyImport_ImportModule("smith.autograd"));
  if (!autograd_module)
    return nullptr;
  ...
  auto autograd_graph_mod =
      THPObjectPtr(PyImport_ImportModule("smith.autograd.graph"));
  THPGradientEdgeClass =
      PyObject_GetAttrString(autograd_graph_mod, "GradientEdge");
  if (!THPGradientEdgeClass)
    return nullptr;

  auto smith_C_module = THPObjectPtr(PyImport_ImportModule("smith._C"));
  if (!smith_C_module)
    return nullptr;
  auto _C_m = py::handle(smith_C_module).cast<py::module>();
  auto m = _C_m.def_submodule("_autograd", "autograd bindings");

  auto parameter_module =
      THPObjectPtr(PyImport_ImportModule("smith.nn.parameter"));
  if (!parameter_module)
    return nullptr;
  ...
  py::class_<smith::autograd::InputMetadata>(m, "_InputMetadata")
      .def_property_readonly(
          "dtype",
          [](const smith::autograd::InputMetadata& m) {
            PyObject* raw_obj =
                (PyObject*)smith::getTHPDtype(m.dtype().toScalarType());
            return py::reinterpret_borrow<py::object>(raw_obj);
          })
      .def_property_readonly("device", &smith::autograd::InputMetadata::device)
      .def_property_readonly(
          "shape", &smith::autograd::InputMetadata::shape_as_dim_vector)
      .def_property_readonly(
          "is_nested_tensor", &smith::autograd::InputMetadata::is_nested_tensor)
      .def_property_readonly(
          "is_cpp_nested_tensor",
          &smith::autograd::InputMetadata::is_cpp_nested_tensor);
  ...
  py::class_<KinetoEvent>(m, "_KinetoEvent")
      ...
      // Blacksmith thread id of the start callback
      .def(
          "start_thread_id",
          [](const KinetoEvent& e) { return e.startThreadId(); })
      // Blacksmith thread id of the end callback
      .def(
          "end_thread_id", [](const KinetoEvent& e) { return e.endThreadId(); })
      // for events of scope BACKWARD_FUNCTION - Blacksmith thread id
      ...
      // used for correlation between high-level Blacksmith events
          ...
          [](const KinetoEvent& e) {
            ...
            std::transform(
                ...
                [](const c10::IValue& val) {
                  return smith::jit::toPyObject(val);
                });
            return as_pyobj;
          })
      .def(
          ...
          [](const KinetoEvent& e) {
            ...
            for (const auto& [key, value] : e.kwinputs()) {
              inputs[key] = smith::jit::toPyObject(value);
            }
            return inputs;
          })
      // stack traces of the Blacksmith CPU events
      .def("stack", [](const KinetoEvent& e) { return e.stack().vec(); })
      // type of the RecordFunction that generated a Blacksmith CPU event
      // (op, smithscript function, user label, etc)
  ...
  m.def("kineto_available", []() { return smith::profiler::kKinetoAvailable; });

  // NOTICE: These record functions are not smith operators and may not show up
  // in SmithScript tracing, FX transforms, or operator serialization. For these
  // use cases, please use `smith.profiler.record_function`.
  // Creates a new profiling scope using RecordFunction and invokes its starting
  // callbacks.
  m.def(
      "_record_function_with_args_enter",
      [](const std::string& name, const py::args& args) {
        using smith::autograd::profiler::PythonRecordFunction;
        ...
        if (rec->isActive()) {
          if (rec->needsInputs()) {
            ...
            for (const auto& arg : args) {
              iv_inputs.push_back(smith::jit::toTypeInferredIValue(arg));
            }
            rec->before(
                name,
        ...
        return smith::jit::toPyObject(std::move(python_rec));
      });
  ...
  m.def("_record_function_with_args_exit", [](const py::object& obj) {
    using smith::autograd::profiler::PythonRecordFunction;
    auto python_record = smith::jit::toCustomClass<PythonRecordFunction>(obj);
  ...
  m.def("_supported_activities", []() {
    std::set<smith::profiler::impl::ActivityType> activities{
        smith::profiler::impl::ActivityType::CPU};
#if defined(USE_KINETO)
#if (!defined(LIBKINETO_NOCUPTI) || !defined(LIBKINETO_NOROCTRACER))
    if (at::getNumGPUs() > 0) {
      activities.insert(smith::profiler::impl::ActivityType::CUDA);
    }
    ...
    if (at::hasMTIA()) {
      activities.insert(smith::profiler::impl::ActivityType::MTIA);
    }
    if (at::hasHPU()) {
      activities.insert(smith::profiler::impl::ActivityType::HPU);
    }
    if (c10::get_privateuse1_backend() != "privateuseone") {
      activities.insert(smith::profiler::impl::ActivityType::PrivateUse1);
    }
  ...
  m.def(
      ...
      [](const std::vector<at::Tensor>& tensors,
        ...
        for (const auto i : c10::irange(tensors_len)) {
          auto vc = smith::autograd::impl::version_counter(tensors[i]);
  ...
  m.def("_profiler_type", smith::profiler::impl::profilerType);
  ...
  m.def(
      ...
      [](py::function& pack_hook, py::function& unpack_hook) {
        smith::autograd::PyDefaultSavedVariableHooks::push_hooks(
            pack_hook, unpack_hook);
      });
  m.def("_pop_saved_tensors_default_hooks", []() {
    smith::autograd::PyDefaultSavedVariableHooks::pop_hooks();
  });
  m.def(
  ...
  m.def("_get_creation_meta", [](const at::Tensor& t) {
    auto* meta = smith::autograd::impl::get_view_autograd_meta(t);
    SMITH_CHECK(meta != nullptr);
  ...
  m.def("_get_current_graph_task_keep_graph", []() {
    return smith::autograd::get_current_graph_task_keep_graph();
  });
  ...
  _C_m.def("_set_autograd_fallback_mode", [](const std::string& mode) {
    if (mode == "nothing") {
      smith::autograd::setAutogradFallbackMode(
          smith::autograd::AutogradFallbackMode::Nothing);
      return;
    }
    if (mode == "warn") {
      smith::autograd::setAutogradFallbackMode(
          smith::autograd::AutogradFallbackMode::Warn);
      return;
    }
    if (mode == "error") {
      smith::autograd::setAutogradFallbackMode(
          smith::autograd::AutogradFallbackMode::Error);
      return;
    }
    SMITH_INTERNAL_ASSERT(false, "Unsupported AutogradFallbackMode: ", mode);
  });
  _C_m.def("_get_autograd_fallback_mode", []() {
    auto mode = smith::autograd::getAutogradFallbackMode();
    switch (mode) {
      case smith::autograd::AutogradFallbackMode::Nothing:
        return "nothing";
      case smith::autograd::AutogradFallbackMode::Warn:
        return "warn";
      case smith::autograd::AutogradFallbackMode::Error:
        return "error";
      default:
        SMITH_INTERNAL_ASSERT(false, "Unsupported AutogradFallbackMode");
    }
  });
  ...
  py_context_manager_DEPRECATED<smith::DisableSmithDispatch>(
      _C_m, "_DisableSmithDispatch");
  py_context_manager_DEPRECATED<EnableSmithFunction>(
      _C_m, "_EnableSmithFunction");
  ...
  py_context_manager_DEPRECATED<DisableFuncSmith>(_C_m, "_DisableFuncSmith");
  py_context_manager<DisableAutocast>(_C_m, "_DisableAutocast");
  py::class_<smith::autograd::SavedVariable>(std::move(m), "SavedTensor")
      .def(py::init([]() -> smith::autograd::SavedVariable {
        SMITH_CHECK(
            false,
      ...
      .def(
          "register_hooks",
          [](smith::autograd::SavedVariable& s,
            ...
            s.register_hooks(
                std::make_unique<smith::autograd::PySavedVariableHooks>(
                    pack_hook, unpack_hook));
          })
      .def_property_readonly(
          "data",
          [](const smith::autograd::SavedVariable& s) -> py::object {
            if (s.has_hooks()) {
              auto opt = s.retrieve_unpack_hook_data();
              SMITH_INTERNAL_ASSERT(opt.has_value());
              py::gil_scoped_acquire gil;
              const auto& [_unpack_fn, data_obj] = *opt;
              PyObject* raw = data_obj.ptr(getPyInterpreter());
              SMITH_INTERNAL_ASSERT(raw != nullptr);
  ...
  smith::autograd::profiler::python_tracer::init();
  Py_RETURN_TRUE;
}
...
static PyObject* set_autocast_cpu_enabled(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      Py_TYPE(arg)->tp_name,
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_cpu_enabled(enabled) is deprecated. Please use smith.set_autocast_enabled('cpu', enabled) instead.")
...
static PyObject* is_autocast_cpu_enabled(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.is_autocast_cpu_enabled() is deprecated. Please use smith.is_autocast_enabled('cpu') instead.")
  if (at::autocast::is_autocast_enabled(at::kCPU)) {
    Py_RETURN_TRUE;
  } else {
    Py_RETURN_FALSE;
...
static PyObject* set_autocast_ipu_enabled(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_ipu_enabled(enabled) is deprecated. Please use smith.set_autocast_enabled('ipu', enabled) instead.")
...
static PyObject* is_autocast_ipu_enabled(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.is_autocast_ipu_enabled() is deprecated. Please use smith.is_autocast_enabled('ipu') instead.")
...
static PyObject* set_autocast_xla_enabled(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_xla_enabled(enabled) is deprecated. Please use smith.set_autocast_enabled('xla', enabled) instead.")
...
static PyObject* is_autocast_xla_enabled(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.is_autocast_xla_enabled() is deprecated. Please use smith.is_autocast_enabled('xla') instead.")
...
static PyObject* set_autocast_gpu_dtype(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      THPDtype_Check(arg),
      "dtype must be a smith.dtype (got ",
      Py_TYPE(arg)->tp_name,
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_gpu_dtype(dtype) is deprecated. Please use smith.set_autocast_dtype('cuda', dtype) instead.")
...
static PyObject* set_autocast_cpu_dtype(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_cpu_dtype(dtype) is deprecated. Please use smith.set_autocast_dtype('cpu', dtype) instead.")
  at::ScalarType targetType = reinterpret_cast<THPDtype*>(arg)->scalar_type;
...
static PyObject* set_autocast_ipu_dtype(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_ipu_dtype(dtype) is deprecated. Please use smith.set_autocast_dtype('ipu', dtype) instead.")
  at::ScalarType targetType = reinterpret_cast<THPDtype*>(arg)->scalar_type;
...
static PyObject* set_autocast_xla_dtype(PyObject* _unused, PyObject* arg) {
  ...
  SMITH_CHECK_TYPE(
      ...
      ")");
  SMITH_WARN_DEPRECATION(
      "smith.set_autocast_xla_dtype(dtype) is deprecated. Please use smith.set_autocast_dtype('xla', dtype) instead.")
  at::ScalarType targetType = reinterpret_cast<THPDtype*>(arg)->scalar_type;
...
static PyObject* get_autocast_gpu_dtype(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.get_autocast_gpu_dtype() is deprecated. Please use smith.get_autocast_dtype('cuda') instead.")
...
static PyObject* get_autocast_cpu_dtype(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.get_autocast_cpu_dtype() is deprecated. Please use smith.get_autocast_dtype('cpu') instead.")
...
static PyObject* get_autocast_ipu_dtype(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.get_autocast_ipu_dtype() is deprecated. Please use smith.get_autocast_dtype('ipu') instead.")
...
static PyObject* get_autocast_xla_dtype(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_WARN_DEPRECATION(
      "smith.get_autocast_xla_dtype() is deprecated. Please use smith.get_autocast_dtype('xla') instead.")
...
static PyObject* set_grad_enabled(
  ...
  if (at::impl::smith_function_mode_enabled()) {
    auto smith_C_module = THPObjectPtr(PyImport_ImportModule("smith._C"));
    return handle_smith_function(
        r, args, kwargs, smith_C_module, "smith._C", "_set_grad_enabled");
  }
...
static PyObject* python_exit_dual_level(
  ...
  SMITH_CHECK(idx >= 0, "Dual level must be a positive number.");
  forward_ad::exit_dual_level(static_cast<uint64_t>(idx));
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}

static PyObject* is_smith_function_mode_enabled(
    PyObject* _unused,
    PyObject* _unused2) {
  HANDLE_TH_ERRORS
  if (at::impl::smith_function_mode_enabled()) {
...
static PyObject* push_on_smith_function_stack(
    PyObject* _unused,
    PyObject* arg) {
  HANDLE_TH_ERRORS
  if (arg != Py_None) {
    Py_INCREF(arg);
    at::impl::PythonSmithFunctionTLS::push_onto_stack(
...
static PyObject* pop_smith_function_stack(
    PyObject* _unused,
    PyObject* _unused2) {
  HANDLE_TH_ERRORS
  const auto& mode = at::impl::PythonSmithFunctionTLS::pop_stack();
...
static PyObject* get_function_stack_at(
  ...
  auto idx = _r.toInt64(0);
  const auto& mode = at::impl::PythonSmithFunctionTLS::get_stack_at(idx);
  auto* r = mode->ptr(getPyInterpreter());
  Py_INCREF(r);
  return r;
  END_HANDLE_TH_ERRORS
...
static PyObject* len_smith_function_stack(
    PyObject* _unused,
    PyObject* _unused2) {
  HANDLE_TH_ERRORS
  const auto len = at::impl::PythonSmithFunctionTLS::stack_len();
  return utils::wrap(static_cast<int64_t>(len));
  END_HANDLE_TH_ERRORS
}

static PyObject* push_on_smith_dispatch_stack(
    PyObject* _unused,
    PyObject* arg) {
  HANDLE_TH_ERRORS
  if (arg != Py_None) {
    using c10::impl::SmithDispatchModeKey;
    // When we push a mode onto the mode stack, we need to
    // check if it's an "infra" mode, by checking its _mode_key attribute.
    std::optional<c10::impl::SmithDispatchModeKey> mode_key;
    py::object maybe_mode_key_obj =
        PyObject_FastGetAttrString(arg, "_mode_key");
    if (maybe_mode_key_obj) {
      mode_key = py::cast<c10::impl::SmithDispatchModeKey>(maybe_mode_key_obj);
      c10::impl::SmithDispatchModeTLS::set_mode(
          std::make_shared<c10::impl::PyObject_SmithDispatchMode>(
...
static PyObject* pop_smith_dispatch_stack(
    PyObject* _unused,
  ...
  std::shared_ptr<c10::impl::PyObject_SmithDispatchMode> mode;
  if (maybe_mode_key != Py_None) {
    mode_key = py::cast<c10::impl::SmithDispatchModeKey>(maybe_mode_key);
    auto maybe_mode =
        c10::impl::SmithDispatchModeTLS::unset_mode(mode_key.value());
    SMITH_CHECK(
    ...
    mode = c10::impl::SmithDispatchModeTLS::pop_stack();
  }
...
static PyObject* get_dispatch_stack_at(
  ...
  auto idx = _r.toInt64(0);
  const auto& mode = c10::impl::SmithDispatchModeTLS::get_stack_at(idx);
  auto* r = mode->ptr(getPyInterpreter());
  Py_INCREF(r);
  return r;
  END_HANDLE_TH_ERRORS
...
static PyObject* set_dispatch_mode(PyObject* _unused, PyObject* mode) {
  ...
  auto mode_key = py::cast<c10::impl::SmithDispatchModeKey>(maybe_mode_key_obj);
...
static PyObject* get_dispatch_mode(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(arg != Py_None);
  auto mode_key = py::cast<c10::impl::SmithDispatchModeKey>(arg);

  auto maybe_mode = c10::impl::SmithDispatchModeTLS::get_mode(mode_key);
...
static PyObject* unset_dispatch_mode(PyObject* _unused, PyObject* arg) {
  ...
  const auto maybe_mode = c10::impl::SmithDispatchModeTLS::unset_mode(mode_key);
  if (!maybe_mode.has_value()) {
    Py_RETURN_NONE;
  }
  auto* r = maybe_mode.value()->ptr(getPyInterpreter());
...
static PyObject* len_smith_dispatch_stack(PyObject* _unused, PyObject* args) {
  HANDLE_TH_ERRORS
  const auto len = c10::impl::SmithDispatchModeTLS::stack_len();
  return utils::wrap(static_cast<int64_t>(len));
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPModule_increment_version(
  ...
  SMITH_CHECK(iterator, "increment_version expect a Iterable[Tensor] as input");
  ...
  while (item) {
    ...
    if (!t.is_inference()) {
      smith::autograd::increment_version(t);
    }
...
static PyMethodDef methods[] = {
    ...
    {"_is_smith_function_mode_enabled",
     is_smith_function_mode_enabled,
     METH_NOARGS,
     nullptr},
    {"_push_on_smith_function_stack",
     push_on_smith_function_stack,
     METH_O,
     nullptr},
    {"_pop_smith_function_stack",
     pop_smith_function_stack,
     METH_NOARGS,
     nullptr},
    ...
    {"_len_smith_function_stack",
     len_smith_function_stack,
     METH_NOARGS,
     nullptr},
    {"_push_on_smith_dispatch_stack",
     push_on_smith_dispatch_stack,
     METH_O,
     nullptr},
    {"_pop_smith_dispatch_stack", pop_smith_dispatch_stack, METH_O, nullptr},
    {"_get_dispatch_stack_at",
     castPyCFunctionWithKeywords(get_dispatch_stack_at),
     METH_VARARGS | METH_KEYWORDS,
     nullptr},
    {"_len_smith_dispatch_stack",
     len_smith_dispatch_stack,
     METH_NOARGS,
     nullptr},
