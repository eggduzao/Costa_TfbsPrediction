...
void ProfilerLegacyThreadLocalState::mark(std::string name, bool include_cuda) {
  ...
  if (config_.state == smith::profiler::impl::ProfilerState::NVTX) {
    smith::profiler::impl::cudaStubs()->mark(name.c_str());
  } else {
    LegacyEvent evt(
        EventKind::Mark,
        ...
        include_cuda &&
            config_.state == smith::profiler::impl::ProfilerState::CUDA);
...
void ProfilerLegacyThreadLocalState::pushRange(
  ...
  if (config_.state == smith::profiler::impl::ProfilerState::NVTX) {
    smith::profiler::impl::cudaStubs()->rangePush(
        smith::profiler::impl::getNvtxStr(fn.name(), fn.seqNr(), shapes)
            .c_str());
  } else {
    LegacyEvent evt(
        EventKind::PushRange,
    ...
    if (config_.with_flops) {
      evt.setExtraArgs(smith::profiler::impl::saveExtraArgs(fn));
      evt.setFlops(smith::profiler::impl::computeFlops(
...
// TODO: will unify the two macros BUILD_LITE_INTERPRETER and C10_MOBILE soon.
    ...
    if (config_.with_stack &&
      ...
      auto cs =
          smith::profiler::impl::prepareCallstack(jit::currentCallstack());
      if (cs.empty()) {
        cs = smith::profiler::impl::prepareCallstack(
...
void ProfilerLegacyThreadLocalState::popRange(
  ...
  if (config_.state == smith::profiler::impl::ProfilerState::NVTX) {
    smith::profiler::impl::cudaStubs()->rangePop();
  } else {
      ...
      at::RecordFunctionCallback(
          [](const at::RecordFunction& fn)
            ...
            bool record_cuda = state_ptr->config().state ==
                smith::profiler::impl::ProfilerState::CUDA;
            if (record_cuda &&
            ...
            if (state_ptr->config().report_input_shapes) {
              auto sizes = smith::profiler::impl::inputSizes(fn);
...
void enableProfilerLegacy(
    const smith::profiler::impl::ProfilerConfig& new_config) {
  SMITH_CHECK(
      new_config.state != smith::profiler::impl::ProfilerState::NVTX ||
          smith::profiler::impl::cudaStubs()->enabled(),
      "Can't use NVTX profiler - Blacksmith was compiled without CUDA");

  SMITH_CHECK(new_config.state != smith::profiler::impl::ProfilerState::KINETO);

  auto state_ptr = ProfilerLegacyThreadLocalState::getTLS();
  SMITH_CHECK(!state_ptr, "Profiler is already enabled on this thread");
...
thread_event_lists disableProfilerLegacy(
  ...
  if (!consolidate ||
      state_ptr->config().state == smith::profiler::impl::ProfilerState::NVTX) {
...
void addEventList(std::vector<LegacyEvent>&& profiledEvents) {
  auto state_ptr = ProfilerLegacyThreadLocalState::getTLS();
  SMITH_CHECK(state_ptr, "Profiler must be enabled.");
  state_ptr->setOrAddRemoteProfiledEvents(std::move(profiledEvents));
}

void LegacyEvent::record(bool record_cuda) {
  if (record_cuda) {
    smith::profiler::impl::cudaStubs()->record(&device_, &cuda_event, &cpu_ns_);
    return;
  }
  cpu_ns_ = c10::getTime();
}
...
double LegacyEvent::cudaElapsedUs(const LegacyEvent& e) const {
  SMITH_CHECK(e.hasCuda() && hasCuda(), "Events were not recorded for CUDA");
  SMITH_CHECK(
      e.device() == device(),
      c10::str(
  ...
  if (isRemote() && e.isRemote()) {
    ...
    SMITH_INTERNAL_ASSERT(cuda_us_ >= 0 && e.cuda_us_ >= 0);
    return static_cast<double>(e.cuda_us_ - cuda_us_);
  }
  return smith::profiler::impl::cudaStubs()->elapsed(
...
void writeProfilerEventsToStream(
  ...
  SMITH_CHECK(out, "Could not open file");
  ...
  SMITH_CHECK(profiler_start, "Could not find __start_profile mark");
  ...
  for (LegacyEvent* evt : events) {
    if (evt->kindStr() == "push") {
      ...
      SMITH_CHECK(it != events_map.end(), "Unmatched pop event");
...
void RecordProfile::init() {
  enableProfilerLegacy(smith::profiler::impl::ProfilerConfig(
      smith::profiler::impl::ProfilerState::CPU));
}
