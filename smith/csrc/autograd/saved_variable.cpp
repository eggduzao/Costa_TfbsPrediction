...
SavedVariable::SavedVariable(
  ...
  if (variable.defined()) {
    ...
    SMITH_CHECK(
        ...
        "use it in autograd, or use `smith.no_grad()` instead of "
    ...
    if (is_inplace_on_view) {
      SMITH_INTERNAL_ASSERT(!is_leaf_ && is_output);
    ...
    if (maybe_hooks && !variable.unsafeGetTensorImpl()->is_wrapped_number()) {
      ...
      SMITH_INTERNAL_ASSERT(!data_.defined());
      return;
    }
...
Variable SavedVariable::unpack(std::shared_ptr<Node> saved_for) const {
  ...
  if (!data_.defined()) {
    SMITH_CHECK(hooks_, ERR_BACKWARD_TWICE);
  }
    ...
    if (saved_version_ != current_version) {
      ...
      if (!AnomalyMode::is_enabled()) {
        message << " Hint: enable anomaly detection to find the operation "
                   "that failed to compute its gradient, with smith.autograd."
