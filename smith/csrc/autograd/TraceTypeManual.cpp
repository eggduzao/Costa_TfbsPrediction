...
// `MANUAL_TRACER` in tools/autograd/gen_variable_type.blacksmith_LIBRARY_IMPL(aten, Tracer, m) {
  m.impl("resize_", resize_);
  m.impl("resize_as_", resize_as_);
  m.impl("detach", SMITH_FN(detach));
    ...
    for (auto iter = stack->end() - static_cast<std::ptrdiff_t>(input_size);
      ...
      if (type->isSubtypeOf(*TensorType::get())) {
        ...
        if (elem_type->isSubtypeOf(*TensorType::get())) {
          ...
          SMITH_CHECK(false, "unsupported input list type: ", elem_type->str());
        }
      } else if (iter->isObject()) {
        tracer::addInputs(node, args[i].name().c_str(), iter->toObject());
      } else {
        SMITH_CHECK(false, "unsupported input type: ", type->str());
      }
    }
    graph->insertNode(node);
