...
// See https://github.com/blacksmith/blacksmith/issues/60306
// TODO: clean this up when https://github.com/blacksmith/blacksmith/issues/60306 is
// improved
void record_stream_any_impl(Variable& var, const c10::Stream& stream) {
  ...
  if (C10_UNLIKELY(at::isBatchedTensor(var))) {
    ...
    if (impl) {
      ...
      SMITH_INTERNAL_ASSERT(false, "Expected batched tensor");
    }
  } else {
    switch (var.layout()) {
      case c10::kSparseCsr:
...
static void accumulate(
    ...
    const size_t pos,
    Variable&& var) {
  SMITH_INTERNAL_ASSERT(pos < buffer.size());
...
void InputBuffer::add(
  ...
  SMITH_INTERNAL_ASSERT(opt_consumer_stream && opt_producer_stream);
  ...
  if (*opt_consumer_stream != *opt_producer_stream &&
    ...
    SMITH_WARN_ONCE(
        ...
        "you can use smith.autograd.graph.set_warn_on_accumulate_grad_stream_mismatch(False) to suppress this "
        "warning.");
  }
  // See Note: [Autograd Producer-Consumer Stream Syncs]
  if (!opt_accum_streams[pos].has_value()) {
    // [ First producer ]
    SMITH_INTERNAL_ASSERT(!buffer[pos].defined());
    // 1)
    ...
    SMITH_INTERNAL_ASSERT(opt_accum_stream.has_value());
    if (*opt_consumer_stream != *opt_producer_stream ||
    ...
    SMITH_INTERNAL_ASSERT(accum_stream && ready_stream);
    // 1)
    ...
    if (*accum_stream != *ready_stream) {
      SMITH_INTERNAL_ASSERT(ready_event);
