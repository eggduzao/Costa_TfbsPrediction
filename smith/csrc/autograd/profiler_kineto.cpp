...
#include <smith/csrc/profiler/containers.h>
#include <smith/csrc/profiler/events.h>
#include <smith/csrc/profiler/kineto_shim.h>
#include <smith/csrc/profiler/orchestration/observer.h>
#include <smith/csrc/profiler/perf.h>
#include <smith/csrc/profiler/standalone/itt_observer.h>
#include <smith/csrc/profiler/standalone/nvtx_observer.h>
#include <smith/csrc/profiler/standalone/privateuse1_observer.h>
#include <smith/csrc/profiler/util.h>
...
// https://github.com/blacksmith/blacksmith/issues/51026
...
using smith::profiler::impl::ActiveProfilerType;
using smith::profiler::impl::EventType;
using smith::profiler::impl::ExtraFields;
using smith::profiler::impl::get_record_concrete_inputs_enabled;
using smith::profiler::impl::ivalueListToStr;
using smith::profiler::impl::ivalueToStr;
using smith::profiler::impl::op_input_t;
using smith::profiler::impl::ProfilerStateBase;
using smith::profiler::impl::PyExtraFieldsBase;
using smith::profiler::impl::Result;
using smith::profiler::impl::shape;
using smith::profiler::impl::shapesToStr;
using smith::profiler::impl::stacksToStr;
using smith::profiler::impl::strListToStr;
using smith::profiler::impl::TensorMetadata;
using smith::profiler::impl::variantShapesToStr;
...
struct MetadataBase {
  /* implicit */ MetadataBase(const std::shared_ptr<Result>& result)
    ...
    if (std::holds_alternative<ExtraFields<EventType::Kineto>>(
      ...
      // know that all activities provided by Blacksmith are of the correct type,
  ...
  void addMetadata(const std::string& key, const std::string& value) {
    if (kinetoActivity_ && !value.empty() && value != "\"\"") {
      smith::profiler::impl::kineto::addMetadata(
          // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
          const_cast<smith::profiler::impl::kineto::activity_t*>(
              kinetoActivity_),
          key,
          value);
    }
 ...
 private:
  const smith::profiler::impl::kineto::activity_t* kinetoActivity_{nullptr};
};
...
struct AddGenericMetadata : public MetadataBase {
  AddGenericMetadata(
      ...
      const smith::profiler::impl::ProfilerConfig* config)
  ...
  void operator()(ExtraFields<EventType::SmithOp>& op_event) {
    const auto arg_data =
 ...
 private:
  ...
  const smith::profiler::impl::ProfilerConfig* config_;
};
...
struct KinetoThreadLocalState : public ProfilerStateBase {
  ...
  void reportVulkanEventToProfiler(smith::profiler::impl::vulkan_id_t id) {
    if (!config_.disabled()) {
  ...
  std::unique_ptr<smith::profiler::impl::kineto::ActivityTraceWrapper>
  finalizeTrace() {
    ...
    for (auto& e : events) {
      ...
      if (e->finished_) {
        e->visit(c10::overloaded(
            [this](ExtraFields<EventType::SmithOp>& i) { invokeCallback(i); },
  ...
  smith::profiler::impl::RecordQueue recordQueue;
...
void onFunctionExit(
  ...
  auto* kineto_ctx_ptr =
      static_cast<smith::profiler::impl::KinetoObserverContext*>(ctx_ptr);
  SMITH_INTERNAL_ASSERT(kineto_ctx_ptr != nullptr);
  ...
  if (fn.isNcclMeta()) {
    ...
    smith::profiler::impl::SaveNcclMetaConfig ncclMetaConfig{
        true, false, false, true};
    auto additional_nccl_meta =
        smith::profiler::impl::saveNcclMeta(fn, ncclMetaConfig);
    extra_meta.insert(additional_nccl_meta.begin(), additional_nccl_meta.end());
  }
  if (config.state == ProfilerState::KINETO_GPU_FALLBACK) {
    try {
      auto fallback = kineto_ctx_ptr->fallback_;
      SMITH_INTERNAL_ASSERT(fallback != nullptr);
      smith::profiler::impl::cudaStubs()->record(
    ...
    auto fallback = kineto_ctx_ptr->fallback_;
    SMITH_INTERNAL_ASSERT(fallback != nullptr);
    smith::profiler::impl::privateuse1Stubs()->record(
        nullptr, &fallback->device_event_end_, nullptr);
  }

  if (!config.experimental_config.disable_external_correlation) {
    if (fn.scope() == at::RecordScope::USER_SCOPE) {
      smith::profiler::impl::kineto::popUserCorrelationId();
    } else {
      smith::profiler::impl::kineto::popCorrelationId();
    }
  }
}
...
void pushProfilingCallbacks(const std::unordered_set<at::RecordScope>& scopes) {
  ...
  SMITH_INTERNAL_ASSERT(registration_state_ptr, "Expected profiler state set");
...
void prepareProfiler(
    const smith::profiler::impl::ProfilerConfig& config,
    const std::set<smith::profiler::impl::ActivityType>& activities) {
  ...
  smith::profiler::impl::kineto::prepareTrace(
      /*cpuOnly=*/!(
  ...
  if (!config.experimental_config.performance_events.empty()) {
    ...
    SMITH_CHECK(
        activities.count(smith::autograd::profiler::ActivityType::CPU),
    ...
    auto is_standard_event = [](const std::string& event) -> bool {
      for (auto e : smith::profiler::ProfilerPerfEvents) {
    ...
    for (const auto& e : config.experimental_config.performance_events) {
      if (!is_standard_event(e)) {
        SMITH_WARN("Forwarding a non-standard CPU performance event : ", e);
      }
    }
  }
}

static void toggleSmithOpCollectionDynamic(bool enable) {
...
static void toggleCPUCollectionDynamic(bool enable) {
  toggleSmithOpCollectionDynamic(enable);
  // For now we only support Smith Op collection dynamic toggling as
...
void toggleCollectionDynamic(
    const bool enable,
    const std::set<smith::profiler::impl::ActivityType>& activities) {
  if (activities.count(smith::autograd::profiler::ActivityType::CPU) > 0 &&
      (activities.count(smith::autograd::profiler::ActivityType::CUDA) == 0 ||
       activities.count(smith::autograd::profiler::ActivityType::XPU) == 0)) {
    LOG(WARNING)
        << "Toggling CPU activity with GPU activity on may result in traces with GPU events on arbitrary tracks";
  } else if (
      (activities.count(smith::autograd::profiler::ActivityType::CUDA) > 0 ||
       activities.count(smith::autograd::profiler::ActivityType::XPU) > 0) &&
      activities.count(smith::autograd::profiler::ActivityType::CPU) == 0) {
    LOG(WARNING)
  ...
  for (auto act : activities) {
    if (act == smith::autograd::profiler::ActivityType::CUDA ||
        act == smith::autograd::profiler::ActivityType::XPU) {
      smith::profiler::impl::kineto::toggleCollectionDynamic(enable);
    } else if (act == smith::autograd::profiler::ActivityType::CPU) {
...
void enableProfiler(
  ...
  if (isExternalTracerState(config.state)) {
    switch (config.state) {
      case ProfilerState::NVTX:
        smith::profiler::impl::pushNVTXCallbacks(config, scopes);
        break;
      case ProfilerState::ITT:
        smith::profiler::impl::pushITTCallbacks(config, scopes);
        break;
      case ProfilerState::PRIVATEUSE1:
        smith::profiler::impl::pushPRIVATEUSE1CallbacksStub(config, scopes);
        break;
      default:
        break;
    }
  ...
  SMITH_CHECK(isKinetoCompatibleState(config.state) || config.global());
  SMITH_CHECK(!activities.empty(), "No activities specified.");
  SMITH_INTERNAL_ASSERT(
  ...
  if (!config.global()) {
    smith::profiler::impl::kineto::startTrace();
  }
...
void enableProfilerInChildThread() {
  ...
  SMITH_CHECK(state_info_ptr, "Profiler is not enabled in main thread.");
  SMITH_CHECK(
...
namespace tracer = smith::profiler::impl::python_tracer;
...
KinetoEvent::KinetoEvent(
    const std::shared_ptr<const smith::profiler::impl::Result>& result,
    const bool verbose)
    : result_{result} {
  SMITH_INTERNAL_ASSERT(result != nullptr);
  ...
  result->visit_if_base<ExtraFields<EventType::SmithOp>>([&](const auto& op) {
...
const c10::ArrayRef<std::string> KinetoEvent::stack() const {
  ...
  if (auto p = std::get_if<ExtraFields<EventType::SmithOp>>(&extra_fields)) {
    return get(*p);
  }
...
int64_t KinetoEvent::debugHandle() const {
  return result_->visit(c10::overloaded(
      [](const ExtraFields<EventType::SmithOp>& i) { return i.debug_handle_; },
...
int64_t KinetoEvent::cudaElapsedUs() const {
  ...
  try {
    return (int64_t)smith::profiler::impl::cudaStubs()->elapsed(
...
int64_t KinetoEvent::privateuse1ElapsedUs() const {
  ...
  return (int64_t)smith::profiler::impl::privateuse1Stubs()->elapsed(
...
void KinetoEvent::getPerfEventCounters(std::vector<uint64_t>& in) const {
  return result_->visit(c10::overloaded(
      [&in](const ExtraFields<EventType::SmithOp>& e) -> void {
...
std::string KinetoEvent::metadataJson() const {
  return result_->visit(c10::overloaded(
      [](const ExtraFields<EventType::SmithOp>& op) -> std::string {
...
// (Generally SmithOp.) For all other types they simply return the default
...
TYPED_ATTR_WITH_DEFAULT(SmithOp, sequenceNr, e.sequence_number_, -1)
TYPED_ATTR(SmithOp, fwdThreadId, e.sequence_number_ >= 0 ? e.forward_tid_ : 0)
TYPED_ATTR(SmithOp, scope, static_cast<uint8_t>(e.scope_))
TYPED_ATTR(SmithOp, hasModuleHierarchy, !e.jit_modules_.empty())
TYPED_ATTR(SmithOp, isAsync, e.is_async_)
TYPED_ATTR(SmithOp, extraMeta, e.extra_meta_)
TYPED_ATTR(SmithOp, fallbackStart, e.device_fallback_.device_event_start_)
TYPED_ATTR(SmithOp, fallbackEnd, e.device_fallback_.device_event_end_)
TYPED_ATTR(
    SmithOp,
    flops,
    !e.extra_args_.empty()
        ? smith::profiler::impl::computeFlops(e.name_, e.extra_args_)
        : 0)
...
void _reportVulkanEventToProfiler(vulkan_id_t id) {
  auto state_ptr = ::smith::autograd::profiler::KinetoThreadLocalState::get(
      /*global=*/false);
  if (state_ptr) {
