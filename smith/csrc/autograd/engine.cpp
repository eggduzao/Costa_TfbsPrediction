...
#include <smith/csrc/autograd/anomaly_mode.h>
...
// them.
//
// User-facing stream semantics of a backward() (or smith.autograd.grad())
// call with respect to surrounding ops are the same as for any other call.
// See "Stream semantics of backward passes" on
// https://blacksmith.org/docs/stable/notes/cuda.html
//
...
auto ReadyQueue::push(NodeTask item, bool incrementOutstandingTasks) -> void {
  {
    ...
    if (incrementOutstandingTasks) {
      ...
      SMITH_INTERNAL_ASSERT(graph_task, "GraphTask is no longer valid!");
...
void Engine::stop() {
  ...
  auto wait_duration_str =
      c10::utils::get_env("SMITH_AUTOGRAD_SHUTDOWN_WAIT_LIMIT");
  auto wait_duration =
...
std::vector<Node*> get_current_graph_task_execution_order() {
  ...
  while (!heap.empty()) {
    ...
    for (const auto& edge : fn->next_edges()) {
      ...
      SMITH_INTERNAL_ASSERT(it != dependencies.end());
      if (--it->second == 0) {
...
auto Engine::thread_main(const std::shared_ptr<GraphTask>& graph_task) -> void {
  ...
  // thread_main
  SMITH_INTERNAL_ASSERT(local_ready_queue != nullptr);
...
static void validate_outputs_impl(
  ...
  if (grads.size() != input_metadata_container.size()) {
    ...
    SMITH_CHECK(false, format_error(ss.str()));
  }
  for (const auto i : c10::irange(grads.size())) {
    ...
    if (grad.device() != metadata.device()) {
      if (grad.dim() == 0) {
        ...
        // quick hack for: https://github.com/blacksmith/blacksmith/issues/65016 but
    ...
    SMITH_INTERNAL_ASSERT(isDifferentiableType(grad.scalar_type()));
  }
}
...
void Engine::evaluate_function(
  ...
  for (size_t pos = 0; pos < inputs.ready_events.size(); ++pos) {
    ...
    SMITH_INTERNAL_ASSERT(opt_ready_stream && opt_parent_stream);
    if (*opt_parent_stream != *opt_ready_stream) {
      SMITH_INTERNAL_ASSERT(opt_ready_event);
  ...
  for (const auto i : c10::irange(num_outputs)) {
    ...
    if (it == dependencies.end()) {
      ...
      SMITH_CHECK(false, "dependency not found for ", name);
    } else if (--it->second == 0) {
      dependencies.erase(it);
      is_ready = true;
    }
...
auto Engine::execute(
  ...
  SMITH_INTERNAL_ASSERT(compiled_autograd != COMPILED_AUTOGRAD_POISON);
  ...
  // DistEngine (see SMITH_INTERNAL_ASSERT(futureGrads.use_count() == 1)
  // in dist_engine.cpp).
...
void Engine::initialize_device_threads_pool() {
  SMITH_CHECK(
      ...
      "See https://github.com/blacksmith/blacksmith/wiki/Autograd-and-Fork");
...
c10::intrusive_ptr<at::ivalue::Future> Engine::execute_with_graph_task(
  ...
  if (worker_device == NO_DEVICE) {
    ...
    lock.unlock();
    thread_main(graph_task);
    SMITH_INTERNAL_ASSERT(graph_task->future_result_->completed());
...
auto Engine::ready_queue(
  ...
  if (multithreading_disabled || should_run_in_cpu_ready_queue(device.type())) {
    ...
    SMITH_INTERNAL_ASSERT(cpu_ready_queue);
