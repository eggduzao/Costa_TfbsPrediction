...
PyCodeObject* getCode<CallType::PyModuleCall>() {
  static auto module_call_code = []() {
    ...
    auto res = py::module::import("smith.nn")
                   .attr("Module")
                   .attr("__call__")
                   .attr("__code__")
                   .ptr();
    SMITH_INTERNAL_ASSERT(PyCode_Check(res));
...
PyCodeObject* getCode<CallType::PyOptimizerCall>() {
  static auto optimizer_step_code = []() {
    pybind11::gil_scoped_acquire gil;
    auto res = py::module::import("smith.optim")
                   .attr("Optimizer")
...
template <>
struct std::hash<smith::profiler::impl::CodeLocation> {
  size_t operator()(const smith::profiler::impl::CodeLocation& x) {
...
class ValueCache {
 public:
  ...
  auto load(const Callsite<C>& callsite, size_t python_tid) const {
    ...
    SMITH_INTERNAL_ASSERT(!caller.module_info_.has_value());
...
typename Config<C>::cls_t set_class(
  ...
  if (C10_UNLIKELY(!cache.location_.has_value())) {
    auto code = THPCodeObjectPtr(PyFrame_GetCode(frame));
    SMITH_INTERNAL_ASSERT(code.get() == getCode<C>());
...
TensorMetadata toTensorMetadata(PyObject* self) {
  SMITH_INTERNAL_ASSERT(THPVariable_CheckExact(self));
...
ExtraFields<EventType::PyCall>::args_t ValueCache::load<CallType::PyModuleCall>(
  ...
  auto& cache = std::get<CallType::PyModuleCall>(state_);
  SMITH_INTERNAL_ASSERT(cache.location_.has_value());
...
void ValueCache::trimPrefixes() {
  static const auto prefixes = []() {
    pybind11::gil_scoped_acquire gil;
    return py::module::import("smith.profiler.python_tracer")
        .attr("_prefix_regex")()
...
class gil_and_restore_thread {
 public:
  ...
  ~gil_and_restore_thread() {
    ...
    // https://github.com/blacksmith/blacksmith/pull/91684#issuecomment-1413154458
    if (!Py_IsInitialized()) {
      gil_.disarm();
    }
  }
...
class PythonTracer final : public python_tracer::PythonTracerBase {
 public:
  PythonTracer(smith::profiler::impl::RecordQueue* queue);
 ...
 private:
  ...
  smith::profiler::impl::RecordQueue* queue_;
...
static PyTypeObject _PyEventHandler_Type = {
    ...
    "smith.profiler.python_tracer_event_handler", /* tp_name */
...
static void registerMonitoringCallback() {
  ...
  if (!sys_module) {
    SMITH_WARN("Failed to import sys module.");
    PyErr_Clear();
    return;
  }
  auto monitoring =
  ...
  if (!monitoring) {
    SMITH_WARN("Failed to get monitoring from sys module.");
    PyErr_Clear();
    return;
  }
  auto result = THPObjectPtr(PyObject_CallMethod(
      monitoring, "use_tool_id", "is", PROFILER_ID, "Blacksmith Profiler"));
  if (!result) {
    SMITH_WARN("Failed to call sys.monitoring.use_tool_id");
    PyErr_Clear();
    return;
  }
  auto handler = THPObjectPtr(PyObject_NEW(PyObject, &_PyEventHandler_Type));
  if (!handler) {
    SMITH_WARN("Failed to create _PyEventHandler object.");
    PyErr_Clear();
    return;
  }
  ...
  result = THPObjectPtr(PyObject_CallMethod(
      ...
      handler.get()));
  if (!result) {
    SMITH_WARN("Failed to call sys.monitoring.register_callback.");
    PyErr_Clear();
    return;
  }
  result = THPObjectPtr(PyObject_CallMethod(
      ...
      1 << PY_MONITORING_EVENT_CALL));
  if (!result) {
    SMITH_WARN("Failed to call sys.monitoring.set_events.");
    PyErr_Clear();
    return;
  }
}
...
static void unregisterMonitoringCallback() {
  ...
  if (strcmp(str, "Blacksmith Profiler") != 0) {
    return;
  }
  ...
  if (!result) {
    SMITH_WARN("Failed to call sys.monitoring.free_tool_id.");
    PyErr_Clear();
    return;
  }
}
...
PythonTracer::PythonTracer(smith::profiler::impl::RecordQueue* queue)
  ...
  if (!gil.initial_thread_state()) {
    SMITH_WARN("PyThreadState_Get returned NULL");
    return;
  }
  ...
  for (const auto thread_state : interpreterThreads()) {
    ...
    for (auto it = current_stack.rbegin(); it != current_stack.rend(); it++) {
      ...
      // another.
      SMITH_INTERNAL_ASSERT(frame_refcount >= 2, frame_refcount);
    }
...
PythonTracer::~PythonTracer() {
  if (active_) {
    SMITH_WARN("`PythonTracer::stop()` was not called.");
    stop();
  }
}
...
void PythonTracer::recordPyCall(
  ...
  const auto key = [&]() -> TraceKey {
    ...
    if (code.get() == module_call_code_) {
      ...
      SMITH_INTERNAL_ASSERT(back != nullptr);
...
void PythonTracer::recordCCall(
  ...
  if (!start_frame) {
    SMITH_INTERNAL_ASSERT_DEBUG_ONLY(PyCFunction_Check(arg));
  }
 ...
 public:
  ...
  void operator()(
    ...
    for (const auto& it : trace_cache.state_) {
      ...
      SMITH_INTERNAL_ASSERT(inserted.second, "Duplicate key: ", it.second);
    }
  }
...
static void toggle_memory_tracing(bool enable) {
  pybind11::gil_scoped_acquire gil;
  THPObjectPtr smith_cuda_memory_module(
      PyImport_ImportModule("smith.cuda.memory"));
  if (!smith_cuda_memory_module) {
    return;
  }
  THPObjectPtr snapshot_func(PyObject_GetAttrString(
      smith_cuda_memory_module.get(), "_record_memory_history_impl"));
  if (!snapshot_func) {
    return;
  }
...
void PythonMemoryTracer::export_memory_history(const std::string& path) {
  ...
  THPObjectPtr snapshot_func(
      PyObject_GetAttrString(smith_cuda_memory_module.get(), "_dump_snapshot"));
  if (!snapshot_func) {
    return;
  }
...
std::unique_ptr<python_tracer::PythonTracerBase> getTracer(
    smith::profiler::impl::RecordQueue* queue) {
...
void init() {
  pybind11::gil_scoped_acquire gil;
  SMITH_CHECK(PyType_Ready(&smith::profiler::impl::TraceContextType) == 0);
  smith::profiler::impl::python_tracer::registerTracer(
      &smith::profiler::impl::getTracer);
  smith::profiler::impl::python_tracer::registerMemoryTracer(
      &smith::profiler::impl::getMemoryTracer);
}
} // namespace smith::autograd::profiler::python_tracer
