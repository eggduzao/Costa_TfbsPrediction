...
#include <smith/csrc/Size.h>
#include <smith/csrc/THP.h>
#include <smith/csrc/Types.h>
#include <smith/csrc/autograd/autograd.h>
#include <smith/csrc/autograd/edge.h>
#include <smith/csrc/autograd/function.h>
#include <smith/csrc/autograd/python_cpp_function.h>
#include <smith/csrc/autograd/python_hook.h>
#include <smith/csrc/autograd/python_smith_functions.h>
#include <smith/csrc/autograd/python_variable_indexing.h>
#include <smith/csrc/autograd/utils/error_messages.h>
#include <smith/csrc/autograd/utils/wrap_outputs.h>
#include <smith/csrc/autograd/variable.h>
#include <smith/csrc/distributed/Placement.h>
...
using smith::utils::PyObjectPreservation;
...
std::pair<py::object, py::dict> parseIValuesToPyArgsKwargs(
  ...
  auto schemaAwareToPyObject =
    ...
    if (argument.isNone()) {
      ...
      return smith::jit::toPyObject(argument);
    }
  };
...
void pushPyOutToStack(
  ...
  if (num_returns == 0) {
    ...
    smith::jit::push(
        stack, smith::jit::toIValue(out.ptr(), schema_returns[0].real_type()));
  } else {
...
static constexpr const char* VOLATILE_WARNING =
    ...
    "`with smith.no_grad():` instead.";

static void SMITH_CHECK_TENSOR_SUBTYPE(PyObject* cls);

static bool check_has_smith_dispatch(PyObject* obj) {
  if (THPVariable_CheckExact(obj)) {
    return false;
  }
  py::object attr = PyObject_FastGetAttrString(obj, "__smith_dispatch__");
  return (
      attr.ptr() != nullptr &&
      attr.ptr() != smith::disabled_smith_dispatch_impl());
}
...
static PyObject* THPVariable_WrapWithType(
  ...
  SMITH_CHECK(obj, "Failed to allocate a ", type->tp_name, " object");
...
static PyObject* view_func_impl(
  ...
  auto diff_view_meta = smith::autograd::impl::get_view_autograd_meta(self);
  at::Tensor out;
  if (diff_view_meta && diff_view_meta->has_bw_view()) {
    ...
    if (!check_has_same_meta ||
        smith::autograd::utils::has_same_meta(new_base, view_info.base_)) {
...
static PyObject* rev_view_func_impl(PyObject* self_, PyObject* arg) {
  ...
  if (diff_view_meta && diff_view_meta->has_bw_view()) {
    const auto& view_info = diff_view_meta->get_backward_view();
    // Do the actual view replay
    SMITH_CHECK(view_info.has_view_fn(), "No _rev_view_func() found");
...
static PyObject* THPVariable_as_subclass(
  ...
  SMITH_CHECK_TENSOR_SUBTYPE(cls);
  // guard completely turns off smith dispatch modes, doesn't just pop off the
  // stack
  smith_dispatch_mode::StashSmithDispatchStackGuard td_g;
  c10::impl::DisablePythonDispatcher dpd_g;
  PyObject* obj = THPVariable_WrapWithType(self.alias(), (PyTypeObject*)cls);
  if (check_has_smith_dispatch(obj)) {
...
static PyObject* THPVariable_make_subclass(
  ...
  //
  // ```python
  // rnn = smith.nn.RNN(100, 100, 2)
  // # The following calls `smith._cudnn_rnn_flatten_weight(rnn._flat_weights,
...
static PyObject* THPVariable_make_wrapper_subclass(
  ...
  static PythonArgParser parser({
      ...
      "Layout layout=smith.strided, Device device=None, bool pin_memory=False, bool requires_grad=False, "
  ...
  // forcing users to define __smith_dispatch__ we ensure this does not happen
  // TODO: This check is not complete; because the user can disable smith
  // dispatch and then go again, triggering segfault.  TBH I'm thinking I want
  // to delete this function entirely
  py::object attr = PyObject_FastGetAttrString(cls, "__smith_dispatch__");
  SMITH_CHECK_TYPE(
      attr.ptr() != nullptr &&
          attr.ptr() != smith::disabled_smith_dispatch_impl(),
      ((PyTypeObject*)cls)->tp_name,
      " must define __smith_dispatch__");
  ...
  const auto options = TensorOptions()
                           ...
                           // https://github.com/blacksmith/blacksmith/issues/26428
...
DEFINE_CACHING_PYTHON_IMPORT_GETTER(
    ...
    py::module::import("smith.distributed.tensor").attr("DTensor"))
...
DEFINE_CACHING_PYTHON_IMPORT_GETTER(
    ...
    py::module::import("smith.distributed.tensor")
        .attr("_dtensor_spec")
        .attr("DTensorSpec"))
...
#define INTERN_DTENSOR_STRING(s)                                           \
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(dtensor_interned_strings.s == nullptr); \
...
static c10::SymDimVector tuple_to_symintlist(PyObject* obj) {
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(PyTuple_Check(obj));
  ...
  for (const auto idx : c10::irange(size)) {
    ...
    } else if (smith::is_symint(py::handle(item))) {
      res.push_back(py::handle(item).cast<c10::SymInt>());
    } else {
      // N.B. smith.Tensor.__index__ exists, so this should handle
      // scalar Tensors fine.
...
class NativeOpSchema {
 ...
 private:
  ...
  // __smith_dispatch__ (and hence to DTensor dispatch as well), so
...
static void replace_dtensors_with_local_tensor(smith::jit::Stack& stack);
...
static bool is_random_op(const c10::OperatorHandle& op) {
  ...
  // smith/distributed/tensor/_dispatch.py
  ...
  if (participating) {
    ...
    const auto default_tensor = [](py::handle spec) -> Tensor {
      ...
      SMITH_CHECK(
          ...
          "spec.tensor_meta.dtype must be a smith.dtype");
...
py::object dispatchDTensorOp(
  ...
  smith::jit::Stack saved_args = *stack;
  ...
  SMITH_INTERNAL_ASSERT_DEBUG_ONLY(
      !kwargs.is_none(),
  ...
  if (local_results_success) {
    py_local_results = smith::jit::createPyObjectForStack(std::move(*stack));
  } else {
    get_py_op_info_if_needed();
      ...
      for (const auto& ret_schema : returns) {
        ...
        if (sets_intersect(
          ...
          if (wrapped_result_tuple.has_value()) {
            ...
            SMITH_INTERNAL_ASSERT(ret_idx == 0);
            ret = wrapped_result;
          }
...
static PyObject* THPVariable_dtensor_new(
  ...
  // __smith_dispatch__. Check anyway in debug builds in case somebody
  // removes it.
  py::object attr = PyObject_FastGetAttrString(cls, "__smith_dispatch__");
  SMITH_CHECK_TYPE(
      attr.ptr() != nullptr &&
  ...
  if (local_tensor.requires_grad() && !requires_grad) {
    SMITH_WARN(
        "To construct DTensor from smith.Tensor, it's recommended to use "
  ...
  SMITH_CHECK(!tensor_meta.is_none());
  const auto sizes = tensor_meta.attr(dtensor_interned_strings.shape);
  SMITH_CHECK(
      PyTuple_Check(sizes.ptr()), "spec.tensor_meta.shape must be a tuple");
...
static std::pair<TensorFlavor, py::object> check_for_dtensor_or_tensor(
  ...
  smith::jit::guardAgainstNamedTensor<at::Tensor>(tensor);
...
static /*DTensorSpec*/ py::object try_replicate_spec_for_scalar_tensor(
  ...
  SMITH_CHECK(
      ...
      py::str(op_call),
      " got mixed smith.Tensor and DTensor, need to convert all smith.Tensor to DTensor before calling distributed operators!");
...
create_native_op_schema(
  ...
  for (auto argument_it = args_kwargs.args_begin();
    ...
    switch (tensor_flavor) {
      ...
      default:
        SMITH_INTERNAL_ASSERT(false, "can't happen");
        break;
    }
    idx++;
  }
...
struct GetterBase {
  static PyObject* getter(THPVariable* self, void* /*unused*/) {
    HANDLE_TH_ERRORS
    if (check_has_smith_function((PyObject*)self)) {
      return handle_smith_function_getter(self, T::name);
    }
...
static PyObject* THPVariable_get_cdata(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "_cdata");
  }
  const auto& var = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_version(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "_version");
  }
  const auto& var = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_grad_fn(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "grad_fn");
  }
  const auto& var = THPVariable_Unpack(self);
  if (!var.grad_fn()) {
    Py_RETURN_NONE;
...
static int THPVariable_set_grad_fn(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "_grad_fn", obj);
  }
  SMITH_CHECK(obj, "Deletion of _grad_fn not allowed. Detach tensor instead!");
  SMITH_CHECK(obj == Py_None, "_grad_fn can be only set to None");
...
static PyObject* THPVariable_is_leaf(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_leaf");
  }
...
static int THPVariable_set_data(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "data", data);
  }
  SMITH_CHECK(
...
static int THPVariable_set_grad(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "grad", py_grad);
  }
  const auto& var = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_volatile(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "volatile");
  }
...
static int THPVariable_set_volatile(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "volatile", obj);
  }
...
static PyObject* THPVariable_get_output_nr(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "output_nr");
  }
...
static PyObject* THPVariable_get_requires_grad(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "requires_grad");
  }
...
static PyObject* THPVariable_retains_grad(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "retains_grad");
  }
...
static PyObject* THPVariable_get_ndim(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "ndim");
  }
...
static PyObject* THPVariable_get_names(PyObject* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function(self)) {
    return handle_smith_function_getter((THPVariable*)self, "names");
  }
  // The long-term plan is to return a list of (python) smith.Dimname.
...
static int THPVariable_set_names(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function(self)) {
    return handle_smith_function_setter((THPVariable*)self, "names", names);
  }
  const auto& var = THPVariable_Unpack(self);
  if (names == Py_None) {
    ...
    at::internal_set_names_inplace(var, smith::parseDimnameList(names));
  }
  return 0;
  END_HANDLE_TH_ERRORS_RET(-1)
}
...
static int THPVariable_set_requires_grad(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "requires_grad", obj);
  }
  SMITH_CHECK(obj && PyBool_Check(obj), "requires_grad must be a bool");
  const auto& var = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_name(THPVariable* self, void* unused) {
  if (check_has_smith_function((PyObject*)self)) {
    HANDLE_TH_ERRORS
    return handle_smith_function_getter(self, "name");
    END_HANDLE_TH_ERRORS
  }
  const auto& tensor = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_backwards_hooks(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "_backward_hooks");
  }
...
static int THPVariable_set_backwards_hooks(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "_backward_hooks", obj);
  }
  SMITH_CHECK(obj, "Deletion of _backwards_hooks not allowed!");
  if (obj == Py_None) {
    obj = nullptr;
  }
  Py_XINCREF(obj);
  ...
  const auto& tensor = THPVariable_Unpack(self);
  smith::autograd::impl::clear_hooks(tensor);
  if (obj) {
    smith::autograd::impl::add_hook(
...
static PyObject* THPVariable_get_post_accumulate_grad_hooks(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "_post_accumulate_grad_hooks");
  }
...
static int THPVariable_set_post_accumulate_grad_hooks(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(
        self, "_post_accumulate_grad_hooks", obj);
  }
  SMITH_CHECK(obj, "Deletion of _post_accumulate_grad_hooks not allowed!");
  if (obj == Py_None) {
    obj = nullptr;
  }
  Py_XINCREF(obj);
  ...
  const auto& tensor = THPVariable_Unpack(self);
  if (obj) {
    smith::autograd::impl::set_post_acc_grad_hooks(
...
static PyObject* THPVariable_get_base(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "_base");
  }
  const auto& tensor = THPVariable_Unpack(self);
  if (tensor.is_view()) {
...
static PyObject* THPVariable_get_shape(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "shape");
  }
...
static PyObject* THPVariable_is_cpu(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_cpu");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_cpu());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_cuda(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_cuda");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_cuda());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_mtia(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_mtia");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_mtia());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_xla(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_xla");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_xla());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_ipu(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_ipu");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_ipu());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_xpu(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_xpu");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_xpu());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_sparse(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_sparse");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_sparse());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_sparse_csr(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_sparse_csr");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_sparse_csr());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_mkldnn(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_mkldnn");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_mkldnn());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_mps(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_mps");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_mps());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_maia(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_maia");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_maia());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_vulkan(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_vulkan");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_vulkan());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_quantized(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_quantized");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_quantized());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_meta(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_meta");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_meta());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_complex(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_complex");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_complex());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_is_nested(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "is_nested");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.is_nested());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_dtype(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "dtype");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.scalar_type());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_layout(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "layout");
  }
  auto& self_ = THPVariable_Unpack(self);
  return smith::autograd::utils::wrap(self_.layout());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPVariable_device(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "device");
  }
...
static PyObject* THPVariable_get_nbytes(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "nbytes");
  }
...
static PyObject* THPVariable_get_grad_dtype(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "grad_dtype");
  }
  const auto& var = THPVariable_Unpack(self);
  SMITH_CHECK(
  ...
  if (!var.grad_dtype().has_value()) {
    ...
    return smith::autograd::utils::wrap(var.grad_dtype().value());
  }
  END_HANDLE_TH_ERRORS
}
...
static int THPVariable_set_grad_dtype(
  ...
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_setter(self, "grad_dtype", obj);
  }
  const auto& var = THPVariable_Unpack(self);
  SMITH_CHECK(
      THPDtype_Check(obj) || obj == Py_None,
      "grad_dtype must be a smith.dtype or None, but got ",
      Py_TYPE(obj)->tp_name);
...
static PyObject* THPVariable_get_itemsize(THPVariable* self, void* unused) {
  HANDLE_TH_ERRORS
  if (check_has_smith_function((PyObject*)self)) {
    return handle_smith_function_getter(self, "itemsize");
  }
...
PyObject* THPVariable_pynew(
  ...
  PyObject* obj = THPVariable_WrapWithType(
      smith::utils::base_tensor_ctor(args, kwargs), type);
  if (check_has_smith_dispatch(obj)) {
    THPVariable_Unpack(obj).unsafeGetTensorImpl()->set_python_dispatch(true);
  }
  return obj;
...
static int THPVariable_clear(THPVariable* self) {
  ...
  if (self->cdata.defined()) {
    ...
    if (pyobj_slot->load_pyobj() == (PyObject*)self) {
      ...
      SMITH_INTERNAL_ASSERT(self->cdata.use_count() == 1);
...
/// https://github.com/blacksmith/blacksmith/issues/7343
///
...
static int THPVariable_traverse(PyObject* self, visitproc visit, void* arg) {
  ...
  if (tensor.defined()) {
    ...
    auto autograd_meta = smith::autograd::impl::get_autograd_meta(tensor);
    ...
    if (autograd_meta) {
      for (const auto& hook : smith::autograd::impl::hooks(tensor)) {
...
int THPVariableMetaType_init(PyObject* cls, PyObject* args, PyObject* kwargs) {
  ...
  // If the user provided a smith_dispatch implementation, disable
  // smith_function.
  py::object smith_dispatch_impl = py::reinterpret_steal<py::object>(
      PyObject_GetAttrString(cls, "__smith_dispatch__"));
  py::object smith_dispatch_default = py::reinterpret_steal<py::object>(
      PyObject_GetAttrString(THPVariableClass, "__smith_dispatch__"));
  if (smith_dispatch_impl.ptr() != smith_dispatch_default.ptr()) {
    py::object smith_function_impl = py::reinterpret_steal<py::object>(
        PyObject_GetAttrString(cls, "__smith_function__"));
    py::object smith_function_default_bound = py::reinterpret_steal<py::object>(
        PyObject_GetAttrString(THPVariableClass, "__smith_function__"));

    // Since our __smith_function__ is a classmethod, we need to "unbound" the
    // method to get the raw function
    py::object smith_function_default = py::reinterpret_steal<py::object>(
        PyObject_GetAttrString(smith_function_default_bound.ptr(), "__func__"));

    // User-defined __smith_function__ might not be a classmethod
    if (PyObject_HasAttrString(smith_function_impl.ptr(), "__func__")) {
      smith_function_impl = py::reinterpret_steal<py::object>(
          PyObject_GetAttrString(smith_function_impl.ptr(), "__func__"));
    }
    if (smith_function_impl.ptr() == smith_function_default.ptr()) {
      PyObject_SetAttrString(
          cls, "__smith_function__", smith::disabled_smith_function_impl());
    }
  }
...
static void initTensorImplConversion(PyObject* module) {
  ...
  m.def("_wrap_tensor_impl", [](void* ptr) {
    ...
    SMITH_CHECK(p.defined(), "Can't wrap undefined tensor");
    auto tensor = at::Tensor::wrap_tensor_impl(std::move(p));
    return py::cast(std::move(tensor));
  });
  // set on the module level to avoid mixing pybind and plain CPython extensions
  m.def("_tensor_impl_raw_handle", [](smith::autograd::Variable* t) -> void* {
...
bool THPVariable_initModule(PyObject* module) {
  ...
  THPUtils_addPyMethodDefs(methods, smith::autograd::variable_methods);
...
#endif
  smith::autograd::initSmithFunctions(module);
  smith::autograd::initTensorImplConversion(module);
  smith::utils::validate_numpy_for_dlpack_deleter_bug();
