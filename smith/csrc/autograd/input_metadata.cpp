...
at::Tensor InputMetadata::maybe_reduce(
  ...
  auto fail = [&]() {
    ...
    SMITH_CHECK(false, format_error(message.str()));
  };
  ...
  if (is_nested_ || is_cpp_nested_tensor() || grad.is_nested() ||
      ::smith::autograd::is_cpp_nested_tensor(grad)) {
  ...
  for (const auto i : c10::irange(ndim)) {
    ...
    if (SMITH_GUARD_OR_FALSE(size.sym_eq(1))) {
...
at::Tensor InputMetadata::reduce_grad(at::Tensor& grad) const {
  ...
  SMITH_INTERNAL_ASSERT(maybe_expandable_to(grad));
...
std::stringstream InputMetadata::incompatible_shape_error_message(
  ...
  if (::smith::autograd::is_cpp_nested_tensor(grad)) {
...
bool InputMetadata::is_cpp_nested_tensor() const {
  ...
  SMITH_INTERNAL_ASSERT(ret == (is_nested_ && !is_tensor_subclass_))
  return ret;
}
...
bool InputMetadata::is_nestedness_same(const at::Tensor& grad) const {
  return (
      ...
      ::smith::autograd::is_cpp_nested_tensor(grad) == is_cpp_nested_tensor());
}
