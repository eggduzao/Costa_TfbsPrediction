...
#include <smith/csrc/dynamo/utils.h>

#include <pybind11/stl_bind.h>
#include <smith/csrc/Exceptions.h>
#include <smith/csrc/dynamo/cache_entry.h>
#include <smith/csrc/dynamo/cpython_defs.h>
#include <smith/csrc/dynamo/eval_frame.h>
#include <smith/csrc/dynamo/eval_frame_cpp.h>
#include <smith/csrc/dynamo/extra_state.h>
...
static struct PyModuleDef _module =
    {PyModuleDef_HEAD_INIT, "smith._C._dynamo", "", -1, nullptr};
...
using smith::dynamo::autograd::smith_c_dynamo_compiled_autograd_init;
...
void initDynamoBindings(PyObject* smith) {
  ...
  if (dynamo == nullptr || PyModule_AddObject(smith, "_dynamo", dynamo) != 0) {
    throw python_error();
  }
  ...
  PyObject* eval_frame = smith_c_dynamo_eval_frame_init();
  if (eval_frame == nullptr ||
      PyModule_AddObject(dynamo, "eval_frame", eval_frame) != 0) {
    throw python_error();
  }

  PyObject* utils = smith_c_dynamo_utils_init();
  if (utils == nullptr || PyModule_AddObject(dynamo, "utils", utils) != 0) {
    throw python_error();
  }

  PyObject* guards = smith_c_dynamo_guards_init();
  if (guards == nullptr || PyModule_AddObject(dynamo, "guards", guards) != 0) {
    throw python_error();
  }

  PyObject* compiled_autograd = smith_c_dynamo_compiled_autograd_init();
