...
#include <smith/csrc/dynamo/guards.h>
...
#include <smith/csrc/dynamo/debug_macros.h>
...
uint64_t count_instructions(const std::function<void()>& fn) {
  int fd = open_counter();
  SMITH_CHECK(fd != -1, "perf_event_open failed");
...
static std::vector<std::optional<c10::SymInt>> pyListToVecOptInt(
  ...
  for (Py_ssize_t i = 0; i < size; i++) {
    ...
    if (item == Py_None) {
    ...
    } else if (smith::is_symint(handle)) {
      ...
      if (value == -1 && PyErr_Occurred()) {
        ...
        SMITH_CHECK(false, "Size or stride list item is not a valid integer.");
      }
...
struct GlobalStateGuard {
  ...
  void init() {
    ...
    // if smith function disabled state is
    // 1) enabled, 2) all disabled, 3) subclasses disabled
    // we guard on the stack separately
    _smith_function = smith::smith_function_enabled();
    _smith_function_all_disabled = at::impl::smith_function_all_disabled();
  ...
  bool check() const {
    ...
    return (_grad_mode == at::GradMode::is_enabled() &&
            _autocast_state == AutocastState() &&
            _smith_function == smith::smith_function_enabled() &&
            _smith_function_all_disabled ==
                at::impl::smith_function_all_disabled() &&
            _deterministic_algorithms == ctx.deterministicAlgorithms() &&
  ...
  std::string reason() const {
    ...
    if (_smith_function != smith::smith_function_enabled())
      os << "smith_function ";
  ...
  friend void to_json(T& json_j, const GlobalStateGuard& json_t) {
    ...
    json_j["smith_function"] = json_t._smith_function;
    json_j["smith_function_all_disabled"] = json_t._smith_function_all_disabled;
  ...
  friend void from_json(const T& json_j, GlobalStateGuard& json_t) {
    ...
    json_t._smith_function = json_j.at("smith_function");
    json_t._smith_function_all_disabled =
        json_j.at("smith_function_all_disabled");
  ...
  bool _smith_function;
  bool _smith_function_all_disabled;
...
static PyObject* assert_size_stride(PyObject* dummy, PyObject* args) {
  ...
  */
  ...
  if (num_errors) {
    ...
    msg << "\nUse smith.library.opcheck to test your custom op.";
    msg << "\nSee https://blacksmith.org/docs/stable/library.html#smith.library.opcheck";
    PyErr_SetString(PyExc_AssertionError, msg.str().c_str());
    return nullptr;
  }
...
static PyObject* assert_alignment(PyObject* dummy, PyObject* args) {
   ...
   * This C++ version of smith._inductor.utils.tensor_is_aligned
   */
  PyObject* item = nullptr;
...
static void unwrap_size_tuple(PyObject* obj, T& output) {
  SMITH_CHECK(PyTuple_CheckExact(obj));
...
static void _parse_empty_strided_args(
    ...
    T& strides,
    at::ScalarType& dtype) {
  SMITH_CHECK(PyTuple_CheckExact(args));
  SMITH_CHECK(PyTuple_GET_SIZE(args) == 3);
  ...
  SMITH_CHECK(THPDtype_Check(py_dtype));
...
static PyObject* _empty_strided_device(
  ...
  if (device_type == c10::DeviceType::CPU) {
    ...
    SMITH_CHECK(
        false, "Blacksmith compiled without support for the specified device.");
  }
...
static PyObject* _reinterpret_tensor(PyObject* dummy, PyObject* args) {
  ...
  auto res = smith::inductor::_reinterpret_tensor(
      self, sizes, strides, offset_increment);
  return smith::autograd::utils::wrap(res);
...
bool is_immutable_object(py::handle example_value) {
  py::object config_module = py::module_::import("smith._dynamo.config");
...
bool is_parameter(py::handle tensor) {
  py::object parameter = py::module::import("smith.nn").attr("Parameter");
  ...
  if (x.dim() == 2 && y.dim() == 2 && Meta::stride(x, 1) == 1 &&
    ...
    if (Meta::stride(x, 0) == Meta::stride(y, 0)) {
      ...
      if (offset_delta < Meta::size(x, 1)) {
        ...
        // Example:
        //   base = smith.arange(32).reshape(4, 8)
        //   x = base.narrow(1, 0, 4)
...
class StorageOverlapChecker {
 public:
  ...
  void add(PyObject* obj, bool overlapping) {
    ...
    SMITH_CHECK(THPVariable_CheckExact(obj) || THPVariable_Check(obj));
    Py_INCREF(obj);
   ...
   */
  bool maybe_check() {
    SMITH_CHECK(_expected_overlapping >= _overlapping.size());
    SMITH_CHECK(_expected_non_overlapping >= _non_overlapping.size());
 ...
 public:
  DEFAULT_DEVICE(
      ...
      : LeafGuard(
            ...
            std::move(verbose_code_parts),
            std::move(user_stack)) {
    py::handle device_module = py::module::import("smith.utils._device");
 ...
 public:
  DUAL_LEVEL_MATCH(
      ...
      : LeafGuard(
            ...
            std::move(user_stack)),
        _level(level) {
    forward_ad_module = py::module_::import("smith.autograd.forward_ad");
  }
...
class GuardManager {
 ...
 public:
  ...
  virtual bool check_nopybind(PyObject* value) {
    ...
    // yielding significant speed‑ups in smith.compile’s Guard Tree.
    // -----------------------------------------------------------------------------
...
class RootGuardManager : public GuardManager {
 public:
  ...
  bool check_nopybind_template(T* value) { // borrowed ref
    ...
    // Run accessor guards without SmithFunction enabled
    // Dynamo should only be adding guards on values without
    // smith function at this point, because if there
    // was a smith function, we should've traced through it
    const at::impl::SmithFunctionDisabledState old_state =
    ...
    if (!GuardManager::check_accessors_nopybind(value)) {
      at::impl::PythonSmithFunctionTLS::set_disabled_state(old_state);
      _reset_relational_guard_state();
      return false;
    }
  ...
  GuardDebugInfo check_verbose_nopybind(
    ...
    // Run leaf guards
    // This includes the GlobalStateGuard and the Smith Function Mode stack
    // guard, which require Smith Function to be in its unmodified state
...
class DictGuardManager : public GuardManager {
 public:
  ...
  void fail_on_get_child_manager(
      ...
      const std::string& source,
      const py::object& b) {
    SMITH_CHECK(false, "Can not add an accessor to DictGuardManager");
  }
  ...
  void add_leaf_guard(std::shared_ptr<LeafGuard> leaf_guard) override {
    ...
    SMITH_CHECK(false, "DictGuardManager does not support a leaf_guard");
  }
...
std::unique_ptr<GuardManager> make_guard_manager(
      ...
      storage
          .call_once_and_store_result([]() -> threeobjects {
            py::object guard_manager_enum_class =
                py::module_::import("smith._dynamo.guards")
  ...
  static py::object guard_manager_enum_class =
      py::module_::import("smith._dynamo.guards").attr("GuardManagerType");
...
class SMITH_FUNCTION_MODE_STACK : public LeafGuard {
 public:
  ...
  bool check_nopybind_template(T* value) {
    ...
    const size_t len = (size_t)at::impl::PythonSmithFunctionTLS::stack_len();
    ...
    for (int64_t idx = 0; (size_t)idx < len; idx++) {
      std::shared_ptr<c10::SafePyObject> mode =
          at::impl::PythonSmithFunctionTLS::get_stack_at(idx);
  ...
  GuardDebugInfo check_verbose_nopybind(
    ...
    if (!fail_reason.empty()) {
      if (is_parameter(py::handle(value))) {
        ...
        fail_reason +=
            "smith._dynamo.config.force_parameter_static_shapes = False ";
 ...
 public:
  ...
  bool check_nopybind(PyObject* obj, bool matches_dict_tag = false)
    ...
    if (_prop == TensorProperty::SIZE) {
      ...
      SMITH_CHECK(false, "Unknown property");
    }
...
/**
 * Implements function call no args - e.g, smith.cuda.current_device()
 */
...
static void* _smithinductor_pyobject_tensor_data_ptr(PyObject* obj) {
  SMITH_CHECK(
      ...
      "_smithinductor_pyobject_tensor_data_ptr: non-tensor input");
...
PyObject* smith_c_dynamo_guards_init() {
  // initialize TensorGuardsType
  TensorGuardsType.tp_name = "smith._C._dynamo.guards.TensorGuards";
  ...
  TensorGuardsType.tp_doc = "Check properties of a smith.Tensor";
  ...
  GlobalStateGuardType.tp_name = "smith._C._dynamo.guards.GlobalStateGuard";
  ...
  GlobalStateGuardType.tp_doc = "Guard on Blacksmith global flags such as no_grad";
  ...
  // We expose the address of _smithinductor_pyobject_tensor_data_ptr in order
  // to allow manual linking in our generated SmithInductor Python bindings.
  ...
  if (PyModule_AddObject(
          ...
          PyLong_FromVoidPtr(reinterpret_cast<void*>(
              &_smithinductor_pyobject_tensor_data_ptr))) < 0) {
    return nullptr;
  }
  ...
  py::class_<
      SMITH_FUNCTION_MODE_STACK,
      LeafGuard,
      std::shared_ptr<SMITH_FUNCTION_MODE_STACK>>(
      py_m, "SMITH_FUNCTION_MODE_STACK")
      .def(py::init<RootGuardManager*, py::list, py::list, py::object>())
      .def("__call__", &SMITH_FUNCTION_MODE_STACK::check);
  ...
  py::class_<GuardManager, std::unique_ptr<GuardManager>>(py_m, "GuardManager")
      ...
      .def(
          "add_smith_function_mode_stack_guard",
          [](GuardManager& self,
             const py::list& initial_stack,
             py::object verbose_code_parts,
             py::object user_stack) -> void {
            self.add_leaf_guard(std::make_shared<SMITH_FUNCTION_MODE_STACK>(
                self.get_root(),
                initial_stack,
                std::move(verbose_code_parts),
                std::move(user_stack)));
