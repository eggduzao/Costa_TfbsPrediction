#include <smith/csrc/dynamo/python_compiled_autograd.h>
...
the autograd graph to an FX graph that can be smith.compiled. It caches
...
// see https://github.com/blacksmith/blacksmith/pull/34845
...
static PyObject* call_cpp_tensor_pre_hooks(PyObject* dummy, PyObject* args) {
  ...
  SMITH_INTERNAL_ASSERT(idx > -1);
  SMITH_INTERNAL_ASSERT(grad != nullptr);
  SMITH_INTERNAL_ASSERT(active_rstate != nullptr);
...
static std::string bind_function(
  ...
  auto py_func = py::cpp_function(
      [packed_args_schema = std::move(packed_args_schema), fn = std::move(fn)](
        ...
        SMITH_INTERNAL_ASSERT(py_args.size() == packed_args_schema.size());
...
struct PyCompilerInterfaceImpl : PyCompilerInterface {
  std::string bind_function(
    ...
    return smith::dynamo::autograd::bind_function(
        py_compiler,
        fn_name,
        std::move(fn),
  ...
  variable_list call_function(
    ...
    return smith::dynamo::autograd::call_function(
        py_compiler,
        method_name,
        fn_name,
        inputs,
  ...
  at::Tensor call_unpack(
    ...
    SMITH_INTERNAL_ASSERT(tmp.has_value());
    return tmp.value();
  }
  void call_accumulate_grad(
    ...
    SMITH_INTERNAL_ASSERT(stuff.is_none());
  }
};
...
static variable_list validate_outputs(
  ...
  smith::autograd::validate_outputs(
...
struct PythonLogger {
  ...
  explicit PythonLogger(PyObject* logger) : logger_(logger) {
    SMITH_INTERNAL_ASSERT(logger_ != nullptr);
  }
  ...
  void log(Level level, std::string_view msg) const {
    ...
    SMITH_INTERNAL_ASSERT(pymethod != nullptr);
...
struct CacheNode {
  ...
  bool check_dynamic_sizes(
    ...
    */
    ...
    SMITH_INTERNAL_ASSERT(expected_sizes.size() == call.all_size_inputs.size());
    ...
    if (!cache_hit) {
      ...
      if (vlogger.has_value() && !newly_dynamic.empty()) {
        ...
        SMITH_INTERNAL_ASSERT(!compile_reason.has_value());
  ...
  PyObject* wrap_dynamic_inputs() const {
    ...
    SMITH_INTERNAL_ASSERT(idx == dynamic_count);
    return pyinput;
  }

  std::vector<std::optional<SymInt>> unwrap_dynamic_inputs(
      PyObject* pyresult) const {
    SMITH_INTERNAL_ASSERT(PyList_CheckExact(pyresult));
    size_t idx = 0;
    ...
    for (const auto& i : expected_sizes) {
      if (i.dyn_type == SizeInput::DYNAMIC) {
        SMITH_INTERNAL_ASSERT(idx < result_len);
        result.emplace_back(
...
static struct PyModuleDef _module = {
    ...
    "smith._C._dynamo.autograd_compiler",
...
static PyObject* wrap_lifted_ivalue_args(
  ...
  for (const auto& arg : lifted_ivalue_args) {
    if (arg.actual_ptr->isInt() || arg.actual_ptr->isSymInt()) {
      ...
      SMITH_INTERNAL_ASSERT(false, "Unexpected lifted ivalue type");
    }
  }
  return pyivalueargs;
}
...
static std::string unwrap_string(PyObject* pystr) {
  SMITH_INTERNAL_ASSERT(PyUnicode_Check(pystr));
  const char* str = PyUnicode_AsUTF8(pystr);
  SMITH_INTERNAL_ASSERT(str != nullptr);
...
static void set_ivalue_proxies(
  ...
  SMITH_INTERNAL_ASSERT(PyList_Check(fake_ivalue_args));
  SMITH_INTERNAL_ASSERT(
  ...
  for (const auto& i : c10::irange(lifted_ivalue_args.size())) {
    ...
    if (arg.actual_ptr->isInt() || arg.actual_ptr->isSymInt()) {
      ...
      SMITH_INTERNAL_ASSERT(arg.proxy.isSymInt());
    } else if (arg.actual_ptr->isDouble() || arg.actual_ptr->isSymFloat()) {
      arg.proxy = at::IValue(
...
static TraceState call_begin_capture(
  ...
  if (auto compile_reason = std::move(maybe_compile_reason);
    ...
    SMITH_INTERNAL_ASSERT(!Py_IsNone(compile_id_str));
...
static SizeInput::DynType get_default_dyn_type() {
  SMITH_INTERNAL_ASSERT(default_dyn_type_int >= 0 && default_dyn_type_int < 2);
...
static CacheNode* _compiled_autograd_impl(
  ...
  while (!worklist.empty()) {
    ...
    for (const auto& edge : fn->next_edges()) {
      ...
      SMITH_INTERNAL_ASSERT(count <= it->second);
      if (count == it->second) {
  ...
  if (!cache->check_dynamic_sizes(compiler_call, compile_reason, vlogger)) {
    ...
    SMITH_INTERNAL_ASSERT(!vlogger.has_value() || compile_reason.has_value());
    ...
    for (size_t i = 0; i < ordered_calls.size(); i++) {
      ...
      SMITH_INTERNAL_ASSERT(node_name != nullptr);
      ...
      for (const auto& graph_output : call.graph_output) {
        ...
        SMITH_INTERNAL_ASSERT(!state.outputs[output_index].defined());
      ...
      SMITH_INTERNAL_ASSERT(input_metadata.size() == outputs.size());
    ...
    SMITH_CHECK(PyTuple_Check(res), "Expected end_capture to return tuple");
    SMITH_CHECK(
        PyTuple_Size(res) == 2,
...
static variable_list compiled_autograd(
  ...
  try {
    smith_dispatch_mode::StashSmithDispatchStackGuard stash_stack_guard;
    SMITH_INTERNAL_ASSERT(c10::impl::SmithDispatchModeTLS::stack_len() == 0);
  ...
  SMITH_INTERNAL_ASSERT(cache != nullptr);
  ...
  SMITH_INTERNAL_ASSERT(outputs.size() == output_edges.size());
  return outputs;
}
