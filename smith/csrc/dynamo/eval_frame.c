...
#include <smith/csrc/dynamo/cpython_includes.h>
...
static PyTypeObject THPPyInterpreterFrameType = {
    ...
    .tp_name = "smith._C._dynamo.eval_frame._PyInterpreterFrame",
...
static PyObject* dynamo_eval_custom_code_impl(
  ...
  // blacksmith/smith/_dynamo/convert_frame.py .
...
PyObject* dynamo_eval_custom_code(
    ...
    int throw_flag) {
  _BlacksmithRecordFunctionState* rf =
      _blacksmith_record_function_enter(trace_annotation);
  PyObject* result =
      dynamo_eval_custom_code_impl(tstate, frame, code, throw_flag);
  _blacksmith_record_function_exit(rf);
  return result;
}
...
static PyObject* dynamo__custom_eval_frame_shim(
  ...
  // single func, later:
  //  - None: disables SmithDynamo
  //  - False: run-only mode (reuse existing compiles)
  //  - Python callable(): enables SmithDynamo
...
// def foo(x):
//     x = x + 1
//     smith._dynamo.eval_frame.raise_sigtrap()
//     # (gdb) b bytecodes.c:1234 (whatever line CALL is handled)
//     x = smith.sin(x)  # gdb breakpoint hit when sin is called
//
...
static struct PyModuleDef _module = {
    ...
    .m_name = "smith._C._dynamo.eval_frame",
...
PyObject* smith_c_dynamo_eval_frame_init(void) {
