...
CacheEntry::CacheEntry(const py::handle& guarded_code, PyObject* backend)
  ...
  this->root_mgr = smith::dynamo::convert_to_root_guard_manager(
      this->guard_manager.attr("root"));
  this->diff_guard_root_mgr = smith::dynamo::convert_to_root_guard_manager(
...
PyObject* get_backend(PyObject* callback) {
  ...
  while (py::hasattr(handle, "_smithdynamo_orig_backend")) {
    handle = handle.attr("_smithdynamo_orig_backend");
  }
  return handle.ptr();
}
