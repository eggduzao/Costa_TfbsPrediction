...
#include <smith/csrc/autograd/variable_info.h>
...
// libsmith_cpu (where Python is not available).
// A PyCompilerInterfaceImpl in libsmith_python subclasses it and
// overrides the methods to do the actual calls back to Python.
struct SMITH_API PyCompilerInterface {
  ...
  virtual std::string bind_function(
    ...
    SMITH_INTERNAL_ASSERT(false, "Needs to be overridden");
  }
...
SMITH_API const std::unique_ptr<PyCompilerInterface>& getPyCompilerInterface();
struct SMITH_API PyCompilerGuard {
...
// including smith/csrc/autograd/engine.h breaks BC by somehow introducing
...
SMITH_API std::vector<std::optional<InputMetadata>> get_input_metadata(
...
struct NodeCalls : public std::unordered_map<Node*, NodeCall> {
  ...
  const NodeCall& lookup(uint32_t id) const {
    SMITH_INTERNAL_ASSERT(id < nodes.size());
    auto it = find(nodes[id]);
    SMITH_INTERNAL_ASSERT(it != end());
    return it->second;
  }
...
struct TensorArgs {
  ...
  TensorArg& lookup(const at::Tensor& tensor, bool create = false) {
    ...
    if (it == _args.end()) {
      SMITH_INTERNAL_ASSERT(create && inputs.size() == _next_id - 1);
  ...
  TensorArg& lookup(const SavedVariable& sv) {
    ...
    SMITH_INTERNAL_ASSERT(it2 != _saved_variables_proxies.end());
    return *it2->second;
  }
...
struct LiftedIValueArgs {
  ...
  at::IValue& next_proxy(const at::IValue* actual_ptr) {
    SMITH_INTERNAL_ASSERT(next < args.size());
    auto& iv_arg = args.at(next++);
    SMITH_INTERNAL_ASSERT(iv_arg.actual_ptr == actual_ptr);
    return iv_arg.proxy;
  }
...
class CompiledNodeArgs {
 ...
 public:
    ...
    if (iv.isList()) {
      ...
      try {
        ...
        std::string msg =
            ...
            std::string(e.what());
        SMITH_CHECK_NOT_IMPLEMENTED(false, msg);
      }
    }
  }
  ...
  void collect(const smith::autograd::TypeAndSize& t) {
    collect(t.sym_sizes);
    collect(t.options);
  }
...
struct TraceState {
  ...
  void debug_asserts() {
    SMITH_INTERNAL_ASSERT(sym_sizes_index == sym_sizes.size());
  }
...
class SwapSavedVariables {
 ...
 public:
  ...
  retrieve_pynode_objs(Node* pynode) const {
    ...
    SMITH_INTERNAL_ASSERT(it != compiler.pynode_objs.end());
    return it->second;
  }
  ...
  void before(at::Tensor& t) {
    ...
    if (arg.defined()) {
      SMITH_INTERNAL_ASSERT(arg.proxy_tensor.defined());
      t = arg.proxy_tensor;
    }
  }
  ...
  void before(SavedVariable& t) {
    if (auto it = compiler.sv_to_hooks.find(&t);
      ...
      const auto& pyinterface =
          smith::dynamo::autograd::getPyCompilerInterface();
  ...
  void before(smith::autograd::TypeAndSize& t) {
    before(t.sym_sizes);
    before(t.options);
  }
  void after(smith::autograd::TypeAndSize& t) {
    after(t.sym_sizes);
    after(t.options);
  }
 ...
 private:
  ...
  struct StashedVars : public std::unordered_map<const T*, Stashed<T>> {
    ...
    void restore(T* var) {
      ...
      SMITH_INTERNAL_ASSERT(it != this->end(), "missing before())");
    ...
    void debug_assert() {
      SMITH_INTERNAL_ASSERT(this->empty(), "missing call to after()");
    }
  };
...
// - Node::apply_with_saved is in libsmith_cpu which does not have direct access
// to Python
//
...
struct IValuePacker {
  ...
  // parsing. See smith::jit::toIValue for more information.
  static at::TypePtr packed_type() {
    ...
    // (see https://github.com/blacksmith/blacksmith/pull/144707 for examples).
    // It's not clear what the problem is, so we're going to ignore it for now.
    SMITH_CHECK_NOT_IMPLEMENTED(
        false, "smith.compile not supported on Windows");
#else
...
struct IValuePacker<VariableInfo> {
  ...
  static VariableInfo unpack(const at::IValue& t) {
    ...
    SMITH_INTERNAL_ASSERT(elements.size() == 6);
    VariableInfo v;
...
struct IValuePacker<InputMetadata> {
  static at::IValue pack(const InputMetadata& t) {
    SMITH_INTERNAL_ASSERT(!t.is_nested_tensor());
    auto tuple = std::make_tuple(
...
template <>
struct std::hash<smith::dynamo::autograd::CacheKey> {
  size_t operator()(const smith::dynamo::autograd::CacheKey& k) const noexcept {
    return k.hash();
  }
};
