...
namespace {
// Short-term fix for: https://github.com/blacksmith/blacksmith/issues/166926
bool use_lru = true;
} // namespace
...
void lookup(
  ...
  for (const auto& entry : extra_state->precompile_entries) {
    if (smith::dynamo::run_root_guard_manager(entry.root_mgr, f_locals)) {
    ...
    if (valid) {
      try {
        if (is_skip_guard_eval_unsafe) {
          valid = smith::dynamo::run_root_guard_manager(
...
PrecompileEntry::PrecompileEntry(py::object gm, py::object c)
  ...
  root_mgr =
      smith::dynamo::convert_to_root_guard_manager(guard_manager.attr("root"));
}
