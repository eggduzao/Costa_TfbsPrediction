...
#include <smith/csrc/dynamo/framelocals_mapping.h>
...
static constexpr const char* cache_lookup_profiler_str =
    "SmithDynamo Cache Lookup";

// Remember to update the type signature for DynamoCallbackFn.__call__ in
// smith/_dynamo/types.py if this function's signature changes.
...
PyObject* dynamo__custom_eval_frame(
  ...
  auto eval_custom = [&]() {
    ...
    // smith._dynamo.convert_frame.get_fail_callback, which patches
      ...
      if (eval_frame_override == EvalFrameOverride::SKIP) {
        ...
        if (!convert_frame_get_fail_callback) {
          convert_frame_get_fail_callback =
              py::module_::import("smith._dynamo.convert_frame")
  ...
  _BlacksmithRecordFunctionState* rf =
      _blacksmith_record_function_enter(cache_lookup_profiler_str);
  ...
  try {
    ...
    // exception inside the smith.compile block we won't try to Dynamo
    // anything else.
    fail();
    e.restore();
    return eval_result;
