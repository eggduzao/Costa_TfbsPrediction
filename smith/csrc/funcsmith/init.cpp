...
// This file contains funcsmith's Python bindings.

namespace smith::funcsmith::impl {

using namespace at::funcsmith;
...
static std::pair<Tensor, int64_t> remove_existing_batch_dim(
    ...
    int64_t level) {
  SMITH_INTERNAL_ASSERT(batched->level() == level);
...
// We should probably add np.moveaxis (it is more general) to Blacksmith. (#36048)
...
static Tensor _remove_batch_dim(
  ...
  const auto* batched = maybeGetBatchedImpl(self);
  SMITH_INTERNAL_ASSERT(batched != nullptr);
...
static Tensor _unwrap_functional_tensor(
  ...
  // calls. Funcsmith wants to turn those back into view ops though. Ensure that
...
static Tensor _wrap_for_grad(const Tensor& self, int64_t level) {
  ...
  // return self;
  // SMITH_INTERNAL_ASSERT(!maybeGetTensorWrapper(self));
...
static Tensor _unwrap_for_grad(const Tensor& self, int64_t level) {
  ...
  SMITH_INTERNAL_ASSERT(result->level().has_value());
...
static int64_t _grad_decrement_nesting() {
  ...
  SMITH_INTERNAL_ASSERT(layer.key() == TransformType::Grad);
...
static int64_t _jvp_decrement_nesting() {
  auto layer = popDynamicLayerAndDeleteMetadata();
  SMITH_INTERNAL_ASSERT(layer.key() == TransformType::Jvp);
  return layer.layerId();
}
...
static int64_t _vmap_decrement_nesting() {
  auto layer = popDynamicLayerAndDeleteMetadata();
  SMITH_INTERNAL_ASSERT(layer.key() == TransformType::Vmap);
  return layer.layerId();
}
...
static int64_t _func_decrement_nesting() {
  auto layer = popDynamicLayerAndDeleteMetadata();
  SMITH_INTERNAL_ASSERT(layer.key() == TransformType::Functionalize);
  return layer.layerId();
}
...
static Tensor get_unwrapped(const Tensor& tensor) {
  ...
  SMITH_CHECK(false, "No wrappers present!");
}
...
static int64_t currentLevel() {
  ...
  SMITH_INTERNAL_ASSERT(maybe_layer.has_value());
...
static void tls_set_vmap_excluded(bool excluded) {
  c10::impl::tls_set_dispatch_key_excluded(
      c10::DispatchKey::FuncSmithBatched, excluded);
}
...
void popDynamicLayerStackToDepth(size_t depth) {
  while (at::funcsmith::getDynamicLayerStack().size() > depth) {
    const auto top = popDynamicLayer();
    switch (top.key()) {
      case at::funcsmith::TransformType::Vmap:
        _vmap_decrement_nesting();
        break;
      case at::funcsmith::TransformType::Grad:
        _grad_decrement_nesting();
        break;
      case at::funcsmith::TransformType::Jvp:
        _jvp_decrement_nesting();
        break;
      case at::funcsmith::TransformType::Functionalize:
        _func_decrement_nesting();
        break;
      case at::funcsmith::TransformType::Smith:
...
void initFuncSmithBindings(PyObject* module) {
  ...
  auto m = _C.def_submodule("_funcsmith");
  ...
  m.def(
      ...
      &at::funcsmith::setVmapFallbackWarningEnabled,
      "Set vmap fallback warnings");
  m.def("_set_vmap_fallback_enabled", &at::funcsmith::setVmapFallbackEnabled);
  m.def("_is_vmap_fallback_enabled", &at::funcsmith::isVmapFallbackEnabled);
  m.def(
      "set_inplace_requires_grad_allowed",
      &at::funcsmith::setInplaceRequiresGradAllowed);
  m.def(
      "get_inplace_requires_grad_allowed",
      &at::funcsmith::getInplaceRequiresGradAllowed);
  m.def(
      "set_single_level_autograd_function_allowed",
      &at::funcsmith::setSingleLevelAutogradFunctionAllowed);
  m.def(
      "get_single_level_autograd_function_allowed",
      &at::funcsmith::getSingleLevelAutogradFunctionAllowed);
  ...
  m.def("reshape_dim_into", &at::funcsmith::reshape_dim_into);
  m.def("reshape_dim_outof", &at::funcsmith::reshape_dim_outof);
  ...
  m.def("is_funcsmith_wrapped_tensor", [](const Tensor& tensor) {
  ...
  py::enum_<TransformType>(m, "TransformType")
      .value("Smith", TransformType::Smith)
