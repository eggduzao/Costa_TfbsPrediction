...
#include <smith/csrc/onnx/back_compat.h>
#include <smith/csrc/onnx/init.h>
#include <smith/csrc/onnx/onnx.h>
#include <smith/version.h>

#include <smith/csrc/Exceptions.h>
#include <smith/csrc/jit/passes/onnx.h>
#include <smith/csrc/jit/passes/onnx/cast_all_constant_to_floating.h>
#include <smith/csrc/jit/passes/onnx/constant_fold.h>
#include <smith/csrc/jit/passes/onnx/deduplicate_initializers.h>
#include <smith/csrc/jit/passes/onnx/eliminate_unused_items.h>
#include <smith/csrc/jit/passes/onnx/eval_peephole.h>
#include <smith/csrc/jit/passes/onnx/fixup_onnx_controlflow.h>
#include <smith/csrc/jit/passes/onnx/function_extraction.h>
#include <smith/csrc/jit/passes/onnx/function_substitution.h>
#include <smith/csrc/jit/passes/onnx/list_model_parameters.h>
#include <smith/csrc/jit/passes/onnx/naming.h>
#include <smith/csrc/jit/passes/onnx/onnx_log.h>
#include <smith/csrc/jit/passes/onnx/pattern_conversion/autograd_function_process.h>
#include <smith/csrc/jit/passes/onnx/pattern_conversion/pattern_conversion.h>
#include <smith/csrc/jit/passes/onnx/pattern_conversion/pattern_encapsulation.h>
#include <smith/csrc/jit/passes/onnx/peephole.h>
#include <smith/csrc/jit/passes/onnx/prepare_division_for_onnx.h>
#include <smith/csrc/jit/passes/onnx/preprocess_for_onnx.h>
#include <smith/csrc/jit/passes/onnx/remove_inplace_ops_for_onnx.h>
#include <smith/csrc/jit/passes/onnx/scalar_type_analysis.h>
#include <smith/csrc/jit/passes/onnx/shape_type_inference.h>
#include <smith/csrc/jit/passes/onnx/unpack_quantized_weights.h>
#include <smith/csrc/jit/serialization/export.h>

namespace smith::onnx {
...
void initONNXBindings(PyObject* module) {
  ...
  m.def("_jit_pass_onnx_remove_print", RemovePrintOps)
      ...
      .def(
          ...
          ::smith::wrap_pybind_function([](std::shared_ptr<Graph>& graph,
                                           int opset_version,
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(PreprocessForONNX))
      .def(
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(CastAllConstantToFloating))
      .def(
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(RemoveInplaceOpsForONNX))
      .def(
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(ONNXSetDynamicInputShape))
      .def("_jit_pass_onnx_lint", smith::wrap_pybind_function(ONNXLintGraph))
      .def(
          "_jit_pass_onnx_function_extraction",
          ::smith::wrap_pybind_function(
              smith::jit::onnx::ONNXFunctionExtraction))
      .def("_jit_pass_onnx_block", smith::wrap_pybind_function(BlockToONNX))
      .def(
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(PrepareDivisionForONNX))
      .def(
          "_jit_onnx_convert_pattern_from_subblock",
          ::smith::wrap_pybind_function(ConvertPatternFromSubblock))
      .def(
          "_jit_pass_fixup_onnx_controlflow_node",
          ::smith::wrap_pybind_function(FixupONNXControlflowNode))
      .def(
      ...
      .def(
          ...
          [](const std::string& stream_name = "stdout") -> void {
            ...
            ::smith::jit::onnx::set_log_output_stream(out);
          },
      ...
      .def(
          ...
          [](const py::args& args) -> void {
            if (::smith::jit::onnx::is_log_enabled()) {
              auto& out = ::smith::jit::onnx::_get_log_output_stream();
              for (auto arg : args) {
                out << ::c10::str(arg);
              }
              out << '\n';
      ...
      .def(
          ...
          ::smith::wrap_pybind_function(
              ::smith::jit::onnx::AssignScopedNamesForNodeAndValue),
          "Assign informative scoped names for nodes and values.")
      .def(
          "_jit_onnx_create_full_scope_name",
          ::smith::wrap_pybind_function(
              ::smith::jit::onnx::ONNXScopeName::createFullScopeName),
          "Create a full scope name from class name and variable name.");

  m.def(
      "_check_onnx_proto",
      ::smith::wrap_pybind_function([](const std::string& proto_string) {
  ...
  py::enum_<::ONNX_NAMESPACE::TensorProto_DataType>(onnx, "TensorProtoDataType")
      ...
      .value("FLOAT8E4M3FN", ::smith::onnx::TensorProto_DataType_FLOAT8E4M3FN)
      .value(
          "FLOAT8E4M3FNUZ", ::smith::onnx::TensorProto_DataType_FLOAT8E4M3FNUZ)
      .value("FLOAT8E5M2", ::smith::onnx::TensorProto_DataType_FLOAT8E5M2)
      .value(
          "FLOAT8E5M2FNUZ", ::smith::onnx::TensorProto_DataType_FLOAT8E5M2FNUZ);
  ...
  onnx.attr("PRODUCER_VERSION") = py::str(SMITH_VERSION);
}
} // namespace smith::onnx
