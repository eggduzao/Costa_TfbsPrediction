...
static PyObject* MPSModule_isInBadFork(PyObject* self, PyObject* noargs) {
  HANDLE_TH_ERRORS
  return PyBool_FromLong(smith::utils::is_device_in_bad_fork(at::kMPS));
...
static PyObject* MPSModule_getDefaultMPSGenerator(
    ...
    PyObject* noargs) {
  HANDLE_TH_ERRORS
  smith::utils::register_fork_handler_for_device_init(at::kMPS);
...
std::optional<std::vector<T>> optional_vec_from_pyobject(
  ...
  SMITH_CHECK(vec.size() > 0 && vec.size() < 4);
  return vec;
}
 ...
 public:
  ...
  void setValue(
    ...
    if (cast_str.size() == 0) {
      ...
      SMITH_CHECK(false, "Unsupported cast instruction ", default_cast);
    }
  }
  ...
  void setValue(
    ...
    if (py::isinstance<py::tuple>(arg) || py::isinstance<py::list>(arg)) {
      ...
      if (py::isinstance<py::int_>(element)) {
        ...
        SMITH_CHECK(false, "Unexpected argument types");
      }
      ...
      SMITH_CHECK(false, "Unsupported argument type");
    }
  }
