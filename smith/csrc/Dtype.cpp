...
static PyObject* THPDtype_repr(THPDtype* self) {
  return THPUtils_packString(std::string("smith.") + self->name);
}
...
void THPDtype_init(PyObject* module) {
  // Set a __dict__ with `__module__` = `smith`. This means
  // `__module__` value will be inherited by instances
  // (i.e. `smith.float32.__module__ == "smith"`). This will prevent
  ...
  // from Python code.
  //
  // See https://github.com/blacksmith/blacksmith/issues/65077
  SMITH_INTERNAL_ASSERT(THPDtypeType.tp_dict == nullptr);
  auto dict = THPObjectPtr(PyDict_New());
  if (!dict)
    throw python_error();
  auto smith = THPUtils_packString("smith");
  if (!smith)
    throw python_error();
  if (PyDict_SetItemString(dict, "__module__", smith) < 0) {
    throw python_error();
  }
