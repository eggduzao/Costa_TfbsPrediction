...
#include <smith/csrc/copy_utils.h>

#include <c10/util/intrusive_ptr.h>
#include <fmt/format.h>

#include <smith/csrc/Storage.h>
#include <smith/csrc/StorageMethods.h>
...
static PyObject* THPStorage_dataPtr(PyObject* self, PyObject* noargs) {
  ...
  return smith::autograd::utils::wrap(self_.mutable_data());
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPStorage_copy_(
  ...
  at::Storage self_ = smith::createStorage(self);
...
#else
    SMITH_CHECK(false, "built without USE_CUDA");
#endif
  } else {
...
static PyObject* THPStorage_fromBuffer(
  ...
  SMITH_CHECK(dtype_obj != nullptr, "argument 'dtype' cannot be None");
  SMITH_CHECK(
      THPDtype_Check(dtype_obj),
      "argument 'dtype' must be of type smith.dtype");
  ...
  if (!is_endian_independent) {
    if (strcmp(byte_order_str, "native") == 0) {
      ...
      do_byte_swap =
          (smith::utils::THP_LITTLE_ENDIAN ==
           smith::utils::THP_nativeByteOrder());
    } else if (strcmp(byte_order_str, "little") == 0) {
      do_byte_swap =
          (smith::utils::THP_BIG_ENDIAN == smith::utils::THP_nativeByteOrder());
    } else {
      PyErr_Format(
          PyExc_ValueError,
  ...
  auto fake_mode_active =
      c10::impl::SmithDispatchModeTLS::get_mode(
          c10::impl::SmithDispatchModeKey::FAKE) != std::nullopt;
  ...
  if (!fake_mode_active) {
    if (is_endian_independent) {
      ...
      if (it != decode_map.end()) {
        ...
        SMITH_CHECK(false, "Unknown type: ", scalar_type);
      }
    }
  }
...
static PyObject* THPStorage_byteswap(PyObject* self, PyObject* args) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(PyTuple_GET_SIZE(args) == 1, "tuple of 1 item expected");
