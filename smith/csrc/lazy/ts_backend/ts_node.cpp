...
std::string GetFirstUserFrameInPythonIfEnabled() {
  ...
  return smith::lazy::GetFirstUserFrameInPython();
}
} // namespace
...
TSOpVector TensorList::Lower(
  ...
  std::vector<smith::jit::Value*> tensor_list;
  SMITH_CHECK(!operands().empty());
  for (const smith::lazy::Output& operand : operands()) {
