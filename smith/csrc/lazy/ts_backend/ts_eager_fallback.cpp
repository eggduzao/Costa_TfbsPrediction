...
c10::DispatchKey dispatch_key(c10::DeviceType device_type) {
  switch (device_type) {
    ...
    default: {
      SMITH_CHECK(false, "Unsupported device type: ", device_type);
    }
  }
}
...
static std::unordered_map<std::string, ::smith::lazy::Counter*>
...
void ltc_eager_fallback(
  ...
  // Manually applying the SMITH_LAZY_COUNTER macro.
  ...
  if (_eager_fallback_counters.find(name) == _eager_fallback_counters.end()) {
    _eager_fallback_counters[name] = new ::smith::lazy::Counter(name);
  }
  _eager_fallback_counters[name]->AddValue(1);

  auto& args = op.schema().arguments();
  auto arguments = smith::jit::last(stack, args.size());
  ...
  ts_eager_fallback(
      op, stack, smith::lazy::getBackend()->EagerFallbackDeviceType());
}

void register_ts_ltc_eager_fallback() {
  static auto m = MAKE_SMITH_LIBRARY_IMPL(_, Lazy);
  // Most backends use SMITH_LIBRARY_* macros which perform their dispatcher
  // registrations at static library init time, but the lazy Smithscript backend
  // does not since it is built in the main smith lib but not always used.
  // In particular, if another external backend wants to register itself to the
  // same key (Lazy), Smithscript backend must not be initialized.
  m.fallback(smith::CppFunction::makeFromBoxedFunction<&ltc_eager_fallback>());
}
    ...
    if (returns[idx].isTensor()) {
      ...
      if (return_tens.defined()) {
        ...
        if (alias_info != nullptr && alias_info->isWrite()) {
          ...
          // smith.cat() with an empty list In that case, we shouldn't have any
