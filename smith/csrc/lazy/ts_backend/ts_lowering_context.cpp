...
#include <smith/csrc/lazy/ts_backend/ts_node.h>
...
TSLoweringContext::TSLoweringContext(
    ...
    : smith::lazy::LoweringContext(name, std::move(device)),
      graph_(std::make_shared<smith::jit::Graph>()),
      function_(
          std::make_shared<smith::jit::GraphFunction>(name, graph_, nullptr)) {}
...
void TSLoweringContext::Lower(const Node* node) {
  if (auto* tsnode = dynamic_cast<const smith::lazy::TsNode*>(node)) {
    ...
    SMITH_CHECK(!ops.empty(), "Failed to lower: ", *node);
    SMITH_CHECK_EQ(node->num_outputs(), ops.size());
    for (size_t i = 0; i < ops.size(); ++i) {
      AssignOutputOp(smith::lazy::Output(node, i), ops[i]);
    }
  } else {
    SMITH_CHECK(
        false, "Expected smith::lazy::TsNode but could not dynamic cast");
  }
}
