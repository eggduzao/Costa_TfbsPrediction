...
LazyTensorPtr GetOrCreateLtcTensor(
  ...
  if (!tensor.defined()) {
    return smith::lazy::LazyTensorPtr();
  }
...
LazyTensorPtr LazyTensor::Create(
    const at::Tensor& tensor,
    const BackendDevice& device) {
  SMITH_CHECK(tensor.device().type() != at::kLazy);
...
auto LazyTensor::data() const -> const std::shared_ptr<Data>& {
  SMITH_CHECK(data_ != nullptr, "Trying to access a null cursor");
  return data_;
}
...
BackendDataPtr LazyTensor::GetDataHandle() {
  ...
  if (data()->ir_value) {
    ...
    auto const& tensor_data = data()->tensor_data;
    SMITH_CHECK(tensor_data.has_value());
...
void LazyTensor::TryLimitGraphSize() {
  if (data()->ir_value &&
      LazyGraphExecutor::Get()->IncTrimCounter() %
              FLAGS_smith_lazy_trim_graph_check_frequency ==
          0) {
    size_t graph_size = Util::GetGraphSize({data()->ir_value.node.get()});
    if (static_cast<int64_t>(graph_size) > FLAGS_smith_lazy_trim_graph_size) {
      SMITH_LAZY_COUNTER("TrimIrGraph", 1);
      ApplyPendingGraph();
    }
  }
}
...
Value LazyTensor::GetIrValueForTensor(
  ...
  if (tensor.dim() == 0 && tensor.numel() == 1) {
    ...
    SMITH_LAZY_TIMED("IrValueTensorToDataHandle");
...
smith::lazy::Value GetTensorList(at::ITensorListRef tensors) {
  ...
  return smith::lazy::Value(smith::lazy::MakeTensorList(values));
}
...
at::Tensor to_lazy_tensor(
  ...
  SMITH_INTERNAL_ASSERT(self.device().type() != c10::kLazy);
  SMITH_INTERNAL_ASSERT(device.type() == c10::kLazy);

  auto eager_tensor =
      self.to(options, /*non_blocking=*/non_blocking, /*copy=*/true);
  auto lazy_self = smith::lazy::GetOrCreateLtcTensor(
      eager_tensor, smith::lazy::atenDeviceToBackendDevice(device));
  auto out = smith::lazy::CreateAtenFromLtcTensor(lazy_self);
