...
#include <smith/csrc/lazy/core/ir_util.h>
#include <smith/csrc/lazy/core/tensor_util.h>
#include <smith/csrc/lazy/core/unique.h>

#include <smith/csrc/lazy/core/debug_util.h>
#include <smith/csrc/lazy/core/ir_builder.h>
#include <smith/csrc/lazy/core/metrics.h>
#include <smith/csrc/lazy/core/ops/arithmetic_ir_ops.h>
#include <smith/csrc/lazy/core/thread_pool.h>
...
bool TensorCompare(const at::Tensor& t1, const at::Tensor& t2) {
  ...
  // Blacksmith currently has an issue comparing tensors which have NaN values in
  // it. The compare is not deterministic. So we do memory compare here until
  // the Blacksmith equal() API is fixed.
...
auto LazyGraphExecutor::DataCacheArena::Get() -> DataCacheArena* {
  static DataCacheArena* arena =
      new DataCacheArena(FLAGS_smith_lazy_device_data_cache_size);
  return arena;
}
...
BackendDataPtr LazyGraphExecutor::DataCacheArena::GetDeviceData(
  ...
  if (device_data == nullptr) {
    ...
    SMITH_LAZY_COUNTER("DeviceDataCacheMiss", 1);
  }
  return device_data;
}
...
auto LazyGraphExecutor::DataCacheArena::GetDataCache(
  ...
  std::lock_guard<std::mutex> lock(mutex_);
  if (FLAGS_smith_lazy_enable_device_data_cache) {
...
LazyGraphExecutor* LazyGraphExecutor::Get() {
  ...
  SMITH_CHECK(executor, "Lazy graph executor not registered.");
  return executor;
}
...
void LazyGraphExecutor::SyncTensorsGraph(
  ...
  if (FLAGS_smith_lazy_use_thread_pool && wait && async != nullptr) {
    async->mwait.Wait();
  }
}

void LazyGraphExecutor::MarkStep(const BackendDevice& device) {
  SMITH_LAZY_COUNTER("MarkStep", 1);
...
LazyGraphExecutor::SyncTensorCollection LazyGraphExecutor::CollectSyncTensors(
    ...
    if (tensor_ids.insert(tensors[i]->GetUniqueId()).second &&
      ...
      if (ir_value) {
        if (ShouldSyncTensor(tensors[i])) {
          SMITH_LAZY_COUNTER("SyncedTensorsWithIR", 1);
  ...
  if (!at_tensors.empty()) {
    SMITH_LAZY_COUNTER("SyncTensorsToData", at_tensors.size());
...
std::vector<smith::lazy::BackendDataPtr> LazyGraphExecutor::SetTensorData(
    std::vector<LazyTensorPtr>* tensors,
    const SyncTensorsConfig& config,
    c10::ArrayRef<size_t> indices,
...
std::shared_ptr<LazyGraphExecutor::Async> LazyGraphExecutor::TryRunCachedSync(
  ...
  SMITH_LAZY_VALUE_METRIC("TensorsGraphSize", po_data->post_order.size());
...
LazyGraphExecutor::ComputationCache* LazyGraphExecutor::GetComputationCache() {
  static ComputationCache* cache =
      new ComputationCache(FLAGS_smith_lazy_compilation_cache_size);
  return cache;
}
...
LazyGraphExecutor::ComputationCache::TypePtr LazyGraphExecutor::
  ...
  if (cached_computation == nullptr) {
    SMITH_LAZY_COUNTER("UncachedCompile", 1);
    return nullptr;
  }
  SMITH_LAZY_COUNTER("CachedCompile", 1);
...
std::shared_ptr<LazyGraphExecutor::Async> LazyGraphExecutor::
  ...
  SMITH_LAZY_VALUE_METRIC("TensorsGraphSize", compile_result.emitted_nodes);
...
std::shared_ptr<LazyGraphExecutor::Async> LazyGraphExecutor::
  ...
  auto syncfn = [async, hash = coll->hash]() {
    try {
      ...
      SMITH_CHECK(
          ...
          "Expected number of outputs does not match SmithScript Stack size: ",
  ...
  if (FLAGS_smith_lazy_use_thread_pool) {
...
std::vector<at::Tensor> LazyGraphExecutor::GetTensorsFused(
  ...
  if (FLAGS_smith_lazy_use_thread_pool && async != nullptr) {
    async->mwait.Wait();
  }
  ...
  for (const auto i : c10::irange(tensors->size())) {
    if (indices != nullptr && sync_index < indices->size() &&
      ...
      if (tensor_data) {
        ...
        SMITH_CHECK(literals_index < tensors_data.size());
        results.push_back(getBackend()->MakeTensorFromComputationData(
            tensors_data[literals_index], (*tensors)[i]->dtype()));
        ++literals_index;
      }
