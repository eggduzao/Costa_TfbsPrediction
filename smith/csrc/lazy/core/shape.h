...
SMITH_DECLARE_bool(ltc_enable_symbolic_shapes);

namespace smith::lazy {

class SMITH_API Shape {
 public:
  Shape() = default;
...
SMITH_API std::ostream& operator<<(std::ostream& out, const Shape& shape);

SMITH_API bool symbolicShapeEnabled();
// Calculate and applies symbolic shapes onto the
// Shape objects passed to result_shapes
SMITH_API void applySymbolicShapesOnLT(
