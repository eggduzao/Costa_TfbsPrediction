...
#define SMITH_LAZY_COUNTER(name, value)        \
  do {                                         \
    static ::smith::lazy::Counter* __counter = \
        new ::smith::lazy::Counter(name);      \
    __counter->AddValue(value);                \
  } while (0)

#define SMITH_LAZY_FN_COUNTER(ns) SMITH_LAZY_COUNTER(c10::str(ns, __func__), 1)

#define SMITH_LAZY_VALUE_METRIC(name, value)                         \
  do {                                                               \
    static ::smith::lazy::Metric* __metric =                         \
        new ::smith::lazy::Metric(name, smith::lazy::MetricFnValue); \
    __metric->AddSample(value);                                      \
  } while (0)

// Creates a report with the current metrics statistics.
SMITH_API std::string CreateMetricReport();
...
// variables).
SMITH_API std::vector<std::string> GetMetricNames();

// Retrieves the metric data of a given metric, or nullptr if such metric does
// not exist.
SMITH_API MetricData* GetMetric(const std::string& name);
...
// variables).
SMITH_API std::vector<std::string> GetCounterNames();

// Retrieves the counter data of a given counter, or nullptr if such counter
// does not exist.
SMITH_API CounterData* GetCounter(const std::string& name);

// Retrieves the current EPOCH time in nanoseconds.
SMITH_API int64_t NowNs();

// Scope based utility class SMITH_API to measure the time the code takes within
// a given C++ scope.
class SMITH_API TimedSection {
 public:
...
#define SMITH_LAZY_TIMED(name)                                  \
  static smith::lazy::Metric* timed_metric =                    \
      new smith::lazy::Metric(name, smith::lazy::MetricFnTime); \
  smith::lazy::TimedSection timed_section(timed_metric)

#define SMITH_LAZY_FN_COUNTER_TIMED_TRACING(ns) \
  SMITH_LAZY_FN_COUNTER(ns);                    \
  SMITH_LAZY_TIMED("LazyTracing")
