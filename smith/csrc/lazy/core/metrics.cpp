...
const std::vector<double>* ReadEnvPercentiles() {
  std::vector<std::string> percentiles_list =
      StrSplit(FLAGS_smith_lazy_metrics_percentiles, ':');
  ...
  for (auto& pct_str : percentiles_list) {
    ...
    SMITH_CHECK(pct > 0.0 && pct < 1.0, "Invalid percentile: ", pct);
...
Metric::Metric(std::string name, MetricReprFn repr_fn, size_t max_samples)
    : name_(std::move(name)),
      ...
      max_samples_(
          max_samples != 0 ? max_samples : FLAGS_smith_lazy_metrics_samples),
      data_(nullptr) {}
