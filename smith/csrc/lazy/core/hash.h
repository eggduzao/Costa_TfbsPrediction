/**
 * Hash utils in this file is adapted from Blacksmith/XLA
 * https://github.com/blacksmith/xla/blob/e0e5f937a0ba8d904f9608137dc8c51ba439df2d/third_party/xla_client/util.h
 */
#pragma once
...
class SMITH_API hash_t : public c10::uint128 {
 public:
...
size_t SMITH_API StdDataHash(const void* data, size_t size);

size_t SMITH_API StdHashCombine(uintmax_t a, uintmax_t b);

// Other functions are all 128-bit
hash_t SMITH_API HashBlock(const void* data, size_t n, const hash_t& seed);

hash_t SMITH_API DataHash(const void* data, size_t size);

hash_t SMITH_API HashCombine(const hash_t& a, const hash_t& b);

size_t SMITH_API HashReduce(const hash_t& a);

// Returns a string representation of a hash
std::string SMITH_API HashToString(const hash_t& a);
...
hash_t SMITH_API Hash(const std::vector<bool>& value);
...
static inline hash_t Hash(const c10::Scalar& value) {
  switch (value.type()) {
    ...
    default:
      SMITH_INTERNAL_ASSERT(false, "Unknown scalar type.", value.type());
  }
}
