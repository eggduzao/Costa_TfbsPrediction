...
SMITH_API std::vector<int64_t> DropDimensions(
...
SMITH_API int64_t GetCanonicalDimensionIndex(int64_t dim, int64_t rank);

// Same as above, for multiple dimensions.
SMITH_API std::vector<int64_t> GetCanonicalDimensionIndices(
    c10::ArrayRef<int64_t> dimensions,
    int64_t rank);

// Returns the canonical position in the dim dimension, handling negative
// values for the position.
SMITH_API int64_t GetCanonicalPosition(
    c10::ArrayRef<int64_t> dimensions,
    int64_t dim,
    int64_t pos);

// Creates a transposition from the given input and dimensions.
SMITH_API std::vector<int64_t> MakeTransposePermutation(
    int64_t dim0,
    int64_t dim1,
    int64_t rank);
...
SMITH_API std::vector<int64_t> GetPromotedShape(
...
SMITH_API std::vector<std::string> StrSplit(std::string_view text, char delim);
