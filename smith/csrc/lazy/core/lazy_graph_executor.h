...
class SMITH_API LazyGraphExecutor {
 public:
  ...
  // Retrieves the Blacksmith CPU tensors behind the lazy tensors IR operations.
 ...
 protected:
  ...
  // strictly sequential, dictated by the Blacksmith execution order. The
 ...
 private:
  ...
  std::vector<BackendDataPtr> SetTensorData(
      ...
      const SyncTensorsConfig& config,
      c10::ArrayRef<size_t> indices,
      const std::vector<smith::lazy::BackendDataPtr>& tensor_data_vec);
