...
std::unordered_set<std::string>* LoadExperiments() {
  ...
  std::vector<std::string> experiment_list =
      smith::lazy::StrSplit(experiments, ':');
...
std::string GetFirstUserFrameInPython() {
  std::string empty;
  if (!smith::lazy::GetPythonFramesFunction()) {
    return empty;
  }

  auto frames = smith::lazy::GetPythonFramesFunction()();
...
std::string DebugUtil::GetTensorsGraphInfo(
    ...
    GraphFormat format) {
  std::vector<const smith::lazy::Node*> root_nodes;
  std::vector<smith::lazy::Value> root_values;
  std::vector<smith::lazy::hash_t> root_hashes;
  smith::lazy::Unique<smith::lazy::BackendDevice> unique_device;
  if (indices != nullptr) {
    for (auto index : *indices) {
      const smith::lazy::LazyTensorPtr& tensor = tensors[index];
      smith::lazy::Value ir_value = tensor->CurrentIrValue();
      if (ir_value) {
  ...
  for (const auto i : c10::irange(root_hashes.size())) {
    ...
    ss << smith::lazy::HashToString(root_hashes[i]);
  }
  ss << ")\n";

  std::string graph_str;
  if (format == GraphFormat::kText) {
    graph_str = smith::lazy::DumpUtil::ToText(root_nodes);
  } else if (format == GraphFormat::kDot) {
    graph_str = smith::lazy::DumpUtil::ToDot(root_nodes);
  } else if (format == GraphFormat::kBackend) {
    graph_str = smith::lazy::DumpUtil::ToBackend(
        root_values,
        unique_device ? *unique_device : smith::lazy::BackendDevice());
  } else {
