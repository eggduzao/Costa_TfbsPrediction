...
class SMITH_API SymNodeImpl : public c10::SymNodeImpl {
 public:
...
class SMITH_API LazyTensor : public c10::intrusive_ptr_target {
 public:
...
SMITH_API smith::lazy::Value GetTensorList(at::ITensorListRef tensors);
...
SMITH_API LazyTensorPtr TryGetLtcTensor(const at::Tensor& tensor);

// Extracts the LazyTensor out of an at::Tensor. Throws an exception
// if the tensor is not a lazy tensor.
SMITH_API LazyTensorPtr GetLtcTensor(const at::Tensor& tensor);

// Same as above, applied to a list of tensors.
SMITH_API std::vector<LazyTensorPtr> GetLtcTensors(
...
SMITH_API LazyTensorPtr GetOrCreateLtcTensor(
    const std::optional<at::Tensor>& tensor,
    const BackendDevice& device);

SMITH_API LazyTensorPtr GetLtcTensorOrCreateForWrappedNumber(
    const at::Tensor& tensor,
    const BackendDevice& device);

// Section 2: LazyTensor => at::Tensor.
// Creates an ATen tensor from an LazyTensor.
SMITH_API at::Tensor CreateAtenFromLtcTensor(const LazyTensorPtr& ltc_tensor);
SMITH_API at::Tensor CreateAtenFromLtcTensor(LazyTensor&& ltc_tensor);
...
SMITH_API at::Tensor to_lazy_tensor(
