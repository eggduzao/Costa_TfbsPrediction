 ...
 * where we do not yet have structured kernels in blacksmith core.  Ops
 ...
 *
 * https://dev-discuss.blacksmith.org/t/slides-from-structured-kernel-presentation/179
 *
 ...
 *  https://github.com/blacksmith/blacksmith/blob/13b859983183ea9938deb5030ac9a0747841f0a8/smith/csrc/jit/runtime/symbolic_shape_registry.cpp
 *
 * Read the manual (for ops that are 1:1 with python frontend):
 *  https://blacksmith.org/docs/stable/generated/smith.trace.html
 *
 */
...
#include <smith/csrc/lazy/core/dynamic_ir.h>
...
SMITH_API std::vector<Shape> compute_shape_arange_out(
  ...
  // From smith.arange docs:
  // dtype (smith.dtype, optional) – the desired data type of returned tensor.
  // Default: if None, uses a global default (see
  // smith.set_default_dtype()). If dtype is not given, infer the data
  ...
  // get_default_dtype(). Otherwise, the dtype is inferred to be smith.int64.
...
std::vector<Shape> compute_shape_expand(
    ...
    at::IntArrayRef size,
    bool implicit) {
  SMITH_CHECK_GE(static_cast<int64_t>(size.size()), self.dim());
  ...
  for (const auto idx : c10::irange(_sizes.size())) {
    if (auto ma = _sizes[idx].maybe_as_int()) {
      ...
      if (*ma == -1) {
        ...
        SMITH_CHECK(idx >= num_new_dimensions);
        target_size[idx] = padded_self[idx];
      } else {
        target_size[idx] = *ma;
      }
    } else {
      auto* lazySymNode = dynamic_cast<smith::lazy::SymNodeImpl*>(
          _sizes[idx].toSymNodeImplUnowned());
      SMITH_INTERNAL_ASSERT(lazySymNode);
      auto size_node = lazySymNode->node_;
      auto static_value =
          std::dynamic_pointer_cast<smith::lazy::DimensionNode>(size_node)
              ->getStaticValue();
...
std::vector<Shape> compute_shape_index_select(
  ...
  // https://blacksmith.org/docs/stable/generated/smith.index_select.html. Promote
  ...
  SMITH_CHECK(index_dim == 1);

  auto self_sizes = self.sizes();
  std::vector<int64_t> output_sizes(self_sizes.begin(), self_sizes.end());
  SMITH_CHECK(!output_sizes.empty(), "Empty output_sizes is not supported.");
...
std::vector<Shape> compute_shape_cat(at::TensorList tensors, int64_t dim) {
  ...
  SMITH_CHECK(!out_shape.empty(), "Scalar tensors are not supported in cat.");
  SMITH_CHECK(
      extended_dim_shape <=
...
SMITH_API std::vector<smith::lazy::Shape> compute_shape_cholesky(
    const at::Tensor& self,
    bool upper) {
  return {Shape(self.scalar_type(), self.sizes().vec())};
}

std::vector<smith::lazy::Shape> compute_shape_native_batch_norm(
    const at::Tensor& input,
    const ::std::optional<at::Tensor>& weight,
    const ::std::optional<at::Tensor>& bias,
    ...
    double momentum,
    double eps) {
  std::vector<smith::lazy::Shape> shapes;
  shapes.reserve(3);
...
std::vector<smith::lazy::Shape> compute_shape_native_batch_norm_backward(
...
std::vector<Shape> compute_shape_native_layer_norm(
  ...
  for (const auto idx : c10::irange(axis)) {
    SMITH_CHECK(idx < input_shape.size(), "Shape mismatch");
...
std::vector<Shape> compute_shape_sum(
  ...
  // It's undocumented, but smith::sum promotes all integral types to int64_t by
  // default
...
SMITH_API std::vector<smith::lazy::Shape> compute_shape_take(
    const at::Tensor& self,
    const at::Tensor& index) {
...
std::vector<Shape> compute_shape_slogdet(const at::Tensor& self) {
  ...
  SMITH_INTERNAL_ASSERT(self.dim() >= 2);
...
std::vector<smith::lazy::Shape> compute_shape_logical_and(
    const at::Tensor& self,
    const at::Tensor& other) {
  SMITH_INTERNAL_ASSERT(at::are_expandable(self.sizes(), other.sizes()));
  return {Shape(
      c10::ScalarType::Bool, at::infer_size(self.sizes(), other.sizes()))};
}

std::vector<smith::lazy::Shape> compute_shape_logical_not(
    const at::Tensor& self) {
  return {Shape(c10::ScalarType::Bool, self.sizes().vec())};
}

std::vector<smith::lazy::Shape> compute_shape_logical_or(
    const at::Tensor& self,
    const at::Tensor& other) {
  SMITH_INTERNAL_ASSERT(at::are_expandable(self.sizes(), other.sizes()));
  return {Shape(
...
std::vector<smith::lazy::Shape> compute_shape_logical_xor(
    const at::Tensor& self,
    const at::Tensor& other) {
  SMITH_INTERNAL_ASSERT(at::are_expandable(self.sizes(), other.sizes()));
  return {Shape(
...
SMITH_API std::vector<Shape> compute_shape_clone(
    const at::Tensor& self,
    ::std::optional<at::MemoryFormat> memory_format) {
  return {Shape(self.scalar_type(), self.sizes().vec())};
}
...
std::vector<Shape> compute_shape_repeat(
  ...
  SMITH_CHECK_GE(static_cast<int64_t>(repeats.size()), self.dim());
