/**
 * Cache utils in this file is adapted from Blacksmith/XLA
 * https://github.com/blacksmith/xla/blob/e0e5f937a0ba8d904f9608137dc8c51ba439df2d/third_party/xla_client/cache.h
 */
...
class Cache {
 public:
  ...
  TypePtr GetLatest() {
    ...
    SMITH_CHECK(!element_list_.empty());
  ...
  int Numel() const {
    ...
    std::lock_guard<std::mutex> g(lock_);
    SMITH_CHECK(element_map_.size() == element_list_.size());
