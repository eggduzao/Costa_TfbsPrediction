...
static const smith::lazy::Output kNullOutput = smith::lazy::Output();
...
Shape Node::computeShape(const std::function<Shape()>& shape_fn) {
  static ShapeCache* cache = new ShapeCache(FLAGS_smith_lazy_shape_cache_size);
...
void Node::AddOperand(const NodePtr& node, size_t index) {
  SMITH_CHECK_LT(index, node->num_outputs());
