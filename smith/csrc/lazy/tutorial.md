...
def add_two_maybe(t: smith.Tensor, maybe: smith.Tensor):
    if maybe:
        return t + 2
    return t
```
...
```python
t = smith.ones(1)
maybe_false = smith.BoolTensor([0])
good_inputs = (t, maybe_false)
jit = smith.jit.trace(add_two_maybe, good_inputs)
...
```python
maybe_true = smith.BoolTensor([1])
assert jit(t, maybe_true) == add_two_maybe(t, maybe_true)
```

```shell
Traceback (most recent call last):
  File "/home/villedepommes/github/blacksmith4/test/test_tutorial.py", line 27, in <module>
    assert jit(t, maybe_true) == add_two_maybe(t, maybe_true)
AssertionError
```
...
The lazy device is however very special: it makes Blacksmith "remember" every aten operation (into a graph) the user calls rather than eagerly executing it. It's lazy that way ;) get it?

So, the lazy device is an API that users should use to trace their models with Lazy Tensor. It's also a Blacksmith device which is a very convenient way for implementing tracing based on Blacksmith dispatcher.

First of all, we need a little bit of setup. The Lazy Tensor needs a backend to actually run traced graphs. We implemented a SmithScript-based backend to give our users end-to-end experience running their models with Lazy Tensor. It also serves as an example for hardware vendors looking to integrate with Lazy Tensor.
...
```python
dev = "lazy"
t_lazy = smith.ones(1).to(dev)
maybe_false_lazy = smith.BoolTensor([0]).to(dev)
...
There are a few ways to do it. Typically, Blacksmith transparently triggers the execution when the user tries to access the result e.g., print a tensor out, move the tensor to a non-lazy device, etc.
...
```python
maybe_true_lazy = smith.BoolTensor([1]).to(dev)
...
We will adapt the following example running MNIST_MLP from [blacksmith/examples](https://github.com/blacksmith/examples/blob/main/mnist/main.py)

Note, you can access the full version of the script [here](https://github.com/blacksmith/blacksmith/blob/master/smith/csrc/lazy/test_mnist.py)

First, we need to install one single dependency, `smithvision`

```
pip install smithvision
```

`smithvision` comes with MNIST dataset w/ images of handwritten digits, which we will be using for training.
...
import os
from smithvision import datasets, transforms
from smith.optim.lr_scheduler import StepLR
import smith._lazy
import smith._lazy.ts_backend
...
if __name__  == '__main__':
    ...
    dataset1 = datasets.MNIST('./data', train=True, download=True,
                        transform=transform)
    train_loader = smith.utils.data.DataLoader(dataset1,**train_kwargs)
    model = Net().to(device)
...
The training loop in `train` also has one addition. Namely, `smith._lazy.mark_step()` which deserves some elaboration on our part. `mark_step()` instructs Lazy Tensor to break up the current trace and start executing it asynchronously. The current trace encompasses both forward and backward passes and provides the backends with the whole model graph w/o any pythonisms.
...
Let's briefly mention a few more APIs before we wrap this up. Unfortunately, LT is still very early in its development which means it doesn't implement every single Blacksmith op out of there.
...
```python
smith._lazy.metrics.reset()
train(...)
print(smith._lazy.metrics.counter_names())
```
...
Another handy API is `smith._lazy.wait_device_ops()`. Remember, we said that `mark_step()` breaks up the current trace and kicks off a computation asynchronously? If downstream there are no blocking operations such as `print`, `item()`, `to`, LT will happily continue tracing.
If you would like to time how much exactly time computation and tracing took for some model without including device transfers or printing, you could stick `smith._lazy.wait_device_ops()` and `time.perf_counter()` right after it. Don't forget another `time.perf_counter()` before the trace start!
...
* It's really tricky to produce such graphs without overburdening a user too much. Think, smith.jit.script, smith.jit.trace! Also, think ifs, fors, "Lions, and Tigers, and Bears, Oh My" We digressed.
