 ...
 */
class SMITH_API BackendImplInterface {
 public:
  ...
  virtual BackendDataPtr MakeComputationDataFromScalar(
      ...
      const smith::lazy::BackendDevice& device) const = 0;
...
class SMITH_API BackendRegistrar {
 public:
  BackendRegistrar(const BackendImplInterface* backend_impl_interface);
};

SMITH_API bool hasBackend();
SMITH_API const BackendImplInterface* getBackend();

SMITH_API const IrBuilder* getIrBuilder();
