...
#include <smith/csrc/lazy/core/config.h>
...
#include <smith/csrc/lazy/core/trie.h>
#include <smith/csrc/lazy/python/python_util.h>
...
// device types
static smith::lazy::BackendDevice GetDeviceOrCurrent(
    const std::string& device_str) {
  if (device_str.empty()) {
    getBackend()->GetDefaultDeviceType();
    return smith::lazy::BackendDevice();
  }
  return smith::lazy::atenDeviceToBackendDevice(c10::Device(device_str));
}
...
static std::string GetTensorsDump(
    ...
    const std::function<std::string(c10::ArrayRef<const smith::lazy::Node*>)>&
        converter) {
  std::vector<const smith::lazy::Node*> nodes;
  std::vector<smith::lazy::Value> values;
  for (auto& tensor : tensors) {
    auto inner = at::functionalization::impl::from_functional_tensor(tensor);
    smith::lazy::LazyTensorPtr lazy_tensor =
        smith::lazy::TryGetLtcTensor(inner);
...
static std::vector<smith::lazy::LazyTensorPtr> GetLtcTensors(
    const std::vector<at::Tensor>& tensors,
    bool want_all) {
  std::vector<smith::lazy::LazyTensorPtr> lazy_tensors;
  lazy_tensors.reserve(tensors.size());
  if (want_all) {
    for (auto& tensor : tensors) {
      lazy_tensors.push_back(smith::lazy::TryGetLtcTensor(tensor));
    }
  } else {
    for (auto& tensor : tensors) {
      auto lazy_tensor = smith::lazy::TryGetLtcTensor(tensor);
      if (lazy_tensor) {
...
static std::string GetTensorsBackendGraph(
  ...
  return smith::lazy::LazyGraphExecutor::Get()->DumpBackendComputation(
      lazy_tensors);
}
...
static void SyncTensors(
  ...
  std::vector<smith::lazy::LazyTensorPtr> lazy_tensors =
      GetLtcTensors(tensors, /*want_all=*/false);
  smith::lazy::LazyGraphExecutor::Get()->SyncTensorsGraph(
...
void initLazyBindings(PyObject* module) {
  ...
  lazy.def(
      ...
      [](const std::string& device_str,
        ...
        smith::lazy::LazyGraphExecutor::Get()->SyncLiveTensorsGraph(
            &backend_device, devices, wait);
        smith::lazy::LazyGraphExecutor::Get()->MarkStep(backend_device);
      },
      py::arg("device") = "",
      py::arg("devices"),
      py::arg("wait") = true);
  lazy.def(
      ...
      [](const std::vector<std::string>& devices) {
        ...
        smith::lazy::LazyGraphExecutor::Get()->WaitDeviceOps({});
      },
      py::arg("devices"));
  lazy.def("_reset_metrics", []() {
    smith::lazy::MetricsArena::Get()->ResetCounters();
    smith::lazy::MetricsArena::Get()->ResetMetrics();
  });
  lazy.def("_counter_names", []() { return smith::lazy::GetCounterNames(); });
  lazy.def(
      "_metrics_report", []() { return smith::lazy::CreateMetricReport(); });
  lazy.def("_counter_value", [](const std::string& name) -> py::object {
    smith::lazy::CounterData* data = smith::lazy::GetCounter(name);
  ...
  lazy.def(
      ...
      [](const std::vector<at::Tensor>& tensors) -> std::string {
        auto converter = [](c10::ArrayRef<const smith::lazy::Node*> nodes) {
          return smith::lazy::DumpUtil::ToText(nodes);
        };
  ...
  lazy.def(
      ...
      [](const std::vector<at::Tensor>& tensors) -> std::string {
        auto converter = [](c10::ArrayRef<const smith::lazy::Node*> nodes) {
          return smith::lazy::DumpUtil::ToDot(nodes);
        };
        return GetTensorsDump(tensors, converter);
      });
  lazy.def(
  ...
  lazy.def("_get_force_fallback", []() {
    return smith::lazy::getLTCForceFallback();
  });
  lazy.def("_set_force_fallback", [](std::string newval) {
    smith::lazy::getLTCForceFallback() = std::move(newval);
  });
  ...
  lazy.def("_set_reuse_ir", [](bool val) { FLAGS_smith_lazy_reuse_ir = val; });
...
#else
      SMITH_CHECK(false, "SmithScript backend not yet supported in FBCODE/OVRSOURCE builds");
  ...
  lazy_ts_backend.def(
      ...
      [](const std::vector<at::Tensor>& tensors)
        ...
        for (auto nodeptr : post_order) {
          if (nodeptr->op() == *smith::lazy::ltc_device_data) {
            ...
            auto* tsDataPtr = (smith::lazy::TSData*)backend_data.get();
            ...
            if (tsDataPtr->HasValue()) {
              ...
              SMITH_CHECK(tsDataPtr->scalar.has_value());
...
#else
        SMITH_CHECK(
            false, "SmithScript backend not yet supported in FBCODE builds");
  ...
  lazy_ts_backend.def(
      ...
      [](const std::string& hash_str,
...
#if !(defined(FBCODE_CAFFE2) || defined(OVRSOURCE))
        SMITH_CHECK(hash_str.size() == sizeof(hash_t));
        ...
        auto computationPtr =
            (smith::lazy::TSComputation*)cachedComputation->computation.get();
        ...
        for (smith::jit::IValue elem : stack) {
  ...
  lazy_ts_backend.def("_get_latest_computation_graph", []() {
    ...
    SMITH_CHECK(ts_computation, "Found non-TSComputation in cache");
  ...
  // smithdeploy/multipy possibly because  // codespell:ignore multipy
  ...
  // When libsmith_python is loaded, we register the python frame getter
