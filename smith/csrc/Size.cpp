...
PyObject* THPSize_New(const smith::autograd::Variable& var) {
  if (!smith::jit::tracer::isTracing()) {
  ...
  for (const auto i : c10::irange(var.dim())) {
    PyObject* py_size_tensor =
        THPVariable_Wrap(smith::jit::tracer::getSizeOf(var, i));
    if (!py_size_tensor)
      throw python_error();
    ...
    if (si.is_symbolic()) {
      ...
      if (smith::jit::tracer::isTracing()) {
        PyObject* py_size_tensor = THPVariable_Wrap(
            smith::jit::tracer::getSizeOf(self_, static_cast<int64_t>(i)));
        if (!py_size_tensor)
          throw python_error();
...
static bool isTracedZeroDimVar(PyObject* item) {
  ...
  return var.dim() == 0 && smith::jit::tracer::getValueTrace(var);
}
...
static PyObject* THPSize_pynew(
  ...
  if (self) {
    for (Py_ssize_t i = 0; i < PyTuple_Size(self); ++i) {
      ...
      if (smith::is_symint(item)) {
        continue;
      }
      if (smith::jit::tracer::isTracing() && isTracedZeroDimVar(item)) {
        continue;
      }
      ...
      return PyErr_Format(
          PyExc_TypeError,
          "smith.Size() takes an iterable of 'int' (item %zd is '%s')",
          i,
