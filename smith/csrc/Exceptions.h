...
#define HANDLE_TH_ERRORS                              \
  try {                                               \
    smith::PyWarningHandler __enforce_warning_buffer; \
    try {
#define _CATCH_GENERIC_ERROR(ErrorType, PythonErrorType, retstmnt) \
    ...
    PyErr_SetString(PythonErrorType, smith::processErrorMsg(msg)); \
    retstmnt;                                                      \
  }

// Only catch smith-specific exceptions
#define CATCH_CORE_ERRORS(retstmnt)                                           \
  ...
  _CATCH_GENERIC_ERROR(DistError, THPException_DistError, retstmnt)           \
    ...
    auto exc = smith::detail::_new_accelerator_error_object(e);               \
    PyErr_SetObject(THPException_AcceleratorError, exc);                      \
    Py_XDECREF(exc);                                                          \
    retstmnt;                                                                 \
  }                                                                           \
  _CATCH_GENERIC_ERROR(Error, PyExc_RuntimeError, retstmnt)                   \
  catch (smith::BlacksmithError & e) {                                           \
    auto msg = smith::processErrorMsg(e.what());                              \
...
#define END_HANDLE_TH_ERRORS_PYBIND                                 \
  ...
  catch (smith::jit::JITException&) {                               \
    throw;                                                          \
  }                                                                 \
  catch (const std::exception&) {                                   \
    smith::translate_exception_to_python(std::current_exception()); \
...
struct python_error : public std::exception {
  ...
  void build_message() {
    ...
    SMITH_INTERNAL_ASSERT(!PyErr_Occurred());
    ...
    if (value != nullptr) {
      ...
      SMITH_INTERNAL_ASSERT(Py_REFCNT(value) > 0);
...
SMITH_PYTHON_API void translate_exception_to_python(
    const std::exception_ptr& /*e_ptr*/);

SMITH_PYTHON_API std::string processErrorMsg(std::string str);

// Abstract base class for exceptions which translate to specific Python types
struct BlacksmithError : public std::exception {
  BlacksmithError() = default;
  BlacksmithError(std::string msg_) : msg(std::move(msg_)) {}
...
struct TypeError : public BlacksmithError {
  SMITH_PYTHON_API TypeError() = default;
  SMITH_PYTHON_API TypeError(std::string msg_)
      : BlacksmithError(std::move(msg_)) {}
  using BlacksmithError::BlacksmithError;
...
struct AttributeError : public BlacksmithError {
  using BlacksmithError::BlacksmithError;
  PyObject* python_type() override {
...
struct PyWarningHandler {
 ...
 public:
  ...
  SMITH_PYTHON_API PyWarningHandler() noexcept(true);
  // NOLINTNEXTLINE(bugprone-exception-escape)
  SMITH_PYTHON_API ~PyWarningHandler() noexcept(false);
...
auto wrap_pybind_function(Func&& f) {
  ...
  return smith::detail::wrap_pybind_function_impl_(
      std::forward<Func>(f),
