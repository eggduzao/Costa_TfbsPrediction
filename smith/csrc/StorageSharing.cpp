...
#include <smith/csrc/StorageSharing.h>
...
static PyObject* THPStorage_newSharedFilename(
    ...
    PyObject* args) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(PyTuple_GET_SIZE(args) == 3, "tuple of 3 items expected");
...
static PyObject* THPStorage_newSharedFd(PyObject* _unused, PyObject* args) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(PyTuple_GET_SIZE(args) == 2, "tuple of 2 items expected");
...
static PyObject* THPStorage_shareCuda(PyObject* self, PyObject* noargs) {
  ...
  if (storage.data()) {
    ...
    at::DataPtr sent_data_ptr = smith::GetNewRefCountedSentData(
        storage.mutable_data(), storage.device());
    auto old_data_ptr = storage.set_data_ptr(std::move(sent_data_ptr));
    auto sent_data =
        static_cast<smith::CudaIPCSentData*>(storage.data_ptr().get_context());
  ...
  return tuple.release();
#else
  SMITH_CHECK(false, "CUDA is not available");
#endif
  END_HANDLE_TH_ERRORS
}
...
static PyObject* THPStorage_releaseIPCCounter(
  ...
  try {
    auto sptr = at::RefcountedMapAllocator::makeDataPtr(
        ...
        flags,
        sizeof(int64_t) * smith::CUDA_IPC_REF_COUNTER_FILE_SIZE,
        nullptr);
...
static std::string THPStorage_bytesAsHandleString(PyObject* handle) {
  ...
  if (PyBytes_AsStringAndSize(handle, &buffer, &handle_size) == -1) {
    SMITH_CHECK(handle_size == CUDA_IPC_HANDLE_SIZE, "incorrect handle");
  }
...
static PyObject* THPStorage_newSharedCuda(PyObject* _unused, PyObject* args) {
  HANDLE_TH_ERRORS
#ifdef USE_CUDA
  SMITH_CHECK(PyTuple_GET_SIZE(args) == 8, "tuple of 8 items expected");
  ...
  struct IpcDeleterContext {
    ...
    smith::CudaIPCReceivedData received_data;
  };
...
static PyObject* THPStorage_expired(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  SMITH_CHECK(THPUtils_checkLong(arg), "_expired(): arg must be an 'int'");
  c10::StorageImpl* weak_storage =
      static_cast<c10::StorageImpl*>(PyLong_AsVoidPtr(arg));
...
static PyObject* THPStorage_sharedFd(PyObject* self, PyObject* noargs) {
  ...
  SMITH_CHECK(ctx, "couldn't retrieve a shared file descriptor");
