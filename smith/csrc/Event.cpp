...
static PyObject* THPEvent_pynew(
  ...
  smith::ParsedArgs<4> parsed_args;
  ...
  if (!ptr) {
    SMITH_CHECK(ptr, "Failed to allocate memory for Event");
  }
  ...
  new (&self->event) c10::Event(
      ...
      // BLACKSMITH_DEFAULT is a disable-timing flag.
      (enable_timing ? c10::EventFlag::BACKEND_DEFAULT
                     : c10::EventFlag::BLACKSMITH_DEFAULT));
...
PyObject* THPEvent_new(c10::DeviceType device_type, c10::EventFlag flag) {
  ...
  SMITH_CHECK(self, "Failed to allocate memory for Event");
...
static PyObject* THPEvent_record(
  ...
  if (!PyArg_ParseTupleAndKeywords(
          ...
          &_stream)) {
    SMITH_WARN("Parsing THPEvent_record arg fails");
    return nullptr;
  }
  if (_stream != Py_None) {
...
static PyObject* THPEvent_from_ipc_handle(
  ...
  SMITH_CHECK_NOT_IMPLEMENTED(
      false,
      "smith.Event ipc is not supported yet, please open an issue if you need this!");
  THPObjectPtr ptr(type->tp_alloc(type, 0));
  if (!ptr) {
    return nullptr;
  }
  ...
  new (&self->event) c10::Event(device.type(), c10::EventFlag::BLACKSMITH_DEFAULT);
...
static PyObject* THPEvent_wait(
  ...
  HANDLE_TH_ERRORS {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            ...
            const_cast<char**>(accepted_args),
            &_stream)) {
      SMITH_WARN("Parsing THPEvent_wait arg fails");
      return nullptr;
    }
    if (_stream != Py_None) {
      auto stream = reinterpret_cast<THPStream*>(_stream);
