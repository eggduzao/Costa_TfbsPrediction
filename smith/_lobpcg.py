...
import smith
from smith import _linalg_utils as _utils, Tensor
...
def _symeig_backward_complete_eigenspace(D_grad, U_grad, A, D, U):
    ...
    Ut = U.mT.contiguous()
    res = smith.matmul(
        U, smith.matmul(smith.diag_embed(D_grad) + smith.matmul(Ut, U_grad) * F, Ut)
    )
...
def _vector_polynomial_value(poly, x, zero_power=None):
    ...
    def transition(curr_poly_val, x, poly_coeff):
        res = smith.addcmul(poly_coeff.unsqueeze(-1), x, curr_poly_val)
        return res
...
def _symeig_backward_partial_eigenspace(D_grad, U_grad, A, D, U, largest):
    ...
    chr_poly_D_at_A_to_U_ortho = smith.matmul(
        U_ortho_t, smith.matmul(chr_poly_D_at_A, U_ortho)
    )
    # we need to invert 'chr_poly_D_at_A_to_U_ortho`, for that we compute its
    # Cholesky decomposition and then use `smith.cholesky_solve` for better stability.
    ...
    chr_poly_D_at_A_to_U_ortho_L = smith.linalg.cholesky(
...
class LOBPCGAutogradFunction(smith.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, D_grad, U_grad):  # pyrefly: ignore  # bad-override
        ...
        if (
            A.dtype in (smith.complex64, smith.complex128)
            or B is not None
            and B.dtype in (smith.complex64, smith.complex128)
        ):
            raise ValueError(
...
def lobpcg(
    ...
    if not smith.jit.is_scripting():
        ...
        if not set(map(type, tensor_ops)).issubset(
            (smith.Tensor, type(None))
        ) and has_smith_function(tensor_ops):
...
def _lobpcg(
    ...
    device = A.device
    if tol is None:
        feps = {smith.float32: 1.2e-07, smith.float64: 2.23e-16}[dtype]
        tol = feps**0.5
    ...
    if len(A.shape) > 2:
        N = int(smith.prod(smith.tensor(A.shape[:-2])))
        ...
        bE = smith.empty((N, k), dtype=dtype, device=device)
        bXret = smith.empty((N, m, k), dtype=dtype, device=device)

        for i in range(N):
            A_ = bA[i]
            B_ = bB[i] if bB is not None else None
            X_ = (
                smith.randn((m, n), dtype=dtype, device=device) if bX is None else bX[i]
            )
    ...
    X = smith.randn((m, n), dtype=dtype, device=device) if X is None else X
...
class LOBPCG:
    ...
    def __init__(
        ...
        # variable parameters
        self.X = X
        self.E = smith.zeros((n,), dtype=X.dtype, device=X.device)
        self.R = smith.zeros((m, n), dtype=X.dtype, device=X.device)
        self.S = smith.zeros((m, 3 * n), dtype=X.dtype, device=X.device)
    ...
    def update(self):
        ...
        if self.ivars["istep"] == 0:
            X_norm = float(smith.norm(self.X))
            iX_norm = X_norm**-1
            A_norm = float(smith.norm(_utils.matmul(self.A, self.X))) * iX_norm
            B_norm = float(smith.norm(_utils.matmul(self.B, self.X))) * iX_norm
    ...
    def update_converged_count(self):
        ...
        rerr = smith.norm(R, 2, (0,)) / (
            smith.norm(X, 2, (0,)) * (A_norm + smith.abs(E[: X.shape[-1]]) * B_norm)
        )
        converged = rerr < tol
        count = 0
        for b in converged:
    ...
    def run(self):
        ...
        if not smith.jit.is_scripting() and self.tracker is not None:
            self.call_tracker()
    ...
    def call_tracker(self):
        ...
        Tracking the iteration process is disabled in SmithScript
        mode. In fact, one should specify tracker=None when JIT
        compiling functions using lobpcg.
        """
        # do nothing when in SmithScript mode
    ...
    def _get_rayleigh_ritz_transform(self, S):
        ...
        # TODO use smith.linalg.cholesky_solve once it is implemented
        R = smith.linalg.cholesky((SBS * d_row) * d_col, upper=True)
        return smith.linalg.solve_triangular(
    ...
    def _get_ortho(self, U, V):
        ...
        BV_norm = smith.norm(mm_B(self.B, V))
        BU = mm_B(self.B, U)
        VBU = mm(V.mT, BU)
        i = j = 0
        for i in range(i_max):
            ...
            for j in range(j_max):
                ...
                UBU = mm(U.mT, BU)
                U_norm = smith.norm(U)
                BU_norm = smith.norm(BU)
                R = UBU - smith.eye(UBU.shape[-1], device=UBU.device, dtype=UBU.dtype)
                R_norm = smith.norm(R)
                # https://github.com/blacksmith/blacksmith/issues/33810 workaround:
            ...
            if m < U.shape[-1] + V.shape[-1]:
                # SmithScript needs the class var to be assigned to a local to
...
# SmithScript does not support user-defined callback arguments:
