...
from smith._ops import HigherOrderOperator, OperatorBase, OpOverload, OpOverloadPacket
from smith._prims_common import CustomOutParamAnnotation
...
# TODO: relax key type here; smith registrations should be possible to; but
# right now this type is accurate
global_decomposition_table: dict[str, dict[smith._ops.OperatorBase, Callable]] = (
    defaultdict(dict)
)
...
def _should_decompose_because_unsafe_op(op: smith._ops.OperatorBase) -> bool:
    """
    ...
    if not isinstance(op, smith._ops.OpOverload):
        return False
    if smith.Tag.maybe_aliasing_or_mutating in op.tags:
        return True
    return op is smith.ops.aten.native_batch_norm.default
...
def _add_op_to_registry(registry, op, fn):
    ...
    """
    overloads: list[Union[smith._ops.OperatorBase]] = []
    ...
    for op_overload in overloads:
        ...
        # SmithScript dumps a bunch of extra nonsense overloads
        # which don't have corresponding dispatcher entries, we need
        # to filter those out, e.g aten.add.float_int
        if smith._C._dispatch_has_kernel(op_overload.name()):
...
def _convert_out_params(f):
    ...
    if getattr(out_annotation, "__origin__", None) is tuple:
        ...
        _fn._smith_decompositions_out_wrapper = f._smith_decompositions_out_wrapper  # type: ignore[attr-defined]
...
def register_decomposition(
    ...
    decomposition table.  Use it like this::
        ...
        @register_decomposition(smith.ops.aten.clamp_min)
        def clamp_min(x):
            return smith.clamp(self, min=min)

    If you are writing a new decomposition, consider contributing it
    directly to Blacksmith in smith._decomp.decompositions.
...
def get_decompositions(
    aten_ops: Sequence[Union[smith._ops.OperatorBase, OpOverloadPacket]],
    type: str = "post_autograd",
) -> dict[smith._ops.OperatorBase, Callable]:
    """
    ...
    decompositions: dict[smith._ops.OperatorBase, Callable] = {}
    for op in aten_ops:
        if isinstance(op, OpOverloadPacket) and op in packets_to_overloads:
            for op_overload in packets_to_overloads[op]:
                decompositions[op_overload] = registry[op_overload]
        elif isinstance(op, (smith._ops.OperatorBase)) and op in registry:
...
# populate the table
import smith._decomp.decompositions
import smith._refs


def core_aten_decompositions() -> "CustomDecompTable":
    from smith.export.exported_program import default_decompositions
...
#
# list was copied from smith/_inductor/decomposition.py
# excluding decompositions that results in prim ops
# Resulting opset of decomposition is core aten ops
def _core_aten_decompositions_post_autograd() -> dict[
    smith._ops.OperatorBase, Callable
]:
    aten = smith.ops.aten
