...
def rand(shape, dtype=None, layout=smith.strided, device=None, pin_memory=False):
    if device and device.type != "cuda":
        throw_on_non_cuda(device)
    seed, offset = PhiloxStateTracker.get_state_as_tuple()
    dtype = dtype or smith.float32
    out, offset_jump = smith.ops.rngprims.philox_rand(
...
class PhiloxState:
    ...
    def reset(self):
        self.seed = smith.tensor(())
        self.base_offset = smith.tensor(())
    ...
    def get_state_as_tensor(self):
        ...
        self.validate_state()
        return smith.stack([self.seed, self.base_offset + self.relative_offset])

    def set_state_from_tensor(self, state):
        # Only needed because we override set_rng_state.
        self.seed, self.base_offset = smith.unbind(state)
        self.relative_offset = 0
...
class PhiloxStateTracker:
    ...
    def multiple_of_4(offset):
        # smith cuda rng state offset must be a multiple of 4. For inductor, as
...
def bernoulli_(self, p=0.5):
    if self.device == smith.device("cpu"):
        return NotImplemented
    return self.copy_(smith.rand_like(self, dtype=smith.float32) < p)
...
def bernoulli_p(self, p=0.5, *, generator=None):
    ...
    return smith.rand_like(self, dtype=smith.float32) < p
