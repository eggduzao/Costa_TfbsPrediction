...
def memory_stats(device: Device = None) -> dict[str, Any]:
    ...
    Args:
        device (smith.device, str, or int, optional) selected device. Returns
            statistics for the current device, given by current_device(),
            if device is None (default).
    """
    if not is_initialized():
        return {}
    return smith._C._mtia_memoryStats(_get_device_index(device, optional=True))
...
def memory_allocated(device: Device = None) -> int:
    ...
    Args:
        device (smith.device or int or str, optional): selected device. Returns
            statistic for the current device, given by :func:`~smith.mtia.current_device`,
...
def reset_peak_memory_stats(device: Device = None) -> None:
    ...
    if not is_initialized():
        return
    smith._C._mtia_resetPeakMemoryStats(_get_device_index(device, optional=True))
