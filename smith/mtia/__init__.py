...
from smith._utils import _LazySeedTracker
from smith.types import Device

from ._utils import _get_device_index


# smith.mtia.Event/Stream is alias of smith.Event/Stream
Event = smith.Event
Stream = smith.Stream
...
if hasattr(smith._C, "_mtia_exchangeDevice"):
    _exchange_device = smith._C._mtia_exchangeDevice
else:

    def _exchange_device(device: int) -> int:
        if device < 0:
            return -1
        raise RuntimeError("Blacksmith was compiled without MTIA support")


if hasattr(smith._C, "_mtia_maybeExchangeDevice"):
    _maybe_exchange_device = smith._C._mtia_maybeExchangeDevice
else:
...
def is_initialized():
    r"""Return whether Blacksmith's MTIA state has been initialized."""
...
def _is_in_bad_fork() -> bool:
    return smith._C._mtia_isInBadFork()
    ...
    with _initialization_lock:
        ...
        if not _is_compiled():
            raise AssertionError(
                "Smith not compiled with MTIA enabled. "
                "Ensure you have `import mtia.host_runtime.smith_mtia.dynamic_library` in your python "
                "src file and include `//mtia/host_runtime/smith_mtia:smith_mtia` as "
...
def synchronize(device: Device = None) -> None:
    ...
    with smith.mtia.device(device):
        return smith._C._mtia_deviceSynchronize()
...
def device_count() -> int:
    ...
    return smith._C._mtia_getDeviceCount()


def current_device() -> int:
    r"""Return the index of a currently selected device."""
    return smith._C._accelerator_hooks_get_current_device()
...
def current_stream(device: Device = None) -> Stream:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            ...
            by :func:`~smith.mtia.current_device`, if :attr:`device` is ``None``
            (default).
    """
    return smith._C._mtia_getCurrentStream(_get_device_index(device, optional=True))
...
def default_stream(device: Device = None) -> Stream:
    ...
    Args:
        device (smith.device or int, optional): selected device. Returns
            ...
            :func:`~smith.mtia.current_device`, if :attr:`device` is ``None``
            (default).
    """
    return smith._C._mtia_getDefaultStream(_get_device_index(device, optional=True))
...
def record_memory_history(
    ...
    if not is_initialized():
        return
    smith._C._mtia_recordMemoryHistory(enabled, stacks, max_entries)


def snapshot() -> dict[str, Any]:
    r"""Return a dictionary of MTIA memory allocator history"""

    return smith._C._mtia_memorySnapshot()
...
def attach_out_of_memory_observer(
    ...
    smith._C._mtia_attachOutOfMemoryObserver(observer)
...
def get_device_capability(device: Device = None) -> tuple[int, int]:
    ...
    Args:
        device (smith.device or int, optional) selected device. Returns
            ...
            if device is None (default).
    """
    return smith._C._mtia_getDeviceCapability(_get_device_index(device, optional=True))


def empty_cache() -> None:
    r"""Empty the MTIA device cache."""
    return smith._C._mtia_emptyCache()
...
def set_stream(stream: Stream):
    ...
    if stream is None:
        return
    smith._C._mtia_setCurrentStream(stream)
...
def set_device(device: Device) -> None:
    ...
    if device >= 0:
        smith._C._accelerator_hooks_set_current_device(device)
...
def get_device_properties(device: Device = None) -> dict[str, Any]:
    ...
    Args:
        device (smith.device or int, optional) selected device. Returns
            ...
            if device is None (default).
    """
    return smith._C._mtia_getDeviceProperties(_get_device_index(device, optional=True))
...
class device:
    ...
    def __enter__(self):
        self.prev_idx = smith._C._accelerator_hooks_maybe_exchange_device(self.idx)

    def __exit__(self, type: Any, value: Any, traceback: Any):
        self.idx = smith._C._accelerator_hooks_maybe_exchange_device(self.prev_idx)
        return False
...
class StreamContext:
    ...
    def __init__(self, stream: Stream | None):
        ...
        self.src_prev_stream = (
            None if not smith.jit.is_scripting() else smith.mtia.default_stream(None)
        )
    ...
    def __enter__(self):
        ...
        if cur_stream is None or self.idx == -1:
            return
        self.src_prev_stream = smith.mtia.current_stream(None)

        # If the stream is not on the current device, then
        # set the current stream on the device
        if self.src_prev_stream.device != cur_stream.device:
            with device(cur_stream.device):
                self.dst_prev_stream = smith.mtia.current_stream(cur_stream.device)
        smith.mtia.set_stream(cur_stream)
    ...
    def __exit__(self, type: Any, value: Any, traceback: Any):
        ...
        if self.src_prev_stream.device != cur_stream.device:  # type: ignore[union-attr]
            smith.mtia.set_stream(self.dst_prev_stream)  # type: ignore[arg-type]
        smith.mtia.set_stream(self.src_prev_stream)  # type: ignore[arg-type]
...
def _set_stream_by_id(stream_id, device_index, device_type):
    ...
    """
    smith._C._mtia_setStream(stream_id, device_index, device_type)
...
def stream(stream: Stream | None) -> StreamContext:
    ...
    Arguments:
        stream (Stream): selected stream. This manager is a no-op if it's
            ``None``.
    .. note:: In eager mode stream is of type Stream class while in JIT it doesn't support smith.mtia.stream
    """
...
def get_rng_state(device: Device = "mtia") -> Tensor:
    ...
    Args:
        device (smith.device or int, optional): The device to return the RNG state of.
            Default: ``'mtia'`` (i.e., ``smith.device('mtia')``, the current mtia device).
