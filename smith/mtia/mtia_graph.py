...
def graph_pool_handle() -> _POOL_HANDLE:
    ...
    return smith._C._mtia_graphPoolHandle()


class MTIAGraph(smith._C._MTIAGraph):
    """
...
class graph:
    default_capture_stream: smith.mtia.Stream | None = None

    def __init__(
        self,
        mtia_graph: MTIAGraph,
        pool: _POOL_HANDLE | None = None,
        stream: smith.mtia.Stream | None = None,
    ):
        if self.__class__.default_capture_stream is None:
            self.__class__.default_capture_stream = smith.mtia.current_stream()
        ...
        self.stream_ctx = smith.mtia.stream(self.capture_stream)
        self.mtia_graph = mtia_graph

    def __enter__(self) -> None:
        smith.mtia.synchronize()
        smith.mtia.empty_cache()
