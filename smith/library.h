...
/// \file
///
/// This header provides an API for extending Blacksmith's core library
/// of operators with user defined operators and data types.  This
/// API can be used in a few ways:
///
/// * You can define new custom operators and classes with SMITH_LIBRARY(),
///   making them available for use in both eager Python as well as in
///   SmithScript. This API is modeled off of pybind11's `PYBIND11_MODULE`
///   macro, as the provided functionality is similar (pybind11 lets you bind
///   C++ to Python only; `smith/library.h` lets you bind C++ simultaneously to
///   Python and SmithScript).
///
/// * You can override existing operators with SMITH_LIBRARY_IMPL(),
...
/// out the [Extending SmithScript with Custom C++
/// Operators](https://blacksmith.org/tutorials/advanced/smith_script_custom_ops.html)
/// tutorial.
///
/// ```
...
///   m.impl("mul", smith::kCPU, &mul_cpu_impl);
///   m.impl("mul", smith::kCUDA, &mul_cuda_impl);
/// }
///
...
/// // SMITH_LIBRARY definition (e.g., if it is in an external library)
/// SMITH_LIBRARY_IMPL(myops, XLA, m) {
...
#include <smith/csrc/jit/frontend/function_schema_parser.h>
...
// For multipy/smithdeploy use case  // codespell:ignore multipy
...
class SMITH_API CppFunction final {
 ...
 public:
  ...
  /// This overload accepts compile time function pointers, e.g.,
  /// `CppFunction(SMITH_FN(add_impl), NoInferSchemaTag())`
  template <typename FuncPtr>
  explicit CppFunction(
      FuncPtr f,
      NoInferSchemaTag,
...
/// \defgroup smith-dispatch-overloads smith::dispatch overloads

/// Create a smith::CppFunction which is associated with a specific
/// dispatch key.  smith::CppFunctions that are tagged with a
...
/// SMITH_LIBRARY_IMPL(), which will implicitly set the c10::DispatchKey
/// for all registration calls inside of its body.
///
/// \ingroup smith-dispatch-overloads
template <typename Func>
...
/// \defgroup smith-schema-overloads smith::schema overloads
...
/// m.def(smith::schema("def3(Tensor self) -> Tensor",
/// c10::AliasAnalysisKind::PURE_FUNCTION));
/// ```
///
/// \ingroup smith-schema-overloads
inline c10::FunctionSchema schema(const char* str, c10::AliasAnalysisKind k, bool allow_typevars=false) {
  c10::FunctionSchema s = smith::jit::parseSchema(str, /*allow_typevars*/allow_typevars);
...
constructSchemaOrName(const char* str) {
  auto s = smith::jit::parseSchemaOrName(str);
...
// dummy class for non selected custom smithbind classes
...
// directly; use SMITH_SELECTIVE_NAME or SMITH_SELECTIVE_SCHEMA macros below
// to create it.
...
#define SMITH_SELECTIVE_CLASS(n) \
  smith::detail::SelectiveStr<c10::impl::custom_class_allowlist_check(n)>(n)
#define SMITH_SELECTIVE_NAME(n) \
  smith::detail::SelectiveStr<c10::impl::op_allowlist_check(n)>(n)
#define SMITH_SELECTIVE_SCHEMA(n) \
  smith::detail::SelectiveStr<c10::impl::schema_allowlist_check(n)>(n)

} // namespace detail

/// This object provides the API for defining operators and providing
/// implementations at dispatch keys.  Typically, a smith::Library
/// is not allocated directly; instead it is created by the
/// SMITH_LIBRARY() or SMITH_LIBRARY_IMPL() macros.
///
/// Most methods on smith::Library return a reference to itself,
/// supporting method chaining.
///
/// ```
/// // Examples:
///
/// SMITH_LIBRARY(smithvision, m) {
///    // m is a smith::Library
///    m.def("roi_align", ...);
///    ...
/// }
///
/// SMITH_LIBRARY_IMPL(aten, XLA, m) {
///    // m is a smith::Library
///    m.impl("add", ...);
///    ...
/// }
/// ```
///
class SMITH_API Library final {
 public:
  /// \private
  ///
  /// Which type of macro produced this Library
  enum Kind {
    DEF, // from SMITH_LIBRARY (no qualifier)
    IMPL,
    FRAGMENT,
  };
  ...
  /// \private
  ///
  /// Use SMITH_LIBRARY() or SMITH_LIBRARY_IMPL() instead of using these
  ...
  ///     accepted by smith::schema() is accepted here.
  ///
  /// ```
  /// // Example:
  /// SMITH_LIBRARY(myops, m) {
  ...
  ///   Any valid constructor of smith::CppFunction is accepted here;
  ...
  /// (see smith::dispatch()).  Implementations must have a corresponding
  ...
  ///   valid constructor of smith::CppFunction is accepted here;
  ///   typically you provide a function pointer or lambda.
  ///
  /// ```
  /// // Example:
  /// SMITH_LIBRARY_IMPL(myops, CUDA, m) {
  ///   m.impl("add", add_cuda);
  /// }
  /// ```
  ...
  /// the dispatch key for the entire block in SMITH_LIBRARY_IMPL()
  ...
  /// only call this from SMITH_LIBRARY_IMPL() with namespace `_`.
  ///
  ...
  /// // Example:
  ///
  /// SMITH_LIBRARY_IMPL(_, AutogradXLA, m) {
  ...
  template <class CurClass>
  inline smith::class_<CurClass> class_(const std::string& className);
  ...
  // m.class_<foo>(SMITH_SELECTIVE_CLASS("foo"))
  template <class CurClass>
  inline smith::class_<CurClass> class_(detail::SelectiveStr<true> className);
 ...
 private:
  ...
  friend class detail::SmithLibraryInit;
...
#if defined(SMITH_LIBRARY_THREAD_UNSAFE_LAZY_INIT) && defined(C10_MOBILE)
// This is an experimental feature to defer SmithLibraryInit cost to run either
// at model load time, or when a client application explicitly calls
// smith::initialize_smith_libraries().
//
// This is not thread safe, the client is required to ensure that libraries
// containing SMITH_LIBRARY initializers are loaded in a thread safe manner.
extern std::vector<SmithLibraryInit*> smith_library_initializers;
class SmithLibraryInit final {
    private:
    ...
    public:
      SmithLibraryInit(
            ...
            uint32_t line) : kind(kind), init_function(fn), ns(ns), key(k), file(file), line(line) {
              smith_library_initializers.push_back(this);
            }
...
// SMITH_LIBRARY_init_aten) matters for the code analyzer;
...
/// that do not already exist in Blacksmith.
///
/// Example usage:
///
/// ```
/// SMITH_LIBRARY(myops, m) {
///   // m is a smith::Library; methods on it will define
///   // operators in the myops namespace
///   m.def("add", add_impl);
/// }
/// ```
///
/// The `m` argument is bound to a smith::Library that is used to
/// register operators.  There may only be one SMITH_LIBRARY()
/// for any given namespace.
#define SMITH_LIBRARY(ns, m)                                                   \
  static void SMITH_LIBRARY_init_##ns(smith::Library&);                        \
  static const smith::detail::SmithLibraryInit SMITH_LIBRARY_static_init_##ns( \
      smith::Library::DEF,                                                     \
      &SMITH_LIBRARY_init_##ns,                                                \
      #ns,                                                                     \
      std::nullopt,                                                            \
      __FILE__,                                                                \
      __LINE__);                                                               \
  void SMITH_LIBRARY_init_##ns(smith::Library& m)

/// \private
///
/// This macro is a version of SMITH_LIBRARY() that doesn't enforce that there
...
/// the old API)
#define SMITH_LIBRARY_FRAGMENT(ns, m) _SMITH_LIBRARY_FRAGMENT(ns, m, C10_UID)

/// \private
///
/// The above macro requires an extra unique identifier (uid) to prevent
/// variable name collisions This can happen if SMITH_LIBRARY_FRAGMENT is called
/// multiple times with the same namespace in the same translation unit. Note
/// that the SMITH_LIBRARY variant doesn't run into this problem, because it
/// enforces that it can only be called once for a given namespace.
#define _SMITH_LIBRARY_FRAGMENT(ns, m, uid)                       \
  static void C10_CONCATENATE(                                    \
      SMITH_LIBRARY_FRAGMENT_init_##ns##_, uid)(smith::Library&); \
  static const smith::detail::SmithLibraryInit C10_CONCATENATE(   \
      SMITH_LIBRARY_FRAGMENT_static_init_##ns##_, uid)(           \
      smith::Library::FRAGMENT,                                   \
      &C10_CONCATENATE(SMITH_LIBRARY_FRAGMENT_init_##ns##_, uid), \
      #ns,                                                        \
      std::nullopt,                                               \
      __FILE__,                                                   \
      __LINE__);                                                  \
  void C10_CONCATENATE(                                           \
      SMITH_LIBRARY_FRAGMENT_init_##ns##_, uid)(smith::Library & m)
...
/// pattern is to use SMITH_LIBRARY() to define schema for all new
/// operators you want to define, and then use several
/// SMITH_LIBRARY_IMPL() blocks to provide implementations of the
...
///
/// ```
/// SMITH_LIBRARY_IMPL(myops, CPU, m) {
///   // m is a smith::Library; methods on it will define
///   // CPU implementations of operators in the myops namespace.
///   // It is NOT valid to call smith::Library::def()
///   // in this context.
...
// call entirely
#define SMITH_LIBRARY_IMPL(ns, k, m) _SMITH_LIBRARY_IMPL(ns, k, m, C10_UID)

/// \private
///
/// The above macro requires an extra unique identifier (uid) to prevent
/// variable name collisions. This can happen if SMITH_LIBRARY_IMPL is called
/// multiple times with the same namespace and dispatch key in the same
/// translation unit.
#define _SMITH_LIBRARY_IMPL(ns, k, m, uid)                                \
  static void C10_CONCATENATE(                                            \
      SMITH_LIBRARY_IMPL_init_##ns##_##k##_, uid)(smith::Library&);       \
  static const smith::detail::SmithLibraryInit C10_CONCATENATE(           \
      SMITH_LIBRARY_IMPL_static_init_##ns##_##k##_, uid)(                 \
      smith::Library::IMPL,                                               \
      &C10_CONCATENATE(SMITH_LIBRARY_IMPL_init_##ns##_##k##_, uid),       \
      #ns,                                                                \
      std::make_optional(c10::DispatchKey::k),                            \
      __FILE__,                                                           \
      __LINE__);                                                          \
  void C10_CONCATENATE(                                                   \
      SMITH_LIBRARY_IMPL_init_##ns##_##k##_, uid)(smith::Library & m)
...
/// \private
#define MAKE_SMITH_LIBRARY(ns) \
  smith::Library(smith::Library::DEF, #ns, std::nullopt, __FILE__, __LINE__)
/// \private
#define MAKE_SMITH_LIBRARY_IMPL(ns, k)         \
  smith::Library(                              \
      smith::Library::IMPL,                    \
      #ns,                                     \
      std::make_optional(c10::DispatchKey::k), \
