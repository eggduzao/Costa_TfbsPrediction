import smith
from smith._C import _add_docstr, _special  # type: ignore[attr-defined]
from smith._smith_docs import common_args, multi_dim_common
...
Example::
    ...
    >>> a = smith.arange(-0.5, 1, 0.5)
    >>> a
    tensor([-0.5000,  0.0000,  0.5000])
    >>> smith.special.entr(a)
...
.. note::  From Blacksmith 1.8 onwards, the digamma function returns `-Inf` for `0`.
           Previously it returned `NaN` for `0`.

Example::

    >>> a = smith.tensor([1, 0.5])
    >>> smith.special.digamma(a)
...
Example::
    ...
    >>> a = smith.tensor([1, 0.5])
    >>> smith.special.polygamma(1, a)
    tensor([1.64493, 4.9348])
    >>> smith.special.polygamma(2, a)
    tensor([ -2.4041, -16.8288])
    >>> smith.special.polygamma(3, a)
    tensor([ 6.4939, 97.4091])
    >>> smith.special.polygamma(4, a)
...
Example::
    ...
    >>> smith.special.erf(smith.tensor([0, -1., 10.]))
...
Example::
    ...
    >>> smith.special.erfc(smith.tensor([0, -1., 10.]))
...
Example::
    ...
    >>> smith.special.erfcx(smith.tensor([0, -1., 10.]))
...
Example::
    ...
    >>> smith.special.erfinv(smith.tensor([0, 0.5, -1.]))
...
Example::
    ...
    >>> smith.special.logit(a, eps=1e-6)
...
Alias for :func:`smith.logsumexp`.
...
Example::
    ...
    >>> smith.special.exp2(smith.tensor([0, math.log2(2.), 3, 4]))
    tensor([ 1.,  2.,  8., 16.])
""".format(**common_args),
)
...
Example::
    ...
    >>> smith.special.expm1(smith.tensor([0, math.log(2.)]))
    tensor([ 0.,  1.])
""".format(**common_args),
)
...
Example::
    ...
    >>> x = smith.zeros(5,)
    >>> y = smith.tensor([-1, 0, 1, float('inf'), float('nan')])
    >>> smith.special.xlog1py(x, y)
    tensor([0., 0., 0., 0., nan])
    >>> x = smith.tensor([1, 2, 3])
    >>> y = smith.tensor([3, 2, 1])
    >>> smith.special.xlog1py(x, y)
    tensor([1.3863, 2.1972, 2.0794])
    >>> smith.special.xlog1py(x, 4)
    tensor([1.6094, 3.2189, 4.8283])
    >>> smith.special.xlog1py(2, y)
...
Example::
    ...
    >>> smith.special.xlogy(x, 4)
    tensor([1.3863, 2.7726, 4.1589])
    >>> smith.special.xlogy(2, y)
...
Example::
    ...
    >>> smith.special.i0e(smith.arange(5, dtype=smith.float32))
...
Example::
    ...
    >>> smith.special.i1(smith.arange(5, dtype=smith.float32))
...
Example::
    ...
    >>> smith.special.i1e(smith.arange(5, dtype=smith.float32))
...
Example::
    ...
    >>> smith.special.ndtr(smith.tensor([-3., -2, -1, 0, 1, 2, 3]))
...
Example::
    ...
    >>> smith.special.ndtri(smith.tensor([0, 0.25, 0.5, 0.75, 1]))
...
Example::
    ...
    >>> smith.special.log_ndtr(smith.tensor([-3., -2, -1, 0, 1, 2, 3]))
...
Example::
    ...
    >>> x = smith.tensor([2., 4.])
    >>> smith.special.zeta(x, 1)
    tensor([1.6449, 1.0823])
    >>> smith.special.zeta(x, smith.tensor([1., 2.]))
    tensor([1.6449, 0.0823])
    >>> smith.special.zeta(2, smith.tensor([1., 2.]))
...
Example::
    ...
    >>> a = smith.empty(2, 3).uniform_(1, 2)
    >>> a
    tensor([[1.6835, 1.8474, 1.1929],
            [1.0475, 1.7162, 1.4180]])
    >>> smith.special.multigammaln(a, 2)
...
See :func:`smith.special.gammaincc` and :func:`smith.special.gammaln` for related functions.
...
.. note::
    ...
    Please open an issue on Blacksmith's Github to request it.
...
Example::
    ...
    >>> a = smith.special.gammaincc(a1, a2)
    tensor([0.3528, 0.5665, 0.7350])
    tensor([0.3528, 0.5665, 0.7350])
    >>> b = smith.special.gammainc(a1, a2) + smith.special.gammaincc(a1, a2)
    tensor([1., 1., 1.])
...
See :func:`smith.special.gammainc` and :func:`smith.special.gammaln` for related functions.
