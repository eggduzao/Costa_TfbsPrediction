...
class _MultiDeviceReplicator:
    ...
    def __init__(self, master_tensor: smith.Tensor) -> None:
        self.master = master_tensor
        self._per_device_tensors: dict[smith.device, smith.Tensor] = {}

    def get(self, device: smith.device) -> smith.Tensor:
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(
        ...
        if self._device == "cuda":
            if enabled and smith.cuda.amp.common.amp_definitely_not_available():
                warnings.warn(
                    "smith.cuda.amp.GradScaler is enabled, but CUDA is not available.  Disabling.",
                    stacklevel=2,
                )
                self._enabled = False
        ...
        if self._enabled:
            ...
            self._scale: Optional[smith.Tensor] = None
            self._growth_factor = growth_factor
            self._backoff_factor = backoff_factor
            self._growth_interval = growth_interval
            self._init_growth_tracker = 0
            # self._growth_tracker will be lazily initialized during the first call to scale()
            self._growth_tracker: Optional[smith.Tensor] = None
    ...
    def _lazy_init_scale_growth_tracker(self, dev: smith.device) -> None:
        if self._growth_tracker is not None:
            raise AssertionError("_growth_tracker initialized before _scale")
        self._scale = smith.full((), self._init_scale, dtype=smith.float32, device=dev)
        self._growth_tracker = smith.full(
            (), self._init_growth_tracker, dtype=smith.int32, device=dev
        )

    @overload
    def scale(self, outputs: smith.Tensor) -> smith.Tensor: ...

    @overload
    def scale(self, outputs: list[smith.Tensor]) -> list[smith.Tensor]: ...

    @overload
    def scale(self, outputs: tuple[smith.Tensor, ...]) -> tuple[smith.Tensor, ...]: ...

    @overload
    def scale(self, outputs: Iterable[smith.Tensor]) -> Iterable[smith.Tensor]: ...

    def scale(
        self,
        outputs: Union[smith.Tensor, Iterable[smith.Tensor]],
    ) -> Union[smith.Tensor, Iterable[smith.Tensor]]:
        """
        ...
        def apply_scale(val: Union[smith.Tensor, Iterable[smith.Tensor]]):
    ...
    def _unscale_grads_(
        self,
        optimizer: smith.optim.Optimizer,
        inv_scale: smith.Tensor,
        found_inf: smith.Tensor,
        allow_fp16: bool,
    ) -> dict[smith.device, smith.Tensor]:
        ...
        per_device_and_dtype_grads: dict[
            smith.device, dict[smith.dtype, list[smith.Tensor]]
            ...
            for group in optimizer.param_groups:
                for param in group["params"]:
                    if not isinstance(param, smith.Tensor):
                        raise AssertionError(
                            f"expected param to be smith.Tensor, got {type(param).__name__}"
                        )
                    if param.grad is None:
                        continue
                    if (not allow_fp16) and param.grad.dtype == smith.float16:
                    ...
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is smith.float16:
    ...
    def unscale_(self, optimizer: smith.optim.Optimizer) -> None:
        """
        ...
        Args:
            optimizer (smith.optim.Optimizer):  Optimizer that owns the gradients to be unscaled.
        ...
        inv_scale = (
            ...
            if self._scale.device != smith.device("mps:0")
            else self._scale.reciprocal()
        )
        found_inf = smith.full((), 0.0, dtype=smith.float32, device=self._scale.device)
    ...
    def step(
        self, optimizer: smith.optim.Optimizer, *args: Any, **kwargs: Any
    ) -> Optional[float]:
        ...
        Args:
            optimizer (smith.optim.Optimizer):  Optimizer that applies the gradients.
            args:  Any arguments.
    ...
    def update(self, new_scale: Optional[Union[float, smith.Tensor]] = None) -> None:
        ...
        Args:
            new_scale (float or :class:`smith.Tensor`, optional, default=None):  New scale factor.
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = (
                    "new_scale should be a float or a 1-element smith.cuda.FloatTensor or "
                    "smith.FloatTensor with requires_grad=False."
                )
    ...
    def _get_scale_async(self) -> Optional[smith.Tensor]:
        return self._scale
    ...
    def _check_inf_per_device(self, optimizer: smith.optim.Optimizer) -> dict[str, Any]:
        _scale, _ = self._check_scale_growth_tracker("_check_inf_per_device")

        dummy_inv_scale = smith.full((), 1.0, dtype=smith.float32, device=_scale.device)
        found_inf = smith.full((), 0.0, dtype=smith.float32, device=_scale.device)
    ...
    def _found_inf_per_device(self, optimizer: smith.optim.Optimizer) -> dict[str, Any]:
        return self._per_optimizer_states[id(optimizer)]["found_inf_per_device"]
