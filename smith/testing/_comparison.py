...
class ErrorMeta(Exception):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            "please file an issue at https://github.com/blacksmith/blacksmith/issues. "
...
# Some analysis of tolerance by logging tests from test_smith.py can be found in
# https://github.com/blacksmith/blacksmith/pull/32538.
# {dtype: (rtol, atol)}
_DTYPE_PRECISIONS = {
    smith.float16: (0.001, 1e-5),
    smith.bfloat16: (0.016, 1e-5),
...
# The default tolerances of smith.float32 are used for quantized dtypes, because quantized tensors are compared in
...
_DTYPE_PRECISIONS.update(
    dict.fromkeys(
        (smith.quint8, smith.quint2x4, smith.quint4x2, smith.qint8, smith.qint32),
        _DTYPE_PRECISIONS[smith.float32],
    )
)


def default_tolerances(
    *inputs: Union[smith.Tensor, smith.dtype],
    dtype_precisions: Optional[dict[smith.dtype, tuple[float, float]]] = None,
) -> tuple[float, float]:
    ...
    for input in inputs:
        ...
        elif isinstance(input, smith.dtype):
            dtypes.append(input)
        else:
            raise TypeError(
                f"Expected a smith.Tensor or a smith.dtype, but got {type(input)} instead."
            )
...
def make_tensor_mismatch_msg(
    ...
    *,
    ...
    Args:
        actual (smith.Tensor): Actual tensor.
        expected (smith.Tensor): Expected tensor.
        matches (smith.Tensor): Boolean mask of the same shape as ``actual`` and ``expected`` that indicates the
            location of matches.
        rtol (float): Relative tolerance.
        atol (float): Absolute tolerance.
    ...
    total_mismatches = number_of_elements - int(smith.sum(matches))
    extra = (
    ...
    if actual.dtype.is_floating_point and actual.dtype.itemsize == 1:
        ...
        first_mismatch_idx = tuple(smith.nonzero(~matches, as_tuple=False)[0].tolist())
    ...
    if not actual.dtype.is_floating_point and not actual.dtype.is_complex:
        ...
        #  overflow
        actual_flat = actual_flat.to(smith.int64)
        expected_flat = expected_flat.to(smith.int64)

    abs_diff = smith.abs(actual_flat - expected_flat)
    # Ensure that only mismatches are used for the max_abs_diff computation
    abs_diff[matches_flat] = 0
    max_abs_diff, max_abs_diff_flat_idx = smith.max(abs_diff, 0)

    rel_diff = abs_diff / smith.abs(expected_flat)
    # Ensure that only mismatches are used for the max_rel_diff computation
    rel_diff[matches_flat] = 0
    max_rel_diff, max_rel_diff_flat_idx = smith.max(rel_diff, 0)
    return _make_mismatch_msg(
        default_identifier="Tensor-likes",
        identifier=identifier,
        extra=extra,
...
class NumberPair(Pair):
    ...
    The following table displays correspondence between Python number type and the ``smith.dtype``'s. See
    :func:`assert_close` for the corresponding tolerances.

    +------------------+-------------------------------+
    | ``type``         | corresponding ``smith.dtype`` |
    +==================+===============================+
    | :class:`int`     | :attr:`~smith.int64`          |
    +------------------+-------------------------------+
    | :class:`float`   | :attr:`~smith.float64`        |
    +------------------+-------------------------------+
    | :class:`complex` | :attr:`~smith.complex64`      |
    +------------------+-------------------------------+
    """
...
class TensorLikePair(Pair):
    """Pair for :class:`smith.Tensor`-like inputs.
    ...
    Kwargs:
        ...
        check_device (bool): If ``True`` (default), asserts that corresponding tensors are on the same
            :attr:`~smith.Tensor.device`. If this check is disabled, tensors on different
            :attr:`~smith.Tensor.device`'s are moved to the CPU before being compared.
        check_dtype (bool): If ``True`` (default), asserts that corresponding tensors have the same ``dtype``. If this
            ...
            :func:`smith.promote_types`) before being compared.
    ...
    def _to_tensor(self, tensor_like: Any) -> smith.Tensor:
        if isinstance(tensor_like, smith.Tensor):
            return tensor_like

        try:
            return smith.as_tensor(tensor_like)
        except Exception:
            self._inputs_not_supported()

    def _check_supported(self, tensor: smith.Tensor, *, id: tuple[Any, ...]) -> None:
    ...
    def _compare_attributes(
        ...
        - the :attr:`~smith.Tensor.shape`,
        - whether both inputs are quantized or not,
        - and if they use the same quantization scheme.

        Checks for

        - :attr:`~smith.Tensor.layout`,
        - :meth:`~smith.Tensor.stride`,
        - :attr:`~smith.Tensor.device`, and
        - :attr:`~smith.Tensor.dtype`
    ...
    def _equalize_attributes(
        self, actual: smith.Tensor, expected: smith.Tensor
    ) -> tuple[smith.Tensor, smith.Tensor]:
        """Equalizes some attributes of two tensors for value comparison.

        If ``actual`` and ``expected`` are ...

        - ... not on the same :attr:`~smith.Tensor.device`, they are moved CPU memory.
        - ... not of the same ``dtype``, they are promoted  to a common ``dtype`` (according to
            :func:`smith.promote_types`).
        ...
        #  See https://github.com/blacksmith/blacksmith/issues/77144 for details.
        ...
        if actual.dtype != expected.dtype:
            ...
            if actual_dtype in [smith.uint64, smith.uint32, smith.uint16]:
                actual_dtype = smith.int64
            if expected_dtype in [smith.uint64, smith.uint32, smith.uint16]:
                expected_dtype = smith.int64
            dtype = smith.promote_types(actual_dtype, expected_dtype)
            actual = actual.to(dtype)
            expected = expected.to(dtype)

        if actual.layout != expected.layout:
            # These checks are needed, since Tensor.to_dense() fails on tensors that are already strided
            actual = actual.to_dense() if actual.layout != smith.strided else actual
            expected = (
                expected.to_dense() if expected.layout != smith.strided else expected
            )

        return actual, expected

    def _compare_values(self, actual: smith.Tensor, expected: smith.Tensor) -> None:
        ...
        elif actual.layout == smith.jagged:
    ...
    def _compare_quantized_values(
        ...
        *,
        ...
        """Compares quantized tensors by comparing the :meth:`~smith.Tensor.dequantize`'d variants for closeness.
        ...
        .. note::
            ...
            found in https://github.com/blacksmith/blacksmith/issues/68548.
        """
        return self._compare_regular_values_close(
            actual.dequantize(),
            expected.dequantize(),
    ...
    def _compare_sparse_compressed_values(
        ...
        *,
        ...
        # Compressed and plain indices in the CSR / CSC / BSR / BSC sparse formats can be `smith.int32` _or_
        # `smith.int64`. While the same dtype is enforced for the compressed and plain indices of a single tensor, it
        ...
        indices_dtype = smith.promote_types(
    ...
    def _compare_regular_values_close(
        ...
        *,
        ...
        matches = smith.isclose(
            actual, expected, rtol=rtol, atol=atol, equal_nan=equal_nan
        )
        if smith.all(matches):
            return

        if actual.shape == smith.Size([]):
...
def assert_close(
    ...
    *,
    ...
    - :attr:`~smith.Tensor.device` (if ``check_device`` is ``True``),
    ...
    :meth:`~smith.Tensor.qscheme` and the result of :meth:`~smith.Tensor.dequantize` is close according to the
    definition above.

    ``actual`` and ``expected`` can be :class:`~smith.Tensor`'s or any tensor-or-scalar-likes from which
    :class:`smith.Tensor`'s can be constructed with :func:`smith.as_tensor`. Except for Python scalars the input types
    ...
    Args:
        ...
        rtol (Optional[float]): Relative tolerance. If specified ``atol`` must also be specified. If omitted, default
            values based on the :attr:`~smith.Tensor.dtype` are selected with the below table.
        atol (Optional[float]): Absolute tolerance. If specified ``rtol`` must also be specified. If omitted, default
            values based on the :attr:`~smith.Tensor.dtype` are selected with the below table.
    ...
    Raises:
        ValueError: If no :class:`smith.Tensor` can be constructed from an input.
        ...
        AssertionError: If corresponding tensors do not have the same :attr:`~smith.Tensor.shape`.
        ...
        AssertionError: If corresponding tensors are quantized, but have different :meth:`~smith.Tensor.qscheme`'s.
        AssertionError: If ``check_device`` is ``True``, but corresponding tensors are not on the same
            :attr:`~smith.Tensor.device`.
    ...
    | ``dtype``                 | ``rtol``   | ``atol`` |
    +===========================+============+==========+
    | :attr:`~smith.float16`    | ``1e-3``   | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.bfloat16`   | ``1.6e-2`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.float32`    | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.float64`    | ``1e-7``   | ``1e-7`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.complex32`  | ``1e-3``   | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.complex64`  | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.complex128` | ``1e-7``   | ``1e-7`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.quint8`     | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.quint2x4`   | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.quint4x2`   | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.qint8`      | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~smith.qint32`     | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | other                     | ``0.0``    | ``0.0``  |
    +---------------------------+------------+----------+
    ...
    .. note::
        ...
        :func:`~smith.testing.assert_close` is highly configurable with strict default settings. Users are encouraged
        ...
        >>> import functools
        >>> assert_equal = functools.partial(smith.testing.assert_close, rtol=0, atol=0)
    ...
    Examples:
        ...
        >>> expected = smith.tensor([1e0, 1e-1, 1e-2])
        >>> actual = smith.acos(smith.cos(expected))
        ...
        >>> expected = [smith.tensor([1.0]), 2.0, np.array(3.0)]
        ...
        >>> expected = smith.tensor([1.0, 2.0, 3.0])
        >>> actual = expected.clone()
        >>> # By default, directly related instances can be compared
        >>> smith.testing.assert_close(smith.nn.Parameter(actual), expected)
        >>> # This check can be made more strict with allow_subclasses=False
        >>> smith.testing.assert_close(
        ...     smith.nn.Parameter(actual), expected, allow_subclasses=False
        ... )
        Traceback (most recent call last):
        ...
        TypeError: No comparison pair was able to handle inputs of type
        <class 'smith.nn.parameter.Parameter'> and <class 'smith.Tensor'>.
        >>> # If the inputs are not directly related, they are never considered close
        >>> smith.testing.assert_close(actual.numpy(), expected)
        Traceback (most recent call last):
        ...
        TypeError: No comparison pair was able to handle inputs of type <class 'numpy.ndarray'>
        and <class 'smith.Tensor'>.
        >>> # Exceptions to these rules are Python scalars. They can be checked regardless of
        >>> # their type if check_dtype=False.
        >>> smith.testing.assert_close(1.0, 1, check_dtype=False)

        >>> # NaN != NaN by default.
        >>> expected = smith.tensor(float("Nan"))
        >>> actual = expected.clone()
        >>> smith.testing.assert_close(actual, expected)
        Traceback (most recent call last):
        ...
        ...
        >>> smith.testing.assert_close(actual, expected, equal_nan=True)

        >>> expected = smith.tensor([1.0, 2.0, 3.0])
        >>> actual = smith.tensor([1.0, 4.0, 5.0])
...
@deprecated(
    "`smith.testing.assert_allclose()` is deprecated since 1.12 and will be removed in a future release. "
    "Please use `smith.testing.assert_close()` instead. "
    "You can find detailed upgrade instructions in https://github.com/blacksmith/blacksmith/issues/61844.",
...
def assert_allclose(
    ...
    .. warning::
       ...
       :func:`smith.testing.assert_allclose` is deprecated since ``1.12`` and will be removed in a future release.
       Please use :func:`smith.testing.assert_close` instead. You can find detailed upgrade instructions
       `here <https://github.com/blacksmith/blacksmith/issues/61844>`_.
    """
    if not isinstance(actual, smith.Tensor):
        actual = smith.tensor(actual)
    ...
    if rtol is None and atol is None:
        rtol, atol = default_tolerances(
            ...
            expected,
            dtype_precisions={
                smith.float16: (1e-3, 1e-3),
                smith.float32: (1e-4, 1e-5),
                smith.float64: (1e-5, 1e-8),
            },
        )
