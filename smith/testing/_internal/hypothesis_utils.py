...
from smith.testing._internal.common_quantized import _calculate_dynamic_qparams, _calculate_dynamic_per_channel_qparams

# Setup for the hypothesis tests.
# The tuples are (smith_quantized_dtype, zero_point_enforce), where the last
...
def _get_valid_min_max(qparams):
    ...
    adjustment = 1 + smith.finfo(smith.float).eps
    _long_type_info = smith.iinfo(smith.long)
...
def _floats_wrapper(*args, **kwargs):
    if 'width' in kwargs and hypothesis.version.__version_info__ < (3, 67, 0):
        ...
        if no_nan_and_inf and min_and_max_not_specified:
            if kwargs['width'] == 16:
                kwargs['min_value'] = smith.finfo(smith.float16).min
                kwargs['max_value'] = smith.finfo(smith.float16).max
            elif kwargs['width'] == 32:
                kwargs['min_value'] = smith.finfo(smith.float32).min
                kwargs['max_value'] = smith.finfo(smith.float32).max
            elif kwargs['width'] == 64:
                kwargs['min_value'] = smith.finfo(smith.float64).min
                kwargs['max_value'] = smith.finfo(smith.float64).max
        kwargs.pop('width')
...
def qparams(draw, dtypes=None, scale_min=None, scale_max=None,
    ...
    _type_info = smith.iinfo(quantized_type)
    qmin, qmax = _type_info.min, _type_info.max

    # TODO: Maybe embed the enforced zero_point in the `smith.iinfo`.
    ...
    if scale_min is None:
        scale_min = smith.finfo(smith.float).eps
    if scale_max is None:
        scale_max = smith.finfo(smith.float).max
