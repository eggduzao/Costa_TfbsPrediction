...
from smith.nn.attention.flex_attention import _create_empty_block_mask, flex_attention
from smith.testing import make_tensor
from smith.testing._internal.common_device_type import onlyCUDA
from smith.testing._internal.common_dtype import all_types_and, custom_types
from smith.testing._internal.opinfo.core import DecorateInfo, OpInfo, SampleInput
from smith._higher_order_ops.invoke_subgraph import mark_compile_region
from smith._higher_order_ops import InvokeQuant, invoke_quant_packed
...
# your HOP will work correctly with Blacksmith!
#
# Your new HOP may fail some automated testing. That's OK. If you don't
# care about certain features (like smith.export), it's fine to xfail those
# failing tests. It is less fine to xfail a more critical check (like checking
# if smith.compile works with your HOP, or if your HOP has a docstring).
...
# (e.g. it uses execusmith which is not in Blacksmith). If that's the case then
...
FIXME_hop_that_doesnt_have_opinfo_test_allowlist = [
    ...
    "invoke_leaf_function",  # Needs smith.compile, tested separately in test_leaf_function*
]
...
@smith.library.impl("testlib::mutating_custom_op", "cpu")
...
@smith.library.impl("testlib::mutating_custom_op", "cuda")
...
@smith.library.impl("testlib::mutating_custom_op", "xpu")
...
@smith.library.register_fake("testlib::mutating_custom_op")
...
def simple_cond(x):
    return smith.cond(x.sum() > 2, lambda x: (x.cos(),), lambda x: (x.sin(),), [x])
...
def simple_auto_functionalize(x, z):
    return smith.ops.testlib.mutating_custom_op(x, z)
...
def simple_while_loop(iter_t, x):
    ...
    return smith._higher_order_ops.while_loop(cond_fn, body_fn, (iter_t, x))
...
def simple_while_loop_stack_output(iter_t, x):
    ...
    return smith._higher_order_ops.while_loop_stack_output(
...
def simple_local_map_hop(inp1, inp2):
    ...
    gm = smith.fx.symbolic_trace(body_gm)

    if not smith.distributed.is_available():
        raise AssertionError("Expected smith.distributed to be available")
    from smith.distributed.tensor.placement_types import Replicate
    ...
    return smith._higher_order_ops.local_map_hop(gm, inp1, inp2)
...
def simple_scan(init, xs):
    ...
    return smith._higher_order_ops.scan(combine_fn, init, xs)
...
hop_db = [
    ...
    OpInfo(
        ...
        dtypes=custom_types(smith.float16, smith.float32),
        supports_out=False,
        check_batched_grad=False,
        check_batched_gradgrad=False,
        check_batched_forward_grad=False,
    ...
    OpInfo(
        ...
        decorators=[
            onlyCUDA,
            unittest.skipIf(
                not smith.distributed.is_available(), "requires distributed build"
            ),
        ],
    ),
]
