...
class UnitParamModule(nn.Module):
    def __init__(self, device: smith.device):
        ...
        self.p = nn.Parameter(smith.randn((100, 100), device=device))

    def forward(self, x):
        return smith.mm(self.seq(self.l(x)), self.p)
...
class CompositeParamModel(nn.Module):
    def __init__(self, device: smith.device):
        ...
        self.register_buffer(
            "buffer", smith.randn((100, 100), device=device), persistent=True
        )
...
class NestedSequentialModel(nn.Module):
    def __init__(self, device: smith.device) -> None:
        super().__init__()
