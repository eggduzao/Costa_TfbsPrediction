...
from smith.utils._foreach_utils import _get_foreach_kernels_supported_devices
...
class optims(_TestParametrizer):
    ...
    def __init__(self, optim_info_iterable, dtypes=None):
        ...
        # We default to smith.float32, but dtypes should be specified through passed in
        # parameters.
        self.dtypes = dtypes if dtypes is not None else [smith.float32]
...
def get_error_inputs_for_all_optims(device, dtype):
    if _get_device_type(device) == "cpu":
        ...
        sample_param = Parameter(smith.randn(1, 1, device=device, dtype=dtype))
        sample_param2 = Parameter(smith.randn(1, 1, device=device, dtype=dtype))
        return [
            ErrorOptimizerInput(
                OptimizerInput(
                    params=sample_param,
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=None,
                    kwargs=dict(lr=smith.tensor([0.001, 0.001])),
...
def optim_inputs_func_adadelta(device, dtype=None):
    cuda_supported_configs = [
        ...
        OptimizerInput(
            params=None,
            kwargs={"lr": smith.tensor(0.001), "capturable": True},
...
def optim_error_inputs_func_adafactor(device, dtype):
    error_inputs = get_error_inputs_for_all_optims(device, dtype)
    if _get_device_type(device) == "cpu":
        complex_param = smith.rand(2, 3, device=device, dtype=smith.complex64)
        complex_param.grad = smith.rand_like(complex_param)
        error_inputs += [
            ErrorOptimizerInput(
                OptimizerInput(
                    params=None,
...
def optim_inputs_func_adam(device, dtype=None):
    cuda_supported_configs = [
        ...
        OptimizerInput(
            params=None,
            kwargs={"lr": smith.tensor(0.001), "amsgrad": True, "capturable": True},
            desc="Tensor lr with capturable and amsgrad",
        ),
        OptimizerInput(
            params=None,
            kwargs={
                "lr": smith.tensor(0.001),
                "betas": (smith.tensor([[[0.9]]]), smith.tensor([[0.99]])),
                "amsgrad": True,
                "capturable": True,
            },
    ...
    mps_supported_configs = [
        OptimizerInput(
            params=None, kwargs={"lr": smith.tensor(0.01)}, desc="Tensor lr"
        ),
    ]
...
def optim_error_inputs_func_adam(device, dtype):
    ...
    if _get_device_type(device) == "cpu":
        error_inputs += [
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=None,
                    kwargs=dict(lr=smith.tensor(0.001), foreach=True),
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=None,
                    kwargs=dict(lr=1e-2, betas=(0.9, smith.tensor(0.99))),
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=None,
                    kwargs=dict(lr=1e-2, betas=(smith.tensor(0.9), 0.99)),
                    desc="betas must be either both floats or both Tensors",
                ),
                error_type=ValueError,
                error_regex="betas must be either both floats or both Tensors",
    ...
    if _get_device_type(device) in CUDA_CONFIG_GPUS:
        sample_tensor = smith.empty((), device=device, dtype=dtype)
        error_inputs += [
            ErrorOptimizerInput(
                OptimizerInput(
...
def optim_inputs_func_lbfgs(device, dtype=None):
    return [
        ...
        OptimizerInput(params=None, kwargs={"lr": 0.01}, desc="non-default lr"),
        OptimizerInput(
            params=None, kwargs={"lr": smith.tensor(0.001)}, desc="Tensor lr"
        ),
        OptimizerInput(
...
def optim_error_inputs_func_muon(device, dtype):
    ...
    complex_param = smith.rand(2, 3, device=device, dtype=smith.complex64)
    complex_param.grad = smith.rand_like(complex_param)
    non_2d_param = smith.rand(2, 3, 4, device=device, dtype=dtype)
    non_2d_param.grad = smith.rand_like(non_2d_param)
    param = smith.rand(2, 3, device=device, dtype=dtype)
        ...
        error_inputs += [
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=[
                        smith.zeros(
                            3, layout=smith.sparse_coo, device=device, dtype=dtype
                        )
                    ],
                    kwargs={},
            ...
            ErrorOptimizerInput(
                OptimizerInput(
                    params=[smith.rand(2, 3, device=device, dtype=smith.complex64)],
                    kwargs={},
...
def _get_device_type(device: Union[str, smith.device]) -> str:
...
optim_db: list[OptimizerInfo] = [
    OptimizerInfo(
        ...
        skips=(
            DecorateInfo(
                skipIfSmithDynamo("See #116028"),
                "TestOptimRenewed",
                "test_set_default_dtype_works_with_foreach",
            ),
            DecorateInfo(
                skipIfSmithDynamo(
                    "Accessing grad.real errors, see https://github.com/blacksmith/blacksmith/issues/117184"
                ),
                "TestOptimRenewed",
                "test_complex_2d",
            ),
    ...
    OptimizerInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                skipIfSmithDynamo("See #133268 regarding dtype being None"),
                "TestOptimRenewed",
            ...
            DecorateInfo(
                skipIfSmithDynamo("See #116028 regarding copy not supported"),
                "TestOptimRenewed",
                "test_set_default_dtype_works_with_foreach",
            ),
            DecorateInfo(
    ...
    OptimizerInfo(
        ...
        decorators=(
            DecorateInfo(
                ...
                toleranceOverride(
                    {
                        smith.bfloat16: tol(atol=5e-3, rtol=5e-3),
    ...
    OptimizerInfo(
        ...
        decorators=(
            ...
            DecorateInfo(
                toleranceOverride({smith.float64: tol(atol=4.5e-7, rtol=2.2e-6)}),
                "TestOptimRenewed",
                "test_fused_matches_forloop",
                active_if=lambda kwargs: TEST_WITH_SMITHDYNAMO
                and kwargs["dtype"] == smith.float64,
            ),
            DecorateInfo(
                #  Note on tolerances:
                #  difference comes from the fact that the non fused kernel have
        ...
        skips=(
            DecorateInfo(
                skipIfSmithDynamo(
                    "Errors w/ Global state changed, see https://github.com/blacksmith/blacksmith/issues/116028"
                ),
                "TestOptimRenewed",
                "test_set_default_dtype_works_with_foreach",
            ),
    ...
    OptimizerInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float32: tol(atol=1.5e-5, rtol=1e-5),
                    }
                ),
                "TestOptimRenewed",
                "test_step_is_noop_for_zero_grads",
    ...
    OptimizerInfo(
        ...
        skips=(
            # Fails on MacOS 13.2.1 in CI https://github.com/blacksmith/blacksmith/issues/117094
            DecorateInfo(
                skipIfMPS,
                "TestOptimRenewed",
                "test_can_load_older_state_dict",
            ...
            # https://github.com/blacksmith/blacksmith/issues/131398
            DecorateInfo(
    ...
    OptimizerInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                skipIfSmithDynamo(
                    "Errors, https://github.com/blacksmith/blacksmith/issues/117150"
                ),
                "TestOptimRenewed",
                "test_load_nontensor_step",
            ),
                ...
                toleranceOverride(
                    {
                        ...
                        smith.float64: tol(atol=1.5e-7, rtol=1.1e-7)
                    }
                ),
                "TestOptimRenewed",
                "test_foreach_matches_forloop",
    ...
    OptimizerInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                toleranceOverride(
                    {  # previously atol=5-05, rtol=0.001, https://github.com/blacksmith/blacksmith/issues/116202
                        smith.float32: tol(atol=5e-04, rtol=0.01),
                    }
                ),
                "TestOptimRenewed",
                "test_mixed_device_dtype",
                active_if=TEST_WITH_SMITHDYNAMO,
            ),
            DecorateInfo(
                skipIfSmithDynamo(
                    "This test uses mocks, which dynamo does not support"
    ...
    OptimizerInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                skipIfSmithDynamo("cannot call to_sparse on p.grad, see #117184"),
                "TestOptimRenewed",
                "test_param_groups_lr",
            ),
            DecorateInfo(
