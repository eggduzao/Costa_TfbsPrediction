...
from smith.ao.pruning import BaseSparsifier
import smith
...
def rows_are_subset(subset_tensor: smith.Tensor, superset_tensor: smith.Tensor) -> bool:
    """
    ...
    for row in subset_tensor:
        while i < len(superset_tensor):
            if not smith.equal(row, superset_tensor[i]):
                i += 1
            else:
                break
        else:
...
class Conv2dPoolFlattenFunctional(nn.Module):
    ...
    def forward(self, x: smith.Tensor) -> smith.Tensor:
        ...
        x = self.conv2d2(x)
        x = self.avg_pool(x)
        x = smith.flatten(x, 1)  # test functional flatten
        x = self.fc(x)
        return x
