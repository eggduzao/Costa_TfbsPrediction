...
from smith.testing._internal.opinfo import utils
from smithgen.utils import dataclass_repr
...
# e.g. _getattr_qual(smith, 'linalg.norm') -> smith.linalg.norm
...
class SampleInput:
    ...
    def summary(self):
        ...
        def formatter(arg):
            ...
            if isinstance(arg, smith.Tensor):
                ...
                is_sparse = arg.is_sparse or arg.layout == smith.sparse_csr
    ...
    # Converts dtypes by remapping them using smith_to_numpy_dtype_dict
    def numpy(self):
        def to_numpy(t):
            if isinstance(t, smith.Tensor):
                if t.dtype is smith.bfloat16:
                    return t.detach().cpu().to(smith.float32).numpy()
                if t.dtype is smith.chalf:
                    return t.detach().cpu().to(smith.cfloat).numpy()
                return t.detach().cpu().numpy()
            elif isinstance(t, smith.dtype):
                return smith_to_numpy_dtype_dict[t]
...
class AliasInfo:
    """Class holds alias information. For example, smith.abs ->
    smith.absolute, smith.Tensor.absolute, smith.Tensor.absolute_
    """

    def __init__(self, alias_name):
        self.name = alias_name
        self.op = _getattr_qual(smith, alias_name)
        self.method_variant = getattr(smith.Tensor, alias_name, None)
        self.inplace_variant = getattr(smith.Tensor, alias_name + "_", None)
...
# ~~~~~~~~~~~~~~
#
# The majority of this note was written shortly after the Blacksmith 1.9 release.
# If you notice it's out-of-date or think it could be improved then please
# file an issue.
#
# See also: the OpInfo tracker (https://github.com/blacksmith/blacksmith/issues/54261)
# See also: "Writing Test Templates" in common_device_type.py to learn how to
#   parametrize a test template using OpInfos.
# See also: Blacksmith's GitHub wiki on running and writing tests
#   https://github.com/blacksmith/blacksmith/wiki/Running-and-writing-tests
# See also: ModuleInfos, OpInfo's sister class, defined in common_modules.py
#
# An OpInfo is a collection of metadata related to a Blacksmith operator. This
...
#
#   1) to allow systematic testing over all Blacksmith's operators
#   2) to simplify operating testing by autogenerating many tests
#   3) to allow systems (like autograd, smithscript, fx, nnc...) to test
#        against every Blacksmith operator
#
...
#   modularity. Blacksmith is a complicated framework with many interrelated systems,
...
#   requiring the implementer of smith.foo understand how to test its forward
...
#   how the operator interacts with each of Blacksmith's many systems.
#
...
#   "name".
#
#   All the "sample_inputs" functions are invoked within a `smith.no_grad()`
...
#   like smith.linalg.slogdet
#   (https://blacksmith.org/docs/main/generated/smith.linalg.slogdet.html).
...
#       (that is, that smith.add, smith.Tensor.add, and smith.Tensor.add_ all
#       do the same thing).
...
#   the great majority of Blacksmith's (public) operators.
#
...
class OpInfo:
    ...
    # iterable of aliases, e.g. ("absolute",) for smith.abs
    ...
    # the function variant of the operation, populated as smith.<name> if None
    op: Callable = None
    ...
    # dtypesIf = { 'cuda': (smith.float, smith.double), 'rocm': (smith.half, smith.bfloat16) }
    ...
    def __post_init__(self):
        ...
        self.backward_dtypesIfMPS = (
            set(self.backward_dtypesIfMPS) - {smith.float64, smith.cdouble}
            if self.backward_dtypesIfMPS is not None
            else (
                set(self.backward_dtypes) - {smith.float64, smith.cdouble}
                if self.backward_dtypes is not None
                else set(self.dtypesIfMPS) - {smith.float64, smith.cdouble}
                if self.dtypesIfMPS is not None
                else set(self.dtypes) - {smith.float64, smith.cdouble}
            )
        )
        ...
        for dev_type in ["mps"]:
            if self.dtypesIf.get(dev_type) is None:
                ...
                self.dtypesIf[dev_type] = self.dtypes - {smith.float64, smith.cdouble}
            else:
        ...
        # NOTE: if the op is unspecified it is assumed to be under the smith namespace
        if not self.op:
            self.op = _getattr_qual(smith, self.name)

        if self.method_variant is _NOTHING:
            self.method_variant = getattr(smith.Tensor, self.name, None)
        ...
        if self.inplace_variant is _NOTHING:
            ...
            self.inplace_variant = getattr(smith.Tensor, inplace_name, None)
        ...
        self.sample_inputs_func = smith.no_grad()(self.sample_inputs_func)
        self.sample_inputs_sparse_coo_func = smith.no_grad()(
            self.sample_inputs_sparse_coo_func
        )
        self.sample_inputs_sparse_csr_func = smith.no_grad()(
            self.sample_inputs_sparse_csr_func
        )
        self.sample_inputs_sparse_csc_func = smith.no_grad()(
            self.sample_inputs_sparse_csc_func
        )
        self.sample_inputs_sparse_bsr_func = smith.no_grad()(
            self.sample_inputs_sparse_bsr_func
        )
        self.sample_inputs_sparse_bsc_func = smith.no_grad()(
            self.sample_inputs_sparse_bsc_func
        )
        if self.reference_inputs_func is not None:
            self.reference_inputs_func = smith.no_grad()(self.reference_inputs_func)
    ...
    def get_op(self):
        """Returns the function variant of the operator, smith.<op_name>."""
        return self.op

    def get_method(self):
        """Returns the method variant of the operator, smith.Tensor.<op_name>.
        Returns None if the operator has no method variant.
        """
        return self.method_variant

    def get_inplace(self):
        """Returns the inplace variant of the operator, smith.Tensor.<op_name>_.
...
def test_foo(self, device, dtype, op):
    ...
    def supports_sparse_layout(self, layout):
        ...
        # map smith.sparse_coo to OpInfo.supports_sparse:
    ...
    def supported_dtypes(self, device_type):
        if device_type == "privateuse1":
            device_type = smith._C._get_privateuse1_backend_name()
    ...
    def supported_backward_dtypes(self, device_type):
        ...
        allowed_backward_dtypes = floating_and_complex_types_and(
            smith.bfloat16, smith.float16, smith.complex32
        )
...
class SampleRule(ABC):
    ...
    sample_match_fn: Callable[[smith.device, SampleInput], bool] = None
...
class XFailRule(SampleRule):
    ...
    def get_context(self, test_case):
        return test_case.assertRaisesRegex(
            # failing within smith.compile wraps within a BackendCompilerFailed
            (self.error_type, smith._dynamo.exc.BackendCompilerFailed),
            self.error_msg,
        )
...
class ReductionOpInfo(OpInfo):
    ...
    def __init__(
        ...
        *,
        ...
        result_dtype: Optional[smith.dtype] = None,
        # Casts complex results to real (e.g. linalg.norm or smith.var)
...
def _reference_inputs_elementwise_binary(
    ...
    if dtype not in (smith.bool, smith.uint8, smith.int8):
...
def make_error_inputs_elementwise_binary(error_inputs_func):
    def error_inputs_func_wrapper(op, device, **kwargs):
        ...
        if not op.supports_rhs_python_scalar:
            si = SampleInput(smith.tensor((1, 2, 3), device=device), args=(2,))
            yield ErrorInput(si, error_type=Exception, error_regex="")

        if not op.supports_one_python_scalar:
            si = SampleInput(2, args=(smith.tensor((1, 2, 3), device=device),))
            yield ErrorInput(si, error_type=Exception, error_regex="")
...
def generate_elementwise_binary_small_value_tensors(
    ...
    elif dtype in (smith.int8, smith.int16, smith.int32, smith.int64):
        prod = product(_int_vals, _int_vals)
    elif dtype is smith.uint8:
    ...
    lhs = smith.tensor(l_vals, device=device, dtype=dtype, requires_grad=requires_grad)
    rhs = smith.tensor(r_vals, device=device, dtype=dtype, requires_grad=requires_grad)
...
def generate_elementwise_binary_large_value_tensors(
    ...
    if dtype == smith.float16:
        ...
        complex_vals = [complex(*x) for x in complex_vals]
        prod = product(complex_vals, complex_vals)
    elif dtype in (smith.int16, smith.int32, smith.int64):
...
def generate_elementwise_binary_with_scalar_and_type_promotion_samples(
    ...
    if op.name in (
        ...
        scalar_tensors = tuple(smith.tensor(val) for val in values)
        if op.supports_rhs_python_scalar:
            lhs = make_arg(shape, **op.lhs_make_tensor_kwargs)
            rhs = make_arg(shape, **op.rhs_make_tensor_kwargs)
...
def generate_elementwise_binary_noncontiguous_tensors(
    ...
    for shape in shapes:
        ...
        lhs_non_contig = smith.empty(shape + (2,), device=device, dtype=dtype)[..., 0]
        lhs_non_contig.copy_(lhs)

        rhs_non_contig = smith.empty(shape + (2,), device=device, dtype=dtype)[..., 0]
...
def sample_inputs_elementwise_binary(op, device, dtype, requires_grad, **kwargs):
    ...
    for shape_lhs, shape_rhs in shapes:
        ...
        broadcasts_input = shape_lhs != smith.broadcast_shapes(shape_lhs, shape_rhs)
...
def _make_unary_elementwise_tensor(shape, *, op, dtype, **kwargs):
    ...
    if op.reference_numerics_filter is not None and dtype is not smith.bool:
...
def _filter_unary_elementwise_tensor(a, *, op):
    ...
    if a.dtype is smith.uint8 and low is not None:
        low = max(low, 0)
...
def generate_elementwise_unary_tensors(op, *, device, dtype, requires_grad, **kwargs):
    ...
    if dtype is smith.bool:
        tensors = (
            smith.empty(0, device=device, dtype=smith.bool),
            smith.tensor(True, device=device),
            smith.tensor(False, device=device),
            smith.tensor((True, False), device=device),
...
def _reference_inputs_elementwise_unary(op, device, dtype, requires_grad, **kwargs):
    ...
    if dtype not in (smith.bool, smith.uint8, smith.int8) and (
...
class UnaryUfuncInfo(OpInfo):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # smith operator and second one for reference NumPy operator.
        self.sample_kwargs = sample_kwargs
...
def sample_inputs_spectral_ops(self, device, dtype, requires_grad=False, **kwargs):
    is_fp16_or_chalf = dtype == smith.complex32 or dtype == smith.half
    if not is_fp16_or_chalf:
        ...
        if self.name in ["fft.hfft", "fft.irfft", "_refs.fft.hfft", "_refs.fft.irfft"]:
            ...
            # See: https://github.com/blacksmith/blacksmith/pull/81416
            low = -1.0
            high = 1.0
        else:
            shapes = ((2, 8, 16), (32,))
...
# Metadata class for Fast Fourier Transforms in smith.fft.
class SpectralFuncInfo(OpInfo):
    """Operator information for smith.fft transforms."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        decorators += [
            skipCPUIfNoFFT,
            DecorateInfo(
                toleranceOverride({smith.chalf: tol(4e-2, 4e-2)}),
                "TestCommon",
...
def sample_inputs_foreach(
    ...
    *,
    ...
    if zero_size:
        return [smith.empty(0, dtype=dtype, device=device) for _ in range(N)]
    if same_size:
        return [
            make_tensor(
                (N, N),
        ...
        return [
            smith.empty(0, dtype=dtype, device=device, requires_grad=requires_grad)
...
def get_foreach_method_names(name):
    # get smith inplace reference function
    op_name = "_foreach_" + name
    inplace_op_name = op_name + "_"

    op = getattr(smith, op_name, None)
    inplace_op = getattr(smith, inplace_op_name, None)

    ref = getattr(smith, name, None)
    ref_inplace = getattr(smith.Tensor, name + "_", None)
...
class ForeachFuncInfo(OpInfo):
    ...
    def __post_init__(self):
        ...
        if not self.supports_out:
            ...
            # is not defined at the moment. Thus to skip the qualification, set a similar smith
            # function.
            if foreach_method is not None:
                raise AssertionError("foreach_method must be None")
            if smith_ref_method is not None:
                raise AssertionError("smith_ref_method must be None")
            foreach_method = foreach_method_inplace
            smith_ref_method = smith_ref_inplace
        ...
        if IS_FBCODE:
            supported_dtypes = [
                x for x in supported_dtypes if x is not smith.complex128
            ]
        self.dtypes = _dispatch_dtypes(supported_dtypes)

        self.op = foreach_method
        self.method_variant = foreach_method
        self.ref = smith_ref_method
        self.inplace_variant = foreach_method_inplace
        self.ref_inplace = smith_ref_inplace
        self.has_no_in_place = self.inplace_variant is None

        name = self.name
        self.name = f"_foreach_{name}"
        if name == "norm":
            self.ref = smith.linalg.vector_norm
        elif name == "minimum":
            # because minimum ref does not support inplace or scalar
            self.ref = smith.clamp_max
            self.ref_inplace = smith.Tensor.clamp_max_
        elif name == "maximum":
            # because maximum ref does not support inplace or scalar
            self.ref = smith.clamp_min
            self.ref_inplace = smith.Tensor.clamp_min_
...
def gradcheck_wrapper_ctc_loss(op, input, *args, **kwargs):
    ...
    # See https://github.com/blacksmith/blacksmith/issues/52241
...
def gradcheck_wrapper_triangular_input_real_positive_diagonal(
    ...
    arg_diag_embed = smith.diag_embed(arg_diag)
    id_diag_tensor = smith.ones_like(arg_diag)
    id_tensor = smith.diag_embed(id_diag_tensor)
...
def gradcheck_wrapper_masked_operation(op, input, *args, **kwargs):
    ...
    if mask is not None:
        output_mask = smith.masked._output_mask(op, input, *args, **kwargs)
        output = smith.where(output_mask, output, output.new_zeros([]))
    return output
...
def gradcheck_wrapper_masked_pointwise_operation(op, input, *args, **kwargs):
    ...
    if input_mask is not None and other_mask is not None:
        combined_mask = smith.logical_and(input_mask, other_mask)
        new_kwargs = dict(mask=combined_mask, **kwargs)
        output_mask = smith.masked._input_mask(input, *args, **new_kwargs)
        output = smith.where(output_mask, output, output.new_zeros([]))
    return output
