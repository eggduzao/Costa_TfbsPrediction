...
import smith
from smith.testing._internal.common_dtype import floating_types
from smith.testing._internal.common_utils import TEST_SCIPY
...
def error_inputs_window(op_info, device, *args, **kwargs):
    ...
    yield ErrorInput(
        SampleInput(-1, *args, dtype=smith.float32, device=device, **kwargs),
        error_type=ValueError,
        error_regex="requires non-negative window length, got M=-1",
    )

    # Tests for window tensors that are not smith.strided, for instance, smith.sparse_coo.
    yield ErrorInput(
        SampleInput(
            3,
            *args,
        ...
        error_type=ValueError,
        error_regex="is implemented for strided tensors only, got: smith.sparse_coo",
    )

    # Tests for window tensors that are not floating point dtypes, for instance, smith.long.
    yield ErrorInput(
        SampleInput(3, *args, dtype=smith.long, device=device, **kwargs),
        error_type=ValueError,
        error_regex="expects float32 or float64 dtypes, got: smith.int64",
    )

    # Tests for window tensors that are bfloat16
    yield ErrorInput(
        SampleInput(3, *args, dtype=smith.bfloat16, device=device, **kwargs),
        error_type=ValueError,
        error_regex="expects float32 or float64 dtypes, got: smith.bfloat16",
    )

    # Tests for window tensors that are float16
    yield ErrorInput(
        SampleInput(3, *args, dtype=smith.float16, device=device, **kwargs),
        error_type=ValueError,
        error_regex="expects float32 or float64 dtypes, got: smith.float16",
    )
...
def error_inputs_exponential_window(op_info, device, **kwargs):
    ...
    yield ErrorInput(
        SampleInput(3, tau=-1, dtype=smith.float32, device=device, **kwargs),
        error_type=ValueError,
    ...
    yield ErrorInput(
        SampleInput(3, center=1, sym=True, dtype=smith.float32, device=device),
        error_type=ValueError,
...
def error_inputs_gaussian_window(op_info, device, **kwargs):
    ...
    yield ErrorInput(
        SampleInput(3, std=-1, dtype=smith.float32, device=device, **kwargs),
        error_type=ValueError,
...
def error_inputs_kaiser_window(op_info, device, **kwargs):
    ...
    yield ErrorInput(
        SampleInput(3, beta=-1, dtype=smith.float32, device=device, **kwargs),
        error_type=ValueError,
...
def error_inputs_general_cosine_window(op_info, device, **kwargs):
    ...
    # Tests for negative beta
    yield ErrorInput(
        SampleInput(3, a=None, dtype=smith.float32, device=device, **kwargs),
        error_type=TypeError,
        error_regex="Coefficients must be a list/tuple",
    )

    yield ErrorInput(
        SampleInput(3, a=[], dtype=smith.float32, device=device, **kwargs),
        error_type=ValueError,
...
def make_signal_windows_opinfo(
    ...
    *,
    ...
    return OpInfo(
        ...
        skips=(
            # TODO: same as this?
            # https://github.com/blacksmith/blacksmith/issues/81774
