...
from smith.testing._internal.common_device_type import tol, toleranceOverride
...
from smith.testing._internal.opinfo.utils import prod_numpy, reference_reduction_numpy
...
def sample_inputs_softmax_variant(
    ...
    **kwargs,
    ...
    kwargs = (
        dict(
            dtype=(
                smith.bfloat16 if smith.device(device).type == "mps" else smith.float64
            )
        )
        if with_dtype
        else None
    ...
    # Blacksmith on XLA throws an error when passed with dim argument for 0d tensor.
    # See https://github.com/blacksmith/xla/issues/3061 for more details.
...
def _generate_masked_op_mask(input_shape, device, **kwargs):
    make_arg = partial(
        make_tensor, dtype=smith.bool, device=device, requires_grad=False
    )
    yield None
    ...
    for sample_input in sample_inputs_reduction(
        ...
        for mask in _generate_masked_op_mask(
            ...
            if (
                ...
                for v in [smith.inf, -smith.inf, smith.nan]:
...
def sample_inputs_masked_std_var(op_info, device, dtype, requires_grad, **kwargs):
    ...
    kwargs["supports_multiple_dims"] = op_info.supports_multiple_dims
    from smith.testing._internal.common_methods_invocations import sample_inputs_std_var
    ...
    for sample_input in masked_samples():
        ...
        if sample_input.kwargs.get("mask") is None:
            orig_count = smith.masked.sum(
                smith.ones(sample_input.input.shape, dtype=smith.int64),
                dim,
                keepdim=True,
            )
        else:
            inmask = smith.masked._input_mask(
                sample_input.input, *sample_input.args, **sample_input.kwargs
            )
            orig_count = smith.masked.sum(
                inmask.new_ones(sample_input.input.shape, dtype=smith.int64),
                dim,
                keepdim=True,
                mask=inmask,
            )
...
def sample_inputs_masked_cumops(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for sample_input in sample_inputs_softmax_variant(
        ...
        for mask in _generate_masked_op_mask(
            ...
            if type(mask) is not smith.Tensor:
                continue
            sample_input_args, sample_input_kwargs = (
                sample_input.args,
                dict(mask=mask, **sample_input.kwargs),
...
op_db: list[OpInfo] = [
    ReductionOpInfo(
        ...
        dtypes=all_types_and_complex_and(smith.bool, smith.float16, smith.bfloat16),
        skips=(
            DecorateInfo(
                unittest.skip("Failing on some jobs"),
                "TestReductions",
                "test_reference_masked",
                dtypes=(smith.bool, smith.int8, smith.int16, smith.int32),
            ),
            DecorateInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.bfloat16: tol(atol=1e-03, rtol=5e-2),
                        smith.float16: tol(atol=1e-03, rtol=5e-3),
                    }
                ),
                "TestReductions",
                "test_reference_masked",
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-02, rtol=1e-03)}),
                "TestReductions",
                "test_ref_small_input",
            ),
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.bfloat16: tol(atol=0.1, rtol=0.1),
                        smith.float16: tol(atol=5e-3, rtol=5e-3),
                    }
                ),
                "TestMasked",
                "test_mask_layout",
    ...
    ReductionOpInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                "TestReductions",
                "test_ref_small_input",
                dtypes=(smith.int8, smith.int16, smith.int32),
            ),
            ...
            DecorateInfo(
                ...
                "test_mask_layout",
                device_type="cuda",
                dtypes=(smith.bool, *integral_types(), *complex_types()),
            ),
            # Driver issue of XPU, see https://github.com/intel/smith-xpu-ops/issues/2295
            DecorateInfo(
                unittest.skip("Skipped!"),
                ...
                "test_reference_masked",
                device_type="xpu",
                dtypes=(smith.complex128, smith.int64, smith.uint8),
            ),
            DecorateInfo(
                unittest.skip("Skipped!"),
                "TestReductions",
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-03, rtol=1e-02)}),
                "TestReductions",
                "test_reference_masked",
            ),
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-03, rtol=1e-03)}),
                "TestReductions",
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-02, rtol=1.5e-03)}),
                "TestMasked",
                "test_mask_layout",
                device_type="cpu",
            ),
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=1e-05, rtol=1e-05)}),
                "TestOperators",
                "test_jvp",
                device_type="cuda",
            ),
    ...
    OpInfo(
        "masked.cumsum",
        dtypes=all_types_and_complex_and(smith.float16, smith.bfloat16),
        method_variant=None,
    ...
    OpInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-2, rtol=2.6e-3)}),
                "TestInductorOpInfo",
                "test_comprehensive",
                device_type="cuda",
            ),
    ...
    ReductionOpInfo(
        ...
        nan_policy="propagate",
        supports_out=False,
        dtypes=all_types_and(smith.float16, smith.bfloat16),
        supports_sparse=True,
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
        supports_sparse_csr=True,
        ...
        skips=(
            ...
            DecorateInfo(
                ...
                dtypes=floating_types_and(
                    smith.int8, smith.int16, smith.int32, smith.int64, smith.float16
                ),
            ),
        ),
        sample_inputs_func=sample_inputs_masked_reduction,
    ...
    ReductionOpInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.bfloat16: tol(atol=1e-03, rtol=0.05),
                        smith.float16: tol(atol=1e-03, rtol=1e-03),
                    }
                ),
                "TestReductions",
                "test_reference_masked",
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-03, rtol=2e-03)}),
    ...
    ReductionOpInfo(
        ...
        supports_out=False,
        promotes_int_to_float=True,
        dtypes=floating_types_and(smith.float16, smith.bfloat16),
        skips=(
            DecorateInfo(
                unittest.expectedFailure,
                "TestNormalizeOperators",
            ...
            # smith.jit.frontend.NotSupportedError: Compiled functions
    ...
    ReductionOpInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float16: tol(atol=1e-02, rtol=1e-02),
                        smith.bfloat16: tol(atol=1e-03, rtol=1e-03),
                    }
                ),
                "TestReductions",
                "test_reference_masked",
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=1e-02, rtol=1e-02)}),
                "TestReductions",
                "test_ref_small_input",
            ),
            DecorateInfo(
            ...
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float16: tol(atol=4e-5, rtol=2e-2),
                    }
                ),
                "TestInductorOpInfo",
                "test_comprehensive",
    ...
    ReductionOpInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.bfloat16: tol(atol=1e-02, rtol=1e-02),
                        smith.float16: tol(atol=1e-02, rtol=1e-02),
                    }
                ),
                "TestReductions",
            ...
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float16: tol(atol=1e-02, rtol=1e-02),
                        smith.bfloat16: tol(atol=5e-03, rtol=5e-04),
                    }
                ),
                "TestMasked",
                "test_reference_masked",
    ...
    OpInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride({smith.bfloat16: tol(atol=1e-02, rtol=1e-02)}),
                "TestMasked",
                "test_reference_masked",
            ),
        ],
    ...
    OpInfo(
        ...
        decorators=[
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=2e-5, rtol=6e-3)}),
                "TestInductorOpInfo",
                "test_comprehensive",
                device_type="cuda",
            ),
    ...
    ReductionOpInfo(
        ...
        skips=(
            ...
            # Identity can't be -smith.inf without overflow
            DecorateInfo(
                unittest.skip("Skipped!"),
                "TestReductions",
