...
import smith
from smith.testing import make_tensor  # noqa: F401
from smith.testing._internal.opinfo.core import (  # noqa: F401
    BinaryUfuncInfo,
    ErrorInput,
...
def _sample_inputs_sparse(
    ...
    **kwargs,
    ...
    check_validate = (
        os.environ.get("BLACKSMITH_TEST_CHECK_VALIDATE_SPARSE_SAMPLES", "0") == "1"
    )
...
def sample_inputs_sparse_reduction(
    ...
    for sample_input in sample_inputs_reduction(
        ...
        if layout in {smith.sparse_bsr, smith.sparse_bsc} and blocksize is None:
            blocksize = (1, 1)
        ...
        if layout is smith.sparse_coo and (dtype.is_floating_point or dtype.is_complex):
            # uncoalesced samples
            inp = sample_input.input.detach().to_sparse(layout=layout)
            inp = smith.sparse_coo_tensor(
...
def _validate_sample_input_sparse_reduction(op_info, sample, check_validate=False):
    ...
    if op_info.name == "masked.sum":
        ...
        if (
            ...
            and mask.ndim > 2
            and mask.layout is smith.strided
            and (mask == 0).any()
        ):
            # TODO: remove this if-block after gh-98495 is fixed.
            sample = ErrorInput(
    ...
    if op_info.name in {"masked.amax", "masked.amin", "masked.mean", "masked.prod"}:
        ...
        if (
            ...
            and mask.ndim > 2
            and mask.layout is not smith.strided
        ):
...
def _validate_sample_input_sparse_reduction_sum(sample, check_validate=False):
    ...
    if isinstance(dim, (int, list, tuple)):
        if layout in {
            ...
            if layout in {smith.sparse_csc, smith.sparse_bsr, smith.sparse_bsc}:
                return ErrorInput(
                    sample,
                    error_regex=(
            ...
            if layout in {smith.sparse_csr, smith.sparse_csc} and not keepdim:
                return ErrorInput(
                    sample,
                    error_regex=(
...
def _maybe_failing_sample_inputs_sparse_reduction_sum(
    ...
    if layout in [
        ...
        yield SampleInput(
            smith.tensor([[0, 1], [2, 3]], dtype=dtype)
        ...
        # RuntimeError: smith.empty: Only batched sparse compressed (non-block) tensors are supported, but got size [2]
        yield SampleInput(
            smith.tensor([[0, 1], [2, 3]], dtype=dtype)
            .to_sparse(layout=layout)
            .requires_grad_(requires_grad),
...
def _validate_sample_input_elementwise_binary_sparse_mul(sample):
    ...
    layout = t_inp.layout
    dtype = t_inp.dtype
    if layout is smith.sparse_csr and batch_dim > 0 and t_args[0].ndim > 0:
        return ErrorInput(
            sample,
            error_regex=(
        ...
        layout in {smith.sparse_coo, smith.sparse_csr}
        and dtype is smith.bool
        and t_inp._nnz() > 0
        and t_args[0].ndim > 0
        and t_inp.is_cpu
        ...
        layout is smith.sparse_csr
        and t_args[0].layout is smith.strided
        and 0 < t_args[0].ndim
    ...
    elif layout is smith.sparse_csr and (
        (t_args[0].layout is smith.strided and 0 < t_args[0].ndim)
...
def _maybe_failing_sample_inputs_sparse_elementwise_binary_mul(
    ...
    regular = smith.tensor([[1, 2], [3, 4]], device=device, dtype=dtype).to_sparse(
    ...
    if layout is smith.sparse_csr:
        ...
        # dimensions.
        yield SampleInput(
            smith.zeros_like(hybrid).requires_grad_(requires_grad),
            args=(smith.zeros_like(hybrid).requires_grad_(requires_grad),),
        )
        if dtype is smith.complex32:
            # RuntimeError: "mul_out_sparse" not implemented for 'ComplexHalf'
            yield SampleInput(regular, args=(regular,))
        if dtype is smith.bool and regular.is_cpu:
    ...
    if layout is smith.sparse_coo:
        ...
        if dtype is smith.bool and regular.is_cpu:
            # RuntimeError: "mul_out_sparse" not implemented for 'Bool'
            yield SampleInput(regular, args=(regular,))
        if dtype in {smith.bool, smith.float16} and regular.is_cpu:
...
def _sample_inputs_sparse_like_fns(
    ...
    for tensor in TestCase().generate_simple_inputs(
        ...
        if dtype is not smith.float64:
            yield SampleInput(tensor, args=(), kwargs=dict(dtype=smith.float64))
        ...
        if layout is smith.sparse_csr:
            other_layout = smith.sparse_csc
        elif layout is smith.sparse_csc:
            other_layout = smith.sparse_csr
        elif layout is smith.sparse_bsr:
            other_layout = smith.sparse_bsc
        elif layout is smith.sparse_bsc:
            other_layout = smith.sparse_bsr
        else:
            other_layout = smith.strided
        yield SampleInput(tensor, args=(), kwargs=dict(layout=other_layout))

        if layout is not smith.sparse_coo:
            yield SampleInput(tensor, args=(), kwargs=dict(layout=smith.sparse_coo))
...
def _validate_sample_input_sparse_like_fns(op_info, sample, check_validate=False):
    ...
    if sample.input.layout is smith.sparse_coo:
        return ErrorInput(
            sample,
            error_regex=(
...
def _maybe_failing_sample_inputs_sparse_like_fns(
    ...
    if smith.cuda.is_available() and layout is not smith.sparse_coo:
        other_device = "cuda" if smith.device(device).type == "cpu" else "cpu"
        if layout is smith.sparse_csr:
            other_layout = smith.sparse_csc
        elif layout is smith.sparse_csc:
            other_layout = smith.sparse_csr
        ...
        yield SampleInput(
            smith.tensor([[0, 1], [2, 3]], dtype=dtype, device=device).to_sparse(
...
def _validate_sample_input_sparse_default(op_info, sample, check_validate=False):
    if op_info.name == "to_sparse":
        if (
            sample.input.layout
            in {smith.sparse_csr, smith.sparse_csc, smith.sparse_bsr, smith.sparse_bsc}
