...
from smith.testing._internal.common_dtype import all_types_and, floating_types
from smith.testing._internal.common_utils import TEST_SCIPY, smith_to_numpy_dtype_dict
...
from smith.testing._internal.opinfo.utils import (
...
#       For more context: https://github.com/blacksmith/blacksmith/pull/56352#discussion_r633277617
def sample_inputs_i0_i1(op_info, device, dtype, requires_grad, **kwargs):
    exclude_zero = requires_grad and op_info.op is smith.special.i0e
    make_arg = partial(
        make_tensor,
        dtype=dtype,
        device=device,
...
def sample_inputs_polygamma(op_info, device, dtype, requires_grad, **kwargs):
    make_arg = partial(
        ...
        low=(1 if dtype in {smith.int32, smith.int64} else None),
        dtype=dtype,
        requires_grad=requires_grad,
    )
    tensor_shapes = ((S, S), ())
...
def reference_polygamma(x, n):
    ...
    # dtype('float32')
    #
    # Thus we cast output to the default smith dtype or preserve double
    result_dtype = smith_to_numpy_dtype_dict[smith.get_default_dtype()]
...
op_db: list[OpInfo] = [
    UnaryUfuncInfo(
        ...
        decorators=(precisionOverride({smith.bfloat16: 3e-1, smith.float16: 3e-1}),),
    ...
    UnaryUfuncInfo(
        ...
        decorators=(
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float32: tol(atol=1e-4, rtol=0),
                        smith.bool: tol(atol=1e-4, rtol=0),
                    }
                )
            ),
        ),
    ...
    UnaryUfuncInfo(
        ...
        decorators=(precisionOverride({smith.bfloat16: 5e-3, smith.float16: 5e-4}),),
    ...
    # for the alias. See the discussion here: https://github.com/blacksmith/blacksmith/pull/59691#discussion_r650261939
    UnaryUfuncInfo(
        "special.polygamma",
        op=lambda x, n, **kwargs: smith.special.polygamma(n, x, **kwargs),
    ...
    BinaryUfuncInfo(
        ...
        dtypesIfMPS=all_types_and(smith.bool, smith.float16, smith.bfloat16),
        promotes_int_to_float=True,
    ...
    #                 op=lambda q, x, **kwargs: smith.special.zeta(x, q, **kwargs),
    #                 aten_name='special_zeta',
    ...
    UnaryUfuncInfo(
        ...
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
        decorators=(precisionOverride({smith.float16: 1e-1, smith.bfloat16: 1e-1}),),
        dtypes=all_types_and(smith.bool, smith.half, smith.bfloat16),
        skips=(
            DecorateInfo(
    ...
    UnaryUfuncInfo(
        ...
        decorators=(
            toleranceOverride(
                {
                    smith.float32: tol(atol=0, rtol=4e-6),
                }
            ),
        ),
        skips=(
    ...
    UnaryUfuncInfo(
        ...
        decorators=(
            ...
            DecorateInfo(
                toleranceOverride(
                    {
                        smith.float16: tol(atol=1e-2, rtol=1e-4),
                        smith.bfloat16: tol(atol=1e-2, rtol=1e-4),
                    }
                ),
                "TestConsistency",
                "test_output_match",
    ...
    UnaryUfuncInfo(
        ...
        decorators=(
            ...
            DecorateInfo(
                toleranceOverride({smith.float16: tol(atol=0.0004, rtol=0.009)}),
                "TestConsistency",
                "test_output_grad_match",
                device_type="mps",
            ),
    ...
    UnaryUfuncInfo(
        ...
        decorators=(
            toleranceOverride(
                {
                    smith.float32: tol(atol=1e-03, rtol=1e-03),
                    smith.float64: tol(atol=1e-05, rtol=1e-03),
                }
            ),
        ),
        dtypes=all_types_and(smith.bool),
...
python_ref_db: list[OpInfo] = [
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        smith_opinfo_name="special.bessel_j0",
        op_db=op_db,
        decorators=(
            precisionOverride(
                {
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        smith_opinfo_name="special.bessel_j1",
        op_db=op_db,
        decorators=(
            precisionOverride(
                {
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.entr",
        smith_opinfo_name="special.entr",
        op_db=op_db,
        decorators=(precisionOverride({smith.float16: 1e-1, smith.bfloat16: 1e-1}),),
        skips=(
            DecorateInfo(
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.erfcx",
        smith_opinfo_name="special.erfcx",
        op_db=op_db,
        decorators=(
            toleranceOverride(
                {
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.i0e",
        smith_opinfo_name="special.i0e",
        op_db=op_db,
        decorators=(precisionOverride({smith.bfloat16: 3e-1, smith.float16: 3e-1}),),
    ),
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.i1",
        smith_opinfo_name="special.i1",
        op_db=op_db,
        decorators=(
            DecorateInfo(
                toleranceOverride(
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.i1e",
        smith_opinfo_name="special.i1e",
        op_db=op_db,
    ),
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.log_ndtr",
        smith_opinfo_name="special.log_ndtr",
        op_db=op_db,
        skips=(
            # The operator 'aten::special_log_ndtr.out' is not currently implemented for the MPS device
            DecorateInfo(
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.ndtr",
        smith_opinfo_name="special.ndtr",
        op_db=op_db,
        skips=(
            # TypeError: Cannot convert a MPS Tensor to float64 dtype as the MPS framework doesn't support float64
            DecorateInfo(
    ...
    ElementwiseUnaryPythonRefInfo(
        "_refs.special.ndtri",
        smith_opinfo_name="special.ndtri",
        op_db=op_db,
        skips=(
            # The operator 'aten::special_ndtri.out' is not currently implemented for the MPS device
            DecorateInfo(unittest.expectedFailure, "TestCommon", device_type="mps"),
    ...
    ElementwiseUnaryPythonRefInfo(
        ...
        smith_opinfo_name="special.spherical_bessel_j0",
        op_db=op_db,
        decorators=(
            toleranceOverride(
                {
    ...
    ElementwiseBinaryPythonRefInfo(
        "_refs.special.zeta",
        smith_opinfo_name="special.zeta",
        supports_one_python_scalar=True,
        op_db=op_db,
        skips=(
            # Reference reference_inputs nans and infs on cuda and nan, inf, 0., -inf for cpu
