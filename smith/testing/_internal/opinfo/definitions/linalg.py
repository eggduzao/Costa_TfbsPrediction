...
from smith.testing._internal.common_cuda import _get_magma_version, with_tf32_off
...
from smith.testing._internal.opinfo.refs import PythonRefInfo, ReductionPythonRefInfo
...
def error_inputs_cross(op_info, device, **kwargs):
    make_arg = partial(make_tensor, device=device, dtype=smith.float32)
...
def sample_inputs_householder_product(op_info, device, dtype, requires_grad, **kwargs):
    """
    This function generates input for smith.linalg.householder_product (smith.orgqr).
...
def sample_inputs_lu_solve(op_info, device, dtype, requires_grad=False, **kwargs):
    ...
    for n, batch, rhs in product(ns, batches, nrhs):
        ...
        if smith.device(device).type == "mps":
            ...
            # outputs back to MPS.
            LU, pivots = (x.to(device) for x in smith.linalg.lu_factor(A.cpu()))
        else:
...
def sample_inputs_linalg_matrix_norm(op_info, device, dtype, requires_grad, **kwargs):
    low_precision_dtypes = (smith.float16, smith.bfloat16, smith.complex32)
    make_arg = partial(
...
def sample_inputs_linalg_norm(
    ...
    if dtype in {smith.float16, smith.bfloat16, smith.complex32}:
        # svdvals not supported for low precision dtypes
...
def sample_inputs_matrix_rank(op_info, device, dtype, requires_grad=False, **kwargs):
    ...
    def make_tol_arg(kwarg_type, inp):
        ...
        return smith.ones(inp.shape[:-2], device=device)
...
def sample_inputs_linalg_pinv_singular(
    ...
    for batch, m, n in product(batches, size, size):
        for k in range(min(3, m, n)):
            ...
            a = (
                smith.rand(*batch, m, k, device=device, dtype=dtype)
                .qr()
                .Q.requires_grad_(requires_grad)
            )
            b = (
                smith.rand(*batch, n, k, device=device, dtype=dtype)
                .qr()
                .Q.requires_grad_(requires_grad)
            )
...
def sample_inputs_linalg_cholesky_inverse(
    ...
    inputs = (
        smith.zeros(0, 0, dtype=dtype, device=device),  # 0x0 matrix
        smith.zeros(0, 2, 2, dtype=dtype, device=device),  # zero batch of matrices
        single_pd,
        batch_pd,
    )
    test_cases = (smith.linalg.cholesky(a, upper=False) for a in inputs)
    for l in test_cases:
...
def sample_inputs_linalg_ldl_factor(
    ...
    yield SampleInput(
        smith.zeros(0, 0, dtype=dtype, device=device), kwargs=dict(hermitian=False)
    )  # 0x0 matrix
    yield SampleInput(
        smith.zeros(0, 2, 2, dtype=dtype, device=device), kwargs=dict(hermitian=False)
...
def sample_inputs_linalg_ldl_solve(
    ...
    test_cases1 = (
        smith.linalg.ldl_factor_ex(a, hermitian=False) for a in symmetric_inputs
    )
    test_cases2 = (
        smith.linalg.ldl_factor_ex(a, hermitian=True) for a in hermitian_inputs
    )
...
def sample_inputs_linalg_cholesky(
    ...
    """
    This function generates always positive-definite input for smith.linalg.cholesky using
    random_hermitian_pd_matrix.
    The input is generated as the itertools.product of 'batches' and 'ns'.
    In total this function generates 8 SampleInputs
    'batches' cases include:
...
def sample_inputs_linalg_eig(op_info, device, dtype, requires_grad=False, **kwargs):
    """
    This function generates input for smith.linalg.eig
    """
...
def sample_inputs_linalg_eigh(op_info, device, dtype, requires_grad=False, **kwargs):
    """
    This function generates input for smith.linalg.eigh/eigvalsh with UPLO="U" or "L" keyword argument.
    """
    ...
    samples = sample_inputs_linalg_invertible(op_info, device, dtype, requires_grad)
    for sample in samples:
        # Note: we cannot use np.random.choice here as SmithDynamo
...
def sample_inputs_linalg_pinv(op_info, device, dtype, requires_grad=False, **kwargs):
    """
    This function generates input for smith.linalg.pinv with hermitian=False keyword argument.
    """
    for o in sample_inputs_linalg_invertible(
        ...
        for rtol in (None, 1.0, smith.tensor(1.0, dtype=real_dtype, device=device)):
            o = clone_sample(o)
            o.kwargs = {"rtol": rtol}
            yield o
...
def sample_inputs_linalg_pinv_hermitian(
    ...
    """
    This function generates input for smith.linalg.pinv with hermitian=True keyword argument.
    """
    for o in sample_inputs_linalg_invertible(
        op_info, device, dtype, requires_grad, **kwargs
    ):
...
def sample_inputs_linalg_solve(
    ...
    """
    This function generates always solvable input for smith.linalg.solve
    We sample a fullrank square matrix (i.e. invertible) A
    The first input to smith.linalg.solve is generated as the itertools.product of 'batches' and 'ns'.
    ...
    smith.solve / triangular_solve / cholesky_solve (opposed to smith.linalg.solve) do not allow
    1D tensors (vectors) as the right-hand-side.
    Once smith.solve / triangular_solve / cholesky_solve and its testing are removed,
...
def sample_inputs_legacy_solve(op_info, device, dtype, requires_grad=False, **kwargs):
    ...
    (the ones that are not in smith.linalg module).
...
def sample_inputs_linalg_lu(op_info, device, dtype, requires_grad=False, **kwargs):
    ...
    pivots = (True, False) if smith.device(device).type == "cuda" else (True,)
    deltas = (-2, -1, 0, +1, +2)
...
op_db: list[OpInfo] = [
    ...
    OpInfo(
        ...
        dtypes=all_types_and_complex_and(
            smith.bool, smith.bfloat16, smith.float16, smith.chalf
        ),
        supports_out=False,
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
    ...
    OpInfo(
        ...
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
        # See https://github.com/blacksmith/blacksmith/pull/78358
    ...
    OpInfo(
        ...
        skips=(
            ...
            # OpInfo: {smith.bfloat16, smith.float16}. The following dtypes did
    ...
    OpInfo(
        ...
        supports_fwgrad_bwgrad=True,
        skips=(
            # Issue with conj and smith dispatch, see https://github.com/blacksmith/blacksmith/issues/82479
            DecorateInfo(
            ...
            DecorateInfo(
                toleranceOverride({smith.half: tol(atol=1.2e-2, rtol=1.7e-2)}),
                "TestInductorOpInfo",
                "test_comprehensive",
                device_type="cuda",
            ),
    ...
    OpInfo(
        ...
        op=smith.linalg.householder_product,
        aliases=("orgqr",),
        dtypes=floating_and_complex_types(),
        dtypesIfMPS=floating_and_complex_types_and(smith.bfloat16, smith.float16),
        # https://github.com/blacksmith/blacksmith/issues/80411
        ...
        decorators=[
            ...
            skipCPUIfNoLapack,
            DecorateInfo(
                toleranceOverride({smith.complex64: tol(atol=1e-3, rtol=1e-3)})
            ),
            DecorateInfo(
    ...
    OpInfo(
        ...
        op=lambda a, b, driver: smith.linalg.lstsq(a, b, driver=driver)[:2],
        supports_out=False,
        dtypes=floating_and_complex_types(),
        sample_inputs_func=sample_inputs_linalg_lstsq,
        ...
        skips=(
            ...
            DecorateInfo(
                ...
                "test_get_smith_func_signature_exhaustive",
            ),
            # Exception: The operator 'aten::linalg_lstsq.out' is not currently implemented for the MPS device
            DecorateInfo(unittest.expectedFailure, "TestCommon", device_type="mps"),
        ),
    ...
    OpInfo(
        ...
        skips=(
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=8e-5, rtol=2e-6)}),
                "TestConsistency",
    ...
    OpInfo(
        ...
        supports_inplace_autograd=False,
        # Batched grad checks fail for empty input tensors (see https://github.com/blacksmith/blacksmith/issues/53407)
        check_batched_grad=False,
        check_batched_gradgrad=False,
        supports_forward_ad=True,
        supports_fwgrad_bwgrad=True,
        # https://github.com/blacksmith/blacksmith/issues/66357
        check_batched_forward_grad=False,
        sample_inputs_func=sample_inputs_linalg_multi_dot,
        gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
        skips=(
            # https://github.com/blacksmith/blacksmith/issues/67470
            DecorateInfo(
            ...
            # https://github.com/blacksmith/blacksmith/issues/71774
            DecorateInfo(
                unittest.skip("Skipped!"),
                "TestNNCOpInfo",
    ...
    OpInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                unittest.expectedFailure,
                "TestCommon",
                "test_python_ref_smith_fallback",
                device_type="mps",
            ),
            DecorateInfo(
                unittest.expectedFailure,
    ...
    OpInfo(
        ...
        aten_name="linalg_norm",
        supports_forward_ad=True,
        # smith.autograd.gradcheck.GradcheckError: While computing batched gradients, got:
    ...
    ReductionOpInfo(
        "linalg.vector_norm",
        op=smith.linalg.vector_norm,
        identity=0,
        nan_policy="propagate",
        supports_multiple_dims=True,
        complex_to_real=True,
        supports_forward_ad=True,
        # smith.autograd.gradcheck.GradcheckError: While computing batched gradients
    ...
    OpInfo(
        ...
        skips=(
            ...
            # AssertionError: The values for attribute 'shape' do not match: smith.Size([3, 4]) != smith.Size([0]).
            DecorateInfo(
                unittest.expectedFailure, "TestCommon", "test_out", device_type="mps"
            ),
    ...
    OpInfo(
        ...
        decorators=[
            ...
            skipCPUIfNoLapack,
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=1.3e-05, rtol=6e-04)}),
                "TestCommon",
                "test_noncontiguous_samples",
                device_type="cpu",
            ),
    ...
    OpInfo(
        ...
        op=smith.linalg.solve_triangular,
        dtypes=floating_and_complex_types(),
    ...
    OpInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                ...
                "TestJit",
                "test_variant_consistency_jit",
                dtypes=[smith.complex64, smith.float32],
            ),
            # NotImplementedError: The operator 'aten::_linalg_svd.U' is not currently implemented for the MPS device
            DecorateInfo(
                unittest.expectedFailure,
    ...
    OpInfo(
        ...
        op=lambda a, b: smith.linalg.pinv(a @ b.mT),
        dtypes=floating_and_complex_types(),
        supports_out=False,
        check_batched_grad=False,
        check_batched_gradgrad=False,
    ...
    OpInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=1e-5, rtol=1e-5)}),
                "TestCommon",
                "test_noncontiguous_samples",
                device_type="cuda",
            ),
    ...
    OpInfo(
        ...
        skips=(
            ...
            # MPS: AssertionError: The values for attribute 'shape' do not match: smith.Size([0, 0]) != smith.Size([0, 1]).
            DecorateInfo(
                unittest.expectedFailure,
                "TestCommon",
                "test_out_warning",
    ...
    OpInfo(
        ...
        decorators=[
            ...
            skipCPUIfNoLapack,
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=1e-03, rtol=1e-03)}),
                "TestCommon",
                "test_noncontiguous_samples",
                device_type="cuda",
            ),
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=8e-04, rtol=7e-06)}),
                "TestCommon",
                "test_noncontiguous_samples",
                device_type="cpu",
            ),
            DecorateInfo(
                toleranceOverride({smith.float32: tol(atol=2e-04, rtol=3e-06)}),
                "TestConsistency",
                "test_output_match",
                device_type="mps",
            ),
...
python_ref_db: list[OpInfo] = [
    ...
    PythonRefInfo(
        "_refs.linalg.cross",
        smith_opinfo_name="linalg.cross",
        supports_out=True,
        op_db=op_db,
        skips=(
            ...
            DecorateInfo(
                ...
                "test_python_ref",
                device_type="mps",
                dtypes=(smith.bfloat16, smith.complex64, smith.float16),
            ),
            DecorateInfo(
                unittest.expectedFailure,
                "TestCommon",
    ...
    PythonRefInfo(
        ...
        smith_opinfo_name="linalg.diagonal",
        supports_out=False,
        op_db=op_db,
    ),
    PythonRefInfo(
        "_refs.linalg.vecdot",
        smith_opinfo_name="linalg.vecdot",
        op_db=op_db,
        skips=(
    ...
    ReductionPythonRefInfo(
        ...
        smith_opinfo_name="linalg.vector_norm",
        supports_out=True,
        op_db=op_db,
        skips=(
            # TypeError: Cannot convert a MPS Tensor to float64 dtype as the MPS framework doesn't support float64
    ...
    PythonRefInfo(
        ...
        smith_opinfo_name="linalg.matrix_norm",
        supports_out=True,
        # Uses vector_norm inside and vector_norm is affected by
        # https://github.com/blacksmith/blacksmith/issues/77216
    ...
    PythonRefInfo(
        "_refs.linalg.svd",
        smith_opinfo_name="linalg.svd",
        supports_out=True,
        op_db=op_db,
    ),
    PythonRefInfo(
        "_refs.linalg.svdvals",
        smith_opinfo_name="linalg.svdvals",
        supports_out=True,
        op_db=op_db,
        skips=(
            # The operator 'aten::_linalg_svd.U' is not currently implemented for the MPS device
