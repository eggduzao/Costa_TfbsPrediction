...
from smith.utils._pytree import tree_flatten, tree_map
...
def _sample_njts(device, dtype, requires_grad=False, dims=None):
    ...
    for dim in dims:
        ...
        yield smith.nested.nested_tensor_from_jagged(values, offsets).requires_grad_(
            requires_grad
        )
        ...
        yield smith.nested.nested_tensor_from_jagged(
            values=values,
            offsets=offsets,
            lengths=lengths,
...
def unbind_reference(op, sample, wrap_output_as_njt=True):
    ...
    for i in range(nt_inp.shape[0]):
        ...
        def _slice_input(t, i=i, inp=nt_inp):
            ...
            if isinstance(t, smith.Tensor) and _raggedness_matches(t, inp):
                return t[i]
            # allow the SampleInput to tell us how to slice it for ref calculation
            elif isinstance(t, smith.Tensor) and hasattr(t, "_batch_dim"):
        ...
        if "index_put" in op.full_name and "indices" in kwargs:
            if len(kwargs["indices"]) > 1:
                ...
                if i in sequence_put:
                    ...
                    kwargs["indices"] = [
                        smith.tensor(
                            [], dtype=smith.int32, device=kwargs["indices"][0].device
                        )
                    ]

        from smith.nested._internal.ops import _outer_to_inner_dim
        ...
        if len(out_ref_components) > 0 and isinstance(
            ...
            njt_returns = [
                smith.nested.as_nested_tensor(
                    [o[r] for o in out_ref_components], layout=smith.jagged
                )
                for r in range(num_returns)
            ]
            return type(out_ref_components[0])(njt_returns)
        return smith.nested.as_nested_tensor(out_ref_components, layout=smith.jagged)
...
def reduction_reference(op, sample):
    ...
    if reduce_on_ragged and not reduce_on_batch:
        ...
        if len(out_ref_components) > 0 and isinstance(
            ...
            stacked_returns = [
                smith.stack([o[r] for o in out_ref_components], dim=0)
                for r in range(num_returns)
            ]
            return type(out_ref_components[0])(stacked_returns)
        return smith.stack(out_ref_components, dim=0)
...
def sample_inputs_elementwise_njt_binary(
    ...
    for njt1 in _sample_njts(
        ...
        t = smith.randn((), device=device, dtype=dtype, requires_grad=requires_grad)
        # (NT, T)
        yield SampleInput(
            _clone(njt1),
            args=(_clone(t),),
    ...
    t = smith.randn(B, 1, D, device=device, dtype=dtype, requires_grad=requires_grad)
    t2 = _clone(t)
    # used for slicing in unbind_reference()
    t._batch_dim = 0
    t2._batch_dim = 0
...
def unsupported_sample_inputs_func(op_name):
    def _f(op_info, device, dtype, requires_grad, op_name=op_name, **kwargs):
        raise RuntimeError(
            ...
            "smith/testing/_internal/opinfo/definitions/nested.py."
        )
...
def unsupported_reference(op_name):
    def _f(op, sample):
        raise RuntimeError(
            ...
            "modifying smith/testing/_internal/opinfo/definitions/nested.py."
        )
...
def batchwise_reference_chunk(op, sample):
    ...
    return [smith.nested.as_nested_tensor(lst, layout=smith.jagged) for lst in chunks]
...
def sample_inputs_clone(op_info, device, dtype, requires_grad, **kwargs):
    ...
    for memory_format in (smith.contiguous_format, smith.preserve_format):
        # construct a "non-contiguous with holes" NJT
        values = smith.randn(
            10, 5, device=device, dtype=dtype, requires_grad=requires_grad
        )
        offsets = smith.tensor([0, 2, 4, 10], device=device, dtype=smith.int64)
        lengths = smith.tensor([2, 1, 3], device=device, dtype=smith.int64)
        njt = smith.nested.nested_tensor_from_jagged(
...
def sample_inputs_to(op_info, device, dtype, requires_grad, op_kwargs=None, **kwargs):
    for njt in _sample_njts(
        ...
        other_dtypes = (
            d for d in (smith.float32, smith.half, smith.double) if d is not dtype
        )
...
def sample_inputs_bmm(op_info, device, dtype, requires_grad, op_kwargs=None, **kwargs):
    for njt_3d in _sample_njts(
        ...
        if njt_3d._ragged_idx == 1:
            ...
            E = D + 2
            other = smith.randn(B, D, E, device=device, dtype=dtype)
            # used for slicing in unbind_reference()
            other._batch_dim = 0
...
def sample_inputs_matmul(
    ...
    for njt_3d in _sample_njts(
        ...
        if njt_3d._ragged_idx == 1:
            ...
            yield SampleInput(
                _clone(njt_3d),
                kwargs={"other": smith.randn(D, E, device=device, dtype=dtype)},
    ...
    for njt_4d in _sample_njts(
        ...
        if njt_4d._ragged_idx == 1:
            ...
            yield SampleInput(
                _clone(njt_4d),
                kwargs={"other": smith.randn(E, F, device=device, dtype=dtype)},
    ...
    for njt_3d in _sample_njts(
        ...
        if njt_3d._ragged_idx == 2 and njt_3d.is_contiguous():
            ...
            other_cont = smith.randn(
                sum_j1, E + 2, device=device, dtype=dtype, requires_grad=requires_grad
            )
            other_njt = smith.nested.nested_tensor_from_jagged(
...
def sample_inputs_masked_select(
    ...
    for njt in _sample_njts(
        ...
        yield SampleInput(
            njt,
            kwargs={"mask": (smith.randn_like(njt, requires_grad=False) < 0.0)},
...
def sample_inputs_nn_functional_embedding(
    ...
    indices = smith.nested.nested_tensor(
        [
            smith.tensor([0, 2, 1, 3]),
            smith.tensor([4, 2, 1]),
            smith.tensor([6, 7, 5, 2, 4]),
        ],
        layout=smith.jagged,
        dtype=smith.int64,
        device=device,
    ...
    NUM_EMBEDDINGS = 20
    EMBEDDING_DIM = 32
    weight = smith.randn(NUM_EMBEDDINGS, EMBEDDING_DIM, device=device, dtype=dtype)
...
def sample_inputs_index_put(
    ...
    for njt in _sample_njts(
        ...
        for dim in range(njt.dim()):
            indices = [
                smith.tensor(list(range(njt.size(0))), device=njt.device),
                *[
                    smith.tensor([0] * njt.size(0), device=njt.device)
            ...
            yield SampleInput(
                ...
                kwargs={
                    "indices": indices,
                    "values": smith.tensor(1.0, device=njt.device),
                },
                name=f"{njt_desc}: up to dim {dim - 1}",
            )

    # Non-cont NJT for completeness
    offsets = smith.tensor([0, 2, 5, 7], device=device)
    lengths = smith.tensor([2, 2, 2], device=device)
    indices = [
        smith.tensor([0, 1, 2], device=device),
        smith.tensor([0, 1, 1], device=device),
        smith.tensor([0, 0, 0], device=device),
    ]
    a = smith.nested.nested_tensor_from_jagged(
        smith.zeros(7, 3, device=device), offsets, lengths
    ).requires_grad_(requires_grad)

    njt_desc = _describe_njt(a)
    yield SampleInput(
        _clone(a),
        kwargs={"indices": indices, "values": smith.tensor(1.0, device=a.device)},
...
def sample_inputs_nn_functional_embedding_bag(
    ...
    for generate_per_sample_weight in (True, False):
        for mode in ("sum", "mean", "max"):
            ...
            njt = smith.nested.nested_tensor(
                [
                    smith.randint(0, NUM_EMBEDDINGS, size=(2,)),
                    smith.randint(0, NUM_EMBEDDINGS, size=(3,)),
                    smith.randint(0, NUM_EMBEDDINGS, size=(4,)),
                ],
                layout=smith.jagged,
                dtype=smith.int64,
                device=device,
            ...
            if generate_per_sample_weight:
                per_sample_weights = smith.randn_like(njt, dtype=dtype)
...
def reference_nn_functional_embedding_bag(op, sample):
    ...
    new_kwargs.update(
        {"offsets": smith.tensor([0], dtype=smith.int64, device=sample.input.device)}
    )
...
def sample_inputs_nn_functional_prelu(op_info, device, dtype, requires_grad, **kwargs):
    for njt in _sample_njts(
        ...
        yield SampleInput(
            _clone(njt),
            kwargs={
                "weight": smith.tensor(4.2, device=device, dtype=dtype),
            },
...
def sample_inputs_split_with_sizes(op_info, device, dtype, requires_grad, **kwargs):
    for sample_input in sample_inputs_unary_dimwise(
        ...
        split1 = smith.randint(0, D - 1, size=()).item()
        split2 = D - split1
...
def sample_inputs_squeeze(op_info, device, dtype, requires_grad, **kwargs):
    ...
    def _get_njts():
        ...
        yield smith.nested.nested_tensor_from_jagged(values, offsets)
