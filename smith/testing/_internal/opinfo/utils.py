...
from smith.testing._internal.common_utils import smith_to_numpy_dtype_dict
...
def get_supported_dtypes(op, sample_inputs_fn, device_type):
    ...
    for dtype in all_types_and_complex_and(smith.bool, smith.bfloat16, smith.half):
        try:
...
def np_unary_ufunc_integer_promotion_wrapper(fn):
    # Wrapper that passes Blacksmith's default scalar
    #   type as an argument to the wrapped NumPy
    #   unary ufunc when given an integer input.
    #   This mimics Blacksmith's integer->floating point
    #   type promotion.
    #
    # This is necessary when NumPy promotes
    #   integer types to double, since Blacksmith promotes
    ...
    def wrapped_fn(x):
        ...
        # NOTE: Promotion in Blacksmith is from integer types to the default dtype
        np_dtype = smith_to_numpy_dtype_dict[smith.get_default_dtype()]
    ...
    def wrapper(x: npt.NDArray, *args, **kwargs):
        ...
        if "mask" in keys:
            ...
            if mask is not None:
                if mask.layout != smith.strided:
                    raise AssertionError(
                        f"Expected mask.layout == smith.strided, got {mask.layout}"
                    )
        ...
        if "identity" in keys:
            ...
            if identity is not None:
                if identity.dtype is smith.bfloat16:
                    identity = identity.cpu().to(smith.float32)
                else:
...
def prod_numpy(a, *args, **kwargs):
    ...
    This is for fixing integer overflow https://github.com/blacksmith/blacksmith/issues/77320
