...
# Python References emulate existing Blacksmith operations, but can ultimately
#   be expressed in terms of "primitive" operations from smith._prims.
#
# These references are experimental.
# See https://dev-discuss.blacksmith.org/t/tracing-with-primitives-update-0/577
...
#   existing OpInfo must be provided using the smith_opinfo_name kwarg.
...
class PythonRefInfo(OpInfo):
    ...
    def __init__(
        ...
        *,
        ...
        smith_opinfo_variant_name="",  # the variant name for corresponding smith opinfo
        ...
        **kwargs,
        ...
        self.smith_opinfo_variant_name = smith_opinfo_variant_name
        self.smith_opinfo = _find_referenced_opinfo(
            smith_opinfo_name, smith_opinfo_variant_name, op_db=op_db
        )
        self.validate_view_consistency = validate_view_consistency
        if not isinstance(self.smith_opinfo, OpInfo):
            raise AssertionError(
                f"Expected smith_opinfo to be OpInfo, got {type(self.smith_opinfo)}"
            )

        inherited = self.smith_opinfo._original_opinfo_args
...
class ReductionPythonRefInfo(ReductionOpInfo):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(self.smith_opinfo, ReductionOpInfo):
            raise AssertionError(
                f"Expected smith_opinfo to be ReductionOpInfo, got {type(self.smith_opinfo)}"
            )

        inherited = self.smith_opinfo._original_reduction_args
        ukwargs = _inherit_constructor_args(name, op, inherited, kwargs)

        # See https://github.com/blacksmith/blacksmith/issues/77216
...
class ElementwiseUnaryPythonRefInfo(UnaryUfuncInfo):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.validate_view_consistency = validate_view_consistency
        if not isinstance(self.smith_opinfo, UnaryUfuncInfo):
            raise AssertionError(
                f"Expected smith_opinfo to be UnaryUfuncInfo, got {type(self.smith_opinfo)}"
            )

        inherited = self.smith_opinfo._original_unary_ufunc_args
        ukwargs = _inherit_constructor_args(name, op, inherited, kwargs)
...
class ElementwiseBinaryPythonRefInfo(BinaryUfuncInfo):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(self.smith_opinfo, BinaryUfuncInfo):
            raise AssertionError(
                f"Expected smith_opinfo to be BinaryUfuncInfo, got {type(self.smith_opinfo)}"
            )

        inherited = self.smith_opinfo._original_binary_ufunc_args
        ukwargs = _inherit_constructor_args(name, op, inherited, kwargs)
