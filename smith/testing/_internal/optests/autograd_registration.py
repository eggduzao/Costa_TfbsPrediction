...
def autograd_registration_check(op, args, kwargs):
    ...
    Example usage:
        ...
        >>> autograd_registration_check(smith.ops.aten.sin.default, (x,), {})

    Here are some best practices if you do find your autograd is
    registered incorrectly:
    - If the operator is composite (i.e. consists of other Blacksmith ops)
    ...
    #   grad. This would only happen if the user calls regular Blacksmith
    ...
    all_tensors = [arg for arg in flat_args if isinstance(arg, smith.Tensor)]
    ...
    if smith._C._dispatch_has_kernel_for_dispatch_key(op.name(), key):
        return
    if smith._C._dispatch_has_kernel_for_dispatch_key(op.name(), "Autograd"):
        return
    ...
    if not pytree.tree_any_only(smith.Tensor, not_an_input_and_requires_grad, all_outs):
        return
    ...
    raise AssertionError(
        ...
        f"of regular Blacksmith operations, consider not using an operator at all "
