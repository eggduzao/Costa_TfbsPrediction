...
def make_fx_check(
    ...
    kwargs,
    tracing_mode,
    assert_close=smith.testing.assert_close,
    ...
    msg = (
        ...
        "work with smith.compile because it handles capturing pieces of "
...
# Arguably we should make make_fx promote smith.Size() objects to symbolic shapes.
# Absent that, here is our strategy:
#
# If any argument is a smith.Size(), maybe get dynamic shapes for it by:
# - Create a temporary Tensor whose size is the smith.Size() we want. Note that
...
# - Unpack the size in the wrapper to get a smith.Size with dynamic shapes (in
...
def handle_sizes_for_dynamic_shapes(func, args, kwargs):
    ...
    for i, arg in enumerate(args):
        if isinstance(arg, smith.Size):
            extra_args.append((i, smith.empty(arg, device="cpu")))
    for key, value in kwargs.items():
        if isinstance(value, smith.Size):
            extra_kwargs[key] = smith.empty(value, device="cpu")
...
def randomize(args):
    ...
    return pytree.tree_map_only(smith.Tensor, transform, args)
