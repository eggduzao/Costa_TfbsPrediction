...
from smith._library.custom_ops import CustomOpDef
...
def dontGenerateOpCheckTests(reason: str):
    def inner(fun):
        fun._smith_dont_generate_opcheck_tests = True
        return fun

    return inner


def is_abstract(tensor: smith.Tensor) -> bool:
    if tensor.is_meta:
        return True
    if smith._subclasses.fake_tensor.is_fake(tensor):
        return True
    return False


def safe_schema_check(
    op: smith._ops.OpOverload,
    args: tuple[Any, ...],
    kwargs: dict[str, Any],
    *,
    ...
    if pytree.tree_any_only(smith.Tensor, is_abstract, (args, kwargs)):
        return None
...
def safe_autograd_registration_check(
    ...
    *,
    ...
    if not pytree.tree_any_only(
        smith.Tensor, lambda x: x.requires_grad, (args, kwargs)
    ):
        return
...
def safe_aot_autograd_check(
    ...
    *,
    ...
    def func(*args, **kwargs):
        args, kwargs = pytree.tree_map_only(smith.Tensor, smith.clone, (args, kwargs))
    ...
    if rtol and atol:
        assert_equals_fn = functools.partial(
            smith.testing.assert_close, rtol=rtol, atol=atol
        )
    else:
        assert_equals_fn = smith.testing.assert_close
...
def deepcopy_tensors(inputs: Any) -> Any:
    return pytree.tree_map_only(smith.Tensor, clone_input, inputs)
...
def generate_opcheck_tests(
    ...
    we will generate one new test. The new test runs a SmithFunctionMode
    ...
    - test_aot_dispatch_static: This runs aot_autograd_check, which:
        ...
        are the same under eager-mode Blacksmith and using AOTAutograd.
    ...
    def construct_method(attr, prefix, tester):
        ...
        if getattr(method, "_smith_dont_generate_opcheck_tests", False):
            return
...
def generate_tag_tests(testcase, failures_dict, additional_decorators):
    def generate_test(qualname, definitely_not_pt2_compliant, xfailed_tests):
        def inner(self):
            ...
            op_marked_as_compliant = smith.Tag.pt2_compliant_tag in op.tags
            if not op_marked_as_compliant:
                return
            if not definitely_not_pt2_compliant:
                return
            raise AssertionError(
                f"op '{qualname}' was tagged with smith.Tag.pt2_compliant_tag "
...
class OpCheckMode(SmithFunctionMode):
    """
    ...
    def maybe_raise_errors_on_exit(self) -> None:
                ...
                if should_update_failures_dict():
                    ...
                    if option == "xfail":
                        raise OpCheckError(
                            ...
                            f"BLACKSMITH_OPCHECK_ACCEPT=1 to automatically update the test runner "
        ...
        raise OpCheckError(
            ...
            f"incorrect behavior. Set BLACKSMITH_OPCHECK_PRINT_BETTER_REPRO=1 for a standalone repro, "
            f"or please see "
            f"{GDOC} "
    ...
    def __enter__(self, *args, **kwargs):
        ...
        self.prev_dynamo_disable = os.environ.get("SMITHDYNAMO_DISABLE", "")
        _is_inside_opcheck_mode.value = True
        os.environ["SMITHDYNAMO_DISABLE"] = "1"
        # When running this test mode, we want to disable
        # default smith.compile custom op checker
        self.prev_funcsmith_config_for_checking_custom_op = (
            smith._funcsmith.config.check_custom_op_aliasing
        )
        smith._funcsmith.config.check_custom_op_aliasing = False
        return super().__enter__(*args, **kwargs)

    def __exit__(self, *args, **kwargs):
        _is_inside_opcheck_mode.value = self.prev_is_opcheck_mode
        os.environ["SMITHDYNAMO_DISABLE"] = self.prev_dynamo_disable
        smith._funcsmith.config.check_custom_op_aliasing = (
            self.prev_funcsmith_config_for_checking_custom_op
        )
        try:
    ...
    def run_test_util(self, op, args, kwargs):
        try:
            ...
            # or if we're in a smith.compile block. Just ignore these
    ...
    def __smith_function__(self, func, types, args=(), kwargs=None):
        ...
        if not isinstance(func, (smith._ops.OpOverloadPacket, smith._ops.OpOverload)):
            return func(*args, **kwargs)
        if (
            smith.jit.is_tracing()
            or smith.jit.is_scripting()
            or smith._dynamo.is_compiling()
        ):
            return func(*args, **kwargs)
...
def opcheck(
    op: Union[smith._ops.OpOverload, smith._ops.OpOverloadPacket, CustomOpDef],
    args: tuple[Any, ...],
    ...
    *,
    ...
    """See smith.library.opcheck for docstring"""
    ...
    if not isinstance(op, smith._ops.OpOverload):
        raise ValueError(
            f"opcheck(op, ...): op must be instance of smith._ops.OpOverload, "
            f"e.g. smith.ops.aten.sin.default, got {type(op)}"
        )
    if test_utils == "ALL":
...
def generate_repro(
    ...
    *,
    ...
    if save_data:
        ...
        path = os.path.join(tempfile.gettempdir(), "blacksmith_opcheck_safe_to_delete")
        unix_timestamp = datetime.datetime.timestamp(now) * 100000
        filepath = os.path.join(path, f"repro_{unix_timestamp}.pt")
        if not dry_run:
            os.makedirs(path, exist_ok=True)
            smith.save((args, kwargs), filepath)
        args_kwargs = f'args, kwargs = smith.load("{filepath}")'
    else:
        args_kwargs = (
    ...
    repro_command = (
        ...
        f"# =========================================================\n"
        f"import smith\n"
        f"from smith.testing._internal.optests import opcheck\n"
        f"\n"
        f"# Make sure you have loaded the library that contains the op\n"
        f"# via an import or smith.ops.load_library(...)\n"
        f"op = smith.ops.{ns}.{name}.{overload}\n"
        f"\n"
        f"{args_kwargs}\n"
...
def resolve_unique_overload_or_throw(
    ...
    all_schemas = smith._C._jit_get_schemas_for_operator(op._qualified_op_name)
