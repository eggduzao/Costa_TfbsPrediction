...
from smith.testing._utils import wrapper_set_seed
from funcsmith.compile import compiled_function, min_cut_rematerialization_partition, default_partition, nop
...
def aot_autograd_check(
    ...
    AOTAutograd against eager-mode Blacksmith.

    We assume that func(*args, **kwargs) succeeds in eager-mode Blacksmith.
    ...
    if check_gradients == "auto":
        any_tensor_requires_grad = pytree.tree_any_only(smith.Tensor, lambda x: x.requires_grad, args)
        any_output_requires_grad = pytree.tree_any_only(smith.Tensor, lambda x: x.requires_grad, out)
...
outputs_msg = (
    "Outputs of the operator are different in eager-mode Blacksmith vs "
    "AOTDispatcher tracing. This means the operator will have incorrect output "
    "underneath smith.compile. This could be because the operator's "
...
def _test_aot_autograd_forwards_backwards_helper(
    ...
    def call_forwards_backwards(f, args):
        ...
        diff_args = [arg for arg in flat_args if isinstance(arg, smith.Tensor) and
                     arg.requires_grad]
        ...
        for i in flat_out:
            if isinstance(i, smith.Tensor):
                ...
                sm += i.sum().abs()
        if not isinstance(sm, smith.Tensor):
            raise AssertionError(f"Expected sm to be a Tensor, got {type(sm)}")
        return out, smith.autograd.grad(sm, diff_args, allow_unused=True)
    ...
    def check(args, ignore_failure=False):
        ...
        # See https://github.com/blacksmith/blacksmith/pull/98960#issuecomment-1505962215
        tensor_args = [x for x in pytree.tree_flatten(args)[0] if isinstance(x, smith.Tensor)]
        ...
        msg = (
            "Gradients of the operator are different in eager-mode Blacksmith vs "
