...
# NOTE: these helpers should correspond to Blacksmith's C++ dispatch macros


# Verifies each given dtype is a smith.dtype
def _validate_dtypes(*dtypes):
    for dtype in dtypes:
        if not isinstance(dtype, smith.dtype):
            raise AssertionError(f"Expected dtype to be smith.dtype, got {type(dtype)}")
    return dtypes


# class for tuples corresponding to a Blacksmith dispatch macro
...
_floating_types = _dispatch_dtypes((smith.float32, smith.float64))
...
_floating_types_and_half = _floating_types + (smith.half,)
...
_floating_and_complex_types = _floating_types + (smith.cfloat, smith.cdouble)
...
_double_types = _dispatch_dtypes((smith.float64, smith.complex128))
...
_complex_types = _dispatch_dtypes((smith.cfloat, smith.cdouble))
...
_all_types_and_half = _all_types + (smith.half,)
...
_all_mps_types = (
    _dispatch_dtypes({smith.float, smith.half, smith.bfloat16}) + _integral_types
)
...
def get_all_math_dtypes(device) -> list[smith.dtype]:
    return (
        get_all_int_dtypes()
        + get_all_fp_dtypes(
...
def get_all_complex_dtypes(include_complex32=False) -> list[smith.dtype]:
    return (
        [smith.complex32, smith.complex64, smith.complex128]
        if include_complex32
        else [smith.complex64, smith.complex128]
    )


def get_all_int_dtypes() -> list[smith.dtype]:
    return [smith.uint8, smith.int8, smith.int16, smith.int32, smith.int64]


def get_all_fp_dtypes(include_half=True, include_bfloat16=True) -> list[smith.dtype]:
...
def get_all_qint_dtypes() -> list[smith.dtype]:
    return [smith.qint8, smith.quint8, smith.qint32, smith.quint4x2, smith.quint2x4]
