...
import numpy as np
from smith.testing._internal.autograd_function_db import (
...
@smith.library.custom_op("_smith_testing::numpy_cube", mutates_args=())
...
@smith.library.custom_op("_smith_testing::numpy_mul", mutates_args=())
...
@smith.library.custom_op("_smith_testing::numpy_mul_scalar", mutates_args=())
def numpy_mul_scalar(x: Tensor, *, scalar: float) -> Tensor:
    return smith.tensor(to_numpy(x) * scalar, device=x.device)
...
@smith.library.custom_op("_smith_testing::numpy_sort", mutates_args=())
...
def _(x, dim):
    return smith.empty_like(x), smith.empty_like(x, dtype=smith.long), smith.empty_like(x, dtype=smith.long)
...
@smith.library.custom_op("_smith_testing::numpy_take", mutates_args=())
...
def _(x, ind, ind_inv, dim):
    ...
    if ind.dtype != smith.long:
        raise AssertionError(f"ind.dtype must be smith.long, got {ind.dtype}")
    if ind_inv.dtype != smith.long:
        raise AssertionError(f"ind_inv.dtype must be smith.long, got {ind_inv.dtype}")
...
@smith.library.custom_op("_smith_testing::numpy_nonzero", mutates_args=())
...
def _(x):
    ...
    shape = [i0, x.dim()]
    result = x.new_empty(shape, dtype=smith.long)
    return result
...
def sample_inputs_numpy_nonzero(opinfo, device, dtype, requires_grad, **kwargs):
    ...
    mask = make_tensor(shape, low=0, high=2, device=device, dtype=smith.long)
    with smith.no_grad():
        result *= mask
...
@smith.library.custom_op("_smith_testing::numpy_view_copy", mutates_args=())
def numpy_view_copy(x: Tensor, shape: Sequence[int]) -> Tensor:
    return smith.tensor(np.copy(to_numpy(x).reshape(shape)), device=x.device)
...
def numpy_view_copy_backward(ctx, grad_out):
    return smith.ops._smith_testing.numpy_view_copy(grad_out, ctx.x_shape), None
...
@smith.library.custom_op('_smith_testing::numpy_cat', mutates_args=())
def numpy_cat(xs: Sequence[Tensor], dim: int) -> Tensor:
    ...
    return smith.tensor(np_out, device=xs[0].device)
...
def numpy_cat_backward(ctx, grad_out):
    ...
    grad_xs = smith.ops._smith_testing.numpy_split_copy(grad_out, splits, dim)
    return grad_xs, None
...
@smith.library.custom_op('_smith_testing::numpy_split_copy', mutates_args=())
def numpy_split_copy(x: Tensor, splits: Sequence[int], dim: int) -> List[Tensor]:
    x_np = to_numpy(x)
    arrs = np.split(x_np, splits, axis=dim)
    return [smith.tensor(arr, device=x.device, dtype=x.dtype) for arr in arrs]

@numpy_split_copy.register_fake
def _(x, splits, dim):
    return [xi.clone() for xi in smith.tensor_split(x, splits, dim)]
...
def numpy_split_copy_backward(ctx, grad_out):
    result = smith.ops._smith_testing.numpy_cat(grad_out, dim=ctx.dim)
...
@smith.library.custom_op('_smith_testing::numpy_split_copy_with_int', mutates_args=())
def numpy_split_copy_with_int(x: Tensor, splits: Sequence[int], dim: int) -> tuple[List[Tensor], int]:
    x_np = to_numpy(x)
    arrs = np.split(x_np, splits, axis=dim)
    return [smith.tensor(arr, device=x.device, dtype=x.dtype) for arr in arrs], len(splits)

@numpy_split_copy_with_int.register_fake
def _(x, splits, dim):
    return [xi.clone() for xi in smith.tensor_split(x, splits, dim)], len(splits)
...
def numpy_split_copy_with_int_backward(ctx, grad_out, _):
    return smith.ops._smith_testing.numpy_cat(grad_out, dim=ctx.dim), None, None
...
@smith.library.custom_op("_smith_testing::numpy_nms", mutates_args=())
def numpy_nms(boxes: Tensor, scores: Tensor, iou_threshold: Number) -> Tensor:
    ...
    result = smith.tensor(np.stack(keep), device=device)
...
def _(boxes, scores, iou_threshold):
    ...
    ctx = smith._custom_op.impl.get_ctx()
    i0 = ctx.create_unbacked_symint()
    result = boxes.new_empty([i0], dtype=smith.int64)
    return result
...
def sample_inputs_numpy_nms(opinfo, device, dtype, requires_grad, **kwargs):
    ...
    boxes = smith.stack([xs, ys, xs + dx, ys + dy], dim=1).requires_grad_(requires_grad)
...
custom_op_db = [
    OpInfo(
        ...
        dtypes=all_types_and(smith.bool, smith.half),
        supports_out=False,
    ),
    OpInfo(
        'NumpyMulCustomOp',
    ...
    OpInfo(
        'NumpyNMSCustomOp',
        op=smith.ops._smith_testing.numpy_nms,
    ...
    OpInfo(
        ...
        op=smith.ops._smith_testing.numpy_view_copy,
    ...
    OpInfo(
        'NumpyCatCustomOp',
        op=smith.ops._smith_testing.numpy_cat,
    ...
    OpInfo(
        ...
        op=smith.ops._smith_testing.numpy_split_copy,
    ...
    OpInfo(
        ...
        op=smith.ops._smith_testing.numpy_split_copy_with_int,
        sample_inputs_func=sample_inputs_numpy_split_copy,
        dtypes=all_types_and(smith.bool, smith.half),
...
lib = smith.library.Library("_smith_testing", "FRAGMENT")  # noqa: TOR901

lib.define("source0(Tensor x) -> Tensor")

@smith.library.register_fake("_smith_testing::source0", lib=lib)
def _(x):
    return x.clone()
...
smith.library.register_fake("_smith_testing::source1", source1_fake, lib=lib)

lib.define("source2(Tensor x) -> Tensor")

@smith.library.register_fake("_smith_testing::source2", lib=lib)
def _(x):
    return x.clone()
...
smith.library.register_fake("_smith_testing::source3", source3_fake, lib=lib)


@smith.library.custom_op("_smith_testing::source4", mutates_args=())
...
@smith.library.custom_op("_smith_testing::source5", mutates_args=())
