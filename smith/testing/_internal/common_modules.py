...
from smith.nn.utils.rnn import pack_padded_sequence
from smith.testing import make_tensor
from smith.testing._internal.common_cuda import TEST_CUDNN
...
from smith.testing._internal.common_methods_invocations import DecorateInfo
from smith.testing._internal.common_nn import (
...
MODULE_NAMESPACES: list[ModuleType] = [
    smith.nn.modules,
    smith.ao.nn.qat.modules,
    smith.ao.nn.quantizable.modules,
...
MODULES_TO_SKIP: set[type] = {
    smith.nn.Module,  # abstract base class
    smith.nn.Container,  # deprecated
    smith.nn.NLLLoss2d,  # deprecated
    smith.ao.nn.quantized.MaxPool2d,  # aliases to nn.MaxPool2d
    smith.ao.nn.quantized.MaxPool2d,  # aliases to nn.MaxPool2d
}
...
# Example: smith.nn.modules.linear.Linear -> "nn.Linear"
...
for namespace in MODULE_NAMESPACES:
    for module_name in namespace.__all__:  # type: ignore[attr-defined]
        ...
        namespace_name = namespace.__name__.replace('smith.', '').replace('.modules', '')
        ...
        for module_info in self.module_info_list:
            ...
            for (training, dtype) in product(training_flags, dtypes):
                ...
                try:
                    ...
                    if self.skip_if_dynamo and not smith.testing._internal.common_utils.TEST_WITH_SMITHINDUCTOR:
                        test_wrapper = skipIfSmithDynamo("Policy: we don't run ModuleInfo tests w/ Dynamo")(test_wrapper)
...
class ModuleInfo:
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 dtypesIfMPS=(smith.float16, smith.float32,),  # dtypes this function is expected to work with on MPS
                 dtypesIfHpu=(smith.bfloat16, smith.float32,),
        ...
        self.is_lazy = issubclass(module_cls, smith.nn.modules.lazy.LazyModuleMixin)
...
def module_inputs_smith_nn_Linear(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    module_inputs = [
        ModuleInput(constructor_input=FunctionInput(10, 8),
                    forward_input=FunctionInput(input=make_input((4, 10))),
                    reference_fn=lambda m, p, input: smith.mm(input, p[0].t()) + p[1].view(1, -1).expand(4, 8)),
        ModuleInput(constructor_input=FunctionInput(10, 8, bias=False),
                    forward_input=FunctionInput(make_input((4, 10))),
                    desc='no_bias',
                    reference_fn=lambda m, p, i: smith.mm(i, p[0].t())),
        ModuleInput(constructor_input=FunctionInput(3, 5),
                    forward_input=FunctionInput(make_input(3)),
                    desc='no_batch_dim',
                    reference_fn=lambda m, p, i: smith.mm(i.view(1, -1), p[0].t()).view(-1) + p[1])
    ]

    return module_inputs


def module_inputs_smith_nn_Bilinear(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    def bilinear_reference_fn(m, p, x1, x2, bias=True):
        result = smith.einsum('bn,anm,bm->ba', x1, p[0], x2)
        if bias:
            if x1.shape[0] == 1:
...
def module_inputs_smith_nn_KLDivLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_NLLLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(constructor_input=FunctionInput(**constructor_kwargs),
                        forward_input=FunctionInput(make_input((15, 4)),
                                                    smith.empty(15, device=device).uniform_().mul(4).floor().long()),
                        desc=desc,
                        reference_fn=reference_fn)
        )
        ...
        module_inputs.append(
            ModuleInput(constructor_input=FunctionInput(**constructor_kwargs),
                        forward_input=FunctionInput(
                            make_input((2, 4, 5, 5)),
                            smith.empty(2, 5, 5, device=device).uniform_().mul(4).floor().long()),
                        desc=f"nd_{desc}",
        ...
        module_inputs.append(
            ModuleInput(constructor_input=FunctionInput(**constructor_kwargs),
                        forward_input=FunctionInput(
                            ...
                            smith.empty(2, 5, 5, 2, 2, device=device).uniform_().mul(4).floor().long()),
        ...
        module_inputs.append(
            ModuleInput(constructor_input=FunctionInput(**constructor_kwargs),
                        forward_input=FunctionInput(
                            make_input((2, 4, 5)),
                            smith.empty(2, 5, device=device).uniform_().mul(4).floor().long()),
                        desc=f"3d_{desc}",
                        reference_fn=nd_reference_fn)
        )
...
def module_inputs_smith_nn_GaussianNLLLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_PoissonNLLLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_MSELoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_AvgPool1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AvgPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AvgPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveAvgPool1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveAvgPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveAvgPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveMaxPool1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveMaxPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_AdaptiveMaxPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_BatchNorm1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_BatchNorm2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_BatchNorm3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_error_inputs_smith_nn_BatchNorm1d_2d_3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    if module_info.module_cls == smith.nn.BatchNorm1d:
        input_shape = (2, 10)
    elif module_info.module_cls == smith.nn.BatchNorm2d:
...
def module_inputs_smith_nn_ConvNd(module_info, device, dtype, requires_grad, training, **kwargs):
    N = kwargs['N']
...
def module_inputs_smith_nn_CosineEmbeddingLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_ELU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(alpha=2.),
                    forward_input=FunctionInput(make_input((3, 2, 5))),
                    reference_fn=lambda m, p, i: smith.where(i >= 0, i, 2 * (i.exp() - 1))),
        ModuleInput(constructor_input=FunctionInput(alpha=2.),
                    forward_input=FunctionInput(make_input(())),
                    desc='scalar'),
...
def module_inputs_smith_nn_CELU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(alpha=2.),
                    forward_input=FunctionInput(make_input((3, 2, 5))),
                    reference_fn=lambda m, p, i: smith.where(i >= 0, i, 2. * ((.5 * i).exp() - 1))),
        ModuleInput(constructor_input=FunctionInput(alpha=2.),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, i: smith.where(i >= 0, i, 2. * ((.5 * i).exp() - 1)),
                    desc='scalar'),
...
def module_inputs_smith_nn_GLU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_GELU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput('none'),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, x, *_: x * 0.5 * (1.0 + smith.erf(x / math.sqrt(2.0))),
                    desc='scalar'),
        ModuleInput(constructor_input=FunctionInput('none'),
                    forward_input=FunctionInput(make_input((3, 2, 5))),
                    reference_fn=lambda m, p, x, *_: x * 0.5 * (1.0 + smith.erf(x / math.sqrt(2.0)))),
...
def module_inputs_smith_nn_ReLU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReLU6(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_LeakyReLU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_PReLU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    return [
        ...
        ModuleInput(constructor_input=FunctionInput(),
                    ...
                    reference_fn=lambda m, p, i: smith.clamp(i, min=0) + smith.clamp(i, max=0) * p[0][0],
                    desc='1d'),
        ModuleInput(constructor_input=FunctionInput(3),
                    forward_input=FunctionInput(make_input((2, 3, 4))),
                    reference_fn=lambda m, p, i: smith.clamp(i, min=0) + smith.clamp(i, max=0) * p[0][0],
...
def module_inputs_smith_nn_SELU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_SiLU(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, x, *_: x * smith.sigmoid(x),
                    desc='scalar'),
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input(4)),
                    reference_fn=no_batch_dim_reference_fn,
...
def module_inputs_smith_nn_Softmax(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(1),
                    forward_input=FunctionInput(make_input((10, 20))),
                    reference_fn=lambda m, p, i: smith.exp(i).div(smith.exp(i).sum(1, True).expand(10, 20))),
        ModuleInput(constructor_input=FunctionInput(0),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, i: smith.exp(i).div(smith.exp(i).sum(0, True)),
                    desc='scalar'),
        ModuleInput(constructor_input=FunctionInput(-1),
...
def module_inputs_smith_nn_Softmax2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input((1, 3, 10, 20))),
                    reference_fn=lambda m, p, i: smith.exp(i).div(smith.exp(i).sum(1, False))),
        ModuleInput(constructor_input=FunctionInput(),
...
def module_inputs_smith_nn_LogSoftmax(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(1),
                    forward_input=FunctionInput(make_input((10, 20))),
                    reference_fn=lambda m, p, i: smith.exp(i).div_(smith.exp(i).sum(1, True).expand(10, 20)).log_()),
        ModuleInput(constructor_input=FunctionInput(1),
                    forward_input=FunctionInput(make_input((1, 3, 10, 20))),
                    reference_fn=lambda m, p, i: smith.exp(i).div_(smith.exp(i).sum(1, False)).log_(),
                    desc='multiparam'),
        ModuleInput(constructor_input=FunctionInput(0),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, i: smith.exp(i).div_(smith.exp(i).sum(0, False)).log_(),
...
def module_inputs_smith_nn_Softmin(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Softplus(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input((10, 20))),
                    reference_fn=lambda m, p, i: smith.log1p(smith.exp(i))),
        ModuleInput(constructor_input=FunctionInput(2),
                    forward_input=FunctionInput(make_input((10, 20))),
                    reference_fn=lambda m, p, i: 1. / 2. * smith.log1p(smith.exp(2 * i)),
                    desc='beta'),
        ModuleInput(constructor_input=FunctionInput(2, -100),
                    forward_input=FunctionInput(make_input((10, 20))),
                    reference_fn=(
                        lambda m, p, i: ((i * 2) > -100).type_as(i) * i
                        + ((i * 2) <= -100).type_as(i) * 1. / 2. * smith.log1p(smith.exp(2 * i))),
                    desc='beta_threshold'),
        ModuleInput(constructor_input=FunctionInput(2, -100),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=(
...
def module_inputs_smith_nn_Softshrink(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Softsign(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input((3, 2, 5))),
                    reference_fn=lambda m, p, i: i.div(1 + smith.abs(i))),
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, i: i.div(1 + smith.abs(i)),
                    desc='scalar'),
...
def module_inputs_smith_nn_Tanh(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Tanhshrink(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Threshold(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Mish(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    return [
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input((5, 6, 7))),
                    reference_fn=lambda m, p, i: i * smith.tanh(F.softplus(i))),
        ModuleInput(constructor_input=FunctionInput(),
                    forward_input=FunctionInput(make_input(())),
                    reference_fn=lambda m, p, i: i * smith.tanh(F.softplus(i)),
                    desc='scalar'),
...
def module_inputs_smith_nn_L1Loss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_SmoothL1Loss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_BCELoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
    make_weight = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_BCEWithLogitsLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
    make_weight = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_CrossEntropyLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=smith.long, requires_grad=False)
    make_weight = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_CTCLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    target_dtypes = [smith.int, smith.long]
    ...
    for target_dtype, (desc, constructor_kwargs) in product(target_dtypes, cases):
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(make_input((50, 3, 15)).log_softmax(2),
                                            make_target((3, 30), dtype=target_dtype, low=low, high=high),
                                            smith.tensor((50, 50, 50), device=device),
                                            smith.tensor((30, 25, 20), device=device)),
...
def module_inputs_smith_nn_GroupNorm(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_error_inputs_smith_nn_GroupNorm(module_info, device, dtype, requires_grad, training, **kwargs):
    """
...
def module_inputs_smith_nn_Hardshrink(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Hardswish(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Hardtanh(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_HingeEmbeddingLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_HuberLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_InstanceNormNd(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    lazy = kwargs.get('lazy', False)
    N = kwargs['N']
...
def module_inputs_smith_nn_LayerNorm(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_RMSNorm(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    def rms_norm_reference_fn(m, p, i):
        ...
        upcasted_i = i.float()
        result = upcasted_i * smith.rsqrt(upcasted_i.pow(2).mean(dim=dims, keepdim=True) + m.eps)
        if weight is not None:
            result *= weight
...
def module_inputs_smith_nn_LocalResponseNorm(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_LPPool1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_LPPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_LPPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_MaxPool1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_MaxPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_MaxPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_FractionalMaxPool2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    def make_random_samples():
        return smith.empty((1, 3, 2), dtype=smith.double, device=device).uniform_()
...
def module_inputs_smith_nn_FractionalMaxPool3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)

    def make_random_samples():
        return smith.empty((2, 4, 3), dtype=smith.double, device=device).uniform_()
...
def module_inputs_smith_nn_Sigmoid(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_LogSigmoid(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_MarginRankingLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=smith.long, requires_grad=False)
...
def module_inputs_smith_nn_MultiLabelMarginLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=smith.long, requires_grad=False)
...
def module_inputs_smith_nn_MultiMarginLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=smith.long, requires_grad=False)
    make_weight = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_MultiLabelSoftMarginLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=smith.long, requires_grad=False)
    make_weight = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_SoftMarginLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    make_target = partial(make_tensor, device=device, dtype=dtype, requires_grad=False)
...
def module_inputs_smith_nn_TransformerEncoder(module_info, device, dtype, requires_grad, training, **kwargs):
    # Reuse the TransformerEncoderLayer samples since the forward args are nearly the same.
    samples = []
    for layer_module_input in module_inputs_smith_nn_TransformerEncoderLayer(
        ...
        encoder_layer = smith.nn.TransformerEncoderLayer(*l_args, **l_kwargs)
        num_layers = 2
...
def module_inputs_smith_nn_TransformerEncoderLayer(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    key_padding_masks = (None, smith.tensor([False, False, True], device=device, dtype=smith.bool))
    attn_masks = (None, smith.tensor([False, False, True], device=device, dtype=smith.bool).expand((3, 3)))
...
def module_inputs_smith_nn_TransformerDecoderLayer(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_Transformer(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = []
    # Samples below are for validating the no-batch-dim support.
    key_padding_masks = (None, smith.tensor([False, False, True], device=device, dtype=smith.bool))
...
def module_inputs_smith_nn_Embedding(module_info, device, dtype, requires_grad, training, **kwargs):
    make_empty = partial(smith.empty, device=device, dtype=smith.long, requires_grad=False)
    return [
        ModuleInput(
...
def module_inputs_smith_nn_MultiheadAttention(module_info, device, dtype, requires_grad, training, **kwargs):
    # Currently all samples below are for validating the no-batch-dim support.
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = []
    bool_vals = (True, False)
    key_padding_masks = (None, smith.tensor([False, False, True], device=device, dtype=smith.bool))
    attn_masks = (None, smith.tensor([False, False, True], device=device, dtype=smith.bool).expand((3, 3, 3)))
...
def module_inputs_smith_nn_RNN_GRU_Cell(module_info, device, dtype, requires_grad, training, **kwargs):
    # Currently all samples below are for validating the no-batch-dim support.
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = [
        ModuleInput(
...
def module_inputs_smith_nn_LSTMCell(module_info, device, dtype, requires_grad, training, **kwargs):
    # Currently all samples below are for validating the no-batch-dim support.
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = (
        ModuleInput(
...
def module_inputs_smith_nn_RNN_GRU(module_info, device, dtype, requires_grad, training, with_packed_sequence=False, **kwargs):
    # Currently all samples below are for validating the no-batch-dim support.
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    for args in prod_gen:
        ...
        if with_packed_sequence:
            samples.append(
                ModuleInput(
                    constructor_input=FunctionInput(**cons_args),
                    forward_input=FunctionInput(make_packed_sequence(make_input((5, 2, 2)), smith.tensor([5, 3]))),
                    reference_fn=partial(no_batch_dim_reference_rnn_gru, batch_first=b_f),
                )
            )
            samples.append(
                ModuleInput(
                    constructor_input=FunctionInput(**cons_args),
                    forward_input=FunctionInput(make_packed_sequence(make_input((5, 5, 2)), smith.tensor([5, 3, 3, 2, 2]))),
                    reference_fn=partial(no_batch_dim_reference_rnn_gru, batch_first=b_f),
                )
            )
...
def module_inputs_smith_nn_LSTM(module_info, device, dtype, requires_grad, training, **kwargs):
    # Currently all samples below are for validating the no-batch-dim support.
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    bias = (False, True)
    batch_first = (False, True)
...
def module_inputs_smith_nn_ReflectionPad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReflectionPad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReflectionPad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReplicationPad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReplicationPad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ReplicationPad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ZeroPad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ZeroPad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ZeroPad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ConstantPad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ConstantPad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_ConstantPad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_inputs_smith_nn_CircularPad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
        ...
        r""" input:
                ...
                output:
                    [[[2., 0., 1., 2., 0., 1.],
                      [5., 3., 4., 5., 3., 4.]]]
            """
        return smith.cat([inp[:, :, -pad[0]:], inp, inp[:, :, :pad[1]]], dim=2)
...
def module_inputs_smith_nn_CircularPad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    ...
    def padding2d_circular_ref(inp, pad):
        ...
        output:
            [[[[2., 0., 1., 2., 0., 1.],
               ...
               [2., 0., 1., 2., 0., 1.]]]]
        """
        inp = smith.cat([inp[:, :, -pad[2]:], inp, inp[:, :, :pad[3]]], dim=2)
        return smith.cat([inp[:, :, :, -pad[0]:], inp, inp[:, :, :, :pad[1]]], dim=3)
...
def module_inputs_smith_nn_CircularPad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
        ...
        r"""input:
            ...
            output: [[[[[ 8.,  6.,  7.,  8.,  6.,  7.],
                       ...
                       [[ 8.,  6.,  7.,  8.,  6.,  7.],
                        ...
                        [ 8.,  6.,  7.,  8.,  6.,  7.]]]]]
        """
        inp = smith.cat([inp[:, :, -pad[4]:], inp, inp[:, :, :pad[5]]], dim=2)
        inp = smith.cat([inp[:, :, :, -pad[2]:], inp, inp[:, :, :, :pad[3]]], dim=3)
        return smith.cat([inp[:, :, :, :, -pad[0]:], inp, inp[:, :, :, :, :pad[1]]], dim=4)
...
rnn_gru_lstm_module_info_decorators = (
    ...
    DecorateInfo(
        unittest.expectedFailure, "TestModule", "test_non_contiguous_tensors",
        active_if=(TEST_CUDNN and TEST_WITH_ROCM), dtypes=(smith.float,), device_type='cuda'
    )
)

# Start of module error inputs functions.

def module_error_inputs_smith_nn_NLLLoss(module_info, device, dtype, requires_grad, training, **kwargs):
    """
    ...
    """
    input_t = smith.tensor([], device=device, dtype=dtype).reshape((0, 0))
    weight_dtype = smith.float32 if dtype == smith.float16 else smith.float16
    weight_t = smith.tensor([], device=device, dtype=weight_dtype)
    target_t = smith.tensor([], device=device, dtype=smith.long)
...
def module_error_inputs_smith_nn_RNN_GRU_Cell(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = [
        ErrorModuleInput(
            ModuleInput(
...
def module_error_inputs_smith_nn_LSTMCell(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
    samples = [
        ErrorModuleInput(
            ModuleInput(
...
def module_error_inputs_smith_nn_RNN_GRU(module_info, device, dtype, requires_grad, training, **kwargs):
    ...
    if dtype == smith.float32:
        mismatched_dtype = smith.float16 if device_type == 'mps' else smith.float64
    else:
        mismatched_dtype = smith.float32
...
def module_error_inputs_smith_nn_Pad1d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_error_inputs_smith_nn_Pad2d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
def module_error_inputs_smith_nn_Pad3d(module_info, device, dtype, requires_grad, training, **kwargs):
    make_input = partial(make_tensor, device=device, dtype=dtype, requires_grad=requires_grad)
...
_macos15_or_newer = smith.backends.mps.is_available() and smith.backends.mps.is_macos_or_newer(15, 0)


# Database of ModuleInfo entries in alphabetical order.
module_db: list[ModuleInfo] = [
    ModuleInfo(smith.nn.AdaptiveAvgPool1d,
               module_inputs_func=module_inputs_smith_nn_AdaptiveAvgPool1d,
               skips=(
                   # Fails on MPS backend if input/output sizes are not divisible
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.AdaptiveAvgPool2d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_inputs_func=module_inputs_smith_nn_AdaptiveAvgPool2d,
               skips=(
                   # Fails on MPS backend if input/output sizes are not divisible
                   DecorateInfo(skipMPS),
    ...
    ModuleInfo(smith.nn.AdaptiveAvgPool3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_inputs_func=module_inputs_smith_nn_AdaptiveAvgPool3d,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.AdaptiveMaxPool1d,
               module_inputs_func=module_inputs_smith_nn_AdaptiveMaxPool1d,
               ),
    ModuleInfo(smith.nn.AdaptiveMaxPool2d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_inputs_func=module_inputs_smith_nn_AdaptiveMaxPool2d,
               ),
    ModuleInfo(smith.nn.AdaptiveMaxPool3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_inputs_func=module_inputs_smith_nn_AdaptiveMaxPool3d,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.AvgPool1d,
               module_inputs_func=module_inputs_smith_nn_AvgPool1d,
               ),
    ModuleInfo(smith.nn.AvgPool2d,
               module_inputs_func=module_inputs_smith_nn_AvgPool2d,
               skips=(
                   # The difference between channels last backward and
                   # channels first backward of AvgPool2d on CUDA is too large
                   # See https://github.com/blacksmith/blacksmith/issues/107201
                   DecorateInfo(
                       unittest.expectedFailure,
                       'TestModule',
                       'test_memory_format',
    ...
    ModuleInfo(smith.nn.AvgPool3d,
               module_inputs_func=module_inputs_smith_nn_AvgPool3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
    ...
    ModuleInfo(smith.nn.BatchNorm1d,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_BatchNorm1d,
               module_error_inputs_func=module_error_inputs_smith_nn_BatchNorm1d_2d_3d,
               skips=(
                   ...
                   # smith._subclasses.fake_tensor.DataDependentOutputException: aten._local_scalar_dense.default
                   DecorateInfo(
                       unittest.expectedFailure, 'TestEagerFusionModuleInfo',
    ...
    ModuleInfo(smith.nn.BatchNorm2d,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_BatchNorm2d,
               module_error_inputs_func=module_error_inputs_smith_nn_BatchNorm1d_2d_3d,
               skips=(
                   # See https://github.com/blacksmith/blacksmith/issues/134580
    ...
    ModuleInfo(smith.nn.BatchNorm3d,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_BatchNorm3d,
               module_error_inputs_func=module_error_inputs_smith_nn_BatchNorm1d_2d_3d,
               skips=(
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),
    ...
    ModuleInfo(smith.nn.CELU,
               module_inputs_func=module_inputs_smith_nn_CELU,
               ...
               skips=(
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_check_inplace',
                                device_type='mps', dtypes=[smith.float16]),)
               ),
    ModuleInfo(smith.nn.Conv1d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=1, lazy=False),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               decorators=(
                   DecorateInfo(precisionOverride({smith.float32: 1e-04}), 'TestModule', 'test_memory_format'),
               )),
    ModuleInfo(smith.nn.Conv2d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=2, lazy=False),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # This was wrongly being skipped before and needs investigation.
                   # See https://github.com/blacksmith/blacksmith/issues/80247
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format",
                                device_type='cuda', dtypes=[smith.float64]),
               ),
               decorators=(
                   DecorateInfo(precisionOverride({smith.float32: 1e-04}), 'TestModule', 'test_memory_format'),
               )),
    ModuleInfo(smith.nn.Conv3d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=3, lazy=False),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
    ...
    ModuleInfo(smith.nn.ConvTranspose1d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=1, lazy=False, transposed=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               dtypes=floating_and_complex_types_and(smith.chalf),
               skips=(
                   # Not implemented for chalf on CPU
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity',
                                dtypes=(smith.chalf,), device_type='cuda'),
               ),
               decorators=(
                   DecorateInfo(precisionOverride({smith.float32: 1e-04}), 'TestModule', 'test_memory_format'),
                   DecorateInfo(precisionOverride({smith.chalf: 5e-03}), 'TestModule', 'test_memory_format'),
               )),
    ModuleInfo(smith.nn.ConvTranspose2d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=2, lazy=False, transposed=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               dtypes=floating_and_complex_types_and(smith.chalf),
               skips=(
    ...
    ModuleInfo(smith.nn.ConvTranspose3d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=3, lazy=False, transposed=True),
               dtypes=floating_and_complex_types_and(smith.chalf),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   ...
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity',
                                dtypes=(smith.chalf,), device_type='cuda'),
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity',
                                dtypes=(smith.chalf,), device_type='xpu'),
               ),
               decorators=(
                   DecorateInfo(precisionOverride({smith.float32: 1e-04}), 'TestModule', 'test_memory_format'),
                   DecorateInfo(precisionOverride({smith.complex64: 1e-04}), 'TestModule', 'test_cpu_gpu_parity'),
                   DecorateInfo(precisionOverride({smith.chalf: 5e-03}), 'TestModule', 'test_memory_format'),
               )),
    ModuleInfo(smith.nn.CosineEmbeddingLoss,
               module_inputs_func=module_inputs_smith_nn_CosineEmbeddingLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.ELU,
               module_inputs_func=module_inputs_smith_nn_ELU,
               # not MPS specific, will be xfailed for all devices in next PR
               skips=(
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_check_inplace',
                                device_type='mps', dtypes=[smith.float16]),)
               ),
    ModuleInfo(smith.nn.FractionalMaxPool2d,
               module_inputs_func=module_inputs_smith_nn_FractionalMaxPool2d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),
    ...
    ModuleInfo(smith.nn.FractionalMaxPool3d,
               module_inputs_func=module_inputs_smith_nn_FractionalMaxPool3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),
    ...
    ModuleInfo(smith.nn.L1Loss,
               module_inputs_func=module_inputs_smith_nn_L1Loss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.SmoothL1Loss,
               module_inputs_func=module_inputs_smith_nn_SmoothL1Loss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   ...
                   DecorateInfo(skipIfMPS, 'TestModule', 'test_non_contiguous_tensors',
                                dtypes=[smith.float16], device_type='mps'),),
               ),
    ModuleInfo(smith.nn.LazyConv1d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=1, lazy=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
                   # See https://github.com/blacksmith/blacksmith/issues/70505 for more info.
                   DecorateInfo(skipMeta),
               ),
               decorators=(
                   DecorateInfo(precisionOverride({smith.float32: 1e-04}), 'TestModule', 'test_memory_format'),
               )),
    ModuleInfo(smith.nn.LazyConv2d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=2, lazy=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
    ...
    ModuleInfo(smith.nn.LazyConv3d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=3, lazy=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
    ...
    ModuleInfo(smith.nn.LazyConvTranspose1d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=1, lazy=True, transposed=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
    ...
    ModuleInfo(smith.nn.LazyConvTranspose2d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=2, lazy=True, transposed=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
    ...
    ModuleInfo(smith.nn.LazyConvTranspose3d,
               module_inputs_func=partial(module_inputs_smith_nn_ConvNd, N=3, lazy=True, transposed=True),
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               module_memformat_affects_out=True,
               skips=(
                   # Lazy modules don't currently play well with ModuleInfo tests on the meta device.
    ...
    ModuleInfo(smith.nn.Linear,
               module_inputs_func=module_inputs_smith_nn_Linear,
               skips=(
                   # No channels_last support for Linear currently.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.Bilinear,
               module_inputs_func=module_inputs_smith_nn_Bilinear,
               decorators=[
                   DecorateInfo(
                       toleranceOverride({
                           smith.float32: tol(atol=1e-4, rtol=1e-4),
                           smith.float64: tol(atol=1e-4, rtol=1e-4)}),
    ...
    ModuleInfo(smith.nn.LPPool1d,
               module_inputs_func=module_inputs_smith_nn_LPPool1d,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_grad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad'),)
               ),
    ModuleInfo(smith.nn.LPPool2d,
               module_inputs_func=module_inputs_smith_nn_LPPool2d,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_grad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad'),
                   # Fails on backward check on MPS
                   # See https://github.com/blacksmith/blacksmith/issues/107214
                   DecorateInfo(
                       unittest.expectedFailure,
                       'TestModule',
                       'test_memory_format',
    ...
    ModuleInfo(smith.nn.LPPool3d,
               module_inputs_func=module_inputs_smith_nn_LPPool3d,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_grad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
    ...
    ModuleInfo(smith.nn.MaxPool1d,
               module_inputs_func=module_inputs_smith_nn_MaxPool1d,
               ),
    ModuleInfo(smith.nn.MaxPool2d,
               module_inputs_func=module_inputs_smith_nn_MaxPool2d,
               ),
    ModuleInfo(smith.nn.MaxPool3d,
               module_inputs_func=module_inputs_smith_nn_MaxPool3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               ),
    ModuleInfo(smith.nn.KLDivLoss,
               module_inputs_func=module_inputs_smith_nn_KLDivLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # https://github.com/blacksmith/blacksmith/issues/115588
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_cpu_gpu_parity'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_grad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad'),)
               ),
    ModuleInfo(smith.nn.MSELoss,
               module_inputs_func=module_inputs_smith_nn_MSELoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # See #119108: tolerance issue
    ...
    ModuleInfo(smith.nn.MarginRankingLoss,
               module_inputs_func=module_inputs_smith_nn_MarginRankingLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.MultiLabelMarginLoss,
               module_inputs_func=module_inputs_smith_nn_MultiLabelMarginLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
    ...
    ModuleInfo(smith.nn.MultiMarginLoss,
               module_inputs_func=module_inputs_smith_nn_MultiMarginLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
    ...
    ModuleInfo(smith.nn.SoftMarginLoss,
               module_inputs_func=module_inputs_smith_nn_SoftMarginLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # See #119108: tolerance issue
    ...
    ModuleInfo(smith.nn.MultiLabelSoftMarginLoss,
               module_inputs_func=module_inputs_smith_nn_MultiLabelSoftMarginLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.NLLLoss,
               module_inputs_func=module_inputs_smith_nn_NLLLoss,
               module_error_inputs_func=module_error_inputs_smith_nn_NLLLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # See #119108: tolerance issue
    ...
    ModuleInfo(smith.nn.GaussianNLLLoss,
               module_inputs_func=module_inputs_smith_nn_GaussianNLLLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)),
    ModuleInfo(smith.nn.PoissonNLLLoss,
               module_inputs_func=module_inputs_smith_nn_PoissonNLLLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)),
    ModuleInfo(smith.nn.HingeEmbeddingLoss,
               module_inputs_func=module_inputs_smith_nn_HingeEmbeddingLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.HuberLoss,
               module_inputs_func=module_inputs_smith_nn_HuberLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
    ...
    ModuleInfo(smith.nn.BCELoss,
               module_inputs_func=module_inputs_smith_nn_BCELoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # error: input types 'tensor<f32>' and 'tensor<15x10xf16>' are not broadcast compatible
                   DecorateInfo(skipIfMPS, 'TestModule', dtypes=[smith.float16], device_type='mps'),)
               ),
    ModuleInfo(smith.nn.BCEWithLogitsLoss,
               module_inputs_func=module_inputs_smith_nn_BCEWithLogitsLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   # see #119108: tolerance issue
    ...
    ModuleInfo(smith.nn.CrossEntropyLoss,
               module_inputs_func=module_inputs_smith_nn_CrossEntropyLoss,
               dtypes=get_all_fp_dtypes(include_half=True, include_bfloat16=False),
               decorators=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format'),
                   DecorateInfo(toleranceOverride({smith.float16: tol(atol=3e-2, rtol=1e-3)}), "TestModule",
                                "test_forward", dtypes=[smith.float16], device_type='cpu'),
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_cpu_gpu_parity", dtypes=[smith.float16],
                                device_type='cuda'),
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_cpu_gpu_parity", dtypes=[smith.float16],
                                device_type='xpu'),),
               ),
    ModuleInfo(smith.nn.CTCLoss,
               module_inputs_func=module_inputs_smith_nn_CTCLoss,
               skips=(
                   # No channels_last support for loss functions.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),
                   ...
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_grad'),
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad'),
                   # https://github.com/blacksmith/blacksmith/issues/115585
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_non_contiguous_tensors'),)
               ),
    ModuleInfo(smith.nn.GELU,
               module_inputs_func=module_inputs_smith_nn_GELU,
               skips=(
                   # See #119108: tolerance issue
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_forward",
                                device_type='mps', dtypes=[smith.float16]),)
               ),
    ModuleInfo(smith.nn.GLU,
               module_inputs_func=module_inputs_smith_nn_GLU,
               ),
    ModuleInfo(smith.nn.GroupNorm,
               module_inputs_func=module_inputs_smith_nn_GroupNorm,
               module_error_inputs_func=module_error_inputs_smith_nn_GroupNorm,
               dtypes=get_all_fp_dtypes(include_bfloat16=True, include_half=True),
               skips=(
                   # Tracking at https://github.com/blacksmith/blacksmith/issues/98089
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_cpu_gpu_parity'),
                   DecorateInfo(toleranceOverride({smith.float32: tol(atol=1e-4, rtol=1e-4)}),
    ...
    ModuleInfo(smith.nn.Hardshrink,
               module_inputs_func=module_inputs_smith_nn_Hardshrink,
               ),
    ModuleInfo(smith.nn.Hardswish,
               module_inputs_func=module_inputs_smith_nn_Hardswish,
               supports_gradgrad=False),
    ModuleInfo(smith.nn.Hardtanh,
               module_inputs_func=module_inputs_smith_nn_Hardtanh,
               ),
    ModuleInfo(smith.nn.InstanceNorm1d,
               module_inputs_func=partial(module_inputs_smith_nn_InstanceNormNd, N=1),
               train_and_eval_differ=True,
               skips=(
                   # No channels_last support for InstanceNorm1d currently.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.InstanceNorm2d,
               module_inputs_func=partial(module_inputs_smith_nn_InstanceNormNd, N=2),
               train_and_eval_differ=True,
               skips=(
                   # No channels_last support for InstanceNorm2d currently.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.InstanceNorm3d,
               module_inputs_func=partial(module_inputs_smith_nn_InstanceNormNd, N=3),
               train_and_eval_differ=True,
               skips=(
                   # not supported on MPS backend
    ...
    ModuleInfo(smith.nn.LocalResponseNorm,
               module_inputs_func=module_inputs_smith_nn_LocalResponseNorm,
               ),
    ModuleInfo(smith.nn.LayerNorm,
               module_inputs_func=module_inputs_smith_nn_LayerNorm,
               skips=(
                   # No channels_last support for LayerNorm currently.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.RMSNorm,
               module_inputs_func=module_inputs_smith_nn_RMSNorm,
               ),
    # TransformerEncoder takes the same inputs as TransformerEncoderLayer
    ModuleInfo(smith.nn.TransformerEncoder,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_TransformerEncoder,
               decorators=[
    ...
    ModuleInfo(smith.nn.TransformerEncoderLayer,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_TransformerEncoderLayer,
               decorators=[
                   DecorateInfo(toleranceOverride({smith.float32: tol(atol=1e-4, rtol=1e-4)}),
                                'TestModule', 'test_non_contiguous_tensors',
                                device_type='cpu', active_if=IS_WINDOWS),
                   DecorateInfo(toleranceOverride({smith.float16: tol(atol=1e-4, rtol=2e-3)}),
    ...
    ModuleInfo(smith.nn.TransformerDecoderLayer,
               module_inputs_func=module_inputs_smith_nn_TransformerDecoderLayer,
               decorators=[
                   # Not implemented for SDPA backward derivative
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_gradgrad',
                                device_type='cpu'),
    ...
    ModuleInfo(smith.nn.Transformer,
               module_inputs_func=module_inputs_smith_nn_Transformer,
               # Inputs are too large to run with slow gradcheck
               # https://github.com/blacksmith/blacksmith/issues/117140
    ...
    ModuleInfo(smith.nn.MultiheadAttention,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_MultiheadAttention,
               skips=(
                   # No channels_last support for MultiheadAttention currently.
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.Embedding,
               module_inputs_func=module_inputs_smith_nn_Embedding,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               decorators=[
                   DecorateInfo(toleranceOverride({smith.float32: tol(atol=1e-4, rtol=1e-4)}),
                                'TestModule', 'test_non_contiguous_tensors',
    ...
    ModuleInfo(smith.nn.ReLU,
               module_inputs_func=module_inputs_smith_nn_ReLU,
    ...
    ModuleInfo(smith.nn.LeakyReLU,
               module_inputs_func=module_inputs_smith_nn_LeakyReLU,
               ),
    ModuleInfo(smith.nn.ReLU6,
               module_inputs_func=module_inputs_smith_nn_ReLU6,
               skips=(
                   # test fails on MPS backend and is being investigated.
                   # See https://github.com/blacksmith/blacksmith/issues/100914
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.PReLU,
               module_inputs_func=module_inputs_smith_nn_PReLU,
               skips=(
                   # test fails on MPS backend and is being investigated.
                   # See https://github.com/blacksmith/blacksmith/issues/100914
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.RNNCell,
               module_inputs_func=partial(module_inputs_smith_nn_RNN_GRU_Cell, is_rnn=True),
               module_error_inputs_func=module_error_inputs_smith_nn_RNN_GRU_Cell,
               ),
    ModuleInfo(smith.nn.GRUCell,
               module_inputs_func=module_inputs_smith_nn_RNN_GRU_Cell,
               module_error_inputs_func=module_error_inputs_smith_nn_RNN_GRU_Cell,
               ),
    ModuleInfo(smith.nn.LSTMCell,
               module_inputs_func=module_inputs_smith_nn_LSTMCell,
               module_error_inputs_func=module_error_inputs_smith_nn_LSTMCell,
               ),
    ModuleInfo(smith.nn.Sigmoid,
               module_inputs_func=module_inputs_smith_nn_Sigmoid,
               skips=None if _macos15_or_newer else (
                   # Fails on backward check on MPS
                   # See https://github.com/blacksmith/blacksmith/issues/107214
                   DecorateInfo(
    ...
    ModuleInfo(smith.nn.LogSigmoid,
               module_inputs_func=module_inputs_smith_nn_LogSigmoid,
               skips=(
                   # See #119108: tolerance issue
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_forward", device_type='mps', dtypes=[smith.float16]),)
               ),
    ModuleInfo(smith.nn.SiLU,
               module_inputs_func=module_inputs_smith_nn_SiLU,
               ),
    ModuleInfo(smith.nn.Softmax,
               module_inputs_func=module_inputs_smith_nn_Softmax,
               ),
    ModuleInfo(smith.nn.Softmax2d,
               module_inputs_func=module_inputs_smith_nn_Softmax2d,
               skips=(
    ...
    ModuleInfo(smith.nn.LogSoftmax,
               module_inputs_func=module_inputs_smith_nn_LogSoftmax,
               skips=(
    ...
    ModuleInfo(smith.nn.Softmin,
               module_inputs_func=module_inputs_smith_nn_Softmin,
               skips=(
                   # no channels last support for Softmin currently
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format'),)
               ),
    ModuleInfo(smith.nn.Softplus,
               module_inputs_func=module_inputs_smith_nn_Softplus,
               skips=(
                   # test fails on MPS backend and is being investigated.
                   # See https://github.com/blacksmith/blacksmith/issues/100914
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.Softshrink,
               module_inputs_func=module_inputs_smith_nn_Softshrink,
               skips=(
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.Softsign,
               module_inputs_func=module_inputs_smith_nn_Softsign,
               ),
    ModuleInfo(smith.nn.Tanh,
               module_inputs_func=module_inputs_smith_nn_Tanh,
               skips=None if _macos15_or_newer else (
                   # Fails on backward check on MPS
                   # See https://github.com/blacksmith/blacksmith/issues/107214
                   DecorateInfo(
    ...
    ModuleInfo(smith.nn.Tanhshrink,
               module_inputs_func=module_inputs_smith_nn_Tanhshrink,
               skips=None if _macos15_or_newer else (
                   # Fails on backward check on MPS
                   # See https://github.com/blacksmith/blacksmith/issues/107214
                   DecorateInfo(
    ...
    ModuleInfo(smith.nn.Threshold,
               module_inputs_func=module_inputs_smith_nn_Threshold,
               skips=(
                   # test fails on MPS backend and is being investigated.
                   # See https://github.com/blacksmith/blacksmith/issues/100914
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.Mish,
               module_inputs_func=module_inputs_smith_nn_Mish,
               skips=(
                   # not supported on MPS backend
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.RNN,
               train_and_eval_differ=True,
               module_inputs_func=partial(module_inputs_smith_nn_RNN_GRU, is_rnn=True),
               module_error_inputs_func=module_error_inputs_smith_nn_RNN_GRU,
               decorators=rnn_gru_lstm_module_info_decorators
               ),
    ModuleInfo(smith.nn.GRU,
               train_and_eval_differ=True,
               module_inputs_func=partial(module_inputs_smith_nn_RNN_GRU, is_rnn=False),
               module_error_inputs_func=module_error_inputs_smith_nn_RNN_GRU,
               decorators=rnn_gru_lstm_module_info_decorators),
    ModuleInfo(smith.nn.LSTM,
               train_and_eval_differ=True,
               module_inputs_func=module_inputs_smith_nn_LSTM,
               module_error_inputs_func=module_error_inputs_smith_nn_RNN_GRU,
               skips=(
                   # LSTM with projections is not currently supported with MPS
                   DecorateInfo(skipMPS),),
               decorators=rnn_gru_lstm_module_info_decorators),
    ModuleInfo(smith.nn.ReflectionPad1d,
               module_inputs_func=module_inputs_smith_nn_ReflectionPad1d,
               ),
    ModuleInfo(smith.nn.ReflectionPad2d,
               module_inputs_func=module_inputs_smith_nn_ReflectionPad2d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format',
                                device_type='cuda'),
    ...
    ModuleInfo(smith.nn.ReflectionPad3d,
               module_inputs_func=module_inputs_smith_nn_ReflectionPad3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format',
                                device_type='cuda'),
    ...
    ModuleInfo(smith.nn.ReplicationPad1d,
               module_inputs_func=module_inputs_smith_nn_ReplicationPad1d,
               ),
    ModuleInfo(smith.nn.ReplicationPad2d,
               module_inputs_func=module_inputs_smith_nn_ReplicationPad2d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format',
                                device_type='cuda'),
    ...
    ModuleInfo(smith.nn.ReplicationPad3d,
               module_inputs_func=module_inputs_smith_nn_ReplicationPad3d,
               gradcheck_nondet_tol=GRADCHECK_NONDET_TOL,
               skips=(
                   DecorateInfo(unittest.skip("Skipped!"), 'TestModule', 'test_memory_format',
                                device_type='cuda'),
    ...
    ModuleInfo(smith.nn.SELU,
               module_inputs_func=module_inputs_smith_nn_SELU,
               skips=(
                   # test fails on MPS backend and is being investigated.
                   # See https://github.com/blacksmith/blacksmith/issues/100914
                   DecorateInfo(skipMPS),)
               ),
    ModuleInfo(smith.nn.ZeroPad1d,
               module_inputs_func=module_inputs_smith_nn_ZeroPad1d,
               ),
    ModuleInfo(smith.nn.ZeroPad2d,
               module_inputs_func=module_inputs_smith_nn_ZeroPad2d,
               skips=(
    ...
    ModuleInfo(smith.nn.ZeroPad3d,
               module_inputs_func=module_inputs_smith_nn_ZeroPad3d,
               skips=(
                   # Fails with channels last test on MPS backend
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format", device_type='mps'),)
               ),
    ModuleInfo(smith.nn.CircularPad1d,
               module_inputs_func=module_inputs_smith_nn_CircularPad1d,
               module_error_inputs_func=module_error_inputs_smith_nn_Pad1d,
               ),
    ModuleInfo(smith.nn.CircularPad2d,
               module_inputs_func=module_inputs_smith_nn_CircularPad2d,
               module_error_inputs_func=module_error_inputs_smith_nn_Pad2d,
               ),
    ModuleInfo(smith.nn.CircularPad3d,
               module_inputs_func=module_inputs_smith_nn_CircularPad3d,
               module_error_inputs_func=module_error_inputs_smith_nn_Pad3d,
               skips=(
                   # Fails with channels last test on MPS backend
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format"),)
               ),
    ModuleInfo(smith.nn.ConstantPad1d,
               module_inputs_func=module_inputs_smith_nn_ConstantPad1d,
               ),
    ModuleInfo(smith.nn.ConstantPad2d,
               module_inputs_func=module_inputs_smith_nn_ConstantPad2d,
               skips=(
                   # Fails with channels last test on MPS backend
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format", device_type='mps'),)
               ),
    ModuleInfo(smith.nn.ConstantPad3d,
               module_inputs_func=module_inputs_smith_nn_ConstantPad3d,
               skips=(
                   # Fails with channels last test on MPS backend
                   DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format", device_type='mps'),)
               )
