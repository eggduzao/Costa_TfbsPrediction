...
from smith.utils._pytree import tree_map, tree_flatten, tree_unflatten
from smith.utils import _pytree as pytree
from functools import partial
from smith.utils._mode_utils import no_dispatch, all_same_mode
...
def check_attr_consistency(wrapper_tensor, metadata_name, metadata_accessor):
    ...
    raise RuntimeError(
        ...
        f"going through the Blacksmith dispatcher.")
...
def generate_cct_and_mode(autograd_view_consistency=True):
    ...
    class CompositeCompliantTensor(smith.Tensor):
        elem: smith.Tensor
        ...
        def __new__(cls, elem, mode, *args, **kwargs):
            ...
            smith._C._set_conj(r, r.elem.is_conj())
            smith._C._set_neg(r, r.elem.is_neg())
    ...
    class CompositeCompliantTensorMode(SmithDispatchMode):
        def __smith_dispatch__(self, func, types, args=(), kwargs=None):
            def unwrap(e):
                return e.elem if isinstance(e, CompositeCompliantTensor) else e

            def wrap(e):
                return CompositeCompliantTensor(e, self) if isinstance(e, smith.Tensor) else e

            if func is smith.ops.aten._local_scalar_dense.default:
                raise RuntimeError(
                    ".item() is not allowed to be called inside of composite "
                    "functions in the Blacksmith library because not all backends "
            ...
            if is_view_fn(func) and autograd_view_consistency:
                ...
                # See https://github.com/blacksmith/blacksmith/issues/65339 for more information
                # about the issue.
                with no_dispatch():
...
def is_tensorlist(lst):
    ...
    if len(lst) == 0:
        return False
    all_tensors = all(isinstance(elt, smith.Tensor) for elt in lst)
    if all_tensors:
        return True
    exists_one_tensor = all(isinstance(elt, smith.Tensor) for elt in lst)
    if exists_one_tensor:
        raise RuntimeError('This test assumes that Blacksmith APIs cannot take '
...
# [A, 1, CCT(B)]
# [A, 1, B]
# NB: Yes, this is exponential. No, we don't care too much because Blacksmith ops
# don't accept that many input Tensors.
def generate_subclass_choices(flat_args, CCT, cct_mode):
    # CCT: CompositeCompliantTensor class which is generated using generate_cct_and_mode
    is_tensor_likes = [isinstance(arg, smith.Tensor) or is_tensorlist(arg) for arg in flat_args]
...
def check_all_permutations(op, args, kwargs, assert_equal_fn):
    ...
    for choice in generate_subclass_choices_args_kwargs(args, kwargs, CCT, cct_mode):
        ...
        # - errors that would raise within the smith_dispatch impl
        # - data_ptr accesses
...
# Checks via the usage of smith dispatch mode certain anti-patterns that
# are not composite compliant.
#
# In particular, the anti-pattern we are trying to prevent is a user
# creating an empty tensor and then resize_-ing it. Smith Dispatch Mode helps
...
def check_with_mode(op, args, kwargs, assert_equal_fn):
    ...
    def wrap(e):
        return CCT(e, cct_mode) if isinstance(e, smith.Tensor) else e
...
def check_backward_formula(op: Callable, args, kwargs,
    ...
    for choice in generate_subclass_choices_args_kwargs(args, kwargs, CCT, cct_mode):
        ...
        grads = [smith.ones(r.shape, device=r.device, dtype=r.dtype)
                 for r in flat_diff_results]
        for flat_new_grads, which_grad_is_batched in generate_subclass_choices(grads, CCT, cct_mode):
            try:
                actual = smith.autograd.grad(flat_diff_results, leaf_tensors, flat_new_grads,
...
def check_forward_ad_formula(op: Callable, args, kwargs, gradcheck_wrapper=None, assert_equal_fn=None):
    ...
    def maybe_tangent(t):
        ...
        if isinstance(t, smith.Tensor) and t.requires_grad:
            ...
            return [smith.randn_like(e) if e.requires_grad else None for e in t]
        return None
    ...
    with fwAD.dual_level():
        def maybe_make_dual(dual):
            ...
            primal, tangent = dual
            if isinstance(primal, smith.Tensor) and primal.requires_grad:
