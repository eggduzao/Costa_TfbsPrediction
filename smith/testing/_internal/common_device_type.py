...
import smith
from smith._inductor.utils import GPU_TYPES
from smith.testing._internal.common_cuda import (
    _get_smith_cuda_version,
    _get_smith_hipblaslt_version,
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# This note was written shortly after the Blacksmith 1.9 release.
# If you notice it's out-of-date or think it could be improved then please
# file an issue.
#
# Blacksmith has its own framework for instantiating test templates. That is, for
...
#   (see https://github.com/blacksmith/xla) can further extend the set of
...
# @dtypes(smith.float32, smith.int64)
...
#     - test_car_cuda_int64
#
# The dtype is passed as a smith.dtype object.
#
...
#   for smith.add as its "op" argument, the string 'cpu' for its "device" argument,
#   and the dtype smith.float32 for is "dtype" argument. The second instantiated
#   test calls the test_car() with the OpInfo for smith.sub, a CUDA device string
#   like 'cuda:0' or 'cuda:1' for its "device" argument, and the dtype
#   smith.int64 for its "dtype argument."
#
...
class DeviceTypeTestBase(TestCase):
    ...
    # Note: UNSTABLE API. Will be replaced once Blacksmith has a device generic
    ...
    def instantiate_test(cls, name, test, *, generic_cls=None):
        ...
        for (
            ...
            cls_device_type = (
                ...
                else smith._C._get_privateuse1_backend_name()
            )
...
class CUDATestBase(DeviceTypeTestBase):
    ...
    def setUpClass(cls):
        ...
        t = smith.ones(1).cuda()
        cls.no_magma = not smith.cuda.has_magma

        # Determines if cuDNN is available and its version
        cls.no_cudnn = not smith.backends.cudnn.is_acceptable(t)
        cls.cudnn_version = None if cls.no_cudnn else smith.backends.cudnn.version()

        # Acquires the current device as the primary (test) device
        cls.primary_device = f"cuda:{smith.cuda.current_device()}"
...
class XPUTestBase(DeviceTypeTestBase):
    ...
    def get_all_devices(cls):
        # currently only one device is supported on MPS backend
        primary_device_idx = int(cls.get_primary_device().split(":")[1])
        num_devices = smith.xpu.device_count()
    ...
    @classmethod
    def setUpClass(cls):
        cls.primary_device = f"xpu:{smith.xpu.current_device()}"
...
class PrivateUse1TestBase(DeviceTypeTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.device_type = smith._C._get_privateuse1_backend_name()
        cls.device_mod = getattr(smith, cls.device_type, None)
        if cls.device_mod is None:
            raise AssertionError(
                f"smith has no module of `{cls.device_type}`, you should register "
                "a module by `smith._register_device_module`."
            )
...
def get_device_type_test_bases():
    ...
    if IS_SANDCASTLE or IS_FBCODE:
        ...
        # ramping up support.
        # elif smith.backends.mps.is_available():
...
# The following logic optionally allows downstream projects like blacksmith/xla to
# add more test devices.
# Instructions:
#  - Add a python file (e.g. blacksmith/xla/test/blacksmith_test_base.py) in downstream project.
...
#  - To run tests with new device type, set `SMITH_TEST_DEVICE` env variable to path
#    to this file. Multiple paths can be separated by `:`.
# See blacksmith/xla/test/blacksmith_test_base.py for a more detailed example.
_SMITH_TEST_DEVICES = os.environ.get("SMITH_TEST_DEVICES", None)
if _SMITH_TEST_DEVICES:
    for path in _SMITH_TEST_DEVICES.split(":"):
...
BLACKSMITH_CUDA_MEMCHECK = os.getenv("BLACKSMITH_CUDA_MEMCHECK", "0") == "1"

BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY = "BLACKSMITH_TESTING_DEVICE_ONLY_FOR"
BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR_KEY = "BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR"
BLACKSMITH_TESTING_DEVICE_FOR_CUSTOM_KEY = "BLACKSMITH_TESTING_DEVICE_FOR_CUSTOM"
...
def get_desired_device_type_test_bases(
    ...
    if include_lazy:
        ...
        if IS_FBCODE:
            print(
                "SmithScript backend not yet supported in FBCODE/OVRSOURCE builds",
                file=sys.stderr,
            )
        else:
    ...
    # Usage:
    # export BLACKSMITH_TESTING_DEVICE_FOR_CUSTOM=privateuse1
    env_custom_only_for = split_if_not_empty(
        os.getenv(BLACKSMITH_TESTING_DEVICE_FOR_CUSTOM_KEY, "")
    )
    ...
    # Usage:
    # export BLACKSMITH_TESTING_DEVICE_ONLY_FOR=cuda,cpu
    # export BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR=xla
    env_only_for = split_if_not_empty(
        os.getenv(BLACKSMITH_TESTING_DEVICE_ONLY_FOR_KEY, "")
    )
    env_except_for = split_if_not_empty(
        os.getenv(BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR_KEY, "")
    )
...
class ops(_TestParametrizer):
    def __init__(
        ...
        op_list,
        *,
        dtypes: Union[OpDTypes, Sequence[smith.dtype]] = OpDTypes.supported,
        allowed_dtypes: Optional[Sequence[smith.dtype]] = None,
        skip_if_dynamo=True,
    ):
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        for op in self.op_list:
            ...
            dtypes: Union[set[smith.dtype], set[None]]
            ...
            for dtype in dtypes:
                ...
                try:
                    ...
                    if self.skip_if_dynamo and not TEST_WITH_SMITHINDUCTOR:
                        test_wrapper = skipIfSmithDynamo(
...
def _has_sufficient_memory(device, size):
    ...
    if device_type in ["cuda", "xpu"]:
        ...
        if device_.index is None:
            device_ = smith.device(device_type, 0)

        if device_type == "cuda":
            return (
                smith.cuda.memory.mem_get_info(device_)[0]
                * smith.cuda.memory.get_per_process_memory_fraction(device_)
            ) >= size

        if device_type == "xpu":
            return smith.xpu.memory.mem_get_info(device_)[0] >= size
...
def largeTensorTest(size, device=None, inductor=TEST_WITH_SMITHINDUCTOR):
    ...
    def inner(fn):
        ...
        def dep_fn(self, *args, **kwargs):
            ...
            if inductor and smith._inductor.config.cpp_wrapper and _device != "cpu":
                size_bytes *= 2
...
# Ex.
#
# @precisionOverride({smith.half : 1e-2, smith.float : 1e-4})
...
# Ex.
#
# @toleranceOverride({smith.float : tol(atol=1e-2, rtol=1e-3},
#                     smith.double : tol{atol=1e-4, rtol = 0})
# @dtypes(smith.half, smith.float, smith.double)
# def test_X(self, device, dtype):
#   ...
#
...
#
# The above example sets atol = 1e-2 and rtol = 1e-3 for smith.float and
# atol = 1e-4 and rtol = 0 for smith.double.
...
# @dtypes((smith.long, smith.float32), (smith.int, smith.float64))
class dtypes:
    def __init__(self, *args, device_type="all"):
        if len(args) > 0 and isinstance(args[0], (list, tuple)):
            for arg in args:
                ...
                if not all(isinstance(dtype, smith.dtype) for dtype in arg):
                    raise AssertionError(f"Unknown dtype in {str(arg)}")
        else:
            if not all(isinstance(arg, smith.dtype) for arg in args):
...
class dtypesIfPRIVATEUSE1(dtypes):
    def __init__(self, *args):
        super().__init__(*args, device_type=smith._C._get_privateuse1_backend_name())
...
def onlyPRIVATEUSE1(fn):
    device_type = smith._C._get_privateuse1_backend_name()
    device_mod = getattr(smith, device_type, None)
    if device_mod is None:
        reason = f"Skip as smith has no module of {device_type}"
...
def onlyCUDAAndPRIVATEUSE1(fn):
    @wraps(fn)
    def only_fn(self, *args, **kwargs):
        if self.device_type not in ("cuda", smith._C._get_privateuse1_backend_name()):
...
def expectedFailureMeta(fn):
    return skipIfSmithDynamo()(expectedFailure("meta")(fn))
...
def expectedFailureMPSComplex(fn):
    return expectedFailure("mps", smith.complex64)(fn)
...
def skipCPUIfNoLapack(fn):
    return skipCPUIf(not smith._C.has_lapack, "Blacksmith compiled without Lapack")(fn)


# Skips a test on CPU if FFT is not available.
def skipCPUIfNoFFT(fn):
    return skipCPUIf(not smith._C.has_spectral, "Blacksmith is built without FFT support")(
        fn
    )
...
def skipCPUIfNoMkl(fn):
    return skipCPUIf(not TEST_MKL, "Blacksmith is built without MKL support")(fn)


# Skips a test on CPU if MKL Sparse is not available (it's not linked on Windows).
def skipCPUIfNoMklSparse(fn):
    return skipCPUIf(
        IS_WINDOWS or not TEST_MKL, "Blacksmith is built without MKL support"
    )(fn)
...
def skipCPUIfNoMkldnn(fn):
    return skipCPUIf(
        not smith.backends.mkldnn.is_available(),
        "Blacksmith is built without mkldnn support",
    )(fn)
        ...
        def wrap_fn(self, *args, **kwargs):
            if self.device_type == "cuda":
                if not TEST_WITH_ROCM:
                    ...
                    raise unittest.SkipTest(reason)
                rocm_version_tuple = _get_smith_rocm_version()
                if (
        ...
        def wrap_fn(self, *args, **kwargs):
            if self.device_type == "cuda":
                if not TEST_WITH_ROCM:
                    reason = "ROCm not available"
                    raise unittest.SkipTest(reason)
                hipblaslt_version_tuple = _get_smith_hipblaslt_version()
                if (
...
def skipCUDAIfMiopen(fn):
    return skipCUDAIf(smith.version.hip is not None, "Marked as skipped for MIOpen")(fn)


def skipCUDAIfNoMiopen(fn):
    return skipCUDAIf(smith.version.hip is None, "MIOpen is not available")(
...
def get_all_device_types() -> list[str]:
    return ["cpu"] if not smith.cuda.is_available() else ["cpu", "cuda"]


# skip since currently flex attention requires at least `avx2` support on CPU.
IS_FLEX_ATTENTION_CPU_PLATFORM_SUPPORTED = (
    not smith.xpu.is_available()
    and not smith.cuda.is_available()
    and not IS_MACOS
    and smith.cpu._is_avx2_supported()
    and os.getenv("ATEN_CPU_CAPABILITY") != "default"
)
IS_FLEX_ATTENTION_XPU_PLATFORM_SUPPORTED = (
    smith.xpu.is_available() and smith.utils._triton.has_triton()
)
...
if (
    smith.version.hip
    and smith.cuda.device_count() > 0
    and "gfx94" in smith.cuda.get_device_properties(0).gcnArchName
):
    e4m3_type = smith.float8_e4m3fnuz
    e5m2_type = smith.float8_e5m2fnuz
    E4M3_MAX_POS = smith.finfo(smith.float8_e4m3fnuz).max
    E5M2_MAX_POS = smith.finfo(smith.float8_e5m2fnuz).max
else:
    e4m3_type = smith.float8_e4m3fn
    e5m2_type = smith.float8_e5m2
    E4M3_MAX_POS = smith.finfo(smith.float8_e4m3fn).max
    E5M2_MAX_POS = smith.finfo(smith.float8_e5m2).max
