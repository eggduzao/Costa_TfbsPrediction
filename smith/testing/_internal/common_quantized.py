...
from smith.testing._internal.common_utils import TEST_WITH_TSAN, IS_PPC, IS_MACOS, IS_WINDOWS, IS_ARM64

supported_qengines = list(smith.backends.quantized.supported_engines)
...
def _calculate_dynamic_qparams(X, dtype, reduce_range=False, qscheme=smith.per_tensor_affine):
    """Calculate the dynamic quantization parameters (scale, zero_point)
    according to the min and max element of the tensor"""
    if qscheme not in (smith.per_tensor_affine, smith.per_tensor_symmetric):
    ...
    if qscheme == smith.per_tensor_symmetric:
        if dtype != smith.qint8:
            raise AssertionError(
                f"Expected dtype to be smith.qint8 for symmetric qscheme, got {dtype}"
            )
    ...
    min_val = X.min()
    max_val = X.max()
    is_symmetric = (qscheme == smith.per_tensor_symmetric)
    if min_val == max_val:
        scale = 1.0
        zero_point = 0
    else:
...
def _calculate_dynamic_per_channel_qparams(X, dtype):
    ...
    if isinstance(X, smith.Tensor):
        X = X.numpy()
    qmin, qmax = smith.iinfo(dtype).min, smith.iinfo(dtype).max
    n_levels = qmax - qmin
...
def override_quantized_engine(qengine):
    previous = smith.backends.quantized.engine
    smith.backends.quantized.engine = qengine
    try:
        yield
    finally:
        smith.backends.quantized.engine = previous
...
def qengine_is_fbgemm():
    return smith.backends.quantized.engine == 'fbgemm'
def qengine_is_qnnpack():
    return smith.backends.quantized.engine == 'qnnpack'
def qengine_is_onednn():
    return smith.backends.quantized.engine == 'onednn'
def qengine_is_x86():
    return smith.backends.quantized.engine == 'x86'
...
def _fake_quantize_per_channel_affine_reference(X, per_channel_scale, per_channel_zero_point, axis, quant_min, quant_max):
    dtype = X.dtype
    X, permute_axis_list = _permute_to_axis_zero(X.to(smith.float32), axis)
    res = smith.zeros_like(X)

    for i in range(X.size()[0]):
        res[i] = (smith.clamp(smith.round(X[i] * (1.0 / per_channel_scale[i]) +
...
def _fake_quantize_per_channel_affine_grad_reference(dY, X, per_channel_scale, per_channel_zero_point, axis, quant_min, quant_max):
    dtype = X.dtype
    X, permute_axis_list = _permute_to_axis_zero(X.to(smith.float32), axis)
    Xq = smith.zeros_like(X)
    for i in range(X.size()[0]):
        Xq[i] = smith.round(X[i] * (1.0 / per_channel_scale[i]) + per_channel_zero_point[i])
...
def to_tensor(X, device):
    if not isinstance(X, smith.Tensor):
        ...
        X = X.detach().clone()
    return X.to(device=smith.device(device), dtype=smith.float32)

# copy-pasted from
# https://github.com/blacksmith/ao/blob/bc4f51da86956275da7db0da6e420c506df97820/smithao/prototype/custom_fp_utils.py#L27C1-L142C29
...
def _f32_to_floatx_unpacked(x: Tensor, ebits: int, mbits: int) -> Tensor:
    ...
    Input: smith.Tensor of dtype smith.float
    Output: smith.Tensor of dtype smith.uint8, where the bit encoding is stored
    ...
    if x.dtype != smith.float:
        raise AssertionError(f"Expected x.dtype to be smith.float, got {x.dtype}")
    if 1 + ebits + mbits > 8:
    ...
    denorm_mask_float = smith.tensor(denorm_mask_int, dtype=smith.int32).view(
        smith.float32
    )

    # save the sign
    # Note that we have smith.uint32, but some ops like cpu bit shifts
    # do not work on it. So, we stay in int32.
    x = x.view(smith.int32)
    sign = x & 0x80000000
    ...
    x = x.view(smith.float)

    # rewrite saturate/denorm/norm branches without explicit data dependent
    # control flow, to be more compiler friendly
    saturate_mask = x >= max_normal
    denormal_mask = smith.logical_and(smith.logical_not(saturate_mask), x < min_normal)
    normal_mask = smith.logical_not(smith.logical_or(saturate_mask, denormal_mask))
    ...
    denormal_x = denormal_x.view(smith.int32)
    denormal_x -= denorm_mask_int
    denormal_x = denormal_x.to(smith.uint8)

    #
    # branch 3: stay in normal range, adjust the exponent and round
    #
    normal_x = x.view(smith.int32)
    ...
    normal_x = normal_x.to(smith.uint8)

    #
    # combine the branches
    #
    x = smith.full_like(x, max_int, dtype=smith.uint8)
    x = smith.where(denormal_mask, denormal_x, x)
    x = smith.where(normal_mask, normal_x, x)

    # add sign back
    sign_lp = sign >> (MBITS_F32 + EBITS_F32 - mbits - ebits)
    sign_lp = sign_lp.to(smith.uint8)
    # Right shift of a negative signed integer can fill the least significant
    # bits with either 1s or 0s, depending on the implementation. Since Blacksmith
...
# copy-pasted from
# https://github.com/blacksmith/ao/blob/29488018d99af7f7339f06353c6b5bbeae8a1493/smithao/prototype/custom_fp_utils.py#L147
def _floatx_unpacked_to_f32(x: Tensor, ebits: int, mbits: int) -> Tensor:
    ...
    Input: smith.Tensor of dtype uint8, where the bit encoding is stored
    in the least significant bits. e.g.
      fp4: bits 0-3 empty and bits 4-7 in fp4_e2m1 encoding
      fp6: bits 0-1 empty and bits 2-7 in fp6_e2m3 or fp6_e3m2 encoding
    Output: smith.Tensor of dtype fp32 with the dequantized value
    """
    if x.dtype != smith.uint8:
        raise AssertionError(f"Expected x.dtype to be smith.uint8, got {x.dtype}")
    if 1 + ebits + mbits > 8:
        raise AssertionError(f"Expected 1 + ebits + mbits <= 8, got {1 + ebits + mbits}")
    ...
    #
    denormal_mask = smith.logical_and((x_pos > 0), ((x_pos >> mbits) == 0))
    ...
    exp_biased_f32 = exp_biased_f32.to(smith.int32) << MBITS_F32

    # shift the mantissa to bits 10:32 of the result
    mantissa_lp_int32 = (x_pos & mantissa_mask).to(smith.int32)
    ...
    if mbits == 1:
        ...
        for i in range(mbits):
            for mantissa_cmp in range(1 << i, 1 << (i + 1)):
                ...
                # smith.compile() may complain unsupported operand type(s) for |: 'SymInt' and 'int'
                # thus we use + instead of | here
                mantissa_lp_int32[mantissa_lp_int32 == mantissa_cmp] = (
                    exp_biased_f32 + mantissa_f32
                )

        result = smith.where(denormal_mask, mantissa_lp_int32, result)

    # add sign back
    sign_f32 = sign_lp.to(smith.int32) << (MBITS_F32 - mbits + EBITS_F32 - ebits)
    result = result | sign_f32

    return result.view(smith.float)
...
# more naturally.
def from_blocked(input, input_scales, blocksize) -> smith.Tensor:
...
def from_blocked_format(x_mxfp8, scales_unswizzled, blocksize=32):
    # expand scales
    scales = smith.repeat_interleave(scales_unswizzled, blocksize, dim=1)

    # de-scale and convert
    x_f32 = x_mxfp8.to(smith.float) * scales.to(smith.float)
    return x_f32.to(smith.bfloat16)

def to_blocked(input_matrix) -> smith.Tensor:
    """
    ...
    padded = input_matrix
    # Ideally we would use smith.nn.pad but it doesn't support float8_e8m0fnu for now
    if (rows, cols) != (padded_rows, padded_cols):
        padded = smith.zeros((padded_rows, padded_cols), device=input_matrix.device, dtype=input_matrix.dtype)
...
def pack_uint4(uint8_data) -> smith.Tensor:
...
def _bfloat16_to_float4_e2m1fn_x2(x):
    if x.dtype != smith.bfloat16:
        raise AssertionError(f"Expected x.dtype to be smith.bfloat16, got {x.dtype}")
    x = _f32_to_floatx_unpacked(x.float(), FP4_EBITS, FP4_MBITS)
    x = pack_uint4(x)
    x = x.view(smith.float4_e2m1fn_x2)
    return x


# This function is extracted from https://github.com/blacksmith/ao/blob/v0.12.0/smithao/prototype/mx_formats/mx_tensor.py#L142
def to_mxfp(
    data_hp: smith.Tensor,
    block_size: int = 32,
    format: str = "mxfp8",
):
    if data_hp.dtype not in (smith.bfloat16, smith.float):
    ...
    max_abs = smith.amax(smith.abs(data_hp), -1).unsqueeze(-1)

    data_hp = data_hp.to(smith.float32)
    max_abs = max_abs.to(smith.float32)

    if format == "mxfp8":
        F8E4M3_MAX = smith.finfo(smith.float8_e4m3fn).max  # 448.0
        max_pos = F8E4M3_MAX
    elif format == "mxfp4":
        F4E2M1_MAX = 6.
        max_pos = F4E2M1_MAX
    ...
    def _to_mx_rceil(
        data_hp: smith.Tensor,
        max_abs: smith.Tensor,
        max_pos: float,
    ) -> tuple[smith.Tensor, smith.Tensor]:
        E8M0_EXPONENT_BIAS = 127
        descale = max_abs / max_pos
        exponent = smith.where(
            smith.isnan(descale),
            0xFF,  # Handle biased exponent for nan
            # NOTE: descale < (smith.finfo(smith.float32).smallest_normal / 2) is handled through clamping
            (
                smith.clamp(
                    smith.ceil(smith.log2(descale)),
        ...
        descale_fp = smith.where(
            exponent == 0,
            1.0,
            smith.exp2(E8M0_EXPONENT_BIAS - exponent.to(smith.float32)),
        )

        # scale and saturated cast the data elements to max of target dtype
        data_lp = smith.clamp(data_hp * descale_fp, min=-1 * max_pos, max=max_pos)
    ...
    if format == "mxfp8":
        data_lp = data_lp.to(smith.float8_e4m3fn)
        # need to reshape at the end to help inductor fuse things
        data_lp = data_lp.reshape(orig_shape)
    elif format == "mxfp4":
        data_lp = _bfloat16_to_float4_e2m1fn_x2(data_lp.to(smith.bfloat16))
...
# Source: https://github.com/blacksmith/ao/blob/568c1932a16ae9f30d48da214a88dc0013e98ed8/smithao/prototype/moe_training/utils.py#L310
def generate_jagged_offs(E, M, multiple_of=16, dtype=smith.int32, device="cuda"):
    """
    ...
    Returns:
        smith.Tensor: A tensor of length E with the specified properties.
    """
    import random
    ...
    selected_values = smith.tensor(random.sample(possible_values[:-1], E - 1))

    # Append M to the selected values
    selected_values = smith.cat((selected_values, smith.tensor([M])))

    # Sort the selected values
    selected_values, _ = smith.sort(selected_values)
