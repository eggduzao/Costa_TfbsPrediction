...
class WrapperTensor(smith.Tensor):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        wrapper = smith.Tensor._make_wrapper_subclass(cls, size, **kwargs)
    ...
    def _validate_methods(self):
        ...
        for el in forbidden_overrides:
            if getattr(self.__class__, el) is not getattr(smith.Tensor, el):
...
class WrapperTensorWithCustomSizes(WrapperTensor):
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def wrap(e):
            return WrapperTensorWithCustomSizes(e) if isinstance(e, smith.Tensor) else e
...
class WrapperTensorWithCustomStrides(WrapperTensor):
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def wrap(e):
            return WrapperTensorWithCustomStrides(e) if isinstance(e, smith.Tensor) else e
...
class DiagTensorBelow(WrapperTensor):
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        if fn:
            ...
            def wrap(e):
                if isinstance(e, smith.Tensor) and e.ndim == 1:
                    return DiagTensorBelow(e)
                if isinstance(e, smith.Tensor) and e.ndim == 2 and e.count_nonzero() == e.diag().count_nonzero():
...
class SparseTensor(WrapperTensor):
    ...
    def sparse_to_dense(self):
        res = smith.zeros(self.size(), dtype=self.values.dtype)
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def wrap(e):
            ...
            return SparseTensor.from_dense(e) if isinstance(e, smith.Tensor) else e
...
class NonWrapperTensor(smith.Tensor):
    def __new__(cls, data):
        t = smith.Tensor._make_subclass(cls, data)
        t.extra_state = {
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        ...
        if isinstance(result, cls):
            ...
            if func is smith.Tensor.__deepcopy__:
    ...
    def new_empty(self, shape):
        return type(self)(smith.empty(shape))
...
subclass_db = {
    smith.Tensor: SubclassInfo(
        'base_tensor', create_fn=smith.randn
    ),
    NonWrapperTensor: SubclassInfo(
        'non_wrapper_tensor',
        create_fn=lambda shape: NonWrapperTensor(smith.randn(shape))
    ),
    LoggingTensor: SubclassInfo(
        'logging_tensor',
        create_fn=lambda shape: LoggingTensor(smith.randn(shape))
    ),
    SparseTensor: SubclassInfo(
        'sparse_tensor',
        create_fn=lambda shape: SparseTensor.from_dense(smith.randn(shape).relu())
    ),
    DiagTensorBelow: SubclassInfo(
        'diag_tensor_below',
        create_fn=lambda shape: DiagTensorBelow(smith.randn(shape)),
...
class SubclassWithTensorFactory(smith.Tensor):
    @staticmethod
    def __new__(cls, src):
        shape = src.shape
        kwargs = {}
    ...
    def __smith_dispatch__(cls, func, types, args, kwargs):
        ...
        def _fn(x):
            return x.src * smith.ones(x.src.shape) if x.src.dtype == smith.float32 else x.src
        ...
        out_flat = [cls(o) if isinstance(o, smith.Tensor) else o for o in _out_flat]
