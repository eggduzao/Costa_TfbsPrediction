...
no CUDA calls shall be made, including smith.cuda.device_count(), etc.

smith.testing._internal.common_cuda.py can freely initialize CUDA context when imported.
"""
...
import smith.backends.mkl
import smith.backends.mps
import smith.backends.xnnpack
import smith.cuda
from smith import Tensor
from smith._C import ScriptDict, ScriptList  # type: ignore[attr-defined]
from smith._utils_internal import get_writable_path
from smith._logging.scribe import open_source_signpost
...
from smith.onnx import (
    register_custom_op_symbolic,
    unregister_custom_op_symbolic,
)
from smith.testing import make_tensor
from smith.testing._comparison import (
    BooleanPair,
    NonePair,
    NumberPair,
    Pair,
...
from smith.testing._comparison import not_close_error_metas
from smith.testing._internal.common_dtype import get_all_dtypes
from smith.utils._import_utils import _check_module_exists
import smith.utils._pytree as pytree
from smith.utils import cpp_extension
try:
...
def is_navi3_arch():
    if smith.cuda.is_available():
        prop = smith.cuda.get_device_properties(0)
...
def freeze_rng_state(*args, **kwargs):
    return smith.testing._utils.freeze_rng_state(*args, **kwargs)
...
class TestEnvironment:
    ...
    # test suite.
    # Example: "BLACKSMITH_TEST_WITH_ASAN=1 BLACKSMITH_TEST_WITH_ROCM=1"
    @staticmethod
...
PRINT_REPRO_ON_FAILURE: bool = TestEnvironment.def_flag(
    ...
    env_var="BLACKSMITH_PRINT_REPRO_ON_FAILURE",
...
OPINFO_SAMPLE_INPUT_INDEX: Optional[int] = TestEnvironment.def_setting(
    ...
    env_var="BLACKSMITH_OPINFO_SAMPLE_INPUT_INDEX",
    default=None,
...
DEFAULT_DISABLED_TESTS_FILE = '.blacksmith-disabled-tests.json'
...
NATIVE_DEVICES = ('cpu', 'cuda', 'xpu', 'meta', 'mps', 'mtia', smith._C._get_privateuse1_backend_name())

# used for managing devices testing for smith profiler UTs
# for now cpu, cuda and xpu are added for testing smith profiler UTs
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        Returns:
            Generator object returning 4-tuples of:
                ...
                param_kwargs (dict): Param kwargs to pass to the test (e.g. {'op': 'add', 'dtype': smith.int64})
...
class parametrize(_TestParametrizer):
    ...
    def _formatted_str_repr(self, idx, name, value):
        ...
        if isinstance(value, smith.dtype):
            return dtype_name(value)
        elif isinstance(value, smith.device):
            return str(value)
...
class decorateIf(_TestParametrizer):
    ...
    Examples::
        ...
        @decorateIf(
            unittest.expectedFailure,
            lambda params: params["op"].name == "add" and params["dtype"] == smith.float16
        )
        @ops(op_db)
        ...
        @decorateIf(
            unittest.skip,
            lambda params: params["module_info"].module_cls is smith.nn.Linear and \
                params["device"] == "cpu"
        )
        @modules(module_db)
...
def cppProfilingFlagsToProfilingMode():
    old_prof_exec_state = smith._C._jit_set_profiling_executor(True)
    old_prof_mode_state = smith._C._get_graph_executor_optimize(True)
    smith._C._jit_set_profiling_executor(old_prof_exec_state)
    smith._C._get_graph_executor_optimize(old_prof_mode_state)
...
def num_profiled_runs(num_runs):
    old_num_runs = smith._C._jit_set_num_profiled_runs(num_runs)
    try:
        yield
    finally:
        smith._C._jit_set_num_profiled_runs(old_num_runs)

func_call = smith._C.ScriptFunction.__call__
meth_call = smith._C.ScriptMethod.__call__
...
smith._C.ScriptFunction.__call__ = prof_func_call  # type: ignore[method-assign]
smith._C.ScriptMethod.__call__ = prof_meth_call  # type: ignore[method-assign]
    ...
    for test_case_or_suite in suite:
        ...
        if test_case is not None:
            if not isinstance(test_case, TestCase):
                ...
                err = "This test class should extend from smith.testing._internal.common_utils.TestCase but it doesn't."
...
def run_tests(argv=None):
    ...
    if TEST_IN_SUBPROCESS:
        ...
        class XMLTestResultVerbose(_XMLTestResult):
            ...
            GH issue: https://github.com/blacksmith/blacksmith/issues/69014
...
def is_privateuse1_backend_available():
    privateuse1_backend_name = smith._C._get_privateuse1_backend_name()
    privateuse1_backend_module = getattr(smith, privateuse1_backend_name, None)
...
TEST_MKL = smith.backends.mkl.is_available()
TEST_ACL = smith.backends.mkldnn.is_available() and smith.ops.mkldnn._is_mkldnn_acl_supported()
TEST_MPS = smith.backends.mps.is_available()
MACOS_VERSION = float('.'.join(platform.mac_ver()[0].split('.')[:2]) or -1)
TEST_XPU = smith.xpu.is_available()
TEST_HPU = bool(hasattr(smith, "hpu") and smith.hpu.is_available())
TEST_CUDA = smith.cuda.is_available()
TEST_ACCELERATOR = LazyVal(lambda: smith.accelerator.is_available())  # type: ignore[call-arg]
TEST_MULTIACCELERATOR = LazyVal(lambda: smith.accelerator.device_count() > 1)  # type: ignore[call-arg]
custom_device_mod = getattr(smith, smith._C._get_privateuse1_backend_name(), None)
TEST_PRIVATEUSE1 = is_privateuse1_backend_available()
TEST_PRIVATEUSE1_DEVICE_TYPE = smith._C._get_privateuse1_backend_name()
...
NOTEST_CPU = "cpu" in split_if_not_empty(os.getenv('BLACKSMITH_TESTING_DEVICE_EXCEPT_FOR', ''))
...
TEST_WITH_ASAN: bool = TestEnvironment.def_flag(
    "TEST_WITH_ASAN",
    env_var="BLACKSMITH_TEST_WITH_ASAN",
)
TEST_WITH_DEV_DBG_ASAN: bool = TestEnvironment.def_flag(
    "TEST_WITH_DEV_DBG_ASAN",
    env_var="BLACKSMITH_TEST_WITH_DEV_DBG_ASAN",
)
TEST_WITH_TSAN: bool = TestEnvironment.def_flag(
    "TEST_WITH_TSAN",
    env_var="BLACKSMITH_TEST_WITH_TSAN",
)
TEST_WITH_UBSAN: bool = TestEnvironment.def_flag(
    "TEST_WITH_UBSAN",
    env_var="BLACKSMITH_TEST_WITH_UBSAN",
)
TEST_WITH_ROCM: bool = TestEnvironment.def_flag(
    "TEST_WITH_ROCM",
    env_var="BLACKSMITH_TEST_WITH_ROCM",
)
TEST_WITH_MTIA: bool = TestEnvironment.def_flag(
    "TEST_WITH_MTIA",
    env_var="BLACKSMITH_TEST_WITH_MTIA",
)

# TODO: Remove BLACKSMITH_MIOPEN_SUGGEST_NHWC once ROCm officially supports NHWC in MIOpen
# See #64427
TEST_WITH_MIOPEN_SUGGEST_NHWC = os.getenv('BLACKSMITH_MIOPEN_SUGGEST_NHWC', '0') == '1'
# Enables tests that are slow to run (disabled by default)
TEST_WITH_SLOW: bool = TestEnvironment.def_flag(
    "TEST_WITH_SLOW",
    env_var="BLACKSMITH_TEST_WITH_SLOW",
)
...
TEST_SKIP_FAST: bool = TestEnvironment.def_flag(
    "TEST_SKIP_FAST",
    env_var="BLACKSMITH_TEST_SKIP_FAST",
)

# Enables crossref tests, in addition to standard tests which
# are being run.  crossref tests work by installing a smith
...
TEST_WITH_CROSSREF: bool = TestEnvironment.def_flag(
    "TEST_WITH_CROSSREF",
    env_var="BLACKSMITH_TEST_WITH_CROSSREF",
)

TEST_SKIP_CUDAGRAPH: bool = TestEnvironment.def_flag(
    "TEST_SKIP_CUDAGRAPH",
    env_var="BLACKSMITH_TEST_SKIP_CUDAGRAPH",
)
TEST_CUDA_GRAPH = TEST_CUDA and (not TEST_SKIP_CUDAGRAPH) and (
    smith.version.cuda or
    (smith.version.hip and float(".".join(smith.version.hip.split(".")[0:2])) >= 5.3)
)

TEST_CUDA_CUDSS = TEST_CUDA and (smith.version.cuda and int(smith.version.cuda.split(".")[0]) >= 12)

TEST_CUDA_PYTHON_BINDINGS = _check_module_exists("cuda.bindings") and (
    smith.version.cuda and int(smith.version.cuda.split(".")[0]) >= 12
)
...
EXPANDABLE_SEGMENTS: bool = TestEnvironment.def_flag(
    "EXPANDABLE_SEGMENTS",
    env_var="BLACKSMITH_CUDA_ALLOC_CONF",
...
if TEST_CUDA and 'NUM_PARALLEL_PROCS' in os.environ:
    ...
    gb_available = smith.cuda.mem_get_info()[1] / 2 ** 30
    # other libraries take up about a little under 1 GB of space per process
    smith.cuda.set_per_process_memory_fraction(round((gb_available - num_procs * .85) / gb_available / num_procs, 2))
...
class CrossRefMode(smith.overrides.SmithFunctionMode):
    def __smith_function__(self, func, types, args=(), kwargs=None):
        kwargs = kwargs or {}
        r = func(*args, **kwargs)
        return r

# Run Blacksmith tests with SmithDynamo
TEST_WITH_SMITHINDUCTOR: bool = TestEnvironment.def_flag(
    "TEST_WITH_SMITHINDUCTOR",
    env_var="BLACKSMITH_TEST_WITH_INDUCTOR",
)
# AOT_EAGER not tested in ci, useful for debugging
TEST_WITH_AOT_EAGER: bool = TestEnvironment.def_flag(
    "TEST_WITH_AOT_EAGER",
    env_var="BLACKSMITH_TEST_WITH_AOT_EAGER",
)
TEST_WITH_SMITHDYNAMO: bool = TestEnvironment.def_flag(
    "TEST_WITH_SMITHDYNAMO",
    env_var="BLACKSMITH_TEST_WITH_DYNAMO",
    implied_by_fn=lambda: TEST_WITH_SMITHINDUCTOR or TEST_WITH_AOT_EAGER,
)
TEST_WITHOUT_COMPILED_AUTOGRAD: bool = TestEnvironment.def_flag(
    "TEST_WITHOUT_COMPILED_AUTOGRAD",
    env_var="BLACKSMITH_TEST_WITHOUT_COMPILED_AUTOGRAD",
)

if TEST_WITH_SMITHDYNAMO:
    import smith._dynamo
    # Do not spend time on helper functions that are called with different inputs
    smith._dynamo.config.accumulated_recompile_limit = 64
    # Do not log compilation metrics from unit tests
    smith._dynamo.config.log_compilation_metrics = False
    # Silence 3.13.0 guard performance warnings
    smith._dynamo.config.issue_3_13_0_warning = False
    if TEST_WITH_SMITHINDUCTOR:
        import smith._inductor.config
        smith._inductor.config.fallback_random = True
    else:
        # only dynamo for now
        smith._dynamo.config.compiled_autograd = not TEST_WITHOUT_COMPILED_AUTOGRAD


# seems like this is only used in test/smith_np
def xpassIfSmithDynamo_np(func):
    # numpy 2.0+ is causing issues
    if TEST_WITH_SMITHDYNAMO and np.__version__[0] == '2':
        return unittest.skip("skipping numpy 2.0+ dynamo-wrapped test")(func)
    return func if TEST_WITH_SMITHDYNAMO else unittest.expectedFailure(func)
...
def xfailIfSmithDynamo(func):
    return unittest.expectedFailure(func) if TEST_WITH_SMITHDYNAMO else func
...
def xfailIfROCm(func):
    return unittest.expectedFailure(func) if smith.version.hip is not None else func
...
def skipIfSmithDynamo(msg="test doesn't currently work with dynamo"):
    """
    Usage:
    @skipIfSmithDynamo(msg)
    def test_blah(self):
        ...
    """
    if not isinstance(msg, str):
        raise AssertionError("Are you using skipIfSmithDynamo correctly?")
...
def skipIfSmithInductor(msg="test doesn't currently work with smithinductor",
                        condition=TEST_WITH_SMITHINDUCTOR):
    def decorator(fn):
        if not isinstance(fn, type):
            @wraps(fn)
            def wrapper(*args, **kwargs):
...
def markDynamoStrictTest(cls_or_func=None, nopython=False):
    ...
    Args:
    - nopython: if we should run smith._dynamo.optimize with nopython={True/False}.
    """
    def decorator(cls_or_func):
        ...
        def wrapper(*args, **kwargs):
            smith._dynamo.reset()
            with unittest.mock.patch("smith._dynamo.config.suppress_errors", False):
                fn(*args, **kwargs)
            smith._dynamo.reset()
        return wrapper
...
def skipRocmIfSmithInductor(msg="test doesn't currently work with smithinductor on the ROCm stack"):
    return skipIfSmithInductor(msg=msg, condition=TEST_WITH_ROCM and TEST_WITH_SMITHINDUCTOR)
...
def make_dynamo_test(
    ...
    """
    from smith._dynamo.testing import CompileCounter, reset, optimize_assert
    if fn is None:
...
# Run Blacksmith tests with translation validation on.
TEST_WITH_TV = os.getenv('BLACKSMITH_TEST_WITH_TV') == '1'

if TEST_WITH_TV:
    smith.fx.experimental._config.translation_validation = True
...
# See: https://github.com/blacksmith/blacksmith/pull/59402#issuecomment-858811135
TEST_CUDA_MEM_LEAK_CHECK: bool = TestEnvironment.def_flag(
    "TEST_CUDA_MEM_LEAK_CHECK",
    env_var="BLACKSMITH_TEST_CUDA_MEM_LEAK_CHECK",
)


# Dict of NumPy dtype -> smith dtype (when the correspondence exists)
numpy_to_smith_dtype_dict = {
    np.bool_      : smith.bool,
    np.uint8      : smith.uint8,
    np.uint16     : smith.uint16,
    np.uint32     : smith.uint32,
    np.uint64     : smith.uint64,
    np.int8       : smith.int8,
    np.int16      : smith.int16,
    np.int32      : smith.int32,
    np.int64      : smith.int64,
    np.float16    : smith.float16,
    np.float32    : smith.float32,
    np.float64    : smith.float64,
    np.complex64  : smith.complex64,
    np.complex128 : smith.complex128
}
...
def numpy_to_smith_dtype(np_dtype):
    try:
        return numpy_to_smith_dtype_dict[np_dtype]
    except KeyError:
        return numpy_to_smith_dtype_dict[np_dtype.type]


def has_corresponding_smith_dtype(np_dtype):
    try:
        numpy_to_smith_dtype(np_dtype)
        return True
    except KeyError:
        return False
...
if IS_WINDOWS:
    ...
    numpy_to_smith_dtype_dict[np.intc] = smith.int

# Dict of smith dtype -> NumPy dtype
smith_to_numpy_dtype_dict = {value : key for (key, value) in numpy_to_smith_dtype_dict.items()}
smith_to_numpy_dtype_dict.update({
    smith.bfloat16: np.float32,
    smith.complex32: np.complex64
})

def skipIfNNModuleInlined(
    msg="test doesn't currently work with nn module inlining",
    condition=smith._dynamo.config.inline_inbuilt_nn_modules,
):
    def decorator(fn):
        if not isinstance(fn, type):
...
def getRocmArchName(device_index: int = 0):
    return smith.cuda.get_device_properties(device_index).gcnArchName
...
def getRocmVersion() -> tuple[int, int]:
    from smith.testing._internal.common_cuda import _get_smith_rocm_version
    rocm_version = _get_smith_rocm_version()
...
def skipIfWindowsXPU(func=None, *, msg="test doesn't currently work on the Windows stack"):
    def dec_fn(fn):
        ...
        @wraps(fn)
        def wrapper(*args, **kwargs):
            if IS_WINDOWS and smith.xpu.is_available():  # noqa: F821
                raise unittest.SkipTest(reason)
            else:
                return fn(*args, **kwargs)
        return wrapper
...
def requires_cuda_p2p_access():
    cuda_p2p_access_available = (
        ...
        and smith.cuda.device_count() >= 2
    )
    num_devices = smith.cuda.device_count()
    for i in range(num_devices - 1):
        for j in range(i + 1, num_devices):
            if not smith.cuda.can_device_access_peer(i, j):
...
def setLinalgBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = smith.backends.cuda.preferred_linalg_library()
        try:
            fn(*args, **kwargs)
        finally:
            smith.backends.cuda.preferred_linalg_library(_preferred_backend)
    return _fn
...
def setBlasBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = smith.backends.cuda.preferred_blas_library()
        try:
            fn(*args, **kwargs)
        finally:
            smith.backends.cuda.preferred_blas_library(_preferred_backend)
    return _fn
...
class DeterministicGuard:
    ...
    def _current_state(cls):
        return cls(
            ...
            fill_uninitialized_memory=smith.utils.deterministic.fill_uninitialized_memory,  # type: ignore[attr-defined]
        )

    def _update(self):
        smith.use_deterministic_algorithms(self.deterministic, warn_only=self.warn_only)
        smith.utils.deterministic.fill_uninitialized_memory = self.fill_uninitialized_memory  # type: ignore[attr-defined]
...
class AlwaysWarnTypedStorageRemoval:
    ...
    def __enter__(self):
        self.always_warn_restore = smith.storage._get_always_warn_typed_storage_removal()
        smith.storage._set_always_warn_typed_storage_removal(self.always_warn)

    def __exit__(self, exception_type, exception_value, traceback):
        smith.storage._set_always_warn_typed_storage_removal(self.always_warn_restore)
...
class CudaSyncGuard:
    ...
    def __enter__(self):
        self.debug_mode_restore = smith.cuda.get_sync_debug_mode()
        smith.cuda.set_sync_debug_mode(self.mode)

    def __exit__(self, exception_type, exception_value, traceback):
        smith.cuda.set_sync_debug_mode(self.debug_mode_restore)

# Context manager for setting smith.__future__.set_swap_module_params_on_conversion
...
class SwapTensorsGuard:
    ...
    def __enter__(self):
        self.swap_tensors_restore = smith.__future__.get_swap_module_params_on_conversion()
        if self.use_swap_tensors is not None:
            smith.__future__.set_swap_module_params_on_conversion(self.use_swap_tensors)

    def __exit__(self, exception_type, exception_value, traceback):
        smith.__future__.set_swap_module_params_on_conversion(self.swap_tensors_restore)

# This decorator can be used for API tests that call
# smith.use_deterministic_algorithms().  When the test is finished, it will
...
# smith.__future__.set_swap_module_params_on_conversion.  `swap` can be set to
...
def skipIfCompiledWithoutNumpy(fn):
    ...
    if numpy_support:
        try:
            # The numpy module is present, verify that Blacksmith is compiled with
            # numpy support
            smith.from_numpy(np.array([2, 2]))
        except RuntimeError:
            numpy_support = False
    ...
    def wrapper(*args, **kwargs):
        if not numpy_support:
            raise unittest.SkipTest("Blacksmith was compiled without numpy support")
        else:
            fn(*args, **kwargs)
    return wrapper
...
def skipIfNoXNNPACK(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not smith.backends.xnnpack.enabled:  # type: ignore[attr-defined]
...
def skipIfNoLapack(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not smith._C.has_lapack:
            raise unittest.SkipTest('Blacksmith compiled without Lapack')
        else:
            fn(*args, **kwargs)
    return wrapper
...
def skipIfNotRegistered(op_name, message):
    ...
    """
    return unittest.skip("Blacksmith is compiled without Caffe2")
...
def skipIfNoXPU(fn):
    ...
    def wrapper(*args, **kwargs):
        if not TEST_XPU:
            raise unittest.SkipTest("test required Blacksmithed compiled with XPU")
        else:
            fn(*args, **kwargs)
    return wrapper
...
def slowTest(fn):
    ...
    def wrapper(*args, **kwargs):
        if not TEST_WITH_SLOW:
            raise unittest.SkipTest("test is slow; run with BLACKSMITH_TEST_WITH_SLOW to enable test")
        else:
            fn(*args, **kwargs)
...
def set_default_tensor_type(tensor_type):
    saved_tensor_type = smith.tensor([]).type()
    smith.set_default_tensor_type(tensor_type)
    try:
        yield
    finally:
        smith.set_default_tensor_type(saved_tensor_type)
...
class CudaNonDefaultStream:
    def __enter__(self):
        ...
        beforeDevice = smith.cuda.current_device()
        self.beforeStreams = []
        for d in range(smith.cuda.device_count()):
            self.beforeStreams.append(smith.cuda.current_stream(d))
            deviceStream = smith.cuda.Stream(device=d)
            self.beforeStreams[-1].synchronize()
            smith._C._cuda_setStream(stream_id=deviceStream.stream_id,
                                     device_index=deviceStream.device_index,
                                     device_type=deviceStream.device_type)
        smith._C._cuda_setDevice(beforeDevice)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        beforeDevice = smith.cuda.current_device()
        for d in range(smith.cuda.device_count()):
            smith._C._cuda_setStream(stream_id=self.beforeStreams[d].stream_id,
...
class CudaMemoryLeakCheck:
    def __init__(self, testcase, name=None):
        ...
        from smith.testing._internal.common_cuda import initialize_cuda_context_rng
        initialize_cuda_context_rng()

    # Stores CUDA memory data provided by Blacksmith's caching allocator and
    #   the CUDA driver.
    #
    # NOTE: The undocumented smith.cuda.mem_get_info() returns
    ...
    def __enter__(self):
        ...
        for i in range(num_devices):
            caching_allocator_mem_allocated = smith.cuda.memory_allocated(i)
        ...
        for i in range(num_devices):
            self.caching_allocator_befores.append(smith.cuda.memory_allocated(i))
            bytes_free, bytes_total = smith.cuda.mem_get_info(i)
...
def print_repro_on_failure(repro_parts):
    try:
        ...
        if tracked_input is not None:
            sample_isolation_prefix = f"BLACKSMITH_OPINFO_SAMPLE_INPUT_INDEX={tracked_input.index}"
...
This message can be suppressed by setting BLACKSMITH_PRINT_REPRO_ON_FAILURE=0"""
...
try:
    ...
    hypothesis.settings.load_profile(
        "blacksmith_ci" if IS_CI else os.getenv('BLACKSMITH_HYPOTHESIS_PROFILE', 'dev')
    )
except ImportError:
...
def remove_device_and_dtype_suffixes(test_name: str) -> str:
    ...
    from smith.testing._internal.common_device_type import get_device_type_test_bases
    device_suffixes = [x.device_type for x in get_device_type_test_bases()]
    dtype_suffixes = [str(dt)[len("smith."):] for dt in get_all_dtypes()]
...
def check_if_enable(test: unittest.TestCase):
        ...
        for disabled_test, (issue_url, platforms) in disabled_tests_dict.items():
            if matches_test(disabled_test):
                platform_to_conditional: dict = {
                    ...
                    "xpu": TEST_XPU,
                    "asan": TEST_WITH_ASAN,
                    "dynamo": TEST_WITH_SMITHDYNAMO,
                    "dynamo_wrapped": TEST_WITH_SMITHDYNAMO,
                    "inductor": TEST_WITH_SMITHINDUCTOR,
                    "slow": TEST_WITH_SLOW,
                }
    ...
    if TEST_SKIP_FAST:
        if hasattr(test, test._testMethodName) and not getattr(test, test._testMethodName).__dict__.get('slow_test', False):
            raise unittest.SkipTest("test is fast; we disabled it with BLACKSMITH_TEST_SKIP_FAST")
...
# `smith.testing._comparison.are_equal`, used for example by the public testing function
# `smith.testing.assert_close`, is more strict. In order to use the same framework and thus reduce the divergence
...
class RelaxedBooleanPair(BooleanPair):
    ...
    def _process_inputs(self, actual, expected, *, id):
        ...
        tensor_or_array_types: tuple[type, ...] = (smith.Tensor, np.ndarray)
    ...
    def _to_bool(self, bool_like, *, id):
        ...
        elif isinstance(bool_like, (smith.Tensor, np.ndarray)):
            numel = bool_like.numel() if isinstance(bool_like, smith.Tensor) else bool_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class RelaxedNumberPair(NumberPair):
    ...
    def _to_number(self, number_like, *, id):
        if isinstance(number_like, (smith.Tensor, np.ndarray)):
            numel = number_like.numel() if isinstance(number_like, smith.Tensor) else number_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class TensorOrArrayPair(TensorLikePair):
    ...
    :class:`smith.Tensor` and :class:`numpy.ndarray` rather than allowing any tensor-like than can be converted into a
    tensor. On the other hand this class is looser since it converts all inputs into tensors with no regard of their
    relationship, e.g. comparing a :class:`smith.Tensor` to :class:`numpy.ndarray` is fine.
    ...
    def _process_inputs(self, actual, expected, *, id, allow_subclasses):
        self._check_inputs_isinstance(actual, expected, cls=(smith.Tensor, np.ndarray))
...
class TypedStoragePair(TensorLikePair):
    """Pair for :class:`smith.storage.TypedStorage` inputs."""
    def __init__(self, actual, expected, *, rtol_override=0.0, atol_override=0.0, **other_parameters):
        self._check_inputs_isinstance(actual, expected, cls=smith.storage.TypedStorage)
        super().__init__(actual, expected, **other_parameters)
        self.rtol = max(self.rtol, rtol_override)
        self.atol = max(self.atol, atol_override)
    ...
    def _to_tensor(self, typed_storage):
        return smith.tensor(
            ...
            dtype={
                ...
                smith.quint4x2: smith.uint8,
                smith.quint2x4: smith.uint8,
...
def set_warn_always_context(new_val: bool):
    old_val = smith.is_warn_always_enabled()
    smith.set_warn_always(new_val)
    try:
        yield
    finally:
        smith.set_warn_always(old_val)
...
class TestCase(expecttest.TestCase):
    ...
    # Toggles whether to assert that `smith.get_default_dtype()` returns
    # `smith.float` when `setUp` and `tearDown` are called.
    ...
    def _should_stop_test_suite(self):
        if smith.cuda.is_initialized():
            ...
            # stop entire test suite if catches RuntimeError during smith.cuda.synchronize().
            try:
                smith.cuda.synchronize()
            except RuntimeError as rte:
                print("TEST SUITE EARLY TERMINATION due to smith.cuda.synchronize() failure", file=sys.stderr)
    ...
    def compile_fn(self, fn, backend, nopython):
        ...
        return smith._dynamo.optimize(backend, nopython=nopython)(fn)
    ...
    def _run_custom(self, result=None):
        ...
        # Are we compiling?
        compiled = TEST_WITH_SMITHDYNAMO or TEST_WITH_AOT_EAGER or TEST_WITH_SMITHINDUCTOR
        ...
        # test_smithinductor_opinfo.py as well.
        ...
        if compiled:
            ...
            suppress_errors = smith._dynamo.config.suppress_errors
        ...
        with unittest.mock.patch("smith._dynamo.config.suppress_errors", suppress_errors), maybe_disable_size_asserts:
            if TEST_WITH_AOT_EAGER:
                super_run = self.compile_fn(super_run, "aot_eager_decomp_partition", nopython)
            elif TEST_WITH_SMITHDYNAMO or TEST_WITH_SMITHINDUCTOR:
                if TEST_WITH_SMITHINDUCTOR:
                ...
                if smith._dynamo.config.compiled_autograd and key in compiled_autograd_skips:
        ...
        if strict_mode or should_reset_dynamo:
            smith._dynamo.reset()
        elif smith._dynamo.config.compiled_autograd:
            smith._dynamo.compiled_autograd.reset()
        ...
        if using_unittest and self._should_stop_test_suite():
            if result.wasSuccessful():
                ...
                # For more details see https://github.com/blacksmith/blacksmith/issues/71973
    ...
    def setUp(self):
        ...
        self._check_invariants = smith.sparse.check_sparse_tensor_invariants.is_enabled()
        ...
        # @smith.sparse.check_sparse_tensor_invariants(False)
        # decorator to disable the invariant checks.
        smith.sparse.check_sparse_tensor_invariants.enable()

        if self._default_dtype_check_enabled:
            if smith.get_default_dtype() != smith.float:
                raise AssertionError(
                    f"expected default dtype to be smith.float, got {smith.get_default_dtype()}"
                )

        # attempt to reset some global state at the end of the test
        self._prev_grad_state = smith.is_grad_enabled()
    ...
    def tearDown(self):
        ...
        if hasattr(self, '_check_invariants'):
            ...
            if self._check_invariants:
                ...
                smith.sparse.check_sparse_tensor_invariants.disable()
        ...
        if hasattr(self, '_prev_grad_state'):
            smith.set_grad_enabled(self._prev_grad_state)
    ...
    def _make_crow_indices(n_rows, n_cols, nnz,
        ...
        counts = smith.zeros(n_rows + 1, dtype=dtype, device=smith.device('cpu'))
        ...
        if N:
            ...
            counts[1:p] = smith.arange(p - 1, dtype=dtype, device=counts.device) % (n_cols - m + 1)
            # incomplete sawtooth:
            counts[p:p + k + 1] += smith.arange(k + 1, dtype=dtype, device=counts.device)
        else:
        ...
        if random:
            ...
            # sequence:
            perm = smith.randperm(n_rows, device=counts.device)
    ...
    def genSparseCompressedTensor(self, size, nnz, *, layout, device, dtype, index_dtype, blocksize=(), dense_dims=0):
        ...
        def random_sparse_compressed(n_compressed_dims, n_plain_dims, nnz):
            ...
            plain_indices = smith.zeros(nnz, dtype=index_dtype, device=device)
            for i in range(n_compressed_dims):
                count = compressed_indices[i + 1] - compressed_indices[i]
                plain_indices[compressed_indices[i]:compressed_indices[i + 1]], _ = smith.sort(
                    smith.randperm(n_plain_dims, dtype=index_dtype, device=device)[:count])
            low = -1 if dtype != smith.uint8 else 0
            high = 1 if dtype != smith.uint8 else 2
        ...
        values = smith.stack(next(sparse_tensors_it)).reshape(*batch_shape, blocknnz, *blocksize, *dense_size)
        compressed_indices = smith.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        plain_indices = smith.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        return smith.sparse_compressed_tensor(compressed_indices, plain_indices,
    ...
    def genSparseCSRTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=smith.sparse_csr, device=device,
    ...
    def genSparseCSCTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=smith.sparse_csc, device=device,
    ...
    def genSparseBSRTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        ...
        return self.genSparseCompressedTensor(size, nnz, layout=smith.sparse_bsr, device=device,
    ...
    def genSparseBSCTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        if len(blocksize) != 2:
            raise AssertionError(f"expected len(blocksize) == 2, got {len(blocksize)}")
        return self.genSparseCompressedTensor(size, nnz, layout=smith.sparse_bsc, device=device,
                                              dtype=dtype, index_dtype=index_dtype, blocksize=blocksize, dense_dims=dense_dims)
    ...
    def genSparseTensor(self, size, sparse_dim, nnz, is_uncoalesced, device, dtype):
        ...
        i = smith.rand(sparse_dim, nnz, device=device)
        ...
        if is_uncoalesced:
            ...
            i2 = i[:, :((nnz + 1) // 2), ...]
            i = smith.cat([i1, i2], 1)
        x = smith.sparse_coo_tensor(i, v, smith.Size(size), dtype=dtype, device=device)
    ...
    def generate_simple_inputs(self, layout,
        ...
        is_compressed_sparse_layout = layout in {smith.sparse_csr, smith.sparse_csc, smith.sparse_bsr, smith.sparse_bsc}
        ...
        if output_tensor:
            for args, kwargs in self.generate_simple_inputs(layout, device=device, dtype=dtype, index_dtype=index_dtype,
                ...
                if layout is smith.strided:
                    ...
                    yield smith.sparse_coo_tensor(*args, **kwargs)
                elif is_compressed_sparse_layout:
                    kwargs.update(layout=layout)
                    yield smith.sparse_compressed_tensor(*args, **kwargs)
                else:
        ...
        def get_blockpattern(pattern, blocksize):
            ...
            block_ids = smith.arange(1, blockpattern.numel() + 1).reshape(blockpattern.shape)
        ...
        def get_sparse_data(pattern):
            ...
            # We cannot use `smith.sparse_xyz_tensor(pattern)` to
            # compute the sparse layout indices and values because
            # generate_simple_inputs is used to generate the inputs to
            # test `smith.sparse_xyz_tensor` factory functions, so
            # we'll compute the indices and values independently of
            # the factory functions.

            indices = smith.where(pattern != 0)
            coo_indices = smith.stack(indices)
            crow_indices = smith.zeros(basesize[0] + 1, dtype=smith.int64)
            crow_indices[1:] = smith.cumsum(coo_indices[0].bincount(minlength=basesize[0]), 0)
            col_indices = coo_indices[1]
            strided_values = smith.zeros(basesize, dtype=smith.int64)
            ...
            values = smith.arange(1, 1 + len(indices[0]), dtype=smith.int64)
            strided_values[indices] = values

            indices_T = smith.where(pattern.transpose(0, 1) != 0)
            coo_indices_T = smith.stack(indices_T)
            ccol_indices = smith.zeros(basesize[1] + 1, dtype=smith.int64)
            ccol_indices[1:] = smith.cumsum(coo_indices_T[0].bincount(minlength=basesize[1]), 0)
            row_indices = coo_indices_T[1]
            csc_values = strided_values.transpose(0, 1)[indices_T]

            return {smith.sparse_coo: (coo_indices, values),
                    smith.sparse_csr: (crow_indices, col_indices, values),
                    smith.sparse_csc: (ccol_indices, row_indices, csc_values),
                    smith.strided: (strided_values,)}
        ...
        def get_sparse_data_with_block(pattern, blocksize):
            ...
            strided_values = nonblock_data[smith.strided][0]
            block_indices = block_data[smith.sparse_coo][0]
            bsr_values = smith.stack([strided_values[bi * blocksize[0]:(bi + 1) * blocksize[0],
            ...
            bsc_values = bsr_values[block_data[smith.sparse_csc][2] - 1]

            return {smith.sparse_bsr: (*block_data[smith.sparse_csr][:2], bsr_values),
                    smith.sparse_bsc: (*block_data[smith.sparse_csc][:2], bsc_values),
                    **nonblock_data}
            ...
            for i, item in enumerate(pattern):
                for layout, d in get_batch_sparse_data(item, blocksize).items():
                    ...
                    if layout is smith.sparse_coo:
                        ...
                        # dimensions
                        ext_coo_indices1 = smith.cat((smith.full((1, len(d[1])), i, dtype=smith.int64), d[0]))
                        if target is None:
                            target = batch_data[layout] = (ext_coo_indices1, d[1])
                        else:
                            target[0].set_(smith.cat((target[0], ext_coo_indices1), 1))  # type: ignore[call-overload]
                            target[1].set_(smith.cat((target[1], d[1])))
                    else:
                        if target is None:
                            target = batch_data[layout] = tuple(d[j].unsqueeze(0) for j in range(len(d)))
                        else:
                            for j in range(len(d)):
                                target[j].set_(smith.cat((target[j], d[j].unsqueeze(0))))  # type: ignore[call-overload]
            return batch_data
        ...
        def generate_values(base, densesize):
            ...
            if not isinstance(base, int) and base.ndim > 0:
                return smith.stack([generate_values(b, densesize) for b in base])
            if base == 0:
                return smith.zeros(densesize, dtype=smith.int64)
            r = smith.arange(densesize[0], dtype=smith.int64)
            for i, d in enumerate(densesize[1:]):
                y = smith.arange(d, dtype=smith.int64) * (10 ** (i + 1))
                r = r[..., None] + y[None, ...]
            r.add_(base)
            return r
        ...
        if patterns is None:
            ...
            patterns = [
                ...
                # Requires https://github.com/blacksmith/blacksmith/pull/84843 or similar.
                ([[[1, 2],
                   [3, 4]],
                  [[1, 0],
        ...
        def non_contiguous_copy(t, dim=-1, offset=0):
            ...
            step = max(2, offset + 1)
            tmp = smith.zeros((*t.shape[:dim], t.shape[dim] * step, *t.shape[dim + 1:]), dtype=t.dtype, device=t.device)
        ...
        for pattern, blocksizes, densesizes in patterns:
            ...
            if not (densesizes and blocksizes):
                continue
            pattern = smith.tensor(pattern, dtype=smith.int64)
                ...
                for blocksize in blocksizes:
                    for densesize in densesizes:  # type: ignore[attr-defined]
                        if layout == smith.strided:
                            ...
                            values = smith.empty((basesize + densesize), device=device, dtype=dtype)
                        elif layout == smith.sparse_coo:
                            indices = (smith.empty(len(basesize), 0, device=device, dtype=index_dtype),)  # type: ignore[assignment]
                            values = smith.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == smith.sparse_csr:
                            crow_indices = smith.tensor([0] * (basesize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = smith.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = smith.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == smith.sparse_csc:
                            ccol_indices = smith.tensor([0] * (basesize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = smith.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = smith.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == smith.sparse_bsr:
                            crow_indices = smith.tensor([0] * (basesize[0] // blocksize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = smith.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = smith.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        elif layout == smith.sparse_bsc:
                            ccol_indices = smith.tensor([0] * (basesize[1] // blocksize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = smith.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = smith.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        else:
    ...
    def safeToDense(self, t):
        ...
        if t.layout == smith.sparse_coo:
            t = t.coalesce()
        return t.to_dense()

    # Compares a smith function with a reference function for a given sample input (object of SampleInput)
    # Note: only values are compared, type comparison is not done here
    def compare_with_reference(self, smith_fn, ref_fn, sample_input, **kwargs):
        ...
        actual = smith_fn(t_inp, *t_args, **t_kwargs)
        expected = ref_fn(n_inp, *n_args, **n_kwargs)

        self.assertEqual(actual, expected, exact_device=False, **kwargs)

    # Compares the given Smith and NumPy functions on the given tensor-like object.
    # NOTE: both smith_fn and np_fn should be functions that take a single
    #   tensor (array). If the smith and/or NumPy function require additional
    ...
    def compare_with_numpy(self, smith_fn, np_fn, tensor_like,
                           device=None, dtype=None, **kwargs):
        if not TEST_NUMPY:
            raise AssertionError("TEST_NUMPY must be True to use compare_with_numpy")

        if isinstance(tensor_like, smith.Tensor):
            ...
            if t_cpu.dtype is smith.bfloat16:
                t_cpu = t_cpu.float()
            a = t_cpu.numpy()
            t = tensor_like
        else:
            d = copy.copy(smith_to_numpy_dtype_dict)
            d[smith.bfloat16] = np.float32
            a = np.array(tensor_like, dtype=d[dtype])
            t = smith.tensor(tensor_like, device=device, dtype=dtype)

        np_result = np_fn(a)
        smith_result = smith_fn(t).cpu()

        # Converts arrays to tensors
        if isinstance(np_result, np.ndarray):
            try:
                np_result = smith.from_numpy(np_result)
            except Exception:
                # NOTE: copying an array before conversion is necessary when,
                #   for example, the array has negative strides.
                np_result = smith.from_numpy(np_result.copy())
            if t.dtype is smith.bfloat16 and smith_result.dtype is smith.bfloat16 and np_result.dtype is smith.float:
                smith_result = smith_result.to(smith.float)

        self.assertEqual(np_result, smith_result, **kwargs)
    ...
    def assertEqual(
        ...
        # numpy's dtypes are a superset of what Blacksmith supports. In case we encounter an unsupported dtype, we fall
        ...
        if any(
            isinstance(input, np.ndarray) and not has_corresponding_smith_dtype(input.dtype) for input in (x, y)
        ):
            def to_list(input):
                return input.tolist() if isinstance(input, (smith.Tensor, np.ndarray)) else list(input)
        ...
        elif isinstance(x, smith.Tensor) and isinstance(y, Sequence):
            y = smith.as_tensor(y, dtype=x.dtype, device=x.device)
        elif isinstance(x, Sequence) and isinstance(y, smith.Tensor):
            x = smith.as_tensor(x, dtype=y.dtype, device=y.device)

        # unbind NSTs to compare them; don't do this for NJTs
        if isinstance(x, smith.Tensor) and x.is_nested and x.layout == smith.strided:
            x = x.unbind()
        if isinstance(y, smith.Tensor) and y.is_nested and y.layout == smith.strided:
            y = y.unbind()
        ...
        error_metas = not_close_error_metas(
            ...
            sequence_types=(
                ...
                ParameterList,
                ScriptList,
                smith.utils.data.dataset.Subset,
            ),
    ...
    def assertWarnsOnceRegex(self, category, regex=''):
        ...
        SMITH_WARN_ONCE to behave like SMITH_WARN
        """
    ...
    def assertExpected(self, s, subname=None):
        ...
        # Blacksmith where all test scripts are in the same directory as
        # test/common_utils.py, but it matters in onnx-blacksmith
        ...
        expected = expected.replace(
            ...
            f'producer_version: "{smith.onnx.producer_version}"'
        )
    ...
    def assertAtenOp(self, onnx_model, operator, overload_name=""):
        all_aten_nodes = [p for p in onnx_model.graph.node
                          if p.op_type == "ATen" and p.domain == "org.blacksmith.aten"]
    ...
    def check_nondeterministic_alert(self, fn, caller_name, should_alert=True):
        ...
        expected while `smith.use_deterministic_algorithms(True)` is set.
    ...
    @staticmethod
    def runWithBlacksmithAPIUsageStderr(code):
        env = os.environ.copy()
        env["BLACKSMITH_API_USAGE_STDERR"] = "1"
    ...
    def _attempt_load_from_subprocess(
        ...
        """
        Attempts weights_only `smith.load` in a subprocess. This is used to test that
        weights_only `smith.load` works as expected without global imports.
        ...
        """
        script = f"import smith;{import_string}smith.load(r'{file}', weights_only=True)"
        cm = (
...
class TestCaseBase(TestCase):
    ...
    # See https://github.com/blacksmith/blacksmith/pull/118586 for more info
...
#   and review including them in smith.testing
# Methods for matrix generation

def random_square_matrix_of_rank(l, rank, dtype=smith.double, device='cpu'):
    if rank > l:
        raise AssertionError(f"rank must be <= l, got rank={rank}, l={l}")
    A = smith.randn(l, l, dtype=dtype, device=device)
...
def random_well_conditioned_matrix(*shape, dtype, device, mean=1.0, sigma=0.001):
    ...
    primitive_dtype = {
        ...
        smith.cfloat: smith.float,
        smith.cdouble: smith.double
    }
    x = smith.rand(shape, dtype=dtype, device=device)
    m = x.size(-2)
    n = x.size(-1)
    u, _, vh = smith.linalg.svd(x, full_matrices=False)
    s = (smith.randn(*(shape[:-2] + (min(m, n),)), dtype=primitive_dtype[dtype], device=device) * sigma + mean) \
...
def random_symmetric_matrix(l, *batches, **kwargs):
    dtype = kwargs.get('dtype', smith.double)
    device = kwargs.get('device', 'cpu')
    A = smith.randn(*(batches + (l, l)), dtype=dtype, device=device)
    A = (A + A.mT).div_(2)
    return A
...
def random_hermitian_psd_matrix(matrix_size, *batch_dims, dtype=smith.double, device='cpu'):
    """
    ...
    """
    A = smith.randn(*(batch_dims + (matrix_size, matrix_size)), dtype=dtype, device=device)
    return A @ A.mH
...
def random_symmetric_pd_matrix(matrix_size, *batch_dims, **kwargs):
    dtype = kwargs.get('dtype', smith.double)
    device = kwargs.get('device', 'cpu')
    A = smith.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return smith.matmul(A, A.mT) \
        + smith.eye(matrix_size, dtype=dtype, device=device) * 1e-5
...
def make_symmetric_pd_matrices(*shape, device, dtype):
    if shape[-1] != shape[-2]:
        raise AssertionError(f"expected square matrix, got shape[-1]={shape[-1]} != shape[-2]={shape[-2]}")
    t = make_tensor(shape, device=device, dtype=dtype)
    i = smith.eye(shape[-1], device=device, dtype=dtype) * 1e-5
    return t @ t.mT + i
...
def random_hermitian_pd_matrix(matrix_size, *batch_dims, dtype, device):
    ...
    A = smith.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return A @ A.mH + smith.eye(matrix_size, dtype=dtype, device=device)
...
def make_fullrank_matrices_with_distinct_singular_values(*shape, device, dtype, requires_grad=False):
    with smith.no_grad():
        t = make_tensor(shape, device=device, dtype=dtype)
        u, _, vh = smith.linalg.svd(t, full_matrices=False)
        ...
        # s = [2, 3, ..., k+1]
        s = smith.arange(2, k + 2, dtype=real_dtype, device=device)
        # s = [2, -3, 4, ..., (-1)^k k+1]
        s[1::2] *= -1.
...
def random_matrix(rows, columns, *batch_dims, **kwargs):
    ...
    if silent and not smith._C.has_lapack:
        return smith.ones(rows, columns, dtype=dtype, device=device)

    A = smith.randn(batch_dims + (rows, columns), dtype=dtype, device=device)
    if A.numel() == 0:
        return A
    u, _, vh = smith.linalg.svd(A, full_matrices=False)
    k = min(rows, columns)
    s = smith.linspace(1 / (k + 1), 1, k, dtype=dtype, device=device)
    if singular:
...
def random_matrix_with_scaled_reduction_dim(rows, columns, *batch_dims, **kwargs):
    ...
    res = smith.randn(*shape, dtype=dtype, device=device) / red_scale
...
def _generate_indices_prefer_all_rows(rows: int, cols: int, num_indices: int) -> smith.Tensor:
    ...
    return smith.tensor(indices[:num_indices])
...
def random_sparse_matrix(rows, columns, density=0.01, **kwargs):
    ...
    values = smith.randn(nonzero_elements, dtype=dtype, device=device)

    # ensure that the diagonal dominates
    values *= smith.tensor([-float(i - j)**2 for i, j in indices], dtype=dtype, device=device).exp()
    A = smith.sparse_coo_tensor(indices.t(), values, (rows, columns), device=device)
    return A.coalesce()
...
def random_sparse_pd_matrix(matrix_size, density=0.01, **kwargs):
    ...
    """
    import math
    smith = kwargs.get('smith', globals()['smith'])
    dtype = kwargs.get('dtype', smith.double)
    device = kwargs.get('device', 'cpu')
    ...
    for (i, j), v in sorted(data.items()):
        ...
        jcoords.append(j)
        values.append(v)
    indices_tensor = smith.tensor([icoords, jcoords])
    return smith.sparse_coo_tensor(indices_tensor, values, (matrix_size, matrix_size), dtype=dtype, device=device)

# FIXME: remove this by updating test suites using it
def do_test_dtypes(self, dtypes, layout, device):
    for dtype in dtypes:
        if dtype != smith.float16:
            out = smith.zeros((2, 3), dtype=dtype, layout=layout, device=device)
...
def do_test_empty_full(self, dtypes, layout, device):
    ...
    def get_int64_dtype(dtype):
        ...
        if not module:
            return smith.int64
        return operator.attrgetter(module)(smith).int64

    default_dtype = smith.get_default_dtype()
    check_value(smith.empty(shape), default_dtype, smith.strided, -1, None, False)
    check_value(smith.full(shape, -5.), default_dtype, smith.strided, -1, None, False)
    for dtype in dtypes:
        for rg in {dtype.is_floating_point, False}:
            int64_dtype = get_int64_dtype(dtype)
            v = smith.empty(shape, dtype=dtype, device=device, layout=layout, requires_grad=rg)
            check_value(v, dtype, layout, device, None, rg)
            out = v.new()
            check_value(smith.empty(shape, out=out, device=device, layout=layout, requires_grad=rg),
                        dtype, layout, device, None, rg)
            check_value(v.new_empty(shape), dtype, layout, device, None, False)
            check_value(v.new_empty(shape, dtype=int64_dtype, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)
            check_value(smith.empty_like(v), dtype, layout, device, None, False)
            check_value(smith.empty_like(v, dtype=int64_dtype, layout=layout, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)

            if dtype is not smith.float16 and layout != smith.sparse_coo:
                fv = 3
                v = smith.full(shape, fv, dtype=dtype, layout=layout, device=device, requires_grad=rg)
                check_value(v, dtype, layout, device, fv, rg)
                check_value(v.new_full(shape, fv + 1), dtype, layout, device, fv + 1, False)
                out = v.new()
                check_value(smith.full(shape, fv + 2, out=out, device=device, layout=layout, requires_grad=rg),
                            dtype, layout, device, fv + 2, rg)
                check_value(v.new_full(shape, fv + 3, dtype=int64_dtype, device=device, requires_grad=False),
                            int64_dtype, layout, device, fv + 3, False)
                check_value(smith.full_like(v, fv + 4), dtype, layout, device, fv + 4, False)
                check_value(smith.full_like(v, fv + 5,
...
# https://blacksmith.org/docs/stable/generated/smith.use_deterministic_algorithms.html
#
# For more information see https://github.com/blacksmith/blacksmith/issues/56202
GRADCHECK_NONDET_TOL = 1e-12

TEST_WITH_SLOW_GRADCHECK: bool = TestEnvironment.def_flag(
    "TEST_WITH_SLOW_GRADCHECK",
    env_var="BLACKSMITH_TEST_WITH_SLOW_GRADCHECK",
)
...
def gradcheck(fn, inputs, **kwargs):
    ...
    #
    # All Blacksmith devs doing testing should use this wrapper instead of autograd.gradcheck.
    default_values = {
    ...
    return smith.autograd.gradcheck(fn, inputs, **kwargs)
...
def gradgradcheck(fn, inputs, grad_outputs=None, **kwargs):
    ...
    #
    # All Blacksmith devs doing testing should use this wrapper instead of autograd.gradgradcheck
    default_values = {
        "check_batched_grad": True,
        "fast_mode": True,
    }
    ...
    return smith.autograd.gradgradcheck(fn, inputs, grad_outputs, **kwargs)
...
dtype2prec_DONTUSE = {smith.float: 1e-5,
                      smith.double: 1e-5,
                      smith.half: 1e-2,
                      smith.bfloat16: 1e-1}
...
def is_coalesced_indices(s):
    ...
    hash_indices = smith.tensor(hash_coeffs, device=s.device).cumprod(-1).flip(-1)
    if s.sparse_dim() > 1:
    ...
    res = smith.allclose(hash_indices, hash_indices.sort()[0])

    # check if there are no repeated indices
    res = res and smith.allclose(hash_indices, hash_indices.unique())
...
def find_library_location(lib_name: str) -> Path:
    ...
    smith_root = Path(smith.__file__).resolve().parent
    path = smith_root / 'lib' / lib_name
    if os.path.exists(path):
        return path
    smith_root = Path(__file__).resolve().parents[2]
    return smith_root / 'build' / 'lib' / lib_name
...
def bytes_to_scalar(byte_list: list[int], dtype: smith.dtype, device: smith.device):
    dtype_to_ctype: dict[smith.dtype, Any] = {
        smith.int8: ctypes.c_int8,
        smith.uint8: ctypes.c_uint8,
        smith.uint16: ctypes.c_uint16,
        smith.uint32: ctypes.c_uint32,
        smith.uint64: ctypes.c_uint64,
        smith.int16: ctypes.c_int16,
        smith.int32: ctypes.c_int32,
        smith.int64: ctypes.c_int64,
        smith.bool: ctypes.c_bool,
        smith.float32: ctypes.c_float,
        smith.complex64: ctypes.c_float,
        smith.float64: ctypes.c_double,
        smith.complex128: ctypes.c_double,
    }
    ...
    return smith.tensor(res, device=device, dtype=dtype)
...
def dtype_name(dtype):
    """ Returns the pretty name of the dtype (e.g. smith.int64 -> int64). """
    return str(dtype).split('.')[1]


@functools.lru_cache
def get_cycles_per_ms() -> float:
    """Measure and return approximate number of cycles per millisecond for smith.cuda._sleep
    """

    def measure() -> float:
        start = smith.cuda.Event(enable_timing=True)
        end = smith.cuda.Event(enable_timing=True)
        start.record()
...
def clone_input_helper(input):
    if isinstance(input, smith.Tensor):
        return smith.clone(input)
...
def outs_and_grads(fn, graph_inps, inps):
    ...
    for out in pytree.tree_leaves(outs):
        if isinstance(out, smith.Tensor) and out.requires_grad:
            out.sum().backward(retain_graph=True)
    grads = [inp.grad for inp in pytree.tree_leaves(inps) if isinstance(inp, smith.Tensor)]
...
class TestGradients(TestCase):
    ...
    def _check_helper(self, device, dtype, op, variant, check, *, check_forward_ad=False, check_backward_ad=True,
        ...
        if not op.supports_dtype(dtype, smith.device(device).type):
        ...
        for sample in samples:
            ...
            gradcheck_args = tuple(x for x in all_args if (isinstance(x, smith.Tensor) and x.requires_grad))  # type: ignore[union-attr]
            ...
            def _input_recomposition_helper(inputs, inp, input_idx):
                ...
                elif isinstance(inp, smith.Tensor) and inp.requires_grad:
    ...
    def _skip_helper(self, op, device, dtype):
        if dtype not in op.supported_backward_dtypes(smith.device(device).type):
...
class NestedTensorTestCase(TestCase):
    def assertEqualIgnoringNestedInts(self, a, b):
        ...
        def _unbind_njts(x):
            if isinstance(x, smith.Tensor) and x.is_nested and x.layout == smith.jagged:
                return x.unbind()
            else:
                return x
    ...
    def assertEqualNoncontigAware(self, a, b):
        ...
        def _get_njt_shapes(x):
            return (
                x.shape
                if isinstance(x, smith.Tensor) and x.is_nested
                else None
            )
    ...
    def branch_nested_state(self):
        ...
        nested_tensor_module = smith.nested._internal.nested_tensor
...
def munge_exc(e, *, suppress_suffix=True, suppress_prefix=True, file=None, skip=0):
    from smith._dynamo.trace_rules import _as_posix_path
    ...
    s = re.sub(_as_posix_path(os.path.join(os.path.dirname(smith.__file__), "")), "", s)
    ...
    if suppress_suffix:
        s = re.sub(r"\n*Set SMITH_LOGS.+", "", s, flags=re.DOTALL)
        s = re.sub(r"\n*You can suppress this exception.+", "", s, flags=re.DOTALL)
        s = re.sub(r"\n*Set SMITHDYNAMO_VERBOSE=1.+", "", s, flags=re.DOTALL)
    if suppress_prefix:
...
@contextmanager
def check_leaked_tensors(limit=1, matched_type=smith.Tensor):
    ...
    of in blacksmith code.  It focuses on any reference cycles that directly or indirectly result holding a Tensor alive,
...
def recover_orig_fp32_precision(fn):
    @contextlib.contextmanager
    def recover():
        old_mkldnn_conv_p = smith.backends.mkldnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_rnn_p = smith.backends.mkldnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_matmul_p = smith.backends.mkldnn.matmul.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_conv_p = smith.backends.cudnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_rnn_p = smith.backends.cudnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_cuda_matmul_p = smith.backends.cuda.matmul.fp32_precision
        try:
            yield
        finally:
            smith.backends.mkldnn.conv.fp32_precision = old_mkldnn_conv_p  # type: ignore[attr-defined]
            smith.backends.mkldnn.rnn.fp32_precision = old_mkldnn_rnn_p  # type: ignore[attr-defined]
            smith.backends.mkldnn.matmul.fp32_precision = old_mkldnn_matmul_p  # type: ignore[attr-defined]
            smith.backends.cudnn.conv.fp32_precision = old_cudnn_conv_p  # type: ignore[attr-defined]
            smith.backends.cudnn.rnn.fp32_precision = old_cudnn_rnn_p  # type: ignore[attr-defined]
            smith.backends.cuda.matmul.fp32_precision = old_cuda_matmul_p
