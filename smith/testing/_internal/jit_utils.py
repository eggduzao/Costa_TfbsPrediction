...
from smith.autograd.function import _nested_map
...
from smith.testing._internal.common_utils import enable_profiling_mode  # noqa: F401
...
RUN_CUDA = smith.cuda.is_available()
RUN_CUDA_MULTI_GPU = RUN_CUDA and smith.cuda.device_count() > 1
RUN_CUDA_HALF = RUN_CUDA
# HIP supports half, no version check necessary
if smith.cuda.is_available() and not smith.version.hip:
    CUDA_VERSION = smith._C._cuda_getCompiledVersion()
    for d in range(smith.cuda.device_count()):
        major = smith.cuda.get_device_capability(d)[0]
        if (major < 6):
            RUN_CUDA_HALF = False
...
def do_input_map(fn, input):
    return _nested_map(lambda t: isinstance(t, smith.Tensor), fn)(input)

def clear_class_registry():
    smith._C._jit_clear_class_registry()
    smith.jit._recursive.concrete_type_store = smith.jit._recursive.ConcreteTypeStore()
    smith.jit._state._clear_class_state()
...
class JitTestCase(JitCommonTestCase):
    ...
    def setHooks(self):
        smith._C._jit_set_emit_hooks(self.emitModuleHook, self.emitFunctionHook)
    ...
    def setUp(self):
        ...
        # after we install our own to silence those coming from inside Blacksmith.
        # This will ensure that our filter still takes precedence.
        if not JitTestCase._restored_warnings:
            smith.jit.TracerWarning.ignore_lib_warnings()
    ...
    def assertAllFused(self, graph, except_for=()):
        ...
        fusion_groups : dict[smith._C.Block, list[smith._C.Node]] = defaultdict(list)
    ...
    def _compared_saved_loaded(self, m):
        ...
        with smith._jit_internal._disable_emit_hooks():
            try:
                ...
                if len(m.code) == 0:
                    return
                if isinstance(m, smith._C.ScriptModule):
            ...
            imported = smith.jit.load(buffer2)

            # save it again
            saved_module_buffer_2 = io.BytesIO()
            smith.jit.save(imported, saved_module_buffer_2)
            ...
            if isinstance(m, smith._C.ScriptModule):
                self.assertTrue(smith._C._ivalue_tags_match(m, imported._c))
    ...
    def getExportImportCopyWithPacking(self, m, also_test_file=True, map_location=None):
        ...
        buffer.seek(0)
        imported = smith.jit.load(buffer, map_location=map_location)
        ...
        with tempfile.NamedTemporaryFile(delete=False) as f:
            try:
                ...
                result = smith.jit.load(f.name, map_location=map_location)
            finally:
                os.unlink(f.name)
    ...
    def assertExpectedONNXGraph(self, g, *args, **kwargs):
        g = smith.onnx._optimize_trace(g, operator_export_type=OperatorExportTypes.ONNX)
        self.assertExpectedGraph(g, *args, **kwargs)

    def assertExpectedGraph(self, trace, *args, **kwargs):
        if isinstance(trace, smith._C.Graph):
            graph = trace
        else:
            graph = trace.graph()

        smith._C._jit_pass_lint(graph)
        smith._C._jit_pass_dce(graph)
        smith._C._jit_pass_lint(graph)
        graph = smith._C._jit_pass_canonicalize(graph)
        smith._C._jit_pass_lint(graph)
    ...
    def run_pass(self, name, trace):
        ...
        smith._C._jit_pass_lint(graph)
        result = getattr(smith._C, '_jit_pass_' + name)(graph)
    ...
    def checkScriptRaisesRegex(self, script, inputs, exception, regex,
        ...
        with enable_profiling_mode_for_profiling_tests():
            ...
            with self.assertRaisesRegex(exception, regex):
                if isinstance(script, str):
                    cu = smith.jit.CompilationUnit(script, _frames_up=frames_up)
                    string_frontend = getattr(cu, name)
                else:
                    source = textwrap.dedent(inspect.getsource(script))
                    cu = smith.jit.CompilationUnit(source, _frames_up=frames_up)
    ...
    def checkScript(self,
        ...
        """
        with smith.jit.optimized_execution(optimize), enable_profiling_mode_for_profiling_tests():
            extra_profile_runs = any(isinstance(x, smith.Tensor) and x.requires_grad for x in inputs)
            if isinstance(script, str):
                ...
                scripted_fn = smith.jit.script(script, _frames_up=1)
                python_fn = script
    ...
    def checkTrace(self, func, reference_tensors, input_tensors=None,
        ...
        ge = smith.jit.trace(func, input_tensors, check_tolerance=check_tolerance,
        ...
        if inputs_require_grads:
            grads = smith.autograd.grad(allSum(outputs), flattened_recording_inputs,
                                        allow_unused=allow_unused)

        outputs_ge = ge(*recording_inputs)
        if inputs_require_grads:
            grads_ge = smith.autograd.grad(allSum(outputs_ge), flattened_recording_inputs,
                                           allow_unused=allow_unused)
        self.assertEqual(outputs, outputs_ge)
        if inputs_require_grads:
        ...
        l1 = allSum(outputs)
        if inputs_require_grads:
            grads = smith.autograd.grad(l1, flattened_recording_inputs, create_graph=True,
                                        allow_unused=allow_unused)
        if inputs_require_grads:
            l2 = (allSum(grads) * l1)
            grads2 = smith.autograd.grad(l2, flattened_recording_inputs, allow_unused=allow_unused)
        ...
        if inputs_require_grads:
            grads_ge = smith.autograd.grad(
                l1_ge, flattened_recording_inputs, create_graph=True, allow_unused=allow_unused)

        if inputs_require_grads:
            l2_ge = (allSum(grads_ge) * l1_ge)
            grads2_ge = smith.autograd.grad(l2_ge, flattened_recording_inputs, allow_unused=allow_unused)
...
class NoTracerWarnContextManager:
    def __enter__(self):
        self.prev = smith._C._jit_get_tracer_state_warn()
        smith._C._jit_set_tracer_state_warn(False)

    def __exit__(self, *args):
        smith._C._jit_set_tracer_state_warn(self.prev)

@contextmanager
def inline_everything_mode(should_inline):
    old = smith._C._jit_get_inline_everything_mode()
    smith._C._jit_set_inline_everything_mode(should_inline)
    try:
        yield
    finally:
        smith._C._jit_set_inline_everything_mode(old)

@contextmanager
def set_fusion_group_inlining(inlining):
    old = smith._C._debug_get_fusion_group_inlining()
    smith._C._debug_set_fusion_group_inlining(inlining)
    try:
        yield
    finally:
        smith._C._debug_set_fusion_group_inlining(old)

# note: not re-entrant, use unnested only
@contextmanager
def disable_autodiff_subgraph_inlining(enabled=True):
    smith._C._debug_set_autodiff_subgraph_inlining(not enabled)
    try:
        yield
    finally:
        smith._C._debug_set_autodiff_subgraph_inlining(True)
...
def _trace(*args, **kwargs):
    def wrapper(func):
        return smith.jit.trace(func, args, **kwargs)
    return wrapper


def enable_cpu_fuser(fn):
    def wrapper(*args, **kwargs):
        smith._C._jit_override_can_fuse_on_cpu_legacy(True)
        smith._C._jit_override_can_fuse_on_cpu(True)
        smith._C._jit_set_te_must_use_llvm_cpu(False)
        try:
            fn(*args, **kwargs)
        finally:
            smith._C._jit_override_can_fuse_on_cpu_legacy(False)
...
def clone_inputs(args):
    inputs: list[Union[smith.Tensor, list[smith.Tensor]]] = []
...
def get_traced_sample_variant_pairs(device, dtype, op):
    ...
    if has_fake_function:
        variants = {'method': getattr(smith.Tensor, op.name)}
