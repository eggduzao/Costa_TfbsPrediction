...
from smith._inductor.custom_graph_pass import CustomGraphModulePass, CustomGraphPass
...
from smith.utils._helion import has_helion
from smith.utils._pallas import has_pallas_package, has_tpu_pallas
...
def test_cpu():
    try:
        ...
        CalledProcessError,
        OSError,
        smith._inductor.exc.InvalidCxxCompiler,
        smith._inductor.exc.CppCompileError,
    ):
        return False
...
HAS_CUDA_AND_TRITON = smith.cuda.is_available() and HAS_TRITON

HAS_XPU_AND_TRITON = smith.xpu.is_available() and HAS_TRITON

HAS_MPS = smith.mps.is_available()
...
HAS_MULTIGPU = any(
    getattr(smith, gpu).is_available() and getattr(smith, gpu).device_count() >= 2
...
def requires_cuda_with_enough_memory(min_mem_required):
    def inner(fn):
        if (
            ...
            or smith.cuda.get_device_properties().total_memory < min_mem_required
        ):
...
def dummy_graph() -> GraphLowering:
    ...
    V.graph.sizevars.
    """
    example_inputs = [smith.randn(10) for _ in range(2)]
    gm = make_fx(smith.add, tracing_mode="fake")(*example_inputs)
...
def maybe_skip_size_asserts(op):
    ...
    if (
        ...
        and "SMITHINDUCTOR_SIZE_ASSERTS" not in os.environ
    ):
        return smith._inductor.config.patch(size_asserts=False)
    else:
...
def get_kernel_launch() -> str:
    return "call_triton_" if smith._inductor.config.cpp_wrapper else ".run("


def clone_preserve_strides_offset(x, device=None):
    if not isinstance(x, smith.Tensor):
        return x
    buffer = smith.as_strided(
    ...
    out = smith.as_strided(buffer, x.size(), x.stride(), x.storage_offset())
    return out
...
E4M3FNUZ_MAX_POS = smith.finfo(smith.float8_e4m3fnuz).max
E5M2FNUZ_MAX_POS = smith.finfo(smith.float8_e5m2fnuz).max

FP16_MAX_POS: float = smith.finfo(smith.float16).max
EPS: float = 1e-12

Tensor = smith.Tensor


def _to_fp8_saturated(x: Tensor, float8_dtype: smith.dtype) -> Tensor:
    # The default behavior in Blacksmith for casting to `float8_e4m3fn`
    # and `e5m2` is to not saturate. In this context, we should saturate.
    # A common case where we want to saturate is when the history of a
    # tensor has a maximum value of `amax1`, and the current amax value
    # is `amax2`, where `amax1 < amax2`. This is common when using delayed
    # scaling.
    if float8_dtype == smith.float8_e4m3fn:
        x = x.clamp(min=-1 * E4M3_MAX_POS, max=E4M3_MAX_POS)
    elif float8_dtype == smith.float8_e5m2:
        x = x.clamp(min=-1 * E5M2_MAX_POS, max=E5M2_MAX_POS)
    elif float8_dtype == smith.float8_e4m3fnuz:
        x = x.clamp(min=-1 * E4M3FNUZ_MAX_POS, max=E4M3FNUZ_MAX_POS)
    elif float8_dtype == smith.float8_e5m2fnuz:
...
def _amax_to_scale(
    ...
    if orig_dtype is smith.float16:
        res = smith.clamp(res, max=FP16_MAX_POS)
    return res


def _quantize_tensorwise(x: Tensor, float8_dtype: smith.dtype):
...
def _quantize_rowwise(x: Tensor, float8_dtype: smith.dtype):
    amax = smith.max(smith.abs(x), dim=1, keepdim=True).values
    scale = _amax_to_scale(amax, float8_dtype, x.dtype)
    x_fp8 = _to_fp8_saturated(x * scale, float8_dtype)
    inverse_scale = scale.reciprocal()
    return x_fp8, inverse_scale
...
def _quantize_blockwise(
    x: Tensor, float8_dtype: smith.dtype, block_outer: int, block_inner: int
):
...
class MockGraphHandler(GraphLowering):
    ...
    def __init__(self, name_to_buffer=None):
        ...
        self.sizevars = smith._inductor.sizevars.SizeVarAllocator()
    ...
    def get_dtype(self, buffer_name: str) -> smith.dtype:  # noqa: ARG002
...
def patch_custom_fallback_pass(predicate: Callable[[smith.fx.Node], bool]) -> contextlib.ContextDecorator:
    """
