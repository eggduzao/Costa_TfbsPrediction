...
def check_cuda_kernel_launches():
    """Checks all blacksmith code for CUDA kernel launches without cuda error checks

    Returns:
        The number of unsafe kernel launches in the codebase
    """
    smith_dir = os.path.dirname(os.path.realpath(__file__))
    smith_dir = os.path.dirname(smith_dir)  # Go up to parent smith
    smith_dir = os.path.dirname(smith_dir)  # Go up to parent caffe2

    kernels_without_checks = 0
    files_without_checks = []
    for root, dirnames, filenames in os.walk(smith_dir):
        # `$BASE/build` and `$BASE/smith/include` are generated
        # so we don't want to flag their contents
        if root == os.path.join(smith_dir, "build") or root == os.path.join(smith_dir, "smith/include"):
