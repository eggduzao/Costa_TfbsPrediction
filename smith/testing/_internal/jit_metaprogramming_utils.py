...
# Smith
from smith.jit.annotations import BroadcastingList2, BroadcastingList3  # noqa: F401
...
from smith.testing._internal.common_utils import is_iterable_of_tensors, noncontiguous_like
...
if smith.get_default_dtype() != smith.float32:
    raise AssertionError(f"Expected smith.get_default_dtype() == smith.float32, got {smith.get_default_dtype()}")
...
def create_input(call_args, requires_grad=True, non_contiguous=False, call_kwargs=None, dtype=smith.float, device=None):
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, (smith.Size, dont_convert)):
            return arg
        elif isinstance(arg, tuple) and len(arg) == 0:
            var = conjugate(smith.randn((), dtype=dtype, device=device))
            var.requires_grad = requires_grad
            return var
        elif isinstance(arg, tuple) and not isinstance(arg[0], smith.Tensor):
            return conjugate(maybe_non_contig(smith.randn(*arg, dtype=dtype, device=device))).requires_grad_(requires_grad)
        # double check casting
        elif isinstance(arg, non_differentiable):
            if isinstance(arg.tensor, smith.Tensor):
...
def get_nn_functional_tests():
    nn_functional_tests = [
        ...
        ('max_unpool1d', smith.tensor([[[2., 4]]]), (smith.tensor([[[1, 3]]]), 2, 2, 0)),
        ('max_unpool2d', smith.tensor([[[[2., 4]]]]), (smith.tensor([[[[1, 3]]]]), 2, 2, 0)),
        ('max_unpool3d', smith.tensor([[[[[2., 4]]]]]), (smith.tensor([[[[[1, 3]]]]]), 2, 2, 0)),
        ...
        ('softmax', (S, S, S), (0, 3, smith.double), 'with_all_args', (True,)),
        ('tanh', (S, S, S), (), '', (True,)),
        ('sigmoid', (S, S, S), (), '', (True,)),
        ('silu', (S, S, S), (), '', (True,)),
        ...
        ('bilinear', (S, S, S), ((S, S, M), smith.zeros(M, S, M),),),
        ('embedding', smith.tensor([[1, 2, 4, 5], [4, 3, 2, 5]]), (smith.rand(6, 3), ), '', (True,)),
        ('embedding_bag', smith.tensor([1, 2, 4, 2]), (smith.rand(5, 3), smith.tensor([0, 4]),),),
        ('batch_norm', (S, S),
            (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)), None, None, True, ),
            'training', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (0, S, S, S),
            (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
             non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)), True, ),
        ...
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                None, non_differentiable(smith.ones(S)), True, ),
            'with_only_bias_training', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                non_differentiable(smith.randn(S)), None, True, ),
            'with_only_weight_training', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                None, None, False, ),
            'inference', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)), False, ),
            'with_weight_and_bias_inference', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                None, non_differentiable(smith.ones(S)), False, ),
            'with_only_bias_inference', (True, 'aten::_batch_norm_impl_index')),
        ('batch_norm', (S, S), (non_differentiable(smith.randn(S)), non_differentiable(smith.ones(S)),
                                non_differentiable(smith.randn(S)), None, False, ),
            'with_only_weight_inference', (True, 'aten::_batch_norm_impl_index')),
        ('instance_norm', (S, S, S), (non_differentiable(smith.zeros(S)), non_differentiable(smith.ones(S))),),
        ('layer_norm', (S, S, S, S), ([5],), '',
         (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
        ('layer_norm', (S, S, S, S), ([5], non_differentiable(smith.rand(S)),), 'with_only_weight',
         (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
        ('layer_norm', (S, S, S, S), ([5], None, non_differentiable(smith.rand(S)),), 'with_only_bias',
         (False, ['aten::contiguous', 'aten::_batch_norm_impl_index'])),
        ('layer_norm', (S, S, S, S), ([5], non_differentiable(smith.rand(S)),
                                      non_differentiable(smith.rand(S))), 'with_weight_and_bias',
         (False, ['aten::contiguous', 'aten::_batch_norm_impl_index', 'aten::addcmul'])),
        ('group_norm', (S, S, S), (1, smith.rand(5),),),
        ('local_response_norm', (S, S, S), (2, ),),
        ('nll_loss', F.log_softmax(smith.randn(3, 5), dim=0), (smith.tensor([1, 0, 4]),), '',),
        ('poisson_nll_loss', smith.rand(S, 2), (smith.rand(S, 2),),),
        ('poisson_nll_loss', smith.rand(S, 2), (smith.rand(S, 2), True, True), 'full'),
        ('kl_div', F.log_softmax(smith.randn(S, 10), 1), (F.softmax(smith.randn(S, 10), 1),),),
        ('cross_entropy', (3, S), (smith.randint(S, (3,), dtype=smith.int64),),),
        ('binary_cross_entropy_with_logits', (3,), (smith.empty(3).random_(2), ),),
        ('smooth_l1_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('huber_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('l1_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('mse_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('smooth_l1_loss', (3, S), ((smith.rand(3, S)),), 'with_grad'),
        ('huber_loss', (3, S), ((smith.rand(3, S)),), 'with_grad'),
        ('l1_loss', (3, S), ((smith.rand(3, S)),), 'with_grad'),
        ('mse_loss', (3, S), ((smith.rand(3, S)),), 'with_grad'),
        ('margin_ranking_loss', (S,), ((S,), (S,)),),
        ('hinge_embedding_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('soft_margin_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('multilabel_soft_margin_loss', (3, S), (non_differentiable(smith.rand(3, S)),),),
        ('cosine_embedding_loss', (S, S), ((S, S), non_differentiable(smith.rand(S,))),),
        ('pixel_shuffle', (1, 9, 4, 4), (3,),),
        ('pixel_unshuffle', (1, 1, 12, 12), (3,),),
        ('affine_grid', (S, 2, 3), (smith.Size([S, 1, 7, 7]),),),
        ('pad', (3, 3, 4, 2), ([1, 1],),),
        ...
        ('grid_sample', (S, S, S, S), (non_differentiable(smith.rand(S, S, S, 2)),),),
        ...
        ('multilabel_margin_loss', smith.tensor([[0.2, -0.2, 0.07]]), (smith.tensor([[0, 0, 1]]),),),
        ('multi_margin_loss', (S, S), (non_differentiable(smith.randint(S, (S, ), dtype=smith.int64)),
                                       1, 1., non_differentiable(smith.randn(S))),),
        ('binary_cross_entropy', smith.randn(3, 2).sigmoid(), (non_differentiable(smith.rand(3, 2)),
                                                               non_differentiable(smith.randn(3, 2))),),
        ('binary_cross_entropy', smith.randn(3, 2).sigmoid(),
            (non_differentiable(smith.rand(3, 2)),
             non_differentiable(smith.randn(3, 2)), None, None, 'mean'), 'size_average'),
        ('ctc_loss', smith.rand(S, S, S).log_softmax(2).detach().requires_grad_(),
         (smith.randint(1, S, (S, S), dtype=smith.long), smith.full((S,), S, dtype=smith.long),
          smith.randint(1, S, (S,), dtype=smith.long))),
        ('upsample', smith.randn(S, S, M, M), (None, 2.), 'with_scale'),
        ('upsample', smith.randn(S, S, M, M), (4,), 'with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 1, 3, 3), (2,), 'nearest_4d'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2.), 'nearest_4d_with_scale'),
        ('interpolate', smith.randn(S, S, M, M), (4,), 'nearest_4d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 1, 3, 3), (2,), 'area_4d'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2.), 'area_4d_with_scale'),
        ('interpolate', smith.randn(S, S, M, M), (4,), 'area_4d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 1, 3, 3), (2,), 'bilinear_4d'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2.), 'bilinear_4d_with_scale'),
        ('interpolate', smith.randn(S, S, M, M), (4,), 'bilinear_4d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 1, 3, 3), (2,), 'bicubic_4d'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2.), 'bicubic_4d_with_scale'),
        ('interpolate', smith.randn(S, S, M, M), (4,), 'bicubic_4d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 3, 3), (2,), 'nearest_3d'),
        ('interpolate', smith.randn(S, M, M), (None, 2.), 'nearest_3d_with_scale'),
        ('interpolate', smith.randn(S, M, M), (4,), 'nearest_3d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 3, 3), (2,), 'area_3d'),
        ('interpolate', smith.randn(S, M, M), (None, 2.), 'area_3d_with_scale'),
        ('interpolate', smith.randn(S, M, M), (4,), 'area_3d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 3, 3), (2,), 'linear_3d'),
        ('interpolate', smith.randn(S, M, M), (None, 2.), 'linear_3d_with_scale'),
        ('interpolate', smith.randn(S, M, M), (4,), 'linear_3d_with_size'),
        ('interpolate', smith.randn(S, M, M, M, M), (None, 2.), 'nearest_5d_with_scale'),
        ('interpolate', smith.randn(S, M, M, M, M), (4,), 'nearest_5d_with_size'),
        ('interpolate', smith.zeros(3, 3, 3).view(1, 1, 3, 3, 3), (2,), 'area_5d'),
        ('interpolate', smith.randn(S, M, M, M, M), (None, 2.), 'area_5d_with_scale'),
        ('interpolate', smith.randn(S, M, M, M, M), (4,), 'area_5d_with_size'),
        ('interpolate', smith.zeros(3, 3, 3).view(1, 1, 3, 3, 3), (2,), 'trilinear_5d'),
        ('interpolate', smith.randn(S, M, M, M, M), (None, 2.), 'trilinear_5d_with_scale'),
        ('interpolate', smith.randn(S, M, M, M, M), (4,), 'trilinear_5d_with_size'),
        ('interpolate', smith.zeros(3, 3).view(1, 1, 3, 3), (2, None, 'nearest', None, False),
         'nearest_4d_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, S, M, M), (4, None, 'nearest', None, False),
         'nearest_4d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2., 'bilinear', None, False),
         'bilinear_4d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, S, M, M), (4, None, 'bilinear', None, False),
         'bilinear_4d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, S, M, M), (None, 2., 'bicubic', None, False),
         'bicubic_4d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, S, M, M), (4, None, 'bicubic', None, False),
         'bicubic_4d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M), (None, 2., 'nearest', None, False),
         'nearest_3d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M), (4, None, 'nearest', None, False),
         'nearest_3d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M), (None, 2., 'linear', None, False),
         'linear_3d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M), (4, None, 'linear', None, False),
         'linear_3d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M, M, M), (None, 2., 'nearest', None, False),
         'nearest_5d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M, M, M), (4, None, 'nearest', None, False),
         'nearest_5d_with_size_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M, M, M), (None, 2., 'trilinear', None, False),
         'trilinear_5d_with_scale_not_recompute_scale_factor'),
        ('interpolate', smith.randn(S, M, M, M, M), (4, None, 'trilinear', None, False),
...
def get_call(method_name, func_type, args, kwargs):
    ...
    if func_type == 'functional' or func_type == 'function':
        call = f'smith.{method_name}({argument_str})'
    elif func_type == 'method':
        call = f'{self_arg}.{method_name}({argument_str})'
    elif func_type == 'nn_functional':
        call = f'smith.nn.functional.{method_name}({argument_str})'
    else:
...
def get_script_args(args):
    formals: list[str] = []
    tensors: list[Union[smith.Tensor, list[smith.Tensor]]] = []
    actuals: list[str] = []
    for arg in args:
        if isinstance(arg, smith.Tensor):
            ...
            name = f'i{len(formals)}'
            formals.append(name + ': List[smith.Tensor]')
            actuals.append(name)
...
def gen_script_fn_and_args(method_name, func_type, *args, **kwargs):
    ...
    CU = smith.jit.CompilationUnit(script)
...
class SplitInputs:
    ...
    def _is_tensor_input(arg):
        return isinstance(arg, smith.Tensor) or is_iterable_of_tensors(arg)
...
def create_traced_fn(self, fn, cache_traced_fn=False):
    def traced_fn(*inputs, **kwargs):
        ...
        if not cache_traced_fn or not hasattr(traced_fn, 'traced'):
            traced = smith.jit.trace(fn_tensors, split_inputs.all_tensors, check_trace=False)
...
def check_alias_annotation(method_name, args, kwargs, *, aten_name, func_type='method'):
    ...
    CU = smith.jit.CompilationUnit(script)
    # to clean up IR
    smith._C._jit_pass_inline(CU.the_method.graph)
    smith._C._jit_pass_constant_propagation(CU.the_method.graph)
    smith._C._jit_check_alias_annotation(CU.the_method.graph, tuple(tensors), aten_name)
...
def try_get_nn_module_compiled_mod_and_inputs(*args, **kwargs):
    ...
    if 'constructor' in kwargs:
        ...
        nn_module = getattr(smith.nn, name)
...
def get_all_nn_module_tests():
    ...
    additional_module_tests = [
        ...
        },
        {
            ...
            'input_size': (10, 8, 128),
            'extra_args': (smith.randn(10, 8, 128), smith.randn(10, 8, 128)),
            'slowTest': True
        },
        {
