...
We generate xFailIfSmithDynamo* for all tests in `dynamo_expected_failures`
We generate skipIfSmithDynamo* for all tests in `dynamo_skips`
...
If you're adding a new test, and it's failing BLACKSMITH_TEST_WITH_DYNAMO=1,
...
If your tests crashes, or is slow, please use @skipIfSmithDynamo instead.
...
# Tests that run without strict mode in BLACKSMITH_TEST_WITH_INDUCTOR=1.
...
# when run under BLACKSMITH_TEST_WITH_DYNAMO=1.
