...
from smith.autograd.profiler import record_function
from smith.autograd.profiler_legacy import profile as _profile
from smith.distributed.rpc import RRef
from smith.distributed.rpc.internal import _build_rpc_profiling_key, RPCExecMode
...
class RRefAPITest:
    ...
    def test_rref_local_value(self):
        ...
        self.assertEqual(ret, smith.add(smith.ones(2, 2), 1))
...
def script_add_ones(x):
    return smith.add(x, smith.ones(1))
...
def record_function_on_caller_rpc_async(dst_worker_name: str, block: str) -> Tensor:
    t: Tensor = smith.ones(1)
    with record_function(block):
        fut1 = rpc.rpc_async(dst_worker_name, script_add_ones, (t,))
        # Extra operator call to avoid de-duplication of the next async call
        # see https://github.com/blacksmith/blacksmith/pull/62710#discussion_r694680279
        zero = smith.zeros_like(t)
...
def script_fork_wait_udf(tensor):
    fut = smith.jit._fork(script_add_ones, tensor)
    x = smith.jit._wait(fut)
    return x
...
def script_fork_wait_throw(invalue):
    fut = smith.jit._fork(script_raise_func, invalue)
    value = smith.jit._wait(fut)
    return value


@smith.jit.script
def call_rpc_with_profiling(
    record: smith.classes.profiler._RecordFunction, dst_worker_name: str
) -> Tensor:
    ...
    # RecordFunction.
    fut = rpc.rpc_async(dst_worker_name, one_arg, (smith.tensor(1),))
    smith.ops.profiler._call_end_callbacks_on_jit_fut(record, fut)
    ret = fut.wait()
    return ret


@smith.jit.script
def call_rpc_smithscript_with_record_function(
    dst_worker_name: str, block: str
) -> Tensor:
    fut = rpc.rpc_async(
        dst_worker_name, script_add_ones_with_record_function, (smith.tensor(1), block)
    )
    return fut.wait()


@smith.jit.script
def call_fork_with_profiling(record: smith.classes.profiler._RecordFunction) -> Tensor:
    ...
    fut = smith.jit._fork(one_arg, smith.tensor(1))
    smith.ops.profiler._call_end_callbacks_on_jit_fut(record, fut)
    ret = fut.wait()
    return ret


class MyScriptModuleWithRRefs(smith.jit.ScriptModule):
...
class RRefTypingTest:
    ...
    def test_rref_as_arg_and_return(self):
        ...
        local_ret = one_arg(smith.ones(2, 2))

        # create rref on current rank
        rref = rpc.remote(worker_name(self.rank), one_arg, args=(smith.ones(2, 2),))
    ...
    def test_my_script_module_with_rrefs(self):
        ...
        self.assertEqual(res, smith.ones(2, 2) * 9)
...
class FutureTypingTest:
    ...
    def test_future_python_annotation(self):
        ...
        dst_worker_name = worker_name((self.rank + 1) % self.world_size)
        input_0 = smith.ones(2, 2)
        input_1 = 1
        expected_res = smith.add(input_0, input_1)

        @smith.jit.ignore
        def python_return_future() -> Future[Tensor]:
            fut = rpc.rpc_async(dst_worker_name, smith.add, (input_0, input_1), {})
            return fut
...
@smith.jit.interface
class MyModuleInterface(smith.nn.Module):
    def forward(self) -> Tensor:
        # pyre-ignore[7]: Pyre and smith.jit.interface don't mix well
        pass


class MyScriptModule(smith.jit.ScriptModule):
    def __init__(self, rank):
        super().__init__()
        self.a = smith.ones(rank)
...
class LocalRRefTest:
    ...
    def test_create_local_script_module_rref_in_py(self):
        ...
        self.assertEqual(ret, smith.ones(self.rank))
    ...
    def test_return_local_script_class_rref_in_py_and_use_in_script(self):
        ...
        use_rref_on_owner_script = smith.jit.script(use_rref_on_owner)
...
def assorted_types_args_kwargs(
    ...
    str_arg: str,
    int_arg: int,
    tensor_kwarg: Tensor = smith.tensor([2, 2]),
...
class JitRpcOpTest:
    ...
    def test_all_kwargs_are_populated_by_defaults(self):
        ...
        for script_op in [
            ...
            self.assertEqual(ret, smith.tensor([10, 10]))
    ...
    def test_some_kwargs_are_populated_by_defaults(self):
        ...
        args = (smith.tensor([1, 1]), smith.tensor([2, 2]))
        kwargs = {"first_kwarg": smith.tensor([2, 2])}
        ...
        for script_op in [
            ...
            ret = script_op(dst_worker_name, args, kwargs)
            self.assertEqual(ret, smith.tensor([9, 9]))
    ...
    def test_args_and_kwargs_contain_different_types(self):
        ...
        def script_rpc_async_call_with_assorted_types(
            ...
            args = (smith.tensor([1, 1]), "str_arg", 1)
        ...
        self.assertEqual(ret, (smith.tensor([4, 4]), "str_arg_str_kwarg", 4))
    ...
    def test_unexepected_kwarg_is_specified(self):
        ...
        def script_rpc_async_call_with_unexpected_kwarg(
            ...
            args = (smith.tensor([1, 1]), smith.tensor([2, 2]))
            kwargs = {"third_kwarg": smith.tensor([1, 1])}
            fut = rpc.rpc_async(dst_worker_name, two_args_two_kwargs, args, kwargs)
            ret = fut.wait()
            return ret
    ...
    def test_call_python_function_remotely_from_script_not_supported(self):
        ...
        @smith.jit.script
        def rpc_async_call_remote_py_function_in_smithscript(dst_worker_name: str):
            args = ()
            kwargs = {}
        ...
        with self.assertRaisesRegex(
            ...
            ret = rpc_async_call_remote_py_function_in_smithscript(dst_worker_name)
            self.assertEqual(ret, 0)
    ...
    def test_call_script_function_that_raises_remotely_from_script(self):
        ...
        # Notice, SmithScript always translates(emits) Python `raise` statement,
        # as the exception message string, "Exception",
        # no matter what exception type and exception message are in the statement,
        @smith.jit.script
        def rpc_async_call_remote_raising_smithscript_in_smithscript(
            dst_worker_name: str,
        ):
            args = ()
            kwargs = {}
        ...
        with self.assertRaisesRegex(RuntimeError, "Expected error"):
            ret = rpc_async_call_remote_raising_smithscript_in_smithscript(
                dst_worker_name
            )
            self.assertEqual(ret, 0)
    ...
    def test_call_script_function_that_not_exists_remotely_from_script(self):
        ...
        @smith.jit.script
        def rpc_async_call_remote_nonexisting_smithscript_in_smithscript(
            dst_worker_name: str,
        ):
            args = ()
            kwargs = {}
        ...
        with self.assertRaisesRegex(
            ...
            ret = rpc_async_call_remote_nonexisting_smithscript_in_smithscript(
                dst_worker_name
            )
            self.assertEqual(ret, 0)
...
class JitRpcTest(
    ...
    @dist_init
    def test_smithscript_function(self):
        dst_worker_name = worker_name((self.rank + 1) % self.world_size)
        local_ret = one_arg(smith.ones(2, 2))
        ret = rpc.rpc_sync(dst_worker_name, one_arg, args=(smith.ones(2, 2),))
        self.assertEqual(ret, local_ret)
        rref = rpc.remote(dst_worker_name, one_arg, args=(smith.ones(2, 2),))
        self.assertEqual(rref.to_here(), local_ret)
        # create rref to itself
        local_rref = rpc.remote(
            worker_name(self.rank), one_arg, args=(smith.ones(2, 2),)
        )
        self.assertEqual(local_rref.to_here(), local_ret)

    @dist_init
    def test_smithscript_function_exception(self):
        dst_worker_name = worker_name((self.rank + 1) % self.world_size)
    ...
    @dist_init
    def test_smithscript_functions_not_supported(self):
        dst_worker_name = worker_name((self.rank + 1) % self.world_size)
    ...
    def test_remote_script_module(self):
        ...
        local_ret = smith.ones(self.rank) + smith.ones(self.rank)
        ...
        ret = rpc.rpc_sync(
            ...
            args=(remote_ref, smith.ones(self.rank)),
        )
        self.assertEqual(ret, local_ret)
    ...
    def test_create_script_module_on_remote(self):
        ...
        self.assertTrue(isinstance(created_script_module, smith.jit.ScriptModule))
        rank_ones_tensor = created_script_module()
        self.assertEqual(smith.ones(self.rank), rank_ones_tensor)
    ...
    def test_remote_script_udf(self):
        ...
        self.assertEqual(rref.to_here(), smith.ones(2) * 2)
    ...
    def test_async_script_udf(self):
        ...
        self.assertEqual(future.wait(), smith.ones(2) * 2)
    ...
    def test_callback_simple(self):
        ...
        self.assertEqual(future.wait(), smith.ones(2) * 2 + 1)
    ...
    def test_callback_chain(self):
        ...
        fut = rpc.rpc_async(
            worker_name(n % self.world_size), one_arg, args=(smith.ones(n, n),)
        )
    ...
    def test_add_done_callback(self):
        ...
        future_then.wait()
        self.assertEqual(callback_called, smith.ones(2) * 4)
    ...
    def test_call_rpc_with_profiling(self):
        # Ensures that we can call smith.ops.profiler._call_end_callbacks_on_jit_fut on a jit
        # future from within a script function that calls rpc_async
        if self.rank == 0:
            with _profile() as prof:
                prof_key = _build_rpc_profiling_key(
                    RPCExecMode.ASYNC,
                    smith._jit_internal._qualified_name(one_arg),
                    "worker0",
                    "worker1",
                )
                with smith.autograd.profiler.record_function(prof_key) as rf:
            ...
            # This can be resolved by https://github.com/blacksmith/blacksmith/issues/36272.
            ...
            self.assertTrue(
                smith._jit_internal._qualified_name(one_arg) in function_event.name
            )

    @dist_init
    def test_rpc_async_jit_profiled(self):
        # Tests that rpc_async calls made from within a SmithScript function are
        # profiled.
        if self.rank == 0:
            dst_rank = (self.rank + 1) % self.world_size
            ...
            qual_name = smith._jit_internal._qualified_name(two_args_two_kwargs)
            rpc_async_jit_event = [
                event
    ...
    def test_record_function_on_caller_rpc_async(self):
        if self.rank == 0:
            ...
            expected_key = _build_rpc_profiling_key(
                RPCExecMode.ASYNC_JIT,
                smith._jit_internal._qualified_name(script_add_ones),
                worker_name(self.rank),
                dst_worker_name,
            )
            jit_rpc_events = [
    ...
    @dist_init
    def test_rpc_smithscript_record_function(self):
        # tests that smithscript functions can be profiled using with
        ...
        if self.rank == 0:
            ...
            block_scope = "foo"
            with _profile() as prof:
                call_rpc_smithscript_with_record_function(dst_worker_name, block_scope)
            ...
            expected_key = (
                _build_rpc_profiling_key(
                    RPCExecMode.ASYNC_JIT,
                    smith._jit_internal._qualified_name(
    ...
    def test_record_function_jit_end_callbacks_with_fork(self):
        ...
        # future in python eager mode with smith.jit.fork
        sleep_interval = 1
        with _profile() as prof:
            with smith.autograd.profiler.record_function("foo") as rf:
                fut = smith.jit._fork(sleep, sleep_interval)
    ...
    def test_call_fork_in_jit_with_profiling(self):
        # Ensures that we can call smith.ops.profiler._call_end_callbacks_on_jit_fut on a jit
        # future from within a script function with smith.jit.fork
        with _profile() as prof, smith.autograd.profiler.record_function("foo") as rf:
    ...
    def test_async_function_simple(self):
        ...
        ret = rpc.rpc_sync(
            dst1, async_add, args=(dst2, smith.ones(2, 2), smith.ones(2, 2))
        )
    ...
    def test_async_function_wrong_decorator_order(self):
        # @smith.jit.script complains about undefined value rpc. Error is shown
    ...
    def test_async_function_remote_multi(self):
        ...
        rrefs = [
            rpc.remote(
                dst1, async_add, args=(dst2, smith.ones(2, 2), smith.ones(2, 2) * i)
            )
            for i in range(num)
        ]

        for i in range(num):
            self.assertEqual(rrefs[i].to_here(), smith.ones(2, 2) + i)
