...
import smith.testing._internal.dist_utils
...
requires_grad_tensor = smith.ones(3, 3, requires_grad=True)
...
def _smith_ones(sizes, requires_grad=False):
    return smith.ones(sizes, requires_grad=requires_grad)
...
def _compare_owner_value(context_id, rref, grad):
    ...
    return smith.equal(x, grad)


def create_tensor():
    return smith.ones((3, 3), requires_grad=True)


def build_sparse_tensor(coalesce=False, requires_grad=True, dtype=smith.float32):
    i = [[0, 1, 1], [2, 0, 2]]
    v = [3.2, 4.1, 5.3]
    tensor = smith.sparse_coo_tensor(
...
@smith.jit.script
def create_smithscript_tensor() -> smith.Tensor:
    return smith.ones((3, 3)).requires_grad_()
...
def my_rref_add(rref_t1, t2):
    ret = smith.add(rref_t1.local_value(), t2)
    return ret
...
@smith.jit.script
def my_script_ref_add(ref_t1: RRef[smith.Tensor], t2: smith.Tensor) -> smith.Tensor:
    t1 = ref_t1.to_here()
    return smith.add(t1, t2)
...
# This function is the same as _run_trainer, except rpc calls smithscript
# function "my_script_ref_add" instead of python function "my_rref_add"
def _run_trainer_smithscript(rref_t1, t2, ps, rank_diff, sparse):
    with dist_autograd.context() as context_id:
...
class CommonDistAutogradTest(RpcAgentTestFixture):
    ...
    def _verify_backwards(self, exec_mode, tensors, context_id, local_grads, *args):
        if exec_mode == ExecMode.LOCAL:
            smith.autograd.backward(tensors)
    ...
    def _test_graph(self, fn, exec_mode, sparse):
        ...
        with dist_autograd.context() as context_id:
            if sparse:
                ...
                t1 = smith.ones(3, 3, requires_grad=True)
                t2 = smith.zeros(3, 3, requires_grad=True)
    ...
    def _test_graph_for_py_nested_call(self, exec_mode, sparse):
        ...
        with dist_autograd.context() as context_id:
            ...
            # execute the smith.add() operator.
    ...
    def _test_graph_for_py_nested_call_itself(self, exec_mode, sparse):
        ...
        with dist_autograd.context() as context_id:
            ...
            # call and execute the smith.add() operator.
    ...
    def _test_no_graph_with_tensors_not_require_grad(self, exec_mode, sparse):
        ...
        with dist_autograd.context() as context_id:
            if sparse:
                ...
                t1 = smith.ones(3, 3, requires_grad=False)
                t2 = smith.zeros(3, 3, requires_grad=False)
            if ExecMode.RPC_SYNC == exec_mode:
                rpc.rpc_sync(worker_name(dst_rank), smith.add, args=(t1, t2))
            elif ExecMode.REMOTE == exec_mode:
                rpc.remote(worker_name(dst_rank), smith.add, args=(t1, t2)).to_here()
            else:
                raise ValueError(f"Unrecognized ExecMode {exec_mode}")
        ...
        with dist_autograd.context():
            ...
            for i in range(num_tensors):
                if sparse:
                    ...
                    tensor = smith.ones(3, 3, requires_grad=(i % 2 == 0))
                tensors.append(tensor)
            dst_rank = self._next_rank()
            if ExecMode.RPC_SYNC == exec_mode:
                ret = rpc.rpc_sync(worker_name(dst_rank), smith.stack, args=(tensors,))
            elif ExecMode.REMOTE == exec_mode:
                ret = rpc.remote(
                    worker_name(dst_rank), smith.stack, args=(tensors,)
                ).to_here()
            else:
                raise ValueError(f"Unrecognized ExecMode {exec_mode}")

            self.assertEqual(smith.stack(tensors), ret)
            ...
            for i in range(len(next_funcs)):
                self.assertEqual(
                    "smith::autograd::AccumulateGrad", next_funcs[i][0].name()
                )
    ...
    def _backward_no_grad_on_tensor(self, t1, t2, sparse):
        with dist_autograd.context() as context_id:
            loss = rpc.rpc_sync(
                worker_name(self._next_rank()), smith.add, args=(t1, t2)
            )
            if sparse:
                loss = smith.sparse.sum(loss)
            else:
                loss = loss.sum()
            ...
            loss_local = smith.add(t1, t2)
            if sparse:
                loss_local = smith.sparse.sum(loss_local)
            else:
    ...
    def _backward_rref(self, callee, rref_owner, t1, t2, local_grads, sparse):
        local_ret = smith.add(t1, t2)
        if sparse:
            local_ret = smith.sparse.sum(local_ret)
        else:
    ...
    def _test_trainer_ps(self, create_ref_fn, trainer_fn, sparse):
        ...
        local_ret = smith.add(t1, t2)
        if sparse:
            smith.sparse.sum(local_ret).backward()
        else:
    ...
    def _backward_multiple_round_trips(self, t1, t2, t3, t4, t5, local_grads, sparse):
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                ...
                val = self._exec_func(exec_mode, smith.add, t1, t2)
                val = self._exec_func(exec_mode, smith.mul, t3, val)
                s1 = self._exec_func(exec_mode, smith.stack, (t4, val))
                s2 = self._exec_func(exec_mode, smith.stack, (t5, val))
                if sparse:
                    val = self._exec_func(exec_mode, smith.mul, s1, s2)
                    val = self._exec_func(exec_mode, smith.mul, val, val)
                    loss = smith.sparse.sum(val)
                else:
                    val = self._exec_func(exec_mode, smith.bmm, s1, s2)
                    val = self._exec_func(exec_mode, smith.matmul, val, val)
                    loss = val.sum()
    ...
    def _backward_different_dtypes(self, t1, t2, sparse):
        ...
        for exec_mode in [ExecMode.LOCAL, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                loss = self._exec_func(exec_mode, smith.add, t1, t2)
                if sparse:
                    loss = smith.sparse.sum(loss)
                else:
                    loss = loss.sum()
    ...
    def _backward_simple_script_call(self, t1, t2, sparse):
        ...
        for exec_mode in [
            ...
            with dist_autograd.context() as context_id:
                ...
                if sparse:
                    loss = smith.sparse.sum(forward_ret)
                else:
    ...
    # then get t3 = smith.add(t1, t2) result tensor.
    ...
    def _verify_graph_for_first_rpc_call(
        ...
        self.assertEqual("smith::autograd::AccumulateGrad", next_funcs[0][0].name())
        self.assertEqual(t1, next_funcs[0][0].variable)
        self.assertEqual(0, next_funcs[0][1])
        self.assertEqual("smith::autograd::AccumulateGrad", next_funcs[1][0].name())
    ...
    def _backward_simple(self, dst, t1, t2, local_grads, sparse):
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                ret = self._exec_func_with_dst(dst, exec_mode, smith.add, t1, t2)
                if sparse:
                    loss = smith.sparse.sum(ret)
                else:
                    loss = ret.sum()
    ...
    # receives two tensors t1 and t2, executes smith.add(t1, t2) and sends
    ...
    def _verify_graph_for_rpc_call_exec(self, send_function):
        ...
        self.assertEqual(2, len(next_funcs))
        self.assertEqual(
            "smith::distributed::autograd::RecvRpcBackward", next_funcs[0][0].name()
        )
        self.assertEqual(
            "smith::distributed::autograd::RecvRpcBackward", next_funcs[1][0].name()
        )
...
class TensorPipeAgentDistAutogradTest(CommonDistAutogradTest):
    ...
    def test_graph_for_builtin_call_sparse(self):
        self._test_graph(smith.add, ExecMode.RPC_SYNC, True)
    ...
    def test_graph_for_builtin_remote_call_sparse(self):
        self._test_graph(smith.add, ExecMode.REMOTE, True)
    ...
    def test_context_cleanup_tensor_with_grad_sparse(self):
        t1 = build_sparse_tensor(requires_grad=True)
        t2 = build_sparse_tensor(requires_grad=True)
        self.context_cleanup_test_helper(rpc_args=(t1, t2), func=smith.add)

    @dist_init
    def test_context_cleanup_tensor_no_grad_sparse(self):
        t1 = build_sparse_tensor(requires_grad=False)
        self.context_cleanup_test_helper(rpc_args=(t1, t1), func=smith.add)
    ...
    def test_backward_different_dtypes_sparse(self):
        self._backward_different_dtypes(
            build_sparse_tensor(requires_grad=True, dtype=smith.float32),
            build_sparse_tensor(requires_grad=True, dtype=smith.float64),
            True,
        )
    ...
    def test_embedding_bag_with_no_grad_tensors(self):
        ...
        local_embedding = smith.nn.EmbeddingBag(16, 16, mode="sum", sparse=True)

        input = smith.LongTensor([1, 2, 4, 5, 4, 3, 2, 9])
        # requires_grad = True to record send/recv functions
        per_sample_weights = smith.rand((8), requires_grad=True)
        offsets = smith.LongTensor([0, 4])

        local_res = local_embedding(input, offsets, per_sample_weights)

        # Run backward twice.
        smith.autograd.backward([local_res.sum()], retain_graph=True)
        smith.autograd.backward([local_res.sum()])
...
class DistAutogradTest(CommonDistAutogradTest):
    ...
    def test_graph_for_builtin_call(self):
        self._test_graph(smith.add, ExecMode.RPC_SYNC, False)
    ...
    def test_graph_for_builtin_remote_call(self):
        self._test_graph(smith.add, ExecMode.REMOTE, False)
    ...
    def _test_grad_only_on_return_value(self, exec_mode):
        ...
        with dist_autograd.context() as context_id:
            ...
            self.assertEqual(smith.ones_like(ret), grads[requires_grad_tensor])
    ...
    def test_dist_autograd_profiling(self):
        ...
        backward_event = get_event("smith::distributed::autograd::backward")
    ...
    def test_error_in_context(self):
        with dist_autograd.context():
            t1 = smith.rand(3, 3, requires_grad=True)
            t2 = smith.rand(6, 6, requires_grad=True)

            with self.assertRaises(RuntimeError):
                # This should throw an error since matrix sizes don't match.
                rpc.rpc_sync(
                    worker_name(self._next_rank()), smith.matmul, args=(t1, t2)
                )

    @dist_init
    def test_backward_no_grad_on_tensor(self):
        self._backward_no_grad_on_tensor(
            smith.rand((3, 3), requires_grad=True),
            smith.rand((3, 3), requires_grad=True),
            False,
        )
    ...
    @dist_init
    def test_trainer_ps_smithscript_functions(self):
        ...
        self._test_trainer_ps(
            create_smithscript_tensor, _run_trainer_smithscript, False
        )
    ...
    def test_backward_different_tensor_dims(self):
        local_grads = None
        t1 = smith.rand((4, 6), requires_grad=True)
        t2 = smith.rand((6, 5))
        t3 = smith.rand((5, 7), requires_grad=True)
        t4 = smith.rand((7, 9))

        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                val = self._exec_func(exec_mode, smith.matmul, t1, t2)
                val = self._exec_func(exec_mode, smith.linalg.multi_dot, (val, t3, t4))
                loss = val.sum()
    ...
    def test_backward_unused_tensors(self):
        ...
        t1 = smith.rand((3, 3), requires_grad=True)
        t2 = smith.rand((3, 3), requires_grad=True)
        t3 = smith.rand((3, 3), requires_grad=True)
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                s = self._exec_func(exec_mode, smith.stack, (t1, t2, t3))
                val = self._exec_func(
                    exec_mode,
                    smith.matmul,
                    smith.narrow(s, 0, 0, 1),
                    smith.narrow(s, 0, 2, 1),
                )
    ...
    def test_backward_multiple_output_tensors(self):
        local_grads = None
        t = smith.rand((10, 2), requires_grad=True)
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                tensor_list = self._exec_func(exec_mode, smith.split, t, 2)
                t1 = tensor_list[0]
                t2 = tensor_list[2]
                t3 = tensor_list[4]

                val = self._exec_func(exec_mode, smith.linalg.multi_dot, (t1, t2, t3))
    ...
    def _run_test_backward_unused_send_function_in_thread(self):
        with dist_autograd.context() as context_id:
            ...
            rpc.rpc_sync(worker_name(self._next_rank()), smith.add, args=(t1, t2))
    ...
    def test_backward_autograd_engine_error(self):
        with dist_autograd.context() as context_id:
            ...
            val = rpc.rpc_sync(worker_name(self._next_rank()), smith.add, args=(t2, t3))
            val = rpc.rpc_sync(
                worker_name(self._next_rank()), smith.mul, args=(val, t2)
            )
            val = rpc.rpc_sync(
                worker_name(self._next_rank()), smith.matmul, args=(val, t2)
            )
            val = rpc.rpc_sync(
                worker_name(self._next_rank()), smith.div, args=(val, t2)
            )
    ...
    def test_backward_node_failure(self):
        ...
        with dist_autograd.context() as context_id:
            t1 = smith.rand((3, 3), requires_grad=True)
            t2 = smith.rand((3, 3), requires_grad=True)
            res = rpc.rpc_sync(worker_name(self._next_rank()), smith.add, args=(t1, t2))
    ...
    def test_backward_invalid_args(self):
        with dist_autograd.context() as context_id:
            ...
            with self.assertRaisesRegex(
                ...
                t = smith.rand(3, 3, requires_grad=True)
                dist_autograd.backward(context_id, [t])

            with self.assertRaisesRegex(
                RuntimeError, "does not have a valid gradient function"
            ):
                t = smith.rand(1, requires_grad=True)
                dist_autograd.backward(context_id, [t])
    ...
    def test_backward_multiple_roots(self):
        ...
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC]:
            with dist_autograd.context() as context_id:
                r1 = self._exec_func(exec_mode, smith.add, t1, t2).sum()
                r2 = self._exec_func(exec_mode, smith.mul, t1, t2).sum()
                r3 = self._exec_func(exec_mode, smith.cos, t1).sum()
                r4 = self._exec_func(exec_mode, smith.div, t1, t2).sum()
    ...
    def test_backward_different_dtypes(self):
        self._backward_different_dtypes(
            smith.rand((3, 3), requires_grad=True, dtype=smith.float32),
            smith.rand((3, 3), requires_grad=True, dtype=smith.float64),
            False,
        )
    ...
    def _complex_python_udf(t1, t2):
        t3 = smith.nn.functional.linear(t1, t2)
        t4 = smith.nn.functional.linear(t2, t3)
        t5 = smith.nn.functional.linear(t3, t4)
        return smith.linalg.multi_dot([t1, t2, t3, t4, t5])
    ...
    def _python_udf_with_backward_error(t1, t2):
        ...
        return smith.linalg.multi_dot([t1, t2, t3, t4])
    ...
    def _nested_rpc_call_backward_error(t1, t2, dst):
        ...
        return smith.linalg.multi_dot([t1, t2, res])
    ...
    def test_clean_context_during_backward(self):
        ...
        for _ in range(100):
            dst = self._next_rank()
            t1 = rpc.rpc_sync(worker_name(dst), smith.add, args=(t1, t1))
        ...
        # https://github.com/blacksmith/blacksmith/issues/27643, which would inform
    ...
    def test_mixed_requires_grad(self):
        self._mixed_requires_grad(
            smith.rand(3, 3, requires_grad=True),
            smith.rand(3, 3, requires_grad=False),
            False,
        )
            ...
            for rank in range(self.world_size):
                if rank != self.rank:
                    res[i + 1] = rpc.rpc_sync(
                        worker_name(rank), smith.add, args=(res[i], t2)
                    )
                    i += 1
    ...
    def _workload_thread():
        ...
        t2 = smith.rand((3, 3), requires_grad=True)
        with dist_autograd.context() as context_id:
            t3 = rpc.rpc_sync("worker0", smith.add, args=(t1, t2))
            t4 = rpc.rpc_sync("worker0", smith.mul, args=(t2, t3))
            t5 = rpc.rpc_sync("worker0", smith.matmul, args=(t3, t4))
            t6 = rpc.rpc_sync("worker0", smith.add, args=(t4, t5))
    ...
    def test_backward_accumulate_grads(self):
        ...
        t2 = smith.rand((3, 3), requires_grad=True)
        with dist_autograd.context() as context_id:
            t3 = smith.matmul(t1, t2)
            # Run backward twice.
            smith.autograd.backward([t3.sum()], retain_graph=True)
            smith.autograd.backward([t3.sum()])
    ...
    def _test_nested_backward_accumulate_grads(t1, t2, dst_rank):
        return rpc.rpc_sync(worker_name(dst_rank), smith.add, args=(t1, t2))
    ...
    def test_backward_verify_hooks(self):
        ...
        t2 = smith.ones((3, 3), requires_grad=True)
        local_grads = None
        for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC, ExecMode.REMOTE]:
            with dist_autograd.context() as context_id:
                ret = self._exec_func(exec_mode, smith.matmul, t1, t2)
                loss = ret.sum()
                ret = self._verify_backwards(
                    exec_mode, [loss], context_id, local_grads, t1, t2
                )
    ...
    def test_post_hooks(self):
        ...
        t = smith.rand(10, 10, requires_grad=True)
        a = t + t
...
class CudaDistAutogradTest(CommonDistAutogradTest):
    ...
    def test_gpu_simple(self):
        t1 = smith.rand(3, 3, requires_grad=True, device="cuda:0")
        t2 = smith.rand(3, 3, requires_grad=True, device="cuda:0")
        (t1 + t2).sum().backward()
        with dist_autograd.context() as context_id:
            t3 = t1 + t2
            dist_autograd.backward(context_id, [t3.sum()])
    ...
    def test_gpu_to_cpu_continuation(self):
        ...
        for _ in range(3):
            ...
            for exec_mode in [ExecMode.LOCAL, ExecMode.RPC_SYNC]:
                with dist_autograd.context() as context_id:
                    t3 = self._exec_func(exec_mode, smith.add, t2, t2)
                    t4 = t3.cuda(0) + t1
                    t5 = self._exec_func(exec_mode, smith.add, t4.cpu(), t2)
                    t6 = t5.cuda(0) + t4
                    t7 = self._exec_func(exec_mode, smith.add, t6.cpu(), t5)
...
class TensorPipeCudaDistAutogradTest(RpcAgentTestFixture):
    ...
    def test_device_maps_backward_pass(self):
        ...
        t1 = smith.rand(10, device=self.rank, requires_grad=True)
        t2 = smith.rand(10, device=self.rank, requires_grad=True)
        with dist_autograd.context() as context_id:
            res = rpc.rpc_sync(dst, smith.add, args=(t1, t2))
            dist_autograd.backward(context_id, [res.sum()])
            grads = dist_autograd.get_gradients(context_id)
            self.assertEqual(smith.ones(10), grads[t1])
            self.assertEqual(smith.ones(10), grads[t2])
            self.assertEqual(t1.device, grads[t1].device)
            self.assertEqual(t2.device, grads[t2].device)

        rpc.shutdown()

    class MyRemoteCompute(smith.nn.Module):
        def forward(self, input):
            input = input * 2.0
            return input

    class MyLocalCompute(smith.nn.Module):
    ...
    def test_dist_autograd_sync_streams(self):
        ...
        for _ in range(10):
            input = smith.rand([1000, 10000], device=self.rank, requires_grad=True)
            # Run local autograd
            result = input * 2.0
            r = random.random()
            loss = result.sum() * r
