...
from smith.autograd.profiler_legacy import profile as _profile
from smith.distributed.rpc import (
    _get_debug_info,
    _rref_context_get_debug_info,
    RRef,
    WorkerInfo,
)
from smith.distributed.rpc.api import _thread_local_var, _use_rpc_pickler, _wait_all
from smith.distributed.rpc.internal import (
    _build_rpc_profiling_key,
    _internal_rpc_pickler,
    PythonUDF,
    RPCExecMode,
)
from smith.futures import Future
...
def foo_add():
    return smith.add(smith.ones(1), smith.ones(1))


def udf_with_smith_ops(device=-1, use_record_function=False):
    device_ctx = contextlib.nullcontext() if device == -1 else smith.cuda.device(device)
    record_function_ctx = (
        smith.autograd.profiler.record_function("##forward##")
        if use_record_function
        else contextlib.nullcontext()
    )
    with device_ctx, record_function_ctx:
        t1, t2 = smith.ones(1), smith.ones(1)
        t = smith.add(t1, t2)
        t = smith.mul(t, t)
        t = t.relu()
        t = t.sigmoid()
...
class MyPickleClass:
    ...
    def __getstate__(self):
        (pickled_python_udf, tensors) = _internal_rpc_pickler.serialize(
            PythonUDF(my_tensor_function, (smith.ones(2, 2), smith.ones(2, 2)), None)
        )
...
class MyClass:
    ...
    def my_slow_method(self, my_tensor_arg):
        time.sleep(5)
        return smith.add(self.a, my_tensor_arg)
...
def build_sparse_tensor(coalesce=False):
    i = [[0, 1, 1], [2, 0, 2]]
    v = [3, 4, 5]
    tensor = smith.sparse_coo_tensor(i, v, (2, 3))
    if coalesce:
...
def non_cont_test(t_view, t_cont):
    ...
    if not smith.equal(t_view, t_cont):
...
def my_sleep_func(seconds=1):
    time.sleep(seconds)
    return smith.mul(smith.tensor(1), smith.tensor(1))
...
def nested_rpc(dst):
    return rpc.rpc_sync(dst, smith.add, args=(smith.ones(2, 2), 1))


def nested_rpc_sparse(dst):
    return rpc.rpc_sync(
        dst, smith.add, args=(build_sparse_tensor(), build_sparse_tensor())
    )
...
def nested_rref(dst):
    return (
        rpc.remote(dst, smith.add, args=(smith.ones(2, 2), 1)),
        rpc.remote(dst, smith.add, args=(smith.ones(2, 2), 2)),
    )


def nested_rref_sparse(dst):
    return (
        rpc.remote(dst, smith.add, args=(build_sparse_tensor(), build_sparse_tensor())),
        rpc.remote(dst, smith.add, args=(build_sparse_tensor(), build_sparse_tensor())),
    )


def nested_remote(dst):
    rref = rpc.remote(dst, smith.add, args=(smith.ones(2, 2), 3))
    return rref.to_here()
...
def rpc_return_rref(dst):
    return rpc.remote(dst, smith.add, args=(smith.ones(2, 2), 1))
...
@smith.jit.script
def heavy_rpc_smithscript(tensor):
    for i in range(1, 100):
        tensor *= i
        tensor /= i + 1
    return 0
...
def my_script_func(tensor):
    return smith.add(tensor, tensor)
...
@smith.jit.script
def raise_func_script(expected_err: str) -> smith.Tensor:
    raise ValueError(expected_err)
...
def add_use_future_cb(to, x, y, z):
    ...
    fut = rpc.rpc_async(to, smith.add, args=(x, y))
    fut.then(callback)
    return out.result()
...
def add_use_future_nested_cb(to, x, y, z):
    ...
    def callback(fut1):
        fut2 = rpc.rpc_async(to, smith.add, args=(fut1.wait(), z))
        fut2.then(lambda fut2: out.set_result(fut2.wait()))

    fut1 = rpc.rpc_async(to, smith.add, args=(x, y))
    fut1.then(callback)
    return out.wait()
...
def async_add(to, x, y):
    return rpc.rpc_async(to, smith.add, args=(x, y))
...
def async_add_with_future_ctor(to, x, y, z):
    fut = smith.futures.Future()
    rpc.rpc_async(to, smith.add, args=(x, y)).then(
...
def async_add_chained(to, x, y, z):
    return rpc.rpc_async(to, smith.add, args=(x, y)).then(lambda fut: fut.wait() + z)


@rpc.functions.async_execution
def async_add_chained_multi(to, x, num, step):
    fut = rpc.rpc_async(to, smith.add, args=(x, 0))
    for _ in range(num):
...
def async_add_multi_fanout(to, x, num, step):
    ...
    for i in range(num):
        if i == 0:
            futs.append(rpc.rpc_async(to, smith.add, args=(x, step)))
        else:
            futs.append(rpc.rpc_async(to, smith.add, args=(0, step)))

    # TODO: use smith.futures.collect_all
    lock = Lock()
    state = {"cnt": 0, "ret": smith.zeros_like(x)}
    ret_future = smith.futures.Future()
...
def async_cuda_sleep_and_set_to_one(t):
    device = t.device
    original_stream = smith.cuda.current_stream(device)
    new_stream = smith.cuda.Stream(device)
    new_stream.wait_stream(original_stream)
    with smith.cuda.stream(new_stream):
        smith.cuda._sleep(int(1000 * get_cycles_per_ms()))
        t.fill_(1)
...
def async_cuda_nested_add(to, x, y, z):
    ...
    return rpc.rpc_async(to, smith.add, args=(x, y)).then(cb)
...
class TensorWrapper:
    ...
    def __init__(self, t):
        ...
        self.lock = Lock()
        self.event = smith.cuda.Event(enable_timing=True)
...
class AsyncExecutionClass:
    ...
    def static_async_add(to, x, y, z):
        return rpc.rpc_async(to, smith.add, args=(x, y)).then(
            lambda fut: fut.wait() + z
        )
...
class MyEmbeddingBagModel(smith.nn.Module):
    def __init__(self, sparse):
        super().__init__()
        self.eb = smith.nn.EmbeddingBag(10, 10, sparse=sparse)
...
class MyConvNetForMNIST(nn.Module):
    ...
    def forward(self, x, is_rref=False):
        ...
        with smith.cuda.stream(smith.cuda.current_stream(self.device)):
            # intentionally adding delay to current CUDA stream
            smith.cuda._sleep(10 * FIFTY_MIL_CYCLES)
            return self.net(x)
...
class RpcTestCommon:
    ...
    def _run_uneven_workload(self, f, x, num_repeat=30):
        ...
        if self.rank == 0:
            ...
            for fut in smith.futures.collect_all(futs).wait():
            ...
            for val in smith.futures.wait_all(futs):
    ...
    def _py_rref_args(self, a, b, x, y, expected):
        n = self.rank + 1
        dst_rank = n % self.world_size
        rref_a = rpc.remote(worker_name(dst_rank), smith.add, args=(a, b))
        rref_b = rpc.remote(worker_name(dst_rank), smith.add, args=(x, y))
        rref_c = rpc.remote(
    ...
    def _trainer_func(self, rref, sparse):
        ...
        loss_fn = nn.MSELoss()
        for i in range(10):
            outputs = m(smith.rand(10, 10).long())
            loss_fn(outputs, smith.rand(10, 10)).backward()
            ...
            self.assertTrue(smith.equal(gradient, ps_gradient))
    ...
    def _my_parameter_server(self, sparse):
        ...
        smith.futures.wait_all(futures)
    ...
    def _test_cuda_future_extraction(self, wrapper, unwrapper, sparse_tensor):
        ...
        with smith.cuda.device("cuda:0"):
            stream = smith.cuda.Stream()
            another_stream = smith.cuda.Stream()
            with smith.cuda.stream(stream):
                if sparse_tensor:
                    ...
                    tensor = smith.zeros((100,), device="cuda:0")
                    add_tensor = smith.ones((100,), device="cuda:0")
            ...
            with smith.cuda.stream(another_stream):
                tensor = unwrapper(future.wait())
                if sparse_tensor:
                    self.assertTrue(
                        smith.eq(tensor.indices(), expected_tensor.indices())
                        .all()
                        .item()
                    )
                    self.assertTrue(
                        smith.eq(tensor.values(), expected_tensor.values()).all().item()
                    )
                    self.assertEqual(tensor.size(), expected_tensor.size())
                else:
                    self.assertTrue(smith.eq(tensor, expected_tensor).all().item())
...
class RpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def test_self_add(self):
        self_worker_info = rpc.get_worker_info()
        fut = rpc.rpc_async(self_worker_info, smith.add, args=(smith.ones(2, 2), 1))
        ret = rpc.rpc_sync(self_worker_info, smith.add, args=(smith.ones(2, 2), 1))
        self.assertEqual(fut.wait(), smith.ones(2, 2) + 1)
        self.assertEqual(ret, smith.ones(2, 2) + 1)
    ...
    def test_send_to_rank(self):
        ...
        for exec_mode in [RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE]:
            ret = self._run_func_in_mode(
                dst_rank, smith.add, exec_mode, args=(smith.ones(2, 2), 1)
            )
            self.assertEqual(ret, smith.ones(2, 2) + 1)
        ...
        for exec_mode in [RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE]:
            with self.assertRaises(RuntimeError):
                self._run_func_in_mode(
                    ...
                    smith.add,
                    exec_mode,
                    args=(smith.ones(2, 2), 1),
                )

        for exec_mode in [RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE]:
            with self.assertRaises(RuntimeError):
                self._run_func_in_mode(
                    -1, smith.add, exec_mode, args=(smith.ones(2, 2), 1)
                )

        for exec_mode in [RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE]:
            with self.assertRaises(ValueError):
                self._run_func_in_mode(
                    dst_rank + 0.5, smith.add, exec_mode, args=(smith.ones(2, 2), 1)
                )
    ...
    def test_self_py_udf_remote(self):
        self._self_py_udf_remote(rpc.get_worker_info(), smith.ones(2, 2), 1, 3)

    @dist_init
    def test_self_remote_rref_as_rpc_arg(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        self._self_remote_rref_as_rpc_arg(dst, smith.ones(2, 2), 1, 3)

    @dist_init
    def test_self_remote_rref_as_self_rpc_arg(self):
        self._self_remote_rref_as_rpc_arg(rpc.get_worker_info(), smith.ones(2, 2), 1, 3)

    @dist_init
    def test_self_remote_rref_as_remote_arg(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        self._self_remote_rref_as_remote_arg(dst, smith.ones(2, 2), 1, 3)

    @dist_init
    def test_self_remote_rref_as_self_remote_arg(self):
        self._self_remote_rref_as_remote_arg(
            rpc.get_worker_info(), smith.ones(2, 2), 1, 3
        )

    @dist_init
    def test_rref_proxy_non_exist(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        rref = rpc.remote(dst, my_function, args=(smith.ones(2, 2), 1, 3))
    ...
    def _test_rref_proxy_class(self, dst):
        ...
        self.assertEqual(
            expected.my_class_method(2, smith.zeros(2, 2)),
            rref.rpc_sync().my_class_method(2, smith.zeros(2, 2)),
        )
        self.assertEqual(
            expected.my_class_method(2, smith.ones(3, 3)),
            rref.rpc_async().my_class_method(2, smith.ones(3, 3)).wait(),
        )
        self.assertEqual(
            expected.my_class_method(2, smith.ones(4, 4)),
            rref.remote().my_class_method(2, smith.ones(4, 4)).to_here(),
        )
    ...
    @mock.patch.object(smith.distributed.autograd, "_init")
    @mock.patch.object(smith.distributed.rpc.api, "_set_and_start_rpc_agent")
    ...
    def test_pg_init_no_rpc_init(self):
        ...
        model = MyModel()
        model.train()
        model = smith.nn.parallel.DistributedDataParallel(model)
    ...
    def test_world_size_one(self):
        self._world_size_one(smith.ones(2, 2), smith.ones(2, 2))
    ...
    def test_add(self):
        ...
        ret = rpc.rpc_sync(
            worker_name(dst_rank),
            smith.add,
            args=(smith.ones(n, n), smith.ones(n, n)),
        )
        self.assertEqual(ret, smith.ones(n, n) * 2)
    ...
    def test_add_with_id(self):
        ...
        ret = rpc.rpc_sync(
            workder_info, smith.add, args=(smith.ones(n, n), smith.ones(n, n))
        )
        self.assertEqual(ret, smith.ones(n, n) * 2)
    ...
    def test_scalar_add(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        ret = rpc.rpc_sync(worker_name(dst_rank), smith.add, args=(smith.ones(n, n), n))
        self.assertEqual(ret, (smith.ones(n, n) + n))
    ...
    def test_async_add(self):
        ...
        self.assertEqual(fut.wait(), smith.ones(n, n) * 2)

    @dist_init
    def test_nonzero(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        x = smith.ones(self.world_size, self.world_size)
        x[self.rank][self.rank] = 0
        ret = rpc.rpc_sync(worker_name(dst_rank), smith.nonzero, args=(x,))
    ...
    def test_wait_all_workers_dense(self):
        self._wait_all_workers(heavy_rpc, smith.ones(100, 100))

    def test_wait_all_workers_twice_dense(self):
        self._wait_all_workers_twice(heavy_rpc, smith.ones(100, 100))
    ...
    def test_graceful_shutdown_with_uneven_workload(self):
        ...
        self._run_uneven_workload(heavy_rpc, smith.ones(100, 100))
    ...
    def check_profiling_info(
        ...
        if isinstance(func, smith.jit.ScriptFunction):
            self.assertTrue(smith._jit_internal._qualified_name(func) in rpc_event.name)
        else:
    ...
    def test_profiler_rpc_record_shapes(self):
        ...
        t1, t2 = smith.ones(100), smith.ones(100)
        with _profile(record_shapes=True) as prof:
            rpc.rpc_sync(dst_worker, smith.add, args=(t1, t2))
    ...
    def test_profiler_rpc_memory(self):
        ...
        with _profile(profile_memory=True) as p:
            fut = rpc.rpc_async(dst_worker, udf_with_smith_ops, args=())
            fut.wait()
    ...
    def test_profiler_rpc_key_names(self):
        ...
        def rpc_with_profiling(dst_worker):
            ...
            rpc_profiling_key = _build_rpc_profiling_key(
                RPCExecMode.ASYNC,
                udf_with_smith_ops.__qualname__,
                worker_name(self.rank),
                dst_worker,
            )
    ...
    def _run_rpc_profiling_async_function(self, device="cpu"):
        ...
        remote_add_key = (
            expected_key + REMOTE_OP_STR + smith.jit._builtins._find_builtin(smith.add)
        )
        remote_add_event = [
    ...
    def test_rpc_profiling_remote_record_function(self):
        ...
        for dst_rank in dst_ranks:
            ...
            with _profile() as prof:
                fut = rpc.rpc_async(dst_worker, udf_with_smith_ops, args=(-1, True))
                fut.wait()
    ...
    def _profiler_test_with_rpc(
        ...
        p = _profile if not kineto_profile else smith.profiler.profile  # kineto
        if self.rank == 1:
            with p() as prof:
                record_function_ctx_mgr = (
                    contextlib.nullcontext()
                    if not use_record_function
                    else smith.autograd.profiler.record_function("foo")
                )
    ...
    def _run_test_profiler_with_sync_rpc_builtin(self):
        self._profiler_test_with_rpc(
            RPCExecMode.SYNC, smith.mul, args=(smith.ones(1), smith.ones(1))
        )
        self._profiler_test_with_rpc(
            RPCExecMode.SYNC,
            smith.mul,
            args=(smith.ones(1), smith.ones(1)),
    ...
    def _run_test_profiler_with_async_rpc_builtin(self):
        self._profiler_test_with_rpc(
            RPCExecMode.ASYNC, smith.mul, args=(smith.ones(1), smith.ones(1))
        )
        self._profiler_test_with_rpc(
            RPCExecMode.ASYNC,
            smith.mul,
    ...
    def _run_test_profiler_with_remote_builtin(self):
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE, smith.mul, args=(smith.ones(1), smith.ones(1))
        )
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE,
            smith.mul,
    ...
    def _run_test_profiler_with_script_async_rpc(self):
        self._profiler_test_with_rpc(
            RPCExecMode.ASYNC, my_script_func, args=(smith.tensor(1),)
        )
        self._profiler_test_with_rpc(
            RPCExecMode.ASYNC,
            my_script_func,
    ...
    def _run_test_profiler_with_script_sync_rpc(self):
        self._profiler_test_with_rpc(
            RPCExecMode.SYNC, my_script_func, args=(smith.tensor(1),)
        )
        self._profiler_test_with_rpc(
            RPCExecMode.SYNC,
            my_script_func,
    ...
    def _run_test_profiler_with_script_remote_rpc(self):
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE, my_script_func, args=(smith.tensor(1),)
        )
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE,
            my_script_func,
        ...
        # test remote to self
        self._profiler_test_with_rpc(
            RPCExecMode.REMOTE, my_script_func, args=(smith.tensor(1),), dst=self.rank
        )
    ...
    def test_server_process_global_profiler(self):
        ...
        rpc.rpc_sync(dst_worker_name, smith.add, (x, y))
        inner_profile_rref = rpc.remote(
            dst_worker_name, rpc._server_process_global_profile
        )
        inner_profile_rref.rpc_sync().__enter__()
        rpc.rpc_sync(dst_worker_name, smith.sub, (x, y))
    ...
    def test_async_record_function_double_end_callbacks(self):
        ...
        if self.rank == 1:
            ...
            with _profile():
                with smith.autograd.profiler.record_function("foo") as rf:
                    fut = rpc.rpc_async(
    ...
    def test_async_record_function_legacy(self):
        ...
        # Note: These exist for backward compatibility with SmithScript
        num_sleep_seconds = 1
        if self.rank == 1:
            with _profile():
                try:
                    handle = smith.ops.profiler._record_function_enter("foo", None)
                    fut = rpc.rpc_async(
                        worker_name(0), my_sleep_func, args=(num_sleep_seconds,)
                    )
                    smith.ops.profiler._call_end_callbacks_on_jit_fut(handle, fut)
                finally:
        ...
        if self.rank == 1:
            with _profile() as pf:
                key = _build_rpc_profiling_key(
                    RPCExecMode.ASYNC,
                    smith._jit_internal._qualified_name(my_script_func),
                    "worker1",
                    "worker0",
                )
                with smith.autograd.profiler.record_function(key) as rf:
                    fut = rpc.rpc_async(
                        worker_name(0), my_script_func, args=(smith.tensor(1),)
                    )
                    # Intentionally calling record_function internals
                    fut = smith.ops.profiler._call_end_callbacks_on_jit_fut(
                        rf.record, fut
                    )
                result = fut.wait()
                # Validate that the profiling future returns the same value as the RPC
                # future.
                expected = smith.add(smith.tensor(1), smith.tensor(1))
                self.assertEqual(result, expected)
            events = pf.function_events
            rpc_event = get_function_event(
                events, smith._jit_internal._qualified_name(my_script_func)
            )
            self.assertTrue(
                smith._jit_internal._qualified_name(my_script_func) in rpc_event.name
            )
    ...
    def test_py_tensors(self):
        ...
        self.assertEqual(ret, my_tensor_function(smith.ones(n, n), smith.ones(n, n)))
    ...
    def test_py_tensors_multi_async_call(self):
        ...
        for i in range(100):
            fut = rpc.rpc_async(
                worker_name(dst_rank),
                my_tensor_function,
                args=(smith.ones(i, i), smith.ones(i, i)),
            )
            futs.append(fut)

        for j, val in enumerate(smith.futures.wait_all(futs)):
            self.assertEqual(
                val, my_tensor_function(smith.ones(j, j), smith.ones(j, j))
            )
    ...
    def test_py_tensors_in_container(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        a = [smith.ones(n, n), smith.ones(n, n)]
        b = TensorClass(build_complex_tensors())
        c = {"foo": smith.ones(n, n), "bar": smith.ones(n, n)}
        ret = rpc.rpc_sync(
    ...
    def test_py_nested_pickle(self):
        ...
        ret = rpc.rpc_sync(
            worker_name(dst_rank),
            run_nested_pickle,
            args=(MyPickleClass(), smith.ones(2, 2)),
        )

        m = MyPickleClass()
        m.set(my_tensor_function(smith.ones(2, 2), smith.ones(2, 2)))
        self.assertEqual(ret, run_nested_pickle(m, smith.ones(2, 2)))
    ...
    def test_nested_rpc(self):
        self._nested_rpc(nested_rpc, smith.ones(2, 2) + 1)
    ...
    def test_stress_heavy_rpc(self):
        self._stress_test_rpc(heavy_rpc, repeat=20, args=(smith.ones(100, 100),))

    @dist_init
    def test_stress_heavy_rpc_smithscript(self):
        self._stress_test_rpc(
            heavy_rpc_smithscript, repeat=20, args=(smith.ones(100, 100),)
        )

    @dist_init
    def test_builtin_remote_ret(self):
        self._builtin_remote_ret(
            smith.ones(2, 2), smith.ones(2, 2), smith.ones(2, 2) * 2
        )
    ...
    def _multi_args_fn(n, sparse=False):
        if sparse:
            ...
            return (smith.ones(n, n), smith.ones(n, n))

    @dist_init
    def test_multi_builtin_remote_ret(self):
        self._test_multi_remote_call(smith.add, False, args_fn=RpcTest._multi_args_fn)
    ...
    def _multi_kwargs_fn(n, sparse=False):
        if sparse:
            ...
            return {"a": smith.ones(n, n), "b": smith.ones(n, n), "c": smith.ones(n, n)}
    ...
    def test_py_rref_args(self):
        self._py_rref_args(
            smith.ones(2, 2), 1, smith.ones(2, 2), 2, smith.ones(2, 2) * 2 + 3
        )

    @dist_init
    def test_py_rref_args_user_share(self):
        self._py_rref_args_user_share(
            smith.ones(2, 2), 1, 2, smith.ones(2, 2), 3, 4, smith.ones(2, 2) * 2 + 10
        )
    ...
    def test_nested_remote(self):
        self._nested_remote(nested_remote, smith.ones(2, 2) + 3)

    @dist_init
    def test_nested_rref(self):
        self._nested_rref(nested_rref, smith.ones(2, 2) + 1, smith.ones(2, 2) + 2)

    @dist_init
    def test_nested_rref_stress(self):
        self._nested_rref_stress(
            nested_rref, smith.ones(2, 2) + 1, smith.ones(2, 2) + 2
        )
    ...
    def test_rpc_return_rref(self):
        ...
        self.assertEqual(rref.to_here(), smith.ones(2, 2) + 1)
    ...
    def test_rref_forward_chain(self):
        ...
        rref = rpc.remote(worker_name(dst_rank), smith.add, args=(smith.ones(n, n), 1))
        ...
        ret = ret_rref
        self.assertEqual(ret, smith.add(smith.ones(n, n), 1))
    ...
    def test_local_value_not_on_owner(self):
        ...
        rref = rpc.remote(
            worker_name(next_rank), smith.add, args=(smith.ones(1), smith.ones(1))
        )
        with self.assertRaisesRegex(
            RuntimeError,
            (
    ...
    def _test_rref_type(self, blocking):
        ...
        dst = worker_name((self.rank + 1) % self.world_size)
        rref = rpc.remote(dst, smith.add, args=(smith.ones(2), 1))
        ...
        expected_type = type(smith.ones(2))
        ...
        self.assertEqual(t, type(smith.ones(2)))
    ...
    def _test_rref_type_owner(self, blocking):
        rref = RRef(smith.ones(2) + 1)
        rref_type = rref._get_type(blocking=blocking)
        if not blocking:
            rref_type = rref_type.wait()
        self.assertEqual(rref_type, type(smith.ones(2)))
    ...
    def test_rref_type_slow_init(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        rref = rpc.remote(dst, RpcTest._slow_add, args=(smith.ones(2), 1))
        self.assertEqual(rref._get_type(), type(smith.ones(2)))
    ...
    def test_owner_equality(self):
        ...
        other_a = rpc.remote(
            worker_name(other_rank), smith.add, args=(smith.ones(1), 1)
        )
        other_b = rpc.remote(
            worker_name(other_rank), smith.add, args=(smith.ones(1), 1)
        )
    ...
    def test_remote_same_worker(self):
        ...
        dst_rank = n % self.world_size
        rref_a = rpc.remote(
            worker_name(dst_rank), smith.add, args=(smith.ones(n, n), 2)
        )
        rref_b = rpc.remote(
            worker_name(dst_rank), smith.add, args=(smith.ones(n, n), 1)
        )
        rref_c = rpc.remote(
            worker_name(dst_rank), my_rref_function, args=(rref_a, rref_b)
        )
        self.assertEqual(rref_c.to_here(), smith.ones(n, n) + 4)
    ...
    # `smith.distributed.rpc.api`, so patching
    # `smith.distributed.rpc._delete_all_user_and_unforked_owner_rrefs` will
    # not help.
    @mock.patch.object(
        smith.distributed.rpc.api, "_delete_all_user_and_unforked_owner_rrefs"
    )
    def _test_rref_leak(
        ...
        import smith.distributed.rpc.api as api
    ...
    def test_rref_str(self):
        ...
        dst_rank = (self.rank + 1) % self.world_size
        rref2 = rpc.remote(worker_name(dst_rank), smith.add, args=(smith.ones(2, 2), 1))
        self.assertEqual(
            rref2.__str__(),
    ...
    def test_rref_get_future(self):
        ...
        if self.rank == 0:
            # Builtin
            rref = rpc.remote(worker_name(1), smith.add, args=(1, 1))
            rref.to_here()
            fut = rref._get_future()
            self.assertIsInstance(fut, smith._C.Future)
            ...
            # Script
            rref = rpc.remote(worker_name(1), my_script_func, args=(smith.tensor(1),))
            rref.to_here()
            fut = rref._get_future()
            self.assertIsInstance(fut, smith._C.Future)
    ...
    def test_rref_context_debug_info(self):
        ...
        ####################################################
        rref2 = rpc.remote(worker_name(dst_rank), smith.add, args=(smith.ones(2, 2), 1))
        rref3 = rpc.remote(worker_name(dst_rank), smith.add, args=(smith.ones(2, 2), 1))
        rref2.to_here()
        rref3.to_here()
    ...
    def test_disable_gil_profiling(self):
        ...
        dst_rank = (self.rank + 1) % self.world_size
        rpc.rpc_sync(
            worker_name(dst_rank), smith.add, args=(smith.ones(1), smith.ones(1))
        )
    ...
    def test_handle_send_exceptions(self):
        ...
        if self.rank == 1:
            ...
            fut = rpc.rpc_async(dst_worker, smith.add, args=(smith.ones(1), 3))
    ...
    def test_deadlock(self):
        # this test is copied from https://github.com/blacksmith/blacksmith/issues/45089
        if self.rank == 1:
            dst1 = worker_name((self.rank + 1) % self.world_size)
            x = smith.ones(2)
            y = smith.ones(2)
    ...
    def test_rpc_timeouts(self):
        # TODO: enable timeouts for rpc.remote/RRef (https://github.com/blacksmith/blacksmith/issues/33803)
        dst_rank = (self.rank + 1) % self.world_size
        dst_worker = worker_name(dst_rank)
        timeout = 0.1  # 100 ms
        expected_error = self.get_timeout_error_regex()
    ...
    def test_use_rpc_pickler(self):
        ...
        with _use_rpc_pickler(test_pickler):
            self.assertTrue(smith.distributed.rpc.api._default_pickler is test_pickler)
        self.assertTrue(
            smith.distributed.rpc.api._default_pickler is _internal_rpc_pickler
        )
    ...
    def test_wait_all(self):
        with _wait_all():
            ...
            dst = worker_name((self.rank + 1) % self.world_size)
            fut = rpc.rpc_async(dst, smith.add, (smith.ones(2, 2), 1))
            self.assertTrue(len(_thread_local_var.future_list) == 1)
            self.assertTrue(
                isinstance(_thread_local_var.future_list[0], smith._C.Future)
            )
    ...
    def test_wait_all_multiple_call(self):
        with _wait_all():
            ...
            dst = worker_name((self.rank + 1) % self.world_size)
            for i in range(20):
                fut = rpc.rpc_async(dst, smith.add, (smith.ones(i, i), 1))
                res = rpc.rpc_sync(dst, smith.add, (smith.ones(i, i), 1))
                self.assertEqual(res, smith.ones(i, i) + 1)
                self.assertEqual(fut.wait(), smith.ones(i, i) + 1)
    ...
    def test_wait_all_exit_early_python(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_err):
            smith.futures.wait_all([fut1, fut2, fut3])
    ...
    def test_wait_all_exit_early_builtin(self):
        ...
        dst = worker_name((self.rank + 1) % self.world_size)
        fut1 = rpc.rpc_async(dst, RpcTest.timed_out_rpc)
        fut2 = rpc.rpc_async(dst, smith.add, args=(smith.rand(10), smith.rand(5)))
        fut3 = rpc.rpc_async(dst, smith.add, args=(smith.rand(10), smith.rand(5)))
    ...
    def test_non_garbage_collected_user_rref_due_to_local_circular_dependency(self):
        ...
        n = self.rank
        a.rref = rpc.remote(dst_worker_name, smith.add, args=(smith.ones(n, n), 2))
    ...
    def test_use_rref_after_shutdown(self):
        ...
        with self.assertRaisesRegex(
            ...
            import smith.distributed.rpc.internal as internal

            internal.serialize(rref)

    @staticmethod
    def _return_gpu_tensor():
        return smith.rand(3, 3).cuda(0)

    @staticmethod
    def _return_gpu_tensor_list():
        return [smith.rand(3, 3).cuda(0), smith.rand(3, 3).cuda(1)]

    @staticmethod
    def _gpu_tensor_list_arg(tensor_list):
        return smith.rand(3, 3)

    def _create_rref(self):
        owner_rank = (self.rank + 2) % self.world_size
        return rpc.remote(
            worker_name(owner_rank), smith.add, args=(smith.zeros(2, 2), 1)
        )
    ...
    def test_callback_simple(self):
        ...
        self.assertEqual(fut.wait(), smith.ones(n, n) * 2)
        self.assertEqual(set_by_cb.result(), smith.ones(n, n) * 2 + 1)
        self.assertEqual(fut.wait(), smith.ones(n, n) * 2)
    ...
    def test_callback_wrong_arg_type(self):
        ...
        fut0 = rpc.rpc_async(dst, smith.add, args=(smith.ones(2, 2), 1))
    ...
    def test_callback_multi(self):
        ...
        for idx in range(num_cbs):
            self.assertEqual(cb_futs[idx].wait(), smith.ones(n, n) * 2 + idx)
    ...
    def test_callback_chain(self):
        ...
        fut = rpc.rpc_async(
            worker_name(n % self.world_size), smith.add, args=(smith.ones(n, n), 1)
        )
        ...
        self.assertEqual(fut.wait(), smith.ones(n, n) + 1 + num_cbs)
    ...
    def test_callback_in_rpc(self):
        ...
        ret = rpc.rpc_sync(dst1, add_use_future_cb, args=(dst2, smith.ones(2, 2), 1, 2))
        self.assertEqual(ret, smith.ones(2, 2) + 1 + 2)
    ...
    def test_callback_with_ret(self):
        ...
        def callback(fut0):
            fut2 = rpc.rpc_async(dst, smith.add, args=(fut0.wait(), 1)).then(
                lambda fut1: fut1.wait() + 1
            )

            return fut2.wait()

        fut3 = rpc.rpc_async(dst, smith.add, args=(smith.ones(2, 2), 1)).then(callback)

        self.assertEqual(fut3.wait(), smith.ones(2, 2) + 3)
    ...
    def test_mark_future_twice(self):
        fut = rpc.rpc_async(
            worker_name((self.rank + 1) % self.world_size),
            smith.add,
            args=(smith.zeros(2, 2), 1),
        )
        self.assertEqual(fut.wait(), smith.zeros(2, 2) + 1)
        with self.assertRaisesRegex(
    ...
    def test_pickle_future(self):
        fut = smith.futures.Future()
        errMsg = "Can not pickle smith.futures.Future"
    ...
    def test_future_done(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        fut = rpc.rpc_async(dst, smith.add, args=(smith.zeros(2), 1))
        fut.wait()
        self.assertTrue(fut.done())
    ...
    def _test_future_cb(self, func):
        ...
        ret = rpc.rpc_sync(dst1, func, args=(dst2, smith.ones(2, 2), 1, 2))
        self.assertEqual(ret, smith.ones(2, 2) + 1 + 2)
    ...
    def test_async_function_simple(self):
        ...
        ret = rpc.rpc_sync(dst1, async_add, args=(dst2, smith.ones(2, 2), 1))
        self.assertEqual(ret, smith.ones(2, 2) + 1)
    ...
    def _test_async_function(self, fn, mode=RPCExecMode.SYNC):
        ...
        args = (dst2, smith.ones(2, 2), 1, 2)
        ret = self._run_func_in_mode(dst1, fn, mode, args=args)
        self.assertEqual(ret, smith.ones(2, 2) + 3)
    ...
    def _test_async_function_multi(self, fn, mode=RPCExecMode.SYNC):
        ...
        num = 20
        step = 3
        args = (dst2, smith.ones(2, 2), num, step)
        ret = self._run_func_in_mode(dst1, fn, mode, args=args)
        self.assertEqual(ret, smith.ones(2, 2) + num * step)
    ...
    def _test_return_future(self, mode):
        with self.assertRaisesRegex(
            RuntimeError, "Can not pickle smith.futures.Future"
        ):
            self._run_func_in_mode(
    ...
    @skip_but_pass_in_sandcastle_if(
        ...
        "init_pg_then_rpc does not work with TCP init, see https://github.com/blacksmith/blacksmith/issues/41614.",
    )
    def test_init_pg_then_rpc(self):
        ...
        next_rank = (self.rank + 1) % self.world_size
        ret = rpc.rpc_sync(
            worker_name(next_rank), smith.add, args=(smith.ones(2, 2), 1)
        )
        self.assertEqual(ret, smith.ones(2, 2) + 1)
    ...
    @skip_but_pass_in_sandcastle_if(
        os.environ.get("RPC_INIT_WITH_TCP", None) == "1",
        "init_rpc_then_pg does not work with TCP init, see https://github.com/blacksmith/blacksmith/issues/41614.",
    )
    ...
    def test_wait_all_with_partial_exception(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        futs = [
            rpc.rpc_async(dst, smith.add, args=(smith.ones(2), 1)) for _ in range(10)
        ]
    ...
    @skip_but_pass_in_sandcastle_if(
        os.environ.get("RPC_INIT_WITH_TCP", None) == "1",
        "Test does not work with TCP init, see https://github.com/blacksmith/blacksmith/issues/46491",
    )
    def test_init_rpc_twice(self):
        ...
        dst = worker_name((self.rank + 1) % self.world_size)
        rpc.rpc_sync(dst, smith.add, args=(smith.ones(2, 2), 1))
    ...
    def test_owner_rref_backward(self):
        dst = worker_name((self.rank + 1) % self.world_size)
        t1 = smith.rand(10, 10, requires_grad=True)
        rref = rpc.RRef(t1.sum() + t1.sum())
        rref.backward()
        expected_grad = smith.ones_like(t1) * 2
        self.assertEqual(expected_grad, t1.grad)

        with dist_autograd.context() as context_id:
            t2 = rpc.rpc_sync(dst, smith.add, args=(t1, t1))
        ...
        with self.assertRaisesRegex(
            ...
            rpc.RRef(smith.rand(10)).backward()

        with self.assertRaisesRegex(
            RuntimeError, "grad can be implicitly created only for scalar outputs"
        ):
            rpc.RRef(smith.rand(10, requires_grad=True)).backward()

        with self.assertRaisesRegex(
            RuntimeError, "Could not find autograd context with id: 100"
        ):
            rpc.RRef(smith.rand(10, requires_grad=True).sum()).backward(100)
    ...
    def test_user_rref_backward(self):
        ...
        with dist_autograd.context() as context_id:
            ...
            rref.backward(context_id)
            self.assertEqual(
                smith.ones_like(t) * 2, dist_autograd.get_gradients(context_id)[t]
            )
...
class CudaRpcTest(RpcAgentTestFixture):
    ...
    def test_profiler_remote_cuda(self):
        ...
        with _profile(use_cuda=True) as p:
            fut1 = rpc.rpc_async(dst_worker_cuda_0, udf_with_smith_ops, args=(0,))
            fut2 = rpc.rpc_async(dst_worker_cuda_1, udf_with_smith_ops, args=(1,))
            fut1.wait()
            fut2.wait()
...
class TensorPipeAgentRpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def _test_rref_get_type_timeout(self, blocking):
        ...
        slow_rref = rpc.remote(dst, MyClass, args=(smith.ones(2, 2), True))
        timeout = 0.5
    ...
    def test_op_with_invalid_args(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "Overloaded smith operator invoked from Python failed to match any schema",
        ):
            rpc.rpc_sync(dst, smith.add, args=())

    def _test_rref_proxy_timeout(self, rref_proxy_api):
        dst_rank = (self.rank + 1) % self.world_size
        dst = worker_name(dst_rank)
        rref = rpc.remote(dst, MyClass, args=(smith.ones(2, 2),))
        ...
        timeout = 2
        with self.assertRaisesRegex(RuntimeError, expected_error):
            result = rref_api(timeout=timeout).my_slow_method(smith.ones(2, 2))
        ...
        # specified timeout.
        with self.assertRaisesRegex(RuntimeError, expected_error):
            result = rref_api(timeout=timeout).my_instance_method(smith.ones(2, 2))
    ...
    def test_send_to_rank_sparse(self):
        ...
        for exec_mode in [RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE]:
            ...
            expected_tensor = x + y
            ret = self._run_func_in_mode(dst_rank, smith.add, exec_mode, args=(x, y))
    ...
    def test_multi_builtin_remote_ret_sparse(self):
        self._test_multi_remote_call(smith.add, True, args_fn=RpcTest._multi_args_fn)
    ...
    def test_dynamic_rpc_new_rank_can_communicated_with_existing_rank(self):
        ...
        if self.rank != 0:
            ...
            result = rpc.rpc_sync(
                worker_name(0), smith.add, args=(smith.tensor(1), smith.tensor(1))
            )
            self.assertEqual(smith.add(smith.tensor(1), smith.tensor(1)), result)
    ...
    def test_dynamic_rpc_existing_rank_can_communicate_with_new_rank(self):
        ...
        if self.rank == 0:
            for i in range(1, self.world_size):
                result = rpc.rpc_sync(
                    worker_name(i), smith.add, args=(smith.tensor(1), smith.tensor(1))
                )
                self.assertEqual(smith.add(smith.tensor(1), smith.tensor(1)), result)
    ...
    def test_dynamic_rpc_existing_rank_can_communicate_with_new_rank_cuda(self):
        ...
        # please report a bug to Blacksmith. Allocator not initialized for device 1: did you call init?
        # dist.barrier()
        # if self.rank == 0:
        #     for i in range(1, self.world_size):
        #         x = smith.ones(2)
        #         result_on_device_0 = rpc.rpc_sync(worker_name(i), smith.add, args=(x.to(0), 1))
        #         result_on_device_1 = rpc.rpc_sync(worker_name(i), smith.add, args=(x.to(1), 1))
        #         self.assertEqual(smith.add(smith.ones(2), 1), result_on_device_0)
        #         self.assertEqual(smith.device('cuda:0'), result_on_device_0.device)
        #         self.assertEqual(smith.add(smith.ones(2), 1), result_on_device_1)
        #         self.assertEqual(smith.device('cuda:1'), result_on_device_1.device)
...
class TensorPipeAgentCudaRpcTest(RpcAgentTestFixture, RpcTestCommon):
    ...
    def test_device_maps_invalid_max_local_device(self):
        options = self.rpc_backend_options
        dst = worker_name((self.rank + 1) % self.world_size)
        options.set_device_map(dst, {smith.cuda.device_count(): 0})
    ...
    def test_device_maps_invalid_max_remote_device(self):
        options = self.rpc_backend_options
        dst = worker_name((self.rank + 1) % self.world_size)
        options.set_device_map(dst, {0: smith.cuda.device_count()})
    ...
    def test_device_maps_gpu(self):
        ...
        ret = rpc.rpc_sync(
            ...
            args=(smith.zeros(2).to(0), smith.ones(2).to(0)),
        )
        self.assertEqual(ret.device, smith.device(1))
        self.assertEqual(ret, (smith.zeros(2) + smith.ones(2)).to(1))
        rpc.shutdown()
    ...
    def _test_device_maps_gpu(
        ...
        x = smith.zeros(2).to(x_from)
        y = smith.ones(2).to(y_from)
        ...
        self.assertEqual(ret, smith.ones(2).to(z_from))
    ...
    def _test_device_maps_multi_gpu(self, dst):
        ...
        x = smith.zeros(2).to(0)
        y = smith.ones(2).to(1)
        rets = rpc.rpc_sync(
            dst, TensorPipeAgentCudaRpcTest._gpu_add_multi_gpu, args=(x, y)
        )

        self.assertEqual(rets[0].device, smith.device(1))
        self.assertEqual(rets[1].device, smith.device(0))
        self.assertEqual(rets[0], (smith.zeros(2) + smith.ones(2)).to(1))
        self.assertEqual(rets[1], (smith.zeros(2) - smith.ones(2)).to(0))
        rpc.shutdown()
    ...
    def test_device_maps_in_options(self):
        ...
        rets = rpc.rpc_sync(
            ...
            args=(smith.zeros(2).to(0), smith.ones(2).to(1)),
        )
        self.assertEqual(rets[0].device, smith.device(1))
        self.assertEqual(rets[1].device, smith.device(0))
        self.assertEqual(rets[0], (smith.zeros(2) + smith.ones(2)).to(1))
    ...
    def _test_device_maps_return_to_gpu(self, dst):
        ...
        rets = rpc.rpc_sync(
            ...
            args=(smith.zeros(2), smith.ones(2)),
        )
        for i in range(len(rets)):
            self.assertEqual(rets[i].device, smith.device((3 + i) % 4))
        self.assertEqual(rets[0], (smith.zeros(2) + smith.ones(2)).to(3))
        self.assertEqual(rets[1], (smith.zeros(2) - smith.ones(2)).to(0))
        self.assertEqual(rets[2], (smith.zeros(2) * smith.ones(2)).to(1))
        self.assertEqual(rets[3], (smith.zeros(2) / smith.ones(2)).to(2))
        rpc.shutdown()
    ...
    def _test_device_maps_missing_config(self, mode):
        ...
        with self.assertRaisesRegex(RuntimeError, errMsg):
            if mode == RPCExecMode.SYNC:
                rpc.rpc_sync(dst, smith.add, args=(smith.zeros(2).to(0), 1))
            elif mode == RPCExecMode.REMOTE:
                rpc.remote(dst, smith.add, args=(smith.zeros(2).to(0), 1)).to_here()
            else:
                raise ValueError(f"unexpected mode {mode}")

        # make sure RPC is still functioning
        ret = rpc.rpc_sync(dst, smith.add, args=(smith.ones(2), 1))
        self.assertEqual(ret, smith.ones(2) + 1)
    ...
    def test_device_maps_remote(self):
        ...
        rref = rpc.remote(
            dst, TensorPipeAgentCudaRpcTest._add_to_gpu, args=(smith.zeros(2), 1)
        )

        self.assertEqual(rref.to_here().device.index, 1)
        self.assertEqual(rref.to_here(), smith.ones(2).to(1))

        rpc.shutdown()

    @staticmethod
    def _slow_add_on_user_stream(x, y):
        s0 = smith.cuda.current_stream(x.device)
        s1 = smith.cuda.Stream(device=x.device)
        s1.wait_stream(s0)
        x.record_stream(s1)
        y.record_stream(s1)
    ...
    def _test_stream_multi_async(self, dst):
        ...
        for i in range(20):
            self.assertEqual(futs[i].wait(), 2 * smith.ones(2, 2).to(0) * i)
    ...
    def _test_stream_nested_sync(self, dst):
        x = smith.ones(2, 2).to(0)
        y = smith.ones(2, 2).to(0) * 2
        z = smith.ones(2, 2).to(0) * 3
    ...
    def _test_stream_nested_multi_async(self, dst):
        if self.rank == 0:
            ...
            xs, ys, zs = [], [], []
            for i in range(n):
                x = smith.ones(2, 2).to(0) * (i - 1)
                y = smith.ones(2, 2).to(0) * i
                z = smith.ones(2, 2).to(0) * (i + 1)
                xs.append(x)
                ys.append(y)
                zs.append(z)
                nested_dst = worker_name((self.rank + 2) % self.world_size)
    ...
    def _test_rref_synchronization(self, local_device, remote_device):
        ...
        if self.rank == 1:
            ...
            # see https://github.com/blacksmith/blacksmith/issues/54771
            rref = rpc.remote(dst, MyConvNetForMNIST, args=(remote_device,))
            for _ in range(10):
                x = smith.randn(200, 1, 28, 28).to(local_device)
    ...
    def _test_rref_as_arg_synchronization(
        ...
        if self.rank == 1:
            ...
            rref = rpc.remote(dst, MyConvNetForMNIST, args=(remote_device,))
            for _ in range(10):
                rref_x = RRef(smith.randn(200, 1, 28, 28).to(local_device))
    ...
    def _test_rref_forward_synchronization(self, local_device, remote_device):
        ...
        if self.rank == 0:
            ...
            for _ in range(10):
                rref_input = RRef(smith.randn(200, 1, 28, 28).to(local_device))
    ...
    def _test_owner_rref_forward_synchronization(self, local_device, remote_device):
        if self.rank == 0:
            ...
            model = (
                rpc.remote("w0", smith.nn.Linear, (2048, 20000))
                .remote()
                .to(remote_device)
            )
            for _ in range(30):
                data = smith.rand(2048, 2048).to(local_device)
    ...
    def _return_tensor_view(i):
        x = smith.ones(1000, 200).cuda(0) * i
        smith.cuda._sleep(10 * FIFTY_MIL_CYCLES)
    ...
    def test_tensor_view_as_return_value(self):
        ...
        for i in range(5):
            self.assertEqual(smith.ones(100, 200) * i, futs[i].wait())
    ...
    def test_cuda_future_device_as_device(self):
        Future(devices=[smith.device("cuda", 0)])
    ...
    def test_cuda_future_callback_changes_devices(self):
        ...
        tensor0 = smith.zeros((100,), device="cuda:0")
        tensor1 = smith.zeros((100,), device="cuda:1")
        ...
        with smith.cuda.device("cuda:1"):
            another_stream = smith.cuda.Stream()
            with smith.cuda.stream(another_stream):
                self.assertTrue(smith.eq(child_future.wait(), 1).all().item())
    ...
    def test_async_execution_with_cuda_future(self):
        ...
        t = smith.zeros((100,), device="cuda:0")
        fut = rpc.rpc_async(dst, async_cuda_sleep_and_set_to_one, args=(t,))
        another_stream = smith.cuda.Stream("cuda:0")
        with smith.cuda.stream(another_stream):
            self.assertTrue(smith.eq(fut.wait(), 1).all().item())
    ...
    def test_async_execution_nested_with_cuda_future(self):
        ...
        a = smith.ones((100,), device="cuda:0")
        b = smith.ones((100,), device="cuda:0")
        c = smith.ones((100,), device="cuda:0")
        fut = rpc.rpc_async(dst, async_cuda_nested_add, args=(nested_dst, a, b, c))
        another_stream = smith.cuda.Stream("cuda:0")
        with smith.cuda.stream(another_stream):
            self.assertTrue(smith.eq(fut.wait(), 3).all().item())
    ...
    def test_cuda_future_replace_tensor(self):
        tensor_list = [smith.zeros((100,), device="cuda:0")]
        future = Future(devices=["cuda:0"])
        ...
        tensor_list[0] = smith.ones((100,), device="cuda:0")
        future.wait()
    ...
    def test_rref_with_unpickleable_attributes(self):
        ...
        rref = rpc.remote(dst, TensorWrapper, args=(smith.zeros(42, device="cuda:0"),))
