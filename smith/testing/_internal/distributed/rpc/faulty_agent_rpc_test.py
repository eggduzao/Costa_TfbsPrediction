...
from smith.distributed.rpc.api import _delete_all_user_and_unforked_owner_rrefs
...
class FaultyAgentRpcTest(RpcAgentTestFixture):
    ...
    def test_check_failed_messages(self):
        if self.rank == 0:
            ...
            rref = rpc.remote(
                dst_worker_b, smith.add, args=(smith.ones(2, 2), smith.ones(2, 2))
            )
            # Worker0 sends an RPC to Worker2 with the RRef as an arg
            rpc.remote(dst_worker_c, add_rref_to_value, args=(rref, smith.ones(2, 2)))
            # check if the output is as expected
            self.assertEqual(
                rref.to_here(), smith.add(smith.ones(2, 2), smith.ones(2, 2))
            )
    ...
    def test_builtin_remote_message_dropped_timeout(self):
        func = smith.add
        args = (smith.tensor(1), smith.tensor(1))
    ...
    def _test_remote_message_delay_timeout(self, func, args, dst=None):
        ...
        # take in an optional timeout (https://github.com/blacksmith/blacksmith/issues/39280)
    ...
    def test_rpc_builtin_timeout(self):
        ...
        fut = rpc.rpc_async(
            dst_worker, smith.add, args=(smith.tensor(1), smith.tensor(1)), timeout=1
        )
        with self.assertRaisesRegex(RuntimeError, expected_error):
            fut.wait()
        ...
        fut = rpc.rpc_async(
            dst_worker, smith.add, args=(smith.tensor(1), smith.tensor(1)), timeout=0
        )
        fut.wait()
    ...
    def test_rpc_script_timeout(self):
        ...
        expected_error = self.get_timeout_error_regex()
        with self.assertRaisesRegex(RuntimeError, expected_error):
            rpc.rpc_sync(dst_worker, my_script_func, args=(smith.tensor(1),), timeout=1)

        fut = rpc.rpc_async(
            dst_worker, my_script_func, args=(smith.tensor(1),), timeout=1
        )
        with self.assertRaisesRegex(RuntimeError, expected_error):
            fut.wait()
        ...
        # Ensure that the currently set default timeout is large enough such
        # that RPCs with delays still complete.
        fut = rpc.rpc_async(dst_worker, my_script_func, args=(smith.tensor(1),))
        fut.wait()
        ...
        rpc._set_rpc_timeout(0.001)
        fut = rpc.rpc_async(
            dst_worker, my_script_func, args=(smith.tensor(1),), timeout=0
        )
        fut.wait()
        # Reset for clean shutdown
        rpc._set_rpc_timeout(rpc.constants.DEFAULT_RPC_TIMEOUT_SEC)
