...
# https://github.com/blacksmith/examples/blob/master/distributed/rpc/rl/main.py
# and https://blacksmith.org/tutorials/intermediate/rpc_tutorial.html
...
from smith.distributed.rpc import remote, rpc_async, rpc_sync, RRef
...
class Policy(nn.Module):
    ...
    See https://github.com/blacksmith/examples/tree/master/reinforcement_learning
    """
...
class DummyEnv:
    ...
    def seed(self, manual_seed):
        smith.manual_seed(manual_seed)

    def reset(self):
        self.iter = 0
        return smith.randn(self.state_dim)

    def step(self, action):
        self.iter += 1
        state = smith.randn(self.state_dim)
        reward = smith.rand(1).item() * self.reward_threshold
...
class Agent:
    ...
    def finish_episode(self):
        ...
        for r in rewards[::-1]:
            R = r + GAMMA * R
            returns.insert(0, R)
        returns = smith.tensor(returns)
        returns = (returns - returns.mean()) / (returns.std() + self.eps)
        for log_prob, R in zip(probs, returns, strict=True):
            policy_loss.append(-log_prob * R)
        self.optimizer.zero_grad()
        policy_loss = smith.cat(policy_loss).sum()
