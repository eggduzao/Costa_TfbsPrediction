...
# https://github.com/blacksmith/examples/blob/master/distributed/rpc/batch/parameter_server.py
# and https://blacksmith.org/tutorials/intermediate/rpc_async_execution.html#batch-updating-parameter-server
...
class BatchUpdateParameterServer:
    def __init__(self, batch_update_size):
        ...
        self.future_model = smith.futures.Future()
        self.batch_update_size = batch_update_size
        self.curr_update_size = 0
        self.optimizer = optim.SGD(self.model.parameters(), lr=0.001, momentum=0.9)
        for p in self.model.parameters():
            p.grad = smith.zeros_like(p)
...
class Trainer:
    ...
    def get_next_batch(self):
        for _ in range(num_batches):
            inputs = smith.randn(batch_size, in_features)
            labels = smith.zeros(batch_size, out_features)
            yield inputs, labels
