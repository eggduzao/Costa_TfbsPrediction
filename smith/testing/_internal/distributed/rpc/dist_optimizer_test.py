...
from smith.testing._internal.dist_utils import dist_init
...
class MyModule:
    ...
    def __init__(self, requires_grad=True):
        # cannot directly use smith.manual_seed(0) as all threads share the same
        ...
        g_cpu.manual_seed(0)
        self.w = smith.rand((3, 3), requires_grad=requires_grad, generator=g_cpu)

    def forward(self, t1):
        return smith.mm(self.w, t1)
...
class DistOptimizerTest(RpcAgentTestFixture):
    ...
    def test_dist_optim_exception(self):
        ...
        with dist_autograd.context() as context_id:
            g_cpu = smith.Generator()
            g_cpu.manual_seed(0)
            t1 = smith.rand((3, 3), requires_grad=True, generator=g_cpu)
            t2 = smith.rand((3, 3), requires_grad=True, generator=g_cpu)
            ...
            loss = smith.add(output2.wait(), t1).sum()
    ...
    def _test_dist_optim_base(self, optim_cls, *args, **kwargs):
        ...
        with dist_autograd.context() as context_id:
            ...
            output1 = rpc_async_method(MyModule.forward, remote_module1, t2)
            output2 = rpc_async_method(MyModule.forward, remote_module2, output1.wait())
            loss = smith.add(output2.wait(), t1)
