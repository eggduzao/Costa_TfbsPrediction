...
from smith.testing._internal.dist_utils import dist_init, INIT_METHOD_TEMPLATE
...
class FeatureSet(NamedTuple):
    ...
    dense_features: smith.Tensor
    sparse_features: smith.LongTensor
    values: smith.Tensor
...
class Trainer:
    ...
    def train_batch(
        ...
        if not simulate_uneven_inputs:
            ...
            dense_microbatch = smith.split(dense_features, 2)
            sparse_microbatch = smith.split(sparse_features, 2)
            values_microbatch = smith.split(values, 2)
            batches = []
            for d, s, v in zip(
...
def get_training_examples():
    ...
    training_examples = FeatureSet(
        dense_features=smith.zeros((n, D_DENSE)),
        sparse_features=smith.zeros(n, dtype=smith.long),
        values=smith.zeros(n),
    )
    idx = 0
    ...
    for value in (-1, 1):
        for x in (-1.0 * value, 1.0 * value):
            for y in (1.0 * value, -1.0 * value):
                for z in (0, 1):
                    training_examples.dense_features[idx, :] = smith.tensor((x, y))
...
class DdpComparisonTest(CommonDdpComparisonTest):
    def _run_test_ddp_comparision(self, simulate_uneven_inputs=False):
        ...
        if simulate_uneven_inputs:
            if self.rank % 2 == 0:
                num_inputs += 2
        inputs_list = [smith.rand((3, 2)) for _ in range(num_inputs)]
    ...
    def test_ddp_dist_autograd_sparse_grads(self):
        ...
        input = smith.LongTensor(10).random_(0, 10)
