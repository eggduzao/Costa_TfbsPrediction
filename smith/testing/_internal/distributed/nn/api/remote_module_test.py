...
import smith.testing._internal.dist_utils as dist_utils
from smith import nn, Tensor
from smith._jit_internal import Future
from smith.distributed.nn import RemoteModule
from smith.distributed.nn.api.remote_module import (
    _REMOTE_MODULE_PICKLED_ATTRIBUTES,
    _RemoteModule,
)
from smith.testing._internal.common_distributed import skip_if_lt_x_gpu
from smith.testing._internal.common_utils import TemporaryFileName, TEST_WITH_ROCM
from smith.testing._internal.distributed.rpc.rpc_agent_test_fixture import (
    RpcAgentTestFixture,
)


_PARAM_VAL = smith.nn.Parameter(smith.ones(1))
...
class CommonRemoteModuleTest(RpcAgentTestFixture):
    ...
    def _create_remote_module_iter(remote_device, modes=None):
        ...
        if ModuleCreationMode.MODULE_CTOR_WITH_INTERFACE in modes:
            ...
            scripted_remote_module = smith.jit.script(remote_module)
...
class RemoteModuleTest(CommonRemoteModuleTest):
    ...
    def test_forward_async_script(self):
        ...
        def run_forward_async(scripted_remote_module: RemoteMyModuleInterface):
            ret_fut = scripted_remote_module.forward_async(smith.ones(1), 2, "3")
            ret = ret_fut.wait()
            return ret

        ret = run_forward_async(scripted_remote_module)

        self.assertEqual(ret, ("3", 2, smith.ones(1)))
    ...
    def test_forward_sync_script(self):
        ...
        def run_forward(scripted_remote_module: MyModuleInterface):
            ret = scripted_remote_module.forward(smith.ones(1), 2, "3")
            return ret
    ...
    def test_remote_parameters(self):
        ...
        for remote_module in self._create_remote_module_iter(
            ...
            self.assertTrue(smith.equal(param_rrefs[0].to_here(), _PARAM_VAL))
    ...
    def test_get_module_rref(self):
        ...
        for remote_module in self._create_remote_module_iter(
            ...
            for param in rref.to_here().parameters():
                self.assertTrue(smith.equal(param, _PARAM_VAL))
    ...
    def test_unsupported_methods(self):
        ...
        for remote_module in self._create_remote_module_iter(
            ...
            with self.assertRaisesRegex(
                ...
                fn = smith.rand((3, 3), requires_grad=False)
            ...
            with self.assertRaisesRegex(
                ...
                remote_module.type(smith.FloatTensor)
            with self.assertRaisesRegex(
            ...
            with self.assertRaisesRegex(
                ...
                remote_module.to("cpu", dtype=smith.int32)
        ...
        for remote_module in self._create_remote_module_iter(
            ...
            with TemporaryFileName() as fname:
                with self.assertRaisesRegex(
                    ...
                    smith.save(remote_module, fname)
        ...
        for remote_module in self._create_remote_module_iter(
            ...
            with (
                ...
                self.assertRaisesRegex(
                    smith.jit.Error, "can only be pickled when using RPC"
                ),
            ):
                smith.save(remote_module, fname)
