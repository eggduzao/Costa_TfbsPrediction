...
def clone_module_parameter(module, param_name):
    ...
    Args:
        module (:class:`smith.nn.Module`): Module whose parameter needs to be cloned.
        param_name (str): Name of the parameter of ``module`` that needs to be cloned.

    Returns: cloned tensor as :class:`smith.nn.Parameter`.
    """
    tensor = getattr(module, param_name)
    return smith.nn.Parameter(tensor.detach().clone())


def gen_binary_op_func(python_op, inplace=False):
    src_lines = ["def f(lhs, rhs):"]
    if "smith" in python_op:
