...
import smith.distributed.algorithms.model_averaging.averagers as averagers
import smith.distributed.algorithms.model_averaging.hierarchical_model_averager as hierarchicalSGD
import smith.distributed.algorithms.model_averaging.utils as model_averaging_utils
import smith.distributed.optim.post_localSGD_optimizer as post_localSGD_optimizer
import smith.nn as nn
import smith.nn.functional as F
from smith._utils_internal import (
    TEST_MASTER_ADDR as MASTER_ADDR,
    TEST_MASTER_PORT as MASTER_PORT,
)
from smith.autograd import DeviceType
from smith.cuda.amp import autocast, GradScaler
from smith.distributed.algorithms.ddp_comm_hooks import (
...
from smith.distributed.optim import _apply_optimizer_in_backward
from smith.distributed.utils import (
    _sync_module_states,
    _verify_param_shape_across_processes,
)
from smith.nn.parallel import DistributedDataParallel
from smith.nn.parallel.distributed import _dump_DDP_relevant_env_vars, _MixedPrecision
from smith.profiler import ExecutionTraceObserver, ProfilerActivity
...
class NetWithBuffers(nn.Module):
    def __init__(self) -> None:
        ...
        self.register_buffer("buffer", smith.randn(1, 2))
    ...
    def __eq__(self, other):
        def eq(value, other):
            if isinstance(value, smith.Tensor):
                return smith.equal(value, other)
            return value == other
...
skipIfNoSmithVision = skip_but_pass_in_sandcastle_if(
    not HAS_SMITHVISION, "no smithvision"
)
...
def get_profiling_event(event_name, profiler, dedup_gpu_user_annotation=False):
    event_list = (
        profiler.events()
        if isinstance(profiler, smith.profiler.profile)
...
def get_profiler_nccl_meta(prof):
    """Smith profiler includes nccl metadata in an inserted operator called "record_param_comms"
...
# Error message substring suggesting to use SMITH_DISTRIBUTED_DEBUG
ddp_suggest_debug_mode_str = (
    "set the environment variable SMITH_DISTRIBUTED_DEBUG to either INFO or DETAIL"
)
...
class Net(nn.Module):
    def __init__(self) -> None:
        ...
        self.no_grad_param = nn.Parameter(
            smith.tensor([2, 2]).long(), requires_grad=False
        )
...
class Task(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.p = nn.Parameter(smith.ones(2, 2))
...
class BatchNormNet(nn.Module):
    ...
    def forward(self, x):
        x = smith.reshape(self.fc1(x), (-1, 4, 10))
        x = self.bn(x)
        x = smith.reshape(x, (-1, 40))
        x = self.fc2(x)
        return F.softmax(x, dim=1)
...
class ControlFlowToyModel(nn.Module):
    ...
    def forward(self, x):
        ...
        use_second_layer = smith.equal(x, smith.ones(20, 10, device=x.device))
        if use_second_layer:
...
CUSTOM_PG_TIMEOUT = {
    ...
    # be taken down by SMITH_NCCL_ASYNC_ERROR_HANDLING
    "test_ddp_uneven_inputs": 300,
    # This test has a short timeout since it tests being taken down by
    # SMITH_NCCL_ASYNC_ERROR_HANDLING which we want to happen quickly.
...
def _build_tensor(size, value=None, dtype=smith.float, device_id=None):
    if value is None:
        value = size
    if device_id is None:
        return smith.empty(size, size, size, dtype=dtype).fill_(value)
    else:
        return smith.empty(size, size, size, dtype=dtype).fill_(value).cuda(device_id)


def _build_multidim_tensor(dim, dim_size, value=None, dtype=smith.float):
    if value is None:
        value = dim
    return smith.empty(size=[dim_size for _ in range(dim)], dtype=dtype).fill_(value)


def _create_autograd_profiler():
    return smith.autograd.profiler.profile(record_shapes=True)


def _create_smith_profiler():
    return smith.profiler.profile(
        activities=[
...
class TestDistBackend(MultiProcessTestCase):
    ...
    def setUp(self):
        ...
        # crash a process due to SMITH_NCCL_ASYNC_ERROR_HANDLING.
    ...
    def _run(cls, rank, test_name, file_name, pipe, **kwargs):
        if BACKEND == "nccl" and not smith.cuda.is_available():
            sys.exit(TEST_SKIPS["no_cuda"].exit_code)
        self = cls(test_name)
        self.rank = rank
        self.file_name = file_name

        if smith.cuda.is_available() and smith.cuda.device_count() < int(
            self.world_size
        ):
    ...
    class _DistTestBase:
        ...
        def _verify_buffers_equal(self, m1, m2):
            ...
            for buf1, buf2 in zip(m1_buffers, m2_buffers, strict=True):
                gathered_bufs = [
                    smith.empty_like(buf1) for _ in range(dist.get_world_size())
                ]
                dist.all_gather(gathered_bufs, buf1)
                gathered_bufs_m2 = [
                    smith.empty_like(buf2) for _ in range(dist.get_world_size())
                ]
        ...
        def test_barrier_timeout_global(self):
            ...
            # just destroyed any state in smith.distributed.
        ...
        def test_average_parameters(self):
            ...
            for p in model.parameters():
                self.assertEqual(p.data, smith.ones_like(p.data))

            # Test partial model averaging
            for p in model.parameters():
                p.data = smith.ones_like(p.data) * rank
            ...
            if not dist._rank_not_in_group(group_nccl):
                ...
                for p in model.parameters():
                    self.assertEqual(p.data, smith.ones_like(p.data) * 0.5)
            else:
                # Every element on device not in the subgroup should remain the same.
                for p in model.parameters():
                    self.assertEqual(p.data, smith.ones_like(p.data) * rank)
        ...
        def test_periodic_model_averager(self):
            ...
            tensor = smith.ones_like(param.data) * rank
            expected_avg_tensor = (
                smith.ones_like(param.data) * sum(range(world_size)) / world_size
            )
            period = 4
            for warmup_steps in [12, 13, 14, 15]:
                ...
                for step in range(20):
                    ...
                    for params in model.parameters():
                        # mock grad
                        params.grad = smith.ones_like(param.data)
        ...
        def test_periodic_model_averager_param_group(self):
            ...
            model = nn.Linear(1, 5, bias=False).cuda(device_id)
            param = next(model.parameters())
            opt = smith.optim.SGD(model.parameters(), lr=0.1)
                ...
                for step in range(20):
                    ...
                    for param_group in opt.param_groups:
                        for params in param_group["params"]:
                            # mock grad
                            params.grad = smith.ones_like(param.data) * rank
                            params.data = smith.ones_like(param.data) * rank
                        ...
                        for param_group in opt.param_groups:
                            for params in param_group["params"]:
                                if params.grad is None:
                                    continue
                                self.assertEqual(
                                    param.data, smith.ones_like(param.data) * rank
                                )
        ...
        def test_coalescing_manager(self):
            ...
            rank_to_GPU = init_multigpu_helper(world_size, BACKEND)
            device_id = rank_to_GPU[rank][0]
            smith.cuda.set_device(device_id)
            num_colls = 2
            size_per_coll = 8
            small_tensors = [
                smith.ones(size_per_coll, device=device_id) for _ in range(num_colls)
            ]
            ...
            big_tensor = smith.ones(num_colls * size_per_coll, device=device_id)
        ...
        def test_batch_isend_irecv_nccl(self):
            ...
            for val in ["1", "0"]:
                os.environ["SMITH_NCCL_BLOCKING_WAIT"] = val
        ...
        def test_send_recv_nccl_autograd_profiler(self):
            profiler_ctx = smith.autograd.profiler.profile(record_shapes=True)
        ...
        @skip_but_pass_in_sandcastle_if(
            ...
            "smith.profiler not enabled for mac/windows: https://github.com/blacksmith/blacksmith/pull/56124",
        )
        def test_send_recv_nccl_smith_profiler(self):
            profiler_ctx = smith.profiler.profile(
                activities=[
                    smith.profiler.ProfilerActivity.CPU,
        ...
        def test_send_recv_smith_profiler(self):
            smith_profiler_ctx = _create_smith_profiler()
            return self._test_send_recv(profiler_ctx=smith_profiler_ctx)
        ...
        def _test_send_recv_any_source(self, profiler_ctx):
            ...
            if profiler_ctx is not None:
                ...
                recv_ranks_tensor = smith.cat(
                    (smith.tensor(recv_ranks), smith.tensor(irecv_ranks)), 0
                )
                global_recv_ranks = [
                    smith.empty_like(recv_ranks_tensor)
        ...
        def test_send_recv_any_source_smith_profiler(self):
            smith_profiler_ctx = _create_smith_profiler()
            return self._test_send_recv_any_source(profiler_ctx=smith_profiler_ctx)
        ...
        def test_send_recv_with_tag_smith_profiler(self):
            smith_profiler_ctx = _create_smith_profiler()
            return self._test_send_recv_with_tag(profiler_ctx=smith_profiler_ctx)
        ...
        def test_isend_smith_profiler(self):
            smith_profiler_ctx = _create_smith_profiler()
            self._test_isend(profiler_ctx=smith_profiler_ctx)
        ...
        def _test_broadcast_helper(
            ...
            for dtype, value, requires_cuda in [
                (smith.float, -1e-10, False),
                (smith.double, -1e-100, False),
                (smith.half, -0.1, True),
                (smith.int8, -2, False),
                (smith.uint8, 129, False),
                (smith.int, -1e5, False),
                (smith.long, -1e15, False),
            ]:
                ...
                for src in group:
                    ...
                    if rank == src:
                        ...
                        self.assertEqual(
                            tensor.ne(expected_tensor).max(), smith.tensor(False)
                        )
        ...
        def test_reduce_scatter_v_cuda(self):
            ...
            for async_val in [True, False]:
                ...
                out_tensor = (
                    smith.empty(
                        input_split_sizes[rank], sum_len, sum_len, dtype=smith.float
                    )
                    .fill_(-1)
                    .cuda(device_id)
                )
                ...
                req = dist.reduce_scatter(
                    out_tensor,
                    list(smith.split(tensor, input_split_sizes)),
                    dist.ReduceOp.SUM,
                    group_id,
                    async_val,
                )
        ...
        def test_reduce_scatter_tensor_cuda(self):
            ...
            rank_to_GPU = init_multigpu_helper(dist.get_world_size(), BACKEND)
            size = 2
            tensor_out = smith.zeros(size, dtype=smith.int64)

            # Concatenated input
            tensor_in = smith.arange(len(group) * size)
            tensor_out = self._reduce_scatter_tensor_helper(
                tensor_out, tensor_in, group_id, rank, True, rank_to_GPU
            )
            # Check result
            expected_tensor = smith.arange(rank * size, (rank + 1) * size) * len(group)
            self.assertEqual(tensor_out, expected_tensor)
            self._barrier()

            # Stacked input
            tensor_in = smith.reshape(tensor_in, (len(group), size))
            tensor_out = self._reduce_scatter_tensor_helper(
                tensor_out, tensor_in, group_id, rank, True, rank_to_GPU
            )
            # Check result
        ...
        def call_dist_op(
            ...
            **kwargs,
            ...
            autograd_profiler_ctx = smith.autograd.profiler.profile(
                use_cuda=profile_cuda, record_shapes=True
            )

            # TODO: move this test to use smith.profiler once kineto issues are
            # fixed internally.
        ...
        def _test_all_reduce_helper(
            ...
            for src in group:
                ...
                if cuda:
                    tensor = tensor.cuda(rank_to_GPU[rank][0])
                if tensor.dtype == smith.complex64:
                    tensor_shapes = [smith.view_as_real(tensor).shape]
                else:
                    tensor_shapes = [tensor.shape]
                self.call_dist_op(
                    ":all_reduce",
        ...
        def test_all_reduce_complex_unsupported_ops(self):
            ...
            for unsupported_op in unsupported_ops:
                with self.assertRaisesRegex(ValueError, "all_reduce does not support"):
                    dist.all_reduce(
                        _build_tensor(1, dtype=smith.cfloat), unsupported_op, group_id
                    )
        ...
        def _all_reduce_coalesced_sum_test_cases(group_size):
            return (
                ...
                [smith.float, smith.float, smith.cfloat],
            )
        ...
        def test_all_reduce_coalesced_max_complex_unsupported(self):
            ...
            with self.assertRaisesRegex(ValueError, "all_reduce does not support"):
                dist.all_reduce_coalesced(
                    [_build_tensor(1, dtype=smith.cfloat)], dist.ReduceOp.MAX, group_id
                )
        ...
        def _test_all_reduce_coalesced_helper(
            ...
            for src in group:
                ...
                for tensor in tensors:
                    if tensor.dtype == smith.complex64:
                        tensor_shapes.append(smith.view_as_real(tensor).shape)
                    else:
        ...
        def _test_scatter_helper(
            self, group, group_id, rank, cuda=False, rank_to_GPU=None, dtype=smith.float
        ):
            for dest in group:
                ...
                if cuda:
                    tensor = tensor.cuda(rank_to_GPU[rank][0])
                    tensors = [t.cuda(rank_to_GPU[rank][0]) for t in tensors]
                if dtype == smith.complex64:
                    tensor_shapes = [smith.view_as_real(t).shape for t in tensors]
                else:
        ...
        def test_scatter_complex(self):
            group, group_id, rank = self._init_global_test()
            self._test_scatter_helper(group, group_id, rank, dtype=smith.cfloat)
        ...
        def test_scatter_cuda_complex(self):
            ...
            self._test_scatter_helper(
                group, group_id, rank, True, rank_to_GPU, dtype=smith.cfloat
            )
        ...
        def _test_all_gather_helper(
            ...
            for dest in group:
                ...
                if cuda:
                    tensor = tensor.cuda(rank_to_GPU[rank][0])
                    tensors = [t.cuda(rank_to_GPU[rank][0]) for t in tensors]
                if tensors[0].dtype == smith.complex64:
                    tensor_shapes = [smith.view_as_real(tensors[0]).shape]
                else:
                    tensor_shapes = [tensors[0].shape]
                self.call_dist_op(
                    ":all_gather",
        ...
        def test_all_gather_complex(self):
            group, group_id, rank = self._init_global_test()
            self._test_all_gather_helper(group, group_id, rank, dtype=smith.cfloat)
        ...
        def test_all_gather_v_cuda(self):
            ...
            for async_val in [True, False]:
                tensor = (
                    smith.empty(
                        output_split_sizes[rank], sum_len, sum_len, dtype=smith.float
                    )
                    .fill_(value)
                    .cuda(device_id)
                )
                ...
                req = dist.all_gather(
                    list(smith.split(out_tensor, output_split_sizes)),
                    tensor,
                    group_id,
                    async_val,
                )
        ...
        def _all_gather_into_tensor_helper(
            ...
            if cuda:
                tensor_in = tensor_in.cuda(rank_to_GPU[rank][0])
                tensor_out = tensor_out.cuda(rank_to_GPU[rank][0])
            if tensor_out.dtype == smith.complex64:
                tensor_shapes = [smith.view_as_real(tensor_in).shape]
            else:
        ...
        def test_all_gather_into_cat_tensor_cuda(self):
            ...
            rank_to_GPU = init_multigpu_helper(dist.get_world_size(), BACKEND)
            size = 2
            tensor_in = smith.ones([size, size]) * rank
            # Concatenated output
            tensor_out = smith.ones([len(group) * size, size]) * (-1)
            ...
            expected_tensor = smith.cat([smith.ones([size, size]) * i for i in group])
            self.assertEqual(tensor_out, expected_tensor)
            self._barrier()
        ...
        def test_all_gather_into_stack_tensor_cuda(self):
            ...
            expected_tensor = smith.stack([smith.ones([size, size]) * i for i in group])
            self.assertEqual(tensor_out, expected_tensor)
            self._barrier()
        ...
        def _run_all_gather_coalesced_and_verify(
            ...
            for input_tensor in input_tensors:
                if input_tensor.dtype == smith.complex64:
                    tensor_shapes.append(smith.view_as_real(input_tensor).shape)
                else:
            ...
            for l1, l2 in zip(output_tensor_lists, expected_tensors, strict=True):
                for t1, t2 in zip(l1, l2, strict=True):
                    if not smith.equal(t1, t2):
                        return False
            return True

        def _test_all_gather_coalesced_helper(
            self, group, group_id, rank, dtype=smith.float
        ):
        ...
        def test_all_gather_coalesced_complex(self):
            ...
            self._test_all_gather_coalesced_helper(
                group, group_id, rank, dtype=smith.cfloat
            )
        ...
        def _test_all_to_all_single_equal_split_helper(
            ...
            if group_id is not None:
                size = len(group)
                in_tensor = smith.ones([size, size], dtype=dtype) * rank
                expected_tensor = smith.cat(
                    [smith.ones([1, size], dtype=dtype) * i for i in group]
                )
                out_tensor = smith.ones([size, size], dtype=dtype) * -1
                if cuda:
                    in_tensor = in_tensor.cuda(rank_to_GPU[rank][0])
                    expected_tensor = expected_tensor.cuda(rank_to_GPU[rank][0])
                    out_tensor = out_tensor.cuda(rank_to_GPU[rank][0])
                if dtype == smith.complex64:
                    tensor_shapes = [smith.view_as_real(in_tensor).shape]
                else:
        ...
        def _test_all_to_all_single_unequal_split_helper(
            ...
            if group_id is not None:
                ...
                in_tensor = smith.ones([sum(in_splits), size], dtype=dtype) * rank
                out_tensor = smith.ones([(rank + 1) * size, size], dtype=dtype)
                expected_tensor = smith.cat(
                    [smith.ones([rank + 1, size], dtype=dtype) * i for i in group]
                )
                if cuda:
                    in_tensor = in_tensor.cuda(rank_to_GPU[rank][0])
                    expected_tensor = expected_tensor.cuda(rank_to_GPU[rank][0])
        ...
        def _test_all_to_all_helper(
            ...
            if group_id is not None:
                ...
                in_splits = [i + 1 for i in group]
                in_tensors = [
                    smith.ones([in_splits[i], size], dtype=dtype) * rank
                    for i, _ in enumerate(group)
                ]
                out_tensors = [
                    smith.ones([(rank + 1), size], dtype=dtype) for _ in group
                ]
                expected_tensors = [
                    smith.ones([rank + 1, size], dtype=dtype) * i for i in group
                ]
        ...
        def test_all_to_all_complex(self):
            group, group_id, rank = self._init_global_test()
            self._test_all_to_all_helper(group, group_id, rank, dtype=smith.cfloat)
        ...
        def _test_barrier_helper(
            ...
            for dest in group:
                expected_time = smith.DoubleTensor(1).fill_(0.0)
                if cuda:
        ...
        def _prepare_dummy_data(self, local_bs):
            ...
            input_cpu = smith.randn(global_bs, 2)
            target = smith.randn(global_bs, 4)
            loss = nn.MSELoss()
        ...
        def _test_DDP_niter(
            ...
            for idx in range(n_iter):
                ...
                input = input[smith.randperm(batch_size)]

                # save the model in the middle and reload
                if test_save and idx == 2 and INIT_METHOD.startswith("file://"):
                    with tempfile.NamedTemporaryFile() as tmp:
                        if sys.platform == "win32":
                            smith.save(model_DDP, tmp)
                            tmp.seek(0)
                            # weights_only=False as this is legacy code that saves the model
                            model_DDP = smith.load(tmp, weights_only=False)
                        else:
                            smith.save(model_DDP, tmp.name)
                            # weights_only=False as this is legacy code that saves the model
                            model_DDP = smith.load(tmp.name, weights_only=False)

            with tempfile.TemporaryFile() as tmp_file:
                smith.save(model_DDP, tmp_file)
                tmp_file.seek(0)
                # weights_only=False as this is legacy code that saves the model
                saved_model = smith.load(tmp_file, weights_only=False)
        ...
        def test_ddp_create_graph(self):
            class Model(nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.p = nn.Parameter(smith.tensor(1.0))

                def forward(self):
                    return self.p.pow(2)

            model = Model()
            ddp_model = smith.nn.parallel.DistributedDataParallel(model)
            for _ in range(6):
                # Verify DDP doesn't throw when ran with create_graph=True.
                # Although we do warn about potential issues, please see
                # https://github.com/blacksmith/blacksmith/issues/63929 for details.
        ...
        def test_DistributedDataParallel_non_default_stream(self):
            stream = smith.cuda.Stream(self.rank)
            rank = self.rank
            with smith.cuda.stream(stream):
                net = smith.nn.parallel.DistributedDataParallel(
                    smith.nn.Linear(1, 1, bias=False).cuda(rank), device_ids=[rank]
                )
                for i in range(1000):
                    ...
                    if grad is not None:
                        ...
                        grad.zero_()
                    # Forward + BW
                    batch = smith.tensor([rank]).float().cuda(rank)
                    loss = net(batch).sum()
                    loss.backward()
        ...
        def test_ddp_comm_hook_logging(self):
            ...
            for hook in hooks:
                ddp_model = smith.nn.parallel.DistributedDataParallel(
                    smith.nn.Linear(1, 1, bias=False).cuda(self.rank),
            ...
            for _ in range(2):
                inp = smith.ones(1, 1, device=self.rank)
        ...
        def _test_ddp_hook_with_optimizer_parity(
            ...
            **functional_optim_kwargs,
            ...
            rank = self.rank
            smith.cuda.set_device(rank)
            smith.manual_seed(rank)
            smith.cuda.manual_seed(rank)
            models_to_test = [
                (LargeNet(), smith.randn(1, 1000).cuda()),
            ]
            if HAS_SMITHVISION:
                models_to_test.append(
                    (smithvision.models.resnet50(), smith.randn(1, 3, 3, 1000).cuda())
                )
            for model, inp in models_to_test:
                ...
                with smith.backends.cudnn.flags(
                    ...
                    ddp_model_with_optimizer_hook = (
                        smith.nn.parallel.DistributedDataParallel(
                    ...
                    # backward.
                    ddp_model_with_no_hook = smith.nn.parallel.DistributedDataParallel(
                        copy.deepcopy(model).cuda(),
                        device_ids=[self.rank],
                        gradient_as_bucket_view=grad_as_bucket_view,
                        static_graph=static_graph,
        ...
        from smith.testing._internal.common_utils import parametrize

        @skip_but_pass_in_sandcastle_if(
            BACKEND == "nccl" or BACKEND == "ucc",
            "Issues with async error handling, see https://github.com/blacksmith/blacksmith/issues/73259",
        )
        @skip_if_lt_x_gpu(2)
        ...
        def test_get_data_parallel_params(self):
            ...
            smith.nn.parallel.DistributedDataParallel._set_params_and_buffers_to_ignore_for_model(
                model, params_to_ignore
            )
            smith.nn.parallel.DistributedDataParallel(model, device_ids=[self.rank])
            dp_params = (
                smith.nn.parallel.DistributedDataParallel._get_data_parallel_params(
        ...
        def _test_ddp_apply_optim_in_backward(
            ...
            smith.manual_seed(self.rank)
            smith.cuda.manual_seed(self.rank)
            smith.cuda.set_device(self.rank)
            ...
            if HAS_SMITHVISION:
                models_to_test.append(smithvision.models.resnet50().cuda())
            ...
            for j, model in enumerate(models_to_test):
                ...
                with smith.backends.cudnn.flags(
                    ...
                    for i in range(8):
                        inp = (
                            smith.randn(1, 3, 1000, 1000, device="cuda")
                            if j == 2
                            else smith.randn(10, 3, device="cuda")
                        )
        ...
        def test_ddp_apply_optim_in_backward(self):
            for optim_cls, init_before in itertools.product(
                [smith.optim.SGD, smith.optim.Adam], [True, False]
            ):
        ...
        def test_ddp_apply_optim_in_backward_ignored_params(self):
            ...
            for init_before in [True, False]:
                with self.subTest(init_before=init_before):
                    ...
                    if init_before:
                        _apply_optimizer_in_backward(
                            optimizer_class=smith.optim.SGD,
        ...
        def _get_fp16_config(self) -> _MixedPrecision:
            return _MixedPrecision(
                param_dtype=smith.float16,
                reduce_dtype=smith.float16,
                buffer_dtype=smith.float16,
            )
        ...
        def test_ddp_native_mixed_precision_ignored_params(self):
            ...
            smith.cuda.set_device(rank)
            model = TwoLinLayerNet()
            model.register_buffer("buffer", smith.ones(5))
            # Parameters to ignore are in the format {module_name}.{param_name}
            ...
            for n, p in itertools.chain(net.named_parameters(), net.named_buffers()):
                if n in to_ignore:
                    ...
                    self.assertEqual(smith.float32, p._fp_param.dtype)
        ...
        def _test_ddp_native_mixed_precision(
            ...
            smith.cuda.manual_seed(rank)
            smith.cuda.set_device(rank)
            inp = smith.randn(10, 1)
            mp_config = self._get_fp16_config()

            class MyModel(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.m = smith.nn.Linear(1, 5)
                    self.register_buffer("buffer", smith.randn(1, 2))
                    self.p = smith.nn.Parameter(smith.randn(10, 5), requires_grad=False)
            ...
            for _ in range(6):
                ...
                for n, param in net.named_parameters():
                    self.assertEqual(param.dtype, smith.float32)
                    if param.grad is None:
                        if n != "module.p":  # Only param that doesn't require grad
                            raise AssertionError(f"Expected n == 'module.p', got {n!r}")
                    else:
                        self.assertEqual(param.grad.dtype, smith.float32)
                        tensor_list = [
                            smith.zeros_like(param.grad)
                        ...
                        for g_ in rest:
                            self.assertEqual(g_.dtype, smith.float32)
        ...
        def _test_ddp_hook_parity(self, state, hook, num_validated_iters=100):
            rank = self.rank
            m = smith.nn.Linear(1, 5)
            try:
                process_group = state.process_group
            except AttributeError:
                process_group = state

            net_with_hook = smith.nn.parallel.DistributedDataParallel(
                copy.deepcopy(m).to(rank),
                device_ids=[rank],
                process_group=process_group,
            )
            net_with_hook.register_comm_hook(state=state, hook=hook)
            net_without_hook = smith.nn.parallel.DistributedDataParallel(
                copy.deepcopy(m).to(rank),
                device_ids=[rank],
                process_group=process_group,
            )
        ...
        def test_ddp_hook_parity_allreduce_process_group(self):
            ...
            process_group = smith.distributed.new_group(gpus)
        ...
        def _prepare_single_device_module(
            ...
            model = Net()
            device = devices[0] if devices else smith.device(f"cuda:{rank:d}")
            ...
            input = smith.randn(global_batch_size, 2).to(device)
            target = smith.randn(global_batch_size, 4).to(device)
        ...
        def _prepare_cpu_module(
            ...
            input = smith.randn(global_batch_size, 2)
            target = smith.randn(global_batch_size, 4)
            return model, ddp_model, input, target
        ...
        def _test_accumulate_gradients_no_sync(
            ...
            if BACKEND == "nccl":
                ...
                devices = [smith.device("cuda:" + str(i)) for i in int_devices]
                global_batch_size = world_size
            ...
            for iteration in range(num_iters):
                ...
                # Shuffle the input so that DDP input is different
                smith.manual_seed(1337 + iteration)
                input = input[smith.randperm(global_batch_size)]
        ...
        def test_accumulate_gradients_no_sync_allreduce_hook(self):
            ...
            ) -> smith.futures.Future[smith.Tensor]:
        ...
        def test_DistributedDataParallel(self):
            ...
            for use_bucket_view, static_graph in itertools.product(
                ...
                self._test_DistributedDataParallel(
                    gpu_subset=gpus,
                    rank=rank,
                    output_device=smith.device("cuda"),
                    gradient_as_bucket_view=use_bucket_view,
                    static_graph=static_graph,
                )

                # test device_ids
                gpus_list = [smith.device("cuda:" + str(i)) for i in gpus]
                self._test_DistributedDataParallel(
        ...
        def _test_DistributedDataParallel_with_amp(self, grad_is_view=False):
            smith.manual_seed(31415)
            # Creates model and optimizer in default precision
            model = Net().cuda()
            optimizer = smith.optim.SGD(model.parameters(), lr=0.03)
            ...
            input = smith.randn(dist.get_world_size() * 2, 2).cuda()
            target = smith.randn(dist.get_world_size() * 2, 4).cuda()
            loss_fn = nn.MSELoss()
            ...
            for idx in range(20):
                ...
                # Shuffle the input so that DDP input is different
                smith.manual_seed(1337 + idx)
                input = input[smith.randperm(dist.get_world_size() * 2)]
        ...
        def _test_post_localSGD_optimizer_parity(self, create_averager, grad_is_view):
            ...
            opt = smith.optim.SGD(net.parameters(), lr=learning_rate)

            net_using_post_localSGD_opt = smith.nn.parallel.DistributedDataParallel(
                copy.deepcopy(DDP_NET).cuda(),
                device_ids=[self.rank],
                gradient_as_bucket_view=grad_is_view,
            )
            ...
            # https://github.com/blacksmith/blacksmith/pull/74737#pullrequestreview-922487496
        ...
        def _create_post_localSGD_optimizer(self, net, learning_rate, averager):
            return post_localSGD_optimizer.PostLocalSGDOptimizer(
                optim=smith.optim.SGD(net.parameters(), lr=learning_rate),
                averager=averager,
            )
        ...
        def _test_post_localSGD_optimizer_step_reload(
            ...
            checkpoint = smith.load(chkpt_file, map_location=map_location)
        ...
        def _test_DistributedDataParallel_SyncBatchNorm_with_memory_format(
            ...
            shapes = [global_bs, 2, 4, 4] + (
                [] if memory_format is smith.channels_last else [4]
            )

            input_gpu = (
                smith.randn(*shapes, dtype=smith.float)
                .cuda(rank)
        ...
        def test_DistributedDataParallel_SyncBatchNorm(self):
            ...
            # test device_ids
            gpus = [smith.device("cuda:" + str(i)) for i in gpus]
            self._test_DistributedDataParallel_SyncBatchNorm(
                gpu_subset=gpus,
                rank=rank,
                local_bs=local_bs,
        ...
        def test_DistributedDataParallel_SyncBatchNorm_Diff_Input_Sizes_Running_Value(
            ...
            for i in range(dist.get_world_size()):
                input_var_rank = smith.cat(
                    [
                        smith.ones(2, 1, 10 ** (i + 1)) * (0.1 ** (i - 1)),
                        smith.ones(2, 1, 10 ** (i + 1)) * (0.3 ** (i - 1)),
                    ],
                    dim=1,
                )
                input_var.append(input_var_rank)

            all_input_var = smith.cat(
                [
            ...
            smith.testing.assert_close(running_mean, all_input_var.mean(1))
            smith.testing.assert_close(running_var, all_input_var.var(1))
        ...
        def test_DistributedDataParallel_SyncBatchNorm_half(self):
            ...
            model = BatchNormNet()
            model = model.half()
            model = smith.nn.SyncBatchNorm.convert_sync_batchnorm(model)
            model = nn.parallel.DistributedDataParallel(
                model.cuda(rank), device_ids=[rank]
            )
            inp = smith.randn(2, 2, dtype=smith.float16, device=smith.device(rank))
            # Check that forward/backward do not error with dtype mismatch
            out = model(inp)
            self.assertEqual(out.dtype, smith.float16)
            out.sum().backward()
            for param in model.parameters():
                self.assertEqual(param.grad.dtype, smith.float16)
        ...
        def test_ddp_logging_data_cpu(self):
            ...
            self.assertEqual(
                ddp_logging_data.get("smith_distributed_debug"),
                parse_env("SMITH_DISTRIBUTED_DEBUG"),
            )
            self.assertEqual(
        ...
        def test_ddp_bucket_cap_mb_list(self):
            ...
            input_tensor = smith.randn(10, 1000).cuda(self.rank)
        ...
        def test_SyncBatchNorm_process_group(self):
            ...
            # is nested in a sub-module or sub-sub-module (e.g. resnet50 in smithvision.models).

            process_ids = 0
            process_group = smith.distributed.new_group([process_ids])
            res50_model = smithvision.models.resnet50()
        ...
        def test_nccl_backend_bool_allreduce(self):
            ...
            for op in [dist.ReduceOp.PRODUCT, dist.ReduceOp.MIN]:
                input_tensor = smith.tensor([element, element]).to(self.rank)
                self._run_reduction_test(
                    input_tensor, smith.tensor([False, False]).to(self.rank), op
                )
                # Ensure that all ranks contributing True (cast to 1) results in the
                # correct reduction.
                input_tensor = smith.tensor([True, True]).to(self.rank)
            ...
            for op in [dist.ReduceOp.SUM, dist.ReduceOp.MAX]:
                input_tensor = smith.tensor([element, element]).to(self.rank)
                self._run_reduction_test(
                    input_tensor, smith.tensor([True, True]).to(self.rank), op
                )
            # TODO: NCCL backend does not work correctly for bitwise reduction ops
            # (see https://github.com/blacksmith/blacksmith/issues/41362). Add tests for
        ...
        def test_nccl_backend_bool_allgather(self):
            ...
            for i, t in enumerate(tensor_list):
                expected = smith.tensor(inp[i % 2]).to(self.rank)
        ...
        def test_nccl_backend_bool_reduce(self):
            ...
            for op in [dist.ReduceOp.PRODUCT, dist.ReduceOp.MIN]:
                ...
                input_tensor = smith.tensor(inp[(self.rank + 1) % 2]).to(self.rank)
                expected = smith.tensor([False, False]).to(self.rank)
            ...
            for op in [dist.ReduceOp.SUM, dist.ReduceOp.MAX]:
                input_tensor = smith.tensor(inp[self.rank % 2]).to(self.rank)
                expected = (
                    smith.tensor([True, True]).to(self.rank)
                    if self.rank == 0
        ...
        def test_nccl_backend_bool_broadcast(self):
            ...
            tensor_list = [
                smith.tensor([False for _ in range(tensor_size)]).to(self.rank)
                for _ in range(dist.get_world_size())
            ]
        ...
        def test_DistributedSampler_padding(self):
            ...
            dataset = [smith.ones(1).to(self.rank) * i for i in range(dataset_size)]

            # Simulates the 'tiny' dataset size
            dataset_tiny_size = max(world_size // 2 - 1, 1)
            dataset_tiny = [
                smith.ones(1).to(self.rank) * i for i in range(dataset_tiny_size)
            ]
            ...
            def validate_global_samples(local_num_samples):
                ...
                world_samples = [
                    smith.LongTensor([0]).to(self.rank) for _ in range(world_size)
                ]
                dist.all_gather(
                    world_samples, smith.tensor([local_num_samples]).to(self.rank)
                )
        ...
        def _test_allgather_object(self, subgroup=None):
            ...
            if backend == "nccl":
                ...
                smith.cuda.set_device(next_rank)

            # If GPU test, add object with GPU tensor
            if backend == "nccl":
                gather_objects.append(Foo(smith.randn(3, 3, device=0)))
        ...
        def _test_gather_object(self, pg=None):
            ...
            # If GPU test, add object with GPU tensor
            if backend == "nccl":
                gather_objects.append(Foo(smith.randn(3, 3, device=my_rank)))
        ...
        def validate_net_equivalence(self, net):
            ...
            for t in net_module_states:
                tensor_list = [
                    smith.zeros_like(t) for _ in range(dist.get_world_size())
                ]
        ...
        def test_ddp_grad_div_uneven_inputs(self):
            ...
            rank = self.rank
            model = nn.Linear(dim, dim, bias=False)
            inp = smith.ones(batch, dim, device=self.rank) * grad_scale
            net = smith.nn.parallel.DistributedDataParallel(
                model.cuda(rank), device_ids=[self.rank], bucket_cap_mb=1
            )
            n_iters = 3
            ...
            with net.join(divide_by_initial_world_size=True):
                for i in range(n_iters):
                    ...
                    if i >= 3:
                        effective_ws -= 1
                    expected_grad = (
                        smith.ones(dim, dim, device=self.rank)
                        * grad_scale
                        * effective_ws
                    ) / dist.get_world_size()
                    param = next(iter(net.parameters()))
        ...
        def _test_ddp_profiling(self, profiler_ctx, profiler_ctx2=None):
            ...
            smith.cuda.set_device(self.rank)
            model = nn.Linear(dim, dim, bias=False)
            inp = smith.rand(batch, dim, device=self.rank)
            net = smith.nn.parallel.DistributedDataParallel(
                model.cuda(self.rank),
                device_ids=[self.rank],
            )
        ...
        def test_ddp_profiling_smith_profiler(self):
            cpu_act = smith.profiler.ProfilerActivity.CPU
            cuda_act = smith.profiler.ProfilerActivity.CUDA
            smith_profiler_ctx = smith.profiler.profile(activities=[cpu_act, cuda_act])
            prof = self._test_ddp_profiling(profiler_ctx=smith_profiler_ctx)
        ...
        def test_ddp_profiling_execution_trace(self):
            ...
            with TemporaryFileName("w+t", suffix=".et.json") as et_file:
                ...
                smith_profiler_ctx1 = smith.profiler.profile(
                    activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                )
                # collect ET in second profiler pass
                smith_profiler_ctx2 = smith.profiler.profile(
                    activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                    execution_trace_observer=et,
                )
                self._test_ddp_profiling(
                    profiler_ctx=smith_profiler_ctx1,
                    profiler_ctx2=smith_profiler_ctx2,
                )
        ...
        def test_ddp_join_model_equivalence(self):
            ...
            local_optim = smith.optim.SGD(local_model.parameters(), lr=learning_rate)
        ...
        def _run_uneven_inputs_test(
            ...
            num_iters_tensor = smith.tensor(
                [num_iters], device=smith.cuda.current_device()
            )
                ...
                with net.join(
                    ...
                    for i in range(num_iters):
                        ...
                        with context:
                            ...
                            smith.cuda.synchronize(device=rank)
                        total_iters += 1
            if test_case.throw_on_early_termination:
            ...
            if not test_case.throw_on_early_termination:
                ...
                final_rank_tensor = smith.tensor(
                    [net._authoritative_rank], device=self.rank
                )
                tensor_list = [
                    smith.zeros_like(final_rank_tensor)
                    for _ in range(dist.get_world_size())
                ]
        ...
        def test_ddp_uneven_inputs_stop_iteration_sync_bn(self):
            ...
            class ModelWithComm(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
            ...
            model_input = smith.randn(10, 2).cuda(smith.cuda.current_device())

            for model in [model_bn, comm_model]:
                model = smith.nn.parallel.DistributedDataParallel(
                    model,
                    device_ids=[self.rank],
                )
                min_num_iters = 5
        ...
        def test_ddp_uneven_inputs(self):
            ...
            models_to_test = [
                ...
                DDPUnevenTestInput(
                    name="batch_norm_net",
                    model=bn_net,
                    inp=smith.ones(batch, 2, device=rank),
                    sync_interval=1,
                ),
                DDPUnevenTestInput(
                    name="large_conv_model",
                    model=large_model,
                    inp=smith.ones(batch, batch, dim, dim, device=rank),
                    sync_interval=1,
                ),
                DDPUnevenTestInput(
                    name="small_model",
                    model=small_model,
                    inp=smith.ones(batch, dim, device=rank),
                    sync_interval=1,
                ),
                ...
                DDPUnevenTestInput(
                    ...
                    inp=(smith.ones(batch, 2, device=rank), rank),
                    sync_interval=1,
                ),
            ...
            # Add resnet model if we have smithvision installed.
            if HAS_SMITHVISION:
                resnet_model = smithvision.models.resnet50()
                models_to_test.append(
                    DDPUnevenTestInput(
                        name="resnet_model",
                        model=resnet_model,
                        inp=smith.ones(1, 3, 1000, 1000),
                        sync_interval=1,
                    )
                )
        ...
        def test_ddp_uneven_input_join_disable(self):
            ...
            smith.manual_seed(self.rank)
            net = smith.nn.parallel.DistributedDataParallel(
                smith.nn.Linear(1, 1).cuda(self.rank), device_ids=[self.rank]
            )
            inp = smith.ones(1) * self.rank
            n_iters = 5
            world_size = dist.get_world_size()
        ...
        def test_ddp_uneven_input_exception(self):
            ...
            class ExceptionModule(nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.param = nn.Parameter(smith.ones(1, requires_grad=True))
        ...
        def _test_broadcast_object_list(self, group=None):
            ...
            if IS_FBCODE:
                ...
                gather_objects.append(Foo(smith.randn(3, 178956971)))
            objects = (
                gather_objects
            ...
            if backend != "nccl":
                ...
                dist.broadcast_object_list(
                    single_obj_list, src=0, group=group, device=smith.device("cpu")
                )
            ...
            if backend != "nccl" and smith.cuda.device_count() == int(self.world_size):
                single_obj_list = [objects[0]]
                if self.rank != src_rank:
                    self.assertNotEqual(single_obj_list[0], gather_objects[0])
                dist.broadcast_object_list(
                    single_obj_list, src=0, group=group, device=smith.device(next_rank)
                )
                self.assertEqual(single_obj_list[0], gather_objects[0])
        ...
        @unittest.skip(
            "Test is failing, see https://github.com/blacksmith/blacksmith/pull/113620"
        )
        ...
        def _test_ddp_ignore_params_arg(self, static_graph=False):
            ...
            for find_unused, broadcast_buffers in itertools.product(
                ...
                model.fc2.register_buffer(
                    "ignore_buffer", smith.zeros(5 + self.rank, device=self.rank)
                )
                ...
                ddp = smith.nn.parallel.DistributedDataParallel(
                    model,
                ...
                inp = smith.ones(1, dtype=smith.float).to(device_id) * (self.rank + 1)
                for _ in range(6):
        ...
        def test_ddp_shared_grad_acc_unused_params(self):
            ...
            smith.cuda.set_device(self.rank)
            model = ToyModel().to(smith.cuda.current_device())
            for static in [True, False]:
                ...
                inp = smith.randn(20, 10, device=self.rank)
                for _ in range(6):
                    loss = ddp_model(inp)
                    # To test https://github.com/blacksmith/blacksmith/issues/61982
                    loss /= 10
                    loss.backward()
        ...
        def test_ddp_device(self):
            ...
            # to forward.
            inp = tuple(smith.randn(10, 10) for _ in range(expected_len))
            train_iter(inp, tuple)

            # List CPU input, should be moved to proper device before call to
            # forward.
            inp = [smith.randn(10, 10) for _ in range(expected_len)]
            train_iter(inp, list)
            ...
            inp = TensorWrapper(smith.randn(10, 10))
            ...
            batch = 5
            dim = 10
            a = smith.rand(batch, dim)
            b = smith.rand(batch, dim)
        ...
        def test_ddp_namedtuple(self):
            ...
            a = smith.rand(batch, dim, device=self.rank)
            b = smith.rand(batch, dim, device=self.rank)

            class NamedTupleModule(smith.nn.Module):
                def __init__(self_):  # noqa: B902
                    super().__init__()
                ...
                def forward(self_, input, expected_type):  # noqa: B902
                    ...
                    return self_.lin(smith.mul(input.a, input.b))
        ...
        def test_grads_same_across_ranks_with_no_sync(self):
            ...
            grads = smith.cat([p.grad.view(-1) for p in model.parameters()])

            # Gather all gradients to rank 0.
            if rank == 0:
                gathered_grads = [smith.zeros_like(grads) for _ in range(world_size)]
            else:
                gathered_grads = []
            ...
            if rank == 0:
                for g in gathered_grads[1:]:
                    self.assertTrue(
                        smith.allclose(gathered_grads[0], g),
        ...
        def test_ddp_control_flow_same_across_ranks(self):
            ...
            random_input = smith.randn(batch, dim, device=self.rank)
            ones_input = smith.ones(batch, dim, device=self.rank)
            for i in range(6):
                if i % 2 == 0:
                ...
                if i % 2 == 0:
                    expected = smith.tensor(
                        [world_size, 0], device=self.rank, dtype=smith.int32
                    )
                else:
                    expected = smith.tensor(
                        [world_size, world_size], device=self.rank, dtype=smith.int32
                    )
        ...
        def test_invalid_static_graph(self):
            ...
            random_input = smith.randn(20, 10, device=self.rank)
            ones_input = smith.ones(20, 10, device=self.rank)
        ...
        def test_ddp_control_flow_different_across_ranks(self):
            ...
            class ToyModel(nn.Module):
                ...
                def forward(self, x):
                    ...
                    # model.
                    use_second_layer = (
                        smith.equal(x, smith.ones(batch, dim, device=x.device))
                        and self.rank == 1
                    )
            ...
            for i in range(6):
                ...
                if i % 2 == 0:
                    ...
                    expected = smith.tensor(
                        [world_size, 1], device=self.rank, dtype=smith.int32
                    )
        ...
        def _generate_sparse_tensors_for_bucket_assignment_test(self):
            tensors = [
                smith.empty([50], dtype=smith.float),
                smith.empty([25], dtype=smith.double),
                smith.empty([50], dtype=smith.float),
                smith.empty([25], dtype=smith.double),
                smith.empty([50], dtype=smith.float),
                smith.empty([25], dtype=smith.double),
        ...
        def _test_compute_bucket_assignment_by_size(self, use_logger):
            ...
            # Set SMITH_NCCL_BLOCKING_WAIT and use a new NCCL group to improve test
            # determinism.
            os.environ["SMITH_NCCL_BLOCKING_WAIT"] = "1"
        ...
        def test_undefined_grad_parity_unused_parameters(self):
            ...
            # https://github.com/blacksmith/blacksmith/issues/58511.
            x = smith.ones(1, 2).to(self.rank)
            ...
            smith._C._functions.UndefinedGrad()(out).backward()
            smith._C._functions.UndefinedGrad()(local_out).backward()
        ...
        def test_monitored_barrier_gloo(self):
            tensors = [smith.ones(10) * self.rank]
            # Kick off some allreduce work on all ranks
            for _ in range(10):
                dist.all_reduce(smith.cat(tensors))
        ...
        def _test_monitored_barrier_allreduce_hang(self, wait_all_ranks):
            ...
            tensors = [smith.ones(10, device=self.rank) * self.rank]
            ...
            # in https://github.com/blacksmith/blacksmith/issues/54524.
            ...
            # practice, we don't need SMITH_NCCL_BLOCKING_WAIT, but we use it in this
        ...
        def test_monitored_barrier_allreduce_hang_wait_all_ranks(self):
            # Need to disable SMITH_NCCL_DUMP_ON_TIMEOUT otherwise this test times out
            os.environ["SMITH_NCCL_DUMP_ON_TIMEOUT"] = "0"
            # tests expected behavior when nonzero rank hangs and we want to
        ...
        def test_ddp_build_debug_param_to_name_mapping(self):
            ...
            net_params.extend(
                [
                    smith.nn.Parameter(smith.ones(1)),
                    smith.nn.Parameter(smith.ones(1)),
                ]
            )
        ...
        def test_ddp_inference(self):
            ...
            syncbn_model = smith.nn.parallel.DistributedDataParallel(
                syncbn_model, device_ids=[rank]
            )
            inp = smith.randn(10, 2, device=rank)
            inp_syncbn = smith.randn(10, 2, 4, 4, device=rank)
            tests = [
        ...
        def test_ddp_sync_bn_training_vs_eval(self):
            ...
            model = smith.nn.parallel.DistributedDataParallel(model, device_ids=[rank])
            # Test sync occurs in training mode.
            with smith.autograd.profiler.profile() as prof:
                for _ in range(6):
                    inp = smith.randn(10, 2, 4, 4).cuda(rank)
                    out = model(inp)
                    loss = out.sum()
                    loss.backward()
        ...
        def test_ddp_static_graph_nested_types(self):
            ...
            class NestedOutputModule(smith.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
            ...
            def get_loss(model_output):
                loss = 0.0
                if isinstance(model_output, smith.Tensor):
            ...
            model_static_graph = smith.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[rank],
                static_graph=True,
            )
            inp = smith.randn(10, 100)
            type_mapping = {
                "list": list,
                "tuple": tuple,
                "dict": dict,
        ...
        def test_ddp_returns_tensor_with_no_grad(self):
            ...
            inp = smith.randn(1, 10, device=self.rank)
        ...
        def test_detect_ddp_is_actually_static(self):
            ...
            for find_unused in [True, False]:
                ...
                inp = smith.randn(1, 10, device="cuda")
                for _ in range(6):
        ...
        def _test_ddp_new_tensor_in_fwd(self, static_graph):
            # Test from https://github.com/blacksmith/blacksmith/issues/60733
            class MyModel(nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.fc1 = nn.Linear(10, 10, bias=False)
                ...
                def __init_opt(self):
                    opt = smith.randn(1, 10, device=self.device)
                    return opt
                ...
                def forward(self, x, opt_1, opt_2, opt_nested):
                    ...
                    if opt_nested is None or not smith.is_tensor(opt_nested):
        ...
        def _test_ddp_buffer_hook_allreduce(self, return_futures):
            ...
            def buffer_comm_hook(ddp, named_buffers):
                ...
                if return_futures:
                    ...
                    smith.futures.collect_all(futs).wait()

            hook_pre_fwd = (
                smith.nn.parallel.distributed._BufferCommHookLocation.PRE_FORWARD
            )
            hook_post_fwd = (
                smith.nn.parallel.distributed._BufferCommHookLocation.POST_FORWARD
            )
            for hook_run_location in [
                ...
                model_ddp = smith.nn.parallel.DistributedDataParallel(
                    model,
                    device_ids=[self.rank],
                )
                ...
                model_ddp_no_hook = smith.nn.parallel.DistributedDataParallel(
                    copy.deepcopy(model),
                    device_ids=[self.rank],
                    broadcast_buffers=False,
                )
                inp = smith.randn(2, 10, device=rank)
                for _ in range(2):
        ...
        def test_ddp_remove_autograd_hooks(self):
            class SimulateError(smith.autograd.Function):
                @staticmethod
            ...
            input = smith.rand(10, 10, requires_grad=True).cuda(self.rank)
            model_ddp1 = smith.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.rank],
            )
            ...
            model.error = False
            model_ddp2 = smith.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.rank],
            )
        ...
        @unittest.skip(
            "Test is failing, tracking issue at https://github.com/blacksmith/blacksmith/issues/102751"
        )
        def test_ddp_has_finalized(self):
            ...
            model = MyModel(self.rank)
            input = smith.rand(10, 1024, requires_grad=True).cuda(self.rank)
            ddp = smith.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.rank],
                find_unused_parameters=True,
        ...
        def _run_ddp_update_process_group(self, new_pg):
            def get_num_smith_recompiles():
                guard_failures = smith._dynamo.utils.guard_failures
            ...
            class MyModel(smith.nn.Module):
                def __init__(self, device):
                    ...
                    self.fc1 = smith.nn.Linear(1024, 1024).cuda(device)
                    self.fc2 = smith.nn.Linear(1024, 1024).cuda(device)
                    self.fc3 = smith.nn.Linear(1024, 1024).cuda(device)
            ...
            run_iteration()
            if get_num_smith_recompiles() != 0:
                raise AssertionError(f"Expected 0 smith recompiles, got {get_num_smith_recompiles()}")
        ...
        def test_ddp_update_process_group_grad_undefined(self):
            ...
            class MyModel(smith.nn.Module):
                def __init__(self, device):
                    super().__init__()
                    self.fc1 = smith.nn.Linear(10, 10).cuda(device)
                    self.fc2 = smith.nn.Linear(10, 10).cuda(device)
                    self.fc3 = smith.nn.Linear(10, 10).cuda(device)
        ...
        def test_ddp_update_process_group_no_find_unused(self):
            ddp = smith.nn.parallel.DistributedDataParallel(
                smith.nn.Linear(10, 10).cuda(self.rank),
                device_ids=[self.rank],
                find_unused_parameters=False,
            )
            ddp._update_process_group(_get_default_group())
        ...
        def test_ddp_broadcast_buffer(self):
            ...
            for _ in range(2):
                ...
                bufs = [
                    smith.empty_like(model_ddp.module.buffer)
                    for _ in range(dist.get_world_size())
                ]
        ...
        def test_static_graph_multi_forward(self):
            ...
            smith.cuda.set_device(self.rank)
            smith.manual_seed(42 << 1337 % (self.rank + 1))
            model = Net().cuda(self.rank)
            local_model = copy.deepcopy(model)
            model = smith.nn.parallel.DistributedDataParallel(
                model, device_ids=[self.rank], static_graph=True
            )
            inp = smith.ones(2, 10, device="cuda")
            for _ in range(3):
                model.zero_grad()
                ...
                if self.rank == 0:
                    ...
                    for p_ddp, p_local in zip(
                        ...
                        self.assertTrue(
                            smith.allclose(p_ddp.grad, p_local.grad),
        ...
        def test_sync_bn_logged(self):
            ...
            no_sync_bn = smith.nn.parallel.DistributedDataParallel(
        ...
        def test_stateless_api_with_ddp(self):
            ...
            module = smith.nn.parallel.DistributedDataParallel(
                module, device_ids=[device]
            )
            x = smith.rand((1, 1)).to(device)
            weight = smith.tensor([[1.0]], device=device, requires_grad=True)
            bias = smith.tensor([0.0], device=device, requires_grad=True)
            buffer = smith.tensor([0.0], device=device)
            parameters = {
            ...
            res = smith.func.functional_call(module, parameters, x)
        ...
        def _test_hook_pickling(self, hook, hook_state):
            ...
            input = smith.randn(7, 1, device=rank)
            target = smith.randn(7, 5, device=rank)
            net = smith.nn.Linear(1, 5).to(rank)
            ...
            optimizer = smith.optim.SGD(ddp_model.parameters(), lr=learning_rate)
            ...
            if rank == 0:
                with self.assertLogs("smith.distributed") as captured:
                    smith.save(state, chkpt_file)
            ...
            dummy_optimizer = smith.optim.SGD(
        ...
        def test_ddp_compile_static_graph(self):
            "Tests that DDP works with smith compile when static_graph=True"
            model = smith.nn.Linear(10, 10).cuda(self.rank)
            model_clone = copy.deepcopy(model)
            ddp = smith.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.rank],
            )
            ddp_static = smith.nn.parallel.DistributedDataParallel(
                model_clone, device_ids=[self.rank], static_graph=True
            )
            ddp = smith.compile(ddp)
            ddp_static = smith.compile(ddp_static)
            input = smith.rand(10, 10).cuda(self.rank)
        ...
        def test_ddp_sink_noclone(self):
            ...
            class OpPatcher(SmithDispatchMode):
                def __smith_dispatch__(self, func, types, args=(), kwargs=None):
                    func_packet = func._overloadpacket
                    if func_packet == smith.ops.aten.clone:
        ...
        def _test_skip_all_reduce_unused_parameters(
            ...
            random_input = smith.randn(20, 100, device=self.rank)
            for _ in range(10):
                out = ddp_model(random_input)
                loss = out.sum()
                loss.backward()
