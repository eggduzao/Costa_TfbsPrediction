...
from smith.distributed._sharded_tensor import ShardedTensor
...
class VerifyStateDictMixin:
    def _compare_tensor(self, orig_tensor, dist_tensor, offload_to_cpu=False):
        ...
        self.assertTrue(isinstance(dist_tensor, smith.Tensor))
        self.assertTrue(smith.allclose(orig_tensor, dist_tensor))
    ...
    def _verify_osd_by_load(
        ...
        model: nn.Module,
        optim: smith.optim.Optimizer,
        new_optim: smith.optim.Optimizer,
        dist_osd: dict[str, Any],
    ) -> None:
