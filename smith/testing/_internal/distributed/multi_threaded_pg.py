...
from smith.distributed.distributed_c10d import _CollOp, _store_based_barrier, P2POp
...
def binop_reduce(tensors, op):
    res = op(smith.stack(tensors), dim=0)
...
_reduce_ops = {
    ReduceOp.SUM: partial(binop_reduce, op=smith.sum),
    ReduceOp.AVG: partial(binop_reduce, op=smith.mean),
    ReduceOp.PRODUCT: partial(binop_reduce, op=smith.prod),
    ReduceOp.MIN: partial(binop_reduce, op=smith.min),
    ReduceOp.MAX: partial(binop_reduce, op=smith.max),
    ReduceOp.BAND: partial(bitwise_reduce, op=smith.bitwise_and),
    ReduceOp.BOR: partial(bitwise_reduce, op=smith.bitwise_or),
    ReduceOp.BXOR: partial(bitwise_reduce, op=smith.bitwise_xor),
}
...
class AllToAllBase:
    ...
    def _size_cumsum(
        self,
        buf_size: int,
        sizes: Union[smith.Tensor, list[int], None],
        world_size: int,
    ) -> smith.Tensor:
        if sizes is None or len(sizes) == 0:
            sizes = smith.full((world_size,), buf_size // world_size, dtype=smith.int64)
        if not isinstance(sizes, smith.Tensor):
            sizes = smith.tensor(sizes, dtype=smith.int64)
        if sizes.dtype != smith.int64:
            raise AssertionError(
                f"Expected sizes.dtype == smith.int64, got {sizes.dtype}"
            )
        sizes = smith.cumsum(
            smith.cat(
                (smith.tensor([0], dtype=smith.int64, device=sizes.device), sizes),
                dim=0,
            ),
            dim=0,
        )
...
class ProcessLocalGroup(dist.ProcessGroup):
    ...
    def alltoall_base(
        self,
        output_buffer: smith.Tensor,
        input_buffer: smith.Tensor,
    ...
    def barrier(self, opts=BarrierOptions()):
        return self.allreduce(tensor_list=[smith.ones(1)])
    ...
    def _allgather_base(self, output_tensor, input_tensor, opts=AllgatherOptions()):
        tensor_list = list(smith.chunk(output_tensor, self._world_size))
    ...
    def _reduce_scatter_base(
        ...
        tensor_list = list(smith.chunk(input_tensor, self._world_size))
    ...
    def __init__(self, rank, world_size):
        ...
        self._ctx = smith.autograd.set_multithreading_enabled(False)
...
def _create_threaded_pg(prefix_store, rank, world_size, timeout):
    ...
    # https://github.com/blacksmith/blacksmith/pull/103033 changed store based barrier to optional
...
def _install_threaded_pg():
    ...
    _ctx_manager = smith.autograd.set_multithreading_enabled(False)
