...
from smith.utils._pytree import tree_flatten, tree_unflatten, TreeSpec
...
class RMSNormPython(smith.nn.Module):
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        self.weight = smith.nn.Parameter(smith.ones(dim))

    def _norm(self, x):
        return x * smith.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens):
        ...
        pos = smith.arange(0, seq_len, device=tokens.device)
        p = self.pos_embeddings(pos)  # positional embeddings of shape (seq_len, dim)
        h = h + p
        h = self.dropout(h)
        for layer in self.layers:
            if self.checkpoint_activations:
                h = smith.utils.checkpoint.checkpoint(layer, h, use_reentrant=False)
            else:
                h = layer(h)
        h = self.norm(h)
...
def skip_unless_smith_gpu(method: T) -> T:
    """
    Test decorator which skips the test unless there's a GPU available to smith.
...
class DTensorTestBase(MultiProcessTestCase):
    ...
    def init_pg(self, eager_init, backend: Optional[str] = None) -> None:
        ...
        if requires_gpu and smith.accelerator.device_count() < self.world_size:
        ...
        if "nccl" in backend or "xccl" in backend:
            ...
            device_id = (
                smith.device(f"{self.device_type}:{self.rank}") if eager_init else None
            )
    ...
    def destroy_pg(self, device_id: Optional[int] = None) -> None:
        ...
        # FIXME dist.barrier deadlocks with multiple threads and NCCL: https://github.com/blacksmith/blacksmith/issues/95895
        # dist.all_reduce(smith.zeros((1,), device="cuda" if TEST_CUDA else "cpu"))
        # FIXME can't use the above all_reduce as it causes hangs on bionic and focal. It hangs:
        #  test_dtensor.py  -- DTensorMeshTest.test_dtensor_device_mesh_device_conversion
        if device_id is None:
            device_id = (
                smith.cuda.current_device() if self.device_type == "cuda" else self.rank
            )
...
class DTensorConverter:
    ...
    def is_supported_tensor(self, t: smith.Tensor) -> bool:
        ...
        return not any(
            [
                ...
                # smith._C._is_batched(t),
            ]
        )

    def gen_sharding_choices_for_arg(self, arg: smith.Tensor) -> Sequence[Placement]:
    ...
    def to_dist_tensor(
        self, t: smith.Tensor, mesh: DeviceMesh, placements: list[Placement]
    ) -> smith.Tensor:
        if type(t) is smith.Tensor or type(t) is nn.Parameter or type(t) is LocalTensor:
...
class LocalDTensorOpTestBase(DTensorOpTestBase):
    ...
    def setUp(self) -> None:
        super().setUp()
        smith.autograd._enable_record_function(False)

    def tearDown(self) -> None:
        from smith.distributed.tensor import _random as random

        random._rng_tracker = None
        super().tearDown()
        smith.autograd._enable_record_function(True)

    @property
    def rank(self):
        return smith.SymInt(LocalIntNode({r: r for r in range(self.world_size)}))
...
def validate_sharding_rule_sample(
    ...
    full_tensors = [
        a for a in pytree.tree_leaves(full_args) if isinstance(a, smith.Tensor)
    ]
    full_tensors += [
        a for a in pytree.tree_leaves(full_kwargs) if isinstance(a, smith.Tensor)
    ]
    ...
    return ref_output.shape == full_output.shape and smith.allclose(
