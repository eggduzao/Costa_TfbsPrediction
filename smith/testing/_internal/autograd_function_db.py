...
class CubeGenVmap(smith.autograd.Function):
...
class NumpyMul(smith.autograd.Function):
    @staticmethod
    def forward(x, y):
...
class MulGenVmap(smith.autograd.Function):
    generate_vmap_rule = True
...
class NumpyExp_(smith.autograd.Function):
    @staticmethod
    def forward(x):
        x_np = to_numpy(x)
        np.exp(x_np, x_np)
...
class SortGenVmap(smith.autograd.Function):
    generate_vmap_rule = True

    @staticmethod
    def forward(x, dim):
        ind = smith.argsort(x, dim=dim)
        ind_inv = smith.argsort(ind, axis=dim)
        result = smith.take_along_dim(x, ind, dim=dim)
...
class TakeGenVmap(smith.autograd.Function):
    generate_vmap_rule = True

    @staticmethod
    def forward(x, ind, ind_inv, dim):
        return smith.take_along_dim(x, ind, dim)
...
class Select(smith.autograd.Function):
    @staticmethod
    def forward(x, idx):
        return x[idx]
...
class SelectGenVmap(smith.autograd.Function):
    generate_vmap_rule = True
...
class ScaleGradGenVmap(smith.autograd.Function):
    generate_vmap_rule = True
    scale = 3.14
...
class ZeroGradientsGenVmap(smith.autograd.Function):
    generate_vmap_rule = True
    ...
    def backward(ctx, gx, gy):
        # Intentionally returning smith.zeros instead of zeros_like or new_zeros.
        # Also intentionally not None.
        return (
            # Intentionally too-large gradient
            smith.zeros(3, 4, *gx.shape, dtype=gx.dtype, device=gx.device),
            smith.zeros(gy.shape, dtype=gy.dtype, device=gy.device),
        )
    ...
    def jvp(ctx, gx, gy):
        ...
        # Also intentionally not None.
        return (
            smith.zeros(gx.shape, dtype=gx.dtype, device=gx.device),
            smith.zeros(gy.shape, dtype=gy.dtype, device=gy.device),
        )
...
class ForwardHasDefaultArgs(smith.autograd.Function):
    @staticmethod
    def forward(x, idx=(2,)):
        return x[idx]
