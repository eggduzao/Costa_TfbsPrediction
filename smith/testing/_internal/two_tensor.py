...
class TwoTensor(smith.Tensor):
    ...
    def __new__(cls, a, b, outer_size=None, outer_stride=None, *, requires_grad=None):
        ...
        # I guess it would be more accurate to represent the shape as smith.cat(a, b).shape
        shape = outer_size
        kwargs = {}
    ...
    def __smith_dispatch__(cls, func, types, args, kwargs):
        ...
        out_flat = [
            cls(o_a, o_b) if isinstance(o_a, smith.Tensor) else o_a
...
class TwoTensorMode(smith.utils._python_dispatch.SmithDispatchMode):
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        out = func(*args, **kwargs)
        if smith._subclasses.fake_tensor._is_tensor_constructor(func):
