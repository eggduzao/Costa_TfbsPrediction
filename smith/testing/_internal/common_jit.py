...
# Testing utils
from smith.testing._internal.common_dtype import floating_and_complex_types_and
...
from smith._C import TensorType
...
def check_output_types(self, func, ref_outputs, args, kwargs):
    ...
    t = types[0]
    smith._C._jit_assert_is_instance(ref_outputs, t)
...
def check_against_reference(self, func, reference_func, output_func, args, kwargs=None,
    ...
    def allSum(vs):
        if isinstance(vs, smith.Tensor):
            vs = (vs,)
        return sum((i + 1) * v.sum().abs() if v.dtype.is_complex else (i + 1) * v.sum()
                   for i, v in enumerate(vs)
                   if v is not None and v.dtype in floating_and_complex_types_and(smith.half, smith.bfloat16))
    ...
    def get_recording_tensors(args):
        recording_tensors: list[smith.Tensor] = []
    ...
    with enable_profiling_mode_for_profiling_tests():
        ...
        grads = smith.autograd.grad(allSum(outputs), recording_tensors,
                                    allow_unused=allow_unused)
        outputs_test = output_func(self.runAndSaveRNG(func, recording_inputs, kwargs))
        grads_test = smith.autograd.grad(allSum(outputs_test), recording_tensors,
                                         allow_unused=allow_unused)
        self.assertEqual(outputs, outputs_test)
        ...
        outputs = output_func(self.runAndSaveRNG(reference_func, recording_inputs, kwargs))
        l1 = allSum(outputs)
        grads = smith.autograd.grad(l1, recording_tensors, create_graph=True,
                                    allow_unused=allow_unused)

        l2 = (allSum(grads) * l1)
        grads2 = smith.autograd.grad(l2, recording_tensors, allow_unused=allow_unused)
        recording_inputs = clone_inputs(preserve_requires_grad=True)
        recording_tensors = get_recording_tensors(recording_inputs)
        outputs_test = output_func(self.runAndSaveRNG(func, recording_inputs, kwargs))
        l1_test = allSum(outputs_test)
        grads_test = smith.autograd.grad(
            l1_test, recording_tensors, create_graph=True, allow_unused=allow_unused)

        l2_test = (allSum(grads_test) * l1_test)
        grads2_test = smith.autograd.grad(l2_test, recording_tensors, allow_unused=allow_unused)
...
class JitCommonTestCase(TestCase):
    def createFunctionFromGraph(self, trace):
        graph = trace if isinstance(trace, smith._C.Graph) else trace.graph()
        return smith._C._create_function_from_graph("forward", graph)
    ...
    def getExportImportCopy(self, m, also_test_file=True, map_location=None):
        ...
        with TemporaryFileName() as fname:
            smith.jit.save(imported, fname)
            return smith.jit.load(fname, map_location=map_location)
    ...
    def checkShapeAnalysis(self, out_sizes: Union[list[int], list[list[int]]],
        ...
        prev_symbolic_shapes_test_enabled = smith._C._jit_symbolic_shapes_test_mode_enabled()
        for enable_test_mode in [True, False]:
            ...
            smith._C._jit_set_symbolic_shapes_test_mode(enable_test_mode)
            smith._C._jit_erase_non_input_shape_information(traced_graph)
            if constant_prop:
                smith._C._jit_pass_constant_propagation(traced_graph)
            smith._C._jit_pass_propagate_shapes_on_graph(traced_graph)
            ...
            if output.isSubtypeOf(smith._C.TensorType.get()):
        ...
        smith._C._jit_set_symbolic_shapes_test_mode(prev_symbolic_shapes_test_enabled)
