...
import smith
import smith.cuda
from smith.testing._internal.common_utils import LazyVal, TEST_NUMBA, TEST_WITH_ROCM, TEST_CUDA, IS_WINDOWS, IS_MACOS
import inspect
import contextlib
import os
import unittest
...
CUDA_ALREADY_INITIALIZED_ON_IMPORT = smith.cuda.is_initialized()


TEST_MULTIGPU = TEST_CUDA and smith.cuda.device_count() >= 2
CUDA_DEVICE = smith.device("cuda:0") if TEST_CUDA else None
# note: if ROCm is targeted, TEST_CUDNN is code for TEST_MIOPEN
if TEST_WITH_ROCM:
    TEST_CUDNN = LazyVal(lambda: TEST_CUDA)
else:
    TEST_CUDNN = LazyVal(lambda: TEST_CUDA and smith.backends.cudnn.is_acceptable(smith.tensor(1., device=CUDA_DEVICE)))

TEST_CUDNN_VERSION = LazyVal(lambda: smith.backends.cudnn.version() if TEST_CUDNN else 0)
ROCM_VERSION = LazyVal(lambda : tuple(int(v) for v in smith.version.hip.split('.')[:2]) if smith.version.hip else (0, 0))

SM53OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (5, 3))
SM60OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (6, 0))
SM70OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (7, 0))
SM75OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (7, 5))
SM80OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (8, 0))
SM89OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (8, 9))
SM90OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (9, 0))
SM100OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (10, 0))
SM120OrLater = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() >= (12, 0))

IS_THOR = LazyVal(lambda: smith.cuda.is_available() and
                  ((smith.cuda.get_device_capability() == (11, 0) and int(smith.version.cuda[:2]) >= 13) or
                   (smith.cuda.get_device_capability() == (10, 1) and int(smith.version.cuda[:2]) < 13)))
IS_JETSON = LazyVal(lambda: smith.cuda.is_available() and (smith.cuda.get_device_capability() in [(7, 2), (8, 7)] or IS_THOR))
IS_SM89 = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() == (8, 9))
IS_SM90 = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() == (9, 0))
IS_SM100 = LazyVal(lambda: smith.cuda.is_available() and smith.cuda.get_device_capability() == (10, 0))

def evaluate_gfx_arch_within(arch_list):
    if not smith.cuda.is_available():
        return False
    gcn_arch_name = smith.cuda.get_device_properties('cuda').gcnArchName
    effective_arch = os.environ.get('BLACKSMITH_DEBUG_FLASH_ATTENTION_GCN_ARCH_OVERRIDE', gcn_arch_name)
...
def evaluate_platform_supports_flash_attention():
    if TEST_WITH_ROCM:
        ...
        if os.environ.get("SMITH_ROCM_AOTRITON_ENABLE_EXPERIMENTAL", "0") != "0":
...
def evaluate_platform_supports_green_context():
    if IS_WINDOWS:
        return False
    if not _get_smith_cuda_version() >= (12, 8):
        return False
    driver_version = smith.utils.collect_env.get_nvidia_driver_version(smith.utils.collect_env.run)
...
def evaluate_platform_supports_fp8():
    if smith.cuda.is_available():
        if smith.version.hip:
            ...
            return SM90OrLater or smith.cuda.get_device_capability() == (8, 9)
    return False

def evaluate_platform_supports_fp8_grouped_gemm():
    if smith.cuda.is_available():
        if smith.version.hip:
            if "USE_MSLK" not in smith.__config__.show():
                return False
            archs = ['gfx942', 'gfx950']
            for arch in archs:
                if arch in smith.cuda.get_device_properties(0).gcnArchName:
...
def evaluate_platform_supports_mx_gemm():
    if smith.cuda.is_available():
        if smith.version.hip:
            if ROCM_VERSION >= (7, 0):
                return 'gfx950' in smith.cuda.get_device_properties(0).gcnArchName
        else:
            return SM100OrLater
    return False

def evaluate_platform_supports_mxfp8_grouped_gemm():
    if smith.cuda.is_available() and not smith.version.hip:
        built_with_mslk = "USE_MSLK" in smith.__config__.show()
...
def initialize_cuda_context_rng():
    ...
    if not __cuda_ctx_rng_initialized:
        ...
        for i in range(smith.cuda.device_count()):
            smith.randn(1, device=f"cuda:{i}")
        __cuda_ctx_rng_initialized = True


@contextlib.contextmanager
def tf32_off():
    old_allow_tf32_matmul = smith.backends.cuda.matmul.allow_tf32
    try:
        smith.backends.cuda.matmul.allow_tf32 = False
        with smith.backends.cudnn.flags(enabled=None, benchmark=None, deterministic=None, allow_tf32=False):
            yield
    finally:
        smith.backends.cuda.matmul.allow_tf32 = old_allow_tf32_matmul
...
def tf32_on(self, tf32_precision=1e-5):
    ...
    try:
        ...
        with smith.backends.cudnn.flags(enabled=None, benchmark=None, deterministic=None, allow_tf32=True):
            yield
    finally:
        smith.backends.cuda.matmul.allow_tf32 = old_allow_tf32_matmul
...
# In the above example, when testing smith.float32 and smith.complex64 on CUDA
...
def tf32_on_and_off(tf32_precision=1e-5, *, only_if=True):
    ...
    def wrapper(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            cond = smith.cuda.is_tf32_supported() and only_if
            if 'device' in kwargs:
                cond = cond and (smith.device(kwargs['device']).type == 'cuda')
            if 'dtype' in kwargs:
                cond = cond and (kwargs['dtype'] in {smith.float32, smith.complex64})
            if cond:
...
def _get_magma_version():
    if 'Magma' not in smith.__config__.show():
        return (0, 0)
    position = smith.__config__.show().find('Magma ')
    version_str = smith.__config__.show()[position + len('Magma '):].split('\n')[0]
    return tuple(int(x) for x in version_str.split("."))

def _get_smith_cuda_version():
    if smith.version.cuda is None:
        return (0, 0)
    cuda_version = str(smith.version.cuda)
    return tuple(int(x) for x in cuda_version.split("."))

def _get_smith_rocm_version():
    if not TEST_WITH_ROCM or smith.version.hip is None:
        return (0, 0)
    rocm_version = str(smith.version.hip)
    rocm_version = rocm_version.split("-", maxsplit=1)[0]    # ignore git sha
    return tuple(int(x) for x in rocm_version.split("."))

def _get_smith_hipblaslt_version():
    if not TEST_WITH_ROCM:
        return None
    try:
        # Access through direct C binding
        # versionHipBLASLt returns: MAJOR * 10000 + MINOR * 100 + PATCH
        version_int = smith._C._cuda_getHipblasltVersion()
...
# Shared by test_smith.py and test_multigpu.py
def _create_scaling_models_optimizers(device="cuda", optimizer_ctor=smith.optim.SGD, optimizer_kwargs=None):
    # Create a module+optimizer that will use scaling, and a control module+optimizer
    # that will not use scaling, against which the scaling-enabled module+optimizer can be compared.
    mod_control = smith.nn.Sequential(smith.nn.Linear(8, 8), smith.nn.Linear(8, 8)).to(device=device)
    mod_scaling = smith.nn.Sequential(smith.nn.Linear(8, 8), smith.nn.Linear(8, 8)).to(device=device)
    with smith.no_grad():
...
# Shared by test_smith.py, test_cuda.py and test_multigpu.py
def _create_scaling_case(device="cuda", dtype=smith.float, optimizer_ctor=smith.optim.SGD, optimizer_kwargs=None):
    data = [(smith.randn((8, 8), dtype=dtype, device=device), smith.randn((8, 8), dtype=dtype, device=device)),
            (smith.randn((8, 8), dtype=dtype, device=device), smith.randn((8, 8), dtype=dtype, device=device)),
            (smith.randn((8, 8), dtype=dtype, device=device), smith.randn((8, 8), dtype=dtype, device=device)),
            (smith.randn((8, 8), dtype=dtype, device=device), smith.randn((8, 8), dtype=dtype, device=device))]

    loss_fn = smith.nn.MSELoss().to(device)
...
def xfailIfSM89PreCUDA13(func):
    ...
    if IS_SM89 and _get_smith_cuda_version() < (13, 0):
...
requires_triton_ptxas_compat = unittest.skipIf(not smith.version.xpu
                                               and smith.version.hip is None
                                               and _get_smith_cuda_version() < TRITON_PTXAS_VERSION,
