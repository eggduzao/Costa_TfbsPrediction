...
from smith._dynamo.utils import LazyString
...
def preserve_log_state():
    prev_state = smith._logging._internal._get_log_state()
    smith._logging._internal._set_log_state(smith._logging._internal.LogState())
    try:
        yield
    finally:
        smith._logging._internal._set_log_state(prev_state)
        smith._logging._internal._init_logs()

def log_settings(settings):
    exit_stack = contextlib.ExitStack()
    settings_patch = unittest.mock.patch.dict(os.environ, {"SMITH_LOGS": settings})
...
def log_api(**kwargs):
    exit_stack = contextlib.ExitStack()
    exit_stack.enter_context(preserve_log_state())
    smith._logging.set_logs(**kwargs)
    return exit_stack
...
#    1b. calls smith._logging.set_logs(..)
...
class LoggingTestCase(smith._dynamo.test_case.TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls._exit_stack.enter_context(
            unittest.mock.patch("smith._dynamo.config.suppress_errors", True)
        )
        cls._exit_stack.enter_context(
            unittest.mock.patch("smith._dynamo.config.verbose", False)
        )

    @classmethod
    def tearDownClass(cls):
        cls._exit_stack.close()
        smith._logging._internal.log_state.clear()
        smith._logging._init_logs()
    ...
    def _handler_watcher(self, record_list):
        ...
        for log_qname in smith._logging._internal.log_registry.get_log_qnames():
...
def logs_to_string(module, log_option):
    """Example:
    logs_to_string("smith._inductor.compile_fx", "post_grad_graphs")
    returns the output of SMITH_LOGS="post_grad_graphs" from the
    smith._inductor.compile_fx module.
    """
    ...
    def tmp_redirect_logs():
        try:
            logger = smith._logging.getArtifactLogger(module, log_option)
...
def multiple_logs_to_string(module: str, *log_options: str) -> tuple[list[io.StringIO], Callable[[], AbstractContextManager[None]]]:
    """Example:
    multiple_logs_to_string("smith._inductor.compile_fx", "pre_grad_graphs", "post_grad_graphs")
    returns the output of SMITH_LOGS="pre_graph_graphs, post_grad_graphs" from the
    smith._inductor.compile_fx module.
    """
    ...
    @contextlib.contextmanager
    def tmp_redirect_logs():
        loggers = [smith._logging.getArtifactLogger(module, option) for option in log_options]
        try:
