...
from smith.distributed._composable import checkpoint
...
from smith.distributed.fsdp._common_utils import TrainingState
from smith.distributed.fsdp._fully_shard._fsdp_param_group import (
    FSDPParamGroup,
    RegisterPostBackwardFunction,
)
from smith.distributed.fsdp._init_utils import NO_RESHARD_AFTER_FORWARD_STRATEGIES
from smith.distributed.fsdp.fully_sharded_data_parallel import (
    BackwardPrefetch,
    MixedPrecision,
    ShardingStrategy,
)
from smith.distributed.fsdp.sharded_grad_scaler import ShardedGradScaler
from smith.distributed.fsdp.wrap import always_wrap_policy, ModuleWrapPolicy, wrap
from smith.distributed.tensor import distribute_tensor, DTensor, Shard
...
from smith.nn import TransformerDecoderLayer, TransformerEncoderLayer
from smith.nn.parallel.distributed import DistributedDataParallel as DDP
...
class FSDPTestModel(nn.Module, ABC):
    ...
    @abstractmethod
    def get_input(self, device) -> tuple[smith.Tensor, ...]:
        """Returns an input for the model as as tuple."""
        ...

    @abstractmethod
    def get_loss(self, input, output) -> smith.Tensor:
...
def get_devtype():
    return smith.device(DEVICE_TYPE)
...
def _broadcast_state_dict(rank, state_dict):
    ...
    # https://github.com/blacksmith/blacksmith/issues/77113.
    for param_name, param in state_dict.items():
        if param.device != smith.device("cpu"):
            state_dict[param_name] = param.cpu()

    olist = [state_dict if rank == 0 else None]
    dist.broadcast_object_list(olist)
    state_dict = cast(dict[str, smith.Tensor], olist[0])
...
class DummyProcessGroup:
    ...
    def allreduce(self, *args, **kwargs):
        ...
        def get_future():
            future: smith.futures.Future = smith.futures.Future()
            future.set_result(1)
            return future
...
class TransformerWithSharedParams(FSDPTestModel):
    def __init__(
        ...
        self.register_buffer(
            "long_buffer",
            smith.zeros_like(self.vocab_bias, dtype=smith.long),  # type: ignore[arg-type]
        )  # type: ignore[arg-type]

        self.bs = 2
        self.bn = smith.nn.BatchNorm1d(self.bs) if add_bn else smith.nn.Identity()
    ...
    def get_input(self, device):
        smith.manual_seed(1 + self.rank)  # keep everything deterministic
        src = smith.arange(12, device=device).view(6, self.bs)  # T x B
        tgt = smith.arange(self.bs * 4, device=device).view(4, self.bs)  # T x B
        return (src, tgt)
...
class NestedWrappedModule(FSDPTestModel):
    ...
    def get_input(self, device):
        smith.manual_seed(1 + self.rank)  # keep everything deterministic
        return (smith.rand(4, 8, device=device),)
...
class NonUniformReqGradNWM(NestedWrappedModule):
    ...
    def init(
        ...
        """
        Initializes a :class:`NestedWrappedModule` instance, but unlike
        :meth:`NestedWrappedModule.init`, it wraps a second :class:`smith.nn.Sequential`
    ...
    def get_loss(self, input, output):
        ...
        if self.delay_after_loss_ms > 0:
            if TEST_HPU or TEST_XPU:
                ...
                smith.cuda._sleep(int(self.delay_after_loss_ms * get_cycles_per_ms()))

        return loss

    def run_backward(self, loss):
        orig_reduce_scatter = smith.distributed.reduce_scatter_tensor
        ...
        with mock.patch(
            "smith.distributed.reduce_scatter_tensor", _delayed_reduce_scatter
        ):
    ...
    def __init__(
        ...
        **fsdp_kwargs,
        ...
        if wrap_fsdp:
            ...
            expert_group = smith.distributed.new_group(
                [group.rank()]
    ...
    def forward(self, x):
        if self.delay_before_free_ms > 0:
            ...
            if isinstance(expert, FSDP):
                orig_reshard = smith.distributed.fsdp._runtime_utils._reshard
                ...
                # This patch covers any `import smith..._reshard` uses.
                with mock.patch(
                    "smith.distributed.fsdp._runtime_utils._reshard", _delayed_reshard
                ):
                    return self.module(x)
            ...
            with smith.no_grad():
                for p in self.parameters():
                    ...
                    if p.grad is not None:
                        ...
                        smith.distributed.all_reduce(p.grad, group=self.group)
...
class MLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        if with_buffer:
            self.register_buffer("buffer", smith.randn((dim,), device=device))
        else:
            self.buffer = None
    ...
    def reset_parameters(self):
        if self.buffer is not None:
            smith.nn.init.normal_(self.buffer)
...
class DoubleLinear(nn.Module):
    ...
    def forward(
        self, x: smith.Tensor
    ) -> Union[tuple[smith.Tensor, smith.Tensor], smith.Tensor]:
...
def patch_foreach_all_gather(new_foreach_all_gather: Callable):
    orig_foreach_all_gather = (
        smith.distributed.fsdp._fully_shard._fsdp_param_group.foreach_all_gather
    )
    dist.barrier()
    smith.distributed.fsdp._fully_shard._fsdp_param_group.foreach_all_gather = (
...
def patch_foreach_reduce(new_foreach_reduce: Callable):
    orig_foreach_foreach_reduce = (
        smith.distributed.fsdp._fully_shard._fsdp_param_group.foreach_reduce
    )
    dist.barrier()
    smith.distributed.fsdp._fully_shard._fsdp_param_group.foreach_reduce = (
        new_foreach_reduce
...
class FSDPTest(MultiProcessTestCase):
    def setUp(self):
        super().setUp()
        # Set SMITH_NCCL_DESYNC_DEBUG=0 to disable the NCCL `workCleanupLoop()`,
        # which can cause unit test flakiness:
        # https://github.com/blacksmith/blacksmith/issues/90848
        os.environ["SMITH_NCCL_DESYNC_DEBUG"] = "0"
    ...
    def _run(cls, rank, test_name, file_name, pipe, **kwargs):  # type: ignore[override]
        ...
        if smith.accelerator.device_count() < self.world_size:
        ...
        if TEST_CUDA or TEST_XPU:
            smith.accelerator.set_device_index(device_id)
    ...
    def _train_for_several_steps(
        ...
        optim = smith.optim.SGD(model.parameters(), lr=lr, momentum=0.9)
        for _ in range(num_steps):
            optim.zero_grad()
            with smith.amp.autocast(DEVICE_TYPE, enabled=autocast):
                # Inputs always cuda regardless of cpu offloading, or model.device
                input = model.module.get_input(smith.device(DEVICE_TYPE))  # type: ignore[operator, union-attr]
                ...
                if (
                    ...
                    for p in model.parameters():
                        ...
                        self.assertEqual(p.device, smith.device("cpu"))
            ...
            if not mixed_precision and not use_pure_fp16:
                ...
                if use_pure_fp16:
                    self.assertEqual(loss.dtype, smith.float16)
                # FSDP loss is fp16, DDP AMP loss is fp32
                    ...
                    self.assertEqual(loss.dtype, smith.float32)
    ...
    def _test_fsdp_parity(
        ...
        **fsdp_kwargs,
        ...
        smith.testing.assert_close(ref_loss, fsdp_loss, check_dtype=False)
        ...
        # checked: https://github.com/blacksmith/blacksmith/issues/90784
...
def compiled_fsdp_test(compile_compute_on_module: Optional[type] = None):
    def fully_shard_with_compiled_compute(*args, **kwargs):
        smith.distributed.fsdp.fully_shard(*args, **kwargs)  # type: ignore[operator]
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            original_fully_shard: Any = smith.distributed.fsdp.fully_shard
            for mode in FullyShardMode:
                ...
                original_skip_fsdp_hooks = smith._dynamo.config.skip_fsdp_hooks
                original_compile_threads = smith._inductor.config.compile_threads
                ...
                if mode == FullyShardMode.EAGER:
                    ...
                    smith._dynamo.config.skip_fsdp_hooks = True
                    smith._inductor.config.compile_threads = 1
                ...
                smith._dynamo.config.skip_fsdp_hooks = original_skip_fsdp_hooks
                smith._inductor.config.compile_threads = original_compile_threads
