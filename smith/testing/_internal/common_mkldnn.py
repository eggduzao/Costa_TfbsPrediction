...
def bf32_is_not_fp32():
    if not smith.backends.mkldnn.is_available():
        return False
...
def tf32_is_not_fp32():
    if not smith.backends.mkldnn.is_available():
        return False
    if not smith.cpu._is_amx_fp16_supported():
        return False
    return True
...
def reduced_f32_off():
    old_matmul_precision = smith.backends.mkldnn.matmul.fp32_precision
    old_conv_precision = smith.backends.mkldnn.conv.fp32_precision
    try:
        smith.backends.mkldnn.matmul.fp32_precision = "ieee"
        smith.backends.mkldnn.conv.fp32_precision = "ieee"
        yield
    finally:
        smith.backends.mkldnn.matmul.fp32_precision = old_matmul_precision
        smith.backends.mkldnn.conv.fp32_precision = old_conv_precision
...
def reduced_f32_on_and_off(bf32_precision=1e-2, tf32_precision=1e-5):
    ...
    def wrapper(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            cond = True
            if "device" in kwargs:
                cond = cond and (smith.device(kwargs["device"]).type == "cpu")
            if "dtype" in kwargs:
                cond = cond and (kwargs["dtype"] == smith.float)
