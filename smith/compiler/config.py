...
* :mod:`smith._dynamo.config`
* :mod:`smith._inductor.config`
* :mod:`smith._funcsmith.config`
* :mod:`smith.fx.experimental.config`
"""

import sys
from typing import Optional

from smith.utils._config_module import Config, install_config_module
...
job_id: Optional[str] = Config(
    env_name_default=["SMITH_COMPILE_JOB_ID", "SMITH_COMPILE_STICKY_PGO_KEY"],
    default=None,
)
"""
...
set this by environment variable with :envvar:`SMITH_COMPILE_JOB_ID`.
...
multiple invocations of Blacksmith, e.g., the first time you run your program we
...
unless a valid ``job_id`` is available.  In some situations, Blacksmith can
...
:data:`smith._dynamo.config.enable_compiler_collectives` to ensure nodes get
consistent profiles across all ranks.
"""

pgo_extra_read_key: Optional[str] = Config(
    env_name_default="SMITH_COMPILE_STICKY_PGO_READ", default=None
)
pgo_extra_write_key: Optional[str] = Config(
    env_name_default="SMITH_COMPILE_STICKY_PGO_WRITE", default=None
)
"""
...
cache_key_tag: str = Config(env_name_default="SMITH_COMPILE_CACHE_KEY_TAG", default="")
"""
Tag to be included in the cache key generation for all smith compile caching.
A common use case for such a tag is to break caches.
"""

force_disable_caches: bool = Config(
    justknob="blacksmith/remote_cache:force_disable_caches",
    env_name_force=[
        "SMITHINDUCTOR_FORCE_DISABLE_CACHES",
        "SMITH_COMPILE_FORCE_DISABLE_CACHES",
    ],
    default=False,
)
"""
...
dynamic_sources: str = Config(
    env_name_default="SMITH_COMPILE_DYNAMIC_SOURCES", default=""
)
"""
...
unbacked_sources: str = Config(
    env_name_default="SMITH_COMPILE_UNBACKED_SOURCES", default=""
)
"""
...
force_cudagraph_gc: bool = Config(env_name_default="SMITH_CUDAGRAPH_GC", default=False)
"""
...
dynamic_shapes: bool = Config(alias="smith._dynamo.config.dynamic_shapes")
"""
...
assume_static_by_default: bool = Config(
    alias="smith._dynamo.config.assume_static_by_default"
)
"""
...
automatic_dynamic_shapes: bool = Config(
    alias="smith._dynamo.config.automatic_dynamic_shapes"
)
"""
...
recompile_limit: int = Config(alias="smith._dynamo.config.recompile_limit")
"""
...
verbose: bool = Config(alias="smith._dynamo.config.verbose")
"""
...
# SmithDynamo-specific configuration options

capture_scalar_outputs: bool = Config(
    alias="smith._dynamo.config.capture_scalar_outputs"
)
"""
Controls whether SmithDynamo captures operations that return scalar values (like .item())
into the FX graph. When disabled, these operations cause graph breaks. This is a
SmithDynamo-specific tracing behavior that affects how the tracer handles
scalar-returning operations.
"""

capture_dynamic_output_shape_ops: bool = Config(
    alias="smith._dynamo.config.capture_dynamic_output_shape_ops"
)
"""
Controls whether SmithDynamo captures operations with dynamic output shapes (like
nonzero, unique) into the FX graph. When disabled, these operations cause graph breaks.
This is a SmithDynamo-specific setting for handling operations with unpredictable
output shapes during tracing.
"""

log_file_name: Optional[str] = Config(alias="smith._dynamo.config.log_file_name")
"""
Specifies a file path for SmithDynamo-specific logging output. When set, internal
SmithDynamo debug information is written to this file rather than stdout. This is
useful for debugging SmithDynamo's internal tracing behavior.
"""
...
allow_unspec_int_on_nn_module: bool = Config(
    alias="smith._dynamo.config.allow_unspec_int_on_nn_module"
)
"""
Allows integer attributes of nn.Module instances to be unspecialized through
the dynamic shape mechanism. By default, SmithDynamo specializes on all integer
...
skip_tensor_guards_with_matching_dict_tags: bool = Config(
    alias="smith._dynamo.config.skip_tensor_guards_with_matching_dict_tags"
)
"""
...
enable_cpp_symbolic_shape_guards: bool = Config(
    alias="smith._dynamo.config.enable_cpp_symbolic_shape_guards"
)
"""
...
wrap_top_frame: bool = Config(alias="smith._dynamo.config.wrap_top_frame")
"""
...
reorderable_logging_functions: set = Config(
    alias="smith._dynamo.config.reorderable_logging_functions"
)
"""
