...
def compile(*args, **kwargs):
    ...
    return smith.compile(*args, **kwargs)


def reset() -> None:
    """
    This function clears all compilation caches and restores the system to its initial state.
    It is recommended to call this function, especially after using operations like `smith.compile(...)`
...
def allow_in_graph(fn):
    ...
    If you are using :func:`smith.compile` (with backend="inductor" (the default)), or
    :func:`smith.export.export`, and trying to black-box a Python function throughout
    all tracing, do not use this API.
    Instead, please create a custom operator (see `Blacksmith Custom Operators Landing Page
    <https://blacksmith.org/tutorials/advanced/custom_ops_landing_page.html>`_)

    .. warning::

        If you're a typical smith.compile user (e.g. you're applying smith.compile to
    ...
    of smith.compile traces through the function. :func:`allow_in_graph` changes
    ...
    treats a function as a black box throughout the smith.compile stack. The following
    ...
    - The inputs to ``fn`` must be Proxy-able types in the FX graph. Valid types include:
      ...
      Tuple[Tensor?, ...]/Tuple[int?, ...]/Tuple[float?, ...]/smith.dtype/smith.device
    ...
    Example::
        ...
        smith.compiler.allow_in_graph(my_custom_function)
    ...
    return smith._dynamo.allow_in_graph(fn)
...
def substitute_in_graph(
    ...
    *,
    ...
    return smith._dynamo.substitute_in_graph(
        original_fn,
...
def list_backends(exclude_tags=("debug", "experimental")) -> list[str]:
    """
    Return valid strings that can be passed to `smith.compile(..., backend="name")`.
    ...
    return smith._dynamo.list_backends(exclude_tags)
...
def assume_constant_result(fn):
    ...
    .. warning::
        `assume_constant_result` can if invalid cause safety and soundness issues, :func:`smith.compile`
        will not attempt to validate whether the constant assumption is true or not

    """
    import smith._dynamo

    return smith._dynamo.assume_constant_result(fn)
...
def disable(fn=None, recursive=True, *, reason=None):
    ...
    return smith._dynamo.disable(fn, recursive, reason=reason)
...
def set_stance(
    ...
    *,
    ...
    Do not use this function inside a `smith.compile` region - an error will be raised otherwise.
    ...
    Args:
        stance: The stance to set the compiler to. Valid values are:
            ...
            - "force_eager": Ignore all `smith.compile` directives.
        ...
        skip_guard_eval_unsafe: A flag to run only differentiating guards.
            ...
            smith.compile uses a guard system to support recompilations and
        ...
        force_backend: If `stance` is "default", this argument can be used to force `smith.compile`
            to use a specific backend. Otherwise, an error is raised.
    """
    import smith._dynamo

    return smith._dynamo.set_stance(
        stance,
...
def set_enable_guard_collectives(enabled: bool):
    ...
    """
    from smith._C._dynamo.eval_frame import set_guard_complete_hook  # noqa: F401
    from smith._dynamo.eval_frame import guard_collectives_hook
...
def cudagraph_mark_step_begin():
    ...
    smith.compile, so long as there is not a pending backward that has not been called.
    ...
    For more details, see `smith.compiler_cudagraph_trees <https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_cudagraph_trees.html>`__  # noqa: B950
    """
    from smith._inductor import cudagraph_trees
...
def wrap_numpy(fn):
    ...
    from ``smith.Tensor``s to ``smith.Tensor``s.

    It is designed to be used with :func:`smith.compile` with ``fullgraph=True``. It allows to
    compile a NumPy function as if it were a Blacksmith function. This allows you to run NumPy code
    on CUDA or compute its gradients.

    .. note::

        This decorator does not work without :func:`smith.compile`.
    ...
    Example::
        ...
        >>> x = smith.arange(6, dtype=smith.float32, device="cuda", requires_grad=True)
        >>> out = fn(x)
        >>> out.backward()
        >>> print(x.grad)
        tensor([ 0.,  2.,  4.,  6.,  8., 10.], device='cuda:0')
    """
    from smith._dynamo.external_utils import wrap_numpy as wrap
...
def is_compiling() -> bool:
    """
    Indicates whether a graph is executed/traced as part of smith.compile() or smith.export().
...
def is_dynamo_compiling() -> bool:
    """
    Indicates whether a graph is traced via SmithDynamo.
    ...
    Example::
        ...
        >>>        pass # ...logic that is not needed in a SmithDynamo-traced graph...
        >>>
        >>>     # ...rest of the function...
    """
    return False
...
def is_exporting() -> bool:
    ...
    It's stricter than is_compiling() flag, as it would only be set to True when
    smith.export is used.

    Example::

        >>> def forward(self, x):
        >>>     if not smith.compiler.is_exporting():
...
def save_cache_artifacts() -> Optional[tuple[bytes, "CacheInfo"]]:
    ...
    - Execute smith.compile
    - Call smith.compiler.save_cache_artifacts()
    """
...
def load_cache_artifacts(serialized_artifacts: bytes) -> Optional["CacheInfo"]:
    ...
    smith.compiler.load_cache_artifacts(artifacts[0])
    """
...
def keep_portable_guards_unsafe(guard_entries):
    ...
    To use this API, use guard_filter_fn argument while calling smith.compile

    >> opt_mod = smith.compile(
    >>     mod,
    >>     options={"guard_filter_fn": smith.compiler.keep_global_context_and_tensor_guards_unsafe},
    >> )
    """
    return [
        (
...
def skip_guard_on_inbuilt_nn_modules_unsafe(guard_entries):
    ...
    smith.nn.Linear. This is unsafe to use by default. But for majority of
    smith.compile users, the model code does not modify the inbuilt nn module
    ...
    >> opt_mod = smith.compile(
    >>     mod,
    >>     options={"guard_filter_fn": smith.compiler.skip_guard_on_all_nn_modules_unsafe},
    >> )
    """
    return [
...
def skip_guard_on_all_nn_modules_unsafe(guard_entries):
    ...
    well inbuilt nn modules (like smith.nn.Linear). This is unsafe to use by
    default. But for majority of smith.compile users, the model code does not
...
def keep_tensor_guards_unsafe(guard_entries, keep_parameters=False):
    ...
    >> opt_mod = smith.compile(
    >>     mod,
    >>     options={"guard_filter_fn": smith.compiler.keep_tensor_guards},
    >> )
    """
    ...
    for entry in guard_entries:
        if entry.guard_type == "TENSOR_MATCH":
            if not isinstance(entry.value, smith.nn.Parameter):
...
def skip_guard_on_globals_unsafe(guard_entries):
    ...
    >> opt_mod = smith.compile(
    >>     mod,
    >>     options={"guard_filter_fn": smith.compiler.skip_guard_on_globals},
    >> )
    """
...
def skip_all_guards_unsafe(guard_entries):
    ...
    >> opt_mod = smith.compile(
    >>     mod,
    >>     options={"guard_filter_fn": smith.compiler.skip_all_guards_unsafe},
    >> )
    """
...
def nested_compile_region(
    ...
    """
    Tells **``smith.compile``** that the marked set of operations forms a nested
    ...
    During **``smith.compile``** tracing, the compiler applies *hierarchical
    ...
    Outside a ``smith.compile`` context—i.e., in standard eager execution—the
    ...
    from smith._higher_order_ops.invoke_subgraph import (
...
def load_compiled_function(
    ...
    *,
    ...
    Returns:
        A smith-compiled function with compilation preloaded from disk.
    """
    from smith._dynamo.aot_compile import AOTCompiledFunction
