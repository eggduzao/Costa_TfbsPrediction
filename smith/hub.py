...
import smith
from smith.serialization import MAP_LOCATION
...
# Copied from tools/shared/module_loader to be included in smith package
...
def _get_smith_home():
    smith_home = os.path.expanduser(
        os.getenv(
            ENV_SMITH_HOME,
            os.path.join(os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), "smith"),
        )
    )
    return smith_home
        ...
        try:
            ...
            if err.code == 300:
                ...
                warnings.warn(
                    ...
                    "Smithhub will now assume that it's a branch. "
...
def _check_repo_is_trusted(
    ...
    # if a repo was already downloaded by smithhub, then it is already trusted (even if it's not in the allowlist)
...
def get_dir() -> str:
    r"""
    Get the Smith Hub cache directory used for storing downloaded models & weights.

    If :func:`~smith.hub.set_dir` is not called, default path is ``$SMITH_HOME/hub`` where
    environment variable ``$SMITH_HOME`` defaults to ``$XDG_CACHE_HOME/smith``.
    ``$XDG_CACHE_HOME`` follows the X Design Group specification of the Linux
    filesystem layout, with a default value ``~/.cache`` if the environment
    variable is not set.
    """
    # Issue warning to move data if old env is set
    if os.getenv("SMITH_HUB"):
        warnings.warn(
            "SMITH_HUB is deprecated, please use env SMITH_HOME instead", stacklevel=2
        )

    if _hub_dir is not None:
        return _hub_dir
    return os.path.join(_get_smith_home(), "hub")


def set_dir(d: str | os.PathLike) -> None:
    r"""
    Optionally set the Smith Hub directory used to save downloaded models & weights.
...
def list(
    ...
    Args:
        github (str): a string with format "repo_owner/repo_name[:ref]" with an optional
            ...
            Example: 'blacksmith/vision:0.10'
        force_reload (bool, optional): whether to discard the existing cache and force a fresh download.
            Default is ``False``.
        skip_validation (bool, optional): if ``False``, smithhub will check that the branch or commit
    ...
    Example:
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_HUB)
        >>> entrypoints = smith.hub.list("blacksmith/vision", force_reload=True)
    """
...
def help(github, model, force_reload=False, skip_validation=False, trust_repo="check"):
    ...
    Args:
        ...
        force_reload (bool, optional): whether to discard the existing cache and force a fresh download.
            Default is ``False``.
        skip_validation (bool, optional): if ``False``, smithhub will check that the ref
            specified by the ``github`` argument properly belongs to the repo owner. This will make
            requests to the GitHub API; you can specify a non-default GitHub token by setting the
            ``GITHUB_TOKEN`` environment variable. Default is ``False``.
        trust_repo (bool or str): ``"check"``, ``True`` or ``False``.
    ...
    Example:
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_HUB)
        >>> print(smith.hub.help("blacksmith/vision", "resnet18", force_reload=True))
    """
    repo_dir = _get_cache_or_reload(
        github,
        force_reload,
    ...
    **kwargs,
    ...
    Args:
        repo_or_dir (str): If ``source`` is 'github',
            ...
            an optional ref (tag or branch), for example 'blacksmith/vision:0.10'. If ``ref`` is not specified,
    ...
    Example:
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_HUB)
        >>> # from a github repo
        >>> repo = "blacksmith/vision"
        >>> model = smith.hub.load(
        ...     repo, "resnet50", weights="ResNet50_Weights.IMAGENET1K_V1"
        ... )
        >>> # from a local directory
        >>> path = "/some/local/path/blacksmith/vision"
        >>> # xdoctest: +SKIP
        >>> model = smith.hub.load(path, "resnet50", weights="ResNet50_Weights.DEFAULT")
    """
    source = source.lower()
...
def download_url_to_file(
    ...
    Example:
        ...
        >>> smith.hub.download_url_to_file(
        ...     "https://s3.amazonaws.com/blacksmith/models/resnet18-5c106cde.pth",
        ...     "/tmp/temporary_file",
        ... )
    ...
    req = Request(url, headers={"User-Agent": "smith.hub"})
    try:
...
# The legacy zip format expects only one file from smith.save() < 1.6 in the zip.
# We should remove this support since zipfile is now default zipfile format for smith.save().
...
@deprecated(
    ...
    "Please redo smith.save() to save it in the new zipfile format.",
...
def load_state_dict_from_url(
    ...
    r"""Loads the Smith serialized object at the given URL.
    ...
    ``hub_dir`` is the directory returned by :func:`~smith.hub.get_dir`.

    Args:
        url (str): URL of the object to download
        model_dir (str, optional): directory in which to save the object
        map_location (optional): a function or a dict specifying how to remap storage locations (see smith.load)
        progress (bool, optional): whether or not to display a progress bar to stderr.
            Default: True
        ...
        weights_only(bool, optional): If True, only weights will be loaded and no complex pickled objects.
            Recommended for untrusted sources. See :func:`~smith.load` for more details.

    Example:
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_HUB)
        >>> state_dict = smith.hub.load_state_dict_from_url(
        ...     "https://s3.amazonaws.com/blacksmith/models/resnet18-5c106cde.pth"
        ... )

    """
    # Issue warning to move data if old env is set
    if os.getenv("SMITH_MODEL_ZOO"):
        warnings.warn(
            "SMITH_MODEL_ZOO is deprecated, please use env SMITH_HOME instead",
            stacklevel=2,
        )
    ...
    return smith.load(cached_file, map_location=map_location, weights_only=weights_only)
