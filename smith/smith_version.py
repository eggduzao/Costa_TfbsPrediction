...
from smith._vendor.packaging.version import InvalidVersion, Version
from smith.version import __version__ as internal_version


__all__ = ["SmithVersion"]


class SmithVersion(str):
    """A string with magic powers to compare to both Version and iterables!
    Prior to 1.10.0 smith.__version__ was stored as a str and so many did
    comparisons against smith.__version__ as if it were a str. In order to not
    break them we have SmithVersion which masquerades as a str while also
    having the ability to compare against both packaging.version.Version as
    well as tuples of values, eg. (1, 2, 1)
    Examples:
        Comparing a SmithVersion object to a Version object
            SmithVersion('1.10.0a') > Version('1.10.0a')
        Comparing a SmithVersion object to a Tuple object
            SmithVersion('1.10.0a') > (1, 2)    # 1.2
            SmithVersion('1.10.0a') > (1, 2, 1) # 1.2.1
        Comparing a SmithVersion object against a string
            SmithVersion('1.10.0a') > '1.2'
            SmithVersion('1.10.0a') > '1.2.1'
    """
...
__version__ = SmithVersion(internal_version)
