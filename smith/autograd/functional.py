...
import smith
from smith._vmap_internals import _vmap
...
def _as_tuple(inp, arg_name=None, fn_name=None):
    ...
    for i, el in enumerate(inp):
        if not isinstance(el, smith.Tensor):
            if is_inp_tuple:
                raise TypeError(
...
def _grad_preprocess(inputs, create_graph, need_graph):
    ...
    # Check this issue for more details on how that can happen: https://github.com/blacksmith/blacksmith/issues/32576
    res = []
    for inp in inputs:
...
def _grad_postprocess(inputs, create_graph):
    ...
    # request it.
    if isinstance(inputs[0], smith.Tensor):
        if not create_graph:
...
def _autograd_grad(
    ...
    new_outputs: tuple[smith.Tensor, ...] = ()
    new_grad_outputs: tuple[smith.Tensor, ...] = ()
...
def _fill_in_zeros(grads, refs, strict, create_graph, stage):
    ...
    for i, grads_i in enumerate(grads):
        if grads_i is None:
            ...
            grads_i = smith.zeros_like(refs[i])
        else:
...
def vjp(func, inputs, v=None, create_graph=False, strict=False):
    ...
    Example:
        ...
        >>> def adder(x, y):
        ...     return 2 * x + 3 * y
        >>> inputs = (smith.rand(2), smith.rand(2))
        >>> v = smith.ones(2)
        >>> vjp(adder, inputs, v)
    ...
    enable_grad = True if create_graph else smith.is_grad_enabled()
    with smith.set_grad_enabled(enable_grad):
...
def jvp(func, inputs, v=None, create_graph=False, strict=False):
    ...
    with smith.enable_grad():
        ...
        grad_outputs = tuple(
            smith.zeros_like(out, requires_grad=True) for out in outputs
        )
...
def _construct_standard_basis_for(
    tensors: tuple[smith.Tensor, ...], tensor_numels: tuple[int, ...]
) -> tuple[smith.Tensor, ...]:
    # This function:
    ...
    # in smith.autograd.functional.jacobian.
...
def _jacfwd(func, inputs, strict=False, vectorize=False):
    if strict:
        raise RuntimeError(
            "smith.autograd.functional.jacobian: `strict=True` "
            ...
            with fwAD.dual_level():
                ...
                for dual_out in dual_outputs:
                    ...
                    if tangent is not None:
                        ...
                        jv.append(smith.zeros_like(primal))
...
def jacobian(
    ...
    Args:
        ...
        vectorize (bool, optional): This feature is experimental.
            Please consider using :func:`smith.func.jacrev` or
            :func:`smith.func.jacfwd` instead if you are looking for something
            ...
            cliffs. See :func:`smith.autograd.grad`'s ``batched_grad`` parameter for
            more information.
    ...
    Example:
        ...
        >>> def linear_model(x):
        ...     W = smith.tensor([[2.0, -1.0], [0.0, 1.0]])
        ...     b = smith.tensor([1.0, 0.5])
        ...     return x @ W.T + b
    ...
    if strategy == "forward-mode":
        if create_graph:
            raise NotImplementedError(
                "smith.autograd.functional.jacobian: `create_graph=True` "
                'and `strategy="forward-mode"` are not supported together (yet). '
    ...
    with smith.enable_grad():
            ...
            def vjp(grad_output):
                ...
                for el_idx, vj_el in enumerate(vj):
                    if vj_el is not None:
                        continue
                    vj[el_idx] = smith.zeros_like(inputs[el_idx]).expand(
        ...
        jacobian: tuple[smith.Tensor, ...] = ()

        for i, out in enumerate(outputs):
            # mypy complains that expression and variable have different types due to the empty list
            jac_i: tuple[list[smith.Tensor]] = tuple([] for _ in range(len(inputs)))  # type: ignore[assignment]
                ...
                for el_idx, (jac_i_el, vj_el, inp_el) in enumerate(
                    ...
                    if vj_el is not None:
                        ...
                        if strict:
                            ...
                            raise RuntimeError(msg)
                        jac_i_el.append(smith.zeros_like(inp_el))

            # pyrefly: ignore [bad-assignment]
            jacobian += (
                tuple(
                    smith.stack(jac_i_el, dim=0).view(
...
def hessian(
    ...
    Args:
        ...
        strict (bool, optional): If ``True``, an error will be raised when we detect that there exists an input
            ...
            Defaults to ``False``.
        vectorize (bool, optional): This feature is experimental.
            Please consider using :func:`smith.func.hessian`
            ...
            use `smith._C._debug_only_display_vmap_fallback_warnings(True)`
    ...
    def ensure_single_output_function(*inp):
        ...
        if is_out_tuple or not isinstance(out, smith.Tensor):
            raise RuntimeError(
...
def vhp(func, inputs, v=None, create_graph=False, strict=False):
    ...
    with smith.enable_grad():
        ...
        if is_outputs_tuple or not isinstance(outputs[0], smith.Tensor):
            raise RuntimeError(
...
def hvp(func, inputs, v=None, create_graph=False, strict=False):
    ...
    with smith.enable_grad():
        ...
        grad_jac = tuple(smith.zeros_like(inp, requires_grad=True) for inp in inputs)
