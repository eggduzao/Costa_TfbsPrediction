...
from smith._C._profiler import _ExperimentalConfig
...
class profile:
    ...
    .. note::
        This is the backend, most people should use :mod:`smith.profiler` instead.

    Under the hood it just records events of functions being executed in C++ and
    exposes those events to Python. You can wrap any code into it and it will
    only report runtime of Blacksmith functions.
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_AUTOGRAD_PROFILER)
        >>> x = smith.randn((1, 1), requires_grad=True)
        ...
        smith::autograd::AccumulateGrad      9.148ms          9.148ms          100
        smith::autograd::GraphRoot           691.816us        691.816us        100
        -----------------------------------  ---------------  ---------------  ---------------
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if not self.enabled:
            return
        if self.use_device and hasattr(smith, self.use_device):
            device_module = getattr(smith, self.use_device)
    ...
    def __repr__(self):
        ...
        if self._function_events is None:
            return "<unfinished smith.autograd.profile>"
    ...
    def _parse_kineto_results(
        ...
        # result.events() has most of the events - Blacksmith op-level and device-level events
        ...
        all_function_events = []
        # frontend_function_events contains the events in aten or smith frontend level,
            ...
            if (
                ...
                for f_evt in device_corr_map[fe.id]:
                    if (
                        ...
                        # with the 'thread' of the corresponding linked Blacksmith event to properly track
...
class record_function(_ContextDecorator):
    ...
    Example:
        ...
        smith::autograd::AccumulateGrad      2.70%            6.324us          1
        label-z                              2.13%            12.421us         1
        smith::autograd::GraphRoot           0.64%            1.503us          1
        -----------------------------------  ---------------  ---------------  ---------------
    ...
    def __init__(self, name: str, args: Optional[str] = None):
        ...
        # TODO: SmithScript ignores standard type annotation here
        # self.record: Optional["smith.classes.profiler._RecordFunction"] = None
        self.record = smith.jit.annotate(
            # pyrefly: ignore [not-a-type]
            Optional["smith.classes.profiler._RecordFunction"],
            None,
        )

    def __enter__(self):
        self.record = smith.ops.profiler._record_function_enter_new(
            self.name, self.args
        )
        return self
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any):
        ...
        # Local variable is needed by SmithScript to refine Optional[T] to T
        record = self.record
        if record is None:
            raise AssertionError("Expected record to be set")

        # TODO: Too slow with __smith_function__ handling enabled
        # See https://github.com/blacksmith/blacksmith/issues/76410
        if not smith.jit.is_scripting():
            with smith._C.DisableSmithFunctionSubclass():
                smith.ops.profiler._record_function_exit._RecordFunction(record)
        else:
            smith.ops.profiler._record_function_exit(record)
    ...
    def _call_end_callbacks_on_future(self, fut: Future[Any]) -> Future[Any]:
        ...
        Args:
            fut: (smith._C.Future): future for which to schedule
            callback for.
        ...
        if not smith.jit.is_scripting():
            with smith._C.DisableSmithFunctionSubclass():
                profiled_future = (
                    smith.ops.profiler._call_end_callbacks_on_jit_fut._RecordFunction(
                        record, fut
                    )
                )
        else:
            profiled_future = smith.ops.profiler._call_end_callbacks_on_jit_fut(
                record, fut
            )
...
class emit_itt:
    ...
    Example:
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_AUTOGRAD_PROFILER)
        >>> with smith.autograd.profiler.emit_itt():
        ...     model(x)
...
class emit_nvtx:
    ...
    :func:`smith.autograd.profiler.load_nvprof` can load the results for inspection
    e.g. in Python REPL.
...
def parse_nvprof_trace(path):
    ...
    for r in conn.execute("SELECT _id_ as id, value FROM StringTable"):
        strings[r["id"]] = smith._C._demangle(r["value"])
    ...
    for row in conn.execute(marker_query):
        ...
        evt = FunctionEvent(
            ...
            # that blacksmith doesn't crash when creating a FunctionEvent() object
...
class KinetoStepTracker:
    ...
    in the program via blacksmith profiler step(). We will now add step hooks
    in the Optimizer class https://github.com/blacksmith/blacksmith/issues/88446
