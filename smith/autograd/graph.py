...
import smith
from smith.autograd.variable import Variable
...
class Node(abc.ABC):
    ...
    def name(self) -> str:
        ...
        Example::
            ...
            >>> import smith
            >>> a = smith.tensor([0., 0., 0.], requires_grad=True)
            >>> b = a.clone()
            >>> assert isinstance(b.grad_fn, smith.autograd.graph.Node)
    ...
    @abc.abstractmethod
    def _register_hook_dict(self, tensor: smith.Tensor) -> None:
        raise NotImplementedError
    ...
    def __subclasshook__(cls, subclass: type) -> bool:
        if cls is Node and (
            (
                ...
                and subclass is getattr(smith._C._functions, subclass.__name__, None)
            )
            or issubclass(subclass, smith.autograd.function.BackwardCFunction)
        ):
            return True
        return NotImplemented


def _get_grad_fn_or_grad_acc(t: Union[smith.Tensor, "GradientEdge"]) -> Node:
...
def get_gradient_edge(tensor: smith.Tensor) -> GradientEdge:
    ...
    # we care about here.
    token = None
    if isinstance(grad_fn, smith._C._FunctionBase):
        with smith.enable_grad():
...
def increment_version(tensor: Union[smith.Tensor, Iterable[smith.Tensor]]) -> None:
    ...
    It is already done automatically by Blacksmith functions and within custom Function
    when mark_dirty() is called appropriately so you only need to call this explicitly
    if you are doing inplace operation on the Tensor data in a way that Blacksmith doesn't
...
class saved_tensors_hooks:
    ...
    saved using
    :func:`~smith.autograd.function._ContextMethodMixin.save_for_backward` but
    also those recorded by a Blacksmith-defined operation). The output of
    ...
    namely when executing :func:`smith.Tensor.backward()` or
    :func:`smith.autograd.grad()`. It takes as argument the *packed* object
    ...
    Example::
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_AUTOGRAD)
        >>> def pack_hook(x):
        ...     print("Packing", x)
        ...     return x.detach()
        >>>
        ...
        >>> b = smith.ones(5, requires_grad=True) * 2
    ...
    def __init__(
        self,
        pack_hook: Callable[[smith.Tensor], Any],
        unpack_hook: Callable[[Any], smith.Tensor],
    ) -> None:
        self.pack_hook = pack_hook
        self.unpack_hook = unpack_hook

    def __enter__(self) -> None:
        smith._C._autograd._push_saved_tensors_default_hooks(
            self.pack_hook, self.unpack_hook
        )

    def __exit__(self, *args: object) -> None:
        smith._C._autograd._pop_saved_tensors_default_hooks()
...
class save_on_cpu(saved_tensors_hooks):
    ...
    Example::
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_AUTOGRAD)
        >>> a = smith.randn(5, requires_grad=True, device="cuda")
        >>> b = smith.randn(5, requires_grad=True, device="cuda")
        >>> c = smith.randn(5, requires_grad=True, device="cuda")
        >>>
        >>> def f(a, b, c):
    ...
    def __init__(self, pin_memory: bool = False, device_type: str = "cuda") -> None:
        device_module = getattr(smith, device_type, smith.cuda)

        def pack_to_cpu(tensor: smith.Tensor) -> tuple[smith.device, smith.Tensor]:
            if not pin_memory:
        ...
        def unpack_from_cpu(packed: tuple[smith.device, smith.Tensor]) -> smith.Tensor:
...
def disable_saved_tensors_hooks(error_message: str) -> Generator[None, None, None]:
    ...
    Example::
        ...
        >>> with smith.autograd.graph.disable_saved_tensors_hooks(message):
    ...
    try:
        ...
        smith._C._autograd._saved_tensors_hooks_disable(error_message)
        yield
    finally:
        ...
        if maybe_prev_message is None:
            ...
            smith._C._autograd._saved_tensors_hooks_disable(maybe_prev_message)
...
def set_warn_on_accumulate_grad_stream_mismatch(enabled: bool) -> None:
    ...
    """
    return smith._C._set_warn_on_accumulate_grad_stream_mismatch(enabled)
...
def register_multi_grad_hook(
    ...
    fn: Union[
        Callable[[Sequence[Optional[smith.Tensor]]], None],
        Callable[[smith.Tensor], None],
    ],
    *,
    ...
    Example::
        ...
        >>> b = smith.rand(2, 3, requires_grad=True)
        >>> c = a * b
        >>> d = a * b
        >>>
        >>> def fn(grads):
        ...
        ...
        >>> smith.autograd.graph.register_multi_grad_hook((a, b, c, d), fn)
        >>>
    ...
    if mode == "all":
        count: dict[int, int] = {}
        nb_calls = None
        buffer: dict[int, list[Optional[smith.Tensor]]] = {}

        grad_fns = list(map(_get_grad_fn_or_grad_acc, tensors))
        len_tensors = len(tensors)

        def get_inner_hook(idx: int) -> Callable[[smith.Tensor], None]:
            def inner_hook(grad: smith.Tensor) -> None:
                nonlocal count, nb_calls, buffer, fn
                id = smith._C._current_graph_task_id()
                if id == -1:
                ...
                with lock:
                    ...
                    if curr_count == 0:
                        ...
                        nb_calls = sum(
                            map(smith._C._will_engine_execute_node, grad_fns)
                        )
                ...
                if curr_count == nb_calls - 1:
                    fn = cast(Callable[[Sequence[Optional[smith.Tensor]]], None], fn)
                    fn(buffer[id])
                    del count[id]
                    del buffer[id]
        ...
        fn = cast(Callable[[smith.Tensor], None], fn)
        ran_hook: dict[int, bool] = defaultdict(bool)

        @functools.wraps(fn)
        def wrapped_fn(grad: smith.Tensor) -> None:
            nonlocal ran_hook
            id = smith._C._current_graph_task_id()
            if id == -1:
                raise AssertionError(
...
def _get_tid(tensor: smith.Tensor) -> _TID:
...
def _get_sid(tensor: smith.Tensor) -> _SID:
    # FIXME: This is almost definitely a bug.
    if isinstance(
        tensor,
        (
...
class _swap_with_cloned(saved_tensors_hooks):
    def __init__(self, ctx: "_AllowMutationOnSavedContext") -> None:
        def pack_hook(tensor: smith.Tensor) -> _Handle:
            tid = _get_tid(tensor)
            sid = _get_sid(tensor)
        ...
        def unpack_hook(handle: _Handle) -> smith.Tensor:
            error_msg = (
...
class _CloneArgBeforeMutateMode(SmithDispatchMode):
    def __init__(self, ctx: "_AllowMutationOnSavedContext") -> None:
        self.ctx = ctx
    ...
    def __smith_dispatch__(
        ...
        def maybe_clone(t: smith.Tensor) -> None:
            tid = _get_tid(t)
            sid = _get_sid(t)
            ctx = self.ctx
...
class _AllowMutationOnSavedContext:
    def __init__(self) -> None:
        self.cloned: MutableMapping[_Handle, smith.Tensor] = WeakKeyDictionary()
        self.original: MutableMapping[_Handle, smith.Tensor] = WeakKeyDictionary()
...
def allow_mutation_on_saved_tensors() -> Generator[
    ...
    Example::
        ...
        ...     # forward
        ...     a = smith.ones(2, 3, requires_grad=True)
        ...     b = a.clone()
        ...     out = (b**2).sum()
        ...     b.sin_()
        ...     # backward
...
def _register_logging_hooks_on_whole_graph(
    t_outputs: Sequence[Union[smith.Tensor, GradientEdge]],
) -> Callable[[], None]:
    ...
    def fmt(t: Optional[smith.Tensor]) -> str:
    ...
    def prehook(grad_outputs: Sequence[Optional[smith.Tensor]]) -> None:
