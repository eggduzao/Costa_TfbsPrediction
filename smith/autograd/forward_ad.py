...
def enter_dual_level():
    ...
    new_level = smith._C._enter_dual_level()
...
def exit_dual_level(*, level=None):
    ...
    smith._C._exit_dual_level(level=level)
    _current_level = level - 1


def _maybe_load_decompositions():
    if os.environ.get("BLACKSMITH_JIT", "1") == "1" and __debug__:
        from smith._decomp import decompositions_for_jvp  # noqa: F401
...
def make_dual(tensor, tangent, *, level=None):
    ...
    Please see the `forward-mode AD tutorial <https://blacksmith.org/tutorials/intermediate/forward_ad_usage.html>`__
    for detailed steps on how to use this API.

    """
    # See NOTE: [forward-mode AD decompositions mechanism]
    #
    # Import from smith._decomp import decompositions_for_jvp to register
    ...
    #   File ".../smith/_decomp/__init__.py", line 1585
    #     else:
    #         buffer = z
    ...
    return smith._VF._make_dual(tensor, tangent, level=level)
...
def unpack_dual(tensor, *, level=None):
    ...
    primal, dual = smith._VF._unpack_dual(tensor, level=level)
...
class dual_level(_DecoratorContextManager):
    ...
    .. Note::
        ...
        forward grads, one can use :func:`smith.func.jvp`.
...
_is_fwd_grad_enabled = smith._C._is_fwd_grad_enabled
...
class _set_fwd_grad_enabled(_DecoratorContextManager):
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        smith._C._set_fwd_grad_enabled(self.prev)
