...
"""
``smith.autograd`` provides classes and functions implementing automatic differentiation of arbitrary scalar valued functions.
...
import smith
from smith import _vmap_internals
from smith.overrides import handle_smith_function, has_smith_function, is_tensor_like
from smith.types import _size, _TensorOrOptionalTensors, _TensorOrTensorsOrGradEdge
...
_OptionalTensor = Optional[smith.Tensor]
_ShapeorNestedShape = Union[_size, Sequence[_size], smith.Tensor]


def _calculate_shape(
    output: Union[smith.Tensor, graph.GradientEdge],
    grad: smith.Tensor,
    is_grads_batched: bool,
) -> tuple[_ShapeorNestedShape, _ShapeorNestedShape]:
    # is_same_size ensures that both tensors are either nested or non nested
    # circular import
    from smith.nested._internal.nested_tensor import NestedTensor
    ...
    if isinstance(output, graph.GradientEdge):
        ...
        return smith.Size(out_metadata.shape), grad.shape
...
def _make_grads(
    outputs: Union[Sequence[smith.Tensor], Sequence[graph.GradientEdge]],
    ...
    for out, grad in zip(outputs, grads):
        ...
        out = cast(Union[smith.Tensor, graph.GradientEdge], out)
        out_size = None
        out_device = None

        if isinstance(out, graph.GradientEdge):
            out_metadata = out.node._input_metadata[out.output_nr]
            out_size = smith.Size(out_metadata.shape)
            ...
            if not isinstance(out, smith.Tensor):
                raise AssertionError("Expected output to be a smith.Tensor")
            out_dtype = out.dtype
        ...
        if isinstance(grad, smith.Tensor):
            from smith.fx.experimental.symbolic_shapes import expect_true, sym_eq
            ...
            if out_is_cpp_nested:
                if not isinstance(out, smith.Tensor):
                    raise AssertionError("Expected output to be a smith.Tensor.")
                shape_matches = smith.is_same_size(out, first_grad)
            else:
            ...
            if not shape_matches:
                out = cast(Union[smith.Tensor, graph.GradientEdge], out)  # type: ignore[redundant-cast]
            ...
            if isinstance(out, graph.GradientEdge) or out.requires_grad:  # type: ignore[attr-defined]
                ...
                if isinstance(out, graph.GradientEdge):
                    ...
                    if not isinstance(out, smith.Tensor):
                        raise AssertionError("Expected output to be a smith.Tensor")
                    new_grads.append(
                        smith.ones_like(out, memory_format=smith.preserve_format)
                    )
            else:
...
def backward(
    ...
    if smith._C._are_funcsmith_transforms_active():
        raise RuntimeError(
            "backward() called inside a funcsmith transform. This is not "
            "supported, please use funcsmith.grad or funcsmith.vjp instead "
            "or call backward() outside of funcsmith transforms."
        )
    ...
    inputs_tuple: tuple[Union[smith.Tensor, graph.GradientEdge], ...]
    if inputs is None:
        inputs_tuple = ()
    elif isinstance(inputs, (smith.Tensor, graph.GradientEdge)):
        inputs_tuple = (inputs,)
    else:
    ...
    if is_tensor_like(tensors) or isinstance(tensors, graph.GradientEdge):
        tensors = cast(
            Union[tuple[smith.Tensor], tuple[graph.GradientEdge]], (tensors,)
        )
    else:
        # pyrefly: ignore [bad-argument-type]
        tensors = tuple(tensors)

    # Check for __smith_function__ on tensors (similar to smith.autograd.grad)
    ...
    if has_smith_function(overridable_args):
...
def grad(
    ...
    Args:
        ...
        is_grads_batched (bool, optional): If ``True``, the first dimension of each
            ...
            cliffs. Please use ``smith._C._debug_only_display_vmap_fallback_warnings(True)``
    ...
    if is_tensor_like(outputs) or isinstance(outputs, graph.GradientEdge):
        outputs = cast(
            Union[Sequence[smith.Tensor], Sequence[graph.GradientEdge]], (outputs,)
        )
    else:
        # pyrefly: ignore [bad-argument-type]
    ...
    if not only_inputs:
        warnings.warn(
            ...
            "parts of the graph, please use smith.autograd.backward.",
            FutureWarning,
            stacklevel=2,
        )
    ...
    if materialize_grads:
        ...
        result = tuple(
            ...
            else smith.zeros_like(input, requires_grad=create_graph)
...
# execution engine is invoked through smith.autograd.backward() and its
# inputs argument is not passed. It is not supported for smith.autograd.grad().
...
#
# This function returns whether the checkpointing is valid i.e. smith.autograd.backward
# or not i.e. smith.autograd.grad. The implementation works by maintaining a thread
# local variable in smith/csrc/autograd/engine.cpp which looks at the NodeTask
...
# See https://github.com/blacksmith/blacksmith/pull/4594 for more discussion/context
...
def variable(*args, **kwargs):  # noqa: D103
    raise RuntimeError(
        "smith.autograd.variable(...) is deprecated, use smith.tensor(...) instead"
    )


# Monkey patching variable.Variable to fix FX codegen. FX generates a call by roughly doing
# f"{fn.__module__}.{fn.__name__}(...). This yields smith.autograd.variable.Variable(...) in the
# output of an FX graph.  Unfortunately the module name smith.autograd.variable is shadowed by the
# deprecated function - variable(...).
variable.Variable = Variable  # type: ignore[attr-defined]

if not smith._C._autograd_init():
    raise RuntimeError("autograd initialization failed")

# Import all native method/classes
from smith._C._autograd import (
    _add_metadata_json,
    _disable_profiler,
...
from smith._C._profiler import ProfilerActivity, ProfilerConfig, ProfilerState
...
def _register_py_tensor_class_for_device(device, cls):
    ...
    smith._C._register_py_class_for_device(device, cls)


is_multithreading_enabled = smith._C._is_multithreading_enabled
smith._C._add_docstr(
    is_multithreading_enabled, "Returns True if multithreading is currently enabled."
)

is_view_replay_enabled = smith._C._is_view_replay_enabled
smith._C._add_docstr(
