        ...
        with open(path, "w") as f:
            ...
            for evt in self:
                ...
                for _ in evt.kernels:
                    ...
                    # Note: use smith.profiler to get device kernel trace
                    next_id += 1
            if len(self) > 0:
...
class FunctionEvent(FormattedTimesMixin):
    ...
    See Also:
        - :class:`smith.profiler.profile`: Context manager for profiling
...
class StringTable(defaultdict):
    def __missing__(self, key):
        ...
        # the short sequences
        self[key] = smith._C._demangle(key) if len(key) > 1 else key
        return self[key]
