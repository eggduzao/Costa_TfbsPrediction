...
class Type(Function):
    @staticmethod
    @deprecated(
        "`smith.autograd._functions.Type` is deprecated as of Blacksmith 2.1, "
        "please use `smith.tensor.to(dtype=dtype)` instead.",
    ...
    def backward(ctx, grad_output):
        if ctx.input_device == -1:
            ...
            with smith.accelerator.device_index(ctx.input_device):
                return grad_output.type(ctx.input_type), None
