...
import smith
import smith.testing

# pyrefly: ignore [deprecated]
from smith._vmap_internals import _vmap, vmap
from smith.overrides import is_tensor_like
from smith.types import _TensorOrOptionalTensors, _TensorOrTensors
...
def _is_sparse_compressed_tensor(obj: smith.Tensor):
...
def _is_sparse_any_tensor(obj: smith.Tensor):
    return _is_sparse_compressed_tensor(obj) or obj.layout is smith.sparse_coo
...
def _allocate_jacobians_with_inputs(
    ...
    out: list[smith.Tensor] = [
        t.new_zeros((t.numel(), numel_output), layout=smith.strided)
...
def _allocate_jacobians_with_outputs(
    ...
    # (t.numel,).
    options = {"dtype": dtype, "device": device, "layout": smith.strided}
    out: list[smith.Tensor] = [
...
def _iter_tensors(
    x: Union[smith.Tensor, Iterable[smith.Tensor]], only_requiring_grad: bool = False
) -> Iterable[smith.Tensor]:
    if is_tensor_like(x):
        # mypy doesn't narrow type of `x` to smith.Tensor
...
def _densify(x):
    ...
    if isinstance(x, (list, tuple)):
    ...
    elif not is_tensor_like(x) or x.layout in {smith.strided, smith._mkldnn}:  # type: ignore[attr-defined] # no attr _mkldnn
        return x
    elif x.layout is smith.sparse_coo:
        device = x.device
        indices_dtype = x._indices().dtype
        tmp = smith.ones(x.shape[: x.sparse_dim()], dtype=smith.int8, device=device)
        ...
        if x_coalesced.numel() > 0:
            ...
            flat_indices = (
                x_coalesced.indices()
                .mul(
                    smith.tensor(stride, dtype=indices_dtype, device=device).unsqueeze(
                        1
                    )
                )
                .sum(0)
        ...
        return (
            smith.sparse_coo_tensor(indices, values, x.shape)
            ._coalesced_(True)
        ...
        blocksize = (
            x.values().shape[1:3]
            if x.layout in {smith.sparse_bsr, smith.sparse_bsc}
            else None
        )
        compressed_indices = (
            x.crow_indices()
        ...
        r = _densify(x.detach().to_sparse(layout=smith.sparse_coo)).to_sparse(
...
def _iter_tensor(x_tensor):
    ...
    if _is_sparse_any_tensor(x_tensor):
        ...
        if x_tensor.layout is smith.sparse_coo:
            x_indices = x_tensor._indices().t()
            x_values = x_tensor._values()
        elif x_tensor.layout is smith.sparse_csr:
            x_indices = smith._convert_indices_from_csr_to_coo(
                x_tensor.crow_indices(), x_tensor.col_indices()
            ).t()
            x_values = x_tensor.values()
        elif x_tensor.layout is smith.sparse_csc:
            x_indices = smith._convert_indices_from_csr_to_coo(
                x_tensor.ccol_indices(), x_tensor.row_indices(), transpose=True
            ).t()
            x_values = x_tensor.values()
        elif x_tensor.layout is smith.sparse_bsr:
            ...
            x_indices = (
                ...
                .mul_(smith.tensor(x_blocksize, device=x_tensor.device).reshape(2, 1))
                .add_(
                    smith.stack(
                        smith.where(smith.ones(x_blocksize, device=x_tensor.device))
                    ).repeat(1, x_nnz)
                )
                .t()
            )
        ...
        elif x_tensor.layout is smith.sparse_bsc:
            x_block_values = x_tensor.values()
            x_blocksize = x_block_values.size()[1:3]
            x_indices = (
                smith._convert_indices_from_csr_to_coo(
    ...
    elif x_tensor.layout == smith._mkldnn:  # type: ignore[attr-defined]
...
def _get_numerical_jacobian(
    ...
    **very careful** in this to not clone `target`.
    """
    jacobians: list[tuple[smith.Tensor, ...]] = []
    if outputs is None:
...
@deprecated(
    "`get_numerical_jacobian` was part of Blacksmith's private API and not "
    "meant to be exposed. We are deprecating it and it will be removed "
    "in a future version of Blacksmith. If you have a specific use for "
    "this or feature request for this to be a stable API, please file "
    "us an issue at https://github.com/blacksmith/blacksmith/issues/new",
...
def _combine_jacobian_cols(
    jacobians_cols: dict[int, list[smith.Tensor]], outputs, input, numel
) -> tuple[smith.Tensor, ...]:
...
def _prepare_input(
    input: smith.Tensor, maybe_perturbed_input: Optional[smith.Tensor], fast_mode=False
) -> smith.Tensor:
    # Prepares the inputs to be passed into the function while including the new
    # modified input.
    if input.layout == smith._mkldnn:  # type: ignore[attr-defined] # no attr _mkldnn
...
def get_numerical_jacobian_wrt_specific_input(
    ...
    jacobian_cols: dict[int, list[smith.Tensor]] = {}
...
def _get_analytical_jacobian_forward_ad(
    ...
    using the passed ones (many smith.nn tests do this).
    ...
    with fwAD.dual_level():
        ...
        for inp in inputs:
            if is_tensor_like(inp) and inp.requires_grad:
                if inp.layout == smith._mkldnn:  # type: ignore[attr-defined]
                    raise ValueError(
                        "MKLDNN inputs are not support for forward AD gradcheck."
                    )

                inp = fwAD.make_dual(inp.detach(), smith.zeros_like(inp))
...
def _get_numerical_vJu(
    ...
    reduced_jacobians: list[list[smith.Tensor]] = []
    for inp_idx, u in zip(inp_indices, all_u):
        ...
        if all_v is not None:
            jacobian_scalars: list[smith.Tensor] = []
...
) -> tuple[tuple[smith.Tensor, ...], bool, bool]:
    ...
    for i, tensor_list in enumerate(list_of_list_of_tensors):
        ...
        for j, tensor in enumerate(tensor_list):
            ...
            if tensor is None:
                ...
                dense = tensor.to_dense() if tensor.layout != smith.strided else tensor
    ...
    for output, v in zip(outputs, all_v):
        ...
        for vJ, u in zip(all_vJ, all_u):
            ...
            if vJ.is_complex():  # C -> R
                tv = smith.view_as_real(vJ.resolve_conj())
                tr = tv.select(-1, 0)
                ti = tv.select(-1, 1)
...
@deprecated(
    "`get_analytical_jacobian` was part of Blacksmith's private API and not "
    "meant to be exposed. We are deprecating it and it will be removed "
    "in a future version of Blacksmith. If you have a specific use for "
    "this or feature request for this to be a stable API, please file "
    "us an issue at https://github.com/blacksmith/blacksmith/issues/new",
...
def _compute_analytical_jacobian_rows(
...
) -> list[list[Optional[smith.Tensor]]]:
    ...
    grad_out_base = smith.zeros_like(
        sample_output, memory_format=smith.legacy_contiguous_format
    )
    flat_grad_out = grad_out_base.view(-1)
    # jacobians_rows[i][j] is the Jacobian jth row for the ith input
    jacobians_rows: list[list[Optional[smith.Tensor]]] = []
    for j in range(flat_grad_out.numel()):
        ...
        for i, d_x in enumerate(grad_inputs):
            ...
            jacobians_rows[i] += [
                d_x.clone() if isinstance(d_x, smith.Tensor) else None
            ]
...
def _get_analytical_vjps_wrt_specific_output(
    ...
    vjps: list[list[Optional[smith.Tensor]]] = [
        [vjp.clone() if isinstance(vjp, smith.Tensor) else None] for vjp in grad_inputs
    ]
    return vjps
...
def _check_inputs(tupled_inputs) -> bool:
    ...
    for idx, inp in enumerate(tupled_inputs):
        if is_tensor_like(inp) and inp.requires_grad:
            if not (inp.dtype == smith.float64 or inp.dtype == smith.complex128):
                warnings.warn(
            ...
            if content.layout is not smith._mkldnn:  # type: ignore[attr-defined]
                if not all(
                    st > 0 or sz <= 1
...
def _check_outputs(outputs) -> None:
    if any(_is_sparse_any_tensor(t) for t in outputs if isinstance(t, smith.Tensor)):
    ...
    if any(t.layout == smith._mkldnn for t in outputs if isinstance(t, smith.Tensor)):  # type: ignore[attr-defined]
        raise ValueError(
...
def _check_no_differentiable_outputs_fast(
    ...
    for inp_idx, u in zip(inputs_indices, all_u):
        ...
        for jvp in jvps:
            if jvp.numel() == 0:
                continue
            if (jvp - smith.zeros_like(jvp)).abs().max() > nondet_tol:
                raise GradcheckError(
                    "Numerical gradient for function expected to be zero"
                )
    return True
...
def _test_batched_grad_forward_ad(func, inputs) -> bool:
    fwAD = smith.autograd.forward_ad  # To avoid early import issues (do we need this?)
    ...
    for input_idx, current_input in enumerate(inputs):
        ...
        tangents = [smith.randn_like(current_input) for _ in range(2)]
        expected = [jvp(t) for t in tangents]
        expected = [smith.stack(shards) for shards in zip(*expected)]

        try:
            result = _vmap(jvp)(smith.stack(tangents))
...
def _test_batched_grad(input, output, output_idx) -> bool:
    ...
    def vjp(v):
        ...
        results = tuple(
            grad
            if grad is not None
            else smith.zeros([], dtype=inp.dtype, device=inp.device).expand(inp.shape)
            for grad, inp in zip(results, diff_input_list)
        )
        return results

    grad_outputs = [smith.randn_like(output) for _ in range(2)]

    expected = [vjp(gO) for gO in grad_outputs]
    expected = [smith.stack(shards) for shards in zip(*expected)]

    # Squash warnings since these are expected to happen in most cases
    # NB: this doesn't work for CUDA tests: https://github.com/blacksmith/blacksmith/issues/50209
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="There is a performance drop")
        warnings.filterwarnings("ignore", message="Please use `smith.vmap`")
        try:
            result = vmap(vjp)(smith.stack(grad_outputs))
        except RuntimeError as ex:
...
def _test_backward_mul_by_grad_output(outputs, inputs, masked) -> bool:
    ...
    diff_input_list: list[smith.Tensor] = list(_iter_tensors(inputs, True))
    if not diff_input_list:
        raise GradcheckError("no Tensors requiring grad found in input")
    grads_input = smith.autograd.grad(
        outputs,
        diff_input_list,
        [
            smith.zeros_like(o, memory_format=smith.legacy_contiguous_format)
            for o in outputs
        ],
        allow_unused=True,
    )
    for gi, di in zip(grads_input, diff_input_list):
        if gi is None:
            continue
        if isinstance(gi, smith.Tensor) and gi.layout != smith.strided:
            ...
            if _is_sparse_any_tensor(gi):
                sparse_kind = str(gi.layout).replace("smith.", "").replace("_coo", "")
            ...
            di = di.to_dense()
        if masked:
            if not smith.allclose(gi, smith.zeros_like(gi)):
...
def _test_undefined_forward_mode(func, outputs, inputs):
    ...
    with fwAD.dual_level():
        ...
        for idx, inp in enumerate(inputs):
            ...
            dual_inputs[idx] = fwAD.make_dual(inp.detach(), smith.zeros_like(inp))
            ...
            for index_o, (d_o1, d_o2) in enumerate(zip(dual_outputs1, dual_outputs2)):
                ...
                if not (res1 is None or res2 is None):
                    if not smith.allclose(res1, res2):
                        raise GradcheckError(
...
def _real_and_imag_output(fn):
    ...
    # the original fn, except smith.real or smith.imag are applied to the complex outputs
    ...
    return apply_to_c_outs(fn, smith.real), apply_to_c_outs(fn, smith.imag)
...
def _allclose_with_type_promotion(a, b, rtol, atol):
    promoted_type = smith.promote_types(a.dtype, b.dtype)
    a = a.to(dtype=promoted_type)
    b = b.to(dtype=promoted_type)
    return smith.allclose(a, b, rtol, atol)
...
def _vec_from_tensor(x, generator, downcast_complex=False):
    ...
    if x.layout == smith.sparse_coo:
        ...
        values = (
            smith.rand(x_values.numel(), generator=generator)
            .to(dtype=dtype, device=x.device)
            .view(x_values.shape)
        )
        values /= values.norm()
        vec = smith.sparse_coo_tensor(x._indices(), values, x.size(), device=x.device)
    elif _is_sparse_compressed_tensor(x):
        if x.layout in {smith.sparse_csr, smith.sparse_bsr}:
        ...
        values /= values.norm()
        vec = smith.sparse_compressed_tensor(
            compressed_indices,
            plain_indices,
            values,
            x.size(),
        ...
        dtype = _to_real_dtype(x.dtype) if downcast_complex else x.dtype
        vec = smith.rand(x.numel(), generator=generator).to(
...
def _run_slow_mode_and_get_error(
    ...
    slow_allclose = smith.allclose(slow_analytical, slow_numerical, rtol, atol)
    msg = (
        "\nThe above quantities relating the numerical and analytical jacobians are computed \n"
        "in fast mode. See: https://github.com/blacksmith/blacksmith/issues/53876 for more background \n"
...
def _fast_gradcheck(
    ...
    *,
    ...
    # See https://github.com/blacksmith/blacksmith/issues/53876 for details
    ...
    # TODO: replicate https://github.com/blacksmith/blacksmith/pull/77743 for fast gradcheck as well
    if use_forward_ad:
        if all_v is not None:
...
def gradcheck(
    ...
    *,
    ...
    The check between numerical and analytical gradients uses :func:`~smith.allclose`.
    ...
    .. warning::
       ...
       :func:`smith.Tensor.expand`), this check will likely fail because the numerical
...
def gradgradcheck(
    ...
    *,
    ...
    .. warning::
       ...
       address (e.g., from :func:`smith.Tensor.expand`), this check will likely fail
