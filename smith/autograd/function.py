...
import smith._funcsmith as _funcsmith
import smith.utils.hooks as hooks
from smith._C import _functions
from smith._funcsmith.autograd_function import custom_function_call
...
class FunctionCtx:
    def save_for_backward(self, *tensors: smith.Tensor):
        ...
        tensor hooks. See :class:`smith.autograd.graph.saved_tensors_hooks`.
        ...
        `double backward tutorial <https://blacksmith.org/tutorials/intermediate/custom_function_double_backward_tutorial.html>`_
        for more details.
        ...
        Example::
            ...
            >>>         return gx, gy, gz
            >>>
            >>> a = smith.tensor(1., requires_grad=True, dtype=smith.double)
            >>> b = smith.tensor(2., requires_grad=True, dtype=smith.double)
            >>> c = 4
            >>> d = Func.apply(a, b, c)
    ...
    def save_for_forward(self, *tensors: smith.Tensor):
        ...
        Example::
            ...
            >>>
            >>>     a = smith.tensor(1., requires_grad=True, dtype=smith.double)
            >>>     t = smith.tensor(1., dtype=smith.double)
            >>>     b = smith.tensor(2., requires_grad=True, dtype=smith.double)
            >>>     c = 4
            >>>
        ...
        for tensor in tensors:
            if not (isinstance(tensor, smith.Tensor) or tensor is None):
    ...
    def mark_dirty(self, *args: smith.Tensor):
        ...
        Examples::
            ...
            >>>         return grad_output
            >>>
            >>> a = smith.tensor(1., requires_grad=True, dtype=smith.double).clone()
            >>> b = a * a
    ...
    def mark_non_differentiable(self, *args: smith.Tensor):
    ...
    def set_materialize_grads(self, value: bool):
        ...
        Example::
            ...
            >>>         return grad_input
            >>>
            >>> a = smith.tensor(1., requires_grad=True)
...
class _SingleLevelFunction(
    ...
    def forward(*args: Any, **kwargs: Any) -> Any:
        ...
        - Instead, you must also override the :meth:`smith.autograd.Function.setup_context`
    ...
    def setup_context(ctx: Any, inputs: tuple[Any, ...], output: Any) -> Any:
        ...
        See :meth:`smith.autograd.Function.forward` and :ref:`extending-autograd` for more details.
        """
    ...
    """
    Bool that specifies if Blacksmith should clear saved tensors after the first
...
class Function(_SingleLevelFunction):
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn(
            ...
            "error in a future version of Blacksmith.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def __call__(self, *args, **kwargs):
        raise RuntimeError(
            ...
            "(Example: https://blacksmith.org/docs/stable/autograd.html#smith.autograd.Function)"
        )

    """
    Bool that specifies if Blacksmith should attempt to autogenerate
    :func:`smith.vmap` support for this autograd.Function. You may set this to
    True only if this autograd.Function's forward, backward, and jvp (if they
    exist) are written using Blacksmith operations; otherwise, please override
    :meth:`smith.autograd.Function.vmap` to add support for :func:`smith.vmap`.
    ...
    def vmap(info, in_dims, *args):
        r"""Define the behavior for this autograd.Function underneath :func:`smith.vmap`.

        For a :func:`smith.autograd.Function` to support
        :func:`smith.vmap`, you must either override this static method, or set
    ...
    def apply(cls, *args, **kwargs):
        ...
        if not smith._C._are_funcsmith_transforms_active():
            # See NOTE: [funcsmith vjp and autograd interaction]
            args = _funcsmith.utils.unwrap_dead_wrappers(args)
            return super().apply(*args, **kwargs)  # type: ignore[misc]

        if not is_setup_ctx_defined:
            raise RuntimeError(
                "In order to use an autograd.Function with funcsmith transforms "
                "(vmap, grad, jvp, jacrev, ...), it must override the setup_context "
                "staticmethod. For more details, please see "
                "https://blacksmith.org/docs/main/notes/extending.func.html"
            )
...
def once_differentiable(
    ...
    def wrapper(ctx: _T, *args: _P.args, **kwargs: _P.kwargs) -> _R:
        ...
        requires_grad = any(
            isinstance(arg, smith.Tensor) and arg.requires_grad for arg in args
        )
...
def _unflatten(input, proto):
    ...
    def unflatten_helper(input, proto):
        res: list[Optional[smith.Tensor]] = []
...
_iter_jit_values = _iter_filter(
    lambda o: o is None or isinstance(o, smith._C.Value),
...
_iter_None_tensors = _iter_filter(
    lambda o: o is None or isinstance(o, smith.Tensor), condition_msg="Tensors or None"
)
_map_tensor_data = _nested_map(
    lambda x: isinstance(x, smith.Tensor), lambda o: o.data, condition_msg="Tensors"
)
