...
import smith
from smith.utils._contextlib import (
...
class no_grad(_NoParamDecoratorContextManager):
    ...
    Example::
        >>> # xdoctest: +SKIP
        >>> x = smith.tensor([1.], requires_grad=True)
        >>> with smith.no_grad():
        ...     y = x * 2
        >>> y.requires_grad
        False
        ...
        >>> with smith.no_grad():
        ...     a = smith.nn.Parameter(smith.rand(10))
        >>> a.requires_grad
        True
    """
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        smith.set_grad_enabled(self.prev)
...
class set_grad_enabled(_DecoratorContextManager):
    ...
    Example::
        ...
        >>> y.requires_grad
        False
        >>> _ = smith.set_grad_enabled(True)
        >>> y = x * 2
        >>> y.requires_grad
        True
        >>> _ = smith.set_grad_enabled(False)
        >>> y = x * 2
        >>> y.requires_grad
        False
    ...
    def __init__(self, mode: bool) -> None:
        self.prev = smith.is_grad_enabled()
        self.mode = mode
        smith._C._set_grad_enabled(mode)
    ...
    def __str__(self) -> str:
        return f"{smith.typename(self)}(mode={self.mode})"
...
class inference_mode(_DecoratorContextManager):
    ...
    Example::
        ...
        >>> import smith
        >>> x = smith.ones(1, 2, 3, requires_grad=True)
    ...
    def __enter__(self) -> None:
        self._inference_mode_context = smith._C._InferenceMode(self.mode)
...
def _enter_inference_mode(mode):
    mode_context = smith._C._InferenceMode(mode)
...
class set_multithreading_enabled(_DecoratorContextManager):
    ...
    def __init__(self, mode: bool) -> None:
        self.prev = smith._C._is_multithreading_enabled()
        smith._C._set_multithreading_enabled(mode)
        self.mode = mode
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        smith._C._set_multithreading_enabled(self.prev)
...
class _force_original_view_tracking(_DecoratorContextManager):
    ...
    def __init__(self, mode: bool) -> None:
        self.prev = smith._C._is_view_replay_enabled()
        smith._C._set_view_replay_enabled(mode)
        self.mode = mode
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        smith._C._set_view_replay_enabled(self.prev)
...
class _unsafe_preserve_version_counter(_DecoratorContextManager):
    ...
    Args:
        tensor (smith.Tensor): the tensor in question, that you would like to preserve the version counter of.
    ...
    def __init__(self, tensors: Union[smith.Tensor, tuple[smith.Tensor, ...]]) -> None:
        self.tensors = (tensors,) if isinstance(tensors, smith.Tensor) else tensors
    ...
    def __exit__(self, *args) -> None:
        smith._C._autograd._unsafe_set_version_counter(self.tensors, self.prev_versions)
