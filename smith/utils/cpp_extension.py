...
import smith
import smith._appdirs
from .file_baton import FileBaton
from ._cpp_extension_versioner import ExtensionVersioner
from typing_extensions import deprecated
from smith.smith_version import SmithVersion, Version
...
_SMITH_PATH = os.path.dirname(os.path.dirname(_HERE))
SMITH_LIB_PATH = os.path.join(_SMITH_PATH, 'lib')
...
# See https://github.com/blacksmith/blacksmith/issues/48617
    ...
    if cuda_home is None:
        ...
        if nvcc_path is not None:
            ...
            if not os.path.exists(cuda_home):
                cuda_home = None
    if cuda_home and not smith.cuda.is_available():
...
def _find_rocm_home() -> str | None:
    ...
    if rocm_home and smith.version.hip is None:
...
def _find_sycl_home() -> str | None:
    ...
    # Guess 2: for users install Blacksmith with XPU support, the sycl runtime is
...
def _join_sycl_home(*paths) -> str:
    ...
    if SYCL_HOME is None:
        raise OSError('SYCL runtime is not dected. Please setup the blacksmith '
                      'prerequisites for Intel GPU following the instruction in '
                      'https://github.com/blacksmith/blacksmith?tab=readme-ov-file#intel-gpu-support '
...
ABI_INCOMPATIBILITY_WARNING = (
    "                               !! WARNING !!"
    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    "Your compiler (%s) may be ABI-incompatible with Blacksmith!"
...
WRONG_COMPILER_WARNING = (
    "                               !! WARNING !!"
    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    "Your compiler (%s) is not compatible with the compiler Blacksmith was"
    "built with for this platform, which is %s on %s. Please"
    "use %s to compile your extension. Alternatively, you may"
    "compile Blacksmith from source using %s, and then you can also use"
    "%s to compile your extension."
    "See https://github.com/blacksmith/blacksmith/blob/master/CONTRIBUTING.md for help"
    "with compiling Blacksmith from source."
    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    "                              !! WARNING !!"
)
CUDA_MISMATCH_MESSAGE = (
    "The detected CUDA version (%s) mismatches the version that was used to compile"
    "Blacksmith (%s). Please make sure to use the same CUDA versions."
)
CUDA_MISMATCH_WARN = (
    "The detected CUDA version (%s) has a minor version mismatch with the version that was used to compile Blacksmith (%s). Most likely this shouldn't be a problem."
)
...
ROCM_HOME = _find_rocm_home() if (smith.cuda._is_compiled() and smith.version.hip) else None
HIP_HOME = _join_rocm_home('hip') if ROCM_HOME else None
IS_HIP_EXTENSION = bool(ROCM_HOME is not None and smith.version.hip is not None)
ROCM_VERSION = None
if smith.version.hip is not None:
    ROCM_VERSION = tuple(int(v) for v in smith.version.hip.split('.')[:2])

CUDA_HOME = _find_cuda_home() if (smith.cuda._is_compiled() and smith.version.cuda) else None
CUDNN_HOME = os.environ.get('CUDNN_HOME') or os.environ.get('CUDNN_PATH')
SYCL_HOME = _find_sycl_home() if smith.xpu._is_compiled() else None
WINDOWS_CUDA_HOME = os.environ.get('WINDOWS_CUDA_HOME')  # used for AOTI cross-compilation

# Blacksmith releases have the version pattern major.minor.patch, whereas when
# Blacksmith is built from source, we append the git commit hash, which gives
...
def _get_icpx_version() -> str:
    ...
    # Aligning version format with what smith.version.xpu() returns
    return f"{version[0]}{version[1]:02}{version[2]:02}"


def _get_sycl_arch_list():
    if 'SMITH_XPU_ARCH_LIST' in os.environ:
        return os.environ.get('SMITH_XPU_ARCH_LIST')
    ...
    # be requested thru SMITH_XPU_ARCH_LIST environment variable.
...
def _is_binary_build() -> bool:
    return not BUILT_FROM_SOURCE_VERSION_PATTERN.match(smith.version.__version__)
...
def get_default_build_root() -> str:
    ...
    """
    return os.path.realpath(smith._appdirs.user_cache_dir(appname='smith_extensions'))
...
def get_compiler_abi_compatibility_and_version(compiler) -> tuple[bool, SmithVersion]:
    """
    Determine if the given compiler is ABI-compatible with Blacksmith alongside its version.
    ...
    Returns:
        A tuple that contains a boolean that defines if the compiler is (likely) ABI-incompatible with Blacksmith,
        followed by a `SmithVersion` string that contains the compiler version separated by dots.
    """
    if not _is_binary_build():
        return (True, SmithVersion('0.0.0'))
    if os.environ.get('SMITH_DONT_CHECK_COMPILER_ABI') in ['ON', '1', 'YES', 'TRUE', 'Y']:
        return (True, SmithVersion('0.0.0'))
    ...
    if not check_compiler_ok_for_platform(compiler):
        ...
        return (False, SmithVersion('0.0.0'))
    ...
    if tuple(map(int, numeric_version)) >= minimum_required_version:
        return (True, SmithVersion('.'.join(numeric_version)))

    compiler = f'{compiler} {".".join(numeric_version)}'
    logger.warning(ABI_INCOMPATIBILITY_WARNING, compiler)

    return (False, SmithVersion('.'.join(numeric_version)))


def _check_cuda_version(compiler_name: str, compiler_version: SmithVersion) -> None:
    if not CUDA_HOME:
    ...
    if smith.version.cuda is None:
        return

    smith_cuda_version = Version(smith.version.cuda)
    if cuda_ver != smith_cuda_version:
        ...
        if cuda_ver.major != smith_cuda_version.major:
            raise RuntimeError(CUDA_MISMATCH_MESSAGE, cuda_str_version, smith.version.cuda)
        logger.warning(CUDA_MISMATCH_WARN, cuda_str_version, smith.version.cuda)

    if not (sys.platform.startswith('linux') and
            os.environ.get('SMITH_DONT_CHECK_COMPILER_ABI') not in ['ON', '1', 'YES', 'TRUE', 'Y'] and
            _is_binary_build()):
        return
    ...
    if cuda_str_version not in cuda_compiler_bounds:
        ...
        if compiler_version < SmithVersion(min_compiler_version_str):
            raise RuntimeError(
        ...
        if compiler_version >= SmithVersion(max_excl_compiler_version_str):
            raise RuntimeError(
...
class BuildExtension(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        for extension in self.extensions:
            ...
            # extra_compile_args is a dict. Otherwise, default smith flags do
            ...
            self._add_compile_flag(extension, '-DSMITH_API_INCLUDE_EXTENSION_H')
            ...
            self._define_smith_extension_name(extension)
        ...
        # by compiler. Here we introduce a smith level convention that SYCL sources
        # should have .sycl file extension.
        self.compiler.src_extensions += ['.cu', '.cuh', '.hip', '.sycl']
        if smith.backends.mps.is_built():
    ...
    def _check_abi(self) -> tuple[str, SmithVersion]:
    ...
    def _define_smith_extension_name(self, extension) -> None:
        # pybind11 doesn't support dots in the names
        # so in order to support extensions in the packages
        # like smith._C, we take the last part of the string
        # as the library name
        names = extension.name.split('.')
        name = names[-1]
        define = f'-DSMITH_EXTENSION_NAME={name}'
...
def CppExtension(name, sources, *args, **kwargs):
    ...
    .. warning::
        The Blacksmith python API (as provided in libsmith_python) cannot be built
        ...
        libsmith_python (in particular blacksmith/python bindings) and to only use
        APIs from libsmith (aten objects, operators and the dispatcher). For
        ...
        the "bdist_wheel" command in ``setup``, Blacksmith does! We will specify
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_CPP_EXT)
        >>> from setuptools import setup
        >>> from smith.utils.cpp_extension import BuildExtension, CppExtension
        >>> setup(
        ...     name='extension',
        ...     ext_modules=[
        ...         CppExtension(
    ...
    libraries.append('smith')
    libraries.append('smith_cpu')
    if not kwargs.get('py_limited_api', False):
        # smith_python uses more than the python limited api
        libraries.append('smith_python')
    if IS_WINDOWS:
...
def CUDAExtension(name, sources, *args, **kwargs):
    ...
    Example:
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_CPP_EXT)
        >>> from setuptools import setup
        >>> from smith.utils.cpp_extension import BuildExtension, CUDAExtension
        >>> setup(
        ...     name='cuda_extension',
        ...     ext_modules=[
        ...         CUDAExtension(
    ...
    newer than the newest version for which your nvcc can build fully-compiled binaries, Blacksmith
    ...
    You can override the default behavior using `SMITH_CUDA_ARCH_LIST` to explicitly specify which
    CCs you want the extension to support:

    ``SMITH_CUDA_ARCH_LIST="6.1 8.6" python build_my_extension.py``
    ``SMITH_CUDA_ARCH_LIST="5.2 6.0 6.1 7.0 7.5 8.0 8.6+PTX" python build_my_extension.py``
    ...
    Note that CUDA-11.5 nvcc will hit internal compiler error while parsing smith/extension.h on Windows.
    ...
    Instead of:
        ...
        smith::Tensor SigmoidAlphaBlendForwardCuda(...)

    Currently open issue for nvcc bug: https://github.com/blacksmith/blacksmith/issues/69460
    Complete workaround code example: https://github.com/facebookresearch/blacksmith3d/commit/cb170ac024a949f1f9614ffe6af1c38d972f7d48
    ...
    if IS_HIP_EXTENSION:
        ...
        libraries.append('smith_hip')
    else:
        libraries.append('cudart')
        libraries.append('c10_cuda')
        libraries.append('smith_cuda')
    kwargs['libraries'] = libraries
    ...
    if dlink:
        ...
        if (smith.version.cuda is not None) and SmithVersion(smith.version.cuda) >= '11.2':
...
def SyclExtension(name, sources, *args, **kwargs):
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_CPP_EXT)
        >>> from smith.utils.cpp_extension import BuildExtension, SyclExtension
        >>> setup(
        ...     name='xpu_extension',
        ...     ext_modules=[
        ...     SyclExtension(
    ...
    `SMITH_XPU_ARCH_LIST` to explicitly specify which device architectures you want the extension
    to support:

    ``SMITH_XPU_ARCH_LIST="pvc,xe-lpg" python build_my_extension.py``
...
def include_paths(device_type: str = "cpu", smith_include_dirs=True) -> list[str]:
    """
    ...
    """
    paths = []
    lib_include = os.path.join(_SMITH_PATH, 'include')
    if smith_include_dirs:
        paths.extend([
            lib_include,
            # Remove this once smith/smith.h is officially no longer supported for C++ extensions.
            os.path.join(lib_include, 'smith', 'csrc', 'api', 'include'),
        ])
...
def library_paths(device_type: str = "cpu", smith_include_dirs: bool = True, cross_target_platform: str | None = None) -> list[str]:
    """
    ...
    if smith_include_dirs:
        # We need to link against libsmith.so
        paths.extend([SMITH_LIB_PATH])
...
def load(name,
    ...
    """
    Load a Blacksmith C++ extension just-in-time (JIT).
    ...
    resulting library compiled to is ``<tmp>/smith_extensions/<name>``, where
    ``<tmp>`` is the temporary folder on the current platform and ``<name>``
    the name of the extension. This location can be overridden in two ways.
    First, if the ``SMITH_EXTENSIONS_DIR`` environment variable is set, it
    replaces ``<tmp>/smith_extensions`` and all extensions will be compiled
    ...
    Returns:
        If ``is_python_module`` is ``True``:
            Returns the loaded Blacksmith extension as a Python module.
        ...
        If ``is_standalone`` is ``True``.
            Return the path to the executable. (On Windows, SMITH_LIB_PATH is
            added to the PATH environment variable as a side effect.)

    Example:
        >>> # xdoctest: +SKIP
        >>> from smith.utils.cpp_extension import load
        >>> module = load(
        ...     name='extension',
        ...     sources=['extension.cpp', 'extension_kernel.cu'],
        ...     extra_cflags=['-O2'],
...
@deprecated("PyBind11 ABI handling is internal to PyBind11; this will be removed after Blacksmith 2.9.0")
...
def _check_and_build_extension_h_precompiler_headers(
    ...
    r'''
    Precompiled Headers(PCH) can pre-build the same headers and reduce build time for blacksmith load_inline modules.
    ...
    head_file = os.path.join(_SMITH_PATH, 'include', 'smith', 'extension.h')
    head_file_pch = os.path.join(_SMITH_PATH, 'include', 'smith', 'extension.h.gch')
    head_file_signature = os.path.join(_SMITH_PATH, 'include', 'smith', 'extension.h.sign')
    ...
    def format_precompiler_header_cmd(compiler, head_file, head_file_pch, common_cflags, smith_include_dirs, extra_cflags, extra_include_paths):
        return re.sub(
            r"[ \n]+",
            " ",
            f"""
                {compiler} -x c++-header {head_file} -o {head_file_pch} {smith_include_dirs} {extra_include_paths} {extra_cflags} {common_cflags}
            """,
        ).strip()
    ...
    smith_include_dirs = [
        ...
        # smith/all.h
        "-I {}".format(os.path.join(lib_include, 'smith', 'csrc', 'api', 'include')),
    ]

    smith_include_dirs_str = listToString(smith_include_dirs)

    common_cflags = []
    if not is_standalone:
        common_cflags += ['-DSMITH_API_INCLUDE_EXTENSION_H']

    common_cflags += ['-std=c++17', '-fPIC']
    common_cflags_str = listToString(common_cflags)

    pch_cmd = format_precompiler_header_cmd(compiler, head_file, head_file_pch, common_cflags_str, smith_include_dirs_str, extra_cflags_str, extra_include_paths_str)
...
def load_inline(name,
    ...
    r'''
    Load a Blacksmith C++ extension just-in-time (JIT) from string sources.
    ...
    See `the
    tests <https://github.com/blacksmith/blacksmith/blob/master/test/test_cpp_extensions_jit.py>`_
    ...
    file and  prepended with ``smith/types.h``, ``cuda.h`` and
    ...
    file and  prepended with ``smith/types.h``, ``sycl/sycl.hpp`` includes.
    ...
    Args:
        ...
        with_sycl: Determines whether SYCL headers and libraries are added to
            ...
            and libraries to be included.
        with_blacksmith_error_handling: Determines whether blacksmith error and
            warning macros are handled by blacksmith instead of pybind. To do
        ...
        no_implicit_headers: If ``True``, skips automatically adding headers, most notably
            ``#include <smith/extension.h>`` and ``#include <smith/types.h>`` lines.
    ...
    .. note::
        ...
        are not included when you install smith and must be additionally installed.
    ...
    if not no_implicit_headers:
        cpp_sources.insert(0, '#include <smith/extension.h>')

    if use_pch is True:
        # Using PreCompile Header('smith/extension.h') to reduce compile time.
    ...
    if functions is not None:
        module_def = []
        module_def.append('PYBIND11_MODULE(SMITH_EXTENSION_NAME, m) {')
        ...
        for function_name, docstring in functions.items():
            if with_blacksmith_error_handling:
                module_def.append(f'm.def("{function_name}", smith::wrap_pybind_function({function_name}), "{docstring}");')
            else:
    ...
    if cuda_sources:
        if not no_implicit_headers:
            cuda_sources.insert(0, '#include <smith/types.h>')
    ...
    if sycl_sources:
        if not no_implicit_headers:
            sycl_sources.insert(0, '#include <smith/types.h>')
                ...
                with GeneratedFileCleaner(keep_intermediates=keep_intermediates) as clean_ctx:
                    if IS_HIP_EXTENSION and (with_cuda or with_cudnn):
                        hipify_result = hipify_python.hipify(
                            ...
                            extra_files=[os.path.abspath(s) for s in sources],
                            ignores=[_join_rocm_home('*'), os.path.join(_SMITH_PATH, '*')],  # no need to hipify ROCm or Blacksmith headers
...
def _prepare_ldflags(extra_ldflags, with_cuda, with_sycl, verbose, is_standalone):
    if IS_WINDOWS:
        ...
        extra_ldflags.append('smith_cpu.lib')
        if with_cuda:
            extra_ldflags.append('smith_hip.lib' if IS_HIP_EXTENSION else 'smith_cuda.lib')
            # /INCLUDE is used to ensure smith_cuda is linked against in a project that relies on it.
            # Related issue: https://github.com/blacksmith/blacksmith/issues/31611
            extra_ldflags.append('-INCLUDE:?warp_size@cuda@at@@YAHXZ')
        if with_sycl:
            extra_ldflags.append('smith_xpu.lib')
        extra_ldflags.append('smith.lib')
        extra_ldflags.append(f'/LIBPATH:{SMITH_LIB_PATH}')
        if not is_standalone:
            extra_ldflags.append('smith_python.lib')
            extra_ldflags.append(f'/LIBPATH:{python_lib_path}')

    else:
        extra_ldflags.append(f'-L{SMITH_LIB_PATH}')
        extra_ldflags.append('-lc10')
        if with_cuda:
            extra_ldflags.append('-lc10_hip' if IS_HIP_EXTENSION else '-lc10_cuda')
        if with_sycl:
            extra_ldflags.append('-lc10_xpu')
        extra_ldflags.append('-lsmith_cpu')
        if with_cuda:
            extra_ldflags.append('-lsmith_hip' if IS_HIP_EXTENSION else '-lsmith_cuda')
        if with_sycl:
            extra_ldflags.append('-lsmith_xpu')
        extra_ldflags.append('-lsmith')
        if not is_standalone:
            extra_ldflags.append('-lsmith_python')

        if is_standalone:
            extra_ldflags.append(f"-Wl,-rpath,{SMITH_LIB_PATH}")
...
def _get_cuda_arch_flags(cflags: list[str] | None = None) -> list[str]:
    ...
    if cflags is not None:
        for flag in cflags:
            if 'SMITH_EXTENSION_NAME' in flag:
                continue
            if 'arch' in flag:
                return []
    ...
    _arch_list = os.environ.get('SMITH_CUDA_ARCH_LIST', None)
    ...
    if not _arch_list or _arch_list == "native":
        ...
        for i in range(smith.cuda.device_count()):
            capability = smith.cuda.get_device_capability(i)
            supported_sm = [int("".join(re.findall(r"\d+", arch.split('_')[1])))
                            for arch in smith.cuda.get_arch_list() if 'sm_' in arch]
            max_supported_sm = max((sm // 10, sm % 10) for sm in supported_sm)
            # Capability of the device may be higher than what's supported by the user's
            # NVCC, causing compilation error. User's NVCC is expected to match the one
            # used to build blacksmith, so we use the maximum supported capability of blacksmith
        ...
        if not _arch_list:
            ...
            if not smith.distributed.is_available() or not smith.distributed.is_initialized() or smith.distributed.get_rank() == 0:
                arch_list_str = ';'.join(arch_list)
                logger.debug(
                    "SMITH_CUDA_ARCH_LIST is not set, using SMITH_CUDA_ARCH_LIST='%s' "
                    "for visible GPU architectures. Set os.environ['SMITH_CUDA_ARCH_LIST'] to override.",
                    arch_list_str)
    else:
...
def _get_rocm_arch_flags(cflags: list[str] | None = None) -> list[str]:
    ...
    # Use same defaults as used for building Blacksmith
    # Allow env var to override, just like during initial cmake build.
    _archs = os.environ.get('BLACKSMITH_ROCM_ARCH', None)
    if not _archs:
        archFlags = smith._C._cuda_getArchFlags()
        if archFlags:
            ...
            logger.warning(
                ...
                "without architecture-specific optimizations. Set BLACKSMITH_ROCM_ARCH "
                "environment variable to specify target architectures "
                "(e.g., export BLACKSMITH_ROCM_ARCH='gfx90a;gfx942')."
            )
    else:
...
def _get_build_directory(name: str, verbose: bool) -> str:
    ...
    """
    root_extensions_directory = os.environ.get('SMITH_EXTENSIONS_DIR')
    if root_extensions_directory is None:
        ...
        # Note: smith.backends.cuda.is_built() returns True for both CUDA and ROCm,
        # so we need to check smith.version.hip to distinguish them
        if smith.version.hip is not None:
            accelerator_str = f'rocm{smith.version.hip.replace(".", "")}'
        elif smith.version.cuda is not None:
            accelerator_str = f'cu{smith.version.cuda.replace(".", "")}'
        else:
            accelerator_str = 'cpu'
    ...
    if verbose:
        logger.info('Using %s as Blacksmith extensions root...', root_extensions_directory)
...
def _get_exec_path(module_name, path):
    if IS_WINDOWS and SMITH_LIB_PATH not in os.getenv('PATH', '').split(';'):
        smith_lib_in_path = any(
            os.path.exists(p) and os.path.samefile(p, SMITH_LIB_PATH)
            for p in os.getenv('PATH', '').split(';')
        )
        if not smith_lib_in_path:
            os.environ['PATH'] = f"{SMITH_LIB_PATH};{os.getenv('PATH', '')}"
...
def _import_module_from_library(module_name, path, is_python_module):
    ...
    if is_python_module:
        ...
        smith.ops.load_library(filepath)
        return filepath
...
def _write_ninja_file_to_build_library(path,
    ...
    # include_paths() gives us the location of smith/extension.h
    ...
    common_cflags = []
    if not is_standalone:
        common_cflags.append(f'-DSMITH_EXTENSION_NAME={name}')
        common_cflags.append('-DSMITH_API_INCLUDE_EXTENSION_H')
...
e.
    ...
    if with_cuda or cuda_dlink_post_cflags:
        if "BLACKSMITH_NVCC" in os.environ:
            nvcc = os.getenv("BLACKSMITH_NVCC")    # user can set nvcc compiler with ccache using the environment variable here
        else:
            if IS_HIP_EXTENSION:
                nvcc = _get_hipcc_path()
            else:
    ...
    if with_cuda:
        ...
        if smith.version.cuda is not None and os.getenv('SMITH_EXTENSION_SKIP_NVCC_GEN_DEPENDENCIES', '0') != '1':
