...
class _Config(Generic[T]):
    ...
    Precedence Order:
        ...
        env_name_default: If set, this environment variable will override everything
            ...
            If multiple env variables are given, the precedence order is from
            left to right.
        justknob: If this blacksmith installation supports justknobs, that will
    ...
    def visit(
        ...
        for key, value in list(source.__dict__.items()):
            if (
                ...
                # Handle from smith.utils._config_module import Config
...
class ConfigModule(ModuleType):
    ...
    # The actual configuration settings.  E.g., smith._dynamo.config.debug
    # would live as "debug" in the key, and smith._inductor.config.triton.cudagraphs
    ...
    def codegen_config(self) -> str:
        ...
        def get_config_line(mod, k, v) -> str:  # type: ignore[no-untyped-def]
            ...
            Example output:
                import logging
                import _warnings
                smith._dynamo.config.reorderable_logging_functions = { _warnings.warn, logging.warn, print }
            """
