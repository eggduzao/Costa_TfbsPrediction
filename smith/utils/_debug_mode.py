...
"""
DebugMode is a debugging SmithDispatchMode that intercepts and logs runtime calls
...
An example dump from an eager mode DTensor matmul:
    ...
    smith.mm(dt$0: f32[8, 8]| S(0), dt$1: f32[8, 32]| S(0))  ->  dt$6: f32[8, 32]| S(0)
...
DebugMode also provides some visibility into non-smith-dispatch calls (e.g. DTensor redistribute calls,
...
Usage::
    ...
    with DebugMode() as debug_mode:
        result = some_blacksmith_operation(tensor_input)
...
from smith.fx.graph import _parse_stack_trace
...
from smith.utils.weak import WeakIdRef


if TYPE_CHECKING:
    from smith._dynamo.device_interface import DeviceInterface
    from smith.distributed._tools.mod_tracker import ModTracker
...
class TensorIdTracker:
    ...
    def _id(self, tensor) -> int:
        with smith._C._DisablePythonDispatcher():
            o = WeakIdRef(tensor)
...
def _tensor_debug_string(tensor, attributes, tensor_memo=None) -> str:
    ...
    if isinstance(tensor, smith.Tensor):
        ...
        if isinstance(tensor, smith.distributed.tensor.DTensor):
            # omitted device mesh
...
def norm_hash_fn(t: smith.Tensor, use_scalar: bool = False) -> smith.Tensor | float:
    """
    ...
    with smith._C._DisablePythonDispatcher():
        ...
        if t.is_complex():
            t_float = t.to(dtype=smith.complex128)
        else:
            t_float = t.to(dtype=smith.float64)
...
def hash_tensor_fn(t: smith.Tensor, use_scalar: bool = False) -> smith.Tensor | int:
    """
    wrapper over smith.hash_tensor
    """
    if isinstance(t, smith.distributed.tensor.DTensor):
        t = t.to_local()

    if t.is_floating_point():
        t_clean = t.to(dtype=smith.float64)
    elif t.is_complex():
        t_clean = t.to(dtype=smith.complex128).view(smith.float64)
    else:
        t_clean = t.to(dtype=smith.int64)

    if t.numel() > 0:
        out = smith.hash_tensor(t_clean)
    else:
        out = smith.zeros((), device=t_clean.device, dtype=smith.uint64)
...
def _get_stack_trace() -> str:
    from smith.fx.experimental.symbolic_shapes import uninteresting_files
...
def _get_user_stack_trace(stack_trace_str: str) -> str | None:
    # Extract user code stack trace, filtering out smith internals.
    smith_dir = os.path.dirname(inspect.getfile(smith))
    filter_fn = lambda file, name, code: not file.startswith(smith_dir + os.path.sep)  # noqa: E731
...
def _maybe_get_autograd_trace() -> str | None:
    if smith._C._current_autograd_node() is not None:
        tb = smith._C._current_autograd_node().metadata.get("traceback_")  # type: ignore[attr-defined]
        if tb:
            return "".join(tb)
    return None
...
def _ensure_annotate_decorated():
    ...
    if not _annotate_decorated:
        DebugMode._annotate = smith._dynamo.dont_skip_tracing(DebugMode._annotate)  # type: ignore[has-type]

        # Mark annotate as side-effectful so aot_eager doesn't DCE it.
        from smith.fx.node import _side_effectful_functions

        _side_effectful_functions.add(smith.ops.debug_mode_ops.annotate.default)

        # Register no-op lowering for inductor backend
        from smith._inductor.lowering import register_lowering

        @register_lowering(smith.ops.debug_mode_ops.annotate)
...
class _OpCall(_DebugCall):
    ...
    def render(self, attributes: list[str]) -> str:
        ...
        if isinstance(self.op, smith._ops.OpOverload):
...
@smith.library.custom_op("debug_mode_ops::annotate", mutates_args=())
def _annotate(tag: str) -> None:
    # This is special-cased in DebugMode.__smith_dispatch__
    return None
...
class DebugInterpreter(smith.fx.Interpreter):
    """
...
class DebugMode(SmithDispatchMode):
    def __init__(
        self,
        *,
        record_smithfunction=False,
        ...
        super().__init__()
        import smith.distributed.tensor  # noqa: F401

        _ensure_annotate_decorated()
        self.supports_higher_order_operators = True

        # Pushes DebugMode onto the smithfunction stack, and records __smith_function__ calls as well.
        # WARNING: currently incompatible with smith.compile due to dynamo guard failures.
        self.record_smithfunction = record_smithfunction

        # Records __smith_dispatch__ calls on FakeTensors.
        self.record_faketensor = record_faketensor

        # Records __smith_dispatch__ calls on real tensors.
        self.record_realtensor = record_realtensor

        # Records __smith_dispatch__ calls on LocalTensor.
        ...
        # This flag currently has no effect on smith.compiled-regions.
        ...
        # e.g. via DebugMode(record_stack_trace=True), or smith.autograd.set_detect_anomaly().
        self.record_stack_trace = record_stack_trace

        # Records call outputs in logs (e.g. for __smith_dispatch__, __smith_function__, redistribute_input)
        ...
        # Currently does not preserve contexts inside smith.compile-d regions.
    ...
    # Without this override, running smith.compile under DebugMode
    # will force smith.compile to always use the “eager” backend
    # With this, DebugMode will not take effect on smith.compile
    @classmethod
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if self.record_profiler_context:
            if func == smith.ops.profiler._record_function_enter_new.default:
                if len(args) != 1:
                    raise AssertionError(f"expected 1 arg, got {len(args)}")
                self._maybe_record_function(args[0])
            elif func == smith.ops.profiler._record_function_exit._RecordFunction:
                self._maybe_exit_record_function()

        # Handle DebugMode._annotate()
        if func is smith.ops.debug_mode_ops.annotate.default:
            if len(args) != 1:
                raise AssertionError(f"expected 1 arg, got {len(args)}")
            self._handle_annotate(args[0])
            return

        from smith.distributed._local_tensor import LocalTensor

        # Record the operation with its call depth
        call = None
        if smith.distributed.tensor.DTensor in types:
            call = _OpCall(
                func, args, kwargs, self.call_depth, stack=self.record_stack_trace
            )
            self._record_call(call)
            ...
            if self.record_faketensor:
                if func != smith.ops.prim.device.default:
                    call = _OpCall(
                        func,
                        args,
                        kwargs,
    ...
    def __enter__(self):
        ...
        if self.record_smithfunction:
            smith._C._push_on_smith_function_stack(self)
        ...
        if self.record_stack_trace:
            self.anomaly_for_traces = smith.autograd.set_detect_anomaly(
    ...
    def dispatch_hooks(
        ...
        """
        Allows installing post-hooks on arguments to intercepted __smith_dispatch__ calls;
        hook signatures are expected as (func, types, args, kwargs, result),
        i.e. __smith_dispatch__ args + return value.
    ...
    def record_outputs():
        ...
        def dispatch_hook(func, types, args, kwargs, result):
            out = tree_map(
                lambda x: x.clone() if isinstance(x, smith.Tensor) else x, result
            )
            return {"output": out}
    ...
    def log_tensor_hashes(
        ...
        hash_fn: One of:
            ...
            - String: one of ("norm", "hash_tensor")
                ...
                - "hash_tensor": uses smith.hash_tensor (XOR sum reduction)
        ...
        def _tree_hash(obj):
            return tree_map(
                lambda x: fn(x) if isinstance(x, smith.Tensor) else None, obj
            )
    ...
    def _annotate(tag: Any) -> None:
        ...
        """
        smith.ops.debug_mode_ops.annotate(tag)
    ...
    def check_hash_mismatches(
        ...
        Returns:
            List of dictionaries describing hash mismatches. Each dict contains:
                - call_type: "smith op" or "triton kernel"
                - call: Operator/kernel name
                - arg_name: For triton kernels, the argument name; None for smith ops
                - pytree_path: For smith ops, the pytree path to the differing tensor; None for kernels
