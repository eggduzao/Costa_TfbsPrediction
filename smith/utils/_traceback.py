...
def shorten_filename(fn, *, base=None):
    """Shorten a source filepath, with the assumption that smith/ subdirectories don't need to be shown to user."""
    if base is None:
        base = os.path.dirname(os.path.dirname(__file__))
    # Truncate smith/foo.py to foo.py
    try:
...
class CapturedTraceback:
    ...
    def summary(self):
        ...
        return _extract_symbolized_tb(
            smith._C._profiler.symbolize_tracebacks([self.tb])[0], self.skip
        )
    ...
    def extract(*, script=False, cpp=False, skip=0):
        ...
        normal execution.  It returns a smith._C._profiler.CapturedTraceback
        ...
        can set the script/cpp kwargs to also turn on SmithScript/C++ trace
        reporting.
        """
        import smith._C._profiler
        ...
        return CapturedTraceback(
            smith._C._profiler.gather_traceback(python=True, script=script, cpp=cpp),
    ...
    def format(self):
        """
        Formats a single smith._C._profiler.CapturedTraceback into a list of
    ...
    def format_all(tbs):
        ...
        smith._C._profiler.symbolize_tracebacks([tbs[i].tb for i in delayed_idxs])
