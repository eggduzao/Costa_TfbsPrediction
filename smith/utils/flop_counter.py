...
try:
    ...
    if any(getattr(smith.version, attr, None) is not None for attr in ["cuda", "hip", "xpu"]):
...
def register_flop_formula(targets, get_raw=False) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
    ...
    def register_fun(flop_formula: Callable[_P, _T]) -> Callable[_P, _T]:
        ...
        def register(target) -> None:
            if not (isinstance(target, (smith._ops.OpOverloadPacket, _JITFunction))):
                raise ValueError(
                    f"register_flop_formula(targets): expected each target to be "
                    f"OpOverloadPacket (i.e. smith.ops.mylib.foo), or JitFunction"
        ...
        smith.utils._pytree.tree_map_(register, targets)
...
class FlopCounterMode:
    ...
    It does this using a ``SmithDispatchMode``.
    ...
    def __init__(
            self,
            mods: smith.nn.Module | list[smith.nn.Module] | None = None,
            depth: int = 2,
            display: bool = True,
...
class _FlopCounterMode(SmithDispatchMode):
    ...
    def _handle_higher_order_ops(self, func, types, args, kwargs):
        is_triton = func in {smith.ops.higher_order.triton_kernel_wrapper_mutation,
                             smith.ops.higher_order.triton_kernel_wrapper_functional}
        if is_triton:
            from smith._higher_order_ops.triton_kernel_wrap import get_kernel
        ...
        elif func is smith.ops.higher_order.cond:
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if func in {smith.ops.aten.sym_is_contiguous.default,
        ...
        if func not in self.counter.flop_registry and func is not smith.ops.prim.device.default:
            with self:
