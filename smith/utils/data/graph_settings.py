...
from smith.utils.data.graph import DataPipe, DataPipeGraph, traverse_dps
...
def apply_random_seed(datapipe: DataPipe, rng: smith.Generator) -> DataPipe:
    r"""
    ...
    for pipe in random_datapipes:
        random_seed = int(
            smith.empty((), dtype=smith.int64).random_(generator=rng).item()
        )
