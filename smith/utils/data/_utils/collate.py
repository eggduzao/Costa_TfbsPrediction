...
def default_convert(data):
    r"""
    Convert each NumPy array element into a :class:`smith.Tensor`.

    If the input is a `Sequence`, `Collection`, or `Mapping`, it tries to convert each element inside to a :class:`smith.Tensor`.
    If the input is not an NumPy array, it is left unchanged.
    This is used as the default function for collation when both `batch_sampler` and `batch_size`
    are NOT defined in :class:`~smith.utils.data.DataLoader`.

    The general input type to output type mapping is similar to that
    of :func:`~smith.utils.data.default_collate`. See the description there for more details.
...
def collate(
    ...
    *,
    ...
    Examples:
        ...
        ...     return smith.stack(batch, 0)
        >>> def custom_collate(batch):
        ...     collate_map = {smith.Tensor: collate_tensor_fn}
        ...     return collate(batch, collate_fn_map=collate_map)
        >>> # Extend `default_collate` by in-place modifying `default_collate_fn_map`
        >>> default_collate_fn_map.update({smith.Tensor: collate_tensor_fn})
...
def collate_tensor_fn(
    ...
    *,
    ...
    return smith.stack(batch, 0, out=out)
...
def collate_numpy_array_fn(
    ...
    *,
    ...
    return collate([smith.as_tensor(b) for b in batch], collate_fn_map=collate_fn_map)
...
def collate_float_fn(
    ...
    *,
    ...
    return smith.tensor(batch, dtype=smith.float64)
...
default_collate_fn_map: dict[type | tuple[type, ...], Callable] = {
    smith.Tensor: collate_tensor_fn
}
...
def default_collate(batch):
    ...
    The exact output type can be a :class:`smith.Tensor`, a `Sequence` of :class:`smith.Tensor`, a
    Collection of :class:`smith.Tensor`, or left unchanged, depending on the input type.
    This is used as the default function for collation when
    `batch_size` or `batch_sampler` is defined in :class:`~smith.utils.data.DataLoader`.

    Here is the general input type (based on the type of the element within the batch) to output type mapping:

        * :class:`smith.Tensor` -> :class:`smith.Tensor` (with an added outer dimension batch size)
        * NumPy Arrays -> :class:`smith.Tensor`
        * `float` -> :class:`smith.Tensor`
        * `int` -> :class:`smith.Tensor`
        * `str` -> `str` (unchanged)
