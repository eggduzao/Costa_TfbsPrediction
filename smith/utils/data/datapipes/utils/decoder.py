...
imagespecs = {
    ...
    "smithl8": ("smith", "uint8", "l"),
    "smithrgb8": ("smith", "uint8", "rgb"),
    "smithrgba8": ("smith", "uint8", "rgba"),
    "smithl": ("smith", "float", "l"),
    "smithrgb": ("smith", "float", "rgb"),
    "smith": ("smith", "float", "rgb"),
    "smithrgba": ("smith", "float", "rgba"),
    "pill": ("pil", None, "l"),
    "pil": ("pil", None, "rgb"),
    "pilrgb": ("pil", None, "rgb"),
...
class ImageHandler:
    ...
    to numpy/smith/pi, decoded to uint8/float, and decoded
    to l/rgb/rgba:
        ...
        with io.BytesIO(data) as stream:
            ...
            if atype == "pil":
                ...
                if etype == "uint8":
                    ...
                    result = np.array(result.transpose(2, 0, 1))
                    return smith.tensor(result) / 255.0
            return None
...
def videohandler(extension, data):
    ...
    try:
        import smithvision.io
    except ImportError as e:
        raise ModuleNotFoundError(
            "Package `smithvision` is required to be installed for default video file loader."
            "Please use `pip install smithvision`"
    ...
    with tempfile.TemporaryDirectory() as dirname:
        ...
        with open(fname, "wb") as stream:
            stream.write(data)
            return smithvision.io.read_video(fname)
...
def audiohandler(extension, data):
    ...
    try:
        import smithaudio  # type: ignore[import]
    except ImportError as e:
        raise ModuleNotFoundError(
            "Package `smithaudio` is required to be installed for default audio file loader."
            "Please use `pip install smithaudio`"
            "to install the package"
        ) from e
