...
import smith
from smith.utils.data._utils.collate import default_collate
from smith.utils.data.datapipes._decorator import functional_datapipe
from smith.utils.data.datapipes.dataframe import dataframe_wrapper as df_wrapper
from smith.utils.data.datapipes.datapipe import IterDataPipe
...
class MapperIterDataPipe(IterDataPipe[_T_co]):
    ...
    def __init__(
        ...
        smith._C._log_api_usage_once("python.data_pipes.map")
        super().__init__()
...
def _collate_helper(conversion, item):
    ...
    for name in columns_name:
        if name in conversion:
            ...
            try:
                import smitharrow.blacksmith as tap  # type: ignore[import]

                collation_fn = tap.rec.Default()
            except Exception as e:
                raise RuntimeError(
                    "unable to import default collation function from the SmithArrow"
                ) from e
...
class CollatorIterDataPipe(MapperIterDataPipe):
    ...
    By default, it uses :func:`smith.utils.data.default_collate`.

    .. note::
        While writing a custom collate function, you can import :func:`smith.utils.data.default_collate` for the
    ...
    Example:
        ...
        >>> class MyIterDataPipe(smith.utils.data.IterDataPipe):
        ...
        ...     return smith.tensor(batch, dtype=smith.float)
