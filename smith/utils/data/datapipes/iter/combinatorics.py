...
from smith.utils.data.sampler import Sampler, SequentialSampler
...
class ShufflerIterDataPipe(IterDataPipe[_T_co]):
    ...
    When it is used with :class:`smith.utils.data.DataLoader`, the methods to
    set up random seed are different based on :attr:`num_workers`.

    For single-process mode (:attr:`num_workers == 0`), the random seed is set before
    the :class:`~smith.utils.data.DataLoader` in the main process. For multi-process
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> from smithdata.datapipes.iter import IterableWrapper
    ...
    def reset(self) -> None:
        ...
        if self._enabled:
            if self._seed is None:
                self._seed = int(smith.empty((), dtype=smith.int64).random_().item())
