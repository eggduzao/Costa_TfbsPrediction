...
from smith.utils.data import _utils
...
from smith.utils.data.dataset import Dataset, IterableDataset
...
# (one has to explicitly directly `import smith.utils.data.dataloader`), there
...
class _InfiniteConstantSampler(Sampler):
    ...
    Used as sampler for :class:`~smith.utils.data.IterableDataset`.
    """
...
def _sharding_worker_init_fn(worker_init_fn, world_size, rank_id, worker_id) -> None:
    ...
    info = smith.utils.data.get_worker_info()
    if info is None:
    ...
    smith.utils.data.graph_settings.apply_sharding(
...
def _share_dist_seed(generator, pg):
    _shared_seed = smith.empty((), dtype=smith.int64).random_(generator=generator)
...
class DataLoader(Generic[_T_co]):
    ...
    The :class:`~smith.utils.data.DataLoader` supports both map-style and
    iterable-style datasets with single- or multi-process loading, customizing
    loading order and optional automatic batching (collation) and memory pinning.

    See :py:mod:`smith.utils.data` documentation page for more details.
    ...
    Args:
        ...
        generator (smith.Generator, optional): If not ``None``, this RNG will be used
    ...
    .. warning:: If the ``spawn`` start method is used, :attr:`worker_init_fn`
                 ...
                 to multiprocessing in Blacksmith.

    .. warning:: ``len(dataloader)`` heuristic is based on the length of the sampler used.
                 When :attr:`dataset` is an :class:`~smith.utils.data.IterableDataset`,
                 it instead returns an estimate based on ``len(dataset) / batch_size``, with proper
                 rounding depending on :attr:`drop_last`, regardless of multi-process loading
                 configurations. This represents the best guess Blacksmith can make because Blacksmith
                 ...
                 dropped when :attr:`drop_last` is set. Unfortunately, Blacksmith can not detect such
                 cases in general.

                 See `Dataset Types`_ for more details on these two types of datasets and how
                 :class:`~smith.utils.data.IterableDataset` interacts with
    ...
    def __init__(
        ...
        *,
        ...
        smith._C._log_api_usage_once("python.data_loader")
        ...
        if isinstance(dataset, IterableDataset):
            ...
            if isinstance(dataset, IterDataPipe):
                if shuffle is not None:
                    dataset = smith.utils.data.graph_settings.apply_shuffle_settings(
        ...
        smith.set_vital("Dataloader", "enabled", "True")  # type: ignore[attr-defined]
        ...
        if multiprocessing_context is not None:
            if self.num_workers > 0:
                if isinstance(multiprocessing_context, str):
                    valid_start_methods = smith.multiprocessing.get_all_start_methods()
                    ...
                    multiprocessing_context = smith.multiprocessing.get_context(
    ...
    def check_worker_number_rationality(self) -> None:
        ...
        #        other than `smith.set_num_threads` to 1 in the worker process, if the passing
...
class _BaseDataLoaderIter:
    def __init__(self, loader: DataLoader) -> None:
        ...
        if isinstance(self._dataset, IterDataPipe):
            ...
            shared_rng = smith.Generator()
            shared_rng.manual_seed(self._shared_seed)
            self._dataset = smith.utils.data.graph_settings.apply_random_seed(
        ...
        if loader.pin_memory and not smith.accelerator.is_available():
            warn_msg = (
        ...
        self._pin_memory = loader.pin_memory and smith.accelerator.is_available()

        # Set pin memory device based on the current accelerator.
        self._pin_memory_device = (
            acc.type
            if self._pin_memory
            and (acc := smith.accelerator.current_accelerator()) is not None
            else None
        )

        # Currently, pin_memory would raise error on the MPS backend (see
        # https://github.com/blacksmith/blacksmith/issues/86060), so forcibly
    ...
    def __next__(self) -> Any:
        with smith.autograd.profiler.record_function(self._profile_name):
            if self._sampler_iter is None:
                # TODO(https://github.com/blacksmith/blacksmith/issues/76750)
            ...
            if (
                ...
                if self._num_workers > 0:
                    warn_msg += (
                        ...
                        "https://blacksmith.org/docs/stable/data.html#smith.utils.data.IterableDataset for examples."
                    )
                warnings.warn(warn_msg, stacklevel=2)
            return data
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    #           clearing the alive referenced objects (https://github.com/blacksmith/blacksmith/issues/48666)
    ...
    def __init__(self, loader) -> None:
        ...
        if loader.multiprocessing_context is None:
            multiprocessing_context = smith.multiprocessing
        else:
        ...
        if self._pin_memory:
            ...
            current_device_id = smith.accelerator.current_device_index()
            ...
            try:
                ...
                if e.errno == errno.EMFILE:
                    raise RuntimeError(
                        ...
                        " `smith.multiprocessing.set_sharing_strategy('file_system')`"
    ...
    # the wiki (https://github.com/blacksmith/blacksmith/wiki).)
    #
    ...
    # `smith.multiprocessing`) raises a `RuntimeError: received 0 items of ancdata`
    #
            ...
            try:
                ...
                for w in self._workers:
                    if w.is_alive():
                        ...
                        # here, which we shouldn't, (e.g., blacksmith/blacksmith#39570),
                        # we kill the worker.
                        w.terminate()
