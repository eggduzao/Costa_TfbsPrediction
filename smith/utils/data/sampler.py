...
class Sampler(Generic[_T_co]):
    ...
    Example:
        ...
        >>>         sizes = smith.tensor([len(x) for x in self.data])
        >>>         yield from smith.argsort(sizes).tolist()
        >>>
        ...
        >>>         sizes = smith.tensor([len(x) for x in self.data])
        >>>         for batch in smith.chunk(smith.argsort(sizes), len(self)):
        >>>             yield batch.tolist()

    .. note:: The :meth:`__len__` method isn't strictly required by
              :class:`~smith.utils.data.DataLoader`, but is expected in any
              calculation involving the length of a :class:`~smith.utils.data.DataLoader`.
    """
    ...
    # data, e.g., `smith.utils.data.Sampler`, with its subclasses optionally
...
class RandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        if self.generator is None:
            seed = int(smith.empty((), dtype=smith.int64).random_().item())
        ...
        if self.replacement:
            for _ in range(self.num_samples // 32):
                yield from smith.randint(
                    high=n, size=(32,), dtype=smith.int64, generator=generator
                ).tolist()
            yield from smith.randint(
                high=n,
            ...
            for _ in range(self.num_samples // n):
                yield from smith.randperm(n, generator=generator).tolist()
            yield from smith.randperm(n, generator=generator).tolist()[
                : self.num_samples % n
            ]
...
class SubsetRandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        for i in smith.randperm(len(self.indices), generator=self.generator).tolist():
...
class WeightedRandomSampler(Sampler[int]):
    ...
    def __init__(
        ...
        weights_tensor = smith.as_tensor(weights, dtype=smith.double)
