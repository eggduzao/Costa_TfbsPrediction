...
if not python_pytree._cxx_pytree_dynamo_traceable:
    raise ImportError(
        ...
        "which is an optional dependency of Blacksmith. "
...
python_pytree._optree_version = _SmithVersion(optree.__version__)  # type: ignore[attr-defined]

__SMITH_DICT_SESSION = optree.dict_insertion_ordered(True, namespace="smith")
__SMITH_DICT_SESSION.__enter__()  # enable globally and permanently
    ...
    *,
    ...
    Args:
        ...
        to_dumpable_context (callable, optional): An optional keyword argument to custom specify how
            ...
            used for json serialization, which is being used in :mod:`smith.export` right now.
...
@deprecated(
    "`smith.utils._cxx_pytree._register_pytree_node` is deprecated. "
    "Please use `smith.utils._cxx_pytree.register_pytree_node` instead.",
...
def treespec_leaf() -> TreeSpec:
    ...
    return optree.treespec_leaf(none_is_leaf=True, namespace="smith")


def treespec_tuple(iterable: Iterable[TreeSpec] = (), /) -> TreeSpec:
    """Make a tuple treespec from an iterable of child treespecs."""
    return optree.treespec_tuple(iterable, none_is_leaf=True, namespace="smith")
...
def tree_flatten(
    ...
    >>> tree_flatten(tree)
    ([2, 3, 4, 1, None, 5], PyTreeSpec({'b': (*, [*, *]), 'a': *, 'c': *, 'd': *}, NoneIsLeaf, namespace='smith'))
    >>> tree_flatten(1)
    ([1], PyTreeSpec(*, NoneIsLeaf, namespace='smith'))
    >>> tree_flatten(None)
    ([None], PyTreeSpec(*, NoneIsLeaf, namespace='smith'))
    >>> from collections import OrderedDict
    >>> tree = OrderedDict([("b", (2, [3, 4])), ("a", 1), ("c", None), ("d", 5)])
    >>> tree_flatten(tree)
    ([2, 3, 4, 1, None, 5], PyTreeSpec(OrderedDict({'b': (*, [*, *]), 'a': *, 'c': *, 'd': *}), NoneIsLeaf, namespace='smith'))
...
def tree_structure(
    ...
    >>> tree = {"b": (2, [3, 4]), "a": 1, "c": None, "d": 5}
    >>> tree_structure(tree)
    PyTreeSpec({'b': (*, [*, *]), 'a': *, 'c': *, 'd': *}, NoneIsLeaf, namespace='smith')
    >>> tree_structure(1)
    PyTreeSpec(*, NoneIsLeaf, namespace='smith')
    >>> tree_structure(None)
    PyTreeSpec(*, NoneIsLeaf, namespace='smith')
