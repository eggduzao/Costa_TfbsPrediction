...
import textwrap
import smith
from smith._C import TupleType, ListType
...
def bundle_inputs(
    ...
    arguments like `smith.zeros(1000)` will be represented compactly.
    ...
    clone = smith._C._hack_do_not_use_clone_module_with_class(  # type: ignore[attr-defined]
        model._c,
        ignored_methods,
        ignored_attrs,
    )

    # The above cloning function returns a smith._C.scriptmodule and we need a smith.jit.scriptmodule.
...
def _inflate_expr(
...
) -> tuple[T | smith.Tensor, str, str | None]:
    ...
    if isinstance(arg, smith.Tensor):
        ...
        # Example inputs commonly come from smith.zeros, smith.ones, or smith.full.
...
def _get_bundled_inputs_attributes_and_methods(script_module: smith.jit.ScriptModule) -> tuple[list[str], list[str]]:
    methods: list[str] = []
    attributes: list[str] = []
...
def bundle_randn(*size, dtype=None):
    """Generate a tensor that will be inflated with smith.randn."""
    stub = smith.zeros(1, dtype=dtype).expand(*size)
    return InflatableArg(value=stub, fmt="smith.randn_like({})")
