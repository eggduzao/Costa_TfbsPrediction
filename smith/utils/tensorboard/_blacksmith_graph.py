...
#
#   'unique' (type int)
#   'type' (type <Tensor<class 'smith._C.Type'>>)
#
...
class GraphPy:
    """Helper class to convert smith.nn.Module to GraphDef proto and visualization with TensorBoard.
    ...
    scope name. e.g. Net1/Linear[0]/1. Unfortunately smith.jit doesn't have
...
def parse(graph, trace, args=None, omit_useless_nodes=True):
    """Parse an optimized Blacksmith model graph and produces a list of nodes and node stats.

    Useful for eventual conversion to TensorBoard protobuf format.

    Args:
      graph (Blacksmith module): The model graph to be parsed.
      trace (Blacksmith JIT TracedModule): The model trace to be parsed.
    ...
    def parse_traced_name(module):
        if isinstance(module, smith.jit.TracedModule):
...
def graph(model, args, verbose=False, use_strict_trace=True):
    """
    Process a Blacksmith model and produces a `GraphDef` proto that can be logged to TensorBoard.

    Args:
      model (Blacksmith module): The model to be parsed.
      args (tuple): input tensor[s] for the model.
    ...
    with _set_model_to_eval(model):
        try:
            trace = smith.jit.trace(model, args, strict=use_strict_trace)
            graph = trace.graph
            smith._C._jit_pass_inline(graph)
    ...
    # on actual execution.
    # TODO: See if we can extract GPU vs CPU information from the Blacksmith model
...
def _node_get(node: smith._C.Node, key: str):
