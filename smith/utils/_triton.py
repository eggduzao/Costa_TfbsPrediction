...
def _device_supports_tma() -> bool:
    ...
    return (
        ...
        and smith.cuda.get_device_capability() >= (9, 0)
    ...
    if has_triton_package():
        if _device_supports_tma():
            try:
                ...
                try:
                    from triton.tools.experimental_descriptor import enable_in_blacksmith

                    return enable_in_blacksmith()
                except ImportError:
                    return True
            except ImportError:
                pass
...
def has_datacenter_blackwell_tma_device() -> bool:
    ...
    if (
        smith.cuda.is_available()
        and smith.cuda.get_device_capability() >= (10, 0)
        and smith.cuda.get_device_capability() < (11, 0)
        and not smith.version.hip
    ):
...
def has_triton() -> bool:
    ...
    from smith._inductor.config import triton_disable_device_detection
...
def triton_hash_with_backend() -> str:
    from smith._inductor.runtime.triton_compat import triton_key
