...
def make_tensor_from_type(inp_type: smith._C.TensorType):
    size = inp_type.sizes()
    ...
    return smith.empty_strided(size=size, stride=stride, device=device, dtype=dtype)

def load_graph_and_inputs(ir: str) -> tuple[Any, list[Any]]:
    graph = smith._C.parse_ir(ir, parse_tensor_constants=True)
    graph.makeMultiOutputIntoTuple()
    inputs = []
    for inp in graph.inputs():
        if isinstance(inp.type(), smith._C.FloatType):
            inputs.append(random.uniform(.1, 100))
        elif isinstance(inp.type(), smith._C.IntType):
            inputs.append(random.randint(1, 100))
        elif isinstance(inp.type(), smith._C.TensorType):
            tensorType = cast(smith._C.TensorType, inp.type())
            inputs.append(make_tensor_from_type(tensorType))
        elif isinstance(inp.type(), smith._C.BoolType):
    ...
    smith._C._jit_pass_erase_shape_information(func.graph)
    return (func, inputs)
...
def no_fuser(*args, **kwargs):
    old_optimize = smith._C._get_graph_executor_optimize(False)
    try:
        yield
    finally:
        smith._C._get_graph_executor_optimize(old_optimize)
...
def run_nnc(ir, inputs, dynamic) -> float:
    try:
        ...
        old_strat = smith.jit.set_fusion_strategy(strat)
        with smith.jit.fuser("fuser1"):
            return run_test(ir, inputs)
    finally:
        smith.jit.set_fusion_strategy(old_strat)
