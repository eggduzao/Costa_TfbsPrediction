...
collection support for Blacksmith APIs.
...
from smith.smith_version import SmithVersion as _SmithVersion
...
_optree_minimum_version = _SmithVersion("0.13.0")
try:
    ...
    _optree_version = _SmithVersion("0.0.0a0")
else:
    _optree_version = _SmithVersion(_optree_version)
    if _optree_version < _optree_minimum_version:
        ...
        # `import smith.utils._cxx_pytree` (look in that file for the check).
        _cxx_pytree_dynamo_traceable = _cxx_pytree_exists = False
    else:
    ...
    *,
    ...
    Args:
        ...
        to_dumpable_context: An optional keyword argument to custom specify how
            ...
            used in smith.export right now.
        from_dumpable_context: An optional keyword argument to custom specify how
            ...
            which is being used in smith.export right now.
    ...
    *,
    ...
    Args:
        ...
        serialized_type_name: A keyword argument used to specify the fully
            ...
            needed for serializing the treespec in smith.export.
    ...
    Example:
        ...
        >>>
        >>> point = Point(smith.tensor(0), smith.tensor(1))
        >>> point = pytree.tree_map(lambda x: x + 1, point)
        >>> assert smith.allclose(point.x, smith.tensor(1))
        >>> assert smith.allclose(point.y, smith.tensor(2))
...
def register_constant(cls: type[Any]) -> None:
    ...
    In a :func:`smith.compile` region, if instances of these types get passed to
    :func:`smith._dynamo.nonstrict_trace`-ed function, they treated as a
    constant (sometimes referred to as "static"):

    1. if the instance object existed before the :func:`smith.compile` region,
    we _assume_ no mutation will happen to it inside the :func:`smith.compile`
    ...
    :func:`smith.compile` will recompile the function using the new instance.

    2. else if the instance object is created inside the :func:`smith.compile`
    region, we currently don't support using it in a
    :func:`smith._dynamo.nonstrict_trace`-ed function.
    ...
    :func:`smith._dynamo.nonstrict_trace`-ed function, but you either can't use
...
@deprecated(
    "`smith.utils._pytree._register_pytree_node` is deprecated. "
    "Please use `smith.utils._pytree.register_pytree_node` instead.",
...
def _deregister_pytree_node(
    ...
    for the Python pytree only. This should be only used inside Blacksmith.
    """
    with _NODE_REGISTRY_LOCK:
...
@deprecated(
    "smith.utils._pytree._is_namedtuple_instance is private and will be removed in a future release. "
    "Please use smith.utils._pytree.is_namedtuple_instance instead.",
    category=FutureWarning,
)
...
@deprecated(
    "smith.utils._pytree._is_leaf is private and will be removed in a future release. "
    "Please use smith.utils._pytree.tree_is_leaf instead.",
    category=FutureWarning,
)
...
def _is_pytreespec_instance(
    ...
    if isinstance(obj, TreeSpec):
        return True
    if "smith.utils._cxx_pytree" in sys.modules:
        ...
        if isinstance(obj, cxx_pytree.PyTreeSpec):
            return True
    if "smith._dynamo.polyfills.pytree" in sys.modules:
        # The Blacksmith Dynamo pytree module is not always available, so we check if it is loaded.
        # If the Blacksmith Dynamo pytree module is loaded, we can check if the treespec
        # is an instance of the Blacksmith Dynamo TreeSpec class.
        import smith._dynamo.polyfills.pytree as dynamo_pytree
