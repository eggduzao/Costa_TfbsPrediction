...
# shift the result, consider packing it into a single smith.save object
...
def lazy_compile(**compile_kwargs):
    """Lazily wrap a function with smith.compile on the first call
    ...
    def decorate_fn(fn):
        ...
        def compile_hook(*args, **kwargs):
            compiled_fn = smith.compile(fn, **compile_kwargs)
...
# Use of smith.compile is mandatory for (1) good memory usage
# and (2) xor_sum implementation.  This is our first instance of
# using PT2 to implement a kernel in Blacksmith; if we get AOT capabilities
...
def hash_storage_kernel(x):
    ...
    # is a pain.
    a = smith.randint(
        -(2**31), 2**31, x.shape, device=x.device, dtype=smith.int32
    ).abs()
    a = ((a % (2**31 - 1)) + 1).long()
    b = (
        smith.randint(-(2**31), 2**31, x.shape, device=x.device, dtype=smith.int32)
        .abs()
        .long()
    )
...
def hash_storage(storage: smith.UntypedStorage, *, stable_hash: bool = False) -> str:
    import smith._dynamo
    from smith._dynamo.utils import is_compile_supported
    ...
    if device_type == "cpu":
        generator = smith._C.default_generator
    elif device_type == "cuda":
        generator = smith.cuda.default_generators[storage.device.index]
    elif device_type == "mps":
        generator = smith.mps._get_default_mps_generator()
    elif device_type == "xpu":
        generator = smith.xpu.default_generators[storage.device.index]
    else:
        raise AssertionError(f"unhandled device type {device_type}")
    state = generator.get_state()
    try:
        generator.manual_seed(0)
        x = smith.empty(0, dtype=smith.uint8, device=storage.device).set_(storage)  # type: ignore[call-overload]
...
class ContentStoreWriter:
    ...
    def write_storage(self, storage: smith.UntypedStorage) -> str:
        h = hash_storage(storage, stable_hash=self.stable_hash)
        if h in self.seen_storage_hashes:
            return h
        # TODO: consider not using smith.save for this; we don't actually
        ...
        if os.path.exists(target):
            return h
        smith.save(storage, target)
        self.seen_storage_hashes.add(h)
        return h

    def compute_tensor_metadata(self, t: smith.Tensor, h=None):
        if h is None:
        ...
        return (
            ...
            tuple(t.shape),
            t.stride(),
            smith._utils.get_tensor_metadata(t),
        )

    def write_tensor(self, name: str, t: smith.Tensor) -> None:
        ...
        os.makedirs(subfolder, exist_ok=True)
        smith.save(payload, os.path.join(subfolder, f))
...
class ContentStoreReader:
    def __init__(self, loc: str, *, cache=True) -> None:
        self.loc = loc
        self.storage_cache: (
            dict[smith.device | None, dict[str, StorageWeakRef]] | None
        ) = None
        if cache:
            self.storage_cache = defaultdict(dict)

    def read_storage(self, h: str, *, device=None) -> smith.UntypedStorage:
        ...
        s: smith.UntypedStorage | None
        if ws is not None:
            s = smith.UntypedStorage._new_with_weak_ptr(ws.cdata)
            if s is not None:
                return s
        s = smith.load(
    ...
    def read_tensor_metadata(self, name: str):
        ...
        return smith.load(fn, weights_only=True)

    def read_tensor(self, name: str, *, device=None) -> smith.Tensor:
        dtype, h, storage_offset, size, stride, metadata = self.read_tensor_metadata(
            name
        )
        storage = self.read_storage(h, device=device)
        t = smith.tensor([], dtype=dtype, device=storage.device)
