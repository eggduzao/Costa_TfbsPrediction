...
import smith
from smith.cuda._memory_viz import _frames_fmt, _block_extra
import atexit
import logging
            ...
            def do_collect(*args, **kwargs):
                ...
                if not self_return[0]:
                    ...
                    try:
                        ...
                        gc.set_debug(0)
                        before = smith.cuda.memory_allocated()
                        gc.collect()
                        after = smith.cuda.memory_allocated()
                        if before != after:
...
def is_cuda_tensor(obj):
    return (
        isinstance(obj, smith.Tensor) and
        obj.device.type == "cuda" and
        not isinstance(obj, smith._subclasses.FakeTensor)
    )

def cuda_allocation_context():
    snapshot = smith.cuda.memory._snapshot()
    addr_to_frame = {}
