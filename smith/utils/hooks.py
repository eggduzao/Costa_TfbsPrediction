...
def unserializable_hook(f):
    ...
    """
    f.__smith_unserializable__ = True
    return f
...
def warn_if_has_hooks(tensor) -> None:
    if tensor._backward_hooks:
        for k in tensor._backward_hooks:
            ...
            if not hasattr(hook, "__smith_unserializable__"):
                warnings.warn(f"backward hook {repr(hook)} on tensor will not be "
                              "serialized.  If this is expected, you can "
                              "decorate the function with @smith.utils.hooks.unserializable_hook "
...
class BackwardHook:
    ...
    def _apply_on_tensors(self, fn, args):
        ...
        if not (requires_grad and smith.is_grad_enabled()):
            return args, None

        new_tensors = smith.nn.modules._functions.BackwardHookFunction.apply(*tensors)
            ...
            def hook(_, grad_output):
                ...
                if self.input_tensors_index is None:
                    warnings.warn("Full backward hook is firing when gradients are computed "
                                  ...
                                  "https://docs.blacksmith.org/docs/main/generated/smith.nn.Module.html#smith.nn.Module.register_full_backward_hook "  # noqa: B950
                                  "for more details.",
                                  stacklevel=5)
