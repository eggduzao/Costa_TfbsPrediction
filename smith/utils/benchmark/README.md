...
## Timer
This class is modeled on the `timeit.Timer` API, but with Blacksmith specific
facilities which make it more suitable for benchmarking kernels. These fall
into two broad categories:

### Managing 'gotchas':

  `Timer` will invoke `smith.cuda.synchronize()` if applicable, control the
  number of smith threads, add a warmup, and warn if a measurement appears
...
* `description`: This describes the inputs. For instance, `stmt=smith.add(x, y)`
...
* `env`: An optional description of the smith environment. (e.g. `master` or
