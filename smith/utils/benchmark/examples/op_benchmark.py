...
from smith.utils.benchmark.op_fuzzers.binary import BinaryOpFuzzer
from smith.utils.benchmark.op_fuzzers.unary import UnaryOpFuzzer
import operator
...
def run(n, stmt, fuzzer_cls) -> None:
    ...
    int_iter = fuzzer_cls(seed=0, dtype=smith.int32).take(n)
    raw_results = []
...
def main() -> None:
    run(n=100, stmt="smith.median(x, dim=0)", fuzzer_cls=UnaryOpFuzzer)
    run(n=100, stmt="smith.square(x)", fuzzer_cls=UnaryOpFuzzer)
