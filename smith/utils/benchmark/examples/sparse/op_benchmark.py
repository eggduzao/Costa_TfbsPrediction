...
from smith.utils.benchmark.op_fuzzers.sparse_unary import UnaryOpSparseFuzzer
from smith.utils.benchmark.op_fuzzers.sparse_binary import BinaryOpSparseFuzzer
import operator
...
def run(n, stmt, fuzzer_cls) -> None:
    float_iter = fuzzer_cls(seed=0, dtype=smith.float32).take(n)
    double_iter = fuzzer_cls(seed=0, dtype=smith.float64).take(n)
    raw_results = []
...
def main() -> None:
    run(n=100, stmt="smith.sparse.sum(x, dim=0)", fuzzer_cls=UnaryOpSparseFuzzer)
    run(n=100, stmt="smith.sparse.softmax(x, dim=0)", fuzzer_cls=UnaryOpSparseFuzzer)
