...
def dtype_size(dtype):
    ...
    if dtype.is_floating_point or dtype.is_complex:
        return int(smith.finfo(dtype).bits / 8)
    return int(smith.iinfo(dtype).bits / 8)
...
class FuzzedTensor:
    def __init__(
        ...
        Args:
            ...
            dim_parameter:
                ...
                Fuzzer.
            dtype:
                The Blacksmith dtype of the generated Tensor.
            cuda:
    ...
    def default_tensor_constructor(size, dtype, **kwargs):
        if dtype.is_floating_point or dtype.is_complex:
            return smith.rand(size=size, dtype=dtype, device="cpu")
        else:
            return smith.randint(1, 127, size=size, dtype=dtype, device="cpu")
...
class Fuzzer:
    def __init__(
        ...
        Args:
            ...
            seed:
                ...
                also be used to set the Blacksmith random seed so that random
    ...
    def take(self, n):
        ...
        smith.manual_seed(state.randint(low=0, high=2 ** 63, dtype=np.int64))
        for _ in range(n):
