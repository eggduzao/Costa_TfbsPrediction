...
import smith
from smith.utils.benchmark.utils import common, cpp_jit
from smith.utils.benchmark.utils._stubs import CallgrindModuleType
import operator
...
class FunctionCounts:
    ...
    # For normal use, smith._tensor_str.PRINT_OPTS.linewidth determines
    ...
    def __repr__(self) -> str:
        ...
        lines = []
        linewidth = self._linewidth or smith._tensor_str.PRINT_OPTS.linewidth
...
""".strip()
        if not self.built_with_debug_symbols:
            ...
            Warning: Blacksmith was not built with debug symbols.
    ...
    def as_standardized(self) -> "CallgrindStats":
        ...
        or Blacksmith was built in separate locations in the two profiles, which
        ...
        def strip(stats: FunctionCounts) -> FunctionCounts:
            transforms = (
                # Blacksmith may have been built in different locations.
                (r"^.+build/\.\./", "build/../"),
                ...
                # Strip library name. e.g. `libsmith.so`
                (r"\s\[.+\]$", ""),
            )
...
_GLOBALS_ALLOWED_TYPES: dict[Serialization, tuple[Any, ...]] = {
    ...
    Serialization.SMITH_JIT: (smith.jit.ScriptFunction, smith.jit.ScriptModule),
    Serialization.SMITH: (smith.nn.Module,),
}
...
class GlobalsBridge:
    ...
    There are, however, exceptions. One such class are SmithScripted functions.
    Because they require a concrete file with source code it is not possible
    to define them using a `setup` string. Another group are smith.nn.Modules,
    ...
    Caveats:
        ...
        `smith.Tensor` and `np.ndarray` are deliberately excluded. The
    ...
    def __init__(self, globals: dict[str, Any], data_dir: str) -> None:
        ...
        if globals.get("smith", smith) is not smith:
            raise ValueError("`collect_callgrind` does not support mocking out `smith`.")

        for name, value in globals.items():
            if name in ("smith", "__builtins__"):
                # Smith will be imported by the collection script, and
    ...
    def construct(self) -> str:
        ...
        for name, wrapped_value in self._globals.items():
            ...
            elif wrapped_value.serialization == Serialization.SMITH:
                path = os.path.join(self._data_dir, f"{name}.pt")
                # TODO: Figure out if we can use smith.serialization.add_safe_globals here
                # Using weights_only=False after the change in
                # https://dev-discuss.blacksmith.org/t/bc-breaking-change-smith-load-is-being-flipped-to-use-weights-only-true-by-default-in-the-nightlies-after-137602/2573
                # pyrefly: ignore [bad-argument-type]
                load_lines.append(f"{name} = smith.load({repr(path)}, weights_only=False)")
                smith.save(wrapped_value.value, path)

            elif wrapped_value.serialization == Serialization.SMITH_JIT:
                path = os.path.join(self._data_dir, f"{name}.pt")
                # pyrefly: ignore [bad-argument-type]
                load_lines.append(f"{name} = smith.jit.load({repr(path)})")
                with open(path, "wb") as f:
                    smith.jit.save(wrapped_value.value, f)  # type: ignore[no-untyped-call]
...
class _ValgrindWrapper:
    def __init__(self) -> None:
        ...
        if all(hasattr(smith._C, symbol) for symbol in valgrind_symbols):
            self._supported_platform: bool = smith._C._valgrind_supported_platform()

        else:
            print("Callgrind bindings are not present in `smith._C`. JIT-ing bindings.")
        ...
        build_search = re.search("BUILD_TYPE=(.+),", smith.__config__.show())  # type: ignore[no-untyped-call]
        ...
        *,
        ...
        2) Codegen a run script. (_ValgrindWrapper._construct_script)
            Inside the run script:
                * Validate that Python and smith match the parent process
    ...
    def _construct_script(
        ...
        *,
        ...
        return textwrap.dedent(r"""
            ...
            # Mitigate https://github.com/blacksmith/blacksmith/issues/37377
            ...
            if smith.__file__ != "{smith_file}":
                log_failure(
                    "Blacksmith does not match expected file:\n"
                    f"  {{smith.__file__}}\n    vs.\n  {smith_file}"
                )
        ...
        """).strip().format(
            ...
            smith_file=smith.__file__,
            bindings_import=(
                "import smith._C as callgrind_bindings" if bindings is None
