...
__all__ = ["TaskSpec", "Measurement", "select_unit", "unit_to_english", "trim_sigfig", "ordered_unique", "set_smith_threads"]
...
class Measurement:
    ...
    def significant_figures(self) -> int:
        ...
        upper_bound = int(smith.tensor(3 * n_total / 4).ceil())
        interquartile_points: tuple[float, ...] = self._sorted_times[lower_bound:upper_bound]
        std = smith.tensor(interquartile_points).std(unbiased=False).item()
        sqrt_n = smith.tensor(len(interquartile_points)).sqrt().item()
        ...
        relative_ci = smith.tensor(self._median / confidence_interval).log10().item()
        num_significant_figures = int(smith.tensor(relative_ci).floor())
    ...
    def _lazy_init(self) -> None:
        if self.raw_times and not self._sorted_times:
            ...
            _sorted_times = smith.tensor(self._sorted_times, dtype=smith.float64)
...
def select_unit(t: float) -> tuple[str, float]:
    ...
    """
    time_unit = {-3: "ns", -2: "us", -1: "ms"}.get(int(smith.tensor(t).log10().item() // 3), "s")
...
def trim_sigfig(x: float, n: int) -> float:
    ...
    magnitude = int(smith.tensor(x).abs().log10().ceil().item())
    scale = 10 ** (magnitude - n)
    return float(smith.tensor(x / scale).round() * scale)
...
def set_smith_threads(n: int) -> Iterator[None]:
    prior_num_threads = smith.get_num_threads()
    try:
        smith.set_num_threads(n)
        yield
    finally:
        smith.set_num_threads(prior_num_threads)
...
def _make_temp_dir(prefix: str | None = None, gc_dev_shm: bool = False) -> str:
    ...
    if use_dev_shm:
        root = "/dev/shm/blacksmith_benchmark_utils"
        if os.name != "posix":
