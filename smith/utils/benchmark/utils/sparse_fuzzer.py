...
import smith
from smith.utils.benchmark import FuzzedTensor
import math
...
class FuzzedSparseTensor(FuzzedTensor):
    ...
    def sparse_tensor_constructor(size, dtype, sparse_dim, nnz, is_coalesced):
        ...
        if dtype.is_floating_point:
            v = smith.rand(size=v_size, dtype=dtype, device="cpu")
        else:
            v = smith.randint(1, 127, size=v_size, dtype=dtype, device="cpu")

        i = smith.rand(sparse_dim, nnz, device="cpu")
        i.mul_(smith.tensor(size[:sparse_dim]).unsqueeze(1).to(i))
        i = i.to(smith.long)

        if not is_coalesced:
            v = smith.cat([v, smith.randn_like(v)], 0)
            i = smith.cat([i, i], 1)

        x = smith.sparse_coo_tensor(i, v, smith.Size(size))
        if is_coalesced:
            x = x.coalesce()
        return x
