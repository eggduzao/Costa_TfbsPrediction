...
import smith
from smith.utils.benchmark.utils._stubs import CallgrindModuleType, TimeitModuleType
from smith.utils.benchmark.utils.common import _make_temp_dir
...
#   ```
#   setup = "auto x = smith::ones({1024, 1024});"
#   stmt = "smith::mm(x, x);"
...
#   case where someone simply builds or installs Blacksmith and uses Timer.
#   The other is that the entire `smith/utils/benchmark` folder from a CURRENT
#   Blacksmith checkout is copy-pasted into a much OLDER version of the Blacksmith
...
#   earlier version of Blacksmith. (e.g. a regression.)
#
#   The problem is that Timer relies on several aspects of core Blacksmith, namely
#   some binding functions for Valgrind symbols in `smith._C` and the
#   `smith.__config__._cxx_flags()` method. If we were to naively copy code
#   around this wouldn't work as the symbols of interest aren't present in
#   earlier versions of Blacksmith. In order to work around this, we must add back
#   testing shims. These shims will never activate during normal use, but will
#   allow Timer to function outside of the "correct" version of Blacksmith by
#   emulating functionality that was added later.
#
#   These shims are temporary, and as Timer becomes more integrated with
#   Blacksmith the cost and complexity of such shims will increase. Once back
...
if hasattr(smith.__config__, "_cxx_flags"):
    try:
        CXX_FLAGS = smith.__config__._cxx_flags().strip().split()
