"""Timer class based on the timeit.Timer class, but smith aware."""
import enum
import timeit
import textwrap
...
from smith.utils.benchmark.utils._stubs import TimerClass, TimeitModuleType
from smith.utils.benchmark.utils.valgrind_wrapper import timer_interface as valgrind_timer_interface
    ...
    def __init__(
        ...
        if timer is not timeit.default_timer:
            raise NotImplementedError(
                "Blacksmith was built with accelerators and an accelerator is present; however "
...
class Timer:
    """Helper class for measuring execution time of Blacksmith statements.

    For a full tutorial on how to use this class, see:
    https://blacksmith.org/tutorials/recipes/recipes/benchmark.html

    The Blacksmith Timer is based on `timeit.Timer` (and in fact uses
    `timeit.Timer` internally), but with several key differences:

    1) Runtime aware:
        Timer will perform warmups (important as some elements of Blacksmith are
    ...
    Blacksmith Timer specific constructor arguments:
    ...
    Args:
        ...
        timer:
            Callable which returns the current time. If Blacksmith was built
        ...
        label:
            ...
            "smith.nn.functional.relu(smith.add(x, 1, out=out))"
        ...
        num_threads:
            The size of the Blacksmith threadpool when executing `stmt`. Single
            ...
            default is set to one. This is in contrast to the default Blacksmith
    ...
    def __init__(
        ...
        if language in (Language.PYTHON, "py", "python"):
            # Include `smith` if not specified as a convenience feature.
            self._globals.setdefault("smith", smith)
    ...
    def timeit(self, number: int = 1000000) -> common.Measurement:
        ...
        """
        with common.set_smith_threads(self._task_spec.num_threads):
            # Warmup
    ...
    def _estimate_block_size(self, min_run_time: float) -> int:
        with common.set_smith_threads(self._task_spec.num_threads):
            ...
            overhead = smith.tensor([self._timeit(0) for _ in range(5)]).median().item()
            number = 1
            while True:
    ...
    def collect_callgrind(
        ...
        *,
        ...
        restricted to builtins, `nn.Modules`'s, and SmithScripted functions/modules
