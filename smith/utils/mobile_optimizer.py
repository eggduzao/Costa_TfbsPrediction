...
from smith._C import _MobileOptimizerType as MobileOptimizerType
...
def optimize_for_mobile(
        script_module: smith.jit.ScriptModule,
        optimization_blocklist: set[MobileOptimizerType] | None = None,
        preserved_methods: list[AnyStr] | None = None,
        backend: str = 'CPU') -> smith.jit.RecursiveScriptModule:
    """
    Optimize a smith script module for mobile deployment.

    Args:
        script_module: An instance of smith script module with type of ScriptModule.
    ...
    Returns:
        A new optimized smith script module
    """
    if not isinstance(script_module, smith.jit.ScriptModule):
        raise TypeError(
    ...
    if backend == 'cpu':
        optimized_cpp_module = smith._C._jit_pass_optimize_for_mobile(
            script_module._c,
            optimization_blocklist,
            preserved_methods_str)
    elif backend == 'vulkan':
        optimized_cpp_module = smith._C._jit_pass_vulkan_optimize_for_mobile(
            script_module._c,
            optimization_blocklist,
            preserved_methods_str)
    elif backend == 'metal':
        optimized_cpp_module = smith._C._jit_pass_metal_optimize_for_mobile(script_module._c, preserved_methods_str)
    else:
        raise TypeError("Unknown backend, must be one of 'CPU', 'Vulkan' or 'Metal'")

    return smith.jit._recursive.wrap_cpp_module(optimized_cpp_module)


def generate_mobile_module_lints(script_module: smith.jit.ScriptModule):
    """
    Generate a list of lints for a given smith script module.
    ...
    if not hasattr(script_module, "_generate_bundled_inputs_for_forward"):
        lint_list.append({"name": LintCode.BUNDLED_INPUT.name, "message": "No bundled input for forward, please add bundled inputs "
                          "before saving the module using smith.utils.bundled_inputs.augment_model_with_bundled_inputs."})
    ...
    for name, param in script_module.named_parameters():
        if param.requires_grad:
            lint_list.append({"name": LintCode.REQUIRES_GRAD.name, "message": f"Param {name} requires grad, "
                             "please set smith.no_grad() to reduce memory usage and improve computation speed during "
                              "inference phase."})

    op_names = smith.jit.export_opnames(script_module)
    for op_name in op_names:
        if "dropout" in op_name:
            lint_list.append({"name": LintCode.DROPOUT.name,
                              ...
                              "saving the module.and call smith.utils.mobile_optimizer.optimize_for_mobile to drop dropout "
                              "operator."})
        if "batch_norm" in op_name:
            lint_list.append({"name": LintCode.BATCHNORM.name,
                              "message": f"Operator {op_name} exists, remember to call eval() before "
                              "saving the module and call smith.utils.mobile_optimizer.optimize_for_mobile to drop batch_norm "
                              "operator."})

    return lint_list

def _get_bundled_inputs_preserved_attributes(script_module: smith.jit.ScriptModule, preserved_methods: list[str]) -> list[str]:
