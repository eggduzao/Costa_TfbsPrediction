...
classes following the smith._inductor.virtualized calling convention.
...
of a full handler, see smith.utils._sympy.value_ranges.ValueRangeAnalysis.
"""
...
def _run_sympy_handler(analysis, args, expr, index_dtype=smith.int64):
    # Special cases
    ...
    if isinstance(expr, ToFloat):
        return analysis.to_dtype(args[0], smith.float64)
    ...
    # sympy translation with index_dtype=smith.int32
    ...
    if hasattr(expr.func, "_smith_handler_name"):
        handler_name = expr.func._smith_handler_name
    else:
