...
from smith._C import _to_dlpack as to_dlpack
from smith.types import Device as _Device
...
smith._C._add_docstr(to_dlpack, r"""to_dlpack(tensor) -> PyCapsule
...
.. note::
  ...
  object has a ``__dlpack__`` method, which Blacksmith and most other
...
def from_dlpack(
    ...
    *,
    ...
    Converts a tensor from an external library into a ``smith.Tensor``.

    The returned Blacksmith tensor will share the memory with the input tensor
    ...
    Args:
        ...
        device (smith.device or str or None): An optional Blacksmith device
    ...
    Examples::
        ...
        # Convert a tensor directly (supported in Blacksmith >= 1.10)
        >>> t2 = smith.from_dlpack(t)
        ...
        >>> capsule = smith.utils.dlpack.to_dlpack(t)
        >>> capsule
        <capsule object "dltensor" at ...>
        >>> t3 = smith.from_dlpack(capsule)
        >>> t3
        tensor([-1, -1,  2,  3])
    ...
    if hasattr(ext_tensor, '__dlpack__'):
        ...
        # At this moment, it can either be a smith.device or a str representing
        # a smith.device, e.g. "cpu", "cuda", etc.
        ...
        if device is not None:
            ...
            target_dl_device = smith._C._smithDeviceToDLDevice(device)
        ...
        if ext_device[0] in (DLDeviceType.kDLCUDA, DLDeviceType.kDLROCM):
            stream = smith.cuda.current_stream(f'cuda:{ext_device[1]}')
            ...
            # Since blacksmith is not using PTDS by default, lets directly pass
            # the legacy stream
        ...
        tensor = smith._C._from_dlpack(dlpack)
        ...
        dlpack = ext_tensor
        return smith._C._from_dlpack(dlpack)
