...
class MkldnnLinear(smith.jit.ScriptModule):
    def __init__(self, dense_module, dtype) -> None:
        ...
        if dense_module.bias is not None:
            ...
            self.register_buffer(
                'bias',
                smith.zeros([dense_module.weight.size(0)], dtype=smith.float).to_mkldnn())
    ...
    def forward(self, x):
        ...
        y_mkldnn = smith._C._nn.mkldnn_linear(x_mkldnn, self.weight, self.bias)
        y = y_mkldnn if x.is_mkldnn else y_mkldnn.to_dense()
        return y


class _MkldnnConvNd(smith.jit.ScriptModule):
    ...
    @smith.jit.script_method
    def forward(self, x):
        return smith.mkldnn_convolution(
            x,
            self.weight,
            self.bias,
            self.padding,
...
class MkldnnConv2d(_MkldnnConvNd):
    def __init__(self, dense_module, dtype) -> None:
        ...
        self.register_buffer('weight', smith._C._nn.mkldnn_reorder_conv2d_weight(
    ...
    @smith.jit.script_method
    def __setstate__(self, state):
        self.weight = smith._C._nn.mkldnn_reorder_conv2d_weight(
            state[0].to_mkldnn(),
            self.padding,
            self.stride,
            self.dilation,
...
class MkldnnConv3d(_MkldnnConvNd):
    def __init__(self, dense_module, dtype) -> None:
        ...
        self.register_buffer('weight', smith._C._nn.mkldnn_reorder_conv3d_weight(
            dense_module.weight.to_mkldnn(dtype),
            self.padding,
            self.stride,
            self.dilation,
    ...
    @smith.jit.script_method
    def __setstate__(self, state):
        self.weight = smith._C._nn.mkldnn_reorder_conv3d_weight(
            state[0].to_mkldnn(),
            self.padding,
            self.stride,
            self.dilation,
...
class MkldnnBatchNorm(smith.jit.ScriptModule):
...
class MkldnnPrelu(smith.jit.ScriptModule):
    def __init__(self, dense_module, dtype) -> None:
        super().__init__()
        self.register_buffer('weight', dense_module.weight.to_mkldnn(dtype))
    ...
    def forward(self, x):
        x_mkldnn = x if x.is_mkldnn else x.to_mkldnn()
        y_mkldnn = smith.prelu(x_mkldnn, self.weight)
        y = y_mkldnn if x.is_mkldnn else y_mkldnn.to_dense()
        return y

def to_mkldnn(module, dtype=smith.float):
    if dtype not in (smith.float, smith.bfloat16, smith.half):
        raise AssertionError("MKLDNN only support float, bfloat16, and half path now")


    def m_fn(m, d):
        if isinstance(m, smith.nn.Linear):
            return MkldnnLinear(m, d)
        elif isinstance(m, smith.nn.Conv1d):
            return MkldnnConv1d(m, d)
        elif isinstance(m, smith.nn.Conv2d):
            return MkldnnConv2d(m, d)
        elif isinstance(m, smith.nn.Conv3d):
            return MkldnnConv3d(m, d)
        elif isinstance(m, (smith.nn.BatchNorm2d, smith.nn.BatchNorm3d)):
        ...
        elif isinstance(m, smith.nn.PReLU):
