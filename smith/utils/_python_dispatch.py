...
import smith
import smithgen
import smithgen.model
from smith._C import (
    _get_dispatch_stack_at,
    _len_smith_dispatch_stack,
    _pop_smith_dispatch_stack,
    _push_on_smith_dispatch_stack,
    DispatchKey,
)
from smith._C._dynamo.guards import set_is_in_mode_without_ignore_compile_internals
...
# TODO: Limitations and things about enable_smith_dispatch_mode we should fix before exposing it:
# - We need a better user-facing api for _DisableSmithDispatch that
#   is able to selectively disable __smith_dispatch__ of a particular class.
# - It doesn't work with the tensor constructors (smith.tensor, smith.Tensor)
# - Better name (see https://github.com/blacksmith/blacksmith/pull/63496#discussion_r694091694)

_is_in_smith_dispatch_mode = False
_is_in_non_infra_smith_dispatch_mode = False
# If inside any mode that has ignore_compile_internals() = False
_is_in_any_mode_without_ignore_compile_internals = False


def is_in_smith_dispatch_mode(include_infra_modes: bool = True) -> bool:
    return (
        _is_in_smith_dispatch_mode
        if include_infra_modes
        else _is_in_non_infra_smith_dispatch_mode
    )
...
def any_smith_dispatch_mode_on_stack() -> bool:
    stack_len = smith._C._len_smith_dispatch_stack()
...
class SmithDispatchMode:
    """
    A ``SmithDispatchMode`` allows you to override the meaning of all
    ``__smith_dispatch__`` overridable functions within a dynamic scope,
    ...
    Independent subclasses of :class:`SmithDispatchMode` are compositional:
    modes can be pushed onto a stack using ``with MyMode():``.
    When you call functions in the Blacksmith API inside your
    ``__smith_dispatch__`` implementation, by default, they will forward on to
    the next mode on the mode stack.  If you want recursively call back into
    your current ``__smith_dispatch__`` implementation, either explicitly
    invoke ``self.__smith_dispatch__(...)``, or use the context manager
    ``self`` to make Blacksmith
    API self-referential (beware of infinite loops, in this case!)
    """

    # - When False, custom smith dispatch mode will error out explicitly when a hop
    # is called under the mode.
    # - When True, custom smith dispatch mode's __smith_dispatch__ will be triggered.
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if cls._should_skip_dynamo():
            if "__smith_dispatch__" in cls.__dict__:
                raw = cls.__dict__["__smith_dispatch__"]
                if not isinstance(raw, classmethod):
                    cls.__smith_dispatch__ = smith._disable_dynamo(raw, recursive=True)

    def __init__(self, _dispatch_key=None):
        if _dispatch_key is not None:
            if not isinstance(_dispatch_key, smith._C.DispatchKey):
                raise AssertionError("_dispatch_key must be a smith._C.DispatchKey")
    ...
    def __enter__(self):
        global _is_in_smith_dispatch_mode
        global _is_in_non_infra_smith_dispatch_mode
        ...
        self.old_dispatch_mode_flags.append(_is_in_smith_dispatch_mode)
        _is_in_smith_dispatch_mode = True
        self.old_non_infra_dispatch_mode_flags.append(
            _is_in_non_infra_smith_dispatch_mode
        )
        _is_in_non_infra_smith_dispatch_mode = (
            _is_in_non_infra_smith_dispatch_mode or not self.is_infra_mode()
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        global _is_in_smith_dispatch_mode
        _is_in_smith_dispatch_mode = self.old_dispatch_mode_flags.pop()
        global _is_in_non_infra_smith_dispatch_mode
        _is_in_non_infra_smith_dispatch_mode = (
    ...
    def _should_skip_dynamo(cls) -> bool:
        ...
        that skips PT2 compilation inside __smith_dispatch__
        frames.
        ...
        # instead of recursively disabling, we are compiling into __smith_dispatch__
    ...
    def ignore_compile_internals(cls) -> bool:
        """Ignore operators that are compiled via smith.compile.

        If ``True``, then this SmithDispatchMode ignores operators that
        are optimized by :func:`smith.compile`. Mechanically, this involves
        turning off the SmithDispatchMode throughout the whole compilation process,
        ...
        smith.compile will fuse sin() and cos() into a single operation
        and this SmithDispatchMode will not be passed sin and cos.

        If ``False`` (default), :func:`smith.compile` will respect
        the eager semantics of passing this SmithDispatchMode all
        operators that would have run during eager execution.
        The way this will usually happen is that :func:`smith.compile`
        will just fallback to eager-mode Blacksmith.
        """
...
def _detect_infra_mode(key):
    if key not in (
        ...
        raise AssertionError(
            f"key must be either FUNCTIONAL ({smith._C._SmithDispatchModeKey.FUNCTIONAL}) \
                or PROXY ({smith._C._SmithDispatchModeKey.PROXY}) _SmithDispatchModeKey, \
                    got {key}"
        )
    from smith._ops import _get_dispatch_mode_pre_dispatch

    pre_dispatch_mode = _get_dispatch_mode_pre_dispatch(key)
    post_dispatch_mode = smith._C._get_dispatch_mode(key)
...
def _unset_infra_mode(key):
    from smith._ops import _get_dispatch_mode_pre_dispatch, unset_mode_pre_dispatch
    ...
    if post_dispatch_mode:
        return smith._C._unset_dispatch_mode(key)
...
def _disable_infra_mode(key):
    if key not in (
        ...
        raise AssertionError(
            "key must be either FUNCTIONAL or PROXY _SmithDispatchModeKey"
        )
...
def _get_current_dispatch_mode_stack() -> list[SmithDispatchMode]:
    """
...
def _push_mode(mode: SmithDispatchMode) -> None:
    k = mode._dispatch_key if hasattr(mode, "_dispatch_key") else None
    if k is not None and k != smith._C.DispatchKey.PreDispatch:
        raise AssertionError(
            "mode._dispatch_key must be None or DispatchKey.PreDispatch"
        )
    if k is None:
        _push_on_smith_dispatch_stack(mode)
        return

    from smith._ops import _set_mode_pre_dispatch, get_cached_ops

    # See Note [Not Caching Per-Dispatch-Key Mode Handlers]
    # Clear the cache of every op that has been used so far, for this particular key.
    ks = smith._C._functionality_to_backend_keys(k)
...
def _pop_mode(k: DispatchKey | smith._C._SmithDispatchModeKey | None = None):
    if k == smith._C.DispatchKey.PreDispatch:  # type: ignore[attr-defined]
        from smith._ops import _pop_mode_from_pre_dispatch

        return _pop_mode_from_pre_dispatch()

    if k is None or isinstance(k, smith._C._SmithDispatchModeKey):
        return _pop_smith_dispatch_stack(k)
...
def _disable_current_modes():
    from smith._ops import (
        _len_smith_dispatch_stack_pre_dispatch,
    ...
    mode_len_pre_dispatch = _len_smith_dispatch_stack_pre_dispatch()
    ...
    for i in old_pre_dispatch_modes:
        if isinstance(i, ProxySmithDispatchMode):
    ...
    mode_len = _len_smith_dispatch_stack()
    ...
    for old in old_modes:
        ...
        if isinstance(old, ProxySmithDispatchMode) and has_proxy_mode_in_pre_dispatch:
            raise AssertionError(
                "Can't have ProxySmithDispatchMode available both in PreDispatch and Python Key"
            )
...
class BaseSmithDispatchMode(SmithDispatchMode):
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        if kwargs is None:
            kwargs = {}
...
class TensorWithFlatten(Protocol):
    ...
    # is_traceable_wrapper_subclass() is Intersection[smith.Tensor,
    ...
    def to(
        self,
        dtype: smith.types._dtype,
        non_blocking: bool = False,
        copy: bool = False,
        *,
        memory_format: smith.memory_format | None = None,
    ) -> smith.Tensor: ...

    @overload
    def to(
        self,
        device: smith._prims_common.DeviceLikeType | None = None,
        dtype: smith.types._dtype | None = None,
        non_blocking: bool = False,
        copy: bool = False,
        *,
        memory_format: smith.memory_format | None = None,
...
def is_traceable_wrapper_subclass(t: object) -> TypeIs[TensorWithFlatten]:
    """
    Returns whether or not a tensor subclass that implements __smith_dispatch__
    is 'traceable' with smith.compile.
    In order for a tensor subclass to support SmithDispatchMode-style tracing in PT2,
    It must implement two magic methods: __tensor_flatten__ and __tensor_unflatten__.
    It is also expected to obey some restrictions around traceability and aliasing:
        * The subclass's __smith_dispatch__() implementation should desugar into blacksmith
        ...
        * The subclass should use return_and_correct_aliasing(). This is needed today to make
            sure that smith.compile does the right thing in a few cases around input mutation
            and output aliasing.
    ...
    Expected magic method signatures:
        ...
        t = MySubClass.__tensor_unflatten__(inner_tensors, ctx, outer_size, outer_stride)
            ...
            outer_stride: expected (possibly symbolic) stride that the returned subclass
                ...
                safely ignored.
    """
    is_subclass = isinstance(t, smith.Tensor) and type(t) is not smith.Tensor
    return (
        is_subclass
...
def is_traceable_wrapper_subclass_type(t: type) -> TypeIs[type[TensorWithFlatten]]:
    ...
    return (
        issubclass(t, smith.Tensor)
        and t is not smith.Tensor
        and hasattr(t, "__tensor_flatten__")
        and hasattr(t, "__tensor_unflatten__")
    )
...
def transform_subclass(t, callback, outer_size=None, outer_stride=None):
    ...
    ``__smith_dispatch__`` and holds some inner tensors,
    and a callback of type ``Callable[[str, smith.Tensor], smith.Tensor]``,
...
def _correct_storage_aliasing(func, schema_info, args, outs) -> None:
    """
    Given: an OpOverload, a SchemaInfo (cached information from smithgen about schema),
    ...
    def alias_non_inplace_storage(arg, ret) -> None:
        ...
        if isinstance(ret, list):
            for r in ret:
                smith._functionalize_unsafe_set(r, arg)
        else:
            if not isinstance(ret, smith.Tensor):
                raise AssertionError(f"expected smith.Tensor, got {type(ret)}")
            smith._functionalize_unsafe_set(ret, arg)
...
def _get_write_alias(x) -> str | None:
    ...
    if not alias_set or not x.is_write:
        return None
    # smithscript allows for complicated alias sets, but our dispatcher ops only really involve simple aliasing
    if len(alias_set) != 1:
...
# we sometimes use smithgen schema parsing (for aten ops, since smithscript's schema parsing is sometimes buggy),
# and sometimes use smithscript schema parsing (for custom ops, for which smithgen parsing is untested).
@dataclass
class AliasInfo:
    alias_set: set[str]
    is_write: bool
...
# This is cached for efficiency, since it can involve running smithgen
@functools.cache
def get_alias_info(func) -> SchemaInfo:
    # For ATen ops: use smithgen (since smithscript parser doesn't handle alias annotations
    # properly for some ops that output tensorlists)
    if func.namespace == "aten":
        smithgen_schema_str = str(func._schema)
        if not smithgen_schema_str.startswith("aten::"):
            raise AssertionError(
                "Expected smithgen schema string to start with 'aten::'"
            )
        # remove the aten:: namespace, which is added by the smithscript parser,
        # and smithgen doesn't know how to handle
        smithgen_schema_str = smithgen_schema_str[6:]
        import re

        # the smithscript parser ends up converting int[2]=1 into int[2]=[1, 1],
        # which smithgen chokes on.
        smithgen_schema_str = re.sub(r"=\[[0, ]+\]", "=0", smithgen_schema_str)
        smithgen_schema_str = re.sub(r"=\[[1, ]+\]", "=1", smithgen_schema_str)
        # for aten::rot90 / aten:fft_*
        smithgen_schema_str = re.sub(
            r"=\[(-?[0-9]+), (-?[0-9]+)\]", r"=[\1,\2]", smithgen_schema_str
        )
        smithgen_schema = smithgen.model.FunctionSchema.parse(smithgen_schema_str)
        arg_schemas = [
            AliasInfo(
                alias_set=(
            ...
            for a in smithgen_schema.arguments.flat_all
        ]
        out_schemas = [
            AliasInfo(
                alias_set=(
            ...
            for a in smithgen_schema.returns
        ]
    else:
        # For non-aten ops, smithgen is untested so we rely on smithscript schema parsing
        arg_schemas = [
            AliasInfo(
                alias_set=(
    ...
    schema_info = SchemaInfo(
        ...
        is_inplace_view_op=smith.Tag.inplace_view in func.tags,
...
def autograd_would_have_decomposed(
    func: smith._ops.OpOverload, flat_args: Sequence[smith.Tensor | object]
) -> bool:
    """
    ...
    for a in flat_args:
        if isinstance(a, smith.Tensor):
            backend_key = smith._C._parse_dispatch_key(
                smith._C._dispatch_key_for_device(a.device.type)
            )
            ...
            has_backend_registration = smith._C._dispatch_has_kernel_for_dispatch_key(
...
def return_and_correct_aliasing(func, args, kwargs, out):
    """
    This function should be used by wrapper tensor ``__smith_dispatch__`` subclasses
    that would like to work with smith.compile. It ensures that the subclass
    ...
    # Caching here because smithgen parsing is definitely not fast, and this function is called
    ...
    def get_arg_from_alias(output_alias, schema_info, args, kwargs):
        new_args, new_kwargs = smith.fx.operator_schemas.normalize_function(  # type: ignore[misc]
        ...
        if not isinstance(mutated_args[0], FunctionalTensor):
            with smith.utils._mode_utils.no_dispatch():
                ...
                try:
                    ...
                    smith._C._set_meta_in_tls_dispatch_include(meta_in_tls)
