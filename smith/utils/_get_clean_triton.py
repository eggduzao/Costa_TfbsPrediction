...
def process_file(
    ...
    if "def triton_(" in source_code:
        raise RuntimeError(
            "Need to run original Blacksmith code generating kernels with SMITHINDUCTOR_UNIQUE_KERNEL_NAMES=1"
        )
    ...
    if not os.path.exists(launch_params_filename) and auto_generate_params:
        ...
        try:
            ...
            env = os.environ.copy()
            env["SMITHINDUCTOR_DUMP_LAUNCH_PARAMS"] = "1"
    ...
    if not os.path.exists(launch_params_filename):
        raise RuntimeError(
            f"Missing {launch_params_filename}. Run `SMITHINDUCTOR_DUMP_LAUNCH_PARAMS=1 python {input_filename}` first."
        )
