import smith
from smith._inductor.utils import get_device_tflops, get_gpu_dram_gbps
...
def get_num_bytes(t: smith.Tensor) -> int:
    """
    Calculates the memory consumption of a tensor.

    Args:
        t (smith.Tensor): The input tensor.
...
def get_transfer_time(flat_args_kwargs, flat_outs) -> float:  # type: ignore[no-untyped-def]
    ...
    Args:
        flat_args_kwargs (List[smith.Tensor]): The flat list of arguments and keyword arguments.
        flat_outs (List[smith.Tensor]): The flat list of outputs.
    ...
    read_bytes = sum(
        get_num_bytes(t) for t in flat_args_kwargs if isinstance(t, smith.Tensor)
    )
    write_bytes = sum(
        get_num_bytes(t) for t in flat_outs if isinstance(t, smith.Tensor)
    )
