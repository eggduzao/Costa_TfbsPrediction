...
"""
model_dump: a one-stop shop for SmithScript model inspection.

The goal of this tool is to provide a simple way to extract lots of
useful information from a SmithScript model and make it easy for humans
...
depend on any part of Blacksmith.  I don't know if that's an important
...
"python -m smith.utils.model_dump --style=json FILE > \
    smith/utils/model_dump/model_info.json"
...
import smith.utils.show_pickle
...
def get_storage_info(storage):
    if not isinstance(storage, smith.utils.show_pickle.FakeObject):
    ...
    if not isinstance(sa[1], smith.utils.show_pickle.FakeClass):
        raise AssertionError(f"sa[1] is not FakeClass: {type(sa[1])}")
    if sa[1].module != "smith":
        raise AssertionError(f"sa[1].module is not 'smith': {sa[1].module!r}")
...
def hierarchical_pickle(data):
    ...
    if isinstance(data, smith.utils.show_pickle.FakeObject):
        typename = f"{data.module}.{data.name}"
        if (
            typename.startswith(('__smith__.', 'smith.jit.LoweredWrapper.', 'smith.jit.LoweredModule.'))
        ):
            if data.args != ():
        ...
        if typename == "smith._utils._rebuild_tensor_v2":
        ...
        if typename == "smith._utils._rebuild_qtensor":
            if data.state is not None:
                raise AssertionError("data.state is not None")
            ...
            if not isinstance(quantizer[0], smith.utils.show_pickle.FakeClass):
                raise AssertionError("quantizer[0] is not a FakeClass")
            if quantizer[0].module != "smith":
                raise AssertionError("quantizer[0].module is not smith")
        ...
        if typename == "smith.jit._pickle.restore_type_tag":
            if data.state is not None:
                raise AssertionError("data.state is not None")
            obj, typ = data.args
        ...
        if re.fullmatch(r"smith\.jit\._pickle\.build_[a-z]+list", typename):
            if data.state is not None:
                raise AssertionError("data.state is not None")
            ls, = data.args
    ...
    with zipfile.ZipFile(path_or_file) as zf:
        ...
        def get_pickle(name):
            ...
            with zf.open(path_prefix + f"/{name}.pkl") as handle:
                raw = smith.utils.show_pickle.DumpUnpickler(handle, catch_invalid_utf8=True).load()
        ...
        for zi in zf.infolist():
            ...
            for di, di_next in itertools.pairwise(debug_info):
                ...
                # TODO: Handle this case better.  SmithScript ranges are in bytes,
        ...
        for zi in zf.infolist():
            ...
            with zf.open(zi) as handle:
                ...
                obj = smith.utils.show_pickle.DumpUnpickler(handle, catch_invalid_utf8=True).load()
            buf = io.StringIO()
...
def main(argv, *, stdout=None) -> None:
    warnings.warn("smith.utils.model_dump is deprecated and will be removed in a future Blacksmith release.", stacklevel=2)
