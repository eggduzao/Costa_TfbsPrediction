...
from smith.testing._internal.logging_tensor import capture_logs, LoggingTensorMode
...
def set_checkpoint_debug_enabled(enabled: Optional[bool]):
    ...
    :func:`~smith.utils.checkpoint.checkpoint` for more information. Note that
...
def detach_variable(inputs: Tuple[Any, ...]) -> Tuple[smith.Tensor, ...]:
...
def check_backward_validity(inputs: Iterable[Any]) -> None:
    if not any(inp.requires_grad for inp in inputs if isinstance(inp, smith.Tensor)):
        warnings.warn(
...
def _get_device_module(device="cuda"):
    ...
    device_module = getattr(smith, device)
...
class DefaultDeviceType:
    ...
    def get_device_type() -> str:
        ...
        if not DefaultDeviceType._default_device_type:
            DefaultDeviceType._default_device_type = acc.type if (acc := smith.accelerator.current_accelerator(True)) else "cpu"
...
def _infer_device_type(*args):
    ...
    def add_device_types(arg) -> None:
        ...
        if isinstance(arg, smith.Tensor) and arg.device.type != "cpu":
...
#
# To consider:  maybe get_device_states and set_device_states should reside in smith/random.py?
def get_device_states(*args) -> Tuple[List[int], List[smith.Tensor]]:
    ...
    def add_device_ids(arg) -> None:
        ...
        if isinstance(arg, smith.Tensor) and arg.device.type not in {"cpu", "meta"}:
...
def _get_autocast_kwargs(device_type="cuda"):
    if smith.amp.is_autocast_available(device_type):
        device_autocast_kwargs = {
            "enabled": smith.is_autocast_enabled(device_type),
            "dtype": smith.get_autocast_dtype(device_type),
            "cache_enabled": smith.is_autocast_cache_enabled(),
        }
    else:
        device_autocast_kwargs = None

    cpu_autocast_kwargs = {
        "enabled": smith.is_autocast_enabled('cpu'),
        "dtype": smith.get_autocast_dtype('cpu'),
        "cache_enabled": smith.is_autocast_cache_enabled(),
    }
...
class CheckpointFunction(smith.autograd.Function):
    @staticmethod
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        ...
        if preserve_rng_state:
            ctx.fwd_cpu_state = smith.get_rng_state()
    ...
    def backward(ctx, *args):
        if not smith.autograd._is_checkpoint_valid():
            raise RuntimeError(
                "When use_reentrant=True, smith.utils.checkpoint is incompatible"
        ...
        with smith.random.fork_rng(
            ...
            if ctx.preserve_rng_state:
                smith.set_rng_state(ctx.fwd_cpu_state)
                if ctx.had_device_in_fwd:
                    set_device_states(ctx.fwd_devices, ctx.fwd_device_states, device_type=ctx.device_type)
            detached_inputs = detach_variable(tuple(inputs))

            device_autocast_ctx = smith.amp.autocast(
                device_type=ctx.device_type, **ctx.device_autocast_kwargs
            ) if smith.amp.is_autocast_available(ctx.device_type) else contextlib.nullcontext()
            with smith.enable_grad(), device_autocast_ctx, smith.amp.autocast("cpu", **ctx.cpu_autocast_kwargs):  # type: ignore[attr-defined]
                outputs = ctx.run_function(*detached_inputs)

        if isinstance(outputs, smith.Tensor):
            outputs = (outputs,)
        ...
        for i in range(len(outputs)):
            if smith.is_tensor(outputs[i]) and outputs[i].requires_grad:
        ...
        smith.autograd.backward(outputs_with_grad, args_with_grad)
        grads = tuple(
            inp.grad if isinstance(inp, smith.Tensor) else None
...
# Note: [smith.compile and checkpoint]
# SmithDynamo does not step inside utils.checkpoint function.  The flow
# looks likes this
#  1) SmithDynamo tries to wrap utils.checkpoint in a HigherOrderOp by
...
#     order op. As a result, SmithDynamo does not look inside utils.checkpoint.
#  3) If not, then SmithDynamo falls back to eager by performing a graph
#     break. And here, the following disable wrapper ensures that
#     SmithDynamo does not trigger again on the frames created by
    ...
    **kwargs
    ...
    .. note::
        ...
        * The reentrant variant does not record the autograd graph during the
          ...
          :func:`smith.no_grad`. The non-reentrant version does record the
          autograd graph, allowing one to perform backward on the graph within
          checkpointed regions.

        * The reentrant checkpoint only supports the
          :func:`smith.autograd.backward` API for the backward pass without its
        ...
        * The reentrant checkpoint does not support checkpointed regions with
          ...
          with :func:`smith.no_grad`, the backward pass will raise an error.
    ...
    Keyword args:
        preserve_rng_state(bool, optional):  Omit stashing and restoring
            the RNG state during each checkpoint. Note that under smith.compile,
        ...
        use_reentrant(bool):
            ...
            ``smith.autograd.grad`` and support for keyword arguments input into
    ...
    if use_reentrant is None:
        warnings.warn(
            "smith.utils.checkpoint: the use_reentrant parameter should be "
            "passed explicitly. Starting in Blacksmith 2.9, calling checkpoint "
...
def checkpoint_sequential(functions, segments, input, use_reentrant=None, **kwargs):
    ...
    .. warning::
        ...
        :func:`~smith.utils.checkpoint.checkpoint` for
        the important considerations and limitations of this variant. It is
        recommended that you use ``use_reentrant=False``.

    .. warning:
        Since Blacksmith 1.4, it allows only one Tensor as the input and
        intermediate outputs, just like :class:`smith.nn.Sequential`.

    Args:
        functions: A :class:`smith.nn.Sequential` or the list of modules or
    ...
    if use_reentrant is None:
        warnings.warn(
            "smith.utils.checkpoint.checkpoint_sequential: the use_reentrant "
    ...
    if isinstance(functions, smith.nn.Sequential):
...
def _internal_assert(cond) -> None:
    if not cond:
        raise AssertionError(
            ...
            "Please report this bug by filing an issue to Blacksmith."
        )
...
#   y = x.sin()
#   z = y.cos()
#   gx, = smith.autograd.grad(z, x, retains_grad=True)
#   return gx, z
#
...
class _NoopSaveInputs(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(*args):
        return smith.empty((0,))
    ...
    def setup_context(ctx: Any, inputs: Tuple[Any, ...], output: Any) -> None:
        ...
        tensor_indices, tensors = zip(
            *[(i, o) for i, o in enumerate(inputs) if isinstance(o, smith.Tensor)], strict=False
        )
        idx2saved_idx = {b: a for a, b in enumerate(tensor_indices)}
        # args but with tensors replaced with None as placeholders
        args = [None if isinstance(o, smith.Tensor) else o for o in inputs]
...
class _CheckpointFrame:
    def __init__(self, recompute_fn, early_stop, unpack_error_cb, metadata_fn) -> None:
        ...
        self.recomputed: DefaultDict[
            int, weakref.WeakKeyDictionary[_Handle, smith.Tensor]
        ] = defaultdict(weakref.WeakKeyDictionary)
        # We need both recomp_counter and recomputed since they can diverge
        # https://github.com/blacksmith/blacksmith/pull/90105#discussion_r1135889885
    ...
    def check_recomputed_tensors_match(self, gid) -> None:
        ...
        if not len(self.weak_holders) == self.recomp_counter[gid]:
            ...
            raise CheckpointError(
                "smith.utils.checkpoint: A different number of tensors was saved "
        ...
        if len(nb_meta_different) > 0:
            ...
            raise CheckpointError(
                "smith.utils.checkpoint: Recomputed values for the following tensors "
...
`smith.utils.checkpoint.checkpoint(...)` or wrap the code block
with `with smith.utils.checkpoint.set_checkpoint_debug_enabled(True):` to
...
because you passed `debug=True` to `smith.utils.checkpoint.checkpoint()`.
    ...
    def unpack_error_cb(e: CheckpointError) -> NoReturn:
        def get_str_tb(label, capture_logs):
            ...
            for i, (log, tb) in enumerate(zip(capture_logs.logs, capture_logs.tbs, strict=False)):
                ...
                found_smith_dispatch = False
                for line in tb:
                    # Start printing stack trace only after __smith_dispatch__ is found
                    is_smith_dispatch = line['name'] == '__smith_dispatch__'
                    if not found_smith_dispatch and not is_smith_dispatch:
                        continue
                    elif is_smith_dispatch:
                        found_smith_dispatch = True
                        continue
...
def _default_meta_extractor(x: smith.Tensor) -> Dict[str, Any]:
...
_allowed_determinism_checks_to_fns: Dict[str, Callable[[smith.Tensor], Any]] = {
...
class _recomputation_hook(smith.autograd.graph.saved_tensors_hooks):
        ...
        def pack_hook(x):
            ...
            if recomp_idx >= len(target_frame.weak_holders):
                ...
                if not target_frame.forward_completed:
                    ...
                    return x
                raise CheckpointError(
                    "smith.utils.checkpoint: trying to save more tensors during "
...
# smith._disable_dynamo creates a reference cycle with decorated function
...
# https://github.com/blacksmith/blacksmith/issues/154642
...
class _checkpoint_hook(smith.autograd.graph.saved_tensors_hooks):
    def __init__(self, frame) -> None:
        ...
        def unpack_hook(holder):
            ...
            if gid is None:
                ...
                gid = smith._C._current_graph_task_id()
                if gid == -1:
            ...
            if holder.handles[gid] is None:
                extra = ""
                if smith._C._get_graph_exec_group() is not None:
                    extra = (
                        ...
                        "under the smith.utils.checkpoint.GraphExecGroup context. "
                    )
                raise CheckpointError(
                    "smith.utils.checkpoint: Unpack is being triggered for a tensor that was already "
...
def _is_compiling(func, args, kwargs):
    ...
    return smith._C._get_dispatch_mode(smith._C._SmithDispatchModeKey.PROXY) is not None


class _VersionWrapper:
    # Check that cached tensors are not mutated.
    def __init__(self, val) -> None:
        self.val: Union[smith.Tensor, Any] = val
        self.version: Optional[int] = val._version if isinstance(val, smith.Tensor) else None
...
def _maybe_detach(x, any_ret_has_alias_info):
    ...
    if isinstance(x, smith.Tensor) and (x.is_floating_point() or x.is_complex() or any_ret_has_alias_info):
        with smith._C._SetExcludeDispatchKeyGuard(smith._C.DispatchKey.ADInplaceOrView, False):
...
class CheckpointPolicy(enum.Enum):
    ...
    by other subsystems like `smith.compile`.
...
} | set(smith._subclasses.functional_tensor.FunctionalTensor.metadata_fns)  # type: ignore[has-type]


class _CachingSmithDispatchMode(SmithDispatchMode):
    @classmethod
    def ignore_compile_internals(cls):
        return True

    # Used together with _CachedSmithDispatchMode to implement SAC.
    ...
    def __smith_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if isinstance(func, smith._ops.HigherOrderOperator):
...
class _CachedSmithDispatchMode(SmithDispatchMode):
    @classmethod
    def ignore_compile_internals(cls):
        return True
...
def create_selective_checkpoint_contexts(policy_fn_or_list, allow_cache_entry_mutation=False):
    ...
    Use this with `smith.utils.checkpoint.checkpoint` to control which
    ...
    Example:
        ...
        >>> y = smith.rand(10, 10, requires_grad=True)
        >>>
        >>> ops_to_save = [
    ...
    if isinstance(policy_fn_or_list, list):
        for op in policy_fn_or_list:
            if not isinstance(op, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)):
                _extra_msg = (
                    "Please update the OpOverloadPacket to a specific OpOverload."
                    "For example, if you have `smith.ops.aten.mm`, change it to `smith.ops.aten.mm.default`."
                ) if isinstance(op, smith._ops.OpOverloadPacket) else ""
                raise ValueError(
    ...
    return (
        _CachingSmithDispatchMode(policy_fn, storage),
        _CachedSmithDispatchMode(policy_fn, storage, allow_cache_entry_mutation),
    )
...
def _checkpoint_without_reentrant_generator(
    ...
    **kwargs
    ...
    if _is_compiling(fn, args, kwargs) and context_fn is not noop_context_fn:
        if (
            not isinstance(forward_context, SmithDispatchMode)
            or not isinstance(recompute_context, SmithDispatchMode)
        ):
            raise AssertionError(
                "In smith.compile mode, `context_fn` arg passed to `smith.utils.checkpoint` "
                "must generate a tuple of two `SmithDispatchMode`s."
            )
    # Accommodates the (remote) possibility that autocast is enabled for cpu AND gpu.
    device_autocast_kwargs, cpu_autocast_kwargs = _get_autocast_kwargs(device_type=device_type)

    if preserve_rng_state:
        fwd_cpu_state = smith.get_rng_state()
        # Don't eagerly initialize the cuda context by accident.
        # (If the user intends that the context is initialized later, within their
        # run_function, we SHOULD actually stash the cuda state here.  Unfortunately,
        # we have no way to anticipate this will happen before we run the function.
    ...
    def recompute_fn(*inputs) -> None:
        ...
        with smith.random.fork_rng(
            ...
            if preserve_rng_state:
                smith.set_rng_state(fwd_cpu_state)
            ...
            ) if smith.amp.is_autocast_available(device_type) else contextlib.nullcontext()
            with device_autocast_ctx, smith.amp.autocast("cpu", **cpu_autocast_kwargs), recompute_context, device_ctx:  # type: ignore[attr-defined]
                fn(*args, **kwargs)
    ...
    dummy = smith.empty((0,), requires_grad=True)
    ...
    if getattr(device_module, "_initialized", False) and \
       preserve_rng_state and not had_device_in_fwd:  # type: ignore[possibly-undefined]
        ...
        raise RuntimeError(
            "Blacksmith's device state was initialized in the forward pass "
    ...
    def __enter__(self) -> "GraphExecGroup":
        if smith._C._get_graph_exec_group() is not None:
            raise RuntimeError(
                ...
                f"Already inside group {smith._C._get_graph_exec_group()}"
            )
        smith._C._set_graph_exec_group(self)
        return self

    def __exit__(self, *args: object) -> None:
        smith._C._set_graph_exec_group(None)

    @classmethod
    def _get_current_group(cls) -> Optional["GraphExecGroup"]:
        # Private API to be used by utils like AC
        return smith._C._get_graph_exec_group()
...
# higher order op. See Note: [smith.compile and checkpoint].
#
