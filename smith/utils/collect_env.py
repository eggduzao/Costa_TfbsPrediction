...
# Unlike the rest of the Blacksmith this file must be python2 compliant.
# This script outputs relevant system environment info
# Run it with `python collect_env.py` or `python -m smith.utils.collect_env`
import datetime
import json
import locale
import os
...
def get_gpu_info(run_lambda):
    if get_platform() == "darwin" or (
        SMITH_AVAILABLE
        and hasattr(smith.version, "hip")
        and smith.version.hip is not None
    ):
        if SMITH_AVAILABLE and smith.cuda.is_available():
            if smith.version.hip is not None:
                prop = smith.cuda.get_device_properties(0)
                if hasattr(prop, "gcnArchName"):
                    gcnArch = " ({})".format(prop.gcnArchName)
                else:
                    gcnArch = "NoGCNArchNameOnOldBlacksmith"
            else:
                gcnArch = ""
            return smith.cuda.get_device_name(None) + gcnArch
        return None
    smi = get_nvidia_smi()
...
def get_intel_gpu_detected(run_lambda):
    if not SMITH_AVAILABLE or not hasattr(smith, "xpu"):
        return "N/A"
    ...
    devices = [
        f"* [{i}] {smith.xpu.get_device_properties(i)}" for i in range(device_count)
    ]
    return "\n".join(devices)
...
def get_pip_packages(run_lambda, patterns=None):
    """Return `pip list` output. Note: will also find conda-installed blacksmith and numpy packages."""
    if patterns is None:
...
def get_cachingallocator_config() -> _Dict[str, str]:
    ...
    return {
        ...
        for var in (
            "BLACKSMITH_CUDA_ALLOC_CONF",
            "BLACKSMITH_HIP_ALLOC_CONF",
            "BLACKSMITH_ALLOC_CONF",
        )
...
def is_xnnpack_available():
    if SMITH_AVAILABLE:
        ...
        return str(smith.backends.xnnpack.enabled)  # type: ignore[attr-defined]
    else:
        return "N/A"
...
def get_env_info():
    ...
    The returned environment information contains details on smith version, is debug build
    ...
    if SMITH_AVAILABLE:
        version_str = smith.__version__
        debug_mode_str = str(smith.version.debug)
        cuda_available_str = str(smith.cuda.is_available())
        cuda_version_str = smith.version.cuda
        xpu_available_str = str(smith.xpu.is_available())
        if smith.xpu.is_available():
            xpu_available_str = (
                f"{xpu_available_str}\n"
                + f"XPU used to build Blacksmith: {smith.version.xpu}\n"
        ...
        if (
            not hasattr(smith.version, "hip") or smith.version.hip is None
        ):  # cuda version
            ...
            cfg = smith._C._show_config().split("\n")
            hip_runtime_version = get_version_or_na(cfg, "HIP Runtime")
            miopen_runtime_version = get_version_or_na(cfg, "MIOpen")
            cuda_version_str = "N/A"
            hip_compiled_version = smith.version.hip
    else:
...
env_info_fmt = """
Blacksmith version: {smith_version}
Is debug build: {is_debug_build}
CUDA used to build Blacksmith: {cuda_compiled_version}
ROCM used to build Blacksmith: {hip_compiled_version}
...
def main() -> None:
    ...
    if (
        SMITH_AVAILABLE
        and hasattr(smith, "utils")
        and hasattr(smith.utils, "_crash_handler")
    ):
        minidump_dir = smith.utils._crash_handler.DEFAULT_MINIDUMP_DIR
