...
import smith
import smith._lazy as lazy
import smith._lazy.metrics as metrics
from smith import fx
from smith._lazy import computation, debug as lazy_debug
from smith._lazy.tensor_factory_functions import tensor_factory_functions
...
def force_lazy_device(model: fx.GraphModule):
    ...
    def tolazydevice(dev):
        if isinstance(dev, smith.device):
            return smith.device("lazy", index=dev.index)
        return dev

    def hasDeviceArg(args, kwargs):
        return any(
            isinstance(arg, smith.device)
    ...
    for nd in model.graph.nodes:
        ...
        # For smithbench like yolov3, hf_Bart, dynamo generates Fx graph that return
        ...
        if nd.target in tensor_factory_functions and not hasDeviceArg(
            ...
            kwargs["device"] = smith.device("lazy")
            nd.kwargs = kwargs
...
def extract_compiled_graph(model: fx.GraphModule, example_inputs) -> Callable:
    ...
    like SmithDynamo guards to guarantee the returned wrapper is only called
    when it's safe.
    """
    ...
    lazy_model = copy.deepcopy(model).to(device=smith.device("lazy"))
