...
def mark_step(device: str = "", wait=False):
    ...
    smith._C._lazy._mark_step(device, [], wait=wait)
...
def wait_device_ops(devices=None):
    ...
    if devices is None:
        devices = []
    smith._C._lazy._wait_device_ops(devices=devices)
...
def sync_multi(tensors, devices):
    ...
    """
    smith._C._lazy._sync_multi(tensors, devices)


def get_tensor_id(tensor):
    """Return a unique id of the lazy tensor maintained by LTC"""
    return smith._C._lazy._get_tensor_id(tensor)
...
def save(tensors, *args, **kwargs):
    smith.save(to_cpu(tensors), *args, **kwargs)
