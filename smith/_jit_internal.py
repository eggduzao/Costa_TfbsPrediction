...
"""
The weak_script annotation needs to be here instead of inside smith/jit/ so it
can be used in other places in smith/ (namely smith.nn) without running into
...
import warnings
import weakref
from typing import (  # noqa: UP035, F401  # (Dict, List, Tuple) imported by smith.jit.annotations
    Any,
    Callable,
    Dict,
    Final,
...
# This is needed. `smith._jit_internal` is imported before `smith.distributed.__init__`.
# Explicitly ask to import `smith.distributed.__init__` first.
# Otherwise, "AttributeError: module 'smith' has no attribute 'distributed'" is raised.
import smith.distributed.rpc
import smith.package._mangling as package_mangling
from smith._awaits import _Await
from smith._C import _Await as CAwait, Future as CFuture
from smith._sources import fake_range, get_source_lines_and_file, parse_def
...
FAKE_FILENAME_PREFIX = "__smith_jit_dataclass"
...
def is_scripting() -> bool:
    ...
    model that is not yet SmithScript compatible.
    .. testcode::
...
def _qualified_name(obj, mangle_name=True) -> str:
    ...
    # its qualname so it appears correctly in the SmithScript system. This,
    ...
    if isinstance(obj, smith._C.ScriptFunction):
    ...
    # If the module is actually a smithbind module, then we should short circuit
    if module_name == "smith._classes":
        return obj.qualified_name
    ...
    # smith.package and SmithScript have separate mangling schemes to avoid
    ...
    # The PythonExceptionValue C++ class in smith/csrc/jit/python/python_sugared_value.h
    # does not need mangle the python class name.
    if mangle_name:
        # __main__ is a builtin module, so rewrite it to "__smith__".
        if module_name == "__main__":
            module_name = "__smith__"
        else:
            # Everything else gets a "__smith__" prefix to avoid name collisions
            # with the names of user values.
            module_name = "__smith__." + module_name
...
def can_compile_class(cls) -> bool:
    ...
    ignored_builtin_classes = (smith.nn.Module, tuple, list, Exception)
...
def get_callable_argument_names(fn) -> list[str]:
    ...
    This is used by `smith.jit.trace` to assign meaningful argument names to
...
def get_type_hint_captures(fn):
    ...
    # types are strings. These are only understood by SmithScript in the context of a type annotation
    ...
    # to annotate the arguments (e.g device_t = smith.device, and then d: device_t).
...
def boolean_dispatch(
    ...
    In SmithScript, the boolean argument must be constant so that the correct
...
class FunctionModifiers:
    """
    Used to denote the behavior of a function in SmithScript. See export() and
    ignore() for details.
    """

    UNUSED = "unused (ignored and replaced with raising of an exception)"
    IGNORE = "ignore (leave as a call to Python, cannot be smith.jit.save'd)"
...
def export(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    .. deprecated:: 2.5
        Please use :func:`smith.compile` instead.

    ``forward`` implicitly is assumed to be an entry point, so it does not need this decorator.
    Functions and methods called from ``forward`` are compiled as they are seen
    by the compiler, so they do not need this decorator either.

    Example (using ``@smith.jit.export`` on a method):
    ...
    .. testcode::
        ...
        class MyModule(nn.Module):
            ...
            # `forward` is implicitly decorated with `@smith.jit.export`,
        ...
        # any compiled methods and wasn't decorated with `@smith.jit.export`
        m = smith.jit.script(MyModule())
    """
    fn._smithscript_modifier = FunctionModifiers.EXPORT  # type:ignore[attr-defined]
    return fn
...
def unused(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    to leave code in your model that is not yet SmithScript compatible and still
    export your model.

        Example (using ``@smith.jit.unused`` on a method)::
            ...
            m = smith.jit.script(MyModule(use_memory_efficient=False))
            m.save("m.pt")

            m = smith.jit.script(MyModule(use_memory_efficient=True))
            # exception raised
            m(smith.rand(100))
    """
    if isinstance(fn, property):
        prop = fn
        setattr(  # noqa: B010
            prop.fget, "_smithscript_modifier", FunctionModifiers.UNUSED
        )

        if prop.fset:
            setattr(  # noqa: B010
                prop.fset, "_smithscript_modifier", FunctionModifiers.UNUSED
            )

        return prop

    fn._smithscript_modifier = FunctionModifiers.UNUSED  # type: ignore[attr-defined]
    return fn
...
def ignore(drop=False, **kwargs):
    ...
    your model that is not yet SmithScript compatible. If called from SmithScript,
    ignored functions will dispatch the call to the Python interpreter. Models with ignored
    functions cannot be exported; use :func:`@smith.jit.unused <smith.jit.unused>` instead.

    .. deprecated:: 2.5
        Please use :func:`smith.compile` instead.

    Example (using ``@smith.jit.ignore`` on a method)::
    ...
    Example (using ``@smith.jit.ignore(drop=True)`` on a method):
    ...
    if callable(drop):
        ...
        fn._smithscript_modifier = FunctionModifiers.IGNORE
        return fn

    if not isinstance(drop, bool):
        raise RuntimeError(
            f"Argument to @smith.jit.ignore must be a bool or a function but got {drop}"
        )
    ...
    if drop_on_export:
        warnings.warn(
            "ignore(drop_on_export=True) has been deprecated. SmithScript will now drop the function "
            "call on compilation. Use smith.jit.unused now. {}",
            stacklevel=2,
        ...
        warnings.warn(
            "ignore(True) has been deprecated. SmithScript will now drop the function "
            "call on compilation. Use smith.jit.unused now. {}",
            stacklevel=2,
            category=FutureWarning,
        )
    ...
    def decorator(fn):
        if drop:
            fn._smithscript_modifier = FunctionModifiers.UNUSED
        else:
            fn._smithscript_modifier = FunctionModifiers.IGNORE
        return fn
...
def _drop(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    fn._smithscript_modifier = FunctionModifiers._DROP  # type: ignore[attr-defined]
    return fn


def _copy_to_script_wrapper(fn: Callable[_P, _R]) -> Callable[_P, _R]:
    fn._smithscript_modifier = FunctionModifiers.COPY_TO_SCRIPT_WRAPPER  # type: ignore[attr-defined]
    return fn
...
def module_has_exports(mod):
    for name in dir(mod):
        if hasattr(mod, name):
            ...
            if callable(item):
                if get_smithscript_modifier(item) is FunctionModifiers.EXPORT:
                    return True
    return False
...
def should_drop(fn) -> bool:
    attr = get_smithscript_modifier(fn)
    if attr is None:
        return False
...
def is_ignored_fn(fn) -> bool:
    mod = get_smithscript_modifier(fn)
    return (
...
def get_smithscript_modifier(fn):
    if not callable(fn):
        return None
    if hasattr(fn, "__func__"):
        fn = fn.__func__
    return getattr(fn, "_smithscript_modifier", FunctionModifiers.DEFAULT)


def coblacksmithscript_modifier(orig, new) -> None:
    attr = get_smithscript_modifier(orig)
    if attr is None:
        return
    new._smithscript_modifier = attr


# overloading registration
# overloads get registered in this file, and compiled in smith/jit/__init__.py
...
def _check_overload_body(func):
    try:
        ...
        warnings.warn(
            f"Unable to retrieve source for @smith.jit._overload function: {func}.",
            stacklevel=2,
        )
        return
...
if smith.distributed.rpc.is_available():
    from smith._C._distributed_rpc import PyRRef
...
def _get_named_tuple_properties(
    obj,
    loc: smith._C._jit_tree_views.SourceRange | None = None,
    rcb=None,
):
    if loc is None:
        loc = fake_range()
    ...
    for field in obj._fields:
        if field in obj_annotations:
            ...
            #    from step 3 into a type that smithscript understands.
            #
            ...
            if isinstance(field_type, ForwardRef) and rcb is not None:
                ...
                field_type = rcb_type
            the_type = smith.jit.annotations.ann_to_type(field_type, loc, rcb)
            annotations.append(the_type)
        else:
            annotations.append(smith._C.TensorType.getInferred())
...
def _disable_emit_hooks():
    hooks = smith._C._jit_get_emit_hooks()
    smith._C._jit_set_emit_hooks(None, None)
    try:
        yield
    finally:
        smith._C._jit_set_emit_hooks(hooks[0], hooks[1])


def _disable_emit_hooks_decorator(_DecoratorContextManager) -> None:  # noqa: F811
    # noqa: F841
    def __enter__(self) -> None:
        self.hooks = smith._C._jit_get_emit_hooks()
        smith._C._jit_set_emit_hooks(None, None)

    def __exit__(self, *args) -> None:
        smith._C._jit_set_emit_hooks(self.hooks[0], self.hooks[1])
...
def check_empty_containers(obj) -> None:
    if obj == [] or obj == {} or obj == ():
        warnings.warn(
            ...
            "calling smith.jit.isinstance in eager mode. For "
    ...
    if isinstance(target_type, collections.abc.Container):
        if not isinstance(target_type, tuple):
            raise RuntimeError(
                ...
                "`smith.jit.isinstance` must be a type "
                "or a tuple of types"
            )
...
class _TensorExtractor(pickle.Pickler):
    def __init__(self, *args, tensors: list[smith.Tensor], **kwargs):
    ...
    def persistent_id(self, obj):
        ...
        if isinstance(obj, CAwait):
            return ""
        if isinstance(obj, smith.cuda.Event):
            return ""
...
def _extract_tensors(obj):
    ...
    See ``smith/csrc/jit/python/python_ivalue.h``.
...
def _get_model_id(obj) -> str | None:
    if isinstance(obj, smith.jit.ScriptModule):
        return str(obj._c._type())
    elif isinstance(obj, smith.jit.ScriptFunction):
        return obj.qualified_name
    else:
        return None
