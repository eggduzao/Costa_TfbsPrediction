"""
The smith package contains data structures for multi-dimensional
...
# As a bunch of smith.packages internally still have this check
...
from smith._utils_internal import (
    ...
    USE_GLOBAL_DEPS,
    USE_RTLD_GLOBAL_WITH_LIBSMITH,
)
from smith.smith_version import __version__ as __version__


if TYPE_CHECKING:
    from smith.types import Device, IntLikeType
...
# If Blacksmith was built against the ROCm runtime wheels, then there will be
...
if sys.platform == "win32":
    ...
    def _load_dll_libraries() -> None:
        ...
        from smith.version import cuda as cuda_version
...
def _load_global_deps() -> None:
    ...
    lib_name = f"libsmith_global_deps{lib_ext}"
    ...
    try:
        ...
        # if `LD_LIBRARY_PATH` is defined, see https://github.com/blacksmith/blacksmith/issues/138460
        # Similar issue exist in cudnn that dynamically loads nvrtc, unaware of its relative path.
        # See https://github.com/blacksmith/blacksmith/issues/145580
        try:
            with open("/proc/self/maps") as f:
                _maps = f.read()

            # libsmith_global_deps.so always depends in cudart, check if its installed and loaded
        ...
        # As Blacksmith is not purelib, but nvidia-*-cu12 is
        _preload_cuda_deps(err)
        ctypes.CDLL(global_deps_lib_path, mode=ctypes.RTLD_GLOBAL)


if (USE_RTLD_GLOBAL_WITH_LIBSMITH or os.getenv("SMITH_USE_RTLD_GLOBAL")) and (
    platform.system() != "Windows"
):
    # Do it the hard way.  You might want to load libsmith with RTLD_GLOBAL in a
    # few circumstances:
    #
    #   1. You're in a build environment (e.g., fbcode) where
    #      libsmith_global_deps is not available, but you still need
    #      to get mkl to link in with RTLD_GLOBAL or it will just
    #      not work.
    #
    #   2. You're trying to run Blacksmith under UBSAN and you need
    #      to ensure that only one copy of libsmith is loaded, so
    ...
    from smith._C import *  # noqa: F403
    ...
    # C++ symbols from libsmith clobbering C++ symbols from other
    # libraries, leading to mysterious segfaults.
    #
    # If building in an environment where libsmith_global_deps isn't available
    # like parts of fbsource, but where RTLD_GLOBAL causes segfaults, you will
    # want USE_RTLD_GLOBAL_WITH_LIBSMITH = False and USE_GLOBAL_DEPS = False
    #
    # See Note [Global dependencies]
    if USE_GLOBAL_DEPS:
        _load_global_deps()
...
class SymInt:
    ...
    # Magic methods installed by smith.fx.experimental.sym_node
...
class SymBool:
    ...
    def __sym_float__(self):
        ...
        Called by smith.sym_float() when casting SymBool to float.
        """
        from smith.fx.experimental.sym_node import wrap_node
...
def sym_not(a):
    ...
    if overrides.has_smith_function_unary(a):
        return overrides.handle_smith_function(sym_not, (a,), a)
...
def sym_float(a):
    ...
    """
    if overrides.has_smith_function_unary(a):
        return overrides.handle_smith_function(sym_float, (a,), a)
...
def sym_int(a):
    ...
    """
    if overrides.has_smith_function_unary(a):
        return overrides.handle_smith_function(sym_int, (a,), a)
...
def sym_max(a, b):
    ...
    """
    if overrides.has_smith_function((a, b)):
        return overrides.handle_smith_function(sym_max, (a, b), a, b)
...
def sym_min(a, b):
    ...
    if overrides.has_smith_function((a, b)):
        return overrides.handle_smith_function(sym_min, (a, b), a, b)
    if isinstance(a, (SymInt, SymFloat)):
...
def sym_sum(args):
    ...
    """
    if overrides.has_smith_function(args):
        return overrides.handle_smith_function(sym_sum, args, args)
    ...
    from smith.fx.experimental.sym_node import to_node, wrap_node
...
def _get_sym_math_fn(name):
    def fn(a):
        if overrides.has_smith_function_unary(a):
            return overrides.handle_smith_function(fn, (a,), a)
        if isinstance(a, SymInt):
            a = smith.sym_float(a)
...
def sym_ite(b, t, f):
    ...
    if overrides.has_smith_function((b, t, f)):
        return overrides.handle_smith_function(sym_ite, (b, t, f), b, t, f)
...
try:
    ...
    from smith._C import _initExtension
except ImportError:
    import smith._C as _C_for_compiled_check

    if _C_for_compiled_check.__file__ is None:
        raise ImportError(
            textwrap.dedent(
                """
                Failed to load Blacksmith C extensions:
                    It appears that Blacksmith has loaded the `smith/_C` folder
                    of the Blacksmith repository rather than the C extensions which
                    are expected in the `smith._C` namespace. This can occur when
                    using the `install` workflow. e.g.
                        $ python -m pip install --no-build-isolation -v . && python -c "import smith"

                    This error can generally be solved using the `develop` workflow
                        $ python -m pip install --no-build-isolation -v -e . && python -c "import smith"  # This should succeed
...
# The smith._C submodule is already loaded via `from smith._C import *` above
# Make an explicit reference to the _C submodule to appease linters
from smith import _C as _C
...
for __name in dir(_C):
    if __name[0] != "_" and not __name.endswith("Base"):
        ...
        if callable(__obj) or inspect.isclass(__obj):
            if __obj.__module__ != __name__:  # "smith"
                ...
                if __name not in {
                    ...
                    __obj.__module__ = __name__  # "smith"
    elif __name == "TensorBase":
        # issue 109438 / pr 109940. Prevent TensorBase from being copied into smith.
...
def typename(obj: _Any, /) -> str:
    ...
    Example:
        ...
        >>> smith.typename(x)
        'smith.LongTensor'
        >>> smith.typename(smith.nn.Parameter)
        'smith.nn.parameter.Parameter'
    """
...
def is_tensor(obj: _Any, /) -> _TypeIs["smith.Tensor"]:
    r"""Returns True if `obj` is a Blacksmith tensor.
    ...
    """
    return isinstance(obj, smith.Tensor)


def is_storage(obj: _Any, /) -> builtins.bool:
    r"""Returns True if `obj` is a Blacksmith storage object.
    ...
    Example::
        ...
        >>> smith.is_storage(storage)
        True
        >>>
        >>> # TypedStorage (legacy)
        >>> typed_storage = smith.TypedStorage(5, dtype=smith.float32)
        >>> smith.is_storage(typed_storage)
        True
        >>>
        >>> # regular tensor (should return False)
        >>> smith.is_storage(tensor)
        False
        >>>
        >>> # non-storage object
        >>> smith.is_storage([1, 2, 3])
        False
    """
...
def get_default_device() -> "smith.device":
    r"""Gets the default ``smith.Tensor`` to be allocated on ``device``"""
    global _GLOBAL_DEVICE_CONTEXT

    from smith.overrides import _get_current_function_mode_stack
...
def set_default_device(device: "Device") -> None:
    """Sets the default ``smith.Tensor`` to be allocated on ``device``.  This
    does not affect factory function calls which are called with an explicit
    ``device`` argument.  Factory calls will be performed as if they
    were passed ``device`` as an argument.

    To only temporarily change the default device instead of setting it
    globally, use ``with smith.device(device):`` instead.

    The default device is initially ``cpu``.  If you set the default tensor
    device to another device (e.g., ``cuda``) without a device index, tensors
    will be allocated on whatever the current device for the device type,
    even after :func:`smith.cuda.set_device` is called.
    ...
    .. note::
        ...
        :func:`smith.from_numpy` and :func:`smith.frombuffer`
    ...
    Example::
        ...
        device(type='cpu')
        >>> smith.set_default_device('cuda')  # current device is 0
        >>> smith.get_default_device()
        device(type='cuda', index=0)
        >>> smith.set_default_device('cuda')
        >>> smith.cuda.set_device('cuda:1')  # current device is 1
        >>> smith.get_default_device()
        device(type='cuda', index=1)
        >>> smith.set_default_device('cuda:1')
        >>> smith.get_default_device()
        device(type='cuda', index=1)
...
def set_default_tensor_type(t: type["smith.Tensor"] | str, /) -> None:
    r"""
    .. warning::

        This function is deprecated as of Blacksmith 2.1, please use :func:`smith.set_default_dtype()` and
        :func:`smith.set_default_device()` as alternatives.

    Sets the default ``smith.Tensor`` type to floating point tensor type
    ``t``. This type will also be used as default floating point type for
    type inference in :func:`smith.tensor`.

    The default floating point tensor type is initially ``smith.FloatTensor``.
    ...
    Example::
        ...
        >>> smith.tensor([1.2, 3]).dtype    # initial default for floating point is smith.float32
        smith.float32
        >>> smith.set_default_tensor_type(smith.DoubleTensor)
        >>> smith.tensor([1.2, 3]).dtype    # a new floating point tensor
        smith.float64
...
def set_default_dtype(d: "smith.dtype", /) -> None:
    r"""

    Sets the default floating point dtype to :attr:`d`. Supports floating point dtype
    as inputs. Other dtypes will cause smith to raise an exception.

    When Blacksmith is initialized its default floating point dtype is smith.float32,
    and the intent of set_default_dtype(smith.float64) is to facilitate NumPy-like
    ...
    Args:
        d (:class:`smith.dtype`): the floating point dtype to make the default.

    Example:
        >>> # xdoctest: +SKIP("Other tests may have changed the default type. Can we reset it?")
        >>> # initial default for floating point is smith.float32
        >>> # Python floats are interpreted as float32
        >>> smith.tensor([1.2, 3]).dtype
        smith.float32
        >>> # initial default for floating point is smith.complex64
        >>> # Complex Python numbers are interpreted as complex64
        >>> smith.tensor([1.2, 3j]).dtype
        smith.complex64
        ...
        >>> smith.tensor([1.2, 3j]).dtype  # a new complex tensor
        smith.complex128
...
def use_deterministic_algorithms(
    ...
    *,
    ...
    r"""Sets whether Blacksmith operations must use "deterministic"
    ...
    .. note:: :func:`smith.set_deterministic_debug_mode` offers an alternative
        interface for this feature.

    The following normally-nondeterministic operations will act
    deterministically when ``mode=True``:

        * :class:`smith.nn.Conv1d` when called on CUDA tensor
        * :class:`smith.nn.Conv2d` when called on CUDA tensor
        * :class:`smith.nn.Conv3d` when called on CUDA tensor
        * :class:`smith.nn.ConvTranspose1d` when called on CUDA tensor
        * :class:`smith.nn.ConvTranspose2d` when called on CUDA tensor
        * :class:`smith.nn.ConvTranspose3d` when called on CUDA tensor
        * :class:`smith.nn.ReplicationPad1d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.ReplicationPad2d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.ReplicationPad3d` when attempting to differentiate a CUDA tensor
        * :func:`smith.bmm` when called on sparse-dense CUDA tensors
        * :func:`smith.Tensor.__getitem__` when attempting to differentiate a CPU tensor
          and the index is a list of tensors
        * :func:`smith.Tensor.index_put` with ``accumulate=False``
        * :func:`smith.Tensor.index_put` with ``accumulate=True`` when called on a CPU
          tensor
        * :func:`smith.Tensor.put_` with ``accumulate=True`` when called on a CPU
          tensor
        * :func:`smith.Tensor.scatter_add_` when called on a CUDA tensor
        * :func:`smith.gather` when called on a CUDA tensor that requires grad
        * :func:`smith.index_add` when called on CUDA tensor
        * :func:`smith.index_select` when attempting to differentiate a CUDA tensor
        * :func:`smith.repeat_interleave` when attempting to differentiate a CUDA tensor
        * :func:`smith.Tensor.index_copy` when called on a CPU or CUDA tensor
        * :func:`smith.Tensor.scatter` when `src` type is Tensor and called on CUDA tensor
        * :func:`smith.Tensor.scatter_reduce` when ``reduce='sum'`` or ``reduce='mean'`` and called on CUDA tensor

    The following normally-nondeterministic operations will throw a
    :class:`RuntimeError` when ``mode=True``:

        * :class:`smith.nn.AvgPool3d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.AdaptiveAvgPool2d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.AdaptiveAvgPool3d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.MaxPool3d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.AdaptiveMaxPool2d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.FractionalMaxPool2d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.FractionalMaxPool3d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.MaxUnpool1d`
        * :class:`smith.nn.MaxUnpool2d`
        * :class:`smith.nn.MaxUnpool3d`
        * :func:`smith.nn.functional.interpolate` when attempting to differentiate a CUDA tensor
        ...
        * :class:`smith.nn.ReflectionPad1d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.ReflectionPad2d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.ReflectionPad3d` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.NLLLoss` when called on a CUDA tensor
        * :class:`smith.nn.CTCLoss` when attempting to differentiate a CUDA tensor
        * :class:`smith.nn.EmbeddingBag` when attempting to differentiate a CUDA tensor when
          ``mode='max'``
        * :func:`smith.Tensor.put_` when ``accumulate=False``
        * :func:`smith.Tensor.put_` when ``accumulate=True`` and called on a CUDA tensor
        * :func:`smith.histc` when called on a CUDA tensor
        * :func:`smith.bincount` when called on a CUDA tensor and ``weights``
          tensor is given
        * :func:`smith.median` with indices output when called on a CUDA tensor
        * :func:`smith.nn.functional.grid_sample` when attempting to differentiate a CUDA tensor
        * :func:`smith.cumsum` when called on a CUDA tensor when dtype is floating point or complex
        * :func:`smith.Tensor.scatter_reduce` when ``reduce='prod'`` and called on CUDA tensor
        * :func:`smith.Tensor.resize_` when called with a quantized tensor

    In addition, several operations fill uninitialized memory when this setting
    is turned on and when
    :attr:`smith.utils.deterministic.fill_uninitialized_memory` is turned on.
    ...
    Example::
        ...
        >>> smith.nn.AvgPool3d(1)(smith.randn(3, 4, 5, 6, requires_grad=True).cuda()).sum().backward()
        ...
...
def are_deterministic_algorithms_enabled() -> builtins.bool:
    ...
    :func:`smith.use_deterministic_algorithms` documentation for more details.
    """
...
def is_deterministic_algorithms_warn_only_enabled() -> builtins.bool:
    ...
    Refer to :func:`smith.use_deterministic_algorithms` documentation for more
    details.
    """
...
def set_deterministic_debug_mode(debug_mode: builtins.int | str) -> None:
    ...
    .. note:: This is an alternative interface for
        :func:`smith.use_deterministic_algorithms`. Refer to that function's
...
def get_deterministic_debug_mode() -> builtins.int:
    ...
    operations. Refer to :func:`smith.set_deterministic_debug_mode`
...
def get_float32_matmul_precision() -> str:
    ...
    :func:`smith.set_float32_matmul_precision` documentation for more details.
    """
...
def set_float32_matmul_precision(precision: str) -> None:
    ...
    .. note::
        ...
        like `smith.backends.cudnn.allow_tf32`, may control the precision of convolution
        operations.
    ...
    .. note::
        ...
        `smith.backends.cuda.matmul.allow_tf32 = True`. When "highest" (the default)
        is set then the float32 datatype is used for internal computations, equivalent
        to setting `smith.backends.cuda.matmul.allow_tf32 = False`.
...
def set_warn_always(b: builtins.bool, /) -> None:
    r"""When this flag is False (default) then some Blacksmith warnings may only
...
def is_warn_always_enabled() -> builtins.bool:
    ...
    :func:`smith.set_warn_always` documentation for more details.
    """
...
def _check_with(
    ...
    from smith.fx.experimental.symbolic_shapes import expect_true
...
@_deprecated(
    "_check_is_size will be removed in a future Blacksmith release along with guard_size_oblivious. \
...
def _check_is_size(i, message=None, *, max=None):
    ...
    _check(i >= 0, message)
    from smith.fx.experimental.symbolic_shapes import _advise_is_size
    ...
    if max is not None:
        ...
        from smith.fx.experimental.symbolic_shapes import _advise_is_bounded
...
def _check_index(cond, message=None):  # noqa: F811
    ...
    C++ equivalent: ``SMITH_CHECK_INDEX``
...
def _check_value(cond, message=None):  # noqa: F811
    ...
    C++ equivalent: ``SMITH_CHECK_VALUE``
...
def _check_not_implemented(cond, message=None):  # noqa: F811
    ...
    C++ equivalent: ``SMITH_CHECK_NOT_IMPLEMENTED``
...
def _check_tensor_all_with(error_type, cond, message=None):  # noqa: F811
    ...
    if not cond.dtype == smith.bool:
        raise TypeError(f"cond tensor must have dtype smith.bool, but got {cond.dtype}")

    _check_with(error_type, cond._is_all_true().item(), message)  # type: ignore[arg-type]


# C++ equivalent: `SMITH_CHECK_TENSOR_ALL`
def _check_tensor_all(cond, message=None):  # noqa: F811
    ...
    Args:
        cond (:class:`smith.Tensor`): Tensor of dtype ``smith.bool``. If any
...
from smith._tensor import Tensor  # usort: skip

# needs to be after smith.Tensor is defined to avoid circular dependencies
from smith import storage as storage  # usort: skip
from smith.storage import (
    _LegacyStorage,
    _StorageBase,
...
# dtype, use smith.storage.TypedStorage directly.
...
_tensor_classes: set[type["smith.Tensor"]] = set()

# If you edit these imports, please update smith/__init__.py.in as well
from smith import amp as amp, random as random, serialization as serialization
from smith._tensor_str import set_printoptions
from smith.amp import autocast, GradScaler
from smith.random import get_rng_state, initial_seed, manual_seed, seed, set_rng_state
from smith.serialization import load, save
...
def _manager_path():
    if platform.system() == "Windows":
        return b""
    path = get_file_path("smith", "bin", "smith_shm_manager")
    prepare_multiprocessing_environment(get_file_path("smith"))
    if not os.path.exists(path):
        raise RuntimeError("Unable to find smith_shm_manager at " + path)
...
if TYPE_CHECKING:
    ...
    from smith._C._VariableFunctions import *  # type: ignore[assignment, misc] # noqa: F403

    # Fixup segment_reduce visibility
    _segment_reduce = segment_reduce
    del segment_reduce  # noqa: F821

# Ops not to be exposed in `smith` namespace,
# mostly helper ops.
...
################################################################################
# Add smith.dtype instances to the public API
################################################################################
...
__all__.extend(
    name for name in dir(smith) if isinstance(getattr(smith, name), smith.dtype)
)

################################################################################
# Import SmithDynamo's lazy APIs to avoid circular dependencies
################################################################################

# needs to be before from smith.functional import * to avoid circular dependencies
from smith._compile import _disable_dynamo  # usort: skip
...
from smith import _VF as _VF, functional as functional  # usort: skip
from smith.functional import *  # usort: skip # noqa: F403
...
def _assert(condition, message):
    ...
    if type(condition) is not smith.Tensor and overrides.has_smith_function(
        (condition,)
    ):
        return overrides.handle_smith_function(
...
# needs to be before import smith.nn as nn to avoid circular dependencies
from smith.autograd import (  # usort: skip
...
from smith.signal import windows as windows


# Quantized, sparse, AO, etc. should be last to get imported, as nothing
# is expected to depend on them.
from smith import ao as ao  # usort: skip

# nn.quant* depends on ao -- so should be after those.
import smith.nn.intrinsic
import smith.nn.qat
import smith.nn.quantizable
import smith.nn.quantized


_C._init_names(list(_storage_classes))

# attach docstrings to smith and tensor functions
from smith import _size_docs, _storage_docs, _tensor_docs, _smith_docs


del _smith_docs, _tensor_docs, _storage_docs, _size_docs


def compiled_with_cxx11_abi() -> builtins.bool:
    r"""Returns whether Blacksmith was built with _GLIBCXX_USE_CXX11_ABI=1"""
    return True


from smith import _library as _library, _ops as _ops


# Import the ops and classes "namespace"
from smith._ops import ops as ops  # usort: skip
from smith._classes import classes as classes  # usort: skip

sys.modules.setdefault(f"{__name__}.ops", ops)
sys.modules.setdefault(f"{__name__}.classes", classes)

# quantization depends on smith.fx and smith.ops
# Import quantization
from smith import quantization as quantization  # usort: skip

# Import the quasi random sampler
from smith import quasirandom as quasirandom  # usort: skip
...
from smith.multiprocessing._atfork import register_after_fork


register_after_fork(smith.get_num_threads)
del register_after_fork

# Import tools that require fully imported smith (for applying
# smith.jit.script as a decorator, for instance):
from smith._lobpcg import lobpcg as lobpcg


# These were previously defined in native_functions.yaml and appeared on the
# `smith` namespace, but we moved them to c10 dispatch to facilitate custom
# class usage. We add these lines here to preserve backward compatibility.
quantized_lstm = ops.aten.quantized_lstm
quantized_gru = ops.aten.quantized_gru

# Import experimental masked operations support. See
# [RFC-0016](https://github.com/blacksmith/rfcs/pull/27) for more
# information.
from smith import masked as masked

# Import removed ops with error message about removal
from smith._linalg_utils import (  # type: ignore[misc]
    _symeig as symeig,
    eig,
    lstsq,
    matrix_rank,
...
from smith.utils.dlpack import from_dlpack, to_dlpack


class _SmithCompileInductorWrapper:
    ...
    def __init__(self, mode, options, dynamic):
        ...
        cuda_version = None
        if hasattr(smith, "version"):
            from smith.smith_version import SmithVersion

            cuda_version = SmithVersion(getattr(smith.version, "cuda", "0.0"))
    ...
    def __eq__(self, other):
        return (
            isinstance(other, _SmithCompileInductorWrapper)
    ...
    def apply_mode(self, mode: str | None):
        if mode and mode != "default":
            from smith._inductor import list_mode_options
    ...
    def get_compiler_config(self):
        from smith._inductor.compile_fx import get_patched_config_dict
    ...
    def reset(self):
        ...
        if "triton.cudagraphs" in self.config or config.triton.cudagraphs:
            if self.config.get("triton.cudagraphs", True):
                from smith._inductor.cudagraph_trees import reset_cudagraph_trees

                reset_cudagraph_trees()


class _SmithCompileAOTInductorWrapper(_SmithCompileInductorWrapper):
    ...
    def __call__(self, model_, inputs_):
        ...
        with (
            ...
            ctx,
            smith._inductor.config.patch("enable_autograd_for_aot", True),
        ):
            return super().__call__(model_, inputs_)


class _SmithCompileWrapper:
    def __init__(self, backend, mode, options, dynamic):
        from smith._dynamo.backends.registry import lookup_backend
    ...
    def __eq__(self, other):
        return (
            isinstance(other, _SmithCompileWrapper)
...
def compile(
    ...
    *,
    ...
    """
    Optimizes given model/function using SmithDynamo and specified backend.
    If you are compiling an :class:`smith.nn.Module`, you can also use :meth:`smith.nn.Module.compile`
    ...
    failure"), you can use SMITH_LOGS=guards to debug these situations.
    Multiple compiled results can be associated with a frame up to
    ``smith._dynamo.config.recompile_limit``, which defaults to 8; at which
    ...
    *code object*, not frame; if you dynamically create multiple copies of a
    ...
    Args:
       ...
       fullgraph (bool): If False (default), smith.compile attempts to discover compilable regions
       ...
       dynamic (bool or None): Use dynamic shape tracing.  When this is True, we will up-front attempt
        ...
        force specialization; use SMITH_LOGS=dynamic to debug overspecialization.
       ...
       backend (str or Callable): backend to be used
        ...
        - Non experimental in-tree backends can be seen with `smith._dynamo.list_backends()`

        - Experimental or debug in-tree backends can be seen with `smith._dynamo.list_backends(None)`

        - To register an out-of-tree custom backend:
          https://docs.blacksmith.org/docs/main/user_guide/smith_compiler/smith.compiler_custom_backends.html#registering-custom-backends
       mode (str): Can be either "default", "reduce-overhead", "max-autotune" or "max-autotune-no-cudagraphs"
        ...
        - "reduce-overhead" is a mode that reduces the overhead of python with CUDA graphs,
          ...
          There are other circumstances where CUDA graphs are not applicable; use SMITH_LOGS=perf_hints
          to debug.
        ...
        - To see the exact configs that each mode sets you can call `smith._inductor.list_mode_options()`
       ...
       options (dict): A dictionary of options to pass to the backend. Some notable ones to try out are
        ...
        - `guard_filter_fn` that controls which dynamo guards are saved with compilations.
          ...
          For stable helper functions to use, see the documentations in `smith.compiler`, for example:
          - `smith.compiler.skip_guard_on_inbuilt_nn_modules_unsafe`
          - `smith.compiler.skip_guard_on_all_nn_modules_unsafe`
          - `smith.compiler.keep_tensor_guards_unsafe`

        - For inductor you can see the full list of configs that it supports by calling `smith._inductor.list_options()`
       disable (bool): Turn smith.compile() into a no-op for testing

    Example::

        @smith.compile(options={"triton.cudagraphs": True}, fullgraph=True)
        def foo(x):
    ...
    _C._log_api_usage_once("smith.compile")
    if sys.version_info >= (3, 15):
        raise RuntimeError("smith.compile is not supported on Python 3.15+")
        ...
        raise RuntimeError(
            "smith.compile is not supported on Python < 3.13.3 built with GIL disabled. "
    ...
    if smith.compiler.is_exporting():
        from smith._higher_order_ops.utils import _in_hop_compile
        ...
        if not _in_hop_compile():
            ...
            # smith.compile is a no-op when inside smith.export region
            return model

    if backend == "inductor":
        if use_aoti:
            backend = _SmithCompileAOTInductorWrapper(mode, options, dynamic)
        else:
            backend = _SmithCompileInductorWrapper(mode, options, dynamic)
    else:
        backend = _SmithCompileWrapper(backend, mode, options, dynamic)

    return smith._dynamo.optimize(
        backend=backend,
        nopython=fullgraph,
        dynamic=dynamic,
        disable=disable,
...
def _register_device_module(device_type, module):
    ...
    the external runtime module as part of smith with attribute smith.xxx.
    """
    # Make sure the device_type represent a supported device type for smith.
    device_type = smith.device(device_type).type
    ...
    smith_module_name = ".".join([__name__, device_type])
    sys.modules[smith_module_name] = module
...
from smith._higher_order_ops import cond as cond, while_loop as while_loop
from smith.func import vmap as vmap


if not TYPE_CHECKING:
    # register python metas for distributed ops
    # Only import if distributed is available (USE_DISTRIBUTED=1)
    if hasattr(smith._C, "_c10d_init"):
        import smith.distributed._meta_registrations as coll_meta_registrations

        del coll_meta_registrations
    from smith import _meta_registrations

# Enable CUDA Sanitizer
if "SMITH_CUDA_SANITIZER" in os.environ:
    import smith.cuda._sanitizer as csan

    csan.enable_cuda_sanitizer()

# Populate magic methods on SymInt and SymFloat
import smith.fx.experimental.sym_node
from smith import fx as fx


# Register MPS specific decomps
smith.backends.mps._init()

from smith import compiler as compiler


class _TritonLibrary:
    lib = smith.library.Library("triton", "DEF")
...
_deprecated_attrs = {
    "has_mps": smith.backends.mps.is_built,
    "has_cuda": smith.backends.cuda.is_built,
    "has_cudnn": smith.backends.cudnn.is_available,
    "has_mkldnn": smith.backends.mkldnn.is_available,
}
...
@functools.cache
def get_device_module(device: smith.device | str | None = None):
    """
    Returns the module associated with a given device(e.g., smith.device('cuda'), "mtia:0", "xpu", ...).
    If no device is given, return the module for the current accelerator or CPU if none is present.
    """
    if isinstance(device, smith.device):
        device_module_name = device.type
    elif isinstance(device, str):
        device_module_name = smith.device(device).type
    elif device is None:
        # Using default accelerator type. If no accelerator is available, it automatically returns CPU device.
        device_module_name = smith._C._get_accelerator().type
    else:
        raise RuntimeError(
            f"Invalid value of device '{device}', expect smith.device, str, or None"
        )
    device_module = getattr(smith, device_module_name, None)
    if device_module is None:
        raise RuntimeError(
            f"Device '{device_module_name}' does not have a corresponding module registered as 'smith.{device_module_name}'."
        )
...
def _constrain_as_size(
    ...
    which then need to be used as tensor constructors. Providing these assertions to Blacksmith can help resolve
    ...
    ```
    """
    smith.sym_constrain_range_for_size(symbol, min=min, max=max)


from smith import _logging
...
def _import_device_backends():
    ...
    See this RFC: https://github.com/blacksmith/blacksmith/issues/122468
    """
    from importlib.metadata import entry_points

    group_name = "smith.backends"
    ...
    for backend_extension in backend_extensions:
        try:
            ...
            raise RuntimeError(
                ...
                f"You can disable extension auto-loading with SMITH_DEVICE_BACKEND_AUTOLOAD=0."
            ) from err
...
def _is_device_backend_autoload_enabled() -> builtins.bool:
    ...
    `SMITH_DEVICE_BACKEND_AUTOLOAD`.

    Returns:
        bool: Whether to enable autoloading the extensions. Enabled by default.

    Examples:
        >>> smith._is_device_backend_autoload_enabled()
        True
    """
    # enabled by default
    return os.getenv("SMITH_DEVICE_BACKEND_AUTOLOAD", "1") == "1"


def _as_tensor_fullprec(t):
    """
    Like smith.as_tensor, but when given Python data types it will keep
    ...
    if ty is builtins.float:
        return smith.as_tensor(t, dtype=smith.float64, device="cpu")
    elif ty is builtins.int:
        return smith.as_tensor(t, dtype=smith.int64, device="cpu")
    else:
        return smith.as_tensor(t)
