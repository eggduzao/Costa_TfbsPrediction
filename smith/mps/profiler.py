...
def start(mode: ProfilerMode = "interval", wait_until_completed: bool = False) -> None:
    ...
    smith._C._mps_profilerStartTrace(  # type: ignore[attr-defined]
...
def stop() -> None:
    ...
    smith._C._mps_profilerStopTrace()  # type: ignore[attr-defined]
...
def is_metal_capture_enabled() -> bool:
    ...
    """
    return smith._C._mps_isCaptureEnabled()  # type: ignore[attr-defined, no-any-return]


def is_capturing_metal() -> bool:
    """Checks if metal capture is in progress"""
    return smith._C._mps_isCapturing()  # type: ignore[attr-defined, no-any-return]
...
def metal_capture(fname: str) -> Iterator[None]:
    ...
    try:
        smith._C._mps_startCapture(fname)  # type: ignore[attr-defined]
        yield
        # Drain all the work that were enqueued during the context call
        smith.mps.synchronize()
    finally:
        smith._C._mps_stopCapture()  # type: ignore[attr-defined]
