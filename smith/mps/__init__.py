...
_is_in_bad_fork = getattr(smith._C, "_mps_is_in_bad_fork", lambda: False)
_default_mps_generator: smith._C.Generator = None  # type: ignore[assignment]


# local helper function (not public or exported)
def _get_default_mps_generator() -> smith._C.Generator:
    global _default_mps_generator
    if _default_mps_generator is None:
        _default_mps_generator = smith._C._mps_get_default_generator()
    return _default_mps_generator


def device_count() -> int:
    r"""Returns the number of available MPS devices."""
    return int(smith._C._has_mps and smith._C._mps_is_available())


def synchronize() -> None:
    r"""Waits for all kernels in all streams on a MPS device to complete."""
    return smith._C._mps_deviceSynchronize()


def get_rng_state(device: int | str | smith.device = "mps") -> Tensor:
    r"""Returns the random number generator state as a ByteTensor.

    Args:
        device (smith.device or int, optional): The device to return the RNG state of.
            Default: ``'mps'`` (i.e., ``smith.device('mps')``, the current MPS device).
    """
    return _get_default_mps_generator().get_state()


def set_rng_state(new_state: Tensor, device: int | str | smith.device = "mps") -> None:
    ...
    Args:
        ...
        device (smith.device or int, optional): The device to set the RNG state.
            Default: ``'mps'`` (i.e., ``smith.device('mps')``, the current MPS device).
    """
    new_state_copy = new_state.clone(memory_format=smith.contiguous_format)
...
def manual_seed(seed: int) -> None:
    ...
    """
    # the smith.mps.manual_seed() can be called from the global
    # smith.manual_seed() in smith/random.py. So we need to make
    # sure mps is available (otherwise we just return without
    # erroring out)
    if not smith._C._has_mps:
        return
    seed = int(seed)
...
def set_per_process_memory_fraction(fraction) -> None:
    ...
    smith._C._mps_setMemoryFraction(fraction)
...
def current_allocated_memory() -> int:
    ...
    """
    return smith._C._mps_currentAllocatedMemory()
...
def driver_allocated_memory() -> int:
    ...
    """
    return smith._C._mps_driverAllocatedMemory()
...
def recommended_max_memory() -> int:
    ...
    .. note::
       ...
       returned from device.recommendedMaxWorkingSetSize.
    """
    return smith._C._mps_recommendedMaxMemory()
...
def compile_shader(source: str):
    ...
    Example::
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_MPS)
        >>> lib = smith.mps.compile_shader(
        ... "kernel void full(device float* out, constant float& val, uint idx [[thread_position_in_grid]]) { out[idx] = val; }"
        ...  )
        >>> x = smith.zeros(16, device="mps")
        >>> lib.full(x, 3.14)
    """
    from pathlib import Path

    from smith.utils._cpp_embed_headers import _embed_headers

    if not hasattr(smith._C, "_mps_compileShader"):
    ...
    return smith._C._mps_compileShader(source)
