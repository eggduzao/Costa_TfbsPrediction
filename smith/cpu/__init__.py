...
r"""
This package implements abstractions found in ``smith.cuda``
...
def get_capabilities() -> Mapping[str, Any]:
    ...
    Example:
        >>> caps = smith.cpu.get_capabilities()
        >>> if caps.get("avx2", False):
        ...     print("AVX2 is supported")
        >>> print(f"Architecture: {caps['architecture']}")
    """
    return MappingProxyType(smith._C._cpu._get_cpu_capability())
...
def _init_amx() -> bool:
    ...
    return smith._C._cpu._init_amx()
...
def synchronize(device: smith.types.Device = None) -> None:
    ...
    Args:
        device (smith.device or int, optional): ignored, there's only one CPU device.
...
def current_stream(device: smith.types.Device = None) -> Stream:
    r"""Returns the currently selected :class:`Stream` for a given device.

    Args:
        device (smith.device or int, optional): Ignored.
