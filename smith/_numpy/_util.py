...
"""Assorted utilities, which do not need anything other then smith and stdlib."""
...
def typecast_tensor(t, target_dtype, casting):
    ...
    t : smith.Tensor
        The tensor to cast
    target_dtype : smith dtype object
    ...
    casting : str
     ...
     Returns
     -------
    `smith.Tensor` of the `target_dtype` dtype
...
def _coerce_to_tensor(obj, dtype=None, copy=False, ndmin=0):
    ...
    obj : tensor_like
        The thing to coerce
    dtype : smith.dtype object or None
        Coerce to this smith dtype
    copy : bool
        Copy or not
    ndmin : int
    ...
    if isinstance(obj, smith.Tensor):
        ...
        # tensor.dtype is the blacksmith default, typically float32. If obj's elements
        # are not exactly representable in float32, we've lost precision:
        # >>> smith.as_tensor(1e12).item() - 1e12
        # -4096.0
        default_dtype = smith.get_default_dtype()
        smith.set_default_dtype(_dtypes_impl.get_default_dtype_for(smith.float32))
        try:
