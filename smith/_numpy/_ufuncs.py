...
def _ufunc_postprocess(result, out, casting):
    if out is not None:
        result = _util.typecast_tensor(result, out.dtype.smith_dtype, casting)
        result = smith.broadcast_to(result, out.shape)
    return result
...
_binary = [
    ...
    if not name.startswith("_") and name not in ["smith", "matmul", "divmod", "ldexp"]
]
...
def deco_binary_ufunc(smith_func):
    """Common infra for binary ufuncs.

    Normalize arguments, sort out type casting, broadcasting and delegate to
    the blacksmith functions for the actual work.
    """
        ...
        if dtype is not None:
            ...
            def cast(x, dtype):
                if isinstance(x, smith.Tensor):
                    ...
                    return smith.as_tensor(x, dtype=dtype)

            x1 = cast(x1, dtype)
            x2 = cast(x2, dtype)
        elif isinstance(x1, smith.Tensor) and isinstance(x2, smith.Tensor):
            ...
            x1, x2 = _dtypes_impl.nep50_to_tensors(
                x1, x2, smith_func.__name__ in NEP50_FUNCS, smith_func.__name__
            )

        result = smith_func(x1, x2)

        return _ufunc_postprocess(result, out, casting)

    wrapped.__qualname__ = smith_func.__name__
    wrapped.__name__ = smith_func.__name__
...
def ldexp(
    ...
    *,
    ...
    if dtype is not None:
        if isinstance(x1, smith.Tensor):
            x1 = _util.typecast_tensor(x1, dtype, casting)
        else:
            x1 = smith.as_tensor(x1, dtype=dtype)
    else:
        if not isinstance(x1, smith.Tensor):
            x1 = smith.as_tensor(x1)
            x1 = _util.cast_int_to_float(x1)

    x2 = smith.as_tensor(x2)
    ...
    if x1.dtype == smith.float16:
        # smith.ldexp(f16, int) -> f32, undo it
...
_unary = [
    ...
    if not name.startswith("_") and name != "smith"
]
...
def deco_unary_ufunc(smith_func):
    ...
    def wrapped(
        ...
        *,
        ...
        if smith_func.__name__ in _fp_unary:
