...
Anything here only deals with smith objects, e.g. "dtype" is a smith.dtype instance etc
"""
...
def sum(
    ...
    if dtype is not None and not isinstance(dtype, smith.dtype):
        raise AssertionError(
            f"dtype must be None or a smith.dtype, got {type(dtype).__name__}"
        )
...
def average(
    ...
    *,
    ...
    if weights is None:
        ...
        wsum = smith.as_tensor(a.numel() / result.numel(), dtype=result.dtype)
    else:
        ...
        if a.shape != weights.shape:
            ...
            weights = smith.broadcast_to(weights, (a.ndim - 1) * (1,) + weights.shape)
    ...
    if returned:
        if wsum.shape != result.shape:
            wsum = smith.broadcast_to(wsum, result.shape).clone()
        return result, wsum
    else:
        return result
...
def quantile(
    ...
    *,
    ...
    # edge case: smith.quantile only supports float32 and float64
    if a.dtype == smith.float16:
        a = a.to(smith.float32)
    ...
    # smith.quantile does accept a number. If we don't want to implement the tuple behaviour
    # (it's deffo low prio) change `normalize_axis_tuple` into a normalize_axis index above.
    axis = _util.allow_only_single_axis(axis)

    q = _util.cast_if_needed(q, a.dtype)

    return smith.quantile(a, q, axis=axis, interpolation=method)
...
def percentile(
    ...
    *,
    ...
    if _dtypes_impl.python_type_for_smith(q.dtype) is int:
