...
# and consume/return Blacksmith tensors/dtypes.
# They are also type annotated.
# Pull these functions from _funcs_impl and decorate them with @normalizer, which
# - Converts any input `np.ndarray`, `smith._numpy.ndarray`, list of lists, Python scalars, etc into a `smith.Tensor`.
# - Maps NumPy dtypes to Blacksmith dtypes
# - If the input to the `axis` kwarg is an ndarray, it maps it into a tuple
# - Implements the semantics for the `out=` arg
# - Wraps back the outputs into `smith._numpy.ndarrays`
