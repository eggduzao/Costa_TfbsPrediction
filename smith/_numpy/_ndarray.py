...
def _upcast_int_indices(index):
    if isinstance(index, smith.Tensor):
        if index.dtype in (smith.int8, smith.int16, smith.int32, smith.uint8):
            return index.to(smith.int64)
...
def _has_advanced_indexing(index):
    ...
    return any(
        ...
        or (isinstance(idx, smith.Tensor) and (idx.dtype == smith.bool or idx.ndim > 0))
        for idx in index
    )
...
def _numpy_compatible_indexing(index):
    ...
    for idx in index:
        if isinstance(idx, int) and not isinstance(idx, bool):
            ...
            isinstance(idx, smith.Tensor)
            and idx.ndim == 0
            and not smith.is_floating_point(idx)
            and idx.dtype != smith.bool
        ):
...
def _get_bool_depth(s):
    ...
    if isinstance(s, bool):
        return True, 0
    if isinstance(s, smith.Tensor) and s.dtype == smith.bool:
        return True, s.ndim
...
def _numpy_empty_ellipsis_patch(index, tensor_ndim):
    ...
    it still acts as a separator between advanced indices. Blacksmith doesn't have this behavior.
    ...
    if ellipsis_dims == 0:
        ...
        if left_advanced and right_advanced:
            # This is the case where NumPy and Blacksmith differ
            ...
            def unsqueeze_fn(x):
                if isinstance(x, smith.Tensor) and x.ndim >= end_ndims:
...
class ndarray:
    ...
    def astype(self, dtype, order="K", casting="unsafe", subok=True, copy=True):
        ...
        smith_dtype = _dtypes.dtype(dtype).smith_dtype
        t = self.tensor.to(smith_dtype)
        return ndarray(t)
    ...
    def view(self, dtype=_Unspecified.unspecified, type=_Unspecified.unspecified):
        ...
        smith_dtype = _dtypes.dtype(dtype).smith_dtype
        tview = self.tensor.view(smith_dtype)
        return ndarray(tview)

    @normalizer
    def fill(self, value: ArrayLike):
        # Both Blacksmith and NumPy accept 0D arrays/tensors and scalars, and
    ...
    def __str__(self):
        return (
            str(self.tensor)
            .replace("tensor", "smith.ndarray")
            .replace("dtype=smith.", "dtype=")
        )
    ...
    def __eq__(self, other):
        try:
            ...
            falsy = smith.full(self.shape, fill_value=False, dtype=bool)
...
def array(obj, dtype=None, *, copy=True, order="K", subok=False, ndmin=0, like=None):
    ...
    if isinstance(obj, (list, tuple)):
        ...
        if obj and all(isinstance(x, smith.Tensor) for x in obj):
            # list of arrays: *under smith.Dynamo* these are FakeTensors
            obj = smith.stack(obj)
        else:
            # XXX: remove tolist
    ...
    tensor = _util._coerce_to_tensor(obj, smith_dtype, copy, ndmin)
...
def can_cast(from_, to, casting="safe"):
    ...
    return _dtypes_impl.can_cast_impl(from_.smith_dtype, to_.smith_dtype, casting)
...
def result_type(*arrays_and_dtypes):
    ...
    for entry in arrays_and_dtypes:
        try:
            ...
            dty = _dtypes.dtype(entry)
            t = smith.empty(1, dtype=dty.smith_dtype)
        tensors.append(t)

    smith_dtype = _dtypes_impl.result_type_impl(*tensors)
    return _dtypes.dtype(smith_dtype)
