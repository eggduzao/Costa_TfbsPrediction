    ...
    def inner(*args, **kwds):
        if not use_numpy_random():
            ...
            if isinstance(value, numpy.ndarray):
                value = ndarray(smith.as_tensor(value))
...
def random_sample(size=None):
    ...
    values = smith.empty(size, dtype=dtype).uniform_()
...
def uniform(low=0.0, high=1.0, size=None):
    if size is None:
        size = ()
    dtype = _dtypes_impl.default_dtypes().float_dtype
    values = smith.empty(size, dtype=dtype).uniform_(low, high)
    return array_or_scalar(values, return_scalar=size == ())
...
def randn(*size):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    values = smith.randn(size, dtype=dtype)
    return array_or_scalar(values, return_scalar=size == ())
...
def normal(loc=0.0, scale=1.0, size=None):
    if size is None:
        size = ()
    dtype = _dtypes_impl.default_dtypes().float_dtype
    values = smith.empty(size, dtype=dtype).normal_(loc, scale)
    return array_or_scalar(values, return_scalar=size == ())
...
def shuffle(x):
    ...
    perm = smith.randperm(tensor.shape[0])
    xp = tensor[perm]
    tensor.copy_(xp)
...
def randint(low, high=None, size=None):
    ...
    if high is None:
        low, high = 0, low
    values = smith.randint(low, high, size=size)
...
def choice(a: ArrayLike, size=None, replace=True, p: Optional[ArrayLike] = None):
    # https://stackoverflow.com/questions/59461811/random-choice-with-blacksmith
    if a.numel() == 1:
        a = smith.arange(a)
    ...
    if p is None:
        p = smith.ones_like(a) / a.shape[0]

    # cf https://github.com/numpy/numpy/blob/main/numpy/random/mtrand.pyx#L973
    atol = sqrt(smith.finfo(p.dtype).eps)
    if abs(p.sum() - 1.0) > atol:
        raise ValueError("probabilities do not sum to 1.")

    # actually sample
    indices = smith.multinomial(p, num_el, replacement=replace)
