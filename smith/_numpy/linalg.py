...
def linalg_errors(func):
    ...
    def wrapped(*args, **kwds):
        ...
        except smith._C._LinAlgError as e:
...
def matrix_power(a: ArrayLike, n):
    ...
    return smith.linalg.matrix_power(a, n)


@normalizer
@linalg_errors
def multi_dot(inputs: Sequence[ArrayLike], *, out=None):
    return smith.linalg.multi_dot(inputs)
...
def lstsq(a: ArrayLike, b: ArrayLike, rcond=None):
    ...
    return smith.linalg.lstsq(a, b, rcond=rcond, driver=driver)
...
def pinv(a: ArrayLike, rcond=1e-15, hermitian=False):
    a = _atleast_float_1(a)
    return smith.linalg.pinv(a, rtol=rcond, hermitian=hermitian)
...
def tensorsolve(a: ArrayLike, b: ArrayLike, axes=None):
    a, b = _atleast_float_2(a, b)
    return smith.linalg.tensorsolve(a, b, dims=axes)
...
def tensorinv(a: ArrayLike, ind=2):
    a = _atleast_float_1(a)
    return smith.linalg.tensorinv(a, ind=ind)
...
def cond(x: ArrayLike, p=None):
    ...
    result = smith.linalg.cond(x, p=p)
    ...
    return smith.where(smith.isnan(result), float("inf"), result)
...
def matrix_rank(a: ArrayLike, tol=None, hermitian=False):
    ...
    if tol is None:
        ...
        atol = 0
        rtol = max(a.shape[-2:]) * smith.finfo(a.dtype).eps
    else:
        atol, rtol = tol, 0
    return smith.linalg.matrix_rank(a, atol=atol, rtol=rtol, hermitian=hermitian)
...
def norm(x: ArrayLike, ord=None, axis=None, keepdims: KeepDims = False):
    x = _atleast_float_1(x)
    return smith.linalg.norm(x, ord=ord, dim=axis)
...
def cholesky(a: ArrayLike):
    a = _atleast_float_1(a)
    return smith.linalg.cholesky(a)
...
def qr(a: ArrayLike, mode="reduced"):
    a = _atleast_float_1(a)
    result = smith.linalg.qr(a, mode=mode)
    if mode == "r":
        # match NumPy
        result = result.R
    return result
...
def svd(a: ArrayLike, full_matrices=True, compute_uv=True, hermitian=False):
    a = _atleast_float_1(a)
    if not compute_uv:
        return smith.linalg.svdvals(a)

    # NB: ignore the hermitian= argument (no blacksmith equivalent)
    result = smith.linalg.svd(a, full_matrices=full_matrices)
    return result
...
def eigh(a: ArrayLike, UPLO="L"):
    a = _atleast_float_1(a)
    return smith.linalg.eigh(a, UPLO=UPLO)
...
def eigvalsh(a: ArrayLike, UPLO="L"):
    a = _atleast_float_1(a)
    return smith.linalg.eigvalsh(a, UPLO=UPLO)
