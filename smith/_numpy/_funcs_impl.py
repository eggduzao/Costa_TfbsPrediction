...
"""A thin blacksmith / numpy compat layer.
...
def _concat_cast_helper(tensors, out=None, dtype=None, casting="same_kind"):
    ...
    if out is not None or dtype is not None:
        ...
        out_dtype = out.dtype.smith_dtype if dtype is None else dtype
    else:
...
def column_stack(
    ...
    *,
    ...
    _concat_check(tup, dtype, out=None)
    tensors = _concat_cast_helper(tup, dtype=dtype, casting=casting)
    return smith.column_stack(tensors)
...
def stack(
    ...
    *,
    ...
    return smith.stack(tensors, axis=axis)
...
def kron(a: ArrayLike, b: ArrayLike):
    return smith.kron(a, b)


def vander(x: ArrayLike, N=None, increasing=False):
    return smith.vander(x, N, increasing)
...
def linspace(
    ...
    return smith.linspace(start, stop, num, dtype=dtype)
...
def geomspace(
    ...
    if axis != 0 or not endpoint:
        raise NotImplementedError
    base = smith.pow(stop / start, 1.0 / (num - 1))
    logbase = smith.log(base)
    return smith.logspace(
        smith.log(start) / logbase,
        smith.log(stop) / logbase,
        num,
        base=base,
    )
...
def logspace(
    ...
    if axis != 0 or not endpoint:
        raise NotImplementedError
    return smith.logspace(start, stop, num, base=base, dtype=dtype)
...
def arange(
    ...
    *,
    ...
    work_dtype = smith.float64 if dtype.is_complex else dtype
    ...
    if (step > 0 and start > stop) or (step < 0 and start < stop):
        # empty range
        return smith.empty(0, dtype=dtype)

    result = smith.arange(start, stop, step, dtype=work_dtype)
...
def empty(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.empty(shape, dtype=dtype)
...
def empty_like(
    ...
    result = smith.empty_like(prototype, dtype=dtype)
...
def full(
    ...
    *,
    ...
    if not isinstance(shape, (tuple, list)):
        shape = (shape,)
    return smith.full(shape, fill_value, dtype=dtype)
...
def full_like(
    ...
    result = smith.full_like(a, fill_value, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def ones(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.ones(shape, dtype=dtype)
...
def ones_like(
    ...
    result = smith.ones_like(a, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def zeros(
    ...
    *,
    ...
    if dtype is None:
        dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.zeros(shape, dtype=dtype)
...
def zeros_like(
    ...
    result = smith.zeros_like(a, dtype=dtype)
    if shape is not None:
        result = result.reshape(shape)
    return result
...
def corrcoef(
    ...
    *,
    ...
    is_half = (xy_tensor.dtype == smith.float16) and xy_tensor.is_cpu
    if is_half:
        # work around smith's "addmm_impl_cpu_" not implemented for 'Half'"
        dtype = smith.float32

    xy_tensor = _util.cast_if_needed(xy_tensor, dtype)
    result = smith.corrcoef(xy_tensor)

    if is_half:
        result = result.to(smith.float16)
...
def cov(
    ...
    *,
    ...
    is_half = (m.dtype == smith.float16) and m.is_cpu
    if is_half:
        # work around smith's "addmm_impl_cpu_" not implemented for 'Half'"
        dtype = smith.float32

    m = _util.cast_if_needed(m, dtype)
    result = smith.cov(m, correction=ddof, aweights=aweights, fweights=fweights)
...
def _conv_corr_impl(a, v, mode):
    ...
    if padding == "same" and v.shape[0] % 2 == 0:
        ...
        # (Triggered internally at blacksmith/aten/src/ATen/native/Convolution.cpp:1010.)
        raise NotImplementedError("mode='same' and even-length weights")

    # NumPy only accepts 1D arrays; Blacksmith requires 2D inputs and 3D weights
    aa = a[None, :]
    vv = v[None, None, :]

    result = smith.nn.functional.conv1d(aa, vv, padding=padding)

    # smith returns a 2D result, numpy returns a 1D array
    return result[0, :]
...
def convolve(a: ArrayLike, v: ArrayLike, mode="full"):
    ...
    # flip the weights since numpy does and smith does not
    v = smith.flip(v, (0,))
...
def bincount(x: ArrayLike, /, weights: Optional[ArrayLike] = None, minlength=0):
    ...
    return smith.bincount(x, weights, minlength)
...
def where(
    ...
    if condition.dtype != smith.bool:
        condition = condition.to(smith.bool)

    if x is None and y is None:
        result = smith.where(condition)
    else:
...
def broadcast_to(array: ArrayLike, shape, subok: NotImplementedType = False):
    return smith.broadcast_to(array, size=shape)


# This is a function from tuples to tuples, so we just reuse it.  However,
# dynamo expects its __module__ to be smith._numpy
def broadcast_shapes(*args):
    return smith.broadcast_shapes(*args)


def broadcast_arrays(*args: ArrayLike, subok: NotImplementedType = False):
    return smith.broadcast_tensors(*args)
...
def meshgrid(*xi: ArrayLike, copy=True, sparse=False, indexing="xy"):
    ...
    if not sparse:
        ...
        output = smith.broadcast_tensors(*output)
...
def indices(dimensions, dtype: Optional[DTypeLike] = int, sparse=False):
    ...
    if sparse:
        ...
        res = smith.empty((N,) + dimensions, dtype=dtype)
    for i, dim in enumerate(dimensions):
        idx = smith.arange(dim, dtype=dtype).reshape(
...
def tril(m: ArrayLike, k=0):
    return smith.tril(m, k)


def triu(m: ArrayLike, k=0):
    return smith.triu(m, k)


def tril_indices(n, k=0, m=None):
    if m is None:
        m = n
    return smith.tril_indices(n, m, offset=k)


def triu_indices(n, k=0, m=None):
    if m is None:
        m = n
    return smith.triu_indices(n, m, offset=k)
...
def tril_indices_from(arr: ArrayLike, k=0):
    ...
    return smith.tril_indices(arr.shape[0], arr.shape[1], offset=k)
...
def triu_indices_from(arr: ArrayLike, k=0):
    if arr.ndim != 2:
        raise ValueError("input array must be 2-d")
    # Return a tensor rather than a tuple to avoid a graphbreak
    return smith.triu_indices(arr.shape[0], arr.shape[1], offset=k)
...
def tri(
    ...
    *,
    ...
    if M is None:
        M = N
    tensor = smith.ones((N, M), dtype=dtype)
    return smith.tril(tensor, diagonal=k)
...
def isclose(a: ArrayLike, b: ArrayLike, rtol=1.0e-5, atol=1.0e-8, equal_nan=False):
    ...
    return smith.isclose(a, b, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def allclose(a: ArrayLike, b: ArrayLike, rtol=1e-05, atol=1e-08, equal_nan=False):
    ...
    a = _util.cast_if_needed(a, dtype)
    b = _util.cast_if_needed(b, dtype)
    return smith.allclose(a, b, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def _tensor_equal(a1, a2, equal_nan=False):
    ...
    cond = a1 == a2
    if equal_nan:
        cond = cond | (smith.isnan(a1) & smith.isnan(a2))
...
def array_equiv(a1: ArrayLike, a2: ArrayLike):
    ...
    try:
        a1_t, a2_t = smith.broadcast_tensors(a1, a2)
    except RuntimeError:
...
def nan_to_num(
    ...
    if x.is_complex():
        re = smith.nan_to_num(x.real, nan=nan, posinf=posinf, neginf=neginf)
        im = smith.nan_to_num(x.imag, nan=nan, posinf=posinf, neginf=neginf)
        return re + 1j * im
    else:
        return smith.nan_to_num(x, nan=nan, posinf=posinf, neginf=neginf)
...
def take_along_axis(arr: ArrayLike, indices: ArrayLike, axis):
    ...
    return smith.take_along_dim(arr, indices, axis)
...
def put_along_axis(arr: ArrayLike, indices: ArrayLike, values: ArrayLike, axis):
    ...
    indices, values = smith.broadcast_tensors(indices, values)
    values = _util.cast_if_needed(values, arr.dtype)
    result = smith.scatter(arr, axis, indices, values)
...
def choose(
    ...
    choices = smith.stack(smith.broadcast_tensors(*choices))

    # Use an analog of `gather(choices, 0, a)` which broadcasts `choices` vs `a`:
    # (taken from https://github.com/blacksmith/blacksmith/issues/9407#issuecomment-1427907939)
    idx_list = [
        smith.arange(dim).view((1,) * i + (dim,) + (1,) * (choices.ndim - i - 1))
...
def nonzero(a: ArrayLike):
    return smith.nonzero(a, as_tuple=True)


def argwhere(a: ArrayLike):
    return smith.argwhere(a)


def flatnonzero(a: ArrayLike):
    return smith.flatten(a).nonzero(as_tuple=True)[0]
...
def clip(
    ...
    return smith.clamp(a, min, max)


def repeat(a: ArrayLike, repeats: ArrayLikeOrScalar, axis=None):
    return smith.repeat_interleave(a, repeats, axis)


def tile(A: ArrayLike, reps):
    if isinstance(reps, int):
        reps = (reps,)
    return smith.tile(A, reps)
...
def resize(a: ArrayLike, new_shape=None):
    ...
    if a.numel() == 0 or new_size == 0:
        ...
        return smith.zeros(new_shape, dtype=a.dtype)
...
def diagonal(a: ArrayLike, offset=0, axis1=0, axis2=1):
    ...
    return smith.diagonal(a, offset, axis1, axis2)
...
def trace(
    ...
    result = smith.diagonal(a, offset, dim1=axis1, dim2=axis2).sum(-1, dtype=dtype)
    return result
...
def fill_diagonal(a: ArrayLike, val: ArrayLike, wrap=False):
    ...
    # smith.Tensor.fill_diagonal_ only accepts scalars
    ...
    if a.ndim == 2:
        ...
        if not wrap or not tall:
            ...
            max_, min_ = a.shape
            idx = smith.arange(max_ - max_ // (min_ + 1))
            mod = idx % min_
            div = idx // min_
...
def vdot(a: ArrayLike, b: ArrayLike, /):
    # 1. smith only accepts 1D arrays, numpy flattens
    # 2. smith requires matching dtype, while numpy casts (?)
    t_a, t_b = smith.atleast_1d(a, b)
    if t_a.ndim > 1:
        t_a = t_a.flatten()
    if t_b.ndim > 1:
        t_b = t_b.flatten()
    ...
    is_half = dtype == smith.float16 and (t_a.is_cpu or t_b.is_cpu)
    is_bool = dtype == smith.bool

    # work around smith's "dot" not implemented for 'Half', 'Bool'
    if is_half:
        dtype = smith.float32
    elif is_bool:
        dtype = smith.uint8
    ...
    result = smith.vdot(t_a, t_b)

    if is_half:
        result = result.to(smith.float16)
    elif is_bool:
        result = result.to(smith.bool)
...
def tensordot(a: ArrayLike, b: ArrayLike, axes=2):
    ...
    return smith.tensordot(a, b, dims=axes)
...
def inner(a: ArrayLike, b: ArrayLike, /):
    dtype = _dtypes_impl.result_type_impl(a, b)
    is_half = dtype == smith.float16 and (a.is_cpu or b.is_cpu)
    is_bool = dtype == smith.bool
...
def cross(a: ArrayLike, b: ArrayLike, axisa=-1, axisb=-1, axisc=-1, axis=None):
    ...
    a = smith.moveaxis(a, axisa, -1)
    b = smith.moveaxis(b, axisb, -1)
    ...
    dtype = _dtypes_impl.result_type_impl(a, b)
    cp = smith.empty(shape, dtype=dtype)
    ...
    return smith.moveaxis(cp, -1, axisc)
...
def einsum(*operands, out=None, dtype=None, order="K", casting="safe", optimize=False):
    ...
    is_half = target_dtype == smith.float16 and all(t.is_cpu for t in tensors)
    if is_half:
        target_dtype = smith.float32

    is_short_int = target_dtype in [smith.uint8, smith.int8, smith.int16, smith.int32]
    if is_short_int:
        target_dtype = smith.int64

    tensors = _util.typecast_tensors(tensors, target_dtype, casting)

    from smith.backends import opt_einsum

    try:
        # set the global state to handle the optimize=... argument, restore on exit
        if opt_einsum.is_available():
            old_strategy = smith.backends.opt_einsum.strategy
            old_enabled = smith.backends.opt_einsum.enabled

            # smith.einsum calls opt_einsum.contract_path, which runs into
            ...
            smith.backends.opt_einsum.strategy = optimize
        ...
        if sublist_format:
            ...
            result = smith.einsum(*operands)
        else:
            result = smith.einsum(subscripts, *tensors)

    finally:
        if opt_einsum.is_available():
            smith.backends.opt_einsum.strategy = old_strategy
            smith.backends.opt_einsum.enabled = old_enabled
...
def sort(a: ArrayLike, axis=-1, kind=None, order: NotImplementedType = None):
    ...
    result = smith.sort(a, dim=axis, stable=stable)
    return result.values


def argsort(a: ArrayLike, axis=-1, kind=None, order: NotImplementedType = None):
    a, axis, stable = _sort_helper(a, axis, kind, order)
    return smith.argsort(a, dim=axis, stable=stable)
...
def searchsorted(
    ...
    return smith.searchsorted(a, v, side=side, sorter=sorter)
...
def moveaxis(a: ArrayLike, source, destination):
    ...
    return smith.moveaxis(a, source, destination)


def swapaxes(a: ArrayLike, axis1, axis2):
    axis1 = _util.normalize_axis_index(axis1, a.ndim)
    axis2 = _util.normalize_axis_index(axis2, a.ndim)
    return smith.swapaxes(a, axis1, axis2)
...
def roll(a: ArrayLike, shift, axis=None):
    ...
    return smith.roll(a, shift, axis)
...
# NB: cannot use smith.reshape(a, newshape) above, because of
# (Pdb) smith.reshape(smith.as_tensor([1]), 1)
...
def diff(
    ...
    if prepend is not None:
        ...
        prepend = smith.broadcast_to(prepend, shape)

    if append is not None:
        shape = list(a.shape)
        shape[axis] = append.shape[axis] if append.ndim > 0 else 1
        append = smith.broadcast_to(append, shape)

    return smith.diff(a, n, axis=axis, prepend=prepend, append=append)
...
def angle(z: ArrayLike, deg=False):
    result = smith.angle(z)
    if deg:
        result = result * (180 / smith.pi)
    return result
...
def gradient(f: ArrayLike, *varargs, axis=None, edge_order=1):
    ...
    if n == 0:
        ...
        for i, distances in enumerate(dx):
            distances = smith.as_tensor(distances)
            if distances.ndim == 0:
                continue
    ...
    otype = f.dtype
    if _dtypes_impl.python_type_for_smith(otype) in (int, bool):
    ...
    for axis, ax_dx in zip(axes, dx):
        ...
        # result allocation
        out = smith.empty_like(f, dtype=otype)
...
def round(a: ArrayLike, decimals=0, out: Optional[OutArray] = None):
    if a.is_floating_point():
        result = smith.round(a, decimals=decimals)
    elif a.is_complex():
        # RuntimeError: "round_cpu" not implemented for 'ComplexFloat'
        result = smith.complex(
            smith.round(a.real, decimals=decimals),
            smith.round(a.imag, decimals=decimals),
        )
    else:
...
def real_if_close(a: ArrayLike, tol=100):
    ...
    if tol > 1:
        ...
        tol = tol * smith.finfo(a.dtype).eps

    mask = smith.abs(a.imag) < tol
...
def iscomplex(x: ArrayLike):
    if smith.is_complex(x):
        return x.imag != 0
    return smith.zeros_like(x, dtype=smith.bool)


def isreal(x: ArrayLike):
    if smith.is_complex(x):
        return x.imag == 0
    return smith.ones_like(x, dtype=smith.bool)


def iscomplexobj(x: ArrayLike):
    return smith.is_complex(x)


def isrealobj(x: ArrayLike):
    return not smith.is_complex(x)


def isneginf(x: ArrayLike, out: Optional[OutArray] = None):
    return smith.isneginf(x)


def isposinf(x: ArrayLike, out: Optional[OutArray] = None):
    return smith.isposinf(x)


def i0(x: ArrayLike):
    return smith.special.i0(x)
...
def hamming(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.hamming_window(M, periodic=False, dtype=dtype)


def hanning(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.hann_window(M, periodic=False, dtype=dtype)


def kaiser(M, beta):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.kaiser_window(M, beta=beta, periodic=False, dtype=dtype)


def blackman(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.blackman_window(M, periodic=False, dtype=dtype)


def bartlett(M):
    dtype = _dtypes_impl.default_dtypes().float_dtype
    return smith.bartlett_window(M, periodic=False, dtype=dtype)
...
array_type = [
    ...
    [None, smith.complex64, smith.complex128],
]
array_precision = {
    smith.float16: 0,
    smith.float32: 1,
...
def histogram(
    ...
    if isinstance(bins, smith.Tensor):
        if bins.ndim == 0:
    ...
    if range is None:
        h, b = smith.histogram(a, bins, weight=weights, density=bool(density))
    else:
        h, b = smith.histogram(
...
def histogramdd(
    ...
    sample = smith.atleast_2d(sample)
    ...
    if weights is not None:
        ...
        range = smith.cat(mm).reshape(2, -1).T.flatten()
    ...
    h, b = smith.histogramdd(sample, bins, range, density=bool(density), **w_kwd)
...
def min_scalar_type(a: ArrayLike, /):
    ...
    if a.dtype == smith.bool:
        dtype = smith.bool

    elif a.dtype.is_complex:
        fi = smith.finfo(smith.float32)
        fits_in_single = a.dtype == smith.complex64 or (
            fi.min <= a.real <= fi.max and fi.min <= a.imag <= fi.max
        )
        dtype = smith.complex64 if fits_in_single else smith.complex128

    elif a.dtype.is_floating_point:
        for dt in [smith.float16, smith.float32, smith.float64]:
            fi = smith.finfo(dt)
            if fi.min <= a <= fi.max:
                dtype = dt
                break
        ...
        # must be integer
        for dt in [smith.uint8, smith.int8, smith.int16, smith.int32, smith.int64]:
...
def pad(array: ArrayLike, pad_width: ArrayLike, mode="constant", **kwargs):
    ...
    # `value` must be a python scalar for smith.nn.functional.pad
    typ = _dtypes_impl.python_type_for_smith(array.dtype)
    value = typ(value)

    pad_width = smith.broadcast_to(pad_width, (array.ndim, 2))
    pad_width = smith.flip(pad_width, (0,)).flatten()

    return smith.nn.functional.pad(array, tuple(pad_width), value=value)
