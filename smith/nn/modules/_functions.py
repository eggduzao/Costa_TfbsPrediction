...
from smith.autograd.function import Function
...
class SyncBatchNorm(Function):
    ...
    def forward(
        ...
        if not (
            input.is_contiguous(memory_format=smith.channels_last)
            or input.is_contiguous(memory_format=smith.channels_last_3d)
        ):
        ...
        if input.numel() > 0:
            ...
            mean, invstd = smith.batch_norm_stats(input, eps)
            ...
            # C, C, 1 -> (2C + 1)
            combined = smith.cat([mean, invstd, count], dim=0)
        else:
            # for empty input, set stats and the count to zero. The stats with
            # zero count will be filtered out later when computing global mean
            # & invstd, but they still needs to participate the all_gather
            # collective communication to unblock other peer processes.
            combined = smith.zeros(
        ...
        if process_group._get_backend_name() != "gloo":
            ...
            combined = smith.reshape(combined_flat, (world_size, combined_size))
            # world_size * (2C + 1) -> world_size * C, world_size * C, world_size * 1
            mean_all, invstd_all, count_all = smith.split(combined, num_channels, dim=1)
        else:
            # world_size * (2C + 1)
            combined_list = [smith.empty_like(combined) for _ in range(world_size)]
            dist.all_gather(combined_list, combined, process_group, async_op=False)
            combined = smith.stack(combined_list, dim=0)
            # world_size * (2C + 1) -> world_size * C, world_size * C, world_size * 1
            mean_all, invstd_all, count_all = smith.split(combined, num_channels, dim=1)

        if not (smith.cuda.is_available() and smith.cuda.is_current_stream_capturing()):
            ...
            # See https://github.com/blacksmith/blacksmith/issues/78549
            # FIXME: https://github.com/blacksmith/blacksmith/issues/78656 describes
        ...
        mean, invstd = smith.batch_norm_gather_stats_with_counts(
            input,
            mean_all,
            invstd_all,
            running_mean,
        ...
        self.save_for_backward(input, weight, mean, invstd, count_all.to(smith.int32))
        self.process_group = process_group

        # apply element-wise normalization
        if input.numel() > 0:
            return smith.batch_norm_elemt(input, weight, bias, mean, invstd, eps)
        else:
    ...
    def backward(self, grad_output):
        if not (
            grad_output.is_contiguous(memory_format=smith.channels_last)
            or grad_output.is_contiguous(memory_format=smith.channels_last_3d)
        ):
        ...
        if saved_input.numel() > 0:
            ...
            if self.needs_input_grad[0]:
                ...
                combined = smith.cat([sum_dy, sum_dy_xmu], dim=0)
                ...
                sum_dy, sum_dy_xmu = smith.split(combined, num_channels)

                # backward pass for gradient calculation
                if weight is not None and weight.dtype != mean.dtype:
                    weight = weight.to(mean.dtype)
                grad_input = smith.batch_norm_backward_elemt(
                    grad_output,
                    saved_input,
                    mean,
                    invstd,
...
class CrossMapLRN2d(Function):
    ...
    def forward(ctx, input, size, alpha=1e-4, beta=0.75, k=1):
        ...
        input_square = output
        smith.pow(input, 2, out=input_square)
        ...
        smith.pow(ctx.scale, -ctx.beta, out=output)
        output.mul_(input)
    ...
    def backward(ctx, grad_output):
        ...
        smith.pow(ctx.scale, -ctx.beta, out=grad_input).mul_(grad_output)

        paddded_ratio.zero_()
        padded_ratio_center = paddded_ratio.narrow(0, inversePrePad, channels)
        for n in range(batch_size):
            smith.mul(grad_output[n], output[n], out=padded_ratio_center)
...
class BackwardHookFunction(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
