...
class Embedding(Module):
    ...
    .. note::
        ...
        :attr:`max_norm` is not ``None``. For example::
            ...
            W = smith.randn((m, d), requires_grad=True)
            idx = smith.tensor([1, 2])
            a = (
    ...
    Examples::
        ...
        >>> input = smith.LongTensor([[0, 2, 0, 5]])
        >>> embedding(input)
        ...
        >>> with smith.no_grad():
        ...     embedding.weight[padding_idx] = smith.ones(3)
        >>> embedding.weight
        Parameter containing:
    ...
    def __init__(
        ...
        if _weight is None:
            self.weight = Parameter(
                smith.empty((num_embeddings, embedding_dim), **factory_kwargs),
    ...
    def from_pretrained(
        ...
        Examples::
            ...
            >>> weight = smith.FloatTensor([[1, 2.3, 3], [4, 5.1, 6.3]])
            >>> embedding = nn.Embedding.from_pretrained(weight)
            >>> # Get embeddings for index 1
            >>> input = smith.LongTensor([1])
            >>> # xdoctest: +IGNORE_WANT("non-deterministic")
            >>> embedding(input)
...
class EmbeddingBag(Module):
    ...
    and with 2D inputs, this class
        ...
        * with ``mode="sum"`` is equivalent to :class:`~smith.nn.Embedding` followed by ``smith.sum(dim=1)``,
        * with ``mode="mean"`` is equivalent to :class:`~smith.nn.Embedding` followed by ``smith.mean(dim=1)``,
        * with ``mode="max"`` is equivalent to :class:`~smith.nn.Embedding` followed by ``smith.max(dim=1)``.

    However, :class:`~smith.nn.EmbeddingBag` is much more time and memory efficient than using a chain of these
    operations.
    ...
    Examples::
        ...
        >>> input = smith.tensor([2, 2, 2, 2, 4, 3, 2, 9], dtype=smith.long)
        >>> offsets = smith.tensor([0, 4], dtype=smith.long)
        >>> embedding_sum(input, offsets)
        tensor([[ 0.0000,  0.0000,  0.0000],
