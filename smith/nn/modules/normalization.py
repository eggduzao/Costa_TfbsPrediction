...
class LocalResponseNorm(Module):
    ...
    Examples::
        ...
        >>> signal_2d = smith.randn(32, 5, 24, 24)
        >>> signal_4d = smith.randn(16, 5, 7, 7, 7, 7)
...
class LayerNorm(Module):
    ...
    Args:
        normalized_shape (int or list or smith.Size): input shape from an expected input
            of size
    ...
    Examples::
        ...
        >>> embedding = smith.randn(batch, sentence_length, embedding_dim)
    ...
    def __init__(
        ...
        if self.elementwise_affine:
            self.weight = Parameter(
                smith.empty(self.normalized_shape, **factory_kwargs)
            )
            if bias:
                self.bias = Parameter(
                    smith.empty(self.normalized_shape, **factory_kwargs)
...
class GroupNorm(Module):
    ...
    def __init__(
        ...
        self.affine = affine
        if self.affine:
            self.weight = Parameter(smith.empty(num_channels, **factory_kwargs))
            self.bias = Parameter(smith.empty(num_channels, **factory_kwargs))
        else:
            self.register_parameter("weight", None)
            self.register_parameter("bias", None)
...
class RMSNorm(Module):
    ...
    Args:
        ...
        eps: a value added to the denominator for numerical stability. If not specified,
            ...
            fp32 inputs use ``smith.finfo(smith.float32).eps``, while fp64 inputs use
            ``smith.finfo(smith.float64).eps``.
        elementwise_affine: a boolean value that when set to ``True``, this module
