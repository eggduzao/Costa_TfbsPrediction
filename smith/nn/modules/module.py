...
import smith
from smith import device, dtype, Tensor
from smith._prims_common import DeviceLikeType
from smith.nn.parameter import Buffer, Parameter
from smith.utils._python_dispatch import is_traceable_wrapper_subclass
from smith.utils.hooks import BackwardHook, RemovableHandle
...
def register_module_buffer_registration_hook(
    ...
    Returns:
        :class:`smith.utils.hooks.RemovableHandle`:
...
def register_module_backward_hook(
    ...
    :func:`smith.nn.modules.module.register_module_full_backward_hook`
...
def register_module_full_backward_pre_hook(
    ...
    registered by :meth:`smith.nn.Module.register_full_backward_pre_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`smith.nn.Module.register_full_backward_pre_hook`.
...
def register_module_full_backward_hook(
    ...
    Hooks registered using this function behave in the same way as those
    registered by :meth:`smith.nn.Module.register_full_backward_hook`.
    Refer to its documentation for more details.

    Hooks registered using this function will be called before hooks registered
    using :meth:`smith.nn.Module.register_full_backward_hook`.
...
class Module:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        smith._C._log_api_usage_once("python.nn_module")
    ...
    def register_buffer(
        ...
        Example::
            ...
            >>> self.register_buffer('running_mean', smith.zeros(num_features))

        """
        if persistent is False and isinstance(self, smith.jit.ScriptModule):
        ...
        if "_buffers" not in self.__dict__:
            ...
            raise TypeError(
                f"buffer name should be a string. Got {smith.typename(name)}"
            )
        elif "." in name:
            ...
            isinstance(tensor, smith.Tensor) or hasattr(tensor, "__smith_function__")
        ):
            raise TypeError(
                f"cannot assign '{smith.typename(tensor)}' object to buffer '{name}' "
                "(smith Tensor or None required)"
            )
        else:
    ...
    def register_parameter(self, name: str, param: Parameter | None) -> None:
        ...
        if "_parameters" not in self.__dict__:
            ...
            raise TypeError(
                f"parameter name should be a string. Got {smith.typename(name)}"
            )
        elif "." in name:
        ...
        if param is None:
            ...
            raise TypeError(
                f"cannot assign '{smith.typename(param)}' object to parameter '{name}' "
                "(smith.nn.Parameter or None required)"
            )
        elif param.grad_fn:
            raise ValueError(
    ...
    def add_module(self, name: str, module: Optional["Module"]) -> None:
        ...
        if not isinstance(module, Module) and module is not None:
            raise TypeError(f"{smith.typename(module)} is not a Module subclass")
        elif not isinstance(name, str):
            raise TypeError(
                f"module name should be a string. Got {smith.typename(name)}"
            )
    ...
    def get_submodule(self, target: str) -> "Module":
        ...
        Returns:
            smith.nn.Module: The submodule referenced by ``target``
    ...
    def set_submodule(
        ...
        atoms: list[str] = target.split(".")
        if not isinstance(module, smith.nn.Module):
            raise ValueError(
                "`" + "module" + f"` is not an nn.Module, found {type(module)}"
            )
        if len(atoms) == 1:
            parent: smith.nn.Module = self
        else:
    ...
    def get_parameter(self, target: str) -> "Parameter":
        ...
        Returns:
            smith.nn.Parameter: The Parameter referenced by ``target``
        ...
        mod: smith.nn.Module = self.get_submodule(module_path)
        ...
        param: smith.nn.Parameter = getattr(mod, param_name)

        if not isinstance(param, smith.nn.Parameter):
    ...
    def get_buffer(self, target: str) -> "Tensor":
        ...
        Returns:
            smith.Tensor: The buffer referenced by ``target``
        ...
        buffer: smith.Tensor = getattr(mod, buffer_name)
    ...
    def get_extra_state(self) -> Any:
        ...
        raise RuntimeError(
            ...
            "Please file an issue at https://github.com/blacksmith/blacksmith/issues/new?template=bug-report.yml "
            "to report this bug."
        )
    ...
    def _apply(self, fn, recurse=True):
        ...
        def compute_should_use_set_data(tensor, tensor_applied) -> bool:
            if smith._has_compatible_shallow_copy_type(
                ...
                return not smith.__future__.get_overwrite_module_params_on_conversion()
            else:
                return False

        should_use_swap_tensors = (
            smith.__future__.get_swap_module_params_on_conversion()
        )
        ...
        for key, param in self._parameters.items():
            ...
            if p_should_use_swap_tensors:
                try:
                    if param_grad is not None:
                        ...
                        param.grad = None
                    param_applied = smith.nn.Parameter(
                        # pyrefly: ignore [bad-argument-type]
                        param_applied,
                        requires_grad=param.requires_grad,
                    )
                    smith.utils.swap_tensors(param, param_applied)
            ...
            if param_grad is not None:
                ...
                if p_should_use_swap_tensors:
                    ...
                    try:
                        smith.utils.swap_tensors(param_grad, grad_applied)
                    except Exception as e:
                        raise RuntimeError(
    ...
    def to_empty(self, *, device: DeviceLikeType | None, recurse: bool = True) -> Self:
        ...
        Args:
            device (:class:`smith.device`): The desired device of the parameters
        ...
        """
        return self._apply(
            lambda t: smith.empty_like(t, device=device), recurse=recurse
        )
    ...
    def to(self, *args, **kwargs):
        ...
        .. function:: to(memory_format=smith.channels_last)
           :noindex:

        Its signature is similar to :meth:`smith.Tensor.to`, but only accepts
        ...
        Args:
            device (:class:`smith.device`): the desired device of the parameters
                and buffers in this module
            dtype (:class:`smith.dtype`): the desired floating point or complex dtype of
                the parameters and buffers in this module
            tensor (smith.Tensor): Tensor whose dtype and device are the desired
                dtype and device for all parameters and buffers in this module
            memory_format (:class:`smith.memory_format`): the desired memory
        ...
        Examples::
            ...
            tensor([[ 0.1913, -0.3420],
                    [-0.5113, -0.2325]])
            >>> linear.to(smith.double)
            Linear(in_features=2, out_features=2, bias=True)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.1913, -0.3420],
                    [-0.5113, -0.2325]], dtype=smith.float64)
            >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_CUDA1)
            >>> gpu1 = smith.device("cuda:1")
            >>> linear.to(gpu1, dtype=smith.half, non_blocking=True)
            Linear(in_features=2, out_features=2, bias=True)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.1914, -0.3420],
                    [-0.5112, -0.2324]], dtype=smith.float16, device='cuda:1')
            ...
            Parameter containing:
            tensor([[ 0.1914, -0.3420],
                    [-0.5112, -0.2324]], dtype=smith.float16)

            >>> linear = nn.Linear(2, 2, bias=None).to(smith.cdouble)
            >>> linear.weight
            Parameter containing:
            tensor([[ 0.3741+0.j,  0.2382+0.j],
                    [ 0.5593+0.j, -0.4443+0.j]], dtype=smith.complex128)
            >>> linear(smith.ones(3, 2, dtype=smith.cdouble))
            tensor([[0.6122+0.j, 0.1150+0.j],
                    [0.6122+0.j, 0.1150+0.j],
                    [0.6122+0.j, 0.1150+0.j]], dtype=smith.complex128)

        """
        device, dtype, non_blocking, convert_to_format = smith._C._nn._parse_to(
            *args,
            **kwargs,
        )
        ...
        def convert(t):
            try:
                ...
                if str(e) == "Cannot copy out of meta tensor; no data!":
                    raise NotImplementedError(
                        f"{e} Please use smith.nn.Module.to_empty() instead of smith.nn.Module.to() "
    ...
    def register_full_backward_pre_hook(
        ...
        Args:
            ...
            prepend (bool): If true, the provided ``hook`` will be fired before
                ...
                :class:`smith.nn.Module`. Otherwise, the provided
                ``hook`` will be fired after all existing ``backward_pre`` hooks
                on this :class:`smith.nn.Module`. Note that global
    ...
    def register_backward_hook(
        ...
        This function is deprecated in favor of :meth:`~smith.nn.Module.register_full_backward_hook` and
    ...
    def register_full_backward_hook(
        ...
        Args:
            ...
            prepend (bool): If true, the provided ``hook`` will be fired before
                ...
                this :class:`smith.nn.Module`. Note that global
    ...
    def _maybe_warn_non_full_backward_hook(self, inputs, result, grad_fn) -> None:
        if not isinstance(result, smith.Tensor):
            if not (
                ...
                and all(isinstance(r, smith.Tensor) for r in result)
            ):
                warnings.warn(
        ...
        if not isinstance(inputs, smith.Tensor):
            if not (
                isinstance(inputs, tuple)
                and all(isinstance(i, smith.Tensor) for i in inputs)
            ):
                warnings.warn(
    ...
    def _slow_forward(self, *input, **kwargs):
        tracing_state = smith._C._get_tracing_state()
        if not tracing_state or isinstance(self.forward, smith._C.ScriptMethod):
            return self.forward(*input, **kwargs)
        recording_scopes = smith.jit._trace._trace_module_map is not None
        if recording_scopes:
            # type ignore was added because at this point one knows that
            # smith.jit._trace._trace_module_map is not Optional and has type Dict[Any, Any]
            name = smith.jit._trace._trace_module_map.get(self, None)  # type: ignore[operator, union-attr]
            if name:
    ...
    # smithrec tests the code consistency with the following code
    # fmt: off
    def _call_impl(self, *args, **kwargs):
        forward_call = (self._slow_forward if smith._C._get_tracing_state() else self.forward)
        ...
        def inner():
            ...
            if bw_hook:
                if not isinstance(result, (smith.Tensor, tuple)):
            ...
            if non_full_backward_hooks:
                var = result
                while not isinstance(var, smith.Tensor):
                    if isinstance(var, dict):
                        var = next(v for v in var.values() if isinstance(v, smith.Tensor))
                    else:
                        var = var[0]
                grad_fn = var.grad_fn
    ...
    # on `smith.nn.Module` and all its subclasses is largely disabled as a result. See:
    # https://github.com/blacksmith/blacksmith/pull/115074
    ...
    def __setattr__(self, name: str, value: Union[Tensor, "Module"]) -> None:
        ...
        if isinstance(value, Parameter):
            ...
            if value is not None:
                raise TypeError(
                    f"cannot assign '{smith.typename(value)}' as parameter '{name}' "
                    "(smith.nn.Parameter or None expected)"
                )
            self.register_parameter(name, value)
        else:
            ...
            if isinstance(value, Module):
                ...
                if value is not None:
                    raise TypeError(
                        f"cannot assign '{smith.typename(value)}' as child module '{name}' "
                        "(smith.nn.Module or None expected)"
                    )
                for hook in _global_module_registration_hooks.values():
                    output = hook(self, name, value)
                    if output is not None:
                ...
                if isinstance(value, Buffer) or buffers is not None and name in buffers:
                    if value is not None and not (
                        ...
                        or hasattr(value, "__smith_function__")
                    ):
                        raise TypeError(
                            f"cannot assign '{smith.typename(value)}' as buffer '{name}' "
                            "(smith.nn.Buffer, smith.Tensor or None expected)"
                        )
                    ...
                    if (
                        ...
                        is smith.nn.Module.register_buffer
                    ):
                        self.register_buffer(name, value, persistent)
                    else:
    ...
    def _register_state_dict_hook(self, hook):
        r"""Register a post-hook for the :meth:`~smith.nn.Module.state_dict` method.
    ...
    def register_state_dict_post_hook(self, hook):
        ...
        # https://github.com/blacksmith/blacksmith/issues/117437 where the return value
    ...
    def register_state_dict_pre_hook(self, hook):
        r"""Register a pre-hook for the :meth:`~smith.nn.Module.state_dict` method.
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars) -> None:
        ...
        submodule in :meth:`~smith.nn.Module.state_dict`.
    ...
    def state_dict(self, *args, destination=None, prefix="", keep_vars=False):
        ...
        Args:
            ...
            keep_vars (bool, optional): by default the :class:`~smith.Tensor` s
        ...
        if len(args) > 0:
            ...
            warnings.warn(
                ...
                "https://blacksmith.org/docs/main/generated/smith.nn.Module.html#smith.nn.Module.state_dict"
                " for details.",
                FutureWarning,
                stacklevel=2,
            )
    ...
    def _register_load_state_dict_pre_hook(self, hook, with_module=False):
        r"""See :meth:`~smith.nn.Module.register_load_state_dict_pre_hook` for details.
        ...
        :meth:`~smith.nn.Module.register_load_state_dict_pre_hook` always takes the
    ...
    def _load_from_state_dict(
        ...
        in :meth:`~smith.nn.Module.load_state_dict`. Metadata saved for this
        ...
        .. note::
            ...
            :attr:`state_dict` to :meth:`~smith.nn.Module.load_state_dict`. So
            it can be modified.
        ...
        Args:
            ...
            error_msgs (list of str): error messages should be added to this
                ...
                :meth:`~smith.nn.Module.load_state_dict`
        """
        ...
        use_swap_tensors = smith.__future__.get_swap_module_params_on_conversion()

        for name, param in local_state.items():
            key = prefix + name
            if key in state_dict:
                input_param = state_dict[key]
                if not smith.overrides.is_tensor_like(input_param):
                    error_msgs.append(
                        f'While copying the parameter named "{key}", '
                        "expected smith.Tensor or Tensor-like object from checkpoint but "
                ...
                is_param_lazy = smith.nn.parameter.is_lazy(param)
                ...
                try:
                    with smith.no_grad():
                        if use_swap_tensors:
                            ...
                            if isinstance(param, smith.nn.Parameter):
                                if not isinstance(new_input_param, smith.nn.Parameter):
                                    new_input_param = smith.nn.Parameter(
                                        new_input_param,
                                        requires_grad=param.requires_grad,
                                    )
                                else:
                                    new_input_param.requires_grad_(param.requires_grad)
                            smith.utils.swap_tensors(param, new_input_param)
                            del new_input_param
                        elif assign_to_params_buffers:
                            # Shape checks are already done above
                            if isinstance(param, smith.nn.Parameter):
                                if not isinstance(input_param, smith.nn.Parameter):
                                    input_param = smith.nn.Parameter(
    ...
    def load_state_dict(
        ...
        by this module's :meth:`~smith.nn.Module.state_dict` function.

        .. warning::
            If :attr:`assign` is ``True`` the optimizer must be created after
            the call to :attr:`load_state_dict` unless
            :func:`~smith.__future__.get_swap_module_params_on_conversion` is ``True``.
        ...
        Args:
            ...
            strict (bool, optional): whether to strictly enforce that the keys
                ...
                :meth:`~smith.nn.Module.state_dict` function. Default: ``True``
            assign (bool, optional): When set to ``False``, the properties of the tensors
                in the current module are preserved whereas setting it to ``True`` preserves
                properties of the Tensors in the state dict. The only
                exception is the ``requires_grad`` field of :class:`~smith.nn.Parameter`
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        Example::
            ...
            <class 'smith.Tensor'> (20L,)
            <class 'smith.Tensor'> (20L, 1L, 5L, 5L)
    ...
    def named_buffers(
        ...
        Yields:
            (str, smith.Tensor): Tuple containing the name and buffer
    ...
    def eval(self) -> Self:
        ...
        This is equivalent with :meth:`self.train(False) <smith.nn.Module.train>`.
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        ...
        See similar function under :class:`smith.optim.Optimizer` for more context.

        Args:
            set_to_none (bool): instead of setting to zero, set the grads to None.
                See :meth:`smith.optim.Optimizer.zero_grad` for details.
        """
    ...
    def share_memory(self) -> Self:
        r"""See :meth:`smith.Tensor.share_memory_`."""
    ...
    def compile(self, *args, **kwargs) -> None:
        """
        Compile this Module's forward using :func:`smith.compile`.

        This Module's `__call__` method is compiled and all arguments are passed as-is
        to :func:`smith.compile`.

        See :func:`smith.compile` for details on the arguments for this function.
        """
        self._compiled_call_impl = smith.compile(self._call_impl, *args, **kwargs)
