...
import smith
from smith._jit_internal import _copy_to_script_wrapper
...
# Copied from smith.nn.modules.module, required for a custom __repr__ for ModuleList
...
class Sequential(Module):
    ...
    :class:`smith.nn.ModuleList`? A ``ModuleList`` is exactly what it
    ...
    # with Any as SmithScript expects a more precise type
...
class ModuleList(Module):
    ...
    :class:`~smith.nn.ModuleList` can be indexed like a regular Python list, but
    modules it contains are properly registered, and will be visible by all
    :class:`~smith.nn.Module` methods.
...
class ModuleDict(Module):
    ...
    :class:`~smith.nn.ModuleDict` can be indexed like a regular Python dictionary,
    but modules it contains are properly registered, and will be visible by all
    :class:`~smith.nn.Module` methods.

    :class:`~smith.nn.ModuleDict` is an **ordered** dictionary that respects

    * the order of insertion, and

    * in :meth:`~smith.nn.ModuleDict.update`, the order of the merged
      ``OrderedDict``, ``dict`` or another
      :class:`~smith.nn.ModuleDict` (the argument to
      :meth:`~smith.nn.ModuleDict.update`).

    Note that :meth:`~smith.nn.ModuleDict.update` with other unordered mapping
    ...
    def update(self, modules: Mapping[str, Module]) -> None:
        r"""Update the :class:`~smith.nn.ModuleDict` with key-value pairs from a mapping, overwriting existing keys.

        .. note::
            If :attr:`modules` is an ``OrderedDict``, a :class:`~smith.nn.ModuleDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            modules (iterable): a mapping (dictionary) from string to :class:`~smith.nn.Module`,
                or an iterable of key-value pairs of type (string, :class:`~smith.nn.Module`)
        """
        if not isinstance(modules, container_abcs.Iterable):
            raise TypeError(
...
class ParameterList(Module):
    ...
    :class:`~smith.nn.ParameterList` can be used like a regular Python
    list, but Tensors that are :class:`~smith.nn.Parameter` are properly registered,
    and will be visible by all :class:`~smith.nn.Module` methods.

    Note that the constructor, assigning an element of the list, the
    :meth:`~smith.nn.ParameterList.append` method and the :meth:`~smith.nn.ParameterList.extend`
    method will convert any :class:`~smith.Tensor` into :class:`~smith.nn.Parameter`.
        ...
        class MyModule(nn.Module):
            def __init__(self) -> None:
                ...
                self.params = nn.ParameterList(
                    [nn.Parameter(smith.randn(10, 10)) for i in range(10)]
                )
    ...
    def __setitem__(self, idx: int, param: Any) -> None:
        ...
        idx = self._get_abs_string_index(idx)
        if isinstance(param, smith.Tensor) and not isinstance(param, Parameter):
    ...
    def extra_repr(self) -> str:
        ...
        for k, p in enumerate(self):
            if isinstance(p, smith.Tensor):
                size_str = "x".join(str(size) for size in p.size())
                if p.device.type in ["cuda", smith._C._get_privateuse1_backend_name()]:
...
class ParameterDict(Module):
    ...
    :class:`~smith.nn.ParameterDict` is an **ordered** dictionary.
    :meth:`~smith.nn.ParameterDict.update` with other unordered mapping
    types (e.g., Python's plain ``dict``) does not preserve the order of the
    merged mapping. On the other hand, ``OrderedDict`` or another :class:`~smith.nn.ParameterDict`
    will preserve their ordering.

    Note that the constructor, assigning an element of the dictionary and the
    :meth:`~smith.nn.ParameterDict.update` method will convert any :class:`~smith.Tensor` into
    :class:`~smith.nn.Parameter`.
        ...
        class MyModule(nn.Module):
            def __init__(self) -> None:
                ...
                self.params = nn.ParameterDict(
                    {
                        "left": nn.Parameter(smith.randn(5, 10)),
                        "right": nn.Parameter(smith.randn(5, 10)),
                    }
                )
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        ...
        self._keys[key] = None
        attr = self._key_to_attr(key)
        if isinstance(value, smith.Tensor) and not isinstance(value, Parameter):
    ...
    def copy(self) -> ParameterDict:
        """Return a copy of this :class:`~smith.nn.ParameterDict` instance."""
    ...
    def update(self, parameters: Mapping[str, Any] | ParameterDict) -> None:
        r"""Update the :class:`~smith.nn.ParameterDict` with key-value pairs from ``parameters``, overwriting existing keys.

        .. note::
            If :attr:`parameters` is an ``OrderedDict``, a :class:`~smith.nn.ParameterDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            parameters (iterable): a mapping (dictionary) from string to
                :class:`~smith.nn.Parameter`, or an iterable of
                key-value pairs of type (string, :class:`~smith.nn.Parameter`)
        """
