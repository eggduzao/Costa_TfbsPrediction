...
from smith.nn.parameter import Parameter, UninitializedBuffer, UninitializedParameter
...
class _NormBase(Module):
    ...
    def __init__(
        ...
        if self.affine:
            self.weight = Parameter(smith.empty(num_features, **factory_kwargs))
            self.bias = Parameter(smith.empty(num_features, **factory_kwargs))
        else:
            self.register_parameter("weight", None)
            self.register_parameter("bias", None)
        if self.track_running_stats:
            self.register_buffer(
                "running_mean", smith.zeros(num_features, **factory_kwargs)
            )
            self.register_buffer(
                "running_var", smith.ones(num_features, **factory_kwargs)
            )
        ...
        if (version is None or version < 2) and self.track_running_stats:
            ...
            if num_batches_tracked_key not in state_dict:
                state_dict[num_batches_tracked_key] = (
                    ...
                    and self.num_batches_tracked.device != smith.device("meta")
                    else smith.tensor(0, dtype=smith.long)
                )
...
class _LazyNormBase(LazyModuleMixin, _NormBase):
    ...
    def __init__(
        ...
        if self.track_running_stats:
            ...
            self.num_batches_tracked = smith.tensor(
                0,
                dtype=smith.long,
                # pyrefly: ignore [bad-argument-type]
                **{k: v for k, v in factory_kwargs.items() if k != "dtype"},
...
class BatchNorm1d(_BatchNorm):
    ...
    equivalent to ``smith.var(input, correction=0)``. However, the value stored in the
    moving average of the variance is calculated via the unbiased  estimator, equivalent to
    ``smith.var(input, correction=1)``.
...
class LazyBatchNorm1d(_LazyNormBase, _BatchNorm):
    r"""A :class:`smith.nn.BatchNorm1d` module with lazy initialization.
...
class BatchNorm2d(_BatchNorm):
    ...
    ``smith.var(input, correction=0)``. However, the value stored in the moving average of the
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 100, 35, 45)
        >>> output = m(input)
    """
...
class LazyBatchNorm2d(_LazyNormBase, _BatchNorm):
    r"""A :class:`smith.nn.BatchNorm2d` module with lazy initialization.
...
class BatchNorm3d(_BatchNorm):
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 100, 35, 45, 10)
        >>> output = m(input)
    """
...
class LazyBatchNorm3d(_LazyNormBase, _BatchNorm):
    r"""A :class:`smith.nn.BatchNorm3d` module with lazy initialization.
...
class SyncBatchNorm(_BatchNorm):
    ...
    :class:`~smith.nn.DistributedDataParallel` (DDP) with single GPU per process. Use
    :meth:`smith.nn.SyncBatchNorm.convert_sync_batchnorm()` to convert
    ...
    Examples::
        ...
        >>> # part of the group.
        >>> process_groups = [smith.distributed.new_group(pids) for pids in [r1, r2]]
        ...
        >>> ddp_sync_bn_network = smith.nn.parallel.DistributedDataParallel(
        >>>                         sync_bn_network,
        ...
        if need_sync:
            ...
            if input.device.type not in [
                ...
                raise ValueError(
                    ...
                    f"{smith._C._get_privateuse1_backend_name()}"
                )

            process_group = smith.distributed.group.WORLD
            if self.process_group:
                process_group = self.process_group
            world_size = smith.distributed.get_world_size(process_group)
    ...
    def convert_sync_batchnorm(cls, module, process_group=None):
        r"""Converts all :attr:`BatchNorm*D` layers in the model to :class:`smith.nn.SyncBatchNorm` layers.
        ...
        Returns:
            The original :attr:`module` with the converted :class:`smith.nn.SyncBatchNorm`
            layers. If the original :attr:`module` is a :attr:`BatchNorm*D` layer,
            a new :class:`smith.nn.SyncBatchNorm` layer object will be returned
            instead.
        ...
        Example::
            ...
            >>> process_groups = [smith.distributed.new_group(pids) for pids in [r1, r2]]
            >>> process_group = process_groups[0 if dist.get_rank() <= 3 else 1]
            >>> sync_bn_module = smith.nn.SyncBatchNorm.convert_sync_batchnorm(module, process_group)

        """
        module_output = module
        if isinstance(module, smith.nn.modules.batchnorm._BatchNorm):
            module_output = smith.nn.SyncBatchNorm(
                module.num_features,
                module.eps,
                module.momentum,
                module.affine,
