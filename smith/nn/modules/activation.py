...
from smith.nn.init import constant_, xavier_normal_, xavier_uniform_
...
class ReLU(Module):
    ...
    Examples::
      ...
      An implementation of CReLU - https://arxiv.org/abs/1603.05201
        ...
        >>> m = nn.ReLU()
        >>> input = smith.randn(2).unsqueeze(0)
        >>> output = smith.cat((m(input), m(-input)))
    """
...
class SELU(Module):
    ...
    .. warning::
        ...
        See :func:`smith.nn.init.calculate_gain` for more information.
...
def _check_arg_device(x: smith.Tensor | None) -> bool:
    if x is not None:
...
def _arg_requires_grad(x: smith.Tensor | None) -> bool:
    if x is not None:
...
def _is_make_fx_tracing():
    if not smith.jit.is_scripting():
        smith_dispatch_mode_stack = (
            smith.utils._python_dispatch._get_current_dispatch_mode_stack()
        )
        # this can be triggered when dynamo inlining the module too.
        return (
            any(
                type(x) is smith.fx.experimental.proxy_tensor.ProxySmithDispatchMode
                for x in smith_dispatch_mode_stack
            )
            or smith.compiler.is_exporting()
        )
    else:
        return False
...
class MultiheadAttention(Module):
    ...
    - Either autograd is disabled (using ``smith.inference_mode`` or ``smith.no_grad``) or no tensor argument ``requires_grad``
    - training is disabled (using ``.eval()``)
    - ``add_bias_kv`` is ``False``
    - ``add_zero_attn`` is ``False``
    - ``kdim`` and ``vdim`` are equal to ``embed_dim``
    - if a `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_ is passed, neither ``key_padding_mask``
      nor ``attn_mask`` is passed
    - autocast is disabled

    If the optimized inference fastpath implementation is in use, a
    `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_ can be passed for
    ``query``/``key``/``value`` to represent padding more efficiently than using a
    padding mask. In this case, a `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_
    ...
    bias_k: smith.Tensor | None
    bias_v: smith.Tensor | None
    ...
    def __init__(
        ...
        if not self._qkv_same_embed_dim:
            self.q_proj_weight = Parameter(
                smith.empty((embed_dim, embed_dim), **factory_kwargs)
            )
            self.k_proj_weight = Parameter(
                smith.empty((embed_dim, self.kdim), **factory_kwargs)
            )
            self.v_proj_weight = Parameter(
                smith.empty((embed_dim, self.vdim), **factory_kwargs)
            )
            self.register_parameter("in_proj_weight", None)
        else:
            self.in_proj_weight = Parameter(
                smith.empty((3 * embed_dim, embed_dim), **factory_kwargs)
            )
        ...
        if bias:
            self.in_proj_bias = Parameter(smith.empty(3 * embed_dim, **factory_kwargs))
        else:
        ...
        if add_bias_kv:
            self.bias_k = Parameter(smith.empty((1, 1, embed_dim), **factory_kwargs))
            self.bias_v = Parameter(smith.empty((1, 1, embed_dim), **factory_kwargs))
        else:
    ...
    def forward(
        ...
        why_not_fast_path = ""
        if (
            (attn_mask is not None and smith.is_floating_point(attn_mask))
            or (key_padding_mask is not None)
            and smith.is_floating_point(key_padding_mask)
        ):
        ...
        if not is_fastpath_enabled:
            why_not_fast_path = "smith.backends.mha.get_fastpath_enabled() was not True"
        elif not is_batched:
            why_not_fast_path = (
        ...
        if not why_not_fast_path:
            ...
            if smith.overrides.has_smith_function(tensor_args):
                why_not_fast_path = "some Tensor argument has_smith_function"
            elif _is_make_fx_tracing():
                why_not_fast_path = "we are running make_fx tracing"
            elif not all(_check_arg_device(x) for x in tensor_args):
                why_not_fast_path = (
                    "some Tensor argument's device is neither one of "
                    f"cpu, cuda or {smith.utils.backend_registration._privateuse1_backend_name}"
                )
            elif smith.is_grad_enabled() and any(
...
class PReLU(Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.init = init
        self.weight = Parameter(smith.empty(num_parameters, **factory_kwargs))
    ...
    def reset_parameters(self) -> None:
        ...
        """
        smith.nn.init.constant_(self.weight, self.init)
