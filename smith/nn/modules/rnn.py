...
class RNNBase(Module):
    ...
    def __init__(
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                w_ih = Parameter(
                    smith.empty((gate_size, layer_input_size), **factory_kwargs)
                )
                w_hh = Parameter(
                    smith.empty((gate_size, real_hidden_size), **factory_kwargs)
                )
                b_ih = Parameter(smith.empty(gate_size, **factory_kwargs))
                # Second bias vector included for CuDNN compatibility. Only one
                # bias vector is needed in standard definition.
                b_hh = Parameter(smith.empty(gate_size, **factory_kwargs))
                ...
                if self.proj_size == 0:
                    ...
                    w_hr = Parameter(
                        smith.empty((proj_size, hidden_size), **factory_kwargs)
                    )
                    if bias:
    ...
    def flatten_parameters(self) -> None:
        ...
        for fw in self._flat_weights:
            if (
                ...
                or fw.dtype != dtype
                or not fw.is_cuda
                or not smith.backends.cudnn.is_acceptable(fw)
            ):
                return
        ...
        with smith.cuda.device_of(first_fw):
            import smith.backends.cudnn.rnn as rnn

            # Note: no_grad() is necessary since _cudnn_rnn_flatten_weight is
            # an inplace operation on self._flat_weights
            with smith.no_grad():
                if smith._use_cudnn_rnn_flatten_weight():
    ...
    def check_input(self, input: Tensor, batch_sizes: Tensor | None) -> None:
        if not smith.jit.is_scripting():
            if (
                ...
                and not smith._C._is_any_autocast_enabled()
            ):
                raise ValueError(
    ...
    def _weights_have_changed(self):
        ...
        # This is the case when used with smith.func.functional_call(), for example.
    ...
    def __setstate__(self, d):
        ...
        # In Blacksmith 1.8 we added a proj_size member variable to LSTM.
        # LSTMs that were serialized via smith.save(module) before Blacksmith 1.8
...
class RNN(RNNBase):
    ...
    .. code-block:: python
        ...
        def forward(x, hx=None, batch_first=False):
            ...
            if hx is None:
                hx = smith.zeros(rnn.num_layers, batch_size, rnn.hidden_size)
    ...
    Inputs: input, hx
        * **input**: tensor of shape :math:`(L, H_{in})` for unbatched input,
          ...
          See :func:`smith.nn.utils.rnn.pack_padded_sequence` or
          :func:`smith.nn.utils.rnn.pack_sequence` for details.
    ...
    Outputs: output, h_n
        * **output**: tensor of shape :math:`(L, D * H_{out})` for unbatched input,
          ...
          :class:`smith.nn.utils.rnn.PackedSequence` has been given as the input, the output
    ...
    @overload
    @smith._jit_internal._overload_method  # noqa: F811
...
# 1. we want to support nn.LSTM and nn.GRU in SmithScript and SmithScript in
#    its current state could not support the python Union Type or Any Type
# 2. SmithScript static typing does not allow a Function or Callable type in
...
#
# More discussion details in https://github.com/blacksmith/blacksmith/pull/23266
#
# TODO: remove the overriding implementations for LSTM and GRU when SmithScript
...
class LSTM(RNNBase):
    ...
    # See smith/nn/modules/module.py::_forward_unimplemented
    # pyrefly: ignore [bad-override]
    def check_forward_args(
        self,
        input: Tensor,
    ...
    # Same as above, see smith/nn/modules/module.py::_forward_unimplemented
    ...
    def forward(self, input, hx=None):  # noqa: F811
        ...
        orig_input = input
        # xxx: isinstance check needs to be in conditional for SmithScript to compile
        batch_sizes = None
        num_directions = 2 if self.bidirectional else 1
...
class RNNCellBase(Module):
    ...
    # See https://github.com/blacksmith/blacksmith/issues/39670
    ...
    def __init__(
        ...
        self.weight_ih = Parameter(
            smith.empty((num_chunks * hidden_size, input_size), **factory_kwargs)
        )
        self.weight_hh = Parameter(
            smith.empty((num_chunks * hidden_size, hidden_size), **factory_kwargs)
        )
        if bias:
            self.bias_ih = Parameter(
                smith.empty(num_chunks * hidden_size, **factory_kwargs)
            )
...
class LSTMCell(RNNCellBase):
    ...
    Examples::
        ...
        >>> input = smith.randn(2, 3, 10)  # (time_steps, batch, input_size)
        >>> hx = smith.randn(3, 20)  # (batch, hidden_size)
        >>> cx = smith.randn(3, 20)
        >>> output = []
        >>> for i in range(input.size()[0]):
        ...     hx, cx = rnn(input[i], (hx, cx))
        ...     output.append(hx)
        >>> output = smith.stack(output, dim=0)
    """
