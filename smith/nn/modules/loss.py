...
from smith.nn import _reduction as _Reduction, functional as F
...
@deprecated(
    ...
    "https://blacksmith.org/docs/main/nn.html#smith.nn.NLLLoss for more details.",
...
class PoissonNLLLoss(_Loss):
    ...
    Examples:
        ...
        >>> log_input = smith.randn(5, 2, requires_grad=True)
        >>> target = smith.randn(5, 2)
...
class GaussianNLLLoss(_Loss):
    ...
    Examples:
        ...
        >>> input = smith.randn(5, 2, requires_grad=True)
        >>> target = smith.randn(5, 2)
        >>> var = smith.ones(5, 2, requires_grad=True)  # heteroscedastic
        ...
        >>> input = smith.randn(5, 2, requires_grad=True)
        >>> target = smith.randn(5, 2)
        >>> var = smith.ones(5, 1, requires_grad=True)  # homoscedastic
        >>> output = loss(input, target, var)
        >>> output.backward()
...
class KLDivLoss(_Loss):
    ...
    .. note::
        As all the other losses in Blacksmith, this function expects the first argument,
    ...
    Examples:
        ...
        >>> input = F.log_softmax(smith.randn(3, 5, requires_grad=True), dim=1)
        >>> # Sample a batch of distributions. Usually this would come from the dataset
        >>> target = F.softmax(smith.rand(3, 5), dim=1)
        >>> output = kl_loss(input, target)
        >>>
        >>> kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)
        >>> log_target = F.log_softmax(smith.rand(3, 5), dim=1)
...
class BCELoss(_WeightedLoss):
    ...
    mathematically undefined in the above loss equation. Blacksmith chooses to set
...
class BCEWithLogitsLoss(_Loss):
    ...
    Examples:
        ...
        >>> target = smith.ones([10, 64], dtype=smith.float32)  # 64 classes, batch size = 10
        >>> output = smith.full([10, 64], 1.5)  # A prediction (logit)
        >>> pos_weight = smith.ones([64])  # All weights are equal to 1
        >>> criterion = smith.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
...
class MultiLabelMarginLoss(_Loss):
    ...
    Examples:
        ...
        >>> x = smith.FloatTensor([[0.1, 0.2, 0.4, 0.8]])
        >>> # for target y, only consider labels 3 and 0, not after label -1
        >>> y = smith.LongTensor([[3, 0, -1, 1]])
...
class SmoothL1Loss(_Loss):
    ...
    It is less sensitive to outliers than :class:`smith.nn.MSELoss` and in some cases
...
class CrossEntropyLoss(_WeightedLoss):
    ...
    - Class indices in the range :math:`[0, C)` where :math:`C` is the number of classes; if
      ...
      Note that this case is equivalent to applying :class:`~smith.nn.LogSoftmax`
      on an input, followed by :class:`~smith.nn.NLLLoss`.
    ...
    Shape:
        ...
        - Target: If containing class indices, shape :math:`()`, :math:`(N)` or :math:`(N, d_1, d_2, ..., d_K)` with
          ...
          data type is required to be float when using class probabilities. Note that Blacksmith does not strictly enforce
    ...
    .. note::
        ...
        Blacksmith does not validate whether the values provided in ``target`` lie in the range ``[0,1]``
...
class MultiLabelSoftMarginLoss(_WeightedLoss):
    ...
    Examples:
        ...
        >>> input = smith.randn(3, 5, requires_grad=True)
        >>> target = smith.empty(3, 5).random_(2)
        >>> output = loss(input, target)
        >>> output.backward()
    """
...
class CosineEmbeddingLoss(_Loss):
    ...
    Examples:
        ...
        >>> input1 = smith.randn(3, 5, requires_grad=True)
        >>> input2 = smith.randn(3, 5, requires_grad=True)
        >>> target = smith.ones(3)
...
class MarginRankingLoss(_Loss):
    ...
    Examples:
        ...
        >>> input1 = smith.randn(3, requires_grad=True)
        >>> input2 = smith.randn(3, requires_grad=True)
        >>> target = smith.randn(3).sign()
        >>> output = loss(input1, input2, target)
        >>> output.backward()
    """
...
class MultiMarginLoss(_WeightedLoss):
    ...
    Examples:
        ...
        >>> x = smith.tensor([[0.1, 0.2, 0.4, 0.8]])
        >>> y = smith.tensor([3])
...
class TripletMarginLoss(_Loss):
    ...
    See also :class:`~smith.nn.TripletMarginWithDistanceLoss`, which computes the
    ...
    >>> anchor = smith.randn(100, 128, requires_grad=True)
    >>> positive = smith.randn(100, 128, requires_grad=True)
    >>> negative = smith.randn(100, 128, requires_grad=True)
...
class TripletMarginWithDistanceLoss(_Loss):
    ...
    See also :class:`~smith.nn.TripletMarginLoss`, which computes the triplet
    ...
    >>> anchor_ids = smith.randint(0, 1000, (1,))
    >>> positive_ids = smith.randint(0, 1000, (1,))
    >>> negative_ids = smith.randint(0, 1000, (1,))
    ...
    >>>     return smith.max(smith.abs(x1 - x2), dim=1).values
    >>>
...
class CTCLoss(_Loss):
    ...
    Shape:
        - Log_probs: Tensor of size :math:`(T, N, C)` or :math:`(T, C)`,
          ...
          :func:`smith.nn.functional.log_softmax`).
    ...
    Examples:
        ...
        >>> input = smith.randn(T, N, C).log_softmax(2).detach().requires_grad_()
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target = smith.randint(low=1, high=C, size=(N, S), dtype=smith.long)
        >>>
        >>> input_lengths = smith.full(size=(N,), fill_value=T, dtype=smith.long)
        >>> target_lengths = smith.randint(
        ...     low=S_min,
        ...     high=S,
        ...     size=(N,),
        ...     dtype=smith.long,
        ...
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target_lengths = smith.randint(low=1, high=T, size=(N,), dtype=smith.long)
        >>> target = smith.randint(
        ...     low=1,
        ...     high=C,
        ...
        >>> input = smith.randn(T, C).log_softmax(1).detach().requires_grad_()
        >>> input_lengths = smith.tensor(T, dtype=smith.long)
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target_lengths = smith.randint(low=1, high=T, size=(), dtype=smith.long)
        >>> target = smith.randint(
        ...     low=1,
        ...     high=C,
        ...     size=(target_lengths,),
    ...
    Note:
        ...
        dtype :attr:`smith.int32`, and the :attr:`log_probs` itself must be of
        dtype :attr:`smith.float32`.

        The regular implementation uses the (more common in Blacksmith) `smith.long` dtype.
    ...
    Note:
        ...
        a performance cost) by setting ``smith.backends.cudnn.deterministic =
        True``.
