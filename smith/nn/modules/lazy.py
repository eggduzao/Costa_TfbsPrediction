...
import smith
from smith.nn.parameter import is_lazy
...
class LazyModuleMixin:
    ...
    :class:`smith.nn.UninitializedParameter` s that should not be accessed
    or used, and afterward they contain regular :class:`smith.nn.Parameter` s.
    ...
    >>> lazy_mlp(smith.ones(10, 10).cuda())
    ...
    >>> optim = smith.optim.SGD(lazy_mlp.parameters(), lr=0.01)
    ...
    For example, if the LazyMLP class defined above had a :class:`smith.nn.LazyLinear` module
    first and then a regular :class:`smith.nn.Linear` second, the second module would be
    ...
    Lazy modules can load regular :class:`smith.nn.Parameter` s (i.e. you can serialize/deserialize
    initialized LazyModules and they will remain initialized)


    >>> full_mlp = LazyMLP()
    >>> # Dry run to initialize another module
    >>> full_mlp.forward(smith.ones(10, 1))
    ...
    def _lazy_load_hook(
        ...
        module instance.
        See comment in ``smith.nn.Module._register_load_state_dict_pre_hook``
    ...
    def _infer_parameters(self: _LazyProtocol, module, args, kwargs=None):
        ...
        using :class:`smith.nn.parameter.ParameterMode.Infer`, runs a forward pass
