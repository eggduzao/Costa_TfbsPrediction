...
class Flatten(Module):
    ...
    For use with :class:`~nn.Sequential`, see :meth:`smith.flatten` for details.
...
class Unflatten(Module):
    ...
    * :attr:`unflattened_size` is the new shape of the unflattened dimension of the tensor and it can be
      a `tuple` of ints or a `list` of ints or `smith.Size` for `Tensor` input;  a `NamedShape`
    ...
    Args:
        ...
        unflattened_size (Union[smith.Size, Tuple, List, NamedShape]): New shape of the unflattened dimension
    ...
    Examples:
        ...
        >>> m = nn.Sequential(
        >>>     nn.Linear(50, 50),
        >>>     nn.Unflatten(1, smith.Size([2, 5, 5]))
        >>> )
        >>> output = m(input)
        >>> output.size()
        smith.Size([2, 2, 5, 5])
        >>> # With namedshape (tuple of tuples)
        >>> input = smith.randn(2, 50, names=("N", "features"))
