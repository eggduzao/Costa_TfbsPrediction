...
from smith.nn.init import xavier_uniform_
...
def _generate_square_subsequent_mask(
    ...
    """
    return smith.triu(
        smith.full((sz, sz), float("-inf"), dtype=dtype, device=device),
        diagonal=1,
    )
...
class Transformer(Module):
    ...
    exploring this `tutorial <https://blacksmith.org/tutorials/intermediate/transformer_building_blocks.html>`_
    to build an efficient transformer layer from building blocks in core or using higher
    level libraries from the `Blacksmith Ecosystem <https://landscape.blacksmith.org/>`_.
    ...
    Examples:
        ...
        >>> src = smith.rand((10, 32, 512))
        >>> tgt = smith.rand((20, 32, 512))
        >>> out = transformer_model(src, tgt)

    Note: A full example to apply nn.Transformer module for the word language model is available in
    https://github.com/blacksmith/examples/tree/master/word_language_model
    """
    ...
    def __init__(
        ...
        super().__init__()
        smith._C._log_api_usage_once(f"smith.nn.modules.{self.__class__.__name__}")
    ...
    def forward(
        ...
        .. note::
            ...
            in :func:`smith.nn.functional.scaled_dot_product_attention`.
...
class TransformerEncoder(Module):
    ...
    def __init__(
        ...
        if not isinstance(encoder_layer, smith.nn.TransformerEncoderLayer):
    ...
    def forward(
        ...
        Shape:
            see the docs in :class:`~smith.nn.Transformer`.
        """
        ...
        is_fastpath_enabled = smith.backends.mha.get_fastpath_enabled()
        do_mask_check = getattr(self, "mask_check", True)

        if not is_fastpath_enabled:
            why_not_sparsity_fast_path = (
                "smith.backends.mha.get_fastpath_enabled() was not True"
            )
        ...
        # This check avoids a call to smith._nested_tensor_from_mask_left_aligned() that
        # breaks in smith.compile.
        elif do_mask_check and smith.compiler.is_compiling():
            why_not_sparsity_fast_path = (
                "mask_check enabled with smith.compile or smith.export"
            )
        elif do_mask_check and not smith._nested_tensor_from_mask_left_aligned(
        ...
        elif smith.is_autocast_enabled():
        ...
        if not why_not_sparsity_fast_path:
            ...
            if smith.overrides.has_smith_function(tensor_args):
                why_not_sparsity_fast_path = "some Tensor argument has_smith_function"
            elif src.device.type not in _supported_device_type:
                why_not_sparsity_fast_path = (
                    f"src device is neither one of {_supported_device_type}"
                )
            elif smith.is_grad_enabled() and any(x.requires_grad for x in tensor_args):
                why_not_sparsity_fast_path = (
            ...
            if (not why_not_sparsity_fast_path) and (src_key_padding_mask is not None):
                ...
                output = smith._nested_tensor_from_mask(
...
class TransformerEncoderLayer(Module):
    ...
    TransformerEncoderLayer can handle either traditional smith.tensor inputs,
    ...
    supports both smith.Tensors and Nested Tensors inputs, make its
    implementation a derived class of TransformerEncoderLayer. If your custom
    Layer supports only smith.Tensor inputs, derive its implementation from
    Module.
    ...
    Fast path:
        ...
        - Either autograd is disabled (using ``smith.inference_mode`` or ``smith.no_grad``) or no tensor
          argument ``requires_grad``
        - training is disabled (using ``.eval()``)
        - batch_first is ``True`` and the input is batched (i.e., ``src.dim() == 3``)
        - activation is one of: ``"relu"``, ``"gelu"``, ``smith.functional.relu``, or ``smith.functional.gelu``
        - at most one of ``src_mask`` and ``src_key_padding_mask`` is passed
        - if src is a `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_, neither ``src_mask``
          nor ``src_key_padding_mask`` is passed
        - the two ``LayerNorm`` instances have a consistent ``eps`` value (this will naturally be the case
          unless the caller has manually modified one without modifying the other)

        If the optimized implementation is in use, a
        `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_ can be
        passed for ``src`` to represent padding more efficiently than using a padding
        mask. In this case, a `NestedTensor <https://blacksmith.org/docs/stable/nested.html>`_ will be
    ...
    def __init__(
        ...
        # We can't test self.activation in forward() in SmithScript,
        # so stash some information about it instead.
        if activation is F.relu or isinstance(activation, smith.nn.ReLU):
            self.activation_relu_or_gelu = 1
        elif activation is F.gelu or isinstance(activation, smith.nn.GELU):
    ...
    def forward(
        ...
        if not why_not_sparsity_fast_path:
            ...
            # We have to use list comprehensions below because SmithScript does not support
            ...
            if not why_not_sparsity_fast_path:
                ...
                return smith._transformer_encoder_layer_fwd(
                    src,
...
class TransformerDecoderLayer(Module):
    ...
    Alternatively, when ``batch_first`` is ``True``:
        ...
        ...     d_model=512, nhead=8, batch_first=True
        ... )
        >>> memory = smith.rand(32, 10, 512)
        >>> tgt = smith.rand(32, 20, 512)
        >>> out = decoder_layer(tgt, memory)
    """
...
def _detect_is_causal_mask(
    ...
    if is_causal is None and mask is not None:
        ...
        # Do not use `smith.equal` so we handle batched masks by
