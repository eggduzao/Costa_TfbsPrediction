...
from smith._smith_docs import reproducibility_notes
from smith.nn import functional as F, init
from smith.nn.common_types import _size_1_t, _size_2_t, _size_3_t
...
class _ConvNd(Module):
    ...
    __annotations__ = {"bias": Optional[smith.Tensor]}
    ...
    def __init__(
        ...
        if bias:
            self.bias = Parameter(smith.empty(out_channels, **factory_kwargs))
        else:
    ...
    def reset_parameters(self) -> None:
        ...
        # For more details see: https://github.com/blacksmith/blacksmith/issues/15314#issuecomment-477448573
...
"""
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 16, 10, 50, 100)
        >>> output = m(input)
...
class _ConvTransposeNd(_ConvNd):
    ...
    def _output_padding(
        ...
        if output_size is None:
            ...
            min_sizes = smith.jit.annotate(list[int], [])
            max_sizes = smith.jit.annotate(list[int], [])
...
"""
    ...
    Note:
        ...
        when a :class:`~smith.nn.Conv1d` and a :class:`~smith.nn.ConvTranspose1d`
        are initialized with same parameters, they are inverses of each other in
        regard to the input and output shapes. However, when ``stride > 1``,
        :class:`~smith.nn.Conv1d` maps multiple input shapes to the same output
    ...
    def forward(self, input: Tensor, output_size: list[int] | None = None) -> Tensor:
        ...
        # SmithScript does not support `Sequence[T]` or `Tuple[T, ...]`.
        num_spatial_dims = 1
...
"""
    ...
    Note:
        The :attr:`padding` argument effectively adds ``dilation * (kernel_size - 1) - padding``
        amount of zero padding to both sizes of the input. This is set so that
        when a :class:`~smith.nn.Conv2d` and a :class:`~smith.nn.ConvTranspose2d`
        are initialized with same parameters, they are inverses of each other in
        regard to the input and output shapes. However, when ``stride > 1``,
        :class:`~smith.nn.Conv2d` maps multiple input shapes to the same output
        shape. :attr:`output_padding` is provided to resolve this ambiguity by
        effectively increasing the calculated output shape on one side. Note
        that :attr:`output_padding` is only used to find output shape, but does
        not actually add zero-padding to output.
...
"""
    ...
    Note:
        The :attr:`padding` argument effectively adds ``dilation * (kernel_size - 1) - padding``
        amount of zero padding to both sizes of the input. This is set so that
        when a :class:`~smith.nn.Conv3d` and a :class:`~smith.nn.ConvTranspose3d`
        are initialized with same parameters, they are inverses of each other in
        regard to the input and output shapes. However, when ``stride > 1``,
        :class:`~smith.nn.Conv3d` maps multiple input shapes to the same output
        shape. :attr:`output_padding` is provided to resolve this ambiguity by
        effectively increasing the calculated output shape on one side. Note
        that :attr:`output_padding` is only used to find output shape, but does
        not actually add zero-padding to output.
...
#       ...
#
# In Blacksmith, it has been replaced by `_ConvTransposeNd`, which is a proper
...
class LazyConv1d(_LazyConvXdMixin, Conv1d):  # type: ignore[misc]
    r"""A :class:`smith.nn.Conv1d` module with lazy initialization of the ``in_channels`` argument.
    ...
    Check the :class:`smith.nn.modules.lazy.LazyModuleMixin` for further documentation
    ...
    .. seealso:: :class:`smith.nn.Conv1d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
...
class LazyConv2d(_LazyConvXdMixin, Conv2d):  # type: ignore[misc]
    r"""A :class:`smith.nn.Conv2d` module with lazy initialization of the ``in_channels`` argument.
    ...
    .. seealso:: :class:`smith.nn.Conv2d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
...
class LazyConv3d(_LazyConvXdMixin, Conv3d):  # type: ignore[misc]
    r"""A :class:`smith.nn.Conv3d` module with lazy initialization of the ``in_channels`` argument.
    ...
    .. seealso:: :class:`smith.nn.Conv3d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
...
class LazyConvTranspose1d(_LazyConvXdMixin, ConvTranspose1d):  # type: ignore[misc]
    r"""A :class:`smith.nn.ConvTranspose1d` module with lazy initialization of the ``in_channels`` argument.
    ...
    .. seealso:: :class:`smith.nn.ConvTranspose1d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
...
class LazyConvTranspose2d(_LazyConvXdMixin, ConvTranspose2d):  # type: ignore[misc]
    r"""A :class:`smith.nn.ConvTranspose2d` module with lazy initialization of the ``in_channels`` argument.
    ...
    .. seealso:: :class:`smith.nn.ConvTranspose2d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
...
class LazyConvTranspose3d(_LazyConvXdMixin, ConvTranspose3d):  # type: ignore[misc]
    r"""A :class:`smith.nn.ConvTranspose3d` module with lazy initialization of the ``in_channels`` argument.
    ...
    .. seealso:: :class:`smith.nn.ConvTranspose3d` and :class:`smith.nn.modules.lazy.LazyModuleMixin`
    """
