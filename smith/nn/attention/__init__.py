...
"""This module contains functions and classes that alter the behavior of smith.nn.functional.scaled_dot_product_attention"""
...
from smith._C import _SDPBackend as SDPBackend
...
# smith.nn.attention.WARN_FOR_UNFUSED_KERNELS = True
...
def _cur_sdpa_kernel_backends(with_priority: bool = False):
    ...
    for name, val in _backend_names.items():
        if getattr(smith._C, f"_get_{name}_sdp_enabled")():
            backends.append(getattr(SDPBackend, val))
    if with_priority:
        curr_priority = smith._C._get_sdp_priority_order()
        backends = sorted(
...
def sdpa_kernel(backends: list[SDPBackend] | SDPBackend, set_priority: bool = False):
    ...
    .. code-block:: python
        ...
        from smith.nn.attention import SDPBackend, sdpa_kernel
