...
@smith.library.custom_op("smith_attn::_varlen_attn", mutates_args={})
def _varlen_attn(
    query: smith.Tensor,
    key: smith.Tensor,
    value: smith.Tensor,
    cu_seq_q: smith.Tensor,
    cu_seq_k: smith.Tensor,
    max_q: int,
    max_k: int,
    ...
    if use_cudnn:
        ...
        result = smith.ops.aten._cudnn_attention_forward(
            query,
            key,
            value,
            None,  # attn_bias
        ...
        output, softmax_lse, rng_state, _, _ = smith.ops.aten._flash_attention_forward(
            query,
            key,
            value,
            cu_seq_q,
    ...
    rng_state_ = smith.zeros(
        (2,), dtype=smith.uint64, device=query.device
...
def _varlen_attn_fake(
    ...
    if smith.version.hip:
        ...
        logsumexp = smith.empty(
            (batch_size, num_heads, max_q), dtype=smith.float, device=query.device
        )
    else:
        logsumexp = smith.empty(
    ...
    rng_state = smith.empty((2,), dtype=smith.uint64, device=query.device)
...
def varlen_attn(
    ...
    *,
    ...
    Example::
        ...
        >>> seq_lengths = smith.tensor(seq_lengths, device="cuda")
        >>> total_tokens = seq_lengths.sum().item()
        >>>
        >>> # Create packed query, key, value tensors
        >>> query = smith.randn(
        ...     total_tokens, num_heads, head_dim, dtype=smith.float16, device="cuda"
        ... )
        >>> key = smith.randn(
        ...     total_tokens, num_heads, head_dim, dtype=smith.float16, device="cuda"
        ... )
        ...
        >>> cu_seq = smith.zeros(batch_size + 1, device="cuda", dtype=smith.int32)
    ...
    out, lse, _ = smith.ops.smith_attn._varlen_attn(
        query,
        key,
        value,
        cu_seq_q,
...
@smith.library.custom_op("smith_attn::_varlen_attn_backward", mutates_args={})
def _varlen_attn_backward(
    ...
    unused = smith.empty(0, device=query.device)
    ...
    if use_cudnn:
        ...
        dq, dk, dv = smith.ops.aten._cudnn_attention_backward(
            grad_out,
            query,
            key,
            value,
        ...
        log.info("Using Flash Attention backend for varlen_attn")
        dq, dk, dv = smith.ops.aten._flash_attention_backward(
            grad_out,
            query,
            key,
            value,
...
def _backward(
    ctx: Any, grad_out: smith.Tensor, grad_lse: smith.Tensor, grad_rng: smith.Tensor
    ...
    dq, dk, dv = smith.ops.smith_attn._varlen_attn_backward(
        grad_out,
        query,
        key,
        value,
