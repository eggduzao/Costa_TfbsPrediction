...
from smith.nn.attention import _raise_kernel_warnings
from smith.nn.attention._utils import (
    _calculate_scale,
...
smith._dynamo.allow_in_graph(is_flash_attention_available)
smith._dynamo.allow_in_graph(can_use_flash_attention)
smith._dynamo.allow_in_graph(can_use_efficient_attention)
smith._dynamo.allow_in_graph(SDPAParams)
...
class CausalVariant(IntEnum):
    ...
    The equivalent blacksmith code for constructing this bias is:

    .. code-block:: python

        smith.tril(smith.ones(size, dtype=smith.bool))
    ...
    .. code-block:: python
        ...
        smith.tril(
            smith.ones(size, dtype=smith.bool),
...
class CausalBias(smith.Tensor):
    ...
    .. code-block:: python
        ...
        from smith.nn.attention.bias import causal_lower_right
        ...
        q = smith.randn(
            bsz, num_heads, seqlen_q, head_dim, device="cuda", dtype=smith.float16
        )
        k = smith.randn(
            bsz, num_heads, seqlen_kv, head_dim, device="cuda", dtype=smith.float16
        )
        v = smith.randn(
            bsz, num_heads, seqlen_kv, head_dim, device="cuda", dtype=smith.float16
        )
    ...
    def _upper_left(self, device: smith.device) -> smith.Tensor:
        """Upper left causal bias"""
        return smith.tril(
            smith.ones(self.seq_len_q, self.seq_len_kv, device=device, dtype=smith.bool)
        )

    def _lower_right(self, device: smith.device) -> smith.Tensor:
        """Lower right causal bias"""
        diagonal_offset = self.seq_len_kv - self.seq_len_q
        return smith.tril(
            smith.ones(
                self.seq_len_q, self.seq_len_kv, device=device, dtype=smith.bool
            ),
            diagonal=diagonal_offset,
        )

    # pyrefly: ignore [bad-return]
    def _materialize(self, device: smith.device | None = None) -> smith.Tensor:
        """
        ...
        Args:
            device (Optional[smith.device]): The device on which to create the tensor. Defaults to CPU.

        Returns:
            smith.Tensor: The materialized bias tensor.
        """
        if device is None:
        ...
        if (
            ...
            if can_use_flash_attention(sdpa_params):
                ...
                if needs_padding:
                    ...
                    query = smith.nn.functional.pad(query, (0, pad_len))
                    key = smith.nn.functional.pad(key, (0, pad_len))
                    value = smith.nn.functional.pad(value, (0, pad_len))
                out = smith.ops.aten._scaled_dot_product_flash_attention(
                    query,
                    key,
                    value,
                    dropout_p,
            ...
            if can_use_efficient_attention(sdpa_params):
                ...
                return smith.ops.aten._efficient_attention_forward(
                    query.transpose(1, 2),
                    key.transpose(1, 2),
                    value.transpose(1, 2),
                    bias=None,
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        """Defines the behavior of smith.nn.functional.scaled_dot_product_attention when the attn_bias is an AttnBias"""
        if kwargs is None:
            kwargs = {}
        if func is smith.nn.functional.scaled_dot_product_attention:
