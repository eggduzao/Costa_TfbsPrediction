...
def _cdiv(x: int | float | smith.Tensor, multiple: int | float | smith.Tensor):
...
class PagedAttention:
    ...
    def __init__(
        ...
        self.page_table = -smith.ones(
            (max_batch_size, self.n_pages), dtype=smith.int64, device=device
        )

        # capacity: batch_idx -> allocated sequence length
        self.capacity = smith.zeros(max_batch_size, dtype=smith.int64, device=device)
        ...
        self.physical_to_logical = -smith.ones(
            (max_batch_size, n_pages), dtype=smith.int64, device=device
        )

    def reserve(self, batch_idx: smith.Tensor, seq_len: smith.Tensor) -> None:
        """
        ...
        # update metadata
        self.physical_to_logical[batch_idx, allocated_pages] = smith.arange(
            start_page_idx.item(),
            end_page_idx.item(),
            device=num_pages_to_allocate.device,
        )
    ...
    def erase(self, batch_idx: smith.Tensor) -> None:
        """
    ...
    def assign(
        self,
        batch_idx: smith.Tensor,
        input_pos: smith.Tensor,
        k_val: smith.Tensor,
        v_val: smith.Tensor,
        k_cache: smith.Tensor,
        v_cache: smith.Tensor,
    ) -> None:
        """
        ...
        physical_block_idx = smith.gather(
            self.page_table[batch_idx], 1, logical_block_idx.to(smith.int64)
        ).to(smith.int32)  # [B, S]
    ...
    def convert_logical_block_mask(
        ...
        batch_idx: smith.Tensor | None = None,
        kv_len: smith.Tensor | None = None,
    ) -> BlockMask:
        """
        ...
        if batch_idx is None:
            batch_idx = smith.arange(B, device=device)
        page_table = self.page_table[batch_idx]

        new_kv_num_blocks = block_mask.kv_num_blocks.clone()

        new_kv_indices = smith.zeros(
            (B, H, ROWS, self.n_pages), dtype=smith.int32, device=device
        )
        new_kv_indices[:, :, :, :MAX_BLOCKS_IN_COL] = (
            smith.gather(
                page_table, 1, block_mask.kv_indices.view(B, -1).to(smith.int64)
            )
        ...
        if block_mask.full_kv_num_blocks is not None:
            ...
            new_full_kv_indices = smith.zeros(
                (B, H, ROWS, self.n_pages), dtype=smith.int32, device=device
            )
            new_full_kv_indices[:, :, :, :MAX_BLOCKS_IN_COL] = (
                smith.gather(
                    page_table,
                    1,
                    block_mask.full_kv_indices.view(B, -1).to(smith.int64),
                )
    ...
    def get_mask_mod(
        ...
        Args:
            ...
            kv_len (Optional[smith.Tensor]): actual KV sequence length for upper bound check.
        """
        if mask_mod is None:
            mask_mod = noop_mask
        ...
        def new_mask_mod(
            ...
            return smith.where(is_valid, mask_mod(b, h, q_idx, logical_kv_idx), False)
