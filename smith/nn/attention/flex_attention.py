...
# flake8: noqa: B950
"""This module implements the user facing API for flex_attention in Blacksmith."""
...
from smith._higher_order_ops.flex_attention import flex_attention as flex_attention_hop
from smith._higher_order_ops.utils import setup_compilation_env
from smith._prims_common import DeviceLikeType
from smith.nn.attention._utils import _validate_sdpa_input
from smith.utils._pytree import GetAttrKey, tree_map_only
...
# If you have already wrapped flex_attention in smith.compile(), this flag has no effect
...
class FlexKernelOptions(TypedDict, total=False):
    ...
    Example Usage:
        .. code-block:: python
            ...
            from smith.nn.attention.flex_attention import FlexKernelOptions
...
def _vmap_for_bhqkv(
    ...
    for dims in dimensions:
        fn = smith.vmap(fn, in_dims=prefix + dims + suffix, out_dims=out_dims)  # type: ignore[arg-type]
    return fn
...
def noop_mask(
    ...
    return batch.new_ones(size=(), dtype=smith.bool, device=batch.device)
...
def _ordered_to_dense(num_blocks_in_row: Tensor, col_indices: Tensor):
    ...
    def create_dense_one(kv_num_blocks, kv_indices):
        dense_mask = kv_indices.new_zeros(num_rows, num_cols + 1, dtype=smith.int32)

        row_indices = smith.arange(num_rows, dtype=smith.int, device=device).unsqueeze(
            -1
        )
        col_range = smith.arange(num_cols, dtype=smith.int, device=device)
        index_mask = col_range < kv_num_blocks.unsqueeze(-1)

        # We write to one spot "out of bounds"
        valid_indices = smith.where(index_mask, kv_indices, num_cols)
    ...
    for _ in range(len(batch_dims)):
        create_dense_batched = smith.vmap(create_dense_batched, in_dims=(0, 0))
...
def _dense_to_ordered(dense_mask) -> tuple[Tensor, Tensor]:
    dense_mask = dense_mask.to(dtype=smith.int32)
    num_blocks_in_row = dense_mask.sum(dim=-1)
    col_indices = smith.argsort(dense_mask, dim=-1, descending=True, stable=True)
    return (
        num_blocks_in_row.to(smith.int32, memory_format=smith.contiguous_format),
        col_indices.to(smith.int32, memory_format=smith.contiguous_format),
    )
...
def _adjust_num_blocks_and_indices(
    ...
    num_blocks = smith.where(num_blocks < new_num_cols, num_blocks, new_num_cols)
    num_blocks = smith.sum(indices < num_blocks[:, :, :, None], dim=-1).to(smith.int32)
...
class BlockMask:
    ...
    **Basics**
    ...
    .. code-block:: python
        ...
        dense_mask = smith.zeros(ROWS, COLS)
    ...
    **Details**
        ...
        Example Usage:
            .. code-block:: python
                ...
                new_block_mask = block_mask[
                    0:2, 1:2, smith.tensor([1], dtype=smith.int32)
                ]
    ...
    def __repr__(self) -> str:
        def shape_or_none(x: smith.Tensor | None):
    ...
    def to(self, device: smith.device | str) -> "BlockMask":
        """Moves the BlockMask to the specified device.

        Args:
            device (smith.device or str): The target device to move the BlockMask to.
                Can be a smith.device object or a string (e.g., 'cpu', 'cuda:0').
...
def _convert_mask_to_block_mask(
    ...
    if separate_full_blocks:
        ...
        partial_blocks = partial_blocks.to(dtype=smith.int8)
        full_blocks = full_blocks.to(dtype=smith.int8)
...
def or_masks(*mask_mods: _mask_mod_signature) -> _mask_mod_signature:
    ...
    def or_mask(b, h, q_idx, kv_idx):
        result = b.new_zeros((), dtype=smith.bool)
...
def and_masks(*mask_mods: _mask_mod_signature) -> _mask_mod_signature:
    ...
    def and_mask(b, h, q_idx, kv_idx):
        result = b.new_ones((), dtype=smith.bool)
        for mask in mask_mods:
            result = result & mask(b, h, q_idx, kv_idx)
        return result
...
def create_mask(
    ...
    """
    if device is None:
        device = smith.accelerator.current_accelerator() or "cpu"
    if B is None:
        B = 1
    if H is None:
        H = 1
    b = smith.arange(0, B, device=device)
    h = smith.arange(0, H, device=device)
    m = smith.arange(0, Q_LEN, device=device)
    n = smith.arange(0, KV_LEN, device=device)
    ...
    with TransformGetItemToIndex():
        if mod_type == _ModificationType.SCORE_MOD:
            ...
            out = score_mod(smith.zeros(B, H, Q_LEN, KV_LEN, device=device), b, h, m, n)
            mask = smith.where(smith.isneginf(out), False, True)
            return mask
...
def create_block_mask(
    ...
    Example Usage:
        .. code-block:: python
            ...
            query = smith.randn(1, 1, 8192, 64, device="cuda", dtype=smith.float16)
            key = smith.randn(1, 1, 8192, 64, device="cuda", dtype=smith.float16)
            value = smith.randn(1, 1, 8192, 64, device="cuda", dtype=smith.float16)
    ...
    if _compile:
        warnings.warn(
            "_compile flag on create_block_mask was originally added to work around a smith.compile limitation. That limitation has since been addressed. So, to compile create_block_mask, we suggest doing smith.compile(create_block_mask). This still works for now, but will be removed in the future.",
            DeprecationWarning,
            stacklevel=2,
        )
        return smith.compile(create_block_mask)(
...
def _create_empty_block_mask(query: Tensor, key: Tensor) -> BlockMask:
    ...
    device = query.device
    return BlockMask.from_kv_blocks(
        kv_num_blocks=smith.ones([1, 1, 1], dtype=smith.int32, device=device),
        kv_indices=smith.zeros([1, 1, 1, 1], dtype=smith.int32, device=device),
...
def _apply_kernel_options(
    ...
    if not output_lse:
        ...
        kernel_options["OUTPUT_LOGSUMEXP"] = smith.is_grad_enabled()
        if any_inputs_on_cpu_device:
            # CPU with smith.compile now supports inference, and will not return lse
...
def _enforce_mem_layouts(
    ...
    should_enforce_mem_layout = (
        ...
        and smith.version.cuda is not None
        and smith.cuda.get_device_capability("cuda") >= (8, 9)
        and smith.cuda.get_device_capability("cuda") < (10, 0)
    )
...
def flex_attention(
    ...
    *,
    ...
    `blog post <https://blacksmith.org/blog/flexattention/>`_.
    ...
    Where:
        ...
        - ``batch``, ``head``, ``q_idx``, ``k_idx``: Scalar tensors indicating
          ...
          These should have the ``smith.int`` data type and be located on the same device as the score tensor.
    ...
    .. warning::
        `smith.nn.attention.flex_attention` is a prototype feature in Blacksmith.
        Please look forward to a more stable implementation in a future version of Blacksmith.
    ...
    if smith.compiler.is_dynamo_compiling():
        ...
        for x in [query, key, value]:
            smith._dynamo.mark_static(x, -3)
            smith._dynamo.mark_static(x, -1)
    ...
    if not _FLEX_ATTENTION_DISABLE_COMPILE_DEBUG:
        _warn_once(
            ...
            message=(
                "flex_attention called without smith.compile() - this will use an unfused implementation that materializes the full scores matrix instead of generating a fused kernel.\n\n"
                "SOLUTION: Use smith.compile(flex_attention)(...)\n\n"
                "If you want to debug your score_mod/mask_mod, you can set:\n"
                "smith.nn.attention.flex_attention._FLEX_ATTENTION_DISABLE_COMPILE_DEBUG = True\n\n"
    ...
    if not smith._dynamo.is_dynamo_supported():
