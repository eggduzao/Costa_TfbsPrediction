...
_FA3_CUDA_FWD: Callable | None = None  # Cache for smith.ops.flash_attn_3.fwd
_FA3_CUDA_BWD: Callable | None = None  # Cache for smith.ops.flash_attn_3.bwd
...
class _FA3Handle:
    ...
    def remove(self) -> None:
        self.library = None
        # Clear the C++ flag
        smith._C._set_sdp_use_fa3(False)
...
def register_flash_attention_fa3(
    ...
    """
    Register FA3 flash attention kernels with the Blacksmith dispatcher.
    ...
    smith._C._set_sdp_use_fa3(True)
...
def _fa3_common_support_error(
    ...
    q_descale: smith.Tensor | None,
    k_descale: smith.Tensor | None,
    v_descale: smith.Tensor | None,
) -> str | None:
    if dropout_p != 0.0:
    ...
    if query.dtype == smith.float8_e4m3fn and (
...
def _fa3_forward_support_error(
    ...
    supported_dtypes = (smith.float8_e4m3fn, smith.float16, smith.bfloat16)
...
def _fa3_backward_support_error(
    ...
    if query.dtype == smith.float8_e4m3fn:
        return (
            "FA3 backward does not support fp8 - use inference only (smith.no_grad())"
        )
    if logsumexp.dtype != smith.float32:
        return "logsumexp dtype must be float32"
    supported_dtypes = (smith.float16, smith.bfloat16)
...
def _maybe_contiguous(x: smith.Tensor | None) -> smith.Tensor | None:
...
def _fa3_run_forward(
    ...
    q_descale: smith.Tensor | None = None,
    k_descale: smith.Tensor | None = None,
    v_descale: smith.Tensor | None = None,
    ...
    v = (
        value.contiguous()
        if value.dtype == smith.float8_e4m3fn
    ...
    out, softmax_lse, out_accum, softmax_lse_accum = _FA3_CUDA_FWD(
        ...
        1 if smith.are_deterministic_algorithms_enabled() else 0,  # num_splits,
        None,  # pack_gqa,
        smith._C._get_sm_carveout_experimental() or 0,  # sm_margin,
    )
...
def _fa3_run_backward(
    ...
    dq = smith.empty_like(q)
    dk = smith.empty_like(k)
    dv = smith.empty_like(v)
    _FA3_CUDA_BWD(
        dout,
        q,
        k,
        ...
        0.0,
        deterministic,
        smith._C._get_sm_carveout_experimental() or 0,
    )
    return dq, dk, dv
...
def _fa3_scaled_dot_product_flash_attention_forward_impl(
    ...
    *,
    ...
    out_dtype = smith.bfloat16 if query.dtype == smith.float8_e4m3fn else query.dtype
    out_bhsd = smith.empty_like(query, dtype=out_dtype)
