...
from smith._utils import _get_device_index
from smith.autograd import Function, Variable
from smith.distributed.algorithms.join import Join, Joinable, JoinHook
from smith.nn.modules import Module
from smith.nn.parallel.scatter_gather import gather, scatter_kwargs
...
class _MixedPrecision:
    ...
    Attributes:
        param_dtype (smith.dtype): This specifies the dtype for model
            ...
            precision.
        reduce_dtype (smith.dtype): This specifies the dtype for gradient
            reduction, which is permitted to differ from ``param_dtype``.
        buffer_dtype (smith.dtype): This specifies the dtype for buffers.
    ...
    .. note:: Each low precision dtype must be specified explicitly. For
        example, ``_MixedPrecision(reduce_dtype=smith.float16)`` only specifies
    ...
    .. note:: If a ``reduce_dtype`` is not specified, then gradient reduction
        ...
        otherwise. For example, ``_MixedPrecision(param_dtype=smith.float16)``
        would result in communication occurring in fp16.
    """

    param_dtype: smith.dtype | None = None
    reduce_dtype: smith.dtype | None = None
    buffer_dtype: smith.dtype | None = None
...
def _setup_mixed_precision_params(mixed_precision_config, root_module):
    ...
    for param in root_module.parameters():
        ...
        if not hasattr(param, "_mp_param"):
            param._mp_param = smith.zeros_like(
                param,
                device=param.device,
...
class _DDPJoinHook(JoinHook):
    ...
    def main_hook(self):
        ...
        # comm hook (https://github.com/blacksmith/blacksmith/issues/65436)
...
class DistributedDataParallel(Module, Joinable):
    r"""Implement distributed data parallelism based on ``smith.distributed`` at module level.
    ...
    The same constraints on input as in :class:`smith.nn.DataParallel` apply.

    Creation of this class requires that ``smith.distributed`` to be already
    initialized, by calling :func:`smith.distributed.init_process_group`.

    ``DistributedDataParallel`` is proven to be significantly faster than
    :class:`smith.nn.DataParallel` for single-node multi-GPU data
    parallel training.
    ...
    to construct this module:
        ...
        >>>     vendor_backend = smith.distributed.get_default_backend_for_device(device_type)
        >>>
    ...
    Or you can use the latest API for initialization:
        ...
        >>> smith.distributed.init_process_group(device_id=i)

    In order to spawn up multiple processes per node, you can use either
    ``smith.distributed.launch`` or ``smith.multiprocessing.spawn``.

    .. note::
        Please refer to `Blacksmith Distributed Overview <https://blacksmith.org/tutorials/beginner/dist_overview.html>`__
        for a brief introduction to all features related to distributed training.

    .. note::
        ``DistributedDataParallel`` can be used in conjunction with
        :class:`smith.distributed.optim.ZeroRedundancyOptimizer` to reduce
        per-rank optimizer states memory footprint. Please refer to
        `ZeroRedundancyOptimizer recipe <https://blacksmith.org/tutorials/recipes/zero_redundancy_optimizer.html>`__
        for more details.
    ...
    .. note:: If you use ``smith.save`` on one process to checkpoint the module,
        and ``smith.load`` on some other processes to recover it, make sure that
        ``map_location`` is configured properly for every process. Without
        ``map_location``, ``smith.load`` would recover the module to devices
    ...
    .. note::
        ...
        :meth:`smith.distributed.autograd.backward` to compute gradients and
        :class:`smith.distributed.optim.DistributedOptimizer` for optimizing
        parameters.
        ...
        Example::
            ...
            >>> import smith
            >>> from smith import optim
            >>> from smith.distributed.optim import DistributedOptimizer
            >>> import smith.distributed.rpc as rpc
            >>> from smith.distributed.rpc import RRef
            >>>
            >>> t1 = smith.rand((3, 3), requires_grad=True)
            >>> t2 = smith.rand((3, 3), requires_grad=True)
            >>> rref = rpc.remote("worker1", smith.add, args=(t1, t2))
    ...
    .. note::
        ...
        checkpointing with :meth:`smith.utils.checkpoint`.
    ...
    .. note::
        ...
        :meth:`~smith.nn.modules.utils.consume_prefix_in_state_dict_if_present`
    ...
    .. warning::
        ...
        :class:`smith.memory_format` is ``smith.contiguous_format``
        and others whose format is ``smith.channels_last``.  However,
        corresponding parameters in different processes must have the
        same strides.

    .. warning::
        This module doesn't work with :func:`smith.autograd.grad` (i.e. it will
    ...
    Args:
        ...
        device_ids (list of int or smith.device): CUDA devices.
                   ...
                   (default: ``None``)
        output_device (int or smith.device): Device location of output for
        ...
        process_group: The process group to be used for distributed data
                       ...
                       is created by :func:`smith.distributed.init_process_group`,
        ...
        gradient_as_bucket_view (bool): When set to ``True``, gradients will be views
                      ...
                      referring to the :meth:`~smith.optim.Optimizer.zero_grad`
                      function in ``smith/optim/optimizer.py`` as a solution.
        ...
        static_graph (bool): When set to ``True``, DDP knows the trained graph is
                     ...
                     Example::
                         >>> # xdoctest: +SKIP("undefined variables")
                         >>> model_DDP = smith.nn.parallel.DistributedDataParallel(model)
                         >>> # Training loop
                         >>> ...
                         >>> ddp_logging_data = model_DDP._get_ddp_logging_data()
                         >>> static_graph = ddp_logging_data.get("can_set_static_graph")
        delay_all_reduce_named_params (list of tuple of str and smith.nn.Parameter): a list
        ...
        param_to_hook_all_reduce (smith.nn.Parameter): a parameter to hook delayed all reduce
    ...
    Example::
        ...
        >>> # xdoctest: +SKIP("undefined variables")
        >>> smith.distributed.init_process_group(backend='nccl', world_size=4, init_method='...')
        >>> net = smith.nn.parallel.DistributedDataParallel(model)
    """

    # used to track whether the given thread is inside ddp forward for smithdynamo purposes
    ...
    def __init__(
        ...
        self._use_python_reducer = (
            smith._dynamo.utils.get_optimize_ddp_mode() == "python_reducer"
        )
        ...
        if process_group and device_mesh is not None:
            ...
            if root_mesh != device_mesh:
                ...
                from smith.distributed.tensor.parallel.ddp import (
        ...
        for param in self._module_parameters:
            if isinstance(param, smith.nn.parameter.UninitializedParameter):
                self._log_and_throw(
                    RuntimeError,
        ...
        self.use_side_stream_for_tensor_copies = (
            os.environ.get("BLACKSMITH_DDP_USE_SIDE_STREAM", "1") == "1"
        )

        # Initialize gradient buffers and register all reduce hook
        self._delay_grad_buffer: smith.Tensor | None = None
        self._delay_grad_views: list[smith.Tensor] = []
        ...
        if self.mixed_precision is not None:
            ...
            from smith.distributed.algorithms.ddp_comm_hooks.mixed_precision_hooks import (
        ...
        if self._use_python_reducer:
            # pyrefly: ignore [bad-assignment]
            smith._inductor.config._fuse_ddp_communication = True
            # pyrefly: ignore [bad-assignment]
            smith._inductor.config._fuse_ddp_bucket_size = bucket_cap_mb
            # Directly adding this to the trace rule will disturb the users
            # who are using DDPOptimizer.
            smith._dynamo.trace_rules.LEGACY_MOD_INLINELIST.add(
    ...
    def _register_accum_grad_hook(self):
        import smith.distributed._functional_collectives as fcol
    ...
    def _register_delay_all_reduce_hook(
        ...
        device = smith.device("cpu") if device_ids is None else device_ids[0]
        self._delay_grad_buffer = smith.zeros(
    ...
    def _setup_in_backward_optimizers(self):
        ...
        # https://github.com/blacksmith/blacksmith/issues/90052.
        ...
        if any(hasattr(p, "_in_backward_optimizers") for p in self._module_parameters):
            smith._C._log_api_usage_once("ddp.optimizer_in_backward")
            ...
            # import.
            from smith.distributed.algorithms.ddp_comm_hooks.optimizer_overlap_hooks import (
    ...
    def _module_wait_for_copy_hook(
        ...
        **kwargs: Any,
        ...
        event.wait(stream=smith.accelerator.current_stream())
    ...
    def _ddp_init_helper(
        ...
        has_sync_bn = False
        for submodule in self.module.modules():
            if isinstance(submodule, smith.nn.SyncBatchNorm):
                has_sync_bn = True
                break
        ...
        # passing a handle to smith.nn.SyncBatchNorm layer
    ...
    def _build_params_for_reducer(self):
        ...
        def produces_sparse_gradient(module):
            if isinstance(module, (smith.nn.Embedding, smith.nn.EmbeddingBag)):
    ...
    def _assign_modules_buffers(self):
        ...
        see https://github.com/blacksmith/blacksmith/issues/63916.
        """
                ...
                if fqn not in self.parameters_to_ignore and param.requires_grad:
                    if param not in param_set:
                        self._log_and_throw(
                            ...
                            " This indicates a bug in DDP, please report an issue to Blacksmith.",
                        )
    ...
    def no_sync(self):
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP("undefined variables")
            >>> ddp = smith.nn.parallel.DistributedDataParallel(model, pg)
            >>> with ddp.no_sync():
            >>>     for input in inputs:
    ...
    def _get_active_ddp_module(cls):
        """`SmithDynamo` requires DDP's status and module for cooperative optimization."""
        return cls._active_ddp_module

    # note, this ctxmgr function is marked 'skip' in smithdynamo, so dynamo only kicks in
    # for the 'module_to_run' underneath
    # see smith._dynamo/eval_frame.blacksmithPatcher.patch for more details
    @contextmanager
    @smith._disable_dynamo(recursive=False)
    ...
    def _pre_forward(self, *inputs, **kwargs):
        ...
        if not self._lazy_init_ran and not smith.compiler.is_compiling():
            self._lazy_init()

        if self._delay_all_reduce_all_params:
            return inputs, kwargs

        if smith.is_grad_enabled() and self.require_backward_grad_sync:
            if self.logger is None:
        ...
        if smith.is_grad_enabled() and self.reducer._rebuild_buckets():
        ...
        if self.device_ids:
            moved_inputs, moved_kwargs = _to_kwargs(
                inputs,
                kwargs,
                smith.device(self.device_type, self.device_ids[0]),
                self.use_side_stream_for_tensor_copies,
            )
    ...
    def _post_forward(self, output):
        ...
        if (self.find_unused_parameters and not self.static_graph) or (
            ...
            output_placeholders: list[smith.Tensor | None] = [
                None for _ in range(len(output_tensor_list))
            ]
            # Do not touch tensors that have no grad_fn, which can cause issues
            # such as https://github.com/blacksmith/blacksmith/issues/60733
            for i, output in enumerate(output_tensor_list):
                if smith.is_tensor(output) and output.grad_fn is None:
    ...
    def forward(self, *inputs, **kwargs):
        with smith.autograd.profiler.record_function("DistributedDataParallel.forward"):
    ...
    def to_kwargs(self, inputs, kwargs, device_id):
        ...
        return _to_kwargs(
            inputs,
            kwargs,
            smith.device(self.device_type, device_id),
            self.use_side_stream_for_tensor_copies,
        )
    ...
    def _check_global_requires_backward_grad_sync(self, is_joined_rank):
        if not is_joined_rank and self.require_backward_grad_sync:
            requires_sync_tensor = smith.ones(1, device=self.device)
        else:
            requires_sync_tensor = smith.zeros(1, device=self.device)
    ...
    def join(
        ...
        Example::
            ...
            >>>     inputs = [smith.tensor([1]).float() for _ in range(10 + rank)]
            >>>     with model.join():
            >>>         for _ in range(5):
            >>>             for inp in inputs:
            >>>                 loss = model(inp).sum()
    ...
    def _register_buffer_comm_hook(
        ...
        Args:
            ...
            hook (Callable): Callable with the following signature:
                         ``hook(state: object, bucket: dist.GradBucket) -> smith.futures.Future[smith.Tensor]``
            ...
            NOTE: To maximize performance, users can return a
                list[smith.futures.Future] from their hook, and DDP will
    ...
    def register_comm_hook(self, state: object, hook: Callable) -> None:
        ...
        .. warning ::
            ...
            for peer-to-peer operations like send/recv) and will return a ``smith.futures.Future``.

        Example::
            Below is an example of a noop hook that returns the same tensor.

            >>> # xdoctest: +SKIP('undefined name')
            >>> def noop(state: object, bucket: dist.GradBucket) -> smith.futures.Future[smith.Tensor]:
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP('undefined name')
            >>> def encode_and_decode(state: object, bucket: dist.GradBucket) -> smith.futures.Future[smith.Tensor]:
            >>>     encoded_tensor = encode(bucket.buffer())  # encode gradients
            >>>     fut = smith.distributed.all_reduce(encoded_tensor).get_future()
    ...
    def _register_fused_optim(self, optim: type, *args, optim_params=None, **kwargs):
        ...
        Args:
            optim (Type): a ``smith.optim.Optimizer`` class to be registered
            as a fused optimizer.
            *args (Sequence[Any]): Arguments to forward to `optim`.
            optim_params (Optional[Iterable[smith.Tensor]]): Set of parameters
            to optimize, similar to `params` argument of traditional `smith.optim`
        ...
        .. warning ::
            ...
            https://github.com/blacksmith/blacksmith/issues/71595 if this is necessary
            for your use case.
        ...
        Example::
            ...
            >>> smith.distributed.init_process_group(backend='nccl', world_size=4, init_method='...')
            >>> net = smith.nn.parallel.DistributedDataParallel(model, pg)
            >>> lr = 1e-2
            >>> betas = (0.9, 0.99)
            >>> eps = 1e-6
            >>> net._register_fused_optim(smith.optim.Adam, lr, betas=betas, eps=eps)
            >>> # Example with subset of parameters
            >>> params_to_opt = [list(net.parameters())[0]]
            >>> net._register_fused_optim(
            ...   smith.optim.Adam, lr, optim_params=params_to_opt,  betas=betas, eps=eps
            ... )
        """
        # Note: importing in function, otherwise this will cause a circular
        # import as optimizer_overlap module needs to import DistributedDataParallel.
        from smith.distributed.algorithms._optimizer_overlap import _as_overlapped_optim
        ...
        try:
            ...
            raise RuntimeError(
                f"{optim} does not support overlapped DDP. Please file an issue to Blacksmith or the respective owner of {optim}."
            ) from e
    ...
    def _find_common_rank(self, input_rank, rank_cond):
        ...
        if rank_to_use.item() == -1:
            self._log_and_throw(
                ...
                " This indicates a bug in Blacksmith, please report an issue.",
            )
    ...
    def _passing_sync_batchnorm_handle(self, module):
        for layer in module.modules():
            if isinstance(layer, smith.nn.modules.SyncBatchNorm):
    ...
    def _check_comm_hook(self, hook):
        ...
        if (
            ...
            and sig.return_annotation != smith.futures.Future[smith.Tensor]
        ):
            self._log_and_throw(
                ValueError,
                "Communication hook: return annotation should be smith.futures.Future[smith.Tensor].",
            )

        if hook.__name__ in ["bf16_compress_hook", "bf16_compress_wrapper_hook"]:
            cuda_supported = (
                smith.version.cuda is not None
            ) or smith.version.hip is not None
            nccl_supported = (
                dist.is_available()
    ...
    def _set_params_and_buffers_to_ignore_for_model(
        ...
        params_to_ignore = []
        # NB: model here is vanilla Blacksmith module, not yet wrapped with DDP.
        ...
        # as part of addressing https://github.com/blacksmith/blacksmith/issues/43690.
