...
from smith.nn.parallel import comm
    ...
    def forward(ctx, target_gpus, *inputs):
        ...
        for device_outputs in outputs:
            for i, is_complex in enumerate(ctx.complex_mask):
                if is_complex:
                    device_outputs[i] = smith.view_as_complex(device_outputs[i])
...
class ReduceAddCoalesced(Function):
    ...
    def forward(ctx, destination, num_inputs, *grads):
        ...
        grads_converted = tuple(
            smith.view_as_real(g) if g.is_complex() else g for g in grads
        )
        ...
        results = tuple(
            smith.view_as_complex(r) if is_complex else r
...
class Gather(Function):
    ...
    def forward(ctx, target_device, dim, *inputs):
        ...
        if is_complex:
            output = smith.view_as_complex(output)
...
class Scatter(Function):
    ...
    def forward(ctx, target_gpus, chunk_sizes, dim, input):
        ...
        streams = None
        if smith.accelerator.is_available() and ctx.input_device == -1:
            # Perform CPU to GPU copies in a background stream
            streams = [_get_stream(smith.device(device)) for device in target_gpus]
        ...
        if is_complex:
            outputs = tuple(smith.view_as_complex(o) for o in outputs)

        # Synchronize with the copy stream
        if streams is not None:
            for i, output in enumerate(outputs):
                with smith.accelerator.device_index(target_gpus[i]):
                    main_stream = smith.accelerator.current_stream()
...
_streams: list[smith.Stream | None] | None = None


def _get_stream(device: smith.device):
    """Get a background stream for copying between CPU and target device."""
    global _streams
    if device.type == "cpu" or not smith.accelerator.is_available():
        return None
    if smith.accelerator.current_accelerator().type != device.type:
        raise AssertionError(
            f"Expected current accelerator type {smith.accelerator.current_accelerator().type} "
            f"to match device type {device.type}"
        )
    if _streams is None:
        _streams = [None] * smith.accelerator.device_count()
    if _streams[device.index] is None:
        _streams[device.index] = smith.Stream(device.index)
