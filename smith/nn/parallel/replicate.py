...
if TYPE_CHECKING:
    from smith._C import ScriptMethod
    from smith.jit import ScriptModule
    from smith.jit._state import EnabledProxy
...
def _is_script_module(module: Module) -> TypeIs["ScriptModule"]:
    ...
    return isinstance(module, smith.jit.ScriptModule)


def _is_script_method(module: object) -> TypeIs["ScriptMethod"]:
    import smith.jit

    return isinstance(module, smith._C.ScriptMethod)


def _init_script_module() -> "ScriptModule":
    import smith.jit

    return smith.jit.ScriptModule()


def _is_jit_enabled() -> "EnabledProxy":
    import smith.jit._state

    return smith.jit._state._enabled
...
def _broadcast_coalesced_reshape(
    ...
    devices: Sequence[int | smith.device],
    detach: bool = False,
    ...
    if detach:
        complex_mask = [
            not isinstance(t, smith.nn.UninitializedParameter) and t.is_complex()
            for t in tensors
        ]
...
def replicate(
    ...
    buffers_rg: list[smith.Tensor] = []
    buffers_not_rg: list[smith.Tensor] = []
    for buf in buffers:
