...
import smith
from smith.nn.parallel._functions import Gather, Scatter
...
def scatter(
    inputs: smith.Tensor,
    target_gpus: Sequence[int | smith.device],
    dim: int = ...,
) -> tuple[smith.Tensor, ...]: ...
...
def gather(outputs: Any, target_device: int | smith.device, dim: int = 0) -> Any:
    ...
    Args:
        ...
        target_device (Union[int, smith.device]): The device to gather the tensors to.
