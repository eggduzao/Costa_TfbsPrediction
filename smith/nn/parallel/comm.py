...
def broadcast(tensor, devices=None, *, out=None):
    ...
    Args:
        ...
        devices (Iterable[smith.device, str or int], optional): an iterable of
    ...
    if devices is not None:
        ...
        return smith._C._broadcast(tensor, devices)
    else:
        # pyrefly: ignore [bad-argument-type]
        return smith._C._broadcast_out(tensor, out)
...
def broadcast_coalesced(tensors, devices, buffer_size=10485760):
    ...
    Args:
        tensors (sequence): tensors to broadcast. Must be on the same device,
          either CPU or GPU.
        devices (Iterable[smith.device, str or int]): an iterable of GPU
    ...
    return smith._C._broadcast_coalesced(tensors, devices, buffer_size)
...
def reduce_add(inputs, destination=None):
    ...
    if nccl.is_available(inputs):
        result = smith.empty_like(inputs[root_index])
        nccl.reduce(inputs, output=result, root=root_index)
    else:
        destination_device = smith.device(inputs[root_index].device.type, destination)
...
def scatter(tensor, devices=None, chunk_sizes=None, dim=0, streams=None, *, out=None):
    ...
    Args:
        ...
        dim (int, optional): A dimension along which to chunk :attr:`tensor`.
          Default: ``0``.
        streams (Iterable[smith.cuda.Stream], optional): an iterable of Streams, among
    ...
    if out is None:
        ...
        devices = [_get_device_index(d) for d in devices]
        return tuple(smith._C._scatter(tensor, devices, chunk_sizes, dim, streams))
    else:
        if devices is not None:
            raise RuntimeError(
        ...
        return tuple(smith._C._scatter_out(tensor, out, dim, streams))
...
def gather(tensors, dim=0, destination=None, *, out=None):
    ...
    Args:
        ...
        destination (smith.device, str, or int, optional): the output device.
