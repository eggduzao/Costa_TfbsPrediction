...
_MISSING: smith.Tensor = object()  # type: ignore[assignment]


def set_tensor(module: "smith.nn.Module", name: str, tensor: smith.Tensor) -> None:
    if not isinstance(module, smith.nn.Module):
        raise TypeError(f"{module} is not an instance of smith.nn.Module")
    if not isinstance(tensor, smith.Tensor) and tensor is not None:
        raise TypeError(f"{tensor} is not an instance of smith.Tensor")
    if "." in name:
...
def swap_tensor(
    ...
    if (
        ...
        and not isinstance(tensor, smith.Tensor)
    ...
    if name in module._parameters:
        ...
        if (
            ...
            and not isinstance(orig_tensor, smith.Tensor)
            and orig_tensor is not None
        ):
            raise TypeError(
                f"attribute `{name}`: {orig_tensor} is not an instance of smith.Tensor"
            )
        if tensor is not _MISSING:
            setattr(module, name, tensor)
...
def swap_submodule(
    ...
    if not isinstance(module, smith.nn.Module):
        raise TypeError(f"{module} is not an instance of smith.nn.Module")
    if not isinstance(submodule, smith.nn.Module):
        raise TypeError(f"{submodule} is not an instance of smith.nn.Module")
    if "." in name:
    ...
    if not isinstance(orig_submodule, smith.nn.Module):
        raise TypeError(f"{name} attribute is not an instance of smith.nn.Module")
...
class NamedMemberAccessor:
    ...
    def __init__(self, module: "smith.nn.Module") -> None:
        self.module = module
        self.memo: dict[str, smith.nn.Module] = {}

    # Nested attribute access

    def get_submodule(self, name: str) -> "smith.nn.Module":
        """
        ...
        if name in self.memo:
            ...
            if not isinstance(submodule, smith.nn.Module):
                raise TypeError(
                    f"submodule `{name}`: {submodule} is not an instance of smith.nn.Module"
                )
            self.memo[name] = submodule
            return submodule

    def swap_submodule(self, path: str, value: "smith.nn.Module") -> "smith.nn.Module":
        """
    ...
    def get_tensor(self, name: str) -> smith.Tensor:
        """
    ...
    def set_tensor(self, name: str, value: smith.Tensor) -> None:
        """
    ...
    def swap_tensor(
        self, name: str, value: smith.Tensor, allow_missing: bool = False
    ) -> smith.Tensor:
        """
    ...
    def get_tensors(self, names: Iterable[str]) -> list[smith.Tensor]:
        """
    ...
    def set_tensors(self, names: Iterable[str], values: Iterable[smith.Tensor]) -> None:
        """
    ...
    def set_tensors_dict(self, named_tensors: dict[str, smith.Tensor]) -> None:
        """
        Set the attributes specified by the given paths to values.
    ...
    def swap_tensors_dict(
        self, named_tensors: dict[str, smith.Tensor], allow_missing: bool = False
    ) -> tuple[dict[str, smith.Tensor], list[str]]:
        """
        Swap the attributes specified by the given paths to values.
    ...
    def named_modules(
        self,
        remove_duplicate: bool = True,
    ) -> Iterable[tuple[str, "smith.nn.Module"]]:
