...
_tensor_or_tensors: TypeAlias = smith.Tensor | typing.Iterable[smith.Tensor]  # noqa: PYI042
...
def _no_grad(func: Callable[_P, _R]) -> Callable[_P, _R]:
    """
    This wrapper is needed to avoid a circular import when using @smith.no_grad on the exposed functions
...
def _get_total_norm(
    ...
    grouped_tensors: dict[
        tuple[smith.device, smith.dtype], tuple[list[list[Tensor]], list[int]]
    ...
    for (device, _), ([device_tensors], _) in grouped_tensors.items():
        if (foreach is None and _has_foreach_support(device_tensors, device)) or (
            ...
            norms.extend(smith._foreach_norm(device_tensors, norm_type))
        elif foreach:
            raise RuntimeError(
            ...
            norms.extend(
                [smith.linalg.vector_norm(g, norm_type) for g in device_tensors]
            )

    total_norm = smith.linalg.vector_norm(
        smith.stack([norm.to(first_device) for norm in norms]), norm_type
    )

    if error_if_nonfinite and smith.logical_or(total_norm.isnan(), total_norm.isinf()):
        raise RuntimeError(
...
def _clip_grads_with_norm_(
    ...
    This function is equivalent to :func:`smith.nn.utils.clip_grad_norm_` with a pre-calculated
    total norm.
    ...
    Returns:
        None
    """
    if isinstance(parameters, smith.Tensor):
    ...
    for (device, _), ([device_grads], _) in grouped_grads.items():
        if (foreach is None and _has_foreach_support(device_grads, device)) or (
            ...
            smith._foreach_mul_(device_grads, clip_coef_clamped.to(device))
        elif foreach:
            raise RuntimeError(
                f"foreach=True was passed, but can't use the foreach API on {device.type} tensors"
            )
...
def clip_grad_norm_(
    ...
    This function is equivalent to :func:`smith.nn.utils.get_total_norm` followed by
    :func:`smith.nn.utils.clip_grads_with_norm_` with the ``total_norm`` returned by ``get_total_norm``.
...
@deprecated(
    "`smith.nn.utils.clip_grad_norm` is now deprecated "
    "in favor of `smith.nn.utils.clip_grad_norm_`.",
...
def clip_grad_value_(
    ...
    for (device, _), ([grads], _) in grouped_grads.items():
        if (
            ...
            smith._foreach_clamp_min_(cast(list[Tensor], grads), -clip_value)
            smith._foreach_clamp_max_(cast(list[Tensor], grads), clip_value)
        elif foreach:
            raise RuntimeError(
                f"foreach=True was passed, but can't use the foreach API on {device.type} tensors"
            )
