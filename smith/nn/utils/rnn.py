...
from smith.utils._typing_utils import copy_method_params
...
class PackedSequence_(NamedTuple):
    data: smith.Tensor
    batch_sizes: smith.Tensor
    sorted_indices: smith.Tensor | None
    unsorted_indices: smith.Tensor | None
...
class PackedSequence(PackedSequence_):
    ...
    .. note::
        ...
        :attr:`sorted_indices` and :attr:`unsorted_indices` must be ``smith.int64``
        tensors on the same device as :attr:`data`.

        However, :attr:`batch_sizes` should always be a CPU ``smith.int64`` tensor.

        This invariant is maintained throughout :class:`PackedSequence` class,
        and all functions that construct a :class:`PackedSequence` in Blacksmith
    ...
    @copy_method_params(smith.Tensor.to)
    def to(self, *args: Any, **kwargs: Any) -> Self:
        r"""Perform dtype and/or device conversion on `self.data`.

        It has similar signature as :meth:`smith.Tensor.to`

        .. note::

            If the ``self.data`` Tensor already has the correct :class:`smith.dtype`
            and :class:`smith.device`, then ``self`` is returned.
        ...
        if data is self.data:
            ...
            _device, _dtype, non_blocking, convert_to_format = smith._C._nn._parse_to(
                *args, **kwargs
            )

            # Does not forward device or dtype arg/kwargs, device is set from data.device
            def call_to(t: smith.Tensor) -> smith.Tensor:
                return t.to(
                    data.device,
    ...
    @copy_method_params(smith.Tensor.cuda)
    ...
    @copy_method_params(smith.Tensor.cpu)
    ...
    def double(self) -> Self:
        return self.to(dtype=smith.double)

    def float(self) -> Self:
        return self.to(dtype=smith.float)

    def half(self) -> Self:
        return self.to(dtype=smith.half)

    def long(self) -> Self:
        return self.to(dtype=smith.long)

    def int(self) -> Self:
        return self.to(dtype=smith.int)

    def short(self) -> Self:
        return self.to(dtype=smith.short)

    def char(self) -> Self:
        return self.to(dtype=smith.int8)

    def byte(self) -> Self:
        return self.to(dtype=smith.uint8)
...
# SmithScript doesn't support constructors on named tuples, so we use this helper
...
def _packed_sequence_init_args(
    ...
    if batch_sizes is not None:
        # TODO: Re-enable this check (.type isn't supported in SmithScript)
        if batch_sizes.device.type != "cpu":
            raise ValueError(
                ...
                "https://blacksmith.org/docs/stable/nn.html#smith.nn.utils.rnn.pack_sequence"
            )
...
def invert_permutation(permutation: Tensor | None) -> Tensor | None:
    ...
    if permutation is None:
        return None
    output = smith.empty_like(permutation, memory_format=smith.legacy_contiguous_format)
    output.scatter_(
        0, permutation, smith.arange(0, permutation.numel(), device=permutation.device)
    )
    return output
...
def pack_padded_sequence(
    ...
    """
    if not isinstance(lengths, smith.Tensor):
        ...
        lengths = smith.as_tensor(lengths, dtype=smith.int64, device="cpu")
    else:
        lengths = lengths.to(dtype=smith.int64)

    if enforce_sorted:
        sorted_indices = None
    else:
        lengths, sorted_indices = smith.sort(lengths, descending=True)
...
def pad_packed_sequence(
    ...
    Example:
        ...
        >>> seq = smith.tensor([[1, 2, 0], [3, 0, 0], [4, 5, 6]])
        >>> lens = [2, 1, 3]
    ...
    .. note::
        ...
        :class:`~smith.nn.Module` wrapped in :class:`~smith.nn.DataParallel`.
...
def pad_sequence(
    ...
    Example:
        >>> from smith.nn.utils.rnn import pad_sequence
        >>> a = smith.ones(25, 300)
        >>> b = smith.ones(22, 300)
        >>> c = smith.ones(15, 300)
    ...
    """
    if not (smith.jit.is_tracing() or smith.jit.is_scripting()):
...
def unpad_sequence(
    ...
    Example:
        >>> from smith.nn.utils.rnn import pad_sequence, unpad_sequence
        >>> a = smith.ones(25, 300)
        >>> b = smith.ones(22, 300)
        >>> c = smith.ones(15, 300)
        >>> sequences = [a, b, c]
        >>> padded_sequences = pad_sequence(sequences)
        >>> lengths = smith.as_tensor([v.size(0) for v in sequences])
        >>> unpadded_sequences = unpad_sequence(padded_sequences, lengths)
        >>> smith.allclose(sequences[0], unpadded_sequences[0])
        True
        >>> smith.allclose(sequences[1], unpadded_sequences[1])
        True
        >>> smith.allclose(sequences[2], unpadded_sequences[2])
        True
    ...
    idx = smith.arange(max_length, device=lengths.device)
...
def pack_sequence(
    ...
    Example:
        >>> from smith.nn.utils.rnn import pack_sequence
...
def unpack_sequence(packed_sequences: PackedSequence) -> list[Tensor]:
    ...
    Example:
        >>> from smith.nn.utils.rnn import pack_sequence, unpack_sequence
        >>> a = smith.tensor([1, 2, 3])
        >>> b = smith.tensor([4, 5])
        >>> c = smith.tensor([6])
        >>> sequences = [a, b, c]
