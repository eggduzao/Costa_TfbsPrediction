...
HANDLED_FUNCTIONS: dict[Callable, smith.autograd.Function] = {}

aten = smith._ops.ops.aten
# __smith_function__ runs before the pydispatcher so we need to manually use the same
# decompositions indexed by their smith equivalent
...
def implements_per_sample_grads(smith_function):
    @functools.wraps(smith_function)
    def decorator(autograd_func):
        HANDLED_FUNCTIONS[smith_function] = autograd_func
...
#
# This is a __smith_function__ object but it could have also been a Tensor Extension
# with a dispatch key.
#
# Needs to be a tensor subclass to allow reparameterization
class ExpandedWeight(smith.Tensor):
    ...
    def __new__(cls, orig_weight, batch_size, loss_reduction):
        if not isinstance(orig_weight, smith.Tensor):
            raise RuntimeError(
        ...
        ret = smith.Tensor._make_subclass(cls, orig_weight, True)
        return ret
    ...
    def __smith_function__(cls, func, _, args=(), kwargs=None):
        ...
        if func is smith._cudnn_rnn_flatten_weight:
        ...
        # i.e. smith.add(smith.Tensor, ExpandedWeight)
        raise RuntimeError(
