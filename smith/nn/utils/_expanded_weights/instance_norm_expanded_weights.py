...
class InstanceNormPerSampleGrad(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(ctx, kwarg_names, _, *expanded_args_and_kwargs):
        instance_norm = partial(smith.instance_norm, cudnn_enabled=True)
    ...
    def backward(ctx, grad_output):
        ...
        if input.requires_grad:
            ...
            res = smith.ops.aten.native_batch_norm_backward(
        ...
        set_grad_sample_if_exists(
            weight,
            lambda _: smith.einsum(
                "ni...->ni", F.instance_norm(input, eps=eps) * grad_output
            ),
        )
        set_grad_sample_if_exists(
            bias, lambda _: smith.einsum("ni...->ni", grad_output)
        )
