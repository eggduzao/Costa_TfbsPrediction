...
def conv_backward(func, ctx, grad_output):
    ...
    results: list[smith.Tensor | None] = []
...
def conv_unfold_weight_grad_sample(
    ...
    weight_grad_sample = smith.einsum("noq,npq->nop", grad_output, input)
    ...
    weight_grad_sample = smith.einsum(
...
def unfold3d(
    ...
    :class:`smith.nn.Unfold` only supports 4D inputs (batched image-like tensors).
    ...
    Returns:
        ...
        See :class:`smith.nn.Unfold` for more details
    Example:
        >>> # xdoctest: +SKIP
        >>> B, C, D, H, W = 3, 4, 5, 6, 7
        >>> tensor = smith.arange(1, B * C * D * H * W + 1.0).view(B, C, D, H, W)
    ...
    # For D_out, H_out, W_out definitions see :class:`smith.nn.Unfold`
