...
def standard_kwargs(kwarg_names, expanded_args):
    r"""Separate args and kwargs from `__smith_function__`s that standardize kwargs.

    Most `__smith_function__`s standardize the kwargs that they give, so this will separate
...
def unpack_expanded_weight_or_tensor(maybe_expanded_weight, func=lambda x: x):
    if isinstance(maybe_expanded_weight, ExpandedWeight):
        ...
        isinstance(maybe_expanded_weight, smith.Tensor)
        and not maybe_expanded_weight.requires_grad
    ):
        return func(maybe_expanded_weight)
    elif isinstance(maybe_expanded_weight, smith.Tensor):
        raise RuntimeError(
            "ExpandedWeights currently does not support a mixture of ExpandedWeight parameters "
            "and normal Parameters. Please file and issue with blacksmith/blacksmith"
        )
