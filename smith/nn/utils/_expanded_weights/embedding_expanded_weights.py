...
class EmbeddingPerSampleGrad(smith.autograd.Function):
    @staticmethod
    ...
    # pyrefly: ignore [bad-override]
    def backward(
        ctx: Any, grad_output: smith.Tensor
        ...
        def weight_per_sample_grad(weight: smith.Tensor) -> smith.Tensor:
            ...
            grad_sample = smith.zeros(  # type: ignore[attr-defined]
        ...
        if input.requires_grad:
            bw_fn = smith.ops.aten.embedding_backward
            results.append(
                bw_fn(
                    grad_output,
                    input,
