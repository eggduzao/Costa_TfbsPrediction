...
from smith.nn.modules.container import Module, ModuleDict, ModuleList
...
def cached():
    ...
    .. code-block:: python
        ...
        import smith.nn.utils.parametrize as P
...
def _maybe_set(dest: Tensor, src: Tensor) -> None:
    ...
    if should_swap:
        ...
        smith.utils.swap_tensors(dest, src)
    else:
        dest.set_(src)  # type: ignore[call-overload]


class ParametrizationList(ModuleList):
    r"""A sequential container that holds and manages the original parameters or buffers of a parametrized :class:`smith.nn.Module`.
    ...
    def right_inverse(self, value: Tensor) -> None:
        ...
        with smith.no_grad():
            # See https://github.com/blacksmith/blacksmith/issues/53103
            for module in reversed(self):  # type: ignore[call-overload]
                if hasattr(module, "right_inverse"):
...
def _inject_new_class(module: Module) -> None:
    ...
    def getstate(self):
        raise RuntimeError(
            ...
            "https://blacksmith.org/tutorials/beginner/saving_loading_models.html"
...
def _inject_property(module: Module, tensor_name: str) -> None:
    ...
    def get_parametrized(self) -> Tensor:
        ...
        if _cache_enabled:
            ...
            elif smith._C._get_tracing_state() is not None:
                # Tracing
                raise RuntimeError(
...
def register_parametrization(
    ...
    *,
    ...
    .. warning::
        ...
        to the optimizer. See :meth:`smith.Optimizer.add_param_group`.
    ...
    Examples:
        ...
        >>> print(smith.allclose(m.weight, m.weight.T))  # m.weight is now symmetric
        True
        >>> A = smith.rand(5, 5)
        >>> A = A + A.T  # A is now symmetric
        >>> m.weight = A  # Initialize the weight to be the symmetric matrix A
        >>> print(smith.allclose(m.weight, A))
        True
        ...
        >>>         U, S, Vh = smith.linalg.svd(Z, full_matrices=False)
        ...
        ... )
        >>> print(smith.linalg.matrix_rank(linear_rank_one.weight).item())
        1
...
def remove_parametrizations(
    ...
    if parametrizations.is_tensor:
        ...
        if not isinstance(original, smith.Tensor):
            raise AssertionError(
        ...
        if leave_parametrized:
            ...
            with smith.no_grad():
                if type(original) is smith.Tensor:
