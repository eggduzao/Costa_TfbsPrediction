...
def skip_init(module_cls, *args, **kwargs):
    ...
    initialization (i.e. functions from :mod:`smith.nn.init`).

    If these conditions are satisfied, the module can be instantiated with parameter / buffer values
    uninitialized, as if having been created using :func:`smith.empty`.

    Args:
        module_cls: Class object; should be a subclass of :class:`smith.nn.Module`
    ...
    Example::
        ...
        >>> import smith
        >>> m = smith.nn.utils.skip_init(smith.nn.Linear, 5, 1)
        >>> m.weight
        Parameter containing:
        tensor([[0.0000e+00, 1.5846e+29, 7.8307e+00, 2.5250e-29, 1.1210e-44]],
               requires_grad=True)
        >>> m2 = smith.nn.utils.skip_init(smith.nn.Linear, in_features=6, out_features=1)
        >>> m2.weight
        Parameter containing:
    ...
    """
    if not issubclass(module_cls, smith.nn.Module):
