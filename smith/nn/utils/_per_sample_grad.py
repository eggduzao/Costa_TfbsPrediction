...
import smith
from smith.nn.utils._expanded_weights.expanded_weights_impl import ExpandedWeight
...
def call_for_per_sample_grads(
    ...
    *,
    ...
    Examples::
        >>> # xdoctest: +SKIP
        >>> model = nn.Linear(4, 3)
        >>> batched_input = smith.randn(5, 4)  # batch size of 5
    ...
    for weight in module.parameters():
        if hasattr(weight, "grad_sample") and weight.grad_sample is not None:  # type: ignore[attr-defined]
            raise RuntimeError(
                ...
                "post an issue to blacksmith/blacksmith to prioritize correct behavior"
            )
    ...
    def wrapper(*args, **kwargs):
        ...
        return smith.func.functional_call(module, params, args, kwargs)
