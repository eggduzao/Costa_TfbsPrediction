...
_M = TypeVar("_M", bound="smith.nn.Module")


def convert_conv2d_weight_memory_format(
    module: _M, memory_format: smith.memory_format
) -> _M:
    ...
    .. note::
        Calling ``model.to(memory_format=smith.channels_last)`` is more aggressive
        ...
        This claim might change when Blacksmith supports fusion of permutation, as
    ...
    Args:
        ...
        memory_format: user specified ``memory_format``,
            e.g. ``smith.channels_last`` or ``smith.contiguous_format``
    ...
    Example:
        ...
        >>> input = smith.randint(
        ...     1, 10, (2, 8, 4, 4), dtype=smith.float16, device="cuda"
        ... )
        >>> model = nn.Sequential(
        >>>     nn.Conv2d(8, 4, 3)).cuda().half()
        >>> # This is identical to:
        >>> # nn.utils.convert_conv2d_weight_memory_format(model, smith.channels_last)
        >>> model = nn.utils.convert_conv2d_weight_memory_format(
        ...     model, smith.channels_last
        ... )
        >>> out = model(input)
    """
    # TODO: expand this to `_ConvNd` when channels_last support is extended
    # beyond only 4d tensors.
    if isinstance(module, (smith.nn.Conv2d, smith.nn.ConvTranspose2d)):
...
def convert_conv3d_weight_memory_format(
    ...
    .. note::
        Calling ``model.to(memory_format=smith.channels_last_3d)`` is more aggressive
    ...
    Example:
        ...
        >>> # xdoctest: +REQUIRES(env:CUBLAS_WORKSPACE_CONFIG)
        >>> input = smith.randint(
        ...     1, 10, (2, 8, 4, 4, 4), dtype=smith.float16, device="cuda"
        ... )
        >>> model = nn.Sequential(
        >>>     nn.Conv3d(8, 4, 3)).cuda().half()
        >>> # This is identical to:
        >>> # nn.utils.convert_conv3d_weight_memory_format(model, smith.channels_last_3d)
        >>> model = nn.utils.convert_conv3d_weight_memory_format(
        ...     model, smith.channels_last_3d
        ... )
        >>> out = model(input)
    """
    ...
    # TODO: expand this to `_ConvNd` when channels_last support is extended
    # beyond only 4d tensors.
    if isinstance(module, (smith.nn.Conv3d, smith.nn.ConvTranspose3d)):
        weight_data = module.weight.detach().clone(memory_format=memory_format)
        module.weight.data = weight_data.resize_(
            weight_data.size(), memory_format=memory_format
        )
