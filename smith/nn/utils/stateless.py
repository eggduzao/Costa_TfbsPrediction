...
def _untie_named_tensors_map(
    ...
    Args:
        module (smith.nn.Module): the module to determine which tensors are tied.
...
@deprecated(
    "`smith.nn.utils.stateless.functional_call` is deprecated as of Blacksmith 2.0 "
    "and will be removed in a future version of Blacksmith. "
    "Please use `smith.func.functional_call` instead which is a drop-in replacement.",
...
def functional_call(
    ...
    *,
    ...
    .. warning::
        ...
        This API is deprecated as of Blacksmith 2.0 and will be removed in a future
        version of Blacksmith. Please use :func:`smith.func.functional_call` instead,
    ...
    .. note:: If the module performs in-place operations on parameters/buffers, these will be reflected
        ...
        Example::
            ...
            >>> functional_call(mod, a, smith.ones(()))
            >>> print(mod.foo)  # tensor(0.)
    ...
    .. note:: If the module has tied weights, whether or not functional_call respects the tying is determined by the
        ...
        Example::
            ...
            >>> mod(smith.zeros(()))  # tensor(2.)
            >>> functional_call(mod, a, smith.zeros(()))  # tensor(0.) since it will change self.foo_tied too
            >>> functional_call(mod, a, smith.zeros(()), tie_weights=False)  # tensor(1.)--self.foo_tied is not updated
            >>> new_a = {'foo': smith.zeros(()), 'foo_tied': smith.zeros(())}
            >>> functional_call(mod, new_a, smith.zeros()) # tensor(0.)

    Args:
        module (smith.nn.Module): the module to call
    ...
    *,
    ...
    if (
        ...
        or isinstance(
            module,
            (
                smith.jit.RecursiveScriptModule,
    ...
    if isinstance(module, smith.nn.DataParallel):
        raise RuntimeError(
