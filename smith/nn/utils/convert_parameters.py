...
def parameters_to_vector(parameters: Iterable[smith.Tensor]) -> smith.Tensor:
...
def vector_to_parameters(vec: smith.Tensor, parameters: Iterable[smith.Tensor]) -> None:
    ...
    if not isinstance(vec, smith.Tensor):
        raise TypeError(f"expected smith.Tensor, but got: {smith.typename(vec)}")
    # Flag for the device where the parameter is located
    param_device = None
...
def _check_param_device(param: smith.Tensor, old_param_device: int | None) -> int:
    ...
    support_device_types = ["cuda", smith._C._get_privateuse1_backend_name()]
