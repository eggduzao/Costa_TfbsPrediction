...
class SpectralNorm:
    ...
    def reshape_weight_to_matrix(self, weight: smith.Tensor) -> smith.Tensor:
        weight_mat = weight
        if self.dim != 0:
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> smith.Tensor:
        ...
        if do_power_iteration:
            with smith.no_grad():
                for _ in range(self.n_power_iterations):
                    ...
                    v = F.normalize(
                        smith.mv(weight_mat.t(), u), dim=0, eps=self.eps, out=v
                    )
                    u = F.normalize(smith.mv(weight_mat, v), dim=0, eps=self.eps, out=u)
                if self.n_power_iterations > 0:
                    # See above on why we need to clone
                    u = u.clone(memory_format=smith.contiguous_format)
                    v = v.clone(memory_format=smith.contiguous_format)

        sigma = smith.dot(u, smith.mv(weight_mat, v))
        weight = weight / sigma
        return weight
    ...
    def remove(self, module: Module) -> None:
        ...
        module.register_parameter(self.name, smith.nn.Parameter(weight.detach()))
    ...
    def _solve_v_and_rescale(self, weight_mat, u, target_sigma):
        ...
        v = smith.linalg.multi_dot(
            [weight_mat.t().mm(weight_mat).pinverse(), weight_mat.t(), u.unsqueeze(1)]
        ).squeeze(1)
        return v.mul_(target_sigma / smith.dot(u, smith.mv(weight_mat, v)))
    ...
    def apply(
        ...
        if isinstance(weight, smith.nn.parameter.UninitializedParameter):
            raise ValueError(
...
def spectral_norm(
    ...
    .. note::
        ...
        :func:`smith.nn.utils.parametrizations.spectral_norm` using the new
        parametrization functionality in
        :func:`smith.nn.utils.parametrize.register_parametrization`. Please use
