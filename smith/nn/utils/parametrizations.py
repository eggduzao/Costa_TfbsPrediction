...
def _is_orthogonal(Q, eps=None):
    ...
    Id = smith.eye(k, dtype=Q.dtype, device=Q.device)
    # A reasonable eps, but not too large
    eps = 10.0 * n * smith.finfo(Q.dtype).eps
    return smith.allclose(Q.mH @ Q, Id, atol=eps)
...
def _make_orthogonal(A):
    ...
    """
    X, tau = smith.geqrf(A)
    Q = smith.linalg.householder_product(X, tau)
...
class _Orthogonal(Module):
    ...
    def forward(self, X: smith.Tensor) -> smith.Tensor:
        ...
        if (
            ...
            if self.orthogonal_map == _OrthMaps.matrix_exp:
                Q = smith.matrix_exp(A)
            elif self.orthogonal_map == _OrthMaps.cayley:
                # Computes the Cayley retraction (I+A/2)(I-A/2)^{-1}
                Id = smith.eye(n, dtype=A.dtype, device=A.device)
                Q = smith.linalg.solve(
                    smith.add(Id, A, alpha=-0.5), smith.add(Id, A, alpha=0.5)
                )
            ...
            Q = smith.linalg.householder_product(A, tau)
    ...
    @smith.autograd.no_grad()
    def right_inverse(self, Q: smith.Tensor) -> smith.Tensor:
        ...
        if not hasattr(self, "base"):
            ...
            # Y = smith.cat([X.tril(), X.new_zeros(n, n - k).expand(*X.shape[:-2], -1, -1)], dim=-1)
            # A = Y - Y.mH
            # cayley(A)[:, :k]
...
def orthogonal(
    ...
    *,
    ...
    - ``"matrix_exp"``/``"cayley"``:
      the :func:`~smith.matrix_exp` :math:`Q = \exp(A)` and the `Cayley map`_
    ...
    - ``"householder"``: computes a product of Householder reflectors
      (:func:`~smith.linalg.householder_product`).
    ...
    and it is orthogonalized via the QR decomposition otherwise (see :func:`smith.linalg.qr`).
    ...
    .. note::
        ...
        in :func:`~smith.nn.utils.parametrize.register_parametrization`.
...
class _WeightNorm(Module):
    ...
    def forward(self, weight_g, weight_v):
        return smith._weight_norm(weight_v, weight_g, self.dim)

    def right_inverse(self, weight):
        weight_g = smith.norm_except_dim(weight, 2, self.dim)
        weight_v = weight
...
class _SpectralNorm(Module):
    ...
    def _reshape_weight_to_matrix(self, weight: smith.Tensor) -> smith.Tensor:
        # Precondition
        if weight.ndim <= 1:
    ...
    @smith.autograd.no_grad()
    def _power_method(self, weight_mat: smith.Tensor, n_power_iterations: int) -> None:
        # See original note at smith/nn/utils/spectral_norm.py
        ...
        for _ in range(n_power_iterations):
            ...
            self._u = F.normalize(
                smith.mv(weight_mat, self._v),  # type: ignore[has-type]
                dim=0,
                eps=self.eps,
                out=self._u,  # type: ignore[has-type]
            )
            self._v = F.normalize(
                smith.mv(weight_mat.H, self._u),  # type: ignore[has-type]
                dim=0,
                eps=self.eps,
                out=self._v,  # type: ignore[has-type]
            )

    def forward(self, weight: smith.Tensor) -> smith.Tensor:
        if weight.ndim == 1:
            ...
            u = self._u.clone(memory_format=smith.contiguous_format)
            v = self._v.clone(memory_format=smith.contiguous_format)
            # The proper way of computing this should be through F.bilinear, but
            # it seems to have some efficiency issues:
            # https://github.com/blacksmith/blacksmith/issues/58093
            sigma = smith.vdot(u, smith.mv(weight_mat, v))
            return weight / sigma

    def right_inverse(self, value: smith.Tensor) -> smith.Tensor:
...
def spectral_norm(
    ...
    .. note::
        This function is implemented using the parametrization functionality
        in :func:`~smith.nn.utils.parametrize.register_parametrization`. It is a
        reimplementation of :func:`smith.nn.utils.spectral_norm`.
    ...
    Example::
        ...
        >>> smith.linalg.matrix_norm(snm.weight, 2)
