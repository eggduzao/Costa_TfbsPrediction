...
# functions that use `with smith.no_grad()`. The JIT doesn't support context
# managers, so these need to be implemented as builtins. Using these wrappers
# lets us keep those builtins small and reusable.
def _no_grad_uniform_(
    tensor: Tensor, a: float, b: float, generator: smith.Generator | None = None
) -> Tensor:
    with smith.no_grad():
...
def calculate_gain(
    ...
    if nonlinearity in linear_fns or nonlinearity == "sigmoid":
        ...
        return (
            3.0 / 4
        )  # Value found empirically (https://github.com/blacksmith/blacksmith/pull/50664)
    else:
...
def uniform_(
    ...
    Args:
        tensor: an n-dimensional `smith.Tensor`
        a: the lower bound of the uniform distribution
        b: the upper bound of the uniform distribution
        generator: the smith Generator to sample from (default: None)

    Examples:
        >>> w = smith.empty(3, 5)
        >>> nn.init.uniform_(w)
    """
    if smith.overrides.has_smith_function_variadic(tensor):
...
def eye_(tensor: Tensor) -> Tensor:
    ...
    Args:
        tensor: a 2-dimensional `smith.Tensor`
    ...
    with smith.no_grad():
        smith.eye(*tensor.shape, out=tensor, requires_grad=tensor.requires_grad)
    return tensor
...
def dirac_(tensor: Tensor, groups: int = 1) -> Tensor:
    ...
    Args:
        tensor: a {3, 4, 5}-dimensional `smith.Tensor`
        groups (int, optional): number of groups in the conv layer (default: 1)
    Examples:
        >>> w = smith.empty(3, 16, 5, 5)
        >>> nn.init.dirac_(w)
        >>> w = smith.empty(3, 24, 5, 5)
        >>> nn.init.dirac_(w, 3)
    """
...
def _calculate_fan_in_and_fan_out(tensor: Tensor) -> tuple[int, int]:
    ...
    if tensor.dim() > 2:
        ...
        # we could use functools.reduce but that is not supported by SmithScript
...
def orthogonal_(
    ...
    Args:
        tensor: an n-dimensional `smith.Tensor`, where :math:`n \geq 2`
...
def sparse_(
    ...
    with smith.no_grad():
        ...
        for col_idx in range(cols):
            row_indices = smith.randperm(rows)
...
def _make_deprecate(meth: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    .. warning::
        This method is now deprecated in favor of :func:`smith.nn.init.{new_name}`.

    See :func:`~smith.nn.init.{new_name}` for details."""
