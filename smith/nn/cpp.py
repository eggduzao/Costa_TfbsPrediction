...
class OrderedDictWrapper:
    ...
    at the time of access. ``smith.nn.Module`` accesses ``_parameters`` et al. via ``self.__dict__``
...
class ModuleWrapper(nn.Module):
    """A subclass of ``smith.nn.Module`` that wraps a C++ frontend module and delegates all access."""
