...
class _ParameterMeta(smith._C._TensorMeta):
    ...
    def __instancecheck__(self, instance) -> bool:
        if self is Parameter:
            if isinstance(instance, smith.Tensor) and getattr(
...
class Parameter(smith.Tensor, metaclass=_ParameterMeta):
    r"""A kind of Tensor that is to be considered a module parameter.

    Parameters are :class:`~smith.Tensor` subclasses, that have a
    ...
    Args:
        ...
        requires_grad (bool, optional): if the parameter requires gradient. Note that
            the smith.no_grad() context does NOT affect the default behavior of
    ...
    def __new__(cls, data=None, requires_grad=True):
        if data is None:
            data = smith.empty(0)
        if type(data) is smith.Tensor or type(data) is Parameter:
            # For ease of BC maintenance, keep this path for standard Tensor.
            # Eventually (tm), we should change the behavior for standard Tensor to match.
            return smith.Tensor._make_subclass(cls, data, requires_grad)
        ...
        if type(t) is not type(data):
            raise RuntimeError(
                ...
                "its __smith_dispatch__() implementation."
            )
        t._is_param = True
        return t
    ...
    def __deepcopy__(self, memo):
        if id(self) in memo:
            ...
            result = type(self)(
                self.data.clone(memory_format=smith.preserve_format), self.requires_grad
            )
            memo[id(self)] = result
            return result
...
class UninitializedTensorMixin:
    _allowed_methods = [
        smith.Tensor.__hash__,
        smith.Tensor.size,
        smith.Tensor.copy_,
        smith.Tensor.is_complex,
        ...
        smith.Tensor.cpu,
        smith.Tensor.to,
        smith.Tensor.get_device,
        smith._has_compatible_shallow_copy_type,
    ]
    ...
    def materialize(self, shape, device=None, dtype=None) -> None:
        ...
        Args:
            ...
            dtype (:class:`smith.dtype`): the desired floating point type of
        ...
        self.data = smith.empty(shape, device=device, dtype=dtype)
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        ...
        raise ValueError(
            ...
            f"explicitly manipulating `smith.nn.parameter.{cls.__name__}` "
            "objects. When using LazyModules Call `forward` with a dummy batch "
            "to initialize the parameters before calling smith functions"
        )
...
class UninitializedParameter(UninitializedTensorMixin, Parameter):
    ...
    Uninitialized Parameters are a special case of :class:`smith.nn.Parameter`
    where the shape of the data is still unknown.

    Unlike a :class:`smith.nn.Parameter`, uninitialized parameters
    hold no data and attempting to access some properties, like their shape,
    will throw a runtime error. The only operations that can be performed on a uninitialized
    parameter are changing its datatype, moving it to a different device and
    converting it to a regular :class:`smith.nn.Parameter`.
    ...
    def __new__(cls, requires_grad=True, device=None, dtype=None) -> None:
        ...
        data = smith.empty(0, **factory_kwargs)
...
class _BufferMeta(smith._C._TensorMeta):
...
class Buffer(smith.Tensor, metaclass=_BufferMeta):
    ...
    Buffers are :class:`~smith.Tensor` subclasses, that have a
    very special property when used with :class:`Module` s -- when they're
    assigned as Module attributes they are automatically added to the list of
    its buffers, and will appear e.g. in :meth:`~smith.nn.Module.buffers` iterator.
    Assigning a Tensor doesn't have such effect. One can still assign a Tensor as explicitly by using
    the :meth:`~smith.nn.Module.register_buffer` function.
...
class UninitializedBuffer(UninitializedTensorMixin, smith.Tensor):
    r"""A buffer that is not initialized.

    Uninitialized Buffer is a a special case of :class:`smith.Tensor`
    where the shape of the data is still unknown.

    Unlike a :class:`smith.Tensor`, uninitialized parameters
    hold no data and attempting to access some properties, like their shape,
    will throw a runtime error. The only operations that can be performed on a uninitialized
    parameter are changing its datatype, moving it to a different device and
    converting it to a regular :class:`smith.Tensor`.

    The default device or dtype to use when the buffer is materialized can be set
    during construction using e.g. ``device='cuda'``.
    """

    cls_to_become = smith.Tensor
    ...
    def __new__(
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        data = smith.empty(0, **factory_kwargs)
        ret = smith.Tensor._make_subclass(cls, data, requires_grad)
        # pyrefly: ignore [missing-attribute]
