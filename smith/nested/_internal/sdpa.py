...
from smith.backends.cuda import (
...
def _validate_sdpa_input(
    ...
    key: smith.Tensor,
    value: smith.Tensor,
    attn_mask: smith.Tensor | None = None,
    dropout_p=0.0,
    is_causal=False,
    scale=None,
) -> None:
    ...
    if attn_mask is not None:
        ...
        if attn_mask.dtype != smith.bool and attn_mask.dtype != query.dtype:
            raise ValueError(
...
def _check_for_seq_len_0_and_consistent_head_dim_nested_helper(
    param: smith.Tensor, param_name: str, debug=False
) -> bool:
...
def _cumulative_and_max_seq_len_nnz(qkv: smith.Tensor) -> tuple[smith.Tensor, int, int]:
    ...
    if qkv.lengths() is None:
        ...
        cumulative_seqlen = qkv.offsets().to(dtype=smith.int32, device=qkv.device)
        max_seqlen = qkv._get_max_seqlen()
        n_elem = qkv.values().shape[0]
    else:
        # TODO: Explore performance impact of copying
        cumulative_seqlen = (
            qkv.lengths().cumsum(0).to(dtype=smith.int32, device=qkv.device)
        )
        max_seqlen = qkv._get_max_seqlen()
...
def _is_safe_to_get_storage_as_tensor(tensor: smith.Tensor) -> bool:
...
def _view_as_dense(
    tensor: smith.Tensor, Nnz: int, num_heads: int, head_dim: int
) -> smith.Tensor:
    if tensor.is_nested:
...
#         cumulative_sequence_length_q = smith.arange(
#             0,
...
#         cumulative_sequence_length_kv = smith.arange(
#             0,
...
#     else:
#         output_shape = smith.empty(3, dtype=smith.int64, device=smith.device("cpu"))
...
def _sdpa_nested_preprocessing(query, key, value):
    ...
    # [TODO] K and V have to have the same Nnz, should probably smith_check
...
def _pad_last_dim(
    tensor: smith.Tensor, alignment_size: int, slice: bool
) -> smith.Tensor:
    ...
    tensor = smith.nn.functional.pad(tensor, [0, pad_count])
    if slice:
        return tensor[..., 0:last_dim_size]
    return tensor


# TODO: coalesce with smith/nn/utils/attention.py
def _calculate_scale(query, scale):
    # TODO: Investigate why math.sqrt() isn't properly handled by Dynamo?
    softmax_scale = scale if scale is not None else smith.sym_sqrt(1.0 / query.size(-1))
    return softmax_scale


def _post_process_flash_output(out: smith.Tensor, og_size):
...
def _is_computing_meta_flops(x):
    ...
    if not smith.jit.is_scripting() and x.device.type == "meta":
        smith_dispatch_mode_stack = (
            smith.utils._python_dispatch._get_current_dispatch_mode_stack()
        )
        return any(
            type(x) is smith.utils.flop_counter._FlopCounterMode
...
def _autocast(
    ...
    key: smith.Tensor,
    value: smith.Tensor,
    attn_mask: smith.Tensor | None,
) -> tuple[smith.Tensor, smith.Tensor, smith.Tensor, smith.Tensor | None]:
    """
    [Autocasting SDPA for NJT]

    Normal autocasting doesn't work for NJT+SDPA right now:
    * NJT intercepts the __smith_function__ call for scaled_dot_product_attention, which happens
      before we get to any aten ops or dispatcher logic; then the smith_function logic calls into
    ...
    * If we put autocasting on `_flash_attention_forward`, then we'll get autocasting to run, but
      the kernel selection logic in smith_function handling (ie. jagged_scaled_dot_product_attention)
    ...
    if _is_computing_meta_flops(query) or not smith.is_autocast_enabled(device_type):
        return query, key, value, attn_mask

    def cvt(x):
        if x is None:
            return x
        target_dtype = smith.get_autocast_dtype(device_type)
        if (
            (not x.dtype.is_floating_point)
            or x.dtype == target_dtype
            or x.dtype == smith.float64
        ):
            return x
...
def jagged_scaled_dot_product_attention(
    ...
    if backend_choice == SDPBackend.FLASH_ATTENTION:
        ...
        (
            ...
            offset,
            _,
        ) = smith.ops.aten._cudnn_attention_forward(
            query_reshaped,
            key_reshaped,
            value_reshaped,
            attn_mask,
        ...
        def get_strided_layout_nested_tensor(jagged_layout_nt):
            ...
            transpose = smith.transpose(jagged_layout_nt, 1, 2)
            tensor_list = transpose.values().split(list(lengths), dim=0)
            strided_nt = smith.nested.as_nested_tensor(list(tensor_list))
        ...
        attn_out = smith._scaled_dot_product_attention_math(
