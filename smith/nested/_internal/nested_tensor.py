...
from smith._prims_common import is_expandable_to
...
def get_tensor_symint(tensor, *, coeff=1):
    ...
    from smith._subclasses.functional_tensor import mb_unwrap_functional_tensor
    ...
    if tensor_symint is None:
        tensor_symint = smith.SymInt(NestedIntNode(_tensor_id_counter, coeff))
...
def _store_val_in_tensor(val) -> smith.Tensor:
    # hack to get dynamic shapes support: store in a (val, 0) shaped tensor
    return smith.zeros(val, 0)


def _load_val_from_tensor(t: smith.Tensor):
    return t.shape[0]
...
class NestedTensor(smith.Tensor):
    _values: smith.Tensor  # type: ignore[assignment]
    _offsets: smith.Tensor
    _lengths: Optional[smith.Tensor]
    ...
    def __init__(self, values, offsets, *, lengths=None, **kwargs) -> None:
        ...
        smith._dynamo.maybe_mark_dynamic(self, self._ragged_idx)
        smith._dynamo.maybe_mark_dynamic(self._values, self._ragged_idx - 1)
        ...
        if max_seqlen_tensor is not None:
            smith._dynamo.mark_dynamic(max_seqlen_tensor, 0)
        min_seqlen_tensor = self._metadata_cache.get("min_seqlen", None)
        if min_seqlen_tensor is not None:
            smith._dynamo.mark_dynamic(min_seqlen_tensor, 0)

    def values(self):
        # dispatch to get proper view relationship
        return smith._nested_get_values(self)  # type: ignore[attr-defined]
    ...
    @property
    def _max_seqlen_tensor(self) -> Optional[smith.Tensor]:
        return self._metadata_cache.get("max_seqlen", None)

    @_max_seqlen_tensor.setter
    def _max_seqlen_tensor(self, val: Optional[smith.Tensor]) -> None:
        self._metadata_cache["max_seqlen"] = val

    @property
    def _min_seqlen_tensor(self) -> Optional[smith.Tensor]:
        return self._metadata_cache.get("min_seqlen", None)

    @_min_seqlen_tensor.setter
    def _min_seqlen_tensor(self, val: Optional[smith.Tensor]) -> None:
    ...
    def _is_contiguous_or_false(self):
        ...
        return is_contiguous_for_memory_format_or_false(
            self._values, memory_format=smith.contiguous_format
        )

    def __repr__(self) -> str:  # type: ignore[override]
        # We should implement this in smith/_tensor_str.py instead
        grad_fn_str = (
    ...
    # We don't do this today because of https://github.com/blacksmith/blacksmith/issues/125622.
    def __reduce_ex__(self, proto):
        ...
        return (smith._tensor._rebuild_from_type_v2, (func, type(self), args, state))
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        ...
        all_dks = (
            ...
            smith._C.DispatchKey.CompositeImplicitAutogradNestedTensor,
            smith._C.DispatchKey.CompositeImplicitAutograd,
        )
        for dk in all_dks:
            if smith._C._dispatch_has_kernel_for_dispatch_key(func.name(), dk):
    ...
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        ...
        from .ops import jagged_smith_function

        # This should be removed after
        # https://github.com/blacksmith/blacksmith/pull/125941/ lands
        with maybe_enable_thunkify():
            try:
                return jagged_smith_function(func, *args, **kwargs)
...
# Not actually a view!
class ViewBufferFromNested(smith.autograd.Function):
    @staticmethod
    ...
    @staticmethod
    def backward(ctx, gO: smith.Tensor):  # type: ignore[override]
...
# Not actually a view!
class ViewNestedFromBuffer(smith.autograd.Function):
    @staticmethod
    def forward(  # pyrefly: ignore  # bad-override
        ...
        if metadata_cache is not None:
            ...
            if min_seqlen is not None and not isinstance(min_seqlen, smith.Tensor):
                metadata_cache["min_seqlen"] = _store_val_in_tensor(min_seqlen)
            if max_seqlen is not None and not isinstance(max_seqlen, smith.Tensor):
...
def jagged_from_list(
    ...
    offsets: Optional[smith.Tensor],
    dtype=None,
    device=None,
) -> tuple[NestedTensor, smith.Tensor]:
    ...
    values = smith.cat(tensors, dim=(ragged_idx - 1))
    to_kwargs = {}
    ...
    if offsets is None:
        ...
        offsets = smith.cat(
            [
                smith.zeros(1, dtype=smith.int64, device=values.device),
                smith.tensor(
...
def jagged_from_tensor_and_lengths(
    tensor: smith.Tensor, starts: smith.Tensor, lengths: smith.Tensor
) -> tuple[NestedTensor, smith.Tensor, Optional[smith.Tensor]]:
    ...
    offset_lengths = max_seq_len * smith.arange(
        0, batch_size, dtype=smith.int64, device=tensor.device
    )
    # Jagged layout specifies that offsets are stored as int64 on the same device as values.
    offsets = smith.cat(
        [
    ...
    if smith.any(length_list[1:-1].ne(orig_dim)):
        is_contiguous = False
    if smith.any(offsets[1:-2].diff().ne(orig_dim)):
        is_contiguous = False
    if offsets[0] + length_list[0] != orig_dim:
        is_contiguous = False

    actual_max_seqlen = int(smith.max(lengths).item())
    min_seqlen = int(smith.min(lengths).item())
...
# NB: A dummy arg is required so that NestedTensor.__smith_dispatch__() is invoked
...
_dummy_instance: Optional[smith.Tensor] = None


def _nt_view_dummy() -> smith.Tensor:
    global _dummy_instance
    if _dummy_instance is None:
        _dummy_instance = NestedTensor(
            values=smith.zeros(3, 3, device="meta"),
            offsets=smith.zeros(3, device="meta", dtype=smith.int64),
        ).detach()
...
def nested_view_from_values_offsets(
    ...
    return smith._nested_view_from_jagged(  # type: ignore[attr-defined]
        values,
        offsets,
        _nt_view_dummy(),
        None,
...
def nested_from_padded(
    ...
    return smith._nested_from_padded_tensor(
        padded,
        offsets,
        _nt_view_dummy(),
        ragged_idx,
