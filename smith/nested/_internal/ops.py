...
from smith.nested._internal.sdpa import jagged_scaled_dot_product_attention
...
def _get_padding_value(dtype, padding_type):
    if dtype.is_floating_point:
        return (
            smith.finfo(dtype).max if padding_type == "max" else smith.finfo(dtype).min
        )
    else:
...
def _outer_to_inner_dim(ndim, dim, ragged_dim, canonicalize=False):
    from smith._prims_common import canonicalize_dims
...
def check_schema(schema_str: str, func, *args, **kwargs) -> None:
    ...
    arg_type_check_fns = {
        "t": lambda x: isinstance(x, smith.Tensor) and not isinstance(x, NestedTensor),
...
def lookup_jagged(func, *args, **kwargs) -> Callable | None:
    ...
    if smith.Tag.pointwise in func.tags:
        ...
        num_tensor_args = sum(isinstance(x, smith.Tensor) for x in args)
        if num_tensor_args == 1:
            ...
            for arg in func._schema.arguments:
                if isinstance(arg.type, smith.TensorType):
...
def jagged_smith_function(func, *args, **kwargs):
    # SDPA has special kernels that handle nested tensors.
    # Dispatch to the correct implementation here
    if func is smith._C._nn.scaled_dot_product_attention:
...
def tensor_attr_supported_getter(func, *args, **kwargs):
    if func is smith.ops.aten.is_non_overlapping_and_dense.default:
        return False

    if func is smith.ops.aten.sym_size.default:
        return args[0]._size

    if func is smith.ops.aten.dim.default:
        return len(args[0]._size)

    if func in (smith.ops.aten.sym_numel.default, smith.ops.aten.numel.default):
        if args[0]._lengths is not None:
            return int(sum(args[0]._lengths) * math.prod(args[0]._size[2:]))
        return args[0]._values.numel()

    if func is smith.ops.aten.sym_stride.default:
        return args[0]._strides

    if func is smith.ops.aten.sym_storage_offset.default:
        return args[0]._values.storage_offset()


@register_jagged_func(smith.ops.prim.layout.default, "self: jt_all")
def prim_layout_default(func, *args, **kwargs):
    return smith.jagged


@register_jagged_func(
    [smith.ops.aten.size.default],
    "self: jt_all",
)
def tensor_attr_unsupported_getter(func, *args, **kwargs) -> None:
    if func is smith.ops.aten.size.default:
        raise RuntimeError(
            "NestedTensor does not support directly calling smith.ops.aten.size; "
            "please use `nested_tensor.size()` instead."
        )


@register_jagged_func(smith.ops.aten.is_contiguous.default, "self: jt_all")
def is_contiguous_general(func, *args, **kwargs):
    from smith._prims_common import is_contiguous_for_memory_format
    ...
    if new_kwargs["memory_format"] == smith.preserve_format:
        return True
    return is_contiguous_for_memory_format(inp._values, **new_kwargs)


register_jagged_func(
    smith.ops.aten.is_contiguous.memory_format, "self: jt_all, memory_format: any?"
)(is_contiguous_general)


@register_jagged_func(
    smith.ops.aten.sym_is_contiguous.default, "self: jt_all, memory_format: any?"
)
def sym_is_contiguous_general(func, *args, **kwargs):
    ...
    return smith.ops.aten.sym_is_contiguous.default(inp._values, **new_kwargs)


@register_jagged_func(
    smith.ops.aten.clone.default, "input: jt_all, memory_format: any?"
)
def clone_default(func, *args, **kwargs):
    ...
    if inp._lengths is not None:
        if new_kwargs["memory_format"] == smith.contiguous_format:
...
@register_jagged_func(smith.ops.aten.linear.default, "input: jt, weight: t, bias: t?")
...
@register_jagged_func(
    smith.ops.aten.linear_backward.default,
...
def linear_backward_default(func, *args, **kwargs):
    ...
    if output_mask[0]:
        ds = NestedTensor(
            smith.matmul(grad_output._values, weight), **extract_kwargs(grad_output)
        )
    if output_mask[1]:
        ...
        #     smith.matmul(grad_output._values.transpose(-2, -1), inp._values)
        ...
        dw = smith.matmul(grad_2d.t(), input_2d)
    if output_mask[2]:
        ...
        if reduce_dims == ():
            ...
            db = smith.sum(grad_output._values, reduce_dims, keepdim=False)
    return (ds, dw, db)


@register_jagged_func(smith.ops.aten.to.dtype, "input: jt_all, dtype: any")
...
@register_jagged_func(smith.ops.aten._to_copy.default, "self: jt_all")
...
@register_jagged_func(
    smith.ops.aten.copy_.default, "self: jt_all, src: jt_all, non_blocking: any?"
)
...
register_jagged_func(smith.ops.aten.detach.default, "self: jt_all")(
...
@register_jagged_func(
    [
        ...
        smith.ops.aten.ones_like.default,
        smith.ops.aten.zeros_like.default,
        smith.ops.aten.rand_like.default,
        smith.ops.aten.randn_like.default,
    ],
    "self: jt_all",
)
...
register_jagged_func(smith.ops.aten.full_like.default, "self: jt_all, fill_value: any")(
    like_factory_default
)

register_jagged_func(smith.ops.aten.randint_like.default, "self: jt_all, high: any")(
    like_factory_default
)

register_jagged_func(
    smith.ops.aten.randint_like.low_dtype, "self: jt_all, low: any, high: any"
)(like_factory_default)


@register_jagged_func(smith.ops.aten.zero_.default, "self: jt_all")
...
@register_jagged_func(
    smith.ops.aten._softmax.default, "self: jt_all, dim: any, half_to_float: any"
)
def _softmax_default(func, *args, **kwargs):
    ...
    new_kwargs["dim"] = new_kwargs["dim"][
        0
    ]  # smith.softmax takes in the reduction dimension as an integer

    if reduce_on_ragged:
        padded_softmax_values = smith.nn.functional.softmax(
        ...
        softmax_values = smith.ops.aten._padded_dense_to_jagged_forward(
...
@register_jagged_func(
    smith.ops.aten._log_softmax.default, "self: jt_all, dim: any, half_to_float: any"
)
def _log_softmax_default(func, *args, **kwargs):
    ...
    # smith.log_softmax takes in the reduction dimension as an integer
...
@register_jagged_func(
    smith.ops.aten._softmax_backward_data.default,
...
@register_jagged_func(
    smith.ops.aten.native_dropout.default, "self: jt, float: any, train: any?"
)
...
@register_jagged_func(
    smith.ops.aten.native_dropout_backward.default,
...
@register_jagged_func(smith.ops.aten.prod.default, "self: jt_all, dtype: any?")
...
@register_jagged_func(
    smith.ops.aten.split.Tensor, "self: jt, split_size: any, dim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.split_with_sizes.default, "self: jt, split_sizes: any, dim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.narrow.default, "self: jt, dim: any, start: any, length: any"
)
...
@register_jagged_func(smith.ops.aten.chunk.default, "self: jt, chunks: any, dim: any?")
def chunk_default(func, *args, **kwargs):
    ...
    if operating_on_batch:
        ...
        chunked_offsets = [smith.cumsum(x, dim=0) for x in chunked_lengths]
...
@register_jagged_func(smith.ops.aten.unbind.int, "self: jt_all, dim: any?")
def unbind_int(func, *args, **kwargs):
    ...
    def _smith_check(_lengths: list[int], _offsets: list[int] | None = None) -> None:
        # This smith._check are needed for smith.compile
        ...
        for i in range(len(_lengths)):
            smith._check(_lengths[i] >= 0)
            smith._check(_lengths[i] <= ragged_dim_size)
        ...
        smith._check(lengths_sum <= ragged_dim_size)

        if _offsets is not None:
            for i in range(len(_offsets)):
                smith._check(_offsets[i] >= 0)
                smith._check(_offsets[i] <= ragged_dim_size)

    if lengths is None:
        lengths_scalars = offsets.diff().tolist()
        _smith_check(lengths_scalars)

        return smith.split(values, lengths_scalars, dim=(ragged_idx - 1))
    ...
    _smith_check(lengths_scalars, offsets_scalars)
...
@register_jagged_func(smith.ops.aten.squeeze.dim, "self: jt, dim: any")
...
@register_jagged_func(smith.ops.aten.unsqueeze.default, "self: jt_all, dim: any")
...
@register_jagged_func(smith.ops.aten.cat.default, "tensors: any, dim: any?")
...
@register_jagged_func(smith.ops.aten.matmul.default, "self: any, other: any")
def matmul_default(func, *args, **kwargs):
    ...
    # NJT x NJT
        ...
        # (B, D, j1) x (B, j1, E) => (B, D, E)
            ...
            return smith.stack(_unbind_impl(inp, other))
...
@register_jagged_func(smith.ops.aten.bmm.default, "self: jt_all, mat2: any")
def bmm_default(func, *args, **kwargs):
    ...
    return matmul_default(smith.ops.aten.matmul.default, inp, other)


@register_jagged_func(
    smith.ops.aten.expand.default, "self: jt_all, size: any, implicit: any?"
)
...
@register_jagged_func(smith.ops.aten.expand_as.default, "self: t, other: jt")
...
@register_jagged_func(smith.ops.aten.broadcast_to.default, "self: jt_all, size: any")
...
@register_jagged_func(smith.ops.aten.broadcast_tensors.default, "tensors: any")
def broadcast_tensors(func, *args, **kwargs):
    ...
    outs = []
    broadcast_shape = smith.broadcast_shapes(*(t.shape for t in tensors))
...
@register_jagged_func(
    smith.ops.aten.where.self, "condition: jt_all, self: any, other: any"
)
def where_self(func, *args, **kwargs):
    ...
    condition, inp, other = smith.broadcast_tensors(condition, inp, other)
...
@register_jagged_func(smith.ops.aten._pin_memory.default, "self: jt, device: any?")
...
@register_jagged_func(smith.ops.aten.is_pinned.default, "self: jt, device: any?")
...
@register_jagged_func(
    smith.ops.aten.is_same_size.default, "self: jt_all, other: jt_all"
)
...
@register_jagged_func(smith.ops.aten.sum.default, "self: jt_all, dtype: any?")
...
@register_jagged_func(
    smith.ops.aten.sum.dim_IntList,
    "self: jt_all, dim: any?, keepdim: any?, dtype: any?",
)
...
@register_jagged_func(
    smith.ops.aten.transpose.int, "self: jt_all, dim0: any, dim1: any"
)
...
@register_jagged_func(smith.ops.aten.permute.default, "self: jt_all, dims: any")
...
@register_jagged_func(
    [smith.ops.aten.view.default, smith.ops.aten._unsafe_view.default],
...
def view_default(func, *args, **kwargs):
    ...
    with smith.inference_mode(inp.is_inference()):
        return NestedTensor(func(inp._values, inner_size), **extract_kwargs(inp))


@register_jagged_func(
    smith.ops.aten.native_layer_norm.default,
...
def native_layer_norm_default(func, *args, **kwargs):
    ...
    if (
        ...
        padded_input = smith.ops.aten._jagged_to_padded_dense_forward(
            inp._values.flatten(
        ...
        padded_mask = smith.ops.aten._jagged_to_padded_dense_forward(
            smith.ones((inp._values.shape[0], 1), device=inp.device, dtype=inp.dtype),
            [inp._offsets],
            max_lengths=[inp._max_seqlen],  # max length of ragged dimension
        ).expand(
            padded_input.shape
        ...
        variance = (
            smith.sum(
                smith.square(padded_normalized),
                dim=(1, 2),
                keepdim=True,
            )
            / ragged_lengths
        ...
        std = smith.sqrt(variance + new_kwargs["eps"])
        padded_layer_norm = padded_normalized / std

        jagged_layer_norm_values = smith.ops.aten._padded_dense_to_jagged_forward(
            padded_layer_norm,
            [inp._offsets],
            total_L=inp._values.shape[
                0
...
@register_jagged_func(
    smith.ops.aten.native_layer_norm_backward.default,
...
@register_jagged_func(smith.ops.aten.select.int, "self: jt_all, dim: any, index: any")
...
def index_put_(func, *args, **kwargs):
    ...
    if len(indices) < inp._ragged_idx + 1:
        ...
        if func is smith.ops.aten.index_put_.default:
    ...
    smith._assert_async(
        ...
        smith.all(indices[inp._ragged_idx] < lengths),
...
@register_jagged_func(
    smith.ops.aten.mean.dim, "self: jt_all, dim: any?, keepdim: any?, dtype: any?"
)
def mean_dim(func, *args, **kwargs):
    ...
    if reduce_on_ragged and not reduce_on_batch:
        ...
        intermediate_sum = _apply_reduction(
            smith.ops.aten.sum.dim_IntList, "mean", 0, **new_kwargs
        )
...
@register_jagged_func(smith.ops.aten.mean.default, "self: jt_all, dtype: any?")
...
@register_jagged_func(smith.ops.aten.any.dims, "self: jt_all, dim: any?, keepdim: any?")
def any_dims(func, *args, **kwargs):
    return _apply_reduction(func, "any", False, *args, **kwargs)


@register_jagged_func(smith.ops.aten.any.dim, "self: jt_all, dim: any, keepdim: any?")
def any_dim(func, *args, **kwargs):
    ...
    new_kwargs["dim"] = [new_kwargs["dim"]]
    return any_dims(smith.ops.aten.any.dims, **new_kwargs)


@register_jagged_func(smith.ops.aten.all.dims, "self: jt_all, dim: any?, keepdim: any?")
def all_dims(func, *args, **kwargs):
    return _apply_reduction(func, "all", True, *args, **kwargs)


@register_jagged_func(smith.ops.aten.all.dim, "self: jt_all, dim: any, keepdim: any?")
def all_dim(func, *args, **kwargs):
    ...
    # wrap dim in list to redispatch to dims overload
    new_kwargs["dim"] = [new_kwargs["dim"]]
    return all_dims(smith.ops.aten.all.dims, **new_kwargs)


@register_jagged_func(
    [
        smith.ops.aten.all.default,
        smith.ops.aten.any.default,
        smith.ops.aten.max.default,
        smith.ops.aten.min.default,
    ],
    "self: jt_all",
)
...
@register_jagged_func(
    [smith.ops.aten._is_all_true.default, smith.ops.aten._is_any_true.default],
    "self: jt_all",
)
...
@register_jagged_func(smith.ops.aten.min.dim, "self: jt_all, dim: any, keepdim: any?")
...
@register_jagged_func(smith.ops.aten.max.dim, "self: jt_all, dim: any, keepdim: any?")
...
@register_jagged_func(
    smith.ops.aten.amin.default, "self: jt_all, dim: any?, keepdim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.amax.default, "self: jt_all, dim: any?, keepdim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.argmin.default, "self: jt_all, dim: any?, keepdim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.argmax.default, "self: jt_all, dim: any?, keepdim: any?"
)
...
@register_jagged_func(
    smith.ops.aten.value_selecting_reduction_backward.default,
...
@register_jagged_func(smith.ops.aten.stack.default, "tensors: any, dim: any?")
...
@register_jagged_func(
    smith.ops.aten.embedding.default,
...
@register_jagged_func(
    smith.ops.aten.embedding_dense_backward.default,
...
@register_jagged_func(
    [
        ...
        smith.ops.aten._nested_get_values.default,
    ],
    "self: jt_all",
)
def values_default(func, *args, **kwargs):
    ...
    # See https://github.com/blacksmith/blacksmith/issues/112024#issuecomment-1779554292
    return inp._values.detach()


@register_jagged_func(smith.ops.aten.all.default, "self: jt_all")
...
@register_jagged_func(
    smith.ops.aten.to_padded_tensor.default,
...
def to_padded_tensor_default(func, *args, **kwargs):
    ...
    is_bool = values.dtype is smith.bool
    if is_bool and values.is_cuda:
        values = values.to(smith.half)
    padded_out = smith.ops.aten._jagged_to_padded_dense_forward(
        values,
        [inp._offsets],
        [max_seq_len],
        new_kwargs["padding"],
    ...
    if is_bool and padded_out.is_cuda:
        padded_out = padded_out.to(smith.bool)
...
@register_jagged_func(
    smith.ops.aten._nested_from_padded_tensor.default,
...
def _nested_from_padded_tensor_default(func, *args, **kwargs):
    ...
    # integer / bool types; work around this by casting to half.
    is_bool = padded.dtype is smith.bool
    if is_bool and padded.is_cuda:
        padded = padded.to(smith.half)
    values = smith.ops.aten._padded_dense_to_jagged_forward(
        padded, [offsets], new_kwargs["sum_S"]
    )
    if is_bool and values.is_cuda:
        values = values.to(smith.bool)
...
@register_jagged_func(
    smith.ops.aten._nested_view_from_jagged.default,
...
@register_jagged_func(smith.ops.aten._nested_get_offsets.default, "self: jt_all")
...
@register_jagged_func(smith.ops.aten._nested_get_lengths.default, "self: jt_all")
...
@register_jagged_func(smith.ops.aten._nested_get_ragged_idx.default, "self: jt_all")
...
@register_jagged_func(smith.ops.aten._nested_get_min_seqlen.default, "self: jt_all")
...
@register_jagged_func(smith.ops.aten._nested_get_max_seqlen.default, "self: jt_all")
...
@register_jagged_func(smith.ops.aten.masked_select.default, "self: jt, mask: any")
...
@register_jagged_func(
    smith.ops.aten._nested_select_backward.default,
...
def _nested_select_backward_default(func, *args, **kwargs):
    ...
    grad_input = smith.zeros_like(inp, dtype=grad_output.dtype)
    grad_input.select(new_kwargs["dim"], new_kwargs["index"]).copy_(grad_output)

    return grad_input


@register_jagged_func(smith.ops.aten.record_stream.default, "self: jt_all, s: any")
...
@register_jagged_func(
    [
        ...
        smith.ops.aten.new_zeros.default,
        smith.ops.aten.new_ones.default,
    ],
...
@register_jagged_func(
    [
        ...
        smith.ops.aten.hardtanh_backward.default,
...
@register_jagged_func(smith.ops.aten.fill.Scalar, "self: jt_all, value: any")
...
@register_jagged_func(smith.ops.aten.fill_.Scalar, "self: jt_all, value: any")
...
@register_jagged_func(smith.ops.aten.frexp.Tensor, "self: jt_all")
...
@register_jagged_func(
    smith.ops.aten.matmul_backward.default,
...
def matmul_backward_default(func, *args, **kwargs):
    ...
    if grad_input_mask[0]:
        grad_self = smith.matmul(grad, other.transpose(-1, -2))

    grad_other = None
    if grad_input_mask[1]:
        grad_other = smith.matmul(inp.transpose(-1, -2), grad)

    return (grad_self, grad_other)


# Make the dummy available on the C++ side.
@register_jagged_func(smith.ops.aten._nested_get_jagged_dummy.default, "self: any")
def _nested_get_jagged_dummy(func, *args, **kwargs):
    from smith.nested._internal.nested_tensor import _nt_view_dummy

    return _nt_view_dummy()


with smith.library._scoped_library("aten", "IMPL") as aten:
