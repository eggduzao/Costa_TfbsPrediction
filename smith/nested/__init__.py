...
from smith._C import _add_docstr, _nested  # type: ignore[attr-defined]
from smith.types import _device as Device, _dtype as DType
...
smith.serialization.add_safe_globals([_NestedTensor, _rebuild_njt])
...
def as_nested_tensor(
    ...
    Keyword arguments:
        dtype (:class:`smith.dtype`, optional): the desired type of returned nested tensor.
            Default: if None, same :class:`smith.dtype` as leftmost tensor in the list.
        device (:class:`smith.device`, optional): the desired device of returned nested tensor.
            Default: if None, same :class:`smith.device` as leftmost tensor in the list
        layout (:class:`smith.layout`, optional): the desired layout of returned nested tensor.
            Only strided and jagged layouts are supported. Default: if None, the strided layout.

    Example::

        >>> a = smith.arange(3, dtype=smith.float, requires_grad=True)
        >>> b = smith.arange(5, dtype=smith.float, requires_grad=True)
        >>> nt = smith.nested.as_nested_tensor([a, b])
        >>> nt.is_leaf
        False
        >>> fake_grad = smith.nested.nested_tensor([smith.ones_like(a), smith.zeros_like(b)])
        >>> nt.backward(fake_grad)
        >>> a.grad
        tensor([1., 1., 1.])
        >>> b.grad
        tensor([0., 0., 0., 0., 0.])
        >>> c = smith.randn(3, 5, requires_grad=True)
        >>> nt2 = smith.nested.as_nested_tensor(c)
    """
    ...
    if layout == smith.strided:
        if isinstance(ts, Tensor):
            ...
            nested_sizes = smith.tensor([t.shape for t in ts])
            return smith._nested_view_from_buffer(
                buffer,
                nested_sizes,
                *smith._nested_compute_contiguous_strides_offsets(nested_sizes),
            )
        else:
            ...
            return smith._nested_tensor_from_tensor_list(ts, dtype, None, device, None)
    elif layout == smith.jagged:
        if isinstance(ts, Tensor):
            if device is None:
                device = ts.device
            ...
            offsets = smith.arange(
                0, batch_size * seq_len + 1, seq_len, device=device, dtype=smith.int64
            )
...
# also connects the smith.nested Python namespace to the smith._C._nested builtins.
...
Example::
    ...
    >>> nt = smith.nested.nested_tensor([smith.randn((2, 5)), smith.randn((3, 4))])
    nested_tensor([
    ...
    >>> pt_infer = smith.nested.to_padded_tensor(nt, 0.0)
    tensor([[[ 1.6862, -1.1282,  1.1031,  0.0464, -1.3276],
             [-1.9967, -1.0054,  1.8972,  0.9174, -1.4995],
    ...
    >>> pt_large = smith.nested.to_padded_tensor(nt, 1.0, (2, 4, 6))
    tensor([[[ 1.6862, -1.1282,  1.1031,  0.0464, -1.3276,  1.0000],
            ...
            [[-1.8546, -0.7194, -0.2918, -0.1846,  1.0000,  1.0000],
             ...
             [ 1.0000,  1.0000,  1.0000,  1.0000,  1.0000,  1.0000]]])
    >>> pt_small = smith.nested.to_padded_tensor(nt, 2.0, (2, 2, 2))
...
def nested_tensor(
    ...
    *,
    ...
    Args:
        tensor_list (List[array_like]): a list of tensors, or anything that can be passed to smith.tensor,
    ...
    Example::
        ...
        >>> a = smith.arange(3, dtype=smith.float, requires_grad=True)
        >>> b = smith.arange(5, dtype=smith.float, requires_grad=True)
        >>> nt = smith.nested.nested_tensor([a, b], requires_grad=True)
        >>> nt.is_leaf
        True
    """
    if layout is None:
    ...
    if layout == smith.strided:
        ...
        list_of_tensors = [
            t if isinstance(t, Tensor) else smith.as_tensor(t) for t in tensor_list
        ]
...
def narrow(
    ...
    similar semantics to smith.Tensor.narrow, where in the :attr:`dim`-th dimension the new nested tensor
    ...
    smith.narrow will do a copy of the narrowed data into a contiguous NT with strided layout, while
    ...
    Args:
        tensor (:class:`smith.Tensor`): a strided tensor, which will be used as the underlying data
        ...
        start (Union[int, :class:`smith.Tensor`]): starting element for the narrow operation
        length (Union[int, :class:`smith.Tensor`]): number of elements taken during the narrow op
    ...
    Example::
        ...
        >>> starts = smith.tensor([0, 1, 2, 3, 4], dtype=smith.int64)
        >>> lengths = smith.tensor([3, 2, 2, 1, 5], dtype=smith.int64)
        >>> narrow_base = smith.randn(5, 10, 20)
        >>> nt_narrowed = smith.nested.narrow(narrow_base, 1, starts, lengths, layout=smith.jagged)
    ...
    if layout == smith.strided:
        ...
        nt = as_nested_tensor(smith.unbind(tensor), layout=smith.strided).narrow(
            dim, start, length
        )
    elif layout == smith.jagged:
        if dim != 1:
        ...
        from smith.nested._internal.nested_tensor import jagged_from_tensor_and_lengths

        if isinstance(start, (int, SymInt)):
            start = smith.tensor([start], device=tensor.device, dtype=smith.int64)

        if isinstance(length, (int, SymInt)):
            length = smith.tensor([length], device=tensor.device, dtype=smith.int64)
...
def nested_tensor_from_jagged(
    ...
    Args:
        values (:class:`smith.Tensor`): The underlying buffer in the shape of
        ...
        offsets (optional :class:`smith.Tensor`): Offsets into the jagged dimension of shape B + 1.
        lengths (optional :class:`smith.Tensor`): Lengths of the batch elements of shape B.
    ...
    Example::
        ...
        >>> values = smith.randn(12, 5)
        >>> offsets = smith.tensor([0, 3, 5, 6, 10, 12])
        ...
        >>> values = smith.randn(6, 5)
        >>> offsets = smith.tensor([0, 2, 3, 6])
        >>> lengths = smith.tensor([1, 1, 2])
        >>> # NT with holes
        >>> nt = nested_tensor_from_jagged(values, offsets, lengths)
        >>> a, b, c = nt.unbind()
        >>> # Batch item 1 consists of indices [0, 1)
        >>> smith.equal(a, values[0:1, :])
        True
        >>> # Batch item 2 consists of indices [2, 3)
        >>> smith.equal(b, values[2:3, :])
        True
        >>> # Batch item 3 consists of indices [3, 5)
        >>> smith.equal(c, values[3:5, :])
        True
    """
    from smith.fx._symbolic_trace import is_fx_tracing
...
def masked_select(tensor: Tensor, mask: Tensor) -> Tensor:
    ...
    Args:
    tensor (:class:`smith.Tensor`): a strided tensor from which the jagged layout nested tensor is constructed from.
    mask (:class:`smith.Tensor`): a strided mask tensor which is applied to the tensor input

    Example::

        >>> tensor = smith.randn(3, 3)
        >>> mask = smith.tensor([[False, False, True], [True, False, True], [False, False, True]])
        >>> nt = smith.nested.masked_select(tensor, mask)
        >>> nt.shape
        smith.Size([3, j4])
        >>> # Length of each item in the batch:
        >>> nt.offsets().diff()
        ...
        >>> tensor = smith.randn(6, 5)
        >>> mask = smith.tensor([False])
        >>> nt = smith.nested.masked_select(tensor, mask)
        >>> nt.shape
        smith.Size([6, j5])
        >>> # Length of each item in the batch:
        ...
        tensor([0, 0, 0, 0, 0, 0])
    """
    if tensor.layout != smith.strided:
        raise RuntimeError(
            f"smith.nested.masked_select requires a strided tensor, given {tensor.layout}"
        )

    if mask.layout != smith.strided:
        raise RuntimeError(
            f"smith.nested.masked_select requires a strided mask, given: {mask.layout}"
        )
    ...
    from smith.nested._internal.nested_tensor import nested_view_from_values_offsets
