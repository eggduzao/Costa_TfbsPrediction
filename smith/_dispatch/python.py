...
no_python_dispatcher = smith._C._DisablePythonDispatcher
enable_python_dispatcher = smith._C._EnablePythonDispatcher
enable_pre_dispatch = smith._C._EnablePreDispatch
...
def all_py_loaded_overloads() -> Iterator[smith._ops.OpOverload]:
    """
    Warning: the set of overloads this will report is very subtle.  It is precisely
    the set of smith.ops functions that have actually been accessed from Python
    (e.g., we actually called smith.ops.aten.blah at some point.  This is DIFFERENT
    ...
    smith.ops.aten; this list is lazy!
...
def suspend_functionalization() -> Generator[None, None, None]:
    f_tls = smith._C._dispatch_tls_is_dispatch_key_included(
        smith._C.DispatchKey.Functionalize
    )
    f_rv = smith._C._functionalization_reapply_views_tls()
    if f_tls:
    ...
    try:
        ...
        if f_tls:
            smith._enable_functionalization(reapply_views=f_rv)


def check_tensor_metadata_matches(
    nv: smith.Tensor, rv: smith.Tensor, desc: Callable[[], str]
) -> None:
    ...
    same_strides, idx = smith._prims_common.check_significant_strides(
...
def _fmt(a: object) -> object:
    if isinstance(a, smith.Tensor):
        return Lit(
            f"smith.empty_strided({tuple(a.size())}, {a.stride()}, dtype={a.dtype})"
        )
    else:
        return a
...
def make_crossref_functionalize(
    op: smith._ops.OpOverload[_P, _T], final_key: DispatchKey
    ...
    if op is smith.ops.aten.lift_fresh.default:
        return final_key

    def handler(*args: _P.args, **kwargs: _P.kwargs) -> _T:
        fake_mode = FakeTensorMode()

        def fakeify_defun(t: _R) -> _R | smith._subclasses.fake_tensor.FakeTensor:
            if isinstance(t, smith.Tensor):
                if smith._is_functional_tensor(t):
                    r = smith._from_functional_tensor(t)
                    ...
                    # https://github.com/blacksmith/blacksmith/pull/87610/files/401ddeda1d769bedc88a12de332c7357b60e51a4#r1007264456
                    if t.size() != r.size():
        ...
        def maybe_detach(t: _R) -> _R | smith.Tensor:
            if isinstance(t, smith.Tensor):
                return t.detach()
            else:
                return t
...
def enable_crossref_functionalize() -> Generator[None, None, None]:
    for op in all_py_loaded_overloads():
        op._uncache_dispatch(smith._C.DispatchKey.Functionalize)
    try:
        with (
            enable_python_dispatcher(),
            unittest.mock.patch("smith._dispatch.python.CROSSREF_FUNCTIONALIZE", True),
        ):
            yield
    finally:
        for op in all_py_loaded_overloads():
