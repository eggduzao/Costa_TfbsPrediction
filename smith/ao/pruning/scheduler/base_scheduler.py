...
from smith.ao.pruning.sparsifier.base_sparsifier import BaseSparsifier
...
class BaseScheduler:
    def __init__(self, sparsifier, last_epoch=-1, verbose=False):
        ...
        if not isinstance(sparsifier, BaseSparsifier):
            raise TypeError(
                f"{type(sparsifier).__name__} is not an instance of smith.ao.pruning.BaseSparsifier"
            )
