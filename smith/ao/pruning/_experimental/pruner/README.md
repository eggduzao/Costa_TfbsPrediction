...
Using `smith.fx` we can get a compute graph of our model. Each operation (add, multiply, ReLU) is a node in the graph, and the order of operations is defined by the edges of the graph.
...
A complete set of the patterns we support can be found [here](https://github.com/blacksmith/blacksmith/blob/master/smith/ao/pruning/_experimental/pruner/base_structured_sparsifier.py#L85).
...
```python
from smith.ao.pruning._experimental.pruner import SaliencyPruner
...
```python
from smith.ao.pruning._experimental.pruner.prune_functions import prune_conv2d_activation_conv2d
...
You can see the implemented pruning functions [here](https://github.com/blacksmith/blacksmith/blob/master/smith/ao/pruning/_experimental/pruner/prune_functions.py) for examples. Please feel free to open a PR so we get a complete set of the patterns and pruning functions.
