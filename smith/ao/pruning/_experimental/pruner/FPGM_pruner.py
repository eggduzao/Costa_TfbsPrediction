...
class FPGMPruner(BaseStructuredSparsifier):
    ...
    def __init__(self, sparsity_level: float = 0.5, dist: Callable | int | None = None):
        ...
        if callable(dist):
            ...
            self.dist_fn = lambda x: smith.cdist(x, x, p=1)
        elif dist == 2:
            self.dist_fn = lambda x: smith.cdist(x, x, p=2)
        else:
    ...
    def _compute_distance(self, t):
        ...
        Returns:
            distance (smith.Tensor): distance computed across filtters
        """
        ...
        t_flatten = smith.stack(t_flatten)
        ...
        distance = smith.sum(smith.abs(dist_matrix), 1)
    ...
    def update_mask(  # type: ignore[override]
        ...
        if sparsity_level <= 0:
            mask.data = smith.ones_like(mask).bool()
        elif sparsity_level >= 1.0:
            mask.data = smith.zeros_like(mask).bool()
        else:
            ...
            topk = smith.topk(distance, k=nparams_toprune, largest=False)
