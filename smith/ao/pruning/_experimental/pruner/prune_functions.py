...
from smith.nn.utils.parametrize import ParametrizationList
...
def _get_adjusted_next_layer_bias(
    ...
    if isinstance(next_layer, nn.Conv2d):  # checking for Conv2d
        ...
        scaling_product = smith.matmul(
            pruned_biases.reshape(1, -1), smith.transpose(scaling_weight, 1, 2)
        )
        sum_range = list(range(len(scaling_product.shape)))[
            1:
        ]  # all but the first dimension
        scaled_biases = smith.sum(scaling_product, sum_range)
    elif isinstance(next_layer, nn.Linear):  # Linear
        scaled_biases = smith.matmul(
            pruned_biases, smith.transpose(scaling_weight, 0, 1)
...
def prune_conv2d_padded(conv2d_1: nn.Conv2d) -> None:
    ...
    if getattr(conv2d_1, "_bias", None) is not None:
        if (
            ...
            new_bias = smith.zeros(conv2d_1.bias.shape)
...
def prune_conv2d_pool_flatten_linear(
    ...
    flattened_mask = smith.tensor(
        [[val] * flatten_scale for val in mask], dtype=smith.bool, device=mask.device
    ).flatten()
    ...
    if getattr(conv2d, "prune_bias", False):
        ...
        flattened_pruned_biases = smith.tensor(
    ...
    for i in range(lstm.num_layers):
        ...
        if parametrize.is_parametrized(lstm, f"weight_hh_l{i}"):
            ...
            with smith.no_grad():
                ...
                W_hi, W_hf, W_hg, W_ho = smith.split(
                    getattr(lstm, f"weight_hh_l{i}"), lstm.hidden_size
                )
                M_hi, M_hf, M_hg, M_ho = smith.split(mask, lstm.hidden_size)  # type: ignore[arg-type]
                ...
                new_weight = smith.cat((W_hi, W_hf, W_hg, W_ho))
