...
from smith.ao.pruning.sparsifier.utils import fqn_to_module, module_to_fqn
...
class ActivationSparsifier:
    ...
    Args:
        ...
        aggregate_fn (Optional, Callable):
            ...
            The aggregate_fn should usually take 2 smith tensors and return the aggregated tensor.
            Example
    ...
    Example:
        ...
        >>> # Initialize mask_fn
        >>> def mask_fn(data):
        >>>     return smith.eye(data.shape).to(data.device)
        >>>
        >>>
    ...
    def _aggregate_hook(self, name):
        ...
        def hook(module, input) -> None:
            ...
            if features is None:
                ...
                if data is None:
                    data = smith.zeros_like(input_data)
                    self.state[name]["mask"] = smith.ones_like(input_data)
                ...
                for feature_idx in range(len(features)):
                    # each feature is either a list or scalar, convert it to smith tensor
                    feature_tensor = (
                        smith.Tensor([features[feature_idx]])
                        .long()
                        .to(input_data.device)
                    )
                    data_feature = smith.index_select(
                        input_data, feature_dim, feature_tensor
                    )
                    if data is None:
                        curr_data = smith.zeros_like(data_feature)
                        self.state[name]["mask"][feature_idx] = smith.ones_like(
                            data_feature
                        )
                    else:
    ...
    def get_mask(self, name: str | None = None, layer: nn.Module | None = None):
        ...
        The mask is
            - a smith tensor is features for that layer is None.
            - a list of smith tensors for each feature, otherwise
        ...
        def hook(module, input):
            ...
            if features is None:
                ...
                for feature_idx in range(len(features)):
                    ...
                    sparsified = (
                        smith.index_select(input_data, feature_dim, feature)
                        * mask[feature_idx]
                    )
