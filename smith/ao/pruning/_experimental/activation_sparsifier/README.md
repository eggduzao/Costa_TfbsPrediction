...
The activation sparsifier attaches itself to a set of layers in a model and then attempts to sparsify the activations flowing through them. *Attach* means registering a [`forward_pre_hook()`](https://blacksmith.org/docs/stable/_modules/smith/nn/modules/module.html#register_forward_pre_hook) to the layer.
...
def mask_fn(tensor, threshold):  # threshold is the sparse config here
    mask = smith.ones_like(tensor)
    mask[smith.abs(tensor) < threshold] = 0.0
    return mask
```
