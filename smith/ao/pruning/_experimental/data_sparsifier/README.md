...
class ImplementedDataSparsifier(BaseDataSparsifier):
    ...
    def update_mask(self, name, data, threshold):
        ...
        mask[smith.abs(data) < threshold] = 0.0
```
...
```
tensor1 = smith.randn(100, 100)
param1 = nn.Parameter(smith.randn(200, 32))
...
2. The mask should be modified in place.
    ...
    1. Change a portion of a mask: `mask[:10] = smith.zeros(10)`
    2. Use an inplace operator: `mask *= another_mask`
    3. Change the underlying data: `mask.data = smith.zeros_like(mask)`

    Non-inplace operations are not valid, and might lead to bugs. For example:

    1. Reassignment of a mask: `mask = smith.zeros_like(mask)`
