...
from smith.ao.pruning._experimental.data_scheduler.base_data_scheduler import (
    BaseDataScheduler,
)
from smith.ao.pruning._experimental.data_sparsifier.base_data_sparsifier import (
    SUPPORTED_TYPES,
)
from smith.ao.pruning._experimental.data_sparsifier.data_norm_sparsifier import (
    DataNormSparsifier,
)
from smith.ao.pruning._experimental.data_sparsifier.lightning.callbacks._data_sparstity_utils import (
    _get_valid_name,
)
from smith.ao.pruning._experimental.data_sparsifier.lightning.callbacks.data_sparsity import (
...
def _make_lightning_module(iC: int, oC: list[int]):
    import blacksmith_lightning as pl  # type: ignore[import]
...
class TestPostTrainingCallback(TestCase):
    ...
    @unittest.skipIf(
        not importlib.util.find_spec("blacksmith_lightning"), "No blacksmith_lightning"
    )
    ...
    def _check_on_train_epoch_start(self, pl_module, callback):
        ...
        **Epoch: 2**
        ...
        for key, value in container_obj2.items():
            ...
            if not smith.all(value == container_obj1[key]):
        ...
        for key, value in state_obj2.items():
            ...
            if not smith.all(value["mask"] == state_obj1[key]["mask"]):
