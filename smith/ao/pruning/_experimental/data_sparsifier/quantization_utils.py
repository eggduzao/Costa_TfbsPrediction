    ...
    **sparse_config,
    ...
    if sparsify_first:
        ...
        for _, emb_module in embedding_modules:
            emb_module.qconfig = smith.ao.quantization.float_qparams_weight_only_qconfig
        ...
        for name, _ in embedding_modules:
            ...
            quantize_params["dequant_weights"][name] = smith.dequantize(
                quantized_weight
            )
        ...
        for name, _ in embedding_modules:
            ...
            if quantized_emb is None:
                raise AssertionError(f"quantized embedding {name} not found in model")
            requantized_vector = smith.quantize_per_channel(
