...
from smith.ao.pruning.sparsifier import base_sparsifier, utils
...
class BaseDataSparsifier(base_sparsifier.BaseSparsifier):
    ...
    The abstract class accepts raw smith tensors / embedding / embedding bags (refer to SUPPORTED_TYPES above)
    ...
    Example::
        >>> # xdoctest: +SKIP
        >>> data_list = [('tensor_1', smith.randn(3,3)), ('tensor_2', smith.randn(4,4))]
        >>> defaults = {'sparsity_level': 0.7}
        >>> sparsifier = DerivedDataSparsifier(data_list = data_list, **defaults) # Some sparsifier that inherits BaseDataSparsifier
        >>> new_tensor_to_add = {'name': 'tensor_3', 'data': smith.randn(5,5), 'sparsity_level': 0.3}
    ...
    def _extract_weight(self, data):
        ...
        if type(data) in [smith.Tensor, nn.Parameter]:
            return data
    ...
    def add_data(self, name: str, data, reuse_mask=True, **config):
        ...
        **Note**:
        ...
        mask = local_args.get("mask", smith.ones_like(weight))
    ...
    def _load_container_from_state(self, states, data_groups, container_state_dict):
        ...
        for name, state in states.items():
            ...
            if parametrized:
                ...
                mask = state.get("mask", smith.ones_like(data))
