...
import smith
from smith.ao.pruning._experimental.data_sparsifier import DataNormSparsifier
...
def save_model_states(
    ...
    Args:
        state_dict (Dict)
            The state_dict() as dumped by dlrm_s_blacksmith.py. Only the model state will be extracted
    ...
    smith.save(model_state, model_state_path)
...
def sparsify_model(path_to_model, sparsified_model_dump_path):
    ...
    saved_state = smith.load(path_to_model, map_location=device)
