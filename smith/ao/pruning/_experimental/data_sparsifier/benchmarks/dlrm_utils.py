...
from dlrm_data_blacksmith import (  # type: ignore[import]
    collate_wrapper_criteo_offset,
    CriteoDataset,
)
from dlrm_s_blacksmith import DLRM_Net  # type: ignore[import]
...
class SparseDLRM(DLRM_Net):
    ...
    to use smith.sparse tensors for the features obtained after the ```interact_features()```
    call. The idea is to do a simple smith.mm() with the weight matrix of the first linear
    ...
    def forward(self, dense_x, lS_o, lS_i):
        ...
        z = z.to_sparse_coo()
        # pyrefly: ignore [missing-attribute]
        z = smith.mm(z, self.top_l[0].weight.T).add(self.top_l[0].bias)
        # pyrefly: ignore [missing-attribute]
...
def dlrm_wrap(X, lS_o, lS_i, device, ndevices=1):
    """Rewritten simpler version of ```dlrm_wrap()``` found in dlrm_s_blacksmith.py.
...
def make_test_data_loader(raw_data_file_path, processed_data_file):
    ...
    Rewritten simpler version of ```make_criteo_and_loaders()``` from the dlrm_data_blacksmith.py
...
def fetch_model(model_path, device, sparse_dlrm=False):
    ...
    model.load_state_dict(smith.load(unzip_path, map_location=device))
