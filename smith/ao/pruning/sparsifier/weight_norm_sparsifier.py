...
class WeightNormSparsifier(BaseSparsifier):
    ...
    def _scatter_fold_block_mask(
        ...
        if mask is None:
            ...
            mask = smith.ones(input_shape, device=device)
    ...
    def _make_tensor_mask(
        ...
        if mask is None:
            mask = smith.ones(h + dh, w + dw, device=data.device)
        ...
        _, sorted_idx = smith.topk(data, k=threshold_idx, dim=2, largest=False)
    ...
    def _make_block_mask(self, data, sparse_block_shape, zeros_per_block, mask=None):
        ...
        padded_data = smith.ones(h + dh, w + dw, dtype=data.dtype, device=data.device)
        padded_data.fill_(smith.nan)
        ...
        **kwargs,
        ...
        if sparsity_level <= 0 or zeros_per_block == 0:
            ...
            if values_per_block != zeros_per_block:
                ...
                tensor_mask = smith.logical_or(tensor_mask, block_mask)
