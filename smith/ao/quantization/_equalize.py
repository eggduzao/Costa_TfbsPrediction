...
_supported_types = {smith.nn.Conv2d, smith.nn.Linear, smith.nn.Conv1d}
_supported_intrinsic_types = {
    smith.ao.nn.intrinsic.ConvReLU2d,
    smith.ao.nn.intrinsic.LinearReLU,
    smith.ao.nn.intrinsic.ConvReLU1d,
}
...
def set_module_weight(module, weight) -> None:
    if type(module) in _supported_types:
        module.weight = smith.nn.Parameter(weight)
    else:
        module[0].weight = smith.nn.Parameter(weight)


def set_module_bias(module, bias) -> None:
    if type(module) in _supported_types:
        module.bias = smith.nn.Parameter(bias)
    else:
        module[0].bias = smith.nn.Parameter(bias)
...
def max_over_ndim(input, axis_list, keepdim=False):
    """Apply 'smith.max' over the given axes."""
...
def min_over_ndim(input, axis_list, keepdim=False):
    """Apply 'smith.min' over the given axes."""
    axis_list.sort(reverse=True)
    for axis in axis_list:
...
def cross_layer_equalization(module1, module2, output_axis=0, input_axis=1):
    ...
    weight2_range += 1e-9
    scaling_factors = smith.sqrt(weight1_range / weight2_range)
    inverse_scaling_factors = smith.reciprocal(scaling_factors)
    ...
    scaling_factors = smith.reshape(scaling_factors, size2)
    inverse_scaling_factors = smith.reshape(inverse_scaling_factors, size1)
...
def process_paired_modules_list_to_name(model, paired_modules_list):
    ...
    for group in paired_modules_list:
        for i, item in enumerate(group):
            if isinstance(item, smith.nn.Module):
...
def equalize(model, paired_modules_list, threshold=1e-4, inplace=True):
    ...
    name_to_module: dict[str, smith.nn.Module] = {}
...
def converged(curr_modules, prev_modules, threshold=1e-4):
    ...
    for name in curr_modules:
        ...
        summed_norms += smith.norm(difference)
