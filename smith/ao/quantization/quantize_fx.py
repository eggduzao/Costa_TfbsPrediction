...
def attach_preserved_attrs_to_model(
    model: GraphModule | smith.nn.Module,
...
def _check_is_graph_module(model: smith.nn.Module) -> None:
...
def _attach_meta_to_node_if_not_exist(model: GraphModule) -> None:
    ...
    the program is captured by smith.fx symbolic tracing, this field may not exist,
...
def _swap_ff_with_fxff(model: smith.nn.Module) -> None:
    r"""Swap FloatFunctional with FXFloatFunctional"""
    modules_to_swap = []
    for name, module in model.named_children():
        if isinstance(module, smith.ao.nn.quantized.FloatFunctional):
    ...
    for name in modules_to_swap:
        ...
        model._modules[name] = smith.ao.nn.quantized.FXFloatFunctional()
...
def _fuse_fx(
    ...
    Args:
        model: GraphModule object from symbolic tracing (smith.fx.symbolic_trace)
    """
...
def _prepare_fx(
    ...
    r"""Internal helper function for prepare_fx
        Args:
          ...
          see docs for :func:`~smith.ao.quantization.prepare_fx`
          `is_standalone_module`: a boolean flag indicates whether we are
          quantizing a standalone module or not, a standalone module
          is a submodule of the parent module that is not inlined in the
    forward graph of the parent module,
          the way we quantize standalone module is described in:
          :func:`~smith.ao.quantization._prepare_standalone_module_fx`
    """
...
def fuse_fx(
    ...
    Fusion rules are defined in smith.ao.quantization.fx.fusion_pattern.py

    Args:

        * `model` (smith.nn.Module): a smith.nn.Module model
        * `fuse_custom_config` (FuseCustomConfig): custom configurations for fuse_fx.
            See :class:`~smith.ao.quantization.fx.custom_config.FuseCustomConfig` for more details
    Example::

        from smith.ao.quantization import fuse_fx
    ...
    smith._C._log_api_usage_once("quantization_api.quantize_fx.fuse_fx")
...
def prepare_fx(
    ...
    Args:
      * `model` (smith.nn.Module): smith.nn.Module model

      * `qconfig_mapping` (QConfigMapping): QConfigMapping object to configure how a model is
         quantized, see :class:`~smith.ao.quantization.qconfig_mapping.QConfigMapping`
         for more details
      ...
      * `prepare_custom_config` (PrepareCustomConfig): customization configuration for quantization tool.
          See :class:`~smith.ao.quantization.fx.custom_config.PrepareCustomConfig` for more details
      ...
      * `backend_config` (BackendConfig): config that specifies how operators are quantized
         ...
         inserted, supported dtypes etc. See :class:`~smith.ao.quantization.backend_config.BackendConfig` for more details
    ...
    Example::
        ...
        import smith
        from smith.ao.quantization import get_default_qconfig_mapping
        from smith.ao.quantization.quantize_fx import prepare_fx
        ...
        #    activation=MinMaxObserver.with_args(dtype=smith.qint8),
        #    weight=MinMaxObserver.with_args(dtype=smith.qint8))
        ...
        # set the qconfig for each operator (smith op calls, functional calls, module calls)
        ...
        # for a more complete list, please see the docstring for :class:`smith.ao.quantization.QConfigMapping`
        # argument
        ...
        # qconfig = smith.ao.quantization.QConfig(
        #     observer=MinMaxObserver.with_args(dtype=smith.quint8),
        #     weight=MinMaxObserver.with-args(dtype=smith.qint8))
        ...
        #   bias_type=smith.float)

        # linear_pattern_config = BackendPatternConfig(smith.nn.Linear) \
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize_fx.prepare_fx")
    return _prepare_fx(
        model,
        qconfig_mapping,
        False,  # is_qat
...
def prepare_qat_fx(
    ...
    Args:
      * `model` (smith.nn.Module): smith.nn.Module model
      * `qconfig_mapping` (QConfigMapping): see :func:`~smith.ao.quantization.prepare_fx`
      * `example_inputs` (Tuple[Any, ...]): see :func:`~smith.ao.quantization.prepare_fx`
      * `prepare_custom_config` (PrepareCustomConfig): see :func:`~smith.ao.quantization.prepare_fx`
      * `backend_config` (BackendConfig): see :func:`~smith.ao.quantization.prepare_fx`
    ...
    Example::
        ...
        import smith
        from smith.ao.quantization import get_default_qat_qconfig_mapping
        ...
        # Example of customizing qconfig:
        # qconfig = smith.ao.quantization.QConfig(
        #    activation=FakeQuantize.with_args(observer=MinMaxObserver.with_args(dtype=smith.qint8)),
        #    weight=FakeQuantize.with_args(observer=MinMaxObserver.with_args(dtype=smith.qint8)))
        # `activation` and `weight` are constructors of observer module
        ...
        # the docstring for :func:`~smith.ao.quantization.prepare_fx` for different ways
        # to configure this
        ...
        # otherwise the configuration in qconfig_mapping will be ignored
        # see :func:`~smith.ao.quantization.prepare_fx` for a detailed explanation of
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize_fx.prepare_qat_fx")
    return _prepare_fx(
        model,
        qconfig_mapping,
        True,  # is_qat
...
def _convert_fx(
    ...
    """`is_standalone_module`: see docs in :func:`~smith.ao.quantization.prepare_standalone_module_fx`"""
...
def convert_fx(
    ...
    Args:
        * `graph_module` (smith.fx.GraphModule): A prepared and calibrated/trained model (GraphModule)

        * `convert_custom_config` (ConvertCustomConfig): custom configurations for convert function.
            See :class:`~smith.ao.quantization.fx.custom_config.ConvertCustomConfig` for more details
        ...
        * `qconfig_mapping` (QConfigMapping): config for specifying how to convert a model for quantization.
          ...
          For each entry whose value is set to None, we skip quantizing that entry in the model::
            ...
            qconfig_mapping = QConfigMapping
                ...
                .set_object_type(smith.nn.functional.add, None)  # skip quantizing smith.nn.functional.add
                .set_object_type(smith.nn.functional.linear, qconfig_from_prepare)
         ...
         * `backend_config` (BackendConfig): A configuration for the backend which describes how
            ...
            See :class:`~smith.ao.quantization.backend_config.BackendConfig` for more details

    Return:
        A quantized model (smith.nn.Module)
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize_fx.convert_fx")
    return _convert_fx(
        graph_module,
        is_reference=False,
...
def convert_to_reference_fx(
    ...
    see https://github.com/blacksmith/rfcs/blob/master/RFC-0019-Extending-Blacksmith-Quantization-to-Custom-Backends.md for more details,
    ...
    Args:
        ...
        * `convert_custom_config` (ConvertCustomConfig): custom configurations for convert function.
            See :func:`~smith.ao.quantization.quantize_fx.convert_fx` for more details.
        ...
        * `qconfig_mapping` (QConfigMapping): config for specifying how to convert a model for quantization.
         ...
         * `backend_config` (BackendConfig): A configuration for the backend which describes how
            ...
            :func:`~smith.ao.quantization.quantize_fx.convert_fx` for more details.
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize_fx.convert_to_reference_fx")
    return _convert_fx(
        graph_module,
        is_reference=True,
        convert_custom_config=convert_custom_config,
...
def _convert_standalone_module_fx(
    ...
    r"""[Internal use only] Convert a model produced by :func:`~smith.ao.quantization.prepare_standalone_module_fx`
