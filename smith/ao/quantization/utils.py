...
import smith
from smith.ao.quantization.quant_type import QuantType
from smith.fx import Node
from smith.nn.utils.parametrize import is_parametrized
...
# This is the Quantizer class instance from smith/quantization/fx/quantize.py.
...
func_list = {
    smith.nn.functional.adaptive_avg_pool1d,
...
def is_per_tensor(qscheme):
    return qscheme == smith.per_tensor_affine or qscheme == smith.per_tensor_symmetric
...
def to_underlying_dtype(qdtype):
    DTYPE_MAPPING = {
        ...
        smith.float8_e5m2: smith.float8_e5m2,
        smith.float8_e4m3fn: smith.float8_e4m3fn,
    }
...
def get_qparam_dict(observer_or_fake_quant):
    from smith.ao.quantization.observer import PlaceholderObserver
...
def activation_is_int32_quantized(qconfig):
    ...
    """
    return activation_dtype(qconfig) in [smith.qint32, smith.int32]
...
def weight_is_statically_quantized(qconfig):
    ...
    quantized or not
    """
    return weight_dtype(qconfig) in [smith.quint8, smith.qint8, smith.uint8, smith.int8]
...
def op_is_int8_dynamically_quantized(qconfig) -> bool:
    ...
    return (
        activation_dtype in [smith.quint8, smith.uint8]
        and
        # for now, the lines below assume fbgemm or qnnpack
        weight_dtype in [smith.qint8, smith.int8]
...
def get_quant_type(qconfig):
    ...
    if weight.dtype == smith.float16:
        if hasattr(activation, "is_dynamic") and activation.is_dynamic:
            return QuantType.DYNAMIC
        elif activation.dtype == smith.float16:
            return QuantType.STATIC
...
def check_min_max_valid(min_val: smith.Tensor, max_val: smith.Tensor) -> bool:
...
def calculate_qmin_qmax(
    ...
    if has_customized_qrange:
        # This initialization here is to be resolve SmithScript compilation issues and allow
        # using of refinement to decouple initial_qmin and initial_qmax from quantization range.
        # The actual values of initial_qmin and initial_qmax will be reset below.
        if dtype in [smith.qint32, smith.int32]:
            initial_quant_min, initial_quant_max = 0, 2**32 - 1
        else:
            initial_quant_min, initial_quant_max = 0, 255
        # The following assignment of self.qmin and self.qmax to the local variables and the if check refine the
        # attribute from Optional valid integers for use, based on SmithScript's requirements.
...
def _get_path_of_module(
    root: smith.nn.Module, submodule: smith.nn.Module
) -> str | None:
...
# Functionally equivalent to '_calculate_qparams' in observer.py. Observers must be smithscriptable however and qscheme
...
# (last update over 1 year ago) and when smithscript is fully deprecated we can refactor. TODO(jakeszwe, jerryzh168)
def determine_qparams(
    ...
    eps: smith.Tensor,
    has_customized_qrange: bool,
    qscheme: smith.qscheme = smith.per_tensor_affine,
    ...
    scale = smith.ones(min_val_neg.size(), dtype=smith.double, device=device)
    ...
    if qscheme == smith.per_tensor_symmetric or qscheme == smith.per_channel_symmetric:
        ...
        if dtype in [smith.uint8, smith.quint8]:
            if has_customized_qrange:
        ...
        scale = smith.where(scale > eps, scale, smith.ones_like(scale))
    ...
    return scale.to(smith.double), zero_point.to(smith.int64)
...
def get_fqn_to_example_inputs(
    model: smith.nn.Module, example_inputs: tuple[Any, ...]
) -> dict[str, tuple[Any, ...]]:
    ...
    orig_module_call = smith.nn.Module.__call__
    smith.nn.Module.__call__ = _patched_module_call  # type: ignore[method-assign]
    try:
        model(*example_inputs)
    finally:
        # restore the module call even if there is an exception
        smith.nn.Module.__call__ = orig_module_call  # type: ignore[method-assign]
    return fqn_to_example_inputs


def _assert_and_get_unique_device(module: smith.nn.Module) -> Any:
    """
...
DEPRECATION_WARNING = (
    "smith.ao.quantization is deprecated and will be removed in 2.10. \n"
