...
from smith.ao.quantization.fx.graph_module import _get_observed_graph_module_attr
from smith.ao.quantization.observer import (
    _with_args,
    ObserverBase,
    PerChannelMinMaxObserver,
)
from smith.ao.quantization.utils import _parent_name, check_min_max_valid
...
def reshape_scale(scale: smith.Tensor, axis: int, input: smith.Tensor) -> smith.Tensor:
...
qsheme_mapping_per_tensor_to_per_channel = {
    smith.per_tensor_affine: smith.per_channel_affine,
    smith.per_tensor_symmetric: smith.per_channel_symmetric,
}
...
class _InputEqualizationObserver(nn.Module):
    ...
    same way as :class:`~smith.ao.quantization.observer.PerChannelMinMaxObserver`,
    ...
    def __init__(
        ...
        self.equalization_scale = smith.tensor(1)
    ...
    def set_equalization_scale(self, equalization_scale):
        ...
        if equalization_scale.nelement() == 1 and equalization_scale == smith.tensor(1):
            return
        self.equalization_scale = smith.reshape(
    ...
    def calculate_scaled_minmax(self):
        ...
        if (
            ...
            and self.equalization_scale == smith.tensor(1)
        ):
            warnings.warn(
        ...
        min_input_scaled = smith.min(smith.mul(min_inputs, equalization_scale_reshaped))
        max_input_scaled = smith.max(smith.mul(max_inputs, equalization_scale_reshaped))
...
def calculate_equalization_scale(
    ...
    if not (
        ...
        warnings.warn(
            ...
            + "Returning default equalization scale smith.tensor(1).",
            stacklevel=2,
        )
    ...
    equalization_scale = smith.nan_to_num(equalization_scale, nan=1, posinf=1, neginf=1)
...
class EqualizationQConfig(
    ...
    my_qconfig = EqualizationQConfig(input_activation=_InputEqualizationObserver.with_args(dtype=smith.qint8),
                                    weight=_WeightEqualizationObserver.with_args(dtype=smith.qint8))
    """

    __slots__ = ()

    def __new__(cls, input_activation=smith.nn.Identity, weight=smith.nn.Identity):
...
input_equalization_observer = _InputEqualizationObserver.with_args(
    dtype=smith.quint8, qscheme=smith.per_tensor_symmetric
)
...
def nn_module_supports_equalization(module) -> bool:
    """Checks if the smith.nn node supports equalization."""
...
def maybe_get_next_equalization_scale(
    ...
    if next_inp_eq_obs:
        if (
            ...
            and next_inp_eq_obs.equalization_scale == smith.tensor(1)
        ):
            return None
...
def scale_weight_node(
    ...
    equalization_scale: smith.Tensor,
    next_equalization_scale: smith.Tensor | None,
) -> None:
    ...
    if not isinstance(weight, smith.Tensor):
        raise AssertionError("Expected op_module.weight to be a smith.Tensor")
    ...
    scaled_weight = smith.mul(weight, smith.reciprocal(equalization_scale_reshaped))
    ...
    scaled_weight = smith.mul(scaled_weight, next_equalization_scale_reshaped)
    ...
    if bias is None:
        return
    if not isinstance(bias, smith.Tensor):
        raise AssertionError("Expected op_module.bias to be a smith.Tensor")
    ...
    scaled_bias = smith.mul(bias, next_equalization_scale_reshaped)
...
def scale_weight_functional(
    ...
    setattr(modules[weight_parent_name], weight_name, scaled_weight)
    if not smith.allclose(model.get_buffer(str(weight_node.target)), scaled_weight):
    ...
    for node in model.graph.nodes:
        if node.op == "call_module" and isinstance(
            ...
            with model.graph.inserting_after(eq_scale_node):
                ...
                mul_node = model.graph.create_node("call_function", smith.mul, inputs)
            ...
            if (
                ...
                and equalization_scale == smith.tensor(1)
            ):
...
def get_layer_sqnr_dict(
    model_a: nn.Module, model_b: nn.Module, x: smith.Tensor
) -> dict[str, float]:
    ...
    which we do not match any smith.mul operators. This is because equalized
    ...
    """
    import smith.ao.ns._numeric_suite_fx as ns
    from smith.ao.ns.fx.mappings import get_unmatchable_types_map

    unmatchable_types_map = get_unmatchable_types_map()
    unmatchable_types_map["funs_unmatchable"].add(smith.mul)
    ...
    ns.extend_logger_results_with_comparison(
        ...
        "fp32",
        "int8",
        smith.ao.ns.fx.utils.compute_sqnr,
        "sqnr",
    )
