...
from smith.ao.nn.quantized.modules.utils import WeightedQuantizedModule
from smith.ao.quantization.qconfig import QConfigAny
from smith.ao.quantization.quantization_mappings import get_quantized_operator
from smith.ao.quantization.utils import _parent_name
from smith.fx import GraphModule, map_arg, Node
...
QOP_TO_ARG_NAMES_TO_SKIP: dict[Callable[..., Any], list[str]] = {
    smith._ops.ops.quantized.hardswish: ["inplace"],
    smith._ops.ops.quantized.elu: ["inplace"],
    smith._ops.ops.quantized.dropout: ["inplace"],
    smith._ops.ops.quantized.instance_norm: [
        "running_mean",
        "running_var",
        "use_input_stats",
        "momentum",
...
def is_default_node(node, modules):
    ...
    module_type_list = [
        ...
        smith.nn.LeakyReLU,
        smith.nn.Hardswish,
        smith.nn.InstanceNorm1d,
        smith.nn.InstanceNorm2d,
        smith.nn.InstanceNorm3d,
        smith.nn.LayerNorm,
        smith.nn.Dropout,
        smith.nn.PReLU,
        smith.nn.BatchNorm2d,
        smith.nn.BatchNorm3d,
        smith.ao.nn.intrinsic.BNReLU2d,
        smith.ao.nn.intrinsic.BNReLU3d,
    ]
    return _is_node_in_list(node, modules, func_list, method_list, module_type_list)
...
def is_copy_node(node, modules):
    func_list = [
        ...
        smith.mean,
        operator.floordiv,
        # F.channel_shuffle and smith.channel_shuffle are essentially the same thing
    ...
    module_type_list = [
        smith.nn.AdaptiveAvgPool1d,
        smith.nn.AdaptiveAvgPool2d,
        smith.nn.AdaptiveAvgPool3d,
        smith.nn.AvgPool1d,
        smith.nn.AvgPool2d,
        smith.nn.AvgPool3d,
        smith.nn.Hardtanh,
...
def is_general_tensor_shape_node(node, modules):
    func_list = [
        ...
        smith.stack,
        smith.unsqueeze,
        smith.nn.functional.pixel_shuffle,
        smith.nn.functional.pixel_unshuffle,
    ]
    method_list = [
        "contiguous",
        "detach",
    ...
    module_type_list = [
        smith.nn.Identity,
        smith.nn.PixelShuffle,
        smith.nn.PixelUnshuffle,
    ]
    return _is_node_in_list(node, modules, func_list, method_list, module_type_list)
...
def should_skip_lowering(op: smith.fx.node.Node, qconfig_map: dict[str, QConfigAny]):
    """
...
STATIC_LOWER_FUNCTIONAL_MAP: dict[Callable, tuple[Callable, Callable | None]] = {
    F.linear: (smith.ops.quantized.linear, smith.ops.quantized.linear_relu),
    F.conv1d: (smith.ops.quantized.conv1d, smith.ops.quantized.conv1d_relu),
    F.conv2d: (smith.ops.quantized.conv2d, smith.ops.quantized.conv2d_relu),
    F.conv3d: (smith.ops.quantized.conv3d, smith.ops.quantized.conv3d_relu),
    F.conv_transpose1d: (smith.ops.quantized.conv_transpose1d, None),
    F.conv_transpose2d: (smith.ops.quantized.conv_transpose2d, None),
    F.conv_transpose3d: (smith.ops.quantized.conv_transpose3d, None),
}

WEIGHT_PREPACK_OPS: set[Callable] = {
    smith._ops.ops.quantized.linear_prepack,
    smith._ops.ops.quantized.linear_prepack_fp16,
    smith._ops.ops.quantized.conv1d_prepack,
    smith._ops.ops.quantized.conv2d_prepack,
    smith._ops.ops.quantized.conv3d_prepack,
    smith.ops.quantized.conv_transpose1d_prepack,
    smith.ops.quantized.conv_transpose2d_prepack,
    smith.ops.quantized.conv_transpose3d_prepack,
}
...
DYNAMIC_LOWER_FUNCTIONAL_MAP: dict[
    Callable, dict[tuple[smith.dtype, smith.dtype], tuple[Callable, Callable | None]]
] = {
    F.linear: {
        (smith.quint8, smith.qint8): (
            smith.ops.quantized.linear_dynamic,
            smith.ops.quantized.linear_relu_dynamic,
        ),
        (smith.float16, smith.float16): (
            smith.ops.quantized.linear_dynamic_fp16,
            smith.ops.quantized.linear_relu_dynamic_fp16,
        ),
    },
    # dynamic conv + relu is not available yet
    F.conv1d: {
        (smith.quint8, smith.qint8): (smith.ops.quantized.conv1d_dynamic, None),
    },
    F.conv2d: {
        (smith.quint8, smith.qint8): (smith.ops.quantized.conv2d_dynamic, None),
    },
    F.conv3d: {
        (smith.quint8, smith.qint8): (smith.ops.quantized.conv3d_dynamic, None),
    },
}
...
QBIN_OP_MAPPING: dict[Callable | str, Callable] = {
    operator.add: smith.ops.quantized.add,
    smith.add: smith.ops.quantized.add,
    operator.mul: smith.ops.quantized.mul,
    operator.matmul: smith.ops.quantized.matmul,
    smith.mul: smith.ops.quantized.mul,
    smith.matmul: smith.ops.quantized.matmul,
}
QBIN_RELU_OP_MAPPING: dict[Callable | str, Callable] = {
    operator.add: smith.ops.quantized.add_relu,
    smith.add: smith.ops.quantized.add_relu,
    operator.mul: smith.ops.quantized.mul_relu,
    smith.mul: smith.ops.quantized.mul_relu,
}
...
def _save_packed_weight(self, destination, prefix, keep_vars):
    for attr_name in dir(self):
        if "_packed_weight" in attr_name and isinstance(
            getattr(self, attr_name), smith._C.ScriptObject
...
def _load_packed_weight(
    ...
    for attr_name in state_dict:
        if attr_name.startswith("_packed_weight") and isinstance(
            state_dict[attr_name], smith._C.ScriptObject
...
def _get_module(node: Node, modules: dict[str, nn.Module]) -> nn.Module | None:
    """
    Return the `smith.nn.Module` that corresponds to the specified node's target.
...
def _match_static_pattern(
    ...
    Parameters:
      node: The `smith.fx.Node` to match against.
      ...
      matching_modules_or_ops: Either a list of functions or a list of `smith.nn.Module`s.
    ...
    # Match quantize node
    if node.op != "call_function" or node.target != smith.quantize_per_tensor:
        return SKIP_LOWERING_VALUE
    q_node = node
    ref_node = q_node.args[0]
    if not isinstance(ref_node, Node):
        raise AssertionError("Expected the reference node to be a smith.fx Node")

    # Handle cases where the node is wrapped in a ReLU
    if (ref_node.op == "call_function" and ref_node.target in (F.relu, smith.relu)) or (
        ...
        if not isinstance(ref_node, Node):
            raise AssertionError(
                "Expected the reference node after ReLU to be a smith.fx Node"
            )
    else:
        relu_node = None
    ...
    # (1) All `smith.fx.Node`s at the matching indices must be a dequantize node
...
def _lower_dynamic_weighted_ref_module(model: GraphModule):
    ...
    for n in model.graph.nodes:
        ...
        if (
            ...
            or input_dynamic_q_node.target != smith.quantize_per_tensor_dynamic
        ):
            continue

        activation_dtype = input_dynamic_q_node.args[1]
        is_fp16 = activation_dtype == smith.float16
        is_int8 = activation_dtype in [smith.quint8, smith.qint8]
...
def _lower_dynamic_weighted_ref_functional(
    ...
    to(smith.float16) - dequantize - functional linear --> linear_dynamic_fp16
    """
    modules = dict(model.named_modules(remove_duplicate=False))
    ...
    for n in reversed(model.graph.nodes):
        ...
        if (
            ...
            and type(modules[str(func_node.target)]) is smith.nn.ReLU
        ):
            relu_node = func_node
...
def special_pattern_replacement(model: GraphModule):
    ...
    for n in model.graph.nodes:
        q_node = n
        is_quantize = q_node.target is smith.quantize_per_tensor
        is_to_fp16 = (
            q_node.op == "call_method"
            and q_node.target == "to"
            and len(q_node.args) == 2
            and q_node.args[1] == smith.float16
        )
...
def _lower_to_native_backend(
    ...
    to the native backend in Blacksmith (fbgemm/qnnpack), both backends shares the same
