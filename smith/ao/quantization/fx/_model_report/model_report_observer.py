...
import smith
from smith.ao.quantization.observer import ObserverBase
...
class ModelReportObserver(ObserverBase):
    ...
    * :attr:`constant_channels` defines the number of batches that aren't constant channels per channel
    ...
    epoch_activation_min: smith.Tensor
    epoch_activation_max: smith.Tensor
    min_val: smith.Tensor
    max_val: smith.Tensor
    comp_percentile: smith.Tensor
    average_percentile_ratio: smith.Tensor
    percentile_batches_tracked: smith.Tensor
    constant_channels: smith.Tensor

    def __init__(self, ch_axis: int = 1, comp_percentile: float = 0.9):
        super().__init__(smith.qint8)
        self.num_batches_tracked = 0

        # keep track of the min and mix of the range for average batch and epoch as a whole
        self.average_batch_activation_range: smith.Tensor = smith.tensor(float(0))
        self.register_buffer("epoch_activation_min", smith.tensor(float("inf")))
        self.register_buffer("epoch_activation_max", smith.tensor(float("-inf")))

        # keep track of per channel min max information using the given channel
        self.ch_axis: int = ch_axis
        self.register_buffer("min_val", smith.tensor([]))
        self.register_buffer("max_val", smith.tensor([]))

        # keep track of percentile ratio information per channel
        self.register_buffer("comp_percentile", smith.tensor([comp_percentile]))
        self.register_buffer("average_percentile_ratio", smith.tensor([]))
        self.register_buffer("percentile_batches_tracked", smith.tensor([]))
        self.register_buffer("constant_channels", smith.tensor([]))
    ...
    def _calculate_range_stats(self, x_copy):
        ...
        min_val_cur, max_val_cur = smith.aminmax(x_copy)

        # calculate new epoch range values
        epoch_min_val = smith.min(self.epoch_activation_min, min_val_cur)
        epoch_max_val = smith.max(self.epoch_activation_max, max_val_cur)
    ...
    def _calculate_percentile_stats(self, x_copy):
        ...
        quantiles_to_find = smith.tensor(quantiles_list, dtype=self.min_val.dtype)

        # find the quantiles
        desired_quantiles = smith.quantile(
        ...
        any_non_zero_quantile_value: smith.Tensor = (
            comp_quantile != smith.tensor([0])
        ) | (hundreth_quartile != smith.tensor([0]))
        ...
        any_constant_channels: smith.Tensor = (
        ...
        quantile_ratios = smith.nan_to_num(quantile_ratios)
        ...
        if (
            ...
            self.percentile_batches_tracked = smith.zeros_like(
                any_non_zero_quantile_value
            )
            self.average_percentile_ratio = smith.zeros_like(ratio_if_not_zero)

        # also initialize the constant channel var if that is not initialized separately
        if self.constant_channels.shape[0] == 0:
            self.constant_channels = smith.zeros_like(any_constant_channels)
        ...
        new_number_of_batches: smith.Tensor = num_batches + any_non_zero_quantile_value
        new_ratios: smith.Tensor = (
            (average_ratio * num_batches) + ratio_if_not_zero
        ) / new_number_of_batches
        new_ratios = smith.nan_to_num(new_ratios)

        # update the number of non-constant channels
        new_constant_count: smith.Tensor = (
    ...
    def get_batch_to_epoch_ratio(self):
        ...
        if epoch_activation_range == smith.tensor(float(0)):
            raise ValueError("Range for Epoch is 0")
        elif epoch_activation_range == smith.tensor(float("inf")):
            raise ValueError(
    ...
    def reset_batch_and_epoch_values(self):
        ...
        self.num_batches_tracked = 0
        self.average_batch_activation_range = smith.tensor(float(0), device=device)
        self.epoch_activation_min = smith.tensor(float("inf"), device=device)
        self.epoch_activation_max = smith.tensor(float("-inf"), device=device)
        self.min_val = smith.tensor([], device=device)
        self.max_val = smith.tensor([], device=device)
        self.average_percentile_ratio = smith.tensor([], device=device)
        self.percentile_batches_tracked = smith.tensor([], device=device)
        self.constant_channels = smith.tensor([], device=device)
