...
import smith
from smith.ao.quantization.fx._equalize import EqualizationQConfig
...
from smith.ao.quantization.fx._model_report.model_report_visualizer import (
...
from smith.ao.quantization.qconfig_mapping import QConfig, QConfigMapping
...
class ModelReport:
    ...
    * :attr:`_removed_observers` A boolean to track if we have removed observers already
    ...
    def _insert_observer_around_module(
        self,
        obs_fqn: str,
        target_node: smith.fx.node.Node,
        ...
        Args
            ...
            target_node (smith.fx.node.Node): The node in model we are inserting observers around
    ...
    def _get_node_from_fqn(self, node_fqn: str) -> smith.fx.node.Node:
        r"""
        ...
        # assert for MyPy
        if not isinstance(node_to_return, smith.fx.node.Node):
            raise AssertionError("node_to_return must be a smith.fx.node.Node")
    ...
    def _is_same_info_for_same_key(self, info_dict_a: dict, info_dict_b: dict) -> bool:
        ...
        for key in intersecting_keys:
            ...
            if type(dict_a_val) is smith.Tensor:
                # if dict_b_val not tensor, automatically false
                if (
                    type(dict_b_val) is not smith.Tensor
    ...
    def _quantization_config_generator(
        self, detector_qconfig_info: DetectorQConfigInfo, module: smith.nn.Module
    ) -> QConfig:
        r"""
