...
from smith.ao.quantization.fx._model_report.model_report_observer import (
    ModelReportObserver,
)
from smith.ao.quantization.fx.graph_module import GraphModule
...
class DetectorQConfigInfo:
    ...
    def generate_quantization_qconfig(self, module: smith.nn.Module) -> QConfig:
        r"""
        Args:
            module (smith.nn.Module) The module we are generating
            the qconfig for
...
class PerChannelDetector(DetectorBase):
    ...
    Args:
        backend (str, optional): the backend the user wishes to use in production
            Default value is current smith.backends.quantized.engine
    """
    ...
    def __init__(self, backend: str = smith.backends.quantized.engine):
        super().__init__()
...
class DynamicStaticDetector(DetectorBase):
    ...
    def determine_observer_insert_points(
        ...
        Returns a Dict mapping from unique observer fqns (where we want to insert them) to a Dict with:
            key "target_node" -> the node we are trying to observe with this observer (smith.fx.node.Node)
...
class InputWeightEqualizationDetector(DetectorBase):
    ...
    * :attr:`DEFAULT_PRE_OBSERVER_NAME`: The name of the pre-observer to be inserted for this detector
    ...
    def get_qconfig_info(self, model) -> dict[str, DetectorQConfigInfo]:
        ...
        comp_stats: dict[str, smith.Tensor] = self._generate_comparison_values(
    ...
    def _extract_weight_info(self, model: GraphModule) -> dict[str, dict]:
        ...
        for fqn, module in model.named_modules():
            ...
            if self._is_supported(module):
                ...
                min_val: smith.Tensor = smith.tensor([float("inf")], device=device)
                max_val: smith.Tensor = smith.tensor([float("-inf")], device=device)
    ...
    def _generate_comparison_values(
        ...
        module_fqn_to_channel: dict[str, smith.Tensor] = {}
        ...
        for module_fqn in input_info:
            ...
            s = smith.sqrt(weight_ratio) / smith.sqrt(input_ratio)
    ...
    def generate_detector_report(
        ...
        for module_fqn in input_weight_equalization_info:
            ...
            recommendation_per_channel: smith.Tensor = mod_info[self.RECOMMENDED_KEY]
...
class OutlierDetector(DetectorBase):
    ...
    * :attr:`DEFAULT_PRE_OBSERVER_NAME`: The name of the pre-observer to be inserted for this detector
    ...
    def _calculate_outlier_info(
        self,
        percentile_ratios: smith.Tensor,
        counted_batches: smith.Tensor,
        total_batches: int,
    ) -> dict[str, list[bool]]:
        r"""
        ...
        Args:
            percentile_ratios (smith.Tensor): The average percentile_ratios per channel calculated by the observer
            counted_batches (smith.Tensor): The number of batches used for average calculation per tensor
    ...
    def _generate_info_dict(self, model: GraphModule) -> dict[str, dict]:
        ...
        for fqn, module in model.named_modules():
            ...
            if self._supports_report_gen(module):
                ...
                num_batches: smith.Tensor = pre_obs.percentile_batches_tracked
                average_ratios: smith.Tensor = pre_obs.average_percentile_ratio
                channel_batch_cnts: smith.Tensor = pre_obs.constant_channels
                total_batches: int = pre_obs.num_batches_tracked

                # also get the max values
                max_vals: smith.Tensor = pre_obs.max_val
