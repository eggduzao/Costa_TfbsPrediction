...
from smith.ao.quantization.utils import MatchAllNode, Pattern
...
def _find_matches(
    ...
    For example, {
      'relu_1': (relu_1, [relu_1], smith.nn.functional.relu,
    ...
    def is_standalone_module(node_target: str, modules: dict[str, smith.nn.Module]):
        if modules is None:
            raise AssertionError("modules must not be None")
        return (
