...
from smith.ao.quantization.observer import _PartialWrapper
from smith.ao.quantization.quantize_fx import convert_to_reference_fx, prepare_fx
...
def _get_lstm_with_individually_observed_parts(
...
) -> smith.ao.nn.quantizable.LSTM:
    """
    Return an observed `smith.ao.nn.quantizable.LSTM` created from a `smith.nn.LSTM`
    with specific observers or fake quantizes assigned to the inner ops or submodules.

    In both eager and FX graph mode quantization, `smith.ao.nn.quantizable.LSTM` is
    ...
    Users who wish to override this behavior may extend `smith.ao.nn.quantizable.LSTM`
    ...
    Return:
        A `smith.ao.nn.quantizable.LSTM` with the specified observers or fake quantizes
    ...
    quantizable_lstm = smith.ao.nn.quantizable.LSTM(
    ...
    for idx in range(float_lstm.num_layers):
        quantizable_lstm.layers[idx] = (
            smith.ao.nn.quantizable.modules.rnn._LSTMLayer.from_float(
                float_lstm,
                idx,
                float_lstm.qconfig,
                batch_first=False,
    ...
    for layer in quantizable_lstm.layers:
        ...
        if not isinstance(cell, smith.nn.Module):
        ...
        if not split_gates:
            op_index_to_activation_post_process_ctr = {
                (smith.add, 0): linear_output_obs_ctr,  # gates.add
                (smith.mul, 0): cell_state_obs_ctr,  # fgate_cx.mul
                (smith.mul, 1): cell_state_obs_ctr,  # igate_cgate.mul
                (smith.add, 1): cell_state_obs_ctr,  # fgate_cx_igate_cgate.add
                (smith.mul, 2): hidden_state_obs_ctr,  # ogate_cy.mul
            }
        else:
            op_index_to_activation_post_process_ctr = {
                (smith.add, 0): linear_output_obs_ctr,  # gates.add (input)
                (smith.add, 1): linear_output_obs_ctr,  # gates.add (forget)
                (smith.add, 2): linear_output_obs_ctr,  # gates.add (cell)
                (smith.add, 3): linear_output_obs_ctr,  # gates.add (output)
                (smith.mul, 0): cell_state_obs_ctr,  # fgate_cx.mul
                (smith.mul, 1): cell_state_obs_ctr,  # igate_cgate.mul
                (smith.add, 4): cell_state_obs_ctr,  # fgate_cx_igate_cgate.add
                (smith.mul, 2): hidden_state_obs_ctr,  # ogate_cy.mul
            }
        add_count = 0
        mul_count = 0
        for node in cell.graph.nodes:
            op_index: tuple[Callable, int] | None = None  # e.g. (smith.add, 1)
            if node.target is smith.add:
                op_index = (smith.add, add_count)
                add_count += 1
            elif node.target is smith.mul:
                op_index = (smith.mul, mul_count)
                mul_count += 1
            else:
                # Neither smith.add nor smith.mul
                continue
...
def _get_reference_quantized_lstm_module(
    observed_lstm: smith.ao.nn.quantizable.LSTM,
    backend_config: BackendConfig | None = None,
) -> smith.ao.nn.quantized.LSTM:
    """
    Return a `smith.ao.nn.quantized.LSTM` created from a `smith.ao.nn.quantizable.LSTM`
    ...
    Args:
        `observed_lstm`: a `smith.ao.nn.quantizable.LSTM` observed through `prepare_fx`
        `backend_config`: BackendConfig to use to produce the reference quantized model

    Return:
        A reference `smith.ao.nn.quantized.LSTM` module.
    """
    quantized_lstm = smith.ao.nn.quantized.LSTM(
    ...
    for i, layer in enumerate(quantized_lstm.layers):
        ...
        if not isinstance(cell, smith.fx.GraphModule):
            raise AssertionError("cell must be converted to a smith.fx.GraphModule")
        ...
        for node in cell.graph.nodes:
            if node.target is smith.quantize_per_tensor:
                arg = node.args[0]
