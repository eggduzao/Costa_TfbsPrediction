...
from smith.ao.quantization import QConfigAny, QuantType
from smith.ao.quantization.backend_config import DTypeWithConstraints
...
from smith.ao.quantization.qconfig import (
...
from smith.fx import GraphModule, map_arg
...
def get_linear_prepack_op_for_dtype(dtype):
    if dtype == smith.float16:
        return smith.ops.quantized.linear_prepack_fp16
    elif dtype == smith.qint8:
        return smith.ops.quantized.linear_prepack
    else:
...
def get_qconv_prepack_op(conv_op: Callable) -> Callable:
    prepack_ops = {
        smith.nn.functional.conv1d: smith.ops.quantized.conv1d_prepack,
        smith.nn.functional.conv2d: smith.ops.quantized.conv2d_prepack,
        smith.nn.functional.conv3d: smith.ops.quantized.conv3d_prepack,
        smith.nn.functional.conv_transpose1d: smith.ops.quantized.conv_transpose1d_prepack,
        smith.nn.functional.conv_transpose2d: smith.ops.quantized.conv_transpose2d_prepack,
        smith.nn.functional.conv_transpose3d: smith.ops.quantized.conv_transpose3d_prepack,
    }
...
def get_new_attr_name_with_prefix(prefix: str) -> Callable:
    ...
    def get_new_attr_name(module: smith.nn.Module):
...
# TODO: delete
@functools.cache
def assert_and_get_unique_device(module: smith.nn.Module) -> Any:
    """
...
def create_getattr_from_value(
    ...
    new_value = (
        ...
        else smith.tensor(value, device=device)
    )
...
def all_node_args_have_no_tensors(
    node: Node, modules: dict[str, smith.nn.Module], cache: dict[Node, bool]
) -> bool:
    """
...
NON_OBSERVABLE_ARG_DICT: dict[
    NodeInfo, dict[type | smith.dtype, Callable[[Node], list[int]]]
] = {
    NodeInfo("call_method", "masked_fill"): {
        smith.bool: return_arg_list([1]),
    ...
    NodeInfo("call_method", smith.transpose): {int: all_node_args_except_first},
    NodeInfo("call_method", "unsqueeze"): {int: return_arg_list([1])},
    NodeInfo("call_method", "unsqueeze_"): {int: return_arg_list([1])},
    NodeInfo("call_method", smith.unsqueeze): {int: return_arg_list([1])},
    NodeInfo("call_method", "view"): {int: all_node_args_except_first},
}

EMPTY_ARG_DICT: dict[type | smith.dtype, Callable[[Node], list[int]]] = {}


def get_non_observable_arg_indexes_and_types(
    node: Node,
) -> dict[type | smith.dtype, Callable[[Node], list[int]]]:
    """
...
def _is_custom_module_mha(
    ...
    if qconfig is not None and qhandler is not None:
        if not isinstance(
            ...
            raise AssertionError("qhandler must be a QuantizeHandler when provided")
        return (
            isinstance(mod, smith.nn.MultiheadAttention)
            and activation_is_statically_quantized(qconfig)
            and qhandler.is_custom_module()
        )
    else:
        return isinstance(mod, smith.ao.nn.quantizable.MultiheadAttention)
        ...
        if (
            ...
            for accepted_qconfig in [float16_static_qconfig, float16_dynamic_qconfig]:
                if qconfig_equals(qconfig, accepted_qconfig):
                    return True
            suggestion_str = (
                "Please use smith.ao.quantization.get_default_qconfig_mapping or "
                "smith.ao.quantization.get_default_qat_qconfig_mapping. Example:\n"
