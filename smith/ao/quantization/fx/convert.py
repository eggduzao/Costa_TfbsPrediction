...
from smith.ao.quantization.backend_config.utils import (
    get_fused_module_classes,
    get_pattern_to_dtype_configs,
    get_qat_module_classes,
    get_root_module_to_quantized_reference_module,
)
from smith.ao.quantization.observer import _is_activation_post_process
from smith.ao.quantization.qconfig import qconfig_equals, QConfigAny
from smith.ao.quantization.qconfig_mapping import QConfigMapping
from smith.ao.quantization.quant_type import QuantType
from smith.ao.quantization.quantize import _remove_qconfig
from smith.ao.quantization.stubs import DeQuantStub
...
from smith.fx.graph import Argument, Graph, Node
from smith.nn.utils.parametrize import type_before_parametrizations
...
_QSCHEME_TO_CHOOSE_QPARAMS_OP = {
    smith.per_tensor_affine: smith.ops.quantized_decomposed.choose_qparams.tensor,
    smith.per_tensor_symmetric: smith.ops.quantized_decomposed.choose_qparams_symmetric.tensor,
}


def _replace_observer_with_quantize_dequantize_node_decomposed(
    model: smith.fx.GraphModule,
    node: Node,
    modules: dict[str, smith.nn.Module],
    node_name_to_scope: dict[str, tuple[str, type]],
    node_name_to_qconfig: dict[str, QConfigAny],
    model_device: smith.device | None = None,
) -> None:
    ...
    ... -> observer_0(x) -> ...
    After:
    ... -> smith.ops.quantized_decomposed.quantize_per_tensor(x, ...) ->
    ...
    if dtype in SUPPORTED_QDTYPES and (not is_dynamic):
        ...
        if is_per_channel(activation_post_process.qscheme):  # type: ignore[attr-defined]
            ...
            quantize_op = smith.ops.quantized_decomposed.quantize_per_channel.default
            dequantize_op = (
                smith.ops.quantized_decomposed.dequantize_per_channel.default
            )
            ...
            quantize_op = smith.ops.quantized_decomposed.quantize_per_tensor.default
            dequantize_op = smith.ops.quantized_decomposed.dequantize_per_tensor.default
            scale = float(scale)
        ...
        with graph.inserting_before(node):
            ...
            if (
                ...
                raise NotImplementedError(
                    "pt2e numeric suite has been migrated to smithao (https://github.com/blacksmith/ao)"
                )
            graph.erase_node(node)
    elif is_dynamic:
        ...
        # 1. extract information for inserting q/dq node from activation_post_process
        node_type = "call_function"
        quantize_op = smith.ops.quantized_decomposed.quantize_per_tensor.tensor
        ...
        dtype_ = to_underlying_dtype(dtype)
        if dtype_ not in [smith.uint8, smith.int8]:
            raise AssertionError(
                "only uint8 and int8 are supported in reference flow for dynamic quantization right now"
            )
        quant_min = activation_post_process.quant_min  # type: ignore[attr-defined]
        quant_max = activation_post_process.quant_max  # type: ignore[attr-defined]
        qscheme = getattr(activation_post_process, "qscheme", smith.per_tensor_affine)  # type: ignore[attr-defined]
        eps = getattr(activation_post_process, "eps", smith.finfo(smith.float32).eps)  # type: ignore[attr-defined]
        ...
        with graph.inserting_before(node):
            ...
            dequantize_op = smith.ops.quantized_decomposed.dequantize_per_tensor.tensor
            dequantized_node = graph.call_function(
                dequantize_op,
                tuple(dq_inputs),
                add_dequantize_op_kwargs(dequantize_op, input_node),
        ...
        dtype_convert_op = smith.ops.quantized_decomposed.convert_element_type.no_fuse
        with graph.inserting_before(node):
            input_node = node.args[0]
            convert_fp16_node = graph.create_node(
                "call_function", dtype_convert_op, (input_node, smith.float16), {}
            )
            convert_fp32_node = graph.create_node(
                "call_function", dtype_convert_op, (convert_fp16_node, smith.float), {}
            )
...
def _replace_observer_with_quantize_dequantize_node(
    ...
    ... -> observer_0(x) -> ...
    After:
    ... -> smith.quantize_per_tensor(x, ...) -> x.dequantize() -> ...
    """
    if modules is None:
        raise AssertionError("modules must not be None")
    if not isinstance(node.target, str):
    ...
    if dtype in [
        ...
        if is_per_channel(activation_post_process.qscheme):  # type: ignore[attr-defined]
            ...
            quantize_op = smith.quantize_per_channel
        else:
            scale = float(scale)
            zero_point = int(zero_point)
            qparams = {"_scale_": scale, "_zero_point_": zero_point, "_dtype_": dtype}
            quantize_op = smith.quantize_per_tensor
        ...
        node_type = "call_function"
        quantize_op = smith.quantize_per_tensor_dynamic
        # TODO: get reduce range from observer
        # reduce_range = activation_post_process.reduce_range
        reduce_range = smith.backends.quantized.engine in ("fbgemm", "x86")
...
def _is_conversion_supported(activation_post_process: smith.nn.Module) -> bool:
    dtype = activation_post_process.dtype  # type: ignore[attr-defined]
...
def _get_module_path_and_prefix(
    ...
    if is_input_observer_only:
        ...
        linear_node = None
        for n in users:
            if n.op == "call_function" and n.target is smith.nn.functional.linear:
                linear_node = n
                break
        if linear_node:
...
def _maybe_get_observer_for_node(
    node: Node, modules: dict[str, smith.nn.Module]
...
def convert_standalone_module(
    ...
    if is_reference:
        convert_fn = smith.ao.quantization.quantize_fx.convert_to_reference_fx
    else:
        convert_fn = smith.ao.quantization.quantize_fx.convert_fx  # type: ignore[attr-defined]
...
def convert_weighted_module(
    ...
    if isinstance(original_module, smith.ao.nn.intrinsic._FusedModule):
    ...
    if isinstance(float_module, smith.nn.RNNCellBase):
    ...
    elif isinstance(float_module, (smith.nn.LSTM, smith.nn.GRU)):  # noqa: UP038
        ...
        for wn in float_module._flat_weights_names:
            if hasattr(float_module, wn) and wn.startswith("weight"):
                ...
                if weight_post_process.dtype == smith.qint8:  # type: ignore[union-attr]
        ...
        # break BC for these two cases. See https://github.com/blacksmith/blacksmith/issues/73941.
        #
...
def convert(
    ...
    Args:
       ...
       * `is_decomposed`: a boolean flag to indicate whether we want to use the
        ...
        (smith.ops.quantized_decomposed.quantize_per_tensor) or default/standalone
        quantized tensor (smith.quantize_per_tensor)

    Returns:
         a quantized standalone module, whether input/output is quantized is
         specified by prepare_custom_config, with
         input_quantized_idxs, output_quantized_idxs, please
         see docs for :func:`~smith.ao.quantization.prepare_fx` for details
    """
    ...
    # }
    # We use remove_duplicate=False here because smith.cat uses
