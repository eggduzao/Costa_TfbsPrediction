...
from smith.ao.quantization.qconfig import _is_reuse_input_qconfig, QConfigAny
from smith.ao.quantization.qconfig_mapping import QConfigMapping
from smith.ao.quantization.quantize import convert, propagate_qconfig_
...
from smith.fx.graph import Graph, Node
...
_DO_NOT_OBS_DTYPE_LIST = [int, float, smith.bool, None]
_OBS_DTYPE_LIST = [
    smith.quint8,
    smith.qint8,
    smith.qint32,
...
_DEFAULT_FP32_OBS_OR_FQ_CTR = PlaceholderObserver.with_args(dtype=smith.float)
...
_DEFAULT_FP32_QCONFIG_FOR_TARGET_DTYPE_INFO = {
    "input_act_obs_or_fq_ctr": smith.ao.quantization.qconfig._default_fp32_placeholder_qconfig.activation,
    "output_act_obs_or_fq_ctr": smith.ao.quantization.qconfig._default_fp32_placeholder_qconfig.activation,
}

_DEFAULT_QUINT8_QCONFIG_FOR_TARGET_DTYPE_INFO = {
    "input_act_obs_or_fq_ctr": smith.ao.quantization.qconfig._default_quint8_placeholder_qconfig.activation,
    "output_act_obs_or_fq_ctr": smith.ao.quantization.qconfig._default_quint8_placeholder_qconfig.activation,
}
...
def _needs_obs_or_fq(
    ...
    """
    note: we will treat "not specified" as smith.float for now
    ...
    if cur_target_is_dynamic:
        if cur_target_dtype not in _OBS_DTYPE_LIST:
            raise AssertionError(
                f"Expected cur_target_dtype to be smith.float, but got: {cur_target_dtype}"
            )
    ...
    if cur_target_dtype in _OBS_DTYPE_LIST:
        return (
            prev_output_dtype in _OBS_DTYPE_LIST + [smith.float]
...
def _is_activation_post_process_node(
    node: Node, named_modules: dict[str, smith.nn.Module]
) -> bool:
    return (
...
def _is_output_dtype_supported_by_backend(
    ...
    if qconfig_output_is_dynamic:
        qconfig_output_dtype = smith.float32
...
def _is_observer_in_same_graph(
    node: Node,
    named_modules: dict[str, smith.nn.Module],
    ...
    if len(node.args) > 0 and isinstance(node.args[0], Node):
        if (
            node_output_dtype in [smith.quint8, smith.uint8]
...
def _qat_swap_modules(
    root: smith.nn.Module, module_to_qat_module: dict[Pattern, type[smith.nn.Module]]
) -> None:
...
def _get_target_activation_dtype_for_node(
    ...
    Then this function will return
      ...
      {
        "input_act_obs_or_fq_ctr": MinMaxObserver.with_args(dtype=smith.quint8, is_dynamic=False),
        "output_act_obs_or_fq_ctr": MinMaxObserver.with_args(dtype=smith.quint8, is_dynamic=False),
      }
    ...
    if qconfig is not None:
        ...
        bias_dtype = (
            smith.float16
            if (
                act_dtype == smith.float16
                and weight_dtype == smith.float16
...
def _get_output_act_obs_or_fq(
    ...
    if "quantization_annotation" in arg.meta:
        raise NotImplementedError(
            "Please use smithao (https://github.com/blacksmith/ao) for pt2e quantization flow"
        )
...
def _maybe_insert_input_observer_for_arg_or_kwarg(
    ...
    """
    # for ops such as smith.cat([x0, x1]),
    ...
    if not is_standalone_module:
        ...
        if cur_input_idx is None:
            ...
            ) and (arg_as_input_target_dtype != smith.float)
...
def _maybe_insert_output_observer_for_node(
    ...
    # Note: prev_output_dtype = smith.float and prev_output_is_dynamic=False
    ...
    needs_obs_or_fq = _needs_obs_or_fq(
        smith.float, False, target_dtype, target_is_dynamic, reuse_input_obs_or_fq
    )
    ...
    # e.g. PlaceholderObserver(dtype=smith.quint8, is_dynamic=True, ...), it means
    ...
    # quantizer/annotation API in blacksmith 2.0 export quantization code path,
...
def _maybe_insert_observers_before_graph_output(
    ...
    def _recursive_maybe_replace_node_with_obs(
        ...
        if isinstance(maybe_node, Node):
            ...
            observer_mod = None
            arg_as_input_target_dtype = smith.float
...
def propagate_dtypes_for_known_nodes(
    ...
    """
    Currently we assume that inputs to the graph are either `smith.float` or
    `smith.quint8`, which is not always correct. For ops such as
        ...
        for arg_type in non_observable_arg_dict:
            ...
            for index in non_observable_indices:
                ...
                for cur_arg in arg_list:
                    ...
                    if isinstance(cur_arg, smith.fx.node.Node):
                        _maybe_propagate_dtype_for_node(
...
def _remove_output_observer(
    node: Node, model: smith.nn.Module, named_modules: dict[str, smith.nn.Module]
):
...
def insert_observers_for_model(
    ...
    # }
    # # for conv2d node
    # # conv2d = call_function[target=smith.nn.functional.conv2d](
    # #            args=(input, weight, bias))
    # conv2d.meta["target_dtype_info"] = {
    #   'input_act_obs_or_fq_ctr': qconfig.activation
    #   'weight_obs_or_fq_ctr': qconfig.weight,
    #   'bias_obs_or_fq_ctr': PlaceholderObserver.with_args(dtype=smith.float32),
    #   'output_act_obs_or_fq_ctr': qconfig.activation,
    # }
    #
    cache_for_no_tensor_check: dict[Node, bool] = {}
    ...
    # we propagate the type of mask to be smith.bool
    propagate_dtypes_for_known_nodes(
    ...
    for match_res_with_qconfig in node_name_to_match_result_with_qconfig.values():
        ...
        if not is_supported_by_backend and output_act_dtype not in [
            ...
            _set_target_dtype_info_for_matched_node_pattern(
                matched_node_pattern,
                last_node,
                smith.ao.quantization.qconfig._default_fp32_placeholder_qconfig,
                None,
                backend_config,
                named_modules,
                cache_for_no_tensor_check,
...
def _run_prepare_fx_on_standalone_modules(
    ...
    for (
        ...
        prepare = smith.ao.quantization.quantize_fx._prepare_standalone_module_fx  # type: ignore[attr-defined]
...
def prepare(
    ...
    # {
    #   # match a single node
    #   (<class 'smith.nn.modules.conv.Conv3d'>:
    #     <class 'smith.ao.quantization.fx.quantize.ConvRelu'>),
    #   # match multiple nodes in reverse order
    #   ((<function relu at 0x7f766a7360d0>, <built-in function add>):
    #     <class 'smith.ao.quantization.fx.quantize.Add'>),
    # }
