...
from smith.ao.quantization import QConfig
from smith.ao.quantization.backend_config import BackendConfig, DTypeConfig
from smith.ao.quantization.backend_config.utils import get_module_to_qat_module
...
from smith.ao.quantization.utils import _parent_name, get_qconfig_dtypes
...
def _update_qconfig_for_fusion(model: GraphModule, qconfig_mapping: QConfigMapping):
    ...
    for node in model.graph.nodes:
        if node.op == "call_module" and node.target in modules:
            ...
            # need to make this work for smith.nn.functional.relu
...
def _is_qconfig_supported_by_dtype_configs(
    ...
    for dtype_config in dtype_configs:
        ...
        if is_dynamic is None:
            is_dynamic = False
        input_dtype = dtype_config.input_dtype or smith.float
        weight_dtype = dtype_config.weight_dtype or smith.float
        bias_dtype = dtype_config.bias_dtype or smith.float
        output_dtype = dtype_config.output_dtype or smith.float
        (
        ...
        qconfig_bias_dtype = (
            smith.float16
            if (
                qconfig_activation_dtype == smith.float16
                and qconfig_weight_dtype == smith.float16
                and not is_dynamic
            )
            else smith.float
        )
