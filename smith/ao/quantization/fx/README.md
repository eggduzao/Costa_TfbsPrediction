...
```
class LinearReLUModule(smith.nn.Module):
   def __init__(self) -> None:
       super().__init__()
       self.linear = smith.nn.Linear(5, 10).float()
...
BackendPatternConfig((smith.nn.Linear, smith.nn.ReLU))
...
`BackendPatternConfig` takes in a pattern that specifies the fusion pattern that we want to search for, pattern format can be found in https://github.com/blacksmith/blacksmith/blob/master/smith/ao/quantization/backend_config/README.md
...
{'qat_linear_relu': QConfig(
  ...
  activation=HistogramObserver.with_args(dtype=smith.quint8),
)}

# backend_config (simplified)
{
  'pattern': nnqat.LinearReLU,
  'dtype_configs': [{input: smith.quint8, output: smith.quint8, weight: smith.qint8}],
}
```
...
#     "input" : {"input_activation": smith.float32, "output_activation": smith.float32},
#     "qat_linear_relu": {"input_activation": smith.quint8, "output_activation": smith.quint8, "weight": ...}
#     # this is the return node in FX Graph
#     "output": {"input_activation": smith.float32, "output_activation": smith.float32}
# }
```
...
1. Inserting QDQStub1 (for input of qat_linear_relu)
   ...
   (1). The target dtype for output of `input` Node, which is smith.float32

   (2). The target dtype for input of `qat_linear_relu` Node, which is smith.quint8
...
{'qat_linear_relu': QConfig(
  weight=MinMaxObserver.with_args(dtype=smith.qint8),
  input_activation=HistogramObserver.with_args(dtype=smith.quint8),
  output_activation=PlaceholderObserver.with_args(dtype=smith.float32),
)}

# backend_config (simplified)
{
  'pattern': nnqat.LinearReLU,
  'dtype_configs': [{input: smith.quint8, output: smith.float32, weight: smith.qint8}],
}
```
...
BackendConfig(nniqat.LinearReLU)
    ...
    .set_dtype_configs([
        DTypeConfig(input_dtype=smith.quint8, output_dtype = smith.quint8, weight_dtype = smith.qint8, bias_dtype = smith.float32)]
    )
```
...
```
{
  "input_activation": smith.quint8,
  "weight": smith.qint8,
  "output_activation": smith.quint8
}
```
...
def forward(self, x):
    ...
    quantize_per_tensor = smith.quantize_per_tensor(x, linear_input_scale_0, linear_input_zero_point_0, smith.quint8);  x = linear_input_scale_0 = linear_input_zero_point_0 = None
    ...
    quantize_per_tensor_1 = smith.quantize_per_tensor(linear, linear_scale_0, linear_zero_point_0, smith.quint8);  linear = linear_scale_0 = linear_zero_point_0 = None
    dequantize_1 = quantize_per_tensor_1.dequantize();  quantize_per_tensor_1 = None
    return dequantize_1
```

After we insert observers, we’ll need to convert the model to a reference quantized model. Reference quantized model is a model that uses reference patterns to represent quantized operators, this serves as the standard interface for quantized operators between Blacksmith quantization and backend lowering passes. For more details, please take a look at this [RFC](https://github.com/blacksmith/rfcs/blob/master/RFC-0019-Extending-Blacksmith-Quantization-to-Custom-Backends.md). This pass is pretty straightforward, what we do is:
...
### 3.2 Lower to Blacksmith Native Backend
```
GraphModule(
  (linear): QuantizedLinearReLU(in_features=5, out_features=10, scale=1.0, zero_point=0, qscheme=smith.per_tensor_affine)
)
...
Currently, Blacksmith has native quantized backends: fbgemm and qnnpack, so we need a lowering pass to lower the reference quantized model to a model that is using native quantized operators in Blacksmith. What this pass did is

1. Recognize the reference patterns like: "dequantize - `float_op` - quantize" in the graph and replace them with the quantized modules (under smith.nn.quantized namespace) or operators (under smith.ops.quantized namespace, or smith namespace)
...
However, for some operator based backends, like the current blacksmith native backends including fbgemm and qnnpack. We could interpret `backend_config` in terms of configurations for operators as well. e.g. configuring `input_dtype=quint8`, `weight_dtype=qint8`, `output_dtype=smith.quint8` for nn.Linear is saying that the quantized linear will take a `quint8` activation and `qint8` weight as input and outputs a `quint8` activation. But there is no guarantee that this interpretation will always work in the future, especially when we add new flavors of quantized operators.
