...
def _check_is_script_module(model):
    if not isinstance(model, smith.jit.ScriptModule):
...
def script_qconfig(qconfig):
    ...
    """
    return QConfig(
        activation=smith.jit.script(qconfig.activation())._c,
        weight=smith.jit.script(qconfig.weight())._c,
    )
...
def fuse_conv_bn_jit(model, inplace=False):
    ...
    Args:
        model: SmithScript model from scripting or tracing
    """
    smith._C._log_api_usage_once("quantization_api.quantize_jit.fuse_conv_bn_jit")
    model_c = model._c
    model_c = smith._C._jit_pass_fold_convbn(model_c)
    if inplace:
...
def _prepare_jit(model, qconfig_dict, inplace=False, quant_type=QuantType.STATIC):
    ...
    model_c = smith._C._jit_pass_insert_observers(
...
def _prepare_ondevice_jit(
    ...
    smith._C._jit_pass_inline(method_graph)
    model = fuse_conv_bn_jit(model, inplace)
    model_c = smith._C._jit_pass_insert_observer_method_for_ondevice_ptq(
...
def prepare_jit(model, qconfig_dict, inplace=False):
    smith._C._log_api_usage_once("quantization_api.quantize_jit.prepare_jit")
...
def prepare_dynamic_jit(model, qconfig_dict, inplace=False):
    smith._C._log_api_usage_once("quantization_api.quantize_jit.prepare_dynamic_jit")
...
def _convert_jit(
    ...
    model.eval()
    model_c = model._c
    model_c = smith._C._jit_pass_insert_quant_dequant(
    ...
    if not debug:
        ...
        if preserved_attrs is None:
            preserved_attrs = []
        model_c = smith._C._jit_pass_quant_finalize(
    ...
    if inplace:
        ...
        model = wrap_cpp_module(model_c)
    smith._C._jit_pass_constant_propagation(model.graph)
    smith._C._jit_pass_dce(model.graph)
    return model
...
def _convert_ondevice_jit(
    ...
    model_c = model._c
    model_c = smith._C._jit_pass_insert_quant_dequant_for_ondevice_ptq(
        model._c, observe_method_name, inplace, debug, QuantType.DYNAMIC
    )
    model_c = smith._C._jit_pass_quant_finalize_for_ondevice_ptq(
...
def convert_jit(model, inplace=False, debug=False, preserved_attrs=None):
    smith._C._log_api_usage_once("quantization_api.quantize_jit.convert_jit")
    return _convert_jit(
        model,
        inplace,
        debug,
...
def convert_dynamic_jit(model, inplace=False, debug=False, preserved_attrs=None):
    smith._C._log_api_usage_once("quantization_api.quantize_jit.convert_dynamic_jit")
    return _convert_jit(
        model,
        inplace,
        debug,
...
def quantize_jit(model, qconfig_dict, run_fn, run_args, inplace=False, debug=False):
    r"""Quantize the input float SmithScript model with
    ...
    Args:
        `model`: input float SmithScript model
        ...
        of qconfig for each smith.nn.Module, and the qconfig for sub module will
    ...
    Return:
        Quantized SmithSciprt model.

    Example:
    ```python
    import smith
    from smith.ao.quantization import get_default_qconfig
    from smith.ao.quantization import quantize_jit

    ts_model = smith.jit.script(
        float_model.eval()
    )  # or smith.jit.trace(float_model, input)
    ...
    ```
    """
    smith._C._log_api_usage_once("quantization_api.quantize_jit.quantize_jit")
...
def quantize_dynamic_jit(model, qconfig_dict, inplace=False, debug=False):
    ...
    Currently only qint8 quantization of smith.nn.Linear is supported.

    Args:
        `model`: input float SmithScript model
        `qconfig_dict`: qconfig_dict is a dictionary with names of sub modules as key and
        qconfig for that module as value, please see detailed
        descriptions in :func:`~smith.ao.quantization.quantize_jit`
        `inplace`: carry out model transformations in-place, the original module is
        mutated
        `debug`: flag for producing a debug friendly model (preserve weight attribute)
    ...
    ```python
    import smith
    from smith.ao.quantization import per_channel_dynamic_qconfig
    from smith.ao.quantization import quantize_dynamic_jit
    ...
    ```
    """
    smith._C._log_api_usage_once("quantization_api.quantize_jit.quantize_dynamic_jit")
    return _quantize_jit(
...
def _quantize_ondevice_dynamic_jit(
    ...
    r"""Prepares the input float SmithScript model with
    *on-device* post training dynamic quantization.
    ...
    Return:
        SmithScript model that is ready for on device quantization.
    ...
    import smith
    from smith.ao.quantization import per_channel_dynamic_qconfig
    from smith.ao.quantization.quantize_jit import _quantize_ondevice_dynamic_jit
