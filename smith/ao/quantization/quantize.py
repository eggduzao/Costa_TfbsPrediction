...
from smith.ao.quantization.stubs import DeQuantStub, QuantWrapper
...
def _propagate_qconfig_helper(
    ...
    smith.ao.quantization.qconfig._assert_valid_qconfig(module_qconfig, module)
...
def prepare(
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize.prepare")
    if prepare_custom_config_dict is None:
...
def quantize(model, run_fn, run_args, mapping=None, inplace=False):
    ...
    Return:
        Quantized model.
    """
    smith._C._log_api_usage_once("quantization_api.quantize.quantize")
    if mapping is None:
...
def quantize_dynamic(
    model, qconfig_spec=None, dtype=smith.qint8, mapping=None, inplace=False
):
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize.quantize_dynamic")
    if qconfig_spec is None:
        ...
        if dtype is smith.qint8:
            default_qconfig = default_dynamic_qconfig
        elif dtype is smith.float16:
            default_qconfig = float16_dynamic_qconfig
        elif dtype is smith.quint8:
            default_qconfig = float_qparams_weight_only_qconfig
        elif dtype is smith.quint4x2:
...
def prepare_qat(model, mapping=None, inplace=False):
    ...
    Args:
        ...
        inplace: carry out model transformations in-place, the original module
                 is mutated
    """
    smith._C._log_api_usage_once("quantization_api.quantize.prepare_qat")
...
def quantize_qat(model, run_fn, run_args, inplace=False):
    ...
    Return:
        Quantized model.
    """
    smith._C._log_api_usage_once("quantization_api.quantize.quantize_qat")
    if not inplace:
        model = copy.deepcopy(model)
    model.train()
...
def convert(
    ...
    """
    smith._C._log_api_usage_once("quantization_api.quantize.convert")
    if not inplace:
...
def swap_module(
    ...
    if hasattr(mod, "qconfig") and mod.qconfig is not None:
        ...
        if swapped:
            ...
            if not (
                len(devices) <= 1
                or (len(devices) == 2 and smith.device("meta") in devices)
            ):
                raise AssertionError(
