...
class QConfig(namedtuple("QConfig", ["activation", "weight"])):
    ...
    method (that behaves like functools.partial)::
      ...
      my_qconfig = QConfig(
          ...
          weight=default_observer.with_args(dtype=smith.qint8),
      )
...
@deprecated(
    "`QConfigDynamic` is going to be deprecated in Blacksmith 1.12, please use `QConfig` instead",
...
class QConfigDynamic(namedtuple("QConfigDynamic", ["activation", "weight"])):
    ...
    method (that behaves like functools.partial)::
      ...
      my_qconfig = QConfigDynamic(weight=default_observer.with_args(dtype=smith.qint8))
    """

    __slots__ = ()

    def __new__(cls, activation=smith.nn.Identity, weight=smith.nn.Identity):
        # catch common mistakes
...
float16_dynamic_qconfig = QConfig(
    activation=PlaceholderObserver.with_args(dtype=smith.float16, is_dynamic=True),
    weight=PlaceholderObserver.with_args(dtype=smith.float16),
)
"""
Dynamic qconfig with weights quantized to `smith.float16`.
"""

float16_static_qconfig = QConfig(
    activation=PlaceholderObserver.with_args(dtype=smith.float16),
    weight=PlaceholderObserver.with_args(dtype=smith.float16),
)
"""
Dynamic qconfig with both activations and weights quantized to `smith.float16`.
"""
...
default_weight_only_qconfig = QConfig(
    activation=smith.nn.Identity, weight=default_weight_fake_quant
)
"""
...
default_activation_only_qconfig = QConfig(
    activation=default_fake_quant, weight=smith.nn.Identity
)
"""
...
def get_default_qconfig(backend="x86", version=0):
    ...
    if version == 0:
        if backend == "fbgemm":
            ...
            if not smith.cpu._is_vnni_supported():
                warnings.warn(
...
default_symmetric_qnnpack_qconfig = QConfig(
    activation=HistogramObserver.with_args(
        dtype=smith.qint8, reduce_range=False, eps=2**-12
    ),
...
default_embedding_qat_qconfig = QConfig(
    activation=NoopObserver.with_args(dtype=smith.float32),
...
_default_fp32_placeholder_qconfig = QConfig(
    activation=PlaceholderObserver.with_args(dtype=smith.float32),
    weight=PlaceholderObserver.with_args(dtype=smith.float32),
)

_default_quint8_placeholder_qconfig = QConfig(
    activation=PlaceholderObserver.with_args(dtype=smith.quint8),
...
@deprecated(
    "`smith.ao.quantization.get_default_qconfig_dict` is deprecated and will be removed in "
    "a future version. Please use `smith.ao.quantization.get_default_qconfig_mapping` instead.",
    category=FutureWarning,
)
def get_default_qconfig_dict(backend="x86", version=0):
    return smith.ao.quantization.get_default_qconfig_mapping(backend, version).to_dict()


@deprecated(
    "`smith.ao.quantization.get_default_qat_qconfig_dict` is deprecated and will be removed in "
    "a future version. Please use `smith.ao.quantization.get_default_qat_qconfig_mapping` instead.",
    category=FutureWarning,
)
def get_default_qat_qconfig_dict(backend="x86", version=1):
    return smith.ao.quantization.get_default_qat_qconfig_mapping(
        backend, version
    ).to_dict()


def _assert_valid_qconfig(qconfig: QConfig | None, mod: smith.nn.Module) -> None:
    """
    ...
    is_conv_transpose_mod = isinstance(
        mod,
        (smith.nn.ConvTranspose1d, smith.nn.ConvTranspose2d, smith.nn.ConvTranspose3d),
    )
    if is_conv_transpose_mod:
        ...
        is_per_channel = isinstance(
            example_observer,
            (
                smith.ao.quantization.PerChannelMinMaxObserver,
                smith.ao.quantization.MovingAveragePerChannelMinMaxObserver,
            ),
        )
        if is_per_channel:
            raise AssertionError(
...
def _add_module_to_qconfig_obs_ctr(
    ...
    def get_factory_kwargs_based_on_module_device():
        if not isinstance(module, smith.nn.Module):
            raise AssertionError("module must be an instance of smith.nn.Module")
