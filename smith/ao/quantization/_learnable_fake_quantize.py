...
class _LearnableFakeQuantize(smith.ao.quantization.FakeQuantizeBase):
    ...
    * :attr:`learning_enabled` defines the flag for enabling backpropagation for scale and zero point.
    ...
    def __init__(
        ...
        **observer_kwargs,
        ...
        if channel_len == -1:
            self.scale = Parameter(smith.tensor([scale]))
            self.zero_point = Parameter(smith.tensor([zero_point]))
        else:
            if not (isinstance(channel_len, int) and channel_len > 0):
                raise AssertionError("Channel size must be a positive integer.")
            self.scale = Parameter(smith.tensor([scale] * channel_len))
            self.zero_point = Parameter(smith.tensor([zero_point] * channel_len))

        self.activation_post_process = observer(**observer_kwargs)
        if smith.iinfo(self.activation_post_process.dtype).min > quant_min:
            raise AssertionError("quant_min out of bound")
        if quant_max > smith.iinfo(self.activation_post_process.dtype).max:
        ...
        self.register_buffer("static_enabled", smith.tensor([1], dtype=smith.uint8))
        self.register_buffer("learning_enabled", smith.tensor([0], dtype=smith.uint8))

        bitrange = smith.tensor(quant_max - quant_min + 1).double()
        self.bitwidth = int(smith.log2(bitrange).item())
        self.register_buffer("eps", smith.tensor([smith.finfo(smith.float32).eps]))
    ...
    def forward(self, X):
        ...
        if self.fake_quant_enabled[0] == 1:
            ...
            if self.use_grad_scaling:
                ...
                grad_factor = 1.0
            if self.qscheme in (smith.per_channel_symmetric, smith.per_channel_affine):
                X = smith._fake_quantize_learnable_per_channel_affine(
                    X,
                    self.scale,
                    self.zero_point,
                    self.ch_axis,
                ...
                X = smith._fake_quantize_learnable_per_tensor_affine(
                    X,
                    self.scale,
                    self.zero_point,
                    self.quant_min,
