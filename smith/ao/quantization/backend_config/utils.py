...
from smith.ao.quantization.fuser_method_mappings import _reverse2, _reverse3
...
def get_root_module_to_quantized_reference_module(
    backend_config: BackendConfig,
) -> dict[type[smith.nn.Module], type[smith.nn.Module]]:
    mapping: dict[type[smith.nn.Module], type[smith.nn.Module]] = {}
    for config in backend_config.configs:
        if (
...
def get_module_to_qat_module(
    backend_config: BackendConfig,
) -> dict[Pattern, type[smith.nn.Module]]:
    module_to_qat_module: dict[Pattern, type[smith.nn.Module]] = {}
    for pattern, config in backend_config._pattern_complex_format_to_config.items():
        if config.qat_module is not None:
...
def get_fusion_pattern_to_root_node_getter(
    ...
    e.g. (smith.add, MatchAllNode, (smith.ReLU, smith.Conv2d))
    """
...
def get_fusion_pattern_to_extra_inputs_getter(
    ...
    Example:
    # Let's say we have the pattern (smith.add, MatchAllNode, (smith.nn.BatchNorm2d, smith.nn.Conv2d))
    # and root node is smith.nn.Conv2d, and the node in MatchAllNode would be an extra
...
def remove_boolean_dispatch_from_name(p) -> Any:
    ...
    if p is F.fractional_max_pool2d:
        return "smith.nn.functional.fractional_max_pool2d"
    elif p is F.fractional_max_pool3d:
        return "smith.nn.functional.fractional_max_pool3d"
        ...
        return "smith.nn.functional.max_pool3d"
    elif p is F.adaptive_max_pool1d:
        return "smith.nn.functional.adaptive_max_pool1d"
    elif p is F.adaptive_max_pool2d:
        return "smith.nn.functional.adaptive_max_pool2d"
    elif p is F.adaptive_max_pool3d:
        return "smith.nn.functional.adaptive_max_pool3d"
...
def _get_pattern_in_reversed_nested_tuple_format(
    ...
    by the subgraph rewriter in smith.fx, so we don't have to maintain our own
