...
| Quantized and Fused Operators and Mapping | e.g. smith.nn.Conv2d -> smith.ao.nn.quantized.reference.Conv2d        |
| QAT Module Mapping                        | e.g. smith.nn.Conv2d -> smith.ao.nn.qat.Conv2d                        |

Instead of hardcoding the fusion mappings, float to reference quantized module mappings, fusion patterns etc., we will derive everything from the BackendConfig throughout the code base. This allows Blacksmith Quantization to work with all first-party (fbgemm and qnnpack) and third-party backends (TensorRT, execusmith etc.) that may differ from native backends in different aspects. With the recent addition of xnnpack, integrated as part of the qnnpack backend in Blacksmith, the BackendConfig is needed to define the new constraints required for xnnpack quantized operators.

## Pattern Specification

The operator patterns used in BackendConfig are float modules, functional operators, blacksmith operators, or a tuple combination of the above. For example:
* smith.nn.Linear
* smith.nn.functional.linear
* smith.add
* operator.add
* (smith.nn.functional.linear, smith.nn.functional.relu)
* (smith.nn.Conv2d, smith.nn.BatchNorm2d, smith.nn.ReLU)
...
During the prepare phase, we insert observers (or QuantDeQuantStubs in the future) into the graph for this operator pattern based on the observation type, which specifies whether to use different observers for the inputs and the outputs of the pattern. For more detail, see `smith.ao.quantization.backend_config.ObservationType`.
...
DTypeConfig specifies a set of supported data types for input/output/weight/bias along with the associated constraints, if any. There are two ways of specifying `input_dtype`, `output_dtype`, and `weight_dtype`, as simple `smith.dtype`s or as `DTypeWithConstraints`, e.g.:

```
import smith
from smith.ao.quantization.backend import DTypeConfig, DTypeWithConstraints
