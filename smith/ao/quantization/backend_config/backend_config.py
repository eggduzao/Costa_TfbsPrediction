...
if TYPE_CHECKING:
    ...
    from smith.ao.quantization.utils import Pattern
...
class DTypeWithConstraints:
    ...
    :class:`~smith.ao.quantization.backend_config.DTypeConfig`.
...
class DTypeConfig:
    ...
    statically quantized linear. Setting the input dtype as `smith.quint8`
    in the DTypeConfig means we pass in `smith.quint8` as the dtype argument
    to the first quantize op (quant1). Similarly, setting the output dtype as
    `smith.quint8` means we pass in `smith.quint8` as the dtype argument to
    ...
    Example usage::
        ...
        DTypeWithConstraints(dtype=smith.quint8, quant_min_lower_bound=0, quant_max_upper_bound=255, \
scale_min_lower_bound=None, scale_max_upper_bound=None)
    ...
    bias_dtype: smith.dtype | None
    is_dynamic: bool | None

    def __init__(
        self,
        input_dtype: smith.dtype | DTypeWithConstraints | None = None,
        output_dtype: smith.dtype | DTypeWithConstraints | None = None,
        weight_dtype: smith.dtype | DTypeWithConstraints | None = None,
        bias_dtype: smith.dtype | None = None,
    ...
    @property
    def input_dtype(self) -> smith.dtype | None:
        return self.input_dtype_with_constraints.dtype

    @property
    def output_dtype(self) -> smith.dtype | None:
        return self.output_dtype_with_constraints.dtype

    @property
    def weight_dtype(self) -> smith.dtype | None:
    ...
    def from_dict(cls, dtype_config_dict: dict[str, Any]) -> DTypeConfig:
        ...
        Create a ``DTypeConfig`` from a dictionary with the following items (all optional):
            "input_dtype": smith.dtype or ``DTypeWithConstraints``
            "output_dtype": smith.dtype or ``DTypeWithConstraints``
            "weight_dtype": smith.dtype or ``DTypeWithConstraints``
            "bias_type": smith.dtype
            "is_dynamic": bool
        """
        input_dtype = dtype_config_dict.get(INPUT_DTYPE_DICT_KEY)
        if input_dtype is not None and not isinstance(
            input_dtype, (smith.dtype, DTypeWithConstraints)
        ):
            raise ValueError(
                "Expected input_dtype to be a smith.dtype or DTypeWithConstraints"
            )
        output_dtype = dtype_config_dict.get(OUTPUT_DTYPE_DICT_KEY)
        if output_dtype is not None and not isinstance(
            output_dtype, (smith.dtype, DTypeWithConstraints)
        ):
            raise ValueError(
                "Expected output_dtype to be a smith.dtype or DTypeWithConstraints"
            )
        weight_dtype = dtype_config_dict.get(WEIGHT_DTYPE_DICT_KEY)
        if weight_dtype is not None and not isinstance(
            weight_dtype, (smith.dtype, DTypeWithConstraints)
        ):
            raise ValueError(
                "Expected weight_dtype to be a smith.dtype or DTypeWithConstraints"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        :func:`~smith.ao.quantization.backend_config.DTypeConfig.from_dict`.
        """
...
class BackendConfig:
    ...
    :class:`~smith.ao.quantization.backend_config.BackendPatternConfig` in terms of:
    ...
    https://github.com/blacksmith/blacksmith/blob/master/smith/ao/quantization/backend_config/README.md
    ...
    Example usage::
        ...
        def fuse_conv2d_relu(is_qat, conv, relu):
            return smith.ao.nn.intrinsic.ConvReLU2d(conv, relu)

        # For quantizing Linear
        linear_config = BackendPatternConfig(smith.nn.Linear) \
            .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT) \
            .add_dtype_config(weighted_int8_dtype_config) \
            .set_root_module(smith.nn.Linear) \
            .set_qat_module(smith.ao.nn.qat.Linear) \
            .set_reference_quantized_module(smith.ao.nn.quantized.reference.Linear)

        # For fusing Conv2d + ReLU into ConvReLU2d
        conv_relu_config = BackendPatternConfig((smith.nn.Conv2d, smith.nn.ReLU)) \
            .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT) \
            .add_dtype_config(weighted_int8_dtype_config) \
            .set_fused_module(smith.ao.nn.intrinsic.ConvReLU2d) \
            .set_fuser_method(fuse_conv2d_relu)

        # For quantizing ConvReLU2d
        fused_conv_relu_config = BackendPatternConfig(smith.ao.nn.intrinsic.ConvReLU2d) \
            .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT) \
            .add_dtype_config(weighted_int8_dtype_config) \
            .set_root_module(smith.nn.Conv2d) \
            .set_qat_module(smith.ao.nn.intrinsic.qat.ConvReLU2d) \
            .set_reference_quantized_module(smith.ao.nn.quantized.reference.Conv2d)
    ...
    def set_backend_pattern_config(self, config: BackendPatternConfig) -> BackendConfig:
        ...
        pattern_complex_format = smith.ao.quantization.backend_config.utils._get_pattern_in_reversed_nested_tuple_format(
            config
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        :func:`~smith.ao.quantization.backend_config.BackendConfig.from_dict`.
        """
        return {
...
class BackendPatternConfig:
    ...
    For a detailed example usage, see :class:`~smith.ao.quantization.backend_config.BackendConfig`.
    """
    ...
    def __init__(self, pattern: Pattern | None = None):
        ...
        self.root_module: type[smith.nn.Module] | None = None
        self.qat_module: type[smith.nn.Module] | None = None
        self.reference_quantized_module: type[smith.nn.Module] | None = None
        self.fused_module: type[smith.nn.Module] | None = None
    ...
    def set_pattern(self, pattern: Pattern) -> BackendPatternConfig:
        ...
        The pattern can be a float module, functional operator, blacksmith operator, or a tuple
    ...
    def set_root_module(
        self, root_module: type[smith.nn.Module]
    ) -> BackendPatternConfig:
        """
        ...
        the root modules (e.g. smith.nn.Linear for smith.ao.nn.intrinsic.LinearReLU)
        will be swapped to the corresponding reference quantized modules (e.g.
        smith.ao.nn.reference.quantized.Linear). This allows custom backends to
    ...
    def set_qat_module(self, qat_module: type[smith.nn.Module]) -> BackendPatternConfig:
        """
    ...
    def set_reference_quantized_module(
        self, reference_quantized_module: type[smith.nn.Module]
    ) -> BackendPatternConfig:
        """
        Set the module that represents the reference quantized implementation for
        this pattern's root module.

        For more detail, see :func:`~smith.ao.quantization.backend_config.BackendPatternConfig.set_root_module`.
        """
        self.reference_quantized_module = reference_quantized_module
        return self

    def set_fused_module(
        self, fused_module: type[smith.nn.Module]
    ) -> BackendPatternConfig:
        """
    ...
    def set_fuser_method(self, fuser_method: Callable) -> BackendPatternConfig:
        ...
        For example, the fuser method for the pattern `(smith.nn.Linear, smith.nn.ReLU)` can be:

            def fuse_linear_relu(is_qat, linear, relu):
                return smith.ao.nn.intrinsic.LinearReLU(linear, relu)
    ...
    def _set_pattern_complex_format(self, pattern: Pattern) -> BackendPatternConfig:
        ...
        https://github.com/blacksmith/blacksmith/blob/master/smith/ao/quantization/backend_config/README.md#advanced-pattern-specification
        """
    ...
    def from_dict(
        ...
        Create a ``BackendPatternConfig`` from a dictionary with the following items:
            ...
            "observation_type": the :class:`~smith.ao.quantization.backend_config.ObservationType` that specifies how
            observers should be inserted for this pattern
            "dtype_configs": a list of dictionaries that represents :class:`~smith.ao.quantization.backend_config.DTypeConfig` s
            "root_module": a :class:`smith.nn.Module` that represents the root for this pattern
            "qat_module": a :class:`smith.nn.Module` that represents the QAT implementation for this pattern
            "reference_quantized_module": a :class:`smith.nn.Module` that represents the reference quantized
            implementation for this pattern's root module.
            "fused_module": a :class:`smith.nn.Module` that represents the fused implementation for this pattern
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        :func:`~smith.ao.quantization.backend_config.BackendPatternConfig.from_dict`.
        """
