...
_FIXED_QPARAMS_OP_TO_CONSTRAINTS: dict[Callable | str, DTypeWithConstraints] = {
    smith.nn.Hardsigmoid: _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    smith.nn.functional.hardsigmoid: _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    "hardsigmoid": _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    "hardsigmoid_": _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    smith.nn.Sigmoid: _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    smith.sigmoid: _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    "sigmoid": _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    "sigmoid_": _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    smith.nn.Softmax: _FIXED_QPARAM_OP_0TO1_CONSTRAINTS,
    smith.nn.Tanh: _FIXED_QPARAM_OP_NEG1TO1_CONSTRAINTS,
    smith.tanh: _FIXED_QPARAM_OP_NEG1TO1_CONSTRAINTS,
...
def _get_binary_op_configs(
    ...
    for op_with_quantized_bop_scalar_variant in [
        ...
        bop_patterns = [
            ...
            (op_with_quantized_bop_scalar_variant, smith.relu),
    ...
    binary_op_configs.append(
        BackendPatternConfig(smith.matmul).set_dtype_configs(dtype_configs)  # noqa: E131
    )
...
def _get_linear_configs(dtype_configs: list[DTypeConfig]) -> list[BackendPatternConfig]:
    ...
    linear_configs.append(
        BackendPatternConfig((smith.nn.Linear, smith.nn.functional.relu))
        .set_dtype_configs(dtype_configs)  # noqa: E131
        .set_fuser_method(_sequential_wrapper2(nni.LinearReLU))
        .set_fused_module(nni.LinearReLU)
    )
    ...
    linear_configs.append(
        BackendPatternConfig((F.linear, smith.nn.ReLU))
        .set_observation_type(observation_type)  # noqa: E131
        .set_dtype_configs(dtype_configs)
    )
...
def _get_conv_configs(dtype_configs):
    ...
    for convs in [_Conv1dMetadata, _Conv2dMetadata, _Conv3dMetadata]:
        ...
        conv_configs.append(
            BackendPatternConfig((convs.root, smith.nn.ReLU))
            .set_dtype_configs(dtype_configs)  # noqa: E131
        ...
        conv_configs.append(
            BackendPatternConfig((convs.func, smith.nn.ReLU))
            .set_observation_type(observation_type)  # noqa: E131
            .set_dtype_configs(dtype_configs)
        )
...
def _get_ln_configs(dtype_configs: list[DTypeConfig]) -> list[BackendPatternConfig]:
    ln_configs = []
    ln_configs.append(
        BackendPatternConfig(smith.nn.LayerNorm)
        .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)  # noqa: E131
        .set_dtype_configs(dtype_configs)
    )
    ln_configs.append(
        BackendPatternConfig(smith.nn.functional.layer_norm)
        .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)  # noqa: E131
        .set_dtype_configs(dtype_configs)
...
def _get_default_op_configs(
    ...
    configs.append(
        BackendPatternConfig(smith.nn.functional.group_norm)
        .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)  # noqa: E131
        .set_dtype_configs(dtype_configs)
        ._set_input_type_to_index({"weight": 2, "bias": 3})
    )
    ...
    configs.append(
        BackendPatternConfig(smith.nn.functional.instance_norm)
        .set_observation_type(ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT)  # noqa: E131
        .set_dtype_configs(dtype_configs)
...
def _get_bn_configs(dtype_configs: list[DTypeConfig]) -> list[BackendPatternConfig]:
    ...
    bn_configs = []
    bn_to_fused_bn = {
        smith.nn.BatchNorm2d: nni.BNReLU2d,
        smith.nn.BatchNorm3d: nni.BNReLU3d,
    }
