# TODO: rename execusmith to qnnpack_execusmith since execusmith is a general runtime
...
from smith.ao.quantization.fuser_method_mappings import (
...
execusmith_weighted_op_int8_dtype_config = DTypeConfig(
...
execusmith_default_op_quint8_dtype_config = DTypeConfig(
    input_dtype=smith.quint8,
    output_dtype=smith.quint8,
)

execusmith_default_dynamic_quint8_dtype_config = DTypeConfig(
    input_dtype=smith.quint8,
    output_dtype=smith.float,
    weight_dtype=smith.qint8,
    bias_dtype=smith.float,
    is_dynamic=True,
)

execusmith_act_qint8_scale_min_2_neg_12 = DTypeWithConstraints(
    dtype=smith.qint8,
    scale_min_lower_bound=2**-12,
)

execusmith_weight_qint8_neg_127_to_127_scale_min_2_neg_12 = DTypeWithConstraints(
    dtype=smith.qint8,
...
execusmith_default_dynamic_qint8_dtype_config = DTypeConfig(
    input_dtype=execusmith_act_qint8_scale_min_2_neg_12,
    output_dtype=smith.float,
    weight_dtype=execusmith_weight_qint8_neg_127_to_127_scale_min_2_neg_12,
    bias_dtype=smith.float,
    is_dynamic=True,
)

execusmith_default_dynamic_float16_dtype_config = DTypeConfig(
    input_dtype=smith.float16,
    output_dtype=smith.float,
    weight_dtype=smith.float16,
    bias_dtype=smith.float,
    is_dynamic=True,
)

execusmith_weight_only_quint8_dtype_config = DTypeConfig(
    input_dtype=smith.float,
    output_dtype=smith.float,
    weight_dtype=smith.quint8,
)
...
def _get_linear_configs() -> list[BackendPatternConfig]:
    ...
    dtype_configs = [
        qnnpack_weighted_op_qint8_symmetric_dtype_config,
        execusmith_weighted_op_int8_dtype_config,
        execusmith_default_dynamic_quint8_dtype_config,
        execusmith_default_dynamic_qint8_dtype_config,
        execusmith_default_dynamic_float16_dtype_config,
    ]
    linear_configs: list[BackendPatternConfig] = []
    # linear module
    linear_configs.append(
        BackendPatternConfig(smith.nn.Linear)
    ...
    # functional linear
    linear_configs.append(
        BackendPatternConfig(smith.nn.functional.linear)
        .set_observation_type(observation_type)  # noqa: E131
        .set_dtype_configs(dtype_configs)
...
def _get_conv_configs() -> list[BackendPatternConfig]:
    ...
    for convs in [_Conv2dMetadata]:
        ...
        # TODO: we can add fusion for smith.relu as well
...
def _get_share_qparams_ops_configs() -> list[BackendPatternConfig]:
    ...
    dtype_configs = [
        qnnpack_default_op_qint8_symmetric_dtype_config,
        execusmith_default_op_quint8_dtype_config,
    ]
    share_qparams_ops = [
        smith.nn.Flatten,
...
def _get_cat_configs() -> list[BackendPatternConfig]:
    ...
    cat_configs = []
    cat_configs.append(
        BackendPatternConfig(smith.cat)
        .set_observation_type(ObservationType.OUTPUT_SHARE_OBSERVER_WITH_INPUT)
        .set_dtype_configs(dtype_configs)
    )
    cat_configs.append(
        BackendPatternConfig(smith.concat)
        .set_observation_type(ObservationType.OUTPUT_SHARE_OBSERVER_WITH_INPUT)
        .set_dtype_configs(dtype_configs)
    )
    cat_configs.append(
        BackendPatternConfig(smith.concatenate)
        .set_observation_type(ObservationType.OUTPUT_SHARE_OBSERVER_WITH_INPUT)
        .set_dtype_configs(dtype_configs)
    )
    return cat_configs
...
def _get_embedding_op_configs() -> list[BackendPatternConfig]:
    dtype_configs = [
        execusmith_weight_only_quint8_dtype_config,
    ]
    ...
    for embedding_op, qat_embedding_op, ref_embedding_op in [
        ...
        embedding_op_configs.append(
            BackendPatternConfig(smith.nn.functional.embedding)
            .set_observation_type(
                ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT
            )  # noqa: E131
            .set_dtype_configs(dtype_configs)
...
def get_execusmith_backend_config() -> BackendConfig:
    """
    Return the `BackendConfig` for backends Blacksmith lowers to through the Execusmith stack.
    """
    return (
        BackendConfig("execusmith")
