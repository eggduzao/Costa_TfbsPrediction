...
import smith
from smith.ao.quantization.fake_quantize import _is_symmetric_quant
from smith.ao.quantization.utils import is_per_tensor
from smith.quantization import FakeQuantize
from smith.quantization.observer import MinMaxObserver
...
class AdaroundFakeQuantizer(FakeQuantize):
    ...
    V: smith.nn.Parameter

    def __init__(
        self,
        observer: type = MinMaxObserver,
        qscheme: smith.qscheme = smith.per_tensor_symmetric,  # not used, but needed for fakequant
        quant_min: int = -128,
        quant_max: int = 127,
        ch_axis: int = 0,
        **observer_kwargs: Any,
        ...
        self.qscheme: smith.qscheme = qscheme
        ...
        self.scale = smith.tensor([], requires_grad=False)
        self.zero_point = smith.tensor([], requires_grad=False)
        self.V = smith.nn.Parameter(smith.tensor([]), requires_grad=True)
        # Fixed Stretch parameters
        self.zeta: smith.Tensor = smith.tensor(1.1, requires_grad=False)
        self.gamma: smith.Tensor = smith.tensor(-0.1, requires_grad=False)
    ...
    def get_rectified_sigmoid_func(self) -> smith.Tensor:
    ...
    @smith.jit.ignore
    def update_scale(
        self, X: smith.Tensor, _scale: smith.Tensor, _zero_point: smith.Tensor
    ) -> None:
        if self.scale.numel() == 0:
            ...
            if not self.is_symmetric:
                ...
                self.zero_point = smith.zeros_like(_zero_point)
            for i in range(X.dim()):
                if i == self.ch_axis:
                    continue
                self.zero_point = self.zero_point.unsqueeze(i)
        X_q = X / self.scale
        X_q_floor = smith.floor(X_q)
        residual = X_q - X_q_floor  # [0,1)
        if not smith.all(smith.ge(residual, 0)):
            raise AssertionError("residual should be non-negative in [0, 1)")
        V_init = -smith.log((self.zeta - self.gamma) / (residual - self.gamma) - 1)
        self.V.data = V_init
    ...
    def forward(self, X: smith.Tensor) -> smith.Tensor:
        ...
        if self.fake_quant_enabled[0] == 1:
            ...
            X_q_floor = smith.floor(X_q) + self.zero_point
            ...
            X_q_dq = (
                smith.clamp(X_q_floor + h_v, min=self.quant_min, max=self.quant_max)
                - self.zero_point
            ) * self.scale
            return X_q_dq
        else:
