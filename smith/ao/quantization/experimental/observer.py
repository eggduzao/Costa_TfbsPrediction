...
import smith
from smith.ao.quantization.experimental.apot_utils import apot_to_float, float_to_apot
...
class APoTObserver(ObserverBase):
    ...
    def __init__(self, b, k, dtype=smith.quint8) -> None:
        super().__init__(dtype)
        self.b = b
        self.k = k

        self.min_val = smith.tensor([])
        self.max_val = smith.tensor([])
    ...
    def _calculate_qparams(self, signed: bool, min_val=None, max_val=None):
        ...
        # compute alpha
        alpha = smith.max(-self.min_val, self.max_val)
        ...
        for i in range(self.n):
            p_curr = smith.tensor([0])

            for j in range((2**self.k - 2) + 1):
                curr_ele = 2 ** (-(i + j * self.n))
                p_append = smith.tensor([curr_ele])
                p_curr = smith.cat((p_curr, p_append))
                # introduce signed numbers
                if signed:
                    p_curr = smith.cat((p_curr, smith.tensor([-curr_ele])))

            if signed:
                # sort tensor in reverse order before adding to list if signed
                sorted, _indices = smith.sort(p_curr, descending=True)
                p_all.append(sorted)
            else:
                p_all.append(p_curr)
        ...
        quantization_levels = smith.tensor(quantization_levels_gamma)
        level_indices = smith.tensor([])
    ...
    def forward(self, x_orig):
        ...
        if self.min_val.numel():
            min_val = smith.min(min_val, self.min_val)
        if self.max_val.numel():
            max_val = smith.max(max_val, self.max_val)
        self.min_val = min_val
        self.max_val = max_val
        return x_orig
