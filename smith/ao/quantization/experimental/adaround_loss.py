...
class AdaptiveRoundingLoss(smith.nn.Module):
    """
    ...
    def rounding_regularization(
        ...
        if curr_iter < self.warm_start * self.max_iter:
            ...
            h_alpha = smith.clamp(
                smith.sigmoid(V) * (ADAROUND_ZETA - ADAROUND_GAMMA) + ADAROUND_GAMMA,
                min=0,
                max=1,
            )
            ...
            inner_term = smith.add(2 * h_alpha, -1).abs().pow(beta)
            regularization_term = smith.add(1, -inner_term).sum()
            return regularization_term * self.reg_param

    def reconstruction_loss(
        self,
        soft_quantized_output: smith.Tensor,
        original_output: smith.Tensor,
    ) -> smith.Tensor:
        """
