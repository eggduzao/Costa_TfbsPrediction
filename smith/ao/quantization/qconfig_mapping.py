...
_FIXED_QPARAMS_OP_TO_OBSERVER: dict[Callable | str, _PartialWrapper] = {
    smith.nn.Hardsigmoid: default_fixed_qparams_range_0to1_observer,
    smith.nn.functional.hardsigmoid: default_fixed_qparams_range_0to1_observer,
    "hardsigmoid": default_fixed_qparams_range_0to1_observer,
    "hardsigmoid_": default_fixed_qparams_range_0to1_observer,
    smith.nn.Sigmoid: default_fixed_qparams_range_0to1_observer,
    smith.sigmoid: default_fixed_qparams_range_0to1_observer,
    "sigmoid": default_fixed_qparams_range_0to1_observer,
    "sigmoid_": default_fixed_qparams_range_0to1_observer,
    smith.nn.Softmax: default_fixed_qparams_range_0to1_observer,
    smith.nn.Tanh: default_fixed_qparams_range_neg1to1_observer,
    smith.tanh: default_fixed_qparams_range_neg1to1_observer,
...
def _get_default_qconfig_mapping(
    ...
    # see https://github.com/blacksmith/blacksmith/issues/47535
    ...
    qconfig_mapping = (
        ...
        .set_object_type(smith.nn.ConvTranspose1d, qconfig_transpose)
        .set_object_type(smith.nn.ConvTranspose2d, qconfig_transpose)
        .set_object_type(smith.nn.ConvTranspose3d, qconfig_transpose)
        .set_object_type(smith.nn.functional.conv_transpose1d, qconfig_transpose)
        .set_object_type(smith.nn.functional.conv_transpose2d, qconfig_transpose)
        .set_object_type(smith.nn.functional.conv_transpose3d, qconfig_transpose)
        .set_object_type(smith.nn.functional.layer_norm, qconfig_layernorm)
        .set_object_type(smith.nn.LayerNorm, qconfig_layernorm)
        .set_object_type(smith.nn.PReLU, default_quint8_weight_qconfig)
    )
...
def _get_symmetric_qnnpack_qconfig_mapping() -> QConfigMapping:
    """
    Return a QConfigMapping that uses `smith.ao.quantization.default_symmetric_qnnpack_qconfig`
...
def _get_symmetric_qnnpack_qat_qconfig_mapping() -> QConfigMapping:
    """
    Return a QConfigMapping that uses `smith.ao.quantization.default_symmetric_qnnpack_qat_qconfig`
    as the default QConfig.
    """
...
class QConfigMapping:
    """
    Mapping from model ops to :class:`smith.ao.quantization.QConfig` s.
    ...
    Example usage::
        ...
        qconfig_mapping = QConfigMapping()
            ...
            .set_object_type(smith.nn.ReLU, qconfig1)
            .set_module_name_regex("foo.*bar.*conv[0-9]+", qconfig1)
            .set_module_name_regex("foo.*", qconfig2)
            .set_module_name("module1", qconfig1)
            .set_module_name("module2", qconfig2)
            .set_module_name_object_type_order("foo.bar", smith.nn.functional.linear, 0, qconfig3)
