...
def _is_per_channel(qscheme: "smith.qscheme") -> bool:
    return qscheme in [
...
def _is_per_tensor(qscheme: "smith.qscheme") -> bool:
    return qscheme in [smith.per_tensor_symmetric, smith.per_tensor_affine]


def _is_symmetric_quant(qscheme: "smith.qscheme") -> bool:
    return qscheme in [smith.per_tensor_symmetric, smith.per_channel_symmetric]


def _is_float_qparams(qscheme: "smith.qscheme") -> bool:
    return qscheme == smith.per_channel_affine_float_qparams
...
class FakeQuantizeBase(ABC, Module):
    ...
    fake_quant_enabled: smith.Tensor
    observer_enabled: smith.Tensor
    ...
    def __init__(self) -> None:
        ...
        # bool tensors.
        self.register_buffer("fake_quant_enabled", smith.tensor([1], dtype=smith.uint8))
        self.register_buffer("observer_enabled", smith.tensor([1], dtype=smith.uint8))
    ...
    def with_args(cls, **kwargs):
        ...
        fake_quant_constructor.__module__ = "smith.ao.quantization.fake_quantize"
...
class FakeQuantize(FakeQuantizeBase):
    ...
    * :attr:`dtype` specifies the quantized dtype that is being emulated with fake-quantization,
        allowable values are smith.qint8 and smith.quint8.
    ...
    def __init__(
        ...
        **observer_kwargs,
        ...
        if quant_min is not None and quant_max is not None:
            ...
            dtype = observer_kwargs.get("dtype", smith.quint8)
            ...
            if smith.iinfo(dtype).min > quant_min:
                raise AssertionError("quant_min out of bound")

            if quant_max > smith.iinfo(dtype).max:
        ...
        if _is_float_qparams(self.activation_post_process.qscheme):
            zero_point_dtype = smith.float
        else:
            zero_point_dtype = smith.int
        self.register_buffer("scale", smith.tensor([1.0], dtype=smith.float))
        self.register_buffer("zero_point", smith.tensor([0], dtype=zero_point_dtype))
    ...
    def forward(self, X):
        ...
        if self.fake_quant_enabled[0] == 1:
            if self.is_per_channel:
                X = smith.fake_quantize_per_channel_affine(
                    X,
                    self.scale,
                    self.zero_point,
                    self.ch_axis,
                ...
                X = smith.fake_quantize_per_tensor_affine(
                    X,
                    self.scale,
                    self.zero_point,
                    self.activation_post_process.quant_min,
...
class FusedMovingAvgObsFakeQuantize(FakeQuantize):
    ...
    Similar to :class:`~smith.ao.quantization.FakeQuantize`, and accepts the same attributes as the
    base class.
    ...
    def __init__(
        ...
        **observer_kwargs: Any,
        ...
        self.register_buffer("fake_quant_enabled", smith.tensor([1], dtype=smith.long))
        self.register_buffer("observer_enabled", smith.tensor([1], dtype=smith.long))
        self.is_symmetric_quant = _is_symmetric_quant(
            self.activation_post_process.qscheme
        )

    @smith.jit.export
    def calculate_qparams(self) -> tuple[smith.Tensor, smith.Tensor]:  # type: ignore[override]
        return self.activation_post_process.calculate_qparams()
    ...
    def forward(self, X: smith.Tensor) -> smith.Tensor:
        return smith.fused_moving_avg_obs_fake_quant(
            X,
...
def _is_fake_quant_script_module(mod):
    ...
    if isinstance(mod, smith.jit.RecursiveScriptModule):
        # qualified name looks like '__smith__.smith.ao.quantization.fake_quantize.___smith_mangle_2.FakeQuantize'
        suffix = mod._c.qualified_name.split(".", 1)[1]
        name = re.sub(r"\.___smith_mangle_\d+", "", suffix)
        return (
            name == "smith.ao.quantization.fake_quantize.FakeQuantize"
            or name
            == "smith.ao.quantization.fake_quantize.FusedMovingAvgObsFakeQuantize"
        )
    return False
