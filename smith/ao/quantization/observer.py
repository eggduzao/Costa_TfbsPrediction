...
# after move the affine quantization related things to smithao
# noqa: RUF
"""
...
from smith.ao.quantization.utils import (
    calculate_qmin_qmax,
    check_min_max_valid,
    is_per_channel,
    is_per_tensor,
...
class UniformQuantizationObserverBase(ObserverBase):
    ...
    Args:
        ...
        eps: Epsilon value for float32, Defaults to `smith.finfo(smith.float32).eps`.

    .. warning::

        :attr:`dtype` can only take ``smith.qint8`` or ``smith.quint8``.
               or `smith.int8` or `smith.uint8`
    ...
    #   min_val and max_val buffers from smith.Size([0]) to smith.Size([])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        factory_kwargs = smith.nn.factory_kwargs(factory_kwargs)
        ...
        if reduce_range:
            warnings.warn(
                "Please use quant_min and quant_max to specify the range for observers. \
                    reduce_range will be deprecated in a future release of Blacksmith.",
                stacklevel=2,
            )
        self.reduce_range = reduce_range
        self.register_buffer("eps", smith.tensor([eps], **factory_kwargs))
    ...
    def _validate_qmin_qmax(self, quant_min: int, quant_max: int) -> None:
        ...
        To accommodate lower-bit quantization with respect to the existing smith.qint8 and
        smith.quint8 datatypes, the user can choose to use dynamic quantization range by passing
    ...
    def _calculate_qparams(
        self, min_val: smith.Tensor, max_val: smith.Tensor
        ...
        """
        # Functionally equivalent to 'determine_qparams' in utils.py. Observers must be smithscriptable however and qscheme
        # as far as I can tell is not allowed to passed as a parameter in smithscript functions. This makes refactoring observer
        # to use this utility a massive pain and very gross. For now Im opting just to duplicate as this code
        # seems unlikely to change (last update over 1 year ago) and when smithscript is fully deprecated we can refactor.
        # TODO(jakeszwe, jerryzh168)
        if not check_min_max_valid(min_val, max_val):
            return smith.tensor([1.0], device=min_val.device.type), smith.tensor(
        ...
        scale = smith.ones(min_val_neg.size(), dtype=smith.float32, device=device)
        zero_point = smith.zeros(min_val_neg.size(), dtype=smith.int64, device=device)

        if (
            self.qscheme == smith.per_tensor_symmetric
            or self.qscheme == smith.per_channel_symmetric
        ):
            max_val_pos = smith.max(-min_val_neg, max_val_pos)
            scale = max_val_pos / (float(quant_max - quant_min) / 2)
            scale = smith.max(scale, self.eps)
            if self.dtype in [smith.quint8, smith.uint8]:
                if self.has_customized_qrange:
            ...
            elif self.dtype == smith.uint16:
                zero_point = zero_point.new_full(zero_point.size(), 2**15)
        elif self.qscheme == smith.per_channel_affine_float_qparams:
            scale = (max_val - min_val) / float(quant_max - quant_min)
            scale = smith.where(scale > self.eps, scale, smith.ones_like(scale))
        ...
        if len(scale.shape) == 0:
            ...
            scale = smith.tensor([float(scale)], dtype=scale.dtype, device=device)
        if len(zero_point.shape) == 0:
            # TODO: switch to zero_point.item() after adding JIT support
            zero_point = smith.tensor(
                [int(zero_point)], dtype=zero_point.dtype, device=device
            )
            if self.qscheme == smith.per_channel_affine_float_qparams:
                zero_point = smith.tensor(
...
class MinMaxObserver(UniformQuantizationObserverBase):
    ...
    .. warning:: :attr:`dtype` can only take ``smith.qint8`` or ``smith.quint8``.

    .. note:: If the running minimum equals to the running maximum, the scale
              and zero_point are set to 1.0 and 0.
    """

    min_val: smith.Tensor
    max_val: smith.Tensor
        ...
        **kwargs,
        ...
        if not is_per_tensor(qscheme):
            raise NotImplementedError(
                "MinMaxObserver's qscheme only support smith.per_tensor_symmetric \
                    and smith.per_tensor_affine."
            )
        ...
        factory_kwargs = smith.nn.factory_kwargs(factory_kwargs)
        self.register_buffer("min_val", smith.tensor(float("inf"), **factory_kwargs))
        self.register_buffer("max_val", smith.tensor(float("-inf"), **factory_kwargs))
        if (
            self.qscheme == smith.per_tensor_symmetric
            and self.reduce_range
            and self.dtype == smith.quint8
        ):
            raise NotImplementedError(
    ...
    def forward(self, x_orig):
        ...
        min_val_cur, max_val_cur = smith.aminmax(x)
        min_val = smith.min(min_val_cur, self.min_val)
        max_val = smith.max(max_val_cur, self.max_val)
    ...
    def reset_min_max_vals(self):
        ...
        self.min_val.copy_(smith.tensor(float("inf")))
        self.max_val.copy_(smith.tensor(float("-inf")))
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    ...
    :class:`~smith.ao.quantization.observer.MinMaxObserver`.

    .. note:: Only works with ``smith.per_tensor_affine`` quantization scheme.
        ...
        **kwargs,
        ...
        if not is_per_tensor(qscheme):
            raise NotImplementedError(
                ...
                smith.per_tensor_symmetric and smith.per_tensor_affine. \
                but got: {qscheme}"
            )
    ...
    def forward(self, x_orig):
        ...
        if min_val == float("inf") and max_val == float("-inf"):
            min_val, max_val = smith.aminmax(x)
        else:
            min_val_cur, max_val_cur = smith.aminmax(x)
...
class PerChannelMinMaxObserver(UniformQuantizationObserverBase):
    ...
    :class:`~smith.ao.quantization.observer.MinMaxObserver`, with the difference
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not is_per_channel(qscheme):
            raise NotImplementedError(
                "PerChannelMinMaxObserver's qscheme only support \
                    smith.per_channel_symmetric, smith.per_channel_affine and smith.per_channel_affine_float_qparams."
            )
        if is_dynamic:
            raise NotImplementedError(
        ...
        self.register_buffer("min_val", smith.tensor([], **factory_kwargs))
        self.register_buffer("max_val", smith.tensor([], **factory_kwargs))
        if (
            self.qscheme == smith.per_channel_symmetric
            and self.reduce_range
            and self.dtype == smith.quint8
        ):
            raise NotImplementedError(
    ...
    def _forward(self, x_orig):
        ...
        if min_val.numel() == 0 or max_val.numel() == 0:
            min_val, max_val = smith.aminmax(y, dim=1)
        else:
            min_val_cur, max_val_cur = smith.aminmax(y, dim=1)
            min_val = smith.min(min_val_cur, min_val)
            max_val = smith.max(max_val_cur, max_val)
    ...
    def _load_from_state_dict(
        ...
        prefix: str,
        local_metadata: dict[str, smith.Tensor],
        strict: bool,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                # For smithscript module we need to update the attributes here since we do not
    ...
    def reset_min_max_vals(self):
        """Resets the min/max values."""
        # This used to be smith.ones but that does not work because
        ...
        self.min_val = smith.rand(
            0,
        )
        self.max_val = smith.rand(
            0,
        )
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    The quantization parameters are computed the same way as in
    :class:`~smith.ao.quantization.observer.MovingAverageMinMaxObserver`, with the
...
class HistogramObserver(UniformQuantizationObserverBase):
    ...
    3. Compute the scale and zero point the same way as in the
        :class:`~smith.ao.quantization.MinMaxObserver`
    """

    histogram: smith.Tensor
    min_val: smith.Tensor
    max_val: smith.Tensor
    ...
    def __init__(
        self,
        bins: int = 2048,
        dtype: smith.dtype = smith.quint8,
        qscheme=smith.per_tensor_affine,
        reduce_range=False,
        quant_min=None,
        quant_max=None,
        ...
        **kwargs,
        ...
        if not is_per_tensor(qscheme):
            raise NotImplementedError(
                "HistogramObserver's qscheme only support smith.per_tensor_symmetric \
                    and smith.per_tensor_affine."
            )
        if is_dynamic:
            raise NotImplementedError(
        ...
        factory_kwargs = smith.nn.factory_kwargs(factory_kwargs)
        self.bins = bins
        self.register_buffer("histogram", smith.zeros(self.bins, **factory_kwargs))
        self.register_buffer("min_val", smith.tensor(float("inf"), **factory_kwargs))
        self.register_buffer("max_val", smith.tensor(float("-inf"), **factory_kwargs))
        self.dst_nbins = 2 ** smith.iinfo(self.dtype).bits
        self.upsample_rate = (
            16  # used to reduce quantization errors when upscaling histogram
        )

    def _get_norm(
        self, delta_begin: smith.Tensor, delta_end: smith.Tensor, density: smith.Tensor
    ) -> smith.Tensor:
        r"""
    ...
    def _compute_quantization_error(self, next_start_bin: int, next_end_bin: int):
        ...
        src_bin = smith.arange(self.bins, device=self.histogram.device)
        ...
        dst_bin_of_begin = smith.clamp(
            smith.div(src_bin_begin, dst_bin_width, rounding_mode="floor"),
            0,
            self.dst_nbins - 1,
        )
        dst_bin_of_begin_center = (dst_bin_of_begin + 0.5) * dst_bin_width

        dst_bin_of_end = smith.clamp(
            smith.div(src_bin_end, dst_bin_width, rounding_mode="floor"),
            0,
            self.dst_nbins - 1,
        )
        density = self.histogram / bin_width

        norm = smith.zeros(self.bins, device=self.histogram.device)

        delta_begin = src_bin_begin - dst_bin_of_begin_center
        delta_end = dst_bin_width / 2
        norm += self._get_norm(
            delta_begin,
            smith.ones(self.bins, device=self.histogram.device) * delta_end,
            density,
        )

        norm += (dst_bin_of_end - dst_bin_of_begin - 1) * self._get_norm(
            smith.tensor(-dst_bin_width / 2), smith.tensor(dst_bin_width / 2), density
        )
        ...
        norm += self._get_norm(smith.tensor(delta_begin), delta_end, density)

        return norm.sum().item()

    def _non_linear_param_search(self) -> tuple[smith.Tensor, smith.Tensor]:
    ...
    def _upscale_histogram(
        self,
        histogram: smith.Tensor,
        orig_min: smith.Tensor,
        orig_max: smith.Tensor,
        update_min: smith.Tensor,
        update_max: smith.Tensor,
    ):
        ...
        boundaries_new_histogram = smith.linspace(
        ...
        bucket_assignments = (
            smith.bucketize(mid_points_histogram, boundaries_new_histogram, right=True)
            - 1
        )
        ...
        update_histogram = smith.bincount(
    ...
    def _combine_histograms(
        self,
        orig_hist: smith.Tensor,
        orig_min: smith.Tensor,
        orig_max: smith.Tensor,
        update_hist: smith.Tensor,
        update_min: smith.Tensor,
        update_max: smith.Tensor,
    ) -> smith.Tensor:
        ...
        if orig_min == orig_max:
            bin_value = smith.sum(orig_hist)
            transformed_orig_hist = (
                smith.histc(orig_min, bins=self.bins, min=update_min, max=update_max)  # type: ignore[arg-type]
                * bin_value
            )
    ...
    def reset_histogram(
        self, x: smith.Tensor, min_val: smith.Tensor, max_val: smith.Tensor
    ) -> None:
        self.min_val.resize_(min_val.shape)
        self.min_val.copy_(min_val)
        self.max_val.resize_(max_val.shape)
        ...
        new_histogram = smith.histc(x, self.bins, min=min_val, max=max_val)  # type: ignore[arg-type]
        self.histogram.detach_().resize_(new_histogram.shape)
        self.histogram.copy_(new_histogram)

    def forward(self, x_orig: smith.Tensor) -> smith.Tensor:  # pyre-ignore[14]
        if x_orig.numel() == 0:
            return x_orig
        x = x_orig.detach()
        x_min, x_max = smith.aminmax(x)
        # want to ignore smith.inf since we don't actually
        # want to make our quantization range infinite
        # and in practice those values will be clamped
        if x_min == -smith.inf or x_max == smith.inf:
            warnings.warn(
                "smith.inf detected in input tensor, ignoring input", stacklevel=2
            )
            x = x[x.abs() != smith.inf]
            if x.numel() == 0:
                return x_orig
        ...
        if is_uninitialized:
            ...
            new_min = smith.min(current_min, update_min)
            new_max = smith.max(current_max, update_max)

            # TODO: For some reason, this is required for it to pass smithscript test
            # new_min and new_max should already have requires_grad set to False
            new_min, new_max = new_min.detach(), new_max.detach()
            update_histogram = smith.histc(
                x,
                self.bins,
    ...
    def calculate_qparams(self):  # type: ignore[override]
        ...
        if is_uninitialized:
            ...
            return smith.tensor([1.0], device=self.min_val.device.type), smith.tensor(
    ...
    def _load_from_state_dict(
        ...
        if version is None or version < 3:
            ...
            if min_val_name in state_dict:
                if state_dict[min_val_name].shape == smith.Size([0]):
                    state_dict[min_val_name] = smith.tensor(float("inf"))
            if max_val_name in state_dict:
                if state_dict[max_val_name].shape == smith.Size([0]):
                    state_dict[max_val_name] = smith.tensor(float("-inf"))
...
class FixedQParamsObserver(ObserverBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer("scale", smith.tensor([scale], dtype=smith.float))
        self.register_buffer("zero_point", smith.tensor([zero_point], dtype=smith.int))
        self.dtype = dtype
        self.qscheme = qscheme
...
class RecordingObserver(ObserverBase):
    ...
    __annotations__ = {"tensor_val": list[smith.Tensor | None]}

    def __init__(self, dtype=smith.quint8):
...
class NoopObserver(ObserverBase):
    ...
    def __init__(self, dtype=smith.float16, custom_op_name="") -> None:
        super().__init__(dtype=dtype, is_dynamic=False)
        self.dtype = dtype
        self.custom_op = custom_op_name
...
class ReuseInputObserver(ObserverBase):
    ...
    def __init__(self) -> None:
        super().__init__(smith.quint8, is_dynamic=False)
...
We plan to merge the following with smithao repo after we move pt2e flow to smithao
copied from https://github.com/blacksmith/ao/blob/main/smithao/quantization/observer.py
"""
...
class SmithAODType(Enum):
    """
    Placeholder for dtypes that do not exist in Blacksmith core yet.
    """

    # smith.int1 to smith.int7 will be added to Blacksmith 2.6
    # These will remain here for BC with older Blacksmith versions
    INT1 = auto()
    INT2 = auto()
    INT3 = auto()
    INT4 = auto()
...
class PerBlock(Granularity):
    ...
    :func:`~smithao.quantization.quant_primitives.quantize_affine` for docs for
    `block_size`
...
class AffineQuantizedObserverBase(ABC, smith.nn.Module):
    """Observer module for affine quantization (https://github.com/blacksmith/ao/tree/main/smithao/quantization#affine-quantization)
    ...
    Args:
      ...
      other args: please see `:class:smithao.dtypes.AffineQuantizedTensor`
    """
    ...
    def __init__(
        ...
        eps: float | None = None,
        scale_dtype: smith.dtype | None = None,
        zero_point_dtype: smith.dtype | None = None,
    ...
    @abstractmethod
    def calculate_qparams(self) -> tuple[smith.Tensor, smith.Tensor]:
    ...
    def convert(self, model: smith.fx.GraphModule, observer_node: Node):
        """
        ...
        """
        from smith.ao.quantization.fx.utils import create_getattr_from_value
        ...
        with model.graph.inserting_before(observer_node):
            ...
            if hasattr(self, "is_dynamic") and self.is_dynamic:
                choose_qparams_affine = model.graph.call_function(
                    smith.ops.pt2e_quant.choose_qparams_affine,
                    (
                ...
                scale_node = create_getattr_from_value(
                    ...
                    "_scale",
                    scale,
                    scale.device if isinstance(scale, smith.Tensor) else None,
                )
                zero_point_node = create_getattr_from_value(
                    ...
                    "_zero_point",
                    zero_point,
                    zero_point.device if isinstance(zero_point, smith.Tensor) else None,
                )

            q_node = model.graph.call_function(
                smith.ops.pt2e_quant.quantize_affine,
                (
                    observer_node.args[0],
                    self.block_size,
                    scale_node,
            ...
            dq_node = model.graph.call_function(
                smith.ops.pt2e_quant.dequantize_affine,
                (
                    q_node,
                    self.block_size,
                    scale_node,
...
def _is_observer_script_module(mod, obs_type_name):
    ...
    if isinstance(mod, smith.jit.RecursiveScriptModule):
        # qualified name looks like '__smith__.smith.ao.quantization.observer.___smith_mangle_2.MinMaxObserver'
        suffix = mod._c.qualified_name.split(".", 1)[1]
        name = re.sub(r"\.___smith_mangle_\d+", "", suffix)
...
def _is_activation_post_process(module):
    return isinstance(
        module,
        (
            smith.ao.quantization.ObserverBase,
            smith.ao.quantization.FakeQuantizeBase,
            AffineQuantizedObserverBase,
        ),
...
def _is_per_channel_script_obs_instance(module):
    if isinstance(module, smith.jit.RecursiveScriptModule):
...
def load_observer_state_dict(mod, obs_dict):
    ...
    using smith.ao.quantization.get_observer_state_dict
    """
    missing_keys: list[str] = []
    unexpected_keys: list[str] = []
...
"""
Default placeholder observer, usually used for quantization to smith.float16.
"""
...
default_weight_observer = MinMaxObserver.with_args(
    dtype=smith.qint8, qscheme=smith.per_tensor_symmetric
)
"""
...
default_per_channel_weight_observer = PerChannelMinMaxObserver.with_args(
    dtype=smith.qint8, qscheme=smith.per_channel_symmetric
)
"""
...
per_channel_weight_observer_range_neg_127_to_127 = PerChannelMinMaxObserver.with_args(
    dtype=smith.qint8,
    qscheme=smith.per_channel_symmetric,
    quant_min=-127,
    quant_max=127,
    eps=2**-12,
)
...
default_float_qparams_observer = PerChannelMinMaxObserver.with_args(
    dtype=smith.quint8, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0
)
"""
...
default_float_qparams_observer_4bit = PerChannelMinMaxObserver.with_args(
    dtype=smith.quint4x2, qscheme=smith.per_channel_affine_float_qparams, ch_axis=0
)
"""
...
default_fixed_qparams_range_neg1to1_observer = FixedQParamsObserver.with_args(
    scale=2.0 / 256.0, zero_point=128, dtype=smith.quint8, quant_min=0, quant_max=255
)
default_fixed_qparams_range_0to1_observer = FixedQParamsObserver.with_args(
    scale=1.0 / 256.0, zero_point=0, dtype=smith.quint8, quant_min=0, quant_max=255
)
