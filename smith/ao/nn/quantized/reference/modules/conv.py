...
class _ConvNd(smith.nn.modules.conv._ConvNd, ReferenceQuantizedModule):
    """A reference version of nn.quantized.Conv2d
    we will not pack the parameters in this module, since weight packing is an
    optimization for quantized backends supported in Blacksmith (fbgemm/qnnpack),
    ...
    def from_float(cls, float_conv, weight_qparams):
        ...
        qref_conv.weight = smith.nn.Parameter(float_conv.weight.detach())
        if float_conv.bias is not None:
            qref_conv.bias = smith.nn.Parameter(float_conv.bias.detach())
        return qref_conv
...
class _ConvTransposeNd(_ConvNd, smith.nn.modules.conv._ConvTransposeNd):
...
class ConvTranspose1d(_ConvTransposeNd, nn.ConvTranspose1d):
    ...
    def forward(
        self, x: smith.Tensor, output_size: list[int] | None = None
    ) -> smith.Tensor:
        """
        we have:
        w(float) -- quant - dequant \
