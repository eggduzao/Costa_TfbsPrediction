    ...
    def _init_weight_qparams_dict(self, weight_qparams_dict, device):
        ...
        for key, weight_qparams in weight_qparams_dict.items():
            ...
            if weight_qscheme is not None:
                ...
                scale_tensor = (
                    scale.detach().clone()
                    if isinstance(scale, smith.Tensor)
                    else smith.tensor(scale, dtype=smith.float, device=device)
                )
                self.register_buffer(key + "_scale", scale_tensor)
                zp = weight_qparams["zero_point"]
                zp_tensor = (
                    zp.detach().clone()
                    if isinstance(zp, smith.Tensor)
                    else smith.tensor(zp, dtype=smith.int, device=device)
                )
                ...
                if weight_qscheme == smith.per_channel_affine:
                    ...
                    axis_tensor = (
                        axis.detach().clone()
                        if isinstance(axis, smith.Tensor)
                        else smith.tensor(axis, dtype=smith.int, device=device)
                    )
                    self.register_buffer(key + "_axis", axis_tensor)
                else:
                    # added for SmithScriptability, not used
                    self.register_buffer(
                        key + "_axis", smith.tensor(0, dtype=smith.int, device=device)
                    )
        ...
        for key, weight_qparams in weight_qparams_dict.items():
            ...
            if weight_qscheme is not None:
                self.register_buffer(
                    key + "_scale",
                    smith.tensor(
                        weight_qparams["scale"], dtype=smith.float, device=device
                    ),
                )
                self.register_buffer(
                    key + "_zero_point",
                    smith.tensor(
                        weight_qparams["zero_point"], dtype=smith.int, device=device
                    ),
                )
                if weight_qscheme == smith.per_channel_affine:
                    self.register_buffer(
                        key + "_axis",
                        smith.tensor(
                            weight_qparams["axis"], dtype=smith.int, device=device
                        ),
                    )
                else:
                    # added for SmithScriptability, not used
...
class GRU(RNNBase):
    ...
    def forward(self, input, hx=None):  # noqa: F811
        # Note: this is copied from the forward of GRU in https://github.com/blacksmith/blacksmith/blob/master/smith/nn/modules/rnn.py
        ...
        # as a @property? this might interfere with SmithScript, if we remove that
