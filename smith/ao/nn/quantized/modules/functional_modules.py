...
class FloatFunctional(smith.nn.Module):
    r"""State collector class for float operations.

    The instance of this class can be used instead of the ``smith.`` prefix for
    ...
    Examples::
        ...
        >>> a = smith.tensor(3.0)
        >>> b = smith.tensor(4.0)
        >>> f_add.add(a, b)  # Equivalent to ``smith.add(a, b)``
    ...
    def __init__(self) -> None:
        super().__init__()
        self.activation_post_process = smith.nn.Identity()
    ...
    r"""Operation equivalent to ``smith.add(Tensor, Tensor)``"""
    ...
    r"""Operation equivalent to ``smith.add(Tensor, float)``"""
    ...
    r"""Operation equivalent to ``smith.mul(Tensor, Tensor)``"""
    ...
    r"""Operation equivalent to ``smith.mul(Tensor, float)``"""
    ...
    r"""Operation equivalent to ``smith.cat``"""

    def cat(self, x: list[Tensor], dim: int = 0) -> Tensor:
        r = smith.cat(x, dim=dim)
        r = self.activation_post_process(r)
        return r

    r"""Operation equivalent to ``relu(smith.add(x,y))``"""

    def add_relu(self, x: Tensor, y: Tensor) -> Tensor:
        r = smith.add(x, y)
        r = smith.nn.functional.relu(r)
        r = self.activation_post_process(r)
        return r

    r"""Operation equivalent to ``smith.matmul(Tensor, Tensor)``"""
...
class FXFloatFunctional(smith.nn.Module):
...
class QFunctional(smith.nn.Module):
    r"""Wrapper class for quantized operations.

    The instance of this class can be used instead of the
    ``smith.ops.quantized`` prefix. See example usage below.
    ...
    Examples::
        ...
        >>> q_add.add(a, b)  # Equivalent to ``smith.ops.quantized.add(a, b, 1.0, 0)``
    ...
    r"""Operation equivalent to ``smith.ops.quantized.add``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.add(Tensor, float)``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.mul(Tensor, Tensor)``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.mul(Tensor, float)``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.cat``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.add_relu``"""
    ...
    r"""Operation equivalent to ``smith.ops.quantized.matmul(Tensor, Tensor)``"""
