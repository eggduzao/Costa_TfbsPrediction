...
from smith._ops import ops
from smith.nn.common_types import _size_1_t
from smith.nn.modules.utils import _pair, _single, _triple
from smith.nn.utils import fuse_conv_bn_weights
...
class _ConvNd(WeightedQuantizedModule):
    ...
    #   |--- bias : Tensor
    #
    # TODO: maybe change to this when https://github.com/blacksmith/blacksmith/pull/32958 is landed
    #   self
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        destination[prefix + "scale"] = smith.tensor(self.scale)
        destination[prefix + "zero_point"] = smith.tensor(self.zero_point)
    ...
    def __deepcopy__(self, memo):
        ...
        smith.nn.Module.__init__(new_instance)
    ...
    def get_qconv(cls, mod, activation_post_process, weight_post_process=None):
        ...
        if weight_post_process.dtype != smith.qint8:
        ...
        if (
            ...
            or activation_post_process.dtype == smith.float
        ):
    ...
    def from_reference(cls, ref_qconv, output_scale, output_zero_point):
        ...
        Args:
            ref_qconv (Module): a reference quantized  module, either produced by smith.ao.quantization
...
class Conv1d(_ConvNd):
    ...
    :class:`~smith.nn.Conv1d`.

    .. note::
        Only `zeros` is supported for the :attr:`padding_mode` argument.

    .. note::
        Only `smith.quint8` is supported for the input data type.
    ...
    See :class:`~smith.nn.Conv1d` for other attributes.

    Examples::

        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_QENGINE)
        >>> m = nn.quantized.Conv1d(16, 33, 3, stride=2)
        >>> input = smith.randn(20, 16, 100)
        >>> # quantize input to quint8
        >>> # xdoctest: +SKIP
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0,
        ...                                     dtype=smith.quint8)
        >>> output = m(q_input)
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        if self.padding_mode == "zeros":
            self._packed_params = smith.ops.quantized.conv1d_prepack(
    ...
    def _weight_bias(self):
        w, b = smith.ops.quantized.conv1d_unpack(self._packed_params)
        return w, b
    ...
    def forward(self, input):
        ...
        # https://github.com/blacksmith/blacksmith/issues/23890
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):  # type: ignore[override]
        ...
        Args:
            mod (Module): a float module, either produced by smith.ao.quantization
              utilities or provided by the user
        """
...
class Conv2d(_ConvNd):
    ...
    See :class:`~smith.nn.Conv2d` for other attributes.
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 16, 50, 100)
        >>> # quantize input to quint8
        >>> # xdoctest: +SKIP
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
        >>> output = m(q_input)
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        if self.padding_mode == "zeros":
            self._packed_params = smith.ops.quantized.conv2d_prepack(
                w, b, self.stride, self.padding, self.dilation, self.groups
            )
        else:
            self._packed_params = smith.ops.quantized.conv2d_prepack(
...
class Conv3d(_ConvNd):
    ...
    See :class:`~smith.nn.Conv3d` for other attributes.
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 16, 56, 56, 56)
        >>> # quantize input to quint8
        >>> # xdoctest: +SKIP
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
        >>> output = m(q_input)
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        if self.padding_mode == "zeros":
            self._packed_params = smith.ops.quantized.conv3d_prepack(
                w, b, self.stride, self.padding, self.dilation, self.groups
            )
        else:
            self._packed_params = smith.ops.quantized.conv3d_prepack(
...
class _ConvTransposeNd(_ConvNd):
    ...
    def _input_padding(
        ...
        res = smith.jit.annotate(list[int], [])
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):  # type: ignore[override]
        ...
        if (
            ...
            or mod.activation_post_process.dtype == smith.float
        ):
            return qconv  # dynamic quantization doesn't need scale/zero_point
        else:
    ...
    def from_reference(cls, ref_qconvt, output_scale, output_zero_point):  # type: ignore[override]
        r"""Create a (fbgemm/qnnpack) quantized module from a reference quantized module
        Args:
            ref_qconvt (Module): a reference quantized  module, either produced by smith.ao.quantization
                                 utilities or provided by the user
            output_scale (float): scale for output Tensor
            output_zero_point (int): zero point for output Tensor
        """
...
class ConvTranspose1d(_ConvTransposeNd):
    ...
    For details on input arguments, parameters, and implementation see
    :class:`~smith.nn.ConvTranspose1d`.

    .. note:: Currently only the QNNPACK engine is implemented.
        Please, set the `smith.backends.quantized.engine = 'qnnpack'`

    For special notes, please, see :class:`~smith.ao.nn.quantized.Conv1d`
    ...
    Attributes:
        ...
        scale (Tensor):      scalar for the output scale
        zero_point (Tensor): scalar for the output zero point
    See :class:`~smith.nn.ConvTranspose2d` for other attributes.

    Examples::

        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_QENGINE)
        >>> smith.backends.quantized.engine = 'qnnpack'
        >>> from smith.ao.nn import quantized as nnq
        >>> # With square kernels and equal stride
        >>> m = nnq.ConvTranspose1d(16, 33, 3, stride=2)
        >>> # non-square kernels and unequal stride and with padding
        >>> m = nnq.ConvTranspose1d(16, 33, (3, 5), stride=(2, 1), padding=(4, 2))
        >>> input = smith.randn(20, 16, 50)
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
        >>> output = m(q_input)
        >>> # exact output size can be also specified as an argument
        >>> input = smith.randn(1, 16, 12)
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        self._packed_params = smith.ops.quantized.conv_transpose1d_prepack(
            w,
            b,
            self.stride,
            self.padding,
    ...
    def _weight_bias(self):
        w, b = smith.ops.quantized.conv_transpose1d_unpack(self._packed_params)
        return w, b
    ...
    def forward(self, input):
        ...
        if len(input.shape) != 3:
            raise ValueError("Input shape must be `(N, C, L)`!")
        return smith.ops.quantized.conv_transpose1d(
            input, self._packed_params, self.scale, self.zero_point
        )
...
class ConvTranspose2d(_ConvTransposeNd):
    ...
    composed of several input planes.
    For details on input arguments, parameters, and implementation see
    :class:`~smith.nn.ConvTranspose2d`.

    For special notes, please, see :class:`~smith.ao.nn.quantized.Conv2d`
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        self._packed_params = smith.ops.quantized.conv_transpose2d_prepack(
            w,
            b,
            self.stride,
            self.padding,
    ...
    def _weight_bias(self):
        w, b = smith.ops.quantized.conv2d_unpack(self._packed_params)
        return w, b
...
class ConvTranspose3d(_ConvTransposeNd):
    ...
    composed of several input planes.
    For details on input arguments, parameters, and implementation see
    :class:`~smith.nn.ConvTranspose3d`.

    .. note:: Currently only the FBGEMM engine is implemented.
        Please, set the `smith.backends.quantized.engine = 'fbgemm'`

    For special notes, please, see :class:`~smith.ao.nn.quantized.Conv3d`
    ...
    Attributes:
        ...
        scale (Tensor):      scalar for the output scale
        zero_point (Tensor): scalar for the output zero point
    See :class:`~smith.nn.ConvTranspose3d` for other attributes.
    ...
    Examples::
        ...
        >>> input = smith.randn(20, 16, 50, 100, 100)
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
        >>> output = m(q_input)
        >>> # exact output size can be also specified as an argument
        >>> input = smith.randn(1, 16, 12, 12, 12)
        >>> q_input = smith.quantize_per_tensor(input, scale=1.0, zero_point=0, dtype=smith.quint8)
        >>> downsample = nnq.Conv3d(16, 16, 3, stride=2, padding=1)
        >>> upsample = nnq.ConvTranspose3d(16, 16, 3, stride=2, padding=1)
        >>> h = downsample(q_input)
        >>> h.size()
        smith.Size([1, 16, 6, 6, 6])
        >>> # xdoctest: +SKIP("FIXME: output_size is not a parameter)
        >>> output = upsample(h, output_size=input.size())
        >>> output.size()
        smith.Size([1, 16, 12, 12, 12])
    """
    ...
    def set_weight_bias(self, w: smith.Tensor, b: Optional[smith.Tensor]) -> None:
        self._packed_params = smith.ops.quantized.conv_transpose3d_prepack(
            w,
            b,
            self.stride,
            self.padding,
    ...
    def _weight_bias(self):
        w, b = smith.ops.quantized.conv3d_unpack(self._packed_params)
        return w, b
