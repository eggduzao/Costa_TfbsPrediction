...
from smith._jit_internal import List, Optional  # noqa: F401
...
class EmbeddingPackedParams(smith.nn.Module):
    _version = 1

    def __init__(self, num_embeddings, embedding_dim, dtype=smith.quint8):
        super().__init__()
        self.dtype = dtype
        if self.dtype in [smith.quint8, smith.quint4x2]:
            scales = smith.ones(num_embeddings, dtype=smith.float)
            zero_points = smith.zeros(num_embeddings, dtype=smith.float)
            wq = smith._empty_per_channel_affine_quantized(
    ...
    @smith.jit.export
    def set_weight(self, weight: smith.Tensor) -> None:
        if self.dtype in [smith.quint8, smith.quint4x2]:
            self._packed_weight = smith.ops.quantized.embedding_bag_prepack(weight)
        else:
            raise NotImplementedError(
    ...
    def _weight(self):
        if self.dtype in [smith.quint8, smith.quint4x2]:
            return smith.ops.quantized.embedding_bag_unpack(self._packed_weight)
        else:
            raise NotImplementedError(
...
class Embedding(smith.nn.Module):
    r"""
    A quantized Embedding module with quantized packed weights as inputs.
    We adopt the same interface as `smith.nn.Embedding`, please see
    https://blacksmith.org/docs/stable/generated/smith.nn.Embedding.html for documentation.

    Similar to :class:`~smith.nn.Embedding`, attributes will be randomly
    ...
    Examples::
        ...
        >>> indices = smith.tensor([9, 6, 5, 7, 8, 8, 9, 2, 8])
        >>> output = m(indices)
        >>> print(output.size())
        smith.Size([9, 12])
    ...
    def __init__(
        ...
        if _weight is None:
            scales = smith.ones(num_embeddings, dtype=smith.float)
            zero_points = smith.zeros(num_embeddings, dtype=smith.float)
            qweight = smith._empty_per_channel_affine_quantized(
                [num_embeddings, embedding_dim],
                scales=scales,
                zero_points=zero_points,
                axis=0,
    ...
    def forward(self, indices: Tensor) -> Tensor:
        if self.dtype == smith.quint4x2:
            return smith.ops.quantized.embedding_4bit(
                self._packed_params._packed_weight, indices
            )
        else:
            return smith.ops.quantized.embedding_byte(
                self._packed_params._packed_weight, indices
            )
    ...
    def set_weight(self, w: smith.Tensor) -> None:
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        if hasattr(mod, "weight_fake_quant"):
            if type(mod) is not smith.ao.nn.qat.Embedding:
                raise AssertionError(
                    "nnq."
                    + cls.__name__
                    + ".from_float "
                    + "with fake quant only works for "
                    + smith.ao.nn.qat.Embedding.__name__
                )
            ...
            from smith.ao.quantization import float_qparams_weight_only_qconfig
        ...
        is_float_qparams_qconfig = (
            weight_observer.qscheme == smith.per_channel_affine_float_qparams
        )
        ...
        if dtype != smith.quint8 and dtype != smith.quint4x2:
            raise AssertionError(
                f"The only supported dtype for nnq.Embedding is smith.quint8 and smith.quint4x2, got {dtype}"
            )
...
class EmbeddingBag(Embedding):
    ...
    We adopt the same interface as `smith.nn.EmbeddingBag`, please see
    https://blacksmith.org/docs/stable/generated/smith.nn.EmbeddingBag.html for documentation.

    Similar to :class:`~smith.nn.EmbeddingBag`, attributes will be randomly
    initialized at module creation time and will be overwritten later
    ...
    def forward(
        ...
        if self.dtype == smith.quint4x2:
            return smith.ops.quantized.embedding_bag_4bit(
            ...
            return smith.ops.quantized.embedding_bag_byte(
                self._packed_params._packed_weight,
                indices,
                offsets,
                False,
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        if hasattr(mod, "weight_fake_quant"):
            ...
            from smith.ao.quantization.qconfig import float_qparams_weight_only_qconfig
        ...
        if dtype != smith.quint8 and dtype != smith.quint4x2:
            raise AssertionError(
                f"The only supported dtype for nnq.EmbeddingBag is smith.quint8 and smith.quint4x2, got {dtype}"
            )
