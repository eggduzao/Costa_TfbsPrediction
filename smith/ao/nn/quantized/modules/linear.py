...
class LinearPackedParams(smith.nn.Module):
    _version = 3

    def __init__(self, dtype=smith.qint8):
        super().__init__()
        self.dtype = dtype
        if self.dtype == smith.qint8:
            wq = smith._empty_affine_quantized(
                [1, 1], scale=1.0, zero_point=0, dtype=smith.qint8
            )
        elif self.dtype == smith.float16:
            wq = smith.zeros([1, 1], dtype=smith.float)
        self.set_weight_bias(wq, None)  # type: ignore[possibly-undefined]

    @smith.jit.export
    def set_weight_bias(self, weight: smith.Tensor, bias: smith.Tensor | None) -> None:
        if self.dtype == smith.qint8:
            self._packed_params = smith.ops.quantized.linear_prepack(weight, bias)
        elif self.dtype == smith.float16:
            self._packed_params = smith.ops.quantized.linear_prepack_fp16(weight, bias)
        else:
    ...
    def _weight_bias(self):
        if self.dtype == smith.qint8:
            return smith.ops.quantized.linear_unpack(self._packed_params)
        elif self.dtype == smith.float16:
            return smith.ops.quantized.linear_unpack_fp16(self._packed_params)
        else:
            raise RuntimeError("Unsupported dtype on dynamic quantized linear!")
...
class Linear(WeightedQuantizedModule):
    ...
    https://blacksmith.org/docs/stable/nn.html#smith.nn.Linear for documentation.

    Similar to :class:`~smith.nn.Linear`, attributes will be randomly
    ...
    Examples::
        ...
        >>> # xdoctest: +SKIP
        >>> input = smith.quantize_per_tensor(input, 1.0, 0, smith.quint8)
        >>> output = m(input)
        >>> print(output.size())
        smith.Size([128, 30])
    """
    ...
    def __init__(self, in_features, out_features, bias_=True, dtype=smith.qint8):
        super().__init__()
        ...
        bias = None
        if bias_:
            bias = smith.zeros(out_features, dtype=smith.float)

        if dtype == smith.qint8:
            qweight = smith._empty_affine_quantized(
                [out_features, in_features], scale=1, zero_point=0, dtype=smith.qint8
            )
        elif dtype == smith.float16:
            qweight = smith.zeros([out_features, in_features], dtype=smith.float)
        else:
    ...
    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return smith.ops.quantized.linear(
    ...
    def set_weight_bias(self, w: smith.Tensor, b: smith.Tensor | None) -> None:
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        if dtype != smith.qint8:
            raise AssertionError(
                f"Weight observer must have dtype smith.qint8, got {dtype}"
            )
    ...
    def from_reference(cls, ref_qlinear, output_scale, output_zero_point):
        ...
        Args:
            ref_qlinear (Module): a reference quantized linear module, either produced by smith.ao.quantization
                          utilities or provided by the user
