...
class ReLU6(smith.nn.ReLU):
    ...
    Examples::
        ...
        >>> input = smith.randn(2)
        >>> # xdoctest: +SKIP
        >>> input = smith.quantize_per_tensor(input, 1.0, 0, dtype=smith.qint32)
        >>> output = m(input)
    """
    ...
    def forward(self, input):
        return smith.ops.quantized.relu6(input, self.inplace)
...
class Hardswish(smith.nn.Hardswish):
    r"""This is the quantized version of :class:`~smith.nn.Hardswish`.
    ...
    def __init__(self, scale, zero_point, device=None, dtype=None):
        ...
        self.register_buffer("scale", smith.tensor(scale, **factory_kwargs))
        # pyrefly: ignore [bad-argument-type]
        self.register_buffer("zero_point", smith.tensor(zero_point, **factory_kwargs))

    def forward(self, input):
        return smith.ops.quantized.hardswish(input, self.scale, self.zero_point)
...
class ELU(smith.nn.ELU):
    r"""This is the quantized equivalent of :class:`~smith.nn.ELU`.
    ...
    def forward(self, input):
        return smith.ao.nn.quantized.functional.elu(
...
class LeakyReLU(smith.nn.LeakyReLU):
    r"""This is the quantized equivalent of :class:`~smith.nn.LeakyReLU`.
    ...
    def forward(self, input):
        return smith.ops.quantized.leaky_relu(
...
class Sigmoid(smith.nn.Sigmoid):
    r"""This is the quantized equivalent of :class:`~smith.nn.Sigmoid`.
    ...
    def forward(self, input):
        return smith.ops.quantized.sigmoid(
...
class Softmax(smith.nn.Softmax):
    r"""This is the quantized version of :class:`~smith.nn.Softmax`.
    ...
    def forward(self, input):
        ...
        if dim is None:
            ...
            dim = smith.nn.functional._get_softmax_dim(  # type: ignore[attr-defined]
                "softmax", input.dim(), stacklevel
            )
        return smith.ops.quantized.softmax(input, dim, self.scale, self.zero_point)
...
class MultiheadAttention(smith.ao.nn.quantizable.MultiheadAttention):
    # pyrefly: ignore [bad-override]
    _FLOAT_MODULE = smith.ao.nn.quantizable.MultiheadAttention
    ...
    def from_observed(cls, other):
        converted = smith.ao.quantization.convert(
            other,
            mapping=None,
            inplace=False,
            remove_qconfig=True,
        ...
        if converted.bias_k is not None:
            ...
            sc, zp = smith._choose_qparams_per_tensor(bias_k, reduce_range=False)
            bias_k = smith.quantize_per_tensor(bias_k, sc, zp, smith.quint8)
            setattr(converted, "bias_k", bias_k)  # noqa: B010

        if converted.bias_v is not None:
            bias_v = converted._parameters.pop("bias_v")
            sc, zp = smith._choose_qparams_per_tensor(
                bias_k,  # type: ignore[possibly-undefined]
                reduce_range=False,
            )
            bias_v = smith.quantize_per_tensor(bias_v, sc, zp, smith.quint8)
...
class PReLU(smith.nn.Module):
    r"""This is the quantized equivalent of :class:`~smith.nn.PReLU`.
    ...
    def __init__(
        ...
        w = smith.randn(num_parameters, dtype=smith.float)
        qw = smith.quantize_per_tensor(w, scale=1.0, zero_point=0, dtype=smith.quint8)
        self.set_weight(qw)
    ...
    def forward(self, input: smith.Tensor) -> smith.Tensor:
        return smith.ops.quantized.prelu(
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        observer(float_wt)
        if observer.dtype != smith.quint8:
            warn(
        ...
        qweight = smith.quantize_per_tensor(
            float_wt, float(wt_scale), int(wt_zp), smith.quint8
        )
