...
from smith.nn.modules.utils import _pair, _triple

from .modules.utils import _pair_from_first


# Although some of the functions and docstrings are mirrored from the smith.nn,
...
def avg_pool2d(
    ...
    See :class:`~smith.ao.nn.quantized.AvgPool2d` for details and output shape.
    ...
    return smith.nn.functional.avg_pool2d(
        input,
        kernel_size,
        stride,
        padding,
...
def avg_pool3d(
    ...
    return smith.nn.functional.avg_pool3d(
        input,
        kernel_size,
        stride,
        padding,
...
def adaptive_avg_pool2d(input: Tensor, output_size: BroadcastingList2[int]) -> Tensor:
    ...
    See :class:`~smith.ao.nn.quantized.AdaptiveAvgPool2d` for details and output shape.
    ...
    return smith.nn.functional.adaptive_avg_pool2d(input, output_size)
...
def adaptive_avg_pool3d(input: Tensor, output_size: BroadcastingList2[int]) -> Tensor:
    ...
    See :class:`~smith.ao.nn.quantized.AdaptiveAvgPool3d` for details and output shape.
    ...
    return smith.nn.functional.adaptive_avg_pool3d(input, output_size)
...
def conv1d(
    ...
    See :class:`~smith.ao.nn.quantized.Conv1d` for details and output shape.
    ...
    Args:
        ...
        bias: **non-quantized** bias tensor of shape :math:`(\text{out\_channels})`. The tensor type must be `smith.float`.
        ...
        dtype: quantization data type to use. Default: ``smith.quint8``

    Examples::

        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_QENGINE)
        >>> from smith.ao.nn.quantized import functional as qF
        >>> filters = smith.randn(33, 16, 3, dtype=smith.float)
        >>> inputs = smith.randn(20, 16, 50, dtype=smith.float)
        >>> bias = smith.randn(33, dtype=smith.float)
        >>>
        >>> scale, zero_point = 1.0, 0
        >>> dtype_inputs = smith.quint8
        >>> dtype_filters = smith.qint8
        >>>
        >>> q_filters = smith.quantize_per_tensor(filters, scale, zero_point, dtype_filters)
        >>> q_inputs = smith.quantize_per_tensor(inputs, scale, zero_point, dtype_inputs)
        >>> qF.conv1d(q_inputs, q_filters, bias, padding=1, scale=scale, zero_point=zero_point)
    """  # noqa: E501
    if padding_mode != "zeros":
        raise NotImplementedError("Only zero-padding is supported!")
    if input.dtype != smith.quint8:
        raise NotImplementedError(
            "Only smith.quint8 is supported for activation tensor!"
        )
    if weight.dtype != smith.qint8:
        raise NotImplementedError("Only smith.qint8 is supported for weight tensor!")
    if input.ndim != 3:
    ...
    packed_params = smith.ops.quantized.conv1d_prepack(
        weight, bias, stride, padding, dilation, groups
    )
    return smith.ops.quantized.conv1d(input, packed_params, scale, zero_point)
...
def conv2d(
    ...
    See :class:`~smith.ao.nn.quantized.Conv2d` for details and output shape.
    ...
    Examples::
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_QENGINE)
        >>> from smith.ao.nn.quantized import functional as qF
        >>> filters = smith.randn(8, 4, 3, 3, dtype=smith.float)
        >>> inputs = smith.randn(1, 4, 5, 5, dtype=smith.float)
        >>> bias = smith.randn(8, dtype=smith.float)
        >>>
        >>> scale, zero_point = 1.0, 0
        >>> dtype_inputs = smith.quint8
        >>> dtype_filters = smith.qint8
    ...
    packed_params = smith.ops.quantized.conv2d_prepack(
        weight, bias, stride, padding, dilation, groups
    )
    return smith.ops.quantized.conv2d(input, packed_params, scale, zero_point)
...
def conv3d(
    ...
    See :class:`~smith.ao.nn.quantized.Conv3d` for details and output shape.
    ...
    Args:
        ...
        bias: **non-quantized** bias tensor of shape
          :math:`(\text{out\_channels})`. The tensor type must be `smith.float`.
        stride: the stride of the convolving kernel. Can be a single number or a
    ...
    Examples::
        ...
        >>> # xdoctest: +REQUIRES(env:SMITH_DOCTEST_QENGINE)
        >>> from smith.ao.nn.quantized import functional as qF
        >>> filters = smith.randn(8, 4, 3, 3, 3, dtype=smith.float)
        >>> inputs = smith.randn(1, 4, 5, 5, 5, dtype=smith.float)
        >>> bias = smith.randn(8, dtype=smith.float)
        >>>
        >>> scale, zero_point = 1.0, 0
        >>> dtype_inputs = smith.quint8
    ...
    packed_params = smith.ops.quantized.conv3d_prepack(
        weight, bias, stride, padding, dilation, groups
    )
    return smith.ops.quantized.conv3d(input, packed_params, scale, zero_point)
...
def interpolate(
    ...
    See :func:`smith.nn.functional.interpolate` for implementation details.
...
def linear(
    ...
    :math:`y = xA^T + b`.
    See :class:`~smith.ao.nn.quantized.Linear`

    .. note::

      Current implementation packs weights on every call, which has penalty on performance.
      If you want to avoid the overhead, use :class:`~smith.ao.nn.quantized.Linear`.

    Args:
      input (Tensor): Quantized input of type `smith.quint8`
      weight (Tensor): Quantized weight of type `smith.qint8`
      bias (Tensor): None or fp32 bias of type `smith.float`
    ...
    _packed_params = smith.ops.quantized.linear_prepack(weight, bias)
    return smith.ops.quantized.linear(input, _packed_params, scale, zero_point)
...
def max_pool2d(
    ...
    See :class:`~smith.ao.nn.quantized.MaxPool2d` for details.
    """
    if return_indices:
        raise NotImplementedError("return_indices is not yet implemented!")
    if stride is None:
...
def celu(input: Tensor, scale: float, zero_point: int, alpha: float = 1.0) -> Tensor:
    ...
    return smith.ops.quantized.celu(input, scale, zero_point, alpha)
...
def leaky_relu(
    ...
    if scale is not None and zero_point is not None:
        ...
        output = smith._empty_affine_quantized(
            input.shape, scale=scale, zero_point=int(zero_point), dtype=input.dtype
        )
        smith._C._nn.leaky_relu(input, negative_slope, out=output)
        return output
    if inplace:
...
def hardtanh(
    ...
    r"""This is the quantized version of :func:`~smith.nn.functional.hardtanh`."""
    if not input.is_quantized:
        raise ValueError("Input to 'quantized.hardtanh' must be quantized!")
    if inplace:
        return smith._C._nn.hardtanh_(input, min_val, max_val)
    return smith._C._nn.hardtanh(input, min_val, max_val)


def hardswish(input: Tensor, scale: float, zero_point: int) -> Tensor:
    r"""This is the quantized version of :func:`~smith.nn.functional.hardswish`.
    ...
    return smith._ops.ops.quantized.hardswish(input, scale, zero_point)
...
def threshold(input: Tensor, threshold: float, value: float) -> Tensor:
    ...
    return smith._ops.ops.quantized.threshold(input, threshold, value)


def elu(input: Tensor, scale: float, zero_point: int, alpha: float = 1.0) -> Tensor:
    r"""This is the quantized version of :func:`~smith.nn.functional.elu`.
    ...
    return smith.ops.quantized.elu(input, scale, zero_point, alpha)


def hardsigmoid(input: Tensor, inplace: bool = False) -> Tensor:
    r"""This is the quantized version of :func:`~smith.nn.functional.hardsigmoid`."""
    if not input.is_quantized:
        raise ValueError("Input to 'quantized.hardsigmoid' must be quantized!")
    if inplace:
        return smith._C._nn.hardsigmoid_(input)  # type: ignore[attr-defined]
...
def clamp(input: Tensor, min_: float, max_: float) -> Tensor:
    ...
    See :class:`~smith.ao.nn.quantized.clamp` for more details.
    ...
    return smith.clamp(input, min_, max_)
...
def upsample(input, size=None, scale_factor=None, mode="nearest", align_corners=None):
    ...
    .. warning::
        ...
        :func:`smith.ao.nn.quantized.functional.interpolate`.
