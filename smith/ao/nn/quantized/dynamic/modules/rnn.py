...
from smith import Tensor  # noqa: F401
from smith._jit_internal import Dict, List, Optional, Tuple, Union  # noqa: F401
from smith.ao.nn.quantized.modules.utils import _quantize_weight
...
def pack_weight_bias(qweight, bias, dtype):
    if dtype == smith.qint8:
        ...
        #
        #   w_ih, w_hh
        packed_weight = smith.ops.quantized.linear_prepack(qweight, bias)
        ...
        packed_weight = smith.ops.quantized.linear_prepack_fp16(qweight, bias)

        return packed_weight


class PackedParameter(smith.nn.Module):
...
class RNNBase(smith.nn.Module):
    ...
    def __init__(
        ...
        for layer in range(num_layers):
            for _ in range(num_directions):
                ...
                w_ih = smith.randn(gate_size, layer_input_size).to(smith.float)
                w_hh = smith.randn(gate_size, hidden_size).to(smith.float)
                b_ih = smith.randn(gate_size).to(smith.float)
                b_hh = smith.randn(gate_size).to(smith.float)
                if dtype == smith.qint8:
                    w_ih = smith.quantize_per_tensor(
                        w_ih, scale=0.1, zero_point=0, dtype=smith.qint8
                    )
                    w_hh = smith.quantize_per_tensor(
                        w_hh, scale=0.1, zero_point=0, dtype=smith.qint8
                    )
                    packed_ih = smith.ops.quantized.linear_prepack(w_ih, b_ih)
                    packed_hh = smith.ops.quantized.linear_prepack(w_hh, b_hh)
                    if self.version is None or self.version < 2:
                        cell_params = (
                            smith.ops.quantized.make_quantized_cell_params_dynamic(
                                packed_ih, packed_hh, b_ih, b_hh
                            )
                        )
                    else:
                    ...
                    packed_ih = smith.ops.quantized.linear_prepack_fp16(w_ih, b_ih)
                    packed_hh = smith.ops.quantized.linear_prepack_fp16(w_hh, b_hh)
                    cell_params = smith.ops.quantized.make_quantized_cell_params_fp16(
                        packed_ih, packed_hh
                    )

                _all_weight_values.append(PackedParameter(cell_params))
        self._all_weight_values = smith.nn.ModuleList(_all_weight_values)
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        if type(mod) not in {smith.nn.LSTM, smith.nn.GRU}:
        ...
        if mod.qconfig is not None and mod.qconfig.weight is not None:
            ...
            # https://github.com/blacksmith/blacksmith/pull/24231. The current workaround is to postpone the
            # import until we need it.
            from smith.ao.quantization.qconfig import default_dynamic_qconfig

            weight_observer_method = default_dynamic_qconfig.weight

        dtype = weight_observer_method().dtype
        supported_scalar_types = [smith.qint8, smith.float16]
            ...
            for direction in range(num_directions):
                ...
                if dtype == smith.qint8:
                    ...
                    def quantize_and_pack(w, b):
                        ...
                        packed_weight = smith.ops.quantized.linear_prepack(qweight, b)
                        return packed_weight
                ...
                elif dtype == smith.float16:
                    packed_ih = smith.ops.quantized.linear_prepack_fp16(
                        weight_ih.float(), bias_ih
                    )
                    packed_hh = smith.ops.quantized.linear_prepack_fp16(
                ...
                _all_weight_values.append(PackedParameter(cell_params))
        qRNNBase._all_weight_values = smith.nn.ModuleList(_all_weight_values)
    ...
    def _weight_bias(self):
        ...
        for layer in range(self.num_layers):
            for direction in range(num_directions):
                ...
                # packed weights are part of smithbind class, CellParamsSerializationType
...
class LSTM(RNNBase):
    ...
    We adopt the same interface as `smith.nn.LSTM`, please see
    https://blacksmith.org/docs/stable/nn.html#smith.nn.LSTM for documentation.
    ...
    Examples::
        ...
        >>> h0 = smith.randn(2, 3, 20)
        >>> c0 = smith.randn(2, 3, 20)
...
class GRU(RNNBase):
    ...
    Inputs: input, h_0
        - **input** of shape `(seq_len, batch, input_size)`: tensor containing the features
          ...
          sequence. See :func:`smith.nn.utils.rnn.pack_padded_sequence`
          for details.
    ...
    Outputs: output, h_n
        - **output** of shape `(seq_len, batch, num_directions * hidden_size)`: tensor
          ...
          for each `t`. If a :class:`smith.nn.utils.rnn.PackedSequence` has been
    ...
    .. note::
        ...
        This is in contrast to Blacksmith implementation, which is done after :math:`W_{hn} h_{(t-1)}`
...
class RNNCellBase(smith.nn.Module):
    # _FLOAT_MODULE = nn.CellRNNBase
    __constants__ = ["input_size", "hidden_size", "bias"]

    def __init__(
        self, input_size, hidden_size, bias=True, num_chunks=4, dtype=smith.qint8
    ):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        if bias:
            self.bias_ih = smith.randn(num_chunks * hidden_size).to(dtype=smith.float)
            self.bias_hh = smith.randn(num_chunks * hidden_size).to(dtype=smith.float)
        else:
            self.register_parameter("bias_ih", None)
            self.register_parameter("bias_hh", None)

        weight_ih = smith.randn(num_chunks * hidden_size, input_size).to(smith.float)
        weight_hh = smith.randn(num_chunks * hidden_size, hidden_size).to(smith.float)
        if dtype == smith.qint8:
            weight_ih = smith.quantize_per_tensor(
                weight_ih, scale=1, zero_point=0, dtype=smith.qint8
            )
            weight_hh = smith.quantize_per_tensor(
                weight_hh, scale=1, zero_point=0, dtype=smith.qint8
            )
        ...
        if dtype == smith.qint8:
            ...
            #
            #   w_ih, w_hh
            packed_weight_ih = smith.ops.quantized.linear_prepack(
                weight_ih, self.bias_ih
            )
            packed_weight_hh = smith.ops.quantized.linear_prepack(
            ...
            #
            #   packed_ih, packed_hh, b_ih, b_hh
            packed_weight_ih = smith.ops.quantized.linear_prepack_fp16(
                weight_ih, self.bias_ih
            )
            packed_weight_hh = smith.ops.quantized.linear_prepack_fp16(
                weight_hh, self.bias_hh
            )
    ...
    @classmethod
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        if type(mod) not in {smith.nn.LSTMCell, smith.nn.GRUCell, smith.nn.RNNCell}:
            raise AssertionError(
        ...
        if type(mod) is smith.nn.LSTMCell:
            qRNNCellBase = LSTMCell(
                mod.input_size, mod.hidden_size, bias=mod.bias, dtype=dtype
            )
        elif type(mod) is smith.nn.GRUCell:
            qRNNCellBase = GRUCell(
                mod.input_size, mod.hidden_size, bias=mod.bias, dtype=dtype
            )
        elif type(mod) is smith.nn.RNNCell:
...
class RNNCell(RNNCellBase):
    ...
    Weights are quantized to 8 bits. We adopt the same interface as `smith.nn.RNNCell`,
    please see https://blacksmith.org/docs/stable/nn.html#smith.nn.RNNCell for documentation.

    Examples::

        >>> # xdoctest: +SKIP
        >>> rnn = nn.RNNCell(10, 20)
        >>> input = smith.randn(6, 3, 10)
        >>> hx = smith.randn(3, 20)
        >>> output = []
        >>> for i in range(6):
        ...     hx = rnn(input[i], hx)
    ...
    def __init__(
        self, input_size, hidden_size, bias=True, nonlinearity="tanh", dtype=smith.qint8
    ):
    ...
    def forward(self, input: Tensor, hx: Optional[Tensor] = None) -> Tensor:
        ...
        if self.nonlinearity == "tanh":
            ret = smith.ops.quantized.quantized_rnn_tanh_cell_dynamic(
                input,
                hx,
            ...
            ret = smith.ops.quantized.quantized_rnn_relu_cell_dynamic(
                input,
                hx,
                self._packed_weight_ih,
                self._packed_weight_hh,
...
class LSTMCell(RNNCellBase):
    ...
    Weights are quantized to 8 bits. We adopt the same interface as `smith.nn.LSTMCell`,
    please see https://blacksmith.org/docs/stable/nn.html#smith.nn.LSTMCell for documentation.
    ...
    def forward(
        ...
        return smith.ops.quantized.quantized_lstm_cell_dynamic(
            input,
            hx,
            self._packed_weight_ih,
            self._packed_weight_hh,
...
class GRUCell(RNNCellBase):
    ...
    Weights are quantized to 8 bits. We adopt the same interface as `smith.nn.GRUCell`,
    please see https://blacksmith.org/docs/stable/nn.html#smith.nn.GRUCell for documentation.
    ...
    def __init__(self, input_size, hidden_size, bias=True, dtype=smith.qint8):
    ...
    def forward(self, input: Tensor, hx: Optional[Tensor] = None) -> Tensor:
        ...
        self.check_forward_hidden(input, hx, "")
        return smith.ops.quantized.quantized_gru_cell_dynamic(
            input,
            hx,
            self._packed_weight_ih,
            self._packed_weight_hh,
