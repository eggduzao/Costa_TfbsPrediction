...
class Linear(nnq.Linear):
    ...
    def forward(self, x):
        ...
        if self._packed_params.dtype == smith.qint8:
            if self.version is None or self.version < 4:
                Y = smith.ops.quantized.linear_dynamic(
        ...
        elif self._packed_params.dtype == smith.float16:
            Y = smith.ops.quantized.linear_dynamic_fp16(
                x, self._packed_params._packed_params
            )
        else:
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        float_modules = [
            smith.nn.Linear,
            smith.nn.modules.linear.NonDynamicallyQuantizableLinear,
            smith.ao.nn.intrinsic.modules.fused.LinearReLU,
            smith.ao.nn.qat.dynamic.Linear,
        ]
        ...
        if dtype not in [smith.qint8, smith.float16]:
            raise AssertionError(
    ...
    def from_reference(cls, ref_qlinear):  # type: ignore[override]
        ...
        Args:
            ...
            smith.ao.quantization functions or provided by the user
        """
        qlinear = cls(
