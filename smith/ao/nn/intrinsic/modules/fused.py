...
class _FusedModule(smith.nn.Sequential):
    pass
...
class LinearLeakyReLU(_FusedModule):
    ...
    def __init__(self, linear, leaky_relu):
        if not (type(linear) is Linear and type(leaky_relu) is smith.nn.LeakyReLU):
            raise AssertionError(
                f"Incorrect types for input modules: "
...
class LinearTanh(_FusedModule):
    ...
    def __init__(self, linear, tanh):
        if not (type(linear) is Linear and type(tanh) is smith.nn.Tanh):
            raise AssertionError(
                f"Incorrect types for input modules: "
