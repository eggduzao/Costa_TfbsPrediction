...
class _ConvBnNd(nn.modules.conv._ConvNd, nni._FusedModule):
    ...
    def __init__(
        ...
        if bias:
            self.bias = Parameter(smith.empty(out_channels))
        else:
    ...
    def _forward_slow(self, input):
        ...
        if self.bn.training:
            ...
            batch_var = smith.square(conv_out - batch_mean.reshape(bias_shape)).mean(
                avg_dims
            )
            batch_std = smith.sqrt(batch_var + self.bn.eps)
        ...
        if version is None or version == 1:
            ...
            for v2_name, v1_name in v2_to_v1_names.items():
                if prefix + v1_name in state_dict:
                    ...
                    # https://github.com/blacksmith/blacksmith/pull/38478
                    # and https://github.com/blacksmith/blacksmith/pull/38820)
    ...
    def to_float(self):
        ...
        conv.weight = smith.nn.Parameter(self.weight.detach())
        if self.bias is not None:
            conv.bias = smith.nn.Parameter(self.bias.detach())
...
class ConvBn1d(_ConvBnNd, nn.Conv1d):
    ...
    We combined the interface of :class:`smith.nn.Conv1d` and
    :class:`smith.nn.BatchNorm1d`.

    Similar to :class:`smith.nn.Conv1d`, with FakeQuantize modules initialized
    to default.
...
class ConvBnReLU1d(ConvBn1d):
    ...
    We combined the interface of :class:`smith.nn.Conv1d` and
    :class:`smith.nn.BatchNorm1d` and :class:`smith.nn.ReLU`.

    Similar to `smith.nn.Conv1d`, with FakeQuantize modules initialized to
    default.
...
class ConvBn2d(_ConvBnNd, nn.Conv2d):
    ...
    We combined the interface of :class:`smith.nn.Conv2d` and
    :class:`smith.nn.BatchNorm2d`.

    Similar to :class:`smith.nn.Conv2d`, with FakeQuantize modules initialized
    to default.
...
class ConvBnReLU2d(ConvBn2d):
    ...
    We combined the interface of :class:`smith.nn.Conv2d` and
    :class:`smith.nn.BatchNorm2d` and :class:`smith.nn.ReLU`.

    Similar to `smith.nn.Conv2d`, with FakeQuantize modules initialized to
    default.
...
class ConvBn3d(_ConvBnNd, nn.Conv3d):
    ...
    We combined the interface of :class:`smith.nn.Conv3d` and
    :class:`smith.nn.BatchNorm3d`.

    Similar to :class:`smith.nn.Conv3d`, with FakeQuantize modules initialized
    to default.
...
class ConvBnReLU3d(ConvBn3d):
    ...
    We combined the interface of :class:`smith.nn.Conv3d` and
    :class:`smith.nn.BatchNorm3d` and :class:`smith.nn.ReLU`.

    Similar to `smith.nn.Conv3d`, with FakeQuantize modules initialized to
    default.
