...
class LinearBn1d(nn.modules.linear.Linear, nni._FusedModule):
    ...
    We combined the interface of :class:`smith.nn.Linear` and
    :class:smith.nn.BatchNorm1d`.

    Similar to :class:`smith.nn.Linear`, with FakeQuantize modules initialized
    to default.
    ...
    def __init__(
        ...
        if bias:
            self.bias = Parameter(smith.empty(out_features))
        else:
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            zero_bias = smith.zeros_like(self.bias)
        else:
            zero_bias = smith.zeros(self.out_features, device=scaled_weight.device)
    ...
    def from_float(cls, mod, use_precomputed_fake_quant=False):
        ...
        Args:
            mod: A float module, either produced by smith.ao.quantization
    ...
    def to_float(self):
        linear = smith.nn.Linear(self.in_features, self.out_features)
