...
from smith.ao.nn.intrinsic.modules.fused import _FusedModule
...
class LinearReLU(nnqat.Linear, _FusedModule):
    ...
    We adopt the same interface as :class:`smith.nn.Linear`.

    Similar to `smith.ao.nn.intrinsic.LinearReLU`, with FakeQuantize modules initialized to
    default.
    ...
    def to_float(self) -> nni.LinearReLU:
        ...
        relu = smith.nn.ReLU()
        return smith.ao.nn.intrinsic.LinearReLU(linear, relu)  # type: ignore[no-untyped-call]
