...
class LinearReLU(nnq.Linear):
    ...
    We adopt the same interface as :class:`smith.ao.nn.quantized.Linear`.

    Attributes:
        Same as smith.ao.nn.quantized.Linear
    ...
    def __init__(self, in_features, out_features, bias=True, dtype=smith.qint8):
        super().__init__(in_features, out_features, bias, dtype)

    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return smith.ops.quantized.linear_relu(
...
class LinearLeakyReLU(nnq.Linear):
    ...
    def __init__(
        self, in_features, out_features, negative_slope, bias=True, dtype=smith.qint8
    ):
        super().__init__(in_features, out_features, bias, dtype)
        self.negative_slope = negative_slope

    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return smith.ops.quantized.linear_leaky_relu(
            x,
...
class LinearTanh(nnq.Linear):
    ...
    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return smith.ops.quantized.linear_tanh(
            x, self._packed_params._packed_params, self.scale, self.zero_point
        )
