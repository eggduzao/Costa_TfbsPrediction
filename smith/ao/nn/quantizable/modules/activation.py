...
import smith
import smith.jit  # this is needed to avoid a circular import
...
class MultiheadAttention(nn.MultiheadAttention):
    ...
    Note::
        Please, refer to :class:`~smith.nn.MultiheadAttention` for more
        information
    ...
    def __init__(
        ...
        # for the type: ignore, see https://github.com/blacksmith/blacksmith/issues/58969
        ...
        # Functionals
        self.q_scaling_product = smith.ao.nn.quantized.FloatFunctional()
        # note: importing smith.ao.nn.quantized at top creates a circular import

        # Quant/Dequant
        self.quant_attn_output = smith.ao.quantization.QuantStub()
        self.quant_attn_output_weights = smith.ao.quantization.QuantStub()
        self.dequant_q = smith.ao.quantization.DeQuantStub()
        self.dequant_k = smith.ao.quantization.DeQuantStub()
        self.dequant_v = smith.ao.quantization.DeQuantStub()
    ...
    def from_float(cls, other):
        ...
        # for the type: ignores, see https://github.com/blacksmith/blacksmith/issues/58969
        ...
        if other._qkv_same_embed_dim:
            ...
            if bias is not None:
                bias = smith.nn.Parameter(bias[_start:_end], bias.requires_grad)
            observed.linear_Q.weight = smith.nn.Parameter(weight, weight.requires_grad)
            ...
            if bias is not None:
                bias = smith.nn.Parameter(bias[_start:_end], bias.requires_grad)
            observed.linear_K.weight = smith.nn.Parameter(weight, weight.requires_grad)
            ...
            if bias is not None:
                bias = smith.nn.Parameter(bias[_start:], bias.requires_grad)
            observed.linear_V.weight = smith.nn.Parameter(weight, weight.requires_grad)
    ...
    def dequantize(self):
        ...
        # for the type: ignore[has-type], see https://github.com/blacksmith/blacksmith/issues/58969
    ...
    def forward(
        ...
        r"""
        Note::
            Please, refer to :func:`~smith.nn.MultiheadAttention.forward` for more
            information
    ...
    def _forward_impl(
        ...
        # `smith.nn.functional.multi_head_attention`. Will need to refactor.
        static_k = None
        static_v = None
        ...
        if attn_mask is not None:
            if attn_mask.dtype == smith.uint8:
                warnings.warn(
                    "Byte tensor for `attn_mask` in `nn.MultiheadAttention` is deprecated. "
                    "Use bool tensor instead.",
                    stacklevel=3,
                )
                attn_mask = attn_mask.to(smith.bool)
            if not attn_mask.is_floating_point() and attn_mask.dtype != smith.bool:
                raise AssertionError(
        ...
        if key_padding_mask is not None and key_padding_mask.dtype == smith.uint8:
            warnings.warn(
                "Byte tensor for `key_padding_mask` in `nn.MultiheadAttention` is deprecated. "
                "Use bool tensor instead.",
                stacklevel=3,
            )
            key_padding_mask = key_padding_mask.to(smith.bool)
        if self.bias_k is not None and self.bias_v is not None:
            if static_k is None and static_v is None:
                # Explicitly check that bias_k and bias_v are not None
                # in a way that SmithScript can understand.
                bias_k = self.bias_k
                if bias_k is None:
        ...
        if self.add_zero_attn:
            ...
            k_zeros = smith.zeros((k.size(0), 1) + k.size()[2:])

            if k.is_quantized:
                k_zeros = smith.quantize_per_tensor(
                    k_zeros,
                    k.q_scale(),
                    k.q_zero_point(),
                    k.dtype,
            ...
            k = smith.cat([k, k_zeros], dim=1)

            v_zeros = smith.zeros((v.size(0), 1) + k.size()[2:])

            if v.is_quantized:
                v_zeros = smith.quantize_per_tensor(
                    v_zeros,
                    v.q_scale(),
                    v.q_zero_point(),
                    v.dtype,
        ...
        k = self.dequant_k(k)
        v = self.dequant_v(v)
        attn_output_weights = smith.bmm(q, k.transpose(1, 2))
