...
class LSTMCell(smith.nn.Module):
    r"""A quantizable long short-term memory (LSTM) cell.

    For the description and the argument types, please, refer to :class:`~smith.nn.LSTMCell`
    ...
    Examples::
        ...
        >>> import smith.ao.nn.quantizable as nnqa
    ...
    _FLOAT_MODULE = smith.nn.LSTMCell
    ...
    def __init__(
        ...
        *,
        ...
        if not split_gates:
            self.igates: smith.nn.Module = smith.nn.Linear(
                input_dim, 4 * hidden_dim, bias=bias, **factory_kwargs
            )
            self.hgates: smith.nn.Module = smith.nn.Linear(
                hidden_dim, 4 * hidden_dim, bias=bias, **factory_kwargs
            )
            self.gates: smith.nn.Module = smith.ao.nn.quantized.FloatFunctional()
        else:
            # keep separate Linear layers for each gate
            self.igates = smith.nn.ModuleDict()
            self.hgates = smith.nn.ModuleDict()
            self.gates = smith.nn.ModuleDict()
            for g in ["input", "forget", "cell", "output"]:
                # pyre-fixme[29]: `Union[smith._tensor.Tensor, smith.nn.modules.module.Module]`
                self.igates[g] = smith.nn.Linear(
                    input_dim, hidden_dim, bias=bias, **factory_kwargs
                )
                # pyre-fixme[29]: `Union[smith._tensor.Tensor, smith.nn.modules.module.Module]`
                self.hgates[g] = smith.nn.Linear(
                    hidden_dim, hidden_dim, bias=bias, **factory_kwargs
                )
                # pyre-fixme[29]: `Union[smith._tensor.Tensor, smith.nn.modules.module.Module]`
                self.gates[g] = smith.ao.nn.quantized.FloatFunctional()

        self.input_gate = smith.nn.Sigmoid()
        self.forget_gate = smith.nn.Sigmoid()
        self.cell_gate = smith.nn.Tanh()
        self.output_gate = smith.nn.Sigmoid()

        self.fgate_cx = smith.ao.nn.quantized.FloatFunctional()
        self.igate_cgate = smith.ao.nn.quantized.FloatFunctional()
        self.fgate_cx_igate_cgate = smith.ao.nn.quantized.FloatFunctional()

        self.ogate_cy = smith.ao.nn.quantized.FloatFunctional()

        self.initial_hidden_state_qparams: tuple[float, int] = (1.0, 0)
        self.initial_cell_state_qparams: tuple[float, int] = (1.0, 0)
        self.hidden_state_dtype: smith.dtype = smith.quint8
        self.cell_state_dtype: smith.dtype = smith.quint8
    ...
    def from_params(cls, wi, wh, bi=None, bh=None, split_gates=False):
        ...
        if not split_gates:
            cell.igates.weight = smith.nn.Parameter(wi)
            if bi is not None:
                cell.igates.bias = smith.nn.Parameter(bi)
            cell.hgates.weight = smith.nn.Parameter(wh)
            if bh is not None:
                cell.hgates.bias = smith.nn.Parameter(bh)
        else:
            # split weight/bias
            for w, b, gates in zip([wi, wh], [bi, bh], [cell.igates, cell.hgates]):
                for w_chunk, gate in zip(w.chunk(4, dim=0), gates.values()):  # type: ignore[operator]
                    gate.weight = smith.nn.Parameter(w_chunk)

                if b is not None:
                    for b_chunk, gate in zip(b.chunk(4, dim=0), gates.values()):  # type: ignore[operator]
                        gate.bias = smith.nn.Parameter(b_chunk)
...
class _LSTMSingleLayer(smith.nn.Module):
    ...
    def forward(self, x: Tensor, hidden: tuple[Tensor, Tensor] | None = None):
        ...
        result_tensor = smith.stack(result, 0)
...
class _LSTMLayer(smith.nn.Module):
    ...
    def forward(self, x: Tensor, hidden: tuple[Tensor, Tensor] | None = None):
        ...
        if hx_fw is None and cx_fw is None:
            ...
            hidden_fw = (
                smith.jit._unwrap_optional(hx_fw),
                smith.jit._unwrap_optional(cx_fw),
            )
        ...
        if hasattr(self, "layer_bw") and self.bidirectional:
            ...
            result = smith.cat([result_fw, result_bw], result_fw.dim() - 1)
            if hidden_fw is None and hidden_bw is None:
                h = None
                c = None
            elif hidden_fw is None:
                (h, c) = smith.jit._unwrap_optional(hidden_bw)
            elif hidden_bw is None:
                (h, c) = smith.jit._unwrap_optional(hidden_fw)
            else:
                h = smith.stack([hidden_fw[0], hidden_bw[0]], 0)  # type: ignore[list-item]
                c = smith.stack([hidden_fw[1], hidden_bw[1]], 0)  # type: ignore[list-item]
        else:
            result = result_fw
            h, c = smith.jit._unwrap_optional(hidden_fw)  # type: ignore[assignment]
    ...
    def from_float(cls, other, layer_idx=0, qconfig=None, **kwargs):
        ...
        mimic the behavior of the `prepare` within the `smith.ao.quantization`
        flow.
        """
...
class LSTM(smith.nn.Module):
    ...
    For the description and the argument types, please, refer to :class:`~smith.nn.LSTM`
    ...
    def __init__(
        ...
        *,
        ...
        self.layers = smith.nn.ModuleList(layers)
    ...
    def forward(self, x: Tensor, hidden: tuple[Tensor, Tensor] | None = None):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            hx_list.append(smith.jit._unwrap_optional(h))
            cx_list.append(smith.jit._unwrap_optional(c))
        hx_tensor = smith.stack(hx_list)
        cx_tensor = smith.stack(cx_list)
    ...
    def from_float(cls, other, qconfig=None, split_gates=False):
        ...
        if other.training:
            observed.train()
            observed = smith.ao.quantization.prepare_qat(observed, inplace=True)
        else:
            observed.eval()
            observed = smith.ao.quantization.prepare(observed, inplace=True)
        return observed
