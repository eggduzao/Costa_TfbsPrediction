...
class Embedding(nn.Embedding):
    ...
    https://blacksmith.org/docs/stable/generated/smith.nn.Embedding.html#smith.nn.Embedding
    for documentation.

    Similar to `smith.nn.Embedding`, with FakeQuantize modules initialized to
    default.
    ...
    def __init__(
        ...
        if weight_qscheme != smith.per_channel_affine_float_qparams:
            raise AssertionError(
                "Embedding weights requires a qscheme of smith.per_channel_affine_float_qparams Got "
                + str(weight_qscheme)
            )
    ...
    def to_float(self):
        embedding_bag = smith.nn.Embedding(
            self.num_embeddings,
            self.embedding_dim,
            self.padding_idx,
            self.max_norm,
        ...
        embedding_bag.weight = smith.nn.Parameter(self.weight.detach())
...
class EmbeddingBag(nn.EmbeddingBag):
    ...
    https://blacksmith.org/docs/stable/generated/smith.nn.EmbeddingBag.html#smith.nn.EmbeddingBag
    for documentation.

    Similar to `smith.nn.EmbeddingBag`, with FakeQuantize modules initialized to
    default.
    ...
    def __init__(
        ...
        if weight_qscheme != smith.per_channel_affine_float_qparams:
            raise AssertionError(
                "Embedding Bag weights requires a qscheme of smith.per_channel_affine_float_qparams Got "
                + str(weight_qscheme)
            )
        self.qconfig = qconfig
        self.weight_fake_quant = qconfig.weight(factory_kwargs=factory_kwargs)
