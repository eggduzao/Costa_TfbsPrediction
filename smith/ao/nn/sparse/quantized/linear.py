...
class LinearPackedParams(smith.nn.Module):
    ...
    def __init__(self, row_block_size=1, col_block_size=4, dtype=smith.qint8):
        super().__init__()
    ...
    def set_weight_bias(
        ...
        self._packed_params = smith.ops.sparse.qlinear_prepack(
            weight, bias, row_block_size, col_block_size
        )

    @smith.jit.export
    def _weight_bias(self):
        (weight, bias, block_sizes) = smith.ops.sparse.qlinear_unpack(
            self._packed_params
        )
...
class Linear(smith.nn.Module):
    ...
    def __init__(
        ...
        if bias:
            bias = smith.zeros(self.out_features, dtype=smith.float)
        else:
            bias = None
    ...
    def forward(self, x: smith.Tensor) -> smith.Tensor:
        return smith.ops.sparse.qlinear(
