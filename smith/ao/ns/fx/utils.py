...
from smith.ao.quantization import FakeQuantizeBase, ObserverBase
...
# TODO(future PR): consider deleting this enum and using the smith types
# directly.  This might be tricky because it is not a one to one mapping.
class NodeInputOrOutputType(enum.Enum):
    FP32 = enum.auto()  # smith.float
    INT8 = enum.auto()  # smith.qint8 or smith.quint8
    FP16 = enum.auto()  # smith.float16
    ...
    FP32_OR_INT8 = enum.auto()  # either smith.float or smith.quint8 or smith.qint8
...
def get_node_first_input_and_output_type(
    ...
    if node.op == "call_function":
        ...
        if node.target == "dequantize":
            ...
            if cur_node_dtype_target is not smith.float16:
...
def get_node_input_qparams(
    ...
    gm: GraphModule,
    node_type_to_io_type_map: dict[str, set[NSNodeTargetType]],
) -> tuple[smith.Tensor | float, smith.Tensor | int] | None:
    """
    ...
    if prev_node.op == "call_function":
        ...
        if prev_node.target is smith.quantize_per_tensor:
...
def get_arg_indices_of_inputs_to_log(node: Node) -> list[int]:
    ...
    * by default, returns [0]
    ...
    if node.op == "call_function" and (
        ...
        node.target in (smith.add, smith.ops.quantized.add, operator.add)
        or node.target in (smith.mul, smith.ops.quantized.mul, operator.mul)
    ):
...
def get_target_type_str(node: Node, gm: GraphModule) -> str:
    ...
    if node.op in ("call_function", "call_method"):
        target_type = smith.typename(node.target)
    elif node.op == "call_module":
        if not isinstance(node.target, str):
            raise AssertionError(f"Expected str, got {type(node.target)}")
        target_mod = getattr_from_fqn(gm, node.target)
        target_type = smith.typename(target_mod)
    return target_type
...
def maybe_dequantize_first_two_tensor_args_and_handle_tuples(f):
    def inner(*args, **kwargs):
        ...
        elif isinstance(a0, smith.Tensor) and isinstance(a1, smith.Tensor):
            if a0.is_quantized:
                a0 = a0.dequantize()
            if a1.is_quantized:
                a1 = a1.dequantize()
        ...
        if a0.dtype != smith.float or a1.dtype != smith.float:
            return None
...
def compute_sqnr(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    """
...
@maybe_dequantize_first_two_tensor_args_and_handle_tuples
def compute_normalized_l2_error(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    """
    ...
    return smith.sqrt(((x - y) ** 2).sum() / (x**2).sum())


@maybe_dequantize_first_two_tensor_args_and_handle_tuples
def compute_cosine_similarity(x: smith.Tensor, y: smith.Tensor) -> smith.Tensor:
    """
    ...
    x = x.reshape(1, -1)
    y = y.reshape(1, -1)
    return smith.nn.functional.cosine_similarity(x, y)
...
def get_normalized_nth_input(node: Node, gm: GraphModule, idx: int) -> Node:
    ...
    try:
        ...
        # requires typehints to proceed, such as for smith.add where
