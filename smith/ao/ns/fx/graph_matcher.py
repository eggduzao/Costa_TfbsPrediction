...
class _NSGraphMatchableSubgraphsIterator:
    ...
    def __next__(self) -> NSSubgraph:
        ...
        while len(self.stack) > 0:
            ...
            # Subgraphs like dequantize-linear-relu-to(smith.float16) have the
    ...
    def _recursively_add_node_arg_to_stack(self, arg: Any) -> None:
        ...
        if isinstance(arg, Node):
            ...
            isinstance(arg, smith.fx.immutable_collections.immutable_list)
            or type(arg) is tuple
        ):
            for inner_arg in arg:
                self._recursively_add_node_arg_to_stack(inner_arg)
        elif isinstance(arg, smith.fx.immutable_collections.immutable_dict):
...
def _get_name_for_subgraph(
    ...
    1. the name of the set containing the underlying type of the base op in the
       subgraph (i.e. 'smith.nn.functional.linear' if this is related to a linear op)
    ...
    `base_op_smith.nn.functional.linear_0`, and the name given to (linear0, relu0)
    will be `base_op_smith.nn.functional.linear_1`.
...
def get_matching_subgraph_pairs(
    ...
    `F.linear` and `smith.ops.quantized.linear` are related, and
    `F.linear` and `smith.nn.Conv` are not related.
