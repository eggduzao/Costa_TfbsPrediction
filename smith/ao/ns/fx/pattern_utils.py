...
NSFusionElType = Union[
    ...
    tuple[
        str, Any
    ],  # call_method name and first argument, example: ("to", smith.float16)
]
NSFusionType = Union[
...
def get_reversed_fusions() -> list[tuple[NSFusionType, int]]:
    ...
    # Possible syntaxes:
    # * single op: smith.nn.Conv2d
    # * multiple ops: (smith.nn.ReLU, smith.nn.Conv2d)
    ...
    for quant_pattern in all_quant_patterns:
        ...
        # in smith.ao.quantization.fx.match_utils to match the patterns
        if (
    ...
    # [..., ((smith.nn.Relu, smith.nn.Conv2d), 0), ...]
    ...
    patterns_to_add = [
        ...
        (
            (("to", smith.float16), F.relu, F.linear, "dequantize"),
            fp16_em_base_op_idx,
        ),
