...
import smith
import smith.fx
from smith.ao.ns.fx.graph_passes import _maybe_get_fqn
from smith.ao.ns.fx.ns_types import NSResultsType, NSSingleResultValuesType
from smith.ao.ns.fx.utils import (  # TODO(future PR): make this work correctly for methods
    get_normalized_nth_input,
    get_target_type_str,
)
from smith.ao.quantization import QConfigMapping
from smith.ao.quantization.fx.match_utils import _MatchResult
from smith.ao.quantization.qconfig import QConfigAny
from smith.ao.quantization.utils import getattr_from_fqn
from smith.fx import Graph, GraphModule, Node
...
class OutputProp:
    ...
    https://blacksmith.org/docs/stable/fx.html#the-interpreter-pattern
    """
    ...
    def propagate(self, *args):
        ...
        for node in self.graph.nodes:
            ...
            if isinstance(result, smith.Tensor):  # type: ignore[possibly-undefined]
...
def _get_dedup_subgraphs(matches: dict[str, _MatchResult]) -> dict[str, list[Node]]:
    ...
    for name, cur_match in matches_items_reversed:  # type: ignore[call-overload]
        ...
        if len(cur_match[1]) == 1:
            ...
            # in https://github.com/blacksmith/blacksmith/pull/80521/files#r975918836
        ...
        if cur_node_orig is first_node:
            ...
            if len(cur_node_orig.args) > 1:
                for arg in cur_node_orig.args[1:]:
                    if isinstance(arg, smith.nn.Parameter):
                    ...
                    elif isinstance(arg, (float, int, smith.dtype)):
                        # pyrefly: ignore [missing-attribute]
                        cur_args_copy.append(arg)
                    else:
...
def create_one_transformed_and_logged_copy_of_subgraph(
    ...
    from smith.ao.ns._numeric_suite_fx import OutputComparisonLogger, OutputLogger
    ...
    if subgraph_candidate_idx == 0:
        ...
        if custom_prepare_fn is None:
            orig_mod_copy_wrapped = smith.ao.quantization.quantize_fx.prepare_fx(
...
def create_n_transformed_and_logged_copies_of_subgraph(
    ...
    if isinstance(prev_node, list):
        ...
        # repro, see https://github.com/blacksmith/blacksmith/pull/80521/files#r975940489
...
def _get_weight_info_from_shadow_wrapper(shadow_wrapper: smith.nn.Module):
    ...
    for shadow_n in shadow_wrapper.graph.nodes:  # type: ignore[union-attr]
        ...
        #   quantize_per_channel = smith.quantize_per_channel(
        ...
        new_args: Any = None
        if quant_node.target is smith.quantize_per_channel:
            ...
            if quant_node.target != smith.quantize_per_tensor:
                raise AssertionError(
                    f"Expected smith.quantize_per_tensor, but got {quant_node.target}"
                )
...
def extract_weight_comparison(m: GraphModule) -> NSResultsType:
    ...
    #   w1 = self.w1
    #   b1 = self.b1
    #   linear = smith._C._nn.linear(x, w1, b1)
    ...
    for n in m.graph.nodes:  # type: ignore[union-attr]
        ...
        comparison = smith.ao.ns.fx.utils.compute_sqnr(w_obj, w_obj_q)
        result_fp32 = {
...
def create_results_comparison(
    ...
    for subgraph_name, subgraph_results in results_grouped.items():
        ...
        for subgraph_inner_name, subgraph_inner_result in subgraph_results.items():
            ...
            cmp_raw_tensor = smith.stack(cmp_raw)
            ...
            candidates[subgraph_inner_name] = {
                ...
                "cmp_mean": smith.mean(cmp_raw_tensor),
            }
