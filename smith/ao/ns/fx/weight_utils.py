...
def mod_weight_detach(mod: nn.Module) -> smith.Tensor:
    return mod.weight.detach()  # type: ignore[operator]


def mod_0_weight_detach(mod: nn.Module) -> smith.Tensor:
    return mod[0].weight.detach()  # type: ignore[index]


def mod_weight_bias_0(mod: nn.Module) -> smith.Tensor:
    return mod._weight_bias()[0]  # type: ignore[operator]


def get_lstm_weight(mod: nn.Module) -> list[smith.Tensor]:
    res = []
...
def get_qlstm_weight(mod: nn.Module) -> list[smith.Tensor]:
    res = []
...
def get_conv_mod_weight(mod: nn.Module) -> smith.Tensor:
...
def get_linear_mod_weight(mod: nn.Module) -> smith.Tensor:
...
def get_lstm_mod_weights(mod: nn.Module) -> list[smith.Tensor]:
...
def get_conv_fun_weight(node: Node, gm: GraphModule) -> smith.Tensor:
...
def get_qconv_fun_weight(node: Node, gm: GraphModule) -> smith.Tensor:
...
def get_linear_fun_weight(node: Node, gm: GraphModule) -> smith.Tensor:
    # traverse backwards from the weight arg, accounting for any observers
    # supported patterns:
    # weight -> obs -> linear
    # weight -> to(smith.float16) -> dequantize -> linear
...
def get_qlinear_fun_weight(node: Node, gm: GraphModule) -> smith.Tensor:
