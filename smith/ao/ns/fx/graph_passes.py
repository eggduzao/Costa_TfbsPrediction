...
import smith
from smith.ao.ns.fx.mappings import get_node_type_to_io_type_map
from smith.ao.quantization.fx.utils import get_new_attr_name_with_prefix
            ...
            if node in node_to_instrument_inputs_to_ref_node_name:
                ...
                for node_arg_idx in arg_indices_to_log:
                    ...
                    if type(node_arg) is Node:
                        ...
                        type(node_arg) is smith.fx.immutable_collections.immutable_list
                    ):
...
def _insert_quantize_per_tensor_node(
    ...
    gm_b: GraphModule,
    graph_c: Graph,
    scale: smith.Tensor | float,
    zero_point: smith.Tensor | int,
    dtype_cast_name: str,
) -> Node:
    # copy scale
    ...
    return graph_c.create_node(
        ...
        (prev_node_c, scale_node, zero_point_node, smith.quint8),
        {},
        dtype_cast_name,
    )
...
def _insert_dtype_cast_after_node(
    ...
    if (
        ...
        if node_a_input_qparams is not None:
            dtype_cast_op = smith.quantize_per_tensor  # type: ignore[assignment]
        ...
        dtype_cast_method_dtype = smith.float16
    else:
...
def _insert_copy_of_node_a_after_input_node_c(
    ...
    def _copy_arg(arg):
        ...
        if isinstance(arg, Node):
            ...
            return arg
        elif isinstance(arg, (int, float, smith.dtype)):
            return arg
