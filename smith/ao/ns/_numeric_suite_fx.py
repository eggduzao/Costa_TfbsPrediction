...
across models. Example usage::
    ...
    m = smith.nn.Sequential(smith.nn.Conv2d(1, 1, 1)).eval()
    mp = quantize_fx.prepare_fx(m, {"": smith.ao.quantization.default_qconfig})
    ...
    ns.extend_logger_results_with_comparison(
        weight_comparison, "a", "b", smith.ao.ns.fx.utils.compute_sqnr, "sqnr"
    )
    ...
    # add SQNR for each comparison, inplace
    ns.extend_logger_results_with_comparison(
        act_comparison, "a", "b", smith.ao.ns.fx.utils.compute_sqnr, "sqnr"
    )
    ...
    # add SQNR for each comparison, inplace
    ns.extend_logger_results_with_comparison(
        shadow_act_comparison, "a", "b", smith.ao.ns.fx.utils.compute_sqnr, "sqnr"
    )
...
from smith.ao.ns.fx.graph_matcher import get_matching_subgraph_pairs
from smith.ao.ns.fx.mappings import get_base_name_to_sets_of_related_ops
from smith.ao.ns.fx.n_shadows_utils import (
...
from smith.ao.quantization import QConfigMapping
from smith.ao.quantization.backend_config import BackendConfig
...
from smith.ao.quantization.fx.match_utils import _find_matches
...
RNNReturnType = tuple[smith.Tensor, tuple[smith.Tensor, smith.Tensor]]
...
class OutputLogger(nn.Module):
    ...
    # Note: cannot annotate the type of x because SmithScript does not support
    #   the Union type.
    def forward(self, x):
        # fmt: off
        """
...
class OutputComparisonLogger(OutputLogger):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.comparison_fn = smith.ao.ns.fx.utils.compute_sqnr
...
class NSTracer(quantize_fx.QuantizationTracer):
    ...
    def is_leaf_module(self, m: smith.nn.Module, module_qualified_name: str) -> bool:
        # fmt: off
        """
        """  # blank docblock to make autodoc happy
        # fmt: on
        if isinstance(m, smith.ao.quantization.ObserverBase):
            return True
        elif isinstance(m, smith.ao.quantization.FakeQuantizeBase):
            return True
...
def extract_weights(
    ...
    smith._C._log_api_usage_once("quantization_api._numeric_suite_fx.extract_weights")
...
def _add_loggers_impl(
    ...
    smith._C._log_api_usage_once("quantization_api._numeric_suite_fx._add_loggers_impl")
    matched_subgraph_pairs = get_matching_subgraph_pairs(
        gm_a, gm_b, base_name_to_sets_of_related_ops, unmatchable_types_map
    )
...
def add_loggers(
    ...
    smith._C._log_api_usage_once("quantization_api._numeric_suite_fx.add_loggers")
    # TODO(future PR): expose these
    skipped_module_names: list[str] = []
    skipped_module_classes: list[Callable] = []
    tracer_a = NSTracer(skipped_module_names, skipped_module_classes)
...
def _extract_logger_info_one_model(
    ...
    for _gm_name, mod in model.named_modules():
        ...
        is_logger = isinstance(mod, logger_cls) or (  # type: ignore[arg-type]
            isinstance(mod, smith.jit.RecursiveScriptModule)
...
def extend_logger_results_with_comparison(
    ...
    model_name_1: str,
    model_name_2: str,
    comparison_fn: Callable[[smith.Tensor, smith.Tensor], smith.Tensor],
    comparison_name: str,
) -> None:
    """
...
def prepare_n_shadows_model(
    ...
    * add examples to docblocks
    ...
    mt = smith.fx.GraphModule(model, tracer.trace(model))
...
def loggers_set_enabled(model: smith.nn.Module, enabled: bool) -> None:
    """
    ...
    for node in model.graph.nodes:
        ...
        if node.name.startswith(SHADOW_WRAPPER_NODE_NAME_PREFIX):
            ...
            if custom_convert_fn is None:
                converted_mod = smith.ao.quantization.quantize_fx.convert_fx(orig_mod)
            else:
...
def extract_results_n_shadows_model(model: smith.nn.Module) -> NSResultsType:
    """
