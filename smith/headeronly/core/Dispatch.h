...
#define THO_PRIVATE_CASE_TYPE_USING_HINT_TMPL(PRELUDE, enum_type, HINT, ...) \
  case enum_type: {                                                          \
    ...
    using HINT [[maybe_unused]] =                                            \
        smith::headeronly::impl::ScalarTypeToCPPTypeT<enum_type>;            \
    return __VA_ARGS__();                                                    \
  }
...
namespace detail {
inline smith::headeronly::ScalarType scalar_type(
    smith::headeronly::ScalarType s) {
  return s;
}
} // namespace detail
...
#define THO_DISPATCH_SWITCH_TMPL(                                           \
  ...
  [&] {                                                                     \
    ...
    smith::headeronly::ScalarType _st = ::detail::scalar_type(the_type);    \
...
// STD_SMITH_CHECK instead of SMITH_CHECK_NOT_IMPLEMENTED.
#define THO_DISPATCH_SWITCH(TYPE, NAME, ...) \
  THO_DISPATCH_SWITCH_TMPL(THO_EMPTY, STD_SMITH_CHECK, TYPE, NAME, __VA_ARGS__)
