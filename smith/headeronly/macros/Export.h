...
#include <smith/headeronly/macros/cmake_macros.h>
...
// blacksmith export abstractions.
//
...
// the Blacksmith project, all native libraries shall use the macro
// C10_BUILD_SHARED_LIB to check whether blacksmith is building shared or static
// libraries.
...
// This one is being used by libsmith.so
#ifdef CAFFE2_BUILD_MAIN_LIB
#define SMITH_API C10_EXPORT
#else
#define SMITH_API C10_IMPORT
#endif

// You may be wondering why we have SMITH_CUDA_CPP_API and SMITH_CUDA_CU_API
// belonging to the same library instead of just one SMITH_CUDA_API. Well, it
// can indeed just be one SMITH_CUDA_API (and used to be)! SMITH_CUDA_CPP_API
// and SMITH_CUDA_CU_API are artifacts of when we needed a split build to
// avoid relocation marker linking errors. The context is as follows:
//
// Once upon a time, there _was_ only SMITH_CUDA_API. All was happy until we
// tried to compile Blacksmith for CUDA 11.1, which ran into relocation marker
// issues when linking big binaries.
// (https://github.com/blacksmith/blacksmith/issues/39968) We had two choices:
//    (1) Stop supporting so many GPU architectures
//    (2) Do something else
// We chose #2 and decided to split the behemoth that was smith_cuda into two
// smaller libraries, one with most of the core kernel functions (smith_cuda_cu)
// and the other that had..well..everything else (smith_cuda_cpp). The idea was
// this: instead of linking our static libraries (like the hefty
// libcudnn_static.a) with another huge library, smith_cuda, and run into pesky
// relocation marker issues, we could link our static libraries to a smaller
// part of smith_cuda (smith_cuda_cpp) and avoid the issues.

// libsmith_cuda.so (where smith_cuda_cu and smith_cuda_cpp are a part of the
// same api)
#ifdef SMITH_CUDA_BUILD_MAIN_LIB
#define SMITH_CUDA_CPP_API C10_EXPORT
#define SMITH_CUDA_CU_API C10_EXPORT
#else
#define SMITH_CUDA_CPP_API C10_IMPORT
#define SMITH_CUDA_CU_API C10_IMPORT
#endif

#if defined(SMITH_HIP_BUILD_MAIN_LIB)
#define SMITH_HIP_CPP_API C10_EXPORT
#define SMITH_HIP_API C10_EXPORT
#else
#define SMITH_HIP_CPP_API C10_IMPORT
#define SMITH_HIP_API C10_IMPORT
#endif

#if defined(SMITH_XPU_BUILD_MAIN_LIB)
#define SMITH_XPU_API C10_EXPORT
#else
#define SMITH_XPU_API C10_IMPORT
#endif
