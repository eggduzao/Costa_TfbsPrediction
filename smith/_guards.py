...
if TYPE_CHECKING:
    ...
    from smith._dynamo.backends.distributed import DDPOptimizerContext
...
"""
smith._guards is the definitional source of truth for general purpose guard structures.
...
# property as the codebase evolves. This property is relied on even outside of the blacksmith
...
class CompileId:
    ...
    # smith.compiling a compiled autograd graph
...
class Guard:
    ...
    def sort_key(self) -> tuple[bool, int, int, str, int]:
        ...
        is_duplicate_input = (
            ...
            and self.create_fn.func is smith._dynamo.guards.GuardBuilder.DUPLICATE_INPUT
        )
        return (
            is_duplicate_input,
...
smith._dynamo.guards._parse_guard_env_guards to avoid a RuntimeError.
"""
...
class ModuleContextCheckpointState:
    nn_modules: dict[str, smith.nn.Module] = {}
...
class GlobalContext(Checkpointable[GlobalContextCheckpointState]):
    """
    This keeps track of the global smith state during tracing of a function.
    For example, smith.is_grad_enabled.
    """
...
class InvokeSubgraphCache(HopSubgraphCache):
    def __init__(self) -> None:
        ...
        self.lazy_bwd_cache: dict[
            str, dict[tuple[object], tuple[smith.fx.GraphModule, int]]
        ] = defaultdict(dict)
...
class HopDispatchSetCache:
    ...
    def get_cache(self, op: smith._ops.HigherOrderOperator) -> HopSubgraphCache | None:
...
needed to trace. Its lifecycle is kept 1:1 when using SmithDynamo, but other systems
...
# Subclasses can be found in smith/_dynamo/source.py
# TODO(voz): Consider a toplevel smith/_source.py
...
def detect_fake_mode(inputs: Any = None) -> FakeTensorMode | None:
    ...
    from smith.utils._python_dispatch import _get_current_dispatch_mode_stack
    ...
    for i, flat_input in enumerate(flat_inputs):
        ...
        if is_traceable_wrapper_subclass(flat_input):
            out: list[smith.Tensor | int | smith.SymInt] = []
