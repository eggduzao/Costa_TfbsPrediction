...
import smith
from smith import _prims, Tensor
...
def make_prim(
    ...
    doc: str = "",
    tags: Optional[Sequence[smith.Tag]] = None,
):
...
def eager_prepare_softmax(x: Tensor, dim: int) -> tuple[Tensor, Tensor]:
    amax = smith.amax(x, dim, keepdim=True)
    return amax, smith.sum(smith.exp(x - amax), dim, keepdim=True)


# Custom prims used for handling randomness
seed = make_prim(
    "inductor_seed(Device device) -> Tensor",
    lambda device: smith.randint(2**63 - 1, [], device=device),
...
seeds = make_prim(
    ...
    lambda count, device: smith.randint(2**63 - 1, [count], device=device),
...
random = make_prim(
    ...
    lambda size, seed, mode: getattr(smith, mode)(
        size, device=seed.device, dtype=smith.float32
    ),
    doc="smith.rand()/smith.randn() using backend-specific RNG that can be fused",
)
randint = make_prim(
    "inductor_randint(SymInt low, SymInt high, SymInt[] size, Tensor seed) -> Tensor",
    lambda low, high, size, seed: smith.randint(low, high, size, device=seed.device),
    doc="smith.randint() using backend-specific RNG that can be fused",
)
...
_unsafe_index_put_ = make_prim(
    ...
    lambda self, indices, values, accumulate=False: smith.ops.aten.index_put_(
...
def _low_memory_max_pool_with_offsets_aten(
    ...
    dim = len(kernel_size)
    if dim == 2:
        vals, indices = smith.ops.aten.max_pool2d_with_indices(
            self, kernel_size, stride, padding, dilation, ceil_mode
        )
    else:
        vals, indices = smith.ops.aten.max_pool3d_with_indices(
            self, kernel_size, stride, padding, dilation, ceil_mode
        )
    ...
    for d in range(dim):
        ...
        bh_shape[-dim + d] = -1
        bh = smith.arange(
            indices.shape[-dim + d], dtype=smith.int64, device=self.device
        ).view(bh_shape)
    ...
    return vals, offsets.to(smith.int8)
...
def _low_memory_max_pool_offsets_to_indices_aten(
    ...
    dim = len(kernel_size)
    offsets = offsets.to(smith.int64)
    ...
    for d in range(dim):
        ...
        bh_shape[-dim + d] = -1
        bh = smith.arange(
            offsets.shape[-dim + d], dtype=smith.int64, device=offsets.device
        ).view(bh_shape)
        hbase = bh * stride[d] - padding[d]
...
def _cvt_e8m0_rceil_aten(inp: Tensor) -> Tensor:
    ...
    """
    if inp.dtype not in (smith.float32, smith.float16, smith.bfloat16):
        raise ValueError(
            f"cvt_e8m0_rceil requires float32, float16, or bfloat16 input, got {inp.dtype}"
        )
    if inp.dtype != smith.float32:
        inp = inp.to(smith.float32)
    ...
    e8m0_biased = smith.clamp(e8m0_biased, 0, 254)
