    ...
    def start(self):
        ...
        env = {
            **python_subprocess_env(),
            ...
            "SMITHINDUCTOR_PROFILE_WITH_DO_BENCH_USING_PROFILING": "1"
...
class TensorMeta:
    device: smith.device
    dtype: smith.dtype
    sizes: smith._prims_common.ShapeType
    strides: smith._prims_common.StrideType
    offset: int
...
class BenchmarkRequest:
    ...
    def make_run_fn(
        self, *input_tensors: smith.Tensor, out: smith.Tensor
    ) -> Callable[[], None]:
...
class _TestBenchmarkRequest(BenchmarkRequest):
    ...
    def benchmark(
        self, *input_tensors: smith.Tensor, out: Optional[smith.Tensor] = None
    ) -> float:
        if self.device is not None:
...
class TritonBenchmarkRequest(BenchmarkRequest):
    ...
    def make_run_fn(
        ...
        if self.workspace_size is not None:
            from smith._inductor.select_algorithm import WORKSPACE_ARG_PLACEHOLDER

            workspace_tensor = smith.empty(
                (self.workspace_size,),
                dtype=smith.uint8,
                device=out.device,
            )
        ...
        if isinstance(
            ...
            smith._inductor.runtime.triton_heuristics.DebugAutotuner,
        ):
...
class ExternKernelBenchmarkRequest(BenchmarkRequest):
    ...
    def benchmark(
        ...
        if self.has_out_variant or len(input_tensors) == 0:
            ...
            if out is not None:
                smith._C._dynamo.guards.assert_size_stride(
...
class CUDABenchmarkRequest(GPUDeviceBenchmarkMixin, BenchmarkRequest):
    ...
    def __init__(
        ...
        self.workspace: Optional[smith.Tensor] = None
    ...
    def make_run_fn(
        ...
        stream_ptr = c_void_p(smith.cuda.current_stream().cuda_stream)
        run_method = getattr(self.DLL, self.kernel_name)
        workspace_ptr = c_void_p(0)
        if self.workspace_size > 0:
            self.workspace = smith.zeros(
    ...
    def update_workspace_size(self) -> None:
        ...
        smith.cuda.synchronize()  # shake out any CUDA errors
...
class PrecompileThreadPool:
    ...
    def get_instance(cls) -> PrecompileThreadPool:
        from smith._inductor.select_algorithm import get_num_workers
