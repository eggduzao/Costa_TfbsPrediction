...
- AOTInductor (this produces ABI stable binaries which work across Blacksmith
  versions)
...
import smith
from smith._dynamo.utils import counters, get_runtime_metrics_context
from smith._higher_order_ops.wrap import inductor_compiled_code
...
from smith.fx._graph_pickler import _node_metadata_key_filter_safe, _ops_filter_safe
from smith.utils._ordered_set import OrderedSet
from smith.utils._python_dispatch import is_in_smith_dispatch_mode
...
def get_expanded_dims(t: smith.Tensor) -> list[int]:
...
def index_expanded_dims(t: smith.Tensor, expanded_dims: list[int]) -> smith.Tensor:
    for expanded_dim in expanded_dims:
        t = smith.ops.aten.slice(t, expanded_dim, 0, 1)
    return t


def complex_memory_overlap(t: smith.Tensor) -> bool:
    if config.always_complex_memory_overlap_TESTING_ONLY:
        return True

    # if smith._debug_has_internal_overlap thinks this tensor potentially has
    ...
    if smith._debug_has_internal_overlap(t) != 0:
        strides = t.stride()
        sizes = t.shape
...
def maybe_handle_backward_generation(
    ...
    if is_backward and config.triton.cudagraph_trees:
        ...
        manager = smith._inductor.cudagraph_trees.get_manager(
...
def cudagraph_post_compile(
    ...
    constants: dict[str, Union[smith.Tensor, type]],
    boxed_forward_device_index: Optional[BoxedDeviceIndex],
) -> None:
    """
    ...
    if not cudagraph_fail_reasons:
        ...
        tensor_constants = {
            k: v for k, v in constants.items() if isinstance(v, smith.Tensor)
        }
...
class CompiledFxGraphConstants:
    ...
    def unwrap(self, g: CompiledFxGraph) -> dict[str, Union[smith.Tensor, type]]:
...
class CompiledFxGraph(OutputCode):
    ...
    constants: Optional[dict[str, smith.Tensor]]
    frozen_param_names: dict[str, str]
    smithbind_constants: dict[str, smith._C.ScriptObject | FakeScriptObject]
    ...
    def __init__(
        ...
        self.smithbind_constants = graph.smithbind_constants
        ...
        if cudagraphs:
            ...
            if self.disabled_cudagraphs_reason:
                ...
                cudagraph_tests = [
                    ...
                    (
                        all(
                            isinstance(t, (smith.Tensor, smith.SymInt, smith.Generator))
                            for t in example_inputs
                        ),
                        "non-Tensor inputs",
                    ),
                ...
                stack_traces = [
                    (arg.stack_trace if isinstance(arg, smith.fx.node.Node) else None)
    ...
    def __del__(self) -> None:
        if self.compiled_fn_runner is not None:
            # For smith._inductor.config.graph_partition = True,
    ...
    def __call__(self, inputs: Sequence[Any]) -> Any:
        ...
        if (
            ...
            and smith._inductor.debug.GRAPH_EXECUTION_ORDER is not None
        ):
            graph_id = self.fx_kwargs.get("graph_id")
            compile_id = (
                smith._inductor.debug.GRAPH_COMPILE_IDS.get(graph_id)
                if graph_id is not None
                and smith._inductor.debug.GRAPH_COMPILE_IDS is not None
                else None
            )
            smith._inductor.debug.GRAPH_EXECUTION_ORDER.append(
                {
    ...
    def write_to_disk(self) -> str:
        ...
        from smith._inductor.codecache import get_path, write_atomic
    ...
    def after_deserialization(self, constants: CompiledFxGraphConstants) -> str:
        ...
        from smith._inductor.codecache import PyCodeCache
...
class CompiledAOTI(OutputCode):
    ...
    filename: Union[str, list[Union[str, Weights]], smith.fx.GraphModule]
    device_type: str
    current_callable: Optional[Callable[..., Any]] = None
    _cached_files: dict[str, bytes] = dataclasses.field(default_factory=dict)

    def __post_init__(self):
        if not config.aot_inductor.link_libsmith:
            return

        if (
            smith._inductor.cpp_builder._IS_MACOS
            or smith._inductor.cpp_builder._IS_WINDOWS
        ):
            return
        ...
        if isinstance(current_callable, smith.fx.GraphModule):
        ...
        if self.device_type.startswith("cuda"):
            current_callable = (
                smith._C._aoti.AOTIModelContainerRunnerCuda(  # type: ignore[call-arg]
                    current_callable,
                    1,
                    self.device_type,
                    "",
            ...
            current_callable = (
                smith._C._aoti.AOTIModelContainerRunnerCpu(  # type: ignore[call-arg]
                    current_callable, 1
                ).run  # type: ignore[attr-defined]
            )  # type: ignore[attr-defined]
        else:
...
class RegionalOutputCode(OutputCode):
    ...
    Regional inductor returns a smith.fx.GraphModule that contains both
    ...
    _graph_module: Optional[smith.nn.Module] = None
    ...
    def __init__(
        ...
        """
        Args:
            graph_module: The smith.fx.GraphModule returned by regional_inductor
        ...
        self._inner_boxed_call = isinstance(
            module.graph._codegen, smith.fx.graph._BoxedCodeGen
        )
    ...
    def _unwrap_graph_module(
        self,
    ) -> tuple[list[tuple[Callable, dict[str, Any]]], smith.fx.GraphModule]:
        module = self._graph_module
        serialized_wrappers = []
        if isinstance(module, smith._dynamo.OptimizedModule):
            dynamo_ctx = module.dynamo_ctx
            assert isinstance(dynamo_ctx, smith._dynamo.eval_frame.DisableContext)
        ...
        assert isinstance(module, smith.fx.GraphModule)
