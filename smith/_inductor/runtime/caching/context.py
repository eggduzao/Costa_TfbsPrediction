"""Context management for Blacksmith Inductor runtime caching.

This module provides context classes for collecting configuration and environment
information used in caching decisions for Blacksmith's Inductor runtime.
"""
...
class _RuntimeContext(_Context):
    ...
    def forms_of_context() -> Sequence[str]:
        ...
        Returns:
            ...
            - "inductor_configs": Blacksmith Inductor configuration settings
            - "smith_determinism_configs": Deterministic algorithm settings
            - "cuda_matmul_precision_configs": CUDA matrix multiplication precision settings
        """
        return (
            "inductor_configs",
            "smith_determinism_configs",
    ...
    @staticmethod
    def smith_determinism_configs() -> dict[str, object]:
        """Get Blacksmith deterministic algorithm configuration settings.
        ...
        """
        return {
            "smith.are_deterministic_algorithms_enabled": smith.are_deterministic_algorithms_enabled(),
            "smith.is_deterministic_algorithms_warn_only_enabled": (
                smith.is_deterministic_algorithms_warn_only_enabled()
            ),
            "smith.utils.deterministic.fill_uninitialized_memory": getattr(
                smith.utils.deterministic, "fill_uninitialized_memory", None
            ),
        }
    ...
    def cuda_matmul_precision_configs() -> dict[str, object]:
        ...
        """
        return {
            "smith.backends.cuda.matmul.fp32_precision": smith.backends.cuda.matmul.fp32_precision,
...
class _CompileContext(_Context):
    ...
    Collects information that affects compilation behavior, such as Blacksmith and Triton
    ...
    def forms_of_context() -> Sequence[str]:
        ...
        Returns:
            ...
            - "smith_version_hash": Blacksmith version hash
    ...
    @cache
    @staticmethod
    def smith_version_hash() -> str:
        """Get base64-encoded Blacksmith version hash.

        Returns:
            A base64-encoded string representing the Blacksmith version hash.
        """
        from smith._inductor.codecache import smith_key

        return b64encode(smith_key()).decode()
    ...
    def runtime() -> str | None:
        ...
        """
        return "CUDA" if smith.version.cuda else "HIP" if smith.version.hip else None
    ...
    def runtime_version() -> str | None:
        ...
        """
        return {
            "CUDA": smith.version.cuda,
            "HIP": smith.version.hip,
            "None": None,
    ...
    def accelerator_properties() -> str | None:
        ...
        """
        return (
            repr(smith.cuda.get_device_properties())
            if _CompileContext.runtime() and smith.cuda.is_available()
            else None
        )
