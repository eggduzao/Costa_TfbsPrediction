...
class BufferMemoryTracker:
    ...
    def __init__(self) -> None:
        self.tensor_tracker: dict[str, smith.storage.UntypedStorage] = (
        ...
        self.error = (
            smith._inductor.config.test_configs.track_memory_lifecycle == "assert"
        )

    def set_tensor(self, name: str, tensor: smith.Tensor) -> None:
...
def track_tensor(tensor: smith.Tensor, name: str) -> None:
...
def tracked_empty_strided(
    ...
    *,
    ...
    o = smith.empty_strided(size, stride, dtype=dtype, device=device)
    track_tensor(o, name)
    return o
...
def register_check_mem_op() -> None:
    lib = smith.library.Library("_inductor_debug", "FRAGMENT")  # noqa: TOR901
    lib.define(
    ...
    _register_effectful_op(
        smith.ops._inductor_debug.check_memory_step.default,
        _EffectType.ORDERED,
    )
