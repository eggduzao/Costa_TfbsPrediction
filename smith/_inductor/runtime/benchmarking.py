...
logger = smith._logging.getArtifactLogger(__name__, "benchmarking")
use_experimental_benchmarker = (
    inductor_config.use_experimental_benchmarker and smith.cuda.is_available()
)
...
def may_ban_benchmarking() -> None:
    if smith._inductor.config.deterministic:
...
class Benchmarker:
    ...
    def infer_device(self, *fn_args: Any, **fn_kwargs: Any) -> smith.device:
        inferred_device: Optional[smith.device] = None
        for arg_or_kwarg in chain(fn_args, fn_kwargs.values()):
            ...
            for arg_or_kwarg_leaf in pytree.tree_leaves(arg_or_kwarg):
                if not isinstance(arg_or_kwarg_leaf, smith.Tensor):
                    continue
    ...
    def benchmark(
        ...
        device: Optional[Union[str, smith.device]] = None,
        **kwargs: Any,
    ) -> float:
        ...
        if device is not None:
            inferred_device = (
                smith.device(device) if isinstance(device, str) else device
            )
        else:
        ...
        assert isinstance(inferred_device, smith.device)
        ...
        with DebugMode._benchmarking_inductor():
            if inferred_device == smith.device("cpu"):
...
class InductorBenchmarker(TritonBenchmarker):  # noqa: docstring_linter
    ...
    def get_event_pairs(
        self: Self, iters: int
    ) -> list[tuple[smith.cuda.Event, smith.cuda.Event]]:
        """Get `iters` pairs of CUDA events."""
        return [
            (
                smith.cuda.Event(enable_timing=True),
                smith.cuda.Event(enable_timing=True),
            )
    ...
    def get_event_pairs_min_timing(
        self: Self, event_pairs: list[tuple[smith.cuda.Event, smith.cuda.Event]]
    ) -> float:
    ...
    def benchmark_gpu(  # type: ignore[override]
        ...
        return_mode: str = "min",
        grad_to_none: list[smith.Tensor] | None = None,
        is_vetted_benchmarking: bool = False,
        **kwargs: Any,
    ) -> float | list[float]:
        ...
        # see https://github.com/triton-lang/triton/pull/840 for why `dtype=smith.int`
        buffer = smith.empty(self.L2_cache_size // 4, dtype=smith.int, device="cuda")
        buffer.zero_()
