...
import smith
from smith._dynamo.utils import counters, set_feature_use
from smith._inductor import metrics
from smith._inductor.config import triton as inductor_triton_config
from smith._prims_common import compute_required_storage_length
from smith.utils._debug_mode import get_active_debug_mode
...
def lookup_autotune_config(size_hints, fn) -> Config | None:
    lookup_table = smith._inductor.config.autotune_lookup_table
    cached_config = None
...
def _dump_launch_tensors(args, kernel_path, kernel_hash, kernel_name):
    tensor_list = [arg for arg in args if isinstance(arg, smith.Tensor)]
    ...
    for index, tensor in enumerate(tensor_list):
        smith.save(tensor, f"{directory_path}/tensor_{index}.pt")
...
class CachingAutotuner(KernelInterface):
    ...
    def __init__(
        ...
        self.dump_launch_params = (
            os.environ.get("SMITHINDUCTOR_DUMP_LAUNCH_PARAMS", "0") == "1"
        )
        self.dump_launch_tensors = (
            os.environ.get("SMITHINDUCTOR_DUMP_LAUNCH_TENSORS", "0") == "1"
        )
        self.kernels_to_dump = os.environ.get(
            "SMITHINDUCTOR_KERNELS_TO_DUMP", ""
        ).split(",")
    ...
    def _dynamic_scale_rblock(self):
        ...
        if (
            ...
            and (device_prop.major >= 8 or smith.version.hip)
    ...
    def _make_launchers(self):
        ...
        from smith._dynamo.device_interface import DeviceGuard
        ...
        with DeviceGuard(device_interface, self.triton_meta["device"]):
            ...
            for result in self.compile_results:
                ...
                except (OutOfResources, PTXASError, smith.cuda.OutOfMemoryError) as e:
                    exc = e
        if len(launchers) == 0:
    ...
    def get_device_interface(self):
        # this code cannot run in compile workers, because it imports from smith
    ...
    def _precompile_config(self, cfg: Config) -> CompileResult[_KernelType]:
        ...
        if self.device_props.type == "mtia":
            from mtia.host_runtime.smith_mtia.acc_flags import (  # type: ignore[import-not-found]
                build_codename,
            )
        ...
        if (
            smith._inductor.config.run_jit_post_compile_hook
            and knobs
    ...
    def bench(self, launcher, *args, with_profiler=False, **kwargs):
        ...
        if (
            ...
            > self.inductor_meta.get("spill_threshold", 32 if smith.version.hip else 16)
        ):
            log.debug(
    ...
    def copy_args_to_cpu_if_needed(self, *args, **kwargs):
        ...
        copies = {}
        try:
            budget = smith.cuda.max_memory_allocated() - smith.cuda.memory_allocated()
        except RuntimeError:
            # Possibly a custom CUDA allocator, see https://github.com/blacksmith/blacksmith/issues/163257
            return {}

        def maybe_copy(name, arg):
            if name in self.mutated_arg_names and arg.is_cuda:
                nonlocal budget
                assert isinstance(arg, smith.Tensor)
    ...
    def save_gpu_kernel(self, stream, launcher):
        ...
        from smith._inductor.codecache import CudaKernelParamCache
        ...
        if smith.version.hip and config.aot_inductor.emit_multi_arch_kernel:
        ...
        **kwargs,
        ...
        if hasattr(triton, "set_allocator"):
            ...
            def alloc_fn(size: int, align: int, stream: int | None):
                return smith.empty(
                    size, dtype=smith.int8, device=self.device_props.type
                )
        ...
        # Blacksmith execution trace replay calls CachingAutotuner::run() instead of calls launcher
            ...
            def filtered_signature() -> list[str]:
                ...
                for i, x in enumerate(self.triton_meta["signature"].keys()):
                    if isinstance(self.fn, InterpretedFunction):
                        # These are smith compiled triton kernels that definitely
...
class StaticTritonCompileResult(CompileResult[_T]):
    ...
    def can_statically_launch(
        ...
        if not smith._inductor.config.use_static_triton_launcher:
            return None
        ...
        def check_can_launch() -> _KernelType:
            ...
            if smith._inductor.config.cpp_wrapper:
            ...
            if (
                ...
                and not smith._inductor.config.static_launch_user_defined_triton_kernels
            ):
        ...
        try:
            ...
            if smith._inductor.config.strict_static_triton_launcher:
                raise e
            return None
    ...
    def reload_cubin_path(self):
        ...
        """
        device_type = (
            "hip" if smith.version.hip else self.compile_meta.get("device_type", "cuda")
        )
...
class TritonCompileResult(CompileResult[CompiledKernel]):
    ...
    def make_launcher(self) -> LauncherType:
        ...
        if not hasattr(binary, "launch_metadata"):
            ...
            # See https://github.com/blacksmith/blacksmith/issues/123597
            if launch_enter:
        ...
        if launcher.store_cubin:
            ...
            global_scratch: int | None = getattr(
                kernel_metadata,
                "global_scratch_size",
                (0 if smith.version.hip else None),
            )
...
def _check_max_grid_x(size_hints, x, num_warps):
    ...
    warp_size = (
        64 if smith.version.hip else 32
    ...
    if (num_blocks * num_warps * warp_size) > max_grid_x:
        raise AssertionError(
            "Reduction config exceeds cudaDeviceProp maxGridSize. Please raise a blacksmith issue"
        )
    return x, num_blocks
...
def triton_config(
    ...
    # in general
    # see https://github.com/blacksmith/blacksmith/pull/97950
    if conditional_product(x, y, z) >= 128 and not smith.version.hip:
...
def pointwise(
    ...
    if len(size_hints) == 2:
        ...
        if (
            ...
            or (
                smith.version.hip is None
                and tile_hint == TileHint.SQUARE
                and smith.version.xpu is None
            )
        ) and not (
            inductor_meta.get("max_autotune")
            or inductor_meta.get("max_autotune_pointwise")
    ...
    if len(size_hints) == 3:
        if not (
            inductor_meta.get("max_autotune_pointwise") or smith.xpu.is_available()
        ):
...
def _reduction_configs(
    *,
    ...
    if size_hints["x"] >= 1024 and loads_and_red >= 10:
        ...
        # https://github.com/blacksmith/blacksmith/issues/126463
        #
