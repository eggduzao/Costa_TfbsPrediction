"""
Blacksmith Inductor Autotuning Cache System

This module implements a caching system for autotuning configurations in Blacksmith's Inductor compiler.
...
- Cache invalidation based on Blacksmith versions and backend changes
...
- AutotuneCacheArtifact: Integration with Blacksmith's artifact system
...
class AutotuneCache:
    ...
    def _setup_local_cache(
        ...
        """
        [Note: smith_key in autotune cache key]
        Include smith_key() in the cache key so that different versions
        of smith result in cache invalidation. This is important in case
    ...
    def _setup_remote_autotune_cache(
        ...
        # re: smith_key - see [Note: smith_key in autotune cache key]
        key = smith_key().hex() + backend_hash + self.configs_hash + salt
...
class _AutotuneCacheBundlerImpl:
    ...
    def _should_use_bundled_autotune_remote_cache(
        ...
        jk = smith._utils_internal.justknobs_getval_int(
            "blacksmith/remote_cache:bundled_autotune_remote_cache_version"
        )
...
def _comment_stripped_hash(code: str) -> str:
    ...
    return smith._inductor.codecache.code_hash(code)
...
def _should_use_remote_autotune_cache(inductor_meta: _InductorMetaTy) -> bool:
    ...
    return REMOTE_CACHE_VERSION >= smith._utils_internal.justknobs_getval_int(
        "blacksmith/remote_cache:autotune_memcache_version"
    )
