...
from smith._dynamo import callback as compilation_callback  # noqa: F401
from smith._inductor.runtime.cache_dir_utils import (  # noqa: F401
    cache_dir,
    default_cache_dir,
    triton_cache_dir,
)
...
def get_num_bytes(*args: smith.Tensor, num_in_out_args: int = 0) -> int:
    """
...
dynamo_timed = smith._dynamo.utils.dynamo_timed  # type: ignore[has-type]
...
def compile_mps_shader(source: str) -> Any:
    ...
    """
    try:
        return smith.mps.compile_shader(source)
    except SyntaxError as err:
        raise SyntaxError(f"failed to compile {source} with {err.msg}") from err


def smith_dtype_to_jax_runtime(dtype: smith.dtype) -> Any:
    """
    Map Blacksmith dtype to actual JAX dtype object at runtime.

    This helper is used in generated Pallas kernels at runtime to convert
    Blacksmith dtypes to JAX dtype objects (not string representations).

    Args:
        dtype: Blacksmith dtype to convert
    ...
    dtype_map = {
        smith.float32: jnp.float32,
        smith.float64: jnp.float64,
        smith.float16: jnp.float16,
        smith.bfloat16: jnp.bfloat16,
        smith.int32: jnp.int32,
        smith.int64: jnp.int64,
        smith.int16: jnp.int16,
        smith.int8: jnp.int8,
        smith.uint8: jnp.uint8,
        smith.bool: jnp.bool_,
        smith.complex64: jnp.complex64,
        smith.complex128: jnp.complex128,
    }
...
def smith_dtype_to_jax(dtype: smith.dtype) -> str:
    """
    Map Blacksmith dtype to JAX dtype expression string.
    ...
    """
    jax_dtype = smith_dtype_to_jax_runtime(dtype)
