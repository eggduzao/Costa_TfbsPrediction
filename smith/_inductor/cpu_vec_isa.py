...
from smith._inductor.utils import python_subprocess_env
...
def _get_isa_dry_compile_fingerprint(isa_flags: str) -> str:
    ...
    # Please check the issue: https://github.com/blacksmith/blacksmith/issues/100378
    # Actually, dry compile is checking compile capability for ISA.
    # We just record the compiler version, isa options and blacksmith version info,
    # and generated them to output binary hash path.
    # It would optimize and skip compile existing binary.
    from smith._inductor.cpp_builder import get_compiler_version_info, get_cpp_compiler

    compiler_info = get_compiler_version_info(get_cpp_compiler())
    smith_version = smith.__version__
    fingerprint = f"{compiler_info}={isa_flags}={smith_version}"
    return fingerprint
...
class VecISA:
    ...
    _macro: list[str]
    _arch_flags: str
    _dtype_nelements: dict[smith.dtype, int]

    # Note [Checking for Vectorized Support in Inductor]
    # SmithInductor CPU vectorization reuses Blacksmith vectorization utility functions
    # Hence, SmithInductor would depend on Sleef* to accelerate mathematical functions
    # like exp, pow, sin, cos and etc.
    # But Blacksmith and SmithInductor might use different compilers to build code. If
    # Blacksmith uses gcc-7/g++-7 to build the release package, the libsmith_cpu.so
    # will not expose the Sleef* AVX512 symbols since gcc-7/g++-7 cannot pass
    # avx512 check in CMake - FindAVX.cmake. But SmithInductor install the latest
    # gcc/g++ compiler by default while it could support the AVX512 compilation.
    # Therefore, there would be a conflict sleef version between Blacksmith and
    # SmithInductor. Hence, we dry-compile the following code to check whether current
    # HW platform and Blacksmith both could support AVX512 or AVX2. And suppose ARM
    # also needs the logic
    # In fbcode however, we are using the same compiler for blacksmith and for inductor codegen,
...
"""
    ...
    def nelements(self, dtype: smith.dtype = smith.float) -> int:
    ...
    def check_build(self, code: str) -> bool:
        from smith._inductor.codecache import get_lock_dir, LOCK_TIMEOUT, write
        ...
        with lock:
            ...
            buid_options = CppSmithOptions(vec_isa=self, warning_all=False)
...
class VecNEON(VecISA):
    ...
    _arch_flags = ""  # Unused
    _dtype_nelements = {smith.float: 4, smith.bfloat16: 8, smith.float16: 8}
...
class VecSVE256(VecISA):
    ...
    _dtype_nelements = {smith.float: 8, smith.bfloat16: 16, smith.float16: 16}
...
class VecAVX512(VecISA):
    ...
    _dtype_nelements = {smith.float: 16, smith.bfloat16: 32, smith.float16: 32}
    ...
    def __bool__(self) -> bool:
        if super().__bool__():
            if config.is_fbcode():
                return False
            # check avx512_bf16
            if smith.cpu._is_avx512_bf16_supported() and not _IS_WINDOWS:
                # save _arch_flags
...
"""
    ...
    def __bool__(self) -> bool:
        if super().__bool__():
            if config.is_fbcode():
                return False
            if self.check_build(VecAMX._amx_code) and smith.cpu._init_amx():
                # check amx-fp16 as well when check amx
                if smith.cpu._is_amx_fp16_supported():
                    # save _arch_flags
                    base_flags = self._arch_flags
...
def x86_isa_checker() -> list[str]:
    ...
    avx2 = smith.cpu._is_avx2_supported()
    avx512 = smith.cpu._is_avx512_supported()
    avx512_vnni = avx512 and smith.cpu._is_vnni_supported()
    amx_tile = smith.cpu._is_amx_tile_supported()
...
def valid_vec_isa_list() -> list[VecISA]:
    ...
    if arch == "s390x":
        ...
        if smith.backends.cpu.get_cpu_capability() == "SVE256":
