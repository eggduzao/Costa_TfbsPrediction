...
a pass (such as smith._inductor.fx_passes.joint_graph.patterns).
...
from smith.fx.graph_module import _get_attr
...
from smith.fx.traceback import preserve_node_meta
from smith.utils._ordered_set import OrderedSet

from .._funcsmith import config as funcsmith_config
from .._funcsmith.aot_autograd import aot_function, make_boxed_func
from .._funcsmith.partitioners import default_partition
...
Constant = Any
NodeOrConstant = Union[Constant, smith.fx.Node]
...
FnsType = Union[smith.fx.node.Target, str]
...
def _transfer_meta(
    new_meta: dict[str, Any], old_node: smith.fx.Node, pass_name: str = ""
) -> None:
    from smith.fx.traceback import NodeSource, NodeSourceAction
    ...
    if config.trace.provenance_tracking_level == 1:
        ...
        new_meta.update(
            ...
            if k in smith.fx.proxy._COPY_META_FIELDS
        )
...
class Match:
    ...
    args: list[Any]
    kwargs: dict[str, Any]
    nodes: list[smith.fx.Node]
    targets: dict[_TargetExpr, smith.fx.node.Target]
    ctx: MatchContext
    replacement_graph: Optional[smith.fx.GraphModule]
    ...
    def replace_by_example(
        ...
        """
        from smith._inductor.virtualized import NullHandler, V
        ...
        def should_propagate_eager_input_vals(nodes: list[smith.fx.Node]) -> bool:
            if len(nodes) != 1:
                return False
            node = nodes[0]
            ...
            return node.target in OrderedSet(
                [
                    ...
                    smith.ops.higher_order.auto_functionalized_v2,
                ]
            )
        ...
        with context:
            ...
            if should_propagate_eager_input_vals(self.nodes):
                ...
                def record(node: smith.fx.Node, val: Any) -> None:
                    if isinstance(node, smith.fx.Node):
                        node_to_val[node] = val

                smith.utils._pytree.tree_map(
                    record, (match_args, match_kwargs), (fake_args, fake_kwargs)
                )
                # map args to their FakeTensor val in eager_input_vals
                example_vals = smith.fx.map_arg(args, lambda arg: node_to_val[arg])

                # first graph
                graph_with_eager_vals = trace_fn(replacement_fn, example_vals)

                # second graph
                example_vals = smith.fx.map_arg(args, lambda arg: arg.meta["val"])
...
class MatchContext:
    ...
    pattern_to_node: dict[PatternExpr, Optional[smith.fx.Node]]
    ...
    def __init__(
        self,
        outputs: list[Optional[PatternExpr]],
        pattern_to_node: Optional[dict[PatternExpr, smith.fx.Node]] = None,
        *,
        graph: smith.fx.Graph,
    ) -> None:
    ...
    def filter_multi_user_patterns(self) -> dict[PatternExpr, smith.fx.Node]:
        return {
            pattern: node
...
class PatternExpr(ABC):
    ...
    @abstractmethod
    def _match(self, node: smith.fx.Node, ctx: MatchContext) -> MatchResult: ...

    def match(self, node: smith.fx.Node) -> MatchResult:
        try:
    ...
    def find_anchor_nodes(
        self, ctx: MatchContext, searched: OrderedSet[smith.fx.Node]
    ) -> Generator[Optional[smith.fx.Node], None, None]:
...
class _TargetExpr(PatternExpr):
    ...
    def fns_repr(self) -> str:
        ...
        if len(self.fns) > 1:
            return f"[{first_repr}, ...]"
        elif self.fns[0] is getattr(smith, first_repr, None):
            return f"smith.{first_repr}"
        elif self.fns[0] is getattr(operator, first_repr, None):
            return f"operator.{first_repr}"
        elif isinstance(self.fns[0], smith._ops.OpOverload):
    ...
    def _match_fns(self, node: smith.fx.Node) -> bool:
        return (
            isinstance(node, smith.fx.Node)
            and node.op == self.op
            and extract_target(node) in self.fns_set
        )

    def _match_users(self, node: smith.fx.Node, ctx: MatchContext) -> bool:
        return (
            self in ctx.outputs
...
class _TargetArgsExpr(_TargetExpr):
    ...
    def __init__(
        self,
        fns: Union[smith.fx.node.Target, str, Sequence[Any]],
        *args: Any,
    ...
    def _match(self, node: smith.fx.Node, ctx: MatchContext) -> MatchResult:
        ...
        for pattern, child_node in zip(self_items, node_items):
            ...
            elif isinstance(child_node, smith.fx.Node) or child_node != pattern:
                return FailedMatch(
...
class ListOf(PatternExpr):
    ...
    def _match(self, node: list[smith.fx.Node], ctx: MatchContext) -> MatchResult:  # type: ignore[override]
...
class PatternPrettyPrinter:
    ...
    def __init__(self) -> None:
        self.namespace = smith.fx.graph._Namespace()
    ...
    def memoize(self, obj: _TargetArgsExpr) -> str:
        ...
        for prefix in ("aten.", "smith.", "prims."):
...
class _PassDictsType(Protocol):
    def __getitem__(
        self, k: tuple[str, smith.fx.node.Target]
    ) -> list[PatternEntry]: ...
...
class PatternEntry:
    ...
    def apply(self, match: Match, graph: smith.fx.Graph, node: smith.fx.Node) -> None:
        raise NotImplementedError

    def register(
        self,
        pass_dicts: Union[_PassDictsType, Sequence[_PassDictsType]],
        target: Union[smith.fx.node.Target, None] = None,
        prepend: bool = False,
    ) -> None:
        if target is None:
...
class ReplacementPatternEntry(PatternEntry):
    ...
    def replace_with_graph(
        match: Match,
        graph: smith.fx.Graph,
        replacement_graph: Union[smith.fx.Graph, smith.fx.GraphModule],
        args: Sequence[smith.fx.Node],
    ) -> None:
        """
        Inserts the replacement graph into the toplevel graph at the match
        """

        added_replacement_nodes: list[smith.fx.Node] = []

        class Replacer(smith.fx.Interpreter):
            call_method = None  # type: ignore[assignment]
            call_module = None  # type: ignore[assignment]
            get_attr = None  # type: ignore[assignment]

            def run_node(self, node: smith.fx.Node) -> Any:
                ...
                if node.op == "call_function":
                    ...
                    if "val" in node.meta and "val" not in result.meta:
                        ...
                        if isinstance(node.meta["val"], smith.Tensor):
                ...
                if node.op == "get_attr":
                    ...
                    if not isinstance(sub_gm, smith.fx.GraphModule):
                        raise NotImplementedError(
        ...
        def percolate_tags(
            ...
            tag_name: str,
            tag_value: str,
            input_stops: OrderedSet[smith.fx.Node],
        ) -> None:
            queue = [node]
            visited = OrderedSet[smith.fx.Node]()
        ...
        with graph.inserting_before(last_node):
            assert isinstance(replacement_graph, smith.fx.GraphModule)
            replacement = Replacer(replacement_graph).run(*args)
            if isinstance(replacement, smith.fx.Node):
                replacement = [replacement]

            def maybe_getitem(node: smith.fx.Node) -> Any:
                if node.op != "call_function":
                    return None
            ...
            def replace(
                old: Union[smith.fx.Node, None],
                new: Union[smith.fx.Node, Sequence[smith.fx.Node], None],
            ) -> None:
                def filter_nodes_in_newly_added_nodes(node: smith.fx.Node) -> bool:
                ...
                if old is None:
                    assert new is None
                    return
                assert isinstance(old, smith.fx.Node)
                if new is None:
                    ...
                    return
                if isinstance(new, smith.fx.Node):
...
def register_replacement(
    ...
    def check_fn(match: Match) -> bool:
        ...
        sym_args: list[smith.SymInt] = []
        fake_mode = smith._dynamo.utils.detect_fake_mode(args)
        assert fake_mode is not None
        with fake_mode:
            for i, grad in enumerate(requires_grad):
                if isinstance(args[i], smith.Tensor):
                    if grad and is_integer_dtype(args[i].dtype):
                        return False

                    args[i] = smith.empty_strided(
                        args[i].size(),
                        args[i].stride(),
                        dtype=args[i].dtype,
                    ...
                    for v in itertools.chain(args[i].shape, args[i].stride()):
                        if isinstance(v, smith.SymInt) and all(
            ...
            if os.environ.get("SMITHINDUCTOR_PATTERN_MATCH_DEBUG") == node.name:
                log.warning(
    ...
    with funcsmith_config.patch(functionalize_rng_ops=False):
        requires_grad: list[bool] = [
            isinstance(x, smith.Tensor) and x.requires_grad for x in example_inputs
        ]
...
def _serialize_pattern(
    ...
    def get_file_template() -> str:
        ...
        for name in dir(smith._inductor.pattern_matcher):
            attr = getattr(smith._inductor.pattern_matcher, name)
            try:
        ...
        formatted_imports = f"from smith._inductor.pattern_matcher import (\n   {formatted_imports},\n)\n"
...
_known_precompiled_patterns: list[
    tuple[
        Any,
        Iterable[Any],
        Callable[[Callable[..., Any], Iterable[Any]], smith.fx.GraphModule],
        Any,
        PatternExpr,
    ]
] = []
...
def gen_register_replacement(
    ...
    if "BLACKSMITH_GEN_PATTERNS" in os.environ:
        ...
        m = importlib.import_module(
            f"smith._inductor.fx_passes.serialized_patterns.{pattern_name}"
        )
        if not m or not hasattr(m, unique_name):
            log.warning(
                "Precompiled pattern %r not found. Run smithgen/fuse/gen_patterns.py.",
                unique_name,
            )
...
@funcsmith_config.patch(functionalize_rng_ops=False)  # type: ignore[misc]
def gen_pattern_and_search_gm(
    ...
    scalar_workaround: Union[dict[str, Union[float, int]], None] = None,
    exclusive_arg_names: Sequence[str] = (),
) -> tuple[PatternExpr, smith.fx.GraphModule]:
    ...
    return (
        fx_to_pattern(
            search_gm,
            ignore_types=(int, float, list, smith.device, smith.dtype),
            argnames=argnames,
            scalar_workaround=scalar_workaround,
            exclusive_arg_names=exclusive_arg_names,
        ),
...
def is_start_of_fx_graph(graph: smith.fx.Graph, node: smith.fx.Node) -> bool:
...
def fixme_incorrect_inductor_schema_op(op: smith._ops.OpOverload) -> bool:
    ...
    # Dont add any more !
    return op in (
        smith.ops.inductor.accumulate_grad_.default,
        smith.ops.inductor.resize_storage_bytes_.default,
    )


def is_mutation_op(node: smith.fx.Node) -> bool:
    if isinstance(
        ...
        node.target, smith._higher_order_ops.auto_functionalize.AutoFunctionalized
    ):
        return False
    if node.op == "call_function":
        assert callable(node.target)
...
def same_mutation_regions(a: smith.fx.Node, b: smith.fx.Node) -> bool:
...
def get_mutation_region_id(graph: smith.fx.Graph, node: smith.fx.Node) -> int:
    n = node
...
def should_compute_mutation_region_ids(graph: smith.fx.Graph) -> bool:
    return "mutation_region_id" not in next(iter(graph.nodes)).meta


def compute_mutation_region_ids(graph: smith.fx.Graph) -> None:
...
class PatternMatcherPass:
    def __init__(
        ...
        self.patterns: defaultdict[
            tuple[str, smith.fx.node.Target], list[PatternEntry]
        ] = defaultdict(list)
    ...
    def __getitem__(self, item: tuple[str, smith.fx.node.Target]) -> list[PatternEntry]:
        return self.patterns[item]

    def apply(self, gm: Union[smith.fx.GraphModule, smith.fx.Graph]) -> int:
        if not self.patterns:
            return 0
        if isinstance(gm, smith.fx.GraphModule):
            graph = gm.graph
        elif isinstance(gm, smith.fx.Graph):
            graph = gm
...
def fx_to_pattern(
    gm: Union[smith.fx.GraphModule, smith.fx.Graph],
    ...
    # see https://github.com/blacksmith/blacksmith/issues/97894
    ...
    class Converter(smith.fx.Interpreter):
        ...
        def run_node(self, n: smith.fx.Node) -> Any:
            rv = super().run_node(n)
...
def fwd_only(
    ...
    *,
    ...
    if run_functional_passes:
        ...
        # https://github.com/blacksmith/blacksmith/pull/170649#issuecomment-3693427775
        # for more details.
...
@smith.enable_grad()
def joint_fwd_bwd(fn: Callable[..., Any], args: Sequence[Any]) -> smith.fx.GraphModule:
    """Build a normalized training graph, for use with fx_to_pattern"""
    gm: Optional[smith.fx.GraphModule] = None

    def record_joint_graph(
        joint_graph: smith.fx.GraphModule, inputs: Sequence[Any], **kwargs: Any
    ...
    with smith._guards.tracing(None):
        aot_function(
            fn,
            # pyrefly: ignore[bad-argument-type]
...
def _args(n: smith.fx.Node) -> list[smith.fx.node.Argument]:
    args: list[smith.fx.node.Argument] = []
    smith.fx.map_arg((n.args, n.kwargs), args.append)
    return args


def stable_topological_sort(graph: smith.fx.Graph) -> None:
    ...
    #   order.
    ready = OrderedSet[smith.fx.Node]()
...
def init_once_fakemode(fn: Callable[..., Any]) -> Callable[[], Any]:
    ...
    def lazy_init() -> Any:
        ...
        with smith._guards.tracing(None), unset_fake_temporarily(), FakeTensorMode():
            result = fn()
...
def clone_graph(input_graph: smith.fx.GraphModule) -> smith.fx.GraphModule:
    class CopyGraph(Transformer):
        def run_node(self, old_node: smith.fx.Node) -> smith.fx.Node:
            new_node = super().run_node(old_node)
            if isinstance(new_node, smith.fx.Proxy):
...
def get_arg_value(
    node: smith.fx.Node, arg_number: int, kwarg_name: Optional[str] = None
) -> Any:
...
def filter_nodes(nodes: Iterable[smith.fx.Node], fn: Any) -> list[smith.fx.Node]:
    fns = [fn]
    if isinstance(fn, smith._ops.OpOverloadPacket):
...
def extract_target(node: smith.fx.Node) -> smith.fx.node.Target:
