...
from smith._dynamo.utils import dynamo_timed, lazy_format_graph_code
from smith._funcsmith.aot_autograd import MutationType
from smith._funcsmith.compile_utils import fx_graph_cse
from smith._inductor.constant_folding import constant_fold, replace_node_with_constant
from smith._inductor.freezing_utils import enter_freezing, record_has_frozen_params
from smith._inductor.fx_passes.freezing_patterns import freezing_passes
from smith._inductor.fx_passes.post_grad import view_to_reshape
...
def replace_params_with_constants(
    ...
    fw_metadata: smith._funcsmith.aot_autograd.ViewAndMutationMeta,
) -> list[int]:
    """
    Replaces the parameters of a Blacksmith GraphModule with constants wherever possible.
...
def freeze(
    dynamo_gm: smith.fx.GraphModule,
    aot_autograd_gm: smith.fx.GraphModule,
    example_inputs: list[smith._subclasses.FakeTensor],
) -> tuple[smith.fx.GraphModule, list[int]]:
    """
    ...
    Args:
        dynamo_gm (smith.fx.GraphModule): The Dynamo constructed GraphModule.
        aot_autograd_gm (smith.fx.GraphModule): The aot_autograd constructed GraphModule to be frozen.
        example_inputs (List[smith.Tensor]): A list of example input tensors to be used in the freezing process.

    Returns:
        Tuple[smith.fx.GraphModule, List[int]]: A tuple containing the frozen GraphModule and a list of indices
...
class ErasedTensor(smith.Tensor):
    @staticmethod
    ...
    def __smith_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        ...
        raise RuntimeError(
            f"Trying to run Blacksmith Eager Module after Dynamo Freezing. "
...
def invalidate_eager_modules():
    with smith.utils._python_dispatch._disable_current_modes():
        for (
            mod
        ) in smith._guards.TracingContext.get().module_context.nn_modules.values():
            ...
            for attr_name, tensor in list(
                ...
                if isinstance(tensor, smith.nn.Parameter):
                    e_t.requires_grad_(True)
                    e_t._is_param = True
                setattr(mod, attr_name, e_t)


def discard_traced_gm_params(mod: smith.fx.GraphModule):
    with smith.utils._python_dispatch._disable_current_modes():
        for attr_name, tensor in list(
            itertools.chain(
                mod.named_parameters(recurse=False),
...
def enforce_output_layout(gm: smith.fx.GraphModule):
    """
    ...
    *_, output_node = gm.graph.nodes
    ...
    with gm.graph.inserting_before(output_node):
        for n in out_list:
            ...
            ) or not smith._prims_common.is_non_overlapping_and_dense_or_false(
                n.meta["val"]
            ):
                continue
...
def enforce_as_strided_input_layout(gm: smith.fx.GraphModule):
    """
...
def convert_conv_weights_to_channels_last(gm: smith.fx.GraphModule):
    """
    ...
    with dynamo_timed("convert_conv_weights_to_channels_last"):
        ...
        for conv in convs:
            ...
            if len(weight_node.meta["val"].size()) != 4 or weight_node.meta[
                "val"
            ].is_contiguous(memory_format=smith.channels_last):
