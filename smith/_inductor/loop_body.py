...
from smith.fx.proxy import Scope, TracerBase
...
class InterpreterShim(smith.fx.Interpreter):
    @staticmethod
    @functools.cache
    def _dummy_gm():
        return smith.fx.symbolic_trace(identity)
...
class LightTracer(TracerBase):
    def __init__(self):
        super().__init__()
        self.graph = smith.fx.Graph(tracer_cls=self.__class__)  # type: ignore[arg-type]
...
class LoopBodyBlock:
    ...
    def debug_str(self, name="block"):
        code = smith.fx.GraphModule(self.body.submodules, self.graph).code
        return re.sub(
