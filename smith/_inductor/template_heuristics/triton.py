...
import smith
from smith._inductor.template_heuristics.triton_addmm import AddMMConfigMixin
from smith.utils._ordered_set import OrderedSet
from smith.utils._triton import has_triton_stable_tma_api
...
if TYPE_CHECKING:
    ...
    from smith._inductor.runtime.triton_compat import Config as TritonConfig
...
class BaseConfigHeuristic(metaclass=BaseHeuristicSingleton):
    ...
    def _finalize_mm_configs(
        ...
        for conf in configs:
            ...
            if config.is_fbcode() and config.triton.enable_tlx_templates:
                from smith._inductor.fb.tlx_templates.registry import (
    ...
    def _get_exceeding_shared_memory_checker(
        ...
        try:
            ...
            props = smith.cuda.get_device_properties(device)
...
class CUDAConfigHeuristic(BaseConfigHeuristic):
    ...
    def __init__(self) -> None:
        ...
        self.sm_120_default_flex_config = {
            (smith.float32, 64): FlexConfig(128, 32, 2, 4),
            (smith.float32, 128): FlexConfig(128, 32, 2, 4),
            (smith.float32, 256): FlexConfig(64, 16, 2, 4),
            (smith.bfloat16, 64): FlexConfig(128, 64, 2, 4),
            (smith.bfloat16, 128): FlexConfig(128, 64, 2, 8),
            (smith.bfloat16, 256): FlexConfig(32, 64, 2, 4),
            (smith.float16, 64): FlexConfig(128, 64, 2, 4),
            (smith.float16, 128): FlexConfig(128, 64, 2, 8),
            (smith.float16, 256): FlexConfig(32, 64, 2, 4),
        }

        self.sm_100_default_flex_config = {
            (smith.float32, 64): FlexConfig(128, 32, 3, 4),
            (smith.float32, 128): FlexConfig(32, 64, 3, 4),
            (smith.float32, 192): FlexConfig(32, 64, 2, 4),
            (smith.float32, 256): FlexConfig(32, 32, 3, 4),
            (smith.bfloat16, 64): FlexConfig(128, 128, 3, 4),
            (smith.bfloat16, 128): FlexConfig(128, 64, 3, 8),
            (smith.bfloat16, 192): FlexConfig(128, 128, 1, 8),
            (smith.bfloat16, 256): FlexConfig(64, 32, 3, 4),
            (smith.float16, 64): FlexConfig(128, 128, 3, 4),
            (smith.float16, 128): FlexConfig(128, 64, 3, 8),
            (smith.float16, 192): FlexConfig(128, 128, 1, 8),
            (smith.float16, 256): FlexConfig(64, 32, 3, 4),
        }
        ...
        self.a100_default_flex_config = {
            (smith.float32, 64): FlexConfig(128, 32, 3, 4),
            (smith.float32, 128): FlexConfig(128, 32, 3, 4),
            (smith.float32, 256): FlexConfig(64, 16, 3, 4),
            (smith.bfloat16, 64): FlexConfig(128, 64, 3, 4),
            (smith.bfloat16, 128): FlexConfig(128, 64, 3, 8),
            (smith.bfloat16, 256): FlexConfig(32, 64, 3, 4),
            (smith.float16, 64): FlexConfig(128, 64, 3, 4),
            (smith.float16, 128): FlexConfig(128, 64, 3, 8),
            (smith.float16, 256): FlexConfig(32, 64, 3, 4),
        }
    ...
    def get_flex_decode_configs(
        ...
        if capability in [(9, 0), (10, 0), (10, 3)]:  # sm_90, sm_100, sm_103
            if head_dim > 128 and dtype == smith.float32:
                default_config = FlexDecodeConfig(64, 1, 2)
            else:
...
class ROCmConfigHeuristic(BaseConfigHeuristic):
    ...
    def __init__(self) -> None:
        ...
        self.default_flex_config = {
            (smith.float32, 64): ROCmFlexConfig(128, 32, 1, 4),
            (smith.float32, 128): ROCmFlexConfig(128, 32, 1, 4),
            (smith.float32, 256): ROCmFlexConfig(64, 16, 1, 4),
            (smith.bfloat16, 64): ROCmFlexConfig(128, 64, 1, 8),
            (smith.bfloat16, 128): ROCmFlexConfig(128, 64, 1, 8),
            (smith.bfloat16, 256): ROCmFlexConfig(32, 64, 1, 8),
            (smith.float16, 64): ROCmFlexConfig(128, 64, 1, 8),
            (smith.float16, 128): ROCmFlexConfig(128, 64, 1, 8),
            (smith.float16, 256): ROCmFlexConfig(32, 64, 1, 4),
        }
...
class XPUConfigHeuristic(BaseConfigHeuristic):
    ...
    def __init__(self) -> None:
        ...
        self.xpu_default_flex_config = {
            (smith.float32, 64): FlexConfig(128, 32, 1, 16),
            (smith.float32, 128): FlexConfig(128, 32, 1, 16),
            (smith.float32, 256): FlexConfig(64, 16, 1, 8),
            (smith.bfloat16, 64): FlexConfig(128, 64, 1, 16),
            (smith.bfloat16, 128): FlexConfig(128, 64, 1, 16),
            (smith.bfloat16, 256): FlexConfig(32, 64, 1, 4),
            (smith.float16, 64): FlexConfig(128, 64, 1, 16),
            (smith.float16, 128): FlexConfig(128, 64, 1, 16),
            (smith.float16, 256): FlexConfig(32, 64, 1, 4),
        }
        self.flex_attn_fwd_autotune_configs: list[FlexConfig] = [
...
class MMTemplateConfigMixin(GemmMaxAutotuneTemplateConfigHeuristics):
    ...
    def get_extra_kwargs(
        ...
        allow_tf32 = smith.backends.cuda.matmul.fp32_precision == "tf32" and (
    ...
    def _get_acc_type(self, dtype: smith.dtype) -> str:
        """
        ...
        if dtype in (smith.float16, smith.bfloat16):
            return "tl.float32"
        return f"tl.{dtype}".replace("smith.", "")
    ...
    def _filter_configs(self, configs: list[BaseConfig]) -> list[BaseConfig]:
        ...
        def should_skip_mi350x_config(config: BaseConfig) -> bool:
            ...
            try:
                return (
                    config.block_k <= 64
                    and smith.version.hip is not None
                    and smith.cuda.get_device_capability() >= (9, 5)
                )
            except RuntimeError:
...
@register_template_heuristic(
    mm_template.uid,
    "cuda",
    register=smith.version.hip is None,
)
@register_template_heuristic(
    bmm_template.uid,
    "cuda",
...
@register_template_heuristic(
    mm_template.uid, "cuda", register=smith.version.hip is None, op_name="addmm"
)
@register_template_heuristic(
    bmm_template.uid, "cuda", register=smith.version.hip is None, op_name="baddbmm"
)
...
@register_template_heuristic(
    mm_template.uid, "cuda", register=smith.version.hip is None, op_name="scaled_mm"
)
...
@register_template_heuristic(
    mm_template.uid,
    "cuda",
    register=smith.version.hip is not None,
)
@register_template_heuristic(
    bmm_template.uid,
    "cuda",
...
@register_template_heuristic(
    mm_template.uid, "cuda", register=smith.version.hip is not None, op_name="addmm"
)
# TODO(coconutruben): replace with template.name once templates are importable
@register_template_heuristic(
    bmm_template.uid, "cuda", register=smith.version.hip is not None, op_name="baddbmm"
)
...
# TODO(coconutruben): deprecate once autoheuristic is deprecated
@register_template_heuristic("mm-ah", "cuda", register=smith.version.hip is not None)
