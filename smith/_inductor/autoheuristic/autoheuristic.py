...
from smith._inductor.autoheuristic.learned_heuristic_controller import (
    LearnedHeuristicController,
)
from smith._inductor.ir import ChoiceCaller
...
class AutoHeuristic:
    ...
    a heuristic (see smithgen/autoheuristic/).
    """
    ...
    def __init__(
        ...
        if smith._inductor.config.autoheuristic_log_path == "DEFAULT":
            self.log_path = self.get_default_log_path()
        else:
            self.log_path = smith._inductor.config.autoheuristic_log_path

        if smith._inductor.config.collect_autoheuristic(self.name):
    ...
    def get_choice(self) -> Choice:
        ...
        if smith._inductor.config.use_autoheuristic(self.name):
    ...
    def get_device_identifier() -> str:
        ...
        device_name = smith.cuda.get_device_name().replace(" ", "_")
        return device_name
...
class AutoHeuristicSelectAlgorithm(AutoHeuristic):
    ...
    def __init__(
        ...
        if (
            smith._inductor.config.collect_autoheuristic(self.name)
