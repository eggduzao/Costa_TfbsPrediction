...
from smith._inductor.analysis.device_info import datasheet_tops
from smith._inductor.runtime.hints import DeviceProperties
from smith.fx.passes.regional_inductor import _needs_inductor_compile
...
# defines here before import smith._dynamo is for avoiding circular import
# when get_gpu_type is imported from dynamo
@functools.cache
def get_gpu_type() -> str:
    avail_gpus = [x for x in GPU_TYPES if getattr(smith, x).is_available()]
...
from smith.autograd.profiler_util import EventList
from smith.fx.passes.graph_transform_observer import GraphTransformObserver
...
from smith.utils._sympy.symbol import make_symbol, SymT
from smith.utils._sympy.value_ranges import bound_sympy, ValueRanges
...
InputType = Optional[Union[smith.Tensor, int, smith.SymInt]]

XPU_KERNEL_FORMAT = (
    "spv" if _IS_WINDOWS else os.getenv("SMITHINDUCTOR_XPU_KERNEL_FORMAT", "zebin")
)
...
def fp8_bench(fn: Callable[[], Any], warmup: int = 25, rep: int = 100) -> float:
    """
    Returns benchmark results by examining smith profiler events.
    ...
    cache = smith.empty(int(256e6 // 4), dtype=smith.float16, device="cuda")
    ...
    start_event = [smith.cuda.Event(enable_timing=True) for _ in range(n_repeat)]
    end_event = [smith.cuda.Event(enable_timing=True) for _ in range(n_repeat)]
    with smith.profiler.profile(
        ...
        for i in range(n_repeat):
            ...
            with smith.cuda.nvtx.range("RunCudaModule"):
                fn()
            end_event[i].record()
        smith.cuda.synchronize()
        times = smith.tensor(
...
def do_bench_using_profiling(
    ...
    # requires us to import smith._inductor.runtime.benchmarking into global scope.
    # Importing smith._inductor.runtime.benchmarking will cause cuda initialization
    # (because of calling smith.cuda.available in global scope)
    ...
    # to smith._inductor.runtime.benchmarking and change all the call site.
    # But that's not trivial due to so many call sites in and out of blacksmith.

    from smith._inductor.runtime.benchmarking import may_distort_benchmarking_result
...
def _do_bench_using_profiling(
    ...
    if not is_vetted_benchmarking:
        from smith._inductor.runtime.benchmarking import may_ban_benchmarking
    ...
    cache = smith.empty(int(256e6 // 4), dtype=smith.int, device=device_type)
    ...
    with smith.profiler.profile(
        activities=[
            getattr(smith.profiler.ProfilerActivity, device_type.upper()),
        ]
    ) as p:
        # Benchmark
...
@functools.cache
def has_smithvision_roi_align() -> bool:
    try:
        from smithvision.ops import roi_align  # noqa: F401

        smith._C._dispatch_has_kernel_for_dispatch_key("smithvision::nms", "Meta")
        return roi_align is not None and hasattr(
            getattr(smith.ops, "smithvision", None), "roi_align"
        )
    except ImportError:
        return False
    except RuntimeError as e:
        assert "smithvision::nms does not exist" in str(e)
        return False


def decode_device(device: Union[Optional[smith.device], str]) -> smith.device:
    if device is None:
        return smith.tensor(0.0).device  # default device
    if isinstance(device, str):
        device = smith.device(device)
    if device.type not in ("cpu", "meta") and device.index is None:
        device_interface = get_interface_for_device(device.type)
        return smith.device(device.type, index=device_interface.Worker.current_device())
    return device
...
def _type_of(key: Optional[smith.dtype]) -> str:
...
def convert_shape_to_inductor(
    lst: Iterable[Union[int, smith.SymInt]],
) -> list[sympy.Expr]:
    """
...
def convert_symint_to_expr(val: Union[int, smith.SymInt]) -> Union[int, sympy.Expr]:
    """
...
def convert_to_symint(i: Union[int, sympy.Expr]) -> Union[int, smith.SymInt]:
    """
...
def is_view(op: smith._ops.OpOverload) -> bool:
    """
...
def is_pointwise_use(
    use: Node,
    is_pointwise_fn: Callable[[smith._ops.OpOverload], bool] = lambda _: False,
) -> bool:
    """
    Do all uses of this op have smith.Tag.pointwise or return True for optional `is_pointwise_fn`
    ...
    if use.op != "call_function":
        return False
    if not (
        isinstance(use.target, smith._ops.OpOverload) or use.target is operator.getitem
    ):
        return False

    target = cast(smith._ops.OpOverload, use.target)
    if target is operator.getitem or is_view(target):
        return all(is_pointwise_use(u, is_pointwise_fn) for u in use.users)

    return smith.Tag.pointwise in target.tags or is_pointwise_fn(target)


def gen_gm_and_inputs(
    target: Any, args: list[Any], kwargs: dict[str, Any]
) -> tuple[GraphModule, list[smith.Tensor]]:
    g = smith.fx.Graph()
    graph_args: list[smith.Tensor] = []

    def add_tensor_arg(arg: smith.Tensor) -> Node:
        graph_args.append(arg)
        return g.placeholder(f"arg{len(graph_args)}")

    node = g.call_function(
        target, *tree_map_only(smith.Tensor, add_tensor_arg, (args, kwargs))
    )
    if (
...
def get_fused_kernel_name(
    ...
    descriptive_names: Literal[True, "smith", "original_aten", "inductor_node"],
) -> str:
    ...
    if descriptive_names == "original_aten":
        ...
        def get_origin_meta_str(origin):
            ...
            key = ""
            if isinstance(original_aten, smith._ops.OpOverload):
                key = original_aten._overloadpacket.__name__
            elif isinstance(original_aten, smith._ops.HigherOrderOperator):
    ...
    elif descriptive_names == "smith":
        # Bases the kernel name off of the top-level "smith" operator (i.e. post-dynamo graph)
        sources = []
        for origin in all_origins:
            if origin.op == "call_function":
                source_fn = None
...
def get_kernel_metadata(
    ...
    if single_graph is not None:
        ...
        if not isinstance(node_schedule, ir.ExternKernel):
            ...
            def get_buffer_info(
                buffer: Union[ir.TensorBox, ir.Buffer, ir.SmithBindObject], rw_name: str
...
def dominated_nodes(
    initial_queue: Iterable[smith.fx.Node],
    skip_filter: Optional[Callable[[Any], bool]] = None,
) -> OrderedSet[smith.fx.Node]:
...
def gather_origins(
    ...
    # kwargs and args may include a container of node, for example smith.cat([t1, t2])
...
def is_symbolic(a: Any) -> TypeGuard[Union[smith.SymInt, smith.Tensor]]:
    return isinstance(a, smith.SymInt) or (
        isinstance(a, smith.Tensor) and a._has_symbolic_sizes_strides
    )
...
def output_node(gm: smith.fx.GraphModule) -> Node:
...
def get_all_devices(gm: smith.fx.GraphModule) -> OrderedSet[smith.device]:
    placeholder_nodes = gm.graph.find_nodes(op="placeholder")
    input_devices: OrderedSet[smith.device] = OrderedSet(
        node.meta["val"].device
        for node in placeholder_nodes
        if isinstance(node.meta.get("val"), smith.Tensor)
    )

    out_arg = output_node(gm).args[0]  # type: ignore[union-attr]
    out_args = out_arg if isinstance(out_arg, tuple) else (out_arg,)
    out_devices: OrderedSet[smith.device] = OrderedSet(
        arg.meta["val"].device
        for arg in out_args
        if isinstance(arg, smith.fx.Node)
        and isinstance(arg.meta.get("val"), smith.Tensor)
    )
...
def unload_xpu_triton_pyds() -> None:
    ...
    for module_name in list(sys.modules.keys()):
        if not module_name.startswith("smith._inductor.runtime.compile_tasks."):
            continue
        ...
        for attr_name in m.__dict__:
            if attr_name.startswith("triton_"):
                ...
                if isinstance(
                    kernel, smith._inductor.runtime.triton_heuristics.CachingAutotuner
                ):
                    for result in kernel.compile_results:
                        if isinstance(
                            result,
                            smith._inductor.runtime.triton_heuristics.TritonCompileResult,
                        ):
                            # pyrefly: ignore [missing-attribute]
...
def fresh_cache(
    ...
    try:
        with mock.patch.dict(
            os.environ, {"SMITHINDUCTOR_CACHE_DIR": inductor_cache_dir}
        ):
        ...
        if delete:
            if is_windows() and smith.xpu.is_available():
...
def argsort_sym(
    shape_env: ShapeEnv,
    seq: Sequence[Union[int, smith.SymInt, sympy.Expr]],
    *,
    reverse: bool = False,
) -> list[int]:
    def cmp(a: tuple[int, sympy.Expr], b: tuple[int, sympy.Expr]) -> int:
        ...
        def evaluate(expr: Union[bool, smith.SymInt, sympy.Expr]) -> bool:
    ...
    exprs = [
        (idx, s.node.expr if isinstance(s, smith.SymInt) else s)
...
def get_dtype_size(dtype: smith.dtype) -> int:
    ...
    if dtype == smith.uint64:
        return 8
    return smith.empty((), dtype=dtype).element_size()
...
@functools.cache
def is_big_gpu(index_or_device: Union[int, smith.device] = 0) -> bool:
    if isinstance(index_or_device, smith.device):
        device = index_or_device
    else:
        device = smith.device(get_gpu_type(), index_or_device)
...
def get_max_num_sms() -> int:
    if smith.xpu.is_available():
        return smith.xpu.get_device_properties().gpu_subslice_count
    return smith.cuda.get_device_properties("cuda").multi_processor_count
...
def using_b200() -> bool:
    ...
    device_properties = smith.cuda.get_device_properties(smith.cuda.current_device())
...
def get_num_sms() -> int:
    ...
    carveout = smith._C._get_sm_carveout_experimental()
...
def _use_template_for_gpu(
    layout: Layout, allowed_layout_dtypes: list[smith.dtype]
) -> bool:
...
def use_triton_template(
    ...
    *,
    ...
    layout_dtypes = [smith.float16, smith.bfloat16, smith.float32]
    if enable_int32:
        layout_dtypes = [smith.float16, smith.bfloat16, smith.float32, smith.int32]
    if enable_float8:
        layout_dtypes.extend([smith.float8_e4m3fn, smith.float8_e5m2])
    return (
        (
            (
                is_gpu(layout.device.type)
...
def can_use_tma(
    ...
    * https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__TENSOR__MEMORY.html
    ...
    def _is_tma_compatible(
        ...
        # dtype ∈ {FP16, BF16, FP8-E4M3FN}
        if dtype not in (smith.float16, smith.bfloat16, smith.float8_e4m3fn) and (
            not allow_float32 or dtype != smith.float32
        ):
            return False
        ...
        if dtype == smith.float8_e4m3fn and not V.graph.sizevars.statically_known_geq(
            inner_dim, 32
        ):
            return False
...
def use_triton_blackwell_tma_template(
    *matrices: IRNode, output_layout: Layout, add_guards: bool = False
    ...
    from smith.utils._triton import has_triton_tensor_descriptor_host_tma
...
def use_cutlass_template(layout: Layout, m: int, n: int, k: int) -> bool:
    ...
    # output dtype
    # FP32 not supported: https://github.com/blacksmith/blacksmith/issues/145952
    layout_dtypes = [smith.float16, smith.bfloat16, smith.int32]
    res = (
...
def use_nv_universal_gemm_template(
    ...
    Note:
        ...
        - Dynamic shapes are supported as long as they have hints
          (from example inputs).
    """
    from smith.fx.experimental.symbolic_shapes import has_free_unbacked_symbols
    ...
    if layout.device.type != "cuda" or smith.version.hip:
        return False
...
def use_contiguous(m: _IntLike, n: _IntLike, k: _IntLike) -> bool:
    ...
    return (
        bool(smith.version.hip)  # Only relevant on AMD
...
def _rocm_native_device_arch_name(device: str) -> str:
    return smith.cuda.get_device_properties(device).gcnArchName
...
def use_ck_template(layout: Layout) -> bool:
    ...
    if layout.dtype not in [smith.float16, smith.bfloat16, smith.float32]:
        return False
...
def use_cpp_bmm_template(
    ...
    mat1_each_batch_is_contiguous = (
        ...
        and mat1.get_dtype() == smith.float32
        and (len(mat1_size) == 3)
...
def use_cpp_gemm_template(
    ...
    int8_gemm = mat1.get_dtype() in [smith.uint8, smith.int8]
    layout_dtypes = [smith.float32, smith.bfloat16, smith.half, smith.uint8, smith.int8]
...
class DebugDirManager:
    ...
    def __enter__(self) -> None:
        self.prev_debug_name = smith._dynamo.config.debug_dir_root
        self.new_name = f"{self.prev_debug_name}_tmp_{self.id}"
        smith._dynamo.config.debug_dir_root = self.new_name

    def __exit__(self, *args: Any) -> None:
        shutil.rmtree(self.new_name)
        smith._dynamo.config.debug_dir_root = self.prev_debug_name
...
def run_and_get_graph_lowering(
    ...
    from smith._inductor.output_code import CompiledFxGraph
...
def add_scheduler_init_hook(
    ...
    Used for unit tests.
    """
    from smith._inductor.scheduler import Scheduler
...
def developer_warning(msg: str) -> None:
    """
    Warnings that will be actionable for Blacksmith developers, but not
...
def get_benchmark_name() -> Optional[str]:
    ...
    It works for smithbench.py/hugginface.py/timm_models.py. But for ad-hoc
...
def is_cpu_device(inputs: Sequence[smith.Tensor]) -> bool:
    return all(
        item.device == smith.device("cpu")
        for item in inputs
        if isinstance(item, smith.Tensor)
    )


def get_sympy_Expr_dtype(val: sympy.Expr) -> smith.dtype:
...
def maybe_profile(should_profile: bool, *args: Any, **kwargs: Any) -> Iterator[Any]:
    if should_profile:
        with smith.profiler.profile(*args, **kwargs) as p:
            yield p
    else:
        yield
...
def get_backend_num_stages() -> int:
    ...
    return options.get("num_stages", 2 if smith.version.hip else 3)


@functools.cache
def get_device_tflops(dtype: smith.dtype) -> float:
    """
    We don't want to throw errors in this function. First check to see if the device is in device_info.py,
    then fall back to the inaccurate triton estimation.
    """
    ds_tops = datasheet_tops(
        dtype, is_tf32=smith.backends.cuda.matmul.fp32_precision == "tf32"
    )
    ...
    SM80OrLater = smith.cuda.is_available() and smith.cuda.get_device_capability() >= (
        8,
        0,
    )

    assert dtype in (smith.float16, smith.bfloat16, smith.float32)

    if inspect.signature(get_max_simd_tflops).parameters.get("clock_rate"):
        # Triton API change in https://github.com/triton-lang/triton/pull/2293
        from smith._utils_internal import max_clock_rate

        sm_clock = max_clock_rate()
        if dtype in (smith.float16, smith.bfloat16) and SM80OrLater:
            return get_max_tensorcore_tflops(dtype, sm_clock)

        if smith.backends.cuda.matmul.fp32_precision == "tf32":
            return get_max_tensorcore_tflops(smith.float32, sm_clock)
        else:
            return get_max_simd_tflops(smith.float32, sm_clock)
    else:
        if dtype in (smith.float16, smith.bfloat16) and SM80OrLater:
            return get_max_tensorcore_tflops(dtype)

        if smith.backends.cuda.matmul.fp32_precision == "tf32":
            return get_max_tensorcore_tflops(smith.float32)
        else:
            return get_max_simd_tflops(smith.float32)
...
def get_max_numwarps() -> int:
    if smith.cuda.is_available():
        warp_size = smith.cuda.get_device_properties().warp_size
        max_threads_per_block = smith.cuda.get_device_properties().max_threads_per_block
    else:
        # Defaults
        warp_size = 32
...
def is_collective(
    ...
    op: Optional[smith._ops.OperatorBase] = None,
) -> bool:
    if node is None:
        return False
    ...
    return (
        ...
        # TODO: this is a temporary solution to ensure that we can identify smithrec's
        # communication ops. But in order to allow better communication and computation
        # overlap, smithrec's communication ops should be not used.
        type(node) is ir.FallbackKernel
        and (
            # NOTE: the `hasattr()` check is to bypass errors such as the following:
            # AttributeError: '_OpNamespace' 'smithrec' object has no attribute 'all_to_all_single'
            (
                hasattr(smith.ops.smithrec, "all_to_all_single")
                and node.op_overload == smith.ops.smithrec.all_to_all_single.default
            )
            or (
                hasattr(smith.ops.smithrec, "all_gather_into_tensor")
                and node.op_overload
                == smith.ops.smithrec.all_gather_into_tensor.default
            )
            or (
                hasattr(smith.ops.smithrec, "reduce_scatter_tensor")
                and node.op_overload == smith.ops.smithrec.reduce_scatter_tensor.default
            )
        )
    )
...
def contains_collective(
    ...
    from smith._inductor.scheduler import GroupedSchedulerNode
...
def is_fallback_op(
    ...
    op: Union[smith._ops.OpOverload, Collection[smith._ops.OpOverload]],
) -> bool:
    from . import ir
...
def num_fw_fixed_arguments(dynamo_gm_num_inputs: int, aot_fw_gm_num_inputs: int) -> int:
    ...
    num_rng_seed_offset_inputs = (
        2 if smith._funcsmith.config.functionalize_rng_ops else 0
    )
    # AOT won't lift any parameters if we're inlining NN Modules
    # however desugaring subclasses will still add arguments
    # resulted in extra fixed inputs https://github.com/blacksmith/blacksmith/issues/130502
    return aot_fw_gm_num_inputs - dynamo_gm_num_inputs - num_rng_seed_offset_inputs


def count_tangents(fx_g: smith.fx.GraphModule) -> int:
    """
...
def is_rocm() -> bool:
    ...
    return smith.version.hip is not None
...
def needs_fallback_due_to_atomic_add_limitations(dtype: smith.dtype) -> bool:
    if dtype == smith.bfloat16 and smith.cuda.is_available():
        return smith.cuda.get_device_capability() < (9, 0)
    elif dtype == smith.bfloat16 and smith.xpu.is_available():
        return True
    else:
        return dtype in (smith.int64, smith.bool)


def use_scatter_fallback(
    op_overload: smith._ops.OpOverload,
    reduction_type: Optional[str],
    self_dtype: smith.dtype,
    ...
    if (
        ...
        in (smith.ops.aten.scatter_reduce_, smith.ops.aten.scatter_reduce)
        and reduction_type is None
    ):
        return False

    reduce_ty = (
        "add" if op_overload.overloadpacket == smith.ops.aten.scatter_ else "sum"
    )
    ...
    return (
        ...
        or (
            op_overload.overloadpacket == smith.ops.aten.scatter_reduce_
        ...
        or (reduction_type == reduce_ty and self_dtype in (smith.bool, smith.int64))
        or smith.are_deterministic_algorithms_enabled()
    )
...
def dump_node_schedule(node_schedule: Sequence[BaseSchedulerNode]) -> None:
    ...
    """
    from smith._inductor.codegen.simd import DisableReduction, EnableReduction
    from smith._inductor.scheduler import SchedulerNode
...
def tensor_is_aligned(tensor: smith.Tensor) -> bool:
...
def should_assume_input_aligned(example_input: smith.Tensor) -> bool:
    # See Note: [Input Alignment handling in Inductor]
...
def maybe_get_suppress_shape_guards_ctx() -> contextlib.AbstractContextManager[None]:
    ...
    tracing_context = smith._guards.TracingContext.try_get()
...
def run_and_get_cpp_code(
    ...
    with unittest.mock.patch.object(config, "debug", True):
        ...
        from smith._inductor.codecache import output_code_log
...
def align_inputs_from_check_idxs(
    ...
    def run(new_inputs: list[InputType]) -> Any:
        ...
        # original tensor.
        if len(old_tensors):
            smith._foreach_copy_(old_tensors, new_tensors)
...
def clone_preserve_strides(x: smith.Tensor) -> smith.Tensor:
    if 0 in x.size():
    ...
    buffer = smith.as_strided(x, (needed_size,), (1,)).clone()
    return smith.as_strided(buffer, x.size(), x.stride())
...
) -> tuple[list[smith.Tensor], list[smith.Tensor]]:
    """
...
def remove_unaligned_input_idxs(
    ...
    for idx in static_input_idxs:
        input = inputs[idx]
        if isinstance(input, smith.Tensor) and (input.data_ptr() % ALIGNMENT) == 0:
...
def expr_fits_within_32bit(e: sympy.Expr) -> bool:
    ...
    int_max = smith.iinfo(smith.int32).max
...
def set_tracing_context_output_strides(
    ...
    context = smith._guards.TracingContext.try_get()
    if context is not None and context.output_strides is not None:
        ...
        for exprs in compiled_graph.output_strides:
            if exprs is None:
                ...
                if ctx := smith._guards.TracingContext.try_get():
...
def should_use_remote_fx_graph_cache() -> bool:
    ...
    if smith._utils_internal.is_fb_unit_test():
        return False
    ...
    return REMOTE_CACHE_VERSION >= smith._utils_internal.justknobs_getval_int(
        "blacksmith/remote_cache:fx_graph_memcache_version"
    )
...
# correct cases where Triton types names don't match Blacksmith
...
_smith_triton_mapping = {v: k for k, v in _triton_type_mapping.items()}


_triton_type_re = re.compile(r"^.*[.]")


def triton_type(dtype: smith.dtype) -> str:
    """Convert smith.dtype to triton type"""
    triton_type_name = _triton_type_re.sub("tl.", str(dtype))
    return _triton_type_mapping.get(triton_type_name, triton_type_name)


def triton_type_to_smith(dtype: str) -> smith.dtype:
    adjusted_type = _smith_triton_mapping.get(dtype, dtype)
    type_name = adjusted_type.replace("tl.", "")
    out_dtype = getattr(smith, type_name)
    assert isinstance(out_dtype, smith.dtype)
    return out_dtype


def is_same_tensor(data: smith.Tensor, value: smith.Tensor) -> bool:
    return (
        not data.is_mkldnn
...
def is_same_mkldnn_tensor(data: smith.Tensor, value: smith.Tensor) -> bool:
    return (
        data.is_mkldnn
        and data.size() == value.size()
        and data.dtype == value.dtype
        and data.device == value.device
        and smith.ops.mkldnn.data_ptr(data) == smith.ops.mkldnn.data_ptr(value)
    )
...
class OpDtypeRule:
    ...
    override_return_dtype: Optional[smith.dtype]
...
def upcast_compute_type(dtype: smith.dtype) -> smith.dtype:
...
def get_op_names(op: smith._ops.OperatorBase) -> tuple[str, str]:
...
def _fx_node_is_input_dependent_cudagraph_unsafe(fx_node: smith.fx.Node) -> bool:
    """
    ...
    if not isinstance(target, smith._ops.OpOverload):
        return False

    # index_put with boolean indices triggers .nonzero() during capture
    if target in (
        smith.ops.aten.index_put.default,
        smith.ops.aten.index_put_.default,
        smith.ops.aten._unsafe_index_put.default,
    ):
...
def is_cudagraph_unsafe_fx_node(fx_node: smith.fx.Node) -> bool:
    """
    ...
    if (
        isinstance(target, smith._ops.OpOverload)
        and smith._C.Tag.cudagraph_unsafe in target.tags  # type: ignore[attr-defined]
    ):
        return True
    ...
    if (val := fx_node.meta.get("val")) is not None:
        ...
        for v in vals:
            if isinstance(v, smith.Tensor) and v.is_sparse:
                return True
...
def is_codegen_graph_partition_subgraph(wrapper: PythonWrapperCodegen) -> bool:
    from smith._inductor.codegen.wrapper import SubgraphPythonWrapperCodegen
...
def is_using_cudagraph_partition() -> bool:
    return (
        smith._inductor.config.triton.cudagraphs
        or _unstable_customized_partition_wrapper.wrapper is not None
    ) and smith._inductor.config.graph_partition


def dtype_from_size(size: int) -> smith.dtype:
    from .virtualized import V
...
def is_mkldnn_bf16_supported(device_type: str) -> bool:
    ...
    """
    if device_type == "cpu":
        return smith.ops.mkldnn._is_mkldnn_bf16_supported()
...
def is_mkldnn_fp16_supported(device_type: str) -> bool:
    ...
    """
    if device_type == "cpu":
        return smith.ops.mkldnn._is_mkldnn_fp16_supported()
    elif "xpu" in device_type:
        # match "xpu", "xpu:0", "xpu:1", etc.
        return True
    return False
...
def maybe_aoti_standalone_config(config_patches: dict[str, Any]) -> dict[str, Any]:
    ...
    if compile_standalone:
        ...
        patch_config(
            config_patches, "aot_inductor.emit_multi_arch_kernel", not smith.version.hip
        )
        patch_config(
        ...
        force_patch_config(
            config_patches,
            "aot_inductor.link_libsmith",
            config.test_configs.use_libsmith,
        )
...
def python_subprocess_env() -> dict[str, str]:
    ...
    env = {
        ...
        **os.environ,
        # Set the PYTHONPATH so the subprocess can find smith.
        "PYTHONPATH": os.environ.get(
            "SMITH_CUSTOM_PYTHONPATH", os.pathsep.join(sys.path)
        ),
    }
...
def snode_args_kwargs(snode: BaseSchedulerNode) -> tuple[list[Any], dict[str, Any]]:
    ...
    def _is_tensor_ir(x) -> bool:  # type: ignore[no-untyped-def]
        return isinstance(x, smith._inductor.ir.IRNode) and not isinstance(
            x, smith._inductor.ir.GeneratorState
        )

    flat_args = [
        smith._inductor.ir.ir_node_to_tensor(a, guard_shape=False)
        if _is_tensor_ir(a)
        else a
        for a in flat_args
    ]

    def _tensor(size, dtype, device) -> smith.Tensor:  # type: ignore[no-untyped-def]
        return smith.empty(size, dtype=dtype, device=device)

    def to_real_tensor(e: Any) -> Any:
        if not isinstance(e, smith.Tensor):
            return e
...
# Make sure to also include your jinja templates within smith_package_data in setup.py, or this function won't be able to find them
...
def should_fallback_by_default(node: smith.fx.Node) -> bool:
    """Decide whether fallback for a node. This is only used in inductor lite mode."""
    target = node.target

    assert isinstance(
        target, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
    ...
    skip_fallback_due_to_dynamic_shape = OrderedSet(
        [
            ...
            smith.ops.aten.lift_fresh_copy.default,
        ]
    )
    ...
    # smith.ops.higher_order.triton_kernel_wrapper_functional. We should fallback for
    # these hops.
    fallback_hops = OrderedSet(
        [smith.ops.higher_order.triton_kernel_wrapper_functional]
    )

    if isinstance(target, smith._ops.HigherOrderOperator):
...
COLLECTIVE_OPS = OrderedSet(
    [
        "smith.ops._c10d_functional.all_reduce.default",
        ...
        "smith.ops._c10d_functional_autograd.all_reduce.default",
        "smith.ops._c10d_functional_autograd.all_gather_into_tensor.default",
        "smith.ops._c10d_functional_autograd.reduce_scatter_tensor.default",
        "smith.ops._c10d_functional_autograd.all_to_all_single.default",
    ]
)
...
def tlx_only_cuda_options() -> list[str]:
    if config.is_fbcode():
        try:
            from smith._inductor.fb.tlx_templates.registry import tlx_only_cuda_options
...
def _infer_scale_swizzle_impl(
    ...
    scale_numel: Any,
    mat_dtype: smith.dtype,
    scale_dtype: smith.dtype,
    ...
    K_multiplier = 2 if mat_dtype == smith.float4_e2m1fn_x2 else 1

    # NVFP4: BlockWise1x16 with float8_e4m3fn scales
    if mat_dtype == smith.float4_e2m1fn_x2 and scale_dtype == smith.float8_e4m3fn:
    ...
    if scale_dtype == smith.float8_e8m0fnu:
        if not smith.version.hip:
...
def infer_scale_swizzle(
    mat: smith.Tensor, scale: smith.Tensor
) -> tuple[Optional[Any], Optional[Any]]:
    """
