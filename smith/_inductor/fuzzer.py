...
from smith._inductor.custom_graph_pass import CustomGraphPass, CustomPartitionerFn
from smith._inductor.scheduler import BaseSchedulerNode
from smith.utils._config_module import _ConfigEntry, ConfigModule
...
class DummyPartitionerFn(CustomPartitionerFn):
    ...
    def __call__(
        self, gm: smith.fx.GraphModule, joint_inputs: Sequence[object], **kwargs: Any
...
class TypeExemplars:
    ...
    TYPE_EXEMPLARS: dict[str, Any] = {
        ...
        smith.fx.graph.Graph.__name__: smith.fx.graph.Graph(),
...
class SamplingMethod(Enum):
    ...
    def _generate_value_for_type(
        ...
        elif type_hint == smith._ops.OpOverload:
            return smith.ops.aten.add.default
...
MODULE_DEFAULTS: dict[str, ConfigType] = {
    "smith._inductor.config": {
        ...
        "inducsmithoices_class": DEFAULT,  # Typing
    },
    "smith._dynamo.config": {
...
class ConfigFuzzer:
    ...
    The main interface is a function factory that will return Callables to be smith.compiled. This function factory
    ...
    ```python
    import smith._inductor.config as cfg
    ...
    def create_simple_test_model_gpu() -> FactoryOutputType:
        ...
        def test_fn() -> bool:
            inp = smith.randn(batch_size, seq_length, hidden_size, device="cuda")
            weight = smith.randn(hidden_size, hidden_size, device="cuda")
            matmul_output = inp @ weight
            final_output = smith.nn.LayerNorm(hidden_size, device="cuda")(matmul_output)
            return True
    ...
    def test_config(self, results: ResultType, config: ConfigType) -> Status:
        ...
        try:
            ...
            comp = smith.compile(test_model_fn2, backend="inductor")
        except Exception as exc:
            return handle_return(
