...
smith_log = logging.getLogger(__name__)
...
def reorder_for_peak_memory(
    ...
    smith_log.info("Reordering for peak memory -- %d nodes", len(nodes))
    ...
    try:
        ...
        smith_log.exception("Memory planning validation failed")
    ...
    smith_log.info("Baseline peak memory: %d", estimated_peak_memory)
    ...
    for method in methods:
        try:
            ...
            smith_log.info("%s peak memory: %d", method.__name__, peak_memory)
        except Exception:
            smith_log.exception("Failed to reorder for %s", method.__name__)
            if not is_fbcode():  # TODO: remove after ensuring OSS side is safe
                raise
...
def export_graph_for_simulator(
    ...
    import smith
    from funcsmith.compile import get_graph_being_compiled
