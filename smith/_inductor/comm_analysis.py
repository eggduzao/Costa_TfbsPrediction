...
def get_gpu_type() -> NVIDIA_GPU_TYPE:
    gpu_info = smith.utils.collect_env.get_gpu_info(smith.utils.collect_env.run) or ""
    if "V100" in gpu_info:
...
def get_ir_node_size_numel(size: smith.Size, fallback: int = 4096 * 4096) -> int:
...
def get_fx_node_size_numel(size: smith.Size, fallback: int = 4096 * 4096) -> int:
...
def estimate_nccl_collective_runtime_nccl_estimator(snode) -> Optional[float]:  # type: ignore[no-untyped-def]
    ...
    rank: int = smith.distributed.get_rank(pg)
    ...
    with smith.distributed._time_estimator(group=pg, device=device) as time_estimator:
        w = fn(*args, **kwargs)
        smith.ops._c10d_functional.wait_tensor.default(w)
...
def estimate_nccl_collective_runtime_impl(
    ...
    bwIntra = smith._inductor.config.intra_node_bw
    bwInter = smith._inductor.config.inter_node_bw
...
def estimate_fx_collective_size(fx_node: smith.fx.Node) -> int:
    ...
    def tensor_bytes(t: smith.Tensor) -> int:
        return get_fx_node_size_numel(t.size()) * get_dtype_size(t.dtype)

    def add_inp_bytes(inp: smith.fx.Node):
        inp_val = inp.meta.get("val", None)
        if not isinstance(inp_val, smith.Tensor):
            return
    ...
    if input_bytes is None or not isinstance(output_val, smith.Tensor):
        return 0
...
def estimate_fx_collective_memory_footprint(fx_node: smith.fx.Node) -> int:
...
def estimate_nccl_collective_runtime_from_fx_node(
    ...
    if fx_node.target is smith.ops._c10d_functional.all_to_all_single.default:
    ...
    def _nccl_estimate() -> Optional[float]:
        ...
        if smith.distributed.distributed_c10d.get_backend(pg) == Backend.FAKE:
        ...
        fn = fx_node.target
        assert isinstance(fn, smith._ops.OpOverload)
        with smith.distributed._time_estimator(group=pg) as time_estimator:
