...
from smith._inductor.freezing_utils import maybe_set_is_frozen_param
...
_dont_constant_fold: list[smith.fx.node.Target] = []


def add_dont_constant_fold(op: smith.fx.node.Target) -> None:
...
def replace_node_with_constant(
    ...
    node: smith.fx.Node,
    constant: Optional[smith.Tensor] = None,
...
def is_const_source(
    node: smith.fx.Node, lifted_constant_names: Optional[list[str]]
) -> bool:
    return node.op == "get_attr" or node.name in (lifted_constant_names or ())


class ConstantFolder(smith.fx.Interpreter):
    def __init__(
        self,
        gm: smith.fx.GraphModule,
        ...
        super().__init__(gm)
        self.node_replacements: dict[smith.fx.Node, Any] = {}
        self.replaced_uses: dict[smith.fx.Node, int] = collections.Counter()
    ...
    def _deduce_value(self, node: smith.fx.Node) -> Any:
    ...
    def is_impure(self, node: smith.fx.node.Node) -> bool:
        def is_woq_int8_pattern(node: smith.fx.node.Node) -> bool:
            return (
                node.target is smith.ops.prims.convert_element_type.default  # type: ignore[return-value]
                and isinstance(node.args[0], smith.fx.Node)
                and "val" in node.args[0].meta
                and node.args[0].meta["val"].dtype == smith.int8  # type: ignore[union-attr]
                and node.args[1] == smith.bfloat16
            )

        if (
            is_woq_int8_pattern(node)
            or (
                node.target is smith.ops.aten.permute.default
        ...
        quant_registered = (
            getattr(smith.ops.quantized_decomposed, "dequantize_per_channel", None)
            is not None
        )
        if quant_registered and node.target in [
            smith.ops.quantized_decomposed.dequantize_per_channel.default,
            smith.ops.quantized_decomposed.dequantize_per_tensor.default,
            smith.ops.quantized_decomposed.dequantize_per_tensor.tensor,
            smith.ops.quantized_decomposed.convert_element_type.no_fuse,
        ]:
    ...
    def node_to_last_non_output_use(self) -> dict[smith.fx.Node, list[smith.fx.Node]]:
        last_non_output_use = collections.defaultdict(list)
        seen_uses = OrderedSet[smith.fx.Node]()
        ...
        for node in reversed(self.module.graph.nodes):  # type: ignore[arg-type, union-attr]
            ...
            def add_use(inp: smith.fx.Node) -> None:
                if inp in seen_uses:
                    return
            ...
            pytree.tree_map_only_(smith.fx.Node, add_use, (node.args, node.kwargs))
    ...
    def run_node(self, node: smith.fx.Node) -> Any:
        if node.target == "output":
            ...
            def set_env(arg: smith.fx.Node) -> None:
                self.env[arg] = self.unknown_value

            # In-place is fine since we don't mutate
            pytree.tree_map_only_(smith.fx.Node, set_env, node.args)
            return super().run_node(node)
        ...
        if (
            ...
            and not any(isinstance(e, smith.Tensor) for e in flattened_inputs)
        ):
            return self.unknown_value
        ...
        if isinstance(out, smith._C.ScriptObject):
            return out
        ...
        if not is_const_source(node, self.lifted_constant_names) and (
            isinstance(out, smith.Tensor) or out == self.deferred_value
        ):
    ...
    def insertable_tensor_check(self, tensor: smith.Tensor) -> bool:
        return True
...
def constant_fold(
    gm: smith.fx.GraphModule,
    constraint_fn: Optional[Callable[[smith.fx.Node], bool]] = None,
) -> None:
...
def run_and_get_constant_graph(
    ...
    def untag(node: smith.fx.Node) -> bool:
        used_to_fold = False
        for u in node.users:
    ...
    node_remapping: dict[smith.fx.Node, smith.fx.Node] = {}
    output_nodes = []
    for node in gm.graph.nodes:
