...
from smith.utils.flop_counter import flop_registry

from .virtualized import V


# Check the pattern: (nn.module, F.function/smith.Tensor.method) matched.
# Works for length 2 patterns with 1 module and 1 function/method.
def matches_module_function_pattern(
    pattern: tuple[type[smith.nn.modules.Module], Callable[..., Any]],
    node: smith.fx.node.Node,
    modules: dict[str, smith.nn.modules.Module],
) -> bool:
    if len(node.args) == 0:
        return False
    if not isinstance(node.args[0], smith.fx.Node) or not isinstance(
        node, smith.fx.Node
    ):
        return False
...
class FakeTensorUpdater:
    ...
    def __init__(self, graph: smith.fx.Graph) -> None:
    ...
    def hash_node(self, node: smith.fx.Node):
    ...
    def incremental_update(self):
        ...
        def is_fake_tensor_same(new, old, *, node):
            ...
            if new is None:
                return old is None
            if not isinstance(new, smith.Tensor):
                assert isinstance(new, (smith.SymInt, smith.SymBool, smith.SymFloat)), (
            ...
            if not is_intlist_same(new.shape, old.shape) or new.layout != old.layout:
                return False
            if new.layout == smith.strided and (
            ...
            def any_user_may_alias(node):
                if not isinstance(node.meta["val"], smith.Tensor):
                ...
                for user in node.users:
                    if not (
                        isinstance(
                            user.target,
                            (smith._ops.OpOverload, smith._ops.HigherOrderOperator),
                        )
                        or user.target
                        is smith._inductor.fx_passes.reinplace._generalized_scatter
                    ):
                        return True
                    if isinstance(user.target, smith._ops.HigherOrderOperator):
                    ...
                    # (that is, we ban things like smith.ops.aten.reshape calls in the graph),
                    ...
                    if not isinstance(new_fake_tensor, smith.Tensor):
                        # analysis too complicated on lists, can support in the future
                        return True
...
def get_storage(t: smith.Tensor) -> int:
    return t.untyped_storage()._cdata


def get_node_storage(node: smith.fx.Node) -> Optional[int]:
    if "val" not in node.meta:
        return None
    if not isinstance(node.meta["val"], smith.Tensor):
        return None
    if not smith._C._has_storage(node.meta["val"]):
        return None
    return get_storage(node.meta["val"])


def get_fake(x):
    if isinstance(x, smith.fx.Node):
...
def get_fake_args_kwargs(x: smith.fx.Node) -> tuple[bool, tuple[Any], dict[str, Any]]:
    """
    First value returns a boolean if any of the input nodes don't have a faketensor.
    """
    args, kwargs = tree_map(get_fake, (x.args, x.kwargs))
    if any(
        isinstance(a, smith.fx.Node) for a in pytree.arg_tree_leaves(*args, **kwargs)
    ):
        return False, args, kwargs
    return True, args, kwargs


def is_node_realized(node: smith.fx.Node) -> bool:
    ...
    """
    from smith._inductor.lowering import fallbacks, needs_realized_inputs

    def is_buffer(node: smith.fx.Node) -> bool:
        if node.op == "call_function" and node.target is operator.getitem:
            # For nodes with multiple outputs, we get the fx graph:
            #     foo = smith.ops.aten.foo(...)
            #     getitem = foo[0]
            #     getitem_1 = foo[1]
    ...
    def realizes_inputs(node: smith.fx.Node) -> bool:
...
def count_flops_fx(node: smith.fx.Node) -> Optional[int]:
    ...
    with FakeTensorMode(allow_non_fake_inputs=True):
        ...
        if success:
            with smith.utils.flop_counter.FlopCounterMode(
                display=False
...
def countable_fx(node: smith.fx.Node) -> bool:
    """
