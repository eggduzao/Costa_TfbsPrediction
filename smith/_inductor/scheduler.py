...
from smith._dynamo.utils import counters, dynamo_timed
from smith._inductor.codecache import LambdaFuture, PyCodeCache
from smith._inductor.ir import TritonTemplateCallerBase
from smith._inductor.metrics import get_metric_table, is_metric_table_enabled
from smith.fx.experimental.symbolic_shapes import free_symbols
...
loop_ordering_log = smith._logging.getArtifactLogger(__name__, "loop_ordering")
compute_dependencies_log = smith._logging.getArtifactLogger(
    __name__, "compute_dependencies"
)
cudagraphs_log = smith._logging.getArtifactLogger(__name__, "cudagraphs")
...
class MixOrderReduction:
    ...
    def is_contiguous_load(cls, buf: str, parent_node: BaseSchedulerNode) -> bool:
        from smith._inductor.loop_body import MemoryUsageType
...
class BaseSchedulerNode:
    ...
    group: tuple[smith.device, tuple[tuple[sympy.Expr, ...], ...]]
...
{name}.outputs = [
    ...
    def debug_str_short(self) -> str:
        ...
        data_str = ""
        if isinstance(maybe_data, smith._inductor.ir.Pointwise):
            data_str = ", " + maybe_data.str_helper(
                [maybe_data.get_size()], shorten=False, multiline=False
            )
        elif isinstance(maybe_data, smith._inductor.ir.Reduction):
            data_str = ", " + maybe_data.str_helper(
    ...
    def decide_inplace_update(self) -> None:
        ...
        if not (
            ...
            and (
                not isinstance(V.kernel, smith._inductor.codegen.simd.SIMDKernel)
            ...
            for read in self.read_writes.reads:
                ...
                if (
                    ...
                    if (
                        ...
                        if isinstance(
                            V.kernel, smith._inductor.codegen.simd.SIMDKernel
                        ):
    ...
    def get_read_write_buffer_accesses(
        ...
        if (
            ...
            is smith._prims.rng_prims.graphsafe_run_with_rng_state
        ):
            return {}
        ...
        for buf_name in reads | writes:
            ...
            buf: Union[ir.Buffer, ir.TensorBox, ir.SmithBindObject]
            ...
            def get_buf_bytes(
                buf: Optional[Union[ir.Buffer, ir.TensorBox, ir.SmithBindObject]],
            ) -> int:
                if not buf:
                    return 0

                if isinstance(buf, ir.SmithBindObject):
    ...
    def estimate_flops(self) -> int | None:
        ...
        if isinstance(flops, smith.SymInt):
...
@functools.cache
def get_estimate_runtime_cache() -> smith._inductor.codecache.LocalCache:
    return smith._inductor.codecache.LocalCache()
...
def _get_mm_like_fn(snode: BaseSchedulerNode) -> Optional[Callable[[Any], Any]]:
    if not isinstance(snode, ExternKernelSchedulerNode):
        return None
    mms_fns = {
        "extern_kernels.mm": smith.ops.aten.mm,
        "extern_kernels.bmm": smith.ops.aten.bmm,
        "extern_kernels.addmm": smith.ops.aten.addmm,
    }
    python_kernel_name = getattr(snode.node, "python_kernel_name", "")
    ...
    def debug_str_extra(self) -> str:
        ...
        for dep in self.read_writes.reads_and_writes():
            if not isinstance(dep, WeakDep):
                ...
                if not isinstance(buf, ir.SmithBindObject):
...
class ForeachKernelSchedulerNode(FusedSchedulerNode):
    ...
    def __init__(
        ...
        self.origins = OrderedSet[smith.fx.Node]()
    ...
    def _default_group_nodes_for_combo_kernels(
        ...
        for nodes in sorted_nodes:
            ...
            device_groups: dict[Optional[smith.device], list[BaseSchedulerNode]] = (
                defaultdict(list)
            )
            for node in nodes:
...
class Scheduler:
    ...
    def _init(self, nodes: list[ir.Operation]) -> None:
        ...
        self.backends: dict[smith.device, BaseScheduling] = {}
        ...
        self.available_buffer_names = OrderedSet(
            [
                ...
                *V.graph.smithbind_constants.keys(),
            ]
        )
        ...
        self.default_device_context: Optional[smith.device] = None
        ...
        from smith._inductor.debug import log_ir_post_fusion, log_ir_pre_fusion
        ...
        if (
            ...
            smith._inductor.select_algorithm.PrecompileThreadPool.shutdown_instance()
        ...
        self.origin_to_index: dict[smith.fx.Node, int] = {}
    ...
    @property
    def current_device(self) -> Optional[smith.device]:
        return V.graph.current_device

    @current_device.setter
    def current_device(self, device: Optional[smith.device]) -> None:
    ...
    def insert_memory_check_nodes(self) -> None:
        ...
        if not smith._inductor.config.reorder_for_peak_memory:
            assign_memory_planning_info_for_scheduler_buffers(
                self.nodes, self.name_to_buf
            )
        ...
        from smith._inductor.runtime.debug_utils import register_check_mem_op
        ...
        def construct_mem_check_node(
            ...
            node = ir.MemoryCheckKernel(
                layout=NoneLayout(device=smith.device("cpu")),
                kernel=smith.ops._inductor_debug.check_memory_step.default,
                tensor_args=[],
    ...
    def benchmark_codegened_module(
        self, module: ModuleType, device: smith.device
    ) -> tuple[float, str]:
        """
        Benchmark fused list of nodes and return the execution time
        in milliseconds on randomly generated inputs.
    ...
    def finalize_multi_template_buffers(self) -> None:
        ...
        for i, node in enumerate(self.nodes):
            if isinstance(node, SchedulerNode) and isinstance(
                ...
                if not config.test_configs.force_extern_kernel_in_multi_template:
                    ...
                    min_node_unfused = next(
                        (
                            ...
                            if isinstance(
                                timing,
                                smith._inductor.select_algorithm.ExternKernelCaller,
                            )
                        ),
                    )
                ...
                if isinstance(
                    min_node_unfused,
                    smith._inductor.ir.TritonTemplateCallerBase,
                ):
    ...
    def speedup_by_fusion(
        ...
        async_compile = smith._inductor.async_compile.AsyncCompile()
        ...
        if is_multi_template and any(
            ...
            for hint_override in config.multi_kernel_hints:
                ...
                for choice, _ in sorted(choice_timings.items(), key=lambda x: x[1]):
                    if not isinstance(
                        choice, smith._inductor.select_algorithm.TritonTemplateCaller
                    ):
                        continue
            ...
            def benchmark_when_ready() -> bool:
                from smith._inductor.runtime.triton_heuristics import (
    ...
    def shared_data_after_inverting_indexing(
        ...
        from smith._inductor.invert_expr_analysis import generate_inverse_formula
    ...
    def shared_data_after_reordering_loop(
        ...
        # and https://github.com/blacksmith/blacksmith/issues/165579
    ...
    def free_buffers(self) -> None:
        ...
        for name in sorted(
            ...
            if name in self.name_to_buf:
                ...
                if isinstance(inp, ir.SmithBindObject):
    ...
    def create_backend(self, device: smith.device) -> BaseScheduling:
        ...
        if not has_triton():
            if (
                device.type == "cuda"
                and (device_props := smith.cuda.get_device_properties(device)).major < 7
            ):
    ...
    def get_backend(self, device: Optional[smith.device]) -> BaseScheduling:
        assert device is not None
    ...
    def enter_context(self, node: BaseSchedulerNode) -> None:
        def get_order(n: smith.fx.Node) -> int:
    ...
    def should_partition(self, node: BaseSchedulerNode) -> Optional[str]:
        ...
        ir_node = node.node
        if isinstance(ir_node, smith._inductor.ir.FallbackKernel) and (
        ...
        if (
            not smith._inductor.config.triton.cudagraphs
    ...
    def _get_cudagraph_unsafe_unbacked_symints(self) -> OrderedSet[sympy.Symbol]:
        ...
        for node in self.nodes:
            ...
            if not isinstance(ir_node, smith._inductor.ir.FallbackKernel):
                continue
    ...
    def get_name_to_nodes(
        self,
    ) -> dict[str, Union[ir.IRNode, ir.SmithBindObject, sympy.Expr]]:
        """
        Return a mapping from name strings to the corresponding graph inputs or
        base scheduler node outputs.
        """
        name_to_node: dict[str, Union[ir.IRNode, ir.SmithBindObject, sympy.Expr]] = {}
    ...
    def get_graph_partition_symbol_inputs(
        ...
        input_nodes: dict[str, Union[ir.IRNode, ir.SmithBindObject, sympy.Expr]],
    ) -> OrderedSet[sympy.Symbol]:
        """
        ...
        def get_input_node_symbols(
            node: Union[ir.IRNode, sympy.Expr, ir.SmithBindObject],
        ) -> OrderedSet[sympy.Symbol]:
            """
            Gets symbols used in input node shapes, strides, and offsets.
            """
            if isinstance(node, ir.SmithBindObject):
                # SmithBindObject does not involve dynamic shapes yet
                return OrderedSet()
    ...
    def update_graph_partition_default_device(
        ...
        def get_cudagraph_partition_device(partition: PartitionType) -> smith.device:
        ...
        def all_on_target_device(
            partition: PartitionType, target_device: smith.device
        ) -> bool:
            for node in partition:
                device = node.get_device()
    ...
    def _codegen(self, nodes: list[BaseSchedulerNode]) -> None:
        if config.check_stack_no_cycles_TESTING_ONLY:
            import smith._dynamo.convert_frame
            ...
            for frame in reversed(stack):
                ...
                if (
                    ...
                    and frame.filename == smith._dynamo.convert_frame.__file__
                ):
                    break
...
class BaseScheduling:  # noqa: docstring_linter
    ...
    def get_backend_features(self, device: smith.device) -> OrderedSet[BackendFeature]:
    ...
    def codegen_comment(
        ...
        if kernel_name:
            from smith._inductor.debug import set_kernel_post_grad_provenance_tracing
