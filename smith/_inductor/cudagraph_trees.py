...
from smith._dynamo.utils import counters, dynamo_timed, preserve_rng_state
...
if TYPE_CHECKING:
    ...
    from smith._guards import CompileId
    from smith._inductor.utils import InputType
    from smith.cuda import _POOL_HANDLE
...
if smith.backends.cuda.is_built():
...
log = smith._logging.getArtifactLogger(__name__, "cudagraphs")
...
def disable_conv_cache_emptying() -> Generator[None, None, None]:
    prev = smith._C._cuda_get_conv_benchmark_empty_cache()
    smith._C._cudnn_set_conv_benchmark_empty_cache(False)
    try:
        yield
    finally:
        smith._C._cudnn_set_conv_benchmark_empty_cache(prev)


@contextlib.contextmanager
def enable_history_recording() -> Generator[None, None, None]:
    "Turns on history recording in the CUDA Caching Allocator"
    enabled = smith._C._cuda_isHistoryEnabled()
    try:
        if not enabled:
...
class TreeManagerContainer:
    ...
    def __init__(self, device_index: int) -> None:
        ...
        self.graph: Optional[smith.cuda.CUDAGraph] = None
...
smith._C._stash_obj_in_tls("tree_manager_containers", local.tree_manager_containers)
smith._C._stash_obj_in_tls("tree_manager_locks", local.tree_manager_locks)
...
def reset_cudagraph_trees() -> None:
    ...
    if not smith._C._is_key_in_tls("tree_manager_containers"):
...
def get_obj(local: Any, attr_name: str) -> Any:
    if hasattr(local, attr_name):
        ...
        assert smith._C._is_key_in_tls(attr_name)
        return smith._C._get_obj_in_tls(attr_name)
...
class StorageWeakRefWrapper:
    ...
    def expired(self) -> bool:
        ...
        stor_count = smith._C._storage_Use_Count(self.ref.cdata)
...
def _use_cuda_memory_pool_manager(
    device: int, mem_pool: tuple[int, int], stream: smith.cuda.Stream
...
class CUDAWarmupNode:
    ...
    def __init__(
        ...
        existing_cuda_graph: Optional[smith.cuda.CUDAGraph],
        device_index: int,
    ...
    def run(self, new_inputs: Any) -> OutputType:
        ...
        with (
            smith.cuda.device(self.device_index),
    ...
    def _is_cuda_graph_recorded_tensor(self, t: smith.Tensor) -> bool:
...
class CUDAGraphNode:
    ...
    def __init__(
        ...
        self.rerecord_if_static_inputs_change = (
            smith._dynamo.config.inline_inbuilt_nn_modules
            or smith._inductor.config.triton.cudagraph_support_input_mutation
        )
        ...
        self.cudagraph_managed_idxs: list[int] = [
            ...
            if isinstance(t, smith.Tensor) and self._is_cuda_graph_recorded_tensor(t)
        ]
        ...
        def maybe_get_static_data_ptr(
            ...
            inp = inputs[idx]
            if isinstance(inp, smith.Tensor) and idx in static_input_idxs:
                return inp.data_ptr()
            return None
        ...
        self.expanded_dims: list[list[int]] = [
            get_expanded_dims(x)
            if isinstance(x, smith.Tensor) and idx not in self.static_input_idxs
            else []
        ...
        rng_states = [inp for inp in inputs if isinstance(inp, smith.Generator)]
        ...
        self.graph: Optional[smith.cuda.CUDAGraph] = smith.cuda.CUDAGraph()

        # TODO: register_generator_state should potentially take explicit device
        with smith.cuda.device(self.device):
    ...
    def _copy_inputs_and_remove_from_src(
        ...
        for idx in self.non_static_input_idx:
            if not isinstance(srcs[idx], smith.Tensor):
                continue
        ...
        if dst_tensors:
            smith._foreach_copy_(dst_tensors, src_tensors)
    ...
    def check_static_inputs_are_stable(self, new_inputs: list[InputType]) -> None:
        ...
        if (
            ...
            and not smith._C._tensors_data_ptrs_at_indices_equal(
            ...
            smith._check(False, lambda: error_msg)
        ...
        for i, (storage_info, metadata) in enumerate(
            ...
            if isinstance(storage, UntypedStorage) or storage is None:
                ...
                out = self._reconstruct_from_tensor_metadata(
                    metadata, cast(smith.Tensor, outputs[storage]).untyped_storage()
                )
    ...
    def prepare_alias_info_for_tensor_construction(
        ...
        if isinstance(out_alias_info, AliasesPriorGraphOutput):
            ...
            return smith.UntypedStorage._new_with_weak_ptr(ref())
    ...
    def _record(self, model: ModelType, inputs: list[InputType]) -> OutputType:
        ...
        def static_input_iter() -> Generator[smith.Tensor, None, None]:
        ...
        if config.triton.slow_path_cudagraph_asserts:
            ...
            memory += [
                ...
                if isinstance(elem, smith.Tensor)
    ...
    def _add_first_outputs(
        ...
        for i, o in enumerate(outputs):
            if o is None or not isinstance(o, smith.Tensor):
        ...
        for out, static_output_tensor in zip(outputs, self.static_output_tensors):
            if not isinstance(out, smith.Tensor) or static_output_tensor is not None:
        ...
        self.checkpointed_caching_state = smith._C._cuda_getCheckpointState(
    ...
    def _initialize_cached_tensors(self) -> None:
        ...
        for i, (storage_info, metadata, make_cached) in enumerate(
            ...
            smith._C._add_cached_tensor(out)
    ...
    def remove_node_cached_tensors(self) -> None:
        for t in self.cached_tensor_outputs:
            if t is not None:
                smith._C._remove_cached_tensor(t)
    ...
    def _tensor_metadata(
        x: smith.Tensor, ignore_storage_offset: bool = True
    ) -> dict[str, Any]:
    ...
    def _reconstruct_from_tensor_metadata(
        ...
        return smith._C._construct_CUDA_Tensor_From_Storage_And_Metadata(metadata, s)  # type: ignore[arg-type]

    def create_storage(self, metadata: dict[str, Any]) -> smith.types.Storage:
        return smith._C._construct_storage_from_data_pointer(
    ...
    def _allocate_and_copy_recording_inputs(
        ...
        smith.cuda.synchronize()
        self.stream.wait_stream(smith.cuda.current_stream())
        ...
        with (
            ...
            for i, inp in enumerate(inputs):
                if not isinstance(inp, smith.Tensor):
                    assert isinstance(inp, (int, smith.Generator))
    ...
    def check_invariants(
        ...
        if not smith._C._tensors_data_ptrs_at_indices_equal(
...
def check_memory_pool(
    ...
    if smith._C._cuda_checkPoolLiveAllocations(device, pool_id, unique_storages):
        return
    ...
    if len(allocated_not_in_live_storages) != 0:
        ...
        history_hint = ""
        if not config.triton.cudagraph_trees_history_recording:
            history_hint = "- Set smith._inductor.config.triton.cudagraph_trees_history_recording = True for allocation origins\n"

        objgraph_hint = (
            (
                "- Objgraph backrefs disabled; set smith._inductor.config.triton.cudagraph_trees_objgraph = True "
        ...
        if config.triton.cudagraph_trees_objgraph:
            ...
            tensors = objgraph.by_type("smith.Tensor")
...
class CUDAGraphTreeManager:
    ...
    Currently this is hardcoded `GenerationTracker.generation` tracked in smith dynamo.
    ...
    def __init__(self, device_index: int) -> None:
        ...
        with smith.cuda.device(device_index):
            smith.cuda.synchronize()
            self.stream = smith.cuda.Stream()
            self.stream.wait_stream(smith.cuda.current_stream())

            # Keeps Memory Pool Alive
            self.graph: Optional[smith.cuda.CUDAGraph] = smith.cuda.CUDAGraph()
            self.cuda_graphs_thread_pool = smith.cuda.graph_pool_handle()
        ...
        # current generation of cudagraph invocations. when smith.compile is run
        ...
        # If multiple smith.compile wrapped forwards are executed with their backwards pending,
        # we should not disregard the outputs from a prior smith.compile since the entire training
        ...
        # invocation. Triggering a backward pass typically doesn't lead to another smith.compile
        ...
        self.disable_invalidate_aliases = (
            False
            if not smith._environment.is_fbcode()
            else smith._utils_internal.justknobs_check(
                "blacksmith/inductor:disable_cudagraph_alias_invalidation"
            )
        )
    ...
    def exceed_rerecord_limit(
        ...
        return (
            ...
            > smith._inductor.config.triton.cudagraph_unexpected_rerecord_limit
        )
            ...
            if unexpected_rerecord:
                ...
                if self.exceed_rerecord_limit(curr_node_id, function_id):
                    ...
                    log_cudagraph_skip_and_bump_counter(
                        ...
                        f"(={smith._inductor.config.triton.cudagraph_unexpected_rerecord_limit}) "
    ...
    def record_function(
        ...
        assert not isinstance(self.current_node, CUDAWarmupNode)
        with smith._dynamo.callback_handler.install_callbacks(
    ...
    def add_function(
        ...
        self.ids_to_funcs[id] = WrappedFunction(
            ...
            id,
            tuple(t for t in constants if isinstance(t, smith.Tensor) and t.is_cuda),
            placeholders,
            mutated_input_idxs,
        )
    ...
    def can_start_new_generation(self) -> bool:
        if not self.in_new_smith_compile_invocation():
            return False
    ...
    def in_new_smith_compile_invocation(self) -> bool:
    ...
    def check_warn_on_unable_to_start_executing(self, function_id: FunctionID) -> None:
        ...
        if (
            ...
            or not self.in_new_smith_compile_invocation()
        ):
            return
        ...
        warnings.warn(
            ...
            "Consider running with smith.no_grad() or using smith.compiler.cudagraph_mark_step_begin() "
    ...
    def format_dealloc_msg(stack_trace: Optional[str]) -> str:
        ...
        return (
            ...
            "To prevent overwriting, clone the tensor outside of smith.compile() "
            "or call smith.compiler.cudagraph_mark_step_begin() before each model invocation."
        )
    ...
    def dealloc_current_path_weakrefs(self) -> None:
        ...
        for storage_ref in self.current_node.path_live_weakrefs():
            ...
            if _storage_deref and storage_ref.data_ptr() not in deleted:
                ...
                smith._C._free_And_Remove_DeleterFn(_storage_deref)

                if self.disable_invalidate_aliases:
                    continue

                smith._C._set_storage_data_ptr_access_error_msg(_storage_deref, msg)
    ...
    def apply_checkpoint_execution_state_in_allocator(self) -> None:
        ...
        for ptr in OrderedSet(ptrs_to_deallocate):
            smith._C._cuda_cudaCachingAllocator_raw_delete(ptr)
        ...
        if config.triton.slow_path_cudagraph_asserts:
            ...
            for wrapper in live_storages_wrappers:
                ...
                assert smith._C._has_Standard_Deleter(storage_ptr)
