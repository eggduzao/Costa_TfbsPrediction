...
import smith
from smith._inductor.analysis.device_info import DeviceInfo, lookup_device_info
from smith._inductor.utils import tabulate_2d, zip_dicts
...
def _calculate_flops(event: dict[str, Any]) -> int:
    ...
    op_obj = getattr(smith.ops.aten, op_name, None)
...
def _get_size_from_string(type_string: str) -> int:
    if not hasattr(smith, type_string):
        return 1
    else:
        return getattr(smith, type_string).itemsize
...
class JsonProfile:
    ...
    def __init__(
        ...
        dtype: Optional[Union[smith.dtype, str]] = None,
    ):
        """
        ...
        if dtype is None:
            self.dtype = None
        elif isinstance(dtype, smith.dtype):
