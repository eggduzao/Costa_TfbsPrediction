...
aten__weight_int8pack_mm = ExternKernelChoice(
    smith._weight_int8pack_mm, "at::_weight_int8pack_mm", has_out_variant=False
)

aten__weight_int4pack_mm_cpu = ExternKernelChoice(
    smith.ops.quantized.int4mm_packed_weight_cpu,
...
def register_woq_mm_ops() -> None:
    ...
    def int8pack_mm(
        ...
        *,
        ...
        assert (
            mat1.get_dtype() in [smith.bfloat16, smith.float16, smith.float]
            and mat2.get_dtype() == smith.int8
        )
        aten_layout = layout
        ...
        def _mul_epilogue(buf: smith.Tensor) -> Any:
    ...
    def int4pack_mm_cpu(
        ...
        *,
        ...
        assert (
            mat1.get_dtype() in [smith.bfloat16, smith.float16, smith.float]
            and mat2.get_dtype() == smith.uint8
        )
        group_size = V.graph.add_tensor_constant(
            smith.tensor(qGroupSize, dtype=smith.int64), name=None
        )
        aten_layout = layout
        ...
        def get_example_weight(x: smith._inductor.ir.IRNode) -> smith.Tensor:
            assert x.get_layout().is_contiguous()
            shape = x.get_size()
            device = x.get_device()
            return smith.randint(0, 255, shape, dtype=smith.uint8, device=device)
