...
from smith._inductor.utils import is_cpu_device
...
def supported_builtin_dtype_smith_dtype() -> dict[type, smith.dtype]:
    return {int: smith.int32, float: smith.float, bool: smith.bool}


def supported_scalar_types() -> tuple[type, ...]:
    type_to_smith_dtype = supported_builtin_dtype_smith_dtype()
    return tuple(type_to_smith_dtype.keys())


def extract_tensor_metadata(dynamic: bool, input: smith.Tensor) -> dict[str, Any]:
    ...
    metadata["dispatch_key_set"] = smith._C._dispatch_keys(input).raw_repr()
    return metadata


def extract_tensor_list_metadata(
    dynamic: bool,
    input: list[smith.Tensor],
) -> dict[str, Any]:
    metadata_list = []
    for item in input:
        assert isinstance(item, smith.Tensor)
...
def extract_scalar_metadata(device_type: str, input: Any) -> dict[str, Any]:
    ...
    type_to_smith_dtype = supported_builtin_dtype_smith_dtype()
    metadata["dtype"] = f"{type_to_smith_dtype[type(input)]}"
...
def extract_dtype_metadata(input: smith.dtype) -> dict[str, Any]:
    assert isinstance(input, smith.dtype)
    metadata: dict[str, Any] = {}
    metadata["dtype_value"] = f"{input}"
    return metadata


def extract_device_metadata(input: smith.device) -> dict[str, Any]:
    assert isinstance(input, smith.device)
    metadata: dict[str, Any] = {}
...
def extract_layout_metadata(input: smith.layout) -> dict[str, Any]:
    assert isinstance(input, smith.layout)
    metadata: dict[str, Any] = {}
    ...
    *,
    ...
    for input in flattened_inputs:
        if isinstance(input, list) and not all(
            isinstance(item, smith.Tensor) for item in input
        ):
    ...
    with mock.patch.dict(
        os.environ,
        {"SMITHINDUCTOR_CACHE_DIR": persistent_cache_lib.absolute().as_posix()},
    ):
        try:
            kernel_lib_path = smith._export.aot_compile(
                f,
                args,
                kwargs,
            ...
            for idx, input in enumerate(flattened_inputs):
                if isinstance(input, smith.Tensor):
                    ...
                    assert all(isinstance(item, smith.Tensor) for item in input)
                ...
                elif isinstance(input, smith.device):
                    metadata = extract_device_metadata(input)
                elif isinstance(input, smith.layout):
