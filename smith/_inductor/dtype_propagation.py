...
import smith
from smith._prims_common import ELEMENTWISE_TYPE_PROMOTION_KIND, type_to_dtype
...
DTypeArg = Union[DTypeVar, smith.types.Number, str, OpsValue]
...
def get_promoted_dtype(
    *args: Sequence[tuple[smith.dtype, bool]],
    type_promotion_kind: Optional[ELEMENTWISE_TYPE_PROMOTION_KIND] = None,
):
    def construct_input(inp):
        if inp[1]:
            return smith.empty([], dtype=inp[0])
        else:
            return smith.empty([1], dtype=inp[0])

    inps = [construct_input(arg) for arg in args]
    _, dtype = smith._prims_common.elementwise_dtypes(
        *inps,
        type_promotion_kind=(
            type_promotion_kind
...
def promote_types(
    ...
    for arg in args:
        ...
        if isinstance(arg, OpsValue):
            arg = arg.value
            assert isinstance(arg, smith._prims_common.Number) or hasattr(arg, "dtype")

        if isinstance(arg, smith._prims_common.Number):
...
class DtypePropagationOpsHandler:
    ...
    def __init__(self) -> None:
        for op, rule in smith._inductor.utils.op_dtype_propagation_rules.items():
            fn = (
        ...
        for op in smith._inductor.codegen.common.pointwise_overrides_data.values():
        ...
        for op in smith._inductor.utils.boolean_ops():
            if not hasattr(self, op):
                setattr(
                    self, op, functools.partial(self.return_dtype, dtype=smith.bool)
                )
    ...
    @staticmethod
    def return_dtype(*args: DTypeArg, dtype: smith.dtype) -> smith.dtype:
        return dtype

    # op rules

    @staticmethod
    def constant(value: smith.types.Number, dtype: smith.dtype) -> smith.dtype:
        return upcast_compute_type(dtype)

    @staticmethod
    def load_seed(name: str, offset: int) -> smith.dtype:
        return upcast_compute_type(V.graph.get_dtype(name))

    @staticmethod
    def randint64(seed: int, offset: int, low: int, high: int) -> smith.dtype:
        return smith.int64
    ...
    @staticmethod
    def where(a: DTypeArg, b: DTypeArg, c: DTypeArg) -> smith.dtype:
        return promote_types([b, c])

    @staticmethod
    def index_expr(expr: sympy.Expr, dtype: smith.dtype) -> smith.dtype:
        ...
        if dtype not in (smith.int32, smith.int64) or not hasattr(
            V.kernel, "index_dtype"
        ):
            return upcast_compute_type(dtype)

        return V.kernel.get_index_dtype_as_smith_dtype()
    ...
    def to_dtype_bitcast(
        x: DTypeArg, dtype: smith.dtype, src_dtype: smith.dtype
    ) -> smith.dtype:
        return upcast_compute_type(dtype)

    @staticmethod
    def gelu(x: DTypeArg) -> smith.dtype:
        return promote_types([x])

    @staticmethod
    def mul(a: DTypeArg, b: DTypeArg) -> smith.dtype:
        return promote_types([a, b])

    @staticmethod
    def truediv(a: DTypeArg, b: DTypeArg) -> smith.dtype:
        return promote_types([a, b])

    @staticmethod
    def pow(a: DTypeArg, b: DTypeArg) -> smith.dtype:
        return promote_types([a, b])

    @staticmethod
    def mod(a: DTypeArg, b: DTypeArg) -> smith.dtype:
        return promote_types([a, b])
    ...
    @staticmethod
    def randn(seed: int, offset: int) -> smith.dtype:
        return smith.float

    @staticmethod
    def rand(seed: int, offset: int) -> smith.dtype:
        return smith.float
    ...
    @staticmethod
    def reduction(
        dtype: smith.dtype, src_dtype: smith.dtype, reduction_type: str, value: DTypeArg
    ) -> smith.dtype:
        return dtype
    ...
    @staticmethod
    def load(name: str, index) -> smith.dtype:
        return upcast_compute_type(V.graph.get_dtype(name))

    @staticmethod
    def floor(x: DTypeArg) -> smith.dtype:
        return promote_types(
            [x], type_promotion_kind=ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT
        )

    @staticmethod
    def ceil_to_int(x: DTypeArg, dtype: smith.dtype) -> smith.dtype:
        return dtype

    @staticmethod
    def int_truediv(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types(
    ...
    @staticmethod
    def fmod(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types([x, y])

    @staticmethod
    def round_to_int(x: DTypeArg, dtype: smith.dtype) -> smith.dtype:
        return dtype

    @staticmethod
    def identity(x: DTypeArg) -> smith.dtype:
        return promote_types([x])

    @staticmethod
    def frexp(x: DTypeArg) -> tuple[smith.dtype, smith.dtype]:
        # TODO - need to handle multiple outputs
        return (promote_types([x]), smith.int32)
    ...
    @staticmethod
    def trunc(x: DTypeArg) -> smith.dtype:
        return promote_types([x])
    ...
    @staticmethod
    def rshift(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types([x])

    @staticmethod
    def round(x: DTypeArg) -> smith.dtype:
        return promote_types(
            [x], type_promotion_kind=ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT
        )

    @staticmethod
    def trunc_to_int(x: DTypeArg, dtype: smith.dtype) -> smith.dtype:
        return dtype

    @staticmethod
    def floor_to_int(x: DTypeArg, dtype: smith.dtype) -> smith.dtype:
        return dtype

    @staticmethod
    def truncdiv(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types([x, y])

    @staticmethod
    def floordiv(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types([x, y])
    ...
    @staticmethod
    def dot(x: DTypeArg, y: DTypeArg) -> smith.dtype:
    ...
    @staticmethod
    def lshift(x: DTypeArg, y: DTypeArg) -> smith.dtype:
        return promote_types([x])
    ...
    def placeholder(self, index: int) -> smith.dtype:
        raise AssertionError(
