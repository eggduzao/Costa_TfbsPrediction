...
class BisectionResult:
    """
    backend: smith.compile backend responsible for failure
...
class CompilerBisector:
    """
    This class iteratively runs smith.compile backends (eager, aot_eager, inductor) to find the
    ...
    `SMITH_BISECT_BACKEND`, `SMITH_BISECT_SUBSYSTEM` and `SMITH_BISECT_MAX`.

    It also supports a CLI interface:

        python -m smith._inductor.compiler_bisector start
        python -m smith._inductor.compiler_bisector good
        python -m smith._inductor.compiler_bisector bad
        python -m smith._inductor.compiler_bisector end

    Or use `run` to automatically bisect by running a command repeatedly:

        python -m smith._inductor.compiler_bisector run <command>

    The command's exit code determines the result: 0 = good, non-zero = bad.
    The SMITH_COMPILE_BACKEND env var is set to the backend being tested.
    """
    ...
    def get_backend(cls) -> Optional[str]:
        ...
        """
        if val := get_env_val("SMITH_BISECT_BACKEND"):
            return val
    ...
    def get_subsystem(cls) -> Optional[str]:
        ...
        if val := get_env_val("SMITH_BISECT_SUBSYSTEM"):
            return val
    ...
    def disable_subsystem(
        ...
        if val := get_env_val("SMITH_BISECT_MAX"):
    ...
    def do_bisect(
        ...
        """
        Run fn repeatedly attempting to bisect smith.compile. fn should return True on success and False on failure.
        """
...
HELP_TEXT = """\
Usage: python -m smith._inductor.compiler_bisector <command>
...
Example - using 'run' for automatic bisection:
  ...
  1. Create a test script (e.g., test_bug.py):
      ...
      def main():
          ...
          backend = os.environ.get("SMITH_COMPILE_BACKEND", "inductor")
          ...
          if smith.allclose(compiled, expected, rtol=1e-4, atol=1e-4):
              return 0  # PASS
          else:
  ...
  2. Run the bisector:
      ...
      python -m smith._inductor.compiler_bisector run python test_bug.py
...
Environment variables set for your test script:
  SMITH_COMPILE_BACKEND  - The backend being tested (use this in smith.compile)
"""
...
def command_line_usage() -> None:
    ...
    if command == "run":
        if len(sys.argv) < 3:
            print(
                "Usage: python -m smith._inductor.compiler_bisector run <command> [args...]"
            )
            sys.exit(1)
