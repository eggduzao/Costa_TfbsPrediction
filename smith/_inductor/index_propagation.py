...
For example, say we have the IR:
    ...
    tmp0 = ops.index_expr(x, smith.int32)
    tmp1 = ops.constant(2, smith.int32)
...
import smith
from smith._prims_common import dtype_to_type, is_integer_dtype
from smith.utils._sympy.functions import FloorDiv, ModularIndexing, Where
    ...
    def __post_init__(self):
        if _is_constant(self.expr):
            ...
            if is_integer_dtype(self.dtype):
                bits = smith.iinfo(self.dtype).bits
...
class SymPyOps:
    ...
    @staticmethod
    def constant(value: Union[int, float, bool], dtype: smith.dtype) -> TypedExpr:
        return TypedExpr(value, dtype)

    @staticmethod
    def index_expr(value: Union[sympy.Expr, int], dtype: smith.dtype) -> TypedExpr:
        return TypedExpr(value, dtype)
    ...
    def add(x: TypedExpr, y: TypedExpr) -> TypedExpr:
        result_type = smith.promote_types(x.dtype, y.dtype)
...
class IndexPropagation(DefaultHandler):
    ...
    def materialize_expr(self, expr: sympy.Expr, dtype: smith.dtype) -> Any:
