...
from smith.utils._config_module import ConfigModule
from smith.utils._ordered_set import OrderedSet
from smith.utils._sympy.numbers import int_oo
from smith.utils._sympy.printers import PythonPrinter as _PythonPrinter
...
class CodegenSymbol(ABC):
    ...
    @abstractmethod
    def get_example(self) -> Union[smith.Tensor, sympy.Symbol]:
        pass
...
class TMADescriptorArg:
    ...
    dtype: Optional[smith.dtype]  # only needed for "stable"
...
# https://github.com/intel/intel-extension-for-blacksmith/blob/5dcc9d57e5422cf295e1a1ee97896d6b6a554a85/intel_extension_for_blacksmith/_inductor/__init__.py#L9
...
def get_backend_features(
    device: Union[smith.device, str, None],
...
def has_backend_feature(
    device: Union[smith.device, str, None], feature: BackendFeature
) -> bool:
...
def init_backend_registration() -> None:
    ...
    private_backend = smith._C._get_privateuse1_backend_name()
    if (
...
DTYPE_TO_COMPUTATION_DTYPE: dict[smith.dtype, smith.dtype] = {
    smith.bfloat16: smith.float,
    smith.float16: smith.float,
    **{
        dtype: dtype
        for dtype in [
            smith.bool,
...
def check_dtype(
    buffer: IndentedBuffer, var: CSEVariableType, dtype: smith.dtype
) -> None:
...
class DataTypePropagation:
    ...
    def deduce_node_dtype_by_inputs(self, node: smith.fx.Node) -> Optional[smith.dtype]:
        inputs = node.all_input_nodes
        input_nodes = [
            n for n in inputs if isinstance(n, smith.fx.Node) and n.op != "placeholder"
        ]
    ...
    def deduce_node_dtype_by_subgraph(self, node: smith.fx.Node) -> smith.dtype:
        sub_graph = self.graphs[node.target]
        dtype = self.propagate_graph(sub_graph)
        assert dtype
        return dtype

    def deduce_node_dtype(self, node: smith.fx.Node) -> Optional[smith.dtype]:
        ...
        if node.target is operator.getitem:
            ...
            assert isinstance(node_arg, smith.fx.Node), type(node_arg)
    ...
    def propagate_graph(self, graph: smith.fx.Graph) -> Optional[smith.dtype]:
        assert graph.nodes
        graph_dtype: Optional[smith.dtype] = None
    ...
    def propagate(self) -> Optional[smith.dtype]:
        return self.propagate_graph(self.graphs["root"])

    @classmethod
    def propagate_loopbody(cls, body: LoopBody) -> Optional[smith.dtype]:
        return cls(body).propagate()

    @classmethod
    def propagate_scheduler_node(cls, node: SchedulerNode) -> Optional[smith.dtype]:
        from ..loop_body import LoopBody
...
class OpDecompositions:
    ...
    def reciprocal(x: OpVarT) -> OpVarT:
        return ops.truediv(ops.constant(1, smith.int32), x)
    ...
    def erfc(x: OpVarT) -> OpVarT:
        return ops.sub(ops.constant(1, smith.float32), ops.erf(x))
    ...
    def expm1(x: OpVarT) -> OpVarT:
        return ops.sub(ops.exp(x), ops.constant(1, smith.float32))

    @staticmethod
    def log10(x: OpVarT) -> OpVarT:
        return ops.mul(ops.log(x), ops.constant(1 / math.log(10), smith.float32))

    @staticmethod
    def log2(x: OpVarT) -> OpVarT:
        return ops.mul(ops.log(x), ops.constant(1 / math.log(2), smith.float32))

    @staticmethod
    def exp2(x: OpVarT) -> OpVarT:
        return ops.exp(ops.mul(x, ops.constant(math.log(2), smith.float32)))

    @staticmethod
    def log1p(x: OpVarT) -> OpVarT:
        return ops.log(ops.add(x, ops.constant(1, smith.int32)))

    @staticmethod
    def sigmoid(x: OpVarT) -> OpVarT:
        one = ops.constant(1, smith.int32)
        return ops.truediv(one, ops.add(one, ops.exp(ops.neg(x))))

    @staticmethod
    def relu(x: OpVarT) -> OpVarT:
        return ops.maximum(x, ops.constant(0, smith.int32))
    ...
    @staticmethod
    def floor_to_int(a: OpVarT, dtype: smith.dtype) -> OpVarT:
        return ops.to_dtype(ops.floor(a), dtype)

    @staticmethod
    def ceil_to_int(a: OpVarT, dtype: smith.dtype) -> OpVarT:
        return ops.to_dtype(ops.ceil(a), dtype)

    @staticmethod
    def trunc_to_int(a: OpVarT, dtype: smith.dtype) -> OpVarT:
    ...
    def remainder(a: OpVarT, b: OpVarT) -> OpVarT:
        r = ops.mod(a, b)
        cond = ops.and_(
            ops.ne(r, ops.constant(0, smith.int32)),
            ops.ne(ops.signbit(r), ops.signbit(b)),
        )
        return ops.where(cond, ops.add(r, b), r)

    @staticmethod
    def round_to_int(a: OpVarT, dtype: smith.dtype) -> OpVarT:
...
class OpOverrides(BasicMathOpsMixin, OpDecompositions, OpsHandler[Any]):
    ...
    @staticmethod
    def constant(value: Union[bool, float, int], dtype: smith.dtype) -> OpVarT:
        return repr(value)
    ...
    def inline_asm_elementwise(
        ...
        *inputs: OpVarT,
        ...
        dtype: smith.dtype = smith.float32,
        is_pure: bool = True,
        pack: int = 1,
    ) -> OpVarT:
        raise NotImplementedError(
...
# smith._inductor.ops_handler too
...
class KernelArgs:
    ...
    def workspace(
        self, nelem: sympy.Expr, zero_fill: bool, dtype: smith.dtype = smith.uint8
    ) -> tuple[str, str, int]:
        """
        ...
        Args:
            ...
            dtype (smith.dtype): the dtype of the workspace tensor
    ...
    def wrap_ptr_arg(self, buf: str, dtype: smith.dtype) -> str:
        return buf
    ...
    def cpp_argdefs(
        self, dtype_to_cpp_type: Optional[dict[smith.dtype, str]] = None
...
class Kernel(CodeGen, Generic[CSEVariableType]):
    ...
    def __init__(
        ...
        self.node_to_bounds: Optional[dict[smith.fx.Node, ValueRanges[Any]]] = None
...
class KernelTemplate:
    ...
    def _fake_get_dtype(
        ...
        def get_dtype(name: str) -> smith.dtype:
    ...
    def uid(self) -> str:
        ...
        in the system, but reproducible e.g. restarting blacksmith should yield the same id
        """
...
class CSEProxy(DefaultHandler):
    ...
    def _default(self, name: str, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        ...
        def do_cse(v: Union[str, CSEVariable]) -> CSEVariable:
            ...
            nonlocal output_idx
            var_dtype: Optional[smith.dtype] = (
    ...
    def indirect_indexing(
        ...
        if var.bounds.lower < 0:
            if wrap_neg:
                stm = ops.add(var, ops.index_expr(size, smith.long))
    ...
    def bucketize(
        ...
        of smith.sort(offsets)).  Otherwise, the result is undefined.
        """
