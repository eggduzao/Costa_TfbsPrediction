...
def _is_int8_gemm(inputs):
    return (
        ...
        and inputs[0].get_dtype() in [smith.uint8, smith.int8]
    ) or (
        isinstance(inputs[0], smith.Tensor)
        and inputs[0].dtype in [smith.uint8, smith.int8]
    )
...
_T = TypeVar("_T", ir.IRNode, smith.Tensor)
...
def transpose_w(W: _T, trans_w: bool) -> _T:
    ...
    if isinstance(W, ir.IRNode):
        ...
        if trans_w:
            assert isinstance(W, smith.Tensor)
            # pyrefly: ignore [bad-assignment]
            W = W.transpose(0, 1)
...
def expand_bias(B: Optional[_T], X: _T) -> Optional[_T]:
    ...
    if B is not None:
        if isinstance(B, ir.IRNode):
            ...
            assert isinstance(B, smith.Tensor)
...
def prune_tensors(input_nodes: list[ir.IRNode], new_input_nodes: list[ir.IRNode]):
    ...
    def share_storage(base_tensor: smith.Tensor, comp_tensor: smith.Tensor):
    ...
    for candidate_node in get_candidates(input_nodes, new_input_nodes):
        ...
        for node in reversed(V.graph.graph.nodes):
            ...
            # https://github.com/blacksmith/blacksmith/issues/134998
            if node.op == "get_attr" and hasattr(
                ...
                if isinstance(comp_tensor, smith.Tensor) and share_storage(
...
class CppGemmTemplate(CppTemplate):
    ...
    def _cache_blocking(self, num_threads: int) -> GemmBlocking:
        def get_cache_blocking(register_blocking, thread_blocking):
            ...
            L1_cache_size = smith.cpu.get_capabilities().get(
                "l1d_cache_size", 0
            ...
            L2_cache_size = smith.cpu.get_capabilities().get(
                "l2_cache_size", 0
            )  # per core cache size in Bytes
            ...
            def get_num_byte(dtype):
                return smith.tensor([], dtype=dtype).element_size()
            ...
            if dtype_A is smith.bfloat16 and dtype_B is smith.int8 and Kr != 1:
            ...
            if (
                ...
                and dtype_A is smith.bfloat16
                and Mt_blocks == 1
            ):
                if dtype_B is smith.uint8:
                    # A16W4
    ...
    def add_choices(
        ...
        def maybe_to_dense(inputs, layout_or_out):
            ...
            if isinstance(inputs[1], smith.Tensor):
                W = inputs[1]
        ...
        def normalize_shapes(inputs, layout_or_out):
            new_inputs = list(inputs)
            if not is_mkldnn_wgt and isinstance(new_inputs[1], smith.Tensor):
        ...
        def preprocessor(inputs, layout):
            ...
            if only_one_input and isinstance(new_inputs[0], smith.Tensor):
        ...
        if not skip_int8_compensation and _is_int8_gemm(new_inputs):
            ...
            if use_int8_fast_compensation_path:
                ...
                if isinstance(W, ir.IRNode):
                    ...
                    BCompensate = smith.sum(W.to_dense().to(smith.float), dim=0)  # type: ignore[assignment]
                    assert all(
                        isinstance(item, smith.Tensor)
    ...
    def block_weight(cls, W, new_size, padding):
        ...
        if isinstance(W, ir.IRNode):
            ...
            blocked_w = (
                smith.nn.functional.pad(W, (0, padding))  # type: ignore[assignment]
    ...
    def get_options(
        ...
        int8_gemm = self.input_nodes[0].get_dtype() in [smith.uint8, smith.int8]
        x_scale = None
        x_zp = None
        w_scale = None
        w_zp = None
        ...
        options = dict(
            ...
            w_scale=w_scale,
            w_zp=w_zp,
            acc_buf_dtype=smith.int32 if int8_gemm else smith.float,
    ...
    def is_int8_woq_gemm_small_m_dim(
        ...
        return (
            ...
            and X.get_dtype() is smith.bfloat16
            and W.get_dtype() is smith.int8
        )
    ...
    def __getitem__(cls, q_group_size):
        class CppWoqInt4GemmTemplateInstance(CppGemmTemplate):
            ...
            def block_weight(cls, W, new_size, padding):
                ...
                # https://github.com/blacksmith/blacksmith/blob/32eee8ed225d9f10fbbcb38c24b8b44c24c0c97c/aten/src/ATen/native/cpu/int4mm_kernel.cpp#L583
                if isinstance(W, ir.IRNode):
                    ...
                    # https://github.com/blacksmith/blacksmith/blob/32eee8ed225d9f10fbbcb38c24b8b44c24c0c97c/aten/src/ATen/native/cpu/int4mm_kernel.cpp#L95
                    qscales_and_zeros = (
                    ...
                    # shape: [K, N]
                    unpacked_w = smith.ops.aten._weight_int4pack_mm_for_cpu(
                        x,
                        W,
                        G,
                        qscales_and_zeros,
