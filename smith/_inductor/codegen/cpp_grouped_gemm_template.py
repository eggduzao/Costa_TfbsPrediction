...
class CppGroupedGemmTemplate(CppGemmTemplate):
    ...
    def add_choices(
        ...
        _U = TypeVar("_U", ir.Layout, smith.Tensor)
        ...
        def maybe_to_dense(
            ...
            for idx in range(wgt_start_idx, wgt_start_idx + gemm_grouped_num):
                if isinstance(inputs[idx], smith.Tensor):
                    W = inputs[idx]
                    assert isinstance(W, smith.Tensor), "W must be a smith.Tensor"
        ...
        def postprocessor(output: _T) -> _T:
            if isinstance(output, ir.TensorBox):
                ...
                for idx in range(wgt_start_idx, wgt_start_idx + gemm_grouped_num):
                    ...
                    assert isinstance(W_packed, smith.Tensor)
