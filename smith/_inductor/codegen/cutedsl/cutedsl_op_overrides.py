...
import smith
from smith._inductor.codegen.common import CSEVariable, OpOverrides
from smith._inductor.virtualized import OpsValue, V
...
class CuteDSLOpOverrides(OpOverrides):
    ...
    SMITH_TO_CUTE_DTYPE = {
        smith.float16: "cutlass.Float16",
        smith.bfloat16: "cutlass.BFloat16",
        smith.float32: "cutlass.Float32",
        smith.float64: "cutlass.Float64",
        smith.int8: "cutlass.Int8",
        smith.int16: "cutlass.Int16",
        smith.int32: "cutlass.Int32",
        smith.int64: "cutlass.Int64",
        smith.bool: "cutlass.Boolean",
        smith.float8_e4m3fn: "cutlass.Float8E4M3FN",
        smith.float8_e5m2: "cutlass.Float8E5M2",
    }
    ...
    def _extract_dtype_and_bounds(
        *args: CuteDSLArg,
    ) -> tuple[Optional[smith.dtype], ValueRanges[sympy.Expr]]:
    ...
    @staticmethod
    def constant(value: Union[bool, float, int], dtype: smith.dtype) -> str:
    ...
    def abs(x: CuteDSLArg) -> CuteDSLArg:
        ...
        abs_op = (
            "mlir_math.absf"
            if x_dtype in (smith.float16, smith.bfloat16, smith.float32)
            else "mlir_math.absi"
        )
        return CuteDSLOpOverrides._apply_unary_op(
            x,
    ...
    @staticmethod
    def to_dtype(
        x: CuteDSLArg, dtype: smith.dtype, src_dtype=None, use_compute_types=True
    ) -> CuteDSLArg:
        """Type conversion using CuteDSL TensorSSA.to(Type[Numeric]).

        Maps smith dtypes to cutlass.cute.typing numeric types and emits
        ...
        cute_type = CuteDSLOpOverrides.SMITH_TO_CUTE_DTYPE.get(dtype)
        if cute_type is None:
            raise NotImplementedError(
                f"CuteDSL dtype cast not implemented for smith dtype: {dtype}"
            )
