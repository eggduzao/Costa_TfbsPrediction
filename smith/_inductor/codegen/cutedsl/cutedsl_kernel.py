...
from smith._inductor.ir import (
    BaseView,
    Buffer,
    ComputedBuffer,
    ExternKernel,
...
from smith._inductor.ops_handler import StoreMode
...
class CuteDSLTemplateKernel(Kernel):
    ...
    def gen_imports(self) -> str:
        ...
        imports.splice(
            ...
            import operator
            from smith._inductor.codegen.cutedsl._cutedsl_utils import ssa_to_indexable, result_to_ssa
            """
        )
...
class ModificationWrapperCuteDSL(V.WrapperHandler):  # type: ignore[name-defined]
    ...
    This class sits between the Blacksmith IR and CuteDSL code generation, providing:
    1. Operation substitution: converts Blacksmith ops to CuteDSL equivalents via CuteDSLOpOverrides
    ...
    def _get_input_dtype(self, name: str) -> smith.dtype:
        ...
        return smith.float32 if name not in ("b", "h", "m", "n") else smith.int32

    def load(self, name: str, index: sympy.Expr):
        """Handle loading from tensor or fixed(template args) input for CuteDSL."""
        from smith._inductor.kernel.flex.flex_flash_attention import HierarchicalIndex
        ...
        if name not in self.fixed_inputs:
            ...
            cute_dtype = CuteDSLOpOverrides.SMITH_TO_CUTE_DTYPE.get(
    ...
    def _emit_scalar_fragment(
        self, expr_str: str, cute_dtype: str, smith_dtype: smith.dtype
    ) -> str:
        """
        ...
        """
        result = self.kernel.cse.newvar(dtype=smith_dtype)
        self.kernel.body.writeline(
    ...
    def _default(self, name: str, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        try:
            ...
            msg = textwrap.dedent(f"""
                ...
                Please open an issue at: https://github.com/blacksmith/blacksmith/issues
