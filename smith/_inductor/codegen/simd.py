...
from smith._inductor.ir import MultiTemplateBuffer
from smith._inductor.tiling_utils import analyze_memory_coalescing
...
from smith.utils._sympy.functions import FloorDiv, Identity, ModularIndexing
from smith.utils._sympy.symbol import (
    free_symbol_is_type,
    prefix_str,
    symbol_is_type,
    SymT,
...
if TYPE_CHECKING:
    ...
    from smith._inductor.tiling_utils import CoalesceVarAnalysis
...
def get_max_tiles(default: int = 2) -> int:
    max_tiles = smith._inductor.config.triton.max_tiles
...
class SIMDKernel(Kernel[CSEVariableType], Generic[CSEVariableType]):
    ...
    def get_index_dtype_as_smith_dtype(self) -> smith.dtype:
        return self.features.select_index_dtype()

    @property
    def index_dtype(self) -> str:
        return self.dtype_to_str(self.get_index_dtype_as_smith_dtype())
...
class SIMDScheduling(BaseScheduling):
    ...
    def can_fuse(self, node1, node2):
        ...
        if node1.is_reduction() and node2.is_reduction():
            ...
            if reduction_can_fuse and (
                ...
                #    For example: C[z,y,x] = smith.bmm(A[z,y,r], B[z,r,x]) keeps (z,y,x) order.
    ...
    def _codegen_mix_order_reduction(self, node1, node2):
        ...
        if (
            not smith._inductor.config.deterministic
        ...
        for idx, partial_accum in enumerate(kernel.saved_partial_accumulate):
            ...
            # The workspace tensor is in smith.float, need a cast if the buffer is
            # not.
            if (buffer_dtype := V.graph.get_dtype(buffer_name)) != smith.float:
    ...
    def codegen_node(
        ...
        if smith._inductor.config.triton.coalesce_tiling_analysis:
    ...
    def can_use_32bit_indexing(
        numel: sympy.Expr,
        buffers: Iterable[
            Union[ir.Buffer, ir.TensorBox, ir.SmithBindObject, ir.IRNode]
        ],
    ) -> bool:
        ...
        for cand, tiling_score in sorted(tilings, key=score_mod):
            if (
                ...
                if tiling_len > get_max_tiles(default=3):
                    perf_hint_log.info(
                        "Found optimal tiling with %s tiles but smith._inductor.config.triton.max_tiles "
                        "set to %s. Consider increasing",
                        tiling_len,
                        smith._inductor.config.triton.max_tiles,
                    )
                    continue
    ...
    def get_tiling_and_scores(
        ...
        if (
            smith._inductor.config.triton.coalesce_tiling_analysis
