...
def inline_subgraph_to_ir_nodes(
    gm: smith.fx.GraphModule, inputs: list[Any], name: str
) -> Any:
    ...
    """
    from smith._inductor.lowering import process_subgraph_nodes
...
class SubgraphChoiceCaller(ir.ChoiceCaller):
    ...
    def __init__(
        ...
        input_gen_fns: dict[int, Callable[[Any], smith.Tensor]] | None = None,
    ) -> None:
    ...
    def _compute_sym_input_values(self) -> list[int]:
        ...
        for inp_node, example_inp in zip(self.input_nodes, self.example_inputs):
            if isinstance(example_inp, smith.Tensor):
    ...
    def benchmark(self, *args: list[Any], out: smith.Tensor) -> float:
    ...
    def benchmark_collective(self, *args: list[Any], out: smith.Tensor) -> None:
...
class SubgraphTemplate(KernelTemplate):
    ...
    def _validate_non_tensor_kwargs(self, kwargs: dict[str, Any]) -> None:
        ...
        for key, value in kwargs.items():
            assert not isinstance(value, (smith.Tensor, Buffer)), (
        ...
        with V.fake_mode:
            ...
            for i, inp in enumerate(input_nodes):
                if input_gen_fns and i in input_gen_fns:
                    ...
                    fake_tensor = smith.empty_strided(
                        concrete_shape,
                        concrete_stride,
                        dtype=inp.get_dtype(),
            ...
            assert isinstance(output, smith.Tensor), (
