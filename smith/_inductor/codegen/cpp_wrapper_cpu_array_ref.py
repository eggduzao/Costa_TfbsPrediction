...
class CppWrapperCpuArrayRef(CppWrapperCpu):
    ...
    represented as ArrayRef, see smith/csrc/inductor/aoti_runtime/arrayref_tensor.h
    """
    ...
    def get_device_include_path(device: str) -> str:
        ...
        if V.graph.aot_mode:
            return "#include <smith/csrc/inductor/aoti_include/array_ref.h>"
        return "#include <smith/csrc/inductor/cpp_wrapper/array_ref.h>"
                ...
                if config.aot_inductor.use_minimal_arrayref_interface:
                    ...
                    self.suffix.splice(
                        ...
                        extern "C" AOTIRuntimeError AOTInductorModelRunMinimalArrayrefInterface(
                          ...
                          auto model = reinterpret_cast<smith::aot_inductor::AOTInductorModel*>(model_handle);
                          CONVERT_EXCEPTION_TO_ERROR_CODE({
                              outputs = model->run_impl_minimal_arrayref_interface<AOTInductorModelInputs, AOTInductorModelOutputs>(
                                  inputs,
                                  (smith::aot_inductor::DeviceStreamType)nullptr,
                                  nullptr);
                          })
                        }
                    """
    ...
    def generate_return(self, output_refs: list[str]):
        ...
        def use_thread_local_cached_output_tensor(idx, output):
            ...
            if arr_iface:
                ...
                self.wrapper_call.writeline(
                    f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_new_uninitialized_tensor(&output_handles[{idx}]));"
                )
                self.wrapper_call.writeline(
                    f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_assign_tensors({cached_output_name}.tensor(), "
            ...
            with self.wrapper_call.indent():
                if arr_iface:
                    ...
                    if is_constant_buffer:
                        ...
                        self.wrapper_call.writeline(
                            f"aoti_smith_clone({output}, &{cached_output_name}_tmp);"
                        )
                        self.wrapper_call.writeline(
    ...
    def codegen_device_copy(self, src, dst, non_blocking: Union[bool, str]):
        # aoti_smith_tensor_copy_ takes AtenTensorHandle as input,
        # while stack-allocation results in ArrayRefTensor
        # so disable stack allocation here
        self.allow_stack_allocation = False
        self.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_copy_(expensive_copy_to_tensor_if_needed({dst}), {src}, {non_blocking}));"
        )
    ...
    def val_to_arg_str(self, val, type_=None) -> str:
        if (
            val is not None
            and isinstance(type_, smith.OptionalType)
            and isinstance(type_.getElementType(), smith.TensorType)
        ):
            # Handle optional tensors as a special case, as in the parent class.
            base_handle = self.val_to_arg_str(val, smith.TensorType)
            if config.aot_inductor.use_minimal_arrayref_interface:
