...
class CppWrapperMps(CppWrapperGpu):
    ...
    def _generate_kernel_call_helper(
        ...
        *,
        ...
        auto mps_lib_0_lambda = [&](AOTIMetalKernelFunctionHandle handle) {
            aoti_smith_mps_start_encoding(handle);
            aoti_smith_mps_set_arg_tensor(handle, 0, buf0);
            aoti_smith_mps_set_arg_tensor(handle, 1, arg0_1);
            aoti_smith_mps_set_arg_tensor(handle, 2, arg1_1);
            aoti_smith_mps_dispatch_single(handle, static_cast<uint64_t>(10LL));
        };

        std::function<void(AOTIMetalKernelFunctionHandle)> mps_lib_0_func_wrapper = mps_lib_0_lambda;
        aoti_smith_mps_run_command_block(get_mps_lib_0_handle(), aoti_smith_mps_shared_callback, &mps_lib_0_func_wrapper);
        ```
        """
        ...
        for idx, (arg, arg_type) in enumerate(zip(call_args[:-2], arg_types[:-2])):
            if isinstance(arg_type, smith.dtype):
                new_args.append(f"aoti_smith_mps_set_arg_tensor(handle, {idx}, {arg});")
            elif arg_type in (int, sympy.core.symbol.Symbol):
                new_args.append(f"aoti_smith_mps_set_arg_int(handle, {idx}, {arg});")
            else:
        ...
        if is_single_value:
            ...
            if group_size is None:
                new_args.append(
                    f"aoti_smith_mps_dispatch_single(handle, {single_value});"
                )
            else:
                ...
                new_args.append(
                    f"aoti_smith_mps_dispatch_single_with_group_size(handle, {single_value}, {group_size_value});"
                )
        else:
            ...
            if group_size is None:
                ...
                new_args.append(
                    f"    aoti_smith_mps_dispatch_array(handle, {threads_var}, {threads_size});"
                )
                new_args.append("}")
            else:
                group_size_str = str(group_size)
                ...
                new_args.append(
                    f"    aoti_smith_mps_dispatch_array_with_group_size({dispatch_args});"
                )
                new_args.append("}")
    ...
    def write_mps_kernel_call(self, name: str, call_args: list[str]) -> None:
        ...
        self.writeline("    aoti_smith_mps_start_encoding(handle);")
        ...
        self.writeline(
            f"aoti_smith_mps_run_command_block(get_{name}_handle(), aoti_smith_mps_shared_callback, &{wrapper_name});"
        )
    ...
    def get_device_include_path(device: str) -> str:
        ...
        return (
            "#include <smith/csrc/inductor/aoti_include/mps.h>\n"
    ...
    def codegen_additional_funcs(self) -> None:
        ...
        AOTIMetalKernelFunctionHandle get_mps_lib_0_handle() {
            static auto kernel_handle = []() {
                ...
                aoti_smith_mps_create_shader_library(mps_lib_0_source, &lib_handle);
                aoti_smith_mps_get_kernel_function(lib_handle, "generated_kernel", &kern_handle);

                // RAII wrapper with custom deleter
                auto lib_deleter = [](AOTIMetalShaderLibraryHandle h) {
                    if (h) aoti_smith_mps_delete_shader_library(h);
                };
...
AOTIMetalKernelFunctionHandle get_{lib_name}_handle() {{
    static auto kernel_handle = []() {{
        ...
        aoti_smith_mps_create_shader_library({lib_name}_source, &lib_handle);
        aoti_smith_mps_get_kernel_function(lib_handle, "generated_kernel", &kern_handle);
