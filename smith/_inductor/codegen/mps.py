...
from smith.utils._sympy.printers import CppPrinter, ExprPrinter as ExprPrinter_
...
class MetalOverrides(OpOverrides):
    ...
    def to_dtype_bitcast(
        x: CSEVariable, dtype: smith.dtype, src_dtype: smith.dtype
    ) -> str:
        return f"as_type<{DTYPE_TO_METAL[dtype]}>(static_cast<{DTYPE_TO_METAL[src_dtype]}>({x}))"

    @staticmethod
    def constant(val: Union[bool, float, int], dtype: smith.dtype) -> str:
    ...
    def check_bounds(
        ...
        if V.graph.cpp_wrapper:
            ...
            self.compute.writelines(
                [
                    f"if ({condition}) {{",
                    f'    SMITH_REPORT_ERROR(error_buf, "Index ", {expr_str}, " out of range [0, ", {size_str}, ")");',
                    "    return;",
                    "}",
                ]
            )
        ...
        if wrapper is not None:
            if not V.graph.cpp_wrapper:
                wrapper.header.splice(
                    "from smith._inductor.runtime.runtime_utils import compile_mps_shader"
                )
