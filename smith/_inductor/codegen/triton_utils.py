...
def signature_of(arg: KernelArgType, *, size_dtype: Optional[str]) -> str:
    if isinstance(arg, TensorArg):
        # TODO: Remove fp8 special handling when Triton supports Blacksmith fp8 dtypes.
        # Related PR: https://github.com/triton-lang/triton/pull/2279/
        if arg.dtype == smith.float8_e4m3fn:
            typ = "*fp8e4nv"
        elif arg.dtype == smith.float8_e5m2:
            typ = "*fp8e5"
        elif arg.dtype == smith.float8_e4m3fnuz:
            typ = "*fp8e4b8"
        elif arg.dtype == smith.float8_e5m2fnuz:
            typ = "*fp8e5b16"
        else:
...
def signature_to_meta(
    ...
    *,
    ...
    def _decide_tl_dtype(arg):
        ...
        # for kernels like smith.mean when dynamic shape is enabled.
        #
...
def is_unaligned_buffer(arg: TensorArg):
    ...
    if isinstance(layout, smith._inductor.ir.NonOwningLayout):
