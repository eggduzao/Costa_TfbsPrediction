...
from smith.utils._sympy.printers import CppPrinter as _CppPrinter
...
DTYPE_TO_CPP = {
    smith.float32: "float",
    smith.float64: "double",
    smith.float16: "at::Half",
    smith.int64: "int64_t",
    smith.int32: "int32_t",
    smith.int16: "int16_t",
    smith.int8: "int8_t",
    smith.uint64: "uint64_t",
    smith.uint32: "uint32_t",
    smith.uint16: "uint16_t",
    smith.uint8: "uint8_t",
    smith.bool: "bool",
    smith.bfloat16: "at::BFloat16",
    smith.complex32: "at::complex<at::Half>",
    smith.complex64: "at::complex<float>",
    smith.complex128: "at::complex<double>",
    smith.float8_e4m3fn: "at::Float8_e4m3fn",
    smith.float8_e5m2: "at::Float8_e5m2",
    smith.float8_e4m3fnuz: "at::Float8_e4m3fnuz",
    smith.float8_e5m2fnuz: "at::Float8_e5m2fnuz",
}

DTYPE_TO_ATEN = {
    smith.float32: "at::kFloat",
    smith.float64: "at::kDouble",
    smith.float16: "at::kHalf",
    smith.int64: "at::kLong",
    smith.int32: "at::kInt",
    smith.int16: "at::kShort",
    smith.int8: "at::kChar",
    smith.uint64: "at::kUInt64",
    smith.uint32: "at::kUInt32",
    smith.uint16: "at::kUInt16",
    smith.uint8: "at::kByte",
    smith.uint32: "at::kUInt32",
    smith.uint64: "at::kUInt64",
    smith.bool: "at::kBool",
    smith.bfloat16: "at::kBFloat16",
    smith.complex32: "at::kComplexHalf",
    smith.complex64: "at::kComplexFloat",
    smith.complex128: "at::kComplexDouble",
    smith.float8_e4m3fn: "at::kFloat8_e4m3fn",
    smith.float8_e5m2: "at::kFloat8_e5m2",
    smith.float8_e4m3fnuz: "at::kFloat8_e4m3fnuz",
    smith.float8_e5m2fnuz: "at::kFloat8_e5m2fnuz",
}
...
LAYOUT_TO_ATEN = {
    smith.strided: "at::kStrided",
    smith._mkldnn: "at::kMkldnn",  # type: ignore[attr-defined]
}
...
def get_promote_dtype(args):
    return (
        ...
        functools.reduce(
            smith.promote_types,  # type: ignore[arg-type]
...
def may_unify_binary_op_mask_type(a, b):
    ...
    if a.dtype == smith.bool:
        ...
        mask_dtype = smith.int32
        return unify_mask_base_type(V.kernel.compute, (a, b), mask_dtype)
    return a, b


def codegen_rand(offset, code, rand_function, dst_dtype=smith.float32):
...
def get_gemm_template_output_and_compute_dtype(input_dtype):
    if input_dtype in [smith.uint8, smith.int8]:
        return (smith.int32, smith.int32)
    else:
        return (smith.float32, smith.float32)
...
def create_epilogue_with_attr(input_buffer, attr, **kwargs):
    ...
    if attr == "relu":
        ...
        if kwargs["algorithm"] == "none":
            ...
            def inner_fn(index):
                ...
                if dtype != smith.float:
                    input = ops.to_dtype(input, smith.float)
                half = ops.constant(0.5, smith.float)
                one = ops.constant(1.0, smith.float)
                const = ops.constant(0.7071067811865476, smith.float)
            ...
            def inner_fn(index):
                ...
                half = ops.constant(0.5, smith.float)
                one = ops.constant(1.0, smith.float)
                const1 = ops.constant(0.7978845608028654, smith.float)
                const2 = ops.constant(0.044715, smith.float)
                result = (
                    half
                    * input
                    * (
        ...
        def hardsigmoid_float(input):
            zero = ops.constant(0, smith.float)
            six = ops.constant(6, smith.float)
            three = ops.constant(3, smith.float)
            one_over_six = ops.constant(0.16666666666666666, smith.float)
        ...
        def inner_fn(index):
            ...
            zero = ops.constant(0, smith.float)
            result = ops.where(
                input > zero, input, input * ops.constant(negative_slope, smith.float)
            )
            if dtype != smith.float:
                result = ops.to_dtype(result, dtype)
            return result
        ...
        def inner_fn(index):
            ...
            if dtype != smith.float:
                input = ops.to_dtype(input, smith.float)
            result = ops.minimum(
                ops.maximum(input, ops.constant(min_value, smith.float)),
                ops.constant(max_value, smith.float),
            )
            if dtype != smith.float:
                result = ops.to_dtype(result, dtype)
            return result
        ...
        def inner_fn(index):
            ...
            input = input_loader(index)
            if beta != 1:
                result = ops.constant(beta, smith.float) * bias + input
            else:
                result = bias + input
            return result
