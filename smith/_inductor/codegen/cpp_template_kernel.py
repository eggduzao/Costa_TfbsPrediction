...
class CppTemplateKernel(CppKernel):
    ...
    def acc_dtype(self, node: ir.Buffer) -> str:
        if node.get_dtype() in [smith.float32, smith.bfloat16, smith.half]:
            return "float"
        else:
    ...
    def define_buffer(self, name, sizes: list[Any], dtype=smith.float) -> str:
        """Define kernel local buffer"""
        sizes = parse_expr_with_index_symbols(sizes)
        buf = ir.Buffer(
            name=name, layout=ir.FixedLayout(smith.device("cpu"), dtype, sizes)
        )
    ...
    def define_stack_allocated_buffer(
        self, name, sizes: list[Any], dtype=smith.float
    ) -> str:
