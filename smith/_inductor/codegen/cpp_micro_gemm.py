...
"""
    ...
    def get_common_options(self):
        if self.input_dtype in [smith.uint8, smith.int8]:
            assert self.compute_dtype == smith.int32
            assert self.output_dtype == smith.int32
            assert self.input2_dtype == smith.int8
        return {
            "smith": smith,
            ...
            "int8_gemm": self.input_dtype in [smith.uint8, smith.int8],
            "vnni_size": 4 if self.input_dtype in [smith.uint8, smith.int8] else 2,
...
class CppMicroGemmConfig:
    ...
    input2_dtype: smith.dtype
    output_dtype: smith.dtype
    compute_dtype: smith.dtype
...
@register_micro_gemm(
    ...
    *generate_gemm_config(
        ...
        [(8, 48, 1), (8, 32, 1), (16, 16, 1)],
        input_dtype=smith.bfloat16,
        input2_dtype=smith.int8,
        output_dtype=smith.float,
        compute_dtype=smith.float,
...
class CppMicroGemmFP32Vec(CppMicroGemm):
    ...
    It supports input types of smith.float, smith.bfloat16, and smith.half with fp32 output.
...
{%- endif %}
    ...
    using Vectorized = at::vec::Vectorized<{{compute_t}}>;
{%- if input2_dtype in [smith.bfloat16, smith.float16] %}
...
def check_vnni_extra(config, m, n, k, alpha, num_threads, **kwargs):
    assert config.input_dtype == smith.uint8 and config.input2_dtype == smith.int8
    vnni_size = 4
...
@register_micro_gemm(
    *generate_gemm_config(
        ...
        [(6, 64, 4)],
        input_dtype=smith.uint8,
        input2_dtype=smith.int8,
        output_dtype=smith.int32,
        compute_dtype=smith.int32,
...
"""
    ...
    def __init__(
        ...
        assert input_dtype == smith.uint8 and input2_dtype == smith.int8, (
...
def check_amx_extra(config, m, n, k, alpha, num_threads, **kwargs):
    vnni_size = 4 if config.input_dtype in [smith.uint8, smith.int8] else 2
...
def check_amx_fp16_extra(config, m, n, k, alpha, num_threads, **kwargs):
    assert config.input_dtype == smith.float16 and config.output_dtype == smith.float
    vec_isa = kwargs.get("vec_isa")
    assert vec_isa is not None
    vnni_size = 2
...
class CppMicroGemmAMX(CppMicroGemm):
    ...
    It supports input types of smith.bfloat16 with fp32 output.
    """
    ...
    def codegen_define(self, kernel: CppTemplateKernel) -> str:
        ...
        options = {
            ...
            "use_cached_dequantized_B": (
                self.input_dtype == smith.bfloat16
                and self.input2_dtype in [smith.int8, smith.uint8]
            ),
            "kernel": kernel,
            "block_m": block_m,
            "block_n": block_n,
...
def check_brgemm_extra(config, m, n, k, alpha, num_threads, **kwargs):
    assert config.input_dtype == smith.half and config.output_dtype == smith.float
    vnni_size = 2
    # use brgemm for Half when amx_fp16 is supported
    return smith.cpu._is_amx_fp16_supported() and k % vnni_size == 0 and alpha == 1
...
class CppMicroBrgemm(CppMicroGemm):
    ...
    It supports input types of smith.half.
    """
...
"""
    ...
    def get_b_layout(self):
        assert self.input_dtype == smith.half and smith.cpu._is_amx_fp16_supported()
        return LayoutType.VNNI2
...
def create_micro_gemm(
    ...
    def skip_amx_kernel_for_woq(dynamic_M):
        ...
        if (
            dynamic_M
            or input_dtype != smith.bfloat16
            or input2_dtype not in [smith.int8, smith.uint8]
        ):
            return False
        m_threshold = 5
