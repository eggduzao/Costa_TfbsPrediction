...
"""
NVIDIA Universal GEMM (NVGEMM) backend for Blacksmith Inductor.
...
from smith._inductor.codegen.cuda.cuda_env import get_cuda_arch
from smith._inductor.codegen.nv_universal_gemm.nv_universal_gemm_utils import (
    to_cutlass_scale_mode,
)
from smith._inductor.ir import Buffer, ChoiceCaller, Layout, TensorBox
from smith._inductor.kernel_inputs import MMKernelInputs
from smith._inductor.template_heuristics.nv_universal_gemm import get_nvgemm_heuristics
from smith._inductor.utils import ensure_nv_universal_gemm_available
...
class NVUniversalGemmBenchmarkRequest(GPUDeviceBenchmarkMixin, BenchmarkRequest):
    ...
    def __init__(
        ...
        self._workspace: Optional[smith.Tensor] = None
    ...
    def make_run_fn(self, *input_tensors: smith.Tensor, out: smith.Tensor):
        ...
        if self.workspace_size > 0:
            self._workspace = smith.empty(
                self.workspace_size, device=out.device, dtype=smith.int8
            )
        else:
...
class NVUniversalGemmCaller(ChoiceCaller):
    ...
    def output_node(self) -> TensorBox:
        from smith._inductor.ir import NVUniversalGemmBuffer
...
def _create_dummy_tensor_from_layout(layout: Layout) -> Optional[smith.Tensor]:
    """
...
def _add_nv_gemm_choices_impl(
    ...
    from smith._inductor.codegen.nv_universal_gemm.kernel_cache import (
...
def add_nv_universal_gemm_choices(
    ...
    accumulator_type: Optional[smith.dtype] = None,
) -> None:
    """
    ...
    _add_nv_gemm_choices_impl(
        ...
        accumulator_type=accumulator_type or smith.float32,
        mm_inputs=inputs,
    )
...
def add_nv_universal_scaled_gemm_choices(
    ...
    from smith._inductor.utils import infer_scale_swizzle_ir
