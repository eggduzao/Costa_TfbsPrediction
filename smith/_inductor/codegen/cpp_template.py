...
class CppTemplate(KernelTemplate):
    ...
    def header(self) -> IndentedBuffer:
        res = IndentedBuffer()
        res.writeline("#include <smith/csrc/inductor/cpp_prefix.h>")
        # TODO: add c10::ForcedUnroll test to test_aoti_abi_check
        res.splice("""#include <c10/util/Unroll.h>""")
        res.splice("""#include <smith/csrc/inductor/aoti_smith/c/shim.h>""")
        enable_kernel_profile = config.cpp.enable_kernel_profile and sys.platform in [
            "linux",
            "win32",
        ]
        if enable_kernel_profile:
            res.writelines(["#include <smith/csrc/inductor/aoti_runtime/utils.h>"])
        return res
