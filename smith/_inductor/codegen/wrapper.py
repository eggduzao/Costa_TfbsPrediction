...
from smith import dtype as smith_dtype
from smith._dynamo.utils import counters, dynamo_timed, get_debug_dir
from smith._inductor.codegen.debug_utils import DebugPrinterManager
from smith._inductor.codegen.multi_kernel import MultiKernelState
from smith._inductor.runtime.runtime_utils import cache_dir
from smith._library.opaque_object import get_opaque_obj_repr, is_opaque_value_type
...
from smith.fx.node import _get_qualified_name
...
ReuseKey = tuple[smith.device, smith.dtype, str, bool]
CommBufferReuseKey = tuple[smith.device, smith.dtype, str, "ir.CommBufferType", str]
...
class ExternKernelOutLine(WrapperLine):
    ...
    def codegen(self, code: IndentedBuffer) -> None:
        ...
        if (
            V.graph.cpp_wrapper
            and node.cpp_kernel_name == "smith::inductor::_mm_plus_mm"
        ):
            # For https://github.com/blacksmith/blacksmith/issues/128474
            kernel_name = "aoti_smith__mm_plus_mm_out"
        else:
            kernel_name = node.get_kernel_name()
...
class FreeLine(WrapperLine):
    wrapper: PythonWrapperCodegen
    node: Union[BufferLike, ir.SmithBindObject]
    ...
    def _codegen_comm_buffer(self, code: IndentedBuffer) -> None:
        ...
        if comm_buffer_type == ir.CommBufferType.SYMM_MEM:
            line = (
                ...
                f"{dtype}, "
                f'smith.device("cuda:{device.index}"), '
...
class PythonWrapperCodegen(CodeGen):
    ...
    def __init__(self):
        ...
        self.args_to_buffers: dict[
            str, Union[None, ir.TensorBox, ir.Buffer, ir.SmithBindObject]
        ] = {}
    ...
    def write_header(self) -> None:
        ...
        if int(config.aot_inductor.debug_intermediate_value_printer) > 0:
            inductor_debug_utils = "from smith._inductor.codegen.debug_utils import _print_debugging_tensor_value_info"
        elif smith._inductor.config.test_configs.track_memory_lifecycle:
            inductor_debug_utils = "from smith._inductor.runtime.debug_utils import tracked_empty_strided\n"
        ...
        self.imports.splice(
            f"""
                ...
                from smith._inductor.hooks import run_intermediate_hooks
                from smith._inductor.utils import maybe_profile
                from smith._inductor.codegen.memory_planning import _align as align
                from smith import device, empty_strided
                from {async_compile.__name__} import AsyncCompile
                from smith._inductor.select_algorithm import extern_kernels
                {inductor_debug_utils}
            """,
            strip=True,
        )
        self.header.splice(
            """
                aten = smith.ops.aten
                inductor_ops = smith.ops.inductor
                _quantized = smith.ops._quantized
                assert_size_stride = smith._C._dynamo.guards.assert_size_stride
                assert_alignment = smith._C._dynamo.guards.assert_alignment
                empty_strided_cpu = smith._C._dynamo.guards._empty_strided_cpu
                empty_strided_cpu_pinned = smith._C._dynamo.guards._empty_strided_cpu_pinned
                empty_strided_cuda = smith._C._dynamo.guards._empty_strided_cuda
                empty_strided_xpu = smith._C._dynamo.guards._empty_strided_xpu
                empty_strided_mtia = smith._C._dynamo.guards._empty_strided_mtia
                reinterpret_tensor = smith._C._dynamo.guards._reinterpret_tensor
                alloc_from_pool = smith.ops.inductor._alloc_from_pool
        ...
        try:
            ...
            # is available
            from smith._C._distributed_c10d import _SymmetricMemory  # noqa: F401

            self.header.splice(
                """
                empty_strided_p2p = smith._C._distributed_c10d._SymmetricMemory.empty_strided_p2p
                """,
                strip=True,
            )
        ...
        if config.annotate_training:
            self.header.writeline("from smith.cuda import nvtx")
        if config.triton.proton_profiling:
            ...
            self.header.writeline(
                ...
                "smith.empty(size, dtype=smith.uint8, device='cuda'))"
            )
            ...
            self.header.writeline(
                "from smith._inductor.runtime.proton_utils import process_proton_trace as _proton_process_trace"
            )
            self.header.splice(
                f"""
    ...
    def write_kernel_autotune_defs_header(self) -> None:
        self.kernel_autotune_defs.splice(
            f"""
                ...
                from smith._inductor.select_algorithm import AlgorithmSelectorCache
                from {async_compile.__name__} import AsyncCompile
        ...
        try:
            from smith._C import _cuda_getCurrentRawStream  # noqa: F401

            self.kernel_autotune_defs.splice(
                """
                get_raw_stream = smith._C._cuda_getCurrentRawStream
                """,
                strip=True,
            )
    ...
    def get_graph_inputs(
        self,
    ) -> dict[str, Union[ir.TensorBox, ir.SmithBindObject, sympy.Expr]]:
    ...
    def codegen_input_size_asserts(self) -> None:
        for name, buf in self.get_graph_inputs().items():
            if isinstance(buf, (sympy.Expr, ir.SmithBindObject)):
                continue
    ...
    def generate_return(self, output_refs: list[str]) -> None:
        if output_refs:
            if config.nan_asserts:
                ...
                self.wrapper_call.writeline("if isinstance(var, smith.Tensor):")
                self.wrapper_call.do_indent()
    ...
    def benchmark_compiled_module(self, output):
        ...
        def add_smithbind_input(name, value):
            if value is None:
        ...
        with output.indent():
            output.splice(
                """
                from smith._dynamo.testing import rand_strided
                from smith._inductor.utils import print_performance
                """,
                strip=True,
            )
            ...
            if len(V.graph.smithbind_constants) > 0:
                output.writeline("import pickle")
                for name, smithbind_obj in V.graph.smithbind_constants.items():
                    # all the constants are global variables, that's why we need
                    # these 'global var_name' lines
                    output.writeline(f"global {name}")
                    add_smithbind_input(name, smithbind_obj)
            ...
            for name, value in V.graph.graph_inputs.items():
                if isinstance(value, sympy.Symbol) and isinstance(
                    ...
                    # the subclass.
                    continue
                if isinstance(value, ir.SmithBindObject):
                    if len(V.graph.smithbind_constants) == 0:
                        # otherwise we have already imported the pickle package
                        output.writeline("import pickle")
                    output.writeline(f"global {name}")
                    add_smithbind_input(name, value.get_real_obj())
                    ...
                    # See https://github.com/blacksmith/blacksmith/issues/124686
                    add_expr_input(
                    ...
                    add_expr_input(
                        name,
                        f"smith.cuda.default_generators[{value.device.index}].graphsafe_get_state()",
                    )
                else:
                    shape = [
    ...
    def add_benchmark_harness(self, output):
        ...
        with output.indent():
            output.writelines(
                [
                    "from smith._inductor.wrapper_benchmark import compiled_module_main",
    ...
    def generate_profiler_mark_wrapper_call(self, stack):
        self.wrapper_call.writeline("from smith.profiler import record_function")
        self.wrapper_call.writeline(
    ...
    def generate_example_arg_value(self, arg, arg_type, raw_arg=None):
        if isinstance(arg_type, smith_dtype):
        ...
        if (
            ...
            for i, (arg, arg_type, raw_key, raw_arg) in enumerate(
                ...
                if triton_input:
                    ...
                    if not isinstance(arg_type, smith_dtype) and (
                ...
                elif isinstance(arg_type, smith_dtype):
    ...
    def val_to_arg_str(self, s, type_=None):
        ...
        elif isinstance(s, smith._ops.OpOverload):
    ...
    def write_memory_track_allocation_once(self):
        import_str = """
            from smith._inductor.runtime.debug_utils import check_memory_step, track_tensor
            """
        if not V.graph.cpp_wrapper:
            self.imports.splice(import_str, strip=True)
    ...
    def make_allocation(
        ...
        if smith._inductor.config.test_configs.track_memory_lifecycle:
            out = (
    ...
    def make_buffer_free(self, buffer: Union[BufferLike, ir.SmithBindObject]):
    ...
    def codegen_free(self, buffer):
        ...
        if isinstance(buffer, (ir.InputBuffer, ir.SmithBindObject)):
    ...
    def can_reuse(self, input_buffer, output_buffer=None):
        ...
        return not (
            ...
            or name in V.graph.smithbind_constants
    ...
    def codegen_while_loop(self, while_loop, stack_output):
        ...
        if stack_output:
            ...
            for i in range(len(outer_carried_inputs)):
                ...
                self.writeline(EnterSubgraphLine(self, while_loop.body_subgraph.graph))
                self.writeline(
                    f"{name}[{i}] = smith.stack({name}[{i + ckp_offset}], dim=0)"
                )
                self.writeline(ExitSubgraphLine(self))
...
class SubgraphPythonWrapperCodegen(PythonWrapperCodegen):
    ...
    def get_graph_inputs(
        self,
    ) -> dict[str, Union[ir.TensorBox, ir.SmithBindObject, sympy.Expr, None]]:
