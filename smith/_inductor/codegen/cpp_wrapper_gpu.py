...
from smith._inductor.codecache import get_cpp_wrapper_cubin_path_name
...
class DeferredTritonCallWrapper:
    ...
    def generate(self, wrapper: CppWrapperGpu):
        ...
        template_types = [
            ...
            if isinstance(arg_type, (smith_dtype, UnwrapUnspecArg))
        ]
        if V.graph.aot_mode:
            ...
            with prefix.indent():
                ...
                for k, v in record_launch_kernel_args:
                    ...
                    prefix.writelines(
                        [
                            ...
                            f"aoti_smith_int64_to_ivalue({v}, &tmp_{arg_name});",
                ...
                def write_dummy_scalar_ivalue(arg_name):
                    ...
                    prefix.writelines(
                        [
                            ...
                            f"C10IValueHandle tmp_{arg_name};",
                            f"aoti_smith_int64_to_ivalue(0, &tmp_{arg_name});",
                            f"RAIIC10IValueHandle RAII_{arg_name}(tmp_{arg_name});",
                        ]
                    )
                    ...
                    if isinstance(
                        ...
                        prefix.writelines(
                            [
                                f"// Create c10::IValue for arg_{curr_arg_id}",
                                f"C10IValueHandle tmp_{arg_name};",
                                f"aoti_smith_tensor_to_ivalue({arg}, &tmp_{arg_name});",
                                f"RAIIC10IValueHandle RAII_{arg_name}(tmp_{arg_name});",
                            ]
                        )
                        # pyrefly: ignore [bad-argument-type]
                ...
                prefix.writelines(
                    [
                        ...
                        f"aoti_smith_strlist_to_ivalue({name_var}.data(), {len(ordered_argsname)}, &tmp_{name_var});",
...
class CppWrapperGpu(CppWrapperCpu):
    ...
    def write_get_raw_stream(self, device_idx: int, graph_name: str) -> str:
        ...
        self.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK({self.device_codegen.aoti_get_stream()}({device_idx}, (void**)&{name}));"
        )
        return name
    ...
    def codegen_inputs(self):
        ...
        if config.is_fbcode():
            ...
            # e.g. aoti_smith_clone_preserve_strides, have landed
            ...
            for idx in V.graph.inputs_to_check:
                ...
                self.prefix.splice(
                    ...
                    if ((reinterpret_cast<std::uintptr_t>({input_name}.data_ptr()) & ({GPU_ALIGN_BYTES} -1)) != 0) {{
                        AOTI_SMITH_WARN("{warn_msg}");
                        AtenTensorHandle {input_name}_aligned;
                        aoti_smith_clone_preserve_strides({input_name}, &{input_name}_aligned);
    ...
    def generate_args_decl(
        ...
        * returns: a string with the list of args, e.g. "var_0, var_1"
        ...
        def process_args(arg, arg_type, arg_signature=None):
            ...
            elif isinstance(arg_type, smith_dtype) and not signature_is_tma_desc(
                arg_signature
            ):
                ...
                and (
                    scratch := self.device_codegen.cpp_scratch(
                        ...
                        workspace=TritonScratchWorkspace(
                            size=workspace_size,
                            generate_dtype_str=(
                                lambda: self.codegen_dtype(smith.uint8)
                            ),
                        ),
                        prefix=scratch_name,
                    )
    ...
    def prepare_triton_wrapper_args(
        ...
        for arg, arg_type in zip(call_args, arg_types):
            if isinstance(arg, str):
                if isinstance(arg_type, smith_dtype) and should_unwrap_unspec_arg(arg):
    ...
    def make_zero_buffer(self, name):
        return f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_zero_({name}.get()));"
