...
from smith._inductor.autotune_process import TensorMeta
from smith._inductor.codegen.cutlass.cache import maybe_fetch_ops
...
from smith._inductor.select_algorithm import create_inputs_key
...
class CUTLASSGemmTemplate(CUTLASSTemplate, ABC):
    ...
    @staticmethod
    def cutlass_layout(smith_layout: ir.Layout) -> "Optional[cutlass_lib.LayoutType]":  # type: ignore[name-defined]  # noqa: F821
        """
        ...
        Args:
            smith_layout (ir.Layout): The layout that needs to be looked up.

        Returns:
            cutlass_lib.LayoutType: The converted layout corresponding to the `smith_layout` or None if no matching
            value is found.
        """
        assert cutlass_utils.try_import_cutlass()
        import cutlass_library.library as cutlass_lib

        if V.graph.sizevars.statically_known_equals(smith_layout.stride[-1], 1):
            return cutlass_lib.LayoutType.RowMajor
        elif V.graph.sizevars.statically_known_equals(smith_layout.stride[-2], 1):
    ...
    def layout_match(
        smith_layout: ir.Layout,
        cutlass_layout: "cutlass_lib.LayoutType",  # type: ignore[name-defined] # noqa: F821
    ) -> bool:
        """Helper Method: Determines whether a given smith layout matches a given Cutlass layout"""
        return CUTLASSGemmTemplate.cutlass_layout(smith_layout) == cutlass_layout

    @staticmethod
    def set_layout(tensor_desc: "TensorDescription", smith_layout: ir.Layout) -> None:  # type: ignore[name-defined]  # noqa: F821
        """
        Helper method: Sets the layout of a given tensor description to match the given smith layout
        """
        if CUTLASSGemmTemplate.layout_match(smith_layout, tensor_desc.layout):
            return
        tensor_desc.layout = CUTLASSGemmTemplate.cutlass_layout(smith_layout)

    @staticmethod
    def set_alignment(smith_layout, op_element) -> bool:
        """
        ...
        Args:
            smith_layout: The layout of the corresponding ir.Buffer node.
        ...
        """
        alignment = cutlass_utils.get_max_alignment(smith_layout)
    ...
    def fix_op_layout(
        ...
        if c_layout is not None:
            ...
            new_op.C.element = cutlass_utils.smith_dtype_to_cutlass_type(c_layout.dtype)
    ...
    def _dtype_match(
        ...
        accumulator_smith_dtype = cutlass_utils.get_accumulator_dtype(
        ...
        if not (
            ...
            and cutlass_utils.dtype_match(
                accumulator_smith_dtype, op.accumulator_type()
            )
        ):
            return False
    ...
    def global_filter_ops(
        ...
        """
        Filter ops without using information about the smith op, input nodes and output node.
        """
        assert cutlass_utils.try_import_cutlass()
        ...
        ops = [
            ...
            for op in ops
            if any(
                dtype_match(smith_dtype, op.accumulator_type())
                for smith_dtype in ACCUMULATOR_DTYPES
            )
        ]
        ...
        ops = [
            op
            for op in ops
            if any(dtype_match(smith_dtype, op.A.element) for smith_dtype in XW_DTYPES)
            and any(dtype_match(smith_dtype, op.B.element) for smith_dtype in XW_DTYPES)
        ]
        ...
        **kwargs,
        ...
        if epilogue_nodes or is_scaled_mm:
            if epilogue_nodes:
                ...
                op.D.element = cutlass_utils.smith_dtype_to_cutlass_type(
...
class CUTLASS3xGemmTemplate(CUTLASSGemmTemplate):
    ...
    def _are_inputs_layout_compatible(self, layouts: list[Layout]) -> bool:
        ...
        for broadcasting, as defined by Blacksmith operations like `smith.matmul`, `smith.aten.mm`,
    ...
    def _render_evt(
        ...
        acc_dtype = smith_dtype_to_cutlass_type(accumulator_dtype)
        output_dtype = smith_dtype_to_cutlass_type(output_dtype)
    ...
    def _set_bias_layout_and_alignment(
        ...
        if has_bias:
            ...
            # bias dtype
            op.C.element = cutlass_utils.smith_dtype_to_cutlass_type(
