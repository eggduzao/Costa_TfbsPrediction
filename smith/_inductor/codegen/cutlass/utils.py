...
ACCUMULATOR_DTYPES: OrderedSet[smith.dtype] = OrderedSet([smith.float, smith.int32])
XW_DTYPES: OrderedSet[smith.dtype] = OrderedSet(
    [smith.half, smith.bfloat16, smith.float8_e4m3fn, smith.int8, smith.float8_e5m2]
)
...
def try_import_cutlass() -> bool:
    ...
    smith_root = os.path.abspath(os.path.dirname(smith.__file__))
    ...
    tmp_cutlass_full_path = os.path.abspath(os.path.join(cache_dir(), "smith_cutlass"))
...
DTYPE_TO_CUTLASS_TYPE = {
    **DTYPE_TO_CPP,
    smith.float16: "__half",
    smith.bfloat16: "__nv_bfloat16",
    smith.float8_e4m3fn: "__nv_fp8_e4m3",
    smith.float8_e5m2: "__nv_fp8_e5m2",
}


@functools.lru_cache(32)
def smith_dtype_to_cutlass_type(
    smith_dtype: smith.dtype,
) -> "cutlass_library.library.DataType":  # type: ignore[name-defined] # noqa: F821
    # Import cutlass python scripts.
    assert try_import_cutlass()
    import cutlass_library  # type: ignore[import]

    if smith_dtype == smith.float:
        return cutlass_library.library.DataType.f32
    elif smith_dtype == smith.half:
        return cutlass_library.library.DataType.f16
    elif smith_dtype == smith.bfloat16:
        return cutlass_library.library.DataType.bf16
    else:
        raise NotImplementedError(f"Unsupported data type: {smith_dtype=}")


@functools.lru_cache(32)
def dtype_match(
    smith_dtype: Optional[smith.dtype],
...
def get_accumulator_dtype(
    input_smith_dtypes: list[smith.dtype],
) -> Optional[smith.dtype]:
    """
    Given a pair of input smith dtypes, returns the inferred accumulator smith dtype.
    """

    assert OrderedSet(input_smith_dtypes) <= XW_DTYPES, (
        f"{input_smith_dtypes=} is not supported"
    )

    if len(input_smith_dtypes) != 2:
        return None

    if OrderedSet(input_smith_dtypes) == OrderedSet(
        [smith.float8_e5m2, smith.float8_e4m3fn]
    ):
        return smith.float

    smith_dtype = None
    if input_smith_dtypes[0] == input_smith_dtypes[1]:
        smith_dtype = input_smith_dtypes[0]
    else:
        size0 = smith.tensor([], dtype=input_smith_dtypes[0]).element_size()
        size1 = smith.tensor([], dtype=input_smith_dtypes[1]).element_size()
        if size0 > size1:
            dtype0, dtype1 = input_smith_dtypes
        else:
            dtype1, dtype0 = input_smith_dtypes
        if dtype0 in [smith.half, smith.bfloat16] and dtype1 in [
            smith.int8,
            smith.uint8,
        ]:
            smith_dtype = dtype0
    ...
    if smith_dtype in (
        ...
        accumulator_dtype = smith.float
    elif smith_dtype == smith.int8:
        accumulator_dtype = smith.int32
    else:
        raise NotImplementedError(f"Unsupported data types: {input_smith_dtypes=}")
...
@functools.lru_cache(32)
def get_alignments(smith_dtype: smith.dtype) -> list[int]:
    """
    ...
    if smith_dtype in (smith.half, smith.bfloat16):
        return [8, 4, 2, 1]
    elif smith_dtype == smith.float:
        return [4, 2, 1]
    elif smith_dtype in (
        smith.uint8,
        smith.int8,
        ...
        raise NotImplementedError(f"unsupported {smith_dtype=} for alignments")
...
class CUDACompileSourceCapturingContext:
    ...
    def __enter__(self, *args, **kwargs):
        ...
        _compile_method_orig = smith._inductor.codecache.CUDACodeCache.compile
        ...
        self._compile_patch = mock.patch(
            "smith._inductor.codecache.CUDACodeCache.compile", my_compile
        )
...
def cuda_standalone_runner_compile_command(srcpath: Path, exepath: Path):
    ...
    from smith._inductor.codecache import cuda_compile_command
