...
from smith._inductor.select_algorithm import extern_kernels  # noqa: F401
from smith._inductor.utils import convert_to_symint
from smith._inductor.virtualized import V
from smith._library.triton import wrap_triton
...
from smith.utils._sympy.interp import _run_sympy_handler, sympy_interp
from smith.utils._sympy.reference import OptimizedPythonReferenceAnalysis
...
class SymbolBuffer(CodegenSymbol):
    ...
    def get_example(self) -> Union[smith.Tensor, smith.SymInt]:
        ...
        assert isinstance(sym_int, smith.SymInt)
        return sym_int
...
class WrapperFxCodegen(PythonWrapperCodegen):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.subgms: dict[str, smith.fx.GraphModule] = {}
...
class FxConverter:
    ...
    lines: list[Line]
    prologue: str
    graph_inputs: dict[str, Union[ir.TensorBox, ir.SmithBindObject, sympy.Expr, None]]
    graph_outputs: list[ir.IRNode]
    subgms: dict[str, smith.fx.GraphModule]
    is_subgraph: bool
    ...
    def __post_init__(self) -> None:
        ...
        self.tracer = smith.fx.proxy.GraphAppendingTracer(graph)
        self.expr_to_proxy: dict[sympy.Expr, smith.fx.Proxy] = {}
    ...
    def _record_allocation(self, buffer: CodegenBuffer, node: smith.fx.Node) -> None:
        """
    ...
    def _free(self, buffer: Union[CodegenBuffer, ir.SmithBindObject]) -> None:
        """
    ...
    def _generate_size_proxy(
        self, node: smith.fx.Node, expr: sympy.Expr
    ) -> smith.fx.Proxy:
        proxy = smith.fx.Proxy(node, tracer=self.tracer)
    ...
    def _generate_graph_input_shapes(self) -> None:
        ...
        def _codegen_symbol(
            ...
            base_node: smith.fx.Node,
            target: smith._ops.OpOverload,
            dim: int,
        ) -> None:
            def codegen_proxy() -> smith.fx.Proxy:
        ...
        for ir_node in self.graph_inputs.values():
            if isinstance(ir_node, ir.TensorBox):
                ...
                for dim, size in enumerate(ir_node.get_size()):
                    _codegen_symbol(
                        size, placeholder_node, smith.ops.aten.sym_size.int, dim
                    )
                for dim, stride in enumerate(ir_node.get_stride()):
                    _codegen_symbol(
                        stride, placeholder_node, smith.ops.aten.sym_stride.int, dim
                    )
    ...
    def _generate_buffer(self, node: ir.IRNode) -> Optional[smith.fx.Node]:
        """
    ...
    def _generate_outputs(
        self,
    ) -> Union[Optional[smith.fx.Node], list[Optional[smith.fx.Node]]]:
        """
    ...
    def _generate_subgm_getattrs(self) -> None:
        ...
        def generate_getattr(name: str, subgm: smith.fx.GraphModule) -> smith.fx.Node:
    ...
    def _get_subgm_attr(self, subgraph: ir.Subgraph) -> smith.fx.Node:
        """
    ...
    def _sympy_interp(self, expr: sympy.Expr) -> smith.fx.Proxy:
        # hash cons
    ...
    def _generate_sym_node(
        ...
        elif isinstance(s, smith.fx.Node):
            return s
    ...
    def _generate_conditional(self, line: WrapperLine) -> None:
        ...
        def get_subgm_attr(subgraph: Optional[ir.Subgraph]) -> smith.fx.Node:
        ...
        def generate_buffer(node: Optional[ir.IRNode]) -> Optional[smith.fx.Node]:
    ...
    def _generate_dynamic_scalar(self, line: WrapperLine) -> None:
        ...
        def generate_item(x: Optional[smith.fx.Node]) -> smith.fx.Node:
    ...
    def _generate_triton_call(self, line: WrapperLine) -> None:
        ...
        def tune_kernel(tuner: CachingAutotuner, call_args: Sequence[Any]) -> None:
            ...
            def node_to_tuning_arg(arg: Any) -> Any:
                ...
                def to_size_hint_list(arg: list[Union[smith.SymInt, int]]) -> list[int]:
                    args_sympy = [
                        x.node.expr if isinstance(x, smith.SymInt) else x for x in arg
                    ]
    ...
    def _generate_unbacked_symbol_defs(self, line: WrapperLine) -> None:
        ...
        def convert_key(node: smith.fx.Node, path: pytree.KeyPath) -> smith.fx.Node:
            """
