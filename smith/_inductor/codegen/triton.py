...
from smith._dynamo.utils import identity, preserve_rng_state
from smith._prims_common import is_integer_dtype
from smith.utils._ordered_set import OrderedSet
from smith.utils._sympy.functions import CeilDiv, FloorDiv, ModularIndexing
...
if TYPE_CHECKING:
    ...
    from smith._inductor.dtype_propagation import DtypePropagationOpsHandler
...
schedule_log = smith._logging.getArtifactLogger(__name__, "schedule")
fusion_log = smith._logging.getArtifactLogger(__name__, "fusion")
...
class OpDtypeSupport:
    ...
    supported_dtypes: dict[str, OrderedSet[smith.dtype]] = {}
    convert_outputs: dict[str, bool] = {}

    @classmethod
    def register_upcast(cls, func: Callable[..., str], convert_output: bool) -> None:
        op_name = func.__name__
        cls.supported_dtypes[op_name] = OrderedSet([smith.float32, smith.float64])
...
class TritonPrinter(PythonPrinter):
    ...
    def _print_Float(self, expr: sympy.Expr) -> str:
        if expr.is_integer:
            ...
            ret = str(int(expr))
        elif config.is_fbcode() and smith.version.hip:
            ret = f"{expr}"
        else:
...
def triton_compute_type(dtype: smith.dtype) -> str:
    """Convert smith.dtype to triton type and upcast [b]float16 to float32"""
    return triton_type(upcast_compute_type(dtype))


def triton_store_type(dtype: smith.dtype) -> str:
    """Convert smith.dtype to triton type, with fix for storing tl.bool"""
    if dtype == smith.bool:
        dtype = smith.int8
    return triton_type(dtype)


def upcast_acc_dtype(dtype: smith.dtype) -> smith.dtype:
...
def triton_acc_type(dtype: smith.dtype) -> str:
    """Convert smith.dtype to triton type, with reduction upcasts"""
    return triton_compute_type(upcast_acc_dtype(dtype))


def low_precision_fp(dtype: smith.dtype) -> bool:
...
def low_precision_fp_var(var: Union[CSEVariable, Any]) -> bool:
    ...
    dtype = var.dtype
    return low_precision_fp(dtype) if isinstance(dtype, smith.dtype) else False
...
def maybe_upcast_float32(convert_output: bool = True) -> Callable[[_T], _T]:
    ...
    def needs_upcast(var) -> bool:
        return (
            ...
            and var.dtype in (smith.float16, smith.bfloat16)
        )
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapped(*args, **kwargs) -> str:
            ...
            needs_downcast = result_dtype not in (smith.float32, None)
            downcast_string = (
...
class TritonOverrides(OpOverrides):
    ...
    def to_dtype(
        x,
        dtype: smith.dtype,
        src_dtype: Optional[smith.dtype] = None,
        use_compute_types=True,
    ):
        def _get_min_elements_per_thread(
            src_dtype: smith.dtype, dst_dtype: smith.dtype
        ) -> int:
            ...
            if src_dtype == smith.float8_e5m2 or dst_dtype == smith.float8_e5m2:
                return 4
            if src_dtype == smith.float8_e4m3fn or dst_dtype == smith.float8_e4m3fn:
                return 2
            # No requirements on min_elem_per_thread.
            return 0

        if src_dtype is not None:
            # Both dtype and src_dtype are set. This is used by smith to(dtype=dtype).
    ...
    @staticmethod
    def to_dtype_bitcast(x, dtype: smith.dtype, src_dtype: smith.dtype):
    ...
    def _shaped_constant(value, dtype, shape):
        type_ = smith._prims_common.dtype_to_type(dtype)
    ...
    def truediv(x, y):
        ...
        if (
            x_dtype == smith.float32
            and y_dtype == smith.float32
        ...
        if low_precision_fp_var(x) or low_precision_fp_var(y):
            ...
            if out_dtype in (smith.float16, smith.float32):
    ...
    def relu(x):
        ...
        if bug == "compile_error":
            ...
            return ops.maximum(ops.constant(0, smith.int32), x)
        else:
    ...
    def dot(a, b):
        ...
        def reshape_transpose_broadcast_for_dot(
            ...
            3. broadcast if needed with broadcast_to.
                - This shows up when matmul operand is broadcasted with smith.expand/repeat.
                - e.g., smith.rand((16,)).expand(16,16) @ B
    ...
    def inline_asm_elementwise(
        *inputs, asm, constraints=None, dtype=smith.float32, is_pure=True, pack=1
    ):
        triton_type = triton_compute_type(dtype)
    ...
    def floordiv(a, b):
        ...
        # Similar to div_floor_kernel_cuda in blacksmith core.
    ...
    @staticmethod
    def sign(x):
        z = ops.constant(0, smith.int32)
        left = ops.to_dtype((ops.lt(z, x)), smith.int8)
        right = ops.to_dtype((ops.lt(x, z)), smith.int8)
...
class TritonKernelOverrides(TritonOverrides):
    ...
    def _setup_libdevice_routing(cls):
        ...
        from smith._inductor.codegen.common import OpDecompositions

        for fn_name in smith._inductor.utils.op_requires_libdevice_fp64:
    ...
    def index_expr(cls, expr, dtype):
        ...
        index_dtype = V.kernel.get_index_dtype_as_smith_dtype()
        dtype = dtype if dtype not in (smith.int32, smith.int64) else index_dtype
        ...
        if dtype not in (smith.int32, smith.int64):
            var = V.kernel.cse.generate(
                V.kernel.compute,
    ...
    @staticmethod
    def masked(mask, body, other):
        if mask is not None and smith.version.hip is not None:
    ...
    def frexp(x):
        ...
        exponent = V.kernel.cse.newvar(dtype=smith.int32, shape=x.shape)
    ...
    def can_use_tma(
        ...
        if not (
            (
                ...
                and smith.cuda.get_device_capability()[0] >= 9
...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def dtype_to_str(self, dtype: smith.dtype) -> str:
        return triton_type(dtype)
        ...
        *,
        ...
        if need_dense and not have_dense:
            if self.inside_reduction and self.is_native_matmul:
                ...
                #       https://github.com/blacksmith/blacksmith/pull/152198
        ...
        if (
            ...
            if indexing.can_lift and var in self.prologue_cache:
                ...
                named_var = self.cse.namedvar(
                    block_descriptor, dtype=smith.uint64, shape=[]
                )
    ...
    def check_bounds(
        ...
        self.cse.generate(buffer, line, assignment=False, dtype=smith.int32)
    ...
    def _enable_pdl_codegen():
        if not smith._inductor.config.triton.enable_pdl:
            return False
        if isinstance(V.kernel, smith._inductor.select_algorithm.TritonTemplateKernel):
            return False
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        NOTE: enabled with env variable SMITHINDUCTOR_SKIP_L1
        """
        has_read_deps = True
        ...
        if should_unwrap_unspec_arg(name):
            ...
            if (
                ...
                line += ".to(tl.float32)"
                dtype = smith.float32
            if dtype == smith.bool and smith.version.hip is None:
    ...
    def bucketize(
        ...
        if indexing_dtype == smith.int32:
            triton_dtype = "tl.int32"
        elif indexing_dtype == smith.int64:
    ...
    def reduction_collapse_dims(
        self, buffer, value: CSEVariable, dtype: smith.dtype
    ) -> CSEVariable:
        """
    ...
    def reduction(
        ...
        def maybe_upcast(value: CSEVariable) -> CSEVariable:
            ...
            return (
                ops.to_dtype(value, smith.float32)
                if value.dtype
                in [
                    smith.float16,
                    smith.bfloat16,
        ...
        if any(x in [smith.float16, smith.bfloat16] for x in original_dtypes):
            # Only promote FB16/BF16; do not promote other integer/boolean dtypes
            src_dtype = smith.promote_types(src_dtype, smith.float32)
            dtype = smith.promote_types(dtype, smith.float32)
        ...
        # Say we have
        #     tmp0 = ops.constant(1, smith.int64)
        #     tmp1 = ops.reduction(smith.int64, smith.int64, "sum", tmp0)
        ...
        def final_reduction(
            buffer,
            value: CSEVariable,
            result_type: Optional[smith.dtype],
        ) -> tuple[str, Optional[smith.dtype], BlockShapeType]:
            """
        ...
        smith_acc_type = upcast_acc_dtype(src_dtype)
        result_shape = list(self.dense_size_list())
        result_shape[dim] = "1"
        result_var: Any = self.cse.newvar(
            dtype=smith_acc_type, shape=tuple(result_shape)
        )
        ...
        if self.persistent_reduction:
            ...
            def update_constant_dtype(constant, src_dtype, dst_dtype):
                ...
                if src_dtype.is_floating_point or src_dtype == smith.bool:
                    return constant

                if src_dtype == dst_dtype or constant == 0:
                    return constant

                if constant == smith.iinfo(src_dtype).max:
                    return smith.iinfo(dst_dtype).max
                elif constant == smith.iinfo(src_dtype).min:
                    return smith.iinfo(dst_dtype).min
                else:
                    return constant
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                accumulator_dtype = V.kernel.get_index_dtype_as_smith_dtype()
                if logical_index:
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                self.body.writeline(
                    ...
                    f"{smith.iinfo(index_dtype).max}, {self.dtype_to_str(index_dtype)})"
                )
                root_op = {"argmax": "max", "argmin": "min"}[reduction_type]
        ...
        if self.cooperative_reduction:
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                peer_idx = self.codegen_cooperative_reduction_peer_combine(
                    result_var, index_dtype, smith.iinfo(index_dtype).max
                )
    ...
    def _online_softmax_reduce(
        self, buffer, accumulator_max, accumulator_sum, dim, dtype: smith.dtype
    ):
    ...
    def _welford(self, buffer, mean, m2, weight, dim, dtype: smith.dtype):
        """
    ...
    def _lift_helper(
        self, fn, values: tuple[CSEVariable, ...], dtypes: tuple[smith.dtype, ...]
    ) -> str:
        ...
        from smith._inductor.dtype_propagation import DtypePropagationOpsHandler
        from smith._inductor.shape_propagation import ShapePropagationOpsHandler
    ...
    def codegen_body(self):
        ...
        if self.mix_order_reduction:
            ...
            for idx, partial_accum in enumerate(self.saved_partial_accumulate):
                ...
                default = ir.Reduction.default_accumulator(reduction_type, smith.float)
                default = self._map_tuple_or_scalar(constant_repr, default)
                name = f"accum{idx}"
                self.body.writeline(
                ...
                accumname2var[name] = self.cse.namedvar(
                    name, dtype=smith.float, shape=("1", "R0_BLOCK")
                )
            ...
            with self.body.indent(offset=1):
                ...
                for idx, partial_accum in enumerate(self.saved_partial_accumulate):
                    ...
                    combine_fn = ir.get_reduction_combine_fn(
                        partial_accum.reduction_type, smith.float
                    )
            ...
            for level, tree in enumerate(loop_trees):
                with self.body.indent(offset=level):
                    ...
                    if smith.version.hip and get_triton_version() > (3, 2):
        ...
        with result.indent():
            ...
            for arg_name, arg_sig in zip(call_args, signature):
                ...
                if buf:
                    ...
                    result.writeline(
                        f"{var_name} = smith.zeros({count}, device='{device}', dtype={arg_sig.dtype})"
                    )
                else:
                    raise KeyError(
    ...
    def gen_common_triton_imports(cls) -> str:
        ...
        imports.splice(
            """
            from smith._inductor.runtime import triton_helpers, triton_heuristics
            from smith._inductor.runtime.triton_helpers import libdevice, math as tl_math
            from smith._inductor.runtime.hints import AutotuneHint, ReductionHint, TileHint, DeviceProperties
            """
        )
    ...
    def inductor_meta_common(cls):
        inductor_meta = {
            "backend_hash": smith.utils._triton.triton_hash_with_backend(),
    ...
    def codegen_kernel(self, name=None) -> str:
        ...
        for tree in self.active_range_trees():
            ...
            # https://github.com/blacksmith/smithdynamo/pull/1362
        ...
        # https://github.com/blacksmith/blacksmith/issues/120478#issuecomment-1962822307
        ...
        for arg, arg_signature in zip(call_args, arg_signatures):
            if isinstance(arg_signature, TensorArg):
                if V.graph.cpp_wrapper:
                    wrapper.writeline(
                        f'AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_check_inf_and_nan("{arg}", {arg}));'
                    )
                else:
...
class TritonScheduling(SIMDScheduling):
    ...
    @classmethod
    def get_backend_features(cls, device: smith.device):
        if (
...
def debug_triton_code(node: BaseSchedulerNode) -> list[str]:
    ...
    if multi_template and multi_template.make_kernel_render is None:
        ...
        from smith._inductor.codegen.cuda_combined_scheduling import (
