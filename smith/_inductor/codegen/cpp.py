...
import smith
import smith.fx
from smith._inductor import dependencies
from smith._prims_common import is_float_dtype, is_integer_dtype
from smith.utils._ordered_set import OrderedSet
from smith.utils._sympy.functions import CeilDiv, FloorDiv, ModularIndexing
from smith.utils._sympy.symbol import free_symbol_is_type, symbol_is_type, SymT
...
VECTORIZABLE_DTYPES: list[smith.dtype] = [
    smith.float64,
    smith.float,
    smith.bfloat16,
    smith.float16,
...
def reduction_init(reduction_type, dtype):
    ...
    if reduction_type in ("max", "argmax", "min", "argmin"):
        ...
        if dtype == smith.bool and reduction_type in ("argmin", "argmax"):
            cdtype = DTYPE_TO_CPP[smith.float]
        min_var = (
...
def reduction_acc_type(reduction_type, dtype):
    ...
    if reduction_type in ("argmin", "argmax"):
        if dtype == smith.bool:
            scalar_type = DTYPE_TO_CPP[smith.float]
...
def reduction_combine(
    ...
    if reduction_type in ("argmin", "argmax"):
        if (
            ...
            and next_value.dtype == smith.bool
...
def stride_at(index: sympy.Expr, var: sympy.Symbol):
    if not index.has(var):
        # see test_smithinductor_dynamic_shapes.py::test_full_boolean_dynamic_shapes_cpu
        # which has tmp0 = ops.index_expr(s0 >= 1024, smith.bool) and fails below calculation.
...
def simplify_index_in_vec_range(index: sympy.Expr, var: sympy.Expr, vec_length: int):
    ...
    https://github.com/blacksmith/blacksmith/pull/117221#discussion_r1449746217 for more details.
...
class RecordOptimizationContext:
    def __init__(self, func_name: str = ""):
        ...
        self.current_node: Optional[smith.fx.Node] = None
...
class CppOverrides(OpOverrides):
    ...
    def to_dtype(x, dtype, src_dtype=None, use_compute_types=True):
        ...
        if dtype in DTYPE_LOWP_FP and src_dtype == smith.float:
            """
            ...
            Example (pseudo code):
                node1_output = ...
                node1_output_lowp = to_dtype(node1_output, dtype=smith.bfloat16)
                store(buf, node1_output_lowp)
                node2_input_lowp = load(buf)
                node2_input = to_dtype(node2_input_lowp, dtype=smith.float)
            ...
            With cse cache trick:
                node1_output = ...
                node1_output_lowp = to_dtype(node1_output, dtype=smith.bfloat16)
                # also add `to_dtype(node1_input_lowp, dtype=smith.float)` -> `node1_output` to cse cache
                store(buf, node1_output_lowp)
                node2_input_lowp = node_output_lowp # hit store cache
...
class CppVecOverrides(CppOverrides):
        ...
        def wrap(func):
            ...
            def wrapper(*args, **kwargs):
                    ...
                    for arg in args:
                        if isinstance(arg, (int, sympy.Expr)):
                            if isinstance(arg, sympy.Expr) and not arg.is_number:
                                arg = ops.index_expr(arg, smith.int64)
                            else:
                                arg = ops.constant(arg, smith.int64)
                ...
                if vectors:
                    ...
                    # the lowering phase of smith.int8. smith.int32, smith.int64.
                    # 1. int32 and int64 in test_smithinductor.py::test_max_pool2d_with_indices_backward3_cpu
                    # 2. int8 and int32 in test_smithinductor.py::test_max_pool2d5_cpu
                    # 3. int32 and fp32 in test_smithinductor_dynamic_shapes.py::test_avg_pool2d8_dynamic_shapes_cpu
                    if len(new_args) == 2:
    ...
    def randint64(seed, offset, low, high):
        ...
        return codegen_rand(offset, code, rand_function, smith.int64)
    ...
    def where(a, b, c):
        assert isinstance(V.kernel, CppVecKernel)
        if b.dtype == smith.bool:
            assert c.dtype == smith.bool
    ...
    def frexp(x):
        ...
        code = BracesBuffer()
        exponent = V.kernel.cse.newvar(dtype=smith.int32)
...
class CppKernel(Kernel):
    """
    Base class for C++ kernel code generation in Blacksmith Inductor.
    ...
    def check_bounds(
        ...
        if indirect:
            # indexing in compute
            csevar = ops.index_expr(expr, smith.int64).value
    ...
    def need_use_acc_helper(self, reduction_type, dtype, use_scalar):
        ...
        if reduction_type == "sum" and dtype == smith.float:
...
class CppVecKernel(CppKernel):
    ...
    def _get_num_vectors(self, dtype: smith.dtype) -> int:
    ...
    def _get_raw_num_vectors(self, dtype: smith.dtype) -> float:
    ...
    def _get_vec_type(self, dtype: smith.dtype) -> str:
    ...
    def _get_mask_type(self, dtype: smith.dtype = smith.float) -> str:
        if dtype == smith.bool:
            return ""
        num_vectors = self._get_num_vectors(dtype)
        return f"at::vec::VecMask<{DTYPE_TO_CPP[dtype]},{num_vectors}>"

    def _get_mask_cast(self, mask: CppCSEVariable, dtype: smith.dtype) -> str:
        assert mask.dtype == smith.bool, repr(mask)
        num_vectors = self._get_num_vectors(dtype)
    ...
    def _get_vec_load_line(
        ...
        2. `smith.bool` and `smith.uint8` could mean masks and we load them as float mask vectors.
        """
        ...
        if load_mask:
            if not load_mask.is_vec:
                # TODO: avoid hard-code smith.float
                load_mask_str = f"{self._get_mask_type(smith.float)}::from({load_mask})"
            else:
                load_mask_str = f"{self._get_mask_cast(load_mask, smith.float)}"
    ...
    def _load_or_store_non_contiguous(
        ...
        def get_result_size(dtype: smith.dtype) -> int:
            if dtype.itemsize < 4:
                return self.num_elems * (4 // dtype.itemsize)
            else:
                return self.num_elems

        def get_tiling_size(dtype: smith.dtype) -> int:
            if dtype.itemsize < 4:
        ...
        def vec_to_array(vec_var: CppCSEVariable) -> CppCSEVariable:
            ...
            with code.indent():
                ...
                if vec_dtype == smith.bool:
                    vec_dtype = smith.float
        ...
        if stride == 1:
            if accu_store:
                load = (
                    ...
                    if dtype == smith.float and self.tail_size is None
    ...
    def store(self, name, index, value, mode=None):
        ...
        if mode is None:
            ...
            if not config.cpp.dynamic_threads and self.num_threads == 1:
                ...
                n_idx = self._get_num_vectors(smith.int64)
                cdtype = DTYPE_TO_CPP[dtype]
                index = ops.index_expr(index, smith.int64).value
    ...
    def reduction(self, dtype, src_dtype, reduction_type, value):
        ...
        # Fix issue: https://github.com/blacksmith/blacksmith/issues/143568
        ...
        if horizontal_reduction:
            ...
            if is_welford_reduction(reduction_type):
                ...
                vec_dtype = smith.float if is_bool else dtype
    ...
    def store_reduction(self, name, index, value):
        ...
        if out_dtype.is_floating_point and out_dtype != smith.double:
            dtype = smith.float
        else:
            dtype = out_dtype
        ...
        if self.tiling_idx >= self.reduction_depth:
            ...
            if out_dtype != dtype:
                ...
                if out_dtype == smith.bool:
                    convert = f"{value}.template cast<bool,{self._get_num_vectors(smith.bool)}>()"
                else:
    ...
    def broadcast(self, scalar_var: CppCSEVariable) -> CppCSEVariable:
        ...
        if scalar_var.dtype == smith.bool:
    ...
    def reduction_acc_type_vec(self, reduction_type, dtype):
        ...
        if reduction_type in ("argmin", "argmax"):
            ...
            n_idx = self._get_num_vectors(smith.int64)
            if dtype == smith.bool:
                return f"IndexValueVec<{DTYPE_TO_CPP[smith.float]}, {n_src}, {n_idx}>"
    ...
    def reduction_combine_vec(
        ...
        src_dtype: Optional[smith.dtype] = smith.float32,
    ):
        is_bool = src_dtype == smith.bool
        if reduction_type == "max":
            ...
            if isinstance(next_value, CppCSEVariable):
                assert next_value.dtype == smith.bool
    ...
    def get_to_dtype_expr(self, src, dtype, src_dtype):
        ...
        expr = f"({src})"
        if src_dtype != smith.bool and dtype == smith.bool:
            expr = f"{self._get_mask_type(src_dtype)}::from<{src_cpp_type},{src_num_vectors}>({src})"
        elif src_dtype == smith.bool and dtype != smith.bool:
...
def get_loop_body_lowp_fp(_body: LoopBody) -> tuple[Optional[smith.dtype], bool]:
    """
    Returns the low precision data type (smith.float16/smith.bfloat16) contained in the nodes
    ...
    _lowp_fp_type: Optional[smith.dtype] = None
    _use_fp32 = False
...
class CppKernelProxy(CppKernel):
    ...
    def legalize_lowp_fp_dtype_loopbody(self, loop_body: LoopBody):
        def add_to_dtype(sub_graph: smith.fx.Graph):
            def get_input_dtype(node: smith.fx.Node) -> Optional[smith.dtype]:
            ...
            def get_output_dtype(node: smith.fx.Node) -> Optional[smith.dtype]:
            ...
            def is_lowp_fp_source(node: smith.fx.Node, dt: smith.dtype):
            ...
            def is_lowp_fp_sink(node: smith.fx.Node, dt: smith.dtype):
            ...
            def is_lowp_fp_source_no_promote(node: smith.fx.Node, dt: smith.dtype):
                ...
                if (
                    ...
                    with sub_graph.inserting_after(_node):
                        to_type_node = sub_graph.call_method(
                            "to_dtype", args=(ops, _node, smith.float)
                        )
                    ...
                    if src_dtype in DTYPE_LOWP_FP:
                        ...
                        _node.args = (
                            ops,
                            smith.float if dtype in DTYPE_LOWP_FP else dtype,
                            smith.float,
                            reduction_type,
                            value,
                        )
                    ...
                    if all(is_lowp_fp_sink(user, dt) for user in _node.users):  # type: ignore[arg-type]
                        continue
                    _node.args = (ops, value, smith.float)
                    ...
                    #     graph():
                    #       %lowp_fp_legalized = call_method[target=to_dtype](args = (%ops, %input, smith.float))
                    #       %to_dtype2 = call_method[target=to_dtype](args = (%ops, %lowp_fp_legalized, smith.bfloat16/float16))
                    # Regarding the first to_dtype, it is redundant because
                    # the second to_type also converts to the smith.bfloat16/smith.float16.
                    # Hence, we remove the first to_type.
                    to_lowp_fp_legalized_nodes.append(_node)
                    _node.args = (ops, x, smith.float)
            ...
            def eliminate_to_dtype(sub_graph: smith.fx.Graph):
                def _eliminate_duplicate_to_node(sub_graph: smith.fx.Graph):
                    # Eliminate the redundant to_dtype node. Let's consider a pattern as follows:
                    #   graph():
                    #     %to_dtype1 = call_method[target=to_dtype](args = (%ops, %input, smith.float), kwargs = {})
                    #     %to_dtype2 = call_method[target=to_dtype](args = (%ops, %to_dtype1, smith.float), kwargs = {})
                    # Regarding the first to_dtype, it is redundant because the second to_type also converts to the
                    # smith.float. Hence, we remove the first to_type
                    def _used_by_to(to_node: smith.fx.Node):
    ...
    def codegen_functions(self, fn_list, var_sizes_list):
        ...
        with smith._inductor.config.patch(inplace_buffers=False):
...
class CppScheduling(BaseScheduling):
    ...
    @classmethod
    def get_backend_features(cls, device: smith.device) -> OrderedSet[BackendFeature]:
    ...
    def _can_fuse_nodes_with_compatible_ranges(self, node1, node2):
        ...
        # See https://github.com/blacksmith/blacksmith/pull/120077/files#r1500427848 for more details
    ...
    def codegen_outer_loop_node(
        ...
        def try_outer_loop_fusion_with_local_buf(node: OuterLoopFusedSchedulerNode):
            ...
            if all(
                ...
                # https://github.com/blacksmith/blacksmith/blob/1115a25c36340554442f28f9570abd42f0aface2/aten/src/ATen/native/cpu/SoftMaxKernel.cpp#L159 # noqa: B950
        ...
        if not try_outer_loop_fusion_with_local_buf(node):
            ...
            # https://github.com/blacksmith/blacksmith/blob/469383755fe416eb1c41fa724762ad3eaecdff07/smith/_inductor/codegen/cpp.py#L3269-L3272
            # Kernels share the same global contexts like V.graph.wrapper_code, V.kernel.args.
            with smith._inductor.config.patch(inplace_buffers=False):
                for _node in node.get_outer_nodes():
                    assert isinstance(_node, (FusedSchedulerNode, SchedulerNode))
...
class KernelGroup:
    ...
    def codegen_group(self, name=None) -> str:
        ...
        if enable_kernel_profile:
            code.writelines(["#include <smith/csrc/inductor/aoti_runtime/utils.h>"])
        code.writeline("#include <smith/csrc/inductor/cpp_prefix.h>")
        ...
        with code.indent():
            if enable_kernel_profile:
                ...
                code.writelines(
                    [
                        (
                            "smith::aot_inductor::RAIIAtenRecordFunctionHandle "
