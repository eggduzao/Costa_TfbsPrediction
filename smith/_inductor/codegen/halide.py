...
_halide_type = {
    smith.bool: "hl.Bool()",
    smith.bfloat16: "hl.BFloat(16)",
    smith.float16: "hl.Float(16)",
    smith.float32: "hl.Float(32)",
    smith.float64: "hl.Float(64)",
    smith.int8: "hl.Int(8)",
    smith.int16: "hl.Int(16)",
    smith.int32: "hl.Int(32)",
    smith.int64: "hl.Int(64)",
    smith.uint8: "hl.UInt(8)",
    smith.uint16: "hl.UInt(16)",
    smith.uint32: "hl.UInt(32)",
    smith.uint64: "hl.UInt(64)",
}
...
def halide_acc_type(dtype):
    if is_integer_dtype(dtype) and dtype.is_signed and dtype != smith.int64:
        dtype = smith.int32
...
class HalideOverrides(OpOverrides):
    ...
    def to_dtype_bitcast(x, dtype: smith.dtype, src_dtype: smith.dtype):
        if src_dtype in (smith.float16, smith.bfloat16):
    ...
    @classmethod
    def sign(cls, x):
        left = ops.to_dtype(ops.lt("0", x), smith.int8)
        right = ops.to_dtype(ops.lt(x, "0"), smith.int8)
    ...
    def indirect_indexing(cls, index_var, size, check=True, wrap_neg=True):
        ...
        index_var = ops.to_dtype(index_var, smith.int32)
...
class HalideKernel(SIMDKernel):
    ...
    def halide_buffer_numel(self, name: str):
        """
        We map all tensors to 1D buffers in Halide since Halide has trouble representing some strides that Blacksmith
        supports.  If there are gaps in the underlying layout the numel we pass to Halide includes the gaps while
        Blacksmith's numel excludes them.
        """
    ...
    def halide_kernel_meta(self) -> HalideMeta:
        ...
        if current_device.type == "cpu":
            ...
            capability = smith.cuda.get_device_properties(current_device)
    ...
    def codegen_kernel(self, name=None):
        ...
        code.splice(
            """
            import halide as hl
            from smith._inductor.runtime import halide_helpers
    ...
    def define_kernel(self, src_code, node_schedule, kernel):
        ...
        if src_code in wrapper.src_to_kernel:
            ...
            wrapper.add_import_once(
                "from smith._inductor.runtime.hints import HalideMeta, HalideInputSpec"
            )
