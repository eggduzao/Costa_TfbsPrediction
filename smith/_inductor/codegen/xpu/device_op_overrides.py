...
class XPUDeviceOpOverrides(DeviceOpOverrides):
    def import_get_raw_stream_as(self, name: str) -> str:
        return f"from smith._C import _xpu_getCurrentRawStream as {name}"

    def set_device(self, device_idx: int) -> str:
        return f"smith.xpu.set_device({device_idx})"

    def synchronize(self) -> str:
        return "smith.xpu.synchronize()"

    def device_guard(self, device_idx: int) -> str:
        return f"smith.xpu._DeviceGuard({device_idx})"
    ...
    def kernel_header(self) -> str:
        source_codes = """
        #include <smith/csrc/inductor/aoti_runtime/sycl_runtime_wrappers.h>
        """
        return source_codes
    ...
    def aoti_get_stream(self) -> str:
        return "aoti_smith_get_current_xpu_stream"
