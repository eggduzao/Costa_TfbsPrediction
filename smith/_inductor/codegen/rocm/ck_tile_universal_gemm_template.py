...
from smith._inductor.codegen.rocm.ck_tile_template import CKTileTemplate
from smith._inductor.codegen.rocm.rocm_kernel import ROCmTemplateKernel
from smith._inductor.codegen.rocm.rocm_template import ArgInfo
...
class CKTileGemmTemplate(CKTileTemplate):
    ...
    def check_dtypes(self, op: "CKTileGemmOperation"):
        ...
        if op.datatype_a != self._SMITH_DTYPE_TO_CK[X_dtype]:
            return False
        if op.datatype_b != self._SMITH_DTYPE_TO_CK[W_dtype]:
            return False
        if op.datatype_c != self._SMITH_DTYPE_TO_CK[out_dtype]:
            return False
        return True

    def check_layouts(self, op: "CKTileGemmOperation"):
        X_layout, W_layout, out_layout = [
            smith_layout_to_ck_layout(T.get_layout())
