...
from smith._inductor.codegen.cpp_utils import DTYPE_TO_CPP
from smith._inductor.codegen.rocm.ck_template import CKTemplate
from smith._inductor.codegen.rocm.compile_command import rocm_compile_command
from smith._inductor.codegen.rocm.rocm_kernel import ROCmTemplateKernel
from smith._inductor.ir import Buffer, Layout
...
def smith_layout_to_ck_layout(smith_layout):
    if smith_layout.stride[-1] == 1:
        return "Row"
    elif smith_layout.stride[-2] == 1:
        return "Col"
    else:
        return None
...
class CKGemmTemplate(CKTemplate):
    ...
    int run_main(int argc, char** argv) {
        ...
        using AArgType = {{a_smith_dtype}};
        using BArgType = {{b_smith_dtype}};
        using CArgType = {{c_smith_dtype}};
        {% if has_bias %}
        using BiasArgType = {{bias_smith_dtype}};
        {% endif %}
        {% if has_scale %}
        using ScaleAArgType = {{scale_a_smith_dtype}};
        using ScaleBArgType = {{scale_b_smith_dtype}};
        {% endif %}
    ...
    def filter_op(self, op_info: InductorROCmOp):
        ...
        if op.a_element_dtype != self._SMITH_DTYPE_TO_CK[X_meta.dtype]:
            return None
        if op.b_element_dtype != self._SMITH_DTYPE_TO_CK[W_meta.dtype]:
            return None
        if op.c_element_dtype != self._SMITH_DTYPE_TO_CK[Y_meta.dtype]:
            return None
        # disable the instance if layouts don't match
        if op.a_layout != smith_layout_to_ck_layout(X_meta):
            return None
        if op.b_layout != smith_layout_to_ck_layout(W_meta):
            return None
        if op.c_layout != smith_layout_to_ck_layout(Y_meta):
            return None
            ...
            if op.block_gemm_pipeline_version != "BlockGemmPipelineVersion::v1":
                try:
                    prefetch_stages = self._prefetch_stages(
                        op,
                        smith.empty((), dtype=X_meta.dtype).element_size(),
                        smith.empty((), dtype=W_meta.dtype).element_size(),
                        smith.cuda.get_device_properties(X_meta.device).warp_size,
                    )
        ...
        **kwargs,
        ...
        if has_scale:
            ...
            if 1 == scale_x.get_numel() and 1 == scale_w.get_numel():
                ...
                op.ds_layouts = (
                    smith_layout_to_ck_layout(scale_x.get_layout()),
                    smith_layout_to_ck_layout(scale_w.get_layout()),
                )
                op.ds_element_dtypes = (
                    self._SMITH_DTYPE_TO_CK[scale_x.get_layout().dtype],
                    self._SMITH_DTYPE_TO_CK[scale_w.get_layout().dtype],
                )
        ...
        bias_dtype = ""
        if Bias is not None:
            bias_layout = smith_layout_to_ck_layout(Bias.get_layout())
            bias_dtype = self._SMITH_DTYPE_TO_CK[Bias.get_layout().dtype]
...
*
* {smith.__version__=}
* smith.version.git_version={getattr(smith.version, "git_version", "None")}
*/
"""
        epilogue = None
        ...
        if config.rocm.generate_test_runner:
            ...
            runner_code = self._template_from_string(
                ...
                a_smith_dtype=DTYPE_TO_CPP[X.get_layout().dtype],
                b_smith_dtype=DTYPE_TO_CPP[W.get_layout().dtype],
                c_smith_dtype=DTYPE_TO_CPP[Y.get_layout().dtype],
                bias_smith_dtype=DTYPE_TO_CPP[Bias.get_layout().dtype]
                if Bias is not None
                else "",
                scale_a_smith_dtype=DTYPE_TO_CPP[scale_x.get_layout().dtype]
                if scale_x is not None
                else "",
                scale_b_smith_dtype=DTYPE_TO_CPP[scale_w.get_layout().dtype]
                if scale_w is not None
                else "",
                a_layout=smith_layout_to_ck_layout(X.get_layout()),
                b_layout=smith_layout_to_ck_layout(W.get_layout()),
                c_layout=smith_layout_to_ck_layout(Y.get_layout()),
                bias_layout=smith_layout_to_ck_layout(Bias.get_layout())
                if Bias is not None
                else "",
    ...
    def _is_rcr_f16(self):
        ...
        X_dtype, W_dtype, Y_dtype = (
            self._SMITH_DTYPE_TO_CK[m.dtype] for m in (X_meta, W_meta, Y_meta)
        )
        X_layout, W_layout, Y_layout = (
            smith_layout_to_ck_layout(m) for m in (X_meta, W_meta, Y_meta)
        )
    ...
    def _get_kBatch(self, op):
        ...
        cus = smith.cuda.get_device_properties(X_meta.device).multi_processor_count
