...
def smith_layout_to_ck_layouts(smith_layout):
    # logically, smith tensors are always NCHW,
    ...
    if V.graph.sizevars.statically_known_equals(smith_layout.stride[-1], 1):
        ...
        # NB: smith.conv2d result is always NCHW
        return ["NGCHW", "GKCYX", "NGKHW"]
    elif V.graph.sizevars.statically_known_equals(smith_layout.stride[-3], 1):
...
def smith_layout_to_ck_input_layout(smith_layout):
    if V.graph.sizevars.statically_known_equals(smith_layout.stride[-1], 1):
        return "NGCHW"
    elif V.graph.sizevars.statically_known_equals(smith_layout.stride[-3], 1):
        return "NHWGC"
...
def smith_layout_to_ck_weight_layout(smith_layout):
    if V.graph.sizevars.statically_known_equals(smith_layout.stride[-1], 1):
        return "GKCYX"
    elif V.graph.sizevars.statically_known_equals(smith_layout.stride[-3], 1):
        return "GKYXC"
...
def smith_layout_to_ck_output_layout(smith_layout):
    if V.graph.sizevars.statically_known_equals(smith_layout.stride[-1], 1):
        return "NGKHW"
    elif V.graph.sizevars.statically_known_equals(smith_layout.stride[-3], 1):
        return "NHWGK"
...
"""
    ...
    def filter_op(self, op: "CKGroupedConvFwdOp"):  # type: ignore[name-defined]
        ...
        if op.b_element_dtype != self._SMITH_DTYPE_TO_CK[W_meta.dtype]:
            return None
        if op.e_element_dtype != self._SMITH_DTYPE_TO_CK[Y_meta.dtype]:
            return None
        # disable the instance if layouts don't match
        if op.a_layout != smith_layout_to_ck_input_layout(X_meta):
            return None
        if op.b_layout != smith_layout_to_ck_weight_layout(W_meta):
            return None
        if op.e_layout != smith_layout_to_ck_output_layout(Y_meta):
            return None
