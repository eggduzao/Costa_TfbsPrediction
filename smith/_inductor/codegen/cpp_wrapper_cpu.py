...
import smith
import smith._higher_order_ops.smithbind
import smith._inductor.async_compile  # noqa: F401 required to warm up AsyncCompile pools
import smith._ops
from smith._inductor.runtime.runtime_utils import dynamo_timed
from smith.fx.experimental.symbolic_shapes import ConvertIntKey, DivideByKey, SymTypes
...
class CppWrapperCpu(PythonWrapperCodegen):
    ...
    def get_device_include_path(device: str) -> str:
        if V.graph.aot_mode:
            return f"#include <smith/csrc/inductor/aoti_include/{device}.h>"
        return f"#include <smith/csrc/inductor/cpp_wrapper/{device}.h>"
    ...
    def add_device_include(self, device: str) -> None:
        ...
        extend_aoti_c_shim_include = (
            f"smith/csrc/inductor/aoti_smith/generated/extend/c_shim_{self.device}.h"
        )
    ...
    def write_header(self):
        ...
        if not V.graph.aot_mode:
            self.header.splice(
                """
                import smith
                from smith._inductor.codecache import CppWrapperCodeCache
        ...
        if config.cpp.enable_kernel_profile:
            self.header.splice(
                "#include <smith/csrc/inductor/aoti_runtime/kernel_context_tls.h>"
            )
            self.header.splice(
                """
    ...
    def write_prefix(self):
        ...
        if V.graph.aot_mode:
            self.prefix.writeline("namespace smith::aot_inductor {")
    ...
    def codegen_input_symbol_assignment(
        ...
        elif isinstance(value, ir.SmithBindObject):
            # smithbind objects are loaded in proxy executor
            pass
        else:
    ...
    def generate_input_output_runtime_checks(self):
        ...
        def gen_check(handle_kind, idx, name, tensor):
            ...
            self.prefix.splice(
                f"""
                    int32_t {name}_expected_dtype = aoti_smith_dtype_{dtype_str}();
            ...
            for dim_idx, d in enumerate(tensor.get_size()):
                if isinstance(d, (int, sympy.Integer)):
                    ...
                    from smith.utils._sympy.value_ranges import bound_sympy
    ...
    def write_wrapper_decl(self):
        ...
        with self.prefix.indent():
            ...
            assert all(
                isinstance(v, smith.Tensor) for v in list(V.graph.constants.values())
    ...
    def codegen_tensor_dtype_var_decl(self, code: IndentedBuffer, name):
        ...
        code.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_dtype({name}, &{name}_dtype));"
        )
    ...
    def codegen_input_device_type_var_decl(self, code: IndentedBuffer, name):
        ...
        code.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_get_device_type({name}, &{name}_device_type));"
        )
    ...
    def codegen_model_kernels(self):
        ...
        for name, kernel in self.initialized_kernels.items():
            ...
            self.prefix.writeline(f"    {signature} = smith::aot_inductor::{name};")
        self.prefix.writeline("};")
        self.prefix.writeline("}  // namespace\n\n")
    ...
    def codegen_model_constructor(self):
        ...
        inputs_info_[0].dtype = "smith.float16";
        ...
        ...
        outputs_info_[0].dtype = "smith.float16";
        }
        """
        ...
        with self.prefix.indent():
            ...
            for idx, name in enumerate(V.graph.constants.keys()):
                ...
                data_size = (
                    smith.ops.mkldnn._nbytes(tensor)
                    if tensor.is_mkldnn
                ...
                self.prefix.writeline(
                    f"constants_info_[{idx}].type = static_cast<int32_t>(smith::aot_inductor::ConstantType::{constant_type_str});"
                )
                ...
                if tensor.is_mkldnn:
                    opaque_metadata_tensor = smith.ops.mkldnn._get_mkldnn_serialized_md(
                        tensor
                    )
    ...
    def finalize_prefix(self):
        ...
        if V.graph.aot_mode and not V.graph.is_const_graph:
            aot_mode_decls.writeline("namespace smith::aot_inductor {")
            self.codegen_model_kernels()
            self.codegen_model_constructor()
            self.codegen_const_run_driver()
            aot_mode_decls.writeline("} // namespace smith::aot_inductor")
            aot_mode_decls.writeline("using namespace smith::aot_inductor;")

        self.prefix = cache_decls = IndentedBuffer()
        for dtype in self.used_cached_dtypes:
            cache_decls.writeline(f"CACHE_SMITH_DTYPE({dtype});")
        for device in self.used_cached_devices:
            cache_decls.writeline(f"CACHE_SMITH_DEVICE({device});")
        for layout in self.used_cached_layouts:
            cache_decls.writeline(f"CACHE_SMITH_LAYOUT({layout});")
        for memory_format in self.used_cached_memory_formats:
            cache_decls.writeline(f"CACHE_SMITH_MEMORY_FORMAT({memory_format});")
    ...
    def codegen_tensor_item(
        self, dtype: smith.dtype, tensor: str, scalar: str, indented_buffer=None
    ):
        ...
        if dtype == smith.float16 or dtype == smith.bfloat16:
            ...
            writer.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_item_{dtype_str}({tensor}, &{scalar_tmp}));"
            )
            writer.writeline(f"float {scalar} = float({scalar_tmp});")
        else:
            writer.writeline(f"{DTYPE_TO_CPP[dtype]} {scalar};")
            writer.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_item_{dtype_str}({tensor}, &{scalar}));"
            )
    ...
    def generate_return(self, output_refs: list[str]):
        ...
        for idx, output in enumerate(output_refs):
            ...
            if is_constant_buffer:
                ...
                self.wrapper_call.writeline(
                    f"aoti_smith_clone({output}, &output_handles[{idx}]);"
                )
            else:
    ...
    def generate_end(self, result):
        ...
        if V.graph.aot_mode:
            if V.graph.is_const_graph:
                ...
                result.writeline("} // namespace smith::aot_inductor\n\n\n")
            return
        ...
        wrapper_body = "input_tensors = [arg if isinstance(arg, smith.Tensor) else smith.tensor(arg, device='cpu') for arg in args]"
        ...
        wrapper_body += """
                    input_handles = smith._C._aoti.unsafe_alloc_void_ptrs_from_tensors(input_tensors)
        """
        ...
        wrapper_body += f"""
                    ...
                    output_tensors = smith._C._aoti.alloc_tensors_by_stealing_from_void_ptrs(output_handles)
                    return {outputs_str}
        """
    ...
    def get_c_shim_func_name(kernel: str, device: str) -> str:
        if kernel.startswith("aoti_smith_"):
            return kernel
        ...
        shim_fn = f"aoti_smith_{device}_{kernel_suffix}"
        return shim_fn
    ...
    def generate_c_shim_extern_kernel_call(
        ...
        *,
        ...
        with debug_printer_manager:
            ...
            shim_fn_codes = [
                f"AOTI_SMITH_ERROR_CODE_CHECK({shim_fn}({', '.join(args)}));"
            ]
    ...
    def generate_c_shim_extern_kernel_alloc(
        ...
        is_inplace = (
            isinstance(extern_kernel.op_overload, smith._ops.OpOverload)
            and smith.Tag.inplace_view in extern_kernel.op_overload.tags
        )
        ...
        if extern_kernel.python_kernel_name in (
            ...
            self.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_delete_tensor_object({output_handle_name}));"
            )
        elif not is_inplace:
    ...
    def _generate_index_put_fallback(self, kernel, x, indices, values, accumulate):
        # TODO: update aoti_smith_index_put_out in ir.py to use autogen out version
    ...
    def generate_inf_and_nan_checker(self, nodes):
        for buf in nodes.get_names():
            ...
            self.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_check_inf_and_nan({buf}));"
            )
    ...
    def codegen_device(self, device):
        ...
        return f"cached_smith_device_type_{device_str}, {device.index if device.index else 0}"

    def codegen_dtype(self, dtype):
        dtype_str = str(dtype).split(".")[-1]
        self.used_cached_dtypes.add(dtype_str)
        return f"cached_smith_dtype_{dtype_str}"

    def codegen_layout(self, layout):
        layout_str = str(layout).split(".")[-1]
        self.used_cached_layouts.add(layout_str)
        return f"cached_smith_layout_{layout_str}"
    ...
    def codegen_memory_format(self, memory_format):
        ...
        return f"cached_smith_memory_format_{memory_format_str}"
    ...
    def make_allocation(
        ...
        self.wrapper_call.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_empty_strided{pinned_str}({', '.join(args)}));"
        )
        ...
        if allocation_size != size:
            ...
            self.wrapper_call.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_as_strided({', '.join(args)}));"
            )
            self.wrapper_call.writeline(
    ...
    def codegen_alloc_from_pool(
        ...
        # If you write aoti_smith__alloc_from_pool lines, you must write the RAIIAtenTensorHandle
        # as well, otherwise you get memory leaks
        allocations_to_write = [
            f"AtenTensorHandle {tmp_name};",
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith__alloc_from_pool({', '.join(args)}));",
        ]
    ...
    def codegen_reinterpret_view(
        ...
        def create_dtypeview_call(reinterpret_call: str) -> tuple[str, list[str]]:
            ...
            dtypeview_function = f"aoti_smith_{device_name}_view_dtype"
            tmp_call_strs.append(
                f"AOTI_SMITH_ERROR_CODE_CHECK({dtypeview_function}"
        ...
        def create_new_tensor_handle() -> tuple[str, list[str]]:
            ...
            tmp_call_strs = [
                ...
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_new_tensor_handle({data.get_name()}, &{tmp_AtenTensorHandle}));",
            ]
            return f"RAIIAtenTensorHandle({tmp_AtenTensorHandle})", tmp_call_strs
        ...
        # aoti_smith_proxy_executor_call_function(..., tensor_args);
        # ````
        ...
        # ```
        # aoti_smith_proxy_executor_call_function(...,
    ...
    def codegen_device_copy(self, src, dst, non_blocking: Union[bool, str]):
        ...
        self.writeline(
            f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_copy_({dst}, {src}, {non_blocking}));"
        )
    ...
    def codegen_subgraph_prefix(self, subgraph, outer_inputs, outer_outputs):
        ...
        for (inner_input, inner_input_val), outer_input in zip(
            ...
            self.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_assign_tensors_out({outer_input}, &{inner_input}_handle));"
            )
    ...
    def codegen_while_loop(self, while_loop, stack_output=False):
        ...
        for inp, out in zip(outer_carried_inputs, while_loop.outputs):
            ...
            self.writeline(
                f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_assign_tensors_out({inp}, &{out_name}_handle));"
            )
        ...
        if is_bool_pred:
            ...
            self.codegen_tensor_item(smith.bool, cond_result_name, cond_result)
    ...
    def generate_extern_kernel_args_decl_if_needed(
        self,
        op_overload: Union[smith._ops.OpOverload, smith._ops.HigherOrderOperator],
        ...
        """
        schema = None
        if isinstance(op_overload, smith._higher_order_ops.smithbind.CallSmithBind):
            obj = raw_args[0]
            method = raw_args[1]
            schema = op_overload.schema(obj, method)
        else:
            assert isinstance(op_overload, smith._ops.OpOverload), type(op_overload)
        ...
        def fill_args(arg, arg_type):
            ...
            if isinstance(arg_type, smith.TensorType):
                assert isinstance(arg, inductor_tensor_buffers), f"got {type(arg)}"
                new_tensor_args.append(f"{arg.codegen_reference()}")
            elif isinstance(arg_type, smith.IntType):
                # int
                new_int_args.append(str(arg))
            elif isinstance(arg_type, smith.SymIntType):
                # SymInt
                expr = arg.node.expr if isinstance(arg, smith.SymInt) else arg
                new_int_args.append(cexpr(expr))
            elif isinstance(arg_type, smith.NumberType):
                # Scalar of type int
                assert isinstance(arg, (int, float, bool))
                # Only treat int Scalar as dynamic
                if isinstance(arg, int):
                    new_int_args.append(str(arg))
            elif isinstance(arg, ir.SmithBindObject):
                # smithbind objects are loaded in proxy executor
                pass
            elif isinstance(arg_type, smith.ListType):
                assert isinstance(arg, (list, tuple))

                # List[Tensor]
                if isinstance(arg_type.getElementType(), smith.TensorType):
                    new_tensor_args.extend([f"{a.codegen_reference()}" for a in arg])
                # List[Optional[Tensor]]
                elif isinstance(
                    arg_type.getElementType(), smith.OptionalType
                ) and isinstance(
                    arg_type.getElementType().getElementType(), smith.TensorType
                ):
                    new_tensor_args.extend(
                        [f"{a.codegen_reference()}" for a in arg if a is not None]
                    )
                # List[int]
                elif isinstance(arg_type.getElementType(), smith.IntType):
                    new_int_args.extend([str(a) for a in arg])
                # List[SymInt]
                elif isinstance(arg_type.getElementType(), smith.SymIntType):
                    expressions = [
                        a.node.expr if isinstance(a, smith.SymInt) else a for a in arg
                    ]
                    new_int_args.extend([cexpr(expr) for expr in expressions])
                # List[Scalar]
                elif isinstance(arg_type.getElementType(), smith.NumberType):
                    # Only treat int Scalar as dynamic
        ...
        def fill_output_arg(
            arg: str, return_type: smith.JitType, is_mutated_output: bool
        ) -> None:
            if isinstance(return_type, smith.TensorType):
                if not is_mutated_output:
                    self.writeline(f"AtenTensorHandle {arg}_handle;  // output buffer")
                    self.writeline(
                        f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_new_uninitialized_tensor(&{arg}_handle));"
                    )
                    self.writeline(f"RAIIAtenTensorHandle {arg}({arg}_handle);")
                new_tensor_args.append(f"{arg}")
            elif isinstance(return_type, smith.SymIntType):
                raise NotImplementedError("NYI support for return type: SymInt")
            elif isinstance(return_type, smith.ListType) and isinstance(
                return_type.getElementType(), smith.SymIntType
            ):
        ...
        for return_type in return_types:
            if isinstance(
                return_type, (smith.TensorType, smith.NoneType, smith.IntType)
            ):
                pass
            elif isinstance(return_type, smith.OptionalType):
                assert isinstance(return_type.getElementType(), smith.TensorType)
            elif isinstance(return_type, smith.ListType):
                assert isinstance(return_type.getElementType(), smith.TensorType)
            else:
                raise NotImplementedError(
    ...
    @staticmethod
    def _compatible_with_stableivalue(op: smith._ops.OpOverload) -> bool:
        ...
        supported_types = (
            ...
            # when queried via smith.JitType.type.
            smith.IntType,
            smith.TensorType,
        )

        def type_supported(t: smith.JitType) -> bool:
    ...
    def generate_fallback_kernel_with_runtime_lookup(
        ...
        if isinstance(op_overload, smith._ops.HigherOrderOperator):
            assert isinstance(
                op_overload, smith._higher_order_ops.smithbind.CallSmithBind
            ), type(op_overload)
        ...
        # In non-AOT mode, we use aoti_smith_call_dispatcher if all the inputs and
    ...
    def load_custom_op_wrapper(self):
        ...
        lines = """
RAIIPyObject codecache_module(PyImport_ImportModule("smith._inductor.codecache"));
if (!codecache_module) {
    throw std::runtime_error("Failed to load smith._inductor.codecache");
}
custom_op_wrapper = PyObject_GetAttrString(codecache_module, "custom_op_wrapper");
if (!custom_op_wrapper) {
    throw std::runtime_error("Failed to load smith._inductor.codecache.custom_op_wrapper");
}"""
    ...
    def generate_py_arg(self, py_args_var, idx, raw_arg, arg_type):
        def generate_py_arg_inner(lines, raw_arg, arg_type):
            def handle_scalar(scalar):
                ...
                if isinstance(scalar, SymTypes):
                    ...
                    if isinstance(scalar, smith.SymBool):
                        return f"PyBool_FromLong({scalar_var})"
                    if isinstance(scalar, smith.SymFloat):
            ...
            if raw_arg is None:
                ...
                return "Py_None"
            elif isinstance(arg_type, smith.TensorType):
            ...
            elif isinstance(arg_type, smith.OptionalType):
                ...
                # SymInt
                expr = (
                    raw_arg.node.expr if isinstance(raw_arg, smith.SymInt) else raw_arg
                )
                return f"PyLong_FromLongLong({cexpr(expr)})"
            elif isinstance(arg_type, smith.FloatType):
                return f"PyFloat_FromDouble({self.generate_float_value(raw_arg)})"
            elif isinstance(arg_type, smith.BoolType):
                return f"PyBool_FromLong({1 if raw_arg else 0})"
            elif isinstance(arg_type, smith.StringType):
                return f'PyUnicode_FromString("{raw_arg}")'
            elif isinstance(arg_type, smith.NumberType):
                # Union[bool, int, float, complex]
                # smith/_prims_common/__init__.py
                return handle_scalar(raw_arg)
            elif isinstance(raw_arg, smith.device):
            ...
            elif isinstance(raw_arg, smith.dtype):
                return f"Py_NewRef(smith::getTHPDtype(static_cast<c10::ScalarType>({self.codegen_dtype(raw_arg)})))"
            elif isinstance(raw_arg, smith.layout):
                return f"Py_NewRef(smith::getTHPLayout(static_cast<c10::Layout>({self.codegen_layout(raw_arg)})))"
            elif isinstance(raw_arg, smith.memory_format):
                return (
                    "Py_NewRef(smith::utils::getTHPMemoryFormat(static_cast<c10::MemoryFormat>("
        ...
        lines = []
        if isinstance(arg_type, smith.ListType):
    ...
    def generate_fallback_kernel_with_runtime_lookup_nopython(
        ...
        with dispatch_lines.indent():
            ...
            def parse_arg(arg_type: smith.JitType, codegen_arg: str) -> str:
                ...
                if isinstance(arg_type, smith.OptionalType):
                    ...
                    return f"smith::stable::detail::from({var_name})"
                ...
                if isinstance(arg_type, smith.TensorType):
                    if not temp_handle:
                        ...
                        dispatch_lines.writeline(
                            f"aoti_smith_new_tensor_handle({raii_var}, &{var_name});"
                        )
                        return f"smith::stable::detail::from({var_name})"
                    # If the RAII tensor _is_ a temporary scoped to this fallback call,
                    # simply release and steal the handle.
                    return f"smith::stable::detail::from({raii_var}.release())"
                return f"smith::stable::detail::from({codegen_arg})"
            ...
            dispatch_lines.writeline("AOTI_SMITH_ERROR_CODE_CHECK(")
            with dispatch_lines.indent():
                dispatch_lines.writeline(
                    f'aoti_smith_call_dispatcher("{op_overload._schema.name}", "{op_overload._schema.overload_name}", dispatch_vars.data())'  # noqa: B950
                )
            dispatch_lines.writeline(");")
            ...
            for idx, output_arg in enumerate(output_args):
                if output_arg is None:
                    continue
                dispatch_lines.writeline(
                    f"{output_arg} = smith::stable::detail::to<AtenTensorHandle>(dispatch_vars[{idx}]);"
                )
    ...
    def generate_fallback_kernel_with_runtime_lookup_aot(
        ...
        self.writeline(
            f"aoti_smith_proxy_executor_call_function(proxy_executor, "
    ...
    def c_type_for_prim_type(self, val, type_) -> str:
        if isinstance(type_, smith.OptionalType):
            return f"{self.c_type_for_prim_type(val, type_.getElementType())}*"
        elif isinstance(type_, smith.TensorType):
            return "AtenTensorHandle"
        elif isinstance(type_, (smith.IntType, smith.SymIntType)):
            return "int64_t"
        elif isinstance(
            type_, (smith.BoolType, smith.SymBoolType, smith.EnumType)
        ) or repr(type_) in ("Layout", "MemoryFormat", "ScalarType"):
            return "int32_t"
        elif isinstance(type_, smith.FloatType):
            return "double"
        elif isinstance(type_, smith.NumberType):
        ...
        elif isinstance(type_, smith.StringType):
            return "const char*"
        else:
    ...
    def val_to_arg_str_for_prim_type(self, val, type_) -> str:
        ...
        elif isinstance(val, smith.device):
            return self.codegen_device(val)
        elif isinstance(val, smith.dtype):
            return self.codegen_dtype(val)
        elif isinstance(val, smith.layout):
            return self.codegen_layout(val)
        elif isinstance(val, smith.memory_format):
            return self.codegen_memory_format(val)
        elif isinstance(val, float):
            return self.generate_float_value(val)
        elif isinstance(val, (list, tuple)):
            # FIXME: This happens because type_ is not always properly set to smith.ListType
    ...
    def val_to_arg_str(self, val, type_=None) -> str:
        if val is None:
            ...
            if type_ is None or isinstance(type_, smith.OptionalType):
            ...
            if isinstance(type_, smith.TensorType):
                # create an empty tensor, the equivalent of at::Tensor()
                var_name = f"var_{next(self.arg_var_id)}"
                self.writeline(f"AtenTensorHandle {var_name}_handle;")
                self.writeline(
                    f"AOTI_SMITH_ERROR_CODE_CHECK(aoti_smith_new_uninitialized_tensor(&{var_name}_handle));"
                )
        ...
        if isinstance(type_, smith.OptionalType):
            ...
            # memory usage.
            if isinstance(element_type, (smith.ListType, smith.TupleType)):
        ...
        if isinstance(type_, (smith.ListType, smith.TupleType)):
            ...
            if isinstance(element_type, smith.OptionalType) and not c_type.startswith(
                "const"
            ):
        ...
        if isinstance(type_, smith.TensorType) and val_is_scalar:
