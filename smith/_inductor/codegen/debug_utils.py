        ...
        for arg in input_args_to_print:
            if V.graph.cpp_wrapper:
                V.graph.wrapper_code.prefix.writeline(
                    f'aoti_smith_print_tensor_handle({arg}, "aoti_model_inputs - {arg}");'
                )
    ...
    def codegen_intermediate_tensor_value_save(
        ...
        for i, arg in enumerate(args_to_save):
            if arg_signatures is not None and not isinstance(
                arg_signatures[i], smith_dtype
            ):
                # infer from the arg data type (has smith.dtype) to see if it is a tensor type
                continue
            launch_prefix = "before_launch" if before_launch else "after_launch"
            if V.graph.cpp_wrapper:
                V.graph.wrapper_code.writeline(
                    f'aoti_smith_save_tensor_handle({arg}, "{arg}", "{launch_prefix}", "{kernel_name}");'
                )
            else:
                cwd = os.getcwd()
                ...
                line = f"smith.save({arg}, '{saved_path}')"
        ...
        for i, arg in enumerate(args_to_print):
            ...
            if V.graph.cpp_wrapper:
                if arg_signatures is not None and isinstance(
                    ...
                    # infer from the arg data type (has smith.dtype) to see if it is a tensor type
                    V.graph.wrapper_code.writeline(
                        f'aoti_smith_print_tensor_handle({arg}, "{launch_prefix} - {kernel_name} - {arg}");'
                    )
                elif arg_signatures is not None and isinstance(
                    arg_signatures[i],
                    (
                        type(smith._inductor.codegen.wrapper.SymbolicCallArg),
                        type(int),
                        type(float),
                        type(bool),
                    ),
