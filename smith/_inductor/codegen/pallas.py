...
import smith  # noqa: TC001
from smith.utils._ordered_set import OrderedSet
from smith.utils._pallas import has_tpu_pallas
from smith.utils._sympy.functions import ModularIndexing

from .. import config
from ..ir import ComputedBuffer
from ..runtime.runtime_utils import smith_dtype_to_jax
...
class PallasKernelOverrides(OpOverrides):
    ...
    @staticmethod
    def index_expr(expr: sympy.Expr, dtype: smith.dtype) -> str:
    ...
    @staticmethod
    def constant(val, dtype: smith.dtype) -> str:
    ...
    def gammainc(x: str, y: str) -> str:
        ...
        # Note: Blacksmith uses gammainc(input, other) where input is a (shape param)
    ...
    def rand(seed: str, offset: str) -> str:
        ...
        # Use float32 dtype to match Blacksmith's default
        return (
...
class PallasKernel(SIMDKernel):
    ...
    def _needs_strided_indexing(
        ...
        is_tpu = smith._inductor.config._debug_cpu_to_tpu_pallas
    ...
    def _handle_mixed_indexing(self, index: sympy.Expr) -> str:
        ...
        Blacksmith advanced indexing semantics: When multiple indirect indices have
        ...
        # In Blacksmith, when multiple advanced indices have the same shape, they pair up.
    ...
    def codegen_kernel(self, name: Optional[str] = None) -> str:  # type: ignore[override]
        ...
        - The main wrapper function that handles Blacksmith<->JAX conversions via DLPack
        ...
        if is_tpu:
            if not smith._inductor.config.pallas_take_first_jax_device_only:
                raise RuntimeError(
...
from smith._inductor.runtime.runtime_utils import pallas_partial_reduce, smith_dtype_to_jax_runtime
""" + (
        ...
        with kernel_body.indent():
            ...
            # These are used by index_expr operations like smith.arange
        ...
        with code.indent():
            ...
            if alias_params:
                code.writeline("# Convert Smith -> JAX for donated outputs")
                for alias_name in alias_params:
                    # TODO: The `jax.device_put` path is a temporary workaround for a Mosaic compiler bug
                    # that occurs with DLPack. Once SmithTPU provides a direct method for placing a
                    # `smith.Tensor` on a TPU device, this should be reverted to use the
            ...
            code.writeline("# Convert Smith -> JAX for in-place tensors")
            ...
            code.writeline("# Convert Smith -> JAX for inputs")
            for ptr in pointer_tail:
            ...
            code.writeline("# Prepare output metadata from Blacksmith tensor")
            code.writeline(
                "out_shapes = ("
            ...
            code.writeline(
                ...
                + ", ".join(
                    [
                        f"smith_dtype_to_jax_runtime({name}.dtype)"
            ...
            # Synchronize JAX computation to ensure results are visible to Blacksmith
            # This is needed because JAX and Blacksmith use different CUDA streams
            ...
            if copy_output_indices:
                ...
                for idx in copy_output_indices:
                    ...
                    if is_tpu:
                        ...
                        code.writeline(f"{name}.copy_(smith.from_dlpack(res_cpu))")
                    else:
                        code.writeline(
                            f"{name}.copy_(smith.from_dlpack(result_values[{idx}]))"
                        )
