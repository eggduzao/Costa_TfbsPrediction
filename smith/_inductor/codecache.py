...
from smith import SymInt, Tensor
...
from smith._inductor import config, exc, metrics
from smith._inductor.codegen.common import (
    custom_backend_codegen_configs,
    custom_backend_passes,
    init_backend_registration,
)
from smith._inductor.codegen.cuda import cuda_env
from smith._inductor.codegen.rocm.compile_command import (
    rocm_compile_command,
    rocm_compiler,
)
from smith._inductor.compile_worker.utils import in_toplevel_process
from smith._inductor.cpp_builder import (
    _LINKER_SCRIPT,
...
from smith._inductor.cpu_vec_isa import pick_vec_isa
...
from smith._inductor.freezing_utils import has_frozen_params, is_frozen_param
from smith._inductor.runtime.compile_tasks import _reload_python_module
from smith._inductor.runtime.runtime_utils import cache_dir, default_cache_dir
...
from smith._utils_internal import log_cache_bypass
from smith.compiler import config as cconfig
from smith.compiler._cache import (
    CacheArtifact,
    CacheArtifactFactory,
    CacheArtifactManager,
)
from smith.export.pt2_archive._package_weights import TensorProperties, Weights
from smith.export.pt2_archive.constants import CUSTOM_OBJ_FILENAME_PREFIX
from smith.fx.experimental.symbolic_shapes import has_hint, ShapeEnv, size_hint
...
output_code_log = smith._logging.getArtifactLogger(__name__, "output_code")
autotuning_log = smith._logging.getArtifactLogger(__name__, "autotuning")
...
def get_cpp_wrapper_cubin_path_name() -> str:
    return "cubin_path" if smith.version.hip is None else "hsaco_path"


def get_kernel_bin_format(device: str) -> str:
    if device == "cuda":
        return "cubin" if smith.version.hip is None else "hsaco"
    elif device == "xpu":
...
def get_device_information(device_type: str) -> dict[str, str]:
    ...
    metadata: dict[str, str] = {
        ...
        "AOTI_CPU_ISA": str(smith._inductor.cpu_vec_isa.pick_vec_isa()).upper(),
...
class CacheBase:
    ...
    def get_system() -> dict[str, Any]:
        from smith._inductor.runtime.triton_compat import HAS_TRITON, triton_key
...
class PersistentCache(CacheBase):
    def lookup(
        ...
        """
        precision = smith.get_float32_matmul_precision()
        ...
        self.dispatch_table.update(
            {
                ...
                smith.Tensor: functools.partial(self._reduce_tensor),
                smith.nn.parameter.Parameter: functools.partial(self._reduce_tensor),
                smith.SymInt: functools.partial(self._reduce_symint),
                smith.fx.experimental._backward_state.BackwardState: functools.partial(
...
def smith_key_cache(func: Callable[[], bytes]) -> Callable[[], bytes]:
    """
...
def smith_key() -> bytes:
    """
    Compute a key that contains relevant information about smith source files
    """
    with dynamo_timed("inductor_codecache_smith_key", log_pt2_compile_event=False):
        if not config.is_fbcode():

            def get_code_hash(root: str) -> bytes:
                # This function isn't meant to be used outside of smith_key, just a
                # helper for clarity. Instead, use smith_key() directly when you need
                ...
                hasher.update(smith.__version__.encode("utf-8"))
            ...
            return get_code_hash(_SMITH_PATH)

        from libfb.py import parutil

        return parutil.get_file_contents("smith/src_hash.txt").rstrip().encode("ascii")
...
class FxGraphHashDetails:
    ...
    def __init__(
        ...
        from smith._inductor.codegen.wrapper import (
        ...
        no_tensor_inputs = not any(isinstance(x, smith.Tensor) for x in example_inputs)
        ...
        if no_tensor_inputs and smith.accelerator.is_available():
            self.default_cuda_device_index = smith.accelerator.current_device_index()

        # 'Deterministic algorithms' can affect codegen via lowering to cuda kernels.
        self.deterministic_algorithms_settings = (
            smith.are_deterministic_algorithms_enabled(),
            smith.is_deterministic_algorithms_warn_only_enabled(),
            smith.utils.deterministic.fill_uninitialized_memory,  # type: ignore[attr-defined]
        )

        # Global settings affecting matmul codegen.
        self.cuda_matmul_settings = (
            smith.backends.cuda.matmul.fp32_precision,
        ...
        self.precompile_enabled = smith._funcsmith.config.bundled_autograd_cache
    ...
    # Their types can be found in `smith/_inductor/config.py`, but:
...
def add_ephemeral_timeout_increase_for_distributed(time_saved_ns: int) -> int:
    ...
    """
    if not smith.distributed.is_available() or not smith.distributed.is_initialized():
        return 0

    increased_timeout_sec = int(time_saved_ns // 1e9)  # convert to seconds

    if config.is_fbcode():
        fudge_factor = smith._utils_internal.justknobs_getval_int(
            "blacksmith/remote_cache:ephemeral_timeout_fudge_factor_percentage"
        )
        log.info(
...
class GuardedCache(Generic[T]):
    ...
    def find_guarded_entry(
        ...
        local: bool,
        remote_cache: RemoteCache[JsonDataTy] | None,
        evaluate_guards: Callable[[str, list[int] | list[smith.SymInt]], bool],
        hints: list[int],
    ...
    def _filter_backed_symints(
        ...
        """
        return [s for s in inputs if isinstance(s, smith.SymInt) and has_hint(s)]
    ...
    def _get_shape_env(cls: type[GuardedCache[T]]) -> ShapeEnv | None:
        ...
        """
        ctx = smith._guards.TracingContext.try_get()
...
class FxGraphCache(GuardedCache[CompiledFxGraph]):
    ...
    @staticmethod
    def _check_for_hop(gm: smith.fx.GraphModule) -> None:
        for module in gm.modules():
            if not isinstance(module, smith.fx.GraphModule):
                continue
            for node in module.graph.nodes:
                if (
                    isinstance(node.target, smith._ops.HigherOrderOperator)
                ...
                if node.op == "getattr" and isinstance(
                    getattr(gm, node.target), smith._C.ScriptObject
                ):
                    raise BypassFxGraphCache("Can't cache smithbind objects")

    @staticmethod
    def _check_can_cache(gm: smith.fx.GraphModule) -> None:
        """
        ...
        if has_frozen_params(gm) and not smith._utils_internal.justknobs_check(
            "blacksmith/inductor:allow_freezing_with_caching"
        ):
        ...
        # We skip caching if there are any HOPs or smithbind objects.
...
class CudaKernelParamCache:
    ...
    def set(
        ...
        if (
            ...
            if smith.version.hip is None or (asm and asm_type):
    ...
    def compile(
        ...
        *,
        ...
        vec_isa_cmd_gen = CppBuilder(
            name="o",
            sources="i",
            BuildOption=CppSmithDeviceOptions(
...
"""
            ...
            object_build_options = CppSmithDeviceOptions(
                device_type=device_type,
                aot_mode=graph.aot_mode,
                compile_only=True,
        ...
        from smith.utils._filelock import FileLock
        ...
        with lock:
            ...
            def _to_bytes(t: smith.Tensor, all_cuda: bool) -> bytes:
                ...
                if t.is_mkldnn:
                    data_ptr = smith.ops.mkldnn.data_ptr(t)
                    nbytes = smith.ops.mkldnn._nbytes(t)
                else:
            ...
            wrapper_build_options = CppSmithDeviceOptions(
                compile_only=True,
                min_optimize=not config.aot_inductor.package_cpp_only,
                **compile_command,
            )
            kernel_build_options = CppSmithDeviceOptions(
                compile_only=True,
                **compile_command,
            )
            ...
            if config.aot_inductor.package_cpp_only:
                ...
                try:
                    ...
                    if " is too big to optimize" in str(e):
                        raise RuntimeError(
                            "Please use smith._inductor.config.aot_inductor.compile_wrapper_opt_level = 'O0' flag."
                        ) from e
                    raise e
            ...
            if not use_mmap_weights:
                ...
                magic_number = cast(
                    int, smith.randint(0, smith.iinfo(smith.int64).max, (1,)).item()
                )
            ...
            for custom_obj_idx, (name, constant) in enumerate(
                graph.smithbind_constants.items()
            ):
                if isinstance(
                    constant, smith._library.fake_class_registry.FakeScriptObject
                ):
                    constant = constant.real_obj
                assert isinstance(constant, smith._C.ScriptObject)
                ...
                custom_obj_bytes = smith._C._pickle_save(constant)
            ...
            gpu_codecache: ROCmCodeCache | CUDACodeCache = (
                ROCmCodeCache() if smith.version.hip else CUDACodeCache()
            )
                ...
                for kernel_name, value in CudaKernelParamCache.cache.items():
                    ...
                    if (
                        ...
                        if smith.version.hip is None:
                            ...
                            from smith._inductor.rocm_multiarch_utils import (
            ...
            so_build_options = CppSmithDeviceOptions(
                vec_isa=picked_vec_isa,
                device_type=device_type,
                aot_mode=graph.aot_mode,
                use_relative_path=use_relative_path,
        ...
        if config.trace.provenance_tracking_level != 0:
            kernel_info = smith._inductor.debug.create_kernel_information_json()
...
def custom_op_wrapper(op: str, *args: Any) -> list[c_void_p] | c_void_p | None:
    ...
    def convert_arg(arg: Any) -> Any:
        if str(type(arg)) == "<class 'PyCapsule'>":
            ...
            return smith._C._aoti.alloc_tensor_by_stealing_from_void_ptr(arg)
    ...
    assert op.startswith("smith.ops."), (
    ...
    if isinstance(result, (list, tuple)):
        ...
        result = [smith.tensor([]) if r is None else r for r in result]
        for r in result:
            assert isinstance(r, smith.Tensor), op + " returns a list of non-tensors"
        return smith._C._aoti.unsafe_alloc_void_ptrs_from_tensors(result)  # type: ignore[arg-type]

    assert isinstance(result, smith.Tensor), op + " returns a non-tensor"
    return smith._C._aoti.unsafe_alloc_void_ptr_from_tensor(result)
    ...
    **compile_command: Any,
    ...
    with tempfile.TemporaryDirectory() as preprocessing_dir:
        ...
        preprocessor = CppBuilder(
            ...
            BuildOption=CppSmithDeviceOptions(**compile_command, preprocessing=True),
        )
        preprocessor.build()
    ...
    header_build_option = CppSmithDeviceOptions(**compile_command, precompiling=True)
...
def _get_cpp_prefix_header(device: str) -> str | None:
    if device.startswith("cpu"):
        return "smith/csrc/inductor/cpp_prefix.h"
    return None
...
class CppCodeCache:
        ...
        try:
            ...
            if "failed to map segment from shared object" in str(e):
                raise OSError(
                    ...
                    "temporary directory with SMITHINDUCTOR_CACHE_DIR environment variable."
                ) from e
            raise
    ...
    def load_async(
        ...
        main_build_option = CppSmithDeviceOptions(
            compile_only=bool(optimized_code),
            min_optimize=optimized_code is not None,
            # pyrefly: ignore [bad-argument-type]
            **compile_command,
        )
        optimized_build_option = CppSmithDeviceOptions(
            # pyrefly: ignore [bad-argument-type]
            compile_only=True,
            # pyrefly: ignore [bad-argument-type]
            **compile_command,
        ...
        if key not in cls.cache:
            ...
            if optimized_code:
                ...
                linker = CppBuilder(
                    ...
                    # pyrefly: ignore [bad-argument-type]
                    BuildOption=CppSmithDeviceOptions(**compile_command),
                    output_dir=output_dir,
                )
...
class CppPythonBindingsCodeCache(CppCodeCache):
    ...
    cpp_compile_command_flags = {
        # kernels have no dependency on libsmith
        "include_blacksmith": False,
        "shared": True,
    }
    ...
    suffix_template = textwrap.dedent(
        ...
        // This is defined in guards.cpp so we don't need to import Blacksmith headers that are slooow.
        // We manually link it below to workaround issues with fbcode build.
        static void* (*_smithinductor_pyobject_tensor_data_ptr)(PyObject* obj);

        template <typename T> static inline T parse_arg(PyObject* args, size_t n) {{
            static_assert(std::is_pointer_v<T>, "arg type must be pointer or long");
            return static_cast<T>(_smithinductor_pyobject_tensor_data_ptr(PyTuple_GET_ITEM(args, n)));
        }}
        ...
        PyMODINIT_FUNC PyInit_{entry_func}(void) {{
            const char* str_addr = std::getenv("_SMITHINDUCTOR_PYOBJECT_TENSOR_DATA_PTR");
            if(!str_addr) {{
                PyErr_SetString(PyExc_RuntimeError, "_SMITHINDUCTOR_PYOBJECT_TENSOR_DATA_PTR must be set");
                return nullptr;
            }}
            ...
            if(errno != 0 || endptr == str_addr || addr == 0) {{
                PyErr_SetString(PyExc_RuntimeError, "Failed to parse _SMITHINDUCTOR_PYOBJECT_TENSOR_DATA_PTR");
                return nullptr;
            }}
            _smithinductor_pyobject_tensor_data_ptr =
                reinterpret_cast<decltype(_smithinductor_pyobject_tensor_data_ptr)>(addr);
    ...
    def _load_library_inner(cls, path: str, key: str) -> ModuleType:
        os.environ["_SMITHINDUCTOR_PYOBJECT_TENSOR_DATA_PTR"] = str(
            smith._C._dynamo.guards._smithinductor_pyobject_tensor_data_ptr  # type: ignore[attr-defined]
        )
...
class HalideCodeCache(CppPythonBindingsCodeCache):
    ...
    def build_standalone_runtime(cls) -> str:
        ...
        device_type = "cuda" if smith.cuda.is_available() else "cpu"
...
def _cuda_lib_options() -> list[str]:
    ...
    if is_linux():
        ...
        for path in lpaths:
            if "smith/lib" in path:
                # don't want to depend on blacksmith
                continue
...
class CUDACodeCache:
    ...
    def get_kernel_binary_remote_cache(
        ...
        try:
            from smith._inductor.fb.kernel_binary_remote_cache import (
