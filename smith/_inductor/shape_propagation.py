...
ShapeArg = Union[ShapeVar, smith.types.Number, str, OpsValue, smith.dtype]
...
def broadcast_shapes_for_args(args: Sequence[ShapeArg]) -> BlockShapeType:
    ...
    for arg in args:
        if hasattr(arg, "shape"):
            ...
            from smith._inductor.loop_body import LoopBody, LoopBodyBlock
...
class ShapePropagationOpsHandler:
    ...
    @staticmethod
    def constant(value: smith.types.Number, dtype: smith.dtype) -> BlockShapeType:
        # TritonKernelOverrides.constant uses tl.full with shape=[1]*ndim for all types
        from smith._inductor.codegen.triton import TritonKernel
    ...
    @staticmethod
    def index_expr(expr: sympy.Expr, dtype: smith.dtype) -> BlockShapeType:
