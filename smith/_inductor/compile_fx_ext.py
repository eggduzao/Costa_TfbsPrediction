...
import smith.fx
from smith._inductor.codecache import BypassFxGraphCache, FxGraphCache
from smith._inductor.metrics import CachedMetricsDeltas, CachedMetricsHelper
...
class _VirtualizedSerializer:
    ...
    def serialize(cls) -> _VirtualizedSerializer:
        """
        Turn the current state of smith._inductor.virtualized.V into a
    ...
    def __enter__(self) -> Self:
        ...
        for set_name in dir(V):
            ...
            if hasattr(self.virtualized, name):
                ...
                value = smith._inductor.virtualized._PoisonedVirtual
...
def _current_fake_mode() -> FakeTensorMode:
    fake_mode = None
    if context := smith._guards.TracingContext.try_get():
    ...
    def __init__(self) -> None:
        ...
        def filter(
            ...
            if not isinstance(logger, logging.Logger):
                ...
                return False
            # We only want to track smith._inductor logging
            if not logger.name.startswith("smith._inductor"):
                return False
        ...
        root = logging.getLogger("smith._inductor")
...
class _SerializedFxCompile(FxCompile):
    ...
    def serialize_compile(
        ...
        try:
            deterministic_guard_for_testing = (
                smith.testing._internal.common_utils.DeterministicGuard._current_state()  # type: ignore[attr-defined]  # mypy bug
            )
        except AttributeError:
            pass
    ...
    def _run_in_child(
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(smith._guards.tracing(input.tracing_context))
...
#     idx = 0
#     with open(f"/tmp/blacksmith_compile_fx_tmp_input_{idx}.bin", "rb") as f:
#         input = compile_fx._WireProtocolPickledInput(f.read())
#     result = compile_fx._SubprocessFxCompile._run_in_child(input)
#     with open(f"/tmp/blacksmith_compile_fx_tmp_output_{idx}.bin", "wb") as f:
...
class _DebugFileFxCompile(_SerializedFxCompile):
    ...
    def _send_to_child(
        ...
        name = os.path.join(
            tempfile.gettempdir(), f"blacksmith_compile_fx_tmp_input_{idx}.bin"
        )
        ...
        if False:
            name = os.path.join(
                tempfile.gettempdir(), f"blacksmith_compile_fx_tmp_actual_{idx}.bin"
            )
            ...
            name = os.path.join(
                tempfile.gettempdir(), f"blacksmith_compile_fx_tmp_output_{idx}.bin"
            )
