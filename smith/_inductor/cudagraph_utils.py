...
import smith
from smith._dynamo.utils import counters, get_metrics_context
from smith._inductor.utils import GraphPartitionMap, InputType
from smith._subclasses.fake_tensor import get_plain_tensors, is_fake
...
OutputType = list[Optional[Union[int, smith.Tensor]]]
...
class PlaceholderInfo:
    """
    A serializable version of smith.fx.Node that contains information
...
def get_mutating_use_stack_trace_from_node(
    placeholder_node: smith.fx.Node,
) -> Optional[str]:
    ...
    for use in placeholder_node.users:
        if use.target is smith.ops.aten.copy_.default:
...
def to_placeholder_info(placeholder_node: smith.fx.Node) -> PlaceholderInfo:
...
def get_placeholder_info(graph: smith.fx.Graph) -> list[PlaceholderInfo]:
    return [
...
def check_for_mutation(
    ...
    is_cuda_graph_recorded_tensor: Callable[[smith.Tensor], bool],
) -> Optional[str]:
    # doesn't work for non-trees because the warmup run would apply mutation twice
    if smith._inductor.config.triton.cudagraph_trees:
...
def _get_use_stack_trace(node: smith.fx.Node) -> Optional[str]:
...
def check_multiple_devices_or_any_cpu_nodes(
    ...
    device_node_mapping.pop(smith.device("meta"), None)
    ...
    if is_using_cudagraph_partition():
        ...
        device_node_mapping.pop(smith.device("cpu"), None)

    if cpu_node := device_node_mapping.get(smith.device("cpu")):
...
def log_cudagraph_skip_and_bump_counter(msg: str) -> None:
    ...
    if smith._inductor.config.triton.cudagraph_or_error:
...
def maybe_warning_due_to_dynamic_shape(
    ...
    if (
        smith._inductor.config.triton.cudagraph_dynamic_shape_warn_limit
        and num_cudagraphs
        > smith._inductor.config.triton.cudagraph_dynamic_shape_warn_limit
    ):
