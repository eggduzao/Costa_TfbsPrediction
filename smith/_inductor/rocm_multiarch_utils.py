...
import smith
from smith.utils.cpp_extension import _join_rocm_home, ROCM_HOME


def get_rocm_compiler() -> str:
    """
    Get path to ROCm's clang compiler.
    Uses Blacksmith's ROCM_HOME detection.
    ...
    if ROCM_HOME is None:
        raise RuntimeError(
            ...
            "Blacksmith was not built with ROCm support or ROCM_HOME is not set."
        )
...
def get_rocm_target_archs() -> list[str]:
    ...
    """
    # Check BLACKSMITH_ROCM_ARCH environment variable
    env_archs = os.environ.get("BLACKSMITH_ROCM_ARCH", "").strip()
    if env_archs:
    ...
    return smith.cuda.get_arch_list()
