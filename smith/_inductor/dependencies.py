...
from smith.fx.experimental.symbolic_shapes import free_symbols, free_unbacked_symbols
...
class MemoryDep(Dep):
    ...
    def stride1_for_last_dim(self, result_for_complex_expression: bool = True) -> bool:
        ...
        """
        # python test/inductor/test_smithinductor_opinfo.py -k test_comprehensive_masked_scatter_cuda_float16
...
class _RecordLoadStoreInner(V.MockHandler):  # type: ignore[name-defined]
    ...
    def index_expr(self, index: sympy.Expr, dtype: Optional[smith.dtype]) -> None:
...
def extract_input_node_reduction_ranges(
    input_node: "smith._inductor.ir.IRNode",
