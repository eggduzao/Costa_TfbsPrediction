...
from smith._dynamo.repro.after_aot import wrap_compiler_debug
...
from smith._inductor.codecache import code_hash, FxGraphCache, output_code_log
from smith._inductor.cudagraph_utils import (
    BoxedDeviceIndex,
...
from smith._inductor.debug import (
    create_mapping_pre_post_grad_nodes,
    save_args_for_compile_fx_inner,
)
from smith._inductor.output_code import (
    CompiledAOTI,
    CompiledFxGraph,
...
from smith._inductor.runtime.cache_dir_utils import cache_dir
...
from smith._utils_internal import compile_time_strobelight_meta
from smith.fx import GraphModule
from smith.fx.experimental.symbolic_shapes import free_unbacked_symbols, SymExprPrinter
...
if TYPE_CHECKING:
    ...
    from smith._inductor.output_code import _StrideExprStr
    from smith._ops import OpOverload
    from smith.export.pt2_archive._package_weights import Weights
...
if TYPE_CHECKING or not config.is_fbcode():
    ...
    from smith._inductor.fb.utils import log_optimus_to_scuba, time_and_log

if TYPE_CHECKING:
    import types

    from smith._funcsmith._aot_autograd.schemas import (
        FQN,
        GraphInputName,
        GraphSignature,
    )
    ...
    CompileFxOutput = Union[
        Callable[[list[object]], Sequence[smith.Tensor]],
        str,
        list[str],
        Weights,
    ]
...
def _fx_compile_mode_default() -> FxCompileConfig:
    name = "SMITHINDUCTOR_FX_COMPILE_MODE"
...
pre_grad_graphs_log = smith._logging.getArtifactLogger(__name__, "pre_grad_graphs")
post_grad_graphs_log = smith._logging.getArtifactLogger(__name__, "post_grad_graphs")
static_inputs_log = smith._logging.getArtifactLogger(
    __name__, "cudagraph_static_inputs"
)
inductor_metrics_log = smith._logging.getArtifactLogger(__name__, "inductor_metrics")


def get_static_input_idxs(num_fixed: int) -> list[int]:
    # If we are inlining NNModules, we treat all smith.nn.Parameters as static for the purposes
...
def record_original_output_strides(gm: GraphModule) -> None:
    ...
    for output in output_node_args:
        if (
            isinstance(output, smith.fx.Node)
            and (val := output.meta.get("val")) is not None
            and isinstance(val, smith.Tensor)
        ):
...
def _recursive_record_original_output_strides(gm: GraphModule) -> None:
    ...
    for node in gm.graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.invoke_subgraph
    ):
    ...
    for node in gm.graph.find_nodes(
        ...
        for node in subgraph.graph.find_nodes(op="output"):
            node.meta["user_visible_output_idxs"] = [
                ...
                if isinstance(node.args[0][idx], smith.fx.Node)
            ]
...
def _warn_tf32_disabled() -> None:
    if (
        ...
        and smith.backends.cuda.matmul.fp32_precision != "tf32"
        and smith.cuda.get_device_capability() >= (8, 0)
    ):
        warnings.warn(
            "TensorFloat32 tensor cores for float32 matrix multiplication available but not enabled. "
            "Consider setting `smith.set_float32_matmul_precision('high')` for better performance."
        )
...
def _resolve_name_collision(mod: GraphModule, gm: GraphModule) -> None:
    ...
    "_tensor_constant" and "_smithbind_obj". See Tracer.create_arg() in
    smith/fx/_symbolic_trace.py
    ...
    for node in gm.graph.nodes:
        if node.op == "get_attr":
            ...
            if not target_name.startswith(
                "_tensor_constant"
            ) and not target_name.startswith("_smithbind_obj"):
                continue
            ...
            if isinstance(gm_target, FakeScriptObject):
                ...
                and smith.equal(gm_target, model_target)
            ):
                # If tensors with same name from gm and model are indeed the same, we don't need to rename
                # Check device first, to avoid smith.equal(wrapper_CUDA__equal) raise when different device
                continue
...
def _unlift_graph(
    ...
    from smith.export.unflatten import _assign_attr, _AttrKind

    _resolve_name_collision(mod, gm)

    state_dict: dict[str, Union[smith.nn.parameter.Parameter, smith.Tensor]] = {}
    ...
    from smith.export._unlift import _unlift

    outputs: tuple[smith.fx.Node, ...] = tuple(gm.graph.output_node().args[0])  # type: ignore[arg-type]
    mutated_outputs = []
...
def _get_subgraph_names(
    ...
    for child_name, child_module in gm.named_children():
        ...
        if child_name in all_subgraph_names and isinstance(
            child_module, smith.fx.GraphModule
        ):
...
def split_const_gm(
    ...
    skip_folding_node_fn: Optional[Callable[[smith.fx.Node], bool]] = None,
...
def is_tf32_warning_applicable(gm: GraphModule) -> bool:
    ...
    for target in tf32_ops:
        for node in gm.graph.find_nodes(op="call_function", target=target):
            if (
                isinstance(node.meta.get("val", None), smith.Tensor)
                and node.meta["val"].dtype == smith.float32
...
def maybe_disable_comprehensive_padding(
    ...
    """
    has_gpu = any(
        is_gpu(t.device.type) for t in example_inputs if isinstance(t, smith.Tensor)
    )
...
) -> smith._subclasses.FakeTensorMode:
    """
...
def compile_fx_inner(
    ...
    **kwargs: Unpack[_CompileFxKwargs],
    ...
    with contextlib.ExitStack() as stack:
        stack.enter_context(smith.utils._python_dispatch._disable_current_modes())
...
def _compile_fx_inner(
    ...
    **graph_kwargs: Unpack[_CompileFxKwargs],
    ...
    if config.pipeline_max_autotune_gemm:
        ...
        from smith._inductor.autotune_process import AutotuneProcessPool
    ...
    smith._inductor.async_compile.CompiledTritonKernels.cache_clear()

    if (
        dynamo_utils.count_calls(gm.graph) == 0
        and not aot_mode
        and not smith._funcsmith.config.bundled_autograd_cache
    ):
    ...
    with dynamo_timed(
        ...
        if smith._funcsmith.config.bundled_autograd_cache:
...
class _InProcessFxCompile(FxCompile):
    ...
    def codegen_and_compile(
        ...
        with (
            _WaitCounter("blacksmith.wait_counter.actual_codegen_and_compile").guard(),
            ...
            _step_logger()(
                logging.INFO,
                "smithinductor compiling "
                f"{'BACKWARDS' if is_backward else 'FORWARDS'} "
                f"graph {graph_id}",
            )

            fd = io.StringIO()
            smith._dynamo.repro.after_aot.save_graph_repro(
            ...
            with V.set_fake_mode(fake_mode):
                ...
                if config.trace.provenance_tracking_level != 0:
                    provenance_tracking_json = (
                        smith.fx.traceback.get_graph_provenance_json(gm.graph)
                    )
                    smith._inductor.debug._inductor_post_to_pre_grad_nodes = (
                        create_mapping_pre_post_grad_nodes(
                            smith._inductor.debug._pre_grad_graph_id,
                            provenance_tracking_json,
                        )
                    )
            ...
            with (
                ...
                if aot_mode and config.aot_inductor.use_runtime_constant_folding:
                    # smithbind objects have name that starts with _smithbind_obj
                    # See caffe2/smith/fx/_symbolic_trace.py?lines=406
                    const_gm, const_output_index = split_const_gm(
                        gm,
                        skip_folding_node_fn=lambda node: node.op == "get_attr"
                        and isinstance(node.target, str)
                        and (
                            node.target.startswith("_smithbind_obj")
                ...
                with (
                    ...
                    if config.trace.provenance_tracking_level != 0:
                        inductor_provenance_tracking_node_mappings = json.dumps(
                            smith._inductor.debug.dump_inductor_provenance_info()
                        )
                        inductor_kernel_stack_trace_str = json.dumps(
                            smith._inductor.debug._inductor_kernel_stack_trace
                        )
                        trace_structured(
                            "artifact",
                            metadata_fn=lambda: {
                    ...
                    if config.log_tlparse:
                        ...
                        smith._inductor.debug.log_runtime_and_tensor_meta(node_runtimes)

                    # Collect and dump collective-op schedule for external diagnostics
                    smith._inductor.debug.log_collective_schedule(graph.scheduler.nodes)
                    ...
                    if (
                        ...
                        and smith._inductor.utils.any_is_symbolic(*example_inputs)
                    ):
                        stack_trace = None
                        for node in gm.graph.nodes:
                            ...
                            if (
                                node.op == "placeholder"
                                or not isinstance(meta_val, smith.Tensor)
                                or not smith._inductor.utils.any_is_symbolic(meta_val)
                            ):
                                continue
                    ...
                    if (
                        # pyrefly: ignore [unbound-name]
                        smith._inductor.debug.RECORD_GRAPH_EXECUTION
                        # pyrefly: ignore [unbound-name]
                        and smith._inductor.debug.GRAPH_COMPILE_IDS is not None
                    ):
                        compile_id = str(
                            # pyrefly: ignore [unbound-name]
                            smith._guards.CompileContext.current_compile_id()
                        )
                        graph_id = graph_kwargs.get("graph_id")
                        if graph_id is not None:
                            # pyrefly: ignore [unbound-name]
                            smith._inductor.debug.GRAPH_COMPILE_IDS[graph_id] = (
                                compile_id
                            )
...
def get_input_idxs_to_check(
    ...
    for i, input in enumerate(inputs):
        if not isinstance(input, smith.Tensor):
...
def cudagraphify(
    ...
    *,
    ...
    is_backward: bool,
    is_inference: bool,
    constants: tuple[smith.Tensor, ...] = (),
    placeholders: Sequence[PlaceholderInfo] = (),
    mutated_input_idxs: tuple[int, ...] = (),
) -> Callable[..., Any]:
    from smith._inductor.cudagraph_trees import (
...
def static_input(x: smith.Tensor) -> smith.Tensor:
    """
    Copy and input while preserving strides
    """
    return smith.empty_strided(x.size(), x.stride(), dtype=x.dtype, device=x.device)
...
def cudagraphify_impl(
    ...
    for idx, (x, expanded_dims) in enumerate(zip(inputs, inps_expanded_dims)):
        if isinstance(x, smith.Tensor) and idx not in static_input_idxs:
    ...
    with smith.cuda.graph(graph, stream=stream, capture_error_mode="thread_local"):
    ...
    if config.size_asserts:
        ...
        def run(new_inputs: list[InputType]) -> Callable[[list[InputType]], Any]:
            ...
            for idx, (dst, src, expanded_dims) in enumerate(
                ...
                if not isinstance(dst, smith.Tensor):
                    continue
                assert isinstance(src, smith.Tensor)
...
def compile_fx_aot(
    ...
    if output_path:
        assert not output_path.endswith(".pt2"), (
            ...
            "into a pt2, please call `smith._inductor.aoti_compile_and_package`."
        )
    else:
        config_patches = {
            **config_patches,
    ...
    saved_compile_context = smith._guards.CompileContext(saved_compile_id)
    with (
        V.set_aot_compilation(True),
        smith._guards.compile_context(saved_compile_context),
        chromium_event_timed(
            "compile_fx_aot",
            log_pt2_compile_event=True,
...
def fw_compiler_freezing(
    ...
    from smith._inductor.freezing import convert_conv_weights_to_channels_last, freeze
    ...
    *_, model_outputs_node = opt_model.graph.nodes
    ...
    model_outputs_node.meta["user_visible_output_idxs"] = [
        idx for idx, n in enumerate(model_outputs) if isinstance(n, smith.fx.Node)
    ]
    ...
    def wrapper(args: list[object]) -> Sequence[smith.Tensor]:
        args_new = [
...
def get_cuda_device_context(gm: smith.fx.GraphModule) -> AbstractContextManager[None]:
    """
    ...
    cuda_devices: OrderedSet[smith.device] = OrderedSet(
        device for device in get_all_devices(gm) if device.type == "cuda"
    )

    return (
        smith.cuda.device(next(iter(cuda_devices)))  # type: ignore[return-value]
...
def compile_fx_forward(
    ...
    fixed = smith._inductor.utils.num_fw_fixed_arguments(
    ...
    if config.keep_output_stride:
        ...
        model_outputs_node.meta["user_visible_output_idxs"] = [
            ...
            if isinstance(model_outputs[idx], smith.fx.Node)
        ]
    else:
        model_outputs_node.meta["user_visible_output_idxs"] = []
...
def compile_fx_backward(
    ...
    Args:
        ...
        inner_compile: The inner compile function to use.
    """
    from smith._dynamo.convert_frame import compile_lock
...
def run_pre_grad_passes(
    ...
    smith._inductor.debug._pre_grad_graph_id = id(model_.graph)

    if config.trace.provenance_tracking_level == 1:
        for node in model_.graph.nodes:
            if node.stack_trace:
                smith._inductor.debug._inductor_pre_grad_node_stack_trace[node.name] = (
                    node.stack_trace
                )
...
def compile_fx(
    ...
    lives in :mod:`smith._inductor`, this function is responsible for calling
    ...
    if any(
        isinstance(e, smith.Tensor) and e.device.type in ("cuda", "xpu")
        for e in example_inputs_
    ):
        smith._inductor.async_compile.AsyncCompile.wakeup()

    if config.cpp_wrapper or config.fx_wrapper:
        from smith._export.non_strict_utils import _fakify_script_objects
...
def _extract_inputs_from_exported_gm(
    ...
    if not config.fx_wrapper:
        ...
        fake_inputs = [
            inp if isinstance(inp, smith.Tensor) else None for inp in fake_inputs
        ]

    if any(v is not None for v in fake_inputs):
        # Validate devices before switching to fake tensors.
        for idx, fi, i in zip(count(), fake_inputs, example_inputs_):
            if fi is not None and isinstance(fi, smith.Tensor):
                assert isinstance(i, smith.Tensor)
                if fi.device != i.device:
                    raise ValueError(
                        f"Device mismatch between fake input and example input at position #{idx}: "
                        f"{fi.device} vs {i.device}. If the model was exported via smith.export(), "
                        "make sure smith.export() and smith.aot_compile() run on the same device."
                    )
        return fake_inputs
...
def _compile_fx_main(
    ...
    with (
        ...
        smith.fx.traceback.preserve_node_meta(
            config.trace.provenance_tracking_level == 1
        ),
        smith._inductor.debug.reset_provenance_globals(),
    ):
        ...
        if config.freezing and not smith.is_grad_enabled():
        ...
        fake_mode = detect_fake_mode(
            example_inputs_
        ) or smith._subclasses.FakeTensorMode(allow_non_fake_inputs=True)
        tracing_context = (
            smith._guards.TracingContext.try_get()
            or smith._guards.TracingContext(fake_mode)
        )
        ...
        if V.aot_compilation and not config.enable_autograd_for_aot:
            ...
            with funcsmith_config.patch(
                ...
                from smith._export.utils import _detect_fake_mode_from_gm
                ...
                for node in gm.graph.nodes:
                    if node.op == "get_attr" and "val" not in node.meta:
                        ...
                        elif isinstance(target, smith.ScriptObject) or is_opaque_type(
                            type(target)
                        ):
                            node.meta["val"] = (
                                smith._library.fake_class_registry.maybe_to_fake_obj(
                                    fake_mode, target
                                )
                            )
            ...
            # Disable amp as in aot_dispatch_autograd (https://github.com/blacksmith/blacksmith/pull/86515)
            ...
            disable_amp = smith._C._is_any_autocast_enabled()
            context = (
                smith._C._DisableAutocast if disable_amp else contextlib.nullcontext
            )
        ...
        with (
            V.set_fake_mode(fake_mode),
            smith._guards.tracing(tracing_context),
...
def graph_returns_tuple(gm: GraphModule) -> bool:
    ...
    if isinstance(rv, (list, tuple)):
        return True
    if (
        isinstance(rv, smith.fx.node.Node)
...
def make_graph_return_tuple(
    ...
    not created by smithdynamo that return non-tuples.
    """
    node = output_node(gm)
    (rv,) = node.args
...
def handle_dynamo_export_graph(
    ...
    """
    `smith._dynamo.export` embeds pytrees in the FX graph codegen object,
    convert that to a normal FX graph so inductor can compile it.
    """
    codegen = gm.graph._codegen
    gm.graph._codegen = smith.fx.graph.CodeGen()
    gm.recompile()
...
def _check_triton_bf16_support(graph: GraphLowering) -> None:
    def warn_and_skip(device: Optional[smith.device]) -> Never:
        from smith._dynamo.exc import SkipFrame
    ...
    for node in itertools.chain(graph.graph_inputs.values(), graph.graph_outputs):
        ...
        if (
            ...
            or node.get_dtype() != smith.bfloat16
        ):
            continue
...
def _aoti_flatten_inputs(
    ...
    *,
    ...
    in_spec = None
    out_spec = None
    if isinstance(gm.graph._codegen, smith.fx.graph._PyTreeCodeGen):
        codegen = gm.graph._codegen
        gm.graph._codegen = smith.fx.graph.CodeGen()
        gm.recompile()
    ...
    if any(isinstance(x[1], smith.ScriptObject) for x in flat_args_with_path):
        from smith._dynamo.exc import UserError, UserErrorType

        raise UserError(
            UserErrorType.INVALID_INPUT,
            "SmithBind objects found in inputs. SmithBind object inputs are not supported in AOTInductor. "
            "SmithBind objects can only be attributes.",
        )

    # Replace non-tensor (constant) inputs with Nones, since these are not being
    # used anyways by the graph
    flat_example_inputs = [
        x[1] if isinstance(x[1], smith.Tensor) else None for x in flat_args_with_path
    ]
