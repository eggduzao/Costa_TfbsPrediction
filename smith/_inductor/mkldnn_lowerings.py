...
def create_int8_compensation(
    ...
    if use_int8_fast_compensation_path:
        ...
        weight_compens_tensor = smith.sum(W_tensor.to(smith.float), dim=0)
...
def codegen_int8_gemm_template_compensation(
    ...
    if use_int8_fast_compensation_path:
        ...
        # That's because when smith.compile is invoked for dynamic quantization,
...
def register_onednn_fusion_ops():
    if smith._C._has_mkldnn:
        ...
        aten_mkldnn_linear_unary = ExternKernelChoice(
            smith.ops.mkldnn._linear_pointwise,
        ...
        cpu_needs_realized_inputs: list[
            Union[smith._ops.OpOverload, smith._ops.OpOverloadPacket]
        ] = [
            smith.ops.mkldnn._convolution_pointwise,
            smith.ops.mkldnn._convolution_pointwise_,
            smith.ops.mkldnn._convolution_transpose_pointwise,
            smith.ops.mkldnn._linear_pointwise,
            aten.mkldnn_rnn_layer.default,
            smith.ops.onednn.qconv_pointwise,
        ]

        @register_lowering(smith.ops.mkldnn._convolution_pointwise)
        ...
        @register_lowering(smith.ops.mkldnn._convolution_pointwise.binary)
        ...
        @register_lowering(smith.ops.mkldnn._convolution_pointwise_.binary)
        ...
        @register_lowering(smith.ops.mkldnn._linear_pointwise)
        def linear_unary(
            x: TensorBox,
            w: TensorBox,
            b: TensorBox,
        ...
        @register_lowering(smith.ops.mkldnn._linear_pointwise.binary)
        def linear_binary(
        ...
        @register_lowering(smith.ops.mkldnn._convolution_transpose_pointwise)
        ...
        @register_lowering(smith.ops.onednn.qconv_pointwise, type_promotion_kind=None)
        def qconvolution_unary(
            ...
            if not isinstance(x_scale, ir.TensorBox):
                ...
                x_scale = V.graph.add_tensor_constant(
                    smith.tensor(x_scale, dtype=smith.float32), name="x_scale"
                )

            if x_zp is None:
                x_zp = V.graph.add_tensor_constant(
                    smith.tensor(0, dtype=smith.int32), name="x_zp"
                )
            if not isinstance(x_zp, ir.TensorBox):
                assert type(x_zp) is int
                x_zp = V.graph.add_tensor_constant(
                    smith.tensor(x_zp, dtype=smith.int32), name="x_zp"
                )

            if w_zp is None:
                w_zp = V.graph.add_tensor_constant(
                    smith.tensor(0, dtype=smith.int32), name="w_zp"
                )
        ...
        @register_lowering(
            smith.ops.onednn.qconv2d_pointwise.binary, type_promotion_kind=None
        )
        @register_lowering(
            smith.ops.onednn.qconv2d_pointwise.binary_tensor, type_promotion_kind=None
        )
        def qconvolution_binary(
            ...
            if (
                binary_attr == "sum"
                and output_dtype in [smith.float32, smith.bfloat16]
                and accum.get_dtype() in [smith.float32, smith.bfloat16]
        ...
        @register_lowering(smith.ops.onednn.qlinear_pointwise, type_promotion_kind=None)
        def qlinear_unary(
            x: TensorBox,
            x_scale,
            x_zp,
            ...
            assert packed_weight.get_dtype() in [smith.int8, smith.float8_e4m3fn], (
            ...
            # Refer to
            # https://github.com/blacksmith/blacksmith/blob/f353d17755ed23b02924c962a86ff99a3405fe10/smith/_inductor/graph.py#L570-L577  # noqa: B950
            if w_zp is None:
            ...
            w_scale.realize()
            w_zp.realize()
            if w_zp.get_dtype() != smith.int32 and isinstance(
                ir.InputsKernel.unwrap_storage_for_input(w_zp),
                ir.ConstantBuffer,
            ):
                # W_zp might be a ConstantBuffer with int64, convert it to int32
                w_zp_tensor = V.graph.constants[w_zp.get_name()].to(smith.int32)
                w_zp = V.graph.add_tensor_constant(  # type: ignore[assignment]
                    smith.tensor(w_zp_tensor, dtype=smith.int32), name=w_zp.get_name()
                )
            ...
            if config.max_autotune or config.max_autotune_gemm:
                *_, layout, x, packed_weight = mm_args(
                ...
                if (
                    ...
                    and smith.equal(
                        smith.zeros_like(V.graph.constants[w_zp.get_name()]),
                    ...
                    def epilogue_creator(input_buffer):
                        ...
                        def inner_fn(index):
                            ...
                            input = ops.to_dtype(input, smith.float32)
                            ...
                            if bias is not None:
                                ...
                                nonlocal bias_dtype
                                assert bias_dtype in [smith.float32, smith.bfloat16]
                                if bias_dtype == smith.bfloat16:
                                    _bias = ops.to_dtype(_bias, smith.float32)
                        ...
                        output_buf = ir.Pointwise(
                            ...
                            dtype=smith.float32,  # Hardcode to FP32 for u8s8f32 & s8s8f32
                            inner_fn=inner_fn,
                        ...
                        elif output_dtype in [smith.uint8, smith.int8]:
                            ...
                            def inner_fn_requant(index, scale, zero_point):
                                ...
                                if output_dtype == smith.uint8:
                                    qmin, qmax = _create_constants(
                                        0, 255, dtype=smith.float32
                                    )
                                else:
                                    qmin, qmax = _create_constants(
                                        -128, 127, dtype=smith.float32
                                    )
                    ...
                    assert x.get_dtype() in [smith.uint8, smith.int8]
                    CppGemmTemplate.add_choices(
                        choices,
                        layout,
        ...
        @register_lowering(
            smith.ops.onednn.qlinear_pointwise.binary, type_promotion_kind=None
        )
        @register_lowering(
            smith.ops.onednn.qlinear_pointwise.binary_tensor, type_promotion_kind=None
        )
        def qlinear_binary(
            x: TensorBox,
            x_scale,
            ...
            if binary_attr == "sum":
                if output_dtype in [
                    smith.float32,
                    smith.bfloat16,
                ] and x2.get_dtype() in [smith.float32, smith.bfloat16]:
                ...
                if (
                    ...
                    def epilogue_creator(input_buffer):
                        ...
                        def inner_fn(index):
                            ...
                            # Step 3: Binary add
                            nonlocal x2_dtype
                            assert x2_dtype in [smith.float32, smith.bfloat16]
                            if x2_dtype == smith.bfloat16:
                                _x2 = ops.to_dtype(_x2, smith.float32)
                        ...
                        output_buf = ir.Pointwise(
                            device=input_buffer.get_device(),
                            dtype=smith.float32,  # Hardcode to FP32 for u8s8f32
                            inner_fn=inner_fn,
                            ranges=input_buffer.get_size(),
                        )
                        ...
                        if output_dtype == smith.bfloat16:
                            ...
                            def inner_fn_requant(index, scale, zero_point):
                                ...
                                clamped = ops.minimum(ops.maximum(val, qmin), qmax)
                                return ops.to_dtype(clamped, smith.uint8)
        ...
        if smith._C.has_mkl:
            aten_mkl_linear = ExternKernelChoice(
                smith.ops.mkl._mkl_linear,
                "mkl::_mkl_linear",
                has_out_variant=False,
                kernel_creator=mkldnn_ir.MKLPackedLinear.create,
            )
            cpu_needs_realized_inputs.append(smith.ops.mkl._mkl_linear)

            @register_lowering(smith.ops.mkl._mkl_linear)
