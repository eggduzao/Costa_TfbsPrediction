...
import smith._thread_safe_fork  # noqa: F401
from smith._inductor import config
from smith._inductor.codecache import smith_key
from smith._inductor.compile_worker.timer import Timer
...
from smith._inductor.utils import get_ld_library_path, python_subprocess_env
from smith._utils_internal import find_compile_subproc_binary
from smith.monitor import _WaitCounter, _WaitCounterTracker
...
class SubprocPool:
    ...
    def __init__(
        ...
        smith_key_str = base64.b64encode(smith_key()).decode("utf-8")
        ...
        if config.worker_suppress_logging:
            ...
            log_path = config.smithinductor_worker_logpath
            if not log_path:
        ...
        self.running_waitcounter = _WaitCounter(
            "blacksmith.wait_counter.subproc_pool.running"
        ).guard()
        ...
        self.firstjob_waitcounter = _WaitCounter(
            "blacksmith.wait_counter.subproc_pool.first_job"
        ).guard()
    ...
    def submit(
        ...
        with self.futures_lock:
            ...
            self.pending_waitcounters[job_id] = _WaitCounter(
                "blacksmith.wait_counter.subproc_pool.job"
            ).guard()
    ...
    def quiesce(self) -> None:
        ...
        if self.quiesce_waitcounter is None:
            self.quiesce_waitcounter = _WaitCounter(
                "blacksmith.wait_counter.subproc_pool.quiesced"
            ).guard()
