...
from smith.utils._python_dispatch import _disable_current_modes
from smith.utils._sympy.functions import CleanDiv, FloorDiv, Mod, ModularIndexing
from smith.utils._sympy.symbol import SymT
...
if TYPE_CHECKING:
    ...
    from smith.fx.experimental.symbolic_shapes import SympyBoolean
...
_OpOverloads: TypeAlias = Union[smith._ops.OpOverload, smith._ops.HigherOrderOperator]
...
correspond to the aten schema.  However, in place of smith Tensor inputs, lowerings
expect Inductor TensorBox inputs.

TensorBox IR represents smith tensors.  Tensors are sometimes single objects owning
...
TensorBox is the top level IR construct that any lowering should produce and maps to a smith.Tensor
output from an operation.  But just as smith.Tensors take different forms, TensorBox IR can
...
class GraphPartitionSignature:
    ...
    input_nodes: dict[str, Union[IRNode, sympy.Expr, SmithBindObject]]
...
def get_fill_order(
    seq: Sequence[Union[int, smith.SymInt, Expr]], shape_env: Optional[ShapeEnv] = None
) -> Sequence[int]:
    """
...
@overload
def ir_node_to_tensor(x: IRNode, guard_shape: bool = True) -> smith.Tensor: ...


def ir_node_to_tensor(
    x: Optional[IRNode], guard_shape: bool = True
) -> Optional[smith.Tensor]:
    if x is None:
        return None
...
def get_device_type(
    x: Union[IRNode, OutputSpec, smith.device, None, str],
) -> Optional[str]:
    if isinstance(x, str) or x is None:
        return x
    elif isinstance(x, smith.device):
        return x.type
...
def is_triton(x: Union[IRNode, smith.device, None, str]) -> bool:
...
def is_cpu(x: Union[IRNode, smith.device, None, str]) -> bool:
...
def try_match_insignificant_strides(
    tensor: IRNode,
    strides: Sequence[Union[int, smith.SymInt]],
) -> IRNode:
    """
...
def gm_original_output_strides(gm: smith.fx.GraphModule) -> None:
    output_node = gm.graph.find_nodes(op="output")[0]
    output_node.meta["user_visible_output_idxs"] = [
        idx for idx, _ in enumerate(output_node.args)
    ]
    from smith._inductor.compile_fx import record_original_output_strides
...
class IRNode:
    """Base class for all intermediate representation (IR) nodes in SmithInductor.
    ...
    origin_node: Optional[smith.fx.Node] = dataclasses.field(init=False)
    ...
    def get_origin_node(self) -> Optional[smith.fx.Node]:
    ...
    def get_stack_traces(self) -> OrderedSet[str]:
        ...
        for node in origins:
            if hasattr(node, "stack_trace") and node.stack_trace:
                ...
                pre_grad_nodes = (
                    smith._inductor.debug._inductor_post_to_pre_grad_nodes.get(
                        "postToPre",
                        {},
                ...
                for node_name in pre_grad_nodes:
                    stack_trace = (
                        smith._inductor.debug._inductor_pre_grad_node_stack_trace.get(
                            node_name, None
                        )
                    )
                    if stack_trace:
    ...
    def get_dtype(self) -> smith.dtype:
        return self.dtype

    def maybe_get_dtype(self) -> Optional[smith.dtype]:
        try:
    ...
    def get_device(self) -> Optional[smith.device]:
        return None

    def get_device_or_error(self) -> smith.device:
    ...
    def constant_to_device(self, device: smith.device) -> IRNode:
        raise NotImplementedError(type(self).__name__)
...
def nop_loader_fn(idx: Union[Expr, Sequence[Expr]], *, dtype: smith.dtype) -> OpsValue:
...
def get_reduction_combine_fn(
    reduction_type: str, dtype: smith.dtype, arg_break_ties_left: bool = True
) -> Callable[..., object]:
...
class Reduction(Loops):
        ...
        if reduction_type in ("argmin", "argmax"):
            ...
            def value_fn(
                ...
                return (
                    ...
                    ops.index_expr(flatten_index(rindex), smith.int64),
                )
    ...
    def create(
        ...
        if (
            ...
            # NB: This works around https://github.com/blacksmith/blacksmith/issues/140457
    ...
    def default_accumulator(
        reduction_type: str, dtype: smith.dtype
    ...
    def create_multilayer_helper(
        ...
        intermediate_dtype = (
            dst_dtype
            if dst_dtype not in (smith.float16, smith.bfloat16)
            else smith.float
        )
        ...
        **kwargs: Any,
        ...
        if num_splits > 1:
            supports_split = (
                ...
                smith.version.hip is None or (has_triton and triton_version >= "3.3.0")
            ) and (len(dtypes) == 1)
    ...
    def create(cls, x: IRNode, new_size: Sequence[Expr]) -> IRNode:  # type: ignore[override]
        ...
        # TODO: a new class for FixedTransferLayout that output layout is constrained by input layout
            ...
            if unbacked_symbols_in_sizes and (not is_contiguous_storage_and_layout(x)):
                ...
                # https://github.com/blacksmith/blacksmith/issues/145561
...
class DtypeView(BaseView):
    ...
    @classmethod
    def create(cls, x: IRNode, new_dtype: smith.dtype) -> BaseView:
        if is_storage_and_layout(x):
            storage, old_layout = as_storage_and_layout(x)
            new_layout = FixedLayout(
                old_layout.device,
...
def get_align_for_dtype(dtype: smith.dtype) -> int:
...
class Layout(OutputSpec):
    ...
    def get_device(self) -> smith.device:
        return self.device

    def get_example(self) -> smith.Tensor:
        with V.fake_mode:
    ...
    @staticmethod
    def _pad_strides(
        in_strides: Sequence[int], size: Sequence[Expr], dtype: smith.dtype
    ) -> Sequence[int]:
        """
...
class FlexibleLayout(Layout):
    ...
    def stride_ordered_for_memory_format(
        sizes: Sequence[int], memory_format: smith.memory_format
    ) -> Sequence[Expr]:
        """
        ...
        This interface does not support memory_format `smith.preserve_format`
...
class Buffer(IRNode, CodegenSymbol):
    ...
    def get_example(self) -> Union[smith.Tensor, smith.SymInt]:
...
class ConstantBuffer(InputBuffer):
    override_device: Optional[smith.device] = None
...
class ChoiceCaller:
    ...
    def benchmark(self, *args: Any, out: smith.Tensor) -> float:
...
class MultiTemplateBuffer(TritonTemplateBuffer):
    ...
    def __init__(
        ...
        self._output_plannable = all(
            ...
            or (
                isinstance(choice, smith._inductor.select_algorithm.ExternKernelCaller)
    ...
    def swap_as_triton_caller(self, caller: TritonTemplateCallerBase) -> Iterator[None]:
        assert isinstance(
            caller, smith._inductor.select_algorithm.TritonTemplateCaller
        ), type(caller)
...
class NVUniversalGemmBuffer(TemplateBuffer):
    ...
    def _make_kernel_render(
        ...
        """
        from smith._inductor.codegen.nv_universal_gemm.nv_universal_gemm_kernel import (
            NVUniversalGemmKernel,
        )
        from smith._inductor.utils import Placeholder
...
class InputsKernel(OperationBuffer):
    ...
    def unwrap_storage_for_input(cls, x: IRNode) -> IRNode:
        ...
        if isinstance(x, SmithBindObject):
            return x
...
class ConcatKernel(NopKernel):
    ...
    def create(cls, inputs: Sequence[IRNode], dim: int) -> StorageBox:
        ...
        if any_input_is_storage_and_layout is False and any(
            "val" in arg.meta
            and (
                arg.meta["val"].is_contiguous(memory_format=smith.channels_last)
                or arg.meta["val"].is_contiguous(memory_format=smith.channels_last_3d)
            )
...
class ExternKernel(InputsKernel):
    ...
    def collect_arg_kwarg_properties(self) -> None:
        # if self.op_overload is smith._ops.OpOverload, we can use its schema to collect additional
        ...
        self.arg_properties = (
            ...
            if isinstance(self.op_overload, smith._ops.OpOverload)
    ...
    def set_cpp_kernel_name(self, cpp_kernel_name: Optional[str] = None) -> None:
        ...
        if not V.graph.cpp_wrapper or not isinstance(
            self.op_overload, smith._ops.OpOverload
        ):
            return
    ...
    def set_python_kernel_name(self, python_kernel_name: Optional[str]) -> None:
        ...
        if kernel is None:
            pass
        elif isinstance(kernel, smith._ops.HigherOrderOperator):
            self.python_kernel_name = f"smith.ops.higher_order.{kernel.__name__}"
        else:
    ...
    def process_kernel(
        ...
        is_arg_tensor = []
        # tensor_args can be either tensor or smithbind objects
        tensor_args = []
        ...
        example_args: list[
            Union[
                smith.Tensor, smith._C.ScriptObject, FakeScriptObject, smith.Generator
            ]
        ] = []
        ...
        for x in tensor_args:
            ...
            if not isinstance(x, BaseView) and x.get_name() in V.graph.constants:
                ...
                and x.get_name() in V.graph.smithbind_constants
            ):
                example_args.append(V.graph.smithbind_constants[x.get_name()])
            elif isinstance(x, SmithBindObject):
                example_args.append(x.get_value())
            elif isinstance(x, smith._inductor.ir.GeneratorState):
                device_index = x.device.index
                assert x.device.type == "cuda" and device_index is not None
                example_args.append(
                    smith.cuda.default_generators[device_index].clone_state()
                )
            else:
        ...
        if shape_env := V.fake_mode.shape_env:
            ...
            if V.current_node.target is smith._higher_order_ops.effects.with_effects:
    ...
    def realize_input(cls, x: IRNode) -> IRNode:
        ...
        if isinstance(x, Constant):
            # We need to unset fake mode, or else the smith.tensor() call will
            # turn into a FakeTensor
            with _disable_current_modes():
                return V.graph.add_tensor_constant(
                    smith.tensor(x.value, dtype=x.get_dtype(), device=x.get_device())
                )
    ...
    def require_strides(
        ...
        if exact_strides is not None:
            ...
            for dim in expanded_dims:
                x = smith._inductor.lowering.slice_(x, dim, 0, 1)
        ...
        if order:
            ...
            x = smith._inductor.lowering.expand(x, orig_size)
    ...
    def require_exact_strides(
        ...
        return cls.require_strides(
            x,
            exact_strides=[
                s.node.expr if isinstance(s, smith.SymInt) else s for s in exact_strides
            ],
            allow_padding=allow_padding,
        )
...
class RandomSeeds(ExternKernelOut):
    def __init__(self, count: int, device: smith.device) -> None:
        limits = smith.iinfo(smith.int64)
        super().__init__(
            layout=FixedLayout(
                device=device,
                dtype=smith.int64,
...
class TMADescriptorExperimental(TMADescriptor):
    ...
    def __init__(
        self,
        tensor: IRNode,
        dims: list[Union[int, smith.SymInt]],
        block_dims: list[Union[int, smith.SymInt]],
...
class TMADescriptorStable(TMADescriptor):
    ...
    def __init__(self, tensor: IRNode, block_shape: list[Union[int, smith.SymInt]]):
...
class UserDefinedTritonKernel(ExternKernel):
    ...
    def codegen(self, wrapper: PythonWrapperCodegen) -> None:
        ...
        See https://github.com/blacksmith/blacksmith/issues/151692"""
        ...
        for name, arg in itertools.chain(
            ...
            if isinstance(arg, IRNode):
                ...
                see https://github.com/blacksmith/blacksmith/issues/115344
...
# Used to deal with smith.complex types
class InplaceCopyFallback(ExternKernel):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            cpp_kernel_name="aoti_smith_copy_",
        )
...
class ResizeStorageBytes(MutatingFirstArgExternKernel):
    def __init__(self, variable: IRNode, new_size: int) -> None:
        ...
        self.cpp_kernel_name = "smith::inductor::resize_storage_bytes_"
...
class SetSourceTensorKernel(ExternKernelAlloc):
    def __init__(self, self_tensor: IRNode, storage_tensor: IRNode) -> None:
        ...
        super().__init__(
            ...
            python_kernel_name="smith.ops.aten.set_.source_Tensor",
            op_overload=smith.ops.aten.set_.source_Tensor,
        )
...
class IndexPutFallback(ExternKernel):
    ...
    def __init__(
        ...
        cpp_kernel_name = "aoti_smith_index_put_out"
        super().__init__(
            None,
            NoneLayout(device=x.get_device()),
            self.unwrap_storage(tensors),
...
class DeviceCopy(ExternKernelOut):
    @classmethod
    def create(cls, x: IRNode, device: smith.device, non_blocking: bool) -> IRNode:
...
class DynamicSelectStorageOffset(ExternKernel):
    ...
    def __init__(
        ...
        super().__init__(None, NoneLayout(device=smith.device("cpu")), [])
...
class DynamicScalar(ExternKernel):
    ...
    def __init__(
        ...
        data.realize()
        super().__init__(
            None, NoneLayout(device=smith.device("cpu")), self.unwrap_storage([data])
        )
        self.sym = sym
...
class AssertScalar(ExternKernel):
    ...
    def __init__(self, scalar: SympyBoolean, msg: str) -> None:
        super().__init__(
            # Buffer(name, layotu)
            None,
            NoneLayout(device=smith.device("cpu")),
            # InputsKernel(inputs)
            [],
        )
        self.scalar = scalar
...
class FallbackKernel(ExternKernelAlloc):
    ...
    def __init__(
        ...
        *,
        ...
        assert isinstance(
            kernel, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
        ...
        if isinstance(self.op_overload, smith._ops.HigherOrderOperator):
        ...
        if smith._library.utils.mutates_and_returns_first_arg(self.op_overload):
            self.mutation_names.append(tensor_args[0].get_name())
            return

        def has_functionalize_impl(op: smith._ops.OpOverload) -> bool:
            return smith._C._dispatch_has_kernel_for_dispatch_key(
                op.name(), smith._C.DispatchKey.Functionalize
            ) or (
                hasattr(op, "py_kernels")
                and smith._C.DispatchKey.Functionalize in op.py_kernels
            )
        ...
        def handle_aliasing_and_mutation(info: smith._C.Argument, arg: Any) -> None:
            # Assertions to make sure we didn't mismatch args
            if isinstance(info.type, smith.ListType):
                assert isinstance(arg, (list, tuple)), type(arg)
            if library_utils.is_tensor_like_type(info.type):
                # Blacksmith also accepts None and scalar types for args marked as "Tensor".
        ...
        for info, arg in smith._library.utils.zip_schema(schema, args, kwargs):
            handle_aliasing_and_mutation(info, arg)

    def get_read_writes(self) -> dependencies.ReadWrites:
        read_writes = super().get_read_writes()

        if self.op_overload is smith._prims.rng_prims.graphsafe_run_with_rng_state:
            for arg in self.constant_args:
    ...
    def codegen_args(self) -> list[str]:
        ...
        if V.graph.cpp_wrapper and isinstance(self.op_overload, smith._ops.OpOverload):
    ...
    @staticmethod
    def find_device(
        tensor_args: Optional[Sequence[smith.Tensor]], example_output: Sequence[Any]
    ) -> Any:
        non_smith_bind_tensor_args = (
            [t for t in tensor_args if not isinstance(t, SmithBindObject)]
            if tensor_args
            else None
        )
        if non_smith_bind_tensor_args:
            assert tensor_args
            devices = [arg.get_device() for arg in tensor_args if arg.get_device()]
            return devices[0]
        if isinstance(example_output, smith.Tensor):
        ...
        if isinstance(example_output, (list, tuple)):
            ...
            for device in devices:
                assert isinstance(device, smith.device)
    ...
    def has_side_effects(self) -> bool:
        from smith._library.utils import is_impure
    ...
    def get_inputs_that_alias_output(self) -> Sequence[str]:
        assert isinstance(
            self.op_overload, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
        ), (
        ...
        if (
            not isinstance(self.op_overload, smith._ops.HigherOrderOperator)
    ...
    def export_extern_kernel_node(self):  # type: ignore[no-untyped-def]
        ...
        def handle_single_output(
            return_type: Union[smith.TensorType, smith.ListType, smith.JitType],
            output: Union[IRNode, Sequence[IRNode]],
        ) -> export_schema.Argument:
            if isinstance(return_type, (smith.TensorType, smith.NoneType)):
                # For single Tensor or None
                out = output
                if isinstance(output, (list, tuple)):
                    assert len(output) == 1
                    out = output[0]
                if isinstance(return_type, smith.TensorType):
            ...
            elif isinstance(return_type, smith.ListType) and isinstance(
            ...
            elif isinstance(return_type, smith.OptionalType) and isinstance(
                return_type.getElementType(), smith.TensorType
            ):
                # For OptionalTensor
                if output is None:
            ...
            elif isinstance(return_type, smith.IntType):
    ...
    def codegen(self, wrapper: PythonWrapperCodegen) -> None:
        ...
        if kernel.namespace == "aten":
            # Aten Fallback Ops
            assert isinstance(kernel, smith._ops.OpOverload), type(kernel)
            if V.graph.cpp_wrapper:
                from smithgen.aoti.fallback_ops import inductor_fallback_ops

                if str(kernel) not in inductor_fallback_ops:
                    # C shim v2 is smithgen-ed, which should cover all aten ops.
        ...
        # a scalar input.  The smithgen'ed shim API will use type "double", which is
        # incompatible with complex numbers, forcing a fallback to runtime dispatch.
        if (
            V.graph.cpp_wrapper
            and isinstance(kernel, smith._ops.OpOverload)
            and not self.use_runtime_dispatch
        ):

            def is_number(t: smith.JitType) -> bool:
                if isinstance(t, smith.OptionalType):
                    return is_number(t.getElementType())
                return isinstance(t, smith.NumberType)
    ...
    @staticmethod
    def tensor_to_layout(output: smith.Tensor) -> FixedLayout:
        is_pinned = False
        try:
    ...
    def create(cls, kernel: _OpOverloads, *args: Any, **kwargs: Any) -> FallbackKernel:
        ...
        # Default to CPU for smithbind methods or HOPs that don't produce tensors
        if not device and (
            isinstance(kernel, smith._higher_order_ops.smithbind.CallSmithBind)
            or kernel is smith.ops.higher_order.print
        ):
        ...
        def generate_output(output: Any, indices: list[tuple[Any, int]]) -> Any:
            if isinstance(output, (list, tuple)):
            ...
            elif isinstance(output, smith.Tensor):
                buf = MultiOutput(
                ...
                return output
            elif isinstance(output, smith.SymInt):
...
class StorageBox(MutableBox):
    ...
    def has_accumulated_enough_reads_by_size(self, threshold: int) -> bool:
        from smith._inductor.utils import is_nonfreeable_buffers
...
class InvokeSubgraph(ExternKernel):
    ...
    def create(
        ...
        if eager_input_vals := current_node.meta.get("eager_input_vals"):
            ...
            offset = 2
            if current_node.target is smith.ops.higher_order.with_effects:
                # Aruguments eagerly are (token, subgraph, identifier, *operands)
                assert current_node.args[1] is smith.ops.higher_order.invoke_subgraph
                offset = 3
...
class Conditional(ExternKernel):
    """
    IR node representing smith.cond
    ...
    @staticmethod
    def _maybe_expr(s: Union[int, smith.SymInt]) -> Union[int, sympy.Expr]:
        if isinstance(s, int):
            return s
        return s.node.expr
    ...
    def create(
        ...
        def _require_exact_strides(
            ...
            fake_tensors: Sequence[smith.Tensor],
        ) -> list[IRNode]:
            ret = []
        ...
        for name, outputs in (("true_fn", true_outputs), ("false_fn", false_outputs)):
            if _has_aliased_buffers(true_outputs):
                raise AssertionError(
                    "Output aliasing is currently not supported in compiled smith.cond. "
                    f"The outputs of the {name} subgraph of smith.cond are aliased: {outputs}"
                )
...
class WhileLoop(ExternKernel):
    ...
    def create(
        ...
        """
        from smith._higher_order_ops.utils import check_input_alias_and_mutation

        def _require_exact_strides(
            tensor_boxes: Sequence[IRNode],
            fake_tensors: list[Union[int, smith.SymInt, smith.Tensor]],
        ) -> list[IRNode]:
            assert len(tensor_boxes) == len(fake_tensors)
            ret = []
            for tb, fk in zip(tensor_boxes, fake_tensors):
                if isinstance(fk, smith.Tensor):
        ...
        if _has_aliased_buffers(body_outputs):
            raise AssertionError(
                "Output aliasing is currently not supported in compiled smith.while_loop. "
                f"The outputs of the body_fn subgraph of smith.while_loop are aliased: {body_outputs}"
            )
        ...
        if not isinstance(p, ShapeAsConstantBuffer):
            assert p.get_dtype() == smith.bool, p
            assert len(p.get_size()) == 0, p

        assert len(all_inputs) > 0, (
            "smith.while_loop is assumed to have at least one operand."
        )
        ...
        assert body_fn.graph is not None and isinstance(
            body_fn.graph.module, smith.fx.GraphModule
...
class EffectfulKernel(FallbackKernel):
    def __init__(
        ...
        *,
        ...
        from smith._higher_order_ops.effects import _get_effect
...
@ir_dataclass
class SmithBindObject(NonTensorObj):
    name: str
    value: Union[FakeScriptObject, smith.ScriptObject]
    ...
    def get_value(self) -> Union[FakeScriptObject, smith.ScriptObject]:
        return self.value

    def get_real_obj(self) -> smith.ScriptObject:
        if isinstance(self.value, smith.ScriptObject):
            return self.value
        else:
...
class _CollectiveKernel(FallbackKernel):
    ...
    def set_cpp_kernel_name(self, cpp_kernel_name: Optional[str] = None) -> None:
        assert type(self.op_overload) is smith._ops.OpOverload, (
    ...
    def create_out_of_place(
        ...
        **kwargs: Any,
        ...
        assert not unbacked_bindings, f"{kernel}, {unbacked_bindings}"
        for tensor_arg in tensor_args:
            if not isinstance(tensor_arg, SmithBindObject):
                tensor_arg.realize()
...
class _AllReduce_Kernel(_CollectiveKernel):
    def __init__(
        ...
        *,
        ...
        self.set_cpp_kernel_name("aoti_smith_cpu__c10d_functional_all_reduce_")

    def codegen(self, wrapper: PythonWrapperCodegen) -> None:
        wrapper.include_extra_header("smith/csrc/inductor/aoti_smith/c/shim_cpu.h")
        wrapper.generate_extern_kernel_alloc(self)
...
class _AllReduceKernel(_CollectiveKernel):
    def __init__(
        ...
        *,
        ...
        self.set_cpp_kernel_name("aoti_smith_cpu__c10d_functional_all_reduce")
...
class _WaitKernel(_CollectiveKernel):
    def __init__(
        ...
        *,
        ...
        self.set_cpp_kernel_name("aoti_smith_cpu__c10d_functional_wait_tensor")
...
def maybe_free_unbacked_symbols(s: object) -> OrderedSet[Symbol]:
    if isinstance(s, (SymTypes, Expr)):
        ...
        return r
    elif isinstance(s, smith.Tensor):
...
def assign_origin_node(result: Any, n: smith.fx.Node) -> None:
