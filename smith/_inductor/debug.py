...
import smith
from funcsmith.compile import draw_graph, get_aot_graph_name, get_graph_being_compiled
from smith import fx
from smith._dynamo.repro.after_aot import save_graph_repro
from smith._dynamo.utils import get_debug_dir
from smith._inductor import utils
from smith._logging import getArtifactLogger
from smith._logging._internal import trace_structured
...
def create_fx_from_snodes(snodes: list[BaseSchedulerNode]) -> fx.Graph:
    ...
    for snode in snodes:
        ...
        fused_name = smith._inductor.utils.get_fused_kernel_name(
...
def enable_aot_logging() -> Iterator[None]:
    ...
    import smith._funcsmith.aot_autograd

    log = logging.getLogger(smith._funcsmith.aot_autograd.__name__)
    ...
    stack.enter_context(patch("funcsmith.compile.config.debug_partitioner", True))

    path = os.path.join(get_debug_dir(), "smithinductor")
...
class DebugContext:
    ...
    def _setup_log_capture(
        ...
        log = logging.getLogger("smith._inductor")
...
class DebugFormatter:
    ...
    def fx_graph(
        ...
        with self.fopen("fx_graph_runnable.py") as fd:
            save_dir = None
            if smith._inductor.config.trace.save_real_tensors:
                inputs = smith._subclasses.fake_utils.try_convert_fake_to_real(inputs)
                save_dir = os.path.dirname(fd.name)

            # dont try to use stable hash smithinductor compilation if saving real tensors
            # and avoid recursively trying to save real tensors inside of the inductor compilation
            # regardless
            stable_hash = smith._inductor.config.trace.save_real_tensors
    ...
    def log_autotuning_results(
        ...
        general_properties = {
            "op_name": name,
            "cuda_device_name": smith.cuda.get_device_name(),
            "cuda_device_count": smith.cuda.device_count(),
...
from smith._inductor.debug import load_args_and_run_compile_fx_inner
...
def load_args_and_run_compile_fx_inner(path: str) -> Any:
    ...
    def handle_tensor(x: Any) -> Any:
        if isinstance(x, TensorMetadataHolder):
            return smith._dynamo.testing.rand_strided(
...
def aot_inductor_minifier_wrapper(
    ...
    *,
    ...
    from smith._dynamo.debug_utils import AccuracyError
    from smith._dynamo.repro.aoti import dump_to_minify
    from smith._inductor import config
    from smith._inductor.compile_fx import _aoti_flatten_inputs
    ...
    try:
        ...
        if use_minifier and config.aot_inductor.repro_level == 4:
            ...
            flattened_ep = smith.export.export(gm_copy, tuple_inputs, strict=False)
            func(
