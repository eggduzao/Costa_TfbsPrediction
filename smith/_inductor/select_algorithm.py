...
from smith._inductor.await_utils import await_sync
from smith._inductor.utils import clear_on_fresh_cache
...
if TYPE_CHECKING:
    ...
    from smith._inductor.autotune_process import BenchmarkRequest
    from smith._inductor.codegen.simd import IterationRangesEntry, IterationRangesRoot
...
class BenchmarkTensors:
    ...
    output_tensor: Optional[smith.Tensor]
...
class AutotuneArgs:
    ...
    expected: Optional[smith.Tensor] = None
    ...
    def from_choice_args(
        ...
        example_inputs_extern: list[smith.Tensor],
        out: smith.Tensor,
        out_extern: smith.Tensor,
        expected: Optional[smith.Tensor] = None,
    ) -> Self:
    ...
    def verify(self, **kwargs):
        ...
        smith.testing.assert_close(self.extern.output_tensor, self.expected, **kwargs)
...
class ModificationWrapper(V.WrapperHandler):  # type: ignore[name-defined]
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        if name not in self.fixed_inputs:
            ...
            if (
                var_dtype in (smith.float16, smith.bfloat16)
                and config.triton.codegen_upcast_to_fp32
            ):
                line += ".to(tl.float32)"
                var_dtype = smith.float32
...
class TritonTemplateKernel(TritonKernel):
    ...
    def store_output(
        ...
        with self.create_subgraph_body(subgraph_name, clear_cse=True):
            ...
            acc_dtype = (
                triton_type_to_smith(self.meta["ACC_TYPE"])
...
class GeneratedCodeCache:
    ...
    def get_entry(self, cache_key: Optional[str]) -> Optional[GeneratedCodeCacheEntry]:
        ...
        if entry is None:
            smith._dynamo.utils.counters["inductor"]["generated_module_cache_miss"] += 1
        else:
            smith._dynamo.utils.counters["inductor"]["generated_module_cache_hit"] += 1
        return entry
...
class TritonTemplate(KernelTemplate):
    ...
    def generate_and_load(
        ...
        caching_enabled = (
            generate_with_caching
            and smith._inductor.config.enable_caching_generated_triton_templates
        )
    ...
    def generate(  # type: ignore[override]
        ...
        **kwargs,
        ...
        if smith.cuda.is_available() and not smith.cuda.is_tf32_supported():
...
def get_mm_log_filename() -> Optional[str]:
    mm_file_name = os.environ.get("SMITHINDUCTOR_MM_LOGGING_FILE", None)
    if not mm_file_name:
        return None
...
def get_flex_attention_log_filename() -> Optional[str]:
    flex_attention_file_name = os.environ.get(
        "SMITHINDUCTOR_FLEX_ATTENTION_LOGGING_FILE", None
    )
...
def get_num_workers() -> int:
    if "SMITHINDUCTOR_COMPILE_THREADS" in os.environ:
        return int(os.environ["SMITHINDUCTOR_COMPILE_THREADS"])
    ...
    if (
        ...
        cpu_count = cpu_count // smith.cuda.device_count()
...
class AlgorithmSelectorCache(PersistentCache):
    ...
    def __call__(
        ...
        # generating a smith.Tensor for that input from the
        # corresponding ir.Buffer. if passed for a given
        # arg, the function will be called instead of
        # generating a random smith.Tensor for benchmarking.
        input_gen_fns: Optional[dict[int, Callable[[ir.Buffer], smith.Tensor]]] = None,
        ...
        if config.autotune_in_subproc:
            ...
            smith._inductor.autotune_process.get_tuning_process_pool()
        ...
        if return_multi_template and (config.max_autotune or config.max_autotune_gemm):
            ...
            return smith._inductor.ir.TensorBox.create(
                smith._inductor.ir.MultiTemplateBuffer(
                    layout,
                    input_nodes,
                    get_timings,
                    choices,
    ...
    def do_autotuning(
        ...
        Args:
            ...
            input_gen_fns: Optional dict mapping argument indices to functions that generate
                smith.Tensor inputs from ir.Buffer for benchmarking. If provided, these are
    ...
    def create_no_valid_choices(self, name: str, reason: str) -> NoValidChoicesError:
        ...
        return NoValidChoicesError(
            ...
            "config (defined in smith/_inductor/config.py) to allow at least one choice. "
        )
    ...
    def get_inputs(
        ...
        layout: ir.Layout,
        input_gen_fns: Optional[dict[int, Callable[[ir.Buffer], smith.Tensor]]],
        hint_override: Optional[int] = None,
    ) -> AutotuneArgs:
        """
        ...
        for i, input_node in enumerate(input_nodes):
            if unique_example_inputs[input_node.get_name()].is_mkldnn:
                ...
                needed_size = smith._prims_common.compute_required_storage_length(
                ...
                example_inputs_extern.append(
                    smith.as_strided(base, sizes, strides, storage_offset)
                )
        ...
        needed_out_size = smith._prims_common.compute_required_storage_length(
        ...
        out_extern = smith.as_strided(out_base, out.size(), out.stride(), out_offset)
        expected = None
        if VERIFY:
    ...
    def _run_collective_benchmark(
        ...
        for i in range(nruns):
            ...
            start_evt = smith.cuda.Event(enable_timing=True)
            end_evt = smith.cuda.Event(enable_timing=True)
    ...
    def benchmark_collective_choice(
        ...
        try:
            ...
            time_tensor = smith.tensor(
                [avg_time], dtype=smith.float32, device=f"cuda:{rank}"
            )
    ...
    def benchmark_choices(
        ...
        for choice in choices:
            try:
                ...
                from smith._inductor.codegen.cutlass.cuda_kernel import (
                    CUDATemplateCaller,
                )
                ...
                if "invalid argument" in msg:
                    ...
                    msg += "\nPlease try re-running with SMITHINDUCTOR_AUTOTUNE_IN_SUBPROC=1.\n\n"
    ...
    def get_flex_attention_choice_info(
        ...
        if isinstance(choice, smith._inductor.select_algorithm.ExternKernelCaller):
            return {"type": "extern", "time": timings[choice]}

        assert isinstance(choice, smith._inductor.select_algorithm.TritonTemplateCaller)
    ...
    def benchmark_example_value(node, hint_override: Optional[int] = None):
        """
        Convert an ir.Buffer into a concrete smith.Tensor we can use for
        benchmarking.
        """
...
def autotune_select_algorithm(*args, **kwargs):
    ...
    if "return_multi_template" not in kwargs:
        kwargs["return_multi_template"] = (
            smith._inductor.config.benchmark_epilogue_fusion
            or smith._inductor.config.pipeline_max_autotune_gemm
        )
