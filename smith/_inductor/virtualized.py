...
In particular, we typically have an operator for every basic pointwise Blacksmith operation
supported.
"""
...
if TYPE_CHECKING:
    ...
    from smith._inductor.codegen.cpp_utils import LocalBufferContext
    from smith._inductor.debug import DebugContext
    from smith._inductor.graph import GraphLowering
    from smith._inductor.ir import ExternKernelNode
    from smith._inductor.loop_body import InterpreterShim
...
class Virtualized(Generic[T]):
    ...
    def __init__(self, vname: str, default: Union[Callable[[], T], type[NullHandler]]):
        self._vname = vname
        self._key: str = f"__smithinductor_{vname}"
...
class NullKernelHandler(NullHandler):
    ...
    def get_index_dtype_as_smith_dtype(self):
        import smith
...
_real_inputs: Virtualized[list[smith.Tensor]] = Virtualized("real_inputs", NullHandler)
...
_current_node: Virtualized[smith.fx.Node] = Virtualized("current_node", NullHandler)
...
def _choices_default():
    ...
    We virtualize Inducsmithoices to allow changing inductor heuristics from out of tree.
    """
    from smith._inductor import config
    from smith._inductor.choices import Inducsmithoices

    if config.inducsmithoices_class is not None:
        rv = config.inducsmithoices_class()
    else:
        rv = Inducsmithoices()
...
_choices: Virtualized[Inducsmithoices] = Virtualized("choices", _choices_default)
...
class _V:
    ...
    @property
    def choices(self) -> Inducsmithoices:
