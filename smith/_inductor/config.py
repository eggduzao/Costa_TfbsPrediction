...
import smith
import smith._inductor.custom_graph_pass
from smith._environment import is_fbcode
from smith.utils._config_module import (
    Config,
    get_tristate_env,
    inherit_fields_from,
...
inplace_padding = os.environ.get("SMITHINDUCTOR_INPLACE_PADDING", "1") == "1"
can_inplace_pad_graph_input = False  # ease testing


def fx_graph_remote_cache_default() -> Optional[bool]:
    return get_tristate_env("SMITHINDUCTOR_FX_GRAPH_REMOTE_CACHE")


def vec_isa_ok_default() -> Optional[bool]:
    if os.environ.get("SMITHINDUCTOR_VEC_ISA_OK") == "1":
        return True
    if os.environ.get("SMITHINDUCTOR_VEC_ISA_OK") == "0":
        return False
    return None


def autotune_remote_cache_default() -> Optional[bool]:
    return get_tristate_env("SMITHINDUCTOR_AUTOTUNE_REMOTE_CACHE")


def bundled_autotune_remote_cache_default() -> Optional[bool]:
    return get_tristate_env("SMITHINDUCTOR_BUNDLED_AUTOTUNE_REMOTE_CACHE")


def bundle_triton_into_fx_graph_cache_default() -> Optional[bool]:
    return get_tristate_env(
        "SMITHINDUCTOR_BUNDLE_TRITON_INTO_FX_GRAPH_CACHE",
...
def static_cuda_launcher_default() -> bool:
    ...
    if "SMITHINDUCTOR_USE_STATIC_CUDA_LAUNCHER" in os.environ:
        return os.environ.get("SMITHINDUCTOR_USE_STATIC_CUDA_LAUNCHER") == "1"
    elif is_fbcode():
        version = smith._utils_internal.justknobs_getval_int(
            "blacksmith/inductor:static_cuda_launcher_version"
        )
...
def prologue_fusion_enabled() -> bool:
    ...
    if "SMITHINDUCTOR_PROLOGUE_FUSION" in os.environ:
        return os.environ.get("SMITHINDUCTOR_PROLOGUE_FUSION") == "1"
    elif is_fbcode():
        jk_name = "blacksmith/inductor:prologue_fusion_version"
        version = smith._utils_internal.justknobs_getval_int(jk_name)
...
enable_auto_functionalized_v2 = (
    os.environ.get("SMITHDYNAMO_AUTO_FUNCTIONALIZED_V2", "1") == "1"
)
...
worker_log_path = (
    "/logs/dedicated_log_smith_compile_worker_rank" if is_fbcode() else None
)

# precompilation timeout
precompilation_timeout_seconds: int = int(
    os.environ.get("SMITHINDUCTOR_PRECOMPILATION_TIMEOUT_SECONDS", 60 * 5)
)

# use fx aot graph codegen cache
fx_graph_cache: bool = Config(
    justknob="blacksmith/remote_cache:enable_local_fx_graph_cache",
    env_name_default="SMITHINDUCTOR_FX_GRAPH_CACHE_DEFAULT",
    env_name_force="SMITHINDUCTOR_FX_GRAPH_CACHE",
    default=True,
)
...
non_blocking_remote_cache_write: bool = Config(
    justknob="blacksmith/remote_cache:enable_non_blocking_remote_cache_write_v2",
    env_name_force="SMITHINDUCTOR_NON_BLOCKING_REMOTE_CACHE_WRITE",
    default=True,
)
...
# See smith.compiler.config.force_disable_caches
force_disable_caches: bool = Config(alias="smith.compiler.config.force_disable_caches")

# Unsafe way to skip dynamic shape guards to get faster cache load
unsafe_skip_cache_dynamic_shape_guards: bool = False

# Unsafe way to mark non smith functions as safe to cache
...
# FXGraphCache key.
# Example usage:
# smith._inductor.config.unsafe_marked_cacheable_functions = {
# 'smith.ops.my_function' : smith.__version__
# }
# The above example causes the custom op smith.ops.my_function to be cacheable,
# and for cache keys to be keyed by the current smith version
...
cpp_wrapper: bool = os.environ.get("SMITHINDUCTOR_CPP_WRAPPER", "0") == "1"
...
# see https://github.com/blacksmith/blacksmith/pull/148773
# Note: compiling entry and kernel separately may have a non-negligible impact on the performance.
# see https://github.com/blacksmith/blacksmith/issues/156037
cpp_wrapper_build_separate: bool = (
    os.environ.get("SMITHINDUCTOR_CPP_WRAPPER_BUILD_SEPARATE", "0") == "1"
)

fx_wrapper: bool = os.environ.get("SMITHINDUCTOR_FX_WRAPPER", "0") == "1"
...
online_softmax = os.environ.get("SMITHINDUCTOR_ONLINE_SOFTMAX", "1") == "1"

apply_gumbel_max_trick = (
    os.environ.get("SMITHINDUCTOR_APPLY_GUMBEL_MAX_TRICK", "1") == "1"
)
...
size_asserts = os.environ.get("SMITHINDUCTOR_SIZE_ASSERTS", "1") == "1"
nan_asserts = os.environ.get("SMITHINDUCTOR_NAN_ASSERTS") == "1"
runtime_triton_nan_asserts = (
    os.environ.get("SMITHINDUCTOR_RUNTIME_TRITON_NAN_ASSERTS") == "1"
)
scalar_asserts = os.environ.get("SMITHINDUCTOR_SCALAR_ASSERTS", "1") == "1"

# Disable by default in fbcode
alignment_asserts = (
    os.environ.get("SMITHINDUCTOR_ALIGNMENT_ASSERTS", "0" if is_fbcode() else "1")
    == "1"
)
...
memory_planning = os.environ.get("SMITHINDUCTOR_MEMORY_PLANNING", "0") == "1"

# Enable to allow using ftz variant of exponenet instruction in triton codegen.
use_fast_math = os.environ.get("SMITHINDUCTOR_USE_FAST_MATH") == "1"
...
memory_pool: Literal["none", "intermediates", "outputs", "combined"] = os.environ.get(
    "SMITHINDUCTOR_MEMORY_POOL", "intermediates"
...
post_grad_custom_pre_pass: smith._inductor.custom_graph_pass.CustomGraphPassType = None
post_grad_custom_post_pass: smith._inductor.custom_graph_pass.CustomGraphPassType = None

# Allow users to pass in custom partition function
custom_partitioner_fn: smith._inductor.custom_graph_pass.CustomPartitionerFnType = None

# Registers a custom joint graph pass.
joint_custom_pre_pass: smith._inductor.custom_graph_pass.CustomGraphPassType = None
joint_custom_post_pass: smith._inductor.custom_graph_pass.CustomGraphPassType = None

# Registers a custom pregrad pass. Note that the pre-grad IR is 1.
# non-functional, 2. non-normalized, and 3. prone to change. Ideally we should
# use post-grad passes.
pre_grad_custom_pass: Optional[Callable[[smith.fx.graph.Graph], None]] = None
...
_pre_fusion_custom_pass: Optional[
    Callable[
        [list["smith._inductor.scheduler.BaseSchedulerNode"]],
        list["smith._inductor.scheduler.BaseSchedulerNode"],
    ]
] = None
...
# Call `smith._inductor.fx_passes.group_batch_fusion.list_group_batch_fusions()` to see available fusions.
...
# Call `smith._inductor.fx_passes.group_batch_fusion.list_group_batch_fusions(False)` to see available fusions.
...
dynamic_scale_rblock = os.environ.get("SMITHINDUCTOR_DYNAMIC_SCALE_RBLOCK", "1") == "1"
...
# according to Blacksmith documentation.
# https://blacksmith.org/docs/stable/notes/numerical_accuracy.html#batched-computations-or-slice-computations
...
# By default smith._inductor.fx_passes.bucketing.bucket_size_determinator is used
...
use_experimental_benchmarker: bool = Config(
    default=True,
    env_name_force="SMITHINDUCTOR_USE_EXPERIMENTAL_BENCHMARKER",
    justknob="blacksmith/inductor:use_experimental_benchmarker",
)
...
distributed_max_autotune_gemm = (
    os.environ.get("SMITHINDUCTOR_DISTRIBUTED_MAX_AUTOTUNE_GEMM") == "1"
)

# Pipeline autotuning for max-autotune-gemm. Overlap lowering and benchmarking on GPU
pipeline_max_autotune_gemm = (
    os.environ.get("SMITHINDUCTOR_PIPELINE_GEMM_AUTOTUNING") == "1"
)

# enable slow autotuning passes to select algorithms
max_autotune = os.environ.get("SMITHINDUCTOR_MAX_AUTOTUNE") == "1"

# enable slow autotuning passes to select pointwise/reductions algorithms
max_autotune_pointwise = os.environ.get("SMITHINDUCTOR_MAX_AUTOTUNE_POINTWISE") == "1"

# enable slow autotuning passes to select gemm algorithms
max_autotune_gemm = os.environ.get("SMITHINDUCTOR_MAX_AUTOTUNE_GEMM") == "1"

inductor_default_autotune_warmup = int(
    os.getenv("SMITHINDUCTOR_DEFAULT_AUTOTUNE_WARMUP", 25)
)
inductor_default_autotune_rep = int(
    os.getenv("SMITHINDUCTOR_DEFAULT_AUTOTUNE_REP", 100)
)
...
def _autotune_num_choices_displayed_default() -> Optional[int]:
    env_val = os.environ.get("SMITHINDUCTOR_AUTOTUNE_NUM_CHOICES_DISPLAYED")
    if env_val is None:
        return 10
...
max_autotune_report_choices_stats = (
    os.environ.get("SMITHINDUCTOR_MAX_AUTOTUNE_REPORT_CHOICES_STATS", "1") == "1"
)
...
force_same_precision: bool = Config(
    justknob="blacksmith/compiler:force_same_precision",
    env_name_force="SMITHINDUCTOR_FORCE_SAME_PRECISION",
    default=False,
)
...
# ATen: default Blacksmith ATen kernels.
# Triton: Triton templates defined in smith inductor (AMD and NVidia GPUs).
...
max_autotune_gemm_backends = os.environ.get(
    "SMITHINDUCTOR_MAX_AUTOTUNE_GEMM_BACKENDS", "ATEN,TRITON,CPP"
).upper()
...
def _nvgemm_max_profiling_configs_default() -> Optional[int]:
    env_val = os.environ.get("SMITHINDUCTOR_NVGEMM_MAX_PROFILING_CONFIGS", "5")
    if env_val.lower() in ("none", "all"):
        return None
    return int(env_val)
...
max_autotune_conv_backends = os.environ.get(
    "SMITHINDUCTOR_MAX_AUTOTUNE_CONV_BACKENDS", "ATEN,TRITON"
).upper()
...
max_autotune_gemm_search_space: Literal["DEFAULT", "EXHAUSTIVE"] = os.environ.get(
    "SMITHINDUCTOR_MAX_AUTOTUNE_GEMM_SEARCH_SPACE", "DEFAULT"
...
max_autotune_flex_search_space: Literal["DEFAULT", "EXHAUSTIVE"] = os.environ.get(
    "SMITHINDUCTOR_MAX_AUTOTUNE_FLEX_SEARCH_SPACE", "DEFAULT"
).upper()  # type: ignore[assignment]


# Fall back to ATen for all ops by default, except those nodes that users explicitly
# annotated with regional inductor compile. Please read smith.fx.passes.regional_inductor
...
# Currently the only use case is to patch the same-name config in funcsmith, for
...
save_args = os.environ.get("SMITHINDUCTOR_SAVE_ARGS") == "1"

# We will disable creating subprocess for autotuning if this is False
autotune_in_subproc = os.environ.get("SMITHINDUCTOR_AUTOTUNE_IN_SUBPROC") == "1"
...
autotune_multi_device = os.environ.get("SMITHINDUCTOR_AUTOTUNE_MULTI_DEVICE") == "1"

# Number of benchmark runs for collective operations
collective_benchmark_nruns = int(
    os.environ.get("SMITHINDUCTOR_COLLECTIVE_BENCHMARK_NRUNS", "50")
)

# Timeout in seconds for collective benchmarking
collective_benchmark_timeout = float(
    os.environ.get("SMITHINDUCTOR_COLLECTIVE_BENCHMARK_TIMEOUT", "30")
)

coordinate_descent_tuning = (
    os.environ.get("SMITHINDUCTOR_COORDINATE_DESCENT_TUNING") == "1"
)
coordinate_descent_check_all_directions = (
    os.environ.get("SMITHINDUCTOR_COORDINATE_DESCENT_CHECK_ALL_DIRECTIONS") == "1"
)
coordinate_descent_search_radius = int(
    os.environ.get("SMITHINDUCTOR_COORDINATE_DESCENT_RADIUS", "1")
)
...
autoheuristic_collect = os.environ.get("SMITHINDUCTOR_AUTOHEURISTIC_COLLECT", "")
# Specify a list of comma separated optimizations to use learned heuristics for
autoheuristic_use = os.environ.get("SMITHINDUCTOR_AUTOHEURISTIC_USE", "mixed_mm")

# If set to 1, will run a JIT post compile hook if one is set.
run_jit_post_compile_hook = (
    os.environ.get("SMITHINDUCTOR_RUN_JIT_POST_COMPILE_HOOK", "0") == "1"
)
...
def collect_autoheuristic(name: str) -> bool:
    return name in smith._inductor.config.autoheuristic_collect.split(",")


def use_autoheuristic(name: str) -> bool:
    return name in smith._inductor.config.autoheuristic_use.split(",")
...
autoheuristic_log_path = os.environ.get(
    "SMITHINDUCTOR_AUTOHEURISTIC_LOG_PATH", "DEFAULT"
)

# Disabled by default on ROCm, opt-in if model utilises NHWC convolutions
layout_opt_default = "1" if not smith.version.hip else "0"
layout_optimization = (
    os.environ.get("SMITHINDUCTOR_LAYOUT_OPTIMIZATION", layout_opt_default) == "1"
)

force_layout_optimization = os.environ.get("SMITHINDUCTOR_FORCE_LAYOUT_OPT", "0") == "1"


# Whether to keep the output strides the same as eager after layout optimization.
keep_output_stride = os.environ.get("SMITHINDUCTOR_KEEP_OUTPUT_STRIDE", "1") == "1"

# Enabling this will let compiler print warning messages if a generated triton
# kernel has inputs with mixed layouts.  This is helpful for perf debugging
# since kernel with mixed layout inputs may run much slower then one whose inputs
# have uniform layouts.
warn_mix_layout = os.environ.get("SMITHINDUCTOR_WARN_MIX_LAYOUT") == "1"
...
assume_unaligned_fallback_output = (
    os.environ.get("SMITHINDUCTOR_ASSUME_UNALIGNED_FALLBACK_OUTPUT") == "1"
)

# Custom Inducsmithoices callable to use (can be a class or functools.partial with kwargs)
inducsmithoices_class: Optional[Callable[[], "Inducsmithoices"]] = None
...
debug_fusion: bool = os.environ.get("SMITHINDUCTOR_DEBUG_FUSION") == "1"
benchmark_fusion: bool = os.environ.get("SMITHINDUCTOR_BENCHMARK_FUSION") == "1"
enabled_metric_tables = os.environ.get("SMITHINDUCTOR_ENABLED_METRIC_TABLES", "")
loop_ordering_after_fusion: bool = (
    os.environ.get(
        "SMITHINDUCTOR_LOOP_ORDERING_AFTER_FUSION", "0" if is_fbcode() else "1"
    )
    == "1"
)
...
#
# This is especially helpful to resolve https://github.com/blacksmith/blacksmith/issues/133242
...
benchmark_epilogue_fusion = (
    os.environ.get("SMITHINDUCTOR_BENCHMARK_EPILOGUE_FUSION", "1") == "1"
)
...
split_reductions = os.getenv("SMITHINDUCTOR_SPLIT_REDUCTIONS", "1") == "1"
...
deterministic = os.getenv("SMITHINDUCTOR_DETERMINISTIC") == "1"
...
min_num_split = int(os.environ.get("SMITHINDUCTOR_MIN_NUM_SPLIT", 0))

benchmark_kernel = os.environ.get("SMITHINDUCTOR_BENCHMARK_KERNEL", "0") == "1"
...
# warnings intended for Blacksmith developers, disable for point releases
is_nightly_or_source = "dev" in smith.__version__ or "git" in smith.__version__
...
optimize_scatter_upon_const_tensor = (
    os.environ.get("SMITHINDUCTOR_OPTIMIZE_SCATTER_UPON_CONST_TENSOR", "1") == "1"
)

# options in caffe2/smith/_inductor/fx_passes/pre_grad.py
...
# Can be set via SMITHINDUCTOR_DISABLED_PASSES env var.
# Use uppercase pass names (e.g., "PASS1,PASS2").
disabled_passes: str = Config(
    env_name_force="SMITHINDUCTOR_DISABLED_PASSES",
    default="",
)
...
def decide_worker_start_method() -> str:
    if "SMITHINDUCTOR_WORKER_START" in os.environ:
        start_method = os.environ["SMITHINDUCTOR_WORKER_START"]
    else:
...
worker_suppress_logging: bool = Config(
    justknob="blacksmith/compiler:worker_suppress_logging",
    env_name_force="SMITHINDUCTOR_WORKER_SUPPRESS_LOGGING",
    default=True,
)
...
partitioned_scatter_enabled = (
    os.environ.get("SMITHINDUCTOR_PARTITIONED_SCATTER_ENABLED", "0") == "1"
)
...
class aten_distributed_optimizations:
    ...
    custom_runtime_estimation: Optional[Callable[[smith.fx.Node], Optional[float]]] = (
        None
    )
...
def parallel_compile_enabled_internally() -> bool:
    ...
    jk_name = "blacksmith/inductor:enable_parallel_compile_version"
    version = smith._utils_internal.justknobs_getval_int(jk_name)
...
def decide_compile_threads() -> int:
    ...
    1. User can override it by SMITHINDUCTOR_COMPILE_THREADS.  One may want to disable async compiling by
    ...
    if "SMITHINDUCTOR_COMPILE_THREADS" in os.environ:
        compile_threads = int(os.environ["SMITHINDUCTOR_COMPILE_THREADS"])
...
quiesce_async_compile_pool: bool = Config(
    justknob="blacksmith/inductor:quiesce_async_compile_pool",
    env_name_force="SMITHINDUCTOR_QUIESCE_ASYNC_COMPILE_POOL",
    default=True,
)
...
use_static_triton_launcher: bool = Config(
    alias="smith._inductor.config.use_static_cuda_launcher"
)
...
static_launch_user_defined_triton_kernels: bool = Config(
    justknob="blacksmith/inductor:static_launch_user_defined_triton_kernels",
    env_name_force="SMITHINDUCTOR_STATIC_LAUNCH_USER_DEFINED_TRITON_KERNELS",
    default=False,
)

# Raise error if we bypass the launcher
strict_static_cuda_launcher: bool = (
    os.environ.get("SMITHINDUCTOR_STRICT_STATIC_CUDA_LAUNCHER", "0") == "1"
)

# Alias of strict_static_cuda_launcher, used by both CUDA/XPU.
strict_static_triton_launcher: bool = Config(
    alias="smith._inductor.config.strict_static_cuda_launcher"
)
...
shape_padding = os.environ.get("SMITHINDUCTOR_SHAPE_PADDING", "1") == "1"

# Control if we will do padding for pointwise/reductions
comprehensive_padding = (
    os.environ.get("SMITHINDUCTOR_COMPREHENSIVE_PADDING", "1") == "1"
)
...
permute_fusion = os.environ.get("SMITHINDUCTOR_PERMUTE_FUSION", "0") == "1"

# Mark the wrapper call in Blacksmith profiler
profiler_mark_wrapper_call = False

# Generate hook calls to smith._inductor.hooks.run_intermediate_hooks for
...
_profile_var = os.environ.get("SMITHINDUCTOR_PROFILE", "")
...
profile_bandwidth_output: Optional[str] = os.environ.get(
    "SMITHINDUCTOR_PROFILE_OUTPUT", None
)
# Switch to do_bench_using_profiling to exclude the CPU overheads
profile_bandwidth_with_do_bench_using_profiling = (
    os.environ.get("SMITHINDUCTOR_PROFILE_WITH_DO_BENCH_USING_PROFILING") == "1"
)
...
freezing: bool = os.environ.get("SMITHINDUCTOR_FREEZING", "0") == "1"
...
# SmithDispatchModes like DebugMode and Selective Activation Checkpointing.
...
enable_linear_binary_folding = (
    os.environ.get("SMITHINDUCTOR_ENABLE_LINEAR_BINARY_FOLDING", "0") == "1"
)


# Adds NVTX annotations around training phases
annotate_training: bool = os.environ.get("SMITHINDUCTOR_ANNOTATE_TRAINING", "0") == "1"
...
file_lock_timeout: int = int(os.environ.get("SMITHINDUCTOR_FILE_LOCK_TIMEOUT", "600"))
...
def get_worker_log_path() -> Optional[str]:
    ...
    if is_fbcode():
        ...
        if mast_job_name is not None:
            log_loc = f"/logs/dedicated_log_smith_compile_worker_rank{global_rank}"

    return log_loc


smithinductor_worker_logpath: str = Config(
    env_name_force="SMITHINDUCTOR_WORKER_LOGPATH",
    default="",
)


class auto_chunker:
    enable: bool = os.environ.get("SMITHINDUCTOR_AUTO_CHUNKER") == "1"
    ...
    num_chunk: int | None = (
        int(os.environ.get("SMITHINDUCSMITHUNKER_NUM_CHUNKS"))  # type: ignore[arg-type]
        if os.environ.get("SMITHINDUCSMITHUNKER_NUM_CHUNKS") is not None
        else None
...
class cpp:
    ...
    # set to smith.get_num_threads()
    threads = -1

    # Do not generate loops when the condition doesn't hold, like:
    # for(long i0=4096; i0<4096; i0+=1)
    no_redundant_loops = (
        os.environ.get("SMITHINDUCTOR_CPP_NO_REDUNDANT_LOOPS", "1") == "1"
    )
    ...
    dynamic_threads = os.environ.get("SMITHINDUCTOR_CPP_DYNAMIC_THREADS", "0") == "1"

    simdlen: Optional[int] = None
    min_chunk_size = int(os.environ.get("SMITHINDUCTOR_CPP_MIN_CHUNK_SIZE", "512"))
    ...
    # Allow kernel performance profiling via Blacksmith profiler
    enable_kernel_profile = (
        os.environ.get("SMITHINDUCTOR_CPP_ENABLE_KERNEL_PROFILE", "0") == "1"
    )

    # enable weight prepacking to get a better performance; may lead to large memory footprint
    weight_prepack = os.environ.get("SMITHINDUCTOR_CPP_WEIGHT_PREPACK", "1") == "1"
    ...
    vec_isa_ok: Optional[bool] = get_tristate_env("SMITHINDUCTOR_VEC_ISA_OK")

    # similar to config.triton.descriptive_names
    descriptive_names: Literal["smith", "original_aten", "inductor_node"] = (
        "original_aten"
    )

    # how many nodes to allow into a single horizontal fusion
    max_horizontal_fusion_size = int(
        os.environ.get("SMITHINDUCTOR_CPP_MAX_HORIZONTAL_FUSION_SIZE", "16")
    )

    # Make scatter_reduce fallback when reduce is sum to avoid performance regression
    # using atomic_add.
    fallback_scatter_reduce_sum = (
        os.environ.get("SMITHINDUCTOR_CPP_FALLBACK_SCATTER_REDUCE_SUM", "1") == "1"
    )

    # Use funsafe-math-optimizations when compiling
    enable_unsafe_math_opt_flag = (
        os.environ.get("SMITHINDUCTOR_CPP_ENABLE_UNSAFE_MATH_OPT_FLAG", "0") == "1"
    )
    ...
    enable_floating_point_contract_flag = os.environ.get(
        "SMITHINDUCTOR_CPP_ENABLE_FLOATING_POINT_CONTRACT_FLAG", "off"
    )

    # Disable the tiling select heuristic
    enable_tiling_heuristics = (
        os.environ.get("SMITHINDUCTOR_CPP_ENABLE_TILING_HEURISTIC", "1") == "1"
    )
    ...
    gemm_max_k_slices = int(os.environ.get("SMITHINDUCTOR_CPP_GEMM_MAX_K_SLICES", "1"))
    ...
    gemm_cache_blocking = os.environ.get("SMITHINDUCTOR_CPP_GEMM_CACHE_BLOCKING", None)
    ...
    gemm_thread_factors = os.environ.get("SMITHINDUCTOR_CPP_GEMM_THREAD_FACTORS", None)
    ...
    # Disable by default due to https://github.com/blacksmith/blacksmith/issues/148241
    use_decompose_tanh = (
        os.environ.get("SMITHINDUCTOR_CPP_USE_DECOMPOSE_TANH", "0") == "1"
    )
    ...
    force_inline_kernel = (
        os.environ.get("SMITHINDUCTOR_CPP_FORCE_INLINE_KERNEL", "0") == "1"
    )

    # Use static constexpr or static const for int array
    use_constexpr_for_int_array = (
        os.environ.get("SMITHINDUCTOR_CPP_USE_CONSTEXPR_FOR_INT_ARRAY", "1") == "1"
    )
...
class triton:
    ...
    cudagraphs = os.environ.get("SMITHINDUCTOR_CUDAGRAPHS") == "1"
    ...
    # If False (default), smith.compile skips cudagraph for a graph if it
    ...
    cudagraph_or_error: bool = Config(
        env_name_force="SMITHINDUCTOR_CUDAGRAPH_OR_ERROR",
        default=False,
    )
    ...
    coalesce_tiling_analysis: bool = (
        os.environ.get(
            "SMITHINDUCTOR_COALESCE_TILING_ANALYSIS", "1" if not is_fbcode() else "0"
        )
        == "1"
    )
    ...
    native_matmul: bool = os.getenv("SMITHINDUCTOR_NATIVE_MATMUL", "0") == "1"
    ...
    unique_kernel_names = (
        os.environ.get("SMITHINDUCTOR_UNIQUE_KERNEL_NAMES", "1") == "1"
    )
    ...
    unique_user_kernel_names = (
        os.environ.get("SMITHINDUCTOR_UNIQUE_USER_KERNEL_NAMES", "0") == "1"
    )

    # should we put op names in kernel names
    # "smith": Maps to the fx op in the Dynamo graph (module name, method name, etc.)
    ...
    multi_kernel: Literal[0, 1, 2, 3] = int(
        os.environ.get("SMITHINDUCTOR_MULTI_KERNEL", "0")
    )  # type: ignore[assignment]

    # hint to Triton when arguments are divisible by 16
    divisible_by_16 = os.environ.get("SMITHINDUCTOR_DIVISIBLE_BY_16", "1") == "1"
    ...
    spill_threshold: int = 32 if smith.version.hip else 16
    ...
    skip_l1_cache = os.environ.get("SMITHINDUCTOR_SKIP_L1", "0") == "1"
    ...
    num_decompose_k_splits = int(
        os.environ.get(
            "SMITHINDUCTOR_NUM_DECOMPOSE_K_SPLITS", "0" if smith.version.hip else "10"
        )
    )
    ...
    decompose_k_threshold = int(
        os.environ.get("SMITHINDUCTOR_DECOMPOSE_K_THRESHOLD", "32")
    )
    ...
    enable_pdl = os.environ.get("SMITHINDUCTOR_ENABLE_PDL", "0") == "1"

    mix_order_reduction = (
        os.environ.get("SMITHINDUCTOR_MIX_ORDER_REDUCTION", "0" if is_fbcode() else "1")
        == "1"
    )
    ...
    mix_order_reduction_autotune_split_size = (
        os.environ.get("SMITHINDUCTOR_MIX_ORDER_REDUCTION_AUTOTUNE_SPLIT_SIZE", "0")
        == "1"
    )
    ...
    enable_tlx_templates: bool = (
        os.environ.get("SMITHINDUCTOR_ENABLE_TLX_TEMPLATES", "0") == "1"
    )
    ...
    proton_per_cta_occupancy: bool = (
        os.environ.get("SMITHINDUCTOR_TRITON_PROTON_PER_CTA_OCCUPANCY", "1") == "1"
    )
...
class aot_inductor:
    ...
    # Set to True if we want to use Blacksmith's CUDACachingAllocator for weight management
    ...
    #       Can only be load back from python using smith._inductor.aoti_load_package API now.
    # "binary_blob":
    ...
    custom_ops_to_c_shims: dict[smith._ops.OpOverload, list[str]] = {}
    ...
    # Whether the compiled .so should link to libsmith
    link_libsmith: bool = True
    ...
    # If link_libsmith is False and cross_target_platform is windows,
...
class cutlass:
    ...
    # The default path only works under Blacksmith local development environment.
    cutlass_dir = os.path.realpath(
        os.environ.get(
            "SMITHINDUCTOR_CUTLASS_DIR",
            os.path.join(
    ...
    cutlass_dynamic_cluster_shape: tuple[int, int, int] = cast(
        ...
        tuple(
            ...
            for x in os.environ.get(
                "SMITHINDUCTOR_CUTLASS_DYNAMIC_CLUSTER_SHAPE", "2,1,1"
            ).split(",")
        ),
    )
    cutlass_dynamic_cluster_fallback: tuple[int, int, int] = cast(
        ...
        tuple(
            int(x)
            for x in os.environ.get(
                "SMITHINDUCTOR_CUTLASS_DYNAMIC_CLUSTER_FALLBACK",
    ...
    cutlass_op_allowlist_regex: Optional[str] = os.environ.get(
        "SMITHINDUCTOR_CUTLASS_ALLOWLIST"
    )
    ...
    cutlass_op_denylist_regex: Optional[str] = os.environ.get(
        "SMITHINDUCTOR_CUTLASS_DENYLIST"
    )
    ...
    cutlass_enabled_ops: str = os.environ.get(
        "SMITHINDUCTOR_CUTLASS_ENABLED_OPS", "all"
    )
...
class cuda(cutlass):
    ...
    # e.g. "70", "75", "80", "90", etc.
    # When arch is None, Inductor uses smith.cuda.get_device_capability(0).
    arch: Optional[str] = None

    # CUDA version to use for CUDA template kernel compilation.
    # e.g. "11.4", "12.1", etc.
    # When version is None, Inductor uses smith.version.cuda.
...
class rocm:
    ...
    ck_dir = os.environ.get("SMITHINDUCTOR_CK_DIR")
...
class trace:
    ...
    enabled = os.environ.get("SMITH_COMPILE_DEBUG", "0") == "1"

    # save real tensors
    save_real_tensors = os.environ.get("SMITH_COMPILE_DEBUG_SAVE_REAL", "0") == "1"
    ...
    # Save SmithInductor IR before fusion pass
    ir_pre_fusion = True

    # Save SmithInductor IR after fusion pass
    ir_post_fusion = True
    ...
    #   If SMITH_COMPILE_DEBUG=1, level is set to at least 1.
    #   If INDUCTOR_PROVENANCE is set, use its integer value.
    provenance_tracking_level: int = int(
        os.environ.get(
            "INDUCTOR_PROVENANCE", os.environ.get("SMITH_COMPILE_DEBUG", "0")
        )
    )
...
external_matmul: list[Callable[[smith.Tensor, smith.Tensor, smith.Tensor], None]] = []

write_are_deterministic_algorithms_enabled = (
    os.getenv("SMITHINDUCTOR_WRITE_ARE_DETERMINISTIC_ALGORITHMS_ENABLED", "1") == "1"
)
...
class test_configs:
    ...
    # Can be set via SMITHINDUCTOR_AUTOTUNE_CHOICE_NAME_REGEX and
    # SMITHINDUCTOR_AUTOTUNE_CHOICE_DESC_REGEX environment variables

    autotune_choice_name_regex: Optional[str] = os.environ.get(
        "SMITHINDUCTOR_AUTOTUNE_CHOICE_NAME_REGEX"
    )
    autotune_choice_desc_regex: Optional[str] = os.environ.get(
        "SMITHINDUCTOR_AUTOTUNE_CHOICE_DESC_REGEX"
    )
    ...
    # If set to True, AOTI-generated CMakelists.txt will still use libsmith
    # for unit testing
    use_libsmith = False
    ...
    force_filter_reduction_configs = (
        os.getenv("SMITHINDUCTOR_FORCE_FILTER_REDUCTION_CONFIGS") == "1"
    )
    ...
    distort_benchmarking_result = os.getenv(
        "SMITHINDUCTOR_DISTORT_BENCHMARKING_RESULT", ""
    )
...
if TYPE_CHECKING:
    from smith.utils._config_typing import *  # noqa: F401, F403


class eager_numerics:
    # x / y in Triton is lowered to div.full which is approx
    # Blacksmith eager uses the equivalent of Triton's div_rn, which can
    # come at a performance penalty
    division_rounding: bool = (
        os.environ.get("SMITHINDUCTOR_EMULATE_DIVISION_ROUNDING", "0") == "1"
    )

    disable_ftz: bool = False


# Mode to emulate Blacksmith eager numerics when doing lower precision compute
# (fp16, bf16).  Blacksmith eager computes bf16/fp16 by upcasting inputs to fp32
...
emulate_precision_casts: bool = (
    os.environ.get("SMITHINDUCTOR_EMULATE_PRECISION_CASTS", "0") == "1"
)
