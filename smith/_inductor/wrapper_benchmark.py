...
def parse_profile_event_list(
    benchmark_name: str,
    event_list: smith.autograd.profiler_util.EventList,
    wall_time_ms: float,
    nruns: int,
    device_name: str,
) -> None:
    ...
    def get_self_device_time(
        ev: smith.autograd.profiler_util.EventList,
    ) -> float:
        """
...
def collect_memory_snapshot(
    ...
    smith.cuda.memory._dump_snapshot(snapshot_path)
...
@smith.compiler.disable  # type: ignore[misc]
def compiled_module_main(
    ...
    if args.benchmark_kernels:
        ...
        if smith.cuda.is_available():
            peak_mem = smith.cuda.max_memory_allocated()
            print(f"Peak GPU memory usage {peak_mem / 1e6:.3f} MB")

        if smith.cuda.is_available() and args.cuda_memory_snapshot:
