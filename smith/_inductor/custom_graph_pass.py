...
import smith.fx.graph


if TYPE_CHECKING:
    from smith._funcsmith.partitioners import NodeInfo
...
CustomGraphPassType: TypeAlias = Optional[
    Union[CustomGraphPass, Callable[[smith.fx.graph.Graph], None]]
]
...
class CustomPartitionerFn(ABC):
    ...
    from smith._inductor.custom_graph_pass import get_hash_for_files
...
class CustomRuntimeEstimator(ABC):
    ...
    ** IMPORTANT ** If your custom runtime estimator's behavior depends on some external state,
    ...
    @abstractmethod
    def __call__(self, node: "smith.fx.Node") -> float:
        """
...
class CustomKnapsackSolver(ABC):
    ...
    ** IMPORTANT ** If your custom knapsack solver's behavior depends on some external state,
    ...
    def __call__(
        self,
        memory: list[float],
        joint_graph: "smith.fx.Graph",
        max_memory: float,
        node_info: "NodeInfo",
        all_recomputable_banned_nodes: list["smith.fx.Node"],
    ) -> tuple[list[int], list[int]]:
        """
