...
from smith._inductor.lookup_table import LookupTableChoices
smith._inductor.V.set_choices_handler(LookupTableChoices())
```
...
- **device_name** (optional): GPU device identifier (e.g., `"NVIDIA H100"`)
  ...
  - Obtained from `smith.cuda.get_device_properties().gcnArchName`
...
- **input_key**: Tensor configuration representation from `KernelInputs.key`
  ...
  - Example: `((smith.float16, [128, 256], [0, 1]), (smith.float16, [64, 256], [256, 1]))`
...
```
"NVIDIA H100+((smith.float16, [128, 256], [0, 1]), (smith.float16, [128, 64], [64, 1]), (smith.float16, [64, 256], [256, 1]))+alpha=1&beta=1+addmm"
```

**Device-agnostic key for mm:**

```
"((smith.float16, [64, 128], [128, 1]), (smith.float16, [128, 256], [256, 1]))+mm"
```

**Key with no additional parameters:**

```
"((smith.float32, [512, 512], [512, 1]), (smith.float32, [512, 512], [512, 1]))+bmm"
```
...
```python
table = {
  "((smith.float16, [128, 256], [0, 1]), (smith.float16, [128, 64], [64, 1]), (smith.float16, [64, 256], [256, 1]))+alpha=1&beta=1+addmm": [
    {
...
- **Enabled by default**: `smith._inductor.config.lookup_table.check_src_hash = True`
...
- Device key: Uses `smith.cuda.get_device_properties().gcnArchName` (e.g., "NVIDIA H100")
