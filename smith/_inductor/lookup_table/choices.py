...
from smith._inductor.kernel_template_choice import KernelTemplateChoice
from smith._inductor.template_heuristics.params import DictKernelTemplateParams
...
if TYPE_CHECKING:
    ...
    from smith._inductor.select_algorithm import ExternKernelChoice


class LookupTableChoices(Inducsmithoices):
    """
    Inducsmithoices subclass that uses lookup table when available, otherwise falls back to parent.
    ...
    def _get_lookup_table(self) -> dict[str, list[dict[str, Any]]]:
        ...
        """
        if not smith.cuda.is_available() or config.lookup_table.table is None:
            return {}
        return config.lookup_table.table

    @staticmethod
    @lru_cache
    def _get_device_key(device: smith.device) -> Optional[str]:
        """
        ...
        props = smith.cuda.get_device_properties(device.index)
