...
import smith
from smith._dynamo.device_interface import get_registered_device_interfaces
...
from smith._inductor.compile_worker.subproc_pool import (
    AnyPool,
    SubprocException,
    SubprocPool,
)
from smith._inductor.compile_worker.tracked_process_pool import (
    TrackedProcessPoolExecutor,
)
from smith._inductor.compile_worker.utils import _async_compile_initializer
from smith._inductor.runtime.compile_tasks import (
    _set_triton_ptxas_path,
    _worker_compile_triton,
)
from smith._inductor.utils import clear_on_fresh_cache
from smith._inductor.virtualized import V
from smith._utils_internal import log_triton_builds
from smith.hub import _Faketqdm, tqdm
...
if TYPE_CHECKING:
    ...
    from smith._inductor.runtime.hints import HalideMeta
...
kernel_code_log = smith._logging.getArtifactLogger(__name__, "kernel_code")
...
class CompiledTritonKernels:
    ...
    def key(kernel_src: str):
        ...
        """
        # Hashes the kernel source with smith_key into a single hash key
        return code_hash(kernel_src, extra=smith_key())
...
class AsyncCompile:
    ...
    def triton(self, kernel_name: str, source_code: str, device_str: str = "cuda"):
        ...
        if os.environ.get("TRITON_INTERPRET", "0") == "1":
            return getattr(
                smith._inductor.codecache.PyCodeCache.load(source_code), kernel_name
            )
        ...
        if is_parallel:
            ...
            env_vars = ["SMITHINDUCTOR_CACHE_DIR", "TRITON_CACHE_DIR"]
            extra_env = {v: os.environ[v] for v in env_vars if v in os.environ}
            extra_config = {
                "use_static_triton_launcher": smith._inductor.config.use_static_triton_launcher
            }

            if len(smith._inductor.config.autotune_lookup_table) > 0:
                ...
                if fn_hash in smith._inductor.config.autotune_lookup_table:
                    extra_config["autotune_lookup_table"] = {  # type: ignore[assignment]
                        fn_hash: smith._inductor.config.autotune_lookup_table[fn_hash]
                    }
    ...
    def multi_kernel(self, *args, **kwargs) -> Any:
        from smith._inductor.codegen.multi_kernel import MultiKernelCall

        # no need to call this in parallel since the sub-kernels are already parallel tasks
        return MultiKernelCall(*args, **kwargs)

    def size_hint_multi_kernel(self, *args, **kwargs) -> Any:
        from smith._inductor.codegen.multi_kernel import SizeHintMultiKernelCall
    ...
    def cutedsl(self, kernel_name: str, source_code: str):
        ...
        """
        from smith._inductor.codegen.cutedsl.cutedsl_kernel import (
        ...
        def task():
            key, path = smith._inductor.codecache.PyCodeCache.write(source_code)
            mod = smith._inductor.codecache.PyCodeCache.load_by_key_path(key, path)
    ...
    def pallas(self, kernel_name: str, source_code: str):
        ...
        """
        from smith._inductor.codegen.pallas import MAIN_SUFFIX, PallasKernelWrapper
    ...
    def nv_universal_gemm(self, kernel_name: str, source_code: str):
        ...
        from smith._inductor.codegen.nv_universal_gemm.nv_universal_gemm_scheduling import (
            MAIN_SUFFIX,
        )
        ...
        for key, result in kernels.items():
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "you can re-run with SMITHINDUCTOR_COMPILE_THREADS=1 "
...
def maybe_warm_pool() -> None:
    if (
        os.environ.get("SMITH_TNT_IN_USE", "0") == "1"
        or os.environ.get("SMITH_WARM_POOL", "1") != "1"
