...
if TYPE_CHECKING:
    ...
    from smith.utils._ordered_set import OrderedSet  # isort: skip
...
class Inducsmithoices:
    ...
    You can override the choices made here by doing:
            ...
            class MyHeuristics(Inducsmithoices):
                ...

            smith._inductor.virtualized.V.set_choices_handler(MyHeuristics())
    """
    ...
    def get_flex_attention_fwd_configs(
        self, head_dim: int, dtype: smith.dtype, device_type: Optional[str] = "cuda"
    ) -> list[Any]:
        ...
        if features.get_reduction_hint() not in (
            ...
            if not all(
                (
                    ...
                    and not smith.utils._sympy.numbers.is_infinite(bound)
                )
