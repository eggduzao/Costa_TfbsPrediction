...
class OpsHandler(Generic[T]):
    """
    Protocol describing the set of valid operations on ``smith._inductor.virtualized.ops``,
    ...
    operations as defined by smith, so when in doubt about semantics, check the
    corresponding smith documentation.  These are all scalar operations (so they
    ...
    def constant(self, value: Union[bool, float, int], dtype: smith.dtype) -> T:
    ...
    def where(self, condition: T, input: T, other: T) -> T:
        """
        Computes smith.where: when condition is true, return input; otherwise return other.
        """
        raise NotImplementedError

    def index_expr(self, expr: sympy.Expr, dtype: smith.dtype) -> T:
        """
    ...
    def to_dtype(
        ...
        dtype of x was, which can improve code generation (used by smith to(dtype=dtype)).
        """
        raise NotImplementedError

    def trunc_to_int(self, x: T, dtype: smith.dtype) -> T:
        """
    ...
    def ceil_to_int(self, x: T, dtype: smith.dtype) -> T:
        """
        Convert x to dtype with ceiling semantics.  See also trunc_to_int.
        """
        raise NotImplementedError

    def floor_to_int(self, x: T, dtype: smith.dtype) -> T:
        """
        Convert x to dtype with ceiling semantics.  See also trunc_to_int.
        """
        raise NotImplementedError

    def round_to_int(self, x: T, dtype: smith.dtype) -> T:
        """
        Convert x to dtype with round-to-even semantics.  See also trunc_to_int.
        """
        raise NotImplementedError

    def to_dtype_bitcast(self, x: T, dtype: smith.dtype, src_dtype: smith.dtype) -> T:
        """
    ...
    # smith._inductor.codegen.common.CSEProxy for their typical implementation
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # The following ops have semantics that correspond exactly to the smith
    ...
    # NB: this returns a float, like the smith operation
...
class ExtractConstantsHandler(NoopHandler):
    def __init__(self, device: Optional[smith.device]):
        self.device = device

    def constant(self, value: Any, dtype: smith.dtype) -> smith._inductor.ir.Constant:
        from smith._inductor import ir

        return ir.Constant(
            value=value, dtype=dtype, device=self.device or smith.get_default_device()
        )
