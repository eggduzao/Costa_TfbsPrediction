...
import smith.fx
from smith import device, Tensor
from smith._decomp import get_decompositions
from smith._dynamo.utils import defake, dynamo_timed
...
from smith._logging import LazyString, trace_structured
...
from smith.fx.passes.reinplace import _is_view_op
...
if TYPE_CHECKING:
    ...
    from smith._higher_order_ops.effects import _EffectType
    from smith.fx import GraphModule
    from smith.fx.graph import Graph
...
perf_hint_log = smith._logging.getArtifactLogger(__name__, "perf_hints")
...
if config.is_fbcode():
    from smith._inductor.fb.utils import log_module_code
else:
...
def may_get_constant_buffer_dtype(constant_buffer: sympy.Expr) -> Optional[smith.dtype]:
    assert isinstance(
...
) -> Union[Tensor, smith._C.ScriptObject, GraphModule]:
...
def get_user_visible_output_strides(g: Graph) -> dict[Node, tuple[int, ...]]:
    ...
    if not isinstance(output_node.args[0], smith.fx.Node):
...
def extend_user_visible_output_strides(
    ...
    while queue:
        ...
        if (
            ...
            and current.args
            and isinstance(current.args[0], smith.fx.Node)
        ):
            base = current.args[0]
...
def mark_nodes_dislike_padding(
    ...
    for cur in reversed(g.nodes):
        ...
        if (
            isinstance(cur.target, smith._ops.OpOverload)
            and get_layout_constraint_tag(cur.target)
            == smith._C.Tag.needs_exact_strides
        ):
            cur.meta["dislike_padding"] = True
            continue
...
def is_mkldnn_conv(node: Node) -> bool:
    ...
    # See _register_unary_fusion_lowering in smith/_inductor/fx_passes/mkldnn_fusion.py.
    if (
        getattr(smith.ops, "mkldnn", None) is not None
        and getattr(smith.ops.mkldnn, "_convolution_pointwise", None) is not None
        ...
        for target in node.target.args[0].targets:
            if target.fns[0] in [
                smith.ops.mkldnn._convolution_pointwise.default,
                smith.ops.mkldnn._convolution_pointwise.binary,
                smith.ops.mkldnn._convolution_pointwise_.binary,
            ]:
                return True
...
class GraphLowering(smith.fx.Interpreter):
    ...
    def __init__(
        ...
        self.graph_inputs: dict[str, Union[TensorBox, SmithBindObject, sympy.Expr]] = {}
        ...
        self.constants: dict[str, smith.Tensor] = (
            const_module.constants if const_module else {}
        )
        self.named_buffers: dict[str, smith.Tensor] = (
            const_module.named_buffers if const_module else {}
        )
        self.mutated_named_buffers: OrderedSet[smith.Tensor] = gm.meta.get(
            "mutated_named_buffers", OrderedSet()
        )
        self.named_parameters: dict[str, smith.Tensor] = (
            const_module.named_parameters if const_module else {}
        )
        self.smithbind_constants: dict[
            str, Union[smith._C.ScriptObject, FakeScriptObject]
        ] = {}
        ...
        from smith._inductor.extern_node_serializer import extern_node_json_serializer
        ...
        self.current_node: smith.fx.Node = None  # type: ignore[assignment]
        ...
        self.scheduler: smith._inductor.scheduler.Scheduler = None  # type: ignore[assignment]

        # record intermediate results for input of UsedDefinedTritonKernels
        # This will be used if autotuning is done in one pass.
        self.autotuning_inputs: Optional[list[smith.Tensor]] = None
        ...
        self.current_device: Optional[smith.device] = None
        ...
        self.device_node_mapping: dict[smith.device, smith.fx.Node] = {}
        self.orig_gm: smith.fx.GraphModule = gm.__copy__()
    ...
    def symbolic_sizes_strides(
        ...
        if self.reuse_shape_env:
            ...
            # https://github.com/blacksmith/blacksmith/pull/94031#discussion_r1096044816
        ...
        r_size = [i.node.expr if isinstance(i, smith.SymInt) else i for i in size]
        r_stride = [i.node.expr if isinstance(i, smith.SymInt) else i for i in stride]
    ...
    def get_allocation_size(
        self,
        node: Union[
            ir.TensorBox, ir.StorageBox, ir.Buffer, WorkspaceArg, ir.SmithBindObject
        ],
    ) -> Sequence[Expr]:
    ...
    def get_allocation_storage_size(
        self, node: Union[ir.Buffer, WorkspaceArg, ir.SmithBindObject]
    ) -> Expr:
    ...
    def has_feature(
        self,
        device: Union[smith._inductor.ir.IRNode, device, None],
    ...
    def get_dep_size_hint(self, dep: Dep, count_bytes: bool = True) -> int:
        ...
        if (dep, count_bytes) not in self.dep_size_hint_cache:
            ...
            try:
                ...
                # In at least one test (test/inductor/test_smithbind.py) we
    ...
    def get_current_device_or_throw(self) -> smith.device:
    ...
    def set_current_device(self, device: smith.device) -> Iterator[None]:
    ...
    def decide_layout_opt(gm: GraphModule, *, is_inference: bool) -> bool:
        ...
        conv_nodes = [
            n for n in gm.graph.nodes if n.target is smith.ops.aten.convolution.default
        ]
        ...
        if (
            smith.backends.mkldnn.enabled  # pyrefly: ignore [unbound-name]
            and smith.backends.mkldnn.is_available()  # pyrefly: ignore [unbound-name]
            and all(
        ...
        if any(
            ...
            log.debug(
                "See perf regression with dynamic shape. Follow up in https://github.com/blacksmith/blacksmith/issues/102670"
            )
            return False

        def is_grouped(n: Any) -> bool:
            meta_val = n.args[1].meta["val"]  # type: ignore[union-attr, operator]
            assert isinstance(meta_val, smith.Tensor)
            return n.args[-1] > 1 and meta_val.size(1) > 1  # type: ignore[union-attr, operator]

        def is_in_out_channel(n: smith.fx.Node) -> bool:
            return (
        ...
        def is_small_channel(n: smith.fx.Node) -> bool:
            return (
        ...
        if is_inference:
            ...
            # taken from the set of convolution inputs in benchmarks/dynamo/microbenchmarks/operator_inp_logs/smithbench_train/
        ...
        # - blacksmith_CycleGAN_and_pix2pix (1.597x -> 1.294x)
    ...
    def add_device_info(self, device: smith.device) -> None:
    ...
    @property
    def fake_mode(self) -> smith._subclasses.fake_tensor.FakeTensorMode:
        return V.fake_mode

    def try_get_buffer(
        self, buffer_name: str
    ) -> Optional[Union[ir.TensorBox, ir.Buffer, ir.SmithBindObject]]:
    ...
    def register_buffer(self, buffer: ir.Buffer, *, set_name: bool = False) -> str:
        ...
        if (
            # Skip empty CPU tensor so that CUDA graphs can succeed, see https://github.com/blacksmith/blacksmith/pull/114144
            device is not None
            and not (
                isinstance(buffer, ir.ComputedBuffer)
                and buffer.is_zero_elements()
                and device == smith.device("cpu")
            )
        ):
    ...
    def get_original_value_of_constant(self, name: str) -> smith.Tensor:
        """
    ...
    def constant_name(self, name: str, device_override: Optional[smith.device]) -> str:
        """
        ...
        with smith.utils._python_dispatch._disable_current_modes():
            ...
            # to codegen correct smith::aot_inductor::ConstantType for them rather than `Unknown`
            if any(
    ...
    def placeholder(
        ...
        if isinstance(example, SymTypes):
            ...
            # https://github.com/blacksmith/blacksmith/issues/155468.
            ...
            obj = SmithBindObject(name=target, value=example)
        ...
        elif isinstance(example, smith.Generator):
            assert len(V.graph.current_node.users) == 1 and next(
                iter(V.graph.current_node.users)
            ).target in (
                smith._prims.rng_prims.graphsafe_run_with_rng_state,
        ...
        assert isinstance(example, smith.Tensor), example
    ...
    def call_function(self, target: Callable, args: Any, kwargs: dict[str, Any]) -> Any:  # type: ignore[type-arg, override]
        ...
        if not isinstance(target, smith._ops.OpOverloadPacket) and hasattr(
        ...
        if target not in lowerings:
            assert isinstance(target, smith._ops.OpOverload), (
            ...
            if base_name in FALLBACK_ALLOW_LIST:
                ...
                tag: Optional[smith._C.Tag] = get_layout_constraint_tag(
                    target, with_default=False
                )
                if (
                    tag is None
                    and smith._library.utils.is_builtin(target)
                    ...
                    # accuracy problems. Check https://github.com/blacksmith/blacksmith/issues/140452
                    ...
                    default_tag: smith._C.Tag = get_layout_constraint_tag(
                ...
                if layout_constraints is constrain_to_fake_tensors:
                    ...
                    if "eager_input_vals" in n.meta:
                        ...
                        def normalize(args: Any, kwargs: Any) -> tuple[Any, Any]:
                            result = smith.fx.operator_schemas.normalize_function(
                                target, args, kwargs
                            )
    ...
    @staticmethod
    def can_inline_constant(t: smith.Tensor) -> bool:
        """
    ...
    def get_attr(
        ...
        Constant, TensorBox, ShapeAsConstantBuffer, ir.Subgraph, SmithBindObject
    ]:
        # this is a constant
        value = getattr_recursive(self.module, target)  # type: ignore[arg-type]

        if isinstance(value, smith.fx.GraphModule):
        ...
        if isinstance(value, smith._C.ScriptObject):
            self.smithbind_constants[target] = value
            self.constant_reprs[target] = ""
            return SmithBindObject(name=target, value=value)
        elif isinstance(value, FakeScriptObject):
            self.smithbind_constants[target] = value
            self.constant_reprs[target] = ""
            return SmithBindObject(name=target, value=value)
        elif is_opaque_type(type(value)):
            self.smithbind_constants[target] = value  # type: ignore[arg-type]
            self.constant_reprs[target] = ""
            return SmithBindObject(name=target, value=value)  # type: ignore[arg-type]

        assert isinstance(value, smith.Tensor)
        if (
    ...
    def output(
        ...
        for r, fx_node in zip(result, fx_node_args):
            if not isinstance(r, (ir.TensorBox, ir.BaseView)):
                ...
                assert smith._inductor.ir.is_storage_and_layout(r)
                meta_strides = [
                    s.node.expr if isinstance(s, smith.SymInt) else s
        ...
        for name, value in self.graph_inputs.items():
            if isinstance(value, SmithBindObject):
                continue
            assert isinstance(
                value, (TensorBox, sympy.Expr, smith._inductor.ir.GeneratorState)
    ...
    @contextmanager
    def set_current_node(self, node: smith.fx.Node):  # type: ignore[no-untyped-def]
    ...
    def propagate_mutation(
        ...
        if fx_node.target is smith.ops.higher_order.triton_kernel_wrapper_mutation:
            kwargs = fx_node.kwargs["kwargs"]
            assert isinstance(kwargs, dict)
            mutated = smith._higher_order_ops.triton_kernel_wrap.get_mutated_tensors(
                old_kwargs["kernel_idx"],
                old_kwargs["constant_args_idx"],
                {
                    k: v.meta["val"] if isinstance(v, smith.fx.Node) else v
            ...
            for name in mutated:
                ...
                self.call_function(smith.ops.aten.copy_.default, (old_arg, new_arg), {})
            return

        assert isinstance(fx_node.target, smith._ops.OpOverload)

        def maybe_propagate(
            schema_arg: smith._C.Argument, old_arg: ir.IRNode, new_arg: ir.IRNode
        ) -> None:
            if old_arg is new_arg:
                return
            if schema_arg.alias_info is not None and schema_arg.alias_info.is_write:
                ...
                for old_arg_item, new_arg_item in zip(old_arg, new_arg):  # type: ignore[call-overload]
                    if old_arg_item is new_arg_item:
                        continue
                    self.call_function(
                        smith.ops.aten.copy_.default, (old_arg_item, new_arg_item), {}
                    )
    ...
    def run_node(self, n: smith.fx.Node) -> object:
        ...
        def maybe_apply_channels_last_stride_order(
            result: ir.IRNode, n: smith.fx.Node
        ) -> ir.IRNode:
            dense = smith._prims_common.is_non_overlapping_and_dense_or_false(
                n.meta["val"]
            )
        ...
        with (
            ...
            if (
                ...
                and n.target
                and isinstance(n.target, smith._ops.OpOverload)
                and smith._library.utils.is_builtin(n.target)
                and (
                ...
                n.op == "call_function"
                and isinstance(
                    n.target, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)
                )
                ...
                n.op == "call_function"
                and n.target is smith.ops.higher_order.triton_kernel_wrapper_mutation
                ...
                # https://github.com/blacksmith/blacksmith/issues/127789
                debug("is_magic_method")
                if isinstance(
                    n.meta["val"], (smith.SymInt, smith.SymFloat, smith.SymBool)
                ):
            ...
            as_strided_ops = [
                smith.ops.aten.as_strided.default,
                smith.ops.aten.as_strided_.default,
                smith.ops.aten.as_strided_scatter.default,
                smith.ops.aten.resize.default,
                smith.ops.aten.resize_as.default,
            ]
            ...
            if n.meta.get("inductor_realize_to_strides", False) and isinstance(
                ...
                result.realize()
                strides = n.meta["val"].stride()
                sym_strides = smith._inductor.utils.any_is_symbolic(*strides)
                ...
                if strides is not None and len(strides) > 0:
                    ...
                    if (
                        ...
                        strides = ir.FlexibleLayout.stride_ordered_for_memory_format(
                            result.get_size(), smith.channels_last
                        )
                ...
                for user in n.users:
                    if user.target in needs_realized_inputs:
                        ...
                        need_fixed_layout = [
                            ...
                            smith.ops.aten._int_mm.default,
                        ]
                        need_fixed_channels_last_layout = []
                        if not self.layout_opt:
                            need_fixed_layout.append(smith.ops.aten.convolution.default)
                        if smith._C._has_mkldnn:
                            need_fixed_layout += [
                                smith.ops.mkldnn._linear_pointwise.default,
                                smith.ops.mkldnn._linear_pointwise.binary,
                                smith.ops.aten.mkldnn_rnn_layer.default,
                                smith.ops.onednn.qlinear_pointwise.default,
                                smith.ops.onednn.qlinear_pointwise.tensor,
                                smith.ops.onednn.qlinear_pointwise.binary,
                                smith.ops.onednn.qlinear_pointwise.binary_tensor,
                            ]
                            need_fixed_channels_last_layout += [
                                smith.ops.mkldnn._convolution_pointwise.default,
                                smith.ops.mkldnn._convolution_pointwise.binary,
                                smith.ops.mkldnn._convolution_pointwise_.binary,
                                smith.ops.mkldnn._convolution_transpose_pointwise.default,
                                smith.ops.onednn.qconv_pointwise.default,
                                smith.ops.onednn.qconv2d_pointwise.binary,
                            ]
                            if smith._C.has_mkl:
                                need_fixed_layout += [smith.ops.mkl._mkl_linear.default]
    ...
    def create_deferred_runtime_asserts(
        self, n: smith.fx.Node, new_unbacked_defs: OrderedSet[sympy.Symbol]
    ) -> None:
        ...
        if (
            ...
            and n.target is smith.ops.aten._assert_scalar.default
            and self.aot_mode
        ):
    ...
    def extract_autotune_inputs(
        self, example_inputs: list[Union[int, float, smith.Tensor]]
    ) -> None:
        import copy
        ...
        for node in cloned_gm.graph.nodes:
            if (
                ...
                and node.target is smith.ops.higher_order.triton_kernel_wrapper_mutation
            ):
                triton_nodes.append(node)

        # Store grid related nodes
        grid_inputs: list[smith.fx.Node] = []
        visited_grids: dict[smith.fx.Node, int] = {}
        # Store kwargs related nodes
        triton_inputs: dict[str, Any] = {}
        kwargs_inputs: list[smith.fx.Node] = []
        ...
        for node in triton_nodes:
            ...
            for grid in node.kwargs["grid"]:
                for val in grid:
                    ...
                    if isinstance(val, smith.fx.Node):
            ...
            with cloned_gm.graph.inserting_before(node):
                for k, v in kwargs.items():
                    if k in mutated:
                        new_node = cloned_gm.graph.call_function(smith.clone, args=(v,))
        ...
        cloned_gm.recompile()
        runner = smith.fx.Interpreter(cloned_gm)
        ...
        if len(grid_inputs) > 0:
            ...
            for node in triton_nodes:
                ...
                for grid in node.kwargs["grid"]:
                    new_grid = []
                    for val in grid:
                        if not isinstance(val, smith.fx.Node):
                            new_grid.append(val)
                            continue
                        dynamic_grid = True
    ...
    def codegen_with_cpp_wrapper(
        ...
        if any(device in self.device_types for device in ["cuda", "xpu"]):
            ...
            def extract_real_inputs() -> list[Union[int, float, smith.Tensor]]:
                def materialize(
                    x: Union[smith.SymInt, smith.SymFloat, smith.Tensor],
                ) -> Union[int, float, smith.Tensor]:
                    if x is None:
                        # pyrefly: ignore [bad-return]
                        return None
                    elif isinstance(x, (smith.SymInt, smith.SymFloat)):
                        # Need concrete value to run dynamic shapes and tune the result
                        return x.node.hint
                    elif isinstance(x, FakeTensor):
                        return defake(x)
                    else:
                        assert isinstance(x, smith.Tensor), (
                ...
                if self.mutated_inputs:
                    ...
                    mutated_input_idxs = [
                        ...
                        and isinstance(real_inputs[idx], smith.Tensor)
                    ]
                    for idx in mutated_input_idxs:
                        ...
                        # e.g. if smith.compile(f)(x) if called on input-mutating
                        ...
                        assert isinstance(mutated_inp, smith.Tensor)
    ...
    def _compile_to_module(self) -> CompiledModule:
        ...
        # Logged twice as per https://github.com/blacksmith/blacksmith/pull/99038#discussion_r1167826029
