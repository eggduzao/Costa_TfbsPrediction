...
import smith
from smith._inductor.dependencies import index_vars_no_squeeze
from smith._inductor.utils import sympy_product, sympy_subs
...
loop_tiling_log = smith._logging.getArtifactLogger(__name__, "loop_tiling")
from smith.utils._sympy.functions import FloorDiv, ModularIndexing


if TYPE_CHECKING:
    from smith._inductor.scheduler import FusedSchedulerNode, SchedulerNode
...
class NodeSplitGetter:
    ...
    def __init__(
        ...
        for n in reversed(node.get_nodes()):
            if not isinstance(n, smith._inductor.scheduler.SchedulerNode):
                continue
            ...
            n_pw_splits, n_red_splits = (
                smith._inductor.codegen.simd.SIMDKernel.prepare_split_iteration_lengths(
    ...
    def try_split(self, pw: Split, red: Split) -> Optional[tuple[Split, Split]]:
        ...
        from smith._inductor.codegen.simd import CantSplit, SIMDKernel
    ...
    for n in list(node.get_nodes()):
        ...
        try:
            new_ranges, return_getters_groups = (
                smith._inductor.codegen.simd.SIMDKernel._split_iteration_ranges(
                    groups, lengths
                )
            )
        except smith._inductor.codegen.simd.CantSplit:
