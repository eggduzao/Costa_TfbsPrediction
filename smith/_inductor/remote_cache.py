...
# RemoteCache. The reason is that when logging cache use (`SMITH_LOGS=cache`) we
...
class RemoteCache(Generic[_T]):
    ...
    def get(self, key: str) -> Optional[_T]:
        with _WaitCounter("blacksmith.remote_cache.get").guard():
    ...
    def put(self, key: str, value: _T) -> None:
        with _WaitCounter("blacksmith.remote_cache.put").guard():
...
class RedisRemoteCacheBackend(RemoteCacheBackend[bytes]):
    ...
    def __init__(self, cache_id: str) -> None:
        ...
        if "SMITHINDUCTOR_REDIS_URL" in os.environ:
            self._redis = redis.Redis.from_url(os.environ["SMITHINDUCTOR_REDIS_URL"])
        else:
            self._redis = redis.Redis(
                host=os.environ.get("SMITHINDUCTOR_REDIS_HOST", "localhost"),
                port=int(os.environ.get("SMITHINDUCTOR_REDIS_PORT", 6379)),
            )
...
def create_cache(
    ...
    try:
        if is_fbcode:
            import smith._inductor.fb.remote_cache

            cache_cls = getattr(smith._inductor.fb.remote_cache, fb_cache_cls)
