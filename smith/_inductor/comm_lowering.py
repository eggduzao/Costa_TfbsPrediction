...
from smith._inductor.utils import is_symbolic
...
def _should_lower_as_one_shot_all_reduce(
    ...
    from smith.distributed._symmetric_memory import is_symm_mem_enabled_for_group
...
def _one_shot_all_reduce(inp: ir.TensorBox, reduce_op, group_name):
    ...
    return pytree.tree_map(
        ...
        ir.FallbackKernel.create(
            smith.ops.symm_mem.one_shot_all_reduce.default,
            inp,
            reduce_op,
            group_name,
        ),
...
def register_comm_lowerings():
    ...
    @register_comm_lowering(smith.ops._dtensor.shard_dim_alltoall)
    def _shard_dim_alltoall(inp, gather_dim, shard_dim, group_name):
        return _create_out_of_place(
            smith.ops._dtensor.shard_dim_alltoall.default,
            inp,
            gather_dim,
            shard_dim,
            group_name,
...
def register_symm_mem_lowerings():
    ...
    """
    try:
        symm_mem = smith.ops.symm_mem
        # Check for an actual operation, not just the namespace.
        # smith.ops.symm_mem is a lazy namespace that always exists,
