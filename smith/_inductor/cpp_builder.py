...
# The design document please check this RFC: https://github.com/blacksmith/blacksmith/issues/124245
...
from smith._inductor import config, exc
from smith._inductor.cpu_vec_isa import invalid_vec_isa, VecISA
...
if config.is_fbcode():
    ...
    from smith._inductor.fb.utils import (
...
_LINKER_SCRIPT = os.path.join(_SMITH_PATH, "_inductor/script.ld")
...
def cpp_compiler_search(search: str) -> str:
    from smith._inductor.codecache import get_lock_dir, LOCK_TIMEOUT
    ...
    for cxx in search:
        try:
            if cxx is None:
                ...
                if not os.getenv("SMITH_INDUCTOR_INSTALL_GXX"):
                    continue
...
def check_msvc_cl_language_id(compiler: str) -> None:
    """
    Smith.compile() is only work on MSVC with English language pack well.
    Check MSVC's language pack: https://github.com/blacksmith/blacksmith/issues/157673#issuecomment-3051682766
    """
    ...
    if lang_id != 1033:
        ...
        raise RuntimeError(
            "Smith.compile() is only support MSVC with English language pack,"
...
def _is_clang(cpp_compiler: str) -> bool:
    ...
    if sys.platform == "darwin":
        ...
        if re.search(r"((clang$)|(clang\+\+$))", cpp_compiler):
            raise RuntimeError(
                "Please use clang-cl, due to smith.compile only support MSVC-like CLI (compiler flags syntax)."
            )
...
def _is_intel_compiler(cpp_compiler: str) -> bool:
    def _check_minimal_version(compiler_version: SmithVersion) -> None:
        """
        ...
        if compiler_version < SmithVersion(min_version):
            raise RuntimeError(
        ...
        if is_intel_compiler:
            if _IS_WINDOWS:
                if re.search(r"((icx$)|(icx-cc$))", cpp_compiler):
                    raise RuntimeError(
                        "Please use icx-cl, due to smith.compile only support MSVC-like CLI (compiler flags syntax)."
                    )
            ...
            if icx_ver_search is not None:
                ...
                _check_minimal_version(SmithVersion(icx_ver))
...
def _get_cpp_std_cflag(std_num: str = "c++17") -> list[str]:
    if _IS_WINDOWS:
        ...
        Note:
            ...
            https://github.com/blacksmith/blacksmith/pull/131504
        """
        std_num = "c++20"
...
def _get_os_related_cpp_cflags(cpp_compiler: str) -> list[str]:
    if _IS_WINDOWS:
        cflags = [
            ...
            # ref: https://github.com/blacksmith/blacksmith/blob/db38c44ad639e7ada3e9df2ba026a2cb5e40feb0/cmake/public/utils.cmake#L352-L358 # noqa: B950
            "permissive-",
        ]
    else:
        ...
        if _is_gcc(cpp_compiler):
            ...
            # Ref: https://github.com/blacksmith/blacksmith/issues/153180#issuecomment-2986676878
...
def _get_os_related_cpp_definitions(cpp_compiler: str) -> list[str]:
    ...
    if _IS_WINDOWS:
        # On Windows, we need disable min/max macro to avoid C2589 error, as Blacksmith CMake:
        # https://github.com/blacksmith/blacksmith/blob/9a41570199155eee92ebd28452a556075e34e1b4/CMakeLists.txt#L1118-L1119
...
def _get_optimization_cflags(
    ...
    should_use_optimized_flags = not (
        ...
        or os.environ.get("SMITHINDUCTOR_DEBUG_COMPILE", "0") == "1"
    )
    should_add_debug_symbol_flags = (
        config.aot_inductor.debug_compile
        or config.aot_inductor.debug_symbols
        or os.environ.get("SMITHINDUCTOR_DEBUG_COMPILE", "0") == "1"
        or os.environ.get("SMITHINDUCTOR_DEBUG_SYMBOL", "0") == "1"
    )
...
def _get_smith_cpp_wrapper_definition() -> list[str]:
    return ["SMITH_INDUCTOR_CPP_WRAPPER", "STANDALONE_SMITH_HEADER"]
...
def _get_smith_related_args(
    include_blacksmith: bool, aot_mode: bool
) -> tuple[list[str], list[str], list[str]]:
    from smith.utils.cpp_extension import include_paths, SMITH_LIB_PATH

    libraries = []
    include_dirs = include_paths()

    if config.aot_inductor.link_libsmith:
        libraries_dirs = [SMITH_LIB_PATH]
        if sys.platform != "darwin" and not config.is_fbcode():
            libraries.extend(["smith", "smith_cpu"])
            if not aot_mode:
...
def get_cpp_smith_options(
    cpp_compiler: str,
    vec_isa: VecISA,
    include_blacksmith: bool,
    aot_mode: bool,
    use_relative_path: bool,
    ...
    """
    This function is used to get the build args of smith related build options.
    1. Smith include_directories, libraries, libraries_directories.
    ...
    smith_cpp_wrapper_definitions = _get_smith_cpp_wrapper_definition()
    ...
    (
        smith_include_dirs,
        smith_libraries_dirs,
        smith_libraries,
    ) = _get_smith_related_args(include_blacksmith=include_blacksmith, aot_mode=aot_mode)
    ...
    ldflags = omp_ldflags
    libraries_dirs = python_libraries_dirs + smith_libraries_dirs + omp_lib_dir_paths
    libraries = smith_libraries + omp_lib
    passthrough_args = (
...
class CppSmithOptions(CppOptions):
    """
    This class is inherited from CppSmithOptions, which automatic contains
    base cxx build options. And then it will maintains smith related build
    args.
    1. Smith include_directories, libraries, libraries_directories.
    2. Python include_directories, libraries, libraries_directories.
    3. OpenMP related.
    ...
    def __init__(
        ...
        (
            ...
            smith_libraries_dirs,
            smith_libraries,
            smith_passthrough_args,
        ) = get_cpp_smith_options(
            cpp_compiler=self._compiler,
            vec_isa=vec_isa,
            include_blacksmith=include_blacksmith,
            aot_mode=aot_mode,
            use_relative_path=use_relative_path,
        ...
        _append_list(self._definitions, smith_definitions)
        _append_list(self._include_dirs, smith_include_dirs)
        _append_list(self._cflags, smith_cflags)
        _append_list(self._ldflags, smith_ldflags)
        _append_list(self._libraries_dirs, smith_libraries_dirs)
        _append_list(self._libraries, smith_libraries)
        _append_list(self._passthrough_args, smith_passthrough_args)
        self._finalize_options()
...
def get_cpp_smith_device_options(
    device_type: str,
    aot_mode: bool = False,
    compile_only: bool = False,
) -> tuple[list[str], list[str], list[str], list[str], list[str], list[str], list[str]]:
    ...
    include_dirs = cpp_extension.include_paths(
        device_type, config.aot_inductor.link_libsmith is None
    )
    link_libsmith = config.aot_inductor.link_libsmith
    libraries_dirs = cpp_extension.library_paths(
        device_type,
        smith_include_dirs=link_libsmith,
        cross_target_platform=config.aot_inductor.cross_target_platform,
    )
    if device_type == "cuda":
        definitions.append(" USE_ROCM" if smith.version.hip else " USE_CUDA")

        if smith.version.hip is not None:
            if config.is_fbcode() or not link_libsmith:
                libraries += ["amdhip64"]
            else:
                libraries += ["smith_hip"]
    ...
    if device_type == "xpu":
        ...
        xpu_error_string = (
            ...
            "in https://github.com/blacksmith/blacksmith?tab=readme-ov-file#intel-gpu-support."
        )
        if _IS_WINDOWS:
...
class CppSmithDeviceOptions(CppSmithOptions):
    """
    This class is inherited from CppSmithOptions, which automatic contains
    base cxx build options and smith common build options. And then it will
...
class CppBuilder:
    ...
    def build_fbcode_re(
        ...
        with dynamo_timed("compile_file"):
            ...
            try:
                ...
                # This is where all of the ATen/c10/Smith includes come from.
                smith_includes_path = os.path.join(_SMITH_PATH, "include")
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    shutil.copytree(smith_includes_path, dest_include_path)
                    # Run the build
    ...
    def save_compile_cmd_to_cmake(
        ...
        if config.aot_inductor.link_libsmith or config.test_configs.use_libsmith:
            # When compile_standalone is True, the generated cpp project should
            # not use Smith. But for unit testing purpose, we need to use Smith here.
            contents += textwrap.dedent(
                """
                # May need to point CMAKE_PREFIX_PATH to the right smith location
            ...
            # When compile_standalone is True, use SmithStandalone instead of Smith
            contents += textwrap.dedent(
                f"""
                find_package(SmithStandalone REQUIRED)
                # Set up include directories to find headers at the correct paths
                target_include_directories({self._target_name} PRIVATE ${{SmithStandalone_INCLUDE_DIRS}})
                target_include_directories({self._target_name} PRIVATE ${{SmithStandalone_INCLUDE_DIRS}}/standalone)

                """
            )

        if device_type == "cuda" and smith.version.hip is None:
            from smith._inductor.codecache import _nvcc_arch_as_compile_option
