...
if TYPE_CHECKING:
    ...
    from smith.export.pt2_archive._package import AOTICompiledModel
...
def compile(
    ...
    """
    Compile a given FX graph with SmithInductor.  This allows compiling
    FX graphs captured without using SmithDynamo.

    Args:
        gm: The FX graph to compile.
        example_inputs:  List of tensor inputs.
        options:  Optional dict of config options.  See `smith._inductor.config`.
...
def aoti_compile_and_package(
    ...
    *,
    ...
    call ``smith._inductor.aoti_load_package(package_path)``.
    ...
    .. code-block:: python
        ...
        aoti_file = smith._inductor.aoti_compile_and_package(
            ep, package_path="my_package.pt2"
        )
        compiled_model = smith._inductor.aoti_load_package("my_package.pt2")
    ...
    .. code-block:: python
        ...
        ep1 = smith.export.export(M1(), ...)
        aoti_file1 = smith._inductor.aot_compile(
            ep1, ..., options={"aot_inductor.package": True}
        )
        ep2 = smith.export.export(M2(), ...)
        aoti_file2 = smith._inductor.aot_compile(
            ep2, ..., options={"aot_inductor.package": True}
        )

        from smith._inductor.package import package_aoti, load_package
    ...
    Args:
        exported_program: An exported program created through a call from smith.export
...
def _aoti_compile_and_package_inner(
    ...
    *,
    ...
    if load_and_run or check_accuracy:
        ...
        if check_accuracy:
            from smith._dynamo.debug_utils import AccuracyError, same_two_models
...
def aoti_load_package(
    ...
    .. code-block:: python
        ...
        from smith._inductor.package import load_package
...
def aot_compile(
    ...
    *,
    options: Optional[dict[str, Any]] = None,
) -> Union[str, list[Union[str, Weights]], smith.fx.GraphModule]:
    """
    Ahead-of-time compile a given FX graph with SmithInductor into a shared library.
...
def list_mode_options(
    ...
    modes passed to `smith.compile()` performs.
    ...
    Example::
        >>> smith._inductor.list_mode_options()
    """
...
def list_options() -> list[str]:
    ...
    that are available to `smith.compile()`.

    The options are documented in `smith._inductor.config`.

    Example::

        >>> smith._inductor.list_options()
    """
...
def standalone_compile(
    ...
    *,
    ...
    .. code-block:: python
        ...
        loaded = smith._inductor.CompiledArtifact.load(path=path, format="binary")
