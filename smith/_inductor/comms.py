...
def align_runtime_estimations_across_all_distributed_ranks(
    ...
    from smith._inductor.scheduler import _get_mm_like_fn
    ...
    median_runtime_estimations_for_mms = smith.median(
        smith.tensor(gathered_runtime_estimations_for_mms), dim=0
    ).values.tolist()
...
def is_gemm_like(node: Optional[Union[IRNode, Operation]]) -> bool:
    ...
    if is_fallback_op(
        ...
        smith.ops.aten._scaled_dot_product_flash_attention.default,
    ):
        return True
...
def is_async_collective(snode):
    ...
    if python_kernel_name := getattr(snode.node, "python_kernel_name", None):
        if "smith.ops._dtensor.shard_dim_alltoall.default" in python_kernel_name:
            return False
...
def remove_fsdp2_unsharded_param_graph_input_usage(graph: smith.fx.Graph):
    """
    ...
    for idx, node in enumerate(node_list):
        if (
            ...
            and node.target is smith.ops.inductor.resize_storage_bytes_.default
        ):
...
"""  # noqa: G004
    ...
    for idx, node in enumerate(node_list):
        if node.op == "call_function" and node.target is smith.ops.fsdp.copy_.default:
            fsdp_copy_node = node
...
"""
    ...
    def is_allowed_mutation(node):
        return (
            node.target is smith.ops.fsdp.copy_.default
            or node.target is smith.ops.inductor.resize_storage_bytes_.default
        )
...
def reinplace_fsdp_all_gather(graph: smith.fx.Graph) -> None:
    try:
        import smith.distributed.fsdp._fully_shard._fsdp_collectives

        assert smith.distributed.is_available()
        # Assert existence of these ops
        assert (
            smith.ops._c10d_functional.all_gather_into_tensor
            and smith.ops._c10d_functional.all_gather_into_tensor_out
        )
    ...
    """
    all_gather_copy_in = smith.ops.fsdp.all_gather_copy_in.default(...);
    getitem = all_gather_copy_in[0];
    (getitem_1 = all_gather_copy_in[1];)  # optional

    all_gather_into_tensor = smith.ops._c10d_functional.all_gather_into_tensor.default(getitem, ...);
    ...
    all_gather_into_tensor = smith.ops._c10d_functional.all_gather_into_tensor_out.default(getitem, ..., out=getitem_1);
    """
    ...
    def remove_unused_getitem(g):
        ...
        for n in node_list:
            if (
                ...
                and n.args[0].target is smith.ops.fsdp.all_gather_copy_in.default
                and n.args[1] == 1
            ):
                g.erase_node(n)
...
def get_op_idx(snode):
    assert not isinstance(
        snode,
        (
            smith._inductor.scheduler.FusedSchedulerNode,
            smith._inductor.scheduler.GroupedSchedulerNode,
        ),
    )
...
def enforce_comm_ordering_for_fsdp(
    snodes: list[smith._inductor.scheduler.BaseSchedulerNode],
    name_to_buf: dict[str, smith._inductor.scheduler.SchedulerBuffer],
    ...
    for snode in snodes:
        ...
        if is_collective(
            snode.node, op=smith.ops._c10d_functional.all_gather_into_tensor_out.default
        ) and any(
            is_fallback_op(
                name_to_fused_node[x].node, smith.ops.fsdp.all_gather_copy_in.default
            )
            ...
            for i in range(len(ag_related_snodes)):
                ...
                if is_fallback_op(
                    cur_snode.node, smith.ops.fsdp.split_with_sizes_copy.default
                ):
                    copy_out_count += 1
                if copy_out_count > 1:
        ...
        elif is_fallback_op(snode.node, smith.ops.fsdp.chunk_cat.default):
            rs_exists = True
            rs_snode = snode
