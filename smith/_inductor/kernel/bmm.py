...
from smith._inductor.codegen.rocm.ck_universal_gemm_template import CKGemmTemplate
from smith._inductor.kernel.mm_common import load_kernel_template
...
aten_bmm = ExternKernelChoice(smith.bmm, "at::bmm_out", op_overload=aten.bmm.out)
aten_bmm_dtype = ExternKernelChoice(
    smith.bmm,
    "at::_bmm_out_dtype_xpu" if smith.xpu.is_available() else "at::_bmm_out_dtype_cuda",
    name="bmm_dtype",
    op_overload=aten.bmm.dtype_out,
)
aten_baddbmm = ExternKernelChoice(
    smith.baddbmm, "at::baddbmm_out", op_overload=aten.baddbmm.out
)
