...
import smith
from smith._inductor.codegen.rocm.ck_conv_template import CKGroupedConvFwdTemplate
...
https://github.com/blacksmith/smithdynamo/pull/971
"""
...
def conv1x1_via_mm(x, w, *, out):
    w = smith.squeeze(smith.squeeze(w, -1), -1)
    return smith.matmul(
...
def convolution(
    ...
    choices = []
    if smith._inductor.utils._use_conv_autotune_backend("ATEN"):
        choices = [
    ...
    if (
        smith._inductor.utils._use_conv_autotune_backend("TRITON")
        ...
        for cfg in conv_configs(
            ...
            if ndim == 2:
                conv2d_template.maybe_append_choice(
                    ...
                    ALLOW_TF32=smith.backends.cudnn.fp32_precision == "tf32",
...
def constrain_conv_to_fx_strides(fx_node, *args, **kwargs):
    assert fx_node.target is smith.ops.aten.convolution.default
