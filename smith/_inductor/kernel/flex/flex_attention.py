...
from smith.nn.attention.flex_attention import _Backend
...
def get_float32_precision():
    if (
        (
            ...
            if smith.backends.cuda.matmul.fp32_precision != "none"
            else smith.get_float32_matmul_precision() == "highest"
        )
        or smith.version.hip
        or smith.mtia.is_available()
    ):
        return "'ieee'"
    else:
        return "'tf32'"
...
@register_lowering(smith.ops.higher_order.flex_attention, type_promotion_kind=None)
def flex_attention(
    query,
    key,
    value,
    ...
    logsumexp = empty_strided(
        logsumexp_shape,
        None,
        dtype=smith.float32,  # The logsumexp is always stored in fp32 regardless of the input dtype
        device=query.get_device(),
    )
    max_scores = empty_strided(
        logsumexp_shape,  # Same shape as logsumexp
        None,
        dtype=smith.float32,  # The max scores are always stored in fp32 regardless of the input dtype
        device=query.get_device(),
    )
    ...
    for conf in configs:
        ...
        cur_kernel_options.setdefault("USE_TMA", bool(smith.xpu.is_available()))
    ...
    out = autotune_select_algorithm(
        ...
        [x for x in inputs_for_autotuning if isinstance(x, smith._inductor.ir.IRNode)],
        layout,
...
def validate_joint_graph(joint_graph: smith.fx.Graph):
    """We do some pre lowering graph checks in order to raise nicer error messages"""
    for node in joint_graph.nodes:
        if (
            node.op == "call_function"
            and node.target is smith.ops.flex_lib.zeros_and_scatter.default
        ):
...
@register_lowering(
    smith.ops.higher_order.flex_attention_backward, type_promotion_kind=None
)
def flex_attention_backward(*args, **kwargs):
    ...
    if _use_flex_flash_attention_backward(
        ...
        if (
            ...
            and not smith.is_deterministic_algorithms_warn_only_enabled()
    ...
    # Inside of Triton kernel, only apply partial masking if partial blocks are computed.
    # full_kv_num_blocks is smith.zeros([1, 1, 1]) if partial blocks are not computed.
    has_full_blocks = full_kv_num_blocks is not None
    kernel_options.setdefault("HAS_FULL_BLOCKS", has_full_blocks)
    if not has_full_blocks:
