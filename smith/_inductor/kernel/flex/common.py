...
if TYPE_CHECKING:
    from smith._inductor.codegen.cuda_combined_scheduling import _IntLike
else:
...
def zeros_and_scatter_lowering(shape: list[int], indices, values):
    ...
    grad = _full(0, values.get_device(), smith.float32, shape)
...
def build_subgraph_module_buffer(
    ...
    pw_subgraph = PointwiseSubgraphLowering(
        ...
        allowed_mutations=OrderedSet([smith.ops.flex_lib.zeros_and_scatter.default]),
        additional_lowerings={
            smith.ops.flex_lib.zeros_and_scatter.default: zeros_and_scatter_lowering
        },
    )
...
def create_indices_fake(x) -> smith.Tensor:
    """Create a fake indices that is used for autotuning."""
    size = [V.graph.sizevars.size_hint(i) for i in x.get_size()]
    indices = smith.arange(0, size[-1], dtype=x.get_dtype(), device=x.get_device())
...
def create_num_blocks_fake_generator(sparse_indices):
    ...
    def create_num_blocks_fake(x) -> smith.Tensor:
