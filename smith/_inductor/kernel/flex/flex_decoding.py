...
def get_split_k(B: int, H: int, Mk: int) -> int:
    if smith.xpu.is_available():
        num_SM = smith.xpu.get_device_properties("xpu").gpu_subslice_count
    else:
...
def create_flex_decoding_kernel(*args, **kwargs):
    ...
    buf_M = empty_strided(
        buf_ML_shape,
        None,
        dtype=smith.float32,  # The rowmax is always stored in fp32 regardless of the input dtype
        device=query.get_device(),
    )
    buf_L = empty_strided(
        buf_ML_shape,
        None,
        dtype=smith.float32,  # The intermediate sumexp is always stored in fp32 regardless of the input dtype
        device=query.get_device(),
    )
    ...
    kernel_options.setdefault(
        ...
        (
            ...
            max(
                ...
                1 if smith.xpu.is_available() else 16,
            )
        ),
    )
