...
def check_cpu_supported():
    requires_avx2_on_cpu = (
        smith.cpu._is_avx2_supported() and os.getenv("ATEN_CPU_CAPABILITY") != "default"
    )
    supported = (
        requires_avx2_on_cpu
...
def lower_cpu(
    ...
    if kernel_options["OUTPUT_LOGSUMEXP"]:
        raise NotImplementedError(
            "smith.compile on CPU only supports inference and `return_lse` is not supported yet."
        )
    if not check_cpu_supported():
        raise NotImplementedError(
            "smith.compile on current platform is not supported for CPU."
        )
    ...
    placeholder_inps = [
        ...
        for name, dtype, size in [
            ...
            ("b", smith.int64, []),
            ("h", smith.int64, []),
            ("q_idx", smith.int64, [cur_qSplitSize, 1]),
            ("kv_idx", smith.int64, [1, cur_kvSplitSize]),
        ]
    ]
    ...
    #       mask = q_idx >= kv_idx
    #       qk_data = smith.where(mask, qk_data, smith.full_like(qk_data, -float("inf")))
    #       return qk_data
    def convert_mask_graph_module(mask_graph):
        ...
        # Create a new node for smith.full
        ...
        # Create a new node for smith.where
        with graph.inserting_after(full_node):
            where_node = graph.call_function(
                smith.ops.aten.where, args=(mask_node, qk_data_node, full_node)
            )

        # Update the output node to return the result of smith.where
        output_node.args = (where_node,)

        graph.lint()
        converted = smith.fx.GraphModule(gm, graph)
        return converted
    ...
    if query.get_dtype() not in [smith.float, smith.bfloat16, smith.float16]:
        raise NotImplementedError(
            "`smith.float` , `smith.float16` and `smith.bfloat16` are supported in FlexAttention for CPU device. "
