...
class HierarchicalIndex(sympy.Function):
    ...
    `smith/_inductor/codegen/cutedsl/cutedsl_kernel.py`).
    """
...
def is_trivial_mask_graph(graph_module: GraphModule) -> bool:
    ...
    return len(placeholders) == 4 and output_val.target is smith.ops.aten.full.default
...
def create_flex_flash_attention_kernel(
    ...
    lse = empty_strided(
        ...
        dtype=smith.float32,  # LSE is always fp32
        device=device,
    )
