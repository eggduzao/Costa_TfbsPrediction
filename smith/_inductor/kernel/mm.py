...
from smith._inductor.autoheuristic.autoheuristic import AutoHeuristicSelectAlgorithm
...
from smith._inductor.codegen.cpp_gemm_template import CppGemmTemplate
from smith._inductor.remote_gemm_autotune_cache import gen_best_config
...
try:
    ...
    triton_version = SmithVersion(triton.__version__)
    has_triton = True
except ImportError:
    triton_version = SmithVersion("0.0.0")
    has_triton = False
...
mm_template = TritonTemplate(
    ...
    if (smith.version.hip is None) or triton_version >= "3.3.0"
    # FIXME: To get around rocm failures like https://github.com/blacksmith/blacksmith/actions/runs/13123783322/job/36617154943
    # The only difference between the two templates is M >= BLOCK_M and N >= BLOCK_N checking.
    # See more details in https://github.com/blacksmith/blacksmith/pull/146293
...
aten_mm = ExternKernelChoice(smith.mm, "at::mm_out", op_overload=aten.mm.out)
aten_mm_dtype = ExternKernelChoice(
    smith.mm,
    "at::_mm_dtype_out_xpu" if smith.xpu.is_available() else "at::_mm_dtype_out_cuda",
    name="mm_dtype",
    op_overload=aten.mm.dtype_out,
)

aten_addmm = ExternKernelChoice(
    smith.addmm, "at::addmm_out", op_overload=aten.addmm.out
)

aten__int_mm = ExternKernelChoice(
    smith._int_mm, "at::_int_mm_out", op_overload=aten._int_mm.out
)
...
aten__fp8_mm = ExternKernelChoice(
    smith._scaled_mm, "at::_scaled_mm_out", op_overload=aten._scaled_mm.out
)


def _is_int8_mat(mat):
    return mat.get_dtype() in (smith.int8, smith.uint8)


def bias_addmm(inp, mat1, mat2, *, out=None, alpha=1, beta=1):
    """
    Giving smith.addmm a 1D tensor calls a different (faster) cublasLt
    kernel under the hood.  There are a few shapes where this is slower,
    but they are rare.
    """
    if (inp.stride(0) == 0 and inp.size(0) != 0) or inp.size(0) == 1:
        return smith.addmm(inp[0], mat1, mat2, out=out, alpha=alpha, beta=beta)
    return smith.addmm(inp, mat1, mat2, out=out, alpha=alpha, beta=beta)
...
def contiguous_mm(a, b):
    return smith.mm(a, b.contiguous())


def contiguous_addmm(inp, a, b):
    return smith.addmm(inp, a, b.contiguous())
...
def tuned_mm(mat1, mat2, out_dtype=None, *, layout=None):
    ...
    if out_dtype is not None:
        ...
        smith._check(
            ...
            or (
                ...
                and input_dtype in (smith.float16, smith.bfloat16)
            ),
            lambda: "out_dtype must be the same as input dtype or fp32 for fp16/bf16 inputs",
        )

    # Lower matmul-related operations (e.g., smith.matmul / smith.bmm / smith.addmm)
    ...
    #
    # For example, given `C = smith.matmul(A, B)`, this can be rewritten as:
    #
    ...
    if (
        ...
        if is_exhaustive or not use_decompose_k_choice(m, n, k, threshold_multiple=2):
            ...
            if (
                ...
                from smith._inductor.fb.tlx_templates.mm_templates import append_tlx
    ...
    if (
        ...
        and smith._inductor.config.run_autoheuristic(name)
        and is_triton(mat1)
    ):
        always_included = []
        if use_aten_gemm_kernels():
        ...
        if not smith._inductor.config.collect_autoheuristic(name):
    ...
    if out_dtype is None and smith._inductor.config.remote_gemm_autotune_cache:
...
def tuned_int_mm(mat1, mat2, *, layout=None):
    # TODO(coconutruben): integrate into MMKernelInputs when all callsites use that
    m, n, k, layout, mat1, mat2 = mm_args(
        mat1, mat2, layout=layout, out_dtype=smith.int32
    )
    name = "int_mm"
    # below is for getting an overview logging info of inductor mms
    ...
    kernel_inputs = MMKernelInputs([mat1, mat2], out_dtype=smith.int32)
...
def tuned_addmm(inp, mat1, mat2, *, alpha=1, beta=1, layout=None):
    ...
    if (
        ...
        CUTLASS3xGemmTemplate.add_cutlass_gemm_choices(
            choices,
            layout,
            # reorder here because CUTLASS expects (x, w, bias) but smith
            # is bias, x, w
    ...
    if is_nonzero and use_ck_gemm_template(layout, m, n, k):
        CKGemmTemplate.add_ck_gemm_choices(
            choices,
            layout,
            # reorder here because CK expects (x, w, bias) but smith
            # is bias, x, w
            kernel_inputs.nodes(reorder=[1, 2, 0]),
            alpha=alpha,
            beta=beta,
...
def tuned_sparse_semi_structured_mm(
    ...
    from smith._inductor.select_algorithm import realize_inputs
...
def get_scaling_options(
    mat_a: Any,
    mat_b: Any,
    scale_a_size: smith.Tensor,
    scale_b_size: smith.Tensor,
...
def tuned_scaled_mm(
    ...
    if scale_a.dtype != smith.float32:
...
def _is_sm7x_or_older_gpu(index: Optional[int]) -> bool:
    props = smith.cuda.get_device_properties(index or 0)
...
def get_size_hints(mat1, mat2, m, n, k):
    if not isinstance(m, int) or not isinstance(k, int):
        (m, k) = V.graph.sizevars.size_hints(
            mat1.get_size(),
            fallback=smith._inductor.config.unbacked_symint_fallback,
        )
