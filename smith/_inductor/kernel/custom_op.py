...
import smith
from smith._inductor.codegen.subgraph import SubgraphTemplate
from smith._inductor.ir import Buffer, FixedLayout, ir_node_to_tensor, TensorBox
from smith._inductor.lowering import lowerings, validate_ir
from smith._inductor.select_algorithm import (
    autotune_select_algorithm,
    ExternKernelChoice,
)
from smith._inductor.utils import convert_symint_to_expr
...
def _detect_collective_ops(choices: list) -> bool:
    ...
    """
    from smith._inductor.utils import is_collective_op
...
def _adapt_user_input_gen_fns(
    inputs: list[Any],
    arg_names: list[str],
    user_input_gen_fns: dict[str, Callable[[smith.Tensor], smith.Tensor]],
) -> dict[int, Callable[[Any], smith.Tensor]]:
    ...
    def create_internal_input_gen_fn(
        user_function: Callable[[smith.Tensor], smith.Tensor], arg_name: str
    ) -> Callable[[Any], smith.Tensor]:
        """Create internal input generator that converts IR buffer to user's fake tensor."""

        def internal_input_gen_fn(ir_buffer: Any) -> smith.Tensor:
...
def _create_range_input_gen_fn(
    base_gen_fn: Callable[[smith.Tensor], smith.Tensor],
    dim_index: int,
    range_start: int,
    range_end: Union[int, float],
    range_upper_bound: int,
) -> Callable[[smith.Tensor], smith.Tensor]:
    ...
    from smith._inductor.kernel.flex.common import construct_strides

    target_dim = range_upper_bound if range_end == float("inf") else int(range_end)

    def constrained_gen_fn(fake_tensor: smith.Tensor) -> smith.Tensor:
        ...
        storage = smith.randn(storage_size, dtype=result.dtype, device=result.device)
        return storage.as_strided(shape, tuple(new_stride))

    return constrained_gen_fn


def _default_input_gen_fn(fake_tensor: smith.Tensor) -> smith.Tensor:
...
def autotune_custom_op(
    ...
    user_input_gen_fns: Optional[
        dict[str, Callable[[smith.Tensor], smith.Tensor]]
    ] = None,
    ...
    input_gen_fns: dict[int, Callable[[Any], smith.Tensor]] = {}
    ...
    if winning_choice.gm is not None:
        ...
        # (e.g., range-based dispatch builds its own smith.cond from winning choices)
        if return_choice:
            log.debug(
        ...
        from smith._inductor.codegen.subgraph import inline_subgraph_to_ir_nodes
...
def _generate_dynamic_configs(
    ...
    config_generator: Callable[[dict[str, smith.Tensor]], list[CustomOpConfig]],
    default_impl: Callable[..., Any],
    operation_name: str,
) -> list[CustomOpConfig]:
...
def _prepare_configs_and_decompositions(
    ...
    config_generator: Optional[
        Callable[[dict[str, smith.Tensor]], list[CustomOpConfig]]
    ],
...
def _standard_lowering_fn(
    ...
    name: str,
    op_overload: smith._ops.OpOverload,
    input_gen_fns: Optional[dict[str, Callable[[smith.Tensor], smith.Tensor]]],
    tensor_inputs: list[Any],
    runtime_kwargs: dict[str, Any],
    config_generator: Optional[
        Callable[[dict[str, smith.Tensor]], list[CustomOpConfig]]
...
def _range_based_lowering_fn(
    ...
    def dispatch_fn(*fake_tensors):
        """Build nested smith.cond dispatch: cond(pred1, impl1, cond(pred2, impl2, ...))."""
        ...
        def build_range_predicate(ranges_list: list[RangeBounds]) -> smith.Tensor:
        ...
        def build_nested_cond(idx: int):
            """Recursively build nested smith.cond for impl_groups[idx:]."""
    ...
    with V.fake_mode:
        ...
        log.info("Tracing smith.cond dispatch with symbolic shapes...")
        ...
        try:
            ...
            log.info("Successfully traced smith.cond dispatch")
    ...
    log.info(
        "Successfully created smith.cond dispatch for %d impl groups", len(impl_groups)
    )
...
def register_custom_op_autotuning(
    custom_op: smith._library.custom_ops.CustomOpDef,
    configs: Optional[Union[list[CustomOpConfig], list[Callable[..., Any]]]] = None,
    config_generator: Optional[
        Callable[[dict[str, smith.Tensor]], list[CustomOpConfig]]
    ] = None,
    name: Optional[str] = None,
    input_gen_fns: Optional[dict[str, Callable[[smith.Tensor], smith.Tensor]]] = None,
    ...
    Args:
        custom_op: Custom operation (decorated function from @smith.library.custom_op)
    ...
    Examples:
        # Static configs
        @smith.library.custom_op("mylib::attention", mutates_args=())
        ...
        register_custom_op_autotuning(
            ...
            input_gen_fns={
                "query": lambda fake: smith.randn_like(fake, device='cuda'),
                "key": lambda fake: smith.randn_like(fake, device='cuda'),
                "value": lambda fake: smith.randn_like(fake, device='cuda'),
            },
        )

        # Dynamic config generation based on input tensor properties
        def generate_k_split_configs(fake_tensors: dict[str, smith.Tensor]) -> list[CustomOpConfig]:
    ...
    if not isinstance(custom_op, CustomOpDef):
        raise TypeError(
            f"custom_op must be a CustomOpDef (decorated function from @smith.library.custom_op), "
