...
def mark_mixed_dtype(computation_node):
    ...
    if computation_node_dtype not in (smith.float16, smith.bfloat16):
        return
    ...
    if not isinstance(computation_node_user.meta["val"], smith.Tensor):
        return

    if computation_node_user.meta["val"].dtype != smith.float32:
        return
...
def binary_folding_init():
    ...
    def _check_conv_and_broadcast_op(conv_node, other):
        ...
        if not weight_meta_value.is_floating_point():
            return False
        if isinstance(other, smith.fx.Node) and other.op == "get_attr":
            other_meta_value = other.meta.get("val")
            if not other_meta_value.is_floating_point():  # type: ignore[union-attr]
                return False
            if (
                smith.promote_types(other_meta_value.dtype, weight_meta_value.dtype)  # type: ignore[union-attr]
                ...
                if (
                    other_meta_value.dtype != smith.float  # type: ignore[union-attr]
                    and weight_meta_value.dtype not in (smith.float16, smith.bfloat16)
                ):
                    return False
