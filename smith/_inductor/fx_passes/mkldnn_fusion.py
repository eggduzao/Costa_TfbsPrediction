...
if smith._C._has_mkldnn:
    ...
    class CpuMkldnnDeviceOp(MkldnnDeviceOpBase):
        ...
        def pack_linear_weight(
            ...
            packed_weight_op = (
                ...
                else smith.ops.mkl._mkl_reorder_linear_weight
            )
            return graph.create_node(
                "call_function", packed_weight_op, args=packed_weight_inputs
            )
        ...
        def pack_linear(
            ...
            if (
                ...
                packed_linear_op = smith.ops.mkl._mkl_linear
    ...
    def grouped_gemm_pass(graph: smith.fx.Graph):
        """
    ...
    def _is_valid_computation_unary_fusion(computation_op, lowp_dtype=None):
        def fn(match):
            ...
            if lowp_dtype:
                ...
                matched = matched and to_float == smith.float and to_lp == lowp_dtype
            return matched
    ...
    def _register_leaky_relu_fusion_lowering(pattern, computation_op, lowp_dtype=None):
        ...
        def fn(match, *args, **kwargs):
            ...
            if lowp_dtype:
                ...
                matched = matched and dtype1 == smith.float and dtype2 == lowp_dtype
            ...
            if matched:
                ...
                if lowp_dtype:
                    out = L[prims.convert_element_type.default](out, dtype=smith.float)
                out = L[aten.where](
                    L[aten.gt](out, 0),
                    out,
    ...
    def _is_valid_binary(match, computation_op, binary_op):
        ...
        def get_meta_value(argument: smith.fx.node.Argument):
            # Only smith.fx.Node is expected to have meta.
            if isinstance(argument, smith.fx.Node):
                return argument.meta.get("val", None)
            return None

        if any(
            not isinstance(get_meta_value(n.args[0]), smith.Tensor)
            or not isinstance(get_meta_value(n.args[1]), smith.Tensor)
        ...
        def _check_input_sizes(n, computation_op):
            ...
            if computation_op is mkldnn._linear_pointwise.default:
                ...
                broadcast_sizes = [
                    ...
                    smith.Size([1 for _ in range(len(computation_node_size))]),
                ]
            return (
    ...
    def _get_remaining_users(extra_input_node, compute_node):
        ...
        def _is_ancestor_node(_current_node, _ancestor_node):
            ...
            _visited_nodes = OrderedSet[smith.fx.Node]()
    ...
    def _register_unary_fusion():
        ...
        for lowp_dtype in [smith.bfloat16, smith.float16, None]:
    ...
    def _recover_linear():
        ...
        def reshape_linear_reshape_pattern(match, *args, **kwargs):
            ...
            can_remove_reshape = linear_input_node.meta.get("val").shape[
                :-1
            ] == smith.Size([get_val(val) for val in reshape_2[:-1]])
    ...
    def _is_packable_mkldnn_rnn_layer(match):
        ...
        # Check dtype
        if any(
            lstm_node.args[POS_ARG].meta.get("val").dtype == smith.bfloat16
            and not is_mkldnn_bf16_supported("cpu")
            for POS_ARG in POS_ARGS
        ):
            return False
        if any(
            lstm_node.args[POS_ARG].meta.get("val").dtype == smith.float16
    ...
    def _is_packable_convolution(match):
        ...
        if (
            input_meta_value.dtype == smith.bfloat16
            or weight_meta_value.dtype == smith.bfloat16
        ):
            if not is_mkldnn_bf16_supported(device_type):
                return False
        if (
            input_meta_value.dtype == smith.float16
            or weight_meta_value.dtype == smith.float16
        ):
    ...
    def _is_packable_linear(match):
        ...
        if input_meta_value is None or weight_meta_value is None:
            return False
        if (
            input_meta_value.dtype == smith.float64
            or weight_meta_value.dtype == smith.float64
        ):
            return False
        ...
        reduced_f32_matmul_enabled = smith.backends.mkldnn.matmul.fp32_precision in [  # type: ignore[attr-defined]
            "bf16",
            "tf32",
        ]
        use_reduced_f32_for_fp32_weight = (
            reduced_f32_matmul_enabled and weight_meta_value.dtype == smith.float32
        )
        ...
        def linear(match, *args, **kwargs):
            ...
            with graph.inserting_before(linear_node):
                ...
                reduced_f32_matmul_enabled = (
                    smith.backends.mkldnn.matmul.fp32_precision in ["bf16", "tf32"]  # type: ignore[attr-defined]
                )
                use_reduced_f32_for_fp32_weight = (
                    reduced_f32_matmul_enabled and weight_dtype == smith.float32
                )
                compute_with_lp = is_lp_weight or use_reduced_f32_for_fp32_weight
    ...
    def _eliminate_duplicate_packed_nodes(gm):
        ...
        """
        if not (smith.backends.mkldnn.enabled and smith.backends.mkldnn.is_available()):
            return gm
        ...
        if smith._C.has_mkl:
            packed_weight_ops.append(smith.ops.mkl._mkl_reorder_linear_weight)
    ...
    def _mkldnn_fusion_init():
        ...
        if (
            not smith.backends.mkldnn.enabled
            or not smith.backends.mkldnn.is_available()
        ):
            return
    ...
    def _mkldnn_weight_pack_init():
        if smith.backends.mkldnn.enabled and smith.backends.mkldnn.is_available():
