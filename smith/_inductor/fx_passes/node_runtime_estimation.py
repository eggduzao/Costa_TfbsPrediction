...
def _get_collective_key(coll_node: fx.Node) -> str:
    ...
    from smith._inductor import fx_utils
    ...
    if success:
        ...
        def extract_first_tensor_bytes(t: smith.Tensor) -> smith.Tensor:
        ...
        smith.utils._pytree.tree_map_only(
            smith.Tensor, extract_first_tensor_bytes, (args, kw)
        )
...
def _get_collective_estimations(coll_node: fx.Node) -> tuple[float, float]:
    ...
    """
    nccl_ms = (
        smith._inductor.comm_analysis.estimate_nccl_collective_runtime_from_fx_node(
...
def get_hint(x: int | smith.SymInt) -> Optional[int]:
    if isinstance(x, int):
        return x
...
def can_benchmark_collective() -> bool:
    ...
    if (
        smith.distributed.distributed_c10d.get_backend(pg)
        == smith.distributed.distributed_c10d.Backend.FAKE
    ):
        return False
...
def _median(lst):
    assert len(lst) > 0
    return smith.median(smith.tensor(lst)).item()
...
def _benchmark_collective_with_cuda_events_impl(
    ...
    def to_real(t: smith.Tensor) -> smith.Tensor:
        shape = [get_hint(dim) for dim in t.shape]
    ...
    smith.ops._c10d_functional.wait_tensor(result)
    smith.cuda.synchronize()
...
def benchmark_collective_with_cuda_events_impl(
    ...
    def extract_tensor_info(t: smith.Tensor) -> smith.Tensor:
    ...
    smith.utils._pytree.tree_map_only(smith.Tensor, extract_tensor_info, (args, kwargs))
...
def _log_compute_estimations(
    ...
    from smith._inductor.fx_utils import count_flops_fx
