...
from smith._inductor.comm_analysis import estimate_fx_collective_memory_footprint
from smith._inductor.fx_passes.bucketing import _schedulable_wait_node, is_wait_tensor
from smith._inductor.fx_passes.memory_estimator import MemoryTracker
...
from smith._inductor.fx_passes.bucketing import bucket_key

from ..pattern_matcher import stable_topological_sort


def make_all_device_put_sync(gm: smith.fx.GraphModule) -> int:
    """
    ...
    device_put_nodes = list(
        g.find_nodes(op="call_function", target=smith.ops.prims.device_put.default)
    )
...
def estimate_runtime_analytical(n: smith.fx.Node) -> float:
    """Estimate runtime using analytical roofline model for mm operations."""
    if n.target != smith.ops.aten.mm.default:
        return 0.0
    import smith.utils._pytree as pytree
    from smith.distributed._tools import RuntimeEstimator
...
def estimate_collective_time(
    ...
    return smith._inductor.comm_analysis.estimate_nccl_collective_runtime_from_fx_node(
        n, override_size
    )
...
def is_compute_node(n: fx.Node) -> bool:
    ...
    return (
        ...
        in smith.utils.flop_counter.flop_registry
    )
...
def estimate_mem_bound_runtime_ms(node: fx.Node) -> float:
    ...
    """
    from smith._inductor.fx_passes.fusion_regions import is_view_node
    from smith.utils._pytree import tree_flatten
    from smith.utils._runtime_estimation import get_transfer_time
...
def get_hint(x: int | smith.SymInt) -> int | None:
    if isinstance(x, int):
        return x
    assert isinstance(x, smith.SymInt)
...
def benchmark_node_with_cache_key(
    ...
    success, args, kwargs = smith._inductor.fx_utils.get_fake_args_kwargs(n)
    ...
    def to_real(t: smith.Tensor) -> smith.Tensor | None:
    ...
    with _disable_current_modes():
        args, kwargs = smith.utils._pytree.tree_map_only(
            smith.Tensor,
            lambda t: to_real(t),
            (args, kwargs),
        )
...
@functools.cache
def get_benchmark_cache() -> smith._inductor.codecache.LocalCache:
...
class OverlapScheduler:
    ...
    def __init__(
        ...
        if enable_fusion_regions:
            from smith._inductor.fx_passes.fusion_regions import (
    ...
    def _align_compute_nodes_runtime_estimations_across_all_distributed_ranks(
        ...
        from smith._inductor.fx_passes.node_runtime_estimation import (
        ...
        with unset_fake_temporarily():
            ...
            median_runtime_estimations = smith.median(
                smith.tensor(gathered_runtime_estimations), dim=0
            ).values.tolist()
    ...
    def run(self) -> smith.fx.GraphModule:
    ...
    def is_cheap_fn(node: fx.Node) -> bool:
        return getattr(node.target, "is_view", False) or smith.Tag.pointwise in getattr(
            node.target, "tags", ()
        )
    ...
    def should_assume_bucketed(self, node: fx.Node) -> bool:
        ...
        """
        if not smith._inductor.config.test_configs.assume_bucketing_reduces_latency:
            return False
...
def schedule_overlap_bucketing_from_inductor_configs(
    ...
    Reads configuration from smith._inductor.config.aten_distributed_optimizations
