...
def replace_random_passes(gm: smith.fx.GraphModule):
...
def fuse_seed_creation_pass(graph: smith.fx.Graph):
    """
    ...
    for device, seeds in device_seeds.items():
        with graph.inserting_before(seeds[0]):
            ...
            with V.fake_mode:
                combined.meta["val"] = smith.empty(
                    [len(seeds)], device=device, dtype=smith.int64
                )
...
def get_device(device):
    if device is not None:
        return device
    return smith.empty([]).device  # default device
