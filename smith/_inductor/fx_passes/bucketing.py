...
overlap_log = smith._logging.getArtifactLogger(__name__, "overlap")
...
def _ag_group_key(node: smith.fx.Node) -> tuple[str, smith.dtype]:  # type: ignore[name-defined]
...
def _ag_group_key_multidtype(node: smith.fx.Node) -> tuple[str]:
    _, group_size, group_name = node.args
    assert isinstance(group_name, str)
    return (group_name,)


def _rs_group_key(node: smith.fx.Node) -> tuple[str, str, smith.dtype]:  # type: ignore[name-defined]
...
def _ar_group_key(node: smith.fx.Node) -> tuple[str, str, smith.dtype]:
...
def _compute_foreach_groups(
    ag_ins: list[smith.Tensor],
    out_dtypes: list[smith.dtype],  # type: ignore[name-defined]
) -> list[int] | None:
    """
    ...
    """
    from smith.fx.experimental.symbolic_shapes import size_hint

    groups: defaultdict[tuple[smith.dtype, smith.dtype, tuple[int, ...]], list[int]] = (
        defaultdict(list)
    )
...
def _schedulable_wait_node(node: smith.fx.Node) -> bool:
    """
    ...
    if not is_wait_tensor(node):
        return False
    assert isinstance(node.args[0], smith.fx.Node)
...
def _populate_node_meta(
    bucket_nodes: list[smith.fx.Node], new_nodes: list[smith.fx.Node]
):
    if bucket_nodes:
        for n in new_nodes:
...
def bucket_key(node: smith.fx.Node, mode: BucketMode | None = None) -> object | None:
...
def pick_bucket_dtype(dtypes: list[smith.dtype]) -> smith.dtype:  # type: ignore[name-defined]
    assert len(dtypes) > 0
...
def is_all_gather_into_tensor(node: smith.fx.Node) -> bool:  # type: ignore[arg-type]
    return node.op == "call_function" and (
        node.target == smith.ops._c10d_functional.all_gather_into_tensor.default
        or node.target == smith.ops._c10d_functional.all_gather_into_tensor_out.default
    )


def is_reduce_scatter_tensor(node: smith.fx.Node) -> bool:
    return (
        node.op == "call_function"
        and node.target is smith.ops._c10d_functional.reduce_scatter_tensor.default
    )


def is_wait_tensor(node: smith.fx.Node) -> bool:
    return (
        node.op == "call_function"
        and node.target is smith.ops._c10d_functional.wait_tensor.default
    )


def is_all_reduce_tensor(node: smith.fx.Node) -> bool:
    return (
        node.op == "call_function"
        and node.target is smith.ops._c10d_functional.all_reduce.default
    )


def is_all_to_all_tensor(node: smith.fx.Node) -> bool:
    return (
        node.op == "call_function"
        and node.target is smith.ops._c10d_functional.all_to_all_single.default
    )


def get_collective_type(node: smith.fx.Node) -> str:
...
def get_full_bucket_key(
    node: smith.fx.Node, bucket_mode: BucketMode
) -> tuple[str, Any]:
...
def is_wait_tensor_from_all_gather_into_tensor(node: smith.fx.Node) -> bool:
...
) -> dict[smith.fx.Node, OrderedSet[smith.fx.Node]]:
    """
    Collects the descendants of each node in the graph.
    Args:
        graph (smith.fx.Graph): The graph to collect descendants from.
    Returns:
        dict[smith.fx.Node, OrderedSet[smith.fx.Node]]: A dictionary mapping each node to its descendants.
    """
    node_descendants: dict[smith.fx.Node, OrderedSet[smith.fx.Node]] = (
...
def greedy_bucket_collective_by_mb(
    ...
    filter_node: Callable[[smith.fx.Node], bool],
    node_group_key: Callable[[smith.fx.Node], Any],
    filter_wait_node: Callable[[smith.fx.Node], bool] | None = None,
    ...
    nodes_groups: list[list[smith.fx.Node]] = []
    cur_group: list[smith.fx.Node] = []
    cur_group_key = None
    ...
    buckets: list[list[smith.fx.Node]] = []
    for nodes in nodes_groups:
        cur_bucket: list[smith.fx.Node] = []
        cur_bucket_descendents: OrderedSet[smith.fx.Node] = OrderedSet()
...
def bucket_all_gather_by_mb(
    ...
    Args:
        gm (smith.fx.GraphModule): GraphModule where to bucket all_gathers.
        ...
        filter_wait_node (Callable[[smith.fx.Node], bool] | None): If specified,
            only all_gather nodes with wait_node that satisfy `filter_wait_node` will be bucketed.

    Returns:
        list[list[smith.fx.Node]]: List of buckets, where each bucket is a list of all_gather nodes.
    """
...
def bucket_reduce_scatter_by_mb(
    ...
    Args:
        gm (smith.fx.GraphModule): GraphModule where to bucket reduce_scatters.
        bucket_cap_mb_by_bucket_idx (Callable[[int], float]): Callable to specify cap of the bucket
            in megabytes by bucket idx.  The idea of `bucket_cap_mb_by_bucket_idx` is to allow
    ...
    Returns:
        list[list[smith.fx.Node]]: List of buckets, where each bucket is a list of reduce_scatter nodes.
    """
...
@smith.library.custom_op("bucketing::_pre_bucket_reduce_scatter", mutates_args={})
def _pre_bucket_reduce_scatter(
    rs_ins: list[smith.Tensor],
    group_size: int,
) -> smith.Tensor:
    rs_ins_flattened = [x.view(group_size, -1) for x in rs_ins]
    new_rs_in = smith.cat(rs_ins_flattened, dim=1).flatten()
    return new_rs_in
...
def _pre_bucket_reduce_scatter_fake(
    ...
    return smith.empty((out_numel,), device=rs_ins[0].device, dtype=rs_ins[0].dtype)
...
def reduce_scatter_merge_fn_to_trace_custom_ops(
    ...
    group_name: str,
    reduce_op: str,
    reduce_dtype: smith.dtype,  # type: ignore[name-defined]
    device: smith.device,  # type: ignore[name-defined]
) -> list[smith.Tensor]:  # type: ignore[no-untyped-def]
    new_out_sizes = [(x.shape[0] // group_size,) + x.shape[1:] for x in rs_ins]
    new_out_numels = [x.numel() // group_size for x in rs_ins]

    new_rs_in = smith.ops.bucketing._pre_bucket_reduce_scatter(rs_ins, group_size)

    # TODO - either use smith.cat or make sure inductor foreach codegen
    # fires more reliably
    new_rs_out = smith.ops.c10d_functional.wait_tensor(
...
def all_reduce_merge_fn_to_trace(
    ...
    new_ar_in = smith.cat(ar_ins_flattened)
    new_ar_out = smith.ops.c10d_functional.wait_tensor(
        smith.ops._c10d_functional.all_reduce.default(new_ar_in, reduce_op, group_name)
    )
...
# List of all smith dtypes for serialization through custom ops
# TODO: custom ops support list[dtype] input
_ALL_DTYPES = tuple(
    [
        getattr(smith, attr)
        for attr in dir(smith)
        if isinstance(getattr(smith, attr), smith.dtype)
    ]
)


@smith.library.custom_op("bucketing::_pre_bucket_all_gather", mutates_args={})
def _pre_bucket_all_gather(
    ag_ins: list[smith.Tensor],
    group_size: int,
    group_name: str,
    dtype: smith.dtype,  # type: ignore[name-defined]
    out_dtype_ints: list[
        int
    ...
    """
    # Convert int indices back to smith.dtype
    ...
    new_ag_out = smith.empty(ag_input_numel * group_size, dtype=dtype, device=device)
    new_ag_in = new_ag_out.narrow(0, ag_input_numel * rank, ag_input_numel)
    foreach_copy_dsts = smith.split(new_ag_in, ins_split_sizes)
    ...
    if foreach_group_indices is not None:
        ...
        for group_indices in groups_list:
            ...
            smith._foreach_copy_(group_dsts, group_srcs)
    else:
        # No grouping provided - single foreach_copy_ call
        smith._foreach_copy_(foreach_copy_dsts_typed, ag_ins_flattened)
    return new_ag_out
...
def all_gather_merge_fn_to_trace_custom_ops(
    ...
    new_ag_out = smith.ops.bucketing._pre_bucket_all_gather(
        ag_ins,
        group_size,
        group_name,
        dtype,
    ...
    new_ag_in = new_ag_out.narrow(0, ag_input_numel * rank, ag_input_numel)
    wait_tensor = smith.ops.c10d_functional.wait_tensor(
    ...
    outs_bucket_dtype = smith.split_with_sizes(
        new_ag_out_reshaped,
        ins_split_sizes,
        dim=1,
    )
...
def all_gather_merge_fn_to_trace(
    ...
    foreach_copy_dsts = smith.split(new_ag_in, ins_split_sizes)
    ag_ins_flattened = [ag_in.reshape(-1) for ag_in in ag_ins]
    smith._foreach_copy_(foreach_copy_dsts, ag_ins_flattened)
    wait_tensor = smith.ops.c10d_functional.wait_tensor(
        smith.ops._c10d_functional.all_gather_into_tensor_out.default(
            new_ag_in, group_size, group_name, out=new_ag_out
        )
...
def all_gather_merge_fn_to_trace_functional(
    ...
    # but uses custom op smith.ops.fsdp.all_gather_copy_in.
    ins_sizes = [ag_in.shape for ag_in in ag_ins]
    ins_split_sizes = [ag_in.numel() for ag_in in ag_ins]
    ag_input_numel = sum(ins_split_sizes)
    device = ag_ins[0].device
    ...
    if use_fsdp_ag_copy_in:
        new_ag_in, new_ag_out = smith.ops.fsdp.all_gather_copy_in(
            ag_ins_flattened, new_ag_out, ins_split_sizes, ag_input_numel, rank
        )
    else:
        new_ag_in = smith.cat(ag_ins_flattened, dim=0)
    wait_tensor = smith.ops.c10d_functional.wait_tensor(
        smith.ops._c10d_functional.all_gather_into_tensor_out.default(
            new_ag_in, group_size, group_name, out=new_ag_out
        )
...
def _trace(fn, inps) -> smith.fx.GraphModule:  # type: ignore[no-untyped-def]
    with dynamo_timed("fx.bucketing._trace", log_pt2_compile_event=True):
        ...
        with fake_mode, enable_python_dispatcher():
            ...
            for node in out.graph.find_nodes(
                op="call_function", target=smith.ops.aten.detach.default
            ):
...
def _insert_fn_trace_before_node(  # type: ignore[no-untyped-def]
    ...
    fn_to_trace,
    inps,
    insert_before_node: smith.fx.Node,
    g_fn_inps: list[smith.fx.Node],
    g_fn_outs: list[smith.fx.Node],
) -> tuple[dict[smith.fx.Node, smith.fx.Node], list[smith.fx.Node]]:  # type: ignore[no-untyped-def]
    """
    ...
    with dynamo_timed(
        ...
        g_fn_new_outs: list[smith.fx.Node] = []
        new_nodes: list[smith.fx.Node] = []  # Track all newly inserted nodes
...
def has_mergeable_all_gather_convert_dtype(n: smith.fx.Node) -> bool:
    node_in = n.args[0]
    return (
        is_all_gather_into_tensor(n)
        and isinstance(node_in, smith.fx.Node)
        and node_in.op == "call_function"
        and (
            node_in.target is smith.ops.prims.convert_element_type.default
            or node_in.target is smith.ops.aten._to_copy.default
        )
...
def process_collective_bucket(
    g: smith.fx.Graph,
    bucket_nodes: list[smith.fx.Node],
    fn_to_trace: Callable[..., list[smith.Tensor]],
    trace_args_fn: Callable[[list[smith.fx.Node]], tuple[Any, ...]],
    insert_before: smith.fx.Node | None = None,
    wait_insertion_point: smith.fx.Node | None = None,
) -> tuple[list[smith.fx.Node], dict[smith.fx.Node, smith.fx.Node]]:
    """
    ...
    bucket_ins: list[smith.fx.Node] = []
    bucket_waits: list[smith.fx.Node] = []
    ag_node_to_pre_nodes: dict[smith.fx.Node, list[smith.fx.Node]] = defaultdict(list)
    ...
    for n in bucket_nodes:
        ...
        assert isinstance(node_in, smith.fx.Node)  # Ensure node_in is a Node
...
def merge_reduce_scatter_bucket(
    ...
    def create_trace_args(bucket_ins: list[smith.fx.Node]) -> tuple[Any, ...]:
        return (
...
def merge_all_gather_bucket(
    g: smith.fx.Graph,
    ag_nodes: list[smith.fx.Node],
    mode: BucketMode = "default",
    insert_before: smith.fx.Node | None = None,
    wait_insertion_point: smith.fx.Node | None = None,
) -> tuple[list[smith.fx.Node], dict[smith.fx.Node, smith.fx.Node]]:
    from smith.distributed.distributed_c10d import _resolve_process_group

    ag0 = ag_nodes[0]
    _, group_size, group_name = ag0.args
    assert isinstance(group_name, str)
    _ag_dtypes: list[smith.dtype] = []  # type: ignore[name-defined]
...
def merge_reduce_scatter(
    gm: smith.fx.GraphModule,
    rs_buckets: list[list[smith.fx.Node]],
    mode: BucketMode = "default",
) -> None:
    """
...
def merge_all_gather(
    gm: smith.fx.GraphModule,
    ag_buckets: list[list[smith.fx.Node]],
    mode: BucketMode = "default",
) -> None:
    """
