...
import smith
import smith._inductor.runtime.runtime_utils
from smith import Tensor
from smith._dynamo.utils import counters, dynamo_timed
from smith._inductor import utils
from smith._inductor.autoheuristic.autoheuristic import (
    AHContext,
    AutoHeuristic,
    LocalFeedback,
)
from smith._inductor.autoheuristic.autoheuristic_utils import (
    context_add_strides,
    context_add_using_tf32,
    pad_mm_operations,
    pad_mm_precondition,
)
from smith._inductor.runtime.caching import encoders, memoizers
...
def get_alignment_size_dtype(dtype: smith.dtype) -> int:
    if dtype == smith.float16 or dtype == smith.half or dtype == smith.bfloat16:
        return 8
    elif dtype == smith.float32 or dtype == smith.float:
        return 4
    else:
        return 0
...
def realize_symbols(
    ds: smith.Size | tuple[smith.SymInt, ...],
) -> list[int]:
...
def can_pad(
    mat1: Tensor,
    mat2: Tensor,
    op: smith._ops.OpOverloadPacket,
    ...
    # Basic safety checks
    if not smith._inductor.config.shape_padding:
        return False
    ...
    with no_dispatch():
        if op is smith.ops.aten.mm or op is smith.ops.aten.addmm:
            m = mat1.shape[0]
            k = mat1.shape[1]
            n = mat2.shape[1]
        elif op is smith.ops.aten.bmm:
            m = mat1.shape[1]
            k = mat1.shape[2]
            n = mat2.shape[2]
        else:
    ...
    if (
        smith._inductor.config.deterministic
        and not smith._inductor.config.force_shape_pad
    ):
        return False
...
def get_padded_length(x: int | smith.SymInt, alignment_size: int) -> int:
    # we don't pad x if it is symbolic
    if isinstance(x, smith.SymInt) or alignment_size == 0 or x % alignment_size == 0:
        return 0
...
def should_pad_addmm(match: Match) -> bool:
    ...
    return should_pad(match, mat1, mat2, smith.ops.aten.addmm, input=input)
...
def is_mm_compute_bound(M: int, K: int, N: int, dtype: smith.dtype) -> bool:
    ...
    if (
        dtype is smith.bfloat16
        and K > M
        and K > N
        and (smith.xpu.is_available() or smith.cuda.get_device_capability() < (9, 0))
...
@functools.cache
def get_pad_cache() -> smith._inductor.codecache.LocalCache:
...
def should_pad_bench_key(
    ...
    def tensor_key(t: Tensor) -> tuple[smith.Size, tuple[int, ...], smith.dtype]:
        return (t.shape, t.stride(), t.dtype)

    tf32_key = (
        None
        if mat1.dtype != smith.float32
        else smith.backends.cuda.matmul.fp32_precision == "tf32"
        or smith.backends.mkldnn.fp32_precision == "tf32"
    )
...
def get_non_view_def(node: smith.fx.Node) -> smith.fx.Node:
...
def should_exclude_padding_time(match: Match, arg_name: str) -> bool:
    ...
    # TODO - see issue https://github.com/blacksmith/blacksmith/issues/128889
    ...
    if (
        ...
        > smith._inductor.config.max_pointwise_cat_inputs
    ):
        return False
...
def is_padded_faster(key: str, ori_time: float, pad_time: float) -> bool:
    ...
    if "shape_padding_multiplier" in smith._inductor.config.post_grad_fusion_options:
        multiplier = smith._inductor.config.post_grad_fusion_options[
...
def should_pad_mm_bf16(dtype: smith.dtype, M: int, N: int, K: int) -> bool:
    # always force pad for mm with bf16 when the following are satisfied to avoid perf regression
    large_k_threshold_to_pad = smith._inductor.config.post_grad_fusion_options[
        "pad_aten_mm_pass"
...
def get_do_bench() -> Callable[[Callable[[], Any]], float]:
    with dynamo_timed("pad_mm_benchmark_get_do_bench"):
        return functools.partial(
            ...
            smith._inductor.runtime.benchmarking.benchmarker.benchmark_gpu,
            warmup=5,
        )
...
def _should_pad(
    ...
    with no_dispatch():
        ...
        if smith._inductor.config.force_shape_pad:
            return True

        # Performance heuristic for bf16 large K scenarios
        if (
            "pad_aten_mm_pass" in smith._inductor.config.post_grad_fusion_options
        ...
        def realize_tensor(t):
            if isinstance(t, FakeTensor):
                ...
                real_t = smith.randn(real_size, dtype=t.dtype, device=t.device)
                return smith.as_strided(real_t, size_hints, stride_hint)
            else:
                return smith.randn_like(t)
        ...
        if ori_time is None and op is smith.ops.aten.addmm and input is not None:
        ...
        is_bmm = op is smith.ops.aten.bmm
        ...
        if op is smith.ops.aten.addmm:
            input_pad = None
            if input is not None and (input.is_cuda or input.is_xpu):
                input_pad = smith.randn_like(input)
            fns.append(
                lambda: pad_addmm(
                    input_pad,
                    mat1_pad,
        ...
        def orig_bench_fn():
            if op is smith.ops.aten.bmm or op is smith.ops.aten.mm:
                op(mat1, mat2)
            else:
                op(input, mat1, mat2)
        ...
        if (
            smith._inductor.config.run_autoheuristic("pad_mm")
            and op is smith.ops.aten.mm
        ):
...
def run_autoheuristic(
    ...
    if smith._inductor.config.collect_autoheuristic(name):
...
def should_pad_mm(match: Match) -> bool:
    ...
    return should_pad(match, mat1, mat2, smith.ops.aten.mm)
...
def should_pad_bmm(match: Match) -> bool:
    mat1, mat2 = fetch_fake_tensors(match, ("mat1", "mat2"))
    return should_pad(match, mat1, mat2, smith.ops.aten.bmm)
...
def _pad_mm_init() -> None:
    ...
    dim2a = functools.partial(smith.empty, (4, 4), device=device, requires_grad=True)
    dim2b = functools.partial(smith.empty, (4, 4), device=device, requires_grad=True)

    dim3a = functools.partial(smith.empty, (4, 4, 4), device=device, requires_grad=True)
    dim3b = functools.partial(smith.empty, (4, 4, 4), device=device, requires_grad=True)

    dim1a = functools.partial(smith.empty, (4), device=device, requires_grad=True)
