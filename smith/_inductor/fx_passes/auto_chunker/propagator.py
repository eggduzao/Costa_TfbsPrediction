...
zero matrix is created by smith.full. We will only know we should chunk that tensor
...
propagate_rules: dict[smith._ops.OpOverload, _HandlerType] = {}


def _register_propagate_rule(
    aten_op: smith._ops.OpOverload | Sequence[smith._ops.OpOverload],
...
def propagate_single_node(
    ...
    if not isinstance(target, smith._ops.OpOverload) or target not in propagate_rules:
        raise CantChunk(
