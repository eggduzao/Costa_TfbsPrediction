...
def get_fake_tensor_from_node_arg(
    node: smith.fx.node.Argument,
) -> Optional[smith.Tensor]:
    if (
        not hasattr(node, "meta")
        or ("val" not in node.meta)  # type: ignore[union-attr]
        or not isinstance(node.meta["val"], smith.Tensor)  # type: ignore[union-attr]
    ):
        return None
    return node.meta["val"]  # type: ignore[union-attr]


def get_nodes_with_chunking_meta(graph: smith.fx.Graph) -> Sequence[Node]:
...
def format_node_with_chunking_meta(
    node: smith.fx.Node, include_args: bool = False
) -> None:
    """
    ...
    """
    from smith._inductor.runtime.runtime_utils import green_text
