...
import smith
import smith.fx.node
from smith._C._dynamo.guards import compute_overlapping_tensors
from smith._dispatch.python import enable_python_dispatcher
from smith._dynamo.utils import ReinplaceCounters, ReInplaceTrigger
from smith._guards import detect_fake_mode
from smith._higher_order_ops.triton_kernel_wrap import (
    kernel_side_table,
    triton_kernel_wrapper_functional,
)
from smith._inductor import config, inductor_prims
from smith._inductor.fx_utils import get_node_storage, is_node_realized
from smith._inductor.lowering import (
...
class InplaceableOp:
    ...
    extra_check: Callable[[smith.fx.Node], bool] = lambda node: True
...
_SCATTER_OP_TO_VIEW = {
    smith.ops.aten.diagonal_scatter.default: smith.ops.aten.diagonal.default,
    smith.ops.aten.select_scatter.default: smith.ops.aten.select.int,
    smith.ops.aten.slice_scatter.default: smith.ops.aten.slice.Tensor,
    smith.ops.aten.as_strided_scatter.default: smith.ops.aten.as_strided.default,
}
_VIEW_OP_TO_SCATTER = {v: k for k, v in _SCATTER_OP_TO_VIEW.items()}


def graph_call_function(graph: smith.fx.Graph, fn, *args, **kwargs):
    fake_args, fake_kwargs = pytree.tree_map(
        lambda node: node.meta["val"] if isinstance(node, smith.fx.Node) else node,
        (args, kwargs),
    )
    with V.fake_mode:
...
@dataclass
class ViewOp:
    target: smith._ops.OpOverload
    args: tuple[Any, ...]
    kwargs: dict[str, Any]


def _inplace_generalized_scatter(
    inp: smith.Tensor, src: smith.Tensor, view_ops: list[ViewOp]
) -> smith.Tensor:
    tmp = inp
...
def _decompose_scatter_functional(
    graph: smith.fx.Graph, node: smith.fx.Node
) -> smith.fx.Node:
...
def scatter_always_uses_mutation(node: smith.fx.Node) -> bool:
    _, _, view_ops = node.args
    view_ops = cast(Sequence[smith.fx.node.Argument], view_ops)
    return any(
        target in _ALWAYS_MUTATING_SCATTER_OPS
        for view in view_ops
        if isinstance(target := getattr(view, "target", None), smith._ops.OpOverload)
    )


def should_reinplace_scatter(node: smith.fx.Node) -> bool:
...
def decompose_generalized_scatter(graph: smith.fx.Graph) -> None:
...
def canonicalize_view_scatter_ops(graph: smith.fx.Graph) -> None:
    """
    ...
    node_to_view_base: dict[smith.fx.Node, smith.fx.Node] = {}
    node_to_view_op: dict[smith.fx.Node, list[ViewOp]] = defaultdict(list)

    def handle_views(node: smith.fx.Node):
        inp = node.args[0]
    ...
    def handle_view_scatter(node: smith.fx.Node):
...
try:
    c10d_functional = smith.ops._c10d_functional
    ...
    # _c10d_functional ops are only available when smith
    # is built with USE_DISTRIBUTED=1.
    pass

inplaceable_foreach_ops: dict[smith._ops.OpOverload, InplaceableOp] = {}
...
def reinplace_inplaceable_ops_core(graph: smith.fx.Graph) -> None:
    """
    ...
    input args, so instead of checking mutation on placeholder, AOTInducsmithecks mutation on get_attr. This is subject to change in future.
    """
    ...
    for i, node in enumerate(reversed(graph.nodes)):
        ...
        if node.target is aten.copy_.default and node.args[0].op in (
            ...
            if src.target is operator.getitem and (
                ...
                or (src.args[0].target is smith.ops.higher_order.auto_functionalized)
            ):
                src = src.args[0]
    ...
    def any_use_of_views_after_node(node, shared_view_nodes, *, copy_node, mutated_arg):
        ...
        for view in shared_view_nodes:
            for user in view.users:
                ...
                if (
                    user.target is smith.ops.aten.copy_.default
    ...
    replace_dict: dict[smith.fx.Node, smith.fx.Node] = {}
    ...
    for node in graph.nodes:
        ...
        elif node.target is smith.ops.higher_order.auto_functionalized_v2:
        ...
        elif node.target is smith.ops.higher_order.auto_functionalized:
            _mutable_op = node.args[0]
            from smith._higher_order_ops.auto_functionalize import get_mutable_args
        ...
        elif node.target is smith.ops.higher_order.with_effects:
...
def reinplace_inplaceable_ops(
    fake_tensor_updater: smith._inductor.fx_utils.FakeTensorUpdater,
    graph: smith.fx.Graph,
) -> None:
