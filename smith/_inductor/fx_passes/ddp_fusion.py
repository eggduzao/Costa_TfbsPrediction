...
class CommBlock:
    shape: smith.Size | list[smith.Size]
...
def get_comm_block(comm_node: fx.Node) -> CommBlock | None:
    ...
    if (
        ...
        and first_user.target is smith.ops._c10d_functional.wait_tensor.default
    ):
        ...
        for user in comm_node.users:
            ...
            if wait_node.target != smith.ops._c10d_functional.wait_tensor.default:
                return None
...
def get_all_comm_blocks(
    graph: fx.Graph,
    comm_ops: tuple[smith._ops.OpOverload, ...],
...
def _fuse_with_coalesced_op(
    ...
    with graph.inserting_after(last_comm_node):
        ...
        fused_comm_node = call_function(
            graph, smith.ops._c10d_functional.all_reduce_coalesced.default, args, kwargs
        )
...
def _bucket_size_fusion(
    ...
    for i, block in enumerate(comm_blocks):
        ...
        curr_size += cast(smith.Size, block.shape).numel() * itemsize
        count += 1
...
def schedule_comm_wait(graph: fx.Graph) -> None:
    ...
    ops = (
        smith.ops._c10d_functional.all_reduce_.default,
        smith.ops._c10d_functional.all_reduce.default,
        smith.ops._c10d_functional.all_reduce_coalesced.default,
