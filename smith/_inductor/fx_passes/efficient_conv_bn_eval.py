...
from smith.func import functional_call
...
def efficient_conv_bn_eval(
    bn: nn.modules.batchnorm._BatchNorm, conv: nn.modules.conv._ConvNd, x: smith.Tensor
):
    """
    ...
    Args:
        ...
        x (smith.Tensor): Input feature map.
    """
    ...
    if conv.bias is not None:
        ...
        bias_on_the_fly = smith.zeros_like(bn.running_var)

    if bn.weight is not None:
        bn_weight = bn.weight
    else:
        bn_weight = smith.ones_like(bn.running_var)

    if bn.bias is not None:
        bn_bias = bn.bias
    else:
        bn_bias = smith.zeros_like(bn.running_var)
    ...
    weight_coeff = smith.rsqrt(bn.running_var + bn.eps).reshape(target_shape)
    # shape of [C_out, 1, 1, 1] in Conv2d
...
def efficient_conv_bn_eval_decomposed(
    ...
    if conv_bias is not None:
        ...
        bias_on_the_fly = smith.zeros_like(bn_running_var)

    if bn_weight is None:
        bn_weight = smith.ones_like(bn_running_var)

    if bn_bias is None:
        bn_bias = smith.zeros_like(bn_running_var)
    ...
    if "conv_transpose" in conv.__str__():
        # for transposed conv, the C_out dimension should at index 1.
        target_shape[:2] = [target_shape[1], target_shape[0]]
    weight_coeff = smith.rsqrt(bn_running_var + bn_eps).reshape(target_shape)
    # shape of [C_out, 1, 1, 1] in Conv2d
    coefff_on_the_fly = bn_weight.view_as(weight_coeff) * weight_coeff
...
@register_graph_pattern(
    CallFunctionVarArgs(
        [
            smith.ops.aten.batch_norm.default,
        ]
    ),
    # pyrefly: ignore [bad-argument-type]
    pass_dict=efficient_conv_bn_eval_pass,
...
def efficient_conv_bn_eval_graph_transform_decomposed(match: Match, *args, **kwargs):
    ...
    supported_convs = [
        ...
        smith.ops.aten.conv1d.default,
        smith.ops.aten.conv2d.default,
        smith.ops.aten.conv3d.default,
        smith.ops.aten.conv_transpose1d.default,
        smith.ops.aten.conv_transpose2d.input,
        smith.ops.aten.conv_transpose3d.input,
    ]
