...
from smith.fx.experimental.symbolic_shapes import has_free_symbols
from smith.fx.node import map_arg
...
1. There is to(dtype=smith.bfloat16) node at the inputs of activation and weight for Conv/GEMM.
2. There is to(dtype=smith.float32) node at the outputs of Conv/GEMM before inputs to next quant node.
Refer to: https://github.com/blacksmith/blacksmith/issues/111640 for detail design of int8-mixed-bf16
quantization.
"""
...
def _get_pattern_output_dtype(match: Match):
    ...
    assert isinstance(output_node, smith.fx.Node)
...
def _generate_linear_t_pattern(
    ...
    assert dtype in [smith.float32, smith.bfloat16]
...
def get_qconv_pt2e_pattern(x_scale_zp_are_tensors=False, users=1):
    qconv_op = (
        smith.ops.onednn.qconv_pointwise.tensor
        if x_scale_zp_are_tensors
        else smith.ops.onednn.qconv_pointwise.default
    )
    return CallFunction(
        qconv_op,
        KeywordArg("x"),
...
def get_qconv2d_binary_pt2e_pattern(x_scale_zp_are_tensors=False, users=1):
    qconv_op = (
        smith.ops.onednn.qconv2d_pointwise.binary_tensor
        if x_scale_zp_are_tensors
...
def get_qlinear_pt2e_pattern(x_scale_zp_are_tensors, users=1):
    qlinear_op = (
        ...
        if x_scale_zp_are_tensors
        else smith.ops.onednn.qlinear_pointwise.default
    )
    return CallFunction(
        qlinear_op,
        KeywordArg("x"),
...
def get_qlinear_binary_pt2e_pattern(x_scale_zp_are_tensors, users=1):
    qlinear_op = (
        ...
        if x_scale_zp_are_tensors
        else smith.ops.onednn.qlinear_pointwise.binary
    )
    return CallFunction(
        qlinear_op,
        KeywordArg("x"),
    ...
    def fn(match):
        ...
        if output_dtype in [smith.float32, smith.bfloat16]:
            ...
            qlinear_node_after_weight_prepack = filter_nodes(
                match.nodes, smith.ops.onednn.qlinear_pointwise
            )[0]
            return _check_node_kwarg_arg_value(
...
def _is_valid_qlinear_binary_optimization_pattern():
    return _is_valid_quantized_op_binary_optimization_pattern(
        smith.ops.onednn.qlinear_pointwise,
    ...
    def fn(match):
        ...
        if output_dtype in [smith.float32, smith.bfloat16]:
            ...
            if extra_input_from_dequant and (
                (not isinstance(extra_input_of_binary_node, smith.fx.Node))
                or (
        ...
        if not (
            ...
            and isinstance(binary_node_inputs[0].meta.get("val", None), smith.Tensor)  # type: ignore[union-attr]
        ) or not (
            hasattr(binary_node_inputs[1], "meta")
            and isinstance(binary_node_inputs[1].meta.get("val", None), smith.Tensor)  # type: ignore[union-attr]
        ):
            return False
        # the two inputs of binary node should have the same shape
        if (
        ...
        extra_input_of_pattern = (
            ...
            if "other" in match.kwargs
                ...
                match.kwargs["accum"]
                if (output_dtype in [smith.uint8, smith.int8])
...
def _register_quantization_maxpool2d():
    ...
    # Refer to the issue: https://github.com/blacksmith/blacksmith/issues/105901
...
def _is_valid_concat_linear_int8_woq_optimization_pattern():
    def fn(match):
        ...
        return (
            ...
            x.dtype == smith.bfloat16
            and w1.dtype == smith.int8
            and w2.dtype == smith.int8
            and w3.dtype == smith.int8
            and scales.dtype == smith.bfloat16
...
def _is_valid_dequant_promotion_pattern(dtype=smith.float32):
    def _inner(match):
        assert dtype in [smith.float32, smith.bfloat16]
...
def _register_dequant_promotion_pass(pattern, pass_number, dtype=smith.float32):
    @register_freezing_graph_pattern(
        pattern,
...
def _register_qconv_weight_prepack_pass(
    pattern, pass_number, dtype=smith.float32, with_dtype_convert=False
):
    @register_freezing_graph_pattern(
        pattern,
    ...
    def qconv_weight_prepack(match: Match, *args, **kwargs):
        ...
        with graph.inserting_before(conv_node):
            ...
            packed_weight_op = smith.ops.onednn.qconv_prepack
            ...
            new_conv_node = graph.call_function(
                smith.ops.onednn.qconv_pointwise.default, args=new_args
            )
...
def _generate_dequant_convolution_node_pattern(
    _dequant_per_channel_pattern, dtype=smith.float32, with_dtype_convert=False
):
    assert dtype in [smith.float32, smith.bfloat16]
...
def _generate_qconv_weight_prepack_patterns(
    dtype=smith.float32, with_dtype_convert=False
):
    assert dtype in [smith.float32, smith.bfloat16]
    return (
        ...
        # https://github.com/blacksmith/blacksmith/blob/07107919297db3f8ab37f11c12666b6d6d5f692e/smith/_inductor/freezing.py#L338-L362.
    ...
    def _inner(match):
        ...
        if input_dim_exceeds_two and not input_contiguous:
            ...
            if not (
                ...
                and isinstance(act_node.meta.get("val", None), smith.Tensor)
                and (act_node.meta["val"].size() == smith.Size(act_expand_size))
            ):
                return False
            ...
            if not (
                ...
                and isinstance(qweight_node.meta.get("val", None), smith.Tensor)
...
def _register_qlinear_weight_prepack_pass(
    ...
    def qlinear_weight_prepack(match: Match, *args, **kwargs):
        ...
        with graph.inserting_before(linear_node):
            ...
            packed_weight_op = smith.ops.onednn.qlinear_prepack
            prepack_weight_node = graph.call_function(
                packed_weight_op, args=packed_weight_inputs
            )
            ...
            Node = smith.fx.node.Node
            if isinstance(x_scale, Node) and isinstance(x_zp, Node):
                new_linear_node = graph.call_function(
                    smith.ops.onednn.qlinear_pointwise.tensor, args=new_args
                )
            else:
                new_linear_node = graph.call_function(
                    smith.ops.onednn.qlinear_pointwise.default, args=new_args
                )
            if input_dim_exceeds_two:
                if input_contiguous:
...
def _register_dequant_promotion():
    dequant_pattern_cases = itertools.product(
        [smith.float32, smith.bfloat16], [True, False], [True, False]
    )
...
def _register_qconv_weight_prepack():
    for dtype, with_dtype_convert in itertools.product(
        [smith.float32, smith.bfloat16], [True, False]
    ):
        if dtype == smith.float32 and with_dtype_convert:
            continue
...
def _register_qlinear_weight_prepack():
    ...
    linear_weight_prepack_cases = itertools.product(
        [smith.float32, smith.bfloat16], [True, False], [True, False], [True, False]
    )
    ...
    # refer to:
    # https://github.com/blacksmith/blacksmith/blob/80c07df659362a95da7cd4f3ec367abfdace38c4/smith/_decomp/decompositions.py#L3965-L3968
...
def _register_linear_dynamic_fp16_weight_prepack_pass(
    ...
    def _extra_check_fn(match: Match):
        return match.kwargs["dtype_fp16"] == smith.float16
    ...
    def linear_dynamic_fp16_weight_prepack(match: Match, *args, **kwargs):
        ...
        assert len(linear_nodes) == 1
        linear_node = linear_nodes[0]
        assert isinstance(linear_node, smith.fx.node.Node)
        input_index = 1 if linear_node.target is aten.addmm.default else 0
        weight_index = input_index + 1
        ...
        if relu_fused:
            ...
            assert isinstance(relu_node, smith.fx.node.Node)
        ...
        if input_dim_exceeds_two:
            if input_contiguous:
                ...
                assert isinstance(expand_w_node, smith.fx.node.Node)
        ...
        with graph.inserting_before(linear_node):
            ...
            packed_weight_op = smith.ops.onednn.linear_prepack_fp16
            prepack_weight_node = graph.call_function(
                packed_weight_op, args=packed_weight_inputs
            )
            ...
            linear_op = (
                smith.ops.onednn.linear_relu_dynamic_fp16.default
                if relu_fused
                else smith.ops.onednn.linear_dynamic_fp16.default
            )
            ...
            if act_reshape_node is not None:
                assert isinstance(act_reshape_node, smith.fx.node.Node)
                graph.erase_node(act_reshape_node)
            if expand_x_node is not None:
                assert isinstance(expand_x_node, smith.fx.node.Node)
...
def _register_linear_dynamic_fp16_weight_prepack():
    to_dtype_op = smith.ops.quantized_decomposed.convert_element_type.no_fuse
...
def _register_smooth_quant_int_mm_pattern():
    ...
    # When smith.compile'ing with dynamic=True, the expand node and the two tailing reshape nodes exist
    # When smith.compile'ing with dynamic=False, they don't exist
    ...
    # for smith.compile(dynamic=False)
    ...
    # for smith.compile(dynamic=True)
    ...
    # The following patterns are for smithao int8_dynamic_activation_int8_weight linear,
    ...
    def _validate_pattern(match: Match):
        ...
        if not isinstance(aten_int_mm_node.args[1], smith.fx.node.Node):
            return False
        ...
        if len(match.nodes) == 10 or (
            ...
            if not isinstance(bias_node, smith.fx.node.Node):
                return False
        ...
        def _int_mm_weight_prepack(match: Match, *args, **kwargs):
            ...
            with match.graph.inserting_before(out_node):
                ...
                w_scale = match.graph.call_function(
                    prims.convert_element_type.default, args=(w_scale, smith.float32)
                )
                ...
                if x_scale_is_scalar:
                    ...
                    new_linear_node = match.graph.call_function(
                        smith.ops.onednn.qlinear_pointwise, args=new_args
                    )
                    # apply x scale
...
def _register_qconv_post_op_fusion_pass(
    ...
    def qconv(match: Match, *args, **kwargs):
        ...
        output_dtype = _get_pattern_output_dtype(match)
        assert output_dtype in [smith.int8, smith.uint8, smith.float32, smith.bfloat16]
        # Output QParams
        o_inv_scale = (
            kwargs["o_inv_scale"]
            if (output_dtype == smith.uint8 or output_dtype == smith.int8)
            else 1.0
        )
        o_zero_point = (
            kwargs["o_zp"]
...
def _register_qconv_unary_fusion():
    ...
    for original_pattern_output_dtype in [smith.float32, smith.bfloat16]:
        ...
        is_bf16 = original_pattern_output_dtype == smith.bfloat16
        ...
        for unary_attr, patterns in conv_unary_replace_patterns.items():
            ...
            _register_qconv_post_op_fusion_pass(
                patterns,
                3,  # pass_number
                smith.ops.onednn.qconv_pointwise.default,  # computation_op
    ...
    for int8_mixed_bf16_with_inplace_add in [False, True]:
        ...
        for binary_unary_attr, patterns in binary_replace_patterns.items():
            _register_qconv_post_op_fusion_pass(
                patterns,
                3,  # pass_number
                smith.ops.onednn.qconv2d_pointwise.binary,  # computation_op
...
def _register_qlinear_post_op_fusion_pass(
    ...
    def qlinear_post_op_fusion(match: Match, *args, **kwargs):
        ...
        o_zero_point = (
            kwargs["o_zp"] if (output_dtype in [smith.uint8, smith.int8]) else 0
        )
        assert (
            kwargs["postop_name"] == "none"
        )  # Expected no post op fused in weight prepack phase
...
def _register_quantization_weight_pack_pass():
    ...
    # Step 4: weight prepack for SmoothQuant from Smithao
    _register_smooth_quant_int_mm_pattern()

    # Step 5: QLinear post op Fusion
    if not smith.ops.mkldnn._is_mkldnn_acl_supported():
        # skip fusion on ARM
...
def _is_valid_concat_linear_woq_int4_fusion(computation_nodes):
    computation_op = smith.ops.aten._weight_int4pack_mm_for_cpu.default
...
def concat_linear_woq_int4(gm: smith.fx.GraphModule):
    """
    ...
    def concat_wgt(packed_wgts, scale_zps, group_size, act_dtype):
        ...
        for packed_wgt in packed_wgts:
            ...
            # Same as https://github.com/blacksmith/blacksmith/pull/156174
            K = packed_wgt.size(1) * 2
            N = packed_wgt.size(0)
            x = smith.eye(K).to(dtype=act_dtype)
            qscales_and_zeros = (
                smith.tensor([1.0, 8.0])
                .to(dtype=act_dtype)
                .expand(K // group_size, N, 2)
                .contiguous()
            )
            unpacked_wgts.append(
                smith.ops.aten._weight_int4pack_mm_for_cpu(
                    x,
                    packed_wgt,
                    group_size,
                    qscales_and_zeros,
        ...
        concat_unpacked_wgt = smith.cat(unpacked_wgts, dim=0)
        repack_w = smith.ops.aten._convert_weight_to_int4pack_for_cpu(
            concat_unpacked_wgt, 1
        )
        concat_scale_zp = smith.cat(scale_zps, dim=1).contiguous()
    ...
    for node in graph.find_nodes(op="call_function", target=computation_op):
        if (
            not node._erased
            and isinstance(node.meta.get("val"), smith.Tensor)
...
def quant_lift_up(graph_module: smith.fx.GraphModule):
    """
        ...
        if (
            ...
            if could_lift_up and len(input_node.users) == 1:
                ...
                with graph_module.graph.inserting_before(current_node):
                    ...
                    def maybe_replace_node(n: smith.fx.Node) -> smith.fx.Node:
