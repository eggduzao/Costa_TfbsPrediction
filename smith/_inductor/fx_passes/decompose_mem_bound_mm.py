...
def realize_inputs(inputs: list[smith.fx.Node]):
    for inp in inputs:
        if isinstance(inp, smith.fx.node.Node):
...
def print_decompose_pattern(match: Match, inputs: list[smith.fx.Node]):
    node = match.nodes[-1]
    log.debug(
...
def decompose_bmm(match: Match, mat1: smith.fx.Node, mat2: smith.fx.Node):
    def repl(mat1, mat2):
        return smith.sum(mat1[:, :, :, None] * mat2[:, None, :, :], dim=-2).to(
            mat1.dtype
        )
...
def decompose_addmm(
    ...
    def repl(mat1, mat2, mat3):
        return (
            smith.sum(mat2[:, :, None] * mat3[None, :, :], dim=-2).to(mat2.dtype) + mat1
        )
...
def decompose_mm(
    ...
    def repl(mat1, mat2):
        return smith.sum(mat1[:, :, None] * mat2[None, :, :], dim=-2).to(mat1.dtype)
