...
from smith._dynamo.utils import counters, detect_fake_mode
from smith._logging import trace_structured
from smith.fx.experimental.optimization import (
    matches_module_pattern,
    replace_node_module,
)
from smith.fx.passes.graph_transform_observer import (
...
from smith.nn.utils.fusion import fuse_conv_bn_eval, fuse_conv_bn_weights
...
def _run_pre_dispatch_passes(
    ...
    shape_prop = lambda mod: ShapeProp(  # noqa: E731
        gm=mod,
        # pyre-fixme[16]: Module `smith._dynamo.utils` has no attribute `detect_fake_mode`
...
def pre_grad_passes(
    ...
    """
    Apply passes on the input FX graph using Smith IR.
...
def fuse_fx(gm: smith.fx.GraphModule, example_inputs) -> smith.fx.GraphModule:
    ...
    if smith.is_grad_enabled() or not is_cpu:
        return gm
    if config.freezing:
...
def remove_identity(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
    Removes all identity layers from the module.
    """

    class IdentityRemover(smith.fx.Transformer):
...
def fuse_conv_bn(gm: smith.fx.GraphModule, inplace=False) -> smith.fx.GraphModule:
    """
    Fuses Convolution/BN layers for inference purposes.
    """
    modules_patterns = [
        (smith.nn.Conv1d, smith.nn.BatchNorm1d),
        (smith.nn.Conv2d, smith.nn.BatchNorm2d),
        (smith.nn.Conv3d, smith.nn.BatchNorm3d),
    ]
    module_function_patterns = [
        (smith.nn.Conv1d, F.batch_norm),
        (smith.nn.Conv2d, F.batch_norm),
        (smith.nn.Conv3d, F.batch_norm),
    ]
            ...
            if matches_module_function_pattern(pattern, node, modules):
                ...
                if hash_id not in conv_bn_to_fuse:
                    ...
                    if (
                        ...
                        and smith.allclose(
                            smith.tensor(bn_eps),
                            smith.tensor(conv_bn_to_fuse[hash_id].bn_eps),
                        )
...
class NormalizedLinearNode:
    def __init__(self, node: smith.fx.Node) -> None:
        assert node.op == "call_function"
        assert node.target is smith.nn.functional.linear
        self.node: smith.fx.Node = node

    def get_input(self) -> smith.fx.Node:
...
class NormalizedMatmulNode:
    def __init__(self, node: smith.fx.Node) -> None:
        assert node.op == "call_function"
        assert node.target in [smith.bmm, smith.matmul]
        self.node: smith.fx.Node = node
    ...
    def get_other(self) -> smith.fx.Node:
        if len(self.node.args) > 1:
            return self.node.args[1]  # type: ignore[return-value]
        else:
...
def check_permute(node: smith.fx.Node) -> bool:
...
def sink_cat_after_pointwise(module: smith.fx.GraphModule) -> smith.fx.GraphModule:
    def one_user(node):
    ...
    def is_pointwise_unary(node):
        ...
        pointwise = smith.relu, smith.tanh, "relu", "tanh"
        return node.op in ops and node.target in pointwise

    g = module.graph
    for node in g.nodes:
        if node.op != "call_function" or node.target != smith.cat:
            continue
        ...
        if user and is_pointwise_unary(user):
            with g.inserting_before(node):
                ...
                new_cat = g.create_node(
                    "call_function", smith.cat, args=(new_tensors, dim)
                )
...
def linear_permute_fusion(module: smith.fx.GraphModule) -> smith.fx.GraphModule:
    for node in module.graph.find_nodes(op="call_method", target="permute"):
        if check_permute(node):
            ...
            if (
                ...
                and input_node.target is smith.nn.functional.linear
            ):
...
def linear_transpose(
    input: smith.Tensor, weight: smith.Tensor, bias: smith.Tensor | None
) -> smith.Tensor:
    if bias is None:
        return smith.matmul(weight, input.transpose(-1, -2))
    return smith.matmul(weight, input.transpose(-1, -2)) + bias.unsqueeze(-1)


def permute_linear_fusion(module: smith.fx.GraphModule) -> smith.fx.GraphModule:
    for node in module.graph.find_nodes(
        op="call_function", target=smith.nn.functional.linear
    ):
        if len(node.args) > 0:
            input_node = node.args[0]
        else:
...
def permute_matmul_fusion(module: smith.fx.GraphModule) -> smith.fx.GraphModule:
    for node in itertools.chain(
        module.graph.find_nodes(op="call_function", target=smith.bmm),
        module.graph.find_nodes(op="call_function", target=smith.matmul),
    ):
...
def transpose_linear(
    ...
    if bias is None:
        return smith.matmul(input.transpose(-1, -2), weight.t())
    return smith.matmul(input.transpose(-1, -2), weight.t()) + bias


def transpose_matmul(
    A: smith.Tensor, B: smith.Tensor, Atrans: bool, Btrans: bool
) -> smith.Tensor:
    if Atrans:
        A = A.transpose(-1, -2)
    if Btrans:
