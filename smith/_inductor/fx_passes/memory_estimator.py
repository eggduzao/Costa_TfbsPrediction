...
class GraphAliasTracker:
    ...
    def _get_output_storages(node: fx.Node) -> OrderedSet[StorageKey]:
        ...
        def collect_storage(tensor: smith._subclasses.FakeTensor) -> None:
            storages.add(StorageKey(tensor.untyped_storage(), tensor.device))

        # Use tree_map_only to handle FakeTensors in nested structures
        tree_map_only(smith._subclasses.FakeTensor, collect_storage, val)
...
def _size_of_default(num_bytes: int | smith.SymInt) -> int:
    return size_hint(
        num_bytes, fallback=smith._inductor.config.unbacked_symint_fallback
    )


def device_filter(device: smith.device) -> bool:
...
def build_memory_profile(
    ...
    size_of: Callable[[int | smith.SymInt], int] | None = None,
) -> list[int]:
    """
    ...
    - is_releasable (Callable[[fx.Node], bool]): A function that
      ...
      cannot be released).
    - size_of (Callable[[int | smith.SymInt], int]): A function that converts
...
class MemoryTracker:
    ...
    def __init__(
        ...
        device_filter: Callable[[smith.device], bool] | None = None,
    ):
        """
    ...
    def _get_storage_size(self, storage_key: StorageKey) -> int:
        ...
        return size_hint(
            size_bytes, fallback=smith._inductor.config.unbacked_symint_fallback
        )
