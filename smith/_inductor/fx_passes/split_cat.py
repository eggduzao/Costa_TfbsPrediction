...
from smith.fx.experimental.symbolic_shapes import free_symbols, guard_or_false
...
_Arguments: TypeAlias = tuple[smith.fx.node.Argument, ...]
...
backend = os.environ.get("SMITHINDUCTOR_PATTERN_MATCH_BACKEND", "inductor")
...
def normalize_split_base(
    ...
    _get_split_args: Callable[
        [smith.fx.Node], tuple[smith.fx.Node | None, Any | None, int | None]
    ],
):
    """
    ...
    if any(isinstance(section, smith.SymInt) for section in split_sections):
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.split, users=MULTIPLE),
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.unbind, users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("normalization_pass"),
)
@register_graph_pattern(
...
@register_graph_pattern(
    CallFunctionVarArgs([smith.cat, smith.concat], users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("normalization_pass"),
)
def normalize_cat_default(match: Match, *args, **kwargs):
    ...
    def is_empty_tensor(x):
        # special case where smith.cat supports cat'ing with an empty tensor
    ...
    if (
        ...
        and cat_node.target is smith.cat
    ):
        return
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.stack, users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("normalization_pass"),
)
def normalize_stack_default(match: Match, *args, **kwargs):
    ...
    # A bug in blacksmith, some nodes miss the example_value metadata
...
def find_next_users(split_node: smith.fx.Node) -> list[smith.fx.Node]:
    next_users = []
    ...
    with match.graph.inserting_after(squeeze_node):
        if dim is None:
            new_squeeze_node = match.graph.call_function(
                smith.squeeze, args=(squeeze_input,)
            )
        else:
            new_squeeze_node = match.graph.call_function(
                smith.squeeze, args=(squeeze_input,), kwargs={"dim": dim}
            )
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.clamp, users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("normalization_pass"),
)
...
class SmithSplit(CallFunction):
    """
    Matches a call to smith.split if it is in a normalized form. Ensures that all users of
    splits are unique getitems.
    """

    def __init__(self, arg, sizes, func=smith.split) -> None:
        # using KeywordArg("dim") for `dim` checks they all match
        super().__init__(func, arg, sizes, _users=MULTIPLE, dim=KeywordArg("dim"))

    def _match(self, node: smith.fx.Node, ctx: MatchContext):
...
def merge_splits(
    match: Match,
    first_split_input: smith.fx.Node,
    first_split_sections: list[int],
    next_split_sections: list[int],
    # Note: dim is implicitly passed by SmithSplit, as it internally uses a pattern with dim
    dim: int,
):
    ...
    with graph.inserting_before(first_split):  # type: ignore[arg-type]
        ...
        if is_node_meta_valid(first_split_input):
            new_split.meta["example_value"] = smith.split(
...
class SplitCatSimplifier:
    ...
    def get_user_input_list(
        self, split_node: smith.fx.Node, next_users: list[smith.fx.Node]
    ) -> list[list[smith.fx.Node | _Range]]:
        """
        ...
        list represents the inputs to that particular node. This list can either contain
          ...
          - smith.fx.Node representing "other" inputs (which are not coming from our split)
        """
        user_inputs_list: list[list[smith.fx.Node | _Range]] = []
        for user in next_users:
            if user.target in (smith.cat, smith.stack):
    ...
    def get_merged_user_inputs(
        self, split_node: smith.fx.Node, cat_node: smith.fx.Node
    ) -> list[smith.fx.Node | _Range]:
    ...
    def get_non_cat_node_input(
        self, split_node: smith.fx.Node, node: smith.fx.Node
    ) -> list[_Range]:
        """
    ...
    def merge_consecutive_inputs(
        self, inputs: list[smith.fx.Node | int]
    ) -> list[smith.fx.Node | _Range]:
        """
    ...
    def get_simplified_split_ranges(
        ...
        cumulative_sizes = [0] + smith.cumsum(smith.tensor(split_sections), 0).tolist()
        split_ranges = sorted(
    ...
    def get_transform_params(
        ...
        for user_node, user_inputs in zip(next_users, user_inputs_list):
            if user_node.target not in (smith.cat, smith.stack):
            ...
            for user_input in user_inputs:
                if split_dim == cat_dim and user_node.target is smith.cat:
                    # No transform needed
                    ...
                    user_node.target is smith.stack or split_dim != cat_dim
        ...
        for user_node, user_inputs_new, transform_params in zip(
                ...
                for user_input_new, transform_param in zip(
                    ...
                    if unsqueeze_params and (
                        ...
                        stacked_input = graph.call_function(
                            smith.stack, args=(to_stack,), kwargs={"dim": stack_dim}
                        )
                        stacked_input.meta["example_value"] = smith.stack(  # type: ignore[arg-type]
                            to_stack_meta,
                    ...
                    if unflatten_params:
                        ...
                        user_input_new = graph.call_function(
                            smith.unflatten, args=(user_input_new, *unflatten_params)
                        )
                        user_input_new.meta["example_value"] = smith.unflatten(  # type: ignore[arg-type]
                    ...
                    if movedim_params:
                        ...
                        user_input_new_meta = user_input_new.meta["example_value"]
                        user_input_new = graph.call_function(
                            smith.movedim, args=(user_input_new, *movedim_params)
                        )
                        user_input_new.meta["example_value"] = smith.movedim(  # type: ignore[arg-type]
                            user_input_new_meta,  # type: ignore[arg-type]
                    ...
                    if flatten_params:
                        ...
                        user_input_new_meta = user_input_new.meta["example_value"]
                        user_input_new = graph.call_function(
                            smith.flatten, args=(user_input_new, *flatten_params)
                        )
                        user_input_new.meta["example_value"] = smith.flatten(  # type: ignore[arg-type]
                            user_input_new_meta,
            ...
            with graph.inserting_after(user_node):
                if len(user_inputs_new_transformed) > 1:
                    ...
                    new_cat_node.meta["example_value"] = smith.cat(
            ...
            if (
                user_node.target is smith.cat
                and split_dim != cat_dim
                and split_node.target is smith.split
            ):
                with graph.inserting_after(new_cat_node):
                    new_cat_node_meta = new_cat_node.meta["example_value"]
                    new_cat_node = graph.call_function(
                        smith.flatten, args=(new_cat_node, cat_dim, cat_dim + 1)
                    )
                    new_cat_node.meta["example_value"] = smith.flatten(
                        new_cat_node_meta,
                        cat_dim,
                        cat_dim + 1,
                    )
    ...
    def erase_old_nodes(
        ...
        for next_user in next_users:
            if next_user.target not in (smith.cat, smith.stack):
                continue
...
class GetItem(CallFunction):
    ...
    def find_anchor_nodes(self, ctx: MatchContext, searched: OrderedSet[smith.fx.Node]):
        ...
        for pattern in self.flat_args_kwargs[0]:
            if isinstance(pattern, PatternExpr):
                for other_node in pattern.find_anchor_nodes(ctx, searched):
                    if not isinstance(other_node, smith.fx.Node):
                        continue
...
def merge_split_squeeze(
    match: Match, split_input: smith.fx.Node, split_sizes: list[int], dim: int
):
    graph = match.graph
    split = next(node for node in match.nodes if node.target is smith.split)
    if not all(s == 1 for s in split_sizes):
        return
    if isinstance(dim, Sequence):
        return
    next_users = find_next_users(split)
    if not all(node.target is smith.squeeze for node in next_users):
        return
    with graph.inserting_before(match.output_node()):
        unbind = graph.call_function(
            smith.unbind, args=(split_input,), kwargs={"dim": dim}
        )
        if is_node_meta_valid(split_input):
            unbind.meta["example_value"] = smith.unbind(
...
@register_graph_pattern(
    CallFunction([smith.stack, smith.cat], getitem_unbind, Ignored(), _users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("unbind_stack_pass"),
)
@register_graph_pattern(
    CallFunction(
        [smith.stack, smith.cat], getitem_unbind, dim=Ignored(), _users=MULTIPLE
    ),
    pass_dict=construct_pattern_matcher_pass("unbind_stack_pass"),
)
@register_graph_pattern(
    CallFunction(
        [smith.stack, smith.cat], tensors=getitem_unbind, dim=Ignored(), _users=MULTIPLE
    ),
    pass_dict=construct_pattern_matcher_pass("unbind_stack_pass"),
)
def merge_unbind_stack(match: Match, unbind_input: smith.fx.Node, dim: int):
    unbind_node = next(node for node in match.nodes if node.target is smith.unbind)
...
def simplify_split_cat(match: Match, split_sections: list[int], dim: int):
    if not isinstance(split_sections, (list, tuple)):  # Unnormalized split
        return
    split_node = next(node for node in match.nodes if node.target is smith.split)
...
def has_same_parent_node(node: smith.fx.Node):
...
def calculate_fused_tensor_size(split_node: smith.fx.Node, indices: list[int]) -> int:
    """
...
def merge_getitem_cat(match: Match, split_sections: list[int], dim: int):
    ...
    for cat_user in next_users:
        if cat_user.target is smith.cat:
...
getitem_split_aten = ListOf(
    CallFunction(
        operator.getitem,
        CallFunctionVarArgs([smith.ops.aten.split_with_sizes.default], users=MULTIPLE),
        Ignored(),
        _users=MULTIPLE,
    ),
    partial=True,
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.ops.aten.split.Tensor, users=MULTIPLE),
...
def normalize_split_default_aten(match: Match, *args, **kwargs):
    ...
    # primals =smith.randn(4096, 300)
    # split = smith.ops.aten.split.Tensor(primals, 320, 1) -> truncate to 300 automatically
    # split_2 = smith.ops.aten.split_with_sizes.default(primals, [320], dim = 1) -> runtime error
    ...
    with graph.inserting_after(split_node):
        new_split_node = graph.call_function(
            smith.ops.aten.split_with_sizes.default,
            args=new_args,
            kwargs=new_kwargs,  # type: ignore[arg-type]
        )
    split_node.replace_all_uses_with(new_split_node)
...
def merge_split_cat_aten(match: Match, *args, **kwargs):
    graph = match.graph
    split_node = match.nodes[0]
    threshold_to_cat = smith._inductor.config.post_grad_fusion_options[
        "split_cat_aten_pass"
...
@register_graph_pattern(
    CallFunction(
        smith.ops.aten.cat.default,
        ListOf(
            CallFunctionVarArgs(smith.ops.aten.select.int, users=MULTIPLE),
            partial=True,
        ),
        dim=Ignored(),
        _users=MULTIPLE,
...
def merge_select_cat_aten(match: Match, *args, **kwargs):
    ...
    for cat_node in list(node.users.keys()):
        if cat_node.target is smith.ops.aten.cat.default:
            cat_dim = get_arg_value(cat_node, 1, "dim")
            cat_inputs = get_arg_value(cat_node, 0, "tensors")
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.ops.aten.cat.default, users=MULTIPLE),
    pass_dict=construct_pattern_matcher_pass("normalization_aten_pass"),
)
def normalize_cat_default_aten(match: Match, *args, **kwargs):
    ...
    def is_empty_tensor(x: smith.fx.Node) -> bool:
        # special case where smith.ops.aten.cat.default supports cat'ing with an empty tensor
...
@register_graph_pattern(
    CallFunction(
        smith.ops.aten.cat,
        ListOf(CallFunctionVarArgs(smith.ops.aten.unsqueeze)),
        _users=MULTIPLE,
    ),
...
def merge_unbind_stack_aten(match: Match, *args, **kwargs):
    ...
    if not all(
        get_arg_value(unsqueeze_node, 0, "input").target is smith.ops.aten.select
    ...
    if not all(
        select_node.target is smith.ops.aten.select for select_node in select_nodes
    ):
        return
...
def update_args_from_split_getitem(
    ...
    parents_seen: list[smith.fx.Node],
    new_cat_args: list[smith.fx.Node],
    new_cat_args_meta: list[smith.fx.Node],
    idx_to_getitems: dict[int, smith.fx.Node],
...
def reshape_cat_node(
    graph: smith.fx.Graph,
    cat_node: smith.fx.Node,
    unbind_input: smith.fx.Node,
    cat_dim: int,
    unbind_dim: int,
    cat_shape: smith.Size,
) -> smith.fx.Node:
    if cat_dim != unbind_dim:
        ...
        with graph.inserting_after(cat_node):
            ...
            permute_node.meta["example_value"] = smith.permute(
    ...
    with graph.inserting_after(permute_node):
        reshape_node = graph.call_function(
            smith.reshape, args=(permute_node, tuple(cat_shape))
        )
        reshape_node.meta["example_value"] = smith.reshape(
...
def update_args_from_unbind_getitem(
    graph: smith.fx.Graph,
    node: smith.fx.Node,  # cat or stack node
    getitem_indices: list[int],
    parents_seen: list[smith.fx.Node],
    new_cat_args: list[smith.fx.Node],
    new_cat_args_meta: list[smith.fx.Node],
    ...
    if size == len(getitem_indices):
        ...
        with graph.inserting_after(node):
            ...
            slice_node.meta["example_value"] = smith.narrow(
...
def construct_cat_args(
    graph: smith.fx.Graph,
    cat_or_stack_node: smith.fx.Node,
    inputs: list[smith.fx.Node],
    split_or_unbind_node: smith.fx.Node,
    threshold_to_cat: int = 2,
    run_update_func: Callable = update_args_from_split_getitem,  # type: ignore[type-arg]
) -> tuple[list[smith.fx.Node], list[smith.Tensor]]:
...
def remove_split_unbind_children(graph: smith.fx.Graph, inputs: list[smith.fx.Node]):
...
def split_cat_to_slices(match: Match, split_sections: list[int], dim: int):
    if not isinstance(split_sections, (list, tuple)):  # Unnormalized split
        return
    split_nodes = [node for node in match.nodes if node.target is smith.split]
    if split_nodes:
        split_node = next(node for node in split_nodes)
    else:
        # Handle the case where there are no nodes with a target of smith.split
        return
    split_dim = get_arg_value(split_node, 2, "dim") or 0
    graph = match.graph
    threshold_to_cat = smith._inductor.config.pre_grad_fusion_options[
        "split_cat_to_slices_pass"
    ].get("threshold_to_cat", 10)
    # get the cat_node and check its inputs and meta data
    next_users = find_next_users(split_node)
    for cat_node in next_users:
        if cat_node.target != smith.cat or not is_node_meta_valid(cat_node):
            continue
...
def unbind_cat_to_view(match: Match, unbind_input: smith.fx.Node, dim: int):
    unbind_node = next(node for node in match.nodes if node.target is smith.unbind)
    graph = match.graph
    # get the cat_node and check its inputs and meta data
...
def convert_reshape_cat_arg_to_stack(
    ...
    reshape_node.meta["example_value"] = smith.Tensor.view(
...
def split_stack_to_cats(match: Match, split_sections: list[int], dim: int):
    ...
    for stack_node in next_users:
        if stack_node.target != smith.stack or not is_node_meta_valid(stack_node):
            continue
        ...
        if len(new_cat_args) > 1 and len(new_cat_args) < len(inputs):
            with graph.inserting_after(stack_node):
                ...
                cat_node.meta["example_value"] = smith.cat(  # type: ignore[arg-type]
...
def unbind_stack_to_slices(match: Match, unbind_input: smith.fx.Node, dim: int):
    unbind_node = next(node for node in match.nodes if node.target is smith.unbind)
    graph = match.graph
    # get the cat_node and check its inputs and meta data
    next_users = find_next_users(unbind_node)
...
def get_view_shape_list(cat_arg: smith.fx.Node, stack_dim: int) -> list[int]:
    # cat_arg must be the split input
    view_shape_list = []
    for user in cat_arg.users:
        if user.target is smith.split:
            for getitem in user.users:
                if getitem.target is operator.getitem:
                    reshape_user = [
                        user for user in getitem.users if user.target is smith.reshape
                    ]
...
def move_reshape_out_of_split_stack(match: Match, *args, **kwargs):
    ...
    stack_nodes = [node for node in match.nodes if node.target is smith.stack]
    graph = match.graph
    threshold_to_cat = smith._inductor.config.pre_grad_fusion_options[
        "move_reshape_out_of_split_stack_pass"
    ].get("threshold_to_cat", 10)
    for stack_node in stack_nodes:
        ...
        for stack_input in stack_inputs:
            if stack_input.target != smith.reshape:
                ...
                if cat_arg not in stack_inputs:
                    if len(stack_node_input) > 0:
                        with graph.inserting_after(stack_node):
                            ...
                            decomposed_stack_node.meta["example_value"] = smith.stack(
                    ...
                    stack_node_shape = smith.reshape(
...
view_getitem_split_aten = ListOf(
    CallFunction(
        ...
        CallFunction(
            operator.getitem,
            CallFunctionVarArgs(
                smith.ops.aten.split_with_sizes.default, users=MULTIPLE
            ),
            Ignored(),
            _users=MULTIPLE,
        ),
...
def move_view_after_cat(match: Match, *args, **kwargs):
    split_node = next(
        ...
        if node.target is smith.ops.aten.split_with_sizes.default
    )
    ...
    if not is_sorted_and_consecutive(getitem_indices):  # type: ignore[arg-type]
        return
    cat_nodes = [
        node for node in match.nodes if node.target is smith.ops.aten.cat.default
    ]
    graph = match.graph
    for cat_node in cat_nodes:
        ...
        if not all(
            view_node.target is smith.ops.aten.reshape.default
        ...
        if cat_dim != split_dim:
            ...
            permute_node = graph.call_function(
                smith.ops.aten.permute.default,
...
@register_graph_pattern(
    CallFunctionVarArgs(smith.functional.einsum, users=MULTIPLE),
