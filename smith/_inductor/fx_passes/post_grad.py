...
import smith
import smith._inductor as inductor
...
from smith._inductor import comms
from smith._inductor.virtualized import ops  # noqa: F401
from smith._logging import trace_structured
from smith._prims_common import is_boolean_dtype, is_expandable_to, is_integer_dtype
...
def _remove_profiler_ops(graph: smith.fx.Graph) -> None:
    """
...
def post_grad_passes(gm: smith.fx.GraphModule, is_inference: bool):
    """
    ...
    GraphTransformObserver = functools.partial(
        smith.fx.passes.graph_transform_observer.GraphTransformObserver,
        subsystem="post_grad_passes",
    )

    if not smith._dynamo.config.skip_fsdp_hooks:
    ...
    if config.bucket_reduce_scatters_fx != "none":
        from smith._inductor.fx_passes.bucketing import bucket_reduce_scatter
        from smith._inductor.fx_passes.fsdp import bucket_fsdp_reduce_scatter
    ...
    if config.bucket_all_reduces_fx != "none":
        from smith._inductor.fx_passes.bucketing import bucket_all_reduce
    ...
    if config.bucket_all_gathers_fx != "none":
        from smith._inductor.fx_passes.bucketing import bucket_all_gather
        from smith._inductor.fx_passes.fsdp import bucket_fsdp_all_gather
...
def prepare_softmax_replacement(x, dim):
    ...
    """
    from smith._inductor.inductor_prims import prepare_softmax_online
...
def decompose_map_to_while_loop(gm: smith.fx.GraphModule):
    """This is similar to decompose_scan_to_while_loop."""
    graph_pass = PatternMatcherPass()

    @register_graph_pattern(
        CallFunctionVarArgs(smith.ops.higher_order.map_impl),
    ...
    def _(match: Match, *args, **kwargs):
        ...
        sub_gm: smith.fx.GraphModule = getattr(gm, subgraph.target)
        ...
        def lower_to_while_loop(*args, **kwargs):
            ...
            def body_fn(*flat_args):
                ...
                smith.ops.aten._assert_scalar.default(idx_int >= 0, "")
                smith.ops.aten._assert_scalar.default(idx_int < map_length, "")
                sub_xs = [smith.ops.aten.select.int(x, 0, idx_int) for x in xs]
                outs = sub_gm(*sub_xs, *additional_inputs)

                for out, buffer in zip(outs, out_bufs):
                    buffer_slice = smith.ops.aten.select.int(buffer, 0, idx_int)
                    buffer_slice.copy_(out)
                return loop_idx + 1, *out_bufs, *xs

            _, final_out, _ = pytree.tree_unflatten(
                smith.ops.higher_order.while_loop(
                    cond_fn,
                    body_fn,
    ...
    for _node in gm.graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.map_impl
    ):
        raise AssertionError("map is not lowered to while_loop")


def resolve_shape_to_proxy(
    shape: list[int | smith.SymInt], bound_symbols: dict[Any, Any]
):
    """
    ...
    (arg0 * arg1, arg0 + arg1).
    """
    from smith.utils._sympy.interp import sympy_interp
    from smith.utils._sympy.reference import PythonReferenceAnalysis
...
def decompose_scan_to_while_loop(gm: smith.fx.GraphModule):
    """
    ...
    memory usage, supporting loops over unbacked shapes and cudagraph etc.
        ...
        def g():
            def step_fn(init: smith.Tensor, x: smith.Tensor):
                next_init = x + init
                return next_init, next_init

            init = smith.zeros([])
            xs = smith.arange(4)
            final_carry, ys = smith._higher_order.scan(step_fn, init, xs)
    ...
    This pass will rewrite scan into:
        ...
        def k():
            ...
            loop_idx = smith.zeros([])
            ys = smith.empty_strided(_shape_stride_of_ys)
            ...
            final_carry, _, _, ys = smith._higher_order.while_loop(cond_fn, body_fn, (loop_idx, ys, init, xs))
            return final_carry, ys
    """
    ...
    @register_graph_pattern(
        CallFunctionVarArgs(smith.ops.higher_order.scan),
        # pyrefly: ignore [bad-argument-type]
        pass_dict=graph_pass,
    )
    def _(match: Match, *args, **kwargs):
        ...
        sub_gm: smith.fx.GraphModule = getattr(gm, combine_subgraph.target)
        cur_node = match.nodes[0]
        ...
        def lower_to_while_loop(*args, **kwargs):
            ...
            # proxies in order to trace the smith.empty_strided call correctly.
            #
            ...
            def body_fn(*flat_args):
                ...
                idx_int = loop_idx.item()
                smith.ops.aten._assert_scalar.default(idx_int >= 0, "")
                smith.ops.aten._assert_scalar.default(idx_int < scan_length, "")
                sub_xs = [smith.ops.aten.select.int(x, 0, idx_int) for x in xs]
                ...
                for y, y_out in zip(ys, ys_outs):
                    y_out_slice = smith.ops.aten.select.int(y_out, 0, idx_int)
                    y_out_slice.copy_(y)
    ...
    for _node in gm.graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.scan
    ):
...
def lazy_init():
    ...
    # https://github.com/blacksmith/blacksmith/issues/148141
    register_replacement(
        # pyrefly: ignore [bad-argument-type]
...
def reorder_for_locality(graph: smith.fx.Graph):
    if smith.distributed.is_available():
        ...
        def check():
            ...
            return node.target != smith.ops._c10d_functional.wait_tensor.default
    else:
    ...
    seen_nodes = OrderedSet[smith.fx.Node]()
    ...
    first_copy = next(
        iter(graph.find_nodes(op="call_function", target=smith.ops.aten.copy_.default)),
        None,
    )
    ...
    for node in reversed(graph.nodes):
        ...
        smith.fx.map_arg((node.args, node.kwargs), visit)
...
def pointless_cumsum_replacement(match: Match, shape, fill_value, device, dtype, dim):
    ...
    def repl(*shape):
        dim_size = shape[dim]
        idx = smith.arange(1, dim_size + 1, device=device, dtype=dtype)
...
def cat_slice_cat(match, cat_input, size, dim=1):
    ...
    Matches:
        cat_1: f32[1024, 4077] = smith.ops.aten.cat.default([add_26, primals_217], 1)
        slice_1: f32[1024, 4077] = smith.ops.aten.slice.Tensor(cat_1, 0, 0, 9223372036854775807)
        slice_2: f32[1024, 19] = smith.ops.aten.slice.Tensor(slice_1, 1, 0, 19)
        cat_2: f32[1024, 4096] = smith.ops.aten.cat.default([cat_1, slice_2], 1)


    Rewrite to:
        slice_2 = smith.ops.aten.slice.Tensor(add_26, 1, 0, 19)
        cat_2 = smith.ops.aten.cat.default([add_26, primals_217, slice2], 1)
    """
...
def same_meta(node1: smith.fx.Node, node2: smith.fx.Node):
...
@register_noop_decomp(smith.ops.prims.convert_element_type)
def convert_element_type_noop(x, dtype: smith.dtype):
    return x.dtype == dtype


@register_noop_decomp(smith.ops.prims.device_put)
...
def remove_noop_ops(graph: smith.fx.Graph):
    """
    Removes both operations that are essentially aten.clone and operations that are essentially aten.alias from the graph.
    """
    inputs = OrderedSet[smith.fx.Node]()
    ...
    if not isinstance(outputs, (list, tuple)):
        ...
        outputs = (outputs,)
    for out in outputs:
        if isinstance(out, smith.fx.Node):
    ...
    for node in graph.nodes:
        if node.target in noop_registry:
            ...
            if not isinstance(src, smith.fx.Node):
                continue
...
def remove_assert_ops(graph: smith.fx.Graph):
    """
    ...
    %_assert_tensor_metadata = aten._assert_tensor_metadata.default(%addmm, None, None, smith.float16)
    %convert_element_type_3 = prims.convert_element_type.default(%addmm, smith.float32)
    ...
    for node in graph.find_nodes(
        op="call_function", target=smith.ops.aten._assert_tensor_metadata.default
    ):
        graph.erase_node(node)
...
def apply_pass_to_subgraphs(pass_fn: Callable[[fx.Graph], None], graph: fx.Graph):
    ...
    for child_name, child_mod in gm.named_children():
        if child_name in subgraph_names and isinstance(child_mod, smith.fx.GraphModule):
...
def decompose_triton_kernel_wrapper_functional(graph):
    ...
    @register_graph_pattern(
        CallFunctionVarArgs(smith.ops.higher_order.triton_kernel_wrapper_functional),
        # pyrefly: ignore [bad-argument-type]
        pass_dict=graph_pass,
    )
    def _(match: Match, *args, **kwargs):
    ...
    for _ in graph.find_nodes(
        op="call_function",
        target=smith.ops.higher_order.triton_kernel_wrapper_functional,
    ):
...
def decompose_auto_functionalized(graph):
    ...
    @register_graph_pattern(
        CallFunctionVarArgs(smith.ops.higher_order.auto_functionalized),
        # pyrefly: ignore [bad-argument-type]
        pass_dict=graph_pass,
    )
    def _(match: Match, *args, **kwargs):
        from smith._higher_order_ops.auto_functionalize import auto_functionalized_dense
    ...
    @register_graph_pattern(
        CallFunctionVarArgs(smith.ops.higher_order.auto_functionalized_v2),
        # pyrefly: ignore [bad-argument-type]
        pass_dict=graph_pass,
    )
    def _(match: Match, *args, **kwargs):
        ...
        def _maybe_resolve_constant_get_attr(node):
            ...
            if (
                ...
                assert isinstance(
                    const_attr, (smith.fx.GraphModule, pytree.TreeSpec)
    ...
    removable_attrs: OrderedSet[smith.fx.node.Target] = OrderedSet()
    protected_attrs: OrderedSet[smith.fx.node.Target] = OrderedSet()
    ...
    for node in graph.nodes:
        ...
        if len(node.users) == 0:
            ...
            if (
                ...
                and isinstance(
                    getattr(graph.owning_module, node.target), smith.fx.GraphModule
                )
    ...
    for _ in graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.auto_functionalized
    ):
        raise AssertionError("auto_functionalized was not removed")

    for _ in graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.auto_functionalized_v2
    ):
...
def view_to_reshape(gm):
    ...
    def _view_to_reshape_graph(graph):
        for nd in graph.find_nodes(
            op="call_function", target=smith.ops.aten.view.default
        ):
            nd.target = smith.ops.aten.reshape.default
...
def is_valid_addmm_fusion(match):
    ...
    if not (
        isinstance(inp, smith.fx.Node) and isinstance(inp.meta["val"], smith.Tensor)
    ):
...
def register_partial_reduction_pattern():
    ...
    for red_op in (aten.amax.default, aten.amin.default):
        ...
        def reuse_partial(match, input, reduced_dims, keepdim):
            ...
            def replacement(inp: smith.Tensor) -> tuple[smith.Tensor, smith.Tensor]:
...
def is_index_put_and_requires_h2d_sync_for_gpu_value(node):
    ...
    indices = args_[1]
    for i in indices:
        if i is not None and i.meta["val"].dtype in [smith.bool, smith.int8]:
...
class ConstructorMoverPass:
    ...
    def allow_cpu_device(self, node: fx.Node) -> bool:
        ...
        return node.target in (
            smith.ops.aten.index.Tensor,
            smith.ops.aten.index_put.default,
            smith.ops.aten.index_put_.default,
            smith.ops.aten.copy.default,
            smith.ops.aten.copy_.default,
            smith.ops.aten.slice_scatter.default,
        )
    ...
    def is_cpu_scalar_tensor(self, node: fx.Node) -> bool:
        ...
        is_scalar = isinstance(ten, smith.Tensor) and len(ten.size()) == 0
    ...
    def get_node_device(self, node: fx.Node) -> smith.device | None:
        """
        Get the device of a node.
        """
        ten = node.meta.get("val")
        return None if not isinstance(ten, smith.Tensor) else ten.device
    ...
    def __call__(self, graph: fx.Graph) -> None:
        target_devices = OrderedSet[smith.device]()
        constructors = []
        ...
        for node in graph.nodes:
            ...
            if not smith._subclasses.fake_tensor._is_tensor_constructor(node.target):
                continue

            if node.kwargs.get("device") != smith.device("cpu"):
                continue
            ...
            for idx, node in enumerate(movable_cpu_placeholders):
                with graph.inserting_after(last_node):
                    ...
                    node.replace_all_uses_with(
                        ...
                        + unsqueezed_nodes
                        and x.target != smith.ops.aten.copy_.default,
                    )
                    last_node = gpu_node
                ...
                noop_device_puts = [
                    ...
                    if user.target is smith.ops.prims.device_put.default
