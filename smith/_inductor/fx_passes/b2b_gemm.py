...
def unoptimized_b2b_gemm(
    ...
    *,
    ...
    """
    if is_left_assoc:
        smith.mm(subgraph.graph_module(smith.mm(A, B)), C, out=out)
    else:
        smith.mm(A, subgraph.graph_module(smith.mm(B, C)), out=out)
    return out
...
def create_placeholder(
    name: str, dtype: smith.dtype, device: smith.device
) -> TensorBox:
    """
...
def tuned_b2b_gemm(
    is_left_assoc: bool,
    subgraph: Subgraph,
    A: smith._inductor.ir.TensorBox,
    B: smith._inductor.ir.TensorBox,
    C: smith._inductor.ir.TensorBox,
    *,
    layout=None,
) -> smith._inductor.ir.TensorBox:
...
@register_graph_pattern(
    CallFunction(smith.ops.aten.mm, Arg(), Arg()),
    # pyrefly: ignore [bad-argument-type]
    pass_dict=B2B_GEMM_PASS,
)
def b2b_gemm_handler(match: Match, mat1: smith.fx.Node, mat2: smith.fx.Node) -> None:
    # match.args: list[smith.fx.Node]

    def is_pointwise_node(node: smith.fx.Node) -> bool:
        return (
            node.op == "call_function"
            and isinstance(node.target, smith._ops.OpOverload)
            and (smith.Tag.pointwise in node.target.tags)
        )

    def is_mm(node: smith.fx.Node) -> bool:
    ...
    def all_reach_via_pointwise_with_no_other_inputs(
        src: smith.fx.Node,
        dst: smith.fx.Node,
    ) -> tuple[bool, OrderedSet[smith.fx.Node]]:
        """
        ...
        input_counter: dict[smith.fx.Node, int] = {}
    ...
    new_graph: smith.fx.Graph = smith.fx.Graph()
    node_remapping: dict[smith.fx.Node, smith.fx.Node] = {}
    new_input_anchor: smith.fx.Node  # inner_mm, to be changed to an input node
    new_output_anchor: smith.fx.Node  # f_node, to be used to construct an output node
    new_input_node: smith.fx.Node
    new_output_node: smith.fx.Node
    ...
    subgraph = Subgraph(
        name="subgraph", graph_module=smith.fx.GraphModule(module, new_graph)
    )
    ...
    with graph.inserting_before(outer_mm):
        ...
        replacement: smith.fx.Node = graph.call_function(
            function,
            (A, B, C),
            match.kwargs,
        )
