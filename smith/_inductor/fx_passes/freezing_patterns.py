...
import smith
from smith._inductor.compile_fx import fake_tensor_prop
...
def freezing_passes(gm: smith.fx.GraphModule, aot_example_inputs):
    """
    ...
    smith._inductor.fx_passes.binary_folding.mark_mixed_dtype_allowed_computation_ops(
        gm
    )
    for _ in range(4):
        constant_fold(gm)
    ...
    smith._inductor.fx_passes.binary_folding.recover_original_precision_folded_computation_ops(
        gm
    )
...
@init_once_fakemode
def lazy_init():
    if smith._C._has_mkldnn and config.cpp.weight_prepack:
...
def addmm_patterns_init():
    ...
    """
    device = next(
        (gpu for gpu in GPU_TYPES if getattr(smith, gpu).is_available()), "cpu"
    )
    val = functools.partial(smith.empty, (10, 10), device=device, requires_grad=False)
    scale = functools.partial(smith.empty, (10,), device=device, requires_grad=False)
    ...
    def check_int8_woq_concat_linear_weights(match):
        ...
        if not all(
            match.kwargs[wgt].target is smith.ops.prims.convert_element_type.default
        ...
        if not all(
            match.kwargs[wgt].meta["val"].dtype is smith.bfloat16
            for wgt in weight_inputs
        ):
            return False
    ...
    def int8_woq_fusion_replacement(inp, w1, w2, w3, s1, s2, s3):
        cat_w = smith.cat((w1, w2, w3), dim=1)
        cat_s = smith.cat((s1, s2, s3), dim=0)
    ...
    def matmul_replacement(inp, w1, w2, w3):
        cat_t = smith.cat((w1, w2, w3), dim=1)
        mm = inp @ cat_t
