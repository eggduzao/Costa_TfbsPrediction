...
from smith.utils._runtime_estimation import get_num_bytes
...
class FusionRegion:
    ...
    def _compute_cost(self) -> tuple[int, float]:
        ...
        output_vals = [
            n.meta.get("val")
            for n in smith._inductor.utils.output_node(subgraph).all_input_nodes
        ]
...
def is_view_node(n: fx.Node) -> bool:
    ...
    return isinstance(n.target, smith._ops.OpOverload) and (
...
def is_fusible_node(n: fx.Node) -> bool:
    ...
    - It has a lowering in smith._inductor.lowering.lowerings
    ...
    from smith._inductor.lowering import fallbacks, lowerings
    ...
    if target == smith.ops.aten.cat.default:
...
def _find_connected_components(span: list[fx.Node]) -> list[list[fx.Node]]:
    ...
    from smith.fx.experimental.optimization import UnionFind
...
def expand_fusion_regions(
    ...
    """
    from smith.fx.experimental.const_fold import _inline_module
    ...
    for module_node, region in list(region_of.items()):
        ...
        output_arg = smith._inductor.utils.output_node(region.subgraph_module).args[0]
