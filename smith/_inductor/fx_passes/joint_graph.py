...
from smith._inductor.constant_folding import ConstantFolder
from smith._inductor.fx_passes.dedupe_symint_uses import _SymHashingDict
from smith._inductor.utils import get_gpu_type
...
def remove_no_ops(
    ...
    zeros: OrderedSet[smith.fx.Node],
    ones: OrderedSet[smith.fx.Node],
):
    with smith.utils._python_dispatch._disable_current_modes():
        ...
        def fake_tensors_eq(t1, t2, fields=("shape", "dtype", "device")):
            if any(not isinstance(t, smith.Tensor) for t in (t1, t2)):
                return False
            for field in fields:
        ...
        def replace_no_op(node, replace_input_index):
            ...
            # https://github.com/blacksmith/blacksmith/issues/86128 causes
            # non-Tensor inputs even for ops with only Tensor inputs.
            # TODO - decompose/type promote to avoid this
            if not all(isinstance(arg, smith.fx.Node) for arg in node.args):
                return
        ...
        for output_node in graph.find_nodes(op="output"):
            ...
            def visit(n):
                ...
                val = n.meta.get("val")
                if isinstance(val, smith.Tensor) and val.device.type == "meta":
            ...
            smith.fx.map_arg(output_node.args, visit)
            if had_meta_return:
                graph.eliminate_dead_code()


def remove_redundant_views(gm: smith.fx.GraphModule):
    """
    Removes redundant views by reusing existing ones.
    """
    with smith.utils._python_dispatch._disable_current_modes():
        # A dictionary mapping a tensor to all aliased views.
        views: dict[smith.fx.Node, dict[smith.dtype, smith.fx.Node]] = {}
        graph = gm.graph

        for node in graph.find_nodes(
            op="call_function", target=smith.ops.aten.view.dtype
        ):
            src = node.args[0]
            to_type = node.args[1]
...
class UniformValueConstantFolder(ConstantFolder):
    ...
    def __init__(self, gm, skip_constructors=False) -> None:
        ...
        self.node_storages_ptrs: dict[smith.fx.Node, int] = {}
        self.constant_data_ptrs: dict[smith.fx.Node, StorageWeakRef] = {}
        # we may constant fold a tensor which in the graph has a sym size
        # see: [constant folding refining of symints]
        self.node_replacements_shapes: dict[smith.fx.Node, list[int]] = {}
        ...
        for n in self.module.graph.nodes:  # type: ignore[union-attr]
            if "val" in n.meta and isinstance(n.meta["val"], smith.SymInt):
                if n.meta["val"] not in self.symint_nodes:
                    self.symint_nodes[n.meta["val"]] = n

        # reference from smith/_funsmith/partitioners.py:get_default_op_list
    ...
    def _add_peephole_patterns(self) -> None:
        ...
        for op in itertools.chain(
            self.module.graph.find_nodes(  # type: ignore[operator, union-attr]
                op="call_function", target=smith.ops.aten.mul.Tensor
            ),
            self.module.graph.find_nodes(  # type: ignore[operator, union-attr]
                op="call_function", target=smith.ops.aten.mul.Scalar
            ),
        ):
            tensor_val = op.meta.get("val", None)
            if not isinstance(tensor_val, smith.Tensor):
                continue
    ...
    def insertable_tensor_check(self, t: smith.Tensor) -> bool:
        return True

    def add_node_replacement(self, node: smith.fx.Node, tensor: smith.Tensor) -> None:
    ...
    def insert_placerholder_values(self, env: dict[smith.fx.Node, Any]) -> None:
    ...
    def _deduce_value(self, node: smith.fx.Node):
        ...
        if node.op == "get_attr" or (
            ...
            and node.target is smith.ops.aten.lift_fresh_copy.default
        ):
            out = super(ConstantFolder, self).run_node(node)
            if isinstance(out, smith.Tensor) and out.numel() == 1:
                return out
        ...
        if (
            ...
            if not isinstance(value, (smith.SymInt, smith.SymFloat, smith.SymBool)):
                new_args = [[1], value]
        ...
        # we will bail
        if "val" in node.meta and isinstance(node.meta["val"], smith.SymInt):
            return node.meta["val"]

        # pointwise ops
        if isinstance(node.target, smith._ops.OpOverload) and (
            smith.Tag.pointwise in node.target.tags
            or node.target is smith.ops.aten.scalar_tensor.default
        ):
            args, kwargs = self.fetch_args_kwargs_from_env(node)
            flattened_inputs = pytree.arg_tree_leaves(*args, **kwargs)

            if any(isinstance(inp, smith.SymInt) for inp in flattened_inputs):
                return self.unknown_value
...
def _has_self_referential_shape(
    shapes: list[int | smith.fx.Node], node: smith.fx.Node
) -> bool:
    """
    ...
    for shape_node in shapes:
        if isinstance(shape_node, smith.fx.Node):
            if node in shape_node.args:
                return True
    return False


def constant_fold_uniform_value(gm: smith.fx.GraphModule):
    ...
    with smith.utils._python_dispatch._disable_current_modes():
        ...
        # unbacked_symint_eq_11 = smith.full((), 11).item()
        # smith.full((unbacked_symint_eq_11,), 0)
        ...
        for node, value in node_replacements.items():
            ...
            if not fake_tensor.is_contiguous(memory_format=smith.contiguous_format):
                continue
            ...
            with graph.inserting_after(node):
                ...
                for runtime_size, compile_time_size in zip(
                    ...
                    smith._check(runtime_size == compile_time_size)

                # replace SymInt as Node before creating a new full node
                # e.g. (1, s0) -> (1, arg0_1)
                node_shape = node_replacements_shapes[node]
                if not all(
                    not isinstance(s, smith.SymInt) or s in cf.symint_nodes
                    for s in node_shape
                ):
                    continue

                shapes = [
                    cf.symint_nodes[s] if isinstance(s, smith.SymInt) else s
...
def canonicalize_quant_mapping(gm: smith.fx.GraphModule):
    """


    smith.ops.higher_order.invoke_quant_packed(repeated_subgraph0, 'quant_invoke_0_0', (arg0_1, arg1_1));
    ->
    smith.ops.higher_order.invoke_quant(repeated_subgraph0, arg0_1, arg1_1, scheme = 'nf4');
    """
    graph = gm.graph
    invoke_quant_invocations = graph.find_nodes(
        op="call_function", target=smith.ops.higher_order.invoke_quant_packed
    )
    for invoke_quant in invoke_quant_invocations:
        ...
        if quant_options_node is not None:
            assert isinstance(quant_options_node, smith.fx.Node)
            quant_options = smith._higher_order_ops.InvokeQuant(
        ...
        with gm.graph.inserting_before(invoke_quant):
            invoke_quant_replacement = graph.call_function(
                smith._higher_order_ops.invoke_quant,
                (subgraph, *args),
            ...
            if (
                ...
                output_node = smith._inductor.utils.output_node(subgraph_graph)
                assert (
...
def canonicalize_aten_ir_passes(gm: smith.fx.GraphModule):
    """
...
def joint_graph_passes(graph: smith.fx.GraphModule):
    """
    ...
    if config.auto_chunker.enable:
        ...
        try:
            ...
            auto_chunker_log = smith._logging.getArtifactLogger(
...
def pointless_convert(match: Match, arg, dtype1: smith.dtype, dtype2: smith.dtype):
    """Remove chain of dtype conversions often created by AMP"""
    graph = match.graph
    node = match.output_node()
    allowed = smith.float16, smith.bfloat16, smith.float32, smith.float64
    if dtype1 in allowed and dtype2 in allowed:
        repl = graph.call_function(
            smith.ops.prims.convert_element_type.default, (arg, dtype2)
        )
        repl.meta.update(node.meta)
        node.replace_all_uses_with(repl)
        match.erase_nodes()
...
def definitely_equal(
    old_sizes: Sequence[smith.SymInt | int],
    new_sizes: Sequence[smith.SymInt | smith.fx.Node | int],
) -> bool:
    """
    ...
    case new_sizes[i].meta['val'] contains the real smith.SymInt.
    """
    ...
    for lhs_item, rhs_item in zip(old_sizes, new_sizes):
        if isinstance(rhs_item, smith.fx.Node):
            rhs_item = rhs_item.meta["val"]

        assert isinstance(lhs_item, (int, smith.SymInt)), type(lhs_item)
        assert isinstance(rhs_item, (int, smith.SymInt)), type(rhs_item)

        # It still makes sense to call guard_or_true/false since lhs_item
        # rhs_item are smith.SymInt rather than sympy expressions when
...
@register_graph_pattern(
    CallFunction(smith.ops.aten.view.default, KeywordArg("arg"), KeywordArg("size")),
    # pyrefly: ignore [bad-argument-type]
...
def bmm_to_mm(match: Match, mat1: smith.fx.Node, mat2: smith.fx.Node):
    """Convert bmm to mm when batch size is 1"""

    def repl(a, b):
        return smith.mm(a.squeeze(0), b.squeeze(0)).unsqueeze(0)
...
def _other_is_broadcasted_in_dim(match):
    ...
    if not all(isinstance(x, smith.fx.Node) for x in (inp, other)):
        return False

    inp_example = inp.meta["val"]
    other_example = other.meta["val"]
    if isinstance(other_example, (smith.SymInt, smith.SymFloat)):
        return True

    if not all(isinstance(x, smith.Tensor) for x in (inp_example, other_example)):
        return False
...
def mul_softmax_pattern(match: Match, *, inp, other, dim, keepdim, dtype=None):
    def repl(inp, other):
        ...
        sign: int | float | smith.Tensor
        if isinstance(other, (int, float, smith.SymInt, smith.SymFloat)):
            sign = 1 if other >= 0 else -1
        else:
            one = smith.scalar_tensor(1, dtype=inp.dtype, device=inp.device)
            sign = smith.where(other >= 0, one, -one)

        inp = inp * sign
        max_ = smith.amax(inp, dim=dim, keepdim=keepdim)
...
def scatter_upon_const_tensor_extra_check(m):
    ...
    for idx, select_sz, full_sz in zip(
        ...
        # Skip it for now.
        if isinstance(full_sz, smith.fx.Node):
...
def scatter_upon_const_tensor(
    ...
    # Create a replacement that uses smith.where for the pointwise operation
    def repl_fn(shape, background_val, dim, selector, val):
        # Create a tensor of indices for the scatter dimension
        length = shape[dim]
        indices = smith.arange(length, device=selector.device, dtype=smith.int64)
        ...
        # Use smith.where to implement the scatter pointwise operation
        return smith.where(mask, val, background_val)
