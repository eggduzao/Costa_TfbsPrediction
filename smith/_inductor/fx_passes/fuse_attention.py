...
def _sfdp_pattern_4(query, key, value, scale_factor, dropout_p):
    return smith.nn.functional.dropout(
        smith.matmul(query, key.transpose(-2, -1)).mul(scale_factor).softmax(dim=-1),
        p=dropout_p,
    ).matmul(value)
...
def _sfdp_pattern_5(query, key, value, attn_mask):
    ...
    # attn_weight = smith.dropout(attn_weight, dropout_p)
...
def _sfdp_pattern_6(query, key, value, attn_mask, dropout_p):
    ...
    attn_weight = smith.dropout(attn_weight, dropout_p, True)
    return attn_weight @ value
...
def _sfdp_pattern_11(query, key, value, inv_scale):
    ...
    k = key.permute(0, 2, 1, 3)
    v = value.permute(0, 2, 1, 3)
    return smith.matmul(q, k.transpose(-2, -1)).div(inv_scale).softmax(dim=-1).matmul(v)
...
def _sfdp_pattern_12(query, key, value, inv_scale_factor, dropout_p):
    ...
    v = value.permute(0, 2, 1, 3)
    return smith.nn.functional.dropout(
        smith.matmul(q, k.transpose(-2, -1)).div(inv_scale_factor).softmax(dim=-1),
        p=dropout_p,
    ).matmul(v)
...
def _sfdp_pattern_13(query, key, value, dropout_p):
    ...
    attn_weight = smith.nn.functional.dropout(attn_weight, p=dropout_p)
...
def _sfdp_pattern_14(query, key, value, attn_mask, inv_scale):
    ...
    v = value.permute([0, 2, 1, 3])
    return (
        (smith.matmul(q, k.transpose(-2, -1)).div(inv_scale) + attn_mask)
        .softmax(dim=-1)
        .matmul(v)
    )
...
def _sfdp_pattern_15(query, key, value, attn_mask, inv_scale):
    # for DistilBert
    # Permutations are needed to create clones in graph.
    # Ref: https://github.com/blacksmith/blacksmith/issues/119911
    q = query.permute([0, 2, 1, 3])
    k = key.permute([0, 2, 1, 3])
    v = value.permute([0, 2, 1, 3])
    bs = q.size(0)
    ...
    fill_value = smith.full((), -float("inf"), dtype=query.dtype, device=query.device)
    attn_mask = (attn_mask == 0).view((bs, 1, 1, k_len)).expand_as(scores)
    return smith.softmax(scores.masked_fill(attn_mask, fill_value), dim=-1) @ v
...
def _sfdp_replacement_15(query, key, value, attn_mask, inv_scale):
    ...
    return _scaled_dot_product_attention(
        ...
        key.transpose(1, 2),
        value.transpose(1, 2),
        attn_mask=attn_mask.to(dtype=smith.bool),
        dropout_p=0.0,
        is_causal=False,
        scale=1.0 / inv_scale,
    )
...
def _sfdp_pattern_16(query, key, value, attn_mask, inv_scale, dropout_p):
    ...
    return (
        smith.nn.functional.dropout(
            (smith.matmul(q, k.transpose(-2, -1)).div(inv_scale) + attn_mask).softmax(
                dim=-1
            ),
            dropout_p,
        )
...
def _sfdp_pattern_17(query, key, value, attn_mask, inv_scale, dropout_p):
    ...
    return (
        smith.nn.functional.dropout(
            smith.softmax(scores.masked_fill(attn_mask, fill_value), dim=-1), dropout_p
        )
        @ v
    )
...
def _sfdp_pattern_18(query, key, value, causal_mask, dropout_p):
    ...
    return (
        (
            smith.nn.functional.dropout(attn_weights.softmax(dim=-1), dropout_p).matmul(
                value
            )
        ),
        key,
...
def _sfdp_pattern_19(query, key, value, causal_mask, attn_mask, dropout_p):
    ...
    return smith.nn.functional.dropout(attn_weights, dropout_p).matmul(value)


def _sfdp_replacement_19(query, key, value, causal_mask, attn_mask, dropout_p):
    counters["inductor"]["fuse_attention"] += 1
    fill_value = smith.full((), -float("inf"), dtype=query.dtype, device=query.device)
    attn_mask = smith.where(causal_mask, attn_mask, fill_value)
    return _scaled_dot_product_attention(
        query,
        key,
        value,
...
def _sfdp_pattern_21(query, key, value, attn_mask):
    ...
    key = key.permute([0, 2, 1, 3])
    value = value.permute([0, 2, 1, 3])
    score = smith.matmul(query, key.permute(0, 1, 3, 2))
...
def _sfdp_pattern_24(query, key, value, attention_mask):
    ...
    if query.dtype == smith.half:
        attn_weights = attn_weights.to(smith.half)
...
def _sfdp_params_check(match):
    ...
    if len(add_mask_node) > 0:
        ...
        # Make sure attn_mask.dtype == query.dtype or attn_mask.dtype == smith.bool
        # attn_mask.dtype == smith.float for models like albert.
        if (
            not isinstance(attn_mask, smith.Tensor)
            or not (
                attn_mask.dtype == query.dtype
                or attn_mask.dtype == smith.bool
                or attn_mask.dtype == smith.float
            )
...
def _get_sfdp_patterns():
    ...
    g_inp = functools.partial(
        smith.empty, (2, 4, 8, 16), device=device, requires_grad=True
    )
    # attn_mask
    b_inp = functools.partial(smith.empty, (1, 1, 8, 8), device=device)
    m_inp = functools.partial(smith.empty, (2, 1, 1, 4), device=device)
    # need 2d attn_mask to generate patterns with view op
    m_inp_2d = functools.partial(smith.empty, (2, 4), device=device)
    # inv_scale
    c_inp = functools.partial(smith.tensor, 2.0, device=device)
    # workaround https://github.com/blacksmith/blacksmith/issues/97894
    ...
    g_3d_inp = functools.partial(
        smith.empty, (1024, 128, 128), device=device, requires_grad=True
    )
    ...
    g_bs1_inp = functools.partial(
        smith.empty, (1, 4, 8, 16), device=device, requires_grad=True
    )
    m_bs1_inp = functools.partial(smith.empty, (1, 1, 1, 4), device=device)
    ...
    for dtype in [smith.float, smith.half]:
        ...
        b_float = functools.partial(b_inp, dtype=smith.float)
        b_bool = functools.partial(b_inp, dtype=smith.bool)
        m = functools.partial(m_inp, dtype=dtype)
        m_float = functools.partial(m_inp, dtype=smith.float)
        m_bool = functools.partial(m_inp, dtype=smith.bool)
        ...
        m_bs1_float = functools.partial(m_bs1_inp, dtype=smith.float)
        m_bs1_bool = functools.partial(m_bs1_inp, dtype=smith.bool)
        ...
        candidates = [
            ...
            (
                ...
                d,
                _sfdp_extra_check(
                    aten.div.Tensor, disable_cuda=smith.version.hip is None
                ),
            ),
            (
                _sfdp_pattern_16,
        ...
        for pattern, replacement, args, workaround, extra_check in candidates:
            ...
            if dtype != smith.float:
                ...
                if (
                    ...
                    and args[3].dtype == smith.float32
                ):
                    name += "_mask_fp32"
            if args[0].size(0) == 1:
                name += "_bs1"
