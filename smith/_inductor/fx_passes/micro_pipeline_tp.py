...
def _is_last_dim(t: smith.Tensor, dim: int) -> bool:
    return dim == t.ndim - 1 or dim == -1


def _is_backward(graph: smith.fx.Graph) -> bool:
    placeholders = []
...
def _filter_nodes_by_target(nodes: list[smith.fx.Node], target) -> list[smith.fx.Node]:
    return [x for x in nodes if x.target == target]


def _find_ancestors(node: smith.fx.Node) -> OrderedSet[smith.fx.Node]:
    ancestors = OrderedSet[smith.fx.Node]()
    ancestors.add(node)
    cur_nodes = [node]
...
def _get_tensor(node: smith.fx.Node) -> smith.Tensor:
    val = node.meta["val"]
...
class _AllGatherMatch:
    match: Match
    shard_node: smith.fx.Node
    ag_node: smith.fx.Node
    res_node: smith.fx.Node
    gather_dim: int
    group_name: "smith.distributed.distributed_c10d.GroupName"

    def replace_with(self, new_node: smith.fx.Node) -> None:
...
def find_all_gather_patterns(graph: smith.fx.Graph):
    c10d = smith.ops._c10d_functional
    ...
    all_gathers = []
    visited_ag_nodes = OrderedSet[smith.fx.Node]()
...
class _ReduceScatterMatch:
    match: Match
    input_node: smith.fx.Node
    reduce_scatter_node: smith.fx.Node
    wait_tensor_node: smith.fx.Node
    reduce_op: str
    scatter_dim: int
    group_name: "smith.distributed.distributed_c10d.GroupName"
    ...
    def _update_save_for_backward(self, new_node: smith.fx.Node) -> None:
        """
...
def find_reduce_scatter_patterns(graph: smith.fx.Graph):
    c10d = smith.ops._c10d_functional
...
class _Matmul:
    ...
    arg_ancestor_nodes: OrderedSet[smith.fx.Node] = field(init=False)
    A_node: smith.fx.Node
    B_node: smith.fx.Node
    pre_mm_reshape: smith.fx.Node | None
    post_mm_reshape: smith.fx.Node | None
    ...
    @classmethod
    def from_match(cls, match: list[smith.fx.Node]) -> "_Matmul":
        ...
        return _Matmul(
            nodes=match,
            A_node=cast("smith.fx.Node", match[0].args[0]),
            B_node=cast("smith.fx.Node", mm_node.args[1]),
...
class _ScaledMatmul(_Matmul):
    A_scale_node: smith.fx.Node
    B_scale_node: smith.fx.Node
    bias_node: smith.fx.Node | None
    result_scale_node: smith.fx.Node | None
    ...
    @classmethod
    def from_match(cls, match: list[smith.fx.Node]) -> "_ScaledMatmul":
        assert len(match) in (1, 3)
        assert match[0].target in (
        ...
        def get_arg(node: smith.fx.Node, idx: int, default: Any) -> Any:
        ...
        A_node = cast("smith.fx.Node", mm_node.args[0])
        B_node = cast("smith.fx.Node", mm_node.args[1])
        A_scale_node = cast("smith.fx.Node", mm_node.args[2])
        B_scale_node = cast("smith.fx.Node", mm_node.args[3])
...
def _find_reshape_mm_reshape(node: smith.fx.Node) -> list[_Matmul]:
    ...
    for mm_node in node.users:
        ...
        for reshape_node in mm_node.users:
            ...
            matmul_input_node = cast("smith.fx.Node", node.args[0])
            B_node = cast("smith.fx.Node", mm_node.args[1])
            matmul_out_shape = smith.Size(
                [
...
def _find_consumer_matmuls(node: smith.fx.Node) -> list[_Matmul]:
    """
...
def _insert_fused_all_gather_matmul(
    ...
    if mm_type == _Matmul:
        ...
        return graph.call_function(
            smith.ops.symm_mem.fused_all_gather_matmul.default,
        ...
        return graph.call_function(
            smith.ops.symm_mem.fused_all_gather_scaled_matmul.default,
            args=(
                shard_node,
...
def fuse_all_gather_matmul(all_gather: _AllGatherMatch) -> None:
    ...
    Fused the pattern
        ...
        C_0 = smith.matmul(A, B_0)
        C_1 = smith.matmul(A, B_1)
        C_2 = smith.matmul(A, B_2)
        ...

    into

        A, Cs = smith.ops.symm_mem.fused_all_gather_matmul(
            A_shard, [B_0, B_1, B_2, ...], gather_dim, group_name,
        )
    """
    if (
        not smith.distributed.is_available()
        or not smith.distributed.is_nccl_available()
    ):
        return
...
def _scatter_dim_after_reshape(
    reshape_node: smith.fx.Node, orig_scatter_dim: int
) -> int:
    """
    ...
    input_tensor_node = cast(smith.fx.Node, reshape_node.args[0])
...
def _find_producer_matmul(node: smith.fx.Node) -> _Matmul | None:
    """
    ...
    if node.target is aten.mm.default:
        ...
        assert isinstance(mm_node, smith.fx.Node)
        if mm_node.target not in (aten.mm.default, aten._scaled_mm.default):
            return None

        reshape_node_0 = mm_node.args[0]
        assert isinstance(reshape_node_0, smith.fx.Node)
...
def _insert_fused_matmul_reduce_scatter(
    ...
    if type(matmul) is _Matmul:
        return graph.call_function(
            smith.ops.symm_mem.fused_matmul_reduce_scatter.default,
            args=(
                matmul.A_node,
                matmul.B_node,
                reduce_op,
        ...
        return graph.call_function(
            smith.ops.symm_mem.fused_scaled_matmul_reduce_scatter.default,
            args=(
                matmul.A_node,
                matmul.B_node,
                matmul.A_scale_node,
...
def fuse_matmul_reduce_scatter(reduce_scatter: _ReduceScatterMatch) -> None:
    ...
    into
        ...
        smith.ops.symm_mem.fused_matmul_reduce_scatter(
...
def _get_node_to_ancestors(
    ...
    """
    node_to_ancestors = defaultdict(OrderedSet[smith.fx.Node])  # type: ignore[var-annotated]
    for node in graph.nodes:
...
def _get_collective_to_overlappable_nodes(
    graph: smith.fx.Graph,
) -> dict[smith.fx.Node, list[smith.fx.Node]]:
    """
...
def _get_unexposed_collectives(graph: smith.fx.Graph) -> list[smith.fx.Node]:
    """
    ...
    def _is_compute_intensive(node: smith.fx.Node) -> bool:
        return node.target is smith.ops.aten.mm.default

    collective_to_overlapping_candidates = defaultdict(list)
    available_nodes = OrderedSet[smith.fx.Node]()
...
def micro_pipeline_tp_pass(graph: smith.fx.Graph):
