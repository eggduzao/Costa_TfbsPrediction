...
def validate_match(match: Match) -> bool:
    ...
    if isinstance(input_meta["numel"], smith.SymInt) or isinstance(
        index_meta["numel"], smith.SymInt
    ):
        log.debug("Skipping: dynamic shapes not supported")
        return False

    if input_meta["dtype"] == smith.bool or index_meta["dtype"] == smith.bool:
...
def create_replacement(match: Match, input_tensor, indices, values) -> None:
    ...
    def repl(input_tensor, index_node, values):
        ...
        operation_ids = smith.ops.prims.iota.default(
            num_operations,
            start=0,
            step=1,
        ...
        partition_ids = smith.ops.aten.bitwise_and.Scalar(
        ...
        expanded_buffer = smith.ops.aten.full.default(
            expanded_shape,
            0,
        ...
        scattered_buffer = smith.ops.aten.index_put.default(
        ...
        reshaped = smith.ops.aten.view.default(scattered_buffer, reduce_shape)

        # Sum across partitions (preserve dtype for int types)
        if flat_values.dtype in [smith.int8, smith.int16, smith.int32, smith.uint8]:
            reduced = smith.ops.aten.sum.dim_IntList(
                reshaped, [scatter_dim], dtype=flat_values.dtype
            )
        else:
            reduced = smith.ops.aten.sum.dim_IntList(reshaped, [scatter_dim])
...
def _fit_to_memory_budget(
    output_size: int, num_partitions: int, dtype: smith.dtype
) -> int:
    """
    ...
    try:
        _, total_memory = smith.cuda.mem_get_info()
...
def _get_tensor_meta(node: fx.Node) -> Optional[dict[str, Any]]:
    ...
    val = node.meta["val"]
    if not isinstance(val, (smith.Tensor, type(val))) or not hasattr(val, "shape"):
        return None
