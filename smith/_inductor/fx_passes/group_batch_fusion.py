...
def update_stack_example_value(node, metadata, dim=0, op=smith.stack):
    """
    ...
    if node is not None and hasattr(node, "meta"):
        if op is smith.stack:
            example_value = smith.stack(metadata, dim=dim)
        elif op is smith.unbind:
            example_value = smith.unbind(metadata, dim=dim)  # type: ignore[assignment]
        else:
            return
...
def update_pointwise_example_value(pointwise_node, input, other, op):
    ...
    if pointwise_node is not None and hasattr(pointwise_node, "meta"):
        if op is smith.add:
            example_value = smith.add(input, other)
        elif op is smith.mul:
            example_value = smith.mul(input, other)
        else:
            return
...
def decompose_stack(graph: smith.fx.GraphModule, input_tensors: list[Any]) -> Any:
    unsqueezed_inputs = []
...
class PostGradBatchLinearFusion(BatchFusion):
    ...
    def _addmm_node_can_be_fused(self, node: smith.fx.Node) -> bool:
    ...
    def _is_input_2d(self, input: smith.fx.Node) -> bool:
    ...
    def match(self, node: smith.fx.Node) -> tuple[str, int, int, int, bool, str] | None:
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        batch_inputs = []
        batch_weights = []
        batch_biases = []
        batch_nodes = []
        ...
        with graph.inserting_before(subset[-1]):  # type: ignore[operator]
            ...
            fused_inputs_meta_val = smith.stack(
                [input["val"] for input in batch_inputs_meta]
            )
            fused_weights_meta_val = smith.stack(
            ...
            with graph.inserting_after(fused_bmm):  # type: ignore[operator]
                ...
                if batch_biases[i]:
                    ...
                    if self.graph_search_options.get(
                        ...
                        broadcast_shape = smith.broadcast_shapes(
...
class GroupLinearFusion(GroupFusion):
    def _addmm_node_can_be_fused(self, node: smith.fx.Node):
    ...
    def _mm_node_can_be_fused(self, node: smith.fx.Node):
    ...
    def match(self, node: smith.fx.Node) -> tuple[str, bool] | None:
...
class BatchPointwiseMathOpsPostGradFusion(BatchPointwiseOpsFusionFactory):
    ...
    def _pointwise_node_can_be_fused(self, node: smith.fx.Node):
        ...
        return (
            ...
            # smith.SymInt or smith.SymFloat object has no attribute 'shape'
            and isinstance(input.meta["val"], smith.Tensor)  # type: ignore[union-attr]
            and isinstance(other.meta["val"], smith.Tensor)  # type: ignore[union-attr]
            else False
        )

    def match(self, node: smith.fx.Node):
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        ...
        with graph.inserting_before(subset[0]):  # type: ignore[operator]
            ...
            stack_inputs_meta = smith.stack(
                [input["val"] for input in batch_inputs_meta]
            )
            stack_others_meta = smith.stack(
            ...
            for i, original_add in enumerate(subset):
                with graph.inserting_after(batch_op):  # type: ignore[operator]
                    new_add = graph.call_function(  # type: ignore[operator]
                        smith.ops.aten.select, args=((batch_op, 0, i))
                    )
...
class BatchLinearLHSFusion(BatchFusion):
    ...
    Batch linear left-hand side fusion. This pass tries to fuse the following patterns:
        ...
        smith.nn.functional.linear(x, w1), linear(x, w2),... * linear(x, wn)
        -> smith.mm(x, smith.cat([w1, w2,... * wn]).transpose(0, 1))

    We have a separate pass to eliminate contiguous transpose in a generic way.
    """

    def match(self, node: smith.fx.Node) -> tuple[str, bool, Any] | None:
        if CallFunctionVarArgs(smith.nn.functional.linear).match(
            node
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        ...
        with graph.inserting_before(subset[0]):  # type: ignore[operator]
            cat_weights = graph.call_function(  # type: ignore[operator]
                smith.cat, args=(batch_weights,), kwargs={"dim": 0}
            )
            cat_weights.meta["example_value"] = smith.cat(batch_weights_meta, dim=0)
            transposed_weights = graph.call_function(  # type: ignore[operator]
                smith.transpose, args=(cat_weights, 0, 1)
            )
            transposed_weights.meta["example_value"] = smith.transpose(
                cat_weights.meta["example_value"], 0, 1
            )
            if len(batch_biases) > 0:
                cat_biases = graph.call_function(  # type: ignore[operator]
                    smith.cat, args=(batch_biases,), kwargs={"dim": 0}
                )
                cat_biases.meta["example_value"] = smith.cat(batch_biases_meta, dim=0)
                fused_lhs = graph.call_function(  # type: ignore[operator]
                    smith.addmm,
                    args=(cat_biases, batch_input, transposed_weights),
                )
                fused_lhs.meta["example_value"] = smith.addmm(
                ...
                fused_lhs.meta["example_value"] = smith.mm(
                    batch_input.meta["example_value"],  # type: ignore[union-attr]
                    transposed_weights.meta["example_value"],
                )
            fused_lhs_list = graph.call_function(  # type: ignore[operator]
                smith.split, args=(fused_lhs, split_sections), kwargs={"dim": 1}
            )
...
# (the graph is smith IR, so we will see smith fns and python operators)
def _is_mutable_node(tgt):
    if str(tgt).endswith("_"):
        # e.g. smith.mul_, smith.Tensor.mul_
        return True
    if (
...
def is_linear_node_can_be_fused(node: smith.fx.Node):
    input = get_arg_value(node, 0, "input")
    weight = get_arg_value(node, 1, "weight")
    return (
...
class PreGradBatchLinearFusion(BatchFusion):
    ...
    Fuse linear with same size with smith.baddmm
    """

    def _getitem_args(self, getitem_node: smith.fx.Node):
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        ...
        with graph.inserting_before(subset[0]):  # type: ignore[operator]
            ...
            if all(bias is None for bias in batch_biases):
                ...
                bmm.meta["example_value"] = smith.bmm(
                ...
                stack_biases = graph.call_function(  # type: ignore[operator]
                    smith.stack, args=(batch_biases,), kwargs={"dim": 0}
                )
                update_stack_example_value(stack_biases, batch_biases_metadata)
                unsqueeze_biases = graph.call_function(  # type: ignore[operator]
                    smith.unsqueeze, args=(stack_biases, 1)
                )
                unsqueeze_biases.meta["example_value"] = smith.unsqueeze(
                ...
                try:
                    ...
                    bmm.meta["example_value"] = smith.baddbmm(
            ...
            bmm = graph.call_function(smith.unbind, args=(bmm,), kwargs={"dim": 0})  # type: ignore[operator]
            if bmm_meta is not None:
                bmm.meta["example_value"] = smith.unbind(bmm_meta, dim=0)
...
class BatchLayernormFusion(BatchFusion):
    ...
    def match(self, node: smith.fx.Node):
        if CallFunctionVarArgs(smith.nn.functional.layer_norm).match(node):
            input = get_arg_value(node, 0, "input")
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        ...
        with graph.inserting_before(subset[0]):  # type: ignore[operator]
            stack_input = graph.call_function(  # type: ignore[operator]
                smith.stack, args=(group_inputs,), kwargs={"dim": stack_dim}
            )
            ...
            if group_weights is not None:
                stack_weight = graph.call_function(  # type: ignore[operator]
                    smith.stack, args=(group_weights,), kwargs={"dim": 0}
                )
            ...
            if group_biases is not None:
                stack_bias = graph.call_function(  # type: ignore[operator]
                    smith.stack, args=(group_biases,), kwargs={"dim": 0}
                )
            ...
            if group_weights is not None and group_biases is not None:
                ...
                batch_layer_norm = graph.call_function(  # type: ignore[operator]
                    smith.mul, args=(stack_weight, batch_layer_norm)
                )
                ...
                previous_batch_layer_norm_meta = batch_layer_norm.meta["example_value"]
                batch_layer_norm = graph.call_function(  # type: ignore[operator]
                    smith.add, args=(stack_bias, batch_layer_norm)
                )
                update_pointwise_example_value(
                    batch_layer_norm,
                    # pyrefly: ignore [missing-attribute]
...
class BatchPointwiseOpsPreGradFusion(BatchPointwiseOpsFusionFactory):
    ...
    def fuse(self, graph: smith.fx.GraphModule, subset: list[smith.fx.Node]):
        ...
        with graph.inserting_before(subset[0]):  # type: ignore[operator]
            ...
            update_stack_example_value(stack_inputs, batch_inputs_metadata)
            if self.op is smith.nn.functional.relu:
                batch_op = graph.call_function(  # type: ignore[operator]
                    self.op,
                    args=(stack_inputs,),
            ...
            unbind_op = graph.call_function(  # type: ignore[operator]
                smith.unbind, args=(batch_op,), kwargs={"dim": 0}
            )
            unbind_op.meta["example_value"] = smith.unbind(
...
def find_independent_subset_greedy(
    node_list: Iterable[smith.fx.Node],
    graph_search_options: dict[str, Any],
) -> Iterator[Iterable[smith.fx.Node]]:
    """
    ...
    def find_dependent_nodes(node, interesting_nodes):
        visited_node_set = OrderedSet[smith.fx.Node]()
        dep_set = OrderedSet[smith.fx.Node]()
    ...
    cache: dict[smith.fx.Node, OrderedSet[smith.fx.Node]] = {}
    while node_list:
        subset: list[smith.fx.Node] = []
        subset_deps = OrderedSet[smith.fx.Node]()
...
def get_fusion_candidates(
    ...
    root_node: smith.fx.Node,
    fused_set: OrderedSet[smith.fx.Node],
) -> collections.defaultdict[Any, list[smith.fx.Node]]:
    """
    ...
    """
    q: collections.deque[tuple[int, smith.fx.Node]] = collections.deque()

    candidate_dict: collections.defaultdict[Any, list[smith.fx.Node]] = (
    ...
    visited_set = OrderedSet[smith.fx.Node]()
...
def apply_group_batch_fusion(graph: smith.fx.GraphModule, rule: GroupBatchFusionBase):
...
def group_batch_fusion_passes(graph: smith.fx.Graph, pre_grad=True):
    fusions: list[GroupBatchFusionBase] = []
