...
from smith._higher_order_ops.associative_scan import associative_scan_op
from smith._higher_order_ops.triton_kernel_wrap import triton_kernel_wrapper_mutation
...
from smith.fx.experimental.sym_node import magic_methods, method_to_operator
...
# https://github.com/blacksmith/smithdynamo/issues/327
FALLBACK_ALLOW_LIST = OrderedSet(
    [
        "smithvision::roi_align",
        "aten::index_add",
    ]
)
...
_maybe_layout_constraints: dict[
    smith._ops.OpOverload, Optional[Callable[..., Any]]
] = {}
fallbacks = OrderedSet[smith._ops.OpOverload]()
aten = smith.ops.aten
tr_c10d = smith.ops.tr_c10d
prims = smith.ops.prims
needs_realized_inputs = OrderedSet[smith._ops.OpOverload]()
foreach_ops = OrderedSet[smith._ops.OpOverload](
    [smith._higher_order_ops._foreach_map]  # type: ignore[list-item]
)
# TODO(rec): smith._higher_order_ops._foreach_map is not an OpOverload
# so why is it in foreach_ops?
inplace_foreach_ops = OrderedSet[smith._ops.OpOverload]()
inplaceable_foreach_ops: dict[smith._ops.OpOverload, smith._ops.OpOverload] = {}
...
def maybe_layout_constraints(fn: Callable[..., Any]) -> Optional[Callable[..., Any]]:
    ...
    if not isinstance(fn, smith._ops.OpOverload):
...
def tag_to_layout_constraint(
    ...
    if tag == smith._C.Tag.needs_exact_strides:
        return constrain_to_fake_tensors
    if tag == smith._C.Tag.needs_contiguous_strides:  # type: ignore[attr-defined]
        return require_contiguous_strides
    if tag == smith._C.Tag.needs_fixed_stride_order:
        return constrain_to_fx_strides
    if tag == smith._C.Tag.flexible_layout:
        return None
...
def add_needs_realized_inputs(
    fn: Union[
        Collection[Union[smith._ops.OpOverload, smith._ops.OpOverloadPacket]],
        smith._ops.OpOverload,
        smith._ops.OpOverloadPacket,
    ],
) -> Optional[list[Any]]:
    if isinstance(fn, (list, set, tuple, OrderedSet)):  # noqa: set_linter
        # pyrefly: ignore [bad-argument-type]
        return [add_needs_realized_inputs(x) for x in fn]
    if isinstance(fn, smith._ops.OpOverload):
        needs_realized_inputs.add(fn)
    elif isinstance(fn, smith._ops.OpOverloadPacket):
...
def add_layout_constraint(
    fn: Union[smith._ops.OpOverloadPacket, smith._ops.OpOverload],
    constraint: Callable[..., tuple[Any, Any]],
) -> None:
    if isinstance(fn, smith._ops.OpOverloadPacket):
...
# based on https://github.com/blacksmith/blacksmith/blob/9e3eb329df8f701/c10/core/ScalarType.h#L28
DTYPE_ID_LOOKUP = {
    0: smith.uint8,
    1: smith.int8,
    2: smith.int16,
...
def decode_dtype(dtype: Union[int, smith.dtype]) -> smith.dtype:
    ...
    *args: Any,
    ...
    def construct_input(inp: Any) -> Any:
        if isinstance(inp, (Number, sympy.Basic)):
            ...
            # construct a tmp tensor to feed into smith.result_type
            return smith.zeros([1] * dim, dtype=inp.get_dtype())
...
def in_namespace(
    op: Union[Any, smith._ops.OpOverloadPacket, smith._ops.OpOverload], namespace: str
) -> bool:
    if isinstance(op, smith._ops.OpOverloadPacket):
        return namespace in op._qualified_op_name
    elif isinstance(op, smith._ops.OpOverload):
        return namespace in op.name()
    return False


def maybe_copy_cpu_scalar(x: TensorBox, device: smith.device) -> TensorBox:
    """
...
def _register_foreach_lowering(
    aten_fn: smith._ops.OpOverload, decomp_fn: Callable[..., Any]
) -> Callable[..., Any]:
    """
    Add a foreach lowering to lowerings dict.

    Arguments:
        aten_fn: smith.ops.aten.* fn we are lowering
...
def make_pointwise(
    ...
    def inner(*inputs: TensorBox, alpha=None):
        ...
        low_pr_fp = (smith.bfloat16, smith.float16)
        ...
        def inner_fn(index):
            ...
            if dtype == smith.bool and override_fn_when_input_bool is not None:
...
def to_dtype(x: TensorBox, dtype: smith.dtype, copy: bool = False):
...
@register_lowering(smith._higher_order_ops._foreach_map, type_promotion_kind=None)
...
def _convert_element_type(x: TensorBox, dtype: smith.dtype):
...
def to_dtype_bitcast(x: TensorBox, dtype: smith.dtype, *, copy=False):
    x_dtype = x.get_dtype()
    if x_dtype == dtype:
        return clone(x) if copy else x
    ...
    def _get_primitive_bitwidth(dtype):
        if dtype.is_floating_point:
            return smith.finfo(dtype).bits
        else:
            return smith.iinfo(dtype).bits
...
def _view_dtype(x: TensorBox, dtype: smith.dtype):
    if dtype.is_complex or x.get_dtype().is_complex:
        return TensorBox.create(
            ir.ComplexView.create(smith.ops.aten.view.dtype, x, dtype)
        )
    return to_dtype_bitcast(x, dtype)


def to_device(x: TensorBox, device: smith.device, *, copy=False, non_blocking=False):
...
def _device_put(x: TensorBox, device: smith.device, non_blocking=False):
...
def ldexp_lowering(x: TensorBox, n: TensorBox):
    ...
    n_is_int = not n_dtype.is_floating_point and n_dtype != smith.bool
    ...
    if x_is_float and n_is_int:
        ...
        out_dtype = smith.float32 if is_integer_type(x) else x_dtype
...
def register_frexp():
    ...
    pw_fns = [
        ...
        make_pointwise(frexp1, override_return_dtype=smith.int32),
    ]
...
def isinf(x):
    if is_integer_type(x):
        return full_like(x, False, dtype=smith.bool)
    fn = ops_wrapper("isinf")
    return make_pointwise(fn, override_return_dtype=smith.bool)(x)
...
def pointwise_cat(inputs, dim=0):
    ...
    def inner_fn(idx):
        idx_dim = ops.index_expr(idx[dim], smith.int64)

        masks = []
        masked_loads = []
        for i in range(len(inputs)):
            start = (
                ops.constant(0, smith.int64)
                if i == 0
                else ops.index_expr(inputs_ranges[i][0], smith.int64)
            )
            end = ops.index_expr(inputs_ranges[i][1], smith.int64)
...
def quantized_decomposed_quantize_per_channel(
    ...
    if input.get_dtype() == smith.bfloat16:
        input = to_dtype(input, smith.float32)
    assert input.get_dtype() == smith.float32, (
        f"Expecting input to have dtype smith.float32, but got dtype: {input.get_dtype()}"
    )
    ...
    def inner_fn(idx):
        ...
        qmin, qmax = _create_constants(quant_min, quant_max, dtype=smith.float32)

        if scales.dtype != smith.float32:
            scale = ops.to_dtype(scale, smith.float32)
        if zero_points.dtype != smith.int32:
            zero_point = ops.to_dtype(zero_point, smith.int32)
...
def quantized_decomposed_dequantize_per_channel(
    ...
    *,
    ...
    def inner_fn(idx):
        ...
        if scales.dtype != smith.float32:
            scale = ops.to_dtype(scale, smith.float32)
        if zero_points.dtype != smith.float32:
            zero_point = ops.to_dtype(zero_point, smith.float32)
        val = ops.sub(ops.to_dtype(input, smith.float32), zero_point) * scale
...
def quantized_decomposed_quantize_per_tensor_default(
    ...
    def inner_fn(idx, scale, zero_point):
        ...
        inv_scale, zero_point = _create_constants(
            1.0 / scale, zero_point, dtype=smith.float32
        )
        val = ops.round(input * inv_scale) + zero_point
        qmin, qmax = _create_constants(quant_min, quant_max, dtype=smith.float32)
...
def quantized_decomposed_dequantize_per_tensor_default(
    ...
    *,
    ...
    def inner_fn(idx, scale, zero_point):
        input = input_loader(idx)
        scale, zero_point = _create_constants(scale, zero_point, dtype=smith.float32)
        val = ops.sub(ops.to_dtype(input, smith.float32), zero_point) * scale
        val = ops.to_dtype(val, out_dtype)
        return val
...
def quantized_decomposed_quantize_per_tensor_tensor(
    ...
    def inner_fn(idx):
        ...
        if scale.dtype != smith.float32:
            _scale = ops.to_dtype(_scale, smith.float32)
        if zero_point.dtype != smith.float32:
            _zero_point = ops.to_dtype(_zero_point, smith.float32)
...
def quantized_decomposed_dequantize_per_tensor_tensor(
    ...
    *,
    ...
    def inner_fn(idx):
        ...
        if zero_point.dtype != smith.float32:
            _zero_point = ops.to_dtype(_zero_point, smith.float32)
        val = ops.sub(ops.to_dtype(input, smith.float32), _zero_point) * _scale
        val = ops.to_dtype(val, out_dtype)
        return val
...
def cat(inputs, dim=0):
    ...
    if cpu_device and all(
        input.get_dtype() in [smith.int8, smith.uint8] for input in inputs
    ):
    ...
    def additional_pointwise_ops(op: smith._ops.OpOverload):
...
def _warn_complex_not_supported():
    warnings.warn(
        "Smithinductor does not support code generation for complex operators. Performance may be worse than eager."
    )


# There are some types (CPU) which we accept as input but not as
# output.
def unsupported_input_tensor(t: smith.Tensor, node=None):
    ...
    if t.dtype == smith.float8_e8m0fnu:
        if not node:
            return True
        ...
        return not (
            isinstance(node.target, smith._ops.OpOverload)
            and node.target
            in (
                aten.view.dtype,
                aten.cat.default,
            ...
            or (isinstance(node.target, smith._ops.OpOverload) and is_view(node.target))
        )

    return False


def unsupported_output_tensor(t: smith.Tensor, node=None):
...
def fallback_node_due_to_unsupported_type(node: smith.fx.Node, allow_cpu_inputs=True):
    ...
    def check_skip_condition(inp_out_node, is_output):
        if not isinstance(inp_out_node, smith.fx.Node):
            return False
        ...
        for meta in pytree.tree_leaves(inp_out_node.meta["val"]):
            if not isinstance(meta, smith._subclasses.FakeTensor):
                continue
...
def make_fallback(op, layout_constraint=None, warn=True, override_decomp=False):
    ...
    if (
        ...
        and not (
            ...
            and op in smith._decomp.decompositions_for_rng.extra_random_decomps
        )
        ...
        if smith._dynamo.config.suppress_errors:
            smith._dynamo.config.suppress_errors = False
            log.warning(
    ...
    elif isinstance(op, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)):
...
def philox_rand_offset(shape):
    """
    SmithInductor offset calculation differs from Blacksmith eager offset
    calculation for random ops (tl.rand vs smith.rand). In future, we should
    strive for same impl for tl.rand and smith.rand.
    """
    numel = 1
    for s in shape:
        numel = numel * s
    return tensor(numel, dtype=smith.int64)


@register_lowering(smith.ops.rngprims.philox_rand, type_promotion_kind=None)
def philox_rand(size, seed, offset, stride, device, dtype):
    ...
    def inner_fn(index):
        ...
        # smith seed and offsets are of type int64, but tl.rand accepts int32
        seed_index_expr = ops.to_dtype(seed_loader([]), smith.int32)
        offset_index_expr = ops.to_dtype(offset_loader([]), smith.int32)
        # Get the offset'd position
        rand_index_expr = ops.add(
            ops.index_expr(random_pos(index), smith.int32), offset_index_expr
        )
        result = ops.rand(
            seed_index_expr,
            rand_index_expr,
...
def bernoulli_(x, *args):
    assert config.fallback_random or x.get_device() == smith.device("cpu"), (
...
make_fallback(smith.ops.streams.record_event.default)
make_fallback(smith.ops.streams.wait_event.default)
...
def inductor_seed(device: smith.device):
...
def inductor_random(size: list[int], seed: TensorBox, mode: str, *, offset: int = 0):
    ...
    def inner_fn(index):
        return getattr(ops, mode)(
            seed_loader([]),
            ops.index_expr(random_pos(index), smith.int32),
        )
...
def inductor_randint(
    ...
    def inner_fn(index):
        return ops.randint64(
            seed_loader([]),
            ops.index_expr(random_pos(index), smith.int32),
            ops.index_expr(low, smith.int64),
            ops.index_expr(high, smith.int64),
        )
...
def searchsorted(
    ...
    *,
    ...
    index_dtype = smith.int32 if out_int32 else smith.int64
...
def require_contiguous_strides(_, *args, **kwargs):
    ...
    # https://github.com/blacksmith/blacksmith/pull/148235 lands.
    args, kwargs = pytree.tree_map_only(
...
def sdpa_constraint(fx_node, *args, **kwargs):
    ...
    def apply_constraint(idx, arg, fx_arg):
        ...
        meta_stride_expr = [
            s.node.expr if isinstance(s, smith.SymInt) else s for s in meta_val.stride()
        ]
        ...
        if (
            ...
            # Check https://github.com/blacksmith/blacksmith/issues/138772
        ...
        # This value can be found in blacksmith/aten/src/ATen/native/transformers/attention.cpp preprocess_mask
        ALIGNMENT = 8
...
make_fallback(aten.exponential.default, warn=False)  # (fails accuracy on test_smith.py)
...
# https://dev-discuss.blacksmith.org/t/blacksmith-sparse-gnn-compiler-rfc/1644/19
...
# see: https://github.com/blacksmith/blacksmith/pull/121354
...
make_fallback(smith._prims.rng_prims.run_and_save_rng_state)
make_fallback(smith._prims.rng_prims.run_with_rng_state)
make_fallback(smith._prims.rng_prims.graphsafe_run_with_rng_state)
...
def select_scatter(x, src, dim: int, index: int):
    ...
    def inner_fn(idx):
        return ops.where(
            ops.eq(
                ops.index_expr(idx[dim], smith.int32),
                ops.index_expr(index, smith.int32),
            ),
            src_loader(idx),
            x_loader(idx),
        )
    ...
    def inner_fn(idx):
        ...
        if start != 0:
            mask.append(
                ops.ge(
                    idx_dim,
                    ops.index_expr(sympy.expand(start), smith.int64),
                )
            )
        if end != dim_size:
            mask.append(
                ops.lt(
                    idx_dim,
                    ops.index_expr(sympy.expand(end), smith.int64),
                )
            )
        if step != 1:
            mask.append(
                ops.eq(
                    ops.index_expr(
                        ModularIndexing(idx[dim] - start, 1, step), smith.int64
                    ),
                    ops.constant(0, smith.int64),
                )
            )
...
@register_lowering([smith.tensor, aten.scalar_tensor])
def tensor(data, *, dtype=None, device=None, layout=None, pin_memory=False):
    assert_nyi(layout in (None, smith.strided), f"layout={layout}")
    assert_nyi(not pin_memory, "pin_memory")
    if isinstance(_unwrap(data), int):
        dtype = dtype or smith.int64
    else:
    ...
    if isinstance(data, sympy.Basic):
        ...
        def inner_fn(index):
            def binary_search(start, end):
                ...
                return ops.where(
                    ops.lt(
                        ops.index_expr(index[0], smith.int64),
                        ops.constant(mid, smith.int64),
                    ),
        ...
        return V.graph.add_tensor_constant(
            smith.tensor(data, dtype=dtype, device=device)
        )
...
@register_lowering(smith.as_tensor)
...
@register_lowering(smith.LongTensor)
def long_tensor(data):
    return tensor(data, dtype=smith.int64)
...
def _local_scalar_dense(data):
    ...
    if isinstance(val, (smith.SymInt, smith.SymFloat, smith.SymBool)):
...
def tensor_constructor(fill_value):
    ...
    def inner(
        *size,
        ...
        device = decode_device(device)
        dtype = dtype or smith.get_default_dtype()
        if len(size) == 1 and isinstance(size[0], (list, tuple, smith.Size)):
            size = tuple(size[0])
        # See https://github.com/blacksmith/blacksmith/issues/118102
        # All sizes at lowering time should be sympy.Symbol, not SymInt!
        for s in size:
            assert not isinstance(s, smith.SymInt)
...
@register_lowering([smith.empty, aten.empty])
def empty(
    *size,
    names=None,
    dtype=None,
...
def empty_strided(
    ...
    assert_nyi(layout in (None, smith.strided), f"layout={layout}")
    # pyrefly: ignore [bad-argument-type]
    dtype = decode_dtype(dtype) or smith.get_default_dtype()
    device = device or smith.tensor(0.0).device
    device = decode_device(device)
...
@register_lowering([smith.full, aten.full])
...
def check_and_broadcast_indices(indices, device):
    assert all(
        i.get_dtype() in (smith.int64, smith.int32, smith.bool, smith.uint8)
        for i in indices
        if i is not None
    ), (
    ...
    if any(
        i.get_dtype() in (smith.bool, smith.uint8) for i in indices if i is not None
    ):
...
def index_output_size_and_inner_fn(
    ...
    #
    # Suppose a = smith.arange(3 * 4 * 5 * 6 * 7).view(3, 4, 5, 6, 7)
    #         x = smith.tensor[1,2]
...
# https://github.com/blacksmith/smithdynamo/issues/1235
# and
# https://github.com/blacksmith/smithdynamo/issues/1863
...
def index_put_impl_(self, indices, values, accumulate, check, may_realize=False):
    if may_realize:
        ...
        def indice_slice_from_randperm(indice):
            # Refer to: https://github.com/blacksmith/blacksmith/pull/139366#discussion_r1825424660
            # For this specific pattern, indices is unique as coming from smith.randperm.
            ...
            if isinstance(indice, TensorBox) and isinstance(indice.data, ir.BaseView):
                ...
                return (
                    ...
                    and indice.data.fx_node.target is smith.ops.aten.randperm.default
                )
            return False
        ...
        if ir.try_get_name(self) in values.get_read_names() and not all(
            ...
            # Fix issue: https://github.com/blacksmith/blacksmith/issues/138908
    ...
    if (
        values.get_numel() == 1
        and len(indices) == 1
        and indices[0].get_dtype() in (smith.bool, smith.uint8)
    ):
        mask = indices[0]
    ...
    # Fallback in smith deterministic mode
    if smith.are_deterministic_algorithms_enabled():
        return index_put_fallback(self, indices, values, accumulate)

    # Fallback if there is a boolean index
    for index in indices:
        if index is not None and index.get_dtype() in (smith.bool, smith.uint8):
            return index_put_fallback(self, indices, values, accumulate)
...
def _unsafe_masked_index(self, mask, indices, fill):
    ...
    def inner_fn(idx):
        if mask.dtype != smith.bool:
            mask_val = ops.to_dtype(mask_loader(idx), smith.bool)
        else:
...
def scatter_fallback(
    op_overload: smith._ops.OpOverload,
    self,
    dim: int,
    index,
    src,
    *,
    ...
    if use_scatter_fallback(
        ...
        reduce,
        self.get_dtype(),
        cast(smith.dtype, src.get_dtype() if src_is_tensor else type(src)),
...
def upsample_nearestnd(
    ...
    def scale_fn(x, scale, size):
        ...
        x = ops.index_expr(x, smith.float32)
        if exact:
            x = ops.add(x, ops.constant(0.5, smith.float32))
        x = ops.mul(x, ops.constant(scale, smith.float32))
        x = ops.to_dtype(x, smith.int32)
...
def range_mask_low(i: sympy.Expr, low: Union[sympy.Expr, int]):
    return ops.ge(
        ops.index_expr(i, smith.int64),
        ops.index_expr(sympy.Integer(low), smith.int64),
    )
...
def _max_pool_with_offsets(
    ...
    *,
    ...
    min_value = (
        ...
        else (float("-inf") if dtype.is_floating_point else smith.iinfo(dtype).min)
    )
...
def _low_memory_max_pool_with_offsets(
    ...
    with config.patch(unroll_reductions_threshold=25):
        ...
        return result, to_dtype(offsets, smith.int8)


def _pool_offsets_to_indices(
    offsets: TensorBox,
    kernel_size: Sequence[Union[int, smith.SymInt]],
    input_size: Sequence[Union[int, smith.SymInt]],
    increments_to_index: Callable[
        [Sequence[Union[int, smith.SymInt]], Sequence[Union[int, smith.SymInt]]],
        smith._inductor.virtualized.OpsValue,
    ],
) -> TensorBox:
    n_dim = len(kernel_size)
    ...
    def offsets_to_indices(idx):
        ...
        return ops.index_expr(
            inductor_prims._flatten_index(idhw, input_size[-n_dim:]), smith.int64
        )
...
def max_pool2d_with_indices_backward(
    ...
    *_, pooled_height, pooled_width = grad_output.get_size()
    ...
    def fn(idx):
        *prefix, h, w = idx
        index_test = ops.index_expr(h * width + w, smith.int32)
        h = h + padding[0]
        w = w + padding[1]
        phstart = ops.index_expr(
            FloorDiv(h - kernel_size[0] + stride[0], stride[0]), smith.int32
        )
        pwstart = ops.index_expr(
            FloorDiv(w - kernel_size[1] + stride[1], stride[1]), smith.int32
        )
        phend = ops.index_expr(FloorDiv(h, stride[0]) + 1, smith.int32)
        pwend = ops.index_expr(FloorDiv(w, stride[1]) + 1, smith.int32)

        phstart = ops.maximum(phstart, ops.constant(0, smith.int32))
        pwstart = ops.maximum(pwstart, ops.constant(0, smith.int32))
        phend = ops.minimum(phend, ops.index_expr(pooled_height, smith.int32))
        pwend = ops.minimum(pwend, ops.index_expr(pooled_width, smith.int32))

        gradient = None
        for ph_ in range(h_window_size):
            for pw_ in range(w_window_size):
                ph = ops.add(phstart, ops.constant(ph_, smith.int32))
                pw = ops.add(pwstart, ops.constant(pw_, smith.int32))
                grad_index = [
                    *prefix,
                    ops.indirect_indexing(
                        ops.minimum(ph, ops.sub(phend, ops.constant(1, smith.int32))),
                        indices_size[-2],
                        check=False,
                    ),
                    ops.indirect_indexing(
                        ops.minimum(pw, ops.sub(pwend, ops.constant(1, smith.int32))),
                        indices_size[-1],
                        check=False,
                    ),
                ]
                ...
                if gradient is None:
                    ...
                    gradient = ops.where(
                        check, grad_part, ops.constant(0.0, smith.float32)
                    )
                else:
                    mask = ops.and_(
                        ops.and_(
...
def pad_adaptive_loader(x, pad_val=0.0):
    ...
    def load(prefix, increments, start_indices, end_indices):
        ...
        mask = ops.and_(
            ops.lt(
                ops.index_expr(h_start_index + ih, smith.int64),
                ops.index_expr(h_end_index, smith.int64),
            ),
            ops.lt(
                ops.index_expr(w_start_index + iw, smith.int64),
                ops.index_expr(w_end_index, smith.int64),
            ),
        )
        ...
        *prefix, bh, bw = idx
        ...
        for ih, iw in itertools.product(range(kernel_maxes[0]), range(kernel_maxes[1])):
            ...
            index = ops.index_expr(
                (h_start_index + ih) * w_in + w_start_index + iw, smith.int64
            )
    ...
    *batch, h_in, w_in = x.get_size()
    ...
    if h_out == 0 or w_out == 0:
        ...
        return empty(o_size, dtype=x.get_dtype(), device=x.get_device()), empty(
            o_size, dtype=smith.int64, device=x.get_device()
        )
...
def _fractional_pooling_offsets(samples, in_sz, out_sz, kernel_sz, dim, ndims):
    ...
    def load(prefix, i):
        ...
        diff = ops.index_expr(in_sz - kernel_sz, smith.int64)
        out_sz_expr = ops.index_expr(out_sz - 1, smith.int64)
        alpha = ops.truediv(
            ops.to_dtype(diff, smith.float64), ops.to_dtype(out_sz_expr, smith.float64)
        )
        alpha = ops.where(ops.eq(out_sz_expr, 0), 0, alpha)
        seq_i = ops.trunc((i_expr + sample) * alpha) - ops.trunc(sample * alpha)
        seq_i = ops.to_dtype(seq_i, smith.int64)
    ...
    if not had_padding or divisor_override:
        ...
        def fn_count(idx):
            ...
            for i in range(dim):
                ...
                factor = ops.index_expr(hend - hstart, smith.int32)
...
def avg_pool2d_backward(
    ...
    *_, pooled_height, pooled_width = grad_output.get_size()
    ...
    def compute_pool_size_without_padding(ph, pw):
        ...
        """
        stride_h = ops.constant(stride[0], smith.int32)
        stride_w = ops.constant(stride[1], smith.int32)
        pad_h = ops.constant(padding[0], smith.int32)
        pad_w = ops.constant(padding[1], smith.int32)
        kernel_h = ops.constant(kernel_size[0], smith.int32)
        kernel_w = ops.constant(kernel_size[1], smith.int32)
        hstart = ops.sub(ops.mul(ph, stride_h), pad_h)
        wstart = ops.sub(ops.mul(pw, stride_w), pad_w)
        hend = ops.minimum(
            ops.add(hstart, kernel_h),
            ops.add(ops.index_expr(height, smith.int32), pad_h),
        )
        wend = ops.minimum(
            ops.add(wstart, kernel_w),
            ops.add(ops.index_expr(width, smith.int32), pad_w),
        )
        hstart = ops.maximum(hstart, ops.constant(0, smith.int32))
        wstart = ops.maximum(wstart, ops.constant(0, smith.int32))
        hend = ops.minimum(hend, ops.index_expr(height, smith.int32))
        wend = ops.minimum(wend, ops.index_expr(width, smith.int32))
        ...
        *prefix, h, w = idx
        ...
        for ph_ in range(h_window_size):
            for pw_ in range(w_window_size):
                    ...
                    grad_loader(
                        [
                            *prefix,
                            ops.indirect_indexing(
                                ops.minimum(
                                    ph, ops.sub(phend, ops.constant(1, smith.int32))
                                ),
                                pooled_height,
                                check=False,
                            ),
                            ops.indirect_indexing(
                                ops.minimum(
                                    pw, ops.sub(pwend, ops.constant(1, smith.int32))
                                ),
                                pooled_width,
                                check=False,
                            ),
                ...
                if gradient is None:
                    gradient = ops.where(mask, part, ops.constant(0.0, smith.float32))
                else:
...
def avg_pool3d_backward(
    ...
    *_, pooled_depth, pooled_height, pooled_width = grad_output.get_size()
    ...
    def compute_pool_size_without_padding(pd, ph, pw):
        stride_d, stride_h, stride_w = (ops.constant(s, smith.int32) for s in stride)
        pad_d, pad_h, pad_w = (ops.constant(p, smith.int32) for p in padding)
        kernel_d, kernel_h, kernel_w = (
            ops.constant(k, smith.int32) for k in kernel_size
        )
        ...
        dend, hend, wend = (
            ops.minimum(
                ops.add(start, k), ops.add(ops.index_expr(dim, smith.int32), pad)
            )
        ...
        dstart, hstart, wstart = (
            ops.maximum(start, ops.constant(0, smith.int32))
            for start in [dstart, hstart, wstart]
        )
        dend, hend, wend = (
            ops.minimum(end, ops.index_expr(dim, smith.int32))
    ...
    def fn(idx):
        *prefix, d, h, w = idx
        ...
        pdstart, phstart, pwstart = (
            ops.index_expr(FloorDiv(v - k + s, s), smith.int32)
            for v, k, s in zip([d, h, w], kernel_size, stride)
        )

        pdend, phend, pwend = (
            ops.index_expr(FloorDiv(v, s) + 1, smith.int32)
            for v, s in zip([d, h, w], stride)
        )

        pdstart, phstart, pwstart = (
            ops.maximum(pstart, ops.constant(0, smith.int32))
            for pstart in [pdstart, phstart, pwstart]
        )
        pdend, phend, pwend = (
            ops.minimum(pend, ops.index_expr(pooled_dim, smith.int32))
        ...
        for pd_ in range(d_window_size):
            for ph_ in range(h_window_size):
                for pw_ in range(w_window_size):
                    pd, ph, pw = (
                        ops.add(pstart, ops.constant(p_, smith.int32))
                        ...
                        grad_loader(
                            [
                                *prefix,
                                ops.indirect_indexing(
                                    ops.minimum(
                                        pd, ops.sub(pdend, ops.constant(1, smith.int32))
                                    ),
                                    pooled_depth,
                                    check=False,
                                ),
                    ...
                    if gradient is None:
                        gradient = ops.where(
                            mask, part, ops.constant(0.0, smith.float32)
                        )
                    else:
                        gradient = ops.where(mask, ops.add(gradient, part), gradient)
        assert gradient is not None
...
def _make_reduction_inner(
    ...
    def loader(index, reduction_index):
        ...
        if should_compute_logical_index:
            ...
            return (value, ops.index_expr(linear_idx, smith.int64))
...
def mean(x, axis=None, keepdim=False, *, dtype=None):
    ...
    if output_dtype in (smith.float16, smith.bfloat16):
        x = to_dtype(x, smith.float)
...
def var_mean_welford_(x, axis, *, correction, keepdim, return_mean):
    ...
    def get_constant_or_index_expr(x, dtype):
        if isinstance(x, sympy.Expr) and not x.is_number:
            return ops.to_dtype(ops.index_expr(x, smith.int64), dtype)
...
def get_constant_value(x: ir.IRNode) -> Optional[ir.Constant]:
    ...
    handler = smith._inductor.ops_handler.ExtractConstantsHandler(x.get_device())
    with (
    ...
    assert isinstance(out, smith._inductor.virtualized.OpsValue)
...
def div_prim(a, b):
    ...
    # see https://github.com/blacksmith/blacksmith/issues/157959
...
def cummax(x, axis=None):
    if len(x.get_size()) == 0:
        assert axis in [0, -1]
        return clone(x), empty_like(x, dtype=smith.int64)
    ...
    kwargs = _make_scan_inner(x, axis=axis, dtype=dtype)
    kwargs["dtypes"] = (dtype, smith.int64)
    kwargs["inner_fns"] = (
        x.make_loader(),
        lambda idx: ops.index_expr(idx[axis], smith.int64),
    )
...
reduce_argmax = register_lowering(aten.argmax)(
    make_reduction("argmax", override_return_dtype=smith.int64)
)
reduce_argmin = register_lowering(aten.argmin)(
    make_reduction("argmin", override_return_dtype=smith.int64)
)
...
def sort_stable(x, *, stable=None, dim=-1, descending=False):
    ...
    if len(shape) == 0:
        return clone(x), _full(0, device, smith.int64, shape)

    dim_size = shape[dim] if len(shape) else 1
    if not V.graph.sizevars.statically_known_lt(dim_size, smith.iinfo(smith.int16).max):
        return sort_fallback(x, stable=stable, dim=dim, descending=descending)

    indices = iota(
        dim_size, start=0, step=1, dtype=smith.int16, device=device, requires_grad=False
    )
    ...
    return values, to_dtype(indices, smith.int64)
...
def register_pointwise_numeric_ldf64(op: smith._ops.OpOverloadPacket):
...
def addcmul(self, tensor1, tensor2, *, value=1):
    ...
    use_fma = dtype.is_floating_point and not smith.version.hip
...
logical_and = register_pointwise(
    ...
    override_return_dtype=smith.bool,
)
...
register_pointwise(aten.signbit, override_return_dtype=smith.bool)

register_lowering(aten._neg_view)(neg)

register_pointwise(aten.le, override_return_dtype=smith.bool)
register_pointwise(aten.lt, override_return_dtype=smith.bool)
register_pointwise(aten.ge, override_return_dtype=smith.bool)
gt = register_pointwise(aten.gt, override_return_dtype=smith.bool)
register_pointwise(aten.eq, override_return_dtype=smith.bool)
register_pointwise(aten.ne, override_return_dtype=smith.bool)
...
@register_lowering(smith.sym_sum)
def sym_sum(args):
...
@register_lowering(smith.ops._inductor_test.realize)
def _realize(x):
    x.realize()
    return clone(x)


@register_lowering(smith.ops.inductor.resize_storage_bytes_)
...
@register_lowering(smith.ops.aten.set_.source_Tensor)
...
if hasattr(smith.ops.fsdp, "copy_"):

    @register_lowering(smith.ops.fsdp.copy_.default)
...
@register_lowering(smith.ops.aten.resize)
def resize(x, size, *, memory_format=None):
    ...
    if (
        ...
        and smith.utils.deterministic.fill_uninitialized_memory  # type: ignore[attr-defined]
    ):
        if is_float_dtype(dtype):
            uninitialized_val = float("nan")
        elif is_integer_dtype(dtype):
            uninitialized_val = smith.iinfo(dtype).max
        else:
    ...
    def inner_fn(idx):
        ...
        flat_index_expr = ops.index_expr(flat_index, smith.int64)
        limit = ops.index_expr(old_numel, smith.int64)
...
@register_lowering(smith.ops.higher_order.cond, type_promotion_kind=None)
def cond(
    ...
    if any(isinstance(x, IRNode) and is_triton(x) for x in [pred, *operands]):
        msg = "control flow operator: smith.cond."
...
@register_lowering(smith.ops.higher_order.while_loop, type_promotion_kind=None)
def while_loop(cond_fn, body_fn, carried_inputs, additional_inputs, stack_output=False):
    ...
    if not config.graph_partition and any(
        ...
        msg = "control flow operator: smith.while_loop."
        if stack_trace := V.graph.current_node.meta.get("stack_trace", None):
            msg = f"{msg} Found from : \n {stack_trace}"
        V.graph.disable_cudagraphs_reason = msg
...
register_lowering(
    smith.ops.higher_order.while_loop_stack_output, type_promotion_kind=None
)(functools.partial(while_loop, stack_output=True))


@register_lowering(smith.ops.higher_order.invoke_subgraph, type_promotion_kind=None)
...
def process_subgraph_nodes(graph_module: smith.fx.GraphModule, args: list[Any]):
    ...
    for i, node in enumerate(graph_module.graph.nodes):
        if node.op == "placeholder":
            ...
            output = smith.fx.Interpreter.output(V.graph, node, output_args, kwargs)
        else:
            assert node not in V.graph.env
...
from smith._inductor.fx_passes.control_dependencies import control_deps
...
@register_lowering(smith._higher_order_ops.invoke_quant, type_promotion_kind=None)
def invoke_quant_tracer(subgraph_fn: ir.Subgraph, *operands, scheme=None):
    ...
    for i, node in enumerate(subgraph_fn.graph_module.graph.nodes):
        ...
        # todo getattr
            ...
            output = smith.fx.Interpreter.output(V.graph, node, args, kwargs)
        else:
            V.graph.env[node] = V.graph.run_node(node)
...
def associative_scan(
    combine_fn: ir.Subgraph, xs, additional_inputs: tuple[smith.Tensor]
):
...
@register_lowering(smith.ops.prims._sink_tokens.default)
def _sink_tokens(tokens):
    return None


@register_lowering(smith.ops.prims._make_token.default)
def _make_token():
    return None


@register_lowering(smith.ops.higher_order.with_effects, type_promotion_kind=None)
def with_effects(token, op, *args, **kwargs):
    ...
    """
    from smith._higher_order_ops.effects import _get_effect, _get_schema

    # Get effect type
    effect_type = _get_effect(op)
    if effect_type is None and op is smith.ops.higher_order.invoke_subgraph:
        from smith._guards import InvokeSubgraphCache, TracingContext

        tracing_ctx = TracingContext.try_get()
        if tracing_ctx:
            invoke_subgraph_cache = tracing_ctx.hop_dispatch_set_cache.get_cache(
                smith.ops.higher_order.invoke_subgraph
            )
    ...
    try:
        ...
        def convert_ir_to_value(a):
            if isinstance(a, ir.SmithBindObject):
                return a.get_value()
...
def prepare_softmax_online(x, dim):
    ...
    if num_split == 1 and V.graph.sizevars.statically_known_geq(
        ...
        warnings.warn(
            ...
            split the reduction. Cut an issue to Blacksmith if this is an
...
def _is_sm100_or_later():
    ...
    return smith.cuda.is_available() and smith.cuda.get_device_capability() >= (10, 0)
...
def cvt_e8m0_rceil_lowering(inp):
    ...
    dtype = inp.get_dtype()
    if dtype not in (smith.float32, smith.float16, smith.bfloat16):
        raise ValueError(
    ...
    if dtype != smith.float32:
        inp = to_dtype(inp, smith.float32)
    ...
    return to_dtype(result, smith.uint8)
...
def force_fallback(op: smith._ops.OpOverload):
    """
    A context manager to force fallback an op. Used in unit test
    for FallbackKernel.
    """
    assert isinstance(op, smith._ops.OpOverload), (
