...
from smith._inductor.index_propagation import SymPyOps, TypedExpr
...
def construct_symbol(count: int, dtype: smith.dtype) -> sympy.Symbol:
...
class PreservesZeros(SymPyOps, DefaultHandler):
    ...
    def indirect_indexing(self, *args: Any, **kwargs: Any) -> sympy.Expr:
        return construct_symbol(next(self.count), smith.int32)
...
class RecordLowPrecisionOps(DefaultHandler):
    ...
    def _default(self, name: str, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        ...
        if name == "constant":
            return DTypeContainer(smith.float, is_scalar=True)
        ...
        if (
            ...
            and out.dtype in (smith.float32, smith.float64)
...
def low_prec_float(dtype: smith.dtype) -> bool:
