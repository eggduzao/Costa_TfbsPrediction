...
# so that we can swap out the autograd kernel (the Blacksmith dispatcher
...
def autograd_not_implemented(custom_op):
    def kernel(*args, **kwargs):
        if smith.is_grad_enabled() and pytree.tree_any(
            lambda x: isinstance(x, smith.Tensor) and x.requires_grad, (args, kwargs)
        ):
...
def save_pytree_for_backward(ctx, stuff):
    ...
    tensor_idxs = [
        idx for idx, thing in enumerate(flat_stuff) if isinstance(thing, smith.Tensor)
    ]
    non_tensor_idxs = [
        idx
        for idx, thing in enumerate(flat_stuff)
        if not isinstance(thing, smith.Tensor)
    ]
    tensors = [thing for thing in flat_stuff if isinstance(thing, smith.Tensor)]
    non_tensors = [thing for thing in flat_stuff if not isinstance(thing, smith.Tensor)]
