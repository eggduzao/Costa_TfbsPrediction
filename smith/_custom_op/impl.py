...
import smith._library.infer_schema
import smith.library as library
from smith._library.infer_schema import infer_schema
...
"""
smith._custom_op is deprecated. We shipped a production-ready version of it into smith.library.
...
# Blacksmith internals to avoid confusion.
RESERVED_NS = {
    "prim",
    "prims",
    "aten",
...
def warn_deprecated():
    warnings.warn(
        "smith._custom_op is deprecated and will be removed in Blacksmith 2.6, please "
        "use the equivalent smith.library API instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def custom_op(
    ...
    r"""
    This API is deprecated, please use smith.library.custom_op instead
    """
    warn_deprecated()
...
class CustomOp:
    ...
    # Note that this doesn't cause smith.library to use the impl, that
    ...
    def _destroy(self):
        ...
        opnamespace = getattr(smith.ops, self._cpp_ns)
    ...
    def __call__(self, *args, **kwargs):
        # Bypass smith.ops.* and directly do OperatorHandle::callBoxed.
        # Using smith.ops.* is a bit of a pain (it can be slow and it has lifetime
    ...
    def _check_doesnt_have_library_impl(self, device_type):
        ...
        if _C._dispatch_has_computed_kernel_for_dispatch_key(self._qualname, key):
            raise RuntimeError(
                ...
                f"pre-existing smith.library or SMITH_LIBRARY registration."
            )
    ...
    def impl_abstract(self, _stacklevel=2) -> typing.Callable:
        ...
        def inner(f):
            ...
            def f_with_ctx(*args, **kwargs):
                ...
                with smith._library.fake_impl.set_ctx_getter(error_on_ctx):
    ...
    def _check_can_register_backward(self):
        def error(detail):
            raise RuntimeError(
                f"Cannot use smith._custom_ops APIs to register backward "
        ...
        for key in ["Autograd", "AutogradCPU", "AutogradCUDA"]:
            if _C._dispatch_has_kernel_for_dispatch_key(self._qualname, key):
                raise RuntimeError(
                    ...
                    f"smith.library or SMITH_LIBRARY registration. Please either "
                    f"remove those registrations or don't use the smith._custom_ops APIs"
                )
...
def validate_schema(schema: FunctionSchema) -> None:
    if not smith._library.utils.is_functional_schema(schema):
        raise ValueError(
...
def parse_qualname(qualname: str) -> tuple[str, str]:
    ...
    if "." in names[1]:
        raise ValueError(
            f"The smith.custom_ops APIs do not handle overloads, "
...
def custom_op_from_existing(op):
    ns = op.namespace
    lib = smith.library.Library(ns, "FRAGMENT")
...
def get_op(qualname):
    def error_not_found():
        raise ValueError(
            ...
            f"loaded it via smith.ops.load_library."
        )

    ns, name = parse_qualname(qualname)
    if not hasattr(smith.ops, ns):
        error_not_found()
    opnamespace = getattr(smith.ops, ns)
...
def get_abstract_impl(qualname):
    if qualname not in smith._custom_op.impl.global_registry:
        return None
    custom_op = smith._custom_op.impl.global_registry[qualname]
...
def _custom_op_with_schema(qualname, schema, needs_fixed_stride_order=True):
    ...
    function_schema = FunctionSchema.parse(schema_str)
    validate_schema(function_schema)
    tags = [smith._C.Tag.needs_fixed_stride_order] if needs_fixed_stride_order else []
    lib = library.Library(ns, "FRAGMENT")
