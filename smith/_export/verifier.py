...
def _check_has_fake_tensor(node: smith.fx.Node) -> None:
    # TODO(angelayi): remove this in favor of _check_val
    return _check_val(node)


def _check_val(node: smith.fx.Node) -> None:
    from smith.fx.experimental.symbolic_shapes import SymBool, SymFloat, SymInt
    ...
    def _check_correct_val(val):
        if val is None:
            ...
            val, (smith.memory_format, smith.dtype, smith.device, smith.layout)
        ):
            return True
        elif isinstance(
            val, (FakeTensor, smith.Tensor)
...
def _check_smith_fn(node: smith.fx.Node) -> None:
    smith_fn = node.meta.get("smith_fn")
    if smith_fn is None:
        raise SpecViolationError(
            f"Unable to find smith_fn metadata for node {node.name}"
        )
    if (
        not isinstance(smith_fn, tuple)
        and isinstance(smith_fn[0], str)
        and isinstance(smith_fn[1], str)
    ):
        raise SpecViolationError(
            f"Node.meta {node.name} has invalid smith_fn field {smith_fn}"
        )
...
class Verifier(metaclass=_VerifierMeta):
    ...
    def allowed_getattr_types(self) -> tuple[type[Any], ...]:
        return (smith.fx.GraphModule, smith.utils._pytree.TreeSpec)
    ...
    def allowed_getattr_types_for_subgm(self) -> tuple[type[Any], ...]:
        ...
        return (
            ...
            smith.Tensor,  # for buffer and constant tensor
            smith.utils._pytree.TreeSpec,
        )
    ...
    @final
    def _check_graph_module(self, gm: smith.fx.GraphModule) -> None:
        ...
        def _check_valid_op(op) -> None:
            ...
            _allowed_smith_functions = (
                ...
                smith.sym_sqrt,
                smith.sym_sum,
                smith.export.custom_ops._call_custom_autograd_function_in_pre_dispatch,
                # TODO (tmanlaibaatar)
                ...
                smith.fx.experimental.symbolic_shapes.cast_symbool_to_symint_guardless,
        ...
        for mod in gm.modules():
            ...
            for node in mod.graph.nodes:
                ...
                if node.op in {"call_module", "call_method"}:
                    ...
                    if isinstance(attr, smith.nn.Module):
...
def _verify_exported_program_signature(exported_program) -> None:
    ...
    for input_spec, node in zip(gs.input_specs, input_node_names):
        ...
        if input_spec.kind == InputKind.USER_INPUT:
            ...
            if not isinstance(exported_program.state_dict[param], smith.nn.Parameter):
                raise SpecViolationError(
                    f"State dict entry for parameter {param} is not an instance of smith.nn.Parameter."
                )
    ...
    output_nodes = [
        arg.name if isinstance(arg, smith.fx.Node) else arg
