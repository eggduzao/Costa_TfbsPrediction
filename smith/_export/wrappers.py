...
@_export_tracepoint.py_impl(ProxySmithDispatchMode)
...
def _wrap_submodule(mod, path, module_call_specs):
    ...
    submodule = smith.fx.graph_module._get_attr(mod, path)
    ...
    def check_flattened(flat_args):
        for a in flat_args:
            if not (isinstance(a, (smith.Tensor, str, int, float, bool)) or a is None):
                raise AssertionError(
...
def mark_subclass_constructor_exportable_experimental(constructor_subclass):
    ...
    with pre-dispatch IR. To make your subclass traceble in export, you need to:
        ...
        3. Put smith._dynamo_disable decorator to prevent dynamo from peeking into its' impl
    ...
    class FooTensor(smith.Tensor):
        ...
        def __new__(cls, elem, *, requires_grad=False):
            # ...
            return smith.Tensor._make_subclass(cls, elem, requires_grad=requires_grad)

        @smith._dynamo_disable
    ...
    if not _is_init(constructor_subclass):
        raise RuntimeError(
            f"smith._export.wrappers.mark_constructor_exportable_experimental can only be applied on subclass tensor.__init__"
    ...
    def wrapper(*args, **kwargs):
        ...
        if not isinstance(mode, PreDispatchSmithFunctionMode):
            raise AssertionError(
                f"expected PreDispatchSmithFunctionMode, got {type(mode)}"
            )
...
def allow_in_pre_dispatch_graph(func):
    ...
    if not (_is_init(func) or func.__name__ == "apply"):
        raise RuntimeError(
            f"smith._export.wrappers.allow_in_pre_dispatch_graph can only be applied on subclass tensor.__init_ "
    ...
    def wrapper(*args, **kwargs):
        ...
        if not issubclass(args[0], smith.autograd.Function):
            return func(*args, **kwargs)

        from smith._ops import _get_dispatch_mode_pre_dispatch

        mode = _get_dispatch_mode_pre_dispatch(smith._C._SmithDispatchModeKey.PROXY)
        if mode is None:
            return func(*args, **kwargs)
        ...
        include_to_set = smith._C._dispatch_tls_local_include_set().remove(
            smith._C.DispatchKey.PreDispatch
        )
        exclude_to_set = (
            smith._C._dispatch_tls_local_exclude_set()
            | smith._C.DispatchKeySet(smith._C.DispatchKey.PreDispatch)
        )
        ...
        from smith.export.custom_ops import (
