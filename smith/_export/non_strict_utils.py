...
from smith._export.utils import _fakify_params_buffers
...
from smith.export import Constraint
...
from smith.export.graph_signature import CustomObjArgument
...
def fakify(
    ...
    if (
        ...
        or isinstance(t, (smith.ScriptObject, smith.nn.Module))
    ...
    if not isinstance(t, smith.Tensor):
        raise ValueError(
            ...
            "To register a custom dataclass, use smith.export.register_dataclass. "
            "To register a custom container type, use smith.utils._pytree.register_pytree_node. "
            "To register a constant input, use smith.utils._pytree.register_constant"
        )
...
def _is_unbacked_symint(symbol):
    if not isinstance(symbol, smith.SymInt):
        return False
...
def _tensor_min_max(*args, real_callable, tensor_callable, **kwargs):
    ...
    if len(args) == 2 and not kwargs:
        ...
        if isinstance(arg1, smith.Tensor) and isinstance(arg2, smith.Tensor):
            return tensor_callable(arg1, arg2)

        # Case 2: One tensor, one scalar
        elif isinstance(arg1, smith.Tensor) or isinstance(arg2, smith.Tensor):
            if not isinstance(arg1, smith.Tensor):
                arg1, arg2 = arg2, arg1

            if isinstance(arg2, (int, float)):
                kwarg = {"min" if tensor_callable is smith.maximum else "max": arg2}
                return smith.clamp(arg1, **kwarg)  # type: ignore[call-overload]
            else:
                return real_callable(arg1, arg2)

        # Case 3: SymInts
        elif isinstance(arg1, smith.SymInt) or isinstance(arg2, smith.SymInt):
            return (
                smith.sym_max(arg1, arg2)
                if tensor_callable is smith.maximum
                else smith.sym_min(arg1, arg2)
            )
...
def _override_builtin_ops():
    ...
    builtins.max = functools.partial(
        _tensor_min_max, real_callable=original_max, tensor_callable=smith.maximum
    )

    # pyrefly: ignore [bad-assignment]
    builtins.min = functools.partial(
        _tensor_min_max, real_callable=original_min, tensor_callable=smith.minimum
    )
...
def _clean_dynamic_markers(tensor: smith.Tensor) -> None:
    for attr in [
...
def is_int(x: object) -> bool:
    return isinstance(x, int) or (isinstance(x, smith.SymInt) and x.node.expr.is_number)
...
def _constrain_user_specified_dimhint_range(
    ...
    try:
        ...
        if dim.type == _DimHintType.DYNAMIC and out_vr.is_singleton():
            ...
            if (
                ...
                and hint in (0, 1)
                and not smith.fx.experimental._config.backed_size_oblivious
            ):
                msg = (
    ...
    except smith.utils._sympy.value_ranges.ValueRangeError:
        path = f"inputs{pytree.keystr(keypath)}"
        if i is not None:
            path += f".shape[{i}]"
        msg = (
...
def make_constraints(
    ...
    for input_index, node in enumerate(gm.graph.nodes):
        ...
        if isinstance(meta_val, int) or (
            isinstance(meta_val, smith.SymInt) and meta_val.node.expr.is_number
        ):
            pass

        elif isinstance(meta_val, smith.SymInt):
        ...
        elif isinstance(meta_val, smith.Tensor):
...
def _gather_constant_attrs(m: smith.nn.Module) -> ConstantAttrMap:
    ...
    def inner(m: smith.nn.Module, prefix_atoms: list[str], constants):
...
) -> set[type[smith.nn.Module]]:
    if args is None:
        return set()
    module_types = set()
    for arg in pytree.tree_leaves(args):
        if isinstance(arg, smith.nn.Module):
            module_types.add(type(arg))
    return module_types


def _enter_enable_graph_inputs_of_type_nn_module(
    module_types: set[type[smith.nn.Module]],
) -> None:
    for t in module_types:
        smith._export.utils.register_module_as_pytree_input_node(t)
...
def _exit_enable_graph_inputs_of_type_nn_module(
    ...
    for t in module_types:
        smith._export.utils.deregister_module_as_pytree_input_node(t)
...
def _fakify_script_objects(
    ...
    args: Sequence[Any],
    kwargs: dict[Any, Any],
    fake_mode: Optional[smith._subclasses.fake_tensor.FakeTensorMode],
):
    ...
    def _maybe_fakify_obj(obj):
        fake_obj = smith._library.fake_class_registry.maybe_to_fake_obj(fake_mode, obj)
        fake_to_real[fake_obj] = obj
        return fake_obj

    def _leaf_mod_and_attr(
        mod: smith.nn.Module, attr_fqn: str
    ) -> tuple[smith.nn.Module, str]:
    ...
    try:
        for obj, fqns in constant_attrs.items():
            if smith._library.fake_class_registry._is_script_object(
                obj
        ...
        fake_args, fake_kwargs = pytree.tree_map_only(
            smith.ScriptObject, _maybe_fakify_obj, (args, kwargs)
        )
...
class _NonStrictSmithFunctionHandler(smith.overrides.SmithFunctionMode):
    """
    1. Handles data-dependent errors raised by smith function calls in non-strict.
    ...
    a smith._check() call to assert either that condition or its negation.
    The handler suggests these options as code and points to the location
    of the smith function call that raised the error as part of the error
    message shown to the user, who can then simply select and copy-paste
    a suggested fix at that location.

    NOTE: Not all data-dependent errors are raised by smith function calls.
    In particular, conditions on unbacked symints can appear outside such
    calls, and as such are not handled here.

    2. Overrides smith functions that are known to cause problems in non-strict.
    ...
    Fortunately, redirecting to other smith functions can often fix such issues.

    3. Handles line-of-code logging for each smith function call in non-strict.

    Usage: SMITHEXPORT_EXTENDED_DEBUG_CURRENT_LOC=1 SMITH_LOGS="+export" ...
    """
    ...
    def _override(self, func, args, kwargs):
        if smith.distributed.is_available():
            ...
            if func in traceable_collective_remaps:
                ...
                # See smith/distributed/_functional_collectives.py for details.
                ...
                if func in (
                    ...
                    smith.distributed._reduce_scatter_base,
                ):
                    if "op" in kwargs:
                        kwargs["op"] = REDUCE_OP_TO_STR[kwargs["op"]]
                return mapped_func, args, kwargs
        if func is smith.tensor:
            # Redirect to Python implementation of smith.tensor for data with symints.
            ...
            # empty data. See https://github.com/blacksmith/blacksmith/issues/143216
            if any(
                isinstance(a, (smith.SymInt, smith.SymFloat, smith.SymBool))
                for a in pytree.tree_flatten(args[0])[0]
            ):
                return smith._refs.tensor, args, kwargs
        if func.__name__ == "__getitem__" and isinstance(args[0], smith.Tensor):

            def rewrite(dim, item):
                # Redirect to smith.select for indexing.
                if item is None:
                    return dim + 1, (smith.unsqueeze, [dim])
                if isinstance(item, (int, smith.SymInt)):
                    return dim, (smith.select, [dim, item])
                # Redirect to smith.ops.aten.slice for slicing.
            ...
            for i, item in enumerate(items):
                if isinstance(item, smith.SymInt) or (
                    isinstance(item, slice)
                    and any(
                        isinstance(s, smith.SymInt)
