...
class ConstantFolder(smith.fx.Interpreter):
    ...
    def is_impure(self, node: smith.fx.Node) -> bool:
        if (
            node.target is smith.ops.prims.convert_element_type.default
    ...
    def run_node(self, node):
        ...
        if node.op != "get_attr" and isinstance(out, smith.Tensor):
...
def constant_graph_tag(gm: smith.fx.GraphModule) -> None:
    with smith.utils._python_dispatch._disable_current_modes():
        cf = ConstantFolder(gm, skip_constructors=True)
        cf.run()
...
def run_and_get_constant_graph(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
