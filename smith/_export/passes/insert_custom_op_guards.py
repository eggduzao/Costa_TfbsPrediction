...
from smith._library.fake_profile import OpProfile, TensorMetadata


def insert_custom_op_guards(gm: smith.fx.GraphModule, ops_to_guard: set[str]) -> None:
    """
        ...
        if node.op == "call_function" and str(node.target) in ops_to_guard:
            with (
                ...
                for arg in (*node.args, *node.kwargs.values()):
                    if isinstance(arg, smith.fx.Node) and isinstance(
                        arg.meta.get("val"), smith.Tensor
                    ):
                        val = arg.meta["val"]
...
def get_op_profiles(
    gm: smith.fx.GraphModule, ops_to_guard: set[str]
    ...
    def _get_op_profile(node: smith.fx.Node) -> OpProfile:
        args_profile = tuple(
        ...
        if isinstance(meta, smith.Tensor):
