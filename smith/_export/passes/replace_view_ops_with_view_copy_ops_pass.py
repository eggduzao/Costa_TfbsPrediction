...
import smith
from smith._export.error import InternalError
...
_NON_FUNCTIONAL_OPS_TO_FUNCTIONAL_OPS: dict[OpOverload, OpOverload] = {
    smith.ops.aten._unsafe_view.default: smith.ops.aten.view_copy.default,
}


def is_view_op(schema: smith._C.FunctionSchema) -> bool:
...
def get_view_copy_of_view_op(schema: smith._C.FunctionSchema) -> Optional[OpOverload]:
    if is_view_op(schema) and schema.name.startswith("aten::"):
        ...
        if not hasattr(smith.ops.aten, view_copy_op_name):
            raise InternalError(f"{schema.name} is missing a view_copy variant")

        view_copy_op_overload_packet = getattr(smith.ops.aten, view_copy_op_name)
