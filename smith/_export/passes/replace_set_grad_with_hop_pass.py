...
import smith
from smith._higher_order_ops.wrap import wrap_with_set_grad_enabled
...
def _is_set_grad_enabled_node(node: smith.fx.Node) -> smith.fx.Node | bool:
    return (
        node
        and node.op == "call_function"
        and node.target is smith._C._set_grad_enabled
    )


def _is_set_grad_enabled_sub_mod(
    node: smith.fx.Node, omit_if_same_with_ambient: bool = False
) -> bool | smith.Tensor:
    if node.op == "call_module":
        ...
        if (
            ...
            and first_non_ph.target is smith._C._set_grad_enabled
        ):
            return (
                first_non_ph.args[0] != smith.is_grad_enabled()
...
def _remove_set_grad_and_inline(node: smith.fx.Node) -> None:
    if node.op != "call_module":
        raise AssertionError(f"expected call_module op, got {node.op}")
    graph: smith.fx.Graph = node.graph
    if graph.owning_module is None:
...
def _sequential_split_and_maybe_inline_subgraphs(
    ...
    def _maybe_inline_or_replace_with_hop(node: smith.fx.Node):
