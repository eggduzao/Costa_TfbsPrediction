...
def _replace_with_hop_helper(
    node: smith.fx.Node,
    enter_block_node: smith.fx.Node,
    ...
    def set_hoo_node_meta(call_func_node):
        ...
        elif isinstance(output_args, smith.fx.Node):
    ...
    with graph.inserting_before(node):
        ...
        # for split_module
        if isinstance(output_node, smith.fx.Node) and output_node.op != "output":
            output_node = None
...
def _sequential_split_and_maybe_inline_subgraphs_helper(
    ...
    maybe_inline_or_replace_with_hop: Callable[[smith.fx.Node], None],
    ...
    if graph_signature is not None:
        ...
        for arg_node, out_spec in zip(
            ...
            if arg_node is None:
                ...
                isinstance(arg_node, smith.fx.Node)
...
def _replace_with_hop_pass_helper(
    ...
    sequential_split_and_maybe_inline_subgraphs: Callable[
        [smith.fx.GraphModule, ExportGraphSignature | None],
        tuple[smith.fx.GraphModule, ExportGraphSignature | None],
    ],
) -> tuple[smith.fx.GraphModule, ExportGraphSignature | None]:
    """
    ...
    for node in new_gm.graph.nodes:
        if node.op == "get_attr":
            ...
            if not isinstance(subgm, smith.fx.GraphModule):
                continue
