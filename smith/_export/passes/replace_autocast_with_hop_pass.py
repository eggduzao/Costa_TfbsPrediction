...
import smith
from smith._higher_order_ops.wrap import wrap_with_autocast
...
def _is_autocast_node(node: smith.fx.Node) -> smith.fx.Node | bool:
    return (
        node
...
def _is_enter_autocast_node(node: smith.fx.Node) -> smith.fx.Node | bool:
    return (
        node
        and node.op == "call_function"
        and node.target is smith.amp.autocast_mode._enter_autocast
    )


def _is_exit_autocast_node(node: smith.fx.Node) -> smith.fx.Node | bool:
    return (
        node
        and node.op == "call_function"
        and node.target is smith.amp.autocast_mode._exit_autocast
    )


def _is_autocast_sub_mod(node: smith.fx.Node) -> bool:
    """
    Check if the first non-placeholder node is `smith.amp.autocast_mode._enter_autocast`.
    """
    if node.op == "call_module":
        ...
        if (
            ...
            and first_non_ph.target is smith.amp.autocast_mode._enter_autocast
        ):
...
def _check_valid_autocast_block(
    enter_autocast_node: smith.fx.Node, exit_autocast_node: smith.fx.Node
) -> None:
...
def _split_autocast(gm: smith.fx.GraphModule) -> smith.fx.GraphModule:
    """
    ...
    E               # 4
    """
    enter_autocast_node_stack: list[smith.fx.Node] = []
    first_node_after_outer_most_exit: bool = False

    def node_call_back(node: smith.fx.Node) -> bool:
...
def _sequential_split_and_maybe_inline_subgraphs(
    gm: smith.fx.GraphModule, graph_signature: ExportGraphSignature | None
) -> tuple[smith.fx.GraphModule, ExportGraphSignature | None]:
    """
    ...
    def _maybe_inline_or_replace_with_hop(node: smith.fx.Node) -> None:
