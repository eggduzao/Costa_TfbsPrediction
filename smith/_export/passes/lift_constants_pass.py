...
class ConstantAttrMap(collections.abc.MutableMapping):
    ...
    but ScriptObjects are stored by hash, because different smith.ScriptObjects can point to
    ...
    def __init__(self) -> None:
        ...
        self._constant_attrs: dict[
            Union[int, smith.Tensor, FakeScriptObject, smith.utils._pytree.TreeSpec],
            list[Any],
        ] = {}
        ...
        self._script_object_map: dict[int, smith.ScriptObject] = {}

    def __getitem__(self, key: _ConstantAttributeType) -> Any:
        real_key = hash(key) if isinstance(key, smith.ScriptObject) else key
        if not isinstance(real_key, (int, smith.Tensor, FakeScriptObject)):
...
The same key can be mapped to multiple values, for handling constant aliasing."""
    ...
    def add(self, key: _ConstantAttributeType, value: Any) -> None:
        if isinstance(key, smith.ScriptObject):
        ...
        elif isinstance(key, (smith.Tensor, FakeScriptObject)):
...
def get_constant_fqn(node: smith.fx.Node, constant_name: str) -> str:
...
def _unused_constant(node: smith.fx.Node) -> Optional[list[smith.fx.Node]]:
    """
    ...
    looks like:
        ...
        %lift_fresh_copy : [num_users=1] = call_function[target=smith.ops.aten.lift_fresh_copy.default](args = (%_tensor_constant0,))
        %detach_ : [num_users=?] = call_function[target=smith.ops.aten.detach_.default](args = (%lift_fresh_copy,))
    ...
    if not (
        ...
        in (
            smith.ops.aten.detach_.default,
...
def lift_constants_pass(
    ...
    Arguments:
        gm (smith.fx.GraphModule): The graph module containing the graph and constants to lift.
    ...
    for node in list(gm.graph.nodes):
        if node.op == "get_attr":
            ...
            # constant (e.g. x + smith.tensor(0)), and thus did not have a
            ...
            if isinstance(
                constant_val, (smith.ScriptObject, FakeScriptObject)
                ...
                if constant_fqn is not None:
                    ...
                    while constant_fqn in used_target_names:
                        ...
                        constant_fqn = get_constant_fqn(node, constant_name)
                    num_custom_obj += 1
            elif isinstance(constant_val, smith.Tensor):
                # Remove the parameterness of constant_val
                if isinstance(constant_val, smith.nn.Parameter):
                    log.debug(
            ...
            with gm.graph.inserting_before(first_user_input):
                ...
                if isinstance(constant_val, smith.Tensor):
                ...
                elif isinstance(constant_val, smith._C.ScriptObject):
...
def rewrite_script_object_meta(
    ...
    for node in gm.graph.nodes:
        ...
        if isinstance(old_meta, smith.ScriptObject):
