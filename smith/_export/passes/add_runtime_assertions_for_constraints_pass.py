    ...
    def _insert_assert_async(self, last_node, op, lower, upper, assert_msg):
        ...
        with graph.inserting_after(cmp):
            cmp_tensor = graph.call_function(
                smith.ops.aten.scalar_tensor.default, (cmp,), {}
            )
...
def _get_existing_inline_assertions(
    ...
    for module in graph_module.modules():
        ...
        # %_local_scalar_dense = call_function[target=smith.ops.aten._local_scalar_dense.default](args = (%arg1_1,), kwargs = {})
        # %ge = call_function[target=operator.ge](args = (%_local_scalar_dense, 0), kwargs = {})
        # %_assert_scalar = call_function[target=smith.ops.aten._assert_scalar.default](args = (%scalar_tensor, "..."), kwargs = {})
        for node in module.graph.nodes:
            if node.target != smith.ops.aten._assert_scalar.default:
                continue

            compare_arg = node.args[0]
            if not (
                isinstance(compare_arg, smith.fx.Node)
            ...
            def maybe_get_symint(x):
                if (
                    isinstance(x, smith.fx.Node)
                    and "val" in x.meta
                    and isinstance(x.meta["val"], smith.SymInt)
                ):
