...
import smith
from smith.export.exported_program import ConstantArgument, TensorArgument
from smith.fx.passes.infra.pass_base import PassBase, PassResult


if TYPE_CHECKING:
    from smith.export.exported_program import ModuleCallSignature
...
class CollectTracepointsPass(PassBase):
    ...
    def call(self, gm: smith.fx.GraphModule) -> PassResult | None:
        def get_arg_spec(arg) -> TensorArgument | ConstantArgument:
            if isinstance(arg, smith.fx.Node):
                if isinstance(arg.meta.get("val"), smith.Tensor):
