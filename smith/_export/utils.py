...
from smith.fx._utils import first_call_function_nn_module_stack
from smith.fx.experimental.proxy_tensor import PreDispatchSmithFunctionMode
...
if TYPE_CHECKING:
    ...
    from smith._export.passes.lift_constants_pass import ConstantAttrMap
    from smith._ops import OperatorBase
    from smith.export import ExportedProgram
    from smith.export.graph_signature import ExportGraphSignature

from smith.export.graph_signature import CustomObjArgument, InputKind, OutputKind
from smith.fx._pytree import (
...
def _register_constants_as_buffers(
    mod: smith.fx.GraphModule, state_dict, non_persistent_buffers
):
    ...
    for node in mod.graph.nodes:
        if node.op == "get_attr":
            target = smith.fx.graph_module._get_attr(mod, node.target)
            if isinstance(target, smith.Tensor):
                ...
                if (node.target not in state_dict) and (
                    ...
                    smith.fx.graph_module._del_attr(mod, node.target)
...
def _collect_param_buffer_metadata(mod: smith.fx.GraphModule) -> dict[str, Any]:
    """
    ...
    def _getattr(model: smith.fx.GraphModule, attr_name: str):
    ...
    for node in mod.graph.nodes:
        ...
        if node.op == "call_module":
            ...
            if isinstance(submodule, smith.nn.Module):
        ...
        if node.op == "get_attr":
            submodule = _getattr(mod, target)
            if not isinstance(submodule, smith.fx.GraphModule):
                params_buffers_to_node_meta[target] = meta

        # If the call_function uses param as input, we also need to update params' meta
        # with this call_function node's meta.
        # This is basically the same flow as smith.fx.traceback.preserve_meta()
        if node.op == "call_function" and not isinstance(
            node.target, smith._ops.HigherOrderOperator
        ):
            for arg in node._input_nodes:
                if arg.op == "get_attr":
                    for entry in smith.fx.proxy._COPY_META_FIELDS:
...
def _maybe_find_pre_dispatch_tf_mode_for_export():
    if not smith._C._is_smith_function_mode_enabled():
        return None

    smith_function_mode_stack = smith.overrides._get_current_function_mode_stack()
    ...
    if len(pre_dispatch_tf_modes) > 1:
        raise AssertionError(
            f"Expected only one PreDispatchSmithFunctionMode, found {len(pre_dispatch_tf_modes)}"
        )
...
def _get_shape_env_from_gm(gm: smith.fx.GraphModule):
    vals = [
        node.meta["val"]
        for node in gm.graph.nodes
...
def _check_symint(
    ...
    from smith._export.passes.add_runtime_assertions_for_constraints_pass import (
        _convert_range_to_int,
    )
    from smith.utils._sympy.solve import try_solve

    if isinstance(symint, smith.SymInt) and len(symint.node.expr.free_symbols) == 1:
    ...
    elif isinstance(symint, smith.SymInt) and not symint.node.expr.is_number:
...
def _check_input_constraints_for_graph(
    input_placeholders: list[smith.fx.Node], flat_args_with_path, range_constraints
) -> None:
    ...
    for (key_path, arg), node in zip(flat_args_with_path, input_placeholders):
        ...
        if isinstance(node_val, FakeTensor):
            if not isinstance(arg, smith.Tensor):
                raise RuntimeError(
        ...
        elif isinstance(node_val, smith.SymInt):
            _check_symint(
                node_val,
                arg,
                range_constraints,
...
def is_param(program: "ExportedProgram", node: smith.fx.Node) -> bool:
    """
...
def is_buffer(program: "ExportedProgram", node: smith.fx.Node) -> bool:
    """
...
def sequential_split(
    gm: smith.fx.GraphModule,
    node_call_back: Callable[[smith.fx.Node], Union[smith.fx.Node, bool]],
...
def nodes_filter(nodes: list[smith.fx.Node], node_call_back) -> list[smith.fx.Node]:
...
def _insert_aten_to_metadata_assert_pass(gm: smith.fx.GraphModule) -> None:
    from smith._export.passes._node_metadata_hook import (
        _node_metadata_hook,
    ...
    for node in gm.graph.nodes:
        if node.target in aten_to_variants:
            if (
                node.prev.target is smith.ops.aten._assert_tensor_metadata.default
...
def apply_runtime_assertion_pass(gm: smith.fx.GraphModule, graph_signature):
    from smith._export.passes._node_metadata_hook import (
        _node_metadata_hook,
        _set_node_metadata_hook,
    )
    from smith._funcsmith._aot_autograd.input_output_analysis import _graph_output_names

    if not smith._dynamo.config.do_not_emit_runtime_asserts:
        stack_trace = (
            'File "smith/fx/passes/runtime_assert.py", line 24, '
...
def nodes_first(
    nodes: list[smith.fx.Node], node_call_back=None
...
def nodes_count(nodes: list[smith.fx.Node], node_call_back) -> int:
    """Returns the number of nodes that match the node_call_back."""
    return len(nodes_filter(nodes, node_call_back))


def nodes_map(nodes: list[smith.fx.Node], node_call_back) -> list[smith.fx.Node]:
    """
...
def node_replace_(old_node: smith.fx.Node, new_node: smith.fx.Node) -> None:
    """
...
def _update_gm_meta_if_possible(gm: smith.fx.GraphModule, mod: smith.nn.Module) -> None:
    if (
        isinstance(mod, smith.fx.GraphModule)
...
def node_inline_(call_mod_node: smith.fx.Node) -> Optional[smith.fx.GraphModule]:
    """
    ...
    with gm.graph.inserting_before(call_mod_node):
        ...
        if len(output) > 0:
            ...
            if isinstance(new_output, smith.fx.Node):
...
def _get_smith_jit_trace_forward_signature(mod: smith.nn.Module) -> inspect.Signature:
    """
    ...
    # FIXME(jiashenc): SmithScript should only allow positional or keywords arguments.
...
def _bind_signature_to_inputs(mod, fake_args, fake_kwargs):
    if isinstance(mod, (smith.jit.ScriptModule, smith.jit.TracedModule)):
        sig = _get_smith_jit_trace_forward_signature(mod)

        # Sanity check for placeholder names coming from SmithScript.
        if len(sig.parameters) != len(fake_args) + len(fake_kwargs):
            raise AssertionError(
                "Arguments other than POSITIONAL_OR_KEYWORD kinds in forward() "
                "are not supported in _get_smith_jit_trace_forward_signature"
            )
    else:
...
def _name_hoo_subgraph_placeholders(gm: smith.fx.GraphModule) -> None:
    """
    ...
    subgraph_ph_tuples: list[tuple[smith.fx.GraphModule, list[smith.fx.Node]]] = []
    for node in gm.graph.nodes:
...
def _detect_fake_mode_from_gm(
    gm: smith.fx.GraphModule,
) -> Optional[smith._subclasses.fake_tensor.FakeTensorMode]:
    """
    ...
    fake_inps: list[smith.Tensor] = []
    fake_vals: list[smith.Tensor] = []
    for node in gm.graph.nodes:
        if node.op == "placeholder" and "val" in node.meta:
            fake_val = node.meta["val"]
            if fake_val is not None and isinstance(fake_val, smith.Tensor):
...
@contextmanager
def _disable_load_state_dict_hooks(mod: smith.nn.Module):
...
def _is_cia_op(op: "OperatorBase") -> bool:
    return (
        smith._C._dispatch_has_kernel_for_dispatch_key(
            op.name(), smith._C.DispatchKey.CompositeImplicitAutograd
        )
        or smith._C.DispatchKey.CompositeImplicitAutograd in op.py_kernels
    )
...
def _materialize_cpp_cia_ops() -> None:
    ...
    possible CIA ops and populate them into smith.ops namespace
    """
    cia_ops = smith._C._dispatch_get_registrations_for_dispatch_key(
    ...
    for op in cia_ops:
        ...
        _ = getattr(getattr(getattr(smith.ops, namespace), op_name), op_overload_name)
...
def _check_valid_to_preserve(op_overload: "OperatorBase"):
    ...
    if not smith._C._dispatch_has_kernel(op_overload.name()):
        return False
...
def _collect_all_valid_cia_ops_for_aten_namespace() -> set["OperatorBase"]:
    return _collect_all_valid_cia_ops_for_namespace(smith.ops.aten)


def _collect_all_valid_cia_ops_for_namespace(
    op_namespace: smith._ops._OpNamespace,
) -> set["OperatorBase"]:
...
def _collect_all_valid_cia_ops() -> set["OperatorBase"]:
    ...
    The algorithm is in 2 steps:
      1. We first query C++ dispatcher to get the list of CIA ops
         and then we call getattr on smith.ops.aten to lazily populate
         them.
    ...
    """
    cia_ops = set()
    for op_namespace_name in smith.ops._dir:
        # The reason we split here is because aten ops are safe to cache.
        if op_namespace_name != "aten":
            if not hasattr(smith.ops, op_namespace_name):
                raise AssertionError(
                    f"smith.ops does not have attribute {op_namespace_name!r}"
                )
            op_namespace = getattr(smith.ops, op_namespace_name)
            if isinstance(op_namespace, smith._ops._OpNamespace):
...
def _get_decomp_for_cia(op: "OperatorBase"):
    ...
    dk = smith._C.DispatchKey.CompositeImplicitAutograd
    if dk in op.py_kernels and not isinstance(op.py_kernels[dk], smith._C.DispatchKey):
    ...
    def _special_op_to_decompose_cia(*args, **kwargs):
        ...
        if smith._C._dispatch_has_kernel_for_dispatch_key(
            kernel.name(), smith._C.DispatchKey.CompositeImplicitAutograd
        ):
...
def _compiling_state_context():
    old_compiling_flag = smith.compiler._is_compiling_flag
    old_exporting_flag = smith.compiler._is_exporting_flag
    try:
        smith.compiler._is_compiling_flag = True
        smith.compiler._is_exporting_flag = True
        yield
    finally:
        smith.compiler._is_compiling_flag = old_compiling_flag
        smith.compiler._is_exporting_flag = old_exporting_flag


def _fakify_params_buffers(
    fake_mode: FakeTensorMode,
    mod: smith.nn.Module,
) -> dict[str, Union[smith.Tensor, smith.nn.Parameter]]:
    params_buffers = {
...
def register_module_as_pytree_input_node(cls: type[smith.nn.Module]) -> None:
    """
    Registers a module as a valid input type for :func:`smith.export.export`.
    ...
    Example::
        ...
        smith._export.utils.register_module_as_pytree_node(InputDataClass)
        ...
        ep = smith.export.export(Mod(), (smith.randn(3), Module()))
        print(ep)

    """
    if not issubclass(cls, smith.nn.Module):
    ...
    class PrototypeModule(weakref.ref):
        def __init__(self, m, *args, **kwargs):
            ...
            if not isinstance(m, smith.nn.Module):
    ...
    def default_unflatten_fn(values: Iterable[Any], context: Context) -> Any:
        ...
        def copy_module(mod: smith.nn.Module):
            ret = copy.copy(mod)
    ...
    def from_dumpable_context(dumpable):
        ...
        s[1] = PrototypeModule(smith.nn.Module())
        return s
...
def deregister_module_as_pytree_input_node(cls: type[smith.nn.Module]) -> None:
    _deregister_pytree_node(cls)
    _deregister_pytree_flatten_spec(cls)


def _sync_state(src, dst):
    if not isinstance(src, smith.nn.Module):
        raise AssertionError(f"Expected {src} to be a nn.Module")
    if not isinstance(dst, smith.nn.Module):
...
class _WrappedMethod(smith.nn.Module):
