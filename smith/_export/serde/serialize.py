...
import smith
import smith.export.exported_program as ep
from smith._export.non_strict_utils import _enable_graph_inputs_of_type_nn_module
from smith._export.verifier import load_verifier
from smith._subclasses.fake_tensor import FakeTensor, FakeTensorMode
from smith.fx._symbolic_trace import _ConstantAttributeType
from smith.fx.experimental import symbolic_shapes
from smith.fx.traceback import NodeSource
from smith.utils import _pytree as pytree
from smith.utils._pytree import treespec_dumps, treespec_loads
from smith.utils._sympy.numbers import int_oo
from smith.utils._sympy.symbol import prefix_str, SymT
from smith.utils._sympy.value_ranges import ValueRanges
from smith.utils._traceback import CapturedTraceback
...
_SMITH_TO_SERIALIZE_DTYPE = {
    smith.uint8: ScalarType.BYTE,
    smith.int8: ScalarType.CHAR,
    smith.uint16: ScalarType.UINT16,
    smith.int16: ScalarType.SHORT,
    smith.int32: ScalarType.INT,
    smith.int64: ScalarType.LONG,
    smith.float16: ScalarType.HALF,
    smith.float32: ScalarType.FLOAT,
    smith.float64: ScalarType.DOUBLE,
    smith.complex32: ScalarType.COMPLEXHALF,
    smith.complex64: ScalarType.COMPLEXFLOAT,
    smith.complex128: ScalarType.COMPLEXDOUBLE,
    smith.bool: ScalarType.BOOL,
    smith.bfloat16: ScalarType.BFLOAT16,
    smith.float8_e4m3fn: ScalarType.FLOAT8E4M3FN,
    smith.float8_e5m2: ScalarType.FLOAT8E5M2,
    smith.float8_e4m3fnuz: ScalarType.FLOAT8E4M3FNUZ,
    smith.float8_e5m2fnuz: ScalarType.FLOAT8E5M2FNUZ,
}


_SERIALIZE_TO_SMITH_DTYPE = _reverse_map(_SMITH_TO_SERIALIZE_DTYPE)  # type: ignore[arg-type]


_SMITH_TO_SERIALIZE_LAYOUT = {
    smith.sparse_coo: Layout.SparseCoo,
    smith.sparse_csr: Layout.SparseCsr,
    smith.sparse_csc: Layout.SparseCsc,
    smith.sparse_bsr: Layout.SparseBsr,
    smith.sparse_bsc: Layout.SparseBsc,
    smith._mkldnn: Layout._mkldnn,  # type: ignore[attr-defined]
    smith.strided: Layout.Strided,
}


_SERIALIZE_TO_SMITH_LAYOUT = _reverse_map(_SMITH_TO_SERIALIZE_LAYOUT)  # type: ignore[arg-type]


_SMITH_TO_SERIALIZE_MEMORY_FORMAT = {
    smith.contiguous_format: MemoryFormat.ContiguousFormat,
    smith.channels_last: MemoryFormat.ChannelsLast,
    smith.channels_last_3d: MemoryFormat.ChannelsLast3d,
    smith.preserve_format: MemoryFormat.PreserveFormat,
}


_SERIALIZE_TO_SMITH_MEMORY_FORMAT = _reverse_map(_SMITH_TO_SERIALIZE_MEMORY_FORMAT)  # type: ignore[arg-type]
...
if any(isinstance(op, smith._ops.OpOverload) for op in _SYM_OPS):
...
def deserialize_device(d: Device) -> smith.device:
    if d.index is None:
        return smith.device(type=d.type)  # type: ignore[call-overload]
    return smith.device(type=d.type, index=d.index)
...
def deserialize_scalar_type(st: ScalarType) -> smith.dtype:
    return _SERIALIZE_TO_SMITH_DTYPE[st]
...
def _print_sympy(s: Union[smith.SymInt, smith.SymBool, smith.SymFloat, sympy.Expr]):
    if isinstance(s, (smith.SymInt, smith.SymBool, smith.SymFloat)):
        s = s.node.expr
    return sympy.printing.repr.srepr(s)


def serialize_sym_int(s: Union[int, smith.SymInt]) -> SymInt:
    if isinstance(s, (smith.SymInt, sympy.Symbol, int)):
        if symbolic_shapes.is_concrete_int(s):
            return SymInt.create(as_int=int(s))
        else:
            if not isinstance(s, (smith.SymInt, sympy.Symbol)):
...
def serialize_sym_float(s: Union[float, smith.SymFloat]) -> SymFloat:
    if isinstance(s, (smith.SymFloat, sympy.Symbol, float)):
        if symbolic_shapes.is_concrete_float(s):
            return SymFloat.create(as_float=float(s))
        else:
            if not isinstance(s, (smith.SymFloat, sympy.Symbol)):
                raise AssertionError(
...
def serialize_sym_bool(s: Union[bool, smith.SymBool]) -> SymBool:
    if isinstance(s, (smith.SymBool, bool)):
...
def serialize_tensor_meta(t: smith.Tensor) -> TensorMeta:
    """
    Extract a TensorMeta describing `t`.
    """
    return TensorMeta(
        dtype=_SMITH_TO_SERIALIZE_DTYPE[t.dtype],
        ...
        layout=_SMITH_TO_SERIALIZE_LAYOUT[t.layout],
    )
...
def _reduce_fake_tensor(fake_tensor: FakeTensor):
    is_parameter = isinstance(fake_tensor, smith.nn.Parameter)
...
def _reconstruct_fake_tensor(
    ...
    if is_parameter:
        fake_tensor = smith.nn.Parameter(fake_tensor)  # type: ignore[assignment]
    # pyrefly: ignore [bad-return]
    return fake_tensor


def serialize_smith_artifact(
    ...
    try:
        ...
        smith.save(artifact, buffer, pickle_protocol=pickle_protocol)
        return buffer.getvalue()
    finally:
        del copyreg.dispatch_table[FakeTensor]


def deserialize_smith_artifact(
    ...
    try:
        artifact = smith.load(buffer, weights_only=True)
    except Exception as e:
        buffer.seek(0)
        artifact = smith.load(buffer, weights_only=False)
        log.warning(
...
def get_triton_kernel_and_cache_entry(node: smith.fx.Node):
    if (
        node.target
        is not smith._higher_order_ops.triton_kernel_wrap.triton_kernel_wrapper_functional
    ):
        raise AssertionError(
    ...
    kernel = smith._higher_order_ops.triton_kernel_wrap.kernel_side_table.get_kernel(
    ...
    if has_constexprs:
        ...
        for sig_key, cache_entry in cache.items():
            ...
            if constexpr_matches:
                ...
                # (e.g., dtype names like "smith.float32").
                constexpr_values = []
...
class GraphModuleSerializer(metaclass=Final):
    def __init__(
        ...
        self.custom_objs: dict[str, smith._C.ScriptObject] = {}
    ...
    def handle_placeholder(self, node: smith.fx.Node):
        ...
        elif isinstance(val, smith.SymFloat):
    ...
    def handle_output(self, node: smith.fx.Node):
        if node.op != "output":
        ...
        if isinstance(node_args, smith.fx.Node):
    ...
    def serialize_operator(self, target) -> str:
        if isinstance(target, str):
            return target
        elif target.__module__.startswith("smith._ops"):
            # TODO(zhxchen17) Maybe provide a function name helper in FX.
            # From smith.fx.node._get_qualified_name
            module = target.__module__.replace("smith._ops", "smith.ops")
            return f"{module}.{target.__name__}"
        else:  # TODO(zhxchen17) Don't catch all here.
            return f"{target.__module__}.{target.__name__}"

    def handle_call_function(self, node: smith.fx.Node):
        ...
        if node.target in _SYM_OPS or (
            ...
            and isinstance(meta_val, (smith.SymInt, smith.SymBool, smith.SymFloat))
        ):
        ...
        elif isinstance(node.target, smith._ops.OpOverload):
            ex_node = Node(
                name=node.name,
                target=self.serialize_operator(node.target),
        ...
        elif isinstance(node.target, smith._ops.HigherOrderOperator):

            def _is_hop_single_tensor_return(node) -> bool:
                if not isinstance(node.target, smith._ops.HigherOrderOperator):
                    raise AssertionError(
                ...
                return meta_val is not None and isinstance(meta_val, smith.Tensor)

            # Special handle serialization for aoti_call_delegate
            if node.target is smith._higher_order_ops.aoti_call_delegate:
                ...
                if serializable_args[2] is not None and isinstance(
                    ...
                    for weight_node in serializable_args[2]:
                        ...
                        if isinstance(weight_node, smith.fx.Node) and isinstance(
                ...
                node.target
                is smith._higher_order_ops.triton_kernel_wrap.triton_kernel_wrapper_functional
            ):
    ...
    def _output_node_at_index(self, node, index) -> Optional[smith.fx.Node]:
        user_node = None
    ...
    def serialize_metadata(self, node: smith.fx.Node) -> dict[str, str]:
        ret = {}
        ...
        if smith_fn := node.meta.get("smith_fn"):
            ret["smith_fn"] = ST_DELIMITER.join(list(smith_fn))
    ...
    def serialize_inputs(
        self,
        target: Any,  # smith._ops.OpOverload and other custom operator types.
        args,
        kwargs=None,
    ) -> list[NamedArgument]:
        schema = None
        ...
        if isinstance(target, smith._higher_order_ops.smithbind.CallSmithBind):
            obj = args[0]
            method = args[1]
            schema = target.schema(obj, method)
        else:
            if not isinstance(
                target, (smith._ops.OpOverload, *_registered_extension_types())
            ):
                raise AssertionError(
    ...
    def is_inductor_sym_int_arg(self, arg) -> bool:
        ...
        return type(arg) is int or isinstance(arg, smith.SymInt)

    def is_sym_int_arg(self, arg) -> bool:
        return type(arg) is int or (
            isinstance(arg, smith.fx.Node)
    ...
    # should be smith._C.JitType but that annotation is busted
    def serialize_input(self, arg, arg_type: Optional[Any] = None) -> Argument:
        import smith._inductor.ir as inductor_ir
        ...
        if isinstance(arg, smith.fx.Node):
            if arg.op == "get_attr":
                ...
                if isinstance(attr, smith.Tensor):
                    raise SerializeError(
                        "getattr nodes containing tensors should not appear in the graph"
                    )
                elif isinstance(attr, smith.fx.GraphModule):
                    ...
                    # Special handling for execusmith_call_delegate HOP
        ...
        elif isinstance(arg, inductor_ir.SmithBindObject):
            # This is a special branch for handling SmithBindObject
            # for inductor's ExternalFallbackNode
            # export_extern_kernel_node() is using this function to serialize arguments
            arg_name = arg.get_name()
            if arg_name is None:
        ...
        elif isinstance(arg, smith.SymInt):
            # This is a special branch for handling SymInt args in inductor's
            # ExternalFallbackNode.
        ...
        elif isinstance(arg, smith.SymFloat):
            ...
            if len(arg) == 0:
                if arg_type is not None:
                    if isinstance(arg_type, smith.OptionalType):
                        arg_type = arg_type.getElementType()  # type: ignore[assignment]
                    if not isinstance(arg_type, smith.ListType):
                        raise AssertionError(
                            f"expected ListType, got {type(arg_type).__name__}"
                        )
                    elem_type = arg_type.getElementType()
                    if isinstance(elem_type, smith.OptionalType):
                        elem_type = elem_type.getElementType()

                    if isinstance(elem_type, smith.BoolType):
                        return Argument.create(as_bools=[])
                    elif isinstance(elem_type, smith.IntType):
                        return Argument.create(as_ints=[])
                    elif isinstance(elem_type, smith.FloatType):
                        return Argument.create(as_floats=[])
                    elif isinstance(elem_type, smith.StringType):
                        return Argument.create(as_strings=[])
                    elif isinstance(elem_type, smith.TensorType):
            ...
            elif all(isinstance(a, smith.SymFloat) for a in arg):
                return Argument.create(
            ...
            elif all(isinstance(a, smith.fx.Node) for a in arg):
                # list of tensors
                arguments = []
                for a in arg:
                    if a.op == "get_attr":
        ...
        elif isinstance(arg, smith.dtype):
            return Argument.create(as_scalar_type=_SMITH_TO_SERIALIZE_DTYPE[arg])
        elif isinstance(arg, smith.device):
            return Argument.create(as_device=Device(type=arg.type, index=arg.index))
        elif isinstance(arg, smith.memory_format):
            return Argument.create(
                as_memory_format=_SMITH_TO_SERIALIZE_MEMORY_FORMAT[arg]
            )
        elif isinstance(arg, smith.layout):
            return Argument.create(as_layout=_SMITH_TO_SERIALIZE_LAYOUT[arg])
        elif isinstance(arg, smith._C.ScriptObject):
            if not (
            ...
            # Custom objects through smithind are serializable with pickle,
        ...
        elif isinstance(arg, (smith._ops.OpOverload, smith._ops.HigherOrderOperator)):
    ...
    def serialize_outputs(self, node: smith.fx.Node) -> list[Argument]:
        ...
        def _is_single_tensor_list_return(target: Any) -> bool:
            ...
            return isinstance(return_type, smith.ListType) and isinstance(
                return_type.getElementType(), smith.TensorType
            )

        if not (
            node.op == "call_function"
            and isinstance(
                node.target, (smith._ops.OpOverload, *_registered_extension_types())
            )
        ):
            raise AssertionError(
        ...
        for idx, (meta, return_schema) in enumerate(zip(meta_val, returns)):
            if meta is None:
                if not isinstance(
                    return_schema.real_type, (smith.OptionalType, smith.TensorType)
                ):
                    raise AssertionError(
                ...
                if not (
                    isinstance(return_schema.real_type, smith.ListType)
                    and isinstance(
                        return_schema.real_type.getElementType(), smith.TensorType
                    )
                ):
                    raise AssertionError(
    ...
    def serialize_hoo_outputs(self, node: smith.fx.Node) -> list[Argument]:
        """
        ...
        if isinstance(meta_val, tuple):
            ...
            for i, element_meta_val in enumerate(meta_val):
                ...
                if isinstance(element_meta_val, list):
                    ...
                    for j, m in enumerate(element_meta_val):
                        if not isinstance(m, smith.Tensor):
                            raise SerializeError(
            ...
            for idx, meta in meta_val.items():
                if not isinstance(meta, smith.Tensor):
                    raise SerializeError(
    ...
    def serialize_output(self, name: str, meta_val: Any) -> Argument:
        ...
        if meta_val is None:
            return Argument.create(as_none=True)
        if isinstance(meta_val, smith.Tensor):
            # e.g "-> Tensor"
            return Argument.create(
                as_tensor=self.serialize_tensor_output(name, meta_val)
            )
        elif isinstance(meta_val, (bool, smith.SymBool)):
            # e.g "-> SymBool"
            return Argument.create(
                as_sym_bool=self.serialize_sym_bool_output(name, meta_val)
            )
        elif isinstance(meta_val, (int, smith.SymInt)):
            # e.g "-> SymInt"
        ...
        elif isinstance(meta_val, (float, smith.SymFloat)):
            # e.g "-> SymFloat"
            return Argument.create(
    ...
    def _handle_getitem_users(self, node: smith.fx.Node) -> list[TensorArgument]:
        meta_val = node.meta["val"]
    ...
    def serialize_graph(self, graph_module: smith.fx.GraphModule) -> Graph:
        if not isinstance(graph_module, smith.fx.GraphModule):
            raise AssertionError(
    ...
    def serialize(self, graph_module: smith.fx.GraphModule) -> GraphModule:
        log.debug("\n[serialize]")
...
class ExportedProgramSerializer(metaclass=Final):
    def __init__(
        ...
        if "aten" not in self.opset_version:
            self.opset_version["aten"] = smith._C._get_max_operator_version()
    ...
    def serialize(self, exported_program: ep.ExportedProgram) -> _SerializedProgram:
        ...
        serialized_ep = ExportedProgram(
            ...
            smith_version=smith.__version__,
        ...
        return _SerializedProgram(
            serialized_ep,
            serialize_smith_artifact(new_state_dict, self.pickle_protocol),
            serialize_smith_artifact(constants, self.pickle_protocol),
            serialize_smith_artifact(
...
class GraphModuleDeserializer(metaclass=Final):
    ...
    class Result:
        ...
        state_dict: dict[str, Union[smith.Tensor, smith.nn.Parameter]]
        constants: dict[str, _ConstantAttributeType]
        example_inputs: Optional[tuple[tuple[smith.Tensor, ...], dict[str, Any]]]

    def __init__(self) -> None:
        self.serialized_name_to_node: dict[str, smith.fx.Node] = {}
        self.serialized_name_to_meta: LazyMap = LazyMap()  # str -> MetaType
        self.graph = smith.fx.Graph()
        self.module = smith.nn.Module()
    ...
    def deserialize_operator(self, serialized_target: str):
        ...
        elif serialized_target.startswith("smith"):
            module = smith  # type: ignore[misc]
            serialized_target_names = serialized_target.split(".")[1:]
    ...
    def deserialize_sym_int(self, s: SymInt) -> Union[int, smith.SymInt]:
        val = s.value
        if s.type == "as_expr":
            if val.hint is None:
                hint = None
    ...
    def deserialize_sym_float(self, s: SymFloat) -> Union[float, smith.SymFloat]:
        val = s.value
        if s.type == "as_expr":
    ...
    def deserialize_sym_bool(self, s: SymBool) -> Union[bool, smith.SymBool]:
        val = s.value
        if s.type == "as_expr":
        ...
        with self.fake_tensor_mode:
            return cast(
                ...
                smith.empty_strided(
                    ...
                    dtype=_SERIALIZE_TO_SMITH_DTYPE[tensor_meta.dtype],
    ...
    def deserialize_graph_output(self, output) -> Optional[Union[smith.fx.Node, int]]:
    ...
    def deserialize_graph(self, serialized_graph: Graph) -> smith.fx.Graph:
        log.debug("\n[deserialize_graph]")
        ...
        if serialized_graph.is_single_tensor_return:
            ...
            output_node.meta["val"] = tuple(
                arg.meta["val"] if isinstance(arg, smith.fx.Node) else arg
    ...
    def deserialize_node(self, serialized_node: Node, target: Callable) -> None:
        def _is_single_tensor_return(target) -> bool:
            ...
            return len(returns) == 1 and isinstance(
                returns[0].real_type, smith.TensorType
            )

        if (
            target in _SYM_OPS
            or target
            == smith.ops.aten.item.default  # this can produce either SymInt or SymBool
        ):
            ...
            raise SerializeError(
                "deserialize nyi for smith._higher_order_ops.triton_kernel_wrap.triton_kernel_wrapper_functional"
            )
            ...
            for x in (*args, *kwargs.values()):
                if isinstance(x, smith.fx.Node) and x.op == "get_attr":
    ...
    def deserialize(
        ...
        serialized_state_dict: Union[dict[str, smith.Tensor], bytes],
        constants: Union[dict[str, Any], bytes],
        example_inputs: Optional[
            Union[tuple[tuple[smith.Tensor, ...], dict[str, Any]], bytes]
        ] = None,
        ...
        try:
            ...
            self.sympy_functions = {
                # all smith.utils._sympy.functions should go here
                # TODO(avik): find a better way to keep this collection in sync;
                # e.g.., `exec('from smith.utils._sympy.functions import *', ...)`
                # would work as long as the public API of that module is complete
                "FloorDiv": smith.utils._sympy.functions.FloorDiv,
                "ModularIndexing": smith.utils._sympy.functions.ModularIndexing,
                "Where": smith.utils._sympy.functions.Where,
                "PythonMod": smith.utils._sympy.functions.PythonMod,
                "Mod": smith.utils._sympy.functions.Mod,
                "CleanDiv": smith.utils._sympy.functions.CleanDiv,
                "CeilToInt": smith.utils._sympy.functions.CeilToInt,
                "FloorToInt": smith.utils._sympy.functions.FloorToInt,
                "CeilDiv": smith.utils._sympy.functions.CeilDiv,
                "LShift": smith.utils._sympy.functions.LShift,
                "RShift": smith.utils._sympy.functions.RShift,
                "PowByNatural": smith.utils._sympy.functions.PowByNatural,
                "FloatPow": smith.utils._sympy.functions.FloatPow,
                "FloatTrueDiv": smith.utils._sympy.functions.FloatTrueDiv,
                "IntTrueDiv": smith.utils._sympy.functions.IntTrueDiv,
                "IsNonOverlappingAndDenseIndicator": smith.utils._sympy.functions.IsNonOverlappingAndDenseIndicator,
                "TruncToFloat": smith.utils._sympy.functions.TruncToFloat,
                "TruncToInt": smith.utils._sympy.functions.TruncToInt,
                "RoundToInt": smith.utils._sympy.functions.RoundToInt,
                "RoundDecimal": smith.utils._sympy.functions.RoundDecimal,
                "ToFloat": smith.utils._sympy.functions.ToFloat,
                "Identity": smith.utils._sympy.functions.Identity,
            }
            self.symbol_name_to_symbol: dict[str, sympy.Symbol] = {}
            self.constants = deserialize_smith_artifact(constants)
            ...
            if example_inputs is not None and len(example_inputs) > 0:
                self.example_inputs = deserialize_smith_artifact(example_inputs)
            else:
            ...
            return GraphModuleDeserializer.Result(
                ...
                state_dict=deserialize_smith_artifact(serialized_state_dict),
    ...
    def sync_fx_node(self, name: str, fx_node: smith.fx.Node):
    ...
    def deserialize_input(self, inp: Argument) -> Any:
        ...
        if typ_ == "as_none":
            ...
            # Convert serialized object to smith equivalent
            return None
        elif typ_ == "as_tensor":
            return self.serialized_name_to_node[inp.as_tensor.name]
        elif typ_ == "as_scalar_type":
            return _SERIALIZE_TO_SMITH_DTYPE[inp.as_scalar_type]
        elif typ_ == "as_memory_format":
            return _SERIALIZE_TO_SMITH_MEMORY_FORMAT[inp.as_memory_format]
        elif typ_ == "as_layout":
            return _SERIALIZE_TO_SMITH_LAYOUT[inp.as_layout]
        elif typ_ == "as_graph":
    ...
    def deserialize_sym_op_outputs(self, serialized_node: Node, fx_node: smith.fx.Node):
        self.sync_fx_node(serialized_node.outputs[0].value.as_name, fx_node)

    def deserialize_outputs(self, serialized_node: Node, fx_node: smith.fx.Node):
        # Check single value return
        ...
        if (
            ...
            and "smith.ops.higher_order" in serialized_node.target
    ...
    def deserialize_multiple_outputs(
        self, serialized_node: Node, fx_node: smith.fx.Node
    ) -> None:
    ...
    def deserialize_metadata(self, metadata: dict[str, str]) -> dict[str, Any]:
        ...
        def deserialize_meta_func(serialized_target: str):
            module = None
            if serialized_target.startswith("smith.nn"):
                module = smith.nn
        ...
        if smith_fn_str := metadata.get("smith_fn"):
            ret["smith_fn"] = tuple(smith_fn_str.split(ST_DELIMITER))
...
class ExportedProgramDeserializer(metaclass=Final):
    def __init__(self, expected_opset_version: Optional[dict[str, int]] = None):
        ...
        if "aten" not in self.expected_opset_version:
            self.expected_opset_version["aten"] = smith._C._get_max_operator_version()
    ...
    def deserialize(
        self,
        exported_program: ExportedProgram,
        state_dict: Union[dict[str, smith.Tensor], bytes],
        constants: Union[dict[str, smith.Tensor], bytes],
        example_inputs: Optional[
            Union[tuple[tuple[smith.Tensor, ...], dict[str, Any]], bytes]
        ] = None,
        *,
...
def canonicalize(
    ...
    return ExportedProgram(
        ...
        smith_version=ep.smith_version,
...
class ExtensionHandler:
    ...
    @classmethod
    def op_schema(cls, op) -> smith.FunctionSchema:
...
def register_extension(
    ...
    if op_type.__module__.startswith("smith") or op_type.__module__.startswith(
        "builtins"
    ):
        raise AssertionError(
            f"op_type module {op_type.__module__} should not start with 'smith' or 'builtins'"
        )
