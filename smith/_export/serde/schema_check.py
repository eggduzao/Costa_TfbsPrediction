...
from smith._export.serde.union import _Union
...
"""
    thrift_schema = f"""
namespace py3 smith._export
namespace cpp2 smith._export.schema
...
def update_schema():
    ...
    smith_prefix = "smith/"
    if not yaml_path.startswith(smith_prefix):
        raise AssertionError(
            f"yaml_path must start with {smith_prefix}, got {yaml_path}"
        )
    if not thrift_schema_path.startswith(smith_prefix):
        raise AssertionError(
            f"thrift_schema_path must start with {smith_prefix}, got {thrift_schema_path}"
        )
    ...
    return _Commit(
        ...
        cpp_header_path=smith_prefix + "csrc/utils/generated_serialization_types.h",
