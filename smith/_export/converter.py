...
from smith._export.passes.replace_quantized_ops_with_standard_ops_pass import (
    replace_quantized_ops_with_standard_ops,
)
from smith.export.dynamic_shapes import _tree_map_with_path, Dim
from smith.export.exported_program import ExportedProgram
...
from smith.fx import subgraph_rewriter
...
def _get_param_count_list(method_graph, args_params):
    ...
    for input_, arg_params_ in zip(method_graph.inputs(), args_params):
        if "PackedParams" in str(input_.type()):
            in_vars, _ = smith.jit._flatten(arg_params_)
...
def _trace_and_get_graph_from_model(model, args):
    ...
    orig_state_dict_keys = smith.jit._unique_state_dict(model).keys()

    # Disable Autocast cache because it replaces kernel's weight and bias
    # by (undesired) constants.
    # No perf impact for when there are reused weights since https://github.com/blacksmith/blacksmith/pull/85665
    prev_autocast_cache_enabled = smith.is_autocast_cache_enabled()
    smith.set_autocast_cache_enabled(False)
    trace_graph, smith_out, _inputs_states = smith.jit._get_trace_graph(
        model,
        args,
        strict=False,
    ...
    smith.set_autocast_cache_enabled(prev_autocast_cache_enabled)

    if orig_state_dict_keys != smith.jit._unique_state_dict(model).keys():
        raise RuntimeError(
            "state_dict changed after running the tracer; "
            "something weird is happening in your model!"
        )

    return trace_graph, smith_out


def _create_jit_graph(
    model: Union[smith.nn.Module, smith.jit.ScriptFunction], args: Sequence[Any]
) -> tuple[smith.Graph, list["_C.IValue"], Any, Optional[smith.ScriptModule]]:
    if isinstance(model, (smith.jit.ScriptFunction, smith.jit.ScriptModule)):
        flattened_args = tuple(smith.jit._flatten(tuple(args))[0])
        smith_out = None

        if isinstance(model, smith.jit.ScriptModule):
            try:
            ...
            in_vars, _ = smith.jit._flatten(args_params)
            graph = _C._propagate_and_assign_input_shapes(
                method_graph, tuple(in_vars), param_count_list, False, False
            )
            return graph, params, smith_out, module
        ...
        return graph, params, smith_out, None

    graph, smith_out = _trace_and_get_graph_from_model(model, args)
    _C._jit_pass_onnx_lint(graph)
    state_dict = smith.jit._unique_state_dict(model)
...
def inplace_optimize_sym_size_div(gm: smith.fx.GraphModule):
    def pattern(im, dim, scale):
        sym_size_int = smith.ops.aten.sym_size.int(im, dim)
        scalar_tensor = smith.ops.aten.scalar_tensor(sym_size_int)
        div_scalar_mode = smith.ops.aten.div.Scalar_mode(
            scalar_tensor, scale, rounding_mode="trunc"
        )
        int_tensor = smith.ops.aten.Int.Tensor(div_scalar_mode)
        return int_tensor
...
_SMITH_ENUM_TO_DTYPE = {value: key for key, value in _SMITH_DTYPE_TO_ENUM.items()}
...
def get_dtype_as_int(tensor):
    ...
    """
    dtype = tensor.dtype
    if dtype not in _SMITH_DTYPE_TO_ENUM:
        raise RuntimeError(f"Unsupported dtype {dtype}")
    return _SMITH_DTYPE_TO_ENUM[dtype]
...
kind_to_standard_operators: dict[str, Callable[..., Any]] = {
    ...
    "aten::numel": smith.ops.aten.sym_numel,
    "aten::size": smith.ops.aten.sym_size,
    "aten::storage_offset": smith.ops.aten.sym_storage_offset,
    "aten::stride": smith.ops.aten.sym_stride,
}
...
def get_block_to_lifted_attrs(
    graph: smith._C.Graph,
) -> tuple[dict[smith._C.Block, set[str]], dict[str, str]]:
    """
    ...
    each block to a graph which will be passed into smith.cond. A restriction for smith.cond is that model
    ...
    blocks_to_lifted_attrs: dict[smith._C.Block, set[str]] = {}
    ...
    def _map_blocks_to_lifted_attrs(entry):
        ...
        if not isinstance(entry, smith._C.Graph):  # Skip the top level.
...
def get_attribute_fqn_from_ts_node(
    name_to_attribute_fqn: dict[str, str], node: smith._C.Node
) -> str:
...
class TS2FXGraphConverter:
    def __init__(
        self,
        ts_graph: Union[smith._C.Graph, smith._C.Block],
        name_to_param: dict[str, smith.Tensor],
        name_to_buffer: dict[str, smith.Tensor],
        blocks_to_lifted_attrs: dict[smith._C.Block, set[str]],
        ...
        self.fx_graph: smith.fx.Graph = smith.fx.Graph()
        self.input_specs: list[InputSpec] = []
        self.output_specs: list[OutputSpec] = []

        # Mapping of TS node name to converted FX node
        self.name_to_node: dict[
            str, Union[smith.fx.Node, list[smith.fx.Node], dict[Any, smith.fx.Node]]
        ] = {}
        # Mapping of TS node name to constant value (int, str, SmithBind obj,
        # tensor constants ...)
        self.name_to_constant: dict[str, Any] = name_to_constant

        # Mapping from smithscript node output name to attribute fully qualified name
        ...
        self.subgraphs: dict[str, smith.fx.GraphModule] = {}
    ...
    def _is_get_attr_node(self, fqn):
        return (
            ...
            or (
                ...
                and isinstance(self.name_to_constant[fqn], smith.ScriptObject)
            )
        )

    def _convert_block_to_subgraph(self, node: smith._C.Node, arguments: list[str]):
    ...
    def is_top_level_graph(self):
        return isinstance(self.ts_graph, smith._C.Graph)
    ...
    def get_args_kwargs(self, node: smith._C.Node, schema):
        args = []
        kwargs = {}
    ...
    def get_fx_value_by_ir_value(self, value: smith._C.Value):
        ...
        if value_name in self.name_to_node:
            ...
            if isinstance(self.name_to_constant[value_name], smith.ScriptObject):
    ...
    def convert(self) -> smith.fx.GraphModule:
    ...
    def convert_graph_inputs(self):
        for graph_input in self.ts_graph.inputs():
            ...
            if name in self.name_to_param:
                ...
                if not isinstance(self.name_to_constant[name], smith.ScriptObject):
            ...
            elif isinstance(graph_input.type(), smith.ClassType):
    ...
    def convert_aten_Float(self, node: smith._C.Node):
        def to_float_tensor(t):
            return t.to(dtype=smith.float).item()
    ...
    def convert_aten_tensor(self, node: smith._C.Node):
        """aten::tensor creates a constant tensor ad-hoc --> GetAttr"""
        args, kwargs = self.get_args_kwargs(node, smith.ops.aten.tensor.default._schema)
        ...
        to_tensor = (
            ...
            else smith._refs.tensor
        )

        def target(*args, **kwargs):
            if "dtype" in kwargs and kwargs["dtype"] is not None:
                kwargs["dtype"] = _SMITH_ENUM_TO_DTYPE[kwargs["dtype"]]
            return to_tensor(*args, **kwargs)

        # def to_dynamic_tensor(*args, **kwargs):
        #     if "dtype" in kwargs and kwargs["dtype"] is not None:
        #         kwargs["dtype"] = _SMITH_ENUM_TO_DTYPE[kwargs["dtype"]]
        #     return smith._refs.tensor(*args, **kwargs)

        output_name = node.output().debugName()
        fx_node = self.fx_graph.call_function(target, args, kwargs)
        self.name_to_node[output_name] = fx_node

    def convert_aten_append(self, node: smith._C.Node):
    ...
    def convert_prim_Constant(self, node: smith._C.Node):
    ...
    def convert_prim_CallMethod(self, node: smith._C.Node):
        inp_list = [self.get_fx_value_by_ir_value(inp) for inp in node.inputs()]  # noqa: C416
    ...
    def convert_prim_device(self, node: smith._C.Node):
        input_type = node.input().type()
        if input_type.isSubtypeOf(smith._C.TensorType.get()):
    ...
    def convert_prim_GetAttr(self, node: smith._C.Node):
        ...
        if self.is_top_level_graph():
            ...
            # Special support for if blocks which do not allow SetAttr SmithScript
            # node and get_attr FX Graph Node.
            if self._is_get_attr_node(attr_fqn):
                self.name_to_node[output_name] = self.name_to_node[attr_fqn]

    def convert_prim_SetAttr(self, node: smith._C.Node):
        attr_fqn = get_attribute_fqn_from_ts_node(self.name_to_attribute_fqn, node)
        ...
        if self._is_get_attr_node(attr_fqn):
            ...
            self.fx_graph.call_function(
                smith.Tensor.copy_, (fx_attr_node, ts_graph_tensor_input)
            )
        else:
            self.name_to_non_tensor_attribute_node[attr_fqn] = ts_graph_tensor_input

    def convert_call_function_op(self, node: smith._C.Node):
    ...
    def convert_prim_TupleConstruct(self, node: smith._C.Node):
        self._convert_prim_iterator(node)

    def convert_prim_ListConstruct(self, node: smith._C.Node):
        self._convert_prim_iterator(node)

    def _convert_prim_iterator(self, node: smith._C.Node):
        output_list = [self.get_fx_value_by_ir_value(inp) for inp in node.inputs()]

        output_name = node.output().debugName()
        self.name_to_node[output_name] = output_list

    def convert_prim_DictConstruct(self, node: smith._C.Node):
        output_dict = {}
        k, v = None, None
    ...
    def convert_prim_ListUnpack(self, node: smith._C.Node):
        self._convert_prim_unpack_iterator(node)

    def convert_prim_TupleUnpack(self, node: smith._C.Node):
        self._convert_prim_unpack_iterator(node)

    def _convert_prim_unpack_iterator(self, node: smith._C.Node):
    ...
    def convert_aten_Int(self, node: smith._C.Node):
        # converts aten::Int as aten._to_copy + aten::_local_scalar_dense
        target = smith.ops.aten._to_copy.default
        args = tuple(self.get_fx_value_by_ir_value(input) for input in node.inputs())
        to_copy_node = self.fx_graph.call_function(target, args, {"dtype": smith.int32})

        fx_node = self.fx_graph.call_function(
            smith.ops.aten._local_scalar_dense.default, (to_copy_node,)
        )
    ...
    def convert_prim_NumToTensor(self, node: smith._C.Node):
        # Converts prim::NumToTensor as aten.scalar_tensor.
        # prim::NumToTensor IRs are currently triggered by:
        # .size() https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/jit/frontend/tracer.cpp#L950
        # .numel() https://github.com/blacksmith/blacksmith/blob/main/smith/csrc/jit/frontend/tracer.cpp#L971
        # For both of those APIs, smith.jit.trace implicitly sets the output tensor type
        # to be LongTensor.
        target = smith.ops.aten.scalar_tensor
        args = tuple(self.get_fx_value_by_ir_value(input) for input in node.inputs())

        fx_node = self.fx_graph.call_function(target, args, {"dtype": smith.long})
        output_name = node.output().debugName()
        self.name_to_node[output_name] = fx_node

    def convert_prim_CreateObject(self, node: smith._C.Node):
        output_name = node.output().debugName()
        self.name_to_attribute_fqn[output_name] = ""

    def convert_aten__convolution(self, node: smith._C.Node):
        # converts aten::_convolution as aten.convolution, since aten::_convolution
        # doesn't have a meta function
        target = smith.ops.aten.convolution.default
        args, kwargs = self.get_args_kwargs(node, target._schema)
    ...
    def convert_aten_div(self, node: smith._C.Node):
        target = get_op_overload(node)
        ...
        if schema.overload_name == "Tensor_mode":
            ...
            if arg1_name in self.name_to_constant and isinstance(
                self.name_to_constant[arg1_name], smith.Tensor
            ):
                ...
                if tensor_constant.numel() == 1:
                    ...
                    fx_node = self.fx_graph.call_function(
                        smith.ops.aten.div.Scalar_mode,
                        tuple(updated_args),
                        kwargs,
                    )
    ...
    def convert_aten___getitem__(self, node: smith._C.Node):
    ...
    def convert_aten_to(self, node: smith._C.Node):
        target = get_op_overload(node)
        ...
        if target is smith.ops.aten.to.dtype or target is smith.ops.aten.to.prim_dtype:
            ...
            if has_mutable_target:
                ...
                fx_node = self.fx_graph.call_function(
                    smith.ops.aten.to.dtype, tuple(new_args)
                )
                # temp hack to work around the issue https://github.com/blacksmith/blacksmith/issues/131679
                # When this issue is fixed, the clone node would be no longer needed
                clone_node = self.fx_graph.call_function(
                    smith.ops.aten.clone.default, (fx_node,)
                )
                output_name = node.output().debugName()
    ...
    def convert_aten_add(self, node: smith._C.Node):
        if node.schema() == "(no schema)":
            if isinstance(node.inputsAt(0).type(), smith.ListType) and isinstance(
                node.inputsAt(1).type(), smith.ListType
            ):
                target = smith.ops.aten.add.t
            else:
                raise RuntimeError(f"unable to determined the target for {node}")
        else:
            target = get_op_overload(node)

        if target is smith.ops.aten.add.t:
    ...
    def convert_prim_Loop(self, node: smith._C.Node):
        inputs = list(node.inputs())
    ...
    def _check_set_attr_in_if_block(self, if_node: smith._C.Node):
        for block in if_node.blocks():
            for node in block.nodes():
                if node.kind() == "prim::SetAttr":
                    raise RuntimeError(
                        "During converting prim::If to smith.cond, found prim::SetAttr op"
                        " which is not supported yet. Please file an issue if you come "
                        "across this error."
                    )

    def convert_prim_If(self, node: smith._C.Node):
        ...
        cond_node = self.fx_graph.call_function(smith.cond, args, {})
    ...
    def convert_aten_Bool(self, node: smith._C.Node):
        self._convert_as_noop(node)

    def convert_prim_Enter(self, node: smith._C.Node):
        # export generally treats prim::Enter as noop
        # The only context manager export supports is aten::enable_grad.
        # Unfortunately, SmithScript does not support aten::enable_grad yet.
        # TODO: support aten::enable_grad in both SmithScript and Converter.
        return

    def convert_prim_Exit(self, node: smith._C.Node):
        # export treats prim::Exit as noop
        return

    def _convert_as_noop(self, node: smith._C.Node):
    ...
    def convert_profiler__record_function_exit(self, node: smith._C.Node):
        ...
        target = smith.ops.profiler._record_function_exit
        args = tuple(self.get_fx_value_by_ir_value(input) for input in node.inputs())
        self.fx_graph.call_function(target, args)

    def convert_prim_tolist(self, node: smith._C.Node):
    ...
    def convert_prim_Uninitialized(self, node: smith._C.Node):
        ...
        output_name = node.output().debugName()
        self.name_to_constant[output_name] = smith.Tensor()

    def _convert_standard_operators(self, node: smith._C.Node):
    ...
    def convert_node(self, node: smith._C.Node):
        node_kind = node.kind()
        ...
        for output_name in outp_name_list:
            if output_name in self.name_to_node:
                ...
                if (
                    ...
                    and isinstance(fx_node, smith.fx.Node)
                    and fx_node.op == "placeholder"
                ):
                    fx_node = self.fx_graph.call_function(smith.clone, (fx_node,))
                args.append(fx_node)
...
class ExplainTS2FXGraphConverter(TS2FXGraphConverter):
    ...
    def __init__(
        ...
        self.unsupported_node_list: list[smith._C.Node] = []
...
class TS2EPConverter:
    # SmithScript model to ExportedProgram converter
    def __init__(
        self,
        ts_model: Union[smith.jit.ScriptModule, smith.jit.ScriptFunction],
        ...
        self.name_to_param: dict[str, smith.Tensor] = {}
        self.name_to_buffer: dict[str, smith.Tensor] = {}
        param_list = (
            list(self.ts_model.parameters())
            if not isinstance(self.ts_model, smith._C.ScriptFunction)
            else []
        )
        if not isinstance(self.ts_model, smith._C.ScriptFunction):
...
INFO: (SMITH_LOGS="export" <cmd>)
    * Log SmithScript IR.

DEBUG: (SMITH_LOGS="+export" <cmd>), additionally
    * Log conversion IR by IR in a format of [<conversion handler name>] converts [<IR>].
        """
        )
        log.info("SmithScript graph\n\n%s\n", self.ts_graph)
        ...
        # the original SmithScript model. Throw warnings for additionally populated
        # state_dict entries.
        if not isinstance(self.ts_model, smith._C.ScriptFunction):
            for k, tensor in self.ts_model.state_dict().items():  # type: ignore[union-attr]
    ...
    def retrace_as_exported_program(
        ...
        dynamic_shapes = _tree_map_with_path(
            lambda path, x: (
                [Dim.AUTO] * x.dim() if isinstance(x, smith.Tensor) else None
            ),
            self.sample_args,
        )
        ...
        ep._constants.update(
            {
                ...
                if isinstance(v, (smith.Tensor, smith.ScriptObject))
            }
        )
        ...
        for spec in ep.graph_signature.input_specs:
            ...
            if spec.kind == InputKind.BUFFER and spec.target in name_to_constant:
                if not isinstance(name_to_constant[spec.target], smith.Tensor):
                    raise AssertionError(
    ...
    def lift_get_attr(self):
        ...
        #     1. Tensor constants attributes (e.g., self.data = smith.tensor([2,3]))
        ...
        #     Since it is hard to manually do so for SmithScript models
        ...
        def get_fqn(node: smith._C.Node):
            attr_name = node.s("name")
            input_name = node.input().debugName()
        ...
        def _dfs_get_attr(block):
            for node in block.nodes():
                ...
                if node.kind() == "prim::GetAttr":
                    ...
                    elif isinstance(value, smith.ScriptObject):
