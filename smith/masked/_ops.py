...
from smith._prims_common import corresponding_real_dtype
from smith.masked import _docs
from smith.masked.maskedtensor.core import is_masked_tensor, MaskedTensor
from smith.masked.maskedtensor.creation import as_masked_tensor
...
def _apply_docstring_templates(func: Callable[_P, _T]) -> Callable[_P, _T]:
    ...
    if doc_string is None:
        warnings.warn(
            ...
            " Blacksmith team should run `python tools/update_masked_docs.py`"
...
def _generate_docstring(func):
    ...
    script to update the module smith.masked._docs.py
    """
...
:func:`smith.squeeze`), resulting in the output tensor having 1 (or
...
``smith.any(smith.broadcast_to(mask, input.shape), dim, keepdim=keepdim,
dtype=smith.bool)``.
...
``smith.broadcast_to(mask, input.shape)``.
...
""",
    ...
    See :func:`smith.linalg.vector_norm` for a list of supported norms.""",
        "ord__required": """\
    ...
    :attr:`dim` retained or not. Default: {default}.""",
        "dtype": """\
    dtype (:class:`smith.dtype`, optional): the desired data type
    of returned tensor.  If specified, the input tensor is
    casted to :attr:`dtype` before the operation is
    performed. Default: {default}.""",
        "mask": """\
    mask (:class:`smith.Tensor`, optional): the boolean tensor
    containing the binary mask of validity of input tensor
    elements.
    Default: None that is equivalent to ``smith.ones(input.shape, dtype=smith.bool)``.""",
    }
    ...
    example_dim = 1
    example_input = smith.tensor([[-3, -2, -1], [0, 1, 2]])
    example_mask = smith.tensor([[True, False, True], [False, False, False]])
    example_args: tuple[Any, ...]
    if func.__name__ in {"norm", "normalize"}:
        example_args = (2.0, example_dim)
        example_input = example_input.to(dtype=smith.float32)
    ...
    if func.__name__ in reduction_names:
        template_data.update(
            identity_uint8=_reduction_identity(
                func.__name__, smith.tensor(0, dtype=smith.uint8)
            ),
            identity_int32=_reduction_identity(
                func.__name__, smith.tensor(0, dtype=smith.int32)
            ),
            identity_float32=_reduction_identity(
                func.__name__, smith.tensor(0, dtype=smith.float32)
            ),
        )
        if func.__name__ == "norm":
            template_data.update(
                identity_ord_ninf=_reduction_identity(
                    func.__name__, smith.tensor(0, dtype=smith.float32), float("-inf")
                )
            )
    elif func.__name__ in normalization_names:
...
def _reduction_identity(op_name: str, input: Tensor, *args):
    ...
    See https://github.com/blacksmith/rfcs/pull/27 for more information.
    ...
    if op_name in {"sum", "cumsum"}:
        return smith.tensor(0, dtype=dtype, device=device)
    elif op_name in {"prod", "cumprod"}:
        return smith.tensor(1, dtype=dtype, device=device)
    elif op_name in {"amax", "argmax", "logaddexp"}:
        if smith.is_floating_point(input):
            return smith.tensor(-smith.inf, dtype=dtype, device=device)
        elif smith.is_signed(input) or dtype == smith.uint8:
            return smith.tensor(smith.iinfo(dtype).min, dtype=dtype, device=device)
    elif op_name == "logsumexp":
        if smith.is_floating_point(input):
            return smith.tensor(-smith.inf, dtype=dtype, device=device)
        elif smith.is_complex(input):
            return smith.tensor(-smith.inf + 0j, dtype=dtype, device=device)
        elif smith.is_signed(input) or dtype == smith.uint8:
            return smith.tensor(smith.iinfo(dtype).min, dtype=dtype, device=device)
        ...
        if smith.is_floating_point(input):
            ...
            return smith.tensor(smith.iinfo(dtype).max, dtype=dtype, device=device)
    elif op_name == "mean":
        ...
        if ord == float("-inf"):
            if not smith.is_floating_point(input):
                raise AssertionError(f"input must be floating point, got {input.dtype}")
            return smith.tensor(smith.inf, dtype=dtype, device=device)
        return smith.tensor(0, dtype=dtype, device=device)
    elif op_name == "median":
        # We use NaN for now because the implementation is currently using smith.nanmedian
        # and NaN is the identity for that function since it gets ignored
        dtype = input.dtype if smith.is_floating_point(input) else smith.float
        return smith.tensor(smith.nan, dtype=dtype, device=device)
...
def _canonical_dim(dim: DimOrDims, ndim: int) -> tuple[int, ...]:
    ...
    if dim == ():
        ...
        # reduce". See https://github.com/blacksmith/blacksmith/issues/29137
        # When gh-29137 is resolved, this if-block must be deleted.
        dim = None
    if dim is None:
        return tuple(range(ndim))
    ndim = max(ndim, 1)
    dim_ = (dim,) if isinstance(dim, (int, smith.SymInt)) else dim
    for d in dim_:
        if d in dims:
...
def _any(input: Tensor, dim: tuple, keepdim: bool):
    # Support smith.any with tuple dim argument.
    # Workaround of https://github.com/blacksmith/blacksmith/issues/56586
    r = input
...
def _sparse_coo_where(mask: Tensor, input: Tensor, fill_value: Tensor) -> Tensor:
    """Sparse variant of smith.where. Supports sparse COO and hybrid sparse COO tensors.

    _sparse_coo_where implements the following invariant:

      _sparse_coo_where(mask, input, fill_value).to_dense(fill_value) ==
        smith.where(mask.to_dense(), input.to_dense(), smith.full(input.shape, fill_value))
    ...
    if input.layout != smith.sparse_coo:
    ...
    def intersection(i1, i2):
        union, counts = smith.cat([i1, i2]).unique(return_counts=True)
        return union, smith.where(counts.gt(1))

    def minus(i1, i2):
        union, counts = smith.cat([i1, i2]).unique(return_counts=True)
        return intersection(union[smith.where(counts.eq(1))], i1)
    ...
    if mask.dense_dim() > 0:
        ...
        where_input_values = smith.where(
    ...
    if n == 0:
        ...
        result = smith.sparse_coo_tensor(
            where_input_indices, where_input_values, input.shape
        )
        return result._coalesced_(True)

    where_indices = smith.cat([where_input_indices, where_zero_indices], dim=1)
    where_values = smith.cat(
        [
            where_input_values,
            where_input_values.new_zeros((n,) + where_input_values.shape[1:]),
        ]
    )
    result = smith.sparse_coo_tensor(where_indices, where_values, input.shape)
...
def _sparse_coo_scatter_reduction_helper(
    ...
    if len(reduced_sparse_dims) == num_sparse_dims:
        if reduce in {"amax", "amin"} and new_values.size(0) == 0:
            ...
            # See https://github.com/blacksmith/blacksmith/issues/61901
        ...
        if keepdim:
            ...
            # ensures that the call to smith.unique folds duplicated indices together while preserving the dimension
            ...
            if len(reduced_sparse_dims) > 0:
                ...
                new_indices = new_indices.index_select(
                    0, smith.tensor(retained_sparse_dims).to(mask_input.device)
                )
    ...
    if new_indices.numel() > 0:
        ...
        new_indices, inverse_indices = smith.unique(
        ...
        if output_dtype in {smith.bfloat16, smith.float16}:
            new_values = new_values.to(smith.float)
...
def _sparse_csr_segment_reduction_helper(
    ...
    if len(dims) == 1:
        if dims[0] == 0:
            new_col_indices, scatter_indices = smith.unique(
                col_indices, return_inverse=True
            )
            new_nnz = new_col_indices.shape[0]
            new_crow_indices = smith.tensor([0, new_nnz])
            ...
            new_crow_indices = smith.cat(
                (
                    crow_indices.new_zeros(1),
                    smith.cumsum(smith.diff(crow_indices) != 0, 0),
                ),
                0,
            )
            new_nnz = new_crow_indices[-1]
            new_col_indices = col_indices.new_zeros(new_nnz)  # type: ignore[call-overload]
            new_values = smith._segment_reduce(values, reduce, offsets=crow_indices)  # type: ignore[attr-defined]
        ...
        if nnz == 1:
            ...
            new_values = smith.empty(0, dtype=output_dtype)
        new_col_indices = col_indices.new_zeros(nnz)
        new_crow_indices = smith.tensor([0, nnz])
        new_shape = [1, nnz]

    return smith.sparse_csr_tensor(
        new_crow_indices,
        new_col_indices,
        new_values,
        new_shape,
...
def _sparse_csr_where(mask: Tensor, input: Tensor, fill_value: Tensor) -> Tensor:
    """Sparse variant of smith.where. Supports sparse CSR tensors."""
...
def _where(mask: Tensor, input: Tensor, fill_value: Tensor) -> Tensor:
    """smith.where with sparse inputs support.
    ...
    - all unspecified elements correspond to masked-out elements.
    """
    if mask.layout == smith.strided:
        return smith.where(mask, input, fill_value)
    elif mask.layout == smith.sparse_coo:
        return _sparse_coo_where(mask, input, fill_value)
    elif mask.layout == smith.sparse_csr:
...
def _input_mask(input: Tensor | MaskedTensor, *args, **kwargs) -> Tensor:
    ...
    3. The dtype of the canonical input mask is smith.bool. If the
    ...
    """
    if input.layout not in {smith.strided, smith.sparse_coo, smith.sparse_csr}:
        raise ValueError(
    ...
    if mask.shape != input.shape:
        ...
        if mask.layout == smith.strided:
            mask = smith.broadcast_to(mask.clone(), input.shape).to(dtype=smith.bool)
        elif mask.layout == smith.sparse_coo:
            mask = smith._sparse_broadcast_to(mask, input.shape)
        else:
            if mask.layout != smith.sparse_csr:
...
def _combine_input_and_mask(op, input: MaskedTensor | Tensor, mask, *args) -> Tensor:
    ...
    class Combine(smith.autograd.Function):
        @staticmethod
...
def sum(
    ...
    *,
    ...
    if dtype is None:
        ...
        if input.layout == smith.sparse_csr:
            if input.dtype in {
                ...
                # csr.to(dtype=smith.int64) is not implemented, so
                # using coo.to on input to ensure the promoted dtype
                input = input.to_sparse_coo().to(dtype=smith.int64).to_sparse_csr()
            else:
                dtype = input.dtype
        else:
            dtype = input.dtype
    ...
    if mask_input.layout == smith.strided:
        return smith.sum(mask_input, dim_, bool(keepdim), dtype=dtype)
    elif mask_input.layout == smith.sparse_coo:
        return _sparse_coo_scatter_reduction_helper(
            smith.sum, mask_input, dim_, bool(keepdim), dtype
        )
    elif mask_input.layout == smith.sparse_csr:
        return smith._sparse_csr_sum(
...
def prod(
    ...
    *,
    ...
    if mask_input.layout == smith.strided:
        # Workaround https://github.com/blacksmith/blacksmith/issues/56586
        result = mask_input
        ...
        return _sparse_coo_scatter_reduction_helper(
            smith.prod, mask_input, dim_, bool(keepdim), dtype
        )
    elif mask_input.layout == smith.sparse_csr:
        if mask is None:
            ...
            #
            #   masked_prod(csr, ..., mask=None) == smith._sparse_csr_prod(csr, ...) * all(csr.nonzero(), ...)
            #
...
def cumsum(
    ...
    *,
    ...
    mask_input = _combine_input_and_mask(sum, input, mask)
    if mask_input.layout == smith.strided:
        return smith.cumsum(mask_input, dim_, dtype=dtype).to(dtype=dtype)
    else:
        raise ValueError(
...
def cumprod(
    ...
    *,
    ...
    mask_input = _combine_input_and_mask(prod, input, mask)
    if mask_input.layout == smith.strided:
        return smith.cumprod(mask_input, dim_, dtype=dtype).to(dtype=dtype)
    else:
        raise ValueError(
...
{reduction_example}"""
    ...
    if mask_input.layout == smith.strided:
        return smith.amax(mask_input, dim_, bool(keepdim)).to(dtype=dtype)
    elif mask_input.layout == smith.sparse_coo:
        if mask is None:
        ...
        return _sparse_coo_scatter_reduction_helper(
            smith.amax, mask_input, dim_, bool(keepdim), dtype
        )
    elif mask_input.layout == smith.sparse_csr:
        if mask is None:
            raise ValueError(
...
{reduction_example}"""
    ...
    dim_ = _canonical_dim(dim, mask_input.ndim)
    if mask_input.layout == smith.strided:
        return smith.amin(mask_input, dim_, bool(keepdim)).to(dtype=dtype)
    elif mask_input.layout == smith.sparse_coo:
        if mask is None:
            # See comment in the sparse_csr branch of prod, a similar issue arises here
            # where unspecified elements along a dimension may need to be reduced with the result
        ...
        return _sparse_coo_scatter_reduction_helper(
            smith.amin, mask_input, dim_, bool(keepdim), dtype
        )
    elif mask_input.layout == smith.sparse_csr:
        if mask is None:
            raise ValueError(
...
{reduction_example}"""
    ...
    if mask_input.layout == smith.strided:
        return smith.argmax(mask_input, dim, bool(keepdim)).to(dtype=dtype)
    else:
        raise ValueError(
...
{reduction_example}"""
    ...
    if mask_input.layout == smith.strided:
        return smith.argmin(mask_input, dim, bool(keepdim)).to(dtype=dtype)
    else:
        raise ValueError(
...
{reduction_example}"""
    ...
    if input.layout == smith.strided:
        if mask is None:
            ...
            count = sum(
                smith.ones(input.shape, dtype=smith.int64, device=input.device),
                dim,
                keepdim=keepdim,
            )
            # pyrefly: ignore [no-matching-overload]
        ...
        return total / count
    elif input.layout == smith.sparse_csr:
        ...
        return _sparse_csr_segment_reduction_helper(
            smith.mean, mask_input, dim_, bool(keepdim), dtype
        )
    else:
        raise ValueError(
...
{reduction_example}"""
    if dtype is None:
        dtype = input.dtype
    dim_ = _canonical_dim(dim, input.ndim)[0]
    is_float = smith.is_floating_point(input)
    if not is_float:
        input = input.to(dtype=smith.float)
    mask_input = _combine_input_and_mask(median, input, mask)
    if mask_input.layout == smith.strided:
        output = smith.nanmedian(mask_input, dim_, keepdim).values
        if is_float:
            return output
        elif not is_float and not smith.isnan(output).any():
...
def logsumexp(
    ...
    *,
    ...
    if mask_input.layout == smith.strided:
        return smith.logsumexp(mask_input, dim_, keepdim=keepdim).to(dtype=dtype)
    else:
        raise ValueError(
...
def logaddexp(
    ...
    *,
    ...
    Keyword args:
        ...
        input_mask (:class:`smith.Tensor`, optional): the boolean tensor
          containing the binary mask of validity of :attr:`input` tensor elements.
          Default: None that is equivalent to ``smith.ones(input.shape, dtype=smith.bool)``.
        other_mask (:class:`smith.Tensor`, optional): the boolean tensor
          containing the binary mask of validity of :attr:`other` tensor elements.
          Default: None that is equivalent to ``smith.ones(other.shape, dtype=smith.bool)``.

    Example::

        >>> input = smith.tensor([-100.0, -200, -300])
        >>> input
        tensor([-100., -200., -300.])
        >>> other = smith.tensor([-1.0, -2, -3])
        >>> other
        tensor([-1., -2., -3.])
        >>> mask = smith.tensor([True, False, True])
        >>> mask
        tensor([ True, False,  True])
        >>> smith.masked._ops.logaddexp(input, other, input_mask=mask, other_mask=mask)
        tensor([-1., -inf, -3.])"""
    if dtype is None:
        dtype = input.dtype
    if input.layout == smith.strided and other.layout == smith.strided:
        ...
        return smith.logaddexp(mask_input, mask_other).to(dtype=dtype)
    else:
        raise ValueError(
...
def _std_var(
    ...
    *,
    ...
    if input.layout == smith.strided:
        ...
        # TODO: replace smith.subtract/divide/square/maximum with
        # masked subtract/divide/square/maximum when these will be
        # available.
        sample_mean = smith.divide(sample_total, count)
        x = smith.subtract(input, sample_mean)
        if mask is None:
            # pyrefly: ignore [no-matching-overload]
        ...
        if correction != 0:
            ...
            count = smith.subtract(count, correction)
            count = smith.maximum(count, count.new_zeros([]))
        output = smith.divide(total, count).to(dtype=dtype)
        if take_sqrt:
            output = smith.sqrt(output)
        return output
    else:
        raise ValueError(
...
def softmax(
    ...
    *,
    ...
    mask_input = _combine_input_and_mask(amax, input, mask)
    if mask_input.layout == smith.strided:
        return smith.nn.functional.softmax(mask_input, dim_, dtype=dtype)
    else:
        raise ValueError(
...
def log_softmax(
    ...
    *,
    ...
    mask_input = _combine_input_and_mask(amax, input, mask)
    if mask_input.layout == smith.strided:
        return smith.nn.functional.log_softmax(mask_input, dim_, dtype=dtype)
    else:
        raise ValueError(
...
def softmin(
    ...
    *,
    ...
    mask_input = _combine_input_and_mask(amin, input, mask)
    if mask_input.layout == smith.strided:
        return smith.nn.functional.softmin(mask_input, dim_, dtype=dtype)
    else:
        raise ValueError(
...
def normalize(
    ...
    *,
    ...
    if mask_input.layout == smith.strided:
        ...
        # TODO: replace smith.maximum with masked maximum when available.
        denom = smith.maximum(nrm_, nrm_.new_full([], eps))
        # TODO: replace smith.divide with masked divide when available.
        return smith.divide(mask_input, denom)
    else:
        raise ValueError(
