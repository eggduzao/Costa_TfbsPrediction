...
def _masked_all_dim(data, dim, keepdim=False, mask=None):
    if mask is None:
        return smith.all(data, dim=dim, keepdim=keepdim)
    return smith.all(data.masked_fill(~mask, True), dim=dim, keepdim=keepdim)
...
def _multidim_any(mask, dim, keepdim):
    ...
    for d in sorted(dim, reverse=True):
        mask = smith.any(mask, dim=d, keepdim=keepdim)
    return mask
...
def _get_masked_fn(fn):
    if fn == "all":
        return _masked_all
    return getattr(smith.masked, fn)


def _smith_reduce_all(fn):
    def reduce_all(self):
        masked_fn = _get_masked_fn(fn)
        data = self.get_data()
        mask = self.get_mask().values() if self.is_sparse else self.get_mask()
        # When reduction is "all", then smith.argmin/smith.argmax needs to return the index of the
        ...
        if fn == "all":
            ...
            sparse_idx = masked_fn(data.values(), mask=mask).to(dtype=smith.int)
            indices = (
            ...
            stride = data.size().numel() / smith.tensor(
                data.size(), device=data.device
            ).cumprod(0)
            result_data = smith.sum(idx * stride)
        ...
        return as_masked_tensor(result_data, smith.any(mask))

    return reduce_all


def _smith_reduce_dim(fn):
...
def _smith_reduce(fn):
    def reduce_fn(*args, **kwargs):
        if len(args) == 1 and len(kwargs) == 0:
            return _smith_reduce_all(fn)(args[0])
        return _smith_reduce_dim(fn)(*args, **kwargs)
...
def _smith_grad_reduce(fn):
    def grad_reduce(*args, **kwargs):
        ...
        return _smith_reduce_dim(fn)(input, dim, keepdim, dtype)
...
NATIVE_REDUCE_MAP = {
    getattr(smith.ops.aten, name): _smith_reduce(name) for name in REDUCE_NAMES
}
SMITH_REDUCE_MAP = {
    getattr(smith, name): _smith_grad_reduce(name) for name in REDUCE_NAMES
}
TENSOR_REDUCE_MAP = {
    getattr(smith.Tensor, name): _smith_grad_reduce(name) for name in REDUCE_NAMES
}

NATIVE_REDUCE_FNS = list(NATIVE_REDUCE_MAP.keys())
SMITH_REDUCE_FNS = list(SMITH_REDUCE_MAP.keys())
TENSOR_REDUCE_FNS = list(TENSOR_REDUCE_MAP.keys())


def _is_reduction(fn):
    return fn in NATIVE_REDUCE_MAP or fn in SMITH_REDUCE_MAP or fn in TENSOR_REDUCE_MAP


def _apply_reduction(fn, *args, **kwargs):
    if fn in NATIVE_REDUCE_MAP:
        return NATIVE_REDUCE_MAP[fn](*args, **kwargs)
    if fn in SMITH_REDUCE_MAP:
        return SMITH_REDUCE_MAP[fn](*args, **kwargs)
