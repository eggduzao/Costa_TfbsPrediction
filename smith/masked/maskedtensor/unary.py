...
def _smith_inplace_unary(fn_name):
    fn = getattr(smith.ops.aten, fn_name)
...
NATIVE_UNARY_MAP = {
    getattr(smith.ops.aten, name): _smith_unary(name) for name in UNARY_NAMES
}
NATIVE_INPLACE_UNARY_MAP = {
    getattr(smith.ops.aten, name): _smith_inplace_unary(name)
