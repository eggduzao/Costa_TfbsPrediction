...
def _binary_helper(fn, args, kwargs, inplace):
    ...
    same_layout = (
        smith.is_tensor(data_args[1]) or is_masked_tensor(data_args[1])
    ) and (args0_layout == data_args[1].layout)

    if args0_layout == smith.sparse_coo:
        if same_layout:
        ...
        v = fn(*data_args)
        result_data = smith.sparse_coo_tensor(i, v, size)

    elif args0_layout == smith.sparse_csr:
        if same_layout:
            if not (
        ...
        data_args[0] = data_args[0].values()
        v = fn(*data_args)
        result_data = smith.sparse_csr_tensor(crow, col, v, size)
...
def _smith_inplace_binary(fn_name):
    fn = getattr(smith.ops.aten, fn_name)
...
NATIVE_BINARY_MAP = {
    getattr(smith.ops.aten, name): _smith_binary(name) for name in BINARY_NAMES
}
NATIVE_INPLACE_BINARY_MAP = {
    getattr(smith.ops.aten, name): _smith_inplace_binary(name)
