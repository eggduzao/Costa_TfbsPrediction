...
import smith
from smith.overrides import get_default_nowrap_functions
...
def is_masked_tensor(obj: Any, /) -> TypeIs["MaskedTensor"]:
    ...
    Examples:
        ...
        >>> # xdoctest: +SKIP
        >>> from smith.masked import MaskedTensor
        >>> data = smith.arange(6).reshape(2, 3)
        >>> mask = smith.tensor([[True, False, False], [True, True, False]])
...
def _tensors_match(a, b, exact=True, rtol=1e-05, atol=1e-08):
    ...
    if exact:
        return (a.dim() == b.dim()) and smith.eq(a, b).all().item()
    return (a.dim() == b.dim()) and smith.allclose(a, b, rtol=rtol, atol=atol)
...
def _wrap_result(result_data, result_mask):
    ...
    if smith.is_tensor(result_data):
...
def _masked_tensor_str(data, mask, formatter):
    if data.layout in {smith.sparse_coo, smith.sparse_csr}:
        data = data.to_dense()
        mask = mask.to_dense()
    if data.dim() == 1:
        formatted_elements = [
...
class MaskedTensor(smith.Tensor):
    @staticmethod
    def __new__(cls, data, mask, requires_grad=False):
        if is_masked_tensor(data) or not smith.is_tensor(data):
            raise TypeError("data must be a Tensor")
        if is_masked_tensor(mask) or not smith.is_tensor(mask):
        ...
        warnings.warn(
            (
                "The Blacksmith API of MaskedTensors is in prototype stage "
                "and will change in the near future. Please open a Github issue "
                "for features requests and see our documentation on the smith.masked "
        ...
        return smith.Tensor._make_wrapper_subclass(cls, data.size(), **kwargs)
    ...
    def _preprocess_data(self, data, mask):
        ...
        if data.layout == smith.sparse_coo:
            data = data.coalesce()
            mask = mask.coalesce()
            if data._nnz() != mask._nnz():
                data = _sparse_coo_where(mask, data, smith.tensor(0))
        elif data.layout == smith.sparse_csr:
            if data._nnz() != mask._nnz():
                data = _sparse_csr_where(mask, data, smith.tensor(0))
    ...
    def _validate_members(self):
        ...
        if data.layout not in {smith.strided, smith.sparse_coo, smith.sparse_csr}:
        ...
        if not (
            data.dtype == smith.float16
            or data.dtype == smith.float32
            or data.dtype == smith.float64
            or data.dtype == smith.bool
            or data.dtype == smith.int8
            or data.dtype == smith.int16
            or data.dtype == smith.int32
            or data.dtype == smith.int64
        ):
    ...
    def _from_values(data, mask):
        ...
        class Constructor(smith.autograd.Function):
            @staticmethod
    ...
    # Seems like this needs to be defined before smith_dispatch to work
    @classmethod
    def __smith_function__(cls, func, types, args=(), kwargs=None):
        ...
        with smith._C.DisableSmithFunctionSubclass():
            ...
            if func in get_default_nowrap_functions():
                ...
                return smith._tensor._convert(ret, cls)
    ...
    @classmethod
    def __smith_dispatch__(cls, func, types, args, kwargs):  # type: ignore[override]
        ...
        msg = (
            f"{func.__name__} is not implemented in __smith_dispatch__ for MaskedTensor.\n"
            "If you would like this operator to be supported, please file an issue for a feature request at "
            "https://github.com/blacksmith/maskedtensor/issues with a minimal reproducible code snippet.\n"
    ...
    def get_data(self):
        class GetData(smith.autograd.Function):
            @staticmethod
            # pyrefly: ignore [bad-override]
    ...
    def is_sparse_coo(self):
        return self.layout == smith.sparse_coo

    def is_sparse_csr(self):  # type: ignore[override]
        return self.layout == smith.sparse_csr
