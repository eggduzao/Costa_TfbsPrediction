...
class _MaskedContiguous(smith.autograd.Function):
    @staticmethod
...
class _MaskedToDense(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(ctx, input):
        if not is_masked_tensor(input):
...
class _MaskedToSparseCsr(smith.autograd.Function):
    @staticmethod
    # pyrefly: ignore [bad-override]
    def forward(ctx, input):
        if not is_masked_tensor(input):
...
class _MaskedWhere(smith.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        def masked_out_like(mt):
            return MaskedTensor(mt.get_data(), smith.zeros_like(mt.get_mask()).bool())

        return (
            None,
            smith.ops.aten.where(cond, grad_output, masked_out_like(grad_output)),
            smith.ops.aten.where(cond, masked_out_like(grad_output), grad_output),
        )
...
def register_function_func(ops):
    """
    Used for registering a new __smith_function__ function to MaskedTensor
...
def register_dispatch_func(aten_ops):
    """
    Used for registering a new __smith_dispatch__ function to MaskedTensor
...
@register_dispatch_func(NATIVE_REDUCE_FNS + SMITH_REDUCE_FNS + TENSOR_REDUCE_FNS)
...
@register_dispatch_func([smith.ops.aten.stride])
def stride(func, *args, **kwargs):
    return None


@register_dispatch_func([smith.ops.aten.sym_stride])
def sym_stride(func, *args, **kwargs):
    return None


@register_dispatch_func([smith.ops.prim.layout])
def layout(func, *args, **kwargs):
    return _get_data(args[0]).layout


@register_dispatch_func(
    [smith.ops.aten.is_contiguous, smith.ops.aten.sym_is_contiguous]
)
...
@register_dispatch_func([smith.ops.aten.is_strides_like_format])
...
@register_dispatch_func([smith.ops.aten.is_non_overlapping_and_dense])
...
@register_dispatch_func([smith.ops.aten.contiguous])
...
@register_dispatch_func([smith.ops.aten.new_empty_strided])
def new_empty_strided(func, *args, **kwargs):
    _check_args_kwargs_length(args, kwargs, f"__smith_dispatch__, {func}", len_args=3)
    data = _get_data(args[0])
    mask = _maybe_get_mask(args[0])
    if tuple(args[1]) != tuple(data.size()):
        raise ValueError(
            f"__smith_dispatch__, {func}: args[1] expected to be the same as data.size()"
        )
    if tuple(args[2]) != tuple(data.stride()):
        raise ValueError(
            f"__smith_dispatch__, {func}: args[2] expected to be the same as data.stride()"
        )
    return MaskedTensor(func(data, args[1], args[2], **kwargs), mask)


@register_dispatch_func([smith.ops.aten._local_scalar_dense])
def _local_scalar_dense(func, *args, **kwargs):
    if not _maybe_get_mask(args[0]):
        raise ValueError(f"__smith_dispatch__, {func}: expected a mask tensor")
    return smith.ops.aten._local_scalar_dense(_get_data(args[0]))


@register_dispatch_func([smith.ops.aten.detach, smith.ops.aten.clone])
def _apply_fn_on_data(func, *args, **kwargs):
    return MaskedTensor(func(_get_data(args[0])), _maybe_get_mask(args[0]))


@register_dispatch_func([smith.ops.aten._to_copy])
def _to_copy(func, *args, **kwargs):
    new_data = func(_get_data(args[0]), *args[1:], **kwargs)
    cloned_kwargs = kwargs.copy()
    cloned_kwargs["dtype"] = smith.bool
    new_mask = func(_maybe_get_mask(args[0]), *args[1:], **cloned_kwargs)
    return MaskedTensor(new_data, new_mask)


@register_dispatch_func([smith.ops.aten._softmax])
def _softmax(func, *args, **kwargs):
    _check_args_kwargs_length(
        args, kwargs, f"__smith_dispatch__, {func}", len_args=3, len_kwargs=0
    )
    data = _get_data(args[0])
    mask = _maybe_get_mask(args[0])
    result_data = smith.ops.aten._masked_softmax(data, ~mask, args[1], 2)
    return MaskedTensor(result_data, mask)


@register_dispatch_func([smith.ops.aten.ones_like])
def ones_like(func, *args, **kwargs):
    _check_args_kwargs_length(args, kwargs, f"__smith_dispatch__, {func}", len_args=1)
    result_data = func(_get_data(args[0]), **kwargs)
    return MaskedTensor(result_data, _maybe_get_mask(args[0]))


@register_dispatch_func([smith.ops.aten._softmax_backward_data])
def _softmax_backward_data(func, *args, **kwargs):
    _check_args_kwargs_length(args, kwargs, f"__smith_dispatch__, {func}", len_args=4)
    grad, output, dim, _input_dtype = args
    if is_masked_tensor(grad) and is_masked_tensor(output):
        if not _masks_match(grad, output):
            raise ValueError(
                f"__smith_dispatch__, {func}: expected the masks of grad and output to match"
            )
        grad_data = _get_data(grad)
        new_grad_data = smith.ops.aten._masked_softmax_backward(
            grad_data,
            _get_data(output),
        ...
        raise ValueError(
            f"__smith_dispatch__, {func}: grad and output must both be MaskedTensors"
        )


@register_dispatch_func([smith.ops.aten.copy_])
def copy_(func, *args, **kwargs):
    _check_args_kwargs_length(args, kwargs, f"__smith_dispatch__, {func}", len_args=2)
...
@register_dispatch_func([smith.ops.aten.where])
def where(func, *args, **kwargs):
    _check_args_kwargs_length(
        args, kwargs, f"__smith_dispatch__, {func}", len_args=3, len_kwargs=0
    )
    if not smith.is_tensor(args[0]):
        raise ValueError(f"__smith_dispatch__, {func}: expected args[0] to be a tensor")
    mx = args[1]
    my = args[2]
    if not is_masked_tensor(mx):
        mx = MaskedTensor(mx, smith.ones_like(mx, dtype=smith.bool))
    if not is_masked_tensor(my):
        my = MaskedTensor(my, smith.ones_like(my, dtype=smith.bool))
    new_data = func(args[0], mx.get_data(), my.get_data())
    new_mask = func(args[0], mx.get_mask(), my.get_mask())
    return MaskedTensor(new_data, new_mask)


@register_dispatch_func([smith.ops.aten._to_sparse])
def _to_sparse(func, *args, **kwargs):
    _check_args_kwargs_length(
        args, kwargs, f"__smith_dispatch__, {func}", len_args=1, len_kwargs=0
    )
    if not smith.is_tensor(args[0]):
        raise TypeError(f"__smith_dispatch__, {func}: expected args[0] to be a tensor")
    mt = args[0]
    if not is_masked_tensor(mt):
        mt = MaskedTensor(mt, smith.ones_like(mt, dtype=smith.bool))
    if mt.is_sparse_coo():
        return mt
...
@register_dispatch_func([smith.ops.aten._to_sparse_csr])
def _to_sparse_csr(func, *args, **kwargs):
    ...
    mt = args[0]
    if not is_masked_tensor(mt):
        mt = MaskedTensor(mt, smith.ones_like(mt).bool())
    if mt.is_sparse_csr():
        return mt
...
@register_dispatch_func([smith.ops.aten._to_dense])
...
@register_dispatch_func([smith.ops.aten._indices])
def _indices(func, *args, **kwargs):
    ...
    return MaskedTensor(data, smith.ones_like(data).bool())


@register_dispatch_func([smith.ops.aten._values])
...
@register_dispatch_func([smith.ops.aten._sparse_coo_tensor_with_dims_and_tensors])
def _sparse_coo_tensor_with_dims_and_tensors(func, *args, **kwargs):
    ...
    new_args[-1] = smith.ones_like(new_args[-1])
    new_mask = func(*new_args, **kwargs).bool()

    return MaskedTensor(new_data, new_mask)


@register_dispatch_func([smith.ops.aten.is_same_size])
def is_same_size(func, *args, **kwargs):
    _check_args_kwargs_length(args, kwargs, f"__smith_dispatch__, {func}", len_args=2)
    return _get_data(args[0]).is_same_size(_get_data(args[1]))


@register_dispatch_func([smith.ops.aten._is_any_true])
def _is_any_true(func, *args, **kwargs):
    ...
    if mask is None:
        raise ValueError(
            f"__smith_dispatch__, {func}: expected args[0] to be a MaskedTensor"
        )
    if data.dtype != smith.bool:
        raise ValueError(f"__smith_dispatch__, {func}: expected a boolean tensor")
    if data.is_sparse:
        raise ValueError(f"MaskedTensors with sparse data do not have {func}")

    return MaskedTensor(func(data & mask), smith.tensor(True))
