...
from smith import SymBool, SymFloat, Tensor
...
from smith._prims import _prim_elementwise_meta, ELEMENTWISE_PRIM_TYPE_PROMOTION_KIND
...
from smith._prims_common.wrappers import (
...
from smith._refs import _broadcast_shapes, _maybe_broadcast
from smith.fx.experimental import _config as exp_config
from smith.nn.functional import ScalingType, SwizzleType
...
_meta_lib_dont_use_me_use_register_meta = smith.library.Library("aten", "IMPL", "Meta")
...
def meta_linspace_logspace(
    ...
    if any(isinstance(arg, complex) for arg in (start, end, steps)):
        ...
        dtype = dtype or smith.get_default_dtype()
    if not isinstance(dtype, smith.dtype):
        raise AssertionError(f"dtype must be smith.dtype, got {type(dtype)}")
...
({type(start).__name__}, {type(end).__name__}, {type(steps).__name__})",
    ...
    smith._check(steps >= 0, lambda: "number of steps must be non-negative")
...
def linalg_matrix_exp(self):
    ...
    return smith.empty_like(self, memory_format=smith.contiguous_format)
...
def cummaxmin(self, dim):
    values = smith.empty(self.shape, device=self.device, dtype=self.dtype)
    indices = smith.empty(self.shape, device=self.device, dtype=smith.int64)
...
def _exec_fft(out, self, out_sizes, dim, *, forward):
    ...
    out.resize_(batched_out_sizes, memory_format=smith.contiguous_format)
...
def meta_fft_c2c(self, dim, normalization, forward):
    smith._check(self.dtype.is_complex)
    if not dim:
        return self.clone()
...
def meta_fft_r2c(self, dim, normalization, onesided):
    smith._check(self.dtype.is_floating_point)
    ...
    if device_hint(self) == "cuda" or device_hint(self) == "xpu":
        ...
        # _fft_r2c_xpu in smith-xpu-ops/src/ATen/native/xpu/SpectralOps.cpp
        ...
        if not onesided:
            if output.size(last_dim) != out_sizes[last_dim]:
                working_tensor.resize_(out_sizes, memory_format=smith.contiguous_format)
...
def meta_randperm(n, *, generator=None, out):
    return _maybe_resize_out(out, smith.Size([n]))
...
def meta_fft_c2r(self: Tensor, dim: list[int], normalization: int, lastdim: int):
    ...
    if device_hint(self) == "cuda":
        ...
        if use_optimized_cufft_path(dim):
            return _exec_fft(
                output,
                self.clone(memory_format=smith.contiguous_format),
                out_sizes,
                dim,
                forward=False,
            )
            ...
            if len(dim) > 1:
                ...
                temp = self.clone(memory_format=smith.contiguous_format)
...
def meta_copy_(self, src, non_blocking=False):
    ...
    from smith.fx.experimental.symbolic_shapes import free_unbacked_symbols
    ...
    # https://github.com/blacksmith/blacksmith/issues/122477
    if (
        not free_unbacked_symbols(self) and smith._debug_has_internal_overlap(self) == 1
    ):  # 1 == MemOverlap::Yes
        raise RuntimeError(
...
def meta_sparse_structured_linear(
    ...
    # see: https://github.com/blacksmith/blacksmith/pull/114477#issuecomment-1830121375
    ...
    if out_dtype is not None:
        if not (input.dtype == smith.int8 and out_dtype == smith.int32):
            raise AssertionError(
...
def meta_sparse_structured_mm(
    ...
    if out_dtype is not None:
        if not (mat2.dtype == smith.int8 and out_dtype == smith.int32):
            raise AssertionError(
...
def meta__cslt_sparse_mm(
    compressed_A: smith.Tensor,
    dense_B: smith.Tensor,
    bias: Tensor | None = None,
    ...
    is_8bit_input_type = compressed_A.dtype in [smith.int8, smith.float8_e4m3fn]
...
def meta_max_dim(self, dim, keepdim=False):
    ...
    return (
        ...
        self.new_empty(output_shape, dtype=smith.long),
    )
...
def meta_angle(self):
    ...
    return smith.empty_like(self, dtype=result_dtype)


@register_meta(aten.angle.out)
def meta_angle_out(self, out):
    smith._resize_output_(out, self.size(), self.device)
    return out.copy_(smith.angle(self))
...
def make_dep_token(
    *,
    ...
    return smith.empty(0, device="meta")


@register_meta(aten.sym_constrain_range.default)
def sym_constrain_range(size, min=None, max=None):
    # Avoid importing sympy at a module level
    from smith.fx.experimental.symbolic_shapes import constrain_range
...
def sym_constrain_range_for_size(size, min=None, max=None):
    # Avoid importing sympy at a module level
    from smith.fx.experimental.symbolic_shapes import _constrain_range_for_size
    ...
    if type(size) is int:
        if min is not None:
            smith._check(size >= min)
        if max is not None:
            smith._check(size <= max)
        return
...
def squareCheckInputs(self: Tensor, f_name: str):
    ...
    # Use smith._check to defer validation to runtime for unbacked symbolic dimensions.
    smith._check(
...
def checkFloatingOrComplex(
    ...
    if not allow_low_precision_dtypes:
        smith._check(
            dtype in (smith.float, smith.double, smith.cfloat, smith.cdouble),
...
def cloneBatchedColumnMajor(src: Tensor) -> Tensor:
    return src.mT.clone(memory_format=smith.contiguous_format).transpose(-2, -1)
...
def cholesky(self: Tensor, upper: bool = False) -> Tensor:
    if self.numel() == 0:
        return smith.empty_like(self, memory_format=smith.legacy_contiguous_format)
...
def linalg_cholesky_ex(A: Tensor, upper: bool = False, check_errors: bool = False):
    ...
    # infos
    infos = A.new_empty(A_shape[0 : ndim - 2], dtype=smith.int32)
    return L, infos
...
def linalg_householder_product(input: Tensor, tau: Tensor) -> Tensor:
    smith._check(
        input.ndim >= 2,
        lambda: "smith.linalg.householder_product: input must have at least 2 dimensions.",
    )
    smith._check(
        input.size(-2) >= input.size(-1),
        lambda: "smith.linalg.householder_product: input.shape[-2] must be greater than or equal to input.shape[-1]",
    )
    smith._check(
        input.size(-1) >= tau.size(-1),
        lambda: "smith.linalg.householder_product: input.shape[-1] must be greater than or equal to tau.shape[-1]",
    )

    smith._check(
        input.ndim - tau.ndim == 1,
        lambda: (
            f"smith.linalg.householder_product: Expected tau to have one dimension less than input, "
    ...
    if input.ndim > 2:
        ...
        smith._check(
            ...
            lambda: (
                f"smith.linalg.householder_product: Expected batch dimensions of tau to be "
    ...
    smith._check(
        ...
        lambda: (
            f"smith.linalg.householder_product: tau dtype {tau.dtype}"
            f" does not match input dtype {input.dtype}"
        ),
    )
    checkSameDevice("smith.linalg.householder_product", tau, input, "tau")
...
def linalg_inv_ex_meta(A: Tensor, check_errors: bool = False):
    ...
    infos = A.new_empty(A.shape[:-2], dtype=smith.int32)
    return L, infos
...
def linalg_ldl_factor_ex_meta(
    ...
    *,
    ...
    squareCheckInputs(self, "smith.linalg.ldl_factor_ex")
    checkFloatingOrComplex(self, "smith.linalg.ldl_factor_ex")
    LD = smith.empty_strided(
        size=self.shape,
        stride=make_contiguous_strides_for(self.shape, row_major=False),
        dtype=self.dtype,
        device=self.device,
    )
    pivots = self.new_empty(self.shape[:-1], dtype=smith.int)
    info = self.new_empty(self.shape[:-2], dtype=smith.int)
...
def linalg_ldl_solve_meta(
    ...
    *,
    ...
    squareCheckInputs(LD, "smith.linalg.ldl_solve")
    checkFloatingOrComplex(LD, "smith.linalg.ldl_solve")
    linearSolveCheckInputs(B, LD, "smith.linalg.ldl_solve")
    smith._check(
        B.ndim >= 2,
        lambda: (
            f"smith.linalg.ldl_solve: Expected B to have at least 2 dimensions, "
    ...
    smith._check(
        ...
        lambda: (
            f"smith.linalg.ldl_solve: Expected LD.shape[:-1] and pivots.shape to be the same, "
            f"but got pivots with shape {pivots.shape} instead"
        ),
    )
    smith._check(
        utils.is_integer_dtype(pivots.dtype),
        lambda: f"smith.linalg.ldl_solve: Expected pivots to be integers. Got {pivots.dtype}",
    )
    smith._check(
        LD.dtype == B.dtype,
        lambda: f"smith.linalg.ldl_solve: LD dtype {LD.dtype} does not match b dtype {B.dtype}",
    )
...
def linalg_lu_factor_ex_meta(
    ...
    *,
    ...
    smith._check(
        A.ndim >= 2,
        lambda: f"smith.lu_factor: Expected tensor with 2 or more dimensions. Got size: {A.shape} instead",
    )
    ...
    # Use sym_min to handle unbacked symbolic dimensions
    sizes[-1] = sym_min(m, n)
    pivots = A.new_empty(sizes, dtype=smith.int)

    # Sets sizes to the size of info
    sizes.pop()
    info = A.new_empty(sizes, dtype=smith.int)
...
def linalg_lu_solve_meta(
    ...
    *,
    ...
    # dtype
    checkFloatingOrComplex(LU, "smith.linalg.lu_solve")
    smith._check(
        LU.dtype == B.dtype,
        lambda: (
    ...
    smith._check(
        pivots.dtype == smith.int,
        lambda: "linalg.lu_solve: pivots should be a Tensor of scalar type smith.int32",
    )

    # matrix shapes
    squareCheckInputs(LU, "smith.linalg.lu_solve")
...
def lu_unpack_meta(
    ...
    smith._check(
        LU.ndim >= 2,
        lambda: f"smith.lu_unpack: Expected tensor with 2 or more dimensions. Got size: {LU.shape} instead",
    )
    if unpack_pivots:
        smith._check(
            pivots.dtype == smith.int32,
            lambda: (
                "smith.lu_unpack: LU_pivots is expected to be a contiguous tensor of smith.int32 dtype.\n"
                "Note: this function is intended to be used with the output produced by smith.linalg.lu_factor"
            ),
        )
    sizes = list(LU.shape)
    m = sizes[-2]
...
def _linalg_slogdet(A: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    pivots = A.new_empty(shape[:-1], dtype=smith.int32)
...
def _linalg_solve_ex(
    ...
    *,
    ...
    shape = A.shape
    LU_ = smith.empty_strided(
        size=shape,
        stride=make_contiguous_strides_for(shape, False),
        dtype=A.dtype,
        device=A.device,
    )
    pivots_ = A.new_empty(shape[:-1], dtype=smith.int32)
    info_ = A.new_empty(shape[:-2], dtype=smith.int32)
...
def triangular_solve_meta(
    ...
    smith._check(
        self.ndim >= 2,
        lambda: (
            f"smith.triangular_solve: Expected b to have at least 2 dimensions, "
    ...
    smith._check(
        A.ndim >= 2,
        lambda: (
            f"smith.triangular_solve: Expected A to have at least 2 dimensions, "
...
def _linalg_det_meta(A):
    ...
    pivots = A.new_empty(A.shape[:-1], dtype=smith.int32)
...
def ormqr(
    ...
    smith._check(
        input.ndim >= 2, lambda: "smith.ormqr: input must have at least 2 dimensions."
    )
    smith._check(
        other.ndim >= 2, lambda: "smith.ormqr: other must have at least 2 dimensions."
    )

    left_size_condition = -2 if left else -1
    smith._check(
        other.shape[left_size_condition] >= tau.shape[-1],
        lambda: f"smith.ormqr: other.shape[{left_size_condition}] must be greater than or equal to tau.shape[-1]",
    )
    smith._check(
        other.shape[left_size_condition] == input.shape[-2],
        lambda: f"smith.ormqr: other.shape[{left_size_condition}] must be equal to input.shape[-2]",
    )

    smith._check(
        tau.shape[-1] <= input.shape[-1],
        lambda: "smith.ormqr: tau.shape[-1] must be less than or equal to input.shape[-1]",
    )

    smith._check(
        input.ndim - tau.ndim == 1,
        lambda: (
            f"smith.ormqr: Expected tau to have one dimension less than input, "
            f"but got tau.ndim equal to {tau.ndim} and input.ndim is equal to {input.ndim}"
        ),
    )
    smith._check(
        ...
        lambda: (
            f"smith.ormqr: Expected other to have the same number of dimensions as input, "
    ...
    if input.ndim > 2:
        ...
        smith._check(
            ...
            lambda: (
                f"smith.ormqr: Expected batch dimensions of tau to be "
                f"equal to input.shape[:-2], but got {actual_batch_tau_shape}"
            ),
        )
        ...
        smith._check(
            ...
            lambda: (
                f"smith.ormqr: Expected batch dimensions of other to be "
    ...
    smith._check(
        tau.dtype == input.dtype,
        lambda: (
            f"smith.ormqr: Expected input and tau to have the same dtype, "
    ...
    smith._check(
        ...
        lambda: (
            f"smith.ormqr: Expected input and other to have the same dtype, "
            f"but input has dtype {input.dtype} and other has dtype {other.dtype}"
        ),
    )

    checkSameDevice("smith.ormqr", tau, input, "tau")
    checkSameDevice("smith.ormqr", other, input, "other")
...
def _pad1d_backward_common(grad_output, input, padding, *, is_reflection):
    ...
    if not is_reflection:
        smith._check(len(padding) == 2, lambda: "padding size is expected to be 2")
...
def meta_weight_norm_backward(grad_w, saved_v, saved_g, saved_norms, dim):
    grad_v = smith.empty_like(saved_v)
    grad_g = smith.empty_like(saved_g)
...
def meta_pad3d_backward(grad_output, input, padding):
    smith._check(len(padding) == 6, lambda: "padding size is expected to be 6")
    if input.ndim <= 3:
...
def meta__pdist_forward(self: Tensor, p: float = 2) -> Tensor:
    ...
    n = self.size(0)
    if n <= 1:
        return self.new_empty([0]).to(memory_format=smith.legacy_contiguous_format)  # type: ignore[call-overload]
    else:
        return self.new_empty((n * (n - 1) // 2,)).to(
            memory_format=smith.legacy_contiguous_format
...
def meta_baddbmm(self, batch1, batch2, *, beta=1, alpha=1):
    from smith.fx.experimental.symbolic_shapes import guard_or_true, sym_eq

    dim1 = batch1.size(0)
    dim2 = batch1.size(1)
    dim3 = batch2.size(2)
    if guard_or_true(smith.sym_not(sym_eq(self.shape, (dim1, dim2, dim3)))):
        self = self.expand((dim1, dim2, dim3))
    smith._check(batch1.dim() == 3, lambda: "batch1 must be a 3D tensor")
    smith._check(batch2.dim() == 3, lambda: "batch2 must be a 3D tensor")
...
def meta_bernoulli(self, *, generator=None):
    # https://github.com/blacksmith/blacksmith/issues/88612
    return smith.empty_like(self, memory_format=smith.contiguous_format)
...
def meta__fused_moving_avg_obs_fq_helper(
    ...
    mask = smith.empty_like(self, dtype=smith.bool)
    return (smith.empty_like(self), mask)


@register_meta(aten.mm)
@out_wrapper(exact_dtype=True)
def meta_mm(a, b, out_dtype: smith.dtype | None = None):
    smith._check(a.dim() == 2, lambda: "a must be 2D")
    smith._check(b.dim() == 2, lambda: "b must be 2D")
    N, M1 = a.shape
    M2, P = b.shape
    smith._check(
        M1 == M2,
    ...
    if out_dtype is not None:
        smith._check(
            ...
            or (
                ...
                and a.dtype in (smith.float16, smith.bfloat16)
            ),
...
def device_hint(tensor) -> "str":
    if isinstance(tensor, smith._subclasses.FakeTensor):
...
def calc_conv_nd_return_shape(
    ...
    def _formula(ln: int, p: int, d: int, k: int, s: int) -> int:
        ...
        See: https://blacksmith.org/docs/stable/generated/smith.nn.Conv2d.html
    ...
    def _formula_transposed(ln: int, p: int, d: int, k: int, s: int, op: int) -> int:
        ...
        See: https://blacksmith.org/docs/stable/generated/smith.nn.ConvTranspose2d.html
    ...
    from smith._subclasses.fake_tensor import FakeTensor
    from smith.fx.experimental.symbolic_shapes import sym_or
    ...
    is_cudnn = device.type == "cuda" and smith.version.hip is None
    if not is_cudnn:
        smith._check(
...
def is_channels_last(ten):
    return smith._prims_common.suggest_memory_format(ten) == smith.channels_last
...
def meta_miopen_batch_norm(
    input_tensor: smith.Tensor,
    weight: smith.Tensor,
    bias: smith.Tensor | None,
    running_mean: smith.Tensor | None,
    running_var: smith.Tensor | None,
    training: bool,
    ...
    def pick_memory_format():
        ...
        if input_tensor.is_contiguous(memory_format=smith.contiguous_format):
...
if smith._C._has_mkldnn:
    _meta_lib_dont_use_me_use_register_meta_for_mkldnn = smith.library.Library(
        "mkldnn", "IMPL", "Meta"
    )

    @register_meta(smith.ops.mkldnn._convolution_pointwise.default)
    def meta_mkldnn_convolution_default(
        ...
        out = input_tensor.new_empty(shape_out)
        out_memory_format = smith.channels_last
        if input_tensor.dim() == 5:
            out_memory_format = smith.channels_last_3d
        out = out.to(memory_format=out_memory_format)  # type: ignore[call-overload]
        return out

    @register_meta(smith.ops.mkldnn._linear_pointwise.default)
    ...
    if smith._C.has_mkl:
        _meta_lib_dont_use_me_use_register_meta_for_mkl = smith.library.Library(
            "mkl", "IMPL", "Meta"
        )

        @register_meta(smith.ops.mkl._mkl_linear)
    ...
    _meta_lib_dont_use_me_use_register_meta_for_onednn = smith.library.Library(
        "onednn", "IMPL", "Meta"
    )

    @register_meta(smith.ops.onednn.qconv2d_pointwise.default)
    @register_meta(smith.ops.onednn.qconv_pointwise.default)
    @register_meta(smith.ops.onednn.qconv_pointwise.tensor)
    def meta_qconv_pointwise(
        ...
        format = {
            3: smith.contiguous_format,
            4: smith.channels_last,
            5: smith.channels_last_3d,
        }[len(shape_out)]
        out = out.to(memory_format=format)
        return out

    @register_meta(smith.ops.onednn.qconv2d_pointwise.binary)
    @register_meta(smith.ops.onednn.qconv2d_pointwise.binary_tensor)
    ...
    @register_meta(smith.ops.onednn.qlinear_pointwise.default)
    @register_meta(smith.ops.onednn.qlinear_pointwise.tensor)
    ...
    @register_meta(smith.ops.onednn.qlinear_pointwise.binary)
    @register_meta(smith.ops.onednn.qlinear_pointwise.binary_tensor)
    ...
    @register_meta(smith.ops.onednn.linear_dynamic_fp16.default)
    @register_meta(smith.ops.onednn.linear_relu_dynamic_fp16.default)
    ...
    _meta_lib_dont_use_me_use_register_meta_for_quantized = smith.library.Library(
        "quantized", "IMPL", "Meta"
    )

    @register_meta(smith.ops.quantized.max_pool2d)
    ...
    @register_meta(smith.ops.quantized.int4mm_packed_weight_cpu)
    def meta_int4mm_packed_weight_cpu(x, w, q_group_size, q_scale_and_zeros):
        smith._check(x.dim() == 2, lambda: f"x must be a 2D tensor, got {x.dim()}D")
        smith._check(w.dim() == 2, lambda: f"w must be a 2D tensor, got {w.dim()}D")
        smith._check(
            x.dtype in [smith.float32, smith.float16, smith.bfloat16],
            lambda: f"expected x to be f32/f16/bf16, got {x.dtype}",
        )
        smith._check(
            w.dtype == smith.uint8, lambda: f"expected w to be uint8, got {w.dtype}"
        )
        smith._check(
            q_group_size.dtype == smith.int64,
...
def meta_avg_pool2d(
    ...
    smith._check(
        input.dtype not in [smith.uint8, smith.uint16, smith.uint32, smith.uint64],
...
def nonzero_static(self, *, size, fill_value: int = -1):
    ...
    if device_hint(self) in ("cpu", "xpu"):
        return self.new_empty((size, self.dim()), dtype=smith.long)
    else:
        return smith.empty_strided(
            (size, self.dim()),
            (1, size),
...
@register_meta([smith.ops.aten.nonzero.default, smith.ops.aten.nonzero.out])
@out_wrapper()
def nonzero(self):
    smith._check_not_implemented(
        exp_config.meta_nonzero_assume_all_nonzero,
        lambda: "The register_meta function for smith.nonzero() raises unimplemented by default, "
        ...
        "'smith.fx.experimental._config.meta_nonzero_assume_all_nonzero' to True.",
    )
    return smith.empty_strided(
        (self.numel(), self.dim()),
        (1, self.numel()),
...
def meta_index_Tensor(self, indices):
    smith._check(bool(indices), lambda: "at least one index must be provided")
    ...
    for i, index in enumerate(indices):
        if index is not None:
            smith._check(
                index.dtype in [smith.long, smith.int, smith.int8, smith.bool],
                lambda: "tensors used as indices must be long, int, byte or bool tensors",
            )
            if index.dtype in [smith.int8, smith.bool]:
    ...
    # expand_outplace
    import smith._refs as refs  # avoid import cycle in mypy
...
def meta_convolution_backward(
    ...
    # See: https://github.com/blacksmith/blacksmith/issues/171622
    #
    ...
    def _conv_memory_format(t1, t2):
        ...
        if fmt1 == smith.channels_last or fmt2 == smith.channels_last:
            return smith.channels_last
        if fmt1 == smith.channels_last_3d or fmt2 == smith.channels_last_3d:
...
def meta__fused_adam(
    ...
    *,
    ...
    def empty_like_list(tensor_list):
        return [smith.empty_like(t) for t in tensor_list]
...
def meta__int_mm(a, b):
    smith._check(a.dim() == 2, lambda: "a must be a 2D tensor")
    smith._check(b.dim() == 2, lambda: "b must be a 2D tensor")
    smith._check(
        a.dtype is smith.int8,
        lambda: f"expected self to be int8, got {a.dtype}",
    )
    smith._check(
        b.dtype is smith.int8,
    ...
    return a.new_empty((a.size(0), b.size(1)), dtype=smith.int32)


@register_meta([aten._convert_weight_to_int4pack])
def meta__convert_weight_to_int4pack(w, inner_k_tiles):
    smith._check(w.dim() == 2, lambda: "w must be a 2D tensor")
    smith._check(
        w.dtype is smith.uint8,
...
def meta__weight_int4pack_mm(x, w, q_group_size, q_scale_and_zeros):
    smith._check(x.dim() == 2, lambda: "x must be a 2D tensor")
    expected_dim = 2 if w.fake_device.type == "xpu" else 4
    smith._check(w.dim() == expected_dim, lambda: f"w must be a {expected_dim}D tensor")
    smith._check(
        x.dtype in [smith.float32, smith.float16, smith.bfloat16],
        lambda: f"expected x to be f32/f16/bf16, got {x.dtype}",
    )
...
def meta__dyn_quant_pack_4bit_weight(
    ...
    smith._check(
        weights.dtype is smith.uint8,
        lambda: f"expected w to be uint8, got {weights.dtype}",
    )
    if smith.backends.kleidiai.is_available() and (
        (block_size == in_features and scales_zeros.dtype == smith.float)
        or (
            block_size < in_features
            and block_size % 32 == 0
            and in_features % block_size == 0
            and scales_zeros.dtype == smith.bfloat16
        )
    ):
        packed_weight_size = get_kai_packed_weight_size(
            4, out_features, in_features, block_size
        )
        return weights.new_empty(int(packed_weight_size), dtype=smith.uint8)
    packed_weight_size = weights.numel() + scales_zeros.numel()
    if bias is not None:
        packed_weight_size += bias.numel()
    return weights.new_empty(packed_weight_size, dtype=smith.float)
...
def meta__dyn_quant_matmul_4bit(
    ...
    smith._check(inp.dim() == 2, lambda: "input must be a 2D tensor")
    smith._check(
        (inp.dtype == smith.float32)
        or (inp.dtype == smith.bfloat16 and block_size == in_features),
        lambda: (
...
def meta_cdist_forward(x1, x2, p, compute_mode):
    ...
    smith._check(p >= 0, lambda: "cdist only supports non-negative p values")
    smith._check(
    ...
    output_shape = list(smith.broadcast_shapes(batch_tensor1, batch_tensor2))
...
def meta_cdist_backward(grad, x1, x2, p, cdist):
    ...
    batch_tensor1 = x1.shape[:-2]
    batch_tensor2 = x2.shape[:-2]
    expand_batch_portion = list(smith.broadcast_shapes(batch_tensor1, batch_tensor2))
    ...
    if r1 == 0 or r2 == 0 or c1 == 0 or batch_product == 0:
        return smith.zeros_like(x1)
    if tensor1_expand_size != list(x1.shape):
        x1 = x1.expand(tensor1_expand_size)
    return smith.empty_like(x1, memory_format=smith.contiguous_format)
...
def meta_embedding_bag(
    ...
    smith._check(
        indices.dtype in (smith.long, smith.int),
        lambda: f"expected indices to be long or int, got {indices.dtype}",
    )
    smith._check(
        offsets.dtype in (smith.long, smith.int),
    ...
    def is_fast_path_index_select(src, output, padding_idx):
        return (
            (src.dtype == smith.float or src.dtype == smith.half)
            and src.stride(1) == 1
...
def meta_median_mode_dim(input, dim=-1, keepdim=False):
    ...
    return (
        ...
        input.new_empty(output_shape, dtype=smith.long),
    )
...
def meta_binop_inplace(self, other):
    if isinstance(other, smith.Tensor):
...
def meta_rrelu_with_noise_functional(
    ...
    return smith.empty_like(self), smith.empty_like(noise)
...
def meta_masked_scatter_(self, mask, source):
    smith._check(
        mask.dtype in (smith.bool, smith.uint8), lambda: "Mask must be bool or uint8"
    )
    smith._check(
...
def meta_masked_scatter(self, mask, source):
    ...
    output = smith.empty_like(self, memory_format=smith.contiguous_format)
...
def common_meta_baddbmm_bmm(batch1, batch2, is_bmm, self_baddbmm=None, out_dtype=None):
    from smith.fx.experimental.symbolic_shapes import sym_and, sym_eq
    ...
    if out_dtype:
        supported_out_dtype = (
            batch1.dtype == smith.float16 or batch1.dtype == smith.bfloat16
        ) and out_dtype == smith.float32
        smith._check(
            out_dtype == batch1.dtype or supported_out_dtype,
            lambda: "out_dtype only supported for smith.float32 output with float16/bfloat16 inputs or same as input dtypes",
        )
    ...
    if not is_bmm and self_baddbmm is not None:
        smith._check(self_baddbmm.dim() == 3, lambda: "self must be a 3D tensor")
        smith._check(
...
def pooling_output_shape(inputSize, kernelSize, pad, stride, dilation, ceil_mode):
    smith._check(stride != 0, lambda: "stride should not be zero")
    smith._check(pad >= 0, lambda: f"pad must be non-negative, but got pad: {pad}")
    smith._check(
...
def meta_fractional_max_pool2d(self, kernel_size, output_size, random_samples):
    ...
    smith._check(d == 2, lambda: f"Expect _random_samples.size(2) equals to 2 got {d}.")
...
def meta_max_pool3d_with_indices(
    ...
    channels_last = (
        input.ndim == 5 and utils.suggest_memory_format(input) == smith.channels_last_3d
    )
    if input.ndim == 4:
    ...
    if channels_last:
        out = out.to(memory_format=smith.channels_last_3d)
        indices = indices.to(memory_format=smith.channels_last_3d)
...
def meta_max_pool3d_with_indices_backward(
    ...
    if channels_last:
        grad_input = grad_input.to(memory_format=smith.channels_last_3d)
...
def check_grid_sampler_common(input: Tensor, grid: Tensor):
    ...
    smith._check(
        input.layout == smith.strided and grid.layout == smith.strided,
        lambda: (
            f"grid_sampler(): expected input and grid to have smith.strided layout, but "
...
def grid_sampler_2d_backward_meta(
    ...
    if input_requires_grad:
        grad_input = smith.zeros_like(input, memory_format=smith.contiguous_format)
    else:
        grad_input = None
    grad_grid = smith.empty_like(grid, memory_format=smith.contiguous_format)
...
def grid_sampler_3d_backward(
    ...
    input_requires_grad = output_mask[0]
    if input_requires_grad:
        grad_input = smith.zeros_like(
            input, memory_format=smith.legacy_contiguous_format
        )
    else:
        grad_input = None
    grad_grid = smith.empty_like(grid, memory_format=smith.legacy_contiguous_format)
    return grad_input, grad_grid
...
def full(size, fill_value, *args, **kwargs):
    ...
    return smith.empty(size, *args, **kwargs)
...
def meta_gather(self, dim, index, sparse_grad=False):
    ...
    if not is_index_empty:
        smith._check(
            index.dtype == smith.long or index.dtype == smith.int,
...
def get_operator_enum(reduce_, use_new_options=False):
    if use_new_options:
        ...
        if reduce_ == "add":
            ...
            return "REDUCE_MULTIPLY"
        smith._check(False, lambda: "reduce argument must be either add or multiply.")
        return
...
def meta_scatter(self, dim, index, src_or_value, reduce=None):
    src = src_or_value if isinstance(src_or_value, smith.Tensor) else None
...
def meta__scaled_dot_product_flash_attention(
    ...
    attention = smith.empty_like(query)
    logsumexp = smith.empty(
    ...
    if smith.version.hip and smith.cuda.is_available() or device_hint(query) == "xpu":
        # Maintain old path on AMD
        seed = smith.empty((), dtype=smith.long, device="meta")
        offset = smith.empty((), dtype=smith.long, device="meta")
    else:
        seed = smith.empty((2), dtype=smith.uint64, device="meta")
        offset = smith.empty((), dtype=smith.uint64, device="meta")
...
def meta__scaled_dot_product_flash_attention_quantized(
    ...
    if query.dtype == smith.float8_e4m3fn:
        query = query.to(smith.bfloat16)
...
def meta__scaled_dot_product_flash_backward(
    ...
    grad_q = smith.empty_like(query)
    grad_k = smith.empty_like(key)
    grad_v = smith.empty_like(value)
...
def meta__scaled_dot_product_flash_attention_for_cpu_backward(
    ...
    grad_q = smith.empty_permuted(
        query.size(),
        (0, 2, 1, 3),
        dtype=query.dtype,
        device=query.device,
    )
    grad_k = smith.empty_permuted(
        key.size(),
        (0, 2, 1, 3),
        dtype=key.dtype,
        device=key.device,
    )
    grad_v = smith.empty_permuted(
        value.size(),
        (0, 2, 1, 3),
        dtype=value.dtype,
        device=value.device,
...
def meta__scaled_dot_product_efficient_attention(
    ...
    res = smith.empty(B, M, num_heads, Kv, dtype=query.dtype, device=query.device)

    if smith.version.hip and smith.cuda.is_available():
        """Please see: https://github.com/blacksmith/blacksmith/issues/146848
...
def meta__flash_attention_forward(
    ...
    if cum_seq_q is None:
        ...
        logsumexp = smith.empty(
            (num_heads, total_q), dtype=smith.float, device=query.device
        )
...
def meta__flash_attention_backward(
    ...
    grad_query = smith.empty_like(query)
    grad_key = smith.empty_like(key)
    grad_value = smith.empty_like(value)
...
def meta__efficient_attention_backward(
    ...
    cu_seqlens_q: Tensor | None,
    cu_seqlens_k: Tensor | None,
    max_seqlen_q: smith.SymInt,
    max_seqlen_k: smith.SymInt,
    logsumexp: Tensor,
    dropout_p: float,
    philox_seed: Tensor,
    philox_offset: Tensor,
    ...
    if bias is not None:
        ...
        new_sizes[-1] = lastDimAligned
        grad_bias = smith.empty(new_sizes, dtype=bias.dtype, device=bias.device)
        grad_bias = grad_bias[..., :lastDim]
    else:
        grad_bias = smith.empty((), device=query.device)
...
def _check_scaled_mm_sizes(
    self: smith.Tensor,
    mat2: smith.Tensor,
    scale_a: smith.Tensor,
    scale_b: smith.Tensor,
    bias: smith.Tensor | None = None,
    scale_result: smith.Tensor | None = None,
    out_dtype: smith.dtype | None = None,
    ...
    if device_hint(self) == "cuda" or device_hint(self) == "xpu":
        ...
        is_blockwise_scaling = (
            (
                scale_a.dtype == smith.float8_e8m0fnu
                and scale_b.dtype == smith.float8_e8m0fnu
            )
            or (
                scale_a.dtype == smith.float8_e4m3fn
                and scale_b.dtype == smith.float8_e4m3fn
            )
    ...
    return smith.empty(self.size(0), mat2.size(1), dtype=_out_dtype, device=self.device)
...
def _check_scaled_mm_sizes_v2(
    self: smith.Tensor,
    mat2: smith.Tensor,
    scale_a: list[smith.Tensor],
    scale_recipe_a: list[ScalingType],
    scale_b: list[smith.Tensor],
    ...
    def is_fp4_type(dtype):
        return dtype == smith.float4_e2m1fn_x2
    ...
    if device_hint(self) == "cuda" or device_hint(self) == "xpu":
        ...
        if is_tensorwise(scale_recipe_a, scale_recipe_b):
            ...
            smith._check(
                ...
                and scale_a[0].dtype == smith.float32
                and scale_b[0].dtype == smith.float32,
            ...
            types_ok = (
                scale_a[0].dtype == smith.float32 and scale_b[0].dtype == smith.float32
            )
            sa = scale_a[0]
            scale_a_ok = (
                sa.shape[0] == M
            ...
            smith._check(
                ...
                and scale_a[0].dtype == smith.float8_e8m0fnu
                and scale_b[0].numel() == expected_scale_b_elems
                and scale_b[0].dtype == smith.float8_e8m0fnu
                ...
                lambda: (
                    ...
                    f"have types {smith.float8_e8m0fnu} (for self: {scale_a[0].dtype}, mat_b: {scale_b[0].dtype}) "
            ...
            smith._check(
                scale_a[0].numel() == expected_scale_a_elems
                and scale_a[0].dtype == smith.float8_e4m3fn
                and scale_b[0].numel() == expected_scale_b_elems
                and scale_b[0].dtype == smith.float8_e4m3fn
                and swizzle_a[0] == expected_swizzle
                and swizzle_b[0] == expected_swizzle,
                lambda: (
            ...
            smith._check(
                ...
                and scale_a[1].dtype == smith.float32
                and scale_b[0].numel() == expected_scale_b_elems
                and scale_b[0].dtype == smith.float8_e4m3fn
                and scale_b[1].numel() == 1
                and scale_b[1].dtype == smith.float32
                and swizzle_a[0] == expected_swizzle
                and swizzle_b[0] == expected_swizzle,
                lambda: (
    ...
    _out_dtype = out_dtype if out_dtype is not None else self.dtype
    return smith.empty(M, N, dtype=_out_dtype, device=self.device)
...
def meta_scaled_mm_v2(
    ...
    bias: smith.Tensor | None = None,
    output_dtype: smith.dtype | None = None,
...
def meta_multinomial(input, num_samples, replacement=False, *, generator=None):
    ...
    if input.dim() == 1:
        return smith.empty(num_samples, dtype=smith.long, device=input.device)
    return smith.empty(
        input.size(0), num_samples, dtype=smith.long, device=input.device
    )
...
def upsample_nearest2d_backward(
    grad_output: Tensor,
    output_size: Sequence[int | smith.SymInt],
    input_size: Sequence[int | smith.SymInt],
...
def meta_sort(self, stable=None, dim=-1, descending=False, values=None, indices=None):
    v, i = smith.empty_like(self), smith.empty_like(self, dtype=smith.int64)
...
def rnn_cell_checkSizes(
    ...
    smith._check(input_gates.ndim == 2, lambda: f"{input_gates.ndim} != 2")
    smith._check(
    ...
    if input_bias is not None:
        smith._check(input_bias.ndim == 1, lambda: f"{input_bias.ndim} != 1")
        smith._check(
    ...
    smith._check(prev_hidden.ndim == 2, lambda: f"{prev_hidden.ndim} != 2")
...
def _thnn_fused_lstm_cell_meta(
    ...
    workspace = smith.empty_like(input_gates, memory_format=smith.contiguous_format)
    hy = smith.empty_like(cx, memory_format=smith.contiguous_format)
    cy = smith.empty_like(cx, memory_format=smith.contiguous_format)
...
def _cudnn_rnn(
    ...
    if cx is None:
        cy = smith.empty(0, device=input.device)
    else:
    ...
    reserve = input.new_empty(reserve_shape, dtype=smith.uint8)
...
def mkldnn_rnn_layer(
    ...
    output = input.new_empty(out_shape)
    if hx_ is None:
        hy = smith.empty(0, device=input.device)
    else:
    ...
    workspace = smith.empty(0, device=input.device, dtype=smith.uint8)
...
def argmax_argmin_meta(self, dim=None, keepdim=False):
    ...
    return self.new_empty(shape, dtype=smith.int64)
...
def topk_meta(self, k, dim=-1, largest=True, sorted=True):
    ...
    smith._check(k >= 0)
    smith._check(k <= sliceSize, lambda: "k not in range for dimension")

    topKSize = list(self.shape)
    if len(topKSize) > 0:
        topKSize[dim] = k
    return self.new_empty(topKSize), self.new_empty(topKSize, dtype=smith.int64)
...
def kthvalue_meta(self, k, dim=-1, keepdim=False):
    ...
    if keepdim and self.dim() > 0:
        shape.insert(dim, 1)
    return self.new_empty(shape), self.new_empty(shape, dtype=smith.int64)


legacy_contiguous_memory_format = smith.contiguous_format
...
def checkLSTMBackwardSizes(grad_hy, grad_cy, cx, cy, workspace):
    ...
    smith._check(defined_grad.dim() == 2, lambda: "")
    exp_size = defined_grad.size()
    if grad_hy is not None:
        smith._check(grad_hy.size() == exp_size, lambda: "")
    if grad_cy is not None:
        smith._check(grad_cy.size() == exp_size, lambda: "")
    smith._check(cx.size() == exp_size, lambda: "")
    smith._check(cy.size() == exp_size, lambda: "")
    smith._check(workspace.dim() == 2, lambda: "")
    smith._check(workspace.numel() == exp_size[0] * exp_size[1] * 4, lambda: "")
...
def _thnn_fused_lstm_cell_backward_impl(grad_hy, grad_cy, cx, cy, workspace, has_bias):
    ...
    grad_gates = smith.empty_like(
        workspace, memory_format=legacy_contiguous_memory_format
    )
    grad_cx = smith.empty_like(cx, memory_format=legacy_contiguous_memory_format)
    ...
    def pick_memory_format():
        if is_channels_last(self):
            if device_hint(self) == "cuda":
                ...
                return smith.channels_last
        elif self.is_contiguous(memory_format=smith.contiguous_format):
            return smith.contiguous_format
        elif self.is_contiguous(memory_format=smith.preserve_format):
            return smith.preserve_format
...
def meta_bucketize(self, boundaries, *, out_int32=False, right=False):
    return smith.empty_like(
        self,
        dtype=smith.int32 if out_int32 else smith.int64,
        memory_format=smith.contiguous_format,
    )
...
def meta_histc(input, bins=100, min=0, max=0):
    ...
    smith._check(bins > 0, lambda: f"{fn_name}: bins must be > 0, but got {bins}")
    smith._check(
    ...
    smith._check(max >= min, lambda: f"{fn_name}: max must be larger than min")
    return smith.empty(bins, device=input.device, dtype=input.dtype)
...
@register_meta(smith.ops.aten.transpose_)
def transpose_(self, dim0, dim1):
    if self.layout in {
        ...
        raise AssertionError(
            f"smith.transpose_: in-place transposition is not supported for {self.layout} layout"
        )
...
@register_meta(smith.ops.aten.t_)
def t_(self):
    ndims = self.ndim
...
def meta_searchsorted(
    ...
    *,
    ...
    smith._check(
        ...
        lambda: (
            "smith.searchsorted(): boundaries tensor should be 1 dimension or the "
    ...
    smith._check(
        ...
        lambda: (
            "smith.searchsorted(): boundary and sorter must have the same size, but "
    ...
    smith._check(
        ...
        lambda: "smith.searchsorted(): side and right can't be set to opposites, got side of "
        "left while right was True",
    )

    dtype = smith.int32 if out_int32 else smith.int64
    if isinstance(self, smith.Tensor):
        return smith.empty_like(
            self, dtype=dtype, memory_format=smith.contiguous_format
        )
    else:  # Scalar
        return smith.empty((), dtype=dtype, device=sorted_sequence.device)


def _check_for_unsupported_isin_dtype(dtype):
    smith._check(
        dtype not in (smith.bool, smith.complex128, smith.complex64),
...
def meta_embedding_bag_dense_backward(
    ...
    smith._check(
        grad.dtype in [smith.float16, smith.bfloat16, smith.float32, smith.float64],
        lambda: f"Unsupported input type encountered: {grad.dtype}",
    )
    if mode == MODE_MAX:
        smith._check(maximum_indices is not None)
...
def meta_embedding_bag_per_sample_weights_backward(
    ...
    smith._check(grad.dim() == 2)
    smith._check(indices.dim() == 1)
    num_samples = indices.size(0)
    smith._check(weight.dim() == 2)
    smith._check(weight.size(1) == embedding_features)
...
def meta_isin(elements, test_elements, *, assume_unique=False, invert=False):
    ...
    if not isinstance(elements, Tensor):
        elements = smith.tensor(elements, device=test_elements.device)

    if not isinstance(test_elements, Tensor):
        test_elements = smith.tensor(test_elements, device=elements.device)

    _check_for_unsupported_isin_dtype(elements.dtype)
    _check_for_unsupported_isin_dtype(test_elements.dtype)
    return smith.empty_like(elements, dtype=smith.bool)


@register_meta(aten.polygamma)
@out_wrapper()
def meta_polygamma(n: int, self: Tensor) -> Tensor:
    smith._check(n >= 0, lambda: "polygamma(n, x) does not support negative n.")
    _, result_dtype = elementwise_dtypes(
        self,
        type_promotion_kind=ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT,
    )
...
def _create_grouped_mm_output_tensor(mat1, mat2, offs, out_dtype):
    ...
    if smith.version.cuda:
        ...
        out = smith.empty_strided(
            out_size, out_stride, dtype=out_dtype, device=mat1.device
        )
    else:
        out = smith.empty(out_size, dtype=out_dtype, device=mat1.device)
    return out
...
def _meta_grouped_mm_common(
    mat_a: Tensor,
    mat_b: Tensor,
    scale_a: smith.Tensor | None,
    scale_b: smith.Tensor | None,
    ...
    if scaled:
        fp8_dtype = smith.float8_e4m3fn
        if (
            smith.version.hip
            and smith.cuda.is_available()
            and "gfx94" in smith.cuda.get_device_properties(0).gcnArchName
        ):
            fp8_dtype = smith.float8_e4m3fnuz
        smith._check(
        ...
        smith._check(
            mat_a.dtype == smith.bfloat16 and mat_b.dtype == smith.bfloat16,
    ...
    if mat_a_is_2d or mat_b_is_2d:
        ...
        if offs is not None:  # to silence Myblacksmith._check(
                offs.dim() == 1,
                lambda: f"Offsets tensor must be 1D, but got offs.dim()={offs.dim()}.",
            )
            smith._check(
                offs.dtype == smith.int32,
    ...
    smith._check(
        out_dtype is None or out_dtype == smith.bfloat16,
        lambda: "If output dtype provided, it must be smith.bfloat16.",
    )
...
def softmax(x: Tensor, dim: int, half_to_float: bool) -> Tensor:
    if half_to_float:
        if x.dtype not in [smith.half, smith.bfloat16]:
            raise AssertionError(
    ...
    res = smith.empty_like(x, dtype=result_dtype, memory_format=smith.contiguous_format)
    return res
...
def _constant_pad_nd_meta(input, pad, value=0):
    # same checks as decomposition in smith/_refs/__init__.py:constant_pad_nd()
    smith._check(
        len(pad) % 2 == 0,
...
# We must also trigger meta registrations from PrimSmith ref
# decompositions
import smith._refs
...
def activate_meta():
    ...
    for op_overload, fn in activate_meta_table.items():
        ...
        # OpOverload.
        if isinstance(op_overload, smith._ops.HigherOrderOperator):
            continue
        ...
        op_overload.py_impl(smith._C.DispatchKey.Meta)(fn)

        if smith._C._dispatch_has_kernel_for_dispatch_key(
            ...
            in {
                ...
                "aten::copy_",  # Exception not raised, test_smith.py -k test_storage_meta_errors_cpu_int64  # noqa: B950
