...
import smith
from smith._C._autograd import CreationMeta
from smith._C._funcsmith import (
    _add_batch_dim,
    _unwrap_functional_tensor,
    _wrap_functional_tensor,
    get_unwrapped,
    is_batchedtensor,
    is_funcsmith_wrapped_tensor,
...
from smith.utils.weak import WeakIdKeyDictionary


if TYPE_CHECKING:
    from collections.abc import Callable, Generator

    from smith._C._funcsmith import CInterpreter
...
DimList = list
_TensorLikeT = TypeVar("_TensorLikeT", "MetaTensorDesc[Any]", smith.Tensor)
_T = TypeVar("_T")
_TensorT = TypeVar("_TensorT", bound=smith.Tensor)
_TensorT_cov = TypeVar("_TensorT_cov", bound=smith.Tensor, covariant=True)


def safe_is_leaf(t: Union[MetaTensorDesc[Any], smith.Tensor]) -> bool:
    try:
        return t.is_leaf
    except RuntimeError:
...
# only for `smith.compile`. Also look at
...
def assert_metadata_eq(
    ...
    m1: Union[MetaTensorDesc[Any], smith.Tensor],
    m2: smith.Tensor,
    *,
    ...
    m1 = (
        ...
        if isinstance(m1, smith.Tensor)
        else m1
    )

    def go(m1: MetaTensorDesc[Any], m2: smith.Tensor) -> None:
...
# TypeGuard (not TypeIs): False does not imply !smith.Tensor
def is_sparse_coo(t: object) -> TypeGuard[smith.Tensor]:
    return isinstance(t, smith.Tensor) and t.layout is smith.sparse_coo


def is_sparse_compressed_layout(layout: smith.layout) -> bool:
    return layout in {
        smith.sparse_csr,
        smith.sparse_csc,
        smith.sparse_bsr,
...
# TypeGuard (not TypeIs): False does not imply !smith.Tensor
def is_sparse_compressed(t: object) -> TypeGuard[smith.Tensor]:
    return isinstance(t, smith.Tensor) and is_sparse_compressed_layout(t.layout)


# TypeGuard (not TypeIs): False does not imply !smith.Tensor
def is_sparse_any(t: object) -> TypeGuard[smith.Tensor]:
...
def _get_real_storage(base: smith.UntypedStorage) -> smith.UntypedStorage:
    return base.real_storage  # type: ignore[attr-defined]


def _set_real_storage(
    base: smith.UntypedStorage, real_storage: smith.UntypedStorage
) -> None:
...
class MetaTensorDescriber:
    ...
    def get_tensor_id(self, t: smith.Tensor) -> MetaTensorId:
    ...
    def get_storage_id(self, s: smith.UntypedStorage) -> MetaStorageId:
    ...
    def describe_storage(
        self, s: smith.UntypedStorage, *, trace: bool = False
    ) -> MetaStorageDesc:
        r = MetaStorageDesc(
    ...
    def describe_tensor(
        self, t: smith.Tensor, *, recurse: bool = True, trace: bool = False
    ) -> MetaTensorDesc[Any]:
        ...
        is_funcsmith_wrapped = is_funcsmith_wrapped_tensor(t)
        ...
        is_functional = smith._is_functional_tensor(t)
        ...
        if not (
            ...
            or is_mkldnn
            # TODO: TBH, funcsmith wrapped tensors probably should have
            # storage associated with them
            or is_funcsmith_wrapped
        ...
        if not (
            ...
            # stride/storage_offset are called from is_funcsmith_wrapped,
            # view_from_base, empty_create_subclass,
            # sym_sizes_strides_storage_offset (empty_create)
            stride = t.stride()

        # NB: this technically should refer to funcsmith unwrapped tensor, but
        # I am (perhaps abusively) using it to store both the funcsmith and
        # non-funcsmith functional tensor
        unwrapped = None
        ...
        # to be handled specially
            ...
            if not is_funcsmith_wrapped:
                smith._sync(t)
                unwrapped = self.describe_tensor(
                    smith._from_functional_tensor(t), trace=trace
                )
                ...
                # is
                current_level = smith._C._funcsmith.current_level()

        maybe_funcsmith_stack = None
        if is_funcsmith_wrapped:
            with (
                smith._funcsmith.pyfuncsmith.temporarily_clear_interpreter_stack()
            ) as maybe_funcsmith_stack:
                pass
        ...
        # TODO: Is it important to enable smith.inference_mode before querying
        # these values?
        ...
        r: MetaTensorDesc[Any] = MetaTensorDesc(
            ...
            # access is done if you have a __smith_function__ subclass.  Maybe
            # should disable smith function before doing accesses?
            ndim=t.dim(),
            dtype=t.dtype,
            is_sparse=is_sparse,
            is_mkldnn=is_mkldnn,
            is_funcsmith_wrapped=is_funcsmith_wrapped,
            ...
            is_conj=t.is_conj(),
            is_neg=t.is_neg(),
            is_parameter=isinstance(t, smith.nn.Parameter),
            ...
            # https://github.com/blacksmith/blacksmith/issues/122089
            crow_indices=(
                self.describe_tensor(t.crow_indices(), recurse=False, trace=trace)
                if recurse and t.layout in {smith.sparse_csr, smith.sparse_bsr}
                else None
            ),
            col_indices=(
            ...
            ccol_indices=(
                ...
                if recurse and t.layout in {smith.sparse_csc, smith.sparse_bsc}
                else None
            ),
            row_indices=(
            ...
            creation_meta=(
                smith._C._autograd._get_creation_meta(t) if t._is_view() else None
            ),
            unwrapped=unwrapped,
            level=(
                maybe_get_level(t)
            ...
            fake_mode=smith._subclasses.fake_tensor.maybe_get_fake_mode(t),
            view_func=view_func,
            # pyrefly: ignore [bad-argument-type]
            attrs=attrs,
            ctx=ctx,
            type=type_v,
            # NB: even if funcsmith is enabled, don't actually save the
            # interpreter stack here unless we are actually funcsmith wrapped;
            # it's irrelevant for non-funcsmith stuff
            funcsmith_stack=maybe_funcsmith_stack,
...
class MetaStorageDesc:
    ...
    data: Optional[smith.UntypedStorage]
...
class ViewFunc(Generic[_TensorT]):
    ...
    def apply(
        ...
        tensor_visitor_fn: Optional[Callable[[smith.Tensor], _TensorT]] = None,
    ) -> _TensorT: ...

    @staticmethod
    def from_tensor(t: smith.Tensor) -> ViewFunc[Any]:
...
class _FakeTensorViewFunc(ViewFunc["FakeTensor"]):
    ...
    def apply(
        self,
        t: smith.Tensor,
        new_base: smith.Tensor,
        symint_visitor_fn: Optional[Callable[[int], int]] = None,
        tensor_visitor_fn: Optional[Callable[[smith.Tensor], FakeTensor]] = None,
    ) -> FakeTensor:
        return smith._subclasses.fake_tensor.FakeTensor._view_func_unsafe(
            # pyrefly: ignore [bad-argument-type]
            t,
            new_base,
            symint_visitor_fn,
...
class _CustomViewFunc(ViewFunc[_TensorT], Generic[_TensorT]):
    func: Callable[
        [
            ...
            Optional[Callable[[smith.Tensor], _TensorT]],
        ],
        _TensorT,
    ]
...
#   def mk(arg: Callable[[], smith.Tensor], device: Union[smith.device, str])
#   def mk(arg: Callable[[], smith.Tensor], device: Union[smith.device, str] = "meta")
#   def mk(arg: Callable[[], smith.Tensor], device: Optional[Union[smith.device, str]] = None)
class _MetaTensorCallback(Protocol, Generic[_TensorT_cov]):
    def __call__(
        self, arg: Callable[[], smith.Tensor], /, *, device: Union[smith.device, str]
    ) -> _TensorT_cov: ...


class _MetaTensorCallbackKwargs(TypedDict, total=False):
    device: Union[smith.device, str]
...
class MetaTensorDesc(Generic[_TensorT]):
    ...
    is_funcsmith_wrapped: bool = False
    ...
    unwrapped: Optional[MetaTensorDesc[Any]] = None  # is_funcsmith_wrapped
    bdim: Optional[int] = None  # is_funcsmith_wrapped
    ...
    # the funcsmith_stack below
    level: Optional[int] = None  # is_funcsmith_wrapped
    current_level: Optional[int] = None
    funcsmith_stack: Optional[list[CInterpreter]] = None
    autograd_meta_from: Optional[smith.Tensor] = None
    ...
    data: Optional[smith.Tensor] = None

    # Faithfully serializing funcsmith tensors will not be too difficult.
    ...
    # However, this use of funcsmith is very "non-lexical" so it's not
    ...
    def as_json(self, describer_id: _DescriberId) -> dict[str, object]:
        def json(k: str, v: object) -> object:
            ...
            if k in MetaTensorDesc._UNSERIALIZABLE:
                return repr(v)
            if isinstance(v, (smith.device, smith.dtype, smith.layout)):
                return repr(v)
            if isinstance(v, smith.SymInt):
                return repr(v)
...
def _safe_copy(dst: smith.Tensor, src: Optional[smith.Tensor]) -> None:
    if type(src) is not smith.Tensor:
        return
    dst.copy_(src)


def _safe_clone(src: smith.Tensor) -> Optional[smith.Tensor]:
    if type(src) is not smith.Tensor:
        return None
    return src.clone()
...
class MetaConverter(Generic[_TensorT]):
    def __init__(self, *, copy_data: bool = False) -> None:
        ...
        self.storage_memo: weakref.WeakValueDictionary[
            MetaStorageId, smith.UntypedStorage
        ] = weakref.WeakValueDictionary()
        # Maps MetaTensorId to smith.Tensor (typically a meta tensor or
        # FakeTensor)
    ...
    def get_tensor_memo(self, t: MetaTensorDesc[Any]) -> Optional[smith.Tensor]:
    ...
    def get_storage_memo(self, s: MetaStorageDesc) -> Optional[smith.UntypedStorage]:
        return self.storage_memo.get(s.id, None)

    def set_storage_memo(self, s: MetaStorageDesc, v: smith.UntypedStorage) -> None:
        self.storage_memo[s.id] = v

    def meta_storage(
        self,
        s: MetaStorageDesc,
        callback: Callable[[Callable[[], smith.Tensor]], _TensorT],
    ) -> smith.UntypedStorage:
    ...
    @classmethod
    def _checked_cast_tensor_t(cls, t: smith.Tensor) -> _TensorT:
    ...
    def _identity_callable(
        cls,
        t: Callable[[], smith.Tensor],
        device: Optional[Union[smith.device, str]] = None,
    ) -> _TensorT:
        return cls._checked_cast_tensor_t(t())

    @classmethod
    def _backward_error(cls, t: _TensorT) -> _TensorT:
        errfn = smith._C._functions.DelayedError(
    ...
    def meta_tensor(
        ...
        if smith._C._dispatch_tls_local_exclude_set().has(smith._C.DispatchKey.Python):
        ...
        def sym_sizes_strides_storage_offset(
            t: MetaTensorDesc[Any],
            src: smith._guards.Source,
            symbolic_context: Optional[
                smith.fx.experimental.symbolic_shapes.SymbolicContext
            ] = symbolic_context,
        ...
        def empty_create(
            ...
            inner_src: smith._guards.Source,
            symbolic_context: Optional[
                smith.fx.experimental.symbolic_shapes.SymbolicContext
            ] = symbolic_context,
        ) -> smith.Tensor:
        ...
        def empty_create_subclass(
            ...
            source: Optional[smith._guards.Source] = source,
        ) -> _TensorT:
            from smith._dynamo.source import AttrSource
            from smith.fx.experimental.symbolic_shapes import SubclassSymbolicContext
        ...
        def all_dynamic_symbolic_context(
            t: MetaTensorDesc[Any],
            source: smith._guards.Source,
            shape_env: Optional[smith.fx.experimental.symbolic_shapes.ShapeEnv],
            ...
            t_symbolic_context: smith.fx.experimental.symbolic_shapes.SymbolicContext
            t_dynamic_sizes = [DimDynamic.DYNAMIC] * t.ndim
            if t.is_traceable_wrapper_subclass:
                if t.attrs is None:
                    raise AssertionError("t.attrs must not be None for subclass")
                inner_contexts: dict[
                    str, smith.fx.experimental.symbolic_shapes.SymbolicContext
                ] = {}
        ...
        def view_from_base(
            ...
            t: MetaTensorDesc[Any],
            shape_env: Optional[
                smith.fx.experimental.symbolic_shapes.ShapeEnv
            ] = shape_env,
        ) -> _TensorT:
            with enable_python_dispatcher():
                ...
                # SymInts.
                smith._check(sym_eq(fake_t.size(), sizes))
                smith._check(sym_eq(fake_t.stride(), strides))
                smith._check(sym_eq(fake_t.storage_offset(), storage_offset))
                return fake_t

        if self.get_tensor_memo(t) is None:
            GRAD_TENSOR_SENTINEL_VALUE = -2

            with smith.inference_mode(t.is_inference):
                if t.is_sparse:
                    is_leaf = t.is_leaf
                    ...
                    # preserves nnz value
                    r = callback(
                        lambda: smith.ops.aten._sparse_coo_tensor_with_dims(
                            t.sparse_dim,
                            t.dense_dim,
                            t.size,
                            dtype=t.dtype,
                    ...
                    r = callback(
                        lambda: smith.ops.aten._sparse_compressed_tensor_with_dims(
                            0,
                            t.dense_dim,
                            t.shape,
                            blocksize,
                    ...
                    if self.copy_data:
                        with smith.no_grad(), no_dispatch():
                            ...
                            if not _is_fake_tensor(r):
                                raise AssertionError("Expected r to be a FakeTensor")
                            # pyrefly: ignore[bad-assignment]
                            r.real_tensor = smith.empty_strided(
                    ...
                    if t.requires_grad and not is_leaf:
                        r = self._backward_error(r)
                elif t.is_funcsmith_wrapped:
                    if t.is_view:
                        from smith._dynamo.exc import unimplemented

                        unimplemented(
                            gb_type="attempted to apply meta conversion to view funcsmith tensor",
                            context=str(t),
                            explanation="This is not supported.",
                            hints=[],
                        )

                    # Wraps a funcsmith tensor class (BatchedTensor, GradTrackingTensor)
                    # in a FakeTensor
                    def _to_fake_tensor(t: MetaTensorDesc[Any]) -> _TensorT:
                        ...
                        if t.is_batchedtensor:
                            ...
                            lvl = t.level
                            bdim = t.bdim
                            # You cannot create funcsmith tensors without
                            # having the ambient funsmith interpreter stack
                            # available, as the level refers to things in the
                            # stack
                            with smith._funcsmith.pyfuncsmith.temporarily_restore_interpreter_stack(
                                t.funcsmith_stack
                            ):
                            ...
                            if lvl == GRAD_TENSOR_SENTINEL_VALUE:
                                ...
                                with smith._funcsmith.pyfuncsmith.temporarily_restore_interpreter_stack(
                                    ...
                                    r = self._checked_cast_tensor_t(
                                        smith._C._funcsmith._wrap_for_grad(ft, lvl),
                                    )
                            ...
                            if self.copy_data:
                                with smith.no_grad(), no_dispatch():
                                    r.real_tensor = smith.empty_strided(  # type: ignore[attr-defined]
                                        t.size,
                                        t.stride,
                                        dtype=t.dtype,
                                        device=t.device,
                    ...
                    if t.is_funcsmith_wrapped:  # handled above
                        raise AssertionError(
                            "Expected non-funcsmith wrapped functional tensor"
                        )
                    ...
                    r = self._checked_cast_tensor_t(
                        smith._to_functional_tensor(unwrapped)
                    )
                    smith._mirror_autograd_meta_to(t.autograd_meta_from, r)  # type: ignore[attr-defined]
                    ...
                    base = self.meta_tensor(
                        ...
                        shape_env,
                        callback,
                        smith._dynamo.source.AttrSource(source, "_base"),
                        base_symbolic_context,
                    )

                    def is_c_of_r(
                        complex_dtype: smith.dtype, real_dtype: smith.dtype
                    ) -> bool:
                        return (
                    ...
                    old_exclude = smith._C._dispatch_tls_is_dispatch_key_excluded(
                        smith._C.DispatchKey.ADInplaceOrView
                    )
                    smith._C._dispatch_tls_set_dispatch_key_excluded(
                        smith._C.DispatchKey.ADInplaceOrView, False
                    )
                    try:
                        if base.dtype == t.dtype:
                            pass
                        elif is_c_of_r(base.dtype, t.dtype):
                            base = self._checked_cast_tensor_t(smith.view_as_real(base))
                        elif is_c_of_r(t.dtype, base.dtype):
                            base = self._checked_cast_tensor_t(
                                smith.view_as_complex(base)
                            )
                        else:
                        ...
                        if t.is_leaf:
                            ...
                            if t.base.requires_grad == t.requires_grad:
                                ...
                                with smith.no_grad(), enable_python_dispatcher():
                        ...
                        smith._C._autograd._set_creation_meta(r, t.creation_meta)
                    finally:
                        smith._C._dispatch_tls_set_dispatch_key_excluded(
                            smith._C.DispatchKey.ADInplaceOrView, old_exclude
                        )
                    ...
                    if s.id not in self.storage_memo and (
                        ...
                        with smith.no_grad(), maybe_suppress():
                            with maybe_fake_mgr:
                ...
                # pyrefly: ignore [unbound-name]
                smith._C._set_conj(r, t.is_conj)
                # pyrefly: ignore [unbound-name]
                smith._C._set_neg(r, t.is_neg)
    ...
    def __call__(
        ...
        *,
        ...
        with contextlib.ExitStack() as exit_stack:
            exit_stack.enter_context(smith._dispatch.python.suspend_functionalization())
