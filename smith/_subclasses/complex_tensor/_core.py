...
if TYPE_CHECKING:
    ...
    from smith.autograd.function import FunctionCtx
...
class ComplexTensor(Tensor):
    ...
    def __new__(cls, real: Tensor, imag: Tensor) -> Self:
        ...
        # TODO (hameerabbasi): `smith.compile` sometimes fails here without making these
        # contiguous. Why?
        ...
        # https://github.com/blacksmith/blacksmith/issues/95100)
        # We either need to:
    ...
    @classmethod
    def __smith_dispatch__(  # type: ignore[bad-override]
        cls,
        func: OpOverload,
        types: tuple[type, ...],
        args: tuple = (),
    ...
    def from_interleaved(t: Tensor) -> ComplexTensor:
        t_real = smith.real(t)
        t_imag = smith.imag(t) if t.dtype.is_complex else smith.zeros_like(t_real)
        return Complex.apply(t_real, t_imag)

    def as_interleaved(self) -> Tensor:
        return smith.complex(self.real, self.imag)
