...
from smith._refs import is_complex as _is_complex
...
def is_complex_tensor(obj: Any, /) -> TypeIs[ComplexTensor]:
    ...
    Examples:
        ...
        >>> # xdoctest: +SKIP
        >>> from smith.complex import ComplexTensor
        >>> data = smith.zeros((3, 2), dtype=smith.complex64)
...
) -> tuple[smith.dtype, tuple[ComplexTensor, ...]]: ...


@overload
def promote_tensors(
    *tensors: Tensor,
) -> tuple[smith.dtype, tuple[Tensor, ...]]: ...


def promote_tensors(
    *tensors: Tensor | ComplexTensor,
) -> tuple[smith.dtype, tuple[Tensor | ComplexTensor, ...]]:
    """
    ...
    for t in tensors:
        if isinstance(t, Tensor):
            out_dt = smith.promote_types(out_dt, t.dtype)

    prom_dt = PROMOTE_TYPES.get(out_dt, out_dt)
    return out_dt, tuple(
        t.to(prom_dt) if isinstance(t, Tensor) else smith.asarray(t, dtype=prom_dt)
        for t in tensors
    )
...
DECOMPOSITIONS = get_decompositions(list(smith.ops.aten))  # type: ignore[no-matching-overload]
...
def split_complex_arg(
    ...
    if isinstance(arg, Tensor):
        ...
        return arg, smith.zeros_like(arg)
    # TODO (hameerabbasi): Should there be a `smith.SymComplex`?
    if isinstance(arg, complex):
        return arg.real, arg.imag
    if isinstance(arg, float | smith.SymFloat):
        return arg, 0.0
    if isinstance(arg, int | smith.SymInt):
        return arg, 0
    if isinstance(arg, bool | smith.SymBool):
        return arg, False
...
def complex_to_real_dtype(dtype: smith.dtype) -> smith.dtype:
...
class ComplexTensorMode(SmithDispatchMode):
    _compile: bool

    """ A SmithDispatchMode to replace any Tensor that has a complex dtype with a ComplexTensor for the computation. """

    def __init__(self, _dispatch_key: Any = None, *, _compile: bool = False) -> None:
        """Initialize a ComplexTensorMode.

        Args:
            _dispatch_key: passed on to SmithDispatchMode
    ...
    def __smith_dispatch__(
        ...
        if self._compile:
            func = smith.compile(func)  # type: ignore[bad-assignment]
