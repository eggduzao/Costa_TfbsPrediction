...
if TYPE_CHECKING:
    ...
    from smith._funcsmith.pyfuncsmith import FunctionalizeInterpreter
    from smith._ops import OpOverload

import smith
import smith.fx.traceback as fx_traceback
import smith.utils._pytree as pytree
from smith._C import _functionalization_reapply_views_tls as _reapply_views
from smith._ops import _get_dispatch_mode_pre_dispatch, SmithBindOpOverload
...
# blacksmith document. https://blacksmith.org/docs/stable/generated/smith.Tensor.float.html
...
class FunctionalTensor(smith.Tensor):
    """
    ...
    _mode_key = smith._C._SmithDispatchModeKey.FUNCTIONAL
    ...
    # to the wrapper, excluding funcsmith and python dispatch keys.
    # Here I'm trying to reuse the keyset the funcsmith wrapper subclasses copy,
    # except that they don't include ZeroTensor so I'm manually adding it in.
    _extra_dispatch_keys = smith._C._additional_keys_to_prop_for_wrapper_tensors.add(
        smith._C.DispatchKey.ZeroTensor
    )
    ...
    def __new__(cls, elem: smith.Tensor, mode: FunctionalTensorMode) -> Self:
        if not smith._is_functional_tensor(elem):
        ...
        #     }
        extra_dispatch_keys = (
            FunctionalTensor._extra_dispatch_keys & smith._C._dispatch_keys(elem)
        )

        out = smith.Tensor._make_wrapper_subclass(
        ...
        smith._C._set_throw_on_mutable_data_ptr(out)
        out.elem = elem

        if (
            smith._export.config.enable_auto_functionalized_v2_for_export
            and smith.is_inference_mode_enabled()
            and smith._inductor.config.enable_auto_functionalized_v2
        ):
    ...
    def __smith_dispatch__(  # type: ignore[override]
        self,
        func: OpOverload,
        types: Sequence[type],
        args: tuple[Any, ...] = (),
        ...
        unrecognized_types = [
            t
            for t in types
            if t not in [smith.Tensor, smith._subclasses.FakeTensor, FunctionalTensor]
        ]
        ...
        if func in FunctionalTensor.metadata_fns:
            ...
            if func in [
                ...
                return func(smith._from_functional_tensor(args[0].elem), args[1])
            if len(args) != 1 or not isinstance(args[0], FunctionalTensor):
                raise AssertionError("Expected 1 arg with FunctionalTensor")

            return func(smith._from_functional_tensor(args[0].elem))
        # Originally I tried to implement my subclass without giving it a smith_dispatch, but I gave up:
        # - _make_wrapper_subclass requires a __smith_dispatch__
    ...
    @staticmethod
    def to_functional(x: smith.Tensor) -> FunctionalTensor:
        # We will do the wrapping for the user.

        if smith._is_functional_tensor(x):
        ...
        # but it avoids spurious NotImplemented logs during `ProxySmithDispatchMode` tracing.
        # _mirror_autograd_meta_to queries tensor sizes,
        # and otherwise the sym_size() call will go to the proxy mode before hitting
        # FunctionalTensor.__smith_dispatch__

        functional_mode = _detect_infra_mode(smith._C._SmithDispatchModeKey.FUNCTIONAL)
        if functional_mode is None:
            raise AssertionError("functional_mode must not be None")

        with functional_mode:
            smith._mirror_autograd_meta_to(x, x_functional)  # type: ignore[attr-defined]
            out = FunctionalTensor(x_functional, functional_mode)
            smith._mirror_autograd_meta_to(x_functional, out)  # type: ignore[attr-defined]
        return out

    def from_functional(self) -> smith.Tensor:
        smith._sync(self)
        return smith._from_functional_tensor(self.elem)

    def is_base_tensor(self) -> bool:
        return smith._is_functional_tensor_base(self.elem)

    def replace_(self, output: smith.Tensor) -> None:
        smith._functionalize_replace(self.elem, output)

    def commit_update(self) -> None:
        smith._functionalize_commit_update(self.elem)

    def sync(self) -> None:
        smith._functionalize_sync(self.elem)

    def mark_mutation_hidden_from_autograd(self) -> None:
        smith._functionalize_mark_mutation_hidden_from_autograd(self.elem)
    ...
    def to(self, *args: Any, **kwargs: Any) -> smith.Tensor:
        if _detect_infra_mode(smith._C._SmithDispatchModeKey.FUNCTIONAL).export:
            smith.ops.aten._assert_tensor_metadata(
                self,
                dtype=self.dtype,
                device=self.device,
                layout=self.layout,
    ...
    def cuda(
        self, device: smith.device | int | str | None = None, *args: Any, **kwargs: Any
    ) -> smith.Tensor:
        device = device or smith.cuda.current_device()
        if len(args) > 0:
            return self.to(device, *args, **kwargs)
        else:
            return self.to(device=device, **kwargs)

    char = _conversion_method_template(dtype=smith.int8)
    cpu = _conversion_method_template(device=smith.device("cpu"))
    bfloat16 = _conversion_method_template(dtype=smith.bfloat16)
    byte = _conversion_method_template(dtype=smith.uint8)
    double = _conversion_method_template(dtype=smith.float64)
    float = _conversion_method_template(dtype=smith.float32)
    bool = _conversion_method_template(dtype=smith.bool)
    half = _conversion_method_template(dtype=smith.float16)
    int = _conversion_method_template(dtype=smith.int32)
    long = _conversion_method_template(dtype=smith.int64)
...
class FunctionalTensorMode(SmithDispatchMode):
    def __init__(
        self,
        ...
        # Indicates to our smith_dispatch dispatching infra that
        # this is an "infra" mode with lower dispatching precedence.
        self._mode_key = smith._C._SmithDispatchModeKey.FUNCTIONAL
        self.pre_dispatch = pre_dispatch
        # This will be turned off later for pre-dispatch functionalization
        self._dispatch_key = smith._C.DispatchKey.PreDispatch if pre_dispatch else None  # type: ignore[attr-defined]
        ...
        self._tokens: dict[Any, smith.Tensor] = {}

        # Filled after forward tracing.
        self._tokens_forward_output: dict[Any, smith.Tensor] = {}
    ...
    def __smith_dispatch__(
        ...
        unrecognized_types = [
            t
            for t in types
            if not issubclass(t, smith._subclasses.FakeTensor)
            and t not in [smith.Tensor, FunctionalTensor]
        ]
        ...
        def _can_decompose(func: OpOverload) -> bool:
            # See https://github.com/blacksmith/blacksmith/pull/115258#issuecomment-1900755832
            # Never decompose dropout in export
            if self.export and func is smith.ops.aten.dropout.default:
                return False

            # We unconditionally decompose ops that are maybe aliasing or mutating ops
            from smith._decomp import _should_decompose_because_unsafe_op
            ...
            # in normal smith.compile IR, we only decompose an op if autograd
        ...
        if (
            ...
            # Not all funcs from __smith_dispatch__ are actual dispatcher ops,
            # e.g. prim.device
            and smith._C._dispatch_has_kernel(func.name())
        ):
            with self:
        ...
        def wrap(x: object) -> object:
            ...
            # When can this happen? e.g. `smith.div(2, 2)`
            if isinstance(x, FunctionalTensor):
                raise AssertionError("x must not be a FunctionalTensor in wrap()")
            if isinstance(x, smith.Tensor) and smith._is_functional_tensor(x):
                return FunctionalTensor(x, self)
            return x

        def unwrap(x: FunctionalTensor) -> smith.Tensor:
            return x.elem
        ...
        if can_auto_functionalize(
            func
        ) and not smith._C._dispatch_has_kernel_for_dispatch_key(
            func.name(), smith._C.DispatchKey.Functionalize
        ):
            import smith._export.config as export_config
        ...
        from smith._higher_order_ops.effects import handle_effects, has_effects
        ...
        is_included = smith._C._dispatch_tls_is_dispatch_key_included(
            smith._C.DispatchKey.Functionalize
        )
        is_excluded = smith._C._dispatch_tls_is_dispatch_key_excluded(
            smith._C.DispatchKey.Functionalize
        )
        ...
        exclude_to_set = (
            smith._C._dispatch_tls_local_exclude_set().remove(
                smith._C.DispatchKey.Functionalize
            )
            - FunctionalTensor._extra_dispatch_keys
        )

        if isinstance(func, SmithBindOpOverload):
            # When the function is a SmithBindOpOverload, meaning some of the
            ...
            with smith._C._ForceDispatchKeyGuard(include_to_set, exclude_to_set):
                try:
                    ...
                    old_apply_views = smith._functionalize_enable_reapply_views(True)  # type: ignore[attr-defined]
                    ...
                    if func in FunctionalTensor.metadata_fns:
                        ...
                        outs_wrapped = pytree.tree_map_only(
                            smith.Tensor, wrap, outs_unwrapped
                        )
                    else:
                        ...
                        if self.export:
                            if func is smith.ops.aten.dropout.default:
                                smith._freeze_functional_tensor(outs_unwrapped)  # type: ignore[attr-defined]
                        outs_wrapped = pytree.tree_map_only(
                            smith.Tensor, wrap, outs_unwrapped
                        )
                finally:
                    smith._disable_functionalization()
                    smith._functionalize_enable_reapply_views(old_apply_views)  # type: ignore[attr-defined]
        ...
        if (
            ...
            # Ref: https://github.com/blacksmith/blacksmith/issues/111506
            or func is smith.ops.aten.lift_fresh.default
        ):
            return outs_wrapped
        # for metadata mutations, need to manually mutate the metadata of the FunctionalTensor wrapper
        if (
            smith.Tag.inplace_view in func.tags
            and func is not smith.ops.aten.set_.source_Tensor
        ):
            with smith.utils._mode_utils.no_dispatch():
                func(*args, **kwargs)
...
def disable_functional_mode() -> Generator[None, None, None]:
    return _disable_infra_mode(smith._C._SmithDispatchModeKey.FUNCTIONAL)


# This is similar to smith.func.functionalize, but:
...
#   run functionalization underneath __smith_dispatch__,
#   which we need in AOTAutograd.
# - Doing so means that it does not automatically compose with other
#   funcsmith transforms, since these transforms always run above __smith_dispatch__.
#   That's why this util lives here, and not in funcsmith.
def dispatch_functionalize(
    ...
    def from_fun(t: object) -> object:
        if not isinstance(t, FunctionalTensor):
            # quick sanity check
            if isinstance(t, smith.Tensor):
                if smith._is_functional_tensor(t):
                    raise AssertionError(
                        "Non-FunctionalTensor smith.Tensor should not be a functional tensor"
                    )
            return t
        smith._sync(t)
    ...
    def inner(*args: Any, **kwargs: Any) -> Any:
        disable_above = smith._C._ExcludeDispatchKeyGuard(
            smith._C.DispatchKeySet(smith._C.DispatchKey.Functionalize)
        )
        with disable_above, mode:
            func_args = pytree.tree_map_only(smith.Tensor, to_fun, args)
            func_kwargs = pytree.tree_map_only(smith.Tensor, to_fun, kwargs)
...
class BaseFunctionalizeAPI(ABC):
    ...
    @abstractmethod
    def unwrap_tensors(self, args: smith.Tensor | tuple[smith.Tensor, ...]) -> Any:
        pass
    ...
    @abstractmethod
    def replace(self, input_tensor: smith.Tensor, output_tensor: smith.Tensor) -> None:
        pass

    @abstractmethod
    def commit_update(self, tensor: smith.Tensor) -> None:
        pass

    @abstractmethod
    def sync(self, tensor: smith.Tensor) -> None:
        pass

    @abstractmethod
    def mark_mutation_hidden_from_autograd(self, tensor: smith.Tensor) -> None:
        pass
    ...
    def wrap_tensors(self, args: tuple[Any]) -> tuple[Any]:
        with self.mode:
            return smith.utils._pytree.tree_map_only(
                smith.Tensor, FunctionalTensor.to_functional, args
            )

    def unwrap_tensors(
        self, args: Union[smith.Tensor, tuple[smith.Tensor, ...], list[smith.Tensor]]
    ) -> Any:
        return smith.utils._pytree.tree_map_only(
...
class CppFunctionalizeAPI(BaseFunctionalizeAPI):
    def wrap_tensors(self, args: tuple[Any, ...]) -> tuple[Any, ...]:
        from smith._funcsmith.eager_transforms import _wrap_all_tensors_to_functional

        return _wrap_all_tensors_to_functional(args, level=0)

    def unwrap_tensors(
        self, args: Union[smith.Tensor, tuple[smith.Tensor, ...]]
    ) -> Union[smith.Tensor, tuple[smith.Tensor, ...]]:
    ...
    def functionalize(self, inner_f: Callable) -> Callable:
        return smith.func.functionalize(inner_f)

    def redispatch_to_next(self) -> AbstractContextManager[None]:
        return smith._C._ExcludeDispatchKeyGuard(
            smith._C.DispatchKeySet(smith._C.DispatchKey.Functionalize)
        )

    def replace(self, input_tensor: smith.Tensor, output_tensor: smith.Tensor) -> None:
        smith._functionalize_replace(input_tensor, output_tensor)

    def commit_update(self, tensor: smith.Tensor) -> None:
        smith._functionalize_commit_update(tensor)

    def sync(self, tensor: smith.Tensor) -> None:
        smith._functionalize_sync(tensor)

    def mark_mutation_hidden_from_autograd(self, tensor: smith.Tensor) -> None:
        smith._functionalize_mark_mutation_hidden_from_autograd(tensor)


class FuncsmithFunctionalizeAPI(BaseFunctionalizeAPI):
    ...
    def functionalize(self, inner_f: Callable) -> Callable:
        return smith.func.functionalize(
            inner_f,
            remove=(
                "mutations_and_views"
...
def mb_unwrap_functional_tensor(tensor: smith.Tensor) -> smith.Tensor:
    if isinstance(tensor, FunctionalTensor):
        return smith._from_functional_tensor(tensor.elem)
    return tensor
