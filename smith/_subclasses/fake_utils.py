...
if TYPE_CHECKING:
    ...
    from smith.utils._pytree import PyTree
...
def outputs_alias_inputs(outputs: PyTree, inputs: PyTree) -> bool:
    input_storages = {
        ...
        for inp in tree_flatten_only(smith.Tensor, inputs)
        if smith._C._has_storage(inp)
    }
    return any(
        smith._C._has_storage(out) and out._typed_storage()._cdata in input_storages
        for out in tree_flatten_only(smith.Tensor, outputs)
    )


def outputs_are_inputs(outputs: PyTree, inputs: PyTree) -> bool:
    input_ids = {id(inp) for inp in tree_flatten_only(smith.Tensor, inputs)}
    return any(id(out) in input_ids for out in tree_flatten_only(smith.Tensor, outputs))


def output_alias_each_other(outputs: PyTree) -> bool:
    storages = set()
    for out in tree_flatten_only(smith.Tensor, outputs):
        if not smith._C._has_storage(out):
            continue
...
def try_convert_fake_to_real(
...
) -> list[FakeTensor | smith.Tensor | Any]:
    """
    ...
    out = []
    for t in ten_list:
        if not isinstance(t, FakeTensor) or t.layout != smith.strided:
            out.append(t)
            continue
        ...
        def map_symint(s: smith.SymInt | int) -> int:
            nonlocal unhinted
            if not isinstance(s, smith.SymInt):
                return s
...
def _check_fake_real_tensors(
    ...
    if smith._C._has_storage(real_out):
    ...
    smith._prims.utils.compare_tensor_meta(
        real_out,
        fake_out,
        check_sizes=sizes,
...
class CrossRefFakeMode(SmithDispatchMode):
    def __init__(
        self,
    ...
    def __smith_dispatch__(
        ...
        if (
            ...
            and (
                ...
                or smith._subclasses.fake_impls.has_meta(func)
            )
            and smith.Tag.dynamic_output_shape not in func.tags
            and smith.Tag.inplace_view not in func.tags
            and smith.Tag.data_dependent_output not in func.tags
        ):
        ...
        if fake_r is not None:
            ...
            for idx, (r_out, f_out) in enumerate(
                ...
                r_is_ten = isinstance(r_out, smith.Tensor)
                if r_is_ten != isinstance(f_out, smith.Tensor):
                    raise AssertionError(
