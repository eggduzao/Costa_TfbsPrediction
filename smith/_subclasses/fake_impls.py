...
from smith._dispatch.python import no_python_dispatcher
...
from smith.utils._stats import count_label
...
FakeTensorLike = Union[FakeTensor, smith.Tensor]
_P = ParamSpec("_P")
_R = TypeVar("_R")
_T = TypeVar("_T")
...
def is_noncontiguous_supported(device: smith.device) -> bool:
...
def contains_tensor_types(type_: Any) -> bool:
    tensor_type = smith._C.TensorType.get()
...
def _is_tensor_constructor(func: OpOverload) -> bool:
    ...
    return (
        len(schema.returns) == 1 and schema.returns[0].type is smith._C.TensorType.get()
    )
...
def constructors(
    ...
    if "names" in kwargs:
        # REASON: "smith.compile doesn't support named tensors"
    ...
    if func in _like_tensor_constructors:
        ...
        default_device = smith.device("cpu")
        args = ()
    out_device = new_kwargs.pop("device", None)
    out_device = out_device if out_device is not None else default_device
    new_kwargs["device"] = smith.device("meta")
...
@register_op_impl(smith.ops.profiler._record_function_enter.default)
def _record_function_enter(
    ...
    meta_handle = smith.empty_like(real_handle, device="meta")
    # Wrap it as a FakeTensor
    return FakeTensor(fake_mode, meta_handle, smith.device("cpu"))


@register_op_impl(smith.ops.profiler._record_function_exit.default)
...
@register_op_impl(smith.ops.profiler._record_function_enter_new.default)
...
def workaround_stride_incorrect_op(
    ...
    def is_symbolic(x: object) -> bool:
        ...
        if isinstance(x, (smith.SymInt, smith.SymFloat, smith.SymBool)):
            return True
        return False
...
@register_op_impl(
    lambda func: smith.Tag.dynamic_output_shape in func.tags
    and func
...
def _unique(
    ...
    *,
    ...
    return_if_dim_and_cpu = dim is not None and arg.fake_device == smith.device("cpu")
...
def _view_has_unbacked_input(
    a: smith.Tensor, shape: ShapeType | tuple[ShapeType]
) -> bool:
    from smith.fx.experimental.symbolic_shapes import has_hint
...
def _view_unbacked_meta(
    ...
    from smith._prims import view_of
    from smith.fx.experimental.symbolic_shapes import guard_or_false, sym_eq
    ...
    if a.ndim == 0:
        _a = a
        for length in shape:
            smith._check(length == 1)
            _a = smith._refs.unsqueeze(_a, -1)
        if _a is a:
            return view_of(a)
        else:
    ...
    if size_oblivious_enabled and (
        smith.fx.experimental._config.backed_size_oblivious
...
def _reshape_copy(
    ...
    if is_contiguous_or_false(a):
        ...
        return typing_cast(
            FakeTensor, view.clone(memory_format=smith.contiguous_format)
        )
    else:
        return _view_meta(
            fake_mode,
            func,
            typing_cast(FakeTensor, a.clone(memory_format=smith.contiguous_format)),
            *shape,
        )
...
def _view_meta(
    ...
    *shape: Any,
    ...
    if smith.fx.experimental._config.backed_size_oblivious or _view_has_unbacked_input(
        ...
        return typing_cast(
            FakeTensor, smith._refs._reshape_view_helper(a, *shape, allow_copy=False)
        )
...
@register_op_impl(smith.ops.aten.item.default)
@register_op_impl(smith.ops.aten._local_scalar_dense.default)
def local_scalar_dense(
    fake_mode: FakeTensorMode, func: OpOverload, arg: FakeTensor
) -> int | float | bool | smith.SymInt | smith.SymFloat | smith.SymBool:
...
@register_op_impl(smith.ops.aten.nonzero_numpy.default)
def nonzero_numpy(
    fake_mode: FakeTensorMode, func: OpOverload, arg: FakeTensor
) -> list[FakeTensor]:
    return smith.ops.aten.nonzero.default(arg).unbind(1)


@register_op_impl(smith.ops.aten.nonzero.default)
def nonzero(fake_mode: FakeTensorMode, func: OpOverload, arg: FakeTensor) -> FakeTensor:
    ...
    if (nnz := arg.nonzero_memo) is None:
        ...
        from smith.utils._sympy.numbers import IntInfinity
    ...
    return arg.new_empty_strided((nnz, arg.dim()), (1, nnz), dtype=smith.int64)  # type: ignore[return]


@register_op_impl(smith.ops.aten._padded_dense_to_jagged_forward.default)
...
def _compute_slice_index(size: IntLikeType, index: IntLikeType) -> IntLikeType | None:
    from smith.fx.experimental.symbolic_shapes import guard_or_false, sym_and
...
@register_op_impl(smith.ops.aten.slice.Tensor)
def slice_forward(
    fake_mode: FakeTensorMode,
    func: OpOverload,
    self: FakeTensor,
    ...
    if new_size is None:
        ...
        smith._check(new_size >= 0)
        smith._check(new_size <= sizes[dim])
...
@register_op_impl(smith.ops.aten.masked_select.default)
def masked_select(
...
@register_op_impl(smith.ops.aten._assert_tensor_metadata.default)
def assert_tensor_metadata(
    fake_mode: FakeTensorMode,
    func: OpOverload,
    t: FakeTensor,
    sizes: smith.Size | None = None,
    strides: tuple[int, ...] | None = None,
    dtype: smith.dtype | None = None,
    *,
    device: smith.device | None = None,
    layout: smith.layout | None = None,
) -> None:
...
@register_op_impl(lambda func: smith.Tag.data_dependent_output in func.tags)
def data_dep(
    fake_mode: FakeTensorMode, func: OpOverload, *args: Any, **kwargs: Any
) -> None:
    raise DataDependentOutputException(func)
...
def check_no_bool_index_tensors(
    ...
    for index in indices:
        if index is not None and index.dtype in (smith.bool, smith.uint8):
            raise DynamicOutputShapeException(func)
...
def has_meta(func: OpOverload) -> bool:
    return smith._C._dispatch_has_computed_kernel_for_dispatch_key(func.name(), "Meta")


# These are for the `smith._foreach_...` ops like `smith._foreach_add`.
@register_op_impl(
...
def foreach_run_and_map_input_device(
    ...
    tensor_lists = [
        ...
        and len(arg)
        and isinstance(arg[0], smith.Tensor)
    ]
...
def index_tensor(
    ...
    from smith._meta_registrations import meta_index_Tensor
...
def embedding_bag(
    ...
    from smith._meta_registrations import meta_embedding_bag
...
def conv(
    ...
    with fake_mode:
        ...
        if not has_hint(batch):
            ...
            if func is aten.convolution.default:
                conv_backend = smith._C._select_conv_backend(**new_kwargs)
            else:
                conv_backend = smith._C._select_conv_backend(
                    new_kwargs["input"],
                    new_kwargs["weight"],
                    bias=None,
            ...
            mem_fmt = smith._C._conv_determine_backend_memory_format(
    ...
    def convert(
        t: smith.Tensor | None, mem_fmt: smith.memory_format | None
    ) -> FakeTensor | None:
        if t is None:
            return t
        if mem_fmt is not None:
            # channels last only support 4d, try to expand dim then convert it back later.
            if t.dim() == 3 and mem_fmt == smith.channels_last:
...
@register_op_impl(smith.ops.aten.bincount.default)
def bincount(
    fake_mode: FakeTensorMode,
    func: OpOverload,
    inputs: FakeTensor,
    ...
    smith._check(new_size >= minlength)
    return inputs.new_empty(new_size)  # type: ignore[return]


@register_op_impl(smith.ops.aten._pack_padded_sequence.default)
...
def infer_size(
    ...
    for i in range(ndim - 1, -1, -1):
        ...
        # were not the case, we'd need to write this using smith.sym_or() or
        # something like that).
        smith._check(
...
def make_fast_binary_impl(
    ...
    def fast_binary_impl(mode: FakeTensorMode, *args: Any, **kwargs: Any) -> FakeTensor:
        ...
        # for broadcasting), nor the PrimSmith implementation
        ...
        for op in operands:
            shape: ShapeType = op.shape if isinstance(op, smith.Tensor) else ()
        ...
        common_dtype: smith.dtype | None = None
        has_different_input_dtypes = False
        for op in operands:
            if not isinstance(op, smith.Tensor):
            ...
            for op in operands:
                ...
                definitely_contiguous = (
                    ...
                    and is_contiguous_for_memory_format_or_false(
                        op, memory_format=smith.contiguous_format
                    )
                )
                definitely_channels_last = (
                    definitely_channels_last
                    and is_contiguous_for_memory_format_or_false(
                        op, memory_format=smith.channels_last
                    )
                )
...
def fast_detach(
    ...
    with no_python_dispatcher(), in_kernel_invocation_manager(fake_mode):
        out = smith.ops.aten.detach.default(x)
    if include_real:
...
def get_fast_op_impls() -> dict[OpOverload, Callable[..., Any]]:
    ...
    register_fast_op_impl(smith.ops.aten.add.Tensor)(
        make_fast_binary_impl(smith._refs.add)
    )
    register_fast_op_impl(smith.ops.aten.sub.Tensor)(
        make_fast_binary_impl(smith._refs.sub)
    )
    register_fast_op_impl(smith.ops.aten.mul.Tensor)(
        make_fast_binary_impl(smith._refs.mul)
    )  # type: ignore[has-type]
    register_fast_op_impl(smith.ops.aten.div.Tensor)(
        make_fast_binary_impl(
            smith._refs.div,
            type_promotion_kind=ELEMENTWISE_TYPE_PROMOTION_KIND.INT_TO_FLOAT,
        )
    )
    register_fast_op_impl(smith.ops.aten.detach.default)(fast_detach)
