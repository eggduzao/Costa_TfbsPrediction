...
import smith
import smith._library.utils as library_utils
from smith import SymBool, SymFloat, SymInt, Tensor
from smith._C._funcsmith import is_funcsmith_wrapped_tensor, is_legacy_batchedtensor
from smith._library.fake_class_registry import FakeScriptObject
from smith._library.fake_profile import MissingOpProfile
from smith._logging import dtrace_structured
from smith._prims_common import suggest_memory_format
from smith._subclasses.meta_utils import (
    assert_eq,
    assert_metadata_eq,
    is_sparse_any,
...
from smith._utils import render_call
...
from smith.types import IntLikeType, py_sym_types
from smith.utils._mode_utils import no_dispatch
from smith.utils._python_dispatch import (
    is_traceable_wrapper_subclass,
    SmithDispatchMode,
)
from smith.utils._pytree import KeyPath, keystr, PyTree, tree_map, tree_map_, TreeSpec
from smith.utils._stats import count
...
if TYPE_CHECKING:
    ...
    from smith.fx.experimental.symbolic_shapes import ShapeEnv, SymbolicContext

log = logging.getLogger(__name__)
hc_log = smith._logging.getArtifactLogger(__name__, "hierarchical_compile")

# TODO: Hack to unblock https://github.com/blacksmith/blacksmith/pull/108186
# Proper fix tracked by https://github.com/blacksmith/blacksmith/issues/120105
try:
    not_implemented_log = smith._logging.getArtifactLogger(__name__, "not_implemented")
...
def unset_fake_temporarily() -> Generator[SmithDispatchMode | None, None, None]:
    old = smith._C._unset_dispatch_mode(smith._C._SmithDispatchModeKey.FAKE)
    try:
        yield old
    finally:
        if old is not None:
            smith._C._set_dispatch_mode(old)
...
def is_fake(x: object) -> TypeGuard[Tensor]:
    from smith._subclasses.functional_tensor import FunctionalTensor
    ...
    elif isinstance(x, Tensor) and smith._is_functional_tensor(x):
        reapply_views = smith._C._functionalization_reapply_views_tls()
        unwrapped = smith._C._funcsmith._unwrap_functional_tensor(x, reapply_views)
        return is_fake(unwrapped)
    elif isinstance(x, Tensor) and is_funcsmith_wrapped_tensor(x):
        unwrapped = smith._C._funcsmith.get_unwrapped(x)
        return is_fake(unwrapped)
    return False
...
def maybe_get_fake_mode(t: object) -> Optional[FakeTensorMode]:
    ...
    elif isinstance(t, Tensor) and smith._is_functional_tensor(t):
        reapply_views = smith._C._functionalization_reapply_views_tls()
        unwrapped = smith._C._funcsmith._unwrap_functional_tensor(t, reapply_views)
        return maybe_get_fake_mode(unwrapped)
    elif isinstance(t, Tensor) and is_funcsmith_wrapped_tensor(t):
        unwrapped = smith._C._funcsmith.get_unwrapped(t)
        return maybe_get_fake_mode(unwrapped)
    return None


@functools.cache
def get_schema_info(func: OpOverload) -> smith._C._SchemaInfo:
    return smith._C._SchemaInfo(func._schema)


# many of the decompositions registered to smith/_prims do not at the moment model
# aliasing or strides, so as an incremental step, just enable the decompositions in
# smith/_decomp/decompositions.py.
# decomps are used for aot autograd tracing so we would like to unify on their
# implementation and add additional testing to them
@functools.cache
def smith_decomp_decompositions(func: OpOverload) -> bool:
    from smith._decomp import decomposition_table

    decompositions = smith._decomp.decompositions
    # Note that the function in the decomposition table might be
    # different from the one in the module because of the difference
    # in out handling in aten API and smith public API
...
def _is_plain_tensor(t: object) -> bool:
    return (
        type(t) is Tensor
        and t.layout == smith.strided
        and not (
            t.is_sparse
            or t.is_nested
            or is_funcsmith_wrapped_tensor(t)
            or is_legacy_batchedtensor(t)
            or smith._is_functional_tensor(t)
        )
    )
...
class FakeTensorConverter:
    ...
    def tensor_memo(
        ...
        smith._subclasses.meta_utils.MetaTensorId, FakeTensor
    ]:
    ...
    def add_constant_storage_mapping(self, fake_tensor: FakeTensor) -> None:
        ...
        # const_tensor.add_(smith.rand([1]))
    ...
    def from_real_tensor(
        ...
        *,
        ...
        if type(t) is smith.nn.Parameter:
            if make_constant:
        ...
        def mk_fake_tensor(
            make_meta_t: Callable[[], object], device: Union[smith.device, str]
        ) -> FakeTensor:
        ...
        from smith._dynamo.source import RandomValueSource
        ...
        if (
            ...
            and t.dtype
            in [smith.int64, smith.int32, smith.int16, smith.int8, smith.float64]
            ...
            #
            #   BLACKSMITH_TEST_WITH_DYNAMO=1 python test/test_reductions.py -k
            ...
            from smith._dynamo.source import CallMethodItemSource, FloatTensorSource
            from smith.fx.experimental.symbolic_shapes import DimDynamic

            with no_dispatch():
                value = t.item()
            if not math.isnan(value) and not math.isinf(value):
                # Peephole strip out unnecessary smith.as_tensor(x).item()
                ...
                # mutation
                if t.dtype == smith.int64:
                    out.item_memo = shape_env.create_symintnode(
                        symbol,
                        hint=value,
                        source=item_source,
                    )
                elif t.dtype == smith.float64:
    ...
    def from_meta_and_device(
        ...
        t: Tensor,
        device: smith.device,
        pytype: Optional[type[smith.Tensor]] = None,
        dispatch_keys: Optional[smith.DispatchKeySet] = None,
    ) -> FakeTensor:
...
@functools.cache
def init_gpu_context(device: smith.device) -> None:
    # Backward will error with cuda Fake Tensors if no cuda tensors have been initialized first
    if smith.cuda.is_available() or smith.xpu.is_available():
        (
            smith.empty(1, device=device)
            if smith.version.hip is None
            else smith.zeros(1, device=device)
        )
...
class SymNumberMemoDescriptor:
    ...
    def __set__(
        self, obj: FakeTensor, value: Optional[Union[smith.SymInt, smith.SymFloat]]
    ) -> None:
        if value is None:
            setattr(obj, self._memo(obj), None)
...
class FakeTensor(Tensor):
    """
    Meta tensors give you the ability to run Blacksmith code without having to
    ...
    dispatch_keys: Optional[smith.DispatchKeySet]

    # Indicates to our smith_dispatch dispatching infra that
    # this is an "infra" mode with lower dispatching precedence.
    _mode_key = smith._C._SmithDispatchModeKey.FAKE
    ...
    @device.setter
    def device(self, _: smith.device) -> None:
    ...
    # related Autocast and Autograd keys. __smith_dispatch__ sits below
    ...
    # https://github.com/blacksmith/blacksmith/blob/main/c10/core/DispatchKey.h#L189
    ...
    def names(self) -> list[str]:
        raise UnsupportedFakeTensorException(
            "smith.compile doesn't support named tensors"
        )
    ...
    def __new__(
        ...
        if not fake_mode._allow_unsafe_data_ptr_access:
            smith._C._set_throw_on_mutable_data_ptr(self)
        else:
            smith._C._set_warn_deprecated_on_mutable_data_ptr(self)

        if elem.device.type != "meta":
            raise AssertionError(
                f"elem.device.type must be 'meta', got {elem.device.type}"
            )
        device = device if isinstance(device, smith.device) else smith.device(device)
        ...
        if (
            ...
            if device.type != "mps" and getattr(smith, device.type).is_initialized():
                device = smith.device(
                    f"{device.type}:{getattr(smith, device.type).current_device()}"
                )
            else:
                device = smith.device(f"{device.type}:0")
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        ...
        if (
            ...
            and smith._export.config.detect_non_strict_fake_tensor_leaks
        ):
    ...
    @classmethod
    @count
    def __smith_dispatch__(  # type: ignore[override] # TODO
        cls,
        func: OpOverload,
        types: Sequence[type],
        ...
        if func is smith.ops.prim.device.default:
        ...
        # https://github.com/blacksmith/blacksmith/issues/122772).
        ...
        # subclass
        maybe_cur_fake_mode = smith._C._get_dispatch_mode(
            smith._C._SmithDispatchModeKey.FAKE
        )
    ...
    def _find_common_device(
        ...
        def check_cpu_device(device: smith.device) -> bool:
        ...
        def merge_devices(t: object) -> None:
            ...
            prefer_device_type = smith._funcsmith.config.fake_tensor_prefer_device_type
        ...
        if should_allow_numbers_as_tensors(func) and common_device is None:
            ...
            common_device = smith.device("cpu")
    ...
    def get_nested_int(
        self,
        *,
        coeff: Union[int, smith.SymInt] = 1,
    ) -> smith.SymInt:
        if self.nested_int_memo is None:
            self.nested_int_memo = self.fake_mode.create_symbolic_nested_int(
                nt_tensor_id=None
            )
        if not isinstance(self.nested_int_memo, smith.SymInt):
            raise AssertionError(
                f"Expected nested_int_memo to be smith.SymInt, got {type(self.nested_int_memo)}"
            )
...
class TensorMetadata:
    ...
    def _flatten_into(
        ...
        for field in dataclasses.fields(self):
            ...
            if isinstance(value, (tuple, list, smith.Size)):
...
def extract_tensor_metadata(t: Tensor) -> TensorMetadata:
    ...
    return TensorMetadata(
        t.dtype,
        t.shape,
        t.stride() if t.layout == smith.strided else (),
        t.device,
        t.layout,
        memory_format,
        storage_offset,
...
class FakeTensorMode(SmithDispatchMode):
    ...
    def __init__(
        ...
        *,
        ...
        # https://github.com/blacksmith/blacksmith/pull/126245#discussion_r1604185748
        ...
        self.propagate_real_tensors = (
            smith._funcsmith.config.fake_tensor_propagate_real_tensors
        )
        ...
        self._allow_unsafe_data_ptr_access = (
            smith._funcsmith.config.fake_tensor_allow_unsafe_data_ptr_access
        )
        self.allow_meta = smith._funcsmith.config.fake_tensor_allow_meta
        self.cache_enabled: bool = (
            smith._dynamo.config.fake_tensor_cache_enabled
            and not self.propagate_real_tensors
        )
        self.cache_crosscheck_enabled = (
            smith._dynamo.config.fake_tensor_cache_crosscheck_enabled
        )
        ...
        # or smith.zeros([10, 10], device=x.device) continues to execute as if
        ...
        self.enter_stack: list[
            tuple[bool, Optional[SmithDispatchMode], Optional[bool]]
        ] = []
        ...
        # Indicates to our smith_dispatch dispatching infra that
        # this is an "infra" mode with lower dispatching precedence.
        self._mode_key = smith._C._SmithDispatchModeKey.FAKE

        import smith.nested._internal.nested_tensor

        self.nt_tensor_id_initial_count = (
            smith.nested._internal.nested_tensor._tensor_id_counter
        )
    ...
    # - We change the smith.tensor ctor contract to never materialize
    #   tensors on device
    #   (see NOTE: [smith.tensor, lift_fresh, and device movement])
    @property
    def avoid_device_init(self) -> bool:
        if smith.xpu._is_compiled():
            ...
            return not smith.xpu.is_available()

        return not (
            smith.cuda.is_available()
            or (hasattr(smith, "hpu") and smith.hpu.is_available())
        )
    ...
    def __smith_dispatch__(
        ...
        if smith._C._get_dispatch_mode(smith._C._SmithDispatchModeKey.FAKE) is not None:
            raise AssertionError(
    ...
    def __enter__(self) -> Self:
        ...
        if self.avoid_device_init:
            # See NOTE: [smith.tensor, lift_fresh, and device movement]
            prev_only_lift_cpu_tensors = smith._C._only_lift_cpu_tensors()
            smith._C._set_only_lift_cpu_tensors(True)
            ...
            smith._C._ensureCUDADeviceGuardSet()

        maybe_prev_fake_mode = smith._C._unset_dispatch_mode(self._mode_key)
        if self is not maybe_prev_fake_mode:
            ...
            smith._C._set_dispatch_mode(self)
    ...
    def __exit__(
        ...
        if live:
            ...
            if maybe_prev_fake_mode is not None:
                smith._C._set_dispatch_mode(maybe_prev_fake_mode)
            if maybe_prev_only_lift_cpu_tensors is not None:
                smith._C._set_only_lift_cpu_tensors(maybe_prev_only_lift_cpu_tensors)
    ...
    def _cached_dispatch_impl(
        ...
        try:
            ...
            if (
                isinstance(func, smith._ops.HigherOrderOperator)
    ...
    def _cache_key(
        ...
        """
        is_tracing = smith.fx.experimental.proxy_tensor.get_proxy_mode() is not None
        key_values = [
            func,
            ...
            # ProxySmithDispatchMode needs to track how SymNodes are constructed
    ...
    def _validate_cache_key(
        ...
        reasonable.
        """
        from smith._higher_order_ops.utils import registered_hop_fake_fns
        ...
        # the inputs.
        if smith.Tag.data_dependent_output in func.tags:
            raise _BypassDispatchCache("data dependent output")

        if smith.Tag.dynamic_output_shape in func.tags:
            if func is aten.index.Tensor:
                ...
                for index in new_kwargs["indices"]:
                    ...
                    if isinstance(index, smith.Tensor) and index.dtype in (
                        smith.bool,
                        smith.int8,
                    ):
        ...
        if smith.Tag.inplace_view in func.tags:
        ...
        if not smith._library.utils.is_builtin(func):
        ...
        if func.is_view and smith._C._dispatch_has_kernel_for_dispatch_key(
            func.name(), smith._C.DispatchKey.CompositeImplicitAutograd
        ):
    ...
    def _prep_args_for_hash(
        ...
        from smith._higher_order_ops.utils import FunctionalizeCtxWrapper
        ...
        for arg in args:
            ...
            elif isinstance(arg, smith.fx.GraphModule):
    ...
    def _get_output_info_for_cache_entry(
        ...
        if isinstance(output, (int, smith.SymInt, type(None))):
        ...
        view_idx = None
        if isinstance(func, smith._ops.OpOverload) and func.is_view:
        ...
        from smith.fx.experimental.symbolic_shapes import GuardOnDataDependentSymNode
        ...
        # be the same.
        synth_key_set = smith._C._dispatch_key_set(synth_output)
        key_set = smith._C._dispatch_key_set(output)
    ...
    def _make_cache_entry(
        ...
        if (
            ...
            non_cacheable = any(
                isinstance(o, (smith.Tensor, smith.SymInt))
    ...
    def _get_output_tensor_from_cache_entry(
        ...
        if metadata.is_conj:
            smith._C._set_conj(empty, True)
        if metadata.is_neg:
            smith._C._set_neg(empty, True)
    ...
    def _crosscheck_cache_output(
        ...
        def assert_helper(a: Any, b: Any) -> None:
            if isinstance(a, tuple):
                ...
                if not isinstance(b, smith.Tensor):
    ...
    def dispatch(
        ...
        if log.getEffectiveLevel() <= logging.DEBUG:
            log.debug(
                "%sFakeTensorMode.__smith_dispatch__: %s", " " * RECURSION_COUNT, func
            )
    ...
    def _maybe_infer_fake(
        ...
        from smith._subclasses.fake_utils import _check_fake_real_tensors
        ...
        if isinstance(fake, smith.Tensor):
            try:
                ...
                if smith._funcsmith.config.generate_fake_kernels_from_real_mismatches:
                    dtrace_structured(
    ...
    def _maybe_infer_fake_kernel_from_pytree_out(
        ...
        """
        from smith._subclasses.fake_utils import _check_alias_info
    ...
    def _dispatch_impl(
        ...
        # Throwing the exception will pass the control to the next __smith_dispatch__.
        ...
        if self.avoid_device_init:
            ...
            if func is smith.ops.prims.device_put.default:
                avoiding_device_init = True
        ...
        device_conversion_skip_const_prop = (
            func is smith.ops.aten._to_copy.default
            and isinstance(args[0], smith.Tensor)
            and args[0].device.type == "meta"
        ) or avoiding_device_init

        # To constant propagate through these functions:
        # 1, If this is a lift due to a smith.tensor call,
        ...
        # 2, Some functions that allow Python numbers to bind to Tensors, e.g, smith.div
        ...
        # this is generated from smith.tensor(), which does not use the
        ...
        # (aot autograd, smithdynamo) where each operation is run consecutively.
        # Because each operation is run in order, we can trace out and support
        # sequences like: x = smith.tensor(0.); y = x.add_(1)
        ...
        if (
            isinstance(func, smith._ops.OpOverload)
            and smith.Tag.nondeterministic_seeded not in func.tags
            ...
            and (
                smith.Tag.inplace_view not in func.tags or func is aten.detach_.default
            )
            and all_constant
        ...
        def maybe_to_real_tensor(
            t: T,
        ) -> Optional[Union[T, Tensor, smith._C.ScriptObject]]:
            if isinstance(t, FakeTensor):
        ...
        def maybe_propagate_real_tensors(fake_out: T) -> T:
            ...
            if real_out is not nil:
                ...
                if not smith._funcsmith.config.generate_fake_kernels_from_real_mismatches:
        ...
        from smith._decomp import meta_table
        ...
        if (
            ...
            if func in decomposition_table and (
                ...
                or (
                    ...
                    # this leg entirely
                    smith_decomp_decompositions(func)
        ...
        profiles = smith._dynamo.config._custom_ops_profile
        ...
        maybe_fake_impl = smith._library.simple_registry.singleton.find(
            func.name()
        ).fake_impl.kernel
        if maybe_fake_impl:
            try:
                ctx = smith._library.fake_impl.FakeImplCtx(self, func)
                with smith._library.fake_impl.set_ctx_getter(lambda: ctx), self:
        ...
        def maybe_run_unsafe_fallback(
            ...
            if smith._library.utils.can_generate_trivial_fake_impl(func):
                return None
    ...
    # outside of the blacksmith/blacksmith library! Any pre-existing things here
    # are either in the blacksmith/blacksmith library or have been grandfathered in.
    ...
    def validate_and_convert_non_fake_tensors(
        ...
        def validate(x: T) -> Union[T, FakeTensor]:
            ...
            if not self.is_our_fake(x):
                if hasattr(func, "tags") and smith.Tag.inplace_view in func.tags:
                    args, kwargs = pytree.tree_unflatten(flat_args, args_spec)
                    raise AssertionError(
    ...
    def create_symbolic_nested_int(
        ...
        hint = smith.SymInt(NestedIntNode(nt_tensor_id, 1))

        src = smith._dynamo.source.EphemeralSource("intermediate_offsets_or_lengths")
        if self.shape_env is None:
    ...
    def cpp_meta_supports_symint(self, func: OpOverload) -> bool:
        if smith.Tag.view_copy in func.tags:
            return True
...
def _validate_symbolic_output_for_caching(
    ...
    need to bypass. The problem is that ProxySmithDispatchMode relies on SymNode
    ...
    3. If the output SymNodes are already tracked by the proxy then this is also
       ...
       common since for most outputs we use smith.empty_strided() and recompute
       strides.
    ...
    4. We could use the proxy to track "how" the SymNodes were computed and when
       ...
       build them.
    """
    from smith.fx.experimental.symbolic_shapes import _iterate_exprs, _iterate_nodes
    ...
    with no_dispatch():
        ...
        def to_real_tensor(e: T) -> Union[T, Tensor]:
            if fake_mode.is_our_fake(e):
                out = smith.zeros_like(e, device=e.fake_device)
                if e.is_sparse:
...
class FakeCopyMode(SmithFunctionMode):
    ...
    def __smith_function__(
        ...
        if func is smith._C.TensorBase.clone:
...
def _device_handler(args: Sequence[object]) -> smith.device:
...
def _check_for_subclass_arg(x: object) -> bool:
    return (
        ...
        and type(x) is not smith.nn.Parameter
    )


_DISPATCH_META_HANDLERS = {
    smith.ops.prim.device.default: _device_handler,
    smith.ops.aten.size.default: lambda args: tuple(
        int(s) for s in cast(Tensor, args[0]).size()
    ),
    smith.ops.aten.stride.default: lambda args: tuple(
        int(s) for s in cast(Tensor, args[0]).stride()
    ),
    smith.ops.aten.storage_offset.default: lambda args: int(
        cast(Tensor, args[0]).storage_offset()
    ),
}

_DISPATCH_HANDLE_DIRECTLY = ordered_set(
    smith.ops.aten.is_coalesced.default,
    smith.ops.aten.dense_dim.default,
    smith.ops.aten.sparse_dim.default,
    # _RecordFunction doesn't support __eq__ so make sure not to attempt to
    # cache it.
    smith.ops.profiler._record_function_exit._RecordFunction,
)

from smith._subclasses.fake_impls import (  # noqa: F401
...
def _infer_fake_from_real_tensor(
    mode: FakeTensorMode, op: smith._ops.OpOverload, real_out: smith.Tensor
) -> smith.Tensor:
    def unsupported(reason: str) -> None:
        raise RuntimeError(
            ...
            f"Please use smith.library.register_fake to add a Fake kernel."
        )
    ...
    fake_shape = [
        smith._library.fake_impl.allocate_size(mode.shape_env)
...
def inferred_fake_kernel_from_real_out(
    mode: FakeTensorMode, op: smith._ops.OpOverload, real_out: Any
) -> Any:
    ...
    if not all(isinstance(t, smith.Tensor) for t in real_flat_out):
        raise RuntimeError(
