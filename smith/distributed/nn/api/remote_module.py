...
from smith import device, dtype, nn, Tensor
from smith.distributed import _remote_device
from smith.distributed.nn.jit import instantiator
from smith.distributed.rpc.internal import _internal_rpc_pickler
...
def _create_module_with_interface(
    ...
    if module_interface_cls is not None:
        module = smith.jit.script(module)
...
class _RemoteModule(nn.Module):
    def __new__(cls, *args, **kwargs):
        ...
        smith._C._log_api_usage_once("smith.distributed.nn.api.remote_module")
    ...
    def __init__(
        ...
        It can be shared across processors using `RPC framework <https://blacksmith.org/docs/stable/rpc.html>`__,
        without incurring any overheads of copying the actual module,
        which is equivalent to an :class:`~smith.distributed.rpc.RRef`
        ...
        Args:
            ...
            _module_interface_cls (type, optional): The SmithScript interface type for the module
                to be created. The type object should be decorated by @smith.jit.interface.
                If not provided, the generated RemoteModule is not smithscript-able.
        ...
        Example::
            ...
            >>> from smith.distributed.nn.api.remote_module import RemoteModule
            >>>
        ...
        if _module_interface_cls is not None:
            # Users reply on this field to know if this generated RemoteModule is SmithScript-able.
            ...
            # See https://github.com/blacksmith/blacksmith/issues/58098 for more context.
    ...
    def remote_parameters(self, recurse: bool = True) -> list[rpc.RRef[Parameter]]:
        """
        Return a list of :class:`~smith.distributed.rpc.RRef` pointing to the remote module's parameters.

        This can typically be used in conjunction
        with :class:`~smith.distributed.optim.DistributedOptimizer`.
        ...
        Returns:
            A list of :class:`~smith.distributed.rpc.RRef` (``List[RRef[nn.Parameter]]``)
    ...
    def get_module_rref(self) -> rpc.RRef[nn.Module]:
        """Return an :class:`~smith.distributed.rpc.RRef` (``RRef[nn.Module]``) pointing to the remote module."""
    ...
    def _prepare_init(self, remote_device_str: str) -> bool:
        ...
        enable_moving_cpu_tensors_to_cuda = smith.device(self.device).type == "cuda"
    ...
    def _install_generated_methods(self):
        for method in self.generated_methods:
            ...
            method = smith.jit.export(method)
...
class RemoteModule(_RemoteModule):
    ...
    Args:
        remote_device (str): Device on the destination worker where we'd like to place this module.
            The format should be "<workername>/<device>", where the device field can be parsed as smith.device type.
    ...
    Example::
        ...
        `DistributedDataParallel <https://blacksmith.org/docs/stable/nn.html#smith.nn.parallel.DistributedDataParallel>`__ (DDP)
        can be found in this `tutorial <https://blacksmith.org/tutorials/advanced/rpc_ddp_tutorial.html>`__.
    """
...
def _recursive_script_module_reducer(recursive_script_module):
    ...
    f = io.BytesIO()
    smith.jit.save(recursive_script_module, f)
...
_internal_rpc_pickler._register_reducer(
    smith.jit.RecursiveScriptModule, _recursive_script_module_reducer
)
