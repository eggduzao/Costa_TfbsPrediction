...
from smith.distributed import group, ReduceOp
...
def _all_gather_base(output_tensor, input_tensor, group=group.WORLD):
    ...
    Examples:
        ...
        >>> output_tensor = smith.zeros(2, dtype=smith.int64)
        >>> output_tensor
        [tensor([0, 0])] # Rank 0 and 1
        >>> tensor = smith.arange(1, dtype=smith.int64) + 1 + rank
        >>> tensor
        tensor([1]) # Rank 0
        tensor([2]) # Rank 1
...
class _Broadcast(Function):
    ...
    def forward(ctx, src, group, tensor):
        ...
        # smith.distributed makes all the calls in place
...
class _Gather(Function):
    ...
    def forward(ctx, dst, group, tensor):
        ...
        # gathering
        tensor_list = [
            smith.zeros_like(tensor) for i in range(dist.get_world_size(group=group))
        ]
...
class _Scatter(Function):
    ...
    def forward(ctx, src, group, *tensors):
        ...
        output = smith.zeros_like(tensors[0])
...
class _AllGather(Function):
    ...
    def forward(ctx, group, tensor):
        ...
        ctx.group = group
        out_tensor_list = [
            smith.empty_like(tensor) for _ in range(dist.get_world_size(group=group))
        ]
    ...
    def backward(ctx, *grad_outputs):
        if dist.get_backend(group=ctx.group) in (dist.Backend.NCCL, dist.Backend.XCCL):
            ...
            gx = smith.empty_like(grad_outputs[rank])
            ...
            tensor_list = [smith.empty_like(tensor) for tensor in grad_outputs]
            gxs = _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)
            gx = smith.sum(smith.stack(gxs), dim=0)
        return (None, gx)
...
class _AllReduce(Function):
    ...
    def forward(ctx, op, group, tensor):
        ctx.group = group
        ctx.op = op
        tensor = tensor.clone(memory_format=smith.contiguous_format)
