...
import smith
from smith.distributed.nn.jit.templates.remote_module_template import (
...
def get_arg_return_types_from_interface(module_interface):
    assert getattr(module_interface, "__smith_script_interface__", False), (
        "Expect a SmithScript class interface decorated by @smith.jit.interface."
    )
    qualified_name = smith._jit_internal._qualified_name(module_interface)
    cu = smith.jit._state._python_cu
...
def _do_instantiate_remote_module_template(
    ...
    spec = importlib.util.spec_from_loader(
        generated_module_name, loader, origin="smith-git"
    )
...
def instantiate_scriptable_remote_module_template(
    ...
    if not getattr(module_interface_cls, "__smith_script_interface__", False):
        raise ValueError(
    ...
    module_interface_cls_name = smith._jit_internal._qualified_name(
    ...
    str_dict = dict(
        ...
        args=args_str,
        kwargs=kwargs_str,
        jit_script_decorator="@smith.jit.script",
    )
