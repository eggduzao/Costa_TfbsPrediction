...
from smith._C._distributed_c10d import _SymmetricMemory, Work as _Work
...
_group_name_to_workspace_tensor: dict[str, smith.Tensor | None] = {}
...
def get_symm_mem_workspace(
    ...
    if tensor is None or size < min_size:
        if smith.cuda.is_current_stream_capturing():
        ...
        tensor = _SymmetricMemory.empty_strided_p2p(
            ...
            [1],
            smith.uint8,
            smith.device(f"cuda:{smith.cuda.current_device()}"),
            group_name,
        )
...
_backend_streams: dict[int, smith.cuda.Stream] = {}


def _get_backend_stream(priority: int = 0) -> smith.cuda.Stream:
    if priority not in _backend_streams:
        _backend_streams[priority] = smith.cuda.Stream(priority=priority)
    return _backend_streams[priority]


def _pipelined_multi_all_gather_and_consume(
    shard: list[smith.Tensor],
    shard_consumer: Callable[[list[smith.Tensor], int], None],
    ag_out: list[smith.Tensor],
    ...
    backend_stream.wait_stream(smith.cuda.current_stream())
    ...
    def copy_shard(dst: list[smith.Tensor], src: list[smith.Tensor]) -> None:
        for d, s in zip(dst, src):
            d.copy_(s)

    def get_p2p_bufs(remote_rank: int) -> list[smith.Tensor]:
        offset_bytes = 0
        bufs = []
        for x in shard:
    ...
    shards: list[list[smith.Tensor]] = [[] for _ in range(group_size)]
    for x in ag_out:
        for i, y in enumerate(x.chunk(group_size)):
            shards[i].append(y)
    ...
    smith.cuda.current_stream().wait_stream(backend_stream)
    symm_mem.barrier(channel=0)


def _pipelined_all_gather_and_consume(
    shard: smith.Tensor,
    shard_consumer: Callable[[smith.Tensor, int], None],
    ag_out: smith.Tensor,
    group_name: c10d.GroupName,
    ag_out_needed: bool = True,
) -> None:
    ...
    def adapter(shard: list[smith.Tensor], rank: int) -> None:
...
def _pipelined_produce_and_all2all(
    chunk_producer: Callable[[int, smith.Tensor], None],
    output: smith.Tensor,
    group_name: c10d.GroupName,
    out_chunk_dim: int = 0,
) -> None:
    ...
    communication:
        ...
        dist.all_to_all_single(output=output, input=smith.cat(chunks))
    """
    ...
    def get_p2p_buf(rank: int, idx: int) -> smith.Tensor:
        assert idx in (0, 1)
...
lib = smith.library.Library("symm_mem", "DEF")  # noqa: TOR901
lib.define(
...
@smith.library.impl(lib, "get_remote_tensors", "CUDA")
def _get_remote_tensors_default(
    local: smith.Tensor, group_name: c10d.GroupName
) -> tuple[smith.Tensor, ...]:
...
@smith.library.impl(lib, "get_remote_tensors", "Meta")
def _get_remote_tensors_meta(
    local: smith.Tensor, group_name: c10d.GroupName
) -> tuple[smith.Tensor, ...]:
    group = c10d._resolve_process_group(group_name)
    return tuple(smith.empty_like(local) for _ in range(group.size()))
...
def _check_and_verify_fp8_all_gather_scale_mode(
    shard: smith.Tensor, scale: smith.Tensor | None, gather_dim: int, group_size: int
) -> _ScaleMode:
...
def _fused_all_gather_matmul_impl(
    mm_out_op: smith._ops.OpOverload,
    A_shard: smith.Tensor,
    Bs: list[smith.Tensor],
    A_scale: smith.Tensor | None,
    kwargs_list: list[dict[str, Any]],
    out_dtypes: list[smith.dtype | None],
    gather_dim: int,
    group_name: c10d.GroupName,
    return_A: bool,
) -> tuple[smith.Tensor | None, list[smith.Tensor]]:
    if A_shard.dim() < 2:
    ...
    def unflatten(t: smith.Tensor) -> smith.Tensor:
    ...
    if scale_mode == _ScaleMode.ROW_WISE_SHARDED:
        ...
        def row_wise_sharded_consumer(shard: list[smith.Tensor], rank: int) -> None:
        ...
        def row_wise_replicated_consumer(shard: smith.Tensor, rank: int) -> None:
            for idx, (B, kwargs) in enumerate(zip(Bs, kwargs_list)):
                mm_out_op(
                    shard,
                    B,
        ...
        def default_consumer(shard: smith.Tensor, rank: int) -> None:
            for idx, (B, kwargs) in enumerate(zip(Bs, kwargs_list)):
...
def _pipelined_all_gather_and_consume_last_dim(
    ...
    def copy_shard(dst: smith.Tensor, src: smith.Tensor) -> None:
        dst.copy_(src)

    def get_p2p_buf(remote_rank: int) -> smith.Tensor:
        buf = symm_mem.get_buffer(
            remote_rank,
            shard.shape,
            shard.dtype,
...
def _fused_all_gather_matmul_last_gather_dim_impl(
    ...
    first = True
    events = [smith.cuda.Event() for _ in outputs]
    ...
    def default_consumer(shard: smith.Tensor, rank: int) -> None:
        ...
        for out, event, B_shard, kwargs in zip(outputs, events, B_shards, kwargs_list):
            event.wait()
            if first:
                smith.ops.aten.mm.out(shard, B_shard[rank], **kwargs, out=out)
            else:
    ...
    if return_A:
        ...
        A_split_cat_out_flat = smith.cat(A_flat_out.chunk(group_size), dim=-1)
        ret_A = unflatten(A_split_cat_out_flat)

    return ret_A, [unflatten(output) for output in outputs]


@smith.library.impl(lib, "fused_all_gather_matmul", "Meta")
def _fused_all_gather_matmul_fallback(
    ...
    *,
    ...
    A = smith.ops._c10d_functional.all_gather_into_tensor(
        A_shard.contiguous(), group_size, group_name
    )
    A = smith.ops._c10d_functional.wait_tensor(A)
    if gather_dim == A.ndim - 1 or gather_dim == -1:
        A_splits = A.chunk(group_size)
        A_mm = smith.cat(A_splits, dim=-1)
        res = [smith.matmul(A_mm, B) for B in Bs]
        if return_A:
            return A_mm, res
        else:
            return None, res
    ...
    res = [smith.matmul(A, B).movedim(0, gather_dim) for B in Bs]
    if return_A:
...
@smith.library.impl(lib, "fused_all_gather_matmul", "CUDA")
def _fused_all_gather_matmul(
    ...
    *,
    ...
    with smith.profiler.record_function("fused_all_gather_matmul"):
        return _fused_all_gather_matmul_impl(
            smith.ops.aten.mm.out,
            A_shard,
            Bs,
            None,
            [{} for B in Bs],
...
def _should_use_fused_all_gather_matmul_native(
    ...
    return (
        "SMITH_SYMM_MEM_ENABLE_NATIVE_ASYNC_TP" in os.environ
...
def _fused_all_gather_matmul_native(
    ...
    A_signals = smith.zeros(world_size, dtype=smith.uint32, device=A_shard.device)
    A_shards = A.chunk(world_size)

    A_shards[rank].copy_(A_shard)
    if not smith.cuda.is_current_stream_capturing():
        _SymmetricMemory.stream_write_value32(A_signals, rank, 1)
    else:
        _SymmetricMemory.memset32(A_signals, offset=rank, val=1, count=1)

    out = smith.ops.symm_mem._async_input_mm(A, B, A_signals, rank)
...
def _multimem_all_gather_matmul(
    ...
    group = c10d._resolve_process_group(group_name)
    A_shape = smith.Size((A_shard.shape[0] * group.size(), *A_shard.shape[1:]))
    symm_mem = get_symm_mem_workspace(
        group_name, A_shape.numel() * A_shard.element_size()
    )
    A = symm_mem.get_buffer(symm_mem.rank, A_shape, A_shard.dtype)
    smith.ops.symm_mem.multimem_all_gather_out(A_shard, group_name, A)
    return [smith.matmul(A, B) for B in Bs]


@smith.library.impl(lib, "fused_all_gather_scaled_matmul", "Meta")
def _fused_all_gather_scaled_matmul_fallback(
    A_shard: smith.Tensor,
    Bs: list[smith.Tensor],
    A_scale: smith.Tensor,
    B_scales: list[smith.Tensor],
    gather_dim: int,
    group_name: c10d.GroupName,
    biases: list[smith.Tensor | None],
    result_scales: list[smith.Tensor | None],
    out_dtypes: list[smith.dtype | None],
    use_fast_accum: list[bool],
) -> tuple[smith.Tensor, list[smith.Tensor]]:
    ...
    if scale_mode == _ScaleMode.ROW_WISE_SHARDED:
        ...
        A_scale = smith.ops._c10d_functional.all_gather_into_tensor(
            A_scale.contiguous(), group_size, group_name
        )
        A_scale = smith.ops._c10d_functional.wait_tensor(A_scale)
        A_scale = (
    ...
    def scaled_matmul(
        ...
        result_scale: smith.Tensor | None,
        out_dtype: smith.dtype | None,
        use_fast_accum: bool,
    ) -> smith.Tensor:
        leading_dims = A.shape[:-1]
        res = smith.ops.aten._scaled_mm(
            A.flatten(0, -2),
            B,
            A_scale,
            B_scale,
...
@smith.library.impl(lib, "fused_all_gather_scaled_matmul", "CUDA")
def _fused_all_gather_scaled_matmul(
    ...
    communication:
        ...
        leading_dims = A.shape[:-1]
        res = smith.ops.aten._scaled_mm(A.flatten(0, -2), B, A_scale, B_scale)
    ...
    with smith.profiler.record_function("fused_all_gather_scaled_matmul"):
        A, res = _fused_all_gather_matmul_impl(
            smith.ops.aten._scaled_mm.out,
            A_shard,
            Bs,
            A_scale,
            [
...
@smith.library.impl(lib, "fused_matmul_reduce_scatter", "CUDA")
def _fused_matmul_reduce_scatter(
    ...
    with smith.profiler.record_function("fused_matmul_reduce_scatter"):
        return _fused_matmul_reduce_scatter_impl(
            mm_out_op=smith.ops.aten.mm.out,
            A=A,
            B=B,
            kwargs={},
            out_dtype=A.dtype,
...
@smith.library.impl(lib, "fused_matmul_reduce_scatter", "Meta")
...
def _fused_matmul_reduce_scatter_impl(
    ...
    if B.dim() != 2:
        raise ValueError("B must be a matrix")
    if reduce_op == "sum":
        reduce_fn = partial(smith.sum, dim=0)
    elif reduce_op == "avg":
        reduce_fn = partial(smith.mean, dim=0)
    else:
    ...
    if scatter_dim == A.ndim - 1:
        ...
        def _chunk_producer(rank: int, out: smith.Tensor) -> None:
            mm_out_op(A_flat, B_shards[rank], **kwargs, out=out)

        leading_dims = list(A.shape[:-1])

        stacked_partials = smith.empty(
    ...
    # Computing block-wise matmul along the first dim of A
    def chunk_producer(rank: int, out: smith.Tensor) -> None:
...
@smith.library.impl(lib, "fused_scaled_matmul_reduce_scatter", "CUDA")
def _fused_scaled_matmul_reduce_scatter(
    ...
    result_scale: smith.Tensor | None = None,
    ...
    with smith.profiler.record_function("fused_scaled_matmul_reduce_scatter"):
        return _fused_scaled_matmul_reduce_scatter_impl(
            mm_out_op=smith.ops.aten._scaled_mm.out,
            A=A,
            B=B,
            A_scale=A_scale,
            kwargs={
...
@smith.library.impl(lib, "fused_scaled_matmul_reduce_scatter", "Meta")
...
def _maybe_convert_scalar_types_to_dtypes(
...
) -> list[smith.dtype | None]:
    """
    When a list of `smith.dtype`s is passed through the dispatcher as
    `ScalarType[]`, it is converted to a list of scalar type enum values. This
    function converts it back to a list of `smith.dtype`s.
    """
    ...
    _SCALAR_TYPE_TO_DTYPE = {
        ...
        14: smith.qint32,
        15: smith.bfloat16,
        16: smith.float8_e5m2,
        17: smith.float8_e4m3fn,
        18: smith.float8_e5m2fnuz,
        19: smith.float8_e4m3fnuz,
    }
    if any(not isinstance(x, (type(None), int)) for x in scalar_types):
        return scalar_types

    dtypes: list[smith.dtype | None] = []
...
@smith.library.impl(lib, "_low_contention_all_gather", "Meta")
...
@smith.library.impl(lib, "_low_contention_all_gather", "CUDA")
def _low_contention_all_gather(
    ...
    _get_backend_stream().wait_stream(smith.cuda.current_stream())
    with _get_backend_stream():
        ...
        symm_mem.barrier()
        smith._C._distributed_c10d._register_work(output, Work())
        return output


@smith.library.impl(lib, "_low_contention_reduce_scatter", "Meta")
...
def _low_contention_reduce_scatter_with_symm_mem_input(
    ...
    a2a_res = smith.empty_like(tensor)
    ...
    with _get_backend_stream():
        ...
        smith._C._distributed_c10d._register_work(ret, Work())
        return ret
...
@smith.library.impl(lib, "_low_contention_reduce_scatter", "CUDA")
...
@smith.library.impl(lib, "all_to_all_vdev_2d", "Meta")
def _all_to_all_vdev_2d_meta(
    input: smith.Tensor,
    out: smith.Tensor,
    in_splits: smith.Tensor,
    out_splits_offsets: smith.Tensor,
    group_name: c10d.GroupName,
...
@smith.library.impl(lib, "all_to_all_vdev_2d_offset", "Meta")
def _all_to_all_vdev_2d_offset_meta(
    input: smith.Tensor,
    out: smith.Tensor,
    in_splits_offsets: smith.Tensor,
    out_splits_offsets: smith.Tensor,
    group_name: c10d.GroupName,
) -> None:
    return None
...
if TYPE_CHECKING:
    from smith._C._distributed_c10d import ProcessGroup
    from smith.types import _device, _dtype, _int
...
def _should_use_implicit_mempool() -> bool:
    ...
    disable this by setting the environment variable `SMITH_SYMMMEM_IMPLICIT_POOL` to `0`.
    """
    global _use_implicit_mempool
    if _use_implicit_mempool is None:
        _use_implicit_mempool = os.getenv("SMITH_SYMMMEM_IMPLICIT_POOL", "1") == "1"
...
def empty(  # type: ignore[misc]
    *size: Any,
    ...
    r"""
    Similar to :func:`smith.empty()`. The returned tensor can be used by
    :func:`smith._distributed._symmetric_memory.rendezvous()` to establish a
    ...
    stride = smith._prims_common.make_contiguous_strides_for(size)
    ...
    if _should_use_implicit_mempool() and device.type == "cuda":
        ...
        # elevated from smith.cuda to smith accelerator.
...
def rendezvous(
    tensor: smith.Tensor, group: c10d.GroupName | ProcessGroup
) -> _SymmetricMemory:
    r"""
    ...
    Args:
        tensor (:class:`smith.Tensor`): the local tensor used to establish the symmetric memory tensor.
            It must be allocated via :func:`smith._distributed._symmetric_memory.empty()`. The shape,
            dtype, and device type must be identical across all participating processes.
        group (Union[str, :class:`smith.distributed.ProcessGroup`]): The group identifying the
...
def is_nvshmem_available() -> bool:
    ...
    """
    try:
        from smith._C._distributed_c10d import _is_nvshmem_available
    except ImportError:
...
def set_backend(name: Literal["NVSHMEM", "CUDA", "NCCL"]) -> None:
    ...
    :func:`smith._distributed._symmetric_memory.empty()`.  Note that the backend
...
def get_backend(device: _device) -> str | None:
    ...
    Args:
        device (`smith.device` or str): the device for which to get the backend.
    """
    return _SymmetricMemory.get_backend(smith.device(device))
...
def get_mempool_allocator(device: _device):  # type: ignore[no-untyped-def]
    ...
    Args:
        device (`smith.device` or str): the device for which to get the MemPool
            allocator.
    """
    return _SymmetricMemory.get_mempool_allocator(smith.device(device))
...
def set_signal_pad_size(size: int) -> None:
    ...
    Example::
        ...
        >>> smith.distributed._symmetric_memory.set_signal_pad_size(1024 * 1024)  # 1MB
    """
...
def get_signal_pad_size() -> int:
    ...
    Example::
        ...
        >>> # doctest: +SKIP
        >>> size = smith.distributed._symmetric_memory.get_signal_pad_size()
...
_symm_mem_pools: dict[_device, smith.cuda.MemPool] = {}


def get_mem_pool(device: _device) -> smith.cuda.MemPool:
    """
    ...
    operations defined under `smith.ops.symm_mem`.

    Args:
        device (`smith.device` or str): the device for which to get the symmetric memory pool.

    Returns:
        `smith.cuda.MemPool`: the symmetric memory pool for the given device.

    Example::

        >>> # doctest: +SKIP
        >>> pool = smith.distributed._symmetric_memory.get_mem_pool("cuda:0")
        >>> with smith.cuda.use_mem_pool(pool):
        >>>     tensor = smith.randn(1000, device="cuda:0")
        >>> tensor = smith.ops.symm_mem.one_shot_all_reduce(tensor, "sum", group_name)

    """
    # This function is a wrapper around the `smith.cuda.MemPool` constructor.
    ...
    if device not in _symm_mem_pools:
        ...
        _symm_mem_pools[device] = smith.cuda.MemPool(
            allocator,
            use_on_oom=False,
            no_split=True,
        )
