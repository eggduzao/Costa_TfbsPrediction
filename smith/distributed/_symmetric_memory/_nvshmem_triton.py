...
class NvshmemLibFinder:
    ...
    def find_device_library(cls) -> str:
        ...
        try:
            ...
            smith_lib = os.path.join(os.path.dirname(smith.__file__), "lib")
            so_path = os.path.join(smith_lib, "libsmith_nvshmem.so")
                    ...
                    for line in result.stdout.splitlines():
                        if ("RPATH" in line or "RUNPATH" in line) and "[" in line:
                            ...
                            for p in rpath.split(":"):
                                p = p.strip().replace("$ORIGIN", smith_lib)
...
def _nvshmem_init_hook(*args, **kwargs) -> None:  # type: ignore[no-untyped-def]
    ...
    """
    from smith._C._distributed_c10d import _nvshmemx_cumodule_init
...
if has_triton():
    ...
    def reduce_extern_wrapper(
        ...
        Args:
            ...
            dtype: Data type specification - accepts smith.dtype, tl.dtype, str, or constexpr.
            _semantic: Optional semantic information for Triton compilation.
        ...
        Example:
            nvshmem.reduce(0, dest_ptr, src_ptr, 100, "sum", smith.float32)
        """
