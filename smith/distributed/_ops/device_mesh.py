...
@smith.library.register_fake("device_mesh::_runtime_compute_coordinate_on_dim")
def _runtime_compute_coordinate_on_dim_fake(
    full_mesh: smith.Tensor, index: int
) -> SymInt:
    ...
    try:
        ...
        in_dynamo = smith._dynamo.symbolic_convert.InstructionTranslator.current_tx()
...
def _runtime_compute_coordinate_on_dim_impl(full_mesh: smith.Tensor, index: int) -> int:
    rank = smith.distributed.get_rank()
