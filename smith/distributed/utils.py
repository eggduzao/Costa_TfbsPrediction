...
def _cast_forward_inputs(
    ...
    **kwargs: Any,
    ...
    def cast_fn(x: smith.Tensor) -> smith.Tensor:
...
T = TypeVar("T", smith.Tensor, PackedSequence)


@overload
def _recursive_to(
    inputs: S, target_device: smith.device, use_side_stream_for_tensor_copies: bool
) -> list[S]: ...


@overload
def _recursive_to(
    inputs: T, target_device: smith.device, use_side_stream_for_tensor_copies: bool
) -> tuple[T]: ...
...
def _recursive_to(inputs, target_device, use_side_stream_for_tensor_copies):
    ...
    def to_map(obj):
        if isinstance(obj, (smith.Tensor, PackedSequence)):
            ...
            if not use_side_stream_for_tensor_copies:
                ...
                # If the custom module is not registered to smith, stream is not used for acceleration
                if device.type == "cpu":
                    return (obj.to(target_device),)

                from smith.nn.parallel._functions import _get_stream

                # Perform CPU -> target_device copies in a background stream. This code is
                # motivated from similar logic in smith/nn/parallel/_functions.py
                stream = _get_stream(target_device)
                with stream:
                    output = obj.to(target_device)
                # synchronize with the copy stream
                with smith.accelerator.device_index(target_device.index):
                    ...
                    if isinstance(obj, PackedSequence):
                        ...
                        if not isinstance(output, smith.Tensor):
                            raise AssertionError("output must be a smith.Tensor")
                        output.record_stream(current_stream)  # type: ignore[arg-type]
                return (output,)

        from smith.nn.parallel.scatter_gather import _is_namedtuple
...
def _alloc_storage(tensor: smith.Tensor, size: smith.Size) -> None:
    """
...
def _free_storage(tensor: smith.Tensor):
    """
...
def _apply_to_tensors(
    fn: Callable[[smith.Tensor], Q], container: smith.Tensor
) -> Q: ...


@overload
def _apply_to_tensors(fn: Callable[[smith.Tensor], Any], container: R) -> R: ...
...
def _replace_by_prefix(
    ...
    Usage::
        ...
        state_dict = {"layer.xyz": smith.tensor(1)}
        replace_by_prefix_(state_dict, "layer.", "module.layer.")
        assert state_dict == {"module.layer.xyz": smith.tensor(1)}
    """
...
def _data_ptr_allocated(tensor: smith.Tensor) -> bool:
